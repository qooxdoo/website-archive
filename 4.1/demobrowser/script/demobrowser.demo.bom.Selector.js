...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Selector.22681602de1f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;if(qx.module.Traversing.isElement(r)||qx.module.Traversing.isDocument(r)||qx.module.Traversing.isWindow(r)||qx.module.Traversing.isDocumentFragment(r)){this.push(r);}
;return this;}
...
;}
;return B;}
,getAncestors:function(E){return this.__zJ(null,E);}
,getAncestorsUntil:function(G,F){return this.__zJ(G,F);}
,__zJ:function(I,J){var H=[];for(var i=0;i<this.length;i++ ){var parent=qx.dom.Element.getParentElement(this[i]);while(parent){var K=[parent];if(I&&qx.bom.Selector.matches(I,K).length>0){break;}
...
;return be;}
,getNextAll:function(bh){var bg=qx.module.Traversing.__zN(this,f,bh);return qxWeb.$init(bg,qxWeb);}
...
;return bn;}
,getPrevAll:function(bq){var bp=qx.module.Traversing.__zN(this,k,bq);return qxWeb.$init(bp,qxWeb);}
...
,getSiblings:function(bx){var bw=qx.module.Traversing.__zN(this,n,bx);return qxWeb.$init(bw,qxWeb);}
...
,__zK:function(bE){if(bE instanceof qxWeb){return bE[0];}
else if(qx.Bootstrap.isString(bE)){return qxWeb(bE)[0];}
;return bE;}
,isElement:function(bF){return qx.dom.Node.isElement(qx.module.Traversing.__zK(bF));}
,isNode:function(bG){return qx.dom.Node.isNode(qx.module.Traversing.__zK(bG));}
,isNodeName:function(bI,bH){return qx.dom.Node.isNodeName(qx.module.Traversing.__zK(bI),bH);}
,isDocument:function(bJ){return qx.dom.Node.isDocument(bJ);}
,isDocumentFragment:function(bK){return qx.dom.Node.isDocumentFragment(bK);}
,getWindow:function(bL){return qx.dom.Node.getWindow(qx.module.Traversing.__zK(bL));}
,isTextNode:function(bM){return qx.dom.Node.isText(bM);}
,isWindow:function(bN){if(bN instanceof qxWeb){bN=bN[0];}
;return qx.dom.Node.isWindow(bN);}
,getDocument:function(bO){return qx.dom.Node.getDocument(qx.module.Traversing.__zK(bO));}
,getNodeName:function(bP){return qx.dom.Node.getName(qx.module.Traversing.__zK(bP));}
,getNodeText:function(bQ){return qx.dom.Node.getText(qx.module.Traversing.__zK(bQ));}
,isBlockNode:function(bR){return qx.dom.Node.isBlockNode(qx.module.Traversing.__zK(bR));}
,equalNodes:function(cb,cd){cb=qx.module.Traversing.__zL(cb);cd=qx.module.Traversing.__zL(cd);if(!cb||!cd){return false;}
;if(qx.core.Environment.get(a)){return cb.isEqualNode(cd);}
else {if(cb===cd){return true;}
;var ca=cb.attributes&&cd.attributes;if(ca&&cb.attributes.length!==cd.attributes.length){return false;}
;var bY=cb.childNodes&&cd.childNodes;if(bY&&cb.childNodes.length!==cd.childNodes.length){return false;}
;var bX=qx.module.Traversing.EQUALITY_ATTRIBUTES;for(var i=0,l=bX.length;i<l;i++ ){var bW=bX[i];if(cb[bW]!==cd[bW]){return false;}
;}
;if(ca){var bS=qx.module.Traversing.__zM(cb);var cc=qx.module.Traversing.__zM(cd);for(var bU in bS){if(bS[bU]!==cc[bU]){return false;}
;}
;}
;if(bY){for(var j=0,m=cb.childNodes.length;j<m;j++ ){var bV=cb.childNodes[j];var bT=cd.childNodes[j];if(!qx.module.Traversing.equalNodes(bV,bT)){return false;}
;}
;}
;return true;}
;}
,__zL:function(ce){if(typeof ce==q){ce=qxWeb(ce);}
;if(ce instanceof Array||ce instanceof qxWeb){ce=ce[0];}
;return qxWeb.isNode(ce)?ce:null;}
,__zM:function(cf){var cg={};for(var ch in cf.attributes){if(ch==p){continue;}
;var name=cf.attributes[ch].name;var ci=cf.attributes[ch].value;cg[name]=ci;}
;return cg;}
,__zN:function(cl,co,cm){var cj=[];var ck=qx.dom.Hierarchy;for(var i=0,l=cl.length;i<l;i++ ){cj.push.apply(cj,ck[co](cl[i]));}
;var cn=qx.lang.Array.unique(cj);if(cm){cn=qx.bom.Selector.matches(cm,cn);}
;return cn;}
},defer:function(cp){qxWeb.$attach({"add":cp.add,"getChildren":cp.getChildren,"forEach":cp.forEach,"getParents":cp.getParents,"getAncestors":cp.getAncestors,"getAncestorsUntil":cp.getAncestorsUntil,"__zJ":cp.__zJ,"getClosest":cp.getClosest,"find":cp.find,"getContents":cp.getContents,"is":cp.is,"eq":cp.eq,"getFirst":cp.getFirst,"getLast":cp.getLast,"has":cp.has,"getNext":cp.getNext,"getNextAll":cp.getNextAll,"getNextUntil":cp.getNextUntil,"getPrev":cp.getPrev,"getPrevAll":cp.getPrevAll,"getPrevUntil":cp.getPrevUntil,"getSiblings":cp.getSiblings,"not":cp.not,"getOffsetParent":cp.getOffsetParent,"isRendered":cp.isRendered,"isChildOf":cp.isChildOf,"contains":cp.contains});qxWeb.$attachStatic({"isElement":cp.isElement,"isNode":cp.isNode,"isNodeName":cp.isNodeName,"isDocument":cp.isDocument,"isDocumentFragment":cp.isDocumentFragment,"getDocument":cp.getDocument,"getWindow":cp.getWindow,"isWindow":cp.isWindow,"isBlockNode":cp.isBlockNode,"getNodeName":cp.getNodeName,"getNodeText":cp.getNodeText,"isTextNode":cp.isTextNode,"equalNodes":cp.equalNodes});}
});}
)();
...
;}
;var Y,bb=[];for(var i=0,l=X.length;i<l;i++ ){Y=X[i];if(typeof Y===k){Y=this.__dq(Y,bc);}
...
,appendTo:function(parent){parent=qx.module.Manipulating.__zO(parent);for(var i=0,l=parent.length;i<l;i++ ){this._forEachElement(function(r,j){if(i==0){qx.dom.Element.insertEnd(this[j],parent[i]);}
else {qx.dom.Element.insertEnd(this.eq(j).clone(true)[0],parent[i]);}
;}
);}
;return this;}
,insertBefore:function(s){s=qx.module.Manipulating.__zO(s);for(var i=0,l=s.length;i<l;i++ ){this._forEachElement(function(t,u){if(i==0){qx.dom.Element.insertBefore(t,s[i]);}
else {qx.dom.Element.insertBefore(this.eq(u).clone(true)[0],s[i]);}
;}
);}
;return this;}
,insertAfter:function(v){v=qx.module.Manipulating.__zO(v);for(var i=0,l=v.length;i<l;i++ ){for(var j=this.length-1;j>=0;j-- ){if(!this[j]||this[j].nodeType!==1){continue;}
;if(i==0){qx.dom.Element.insertAfter(this[j],v[i]);}
else {qx.dom.Element.insertAfter(this.eq(j).clone(true)[0],v[i]);}
;}
;}
;return this;}
,__zO:function(x){if(!qx.lang.Type.isArray(x)){var w=qxWeb(x);x=w.length>0?w:[x];}
;return x.filter(function(y){return (y&&(y.nodeType===1||y.nodeType===11));}
);}
,wrap:function(z){z=qx.module.Manipulating.__zP(z);if(z.length==0){return this;}
;this._forEachElement(function(A){var B=z.eq(0).clone(true);qx.dom.Element.insertAfter(B[0],A);var C=qx.module.Manipulating.__zQ(B[0]);qx.dom.Element.insertEnd(A,C);}
);return this;}
,__zP:function(F){var D;if(qx.lang.Type.isArray(F)){D=qxWeb(F);}
else {var E=qx.bom.Html.clean([F]);if(E.length>0&&qx.dom.Node.isElement(E[0])){D=qxWeb(E);}
else {D=qxWeb(F);}
;}
;return D;}
,__zQ:function(G){if(G.childNodes.length==0){return G;}
;for(var i=0,l=G.childNodes.length;i<l;i++ ){if(G.childNodes[i].nodeType===1){return this.__zQ(G.childNodes[i]);}
;}
;return G;}
...
;return U.scrollTop;}
,_animationDescription:{scrollLeft:{duration:700,timing:c,keep:100,keyFrames:{'0':{},'100':{scrollLeft:1}}},scrollTop:{duration:700,timing:c,keep:100,keyFrames:{'0':{},'100':{scrollTop:1}}}},__zR:function(W,X,V){var Y=qx.lang.Object.clone(qx.module.Manipulating._animationDescription[W],true);Y.keyFrames[100][W]=X;return this.animate(Y,V);}
,setScrollLeft:function(bb,bc){var Node=qx.dom.Node;if(bc&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__zR.bind(this,a,bb,bc)();}
...
;}
;return this;}
,setScrollTop:function(be,bf){var Node=qx.dom.Node;if(bf&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__zR.bind(this,d,be,bf)();}
...
(function(){var a="text",b="backgroundColor",c="li",d="Number of parent elements of all li elems: ",e="Found ",f="<li>inserted via after()</li>",g=":first-child,:last-child",h=" submit button using filter(function)",i="underline",j="green",k="Number of elements following the UL: ",l=" children of ul/ol elements using children().",m=" headers using find()",n="Found li element with text 'List Item 1': ",o="Tag following the UL element: ",p="color",r="for",s="field-title",t="Number of found li elements without text 'List Item 1': ",u="Number of found li elements in the middle: ",v="<li>inserted via append()</li>",w="<h3>Dynamically Inserted H3</h3>",x="opacity",y="Any H2 with form has a class 'form': ",z="Number of ul/ol elements which are closest to a li element: ",A="li:first-child",B="#detail2",C="li a",D="input",E=" siblings",F="center",G="select",H="h1",I="#eee",J="0.5",K="Number of all parent elements of the submit button: ",L="[type=submit]",M="header",N=" first children of ul/ol elements using children(selector)",O="Tap to follow the link",P="name",Q="List Item 3 has ",R="title",S="href",T='<div style="background:#FFFFBB"></div>',U="<li>inserted via before()</li>",V=":contains('List Item 1')",W="input[type=submit]",X="form",Y="<option>Mr</option><option>Mrs</option><option>Dr</option><option>Prof</option>",bP="Number of elements preceding the UL: ",bQ="href of first link: ",bR="ul",bL="submit",bM="fontWeight",bN="demobrowser.demo.bom.Selector",bO="ul li:contains('List Item 3')",bV="Number of child nodes of the body: ",co="p",bW="<b></b>",cd="bold",bS="p:eq(1)",bT="h1,h2",bU=" li elements in uls using find()",bH="red",ca=" Title ",bK="#detail1,#detail2,#detail3",cb=" submit buttons using filter(selector)",cc="h2",bJ="ul,ol",bX="label[for=field-mail]",cn=":first-child",bY="Tag before the UL element: ",ce="Offset of select box: ",cf="a",cg="label",ck="tap",cl="#field-mail",cm="orange",bI="li:last-child",ch="Typing in e-mail field...",ci="Offset parent of select box: ",cj="id";qx.Class.define(bN,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var ct=qx.log.Logger;q(co).setStyle(p,bH);q(bS).setStyle(bM,cd).setStyle(x,J);q(H).setStyles({"textAlign":F,"textDecoration":i,"color":j});ct.debug(bQ+q(cf).getAttribute(S));q(cf).setAttribute(R,O);q(bT).addClass(M);q(co).on(ck,function(){alert(this.innerHTML);}
);q().concat(q(cc)).setStyle(p,cm).concat(q(c)).setStyle(b,I);ct.debug(e+q(bJ).getChildren().length+l);ct.debug(e+q(bJ).getChildren(cn).length+N);ct.debug(z+q(c).getClosest(bJ).length);ct.debug(bV+q(document.body).getContents().length);var cp=q(bR).find(c);ct.debug(e+cp.length+bU);var cp=q(document.body).find(bT);ct.debug(e+cp.length+m);var cp=q(bR).getNext()[0].tagName;ct.debug(o+cp);var cp=q(bR).getNextAll().length;ct.debug(k+cp);var cp=q(c).getParents().length;ct.debug(d+cp);var cp=q(W).getAncestors().length;ct.debug(K+cp);var cp=q(bR).getPrev()[0].tagName;ct.debug(bY+cp);var cp=q(bR).getPrevAll().length;ct.debug(bP+cp);var cp=q(bO).getSiblings().length;ct.debug(Q+cp+E);var cp=q(G).getOffsetParent()[0].tagName;ct.debug(ci+cp);var cp=q(G).getOffset();ct.debug(ce);ct.debug(cp);q(C).eq(1).setStyle(bM,cd);var cp=q(D).filter(L);ct.debug(e+cp.length+cb);var cp=q(D).filter(function(cv,cx,cw){return cv.type===bL;}
);ct.debug(e+cp.length+h);var cp=q(cc).hasClass(X);ct.debug(y+cp);var cp=q(bR).find(c).is(V);ct.debug(n+cp);var cp=q(bR).find(c).not(V);ct.debug(t+cp.length);var cp=q(bR).find(c).not(g);ct.debug(u+cp.length);q(bJ).append(v);q(A).after(f);q(bI).before(U);var cu=document.createElement(G);var cq=document.createElement(cg);q(cu).setAttribute(cj,s).setAttribute(P,R);q(cq).setAttribute(r,s).setAttribute(a,ca);q(G).after(cu).after(cq);q.create(Y).appendTo(cu);qxWeb.create(w).insertAfter(cc);q(cl).on(D,function(){ct.debug(ch);}
);var cr=q(cl);var cs=cr.add(q(bX)[0]).reverse();cr.after(cs.clone(true));q(bK).wrap(bW);q(B).wrap(T);}
}});}
)();
