...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Placeholder.c466f22ab78e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,updatePlaceholder:function(){if(!qxWeb.env.get(e)){for(var i=0;i<this.length;i++ ){var L=qxWeb(this[i]);var O=L.getAttribute(o);var N=L.getProperty(C);if(!O||(N!=v&&N!=u)){continue;}
;var J=L.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);if(!J){J=qx.module.Placeholder.__IQ(L);}
;var K=L.isRendered();var M=J.isRendered();if(K&&!M){L.before(J);}
else if(!K&&M){J.remove();return this;}
;qx.module.Placeholder.__IP(L);}
;}
;return this;}
,__IP:function(T){var U=T.getAttribute(o);var Q=T.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);var S=T.getStyle(A);var P=parseInt(T.getStyle(p))+2*parseInt(T.getStyle(m));var R=parseInt(T.getStyle(y))+2*parseInt(T.getStyle(E));Q.setHtml(U).setStyles({display:T.getValue()==t?g:z,zIndex:S==j?1:S+1,textAlign:T.getStyle(n),width:(T.getWidth()-P-4)+d,height:(T.getHeight()-R-4)+d,left:T.getPosition().left+d,top:T.getPosition().top+d,fontFamily:T.getStyle(l),fontStyle:T.getStyle(H),fontVariant:T.getStyle(f),fontWeight:T.getStyle(q),fontSize:T.getStyle(c),paddingTop:(parseInt(T.getStyle(y))+2)+d,paddingRight:(parseInt(T.getStyle(m))+2)+d,paddingBottom:(parseInt(T.getStyle(E))+2)+d,paddingLeft:(parseInt(T.getStyle(p))+2)+d});}
,__IQ:function(V){var W=qxWeb.create(b).setStyles({position:G,color:r,overflow:D,pointerEvents:z});V.setProperty(qx.module.Placeholder.PLACEHOLDER_NAME,W);V.on(w,function(X){var Y=X.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);Y.setStyle(x,X.getValue()==t?g:z);}
.bind(this,V));if(!qxWeb.env.get(h)){W.setStyle(k,a).on(F,function(ba){ba.focus();}
.bind(this,V));}
;return W;}
...
(function(){var a="monospace",b="cursive",c="input[type=button]",d="bold",e="",f="px",g="sans-serif",h="css.placeholder",i="<input type='text' placeholder='",j="italic",k="#support",l="demobrowser.demo.bom.Placeholder",m="serif",n="disabled",o="'/><br>",p="tap",r="normal";qx.Class.define(l,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);q(k).setHtml(q.env.get(h)+e);q(c).setAttribute(n,null).on(p,this.addTextField);q.placeholder.update();}
,addTextField:function(){var t=Date.now();var s=q.create(i+t+o).appendTo(document.body);s.setStyles({width:((t%100)+50)+f,padding:(t%20)+f,fontSize:((t%30)+10)+f,fontWeight:t%2?d:r,fontStyle:t%2?j:r,fontFamily:[a,m,g,b][t%4]}).updatePlaceholder();}
}});}
)();
