...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Storage.dc8b9ebe0e77.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="<em>no content</em>",b="Memory Storage",c="html.storage.local",d="html.storage.userdata",e="",f="list",g="save",h="value",i="Web Storage",j="demobrowser.demo.bom.Storage",k="<br>",l="tap",m="clear",n=":",o="using",p="remove",r="key",s="userData Storage";qx.Class.define(j,{extend:qx.application.Native,members:{__cW:null,main:function(){qx.application.Native.prototype.main.call(this);this.setUsing();this.__cW=qx.bom.Storage.getLocal();var v=document.getElementById(g);var t=document.getElementById(m);var u=document.getElementById(p);var self=this;q(v).on(l,function(){var w=qx.bom.Input.getValue(document.getElementById(r));var x=qx.bom.Input.getValue(document.getElementById(h));self.__cW.setItem(w,x);self.updateList();}
);q(t).on(l,function(){self.__cW.clear();self.updateList();}
);q(u).on(l,function(){self.__cW.removeItem(self.__cW.getKey(0));self.updateList();}
);this.updateList();}
,updateList:function(){var y=document.getElementById(f);y.innerHTML=e;this.__cW.forEach(function(z,A){y.innerHTML+=z+n+A+k;}
...
(function(){var a="html.storage.local",b="html.storage.userdata",c="qx.bom.Storage";qx.Bootstrap.define(c,{statics:{__Dx:null,getLocal:function(){if(qx.core.Environment.get(a)){return qx.bom.storage.Web.getLocal();}
...
(function(){var a="local",b="session",c="Storage",d="qx.bom.storage.Web",f="Storage full.";qx.Bootstrap.define(d,{statics:{__Dy:null,__Dz:null,getLocal:function(){if(this.__Dy){return this.__Dy;}
;return this.__Dy=new qx.bom.storage.Web(a);}
,getSession:function(){if(this.__Dz){return this.__Dz;}
;return this.__Dz=new qx.bom.storage.Web(b);}
},construct:function(g){this.__pz=g;}
,members:{__pz:null,getStorage:function(){return window[this.__pz+c];}
,getLength:function(){return this.getStorage(this.__pz).length;}
,setItem:function(h,j){j=qx.lang.Json.stringify(j);try{this.getStorage(this.__pz).setItem(h,j);}
catch(e){throw new Error(f);}
;}
,getItem:function(l){var k=this.getStorage(this.__pz).getItem(l);if(qx.lang.Type.isString(k)){k=qx.lang.Json.parse(k);}
else if(k&&k.value&&qx.lang.Type.isString(k.value)){k=qx.lang.Json.parse(k.value);}
;return k;}
,removeItem:function(m){this.getStorage(this.__pz).removeItem(m);}
,clear:function(){var n=this.getStorage(this.__pz);if(!n.clear){for(var i=n.length-1;i>=0;i-- ){n.removeItem(n.key(i));}
;}
else {n.clear();}
;}
,getKey:function(o){return this.getStorage(this.__pz).key(o);}
,forEach:function(p,r){var length=this.getLength();for(var i=0;i<length;i++ ){var q=this.getKey(i);p.call(r,q,this.getItem(q));}
;}
}});}
)();
(function(){var a="session",b="head",c="none",d="qx",e="qx.bom.storage.UserData",f="div",g="local",h="qxtest",j="#default#userdata",k="display";qx.Bootstrap.define(e,{statics:{__Dy:null,__Dz:null,__ck:0,getLocal:function(){if(this.__Dy){return this.__Dy;}
;return this.__Dy=new qx.bom.storage.UserData(g);}
,getSession:function(){if(this.__Dz){return this.__Dz;}
;return this.__Dz=new qx.bom.storage.UserData(a);}
},construct:function(m){this.__DA=document.createElement(f);this.__DA.style[k]=c;document.getElementsByTagName(b)[0].appendChild(this.__DA);this.__DA.addBehavior(j);this.__DB=m;this.__DA.load(this.__DB);this.__cW={};this.__DC={};var l=this.__DA.getAttribute(d+qx.bom.storage.UserData.__ck);while(l!=undefined){l=qx.lang.Json.parse(l);this.__cW[l.key]=l.value;this.__DC[l.key]=d+qx.bom.storage.UserData.__ck;qx.bom.storage.UserData.__ck++ ;l=this.__DA.getAttribute(d+qx.bom.storage.UserData.__ck);}
;}
,members:{__DA:null,__DB:h,__cW:null,__DC:null,getStorage:function(){return this.__cW;}
,getLength:function(){return Object.keys(this.__cW).length;}
,setItem:function(n,p){if(this.__DC[n]){var o=this.__DC[n];}
else {var o=d+qx.bom.storage.UserData.__ck;qx.bom.storage.UserData.__ck++ ;}
;var q=qx.lang.Json.stringify({key:n,value:p});this.__DA.setAttribute(o,q);this.__DA.save(this.__DB);this.__cW[n]=p;this.__DC[n]=o;}
,getItem:function(r){return this.__cW[r]||null;}
,removeItem:function(v){var u=this.__DC[v];if(u==undefined){return;}
;this.__DA.removeAttribute(u);qx.bom.storage.UserData.__ck-- ;delete this.__cW[v];delete this.__DC[v];var t=d+qx.bom.storage.UserData.__ck;if(this.__DA.getAttribute(t)){var w=this.__DA.getAttribute(d+qx.bom.storage.UserData.__ck);this.__DA.removeAttribute(t);this.__DA.setAttribute(u,w);var s=qx.lang.Json.parse(w).key;this.__DC[s]=u;}
;this.__DA.save(this.__DB);}
,clear:function(){for(var x in this.__DC){this.__DA.removeAttribute(this.__DC[x]);}
;this.__DA.save(this.__DB);this.__cW={};this.__DC={};}
,getKey:function(y){return Object.keys(this.__cW)[y];}
,forEach:function(z,B){var length=this.getLength();for(var i=0;i<length;i++ ){var A=this.getKey(i);z.call(B,A,this.getItem(A));}
;}
}});}
)();
(function(){var a="qx.bom.storage.Memory";qx.Bootstrap.define(a,{statics:{__Dy:null,__Dz:null,getLocal:function(){if(this.__Dy){return this.__Dy;}
;return this.__Dy=new qx.bom.storage.Memory();}
,getSession:function(){if(this.__Dz){return this.__Dz;}
;return this.__Dz=new qx.bom.storage.Memory();}
},construct:function(){this.__cW={};}
,members:{__cW:null,getStorage:function(){return this.__cW;}
,getLength:function(){return Object.keys(this.__cW).length;}
,setItem:function(b,c){c=qx.lang.Json.stringify(c);this.__cW[b]=c;}
,getItem:function(e){var d=this.__cW[e];if(qx.lang.Type.isString(d)){d=qx.lang.Json.parse(d);}
;return d;}
,removeItem:function(f){delete this.__cW[f];}
,clear:function(){this.__cW={};}
,getKey:function(h){var g=Object.keys(this.__cW);return g[h];}
