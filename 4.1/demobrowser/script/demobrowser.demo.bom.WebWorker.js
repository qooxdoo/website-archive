...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.WebWorker.9af438065fd0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="GET",b="error",c="onmessage",d='message',f="message",g="html.webworker",h="var onmessage = null, postMessage = ",i="qx.bom.WebWorker",j=";",k="qx.event.type.Data";qx.Class.define(i,{extend:qx.core.Object,construct:function(l){qx.core.Object.call(this);this.__EC=qx.core.Environment.get(g);this.__EC?this.__EE(l):this.__EF(l);}
,events:{"message":k,"error":k},members:{_worker:null,_handleErrorBound:null,_handleMessageBound:null,__EC:true,__ED:null,__EE:function(m){this._worker=new window.Worker(m);this._handleMessageBound=qx.lang.Function.bind(this._handleMessage,this);this._handleErrorBound=qx.lang.Function.bind(this._handleError,this);qx.bom.Event.addNativeListener(this._worker,f,this._handleMessageBound);qx.bom.Event.addNativeListener(this._worker,b,this._handleErrorBound);}
,__EF:function(src){var that=this;var req=new qx.bom.request.Xhr();req.onload=function(){that.__ED=(function(){var postMessage=function(e){that.fireDataEvent(d,e);}
;eval(h+postMessage+j+req.responseText);return {onmessage:eval(c),postMessage:postMessage};}
)();}
;req.open(a,src,false);req.send();}
,postMessage:function(n){var o=this;if(this.__EC){this._worker.postMessage(n);}
else {setTimeout(function(){try{o.__ED.onmessage&&o.__ED.onmessage({data:n});}
...
},destruct:function(){if(this.__EC){qx.bom.Event.removeNativeListener(this._worker,f,this._handleMessageBound);qx.bom.Event.removeNativeListener(this._worker,b,this._handleErrorBound);if(this._worker){this._worker.terminate();this._worker=null;}
;}
else {if(this.__ED){this.__ED=null;}
;}
;}
});}
)();
(function(){var a="Retry open native request with method: ",b="activex",c="No XHR support available.",d="If-None-Match",f="xhr",g="If-Modified-Since",h="engine.version",i="onunload",j="GET",k="-1",l="qx.debug.io",m="HTMLDocument",n="Open native request with method: ",o="error",p="loadend",q="Blob",r="load",s="Received native readyState: ",t="abort",u=", url: ",v="String",w="browser.documentmode",x="",y="readystatechange",z="Microsoft.XMLHTTP",A="Already disposed",B="browser.version",C="opera",D="qx.bom.request.Xhr",E="on",F="timeout",G="gecko",H="If-Match",I="mshtml",J="engine.name",K="Microsoft.XMLDOM",L="file:",M="Send native request",N="FormData",O="If-Range",P="Content-Type",Q="io.xhr",R="Not enough arguments",S="ArrayBuffer",T="undefined",U=", async: ",V="Native XHR object doesn't support overrideMimeType.";qx.Bootstrap.define(D,{extend:Object,construct:function(){var W=qx.Bootstrap.bind(this.__uB,this);if(qx.event&&qx.event.GlobalError&&qx.event.GlobalError.observeMethod){this.__un=qx.event.GlobalError.observeMethod(W);}
else {this.__un=W;}
;this.__uo=qx.Bootstrap.bind(this.__uA,this);this.__ud=qx.Bootstrap.bind(this.__uF,this);this.__uz();this._emitter=new qx.event.Emitter();if(window.attachEvent){this.__up=qx.Bootstrap.bind(this.__uI,this);window.attachEvent(i,this.__up);}
;}
,statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},events:{"readystatechange":D,"error":D,"loadend":D,"timeout":D,"abort":D,"load":D},members:{readyState:0,responseText:x,responseXML:null,status:0,statusText:x,timeout:0,open:function(bc,X,Y,bb,ba){this.__uK();if(typeof X===T){throw new Error(R);}
else if(typeof bc===T){bc=j;}
;this.__ug=false;this.__uq=false;this.__ur=false;this.__us=X;if(typeof Y==T){Y=true;}
;this.__ut=Y;if(!this.__uJ()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();this.__uz();}
;this.__uv.onreadystatechange=this.__un;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,n+bc+u+X+U+Y);}
;this.__uv.open(bc,X,Y,bb,ba);}
catch(bd){if(!qx.util.Request.isCrossDomain(X)){throw bd;}
;if(!this.__ut){this.__uu=bd;}
;if(this.__ut){if(window.XDomainRequest){this.readyState=4;this.__uv=new XDomainRequest();this.__uv.onerror=qx.Bootstrap.bind(function(){this._emit(y);this._emit(o);this._emit(p);}
,this);if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,a+bc+u+X+U+Y);}
;this.__uv.open(bc,X,Y,bb,ba);return;}
;window.setTimeout(qx.Bootstrap.bind(function(){if(this.__ux){return;}
;this.readyState=4;this._emit(y);this._emit(o);this._emit(p);}
,this));}
;}
;if(qx.core.Environment.get(J)===I&&qx.core.Environment.get(w)<9&&this.__uv.readyState>0){this.__uv.setRequestHeader(g,k);}
;if(qx.core.Environment.get(J)===G&&parseInt(qx.core.Environment.get(h),10)<2&&!this.__ut){this.readyState=qx.bom.request.Xhr.OPENED;this._emit(y);}
;}
,setRequestHeader:function(be,bf){this.__uK();if(be==H||be==g||be==d||be==O){this.__ur=true;}
;this.__uv.setRequestHeader(be,bf);return this;}
,send:function(bh){this.__uK();if(!this.__ut&&this.__uu){throw this.__uu;}
;if(qx.core.Environment.get(J)===C&&this.timeout===0){this.timeout=10000;}
;if(this.timeout>0){this.__uw=window.setTimeout(this.__ud,this.timeout);}
;bh=typeof bh==T?null:bh;var bg=qx.Bootstrap.getClass(bh);bh=(bh!==null&&this.__uy.indexOf(bg)===-1)?bh.toString():bh;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,M);}
;this.__uv.send(bh);}
catch(bj){if(!this.__ut){throw bj;}
;if(this._getProtocol()===L){this.readyState=2;this.__uC();var bi=this;window.setTimeout(function(){if(bi.__ux){return;}
;bi.readyState=3;bi.__uC();bi.readyState=4;bi.__uC();}
);}
;}
;if(qx.core.Environment.get(J)===G&&!this.__ut){this.__uB();}
;this.__uq=true;return this;}
,abort:function(){this.__uK();this.__ug=true;this.__uv.abort();if(this.__uv){this.readyState=this.__uv.readyState;}
;return this;}
...
,getResponseHeader:function(bm){this.__uK();return this.__uv.getResponseHeader(bm);}
,getAllResponseHeaders:function(){this.__uK();return this.__uv.getAllResponseHeaders();}
,overrideMimeType:function(bn){this.__uK();if(this.__uv.overrideMimeType){this.__uv.overrideMimeType(bn);}
else {throw new Error(V);}
;return this;}
,getRequest:function(){return this.__uv;}
,dispose:function(){if(this.__ux){return false;}
;window.clearTimeout(this.__uw);if(window.detachEvent){window.detachEvent(i,this.__up);}
;try{this.__uv.onreadystatechange;}
catch(bp){return false;}
;var bo=function(){}
;this.__uv.onreadystatechange=bo;this.__uv.onload=bo;this.__uv.onerror=bo;this.abort();this.__uv=null;this.__ux=true;return true;}
,isDisposed:function(){return !!this.__ux;}
...
,_getProtocol:function(){var br=this.__us;var bs=/^(\w+:)\/\//;if(br!==null&&br.match){var bt=br.match(bs);if(bt&&bt[1]){return bt[1];}
;}
;return window.location.protocol;}
,__uv:null,__ut:null,__un:null,__uo:null,__up:null,__ud:null,__uq:null,__us:null,__ug:null,__gx:null,__ux:null,__uw:null,__uu:null,__ur:null,__uy:null,__uz:function(){this.__uv=this._createNativeXhr();this.__uv.onreadystatechange=this.__un;if(this.__uv.onabort){this.__uv.onabort=this.__uo;}
;this.__ux=this.__uq=this.__ug=false;this.__uy=[S,q,m,v,N];}
,__uA:function(){if(!this.__ug){this.abort();}
;}
,__uB:function(){var bu=this.__uv,bv=true;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,s+bu.readyState);}
;if(this.readyState==bu.readyState){return;}
;this.readyState=bu.readyState;if(this.readyState===qx.bom.request.Xhr.DONE&&this.__ug&&!this.__uq){return;}
;if(!this.__ut&&(bu.readyState==2||bu.readyState==3)){return;}
...
;if(bv){this.__uG();this.__uH();}
;}
;this.__uC();if(this.readyState==qx.bom.request.Xhr.DONE){if(bu){bu.onreadystatechange=function(){}
;}
;}
;}
,__uC:function(){if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__uw);}
;this._emit(y);if(this.readyState===qx.bom.request.Xhr.DONE){this.__uD();}
;}
,__uD:function(){if(this.__gx){this._emit(F);if(qx.core.Environment.get(J)===C){this._emit(o);}
;this.__gx=false;}
else {if(this.__ug){this._emit(t);}
else {if(this.__uE()){this._emit(o);}
else {this._emit(r);}
;}
;}
;this._emit(p);}
,__uE:function(){var bx;if(this._getProtocol()===L){bx=!this.responseText;}
else {bx=!this.statusText;}
;return bx;}
,__uF:function(){var by=this.__uv;this.readyState=qx.bom.request.Xhr.DONE;this.__gx=true;by.abort();this.responseText=x;this.responseXML=null;this.__uC();}
,__uG:function(){var bz=this.readyState===qx.bom.request.Xhr.DONE;if(this._getProtocol()===L&&this.status===0&&bz){if(!this.__uE()){this.status=200;}
;}
;if(this.status===1223){this.status=204;}
;if(qx.core.Environment.get(J)===C){if(bz&&this.__ur&&!this.__ug&&this.status===0){this.status=304;}
;}
;}
,__uH:function(){if(qx.core.Environment.get(J)==I&&(this.getResponseHeader(P)||x).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var bA=new window.ActiveXObject(K);bA.async=false;bA.validateOnParse=false;bA.loadXML(this.responseText);this.responseXML=bA;}
;}
,__uI:function(){try{if(this){this.dispose();}
;}
catch(e){}
;}
,__uJ:function(){var name=qx.core.Environment.get(J);var bB=qx.core.Environment.get(B);return !(name==I&&bB<9||name==G&&bB<3.5);}
,__uK:function(){if(this.__ux){throw new Error(A);}
;}
...
,_applyOrientation:function(F,C){var D=F===x;var E=D?new qx.ui.layout.HBox():new qx.ui.layout.VBox();var content=this.__nP;content.setLayout(E);content.setAllowGrowX(!D);content.setAllowGrowY(D);this._applySpacing(this.getSpacing());}
,_applySpacing:function(H,G){this.__nP.getLayout().setSpacing(H);}
