...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Flow.c0962ed6c27b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var x=new qx.ui.layout.LineSizeIterator(D,this.getSpacingX());var B=z.top;while(x.hasMoreLines()){var C=x.computeNextLine(A);this.__Gs(C,B,A,z);B+=C.height+this.getSpacingY();}
;}
,__Gs:function(L,K,I,E){var H=qx.ui.layout.Util;var F=E.left;if(this.getAlignX()!=l){F=E.left+I-L.width;if(this.getAlignX()==j){F=E.left+Math.round((I-L.width)/2);}
;}
...
,getHeightForWidth:function(P){return this.__Gt(P).height;}
...
;return S;}
,__Gt:function(T){var U=new qx.ui.layout.LineSizeIterator(this._getLayoutChildren(),this.getSpacingX());var V=0;var W=0;var X=0;while(U.hasMoreLines()){var Y=U.computeNextLine(T);X+=1;W=Math.max(W,Y.width);V+=Y.height;}
;return {width:W,height:V+this.getSpacingY()*(X-1)};}
}});}
)();
(function(){var a="No more lines to compute",b="qx.ui.layout.LineSizeIterator";qx.Class.define(b,{extend:Object,construct:function(d,c){this.__gV=d;this.__DF=c;this.__DG=d.length>0;this.__DH=0;}
,members:{__gV:null,__DF:null,__DG:null,__DH:null,computeNextLine:function(k){var k=k||Infinity;if(!this.__DG){throw new Error(a);}
;var p=this.__gV;var n=0;var f=0;var o=[];var j=[];for(var i=this.__DH;i<p.length;i++ ){var m=p[i];var q=m.getSizeHint();var g=this.__DI(i);var h=q.width+g;var e=i==this.__DH;if(!e&&f+h>k){this.__DH=i;break;}
;var l=q.height+m.getMarginTop()+m.getMarginBottom();o.push(m);j.push(g);f+=h;n=Math.max(n,l);if(m.getLayoutProperties().lineBreak){this.__DH=i+1;break;}
;}
;if(i>=p.length){this.__DG=false;}
;return {height:n,width:f,children:o,gapsBefore:j};}
,__DI:function(r){var s=r==this.__DH;if(s){return this.__gV[r].getMarginLeft();}
else {return Math.max(this.__gV[r-1].getMarginRight(),this.__gV[r].getMarginLeft(),this.__DF);}
;}
,hasMoreLines:function(){return this.__DG;}
}});}
)();
