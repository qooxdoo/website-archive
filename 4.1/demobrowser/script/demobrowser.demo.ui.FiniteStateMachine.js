...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.811bccee8526.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b=" => ",c="#autoActionsBeforeOnexit",d=": Could not find friendly name for '",f=": event '",g=" blocked.  Re-queuing.",h="string",j="'",k="Attempt to transition to nonexistent state ",l="Saved-state stack is empty",m="Internal error: invalid nextState",n=" not handled.  Ignoring.",o="State ",p="#autoActionsBeforeOntransition",q=" returned a value other than ",r="#",s="__He",u=" already exists",v="Attempt to transition to POP_STATE_STACK ",w=":",x="Transition ",y=" does not exist",z="' on '",A="qx.util.fsm.FiniteStateMachine",B="String",C="#autoActionsAfterOntransition",D="",E="#actionsAfterOnentry",F="Saved-state stack is full",G="#autoActionsBeforeOnentry",H=": no transition found.  No state change.",I="__Hf",J="#autoActionsAfterOnentry",K="#actionsBeforeOnentry",L=": Pushed event: ",M="Machine started with no available states",N=": Process event: ",O="#entry",P="#ontransition",Q=" on ",R="Invalid state: not an instance of ",S="Number",T="#exit",U="true, false, or null.",V="Explicit transition ",W="  ",X="qx.util.fsm.State",Y="while state stack is empty.",bj=".  Ignoring.",bk="#autoActionsAfterOnexit",bl=": Event '",bg="object",bh=" not handled for target ",bi=": Queued event: ";qx.Class.define(A,{extend:qx.core.Object,construct:function(bm){qx.core.Object.call(this);this.setName(bm);this.__jv={};this.__Hc=null;this.__Hd=[];this.__He=[];this.__Hf=[];this.__Hg={};this.__Hh={};this.__Hi={};this.__Hj={};this.__Hk={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:B,nullable:true},state:{check:B,nullable:true},previousState:{check:B,nullable:true},nextState:{check:B,nullable:true},maxSavedStates:{check:S,init:2},debugFlags:{check:S,init:7}},members:{__jv:null,__Hc:null,__He:null,__Hf:null,__Hd:null,__Hg:null,__Hh:null,__Hi:null,__Hj:null,__Hk:null,__Hl:false,addState:function(bn){if(!bn instanceof qx.util.fsm.State){throw new Error(R+X);}
;var bo=bn.getName();if(bo in this.__jv){throw new Error(o+bo+u);}
;if(this.__Hc==null){this.__Hc=bo;}
;this.__jv[bo]=bn;}
,replaceState:function(bs,br){if(!bs instanceof qx.util.fsm.State){throw new Error(R+X);}
;var bq=bs.getName();var bp=this.__jv[bq];this.__jv[bq]=bs;if(br){bp._bNeedDispose=true;}
;return bp;}
,addObject:function(bw,bt,bu){var bx=bt.toHashCode();this.__Hh[bw]=bx;this.__Hi[bx]=bw;this.__Hg[bw]=bt;if(!bu){return;}
;if(typeof (bu)==h){bu=[bu];}
;for(var i=0;i<bu.length;i++ ){var bv=bu[i];if(!this.__Hj[bv]){this.__Hj[bv]={};}
;this.__Hj[bv][bw]=true;if(!this.__Hk[bw]){this.__Hk[bw]=[];}
;this.__Hk[bw].push(bv);}
;}
,removeObject:function(bB){var by;var bz;var bA;var bC;by=this.__Hh[bB];if(this.__Hk[bB]){for(var i=0;i<this.__Hk[bB].length;i++ ){bz=this.__Hk[bB][i];delete this.__Hj[bz][bB];bC=true;for(bA in this.__Hj[bz]){bC=false;break;}
;if(bC){delete this.__Hj[bz];}
;}
;delete this.__Hk[bB];}
;delete this.__Hi[by];delete this.__Hh[bB];delete this.__Hg[bB];}
,getObject:function(bD){return this.__Hg[bD];}
,getFriendlyName:function(bE){var bF=bE?bE.toHashCode():null;return bF?this.__Hi[bF]:null;}
,getGroupObjects:function(bG){var a=[];for(var name in this.__Hj[bG]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bH in this.__Hh){var bJ=this.__Hh[bH];var bI=this.getObject(bH);this.debug(bH+b+bJ);this.debug(W+bJ+b+this.__Hi[bJ]);this.debug(W+bH+b+this.getObject(bH));this.debug(W+this.getObject(bH)+b+this.getFriendlyName(bI));}
;}
,_getInternalData:function(){return ({"states":this.__jv,"startState":this.__Hc,"eventQueue":this.__He,"blockedEvents":this.__Hf,"savedStates":this.__Hd,"friendlyToObject":this.__Hg,"friendlyToHash":this.__Hh,"hashToFriendly":this.__Hi,"groupToFriendly":this.__Hj,"friendlyToGroups":this.__Hk});}
,start:function(){var bL=this.__Hc;if(bL==null){throw new Error(M);}
;this.setState(bL);this.setPreviousState(null);this.setNextState(null);var bK=(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL);if(bK){this.debug(this.getName()+r+bL+K);}
;this.__jv[bL].getAutoActionsBeforeOnentry()(this);if(bK){this.debug(this.getName()+r+bL+O);}
;this.__jv[bL].getOnentry()(this,null);if(bK){this.debug(this.getName()+r+bL+E);}
;this.__jv[bL].getAutoActionsAfterOnentry()(this);}
,pushState:function(bM){if(this.__Hd.length>=this.getMaxSavedStates()){throw new Error(F);}
;if(bM===true){this.__Hd.push(this.getState());}
else if(bM){this.__Hd.push(bM);}
else {this.__Hd.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Hd.length==0){throw new Error(l);}
;return this.__Hd.pop();}
,postponeEvent:function(event){this.__Hf.unshift(event);}
,enqueueEvent:function(event,bN){if(bN){this.__He.push(event);}
else {this.__He.unshift(event);}
;if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){var bO=this.getFriendlyName(event.getTarget());if(bN){this.debug(this.getName()+L+event.getType()+(bO?Q+bO:D));}
else {this.debug(this.getName()+bi+event.getType()+(bO?Q+bO:D));}
;}
;}
,eventListener:function(event){var e=event.clone();this.enqueueEvent(e,false);this.__Hm();}
...
,this,bV||20);}
,__Hm:function(){if(this.__Hl){return;}
;this.__Hl=true;while(this.__He.length>0){var event=this.__He.pop();var bW=this.__Hn(event);if(bW){event.dispose();}
;}
;this.__Hl=false;}
,__Hn:function(event){var ca;var cb;var bY;var cd;var cg;var e;var cf;var bX=this.getDebugFlags();var ck=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var ci=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var ch=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var ce=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var cj=this.getFriendlyName(event.getTarget());if(ck){this.debug(this.getName()+N+event.getType()+(cj?Q+cj:D));}
;ca=this.getState();cd=this.__jv[ca];cg=cd.transitions;e=cd.getEvents()[event.getType()];if(!e){if(ck){this.debug(this.getName()+bl+event.getType()+j+n);}
;return true;}
;if(typeof (e)==bg){if(!cj){if(ce){this.debug(this.getName()+d+event.getType()+z+event.getTarget()+j);}
;return true;}
...
;this.__Hf.unshift(event);return false;default:if(typeof (cf)==h){if(cg[cf]){var cc=cg[cf];cg={};cg[cf]=cc;}
else {throw new Error(V+cf+y);}
;break;}
;};for(var t in cg){var cc=cg[t];switch(cc.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(x+ca+w+t+q+U);};cb=cc.getNextState();if(typeof (cb)==h){if(!cb in this.__jv){throw new Error(k+cb);}
;this.setNextState(cb);}
else {switch(cb){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cb=ca;this.setNextState(cb);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Hd.length==0){throw new Error(v+Y);}
;cb=this.__Hd.pop();this.setNextState(cb);break;default:throw new Error(m);};}
;if(ch){this.debug(this.getName()+r+ca+r+t+p);}
;cc.getAutoActionsBeforeOntransition()(this);if(ch){this.debug(this.getName()+r+ca+r+t+P);}
;cc.getOntransition()(this,event);if(ch){this.debug(this.getName()+r+ca+r+t+C);}
...
},properties:{name:{transform:x,nullable:true},onentry:{transform:bo,nullable:true,init:function(bG,event){}
},onexit:{transform:m,nullable:true,init:function(bH,event){}
...
},autoActionsBeforeOnexit:{transform:M,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:s,nullable:true,init:function(bL,event){}
},events:{transform:K,nullable:true}},members:{__GS:function(bM){if(typeof (bM)!=t||bM.length<1){throw new Error(y);}
;return bM;}
,__GT:function(bN){switch(typeof (bN)){case X:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(r));default:throw new Error(T+typeof (bN));};}
,__GU:function(bP){switch(typeof (bP)){case X:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(r));default:throw new Error(p+typeof (bP));};}
,__GV:function(bT){if(typeof (bT)!=bB){throw new Error(bc);}
...
,__Hb:function(f,S){if(typeof (f)==b){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
