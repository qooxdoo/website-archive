...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.CommandGroupManager.73b165d6148f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="focusin",b="If focused here, all commands will be disabled! Please press key \"5\"!",c="focusout",d="Page3 - press 5 to change color",f="#ABEFEF",g="Page1 - press 5 to change color",h="Page2 - press 5 to change color",i="All tabview pages are holding a view class with same command shortcut! Press key  \"5\" on any page to change the color of the view. You will see that only the appeared page will change his color.",j="appear",k="demobrowser.demo.ui.CommandGroupManager",l="toggleColor",m="#ABEFAB",n="5",o="ColorSwitch",p="execute";qx.Class.define(k,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);this._manager=new qx.ui.command.GroupManager();this._createWidgets();}
,_createWidgets:function(){var q=new qx.ui.tabview.TabView();var t=new qx.ui.tabview.Page(g);t.setLayout(new qx.ui.layout.Canvas());var r=new qx.ui.tabview.Page(h);r.setLayout(new qx.ui.layout.Canvas());var s=new qx.ui.tabview.Page(d);s.setLayout(new qx.ui.layout.Canvas());t.add(new ColorSwitch(this),{edge:0});r.add(new ColorSwitch(this),{edge:0});s.add(new ColorSwitch(this),{edge:0});q.add(t);q.add(r);q.add(s);this.getRoot().add(q,{edge:10});}
,getGroupManager:function(){return this._manager;}
}});qx.Class.define(o,{extend:qx.ui.container.Composite,construct:function(u){qx.ui.container.Composite.call(this);this.setLayout(new qx.ui.layout.VBox(15));this.setPadding(25);this._controller=u;var v=new qx.ui.command.Command(n);v.addListener(p,this.toggleColor,this);var w=new qx.ui.command.Group();this._group=w;w.add(l,v);u.getGroupManager().add(w);this.addListener(j,this._onAppear,this);this._createWidgets();}
,members:{_group:null,_createWidgets:function(){var x=new qx.ui.form.TextField();x.setPlaceholder(b);x.addListener(a,this._blockCommands,this);x.addListener(c,this._unblockCommands,this);this.add(x);var y=new qx.ui.basic.Label(i);y.set({rich:true,wrap:true});this.add(y);}
,toggleColor:function(A,z){this.setBackgroundColor(this.getBackgroundColor()==f?m:f);}
,_onAppear:function(e){this._controller.getGroupManager().setActive(this._group);}
,_blockCommands:function(e){this._controller.getGroupManager().block();}
,_unblockCommands:function(e){this._controller.getGroupManager().unblock();}
}});}
)();
(function(){var a="qx.ui.command.GroupManager",b="Group must be an instance of qx.ui.command.Group",c="qx.debug",d="Group was not added before. Please use ",e="'add()' method to add the group.",f="Given group is not an instance of qx.ui.command.Group",g="Group was not added before! You have to use 'addCommand()' method before activating!",h="Group is already added!";qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__vo=[];}
,members:{__vo:null,__IT:null,add:function(j){if(qx.core.Environment.get(c)){this.assertInstance(j,qx.ui.command.Group,f);}
;if(qx.lang.Array.contains(this.__vo,j)){if(qx.core.Environment.get(c)){this.debug(h);}
;return false;}
;this.__vo.push(j);j.setActive(false);return true;}
...
;var l=this.__vo.indexOf(m);if(l===-1){if(qx.core.Environment.get(c)){this.debug(d+e);}
;}
;if(this.getActive()===m){this.__IT=null;}
;this.__vo.splice(l,1);return m;}
...
;return false;}
;for(var i=0;i<this.__vo.length;i++ ){var n=this.__vo[i];if(n==o){n.setActive(true);this.__IT=n;continue;}
;n.setActive(false);}
;return true;}
,getActive:function(){return this.__IT;}
,block:function(){if(this.__IT){this.__IT.setActive(false);}
;}
,unblock:function(){if(this.__IT){this.__IT.setActive(true);}
;}
,_getGroup:function(q){var p=this.__vo.indexOf(q);if(p===-1){return null;}
;return this.__vo[p];}
},destruct:function(){this.__vo=this.__IT=null;}
});}
)();
...
,properties:{active:{init:true,check:b,apply:g}},members:{_cmds:null,_applyActive:function(o){for(var n in this._cmds){this._cmds[n].setActive(o);}
;}
,add:function(q,p){if(qx.core.Environment.get(f)){this.assertArgumentsCount(arguments,2,2,a+m);this.assertString(q,j);this.assertInstance(p,qx.ui.command.Command,k);}
;if(this.has(q)){if(qx.core.Environment.get(f)){this.debug(l+q+e);}
;return false;}
;this._cmds[q]=p;return true;}
,get:function(s){if(qx.core.Environment.get(f)){this.assertString(s,j);}
;var r=this._cmds[s];if(!r){if(qx.core.Environment.get(f)){this.debug(d+s+c+h);}
;return null;}
;return r;}
,has:function(t){if(qx.core.Environment.get(f)){this.assertString(t,j);}
;return !!(this._cmds[t]);}
,remove:function(v){if(qx.core.Environment.get(f)){this.assertString(v,j);}
;var u=this._cmds[v];if(!u){if(qx.core.Environment.get(f)){this.debug(d+v+c+h);}
;return null;}
;delete this._cmds[v];return u;}
},destruct:function(){this._cmds=null;}
});}
)();
...
(function(){var a="bottom",b="button",c="qx.event.type.Data",d="lastTab",f="barRight",g="beforeChangeSelection",h="qx.debug",j="__nT",k="horizontal",m="changeSelection",n="bar",o="close-button",p="Index should be less than : ",q="tabview",r="vertical",s="page",t="Incompatible child for TabView: ",u="close",v="_applyBarPosition",w="barLeft",x="top",y="right",z="qx.ui.tabview.TabView",A="barTop",B="barBottom",C="pane",D="left",E="firstTab";qx.Class.define(z,{extend:qx.ui.core.Widget,implement:qx.ui.core.ISingleSelection,include:[qx.ui.core.MContentPadding],construct:function(G){qx.ui.core.Widget.call(this);this.__nS={top:A,right:f,bottom:B,left:w};this._createChildControl(n);this._createChildControl(C);var F=this.__nT=new qx.ui.form.RadioGroup;F.setWrap(false);F.addListener(m,this._onChangeSelection,this);if(G!=null){this.setBarPosition(G);}
else {this.initBarPosition();}
;}
,events:{"changeSelection":c},properties:{appearance:{refine:true,init:q},barPosition:{check:[D,y,x,a],init:x,apply:v}},members:{__nT:null,_createChildControlImpl:function(J,I){var H;switch(J){case n:H=new qx.ui.container.SlideBar();H.setZIndex(10);this._add(H);break;case C:H=new qx.ui.container.Stack;H.setZIndex(5);this._add(H,{flex:1});break;};return H||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,J);}
,_getContentPaddingTarget:function(){return this.getChildControl(C);}
,add:function(O){if(qx.core.Environment.get(h)){if(!(O instanceof qx.ui.tabview.Page)){throw new Error(t+O);}
;}
;var K=O.getButton();var L=this.getChildControl(n);var N=this.getChildControl(C);O.exclude();L.add(K);N.add(O);this.__nT.add(K);O.addState(this.__nS[this.getBarPosition()]);O.addState(d);var M=this.getChildren();if(M[0]==O){O.addState(E);}
else {M[M.length-2].removeState(d);}
;O.addListener(u,this._onPageClose,this);}
,addAt:function(U,P){if(qx.core.Environment.get(h)){if(!(U instanceof qx.ui.tabview.Page)){throw new Error(t+U);}
;}
;var S=this.getChildren();if(!(P==null)&&P>S.length){throw new Error(p+S.length);}
;if(P==null){P=S.length;}
;var R=U.getButton();var Q=this.getChildControl(n);var T=this.getChildControl(C);U.exclude();Q.addAt(R,P);T.addAt(U,P);this.__nT.add(R);U.addState(this.__nS[this.getBarPosition()]);S=this.getChildren();if(P==S.length-1){U.addState(d);}
...
;}
;W.remove(X);ba.remove(Y);this.__nT.remove(X);Y.removeState(this.__nS[this.getBarPosition()]);if(Y.hasState(E)){Y.removeState(E);if(bb[0]){bb[0].addState(E);}
;}
...
,__nS:null,_applyBarPosition:function(bg,bi){var bd=this.getChildControl(n);var bj=this.getChildControl(C);var bl=bg==D||bg==y;var bm=bg==y||bg==a;var bk=bl?qx.ui.layout.HBox:qx.ui.layout.VBox;var bn=this._getLayout();if(bn&&bn instanceof bk){}
else {this._setLayout(bn=new bk);}
;bn.setReversed(bm);bd.setOrientation(bl?r:k);var bh=this.getChildren();if(bi){var bf=this.__nS[bi];bd.removeState(bf);bj.removeState(bf);for(var i=0,l=bh.length;i<l;i++ ){bh[i].removeState(bf);}
;}
;if(bg){var be=this.__nS[bg];bd.addState(be);bj.addState(be);for(var i=0,l=bh.length;i<l;i++ ){bh[i].addState(be);}
;}
;}
,getSelection:function(){var bo=this.__nT.getSelection();var bp=[];for(var i=0;i<bo.length;i++ ){bp.push(bo[i].getUserData(s));}
;return bp;}
,setSelection:function(br){var bq=[];for(var i=0;i<br.length;i++ ){bq.push(br[i].getChildControl(b));}
;this.__nT.setSelection(bq);}
,resetSelection:function(){this.__nT.resetSelection();}
,isSelected:function(bs){var bt=bs.getChildControl(b);return this.__nT.isSelected(bt);}
,isSelectionEmpty:function(){return this.__nT.isSelectionEmpty();}
,getSelectables:function(bv){var bu=this.__nT.getSelectables(bv);var bw=[];for(var i=0;i<bu.length;i++ ){bw.push(bu[i].getUserData(s));}
;return bw;}
...
},destruct:function(){this._disposeObjects(j);this.__nS=null;}
});}
)();
...
,__lM:function(e){var m=e.getOldData()[0];var n=e.getData()[0];if(m){if(this.isDynamic()){m.exclude();}
else {m.hide();}
;}
;if(n){n.show();}
;}
