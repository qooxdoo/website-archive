...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.VSplit.dae6f3fd8e2d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="slider",b="splitter",c="px",d="Integer",f="height",g="move",h="maxHeight",i="resize",j="pointerup",k="vertical",l="width",m="_applyOrientation",n="_applyOffset",o="splitpane",p="qx.ui.splitpane.Pane",q="pointerdown",r="top",s="minHeight",t="knob",u="horizontal",v="minWidth",w="appear",z="losecapture",A="left",B="pointermove",C="maxWidth",D="pointerout";qx.Class.define(p,{extend:qx.ui.core.Widget,construct:function(E){qx.ui.core.Widget.call(this);this.__gV=[];if(E){this.setOrientation(E);}
else {this.initOrientation();}
;this.__lm.addListener(q,this._onPointerDown,this);this.__lm.addListener(j,this._onPointerUp,this);this.__lm.addListener(B,this._onPointerMove,this);this.__lm.addListener(D,this._onPointerOut,this);this.__lm.addListener(z,this._onPointerUp,this);}
,properties:{appearance:{refine:true,init:o},offset:{check:d,init:6,apply:n},orientation:{init:u,check:[u,k],apply:m}},members:{__sO:null,__sP:false,__sQ:null,__sR:null,__mX:null,__sS:null,__sT:null,__gV:null,__lm:null,_createChildControlImpl:function(H,G){var F;switch(H){case a:F=new qx.ui.splitpane.Slider(this);F.exclude();this._add(F,{type:H});break;case b:F=new qx.ui.splitpane.Splitter(this);this._add(F,{type:H});F.addListener(g,this.__sU,this);break;};return F||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,H);}
,__sU:function(e){this.__sW(e.getData());}
,__sV:function(K){this.__lm=new qx.ui.splitpane.Blocker(K);this.getContentElement().add(this.__lm);var I=this.getChildControl(b);var J=I.getWidth();if(!J){I.addListenerOnce(w,function(){this.__sW();}
,this);}
;I.addListener(i,function(e){var L=e.getData();if(L.height==0||L.width==0){this.__lm.hide();}
else {this.__lm.show();}
;}
,this);}
,getBlocker:function(){return this.__lm;}
,_applyOrientation:function(Q,N){var R=this.getChildControl(a);var M=this.getChildControl(b);this.__mX=Q===u;if(!this.__lm){this.__sV(Q);}
;this.__lm.setOrientation(Q);var P=this._getLayout();if(P){P.dispose();}
;var O=Q===k?new qx.ui.splitpane.VLayout:new qx.ui.splitpane.HLayout;this._setLayout(O);M.removeState(N);M.addState(Q);M.getChildControl(t).removeState(N);M.getChildControl(t).addState(Q);R.removeState(N);R.addState(Q);qx.ui.core.queue.Manager.flush();this.__sW();}
,_applyOffset:function(T,S){this.__sW();}
,__sW:function(V){var U=this.getChildControl(b);var ba=this.getOffset();var bb=U.getBounds();var Y=U.getContentElement().getDomElement();if(!Y){return;}
;if(this.__mX){var X=null;if(V){X=V.width;}
else if(bb){X=bb.width;}
;var bc=V&&V.left;if(X){if(isNaN(bc)){bc=qx.bom.element.Location.getPosition(Y).left;}
;this.__lm.setWidth(ba,X);this.__lm.setLeft(ba,bc);}
;}
...
;this.__lm.setHeight(ba,W);this.__lm.setTop(ba,top);}
;}
;}
...
,_onPointerDown:function(e){if(!e.isLeftPressed()){return;}
;var bg=this.getChildControl(b);var bj=bg.getContentLocation();var bh=this.getContentLocation();this.__sO=this.__mX?e.getDocumentLeft()-bj.left+bh.left:e.getDocumentTop()-bj.top+bh.top;var bi=this.getChildControl(a);var bk=bg.getBounds();bi.setUserBounds(bk.left,bk.top,bk.width,bk.height);bi.setZIndex(bg.getZIndex()+1);bi.show();this.__sP=true;this.__lm.capture();e.stop();}
,_onPointerMove:function(e){this._setLastPointerPosition(e.getDocumentLeft(),e.getDocumentTop());if(this.__sP){this.__sX();var bl=this.getChildControl(a);var bm=this.__sS;if(this.__mX){bl.setDomLeft(bm);this.__lm.setStyle(A,(bm-this.getOffset())+c);}
else {bl.setDomTop(bm);this.__lm.setStyle(r,(bm-this.getOffset())+c);}
;e.stop();}
;}
,_onPointerOut:function(e){this._setLastPointerPosition(e.getDocumentLeft(),e.getDocumentTop());}
,_onPointerUp:function(e){if(!this.__sP){return;}
;this._finalizeSizes();var bn=this.getChildControl(a);bn.exclude();this.__sP=false;this.releaseCapture();e.stop();}
,_finalizeSizes:function(){var br=this.__sS;var bo=this.__sT;if(br==null){return;}
...
else {if(this.__mX){bs.setWidth(br);bq.setWidth(bo);}
else {bs.setHeight(br);bq.setHeight(bo);}
;}
;}
,__sX:function(){if(this.__mX){var bx=v,bE=l,by=C,bC=this.__sQ;}
else {var bx=s,bE=f,by=h,bC=this.__sR;}
;var bD=this._getChildren();var bv=bD[2].getSizeHint();var bA=bD[3].getSizeHint();var bB=bD[2].getBounds()[bE]+bD[3].getBounds()[bE];var bz=bC-this.__sO;var bw=bB-bz;if(bz<bv[bx]){bw-=bv[bx]-bz;bz=bv[bx];}
else if(bw<bA[bx]){bz-=bA[bx]-bw;bw=bA[bx];}
;if(bz>bv[by]){bw+=bz-bv[by];bz=bv[by];}
else if(bw>bA[by]){bz+=bw-bA[by];bw=bA[by];}
;this.__sS=bz;this.__sT=bw;}
,_isActiveDragSession:function(){return this.__sP;}
,_setLastPointerPosition:function(x,y){this.__sQ=x;this.__sR=y;}
