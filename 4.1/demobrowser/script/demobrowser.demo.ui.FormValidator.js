...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.FormValidator","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormValidator.1097643000d3.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="You can send...",b="All fields are required. Some by a custom validator, some by a ",c="password",d="complete",e="\n",f="Please enter a password at with least 3 characters.",g="username",h="Server said no!",i="Passwords must be equal.",j="<b>Client side form validation</b><br/>",k="Send",l="Validating...",m="email address",n="demobrowser.demo.ui.FormValidator",o="Accept",p="execute",q="predefined validator and some by the required flag.";qx.Class.define(n,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var r=new qx.ui.form.TextField();r.setPlaceholder(g);r.setWidth(150);r.setRequired(true);this.getRoot().add(r,{left:20,top:70});var A=new qx.ui.form.TextField();A.setPlaceholder(m);A.setWidth(150);this.getRoot().add(A,{left:20,top:100});var s=new qx.ui.form.TextField();s.setPlaceholder(c);s.setWidth(150);this.getRoot().add(s,{left:20,top:130});var t=new qx.ui.form.TextField();t.setPlaceholder(c);t.setWidth(150);this.getRoot().add(t,{left:20,top:160});var z=new qx.ui.form.CheckBox(o);z.setRequired(true);this.getRoot().add(z,{left:20,top:190});var w=new qx.ui.form.validation.Manager();var u=function(D,B){var C=D!=null&&D.length>2;if(!C){B.setInvalidMessage(f);}
;return C;}
;var x=new qx.ui.form.validation.AsyncValidator(function(E,F){window.setTimeout(function(){if(F==null||F.length==0){E.setValid(false,h);}
else {E.setValid(true);}
;}
,1000);}
);w.add(r,x);w.add(A,qx.util.Validate.email());w.add(s,u);w.add(t,u);w.add(z);w.setValidator(function(G){var I=s.getValue()==t.getValue();if(!I){var H=i;s.setInvalidMessage(H);t.setInvalidMessage(H);s.setValid(false);t.setValid(false);}
;return I;}
);var v=new qx.ui.form.Button(k);this.getRoot().add(v,{left:20,top:215});v.addListener(p,function(){v.setEnabled(false);v.setLabel(l);w.validate();}
,this);w.addListener(d,function(){v.setEnabled(true);v.setLabel(k);if(w.getValid()){alert(a);}
else {alert(w.getInvalidMessages().join(e));}
;}
,this);var y=new qx.ui.basic.Label();y.setRich(true);y.setWidth(400);y.setValue(j+b+q);this.getRoot().add(y,{left:20,top:10});}
}});}
)();
