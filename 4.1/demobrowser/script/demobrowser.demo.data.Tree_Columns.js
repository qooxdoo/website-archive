...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Tree_Columns.167ef341a328.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,_applyTarget:function(I,H){if(H!=undefined){this.__CO(H);}
...
;this.__CO();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.__xy(this.getModel(),bg);this.__Je(bg,this.getModel());this.getTarget().setRoot(bg);}
;}
...
;}
,__CO:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__Jf(bv);var bu=bv.getModel();if(bu){this.__mQ(bu);}
...
;this.__mQ(bD);bC.remove(bz);bz.destroy();}
...
;for(var bX in this.__bb[bW]){var bV=this.__bb[bW][bX].treeNode;var bY=qx.core.ObjectRegistry.fromHashCode(bX);this.__mQ(bY);this.__xy(bY,bV);}
;}
...
;}
,__mQ:function(cg){for(var i=0;i<this.__yw.length;i++ ){var ce=this.__yw[i];var cf=this.__bb[ce][cg.toHashCode()];if(cf!=null){if(cf.id){cg.removeBinding(cf.id);cf.id=null;}
...
;this._startSelectionModification();this.__CO();this.__Jd();this._endSelectionModification();this._updateSelection();}
