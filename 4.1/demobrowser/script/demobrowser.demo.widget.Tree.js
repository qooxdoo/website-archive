...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Tree.6fe66a35d975.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="Windows (C:)",b="icon/22/places/user-desktop.png",c="Documents (D:)",d="Inbox",f="changeValue",g="Desktop",h="Trash",j="execute",k="root",l="Open mode:",m="Presets",n="Hide Root Node",o="Network",p="Control",q=" items",r="changeSelection",s="Sent",t="Files",u="Show Open Items",v="",w="mode",x="multi",y="g",z="Workspace",A="double tap",B="single",C="Selection: ",D="Junk #",E="demobrowser.demo.widget.Tree",F="Data",G="Show root open button",H="\n",I="none",J="Enable multi selection",K="Enable drag selection",L="Selection:",M="Show Selected Items",N="Root node:",O="tap",P="Toggle Height",Q=",",R="Edit",S="Show Items";qx.Class.define(E,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var W=new qx.ui.layout.HBox();W.set({spacing:20});var V=new qx.ui.container.Composite(W);this.getRoot().add(V);var T=this.getTree();V.add(T);var U=this.getCommandFrame(T);V.add(U);}
,getTree:function(){var bh=new qx.ui.tree.Tree().set({width:200,height:400,draggable:true});var bm=new qx.ui.tree.TreeFolder(k);bm.setOpen(true);bh.setRoot(bm);var bk=new qx.ui.tree.TreeFolder(g);bk.setOpen(true);bk.setIcon(b);bm.add(bk);var bc=new qx.ui.tree.TreeFolder(t);var ba=new qx.ui.tree.TreeFolder(z);var bb=new qx.ui.tree.TreeFolder(o);var Y=new qx.ui.tree.TreeFolder(h);bk.add(bc,ba,bb,Y);var bi=new qx.ui.tree.TreeFile(a);var X=new qx.ui.tree.TreeFile(c);ba.add(bi,X);var bl=new qx.ui.tree.TreeFolder(d);var bd=new qx.ui.tree.TreeFolder(m);var bg=new qx.ui.tree.TreeFolder(s);var bf=new qx.ui.tree.TreeFolder(h);for(var i=0;i<30;i++ ){bf.add(new qx.ui.tree.TreeFile(D+i));}
...
;}
);bC.add(new qx.ui.core.Spacer(bu,bu),{row:bE++ ,column:0});bC.add(new qx.ui.basic.Label(l),{row:bE,column:0});var bp={"tap":O,"dbltap":A,"none":I};var bo=new qx.ui.form.RadioGroup();for(var by in bp){var bw=new qx.ui.form.RadioButton(bp[by]).set({value:by==bt.getOpenMode()});bw.setUserData(w,by);bo.add(bw);bC.add(bw,{row:bE++ ,column:1});}
;bo.addListener(r,function(e){bt.setOpenMode(e.getData()[0].getUserData(w));}
);bC.add(new qx.ui.core.Spacer(bu,bu),{row:bE++ ,column:0});bC.add(new qx.ui.basic.Label(L),{row:bE,column:0});var bz=new qx.ui.form.CheckBox(J);bC.add(bz,{row:bE++ ,column:1});bz.addListener(f,function(e){var bG=e.getData();bt.setSelectionMode(bG?x:B);}
);var bs=new qx.ui.form.CheckBox(K);bC.add(bs,{row:bE++ ,column:1});bs.addListener(f,function(e){var bH=e.getData();bt.setDragSelection(bH);if(!bz.getValue()){bz.setValue(true);}
;}
);bC.add(new qx.ui.core.Spacer(bu,bu),{row:bE++ ,column:0});bC.add(new qx.ui.basic.Label(N),{row:bE,column:0});var bB=new qx.ui.form.CheckBox(n);bC.add(bB,{row:bE++ ,column:1});bB.addListener(f,function(e){bt.setHideRoot(e.getData());}
);var bq=new qx.ui.form.CheckBox(G);bC.add(bq,{row:bE++ ,column:1});bq.addListener(f,function(e){bt.setRootOpenClose(e.getData());}
);bC.add(new qx.ui.core.Spacer(bu,bu),{row:bE++ ,column:0});var br=new qx.ui.form.Button(S);bC.add(br,{row:bE++ ,column:1});br.addListener(j,function(e){alert((v+bt.getItems()).replace(Q,H,y));}
);var bn=new qx.ui.form.Button(u);bC.add(bn,{row:bE++ ,column:1});bn.addListener(j,function(e){alert((v+bt.getItems(true,false)).replace(Q,H,y));}
);var bx=new qx.ui.form.Button(M);bC.add(bx,{row:bE++ ,column:1});bx.addListener(j,function(e){if(this.getSelectionMode()===B){alert(this.getSelection());}
else {alert(this.getSelection().toString().replace(Q,H,y));}
;}
...
(function(){var a="Boolean",b="one",c="pointerover",d="qx.event.type.Data",f=", because it is not a child element!",g="_applyDragSelection",h="Could not add + ",j=" from selection, because it is not a child element!",k="changeSelection",l="Could not test if ",m="Could not remove ",n="addItem",o="multi",p="_applyQuickSelection",q="single",r="qx.ui.core.MMultiSelectionHandling",s="pointermove",t=" is selected, because it is not a child element!",u="_applySelectionMode",v="additive",w="keypress",x="losecapture",y="Could not select ",z="removeItem",A="tap",B="pointerdown",C=" to selection, because it is not a child element!",D="__gy";qx.Mixin.define(r,{construct:function(){var F=this.SELECTION_MANAGER;var E=this.__gy=new F(this);this.addListener(B,E.handlePointerDown,E);this.addListener(A,E.handleTap,E);this.addListener(c,E.handlePointerOver,E);this.addListener(s,E.handlePointerMove,E);this.addListener(x,E.handleLoseCapture,E);this.addListener(w,E.handleKeyPress,E);this.addListener(n,E.handleAddItem,E);this.addListener(z,E.handleRemoveItem,E);E.addListener(k,this._onSelectionChange,this);}
,events:{"changeSelection":d},properties:{selectionMode:{check:[q,o,v,b],init:q,apply:u},dragSelection:{check:a,init:false,apply:g},quickSelection:{check:a,init:false,apply:p}},members:{__gy:null,selectAll:function(){this.__gy.selectAll();}
,isSelected:function(G){if(!qx.ui.core.Widget.contains(this,G)){throw new Error(l+G+t);}
;return this.__gy.isItemSelected(G);}
...
;this.__gy.removeItem(I);}
,selectRange:function(J,K){this.__gy.selectItemRange(J,K);}
,resetSelection:function(){this.__gy.clearSelection();}
,setSelection:function(L){for(var i=0;i<L.length;i++ ){if(!qx.ui.core.Widget.contains(this,L[i])){throw new Error(y+L[i]+f);}
;}
;if(L.length===0){this.resetSelection();}
else {var M=this.getSelection();if(!qx.lang.Array.equals(M,L)){this.__gy.replaceSelection(L);}
;}
;}
,getSelection:function(){return this.__gy.getSelection();}
,getSortedSelection:function(){return this.__gy.getSortedSelection();}
,isSelectionEmpty:function(){return this.__gy.isSelectionEmpty();}
,getSelectionContext:function(){return this.__gy.getSelectionContext();}
,_getManager:function(){return this.__gy;}
,getSelectables:function(N){return this.__gy.getSelectables(N);}
,invertSelection:function(){this.__gy.invertSelection();}
,_getLeadItem:function(){var O=this.__gy.getMode();if(O===q||O===b){return this.__gy.getSelectedItem();}
else {return this.__gy.getLeadItem();}
;}
,_applySelectionMode:function(Q,P){this.__gy.setMode(Q);}
,_applyDragSelection:function(S,R){this.__gy.setDrag(S);}
,_applyQuickSelection:function(U,T){this.__gy.setQuick(U);}
...
;throw new Error(n);}
,getSelection:function(){return qx.lang.Object.getValues(this.__gC);}
,getSortedSelection:function(){var bU=this.getSelectables();var bT=qx.lang.Object.getValues(this.__gC);bT.sort(function(a,b){return bU.indexOf(a)-bU.indexOf(b);}
);return bT;}
,isItemSelected:function(bV){var bW=this._selectableToHashCode(bV);return this.__gC[bW]!==undefined;}
,isSelectionEmpty:function(){return qx.lang.Object.isEmpty(this.__gC);}
...
;this._fireChange();}
,_setLeadItem:function(cb){var ca=this.__nw;if(ca!==null){this._styleSelectable(ca,u,false);}
;if(cb!==null){this._styleSelectable(cb,u,true);}
;this.__nw=cb;}
,getLeadItem:function(){return this.__nw;}
,_setAnchorItem:function(cd){var cc=this.__nx;if(cc!=null){this._styleSelectable(cc,I,false);}
;if(cd!=null){this._styleSelectable(cd,I,true);}
;this.__nx=cd;}
,_getAnchorItem:function(){return this.__nx!==null?this.__nx:null;}
...
;this._fireChange();}
,handlePointerOver:function(event){if(this.__nz!=null&&this.__nz!=this._getScroll().top){this.__nz=null;return;}
...
;var cD=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cA=event.isShiftPressed();if(this.isItemSelected(cB)&&!cA&&!cD&&!this.getDrag()){this.__ny=cB;this._userInteraction=false;return;}
else {this.__ny=null;}
;this._scrollItemIntoView(cB);var cC=this.getMode();if(this.getDrag()&&cC!==bt&&cC!==h&&!cA&&!cD&&event.getPointerType()==F){this._setAnchorItem(cB);this._setLeadItem(cB);this.__nm=this._getLocation();this.__nj=this._getScroll();this.__nn=event.getDocumentLeft()+this.__nj.left;this.__no=event.getDocumentTop()+this.__nj.top;this.__np=true;this._capture();}
;this._fireChange(bD);this._userInteraction=false;}
,handleTap:function(event){this._userInteraction=true;var cH=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cE=event.isShiftPressed();if(!cH&&!cE&&this.__ny!=null){this._userInteraction=false;var cF=this._getSelectableFromPointerEvent(event);if(cF===null||!this.isItemSelected(cF)){return;}
;}
...
,handlePointerMove:function(event){if(!this.__np){return;}
;this.__nq=event.getDocumentLeft();this.__nr=event.getDocumentTop();this._userInteraction=true;var cI=this.__nq+this.__nj.left;if(cI>this.__nn){this.__ns=1;}
else if(cI<this.__nn){this.__ns=-1;}
else {this.__ns=0;}
;var cJ=this.__nr+this.__nj.top;if(cJ>this.__no){this.__nt=1;}
else if(cJ<this.__no){this.__nt=-1;}
else {this.__nt=0;}
;var location=this.__nm;if(this.__nq<location.left){this.__ng=this.__nq-location.left;}
else if(this.__nq>location.right){this.__ng=this.__nq-location.right;}
else {this.__ng=0;}
;if(this.__nr<location.top){this.__nh=this.__nr-location.top;}
else if(this.__nr>location.bottom){this.__nh=this.__nr-location.bottom;}
else {this.__nh=0;}
;if(!this.__ni){this.__ni=new qx.event.Timer(100);this.__ni.addListener(G,this._onInterval,this);}
;this.__ni.start();this._autoSelect();event.stopPropagation();this._userInteraction=false;}
,handleAddItem:function(e){var cK=e.getData();if(this.getMode()===h&&this.isSelectionEmpty()){this.addItem(cK);}
;}
,handleRemoveItem:function(e){this.removeItem(e.getData());}
,_cleanup:function(){if(!this.getDrag()&&this.__np){return;}
;if(this.__nu){this._fireChange(bD);}
;delete this.__np;delete this.__nk;delete this.__nl;this._releaseCapture();if(this.__ni){this.__ni.stop();}
;}
,_onInterval:function(e){this._scrollBy(this.__ng,this.__nh);this.__nj=this._getScroll();this._autoSelect();}
,_autoSelect:function(){var cT=this._getDimension();var cM=Math.max(0,Math.min(this.__nq-this.__nm.left,cT.width))+this.__nj.left;var cL=Math.max(0,Math.min(this.__nr-this.__nm.top,cT.height))+this.__nj.top;if(this.__nk===cM&&this.__nl===cL){return;}
;this.__nk=cM;this.__nl=cL;var cQ=this._getAnchorItem();var cO=cQ;var cR=this.__ns;var cU,cN;while(cR!==0){cU=cR>0?this._getRelatedSelectable(cO,t):this._getRelatedSelectable(cO,V);if(cU!==null){cN=this._getSelectableLocationX(cU);if((cR>0&&cN.left<=cM)||(cR<0&&cN.right>=cM)){cO=cU;continue;}
;}
;break;}
;var cS=this.__nt;var cV,cP;while(cS!==0){cV=cS>0?this._getRelatedSelectable(cO,z):this._getRelatedSelectable(cO,bB);if(cV!==null){cP=this._getSelectableLocationY(cV);if((cS>0&&cP.top<=cL)||(cS<0&&cP.bottom>=cL)){cO=cV;continue;}
;}
;break;}
...
;this._fireChange(bw);}
,__nA:{Home:1,Down:1,Right:1,PageDown:1,End:1,Up:1,Left:1,PageUp:1},handleKeyPress:function(event){this._userInteraction=true;var dd,dc;var df=event.getKeyIdentifier();var de=this.getMode();var cY=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var da=event.isShiftPressed();var db=false;if(df===A&&cY){if(de!==bt&&de!==h){this._selectAllItems();db=true;}
;}
...
;db=true;}
;}
else if(this.__nA[df]){db=true;if(de===bt||de==h){dd=this._getSelectedItem();}
...
;}
;break;};this.__nz=this._getScroll().top;this._scrollItemIntoView(dc);}
;}
...
;}
,_clearSelection:function(){var di=this.__gC;for(var dj in di){this._removeFromSelection(di[dj]);}
;this.__gC={};}
,_selectItemRange:function(dk,dp,dm){var dq=this._getSelectableRange(dk,dp);if(!dm){var dn=this.__gC;var dr=this.__nB(dq);for(var dl in dn){if(!dr[dl]){this._removeFromSelection(dn[dl]);}
;}
;}
...
;}
,__nB:function(dw){var dx={};var dv;for(var i=0,l=dw.length;i<l;i++ ){dv=dw[i];dx[this._selectableToHashCode(dv)]=dv;}
;return dx;}
,_getSelectedItem:function(){for(var dy in this.__gC){return this.__gC[dy];}
;return null;}
,_setSelectedItem:function(dA){if(this._isSelectable(dA)){var dz=this.__gC;var dB=this._selectableToHashCode(dA);if(!dz[dB]||(dz.length>=2)){this._clearSelection();this._addToSelection(dA);}
;}
;}
,_addToSelection:function(dC){var dD=this._selectableToHashCode(dC);if(this.__gC[dD]==null&&this._isSelectable(dC)){this.__gC[dD]=dC;this._styleSelectable(dC,R,true);this.__nu=true;}
;}
,_toggleInSelection:function(dE){var dF=this._selectableToHashCode(dE);if(this.__gC[dF]==null){this.__gC[dF]=dE;this._styleSelectable(dE,R,true);}
else {delete this.__gC[dF];this._styleSelectable(dE,R,false);}
;this.__nu=true;}
,_removeFromSelection:function(dG){var dH=this._selectableToHashCode(dG);if(this.__gC[dH]!=null){delete this.__gC[dH];this._styleSelectable(dG,R,false);this.__nu=true;}
;}
...
;}
;var dP=dK[0];var dI=dO;var dM=this.__gC;for(var dN in dM){if(dJ[dN]){delete dJ[dN];}
...
;if(!dL){return false;}
;this._scrollItemIntoView(dI);this._setLeadItem(dP);this._setAnchorItem(dP);this.__nu=true;this._fireChange();}
,_fireChange:function(dQ){if(this.__nu){this.__nv=dQ||null;this.fireDataEvent(bs,this.getSelection());delete this.__nu;}
;}
...
;return dS;}
;return null;}
},destruct:function(){this._disposeObjects(bA);this.__gC=this.__ny=this.__nx=null;this.__nw=null;}
});}
)();
(function(){var a="vertical",b="under",c="above",d="qx.ui.core.selection.Widget",e="left",f="right";qx.Class.define(d,{extend:qx.ui.core.selection.Abstract,construct:function(g){qx.ui.core.selection.Abstract.call(this);this.__mn=g;}
,members:{__mn:null,_isSelectable:function(h){return this._isItemSelectable(h)&&h.getLayoutParent()===this.__mn;}
,_selectableToHashCode:function(j){return j.$$hash;}
,_styleSelectable:function(k,m,n){n?k.addState(m):k.removeState(m);}
,_capture:function(){this.__mn.capture();}
,_releaseCapture:function(){this.__mn.releaseCapture();}
,_isItemSelectable:function(o){if(this._userInteraction){return o.isVisible()&&o.isEnabled();}
else {return o.isVisible();}
;}
,_getWidget:function(){return this.__mn;}
,_getLocation:function(){var p=this.__mn.getContentElement().getDomElement();return p?qx.bom.element.Location.get(p):null;}
,_getDimension:function(){return this.__mn.getInnerSize();}
...
,_scrollItemIntoView:function(w){this.__mn.scrollChildIntoView(w);}
,getSelectables:function(x){var y=false;if(!x){y=this._userInteraction;this._userInteraction=true;}
;var B=this.__mn.getChildren();var z=[];var A;for(var i=0,l=B.length;i<l;i++ ){A=B[i];if(this._isItemSelectable(A)){z.push(A);}
;}
;this._userInteraction=y;return z;}
,_getSelectableRange:function(C,D){if(C===D){return [C];}
;var H=this.__mn.getChildren();var E=[];var G=false;var F;for(var i=0,l=H.length;i<l;i++ ){F=H[i];if(F===C||F===D){if(G){E.push(F);break;}
else {G=true;}
;}
;if(G&&this._isItemSelectable(F)){E.push(F);}
;}
;return E;}
,_getFirstSelectable:function(){var I=this.__mn.getChildren();for(var i=0,l=I.length;i<l;i++ ){if(this._isItemSelectable(I[i])){return I[i];}
;}
;return null;}
,_getLastSelectable:function(){var J=this.__mn.getChildren();for(var i=J.length-1;i>0;i-- ){if(this._isItemSelectable(J[i])){return J[i];}
;}
;return null;}
,_getRelatedSelectable:function(N,O){var L=this.__mn.getOrientation()===a;var P=this.__mn.getChildren();var K=P.indexOf(N);var M;if((L&&O===c)||(!L&&O===e)){for(var i=K-1;i>=0;i-- ){M=P[i];if(this._isItemSelectable(M)){return M;}
;}
;}
...
;}
,addIcon:function(){var L=this.getChildControl(s);if(this.__nD){this._remove(L);}
;this._add(L);this.__nD=true;}
,addLabel:function(M){var N=this.getChildControl(D);if(this.__nC){this._remove(N);}
;if(M){this.setLabel(M);}
else {N.setValue(this.getLabel());}
;this._add(N);this.__nC=true;}
,_applyIcon:function(P,O){if(!this.__nG()){this.__nH(P);}
else if(!this.isOpen()){this.__nH(P);}
;}
,_applyIconOpened:function(R,Q){if(this.isOpen()){if(this.__nF()&&this.__nG()){this.__nH(R);}
else if(!this.__nF()&&this.__nG()){this.__nH(R);}
;}
;}
...
;var X;if(W){X=this.__nG()?this.getIconOpened():null;}
else {X=this.getIcon();}
;if(X){this.__nH(X);}
;W?this.addState(t):this.removeState(t);}
,__nF:function(){return qx.util.PropertyUtil.getUserValue(this,s);}
,__nG:function(){return qx.util.PropertyUtil.getUserValue(this,v);}
,__nH:function(Y){var ba=this.getChildControl(s,true);if(ba){ba.setSource(Y);}
;}
...
;}
;if(this.__nE){this.__nE.setWidth((this.getLevel()+1)*this.getIndent()-bf);}
;}
...
(function(){var a="open",b="abstract",c="qx.debug",d="'. It is not a child of this tree item.",e="addItem",f="__gV",g="visible",h="excluded",j="__nM",k="removeItem",m="Invalid child index: ",n="Cannot remove treeitem '",o="qx.ui.tree.core.AbstractTreeItem";qx.Class.define(o,{extend:qx.ui.tree.core.AbstractItem,type:b,construct:function(p){qx.ui.tree.core.AbstractItem.call(this,p);this.__gV=[];}
,properties:{parent:{check:o,nullable:true}},members:{__gV:null,__nM:null,getTree:function(){var r=this;while(r.getParent()){r=r.getParent();}
...
;}
;}
,getChildrenContainer:function(){if(!this.__nM){this.__nM=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?g:h});}
;return this.__nM;}
...
,hasChildren:function(){return this.__gV?this.__gV.length>0:false;}
...
;this.__gV.push(Q);if(!N){this.__nN();}
;if(P){Q.recursiveAddToWidgetQueue();P.fireNonBubblingEvent(e,qx.event.type.Data,[Q]);}
;}
;if(P){qx.ui.core.queue.Widget.add(this);}
;}
,addAt:function(X,T){if(qx.core.Environment.get(c)){this.assert(T<=this.__gV.length&&T>=0,m+T);}
;if(T==this.__gV.length){this.add(X);return;}
;var W=X.getParent();if(W){W.remove(X);}
;var U=this.getChildrenContainer();X.setParent(this);var V=this.hasChildren();var S=this.__gV[T];U.addBefore(X,S);if(X.hasChildren()){U.addAfter(X.getChildrenContainer(),X);}
;qx.lang.Array.insertAt(this.__gV,X,T);if(!V){this.__nN();}
;if(this.getTree()){X.recursiveAddToWidgetQueue();qx.ui.core.queue.Widget.add(this);}
;}
,addBefore:function(Y,bb){if(qx.core.Environment.get(c)){this.assert(this.__gV.indexOf(bb)>=0);}
;var ba=Y.getParent();if(ba){ba.remove(Y);}
;this.addAt(Y,this.__gV.indexOf(bb));}
,addAfter:function(bd,bc){if(qx.core.Environment.get(c)){this.assert(this.__gV.indexOf(bc)>=0);}
;var be=bd.getParent();if(be){be.remove(bd);}
;this.addAt(bd,this.__gV.indexOf(bc)+1);}
,addAtBegin:function(bf){this.addAt(bf,0);}
,remove:function(bk){for(var i=0,l=arguments.length;i<l;i++ ){var bj=arguments[i];if(this.__gV.indexOf(bj)==-1){this.warn(n+bj+d);return;}
...
;}
;qx.lang.Array.remove(this.__gV,bj);bj.setParent(null);bg.remove(bj);}
;var bh=this.getTree();if(bh){bh.fireNonBubblingEvent(k,qx.event.type.Data,[bj]);}
;qx.ui.core.queue.Widget.add(this);}
,removeAt:function(bl){var bm=this.__gV[bl];if(bm){this.remove(bm);}
;}
,removeAll:function(){var bn=this.__gV.concat();for(var i=this.__gV.length-1;i>=0;i-- ){this.remove(this.__gV[i]);}
;return bn;}
},destruct:function(){this._disposeArray(f);this._disposeObjects(j);}
});}
)();
...
(function(){var a="Enter",b="_applyOpenMode",c="excluded",d="Space",f="Left",g="Boolean",h="changeOpenMode",j="_applyRootOpenClose",k="visible",l="__nP",m="changeSelection",n="qx.ui.tree.Tree",o="qx.event.type.Data",p="qx.ui.tree.core.AbstractTreeItem",q="tree",r="dbltap",s="_applyHideRoot",t="changeRoot",u="_applyRoot",v="keypress",w="none",x="pane",y="tap",z="Right";qx.Class.define(n,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IModelSelection,qx.ui.form.IForm],include:[qx.ui.core.MMultiSelectionHandling,qx.ui.core.MContentPadding,qx.ui.form.MModelSelection,qx.ui.form.MForm],construct:function(){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__nP=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({allowShrinkY:false,allowGrowX:true});this.getChildControl(x).add(this.__nP);this.initOpenMode();this.initRootOpenClose();this.addListener(m,this._onChangeSelection,this);this.addListener(v,this._onKeyPress,this);}
...
(function(){var a="indexOf",b="addAfter",c="add",d="addBefore",e="_",f="addAt",g="hasChildren",h="removeAt",i="removeAll",j="getChildren",k="remove",l="qx.ui.core.MRemoteChildrenHandling";qx.Mixin.define(l,{members:{__nU:function(q,m,o,n){var p=this.getChildrenContainer();if(p===this){q=e+q;}
;return (p[q])(m,o,n);}
,getChildren:function(){return this.__nU(j);}
,hasChildren:function(){return this.__nU(g);}
,add:function(s,r){return this.__nU(c,s,r);}
,remove:function(t){return this.__nU(k,t);}
,removeAll:function(){return this.__nU(i);}
,indexOf:function(u){return this.__nU(a,u);}
,addAt:function(x,v,w){this.__nU(f,x,v,w);}
,addBefore:function(y,A,z){this.__nU(d,y,A,z);}
,addAfter:function(D,B,C){this.__nU(b,D,B,C);}
,removeAt:function(E){return this.__nU(h,E);}
}});}
)();
