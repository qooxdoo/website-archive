...
(function(){var a="qx.module.event.PointerHandler",b="pointerup",c="event.dispatchevent",d="gesturemove",e="pointerover",f="gesturebegin",g="pointerdown",h="on",i="pointermove",j="gesturefinish",k="pointercancel",l="pointerout";qx.Bootstrap.define(a,{statics:{TYPES:[i,e,l,g,b,k,f,d,j],register:function(n,m){if(!n[h+m]){n[h+m]=true;}
;if(!n.__pointerListeners){n.__pointerListeners=0;}
;if(!n.$$pointerHandler){if(!qx.core.Environment.get(c)){if(!n.$$emitter){n.$$emitter=new qx.event.Emitter();}
;}
;n.$$pointerHandler=new qx.event.handler.PointerCore(n,n.$$emitter);}
;n.__pointerListeners++ ;}
,unregister:function(o){if(o.$$pointerHandler){o.__pointerListeners-- ;if(o.__pointerListeners===0){o.$$pointerHandler.dispose();o.$$pointerHandler=null;}
;}
;}
...
(function(){var a="touchmove",b="mousedown",c="event.dispatchevent",d="MSPointerDown",e="gesturemove",f="pointerover",g="touch",h="mouseout",j="pointercancel",k="pointerleave",m="touchstart",n="pointerenter",o="mouse",p="event.mspointer",q="mousemove",r="MSPointerCancel",s="gesturefinish",t="browser.documentmode",u="pointerup",v="touchend",w="mouseover",z="pointerdown",A="MSPointerUp",B="device.touch",C="MSPointerOver",D="mshtml",E="engine.name",F="mouseup",G="touchcancel",H="contextmenu",I="MSPointerMove",J="MSPointerOut",K="gesturebegin",L="qx.event.handler.PointerCore",M="pointermove",N="pointerout";qx.Bootstrap.define(L,{extend:Object,statics:{MOUSE_TO_POINTER_MAPPING:{mousedown:z,mouseup:u,mousemove:M,mouseout:N,mouseover:f},TOUCH_TO_POINTER_MAPPING:{touchstart:z,touchend:u,touchmove:M,touchcancel:j},MSPOINTER_TO_POINTER_MAPPING:{MSPointerDown:z,MSPointerMove:M,MSPointerUp:u,MSPointerCancel:j,MSPointerLeave:k,MSPointerEnter:n,MSPointerOver:f,MSPointerOut:N},POINTER_TO_GESTURE_MAPPING:{pointerdown:K,pointerup:s,pointercancel:s,pointermove:e},SIM_MOUSE_DISTANCE:25,SIM_MOUSE_DELAY:2500,__lastTouch:null},construct:function(Q,R){this.__defaultTarget=Q;this.__emitter=R;this.__eventNames=[];this.__buttonStates=[];var P=qx.core.Environment.get(E);var O=parseInt(qx.core.Environment.get(t),10);if(P==D&&O==10){this.__eventNames=[d,I,A,r,C,J];this._initPointerObserver();}
else {if(qx.core.Environment.get(p)){this.__nativePointerEvents=true;}
;this.__eventNames=[z,M,u,j,f,N];this._initPointerObserver();}
...
,_initObserver:function(S,T){this.__wrappedListener=qx.lang.Function.listener(S,this);this.__eventNames.forEach(function(U){if(T&&qx.dom.Node.isDocument(this.__defaultTarget)){if(!this.__defaultTarget.$$emitter){this.__defaultTarget.$$emitter=new qx.event.Emitter();}
;this.__defaultTarget.$$emitter.on(U,this.__wrappedListener);}
else {qx.bom.Event.addNativeListener(this.__defaultTarget,U,this.__wrappedListener);}
;}
.bind(this));}
,_onPointerEvent:function(X){if(!qx.core.Environment.get(p)){return;}
;if(!this.__nativePointerEvents){X.stopPropagation();}
;var V=qx.event.handler.PointerCore.MSPOINTER_TO_POINTER_MAPPING[X.type]||X.type;var Y=qx.bom.Event.getTarget(X);var W=new qx.event.type.dom.Pointer(V,X);this._fireEvent(W,V,Y);}
,_onTouchEvent:function(bc){var bd=qx.event.handler.PointerCore.TOUCH_TO_POINTER_MAPPING[bc.type];var be=bc.changedTouches;bc.stopPropagation();if(bc.type==m&&this.__primaryIdentifier===null){this.__primaryIdentifier=be[0].identifier;}
;for(var i=0,l=be.length;i<l;i++ ){var bf=be[i];var bb={clientX:bf.clientX,clientY:bf.clientY,pageX:bf.pageX,pageY:bf.pageY,identifier:bf.identifier,screenX:bf.screenX,screenY:bf.screenY,target:document.elementFromPoint(bf.clientX,bf.clientY),pointerType:g,pointerId:bf.identifier+2};if(bc.type==m){var ba=new qx.event.type.dom.Pointer(f,bc,bb);this._fireEvent(ba,f,bb.target);}
;var bg=new qx.event.type.dom.Pointer(bd,bc,bb);if(bf.identifier==this.__primaryIdentifier){bg.isPrimary=true;bg.button=0;bg.buttons=1;qx.event.handler.PointerCore.__lastTouch={"x":bf.clientX,"y":bf.clientY,"time":new Date().getTime()};}
;this._fireEvent(bg,bd,bb.target);if(bc.type==v||bc.type==G){var bh=new qx.event.type.dom.Pointer(N,bc,bb);this._fireEvent(bh,N,bc.target);if(this.__primaryIdentifier==bf.identifier){this.__primaryIdentifier=null;}
;}
;}
;}
,_onMouseEvent:function(bi){qx.bom.Event.stopPropagation(bi);if(this._isSimulatedMouseEvent(bi.clientX,bi.clientY)){return;}
;if(bi.type==b){this.__buttonStates[bi.which]=1;}
else if(bi.type==F){this.__buttonStates[bi.which]=0;}
;var bk=qx.event.handler.PointerCore.MOUSE_TO_POINTER_MAPPING[bi.type];var bj=qx.bom.Event.getTarget(bi);var bl=qx.lang.Array.sum(this.__buttonStates);var bo={pointerType:o,pointerId:1};if(this.__lastButtonState!=bl&&bl!==0&&this.__lastButtonState!==0){var bm=new qx.event.type.dom.Pointer(M,bi,bo);this._fireEvent(bm,M,bj);}
;this.__lastButtonState=bl;if(bi.type==b&&bl>1){return;}
;if(bi.type==F&&bl>0){return;}
;if(bi.type==H){this.__buttonStates[bi.which]=0;return;}
;var bn=new qx.event.type.dom.Pointer(bk,bi,bo);this._fireEvent(bn,bk,bj);}
,_isSimulatedMouseEvent:function(x,y){var bq=qx.event.handler.PointerCore.__lastTouch;if(bq){var br=new Date().getTime()-bq.time;var bp=qx.event.handler.PointerCore.SIM_MOUSE_DISTANCE;var bt=Math.abs(x-qx.event.handler.PointerCore.__lastTouch.x);var bs=Math.abs(y-qx.event.handler.PointerCore.__lastTouch.y);if(br<qx.event.handler.PointerCore.SIM_MOUSE_DELAY){if(bt<bp||bs<bp){return true;}
;}
;}
;return false;}
...
;}
,_fireEvent:function(bv,bu,bw){bw=bw||bv.target;bu=bu||bv.type;var bx;if(bu==z||bu==u||bu==M){bx=new qx.event.type.dom.Pointer(qx.event.handler.PointerCore.POINTER_TO_GESTURE_MAPPING[bu],bv);qx.event.type.dom.Pointer.normalize(bx);bx.srcElement=bw;}
;if(qx.core.Environment.get(c)){if(!this.__nativePointerEvents){bw.dispatchEvent(bv);}
;if(bx){bw.dispatchEvent(bx);}
;}
else {bv.srcElement=bw;while(bw){if(bw.$$emitter){bv.currentTarget=bw;if(!bv._stopped){bw.$$emitter.emit(bu,bv);}
;if(bx&&!bx._stopped){bx.currentTarget=bw;bw.$$emitter.emit(bx.type,bx);}
;}
;bw=bw.parentNode;}
;}
;}
...
,_stopObserver:function(){qx.event.handler.GestureCore.GESTURE_EVENTS.forEach(function(L){qxWeb(this.__defaultTarget).off(L,this.checkAndFireGesture,this);}
...
;var K=qx.core.Environment.get(g);qxWeb(K.target).off(K.type,this._fireRoll,this);}
,checkAndFireGesture:function(M,N,O){if(!N){N=M.type;}
;if(!O){O=qx.bom.Event.getTarget(M);}
;if(N==u){this.gestureBegin(M,O);}
else if(N==d){this.gestureMove(M,O);}
else if(N==m){this.gestureFinish(M,O);}
;}
,gestureBegin:function(P,Q){if(this.__gesture[P.pointerId]){this.__stopLongTapTimer(this.__gesture[P.pointerId]);delete this.__gesture[P.pointerId];}
;this.__gesture[P.pointerId]={"startTime":new Date().getTime(),"lastEventTime":new Date().getTime(),"startX":P.clientX,"startY":P.clientY,"clientX":P.clientX,"clientY":P.clientY,"velocityX":0,"velocityY":0,"target":Q,"isTap":true,"isPrimary":P.isPrimary,"longTapTimer":window.setTimeout(this.__fireLongTap.bind(this,P,Q),qx.event.handler.GestureCore.LONGTAP_TIME)};if(P.isPrimary){this.__isMultiPointerGesture=false;this.__primaryTarget=Q;this.__fireTrack(l,P,Q);}
...
;}
;}
,gestureMove:function(S,T){var U=this.__gesture[S.pointerId];if(U){var R=U.clientX;var V=U.clientY;U.clientX=S.clientX;U.clientY=S.clientY;U.lastEventTime=new Date().getTime();if(R){U.velocityX=U.clientX-R;}
;if(V){U.velocityY=U.clientY-V;}
;if(Object.keys(this.__gesture).length===2){this.__fireRotate(S,U.target);this.__firePinch(S,U.target);}
;if(!this.__isMultiPointerGesture){this.__fireTrack(v,S,U.target);this._fireRoll(S,e,U.target);}
;if(U.isTap){U.isTap=this._isBelowTapMaxDistance(S);if(!U.isTap){this.__stopLongTapTimer(U);}
;}
;}
;}
,_hasIntermediaryHandler:function(W){while(W&&W!==this.__defaultTarget){if(W.$$gestureHandler){return true;}
;W=W.parentNode;}
;return false;}
,gestureFinish:function(X,Y){if(!this.__gesture[X.pointerId]){return;}
;var bd=this.__gesture[X.pointerId];this.__stopLongTapTimer(bd);if(this._hasIntermediaryHandler(Y)){return;}
;this.__handleRollImpulse(bd.velocityX,bd.velocityY,X,bd.target);this.__fireTrack(w,X,bd.target);if(Y!==bd.target){delete this.__gesture[X.pointerId];return;}
;if(bd.isTap){this._fireEvent(X,A,X.target||Y);if(Object.keys(this.__lastTap).length>0){var bb=Date.now()-qx.event.handler.GestureCore.DOUBLETAP_TIME;for(var bc in this.__lastTap){if(bc<bb){delete this.__lastTap[bc];}
else {if(this.__isBelowDoubleTapDistance(this.__lastTap[bc].x,this.__lastTap[bc].y,X.clientX,X.clientY,X.getPointerType())){this._fireEvent(X,p,X.target||Y);}
;}
;}
;}
;this.__lastTap[Date.now()]={x:X.clientX,y:X.clientY};}
else if(!this._isBelowTapMaxDistance(X)){var ba=this.__getSwipeGesture(X,Y);if(ba){X.swipe=ba;this._fireEvent(X,a,X.target||Y);}
;}
;delete this.__gesture[X.pointerId];}
,stopMomentum:function(be){this.__stopMomentum[be]=true;}
,cancelGesture:function(bf){if(this.__gesture[bf]){this.__stopLongTapTimer(this.__gesture[bf]);delete this.__gesture[bf];}
;}
,__handleRollImpulse:function(bk,bl,bh,bi,bn){var bm=bh.timeoutId;if((Math.abs(bl)<0.1&&Math.abs(bk)<0.1)||this.__stopMomentum[bm]){delete this.__stopMomentum[bm];return;}
;if(!bn){bn=1;var bj=2.8;bl=bl/bj;bk=bk/bj;}
;bn+=0.0006;bl=bl/bn;bk=bk/bn;var bg=qx.bom.AnimationFrame.request(qx.lang.Function.bind(function(bo,bp,bq,br,bs){this.__handleRollImpulse(bo,bp,bq,br,bs);}
,this,bk,bl,bh,bi,bn));bk=Math.round(bk*100)/100;bl=Math.round(bl*100)/100;bh.delta={x:-bk,y:-bl};bh.momentum=true;bh.timeoutId=bg;this._fireEvent(bh,h,bh.target||bi);}
,_calcAngle:function(){var bu=null;var bv=null;for(var bt in this.__gesture){var bw=this.__gesture[bt];if(bu===null){bu=bw;}
else {bv=bw;}
;}
;var x=bu.clientX-bv.clientX;var y=bu.clientY-bv.clientY;return (360+Math.atan2(y,x)*(180/Math.PI))%360;}
,_calcDistance:function(){var bx=null;var by=null;for(var bA in this.__gesture){var bB=this.__gesture[bA];if(bx===null){bx=bB;}
else {by=bB;}
;}
;var bz=Math.sqrt(Math.pow(bx.clientX-by.clientX,2)+Math.pow(bx.clientY-by.clientY,2));return bz;}
,_calcVelocity:function(bG,bC,bD,bH,bE){var bF=0;if(bE==e){if(bH==D){bF=bG.velocityX;}
else {bF=bG.velocityY;}
;bF=bF/(new Date().getTime()-bG.lastEventTime);}
else {bF=(bD!==0)?bC/bD:0;}
;return bF;}
...
,__isBelowDoubleTapDistance:function(bL,bP,bQ,bR,bS){var bO=qx.event.handler.GestureCore;var bM=Math.abs(bL-bQ)<bO.TAP_MAX_DISTANCE[bS];var bN=Math.abs(bP-bR)<bO.TAP_MAX_DISTANCE[bS];return bM&&bN;}
...
;}
,_onDblClick:function(cd){var ce=qx.bom.Event.getTarget(cd);this._fireEvent(cd,A,ce);this._fireEvent(cd,p,ce);}
...
;}
;if(ba==n||ba==t){delete this.__touchStartPosition[V.changedTouches[0].identifier];}
;}
