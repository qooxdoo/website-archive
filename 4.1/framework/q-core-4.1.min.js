...
,__isGecko:function(){return window.navigator.mozApps&&window.navigator.product===j&&window.navigator.userAgent.indexOf(k)==-1&&window.navigator.userAgent.indexOf(m)==-1;}
...
(function(){var a="qxWeb",b="data-qx-class";qx.Bootstrap.define(a,{extend:qx.type.BaseArray,statics:{__init:[],$$qx:qx,$init:function(g,e){var f=[];for(var i=0;i<g.length;i++ ){var d=!!(g[i]&&(g[i].nodeType===1||g[i].nodeType===9||g[i].nodeType===11));if(d){f.push(g[i]);continue;}
...
;return h;}
,$attach:function(k,j){for(var name in k){if(qxWeb.prototype[name]!=undefined&&Array.prototype[name]==undefined&&j!==true){{}
;}
else {qxWeb.prototype[name]=k[name];}
;}
;}
,$attachStatic:function(n,m){for(var name in n){{}
;qxWeb[name]=n[name];}
;}
,$attachInit:function(o){this.__init.push(o);}
...
;if(!s){s=[];}
else if(qx.Bootstrap.isString(s)){if(r instanceof qxWeb){r=r[0];}
;s=qx.bom.Selector.query(s,r);}
else if((s.nodeType===1||s.nodeType===9||s.nodeType===11)||(s.history&&s.location&&s.document)){s=[s];}
;return qxWeb.$init(s,qxWeb);}
,members:{filter:function(t){if(qx.lang.Type.isFunction(t)){return qxWeb.$init(Array.prototype.filter.call(this,t),this.constructor);}
;return qxWeb.$init(qx.bom.Selector.matches(t,this),this.constructor);}
,unique:function(){var u=qx.lang.Array.unique(this);return qxWeb.$init(u,this.constructor);}
,slice:function(v,w){if(w!==undefined){return qxWeb.$init(Array.prototype.slice.call(this,v,w),this.constructor);}
;return qxWeb.$init(Array.prototype.slice.call(this,v),this.constructor);}
,splice:function(x,y,z){return qxWeb.$init(Array.prototype.splice.apply(this,arguments),this.constructor);}
,map:function(A,B){return qxWeb.$init(Array.prototype.map.apply(this,arguments),qxWeb);}
,concat:function(D){var C=Array.prototype.slice.call(this,0);for(var i=0;i<arguments.length;i++ ){if(arguments[i] instanceof qxWeb){C=C.concat(Array.prototype.slice.call(arguments[i],0));}
else {C.push(arguments[i]);}
;}
;return qxWeb.$init(C,this.constructor);}
,indexOf:function(E){if(!E){return -1;}
;if(qx.Bootstrap.isArray(E)){E=E[0];}
;for(var i=0,l=this.length;i<l;i++ ){if(this[i]===E){return i;}
;}
;return -1;}
,debug:function(){{}
;return this;}
,logThis:function(){{var F,length;}
;return this;}
,_forEachElement:function(H,G){for(var i=0,l=this.length;i<l;i++ ){if(this[i]&&(this[i].nodeType===1||this[i].nodeType===11)){H.apply(G||this,[this[i],i,this]);}
;}
;return this;}
,_forEachElementWrapped:function(J,I){this._forEachElement(function(K,M,L){J.apply(this,[qxWeb(K),M,L]);}
,I);return this;}
},defer:function(N){if(window.q==undefined){q=N;}
;}
});}
)();
...
;}
;return name;}
,getVersion:function(){var S=navigator.userAgent;var T=new RegExp(B+qx.bom.client.Browser.__agents+n);var Q=S.match(T);if(!Q){return z;}
...
;}
,__agents:{"webkit":i,"gecko":p,"mshtml":q,"opera":H}[qx.bom.client.Engine.getName()]},defer:function(U){qx.core.Environment.add(k,U.getName);qx.core.Environment.add(C,U.getVersion);qx.core.Environment.add(J,U.getDocumentMode);qx.core.Environment.add(j,U.getQuirksMode);}
});}
)();
...
,getLinearGradient:function(){qx.bom.client.Css.__WEBKIT_LEGACY_GRADIENT=false;var cc=O;var bY=document.createElement(m);var ca=qx.bom.Style.getAppliedStyle(bY,H,cc);if(!ca){cc=bC;var ca=qx.bom.Style.getAppliedStyle(bY,H,cc,false);if(ca){qx.bom.client.Css.__WEBKIT_LEGACY_GRADIENT=true;}
;}
;if(!ca){return null;}
;var cb=/(.*?)\(/.exec(ca);return cb?cb[1]:null;}
,getFilterGradient:function(){return qx.bom.client.Css.__isFilterSupported(bF,bH);}
...
,getFilterTextShadow:function(){return qx.bom.client.Css.__isFilterSupported(J,U);}
,__isFilterSupported:function(co,cm){var cn=false;var cp=v+co+X+cm+bL;var cl=document.createElement(m);document.body.appendChild(cl);cl.style.filter=cp;if(cl.filters&&cl.filters.length>0&&cl.filters.item(co).enabled==true){cn=true;}
...
(function(){var a="qx.bom.element.Style",b="css.borderimage",c="css.userselect",d="css.boxsizing",e="pixelLeft",f="css.textoverflow",g="Cascaded styles are not supported in this browser!",h="pixelBottom",i="pixelHeight",j="pixelWidth",k="css.appearance",l="pixelRight",m="css.usermodify",n="",o="pixelTop",p="px";qx.Bootstrap.define(a,{statics:{__styleNames:null,__cssNames:null,__detectVendorProperties:function(){var r={"appearance":qx.core.Environment.get(k),"userSelect":qx.core.Environment.get(c),"textOverflow":qx.core.Environment.get(f),"borderImage":qx.core.Environment.get(b),"userModify":qx.core.Environment.get(m),"boxSizing":qx.core.Environment.get(d)};this.__cssNames={};for(var q in qx.lang.Object.clone(r)){if(!r[q]){delete r[q];}
else {this.__cssNames[q]=qx.bom.Style.getCssName(r[q]);}
;}
;this.__styleNames=r;}
,__getStyleName:function(name){var s=qx.bom.Style.getPropertyName(name);if(s){this.__styleNames[name]=s;}
;return s;}
,__mshtmlPixel:{width:j,height:i,left:e,right:l,top:o,bottom:h},__special:{clip:qx.bom.element.Clip,cursor:qx.bom.element.Cursor,opacity:qx.bom.element.Opacity,boxSizing:qx.bom.element.BoxSizing},COMPUTED_MODE:1,CASCADED_MODE:2,LOCAL_MODE:3,set:function(v,name,t,u){{}
;name=this.__styleNames[name]||this.__getStyleName(name)||name;if(u!==false&&this.__special[name]){this.__special[name].set(v,t);}
else {v.style[name]=t!==null?t:n;}
;}
,get:function(z,name,B,D){name=this.__styleNames[name]||this.__getStyleName(name)||name;if(D!==false&&this.__special[name]){return this.__special[name].get(z,B);}
;switch(B){case this.LOCAL_MODE:return z.style[name]||n;case this.CASCADED_MODE:if(z.currentStyle){return z.currentStyle[name]||n;}
;throw new Error(g);default:var x=qx.dom.Node.getDocument(z);var A=x.defaultView?x.defaultView.getComputedStyle:undefined;if(A!==undefined){var w=A(z,null);if(w&&w[name]){return w[name];}
;}
else {if(!z.currentStyle){return z.style[name]||n;}
;var F=z.currentStyle[name]||z.style[name]||n;if(/^-?[\.\d]+(px)?$/i.test(F)){return F;}
;var E=this.__mshtmlPixel[name];if(E&&(E in z.style)){var C=z.style[name];z.style[name]=F||0;var y=z.style[E]+p;z.style[name]=C;return y;}
;return F;}
;return z.style[name]||n;};}
},defer:function(G){G.__detectVendorProperties();}
});}
)();
...
,__getElementFromArgument:function(bE){if(bE instanceof qxWeb){return bE[0];}
else if(qx.Bootstrap.isString(bE)){return qxWeb(bE)[0];}
;return bE;}
,isElement:function(bF){return qx.dom.Node.isElement(qx.module.Traversing.__getElementFromArgument(bF));}
,isNode:function(bG){return qx.dom.Node.isNode(qx.module.Traversing.__getElementFromArgument(bG));}
,isNodeName:function(bI,bH){return qx.dom.Node.isNodeName(qx.module.Traversing.__getElementFromArgument(bI),bH);}
...
,getWindow:function(bL){return qx.dom.Node.getWindow(qx.module.Traversing.__getElementFromArgument(bL));}
...
,getDocument:function(bO){return qx.dom.Node.getDocument(qx.module.Traversing.__getElementFromArgument(bO));}
,getNodeName:function(bP){return qx.dom.Node.getName(qx.module.Traversing.__getElementFromArgument(bP));}
,getNodeText:function(bQ){return qx.dom.Node.getText(qx.module.Traversing.__getElementFromArgument(bQ));}
,isBlockNode:function(bR){return qx.dom.Node.isBlockNode(qx.module.Traversing.__getElementFromArgument(bR));}
,equalNodes:function(cb,cd){cb=qx.module.Traversing.__getNodeFromArgument(cb);cd=qx.module.Traversing.__getNodeFromArgument(cd);if(!cb||!cd){return false;}
...
;}
;if(ca){var bS=qx.module.Traversing.__getAttributes(cb);var cc=qx.module.Traversing.__getAttributes(cd);for(var bU in bS){if(bS[bU]!==cc[bU]){return false;}
;}
;}
...
;return true;}
;}
,__getNodeFromArgument:function(ce){if(typeof ce==q){ce=qxWeb(ce);}
;if(ce instanceof Array||ce instanceof qxWeb){ce=ce[0];}
;return qxWeb.isNode(ce)?ce:null;}
,__getAttributes:function(cf){var cg={};for(var ch in cf.attributes){if(ch==p){continue;}
;var name=cf.attributes[ch].name;var ci=cf.attributes[ch].value;cg[name]=ci;}
;return cg;}
,__hierarchyHelper:function(cl,co,cm){var cj=[];var ck=qx.dom.Hierarchy;for(var i=0,l=cl.length;i<l;i++ ){cj.push.apply(cj,ck[co](cl[i]));}
...
;var z=function(E,event){var G=qx.module.Event.__normalizations;var F=G[c]||[];if(G[C]){F=F.concat(G[C]);}
...
;t.$$lastlistenerId=t.$$emitter.on(C,z,w);t.$$emitter.getEntryById(t.$$lastlistenerId).useCapture=!!u;if(!t.__listener){t.__listener={};}
;if(!t.__listener[C]){t.__listener[C]={};}
;t.__listener[C][t.$$lastlistenerId]=z;if(!B){if(!t.__ctx){t.__ctx={};}
;t.__ctx[t.$$lastlistenerId]=w;}
;}
;return this;}
,off:function(R,J,O,I){var N=(J===null&&O===null);for(var j=0;j<this.length;j++ ){var H=this[j];if(!H.__listener){continue;}
;var T=[];if(R!==null){T.push(R);}
else {for(var L in H.__listener){T.push(L);}
;}
;for(var i=0,l=T.length;i<l;i++ ){for(var M in H.__listener[T[i]]){var Q=H.__listener[T[i]][M];if(N||Q==J||Q.original==J){var K=typeof H.__ctx!==p&&H.__ctx[M];var U;if(!O&&K){U=H.__ctx[M];}
;H.$$emitter.off(T[i],Q,U||O);if(N||Q.original==J){qx.bom.Event.removeNativeListener(H,T[i],Q,I);}
;delete H.__listener[T[i]][M];if(K){delete H.__ctx[M];}
;}
;}
;var P=qx.module.Event.__hooks.off;var S=P[c]||[];if(P[R]){S=S.concat(P[R]);}
...
;}
;}
,__isReady:false,ready:function(bu){if(document.readyState===s){window.setTimeout(bu,1);return;}
;var bv=function(){qx.module.Event.__isReady=true;bu();}
...
else {var bx=function(){if(qx.module.Event.__isReady){return;}
...
;var bR=qx.module.Event.__normalizations;for(var i=0,l=bT.length;i<l;i++ ){var bS=bT[i];if(qx.lang.Type.isFunction(bQ)){if(!bR[bS]){bR[bS]=[];}
;bR[bS].push(bQ);}
;}
;}
,$unregisterNormalization:function(bX,bU){if(!qx.lang.Type.isArray(bX)){bX=[bX];}
;var bV=qx.module.Event.__normalizations;for(var i=0,l=bX.length;i<l;i++ ){var bW=bX[i];if(bV[bW]){qx.lang.Array.remove(bV[bW],bU);}
;}
;}
...
;var cc=qx.module.Event.__hooks.on;for(var i=0,l=ce.length;i<l;i++ ){var cd=ce[i];if(qx.lang.Type.isFunction(cb)){if(!cc[cd]){cc[cd]=[];}
;cc[cd].push(cb);}
;}
;if(!ca){return;}
;var bY=qx.module.Event.__hooks.off;for(var i=0,l=ce.length;i<l;i++ ){var cd=ce[i];if(qx.lang.Type.isFunction(ca)){if(!bY[cd]){bY[cd]=[];}
;bY[cd].push(ca);}
;}
;}
,$unregisterEventHook:function(ck,ch,cg){if(!qx.lang.Type.isArray(ck)){ck=[ck];}
;var ci=qx.module.Event.__hooks.on;for(var i=0,l=ck.length;i<l;i++ ){var cj=ck[i];if(ci[cj]){qx.lang.Array.remove(ci[cj],ch);}
;}
;if(!cg){return;}
;var cf=qx.module.Event.__hooks.off;for(var i=0,l=ck.length;i<l;i++ ){var cj=ck[i];if(cf[cj]){qx.lang.Array.remove(cf[cj],cg);}
;}
;}
...
(function(){var a="touchmove",b="mousedown",c="event.dispatchevent",d="MSPointerDown",e="gesturemove",f="pointerover",g="touch",h="mouseout",k="ms",m="pointercancel",n="pointerleave",o="touchstart",p="pointerenter",q="mouse",r="event.mspointer",s="mousemove",t="MSPointerCancel",u="gesturefinish",v="browser.documentmode",w="pointerup",z="touchend",A="mouseover",B="pointerdown",C="MSPointerUp",D="device.touch",E="MSPointerOver",F="mshtml",G="engine.name",H="mouseup",I="touchcancel",J="contextmenu",K="gesturecancel",L="MSPointerMove",M="MSPointerOut",N="gesturebegin",O="qx.event.handler.PointerCore",P="pointermove",Q="pointerout";qx.Bootstrap.define(O,{extend:Object,statics:{MOUSE_TO_POINTER_MAPPING:{mousedown:B,mouseup:w,mousemove:P,mouseout:Q,mouseover:f},TOUCH_TO_POINTER_MAPPING:{touchstart:B,touchend:w,touchmove:P,touchcancel:m},MSPOINTER_TO_POINTER_MAPPING:{MSPointerDown:B,MSPointerMove:P,MSPointerUp:w,MSPointerCancel:m,MSPointerLeave:n,MSPointerEnter:p,MSPointerOver:f,MSPointerOut:Q},POINTER_TO_GESTURE_MAPPING:{pointerdown:N,pointerup:u,pointercancel:K,pointermove:e},SIM_MOUSE_DISTANCE:25,SIM_MOUSE_DELAY:2500,__lastTouch:null},construct:function(T,U){this.__defaultTarget=T;this.__emitter=U;this.__eventNames=[];this.__buttonStates=[];this.__activeTouches=[];var S=qx.core.Environment.get(G);var R=parseInt(qx.core.Environment.get(v),10);if(S==F&&R==10){this.__eventNames=[d,L,C,t,E,M];this._initPointerObserver();}
else {if(qx.core.Environment.get(r)){this.__nativePointerEvents=true;}
;this.__eventNames=[B,P,w,m,f,Q];this._initPointerObserver();}
;if(!qx.core.Environment.get(r)){if(qx.core.Environment.get(D)){this.__eventNames=[o,z,a,I];this._initObserver(this._onTouchEvent);}
;this.__eventNames=[b,H,s,A,h,J];this._initObserver(this._onMouseEvent);}
;}
,members:{__defaultTarget:null,__emitter:null,__eventNames:null,__nativePointerEvents:false,__wrappedListener:null,__lastButtonState:0,__buttonStates:null,__primaryIdentifier:null,__activeTouches:null,_initPointerObserver:function(){this._initObserver(this._onPointerEvent);}
,_initObserver:function(V,W){this.__wrappedListener=qx.lang.Function.listener(V,this);this.__eventNames.forEach(function(X){if(W&&qx.dom.Node.isDocument(this.__defaultTarget)){if(!this.__defaultTarget.$$emitter){this.__defaultTarget.$$emitter=new qx.event.Emitter();}
;this.__defaultTarget.$$emitter.on(X,this.__wrappedListener);}
else {qx.bom.Event.addNativeListener(this.__defaultTarget,X,this.__wrappedListener);}
;}
.bind(this));}
...
;if(!this.__nativePointerEvents){bb.stopPropagation();}
...
;bf.$$qxProcessed=true;var bg=qx.event.handler.PointerCore.TOUCH_TO_POINTER_MAPPING[bf.type];var bi=bf.changedTouches;this._determineActiveTouches(bf.type,bi);if(bf.touches.length<this.__activeTouches.length){for(var i=this.__activeTouches.length-1;i>=0;i-- ){var bk=new qx.event.type.dom.Pointer(m,bf,{identifier:this.__activeTouches[i].identifier,target:bf.target,pointerType:g,pointerId:this.__activeTouches[i].identifier+2});this._fireEvent(bk,m,bf.target);}
;this.__primaryIdentifier=null;this.__activeTouches=[];return;}
;if(bf.type==o&&this.__primaryIdentifier===null){this.__primaryIdentifier=bi[0].identifier;}
...
;if(bj.identifier==this.__primaryIdentifier){be.isPrimary=true;be.button=0;be.buttons=1;qx.event.handler.PointerCore.__lastTouch={"x":bj.clientX,"y":bj.clientY,"time":new Date().getTime()};}
;var bl=new qx.event.type.dom.Pointer(bg,bf,be);this._fireEvent(bl,bg,be.target);if(bf.type==z||bf.type==I){var bm=new qx.event.type.dom.Pointer(Q,bf,be);this._fireEvent(bm,Q,bf.target);if(this.__primaryIdentifier==bj.identifier){this.__primaryIdentifier=null;}
;}
;}
;}
...
;if(bn.type==b){this.__buttonStates[bn.which]=1;}
else if(bn.type==H){this.__buttonStates[bn.which]=0;}
;var bs=qx.event.handler.PointerCore.MOUSE_TO_POINTER_MAPPING[bn.type];var bo=qx.bom.Event.getTarget(bn);var bp=qx.lang.Array.sum(this.__buttonStates);var bt={pointerType:q,pointerId:1};if(this.__lastButtonState!=bp&&bp!==0&&this.__lastButtonState!==0){var br=new qx.event.type.dom.Pointer(P,bn,bt);this._fireEvent(br,P,bo);}
;this.__lastButtonState=bp;if(bn.type==b&&bp>1){return;}
;if(bn.type==H&&bp>0){return;}
;if(bn.type==J){this.__buttonStates[bn.which]=0;return;}
;var bq=new qx.event.type.dom.Pointer(bs,bn,bt);this._fireEvent(bq,bs,bo);}
,_determineActiveTouches:function(bx,bw){if(bx==o){for(var i=0;i<bw.length;i++ ){this.__activeTouches.push(bw[i]);}
;}
else if(bx==z||bx==I){var bu=[];for(var i=0;i<this.__activeTouches.length;i++ ){var bv=true;for(var j=0;j<bw.length;j++ ){if(this.__activeTouches[i].identifier==bw[j].identifier){bv=false;break;}
;}
;if(bv){bu.push(this.__activeTouches[i]);}
;}
;this.__activeTouches=bu;}
;}
,_isSimulatedMouseEvent:function(x,y){var bz=qx.event.handler.PointerCore.__lastTouch;if(bz){var bA=new Date().getTime()-bz.time;var by=qx.event.handler.PointerCore.SIM_MOUSE_DISTANCE;var bC=Math.abs(x-qx.event.handler.PointerCore.__lastTouch.x);var bB=Math.abs(y-qx.event.handler.PointerCore.__lastTouch.y);if(bA<qx.event.handler.PointerCore.SIM_MOUSE_DELAY){if(bC<by||bB<by){return true;}
;}
;}
;return false;}
...
;if(qx.core.Environment.get(c)){if(!this.__nativePointerEvents){bF.dispatchEvent(bE);}
...
,_stopObserver:function(){qx.event.handler.GestureCore.GESTURE_EVENTS.forEach(function(M){qxWeb(this.__defaultTarget).off(M,this.checkAndFireGesture,this);}
...
;}
,gestureBegin:function(Q,R){if(this.__gesture[Q.pointerId]){this.__stopLongTapTimer(this.__gesture[Q.pointerId]);delete this.__gesture[Q.pointerId];}
;this.__gesture[Q.pointerId]={"startTime":new Date().getTime(),"lastEventTime":new Date().getTime(),"startX":Q.clientX,"startY":Q.clientY,"clientX":Q.clientX,"clientY":Q.clientY,"velocityX":0,"velocityY":0,"target":R,"isTap":true,"isPrimary":Q.isPrimary,"longTapTimer":window.setTimeout(this.__fireLongTap.bind(this,Q,R),qx.event.handler.GestureCore.LONGTAP_TIME)};if(Q.isPrimary){this.__isMultiPointerGesture=false;this.__primaryTarget=R;this.__fireTrack(l,Q,R);}
...
;}
;}
,gestureMove:function(T,U){var V=this.__gesture[T.pointerId];if(V){var S=V.clientX;var W=V.clientY;V.clientX=T.clientX;V.clientY=T.clientY;V.lastEventTime=new Date().getTime();if(S){V.velocityX=V.clientX-S;}
;if(W){V.velocityY=V.clientY-W;}
;if(Object.keys(this.__gesture).length===2){this.__fireRotate(T,V.target);this.__firePinch(T,V.target);}
;if(!this.__isMultiPointerGesture){this.__fireTrack(w,T,V.target);this._fireRoll(T,e,V.target);}
;if(V.isTap){V.isTap=this._isBelowTapMaxDistance(T);if(!V.isTap){this.__stopLongTapTimer(V);}
;}
;}
;}
,_hasIntermediaryHandler:function(X){while(X&&X!==this.__defaultTarget){if(X.$$gestureHandler){return true;}
;X=X.parentNode;}
;return false;}
,gestureFinish:function(ba,bb){if(!this.__gesture[ba.pointerId]){return;}
;var bh=this.__gesture[ba.pointerId];this.__stopLongTapTimer(bh);if(this._hasIntermediaryHandler(bb)){return;}
;this.__handleRollImpulse(bh.velocityX,bh.velocityY,ba,bh.target);this.__fireTrack(z,ba,bh.target);if(bh.isTap){if(bb!==bh.target){delete this.__gesture[ba.pointerId];return;}
;this._fireEvent(ba,B,ba.target||bb);var bc=false;if(Object.keys(this.__lastTap).length>0){var bg=Date.now()-qx.event.handler.GestureCore.DOUBLETAP_TIME;for(var bi in this.__lastTap){if(bi<bg){delete this.__lastTap[bi];}
else {var Y=this.__lastTap[bi];var be=this.__isBelowDoubleTapDistance(Y.x,Y.y,ba.clientX,ba.clientY,ba.getPointerType());var bf=Y.target===(ba.target||bb);var bj=Y.button===ba.button;if(be&&bj&&bf){bc=true;delete this.__lastTap[bi];this._fireEvent(ba,p,ba.target||bb);}
;}
;}
;}
;if(!bc){this.__lastTap[Date.now()]={x:ba.clientX,y:ba.clientY,target:ba.target||bb,button:ba.button};}
;}
else if(!this._isBelowTapMaxDistance(ba)){var bd=this.__getSwipeGesture(ba,bb);if(bd){ba.swipe=bd;this._fireEvent(ba,a,bh.target||bb);}
;}
;delete this.__gesture[ba.pointerId];}
,stopMomentum:function(bk){this.__stopMomentum[bk]=true;}
,gestureCancel:function(bl){if(this.__gesture[bl]){this.__stopLongTapTimer(this.__gesture[bl]);delete this.__gesture[bl];}
;}
,updateGestureTarget:function(bm,bn){this.__gesture[bm].target=bn;}
,__handleRollImpulse:function(bs,bt,bp,bq,bv){var bu=bp.timeoutId;if((Math.abs(bt)<1&&Math.abs(bs)<1)||this.__stopMomentum[bu]){delete this.__stopMomentum[bu];return;}
;if(!bv){bv=1;var br=2.8;bt=bt/br;bs=bs/br;}
;bv+=0.0006;bt=bt/bv;bs=bs/bv;var bo=qx.bom.AnimationFrame.request(qx.lang.Function.bind(function(bw,bx,by,bz,bA){this.__handleRollImpulse(bw,bx,by,bz,bA);}
...
,_calcAngle:function(){var bC=null;var bD=null;for(var bB in this.__gesture){var bE=this.__gesture[bB];if(bC===null){bC=bE;}
else {bD=bE;}
;}
;var x=bC.clientX-bD.clientX;var y=bC.clientY-bD.clientY;return (360+Math.atan2(y,x)*(180/Math.PI))%360;}
,_calcDistance:function(){var bF=null;var bG=null;for(var bI in this.__gesture){var bJ=this.__gesture[bI];if(bF===null){bF=bJ;}
else {bG=bJ;}
;}
...
,__isBelowDoubleTapDistance:function(bN,bR,bS,bT,bU){var bQ=qx.event.handler.GestureCore;var bO=Math.abs(bN-bS)<bQ.TAP_MAX_DISTANCE[bU];var bP=Math.abs(bR-bT)<bQ.TAP_MAX_DISTANCE[bU];return bO&&bP;}
,_getDeltaCoordinates:function(bX){var bY=this.__gesture[bX.pointerId];if(!bY){return null;}
;var bV=bX.clientX-bY.startX;var bW=bX.clientY-bY.startY;var ca=E;if(Math.abs(bV/bW)<1){ca=n;}
;return {"x":bV,"y":bW,"axis":ca};}
,_fireEvent:function(cc,ce,cd){if(!this.__defaultTarget){return;}
...
else if(this.__emitter){cb=new qx.event.type.dom.Custom(ce,cc,{target:this.__defaultTarget,currentTarget:this.__defaultTarget,srcElement:this.__defaultTarget,swipe:cc.swipe,scale:cc.scale,angle:cc.angle,delta:cc.delta,pointerType:cc.pointerType,momentum:cc.momentum});this.__emitter.emit(ce,cc);}
;}
,_onDblClick:function(cf){var cg=qx.bom.Event.getTarget(cf);this._fireEvent(cf,B,cg);this._fireEvent(cf,p,cg);}
,__getSwipeGesture:function(cj,ck){var cq=this.__gesture[cj.pointerId];if(!cq){return null;}
...
,__fireTrack:function(cs,ct,cu){ct.delta=this._getDeltaCoordinates(ct);this._fireEvent(ct,cs,ct.target||cu);}
...
else {var cy=this.__gesture[cw.pointerId];cw.delta={x:-cy.velocityX,y:-cy.velocityY,axis:Math.abs(cy.velocityX/cy.velocityY)<1?n:E};}
;this._fireEvent(cw,h,cw.target||cx);}
,__fireRotate:function(cz,cB){if(!cz.isPrimary){var cA=this._calcAngle();cz.angle=Math.round((cA-this.__initialAngle)%360);this._fireEvent(cz,k,this.__primaryTarget);}
;}
,__firePinch:function(cE,cF){if(!cE.isPrimary){var cC=this._calcDistance();var cD=cC/this.__initialDistance;cE.scale=(Math.round(cD*100)/100);this._fireEvent(cE,b,this.__primaryTarget);}
;}
,__fireLongTap:function(cG,cH){var cI=this.__gesture[cG.pointerId];if(cI){this._fireEvent(cG,f,cG.target||cH);cI.longTapTimer=null;cI.isTap=false;}
;}
,__stopLongTapTimer:function(cJ){if(cJ.longTapTimer){window.clearTimeout(cJ.longTapTimer);cJ.longTapTimer=null;}
;}
...
,dispose:function(){for(var cM in this.__gesture){this.__stopLongTapTimer(cM);}
...
;return y;}
;return 0;}
,__normalize:function(j){var g=Math.abs(j);if(qx.util.Wheel.MINSCROLL==null||qx.util.Wheel.MINSCROLL>g){qx.util.Wheel.MINSCROLL=g;this.__recalculateMultiplicator();}
