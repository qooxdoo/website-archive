...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.CellSpan-qx.theme.Classic.6ea276a8fbaa.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,__Lo:function(A,B,C,D,top,E,F){var H=this._cellProvider.getCellProperties(B,C);var G=H.insets||[0,0];A.push(k,h,e,D,c,o,top,c,this._getCellSizeStyle(E,F,G[0],G[1]),H.style||a,d,g,H.classes||a,d,H.attributes||a,n,H.content||a,p);}
,_fullUpdate:function(I,J,K,L){var U=[];var Q=this._spanManager.findCellsInWindow(I,J,K.length,L.length);if(Q.length>0){var R=this._spanManager.getCellBounds(Q,I,J);var O=this._spanManager.computeCellSpanMap(Q,I,J,K.length,L.length);for(var i=0,l=Q.length;i<l;i++){var S=Q[i];var W=R[i];this.__Lo(U,S.firstRow,S.firstColumn,W.left,W.top,W.width,W.height);}
;}
else {O=[];for(var i=0;i<K.length;i++){O[I+i]=[];}
;}
;var V=0;var top=0;var N=I;var T=J;for(var x=0;x<K.length;x++){var V=0;var T=J;var M=K[x];for(var y=0;y<L.length;y++){var P=L[y];if(!O[N][T]){this.__Lo(U,N,T,V,top,P,M);}
;T++;V+=P;}
;top+=M;N++;}
...
,__IU:function(N,O){var P=[];for(var Q in N){if(O[Q]){P.push(O[Q]);}
;}
;return P;}
...
(function(){var j="x",h="qx.ui.virtual.layer.WidgetCellSpan",g="_spanManager",f="_cellLayer",e="cell.rowspan",d="cell.column",c="cell.colspan",b="cell.row",a="cell.spanning";qx.Class.define(h,{extend:qx.ui.virtual.layer.Abstract,include:[qx.ui.core.MChildrenHandling],construct:function(k,m,n){qx.ui.virtual.layer.Abstract.call(this);this.setZIndex(2);this._spanManager=new qx.ui.virtual.layer.CellSpanManager(m,n);this._cellProvider=k;this.__yW=[];this._cellLayer=new qx.ui.virtual.layer.WidgetCell(this.__Mq());this._cellLayer.setZIndex(0);this._setLayout(new qx.ui.layout.Grow());this._add(this._cellLayer);}
...
;return null;}
;return q;}
,__yW:null,setCellSpan:function(u,v,w,x){var y=u+j+v;this._spanManager.removeCell(y);if(w>1||x>1){this._spanManager.addCell(y,u,v,w,x);}
;qx.ui.core.queue.Widget.add(this);}
,__Mq:function(){var self=this;var B=this._cellProvider;var A=this.__yW;var z={getCellWidget:function(C,D){if(!self._spanMap[C][D]){var E=B.getCellWidget(C,D);}
...
,__Mr:function(G,H,I,J){this._cells=this._spanManager.findCellsInWindow(G,H,I,J);if(this._cells.length>0){this._bounds=this._spanManager.getCellBounds(this._cells,G,H);this._spanMap=this._spanManager.computeCellSpanMap(this._cells,G,H,I,J);}
else {this._bounds=[];this._spanMap=[];for(var i=0;i<I;i++){this._spanMap[G+i]=[];}
;}
;}
,__Ms:function(){var O=this.getChildren();for(var i=O.length-1;i>=0;i--){var N=O[i];if(N!==this._cellLayer){this._cellProvider.poolCellWidget(N);this._remove(N);}
;}
...
;}
;}
,_fullUpdate:function(P,Q,R,S){this.__Mr(P,Q,R.length,S.length);this.__Ms();this._cellLayer.fullUpdate(P,Q,R,S);}
,_updateLayerWindow:function(T,U,V,W){this.__Mr(T,U,V.length,W.length);this.__Ms();this._cellLayer.updateLayerWindow(T,U,V,W);}
},destruct:function(){var X=this._getChildren();for(var i=0;i<X.length;i++){X[i].dispose();}
;this._disposeObjects(g,f);this._cellProvider=this.__yW=this._cells=this._bounds=this._spanMap=null;}
});}
)();
