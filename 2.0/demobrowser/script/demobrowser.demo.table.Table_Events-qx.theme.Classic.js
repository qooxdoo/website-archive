...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Events","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Events-qx.theme.Classic.c08a7169cd71.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="columnVisibilityMenuCreateEnd",j="Boolean",i="icon/16/actions/edit-cut.png",h="A number",g="changeValue",f="Column",d="sorted",c="tableWidthChanged",b="verticalScrollBarChanged",a="icon/16/actions/edit-copy.png",I="icon/22/actions/media-record.png",H="cellContextmenu",G="_events",F="Cut",E="_tableModel",D="Table",C="asc",B="icon/16/actions/edit-paste.png",A="columnVisibilityMenuCreateStart",z="cellDblclick",r="_eventsWin",s="Copy",p="Name",q="desc",n="Paste",o="Event log",l="cellClick",m="metaDataChanged",t="demobrowser.demo.table.Table_Events",u="Row",w="A date",v="Capture events",y="ID",x="";qx.Class.define(t,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return D;}
,main:function(){demobrowser.demo.table.TableDemo.prototype.main.call(this);var K=this._eventsWin=new qx.ui.window.Window(o).set({height:400,width:290,showClose:false,showMinimize:false,contentPadding:0});K.setLayout(new qx.ui.layout.Canvas());K.open();this.getRoot().add(K,{left:670,top:10});this._events=new qx.ui.table.model.Simple();this._events.setColumns([p,u,f]);this._events.setData([]);var J=new qx.ui.table.Table(this._events);J.getTableColumnModel().setColumnWidth(0,150);J.getTableColumnModel().setColumnWidth(1,60);J.getTableColumnModel().setColumnWidth(2,60);K.add(J,{edge:0});}
,createTable:function(){var P=this.createRandomRows(50);var L=this._tableModel=new qx.ui.table.model.Simple();L.setColumns([y,h,w,j]);L.setData(P);L.setColumnEditable(1,true);L.setColumnEditable(2,true);var Q=new qx.ui.table.Table(L);Q.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);var M=Q.getTableColumnModel();M.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());var R=function(e){this._events.addRows([[e.getType(),x,x,x]],0);}
;var O=function(e){this._events.addRows([[e.getType(),e.getRow(),e.getColumn()]],0);}
;var N=function(e){var S=e.getData();this._events.addRows([[e.getType(),S.ascending?C:q,S.columnIndex]],0);}
;Q.addListener(A,R,this);Q.addListener(k,R,this);Q.addListener(c,R,this);Q.addListener(b,R,this);Q.addListener(l,O,this);Q.addListener(z,O,this);Q.addListener(H,O,this);L.addListener(d,N,this);L.addListener(m,R,this);Q.setContextMenu(this.getContextMenu());return Q;}
,createControls:function(){var V=new qx.ui.toolbar.ToolBar();var T,U;U=new qx.ui.toolbar.Part();V.add(U);T=new qx.ui.toolbar.CheckBox(v,I).set({value:true});U.add(T);T.addListener(g,function(e){if(T.isValue()){this._events.setData([]);this._eventsWin.open();}
else {this._eventsWin.close();}
;}
,this);return V;}
,getContextMenu:function(){var W=new qx.ui.menu.Menu();var Y=new qx.ui.menu.Button(F,i);var ba=new qx.ui.menu.Button(s,a);var X=new qx.ui.menu.Button(n,B);W.add(Y);W.add(ba);W.add(X);return W;}
},destruct:function(){this._disposeObjects(E,r,G);}
});}
)();
