...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.Shadow","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.Shadow-qx.theme.Modern.0fa22253adf7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;dJ.length=0;}
,renderSeparator:function(dL,dM){var dN=qx.ui.core.Widget.__hj.getDecoratorElement(dL);this.getContentElement().add(dN);dN.resize(dM.width,dM.height);dN.setStyles({left:dM.left+c,top:dM.top+c});if(!this.__hs){this.__hs=[dN];}
else {this.__hs.push(dN);}
;}
,_computeSizeHint:function(){var dU=this.getWidth();var dT=this.getMinWidth();var dP=this.getMaxWidth();var dS=this.getHeight();var dQ=this.getMinHeight();var dR=this.getMaxHeight();if(qx.core.Environment.get(d)){if(dT!==null&&dP!==null){this.assert(dT<=dP,bs);}
;if(dQ!==null&&dR!==null){this.assert(dQ<=dR,W);}
;}
;var dV=this._getContentHint();var dO=this.getInsets();var dX=dO.left+dO.right;var dW=dO.top+dO.bottom;if(dU==null){dU=dV.width+dX;}
...
;}
,_getContentHint:function(){var ea=this.__hq;if(ea){if(this.hasLayoutChildren()){var eb=ea.getSizeHint();if(qx.core.Environment.get(d)){var dY=bh+this.toString()+df;this.assertInteger(eb.width,cf+dY);this.assertInteger(eb.height,bd+dY);}
;return eb;}
else {return {width:0,height:0};}
;}
else {return {width:100,height:50};}
;}
,_getHeightForWidth:function(ec){var eg=this.getInsets();var ej=eg.left+eg.right;var ei=eg.top+eg.bottom;var eh=ec-ej;var ee=this._getLayout();if(ee&&ee.hasHeightForWidth()){var ed=ee.getHeightForWidth(ec);}
else {ed=this._getContentHeightForWidth(eh);}
;var ef=ed+ei;return ef;}
,_getContentHeightForWidth:function(ek){throw new Error(D);}
,getInsets:function(){var top=this.getPaddingTop();var en=this.getPaddingRight();var ep=this.getPaddingBottom();var eo=this.getPaddingLeft();if(this.__hl){var em=this.__hl.getInsets();if(qx.core.Environment.get(d)){this.assertNumber(em.top,dd+em.top);this.assertNumber(em.right,y+em.right);this.assertNumber(em.bottom,bl+em.bottom);this.assertNumber(em.left,bB+em.left);}
;top+=em.top;en+=em.right;ep+=em.bottom;eo+=em.left;}
;return {"top":top,"right":en,"bottom":ep,"left":eo};}
,getInnerSize:function(){var er=this.getBounds();if(!er){return null;}
;var eq=this.getInsets();return {width:er.width-eq.left-eq.right,height:er.height-eq.top-eq.bottom};}
...
,isSeeable:function(){qx.ui.core.queue.Manager.flush();var es=this.getContainerElement().getDomElement();if(es){return es.offsetWidth>0;}
;return false;}
,_createContainerElement:function(){var eu={"$$widget":this.toHashCode()};if(qx.core.Environment.get(d)){eu.qxType=H;eu.qxClass=this.classname;}
;var et={zIndex:0,position:ct};return new qx.html.Element(bG,et,eu);}
,__ht:function(){var ev=this._createContentElement();if(qx.core.Environment.get(d)){ev.setAttribute(ca,cC);}
;ev.setStyles({"position":ct,"zIndex":10});return ev;}
...
,__hu:null,getLayoutChildren:function(){var ex=this.__hu;if(!ex){return this.__hv;}
;var ey;for(var i=0,l=ex.length;i<l;i++){var ew=ex[i];if(ew.hasUserBounds()||ew.isExcluded()){if(ey==null){ey=ex.concat();}
;qx.lang.Array.remove(ey,ew);}
;}
;return ey||ex;}
,scheduleLayoutUpdate:function(){qx.ui.core.queue.Layout.add(this);}
,invalidateLayoutChildren:function(){var ez=this.__hq;if(ez){ez.invalidateChildrenCache();}
;qx.ui.core.queue.Layout.add(this);}
,hasLayoutChildren:function(){var eA=this.__hu;if(!eA){return false;}
;var eB;for(var i=0,l=eA.length;i<l;i++){eB=eA[i];if(!eB.hasUserBounds()&&!eB.isExcluded()){return true;}
;}
;return false;}
,getChildrenContainer:function(){return this;}
,__hv:[],_getChildren:function(){return this.__hu||this.__hv;}
,_indexOf:function(eC){var eD=this.__hu;if(!eD){return -1;}
;return eD.indexOf(eC);}
,_hasChildren:function(){var eE=this.__hu;return eE!=null&&(!!eE[0]);}
,addChildrenToQueue:function(eF){var eG=this.__hu;if(!eG){return;}
;var eH;for(var i=0,l=eG.length;i<l;i++){eH=eG[i];eF.push(eH);eH.addChildrenToQueue(eF);}
;}
,_add:function(eI,eJ){if(qx.core.Environment.get(d)){this.assertInstance(eI,qx.ui.core.LayoutItem.constructor,ck);}
;if(eI.getLayoutParent()==this){qx.lang.Array.remove(this.__hu,eI);}
;if(this.__hu){this.__hu.push(eI);}
else {this.__hu=[eI];}
;this.__hw(eI,eJ);}
,_addAt:function(eK,eL,eM){if(!this.__hu){this.__hu=[];}
;if(eK.getLayoutParent()==this){qx.lang.Array.remove(this.__hu,eK);}
;var eN=this.__hu[eL];if(eN===eK){eK.setLayoutProperties(eM);}
;if(eN){qx.lang.Array.insertBefore(this.__hu,eK,eN);}
else {this.__hu.push(eK);}
;this.__hw(eK,eM);}
,_addBefore:function(eO,eP,eQ){if(qx.core.Environment.get(d)){this.assertInArray(eP,this._getChildren(),cI);}
;if(eO==eP){return;}
;if(!this.__hu){this.__hu=[];}
;if(eO.getLayoutParent()==this){qx.lang.Array.remove(this.__hu,eO);}
;qx.lang.Array.insertBefore(this.__hu,eO,eP);this.__hw(eO,eQ);}
,_addAfter:function(eR,eS,eT){if(qx.core.Environment.get(d)){this.assertInArray(eS,this._getChildren(),dh);}
;if(eR==eS){return;}
;if(!this.__hu){this.__hu=[];}
;if(eR.getLayoutParent()==this){qx.lang.Array.remove(this.__hu,eR);}
;qx.lang.Array.insertAfter(this.__hu,eR,eS);this.__hw(eR,eT);}
,_remove:function(eU){if(!this.__hu){throw new Error(bT);}
;qx.lang.Array.remove(this.__hu,eU);this.__hx(eU);}
,_removeAt:function(eV){if(!this.__hu){throw new Error(bT);}
;var eW=this.__hu[eV];qx.lang.Array.removeAt(this.__hu,eV);this.__hx(eW);return eW;}
,_removeAll:function(){if(!this.__hu){return [];}
;var eX=this.__hu.concat();this.__hu.length=0;for(var i=eX.length-1;i>=0;i--){this.__hx(eX[i]);}
;qx.ui.core.queue.Layout.add(this);return eX;}
,_afterAddChild:null,_afterRemoveChild:null,__hw:function(eY,fa){if(qx.core.Environment.get(d)){this.assertInstance(eY,qx.ui.core.LayoutItem,x+eY);this.assertNotIdentical(eY,this,bx+eY);if(fa!=null){this.assertType(fa,bN,by+fa);}
;}
;var parent=eY.getLayoutParent();if(parent&&parent!=this){parent._remove(eY);}
;eY.setLayoutParent(this);if(fa){eY.setLayoutProperties(fa);}
else {this.updateLayoutProperties();}
;if(this._afterAddChild){this._afterAddChild(eY);}
;}
,__hx:function(fb){if(qx.core.Environment.get(d)){this.assertNotUndefined(fb);}
;if(fb.getLayoutParent()!==this){throw new Error(cj+fb+bi);}
;fb.setLayoutParent(null);if(this.__hq){this.__hq.invalidateChildrenCache();}
;qx.ui.core.queue.Layout.add(this);if(this._afterRemoveChild){this._afterRemoveChild(fb);}
;}
,capture:function(fc){this.getContainerElement().capture(fc);}
,releaseCapture:function(){this.getContainerElement().releaseCapture();}
,_applyPadding:function(fd,fe,name){this._updateInsets=true;qx.ui.core.queue.Layout.add(this);}
,_createProtectorElement:function(){if(this.__hn){return;}
;var ff=this.__hn=new qx.html.Element;if(qx.core.Environment.get(d)){ff.setAttribute(ca,ci);}
;ff.setStyles({position:ct,top:0,left:0,zIndex:7});var fg=this.getBounds();if(fg){this.__hn.setStyles({width:fg.width+c,height:fg.height+c});}
;if((qx.core.Environment.get(cU)==cW)){ff.setStyles({backgroundImage:dj+qx.util.ResourceManager.getInstance().toUri(bJ)+di,backgroundRepeat:cG});}
;this.getContainerElement().add(ff);}
,_applyDecorator:function(fh,fi){if(qx.core.Environment.get(d)){if(fh&&typeof fh===bN){if(qx.ui.core.Widget.DEBUG){this.warn(cD+K+bq+fh);}
;}
;}
;var fl=qx.ui.core.Widget.__hj;var fj=this.getContainerElement();if(!this.__hn&&!qx.core.Environment.get(cz)){this._createProtectorElement();}
;if(fi){fj.remove(this.__hl);fl.poolDecorator(this.__hl);}
;if(fh){var fk=this.__hl=fl.getDecoratorElement(fh);fk.setStyle(bV,5);fj.add(fk);}
else {delete this.__hl;}
;this._applyBackgroundColor(this.getBackgroundColor());if(this.__hr(fi,fh)){this._updateInsets=true;qx.ui.core.queue.Layout.add(this);}
else if(fh){var fm=this.getBounds();if(fm){fk.resize(fm.width,fm.height);this.__hn&&this.__hn.setStyles({width:fm.width+c,height:fm.height+c});}
;}
;}
,_applyShadow:function(fn,fo){var fw=qx.ui.core.Widget.__hk;var fq=this.getContainerElement();if(fo){fq.remove(this.__hm);fw.poolDecorator(this.__hm);}
;if(fn){var fs=this.__hm=fw.getDecoratorElement(fn);fq.add(fs);var fu=fs.getInsets();fs.setStyles({left:(-fu.left)+c,top:(-fu.top)+c});var ft=this.getBounds();if(ft){var fr=ft.width+fu.left+fu.right;var fp=ft.height+fu.top+fu.bottom;if(fo){var fv=fw.getDecoratorElement(fo).getInsets();fr=fr-fv.left-fv.right;fp=fp-fv.top-fv.bottom;}
;fs.resize(fr,fp);}
;fs.tint(null);}
else {delete this.__hm;}
;}
,_applyToolTipText:function(fx,fy){if(qx.core.Environment.get(bP)){if(this.__hp){return;}
;var fz=qx.locale.Manager.getInstance();this.__hp=fz.addListener(bt,function(){var fA=this.getToolTipText();if(fA&&fA.translate){this.setToolTipText(fA.translate());}
;}
,this);}
;}
...
;this.getContainerElement().setStyle(ba,fL,qx.core.Environment.get(cU)==bv);}
,_applyBackgroundColor:function(fN,fO){var fP=this.getBackgroundColor();var fR=this.getContainerElement();if(this.__hl){this.__hl.tint(fP);fR.setStyle(bI,null);}
else {var fQ=qx.theme.manager.Color.getInstance().resolve(fP);fR.setStyle(bI,fQ);}
;}
,_applyFont:function(fS,fT){}
,__hy:null,$$stateChanges:null,_forwardStates:null,hasState:function(fU){var fV=this.__hy;return !!fV&&!!fV[fU];}
,addState:function(fW){var fX=this.__hy;if(!fX){fX=this.__hy={};}
;if(fX[fW]){return;}
;this.__hy[fW]=true;if(fW===cs){this.syncAppearance();}
else if(!qx.ui.core.queue.Visibility.isVisible(this)){this.$$stateChanges=true;}
else {qx.ui.core.queue.Appearance.add(this);}
;var forward=this._forwardStates;var gb=this.__hB;if(forward&&forward[fW]&&gb){var fY;for(var ga in gb){fY=gb[ga];if(fY instanceof qx.ui.core.Widget){gb[ga].addState(fW);}
;}
;}
;}
,removeState:function(gc){var gd=this.__hy;if(!gd||!gd[gc]){return;}
;delete this.__hy[gc];if(gc===cs){this.syncAppearance();}
else if(!qx.ui.core.queue.Visibility.isVisible(this)){this.$$stateChanges=true;}
else {qx.ui.core.queue.Appearance.add(this);}
;var forward=this._forwardStates;var gg=this.__hB;if(forward&&forward[gc]&&gg){for(var gf in gg){var ge=gg[gf];if(ge instanceof qx.ui.core.Widget){ge.removeState(gc);}
;}
;}
;}
,replaceState:function(gh,gi){var gj=this.__hy;if(!gj){gj=this.__hy={};}
;if(!gj[gi]){gj[gi]=true;}
;if(gj[gh]){delete gj[gh];}
;if(!qx.ui.core.queue.Visibility.isVisible(this)){this.$$stateChanges=true;}
else {qx.ui.core.queue.Appearance.add(this);}
;var forward=this._forwardStates;var gm=this.__hB;if(forward&&forward[gi]&&gm){for(var gl in gm){var gk=gm[gl];if(gk instanceof qx.ui.core.Widget){gk.replaceState(gh,gi);}
;}
;}
;}
,__hz:null,__hA:null,syncAppearance:function(){var gr=this.__hy;var gq=this.__hz;var gs=qx.theme.manager.Appearance.getInstance();var go=qx.core.Property.$$method.setThemed;var gw=qx.core.Property.$$method.resetThemed;if(this.__hA){delete this.__hA;if(gq){var gn=gs.styleFrom(gq,gr,null,this.getAppearance());gq=null;}
;}
;if(!gq){var gp=this;var gv=[];do {gv.push(gp.$$subcontrol||gp.getAppearance());}
while(gp=gp.$$subparent);gq=gv.reverse().join(L).replace(/#[0-9]+/g,J);this.__hz=gq;}
;var gt=gs.styleFrom(gq,gr,null,this.getAppearance());if(gt){if(gn){for(var gu in gn){if(gt[gu]===undefined){this[gw[gu]]();}
;}
;}
;if(qx.core.Environment.get(d)){for(var gu in gt){if(!this[go[gu]]){throw new Error(this.classname+C+gu+ce+gq);}
;}
;}
...
;}
,updateAppearance:function(){this.__hA=true;qx.ui.core.queue.Appearance.add(this);var gB=this.__hB;if(gB){var gz;for(var gA in gB){gz=gB[gA];if(gz instanceof qx.ui.core.Widget){gz.updateAppearance();}
;}
;}
;}
...
;gH.setAttribute(cS,gI);if((qx.core.Environment.get(cU)==cW&&parseFloat(qx.core.Environment.get(cB))<8)||(qx.core.Environment.get(cU)==cW&&qx.core.Environment.get(bD)<8)){gH.setAttribute(w,bj);}
else {gH.setStyle(da,dc);}
;}
else {if(gH.isNativelyFocusable()){gH.setAttribute(cS,-1);}
else if(gG){gH.setAttribute(cS,null);}
;}
;}
,_applyKeepFocus:function(gJ){var gK=this.getFocusElement();gK.setAttribute(bu,gJ?cV:null);}
,_applyKeepActive:function(gL){var gM=this.getContainerElement();gM.setAttribute(bo,gL?cV:null);}
,_applyTabIndex:function(gN){if(gN==null){gN=1;}
else if(gN<1||gN>32000){throw new Error(T);}
...
;this.getContainerElement().setAttribute(cN,gW?cV:null);}
,_applyDroppable:function(gY,ha){if(!this.isEnabled()&&gY===true){gY=false;}
;this.getContainerElement().setAttribute(X,gY?cV:null);}
...
,scrollChildIntoView:function(hd,he,hf,hg){hg=typeof hg==bm?true:hg;var hh=qx.ui.core.queue.Layout;var parent;if(hg){hg=!hh.isScheduled(hd);parent=hd.getLayoutParent();if(hg&&parent){hg=!hh.isScheduled(parent);if(hg){parent.getChildren().forEach(function(hi){hg=hg&&!hh.isScheduled(hi);}
);}
;}
;}
...
,hasChildControl:function(hp){if(!this.__hB){return false;}
;return !!this.__hB[hp];}
,__hB:null,_getCreatedChildControls:function(){return this.__hB;}
,getChildControl:function(hq,hr){if(!this.__hB){if(hr){return null;}
;this.__hB={};}
;var hs=this.__hB[hq];if(hs){return hs;}
...
;return false;}
,_createChildControl:function(hz){if(!this.__hB){this.__hB={};}
else if(this.__hB[hz]){throw new Error(cF+hz+Y);}
...
;if(!hA){throw new Error(bC+hz);}
;hA.$$subcontrol=hz;hA.$$subparent=this;var hB=this.__hy;var forward=this._forwardStates;if(hB&&forward&&hA instanceof qx.ui.core.Widget){for(var hC in hB){if(forward[hC]){hA.addState(hC);}
;}
;}
;this.fireDataEvent(bE,hA);return this.__hB[hz]=hA;}
,_createChildControlImpl:function(hE,hF){return null;}
,_disposeChildControls:function(){var hJ=this.__hB;if(!hJ){return;}
...
;if(!qx.core.ObjectRegistry.inShutDown){var hY=qx.ui.core.Widget;var hX=this.getContainerElement();if(this.__hl){hX.remove(this.__hl);hY.__hj.poolDecorator(this.__hl);}
;if(this.__hm){hX.remove(this.__hm);hY.__hk.poolDecorator(this.__hm);}
...
(function(){var d="main",c="demobrowser.demo.ui.Shadow",b="yellow",a="shadow-window";qx.Class.define(c,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var e=new qx.ui.core.Widget().set({decorator:a});this.getRoot().add(e,{top:10,left:10});var e=new qx.ui.core.Widget().set({shadow:a,decorator:d,backgroundColor:b});this.getRoot().add(e,{top:10,left:150});}
}});}
)();
