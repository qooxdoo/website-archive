...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.FiniteStateMachine","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine-qx.theme.Classic.40f66b874253.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="string",l="Saved-state stack is empty",k="State ",j=" already exists",h="__LB",g="qx.util.fsm.FiniteStateMachine",f="#actionsAfterOnentry",d="Saved-state stack is full",c="#actionsBeforeOnentry",b=": Pushed event: ",A="Machine started with no available states",z="#entry",y="__LA",x=": Queued event: ",w="",v=" on ",u="Invalid state: not an instance of ",s="Number",r="qx.util.fsm.State",q="#",o="  ",p=" => ",n="String";qx.Class.define(g,{extend:qx.core.Object,construct:function(B){qx.core.Object.call(this);this.setName(B);this.__hy={};this.__Ly=null;this.__Lz=[];this.__LA=[];this.__LB=[];this.__LC={};this.__LD={};this.__LE={};this.__LF={};this.__LG={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:n,nullable:true},state:{check:n,nullable:true},previousState:{check:n,nullable:true},nextState:{check:n,nullable:true},maxSavedStates:{check:s,init:2},debugFlags:{check:s,init:7}},members:{__hy:null,__Ly:null,__LA:null,__LB:null,__Lz:null,__LC:null,__LD:null,__LE:null,__LF:null,__LG:null,__LH:false,addState:function(C){if(!C instanceof qx.util.fsm.State){throw new Error(u+r);}
;var D=C.getName();if(D in this.__hy){throw new Error(k+D+j);}
;if(this.__Ly==null){this.__Ly=D;}
;this.__hy[D]=C;}
,replaceState:function(E,F){if(!E instanceof qx.util.fsm.State){throw new Error(u+r);}
;var H=E.getName();var G=this.__hy[H];this.__hy[H]=E;if(F){G._bNeedDispose=true;}
;return G;}
,addObject:function(I,J,K){var M=J.toHashCode();this.__LD[I]=M;this.__LE[M]=I;this.__LC[I]=J;if(!K){return;}
;if(typeof (K)==m){K=[K];}
;for(var i=0;i<K.length;i++){var L=K[i];if(!this.__LF[L]){this.__LF[L]={};}
;this.__LF[L][I]=true;if(!this.__LG[I]){this.__LG[I]=[];}
;this.__LG[I].push(L);}
;}
,removeObject:function(N){var P;var O;var Q;var R;P=this.__LD[N];if(this.__LG[N]){for(var i=0;i<this.__LG[N].length;i++){O=this.__LG[N][i];delete this.__LF[O][N];R=true;for(Q in this.__LF[O]){R=false;break;}
;if(R){delete this.__LF[O];}
;}
;delete this.__LG[N];}
;delete this.__LE[P];delete this.__LD[N];delete this.__LC[N];}
,getObject:function(S){return this.__LC[S];}
,getFriendlyName:function(T){var U=T?T.toHashCode():null;return U?this.__LE[U]:null;}
,getGroupObjects:function(V){var a=[];for(var name in this.__LF[V]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var W in this.__LD){var Y=this.__LD[W];var X=this.getObject(W);this.debug(W+p+Y);this.debug(o+Y+p+this.__LE[Y]);this.debug(o+W+p+this.getObject(W));this.debug(o+this.getObject(W)+p+this.getFriendlyName(X));}
;}
,_getInternalData:function(){return ({"states":this.__hy,"startState":this.__Ly,"eventQueue":this.__LA,"blockedEvents":this.__LB,"savedStates":this.__Lz,"friendlyToObject":this.__LC,"friendlyToHash":this.__LD,"hashToFriendly":this.__LE,"groupToFriendly":this.__LF,"friendlyToGroups":this.__LG});}
,start:function(){var bb=this.__Ly;if(bb==null){throw new Error(A);}
;this.setState(bb);this.setPreviousState(null);this.setNextState(null);var ba=(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL);if(ba){this.debug(this.getName()+q+bb+c);}
;this.__hy[bb].getAutoActionsBeforeOnentry()(this);if(ba){this.debug(this.getName()+q+bb+z);}
;this.__hy[bb].getOnentry()(this,null);if(ba){this.debug(this.getName()+q+bb+f);}
;this.__hy[bb].getAutoActionsAfterOnentry()(this);}
,pushState:function(bc){if(this.__Lz.length>=this.getMaxSavedStates()){throw new Error(d);}
;if(bc===true){this.__Lz.push(this.getState());}
else if(bc){this.__Lz.push(bc);}
else {this.__Lz.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Lz.length==0){throw new Error(l);}
;return this.__Lz.pop();}
,postponeEvent:function(event){this.__LB.unshift(event);}
,enqueueEvent:function(event,bd){if(bd){this.__LA.push(event);}
else {this.__LA.unshift(event);}
;if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){var be=this.getFriendlyName(event.getTarget());if(bd){this.debug(this.getName()+b+event.getType()+(be?v+be:w));}
else {this.debug(this.getName()+x+event.getType()+(be?v+be:w));}
;}
;}
,eventListener:function(event){var e=event.clone();this.enqueueEvent(e,false);this.__LI();}
...
,this,bl||20);}
,__LI:function(){if(this.__LH){return;}
;this.__LH=true;while(this.__LA.length>0){var event=this.__LA.pop();var bm=this.__LJ(event);if(bm){event.dispose();}
;}
;this.__LH=false;}
,__LJ:function(event){var bp;var br;var bo;var bz;var bx;var e;var bv;var bn=this.getDebugFlags();var by=bn&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var bs=bn&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var bw=bn&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var bu=bn&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var bq=this.getFriendlyName(event.getTarget());if(by){this.debug(this.getName()+": Process event: "+event.getType()+(bq?" on "+bq:""));}
;bp=this.getState();bz=this.__hy[bp];bx=bz.transitions;e=bz.getEvents()[event.getType()];if(!e){if(by){this.debug(this.getName()+": Event '"+event.getType()+"'"+" not handled.  Ignoring.");}
;return true;}
...
;this.__LB.unshift(event);return false;default:if(typeof (bv)=="string"){if(bx[bv]){var bt=bx[bv];bx={};bx[bv]=bt;}
else {throw new Error("Explicit transition "+bv+" does not exist");}
;break;}
;};for(var t in bx){var bt=bx[t];switch(bt.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error("Transition "+bp+":"+t+" returned a value other than "+"true, false, or null.");};br=bt.getNextState();if(typeof (br)=="string"){if(!br in this.__hy){throw new Error("Attempt to transition to nonexistent state "+br);}
;this.setNextState(br);}
else {switch(br){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:br=bp;this.setNextState(br);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Lz.length==0){throw new Error("Attempt to transition to POP_STATE_STACK "+"while state stack is empty.");}
;br=this.__Lz.pop();this.setNextState(br);break;default:throw new Error("Internal error: invalid nextState");break;};}
...
;bz=this.__hy[this.getNextState()];this.setPreviousState(bp);this.setState(this.getNextState());this.setNextState(null);bo=bp;bp=br;br=undefined;if(bw){this.debug(this.getName()+"#"+bp+"#autoActionsBeforeOnentry");}
...
;bz.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__LB.length;i++){e=this.__LB.pop();this.__LA.unshift(e);}
...
;return true;}
},destruct:function(){this._disposeArray(y);this._disposeArray(h);this.__Lz=this.__hy=null;}
});}
)();
(function(){var s="State info must be an object",r="  var groupObjects = ",q="parameters",p=")",o=" fsm.getObject('",n="__Lw",m="groups",l="Invalid function parameters: ",k="Invalid onexit type: ",h="  for (var i = 0; i < groupObjects.length; i++)",bB="Invalid numeric value in events object: ",bA="__Lr",bz="__Lp",by="Invalid state name",bx=" value: ",bw="');",bv="State ",bu="events must be an object",bt="onexit",bs="Invalid function request type: ",z="Invalid friendly name in 'objects' list: ",A="  fsm.debug(ex);",x="__Lv",y="Invalid onentry type: ",v="    fsm.getObject(objName).",w=")(fsm)",t="events",u="Invalid ",H="__Lt",I="try",V="    var objName = groupObjects[i];",R="Invalid value in events object: ",be="').",Y="objects",bo="expected object, found ",bj="qx.util.fsm.Transition",M="The events object must be provided in new state info",br="catch(ex)",bq="  {",bp="    fsm.getGroupObjects('",L="Invalid value in events object ",O="Invalid transition: not an instance of ",Q="  }",T="__Lq",W="__Lu",ba="__Ls",bg="Invalid 'objects' list: expected array, got ",bl="Invalid numeric value in events object ",B="qx.util.fsm.State",C="onentry",N="Invalid function request parameter type: ",bd="Invalid 'groups' list: expected array, got ",bc="fsm",bb=",",bi="Adding user-provided field to state: ",bh="autoActionsAfterOnentry",X='"',bf="autoActionsBeforeOnexit",b="autoActionsBeforeOnentry",bk="}",D="): ",E="{",S="expected array, found ",c="autoActionsAfterOnexit",d="undefined",K=";",F="function",G="(",J="number",U="string",bn=": ",bm="object",P="context";qx.Class.define(B,{extend:qx.core.Object,construct:function(bC,bD){var bF;qx.core.Object.call(this);this.setName(bC);if(typeof (bD)!=bm){throw new Error(s);}
;bF=bD.context||window;this.setUserData(P,bF);for(var bE in bD){switch(bE){case C:this.setOnentry(this.__Lx(bD[bE],bF));break;case bt:this.setOnexit(this.__Lx(bD[bE],bF));break;case b:this.setAutoActionsBeforeOnentry(bD[bE]);break;case bh:this.setAutoActionsAfterOnentry(bD[bE]);break;case bf:this.setAutoActionsBeforeOnexit(bD[bE]);break;case c:this.setAutoActionsAfterOnexit(bD[bE]);break;case t:this.setEvents(bD[bE]);break;case P:break;default:this.setUserData(bE,bD[bE]);this.debug(bv+bC+bn+bi+bE);break;};}
;if(!this.getEvents()){throw new Error(M);}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(bG,bH,bI){if(typeof (bH)!=bm){throw new Error(u+bG+bx+typeof (bH));}
;var bN;var bO=I+E;var bL;var bJ;for(var f in bH){var bK=bH[f];if(!bK instanceof Array){throw new Error(bs+S+typeof (bK));}
;for(var i=0;i<bK.length;i++){bJ=bK[i];if(typeof (bJ)!=bm){throw new Error(N+bo+typeof (bK[bL]));}
;var bM=bJ[q];if(!bM){bM=[];}
else {if(!bM instanceof Array){throw new Error(l+S+typeof (bM));}
;}
;bN=f+G;for(var j=0;j<bM.length;j++){if(j!=0){bN+=bb;}
;if(typeof (bM[j])==F){bN+=G+bM[j]+w;}
else if(typeof (bM[j])==U){bN+=X+bM[j]+X;}
else {bN+=bM[j];}
;}
;bN+=p;var a=bJ[Y];if(!a){a=[];}
else if(!a instanceof Array){throw new Error(bg+typeof (a));}
;for(var j=0;j<a.length;j++){if(typeof (a[j])!=U){throw new Error(z+a[j]);}
;bO+=o+a[j]+be+bN+K;}
;var g=bJ[m];if(g){if(!g instanceof Array){throw new Error(bd+typeof (g));}
;for(j=0;j<g.length;j++){bO+=r+bp+g[j]+bw+h+bq+V+v+bN+K+Q;}
;}
;}
;}
;bO+=bk+br+E+A+bk;return qx.lang.Function.bind(new Function(bc,bO),bI);}
},properties:{name:{transform:bz,nullable:true},onentry:{transform:T,nullable:true,init:function(bP,event){}
},onexit:{transform:bA,nullable:true,init:function(bQ,event){}
},autoActionsBeforeOnentry:{transform:H,nullable:true,init:function(bR,event){}
},autoActionsAfterOnentry:{transform:W,nullable:true,init:function(bS,event){}
},autoActionsBeforeOnexit:{transform:x,nullable:true,init:function(bT,event){}
},autoActionsAfterOnexit:{transform:n,nullable:true,init:function(bU,event){}
},events:{transform:ba,nullable:true}},members:{__Lp:function(bV){if(typeof (bV)!=U||bV.length<1){throw new Error(by);}
;return bV;}
,__Lq:function(bW){switch(typeof (bW)){case d:return function(bX,event){}
;case F:return qx.lang.Function.bind(bW,this.getUserData(P));default:throw new Error(y+typeof (bW));};}
,__Lr:function(bY){switch(typeof (bY)){case d:return function(ca,event){}
;case F:return qx.lang.Function.bind(bY,this.getUserData(P));default:throw new Error(k+typeof (bY));return null;};}
,__Ls:function(cb){if(typeof (cb)!=bm){throw new Error(bu);}
;for(var e in cb){var cc=cb[e];if(typeof (cc)==J&&cc!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&cc!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(bB+e+bn+cc);}
else if(typeof (cc)==bm){for(var cd in cc){if(typeof (cc[cd])==J&&cc[cd]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&cc[cd]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(bl+G+e+D+cd+bn+cc[cd]);}
else if(typeof (cc[cd])!=U&&typeof (cc[cd])!=J){throw new Error(L+G+e+D+cd+bn+cc[cd]);}
;}
;}
else if(typeof (cc)!=U&&typeof (cc)!=J){throw new Error(R+e+bn+cb[e]);}
;;}
;return cb;}
,__Lt:function(ce){return qx.util.fsm.State._commonTransformAutoActions(b,ce,this.getUserData(P));}
,__Lu:function(cf){return qx.util.fsm.State._commonTransformAutoActions(bh,cf,this.getUserData(P));}
,__Lv:function(cg){return qx.util.fsm.State._commonTransformAutoActions(bf,cg,this.getUserData(P));}
,__Lw:function(ch){return qx.util.fsm.State._commonTransformAutoActions(c,ch,this.getUserData(P));}
,__Lx:function(f,ci){if(typeof (f)==F){f=qx.lang.Function.bind(f,ci);}
;return f;}
,addTransition:function(cj){if(!cj instanceof qx.util.fsm.Transition){throw new Error(O+bj);}
;this.transitions[cj.getName()]=cj;}
}});}
)();
(function(){var r="predicate",q="nextState",p="autoActionsBeforeOntransition",o=": ",n="function",m="String",l="__LL",k="Transition ",j="Adding user-provided field to transition: ",i="__LM",c="__LK",h="autoActionsAfterOntransition",g="__LO",b="qx.util.fsm.Transition",a="ontransition",e="__LN",d="context";qx.Class.define(b,{extend:qx.core.Object,construct:function(s,t){var v;qx.core.Object.call(this);this.setName(s);v=t.context||window;this.setUserData(d,v);for(var u in t){switch(u){case r:this.setPredicate(this.__Lx(t[u],v));break;case q:this.setNextState(t[u]);break;case p:this.setAutoActionsBeforeOntransition(this.__Lx(t[u],v));break;case h:this.setAutoActionsAfterOntransition(this.__Lx(t[u],v));break;case a:this.setOntransition(this.__Lx(t[u],v));break;case d:break;default:this.setUserData(u,t[u]);this.debug(k+s+o+j+u);break;};}
;}
,properties:{name:{check:m,nullable:true},predicate:{init:function(w,event){return true;}
,transform:c},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:l},autoActionsBeforeOntransition:{init:function(x,event){}
,transform:i},autoActionsAfterOntransition:{init:function(y,event){}
,transform:e},ontransition:{init:function(z,event){}
,transform:g}},members:{__LK:function(A){switch(typeof (A)){case "undefined":return function(B,event){return true;}
;case "boolean":return function(C,event){return A;}
;case "function":return qx.lang.Function.bind(A,this.getUserData("context"));default:throw new Error("Invalid transition predicate type: "+typeof (A));break;};}
,__LL:function(D){switch(typeof (D)){case "string":return D;case "number":switch(D){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return D;default:throw new Error("Invalid transition nextState value: "+D+": "+"nextState must be an explicit state name, "+"or one of the Fsm.StateChange constants");};break;default:throw new Error("Invalid transition nextState type: "+typeof (D));break;};}
,__LM:function(E){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOntransition",E,this.getUserData("context"));}
,__LN:function(F){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOntransition",F,this.getUserData("context"));}
,__LO:function(G){switch(typeof (G)){case "undefined":return function(H,event){}
;case "function":return qx.lang.Function.bind(G,this.getUserData("context"));default:throw new Error("Invalid ontransition type: "+typeof (G));break;};}
,__Lx:function(f,I){if(typeof (f)==n){f=qx.lang.Function.bind(f,I);}
;return f;}
}});}
)();
