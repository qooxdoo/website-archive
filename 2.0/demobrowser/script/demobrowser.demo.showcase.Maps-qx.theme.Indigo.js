...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.showcase.Maps","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.showcase.Maps-qx.theme.Indigo.a0d8e7b1f6a1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j="Google Maps",i="<b>",h="center_changed",g="Karlsruhe",f="Yahoo Maps",e="</b>",d="demobrowser.demo.showcase.Maps",c='zIndex',b="appear",a="main";qx.Class.define(d,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var k=this._createYahooMap();var l=this._createGoogleMap();this.getRoot().add(this._createMapContainer(f,k),{left:20,top:20});this.getRoot().add(this._createMapContainer(j,l),{left:490,top:20});}
,_createMapContainer:function(m,n){var p=new qx.ui.container.Composite();p.setLayout(new qx.ui.layout.VBox().set({spacing:10}));p.setWidth(450);p.setHeight(400);var o=new qx.ui.basic.Label(i+m+e).set({rich:true});p.add(o);p.add(n);return p;}
,_createYahooMap:function(){var q=new qx.ui.core.Widget().set({width:450,height:400,decorator:a});q.addListenerOnce(b,function(){var r=new YMap(q.getContentElement().getDomElement());r.addTypeControl();r.setMapType(YAHOO_MAP_REG);r.drawZoomAndCenter(g,5);}
);return q;}
,_createGoogleMap:function(){var s=new qx.ui.core.Widget().set({width:450,height:400});s.setDecorator(a);s.addListenerOnce(b,function(){var t=new google.maps.Map(s.getContentElement().getDomElement(),{zoom:13,mapTypeId:google.maps.MapTypeId.ROADMAP});google.maps.event.addListenerOnce(t,h,function(){window.setTimeout(function(){var u=s.getContentElement().getStyle(c);s.getContentElement().getDomElement().style.zIndex=u;}
,500);}
);t.setCenter(new google.maps.LatLng(49.011899,8.403311));}
);return s;}
}});}
)();
