...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.Tree","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Tree-qx.theme.Simple.4406f5972af0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="Windows (C:)",k="icon/22/places/user-desktop.png",j="Documents (D:)",h="Inbox",g="Desktop",f="root",d="Open mode:",c="Presets",b="Hide Root Node",a="Network",S="Control",R=" items",Q="Sent",P="Files",O="Show Open Items",N="Workspace",M="Enable drag selection",L="Selection: ",K="Junk #",J="demobrowser.demo.widget.Tree",s="Data",t="click",q="Show root open button",r="none",o="Enable multi selection",p="double click",m="Selection:",n="Show Selected Items",u="Root node:",v="Toggle Height",B="Edit",A="Show Items",D="Trash",C="changeSelection",F="mode",E="multi",x="single",I="",H="g",G="\n",w=",",y="changeValue",z="execute";qx.Class.define(J,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var W=new qx.ui.layout.HBox();W.set({spacing:20});var V=new qx.ui.container.Composite(W);this.getRoot().add(V);var T=this.getTree();V.add(T);var U=this.getCommandFrame(T);V.add(U);}
,getTree:function(){var bh=new qx.ui.tree.Tree().set({width:200,height:400});var bm=new qx.ui.tree.TreeFolder(f);bm.setOpen(true);bh.setRoot(bm);var bk=new qx.ui.tree.TreeFolder(g);bk.setOpen(true);bk.setIcon(k);bm.add(bk);var bc=new qx.ui.tree.TreeFolder(P);var ba=new qx.ui.tree.TreeFolder(N);var bb=new qx.ui.tree.TreeFolder(a);var Y=new qx.ui.tree.TreeFolder(D);bk.add(bc,ba,bb,Y);var bi=new qx.ui.tree.TreeFile(l);var X=new qx.ui.tree.TreeFile(j);ba.add(bi,X);var bl=new qx.ui.tree.TreeFolder(h);var bd=new qx.ui.tree.TreeFolder(c);var bg=new qx.ui.tree.TreeFolder(Q);var bf=new qx.ui.tree.TreeFolder(D);for(var i=0;i<30;i++){bf.add(new qx.ui.tree.TreeFile(K+i));}
;var bj=new qx.ui.tree.TreeFolder(s);var be=new qx.ui.tree.TreeFolder(B);bl.add(bd,bg,bf,bj,be);bm.add(bl);return bh;}
,getCommandFrame:function(bn){var bx=new qx.ui.groupbox.GroupBox(S);var bv=4;bx.setLayout(new qx.ui.layout.Grid(5,3));var bE=0;bx.add(new qx.ui.basic.Label(L).set({paddingTop:4}),{row:bE,column:0});var bB=new qx.ui.form.TextField().set({readOnly:true});bx.add(bB,{row:bE++,column:1});bn.addListener(C,function(e){var bF=e.getData();if(bF.length>0){if(this.getSelectionMode()===E){bB.setValue(bF.length+R);}
else {bB.setValue(bF[0].getLabel());}
;}
else {bB.setValue(I);}
;}
);bx.add(new qx.ui.core.Spacer(bv,bv),{row:bE++,column:0});bx.add(new qx.ui.basic.Label(d),{row:bE,column:0});var br={"click":t,"dblclick":p,"none":r};var bp=new qx.ui.form.RadioGroup();for(var bz in br){var bq=new qx.ui.form.RadioButton(br[bz]).set({value:bz==bn.getOpenMode()});bq.setUserData(F,bz);bp.add(bq);bx.add(bq,{row:bE++,column:1});}
;bp.addListener(C,function(e){bn.setOpenMode(e.getData()[0].getUserData(F));}
);bx.add(new qx.ui.core.Spacer(bv,bv),{row:bE++,column:0});bx.add(new qx.ui.basic.Label(m),{row:bE,column:0});var bt=new qx.ui.form.CheckBox(o);bx.add(bt,{row:bE++,column:1});bt.addListener(y,function(e){var bG=e.getData();bn.setSelectionMode(bG?E:x);}
);var bu=new qx.ui.form.CheckBox(M);bx.add(bu,{row:bE++,column:1});bu.addListener(y,function(e){var bH=e.getData();bn.setDragSelection(bH);if(!bt.getValue()){bt.setValue(true);}
;}
);bx.add(new qx.ui.core.Spacer(bv,bv),{row:bE++,column:0});bx.add(new qx.ui.basic.Label(u),{row:bE,column:0});var bC=new qx.ui.form.CheckBox(b);bx.add(bC,{row:bE++,column:1});bC.addListener(y,function(e){bn.setHideRoot(e.getData());}
);var bs=new qx.ui.form.CheckBox(q);bx.add(bs,{row:bE++,column:1});bs.addListener(y,function(e){bn.setRootOpenClose(e.getData());}
);bx.add(new qx.ui.core.Spacer(bv,bv),{row:bE++,column:0});var bA=new qx.ui.form.Button(A);bx.add(bA,{row:bE++,column:1});bA.addListener(z,function(e){alert((I+bn.getItems()).replace(w,G,H));}
);var bo=new qx.ui.form.Button(O);bx.add(bo,{row:bE++,column:1});bo.addListener(z,function(e){alert((I+bn.getItems(true,false)).replace(w,G,H));}
);var by=new qx.ui.form.Button(n);bx.add(by,{row:bE++,column:1});by.addListener(z,function(e){if(this.getSelectionMode()===x){alert(this.getSelection());}
else {alert(this.getSelection().toString().replace(w,G,H));}
;}
,bn);var bw=new qx.ui.form.Button(v);bx.add(bw,{row:bE++,column:1});var bD=true;bw.addListener(z,function(e){bn.setHeight(bD?600:400);bD=!bD;}
);return bx;}
}});}
)();
