...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.Offline","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Offline-qx.theme.Classic.d8084cdbf9f0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="demobrowser.demo.data.Offline",l="html.storage.local",k="Remove item",j="The added data will be keept on reload. Just add or remove strings ",i="and reload to see the result.",h="Add item",g="initial entry",f="Offline storage (local) not supported.",e="qx-offline-demo",d="<b>Offline data store</b><br/>",a="model",c="item ",b="execute";qx.Class.define(m,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);if(!qx.core.Environment.get(l)){var q=new qx.ui.basic.Label(f);this.getRoot().add(q,{left:30,top:30});return;}
;var content=new qx.ui.container.Composite();var t=new qx.ui.layout.Grid();t.setSpacing(10);content.setLayout(t);this.getRoot().add(content,{left:10,top:80});var p=new qx.ui.form.List();content.add(p,{row:0,column:0,rowSpan:2});var s=new qx.data.controller.List(null,p);var w=new qx.data.store.Offline(e);if(w.getModel()==null){var v=qx.data.marshal.Json.createModel([g]);w.setModel(v);}
;w.bind(a,s,a);var n=new qx.ui.form.Button(h);var u=new qx.ui.form.Button(k);var r=new qx.ui.form.TextField();r.setValue(c+w.getModel().length);content.add(r,{row:0,column:1,rowSpan:2});content.add(n,{row:0,column:2});content.add(u,{row:1,column:2});n.setAllowGrowY(false);u.setAllowGrowY(false);t.setRowFlex(1,1);n.addListener(b,function(){w.getModel().push(r.getValue());r.setValue(c+w.getModel().length);}
,this);u.addListener(b,function(){w.getModel().remove(s.getSelection().getItem(0));}
,this);var o=new qx.ui.basic.Label();o.setRich(true);o.setWidth(300);o.setValue(d+j+i);this.getRoot().add(o,{left:10,top:10});}
}});}
)();
...
(function(){var g="qx.data.store.Offline",f="session",d="qx.debug",c="changeModel",b="changeBubble",a="_applyModel";qx.Class.define(g,{extend:qx.core.Object,construct:function(h,i){qx.core.Object.call(this);try{if(qx.core.Environment.get(d)){this.assertNotUndefined(h);}
;}
catch(e){this.dispose();throw e;}
;if(i==f){this._storage=qx.bom.Storage.getSession();}
else {this._storage=qx.bom.Storage.getLocal();}
;this._marshaler=new qx.data.marshal.Json();this._key=h;this._initializeModel();}
,properties:{model:{nullable:true,event:c,apply:a}},members:{_storage:null,__Ch:null,_applyModel:function(j,k){if(k){k.removeListenerById(this.__Ch);k.dispose();this.__Ch=null;}
;if(j){this.__Ch=j.addListener(b,this.__Ci,this);this.__Ci();}
else {this._storage.removeItem(this._key);}
;}
,__Ci:function(){var l=qx.util.Serializer.toNativeObject(this.getModel());this._storage.setItem(this._key,l);}
,_initializeModel:function(){this._setModel(this._storage.getItem(this._key));}
,_setModel:function(m){this._marshaler.toClass(m,true);if(this.getModel()){this.getModel().dispose();}
;this.setModel(this._marshaler.toModel(m,true));}
...
(function(){var f="local",d="session",c="Storage",b="qx.bom.storage.Web",a="Storage full.";qx.Bootstrap.define(b,{statics:{__Cj:null,__Ck:null,getLocal:function(){if(this.__Cj){return this.__Cj;}
;return this.__Cj=new qx.bom.storage.Web(f);}
,getSession:function(){if(this.__Ck){return this.__Ck;}
;return this.__Ck=new qx.bom.storage.Web(d);}
},construct:function(g){this.__mG=g;}
,members:{__mG:null,getStorage:function(){return window[this.__mG+c];}
,getLength:function(){return this.getStorage(this.__mG).length;}
,setItem:function(h,j){j=qx.lang.Json.stringify(j);try{this.getStorage(this.__mG).setItem(h,j);}
catch(e){throw new Error(a);}
;}
,getItem:function(k){var l=this.getStorage(this.__mG).getItem(k);if(qx.lang.Type.isString(l)){l=qx.lang.Json.parse(l);}
else if(l&&l.value&&qx.lang.Type.isString(l.value)){l=qx.lang.Json.parse(l.value);}
;return l;}
,removeItem:function(m){this.getStorage(this.__mG).removeItem(m);}
,clear:function(){var n=this.getStorage(this.__mG);if(!n.clear){for(var i=n.length-1;i>=0;i--){n.removeItem(n.key(i));}
;}
else {n.clear();}
;}
,getKey:function(o){return this.getStorage(this.__mG).key(o);}
,forEach:function(p,q){var length=this.getLength();for(var i=0;i<length;i++){var r=this.getKey(i);p.call(q,r,this.getItem(r));}
;}
}});}
)();
(function(){var k="session",j="head",h="none",g="qx.bom.storage.UserData",f="div",e="local",d="qxtest",c="#default#userdata",b="display",a="qx";qx.Bootstrap.define(g,{statics:{__Cj:null,__Ck:null,__bV:0,getLocal:function(){if(this.__Cj){return this.__Cj;}
;return this.__Cj=new qx.bom.storage.UserData(e);}
,getSession:function(){if(this.__Ck){return this.__Ck;}
;return this.__Ck=new qx.bom.storage.UserData(k);}
},construct:function(l){this.__Cl=document.createElement(f);this.__Cl.style[b]=h;document.getElementsByTagName(j)[0].appendChild(this.__Cl);this.__Cl.addBehavior(c);this.__Cm=l;this.__Cl.load(this.__Cm);this.__Aj={};this.__Cn={};var m=this.__Cl.getAttribute(a+qx.bom.storage.UserData.__bV);while(m!=undefined){m=qx.lang.Json.parse(m);this.__Aj[m.key]=m.value;this.__Cn[m.key]=a+qx.bom.storage.UserData.__bV;qx.bom.storage.UserData.__bV++;m=this.__Cl.getAttribute(a+qx.bom.storage.UserData.__bV);}
;}
,members:{__Cl:null,__Cm:d,__Aj:null,__Cn:null,getStorage:function(){return this.__Aj;}
,getLength:function(){return qx.Bootstrap.getKeys(this.__Aj).length;}
,setItem:function(n,o){if(this.__Cn[n]){var p=this.__Cn[n];}
else {var p=a+qx.bom.storage.UserData.__bV;qx.bom.storage.UserData.__bV++;}
;var q=qx.lang.Json.stringify({key:n,value:o});this.__Cl.setAttribute(p,q);this.__Cl.save(this.__Cm);this.__Aj[n]=o;this.__Cn[n]=p;}
,getItem:function(r){return this.__Aj[r]||null;}
,removeItem:function(s){var v=this.__Cn[s];if(v==undefined){return;}
;this.__Cl.removeAttribute(v);qx.bom.storage.UserData.__bV--;delete this.__Aj[s];delete this.__Cn[s];var u=a+qx.bom.storage.UserData.__bV;if(this.__Cl.getAttribute(u)){var w=this.__Cl.getAttribute(a+qx.bom.storage.UserData.__bV);this.__Cl.removeAttribute(u);this.__Cl.setAttribute(v,w);var t=qx.lang.Json.parse(w).key;this.__Cn[t]=v;}
;this.__Cl.save(this.__Cm);}
,clear:function(){for(var x in this.__Cn){this.__Cl.removeAttribute(this.__Cn[x]);}
;this.__Cl.save(this.__Cm);this.__Aj={};this.__Cn={};}
,getKey:function(y){return qx.Bootstrap.getKeys(this.__Aj)[y];}
,forEach:function(z,A){var length=this.getLength();for(var i=0;i<length;i++){var B=this.getKey(i);z.call(A,B,this.getItem(B));}
;}
}});}
)();
(function(){var a="qx.bom.storage.Memory";qx.Bootstrap.define(a,{statics:{__Cj:null,__Ck:null,getLocal:function(){if(this.__Cj){return this.__Cj;}
;return this.__Cj=new qx.bom.storage.Memory();}
,getSession:function(){if(this.__Ck){return this.__Ck;}
;return this.__Ck=new qx.bom.storage.Memory();}
},construct:function(){this.__Aj={};}
,members:{__Aj:null,getStorage:function(){return this.__Aj;}
,getLength:function(){return qx.Bootstrap.getKeys(this.__Aj).length;}
,setItem:function(b,c){c=qx.lang.Json.stringify(c);this.__Aj[b]=c;}
,getItem:function(d){var e=this.__Aj[d];if(qx.lang.Type.isString(e)){e=qx.lang.Json.parse(e);}
;return e;}
,removeItem:function(f){delete this.__Aj[f];}
,clear:function(){this.__Aj={};}
,getKey:function(g){var h=qx.Bootstrap.getKeys(this.__Aj);return h[g];}
,forEach:function(j,k){var length=this.getLength();for(var i=0;i<length;i++){var l=this.getKey(i);j.call(k,l,this.getItem(l));}
;}
}});}
)();
(function(){var c="html.storage.local",b="html.storage.userdata",a="qx.bom.Storage";qx.Bootstrap.define(a,{statics:{__AP:null,getLocal:function(){return this.__AP.getLocal();}
,getSession:function(){return this.__AP.getSession();}
},defer:function(d){if(qx.core.Environment.get(c)){d.__AP=qx.bom.storage.Web;}
else if(qx.core.Environment.get(b)){d.__AP=qx.bom.storage.UserData;}
else {d.__AP=qx.bom.storage.Memory;}
;}
});}
)();
