...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.virtual.Complex","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Complex-qx.theme.Classic.e59869984c48.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var d="demobrowser.demo.virtual.Complex",c="horizontal",b="rgb(238, 243, 255)",a="white";qx.Class.define(d,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var e=new qx.ui.virtual.core.Scroller(100,15,32,120);e.getPane().setWidth(450);for(var i=2;i<10;i++){e.getPane().getRowConfig().setItemSize(i,50+Math.round(Math.random()*40));e.getPane().getColumnConfig().setItemSize(i,50+Math.round(Math.random()*80));}
;this.getRoot().add(e,{left:20,top:10});e.getPane().addLayer(new qx.ui.virtual.layer.Row(a,b));e.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));e.getPane().addLayer(new demobrowser.demo.virtual.DemoLayer);new qx.ui.virtual.behavior.Prefetch(e,{minLeft:500,maxLeft:600,minRight:1000,maxRight:1200,minAbove:0,maxAbove:0,minBelow:0,maxBelow:0});}
}});}
)();
...
;this._cellProvider=h;this.__yW=[];}
,properties:{anonymous:{refine:true,init:false}},events:{updated:g},members:{__yW:null,getRenderedCellWidget:function(j,k){var l=this.getColumnSizes().length;var q=this.getRowSizes().length;var p=this.getFirstRow();var o=this.getFirstColumn();if(j<p||j>=p+q||k<o||k>=o+l){return null;}
;var n=(k-o)+(j-p)*l;var m=this._getChildren()[n];if(m.getUserData(a)){return null;}
else {return m;}
;}
,_getSpacer:function(){var r=this.__yW.pop();if(!r){r=new qx.ui.core.Spacer();r.setUserData(a,1);}
;return r;}
...
;}
;}
,_fullUpdate:function(v,w,z,A){var C=this._cellProvider;var G=this._getChildren();for(var i=0;i<G.length;i++){var E=G[i];if(E.getUserData(a)){this.__yW.push(E);}
...
;}
;}
;var U=this._cellProvider;var Y=this._getChildren();for(var i=0;i<Y.length;i++){if(!S[i]){var W=Y[i];if(W.getUserData(a)){this.__yW.push(W);}
...
;this._cellProvider=this.__yW=null;}
});}
)();
...
(function(){var r="icon/",q="",p="folder.png",o="places/",n="demobrowser.demo.virtual.DemoLayer",m="network-server.png",l="mouseover",k="user-trash.png",i="changeValue",h="network-workgroup.png",c="qx.ui.basic.Atom",g="/",f="user-desktop.png",b=" / ",a="row",e="foobar!",d="column";qx.Class.define(n,{extend:qx.ui.virtual.layer.WidgetCell,construct:function(){qx.ui.virtual.layer.WidgetCell.call(this,this);this._pool={atom:[],checkbox:[]};this.__md=[];}
,members:{_pool:null,__md:null,getCellData:function(s,t){if(!this.__md[s]){this.__md[s]=[];}
;if(!this.__md[s][t]){this.__md[s][t]={label:this.__Ro(),icon:this.__Rp()};}
;return this.__md[s][t];}
,getCellWidget:function(u,v){var w;if(v%2==0){w=this._pool.atom.pop();if(!w){w=new qx.ui.basic.Atom();w.addListener(l,function(){var x=this.__Rp();w.setIcon(x);this.__md[u][v].icon=x;}
,this);}
;w.set(this.getCellData(u,v));}
else {w=this._pool.checkbox.pop();if(!w){w=new qx.ui.form.CheckBox();w.addListener(i,function(){this.setLabel(this.getLabel()==e?w.getUserData(a)+b+w.getUserData(d):e);}
,w);}
;w.set({value:u%2==0,label:u+b+v});}
;w.setUserData(a,u);w.setUserData(d,v);return w;}
,poolCellWidget:function(y){if(y.classname==c){this._pool.atom.push(y);}
else {this._pool.checkbox.push(y);}
;}
,__Ro:function(){var name=q;for(var j=0;j<10;j++){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);}
;return name;}
,__Rp:function(){var z=r;var B=o;var C=[p,k,m,h,f];var A=Math.floor(Math.random()*4);return (z+16+g+B+C[A]);}
},destruct:function(){this._pool=this.__md=null;}
});}
)();
...
;}
,properties:{scroller:{check:e,nullable:true,init:null,apply:f},interval:{check:d,init:200,apply:l}},members:{__tE:null,__tF:null,__iG:null,__tG:null,__tH:null,setPrefetchX:function(o,p,q,r){this.__tE=[o,p,q,r];}
,setPrefetchY:function(s,t,u,v){this.__tF=[s,t,u,v];}
,_onInterval:function(){var w=this.__tE;if(w[1]&&w[3]){this.getScroller().getPane().prefetchX(w[0],w[1],w[2],w[3]);qx.ui.core.queue.Manager.flush();}
;var x=this.__tF;if(x[1]&&x[3]){this.getScroller().getPane().prefetchY(x[0],x[1],x[2],x[3]);qx.ui.core.queue.Manager.flush();}
;}
,_applyScroller:function(y,z){if(z){if(this.__tG){z.getChildControl(a).removeListenerById(this.__tG);}
;if(this.__tH){z.getChildControl(b).removeListenerById(this.__tH);}
;}
...
;this.__tG=y.getChildControl(a).addListener(c,this.__iG.restart,this.__iG);this.__tH=y.getChildControl(b).addListener(c,this.__iG.restart,this.__iG);}
else {this.__iG.stop();}
;}
,_applyInterval:function(A,B){this.__iG.setInterval(A);}
},destruct:function(){this.setScroller(null);this.__tE=this.__tF=null;this._disposeObjects(h);}
});}
)();
