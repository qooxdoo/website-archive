...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table-qx.theme.Classic.f065e793246d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="Boolean",j="Row 10 changed",h="icon/22/actions/edit-undo.png",g="Change ID sort method",f="5 rows removed",e="icon/22/status/dialog-information.png",d="Change row with ID 10",c="A number",b="icon/16/apps/office-calendar.png",a="Remove 5 rows",D="Add 10 rows",C="",B="demobrowser.demo.table.Table",A="_tableModel",z="Table",y="icon/22/actions/list-remove.png",x="Whether the the first visible row should ",w=", ",v="icon/22/actions/list-add.png",u="Selected rows:<br>",r="Keep first row",s="be rendered completely when scrolling.",p="Show selection",q="Demonstrate use of alternate sorting algorithm.",n="ID",o="10 rows added",l="changeValue",m="A date",t="execute";qx.Class.define(B,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return z;}
,createTable:function(){var G=this.createRandomRows(50);var F=this._tableModel=new qx.ui.table.model.Simple();F.setColumns([n,c,m,k]);F.setData(G);F.setColumnEditable(1,true);F.setColumnEditable(2,true);F.setColumnSortable(3,false);var H=new qx.ui.table.Table(F);H.set({width:600,height:400,decorator:null});H.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);var E=H.getTableColumnModel();E.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());E.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(b,m));return H;}
,createControls:function(){var L=new qx.ui.toolbar.ToolBar();var J,K,M;K=new qx.ui.toolbar.Part();L.add(K);J=new qx.ui.toolbar.Button(d,h);J.addListener(t,function(N){var O=this.createRandomRows(1);for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,O[0][i]);}
;this.info(j);}
,this);K.add(J);J=new qx.ui.toolbar.Button(D,v);J.addListener(t,function(P){var Q=this.createRandomRows(10);this._tableModel.addRows(Q);this.info(o);}
,this);K.add(J);J=new qx.ui.toolbar.Button(a,y);J.addListener(t,function(R){var S=this._tableModel.getRowCount();this._tableModel.removeRows(S-5,5);this.info(f);}
,this);K.add(J);J=new qx.ui.toolbar.Button(p,e);J.addListener(t,function(T){var U=[];I.getSelectionModel().iterateSelection(function(V){U.push(V+C);}
);this.showDialog(u+U.join(w));}
,this);K.add(J);K=new qx.ui.toolbar.Part();L.add(K);var I=this._table;M=new qx.ui.toolbar.CheckBox(r);M.set({value:I.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(x+s)});M.addListener(l,function(W){I.setKeepFirstVisibleRowComplete(this.getValue());}
,M);K.add(M);M=new qx.ui.toolbar.CheckBox(g);M.set({value:I.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(q)});M.addListener(l,function(X){if(X.getData()){var ba=function(bb,bc){var bd=bb[arguments.callee.columnIndex];var be=bc[arguments.callee.columnIndex];if(bd%2==1&&be%2==0){return 1;}
;if(be%2==1&&bd%2==0){return -1;}
;return (bd>be)?1:((bd==be)?0:-1);}
;var Y=function(bf,bg){var bh=bf[arguments.callee.columnIndex];var bi=bg[arguments.callee.columnIndex];if(bh%2==1&&bi%2==0){return -1;}
;if(bi%2==1&&bh%2==0){return 1;}
...
(function(){var c="qx.ui.table.headerrenderer.Icon",b="String",a="";qx.Class.define(c,{extend:qx.ui.table.headerrenderer.Default,construct:function(d,e){qx.ui.table.headerrenderer.Default.call(this);if(d==null){d=a;}
;this.setIconUrl(d);if(e){this.setToolTip(e);}
;}
,properties:{iconUrl:{check:b,init:a}},members:{updateHeaderCell:function(f,g){qx.ui.table.headerrenderer.Default.prototype.updateHeaderCell.call(this,f,g);g.setIcon(this.getIconUrl());}
}});}
)();
...
;}
,members:{__AS:null,_getDefaultInsets:function(){return this._getDefaultInsetsForBorder();}
,_isInitialized:function(){return !!this.__AS;}
,getMarkup:function(){if(this.__AS){return this.__AS;}
;var j={};this._styleBorder(j);return this.__AS=this._generateMarkup(j);}
