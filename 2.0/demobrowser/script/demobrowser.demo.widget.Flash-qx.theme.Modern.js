...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Flash-qx.theme.Modern.fe8a2346db53.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,__Kn:null,__RA:null,__RB:null,__RC:null,createFlashDemo2:function(){this.self(arguments).setCallBackInstance(this);var R=new qx.ui.container.Composite(new qx.ui.layout.HBox(8));var Q=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));R.add(Q,{flex:1});Q.add(new qx.ui.basic.Label(z).set({rich:true}));Q.add(new qx.ui.basic.Label(g));this.__RA=new qx.ui.form.TextArea(B);this.__RA.setEnabled(false);Q.add(this.__RA,{flex:1});Q.add(new qx.ui.basic.Label(l));this.__RB=new qx.ui.form.TextArea(h);Q.add(this.__RB,{flex:1});var S=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,i));this.__RC=new qx.ui.form.Button(D);this.__RC.setEnabled(false);this.__RC.addListener(v,function(e){this.__Kn.getFlashElement().sendMessage(this.__RB.getValue());}
,this);S.add(this.__RC);Q.add(S);var P=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));R.add(P,{flex:1});P.add(new qx.ui.basic.Label(A).set({rich:true}));this.__Kn=new qx.ui.embed.Flash(F);this.__Kn.setVariables({init:x});P.add(this.__Kn,{flex:1});return R;}
,sendMessage:function(T){this.__RA.setValue(T);}
,initFlash:function(){if(this.__Kn.getFlashElement().setup){this.__Kn.getFlashElement().setup(y);this.__RC.setEnabled(true);}
else {alert(w+E+k);}
;}
...
},destruct:function(){this._disposeObjects(o,r,f,s,c);}
});}
)();
...
,_checkLoading:function(){var bi=this.getSource();if(bi!=t&&bi!=null&&bi!=J){if(!this.isLoaded()){if(!this.__Ki){this.__Ki=new Date().getTime();}
;var bj=new Date().getTime()-this.__Ki;if(this.getLoadTimeout()>bj){var bk=qx.util.TimerManager.getInstance();bk.start(this._checkLoading,0,this,null,10);this.fireEvent(S);}
else {if(qx.core.Environment.get(N)){this.debug(r+bj);}
;this.fireEvent(F);this.__Ki=null;}
;}
else {this.fireEvent(ba);this.__Ki=null;}
;}
;}
...
,_applyQuality:function(bu,bv){this.__Kj(j,bu);}
,_applyScale:function(bw,bx){this.__Kj(f,bw);}
,_applyWmode:function(by,bz){this.__Kj(a,by);}
,_applyPlay:function(bA,bB){this.__Kj(l,bA);}
,_applyLoop:function(bC,bD){this.__Kj(W,bC);}
,_applyMenu:function(bE,bF){this.__Kj(U,bE);}
,_applyAllowScriptAccess:function(bG,bH){this.__Kj(V,bG);}
,_applyLiveConnect:function(bI,bJ){this.__Kj(q,bI);}
,__Kj:function(bK,bL){this.getContentElement().setParam(bK,bL);qx.ui.core.queue.Layout.add(this);}
}});}
)();
(function(){var m="The id cannot be modified after initial creation",l="The variables cannot be modified after initial creation",k="movie",j="The attributes cannot be modified after initial creation",i="The source cannot be modified after initial creation",h="The params cannot be modified after initial creation",g="id",f="qx.html.Flash",e="Invalid attribute 'key'.",d="div",a="Invalid attribute 'value' expected String, Boolean or null.",c="Invalid attribute 'value'.",b="qx.debug";qx.Class.define(f,{extend:qx.html.Element,construct:function(n,o){qx.html.Element.call(this,d,n,o);this.__Kk={};this.__Kl={};this.__Km={};}
,members:{__Kk:null,__Km:null,__Kl:null,__Kn:null,_createDomElement:function(){return qx.dom.Element.create(d);}
,createFlash:function(){this.__Kn=qx.bom.Flash.create(this.getDomElement(),this.getAttributes(),this.__Kl,this.__Kk);}
,setSource:function(p){if(qx.core.Environment.get(b)){qx.core.Assert.assertString(p,c);}
;if(this.__Kn){throw new Error(i);}
;this.setAttribute(k,p);}
,setId:function(q){if(qx.core.Environment.get(b)){qx.core.Assert.assertString(q,c);}
;if(this.__Kn){throw new Error(m);}
;this.setAttribute(g,q);}
,getVariables:function(){return this.__Kl;}
,setVariables:function(r){if(qx.core.Environment.get(b)){qx.core.Assert.assertMap(r,c);}
;if(this.__Kn){throw new Error(l);}
;this.__Kl=r;}
,getAttributes:function(){return this.__Km;}
...
;}
;}
;if(this.__Kn){throw new Error(j);}
;if(t===null||t===undefined){delete this.__Km[s];}
else {this.__Km[s]=t;}
;}
,getParams:function(){return this.__Kk;}
...
;}
;}
;if(this.__Kn){throw new Error(h);}
;if(v===null||v===undefined){delete this.__Kk[u];}
else {this.__Kk[u]=v;}
;}
,getFlashElement:function(){return this.__Kn;}
},destruct:function(){if(this.__Kn){qx.bom.Flash.destroy(this.__Kn);}
;this.__Kk=this.__Kl=this.__Km=null;}
});}
)();
(function(){var k="Invalid parameter 'element'.",j="function",h="application/x-shockwave-flash",g='<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">',f="param",e="Invalid parameter 'attributes'.",d="qx.bom.Flash",c="Invalid attribute 'movie'.",b='<param name="',a="qx.debug",I="type",H="load",G="name",F="data",E="Invalid attribute 'id'.",D="",C="mshtml",B="DOM element is null or undefined!",A="DOM element has or is not a flash object!",z="value",r="&",s='" />',p="The parent DOM element isn't in DOM! The External Interface doesn't work in IE!",q='<object id="',n='" value="',o='" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">',l="undefined",m='</object>',t="100%",u="movie",w="beforeunload",v="=",y="object",x="engine.name";qx.Class.define(d,{statics:{_flashObjects:{},create:function(J,K,L,M,N){if(!N){N=window;}
;if(qx.core.Environment.get(a)){qx.core.Assert.assertElement(J,k);qx.core.Assert.assertMap(K,e);qx.core.Assert.assertString(K.movie,c);qx.core.Assert.assertString(K.id,E);if(!qx.dom.Element.isInDom(J,N)){qx.log.Logger.warn(this,p);}
;}
;if(!K.width){K.width=t;}
;if(!K.height){K.height=t;}
;M=M?qx.lang.Object.clone(M):{};if(!M[u]){M[u]=K.movie;}
;K[F]=K.movie;delete K.movie;if(L){for(var name in L){if(typeof M.flashvars!=l){M.flashvars+=r+name+v+L[name];}
else {M.flashvars=name+v+L[name];}
;}
;}
;var O=this.__Kt(J,K,M,N);this._flashObjects[K.id]=O;return O;}
,destroy:qx.core.Environment.select(x,{"mshtml":function(P,Q){P=this.__Ko(P);if(P.readyState==4){this.__Kp(P);}
else {if(!Q){Q=window;}
;qx.bom.Event.addNativeListener(Q,H,function(){qx.bom.Flash.__Kp(P);}
);}
;}
,"default":function(R,S){R=this.__Ko(R);if(R.parentNode){R.parentNode.removeChild(R);}
;delete this._flashObjects[R.id];}
}),__Ko:function(T){if(!T){throw new Error(B);}
;if(T.tagName.toLowerCase()!==y){T=T.firstChild;}
;if(!T||T.tagName.toLowerCase()!==y){throw new Error(A);}
;return T;}
,__Kp:qx.core.Environment.select(x,{"mshtml":qx.event.GlobalError.observeMethod(function(U){for(var i in U){if(typeof U[i]==j){U[i]=null;}
;}
;if(U.parentNode){U.parentNode.removeChild(U);}
;delete this._flashObjects[U.id];}
),"default":null}),__Kq:qx.event.GlobalError.observeMethod(function(){for(var V in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[V]);}
;window.__Kr=function(){}
;window.__Ks=function(){}
;qx.bom.Event.removeNativeListener(window,w,qx.bom.Flash.__Kq);}
),__Kt:qx.core.Environment.select(x,{"mshtml":function(W,X,Y,ba){Y.movie=X.data;delete X.data;delete X.classid;var bb=D;for(name in Y){bb+=b+name+n+Y[name]+s;}
;if(X.id){W.innerHTML=q+X.id+o+bb+m;delete X.id;}
else {W.innerHTML=g+bb+m;}
;for(var name in X){W.firstChild.setAttribute(name,X[name]);}
;return W.firstChild;}
,"default":function(bc,bd,be,bf){delete bd.classid;delete be.movie;var bh=qx.dom.Element.create(y,bd,bf);bh.setAttribute(I,h);var bg;for(var name in be){bg=qx.dom.Element.create(f,{},bf);bg.setAttribute(G,name);bg.setAttribute(z,be[name]);bh.appendChild(bg);}
;bc.appendChild(bh);return bh;}
})},defer:function(bi){if((qx.core.Environment.get(x)==C)){qx.bom.Event.addNativeListener(window,w,bi.__Kq);}
;}
});}
)();
