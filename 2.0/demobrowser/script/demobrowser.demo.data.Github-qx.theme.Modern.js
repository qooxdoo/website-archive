...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Github-qx.theme.Modern.e3daaa1f3c98.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;return H;}
},members:{__LY:null,__JG:null,__Mc:null,__Ma:null,__Mb:null,__Md:null,configureRequest:function(K){this.__Md=K;}
,_getRequest:function(){return new qx.io.request.Xhr();}
,__Me:function(L){var M=this._getRequest();if(!qx.lang.Type.isArray(this.__LY[L])){this.__LY[L]=[];}
;this.__LY[L].push(M);return M;}
,map:function(N,O,P,Q){this.__JG[N]=[O,P,Q];this.__LY[N]=[];if(N==z){this[N]=undefined;}
;if(typeof this[N]!==u&&this[N].action!==true){if(this[N]!==qx.lang.Function.empty){throw new Error(C+N+x);}
;}
;this.__Mi(N+l);this.__Mi(N+n);this[N]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,N);return this.invoke.apply(this,arguments);}
,this);this[N].action=true;}
,invoke:function(R,S,T){var V=this.__Me(R),S=S==null?{}:S,Y=this._getRequestConfig(R,S);this.__JG[R].params=S;this.__Mf(S,Y.check);this.__IW(V,Y,T);if(this.__Md){this.__Md.call(this,V,R,S,T);}
;this.__Mg(V,Y,T);V.addListenerOnce(q,function X(){var ba=[V.getResponse(),null,false,V,R,V.getPhase()];this.fireEvent(R+l,qx.event.type.Rest,ba);this.fireEvent(q,qx.event.type.Rest,ba);}
...
,setBaseUrl:function(bc){this.__Mc=bc;}
,__Mf:function(bd,be){if(typeof be!==u){if(qx.core.Environment.get(t)){qx.core.Assert.assertObject(be,p);}
...
;}
,__IW:function(bg,bh,bi){bg.set({method:bh.method,url:bh.url});if(bi){bg.setRequestData(bi);}
;}
,__Mg:function(bj,bk,bl){if(bl){var bm=bj.getRequestHeader(s);if(qx.util.Request.methodAllowsRequestBody(bj.getMethod())){if((/application\/.*\+?json/).test(bm)){bl=qx.lang.Json.stringify(bl);bj.setRequestData(bl);}
;}
;}
;}
...
;}
else {var bo=bn;var bs=this.__LY[bo];if(this.__LY[bo]){bs.forEach(function(bt){bt.abort();}
);}
;}
;}
,refresh:function(bu){this.invoke(bu,this.__JG[bu].params);}
,poll:function(bv,bw,bx){if(this.__Ma[bv]){this.__Ma[bv].dispose();}
;if(typeof bx==u){bx=this.__JG[bv].params;}
;this.invoke(bv,bx);var by=this.__Ma[bv]=new qx.event.Timer(bw);by.addListener(A,function bz(){var bA=this.__LY[bv][0];if(bA.isDone()||bA.isDisposed()){this.refresh(bv);}
;}
...
;if(!bJ){bH=0;}
;return false;}
;var bE=this.__Mb[bB]=this.addListener(bB+l,function bI(){if(bD.isDisposed()){return;}
;if(!bC()){bG=new Date();bD.refresh(bB);}
;}
,bF);this.invoke(bB);return bE;}
,_getRequestConfig:function(bK,bL){var bN=this.__JG[bK];var bL=qx.lang.Object.clone(bL);if(!qx.lang.Type.isArray(bN)){throw new Error(E+bK);}
;var bQ=bN[0],bM=this.__Mc!==null?this.__Mc+bN[1]:bN[1],bO=bN[2],bP=qx.io.rest.Resource.placeholdersFromUrl(bM);bL=bL||{};bP.forEach(function(bR){var bS=new RegExp(y+bR+i),bT=bM.match(bS)[1];if(typeof bL[bR]===u){if(bT){bL[bR]=bT;}
else {bL[bR]=w;}
;}
...
,__Mh:function(bU){qx.lang.Object.getKeys(bU).forEach(function(bV){var bX=bU[bV],bY=bX.method,bW=bX.url,ca=bX.check;if(qx.core.Environment.get(t)){qx.core.Assert.assertString(bY,r+bV+o);qx.core.Assert.assertString(bW,D+bV+o);}
;this.map(bV,bY,bW,ca);}
,this);}
,__Mi:function(cb){if(!this.constructor.$$events){this.constructor.$$events={};}
;if(!this.constructor.$$events[cb]){this.constructor.$$events[cb]=m;}
;}
},destruct:function(){var cc;for(cc in this.__LY){if(this.__LY[cc]){this.__LY[cc].forEach(function(cf){cf.dispose();}
);}
;}
;if(this.__Ma){for(cc in this.__Ma){var ce=this.__Ma[cc];ce.stop();ce.dispose();}
;}
;if(this.__Mb){for(cc in this.__Mb){var cd=this.__Mb[cc];this.removeListenerById(cd);}
;}
;this.__LY=this.__JG=this.__Ma=null;}
});}
)();
...
(function(){var j="changeModel",i="model.user.avatar_url",h="application/json",g="github.view.Gist",f="model.description",e="west",d="/gists/{id}",c="Description",b="https://api.github.com",a="demobrowser.demo.data.Github",H="source",G="/gists",F="change",E="Content",D="center",C="#fff",B="Authorization",A="html",z="model.user.login",y="model.files",q="north",r="Gists",o="Accept",p="token ",m="github.rest.Resource",n="description",k="undefined",l="</pre>",s="<pre>",t="GET",v="value",u="get",x="model",w="bold";qx.Class.define(a,{extend:qx.application.Standalone,members:{__PU:null,__PV:null,__PW:null,__PX:null,__CQ:null,__PY:null,main:function(){if(typeof GITHUB==k){return;}
;qx.application.Standalone.prototype.main.call(this);this._setUpResources();this._setUpStores();this._createGui();this._setUpBinding();this.__PU.get();this.__CQ.addListener(j,function(I){var J=I.getData();this.__CQ.getSelection().push(J.getItem(0));}
,this);this.__CQ.getSelection().addListener(F,function(K){var L=this.__CQ.getSelection().getItem(0).getId();this.__PV.get({id:L});}
,this);}
,_setUpResources:function(){this.__PU=new github.rest.Resource({"get":{method:t,url:G}});this.__PV=new github.rest.Resource({"get":{method:t,url:d}});}
,_setUpStores:function(){this.__PW=new qx.data.store.Rest(this.__PU,u);this.__PX=new qx.data.store.Rest(this.__PV,u);}
,_createGui:function(){var M=new qx.ui.container.Composite(new qx.ui.layout.Dock());M.setPadding(10);var N=new qx.ui.basic.Label(r);N.setFont(w);N.setPaddingBottom(10);M.add(N,{edge:q});this.__CQ=new qx.ui.list.List();this.__CQ.setWidth(200);this.__PY=new github.view.Gist();M.add(this.__CQ,{edge:e});M.add(this.__PY,{edge:D});this.getRoot().add(M,{edge:0});}
,_setUpBinding:function(){var P=this.__CQ,O=this.__PW,Q=this.__PX;P.setLabelPath(n);P.setLabelOptions({converter:function(S,T,U,V){if(S===null||!S.length){return T.getId();}
;return S;}
});O.bind(x,P,x);var R=this.__PY;Q.bind(f,R.getDescription(),v);Q.bind(z,R.getUsername(),v);Q.bind(i,R.getGravatar(),H);Q.bind(y,R.getContent(),A,{converter:function(W){var X=qx.Class.getProperties(W.constructor);var content=W.get(X[0]).getContent();content=qx.bom.String.escape(content);return s+content+l;}
});}
...
});qx.Class.define(g,{extend:qx.ui.container.Composite,construct:function(){qx.ui.container.Composite.call(this);var bb=new qx.ui.layout.Grid(5,5);bb.setColumnFlex(0,1);bb.setRowFlex(2,1);this.setLayout(bb);this.setPadding(0,10);var be;var bc=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));be=new qx.ui.basic.Label(c);be.setFont(w);this.__BW=new qx.ui.basic.Label();bc.add(be);bc.add(this.__BW);this.add(bc,{row:0,column:0});var bd=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));this.__Qa=new qx.ui.basic.Image();this.__Qa.set({width:40,height:40,scale:true,backgroundColor:C});this.__Qb=new qx.ui.basic.Label();bd.add(this.__Qa);bd.add(this.__Qb);this.add(bd,{row:0,column:1});be=new qx.ui.basic.Label(E);be.setFont(w);var scroll=new qx.ui.container.Scroll();this.__mq=new qx.ui.embed.Html();this.__mq.setMinHeight(1000);scroll.add(this.__mq);this.add(be,{row:1,column:0});this.add(scroll,{row:2,column:0,colSpan:2});}
,members:{getDescription:function(){return this.__BW;}
,getGravatar:function(){return this.__Qa;}
,getUsername:function(){return this.__Qb;}
,getContent:function(){return this.__mq;}
}});}
)();
...
;this._delegate=j;this._marshaler=new qx.data.marshal.Json(j);if(j&&qx.lang.Type.isFunction(j.configureRequest)){this.__IW();}
;this.__IV=qx.lang.Function.bind(this.__IX,this);this.__wt();}
,properties:{resource:{check:d},actionName:{check:b},model:{nullable:true,event:g}},members:{_marshaler:null,_delegate:null,__IV:null,__IW:function(){var k=this.getResource(),l=this._delegate;k.configureRequest(l.configureRequest);}
,__wt:function(){var n=this.getResource(),m=this.getActionName();if(n&&m){n.addListener(this.getActionName()+c,this.__IV);}
;}
,__IX:function(e){var p=e.getData(),o=this._marshaler,q,r=this.getModel(),s=this._delegate;if(p){if(s&&s.manipulateData){p=s.manipulateData(p);}
