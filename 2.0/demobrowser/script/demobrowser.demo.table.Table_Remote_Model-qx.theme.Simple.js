...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Remote_Model","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Remote_Model-qx.theme.Simple.d0b6405401de.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,properties:{selectionModel:{check:b}},members:{__oD:null,handleMouseDown:function(c,d){if(d.isLeftPressed()){var e=this.getSelectionModel();if(!e.isSelectedIndex(c)){this._handleSelectEvent(c,d);this.__oD=true;}
...
;}
;}
,handleMouseUp:function(f,g){if(g.isLeftPressed()&&!this.__oD){this._handleSelectEvent(f,g);}
;}
...
(function(){var o="]",n="..",m="changeSelection",l="Try to turn off batch mode althoug it was not turned on.",k=" [",h='ie',g="browser.version",f="qx.event.type.Event",d="Ranges:",c="qx.ui.table.selection.Model",a="browser.name",b="_applySelectionMode";qx.Class.define(c,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__oE=[];this.__oF=-1;this.__oG=-1;this.hasBatchModeRefCount=0;this.__oH=false;}
,events:{"changeSelection":f},statics:{NO_SELECTION:1,SINGLE_SELECTION:2,SINGLE_INTERVAL_SELECTION:3,MULTIPLE_INTERVAL_SELECTION:4,MULTIPLE_INTERVAL_SELECTION_TOGGLE:5},properties:{selectionMode:{init:2,check:[1,2,3,4,5],apply:b}},members:{__oH:null,__oF:null,__oG:null,__oE:null,_applySelectionMode:function(p){this.resetSelection();}
,setBatchMode:function(q){if(q){this.hasBatchModeRefCount+=1;}
else {if(this.hasBatchModeRefCount==0){throw new Error(l);}
;this.hasBatchModeRefCount-=1;if(this.__oH){this.__oH=false;this._fireChangeSelection();}
;}
;return this.hasBatchMode();}
,hasBatchMode:function(){return this.hasBatchModeRefCount>0;}
,getAnchorSelectionIndex:function(){return this.__oF;}
,_setAnchorSelectionIndex:function(r){this.__oF=r;}
,getLeadSelectionIndex:function(){return this.__oG;}
,_setLeadSelectionIndex:function(s){this.__oG=s;}
,_getSelectedRangeArr:function(){return this.__oE;}
,resetSelection:function(){if(!this.isSelectionEmpty()){this._resetSelection();this._fireChangeSelection();}
;}
,isSelectionEmpty:function(){return this.__oE.length==0;}
,getSelectedCount:function(){var u=0;for(var i=0;i<this.__oE.length;i++){var t=this.__oE[i];u+=t.maxIndex-t.minIndex+1;}
;return u;}
,isSelectedIndex:function(v){for(var i=0;i<this.__oE.length;i++){var w=this.__oE[i];if(v>=w.minIndex&&v<=w.maxIndex){return true;}
;}
;return false;}
,getSelectedRanges:function(){var x=[];for(var i=0;i<this.__oE.length;i++){x.push({minIndex:this.__oE[i].minIndex,maxIndex:this.__oE[i].maxIndex});}
;return x;}
,iterateSelection:function(y,z){for(var i=0;i<this.__oE.length;i++){for(var j=this.__oE[i].minIndex;j<=this.__oE[i].maxIndex;j++){y.call(z,j);}
;}
;}
...
,removeSelectionInterval:function(G,H){this.__oF=G;this.__oG=H;var I=Math.min(G,H);var K=Math.max(G,H);for(var i=0;i<this.__oE.length;i++){var M=this.__oE[i];if(M.minIndex>K){break;}
else if(M.maxIndex>=I){var N=(M.minIndex>=I)&&(M.minIndex<=K);var L=(M.maxIndex>=I)&&(M.maxIndex<=K);if(N&&L){this.__oE.splice(i,1);i--;}
else if(N){M.minIndex=K+1;}
else if(L){M.maxIndex=I-1;}
else {var J={minIndex:K+1,maxIndex:M.maxIndex};this.__oE.splice(i+1,0,J);M.maxIndex=I-1;break;}
;;}
;}
;this._fireChangeSelection();}
,_resetSelection:function(){this.__oE=[];this.__oF=-1;this.__oG=-1;}
,_addSelectionInterval:function(O,P){this.__oF=O;this.__oG=P;var Q=Math.min(O,P);var S=Math.max(O,P);var R=0;for(;R<this.__oE.length;R++){var T=this.__oE[R];if(T.minIndex>Q){break;}
;}
;this.__oE.splice(R,0,{minIndex:Q,maxIndex:S});var U=this.__oE[0];for(var i=1;i<this.__oE.length;i++){var T=this.__oE[i];if(U.maxIndex+1>=T.minIndex){U.maxIndex=Math.max(U.maxIndex,T.maxIndex);this.__oE.splice(i,1);i--;}
else {U=T;}
;}
;}
,_dumpRanges:function(){var V=d;for(var i=0;i<this.__oE.length;i++){var W=this.__oE[i];V+=k+W.minIndex+n+W.maxIndex+o;}
;this.debug(V);}
,_fireChangeSelection:function(){if(this.hasBatchMode()){this.__oH=true;}
...
(function(){var j="table-row-background-even",i="Boolean",h="content",g="default",f="height:",e="'",d="table-row",c="table-row-background-focused",b="css.boxmodel",a=';color:',v="table-row-background-odd",u="1px solid ",t="table-row-line",s="table-row-background-selected",r="background-color:",q=';border-bottom: 1px solid ',p="table-row-selected",o="table-row-background-focused-selected",n="px;",m="qx.ui.table.rowrenderer.Default",k=";",l="";qx.Class.define(m,{extend:qx.core.Object,implement:qx.ui.table.IRowRenderer,construct:function(){qx.core.Object.call(this);this.__oe=l;this.__oe={};this._colors={};this._renderFont(qx.theme.manager.Font.getInstance().resolve(g));var w=qx.theme.manager.Color.getInstance();this._colors.bgcolFocusedSelected=w.resolve(o);this._colors.bgcolFocused=w.resolve(c);this._colors.bgcolSelected=w.resolve(s);this._colors.bgcolEven=w.resolve(j);this._colors.bgcolOdd=w.resolve(v);this._colors.colSelected=w.resolve(p);this._colors.colNormal=w.resolve(d);this._colors.horLine=w.resolve(t);}
,properties:{highlightFocusRow:{check:i,init:true}},members:{_colors:null,__of:null,__oe:null,_insetY:1,_renderFont:function(x){if(x){this.__of=x.getStyles();this.__oe=qx.bom.element.Style.compile(this.__of);this.__oe=this.__oe.replace(/"/g,e);}
else {this.__oe=l;this.__of=qx.bom.Font.getDefaultStyles();}
;}
,updateDataRowElement:function(y,z){var B=this.__of;var A=z.style;qx.bom.element.Style.setStyles(z,B);if(y.focusedRow&&this.getHighlightFocusRow()){A.backgroundColor=y.selected?this._colors.bgcolFocusedSelected:this._colors.bgcolFocused;}
...
;return f+C+n;}
,createRowStyle:function(D){var E=[];E.push(k);E.push(this.__oe);E.push(r);if(D.focusedRow&&this.getHighlightFocusRow()){E.push(D.selected?this._colors.bgcolFocusedSelected:this._colors.bgcolFocused);}
...
},destruct:function(){this._colors=this.__of=this.__oe=null;}
});}
)();
...
(function(){var e="__oI",d="qx.ui.table.pane.Header",c="hovered",b="first",a="last";qx.Class.define(d,{extend:qx.ui.core.Widget,construct:function(f){qx.ui.core.Widget.call(this);this._setLayout(new qx.ui.layout.HBox());this.__jI=new qx.ui.core.Blocker(this);this.__oI=f;}
,members:{__oI:null,__oQ:null,__oR:null,__jI:null,getPaneScroller:function(){return this.__oI;}
,getTable:function(){return this.__oI.getTable();}
...
;}
,setMouseOverColumn:function(k){if(k!=this.__oR){if(this.__oR!=null){var l=this.getHeaderWidgetAtColumn(this.__oR);if(l!=null){l.removeState(c);}
;}
...
,showColumnMoveFeedback:function(o,x){var s=this.getContainerLocation();if(this.__oQ==null){var y=this.getTable();var p=this.getPaneScroller().getTablePaneModel().getX(o);var r=this._getChildren()[p];var t=y.getTableModel();var v=y.getTableColumnModel();var w={xPos:p,col:o,name:t.getColumnName(o),table:y};var u=v.getHeaderCellRenderer(o);var q=u.createHeaderCell(w);var z=r.getBounds();q.setWidth(z.width);q.setHeight(z.height);q.setZIndex(1000000);q.setOpacity(0.8);q.setLayoutProperties({top:s.top});this.getApplicationRoot().add(q);this.__oQ=q;}
;this.__oQ.setLayoutProperties({left:s.left+x});this.__oQ.show();}
,hideColumnMoveFeedback:function(){if(this.__oQ!=null){this.__oQ.destroy();this.__oQ=null;}
;}
,isShowingColumnMoveFeedback:function(){return this.__oQ!=null;}
...
(function(){var k='<div>',j="</div>",h="overflow: hidden;",g="qx.event.type.Data",f="paneReloadsData",e="div",d='style="',c="_applyMaxCacheLines",b="qx.ui.table.pane.Pane",a="width: 100%;",E="qx.event.type.Event",D="_applyVisibleRowCount",C='>',B="line-height: ",A="appear",z='class="',w="width:100%;",v="px;",u='<div ',t="'>",r="_applyFirstVisibleRow",s="<div style='",p=";position:relative;",q='</div>',n='" ',o="paneUpdated",l="Number",m="";qx.Class.define(b,{extend:qx.ui.core.Widget,construct:function(F){qx.ui.core.Widget.call(this);this.__oI=F;this.__oJ=0;this.__nY=0;this.__oK=[];}
,events:{"paneReloadsData":g,"paneUpdated":E},properties:{firstVisibleRow:{check:l,init:0,apply:r},visibleRowCount:{check:l,init:0,apply:D},maxCacheLines:{check:l,init:1000,apply:c},allowShrinkX:{refine:true,init:false}},members:{__nY:null,__oJ:null,__oI:null,__oL:null,__nV:null,__nU:null,__oK:null,__oM:0,_applyFirstVisibleRow:function(G,H){this.updateContent(false,G-H);}
,_applyVisibleRowCount:function(I,J){this.updateContent(true);}
,_getContentHint:function(){return {width:this.getPaneScroller().getTablePaneModel().getTotalWidth(),height:400};}
,getPaneScroller:function(){return this.__oI;}
,getTable:function(){return this.__oI.getTable();}
,setFocusedCell:function(K,L,M){if(K!=this.__nU||L!=this.__nV){var N=this.__nV;this.__nU=K;this.__nV=L;if(L!=N&&!M){if(N!==null){this.updateContent(false,null,N,true);}
...
,onTableModelDataChanged:function(Q,R,S,T){this.__oN();var V=this.getFirstVisibleRow();var U=this.getVisibleRowCount();if(R==-1||R>=V&&Q<V+U){this.updateContent();}
;}
,onTableModelMetaDataChanged:function(){this.updateContent(true);}
,_applyMaxCacheLines:function(W,X){if(this.__oM>=W&&W!==-1){this.__oN();}
;}
,__oN:function(){this.__oK=[];this.__oM=0;}
,__oO:function(Y,ba,bb){if(!ba&&!bb&&this.__oK[Y]){return this.__oK[Y];}
else {return null;}
;}
,__oP:function(bc,bd,be,bf){var bg=this.getMaxCacheLines();if(!be&&!bf&&!this.__oK[bc]&&bg>0){this._applyMaxCacheLines(bg);this.__oK[bc]=bd;this.__oM+=1;}
;}
,updateContent:function(bh,bi,bj,bk){if(bh){this.__oN();}
...
;by.push(q);var bQ=by.join(m);this.__oP(bE,bQ,bH,bK);bX.push(bQ);}
...
;}
;if(!this.__oL){this.__oL=document.createElement(e);}
;var ck=k;ck+=this._getRowsHtml(ci+ce,Math.abs(cb));ck+=q;this.__oL.innerHTML=ck;var cf=this.__oL.firstChild.childNodes;if(cb>0){for(var i=cf.length-1;i>=0;i--){var ch=cf[0];cl.appendChild(ch);}
;}
else {for(var i=cf.length-1;i>=0;i--){var ch=cf[cf.length-1];cl.insertBefore(ch,cl.firstChild);}
;}
;if(this.__nV!==null){this._updateRowStyles(this.__nV-cb);this._updateRowStyles(this.__nV);}
;this.fireEvent(o);}
...
;var cA=cu.join(m);cs.innerHTML=cA;this.setWidth(cr);this.__oJ=cw;this.__nY=cq;this.fireEvent(o);}
},destruct:function(){this.__oL=this.__oI=this.__oK=null;}
});}
)();
...
(function(){var m="abstract method call",l="_controls",k="OK",j="",i="_table",h="center",g="en",f="icon/16/apps/office-spreadsheet.png",d="icon/32/status/dialog-information.png",c="demobrowser.demo.table.TableDemo",a="_container",b="execute";qx.Class.define(c,{extend:qx.application.Standalone,include:demobrowser.demo.table.MUtil,members:{__QI:null,main:function(){qx.application.Standalone.prototype.main.call(this);qx.locale.Manager.getInstance().setLocale(g);this._container=new qx.ui.window.Window(this.getCaption(),f).set({width:600,height:400,contentPadding:[0,0,0,0],showClose:false,showMinimize:false});this._container.setLayout(new qx.ui.layout.VBox());this._container.open();this.getRoot().add(this._container,{left:50,top:10});this._table=this.createTable();this._controls=this.createControls();if(this._controls){this._container.add(this._controls);}
;this._container.add(this._table,{flex:1});}
,nextId:0,createRandomRows:function(n){var q=[];var p=new Date().getTime();var r=400*24*60*60*1000;for(var s=0;s<n;s++){var o=new Date(p+Math.random()*r-r/2);q.push([this.nextId++,Math.random()*10000,o,(Math.random()>0.5)]);}
;return q;}
,getCaption:function(){return j;}
,createTable:function(){throw new Error(m);}
,createControls:function(){return null;}
,showDialog:function(t){if(!this.__QI){var w=this.__QI=new qx.ui.window.Window().set({modal:true,showMinimize:false,showMaximize:false,width:180,contentPadding:[10,10,10,10]});w.moveTo(315,100);var u=new qx.ui.layout.Grid(15,15);u.setRowFlex(0,1);u.setColumnFlex(1,1);w.setLayout(u);w.add(new qx.ui.basic.Image(d),{row:0,column:0});w.add(new qx.ui.basic.Label().set({rich:true,allowGrowY:true}),{row:0,column:1});var v=new qx.ui.form.Button(k).set({alignX:h,allowGrowX:false,padding:[2,10]});v.addListener(b,function(e){w.close();}
,this);w.add(v,{row:1,column:0,colSpan:2});}
;this.__QI.getChildren()[1].setValue(t);this.__QI.open();this.__QI.getChildren()[2].focus();}
},destruct:function(){this._disposeObjects(i,l,a);}
});}
)();
...
(function(){var b="__pA",a="qx.ui.window.Manager";qx.Class.define(a,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__pA:null,setDesktop:function(c){this.__pA=c;this.updateStack();}
,getDesktop:function(){return this.__pA;}
,changeActiveWindow:function(d,e){if(d){this.bringToFront(d);d.setActive(true);}
;if(e){e.resetActive();}
;}
,_minZIndex:1e5,updateStack:function(){qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(){this.__pA.forceUnblockContent();var f=this.__pA.getWindows();var h=this._minZIndex;var m=h+f.length*2;var j=h+f.length*4;var k=null;for(var i=0,l=f.length;i<l;i++){var g=f[i];if(!g.isVisible()){continue;}
;k=k||g;if(g.isModal()){g.setZIndex(j);this.__pA.blockContent(j-1);j+=2;k=g;}
else if(g.isAlwaysOnTop()){g.setZIndex(m);m+=2;}
else {g.setZIndex(h);h+=2;}
;if(!k.isModal()&&g.isActive()||g.getZIndex()>k.getZIndex()){k=g;}
;}
;this.__pA.setActiveWindow(k);}
,bringToFront:function(n){var o=this.__pA.getWindows();var p=qx.lang.Array.remove(o,n);if(p){o.push(n);this.updateStack();}
;}
,sendToBack:function(q){var r=this.__pA.getWindows();var s=qx.lang.Array.remove(r,q);if(s){r.unshift(q);this.updateStack();}
;}
},destruct:function(){this._disposeObjects(b);}
});}
)();
(function(){var m="mouseup",l="mousedown",k="__pC",j="losecapture",i="qx.ui.core.MMovable",h="The move handle could not be redefined!",g="mousemove",f="__pB",d="maximized",c="move-frame",a="Boolean",b="move";qx.Mixin.define(i,{properties:{movable:{check:a,init:true},useMoveFrame:{check:a,init:false}},members:{__pB:null,__pC:null,__pD:null,__pE:null,__pF:null,__pG:null,__pH:null,__pI:false,__pJ:null,__pK:0,_activateMoveHandle:function(n){if(this.__pB){throw new Error(h);}
;this.__pB=n;n.addListener(l,this._onMoveMouseDown,this);n.addListener(m,this._onMoveMouseUp,this);n.addListener(g,this._onMoveMouseMove,this);n.addListener(j,this.__pO,this);}
,__pL:function(){var o=this.__pC;if(!o){o=this.__pC=new qx.ui.core.Widget();o.setAppearance(c);o.exclude();qx.core.Init.getApplication().getRoot().add(o);}
;return o;}
,__pM:function(){var location=this.getContainerLocation();var q=this.getBounds();var p=this.__pL();p.setUserBounds(location.left,location.top,q.width,q.height);p.show();p.setZIndex(this.getZIndex()+1);}
,__pN:function(e){var s=this.__pD;var v=Math.max(s.left,Math.min(s.right,e.getDocumentLeft()));var u=Math.max(s.top,Math.min(s.bottom,e.getDocumentTop()));var r=this.__pE+v;var t=this.__pF+u;return {viewportLeft:r,viewportTop:t,parentLeft:r-this.__pG,parentTop:t-this.__pH};}
,_onMoveMouseDown:function(e){if(!this.getMovable()||this.hasState(d)){return;}
;var parent=this.getLayoutParent();var x=parent.getContentLocation();var y=parent.getBounds();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isContentBlocked()){this.__pJ=parent.getBlockerColor();this.__pK=parent.getBlockerOpacity();parent.setBlockerColor(null);parent.setBlockerOpacity(1);parent.blockContent(this.getZIndex()-1);this.__pI=true;}
;}
;this.__pD={left:x.left,top:x.top,right:x.left+y.width,bottom:x.top+y.height};var w=this.getContainerLocation();this.__pG=x.left;this.__pH=x.top;this.__pE=w.left-e.getDocumentLeft();this.__pF=w.top-e.getDocumentTop();this.addState(b);this.__pB.capture();if(this.getUseMoveFrame()){this.__pM();}
;e.stop();}
,_onMoveMouseMove:function(e){if(!this.hasState(b)){return;}
;var z=this.__pN(e);if(this.getUseMoveFrame()){this.__pL().setDomPosition(z.viewportLeft,z.viewportTop);}
else {this.setDomPosition(z.parentLeft,z.parentTop);}
;e.stopPropagation();}
,_onMoveMouseUp:function(e){if(!this.hasState(b)){return;}
;this.removeState(b);var parent=this.getLayoutParent();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__pI){parent.unblockContent();parent.setBlockerColor(this.__pJ);parent.setBlockerOpacity(this.__pK);this.__pJ=null;this.__pK=0;this.__pI=false;}
;}
;this.__pB.releaseCapture();var A=this.__pN(e);this.setLayoutProperties({left:A.parentLeft,top:A.parentTop});if(this.getUseMoveFrame()){this.__pL().exclude();}
;e.stopPropagation();}
,__pO:function(e){if(!this.hasState(b)){return;}
;this.removeState(b);if(this.getUseMoveFrame()){this.__pL().exclude();}
;}
},destruct:function(){this._disposeObjects(k,f);this.__pD=null;}
});}
)();
...
(function(){var k="mousedown",j="w-resize",i="sw-resize",h="n-resize",g="resizableRight",f="ne-resize",d="se-resize",c="Integer",b="e-resize",a="resizableLeft",z="mousemove",y="move",x="shorthand",w="maximized",v="nw-resize",u="mouseout",t="qx.ui.core.MResizable",s="mouseup",r="losecapture",q="resize-frame",o="resizableBottom",p="s-resize",m="resizableTop",n="Boolean",l="resize";qx.Mixin.define(t,{construct:function(){var A=this.getContainerElement();A.addListener(k,this.__qd,this,true);A.addListener(s,this.__qe,this);A.addListener(z,this.__qg,this);A.addListener(u,this.__qh,this);A.addListener(r,this.__qf,this);var B=this.getContainerElement().getDomElement();if(B==null){B=window;}
;this.__pP=qx.event.Registration.getManager(B).getHandler(qx.event.handler.DragDrop);}
,properties:{resizableTop:{check:n,init:true},resizableRight:{check:n,init:true},resizableBottom:{check:n,init:true},resizableLeft:{check:n,init:true},resizable:{group:[m,g,o,a],mode:x},resizeSensitivity:{check:c,init:5},useResizeFrame:{check:n,init:true}},members:{__pP:null,__pQ:null,__pR:null,__pS:null,__pT:null,__pU:null,__pV:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__pQ;if(!C){C=this.__pQ=new qx.ui.core.Widget();C.setAppearance(q);C.exclude();qx.core.Init.getApplication().getRoot().add(C);}
;return C;}
,__pW:function(){var location=this.__qa();var D=this._getResizeFrame();D.setUserBounds(location.left,location.top,location.right-location.left,location.bottom-location.top);D.show();D.setZIndex(this.getZIndex()+1);}
,__pX:function(e){var F=this.__pR;var G=this.getSizeHint();var K=this.__pV;var J=this.__pU;var E=J.width;var I=J.height;var H=J.containerWidth;var M=J.containerHeight;var N=J.left;var top=J.top;var L;if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){L=Math.max(K.top,Math.min(K.bottom,e.getDocumentTop()))-this.__pT;if(F&this.RESIZE_TOP){I-=L;M-=L;}
else {I+=L;M+=L;}
...
;}
;if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){L=Math.max(K.left,Math.min(K.right,e.getDocumentLeft()))-this.__pS;if(F&this.RESIZE_LEFT){E-=L;H-=L;}
else {E+=L;H+=L;}
...
,__pY:{'1':h,'2':p,'4':j,'8':b,'5':v,'6':i,'9':f,'10':d},__qa:function(){var O=this.getDecoratorElement();if(O&&O.getDomElement()){return qx.bom.element.Location.get(O.getDomElement());}
else {return this.getContentLocation();}
;}
,__qb:function(e){var location=this.__qa();var P=this.getResizeSensitivity();var S=e.getDocumentLeft();var R=e.getDocumentTop();var Q=this.__qc(location,S,R,P);if(Q>0){Q=Q|this.__qc(location,S,R,P*2);}
;this.__pR=Q;}
,__qc:function(location,T,U,V){var W=0;if(this.getResizableTop()&&Math.abs(location.top-U)<V&&T>location.left-V&&T<location.right+V){W+=this.RESIZE_TOP;}
...
;return W;}
,__qd:function(e){if(!this.__pR||!this.getEnabled()){return;}
;this.addState(l);this.__pS=e.getDocumentLeft();this.__pT=e.getDocumentTop();var bb=this.getContainerLocation();var X=this.__qa();var ba=this.getBounds();this.__pU={top:X.top,left:X.left,containerWidth:bb.right-bb.left,containerHeight:bb.bottom-bb.top,width:X.right-X.left,height:X.bottom-X.top,bounds:qx.lang.Object.clone(ba)};var parent=this.getLayoutParent();var bc=parent.getContentLocation();var Y=parent.getBounds();this.__pV={left:bc.left,top:bc.top,right:bc.left+Y.width,bottom:bc.top+Y.height};if(this.getUseResizeFrame()){this.__pW();}
;this.capture();e.stop();}
,__qe:function(e){if(!this.hasState(l)||!this.getEnabled()){return;}
;if(this.getUseResizeFrame()){this._getResizeFrame().exclude();}
;var bd=this.__pX(e);this.setWidth(bd.containerWidth);this.setHeight(bd.containerHeight);if(this.getResizableLeft()||this.getResizableTop()){this.setLayoutProperties({left:bd.parentLeft,top:bd.parentTop});}
;this.__pR=0;this.removeState(l);this.resetCursor();this.getApplicationRoot().resetGlobalCursor();this.releaseCapture();e.stopPropagation();}
,__qf:function(e){if(!this.__pR){return;}
...
;}
,__qg:function(e){if(!this.getEnabled()){return;}
;if(this.hasState(l)){var bh=this.__pX(e);if(this.getUseResizeFrame()){var bf=this._getResizeFrame();bf.setUserBounds(bh.viewportLeft,bh.viewportTop,bh.width,bh.height);}
...
;}
;e.stopPropagation();}
else if(!this.hasState(w)&&!this.__pP.isSessionActive()){this.__qb(e);var bi=this.__pR;var bg=this.getApplicationRoot();if(bi){var be=this.__pY[bi];this.setCursor(be);bg.setGlobalCursor(be);}
else if(this.getCursor()){this.resetCursor();bg.resetGlobalCursor();}
;}
;}
,__qh:function(e){if(this.getCursor()&&!this.hasState(l)){this.resetCursor();this.getApplicationRoot().resetGlobalCursor();}
;}
},destruct:function(){if(this.__pQ!=null&&!qx.core.ObjectRegistry.inShutDown){this.__pQ.destroy();this.__pQ=null;}
;this.__pP=null;}
});}
)();
...
;var bn=this.getLayoutProperties();this.__qj=bn.left===undefined?0:bn.left;this.__qi=bn.top===undefined?0:bn.top;this.setLayoutProperties({left:null,top:null,edge:0});this.addState(C);this._updateCaptionBar();this.fireEvent(p);}
;}
;}
,minimize:function(){if(!this.isVisible()){return;}
;if(this.fireNonBubblingEvent(j,qx.event.type.Event,[false,true])){var bo=this.getLayoutProperties();this.__qj=bo.left===undefined?0:bo.left;this.__qi=bo.top===undefined?0:bo.top;this.removeState(C);this.hide();this.fireEvent(W);}
;}
...
;var bp=this.__qj;var top=this.__qi;this.setLayoutProperties({edge:null,left:bp,top:top});this.removeState(C);this._updateCaptionBar();this.fireEvent(q);}
;}
...
(function(){var f="icon/22/actions/view-refresh.png",e='90%',d="demobrowser.demo.table.Table_Remote_Model",c='10%',b='Reload',a='execute';qx.Class.define(d,{extend:demobrowser.demo.table.TableDemo,members:{createTable:function(){var g=this._tableModel=new demobrowser.demo.table.RemoteTableModel();var j={tableColumnModel:function(k){return new qx.ui.table.columnmodel.Resize(k);}
};var h=new qx.ui.table.Table(g,j);var i=h.getTableColumnModel().getBehavior();i.setWidth(0,c);i.setWidth(1,e);return h;}
,createControls:function(){var n=new qx.ui.toolbar.ToolBar();var m=new qx.ui.toolbar.Part();n.add(m);var l=new qx.ui.toolbar.Button(b,f);l.addListener(a,function(){this._tableModel.reloadData();}
,this);m.add(l);return n;}
}});}
)();
...
,getColumnCount:function(){return this.__mR.length;}
,getColumnIndexById:function(E){return this.__mT[E];}
,getColumnId:function(F){return this.__mR[F];}
,getColumnName:function(G){return this.__mS[G];}
,setColumnIds:function(H){this.__mR=H;this.__mT={};for(var i=0;i<H.length;i++){this.__mT[H[i]]=i;}
;this.__mS=new Array(H.length);if(!this.__mU){this.fireEvent(b);}
;}
,setColumnNamesByIndex:function(I){if(this.__mR.length!=I.length){throw new Error(f+this.__mR.length+c+I.length);}
;this.__mS=I;this.fireEvent(b);}
,setColumnNamesById:function(J){this.__mS=new Array(this.__mR.length);for(var i=0;i<this.__mR.length;++i){this.__mS[i]=J[this.__mR[i]];}
;}
,setColumns:function(K,L){var M=this.__mR.length==0||L;if(L==null){if(this.__mR.length==0){L=K;}
else {L=this.__mR;}
;}
;if(L.length!=K.length){throw new Error(k+L.length+c+K.length);}
;if(M){this.__mU=true;this.setColumnIds(L);this.__mU=false;}
;this.setColumnNamesByIndex(K);}
},destruct:function(){this.__mR=this.__mS=this.__mT=null;}
});}
)();
(function(){var f="qx.ui.table.model.Remote",e="Boolean",d="Integer",c="_loadRowCount is abstract",b="metaDataChanged",a="dataChanged";qx.Class.define(f,{extend:qx.ui.table.model.Abstract,construct:function(){qx.ui.table.model.Abstract.call(this);this.__mW=-1;this.__nb=true;this.__KG=-1;this.__KH=0;this.__KI=-1;this.__KJ=-1;this.__KK=-1;this.__KL=false;this.__KM={};this.__KN=0;this.__na=null;this.__mY=null;}
,properties:{blockSize:{check:d,init:50},maxCachedBlockCount:{check:d,init:15},clearCacheOnRemove:{check:e,init:false},blockConcurrentLoadRowCount:{check:e,init:true}},members:{__KG:null,__KL:null,__KH:null,__KI:null,__KJ:null,__KK:null,__KM:null,__KN:null,__mW:null,__nb:null,__mY:null,__na:null,__KO:false,_getIgnoreCurrentRequest:function(){return this.__KL;}
,getRowCount:function(){if(this.__KG==-1){if(!this.__KO||!this.getBlockConcurrentLoadRowCount()){this.__KO=true;this._loadRowCount();}
;return (this.__KG==-1)?0:this.__KG;}
else {return this.__KG;}
;}
,_loadRowCount:function(){throw new Error(c);}
,_onRowCountLoaded:function(g){if(this.getBlockConcurrentLoadRowCount()){this.__KO=false;}
;if(g==null||g<0){g=0;}
;this.__KG=Number(g);var h={firstRow:0,lastRow:g-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,h);}
,reloadData:function(){this.clearCache();if(this.__KI!=-1){var j=this._cancelCurrentRequest();if(j){this.__KI=-1;this.__KL=false;}
else {this.__KL=true;}
;}
;this.__KJ=-1;this.__KK=-1;this.__KO=true;this._loadRowCount();}
,clearCache:function(){this.__KM={};this.__KN=0;}
,getCacheContent:function(){return {sortColumnIndex:this.__mW,sortAscending:this.__nb,rowCount:this.__KG,lruCounter:this.__KH,rowBlockCache:this.__KM,rowBlockCount:this.__KN};}
,restoreCacheContent:function(k){if(this.__KI!=-1){var m=this._cancelCurrentRequest();if(m){this.__KI=-1;this.__KL=false;}
else {this.__KL=true;}
;}
;this.__mW=k.sortColumnIndex;this.__nb=k.sortAscending;this.__KG=k.rowCount;this.__KH=k.lruCounter;this.__KM=k.rowBlockCache;this.__KN=k.rowBlockCount;var l={firstRow:0,lastRow:this.__KG-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,l);}
,_cancelCurrentRequest:function(){return false;}
,iterateCachedRows:function(n,o){var q=this.getBlockSize();var p=Math.ceil(this.getRowCount()/q);for(var x=0;x<=p;x++){var r=this.__KM[x];if(r!=null){var w=x*q;var v=r.rowDataArr;for(var u=0;u<v.length;u++){var t=v[u];var s=n.call(o,w+u,t);if(s!=null){v[u]=s;}
;}
;}
;}
;}
,prefetchRows:function(y,z){if(this.__KI==-1){var A=this.getBlockSize();var F=Math.ceil(this.__KG/A);var E=parseInt(y/A,10)-1;if(E<0){E=0;}
;var D=parseInt(z/A,10)+1;if(D>=F){D=F-1;}
;var C=-1;var B=-1;for(var G=E;G<=D;G++){if(this.__KM[G]==null||this.__KM[G].isDirty){if(C==-1){C=G;}
;B=G;}
;}
;if(C!=-1){this.__KJ=-1;this.__KK=-1;this.__KI=C;this._loadRowData(C*A,(B+1)*A-1);}
;}
else {this.__KJ=y;this.__KK=z;}
;}
,_loadRowData:function(H,I){throw new Error(c);}
,_onRowDataLoaded:function(J){if(J!=null&&!this.__KL){var M=this.getBlockSize();var K=Math.ceil(J.length/M);if(K==1){this._setRowBlockData(this.__KI,J);}
else {for(var i=0;i<K;i++){var P=i*M;var O=[];var L=Math.min(M,J.length-P);for(var Q=0;Q<L;Q++){O.push(J[P+Q]);}
;this._setRowBlockData(this.__KI+i,O);}
;}
;var N={firstRow:this.__KI*M,lastRow:(this.__KI+K+1)*M-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,N);}
;this.__KI=-1;this.__KL=false;if(this.__KJ!=-1){this.prefetchRows(this.__KJ,this.__KK);}
;}
,_setRowBlockData:function(R,S){if(this.__KM[R]==null){this.__KN++;while(this.__KN>this.getMaxCachedBlockCount()){var W;var V=this.__KH;for(var U in this.__KM){var T=this.__KM[U].lru;if(T<V&&U>1){V=T;W=U;}
;}
;delete this.__KM[W];this.__KN--;}
;}
;this.__KM[R]={lru:++this.__KH,rowDataArr:S};}
,removeRow:function(X){if(this.getClearCacheOnRemove()){this.clearCache();var bf={firstRow:0,lastRow:this.getRowCount()-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,bf);}
else {var bb=this.getBlockSize();var bc=Math.ceil(this.getRowCount()/bb);var bd=parseInt(X/bb,10);for(var bg=bd;bg<=bc;bg++){var Y=this.__KM[bg];if(Y!=null){var ba=0;if(bg==bd){ba=X-bg*bb;}
;Y.rowDataArr.splice(ba,1);if(bg==bc-1){if(Y.rowDataArr.length==0){delete this.__KM[bg];}
;}
else {var be=this.__KM[bg+1];if(be!=null){Y.rowDataArr.push(be.rowDataArr[0]);}
else {Y.isDirty=true;}
;}
;}
;}
;if(this.__KG!=-1){this.__KG--;}
;if(this.hasListener(a)){var bf={firstRow:X,lastRow:this.getRowCount()-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,bf);}
;}
;}
,getRowData:function(bh){var bi=this.getBlockSize();var bl=parseInt(bh/bi,10);var bj=this.__KM[bl];if(bj==null){return null;}
else {var bk=bj.rowDataArr[bh-(bl*bi)];if(bj.lru!=this.__KH){bj.lru=++this.__KH;}
;return bk;}
;}
,getValue:function(bm,bn){var bo=this.getRowData(bn);if(bo==null){return null;}
else {var bp=this.getColumnId(bm);return bo[bp];}
;}
,setValue:function(bq,br,bs){var bt=this.getRowData(br);if(bt==null){return;}
else {var bv=this.getColumnId(bq);bt[bv]=bs;if(this.hasListener(a)){var bu={firstRow:br,lastRow:br,firstColumn:bq,lastColumn:bq};this.fireDataEvent(a,bu);}
;}
;}
,setEditable:function(bw){this.__mY=[];for(var bx=0;bx<this.getColumnCount();bx++){this.__mY[bx]=bw;}
;this.fireEvent(b);}
,setColumnEditable:function(by,bz){if(bz!=this.isColumnEditable(by)){if(this.__mY==null){this.__mY=[];}
;this.__mY[by]=bz;this.fireEvent(b);}
;}
,isColumnEditable:function(bA){return (this.__mY?(this.__mY[bA]==true):false);}
,setColumnSortable:function(bB,bC){if(bC!=this.isColumnSortable(bB)){if(this.__na==null){this.__na=[];}
;this.__na[bB]=bC;this.fireEvent(b);}
;}
,isColumnSortable:function(bD){return (this.__na?(this.__na[bD]!==false):true);}
,sortByColumn:function(bE,bF){if(this.__mW!=bE||this.__nb!=bF){this.__mW=bE;this.__nb=bF;this.clearCache();this.fireEvent(b);}
;}
,getSortColumnIndex:function(){return this.__mW;}
,isSortAscending:function(){return this.__nb;}
,setSortColumnIndexWithoutSortingData:function(bG){this.__mW=bG;}
,setSortAscendingWithoutSortingData:function(bH){this.__nb=bH;}
},destruct:function(){this.__na=this.__mY=this.__KM=null;}
});}
)();
(function(){var e="text",d="Text",c='demobrowser.demo.table.RemoteTableModel',b="Id",a="id";qx.Class.define(c,{extend:qx.ui.table.model.Remote,construct:function(){qx.ui.table.model.Remote.call(this);this.setColumns([b,d],[a,e]);}
,members:{__QJ:null,__QK:false,_loadRowCount:function(){this.__QL();if(this.__QK){return;}
;if(this.__QJ){this.__QM();}
else {this.__QP();}
;}
,_loadRowData:function(f,g){this.__QL();if(this.__QK){return;}
;if(this.__QJ){this.__QN(f,g);}
else {this.__QQ(f,g);}
;}
,__QL:function(){if(this.__QK||this.__QJ!==null){return;}
;this.__QK=true;this.__QO("method=checkphp",function(h){this.__QK=false;this.__QJ=(h=="WTF PHP");this._loadRowCount();}
);}
,__QM:function(){var j="method=getRowCount";this.__QO(j,function(k){this._onRowCountLoaded(parseInt(k));}
);}
,__QN:function(l,m){var n="method=getRowData&start="+l+"&end="+m;this.__QO(n,function(o){this._onRowDataLoaded(qx.lang.Json.parse(o));}
);}
,__QO:function(p,q){var r=new qx.io.request.Xhr();r.setUrl("../../resource/demobrowser/backend/remote_table.php?"+p);r.addListener("success",function(){q.call(this,r.getResponseText());}
,this);r.send();}
,__QP:function(){var self=this;window.setTimeout(function(){self._onRowCountLoaded(1000000);}
,0);}
,__QQ:function(s,t){var self=this;window.setTimeout(function(){var u=[];for(var i=s;i<=t;i++){u.push({id:i,text:'Hello '+i+' Generated on:'+(new Date())});}
;self._onRowDataLoaded(u);}
,0);}
}});}
)();
...
;this.__qt={};var O=this._transport=this._createTransport();this._setPhase(p);this.__su=qx.lang.Function.bind(this._onReadyStateChange,this);this.__sv=qx.lang.Function.bind(this._onLoad,this);this.__sw=qx.lang.Function.bind(this._onLoadEnd,this);this.__sx=qx.lang.Function.bind(this._onAbort,this);this.__sy=qx.lang.Function.bind(this._onTimeout,this);this.__sz=qx.lang.Function.bind(this._onError,this);O.onreadystatechange=this.__su;O.onload=this.__sv;O.onloadend=this.__sw;O.onabort=this.__sx;O.ontimeout=this.__sy;O.onerror=this.__sz;}
...
,nullable:true},authentication:{check:c,nullable:true}},members:{__su:null,__sv:null,__sw:null,__sx:null,__sy:null,__sz:null,__sA:null,__sB:null,__sC:null,__qt:null,__sD:null,_transport:null,_createTransport:function(){throw new Error(A);}
...
;this.__sB=true;this.__sC=x;this._transport.abort();}
,_setRequestHeaders:function(){var X=this._transport,V=this._getAllRequestHeaders();for(var W in V){X.setRequestHeader(W,V[W]);}
;}
,_getAllRequestHeaders:function(){var Y=qx.lang.Object.merge({},this._getConfiguredRequestHeaders(),this.__sE(),this.__sD,this.__qt);return Y;}
,__sE:function(){var bb=this.getAuthentication(),ba={};if(bb){bb.getAuthHeaders().forEach(function(bc){ba[bc.key]=bc.value;}
);return ba;}
;}
,setRequestHeader:function(bd,be){this.__qt[bd]=be;}
,getRequestHeader:function(bf){return this.__qt[bf];}
,removeRequestHeader:function(bg){if(this.__qt[bg]){delete this.__qt[bg];}
;}
,getTransport:function(){return this._transport;}
,getReadyState:function(){return this._transport.readyState;}
,getPhase:function(){return this.__sC;}
...
,getResponse:function(){return this.__sA;}
,_setResponse:function(bi){var bj=bi;if(this.__sA!==bi){this.__sA=bi;this.fireEvent(m,qx.event.type.Data,[this.__sA,bj]);}
;}
,_onReadyStateChange:function(){var bk=this.getReadyState();if(qx.core.Environment.get(C)){this.debug(D+bk);}
;this.fireEvent(r);if(this.__sB){return;}
;if(bk===3){this._setPhase(M);}
;if(this.isDone()){this.__sF();}
;}
,__sF:function(){var bl;if(qx.core.Environment.get(C)){this.debug(E+this.getStatus());}
...
,_setPhase:function(bn){var bo=this.__sC;if(qx.core.Environment.get(v)){qx.core.Assert.assertString(bn);qx.core.Assert.assertMatch(bn,/^(unsent)|(opened)|(sent)|(loading)|(load)|(success)|(abort)|(timeout)|(statusError)$/);}
;this.__sC=bn;this.fireDataEvent(h,bn,bo);}
...
;var y=v[f+qx.lang.String.firstUp(name)]();if(qx.lang.Type.isArray(y)){var A=qx.data&&qx.data.IListData&&qx.Class.hasInterface(y&&y.constructor,qx.data.IListData);for(var i=0;i<y.length;i++){var z=A?y.getItem(i):y[i];B+=this.__di(name,z,w);}
;}
else if(qx.lang.Type.isDate(y)&&x!=null){B+=this.__di(name,x.format(y),w);}
else {B+=this.__di(name,y,w);}
;}
;return B.substring(0,B.length-1);}
,__di:function(name,D,E){if(D instanceof qx.core.Object&&E!=null){var F=encodeURIComponent(E(D));if(F===undefined){var F=encodeURIComponent(D);}
;}
...
,init:true}},members:{__sG:null,_createTransport:function(){return new qx.bom.request.Xhr();}
...
;}
;return C;}
,setParser:function(D){var E=qx.io.request.Xhr;if(typeof E.PARSER[D]===h){return this.__sG=E.PARSER[D];}
;if(qx.core.Environment.get(o)){qx.core.Assert.assertFunction(D);}
;return this.__sG=D;}
,_getParser:function(){var F=this.__sG,G;if(F){return F;}
...
(function(){var k="Retry open native request with method: ",j="activex",i="No XHR support available.",h="If-None-Match",g="xhr",f="engine.version",d="GET",c="-1",b="Open native request with method: ",a="Received native readyState: ",P="abort",O="Microsoft.XMLHTTP",N="Already disposed",M="browser.version",L="on",K="If-Match",J="Microsoft.XMLDOM",I="Send native request",H="If-Range",G="Content-Type",r="io.xhr",s="Not enough arguments",p="timeout",q="If-Modified-Since",n="onunload",o="load",l=", url: ",m="browser.documentmode",t=", async: ",u="loadend",y="opera",x="gecko",A="file:",z="error",C="mshtml",B="undefined",w="qx.debug.io",F="",E="readystatechange",D="qx.bom.request.Xhr",v="engine.name";qx.Bootstrap.define(D,{construct:function(){this.__sH=qx.Bootstrap.bind(this.__sU,this);this.__sI=qx.Bootstrap.bind(this.__sT,this);this.__sy=qx.Bootstrap.bind(this.__sY,this);this.__sS();this._emitter=new qx.event.Emitter();if(window.attachEvent){this.__sJ=qx.Bootstrap.bind(this.__tc,this);window.attachEvent(n,this.__sJ);}
;}
,statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},events:{"readystatechange":D,"error":D,"loadend":D,"timeout":D,"abort":D,"load":D},members:{readyState:0,responseText:F,responseXML:null,status:0,statusText:F,timeout:0,open:function(Q,R,S,T,U){this.__te();if(typeof R===B){throw new Error(s);}
else if(typeof Q===B){Q=d;}
;this.__sB=false;this.__sK=false;this.__sL=false;this.__sM=R;if(typeof S==B){S=true;}
;this.__sN=S;if(!this.__td()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();this.__sS();}
;this.__sP.onreadystatechange=this.__sH;try{if(qx.core.Environment.get(w)){qx.Bootstrap.debug(qx.bom.request.Xhr,b+Q+l+R+t+S);}
;this.__sP.open(Q,R,S,T,U);}
catch(V){if(!qx.util.Request.isCrossDomain(R)){throw V;}
;if(!this.__sN){this.__sO=V;}
;if(this.__sN){if(window.XDomainRequest){this.readyState=4;this.__sP=new XDomainRequest();this.__sP.onerror=qx.Bootstrap.bind(function(){this._emit(E);this._emit(z);this._emit(u);}
,this);if(qx.core.Environment.get(w)){qx.Bootstrap.debug(qx.bom.request.Xhr,k+Q+l+R+t+S);}
;this.__sP.open(Q,R,S,T,U);return;}
;window.setTimeout(qx.Bootstrap.bind(function(){if(this.__sR){return;}
;this.readyState=4;this._emit(E);this._emit(z);this._emit(u);}
,this));}
;}
;if(qx.core.Environment.get(v)===C&&qx.core.Environment.get(m)<9&&this.__sP.readyState>0){this.__sP.setRequestHeader(q,c);}
;if(qx.core.Environment.get(v)===x&&parseInt(qx.core.Environment.get(f),10)<2&&!this.__sN){this.readyState=qx.bom.request.Xhr.OPENED;this._emit(E);}
;}
,setRequestHeader:function(W,X){this.__te();if(W==K||W==q||W==h||W==H){this.__sL=true;}
;this.__sP.setRequestHeader(W,X);return this;}
,send:function(Y){this.__te();if(!this.__sN&&this.__sO){throw this.__sO;}
;if(qx.core.Environment.get(v)===y&&this.timeout===0){this.timeout=10000;}
;if(this.timeout>0){this.__sQ=window.setTimeout(this.__sy,this.timeout);}
;Y=typeof Y==B?null:Y;try{if(qx.core.Environment.get(w)){qx.Bootstrap.debug(qx.bom.request.Xhr,I);}
;this.__sP.send(Y);}
catch(bb){if(!this.__sN){throw bb;}
;if(this._getProtocol()===A){this.readyState=2;this.__sV();var ba=this;window.setTimeout(function(){if(ba.__sR){return;}
;ba.readyState=3;ba.__sV();ba.readyState=4;ba.__sV();}
);}
;}
;if(qx.core.Environment.get(v)===x&&!this.__sN){this.__sU();}
;this.__sK=true;return this;}
,abort:function(){this.__te();this.__sB=true;this.__sP.abort();if(this.__sP){this.readyState=this.__sP.readyState;}
;return this;}
,_emit:function(event){this[L+event]();this._emitter.emit(event,this);}
...
,getResponseHeader:function(be){this.__te();return this.__sP.getResponseHeader(be);}
,getAllResponseHeaders:function(){this.__te();return this.__sP.getAllResponseHeaders();}
,getRequest:function(){return this.__sP;}
,dispose:function(){if(this.__sR){return false;}
;window.clearTimeout(this.__sQ);if(window.detachEvent){window.detachEvent(n,this.__sJ);}
;try{this.__sP.onreadystatechange;}
catch(bg){return;}
;var bf=function(){}
;this.__sP.onreadystatechange=bf;this.__sP.onload=bf;this.__sP.onerror=bf;this.abort();this.__sP=null;this.__sR=true;return true;}
,_createNativeXhr:function(){var bh=qx.core.Environment.get(r);if(bh===g){return new XMLHttpRequest();}
;if(bh==j){return new window.ActiveXObject(O);}
;qx.Bootstrap.error(this,i);}
,_getProtocol:function(){var bi=this.__sM;var bj=/^(\w+:)\/\//;if(bi!==null&&bi.match){var bk=bi.match(bj);if(bk&&bk[1]){return bk[1];}
;}
;return window.location.protocol;}
,__sP:null,__sN:null,__sH:null,__sI:null,__sJ:null,__sy:null,__sK:null,__sM:null,__sB:null,__dy:null,__sR:null,__sQ:null,__sO:null,__sL:null,__sS:function(){this.__sP=this._createNativeXhr();this.__sP.onreadystatechange=this.__sH;if(this.__sP.onabort){this.__sP.onabort=this.__sI;}
;this.__sR=this.__sK=this.__sB=false;}
,__sT:function(){if(!this.__sB){this.abort();}
;}
,__sU:function(){var bl=this.__sP,bm=true;if(qx.core.Environment.get(w)){qx.Bootstrap.debug(qx.bom.request.Xhr,a+bl.readyState);}
;if(this.readyState==bl.readyState){return;}
;this.readyState=bl.readyState;if(this.readyState===qx.bom.request.Xhr.DONE&&this.__sB&&!this.__sK){return;}
;if(!this.__sN&&(bl.readyState==2||bl.readyState==3)){return;}
...
;if(bm){this.__ta();this.__tb();}
;}
;this.__sV();if(this.readyState==qx.bom.request.Xhr.DONE){if(bl){bl.onreadystatechange=function(){}
;}
;}
;}
,__sV:function(){var bo=this;if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__sQ);}
;if(qx.core.Environment.get(v)==C&&qx.core.Environment.get(m)<8){if(this.__sN&&!this.__sK&&this.readyState>=qx.bom.request.Xhr.LOADING){if(this.readyState==qx.bom.request.Xhr.LOADING){return;}
;if(this.readyState==qx.bom.request.Xhr.DONE){window.setTimeout(function(){if(bo.__sR){return;}
;bo.readyState=3;bo._emit(E);bo.readyState=4;bo._emit(E);bo.__sW();}
);return;}
;}
;}
;this._emit(E);if(this.readyState===qx.bom.request.Xhr.DONE){this.__sW();}
;}
,__sW:function(){if(this.__dy){this._emit(p);if(qx.core.Environment.get(v)===y){this._emit(z);}
;this.__dy=false;}
else {if(this.__sB){this._emit(P);}
else {if(this.__sX()){this._emit(z);}
else {this._emit(o);this._emitter.emit(o);}
;}
;}
;this._emit(u);}
,__sX:function(){var bp;if(this._getProtocol()===A){bp=!this.responseText;}
else {bp=!this.statusText;}
;return bp;}
,__sY:function(){var bq=this.__sP;this.readyState=qx.bom.request.Xhr.DONE;this.__dy=true;bq.abort();this.responseText=F;this.responseXML=null;this.__sV();}
,__ta:function(){var br=this.readyState===qx.bom.request.Xhr.DONE;if(this._getProtocol()===A&&this.status===0&&br){if(!this.__sX()){this.status=200;}
;}
;if(this.status===1223){this.status=204;}
;if(qx.core.Environment.get(v)===y){if(br&&this.__sL&&!this.__sB&&this.status===0){this.status=304;}
;}
;}
,__tb:function(){if(qx.core.Environment.get(v)==C&&(this.getResponseHeader(G)||F).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var bs=new window.ActiveXObject(J);bs.async=false;bs.validateOnParse=false;bs.loadXML(this.responseText);this.responseXML=bs;}
;}
,__tc:function(){try{if(this){this.dispose();}
;}
catch(e){}
;}
,__td:function(){var name=qx.core.Environment.get(v);var bt=qx.core.Environment.get(M);return !(name==C&&bt<9||name==x&&bt<3.5);}
,__te:function(){if(this.__sR){throw new Error(N);}
;}
},defer:function(){qx.core.Environment.add(w,false);}
});}
)();
(function(){var b="qx.event.Emitter",a="*";qx.Bootstrap.define(b,{extend:Object,statics:{__Aj:[]},members:{__Ak:null,__Al:null,on:function(name,c,d){this.__Am(name).push({listener:c,ctx:d});qx.event.Emitter.__Aj.push({name:name,listener:c,ctx:d});return qx.event.Emitter.__Aj.length-1;}
,once:function(name,e,f){this.__Am(name).push({listener:e,ctx:f,once:true});qx.event.Emitter.__Aj.push({name:name,listener:e,ctx:f});return qx.event.Emitter.__Aj.length-1;}
,off:function(name,g,h){var k=this.__Am(name);for(var i=k.length-1;i>=0;i--){var j=k[i];if(j.listener==g&&j.ctx==h){k.splice(i,1);return i;}
;}
;return null;}
,offById:function(l){var m=qx.event.Emitter.__Aj[l];this.off(m.name,m.listener,m.ctx);}
...
,emit:function(name,u){var w=this.__Am(name);for(var i=w.length-1;i>=0;i--){var v=w[i];v.listener.call(v.ctx,u);if(v.once){w.splice(i,1);}
;}
;w=this.__Am(a);for(var i=w.length-1;i>=0;i--){var v=w[i];v.listener.call(v.ctx,u);}
;}
,getListeners:function(){return this.__Ak;}
,__Am:function(name){if(this.__Ak==null){this.__Ak={};}
;if(this.__Ak[name]==null){this.__Ak[name]=[];}
;return this.__Ak[name];}
}});}
)();
...
(function(){var k="Invalid argument 'colCount'.",j="Invalid argument 'visible'.",h="Invalid argument 'factory'.",g="Invalid argument 'newPositions'.",f="Invalid argument 'width'.",e="__nI",d="Invalid argument 'visXPos'.",c="Invalid argument 'toOverXPos'.",b="qx.ui.table.columnmodel.Basic",a="__nG",C="headerCellRendererChanged",B=". The column is not added to this TablePaneModel.",A="setColumnsOrder: Invalid number of column positions given, expected ",z="widthChanged",y="Invalid argument 'overXPos'.",w=", got ",v="Invalid argument 'fromOverXPos'.",u="Showing column failed: ",t="__nH",s="orderChanged",q="Invalid argument 'renderer'.",r="visibilityChanged",o="visibilityChangedPre",p="qx.event.type.Data",m="Column not found in table model",n="Invalid argument 'col'.",l="qx.debug";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__nC=[];this.__nD=[];}
,events:{"widthChanged":p,"visibilityChangedPre":p,"visibilityChanged":p,"orderChanged":p,"headerCellRendererChanged":p},statics:{DEFAULT_WIDTH:100,DEFAULT_HEADER_RENDERER:qx.ui.table.headerrenderer.Default,DEFAULT_DATA_RENDERER:qx.ui.table.cellrenderer.Default,DEFAULT_EDITOR_FACTORY:qx.ui.table.celleditor.TextField},members:{__mU:null,__nE:null,__nD:null,__nC:null,__nF:null,__nG:null,__nH:null,__nI:null,init:function(D,E){if(qx.core.Environment.get(l)){this.assertInteger(D,k);}
;this.__nF=[];var I=qx.ui.table.columnmodel.Basic.DEFAULT_WIDTH;var J=this.__nG||(this.__nG=new qx.ui.table.columnmodel.Basic.DEFAULT_HEADER_RENDERER());var G=this.__nH||(this.__nH=new qx.ui.table.columnmodel.Basic.DEFAULT_DATA_RENDERER());var F=this.__nI||(this.__nI=new qx.ui.table.columnmodel.Basic.DEFAULT_EDITOR_FACTORY());this.__nC=[];this.__nD=[];var L;if(E){L=E.getInitiallyHiddenColumns();}
;L=L||[];for(var M=0;M<D;M++){this.__nF[M]={width:I,headerRenderer:J,dataRenderer:G,editorFactory:F};this.__nC[M]=M;this.__nD[M]=M;}
;this.__nE=null;this.__mU=true;for(var K=0;K<L.length;K++){this.setColumnVisible(L[K],false);}
;this.__mU=false;for(M=0;M<D;M++){var H={col:M,visible:this.isColumnVisible(M)};this.fireDataEvent(o,H);this.fireDataEvent(r,H);}
;}
,getVisibleColumns:function(){return this.__nD!=null?this.__nD:[];}
,setColumnWidth:function(N,O,P){if(qx.core.Environment.get(l)){this.assertInteger(N,n);this.assertInteger(O,f);this.assertNotUndefined(this.__nF[N],m);}
;var R=this.__nF[N].width;if(R!=O){this.__nF[N].width=O;var Q={col:N,newWidth:O,oldWidth:R,isMouseAction:P||false};this.fireDataEvent(z,Q);}
;}
,getColumnWidth:function(S){if(qx.core.Environment.get(l)){this.assertInteger(S,n);this.assertNotUndefined(this.__nF[S],m);}
;return this.__nF[S].width;}
,setHeaderCellRenderer:function(T,U){if(qx.core.Environment.get(l)){this.assertInteger(T,n);this.assertInterface(U,qx.ui.table.IHeaderRenderer,q);this.assertNotUndefined(this.__nF[T],m);}
;var V=this.__nF[T].headerRenderer;if(V!==this.__nG){V.dispose();}
;this.__nF[T].headerRenderer=U;this.fireDataEvent(C,{col:T});}
,getHeaderCellRenderer:function(W){if(qx.core.Environment.get(l)){this.assertInteger(W,n);this.assertNotUndefined(this.__nF[W],m);}
;return this.__nF[W].headerRenderer;}
,setDataCellRenderer:function(X,Y){if(qx.core.Environment.get(l)){this.assertInteger(X,n);this.assertInterface(Y,qx.ui.table.ICellRenderer,q);this.assertNotUndefined(this.__nF[X],m);}
;this.__nF[X].dataRenderer=Y;var ba=this.__nF[X].dataRenderer;if(ba!==this.__nH){return ba;}
;return null;}
,getDataCellRenderer:function(bb){if(qx.core.Environment.get(l)){this.assertInteger(bb,n);this.assertNotUndefined(this.__nF[bb],m);}
;return this.__nF[bb].dataRenderer;}
,setCellEditorFactory:function(bc,bd){if(qx.core.Environment.get(l)){this.assertInteger(bc,n);this.assertInterface(bd,qx.ui.table.ICellEditorFactory,h);this.assertNotUndefined(this.__nF[bc],m);}
;var be=this.__nF[bc].editorFactory;if(be!==this.__nI){be.dispose();}
;this.__nF[bc].editorFactory=bd;}
,getCellEditorFactory:function(bf){if(qx.core.Environment.get(l)){this.assertInteger(bf,n);this.assertNotUndefined(this.__nF[bf],m);}
;return this.__nF[bf].editorFactory;}
,_getColToXPosMap:function(){if(this.__nE==null){this.__nE={};for(var bi=0;bi<this.__nC.length;bi++){var bh=this.__nC[bi];this.__nE[bh]={overX:bi};}
;for(var bg=0;bg<this.__nD.length;bg++){var bh=this.__nD[bg];this.__nE[bh].visX=bg;}
;}
;return this.__nE;}
,getVisibleColumnCount:function(){return this.__nD!=null?this.__nD.length:0;}
...
,getOverallColumnCount:function(){return this.__nC.length;}
,getOverallColumnAtX:function(bl){if(qx.core.Environment.get(l)){this.assertInteger(bl,y);}
;return this.__nC[bl];}
...
;if(bp!=this.isColumnVisible(bo)){if(bp){var bv=this._getColToXPosMap();var bs=bv[bo].overX;if(bs==null){throw new Error(u+bo+B);}
;var bt;for(var x=bs+1;x<this.__nC.length;x++){var bu=this.__nC[x];var bq=bv[bu].visX;if(bq!=null){bt=bq;break;}
;}
;if(bt==null){bt=this.__nD.length;}
;this.__nD.splice(bt,0,bo);}
else {var br=this.getVisibleX(bo);this.__nD.splice(br,1);}
;this.__nE=null;if(!this.__mU){var bw={col:bo,visible:bp};this.fireDataEvent(o,bw);this.fireDataEvent(r,bw);}
;}
;}
,moveColumn:function(bx,by){if(qx.core.Environment.get(l)){this.assertInteger(bx,v);this.assertInteger(by,c);}
;this.__mU=true;var bB=this.__nC[bx];var bz=this.isColumnVisible(bB);if(bz){this.setColumnVisible(bB,false);}
;this.__nC.splice(bx,1);this.__nC.splice(by,0,bB);this.__nE=null;if(bz){this.setColumnVisible(bB,true);}
;this.__mU=false;var bA={col:bB,fromOverXPos:bx,toOverXPos:by};this.fireDataEvent(s,bA);}
,setColumnsOrder:function(bC){if(qx.core.Environment.get(l)){this.assertArray(bC,g);}
;if(bC.length==this.__nC.length){this.__mU=true;var bF=new Array(bC.length);for(var bD=0;bD<this.__nC.length;bD++){var bE=this.isColumnVisible(bD);bF[bD]=bE;if(bE){this.setColumnVisible(bD,false);}
;}
;this.__nC=qx.lang.Array.clone(bC);this.__nE=null;for(var bD=0;bD<this.__nC.length;bD++){if(bF[bD]){this.setColumnVisible(bD,true);}
;}
;this.__mU=false;this.fireDataEvent(s);}
else {throw new Error(A+this.__nC.length+w+bC.length);}
;}
},destruct:function(){for(var i=0;i<this.__nF.length;i++){this.__nF[i].headerRenderer.dispose();this.__nF[i].dataRenderer.dispose();this.__nF[i].editorFactory.dispose();}
;this.__nC=this.__nD=this.__nF=this.__nE=null;this._disposeObjects(a,t,e);}
});}
)();
(function(){var v="columnVisibilityMenuCreateEnd",u="tableWidthChanged",t="ontablewidthchanged",s="verticalScrollBarChanged",r="oncolumnwidthchanged",q="onvisibilitychanged",p="qx.ui.table.columnmodel.resizebehavior.Abstract",n="qx.ui.table.columnmodel.Resize",m="_applyBehavior",l="separator",e="visibilityChanged",k="Reset column widths",h="changeBehavior",c="user-button",b="onverticalscrollbarchanged",g="onappear",f="widthChanged",i="execute",a="appear",j="qx.tableResizeDebug",d="qx.debug";qx.Class.define(n,{extend:qx.ui.table.columnmodel.Basic,include:qx.locale.MTranslation,construct:function(){qx.ui.table.columnmodel.Basic.call(this);this.__nJ=false;this.__nK=false;}
,properties:{behavior:{check:p,init:null,nullable:true,apply:m,event:h}},members:{__nK:null,__nJ:null,__mv:null,_applyBehavior:function(w,x){if(x!=null){x.dispose();x=null;}
;w._setNumColumns(this.getOverallColumnCount());w.setTableColumnModel(this);}
,init:function(y,z){qx.ui.table.columnmodel.Basic.prototype.init.call(this,y,z);if(this.__mv==null){this.__mv=z;z.addListener(a,this._onappear,this);z.addListener(u,this._onTableWidthChanged,this);z.addListener(s,this._onverticalscrollbarchanged,this);z.addListener(v,this._addResetColumnWidthButton,this);this.addListener(f,this._oncolumnwidthchanged,this);this.addListener(e,this._onvisibilitychanged,this);}
;if(this.getBehavior()==null){this.setBehavior(new qx.ui.table.columnmodel.resizebehavior.Default());}
;this.getBehavior()._setNumColumns(y);}
,getTable:function(){return this.__mv;}
,_addResetColumnWidthButton:function(event){var C=event.getData();var B=C.columnButton;var A=C.menu;var o;o=B.factory(l);A.add(o);o=B.factory(c,{text:this.tr(k)});A.add(o);o.addListener(i,this._onappear,this);}
,_onappear:function(event){if(this.__nJ){return;}
;this.__nJ=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(g);}
;}
;this.getBehavior().onAppear(event,event.getType()!==a);this.__mv._updateScrollerWidths();this.__mv._updateScrollBarVisibility();this.__nJ=false;this.__nK=true;}
,_onTableWidthChanged:function(event){if(this.__nJ||!this.__nK){return;}
;this.__nJ=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(t);}
;}
;this.getBehavior().onTableWidthChanged(event);this.__nJ=false;}
,_onverticalscrollbarchanged:function(event){if(this.__nJ||!this.__nK){return;}
;this.__nJ=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(b);}
;}
;this.getBehavior().onVerticalScrollBarChanged(event);qx.event.Timer.once(function(){if(this.__mv&&!this.__mv.isDisposed()){this.__mv._updateScrollerWidths();this.__mv._updateScrollBarVisibility();}
;}
,this,0);this.__nJ=false;}
,_oncolumnwidthchanged:function(event){if(this.__nJ||!this.__nK){return;}
;this.__nJ=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(r);}
;}
;this.getBehavior().onColumnWidthChanged(event);this.__nJ=false;}
,_onvisibilitychanged:function(event){if(this.__nJ||!this.__nK){return;}
;this.__nJ=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(q);}
;}
;this.getBehavior().onVisibilityChanged(event);this.__nJ=false;}
...
,members:{__nL:null,renderLayout:function(f,top,g,h){this.__nL=g;}
,getComputedWidth:function(){return this.__nL;}
...
(function(){var h="onAppear is abstract",g="onVisibilityChanged is abstract",f="onVerticalScrollBarChanged is abstract",e="abstract",d="qx.ui.table.columnmodel.resizebehavior.Abstract",c="_setNumColumns is abstract",b="onTableWidthChanged is abstract",a="onColumnWidthChanged is abstract";qx.Class.define(d,{type:e,extend:qx.core.Object,members:{_setNumColumns:function(i){throw new Error(c);}
,onAppear:function(event,j){throw new Error(h);}
,onTableWidthChanged:function(event){throw new Error(b);}
,onVerticalScrollBarChanged:function(event){throw new Error(f);}
,onColumnWidthChanged:function(event){throw new Error(a);}
,onVisibilityChanged:function(event){throw new Error(g);}
,_getAvailableWidth:function(){var l=this.getTableColumnModel();var o=l.getTable();var k=o._getPaneScrollerArr();if(!k[0]||!k[0].getLayoutParent().getBounds()){return null;}
;var n=k[0].getLayoutParent().getBounds().width;var m=k[k.length-1];n-=m.getPaneInsetRight();return n;}
}});}
)();
(function(){var m="Function",k="Boolean",j="minWidth",h="width",g="qx.ui.table.columnmodel.Resize",f="qx.ui.table.columnmodel.resizebehavior.Default",e="__nN",d="__nO",c="Unknown property: ",b="maxWidth",a="Column number out of range";qx.Class.define(f,{extend:qx.ui.table.columnmodel.resizebehavior.Abstract,construct:function(){qx.ui.table.columnmodel.resizebehavior.Abstract.call(this);this.__nM=[];this.__nN=new qx.ui.layout.HBox();this.__nN.connectToWidget(this);this.__nO=new qx.util.DeferredCall(this._computeColumnsFlexWidth,this);}
,properties:{newResizeBehaviorColumnData:{check:m,init:function(n){return new qx.ui.core.ColumnData();}
},initializeWidthsOnEveryAppear:{check:k,init:false},tableColumnModel:{check:g}},members:{__nN:null,__nP:null,__nM:null,__nO:null,__nQ:false,setWidth:function(o,p,q){if(o>=this.__nM.length){throw new Error(a);}
;this.__nM[o].setColumnWidth(p,q);this.__nO.schedule();}
,setMinWidth:function(r,s){if(r>=this.__nM.length){throw new Error(a);}
;this.__nM[r].setMinWidth(s);this.__nO.schedule();}
,setMaxWidth:function(t,u){if(t>=this.__nM.length){throw new Error(a);}
;this.__nM[t].setMaxWidth(u);this.__nO.schedule();}
,set:function(v,w){for(var x in w){switch(x){case h:this.setWidth(v,w[x]);break;case j:this.setMinWidth(v,w[x]);break;case b:this.setMaxWidth(v,w[x]);break;default:throw new Error(c+x);};}
;}
,onAppear:function(event,y){if(y===true||!this.__nQ||this.getInitializeWidthsOnEveryAppear()){this._computeColumnsFlexWidth();this.__nQ=true;}
;}
...
,onVisibilityChanged:function(event){var z=event.getData();if(z.visible){this._computeColumnsFlexWidth();return;}
;this._extendLastColumn(event);}
,_setNumColumns:function(A){var B=this.__nM;if(A<=B.length){B.splice(A,B.length);return;}
;for(var i=B.length;i<A;i++){B[i]=this.getNewResizeBehaviorColumnData()();B[i].columnNumber=i;}
;}
,getLayoutChildren:function(){return this.__nP;}
,_computeColumnsFlexWidth:function(){this.__nO.cancel();var G=this._getAvailableWidth();if(G===null){return;}
;var C=this.getTableColumnModel();var E=C.getVisibleColumns();var F=E.length;var D=this.__nM;var i,l;if(F===0){return;}
;var I=[];for(i=0;i<F;i++){I.push(D[E[i]]);}
;this.__nP=I;this.__nR();this.__nN.renderLayout(G,100);for(i=0,l=I.length;i<l;i++){var H=I[i].getComputedWidth();C.setColumnWidth(E[i],H);}
;}
,__nR:function(){this.__nN.invalidateChildrenCache();var J=this.__nP;for(var i=0,l=J.length;i<l;i++){J[i].invalidateLayoutCache();}
;}
,_extendNextColumn:function(event){var O=this.getTableColumnModel();var R=event.getData();var M=O.getVisibleColumns();var L=this._getAvailableWidth();var K=M.length;if(R.newWidth>R.oldWidth){return;}
;var i;var N;var Q=0;for(i=0;i<K;i++){Q+=O.getColumnWidth(M[i]);}
;if(Q<L){for(i=0;i<M.length;i++){if(M[i]==R.col){N=M[i+1];break;}
;}
;if(N){var P=(L-(Q-O.getColumnWidth(N)));O.setColumnWidth(N,P);}
;}
;}
,_extendLastColumn:function(event){var V=this.getTableColumnModel();var ba=event.getData();if(ba.visible){return;}
;var U=V.getVisibleColumns();if(U.length==0){return;}
;var T=this._getAvailableWidth(V);var S=U.length;var i;var X;var Y=0;for(i=0;i<S;i++){Y+=V.getColumnWidth(U[i]);}
;if(Y<T){X=U[U.length-1];var W=(T-(Y-V.getColumnWidth(X)));V.setColumnWidth(X,W);}
;}
,_getResizeColumnData:function(){return this.__nM;}
},destruct:function(){this.__nM=this.__nP=null;this._disposeObjects(e,d);}
});}
)();
(function(){var q="_applyStatusBarVisible",p="columnVisibilityMenuCreateStart",o="blur",n="qx.ui.table.Table",m="columnVisibilityMenuCreateEnd",k="changeVisible",h="_applyResetSelectionOnHeaderClick",g="_applyMetaColumnCounts",f="focus",e="changeDataRowRenderer",bH="changeHeaderCellHeight",bG="Escape",bF="A",bE="changeSelectionModel",bD="Left",bC="__nT",bB="Down",bA="Integer",bz="_applyHeaderCellHeight",by="visibilityChanged",y="qx.ui.table.ITableModel",z="orderChanged",v="_applySelectionModel",w="menu-button",t="menu",u="_applyAdditionalStatusBarText",r="_applyFocusCellOnMouseMove",s="table",G="_applyColumnVisibilityButtonVisible",H="changeTableModel",ba="qx.event.type.Event",V="tableWidthChanged",bi="_applyHeaderCellsVisible",bd="Object",bu="_applyShowCellFocusIndicator",bo="resize",O="verticalScrollBarChanged",bx="changeScrollY",bw="_applyTableModel",bv="End",M="__oa",R="_applyKeepFirstVisibleRowComplete",T="widthChanged",X="one of one row",bb="Home",be="_applyRowHeight",bk="F2",bq="Up",A="%1 rows",B="qx.ui.table.selection.Model",Q="one row",bh="PageDown",bg="%1 of %2 rows",bf="__iG",bm="keypress",bl="changeRowHeight",bc="Number",bj="__nS",a="header",bp="__ob",C="_applyContextMenuFromDataCellsOnly",D="qx.ui.table.IRowRenderer",W="Right",b="Space",d="function",L="PageUp",E="dataChanged",F="changeLocale",J="changeSelection",Y="__oc",bs="appear",br="qx.dynlocale",S='"',bt="Enter",N="metaDataChanged",bn="qx.ui.table.pane.CellEvent",I="qx.event.type.Data",K="statusbar",c="column-button",U="Function",P="Boolean";qx.Class.define(n,{extend:qx.ui.core.Widget,construct:function(bI,bJ){qx.ui.core.Widget.call(this);if(!bJ){bJ={};}
...
}},members:{__nU:null,__nV:null,__nS:null,__nT:null,__nX:null,__nY:null,__mU:null,__oa:null,__ob:null,__oc:null,__od:null,__iG:null,_createChildControlImpl:function(bS,bT){var bU;switch(bS){case K:bU=new qx.ui.basic.Label();bU.set({allowGrowX:true});this._add(bU);break;case c:bU=this.getNewColumnMenu()();bU.set({focusable:false});var bV=bU.factory(t,{table:this});bV.addListener(bs,this._initColumnMenu,this);break;};return bU||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,bS);}
,_applySelectionModel:function(bW,bX){this.__nT.setSelectionModel(bW);if(bX!=null){bX.removeListener(J,this._onSelectionChanged,this);}
;bW.addListener(J,this._onSelectionChanged,this);}
,_applyRowHeight:function(bY,ca){var cb=this._getPaneScrollerArr();for(var i=0;i<cb.length;i++){cb[i].updateVerScrollBarMaximum();}
;}
,_applyHeaderCellsVisible:function(cc,cd){var ce=this._getPaneScrollerArr();for(var i=0;i<ce.length;i++){ce[i]._excludeChildControl(a);}
;}
...
;}
,getEmptyTableModel:function(){if(!this.__oc){this.__oc=new qx.ui.table.model.Simple();this.__oc.setColumns([]);this.__oc.setData([]);}
;return this.__oc;}
,_applyTableModel:function(ci,cj){this.getTableColumnModel().init(ci.getColumnCount(),this);if(cj!=null){cj.removeListener(N,this._onTableModelMetaDataChanged,this);cj.removeListener(E,this._onTableModelDataChanged,this);}
;ci.addListener(N,this._onTableModelMetaDataChanged,this);ci.addListener(E,this._onTableModelDataChanged,this);this._updateStatusBar();this._updateTableData(0,ci.getRowCount(),0,ci.getColumnCount());this._onTableModelMetaDataChanged();if(cj&&ci.init&&typeof (ci.init)==d){ci.init(this);}
;}
,getTableColumnModel:function(){if(!this.__ob){var cm=this.__ob=this.getNewTableColumnModel()(this);cm.addListener(by,this._onColVisibilityChanged,this);cm.addListener(T,this._onColWidthChanged,this);cm.addListener(z,this._onColOrderChanged,this);var cl=this.getTableModel();cm.init(cl.getColumnCount(),this);var ck=this._getPaneScrollerArr();for(var i=0;i<ck.length;i++){var cn=ck[i];var co=cn.getTablePaneModel();co.setTableColumnModel(cm);}
;}
;return this.__ob;}
...
;}
,_applyAdditionalStatusBarText:function(cr,cs){this.__nX=cr;this._updateStatusBar();}
,_applyColumnVisibilityButtonVisible:function(ct,cu){if(ct){this._showChildControl(c);}
else {this._excludeChildControl(c);}
;}
,_applyMetaColumnCounts:function(cv,cw){var cD=cv;var cx=this._getPaneScrollerArr();var cB={};if(cv>cw){var cF=qx.event.Registration.getManager(cx[0]);for(var cG in qx.ui.table.Table.__nW){cB[cG]={};cB[cG].capture=cF.getListeners(cx[0],cG,true);cB[cG].bubble=cF.getListeners(cx[0],cG,false);}
;}
...
;}
;}
;var cI=(i==cD.length-1)?1:0;this.__nS.add(cH,{flex:cI});cx=this._getPaneScrollerArr();}
;}
...
;}
,getSelectionManager:function(){return this.__nT;}
,_getPaneScrollerArr:function(){return this.__nS.getChildren();}
...
,_updateTableData:function(dl,dm,dn,dp,dq,dr){var ds=this._getPaneScrollerArr();if(dr){this.getSelectionModel().removeSelectionInterval(dq,dq+dr);if(this.__nV>=dq&&this.__nV<(dq+dr)){this.setFocusedCell();}
;}
;for(var i=0;i<ds.length;i++){ds[i].onTableModelDataChanged(dl,dm,dn,dp);}
;var dt=this.getTableModel().getRowCount();if(dt!=this.__nY){this.__nY=dt;this._updateScrollBarVisibility();this._updateStatusBar();}
;}
,_onScrollY:function(du){if(!this.__mU){this.__mU=true;var dv=this._getPaneScrollerArr();for(var i=0;i<dv.length;i++){dv[i].setScrollY(du.getData());}
;this.__mU=false;}
;}
,_onKeyPress:function(dw){if(!this.getEnabled()){return;}
;var dD=this.__nV;var dA=true;var dE=dw.getKeyIdentifier();if(this.isEditing()){if(dw.getModifiers()==0){switch(dE){case bt:this.stopEditing();var dD=this.__nV;this.moveFocusedCell(0,1);if(this.__nV!=dD){dA=this.startEditing();}
...
else {switch(dE){case b:this.__nT.handleSelectKeyDown(this.__nV,dw);break;case bk:case bt:this.startEditing();dA=true;break;case bb:this.setFocusedCell(this.__nU,0,true);break;case bv:var dB=this.getTableModel().getRowCount();this.setFocusedCell(this.__nU,dB-1,true);break;case bD:this.moveFocusedCell(-1,0);break;case W:this.moveFocusedCell(1,0);break;case bq:this.moveFocusedCell(0,-1);break;case bB:this.moveFocusedCell(0,1);break;case L:case bh:var dz=this.getPaneScroller(0);var dC=dz.getTablePane();var dy=this.getRowHeight();var dx=(dE==L)?-1:1;dB=dC.getVisibleRowCount()-1;dz.setScrollY(dz.getScrollY()+dx*dB*dy);this.moveFocusedCell(0,dx*dB);break;default:dA=false;};}
;}
;if(dD!=this.__nV&&this.getRowFocusChangeModifiesSelection()){this.__nT.handleMoveKeyDown(this.__nV,dw);}
...
;var dJ=dH.getData();if(this.__oa!=null&&dJ.col!=null&&dJ.visible!=null){this.__oa[dJ.col].setVisible(dJ.visible);}
;this._updateScrollerWidths();this._updateScrollBarVisibility();}
...
,setFocusedCell:function(dR,dS,dT){if(!this.isEditing()&&(dR!=this.__nU||dS!=this.__nV)){if(dR===null){dR=0;}
;this.__nU=dR;this.__nV=dS;var dU=this._getPaneScrollerArr();for(var i=0;i<dU.length;i++){dU[i].setFocusedCell(dR,dS);}
;if(dR!==null&&dT){this.scrollCellVisible(dR,dS);}
;}
;}
,resetSelection:function(){this.getSelectionModel().resetSelection();}
,resetCellFocus:function(){this.setFocusedCell(null,null,false);}
,getFocusedColumn:function(){return this.__nU;}
,getFocusedRow:function(){return this.__nV;}
...
;}
,moveFocusedCell:function(ea,eb){var ef=this.__nU;var eg=this.__nV;if(ef==null||eg==null){return;}
...
,scrollCellVisible:function(eh,ei){var ej=this.getContentElement().getDomElement();if(!ej){this.addListenerOnce(bs,function(){this.scrollCellVisible(eh,ei);}
,this);}
...
;}
,isEditing:function(){if(this.__nU!=null){var x=this.getTableColumnModel().getVisibleX(this.__nU);var em=this._getMetaColumnAtColumnX(x);return this.getPaneScroller(em).isEditing();}
;return false;}
,startEditing:function(){if(this.__nU!=null){var x=this.getTableColumnModel().getVisibleX(this.__nU);var eo=this._getMetaColumnAtColumnX(x);var en=this.getPaneScroller(eo).startEditing();return en;}
;return false;}
,stopEditing:function(){if(this.__nU!=null){var x=this.getTableColumnModel().getVisibleX(this.__nU);var ep=this._getMetaColumnAtColumnX(x);this.getPaneScroller(ep).stopEditing();}
;}
,cancelEditing:function(){if(this.__nU!=null){var x=this.getTableColumnModel().getVisibleX(this.__nU);var eq=this._getMetaColumnAtColumnX(x);this.getPaneScroller(eq).cancelEditing();}
;}
...
;}
;if(this.__nX){if(eD){eD+=this.__nX;}
else {eD=this.__nX;}
;}
...
;if(eP&&(eK&eO)){eN=true;}
;}
;for(var i=0;i<eJ.length;i++){var eP=(i==(eJ.length-1));eJ[i].setHorizontalScrollBarVisible(eL);if(eP){if(this.__od==null){this.__od=eJ[i].getVerticalScrollBarVisible();this.__iG=qx.event.Timer.once(function(){this.__od=null;this.__iG=null;}
,this,0);}
;}
;eJ[i].setVerticalScrollBarVisible(eP&&eN);if(eP&&eN!=this.__od){this.fireDataEvent(O,eN);}
;}
;}
,_initColumnMenu:function(){var eS=this.getTableModel();var eT=this.getTableColumnModel();var eU=this.getChildControl(c);eU.empty();var eR=eU.getMenu();var eV={table:this,menu:eR,columnButton:eU};this.fireDataEvent(p,eV);this.__oa={};for(var eW=0,l=eS.getColumnCount();eW<l;eW++){var eQ=eU.factory(w,{text:eS.getColumnName(eW),column:eW,bVisible:eT.isColumnVisible(eW)});qx.core.Assert.assertInterface(eQ,qx.ui.table.IColumnMenuItem);eQ.addListener(k,this._createColumnVisibilityCheckBoxHandler(eW),this);this.__oa[eW]=eQ;}
...
,addListener:function(fd,fe,self,ff){if(this.self(arguments).__nW[fd]){var fh=[fd];for(var i=0,fg=this._getPaneScrollerArr();i<fg.length;i++){fh.push(fg[i].addListener.apply(fg[i],arguments));}
;return fh.join(S);}
else {return qx.ui.core.Widget.prototype.addListener.call(this,fd,fe,self,ff);}
;}
,removeListener:function(fi,fj,self,fk){if(this.self(arguments).__nW[fi]){for(var i=0,fl=this._getPaneScrollerArr();i<fl.length;i++){fl[i].removeListener.apply(fl[i],arguments);}
;}
else {qx.ui.core.Widget.prototype.removeListener.call(this,fi,fj,self,fk);}
;}
,removeListenerById:function(fm){var fq=fm.split(S);var fp=fq.shift();if(this.self(arguments).__nW[fp]){var fo=true;for(var i=0,fn=this._getPaneScrollerArr();i<fn.length;i++){fo=fn[i].removeListenerById.call(fn[i],fq[i])&&fo;}
;return fo;}
...
},destruct:function(){if(qx.core.Environment.get(br)){qx.locale.Manager.getInstance().removeListener(F,this._onChangeLocale,this);}
;var fs=this.getSelectionModel();if(fs){fs.dispose();}
;var fr=this.getDataRowRenderer();if(fr){fr.dispose();}
;this._cleanUpMetaColumns(0);this.getTableColumnModel().dispose();this._disposeObjects(bC,bj,Y,Y,bp,bf);this._disposeMap(M);}
});}
)();
...
,members:{__oC:false,_applyVisible:function(h,i){if(!this.bInListener){this.setValue(h);}
;}
}});}
)();
(function(){var k="touchmove",j="qx.ui.core.scroll.MTouchScroll",i="touchend",h="touchstart",g="Top",f="Left",d="y",c="getDocument",b="x",a="scrollbar-";qx.Mixin.define(j,{construct:function(){this.addListener(k,this._onTouchMove,this);this.addListener(h,this._onTouchStart,this);this.addListener(i,this._onTouchEnd,this,true);this.__cS={};this.__mb={};}
,members:{__oS:false,__oT:false,__mb:null,__cS:null,inTouchMomentum:function(){return this.__oS;}
,_onTouchStart:function(e){if(this.__mb){clearTimeout(this.__mb.x);clearTimeout(this.__mb.y);}
;this.__cS={"x":0,"y":0};if(this.__oS){this.__oT=true;this.__oS=false;}
;}
,_onTouchEnd:function(e){if(this.__oT){this.__oT=false;e.stop();}
;}
,_onTouchMove:function(e){this._onTouchMoveDirectional(b,e);this._onTouchMoveDirectional(d,e);this.__oS=true;e.stop();}
...
;this.__cS[l]=e[c+m]();o.scrollBy(n);if(this.__mb[l]){clearTimeout(this.__mb[l]);this.__mb[l]=null;}
;this.__mb[l]=setTimeout(qx.lang.Function.bind(function(q){this.__mc(q,l);}
,this,n),100);}
;}
,__mc:function(r,s){this.__mb[s]=null;var u=this._isChildControlVisible(a+s);if(r==0||!u){this.__oS=false;return;}
;if(r>0){r=Math.max(0,r-3);}
else {r=Math.min(0,r+3);}
;this.__mb[s]=setTimeout(qx.lang.Function.bind(function(v,w){this.__mc(v,w);}
,this,r,s),10);var t=this.getChildControl(a+s,true);t.scrollBy(r);}
},destruct:function(){clearTimeout(this.__mb.x);clearTimeout(this.__mb.y);this.__mb=this.__cS=this.__oS=null;}
});}
)();
...
(function(){var o="__oU",n="__oV",m="mouseout",l="__pc",k="changeHorizontalScrollBarVisible",j="bottom",i="_applyScrollTimeout",h="changeScrollX",g="_applyTablePaneModel",d="Integer",bq="dblclick",bp="__pd",bo="dataEdited",bn="__oX",bm="mousewheel",bl="interval",bk="__iG",bj="qx.ui.table.pane.Scroller",bi="_applyShowCellFocusIndicator",bh="y",v="resize",w="__pa",t="vertical",u="changeScrollY",r="__oW",s="appear",p="__pb",q="table-scroller",C="beforeSort",D="__oY",M="cellDblclick",J="horizontal",U="losecapture",P="contextmenu",bd="col-resize",ba="disappear",F="_applyVerticalScrollBarVisible",bg="_applyHorizontalScrollBarVisible",bf="os.scrollBarOverlayed",be="cellContextmenu",E="close",H="changeTablePaneModel",I="x",L="qx.ui.table.pane.Model",N="changeVerticalScrollBarVisible",Q="scroll",W="focus-indicator",bc="excluded",y="scrollbar-y",z="table-scroller-focus-indicator",G="visible",T="mousemove",S="header",R="editing",Y="click",X="modelChanged",O="scrollbar-x",V="cellClick",a="pane",bb="mousedown",A="qx.event.type.Data",B="mouseup",K="qx.ui.table.pane.CellEvent",b="resize-line",c="Boolean";qx.Class.define(bj,{extend:qx.ui.core.Widget,include:qx.core.Environment.filter({"true":qx.ui.core.scroll.MScrollBarFactory,"event.touch":qx.ui.core.scroll.MTouchScroll}),construct:function(br){qx.ui.core.Widget.call(this);this.__mv=br;var bs=new qx.ui.layout.Grid();bs.setColumnFlex(0,1);bs.setRowFlex(1,1);this._setLayout(bs);this.__oU=this._showChildControl(S);this.__oV=this._showChildControl(a);this.__oW=new qx.ui.container.Composite(new qx.ui.layout.HBox()).set({minWidth:0});this._add(this.__oW,{row:0,column:0,colSpan:2});this.__oX=new qx.ui.table.pane.Clipper();this.__oX.add(this.__oU);this.__oX.addListener(U,this._onChangeCaptureHeader,this);this.__oX.addListener(T,this._onMousemoveHeader,this);this.__oX.addListener(bb,this._onMousedownHeader,this);this.__oX.addListener(B,this._onMouseupHeader,this);this.__oX.addListener(Y,this._onClickHeader,this);this.__oW.add(this.__oX,{flex:1});this.__oY=new qx.ui.table.pane.Clipper();this.__oY.add(this.__oV);this.__oY.addListener(bm,this._onMousewheel,this);this.__oY.addListener(T,this._onMousemovePane,this);this.__oY.addListener(bb,this._onMousedownPane,this);this.__oY.addListener(B,this._onMouseupPane,this);this.__oY.addListener(Y,this._onClickPane,this);this.__oY.addListener(P,this._onContextMenu,this);this.__oY.addListener(bq,this._onDblclickPane,this);this.__oY.addListener(v,this._onResizePane,this);if(qx.core.Environment.get(bf)){this.__pa=new qx.ui.container.Composite();this.__pa.setLayout(new qx.ui.layout.Canvas());this.__pa.add(this.__oY,{edge:0});this._add(this.__pa,{row:1,column:0});}
else {this._add(this.__oY,{row:1,column:0});}
;this.__pb=this._showChildControl(O);this.__pc=this._showChildControl(y);this.__pd=this.getChildControl(W);this.initShowCellFocusIndicator();this.getChildControl(b).hide();this.addListener(m,this._onMouseout,this);this.addListener(s,this._onAppear,this);this.addListener(ba,this._onDisappear,this);this.__iG=new qx.event.Timer();this.__iG.addListener(bl,this._oninterval,this);this.initScrollTimeout();}
,statics:{MIN_COLUMN_WIDTH:10,RESIZE_REGION_RADIUS:5,CLICK_TOLERANCE:5,HORIZONTAL_SCROLLBAR:1,VERTICAL_SCROLLBAR:2},events:{"changeScrollY":A,"changeScrollX":A,"cellClick":K,"cellDblclick":K,"cellContextmenu":K,"beforeSort":A},properties:{horizontalScrollBarVisible:{check:c,init:false,apply:bg,event:k},verticalScrollBarVisible:{check:c,init:false,apply:F,event:N},tablePaneModel:{check:L,apply:g,event:H},liveResize:{check:c,init:false},focusCellOnMouseMove:{check:c,init:false},selectBeforeFocus:{check:c,init:false},showCellFocusIndicator:{check:c,init:true,apply:bi},contextMenuFromDataCellsOnly:{check:c,init:true},resetSelectionOnHeaderClick:{check:c,init:true},scrollTimeout:{check:d,init:100,apply:i},appearance:{refine:true,init:q}},members:{__nY:null,__mv:null,__pe:null,__pf:null,__pg:null,__ph:null,__pi:null,__pj:null,__pk:null,__pl:null,__pm:null,__pn:null,__po:null,__pp:null,__pq:false,__pr:null,__ps:null,__pt:null,__nU:null,__nV:null,__pu:null,__pv:null,__pw:null,__pb:null,__pc:null,__oU:null,__oX:null,__oV:null,__oY:null,__pa:null,__pd:null,__oW:null,__iG:null,getPaneInsetRight:function(){var bv=this.getTopRightWidget();var bw=bv&&bv.isVisible()&&bv.getBounds()?bv.getBounds().width+bv.getMarginLeft()+bv.getMarginRight():0;var bu=this.__pc;var bt=this.getVerticalScrollBarVisible()?this.getVerticalScrollBarWidth()+bu.getMarginLeft()+bu.getMarginRight():0;return Math.max(bw,bt);}
,setPaneWidth:function(bx){if(this.isVerticalScrollBarVisible()){bx+=this.getPaneInsetRight();}
;this.setWidth(bx);}
,_createChildControlImpl:function(by,bz){var bA;switch(by){case S:bA=(this.getTable().getNewTablePaneHeader())(this);break;case a:bA=(this.getTable().getNewTablePane())(this);break;case W:bA=new qx.ui.table.pane.FocusIndicator(this);bA.setUserBounds(0,0,0,0);bA.setZIndex(1000);bA.addListener(B,this._onMouseupFocusIndicator,this);this.__oY.add(bA);bA.show();bA.setDecorator(null);break;case b:bA=new qx.ui.core.Widget();bA.setUserBounds(0,0,0,0);bA.setZIndex(1000);this.__oY.add(bA);break;case O:bA=this._createScrollBar(J).set({alignY:j});bA.addListener(Q,this._onScrollX,this);if(this.__pa!=null){bA.setMinHeight(qx.bom.element.Overflow.DEFAULT_SCROLLBAR_WIDTH);this.__pa.add(bA,{bottom:0,right:0,left:0});}
else {this._add(bA,{row:2,column:0});}
;break;case y:bA=this._createScrollBar(t);bA.addListener(Q,this._onScrollY,this);if(this.__pa!=null){this.__pa.add(bA,{right:0,bottom:0,top:0});}
else {this._add(bA,{row:1,column:1});}
;break;};return bA||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,by);}
,_applyHorizontalScrollBarVisible:function(bB,bC){this.__pb.setVisibility(bB?G:bc);}
,_applyVerticalScrollBarVisible:function(bD,bE){this.__pc.setVisibility(bD?G:bc);}
...
,_applyShowCellFocusIndicator:function(bH,bI){if(bH){this.__pd.setDecorator(z);this._updateFocusIndicator();}
else {if(this.__pd){this.__pd.setDecorator(null);}
;}
;}
,getScrollY:function(){return this.__pc.getPosition();}
,setScrollY:function(scrollY,bJ){this.__pc.scrollTo(scrollY);if(bJ){this._updateContent();}
;}
,getScrollX:function(){return this.__pb.getPosition();}
,setScrollX:function(scrollX){this.__pb.scrollTo(scrollX);}
,getTable:function(){return this.__mv;}
,onColVisibilityChanged:function(){this.updateHorScrollBarMaximum();this._updateFocusIndicator();}
,setColumnWidth:function(bK,bL){this.__oU.setColumnWidth(bK,bL);this.__oV.setColumnWidth(bK,bL);var bM=this.getTablePaneModel();var x=bM.getX(bK);if(x!=-1){this.updateHorScrollBarMaximum();this._updateFocusIndicator();}
;}
,onColOrderChanged:function(){this.__oU.onColOrderChanged();this.__oV.onColOrderChanged();this.updateHorScrollBarMaximum();}
,onTableModelDataChanged:function(bN,bO,bP,bQ){this.__oV.onTableModelDataChanged(bN,bO,bP,bQ);var bR=this.getTable().getTableModel().getRowCount();if(bR!=this.__nY){this.updateVerScrollBarMaximum();if(this.getFocusedRow()>=bR){if(bR==0){this.setFocusedCell(null,null);}
else {this.setFocusedCell(this.getFocusedColumn(),bR-1);}
;}
;this.__nY=bR;}
;}
,onSelectionChanged:function(){this.__oV.onSelectionChanged();}
,onFocusChanged:function(){this.__oV.onFocusChanged();}
,onTableModelMetaDataChanged:function(){this.__oU.onTableModelMetaDataChanged();this.__oV.onTableModelMetaDataChanged();}
,_onPaneModelChanged:function(){this.__oU.onPaneModelChanged();this.__oV.onPaneModelChanged();}
,_onResizePane:function(){this.updateHorScrollBarMaximum();this.updateVerScrollBarMaximum();this._updateContent();this.__oU._updateContent();this.__mv._updateScrollBarVisibility();}
,updateHorScrollBarMaximum:function(){var bV=this.__oY.getInnerSize();if(!bV){return;}
;var bT=this.getTablePaneModel().getTotalWidth();var bU=this.__pb;if(bV.width<bT){var bS=Math.max(0,bT-bV.width);bU.setMaximum(bS);bU.setKnobFactor(bV.width/bT);var bW=bU.getPosition();bU.setPosition(Math.min(bW,bS));}
else {bU.setMaximum(0);bU.setKnobFactor(1);bU.setPosition(0);}
;}
,updateVerScrollBarMaximum:function(){var cf=this.__oY.getInnerSize();if(!cf){return;}
;var cd=this.getTable().getTableModel();var bY=cd.getRowCount();if(this.getTable().getKeepFirstVisibleRowComplete()){bY+=1;}
;var bX=this.getTable().getRowHeight();var cb=bY*bX;var ce=this.__pc;if(cf.height<cb){var ca=Math.max(0,cb-cf.height);ce.setMaximum(ca);ce.setKnobFactor(cf.height/cb);var cc=ce.getPosition();ce.setPosition(Math.min(cc,ca));}
...
,_onScrollX:function(e){var cg=e.getData();this.fireDataEvent(h,cg,e.getOldData());this.__oX.scrollToX(cg);this.__oY.scrollToX(cg);}
...
;this.__pc.scrollBySteps(ck);ck=e.getWheelDelta(I);if(ck>0&&ck<1){ck=1;}
else if(ck<0&&ck>-1){ck=-1;}
;this.__pb.scrollBySteps(ck);if(this.__ps&&this.getFocusCellOnMouseMove()){this._focusCellAtPagePos(this.__ps,this.__pt);}
;var cj=this.__pc.getPosition();var ci=this.__pc.getMaximum();if(ck<0&&cj<=0||ck>0&&cj>=ci){return;}
;e.stop();}
,__px:function(cl){var cq=this.getTable();var cr=this.__oU.getHeaderWidgetAtColumn(this.__pm);var cm=cr.getSizeHint().minWidth;var co=Math.max(cm,this.__po+cl-this.__pn);if(this.getLiveResize()){var cn=cq.getTableColumnModel();cn.setColumnWidth(this.__pm,co,true);}
else {this.__oU.setColumnWidth(this.__pm,co,true);var cp=this.getTablePaneModel();this._showResizeLine(cp.getColumnLeft(this.__pm)+co);}
;this.__pn+=co-this.__po;this.__po=co;}
,__py:function(cs){var ct=qx.ui.table.pane.Scroller.CLICK_TOLERANCE;if(this.__oU.isShowingColumnMoveFeedback()||cs>this.__pl+ct||cs<this.__pl-ct){this.__pi+=cs-this.__pl;this.__oU.showColumnMoveFeedback(this.__ph,this.__pi);var cu=this.__mv.getTablePaneScrollerAtPageX(cs);if(this.__pk&&this.__pk!=cu){this.__pk.hideColumnMoveFeedback();}
;if(cu!=null){this.__pj=cu.showColumnMoveFeedback(cs);}
else {this.__pj=null;}
;this.__pk=cu;this.__pl=cs;}
;}
,_onMousemoveHeader:function(e){var cB=this.getTable();if(!cB.getEnabled()){return;}
;var cC=false;var cv=null;var cz=e.getDocumentLeft();var cA=e.getDocumentTop();this.__ps=cz;this.__pt=cA;if(this.__pm!=null){this.__px(cz);cC=true;e.stopPropagation();}
else if(this.__ph!=null){this.__py(cz);e.stopPropagation();}
...
;}
;}
;var cx=cC?bd:null;this.getApplicationRoot().setGlobalCursor(cx);this.setCursor(cx);this.__oU.setMouseOverColumn(cv);}
,_onMousemovePane:function(e){var cE=this.getTable();if(!cE.getEnabled()){return;}
;var cG=e.getDocumentLeft();var cH=e.getDocumentTop();this.__ps=cG;this.__pt=cH;var cF=this._getRowForPagePos(cG,cH);if(cF!=null&&this._getColumnForPageX(cG)!=null){if(this.getFocusCellOnMouseMove()){this._focusCellAtPagePos(cG,cH);}
;}
;this.__oU.setMouseOverColumn(null);}
...
;}
;}
,_startResizeHeader:function(cL,cM){var cN=this.getTable().getTableColumnModel();this.__pm=cL;this.__pn=cM;this.__po=cN.getColumnWidth(this.__pm);this.__oX.capture();}
,_startMoveHeader:function(cO,cP){this.__ph=cO;this.__pl=cP;this.__pi=this.getTablePaneModel().getColumnLeft(cO);this.__oX.capture();}
,_onMousedownPane:function(e){var cT=this.getTable();if(!cT.getEnabled()){return;}
;if(cT.isEditing()){cT.stopEditing();}
;var cQ=e.getDocumentLeft();var cS=e.getDocumentTop();var cV=this._getRowForPagePos(cQ,cS);var cU=this._getColumnForPageX(cQ);if(cV!==null){this.__pp={row:cV,col:cU};this.__pq=false;var cR=this.getSelectBeforeFocus();if(cR){cT.getSelectionManager().handleMouseDown(cV,e);}
;if(!this.getFocusCellOnMouseMove()){this._focusCellAtPagePos(cQ,cS);}
;if(!cR){cT.getSelectionManager().handleMouseDown(cV,e);}
;}
;}
,_onMouseupFocusIndicator:function(e){if(this.__pp&&!this.__pq&&!this.isEditing()&&this.__pd.getRow()==this.__pp.row&&this.__pd.getColumn()==this.__pp.col){this.fireEvent(V,qx.ui.table.pane.CellEvent,[this,e,this.__pp.row,this.__pp.col],true);this.__pq=true;}
else if(!this.isEditing()){this._onMousedownPane(e);}
;}
,_onChangeCaptureHeader:function(e){if(this.__pm!=null){this._stopResizeHeader();}
;if(this.__ph!=null){this._stopMoveHeader();}
;}
,_stopResizeHeader:function(){var cW=this.getTable().getTableColumnModel();if(!this.getLiveResize()){this._hideResizeLine();cW.setColumnWidth(this.__pm,this.__po,true);}
;this.__pm=null;this.__oX.releaseCapture();this.getApplicationRoot().setGlobalCursor(null);this.setCursor(null);if(this.isEditing()){var cX=this.__pu.getBounds().height;this.__pu.setUserBounds(0,0,this.__po,cX);}
;}
,_stopMoveHeader:function(){var dd=this.getTable().getTableColumnModel();var de=this.getTablePaneModel();this.__oU.hideColumnMoveFeedback();if(this.__pk){this.__pk.hideColumnMoveFeedback();}
;if(this.__pj!=null){var dg=de.getFirstColumnX()+de.getX(this.__ph);var dc=this.__pj;if(dc!=dg&&dc!=dg+1){var df=dd.getVisibleColumnAtX(dg);var db=dd.getVisibleColumnAtX(dc);var da=dd.getOverallX(df);var cY=(db!=null)?dd.getOverallX(db):dd.getOverallColumnCount();if(cY>da){cY--;}
;dd.moveColumn(da,cY);this._updateFocusIndicator();}
;}
;this.__ph=null;this.__pj=null;this.__oX.releaseCapture();}
...
;if(this.__pm!=null){this._stopResizeHeader();this.__pr=true;e.stop();}
else if(this.__ph!=null){this._stopMoveHeader();e.stop();}
;}
,_onClickHeader:function(e){if(this.__pr){this.__pr=false;return;}
...
;var dw=e.getDocumentLeft();var dx=e.getDocumentTop();var du=this._getRowForPagePos(dw,dx);var dv=this._getColumnForPageX(dw);if(du!=null&&dv!=null){dt.getSelectionManager().handleClick(du,e);if(this.__pd.isHidden()||(this.__pp&&!this.__pq&&!this.isEditing()&&du==this.__pp.row&&dv==this.__pp.col)){this.fireEvent(V,qx.ui.table.pane.CellEvent,[this,e,du,dv],true);this.__pq=true;}
;}
;}
...
;if(!this.getShowCellFocusIndicator()||dz===null||(this.__pp&&dz==this.__pp.row&&dA==this.__pp.col)){this.fireEvent(be,qx.ui.table.pane.CellEvent,[this,e,dz,dA],true);var dy=this.getTable().getContextMenu();if(dy){if(dy.getChildren().length>0){dy.openAtMouse(e);}
else {dy.exclude();}
;e.preventDefault();}
;}
;}
...
;if(this.__pm==null){this.setCursor(null);this.getApplicationRoot().setGlobalCursor(null);}
;this.__oU.setMouseOverColumn(null);if(this.getFocusCellOnMouseMove()){this.__mv.setFocusedCell();}
;}
,_showResizeLine:function(x){var dI=this._showChildControl(b);var dH=dI.getWidth();var dJ=this.__oY.getBounds();dI.setUserBounds(x-Math.round(dH/2),0,dH,dJ.height);}
,_hideResizeLine:function(){this._excludeChildControl(b);}
,showColumnMoveFeedback:function(dK){var dT=this.getTablePaneModel();var dS=this.getTable().getTableColumnModel();var dN=this.__oV.getContainerLocation().left;var dR=dT.getColumnCount();var dO=0;var dM=0;var dW=dN;for(var dL=0;dL<dR;dL++){var dP=dT.getColumnAtX(dL);var dU=dS.getColumnWidth(dP);if(dK<dW+dU/2){break;}
;dW+=dU;dO=dL+1;dM=dW-dN;}
;var dQ=this.__oY.getContainerLocation().left;var dV=this.__oY.getBounds().width;var scrollX=dQ-dN;dM=qx.lang.Number.limit(dM,scrollX+2,scrollX+dV-1);this._showResizeLine(dM);return dT.getFirstColumnX()+dO;}
,hideColumnMoveFeedback:function(){this._hideResizeLine();}
,_focusCellAtPagePos:function(dX,dY){var eb=this._getRowForPagePos(dX,dY);if(eb!=-1&&eb!=null){var ea=this._getColumnForPageX(dX);this.__mv.setFocusedCell(ea,eb);}
;}
,setFocusedCell:function(ec,ed){if(!this.isEditing()){this.__oV.setFocusedCell(ec,ed,this.__pf);this.__nU=ec;this.__nV=ed;this._updateFocusIndicator();}
;}
,getFocusedColumn:function(){return this.__nU;}
,getFocusedRow:function(){return this.__nV;}
,scrollCellVisible:function(ee,ef){var ep=this.getTablePaneModel();var eg=ep.getX(ee);if(eg!=-1){var em=this.__oY.getInnerSize();if(!em){return;}
...
;}
,isEditing:function(){return this.__pu!=null;}
,startEditing:function(){var ew=this.getTable();var eu=ew.getTableModel();var ey=this.__nU;if(!this.isEditing()&&(ey!=null)&&eu.isColumnEditable(ey)){var ez=this.__nV;var es=this.getTablePaneModel().getX(ey);var et=eu.getValue(ey,ez);this.scrollCellVisible(es,ez);this.__pv=ew.getTableColumnModel().getCellEditorFactory(ey);var ev={col:ey,row:ez,xPos:es,value:et,table:ew};this.__pu=this.__pv.createCellEditor(ev);if(this.__pu===null){return false;}
else if(this.__pu instanceof qx.ui.window.Window){this.__pu.setModal(true);this.__pu.setShowClose(false);this.__pu.addListener(E,this._onCellEditorModalWindowClose,this);var f=ew.getModalCellEditorPreOpenFunction();if(f!=null){f(this.__pu,ev);}
;this.__pu.open();}
else {var ex=this.__pd.getInnerSize();this.__pu.setUserBounds(0,0,ex.width,ex.height);this.__pd.addListener(bb,function(e){this.__pp={row:this.__nV,col:this.__nU};e.stopPropagation();}
,this);this.__pd.add(this.__pu);this.__pd.addState(R);this.__pd.setKeepActive(false);this.__pd.setDecorator(z);this.__pu.focus();this.__pu.activate();}
;return true;}
;return false;}
,stopEditing:function(){if(!this.getShowCellFocusIndicator()){this.__pd.setDecorator(null);}
;this.flushEditor();this.cancelEditing();}
,flushEditor:function(){if(this.isEditing()){var eB=this.__pv.getCellEditorValue(this.__pu);var eA=this.getTable().getTableModel().getValue(this.__nU,this.__nV);this.getTable().getTableModel().setValue(this.__nU,this.__nV,eB);this.__mv.focus();this.__mv.fireDataEvent(bo,{row:this.__nV,col:this.__nU,oldValue:eA,value:eB});}
;}
,cancelEditing:function(){if(this.isEditing()&&!this.__pu.pendingDispose){if(this._cellEditorIsModalWindow){this.__pu.destroy();this.__pu=null;this.__pv=null;this.__pu.pendingDispose=true;}
else {this.__pd.removeState(R);this.__pd.setKeepActive(true);this.__pu.destroy();this.__pu=null;this.__pv=null;}
;}
;}
,_onCellEditorModalWindowClose:function(e){this.stopEditing();}
,_getColumnForPageX:function(eC){var eF=this.getTable().getTableColumnModel();var eG=this.getTablePaneModel();var eE=eG.getColumnCount();var eI=this.__oV.getContentLocation().left;for(var x=0;x<eE;x++){var eD=eG.getColumnAtX(x);var eH=eF.getColumnWidth(eD);eI+=eH;if(eC<eI){return eD;}
;}
;return null;}
,_getResizeColumnForPageX:function(eJ){var eN=this.getTable().getTableColumnModel();var eO=this.getTablePaneModel();var eM=eO.getColumnCount();var eQ=this.__oU.getContainerLocation().left;var eK=qx.ui.table.pane.Scroller.RESIZE_REGION_RADIUS;for(var x=0;x<eM;x++){var eL=eO.getColumnAtX(x);var eP=eN.getColumnWidth(eL);eQ+=eP;if(eJ>=(eQ-eK)&&eJ<=(eQ+eK)){return eL;}
;}
;return -1;}
,_getRowForPagePos:function(eR,eS){var eT=this.__oV.getContentLocation();if(eR<eT.left||eR>eT.right){return null;}
;if(eS>=eT.top&&eS<=eT.bottom){var eU=this.getTable().getRowHeight();var scrollY=this.__pc.getPosition();if(this.getTable().getKeepFirstVisibleRowComplete()){scrollY=Math.floor(scrollY/eU)*eU;}
...
;var eW=this.__oU.getContainerLocation();if(eS>=eW.top&&eS<=eW.bottom&&eR<=eW.right){return -1;}
;return null;}
,setTopRightWidget:function(fb){var fc=this.__pw;if(fc!=null){this.__oW.remove(fc);}
;if(fb!=null){this.__oW.add(fb);}
;this.__pw=fb;}
,getTopRightWidget:function(){return this.__pw;}
,getHeader:function(){return this.__oU;}
,getTablePane:function(){return this.__oV;}
,getVerticalScrollBarWidth:function(){var fd=this.__pc;return fd.isVisible()?(fd.getSizeHint().width||0):0;}
,getNeededScrollBars:function(fe,ff){var fo=this.__pc;var fs=fo.getSizeHint().width+fo.getMarginLeft()+fo.getMarginRight();var fu=this.__pb;var ft=fu.getSizeHint().height+fu.getMarginTop()+fu.getMarginBottom();var fm=this.__oY.getInnerSize();var fg=fm?fm.width:0;if(this.getVerticalScrollBarVisible()){fg+=fs;}
...
,getPaneClipper:function(){return this.__oY;}
...
,_oninterval:qx.event.GlobalError.observeMethod(function(){if(this.__pf&&!this.__oV._layoutPending){this.__pf=false;this._updateContent();}
;}
),_updateContent:function(){var fC=this.__oY.getInnerSize();if(!fC){return;}
;var fF=fC.height;var scrollX=this.__pb.getPosition();var scrollY=this.__pc.getPosition();var fz=this.getTable().getRowHeight();var fA=Math.floor(scrollY/fz);var fE=this.__oV.getFirstVisibleRow();this.__oV.setFirstVisibleRow(fA);var fB=Math.ceil(fF/fz);var fy=0;var fD=this.getTable().getKeepFirstVisibleRowComplete();if(!fD){fB++;fy=scrollY%fz;}
;this.__oV.setVisibleRowCount(fB);if(fA!=fE){this._updateFocusIndicator();}
;this.__oY.scrollToX(scrollX);if(!fD){this.__oY.scrollToY(fy);}
;}
,_updateFocusIndicator:function(){var fG=this.getTable();if(!fG.getEnabled()){return;}
;this.__pd.moveToCell(this.__nU,this.__nV);}
},destruct:function(){this._stopInterval();var fH=this.getTablePaneModel();if(fH){fH.dispose();}
;this.__pp=this.__pw=this.__mv=null;this._disposeObjects(p,l,bn,D,bp,o,n,r,bk,w);}
});}
)();
...
(function(){var g="Escape",f="keypress",d="Enter",c="excluded",b="qx.ui.table.pane.FocusIndicator",a="Integer";qx.Class.define(b,{extend:qx.ui.container.Composite,construct:function(h){qx.ui.container.Composite.call(this);this.__pz=h;this.setKeepActive(true);this.addListener(f,this._onKeyPress,this);}
,properties:{visibility:{refine:true,init:c},row:{check:a,nullable:true},column:{check:a,nullable:true}},members:{__pz:null,_onKeyPress:function(e){var i=e.getKeyIdentifier();if(i!==g&&i!==d){e.stopPropagation();}
;}
,moveToCell:function(j,k){if(!this.__pz.getShowCellFocusIndicator()&&!this.__pz.getTable().getTableModel().isColumnEditable(j)){this.exclude();return;}
else {this.show();}
;if(j==null){this.hide();this.setRow(null);this.setColumn(null);}
else {var l=this.__pz.getTablePaneModel().getX(j);if(l==-1){this.hide();this.setRow(null);this.setColumn(null);}
else {var q=this.__pz.getTable();var o=q.getTableColumnModel();var p=this.__pz.getTablePaneModel();var n=this.__pz.getTablePane().getFirstVisibleRow();var m=q.getRowHeight();this.setUserBounds(p.getColumnLeft(j)-2,(k-n)*m-2,o.getColumnWidth(j)+3,m+3);this.show();this.setRow(k);this.setColumn(j);}
;}
;}
...
,events:{"modelChanged":g},statics:{EVENT_TYPE_MODEL_CHANGED:"modelChanged"},properties:{firstColumnX:{check:e,init:0,apply:f},maxColumnCount:{check:h,init:-1,apply:c}},members:{__qk:null,__ql:null,_applyFirstColumnX:function(j,k){this.__qk=null;this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);}
,_applyMaxColumnCount:function(l,m){this.__qk=null;this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);}
,setTableColumnModel:function(n){if(this.__ql){this.__ql.removeListener(a,this._onColVisibilityChanged,this);this.__ql.removeListener(b,this._onColVisibilityChanged,this);}
;this.__ql=n;this.__ql.addListener(a,this._onColVisibilityChanged,this);this.__ql.addListener(b,this._onHeaderCellRendererChanged,this);this.__qk=null;}
,_onColVisibilityChanged:function(o){this.__qk=null;this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);}
,_onHeaderCellRendererChanged:function(p){this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);}
,getColumnCount:function(){if(this.__qk==null){var q=this.getFirstColumnX();var s=this.getMaxColumnCount();var r=this.__ql.getVisibleColumnCount();if(s==-1||(q+s)>r){this.__qk=r-q;}
else {this.__qk=s;}
;}
;return this.__qk;}
,getColumnAtX:function(t){var u=this.getFirstColumnX();return this.__ql.getVisibleColumnAtX(u+t);}
,getX:function(v){var w=this.getFirstColumnX();var y=this.getMaxColumnCount();var x=this.__ql.getVisibleX(v)-w;if(x>=0&&(y==-1||x<y)){return x;}
else {return -1;}
;}
,getColumnLeft:function(z){var C=0;var B=this.getColumnCount();for(var x=0;x<B;x++){var A=this.getColumnAtX(x);if(A==z){return C;}
;C+=this.__ql.getColumnWidth(A);}
;return -1;}
,getTotalWidth:function(){var D=0;var E=this.getColumnCount();for(var x=0;x<E;x++){var F=this.getColumnAtX(x);D+=this.__ql.getColumnWidth(F);}
;return D;}
},destruct:function(){if(this.__ql){this.__ql.removeListener(a,this._onColVisibilityChanged,this);this.__ql.removeListener(b,this._onColVisibilityChanged,this);}
;this.__ql=null;}
});}
)();
(function(){var h="qx.ui.table.model.Simple",g="Boolean",f="this.__rowArr out of bounds: ",e=")",d=" (0..",c="sorted",b="metaDataChanged",a="dataChanged";qx.Class.define(h,{extend:qx.ui.table.model.Abstract,construct:function(){qx.ui.table.model.Abstract.call(this);this.__mV=[];this.__mW=-1;this.__mX=[];this.__mY=null;}
...
},members:{__mV:null,__mY:null,__na:null,__mX:null,__mW:null,__nb:null,getRowData:function(E){var F=this.__mV[E];if(F==null||F.originalData==null){return F;}
else {return F.originalData;}
;}
,getRowDataAsMap:function(G){var I=this.__mV[G];if(I!=null){var H={};for(var J=0;J<this.getColumnCount();J++){H[this.getColumnId(J)]=I[J];}
...
;return L;}
,setEditable:function(N){this.__mY=[];for(var O=0;O<this.getColumnCount();O++){this.__mY[O]=N;}
;this.fireEvent(b);}
,setColumnEditable:function(P,Q){if(Q!=this.isColumnEditable(P)){if(this.__mY==null){this.__mY=[];}
;this.__mY[P]=Q;this.fireEvent(b);}
;}
,isColumnEditable:function(R){return this.__mY?(this.__mY[R]==true):false;}
,setColumnSortable:function(S,T){if(T!=this.isColumnSortable(S)){if(this.__na==null){this.__na=[];}
;this.__na[S]=T;this.fireEvent(b);}
;}
,isColumnSortable:function(U){return (this.__na?(this.__na[U]!==false):true);}
,sortByColumn:function(V,W){var ba;var Y=this.__mX[V];if(Y){ba=(W?Y.ascending:Y.descending);}
...
;}
;ba.columnIndex=V;this.__mV.sort(ba);this.__mW=V;this.__nb=W;var X={columnIndex:V,ascending:W};this.fireDataEvent(c,X);this.fireEvent(b);}
...
;this.__mX[bb]=bd;}
,getSortMethods:function(bg){return this.__mX[bg];}
,clearSorting:function(){if(this.__mW!=-1){this.__mW=-1;this.__nb=true;this.fireEvent(b);}
;}
,getSortColumnIndex:function(){return this.__mW;}
,_setSortColumnIndex:function(bh){this.__mW=bh;}
,isSortAscending:function(){return this.__nb;}
,_setSortAscending:function(bi){this.__nb=bi;}
,getRowCount:function(){return this.__mV.length;}
,getValue:function(bj,bk){if(bk<0||bk>=this.__mV.length){throw new Error(f+bk+d+this.__mV.length+e);}
;return this.__mV[bk][bj];}
,setValue:function(bl,bm,bn){if(this.__mV[bm][bl]!=bn){this.__mV[bm][bl]=bn;if(this.hasListener(a)){var bo={firstRow:bm,lastRow:bm,firstColumn:bl,lastColumn:bl};this.fireDataEvent(a,bo);}
;if(bl==this.__mW){this.clearSorting();}
;}
;}
,setData:function(bp,bq){this.__mV=bp;if(this.hasListener(a)){var br={firstRow:0,lastRow:bp.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,br);}
;if(bq!==false){this.clearSorting();}
;}
,getData:function(){return this.__mV;}
,setDataAsMapArray:function(bs,bt,bu){this.setData(this._mapArray2RowArr(bs,bt),bu);}
,addRows:function(bv,bw,bx){if(bw==null){bw=this.__mV.length;}
;bv.splice(0,0,bw,0);Array.prototype.splice.apply(this.__mV,bv);var by={firstRow:bw,lastRow:this.__mV.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,by);if(bx!==false){this.clearSorting();}
;}
...
;bD.splice(0,0,bE,bD.length);Array.prototype.splice.apply(this.__mV,bD);var bG={firstRow:bE,lastRow:this.__mV.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,bG);if(bF!==false){this.clearSorting();}
;}
,setRowsAsMapArray:function(bH,bI,bJ,bK){this.setRows(this._mapArray2RowArr(bH,bJ),bI,bK);}
,removeRows:function(bL,bM,bN){this.__mV.splice(bL,bM);var bO={firstRow:bL,lastRow:this.__mV.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1,removeStart:bL,removeCount:bM};this.fireDataEvent(a,bO);if(bN!==false){this.clearSorting();}
;}
...
;bT[i]=bS;}
;return bT;}
},destruct:function(){this.__mV=this.__mY=this.__mX=this.__na=null;}
});}
)();
