...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.SearchAsYouType","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType-qx.theme.Modern.01d6a558736f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var d="_applySearchString",c="demobrowser.demo.data.filter.SearchAsYouTypeFilter",b="String",a="";qx.Class.define(c,{extend:qx.core.Object,construct:function(e){qx.core.Object.call(this);this.__sj=e;this.filter=qx.lang.Function.bind(function(f){return f.search(this.getSearchString())!=-1;}
,this);this.__sQ=null;}
,properties:{searchString:{check:b,apply:d,init:a}},members:{__sQ:null,__sj:null,_applySearchString:function(g,h){var i=qx.util.TimerManager.getInstance();if(this.__sQ!=null){i.stop(this.__sQ);this.__sQ=null;}
;this.__sQ=i.start(function(){this.__sj.update();this.__sQ=null;}
,0,this,null,200);}
,filter:null},destruct:function(){this.__sj=null;}
});}
)();
(function(){var c="qx.util.TimerManager",b="singleton",a="interval";qx.Class.define(c,{extend:qx.core.Object,type:b,statics:{__Ku:[],__Kv:{},__sQ:0},members:{__Kw:false,start:function(d,e,f,g,h){if(!h){h=e||0;}
;var j=(new Date()).getTime()+h;this.self(arguments).__Kv[++this.self(arguments).__sQ]={callback:d,userData:g||null,expireAt:j,recurTime:e,context:f||this};this.__Kx(j,this.self(arguments).__sQ);return this.self(arguments).__sQ;}
,stop:function(k){var l=this.self(arguments).__Ku;var length=l.length;for(var i=0;i<length;i++){if(l[i]==k){l.splice(i,1);break;}
;}
;delete this.self(arguments).__Kv[k];if(l.length==0&&this.__Kw){qx.event.Idle.getInstance().removeListener(a,this.__Ky,this);this.__Kw=false;}
;}
,__Kx:function(m,n){var p=this.self(arguments).__Ku;var o=this.self(arguments).__Kv;var length=p.length;for(var i=0;i<length;i++){if(o[p[i]].expireAt>m){p.splice(i,0,n);break;}
;}
;if(p.length==length){p.push(n);}
;if(!this.__Kw){qx.event.Idle.getInstance().addListener(a,this.__Ky,this);this.__Kw=true;}
;}
,__Ky:function(){var t=(new Date()).getTime();var r=this.self(arguments).__Ku;var s=this.self(arguments).__Kv;while(r.length>0&&s[r[0]].expireAt<=t){var v=r.shift();var q=s[v];q.callback.call(q.context,q.userData,v);if(q.recurTime&&s[v]){var u=(new Date()).getTime();q.expireAt=u+q.recurTime;this.__Kx(q.expireAt,v);}
else {delete s[v];}
;}
;if(r.length==0&&this.__Kw){qx.event.Idle.getInstance().removeListener(a,this.__Ky,this);this.__Kw=false;}
;}
}});}
)();
