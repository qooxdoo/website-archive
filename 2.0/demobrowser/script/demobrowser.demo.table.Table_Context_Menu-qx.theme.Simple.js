...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Context_Menu","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Context_Menu-qx.theme.Simple.5732e3c2a1db.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="Row 10 changed",k="icon/22/actions/edit-undo.png",j="Change ID sort method",h="Disabled ctx menu",g="5 rows removed",f="icon/22/status/dialog-information.png",d="On",c="Change row with ID 10",b="Default ctx menu #2",a="Remove 5 rows",G="Add 10 rows",F="Context-menu selectable",E="",D="_tableModel",C="Table",B="icon/22/actions/list-remove.png",A="Whether the the first visible row should ",z=", ",y="demobrowser.demo.table.Table_Context_Menu",x="Default ctx menu #1",s="icon/22/actions/list-add.png",t="Selected rows:<br>",q="Keep first row",r="Off",o="be rendered completely when scrolling.",p="Show selection",m="Demonstrate use of alternate sorting algorithm.",n="10 rows added",u="changeValue",v="value",w="execute";qx.Class.define(y,{extend:demobrowser.demo.table.TableDemo,members:{main:function(){qx.Class.include(qx.ui.table.Table,qx.ui.table.MTableContextMenu);demobrowser.demo.table.TableDemo.prototype.main.call(this);this.getRoot().setNativeContextMenu(true);}
,getCaption:function(){return C;}
,createTable:function(){var J=this.createRandomRows(50);var I=this._tableModel=new qx.ui.table.model.Simple();I.setColumns([x,b,h,F]);I.setData(J);var K=new qx.ui.table.Table(I);K.set({width:600,height:400,decorator:null});var H=K.getTableColumnModel();H.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());K.setContextMenuHandler(3,this._contextMenuHandlerBoolean);K.setContextMenuHandler(2,this._contextMenuHandlerNull);return K;}
,_contextMenuHandlerBoolean:function(L,M,N,O,P){var Q;for(var i=0;i<=1;i++){Q=new qx.ui.menu.Button(i?d:r);Q.setUserData(v,i?true:false);Q.addListener(w,function(e){O.setValue(L,M,this.getUserData(v));}
);P.add(Q);}
;return true;}
,_contextMenuHandlerNull:function(R,S,T,U,V){return true;}
,createControls:function(){var ba=new qx.ui.toolbar.ToolBar();var X,Y,bb;Y=new qx.ui.toolbar.Part();ba.add(Y);X=new qx.ui.toolbar.Button(c,k);X.addListener(w,function(bc){var bd=this.createRandomRows(1);for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,bd[0][i]);}
;this.info(l);}
,this);Y.add(X);X=new qx.ui.toolbar.Button(G,s);X.addListener(w,function(be){var bf=this.createRandomRows(10);this._tableModel.addRows(bf);this.info(n);}
,this);Y.add(X);X=new qx.ui.toolbar.Button(a,B);X.addListener(w,function(bg){var bh=this._tableModel.getRowCount();this._tableModel.removeRows(bh-5,5);this.info(g);}
,this);Y.add(X);X=new qx.ui.toolbar.Button(p,f);X.addListener(w,function(bi){var bj=[];W.getSelectionModel().iterateSelection(function(bk){bj.push(bk+E);}
);this.showDialog(t+bj.join(z));}
,this);Y.add(X);Y=new qx.ui.toolbar.Part();ba.add(Y);var W=this._table;bb=new qx.ui.toolbar.CheckBox(q);bb.set({value:W.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(A+o)});bb.addListener(u,function(bl){W.setKeepFirstVisibleRowComplete(this.getValue());}
,bb);Y.add(bb);bb=new qx.ui.toolbar.CheckBox(j);bb.set({value:W.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(m)});bb.addListener(u,function(bm){if(bm.getData()){var bo=function(bp,bq){var br=bp[arguments.callee.columnIndex];var bs=bq[arguments.callee.columnIndex];if(br%2==1&&bs%2==0){return 1;}
;if(bs%2==1&&br%2==0){return -1;}
;return (br>bs)?1:((br==bs)?0:-1);}
;var bn=function(bt,bu){var bv=bt[arguments.callee.columnIndex];var bw=bu[arguments.callee.columnIndex];if(bv%2==1&&bw%2==0){return -1;}
;if(bw%2==1&&bv%2==0){return 1;}
;return (bv<bw)?1:((bv==bw)?0:-1);}
;W.getTableModel().setSortMethods(0,{ascending:bo,descending:bn});}
else {W.getTableModel().setSortMethods(0,null);}
;}
,bb);Y.add(bb);return ba;}
...
(function(){var f="contextmenu",d="function",c="qx.ui.table.MTableContextMenu",b="cellContextmenu",a="disappear";qx.Mixin.define(c,{construct:function(){this.addListener(b,function(e){var g=this.getContextMenu();if(g&&!g.isDisposed()){g.hide();this.setContextMenu(null);this.getApplicationRoot().remove(g);g.dispose();g=null;}
;var l=e.getColumn();var h=this.getContextMenuHandler(l);if(typeof h!==d){return;}
;var k=this.__MU[l];var i=this.getTableModel();g=new qx.ui.menu.Menu();g.addListener(f,function(e){e.preventDefault();}
);g.addListenerOnce(a,function(){this.setContextMenu(null);}
,this);var j=h.call(k,l,e.getRow(),this,i,g);if(!j){g.dispose();return;}
;this.setContextMenu(g);}
,this);this.__MT=[];this.__MU=[];}
,members:{__MT:null,__MU:null,setContextMenuHandler:function(m,n,o){this.__MT[m]=n;this.__MU[m]=o||this;}
,getContextMenuHandler:function(p){return this.__MT[p];}
},destruct:function(){this.__MT=null;this.__MU=null;}
});}
)();
