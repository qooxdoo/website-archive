...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Decorator_Box-qx.theme.Indigo.36dd2f31d1a7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var n="sliceBottom",m="horizontal",l="qx.debug.dispose",k="qx.ui.decoration.AbstractBox",j="sliceLeft",i="_applyBaseImage",h="sliceRight",g="_applyFill",f="shorthand",e="String",b="sliceTop",d="set",c="Number",a="_applySlices";qx.Class.define(k,{extend:qx.ui.decoration.Abstract,implement:[qx.ui.decoration.IDecorator],construct:function(o,p,q){qx.ui.decoration.Abstract.call(this);this._setOrientation(q);if(qx.ui.decoration.css3.BorderImage.IS_SUPPORTED){this.__AP=new qx.ui.decoration.css3.BorderImage();if(o){this.__AQ(o,q);}
;if(p!=null){this.__AP.setInsets(p);}
;}
else {this.__AP=new qx.ui.decoration.BoxDiv(o,p,q);}
;if(qx.core.Environment.get(l)){this.__AP.$$ignoreDisposeWarning=true;}
;}
,properties:{baseImage:{check:e,nullable:true,apply:i},sliceLeft:{check:c,nullable:true,apply:a},sliceRight:{check:c,nullable:true,apply:a},sliceBottom:{check:c,nullable:true,apply:a},sliceTop:{check:c,nullable:true,apply:a},slices:{group:[b,h,n,j],mode:f},fill:{apply:g}},members:{__AP:null,_isHorizontal:null,_setOrientation:function(r){this._isHorizontal=r==m;}
,getMarkup:function(){return this.__AP.getMarkup();}
,resize:function(s,t,u){this.__AP.resize(s,t,u);}
,tint:function(v,w){}
,getInsets:function(){return this.__AP.getInsets();}
,_applyInsets:function(x,y,name){var z=d+qx.lang.String.firstUp(name);this.__AP[z](x);}
,_applySlices:function(A,B,name){var C=d+qx.lang.String.firstUp(name);if(this.__AP[C]){this.__AP[C](A);}
;}
,_applyFill:function(D,E,name){if(this.__AP.setFill){this.__AP.setFill(D);}
;}
,_applyBaseImage:function(F,G){if(this.__AP instanceof qx.ui.decoration.BoxDiv){this.__AP.setBaseImage(F);}
else {this.__AQ(F);}
;}
,__AQ:function(H){this.__AP.setBorderImage(H);var N=qx.util.AliasManager.getInstance().resolve(H);var O=/(.*)(\.[a-z]+)$/.exec(N);var L=O[1];var M=O[2];var K=qx.util.ResourceManager.getInstance();if(qx.core.Environment.get("qx.debug")){if(!K.has(N)){this.warn("The border image '"+N+"' is not available!");}
;}
;if(this._isHorizontal){var Q=K.getImageWidth(L+"-l"+M);var I=K.getImageWidth(L+"-r"+M);if(I&&Q){this.__AP.setSlice([0,I,0,Q]);}
;}
else {var J=K.getImageHeight(L+"-b"+M);var P=K.getImageHeight(L+"-t"+M);if(P&&J){this.__AP.setSlice([P,0,J,0]);}
;}
;}
},destruct:function(){this.__AS=this.__Bl=this.__Bm=null;this.__AP.dispose();}
});}
)();
...
;}
,properties:{baseImage:{check:x,nullable:true,apply:c}},members:{__AS:null,__Bl:null,__Bm:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};}
,_isInitialized:function(){return !!this.__AS;}
,_setOrientation:function(B){this._isHorizontal=B==r;}
,getMarkup:function(){if(this.__AS){return this.__AS;}
;var C=qx.bom.element.Decoration;var D=this.__Bl;var E=this.__Bm;var F=[];F.push(e);if(this._isHorizontal){F.push(C.create(D.l,k,{top:0,left:0}));F.push(C.create(D.c,p,{top:0,left:E.left+l}));F.push(C.create(D.r,k,{top:0,right:0}));}
...
;F.push(i);return this.__AS=F.join(w);}
,resize:function(G,H,I){G.style.width=H+l;G.style.height=I+l;var J=this.__Bm;if(this._isHorizontal){var innerWidth=H-J.left-J.right;innerWidth=innerWidth<0?0:innerWidth;G.childNodes[1].style.width=innerWidth+l;}
...
;}
,tint:function(K,L){}
,_applyBaseImage:function(M,N){if(qx.core.Environment.get(d)){if(this.__AS){throw new Error(q);}
;}
;var O=qx.util.ResourceManager.getInstance();if(M){var Q=qx.util.AliasManager.getInstance();var S=Q.resolve(M);var T=/(.*)(\.[a-z]+)$/.exec(S);var R=T[1];var P=T[2];var U=this.__Bl={t:R+h+P,b:R+b+P,c:R+u+P,l:R+j+P,r:R+f+P};this.__Bm={top:O.getImageHeight(U.t),bottom:O.getImageHeight(U.b),left:O.getImageWidth(U.l),right:O.getImageWidth(U.r)};}
;}
},destruct:function(){this.__AS=this.__Bl=this.__Bm=null;}
});}
)();
