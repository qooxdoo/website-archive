...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.root.Inline_Window","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.root.Inline_Window-qx.theme.Simple.7d4f6c8e15f8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var x="",w=" needs a height when its height size should be used!",v="px",u="position",t="qx.debug",s=" needs a width when its width size should be used!",r="engine.version",q="qx.ui.root.Inline",p="$$widget",o="appear",f="hidden",n="div",k="left",c="Please use a DOM element to create an inline root.",b="mshtml",h="engine.name",g="height",l="The root element ",a=" of ",m="resize",d="relative";qx.Class.define(q,{extend:qx.ui.root.Abstract,include:[qx.ui.core.MLayoutHandling],construct:function(y,z,A){if(qx.core.Environment.get(t)){this.assertElement(y,c);}
;this.__Bo=y;y.style.overflow=f;y.style.textAlign=k;this.__Bp=z||false;this.__Bq=A||false;this.__Br();qx.ui.root.Abstract.call(this);this._setLayout(new qx.ui.layout.Basic());qx.ui.core.queue.Layout.add(this);qx.ui.core.FocusHandler.getInstance().connectTo(this);if((qx.core.Environment.get(h)==b)){this.setKeepFocus(true);}
;var window=qx.dom.Node.getWindow(y);qx.event.Registration.addListener(window,m,this._onWindowResize,this);}
,members:{__Bp:false,__Bq:false,__Bo:null,__Br:function(){if(this.__Bp||this.__Bq){var B=qx.bom.element.Dimension.getSize(this.__Bo);if(this.__Bp&&B.width<1){throw new Error(l+this.__Bo+a+this+s);}
;if(this.__Bq){if(B.height<1){throw new Error(l+this.__Bo+a+this+w);}
;if(B.height>=1&&qx.bom.element.Style.get(this.__Bo,g,3)==x){qx.bom.element.Style.set(this.__Bo,g,B.height+v);}
;}
;qx.event.Registration.addListener(this.__Bo,m,this._onResize,this);}
;}
,_createContainerElement:function(){var C=this.__Bo;if(this.__Bp||this.__Bq){var G=document.createElement(n);C.appendChild(G);if((qx.core.Environment.get(h)==b)&&qx.core.Environment.get(r)==6){var F=qx.dom.Node.getBodyElement(C);var E;var I;var H=false;var D=qx.dom.Hierarchy.getAncestors(C);for(var i=0,j=D.length;i<j;i++){E=D[i];if(E!=F){I=qx.bom.element.Style.get(E,u);if(I==d){H=true;break;}
;}
else {break;}
;}
;if(H){C.style.position=d;}
;}
;}
else {G=C;}
;var J=new qx.html.Root(G);G.style.position=d;J.setAttribute(p,this.toHashCode());qx.event.Timer.once(function(e){this.fireEvent(o);}
,this,0);return J;}
,_onResize:function(e){var K=e.getData();if((K.oldWidth!==K.width)&&this.__Bp||(K.oldHeight!==K.height)&&this.__Bq){qx.ui.core.queue.Layout.add(this);}
;}
,_onWindowResize:function(){if(qx.ui.popup&&qx.ui.popup.Manager){qx.ui.popup.Manager.getInstance().hideAll();}
;if(qx.ui.menu&&qx.ui.menu.Manager){qx.ui.menu.Manager.getInstance().hideAll();}
;}
,_computeSizeHint:function(){var P=this.__Bp;var M=this.__Bq;if(!P||!M){var L=qx.ui.root.Abstract.prototype._computeSizeHint.call(this);}
else {L={};}
;var Q=qx.bom.element.Dimension;if(P){var O=Q.getContentWidth(this.__Bo);L.width=O;L.minWidth=O;L.maxWidth=O;}
;if(M){var N=Q.getContentHeight(this.__Bo);L.height=N;L.minHeight=N;L.maxHeight=N;}
;return L;}
},defer:function(R,S){qx.ui.core.MLayoutHandling.remap(S);}
,destruct:function(){qx.event.Registration.removeListener(this.__Bo,m,this._onResize,this);this.__Bo=null;}
});}
)();
(function(){var f="resize",d="interval",c="__iG",b="body",a="qx.event.handler.ElementResize";qx.Class.define(a,{extend:qx.core.Object,implement:qx.event.IEventHandler,construct:function(g){qx.core.Object.call(this);this.__dz=g;this.__Cb={};this.__iG=new qx.event.Timer(200);this.__iG.addListener(d,this._onInterval,this);}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{resize:true},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__Cb:null,__dz:null,__iG:null,canHandleEvent:function(h,i){return h.tagName.toLowerCase()!==b;}
,registerEvent:function(j,k,l){var n=qx.core.ObjectRegistry.toHashCode(j);var m=this.__Cb;if(!m[n]){m[n]={element:j,width:qx.bom.element.Dimension.getWidth(j),height:qx.bom.element.Dimension.getHeight(j)};this.__iG.start();}
;}
,unregisterEvent:function(o,p,q){var s=qx.core.ObjectRegistry.toHashCode(o);var r=this.__Cb;if(r[s]){delete r[s];if(qx.lang.Object.isEmpty(r)){this.__iG.stop();}
;}
;}
,_onInterval:function(e){var u=this.__Cb;for(var x in u){var y=u[x];var t=y.element;var w=qx.bom.element.Dimension.getWidth(t);var v=qx.bom.element.Dimension.getHeight(t);if(y.height!==v||y.width!==w){qx.event.Registration.fireNonBubblingEvent(t,f,qx.event.type.Data,[{width:w,oldWidth:y.width,height:v,oldHeight:y.height}]);y.width=w;y.height=v;}
;}
;}
},destruct:function(){this.__dz=this.__Cb=null;this._disposeObjects(c);}
