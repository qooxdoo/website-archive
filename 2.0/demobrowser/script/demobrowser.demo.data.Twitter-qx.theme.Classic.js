...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.Twitter","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
for (var k in envinfo) qx.$$environment[k] = envinfo[k];

if (!qx.$$libraries) qx.$$libraries = {};
var libinfo = {"__out__":{"sourceUri":"../../script"},"demobrowser.demo":{"resourceUri":"../../resource","sourceUri":"../../script"},"qx":{"resourceUri":"../../resource","sourceUri":"../../script","sourceViewUri":"https://github.com/qooxdoo/qooxdoo/blob/%{qxGitBranch}/framework/source/class/%{classFilePath}#L%{lineNumber}"},"qxc.application.twitterdemo":{"resourceUri":"../../resource","sourceUri":"../../script"}};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Twitter-qx.theme.Classic.0d8959b28e12.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="demobrowser.demo.data.Twitter";qx.Class.define(a,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);this.getRoot().add(new qxc.application.twitterdemo.Demo());}
}});}
)();
(function(){var k="qxc/application/twitterdemo/twitter_logo_outline.png",j="Enter",h="one",g="http",f="keydown",e="http://twitter.com/statuses/user_timeline/1and1.json",d="<a href='",c="execute",b="1and1",a="selection[0]",J="username",I="Location: ",H="Details",G="user.name",F="Name: ",E="qxc.application.twitterdemo.Demo",D="target='_blank'",C="</a>",B="Posted with: ",A="' target='_blank'>",r="Show",s="user.location",p=".json",q="http://twitter.com/statuses/user_timeline/",n="Avatar: ",o="Message: ",l="text",m="source",t="mshtml",u="engine.name",w="user.profile_image_url",v="icon",y="model",x="value",z=" ";qx.Class.define(E,{extend:qx.ui.container.Composite,construct:function(){qx.ui.container.Composite.call(this);this._createView();}
,members:{_createView:function(){this.setLayout(new qx.ui.layout.Canvas());var T=new qx.ui.basic.Image(k);this.add(T,{left:10,top:15});var O=new qx.ui.form.List();this.add(O,{left:10,top:135,bottom:5});O.set({selectionMode:h,width:300,maxHeight:400});var R=new qx.data.controller.List(null,O);R.setDelegate(this);R.setLabelPath(l);if(qx.core.Environment.get(u)!=t){R.setIconPath(w);}
;var M=e;var V=new qx.data.store.Jsonp(M);V.bind(y,R,y);var P=new qx.ui.container.Composite(new qx.ui.layout.HBox(5));P.set({width:300});P.add(new qx.ui.core.Spacer(),{flex:1});var K=new qx.ui.form.TextField(b);K.setPlaceholder(J);P.add(K);var S=new qx.ui.form.Button(r);P.add(S);S.addListener(c,function(){var X=q+K.getValue()+p;if(V.getUrl()==X){V.reload();}
else {V.setUrl(X);}
;}
,this);K.addListener(f,function(Y){if(Y.getKeyIdentifier()==j){S.execute();}
;}
,this);this.add(P,{left:10,top:105});var N=new qx.ui.groupbox.GroupBox(H);this.add(N,{left:320,top:116,bottom:5});N.setWidth(270);N.setHeight(220);N.setAllowGrowY(false);N.setLayout(new qx.ui.layout.Grid(5,5));N.add(new qx.ui.basic.Label(F),{row:0,column:0});N.add(new qx.ui.basic.Label(I),{row:1,column:0});N.add(new qx.ui.basic.Label(o),{row:2,column:0});N.add(new qx.ui.basic.Label(B),{row:3,column:0});var name=new qx.ui.basic.Label();N.add(name,{row:0,column:1});var location=new qx.ui.basic.Label();N.add(location,{row:1,column:1});var U=new qx.ui.basic.Label();U.setRich(true);U.setWidth(150);U.setSelectable(true);N.add(U,{row:2,column:1});var W=new qx.ui.basic.Label();W.setRich(true);N.add(W,{row:3,column:1});var L=new qx.data.controller.Object();L.addTarget(name,x,G);L.addTarget(location,x,s);L.addTarget(U,x,l,false,{converter:function(ba){var bb=ba.split(z);for(var i=bb.length-1;i>=0;i--){if(bb[i].indexOf(g)==0){bb[i]=d+bb[i]+A+bb[i]+C;}
;}
;return bb.join(z);}
});L.addTarget(W,x,m,false,{converter:function(bc){bc=bc.split(z);bc.splice(1,0,D);return bc.join(z);}
});if(qx.core.Environment.get(u)!=t){N.add(new qx.ui.basic.Label(n),{row:4,column:0});var Q=new qx.ui.basic.Image();N.add(Q,{row:4,column:1});L.addTarget(Q,m,w);}
;R.bind(a,L,y);}
,configureItem:function(bd){bd.setRich(true);bd.getChildControl(v).setWidth(48);bd.getChildControl(v).setHeight(48);bd.getChildControl(v).setScale(true);}
}});}
)();
...
(function(){var m="qx.bom.request.Jsonp",l="callback",k="open",j="dispose",i="",h="_onNativeLoad",g="qx.debug",f="Expecting JavaScript response to call: ",e="].callback",d=" already exists",a="Callback ",c="qx.bom.request.Jsonp[",b="qx.debug.io";qx.Bootstrap.define(m,{extend:qx.bom.request.Script,construct:function(){qx.bom.request.Script.apply(this);this.__tk();}
,members:{responseJson:null,__bV:null,__tf:null,__tg:null,__th:null,__ti:null,__sR:null,open:function(n,o){if(this.__sR){return;}
;var p={},r,q,s=this;this.responseJson=null;this.__th=false;r=this.__tf||l;q=this.__tg||c+this.__bV+e;if(!this.__tg){this.constructor[this.__bV]=this;}
else {if(!window[this.__tg]){this.__ti=true;window[this.__tg]=function(t){s.callback(t);}
;}
else {if(qx.core.Environment.get(b)){qx.Bootstrap.debug(qx.bom.request.Jsonp,a+this.__tg+d);}
;}
;}
;if(qx.core.Environment.get(b)){qx.Bootstrap.debug(qx.bom.request.Jsonp,f+q);}
;p[r]=q;o=qx.util.Uri.appendParamsToUrl(o,p);this.__fK(k,[n,o]);}
,callback:function(u){if(this.__sR){return;}
;this.__th=true;if(qx.core.Environment.get(g)){u=qx.lang.Json.stringify(u);u=qx.lang.Json.parse(u);}
;this.responseJson=u;this.constructor[this.__bV]=undefined;this.__tj();}
,setCallbackParam:function(v){this.__tf=v;return this;}
,setCallbackName:function(name){this.__tg=name;return this;}
,dispose:function(){this.__tj();this.__fK(j);}
,_onNativeLoad:function(){this.status=this.__th?200:500;this.__fK(h);}
,__tj:function(){if(this.__ti&&window[this.__tg]){window[this.__tg]=undefined;this.__ti=false;}
;}
,__fK:function(w,x){qx.bom.request.Script.prototype[w].apply(this,x||[]);}
,__tk:function(){this.__bV=(new Date().valueOf())+(i+Math.random()).substring(2,5);}
}});}
)();
...
;}
,properties:{model:{check:g,event:h,apply:c,nullable:true,dereference:true}},members:{__ej:null,__bg:null,_applyModel:function(k,l){for(var i=0;i<this.__ej.length;i++){var t=this.__ej[i][0];var q=this.__ej[i][1];var o=this.__ej[i][2];var r=this.__ej[i][3];var s=this.__ej[i][4];var n=this.__ej[i][5];if(l!=undefined&&!l.isDisposed()){this.__ym(t,q,o,l);}
;if(k!=undefined){this.__yl(t,q,o,r,s,n);}
...
;}
;}
,addTarget:function(u,v,w,x,y,z){this.__ej.push([u,v,w,x,y,z]);this.__yl(u,v,w,x,y,z);}
,__yl:function(A,B,C,D,E,F){if(this.getModel()==null){return;}
...
,removeTarget:function(J,K,L){this.__ym(J,K,L,this.getModel());for(var i=0;i<this.__ej.length;i++){if(this.__ej[i][0]==J&&this.__ej[i][1]==K&&this.__ej[i][2]==L){this.__ej.splice(i,1);}
;}
;}
,__ym:function(M,N,O,P){if(!(M instanceof qx.core.Object)){return;}
