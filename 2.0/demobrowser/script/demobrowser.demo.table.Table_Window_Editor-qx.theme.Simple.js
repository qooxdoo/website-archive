...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Window_Editor","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Window_Editor-qx.theme.Simple.3f9026cdc5c3.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var p="Enter",o="A boolean",n="A number",m="Table with window cell editor",l="A date",k="",j="1*",i="50%",h="demobrowser.demo.table.Table_Window_Editor",g="appear",c="demobrowser.demo.table.ModalCellEditorFactory",f="Cell Editor",d="ID",b="Save",a="execute";qx.Class.define(h,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return m;}
,createTable:function(){var u=this.createRandomRows(50);var s=new qx.ui.table.model.Simple();s.setColumns([d,n,l,o]);s.setData(u);s.setColumnEditable(1,true);var r={tableColumnModel:function(w){return new qx.ui.table.columnmodel.Resize(w);}
};var v=new qx.ui.table.Table(s,r);v.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);var t=v.getTableColumnModel();var q=t.getBehavior();q.set(0,{width:j,minWidth:40,maxWidth:80});q.setWidth(1,i);q.setMinWidth(1,100);q.setMaxWidth(1,320);q.set(3,{width:100});t.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());v.setShowCellFocusIndicator(false);t.setCellEditorFactory(1,new demobrowser.demo.table.ModalCellEditorFactory());return v;}
}});qx.Class.define(c,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,members:{createCellEditor:function(x){var A=new qx.ui.window.Window(f);A.setLayout(new qx.ui.layout.HBox(4));A.set({padding:3,modal:true,showClose:false,showMaximize:false,showMinimize:false});A.moveTo(300,250);A.addListener(g,function(e){A.__pu.focus();A.__pu.setTextSelection(0,A.__pu.getValue().length);}
);A.__pu=new qx.ui.form.TextField(x.value+k).set({allowGrowY:true});A.add(A.__pu);var y=new qx.ui.form.Button(b);y.addListener(a,function(e){A.close();}
);A.add(y);var z=new qx.ui.core.Command(p);z.addListener(a,function(e){y.execute();z.dispose();z=null;}
);return A;}
,getCellEditorValue:function(B){return parseFloat(B.__pu.getValue());}
}});}
)();
...
(function(){var j="css.inlineblock",i="repeat",h="scale",g="_identifyImage is abstract",f="string",e="static",d="'",c="qx.ui.table.cellrenderer.AbstractImage",b="}",a="  text-align:center;",x="scale-x",w="repeat-y",v=".qooxdoo-table-cell-icon {",u="",t="<div></div>",s="top",r="abstract",q=" qooxdoo-table-cell-icon",p="repeat-x",o="  padding-top:1px;",m="title='",n="scale-y",k="px",l="no-repeat";qx.Class.define(c,{extend:qx.ui.table.cellrenderer.Abstract,type:r,construct:function(){qx.ui.table.cellrenderer.Abstract.call(this);var y=this.self(arguments);if(!y.stylesheet){y.stylesheet=qx.bom.Stylesheet.createElement(v+a+o+b);}
;}
,properties:{repeat:{check:function(z){var A=[h,x,n,i,p,w,l];return qx.lang.Array.contains(A,z);}
,init:l}},members:{__qm:16,__qn:16,__qo:null,_insetY:2,_identifyImage:function(B){throw new Error(g);}
,_getImageInfos:function(C){var D=this._identifyImage(C);if(D==null||typeof D==f){D={url:D,tooltip:null};}
;if(!D.imageWidth||!D.imageHeight){var E=this.__qp(D.url);D.imageWidth=E.width;D.imageHeight=E.height;}
;D.width=D.imageWidth;D.height=D.imageHeight;return D;}
,__qp:function(F){var I=qx.util.ResourceManager.getInstance();var H=qx.io.ImageLoader;var G,J;if(I.has(F)){G=I.getImageWidth(F);J=I.getImageHeight(F);}
else if(H.isLoaded(F)){G=H.getWidth(F);J=H.getHeight(F);}
else {G=this.__qm;J=this.__qn;}
;return {width:G,height:J};}
,createDataCellHtml:function(K,L){this.__qo=this._getImageInfos(K);return qx.ui.table.cellrenderer.Abstract.prototype.createDataCellHtml.call(this,K,L);}
,_getCellClass:function(M){return qx.ui.table.cellrenderer.Abstract.prototype._getCellClass.call(this)+q;}
,_getContentHtml:function(N){var content=t;if(this.__qo.url){content=qx.bom.element.Decoration.create(this.__qo.url,this.getRepeat(),{width:this.__qo.width+k,height:this.__qo.height+k,display:qx.core.Environment.get(j),verticalAlign:s,position:e});}
;return content;}
,_getCellAttributes:function(O){var P=this.__qo.tooltip;if(P){return m+P+d;}
else {return u;}
;}
},destruct:function(){this.__qo=null;}
});}
)();
(function(){var g="_applyIconTrue",f="decoration/table/boolean-true.png",e="qx.ui.table.cellrenderer.Boolean",d=";padding-top:4px;",c="decoration/table/boolean-false.png",b="_applyIconFalse",a="String";qx.Class.define(e,{extend:qx.ui.table.cellrenderer.AbstractImage,construct:function(){qx.ui.table.cellrenderer.AbstractImage.call(this);this.__xT=qx.util.AliasManager.getInstance();this.initIconTrue();this.initIconFalse();}
,properties:{iconTrue:{check:a,init:f,apply:g},iconFalse:{check:a,init:c,apply:b}},members:{__xU:null,__xV:false,__xT:null,_applyIconTrue:function(h){this.__xU=this.__xT.resolve(h);}
,_applyIconFalse:function(i){this.__xV=this.__xT.resolve(i);}
,_insetY:5,_getCellStyle:function(j){return qx.ui.table.cellrenderer.AbstractImage.prototype._getCellStyle.call(this,j)+d;}
,_identifyImage:function(k){var l={imageWidth:11,imageHeight:11};switch(k.value){case true:l.url=this.__xU;break;case false:l.url=this.__xV;break;default:l.url=null;break;};return l;}
...
(function(){var m="-",l="PageUp",k="Escape",j="qx.event.type.Data",h="_applyShortcut",g="PrintScreen",f="NumLock",d="5",c="8",b="execute",S="Meta",R="0",Q="2",P="Shift",O="You can only specify one non modifier key!",N="3",M="/",L="Delete",K="String",J="changeEnabled",t="*",u="qx.bom.Shortcut",r="6",s="4",p="1",q="Alt",n="Not a valid key name for a shortcut: ",o="PageDown",v="Whitespaces are not allowed within shortcuts",w="_applyEnabled",B="7",A="a",D="z",C="9",F="Boolean",E="+",y="short",I="keydown",H="",G="Control",x="keypress",z="Unidentified";qx.Class.define(u,{extend:qx.core.Object,construct:function(T){qx.core.Object.call(this);this.__yf={};this.__yg=null;if(T!=null){this.setShortcut(T);}
;this.initEnabled();}
,events:{"execute":j},properties:{enabled:{init:true,check:F,event:J,apply:w},shortcut:{check:K,apply:h,nullable:true},autoRepeat:{check:F,init:false}},members:{__yf:H,__yg:H,execute:function(U){this.fireDataEvent(b,U);}
,__yh:function(event){if(this.getEnabled()&&this.__yi(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,__eJ:function(event){if(this.getEnabled()&&this.__yi(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,_applyEnabled:function(V,W){if(V){qx.event.Registration.addListener(document.documentElement,I,this.__yh,this);qx.event.Registration.addListener(document.documentElement,x,this.__eJ,this);}
else {qx.event.Registration.removeListener(document.documentElement,I,this.__yh,this);qx.event.Registration.removeListener(document.documentElement,x,this.__eJ,this);}
;}
,_applyShortcut:function(X,Y){if(X){if(X.search(/[\s]+/)!=-1){var bc=v;this.error(bc);throw new Error(bc);}
;this.__yf={"Control":false,"Shift":false,"Meta":false,"Alt":false};this.__yg=null;var ba;var a=[];while(X.length>0&&ba!=-1){ba=X.search(/[-+]+/);a.push((X.length==1||ba==-1)?X:X.substring(0,ba));X=X.substring(ba+1);}
;var bb=a.length;for(var i=0;i<bb;i++){var bd=this.__yk(a[i]);switch(bd){case G:case P:case S:case q:this.__yf[bd]=true;break;case z:var bc=n+a[i];this.error(bc);throw bc;default:if(this.__yg){var bc=O;this.error(bc);throw bc;}
;this.__yg=bd;};}
;}
;return true;}
,__yi:function(e){var be=this.__yg;if(!be){return;}
;if((!this.__yf.Shift&&e.isShiftPressed())||(this.__yf.Shift&&!e.isShiftPressed())||(!this.__yf.Control&&e.isCtrlPressed())||(this.__yf.Control&&!e.isCtrlPressed())||(!this.__yf.Meta&&e.isMetaPressed())||(this.__yf.Meta&&!e.isMetaPressed())||(!this.__yf.Alt&&e.isAltPressed())||(this.__yf.Alt&&!e.isAltPressed())){return false;}
;if(be==e.getKeyIdentifier()){return true;}
;return false;}
,__yj:{esc:k,ctrl:G,print:g,del:L,pageup:l,pagedown:o,numlock:f,numpad_0:R,numpad_1:p,numpad_2:Q,numpad_3:N,numpad_4:s,numpad_5:d,numpad_6:r,numpad_7:B,numpad_8:c,numpad_9:C,numpad_divide:M,numpad_multiply:t,numpad_minus:m,numpad_plus:E},__yk:function(bf){var bg=qx.event.util.Keyboard;var bh=z;if(bg.isValidKeyIdentifier(bf)){return bf;}
;if(bf.length==1&&bf>=A&&bf<=D){return bf.toUpperCase();}
;bf=bf.toLowerCase();var bh=this.__yj[bf]||qx.lang.String.firstUp(bf);if(bg.isValidKeyIdentifier(bh)){return bh;}
else {return z;}
;}
,toString:function(){var bk=this.__yg;var bj=[];for(var bi in this.__yf){if(this.__yf[bi]){bj.push(qx.locale.Key.getKeyName(y,bi));}
;}
;if(bk){bj.push(qx.locale.Key.getKeyName(y,bk));}
;return bj.join(E);}
},destruct:function(){this.setEnabled(false);this.__yf=this.__yg=null;}
});}
)();
