...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.virtual.SettingsList","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.SettingsList-qx.theme.Classic.6371190930e5.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var p="group.checked",o="#ddd",n="Fruits",m="Apple",l="Carrot",k="Vegetables",j="demobrowser.demo.virtual.SettingsList",h="changeValue",g="Orange",f="Potato",b="name",d="label",c="checked",a="value";qx.Class.define(j,{extend:qx.application.Standalone,members:{__cR:null,main:function(){qx.application.Standalone.prototype.main.call(this);var r=[];var s=[m,g,f,l];for(var i=0;i<s.length;i++){r[i]={name:s[i],checked:i<2,group:null};}
;this.__cR=qx.data.marshal.Json.createModel(r,true);var t=[];for(var i=0;i<2;i++){t[i]={name:null,checked:false};}
;t=qx.data.marshal.Json.createModel(t,true);t.getItem(0).setName(n);t.getItem(1).setName(k);this.__cR.toArray().forEach(function(u,v){v=v<2?0:1;u.setGroup(t.getItem(v));}
);var q=new qx.ui.list.List();q.setWidth(150);this.getRoot().add(q,{left:20,top:20});q.setDelegate(this);q.setModel(this.__cR);}
...
,configureGroupItem:function(x){x.setBackgroundColor(o);x.setTriState(true);x.addListener(h,function(e){if(x.getValue()===null){return;}
;var y=e.getData();this.__cR.forEach(function(z){if(x.getLabel()==z.getGroup().getName()){z.setChecked(y);}
;}
);}
,this);}
,bindItem:function(A,B,C){A.bindProperty(b,d,null,B,C);A.bindProperty(c,a,null,B,C);A.bindPropertyReverse(c,a,null,B,C);var D=this.__cR;A.bindPropertyReverse(p,a,{converter:function(E,F){var H=F.getItem(C);var J=H.getGroup();var G=D.toArray().filter(function(L){return L.getGroup()==J;}
...
,bindGroupItem:function(O,P,Q){O.bindProperty(b,d,null,P,Q);O.bindProperty(c,a,null,P,Q);O.bindPropertyReverse(c,a,null,P,Q);}
...
,_init:function(){this._provider=new qx.ui.list.provider.WidgetProvider(this);this.__yq=[];this.__yr=[];this.__ys={};this.__yt=false;this.__yu=false;this.__yv=false;this.getPane().addListener(o,this._onResize,this);this._initBackground();this._initLayer();}
...
;}
,_getLookupTable:function(){return this.__yq;}
,_lookup:function(O){return this.__yq[O];}
,_lookupGroup:function(P){return this.__yr.indexOf(P);}
,_reverseLookup:function(Q){if(Q<0){return -1;}
;return this.__yq.indexOf(Q);}
...
,_applyDelegate:function(bj,bk){this._provider.setDelegate(bj);this.__yx();}
,_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);}
,_onModelChange:function(e){this.__yx();this._applyDefaultSelection();}
,__yw:function(){this.getPane().getRowConfig().setItemCount(this.__yq.length);this.getPane().fullUpdate();}
,__yx:function(){this.__yq=[];this.__yr=[];this.__ys={};if(this.isAutoGrouping()){this.getGroups().removeAll();}
;var bl=this.getModel();if(bl!=null){this._runDelegateFilter(bl);this._runDelegateSorter(bl);this._runDelegateGroup(bl);}
;this._updateSelection();this.__yw();}
,_runDelegateFilter:function(bm){var bn=qx.util.Delegate.getMethod(this.getDelegate(),h);for(var i=0,l=bm.length;i<l;++i){if(bn==null||bn(bm.getItem(i))){this.__yq.push(i);}
;}
;}
,_runDelegateSorter:function(bo){if(this.__yq.length==0){return;}
;var bp=qx.util.Delegate.getMethod(this.getDelegate(),F);if(bp!=null){this.__yq.sort(function(a,b){return bp(bo.getItem(a),bo.getItem(b));}
);}
;}
,_runDelegateGroup:function(bq){var bu=qx.util.Delegate.getMethod(this.getDelegate(),s);if(bu!=null){for(var i=0,l=this.__yq.length;i<l;++i){var br=this.__yq[i];var bt=this.getModel().getItem(br);var bs=bu(bt);this.__yy(bs,br);}
;this.__yq=this.__yz();}
;}
,__yy:function(bv,bw){if(bv==null){this.__yv=true;bv=A;}
;var name=this.__yA(bv);if(this.__ys[name]==null){this.__ys[name]=[];if(this.isAutoGrouping()){this.getGroups().push(bv);}
;}
;this.__ys[name].push(bw);}
,__yz:function(){this.__yB();var bA=[];var bC=0;var by=this.getGroups();for(var i=0;i<by.getLength();i++){var bx=by.getItem(i);bA.push(-1);this.__yr.push(bC);bC++;var bB=this.__yA(bx);var bz=this.__ys[bB];if(bz!=null){for(var k=0;k<bz.length;k++){bA.push(bz[k]);bC++;}
;}
;}
;return bA;}
,__yA:function(bD){var name=null;if(!qx.lang.Type.isString(bD)){var bE=this.getGroups().indexOf(bD);this.__yu=true;name=s;if(bE==-1){name+=this.getGroups().getLength();}
else {name+=bE;}
;}
else {this.__yt=true;var name=bD;}
;return name;}
,__yB:function(){if(this.__yu&&this.__yv||this.__yu&&this.__yt){throw new Error(G+B);}
;}
},destruct:function(){this._background.dispose();this._provider.dispose();this._layer.dispose();this._background=this._provider=this._layer=this.__yq=this.__yr=this.__ys=null;if(this.__yp){this.__yp.dispose();}
;}
});}
)();
...
(function(){var p="model[",o="groups[",n="model",m="changeDelegate",l="label",k="qx.ui.list.core.MWidgetController",j="icon",i="value",h=".",g="",c="]",f="BindingIds",d="group",b="cell.type",a="String";qx.Mixin.define(k,{construct:function(){this.__yC=[];}
...
;}
,bindProperty:function(s,t,u,v,w){var y=v.getUserData(b);var x=this.__yD(w,s,y);var z=this._list.bind(x,v,t,u);this.__yE(v,z);}
,bindPropertyReverse:function(A,B,C,D,E){var G=D.getUserData(b);var F=this.__yD(E,A,G);var H=D.bind(B,this._list,F,C);this.__yE(D,H);}
,removeBindings:function(){while(this.__yC.length>0){var I=this.__yC.pop();this._removeBindingsFrom(I);}
;}
...
;}
,_removeBindingsFrom:function(T){var U=this.__yF(T);while(U.length>0){var V=U.pop();try{this._list.removeBinding(V);}
catch(e){T.removeBinding(V);}
;}
;if(qx.lang.Array.contains(this.__yC,T)){qx.lang.Array.remove(this.__yC,T);}
;}
,__yD:function(W,X,Y){var ba=p+W+c;if(Y==d){ba=o+W+c;}
;if(X!=null&&X!=g){ba+=h+X;}
;return ba;}
,__yE:function(bb,bc){var bd=this.__yF(bb);if(!qx.lang.Array.contains(bd,bc)){bd.push(bc);}
;if(!qx.lang.Array.contains(this.__yC,bb)){this.__yC.push(bb);}
;}
,__yF:function(be){var bf=be.getUserData(f);if(bf==null){bf=[];be.setUserData(f,bf);}
;return bf;}
...
,styleSelectabled:function(u){var v=this.__yn(u);this._styleSelectabled(v);}
,styleUnselectabled:function(w){var x=this.__yn(w);this._styleUnselectabled(x);}
...
;}
,_styleSelectabled:function(A){this.__yo(A,{selected:1});}
,_styleUnselectabled:function(B){this.__yo(B,{});}
...
,__yn:function(G){return this._list._layer.getRenderedCellWidget(G,0);}
,__yo:function(H,I){if(H==null){return;}
