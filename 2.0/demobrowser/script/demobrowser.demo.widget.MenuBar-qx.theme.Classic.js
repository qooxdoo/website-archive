...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.MenuBar","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.MenuBar-qx.theme.Classic.3346db025c56.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var bJ="Online Forum",bI="New",bH="icon/16/actions/edit-cut.png",bG="icon/16/actions/format-justify-fill.png",bF="Search in files",bE="C",bD="Left aligned",bC="Undo",bB="_openCommand",bA="Uppercase",bp="View",bo="C Dialect",bn="changeSelection",bm="enabled",bl="Save as...",bk="Show line numbers",bj="Search previous...",bi="Quickstart",bh="Cut",bg="Ctrl+N",bQ="icon/16/actions/edit-paste.png",bR="Show console",bO="Search",bP="Help",bM="_redoCommand",bN="Redo",bK="Show status bar",bL="icon/16/actions/format-justify-left.png",bS="ANSI to OEM",bT="Centered",bt="Paragraph",bs="Edit",bv="Ctrl+O",bu="icon/16/actions/document-print.png",bx="icon/16/actions/document-open.png",bw="C Sharp",bz="icon/16/actions/edit-undo.png",by="Close",br="icon/16/actions/document-save-as.png",bq="Python",a="icon/16/apps/utilities-help.png",b="Show tree",c="Spaces to tabs",d="Right aligned",f="OEM to ANSI",g="JavaScript",h="demobrowser.demo.widget.MenuBar",i="_copyCommand",j="XML",k="Save",bX="_undoCommand",bW="Capitals",bV="icon/16/actions/application-exit.png",bU="C Plus Plus",cc="_saveCommand",cb="Lowercase",ca="Search next...",bY="_newCommand",ce="Replace in files",cd="Perl",J="Ctrl+P",K="Topics",H="Panes",I="Justified",N="Ctrl+X",O="Show macros",L="Change selection: ",M="Show ruler",F="Enabled",G="Copy",s=" = ",r="Paste",u="_pasteCommand",t="Execute command: ",o="icon/16/actions/format-justify-right.png",n="Ctrl+Z",q="Ctrl+R",p="icon/16/actions/edit-redo.png",m="Show tags",l="Info...",T="Execute button: ",U="Format",V="File",W="Change checked: ",P="Show tabs",Q="value",R="Print",S="icon/16/actions/document-save.png",X="icon/16/actions/edit-copy.png",Y="Search...",C="icon/16/actions/format-justify-center.png",B="Open",A="_cutCommand",z="Objective C",y="Exit",x="Ctrl+S",w="Syntax",v="Ctrl+C",E="icon/16/actions/document-new.png",D="Tabs to spaces",ba="ASCII table",bb="Replace",bc="HTML",bd="icon/16/actions/system-search.png",be="changeValue",bf="execute";qx.Class.define(h,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var ci=new qx.ui.container.Scroll();var ch=new qx.ui.container.Composite(new qx.ui.layout.Canvas);ch.setPadding(20);ch.setAllowStretchX(false);ci.add(ch);this.getRoot().add(ci,{edge:0});this.createCommands();var cf=this.getMenuBar();ch.add(cf,{left:20,top:50});var cg=new qx.ui.form.ToggleButton(F);cg.setValue(true);ch.add(cg,{left:20,top:10});cg.bind(Q,cf,bm);}
,debugRadio:function(e){this.debug(L+e.getData()[0].getLabel());}
,debugCommand:function(e){this.debug(t+this.getShortcut());}
,debugButton:function(e){this.debug(T+this.getLabel());}
,debugCheckBox:function(e){this.debug(W+this.getLabel()+s+e.getData());}
,createCommands:function(){this._newCommand=new qx.ui.core.Command(bg);this._newCommand.addListener(bf,this.debugCommand);this._openCommand=new qx.ui.core.Command(bv);this._openCommand.addListener(bf,this.debugCommand);this._saveCommand=new qx.ui.core.Command(x);this._saveCommand.addListener(bf,this.debugCommand);this._undoCommand=new qx.ui.core.Command(n);this._undoCommand.addListener(bf,this.debugCommand);this._redoCommand=new qx.ui.core.Command(q);this._redoCommand.addListener(bf,this.debugCommand);this._cutCommand=new qx.ui.core.Command(N);this._cutCommand.addListener(bf,this.debugCommand);this._copyCommand=new qx.ui.core.Command(v);this._copyCommand.addListener(bf,this.debugCommand);this._pasteCommand=new qx.ui.core.Command(J);this._pasteCommand.addListener(bf,this.debugCommand);this._pasteCommand.setEnabled(false);}
,getMenuBar:function(){var cl=new qx.ui.container.Composite(new qx.ui.layout.Grow);var menubar=new qx.ui.menubar.MenuBar;menubar.setWidth(600);cl.add(menubar);var ck=new qx.ui.menubar.Button(V,null,this.getFileMenu());var cj=new qx.ui.menubar.Button(bs,null,this.getEditMenu());var cm=new qx.ui.menubar.Button(bO,null,this.getSearchMenu());var cp=new qx.ui.menubar.Button(bp,null,this.getViewMenu());var co=new qx.ui.menubar.Button(U,null,this.getFormatMenu());var cn=new qx.ui.menubar.Button(bP,null,this.getHelpMenu());menubar.add(ck);menubar.add(cj);menubar.add(cm);menubar.add(cp);menubar.add(co);menubar.add(cn);return cl;}
,getFileMenu:function(){var cs=new qx.ui.menu.Menu;var cq=new qx.ui.menu.Button(bI,E,this._newCommand);var cu=new qx.ui.menu.Button(B,bx,this._openCommand);var ct=new qx.ui.menu.Button(by);var cx=new qx.ui.menu.Button(k,S,this._saveCommand);var cw=new qx.ui.menu.Button(bl,br);var cv=new qx.ui.menu.Button(R,bu);var cr=new qx.ui.menu.Button(y,bV);cq.addListener(bf,this.debugButton);cu.addListener(bf,this.debugButton);ct.addListener(bf,this.debugButton);cx.addListener(bf,this.debugButton);cw.addListener(bf,this.debugButton);cv.addListener(bf,this.debugButton);cr.addListener(bf,this.debugButton);cs.add(cq);cs.add(cu);cs.add(ct);cs.add(cx);cs.add(cw);cs.add(cv);cs.add(cr);return cs;}
,getEditMenu:function(){var cz=new qx.ui.menu.Menu;var cC=new qx.ui.menu.Button(bC,bz,this._undoCommand);var cB=new qx.ui.menu.Button(bN,p,this._redoCommand);var cy=new qx.ui.menu.Button(bh,bH,this._cutCommand);var cD=new qx.ui.menu.Button(G,X,this._copyCommand);var cA=new qx.ui.menu.Button(r,bQ,this._pasteCommand);cC.addListener(bf,this.debugButton);cB.addListener(bf,this.debugButton);cy.addListener(bf,this.debugButton);cD.addListener(bf,this.debugButton);cA.addListener(bf,this.debugButton);cz.add(cC);cz.add(cB);cz.addSeparator();cz.add(cy);cz.add(cD);cz.add(cA);return cz;}
,getSearchMenu:function(){var cH=new qx.ui.menu.Menu;var cF=new qx.ui.menu.Button(Y,bd);var cK=new qx.ui.menu.Button(ca);var cG=new qx.ui.menu.Button(bj);var cJ=new qx.ui.menu.Button(bb);var cE=new qx.ui.menu.Button(bF,bd);var cI=new qx.ui.menu.Button(ce);cG.setEnabled(false);cF.addListener(bf,this.debugButton);cK.addListener(bf,this.debugButton);cG.addListener(bf,this.debugButton);cJ.addListener(bf,this.debugButton);cE.addListener(bf,this.debugButton);cI.addListener(bf,this.debugButton);cH.add(cF);cH.add(cK);cH.add(cG);cH.add(cJ);cH.addSeparator();cH.add(cE);cH.add(cI);return cH;}
,getViewMenu:function(){var cL=new qx.ui.menu.Menu;var cN=new qx.ui.menu.Button(H,null,null,this.getPanesMenu());var cQ=new qx.ui.menu.Button(w,null,null,this.getSyntaxMenu());var cP=new qx.ui.menu.CheckBox(M);var cO=new qx.ui.menu.CheckBox(bk);var cM=new qx.ui.menu.Button(ba);cP.addListener(be,this.debugCheckBox);cO.addListener(be,this.debugCheckBox);cM.addListener(bf,this.debugButton);cL.add(cN);cL.add(cQ);cL.addSeparator();cL.add(cP);cL.add(cO);cL.addSeparator();cL.add(cM);return cL;}
,getPanesMenu:function(){var cS=new qx.ui.menu.Menu;var cU=new qx.ui.menu.CheckBox(P);var cW=new qx.ui.menu.CheckBox(bK);var cX=new qx.ui.menu.CheckBox(b);var cT=new qx.ui.menu.CheckBox(O);var cV=new qx.ui.menu.CheckBox(m);var cR=new qx.ui.menu.CheckBox(bR);cU.setValue(true);cW.setValue(true);cT.setValue(true);cU.addListener(be,this.debugCheckBox);cW.addListener(be,this.debugCheckBox);cX.addListener(be,this.debugCheckBox);cT.addListener(be,this.debugCheckBox);cV.addListener(be,this.debugCheckBox);cR.addListener(be,this.debugCheckBox);cS.add(cW);cS.add(cU);cS.addSeparator();cS.add(cX);cS.add(cT);cS.add(cV);cS.add(cR);return cS;}
,getSyntaxMenu:function(){var dd=new qx.ui.menu.Menu;var cY=new qx.ui.menu.RadioButton(bc);var db=new qx.ui.menu.RadioButton(j);var da=new qx.ui.menu.RadioButton(g);var dc=new qx.ui.menu.Button(bo,null,null,this.getSyntaxCMenu());var dg=new qx.ui.menu.RadioButton(cd);var de=new qx.ui.menu.RadioButton(bq);dd.add(cY);dd.add(db);dd.add(da);dd.add(dc);dd.add(dg);dd.add(de);var df=new qx.ui.form.RadioGroup;df.add(cY,db,da,dg,de);df.add.apply(df,dc.getMenu().getChildren());df.addListener(bn,this.debugRadio);return dd;}
,getSyntaxCMenu:function(){var di=new qx.ui.menu.Menu;var dh=new qx.ui.menu.RadioButton(bE);var dl=new qx.ui.menu.RadioButton(bw);var dk=new qx.ui.menu.RadioButton(z);var dj=new qx.ui.menu.RadioButton(bU);di.add(dh);di.add(dl);di.add(dk);di.add(dj);return di;}
,getFormatMenu:function(){var dq=new qx.ui.menu.Menu;var dp=new qx.ui.menu.Button(bt,null,null,this.getParagraphMenu());var ds=new qx.ui.menu.Button(D);var dt=new qx.ui.menu.Button(c);var dn=new qx.ui.menu.Button(bA);var dr=new qx.ui.menu.Button(cb);var dm=new qx.ui.menu.Button(bW);var dv=new qx.ui.menu.Button(f);var du=new qx.ui.menu.Button(bS);ds.addListener(bf,this.debugButton);dt.addListener(bf,this.debugButton);dn.addListener(bf,this.debugButton);dr.addListener(bf,this.debugButton);dm.addListener(bf,this.debugButton);dv.addListener(bf,this.debugButton);du.addListener(bf,this.debugButton);dq.add(dp);dq.add(ds);dq.add(dt);dq.addSeparator();dq.add(dn);dq.add(dr);dq.add(dm);dq.addSeparator();dq.add(dv);dq.add(du);return dq;}
,getParagraphMenu:function(){var dw=new qx.ui.menu.Menu;var dz=new qx.ui.menu.Button(bD,bL);var dA=new qx.ui.menu.Button(d,o);var dx=new qx.ui.menu.Button(bT,C);var dy=new qx.ui.menu.Button(I,bG);dz.addListener(bf,this.debugButton);dA.addListener(bf,this.debugButton);dx.addListener(bf,this.debugButton);dy.addListener(bf,this.debugButton);dw.add(dz);dw.add(dA);dw.add(dx);dw.add(dy);return dw;}
,getHelpMenu:function(){var dB=new qx.ui.menu.Menu;var dD=new qx.ui.menu.Button(K,a);var dF=new qx.ui.menu.Button(bi);var dE=new qx.ui.menu.Button(bJ);var dC=new qx.ui.menu.Button(l);dD.addListener(bf,this.debugButton);dF.addListener(bf,this.debugButton);dE.addListener(bf,this.debugButton);dC.addListener(bf,this.debugButton);dB.add(dD);dB.add(dF);dB.addSeparator();dB.add(dE);dB.addSeparator();dB.add(dC);return dB;}
},destruct:function(){this._disposeObjects(bY,bB,cc,bX,bM,A,i,u);}
});}
)();
