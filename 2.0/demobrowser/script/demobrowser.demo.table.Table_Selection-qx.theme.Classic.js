...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Selection","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Selection-qx.theme.Classic.b36e5c957972.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="Boolean",j="demobrowser.demo.table.Table_Selection",i="changeValue",h="icon/22/status/dialog-information.png",g="5 rows added",f="A number",d="icon/16/apps/office-calendar.png",c="Reset Cell Focus",b="changeSelection",a="middle",B="Remove 5 rows",A="Reset Selection",z="_tableModel",y="Table",x="Add 5 rows",w="icon/22/actions/list-remove.png",v=", ",u="icon/22/actions/list-add.png",t="Hide Cell Focus",s="Selected rows:<br>",q="5 rows removed",r="Show selection",o="ID",p="value",m="",n="A date",l="execute";qx.Class.define(j,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return y;}
,createTable:function(){var E=this.createRandomRows(50);var D=this._tableModel=new qx.ui.table.model.Simple();D.setColumns([o,f,n,k]);D.setData(E);D.setColumnEditable(1,true);D.setColumnEditable(2,true);D.setColumnSortable(3,false);var F=new qx.ui.table.Table(D);F.set({width:600,height:400});F.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);var C=F.getTableColumnModel();C.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());C.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(d,n));return F;}
,createControls:function(){var H=new qx.ui.toolbar.ToolBar();var J,L;L=new qx.ui.toolbar.Part();H.add(L);var G=new qx.ui.form.SelectBox().set({allowGrowY:false,alignY:a});var O=qx.ui.table.selection.Model;var I={"Single selection":O.SINGLE_SELECTION,"Single interval":O.SINGLE_INTERVAL_SELECTION,"Multiple intervals":O.MULTIPLE_INTERVAL_SELECTION,"Multiple intervals (toggle)":O.MULTIPLE_INTERVAL_SELECTION_TOGGLE,"No selection":O.NO_SELECTION};for(var N in I){var K=new qx.ui.form.ListItem(N);K.setUserData(p,I[N]+m);G.add(K);}
;G.addListener(b,function(e){this._table.getSelectionModel().setSelectionMode(parseInt(e.getData()[0].getUserData(p)));}
,this);L.add(G);L=new qx.ui.toolbar.Part();H.add(L);J=new qx.ui.toolbar.Button(x,u);J.addListener(l,function(P){var Q=this.createRandomRows(10);this._tableModel.addRows(Q);this.info(g);}
,this);L.add(J);J=new qx.ui.toolbar.Button(B,w);J.addListener(l,function(R){var S=this._tableModel.getRowCount();this._tableModel.removeRows(S-5,5);this.info(q);}
,this);L.add(J);L=new qx.ui.toolbar.Part();H.add(L);J=new qx.ui.toolbar.Button(A);J.addListener(l,function(T){this._table.resetSelection();}
,this);L.add(J);J=new qx.ui.toolbar.Button(c);J.addListener(l,function(U){this._table.resetCellFocus();}
,this);L.add(J);var M=new qx.ui.toolbar.CheckBox(t);M.addListener(i,function(V){this._table.setShowCellFocusIndicator(!M.isValue());}
,this);L.add(M);J=new qx.ui.toolbar.Button(r,h);J.addListener(l,function(W){var X=[];this._table.getSelectionModel().iterateSelection(function(Y){X.push(Y+m);}
);this.showDialog(s+X.join(v));}
,this);L.add(J);return H;}
