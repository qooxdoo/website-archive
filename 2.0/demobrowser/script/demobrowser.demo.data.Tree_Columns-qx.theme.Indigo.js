...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Tree_Columns-qx.theme.Indigo.7c064357012d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="Windows (C:)",j="Documents (D:)",h="demobrowser.demo.data.Tree_Columns",g="Inbox",f="Desktop",e="Personal",d="-rw-r--r--",c="Presets",b="Network",a="Sent",E="children",D="Big",C="Files",B=" 2010",A="Workspace",z="Spam",y="Data",x="May ",w="Root",v="label",r="Lists",s="kb",p="Item ",q="Edit",n="Trash",o="size",l="light",m="mode",t="date",u="checked";qx.Class.define(h,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var G=new qx.ui.container.Scroll();var F=new qx.ui.container.Composite(new qx.ui.layout.Basic());F.setAllowGrowX(false);F.setAllowStretchX(false);G.add(F);this.getRoot().add(G,{edge:0});var H=new qx.ui.tree.Tree().set({width:600,height:500});F.add(H,{left:20,top:48});var J={label:w,children:[{label:f,children:[{label:C},{label:A,children:[{label:k},{label:j}]},{label:b},{label:n}]},{label:g,children:[{label:c},{label:a},{label:n},{label:y},{label:q},{label:r},{label:e},{label:D,children:[]},{label:z}]}]};for(var i=0;i<50;i++){J.children[1].children[7].children[i]={label:p+i};}
;this.extendData(J);var I=qx.data.marshal.Json.createModel(J);var K=new qx.data.controller.Tree(null,H,E,v);K.setDelegate(this);K.setModel(I);H.getRoot().setOpen(true);}
,extendData:function(L){L.date=x+Math.round(Math.random()*30+1)+B;L.size=Math.round(Math.random()*100)+s;L.light=Math.floor(Math.random()*4)==0;L.mode=d;L.checked=Math.random()>=0.5;if(L.children){for(var i=0;i<L.children.length;i++){this.extendData(L.children[i]);}
;}
;}
,bindItem:function(M,N,O){M.bindDefaultProperties(N,O);M.bindProperty(o,o,null,N,O);M.bindProperty(u,u,null,N,O);M.bindPropertyReverse(u,u,null,N,O);M.bindProperty(t,t,null,N,O);M.bindProperty(m,m,null,N,O);M.bindProperty(l,l,null,N,O);}
...
(function(){var u="changeModel",t="_applyDelegate",s="qx.core.Object",r="_applyLabelPath",q="icon",p="label",o="_applyTarget",n="changeTarget",m="_applyIconPath",l="Can not reverse bind the same target property '",c="_applyLabelOptions",k="_applyChildPath",g="_applyIconOptions",b="qx.data.controller.Tree",a="_applyModel",f="Can not bind the same target property '",d="' twice.",h="String";qx.Class.define(b,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(v,w,x,y){qx.core.Object.call(this);this.__bg={};this.__Ap=[];this.__CR={};if(x!=null){this.setChildPath(x);}
;if(y!=null){this.setLabelPath(y);}
;if(v!=null){this.setModel(v);}
;if(w!=null){this.setTarget(w);}
;}
,properties:{model:{check:s,apply:a,event:u,nullable:true,dereference:true},target:{apply:o,event:n,init:null,nullable:true,dereference:true},childPath:{check:h,apply:k,nullable:true},labelPath:{check:h,apply:r,nullable:true},iconPath:{check:h,apply:m,nullable:true},labelOptions:{apply:c,nullable:true},iconOptions:{apply:g,nullable:true},delegate:{apply:t,init:null,nullable:true}},members:{__CR:null,__bg:null,__Ap:null,__CS:null,_applyDelegate:function(z,A){this._setConfigureItem(z,A);this._setCreateItem(z,A);this._setBindItem(z,A);}
,_applyIconOptions:function(B,C){this.__AA();}
,_applyLabelOptions:function(D,E){this.__AA();}
,_applyTarget:function(F,G){if(G!=undefined){this.__Cp(G);}
;if(this.getModel()!=null){this.__CV();}
;this._addChangeTargetListener(F,G);}
,_applyModel:function(H,I){this.__CV();}
,_applyChildPath:function(J,K){this.__CS=K;this.__CV();this.__CS=null;}
,_applyIconPath:function(L,M){this.__AA();}
,_applyLabelPath:function(N,O){this.__CV();}
,__CT:function(P){var R=P.getTarget();var Q=this.__CR[R.toHashCode()].treeNode;var S=this.__CR[R.toHashCode()].modelNode;this.__CW(Q,S);this._updateSelection();}
,__CU:function(e){var ba=e.getData();var V=e.getOldData();var W=this.__CR[V.toHashCode()];V.removeListenerById(W.changeListenerId);delete this.__CR[V.toHashCode()];W.modelNode.removeListenerById(W.changeChildernListenerId);var bb=W.modelNode;var Y=qx.Class.getPropertyDefinition(W.modelNode.constructor,this.getChildPath());var T=Y.event;var X=bb.addListener(T,this.__CU,this);var U=W.treeNode;this.__CR[ba.toHashCode()]={modelNode:bb,treeNode:U,changeListenerId:W.changeListenerId,changeChildernListenerId:X};this.__CW(U,bb);this._updateSelection();}
,_createItem:function(){var bd=this.getDelegate();if(bd!=null&&bd.createItem!=null){var bc=bd.createItem();}
else {var bc=new qx.ui.tree.TreeFolder();}
;if(bd!=null&&bd.configureItem!=null){bd.configureItem(bc);}
;return bc;}
,__CV:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__Cp();if(this.getModel()!=null){var be=this._createItem();be.setModel(this.getModel());this.getTarget().setRoot(be);this.__yE(this.getModel(),be);this.__CW(be,this.getModel());}
;}
,__CW:function(bf,bg){if(bg["get"+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bp=bg["get"+qx.lang.String.firstUp(this.getChildPath())]();if(this.__CR[bp.toHashCode()]==undefined){var bk=bp.addListener("change",this.__CT,this);var bo=qx.Class.getPropertyDefinition(bg.constructor,this.getChildPath());var bl=bo.event;var bm=bg.addListener(bl,this.__CU,this);this.__CR[bp.toHashCode()]={modelNode:bg,treeNode:bf,changeListenerId:bk,changeChildernListenerId:bm};}
;for(var i=0;i<bp.length;i++){if(bf.getChildren()[i]==null||bp.getItem(i)!=bf.getChildren()[i].getModel()){for(var j=i;j<bf.getChildren().length;j++){if(bf.getChildren()[j].getModel()===bp.getItem(i)){var bh=j;break;}
;}
;if(bh!=undefined){var bi=bf.getChildren()[bh];if(this.getTarget().isSelected(bi)){var bq=true;}
;bf.removeAt(bh);bf.addAt(bi,i);if(bq){this.getTarget().addToSelection(bi);}
;}
else {var bn=this._createItem();bn.setModel(bp.getItem(i));bf.addAt(bn,i);this.__yE(bp.getItem(i),bn);this.__CW(bn,bp.getItem(i));}
;}
;}
;for(var i=bf.getChildren().length-1;i>=bp.length;i--){var bj=bf.getChildren()[i];this.__CY(bj,bf);}
;}
,__Cp:function(br){if(br==null){br=this.getTarget();}
;if(br==null){return;}
;var bt=br.getRoot();if(bt!=null){br.setRoot(null);this.__CX(bt);var bs=bt.getModel();if(bs){this.__rb(bs);}
;bt.destroy();this.__CR={};}
;}
,__CX:function(bu){var bv=bu.getChildren()||[];for(var i=bv.length-1;i>=0;i--){if(bv[i].getChildren().length>0){this.__CX(bv[i]);}
;this.__CY(bv[i],bu);}
;}
,__CY:function(bw,bx){var bz=bw.getModel();var bB=this.__CS||this.getChildPath();var bC="get"+qx.lang.String.firstUp(bB);if(bz[bC]!=undefined){var bA=bz[bC]();var by=this.__CR[bA.toHashCode()];bA.removeListenerById(by.changeListenerId);bz.removeListenerById(by.changeChildernListenerId);this.__CX(bw);delete this.__CR[bA.toHashCode()];}
;this.__rb(bz);bx.remove(bw);bw.destroy();}
,bindProperty:function(bD,bE,bF,bG,bH){var bJ=bH.bind(bD,bG,bE,bF);if(this.__bg[bE]==null){this.__bg[bE]={};}
;var bI=this.__bg[bE];if(bI[bH.toHashCode()]){if(bI[bH.toHashCode()].id){throw new Error(f+bE+d);}
;bI[bH.toHashCode()].id=bJ;}
else {bI[bH.toHashCode()]={id:bJ,reverseId:null,treeNode:bG};}
;if(!qx.lang.Array.contains(this.__Ap,bE)){this.__Ap.push(bE);}
;}
,bindPropertyReverse:function(bK,bL,bM,bN,bO){var bQ=bN.bind(bL,bO,bK,bM);if(this.__bg[bL]==null){this.__bg[bL]={};}
;var bP=this.__bg[bL];if(bP[bO.toHashCode()]){if(bP[bO.toHashCode()].reverseId){throw new Error(l+bK+d);}
;bP[bO.toHashCode()].reverseId=bQ;}
else {bP[bO.toHashCode()]={id:null,reverseId:bQ,treeNode:bN};}
;if(!qx.lang.Array.contains(this.__Ap,bL)){this.__Ap.push(bL);}
;}
,bindDefaultProperties:function(bR,bS){this.bindProperty(this.getLabelPath(),p,this.getLabelOptions(),bR,bS);if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),q,this.getIconOptions(),bR,bS);}
;}
,__AA:function(){var bU;for(var bW in this.__bg){bU=bW;break;}
;for(var bT in this.__bg[bU]){var bX=this.__bg[bU][bT].treeNode;var bV=qx.core.ObjectRegistry.fromHashCode(bT);this.__rb(bV);this.__yE(bV,bX);}
;}
,__yE:function(bY,ca){var cb=this.getDelegate();if(cb!=null&&cb.bindItem!=null){cb.bindItem(this,ca,bY);}
else {this.bindDefaultProperties(ca,bY);}
;}
,__rb:function(cc){for(var i=0;i<this.__Ap.length;i++){var ce=this.__Ap[i];var cd=this.__bg[ce][cc.toHashCode()];if(cd!=null){if(cd.id){cc.removeBinding(cd.id);cd.id=null;}
;if(cd.reverseId){cd.treeNode.removeBinding(cd.reverseId);cd.reverseId=null;}
;delete this.__bg[ce][cc.toHashCode()];}
;}
;}
,_setConfigureItem:function(cf,cg){if(cf!=null&&cf.configureItem!=null&&this.getTarget()!=null&&this.getModel()!=null){var ch=this.getTarget().getRoot().getItems(true,true,false);for(var i=0;i<ch.length;i++){cf.configureItem(ch[i]);}
;}
;}
,_setCreateItem:function(ci,cj){if(this.getTarget()==null||this.getModel()==null){return;}
;if(ci==null||ci.createItem==null){return;}
;if(cj&&cj.createItem&&ci&&ci.createItem&&cj.createItem==ci.createItem){return;}
;this._startSelectionModification();this.__Cp();this.__CV();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(ck,cl){if(ck!=null&&ck.bindItem!=null){if(cl!=null&&cl.bindItem!=null&&ck.bindItem==cl.bindItem){return;}
;this.__CV();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__bg=this.__CR=this.__Ap=null;}
});}
)();
...
(function(){var p="icon/16/status/dialog-information.png",o="source",n="size",m="changeSize",l="changeMode",k="changeDate",j="mode",i="changeChecked",h="date",g="changeLight",c="demobrowser.demo.data.TreeColumn",f="icon/16/places/user-desktop.png",e="light",b="",a="checked",d="value";qx.Class.define(c,{extend:qx.ui.tree.TreeFolder,construct:function(){qx.ui.tree.TreeFolder.call(this);var t=new qx.ui.basic.Image();t.setWidth(16);this.addWidget(t);this.addSpacer();this.addOpenButton();this.addIcon();this.setIcon(f);var r=new qx.ui.form.CheckBox();r.setFocusable(false);this.addWidget(r);this.addLabel(b);this.addWidget(new qx.ui.core.Spacer(),{flex:1});var u=new qx.ui.basic.Label();u.setWidth(50);this.addWidget(u);var q=new qx.ui.basic.Label();q.setWidth(150);this.addWidget(q);var s=new qx.ui.basic.Label();s.setWidth(80);this.addWidget(s);this.bind(e,t,o,{converter:function(v){return v?p:b;}
});this.bind(a,r,d);r.bind(d,this,a);this.bind(n,u,d);this.bind(h,q,d);this.bind(j,s,d);}
,properties:{size:{event:m,nullable:true},checked:{event:i,nullable:true},date:{event:k,nullable:true},mode:{event:l,nullable:true},light:{event:g,nullable:true}}});}
)();
