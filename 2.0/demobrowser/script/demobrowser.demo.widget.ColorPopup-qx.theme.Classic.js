...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.ColorPopup","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.ColorPopup-qx.theme.Classic.7e6028a0b6ca.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var p="Selected Color",o="Value Listener: ",n="changeGreen",m="Red Listener: ",l="Open Popup",k="changeBlue",j="en",i="changeValue",h="changeRed",g="Green Listener: ",c="main",f="#23F3C1",d="demobrowser.demo.widget.ColorPopup",b="execute",a="Blue Listener: ";qx.Class.define(d,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var q=this.getRoot();qx.locale.Manager.getInstance().setLocale(j);var s=new qx.ui.control.ColorPopup();s.exclude();s.setValue(f);var t=new qx.ui.form.Button(l);t.addListener(b,function(e){s.placeToWidget(t);s.show();}
);var r=new qx.ui.basic.Label(p).set({marginLeft:10,padding:[3,6],decorator:c});q.add(r,{left:120,top:20});q.add(t,{left:20,top:20});q.add(s,{left:100,top:100});s.addListener(i,function(e){this.debug(o+e.getData());r.setBackgroundColor(e.getData());}
);s.addListener(h,function(e){this.debug(m+e.getData());}
);s.addListener(n,function(e){this.debug(g+e.getData());}
);s.addListener(k,function(e){this.debug(a+e.getData());}
);}
}});}
)();
...
(function(){var m="mousedown",l="teal",k="maroon",h="qx.ui.control.ColorPopup",g="#666",f="__MN",d="changeValue",c="#333",b="#000",a="yellow",W="changeGreen",V="colorpopup",U="_applyValue",T="blue",S="changeRed",R="field#",Q="#CCC",P="Color Selector",O="changeVisibility",N="changeBlue",t="mouseover",u="Cancel",r="#FFF",s="right",p="Open ColorSelector",q="mouseout",n="#999",o="__MM",v="Automatic",w="Basic Colors",D="Preview (Old/New)",B="visible",H="Recent Colors",F="OK",J="field",I="green",y="red",M="preview-pane",L="selector-button",K="colorselector-cancelbutton",x="auto-button",z="colorselector-okbutton",A="Number",C="execute",E="selected-preview",G="current-preview";qx.Class.define(h,{extend:qx.ui.popup.Popup,implement:[qx.ui.form.IColorForm],construct:function(){qx.ui.popup.Popup.call(this);this.setLayout(new qx.ui.layout.VBox(5));this._createChildControl(x);this._createBoxes();this._createChildControl(M);this._createChildControl(L);this.addListener(O,this._onChangeVisibility,this);}
,properties:{appearance:{refine:true,init:V},value:{nullable:true,apply:U,event:d},red:{check:A,init:null,nullable:true,event:S},green:{check:A,init:null,nullable:true,event:W},blue:{check:A,init:null,nullable:true,event:N}},members:{__MK:1e5,__ML:null,__MM:null,__MN:null,__MO:"recent",__MP:12,_createChildControlImpl:function(X,Y){var ba;switch(X){case J:ba=new qx.ui.core.Widget;ba.addListener(m,this._onFieldMouseDown,this);ba.addListener(t,this._onFieldMouseOver,this);ba.addListener(q,this._onFieldMouseOut,this);break;case x:ba=new qx.ui.form.Button(this.tr(v));ba.setAllowStretchX(true);ba.addListener(C,this._onAutomaticBtnExecute,this);this.add(ba);break;case L:ba=new qx.ui.form.Button(this.tr(p));ba.addListener(C,this._onSelectorButtonExecute,this);this.add(ba);break;case M:ba=new qx.ui.groupbox.GroupBox(this.tr(D));ba.setLayout(new qx.ui.layout.HBox);ba.add(this._createChildControl(E,true),{flex:1});ba.add(this._createChildControl(G,true),{flex:1});this.add(ba);break;case E:ba=new qx.ui.container.Composite(new qx.ui.layout.Basic);break;case G:ba=new qx.ui.container.Composite(new qx.ui.layout.Basic);break;case z:ba=new qx.ui.form.Button(this.tr(F));ba.addListener(C,this._onColorSelectorOk,this);break;case K:ba=new qx.ui.form.Button(this.tr(u));ba.addListener(C,this._onColorSelectorCancel,this);break;};return ba||qx.ui.popup.Popup.prototype._createChildControlImpl.call(this,X);}
,_createBoxes:function(){this.__ML={};var bc=this._tables;var bf,bb,bd;var j=0;for(var be in bc){bf=bc[be];bb=new qx.ui.groupbox.GroupBox(bf.label);bb.setLayout(new qx.ui.layout.HBox);this.__ML[be]=bb;this.add(bb);for(var i=0;i<this.__MP;i++){bd=this.getChildControl(R+(j++));bd.setBackgroundColor(bf.values[i]||null);bb.add(bd);}
;}
;}
,_createColorSelector:function(){if(this.__MN){return;}
;var bg=new qx.ui.window.Window(this.tr(P));this.__MM=bg;bg.setLayout(new qx.ui.layout.VBox(16));bg.setResizable(false);bg.moveTo(20,20);this.__MN=new qx.ui.control.ColorSelector;bg.add(this.__MN);var bh=new qx.ui.container.Composite(new qx.ui.layout.HBox(8,s));bg.add(bh);var bj=this._createChildControl(K);var bi=this._createChildControl(z);bh.add(bj);bh.add(bi);}
...
;var bp=this._tables[this.__MO].values;var bq=this.__ML[this.__MO];if(!bp){return;}
;var br=this.getValue();if(!br){return;}
;var bo=bp.indexOf(br);if(bo!=-1){qx.lang.Array.removeAt(bp,bo);}
else if(bp.length==this.__MP){bp.shift();}
...
;this.__MN.setRed(bz);this.__MN.setGreen(by);this.__MN.setBlue(bx);this.__MM.open();}
,_onColorSelectorOk:function(){var bA=this.__MN;this.setValue(qx.util.ColorUtil.rgbToRgbString([bA.getRed(),bA.getGreen(),bA.getBlue()]));this.__MM.close();}
,_onColorSelectorCancel:function(){this.__MM.close();}
...
;}
,_tables:{core:{label:w,values:[b,c,g,n,Q,r,y,I,T,a,l,k]},recent:{label:H,values:[]}}},destruct:function(){this._disposeObjects(o,f);this._tables=this.__ML=null;}
});}
)();
...
(function(){var bz="black",by="_applyGreen",bx="#333",bw="aqua",bv="colorbucket",bu="qx.event.type.Data",bt="Hex",bs="#BBB",br="decoration/colorselector/brightness-handle.gif",bq="Visual",bf="_applySaturation",be="Preview (Old/New)",bd="FFFFFF",bc="decoration/colorselector/brightness-field.png",bb="white",ba="orange",Y="x",X="_applyRed",W="_applyBlue",V="maroon",bG="Presets",bH="_applyBrightness",bE="#999",bF="purple",bC="red",bD="blue",bA="_applyHue",bB="decoration/colorselector/huesaturation-handle.gif",bI="colorselector",bJ="Missing backgroundColor value for field: ",bj="qx.ui.control.ColorSelector",bi="lime",bl="#EEE",bk="olive",bn="RGB",bm="decoration/colorselector/huesaturation-field.jpg",bp="navy",bo="teal",bh="green",bg="yellow",a="#666",b="fuchsia",c="Details",d="",f="colorbucket#",g="appear",h="HSB",k="control-pane",l="preset-grid",m="mouseup",bN="preset-field-set",bM="#",bL="qx.event.type.Event",bK="mousemove",bR="hex-field-composite",bQ="y",bP="rgb-spinner-composite",bO="hsb-spinner-composite",bT="control-bar",bS="mousewheel",F="visual-pane",G="input-field-set",D="preview-field-set",E="brightnessModifier",J="blueModifier",K="saturationModifier",H="middle",I="Number",B="redModifier",C="greenModifier",u="hueModifier",t="Integer",w="brightness-pane",v="rgb-spinner-red",q="preview-content-old",p="rgb-spinner-green",s="brightnessField",r="hue-saturation-field",o="hsb-spinner-brightness",n="preview-content-new",P="hue-saturation-pane",Q="rgb-spinner-blue",R="hsb-spinner-hue",S="hsb-spinner-saturation",L="hex-field",M="mousedown",N="hueSaturationField",O="brightness-field",T="changeValue",U="hexField",A="hsbSpinner",z="rgbSpinner",y="hue-saturation-handle",x="brightness-handle";qx.Class.define(bj,{extend:qx.ui.core.Widget,implement:[qx.ui.form.IColorForm],construct:function(){qx.ui.core.Widget.call(this);this._setLayout(new qx.ui.layout.VBox());this._createChildControl(bT);this.addListener(g,this._onAppear,this);}
,events:{"dialogok":bL,"dialogcancel":bL,"changeValue":bu},properties:{appearance:{refine:true,init:bI},red:{check:t,init:255,apply:X},green:{check:t,init:255,apply:by},blue:{check:t,init:255,apply:W},hue:{check:I,init:0,apply:bA},saturation:{check:I,init:0,apply:bf},brightness:{check:I,init:100,apply:bH}},members:{__Ii:null,__Ij:[V,bC,ba,bg,bk,bF,b,bi,bh,bp,bD,bw,bo,bz,bx,a,bE,bs,bl,bb],__Ik:d,__Il:0,__Im:0,__In:0,__mB:true,__Io:false,_createChildControlImpl:function(bU,bV){var bW;switch(bU){case bT:bW=new qx.ui.container.Composite(new qx.ui.layout.HBox(10));bW.add(this.getChildControl(k));bW.add(this.getChildControl(F));this._add(bW);break;case F:bW=new qx.ui.groupbox.GroupBox(this.tr(bq));bW.setLayout(new qx.ui.layout.HBox(10));bW.add(this.getChildControl(P));bW.add(this.getChildControl(w));break;case k:bW=new qx.ui.container.Composite(new qx.ui.layout.VBox(0));bW.add(this.getChildControl(bN));bW.add(this.getChildControl(G));bW.add(this.getChildControl(D),{flex:1});break;case P:bW=new qx.ui.container.Composite(new qx.ui.layout.Canvas());bW.setAllowGrowY(false);bW.addListener(bS,this._onHueSaturationPaneMouseWheel,this);bW.add(this.getChildControl(r));bW.add(this.getChildControl(y),{left:0,top:256});break;case r:bW=new qx.ui.basic.Image(bm);bW.addListener(M,this._onHueSaturationFieldMouseDown,this);break;case y:bW=new qx.ui.basic.Image(bB);bW.addListener(M,this._onHueSaturationFieldMouseDown,this);bW.addListener(m,this._onHueSaturationHandleMouseUp,this);bW.addListener(bK,this._onHueSaturationHandleMouseMove,this);break;case w:bW=new qx.ui.container.Composite(new qx.ui.layout.Canvas());bW.setAllowGrowY(false);bW.addListener(bS,this._onBrightnessPaneMouseWheel,this);bW.add(this.getChildControl(O));bW.add(this.getChildControl(x));break;case O:bW=new qx.ui.basic.Image(bc);bW.addListener(M,this._onBrightnessFieldMouseDown,this);break;case x:bW=new qx.ui.basic.Image(br);bW.addListener(M,this._onBrightnessHandleMouseDown,this);bW.addListener(m,this._onBrightnessHandleMouseUp,this);bW.addListener(bK,this._onBrightnessHandleMouseMove,this);break;case bN:bW=new qx.ui.groupbox.GroupBox(this.tr(bG));bW.setLayout(new qx.ui.layout.Grow());bW.add(this.getChildControl(l));break;case bv:bW=new qx.ui.core.Widget();bW.addListener(M,this._onColorFieldClick,this);break;case l:cc=new qx.ui.layout.Grid(3,3);bW=new qx.ui.container.Composite(cc);var cd;var cb;for(var i=0;i<2;i++){for(var j=0;j<10;j++){cb=i*10+j;cd=this.getChildControl(f+cb);cd.setBackgroundColor(this.__Ij[cb]);bW.add(cd,{column:j,row:i});}
;}
;break;case G:bW=new qx.ui.groupbox.GroupBox(this.tr(c));var cc=new qx.ui.layout.VBox();cc.setSpacing(10);bW.setLayout(cc);bW.add(this.getChildControl(bR));bW.add(this.getChildControl(bP));bW.add(this.getChildControl(bO));break;case D:bW=new qx.ui.groupbox.GroupBox(this.tr(be));var cc=new qx.ui.layout.HBox(10);bW.setLayout(cc);bW.add(this.getChildControl(q),{flex:1});bW.add(this.getChildControl(n),{flex:1});break;case bR:var ca=new qx.ui.layout.HBox(4);ca.setAlignY(H);bW=new qx.ui.container.Composite(ca);var cf=new qx.ui.basic.Label(this.tr(bt));cf.setWidth(30);bW.add(cf);var ce=new qx.ui.basic.Label(bM);bW.add(ce);bW.add(this.getChildControl(L));break;case L:bW=new qx.ui.form.TextField(bd);bW.setMaxLength(6);bW.setFilter(/[0-9A-Fa-f]/);bW.setWidth(55);bW.addListener(T,this._onHexFieldChange,this);break;case bP:var ca=new qx.ui.layout.HBox(4);ca.setAlignY(H);bW=new qx.ui.container.Composite(ca);var bX=new qx.ui.basic.Label(this.tr(bn));bX.setWidth(30);bW.add(bX);bW.add(this.getChildControl(v));bW.add(this.getChildControl(p));bW.add(this.getChildControl(Q));break;case v:bW=new qx.ui.form.Spinner(0,255,255);bW.setWidth(50);bW.addListener(T,this._setRedFromSpinner,this);break;case p:bW=new qx.ui.form.Spinner(0,255,255);bW.setWidth(50);bW.addListener(T,this._setGreenFromSpinner,this);break;case Q:bW=new qx.ui.form.Spinner(0,255,255);bW.setWidth(50);bW.addListener(T,this._setBlueFromSpinner,this);break;case bO:var ca=new qx.ui.layout.HBox(4);ca.setAlignY(H);bW=new qx.ui.container.Composite(ca);var bY=new qx.ui.basic.Label(this.tr(h));bY.setWidth(30);bW.add(bY);bW.add(this.getChildControl(R));bW.add(this.getChildControl(S));bW.add(this.getChildControl(o));break;case R:bW=new qx.ui.form.Spinner(0,0,360);bW.setWidth(50);bW.addListener(T,this._setHueFromSpinner,this);break;case S:bW=new qx.ui.form.Spinner(0,0,100);bW.setWidth(50);bW.addListener(T,this._setSaturationFromSpinner,this);break;case o:bW=new qx.ui.form.Spinner(0,100,100);bW.setWidth(50);bW.addListener(T,this._setBrightnessFromSpinner,this);break;case q:bW=new qx.ui.core.Widget();break;case n:bW=new qx.ui.core.Widget();break;};return bW||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,bU);}
,setValue:function(cg){var ch;if(cg==null){this.__mB=true;ch=[255,255,255];}
else {ch=qx.util.ColorUtil.stringToRgb(cg);this.__mB=false;}
;this.__Io=true;this.setRed(ch[0]);this.setGreen(ch[1]);this.__Io=false;this.setBlue(ch[2]);}
,getValue:function(){return this.__mB?null:qx.util.ColorUtil.rgbToHexString([this.getRed(),this.getGreen(),this.getBlue()]);}
,resetValue:function(){this.__mB=true;this.__Io=true;this.setRed(255);this.setGreen(255);this.__Io=false;this.setBlue(255);}
,__mE:function(){if(!this.__Io){this.__mB=false;this.fireDataEvent(T,this.getValue());}
;}
,_applyRed:function(ci,cj){if(this.__Ii===null){this.__Ii=B;}
;if(this.__Ii!==z){this.getChildControl(v).setValue(ci);}
;if(this.__Ii!==U){this._setHexFromRgb();}
;switch(this.__Ii){case z:case U:case B:this._setHueFromRgb();};this._setPreviewFromRgb();this.__mE();if(this.__Ii===B){this.__Ii=null;}
;}
,_applyGreen:function(ck,cl){if(this.__Ii===null){this.__Ii=C;}
;if(this.__Ii!==z){this.getChildControl(p).setValue(ck);}
;if(this.__Ii!==U){this._setHexFromRgb();}
;switch(this.__Ii){case z:case U:case C:this._setHueFromRgb();};this._setPreviewFromRgb();this.__mE();if(this.__Ii===C){this.__Ii=null;}
;}
,_applyBlue:function(cm,cn){if(this.__Ii===null){this.__Ii=J;}
;if(this.__Ii!==z){this.getChildControl(Q).setValue(cm);}
;if(this.__Ii!==U){this._setHexFromRgb();}
;switch(this.__Ii){case z:case U:case J:this._setHueFromRgb();};this._setPreviewFromRgb();this.__mE();if(this.__Ii===J){this.__Ii=null;}
;}
,_applyHue:function(co,cp){if(this.__Ii===null){this.__Ii=u;}
;if(this.__Ii!==A){this.getChildControl(R).setValue(co);}
;if(this.__Ii!==N){if(this.getChildControl(y).getBounds()){this.getChildControl(y).setDomLeft(Math.round(co/1.40625)+this.getChildControl(P).getPaddingLeft());}
else {this.getChildControl(y).setLayoutProperties({left:Math.round(co/1.40625)});}
;}
;switch(this.__Ii){case A:case N:case u:this._setRgbFromHue();};this._setBrightnessGradiant();if(this.__Ii===u){this.__Ii=null;}
;}
,_applySaturation:function(cq,cr){if(this.__Ii===null){this.__Ii=K;}
;if(this.__Ii!==A){this.getChildControl(S).setValue(cq);}
;if(this.__Ii!==N){this._setBrightnessGradiant();if(this.getChildControl(y).getBounds()){this.getChildControl(y).setDomTop(256-Math.round(cq*2.56)+this.getChildControl(P).getPaddingTop());}
else {this.getChildControl(y).setLayoutProperties({top:256-Math.round(cq*2.56)});}
;}
;switch(this.__Ii){case A:case N:case K:this._setRgbFromHue();};if(this.__Ii===K){this.__Ii=null;}
;}
,_applyBrightness:function(cs,ct){if(this.__Ii===null){this.__Ii=E;}
;if(this.__Ii!==A){this.getChildControl(o).setValue(cs);}
;if(this.__Ii!==s){var cu=256-Math.round(cs*2.56);if(this.getChildControl(x).getBounds()){this.getChildControl(x).setDomTop(cu+this.getChildControl(w).getPaddingTop());}
else {this.getChildControl(x).setLayoutProperties({top:cu});}
;}
;switch(this.__Ii){case A:case s:case E:this._setRgbFromHue();};if(this.__Ii===E){this.__Ii=null;}
;}
,_onBrightnessHandleMouseDown:function(e){this.getChildControl(x).capture();this.__Ik=x;var cx=this.getChildControl(O).getContainerLocation();var cw=this.getChildControl(x).getContainerLocation();var cv=this.getChildControl(O).getBounds();this.__Il=cx.top+(e.getDocumentTop()-cw.top)-cv.top;e.stopPropagation();}
,_onBrightnessHandleMouseUp:function(e){this.getChildControl(x).releaseCapture();this.__Ik=null;}
,_onBrightnessHandleMouseMove:function(e){if(this.__Ik===x){this._setBrightnessOnFieldEvent(e);e.stopPropagation();}
;}
,_onBrightnessFieldMouseDown:function(e){var location=this.getChildControl(O).getContainerLocation();var cy=this.getChildControl(x).getBounds();this.__Il=location.top+(cy.height/2);this._setBrightnessOnFieldEvent(e);this.getChildControl(x).capture();this.__Ik=x;}
...
,_setBrightnessOnFieldEvent:function(e){var cz=qx.lang.Number.limit(e.getDocumentTop()-this.__Il,0,256);this.__Ii=s;if(this.getChildControl(x).getBounds()){this.getChildControl(x).setDomTop(cz);}
else {this.getChildControl(x).setLayoutProperties({top:cz});}
;this.setBrightness(100-Math.round(cz/2.56));this.__Ii=null;}
,_onHueSaturationHandleMouseUp:function(e){if(this.__Ik){e.stopPropagation();this.getChildControl(y).releaseCapture();this.__Ik=null;}
;}
,_onHueSaturationHandleMouseMove:function(e){if(this.__Ik===y){this._setHueSaturationOnFieldEvent(e);e.stopPropagation();}
;}
,_onHueSaturationFieldMouseDown:function(e){var location=this.getChildControl(r).getContainerLocation();var cA=this.getChildControl(y).getBounds();var cB=this.getChildControl(r).getBounds();this.__Im=location.top+(cA.height/2)-cB.top;this.__In=location.left+(cA.width/2)-cB.left;this._setHueSaturationOnFieldEvent(e);this.getChildControl(y).capture();this.__Ik=y;}
,_onHueSaturationPaneMouseWheel:function(e){this.setSaturation(qx.lang.Number.limit(this.getSaturation()-e.getWheelDelta(bQ),0,100));this.setHue(qx.lang.Number.limit(this.getHue()+e.getWheelDelta(Y),0,360));e.stop();}
,_setHueSaturationOnFieldEvent:function(e){var cD=qx.lang.Number.limit(e.getDocumentTop()-this.__Im,0,256);var cC=qx.lang.Number.limit(e.getDocumentLeft()-this.__In,0,256);this.getChildControl(y).setDomPosition(cC,cD);this.__Ii=N;this.setSaturation(100-Math.round(cD/2.56));this.setHue(Math.round(cC*1.40625));this.__Ii=null;}
,_setRedFromSpinner:function(){if(this.__Ii!==null){return;}
;this.__Ii=z;this.setRed(this.getChildControl(v).getValue());this.__Ii=null;}
,_setGreenFromSpinner:function(){if(this.__Ii!==null){return;}
;this.__Ii=z;this.setGreen(this.getChildControl(p).getValue());this.__Ii=null;}
,_setBlueFromSpinner:function(){if(this.__Ii!==null){return;}
;this.__Ii=z;this.setBlue(this.getChildControl(Q).getValue());this.__Ii=null;}
,_setHueFromSpinner:function(){if(this.__Ii!==null){return;}
;this.__Ii=A;this.setHue(this.getChildControl(R).getValue());this.__Ii=null;}
,_setSaturationFromSpinner:function(){if(this.__Ii!==null){return;}
;this.__Ii=A;this.setSaturation(this.getChildControl(S).getValue());this.__Ii=null;}
,_setBrightnessFromSpinner:function(){if(this.__Ii!==null){return;}
;this.__Ii=A;this.setBrightness(this.getChildControl(o).getValue());this.__Ii=null;}
,_onHexFieldChange:function(e){if(this.__Ii!==null){return;}
;try{var cF=this.getChildControl(L);var cE=qx.util.ColorUtil.hexStringToRgb(bM+cF.getValue());}
catch(cG){return;}
;this.__Ii=U;this.setRed(cE[0]);this.setGreen(cE[1]);this.setBlue(cE[2]);this.__Ii=null;}
,_setHexFromRgb:function(){var cH=qx.util.ColorUtil.rgbToHexString([this.getRed(),this.getGreen(),this.getBlue()]);cH=cH.substring(1,cH.length);this.getChildControl(L).setValue(cH);}
,_onColorFieldClick:function(e){var cI=e.getTarget().getBackgroundColor();if(!cI){return this.error(bJ+e.getTarget());}
;var cJ=qx.util.ColorUtil.stringToRgb(cI);this.setRed(cJ[0]);this.setGreen(cJ[1]);this.setBlue(cJ[2]);}
,_setHueFromRgb:function(){switch(this.__Ii){case A:case N:case s:break;default:var cK=qx.util.ColorUtil.rgbToHsb([this.getRed(),this.getGreen(),this.getBlue()]);this.setHue(cK[0]);this.setSaturation(cK[1]);this.setBrightness(cK[2]);};}
,_setRgbFromHue:function(){switch(this.__Ii){case z:case U:break;default:var cL=qx.util.ColorUtil.hsbToRgb([this.getHue(),this.getSaturation(),this.getBrightness()]);this.setRed(cL[0]);this.setGreen(cL[1]);this.setBlue(cL[2]);};}
,_setPreviewFromRgb:function(){var cM=qx.util.ColorUtil.rgbToRgbString([this.getRed(),this.getGreen(),this.getBlue()]);this.getChildControl(n).setBackgroundColor(cM);}
,setPreviousColor:function(cN,cO,cP){var cQ=qx.util.ColorUtil.rgbToRgbString([cN,cO,cP]);this.getChildControl(q).setBackgroundColor(cQ);this.setRed(cN);this.setGreen(cO);this.setBlue(cP);}
,_setBrightnessGradiant:function(){var cR=qx.util.ColorUtil;var cS=cR.hsbToRgb([this.getHue(),this.getSaturation(),255]);var cT=cR.rgbToRgbString(cS);this.getChildControl(O).setBackgroundColor(cT);}
,_onAppear:function(e){var cU=qx.util.ColorUtil.rgbToRgbString([this.getRed(),this.getGreen(),this.getBlue()]);this.getChildControl(q).setBackgroundColor(cU);this.getChildControl(n).setBackgroundColor(cU);}
}});}
)();
