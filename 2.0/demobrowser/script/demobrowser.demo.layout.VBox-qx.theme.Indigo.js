...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.layout.VBox","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.VBox-qx.theme.Indigo.788a8567bc20.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,__Cw:{north:1,south:2,west:3,east:4,center:5},__Cx:{'1':o,'2':g,'3':l,'4':A},__kQ:function(){var H=this._getLayoutChildren();var P,J;var length=H.length;var L=[];var O=[];var M=[];var I=this.getSort()===v;var N=this.getSort()===n;for(var i=0;i<length;i++){P=H[i];M=P.getLayoutProperties().edge;if(M===w){if(J){throw new Error(j);}
;J=P;}
...
;this.__dW=Q;var K=[];for(var i=0;i<length;i++){M=Q[i].getLayoutProperties().edge;K[i]=this.__Cw[M]||5;}
;this.__Bm=K;delete this._invalidChildrenCache;}
,renderLayout:function(R,S){if(this._invalidChildrenCache){this.__kQ();}
;var bm=qx.ui.layout.Util;var Y=this.__dW;var bn=this.__Bm;var length=Y.length;var bc,U,bb,bh,bi,bs,bf,V,bp;var bw=[];var be=[];var bv=this._getSeparatorWidths();var bA=this.getSpacingX();var bz=this.getSpacingY();var ba=-bA;var bo=-bz;if(bv.x){ba-=bv.x+bA;}
;if(bv.y){bo-=bv.y+bz;}
...
;}
;this._clearSeparators();var bt=this.getSeparatorX(),bu=this.getSeparatorY();var bx=this.getConnectSeparators();var bk=0,br=0;var bE,top,bf,V,bj,bB;var bl,by,bD,T;var bC,bd,bg,X;var bq=this.__Cx;for(var i=0;i<length;i++){U=Y[i];bB=bn[i];bb=U.getSizeHint();bC=U.getMarginTop();bd=U.getMarginBottom();bg=U.getMarginLeft();X=U.getMarginRight();switch(bB){case 1:case 2:bf=R-bg-X;if(bf<bb.minWidth){bf=bb.minWidth;}
else if(bf>bb.maxWidth){bf=bb.maxWidth;}
...
;var bR=this.__dW;var cc=this.__Bm;var length=bR.length;var bV,cd;var bX,bW;var bY=0,ck=0;var bO=0,cf=0;var ca=0,cj=0;var bP=0,ce=0;var cg=this._getSeparatorWidths();var ci=this.getSpacingX(),ch=this.getSpacingY();var bT=-ci,bS=-ch;if(cg.x){bT-=cg.x+ci;}
;if(cg.y){bS-=cg.y+ch;}
...
},destruct:function(){this.__Bm=this.__dW=null;}
});}
)();
...
,_applyValue:function(bf,bg){var bh=this.getChildControl(z);this._updateButtons();this.__uL=bf;if(bf!==null){if(this.getNumberFormat()){bh.setValue(this.getNumberFormat().format(bf));}
...
,_applyNumberFormat:function(bn,bo){var bp=this.getChildControl(z);bp.setFilter(this._getFilterRegExp());this.getNumberFormat().addListener(E,this._onChangeNumberFormat,this);this._applyValue(this.__uL,undefined);}
...
;}
;}
,_onKeyDown:function(e){switch(e.getKeyIdentifier()){case r:this.__uM=true;case n:this.getChildControl(y).press();break;case l:this.__uN=true;case q:this.getChildControl(v).press();break;default:return;};e.stopPropagation();e.preventDefault();}
,_onKeyUp:function(e){switch(e.getKeyIdentifier()){case r:this.getChildControl(y).release();this.__uM=false;break;case n:this.getChildControl(y).release();break;case l:this.getChildControl(v).release();this.__uN=false;break;case q:this.getChildControl(v).release();break;};}
...
;this.setValue(bv);}
else {this._applyValue(this.__uL,undefined);}
;}
...
,_countUp:function(){if(this.__uM){var bC=this.getValue()+this.getPageStep();}
...
;}
;this.gotoValue(bC);}
,_countDown:function(){if(this.__uN){var bE=this.getValue()-this.getPageStep();}
