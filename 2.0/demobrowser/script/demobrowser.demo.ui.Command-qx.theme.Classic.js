...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.Command","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.Command-qx.theme.Classic.75176154e164.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var q="Saving...",p="demobrowser.demo.ui.Command",o="Save",n="  Button in the content at the bottom\n",m="label",l="icon",k="Editor",j="Save the current file.",i="  Button in the menu\n",h="icon/16/actions/document-save.png",c="  Button in the toolbar\n",g="CTRL+S",f="and have the same execute listener.",b="File",a="You can see three buttons in this window:\n",e="execute",d="Each of the buttons is configured using the same command ";qx.Class.define(p,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var v=new qx.ui.core.Command(g);v.setLabel(o);v.setIcon(h);v.setToolTipText(j);v.addListener(e,function(){alert(q);}
,this);var u=new qx.ui.window.Window(k);u.setLayout(new qx.ui.layout.VBox(0));u.setWidth(400);u.setHeight(300);u.setContentPadding(0);u.moveTo(30,10);u.open();var menubar=new qx.ui.menubar.MenuBar();u.add(menubar);var t=new qx.ui.menubar.Button(b);menubar.add(t);var w=new qx.ui.menu.Menu();t.setMenu(w);var r=new qx.ui.menu.Button();r.setCommand(v);w.add(r);var toolbar=new qx.ui.toolbar.ToolBar();u.add(toolbar);var z=new qx.ui.toolbar.Button();z.setCommand(v);z.setShow(l);toolbar.add(z);var content=new qx.ui.container.Composite();content.setLayout(new qx.ui.layout.VBox());u.add(content,{flex:1});var s=new qx.ui.form.TextArea();content.add(s,{flex:1});var x=new qx.ui.form.Button();x.setCommand(v);x.setShow(m);content.add(x);var y=a+i+c+n+d+f;s.setValue(y);}
}});}
)();
...
(function(){var b="qx.ui.menubar.MenuBar",a="menubar";qx.Class.define(b,{extend:qx.ui.toolbar.ToolBar,properties:{appearance:{refine:true,init:a}}});}
)();
...
,properties:{wrap:{check:x,init:true,apply:j},appearance:{refine:true,init:n},singleStep:{check:w,init:20},minimalLineHeight:{check:w,apply:h,init:4},autoSize:{check:x,apply:E,init:false}},members:{__uz:null,__uA:null,__uB:null,setValue:function(G){G=qx.ui.form.AbstractField.prototype.setValue.call(this,G);this.__uC();return G;}
...
;}
,__uC:function(){if(this.isAutoSize()){var L=this.__uD();if(L){this.__uB=this.__uB||this._getAreaHeight();var N=this._getScrolledAreaHeight();if(this.getMaxHeight()){var J=this.getInsets();var K=-J.top+this.getMaxHeight()-J.bottom;if(N>K){this.getContentElement().setStyle(u,l);}
else {this.getContentElement().setStyle(u,t);}
;}
;var M=Math.max(N,this.__uB);this._setAreaHeight(M);}
else {this.getContentElement().addListenerOnce(r,function(){this.__uC();}
,this);}
;}
;}
,_getAreaHeight:function(){return this.getInnerSize().height;}
,_setAreaHeight:function(O){if(this._getAreaHeight()!==O){this.__uA=O;qx.ui.core.queue.Layout.add(this);qx.ui.core.queue.Manager.flush();this.__uG();}
;}
,_getScrolledAreaHeight:function(){var Q=this.__uD();var P=Q.getDomElement();if(P){if(!P.parentNode){qx.html.Element.flush();return this._getScrolledAreaHeight();}
...
;Q.setValue(this.getValue());this.__uF(Q);if(qx.core.Environment.get(m)==s){if(!P.scrollTop){qx.html.Element.flush();}
;return P.scrollTop+this._getTextSize().height;}
;return P.scrollTop;}
;}
,__uD:function(){this.__uz=this.__uz||this.__uE();return this.__uz;}
,__uE:function(){var U,S,R,T;U=this.getContentElement();if(!U.getDomElement()){return;}
;R=qx.bom.Collection.create(U.getDomElement()).clone()[0];T=new qx.html.Input(n);T.useElement(R);S=T;S.setStyles({position:y,top:0,left:-9999,height:0,overflow:t},true);S.removeAttribute(A);S.removeAttribute(C);S.setAttribute(D,i);S.setValue(U.getValue());S.insertBefore(U);this.__uF(S);return S;}
,__uF:function(V){V=V.getDomElement();if(V){V.scrollTop=10000;}
;}
,_createInputElement:function(){return new qx.html.Input(n,{overflowX:l,overflowY:l});}
,_applyWrap:function(W,X){this.getContentElement().setWrap(W);this.__uC();}
,_applyMinimalLineHeight:function(){qx.ui.core.queue.Layout.add(this);}
,_applyAutoSize:function(Y,ba){if(qx.core.Environment.get(q)){this.__uH();}
;if(Y){this.__uC();this.addListener(v,this.__uC,this);this.addListenerOnce(r,function(){this.getContentElement().setStyle(u,t);}
);}
else {this.removeListener(v,this.__uC);this.getContentElement().setStyle(u,l);}
;}
,_applyDimension:function(bb){qx.ui.form.AbstractField.prototype._applyDimension.call(this);if(qx.core.Environment.get(q)){this.__uH();}
;if(bb===this.getMaxHeight()){this.__uC();}
;}
,__uG:function(){var content=this.getContentElement();var bd=content.getDomElement();var bc=content.getStyle(o);content.setStyle(o,parseInt(bc,10)+1000+k,true);if(bd){qx.bom.element.Dimension.getWidth(bd);}
;content.setStyle(o,bc,true);}
,__uH:function(){if(this.isAutoSize()&&this.getHeight()){this.warn(f+z+a);}
;}
,_getContentHint:function(){var be=qx.ui.form.AbstractField.prototype._getContentHint.call(this);be.height=be.height*this.getMinimalLineHeight();be.width=this._getTextSize().width*20;if(this.isAutoSize()){be.height=this.__uA||be.height;}
;return be;}
}});}
)();
