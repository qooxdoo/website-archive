...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Environment-qx.theme.Modern.0fccb553cfc3.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="qx.bom.client.Flash",j="ShockwaveFlash.ShockwaveFlash.7",h="plugin.flash.express",g="plugin.flash.version",f="plugin.flash",e="osx",d="10.0.12",c="win",b="ShockwaveFlash.ShockwaveFlash.6",a="$version",x="plugin.flash.strictsecurity",w="6.0.65",v="always",u="9.0.151",t=" ",s="ShockwaveFlash.ShockwaveFlash",r=",",q="object",p="Shockwave Flash",o="undefined",m="",n=".",l="$1";qx.Bootstrap.define(k,{statics:{isAvailable:function(){return parseFloat(qx.bom.client.Flash.getVersion())>0;}
,getVersion:function(){if(navigator.plugins&&typeof navigator.plugins[p]===q){var A=[0,0,0];var C=navigator.plugins[p].description;if(typeof C!=o){C=C.replace(/^.*\s+(\S+\s+\S+$)/,l);A[0]=parseInt(C.replace(/^(.*)\..*$/,l),10);A[1]=parseInt(C.replace(/^.*\.(.*)\s.*$/,l),10);A[2]=/r/.test(C)?parseInt(C.replace(/^.*r(.*)$/,l),10):0;}
;return A.join(n);}
...
;var G=qx.bom.client.OperatingSystem.getName();return (G==c||G==e)&&qx.bom.client.Flash.__IY(w,H);}
,getStrictSecurityModel:function(){var I=qx.bom.client.Flash.getVersion();if(I==m){return false;}
;var J=I.split(n);if(J[0]<10){return qx.bom.client.Flash.__IY(u,I);}
else {return qx.bom.client.Flash.__IY(d,I);}
;}
,_cachedSupportsVersion:{},__IY:function(K,L){var O=K.split(".");var N=L||qx.bom.client.Flash.getVersion();N=N.split(".");for(var i=0;i<O.length;i++){var M=parseInt(N[i],10)-parseInt(O[i],10);if(M>0){return true;}
