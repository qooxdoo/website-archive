...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.virtual.Cells","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Cells-qx.theme.Modern.314f5dbab2f1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j="testImageCell",i="qx.theme",h="</div>",g="<div ",f="testNumberCell",e="class='",d="width:",c="testHtmlCell",b="testDateCell",a="css.boxmodel",F=">",E="height:",D="testStringCell",C="testBooleanCellCustomImage",B="demobrowser.demo.virtual.Cells",A="content",z="testBooleanCell",y="foobar!",x="qx.theme.Modern",w="icon/32/apps/utilities-notes.png",q="medium",r="main",o="icon/16/emotes/face-sad.png",p="icon/16/apps/internet-feed-reader.png",m="icon/16/emotes/face-smile.png",n="This is a feed reader!",k="style='",l="' ",s="<b>HTML</b>",t="px;",v="Juhu",u="";qx.Class.define(B,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var G=new qx.ui.layout.HBox(5);this.topContainer=new qx.ui.container.Composite(G);this.runTest(D);this.runTest(f);this.runTest(b);this.runTest(c);if(qx.core.Environment.get(i)==x){this.runTest(j);this.runTest(z);this.runTest(C);}
...
,testStringCell:function(){var J={value:[v,s,u,null],states:[{},{selected:1}]};var I=new qx.ui.virtual.cell.String;qx.util.Permutation.permute(J,function(K){var L=this.__Lo(I,K.value,K.states);this.container.add(L);}
,this);}
,testNumberCell:function(){var M={value:[-1.666666,0,null],format:[null,new qx.util.format.NumberFormat().set({maximumFractionDigits:2})],states:[{},{selected:1}]};var N=new qx.ui.virtual.cell.Number();qx.util.Permutation.permute(M,function(O){if(O.format){N.setNumberFormat(O.format);}
else {N.resetNumberFormat();}
;var P=this.__Lo(N,O.value,O.states);this.container.add(P);}
,this);}
,testDateCell:function(){var Q={value:[new Date(),null],format:[null,new qx.util.format.DateFormat(qx.locale.Date.getDateFormat(q))],states:[{},{selected:1}]};var R=new qx.ui.virtual.cell.Date();qx.util.Permutation.permute(Q,function(S){if(S.format){R.setDateFormat(S.format);}
else {R.resetDateFormat();}
;var T=this.__Lo(R,S.value,S.states);this.container.add(T);}
,this);}
,testHtmlCell:function(){var U={value:[v,s,u,null],states:[{},{selected:1}]};var V=new qx.ui.virtual.cell.Html;qx.util.Permutation.permute(U,function(W){var X=this.__Lo(V,W.value,W.states);this.container.add(X);}
,this);}
,testImageCell:function(){var Y={value:[{url:p,tooltip:n},{url:w,tooltip:y},null],states:[{},{selected:1}]};var ba=new qx.ui.virtual.cell.Image;qx.util.Permutation.permute(Y,function(bb){var bc=this.__Lo(ba,bb.value,bb.states);this.container.add(bc);}
,this);}
,testBooleanCell:function(){var bd={value:[true,false,null],states:[{},{selected:1}]};var be=new qx.ui.virtual.cell.Boolean;qx.util.Permutation.permute(bd,function(bf){var bg=this.__Lo(be,bf.value,bf.states);this.container.add(bg);}
,this);}
,testBooleanCellCustomImage:function(){var bh={value:[true,false,null],states:[{},{selected:1}]};var bi=new qx.ui.virtual.cell.Boolean;bi.setIconFalse(o);bi.setIconTrue(m);qx.util.Permutation.permute(bh,function(bj){var bk=this.__Lo(bi,bj.value,bj.states);this.container.add(bk);}
,this);}
,__Lo:function(bl,bm,bn){var br=100;var bp=30;var bt=new qx.ui.embed.Html().set({width:br,height:bp,decorator:bn.selected?null:r});var bo=bl.getCellProperties(bm,bn);var bq=bo.insets;var bs=[g,k,this._getCellSizeStyle(br,bp,bq[0],bq[1]),bo.style,l,e,bo.classes,l,bo.attributes,F,bo.content,h].join(u);bt.setHtml(bs);return bt;}
,_getCellSizeStyle:function(bu,bv,bw,bx){var by=u;if(qx.core.Environment.get(a)==A){bu-=bw;bv-=bx;}
;by+=d+bu+t;by+=E+bv+t;return by;}
}});}
)();
...
(function(){var j="qx.ui.virtual.cell.AbstractImage",i="engine.version",h="_identifyImage is abstract",g="string",f="static",e='"',d='<div style="',c='/>',b='<img src="',a='title="',w='></div>',v="img",u="top",t="gecko",s="engine.name",r='" style="',q="abstract",p="inline-block",o="-moz-inline-box",n="px",l='" ',m="no-repeat",k="";qx.Class.define(j,{extend:qx.ui.virtual.cell.Cell,type:q,construct:function(){qx.ui.virtual.cell.Cell.call(this);this._aliasManager=qx.util.AliasManager.getInstance();}
,members:{__qm:16,__qn:16,_aliasManager:null,__qp:function(x){var A=qx.util.ResourceManager.getInstance();var z=qx.io.ImageLoader;var y,B;if(A.has(x)){y=A.getImageWidth(x),B=A.getImageHeight(x);}
else if(z.isLoaded(x)){y=z.getWidth(x);B=z.getHeight(x);}
else {y=this.__qm;B=this.__qn;}
;return {width:y,height:B};}
,__Jy:function(C){if(typeof (C)==g){C={url:C};}
;var D=this._aliasManager.resolve(C.url||null);var E;if(C.width&&C.height){E={width:C.width,height:C.height};}
else {E=this.__qp(D);}
;return {width:E.width,height:E.height,url:D,tooltip:C.tooltip};}
,_identifyImage:function(F){throw new Error(h);}
,getContent:function(G,H){if(G===null){return k;}
;var content=k;var J=this.__Jy(this._identifyImage(G));var N=qx.core.Environment.get(s)==t&&qx.core.Environment.get(i)<1.9;var K=J.tooltip?a+J.tooltip+e:k;var I={width:J.width+n,height:J.height+n,display:N?o:p,verticalAlign:u,position:f};var L=qx.bom.element.Decoration.getTagName(m,J.url);var M=qx.bom.element.Decoration.getAttributes(J.url,m,I);var O=qx.bom.element.Style.compile(M.style);if(L===v){content=b+M.src+r+O+l;content+=K+c;}
else {content=d+O+l;content+=K+w;}
;return content;}
}});}
)();
...
(function(){var e="_applyIconFalse",d="qx.ui.virtual.cell.Boolean",c="_applyIconTrue",b="cell-boolean",a="String";qx.Class.define(d,{extend:qx.ui.virtual.cell.AbstractImage,construct:function(){qx.ui.virtual.cell.AbstractImage.call(this);this.__xT=qx.util.AliasManager.getInstance();}
,properties:{appearance:{refine:true,init:b},iconTrue:{check:a,themeable:true,apply:c},iconFalse:{check:a,themeable:true,apply:e}},members:{__Ke:null,__Kf:null,__xT:null,_applyIconTrue:function(f){this.__Ke=this.__xT.resolve(f);}
,_applyIconFalse:function(g){this.__Kf=this.__xT.resolve(g);}
,_identifyImage:function(h){return h==true?this.__Ke:this.__Kf;}
}});}
)();
