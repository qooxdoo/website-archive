...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.UIDeclaration-qx.theme.Simple.39f4e66d3dd9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,addFromDefinition:function(F){var G=this.getRoot();this.__Ay(G,F);}
,__Ay:function(H,I){var J=new I.clazz();var N=qx.lang.Object.clone(I);delete N.clazz;delete N.layout;delete N.children;delete N.id;delete N.listeners;J.set(N);if(I.id!=null){this[I.id]=J;}
...
(function(){var b="qx.ui.form.Form",a="";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__uI=[];this._buttons=[];this._buttonOptions=[];this._validationManager=new qx.ui.form.validation.Manager();this._resetter=this._createResetter();}
,members:{__uI:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(c,d,e,name,f,g){if(this.__uJ()){this.__uI.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
;this.__uI[this._groupCounter].items.push(c);this.__uI[this._groupCounter].labels.push(d);this.__uI[this._groupCounter].options.push(g);if(name==null){name=d.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,a);}
;this.__uI[this._groupCounter].names.push(name);this._validationManager.add(c,e,f);this._resetter.add(c);}
,addGroupHeader:function(h,k){if(!this.__uJ()){this._groupCounter++;}
;this.__uI.push({title:h,items:[],labels:[],names:[],options:[],headerOptions:k});}
,addButton:function(l,m){this._buttons.push(l);this._buttonOptions.push(m||null);}
,__uJ:function(){return this.__uI.length===0;}
...
,getGroups:function(){return this.__uI;}
,getButtons:function(){return this._buttons;}
,getButtonOptions:function(){return this._buttonOptions;}
,getItems:function(){var n={};for(var i=0;i<this.__uI.length;i++){var o=this.__uI[i];for(var j=0;j<o.names.length;j++){var name=o.names[j];n[name]=o.items[j];}
;}
;return n;}
,_createResetter:function(){return new qx.ui.form.Resetter();}
},destruct:function(){this.__uI=this._buttons=this._buttonOptions=null;this._validationManager.dispose();this._resetter.dispose();}
});}
)();
(function(){var n="Widgets supporting selection can only be validated ",m="Added widget not supported.",l="in the form validator",k="changeValid",j="qx.event.type.Event",h="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",g="qx.ui.form.validation.Manager",f="This field is required",d="qx.event.type.Data",c="complete",a="String",b="";qx.Class.define(g,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__tQ=[];this.__tR={};this.setRequiredFieldMessage(qx.locale.Manager.tr(f));}
,events:{"changeValid":d,"complete":j},properties:{validator:{check:h,init:null,nullable:true},invalidMessage:{check:a,init:b},requiredFieldMessage:{check:a,init:b},context:{nullable:true}},members:{__tQ:null,__tS:null,__tR:null,__tT:null,add:function(o,p,q){if(!this.__tY(o)){throw new Error(m);}
;if(this.__tO(o)&&!o.getValue){if(p!=null){throw new Error(n+l);}
;}
;var r={item:o,validator:p,valid:null,context:q};this.__tQ.push(r);}
,remove:function(s){var t=this.__tQ;for(var i=0,u=t.length;i<u;i++){if(s===t[i].item){t.splice(i,1);return s;}
;}
;return null;}
,getItems:function(){var v=[];for(var i=0;i<this.__tQ.length;i++){v.push(this.__tQ[i].item);}
;return v;}
,validate:function(){var A=true;this.__tT=true;var x=[];for(var i=0;i<this.__tQ.length;i++){var y=this.__tQ[i].item;var B=this.__tQ[i].validator;x.push(y);if(B==null){var w=this.__tU(y);A=A&&w;this.__tT=w&&this.__tT;continue;}
;var w=this.__tV(this.__tQ[i],y.getValue());A=w&&A;if(w!=null){this.__tT=w&&this.__tT;}
;}
;var z=this.__tW(x);if(qx.lang.Type.isBoolean(z)){this.__tT=z&&this.__tT;}
;A=z&&A;this.__ua(A);if(qx.lang.Object.isEmpty(this.__tR)){this.fireEvent(c);}
;return A;}
,__tU:function(C){if(C.getRequired()){if(this.__tO(C)){var D=!!C.getSelection()[0];}
...
;return true;}
,__tV:function(G,H){var M=G.item;var L=G.context;var K=G.validator;if(this.__tX(K)){this.__tR[M.toHashCode()]=null;K.validate(M,M.getValue(),this,L);return null;}
...
,__tW:function(N){var P=this.getValidator();var Q=this.getContext()||this;if(P==null){return true;}
;this.setInvalidMessage(b);if(this.__tX(P)){this.__tR[this.toHashCode()]=null;P.validateForm(N,this,Q);return null;}
...
;}
;return R;}
,__tX:function(S){var T=false;if(!qx.lang.Type.isFunction(S)){T=qx.Class.isSubClassOf(S.constructor,qx.ui.form.validation.AsyncValidator);}
;return T;}
,__tY:function(U){var V=U.constructor;return qx.Class.hasInterface(V,qx.ui.form.IForm);}
,__tO:function(W){var X=W.constructor;return qx.Class.hasInterface(X,qx.ui.core.ISingleSelection);}
,__ua:function(Y){var ba=this.__tS;this.__tS=Y;if(ba!=Y){this.fireDataEvent(k,Y,ba);}
;}
,getValid:function(){return this.__tS;}
,isValid:function(){return this.getValid();}
,getInvalidMessages:function(){var bb=[];for(var i=0;i<this.__tQ.length;i++){var bc=this.__tQ[i].item;if(!bc.getValid()){bb.push(bc.getInvalidMessage());}
;}
;if(this.getInvalidMessage()!=b){bb.push(this.getInvalidMessage());}
;return bb;}
,getInvalidFormItems:function(){var bd=[];for(var i=0;i<this.__tQ.length;i++){var be=this.__tQ[i].item;if(!be.getValid()){bd.push(be);}
;}
;return bd;}
,reset:function(){for(var i=0;i<this.__tQ.length;i++){var bf=this.__tQ[i];bf.item.setValid(true);}
;this.__tS=null;}
,setItemValid:function(bg,bh){this.__tR[bg.toHashCode()]=bh;bg.setValid(bh);this.__ub();}
,setFormValid:function(bi){this.__tR[this.toHashCode()]=bi;this.__ub();}
,__ub:function(){var bk=this.__tT;for(var bl in this.__tR){var bj=this.__tR[bl];bk=bj&&bk;if(bj==null){return;}
;}
;this.__ua(bk);this.__tR={};this.fireEvent(c);}
},destruct:function(){this.__tQ=null;}
});}
)();
(function(){var a="qx.ui.form.validation.AsyncValidator";qx.Class.define(a,{extend:qx.core.Object,construct:function(b){qx.core.Object.call(this);this.__uc=b;}
,members:{__uc:null,__ud:null,__dz:null,__ue:null,validate:function(c,d,e,f){this.__ue=false;this.__ud=c;this.__dz=e;this.__uc.call(f||this,this,d);}
,validateForm:function(g,h,i){this.__ue=true;this.__dz=h;this.__uc.call(i,g,this);}
,setValid:function(j,k){if(this.__ue){if(k!==undefined){this.__dz.setInvalidMessage(k);}
;this.__dz.setFormValid(j);}
else {if(k!==undefined){this.__ud.setInvalidMessage(k);}
;this.__dz.setItemValid(this.__ud,j);}
;}
},destruct:function(){this.__dz=this.__ud=null;}
});}
)();
...
else if(this.__tO(e)){var f=e.getSelection();}
else if(this.__tP(e)){var f=e.getSelection().concat();}
else {throw new Error(b+e+c);}
;;this.__mP.push({item:e,init:f});}
,reset:function(){for(var i=0;i<this.__mP.length;i++){var g=this.__mP[i];this.__tM(g.item,g.init);}
;}
...
;this.__tM(h,j);}
,__tM:function(l,m){if(this._supportsValue(l)){l.setValue(m);}
else if(this.__tO(l)||this.__tP(l)){l.setSelection(m);}
;}
,redefine:function(){for(var i=0;i<this.__mP.length;i++){var n=this.__mP[i].item;this.__mP[i].init=this.__tN(n);}
;}
...
;p.init=this.__tN(p.item);}
,__tN:function(q){if(this._supportsValue(q)){return q.getValue();}
else if(this.__tO(q)||this.__tP(q)){return q.getSelection();}
;}
,__tO:function(r){var s=r.constructor;return qx.Class.hasInterface(s,qx.ui.core.ISingleSelection);}
,__tP:function(t){var u=t.constructor;return qx.Class.hasInterface(u,qx.data.controller.ISelection);}
...
,events:{"changeValue":f},properties:{dateFormat:{check:k,apply:H},placeholder:{check:a,nullable:true,apply:A},appearance:{refine:true,init:B},focusable:{refine:true,init:true},width:{refine:true,init:120}},statics:{__yc:null,__yd:null,getDefaultDateFormatter:function(){var K=qx.locale.Date.getDateFormat(E).toString();if(K==this.__yc){return this.__yd;}
;if(this.__yd){this.__yd.dispose();}
;this.__yd=new qx.util.format.DateFormat(K,qx.locale.Manager.getInstance().getLocale());this.__yc=K;return this.__yd;}
},members:{__ye:null,_forwardStates:{focused:true,invalid:true},_setDefaultDateFormat:function(){this.setDateFormat(qx.ui.form.DateField.getDefaultDateFormatter());}
,_addLocaleChangeLeistener:function(){if(qx.core.Environment.get(l)){this.__ye=qx.locale.Manager.getInstance().addListener(d,function(){this._setDefaultDateFormat();}
,this);}
;}
...
},destruct:function(){if(qx.core.Environment.get(l)){if(this.__ye){qx.locale.Manager.getInstance().removeListenerById(this.__ye);}
;}
;}
});}
)();
...
else {var bv=(bs==null)?-1:bs.getDate();for(var i=0;i<6*7;i++){var bu=this.__xX[i];if(bu.hasState(v)){if(bu.hasState(J)){bu.removeState(J);}
;}
...
;this.__xW=this.__xX=this.__xY=null;}
});}
)();
