...
(function(){var j="select * from feed where url='",h="'",g="feedreader.io.FeedLoader",f="loaded",d="loading",c="changeState",b="failed",a="singleton";
qx.Class.define(g,{extend:qx.core.Object,type:a,members:{loadAll:function(k){var m=k.getFeeds().getItem(0).getFeeds();

for(var i=0;i<m.length;i++){this.load(m.getItem(i));
}var l=k.getFeeds().getItem(1).getFeeds();

for(i=0;i<l.length;i++){this.load(l.getItem(i));
}},load:function(n){n.setState(d);
var o=j+n.getUrl()+h;
var p=new qx.data.store.Yql(o,{manipulateData:function(q){if(q.query.results){q=q.query.results.item||q.query.results.entry;
for(var i=0;i<q.length;i++){if(!qx.lang.Type.isString(q[i].title)){q[i].title=q[i].title.content;
}}return q;
}return b;
}},qx.bom.client.Feature.SSL);
p.addListener(f,this.__BT(n),this);
p.addListener(c,qx.lang.Function.bind(this.__BS,this,n),this);
},__BS:function(r,e){if(e.getData()=="aborted"||e.getData()=="timeout"||e.getData()=="failed"){r.setState("error");
}},__BT:function(s){return function(e){var t=e.getData();
if(t=="failed"){s.setState("error");
return;
}s.setArticles(t);
s.setState("loaded");
};
...
},properties:{callbackParam:{check:a,nullable:false}},members:{__xp:null,_createRequest:function(m){if(this.__xp){this.__xp.dispose();
}this.__xp=new qx.io.ScriptLoader();
var o=this._delegate;

if(o&&qx.lang.Type.isFunction(o.configureRequest)){this._delegate.configureRequest(this.__xp);
}var n=m.indexOf(i)==-1?i:h;
m+=n+this.getCallbackParam()+c;
var p=parseInt(this.toHashCode(),10);
qx.data.store.Jsonp[p]=this;
m+=f+p+e;
this.__xp.load(m,function(q){delete this[p];
},this);
},callback:function(r){if(this.isDisposed()){return;
}this.__xq(r);
},__xq:function(s){if(s==undefined){this.setState(d);
return;
...
}},destruct:function(){if(this.__xp){this.__xp.dispose();
}this.__xp=null;
}});
})();
...
(function(){var o="static",n="execute",m="user",l="settings",k="loading",j="addfeed",h="selection[0]",g="north",f="main",d="new",bu="Surfin' Safari",bt="changeModel",bs="__BU",br="__Cg",bq="selection[0].state",bp="__Ce",bo="Heise",bn="__BY",bm="__Ca",bl="F1",v="article",w="partLoaded",t="Control+A",u="http://blogs.msdn.com/ie/rss.xml",r="http://my.opera.com/desktopteam/xml/rss/blog/",s="feedreader/images/loading22.gif",p="error",q="change",D="http://blogs.msdn.com/jscript/rss.xml",E="loaded",Q="__BV",M="icon/22/places/folder.png",Y="__Cb",T="The Mozilla Blog",bh="enabled",be="http://webkit.org/blog/feed/",I="separator-vertical",bk="http://feeds2.feedburner.com/qooxdoo/news/content",bj="http://blog.mozilla.com/feed/",bi="__Cj",H="__Ci",K="vertical",L="white",O="http://www.heise.de/newsticker/heise-atom.xml",R="__BX",U="state",bb="__Cd",bg="JScript Team Blog",x="Feeds",y="http://feeds2.feedburner.com/ajaxian",J="Control+D",X="icon/22/actions/process-stop.png",W="selection[0].articles",V="IEBlog",bd="Ajaxian",bc="User Feeds",S="icon/22/apps/internet-feed-reader.png",ba="model",a="feedreader.Application",bf="http://daringfireball.net/index.xml",z="Control+P",A="__Cc",N="Static Feeds",b="Control+R",c="qooxdoo News",G="Opera Desktop Blog",B="feedreader.model.Feed",C="Daring Fireball",F="__Ch",P="__Cf";
qx.Class.define(a,{extend:qx.application.Standalone,members:{__BU:null,__BV:null,__BW:null,__BX:null,__BY:null,__Ca:null,__Cb:null,__Cc:null,__Cd:null,__Ce:null,__Cf:null,__Cg:null,__Ch:null,__Ci:null,__Cj:null,main:function(){qx.application.Standalone.prototype.main.call(this);
{};
qx.io.PartLoader.getInstance().addListener(w,function(e){this.debug("part loaded: "+e.getData().getName());
},this);
var bw=qx.locale.Manager.getInstance().getLanguage();
var bv=qx.Part.getInstance().getParts();
if(bv[bw]){qx.io.PartLoader.require([bw],function(){qx.locale.Manager.getInstance().setLocale(bw);
this.buildUpGui();
},this);
}else{this.warn("Cannot load locale part for current language "+bw+", falling back to English.");
this.buildUpGui();
}},buildUpGui:function(){this._initializeCommands();
this._createLayout();
this._initializeModel();
this._setUpBinding();
this.__BX.getRoot().setOpen(true);
this.__BX.getRoot().getChildren()[0].setOpen(true);
this.__BX.getRoot().getChildren()[1].setOpen(true);
this.__BX.setHideRoot(true);
this.__BU.getSelection().push(this.__Cj.getFeeds().getItem(0));
this.reload();
},_initializeModel:function(){this.__Ch=new feedreader.model.FeedFolder(x);
this.__Cj=new feedreader.model.FeedFolder(this.tr(N));
this.__Ch.getFeeds().push(this.__Cj);
this.__Cj.getFeeds().push(new feedreader.model.Feed(c,bk,o));
this.__Cj.getFeeds().push(new feedreader.model.Feed(bg,D,o));
this.__Cj.getFeeds().push(new feedreader.model.Feed(C,bf,o));
this.__Cj.getFeeds().push(new feedreader.model.Feed(bu,be,o));
this.__Cj.getFeeds().push(new feedreader.model.Feed(bd,y,o));
this.__Ci=new feedreader.model.FeedFolder(this.tr(bc));
this.__Ch.getFeeds().push(this.__Ci);
this.__Ci.getFeeds().push(new feedreader.model.Feed(bo,O,m));
this.__Ci.getFeeds().push(new feedreader.model.Feed(V,u,m));
this.__Ci.getFeeds().push(new feedreader.model.Feed(T,bj,m));
this.__Ci.getFeeds().push(new feedreader.model.Feed(G,r,m));
},_setUpBinding:function(){var bx={converter:this._state2iconConverter};
this.__BU=new qx.data.controller.Tree(this.__Ch,this.__BX,"feeds","title");
this.__BU.setIconOptions(bx);
this.__BU.setIconPath(U);
this.__BV=new qx.data.controller.List(null,this.__Cb.getList(),"title");
this.__BU.bind(W,this.__BV,ba);
this.__BV.bind(h,this.__Cf,v);
this.__BV.getSelection().addListener(q,this._listControllerChange,this);
this.__BV.addListener(bt,this._treeControllerChange,this);
var by={converter:this._state2loadingConverter};
this.__BU.bind(bq,this.__Cb,k,by);
by={converter:this._category2enabledConverter};
this.__BU.bind(h,this.__Ca.getRemoveButton(),bh,by);
},_category2enabledConverter:function(bz){return !!bz&&bz.getCategory()==m;
},_state2iconConverter:function(bA){if(bA==d||bA==k){return s;
}else if(bA==E){return S;
}else if(bA==p){return X;
}return M;
},_state2loadingConverter:function(bB){if(bB==d||bB==k){return true;
}return false;
},_listControllerChange:function(bC){var bD=this.__BU.getSelection().getItem(0);
var bE=this.__BV.getSelection().getItem(0);
if(bE!=undefined){bD.setSelectedArticle(bE);
}},_treeControllerChange:function(bF){var bG=this.__BU.getSelection().getItem(0);
if(bG!=null&&bG.classname==B&&bG.getSelectedArticle()!=null&&bG.getArticles().contains(bG.getSelectedArticle())){qx.event.Timer.once(function(){this.__BV.getSelection().push(bG.getSelectedArticle());
},this,0);
}else{this.__Cb.getList().scrollToY(0);
}},_createLayout:function(){var bH=new qx.ui.layout.Dock();
bH.setSeparatorY(I);
var bI=new qx.ui.container.Composite(bH);
this.getRoot().add(bI,{edge:0});
this.__BY=new feedreader.view.Header();
bI.add(this.__BY,{edge:g});
this.__Ca=new feedreader.view.ToolBar(this);
bI.add(this.__Ca,{edge:g});
this.__Cc=new qx.ui.splitpane.Pane();
bI.add(this.__Cc);
this.__BX=new qx.ui.tree.Tree();
this.__BX.setWidth(250);
this.__BX.setBackgroundColor(L);
this.__Cc.add(this.__BX,0);
this.__Cd=new qx.ui.splitpane.Pane(K);
this.__Cd.setDecorator(null);
this.__Cc.add(this.__Cd,1);
this.__Cb=new feedreader.view.List(this.__Ch);
this.__Cb.setHeight(200);
this.__Cb.setDecorator(f);
this.__Cd.add(this.__Cb,0);
this.__Cf=new feedreader.view.Article();
this.__Cf.setDecorator(f);
this.__Cd.add(this.__Cf,1);
},_initializeCommands:function(){var bJ={};
bJ.reload=new qx.ui.core.Command(b);
bJ.reload.addListener(n,this.reload,this);
bJ.about=new qx.ui.core.Command(bl);
bJ.about.addListener(n,this.showAbout,this);
bJ.preferences=new qx.ui.core.Command(z);
bJ.preferences.addListener(n,this.showPreferences,this);
bJ.addFeed=new qx.ui.core.Command(t);
bJ.addFeed.addListener(n,this.showAddFeed,this);
bJ.removeFeed=new qx.ui.core.Command(J);
bJ.removeFeed.addListener(n,this.removeFeed,this);
this.__BW=bJ;
},getCommand:function(bK){return this.__BW[bK];
},addFeed:function(bL,bM,bN){var bO=new feedreader.model.Feed(bL,bM,bN);
this.__Ci.getFeeds().push(bO);
var bP=feedreader.io.FeedLoader.getInstance();
bP.load(bO);
},removeFeed:function(){var bQ=this.__BU.getSelection().getItem(0);
if(bQ&&bQ.getCategory&&bQ.getCategory()!==o){var bR=this.__Ci.getFeeds();
for(var i=0;i<bR.length;i++){if(bQ===bR.getItem(i)){bR.splice(i,1);
return;
}}}},reload:function(){var bS=feedreader.io.FeedLoader.getInstance();
bS.loadAll(this.__Ch);
this.__Cf.setArticle(null);
},showPreferences:function(){this.__Ca.signalLoading(l,true);
qx.io.PartLoader.require([l],function(){if(!this.__Ce){this.__Ce=new feedreader.view.PreferenceWindow();
this.getRoot().add(this.__Ce);
}this.__Ce.center();
this.__Ce.open();
this.__Ca.signalLoading(l,false);
},this);
},showAbout:function(){alert(this.tr("FeedReader (qooxdoo powered)"));
},showAddFeed:function(){this.__Ca.signalLoading(j,true);
qx.io.PartLoader.require([j],function(){if(!this.__Cg){this.__Cg=new feedreader.view.AddFeedWindow(this);
this.getRoot().add(this.__Cg);
}this.__Cg.center();
this.__Cg.open();
this.__Ca.signalLoading(j,false);
},this);
}},destruct:function(){this.__BW=null;
this._disposeObjects(bm,Y,P,R,F,A,bb,bn,bi,H,bs,Q,bp,br);
}});
})();
...
this.add(K);
this.addSeparator();
this.__Cn=new qx.ui.toolbar.Button(this.tr(g),t);
this.__Cn.setCommand(H.getCommand(l));
this.__Cn.setToolTipText(this.tr(w));
this.add(this.__Cn);
this.addSpacer();
...
this.__Co=new qx.ui.menu.Menu();
M.setMenu(this.__Co);
this.addListener(u,this._onHideItem,this);
this.addListener(s,this._onShowItem,this);
},members:{__Cm:null,__Co:null,__Ck:null,__Cl:null,__Cn:null,getRemoveButton:function(){return this.__Cm;
...
P.setVisibility(f);
},_getMenuItem:function(R){var S=this.__Ck[R.toHashCode()];
...
this.__Ck[R.toHashCode()]=S;
}return S;
},signalLoading:function(T,U){if(T==z){var V=this.__Cl;
}else if(T==p){var V=this.__Cn;
}else{return ;
...
(function(){var n="separator-vertical",m="__Cq",l="background-medium",k="Posts",j="bold",i="__Cr",h="_applyLoading",g="center",f="feedreader.view.List",e="__Cp",b="single",d="middle",c="feedreader/images/loading66.gif",a="Boolean";
...
q.setFont(j);
this._add(q);
this.__Cp=new qx.ui.container.Stack();
this._add(this.__Cp,{flex:1});
this.__Cq=new qx.ui.form.List();
this.__Cq.setDecorator(null);
this.__Cq.setSelectionMode(b);
this.__Cp.add(this.__Cq);
this.__Cr=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,g));
var o=new qx.ui.basic.Image(c);
o.setAlignY(d);
this.__Cr.add(o);
this.__Cp.add(this.__Cr);
},properties:{loading:{check:a,init:false,apply:h}},members:{__Cp:null,__Cq:null,__Cr:null,_applyLoading:function(r,s){if(r){this.__Cp.setSelection([this.__Cr]);
}else{this.__Cp.setSelection([this.__Cq]);
}},getList:function(){return this.__Cq;
}},destruct:function(){this._disposeObjects(m,e,i);
}});
})();
...
(function(){var m="_blank",l="Verdana",k="sans-serif",j="#FFFDED",h="feedreader.view.Article",g="_applyArticle",f="feedreader/css/reader.css",e="blogEntry",d="a",c="Candara",a="auto",b="Object";
qx.Class.define(h,{extend:qx.ui.embed.Html,construct:function(){qx.ui.embed.Html.call(this);
if(!feedreader.view.Article.__CF){feedreader.view.Article.__CF=true;
qx.bom.Stylesheet.includeFile(f);
}this.setCssClass(e);
this.setOverflowY(a);
this.setBackgroundColor(j);
var n=new qx.bom.Font(14,[c,l,k]);
n.setLineHeight(1.8);
this.setFont(n);
},properties:{article:{apply:g,nullable:true,check:b}},members:{_applyArticle:function(o,p){var q=this.__CG();
this.setHtml(q);
qx.html.Element.flush();
var s=this.getContentElement().getDomElement();
var r=s.getElementsByTagName(d);
for(var i=0;i<r.length;i++){r[i].target=m;
}},__CG:function(){var u=this.getArticle();
if(!u){return "";
}var v=new qx.util.StringBuilder();
var t=(u.getPubDate&&u.getPubDate())||"";
var x="";

if(u.getDescription){x=u.getDescription();
}else if(u.getContent){x=u.getContent().getContent&&u.getContent().getContent();
}var w=u.getLink();

if(w.getHref){w=w.getHref();
}else if(w instanceof qx.data.Array){w=w.getItem(0).getHref();
}v.add("<div class='container'>");
v.add("<h1 class='blog'>",u.getTitle(),"</h1>");
v.add("<div class='date'>",t,"</div>");
v.add("<div class='description'>",x,"</div>");
v.add("<a target='_blank' href='",w,"'>read more ...</a>");
v.add("</div>");
return v.get();
}}});
})();
