...
qx.Class.define(r,{extend:qx.ui.form.renderer.Single,members:{__QQ:0,__QR:0,addItems:function(C,D,E,F){var I=0;
if(E!=null){this._add(this._createHeader(E),{row:I,column:this.__QQ,colSpan:2});
I++;
}for(var i=0;i<C.length;i++){var H=this._createLabel(D[i],C[i]);
this._add(H,{row:I,column:this.__QQ});
var G=C[i];
H.setBuddy(G);

if(G instanceof qx.ui.form.RadioGroup){G=this._createWidgetForRadioGroup(G);
}this._add(G,{row:I,column:this.__QQ+1});
if(F[i]&&F[i].placeholder){G.setPlaceholder(F[i].placeholder);
}I++;
}this.__QQ+=2;
this.__QR<I?this.__QR=I:null;
...
this._add(this._buttonRow,{row:this.__QR,column:0,colSpan:this.__QQ});
