...
this.__JL=null;
this.__JM=null;
},properties:{blockSize:{check:c,init:50},maxCachedBlockCount:{check:c,init:15},clearCacheOnRemove:{check:b,init:false},blockConcurrentLoadRowCount:{check:b,init:true}},members:{__JD:null,__JI:null,__JE:null,__JF:null,__JG:null,__JH:null,__JJ:null,__JK:null,__JB:null,__JC:null,__JM:null,__JL:null,__JN:false,_getIgnoreCurrentRequest:function(){return this.__JI;
},getRowCount:function(){if(this.__JD==-1){if(!this.__JN||!this.getBlockConcurrentLoadRowCount()){this.__JN=true;
this._loadRowCount();
}return (this.__JD==-1)?0:this.__JD;
}else{return this.__JD;
}},_loadRowCount:function(){throw new Error("_loadRowCount is abstract");
},_onRowCountLoaded:function(f){if(this.getBlockConcurrentLoadRowCount()){this.__JN=false;
}if(f==null||f<0){f=0;
}this.__JD=Number(f);
...
if(this.__JF!=-1){var h=this._cancelCurrentRequest();
...
this.__JN=true;
this._loadRowCount();
},clearCache:function(){this.__JJ={};
this.__JK=0;
},getCacheContent:function(){return {sortColumnIndex:this.__JB,sortAscending:this.__JC,rowCount:this.__JD,lruCounter:this.__JE,rowBlockCache:this.__JJ,rowBlockCount:this.__JK};
},restoreCacheContent:function(j){if(this.__JF!=-1){var k=this._cancelCurrentRequest();

if(k){this.__JF=-1;
this.__JI=false;
}else{this.__JI=true;
}}this.__JB=j.sortColumnIndex;
this.__JC=j.sortAscending;
this.__JD=j.rowCount;
this.__JE=j.lruCounter;
this.__JJ=j.rowBlockCache;
this.__JK=j.rowBlockCount;
var l={firstRow:0,lastRow:this.__JD-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
for(var w=0;w<=o;w++){var q=this.__JJ[w];
...
if(r!=null){u[t]=r;
}}}}},prefetchRows:function(x,y){if(this.__JF==-1){var z=this.getBlockSize();
var E=Math.ceil(this.__JD/z);
...
for(var F=D;F<=C;F++){if(this.__JJ[F]==null||this.__JJ[F].isDirty){if(B==-1){B=F;
}A=F;
}}if(B!=-1){this.__JG=-1;
this.__JH=-1;
this.__JF=B;
...
},_onRowDataLoaded:function(I){if(I!=null&&!this.__JI){var L=this.getBlockSize();
var J=Math.ceil(I.length/L);

if(J==1){this._setRowBlockData(this.__JF,I);
}else{for(var i=0;i<J;i++){var O=i*L;
var N=[];
var K=Math.min(L,I.length-O);

for(var P=0;P<K;P++){N.push(I[O+P]);
}this._setRowBlockData(this.__JF+i,N);
}}var M={firstRow:this.__JF*L,lastRow:(this.__JF+J+1)*L-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(e,M);
}this.__JF=-1;
this.__JI=false;
if(this.__JG!=-1){this.prefetchRows(this.__JG,this.__JH);
}},_setRowBlockData:function(Q,R){if(this.__JJ[Q]==null){this.__JK++;

while(this.__JK>this.getMaxCachedBlockCount()){var V;
var U=this.__JE;

for(var T in this.__JJ){var S=this.__JJ[T].lru;

if(S<U&&T>1){U=S;
V=T;
}}delete this.__JJ[V];
this.__JK--;
}}this.__JJ[Q]={lru:++this.__JE,rowDataArr:R};
...
for(var bf=bc;bf<=bb;bf++){var X=this.__JJ[bf];
...
if(bf==bb-1){if(X.rowDataArr.length==0){delete this.__JJ[bf];
}}else{var bd=this.__JJ[bf+1];

if(bd!=null){X.rowDataArr.push(bd.rowDataArr[0]);
}else{X.isDirty=true;
}}}}
if(this.__JD!=-1){this.__JD--;
...
if(bi.lru!=this.__JE){bi.lru=++this.__JE;
}return bj;
...
}}},setEditable:function(bv){this.__JM=[];

for(var bw=0;bw<this.getColumnCount();bw++){this.__JM[bw]=bv;
}this.fireEvent(d);
},setColumnEditable:function(bx,by){if(by!=this.isColumnEditable(bx)){if(this.__JM==null){this.__JM=[];
}this.__JM[bx]=by;
this.fireEvent(d);
}},isColumnEditable:function(bz){return (this.__JM?(this.__JM[bz]==true):false);
},setColumnSortable:function(bA,bB){if(bB!=this.isColumnSortable(bA)){if(this.__JL==null){this.__JL=[];
}this.__JL[bA]=bB;
this.fireEvent(d);
}},isColumnSortable:function(bC){return (this.__JL?(this.__JL[bC]!==false):true);
},sortByColumn:function(bD,bE){if(this.__JB!=bD||this.__JC!=bE){this.__JB=bD;
this.__JC=bE;
this.clearCache();
this.fireEvent(d);
}},getSortColumnIndex:function(){return this.__JB;
},isSortAscending:function(){return this.__JC;
},setSortColumnIndexWithoutSortingData:function(bF){this.__JB=bF;
},setSortAscendingWithoutSortingData:function(bG){this.__JC=bG;
}},destruct:function(){this.__JL=this.__JM=this.__JJ=null;
}});
})();
