...
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:d,check:[d,b]},x:{init:0,check:c},y:{init:0,check:c}},members:{__KE:null,__KF:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var e=this._getElement();
this.__KE={x:e.scrollLeft,y:e.scrollTop};
if(this._atEndPosition(this.__KE.x,this.__KE.y)){return;
}
if(this.getMode()==b){this.__KF={left:this.getX()-this.__KE.x,top:this.getY()-this.__KE.y};
}},update:function(f){qx.fx.Base.prototype.update.call(this);
var g=this._getElement();

if(this.getMode()==d){if(this.getX()!=0){g.scrollLeft=this.__KE.x+(this.getX()*f);
}
if(this.getY()!=0){g.scrollTop=this.__KE.y+(this.getY()*f);
}}else{g.scrollLeft=this.__KE.x+(this.__KF.left*f);
g.scrollTop=this.__KE.y+(this.__KF.top*f);
