...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.test.TimerManager","qx.theme":"qx.theme.Classic"};
...
(function(){var h="Stopping recurrent 2-second timer",g="Recurrent 2-second timer with limit 3:",f="One-shot 1-second timer: ",e="__OE",d="demobrowser.demo.test.TimerManager",c="Recurrent 5-second timer: ",b="Start timers",a="execute";
qx.Class.define(d,{extend:qx.application.Standalone,members:{__OE:null,startTimers:function(){var i=qx.util.TimerManager.getInstance();
i.start(function(j,k){this.__OF(c+k);
},5000,this,null,0);
i.start(function(l,m){this.__OF(f+m);
},0,this,null,1000);
i.start(function(n,o){this.__OF(g+o);

if(++n.count==3){this.__OF(h);
i.stop(o);
}},2000,this,{count:0},2000);
},__OF:function(p){p=Date.parse(new Date)+" "+p+"\n";
this.__OE.setValue(this.__OE.getValue()+p);
...
qx.Class.define(c,{extend:qx.core.Object,type:a,statics:{__KO:[],__KP:{},__KQ:0},members:{__KR:false,start:function(d,e,f,g,h){if(!h){h=e||0;
}var j=(new Date()).getTime()+h;
this.self(arguments).__KP[++this.self(arguments).__KQ]={callback:d,userData:g||null,expireAt:j,recurTime:e,context:f||this};
this.__KS(j,this.self(arguments).__KQ);
return this.self(arguments).__KQ;
},stop:function(k){var l=this.self(arguments).__KO;
var length=l.length;

for(var i=0;i<length;i++){if(l[i]==k){l.splice(i,1);
break;
}}delete this.self(arguments).__KP[k];
if(l.length==0&&this.__KR){qx.event.Idle.getInstance().removeListener(b,this.__KT,this);
this.__KR=false;
}},__KS:function(m,n){var p=this.self(arguments).__KO;
var o=this.self(arguments).__KP;
var length=p.length;

for(var i=0;i<length;i++){if(o[p[i]].expireAt>m){p.splice(i,0,n);
break;
}}if(p.length==length){p.push(n);
}if(!this.__KR){qx.event.Idle.getInstance().addListener("interval",this.__KT,this);
this.__KR=true;
}},__KT:function(){var t=(new Date()).getTime();
var r=this.self(arguments).__KO;
var s=this.self(arguments).__KP;
...
this.__KS(q.expireAt,v);
}else{delete s[v];
}}if(r.length==0&&this.__KR){qx.event.Idle.getInstance().removeListener("interval",this.__KT,this);
this.__KR=false;
}}}});
})();
