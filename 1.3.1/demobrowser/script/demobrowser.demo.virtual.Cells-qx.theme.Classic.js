...
qx.util.Permutation.permute(A,function(B){var C=this.__PG(z,B.value,B.states);
...
}var G=this.__PG(E,F.value,F.states);
...
qx.util.Permutation.permute(P,function(R){var S=this.__PG(Q,R.value,R.states);
this.container.add(S);
},this);
},testBooleanCell:function(){var T={value:[true,false,null],states:[{},{selected:1}]};
var U=new qx.ui.virtual.cell.Boolean;
qx.util.Permutation.permute(T,function(V){var W=this.__PG(U,V.value,V.states);
...
Y.setIconFalse(n);
Y.setIconTrue(m);
qx.util.Permutation.permute(X,function(ba){var bb=this.__PG(Y,ba.value,ba.states);
this.container.add(bb);
},this);
},__PG:function(bc,bd,be){var bi=100;
var bg=30;
...
(function(){var r="",q="px",p='" ',o="no-repeat",n='"',m='" style="',l='title="',k='<div style="',j='></div>',i="qx.ui.virtual.cell.AbstractImage",c="img",h="inline-block",f='/>',b="static",a="top",e='<img src="',d="abstract",g="-moz-inline-box";
qx.Class.define(i,{extend:qx.ui.virtual.cell.Cell,type:d,construct:function(){qx.ui.virtual.cell.Cell.call(this);
this._aliasManager=qx.util.AliasManager.getInstance();
},members:{__FM:16,__FN:16,_aliasManager:null,__FO:function(s){var v=qx.util.ResourceManager.getInstance();
var u=qx.io.ImageLoader;
var t,w;
if(v.has(s)){t=v.getImageWidth(s),w=v.getImageHeight(s);
}else if(u.isLoaded(s)){t=u.getWidth(s);
w=u.getHeight(s);
}else{t=this.__FM;
w=this.__FN;
}return {width:t,height:w};
},__FP:function(x){if(typeof (x)=="string"){x={url:x};
}var y=this._aliasManager.resolve(x.url||null);
var z;

if(x.width&&x.height){z={width:x.width,height:x.height};
}else{z=this.__FO(y);
}return {width:z.width,height:z.height,url:y,tooltip:x.tooltip};
},_identifyImage:function(A){throw new Error("_identifyImage is abstract");
},getContent:function(B,C){if(B===null){return r;
}var content=r;
var E=this.__FP(this._identifyImage(B));
var I=qx.bom.client.Engine.GECKO&&qx.bom.client.Engine.VERSION<1.9;
var F=E.tooltip?l+E.tooltip+n:r;
var D={width:E.width+q,height:E.height+q,display:I?g:h,verticalAlign:a,position:b};
var G=qx.bom.element.Decoration.getTagName(o,E.url);
var H=qx.bom.element.Decoration.getAttributes(E.url,o,D);
var J=qx.bom.element.Style.compile(H.style);

if(G===c){content=e+H.src+m+J+p;
content+=F+f;
}else{content=k+J+p;
content+=F+j;
}return content;
...
this.__Jc=qx.util.AliasManager.getInstance();
},properties:{appearance:{refine:true,init:a},iconTrue:{check:e,themeable:true,apply:b},iconFalse:{check:e,themeable:true,apply:d}},members:{__Jd:null,__Je:null,__Jc:null,_applyIconTrue:function(f){this.__Jd=this.__Jc.resolve(f);
},_applyIconFalse:function(g){this.__Je=this.__Jc.resolve(g);
},_identifyImage:function(h){return h==true?this.__Jd:this.__Je;
}}});
})();
