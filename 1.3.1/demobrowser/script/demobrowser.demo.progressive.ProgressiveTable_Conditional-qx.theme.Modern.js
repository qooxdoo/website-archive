...
},members:{__Jf:null,join:function(c){this.__Jf=c;
...
this.__Ji=new qx.ui.layout.HBox();
this.__Ji.connectToWidget(this);
},properties:{appearance:{refine:true,init:c}},members:{__Jg:null,__Jj:null,__Jh:null,__Ji:null,join:function(m){qx.ui.progressive.headfoot.Abstract.prototype.join.call(this,m);
},getLayoutChildren:function(){if(this.__Jj){return this.__Jg.getData();
...
this.__Jj=true;
this.__Ji.renderLayout(n,100);
this.__Jj=false;
var o=this.__Jg.getData();
for(var i=0;i<o.length;i++){this.__Jh[i].setWidth(o[i].getComputedWidth());
...
(function(){var m="progressive-progressbar-indicator-undone",l="progress",k="%",j="progressive-progressbar-indicator-done",i="progressive-progressbar-percent-text",h="renderStart",g="__MJ",f="progressive-progressbar-background",d="__MK",c="qx.ui.progressive.headfoot.Progress",a="progressive-progressbar-percent-background",b="renderEnd";
...
this.__MI.background=p.resolve(f);
this.__MI.indicatorDone=p.resolve(j);
this.__MI.indicatorUndone=p.resolve(m);
this.__MI.percentBackground=p.resolve(a);
this.__MI.percentText=p.resolve(i);
this.set({backgroundColor:this.__MI.background});
this.__MJ=new qx.ui.core.Widget();
this.__MJ.set({width:0,backgroundColor:this.__MI.indicatorDone});
this.add(this.__MJ);
var q=new qx.ui.core.Widget();
q.set({backgroundColor:this.__MI.indicatorUndone});
this.add(q,{flex:1});
this.__MK=new qx.ui.basic.Atom("0%");
this.__MK.set({width:100,backgroundColor:this.__MI.percentBackground,textColor:this.__MI.percentText});
this.add(this.__MK);
this.exclude();
},members:{__ML:null,__MI:null,__MJ:null,__MK:null,join:function(r){qx.ui.progressive.headfoot.Abstract.prototype.join.call(this,r);
r.addListener(h,function(e){this.__ML=e.getData().initial;
this.show();
},this);
r.addListener(l,function(e){var u=1.0-(e.getData().remaining/this.__ML);
var v=this.getBounds();

if(v){var s=Math.floor((v.width-this.__MK.getBounds().width)*u);
var t=Math.floor(u*100)+k;

if(!isNaN(s)){this.__MJ.setMinWidth(s);
this.__MK.setLabel(t);
}}},this);
...
if(!e){this.__KG=new qx.ui.core.Widget();
this.__KH=this.__KG;
}else{this.__KG=null;
this.__KH=e;
}this.__KH.getContentElement().setStyle(b,a);
},members:{__KG:null,__KH:null,applyStructure:function(f){throw new Error("applyStructure() is abstract");
},getPane:function(){return this.__KH;
}},destruct:function(){if(this.__KG){this.__KG.dispose();
}this.__KG=this.__KH=null;
}});
})();
...
if(!b){this.__KU=new qx.ui.progressive.headfoot.Null();
this.__KV=this.__KU;
}else{this.__KU=null;
this.__KV=b;
}if(!c){this.__KW=new qx.ui.progressive.headfoot.Null();
this.__KX=this.__KW;
}else{this.__KW=null;
this.__KX=c;
}},members:{__KV:null,__KX:null,__KU:null,__KW:null,applyStructure:function(e){this.__KV.join(e);
this.__KX.join(e);
e.add(this.__KV);
e.add(this.getPane(),{flex:1});
e.add(this.__KX);
},getHeader:function(){return this.__KV;
},getFooter:function(){return this.__KX;
}},destruct:function(){if(this.__KU){this.__KU.dispose();
this.__KU=null;
}
if(this.__KW){this.__KW.dispose();
this.__KW=null;
}this.__KV=this.__KX=null;
}});
})();
...
this.__Lc=false;
this.__Ld=0;
},events:{"renderStart":n,"renderEnd":n,"progress":n,"progressDetail":n},properties:{dataModel:{check:g,apply:j},batchSize:{check:k,init:20},flushWidgetQueueAfterBatch:{check:h,init:false},interElementTimeout:{check:k,init:0}},members:{__KY:null,__Lc:null,__Le:null,__Ld:null,__Lb:null,__La:null,getStructure:function(){return this.__La;
},addRenderer:function(name,p){this.__KY[name]=p;
p.join(this,name);
},removeRenderer:function(name){if(!this.__KY[name]){throw new Error("No existing renderer named "+name);
}delete this.__KY[name];
},render:function(){if(this.__Lc){return;
}this.__Lc=true;
var q=new qx.ui.progressive.State({progressive:this,model:this.getDataModel(),pane:this.__La.getPane(),batchSize:this.getBatchSize(),rendererData:this.__Lg(),userData:{}});
this.__Le=new Date();
if(this.__Lb){this.__Ld=q.getModel().getElementCount();
this.fireDataEvent(l,{state:q,initial:this.__Ld});
this.__Lf(q);
}else{qx.event.Timer.once(function(){this.__Ld=q.getModel().getElementCount();
this.fireDataEvent(l,{state:q,initial:this.__Ld});
this.__Lf(q);
this.__Lb=true;
},this,10);
}},_applyDataModel:function(r,s){if(s){s.removeListener(m,this.__Lh,this);
s.dispose();
}r.addListener(m,this.__Lh,this);
},__Lf:function(t){var u;
var w;
var v;

for(var i=t.getBatchSize();i>0;i--){u=t.getModel().getNextElement();

if(!u){this.debug("Render time: "+(new Date()-this.__Le)+"ms");
this.__Lc=false;
this.fireDataEvent(b,t);
t.dispose();
return ;
}w=u.element;
v=this.__KY[w.renderer];
v.render(t,w);
this.fireDataEvent(a,{initial:this.__Ld,remaining:u.remaining,element:w});
}this.fireDataEvent(f,{initial:this.__Ld,remaining:u.remaining});
if(this.getFlushWidgetQueueAfterBatch()){qx.ui.core.queue.Manager.flush();
}qx.event.Timer.once(function(){this.__Lf(t);
},this,this.getInterElementTimeout());
},__Lg:function(){var x={};

for(var name in this.__KY){x[name]={};
}return x;
},__Lh:function(e){this.__Ld=e.getData();
this.render();
}},destruct:function(){for(var name in this.__KY){this.__KY[name].dispose();
}this.__Le=this.__KY=this.__La=null;
}});
})();
...
this.__Lv=[];
},members:{__Lv:null,addElements:function(c){this.__Lv=this.__Lv.concat(c);
this.fireDataEvent(b,this.__Lv.length);
},addElement:function(d){this.__Lv.push(d);
this.fireDataEvent(b,this.__Lv.length);
},getElementCount:function(){return this.__Lv.length;
},getNextElement:function(){if(this.__Lv.length>0){return ({element:this.__Lv.shift(),remaining:this.__Lv.length});
}return null;
...
(function(){var l=".qx-progressive-",k="-col-",j="px;",h="-row",g="}",f="width: ",d="0px ",c="qx-progressive-",b="px",a="  font-size: 11px;",T="padding: ",S="px ",R='',Q="__JV",P="end",O="qx.client",N="div",M="  position: absolute;",L="  width : 100%;",K="__columnData",s="  border-bottom:1px solid #eeeeee;",t="",q="  top: 0px;",r="  overflow:hidden;",o=" {",p="  cursor:default;",m="  font-family: 'Segoe UI', Corbel, Calibri, Tahoma, 'Lucida Sans Unicode', sans-serif;",n="-row {",u="resize",v="start",C="relative",A="  -o-text-overflow: ellipsis;",E="mshtml",D="  border-right:1px solid #f2f2f2;",G="  white-space:nowrap;",F=';-moz-user-select:none;',x="  padding : 0px 6px 0px 6px;",J="qx.ui.progressive.renderer.table.Row",I="progressive-table-row-background-odd",H="  text-overflow:ellipsis;",w="  height: 100%;",y="progressive-table-row-background-even",z="__JY",B="left: ";
qx.Class.define(J,{extend:qx.ui.progressive.renderer.Abstract,construct:function(U){qx.ui.progressive.renderer.Abstract.call(this);
this.__JT=U;
this.__JU={};
this.__JV=new qx.ui.progressive.renderer.table.cell.Default();
this.__JW=null;
this.__JX={};
var V=qx.theme.manager.Color.getInstance();
this.__JX.bgcol=[];
this.__JX.bgcol[0]=V.resolve(y);
this.__JX.bgcol[1]=V.resolve(I);
this.__JY=new qx.ui.layout.HBox();
this.__JY.connectToWidget(this);
},statics:{__Ka:null,__Kb:6,__Kc:M+q+w+r+H+A+G+D+s+x+p+a+m+(qx.core.Variant.isSet(O,E)?R:F)},properties:{defaultRowHeight:{init:16}},members:{__JW:null,__Kd:null,__Ke:null,__JT:null,__JU:null,__JV:null,__JX:null,__JY:null,join:function(W,name){if(this.__JW){throw new Error("Renderer is already joined to a Progressive.");
}this.__JW=W;
this.__Kd=name;
...
if(!Y.__Ka[X]){Y.__Ka[X]={rowstylesheet:null,cellstylesheet:[]};
var ba=l+X+n+L+g;
Y.__Ka[X].rowstylesheet=qx.bom.Stylesheet.createElement(ba);
var bb=this.__JT.getData();

for(var i=0;i<bb.length;i++){var ba=l+X+k+i+o+Y.__Kc+g;
Y.__Ka[X].cellstylesheet[i]=qx.bom.Stylesheet.createElement(ba);
}this.__Ke=X;
var bc=W.getStructure().getPane();
bc.addListener(u,this._resizeColumns,this);
}},addRenderer:function(bd,be){var bf=this.__JT.getData();

if(bd<0||bd>=bf.length){throw new Error("Column "+bd+" out of range (max: "+(bf.length-1)+")");
}this.__JU[bd]=be;
},removeRenderer:function(bg){var bh=this.__JT.getData();

if(bg<0||bg>=bh.length){throw new Error("Column "+bg+" out of range (max: "+(bh.length-1)+")");
}
if(!this.__JU[bg]){throw new Error("No existing renderer for column "+bg);
}delete this.__JU[bg];
...
var bp;
var bm=0;
if(bi.getRendererData()[this.__Kd].end===undefined){bi.getRendererData()[this.__Kd]={end:0,start:1,rows:0,totalHeight:0};
}var bs=document.createElement(N);
for(var i=0;i<bt.length;i++){var bn=c+this.__Ke+k+i;
bp=this.__JU[i]||this.__JV;
...
var br=bi.getRendererData()[this.__Kd];
br.totalHeight+=bm;
bs.style.position=C;
bs.style.height=bm+b;
bs.className=c+this.__Ke+h;
bs.innerHTML=bo.join(t);
switch(bj.location){case P:var bk=br.end||0;
bs.style.backgroundColor=this.__JX.bgcol[bk];
br.end=(bk==0?1:0);
...
},getLayoutChildren:function(){return this.__JT.getData();
},_resizeColumns:function(e){var bC=this.__JW.getStructure().getPane();
var bz=bC.getBounds().width-qx.bom.element.Overflow.getScrollbarWidth();
var bA=l+this.__Ke+h;
var bw=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bw.__Ka[this.__Ke].rowstylesheet,bA);
var by=f+bz+j;
qx.bom.Stylesheet.addRule(bw.__Ka[this.__Ke].rowstylesheet,bA,by);
this.__JY.renderLayout(bz,100);
var bB=this.__JT.getData();
for(var i=0,bF=0;i<bB.length;i++,bF+=bz){var bA=l+this.__Ke+k+i;
var bw=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bw.__Ka[this.__Ke].cellstylesheet[i],bA);
bz=bB[i].getComputedWidth();
{};
var bx;

if(qx.bom.client.Feature.CONTENT_BOX){bx=qx.ui.progressive.renderer.table.Row.__Kb*2;
}else{bx=-1;
}var bE=(bz-bx)+j;
var bD=d+qx.ui.progressive.renderer.table.Row.__Kb+S+d+qx.ui.progressive.renderer.table.Row.__Kb+j;
var bv=bF+j;
var by=bw.__Kc+f+bE+B+bv+T+bD;
qx.bom.Stylesheet.addRule(bw.__Ka[this.__Ke].cellstylesheet[i],bA,by);
}}},destruct:function(){var name;

for(name in this.__JU){this.__JU[name]=null;
}var bH=qx.ui.progressive.renderer.table.Row;
var bG=this.__JW.toHashCode();

if(bH.__Ka&&bH.__Ka[bG]){if(bH.__Ka[bG].rowstylesheet){var bJ=l+this.__Ke+h;
var bH=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bH.__Ka[this.__Ke].rowstylesheet,bJ);
}if(bH.__Ka[bG].cellstylesheet){for(var i=bH.__Ka[bG].cellstylesheet.length-1;i>=0;i--){var bJ=l+this.__Ke+k+i;
var bI=bH.__Ka[this.__Ke].cellstylesheet[i];
var bH=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bI,bJ);
}}}
if(this.__JW&&this.__JW.getRendererData){var bK=this.__JW.getRendererData();

if(bK&&bK[this.__Kd]&&bK[this.__Kd].end!==undefined){bK[this.__Kd]=null;
}}this.__JX=this.__JU=this.__JW=this.__JT=null;
this._disposeObjects(z,Q,K);
}});
})();
...
qx.Class.define(d,{extend:qx.ui.progressive.renderer.table.cell.Abstract,construct:function(p,q,r,s){qx.ui.progressive.renderer.table.cell.Abstract.call(this);
this.__LF=["==","!=",">","<",">=","<="];
this.__LG=["between","!between"];
this.__LH=[];
this.__LI=p||"";
this.__LJ=q||"";
this.__LK=r||"";
this.__LL=s||"";
},members:{__LF:null,__LG:null,__LH:null,__LI:null,__LJ:null,__LK:null,__LL:null,__LM:function(t,u){if(t.align){u["text-align"]=t.align;
}
if(t.color){u["color"]=t.color;
}
if(t.style){u["font-style"]=t.style;
}
if(t.weight){u["font-weight"]=t.weight;
}},addNumericCondition:function(v,w,x,y,z,A,B){if(!qx.lang.Array.contains(this.__LF,v)||w==null){throw new Error("Condition not recognized or value is null!");
}this.__LH.push({condition:v,align:x,color:y,style:z,weight:A,value1:w,target:B});
},addBetweenCondition:function(C,D,E,F,G,H,I,J){if(!qx.lang.Array.contains(this.__LG,C)||D==null||E==null){throw new Error("Condition not recognized or value1/value2 is null!");
}this.__LH.push({condition:C,align:F,color:G,style:H,weight:I,value1:D,value2:E,target:J});
},addRegex:function(K,L,M,N,O,P){if(!K){throw new Error("regex cannot be null!");
}this.__LH.push({condition:n,align:L,color:M,style:N,weight:O,regex:K,target:P});
},_getCellStyle:function(Q){if(this.__LH.length==0){return Q.style||o;
}var i;
var U;
var R;
var T={"text-align":this.__LI,"color":this.__LJ,"font-style":this.__LK,"font-weight":this.__LL};

for(i=0;i<this.__LH.length;i++){var X=this.__LH[i];
U=false;

if(qx.lang.Array.contains(this.__LF,X.condition)){if(X.target==null){R=Q.cellData;
...
}}else if(qx.lang.Array.contains(this.__LG,X.condition)){if(X.target==null){R=Q.cellData;
}else{R=Q.element.data[X.target];
}
...
}},destruct:function(){this.__LF=this.__LG=this.__LH=null;
}});
})();
