...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.virtual.Complex","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.virtual.Complex-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"a0f10377ff35"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
qx.Class.define(d,{extend:qx.ui.virtual.layer.WidgetCell,construct:function(){qx.ui.virtual.layer.WidgetCell.call(this,this);
this._pool={atom:[],checkbox:[]};
this.__PI=[];
},members:{_pool:null,__PI:null,getCellData:function(i,k){if(!this.__PI[i]){this.__PI[i]=[];
}
if(!this.__PI[i][k]){this.__PI[i][k]={label:this.__PJ(),icon:this.__PK()};
}return this.__PI[i][k];
},getCellWidget:function(l,m){var n;

if(m%2==0){n=this._pool.atom.pop();

if(!n){n=new qx.ui.basic.Atom();
n.addListener(c,function(){var o=this.__PK();
n.setIcon(o);
this.__PI[l][m].icon=o;
},this);
}n.set(this.getCellData(l,m));
}else{n=this._pool.checkbox.pop();

if(!n){n=new qx.ui.form.CheckBox();
n.addListener(b,function(){this.setLabel(this.getLabel()==f?n.getUserData(g)+h+n.getUserData(e):f);
},n);
}n.set({value:l%2==0,label:l+h+m});
}n.setUserData(g,l);
n.setUserData(e,m);
return n;
},poolCellWidget:function(p){if(p.classname==a){this._pool.atom.push(p);
}else{this._pool.checkbox.push(p);
}},__PJ:function(){var name="";

for(var j=0;j<10;j++){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);
}return name;
},__PK:function(){var q="icon/";
var s="places/";
var t=["folder.png","user-trash.png","network-server.png","network-workgroup.png","user-desktop.png"];
var r=Math.floor(Math.random()*4);
return (q+16+"/"+s+t[r]);
}},destruct:function(){this._pool=this.__PI=null;
}});
})();
...
(function(){var k="scroll",j="scrollbar-x",i="scrollbar-y",h="_applyInterval",g="appear",f="interval",e="qx.ui.virtual.behavior.Prefetch",d="_applyScroller",c="qx.ui.virtual.core.Scroller",b="__Mh",a="Integer";
...
}},properties:{scroller:{check:c,nullable:true,init:null,apply:d},interval:{check:a,init:200,apply:h}},members:{__Mi:null,__Mj:null,__Mh:null,__Mk:null,__Ml:null,setPrefetchX:function(n,o,p,q){this.__Mi=[n,o,p,q];
},setPrefetchY:function(r,s,t,u){this.__Mj=[r,s,t,u];
},_onInterval:function(){var v=this.__Mi;
...
qx.ui.core.queue.Manager.flush();
}},_applyScroller:function(x,y){if(y){if(this.__Mk){y.getChildControl(j).removeListenerById(this.__Mk);
}
if(this.__Ml){y.getChildControl(i).removeListenerById(this.__Ml);
}}
if(x){if(!x.getContainerElement().getDomElement()){this.__Mh.stop();
x.addListenerOnce(g,this.__Mh.start,this.__Mh);
}else{this.__Mh.restart();
}this.__Mk=x.getChildControl(j).addListener(k,this.__Mh.restart,this.__Mh);
this.__Ml=x.getChildControl(i).addListener(k,this.__Mh.restart,this.__Mh);
}else{this.__Mh.stop();
}},_applyInterval:function(z,A){this.__Mh.setInterval(z);
}},destruct:function(){this.setScroller(null);
this.__Mi=this.__Mj=null;
