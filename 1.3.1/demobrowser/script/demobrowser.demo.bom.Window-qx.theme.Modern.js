...
qx.Class.define(m,{extend:qx.application.Native,members:{__Rz:null,main:function(){qx.application.Native.prototype.main.call(this);
var r=qx.util.ResourceManager.getInstance();
this.__Rz=[g,k,h,r.toUri(f)];
...
this.window1=qx.bom.Window.open(this.__Rz[0],j,x);
},this);
...
this.window2=qx.bom.Window.open(this.__Rz[3],a,y,true);
},this);
...
this.window3=qx.bom.Window.open(this.__Rz[0],i,A,true,false);
},this);
...
qx.Class.define(l,{statics:{__Nh:null,__Ni:null,__Nj:null,__Nk:{"top":b,left:d,width:j,height:q,scrollbars:h,resizable:s},__Nl:{"top":1,left:1,width:1,height:1,dependent:1,resizable:1,status:1,location:1,menubar:1,scrollbars:1,toolbar:1},__Nm:function(){return window.showModalDialog!=null;
},open:function(t,name,u,v,w){if(t==null){t=i;
}
if(name==null){name=f+new Date().getTime();
}
if(w==null){w=true;
}var x=this.__Nn(u,v&&w);

if(v){if(this.__Nm()&&w){return window.showModalDialog(t,[window.self],x);
}else{this.getBlocker().block();

if(this.__Nj==null){this.__Nj=new qx.event.Timer(500);
this.__Nj.addListener(k,this.__No,this);
}this.__Ni=window.open(t,name,x);
this.__Nj.restart();
return this.__Ni;
}}else{return window.open(t,name,x);
}},__Nn:function(y,z){var A;
var D;
var B=[];

if(z&&this.__Nm()){for(var C in y){if(qx.bom.Window.__Nk[C]){var E=n;

if(C!=e&&C!=s){E=m;
}D=qx.bom.Window.__Nk[C]+g+y[C]+E;
B.push(D);
}else{qx.log.Logger.warn("Option '"+C+"' is not supported for modal windows.");
}}A=B.join(c);
}else{for(var C in y){if(qx.bom.Window.__Nl[C]){if(qx.lang.Type.isBoolean(y[C])){D=C+r+(y[C]?o:p);
}else{D=C+r+y[C];
}B.push(D);
}else{qx.log.Logger.warn("Option '"+C+"' is not supported for native windows.");
}}A=B.join(a);
}return A;
},__No:function(){if(this.isClosed(this.__Ni)){this.getBlocker().unblock();
this.__Nj.stop();
}},getBlocker:function(){if(this.__Nh==null){this.__Nh=new qx.bom.Blocker;
}return this.__Nh;
},close:function(F){if(F){return F.close();
}},isClosed:function(G){var closed=true;

if(G){try{closed=G.closed;
}catch(H){}}return closed;
},moveTo:function(I,top,J){if(!qx.bom.Window.isClosed(I)){try{I.moveTo(J,top);
}catch(K){qx.log.Logger.error("Cross-Domain Scripting problem: Could not move window!",K);
}}},resizeTo:function(L,M,N){if(!qx.bom.Window.isClosed(L)){try{L.resizeTo(M,N);
...
(function(){var i="backgroundColor",h="px",g="zIndex",f="0px",d="resize",c="mshtml",b="qx.client",a="qx.bom.Blocker";
qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__MT();
},members:{__MM:null,__MN:null,__MO:null,__MP:false,__MQ:10000,__MR:0,__MS:"transparent",block:function(j){if(!this.__MP){this.__MO=j;
var k=this.__MW();
this.__MX(k);
this.__MP=true;
}},unblock:function(){if(this.__MP){this.__MY();
this.__MP=false;
}},isBlocked:function(){return this.__MP;
},getBlockerElement:function(){return this.__MN;
},setBlockerColor:function(l){qx.bom.element.Style.set(this.__MN,i,l);
},getBlockerColor:function(){return qx.bom.element.Style.get(this.__MN,i);
},setBlockerOpacity:function(m){qx.bom.element.Opacity.set(this.__MN,m);
},getBlockerOpacity:function(){return qx.bom.element.Opacity.get(this.__MN);
},setBlockerZIndex:function(n){qx.bom.element.Style.set(this.__MN,g,n);
},getBlockerZIndex:function(){return qx.bom.element.Style.get(this.__MN,g);
},__MT:function(){this.__MU();

if(qx.core.Variant.isSet("qx.client","mshtml")){this.__MV();
}qx.event.Registration.addListener(window,"resize",this.__Na,this);
},__MU:function(){this.__MN=qx.bom.Element.create("div");
qx.bom.element.Style.setStyles(this.__MN,{display:"block",opacity:this.__MR,backgroundColor:this.__MS});
this.setBlockerZIndex(this.__MQ);
},__MV:function(){this.__MM=qx.bom.Iframe.create();
qx.bom.element.Attribute.set(this.__MM,"allowTransparency",false);
qx.bom.element.Attribute.set(this.__MM,"src","javascript:false;");
qx.bom.element.Style.setStyles(this.__MM,{display:"block",opacity:this.__MR});
},__MW:function(){var o={position:"absolute"};

if(this.__Nc()){o.left="0px";
o.top="0px";
o.right=null;
o.bottom=null;
o.width=qx.bom.Document.getWidth()+"px";
o.height=qx.bom.Document.getHeight()+"px";
}else{o.width=qx.bom.element.Dimension.getWidth(this.__MO)+"px";
o.height=qx.bom.element.Dimension.getHeight(this.__MO)+"px";
o.left=qx.bom.element.Location.getLeft(this.__MO)+"px";
o.top=qx.bom.element.Location.getTop(this.__MO)+"px";
}return o;
},__MX:function(p){var q=document.body;
qx.bom.element.Style.setStyles(this.__MN,p);
qx.dom.Element.insertEnd(this.__MN,q);

if(qx.core.Variant.isSet("qx.client","mshtml")){p.zIndex=this.getBlockerZIndex()-1;
qx.bom.element.Style.setStyles(this.__MM,p);
qx.dom.Element.insertEnd(this.__MM,document.body);
}},__MY:function(){qx.dom.Element.remove(this.__MN);

if(qx.core.Variant.isSet("qx.client","mshtml")){qx.dom.Element.remove(this.__MM);
}},__Na:function(e){if(this.__Nc()){this.__Nb({width:f,height:f});
qx.event.Timer.once(function(){var r={width:qx.bom.Document.getWidth()+h,height:qx.bom.Document.getHeight()+h};
this.__Nb(r);
},this,0);
}},__Nb:function(s){qx.bom.element.Style.setStyles(this.__MN,s);

if(qx.core.Variant.isSet(b,c)){qx.bom.element.Style.setStyles(this.__MM,s);
}},__Nc:function(){return (this.__MO==null||qx.dom.Node.isWindow(this.__MO)||qx.dom.Node.isDocument(this.__MO));
}},destruct:function(){qx.event.Registration.removeListener(window,d,this.__Na,this);
