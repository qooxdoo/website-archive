...
}},properties:{model:{check:g,apply:a,event:h,nullable:true,dereference:true},target:{check:b,apply:f,event:d,nullable:true,init:null,dereference:true}},members:{__Lj:null,__Li:null,addBindingOptions:function(name,o,p){this.__Li[name]=[o,p];
if(this.getModel()==null||this.getTarget()==null){return;
}var q=this.getTarget().getItems()[name];
var r=this.__Lm(q)?k:j;
this.__Lj.removeTarget(q,r,name);
this.__Lj.addTarget(q,r,name,!this._selfUpdate,o,p);
...
}if(this.__Li[name]&&this.__Li[name][1]){t=this.__Li[name][1].converter(t);
}A[x[i]]=t;
}else{if(!A[x[i]]){A[x[i]]={};
}A=A[x[i]];
...
var D=this.__Lm(E)?k:j;
var C=this.__Li[name];
C=C&&this.__Li[name][1];
qx.data.SingleValueBinding.updateTarget(E,D,this.getModel(),name,C);
}},_applyTarget:function(F,G){if(G!=null){this.__Ll(G);
}if(this.getModel()==null){return;
}if(F!=null){this.__Lk();
}},_applyModel:function(H,I){if(this.__Lj!=null){var J=this.getTarget().getItems();

for(var name in J){var L=J[name];
var K=this.__Lm(L)?k:j;
this.__Lj.removeTarget(L,K,name);
}}if(this.__Lj!=null){this.__Lj.setModel(H);
}if(this.getTarget()==null){return;
}if(H!=null){this.__Lk();
}},__Lk:function(){if(this.__Lj==null){this.__Lj=new qx.data.controller.Object(this.getModel());
}var M=this.getTarget().getItems();
for(var name in M){var P=M[name];
var N=this.__Lm(P)?k:j;
var O=this.__Li[name];
try{if(O==null){this.__Lj.addTarget(P,N,name,!this._selfUpdate);
}else{this.__Lj.addTarget(P,N,name,!this._selfUpdate,O[0],O[1]);
}}catch(Q){{};
}}},__Ll:function(R){if(this.__Lj==null){return;
}var S=R.getItems();
for(var name in S){var U=S[name];
var T=this.__Lm(U)?k:j;
this.__Lj.removeTarget(U,T,name);
}},__Lm:function(V){return qx.Class.hasInterface(V.constructor,qx.ui.core.ISingleSelection)&&qx.Class.hasInterface(V.constructor,qx.ui.form.IModelSelection);
}},destruct:function(){if(this.__Lj){this.__Lj.dispose();
}}});
})();
...
}},properties:{model:{check:f,event:g,apply:b,nullable:true,dereference:true}},members:{__xu:null,__xt:null,_applyModel:function(k,l){for(var i=0;i<this.__xu.length;i++){var t=this.__xu[i][0];
var q=this.__xu[i][1];
var o=this.__xu[i][2];
var r=this.__xu[i][3];
var s=this.__xu[i][4];
var n=this.__xu[i][5];
if(l!=undefined&&!l.isDisposed()){this.__xw(t,q,o,l);
}if(k!=undefined){this.__xv(t,q,o,r,s,n);
...
}}}}},addTarget:function(u,v,w,x,y,z){this.__xu.push([u,v,w,x,y,z]);
this.__xv(u,v,w,x,y,z);
},__xv:function(A,B,C,D,E,F){if(this.getModel()==null){return;
...
if(this.__xt[I]==undefined){this.__xt[I]=[];
}this.__xt[I].push([G,H,B,C,E,F]);
},removeTarget:function(J,K,L){this.__xw(J,K,L,this.getModel());
for(var i=0;i<this.__xu.length;i++){if(this.__xu[i][0]==J&&this.__xu[i][1]==K&&this.__xu[i][2]==L){this.__xu.splice(i,1);
}}},__xw:function(M,N,O,P){if(!(M instanceof qx.core.Object)){return ;
}var Q=this.__xt[M.toHashCode()];
...
qx.Class.define(b,{statics:{toUriParameter:function(t,u,v){var z=o;
var A=qx.util.PropertyUtil.getProperties(t.constructor);

for(var name in A){var w=t[n+qx.lang.String.firstUp(name)]();
if(qx.lang.Type.isArray(w)){var y=w instanceof qx.data.Array;

for(var i=0;i<w.length;i++){var x=y?w.getItem(i):w[i];
z+=this.__JS(name,x,u);
}}else if(qx.lang.Type.isDate(w)&&v!=null){z+=this.__JS(name,v.format(w),u);
}else{z+=this.__JS(name,w,u);
}}return z.substring(0,z.length-1);
},__JS:function(name,B,C){if(B instanceof qx.core.Object&&C!=null){var D=encodeURIComponent(C(B));

if(D===undefined){var D=encodeURIComponent(B);
}}else{var D=encodeURIComponent(B);
}return encodeURIComponent(name)+"="+D+"&";
...
}var I=E[n+qx.lang.String.firstUp(name)]();
J[name]=qx.util.Serializer.toNativeObject(I,F,G);
}return J;
}if(qx.lang.Type.isDate(E)&&G!=null){return G.format(E);
}if(E instanceof qx.locale.LocalizedString){return E.toString();
}if(qx.lang.Type.isObject(E)){J={};

for(var H in E){J[H]=qx.util.Serializer.toNativeObject(E[H],F,G);
}return J;
}return E;
},toJson:function(M,N,O){var R=o;
if(M==null){return j;
}if(qx.Class.hasInterface(M.constructor,qx.data.IListData)){R+=q;

for(var i=0;i<M.getLength();i++){R+=qx.util.Serializer.toJson(M.getItem(i),N,O)+p;
}
if(R!=q){R=R.substring(0,R.length-1);
}return R+m;
}if(qx.lang.Type.isArray(M)){R+=q;

for(var i=0;i<M.length;i++){R+=qx.util.Serializer.toJson(M[i],N,O)+p;
}
if(R!=q){R=R.substring(0,R.length-1);
}return R+m;
}if(M instanceof qx.core.Object){if(N!=null){var S=N(M);
if(S!=undefined){return s+S+s;
}}R+=r;
var T=qx.util.PropertyUtil.getProperties(M.constructor);

for(var name in T){if(T[name].group!=undefined){continue;
}var Q=M[n+qx.lang.String.firstUp(name)]();
R+=s+name+k+qx.util.Serializer.toJson(Q,N,O)+p;
}
if(R!=r){R=R.substring(0,R.length-1);
}return R+l;
}if(M instanceof qx.locale.LocalizedString){M=M.toString();
}if(qx.lang.Type.isDate(M)&&O!=null){return s+O.format(M)+s;
}if(qx.lang.Type.isObject(M)){R+=r;

for(var P in M){R+=s+P+k+qx.util.Serializer.toJson(M[P],N,O)+p;
}
if(R!=r){R=R.substring(0,R.length-1);
}return R+l;
}if(qx.lang.Type.isString(M)){M=M.replace(/([\\])/g,a);
M=M.replace(/(["])/g,c);
M=M.replace(/([\r])/g,e);
M=M.replace(/([\f])/g,g);
M=M.replace(/([\n])/g,h);
...
M=M.replace(/([\b])/g,f);
return s+M+s;
}if(qx.lang.Type.isDate(M)||qx.lang.Type.isRegExp(M)){return s+M+s;
}return M+o;
}}});
})();
