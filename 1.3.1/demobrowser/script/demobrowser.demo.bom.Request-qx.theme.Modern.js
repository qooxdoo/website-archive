...
qx.Class.define(c,{extend:qx.core.Object,construct:function(){this.__HW={};
this.__HX=this.__Ii();
},statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},members:{__HY:null,__Ia:null,__HX:null,__Ib:null,__Ic:null,__Id:null,__Ie:null,__HW:null,readyState:0,responseText:f,responseXML:null,status:0,statusText:f,timeout:0,onreadystatechange:function(){},ontimeout:function(){},onload:function(){},onerror:function(){},onabort:function(){},open:function(j,k,l,m,n){if(l==null){l=true;
}this.__HY=l;
this.__Ia=qx.lang.Function.bind(this.__If,this);
this.__Ib=qx.lang.Function.bind(this.__Ig,this);
this.__HX.onreadystatechange=this.__Ia;
if(arguments.length>4){this.__HX.open(j,k,l,m,n);
}else if(arguments.length>3){this.__HX.open(j,k,l,m);
}else{this.__HX.open(j,k,l);
}if(qx.core.Variant.isSet(i,h)){if(!l){this.readyState=qx.bom.Request.OPENED;
this.__Ij();
}}},send:function(o){var p=this.__HW;
if(o&&o.nodeType){o=window.XMLSerializer?new XMLSerializer().serializeToString(o):o.xml;

if(!p[g]){p[g]=d;
}}for(var q in p){this.__HX.setRequestHeader(q,p[q]);
}if(this.timeout!=null&&this.timeout>0){this.__Ic=window.setTimeout(this.__Ib,this.timeout);
}this.__HX.send(o);
if(qx.core.Variant.isSet(i,h)){if(!this.__HY){this.readyState=qx.bom.Request.OPENED;
this.__Il(this);
...
},abort:function(){if(this.__Ic){window.clearTimeout(this.__Ic);
}this.__Ih();
this.onabort();
this.dispose();
},__If:qx.event.GlobalError.observeMethod(function(){if(qx.core.Variant.isSet(i,h)){if(!this.__HY){return;
}}this.readyState=this.__HX.readyState;
this.__Il();
if(this.__Id){this.readyState=qx.bom.Request.UNSENT;
return ;
}if(this.readyState==qx.bom.Request.DONE&&this.__Ic){window.clearTimeout(this.__Ic);
}this.__Ij();
if(this.readyState==qx.bom.Request.DONE){this.dispose();
}}),__Ig:qx.event.GlobalError.observeMethod(function(){this.__Ih();
this.ontimeout();
this.dispose();
}),getAllResponseHeaders:function(){try{return this.__HX.getAllResponseHeaders();
}catch(r){return null;
}},getResponseHeader:function(s){try{return this.__HX.getResponseHeader(s);
}catch(t){return null;
}},setRequestHeader:function(u,v){if(v==null){delete this.__HW[u];
}else{this.__HW[u]=v;
}},removeRequestHeader:function(w,x){delete this.__HW[w];
},getRequestHeader:function(y){return this.__HW[y]||null;
},__Ih:function(){this.__Il();
if(this.readyState>qx.bom.Request.UNSENT){this.__Id=true;
}this.__HX.abort();
},__Ii:qx.core.Variant.select(i,{"default":function(){return new XMLHttpRequest;
},"mshtml":function(){if(window.ActiveXObject&&qx.xml.Document.XMLHTTP){return new ActiveXObject(qx.xml.Document.XMLHTTP);
}
if(window.XMLHttpRequest){return new XMLHttpRequest;
}}}),__Ij:function(){if(this.__Ie===this.readyState){return;
}this.onreadystatechange();
this.__Ie=this.readyState;
if(this.readyState===4){if(this.isSuccessful()){this.onload();
}else{this.onerror();
}}},__Ik:function(){var z=this.__HX.responseXML;

if(qx.core.Variant.isSet(i,e)){if(z&&!z.documentElement&&this.__HX.getResponseHeader(g).match(/[^\/]+\/[^\+]+\+xml/)){z=new ActiveXObject(qx.xml.Document.DOMDOC);
z.loadXML(this.__HX.responseText);
}if(z&&z.parseError!=0){return null;
}}else if(!z.documentElement||z.documentElement.tagName==a){return null;
}return z;
},__Il:function(){var A=this.__HX;

try{this.responseText=A.responseText;
}catch(B){}
try{this.responseXML=this.__Ik();
}catch(C){}
...
}}},destruct:function(){if(this.__Ic){window.clearTimeout(this.__Ic);
this.__Ic=null;
}if(this.__HX){this.__HX.onreadystatechange=qx.lang.Function.empty;
this.__HX=null;
