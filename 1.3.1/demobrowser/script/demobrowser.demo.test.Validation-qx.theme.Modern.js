...
}R.setHtml(R.getHtml()+I+J);
Q.setGender(f);
R.setHtml(R.getHtml()+h+Q.getGender()+j);
R.setHtml(R.getHtml()+G+t);
try{Q.setAge(L);
R.setHtml(R.getHtml()+h+Q.getGender()+j);
}catch(V){R.setHtml(R.getHtml()+y+q+Q.getGender()+j);
...
qx.Class.define(k,{extend:qx.core.Object,properties:{name:{init:F,validate:M},age:{init:18,validate:qx.util.Validate.number()},gender:{init:g,validate:qx.util.Validate.inArray([g,f])}},members:{__Oh:function(W){if(W.length<2){throw new qx.core.ValidationError(B+H+W+O);
}}}});
})();
