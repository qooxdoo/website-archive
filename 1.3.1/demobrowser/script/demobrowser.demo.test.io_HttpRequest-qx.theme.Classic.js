...
(function(){var j="qx.event.type.Event",i="Boolean",h="String",g="Last-Modified",f="basic",e="GET",d="load",c="text/plain",b="abort",a="http",M="POST",L="error",K="timeout",J="application/xml",I="X-Requested-With",H="application/json",G="text/html",F="If-Modified-Since",E='Basic ',D="Integer",q="qx.event.type.Data",r="HEAD",o="XMLHttpRequest",p='Authorization',m="change",n="Accept",k="OPTIONS",l="text/javascript",s="",t="PUT",w=':',v="application/x-www-form-urlencoded",y="DELETE",x="__Ms",A="qx.io.HttpRequest",z="no-cache",u="Cache-Control",C="Content-Type",B="Thu, 01 Jan 1970 00:00:00 GMT";
qx.Class.define(A,{extend:qx.core.Object,construct:function(N){qx.core.Object.call(this);
this.__Mq={};
if(N!=null){this.setUrl(N);
}},statics:{__Mr:{}},events:{"change":q,"timeout":j,"load":j,"error":j,"abort":j},properties:{refresh:{check:i,init:false},data:{nullable:true},method:{check:[e,M,t,r,y,k],init:e},async:{check:i,init:true},mime:{check:[c,l,H,J,G],init:c},url:{check:h,init:s},username:{check:h,nullable:true},password:{check:h,nullable:true},auth:{check:[a,f],init:a},timeout:{check:D,nullable:true},cache:{check:i,init:false}},members:{__Ms:null,__Mq:null,setRequestHeader:function(O,P){this.__Mq[O]=P;
},removeRequestHeader:function(Q){delete this.__Mq[Q];
},getRequestHeader:function(R){var S=this.__Mq[R];

if(S===undefined){S=null;
}return S;
},getResponseText:function(){var T=this.__Ms;

if(T){return T.responseText;
}},getResponseXml:function(){var U=this.__Ms;

if(U){return U.responseXML;
}},getResponseHeader:function(V){var W=this.__Ms;

if(W){return W.getResponseHeader(V);
}},getAllResponseHeaders:function(){var X=this.__Ms;

if(X){return X.getAllResponseHeaders();
}},isNotModified:function(){var Y=this.__Ms;

if(!Y){return false;
}var ba=Y.getResponseHeader(g);
return Y.status===304||qx.io.HttpRequest.__Mr[this.getUrl()]===ba;
},isSuccessful:function(){var bb=this.__Ms;
return !bb||bb.isSuccessful();
},getStatusCode:function(){var bc=this.__Ms;

if(bc){return bc.status;
}},getStatusText:function(){var bd=this.__Ms;

if(bd){return bd.statusText;
}},getReadyState:function(){var be=this.__Ms;

if(be){return be.readyState;
}},send:function(){if(this.__Ms){if(this.getReadyState()!==4){throw new Error("Request is still pending at ready state: "+this.getReadyState());
}this.__Ms.dispose();
}var bh=this.__Ms=new qx.bom.Request;
bh.onreadystatechange=qx.lang.Function.bind(this.__Mt,this);
bh.ontimeout=qx.lang.Function.bind(this.__Mu,this);
bh.onload=qx.lang.Function.bind(this.__Mv,this);
bh.onerror=qx.lang.Function.bind(this.__Mw,this);
bh.onabort=qx.lang.Function.bind(this.__Mx,this);
var bf=this.getUsername();
var bj=this.getPassword();

if(this.getAuth()==f){bh.setRequestHeader(p,E+qx.util.Base64.encode(bf+w+bj));
bf=bj=null;
}var bg=this.getUrl();
bh.timeout=this.getTimeout();
bh.open(this.getMethod(),bg,this.getAsync(),bf,bj);
if(!this.getCache()){bh.setRequestHeader(u,z);
}if(this.getRefresh()){bh.setRequestHeader(F,qx.io.HttpRequest.__Mr[bg]||B);
}bh.setRequestHeader(I,o);
if(this.getMethod()===M){bh.setRequestHeader(C,v);
}bh.setRequestHeader(n,this.getMime());
var bi=this.__Mq;

for(var name in bi){bh.setRequestHeader(name,bi[name]);
}bh.send(this.getData());
},abort:function(){if(this.__Ms){this.__Ms.abort();
}},__Mt:qx.event.GlobalError.observeMethod(function(){this.fireDataEvent(m,this.getReadyState());
if(this.getRefresh()&&this.getReadyState()===4&&this.isSuccessful()){var bk=this.getResponseHeader(g);

if(bk){qx.io.HttpRequest.__Mr[this.getUrl()]=bk;
}}}),__Mu:qx.event.GlobalError.observeMethod(function(){if(this.hasListener(K)){this.fireEvent(K);
}}),__Mv:qx.event.GlobalError.observeMethod(function(){if(this.hasListener(d)){this.fireEvent(d);
}}),__Mw:qx.event.GlobalError.observeMethod(function(){if(this.hasListener(L)){this.fireEvent(L);
}}),__Mx:qx.event.GlobalError.observeMethod(function(){if(this.hasListener(b)){this.fireEvent(b);
...
qx.Class.define(b,{statics:{__My:a,encode:function(d){var j=this.__My;
var n=c;
var g,f,e;
var k,l,m,h;
var i=0;
