...
this.__Oz(D,C);
},__Oz:function(E,F){var G=new F.clazz();
...
if(J!=null){for(var i=0;i<J.length;i++){if(G instanceof qx.ui.form.Form){this.__OA(G,J[i]);
}else{this.__Oz(G,J[i]);
...
}}},__OA:function(L,M){if(M.type==="Header"){L.addGroupHeader(M.label);
return;
