...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.ui.FiniteStateMachine","qx.theme":"qx.theme.Modern"};
...
(function(){var u="<p>",t="demobrowser/demo/ui/FsmMiceMaze.png",s="FadingOut, and Zombie (about to dispose).",r="which it was instantiated); when it ",q="Each mouse is driven by (and in fact, in this example, is a ",p="The initial mouse is a bit special, in that it looks for ",n="This is a simple example of using a finite state machine, in ",m="which virtual mice solve a maze. ",l="Each mouse is quite dumb, having in ",k="arrives at a new cell, it looks left and right and creates a ",d="<u>The finite state diagram is below the maze.</u>",j="its arsenal of knowledge only these characteristics: it knows ",g="subclass of) a finite state machine.  The FSM has five states: ",c="clone of itself to go each available direction; upon hitting a ",b="only how to travel in a single direction (the direction in ",f="demobrowser.demo.ui.FiniteStateMachine",e="wall, it dies.",h="FadingIn, CellArrival (look for cloning opportunities), Moving, ",a="behind it, as well as to its left and right.",i="the possibility to clone a mouse to travel in the direction ";
...
{};
var w=n+m+u+l+j+b+r+k+c+e+u+p+i+a+u+q+g+h+s+u+d;
var scroll=new qx.ui.container.Scroll();
this.getRoot().add(scroll,{edge:0});
var v=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
scroll.add(v);
var o;
o=new qx.ui.basic.Image(t);
o=null;
o=new qx.ui.basic.Label(w);
o.set({width:600,rich:true,wrap:true});
v.add(o,{left:50});
var z=new demobrowser.demo.util.FSMMaze(10,10,50,240);
v.add(z,{left:50,top:240});
o=new qx.ui.basic.Image(t,800);
v.add(o,{left:50,top:800});
var y;
var x=Math.floor(Math.random()*4);

switch(x){case 0:y=demobrowser.demo.util.FSMMaze.Direction.NORTH;
break;
case 1:y=demobrowser.demo.util.FSMMaze.Direction.EAST;
break;
case 2:y=demobrowser.demo.util.FSMMaze.Direction.SOUTH;
break;
case 3:y=demobrowser.demo.util.FSMMaze.Direction.WEST;
break;
}new demobrowser.demo.util.FSMMouse(null,z,y,v);
...
(function(){var h="String",g="Number",f="__Kr",d="__Ks",c="string",b="qx.util.fsm.FiniteStateMachine";
...
if(!this.__Kw[u]){this.__Kw[u]={};
}this.__Kw[u][q]=true;
if(!this.__Kx[q]){this.__Kx[q]=[];
}this.__Kx[q]=this.__Kx[q].concat(s);
}},removeObject:function(w){var y=this.__Ku[w];
if(this.__Kx[w]){for(var x in this.__Kx[w]){delete this.__Kw[x];
}delete this.__Kx[w];
}delete this.__Kv[y];
delete this.__Ku[w];
delete this.__Kt[w];
},getObject:function(z){return this.__Kt[z];
},getFriendlyName:function(A){var B=A.toHashCode();
return B?this.__Kv[B]:null;
},getGroupObjects:function(C){var a=[];

for(var name in this.__Kw[C]){a.push(name);
}return a;
},displayAllObjects:function(){for(var D in this.__Ku){var F=this.__Ku[D];
var E=this.getObject(D);
this.debug(D+" => "+F);
this.debug("  "+F+" => "+this.__Kv[F]);
this.debug("  "+D+" => "+this.getObject(D));
this.debug("  "+this.getObject(D)+" => "+this.getFriendlyName(E));
}},start:function(){var H=this.__Kp;
...
}this.__Ko[H].getAutoActionsBeforeOnentry()(this);
if(G){this.debug(this.getName()+"#"+H+"#entry");
}this.__Ko[H].getOnentry()(this,null);
if(G){this.debug(this.getName()+"#"+H+"#actionsAfterOnentry");
}this.__Ko[H].getAutoActionsAfterOnentry()(this);
},pushState:function(I){if(this.__Kq.length>=this.getMaxSavedStates()){throw new Error("Saved-state stack is full");
}
if(I===true){this.__Kq.push(this.getState());
}else if(I){this.__Kq.push(I);
}else{this.__Kq.push(this.getPreviousState());
}},postponeEvent:function(event){this.__Ks.unshift(event);
},enqueueEvent:function(event,J){if(J){this.__Kr.push(event);
}else{this.__Kr.unshift(event);
}
if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){if(J){this.debug(this.getName()+": Pushed event: "+event.getType());
...
this.__Kz();
},fireImmediateEvent:function(K,L,M){if(M){var event=qx.event.Registration.createEvent(K,qx.event.type.Data,[M,null,false]);
}else{var event=qx.event.Registration.createEvent(K,qx.event.type.Event,[false,false]);
}event.setTarget(L);
this.eventListener(event);
},scheduleEvent:function(N,O,P,Q){qx.event.Timer.once(function(){this.fireImmediateEvent(N,O,P);
},this,Q||20);
},__Kz:function(){if(this.__Ky){return ;
}this.__Ky=true;
while(this.__Kr.length>0){var event=this.__Kr.pop();
var R=this.__KA(event);
if(R){event.dispose();
}}this.__Ky=false;
},__KA:function(event){var U;
var W;
var T;
var bf;
var bd;
var e;
var bb;
var S=this.getDebugFlags();
var be=S&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;
var X=S&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;
var bc=S&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;
var ba=S&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;

if(be){this.debug(this.getName()+": Process event: "+event.getType());
}U=this.getState();
bf=this.__Ko[U];
bd=bf.transitions;
e=bf.getEvents()[event.getType()];
if(!e){if(be){this.debug(this.getName()+": Event '"+event.getType()+"'"+" not handled.  Ignoring.");
}return true;
}if(typeof (e)=="object"){var V=this.getFriendlyName(event.getTarget());

if(!V){if(ba){this.debug(this.getName()+": Could not find friendly name for '"+event.getType()+"' on '"+event.getTarget()+"'");
}return true;
}bb=e[V];
if(!bb){if(be){this.debug(this.getName()+": Event '"+event.getType()+"'"+" not handled for target "+V+".  Ignoring.");
}return true;
}}else{bb=e;
}
switch(bb){case qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE:break;
case qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED:if(be){this.debug(this.getName()+": Event '"+event.getType()+"'"+" blocked.  Re-queuing.");
}this.__Ks.unshift(event);
return false;
default:if(typeof (bb)=="string"){if(bd[bb]){var Y=bd[bb];
bd={};
bd[bb]=Y;
}else{throw new Error("Explicit transition "+bb+" does not exist");
}break;
}}for(var t in bd){var Y=bd[t];
switch(Y.getPredicate()(this,event)){case true:break;
case false:continue;
case null:return true;
default:throw new Error("Transition "+U+":"+t+" returned a value other than "+"true, false, or null.");
}W=Y.getNextState();

if(typeof (W)=="string"){if(!W in this.__Ko){throw new Error("Attempt to transition to nonexistent state "+W);
}this.setNextState(W);
}else{switch(W){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:W=U;
this.setNextState(W);
break;
case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Kq.length==0){throw new Error("Attempt to transition to POP_STATE_STACK "+"while state stack is empty.");
}W=this.__Kq.pop();
this.setNextState(W);
break;
default:throw new Error("Internal error: invalid nextState");
break;
}}if(bc){this.debug(this.getName()+"#"+U+"#"+t+"#autoActionsBeforeOntransition");
}Y.getAutoActionsBeforeOntransition()(this);
if(bc){this.debug(this.getName()+"#"+U+"#"+t+"#ontransition");
}Y.getOntransition()(this,event);
if(bc){this.debug(this.getName()+"#"+U+"#"+t+"#autoActionsAfterOntransition");
}Y.getAutoActionsAfterOntransition()(this);
if(bc){this.debug(this.getName()+"#"+U+"#autoActionsBeforeOnexit");
}bf.getAutoActionsBeforeOnexit()(this);
if(bc){this.debug(this.getName()+"#"+U+"#exit");
}bf.getOnexit()(this,event);
if(bc){this.debug(this.getName()+"#"+U+"#autoActionsAfterOnexit");
}bf.getAutoActionsAfterOnexit()(this);
if(bf._bNeedDispose){bf.dispose();
}bf=this.__Ko[this.getNextState()];
this.setPreviousState(U);
this.setState(this.getNextState());
this.setNextState(null);
T=U;
U=W;
W=undefined;
if(bc){this.debug(this.getName()+"#"+U+"#autoActionsBeforeOnentry");
}bf.getAutoActionsBeforeOnentry()(this);
if(bc){this.debug(this.getName()+"#"+U+"#entry");
}bf.getOnentry()(this,event);
if(bc){this.debug(this.getName()+"#"+U+"#autoActionsAfterOnentry");
}bf.getAutoActionsAfterOnentry()(this);
var e;

for(var i=0;i<this.__Ks.length;i++){e=this.__Ks.pop();
this.__Kr.unshift(e);
}
if(X){this.debug(this.getName()+"#"+T+" => "+this.getName()+"#"+U);
}return true;
}
if(X){this.debug(this.getName()+"#"+U+": event '"+event.getType()+"'"+": no transition found.  No state change.");
}return true;
}},destruct:function(){this._disposeArray(f);
this._disposeArray(d);
this.__Kq=this.__Ko=null;
}});
})();
...
(function(){var p="string",o="object",n="number",m="function",l="autoActionsAfterOnentry",k='"',h="autoActionsBeforeOnexit",d="autoActionsBeforeOnentry",c="}",b="{",ba="undefined",Y="autoActionsAfterOnexit",X="(",W=";",V="  var groupObjects = ",U="parameters",T=")",S="__Kg",R=" fsm.getObject('",Q="groups",w="__Kk",x="  for (var i = 0; i < groupObjects.length; i++)",u="__Kl",v="__Ki",s="fsm",t="');",q="onexit",r="  fsm.debug(ex);",y="    fsm.getObject(objName).",z=")(fsm)",G="events",E="__Kh",K="    var objName = groupObjects[i];",I="').",M="objects",L="__Km",B="catch(ex)",P="  {",O="    fsm.getGroupObjects('",N="__Kn",A="  }",C="__Kj",D="qx.util.fsm.State",F="onentry",H="try",J=",";
qx.Class.define(D,{extend:qx.core.Object,construct:function(bb,bc){qx.core.Object.call(this);
this.setName(bb);
if(typeof (bc)!=o){throw new Error("State info must be an object");
}for(var bd in bc){switch(bd){case F:this.setOnentry(bc[bd]);
break;
case q:this.setOnexit(bc[bd]);
break;
case d:this.setAutoActionsBeforeOnentry(bc[bd]);
break;
case l:this.setAutoActionsAfterOnentry(bc[bd]);
break;
case h:this.setAutoActionsBeforeOnexit(bc[bd]);
break;
case Y:this.setAutoActionsAfterOnexit(bc[bd]);
break;
case G:this.setEvents(bc[bd]);
break;
default:this.setUserData(bd,bc[bd]);
this.debug("State "+bb+": "+"Adding user-provided field to state: "+bd);
break;
}}if(!this.getEvents()){throw new Error("The events object must be provided in new state info");
}this.transitions={};
},statics:{_commonTransformAutoActions:function(be,bf){if(typeof (bf)!=o){throw new Error("Invalid "+be+" value: "+typeof (bf));
}var bk;
var bl=H+b;
var bi;
var bg;
for(var f in bf){var bh=bf[f];
if(!bh instanceof Array){throw new Error("Invalid function request type: "+"expected array, found "+typeof (bh));
}for(var i=0;i<bh.length;i++){bg=bh[i];
if(typeof (bg)!=o){throw new Error("Invalid function request parameter type: "+"expected object, found "+typeof (bh[bi]));
}var bj=bg[U];
if(!bj){bj=[];
}else{if(!bj instanceof Array){throw new Error("Invalid function parameters: "+"expected array, found "+typeof (bj));
}}bk=f+X;
for(var j=0;j<bj.length;j++){if(j!=0){bk+=J;
}
if(typeof (bj[j])==m){bk+=X+bj[j]+z;
}else if(typeof (bj[j])==p){bk+=k+bj[j]+k;
}else{bk+=bj[j];
}}bk+=T;
var a=bg[M];
if(!a){a=[];
...
for(var j=0;j<a.length;j++){if(typeof (a[j])!=p){throw new Error("Invalid friendly name in 'objects' list: "+a[j]);
}bl+=R+a[j]+I+bk+W;
}var g=bg[Q];
if(g){if(!g instanceof Array){throw new Error("Invalid 'groups' list: expected array, got "+typeof (g));
}
for(j=0;j<g.length;j++){bl+=V+O+g[j]+t+x+P+K+y+bk+W+A;
}}}}bl+=c+B+b+r+c;
return new Function(s,bl);
}},properties:{name:{transform:S,nullable:true},onentry:{transform:E,nullable:true,init:function(bm,event){}},onexit:{transform:v,nullable:true,init:function(bn,event){}},autoActionsBeforeOnentry:{transform:w,nullable:true,init:function(bo,event){}},autoActionsAfterOnentry:{transform:u,nullable:true,init:function(bp,event){}},autoActionsBeforeOnexit:{transform:L,nullable:true,init:function(bq,event){}},autoActionsAfterOnexit:{transform:N,nullable:true,init:function(br,event){}},events:{transform:C,nullable:true}},members:{__Kg:function(bs){if(typeof (bs)!=p||bs.length<1){throw new Error("Invalid state name");
}return bs;
},__Kh:function(bt){switch(typeof (bt)){case ba:return function(bu,event){};
case m:return bt;
default:throw new Error("Invalid onentry type: "+typeof (bt));
}},__Ki:function(bv){switch(typeof (bv)){case ba:return function(bw,event){};
case m:return bv;
default:throw new Error("Invalid onexit type: "+typeof (bv));
return null;
}},__Kj:function(bx){if(typeof (bx)!=o){throw new Error("events must be an object");
}for(var e in bx){var by=bx[e];

if(typeof (by)==n&&by!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&by!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error("Invalid numeric value in events object: "+e+": "+by);
}else if(typeof (by)==o){for(var bz in by){if(typeof (by[bz])==n&&by[bz]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&by[bz]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error("Invalid numeric value in events object "+"("+e+"): "+bz+": "+by[bz]);
}else if(typeof (by[bz])!=p&&typeof (by[bz])!=n){throw new Error("Invalid value in events object "+"("+e+"): "+bz+": "+by[bz]);
}}}else if(typeof (by)!=p&&typeof (by)!=n){throw new Error("Invalid value in events object: "+e+": "+bx[e]);
}}return bx;
},__Kk:function(bA){return qx.util.fsm.State._commonTransformAutoActions(d,bA);
},__Kl:function(bB){return qx.util.fsm.State._commonTransformAutoActions(l,bB);
},__Km:function(bC){return qx.util.fsm.State._commonTransformAutoActions(h,bC);
},__Kn:function(bD){return qx.util.fsm.State._commonTransformAutoActions(Y,bD);
},addTransition:function(bE){if(!bE instanceof qx.util.fsm.Transition){throw new Error("Invalid transition: not an instance of "+"qx.util.fsm.Transition");
}this.transitions[bE.getName()]=bE;
}}});
})();
(function(){var j="predicate",i="nextState",h="__KD",g="autoActionsBeforeOntransition",f="__KB",e="__KC",d="autoActionsAfterOntransition",c="qx.util.fsm.Transition",b="ontransition",a="String";
qx.Class.define(c,{extend:qx.core.Object,construct:function(k,l){qx.core.Object.call(this);
this.setName(k);
for(var m in l){switch(m){case j:this.setPredicate(l[m]);
break;
case i:this.setNextState(l[m]);
break;
case g:this.setAutoActionsBeforeOntransition(l[m]);
break;
case d:this.setAutoActionsAfterOntransition(l[m]);
break;
case b:this.setOntransition(l[m]);
break;
default:this.setUserData(m,l[m]);
this.debug("Transition "+k+": "+"Adding user-provided field to transition: "+m);
break;
}}},properties:{name:{check:a,nullable:true},predicate:{init:function(n,event){return true;
},transform:f},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:e},autoActionsBeforeOntransition:{init:function(o,event){}},autoActionsAfterOntransition:{init:function(p,event){}},ontransition:{init:function(q,event){},transform:h}},members:{__KB:function(r){switch(typeof (r)){case "undefined":return function(s,event){return true;
};
case "boolean":return function(t,event){return r;
};
case "function":return r;
default:throw new Error("Invalid transition predicate type: "+typeof (r));
break;
}},__KC:function(u){switch(typeof (u)){case "string":return u;
case "number":switch(u){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return u;
default:throw new Error("Invalid transition nextState value: "+u+": "+"nextState must be an explicit state name, "+"or one of the Fsm.StateChange constants");
}break;
default:throw new Error("Invalid transition nextState type: "+typeof (u));
break;
}},__KD:function(v){switch(typeof (v)){case "undefined":return function(w,event){};
case "function":return v;
default:throw new Error("Invalid ontransition type: "+typeof (v));
break;
}}}});
})();
