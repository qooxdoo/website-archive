...
(function(){var l="row-layer",k="selected",j="grid-lines",h="Boolean",g="Integer",f="cell.row",d="cell.renderer",c="_applyDelegate",b="qx.ui.virtual.form.List",a="__IB",A="Object",z="_applyCellRenderer",y="changeRowHeight",x="_applyRowCount",w="changeCellRenderer",v="_applyShowGridLines",u="resize",t="changeShowGridLines",s="changeDelegate",r="_applyUseWidgetCells",p="changeRowCount",q="__IC",n="_applyRowHeight",o="horizontal",m="virtual-list";
...
this.initRowHeight();
this.initDelegate();
},properties:{appearance:{refine:true,init:m},rowCount:{check:g,event:p,init:0,apply:x},rowHeight:{check:g,event:y,init:20,apply:n,themeable:true},showGridLines:{check:h,event:t,init:false,apply:v,themeable:true},useWidgetCells:{check:h,init:false,apply:r,themeable:true},cellRenderer:{event:w,apply:z,themeable:true},delegate:{check:A,event:s,init:null,nullable:true,apply:c}},members:{__IA:null,__IB:null,__IC:null,__ID:null,_initWidgetLayer:function(){var self=this;
var B={getCellWidget:function(C,D){var H=self._getCellData(C);

if(!H){return null;
}var E={};

if(self.__IB.isItemSelected(C)){E.selected=true;
...
this.__IC=new qx.ui.virtual.layer.WidgetCell(B);
this.getPane().addLayer(this.__IC);

if(!this.__IA){this.setCellRenderer(qx.ui.virtual.form.ListItemCell.getInstance());
}},_initHtmlLayer:function(){var self=this;
var K={getCellProperties:function(L,M){var N={};

if(self.__IB.isItemSelected(L)){N.selected=true;
}return self._getCellRenderer(L).getCellProperties(self._getCellData(L),N);
}};
this._showChildControl(l);
this.__IC=new qx.ui.virtual.layer.HtmlCell(K);
this.getPane().addLayer(this.__IC);

if(!this.__IA){this.setCellRenderer(new qx.ui.virtual.cell.Cell());
...
},styleSelectable:function(Q,R,S){if(self.__ID){self._styleWidgetSelectable(Q,R,S);
}else{self._styleHtmlSelectable(Q,R,S);
}}};
this.__IB=new qx.ui.virtual.selection.Row(this.getPane(),O);
this.__IB.attachMouseEvents(this.getPane());
this.__IB.attachKeyEvents(this);
},getSelectionManager:function(){return this.__IB;
...
},update:function(){if(this.__IC){this.__IC.updateLayerData();
...
},_applyUseWidgetCells:function(bf,bg){if(this.__ID!==null){throw new Error("The property 'useWidgetCells' cannot be set after the list has "+"been rendered.");
}},_applyCellRenderer:function(bh,bi){this.__IA=bh;

if(this.__IC){this.__IC.fullUpdate();
}},_getCellData:function(bj){return this._delegate.getCellData?this._delegate.getCellData(bj):null;
},_getCellRenderer:function(bk){return this._delegate.getCellRenderer?this._delegate.getCellRenderer(bk):this.__IA;
...
}}},syncWidget:function(){if(this.__ID!==null){return;
}this.__ID=this.getUseWidgetCells();

if(this.__ID){this._initWidgetLayer();
}else{this._initHtmlLayer();
}},_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);
}},destruct:function(){this._delegate=this.__IA=null;
this._disposeObjects(a,q);
}});
})();
...
if(this.__Jp.length==0){return;
}var E=this._getDelegate(w);

if(E!=null){this.__Jp.sort(function(a,b){return E(D.getItem(a),D.getItem(b));
});
}},_runDelegateFilter:function(F){if(typeof F!==k){return;
}var G=this._getDelegate(r);

for(var i=0,l=F.length;i<l;++i){if(G==null||G(F.getItem(i))){this.__Jp.push(i);
...
},__Jx:function(N){return this.__Jp[N];
},_applyDelegate:function(O,P){if(this.getTarget()==null||this.getModel()==null){return;
}
if(this._containsDelegateMethod(O)){this.__Jw();
}},_applyTarget:function(Q,R){if(Q!=null){Q.setDelegate(this);
this.__Jt=Q.getSelectionManager().addListener(u,this._onChangeSelectionView,this);
}
if(R!=null){R.setDelegate(null);
R.getSelectionManager().removeListenerById(this.__Jt);
}
if(this.getModel()==null){return;
}this.__Jw();
this._syncRowCount();
},_applyModel:function(S,T){if(T!=null){T.removeListenerById(this.__Jq);
T.removeListenerById(this.__Jr);
T.removeListenerById(this.__Js);
}
if(S!=null){this.__Jw();
this.__Jq=S.addListener(t,this._onChangeLengthModel,this);
this.__Jr=S.addListener(v,this._onChangeModel,this);
this.__Js=S.addListener(h,this._onChangeBubbleModel,this);
}
if(this.getTarget()!=null){this._syncRowCount();
}},_applySelection:function(U,V){if(U!=null){this.__Ju=U.addListener(v,this._onChangeSelectionModel,this);
this.__Jv=U.addListener(t,this._onChangeSelectionModel,this);
this._syncModelSelectionToView();
}
if(V!=null){V.removeListenerById(this.__Ju);
V.removeListenerById(this.__Jv);
...
},_onChangeLengthModel:function(e){this.__Jw();
this._syncRowCount();
