...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.data.JsonToList","qx.theme":"qx.theme.Classic"};
for (var k in settings) qxsettings[k] = settings[k];

if (!window.qxvariants) qxvariants = {};
var variants = {"qx.aspects":"off","qx.data":"on","qx.debug":"off"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.data.JsonToList-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"110c17b667fe"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var p="icon/16/mimetypes/",o="name",n="demobrowser/demo/data/list.json",m="model.items",l="Loading...",k=" will be created by a converter which converts the type to an icon url.",j=".png",i="model",h="state",g="type",c="list.json</a> and bind the items to the list widget. The icons ",f="' target='_blank'>",e="demobrowser.demo.data.JsonToList",b="<b>List bound to data in a json file</b><br/>",a="Loading the json file <a href='",d="value";
...
this.getRoot().add(s,{left:10,top:80});
var t=new qx.data.controller.List(null,s);
t.setLabelPath(o);
t.setIconOptions({converter:function(v){return p+v+j;
}});
t.setIconPath(g);
var r=qx.util.ResourceManager.getInstance().toUri(n);
var u=new qx.data.store.Json(r);
var status=new qx.ui.basic.Label(l);
...
q.setRich(true);
q.setWidth(260);
q.setValue(b+a+r+f+c+k);
this.getRoot().add(q,{left:10,top:10});
}}});
})();
...
(function(){var t="configured",s="completed",r="changeState",q="changeModel",p="qx.data.store.Json",o="GET",n="_marshaler",m="sending",l="application/json",k="changeUrl",d="failed",j="loaded",g="timeout",c="queued",b="String",f="aborted",e="_applyUrl",h="receiving",a="__xr",i="qx.event.type.Data";
...
}},events:{"loaded":i},properties:{model:{nullable:true,event:q},state:{check:[t,c,m,h,s,f,g,d],init:t,event:r},url:{check:b,apply:e,event:k,nullable:true}},members:{__xr:null,_delegate:null,_applyUrl:function(w,x){if(w!=null){this._createRequest(w);
}},_createRequest:function(y){this.__xr=new qx.io.remote.Request(y,o,l);
this.__xr.addListener(s,this.__xs,this);
var z=this._delegate;

if(z&&qx.lang.Type.isFunction(z.configureRequest)){this._delegate.configureRequest(this.__xr);
}this.__xr.addListener(r,function(A){this.setState(A.getData());
},this);
this.__xr.send();
},__xs:function(B){var D=B.getContent();
...
},events:{"created":j,"configured":j,"sending":j,"receiving":j,"completed":P,"aborted":j,"failed":P,"timeout":P},statics:{__sb:0,methodAllowsRequestBody:function(T){return (T==O)||(T==I);
...
},init:true,apply:B},crossDomain:{check:k,init:false},fileUpload:{check:k,init:false},transport:{check:l,nullable:true},useBasicHttpAuth:{check:k,init:false},parseJson:{check:k,init:true}},members:{__rW:null,__rX:null,__rY:null,__sa:null,__sb:null,send:function(){qx.io.remote.RequestQueue.getInstance().add(this);
...
},__sc:qx.event.GlobalError.observeMethod(function(e){var U=e.clone();
U.setTarget(this);
...
},setRequestHeader:function(bf,bg){this.__rW[bf]=bg;
},removeRequestHeader:function(bh){delete this.__rW[bh];
},getRequestHeader:function(bi){return this.__rW[bi]||null;
},getRequestHeaders:function(){return this.__rW;
},setParameter:function(bj,bk,bl){if(bl){this.__rY[bj]=bk;
}else{this.__rX[bj]=bk;
}},removeParameter:function(bm,bn){if(bn){delete this.__rY[bm];
}else{delete this.__rX[bm];
}},getParameter:function(bo,bp){if(bp){return this.__rY[bo]||null;
}else{return this.__rX[bo]||null;
}},getParameters:function(bq){return (bq?this.__rY:this.__rX);
},setFormField:function(br,bs){this.__sa[br]=bs;
},removeFormField:function(bt){delete this.__sa[bt];
},getFormField:function(bu){return this.__sa[bu]||null;
...
(function(){var s="Integer",r="aborted",q="_onaborted",p="_on",o="_applyEnabled",n="Boolean",m="__sg",l="sending",k="interval",j="failed",c="qx.io.remote.RequestQueue",h="timeout",g="completed",b="queued",a="__se",f="receiving",d="singleton";
...
this.__sg.addListener(k,this._oninterval,this);
},properties:{enabled:{init:true,check:n,apply:o},maxTotalRequests:{check:s,nullable:true},maxConcurrentRequests:{check:s,init:qx.bom.client.Transport.getMaxConcurrentRequestCount()},defaultTimeout:{check:s,init:5000}},members:{__sd:null,__se:null,__sf:null,__sg:null,getRequestQueue:function(){return this.__sd;
...
}},_remove:function(w){qx.lang.Array.remove(this.__se,w);
w.dispose();
this._check();
},__sh:0,_onsending:function(e){{};
...
y[q](event);
}}catch(A){}}},_oninterval:function(e){var H=this.__se;

if(H.length==0){this.__sg.stop();
return;
...
}this.__sg.setEnabled(I);
},add:function(K){K.setState(b);

if(K.isAsynchronous()){this.__sd.push(K);
}else{this.__sd.unshift(K);
}this._check();
...
if(M){M.abort();
}else if(qx.lang.Array.contains(this.__sd,L)){qx.lang.Array.remove(this.__sd,L);
...
},__si:function(){var bR=this.getRequest();
...
}this.__sj=qx.bom.Iframe.create({id:I,name:I,src:K});
qx.bom.element.Style.set(this.__sj,z,A);
this.__sk=qx.bom.Element.create(o,{id:J,name:J,target:I});
qx.bom.element.Style.set(this.__sk,z,A);
qx.dom.Element.insertEnd(this.__sk,qx.dom.Node.getBodyElement(document));
this.__sl=qx.bom.Element.create(c,{id:b,name:b});
qx.dom.Element.insertEnd(this.__sl,this.__sk);
qx.dom.Element.insertEnd(this.__sj,qx.dom.Node.getBodyElement(document));
qx.event.Registration.addListener(this.__sj,a,this._onload,this);
...
},_numericMap:{"uninitialized":1,"loading":2,"loaded":2,"interactive":3,"complete":4}},members:{__sl:null,__sn:0,__sk:null,__sj:null,__sm:null,send:function(){var M=this.getMethod();
var O=this.getUrl();
...
return;
}while(this.__sn<T){this.setState(qx.io.remote.Exchange._nativeMap[++this.__sn]);
...
},destruct:function(){if(this.__sj){qx.event.Registration.removeListener(this.__sj,a,this._onload,this);
qx.bom.Event.removeNativeListener(this.__sj,D,this.__sm);
if(qx.core.Variant.isSet(d,y)){this.__sj.src=qx.util.ResourceManager.getInstance().toUri(w);
...
var s=++qx.io.remote.transport.Script.__sp;

if(s>=2000000000){qx.io.remote.transport.Script.__sp=s=1;
}this.__sq=null;
this.__sp=s;
},statics:{__sp:0,_instanceRegistry:{},ScriptTransport_PREFIX:f,ScriptTransport_ID_PARAM:b,ScriptTransport_DATA_PARAM:h,handles:{synchronous:false,asynchronous:true,crossDomain:true,fileUpload:false,programaticFormFields:false,responseTypes:[m,l,o]},isSupported:function(){return true;
...
}})},members:{__sr:0,__sq:null,__sp:null,send:function(){var x=this.getUrl();
x+=(x.indexOf(p)>=0?r:p)+qx.io.remote.transport.Script.ScriptTransport_ID_PARAM+q+this.__sp;
...
this.__sq=document.createElement(c);
this.__sq.charset=e;
this.__sq.src=x;
{};
...
return;
}while(this.__sr<B){this.setState(qx.io.remote.Exchange._nativeMap[++this.__sr]);
...
}},properties:{parseJson:{check:A,init:true}},members:{__ss:false,__st:0,__su:null,getRequest:function(){if(this.__su===null){this.__su=qx.io.remote.transport.XmlHttp.createRequestObject();
...
if(M&&R&&qx.bom.client.Engine.MSHTML&&qx.bom.client.Engine.VERSION==9){qx.event.Timer.once(function(){O.send(this.getData());
},this,0);
...
if(bi==4){if(!qx.io.remote.Exchange.wasSuccessful(this.getStatusCode(),bi,this.__ss)){if(this.getState()===c){this.setState(f);
}this.failed();
return;
}}if(bi==3&&this.__st==1){this.setState(qx.io.remote.Exchange._nativeMap[++this.__st]);
}while(this.__st<bi){this.setState(qx.io.remote.Exchange._nativeMap[++this.__st]);
...
if(qx.io.remote.Exchange.wasSuccessful(bD,bE,this.__ss)){try{bF=this.getRequest().responseXML;
