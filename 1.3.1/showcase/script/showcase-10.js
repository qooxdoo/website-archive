qx.$$packageData['1abb793decfc']={"locales":{},"resources":{},"translations":{}};

qx.Part.$$notifyLoad("1abb793decfc", function() {
...
this.__xx(s);
return t;
},__xx:function(u){if(!(g in document.documentElement.style)){return;
}var w=true;
...
this.__xz();
},__xy:function(){this.getView().addListener(d,this._onButtonPress,this);
},_onButtonPress:function(e){this.getModel().readToken(e.getData());
},__xz:function(){var w=this.getModel();
...
this.initState();
},properties:{state:{check:[j,h,g],event:r,init:j,apply:q},errorMessage:{check:D,init:o,event:k},input:{check:D,nullable:true,event:y},maxInputLength:{check:x,init:20},operator:{check:[b,f,C,a],nullable:true,event:m},operant:{check:e,nullable:true,event:t},value:{check:e,nullable:true,event:z},memory:{check:e,nullable:true,event:s}},members:{readToken:function(E){if(E.match(/^[0123456789]$/)){this.__xC(E);
}else if(E.match(/^[\+\-\*\/]$/)){this.__xF(E);
}else if(E==v){this.__xD();
}else if(E==d){this.__xE();
}else if(E==l){this.__xG();
}else if(E==B){this.__xH();
}else if(E==c){this.__xI(E);
}else if(E==n){this.__xI(E);
}else if(E==w){this.__xK();
}else if(E==p){this.__xJ();
}},__xA:function(){return parseFloat(this.getInput());
},__xB:function(F,G,H){switch(H){case b:return F+G;
case f:return F-G;
case C:return F*G;
...
}},__xC:function(K){this.setState(j);
var L=this.getInput();
...
},__xF:function(Q){var S=this.getState();
...
if(T!==null){this.setValue(this.__xB(T,R,this.getOperator()));
}else{this.setValue(R);
}this.setOperant(R);
this.setOperator(Q);
},__xG:function(){var U=this.getOperator();

if(!U){return;
}var W=this.getValue();

if(this.getState()==h){this.setValue(this.__xB(W,this.getOperant(),U));
return;
}this.setState(h);
var V=this.__xA();
this.setOperant(V);
this.setValue(this.__xB(W,V,U));
},__xH:function(){this.setState(j);
this.setOperator(null);
this.setValue(null);
this.setInput(i);
},__xI:function(X){var Y=this.getState();
var ba;
...
}},__xJ:function(){var bc=this.getMemory();

if(bc==null){return;
}this.setState(j);
this.setInput(bc.toString());
},__xK:function(){this.setMemory(null);
}}});
})();
