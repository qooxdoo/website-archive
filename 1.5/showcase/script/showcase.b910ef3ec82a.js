...
for(var i=0;i<6*7;i++){var bu=this.__HD[i];
...
for(var bK=0;bK<6;bK++){this.__HE[bK].setValue(cg.format(cc));

for(var i=0;i<7;i++){var ch=this.__HD[bK*7+i];
...
}this.__HC=this.__HD=this.__HE=null;
}});
})();
...
return W==null||W==s;
}},destruct:function(){if(qx.core.Environment.get(r)){if(this.__Iz){qx.locale.Manager.getInstance().removeListenerById(this.__Iz);
}}}});
})();
