...
var envinfo = {"qx.application":"portal.Application","qx.debug":false,"qx.optimization.basecalls":true,"qx.optimization.comments":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.variants":true,"qx.revision":"28155","qx.theme":"qx.theme.Modern","qx.version":"1.5"};
for (var k in envinfo) qx.$$environment[k] = envinfo[k];

if (!qx.$$libraries) qx.$$libraries = {};
var libinfo = {"__out__":{"sourceUri":"script"},"portal":{"resourceUri":"resource","sourceUri":"script","version":"trunk"},"qx":{"resourceUri":"resource","sourceUri":"script","version":"1.5"}};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:portal.3c4288ed3b44.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="os.name",j="os.version",h="css.borderradius",g="default",f="browser.quirksmode",e="browser.name",d="qx.allowUrlSettings",c="io.ssl",b="css.boxmodel",a="html.classlist",P="event.touch",O="css.gradients",N="browser.documentmode",M="ecmascript.objectcount",L="engine.version",K="engine.name",J="css.rgba",I="|",H="css.boxshadow",G="qx.core.Environment",r="qx.debug.databinding",s="qx.optimization.basecalls",p="qx.debug.dispose",q="qx.optimization.variables",n="true",o="qx.optimization.privates",l="qx.aspects",m="qx.debug",t="qx.dynamicmousewheel",u=":",y="qxenv",x="qx.optimization.strings",A="qx.optimization.comments",z="qx.optimization.variants",C="qx.mobile.emulatetouch",B="qx.dynlocale",w="false",F="&",E="qx.mobile.nativescroll",D="qx.allowUrlVariants",v="qx.propertyDebugLevel";
qx.Bootstrap.define(G,{statics:{_checks:{},_asyncChecks:{},__cJ:{},get:function(Q){if(this.__cJ[Q]!=undefined){return this.__cJ[Q];
}var R=this._checks[Q];

if(R){var S=R();
this.__cJ[Q]=S;
return S;
}if(qx.Bootstrap.DEBUG){qx.Bootstrap.warn(Q+" is not a valid key. Please see the API-doc of "+"qx.core.Environment for a list of predefined keys.");
qx.Bootstrap.trace(this);
}},getAsync:function(T,U,self){var W=this;

if(this.__cJ[T]!=undefined){window.setTimeout(function(){U.call(self,W.__cJ[T]);
},0);
return;
}var V=this._asyncChecks[T];

if(V){V(function(X){W.__cJ[T]=X;
U.call(self,X);
});
return;
}if(qx.Bootstrap.DEBUG){qx.Bootstrap.warn(T+" is not a valid key. Please see the API-doc of "+"qx.core.Environment for a list of predefined keys.");
qx.Bootstrap.trace(this);
}},select:function(Y,ba){return this.__cK(this.get(Y),ba);
},selectAsync:function(bb,bc,self){this.getAsync(bb,function(bd){var be=this.__cK(bb,bc);
be.call(self,bd);
},this);
},__cK:function(bf,bg){var bi=bg[bf];

if(bg.hasOwnProperty(bf)){return bi;
}for(var bh in bg){if(bh.indexOf(I)!=-1){var bj=bh.split(I);

for(var i=0;i<bj.length;i++){if(bj[i]==bf){return bg[bh];
}}}}
if(bg[g]!==undefined){return bg[g];
}
if(qx.Bootstrap.DEBUG){throw new Error('No match for variant "'+bf+'" ('+(typeof bf)+' type)'+' in variants ['+qx.Bootstrap.getKeysAsString(bg)+'] found, and no default ("default") given');
}},invalidateCacheKey:function(bk){delete this.__cJ[bk];
},add:function(bl,bm){if(this._checks[bl]==undefined){if(bm instanceof Function){this._checks[bl]=bm;
}else{this._checks[bl]=this.__cN(bm);
}}},addAsync:function(bn,bo){if(this._checks[bn]==undefined){this._asyncChecks[bn]=bo;
}},_initDefaultQxValues:function(){this.add(d,function(){return false;
});
this.add(D,function(){return false;
});
this.add(v,function(){return 0;
});
...
},__cL:function(){if(qx&&qx.$$environment){for(var bq in qx.$$environment){var bp=qx.$$environment[bq];
this._checks[bq]=this.__cN(bp);
}}},__cM:function(){if(window.document&&window.document.location){var br=window.document.location.search.slice(1).split(F);

for(var i=0;i<br.length;i++){var bt=br[i].split(u);

if(bt.length!=3||bt[0]!=y){continue;
}var bu=bt[1];
var bs=decodeURIComponent(bt[2]);
if(bs==n){bs=true;
}else if(bs==w){bs=false;
}else if(/^(\d|\.)+$/.test(bs)){bs=parseFloat(bs);
}this._checks[bu]=this.__cN(bs);
}}},__cN:function(bv){return qx.Bootstrap.bind(function(bw){return bw;
},null,bv);
},useCheck:function(bx){return true;
},_initChecksMap:function(){if(this.useCheck(L)){this._checks[L]=qx.bom.client.Engine.getVersion;
}
if(this.useCheck(K)){this._checks[K]=qx.bom.client.Engine.getName;
}if(this.useCheck(e)){this._checks[e]=qx.bom.client.Browser.getName;
}
if(this.useCheck(N)){this._checks[N]=qx.bom.client.Browser.getDocumentMode;
}
...
}if(this.useCheck(P)){this._checks[P]=qx.bom.client.Event.getTouch;
}if(this.useCheck(M)){this._checks[M]=qx.bom.client.EcmaScript.getObjectCount;
}
...
if(this.useCheck(H)){this._checks[H]=qx.bom.client.Css.getBoxShadow;
}
if(this.useCheck(O)){this._checks[O]=qx.bom.client.Css.getGradients;
}
if(this.useCheck(b)){this._checks[b]=qx.bom.client.Css.getBoxModel;
}
if(this.useCheck(J)){this._checks[J]=qx.bom.client.Css.getRgba;
}}},defer:function(by){by._initDefaultQxValues();
by._initChecksMap();
by.__cL();
if(by.get(d)===true){by.__cM();
}}});
})();
...
this.__sK=[];
this.__sL={};
},statics:{dataKeyRoot:h,dataKeyGroupBoxes:m,dataKeyBoxes:d},events:{"loaded":e},properties:{activeBox:{init:null,check:g,apply:c}},members:{__sJ:null,__sK:null,__sL:null,_applyActiveBox:function(n,o){if(o!=null){o.setActive(false);
}n.setActive(true);
},load:function(){this.__dl(window[portal.box.Manager.dataKeyRoot]);
qx.event.Registration.fireEvent(this,b);
},__dl:function(p){var t,w,u,v,x,p;
...
this.__sJ.push(x);
this.__sL[w].push(x);
}}},getBoxes:function(){return this.__sJ;
},getGroupBoxes:function(){return this.__sK;
},updateGroupBoxMembers:function(y,z,A){var B=qx.dom.Hierarchy.getElementIndex(A.getElement());
qx.lang.Array.remove(this.__sL[z],A);
qx.lang.Array.insertAt(this.__sL[y],A,B);
},getGroupBoxDataOfBox:function(C){for(var i=0,j=this.__sK.length,D;i<j;i++){D=this.__sL[this.__sK[i].id];

for(var k=0,l=D.length;k<l;k++){if(D[k].getId()===C){return this.__sK[i];
}}}return null;
}},destruct:function(){this.__sJ=this.__sK=this.__sL=null;
}});
})();
...
this.__sN=new portal.box.Resizable(this,x);
}if(t.draggable){this.setDraggable(true);
this.__sO=new portal.box.Draggable(this);
}this.__sQ();
},properties:{element:{init:null},groupBoxId:{init:null,check:h},active:{check:s,init:false,apply:j},resizable:{check:s,init:false},draggable:{check:s,init:false}},members:{__df:null,__sM:null,__sO:null,__sN:null,_applyActive:function(y,z){if(y){qx.bom.element.Style.set(this.getElement(),q,a);
}else{qx.bom.element.Style.set(this.getElement(),q,n);
}},getId:function(){return this.getElement()?this.getElement().id:null;
},getData:function(){return this.__sM;
},getDraggableComponent:function(){return this.__sO;
},getResizableComponent:function(){return this.__sN;
},__sP:function(A,B){if(B){for(var C in B){qx.bom.element.Style.set(C,B[C]);
}}qx.bom.element.Style.set(A,l,b);
qx.bom.element.Style.set(A,k,0);
qx.bom.Element.addListener(A,o,this.__sR,this,true);
},__sQ:function(){var E=f+g;
E=E+r+E+r+E;
var D=qx.bom.Element.create(d,{"class":i});
D.innerHTML=E;
qx.dom.Element.insertEnd(D,this.getElement());
},__sR:function(e){portal.box.Manager.getInstance().setActiveBox(this);
}},destruct:function(){this.resetElement();
this.__sM=this.__sN=this.__sO=null;
}});
})();
...
this.__sS=v;
this.__iI=this.__sS.getElement();
this.__sT={};
this.__sU=false;
this.__sV={orientation:null,lastCoord:{left:null,top:null}};
this.__sW={minWidth:10,minHeight:10,maxWidth:null,maxHeight:null,autoHide:true,handles:3};

for(var name in this.__sW){if(w[name]){this.__sW[name]=w[name];
}}this.__sP();
},members:{__sV:null,__sS:null,__iI:null,__sT:null,__sW:null,__sU:false,getBox:function(){return this.__sS;
},__sP:function(){switch(this.__sW.handles){case 1:this.__sT.east=null;
break;
case 2:this.__sT.south=null;
break;
case 3:this.__sT.south=null;
this.__sT.southeast=null;
this.__sT.east=null;
break;
}for(var y in this.__sT){var x=this.__sT[y]=qx.bom.Element.create(f);
qx.bom.element.Attribute.set(x,o,y);
qx.dom.Element.insertEnd(x,this.__iI);

if(this.__iI.id){x.id=this.__iI.id+h+y;
}this.__sX(y);
this.__sY(y);
}},__sX:function(z){qx.bom.element.Class.add(this.__sT[z],z+j);
qx.bom.element.Style.set(this.__sT[z],u,this.__sW.autoHide?t:q);
},__sY:function(A){var C=qx.event.Registration;
var B=this.__sT[A];
C.addListener(B,i,this.__tc,this);
C.addListener(document,m,this.__td,this);

if(this.__sW.autoHide){C.addListener(this.__sS.getElement(),s,this.__ta,this);
C.addListener(this.__sS.getElement(),r,this.__tb,this);
}},__ta:function(e){for(var D in this.__sT){qx.bom.element.Style.set(this.__sT[D],u,q);
}},__tb:function(e){if(!this.__sU){for(var E in this.__sT){qx.bom.element.Style.set(this.__sT[E],u,t);
}}},__tc:function(e){e.stopPropagation();
if(e.isLeftPressed()){qx.event.Registration.addListener(document,p,this.__te,this,true);
this.__sU=true;
var F=e.getTarget();
this.__sV.orientation=qx.bom.element.Attribute.get(F,o);
this.__sV.lastCoord.left=e.getDocumentLeft();
this.__sV.lastCoord.top=e.getDocumentTop();
portal.box.Util.bringToFront(this.__iI);
}},__td:function(e){e.stopPropagation();

if(this.__sU){qx.event.Registration.removeListener(document,p,this.__te,this,true);
portal.box.Util.sendToBack(this.__iI);

if(this.__sW.autoHide){for(var G in this.__sT){qx.bom.element.Style.set(this.__sT[G],u,t);
}}this.__sU=false;
qx.bom.Selection.clear(document.body);
}},__te:function(e){e.stopPropagation();

if(this.__sU){var H={left:null,top:null};

switch(this.__sV.orientation){case n:H.top=e.getDocumentTop();
break;
...
break;
}this.__tf(H,this.__sV.lastCoord);
this.__sV.lastCoord.left=e.getDocumentLeft();
this.__sV.lastCoord.top=e.getDocumentTop();
}},__tf:function(I,J){var Q=qx.bom.element.Dimension;
var N=portal.box.Manager.getInstance().getGroupBoxDataOfBox(this.__sS.getId());
var P=Q.getContentSize(N.element);

if(I.left){var M=I.left-J.left;

if(M!==0){var L=Q.getContentWidth(this.__iI)+M;
var O=Q.getWidth(this.__iI)+M;

if((this.__sW.minWidth==null||L>this.__sW.minWidth)&&(this.__sW.maxWidth==null||L<=this.__sW.maxWidth)){if(P.width>=O){qx.bom.element.Style.set(this.__iI,k,L+d);
this.__sS.getData().width=L;
}}}}
if(I.top){var M=I.top-J.top;

if(M!==0){var K=Q.getContentHeight(this.__iI)+M;

if((this.__sW.minHeight==null||K>this.__sW.minHeight)&&(this.__sW.maxHeight==null||K<=this.__sW.maxHeight)){qx.bom.element.Style.set(this.__iI,c,K+b);
}}}}},destruct:function(){if(this.__sW.autoHide){qx.event.Registration.removeListener(this.__sS.getElement(),s,this.__ta,this);
qx.event.Registration.removeListener(this.__sS.getElement(),r,this.__tb,this);
}this.__sS=this.__sW=this.__iI=this.__sT=null;
this.__sV=null;
}});
})();
...
qx.Class.define(b,{statics:{__tg:1000,bringToFront:function(d){var e=qx.bom.element.Style.get(d,c);
e=e==a?0:e;
qx.bom.element.Style.set(d,c,e+this.__tg);
},sendToBack:function(f){var g=qx.bom.element.Style.get(f,c);
qx.bom.element.Style.set(f,c,g-this.__tg);
}}});
})();
...
(function(){var o="mousemove",n="mouseup",m="dragstart",l="engine.name",k="mousedown",j="portal.box.Draggable",i="mouseover",h="drag",g="dragend",f="div",b="move",d="dragHandle",c="cursor",a="html";
qx.Class.define(j,{extend:qx.core.Object,construct:function(p){qx.core.Object.call(this);
this.__sS=p;
this.__iI=p.getElement();
this.__th=null;
this.__ti=null;
this.__sP();
},members:{__sS:null,__iI:null,__th:null,__ti:null,getBox:function(){return this.__sS;
},getElement:function(){return this.__iI;
},__sP:function(){this.__tj();
this.__sY();
},__tj:function(){this.__th=qx.bom.Element.create(f);
qx.bom.element.Class.add(this.__th,d);
qx.dom.Element.insertBegin(this.__th,this.__iI);
},__sY:function(){qx.bom.Element.addListener(this.__th,k,this.__nr,this);
qx.bom.Element.addListener(this.__th,i,function(e){qx.bom.element.Style.set(this,c,b);
},this.__th);
qx.bom.Element.addListener(this.__th,m,this.__tm,this);
qx.bom.Element.addListener(this.__th,g,portal.dragdrop.Manager.getInstance().stopSession,portal.dragdrop.Manager.getInstance());
qx.bom.Element.addListener(this.__th,h,this.__tn,this);
},__nr:function(e){if(e.isLeftPressed()){this.__ti={left:e.getDocumentLeft()-qx.bom.element.Location.getLeft(this.__iI),top:e.getDocumentTop()-qx.bom.element.Location.getTop(this.__iI)};
qx.bom.Element.addListener(document.body,n,this.__tl,this,true);
this.__tk();
qx.event.Registration.fireEvent(this.__th,m,qx.event.type.Event);
}},__tk:qx.core.Environment.select(l,{"mshtml":function(){var r=this;
var q=qx.lang.Function.bind(this.__tl,r);
document.getElementsByTagName(a)[0].onmouseleave=q;
},"default":qx.lang.Function.empty}),__tl:function(e){try{e.stopPropagation();
}catch(s){}
if(portal.dragdrop.Manager.getInstance().isSessionActive()){qx.event.Registration.removeListener(document.body,o,this.__tn,this,true);
portal.dragdrop.Manager.getInstance().stopSession();
}qx.bom.Element.removeListener(document.body,n,this.__tl,this,true);
},__tm:function(e){if(portal.dragdrop.Manager.getInstance().isSessionActive()){return;
}portal.box.Manager.getInstance().setActiveBox(this.__sS);
portal.dragdrop.Manager.getInstance().startSession(this.__sS);
qx.event.Registration.addListener(document.body,o,this.__tn,this,true);
},__tn:function(e){e.stopPropagation();
var t=e.getDocumentLeft()-this.__ti.left;
var top=e.getDocumentTop()-this.__ti.top;
...
}},destruct:function(){this.__sS=this.__th=this.__iI=this.__ti=null;
}});
})();
...
this.__to={top:null,height:null};
this.__tp={top:0,left:0};
},properties:{activeBox:{check:u,init:null,nullable:true},sessionActive:{check:r,init:false}},members:{__to:null,__tq:null,__tr:false,__tp:null,__ts:null,__tt:null,startSession:function(y){this.setSessionActive(true);
this.setActiveBox(y);
this.__tq=y.getGroupBoxId();
this.__tu();
if(this.__ts==null){this.__ts={};
var z=portal.box.Manager.getInstance().getGroupBoxes();

for(var i=0,j=z.length;i<j;i++){this.__ts[z[i].element.id]={left:qx.bom.element.Location.getLeft(z[i].element),right:qx.bom.element.Location.getLeft(z[i].element)+qx.bom.element.Dimension.getWidth(z[i].element)};
}}},__tu:function(){if(!this.__tt){this.__tt=qx.bom.Element.create(b);
}var C=this.getActiveBox().getElement();
var A=qx.bom.element.Dimension.getContentSize(C);
this.__tv(C,this.__tt);
qx.bom.element.Class.add(this.__tt,qx.bom.element.Class.get(C));
var B=k+d+qx.bom.element.Location.getTop(C)+w+c+qx.bom.element.Location.getLeft(C)+w+o+A.width+w+f+A.height+x;
qx.bom.element.Style.setCss(this.__tt,B);
qx.bom.element.Style.set(C,v,t);
qx.dom.Node.getBodyElement(C).appendChild(this.__tt);
portal.box.Util.bringToFront(this.__tt);
},__tv:function(D,E){while(D.firstChild){E.appendChild(D.firstChild);
}},stopSession:function(e){this.setSessionActive(false);
var G=this.getActiveBox();
var H=G.getElement();
portal.box.Manager.getInstance().updateGroupBoxMembers(G.getGroupBoxId(),this.__tq,G);
G.setGroupBoxId(this.__tq);
var F=new qx.fx.effect.core.Move(this.__tt);
F.set({x:qx.bom.element.Location.getLeft(H),y:qx.bom.element.Location.getTop(H),mode:h,duration:0.5,transition:g});
F.start();
F.addListener(n,function(){this.__tv(this.__tt,H);
qx.bom.element.Style.reset(H,v);
this.__tt.parentNode.removeChild(this.__tt);

if(G.isActive()){G._applyActive(true);
}else{portal.box.Manager.getInstance().setActiveBox(G);
}},this);
},onDragMove:function(top,I){if(this.__tp.left!=I){qx.bom.element.Style.set(this.__tt,q,I+x);
}
if(this.__tp.top!=top){qx.bom.element.Style.set(this.__tt,s,top+x);
}var J=this.getActiveBox().getElement();

if(this.__to.top==null){this.__to.top=qx.bom.element.Location.getTop(J);
}
if(this.__to.height==null){this.__to.height=qx.bom.element.Dimension.getHeight(J);
}if(this.__tr==false&&top>=this.__to.top&&top<=(this.__to.top+this.__to.height)){return;
}var K;
if(this.__tr){K=qx.dom.Hierarchy.getFirstDescendant(document.getElementById(this.__tq));
while(K!=null){if(this.__tw(true,top,J,K)){this.__tr=false;
return;
}K=qx.dom.Hierarchy.getNextElementSibling(K);
}}else{K=J;
}if(top-this.__tp.top>0){K=qx.dom.Hierarchy.getNextElementSibling(K);

while(K!=null){if(K){if(this.__tw(true,top,J,K)){return;
}}K=qx.dom.Hierarchy.getNextElementSibling(K);
}}else{var K=qx.dom.Hierarchy.getPreviousElementSibling(K);

while(K!=null){if(K){if(this.__tw(false,top,J,K)){return;
}}K=qx.dom.Hierarchy.getPreviousElementSibling(K);
}}this.__tp.left=I;
this.__tp.top=top;
},__tw:function(L,top,M,N){var O=qx.bom.element.Location.getTop(N);
...
this.__to.top=qx.bom.element.Location.getTop(M);
}return true;
...
},checkGroupBox:function(T,U){if(this.__ts[this.__tq].left<=T&&this.__ts[this.__tq].right>=T){return;
}
for(var V in this.__ts){if(V!=this.__tq){if(this.__ts[V].left<=T+U&&this.__ts[V].right>=T){this.__tq=V;
this.__tr=true;
return;
}}}}},destruct:function(){this.__to=this.__tp=this.__tq=null;
this.__tx=this.__tt=this.__ts=null;
}});
})();
