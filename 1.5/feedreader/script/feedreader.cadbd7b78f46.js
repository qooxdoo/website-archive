...
},members:{__Q:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(c,d,e,name,f,g){if(this.__R()){this.__Q.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});
}this.__Q[this._groupCounter].items.push(c);
this.__Q[this._groupCounter].labels.push(d);
this.__Q[this._groupCounter].options.push(g);
if(name==null){name=d.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,a);
}this.__Q[this._groupCounter].names.push(name);
this._validationManager.add(c,e,f);
this._resetter.add(c);
},addGroupHeader:function(h,k){if(!this.__R()){this._groupCounter++;
}this.__Q.push({title:h,items:[],labels:[],names:[],options:[],headerOptions:k});
},addButton:function(l,m){this._buttons.push(l);
this._buttonOptions.push(m||null);
},__R:function(){return this.__Q.length===0;
...
},getGroups:function(){return this.__Q;
},getButtons:function(){return this._buttons;
},getButtonOptions:function(){return this._buttonOptions;
},getItems:function(){var n={};
for(var i=0;i<this.__Q.length;i++){var o=this.__Q[i];
for(var j=0;j<o.names.length;j++){var name=o.names[j];
n[name]=o.items[j];
}}return n;
}},destruct:function(){this.__Q=this._buttons=this._buttonOptions=null;
this._validationManager.dispose();
this._resetter.dispose();
}});
})();
(function(){var s="",r="URL",q="Add",p="keypress",o="Title",n="Add a feed",m="feedreader.view.AddFeedWindow",l="user",k="this.__form",j="__A",c="icon/16/actions/document-new.png",i="icon/16/actions/dialog-apply.png",g="__z",b="right",a="Feed Information",f="__B",d="execute",h="Enter";
...
this._addContent();
},members:{__A:null,__B:null,__z:null,__C:null,_addContent:function(){this.setLayout(new qx.ui.layout.VBox(10));
this.__C=new qx.ui.form.Form();
this.__C.addGroupHeader(this.tr(a));
this.__B=new qx.ui.form.TextField().set({required:true,width:250});
this.__C.add(this.__B,this.tr(o));
this.__A=new qx.ui.form.TextField().set({required:true});
this.__C.add(this.__A,this.tr(r),qx.util.Validate.checkUrl);
var u=new qx.ui.form.Button(this.tr(q),i);
u.set({alignX:b,allowGrowX:false});
u.addListener(d,this._addFeed,this);
this.__C.addButton(u);
this.addListener(p,function(e){if(e.getTarget() instanceof qx.ui.form.TextField&&e.getKeyIdentifier()===h){this._addFeed();
}});
this.add(new qx.ui.form.renderer.SinglePlaceholder(this.__C));
},_addFeed:function(e){if(this.__C.validate()){var w=this.__B.getValue();
var v=this.__A.getValue();
this.__z.addFeed(w,v,l);
this.__B.setValue(s);
this.__A.setValue(s);
this.close();
}}},destruct:function(){this.__D=null;
this._disposeObjects(g,f,j,k);
}});
})();
