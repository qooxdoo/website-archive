...
qx.Class.define(d,{statics:{inShutDown:false,__hg:{},__hh:0,__hi:[],__hj:e,__Vb:{},register:function(j){var n=this.__hg;
...
if(qx.core.Environment.get(h)&&qx.dev&&qx.dev.Debug&&qx.dev.Debug.disposeProfilingActive){this.__Vb[m]=qx.dev.StackTrace.getStackTrace();
}}n[m]=j;
...
},getNextHash:function(){return this.__hh;
},getPostId:function(){return this.__hj;
},getStackTraces:function(){return this.__Vb;
...
while(i--){P=P.replace(new RegExp(g+(i+1),e),O[i]+l);
}return P;
},escapeRegexpChars:function(Q){return Q.replace(/([.*+?^${}()|[\]\/\\])/g,h);
},toArray:function(R){return R.split(/\B|\b/g);
},stripTags:function(S){return S.replace(/<\/?[^>]+>/gi,l);
},stripScripts:function(T,U){var W=l;
var V=T.replace(/<script[^>]*>([\s\S]*?)<\/script>/gi,function(){W+=arguments[1]+a;
return l;
});
...
for(var I in K){this.__kH[I]=qx.lang.String.hyphenate(K[I]);
...
}}),isPropertySupported:function(ba){return (this.__kJ[ba]||this.__kG[ba]||ba in document.documentElement.style);
},COMPUTED_MODE:1,CASCADED_MODE:2,LOCAL_MODE:3,set:function(bb,name,bc,bd){name=this.__kG[name]||name;
if(bd!==false&&this.__kJ[name]){return this.__kJ[name].set(bb,bc);
}else{bb.style[name]=bc!==null?bc:m;
}},setStyles:function(be,bf,bg){var bj=this.__kG;
var bl=this.__kJ;
var bh=be.style;
...
}}}},reset:function(bm,name,bn){name=this.__kG[name]||name;
if(bn!==false&&this.__kJ[name]){return this.__kJ[name].reset(bm);
}else{bm.style[name]=m;
}},get:qx.core.Environment.select(k,{"mshtml":function(bo,name,bp,bq){name=this.__kG[name]||name;
if(bq!==false&&this.__kJ[name]){return this.__kJ[name].get(bo,bp);
...
}},"default":function(bv,name,bw,bx){name=this.__kG[name]||name;
if(bx!==false&&this.__kJ[name]){return this.__kJ[name].get(bv,bw);
...
return o?{width:o.width,height:o.height}:this.__hY;
...
}},syncWidget:function(){if(this.__Vc){this._changeTargetSelection();
}
if(this.__nI){this._updateSelection();
}this.__nI=this.__Vc=null;
