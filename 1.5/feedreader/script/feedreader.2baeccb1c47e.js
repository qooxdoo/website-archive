...
},removeAt:function(E){return this.__dL(e,E);
}}});
})();
...
this.__gJ[bk].addListenerOnce(f,this.__gP,this);
}
if(bm){var bo=bn||window;
this.__gJ[bk].addListenerOnce(f,bm,bo);
...
if(k!==null&&k.fonts[i]){var m=this.__hd(k.fonts[i]);
...
if(p!==null&&n&&p.fonts[n]){var o=this.__hd(p.fonts[n]);
q[n]=(new o).set(p.fonts[n]);
return true;
}return false;
},__hc:function(r,s){if(r[s].include){var t=r[r[s].include];
r[s].include=null;
delete r[s].include;
...
for(var y in w){if(w[y].include&&w[w[y].include]){this.__hc(w,y);
}var x=this.__hd(w[y]);
v[y]=(new x).set(w[y]);
v[y].themed=true;
}}this._setDynamic(v);
},__hd:function(z){if(z.sources){return qx.bom.webfonts.WebFont;
}return qx.bom.Font;
}},destruct:function(){this._disposeMap(d);
}});
})();
...
},isCached:function(A){return qx.lang.Object.contains(this.__ih,A);
...
qx.ui.core.queue.Manager.scheduleFlush(b);
},isEmpty:function(){return this.__cX.length==0;
...
qx.Bootstrap.define(d,{construct:function(){this.__mh=qx.Bootstrap.bind(this.__ml,this);
this.__mi=document.createElement(f);
},statics:{TIMEOUT:15},members:{__mj:null,__mk:null,__ey:null,__ez:null,__mh:null,__mi:null,load:function(q,r,s){if(this.__mj){throw new Error("Another request is still running!");
}this.__mj=true;
this.__mk=false;
...
this.__ez=s||window;
u.type=b;
u.onerror=u.onload=u.onreadystatechange=this.__mh;
var self=this;
...
},abort:function(){if(this.__mj){this.dispose(a);
...
if(this.__ey){var v=qx.bom.client.Engine.getName();
...
},__ml:qx.event.GlobalError.observeMethod(function(e){var z=qx.bom.client.Engine.getName();
if(z==n){var A=this.__mi.readyState;
...
}}else if(typeof p==c){this.__mm=new Array(p);
...
},properties:{autoDisposeItems:{check:e,init:false}},events:{"change":f,"changeLength":f},members:{__mm:null,concat:function(q){if(q){var r=this.__mm.concat(q);
...
this.fireDataEvent(n,{value:this.__mm,name:k+(this.__mm.length-1),old:v});
},shift:function(){if(this.length==0){return;
}var w=this.__mm.shift();
this.__mn();
...
return w;
},slice:function(x,y){return new qx.data.Array(this.__mm.slice(x,y));
},splice:function(z,A,B){var J=this.__mm.length;
var F=this.__mm.splice.apply(this.__mm,arguments);
if(this.__mm.length!=J){this.__mn();
}var H=A>0;
var D=arguments.length>2;
var E=null;

if(H||D){if(this.__mm.length>J){var I=m;
}else if(this.__mm.length<J){var I=l;
E=F;
}else{var I=j;
...
}var L=this.__mm.concat();
this.__mm.sort.apply(this.__mm,arguments);
this.fireDataEvent(o,{start:0,end:this.length-1,type:j,items:null},null);
this.fireDataEvent(n,{value:this.__mm,name:k+(this.length-1),old:L});
},unshift:function(M){for(var i=arguments.length-1;i>=0;i--){this.__mm.unshift(arguments[i]);
this.__mn();
this._registerEventChaining(arguments[i],null,0);
this.fireDataEvent(n,{value:[this.__mm[0]],name:g,old:[this.__mm[1]]});
this.fireDataEvent(o,{start:0,end:this.length-1,type:m,items:[arguments[i]]},null);
}return this.length;
},toArray:function(){return this.__mm;
},getItem:function(N){return this.__mm[N];
},setItem:function(O,P){var Q=this.__mm[O];
if(Q===P){return;
}this.__mm[O]=P;
...
},indexOf:function(R){return this.__mm.indexOf(R);
},toString:function(){if(this.__mm!=null){return this.__mm.toString();
}return b;
},contains:function(S){return this.__mm.indexOf(S)!==-1;
...
}Array.prototype.push.apply(this.__mm,bh);
for(var i=0;i<bh.length;i++){this._registerEventChaining(bh[i],null,this.__mm.length+i);
}var bi=this.length;
this.__mn();
...
},forEach:function(bp,bq){for(var i=0;i<this.__mm.length;i++){bp.call(bq,this.__mm[i],i,this);
}},__mn:function(){var br=this.length;
this.length=this.__mm.length;
this.fireDataEvent(a,this.length,br);
}},destruct:function(){for(var i=0;i<this.__mm.length;i++){var bs=this.__mm[i];
...
if(this.isItemSelected(bM)&&!bL&&!bO&&!this.getDrag()){this.__pb=bM;
this._userInteraction=false;
return;
}else{this.__pb=null;
...
if(this.getDrag()&&bP!==n&&bP!==o&&!bL&&!bO){this.__oO=this._getLocation();
...
if(!bT&&!bQ&&this.__pb!=null){var bR=this._getSelectableFromMouseEvent(event);
...
this._userInteraction=true;
var bV=this.__oS+this.__oL.left;

if(bV>this.__oP){this.__oU=1;
}else if(bV<this.__oP){this.__oU=-1;
}else{this.__oU=0;
}var bU=this.__oT+this.__oL.top;

if(bU>this.__oQ){this.__oV=1;
}else if(bU<this.__oQ){this.__oV=-1;
}else{this.__oV=0;
...
var bY=Math.max(0,Math.min(this.__oS-this.__oO.left,cg.width))+this.__oL.left;
var bX=Math.max(0,Math.min(this.__oT-this.__oO.top,cg.height))+this.__oL.top;
if(this.__oM===bY&&this.__oN===bX){return;
}this.__oM=bY;
this.__oN=bX;
...
}},__pd:function(cG){var cI={};
var cH;

for(var i=0,l=cG.length;i<l;i++){cH=cG[i];
cI[this._selectableToHashCode(cH)]=cH;
}return cI;
},_getSelectedItem:function(){for(var cJ in this.__it){return this.__it[cJ];
}return null;
},_setSelectedItem:function(cK){if(this._isSelectable(cK)){var cL=this.__it;
...
this.__oW=true;
this._fireChange();
},_fireChange:function(dc){if(this.__oW){this.__oX=dc||null;
...
qx.Mixin.define(c,{construct:function(){this.__mA=this._createBlocker();
},properties:{blockerColor:{check:a,init:null,nullable:true,apply:f,themeable:true},blockerOpacity:{check:e,init:1,apply:b,themeable:true}},members:{__mA:null,_createBlocker:function(){return new qx.ui.core.Blocker(this);
},_applyBlockerColor:function(g,h){this.__mA.setColor(g);
...
}if(this.getSelection()==null){this.__UM=new qx.data.Array();
this.setSelection(this.__UM);
}},properties:{selection:{check:d,event:f,apply:b,init:null}},members:{_modifingSelection:0,__re:null,__rf:null,__UM:null,_applySelection:function(g,h){if(this.__rf!=undefined&&h!=undefined){h.removeListenerById(this.__rf);
...
},_applyIconOptions:function(F,G){this.__nO();
},_applyLabelOptions:function(H,I){this.__nO();
},_applyIconPath:function(J,K){this.__nO();
},_applyLabelPath:function(L,M){this.__nO();
},_applyModel:function(N,O){if(O!=undefined){if(this.__nG!=undefined){O.removeListenerById(this.__nG);
}}if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();
}if(N!=null){this.__nG=N.addListener(k,this.__nJ,this);
this.__nQ();
this.__nK();
...
if(Q!=null){if(this.getModel()!=null){for(var i=0;i<this.__nB.length;i++){this.__nM(this.__nR(i));
...
this._endSelectionModification();
this._updateSelection();
},_setFilter:function(bP,bQ){if((bP==null||bP.filter==null)&&(bQ!=null&&bQ.filter!=null)){this.__nP();
...
this.__nQ();
if(bS.length>this.__nB.length){for(var j=bS.length;j>this.__nB.length;j--){this.getTarget().removeAt(j-1).destroy();
}}else if(bS.length<this.__nB.length){for(var j=bS.length;j<this.__nB.length;j++){var bT=this._createItem();
this.getTarget().add(bT);
}}var bR=this.getTarget().getChildren();

for(var i=0;i<bR.length;i++){this._bindListItem(bR[i],this.__nR(i));
}this.__nL();
this._endSelectionModification();
this._updateSelection();
...
this.__nI=true;
qx.ui.core.queue.Widget.add(this);
},__nQ:function(){var bW=this.getModel();
...
for(var i=0;i<bW.getLength();i++){if(bV==null||bV(bW.getItem(i))){this.__nB.push(i);
}}},__nR:function(bY){return this.__nB[bY];
...
(function(){var k="enabled",j="originalIcon",i=")",h="__pI",g="Remove feed",f="Preferences",d="excluded",c="addFeed",b="removeFeed",a="icon/22/actions/help-about.png",H="Reload the feeds. (%1)",G="Help",F="visible",E="feedreader.view.ToolBar",D="icon/22/actions/media-seek-forward.png",C="toolbar-button",B="Add feed",A="addfeed",z="icon/22/actions/view-refresh.png",y="(",r="reload",s="showItem",p="settings",q="about",n="icon/22/actions/dialog-cancel.png",o="__pG",l="Reload",m="preferences",t="icon/22/actions/dialog-ok.png",u="icon/22/apps/preferences-theme.png",w="hideItem",v="Open preferences window.",x="feedreader/images/loading22.gif";
qx.Class.define(E,{extend:qx.ui.toolbar.ToolBar,construct:function(I){qx.ui.toolbar.ToolBar.call(this);
this.__pE={};
this.__pF=new qx.ui.toolbar.Button(this.tr(B),t);
this.__pF.setCommand(I.getCommand(c));
this.add(this.__pF);
this.__pG=new qx.ui.toolbar.Button(this.tr(g),n);
this.__pG.setCommand(I.getCommand(b));
this.__pG.setEnabled(false);
this.add(this.__pG);
this.addSeparator();
var L=new qx.ui.toolbar.Button(this.tr(l),z);
var J=I.getCommand(r);
L.setCommand(J);
L.setToolTipText(this.tr(H,J.toString()));
this.add(L);
this.addSeparator();
this.__pH=new qx.ui.toolbar.Button(this.tr(f),u);
this.__pH.setCommand(I.getCommand(m));
this.__pH.setToolTipText(this.tr(v));
this.add(this.__pH);
this.addSpacer();
var M=new qx.ui.toolbar.Button(this.tr(G),a);
var K=I.getCommand(q);
M.setCommand(K);
M.setToolTipText(y+K.toString()+i);
this.add(M);
this.setOverflowHandling(true);
var N=new qx.ui.toolbar.MenuButton(null,D);
N.setAppearance(C);
this.add(N);
...
Q.setVisibility(d);
},_getMenuItem:function(S){var T=this.__pE[S.toHashCode()];
...
this.__pE[S.toHashCode()]=T;
}return T;
},signalLoading:function(U,V){if(U==A){var W=this.__pF;
}else if(U==p){var W=this.__pH;
}else{return ;
...
}catch(y){if(!qx.bom.request.Xhr.isCrossDomain(u)){throw y;
}
