...
(function(){var k="os.name",j="os.version",h="css.borderradius",g="default",f="browser.quirksmode",e="browser.name",d="event.pointer",c="io.ssl",b="locale.variant",a="css.textoverflow",W="html.xul",V="css.boxshadow",U="event.touch",T="io.maxrequests",S="qx.allowUrlSettings",R="css.gradients",Q="browser.documentmode",P="ecmascript.objectcount",O="locale",N="engine.version",r="engine.name",s="css.rgba",p="css.boxmodel",q="css.placeholder",n="|",o="browser.version",l="io.xhr",m="qx.core.Environment",t="qx.debug.databinding",u="qx.optimization.basecalls",B="qx.optimization.variables",z="true",F="qx.optimization.privates",D="qxenv",J="qx.aspects",H="qx.debug",w="qx.dynamicmousewheel",M=":",L="qx.optimization.strings",K="qx.optimization.comments",v="qx.optimization.variants",x="qx.mobile.emulatetouch",y="qx.dynlocale",A="false",C="&",E="qx.mobile.nativescroll",G="qx.allowUrlVariants",I="qx.propertyDebugLevel";
qx.Bootstrap.define(m,{statics:{_checks:{},_asyncChecks:{},__cJ:{},get:function(X){if(this.__cJ[X]!=undefined){return this.__cJ[X];
...
}},_initDefaultQxValues:function(){this.add(S,function(){return false;
});
...
for(var i=0;i<by.length;i++){var bA=by[i].split(M);

if(bA.length!=3||bA[0]!=D){continue;
}var bB=bA[1];
...
}if(this.useCheck(T)){this._checks[T]=qx.bom.client.Transport.getMaxConcurrentRequestCount;
}
if(this.useCheck(c)){this._checks[c]=qx.bom.client.Transport.getSsl;
}
if(this.useCheck(l)){this._checks[l]=qx.bom.client.Transport.getXmlHttpRequest;
}if(this.useCheck(U)){this._checks[U]=qx.bom.client.Event.getTouch;
}
if(this.useCheck(d)){this._checks[d]=qx.bom.client.Event.getPointer;
...
}if(this.useCheck(a)){this._checks[a]=qx.bom.client.Css.getTextOverflow;
}
...
if(this.useCheck(V)){this._checks[V]=qx.bom.client.Css.getBoxShadow;
}
...
},$$registry:{},__gb:null,__gd:function(){}}});
})();
...
},__jO:function(p,q,r,s){var w=r.$$members;

if(w){for(var v in w){if(qx.Bootstrap.isFunction(w[v])){var u=this.__jP(q,v);
...
if(x){p[v]=this.__jS(r,p[v],v,w[v]);
...
}}}}}},__jP:function(y,z){var D=z.match(/^(is|toggle|get|set|reset)(.*)$/);
...
},__jQ:function(E,F){if(F.$$properties){for(var G in F.$$properties){if(!qx.Bootstrap.getPropertyDefinition(E,G)){throw new Error('The property "'+G+'" is not supported by Class "'+E.classname+'"!');
}}}},__jR:function(H,I){if(I.$$events){for(var J in I.$$events){if(!qx.Bootstrap.supportsEvent(H,J)){throw new Error('The event "'+J+'" is not supported by Class "'+H.classname+'"!');
}}}},assertObject:function(K,L){var N=K.constructor;
this.__jO(K,N,L,false);
this.__jQ(N,L);
this.__jR(N,L);
var M=L.$$extends;

if(M){for(var i=0,l=M.length;i<l;i++){this.assertObject(K,M[i]);
}}},assert:function(O,P,Q){this.__jO(O.prototype,O,P,Q);
this.__jQ(O,P);
this.__jR(O,P);
var R=P.$$extends;
...
},$$registry:{},__jS:function(){},__gb:null,__gd:function(){}}});
})();
...
qx.Bootstrap.define(d,{statics:{__fE:{"Boolean":cS,"String":bG,"Number":ci,"Integer":cm,"PositiveNumber":C,"PositiveInteger":u,"Error":cX,"RegExp":cg,"Object":bA,"Array":E,"Map":bS,"Function":bI,"Date":cT,"Node":y,"Element":o,"Document":h,"Window":cP,"Event":cf,"Class":bJ,"Mixin":k,"Interface":r,"Theme":bp,"Color":f,"Decorator":A,"Font":g},__fF:{"Node":true,"Element":true,"Document":true,"Window":true,"Event":true},$$inherit:O,$$store:{runtime:{},user:{},theme:{},inherit:{},init:{},useinit:{}},$$method:{get:{},set:{},reset:{},init:{},refresh:{},setRuntime:{},resetRuntime:{},setThemed:{},resetThemed:{}},$$allowedKeys:{name:bv,dereference:bs,inheritable:bs,nullable:bs,themeable:bs,refine:bs,init:null,apply:bv,event:bv,check:null,transform:bv,deferredInit:bs,validate:null},$$allowedGroupKeys:{name:bv,group:bM,mode:bv,themeable:bs},$$inheritable:{},__fG:function(de){var df=this.__fH(de);

if(!df.length){var dg=function(){};
}else{dg=this.__fI(df);
}de.prototype.$$refreshInheritables=dg;
},__fH:function(dh){var dj=[];
...
},__fI:function(dk){var dp=this.$$store.inherit;
...
},attachRefreshInheritables:function(dq){dq.prototype.$$refreshInheritables=function(){qx.core.Property.__fG(dq);
return this.$$refreshInheritables();
};
},attachMethods:function(dr,name,ds){ds.group?this.__fJ(dr,ds,name):this.__fK(dr,ds,name);
},__fJ:function(dt,du,name){var dB=qx.Bootstrap.firstUp(name);
var dA=dt.prototype;
...
}},__fK:function(dE,dF,name){var dH=qx.Bootstrap.firstUp(name);
var dJ=dE.prototype;
if(dF.dereference===undefined&&typeof dF.check===bv){dF.dereference=this.__fL(dF.check);
}var dI=this.$$method;
var dG=this.$$store;
...
}},__fL:function(dP){return !!this.__fF[dP];
},__fM:function(dQ){return this.__fF[dQ]||qx.Bootstrap.classIsDefined(dQ)||(qx.Interface&&qx.Interface.isDefined(dQ));
},__fN:{0:b,1:X,2:dc,3:bk,4:cj,5:T},error:function(dR,dS,dT,dU,dV){var dW=dR.constructor.classname;
var dX=q+dT+M+dW+cY+this.$$method[dU][dT]+cJ+dV+L;
throw new Error(dX+(this.__fN[dS]||"Unknown reason: "+dS));
},__fO:function(dY,ea,name,eb,ec,ed){var ee=this.$$method[eb][name];
...
}return this.__fO(ef,el,name,eh,ei);
...
var ev=this.__fP(eo,name);
this.__fQ(er,eu,name,eo,eq);

if(eq){this.__fR(er,en,eu,name);
}
if(es){this.__fS(er,eq,ev,eo);
}
if(eu.inheritable){er.push(ba);
}
if(!es){this.__fU(er,name,eo,eq);
}else{this.__fV(er,eu,name,eo,eq);
}
if(eu.inheritable){this.__fW(er,eu,name,eo);
}else if(es){this.__fX(er,eu,name,eo);
}
if(es){this.__fY(er,eu,name);
if(eu.inheritable&&et._getChildren){this.__ga(er,name);
}}if(eq){er.push(bh);
}return this.__fO(em,et,name,eo,er,ep);
},__fP:function(ew,name){if(ew===cs||ew===br){var ex=this.$$store.runtime[name];
}else if(ew===ct||ew===bx){ex=this.$$store.theme[name];
}else if(ew===cr){ex=this.$$store.init[name];
}else{ex=this.$$store.user[name];
}return ex;
},__fQ:function(ey,ez,name,eA,eB){{if(!ez.nullable||ez.check||ez.inheritable){ey.push(cO);
}if(eA===cu){ey.push(U,name,v,eA,Y);
}};
},__fR:function(eC,eD,eE,name){if(eE.transform){eC.push(w,eE.transform,ca);
...
eC.push(cU);
}}},__fS:function(eF,eG,eH,eI){var eJ=(eI===bt||eI===bx||eI===br);

if(eG){eF.push(cq,eH,j);
}else if(eJ){eF.push(cq,eH,x);
}},__fT:undefined,__fU:function(eK,name,eL,eM){if(eL===cs){eK.push(cv,this.$$store.runtime[name],cw);
...
}},__fV:function(eN,eO,name,eP,eQ){if(eO.inheritable){eN.push(R,this.$$store.inherit[name],cy);
}else{eN.push(bR);
...
}},__fW:function(eR,eS,name,eT){eR.push(D);
...
},__fX:function(eU,eV,name,eW){if(eW!==cu&&eW!==cs&&eW!==ct){eU.push(bL);
}eU.push(cc);
if(eV.init!==undefined&&eW!==cr){eU.push(cH,this.$$store.init[name],cB);
}else{eU.push(cb);
}},__fY:function(eX,eY,name){if(eY.apply){eX.push(cv,eY.apply,bg,name,bH);
}if(eY.event){eX.push(bD,bj,eY.event,z,ck,eY.event,bb,J);
}},__ga:function(fa,name){fa.push(cW);
...
w=true;
}var x=this.__gf(name,v.type,v.extend,v.statics,v.construct,v.destruct,v.include);
if(v.extend){if(v.properties){this.__gh(x,v.properties,true);
}if(v.members){this.__gj(x,v.members,true,true,false);
}if(v.events){this.__gg(x,v.events,true);
}if(v.include){for(var i=0,l=v.include.length;i<l;i++){this.__gn(x,v.include[i],false);
}}}if(v.environment){for(var y in v.environment){qx.core.Environment.add(y,v.environment[y]);
}}if(v.implement){for(var i=0,l=v.implement.length;i<l;i++){this.__gl(x,v.implement[i]);
}}if(v.defer){v.defer.self=x;
v.defer(x,x.prototype,{add:function(name,z){var A={};
A[name]=z;
qx.Class.__gh(x,A,true);
}});
}return x;
...
},getByName:qx.Bootstrap.getByName,include:function(E,F){qx.Class.__gn(E,F,false);
},patch:function(G,H){qx.Class.__gn(G,H,true);
...
},$$registry:qx.Bootstrap.$$registry,__gb:null,__gc:null,__gd:function(){},__ge:function(){},__gf:function(name,be,bf,bg,bh,bi,bj){var bm;

if(!bf&&qx.core.Environment.get(u)==false){bm=bg||{};
qx.Bootstrap.setDisplayNames(bm,name);
}else{var bm={};

if(bf){if(!bh){bh=this.__go();
}
if(this.__gq(bf,bj)){bm=this.__gr(bh,name,be);
}else{bm=bh;
...
return bm;
},__gg:function(bo,bp,bq){var br,br;

if(bo.$$events){for(var br in bp){bo.$$events[br]=bp[br];
}}else{bo.$$events=bp;
}},__gh:function(bs,bt,bu){var bv;
...
}},__gl:function(bJ,bK){var bL=qx.Interface.flatten([bK]);
...
}},__gm:function(bM){var name=bM.classname;
var bN=this.__gr(bM,name,bM.$$classtype);
...
return bN;
},__gn:function(bS,bT,bU){if(this.hasMixin(bS,bT)){return;
}var bX=bS.$$original;

if(bT.$$constructor&&!bX){bS=this.__gm(bS);
}var bW=qx.Mixin.flatten([bT]);
var bV;

for(var i=0,l=bW.length;i<l;i++){bV=bW[i];
if(bV.$$events){this.__gg(bS,bV.$$events,bU);
}if(bV.$$properties){this.__gh(bS,bV.$$properties,bU);
}if(bV.$$members){this.__gj(bS,bV.$$members,bU,bU,bU);
}}if(bS.$$includes){bS.$$includes.push(bT);
bS.$$flatIncludes.push.apply(bS.$$flatIncludes,bW);
}else{bS.$$includes=[bT];
bS.$$flatIncludes=bW;
}},__go:function(){function bY(){bY.base.apply(this,arguments);
}return bY;
},__gp:function(){return function(){};
},__gq:function(ca,cb){if(ca&&ca.$$includes){var cc=ca.$$flatIncludes;
...
},__gr:function(ce,name,cf){var ch=function(){var ck=ch;
...
qx.Class.define(f,{statics:{inShutDown:false,__hg:{},__hh:0,__hi:[],__hj:d,register:function(h){var m=this.__hg;

if(!m){return;
}var k=h.$$hash;

if(k==null){var j=this.__hi;

if(j.length>0){k=j.pop();
}else{k=(this.__hh++)+this.__hj;
}h.$$hash=k;
}m[k]=h;
...
}else{t=(this.__hh++)+this.__hj;
}return r.$$hash=t;
},clearHashCode:function(u){var v=u.$$hash;

if(v!=null){this.__hi.push(v);
try{delete u.$$hash;
}catch(w){if(u.removeAttribute){u.removeAttribute(g);
}}}},fromHashCode:function(x){return this.__hg[x]||null;
...
for(var i=0;i<frames.length;i++){if(frames[i]===window){D.__hj=e+(i+1);
return;
}}}D.__hj=c;
}});
})();
...
(function(){var r="|bubble",q="|capture",p="|",o="",n="_",m="unload",k="UNKNOWN_",j="__jy",h="__jz",g="c",c="DOM_",f="WIN_",e="QX_",b="qx.event.Manager",a="capture",d="DOCUMENT_";
qx.Class.define(b,{extend:Object,construct:function(s,t){this.__cp=s;
this.__jH=qx.core.ObjectRegistry.toHashCode(s);
this.__cT=t;
...
this.__jz={};
this.__jJ={};
},statics:{__jK:0,getNextUniqueId:function(){return (this.__jK++)+o;
...
},getHandler:function(u){var v=this.__jy[u.classname];

if(v){return v;
}return this.__jy[u.classname]=new u(this);
},getDispatcher:function(w){var x=this.__jz[w.classname];

if(x){return x;
}return this.__jz[w.classname]=new w(this,this.__cT);
...
if(Q){this.__jM(P,W,R);
}else{this.__jN(P,W,R);
...
if(!bj){bj=bo[bm]=[];
this.__jM(bg,bl.type,bl.capture);
}bj.push({handler:bl.listener,context:bl.self,unique:bl.unique||(bk.__jK++)+o});
}},addListener:function(bp,bq,br,self,bs){var bw;
var bx=bp.$$hash||qx.core.ObjectRegistry.toHashCode(bp);
var bz=this.__jI[bx];

if(!bz){bz=this.__jI[bx]={};
}var bv=bq+(bs?q:r);
var bu=bz[bv];

if(!bu){bu=bz[bv]=[];
}if(bu.length===0){this.__jM(bp,bq,bs);
}var by=(qx.event.Manager.__jK++)+o;
...
},__jM:function(bP,bQ,bR){var bS=this.findHandler(bP,bQ);
...
if(bY.length==0){this.__jN(bT,bU,bW);
}return true;
}}return false;
...
if(ci.length==0){this.__jN(ce,co,cg);
}return true;
}}return false;
...
return true;
},deleteAllListeners:function(cx){delete this.__jI[cx];
},__jN:function(cy,cz,cA){var cB=this.findHandler(cy,cz);
...
}var cG=this.__cT.getDispatchers();
var cF;
var cE=false;
...
qx.Class.define(c,{statics:{__jx:{},getManager:function(d){if(d==null){d=window;
...
},PRIORITY_FIRST:-32000,PRIORITY_NORMAL:0,PRIORITY_LAST:32000,__jy:[],addHandler:function(Q){this.__jy.push(Q);
this.__jy.sort(function(a,b){return a.PRIORITY-b.PRIORITY;
});
},getHandlers:function(){return this.__jy;
},__jz:[],addDispatcher:function(R,S){this.__jz.push(R);
...
qx.Class.define(s,{statics:{__jA:A,setLevel:function(I){this.__jA=I;
},getLevel:function(){return this.__jA;
},setTreshold:function(J){this.__jC.setMaxMessages(J);
},getTreshold:function(){return this.__jC.getMaxMessages();
},__jB:{},__dc:0,register:function(K){if(K.$$id){return;
}var M=this.__dc++;
this.__jB[M]=K;
K.$$id=M;
var L=this.__jD;
var N=this.__jC.getAllLogEvents();

for(var i=0,l=N.length;i<l;i++){if(L[N[i].level]>=L[this.__jA]){K.process(N[i]);
}}},unregister:function(O){var P=O.$$id;

if(P==null){return;
}delete this.__jB[P];
delete O.$$id;
},debug:function(Q,R){qx.log.Logger.__jE(A,arguments);
},info:function(S,T){qx.log.Logger.__jE(e,arguments);
},warn:function(U,V){qx.log.Logger.__jE(r,arguments);
},error:function(W,X){qx.log.Logger.__jE(j,arguments);
},trace:function(Y){qx.log.Logger.__jE(e,[Y,qx.dev.StackTrace.getStackTrace().join(u)]);
...
},__jC:new qx.log.appender.RingBuffer(50),__jD:{debug:0,info:1,warn:2,error:3},__jE:function(bv,bw){var bB=this.__jD;

if(bB[bv]<bB[this.__jA]){return;
}var by=bw.length<2?null:bw[0];
var bA=by?1:0;
var bx=[];

for(var i=bA,l=bw.length;i<l;i++){bx.push(this.__jG(bw[i],true));
}var bC=new Date;
var bD={time:bC,offset:bC-qx.Bootstrap.LOADSTART,level:bv,items:bx,win:window};
if(by){if(by.$$hash!==undefined){bD.object=by.$$hash;
}else if(by.$$type){bD.clazz=by;
}}this.__jC.process(bD);
var bE=this.__jB;

for(var bz in bE){bE[bz].process(bD);
}},__jF:function(bF){if(bF===undefined){return y;
...
},__jG:function(bH,bI){var bP=this.__jF(bH);
var bL=D;
var bK=[];
...
for(var i=0;i<bR.length;i++){bQ.__jE(bR[i][0],bR[i][1]);
...
},__dn:qx.event.Registration,addListener:function(H,I,self,J){if(!this.$$disposed){return this.__dn.addListener(this,H,I,self,J);
}return null;
},addListenerOnce:function(K,L,self,M){var N=function(e){this.removeListener(K,N,this,M);
L.call(self||this,e);
};
return this.addListener(K,N,this,M);
},removeListener:function(O,P,self,Q){if(!this.$$disposed){return this.__dn.removeListener(this,O,P,self,Q);
}return false;
},removeListenerById:function(R){if(!this.$$disposed){return this.__dn.removeListenerById(this,R);
}return false;
},hasListener:function(S,T){return this.__dn.hasListener(this,S,T);
},dispatchEvent:function(U){if(!this.$$disposed){return this.__dn.dispatchEvent(this,U);
}return true;
},fireEvent:function(V,W,X){if(!this.$$disposed){return this.__dn.fireEvent(this,V,W,X);
}return true;
},fireNonBubblingEvent:function(Y,ba,bb){if(!this.$$disposed){return this.__dn.fireNonBubblingEvent(this,Y,ba,bb);
}return true;
},fireDataEvent:function(bc,bd,be,bf){if(!this.$$disposed){if(be===undefined){be=null;
}return this.__dn.fireNonBubblingEvent(this,bc,qx.event.type.Data,[bd,be,!!bf]);
}return true;
},__do:null,setUserData:function(bg,bh){if(!this.__do){this.__do={};
}this.__do[bg]=bh;
},getUserData:function(bi){if(!this.__do){return null;
}var bj=this.__do[bi];
return bj===undefined?null:bj;
},__dp:qx.log.Logger,debug:function(bk){this.__dq(f,arguments);
},info:function(bl){this.__dq(m,arguments);
},warn:function(bm){this.__dq(h,arguments);
},error:function(bn){this.__dq(j,arguments);
},trace:function(){this.__dp.trace(this);
},__dq:function(bo,bp){var bq=qx.lang.Array.fromArguments(bp);
bq.unshift(this);
this.__dp[bo].apply(this.__dp,bq);
...
}},__dr:null,__ds:function(){var bx=qx.Class.getProperties(this.constructor);
...
(function(){var m="element",k="qxSelectable",j="off",h="engine.name",g="on",f="text",d="div",c="",b="mshtml",a="none",F="scroll",E="qx.html.Element",D="|capture|",C="activate",B="blur",A="deactivate",z="capture",w="__iL",v="userSelect",u="-moz-none",s="visible",t="releaseCapture",q="|bubble|",r="tabIndex",o="focus",p="MozUserSelect",n="hidden";
qx.Class.define(E,{extend:qx.core.Object,construct:function(G,H,I){qx.core.Object.call(this);
this.__iq=G||d;
this.__ir=H||null;
this.__is=I||null;
},statics:{DEBUG:false,_modified:{},_visibility:{},_scroll:{},_actions:[],__it:{},_scheduleFlush:function(J){qx.html.Element.__eO.schedule();
},flush:function(){var U;
var M=this.__iu();
var L=M.getFocus();

if(L&&this.__iy(L)){M.blur(L);
}var bc=M.getActive();

if(bc&&this.__iy(bc)){qx.bom.Element.deactivate(bc);
}var P=this.__iw();

if(P&&this.__iy(P)){qx.bom.Element.releaseCapture(P);
}var V=[];
var W=this._modified;

for(var T in W){U=W[T];
if(U.__iP()){if(U.__iz&&qx.dom.Hierarchy.isRendered(U.__iz)){V.push(U);
}else{U.__iO();
}delete W[T];
}}
...
if(!X){delete R[T];
continue;
}if(!U.$$disposed){X.style.display=U.__iB?c:a;
if((qx.core.Environment.get(h)==b)){if(!(document.documentMode>=8)){X.style.visibility=U.__iB?s:n;
}}}delete R[T];
...
if(U.__iE!=null){U.__iz.scrollLeft=U.__iE;
delete U.__iE;
}if(U.__iF!=null){U.__iz.scrollTop=U.__iF;
delete U.__iF;
}var Y=U.__iC;
...
var X=bb.element.__iz;

if(!X||!N[bb.type]&&!bb.element.__iP()){continue;
}var Q=bb.args;
Q.unshift(X);
qx.bom.Element[bb.type].apply(qx.bom.Element,Q);
}this._actions=[];
for(var T in this.__it){var K=this.__it[T];
var bd=K.element.__iz;

if(bd){qx.bom.Selection.set(bd,K.start,K.end);
delete this.__it[T];
}}qx.event.handler.Appear.refresh();
},__iu:function(){if(!this.__iv){var be=qx.event.Registration.getManager(window);
this.__iv=be.getHandler(qx.event.handler.Focus);
}return this.__iv;
},__iw:function(){if(!this.__ix){var bf=qx.event.Registration.getManager(window);
this.__ix=bf.getDispatcher(qx.event.dispatch.MouseCapture);
}return this.__ix.getCaptureElement();
},__iy:function(bg){var bh=qx.core.ObjectRegistry.fromHashCode(bg.$$element);
return bh&&!bh.__iP();
...
if(bj.__iB&&bj.__iA&&!bj.__iz){bj.__iO();
}}}
...
},_insertChildren:function(){var bk=this.__iL;
...
if(bm.__iz&&bm.__iA){bl.appendChild(bm.__iz);
}}this.__iz.appendChild(bl);
}else{var bl=this.__iz;
...
if(!bs||!bs.__iA||bs.__iN!==this){bq.removeChild(bv);
}}for(var i=0;i<bu;i++){bo=bn[i];
if(bo.__iA){bs=bo.__iz;
bv=bt[bp];
...
}}},_copyData:function(bx){var bB=this.__iz;
var bA=this.__is;
...
delete this.__iK;
}},_syncData:function(){var bH=this.__iz;
...
if(bF){var bK=this.__iJ;

if(bK){var bI;

for(var bJ in bF){this._applyProperty(bJ,bK[bJ]);
}}this.__iI=null;
}},__iP:function(){var bL=this;
while(bL){if(bL.__cq){return true;
}
if(!bL.__iA||!bL.__iB){return false;
}bL=bL.__iN;
}return false;
},__iQ:function(bM){if(bM.__iN===this){throw new Error("Child is already in: "+bM);
}
if(bM.__cq){throw new Error("Root elements could not be inserted into other ones.");
}if(bM.__iN){bM.__iN.remove(bM);
}bM.__iN=this;
if(!this.__iL){this.__iL=[];
}if(this.__iz){this._scheduleChildrenUpdate();
}},__iR:function(bN){if(bN.__iN!==this){throw new Error("Has no child: "+bN);
}if(this.__iz){this._scheduleChildrenUpdate();
}delete bN.__iN;
},__iS:function(bO){if(bO.__iN!==this){throw new Error("Has no child: "+bO);
}if(this.__iz){this._scheduleChildrenUpdate();
}},getChildren:function(){return this.__iL||null;
},getChild:function(bP){var bQ=this.__iL;
return bQ&&bQ[bP]||null;
},hasChildren:function(){var bR=this.__iL;
return bR&&bR[0]!==undefined;
},indexOf:function(bS){var bT=this.__iL;
return bT?bT.indexOf(bS):-1;
},hasChild:function(bU){var bV=this.__iL;
return bV&&bV.indexOf(bU)!==-1;
},add:function(bW){if(arguments[1]){for(var i=0,l=arguments.length;i<l;i++){this.__iQ(arguments[i]);
}this.__iL.push.apply(this.__iL,arguments);
}else{this.__iQ(bW);
this.__iL.push(bW);
}return this;
},addAt:function(bX,bY){this.__iQ(bX);
qx.lang.Array.insertAt(this.__iL,bX,bY);
return this;
},remove:function(ca){var cb=this.__iL;
...
},removeAt:function(cd){var ce=this.__iL;
...
qx.lang.Array.removeAt(this.__iL,cd);
return this;
},removeAll:function(){var cg=this.__iL;

if(cg){for(var i=0,l=cg.length;i<l;i++){this.__iR(cg[i]);
}cg.length=0;
}return this;
},getParent:function(){return this.__iN||null;
},insertInto:function(parent,ch){parent.__iQ(this);

if(ch==null){parent.__iL.push(this);
}else{qx.lang.Array.insertAt(this.__iL,this,ch);
}return this;
},insertBefore:function(ci){var parent=ci.__iN;
parent.__iQ(this);
qx.lang.Array.insertBefore(parent.__iL,this,ci);
return this;
},insertAfter:function(cj){var parent=cj.__iN;
parent.__iQ(this);
qx.lang.Array.insertAfter(parent.__iL,this,cj);
return this;
},moveTo:function(ck){var parent=this.__iN;
parent.__iS(this);
var cl=parent.__iL.indexOf(this);

if(cl===ck){throw new Error("Could not move to same index!");
}else if(cl<ck){ck--;
}qx.lang.Array.removeAt(parent.__iL,cl);
qx.lang.Array.insertAt(parent.__iL,this,ck);
return this;
},moveBefore:function(cm){var parent=this.__iN;
return this.moveTo(parent.__iL.indexOf(cm));
},moveAfter:function(cn){var parent=this.__iN;
return this.moveTo(parent.__iL.indexOf(cn)+1);
...
},useMarkup:function(cp){if(this.__iz){throw new Error("Could not overwrite existing element!");
...
return this.__iz;
},useElement:function(cr){if(this.__iz){throw new Error("Could not overwrite existing element!");
}this.__iz=cr;
this.__iz.$$element=this.$$hash;
this._copyData(true);
...
if(ct>=0&&cs[this.__iq]){return true;
}return false;
...
},getScrollX:function(){var cJ=this.__iz;

if(cJ){return cJ.scrollLeft;
}return this.__iE||0;
},scrollToY:function(y,cK){var cL=this.__iz;
...
},getScrollY:function(){var cM=this.__iz;
...
this.scrollToX(0);
this.scrollToY(0);
this.addListener(F,this.__iU,this);
},enableScrolling:function(){this.removeListener(F,this.__iU,this);
},__iT:null,__iU:function(e){if(!this.__iT){this.__iT=true;
this.__iz.scrollTop=0;
this.__iz.scrollLeft=0;
delete this.__iT;
}},getTextSelection:function(){var cN=this.__iz;

if(cN){return qx.bom.Selection.get(cN);
}return null;
},getTextSelectionLength:function(){var cO=this.__iz;

if(cO){return qx.bom.Selection.getLength(cO);
}return null;
},getTextSelectionStart:function(){var cP=this.__iz;

if(cP){return qx.bom.Selection.getStart(cP);
}return null;
},getTextSelectionEnd:function(){var cQ=this.__iz;

if(cQ){return qx.bom.Selection.getEnd(cQ);
}return null;
},setTextSelection:function(cR,cS){var cT=this.__iz;

if(cT){qx.bom.Selection.set(cT,cR,cS);
return;
}qx.html.Element.__it[this.toHashCode()]={element:this,start:cR,end:cS};
qx.html.Element._scheduleFlush(m);
},clearTextSelection:function(){var cU=this.__iz;

if(cU){qx.bom.Selection.clear(cU);
}delete qx.html.Element.__it[this.toHashCode()];
},__iV:function(cV,cW){var cX=qx.html.Element._actions;
cX.push({type:cV,element:this,args:cW||[]});
qx.html.Element._scheduleFlush(m);
},focus:function(){this.__iV(o);
},blur:function(){this.__iV(B);
},activate:function(){this.__iV(C);
},deactivate:function(){this.__iV(A);
},capture:function(cY){this.__iV(z,[cY!==false]);
},releaseCapture:function(){this.__iV(t);
},setStyle:function(da,dc,dd){if(!this.__ir){this.__ir={};
}
if(this.__ir[da]==dc){return;
}
if(dc==null){delete this.__ir[da];
}else{this.__ir[da]=dc;
}if(this.__iz){if(dd){qx.bom.element.Style.set(this.__iz,da,dc);
return this;
...
if(this.__ir[di]==dh){continue;
}
if(dh==null){delete this.__ir[di];
}else{this.__ir[di]=dh;
}if(df){dg.set(this.__iz,di,dh);
continue;
}this.__iG[di]=true;
}qx.html.Element._modified[this.$$hash]=this;
qx.html.Element._scheduleFlush(m);
...
},getStyle:function(dl){return this.__ir?this.__ir[dl]:null;
},getAllStyles:function(){return this.__ir||null;
},setAttribute:function(dm,dn,dp){if(!this.__is){this.__is={};
}
if(this.__is[dm]==dn){return;
}
if(dn==null){delete this.__is[dm];
}else{this.__is[dm]=dn;
}if(this.__iz){if(dp){qx.bom.element.Attribute.set(this.__iz,dm,dn);
return this;
...
},getAttribute:function(dv){return this.__is?this.__is[dv]:null;
},_applyProperty:function(name,dw){},_setProperty:function(dx,dy,dz){if(!this.__iJ){this.__iJ={};
}
if(this.__iJ[dx]==dy){return;
}
if(dy==null){delete this.__iJ[dx];
}else{this.__iJ[dx]=dy;
}if(this.__iz){if(dz){this._applyProperty(dx,dy);
return this;
...
},_getProperty:function(dC){var dD=this.__iJ;
...
if(this.__iz){return qx.event.Registration.addListener(this.__iz,dF,dG,self,dH);
}
...
var dK=dF+(dH?D:q)+dJ;
this.__iK[dK]={type:dF,listener:dG,self:self,capture:dH,unique:dJ};
return dK;
},removeListener:function(dL,dM,self,dN){var dO;

if(this.$$disposed){return null;
}
if(this.__iz){qx.event.Registration.removeListener(this.__iz,dL,dM,self,dN);
}else{var dQ=this.__iK;
var dP;
...
if(this.__iz){qx.event.Registration.removeListenerById(this.__iz,dS);
}else{delete this.__iK[dS];
}return this;
},hasListener:function(dT,dU){if(this.$$disposed){return false;
}
if(this.__iz){return qx.event.Registration.hasListener(this.__iz,dT,dU);
}var dW=this.__iK;
var dV;
...
}},defer:function(dY){dY.__eO=new qx.util.DeferredCall(dY.flush,dY);
},destruct:function(){var ea=this.__iz;
...
qx.Mixin.define(b,{members:{__dL:function(m,n,o,p){var q=this.getChildrenContainer();

if(q===this){m=h+m;
}return (q[m])(n,o,p);
},getChildren:function(){return this.__dL(c);
},hasChildren:function(){return this.__dL(f);
},add:function(r,s){return this.__dL(j,r,s);
},remove:function(t){return this.__dL(a,t);
},removeAll:function(){return this.__dL(d);
},indexOf:function(u){return this.__dL(l,u);
},addAt:function(v,w,x){this.__dL(g,v,w,x);
},addBefore:function(y,z,A){this.__dL(i,y,z,A);
},addAfter:function(B,C,D){this.__dL(k,B,C,D);
},removeAt:function(E){this.__dL(e,E);
}}});
})();
(function(){var o="top",n="right",m="bottom",l="left",k="align-start",j="qx.util.placement.AbstractAxis",i="edge-start",h="align-end",g="edge-end",f="-",c="best-fit",e="qx.util.placement.Placement",d='__dM',b="keep-align",a="direct";
qx.Class.define(e,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__dM=new qx.util.placement.DirectAxis();
},properties:{axisX:{check:j},axisY:{check:j},edge:{check:[o,n,m,l],init:o},align:{check:[o,n,m,l],init:n}},statics:{__dN:null,compute:function(p,q,r,s,t,u,v){this.__dN=this.__dN||new qx.util.placement.Placement();
var y=t.split(f);
var x=y[0];
var w=y[1];
this.__dN.set({axisX:this.__dR(u),axisY:this.__dR(v),edge:x,align:w});
return this.__dN.compute(p,q,r,s);
},__dO:null,__dP:null,__dQ:null,__dR:function(z){switch(z){case a:this.__dO=this.__dO||new qx.util.placement.DirectAxis();
return this.__dO;
case b:this.__dP=this.__dP||new qx.util.placement.KeepAlignAxis();
return this.__dP;
case c:this.__dQ=this.__dQ||new qx.util.placement.BestFitAxis();
return this.__dQ;
default:throw new Error("Invalid 'mode' argument!'");
}}},members:{__dM:null,compute:function(A,B,C,D){var E=this.getAxisX()||this.__dM;
var G=E.computeStart(A.width,{start:C.left,end:C.right},{start:D.left,end:D.right},B.width,this.__dS());
var F=this.getAxisY()||this.__dM;
var top=F.computeStart(A.height,{start:C.top,end:C.bottom},{start:D.top,end:D.bottom},B.height,this.__dT());
return {left:G,top:top};
},__dS:function(){var I=this.getEdge();
var H=this.getAlign();
...
}},__dT:function(){var K=this.getEdge();
var J=this.getAlign();
...
(function(){var bY="px",bX="Boolean",bW="qx.event.type.Drag",bV="qx.event.type.Mouse",bU="visible",bT="qx.event.type.Focus",bS="Integer",bR="qx.event.type.Touch",bQ="qx.event.type.Data",bP="excluded",bx="_applyPadding",bw="qx.event.type.Event",bv="on",bu="hidden",bt="engine.name",bs="contextmenu",br="String",bq="tabIndex",bp="focused",bo="changeVisibility",cg="mshtml",ch="hovered",ce="qx.event.type.KeySequence",cf="absolute",cc="backgroundColor",cd="drag",ca="div",cb="disabled",ci="move",cj="dragstart",bI="qx.dynlocale",bH="dragchange",bK="dragend",bJ="resize",bM="Decorator",bL="zIndex",bO="opacity",bN="default",bG="Color",bF="changeToolTipText",c="beforeContextmenuOpen",d="_applyNativeContextMenu",f="_applyBackgroundColor",g="event.pointer",h="_applyFocusable",j="changeShadow",k="qx.event.type.KeyInput",m="createChildControl",n="Font",o="_applyShadow",cn="_applyEnabled",cm="_applySelectable",cl="__jc",ck="Number",cr="_applyKeepActive",cq="_applyVisibility",cp="__jk",co="repeat",ct="qxDraggable",cs="syncAppearance",N="paddingLeft",O="_applyDroppable",L="#",M="__iX",R="qx.event.type.MouseWheel",S="_applyCursor",P="_applyDraggable",Q="__ji",J="changeTextColor",K="$$widget",w="changeContextMenu",v="paddingTop",y="changeSelectable",x="hideFocus",s="none",r="outline",u="_applyAppearance",t="__iW",q="_applyOpacity",p="url(",X=")",Y="qx.ui.core.Widget",ba="_applyFont",bb="cursor",T="qxDroppable",U="changeZIndex",V="changeEnabled",W="changeFont",bc="_applyDecorator",bd="_applyZIndex",G="_applyTextColor",F="qx.ui.menu.Menu",E="__jg",D="_applyToolTipText",C="true",B="widget",A="changeDecorator",z="__jb",I="_applyTabIndex",H="changeAppearance",be="__jd",bf="shorthand",bg="/",bh="",bi="_applyContextMenu",bj="paddingBottom",bk="changeNativeContextMenu",bl="undefined",bm="qx.ui.tooltip.ToolTip",bn="qxKeepActive",bB="_applyKeepFocus",bA="paddingRight",bz="changeBackgroundColor",by="changeLocale",bE="qxKeepFocus",bD="opera",bC="qx/static/blank.gif";
...
},events:{appear:bw,disappear:bw,createChildControl:bQ,resize:bQ,move:bQ,syncAppearance:bQ,mousemove:bV,mouseover:bV,mouseout:bV,mousedown:bV,mouseup:bV,click:bV,dblclick:bV,contextmenu:bV,beforeContextmenuOpen:bQ,mousewheel:R,touchstart:bR,touchend:bR,touchmove:bR,touchcancel:bR,tap:bR,swipe:bR,keyup:ce,keydown:ce,keypress:ce,keyinput:k,focus:bT,blur:bT,focusin:bT,focusout:bT,activate:bT,deactivate:bT,capture:bw,losecapture:bw,drop:bW,dragleave:bW,dragover:bW,drag:bW,dragstart:bW,dragend:bW,dragchange:bW,droprequest:bW},properties:{paddingTop:{check:bS,init:0,apply:bx,themeable:true},paddingRight:{check:bS,init:0,apply:bx,themeable:true},paddingBottom:{check:bS,init:0,apply:bx,themeable:true},paddingLeft:{check:bS,init:0,apply:bx,themeable:true},padding:{group:[v,bA,bj,N],mode:bf,themeable:true},zIndex:{nullable:true,init:null,apply:bd,event:U,check:bS,themeable:true},decorator:{nullable:true,init:null,apply:bc,event:A,check:bM,themeable:true},shadow:{nullable:true,init:null,apply:o,event:j,check:bM,themeable:true},backgroundColor:{nullable:true,check:bG,apply:f,event:bz,themeable:true},textColor:{nullable:true,check:bG,apply:G,event:J,themeable:true,inheritable:true},font:{nullable:true,apply:ba,check:n,event:W,themeable:true,inheritable:true,dereference:true},opacity:{check:ck,apply:q,themeable:true,nullable:true,init:null},cursor:{check:br,apply:S,themeable:true,inheritable:true,nullable:true,init:null},toolTip:{check:bm,nullable:true},toolTipText:{check:br,nullable:true,event:bF,apply:D},toolTipIcon:{check:br,nullable:true,event:bF},blockToolTip:{check:bX,init:false},visibility:{check:[bU,bu,bP],init:bU,apply:cq,event:bo},enabled:{init:true,check:bX,inheritable:true,apply:cn,event:V},anonymous:{init:false,check:bX},tabIndex:{check:bS,nullable:true,apply:I},focusable:{check:bX,init:false,apply:h},keepFocus:{check:bX,init:false,apply:bB},keepActive:{check:bX,init:false,apply:cr},draggable:{check:bX,init:false,apply:P},droppable:{check:bX,init:false,apply:O},selectable:{check:bX,init:false,event:y,apply:cm},contextMenu:{check:F,apply:bi,nullable:true,event:w},nativeContextMenu:{check:bX,init:false,themeable:true,event:bk,apply:d},appearance:{check:br,init:B,apply:u,event:H}},statics:{DEBUG:false,getWidgetByElement:function(cu,cv){while(cu){var cw=cu.$$widget;
...
},_setLayout:function(cz){if(this.__jg){this.__jg.connectToWidget(null);
}
...
if(cP.size){var cO=this.__jd;

if(cO){cO.setStyles({width:cF+bY,height:cG+bY});
}}
if(cP.size||this._updateInsets){if(this.__jb){this.__jb.resize(cF,cG);
}}
if(cP.size){if(this.__jc){var cH=this.__jc.getInsets();
var cL=cF+cH.left+cH.right;
var cJ=cG+cH.top+cH.bottom;
this.__jc.resize(cL,cJ);
}}
if(cM||cP.local||cP.margin){if(this.__jg&&this.hasLayoutChildren()){this.__jg.renderLayout(innerWidth,innerHeight);
...
return cP;
},__ji:null,clearSeparators:function(){var cS=this.__ji;

if(!cS){return;
}var cT=qx.ui.core.Widget.__iY;
var content=this.getContentElement();
var cR;
...
},renderSeparator:function(cU,cV){var cW=qx.ui.core.Widget.__iY.getDecoratorElement(cU);
this.getContentElement().add(cW);
cW.resize(cV.width,cV.height);
cW.setStyles({left:cV.left+bY,top:cV.top+bY});
if(!this.__ji){this.__ji=[cW];
}else{this.__ji.push(cW);
...
}},_getContentHint:function(){var dj=this.__jg;
...
if(this.__jb){var dv=this.__jb.getInsets();
top+=dv.top;
dw+=dv.right;
dy+=dv.bottom;
dx+=dv.left;
...
},__jj:function(){var dF=this._createContentElement();
...
},__jk:null,getLayoutChildren:function(){var dH=this.__jk;

if(!dH){return this.__jl;
}var dI;
...
},invalidateLayoutChildren:function(){var dJ=this.__jg;

if(dJ){dJ.invalidateChildrenCache();
}qx.ui.core.queue.Layout.add(this);
},hasLayoutChildren:function(){var dK=this.__jk;
...
},_indexOf:function(dM){var dN=this.__jk;

if(!dN){return -1;
}return dN.indexOf(dM);
},_hasChildren:function(){var dO=this.__jk;
return dO!=null&&(!!dO[0]);
},addChildrenToQueue:function(dP){var dQ=this.__jk;
...
}},_add:function(dS,dT){if(dS.getLayoutParent()==this){qx.lang.Array.remove(this.__jk,dS);
}
if(this.__jk){this.__jk.push(dS);
}else{this.__jk=[dS];
}this.__jm(dS,dT);
},_addAt:function(dU,dV,dW){if(!this.__jk){this.__jk=[];
}if(dU.getLayoutParent()==this){qx.lang.Array.remove(this.__jk,dU);
}var dX=this.__jk[dV];

if(dX===dU){dU.setLayoutProperties(dW);
}
if(dX){qx.lang.Array.insertBefore(this.__jk,dU,dX);
}else{this.__jk.push(dU);
}this.__jm(dU,dW);
},_addBefore:function(dY,ea,eb){if(dY==ea){return;
}
if(!this.__jk){this.__jk=[];
}if(dY.getLayoutParent()==this){qx.lang.Array.remove(this.__jk,dY);
}qx.lang.Array.insertBefore(this.__jk,dY,ea);
this.__jm(dY,eb);
},_addAfter:function(ec,ed,ee){if(ec==ed){return;
}
if(!this.__jk){this.__jk=[];
}if(ec.getLayoutParent()==this){qx.lang.Array.remove(this.__jk,ec);
}qx.lang.Array.insertAfter(this.__jk,ec,ed);
this.__jm(ec,ee);
},_remove:function(ef){if(!this.__jk){throw new Error("This widget has no children!");
}qx.lang.Array.remove(this.__jk,ef);
this.__jn(ef);
},_removeAt:function(eg){if(!this.__jk){throw new Error("This widget has no children!");
}var eh=this.__jk[eg];
qx.lang.Array.removeAt(this.__jk,eg);
this.__jn(eh);
return eh;
},_removeAll:function(){if(!this.__jk){return [];
}var ei=this.__jk.concat();
this.__jk.length=0;

for(var i=ei.length-1;i>=0;i--){this.__jn(ei[i]);
}qx.ui.core.queue.Layout.add(this);
return ei;
},_afterAddChild:null,_afterRemoveChild:null,__jm:function(ej,ek){var parent=ej.getLayoutParent();
...
}},__jn:function(em){if(em.getLayoutParent()!==this){throw new Error("Remove Error: "+em+" is not a child of this widget!");
...
}var eq=this.__jd=new qx.html.Element;
eq.setStyles({position:cf,top:0,left:0,zIndex:7});
var er=this.getBounds();

if(er){this.__jd.setStyles({width:er.width+bY,height:er.height+bY});
...
},_applyDecorator:function(es,et){var ew=qx.ui.core.Widget.__iY;
var eu=this.getContainerElement();
if(!this.__jd&&!qx.core.Environment.get(g)){this._createProtectorElement();
}if(et){eu.remove(this.__jb);
ew.poolDecorator(this.__jb);
}if(es){var ev=this.__jb=ew.getDecoratorElement(es);
ev.setStyle(bL,5);
eu.add(ev);
}else{delete this.__jb;
}this._applyBackgroundColor(this.getBackgroundColor());
if(this.__jh(et,es)){this._updateInsets=true;
qx.ui.core.queue.Layout.add(this);
}else if(es){var ex=this.getBounds();

if(ex){ev.resize(ex.width,ex.height);
this.__jd&&
this.__jd.setStyles({width:ex.width+bY,height:ex.height+bY});
}}},_applyShadow:function(ey,ez){var eG=qx.ui.core.Widget.__ja;
var eB=this.getContainerElement();
if(ez){eB.remove(this.__jc);
eG.poolDecorator(this.__jc);
}if(ey){var eD=this.__jc=eG.getDecoratorElement(ey);
eB.add(eD);
var eF=eD.getInsets();
...
}},_applyToolTipText:function(eH,eI){if(qx.core.Environment.get(bI)){if(this.__jf){return;
}var eJ=qx.locale.Manager.getInstance();
this.__jf=eJ.addListener(by,function(){var eK=this.getToolTipText();
...
if(this.__jb){this.__jb.tint(fa);
fc.setStyle(cc,null);
}else{var fb=qx.theme.manager.Color.getInstance().resolve(fa);
fc.setStyle(cc,fb);
}},_applyFont:function(fd,fe){},__jo:null,$$stateChanges:null,_forwardStates:null,hasState:function(ff){var fg=this.__jo;
return !!fg&&!!fg[ff];
},addState:function(fh){var fi=this.__jo;
...
}}}},removeState:function(fm){var fn=this.__jo;
...
}}}},replaceState:function(fr,fs){var ft=this.__jo;
...
}}}},__jp:null,__jq:null,syncAppearance:function(){var fB=this.__jo;
var fA=this.__jp;
...
}}this.fireDataEvent(cs,this.__jo);
...
},hasChildControl:function(gy){if(!this.__jr){return false;
}return !!this.__jr[gy];
},__jr:null,_getCreatedChildControls:function(){return this.__jr;
},getChildControl:function(gz,gA){if(!this.__jr){if(gA){return null;
}this.__jr={};
}var gB=this.__jr[gz];
...
},_createChildControl:function(gI){if(!this.__jr){this.__jr={};
}else if(this.__jr[gI]){throw new Error("Child control '"+gI+"' already created!");
}var gM=gI.indexOf(L);
...
return this.__jr[gI]=gJ;
},_createChildControlImpl:function(gN,gO){return null;
},_disposeChildControls:function(){var gS=this.__jr;
...
}},destruct:function(){if(!qx.core.ObjectRegistry.inShutDown){if(qx.core.Environment.get(bI)){if(this.__jf){qx.locale.Manager.getInstance().removeListenerById(this.__jf);
...
if(this.__jb){hh.remove(this.__jb);
hi.__iY.poolDecorator(this.__jb);
}
if(this.__jc){hh.remove(this.__jc);
hi.__ja.poolDecorator(this.__jc);
}this.clearSeparators();
this.__jb=this.__jc=this.__ji=null;
}else{this._disposeArray(Q);
this._disposeObjects(z,cl);
}this._disposeArray(cp);
this.__jo=this.__jr=null;
this._disposeObjects(E,t,M,be);
}});
})();
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="feedreader.Application",b="os.name",a="engine.version";
...
m.start();
return m;
}},properties:{enabled:{init:true,check:c,apply:d},interval:{check:a,init:1000,apply:f}},members:{__eg:null,__ee:null,_applyInterval:function(n,o){if(this.getEnabled()){this.restart();
}},_applyEnabled:function(p,q){if(q){window.clearInterval(this.__eg);
this.__eg=null;
}else if(p){this.__eg=window.setInterval(this.__ee,this.getInterval());
...
qx.Class.define(b,{statics:{__eH:false,__eI:{},__eJ:0,MAX_RETRIES:10,scheduleFlush:function(i){var self=qx.ui.core.queue.Manager;
self.__eI[i]=true;
...
self.__eO.cancel();
var j=self.__eI;
self.__eL(function(){while(j.visibility||j.widget||j.appearance||j.layout||j.element){if(j.widget){delete j.widget;
...
self.__eL(function(){if(j.dispose){delete j.dispose;
qx.ui.core.queue.Dispose.flush();
}},function(){self.__eK=false;
});
self.__eJ=0;
},__eL:function(k,l){var self=qx.ui.core.queue.Manager;

try{k();
}catch(e){self.__eH=false;
self.__eK=false;
self.__eJ+=1;
...
}},__eM:function(e){var m=qx.ui.core.queue.Manager;
if(e.getData()==g){m.PAUSE=true;

if(m.__eN){window.clearTimeout(m.__eN);
}m.__eN=window.setTimeout(function(){m.PAUSE=false;
m.__eN=null;
m.flush();
},500);
}else{m.flush();
}}},defer:function(n){n.__eO=new qx.util.DeferredCall(n.flush);
qx.html.Element._scheduleFlush=n.scheduleFlush;
qx.event.Registration.addListener(window,h,qx.core.Environment.get(c)?n.__eM:n.flush);
}});
})();
...
},statics:{FONT_FORMATS:["eot","woff","ttf","svg"],VALIDATION_TIMEOUT:5000},members:{__gI:null,__gL:null,__gJ:null,__gK:null,__cX:null,__gM:null,require:function(U,V,W,X){var Y=[];
...
}if(!(qx.core.Environment.get(g)==h&&qx.bom.client.Browser.getVersion()<9)){this.__gN(U,Y,W,X);
return;
}
...
}this.__cX.push([U,Y,W,X]);
},remove:function(bc){var bd=null;

for(var i=0,l=this.__gI.length;i<l;i++){if(this.__gI[i]==bc){bd=i;
this.__gU(bc);
break;
}}
if(bd){qx.lang.Array.removeAt(this.__gI,bd);
}
if(bc in this.__gJ){this.__gJ[bc].dispose();
delete this.__gJ[bc];
...
if(this.__gL){var bj=this.__gL.ownerNode?this.__gL.ownerNode:this.__gL.owningElement;
qx.dom.Element.removeChild(bj,bj.parentNode);
}this.__gL=null;
},__gN:function(bk,bl,bm,bn){if(!qx.lang.Array.contains(this.__gI,bk)){var bq=this.__gQ(bl);
var bp=this.__gR(bk,bq);
...
try{this.__gT(bp);
}catch(br){}this.__gI.push(bk);
}
if(!this.__gJ[bk]){this.__gJ[bk]=new qx.bom.webfonts.Validator(bk);
this.__gJ[bk].setTimeout(qx.bom.webfonts.Manager.VALIDATION_TIMEOUT);
this.__gJ[bk].addListener(f,this.__gP,this);
}
if(bm){var bo=bn||window;
this.__gJ[bk].addListener(f,bm,bo);
}this.__gJ[bk].validate();
},__gO:function(){if(this.__cX.length==0){this.__gM.stop();
return;
}var bs=this.__cX.shift();
this.__gN.apply(this,bs);
},__gP:function(bt){var bu=bt.getData();

if(bu.valid===false){this.remove(bu.family);
}},__gQ:function(bv){var bx=qx.bom.webfonts.Manager.FONT_FORMATS;
var bA={};
...
},__gR:function(bB,bC){var bF=[];
var bD=this.__gK.length>0?this.__gK:qx.bom.webfonts.Manager.FONT_FORMATS;

for(var i=0,l=bD.length;i<l;i++){var bE=bD[i];

if(bC[bE]){bF.push(this.__gS(bE,bC[bE]));
}}var bG=u+bF.join(K)+H;
bG=o+bB+C+bG;
bG=bG+J;
return bG;
},__gS:function(bH,bI){switch(bH){case a:return k+bI+y;
case Q:return k+bI+B;
case S:return k+bI+z;
case e:return k+bI+F;
default:return null;
}},__gT:function(bJ){var bL=t+bJ+r;

if(qx.core.Environment.get(g)==h&&qx.core.Environment.get(R)<9){var bK=this.__gV(this.__gL.cssText);
bK+=bL;
this.__gL.cssText=bK;
}else{this.__gL.insertRule(bL,this.__gL.cssRules.length);
}},__gU:function(bM){var bP=new RegExp(I+bM,P);
...
if(bP.exec(bO)){this.__gL.deleteRule(j);
return;
}}}}},__gV:function(bQ){return bQ.replace(s,A);
}},destruct:function(){delete this.__gI;
this.removeStyleSheet();

for(var bR in this.__gJ){this.__gJ[bR].dispose();
...
(function(){var j="nonScaled",i="scaled",h="alphaScaled",g=".png",f="div",e="replacement",d="qx.event.type.Event",c="engine.name",b="hidden",a="Boolean",y="px",x="scale",w="changeSource",v="qx.ui.basic.Image",u="loaded",t="-disabled.$1",s="loadingFailed",r="String",q="_applySource",p="__kY",n="img",o="image",l="mshtml",m="_applyScale",k="no-repeat";
qx.Class.define(v,{extend:qx.ui.core.Widget,construct:function(z){this.__kY={};
qx.ui.core.Widget.call(this);

if(z){this.setSource(z);
}},properties:{source:{check:r,init:null,nullable:true,event:w,apply:q,themeable:true},scale:{check:a,init:false,themeable:true,apply:m},appearance:{refine:true,init:o},allowShrinkX:{refine:true,init:false},allowShrinkY:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false}},events:{loadingFailed:d,loaded:d},members:{__la:null,__lb:null,__ed:null,__kY:null,getContentElement:function(){return this.__lf();
...
},__lc:function(E){this.__ed=E;
},__ld:function(){if(this.__ed==null){var G=this.getSource();
var F=false;

if(G!=null){F=qx.lang.String.endsWith(G,g);
}
if(this.getScale()&&F&&qx.bom.element.Decoration.isAlphaImageLoaderEnabled()){this.__ed=h;
}else if(this.getScale()){this.__ed=i;
}else{this.__ed=j;
}}return this.__ed;
},__le:function(H){var I;
var J;
...
return K;
},__lf:function(){var L=this.__ld();

if(this.__kY[L]==null){this.__kY[L]=this.__le(L);
}return this.__kY[L];
...
}if(qx.util.ResourceManager.getInstance().has(M)){this.__li(this.getContentElement(),M);
}else if(qx.io.ImageLoader.isLoaded(M)){this.__lj(this.getContentElement(),M);
}else{this.__lk(this.getContentElement(),M);
}},__lg:qx.core.Environment.select(c,{"mshtml":function(O){var Q=qx.bom.element.Decoration.isAlphaImageLoaderEnabled();
var P=qx.lang.String.endsWith(O,g);

if(Q&&P){if(this.getScale()&&this.__ld()!=h){this.__lc(h);
}else if(!this.getScale()&&this.__ld()!=j){this.__lc(j);
}}else{if(this.getScale()&&this.__ld()!=i){this.__lc(i);
}else if(!this.getScale()&&this.__ld()!=j){this.__lc(j);
}}this.__lh(this.__lf());
},"default":function(R){if(this.getScale()&&this.__ld()!=i){this.__lc(i);
}else if(!this.getScale()&&this.__ld(j)){this.__lc(j);
}this.__lh(this.__lf());
}}),__lh:function(S){var V=this.getContainerElement();
var W=V.getChild(0);
...
V.addAt(S,0);
}},__li:function(ba,bb){var bd=qx.util.ResourceManager.getInstance();
...
this.__lm(bd.getImageWidth(bb),bd.getImageHeight(bb));
},__lj:function(be,bf){var bh=qx.io.ImageLoader;
be.setSource(bf);
var bg=bh.getWidth(bf);
var bi=bh.getHeight(bf);
this.__lm(bg,bi);
},__lk:function(bj,bk){var self;
var bl=qx.io.ImageLoader;
if(!bl.isFailed(bk)){bl.load(bk,this.__ll,this);
}else{if(bj!=null){bj.resetSource();
}}},__ll:function(bm,bn){if(this.$$disposed===true){return;
...
},__lm:function(bo,bp){if(bo!==this.__la||bp!==this.__lb){this.__la=bo;
this.__lb=bp;
qx.ui.core.queue.Layout.add(this);
...
}},properties:{spacingX:{check:h,init:0,apply:p},spacingY:{check:h,init:0,apply:p}},members:{__hm:null,__hk:null,__hl:null,__hn:null,__ho:null,__hp:null,__hq:null,__hr:null,__hs:null,verifyLayoutProperty:null,__ht:function(){var B=[];
var A=[];
var C=[];
var w=-1;
var v=-1;
...
},_setRowData:function(G,H,I){var J=this.__hk[G];

if(!J){this.__hk[G]={};
this.__hk[G][H]=I;
}else{J[H]=I;
}},_setColumnData:function(K,L,M){var N=this.__hl[K];
...
for(var i=0,l=this.__ho.length;i<l;i++){var cl=this.__ho[i];
var ch=this.__hu(cl);
...
for(var i=0,l=this.__hn.length;i<l;i++){var ct=this.__hn[i];
var cv=this.__hu(ct);
...
for(var cO=0;cO<=cH;cO++){var cG=this.__hm[cQ][cO];
...
if(this.__ho.length>0){this._fixHeightsRowSpan(cP);
}this.__hr=cP;
return cP;
...
for(var dc=0;dc<=cU;dc++){var cR=this.__hm[dc][db];
...
if(this.__hn.length>0){this._fixWidthsColSpan(cV);
}this.__hs=cV;
return cV;
...
},renderLayout:function(ds,dt){if(this._invalidChildrenCache){this.__ht();
...
for(var dC=0;dC<=du;dC++){var dJ=this.__hm[dC][dP];
if(!dJ){top+=dV[dC]+dB;
continue;
...
}},properties:{locale:{check:b,nullable:true,apply:h,event:g}},members:{__hy:f,__hz:null,__hA:null,__hv:null,__hw:null,__hx:null,getLanguage:function(){return this.__hA;
},getTerritory:function(){return this.getLocale().split(l)[1]||k;
},getAvailableLocales:function(){var C=[];

for(var B in this.__hw){if(B!=this.__hy){C.push(B);
}}return C;
},__hB:function(D){var F;
...
},_applyLocale:function(G,H){this.__hz=G;
this.__hA=this.__hB(G);
},addTranslation:function(I,J){var K=this.__hv;

if(K[I]){for(var L in J){K[I][L]=J[L];
}}else{K[I]=J;
}},addLocale:function(M,N){var O=this.__hw;

if(O[M]){for(var P in N){O[M][P]=N[P];
}}else{O[M]=N;
}},translate:function(Q,R,S){var T=this.__hv;
return this.__hC(T,Q,R,S);
},localize:function(U,V,W){var X=this.__hw;
return this.__hC(X,U,V,W);
},__hC:function(Y,ba,bb,bc){var bd;

if(!Y){return ba;
}
if(bc){var bf=this.__hB(bc);
}else{bc=this.__hz;
bf=this.__hA;
}if(!bd&&Y[bc]){bd=Y[bc][ba];
}if(!bd&&Y[bf]){bd=Y[bf][ba];
}if(!bd&&Y[this.__hy]){bd=Y[this.__hy][ba];
}
if(!bd){bd=ba;
}
...
qx.Class.define(m,{statics:{__ii:qx.core.Environment.select(q,{"mshtml":null,"webkit":[o,k,g],"gecko":[a],"opera":[o]}),__ij:qx.core.Environment.select(q,{"mshtml":null,"webkit":[n,e,b],"gecko":[h],"opera":[n]}),__ik:{tags:{button:true,select:true},types:{search:true,button:true,submit:true,reset:true,checkbox:true,radio:true}},__il:function(r){var s=this.__ik;
return s.tags[r.tagName.toLowerCase()]||s.types[r.type];
},compile:qx.core.Environment.select(q,{"mshtml":function(t){},"default":function(u){var w=this.__ij;
var v=p;

if(w){for(var i=0,l=w.length;i<l;i++){v+=w[i]+f+u+c;
}}return v;
}}),get:qx.core.Environment.select(q,{"mshtml":function(x){if(qx.bom.Document.isStandardMode(qx.dom.Node.getDocument(x))){if(!this.__il(x)){return d;
}}return j;
},"default":function(y){var A=this.__ii;
var z;
...
}}),set:qx.core.Environment.select(q,{"mshtml":function(B,C){},"default":function(D,E){var F=this.__ii;
...
return this.__js={left:this.getInsetLeft()==null?j.left:this.getInsetLeft(),right:this.getInsetRight()==null?j.right:this.getInsetRight(),bottom:this.getInsetBottom()==null?j.bottom:this.getInsetBottom(),top:this.getInsetTop()==null?j.top:this.getInsetTop()};
},_applyInsets:function(){this.__js=null;
...
}},properties:{alignY:{check:[m,j,e],init:m,apply:n},alignX:{check:[k,g,a],init:k,apply:n},spacing:{check:c,init:0,apply:n},separator:{check:h,nullable:true,apply:n},reversed:{check:b,init:false,apply:f}},members:{__jt:null,__ju:null,__jv:null,__iL:null,_applyReversed:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__jw:function(){var w=this._getLayoutChildren();
var length=w.length;
var s=false;
var r=this.__jt&&this.__jt.length!=length&&this.__ju&&this.__jt;
var u;
var t=r?this.__jt:new Array(length);
var v=r?this.__ju:new Array(length);
...
for(i=0;i<length;i+=1){J=this.__jt[i];
B=J!=null?Math.floor((y-C)*J):F[i].getSizeHint().height;
K.push(B);
Q+=B;
}if(this.__jv&&Q!=y){var H={};
var N,R;

for(i=0;i<length;i+=1){N=this.__ju[i];
...
qx.Class.define(w,{statics:{__jT:{},bind:function(G,H,I,J,K){var V=this.__jV(G,H,I,J,K);
var Q=H.split(d);
var M=this.__kc(Q);
var U=[];
var R=[];
...
var T=qx.lang.Function.bind(this.__jU,this,W);
R.push(T);
...
this.__ke(X,G,H,I,J);
return X;
},__jU:function(bc){if(bc.options&&bc.options.onUpdate){bc.options.onUpdate(bc.sources[bc.index],bc.targetObject);
...
if(!bg){this.__jX(bc.targetObject,bc.targetPropertyChain);
break;
...
this.__kb(be,bc.targetObject,bc.targetPropertyChain,bc.options,bc.sources[bc.index]);
bc.listenerIds[j]=this.__kd(bg,g,bc.targetObject,bc.targetPropertyChain,bc.options,bc.arrayIndexValues[j]);
...
this.__kb(be,bc.targetObject,bc.targetPropertyChain,bc.options,bc.sources[bc.index]);
}var bf=this.__jW(bg,bc.propertyNames[j]);
bc.listenerIds[j]=this.__kd(bg,bf,bc.targetObject,bc.targetPropertyChain,bc.options);
}}else{if(bc.listeners[j]==null){var bd=qx.lang.Function.bind(this.__jU,this,bc);
bc.listeners.push(bd);
}if(qx.Class.implementsInterface(bg,qx.data.IListData)){var bf=g;
}else{var bf=this.__jW(bg,bc.propertyNames[j]);
}bc.listenerIds[j]=bg.addListener(bf,bc.listeners[j]);
}}},__jV:function(bi,bj,bk,bl,bm){var bq=bl.split(d);
var bo=this.__kc(bq);
var bv=[];
var bu=[];
var bs=[];
...
}else{try{br.push(this.__jW(bp,bq[i]));
}catch(e){break;
}}bv[i]=bp;
...
}else{try{var bx=qx.data.SingleValueBinding.__jW(by,bq[j]);
}catch(e){break;
...
bF=qx.data.SingleValueBinding.__kf(bF,bC,bD,bE,bA);
this.__jY(bC,bD,bF);
},getValueFromObject:function(o,bG){var bK=this.__ka(o,bG);
var bI;
...
},__jW:function(bN,bO){var bP=this.__kg(bN,bO);
...
}}},__ka:function(cd,ce){var ch=ce.split(d);
var ci=cd;
...
},__kb:function(ck,cl,cm,cn,co){ck=this.__kf(ck,cl,cm,cn,co);
if(ck===undefined){this.__jX(cl,cm);
}if(ck!==undefined){try{this.__jY(cl,cm,ck);
...
},__kd:function(cs,ct,cu,cv,cw,cx){var cy;
var cA=function(cB,e){if(cB!==l){if(cB===h){cB=cs.length-1;
}var cE=cs.getItem(cB);
if(cE===undefined){qx.data.SingleValueBinding.__jX(cu,cv);
...
}cE=qx.data.SingleValueBinding.__kf(cE,cu,cv,cw,cs);
if(qx.core.Environment.get(a)){qx.log.Logger.debug("Data after conversion: "+cE);
}try{if(cE!==undefined){qx.data.SingleValueBinding.__jY(cu,cv,cE);
}else{qx.data.SingleValueBinding.__jX(cu,cv);
...
return cz;
},__ke:function(cF,cG,cH,cI,cJ){if(this.__jT[cG.toHashCode()]===undefined){this.__jT[cG.toHashCode()]=[];
}this.__jT[cG.toHashCode()].push([cF,cG,cH,cI,cJ]);
},__kf:function(cK,cL,cM,cN,cO){if(cN&&cN.converter){var cQ;

if(cL.getModel){cQ=cL.getModel();
}return cN.converter(cK,cQ,cO,cL);
}else{var cS=this.__ka(cL,cM);
...
return this.__kh(cK,cP);
}},__kg:function(cU,cV){var cW=qx.Class.getPropertyDefinition(cU.constructor,cV);

if(cW==null){return null;
}return cW.event;
},__kh:function(cX,cY){var da=qx.lang.Type.getClass(cX);
...
}var dd=this.__jT[db.toHashCode()];
if(dd!=undefined){for(var i=0;i<dd.length;i++){if(dd[i][0]==dc){qx.lang.Array.remove(dd,dd[i]);
return;
}}}throw new Error("Binding could not be found!");
},removeAllBindingsForObject:function(de){var df=this.__jT[de.toHashCode()];

if(df!=undefined){for(var i=df.length-1;i>=0;i--){this.removeBindingFromObject(de,df[i][0]);
}}},getAllBindingsForObject:function(dg){if(this.__jT[dg.toHashCode()]===undefined){this.__jT[dg.toHashCode()]=[];
}return this.__jT[dg.toHashCode()];
},removeAllBindings:function(){for(var di in this.__jT){var dh=qx.core.ObjectRegistry.fromHashCode(di);
if(dh==null){delete this.__jT[di];
continue;
}this.removeAllBindingsForObject(dh);
}this.__jT={};
},getAllBindings:function(){return this.__jT;
},showBindingInLog:function(dj,dk){var dm;
for(var i=0;i<this.__jT[dj.toHashCode()].length;i++){if(this.__jT[dj.toHashCode()][i][0]==dk){dm=this.__jT[dj.toHashCode()][i];
break;
}}
if(dm===undefined){var dl=q;
}else{var dl=v+dm[1]+B+dm[2]+z+dm[3]+B+dm[4]+r;
}qx.log.Logger.debug(dl);
},showAllBindingsInLog:function(){for(var dp in this.__jT){var dn=qx.core.ObjectRegistry.fromHashCode(dp);

for(var i=0;i<this.__jT[dp].length;i++){this.showBindingInLog(dn,this.__jT[dp][i][0]);
}}}}});
})();
...
}},properties:{rich:{check:j,init:false,event:p,apply:n},wrap:{check:j,init:true,apply:a},value:{check:v,apply:y,event:D,nullable:true},buddy:{check:d,apply:w,nullable:true,init:null,dereference:true},textAlign:{check:[m,x,r,s],nullable:true,themeable:true,apply:f,event:b},appearance:{refine:true,init:l},selectable:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false},allowShrinkY:{refine:true,init:false}},members:{__L:null,__kj:null,__kk:null,__kl:null,__M:null,_getContentHint:function(){if(this.__kj){this.__km=this.__kn();
delete this.__kj;
...
}},__km:{width:0,height:0},_applyFont:function(L,M){if(M&&this.__L&&this.__M){this.__L.removeListenerById(this.__M);
this.__M=null;
}var N;

if(L){this.__L=qx.theme.manager.Font.getInstance().resolve(L);
...
}N=this.__L.getStyles();
}else{this.__L=null;
N=qx.bom.Font.getDefaultStyles();
}this.getContentElement().setStyles(N);
this.__kj=true;
qx.ui.core.queue.Layout.add(this);
},__kn:function(O){var S=qx.bom.Label;
var Q=this.getFont();
var P=Q?this.__L.getStyles():qx.bom.Font.getDefaultStyles();
var content=this.getValue()||t;
var R=this.getRich();
return R?S.getHtmlSize(content,P,O):S.getTextSize(content,P);
},_applyBuddy:function(T,U){if(U!=null){U.removeBinding(this.__kk);
this.__kk=null;
this.removeListenerById(this.__kl);
this.__kl=null;
}
if(T!=null){this.__kk=T.bind(h,this,h);
this.__kl=this.addListener(o,function(){if(T.isFocusable()){T.focus.apply(T);
}},this);
...
}},"false":null}),_onWebFontStatusChange:function(ba){if(ba.getData().valid===true){this.__kj=true;
qx.ui.core.queue.Layout.add(this);
...
}if(this.__kk!=null){var bd=this.getBuddy();

if(bd!=null&&!bd.isDisposed()){bd.removeBinding(this.__kk);
}}
...
qx.Class.define(f,{extend:qx.bom.Font,events:{"changeStatus":a},properties:{sources:{nullable:true,apply:b}},members:{__kE:null,_applySources:function(h,j){var n=[];
...
qx.Class.define(x,{statics:{__kF:function(){var G=[F,h,q,b];
var K={};
...
this.__kG[n]=qx.core.Environment.select(k,{"gecko":f,"webkit":v,"default":h});
this.__kH={};

for(var I in K){this.__kH[I]=this.__kL(K[I]);
}this.__kG[c]=qx.core.Environment.select(k,{"mshtml":a,"default":B});
},__kI:{width:s,height:E,left:z,right:u,top:A,bottom:d},__kJ:{clip:qx.bom.element.Clip,cursor:qx.bom.element.Cursor,opacity:qx.bom.element.Opacity,boxSizing:qx.bom.element.BoxSizing,overflowX:{set:qx.lang.Function.bind(qx.bom.element.Overflow.setX,qx.bom.element.Overflow),get:qx.lang.Function.bind(qx.bom.element.Overflow.getX,qx.bom.element.Overflow),reset:qx.lang.Function.bind(qx.bom.element.Overflow.resetX,qx.bom.element.Overflow),compile:qx.lang.Function.bind(qx.bom.element.Overflow.compileX,qx.bom.element.Overflow)},overflowY:{set:qx.lang.Function.bind(qx.bom.element.Overflow.setY,qx.bom.element.Overflow),get:qx.lang.Function.bind(qx.bom.element.Overflow.getY,qx.bom.element.Overflow),reset:qx.lang.Function.bind(qx.bom.element.Overflow.resetY,qx.bom.element.Overflow),compile:qx.lang.Function.bind(qx.bom.element.Overflow.compileY,qx.bom.element.Overflow)}},compile:function(O){var Q=[];
var S=this.__kJ;
var R=this.__kH;
var name,P;
...
}else{Q.push(this.__kL(name),C,P,p);
}}return Q.join(m);
},__kK:{},__kL:function(T){var U=this.__kK;
var V=U[T];
...
}}),isPropertySupported:function(bd){return (this.__kJ[bd]||this.__kG[bd]||bd in document.documentElement.style);
},COMPUTED_MODE:1,CASCADED_MODE:2,LOCAL_MODE:3,set:function(be,name,bf,bg){name=this.__kG[name]||name;
if(bg!==false&&this.__kJ[name]){return this.__kJ[name].set(be,bf);
}else{be.style[name]=bf!==null?bf:m;
}},setStyles:function(bh,bi,bj){var bm=this.__kG;
var bo=this.__kJ;
var bk=bh.style;
...
}}}},reset:function(bp,name,bq){name=this.__kG[name]||name;
if(bq!==false&&this.__kJ[name]){return this.__kJ[name].reset(bp);
}else{bp.style[name]=m;
}},get:qx.core.Environment.select(k,{"mshtml":function(br,name,bs,bt){name=this.__kG[name]||name;
if(bt!==false&&this.__kJ[name]){return this.__kJ[name].get(br,bs);
...
}},"default":function(by,name,bz,bA){name=this.__kG[name]||name;
if(bA!==false&&this.__kJ[name]){return this.__kJ[name].get(by,bz);
...
qx.Class.define(r,{statics:{DEBUG:false,__lq:{},__lr:(qx.core.Environment.get(j)==c)&&qx.core.Environment.get(D)<9,__ls:qx.core.Environment.select(j,{"mshtml":{"scale-x":true,"scale-y":true,"scale":true,"no-repeat":true},"default":null}),__lt:{"scale-x":h,"scale-y":h,"scale":h,"repeat":i,"no-repeat":i,"repeat-x":i,"repeat-y":i},update:function(J,K,L,M){var O=this.getTagName(L,K);
...
if(this.__lr){try{J.filters[v].apply();
...
}},getTagName:function(W,X){if((qx.core.Environment.get(j)==c)){if(X&&this.__lr&&this.__ls[W]&&qx.lang.String.endsWith(X,F)){return i;
}}return this.__lt[W];
...
var bc;
if(this.__lr&&this.__ls[ba]&&bd===q){bc=this.__lw(bb,ba,Y);
}else{if(ba===b){bc=this.__lx(bb,ba,Y);
}else if(ba===d||ba===I){bc=this.__ly(bb,ba,Y);
}else{bc=this.__lB(bb,ba,Y);
}}return bc;
},__lu:function(be,bf,bh){if(be.width==null&&bf!=null){be.width=bf+k;
}
if(be.height==null&&bh!=null){be.height=bh+k;
}return be;
},__lv:function(bi){var bj=qx.util.ResourceManager.getInstance().getImageWidth(bi)||qx.io.ImageLoader.getWidth(bi);
var bk=qx.util.ResourceManager.getInstance().getImageHeight(bi)||qx.io.ImageLoader.getHeight(bi);
return {width:bj,height:bk};
},__lw:function(bl,bm,bn){var bq=this.__lv(bn);
bl=this.__lu(bl,bq.width,bq.height);
var bp=bm==f?E:b;
var bo=C+qx.util.ResourceManager.getInstance().toUri(bn)+s+bp+n;
bl.filter=bo;
bl.backgroundImage=bl.backgroundRepeat=g;
return {style:bl};
},__lx:function(br,bs,bt){var bu=qx.util.ResourceManager.getInstance().toUri(bt);
var bv=this.__lv(bt);
br=this.__lu(br,bv.width,bv.height);
return {src:bu,style:br};
},__ly:function(bw,bx,by){var bz=qx.util.ResourceManager.getInstance();
...
if(bx===d){bw=this.__lz(bw,bD,bE.height);
}else{bw=this.__lA(bw,bD,bE.width);
...
return {src:bA,style:bw};
}},__lz:function(bF,bG,bH){var bI=qx.util.ResourceManager.getInstance().getImageHeight(bG[4]);
...
},__lA:function(bJ,bK,bL){var bM=qx.util.ResourceManager.getInstance().getImageWidth(bK[4]);
...
},__lB:function(bN,bO,bP){var bS=qx.util.ResourceManager.getInstance();
...
}else{bN=this.__lu(bN,ca.width,ca.height);
bN=this.__lC(bN,bP,bO);
return {style:bN};
}},__lC:function(cb,cc,cd){var top=null;
var ch=null;
...
},__lD:function(ci){if(this.DEBUG&&qx.util.ResourceManager.getInstance().has(ci)&&ci.indexOf(H)==-1){if(!this.__lq[ci]){qx.log.Logger.debug("Potential clipped image candidate: "+ci);
this.__lq[ci]=true;
...
if(q){this.setSeparator(q);
}},properties:{alignX:{check:[m,k,c],init:m,apply:n},alignY:{check:[j,g,e],init:j,apply:n},spacing:{check:a,init:0,apply:n},separator:{check:h,nullable:true,apply:n},reversed:{check:d,init:false,apply:f}},members:{__lH:null,__ju:null,__jv:null,__iL:null,_applyReversed:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__jw:function(){var w=this._getLayoutChildren();
var length=w.length;
var t=false;
var r=this.__lH&&this.__lH.length!=length&&this.__ju&&this.__lH;
var u;
...
P.push(K);
F+=K;
}if(this.__jv&&F!=x){var H={};
var L,O;

for(i=0;i<length;i+=1){L=this.__ju[i];
...
qx.Bootstrap.define(a,{construct:function(name,f,g){this.__lI=name;
this._readyState=e;
this._packages=f;
this._loader=g;
...
qx.Mixin.define(f,{events:{"changeSelection":a},members:{__x:null,getSelection:function(){var g=this.__lJ().getSelected();

if(g){return [g];
}else{return [];
}},setSelection:function(h){switch(h.length){case 0:this.resetSelection();
break;
case 1:this.__lJ().setSelected(h[0]);
break;
default:throw new Error("Could only select one item, but the selection"+" array contains "+h.length+" items!");
}},resetSelection:function(){this.__lJ().resetSelected();
},isSelected:function(i){return this.__lJ().isSelected(i);
},isSelectionEmpty:function(){return this.__lJ().isSelectionEmpty();
},getSelectables:function(j){return this.__lJ().getSelectables(j);
},_onChangeSelected:function(e){var l=e.getData();
var k=e.getOldData();
l==null?l=[]:l=[l];
k==null?k=[]:k=[k];
this.fireDataEvent(d,l,k);
},__lJ:function(){if(this.__x==null){var m=this;
...
qx.Bootstrap.define(b,{construct:function(i,j,k){this.__lK=k?e:g;
this.__lL=i;
this.__dc=j;
},members:{__lK:null,__lL:null,__dc:null,__lM:null,__lN:null,__bX:null,__lO:null,getId:function(){return this.__dc;
},getReadyState:function(){return this.__lK;
},getUrls:function(){return this.__lL;
},saveClosure:function(l){if(this.__lK==h){return;
}this.__lM=l;

if(!this.__lN){this.execute();
}else{clearTimeout(this.__bX);
this.__lK=a;
this.__lO(this);
}},execute:function(){if(this.__lM){this.__lM();
delete this.__lM;
}
if(qx.$$packageData[this.__dc]){this.__lQ(qx.$$packageData[this.__dc]);
delete qx.$$packageData[this.__dc];
}this.__lK=e;
},loadClosure:function(m,self){if(this.__lK!==g){return;
}this.__lN=true;
this.__lK=f;
this.__lO=qx.Bootstrap.bind(m,self);
this.__lP(this.__lL,function(){},function(){this.__lK=h;
m.call(self,this);
},this);
var n=this;
this.__bX=setTimeout(function(){n.__lK=h;
m.call(self,n);
},qx.Part.TIMEOUT);
},load:function(o,self){if(this.__lK!==g){return;
}this.__lN=false;
this.__lK=f;
this.__lP(this.__lL,function(){this.__lK=e;
this.execute();
o.call(self,this);
},function(){this.__lK=h;
o.call(self,this);
},this);
},__lP:function(p,q,r,self){if(p.length==0){q.call(self);
return;
}var t=0;
var self=this;
...
if(status!==c){if(self.__lK==f){clearTimeout(self.__bX);
return r.call(self);
...
s(p.concat());
},__lQ:qx.$$loader.importPackageData}});
})();
...
(function(){var f="change",d="qx.event.type.Data",c="qx.ui.form.MModelSelection",b="__lS",a="changeSelection";
qx.Mixin.define(c,{construct:function(){this.__lS=new qx.data.Array();
this.__lS.addListener(f,this.__lV,this);
this.addListener(a,this.__lU,this);
},events:{changeModelSelection:d},members:{__lS:null,__lT:false,__lU:function(){if(this.__lT){return;
...
}}},__lV:function(){this.__lT=true;
var n=this.getSelectables(true);
var p=[];
var o=this.__lS.toArray();
...
if(!qx.lang.Array.equals(q,p)){this.__lU();
}},getModelSelection:function(){return this.__lS;
},setModelSelection:function(t){if(!t){this.__lS.removeAll();
return;
}t.unshift(this.__lS.getLength());
t.unshift(0);
var u=this.__lS.splice.apply(this.__lS,t);
u.dispose();
}},destruct:function(){this._disposeObjects(b);
}});
})();
...
},members:{__lW:0,preload:function(h,self){var j=0;
var l=this;
...
}this.__lW=this._packages.length;
...
}}if(this.__lW<=0){this.__lX();
}},__lX:function(){for(var i=0;i<this._packages.length;i++){this._packages[i].execute();
...
(function(){var q="success",p="fail",o="mshtml",n="complete",m="error",l="load",k="opera",j="loaded",i="engine.name",h="head",c="readystatechange",g="webkit",f="script",b="qx.io.ScriptLoader",a="text/javascript",d="abort";
qx.Bootstrap.define(b,{construct:function(){this.__mh=qx.Bootstrap.bind(this.__ml,this);
this.__mi=document.createElement(f);
},statics:{TIMEOUT:15},members:{__mj:null,__mk:null,__ey:null,__ez:null,__mh:null,__mi:null,load:function(r,s,t){if(this.__mj){throw new Error("Another request is still running!");
}this.__mj=true;
this.__mk=false;
var u=document.getElementsByTagName(h)[0];
var v=this.__mi;
this.__ey=s||null;
this.__ez=t||window;
v.type=a;
v.onerror=v.onload=v.onreadystatechange=this.__mh;
var self=this;

if(qx.core.Environment.get(i)===k&&this._getTimeout()>0){setTimeout(function(){self.dispose(p);
},this._getTimeout()*1000);
}v.src=r;
setTimeout(function(){u.appendChild(v);
},0);
},abort:function(){if(this.__mj){this.dispose(d);
}},dispose:function(status){if(this.__mk){return;
}this.__mk=true;
var y=this.__mi;
y.onerror=y.onload=y.onreadystatechange=null;
var x=y.parentNode;

if(x){x.removeChild(y);
}delete this.__mj;
if(this.__ey){var w=qx.bom.client.Engine.getName();

if(w==o||w==g){var self=this;
setTimeout(qx.event.GlobalError.observeMethod(function(){self.__ey.call(self.__ez,status);
delete self.__ey;
}),0);
}else{this.__ey.call(this.__ez,status);
delete this.__ey;
}}},setTimeout:function(z){},getTimeout:function(){return this._getTimeout();
},_getTimeout:function(){return qx.io.ScriptLoader.TIMEOUT;
},__ml:qx.event.GlobalError.observeMethod(function(e){var A=qx.bom.client.Engine.getName();
if(A==o){var B=this.__mi.readyState;

if(B==j){this.dispose(q);
}else if(B==n){this.dispose(q);
}else{return;
}}else if(A==k){if(qx.Bootstrap.isString(e)||e.type===m){return this.dispose(p);
}else if(e.type===l){return this.dispose(q);
}else{return;
}}else{if(qx.Bootstrap.isString(e)||e.type===m){this.dispose(p);
}else if(e.type===l){this.dispose(q);
}else if(e.type===c&&(e.target.readyState===n||e.target.readyState===j)){this.dispose(q);
}else{return;
}}})}});
})();
...
for(var j in d.packages){var h=new qx.io.part.Package(this.__mg(d.packages[j].uris),j,j==e);
this.__md[j]=h;
}this.__me={};
var g=d.parts;
...
for(var i=0;i<m.length;i++){l.push(this.__md[m[i]]);
...
}},members:{__lY:null,__md:null,__me:null,__mc:null,addToPackage:function(s){this.__md[s.getId()]=s;
},addClosurePackageListener:function(t,u){var v=t.getId();

if(!this.__mc[v]){this.__mc[v]=[];
}this.__mc[v].push(u);
},saveClosure:function(w,x){var z=this.__md[w];
...
},getParts:function(){return this.__me;
...
for(var i=0;i<A.length;i++){E.push(this.__me[A[i]]);
}var D=0;
var C=function(){D+=1;
if(D>=E.length){var F=[];
...
for(var i=0;i<G.length;i++){this.__me[G[i]].preload(function(){I++;

if(I>=G.length){var J=[];

for(var i=0;i<G.length;i++){J.push(this.__me[G[i]].getReadyState());
}
if(H){H.call(self,J);
}}},this);
}},__mf:function(){var K=this.__lY.packages;
var L=[];

for(var M in K){L.push(this.__mg(K[M].uris));
}return L;
},__mg:qx.$$loader.decodeUris,__ma:null,addPartListener:function(N,O){var P=N.getName();

if(!this.__ma[P]){this.__ma[P]=[];
}this.__ma[P].push(O);
...
}},__mb:null,addPackageListener:function(T,U){var V=T.getId();

if(!this.__mb[V]){this.__mb[V]=[];
}this.__mb[V].push(U);
...
if(o==undefined){this.__mm=[];
}else if(arguments.length>1){this.__mm=[];

for(var i=0;i<arguments.length;i++){this.__mm.push(arguments[i]);
}}else if(typeof o==c){this.__mm=new Array(o);
}else if(o instanceof Array){this.__mm=qx.lang.Array.clone(o);
}else{this.__mm=[];
throw new Error("Type of the parameter not supported!");
}for(var i=0;i<this.__mm.length;i++){this._applyEventPropagation(this.__mm[i],null,i);
}this.__mn();
},events:{"change":e,"changeLength":e},members:{__mm:null,concat:function(p){if(p){var q=this.__mm.concat(p);
}else{var q=this.__mm.concat();
}return new qx.data.Array(q);
},join:function(r){return this.__mm.join(r);
},pop:function(){var s=this.__mm.pop();
this.__mn();
...
return s;
},push:function(t){for(var i=0;i<arguments.length;i++){this.__mm.push(arguments[i]);
this.__mn();
...
}var u=this.__mm.concat();
this.__mm.reverse();
this.fireDataEvent(n,{start:0,end:this.length-1,type:h,items:null},null);
this.fireDataEvent(m,{value:this.__mm,name:j+(this.__mm.length-1),old:u});
},shift:function(){if(this.length==0){return;
}var v=this.__mm.shift();
this.__mn();
...
},splice:function(y,z,A){var I=this.__mm.length;
var E=this.__mm.splice.apply(this.__mm,arguments);
if(this.__mm.length!=I){this.__mn();
}var G=z>0;
var C=arguments.length>2;
var D=null;

if(G||C){if(this.__mm.length>I){var H=l;
}else if(this.__mm.length<I){var H=k;
D=E;
}else{var H=h;
...
}var K=this.__mm.concat();
this.__mm.sort.apply(this.__mm,arguments);
this.fireDataEvent(n,{start:0,end:this.length-1,type:h,items:null},null);
this.fireDataEvent(m,{value:this.__mm,name:j+(this.length-1),old:K});
},unshift:function(L){for(var i=arguments.length-1;i>=0;i--){this.__mm.unshift(arguments[i]);
this.__mn();
this._registerEventChaining(arguments[i],null,0);
this.fireDataEvent(m,{value:[this.__mm[0]],name:f,old:[this.__mm[1]]});
this.fireDataEvent(n,{start:0,end:this.length-1,type:l,items:[arguments[i]]},null);
}return this.length;
},toArray:function(){return this.__mm;
},getItem:function(M){return this.__mm[M];
},setItem:function(N,O){var P=this.__mm[N];
if(P===O){return;
}this.__mm[N]=O;
...
},indexOf:function(Q){return this.__mm.indexOf(Q);
},toString:function(){if(this.__mm!=null){return this.__mm.toString();
}return d;
},contains:function(R){return this.__mm.indexOf(R)!==-1;
...
}Array.prototype.push.apply(this.__mm,be);
for(var i=0;i<be.length;i++){this._registerEventChaining(be[i],null,this.__mm.length+i);
}var bf=this.length;
this.__mn();
...
},forEach:function(bm,bn){for(var i=0;i<this.__mm.length;i++){bm.call(bn,this.__mm[i],i,this);
}},__mn:function(){var bo=this.length;
...
(function(){var g="__mq",f="Boolean",e="qx.ui.core.SingleSelectionManager",d="__mp",c="__mo",b="changeSelected",a="qx.event.type.Data";
qx.Class.define(e,{extend:qx.core.Object,construct:function(h){qx.core.Object.call(this);
this.__mo=h;
},events:{"changeSelected":a},properties:{allowEmptySelection:{check:f,init:true,apply:g}},members:{__mp:null,__mo:null,getSelected:function(){return this.__mp;
},setSelected:function(j){if(!this.__ms(j)){throw new Error("Could not select "+j+", because it is not a child element!");
}this.__mr(j);
},resetSelected:function(){this.__mr(null);
},isSelected:function(k){if(!this.__ms(k)){throw new Error("Could not check if "+k+" is selected,"+" because it is not a child element!");
}return this.__mp===k;
},isSelectionEmpty:function(){return this.__mp==null;
},getSelectables:function(l){var m=this.__mo.getItems();
var n=[];

for(var i=0;i<m.length;i++){if(this.__mo.isItemSelectable(m[i])){n.push(m[i]);
}}if(!l){for(var i=n.length-1;i>=0;i--){if(!n[i].getEnabled()){n.splice(i,1);
}}}return n;
},__mq:function(o,p){if(!o){this.__mr(this.__mp);
}},__mr:function(q){var t=this.__mp;
var s=q;
...
},__ms:function(u){var v=this.__mo.getItems();

for(var i=0;i<v.length;i++){if(v[i]===u){return true;
}}return false;
}},destruct:function(){if(this.__mo.toHashCode){this._disposeObjects(c);
}else{this.__mo=null;
...
qx.Bootstrap.define(a,{construct:function(){this.__mP();
this.__mH=qx.Bootstrap.bind(this._onNativeLoad,this);
this.__mI=qx.Bootstrap.bind(this._onNativeError,this);
this.__mJ=qx.Bootstrap.bind(this._onTimeout,this);
this.__mK=document.head||document.getElementsByTagName(f)[0]||document.documentElement;
this.timeout=this.__mT()?0:15000;
},members:{readyState:null,status:null,statusText:null,timeout:null,__mL:null,open:function(l,m){if(this.__mk){return;
}this.__mP();
this.__mM=null;
this.__mN=m;
...
},setRequestHeader:function(n,o){if(this.__mk){return;
}var p={};

if(this.readyState!==1){throw new Error("Invalid state");
}p[n]=o;
this.__mN=qx.util.Uri.appendParamsToUrl(this.__mN,p);
},send:function(){if(this.__mk){return;
}var s=this.__mU(),q=this.__mK,r=this;

if(this.timeout>0){this.__bX=window.setTimeout(this.__mJ,this.timeout);
}
if(qx.core.Environment.get(k)){qx.Bootstrap.debug(qx.bom.request.Script,"Send native request");
}q.insertBefore(s,q.firstChild);
window.setTimeout(function(){r.__mQ(2);
r.__mQ(3);
});
...
this.__mV();
this.onabort();
},onreadystatechange:function(){},onload:function(){},onloadend:function(){},onerror:function(){},onabort:function(){},ontimeout:function(){},getResponseHeader:function(t){if(this.__mk){return;
}return b;
},getAllResponseHeaders:function(){if(this.__mk){return;
}return c;
},setDetermineSuccess:function(u){qx.core.Assert.assertFunction(u);
this.__mL=u;
},dispose:function(){var v=this.__mO;

if(!this.__mk){if(v){v.onload=v.onreadystatechange=null;
this.__mV();
}
if(this.__bX){window.clearTimeout(this.__bX);
}this.__mk=true;
}},_getUrl:function(){return this.__mN;
},_getScriptElement:function(){return this.__mO;
},_onTimeout:function(){this.__mS();

if(!this.__mT()){this.onerror();
}this.ontimeout();

if(!this.__mT()){this.onloadend();
}},_onNativeLoad:function(){var y=this.__mO,w=this.__mL,x=this;
if(this.__mM){return;
...
if(this.__bX){window.clearTimeout(this.__bX);
}window.setTimeout(function(){x.__mR();
x.__mQ(4);
x.onload();
x.onloadend();
});
},_onNativeError:function(){this.__mS();
this.onerror();
this.onloadend();
},__mO:null,__mK:null,__mN:h,__mH:null,__mI:null,__mJ:null,__bX:null,__mM:null,__mk:null,__mP:function(){this.readyState=0;
this.status=0;
this.statusText=h;
},__mQ:function(z){this.readyState=z;
this.onreadystatechange();
},__mR:function(){this.__mV();
this.readyState=4;
if(!this.status){this.status=200;
}this.statusText=h+this.status;
},__mS:function(){this.__mV();
this.readyState=4;
this.status=0;
this.statusText=null;
},__mT:function(){var B=qx.core.Environment.get(j)===i&&qx.core.Environment.get(g)<9;
var A=qx.core.Environment.get(j)===d;
return !(B||A);
},__mU:function(){var C=this.__mO=document.createElement(e);
C.src=this.__mN;
C.onerror=this.__mI;
C.onload=this.__mH;
if(qx.core.Environment.get(j)===i&&qx.core.Environment.get(g)<9){C.onreadystatechange=this.__mH;
}return C;
},__mV:function(){var D=this.__mO;

if(D&&D.parentNode){this.__mK.removeChild(D);
...
if(n){this.__mX(n);
}}else{this.__mW=new qx.ui.decoration.GridDiv(n);
}
if(o!=null){this.__mW.setInsets(o);
...
},resize:function(p,q,r){this.__mW.resize(p,q,r);
},tint:function(s,t){},getInsets:function(){return this.__mW.getInsets();
},_applyInsets:function(u,v,name){var w=f+qx.lang.String.firstUp(name);
this.__mW[w](u);
},_applyBaseImage:function(x,y){if(this.__mW instanceof qx.ui.decoration.GridDiv){this.__mW.setBaseImage(x);
}else{this.__mX(x);
}},__mX:function(z){var B,D;
this.__mW.setBorderImage(z);
...
s.send();
},__nf:function(u){if(this._getRequest()){this._getRequest().dispose();
...
},__ng:function(A){if(A==undefined){this.setState(b);
this.fireEvent(n);
return;
...
}q.include=this.__op(q.include);
q.patch=this.__op(q.patch);
...
}},__op:function(s){if(!s){return [];
}
if(qx.Bootstrap.isArray(s)){return s;
}else{return [s];
}},__oq:function(t,u){var v=u.aliases||{};
...
},__or:function(w){for(var i=0,x=this.__ot,l=x.length;i<l;i++){if(w[x[i]]){return x[i];
}}},__os:function(y,z){var C=this.__or(z);
...
y[C]=new E;
},$$registry:{},__ot:[g,d,f,n,c,o,m,b],__gb:null,__ou:null,__gd:function(){},patch:function(F,G){var I=this.__or(G);

if(I!==this.__or(F)){throw new Error("The mixins '"+F.name+"' are not compatible '"+G.name+"'!");
}var H=G[I];
...
}},__qV:function(G){var J=this._generateBackgroundMarkup(G);
...
G[y]=0;
G[p]=0;
return this.__qT=this._generateBackgroundMarkup(G,J);
},__qW:function(K,L,M){var N=this.getInsets();
L-=N.left+N.right;
M-=N.top+N.bottom;
...
(function(){var f="mousedown",d="__nn",c="blur",b="singleton",a="qx.ui.popup.Manager";
qx.Class.define(a,{type:b,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__nn=[];
qx.event.Registration.addListener(document.documentElement,f,this.__np,this,true);
qx.bom.Element.addListener(window,c,this.hideAll,this);
},members:{__nn:null,add:function(g){this.__nn.push(g);
this.__no();
},remove:function(h){if(this.__nn){qx.lang.Array.remove(this.__nn,h);
this.__no();
...
}}},__no:function(){var m=1e7;

for(var i=0;i<this.__nn.length;i++){this.__nn[i].setZIndex(m++);
}},__np:function(e){var o=qx.ui.core.Widget.getWidgetByElement(e.getTarget());
var p=this.__nn;
...
}}},destruct:function(){qx.event.Registration.removeListener(document.documentElement,f,this.__np,this,true);
...
}},properties:{selection:{check:d,event:f,apply:b,init:null}},members:{_modifingSelection:0,__re:null,__rf:null,_applySelection:function(g,h){if(this.__rf!=undefined&&h!=undefined){h.removeListenerById(this.__rf);
}this.__rf=g.addListener(e,this.__rg,this);
this._updateSelection();
},__rg:function(){this._updateSelection();
},_changeTargetSelection:function(){if(this.getTarget()==null){return;
}if(!this.__rh()&&!this.__ri()){return;
...
},_addChangeTargetListener:function(m,n){if(this.__re!=undefined&&n!=undefined){n.removeListenerById(this.__re);
}
if(m!=null){if(this.__rh()||this.__ri()){this.__re=m.addListener(f,this._changeTargetSelection,this);
...
}}}else if(this.__ri()){var p=this.getSelection().getItem(this.getSelection().length-1);

if(p){this.__rj(p);
this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();
}else{this.getTarget().resetSelection();
}}this._endSelectionModification();
},__rh:function(){var t=this.getTarget().constructor;
return qx.Class.implementsInterface(t,qx.ui.core.IMultiSelection);
},__ri:function(){var u=this.getTarget().constructor;
return qx.Class.implementsInterface(u,qx.ui.core.ISingleSelection);
},__rj:function(v){var w=this.__rk(v);
if(w==null){return;
}if(this.__rh()){this.getTarget().addToSelection(w);
}else if(this.__ri()){this.getTarget().setSelection([w]);
}},__rk:function(x){var y=this.getTarget().getSelectables(true);
...
},_applyIconOptions:function(G,H){this.__nO();
},_applyLabelOptions:function(I,J){this.__nO();
},_applyIconPath:function(K,L){this.__nO();
},_applyLabelPath:function(M,N){this.__nO();
},_applyModel:function(O,P){if(P!=undefined){if(this.__nF!=undefined){P.removeListenerById(this.__nF);
}
if(this.__nG!=undefined){P.removeListenerById(this.__nG);
}}if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length);
}if(O!=null){this.__nF=O.addListener(y,this.__nK,this);
this.__nG=O.addListener(k,this.__nJ,this);
this.__nQ();
this.__nK();
...
for(var i=0;i<length;i++){this.__nN();
}}}},_applyTarget:function(R,S){this._addChangeTargetListener(R,S);
if(S!=undefined){S.removeAll();
this.removeAllBindings();
}
if(R!=null){if(this.getModel()!=null){for(var i=0;i<this.__nB.length;i++){this.__nM(this.__nR(i));
}}}},__nJ:function(){this.__nI=true;
qx.ui.core.queue.Widget.add(this);
if(this.__nB.length!=this.getModel().getLength()){this.update();
}},syncWidget:function(){if(this.__nH){this._changeTargetSelection();
}
if(this.__nI){this._updateSelection();
}this.__nI=this.__nH=null;
},__nK:function(){if(this.getTarget()==null){return;
}this.__nQ();
var U=this.__nB.length;
var T=this.getTarget().getChildren().length;
if(U>T){for(var j=T;j<U;j++){this.__nM(this.__nR(j));
}}else if(U<T){for(var j=T;j>U;j--){this.__nN();
}}},__nL:function(){var V=this.getModel();
if(V!=null){V.removeListenerById(this.__nG);
this.__nG=V.addListener(k,this.__nJ,this);
...
},__nM:function(Y){var ba=this._createItem();
this._bindListItem(ba,Y);
this.getTarget().add(ba);
},__nN:function(){this._startSelectionModification();
...
if(!qx.lang.Array.contains(this.__nC,bn)){this.__nC.push(bn);
...
if(!qx.lang.Array.contains(this.__nD,bt)){this.__nD.push(bt);
}},_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;
}for(var i=0;i<this.__nC.length;i++){if(this.__nE[this.__nC[i]]!=null){this.__nE[this.__nC[i]]();
}}},_removeBindingsFrom:function(bD){for(var i=0;i<this.__nC.length;i++){var bE=bD.getUserData(this.__nC[i]+g);

if(bE!=null){this.removeBinding(bE);
}}for(var i=0;i<this.__nD.length;i++){var bE=bD.getUserData(this.__nD[i]+h);

if(bE!=null){bD.removeBinding(bE);
}}},__nO:function(){if(this.getTarget()==null||this.getModel()==null){return;
}var bF=this.getTarget().getChildren();
for(var i=0;i<bF.length;i++){this._removeBindingsFrom(bF[i]);
this._bindListItem(bF[i],this.__nR(i));
}this.__nL();
...
this._endSelectionModification();
this._updateSelection();
},_setFilter:function(bO,bP){if((bO==null||bO.filter==null)&&(bP!=null&&bP.filter!=null)){this.__nP();
...
this.__nQ();
if(bR.length>this.__nB.length){for(var j=bR.length;j>this.__nB.length;j--){this.getTarget().removeAt(j-1);
}}else if(bR.length<this.__nB.length){for(var j=bR.length;j<this.__nB.length;j++){var bS=this._createItem();
this.getTarget().add(bS);
}}var bQ=this.getTarget().getChildren();

for(var i=0;i<bQ.length;i++){this._bindListItem(bQ[i],this.__nR(i));
}this.__nL();
this._endSelectionModification();
this._updateSelection();
},__nP:function(){this.__nQ();
this.__nK();
this.__nO();
this.__nI=true;
qx.ui.core.queue.Widget.add(this);
},__nQ:function(){var bV=this.getModel();
...
for(var i=0;i<bV.getLength();i++){if(bU==null||bU(bV.getItem(i))){this.__nB.push(i);
}}},__nR:function(bX){return this.__nB[bX];
}},destruct:function(){this.__nB=this.__nE=this.__nC=null;
this.__nD=null;
qx.ui.core.queue.Widget.remove(this);
}});
})();
...
this.__nY();
},members:{responseJson:null,__dc:null,__nS:null,__nT:null,__nU:null,__nV:null,__mk:null,open:function(j,k){if(this.__mk){return;
}var l={},n,m,o=this;
this.responseJson=null;
this.__nU=false;
n=this.__nS||g;
m=this.__nT||a+this.__dc+b;
if(!this.__nT){this.constructor[this.__dc]=this;
}else{if(!window[this.__nT]){this.__nV=true;
window[this.__nT]=function(p){o.callback(p);
};
}else{if(qx.core.Environment.get(i)){qx.log.Logger.debug(qx.bom.request.Jsonp,"Callback "+this.__nT+" already exists");
}}}
...
this.__nX(f,[j,k]);
},callback:function(q){if(this.__mk){return;
}this.__nU=true;
this.responseJson=q;
this.constructor[this.__dc]=undefined;
this.__nW();
},setCallbackParam:function(r){this.__nS=r;
},setCallbackName:function(name){this.__nT=name;
},dispose:function(){this.__nW();
this.__nX(e);
},_onNativeLoad:function(){this.status=this.__nU?200:500;
this.__nX(c);
},__nW:function(){if(this.__nV&&window[this.__nT]){window[this.__nT]=undefined;
this.__nV=false;
}},__nX:function(s,t){qx.bom.request.Script.prototype[s].apply(this,t||[]);
},__nY:function(){this.__dc=(new Date().valueOf())+(d+Math.random()).substring(2,5);
}}});
})();
...
(function(){var m="separator-vertical",l="Posts",k="bold",j="__fA",i="__oC",h="_applyLoading",g="center",f="feedreader.view.List",e="__oD",d="single",a="middle",c="feedreader/images/loading66.gif",b="Boolean";
qx.Class.define(f,{extend:qx.ui.core.Widget,construct:function(){qx.ui.core.Widget.call(this);
var o=new qx.ui.layout.VBox();
o.setSeparator(m);
this._setLayout(o);
var p=new qx.ui.basic.Label(this.tr(l));
p.setPadding(5);
p.setAllowGrowX(true);
p.setFont(k);
this._add(p);
this.__fA=new qx.ui.container.Stack();
this._add(this.__fA,{flex:1});
this.__oC=new qx.ui.form.List();
this.__oC.setDecorator(null);
this.__oC.setSelectionMode(d);
this.__oC.setPadding(0);
this.__oC.setMargin(0);
this.__fA.add(this.__oC);
this.__oD=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,g));
var n=new qx.ui.basic.Image(c);
n.setAlignY(a);
this.__oD.add(n);
this.__fA.add(this.__oD);
},properties:{loading:{check:b,init:false,apply:h}},members:{__fA:null,__oC:null,__oD:null,_applyLoading:function(q,r){if(q){this.__fA.setSelection([this.__oD]);
}else{this.__fA.setSelection([this.__oC]);
}},getList:function(){return this.__oC;
}},destruct:function(){this._disposeObjects(i,j,e);
}});
})();
...
(function(){var m="_blank",l="Verdana",k="sans-serif",j="feedreader.view.Article",h="_applyArticle",g="feedreader/css/reader.css",f="blogEntry",e="a",d="Candara",c="white",a="auto",b="Object";
qx.Class.define(j,{extend:qx.ui.embed.Html,construct:function(){qx.ui.embed.Html.call(this);
if(!feedreader.view.Article.__oE){feedreader.view.Article.__oE=true;
qx.bom.Stylesheet.includeFile(g);
}this.setCssClass(f);
this.setOverflowY(a);
this.setBackgroundColor(c);
var n=new qx.bom.Font(14,[d,l,k]);
n.setLineHeight(1.8);
this.setFont(n);
},properties:{article:{apply:h,nullable:true,check:b}},members:{_applyArticle:function(o,p){var q=feedreader.ArticleBuilder.createHtml(this.getArticle(),true);
this.setHtml(q);
qx.html.Element.flush();
var s=this.getContentElement().getDomElement();
var r=s.getElementsByTagName(e);
for(var i=0;i<r.length;i++){r[i].target=m;
}}}});
})();
...
t.addListener(l,this.__oH(r),this);
t.addListener(c,qx.lang.Function.bind(this.__oG,this,r),this);
},__oG:function(v,e){if(e.getData()==a||e.getData()==g||e.getData()==n){v.setState(m);
}},__oH:function(w){return function(e){var x=e.getData();
if(x==n){w.setState(m);
return;
}w.setArticles(x);
w.setState(l);
...
}},properties:{sort:{check:[l,m,n],init:l,apply:j},separatorX:{check:p,nullable:true,apply:s},separatorY:{check:p,nullable:true,apply:s},connectSeparators:{check:e,init:false,apply:s},spacingX:{check:o,init:0,apply:s},spacingY:{check:o,init:0,apply:s}},members:{__iL:null,__pe:null,verifyLayoutProperty:null,_applySort:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__pf:{north:1,south:2,west:3,east:4,center:5},__pg:{1:q,2:d,3:r,4:g},__jw:function(){var x=this._getLayoutChildren();
var F,z;
var length=x.length;
var B=[];
var E=[];
...
delete this._invalidChildrenCache;
},renderLayout:function(H,I){if(this._invalidChildrenCache){this.__jw();
}var bc=qx.ui.layout.Util;
var O=this.__iL;
var bd=this.__pe;
var length=O.length;
...
}},destruct:function(){this.__pe=this.__iL=null;
}});
})();
...
},events:{"execute":R},properties:{enabled:{init:true,check:l,event:o,apply:I},shortcut:{check:n,apply:Q,nullable:true},autoRepeat:{check:l,init:false}},members:{__pi:g,__pj:g,execute:function(U){this.fireDataEvent(L,U);
},__pk:function(event){if(this.getEnabled()&&this.__pl(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},__dA:function(event){if(this.getEnabled()&&this.__pl(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},_applyEnabled:function(V,W){if(V){qx.event.Registration.addListener(document.documentElement,h,this.__pk,this);
qx.event.Registration.addListener(document.documentElement,d,this.__dA,this);
}else{qx.event.Registration.removeListener(document.documentElement,h,this.__pk,this);
qx.event.Registration.removeListener(document.documentElement,d,this.__dA,this);
}},_applyShortcut:function(X,Y){if(X){if(X.search(/[\s]+/)!=-1){var bc=y;
this.error(bc);
throw new Error(bc);
}this.__pi={"Control":false,"Shift":false,"Meta":false,"Alt":false};
this.__pj=null;
var ba;
var a=[];
...
for(var i=0;i<bb;i++){var bd=this.__pn(a[i]);

switch(bd){case f:case u:case K:case C:this.__pi[bd]=true;
break;
case m:var bc=F+a[i];
this.error(bc);
throw bc;
default:if(this.__pj){var bc=r;
this.error(bc);
throw bc;
}this.__pj=bd;
}}}return true;
},__pl:function(e){var be=this.__pj;

if(!be){return ;
}if((!this.__pi.Shift&&e.isShiftPressed())||(this.__pi.Shift&&!e.isShiftPressed())||(!this.__pi.Control&&e.isCtrlPressed())||(this.__pi.Control&&!e.isCtrlPressed())||(!this.__pi.Meta&&e.isMetaPressed())||(this.__pi.Meta&&!e.isMetaPressed())||(!this.__pi.Alt&&e.isAltPressed())||(this.__pi.Alt&&!e.isAltPressed())){return false;
}
if(be==e.getKeyIdentifier()){return true;
}return false;
},__pm:{esc:S,ctrl:f,print:P,del:q,pageup:b,pagedown:E,numlock:O,numpad_0:J,numpad_1:D,numpad_2:t,numpad_3:s,numpad_4:A,numpad_5:N,numpad_6:B,numpad_7:H,numpad_8:M,numpad_9:z,numpad_divide:p,numpad_multiply:v,numpad_minus:c,numpad_plus:k},__pn:function(bf){var bg=qx.event.handler.Keyboard;
var bh=m;
...
var bh=this.__pm[bf]||qx.lang.String.firstUp(bf);

if(bg.isValidKeyIdentifier(bh)){return bh;
}else{return m;
}},toString:function(){var bk=this.__pj;
var bj=[];

for(var bi in this.__pi){if(this.__pi[bi]){bj.push(qx.locale.Key.getKeyName(j,bi));
}}
if(bk){bj.push(qx.locale.Key.getKeyName(j,bk));
}return bj.join(k);
}},destruct:function(){this.setEnabled(false);
this.__pi=this.__pj=null;
}});
})();
...
(function(){var f="1.5",e="feedreader.view.Header",d="qooxdoo ",c="default",b="Feed Reader",a="app-header";
...
var g=new qx.ui.basic.Label(d+f);
g.setFont(c);
this.add(h);
...
(function(){var l="execute",k="settings",j="loading",h="main",g="addfeed",f="selection[0]",d="title",c="north",b="new",a="changeModel",Y="__pA",X="__pr",W="article",V="__px",U="F1",T="selection[0].state",S="partLoaded",R="Control+A",Q="loaded",P="icon/22/places/folder.png",s="feedreader/images/loading22.gif",t="error",q="change",r="user",o="__pz",p="__pv",m="feeds",n="__pt",v="__pB",w="enabled",D="__ps",B="vertical",H="state",F="__pp",L="__py",J="Control+D",y="icon/22/actions/process-stop.png",O="selection[0].articles",N="__po",M="static",x="icon/22/apps/internet-feed-reader.png",z="model",A="feedreader.Application",C="Control+P",E="Control+R",G="feedreader.model.Feed",I="__pu",K="__pq",u="__pw";
qx.Class.define(A,{extend:qx.application.Standalone,members:{__po:null,__pp:null,__D:null,__pq:null,__pr:null,__ps:null,__pt:null,__pu:null,__pv:null,__pw:null,__px:null,__py:null,__pz:null,__pA:null,__pB:null,__pC:false,__pD:false,main:function(){qx.application.Standalone.prototype.main.call(this);
qx.io.PartLoader.getInstance().addListener(S,function(e){this.debug("part loaded: "+e.getData().getName());
},this);
var bb=qx.locale.Manager.getInstance().getLanguage();
var ba=qx.Part.getInstance().getParts();
if(ba[bb]){qx.io.PartLoader.require([bb],function(){qx.locale.Manager.getInstance().setLocale(bb);
this.buildUpGui();
},this);
}else{this.warn("Cannot load locale part for current language "+bb+", falling back to English.");
this.buildUpGui();
}},buildUpGui:function(){this._initializeCommands();
this._createLayout();
var bc=new feedreader.model.Model();
this.__pz=bc.getFeedFolder();
this.__pB=bc.getStaticFeedFolder();
this.__pA=bc.getUserFeedFolder();
this._setUpBinding();
this.__pq.getRoot().setOpen(true);
this.__pq.getRoot().getChildren()[0].setOpen(true);
this.__pq.getRoot().getChildren()[1].setOpen(true);
this.__pq.setHideRoot(true);
this.__po.getSelection().push(this.__pB.getFeeds().getItem(0));
this.reload();
},_setUpBinding:function(){var bd={converter:this._state2iconConverter};
this.__po=new qx.data.controller.Tree(this.__pz,this.__pq,m,d);
this.__po.setIconOptions(bd);
this.__po.setIconPath(H);
this.__pp=new qx.data.controller.List(null,this.__pt.getList(),d);
this.__po.bind(O,this.__pp,z);
this.__pp.bind(f,this.__px,W);
this.__pp.getSelection().addListener(q,this._listControllerChange,this);
this.__pp.addListener(a,this._treeControllerChange,this);
var be={converter:this._state2loadingConverter};
this.__po.bind(T,this.__pt,j,be);
be={converter:this._category2enabledConverter};
this.__po.bind(f,this.__ps.getRemoveButton(),w,be);
},_category2enabledConverter:function(bf){return !!bf&&bf.getCategory()==r;
},_state2iconConverter:function(bg){if(bg==b||bg==j){return s;
}else if(bg==Q){return x;
}else if(bg==t){return y;
}return P;
},_state2loadingConverter:function(bh){if(bh==b||bh==j){return true;
}return false;
},_listControllerChange:function(bi){var bj=this.__po.getSelection().getItem(0);
var bk=this.__pp.getSelection().getItem(0);
if(bk!=undefined){bj.setSelectedArticle(bk);
}},_treeControllerChange:function(bl){var bm=this.__po.getSelection().getItem(0);
if(bm!=null&&bm.classname==G&&bm.getSelectedArticle()!=null&&bm.getArticles().contains(bm.getSelectedArticle())){qx.event.Timer.once(function(){this.__pp.getSelection().push(bm.getSelectedArticle());
},this,0);
}else{this.__pt.getList().scrollToY(0);
}},_createLayout:function(){var bn=new qx.ui.layout.Dock();
var bo=new qx.ui.container.Composite(bn);
this.getRoot().add(bo,{edge:0});
this.__pr=new feedreader.view.Header();
bo.add(this.__pr,{edge:c});
this.__ps=new feedreader.view.ToolBar(this);
bo.add(this.__ps,{edge:c});
this.__pu=new qx.ui.splitpane.Pane();
bo.add(this.__pu);
this.__pq=new qx.ui.tree.Tree();
this.__pq.setWidth(250);
this.__pq.setDecorator(h);
this.__pq.setPadding(0);
this.__pu.add(this.__pq,0);
this.__pv=new qx.ui.splitpane.Pane(B);
this.__pv.setDecorator(null);
this.__pu.add(this.__pv,1);
this.__pt=new feedreader.view.List(this.__pz);
this.__pt.setHeight(200);
this.__pt.setDecorator(h);
this.__pv.add(this.__pt,0);
this.__px=new feedreader.view.Article();
this.__px.setDecorator(h);
this.__pv.add(this.__px,1);
},_initializeCommands:function(){var bp={};
bp.reload=new qx.ui.core.Command(E);
bp.reload.addListener(l,this.reload,this);
bp.about=new qx.ui.core.Command(U);
bp.about.addListener(l,this.showAbout,this);
bp.preferences=new qx.ui.core.Command(C);
bp.preferences.addListener(l,this.showPreferences,this);
bp.addFeed=new qx.ui.core.Command(R);
bp.addFeed.addListener(l,this.showAddFeed,this);
bp.removeFeed=new qx.ui.core.Command(J);
bp.removeFeed.addListener(l,this.removeFeed,this);
this.__D=bp;
},getCommand:function(bq){return this.__D[bq];
},addFeed:function(br,bs,bt){var bu=new feedreader.model.Feed(br,bs,bt);
this.__pA.getFeeds().push(bu);
var bv=feedreader.io.FeedLoader.getInstance();
bv.load(bu);
},removeFeed:function(){var bw=this.__po.getSelection().getItem(0);
if(bw&&bw.getCategory&&bw.getCategory()!==M){var bx=this.__pA.getFeeds();
for(var i=0;i<bx.length;i++){if(bw===bx.getItem(i)){bx.splice(i,1);
return;
}}}},reload:function(){var by=feedreader.io.FeedLoader.getInstance();
by.loadAll(this.__pz);
this.__px.setArticle(null);
},showPreferences:function(){if(!this.__pD){this.__ps.signalLoading(k,true);
}qx.io.PartLoader.require([k],function(){this.__pD=true;
if(!this.__pw){this.__pw=new feedreader.view.PreferenceWindow();
this.getRoot().add(this.__pw);
}this.__pw.center();
this.__pw.open();
this.__ps.signalLoading(k,false);
},this);
},showAbout:function(){alert(this.tr("FeedReader (qooxdoo powered)"));
},showAddFeed:function(){if(!this.__pC){this.__ps.signalLoading(g,true);
}qx.io.PartLoader.require([g],function(){this.__pC=true;
if(!this.__py){this.__py=new feedreader.view.AddFeedWindow(this);
this.getRoot().add(this.__py);
}this.__py.center();
this.__py.open();
this.__ps.signalLoading(g,false);
},this);
}},destruct:function(){this.__D=null;
this._disposeObjects(D,n,V,K,o,I,p,X,v,Y,N,F,u,L);
}});
})();
...
(function(){var k="enabled",j="originalIcon",i=")",h="Remove feed",g="Preferences",f="excluded",d="addFeed",c="removeFeed",b="icon/22/actions/help-about.png",a="Reload the feeds. (%1)",G="Help",F="visible",E="feedreader.view.ToolBar",D="icon/22/actions/media-seek-forward.png",C="toolbar-button",B="Add feed",A="addfeed",z="icon/22/actions/view-refresh.png",y="(",x="reload",r="showItem",s="settings",p="about",q="icon/22/actions/dialog-cancel.png",n="__pG",o="Reload",l="preferences",m="icon/22/actions/dialog-ok.png",t="icon/22/apps/preferences-theme.png",u="hideItem",w="Open preferences window.",v="feedreader/images/loading22.gif";
qx.Class.define(E,{extend:qx.ui.toolbar.ToolBar,construct:function(H){qx.ui.toolbar.ToolBar.call(this);
this.__pE={};
this.__pF=new qx.ui.toolbar.Button(this.tr(B),m);
this.__pF.setCommand(H.getCommand(d));
this.add(this.__pF);
this.__pG=new qx.ui.toolbar.Button(this.tr(h),q);
this.__pG.setCommand(H.getCommand(c));
this.__pG.setEnabled(false);
this.add(this.__pG);
this.addSeparator();
var K=new qx.ui.toolbar.Button(this.tr(o),z);
var I=H.getCommand(x);
K.setCommand(I);
K.setToolTipText(this.tr(a,I.toString()));
this.add(K);
this.addSeparator();
this.__pH=new qx.ui.toolbar.Button(this.tr(g),t);
this.__pH.setCommand(H.getCommand(l));
this.__pH.setToolTipText(this.tr(w));
this.add(this.__pH);
this.addSpacer();
var L=new qx.ui.toolbar.Button(this.tr(G),b);
var J=H.getCommand(p);
L.setCommand(J);
L.setToolTipText(y+J.toString()+i);
this.add(L);
this.setOverflowHandling(true);
var M=new qx.ui.toolbar.MenuButton(null,D);
M.setAppearance(C);
this.add(M);
this.setOverflowIndicator(M);
this.__pI=new qx.ui.menu.Menu();
M.setMenu(this.__pI);
this.addListener(u,this._onHideItem,this);
this.addListener(r,this._onShowItem,this);
},members:{__pG:null,__pI:null,__pE:null,__pF:null,__pH:null,getRemoveButton:function(){return this.__pG;
},_onHideItem:function(e){var O=e.getData();
var N=this._getMenuItem(O);
N.setVisibility(F);
},_onShowItem:function(e){var Q=e.getData();
var P=this._getMenuItem(Q);
P.setVisibility(f);
},_getMenuItem:function(R){var S=this.__pE[R.toHashCode()];

if(!S){if(R instanceof qx.ui.toolbar.Button){S=new qx.ui.menu.Button(R.getLabel().translate(),R.getIcon(),R.getCommand());
R.bind(k,S,k);
}else{S=new qx.ui.menu.Separator();
}this.__pI.addAt(S,0);
this.__pE[R.toHashCode()]=S;
}return S;
},signalLoading:function(T,U){if(T==A){var V=this.__pF;
}else if(T==s){var V=this.__pH;
}else{return ;
}if(U){V.setUserData(j,V.getIcon());
V.setIcon(v);
}else{var W=V.getUserData(j);
V.setIcon(W);
...
(function(){var m="horizontal",k="qx.event.type.Data",j="vertical",h="",g="qx.ui.form.List",f="text",d="Boolean",c="one",b="addChildWidget",a="_applySpacing",A="Enter",z="Integer",y="action",x="keyinput",w="addItem",v="removeChildWidget",u="_applyOrientation",t="single",s="keypress",r="list",p="label",q="pane",n="removeItem",o="__oF";
...
this.__oF=this._createListItemContainer();
this.__oF.addListener(b,this._onAddChild,this);
this.__oF.addListener(v,this._onRemoveChild,this);
this.getChildControl(q).add(this.__oF);
if(B){this.setOrientation(m);
}else{this.initOrientation();
}this.addListener(s,this._onKeyPress);
this.addListener(x,this._onKeyInput);
this.__pJ=h;
},events:{addItem:k,removeItem:k},properties:{appearance:{refine:true,init:r},focusable:{refine:true,init:true},orientation:{check:[m,j],init:j,apply:u},spacing:{check:z,init:0,apply:a,themeable:true},enableInlineFind:{check:d,init:true}},members:{__pJ:null,__pK:null,__oF:null,SELECTION_MANAGER:qx.ui.core.selection.ScrollArea,getChildrenContainer:function(){return this.__oF;
...
var content=this.__oF;
content.setLayout(F);
content.setAllowGrowX(!E);
content.setAllowGrowY(E);
this._applySpacing(this.getSpacing());
},_applySpacing:function(G,H){this.__oF.getLayout().setSpacing(G);
...
if(U.isRich()){var Q=U.getChildControl(p,true);
...
}},properties:{model:{check:p,apply:d,event:r,nullable:true,dereference:true},target:{apply:k,event:c,init:null,dereference:true},childPath:{check:t,apply:b,nullable:true},labelPath:{check:t,apply:o,nullable:true},iconPath:{check:t,apply:h,nullable:true},labelOptions:{apply:g,nullable:true},iconOptions:{apply:a,nullable:true},delegate:{apply:q,init:null,nullable:true}},members:{__rl:null,__jT:null,__nC:null,_applyDelegate:function(y,z){this._setConfigureItem(y,z);
this._setCreateItem(y,z);
this._setBindItem(y,z);
},_applyIconOptions:function(A,B){this.__nO();
},_applyLabelOptions:function(C,D){this.__nO();
},_applyTarget:function(E,F){if(F!=undefined){var G=F.getRoot();
F.setRoot(null);
G.destroy();
}if(this.getModel()!=null){this.__ro();
}this._addChangeTargetListener(E,F);
},_applyModel:function(H,I){this.__ro();
},_applyChildPath:function(J,K){this.__ro();
},_applyIconPath:function(L,M){this.__nO();
},_applyLabelPath:function(N,O){this.__nO();
},__rm:function(P){var R=P.getTarget();
var Q=this.__rl[R.toHashCode()].treeNode;
var S=this.__rl[R.toHashCode()].modelNode;
this.__rp(Q,S);
this._updateSelection();
},__rn:function(e){var ba=e.getData();
var V=e.getOldData();
var W=this.__rl[V.toHashCode()];
V.removeListenerById(W.changeListenerId);
delete this.__rl[V.toHashCode()];
...
var T=Y.event;
var X=bb.addListener(T,this.__rn,this);
var U=W.treeNode;
this.__rl[ba.toHashCode()]={modelNode:bb,treeNode:U,changeListenerId:W.changeListenerId,changeChildernListenerId:X};
this.__rp(U,bb);
this._updateSelection();
...
},__ro:function(){if(this.getTarget()==null||this.getChildPath()==null){return;
...
this.__rt(this.getModel(),bf);
this.__rp(bf,this.getModel());
}},__rp:function(bg,bh){if(bh[s+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;
}var bq=bh[s+qx.lang.String.firstUp(this.getChildPath())]();
if(this.__rl[bq.toHashCode()]==undefined){var bl=bq.addListener(m,this.__rm,this);
var bp=qx.Class.getPropertyDefinition(bh.constructor,this.getChildPath());
var bm=bp.event;
var bn=bh.addListener(bm,this.__rn,this);
this.__rl[bq.toHashCode()]={modelNode:bh,treeNode:bg,changeListenerId:bl,changeChildernListenerId:bn};
...
}},__nO:function(){var bS;

for(var bU in this.__jT){bS=bU;
break;
}for(var bR in this.__jT[bS]){var bV=this.__jT[bS][bR].treeNode;
var bT=qx.core.ObjectRegistry.fromHashCode(bR);
this.__ru(bT);
this.__rt(bT,bV);
}},__rt:function(bW,bX){var bY=this.getDelegate();
if(bY!=null&&bY.bindItem!=null){bY.bindItem(this,bX,bW);
}else{this.bindDefaultProperties(bX,bW);
}},__ru:function(ca){for(var i=0;i<this.__nC.length;i++){var cc=this.__nC[i];
var cb=this.__jT[cc][ca.toHashCode()];

if(cb!=null){if(cb.id){ca.removeBinding(cb.id);
}
if(cb.reverseId){cb.treeNode.removeBinding(cb.reverseId);
}delete this.__jT[cc][ca.toHashCode()];
...
}}},destruct:function(){this.__jT=this.__rl=this.__nC=null;
}});
})();
