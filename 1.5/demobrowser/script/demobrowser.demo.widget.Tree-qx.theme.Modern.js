...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.Tree","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Tree-qx.theme.Modern.fd841584aa5c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="demobrowser.demo.widget.Tree",d="qx.core.BaseInit",c="engine.name",b="os.name",a="engine.version";
...
(function(){var l="changeValue",k="execute",j="Trash",h="changeSelection",g="mode",f="multi",d="single",c="Windows (C:)",b="icon/22/places/user-desktop.png",a="Documents (D:)",P="Inbox",O="Desktop",N="root",M="Open mode:",L="Presets",K="Hide Root Node",J="Network",I="Control",H=" items",G="Sent",s="Files",t="Show Open Items",q="",r="Workspace",o="Enable drag selection",p="Selection: ",m="Junk #",n="demobrowser.demo.widget.Tree",u="Data",v="click",y="Show root open button",x="none",A="Enable multi selection",z="double click",C="Selection:",B="Show Selected Items",w="Root node:",F="Toggle Height",E="Edit",D="Show Items";
...
T.set({spacing:20});
var S=new qx.ui.container.Composite(T);
this.getRoot().add(S);
var Q=this.getTree();
S.add(Q);
var R=this.getCommandFrame(Q);
S.add(R);
},getTree:function(){var be=new qx.ui.tree.Tree().set({width:200,height:400});
var bj=new qx.ui.tree.TreeFolder(N);
bj.setOpen(true);
be.setRoot(bj);
var bh=new qx.ui.tree.TreeFolder(O);
bh.setOpen(true);
bh.setIcon(b);
bj.add(bh);
var Y=new qx.ui.tree.TreeFolder(s);
var W=new qx.ui.tree.TreeFolder(r);
var X=new qx.ui.tree.TreeFolder(J);
var V=new qx.ui.tree.TreeFolder(j);
bh.add(Y,W,X,V);
var bf=new qx.ui.tree.TreeFile(c);
var U=new qx.ui.tree.TreeFile(a);
W.add(bf,U);
var bi=new qx.ui.tree.TreeFolder(P);
var ba=new qx.ui.tree.TreeFolder(L);
var bd=new qx.ui.tree.TreeFolder(G);
var bc=new qx.ui.tree.TreeFolder(j);

for(var i=0;i<30;i++){bc.add(new qx.ui.tree.TreeFile(m+i));
}var bg=new qx.ui.tree.TreeFolder(u);
var bb=new qx.ui.tree.TreeFolder(E);
bi.add(ba,bd,bc,bg,bb);
bj.add(bi);
return be;
},getCommandFrame:function(bk){var bu=new qx.ui.groupbox.GroupBox(I);
var bs=4;
bu.setLayout(new qx.ui.layout.Grid(5,3));
var bB=0;
bu.add(new qx.ui.basic.Label(p).set({paddingTop:4}),{row:bB,column:0});
var by=new qx.ui.form.TextField().set({readOnly:true});
bu.add(by,{row:bB++,column:1});
bk.addListener(h,function(e){var bC=e.getData();

if(bC.length>0){if(this.getSelectionMode()===f){by.setValue(bC.length+H);
}else{by.setValue(bC[0].getLabel());
}}else{by.setValue(q);
}});
bu.add(new qx.ui.core.Spacer(bs,bs),{row:bB++,column:0});
bu.add(new qx.ui.basic.Label(M),{row:bB,column:0});
var bo={"click":v,"dblclick":z,"none":x};
var bm=new qx.ui.form.RadioGroup();

for(var bw in bo){var bn=new qx.ui.form.RadioButton(bo[bw]).set({value:bw==bk.getOpenMode()});
bn.setUserData(g,bw);
bm.add(bn);
bu.add(bn,{row:bB++,column:1});
}bm.addListener(h,function(e){bk.setOpenMode(e.getData()[0].getUserData(g));
});
bu.add(new qx.ui.core.Spacer(bs,bs),{row:bB++,column:0});
bu.add(new qx.ui.basic.Label(C),{row:bB,column:0});
var bq=new qx.ui.form.CheckBox(A);
bu.add(bq,{row:bB++,column:1});
bq.addListener(l,function(e){var bD=e.getData();
bk.setSelectionMode(bD?f:d);
});
var br=new qx.ui.form.CheckBox(o);
bu.add(br,{row:bB++,column:1});
br.addListener(l,function(e){var bE=e.getData();
bk.setDragSelection(bE);

if(!bq.getValue()){bq.setValue(true);
}});
bu.add(new qx.ui.core.Spacer(bs,bs),{row:bB++,column:0});
bu.add(new qx.ui.basic.Label(w),{row:bB,column:0});
var bz=new qx.ui.form.CheckBox(K);
bu.add(bz,{row:bB++,column:1});
bz.addListener(l,function(e){bk.setHideRoot(e.getData());
});
var bp=new qx.ui.form.CheckBox(y);
bu.add(bp,{row:bB++,column:1});
bp.addListener(l,function(e){bk.setRootOpenClose(e.getData());
});
bu.add(new qx.ui.core.Spacer(bs,bs),{row:bB++,column:0});
var bx=new qx.ui.form.Button(D);
bu.add(bx,{row:bB++,column:1});
bx.addListener(k,function(e){alert((""+bk.getItems()).replace(",","\n","g"));
});
var bl=new qx.ui.form.Button(t);
bu.add(bl,{row:bB++,column:1});
bl.addListener(k,function(e){alert((""+bk.getItems(true,false)).replace(",","\n","g"));
});
var bv=new qx.ui.form.Button(B);
bu.add(bv,{row:bB++,column:1});
bv.addListener(k,function(e){if(this.getSelectionMode()===d){alert(this.getSelection());
}else{alert(this.getSelection().toString().replace(",","\n","g"));
}},bk);
var bt=new qx.ui.form.Button(F);
bu.add(bt,{row:bB++,column:1});
var bA=true;
bt.addListener(k,function(e){bk.setHeight(bA?600:400);
bA=!bA;
});
return bu;
}}});
