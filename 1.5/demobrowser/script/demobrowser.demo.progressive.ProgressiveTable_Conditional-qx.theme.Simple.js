...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.progressive.ProgressiveTable_Conditional","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.progressive.ProgressiveTable_Conditional-qx.theme.Simple.b68433b37c66.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="os.name",b="demobrowser.demo.progressive.ProgressiveTable_Conditional",a="engine.version";
...
(function(){var o="",n="between",m="regex",l="!between",k="==",j=">=",h=">",g="<",f='g',e=":",b=";",d="qx.ui.progressive.renderer.table.cell.Conditional",c="!=",a="<=";
...
this.__BU=r||"";
this.__BV=s||"";
},members:{__BQ:null,__Gg:null,__BR:null,__BS:null,__BT:null,__BU:null,__BV:null,__CJ:function(t,u){if(t.align){u["text-align"]=t.align;
}
...
}},addNumericCondition:function(v,w,x,y,z,A,B){if(!qx.lang.Array.contains(this.__BQ,v)||w==null){throw new Error("Condition not recognized or value is null!");
}this.__BR.push({condition:v,align:x,color:y,style:z,weight:A,value1:w,target:B});
},addBetweenCondition:function(C,D,E,F,G,H,I,J){if(!qx.lang.Array.contains(this.__Gg,C)||D==null||E==null){throw new Error("Condition not recognized or value1/value2 is null!");
}this.__BR.push({condition:C,align:F,color:G,style:H,weight:I,value1:D,value2:E,target:J});
},addRegex:function(K,L,M,N,O,P){if(!K){throw new Error("regex cannot be null!");
}this.__BR.push({condition:m,align:L,color:M,style:N,weight:O,regex:K,target:P});
},_getCellStyle:function(Q){if(this.__BR.length==0){return Q.style||o;
}var i;
var U;
var R;
var T={"text-align":this.__BS,"color":this.__BT,"font-style":this.__BU,"font-weight":this.__BV};

for(i=0;i<this.__BR.length;i++){var X=this.__BR[i];
U=false;

if(qx.lang.Array.contains(this.__BQ,X.condition)){if(X.target==null){R=Q.cellData;
}else{R=Q.element.data[X.target];
}
switch(X.condition){case k:if(R==X.value1){U=true;
}break;
...
switch(X.condition){case n:if(R>=X.value1&&R<=X.value2){U=true;
}break;
case l:if(R<X.value1&&R>X.value2){U=true;
}break;
}}else if(X.condition==m){if(X.target==null){R=Q.cellData;
}else{R=Q.element.data[X.target];
}var S=new RegExp(X.value1,f);
U=S.test(R);
}if(U){this.__CJ(X,T);
}var V=[];

for(var W in T){if(T[W]){V.push(W,e,T[W],b);
}}}return V.join(o);
}},destruct:function(){this.__BQ=this.__Gg=this.__BR=null;
}});
})();
