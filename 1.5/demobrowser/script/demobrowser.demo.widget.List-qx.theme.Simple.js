...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.List","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.List-qx.theme.Simple.a6a076fab098.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="demobrowser.demo.widget.List",b="os.name",a="engine.version";
...
var ce=new qx.ui.container.Scroll();
var cu=new qx.ui.container.Composite(new qx.ui.layout.Basic());
cu.setAllowStretchX(false);
ce.add(cu);
this.getRoot().add(ce,{edge:0});
var co=new qx.ui.basic.Label(bk);
co.setFont(bF);
cu.add(co,{left:20,top:20});
var cw=new qx.ui.form.List;
cw.setScrollbarX(n);
cw.set({height:280,width:150,selectionMode:bH});
var cx;

for(var i=1;i<=25;i++){cx=new qx.ui.form.ListItem(bA+i,J+((i%4)?L:bq)+U);
cw.add(cx);
!(i%9)&&(cx.setEnabled(false));
if(i==20){cw.setSelection([cx]);
}}cw.addListener(bE,function(e){var cD=e.getData();

if(cD[0]){this.debug("Value: "+cD[0].getLabel());
}});
cu.add(cw,{left:20,top:40});
var ck=new qx.ui.form.RadioButton(G);
var ch=new qx.ui.form.RadioButton(bS);
var ci=new qx.ui.form.RadioButton(ca);
var cg=new qx.ui.form.RadioButton(p);
ck.setUserData(bI,bC);
ch.setUserData(bI,bH);
ci.setUserData(bI,bD);
cg.setUserData(bI,bG);
ch.setValue(true);
cu.add(ck,{left:180,top:40});
cu.add(ch,{left:180,top:60});
cu.add(ci,{left:180,top:80});
cu.add(cg,{left:180,top:100});
var cj=new qx.ui.form.RadioGroup(ck,ch,ci,cg);
var cA=new qx.ui.form.RadioButton(j);
var cz=new qx.ui.form.RadioButton(bo);
var cC=new qx.ui.form.RadioButton(bx);
cA.setUserData(bI,F);
cz.setUserData(bI,by);
cC.setUserData(bI,W);
cC.setValue(true);
cu.add(cA,{left:180,top:140});
cu.add(cz,{left:180,top:160});
cu.add(cC,{left:180,top:180});
var cv=new qx.ui.form.RadioGroup(cA,cz,cC);
cv.addListener(bE,function(e){for(var i=0;i<cw.getChildren().length;i++){cw.getChildren()[i].setShow(e.getData()[0].getUserData(bI));
}});
var cs=new qx.ui.form.CheckBox(bv);
var cp=new qx.ui.form.CheckBox(l).set({enabled:false});
cu.add(cs,{left:180,top:220});
cu.add(cp,{left:180,top:240});
cs.addListener(bB,function(e){if(e.getData()){var cE=cw.getSelectionMode();

if(cE==bC||cE==bG){this.debug("Drag selection is only available for the modes multi or additive");
}}cw.setDragSelection(e.getData());
});
cp.addListener(bB,function(e){if(e.getData()){var cF=cw.getSelectionMode();

if(cF==bH||cF==bD){this.debug("Quick selection is only available for the modes multi or additive");
}}cw.setQuickSelection(e.getData());
});
cj.addListener(bE,function(e){var cG=e.getData()[0].getUserData(bI);
cw.setSelectionMode(cG);

if(cG==bC||cG==bG){cs.setEnabled(false);
cp.setEnabled(true);
}else if(cG==bH||cG==cd){cs.setEnabled(true);
cp.setEnabled(false);
}});
var cB=new qx.ui.basic.Label(bV);
cB.setFont(bF);
cu.add(cB,{left:330,top:20});
var cn=new qx.ui.form.List();
cn.set({height:280,width:150,selectionMode:bG});
var cx;

for(var i=1;i<=25;i++){cx=new qx.ui.form.ListItem(bA+i,bj);
cn.add(cx);
if(i==16){cn.setSelection([cx]);
}}cu.add(cn,{left:330,top:40});
var co=new qx.ui.basic.Label(bc);
co.setFont(bF);
cu.add(co,{left:520,top:20});
var cf=new qx.ui.form.List;
var cl;
cf.set({width:150,selectionMode:bD});
var cr=[bu,bf,B,bW,M,br,I,Q,P,bz,bb,s,u,bN,q,be,X,v,bJ,bK,T,bR,O,bM,x,a,bX,N,cb,bg,Y,o,H,bL,bh,t,S,bm,bU,bO,h,C,bl,cc,r,bP,w,g,bn,m,V];

for(var i=0;i<cr.length;i++){cl=new qx.ui.form.ListItem(cr[i]);
cf.add(cl);

if(i==10||i==12||i==16){cf.addToSelection(cl);
}}cu.add(cf,{left:520,top:40});
var co=new qx.ui.basic.Label(bd);
co.setFont(bF);
cu.add(co,{left:20,top:350});
var cy=new qx.ui.form.List(true);
var cm;
cy.set({width:550,selectionMode:bH,height:null});
var cq=[bT,z,bs,c,bw,bi,bY,R,ba,bt,f,d,D,b,bQ,A,K,y,E];
var ct=bp;

for(var i=0;i<cq.length;i++){cm=new qx.ui.form.ListItem(null,ct+cq[i]);
cy.add(cm);

if(i==12){cy.setSelection([cm]);
}}cu.add(cy,{left:20,top:370});
}}});
})();
