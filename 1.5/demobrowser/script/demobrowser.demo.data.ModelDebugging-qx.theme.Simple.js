...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.ModelDebugging","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ModelDebugging-qx.theme.Simple.0fd0eebfc8e1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
}},members:{__nc:null,__nd:function(r){return qx.Bootstrap.getKeys(r).sort().join(h);
},toClass:function(s,t){if(!qx.lang.Type.isObject(s)||s instanceof qx.core.Object){if(s instanceof Array||qx.Bootstrap.getClass(s)==l){for(var i=0;i<s.length;i++){this.toClass(s[i],t);
}}return ;
}var v=this.__nd(s);
for(var w in s){this.toClass(s[w],t);
}if(qx.Class.isDefined(n+v)){return;
}if(this.__nc&&this.__nc.getModelClass&&this.__nc.getModelClass(v)!=null){return;
}var C={};
var B={__ne:this.__ne};
...
if(t){C[w].apply=c;
}if(this.__nc&&this.__nc.getValidationRule){var y=this.__nc.getValidationRule(v,w);

if(y){C[w].validate=k+w;
B[k+w]=y;
}}}if(this.__nc&&this.__nc.getModelSuperClass){var A=this.__nc.getModelSuperClass(v)||qx.core.Object;
}else{var A=qx.core.Object;
}var x=[];

if(this.__nc&&this.__nc.getModelMixins){var z=this.__nc.getModelMixins(v);
if(!qx.lang.Type.isArray(z)){if(z!=null){x=[z];
}}}if(t){x.push(qx.data.marshal.MEventBubbling);
}var u={extend:A,include:x,properties:C,members:B,destruct:this.__nf};
qx.Class.define(n+v,u);
},__nf:function(){var D=qx.util.PropertyUtil.getAllProperties(this.constructor);

for(var E in D){this.__ne(this.get(D[E].name));
}},__ne:function(F){if(!(F instanceof qx.core.Object)){return ;
}if(F.isDisposed()){return;
}F.dispose();
},__ng:function(G){var H;
if(this.__nc&&this.__nc.getModelClass){H=this.__nc.getModelClass(G);
}
...
(function(){var j="qx.event.type.Event",i="qx.debug.io",h="qx.debug",g="fail",f="GET",e="qx.event.type.Data",d="load",c="loadEnd",b="Map",a="qx.io.request.AbstractRequest",D="changePhase",C="sent",B="qx.io.request.authentication.IAuthentication",A="error",z="loading",y="abort",x="success",w="String",v="",u="opened",q="POST",r="statusError",o="readyStateChange",p="abstract",m="unsent",n="changeResponse",k="Number",l="Content-Type",s="timeout",t="undefined";
qx.Class.define(a,{type:p,extend:qx.core.Object,construct:function(E){qx.core.Object.call(this);
...
this.fireEvent(R);
},_setPhase:function(S){var T=this.__ny;
...
}},__nz:function(){var W=this.getRequestHeaders();

if(W){qx.lang.Object.getKeys(W).forEach(function(X){this._transport.setRequestHeader(X,W[X]);
},this);
}},__nA:function(){var ba=this.getAuthentication(),Y=this._transport;
...
(function(){var u="qx.event.type.Event",t="GET",s="function",r="xml",q="POST",p="application/xml",o="HEAD",n="Boolean",m="PUT",l="Accept",e="qx.debug",k="application/json",h="application/x-www-form-urlencoded",c="Cache-Control",b="Content-Type",g="OPTIONS",f="qx.debug.io",i="qx.io.request.Xhr",a="json",j="DELETE",d="String";
qx.Class.define(i,{extend:qx.io.request.AbstractRequest,construct:function(v,w){if(w!==undefined){this.setMethod(w);
}qx.io.request.AbstractRequest.call(this,v);
},events:{"readystatechange":u,"success":u,"load":u,"statusError":u},statics:{PARSER:{json:qx.lang.Json.parse,xml:qx.xml.Document.fromString}},properties:{method:{check:[o,g,t,q,m,j],init:t},async:{check:n,init:true},accept:{check:d,nullable:true},cache:{check:function(x){return qx.lang.Type.isBoolean(x)||qx.lang.Type.isString(x);
},init:true}},members:{__ol:null,_createTransport:function(){return new qx.bom.request.Xhr();
},_getConfiguredUrl:function(){var y=this.getUrl(),z;

if(this.getMethod()===t&&this.getRequestData()){z=this._serializeData(this.getRequestData());
y=qx.util.Uri.appendParamsToUrl(y,z);
}
if(this.getCache()===false){y=qx.util.Uri.appendParamsToUrl(y,{nocache:new Date().valueOf()});
}return y;
},_setRequestHeaders:function(){var A=this._transport;
if(qx.lang.Type.isString(this.getCache())){A.setRequestHeader(c,this.getCache());
}if(this.getMethod()===q){A.setRequestHeader(b,h);
}if(this.getAccept()){if(qx.core.Environment.get(f)){this.debug("Accepting: '"+this.getAccept()+"'");
}A.setRequestHeader(l,this.getAccept());
}},_getParsedResponse:function(){var C=this._transport.responseText,B=this._getParser();

if(typeof B===s){return B.call(this,C);
}return C;
},setParser:function(D){var E=qx.io.request.Xhr;
if(typeof E.PARSER[D]===s){return this.__ol=E.PARSER[D];
}if(qx.core.Environment.get(e)){qx.core.Assert.assertFunction(D);
}return this.__ol=D;
},_getParser:function(){var F=this.__ol;
if(F){return F;
}if(!this.isDone()){return;
}switch(this.getResponseContentType()){case k:F=qx.io.request.Xhr.PARSER[a];
break;
case p:F=qx.io.request.Xhr.PARSER[r];
break;
default:F=null;
break;
}if((/[^\/]+\/[^\+]+\+xml/).test(this.getResponseContentType())){F=qx.io.request.Xhr.PARSER[r];
}return F;
}}});
})();
...
},events:{"created":j,"configured":j,"sending":j,"receiving":j,"completed":R,"aborted":j,"failed":R,"timeout":R},statics:{__rC:0,methodAllowsRequestBody:function(V){return (V==Q)||(V==K);
...
},init:true,apply:H},crossDomain:{check:k,init:false},fileUpload:{check:k,init:false},transport:{check:m,nullable:true},useBasicHttpAuth:{check:k,init:false},parseJson:{check:k,init:true}},members:{__ry:null,__rz:null,__rA:null,__rB:null,__rC:null,send:function(){qx.io.remote.RequestQueue.getInstance().add(this);
...
},__rD:qx.event.GlobalError.observeMethod(function(e){var W=e.clone();
W.setTarget(this);
...
},setRequestHeader:function(bh,bi){this.__ry[bh]=bi;
},removeRequestHeader:function(bj){delete this.__ry[bj];
},getRequestHeader:function(bk){return this.__ry[bk]||null;
},getRequestHeaders:function(){return this.__ry;
},setParameter:function(bl,bm,bn){if(bn){this.__rA[bl]=bm;
}else{this.__rz[bl]=bm;
}},removeParameter:function(bo,bp){if(bp){delete this.__rA[bo];
}else{delete this.__rz[bo];
}},getParameter:function(bq,br){if(br){return this.__rA[bq]||null;
}else{return this.__rz[bq]||null;
}},getParameters:function(bs){return (bs?this.__rA:this.__rz);
},setFormField:function(bt,bu){this.__rB[bt]=bu;
},removeFormField:function(bv){delete this.__rB[bv];
},getFormField:function(bw){return this.__rB[bw]||null;
...
(function(){var l="qx.ioRemoteDebug",k="Integer",j="qx.debug",h="aborted",g="_onaborted",f="_on",d="Boolean",c="__on",b="singleton",a="interval",x="receiving",w="Request-Queue Progress: ",v="queued",u="io.maxrequests",t="_applyEnabled",s="sending",r="completed",q="failed",p="__kr",o="qx.io.remote.RequestQueue",m="timeout",n="/";
...
},properties:{enabled:{init:true,check:d,apply:t},maxTotalRequests:{check:k,nullable:true},maxConcurrentRequests:{check:k,init:qx.core.Environment.get(u)},defaultTimeout:{check:k,init:5000}},members:{__db:null,__on:null,__oo:null,__kr:null,getRequestQueue:function(){return this.__db;
},getActiveQueue:function(){return this.__on;
},_debug:function(){if(qx.core.Environment.get(j)){if(qx.core.Environment.get(l)){var y=this.__on.length+n+(this.__db.length+this.__on.length);
...
}},_remove:function(B){qx.lang.Array.remove(this.__on,B);
B.dispose();
this._check();
},__op:0,_onsending:function(e){if(qx.core.Environment.get(j)){if(qx.core.Environment.get(l)){this.__op++;
e.getTarget()._counted=true;
this.debug("ActiveCount: "+this.__op);
}}e.getTarget().getRequest()._onsending(e);
},_onreceiving:function(e){e.getTarget().getRequest()._onreceiving(e);
},_oncompleted:function(e){if(qx.core.Environment.get(j)){if(qx.core.Environment.get(l)){if(e.getTarget()._counted){this.__op--;
this.debug("ActiveCount: "+this.__op);
...
D[g](event);
}}catch(F){}}},_oninterval:function(e){var M=this.__on;

if(M.length==0){this.__kr.stop();
return;
...
}this.__kr.setEnabled(N);
},add:function(P){P.setState(v);

if(P.isAsynchronous()){this.__db.push(P);
}else{this.__db.unshift(P);
}this._check();
...
if(R){R.abort();
}else if(qx.lang.Array.contains(this.__db,Q)){qx.lang.Array.remove(this.__db,Q);
...
},__mI:function(){var bT=this.getRequest();
...
(function(){var l="qx.debug",k="qx.ioRemoteDebugData",j="=",h="",g="engine.name",f="&",d="application/xml",c="application/json",b="text/html",a="qx.ioRemoteDebug",I="textarea",H="_data_",G="load",F="text/plain",E="text/javascript",D="readystatechange",C="completed",B="?",A="qx.io.remote.transport.Iframe",z="none",s="display",t="gecko",q="frame_",r="aborted",o="pre",p="javascript:void(0)",m="sending",n="opera",u="failed",v="mshtml",x="form_",w="timeout",y="qx/static/blank.gif";
...
if((qx.core.Environment.get(g)==v)){M=p;
}this.__nj=qx.bom.Iframe.create({id:K,name:K,src:M});
qx.bom.element.Style.set(this.__nj,s,z);
this.__Q=qx.bom.Element.create("form",{id:L,name:L,target:K});
qx.bom.element.Style.set(this.__Q,s,z);
qx.dom.Element.insertEnd(this.__Q,qx.dom.Node.getBodyElement(document));
this.__dc=qx.bom.Element.create(I,{id:H,name:H});
qx.dom.Element.insertEnd(this.__dc,this.__Q);
qx.dom.Element.insertEnd(this.__nj,qx.dom.Node.getBodyElement(document));
qx.event.Registration.addListener(this.__nj,G,this._onload,this);
this.__nk=qx.lang.Function.listener(this._onreadystatechange,this);
qx.bom.Event.addNativeListener(this.__nj,D,this.__nk);
},statics:{handles:{synchronous:false,asynchronous:true,crossDomain:false,fileUpload:true,programaticFormFields:true,responseTypes:[F,E,c,d,b]},isSupported:function(){return true;
},_numericMap:{"uninitialized":1,"loading":2,"loaded":2,"interactive":3,"complete":4}},members:{__dc:null,__nl:0,__Q:null,__nj:null,__nk:null,send:function(){var O=this.getMethod();
var Q=this.getUrl();
...
if(R instanceof Array){for(var i=0;i<R.length;i++){T.push(encodeURIComponent(P)+j+encodeURIComponent(R[i]));
}}else{T.push(encodeURIComponent(P)+j+encodeURIComponent(R));
}}
if(T.length>0){Q+=(Q.indexOf(B)>=0?f:B)+T.join(f);
...
if(T.length>0){this.setData(T.join(f));
}}var N=this.getFormFields();

for(var P in N){var S=document.createElement(I);
S.name=P;
S.appendChild(document.createTextNode(N[P]));
this.__Q.appendChild(S);
}this.__Q.action=Q;
this.__Q.method=O;
this.__dc.appendChild(document.createTextNode(this.getData()));
this.__Q.submit();
this.setState(m);
},_onload:qx.event.GlobalError.observeMethod(function(e){if(qx.core.Environment.get(g)==n&&this.getIframeHtmlContent()==h){return;
}
...
}),_switchReadyState:function(V){switch(this.getState()){case C:case r:case u:case w:this.warn("Ignore Ready State Change");
return;
}while(this.__nl<V){this.setState(qx.io.remote.Exchange._nativeMap[++this.__nl]);
...
if(!ba.firstChild){return h;
}if(ba.firstChild.tagName&&ba.firstChild.tagName.toLowerCase()==o){return ba.firstChild.innerHTML;
}else{return ba.innerHTML;
}},getIframeHtmlContent:function(){var bb=this.getIframeBody();
return bb?bb.innerHTML:null;
},getFetchedLength:function(){return 0;
},getResponseContent:function(){if(this.getState()!==C){if(qx.core.Environment.get(l)){if(qx.core.Environment.get(a)){this.warn("Transfer not complete, ignoring content!");
}}return null;
}
...
switch(this.getResponseType()){case F:if(qx.core.Environment.get(l)){if(qx.core.Environment.get(k)){this.debug("Response: "+this._responseContent);
}}return bc;
break;
case b:bc=this.getIframeHtmlContent();
...
},destruct:function(){if(this.__nj){qx.event.Registration.removeListener(this.__nj,G,this._onload,this);
qx.bom.Event.removeNativeListener(this.__nj,D,this.__nk);
if((qx.core.Environment.get(g)==t)){this.__nj.src=qx.util.ResourceManager.getInstance().toUri(y);
...
var v=++qx.io.remote.transport.Script.__nC;

if(v>=2000000000){qx.io.remote.transport.Script.__nC=v=1;
}this.__iI=null;
this.__nC=v;
},statics:{__nC:0,_instanceRegistry:{},ScriptTransport_PREFIX:e,ScriptTransport_ID_PARAM:h,ScriptTransport_DATA_PARAM:g,handles:{synchronous:false,asynchronous:true,crossDomain:true,fileUpload:false,programaticFormFields:false,responseTypes:[o,m,q]},isSupported:function(){return true;
...
}})},members:{__nl:0,__iI:null,__nC:null,send:function(){var A=this.getUrl();
A+=(A.indexOf(l)>=0?t:l)+qx.io.remote.transport.Script.ScriptTransport_ID_PARAM+s+this.__nC;
...
this.__iI=document.createElement(b);
this.__iI.charset=j;
this.__iI.src=A;
...
return;
}while(this.__nl<E){this.setState(qx.io.remote.Exchange._nativeMap[++this.__nl]);
...
}},properties:{parseJson:{check:r,init:true}},members:{__nn:false,__nl:0,__no:null,getRequest:function(){if(this.__no===null){this.__no=qx.io.remote.transport.XmlHttp.createRequestObject();
...
if(bn==4){if(!qx.io.remote.Exchange.wasSuccessful(this.getStatusCode(),bn,this.__nn)){if(this.getState()===N){this.setState(c);
}this.failed();
return;
}}if(bn==3&&this.__nl==1){this.setState(qx.io.remote.Exchange._nativeMap[++this.__nl]);
}while(this.__nl<bn){this.setState(qx.io.remote.Exchange._nativeMap[++this.__nl]);
...
if(qx.io.remote.Exchange.wasSuccessful(bI,bJ,this.__nn)){try{bK=this.getRequest().responseXML;
...
(function(){var l="qx.debug.io",k="engine.name",j="mshtml",i="",h="engine.version",g="qx.debug",f="head",e="script",d="opera",c="Unknown response headers",a="unknown",b="qx.bom.request.Script";
qx.Bootstrap.define(b,{construct:function(){this.__mR();
...
},members:{readyState:null,status:null,statusText:null,timeout:null,__mN:null,open:function(m,n){if(this.__mm){return;
}this.__mR();
this.__mO=null;
this.__mP=n;
...
},setRequestHeader:function(o,p){if(this.__mm){return;
}var q={};

if(this.readyState!==1){throw new Error("Invalid state");
}q[o]=p;
this.__mP=qx.util.Uri.appendParamsToUrl(this.__mP,q);
},send:function(){if(this.__mm){return;
}var t=this.__mW(),r=this.__mM,s=this;
...
window.setTimeout(function(){s.__mS(2);
s.__mS(3);
});
...
this.__mX();
this.onabort();
},onreadystatechange:function(){},onload:function(){},onloadend:function(){},onerror:function(){},onabort:function(){},ontimeout:function(){},getResponseHeader:function(u){if(this.__mm){return;
}
...
this.__mN=v;
},dispose:function(){var w=this.__mQ;

if(!this.__mm){if(w){w.onload=w.onreadystatechange=null;
this.__mX();
}
...
}},_onNativeLoad:function(){var z=this.__mQ,x=this.__mN,y=this;
if(this.__mO){return;
...
if(this.__bX){window.clearTimeout(this.__bX);
}window.setTimeout(function(){y.__mT();
y.__mS(4);
y.onload();
y.onloadend();
});
...
this.onerror();
this.onloadend();
},__mQ:null,__mM:null,__mP:i,__mJ:null,__mK:null,__mL:null,__bX:null,__mO:null,__mm:null,__mR:function(){this.readyState=0;
this.status=0;
this.statusText=i;
},__mS:function(A){this.readyState=A;
this.onreadystatechange();
...
},__mV:function(){var C=qx.core.Environment.get(k)===j&&qx.core.Environment.get(h)<9;
var B=qx.core.Environment.get(k)===d;
return !(C||B);
},__mW:function(){var D=this.__mQ=document.createElement(e);
D.src=this.__mP;
D.onerror=this.__mK;
D.onload=this.__mJ;
if(qx.core.Environment.get(k)===j&&qx.core.Environment.get(h)<9){D.onreadystatechange=this.__mJ;
}return D;
},__mX:function(){var E=this.__mQ;

if(E&&E.parentNode){this.__mM.removeChild(E);
...
this.__oa();
},members:{responseJson:null,__df:null,__nT:null,__nU:null,__nV:null,__nW:null,__mm:null,open:function(k,l){if(this.__mm){return;
}var m={},o,n,p=this;
this.responseJson=null;
this.__nV=false;
o=this.__nT||h;
n=this.__nU||a+this.__df+b;
if(!this.__nU){this.constructor[this.__df]=this;
}else{if(!window[this.__nU]){this.__nW=true;
window[this.__nU]=function(q){p.callback(q);
};
}else{if(qx.core.Environment.get(j)){qx.log.Logger.debug(qx.bom.request.Jsonp,"Callback "+this.__nU+" already exists");
}}}
...
this.__nY(g,[k,l]);
},callback:function(r){if(this.__mm){return;
}this.__nV=true;
...
this.__nX();
},setCallbackParam:function(s){this.__nT=s;
...
this.__nW=false;
}},__nY:function(t,u){qx.bom.request.Script.prototype[t].apply(this,u||[]);
},__oa:function(){this.__df=(new Date().valueOf())+(e+Math.random()).substring(2,5);
}}});
})();
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="demobrowser.demo.data.ModelDebugging",b="os.name",a="engine.version";
...
(function(){var m="auto",l="debugModel function in qx.dev.Debug.",k="objects with properties. To see the properties, you can use the ",j="demobrowser.demo.data.ModelDebugging",i="wittemann",h="loaded",g="Every model created by the data stores consists only of qooxdoo ",f="demobrowser.demo.data.store.Twitter",e="<b>Debugging models</b><br/>",d="white",a="main",c="http://twitter.com/statuses/user_timeline/",b=".json";
...
var p=new demobrowser.demo.data.store.Twitter(i);
var n=new qx.ui.embed.Html();
n.setBackgroundColor(d);
n.setDecorator(a);
n.setWidth(500);
n.setHeight(200);
n.setOverflow(m,m);
this.getRoot().add(n,{left:10,top:80});
p.addListener(h,function(){var q=p.getModel();
this.debug(qx.dev.Debug.debugProperties(q));
n.setHtml(qx.dev.Debug.debugProperties(q,10,true));
},this);
...
qx.Class.define(f,{extend:qx.data.store.Jsonp,construct:function(r){var s=c+r+b;
qx.data.store.Jsonp.call(this,s);
}});
})();
...
(function(){var m=": ",l="",k="construct",h="object",g="-",f="qx.debug.dispose",d=":",c=") ***",b="\r\n",a="px;'>",K="): ",J="function",I="</span><br>",H="*** EXCEPTION (",G="============================================================",F="Object",E="<br>",D="null",C="Array",B="members",t="statics",u="get",r="qx.dev.Debug",s=": EXCEPTION expanding property",p="\n",q="*** TOO MUCH RECURSION: not displaying ***",n="Object, count=",o="  ",v="<span style='padding-left:",w=" ",y="------------------------------------------------------------",x="Array, length=",A="undefined",z="index(";
qx.Class.define(r,{statics:{disposeProfilingActive:false,debugObject:function(L,M,N){qx.log.Logger.debug(this,qx.dev.Debug.debugObjectToString(L,M,N,false));
},debugObjectToString:function(O,P,Q,R){if(!Q){Q=10;
}var X=(R?I:p);
var U=function(Y){var ba;

if(!R){ba=l;

for(var i=0;i<Y;i++){ba+=o;
}}else{ba=v+(Y*8)+a;
}return ba;
};
var W=l;
var T=function(bb,bc,bd){if(bc>bd){W+=(U(bc)+q+X);
return;
}if(typeof (bb)!=h){W+=U(bc)+bb+X;
return;
}for(var be in bb){if(typeof (bb[be])==h){try{if(bb[be] instanceof Array){W+=U(bc)+be+m+C+X;
}else if(bb[be]===null){W+=U(bc)+be+m+D+X;
continue;
}else if(bb[be]===undefined){W+=U(bc)+be+m+A+X;
continue;
}else{W+=U(bc)+be+m+F+X;
}T(bb[be],bc+1,bd);
}catch(e){W+=U(bc)+be+s+X;
}}else{W+=U(bc)+be+m+bb[be]+X;
}}};
if(P){W+=U(0)+P+X;
}
if(O instanceof Array){W+=U(0)+x+O.length+d+X;
}else if(typeof (O)==h){var S=0;
...
try{T(O,0,Q);
}catch(bf){W+=U(0)+H+bf+c+X;
}W+=U(0)+G+X;
return W;
},getFunctionName:function(bg,bh){var bi=bg.self;

if(!bi){return null;
}while(bg.wrapper){bg=bg.wrapper;
}
switch(bh){case k:return bg==bi?k:null;
case B:return qx.lang.Object.getKeyFromValue(bi,bg);
case t:return qx.lang.Object.getKeyFromValue(bi.prototype,bg);
default:if(bg==bi){return k;
...
if(qx.lang.Type.isNumber(bj)||qx.lang.Type.isString(bj)||qx.lang.Type.isBoolean(bj)||bj==null||bk<=0){return bj;
}else if(qx.Class.hasInterface(bj.constructor,qx.data.IListData)){for(var i=0;i<bj.length;i++){for(var j=0;j<bm;j++){bp+=g;
}bp+=z+i+K+this.debugProperties(bj.getItem(i),bk-1,bl,bm+1)+bn;
}return bp+bn;
}else if(bj.constructor!=null){var bq=bj.constructor.$$properties;

for(var bo in bq){bp+=bn;
for(var j=0;j<bm;j++){bp+=g;
}bp+=w+bo+m+this.debugProperties(bj[u+qx.lang.String.firstUp(bo)](),bk-1,bl,bm+1);
}return bp;
}return l;
},startDisposeProfiling:qx.core.Environment.select(f,{"true":function(){this.disposeProfilingActive=true;
this.__Js=qx.core.ObjectRegistry.getNextHash();
},"default":qx.lang.Function.empty}),stopDisposeProfiling:qx.core.Environment.select(f,{"true":function(br){if(!this.__Js){qx.log.Logger.error("Call "+this.classname+".startDisposeProfiling first.");
return [];
}this.disposeProfilingActive=false;
var bt=[];
while(!qx.ui.core.queue.Dispose.isEmpty()){qx.ui.core.queue.Dispose.flush();
}var bu=qx.core.ObjectRegistry.getNextHash();
var bw=qx.core.ObjectRegistry.getPostId();
var bx=qx.core.ObjectRegistry.getStackTraces();

for(var bs=this.__Js;bs<bu;bs++){var bv=qx.core.ObjectRegistry.fromHashCode(bs+bw);

if(bv&&bv.isDisposed&&!bv.isDisposed()){if(br&&typeof br==J&&!br(bv)){continue;
