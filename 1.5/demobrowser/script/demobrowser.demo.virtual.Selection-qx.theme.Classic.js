...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.virtual.Selection","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Selection-qx.theme.Classic.ac872a6c4bc9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="os.name",b="demobrowser.demo.virtual.Selection",a="engine.version";
...
(function(){var k="manager",j="multi",i=" / ",h="bold",g="value",f="white",d="selected",c="#EEE",b="horizontal",a="table-scroller-header",F="one",E="single",D="changeValue",C="table-row-background-selected",B="additive",A="default",z="Additive Selection",y="Cell Line Selection",x="addaptive",w="Options",r="Row Selection",s="changeSelection",p="Column Selection",q="Enable drag selection",n="demobrowser.demo.virtual.Selection",o="Selection Mode",l="Single Selection",m="Cell Rectangle Selection",t="Enable quick selection",u="One Selection",v="Multi Selection";
...
var K=qx.theme.manager.Font.getInstance().resolve(A).getStyles();
this._fontCss=qx.bom.element.Style.compile(K);
var T=new qx.ui.layout.Grid(5,0);
T.setRowFlex(2,1);
T.setColumnFlex(0,1);
T.setColumnFlex(1,1);
T.setColumnFlex(2,1);
T.setColumnFlex(3,1);
var H=[];
var R=new qx.ui.container.Composite(T);
this.getRoot().add(R,{edge:5});
R.add(new qx.ui.basic.Label(r).set({font:h,decorator:a,padding:3,allowGrowX:true}),{row:1,column:0});
var J=this.createRowSelectionScroller();
H.push(J.getUserData(k));
R.add(J,{row:2,column:0});
R.add(new qx.ui.basic.Label(p).set({font:h,decorator:a,padding:3,allowGrowX:true}),{row:1,column:1});
var J=this.createColumnSelectionScroller();
H.push(J.getUserData(k));
R.add(J,{row:2,column:1});
R.add(new qx.ui.basic.Label(m).set({font:h,decorator:a,padding:3,allowGrowX:true}),{row:1,column:2});
var J=this.createCellRectangleSelectionScroller();
H.push(J.getUserData(k));
R.add(J,{row:2,column:2});
R.add(new qx.ui.basic.Label(y).set({font:h,decorator:a,padding:3,allowGrowX:true}),{row:1,column:3});
var J=this.createCellLineSelectionScroller();
H.push(J.getUserData(k));
R.add(J,{row:2,column:3});
var P=new qx.ui.layout.Grid(20,4);
var M=new qx.ui.container.Composite(P).set({padding:15,backgroundColor:f});
R.add(M,{row:0,column:0,colSpan:4});
M.add(new qx.ui.basic.Label(o).set({font:h,padding:3}),{row:0,column:0});
var L=new qx.ui.form.RadioButton(l);
L.setUserData(g,E);
M.add(L,{row:1,column:0});
var O=new qx.ui.form.RadioButton(v);
O.setUserData(g,j);
O.setValue(true);
M.add(O,{row:2,column:0});
var I=new qx.ui.form.RadioButton(z);
I.setUserData(g,B);
M.add(I,{row:3,column:0});
var G=new qx.ui.form.RadioButton(u);
G.setUserData(g,F);
M.add(G,{row:4,column:0});
var N=new qx.ui.form.RadioGroup(L,O,I,G);
N.addListener(s,function(e){var U=e.getData()[0].getUserData(g);
H.forEach(function(V){V.setMode(U);
});

if(U==E||U==F){S.setEnabled(false);
Q.setEnabled(true);
}else if(U==j||U==x){S.setEnabled(true);
Q.setEnabled(false);
}});
M.add(new qx.ui.basic.Label(w).set({font:h,padding:3}),{row:0,column:1});
var S=new qx.ui.form.CheckBox(q);
M.add(S,{row:1,column:1});
S.addListener(D,function(e){if(e.getData()){var W=H[0].getMode();

if(W==E||W==F){this.debug("Drag selection is only available for the modes multi or additive");
}}H.forEach(function(X){X.setDrag(e.getData());
});
});
var Q=new qx.ui.form.CheckBox(t).set({enabled:false});
M.add(Q,{row:2,column:1});
Q.addListener(D,function(e){if(e.getData()){var Y=H[0].getMode();

if(Y==j||Y==B){this.debug("Quick selection is only available for the modes multi or additive");
}}H.forEach(function(ba){ba.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bf=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var bd=new qx.ui.virtual.layer.Row(f,c);
bf.getPane().addLayer(bd);
bf.getPane().addLayer(new qx.ui.virtual.layer.GridLines(b));
var be=new qx.ui.virtual.cell.Cell();
var bc=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bg,bh){var bi={};

if(bb.isItemSelected(bg)){bi.selected=true;
}return be.getCellProperties(bg+i+bh,bi);
}});
bf.getPane().addLayer(bc);
var bb=new qx.ui.virtual.selection.Row(bf.getPane(),{styleSelectable:function(bj,bk,bl){if(bk!==d){return;
}
if(bl){bd.setBackground(bj,d);
}else{bd.setBackground(bj,null);
}bc.updateLayerData();
}});
bb.attachMouseEvents(bf.getPane());
bb.attachKeyEvents(bf);
bb.set({mode:j});
bb.addItem(0);
bf.setUserData(k,bb);
return bf;
},createColumnSelectionScroller:function(){var bp=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bp.getPane().addLayer(new qx.ui.virtual.layer.Row(f,c));
var bo=new qx.ui.virtual.layer.Column();
bp.getPane().addLayer(bo);
bp.getPane().addLayer(new qx.ui.virtual.layer.GridLines(b));
var bq=new qx.ui.virtual.cell.Cell();
var bn=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(br,bs){var bt={};

if(bm.isItemSelected(bs)){bt.selected=true;
}return bq.getCellProperties(br+i+bs,bt);
}});
bp.getPane().addLayer(bn);
var bm=new qx.ui.virtual.selection.Column(bp.getPane(),{styleSelectable:function(bu,bv,bw){if(bv!==d){return;
}
if(bw){bo.setBackground(bu,d);
}else{bo.setBackground(bu,null);
}bn.updateLayerData();
}});
bm.attachMouseEvents(bp.getPane());
bm.attachKeyEvents(bp);
bm.set({mode:j});
bm.addItem(0);
bp.setUserData(k,bm);
return bp;
},createCellRectangleSelectionScroller:function(){var bA=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bA.getPane().addLayer(new qx.ui.virtual.layer.Row(f,c));
var bz=new qx.ui.virtual.cell.Cell();
var bB=new qx.ui.virtual.cell.Cell().set({backgroundColor:C});
var by=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bC,bD){var bE={};

if(bx.isItemSelected({row:bC,column:bD})){bE.selected=true;
}
if(bE.selected){return bB.getCellProperties(bC+i+bD,bE);
}else{bz.resetBackgroundColor();
return bz.getCellProperties(bC+i+bD,bE);
}}});
bA.getPane().addLayer(by);
bA.getPane().addLayer(new qx.ui.virtual.layer.GridLines(b));
var bx=new qx.ui.virtual.selection.CellRectangle(bA.getPane(),{styleSelectable:function(bF,bG,bH){by.updateLayerData();
}});
bx.attachMouseEvents(bA.getPane());
bx.attachKeyEvents(bA);
bx.set({mode:j});
bx.addItem({row:0,column:0});
bA.setUserData(k,bx);
return bA;
},createCellLineSelectionScroller:function(){var bL=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bL.getPane().addLayer(new qx.ui.virtual.layer.Row(f,c));
var bK=new qx.ui.virtual.cell.Cell();
var bM=new qx.ui.virtual.cell.Cell().set({backgroundColor:C});
var bJ=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bN,bO){var bP={};

if(bI.isItemSelected({row:bN,column:bO})){bP.selected=true;
}
if(bP.selected){return bM.getCellProperties(bN+i+bO,bP);
}else{bK.resetBackgroundColor();
return bK.getCellProperties(bN+i+bO,bP);
}}});
bL.getPane().addLayer(bJ);
bL.getPane().addLayer(new qx.ui.virtual.layer.GridLines(b));
var bI=new qx.ui.virtual.selection.CellLines(bL.getPane(),{styleSelectable:function(bQ,bR,bS){bJ.updateLayerData();
}});
bI.attachMouseEvents(bL.getPane());
bI.attachKeyEvents(bL);
bI.set({mode:j});
bI.addItem({row:0,column:0});
bL.setUserData(k,bI);
return bL;
}}});
})();
...
(function(){var w="appear",v="qx.ui.virtual.core.CellEvent",u="update",t="change",s="qx.event.type.Data",r="qx.ui.virtual.core.Pane",q="resize",p="__zp",o="click",n="__zv",f="scrollX",m="dblclick",j="contextmenu",c="qx.debug",b="cellClick",h="__zq",g="cellContextmenu",k="qx.event.type.Event",a="__zu",l="cellDblclick",d="scrollY";
qx.Class.define(r,{extend:qx.ui.core.Widget,construct:function(x,y,z,A){qx.ui.core.Widget.call(this);
this.__zp=new qx.ui.virtual.core.Axis(z,x);
this.__zq=new qx.ui.virtual.core.Axis(A,y);
this.__rJ=0;
this.__rI=0;
this.__zr=0;
this.__zs=0;
...
this._add(this.__zu);
this.__zv=[];
this.__zp.addListener(t,this.fullUpdate,this);
this.__zq.addListener(t,this.fullUpdate,this);
...
for(var i=0;i<this.__zv.length;i++){var D=this.__zv[i];

if(D.isVisible()){C.push(D);
}}return C;
},getScrollMaxX:function(){var E=this.getBounds();

if(E){return Math.max(0,this.__zq.getTotalSize()-E.width);
}return 0;
},getScrollMaxY:function(){var F=this.getBounds();

if(F){return Math.max(0,this.__zp.getTotalSize()-F.height);
}return 0;
...
if(this.__rJ!==G){var I=this.__rJ;
this.__rJ=G;
...
if(J!==this.__rI){var L=this.__rI;
this.__rI=J;
this._deferredUpdateScrollPosition();
this.fireDataEvent(f,J,L);
...
return;
}var Q=this.__zp.getItemPosition(M);
var O=Q+this.__zp.getItemSize(M);
...
return;
}var T=this.__zq.getItemPosition(R);
var S=T+this.__zq.getItemSize(R);
...
}bc=this.__zp.getItemAtPosition(this.getScrollY()+bb-be.top);
bd=this.__zq.getItemAtPosition(this.getScrollX()+ba-be.left);
...
if(!bl){return;
}var bm=this.__rI+bl.width;
var bn=this.__zs-bm;

if(this.__rI-this.__zt.left<Math.min(this.__rI,bf)||this.__zt.right-bm<Math.min(bn,bh)){var bo=Math.min(this.__rI,bg);
var bk=Math.min(bn,bi);
this._setLayerWindow(bj,this.__rI-bo,this.__rJ,bl.width+bo+bk,bl.height,false);
...
if(!bw){return;
}var bu=this.__rJ+bw.height;
var bv=this.__zr-bu;

if(this.__rJ-this.__zt.top<Math.min(this.__rJ,bp)||this.__zt.bottom-bu<Math.min(bv,br)){var by=Math.min(this.__rJ,bq);
var bx=Math.min(bv,bs);
this._setLayerWindow(bt,this.__rI,this.__rJ-by,bw.width,bw.height+by+bx,false);
}},_onResize:function(){if(this.getContainerElement().getDomElement()){this.__zw=true;
this._updateScrollPosition();
this.__zw=null;
this.fireEvent(u);
}},_onAppear:function(){this.fullUpdate();
},_onClick:function(e){this.__zy(e,b);
},_onContextmenu:function(e){this.__zy(e,g);
},_onDblclick:function(e){this.__zy(e,l);
},__zy:function(e,bz){var bA=this.getCellAtPosition(e.getDocumentLeft(),e.getDocumentTop());
...
},_setLayerWindow:function(bB,bC,top,bD,bE,bF){var bK=this.__zp.getItemAtPosition(top);

if(bK){var bM=bK.index;
var bQ=this.__zp.getItemSizes(bM,bE+bK.offset);
...
var bS=0;
var bP=0;
}var bO=this.__zq.getItemAtPosition(bC);

if(bO){var bI=bO.index;
var bH=this.__zq.getItemSizes(bI,bD+bO.offset);
...
var bR=0;
var bJ=0;
}this.__zt={top:bS,bottom:bP,left:bR,right:bJ};
this.__zu.setUserBounds(this.__zt.left-this.__rI,this.__zt.top-this.__rJ,bN,bL);
this.__pi=bH;
this.__zx=bQ;

for(var i=0;i<this.__zv.length;i++){var bG=this.__zv[i];
...
if(this.__zr!==bT.height||this.__zs!==bT.width){this.__zr=bT.height;
this.__zs=bT.width;
this.fireEvent(u);
...
if(bU.length==0){this.__zz();
return;
}
if(!this.getContainerElement().getDomElement()){return ;
}var bV=this.getBounds();
this._setLayerWindow(bU,this.__rI,this.__rJ,bV.width,bV.height,true);
this.__zz();
...
if(bW.length==0){this.__zz();
return;
}var bY=this.getBounds();

if(!bY){return ;
}var bX={top:this.__rJ,bottom:this.__rJ+bY.height,left:this.__rI,right:this.__rI+bY.width};

if(this.__zt.top<=bX.top&&this.__zt.bottom>=bX.bottom&&this.__zt.left<=bX.left&&this.__zt.right>=bX.right){this.__zu.setUserBounds(this.__zt.left-bX.left,this.__zt.top-bX.top,this.__zt.right-this.__zt.left,this.__zt.bottom-this.__zt.top);
}else{this._setLayerWindow(bW,this.__rI,this.__rJ,bY.width,bY.height,false);
}this.__zz();
}},destruct:function(){this._disposeArray(n);
this._disposeObjects(p,h,a);
...
this.customSizes={};
},events:{"change":e},members:{__zA:null,getDefaultItemSize:function(){return this.defaultItemSize;
...
this.__zA=u;
return u;
},__zC:function(A){var B=this.__zA||this.__zB();
var C=0;
var E=B.length-1;
...
}},__zD:function(M){var N=this.__zA||this.__zB();
var O=0;
var Q=N.length-1;
...
}},getTotalSize:function(){var U=this.__zA||this.__zB();
...
},properties:{defaultLineColor:{init:e,check:c,themeable:true},defaultLineSize:{init:1,check:b,themeable:true}},members:{__AB:null,__AC:null,isHorizontal:function(){return this._isHorizontal;
...
if(this.__AD(l)){this.updateLayerData();
...
if(this.__AD(n)){this.updateLayerData();
}},__AD:function(p){if(this._isHorizontal){var t=this.getFirstColumn();
var r=t+this.getColumnSizes().length-1;
return p>=t&&p<=r;
}else{var s=this.getFirstRow();
var q=s+this.getRowSizes().length-1;
return p>=s&&p<=q;
}},getLineSize:function(u){return this.__AC[u]||this.getDefaultLineSize();
},getLineColor:function(v){return this.__AB[v]||this.getDefaultLineColor();
},__AE:function(w,z,A){var top=0;
var B,C;
...
if(this._isHorizontal){this.__AE(N,J,L);
}else{this.__AF(N,K,M);
...
}}},destruct:function(){this.__AB=this.__AC=null;
}});
})();
...
var q=p+qx.bom.element.Style.compile({position:d,overflow:c,cursor:i,textOverflow:h,userSelect:k})+f;

if(!(qx.core.Environment.get(m)==n)){q+=p+qx.bom.element.BoxSizing.compile(b)+g;
}this.__zg=qx.bom.Stylesheet.createElement(q);
this.__zh={};
this.__eS={};
},members:{__zg:null,__zh:null,__eS:null,getStylesheet:function(){return this.__zg;
},getCssClass:function(r){return this.__zh[r]||null;
},computeClassForStyles:function(s,t){var u=this.__eS[t];

if(!u){var u=this.__zi();
qx.bom.Stylesheet.addRule(this.__zg,e+u,t);
this.__eS[t]=u;
}this.__zh[s]=u;
return u;
},__zi:function(){return j+this.toHashCode()+o+(this.__zj++);
},__zj:0},destruct:function(){this.__zg=this.__zh=this.__eS=null;
}});
})();
...
this.__zg=qx.ui.virtual.cell.CellStylesheet.getInstance();
this.__zE={};
this.__zF={};
this.__zG={};
this.__zH={};
...
for(var K in N){if(!O[K]){this.__zJ.push(K);
}}L=L.superclass;
}},_getCssProperties:function(){return [J,l,C,q,d,c,e,h];
},_applyAppearance:function(P,Q){if(Q){this.__zF={};
}},_getValue:function(R){if(this.__zL){return qx.util.PropertyUtil.getThemeValue(this,R);
}else{return qx.util.PropertyUtil.getUserValue(this,R);
}},_storeStyle:function(S,T){var U;

if(this.__zL){U=this.__zF;
}else{U=this.__zE;
}
...
}},getCellProperties:function(bj,bk){this.__zN(bk);
...
},getCssClasses:function(bp,bq){var br=this.__zg.getCssClass(this.__zK)||f;
return n+br;
},__zN:function(bs){if(!bs){bs={};
}var bu=this.getAppearance();
var bt=bu+a+qx.lang.Object.getKeys(bs).sort().join(x);

if(this.__zK==bt){return;
}this.__zK=bt;
var bv=this.__jx[this.__zK];

if(!bv){this.__zO();
this.__zP(bs);
this.__zQ(bs);
this.__zR();
this.__jx[this.__zK]=1;
}this.__zS();
},__zO:function(){var bw=qx.util.PropertyUtil;
var bx=this._getCssProperties();

for(var i=0;i<bx.length;i++){bw.deleteThemeValue(this,bx[i]);
}},__zP:function(by){this.__zF={};
this.__zL=true;
...
},__zQ:function(){var bD=qx.lang.Object.getValues(this.__zF).join(b);
this.__zg.computeClassForStyles(this.__zK,bD);
},__zR:function(){var bI=this.__zJ;
...
}}this.__zI[this.__zK]=bG;
},__zS:function(){var bK=qx.util.PropertyUtil;
var bJ=this.__zI[this.__zK]||{};

for(var bL in bJ){bK.setThemed(this,bL,bJ[bL]);
}},getStyles:function(bM,bN){return qx.lang.Object.getValues(this.__zE).join(b);
},getInsets:function(bO,bP){var bT=this.__zG;
var bS=this.__zH;
...
return [bU+bQ,top+bR];
}},destruct:function(){this.__zg=this.__zE=this.__zF=this.__zG=this.__zH=this.__jx=this.__zI=this.__zJ=null;
}});
})();
