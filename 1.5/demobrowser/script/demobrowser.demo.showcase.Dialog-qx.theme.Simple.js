...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.showcase.Dialog","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.showcase.Dialog-qx.theme.Simple.8a83e9f6c5ff.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="demobrowser.demo.showcase.Dialog",b="os.name",a="engine.version";
...
(function(){var k="right",j="top",h="Last Name",g="Notes",f="male",e="First Name",d="default",c="Cancel",b="Sex",a="Reading",y="Smith",x="Swimming",w="City",v="demobrowser.demo.showcase.Dialog",u="Hobbies",t="icon/22/actions/dialog-apply.png",s="John",r="female",q="Page",p="icon/22/actions/dialog-cancel.png",n="Country",o="OK",l="USA",m="New York";
...
var H=new qx.ui.layout.Grid(9,5);
H.setColumnAlign(0,k,j);
H.setColumnAlign(2,k,j);
H.setColumnWidth(1,160);
H.setColumnWidth(2,72);
H.setColumnWidth(3,108);
var B=new qx.ui.tabview.TabView;
var K=new qx.ui.tabview.Page(q);
K.setLayout(H);
K.setPadding(10);
B.add(K);
this.getRoot().add(B,{left:40,top:40});
var E=[e,h,w,n,g];

for(var i=0;i<E.length;i++){K.add(new qx.ui.basic.Label(E[i]).set({allowShrinkX:false,paddingTop:3}),{row:i,column:0});
}var z=[s,y,m,l];

for(var i=0;i<z.length;i++){K.add(new qx.ui.form.TextField(z[i]),{row:i,column:1});
}K.add(new qx.ui.form.TextArea().set({height:250}),{row:4,column:1,colSpan:3});
K.add(new qx.ui.basic.Label(b).set({allowShrinkX:false,paddingTop:3}),{row:0,column:2});
var G=new qx.ui.form.RadioButton(r);
var J=new qx.ui.form.RadioButton(f);
var F=new qx.ui.form.RadioGroup();
F.add(G,J);
K.add(G,{row:0,column:3});
K.add(J,{row:1,column:3});
J.setValue(true);
K.add(new qx.ui.basic.Label(u).set({allowShrinkX:false,paddingTop:3}),{row:2,column:2});
K.add(new qx.ui.form.CheckBox(a),{row:2,column:3});
K.add(new qx.ui.form.CheckBox(x).set({enabled:false}),{row:3,column:3});
var I=new qx.ui.layout.HBox().set({spacing:4,alignX:k});
var A=new qx.ui.container.Composite(I).set({paddingTop:11});
K.add(A,{row:5,column:0,colSpan:4});
var C=new qx.ui.form.Button(o,t);
C.addState(d);
A.add(C);
var D=new qx.ui.form.Button(c,p);
A.add(D);
}}});
})();
...
(function(){var m="pane",k="lastTab",j="bar",h="page",g="firstTab",f="close",d="right",c="bottom",b="button",a="qx.debug",C="changeSelection",B="top",A="left",z="__yU",y="qx.event.type.Data",x="barRight",w="beforeChangeSelection",v="close-button",u="tabview",t="vertical",r="_applyBarPosition",s="barLeft",p="horizontal",q="qx.ui.tabview.TabView",n="barTop",o="barBottom";
...
this.__Fu={top:n,right:x,bottom:o,left:s};
this._createChildControl(j);
this._createChildControl(m);
var E=this.__yU=new qx.ui.form.RadioGroup;
E.setWrap(false);
E.addListener(C,this._onChangeSelection,this);
if(D!=null){this.setBarPosition(D);
}else{this.initBarPosition();
}},events:{"changeSelection":y},properties:{appearance:{refine:true,init:u},barPosition:{check:[A,d,B,c],init:B,apply:r}},members:{__yU:null,_createChildControlImpl:function(F,G){var H;
...
},add:function(I){if(qx.core.Environment.get(a)){if(!(I instanceof qx.ui.tabview.Page)){throw new Error("Incompatible child for TabView: "+I);
}}var J=I.getButton();
...
M.add(I);
this.__yU.add(J);
I.addState(this.__Fu[this.getBarPosition()]);
I.addState(k);
var L=this.getChildren();

if(L[0]==I){I.addState(g);
}else{L[L.length-2].removeState(k);
}I.addListener(f,this._onPageClose,this);
},addAt:function(N,O){if(qx.core.Environment.get(a)){if(!(N instanceof qx.ui.tabview.Page)){throw new Error("Incompatible child for TabView: "+N);
}}var R=this.getChildren();

if(!(O==null)&&O>R.length){throw new Error("Index should be less than : "+R.length);
}
...
S.addAt(N,O);
this.__yU.add(P);
N.addState(this.__Fu[this.getBarPosition()]);
R=this.getChildren();

if(O==R.length-1){N.addState(k);
}
if(R[0]==N){N.addState(g);
}else{R[R.length-2].removeState(k);
}N.addListener(f,this._onPageClose,this);
},remove:function(T){var Y=this.getChildControl(m);
...
Y.remove(T);
this.__yU.remove(V);
T.removeState(this.__Fu[this.getBarPosition()]);
if(T.hasState(g)){T.removeState(g);

if(X[0]){X[0].addState(g);
}}
if(T.hasState(k)){T.removeState(k);

if(X.length>0){X[X.length-1].addState(k);
}}T.removeListener(f,this._onPageClose,this);
},getChildren:function(){return this.getChildControl(m).getChildren();
},indexOf:function(ba){return this.getChildControl(m).indexOf(ba);
},__Fu:null,_applyBarPosition:function(bb,bc){var bd=this.getChildControl(j);
var bi=bb==A||bb==d;
var bg=bb==d||bb==c;
...
bd.setOrientation(bi?t:p);
var bj=this.getChildren();
if(bc){var be=this.__Fu[bc];
bd.removeState(be);
for(var i=0,l=bj.length;i<l;i++){bj[i].removeState(be);
}}
if(bb){var bf=this.__Fu[bb];
bd.addState(bf);
for(var i=0,l=bj.length;i<l;i++){bj[i].addState(bf);
}}},getSelection:function(){var bl=this.__yU.getSelection();
var bm=[];
...
for(var i=0;i<bn.length;i++){bo.push(bn[i].getChildControl(b));
}this.__yU.setSelection(bo);
},resetSelection:function(){this.__yU.resetSelection();
},isSelected:function(bp){var bq=bp.getChildControl(b);
return this.__yU.isSelected(bq);
},isSelectionEmpty:function(){return this.__yU.isSelectionEmpty();
},getSelectables:function(br){var bs=this.__yU.getSelectables(br);
var bt=[];

for(var i=0;i<bs.length;i++){bt.push(bs[i].getUserData(h));
}return bt;
},_onChangeSelection:function(e){var by=this.getChildControl(m);
var bv=e.getData()[0];
var bx=e.getOldData()[0];
var bu=[];
var bw=[];

if(bv){bu=[bv.getUserData(h)];
by.setSelection(bu);
bv.focus();
this.scrollChildIntoView(bv,null,null,false);
}else{by.resetSelection();
}
if(bx){bw=[bx.getUserData(h)];
}this.fireDataEvent(C,bu,bw);
...
if(bz){this.setSelection([bz.getUserData(h)]);
}else{this.resetSelection();
}},_onPageClose:function(e){var bB=e.getTarget();
var bA=bB.getButton().getChildControl(v);
bA.reset();
this.remove(bB);
