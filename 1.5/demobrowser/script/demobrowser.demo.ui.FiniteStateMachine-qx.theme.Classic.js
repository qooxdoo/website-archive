...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.FiniteStateMachine","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine-qx.theme.Classic.41404cbf3cdc.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var h="String",g="Number",f="__Hx",d="__Hy",c="string",b="qx.util.fsm.FiniteStateMachine";
...
if(!this.__HC[u]){this.__HC[u]={};
}this.__HC[u][q]=true;
if(!this.__HD[q]){this.__HD[q]=[];
}this.__HD[q]=this.__HD[q].concat(s);
}},removeObject:function(w){var y=this.__HA[w];
if(this.__HD[w]){for(var x in this.__HD[w]){delete this.__HC[x];
}delete this.__HD[w];
}delete this.__HB[y];
delete this.__HA[w];
delete this.__Hz[w];
},getObject:function(z){return this.__Hz[z];
},getFriendlyName:function(A){var B=A.toHashCode();
return B?this.__HB[B]:null;
},getGroupObjects:function(C){var a=[];

for(var name in this.__HC[C]){a.push(name);
}return a;
},displayAllObjects:function(){for(var D in this.__HA){var F=this.__HA[D];
var E=this.getObject(D);
this.debug(D+" => "+F);
this.debug("  "+F+" => "+this.__HB[F]);
this.debug("  "+D+" => "+this.getObject(D));
this.debug("  "+this.getObject(D)+" => "+this.getFriendlyName(E));
}},start:function(){var H=this.__Hv;
...
}this.__jx[H].getAutoActionsBeforeOnentry()(this);
if(G){this.debug(this.getName()+"#"+H+"#entry");
}this.__jx[H].getOnentry()(this,null);
if(G){this.debug(this.getName()+"#"+H+"#actionsAfterOnentry");
}this.__jx[H].getAutoActionsAfterOnentry()(this);
},pushState:function(I){if(this.__Hw.length>=this.getMaxSavedStates()){throw new Error("Saved-state stack is full");
}
if(I===true){this.__Hw.push(this.getState());
}else if(I){this.__Hw.push(I);
}else{this.__Hw.push(this.getPreviousState());
}},popState:function(){if(this.__Hw.length==0){throw new Error("Saved-state stack is empty");
}return this.__Hw.pop();
},postponeEvent:function(event){this.__Hy.unshift(event);
},enqueueEvent:function(event,J){if(J){this.__Hx.push(event);
}else{this.__Hx.unshift(event);
}
...
},__HF:function(){if(this.__HE){return ;
}this.__HE=true;
while(this.__Hx.length>0){var event=this.__Hx.pop();
var S=this.__HG(event);
if(S){event.dispose();
}}this.__HE=false;
},__HG:function(event){var V;
var X;
var U;
var bg;
var be;
...
if(typeof (X)=="string"){if(!X in this.__jx){throw new Error("Attempt to transition to nonexistent state "+X);
}this.setNextState(X);
}else{switch(X){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:X=V;
this.setNextState(X);
break;
case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Hw.length==0){throw new Error("Attempt to transition to POP_STATE_STACK "+"while state stack is empty.");
}X=this.__Hw.pop();
this.setNextState(X);
break;
...
}bg=this.__jx[this.getNextState()];
...
for(var i=0;i<this.__Hy.length;i++){e=this.__Hy.pop();
this.__Hx.unshift(e);
}
...
this.__Hw=this.__jx=null;
}});
})();
(function(){var p="context",o="string",n="object",m="number",l="function",k="autoActionsAfterOnentry",h='"',d="autoActionsBeforeOnexit",c="autoActionsBeforeOnentry",b="}",bb="{",ba="undefined",Y="autoActionsAfterOnexit",X="(",W=";",V="__Hp",U="  var groupObjects = ",T="parameters",S="__Hm",R=")",w="__Hr",x=" fsm.getObject('",u="__Hn",v="groups",s="  for (var i = 0; i < groupObjects.length; i++)",t="fsm",q="');",r="onexit",y="__Hs",z="__Ht",G="  fsm.debug(ex);",E="__Hq",K="    fsm.getObject(objName).",I=")(fsm)",N="events",M="    var objName = groupObjects[i];",B="').",Q="objects",P="__Ho",O="catch(ex)",A="  {",C="    fsm.getGroupObjects('",D="  }",F="qx.util.fsm.State",H="onentry",J="try",L=",";
...
for(var be in bd){switch(be){case H:this.setOnentry(this.__Hu(bd[be],bf));
break;
case r:this.setOnexit(this.__Hu(bd[be],bf));
break;
...
for(j=0;j<g.length;j++){bo+=U+C+g[j]+q+s+A+M+K+bn+W+D;
}}}}bo+=b+O+bb+G+b;
return qx.lang.Function.bind(new Function(t,bo),bi);
}},properties:{name:{transform:S,nullable:true},onentry:{transform:u,nullable:true,init:function(bp,event){}},onexit:{transform:P,nullable:true,init:function(bq,event){}},autoActionsBeforeOnentry:{transform:E,nullable:true,init:function(br,event){}},autoActionsAfterOnentry:{transform:w,nullable:true,init:function(bs,event){}},autoActionsBeforeOnexit:{transform:y,nullable:true,init:function(bt,event){}},autoActionsAfterOnexit:{transform:z,nullable:true,init:function(bu,event){}},events:{transform:V,nullable:true}},members:{__Hm:function(bv){if(typeof (bv)!=o||bv.length<1){throw new Error("Invalid state name");
}return bv;
},__Hn:function(bw){switch(typeof (bw)){case ba:return function(bx,event){};
case l:return qx.lang.Function.bind(bw,this.getUserData(p));
default:throw new Error("Invalid onentry type: "+typeof (bw));
}},__Ho:function(by){switch(typeof (by)){case ba:return function(bz,event){};
case l:return qx.lang.Function.bind(by,this.getUserData(p));
default:throw new Error("Invalid onexit type: "+typeof (by));
return null;
}},__Hp:function(bA){if(typeof (bA)!=n){throw new Error("events must be an object");
}for(var e in bA){var bB=bA[e];

if(typeof (bB)==m&&bB!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bB!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error("Invalid numeric value in events object: "+e+": "+bB);
}else if(typeof (bB)==n){for(var bC in bB){if(typeof (bB[bC])==m&&bB[bC]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bB[bC]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error("Invalid numeric value in events object "+"("+e+"): "+bC+": "+bB[bC]);
}else if(typeof (bB[bC])!=o&&typeof (bB[bC])!=m){throw new Error("Invalid value in events object "+"("+e+"): "+bC+": "+bB[bC]);
}}}else if(typeof (bB)!=o&&typeof (bB)!=m){throw new Error("Invalid value in events object: "+e+": "+bA[e]);
}}return bA;
},__Hq:function(bD){return qx.util.fsm.State._commonTransformAutoActions(c,bD,this.getUserData(p));
},__Hr:function(bE){return qx.util.fsm.State._commonTransformAutoActions(k,bE,this.getUserData(p));
},__Hs:function(bF){return qx.util.fsm.State._commonTransformAutoActions(d,bF,this.getUserData(p));
},__Ht:function(bG){return qx.util.fsm.State._commonTransformAutoActions(Y,bG,this.getUserData(p));
},__Hu:function(f,bH){if(typeof (f)==l){f=qx.lang.Function.bind(f,bH);
}return f;
...
(function(){var o="context",n="__HK",m="predicate",l="nextState",k="__HJ",j="autoActionsBeforeOntransition",i="function",h="String",g="__HH",e="__HL",b="autoActionsAfterOntransition",d="qx.util.fsm.Transition",c="__HI",a="ontransition";
qx.Class.define(d,{extend:qx.core.Object,construct:function(p,q){var s;
qx.core.Object.call(this);
this.setName(p);
s=q.context||window;
this.setUserData(o,s);
for(var r in q){switch(r){case m:this.setPredicate(this.__Hu(q[r],s));
break;
case l:this.setNextState(q[r]);
break;
case j:this.setAutoActionsBeforeOntransition(this.__Hu(q[r],s));
break;
case b:this.setAutoActionsAfterOntransition(this.__Hu(q[r],s));
break;
case a:this.setOntransition(this.__Hu(q[r],s));
break;
case o:break;
default:this.setUserData(r,q[r]);
this.debug("Transition "+p+": "+"Adding user-provided field to transition: "+r);
break;
}}},properties:{name:{check:h,nullable:true},predicate:{init:function(t,event){return true;
},transform:g},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:c},autoActionsBeforeOntransition:{init:function(u,event){},transform:k},autoActionsAfterOntransition:{init:function(v,event){},transform:n},ontransition:{init:function(w,event){},transform:e}},members:{__HH:function(x){switch(typeof (x)){case "undefined":return function(y,event){return true;
};
...
break;
}},__HI:function(A){switch(typeof (A)){case "string":return A;
...
break;
}},__HJ:function(B){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOntransition",B,this.getUserData("context"));
},__HK:function(C){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOntransition",C,this.getUserData("context"));
},__HL:function(D){switch(typeof (D)){case "undefined":return function(E,event){};
...
break;
}},__Hu:function(f,F){if(typeof (f)==i){f=qx.lang.Function.bind(f,F);
}return f;
}}});
})();
