...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.progressive.ProgressiveLoader","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.progressive.ProgressiveLoader-qx.theme.Simple.7450d4d20f6d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="demobrowser.demo.progressive.ProgressiveLoader",b="os.name",a="engine.version";
...
for(var i=0;i<e;i++){this.__It[i]=new qx.ui.core.ColumnData();
}},members:{__It:null,getData:function(){return this.__It;
...
}}},setWidth:function(j,k){if(j>this.__It.length-1||j<0){throw new Error("Column number out of range");
}this.__It[j].setColumnWidth(k);
},setMinWidth:function(l,m){if(l>this.__It.length-1||l<0){throw new Error("Column number out of range");
}this.__It[l].setMinWidth(m);
},setMaxWidth:function(n,o){if(n>this.__It.length-1||n<0){throw new Error("Column number out of range");
}this.__It[n].setMaxWidth(o);
}}});
})();
...
},members:{__Gm:null,join:function(c){this.__Gm=c;
...
(function(){var m="progressive-progressbar-indicator-undone",l="__HP",k="progress",j="%",i="progressive-progressbar-indicator-done",h="progressive-progressbar-percent-text",g="renderStart",f="__HO",d="progressive-progressbar-background",c="qx.ui.progressive.headfoot.Progress",a="progressive-progressbar-percent-background",b="renderEnd";
qx.Class.define(c,{extend:qx.ui.progressive.headfoot.Abstract,construct:function(n,o){qx.ui.progressive.headfoot.Abstract.call(this);
this.setHeight(16);
this.setPadding(0);
this.__GV={};
var p=qx.theme.manager.Color.getInstance();
this.__GV.background=p.resolve(d);
this.__GV.indicatorDone=p.resolve(i);
this.__GV.indicatorUndone=p.resolve(m);
this.__GV.percentBackground=p.resolve(a);
this.__GV.percentText=p.resolve(h);
this.set({backgroundColor:this.__GV.background});
this.__HO=new qx.ui.core.Widget();
this.__HO.set({width:0,backgroundColor:this.__GV.indicatorDone});
this.add(this.__HO);
var q=new qx.ui.core.Widget();
q.set({backgroundColor:this.__GV.indicatorUndone});
this.add(q,{flex:1});
this.__HP=new qx.ui.basic.Atom("0%");
this.__HP.set({width:100,backgroundColor:this.__GV.percentBackground,textColor:this.__GV.percentText});
this.add(this.__HP);
this.exclude();
},members:{__HQ:null,__GV:null,__HO:null,__HP:null,join:function(r){qx.ui.progressive.headfoot.Abstract.prototype.join.call(this,r);
r.addListener(g,function(e){this.__HQ=e.getData().initial;
this.show();
},this);
r.addListener(k,function(e){var u=1.0-(e.getData().remaining/this.__HQ);
var v=this.getBounds();

if(v){var s=Math.floor((v.width-this.__HP.getBounds().width)*u);
var t=Math.floor(u*100)+j;

if(!isNaN(s)){this.__HO.setMinWidth(s);
this.__HP.setLabel(t);
}}},this);
...
if(!e){this.__ub=new qx.ui.core.Widget();
this.__Bb=this.__ub;
}else{this.__ub=null;
this.__Bb=e;
}this.__Bb.getContentElement().setStyle(b,a);
},members:{__ub:null,__Bb:null,applyStructure:function(f){throw new Error("applyStructure() is abstract");
},getPane:function(){return this.__Bb;
}},destruct:function(){if(this.__ub){this.__ub.dispose();
}this.__ub=this.__Bb=null;
}});
})();
...
if(!b){this.__Hi=new qx.ui.progressive.headfoot.Null();
this.__ps=this.__Hi;
}else{this.__Hi=null;
this.__ps=b;
}if(!c){this.__Hj=new qx.ui.progressive.headfoot.Null();
this.__Hk=this.__Hj;
}else{this.__Hj=null;
this.__Hk=c;
}},members:{__ps:null,__Hk:null,__Hi:null,__Hj:null,applyStructure:function(e){this.__ps.join(e);
this.__Hk.join(e);
e.add(this.__ps);
e.add(this.getPane(),{flex:1});
e.add(this.__Hk);
},getHeader:function(){return this.__ps;
},getFooter:function(){return this.__Hk;
}},destruct:function(){if(this.__Hi){this.__Hi.dispose();
this.__Hi=null;
}
if(this.__Hj){this.__Hj.dispose();
this.__Hj=null;
}this.__ps=this.__Hk=null;
}});
})();
...
this.__Hc=false;
this.__Hd=0;
},events:{"renderStart":n,"renderEnd":n,"progress":n,"progressDetail":n},properties:{dataModel:{check:g,apply:j},batchSize:{check:k,init:20},flushWidgetQueueAfterBatch:{check:h,init:false},interElementTimeout:{check:k,init:0}},members:{__uu:null,__Hc:null,__He:null,__Hd:null,__Hb:null,__Ha:null,getStructure:function(){return this.__Ha;
},addRenderer:function(name,p){this.__uu[name]=p;
p.join(this,name);
},removeRenderer:function(name){if(!this.__uu[name]){throw new Error("No existing renderer named "+name);
}delete this.__uu[name];
},render:function(){if(this.__Hc){return;
}this.__Hc=true;
var q=new qx.ui.progressive.State({progressive:this,model:this.getDataModel(),pane:this.__Ha.getPane(),batchSize:this.getBatchSize(),rendererData:this.__Hg(),userData:{}});
this.__He=new Date();
if(this.__Hb){this.__Hd=q.getModel().getElementCount();
this.fireDataEvent(l,{state:q,initial:this.__Hd});
this.__Hf(q);
}else{qx.event.Timer.once(function(){this.__Hd=q.getModel().getElementCount();
this.fireDataEvent(l,{state:q,initial:this.__Hd});
this.__Hf(q);
this.__Hb=true;
},this,10);
}},_applyDataModel:function(r,s){if(s){s.removeListener(m,this.__Hh,this);
s.dispose();
}r.addListener(m,this.__Hh,this);
},__Hf:function(t){var u;
var w;
var v;

for(var i=t.getBatchSize();i>0;i--){u=t.getModel().getNextElement();

if(!u){this.debug("Render time: "+(new Date()-this.__He)+"ms");
this.__Hc=false;
this.fireDataEvent(b,t);
t.dispose();
return ;
}w=u.element;
v=this.__uu[w.renderer];
v.render(t,w);
this.fireDataEvent(a,{initial:this.__Hd,remaining:u.remaining,element:w});
}this.fireDataEvent(f,{initial:this.__Hd,remaining:u.remaining});
if(this.getFlushWidgetQueueAfterBatch()){qx.ui.core.queue.Manager.flush();
}qx.event.Timer.once(function(){this.__Hf(t);
},this,this.getInterElementTimeout());
},__Hg:function(){var x={};

for(var name in this.__uu){x[name]={};
}return x;
},__Hh:function(e){this.__Hd=e.getData();
this.render();
}},destruct:function(){for(var name in this.__uu){this.__uu[name].dispose();
}this.__He=this.__uu=this.__Ha=null;
}});
})();
...
this.__wh=[];
},members:{__wh:null,addElements:function(c){this.__wh=this.__wh.concat(c);
this.fireDataEvent(b,this.__wh.length);
},addElement:function(d){this.__wh.push(d);
this.fireDataEvent(b,this.__wh.length);
},getElementCount:function(){return this.__wh.length;
},getNextElement:function(){if(this.__wh.length>0){return ({element:this.__wh.shift(),remaining:this.__wh.length});
}return null;
