...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Remote_Model","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Remote_Model-qx.theme.Classic.74d53044c5f1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
this.__yD=null;
this.__wF=null;
},properties:{blockSize:{check:c,init:50},maxCachedBlockCount:{check:c,init:15},clearCacheOnRemove:{check:b,init:false},blockConcurrentLoadRowCount:{check:b,init:true}},members:{__GJ:null,__GO:null,__GK:null,__GL:null,__GM:null,__GN:null,__GP:null,__GQ:null,__yB:null,__yE:null,__wF:null,__yD:null,__GR:false,_getIgnoreCurrentRequest:function(){return this.__GO;
},getRowCount:function(){if(this.__GJ==-1){if(!this.__GR||!this.getBlockConcurrentLoadRowCount()){this.__GR=true;
this._loadRowCount();
}return (this.__GJ==-1)?0:this.__GJ;
}else{return this.__GJ;
}},_loadRowCount:function(){throw new Error("_loadRowCount is abstract");
},_onRowCountLoaded:function(f){if(this.getBlockConcurrentLoadRowCount()){this.__GR=false;
}if(f==null||f<0){f=0;
}this.__GJ=Number(f);
...
if(this.__GL!=-1){var h=this._cancelCurrentRequest();
...
this.__GR=true;
this._loadRowCount();
},clearCache:function(){this.__GP={};
this.__GQ=0;
},getCacheContent:function(){return {sortColumnIndex:this.__yB,sortAscending:this.__yE,rowCount:this.__GJ,lruCounter:this.__GK,rowBlockCache:this.__GP,rowBlockCount:this.__GQ};
},restoreCacheContent:function(j){if(this.__GL!=-1){var k=this._cancelCurrentRequest();

if(k){this.__GL=-1;
this.__GO=false;
}else{this.__GO=true;
}}this.__yB=j.sortColumnIndex;
this.__yE=j.sortAscending;
this.__GJ=j.rowCount;
this.__GK=j.lruCounter;
this.__GP=j.rowBlockCache;
this.__GQ=j.rowBlockCount;
var l={firstRow:0,lastRow:this.__GJ-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
for(var w=0;w<=o;w++){var q=this.__GP[w];
...
if(r!=null){u[t]=r;
}}}}},prefetchRows:function(x,y){if(this.__GL==-1){var z=this.getBlockSize();
var E=Math.ceil(this.__GJ/z);
...
for(var F=D;F<=C;F++){if(this.__GP[F]==null||this.__GP[F].isDirty){if(B==-1){B=F;
}A=F;
}}if(B!=-1){this.__GM=-1;
this.__GN=-1;
this.__GL=B;
...
},_onRowDataLoaded:function(I){if(I!=null&&!this.__GO){var L=this.getBlockSize();
var J=Math.ceil(I.length/L);

if(J==1){this._setRowBlockData(this.__GL,I);
}else{for(var i=0;i<J;i++){var O=i*L;
var N=[];
var K=Math.min(L,I.length-O);

for(var P=0;P<K;P++){N.push(I[O+P]);
}this._setRowBlockData(this.__GL+i,N);
}}var M={firstRow:this.__GL*L,lastRow:(this.__GL+J+1)*L-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(e,M);
}this.__GL=-1;
this.__GO=false;
if(this.__GM!=-1){this.prefetchRows(this.__GM,this.__GN);
}},_setRowBlockData:function(Q,R){if(this.__GP[Q]==null){this.__GQ++;

while(this.__GQ>this.getMaxCachedBlockCount()){var V;
var U=this.__GK;

for(var T in this.__GP){var S=this.__GP[T].lru;

if(S<U&&T>1){U=S;
V=T;
}}delete this.__GP[V];
this.__GQ--;
}}this.__GP[Q]={lru:++this.__GK,rowDataArr:R};
...
for(var bf=bc;bf<=bb;bf++){var X=this.__GP[bf];
...
if(bf==bb-1){if(X.rowDataArr.length==0){delete this.__GP[bf];
}}else{var bd=this.__GP[bf+1];

if(bd!=null){X.rowDataArr.push(bd.rowDataArr[0]);
}else{X.isDirty=true;
}}}}
if(this.__GJ!=-1){this.__GJ--;
...
if(bi.lru!=this.__GK){bi.lru=++this.__GK;
}return bj;
...
}}},setEditable:function(bv){this.__wF=[];

for(var bw=0;bw<this.getColumnCount();bw++){this.__wF[bw]=bv;
}this.fireEvent(d);
},setColumnEditable:function(bx,by){if(by!=this.isColumnEditable(bx)){if(this.__wF==null){this.__wF=[];
}this.__wF[bx]=by;
this.fireEvent(d);
}},isColumnEditable:function(bz){return (this.__wF?(this.__wF[bz]==true):false);
},setColumnSortable:function(bA,bB){if(bB!=this.isColumnSortable(bA)){if(this.__yD==null){this.__yD=[];
}this.__yD[bA]=bB;
this.fireEvent(d);
}},isColumnSortable:function(bC){return (this.__yD?(this.__yD[bC]!==false):true);
},sortByColumn:function(bD,bE){if(this.__yB!=bD||this.__yE!=bE){this.__yB=bD;
this.__yE=bE;
this.clearCache();
this.fireEvent(d);
}},getSortColumnIndex:function(){return this.__yB;
},isSortAscending:function(){return this.__yE;
},setSortColumnIndexWithoutSortingData:function(bF){this.__yB=bF;
},setSortAscendingWithoutSortingData:function(bG){this.__yE=bG;
}},destruct:function(){this.__yD=this.__wF=this.__GP=null;
}});
})();
(function(){var l="text",k="&end=",j='demobrowser.demo.table.RemoteTableModel',h="Text",g=' Generated on:',f="method=getRowData&start=",e="../../resource/demobrowser/backend/remote_table.php?",d="success",c="Id",b='Hello ',a="id";
qx.Class.define(j,{extend:qx.ui.table.model.Remote,construct:function(){qx.ui.table.model.Remote.call(this);
this.setColumns([c,h],[a,l]);
},members:{__MV:null,__MW:false,_loadRowCount:function(){this.__MX();

if(this.__MW){return;
}
if(this.__MV){this.__MY();
}else{this.__Nc();
}},_loadRowData:function(m,n){this.__MX();

if(this.__MW){return;
}
if(this.__MV){this.__Na(m,n);
}else{this.__Nd(m,n);
}},__MX:function(){if(this.__MW||this.__MV!==null){return;
}this.__MW=true;
this.__Nb("",function(o){this.__MW=false;
this.__MV=(o=="WTF PHP");
this._loadRowCount();
});
},__MY:function(){var p="method=getRowCount";
this.__Nb(p,function(q){this._onRowCountLoaded(parseInt(q));
});
},__Na:function(r,s){var t=f+r+k+s;
this.__Nb(t,function(u){this._onRowDataLoaded(qx.lang.Json.parse(u));
});
},__Nb:function(v,w){var x=new qx.io.request.Xhr();
x.setUrl(e+v);
x.addListener(d,function(){w.call(this,x.getResponseText());
},this);
x.send();
},__Nc:function(){var self=this;
window.setTimeout(function(){self._onRowCountLoaded(1000000);
},0);
},__Nd:function(y,z){var self=this;
window.setTimeout(function(){var A=[];

for(var i=y;i<=z;i++){A.push({id:i,text:b+i+g+(new Date())});
}self._onRowDataLoaded(A);
},0);
}}});
})();
