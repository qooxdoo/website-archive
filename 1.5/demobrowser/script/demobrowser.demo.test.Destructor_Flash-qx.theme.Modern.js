...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Destructor_Flash","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Destructor_Flash-qx.theme.Modern.9df353ebb41c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="demobrowser.demo.test.Destructor_Flash",d="qx.core.BaseInit",c="engine.name",b="os.name",a="engine.version";
...
(function(){var j="demobrowser.demo.test.Destructor_Flash",h="300px",g="interval",f="height",e="TestFlash.swf",d="400px",c="width",b="div",a="FlashMovie";
qx.Class.define(j,{extend:qx.application.Native,members:{__dc:null,__Nf:0,main:function(){qx.application.Native.prototype.main.call(this);
this.__dc=[];
var k=new qx.event.Timer(5000);
k.addListener(g,this.runTest,this);
k.start();
this.runTest();
},runTest:function(){for(var i=0;i<5;i++){var m=qx.bom.Element.create(b);
qx.bom.element.Style.set(m,c,d);
qx.bom.element.Style.set(m,f,h);
document.body.appendChild(m);
var n=a+this.__Nf;
this.__Nf++;
qx.bom.Flash.create(m,{movie:e,id:n});
this.debug("created: "+n);
this.__dc.push(m);
}var o=qx.event.Timer.once(this.clear,this,2500);
o.start();
},clear:function(){var p=this.__dc;

for(var i=0,l=p.length;i<l;i++){var q=p[i].firstChild.id;
qx.bom.Flash.destroy(p[i]);
this.debug("cleaned: "+q);
...
(function(){var q="movie",p="=",o="100%",n="engine.name",m="Invalid parameter 'element'.",l="&",k="Invalid attribute 'id'.",j="mshtml",h="qx.debug",g="beforeunload",c="data",f="load",e="Invalid attribute 'movie'.",b="Invalid parameter 'attributes'.",a="qx.bom.Flash",d="undefined";
qx.Class.define(a,{statics:{_flashObjects:{},create:function(r,s,t,u,v){if(!v){v=window;
}if(qx.core.Environment.get(h)){qx.core.Assert.assertElement(r,m);
qx.core.Assert.assertMap(s,b);
qx.core.Assert.assertString(s.movie,e);
qx.core.Assert.assertString(s.id,k);

if(!qx.dom.Element.isInDom(r,v)){qx.log.Logger.warn(this,"The parent DOM element isn't in DOM! The External Interface doesn't work in IE!");
}}
if(!s.width){s.width=o;
}
if(!s.height){s.height=o;
}u=u?qx.lang.Object.clone(u):{};

if(!u[q]){u[q]=s.movie;
}s[c]=s.movie;
delete s.movie;
if(t){for(var name in t){if(typeof u.flashvars!=d){u.flashvars+=l+name+p+t[name];
}else{u.flashvars=name+p+t[name];
}}}var w=this.__FS(r,s,u,v);
this._flashObjects[s.id]=w;
return w;
},destroy:qx.core.Environment.select(n,{"mshtml":function(x,y){x=this.__FN(x);

if(x.readyState==4){this.__FO(x);
}else{if(!y){y=window;
}qx.bom.Event.addNativeListener(y,f,function(){qx.bom.Flash.__FO(x);
});
}},"default":function(z,A){z=this.__FN(z);

if(z.parentNode){z.parentNode.removeChild(z);
}delete this._flashObjects[z.id];
}}),__FN:function(B){if(!B){throw new Error("DOM element is null or undefined!");
}
...
},__FO:qx.core.Environment.select("engine.name",{"mshtml":qx.event.GlobalError.observeMethod(function(C){for(var i in C){if(typeof C[i]=="function"){C[i]=null;
}}
if(C.parentNode){C.parentNode.removeChild(C);
}delete this._flashObjects[C.id];
}),"default":null}),__FP:qx.event.GlobalError.observeMethod(function(){for(var D in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[D]);
}window.__FQ=function(){};
window.__FR=function(){};
qx.bom.Event.removeNativeListener(window,"beforeunload",qx.bom.Flash.__FP);
}),__FS:qx.core.Environment.select("engine.name",{"mshtml":function(E,F,G,H){G.movie=F.data;
delete F.data;
delete F.classid;
var I="";

for(name in G){I+='<param name="'+name+'" value="'+G[name]+'" />';
}if(F.id){E.innerHTML='<object id="'+F.id+'" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">'+I+'</object>';
delete F.id;
...
},"default":function(J,K,L,M){delete K.classid;
delete L.movie;
var O=qx.bom.Element.create("object",K,M);
O.setAttribute("type","application/x-shockwave-flash");
var N;

for(var name in L){N=qx.bom.Element.create("param",{},M);
N.setAttribute("name",name);
N.setAttribute("value",L[name]);
O.appendChild(N);
}J.appendChild(O);
return O;
}})},defer:function(P){if((qx.core.Environment.get(n)==j)){qx.bom.Event.addNativeListener(window,g,P.__FP);
}}});
})();
