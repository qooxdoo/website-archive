...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Tree.6cd09cdfe676.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="Windows (C:)",k="icon/22/places/user-desktop.png",j="Documents (D:)",h="Inbox",g="Desktop",f="root",d="Open mode:",c="Presets",b="Hide Root Node",a="Network",P="Control",O=" items",N="Sent",M="Files",L="Show Open Items",K="",J="Workspace",I="Enable drag selection",H="Selection: ",G="Junk #",s="demobrowser.demo.widget.Tree",t="Data",q="click",r="Show root open button",o="none",p="Enable multi selection",m="double click",n="Selection:",u="Show Selected Items",v="Root node:",y="Toggle Height",x="Edit",A="Show Items",z="Trash",C="changeSelection",B="mode",w="multi",F="single",E="changeValue",D="execute";qx.Class.define(s,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var T=new qx.ui.layout.HBox();T.set({spacing:20});var S=new qx.ui.container.Composite(T);this.getRoot().add(S);var Q=this.getTree();S.add(Q);var R=this.getCommandFrame(Q);S.add(R);}
,getTree:function(){var be=new qx.ui.tree.Tree().set({width:200,height:400});var bj=new qx.ui.tree.TreeFolder(f);bj.setOpen(true);be.setRoot(bj);var bh=new qx.ui.tree.TreeFolder(g);bh.setOpen(true);bh.setIcon(k);bj.add(bh);var Y=new qx.ui.tree.TreeFolder(M);var W=new qx.ui.tree.TreeFolder(J);var X=new qx.ui.tree.TreeFolder(a);var V=new qx.ui.tree.TreeFolder(z);bh.add(Y,W,X,V);var bf=new qx.ui.tree.TreeFile(l);var U=new qx.ui.tree.TreeFile(j);W.add(bf,U);var bi=new qx.ui.tree.TreeFolder(h);var ba=new qx.ui.tree.TreeFolder(c);var bd=new qx.ui.tree.TreeFolder(N);var bc=new qx.ui.tree.TreeFolder(z);for(var i=0;i<30;i++){bc.add(new qx.ui.tree.TreeFile(G+i));}
;var bg=new qx.ui.tree.TreeFolder(t);var bb=new qx.ui.tree.TreeFolder(x);bi.add(ba,bd,bc,bg,bb);bj.add(bi);return be;}
,getCommandFrame:function(bk){var bu=new qx.ui.groupbox.GroupBox(P);var bs=4;bu.setLayout(new qx.ui.layout.Grid(5,3));var bB=0;bu.add(new qx.ui.basic.Label(H).set({paddingTop:4}),{row:bB,column:0});var by=new qx.ui.form.TextField().set({readOnly:true});bu.add(by,{row:bB++,column:1});bk.addListener(C,function(e){var bC=e.getData();if(bC.length>0){if(this.getSelectionMode()===w){by.setValue(bC.length+O);}
else {by.setValue(bC[0].getLabel());}
;}
else {by.setValue(K);}
;}
);bu.add(new qx.ui.core.Spacer(bs,bs),{row:bB++,column:0});bu.add(new qx.ui.basic.Label(d),{row:bB,column:0});var bo={"click":q,"dblclick":m,"none":o};var bm=new qx.ui.form.RadioGroup();for(var bw in bo){var bn=new qx.ui.form.RadioButton(bo[bw]).set({value:bw==bk.getOpenMode()});bn.setUserData(B,bw);bm.add(bn);bu.add(bn,{row:bB++,column:1});}
;bm.addListener(C,function(e){bk.setOpenMode(e.getData()[0].getUserData(B));}
);bu.add(new qx.ui.core.Spacer(bs,bs),{row:bB++,column:0});bu.add(new qx.ui.basic.Label(n),{row:bB,column:0});var bq=new qx.ui.form.CheckBox(p);bu.add(bq,{row:bB++,column:1});bq.addListener(E,function(e){var bD=e.getData();bk.setSelectionMode(bD?w:F);}
);var br=new qx.ui.form.CheckBox(I);bu.add(br,{row:bB++,column:1});br.addListener(E,function(e){var bE=e.getData();bk.setDragSelection(bE);if(!bq.getValue()){bq.setValue(true);}
;}
);bu.add(new qx.ui.core.Spacer(bs,bs),{row:bB++,column:0});bu.add(new qx.ui.basic.Label(v),{row:bB,column:0});var bz=new qx.ui.form.CheckBox(b);bu.add(bz,{row:bB++,column:1});bz.addListener(E,function(e){bk.setHideRoot(e.getData());}
);var bp=new qx.ui.form.CheckBox(r);bu.add(bp,{row:bB++,column:1});bp.addListener(E,function(e){bk.setRootOpenClose(e.getData());}
);bu.add(new qx.ui.core.Spacer(bs,bs),{row:bB++,column:0});var bx=new qx.ui.form.Button(A);bu.add(bx,{row:bB++,column:1});bx.addListener(D,function(e){alert((""+bk.getItems()).replace(",","\n","g"));}
);var bl=new qx.ui.form.Button(L);bu.add(bl,{row:bB++,column:1});bl.addListener(D,function(e){alert((""+bk.getItems(true,false)).replace(",","\n","g"));}
);var bv=new qx.ui.form.Button(u);bu.add(bv,{row:bB++,column:1});bv.addListener(D,function(e){if(this.getSelectionMode()===F){alert(this.getSelection());}
else {alert(this.getSelection().toString().replace(",","\n","g"));}
;}
,bk);var bt=new qx.ui.form.Button(y);bu.add(bt,{row:bB++,column:1});var bA=true;bt.addListener(D,function(e){bk.setHeight(bA?600:400);bA=!bA;}
);return bu;}
}});}
)();
...
,getIcon:function(){return this.getChildControl(a).getIcon();}
}});}
)();
