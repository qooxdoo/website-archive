...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.TextField.15c4b0317cd4.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var t="monospace",s="Web 2.0",r="Toggle enabled",q="Send selection",p="Send end of selection",o="text\narea\nmonospace",n="Send content",m="20px sans-serif",l="Use the following buttons to interact with the ",k="changeValue",d="read only",j="Toggle live update",h="<b>first textfield</b> widget.",c="demobrowser.demo.widget.TextField",b="Send start of selection",g="text\narea",f="text\narea\nnowrap",i="max15",a="execute";qx.Class.define(c,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var I=new qx.ui.container.Composite(new qx.ui.layout.HBox());this.getRoot().add(I);var u=new qx.ui.layout.VBox();u.setSpacing(10);var J=new qx.ui.container.Composite(u);J.setPadding(20);I.add(J);var w=new qx.ui.form.TextField(i).set({maxLength:15});w.focus();w.addListener(k,function(e){this.debug("ChangeValue: "+e.getData());}
,this);J.add(w);var y=new qx.ui.form.TextField(s).set({font:qx.bom.Font.fromString(m),padding:6});J.add(y);var x=new qx.ui.form.TextField(d).set({readOnly:true});J.add(x);var A=new qx.ui.form.TextArea(g);J.add(A);var E=new qx.ui.form.TextArea(f);E.setWrap(false);J.add(E);var H=new qx.ui.form.TextArea(o);H.setFont(t);J.add(H);var C=new qx.ui.container.Composite(new qx.ui.layout.VBox(8));C.setPadding(20);var B=new qx.ui.form.Button(j);B.addListener(a,function(){w.toggleLiveUpdate();}
);C.add(B);var F=new qx.ui.form.Button(r);var G=false;F.addListener(a,function(){J.setEnabled(G);G=!G;}
);C.add(F);C.add(new qx.ui.core.Spacer(null,20));var K=l+h;var M=new qx.ui.basic.Label(K);M.setRich(true);C.add(M);var D=new qx.ui.form.Button(n);D.addListener(a,function(){this.debug("Sending content: "+w.getValue());}
);C.add(D);var z=new qx.ui.form.Button(q);z.setFocusable(false);z.setKeepFocus(true);z.addListener(a,function(){this.debug("Sending selection: "+w.getTextSelection());}
);C.add(z);var L=new qx.ui.form.Button(b);L.setFocusable(false);L.setKeepFocus(true);L.addListener(a,function(){this.debug("Sending selection: "+w.getTextSelectionStart());}
);C.add(L);var v=new qx.ui.form.Button(p);v.setFocusable(false);v.setKeepFocus(true);v.addListener(a,function(){this.debug("Sending selection: "+w.getTextSelectionEnd());}
);C.add(v);I.add(C);}
}});}
)();
