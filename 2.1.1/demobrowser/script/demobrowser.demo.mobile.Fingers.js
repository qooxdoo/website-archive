...
var envinfo = {"engine.name":"webkit","locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.mobile.Fingers","qx.mobile.emulatetouch":true,"qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.mobile.Fingers.47ed33378031.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="touchmove",k="black",j="yellow",h="qx.debug",g="0px",f="12px",d="demobrowser.demo.mobile.Fingers",c='Lucida Grande',b="touchstart",a="<b>This demo is supposed to be run in a WebKit-based browser.</b>",B="engine.name",A="red",z="webkit",y="<b>Use your fingers to move the dots</b>",x="blue",w="green",v="100%",u="30px",t="100px",s="innerHTML",q="20px",r="white",o="50px",p="absolute",m="px",n="div";qx.Class.define(d,{extend:qx.application.Native,members:{__Sd:null,__Se:null,main:function(){qx.application.Native.prototype.main.call(this);if(qx.core.Environment.get(h)){qx.log.appender.Native;qx.log.appender.Console;}
;var E={"width":v,"height":v,"backgroundColor":k,"margin":g};var J=new qx.html.Element(n,E);J.useElement(document.body);J.setRoot(true);if(qx.core.Environment.get(B)!=z){var H={"color":w,"position":p,"font-family":c,"font-size":f,"left":u,"top":q};var F=new qx.html.Element(n,H);J.add(F);F.setAttribute(s,a);return;}
;var D={"color":r,"position":p,"left":u,"top":q};var F=new qx.html.Element(n,D);J.add(F);F.setAttribute(s,y);var G=[x,A,w,r,j];for(var i=0;i<G.length;i++){var C={"backgroundColor":G[i],"width":t,"height":t,"position":p,"-moz-border-radius":o,"-webkit-border-radius":o,"border-radius":o,"top":((i+5)*30)+m,"left":((i+1)*150)+m};var I=new qx.html.Element(n,C);J.add(I);}
;this.__Sd=[];this.__Se=[];J.addListener(b,this._onTouchStart,this);J.addListener(l,this._onTouchMove,this);}
,_onTouchStart:function(e){var L=e.getAllTouches();for(var i=0;i<L.length;i++){var K=L[i];this.__Sd[i]=parseInt(K.target.style.left)-K.pageX;this.__Se[i]=parseInt(K.target.style.top)-K.pageY;}
;}
,_onTouchMove:function(e){var N=e.getAllTouches();for(var i=0;i<N.length;i++){var M=N[i];if(N.target==document.body){continue;}
;qx.bom.element.Style.setStyles(M.target,{"left":(M.pageX+this.__Sd[i])+m,"top":(M.pageY+this.__Se[i])+m});}
;e.stop();}
}});}
)();
