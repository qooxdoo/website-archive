...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.StackContainer.77408d5add70.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var r="resize",q="demobrowser.demo.widget.StackContainer",p="yellow",o="teal",n="gray",m="red",l="Next",k="Previous",j="green",h="icon/22/actions/go-next.png",c="changeSelection",g="icon/22/actions/go-previous.png",f="orange",b="main",a="blue",d="execute";qx.Class.define(q,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);this.addSimpleStack();this.addDynamicStack();}
,addSimpleStack:function(){var u=new qx.ui.container.Stack();u.setDecorator(b);u.setWidth(200);u.setHeight(120);this.getRoot().add(u,{left:20,top:20});var s=new qx.ui.container.Composite((new qx.ui.layout.HBox).set({spacing:4}));var x=new qx.ui.form.Button(k,g);var v=new qx.ui.form.Button(l,h);s.add(x);s.add(v);this.getRoot().add(s,{left:20,top:150});x.addListener(d,u.previous,u);v.addListener(d,u.next,u);var w=[m,n,a,f,o,p,j];var t;for(var i=0;i<w.length;i++){t=new qx.ui.core.Widget;t.setBackgroundColor(w[i]);u.add(t);}
;u.addListener(c,function(e){this.debug("Selected: "+e.getData()[0].getBackgroundColor());}
);}
,addDynamicStack:function(){var A=new qx.ui.container.Stack();A.setDecorator(b);A.setDynamic(true);this.getRoot().add(A,{left:250,top:20});var y=new qx.ui.container.Composite((new qx.ui.layout.HBox).set({spacing:4}));var D=new qx.ui.form.Button(k,g);var B=new qx.ui.form.Button(l,h);y.add(D);y.add(B);this.getRoot().add(y,{left:250,top:150});D.addListener(d,A.previous,A);B.addListener(d,A.next,A);var C=[m,n,a,f,o,p,j];var z;for(var i=0;i<C.length;i++){z=new qx.ui.core.Widget;z.setBackgroundColor(C[i]);z.setWidth((Math.round(i/2)+1)*50);z.setHeight((Math.round(i/3)+1)*40);A.add(z);}
;A.addListener(c,function(e){var E=e.getData()[0];this.debug("Selected: "+E.getBackgroundColor()+" ("+E.getWidth()+"x"+E.getHeight()+")");}
);A.addListener(r,function(e){var F=this.getBounds();this.debug("Resize to: "+F.width+"x"+F.height);}
);}
}});}
)();
