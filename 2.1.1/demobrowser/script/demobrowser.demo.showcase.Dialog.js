...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.showcase.Dialog.86fb19d63c6f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="qx.event.type.Data",k="barRight",j="beforeChangeSelection",h="close-button",g="tabview",f="vertical",d="_applyBarPosition",c="barLeft",b="horizontal",a="qx.ui.tabview.TabView",C="barTop",B="barBottom",A="__rI",z="bottom",y="button",x="qx.debug",w="changeSelection",v="top",u="left",t="close",r="right",s="page",p="firstTab",q="bar",n="lastTab",o="pane";qx.Class.define(a,{extend:qx.ui.core.Widget,implement:qx.ui.core.ISingleSelection,include:[qx.ui.core.MContentPadding],construct:function(D){qx.ui.core.Widget.call(this);this.__rH={top:C,right:k,bottom:B,left:c};this._createChildControl(q);this._createChildControl(o);var E=this.__rI=new qx.ui.form.RadioGroup;E.setWrap(false);E.addListener(w,this._onChangeSelection,this);if(D!=null){this.setBarPosition(D);}
else {this.initBarPosition();}
;}
,events:{"changeSelection":m},properties:{appearance:{refine:true,init:g},barPosition:{check:[u,r,v,z],init:v,apply:d}},members:{__rI:null,_createChildControlImpl:function(F,G){var H;switch(F){case q:H=new qx.ui.container.SlideBar();H.setZIndex(10);this._add(H);break;case o:H=new qx.ui.container.Stack;H.setZIndex(5);this._add(H,{flex:1});break;};return H||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,F);}
...
;}
;var J=I.getButton();var K=this.getChildControl(q);var M=this.getChildControl(o);I.exclude();K.add(J);M.add(I);this.__rI.add(J);I.addState(this.__rH[this.getBarPosition()]);I.addState(n);var L=this.getChildren();if(L[0]==I){I.addState(p);}
...
;var P=N.getButton();var Q=this.getChildControl(q);var S=this.getChildControl(o);N.exclude();Q.addAt(P,O);S.addAt(N,O);this.__rI.add(P);N.addState(this.__rH[this.getBarPosition()]);R=this.getChildren();if(O==R.length-1){N.addState(n);}
...
;}
;W.remove(V);Y.remove(T);this.__rI.remove(V);T.removeState(this.__rH[this.getBarPosition()]);if(T.hasState(p)){T.removeState(p);if(X[0]){X[0].addState(p);}
;}
...
,__rH:null,_applyBarPosition:function(bb,bc){var bd=this.getChildControl(q);var bi=bb==u||bb==r;var bg=bb==r||bb==z;var bh=bi?qx.ui.layout.HBox:qx.ui.layout.VBox;var bk=this._getLayout();if(bk&&bk instanceof bh){}
else {this._setLayout(bk=new bh);}
;bk.setReversed(bg);bd.setOrientation(bi?f:b);var bj=this.getChildren();if(bc){var be=this.__rH[bc];bd.removeState(be);for(var i=0,l=bj.length;i<l;i++){bj[i].removeState(be);}
;}
;if(bb){var bf=this.__rH[bb];bd.addState(bf);for(var i=0,l=bj.length;i<l;i++){bj[i].addState(bf);}
;}
;}
,getSelection:function(){var bl=this.__rI.getSelection();var bm=[];for(var i=0;i<bl.length;i++){bm.push(bl[i].getUserData(s));}
;return bm;}
,setSelection:function(bn){var bo=[];for(var i=0;i<bn.length;i++){bo.push(bn[i].getChildControl(y));}
;this.__rI.setSelection(bo);}
,resetSelection:function(){this.__rI.resetSelection();}
,isSelected:function(bp){var bq=bp.getChildControl(y);return this.__rI.isSelected(bq);}
,isSelectionEmpty:function(){return this.__rI.isSelectionEmpty();}
,getSelectables:function(br){var bs=this.__rI.getSelectables(br);var bt=[];for(var i=0;i<bs.length;i++){bt.push(bs[i].getUserData(s));}
;return bt;}
...
;}
,_onPageClose:function(e){var bB=e.getTarget();var bA=bB.getButton().getChildControl(h);bA.reset();this.remove(bB);}
},destruct:function(){this._disposeObjects(A);this.__rH=null;}
});}
)();
