...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Selection.ed2786015e10.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="default",j="Additive Selection",i="Cell Line Selection",h="addaptive",g="Options",f="Row Selection",d="changeSelection",c="Column Selection",b="Enable drag selection",a="demobrowser.demo.virtual.Selection",F="Selection Mode",E="Single Selection",D="Cell Rectangle Selection",C="Enable quick selection",B="One Selection",A="Multi Selection",z="changeValue",y="table-row-background-selected",x="additive",w="one",r="single",s="selected",p="#EEE",q="horizontal",n="table-scroller-header",o="value",l="white",m=" / ",t="bold",u="multi",v="manager";qx.Class.define(a,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var K=qx.theme.manager.Font.getInstance().resolve(k).getStyles();this._fontCss=qx.bom.element.Style.compile(K);var T=new qx.ui.layout.Grid(5,0);T.setRowFlex(2,1);T.setColumnFlex(0,1);T.setColumnFlex(1,1);T.setColumnFlex(2,1);T.setColumnFlex(3,1);var H=[];var R=new qx.ui.container.Composite(T);this.getRoot().add(R,{edge:5});R.add(new qx.ui.basic.Label(f).set({font:t,decorator:n,padding:3,allowGrowX:true}),{row:1,column:0});var J=this.createRowSelectionScroller();H.push(J.getUserData(v));R.add(J,{row:2,column:0});R.add(new qx.ui.basic.Label(c).set({font:t,decorator:n,padding:3,allowGrowX:true}),{row:1,column:1});var J=this.createColumnSelectionScroller();H.push(J.getUserData(v));R.add(J,{row:2,column:1});R.add(new qx.ui.basic.Label(D).set({font:t,decorator:n,padding:3,allowGrowX:true}),{row:1,column:2});var J=this.createCellRectangleSelectionScroller();H.push(J.getUserData(v));R.add(J,{row:2,column:2});R.add(new qx.ui.basic.Label(i).set({font:t,decorator:n,padding:3,allowGrowX:true}),{row:1,column:3});var J=this.createCellLineSelectionScroller();H.push(J.getUserData(v));R.add(J,{row:2,column:3});var P=new qx.ui.layout.Grid(20,4);var M=new qx.ui.container.Composite(P).set({padding:15,backgroundColor:l});R.add(M,{row:0,column:0,colSpan:4});M.add(new qx.ui.basic.Label(F).set({font:t,padding:3}),{row:0,column:0});var L=new qx.ui.form.RadioButton(E);L.setUserData(o,r);M.add(L,{row:1,column:0});var O=new qx.ui.form.RadioButton(A);O.setUserData(o,u);O.setValue(true);M.add(O,{row:2,column:0});var I=new qx.ui.form.RadioButton(j);I.setUserData(o,x);M.add(I,{row:3,column:0});var G=new qx.ui.form.RadioButton(B);G.setUserData(o,w);M.add(G,{row:4,column:0});var N=new qx.ui.form.RadioGroup(L,O,I,G);N.addListener(d,function(e){var U=e.getData()[0].getUserData(o);H.forEach(function(V){V.setMode(U);}
);if(U==r||U==w){S.setEnabled(false);Q.setEnabled(true);}
else if(U==u||U==h){S.setEnabled(true);Q.setEnabled(false);}
;}
);M.add(new qx.ui.basic.Label(g).set({font:t,padding:3}),{row:0,column:1});var S=new qx.ui.form.CheckBox(b);M.add(S,{row:1,column:1});S.addListener(z,function(e){if(e.getData()){var W=H[0].getMode();if(W==r||W==w){this.debug("Drag selection is only available for the modes multi or additive");}
;}
;H.forEach(function(X){X.setDrag(e.getData());}
);}
);var Q=new qx.ui.form.CheckBox(C).set({enabled:false});M.add(Q,{row:2,column:1});Q.addListener(z,function(e){if(e.getData()){var Y=H[0].getMode();if(Y==u||Y==x){this.debug("Quick selection is only available for the modes multi or additive");}
;}
;H.forEach(function(ba){ba.setQuick(e.getData());}
);}
);}
,createRowSelectionScroller:function(){var bf=new qx.ui.virtual.core.Scroller(1000,100,20,100);var bd=new qx.ui.virtual.layer.Row(l,p);bf.getPane().addLayer(bd);bf.getPane().addLayer(new qx.ui.virtual.layer.GridLines(q));var be=new qx.ui.virtual.cell.Cell();var bc=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bg,bh){var bi={};if(bb.isItemSelected(bg)){bi.selected=true;}
;return be.getCellProperties(bg+m+bh,bi);}
});bf.getPane().addLayer(bc);var bb=new qx.ui.virtual.selection.Row(bf.getPane(),{styleSelectable:function(bj,bk,bl){if(bk!==s){return;}
;if(bl){bd.setBackground(bj,s);}
else {bd.setBackground(bj,null);}
;bc.updateLayerData();}
});bb.attachMouseEvents(bf.getPane());bb.attachKeyEvents(bf);bb.set({mode:u});bb.addItem(0);bf.setUserData(v,bb);return bf;}
,createColumnSelectionScroller:function(){var bp=new qx.ui.virtual.core.Scroller(1000,100,20,100);bp.getPane().addLayer(new qx.ui.virtual.layer.Row(l,p));var bo=new qx.ui.virtual.layer.Column();bp.getPane().addLayer(bo);bp.getPane().addLayer(new qx.ui.virtual.layer.GridLines(q));var bq=new qx.ui.virtual.cell.Cell();var bn=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(br,bs){var bt={};if(bm.isItemSelected(bs)){bt.selected=true;}
;return bq.getCellProperties(br+m+bs,bt);}
});bp.getPane().addLayer(bn);var bm=new qx.ui.virtual.selection.Column(bp.getPane(),{styleSelectable:function(bu,bv,bw){if(bv!==s){return;}
;if(bw){bo.setBackground(bu,s);}
else {bo.setBackground(bu,null);}
;bn.updateLayerData();}
});bm.attachMouseEvents(bp.getPane());bm.attachKeyEvents(bp);bm.set({mode:u});bm.addItem(0);bp.setUserData(v,bm);return bp;}
,createCellRectangleSelectionScroller:function(){var bA=new qx.ui.virtual.core.Scroller(1000,100,20,100);bA.getPane().addLayer(new qx.ui.virtual.layer.Row(l,p));var bz=new qx.ui.virtual.cell.Cell();var bB=new qx.ui.virtual.cell.Cell().set({backgroundColor:y});var by=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bC,bD){var bE={};if(bx.isItemSelected({row:bC,column:bD})){bE.selected=true;}
;if(bE.selected){return bB.getCellProperties(bC+m+bD,bE);}
else {bz.resetBackgroundColor();return bz.getCellProperties(bC+m+bD,bE);}
;}
});bA.getPane().addLayer(by);bA.getPane().addLayer(new qx.ui.virtual.layer.GridLines(q));var bx=new qx.ui.virtual.selection.CellRectangle(bA.getPane(),{styleSelectable:function(bF,bG,bH){by.updateLayerData();}
});bx.attachMouseEvents(bA.getPane());bx.attachKeyEvents(bA);bx.set({mode:u});bx.addItem({row:0,column:0});bA.setUserData(v,bx);return bA;}
,createCellLineSelectionScroller:function(){var bL=new qx.ui.virtual.core.Scroller(1000,100,20,100);bL.getPane().addLayer(new qx.ui.virtual.layer.Row(l,p));var bK=new qx.ui.virtual.cell.Cell();var bM=new qx.ui.virtual.cell.Cell().set({backgroundColor:y});var bJ=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bN,bO){var bP={};if(bI.isItemSelected({row:bN,column:bO})){bP.selected=true;}
;if(bP.selected){return bM.getCellProperties(bN+m+bO,bP);}
else {bK.resetBackgroundColor();return bK.getCellProperties(bN+m+bO,bP);}
;}
});bL.getPane().addLayer(bJ);bL.getPane().addLayer(new qx.ui.virtual.layer.GridLines(q));var bI=new qx.ui.virtual.selection.CellLines(bL.getPane(),{styleSelectable:function(bQ,bR,bS){bJ.updateLayerData();}
});bI.attachMouseEvents(bL.getPane());bI.attachKeyEvents(bL);bI.set({mode:u});bI.addItem({row:0,column:0});bL.setUserData(v,bI);return bL;}
}});}
)();
