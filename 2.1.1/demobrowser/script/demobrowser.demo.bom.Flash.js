...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Flash.2e17025f18ec.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
;}
;var w=this.__IM(r,s,u,v);this._flashObjects[s.id]=w;return w;}
,destroy:qx.core.Environment.select(d,{"mshtml":function(x,y){x=this.__IH(x);if(x.readyState==4){this.__II(x);}
else {if(!y){y=window;}
;qx.bom.Event.addNativeListener(y,j,function(){qx.bom.Flash.__II(x);}
);}
;}
,"default":function(z,A){z=this.__IH(z);if(z.parentNode){z.parentNode.removeChild(z);}
;delete this._flashObjects[z.id];}
}),__IH:function(B){if(!B){throw new Error("DOM element is null or undefined!");}
...
;return B;}
,__II:qx.core.Environment.select("engine.name",{"mshtml":qx.event.GlobalError.observeMethod(function(C){for(var i in C){if(typeof C[i]=="function"){C[i]=null;}
;}
;if(C.parentNode){C.parentNode.removeChild(C);}
;delete this._flashObjects[C.id];}
),"default":null}),__IJ:qx.event.GlobalError.observeMethod(function(){for(var D in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[D]);}
;window.__IK=function(){}
;window.__IL=function(){}
;qx.bom.Event.removeNativeListener(window,"beforeunload",qx.bom.Flash.__IJ);}
),__IM:qx.core.Environment.select("engine.name",{"mshtml":function(E,F,G,H){G.movie=F.data;delete F.data;delete F.classid;var I="";for(name in G){I+='<param name="'+name+'" value="'+G[name]+'" />';}
...
})},defer:function(P){if((qx.core.Environment.get(d)==n)){qx.bom.Event.addNativeListener(window,l,P.__IJ);}
;}
});}
)();
