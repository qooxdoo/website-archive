...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.DragDrop.72f3e6f01f91.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="drag",k="TextArea Target",j="solid",h="Copy Even Items",g="Reorderable",f="qx.ui.form.ListItem",d="droprequest",c="#33508D",b="Source",a="__MB",C="demobrowser.demo.ui.DragDrop",B="Simple Target",A="Enable drag",z="dragchange",y="\n",x="dragend",w="icon/16/places/folder.png",v="dragstart",u="Item ",t="move",r="multi",s="copy",p="value",q="dragover",n="drop",o="items";qx.Class.define(C,{extend:qx.application.Standalone,members:{__MB:null,__SY:null,main:function(){qx.application.Standalone.prototype.main.call(this);var Q=this.getRoot();var I=new qx.ui.container.Scroll();var D=new qx.ui.layout.Basic();var G=new qx.ui.container.Composite(D).set({allowStretchY:false,allowStretchX:false});I.add(G);Q.add(I,{edge:0});var P=new qx.ui.basic.Label(b);G.add(P,{left:20,top:20});var N=new qx.ui.form.List;N.setDraggable(true);N.setSelectionMode(r);G.add(N,{left:20,top:40});for(var i=0;i<20;i++){N.add(new qx.ui.form.ListItem(u+i,w));}
;var R=new qx.ui.form.CheckBox(A);R.setValue(true);G.add(R,{left:20,top:260});N.addListener(v,function(e){if(!R.isValue()){e.preventDefault();}
;e.addType(p);e.addType(o);e.addAction(s);e.addAction(t);}
);N.addListener(d,function(e){this.debug("Related of droprequest: "+e.getRelatedTarget());var U=e.getCurrentAction();var W=e.getCurrentType();var T;switch(W){case o:T=this.getSelection();if(U==s){var V=[];for(var i=0,l=T.length;i<l;i++){V[i]=T[i].clone();}
;T=V;}
;break;case p:T=this.getSelection()[0].getLabel();break;};if(U==t){var S=this.getSelection();for(var i=0,l=S.length;i<l;i++){this.remove(S[i]);}
;}
;e.addData(W,T);}
);var J=new qx.ui.basic.Label(B);G.add(J,{left:140,top:20});var H=new qx.ui.form.List;H.setDroppable(true);H.setSelectionMode(r);G.add(H,{left:140,top:40});H.addListener(n,function(e){this.debug("Related of drop: "+e.getRelatedTarget());var X=e.getData(o);for(var i=0,l=X.length;i<l;i++){this.add(X[i]);}
;}
);H.addListener(q,function(e){if(!e.supportsType(o)){e.preventDefault();}
;}
);var O=new qx.ui.basic.Label(h);G.add(O,{left:260,top:20});var M=new qx.ui.form.List;M.setDroppable(true);G.add(M,{left:260,top:40});M.addListener(n,function(e){this.debug("Related of drop: "+e.getRelatedTarget());var Y=e.getData(o);for(var i=0,l=Y.length;i<l;i++){this.add(Y[i]);}
;}
);M.addListener(q,function(e){if(!e.supportsType(o)){e.preventDefault();}
;if(e.getRelatedTarget().getSelection()[0].getLabel().substr(5)%2==1){e.preventDefault();}
;}
);M.addListener(z,function(e){if(e.getCurrentAction()!=s){e.preventDefault();}
;}
);var J=new qx.ui.basic.Label(k);G.add(J,{left:380,top:20});var K=new qx.ui.form.TextArea;K.setDroppable(true);K.setHeight(100);G.add(K,{left:380,top:40});K.addListener(n,function(e){this.debug("Related of drop: "+e.getRelatedTarget());this.setValue(e.getData(p).replace(/,/g,y));}
);K.addListener(q,function(e){if(!e.supportsType(p)){e.preventDefault();}
;}
);var L=new qx.ui.basic.Label(g);G.add(L,{left:600,top:20});var E=this.__MB=new qx.ui.form.List;E.setDraggable(true);E.setDroppable(true);E.setSelectionMode(r);G.add(E,{left:600,top:40});for(var i=0;i<20;i++){E.add(new qx.ui.form.ListItem(u+i,w));}
;var F=new qx.ui.core.Widget;F.setDecorator(new qx.ui.decoration.Single().set({top:[1,j,c]}));F.setHeight(0);F.setOpacity(0.5);F.setZIndex(100);F.setLayoutProperties({left:-1000,top:-1000});F.setDroppable(true);this.getRoot().add(F);E.addListener(v,function(e){e.addAction(t);}
);E.addListener(x,function(e){F.setDomPosition(-1000,-1000);}
);E.addListener(m,function(e){var bb=e.getOriginalTarget();if(bb instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__SY=bb;}
;if(!qx.ui.core.Widget.contains(this,bb)&&bb!=F){return;}
;var ba=bb.getContainerLocation();F.setWidth(bb.getBounds().width);F.setDomPosition(ba.left,ba.top);}
);E.addListener(q,function(e){if(e.getRelatedTarget()){e.preventDefault();}
;}
);E.addListener(n,function(e){this.__Ta(e.getOriginalTarget());}
,this);F.addListener(n,function(e){this.__Ta(this.__SY);}
,this);}
,__Ta:function(bc){if(bc.classname!=f){return;}
;var bd=this.__MB.getSortedSelection();for(var i=0,l=bd.length;i<l;i++){this.__MB.addBefore(bd[i],bc);this.__MB.addToSelection(bd[i]);}
;}
