...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.showcase.Translation.f1eebaae48c1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="MessageId:",k="<b>Locale and language:</b>",j="Choose Locale:",h='Territory: ',g="en",f="Simple Translate: this.tr('Hello World')",d='Locale: ',c="Add new Locale<br> at runtime",b="en_QX",a="A comment: 'The mammal fox that is.'<br>is added in the .po file",be="Translation",bd="Singular/Plural Translate: this.trn('I have an apple', I have %1 apples, ",bc="changeLocale",bb="qx.debug",ba="Number of apples",Y="show Translation",X="Locale Name (ex: en_QX)",W='bottom',V="Mark the string 'Orange' for later<br>translation so we do not forget it",U='top',s="demo",t="Add new Locale",q='Territory',r="Simple messageId translation",o="Mark the string 'Orange' for later translation so we do not forget it: this.marktr('Orange')",p=", applesNumber)",m="<b>Dynamic translation. This is added at runtime.</b>",n="<b>Static translation [found in .po files]. Hover the labels below to see more info.</b>",w="Add Translation",x='Language',F="Singular/Plural Translate<br>Choose a number from the select box<br>to see plural translation",D="with .po comment: this.trc('The mammal fox that is.', 'The quick brown fox')",M='Language: ',H="<i>MessageId</i>",Q="Number of apples:",O='Locale',z="click 'Show Translation' button to translate<br>the messageId in the current selected locale",T="<i>Translated Text</i>",S="demobrowser.demo.showcase.Translation",R="MessageId",y="The mammal fox that is.",B="changeSelection",C="Add translation<br>at runtime for: <b>",E="</b>",G="I have %1 apples",I='right',N="The quick brown fox",P="Orange",u="execute",v="Hello World",A="I have an apple",L="value",K="",J='middle';qx.Class.define(S,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);if((qx.core.Environment.get(bb))){qx.log.appender.Native;qx.log.appender.Console;}
;var bu=new qx.ui.container.Composite(new qx.ui.layout.Grid(8,8));this.getRoot().add(bu,{top:20,left:20});var bA=new qx.ui.basic.Label(k);bA.setRich(true);bu.add(bA,{row:0,column:0,colSpan:3});var bT=new qx.ui.basic.Label(j);bT.setMarginTop(5);bT.setAlignY(J);bu.add(bT,{row:1,column:0});var bC=qx.locale.Manager.getInstance();var bL=bC.getAvailableLocales().sort();var bs=bC.getLocale();var br=new qx.ui.form.SelectBox();br.setAllowGrowY(false);br.setAlignY(J);var bB=null;for(var i=0;i<bL.length;i++){var bI=new qx.ui.form.ListItem(bL[i]);br.add(bI);if((!bB&&bL[i]==g)||bL[i]==bs){bB=bI;}
;}
;br.addListener(B,function(e){var bX=e.getData()[0].getLabel();qx.locale.Manager.getInstance().setLocale(bX);}
);bu.add(br,{row:1,column:1});var bU=new qx.ui.basic.Label(O);bU.setMarginTop(5);bU.setWidth(80);var bR=new qx.ui.basic.Label(q);bR.setMarginTop(5);bR.setWidth(80);var bS=new qx.ui.basic.Label(x);bS.setMarginTop(5);bS.setWidth(80);var bm=new qx.ui.container.Composite(new qx.ui.layout.VBox(3));bm.add(bU);bm.add(bR);bm.add(bS);bu.add(bm,{row:1,column:2});qx.locale.Manager.getInstance().addListener(bc,function(){var bY=qx.locale.Manager.getInstance();bU.setValue(d+bY.getLocale());bR.setValue(h+bY.getTerritory());bS.setValue(M+bY.getLanguage());bf.apply(this);bW.setValue(C+bC.getLocale()+E);}
,this);function bf(){bo.setValue(this.tr(v));bo.setToolTipText(f);var ca=parseInt(bP.getSelection()[0].getLabel());bn.setValue(this.trn(A,G,ca,ca));bn.setToolTipText(bd+ca+p);bE.setValue(this.trc(y,N));bE.setToolTipText(D);bl.setValue(this.marktr(P));bl.setToolTipText(o);var cb=bN.getValue();if(cb&&cb.length>0){bD.setValue(bC.translate(cb,[],bC.getLocale()));}
;}
;var bO=new qx.ui.basic.Label(n);bO.setRich(true);bO.setMarginTop(15);bu.add(bO,{row:2,column:0,colSpan:4});var bg=new qx.ui.basic.Label(H);bg.setRich(true);bu.add(bg,{row:3,column:1});var bz=new qx.ui.basic.Label(T);bz.setRich(true);bu.add(bz,{row:3,column:2});var bG=new qx.ui.basic.Label(v);bu.add(bG,{row:4,column:1});var bo=new qx.ui.basic.Label(this.tr(v));bu.add(bo,{row:4,column:2});var bQ=new qx.ui.basic.Label(r);bu.add(bQ,{row:4,column:3});var bq=new qx.ui.basic.Label(Q);bq.setAlignX(I);bq.setAlignY(W);bu.add(bq,{row:5,column:0});var bP=new qx.ui.form.SelectBox();for(var i=1;i<5;i++){var bp=new qx.ui.form.ListItem(i+K);bP.add(bp);}
;bP.setToolTipText(ba);bP.setAlignY(U);bP.setAlignX(I);bu.add(bP,{row:6,column:0});var bh=new qx.ui.basic.Label(A);bh.setAlignY(J);bu.add(bh,{row:5,column:1,rowSpan:2});var bn=new qx.ui.basic.Label(this.trn(A,G,1,1));bn.setAlignY(J);bu.add(bn,{row:5,column:2,rowSpan:2});var bK=new qx.ui.basic.Label(F);bK.setAlignY(J);bK.setRich(true);bu.add(bK,{row:5,column:3,rowSpan:2});bP.addListener(B,function(e){bf.apply(this);}
);var bt=new qx.ui.basic.Label(N);bu.add(bt,{row:7,column:1});var bE=new qx.ui.basic.Label(this.trc(y,N));bE.setWidth(140);bu.add(bE,{row:7,column:2});var bH=new qx.ui.basic.Label(a);bH.setRich(true);bu.add(bH,{row:7,column:3});var bw=new qx.ui.basic.Label(P);bu.add(bw,{row:8,column:1});var bl=new qx.ui.basic.Label(this.marktr(P));bu.add(bl,{row:8,column:2});var bv=new qx.ui.basic.Label(V);bv.setRich(true);bu.add(bv,{row:8,column:3});var bj=new qx.ui.basic.Label(m);bj.setRich(true);bj.setMarginTop(15);bu.add(bj,{row:9,column:0,colSpan:3});var bW=new qx.ui.basic.Label(C+bC.getLocale()+E);bW.setRich(true);bW.setWidth(120);bu.add(bW,{row:10,column:0});var bi=new qx.ui.form.TextField(K);bi.setToolTipText(R);bu.add(bi,{row:10,column:1});var bF=new qx.ui.form.TextField(K);bF.setToolTipText(be);bu.add(bF,{row:10,column:2});var bx=new qx.ui.form.Button(w);bx.setAllowGrowY(false);bu.add(bx,{row:10,column:3});bx.addListener(u,function(e){var cc={};cc[bi.getValue()]=bF.getValue();bC.addTranslation(bC.getLocale(),cc);alert(bF.getValue()+" has been added for messageId: "+bi.getValue()+" for locale: "+bC.getLocale());}
,this);var by=new qx.ui.basic.Label(l);bu.add(by,{row:11,column:0});var bN=new qx.ui.form.TextField(K);bN.setToolTipText(R);bu.add(bN,{row:11,column:1});var bM=new qx.ui.form.Button(Y);bu.add(bM,{row:11,column:2});bM.setAllowGrowY(false);bM.addListener(u,function(e){var cd=bN.getValue();bD.setValue(bC.translate(cd,[],bC.getLocale()));}
,this);var bD=new qx.ui.basic.Label(z);bD.setRich(true);bD.setAlignY(J);bu.add(bD,{row:11,column:3});var bV=new qx.ui.basic.Label(c);bV.setRich(true);bu.add(bV,{row:12,column:0});var bk=new qx.ui.form.TextField(K);bk.setPlaceholder(b);bk.setToolTipText(X);bu.add(bk,{row:12,column:1});var bJ=new qx.ui.form.Button(t);bJ.setAllowGrowY(false);bu.add(bJ,{row:12,column:2});bJ.addListener(u,function(e){var ce=bk.getValue();if(ce!=null&&ce.length<1){alert("new Locale must not be empty string");return;}
;var cf=bC.getAvailableLocales();if(cf.indexOf(ce)!=-1){alert(ce+" already exists");}
else {bC.addTranslation(ce,{"demo":s});alert(ce+" has been added");br.add(new qx.ui.form.ListItem(ce));}
;}
,this);if(bB){br.setSelection([bB]);}
;bi.bind(L,bN,L);bN.bind(L,bi,L);return;}
}});}
)();
