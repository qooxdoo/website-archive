...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Decorator_Box.6e0f4f288dec.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var s="sliceBottom",r="horizontal",q="qx.debug.dispose",p="qx.ui.decoration.AbstractBox",o="-l",n="qx.debug",m="sliceLeft",l="_applyBaseImage",k="-b",j="sliceRight",d="_applyFill",i="shorthand",g="-t",c="-r",b="String",f="sliceTop",e="set",h="Number",a="_applySlices";qx.Class.define(p,{extend:qx.ui.decoration.Abstract,implement:[qx.ui.decoration.IDecorator],construct:function(t,u,v){qx.ui.decoration.Abstract.call(this);this._setOrientation(v);if(qx.ui.decoration.css3.BorderImage.IS_SUPPORTED){this.__BY=new qx.ui.decoration.css3.BorderImage();if(t){this.__Ca(t,v);}
;if(u!=null){this.__BY.setInsets(u);}
;}
else {this.__BY=new qx.ui.decoration.BoxDiv(t,u,v);}
;if(qx.core.Environment.get(q)){this.__BY.$$ignoreDisposeWarning=true;}
;}
,properties:{baseImage:{check:b,nullable:true,apply:l},sliceLeft:{check:h,nullable:true,apply:a},sliceRight:{check:h,nullable:true,apply:a},sliceBottom:{check:h,nullable:true,apply:a},sliceTop:{check:h,nullable:true,apply:a},slices:{group:[f,j,s,m],mode:i},fill:{apply:d}},members:{__BY:null,_isHorizontal:null,_setOrientation:function(w){this._isHorizontal=w==r;}
,getMarkup:function(){return this.__BY.getMarkup();}
,resize:function(x,y,z){this.__BY.resize(x,y,z);}
,tint:function(A,B){}
,getInsets:function(){return this.__BY.getInsets();}
,_applyInsets:function(C,D,name){var E=e+qx.lang.String.firstUp(name);this.__BY[E](C);}
,_applySlices:function(F,G,name){var H=e+qx.lang.String.firstUp(name);if(this.__BY[H]){this.__BY[H](F);}
;}
,_applyFill:function(I,J,name){if(this.__BY.setFill){this.__BY.setFill(I);}
;}
,_applyBaseImage:function(K,L){if(this.__BY instanceof qx.ui.decoration.BoxDiv){this.__BY.setBaseImage(K);}
else {this.__Ca(K);}
;}
,__Ca:function(M){this.__BY.setBorderImage(M);var S=qx.util.AliasManager.getInstance().resolve(M);var T=/(.*)(\.[a-z]+)$/.exec(S);var Q=T[1];var R=T[2];var P=qx.util.ResourceManager.getInstance();if(qx.core.Environment.get(n)){if(!P.has(S)){this.warn("The border image '"+S+"' is not available!");}
;}
;if(this._isHorizontal){var V=P.getImageWidth(Q+o+R);var N=P.getImageWidth(Q+c+R);if(N&&V){this.__BY.setSlice([0,N,0,V]);}
;}
else {var O=P.getImageHeight(Q+k+R);var U=P.getImageHeight(Q+g+R);if(U&&O){this.__BY.setSlice([U,0,O,0]);}
;}
;}
},destruct:function(){this.__Cc=this.__Cs=this.__Ct=null;this.__BY.dispose();}
});}
)();
...
(function(){var j="-l",i='</div>',h="-t",g="browser.quirksmode",f="-r",e='<div style="position:absolute;top:0;left:0;overflow:hidden;font-size:0;line-height:0;">',d="qx.debug",c="_applyBaseImage",b="-b",a="repeat-y",w="String",v="",u="qx.ui.decoration.BoxDiv",t="-c",s="mshtml",r="engine.name",q="horizontal",p="repeat-x",o="engine.version",n="0",l="-1px",m="no-repeat",k="px";qx.Class.define(u,{extend:qx.ui.decoration.Abstract,construct:function(x,y,z){qx.ui.decoration.Abstract.call(this);this._setOrientation(z);if(x!=null){this.setBaseImage(x);}
;if(y!=null){this.setInsets(y);}
;}
,properties:{baseImage:{check:w,nullable:true,apply:c}},members:{__Cc:null,__Cs:null,__Ct:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};}
,_isInitialized:function(){return !!this.__Cc;}
,_setOrientation:function(A){this._isHorizontal=A==q;}
,getMarkup:function(){if(this.__Cc){return this.__Cc;}
;var B=qx.bom.element.Decoration;var C=this.__Cs;var D=this.__Ct;var E=[];E.push(e);if(this._isHorizontal){E.push(B.create(C.l,m,{top:0,left:0}));E.push(B.create(C.c,p,{top:0,left:D.left+k}));E.push(B.create(C.r,m,{top:0,right:0}));}
else {E.push(B.create(C.t,m,{top:0,left:0}));E.push(B.create(C.c,a,{top:D.top+k,left:D.left+k}));E.push(B.create(C.b,m,{bottom:0,left:0}));}
;E.push(i);return this.__Cc=E.join(v);}
,resize:function(F,G,H){F.style.width=G+k;F.style.height=H+k;var I=this.__Ct;if(this._isHorizontal){var innerWidth=G-I.left-I.right;innerWidth=innerWidth<0?0:innerWidth;F.childNodes[1].style.width=innerWidth+k;}
else {var innerHeight=H-I.top-I.bottom;innerHeight=innerHeight<0?0:innerHeight;F.childNodes[1].style.height=innerHeight+k;}
;if((qx.core.Environment.get(r)==s)){if(parseFloat(qx.core.Environment.get(o))<7||(qx.core.Environment.get(g)&&parseFloat(qx.core.Environment.get(o))<8)){if(this._isHorizontal){F.childNodes[2].style.marginRight=(G%2==1)?l:n;}
else {F.childNodes[2].style.marginBottom=(H%2==1)?l:n;}
;}
;}
;}
,tint:function(J,K){}
,_applyBaseImage:function(L,M){if(qx.core.Environment.get(d)){if(this.__Cc){throw new Error("This decorator is already in-use. Modification is not possible anymore!");}
;}
;var N=qx.util.ResourceManager.getInstance();if(L){var P=qx.util.AliasManager.getInstance();var R=P.resolve(L);var S=/(.*)(\.[a-z]+)$/.exec(R);var Q=S[1];var O=S[2];var T=this.__Cs={t:Q+h+O,b:Q+b+O,c:Q+t+O,l:Q+j+O,r:Q+f+O};this.__Ct={top:N.getImageHeight(T.t),bottom:N.getImageHeight(T.b),left:N.getImageWidth(T.l),right:N.getImageWidth(T.r)};}
;}
},destruct:function(){this.__Cc=this.__Cs=this.__Ct=null;}
});}
)();
