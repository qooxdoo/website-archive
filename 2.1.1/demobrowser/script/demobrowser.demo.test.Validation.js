...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Validation.e63cd7565620.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j="demobrowser.demo.test.Validation",i="<b>Setting the age ",h="property to 27</b><br>",g="- name: Name is validated to a minimum length of two.<br/>",f="<br>old name: ",e="age:",d="__SR",c="fmale",b="<br><b>Setting the name to 'Hans'</b><br>",a="property to 'female'</b><br>",M="<br><b>Setting the gender ",L="<b>Testing the gender ",K="<br>old gender: ",J="<br><b>Setting the age ",I="- age: Age is validated to be a number.<br/>",H="<b>Initial property values</b><br>",G="",F="<b>Setting the name to ''</b><br>",E="demobrowser.demo.test.Person",D="property to 'true'</b><br>",q="- gender: Gender is validated to be either male or female.",r="property to 'fmale'</b><br>",o="<font color='red'>GENDER NOT SET</font>",p="gender:",m="<br>old age: ",n="Hans",k="The demo creates an internal class containing three properties:<br/>",l="<font color='red'>AGE NOT SET</font>",s="My name",t="<font color='red'>NAME NOT SET</font>",w="white",v="name:",y="<b>Description</b><br/>",x="new name: ",A="new gender: ",z="male",u="female",C="new age: ",B="<br>";qx.Class.define(j,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var N=new demobrowser.demo.test.Person();var O=new qx.ui.embed.Html();O.setBackgroundColor(w);O.set({padding:10,width:250,height:360});this.getRoot().add(O,{left:10,top:10});O.setHtml(H);O.setHtml(O.getHtml()+v+N.getName()+B);O.setHtml(O.getHtml()+e+N.getAge()+B);O.setHtml(O.getHtml()+p+N.getGender()+B);O.setHtml(O.getHtml()+b);N.setName(n);O.setHtml(O.getHtml()+x+N.getName()+B);O.setHtml(O.getHtml()+F);try{N.setName(G);O.setHtml(O.getHtml()+x+N.getName()+B);}
catch(Q){O.setHtml(O.getHtml()+t+f+N.getName()+B);}
;O.setHtml(O.getHtml()+J+h);N.setAge(27);O.setHtml(O.getHtml()+C+N.getAge()+B);O.setHtml(O.getHtml()+i+D);try{N.setAge(true);O.setHtml(O.getHtml()+C+N.getAge()+B);}
catch(R){O.setHtml(O.getHtml()+l+m+N.getAge()+B);}
;O.setHtml(O.getHtml()+M+a);N.setGender(u);O.setHtml(O.getHtml()+A+N.getGender()+B);O.setHtml(O.getHtml()+L+r);try{N.setAge(c);O.setHtml(O.getHtml()+A+N.getGender()+B);}
catch(S){O.setHtml(O.getHtml()+o+K+N.getGender()+B);}
;var P=new qx.ui.basic.Label();P.setRich(true);P.setWidth(250);P.setValue(y+k+g+I+q);this.getRoot().add(P,{left:280,top:10});}
}});qx.Class.define(E,{extend:qx.core.Object,properties:{name:{init:s,validate:d},age:{init:18,validate:qx.util.Validate.number()},gender:{init:z,validate:qx.util.Validate.inArray([z,u])}},members:{__SR:function(T){if(T.length<2){throw new qx.core.ValidationError("Validation Error: Name "+"must be longer than one characters. ("+T+")");}
;}
}});}
)();
