...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.mobile.Fingers.f9d30624ad32.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="touchmove",b="black",c="px",d="green",f="div",g="100%",h="yellow",j="30px",k="qx.debug",l="0px",m="12px",n="<b>This demo is intended for WebKit-based browsers and IE11+.</b>",o="browser.documentmode",p="100px",q="demobrowser.demo.mobile.Fingers",r='Lucida Grande',s="50px",t="innerHTML",u="touchstart",v="mshtml",w="engine.name",x="20px",y="red",z="webkit",A="white",B="<b>Use your fingers to move the dots</b>",C="blue",D="absolute";qx.Class.define(q,{extend:qx.application.Native,members:{__RT:null,__RU:null,main:function(){qx.application.Native.prototype.main.call(this);if(qx.core.Environment.get(k)){qx.log.appender.Native;qx.log.appender.Console;}
;var I={"width":g,"height":g,"backgroundColor":b,"margin":l};var N=new qx.html.Element(f,I);N.useElement(document.body);N.setRoot(true);var E=qx.core.Environment.get(w);var G=E==v&&qx.core.Environment.get(o)>10;if(E!=z&&!G){var L={"color":d,"position":D,"font-family":r,"font-size":m,"left":j,"top":x};var J=new qx.html.Element(f,L);N.add(J);J.setAttribute(t,n);return;}
;var H={"color":A,"position":D,"left":j,"top":x};var J=new qx.html.Element(f,H);N.add(J);J.setAttribute(t,B);var K=[C,y,d,A,h];for(var i=0;i<K.length;i++ ){var F={"backgroundColor":K[i],"width":p,"height":p,"position":D,"-moz-border-radius":s,"-webkit-border-radius":s,"border-radius":s,"top":((i+5)*30)+c,"left":((i+1)*150)+c};var M=new qx.html.Element(f,F);N.add(M);}
;this.__RT=[];this.__RU=[];N.addListener(u,this._onTouchStart,this);N.addListener(a,this._onTouchMove,this);}
,_onTouchStart:function(e){var P=e.getAllTouches();for(var i=0;i<P.length;i++ ){var O=P[i];this.__RT[i]=parseInt(O.target.style.left)-O.pageX;this.__RU[i]=parseInt(O.target.style.top)-O.pageY;}
;}
,_onTouchMove:function(e){var R=e.getAllTouches();for(var i=0;i<R.length;i++ ){var Q=R[i];if(R.target==document.body){continue;}
;qx.bom.element.Style.setStyles(Q.target,{"left":(Q.pageX+this.__RT[i])+c,"top":(Q.pageY+this.__RU[i])+c});}
;e.preventDefault();}
}});}
)();
