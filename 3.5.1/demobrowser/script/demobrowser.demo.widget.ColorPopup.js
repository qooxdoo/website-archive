...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.ColorPopup.508927bc6ac1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var bn=this._tables[this.__KO].values;var bo=this.__KK[this.__KO];if(!bn){return;}
;var bp=this.getValue();if(!bp){return;}
;var bm=bn.indexOf(bp);if(bm!=-1){qx.lang.Array.removeAt(bn,bm);}
else if(bn.length==this.__KP){bn.shift();}
...
;this.__KM.setRed(bx);this.__KM.setGreen(bw);this.__KM.setBlue(bv);this.__KL.open();}
,_onColorSelectorOk:function(){var by=this.__KM;this.setValue(qx.util.ColorUtil.rgbToRgbString([by.getRed(),by.getGreen(),by.getBlue()]));this.__KL.close();}
,_onColorSelectorCancel:function(){this.__KL.close();}
...
;}
,_tables:{core:{label:qx.locale.Manager.tr(M),values:[m,h,f,J,w,F,V,U,t,n,b,c]},recent:{label:qx.locale.Manager.tr(R),values:[]}}},destruct:function(){if(this.__KL){this.__KL.destroy();this.__KM.destroy();this.__KN.destroy();}
;this._tables=this.__KK=null;}
});}
)();
...
,setValue:function(de){var dd;if(de==null){this.__nG=true;dd=[255,255,255];}
else {dd=qx.util.ColorUtil.stringToRgb(de);this.__nG=false;}
;this.__Fx=true;this.setRed(dd[0]);this.setGreen(dd[1]);this.__Fx=false;this.setBlue(dd[2]);}
,getValue:function(){return this.__nG?null:qx.util.ColorUtil.rgbToHexString([this.getRed(),this.getGreen(),this.getBlue()]);}
,resetValue:function(){this.__nG=true;this.__Fx=true;this.setRed(255);this.setGreen(255);this.__Fx=false;this.setBlue(255);}
,__nI:function(){if(!this.__Fx){this.__nG=false;this.fireDataEvent(f,this.getValue());}
;}
,_applyRed:function(dg,df){if(this.__Fr===null){this.__Fr=ci;}
;if(this.__Fr!==cM){this.getChildControl(n).setValue(dg);}
;if(this.__Fr!==o){this._setHexFromRgb();}
;switch(this.__Fr){case cM:case o:case ci:this._setHueFromRgb();};this._setPreviewFromRgb();this.__nI();if(this.__Fr===ci){this.__Fr=null;}
;}
,_applyGreen:function(di,dh){if(this.__Fr===null){this.__Fr=ct;}
;if(this.__Fr!==cM){this.getChildControl(U).setValue(di);}
;if(this.__Fr!==o){this._setHexFromRgb();}
;switch(this.__Fr){case cM:case o:case ct:this._setHueFromRgb();};this._setPreviewFromRgb();this.__nI();if(this.__Fr===ct){this.__Fr=null;}
;}
,_applyBlue:function(dk,dj){if(this.__Fr===null){this.__Fr=d;}
;if(this.__Fr!==cM){this.getChildControl(cm).setValue(dk);}
;if(this.__Fr!==o){this._setHexFromRgb();}
;switch(this.__Fr){case cM:case o:case d:this._setHueFromRgb();};this._setPreviewFromRgb();this.__nI();if(this.__Fr===d){this.__Fr=null;}
;}
,_applyHue:function(dm,dl){if(this.__Fr===null){this.__Fr=cB;}
;if(this.__Fr!==J){this.getChildControl(cu).setValue(dm);}
;if(this.__Fr!==W){if(this.getChildControl(cb).getBounds()){this.getChildControl(cb).setDomLeft(Math.round(dm/1.40625)+this.getChildControl(cK).getPaddingLeft());}
else {this.getChildControl(cb).setLayoutProperties({left:Math.round(dm/1.40625)});}
;}
;switch(this.__Fr){case J:case W:case cB:this._setRgbFromHue();};this._setBrightnessGradiant();if(this.__Fr===cB){this.__Fr=null;}
;}
,_applySaturation:function(dp,dn){if(this.__Fr===null){this.__Fr=t;}
;if(this.__Fr!==J){this.getChildControl(cA).setValue(dp);}
;if(this.__Fr!==W){this._setBrightnessGradiant();if(this.getChildControl(cb).getBounds()){this.getChildControl(cb).setDomTop(256-Math.round(dp*2.56)+this.getChildControl(cK).getPaddingTop());}
else {this.getChildControl(cb).setLayoutProperties({top:256-Math.round(dp*2.56)});}
;}
;switch(this.__Fr){case J:case W:case t:this._setRgbFromHue();};if(this.__Fr===t){this.__Fr=null;}
;}
,_applyBrightness:function(ds,dr){if(this.__Fr===null){this.__Fr=c;}
;if(this.__Fr!==J){this.getChildControl(ch).setValue(ds);}
;if(this.__Fr!==V){var dq=256-Math.round(ds*2.56);if(this.getChildControl(cl).getBounds()){this.getChildControl(cl).setDomTop(dq+this.getChildControl(cE).getPaddingTop());}
else {this.getChildControl(cl).setLayoutProperties({top:dq});}
;}
;switch(this.__Fr){case J:case V:case c:this._setRgbFromHue();};if(this.__Fr===c){this.__Fr=null;}
;}
,_onBrightnessHandleMouseDown:function(e){this.getChildControl(cl).capture();this.__Ft=cl;var dv=this.getChildControl(cd).getContentLocation();var du=this.getChildControl(cl).getContentLocation();var dt=this.getChildControl(cd).getBounds();this.__Fu=dv.top+(e.getDocumentTop()-du.top)-dt.top;e.stopPropagation();}
,_onBrightnessHandleMouseUp:function(e){this.getChildControl(cl).releaseCapture();this.__Ft=null;}
,_onBrightnessHandleMouseMove:function(e){if(this.__Ft===cl){this._setBrightnessOnFieldEvent(e);e.stopPropagation();}
;}
,_onBrightnessFieldMouseDown:function(e){var location=this.getChildControl(cd).getContentLocation();var dw=this.getChildControl(cl).getBounds();this.__Fu=location.top+(dw.height/2);this._setBrightnessOnFieldEvent(e);this.getChildControl(cl).capture();this.__Ft=cl;}
...
;e.stop();}
,_setBrightnessOnFieldEvent:function(e){var dx=qx.lang.Number.limit(e.getDocumentTop()-this.__Fu,0,256);this.__Fr=V;if(this.getChildControl(cl).getBounds()){this.getChildControl(cl).setDomTop(dx);}
else {this.getChildControl(cl).setLayoutProperties({top:dx});}
;this.setBrightness(100-Math.round(dx/2.56));this.__Fr=null;}
,_onHueSaturationHandleMouseUp:function(e){if(this.__Ft){e.stopPropagation();this.getChildControl(cb).releaseCapture();this.__Ft=null;}
;}
,_onHueSaturationHandleMouseMove:function(e){if(this.__Ft===cb){this._setHueSaturationOnFieldEvent(e);e.stopPropagation();}
;}
,_onHueSaturationFieldMouseDown:function(e){var location=this.getChildControl(bY).getContentLocation();var dy=this.getChildControl(cb).getBounds();var dz=this.getChildControl(bY).getBounds();this.__Fv=location.top+(dy.height/2)-dz.top;this.__Fw=location.left+(dy.width/2)-dz.left;this._setHueSaturationOnFieldEvent(e);this.getChildControl(cb).capture();this.__Ft=cb;}
...
;e.stop();}
,_setHueSaturationOnFieldEvent:function(e){var dB=qx.lang.Number.limit(e.getDocumentTop()-this.__Fv,0,256);var dA=qx.lang.Number.limit(e.getDocumentLeft()-this.__Fw,0,256);this.getChildControl(cb).setDomPosition(dA,dB);this.__Fr=W;this.setSaturation(100-Math.round(dB/2.56));this.setHue(Math.round(dA*1.40625));this.__Fr=null;}
,_setRedFromSpinner:function(){if(this.__Fr!==null){return;}
;this.__Fr=cM;this.setRed(this.getChildControl(n).getValue());this.__Fr=null;}
,_setGreenFromSpinner:function(){if(this.__Fr!==null){return;}
;this.__Fr=cM;this.setGreen(this.getChildControl(U).getValue());this.__Fr=null;}
,_setBlueFromSpinner:function(){if(this.__Fr!==null){return;}
;this.__Fr=cM;this.setBlue(this.getChildControl(cm).getValue());this.__Fr=null;}
,_setHueFromSpinner:function(){if(this.__Fr!==null){return;}
;this.__Fr=J;this.setHue(this.getChildControl(cu).getValue());this.__Fr=null;}
,_setSaturationFromSpinner:function(){if(this.__Fr!==null){return;}
;this.__Fr=J;this.setSaturation(this.getChildControl(cA).getValue());this.__Fr=null;}
,_setBrightnessFromSpinner:function(){if(this.__Fr!==null){return;}
;this.__Fr=J;this.setBrightness(this.getChildControl(ch).getValue());this.__Fr=null;}
,_onHexFieldChange:function(e){if(this.__Fr!==null){return;}
;try{var dD=this.getChildControl(cJ);var dC=qx.util.ColorUtil.hexStringToRgb(N+dD.getValue());}
catch(dE){return;}
;this.__Fr=o;this.setRed(dC[0]);this.setGreen(dC[1]);this.setBlue(dC[2]);this.__Fr=null;}
...
,_setHueFromRgb:function(){switch(this.__Fr){case J:case W:case V:break;default:var dI=qx.util.ColorUtil.rgbToHsb([this.getRed(),this.getGreen(),this.getBlue()]);this.setHue(dI[0]);this.setSaturation(dI[1]);this.setBrightness(dI[2]);};}
,_setRgbFromHue:function(){switch(this.__Fr){case cM:case o:break;default:var dJ=qx.util.ColorUtil.hsbToRgb([this.getHue(),this.getSaturation(),this.getBrightness()]);this.setRed(dJ[0]);this.setGreen(dJ[1]);this.setBlue(dJ[2]);};}
