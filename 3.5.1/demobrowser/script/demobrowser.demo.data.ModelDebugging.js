...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ModelDebugging.34ede1067308.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="url: ",b="qx.debug.io",c="qx.bom.request.Script",d="Invalid state",e="head",f="error",g="loadend",h="qx.debug",i="script",j="load",k="Unknown response headers",l="browser.documentmode",m="abort",n="",o="Received native readyState: loaded",p="readystatechange",q="Response header cannot be determined for ",r="requests made with script transport.",s="opera",t="unknown",u="Open native request with ",v="Response headers cannot be determined for",w="mshtml",x="engine.name",y="Detected error",z="Send native request",A="on",B="timeout",C="Unknown environment key at this phase",D="Received native load";qx.Bootstrap.define(c,{construct:function(){this.__Bb();this.__AV=qx.Bootstrap.bind(this._onNativeLoad,this);this.__AW=qx.Bootstrap.bind(this._onNativeError,this);this.__tN=qx.Bootstrap.bind(this._onTimeout,this);this.__AX=document.head||document.getElementsByTagName(e)[0]||document.documentElement;this._emitter=new qx.event.Emitter();this.timeout=this.__Bd()?0:15000;}
,events:{"readystatechange":c,"error":c,"loadend":c,"timeout":c,"abort":c,"load":c},members:{readyState:null,status:null,statusText:null,timeout:null,__AY:null,on:function(name,E,F){this._emitter.on(name,E,F);return this;}
,open:function(H,G){if(this.__uh){return;}
;this.__Bb();this.__tQ=null;this.__uc=G;if(this.__Bg(b)){qx.Bootstrap.debug(qx.bom.request.Script,u+a+G);}
;this._readyStateChange(1);}
,setRequestHeader:function(I,J){if(this.__uh){return null;}
;var K={};if(this.readyState!==1){throw new Error(d);}
;K[I]=J;this.__uc=qx.util.Uri.appendParamsToUrl(this.__uc,K);return this;}
,send:function(){if(this.__uh){return null;}
;var M=this.__Be(),L=this.__AX,N=this;if(this.timeout>0){this.__eN=window.setTimeout(this.__tN,this.timeout);}
;if(this.__Bg(b)){qx.Bootstrap.debug(qx.bom.request.Script,z);}
;L.insertBefore(M,L.firstChild);window.setTimeout(function(){N._readyStateChange(2);N._readyStateChange(3);}
);return this;}
,abort:function(){if(this.__uh){return null;}
;this.__tQ=true;this.__Bf();this._emit(m);return this;}
...
,onabort:function(){}
,ontimeout:function(){}
,getResponseHeader:function(O){if(this.__uh){return null;}
;if(this.__Bg(h)){qx.Bootstrap.debug(q+r);}
;return t;}
,getAllResponseHeaders:function(){if(this.__uh){return null;}
;if(this.__Bg(h)){qx.Bootstrap.debug(v+r);}
;return k;}
,setDetermineSuccess:function(P){this.__AY=P;}
,dispose:function(){var Q=this.__Ba;if(!this.__uh){if(Q){Q.onload=Q.onreadystatechange=null;this.__Bf();}
;if(this.__eN){window.clearTimeout(this.__eN);}
;this.__uh=true;}
;}
,isDisposed:function(){return !!this.__uh;}
,_getUrl:function(){return this.__uc;}
,_getScriptElement:function(){return this.__Ba;}
,_onTimeout:function(){this.__Bc();if(!this.__Bd()){this._emit(f);}
;this._emit(B);if(!this.__Bd()){this._emit(g);}
;}
,_onNativeLoad:function(){var S=this.__Ba,R=this.__AY,T=this;if(this.__tQ){return;}
;if(this.__Bg(x)===w&&this.__Bg(l)<9){if(!(/loaded|complete/).test(S.readyState)){return;}
else {if(this.__Bg(b)){qx.Bootstrap.debug(qx.bom.request.Script,o);}
;}
;}
;if(this.__Bg(b)){qx.Bootstrap.debug(qx.bom.request.Script,D);}
;if(R){if(!this.status){this.status=R()?200:500;}
;}
;if(this.status===500){if(this.__Bg(b)){qx.Bootstrap.debug(qx.bom.request.Script,y);}
;}
;if(this.__eN){window.clearTimeout(this.__eN);}
;window.setTimeout(function(){T._success();T._readyStateChange(4);T._emit(j);T._emit(g);}
);}
,_onNativeError:function(){this.__Bc();this._emit(f);this._emit(g);}
,__Ba:null,__AX:null,__uc:n,__AV:null,__AW:null,__tN:null,__eN:null,__tQ:null,__uh:null,__Bb:function(){this.readyState=0;this.status=0;this.statusText=n;}
,_readyStateChange:function(U){this.readyState=U;this._emit(p);}
,_success:function(){this.__Bf();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=n+this.status;}
,__Bc:function(){this.__Bf();this.readyState=4;this.status=0;this.statusText=null;}
,__Bd:function(){var W=this.__Bg(x)===w&&this.__Bg(l)<9;var V=this.__Bg(x)===s;return !(W||V);}
,__Be:function(){var X=this.__Ba=document.createElement(i);X.src=this.__uc;X.onerror=this.__AW;X.onload=this.__AV;if(this.__Bg(x)===w&&this.__Bg(l)<9){X.onreadystatechange=this.__AV;}
;return X;}
,__Bf:function(){var Y=this.__Ba;if(Y&&Y.parentNode){this.__AX.removeChild(Y);}
;}
,__Bg:function(ba){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(ba);}
...
(function(){var a="qx.bom.request.Jsonp",b="callback",c="open",d="dispose",e="",f="qx",g="qx.debug",h="Expecting JavaScript response to call: ",i="_onNativeLoad",j=" already exists",k="Callback ",l="qx.debug.io",m=".callback",n="qx.bom.request.Jsonp.";qx.Bootstrap.define(a,{extend:qx.bom.request.Script,construct:function(){qx.bom.request.Script.apply(this);this.__uC();}
,members:{responseJson:null,__ci:null,__uv:null,__uw:null,__ux:null,__uy:null,__uz:null,__uh:null,__uA:e,open:function(t,p){if(this.__uh){return;}
;var r={},q,s,o=this;this.responseJson=null;this.__ux=false;q=this.__uv||b;s=this.__uw||this.__uA+n+this.__ci+m;if(!this.__uw){this.constructor[this.__ci]=this;}
...
;r[q]=s;this.__uz=p=qx.util.Uri.appendParamsToUrl(p,r);this.__gw(c,[t,p]);}
,callback:function(v){if(this.__uh){return;}
;this.__ux=true;if(qx.core.Environment.get(g)){v=qx.lang.Json.stringify(v);v=qx.lang.Json.parse(v);}
;this.responseJson=v;this.constructor[this.__ci]=undefined;this.__uB();}
,setCallbackParam:function(w){this.__uv=w;return this;}
,setCallbackName:function(name){this.__uw=name;return this;}
,setPrefix:function(x){this.__uA=x;}
,getGeneratedUrl:function(){return this.__uz;}
,dispose:function(){this.__uB();this.__gw(d);}
,_onNativeLoad:function(){this.status=this.__ux?200:500;this.__gw(i);}
,__uB:function(){if(this.__uy&&window[this.__uw]){window[this.__uw]=undefined;this.__uy=false;}
;}
,__gw:function(z,y){qx.bom.request.Script.prototype[z].apply(this,y||[]);}
,__uC:function(){this.__ci=f+(new Date().valueOf())+(e+Math.random()).substring(2,5);}
}});}
)();
...
;return bm;}
;return y;}
,startDisposeProfiling:qx.core.Environment.select(c,{"true":function(){this.disposeProfilingActive=true;this.__RM=qx.core.ObjectRegistry.getNextHash();}
,"default":(function(){}
)}),stopDisposeProfiling:qx.core.Environment.select(c,{"true":function(bv){if(!this.__RM){qx.log.Logger.error(r+this.classname+B);return [];}
;this.disposeProfilingActive=false;var bu=[];while(!qx.ui.core.queue.Dispose.isEmpty()){qx.ui.core.queue.Dispose.flush();}
;var bw=qx.core.ObjectRegistry.getNextHash();var by=qx.core.ObjectRegistry.getPostId();var bz=qx.core.ObjectRegistry.getStackTraces();for(var bx=this.__RM;bx<bw;bx++ ){var bt=qx.core.ObjectRegistry.fromHashCode(bx+by);if(bt&&bt.isDisposed&&!bt.isDisposed()){if(bv&&typeof bv==h&&!bv(bt)){continue;}
