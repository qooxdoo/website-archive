...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Dynamic.af87afe9f74d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,this);}
,getLookupTable:function(){return this.__zG;}
,_reverseLookup:function(bl){return bl;}
,_getDataFromRow:function(bm){return this.__zG.getItem(bm);}
,_getSelectables:function(){return this.__zG;}
,getOpenNodes:function(){return this.__FP;}
,isNode:function(bn){return qx.ui.tree.core.Util.isNode(bn,this.getChildProperty());}
,getLevel:function(bo){return this.__FQ[bo];}
...
,_applyModel:function(bM,bL){this.__FP=[];if(bM!=null){if(qx.core.Environment.get(w)){if(!qx.Class.hasMixin(bM.constructor,qx.data.marshal.MEventBubbling)){this.warn(F+h+k);}
;}
;bM.addListener(V,this._onChangeBubble,this);this.__FU(bM);}
...
;}
;if(this.__zG.indexOf(bR)!=-1){this.__FS();}
;}
;}
,_onUpdated:function(event){if(this.__fy==null){this.__fy=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__fy.schedule();}
,_onOpen:function(event){var bV=event.getRow();var bU=this.__zG.getItem(bV);if(this.isNode(bU)){if(this.isNodeOpen(bU)){this.closeNode(bU);}
...
;break;};}
;}
,_beforeApplySelection:function(cc){if(cc.length===0&&this.getSelectionMode()===g){var cb=this.__FX();var cd=this.getLookupTable().indexOf(cb);if(cd>=0){cc.push(cd);}
;}
;}
,_afterApplySelection:function(){var ce=this.getSelection();if(ce.getLength()>0&&this.getSelectionMode()===g){this.__FW(ce.getItem(0));}
else {this.__FR=[];}
;}
,__FS:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(Q+f);}
;this._itemWidth=0;var ci=[];this.__FQ=[];var cg=-1;var ch=this.getModel();if(ch!=null){if(!this.isHideRoot()){cg++ ;ci.push(ch);this.__FQ.push(cg);}
;if(this.isNodeOpen(ch)){var cf=this.__FT(ch,cg);ci=ci.concat(cf);}
;}
;if(!qx.lang.Array.equals(this.__zG.toArray(),ci)){this._provider.removeBindings();this.__zG.removeAll();this.__zG.append(ci);this.__zM();this._updateSelection();}
;}
,__FT:function(cj,co){var cn=[];co++ ;if(!this.isNode(cj)){return cn;}
...
;if(this.isNode(cq)){this.__FQ.push(co);cn.push(cq);if(this.isNodeOpen(cq)){var ck=this.__FT(cq,co);cn=cn.concat(ck);}
;}
else {if(this.isShowLeafs()){this.__FQ.push(co);cn.push(cq);}
;}
;}
;cr.dispose();return cn;}
,__FU:function(cs){if(!qx.lang.Array.contains(this.__FP,cs)){this.__FP.push(cs);this.fireDataEvent(u,cs);}
;}
,__FV:function(ct,cu){if(ct===cu){this.__FU(cu);return true;}
;if(!this.isNode(ct)){return false;}
;var cx=ct.get(this.getChildProperty());if(cx==null){return false;}
;for(var i=0;i<cx.getLength();i++ ){var cw=cx.getItem(i);var cv=this.__FV(cw,cu);if(cv===true){this.__FU(cw);return true;}
;}
;return false;}
,__zM:function(){this.getPane().getRowConfig().setItemCount(this.__zG.getLength());this.getPane().fullUpdate();}
,getParent:function(cz){var cA=this.__zG.indexOf(cz);if(cA<0){return null;}
;var cB=this.__FQ[cA];while(cA>0){cA-- ;var cy=this.__FQ[cA];if(cy<cB){return this.__zG.getItem(cA);}
;}
;return null;}
,__FW:function(cC){this.__FR=[];var parent=this.getParent(cC);while(parent!=null){this.__FR.unshift(parent);parent=this.getParent(parent);}
;}
,__FX:function(){if(this.__FR==null){return this.getModel();}
;var cD=this.getLookupTable();var parent=this.__FR.pop();while(parent!=null){if(cD.contains(parent)){return parent;}
;parent=this.__FR.pop();}
...
;}
;if(!qx.core.ObjectRegistry.inShutDown&&this.__fy!=null){this.__fy.cancel();this.__fy.dispose();}
;var cE=this.getModel();if(cE!=null){cE.removeListener(V,this._onChangeBubble,this);}
;this._layer.removeListener(R,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__zG.dispose();this._layer=this._provider=this.__zG=this.__FP=this.__fy=null;}
});}
)();
...
(function(){var a="BindingIds",b="model",c="",d="changeDelegate",f="virtual-tree-file",g="label",h="appearance",i="icon",j="bindItem",k=".length",l="]",m="virtual-tree-folder",n="[",o=".",p="String",q="qx.ui.tree.core.MWidgetController";qx.Mixin.define(q,{construct:function(){this.__Aa=[];}
...
;}
,bindProperty:function(x,A,y,w,u){var z=this.__Ab(u,x);var v=this._tree.getLookupTable();var B=v.bind(z,w,A,y);this.__Ac(w,B);}
,bindPropertyReverse:function(I,E,G,H,C){var F=this.__Ab(C,I);var D=this._tree.getLookupTable();var J=H.bind(E,D,F,G);this.__Ac(H,J);}
,removeBindings:function(){while(this.__Aa.length>0){var K=this.__Aa.pop();this._removeBindingsFrom(K);}
;}
...
;}
,_removeBindingsFrom:function(O){var P=this.__Ad(O);while(P.length>0){var Q=P.pop();try{this._tree.getLookupTable().removeBinding(Q);}
catch(e){O.removeBinding(Q);}
;}
;if(qx.lang.Array.contains(this.__Aa,O)){qx.lang.Array.remove(this.__Aa,O);}
;}
,__Ab:function(S,T){var R=n+S+l;if(T!=null&&T!=c){R+=o+T;}
;return R;}
,__Ac:function(V,W){var U=this.__Ad(V);if(!qx.lang.Array.contains(U,W)){U.push(W);}
;if(!qx.lang.Array.contains(this.__Aa,V)){this.__Aa.push(V);}
;}
,__Ad:function(X){var Y=X.getUserData(a);if(Y==null){Y=[];X.setUserData(a,Y);}
;return Y;}
...
;var o=this._renderer.getCellWidget();o.setOpen(p&&this._tree.isNodeOpen(s));o.addListener(a,this.__FY,this);o.setUserData(b,this.getChildProperty());o.setUserData(g,this._tree.isShowLeafs());if(this._tree.getSelection().contains(s)){this._styleSelectabled(o);}
...
,poolCellWidget:function(u){u.removeListener(a,this.__FY,this);this._removeBindingsFrom(u);this._renderer.pool(u);this._onPool(u);}
...
,__FY:function(event){var K=event.getTarget();var L=K.getUserData(k);var J=this._tree.getLookupTable().getItem(L);if(event.getData()){this._tree.openNode(J);}
