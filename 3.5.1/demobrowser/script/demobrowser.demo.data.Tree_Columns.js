...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Tree_Columns.1e519edae0eb.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="open",b="abstract",c="__fn",d="__mK",e="qx.debug",f="'. It is not a child of this tree item.",g="addItem",h="visible",j="excluded",k="removeItem",m="Invalid child index: ",n="Cannot remove treeitem '",o="qx.ui.tree.core.AbstractTreeItem";qx.Class.define(o,{extend:qx.ui.tree.core.AbstractItem,type:b,construct:function(p){qx.ui.tree.core.AbstractItem.call(this,p);this.__fn=[];}
,properties:{parent:{check:o,nullable:true}},members:{__fn:null,__mK:null,getTree:function(){var r=this;while(r.getParent()){r=r.getParent();}
...
,hasChildren:function(){return this.__fn?this.__fn.length>0:false;}
...
;this.__fn.push(Q);if(!N){this.__mL();}
;if(P){Q.recursiveAddToWidgetQueue();P.fireNonBubblingEvent(g,qx.event.type.Data,[Q]);}
;}
;if(P){qx.ui.core.queue.Widget.add(this);}
;}
,addAt:function(X,T){if(qx.core.Environment.get(e)){this.assert(T<=this.__fn.length&&T>=0,m+T);}
;if(T==this.__fn.length){this.add(X);return;}
;var W=X.getParent();if(W){W.remove(X);}
;var U=this.getChildrenContainer();X.setParent(this);var V=this.hasChildren();var S=this.__fn[T];U.addBefore(X,S);if(X.hasChildren()){U.addAfter(X.getChildrenContainer(),X);}
;qx.lang.Array.insertAt(this.__fn,X,T);if(!V){this.__mL();}
;if(this.getTree()){X.recursiveAddToWidgetQueue();qx.ui.core.queue.Widget.add(this);}
;}
,addBefore:function(Y,bb){if(qx.core.Environment.get(e)){this.assert(this.__fn.indexOf(bb)>=0);}
;var ba=Y.getParent();if(ba){ba.remove(Y);}
;this.addAt(Y,this.__fn.indexOf(bb));}
,addAfter:function(bd,bc){if(qx.core.Environment.get(e)){this.assert(this.__fn.indexOf(bc)>=0);}
;var be=bd.getParent();if(be){be.remove(bd);}
;this.addAt(bd,this.__fn.indexOf(bc)+1);}
,addAtBegin:function(bf){this.addAt(bf,0);}
,remove:function(bk){for(var i=0,l=arguments.length;i<l;i++ ){var bj=arguments[i];if(this.__fn.indexOf(bj)==-1){this.warn(n+bj+f);return;}
...
;}
;qx.lang.Array.remove(this.__fn,bj);bj.setParent(null);bg.remove(bj);}
;var bh=this.getTree();if(bh){bh.fireNonBubblingEvent(k,qx.event.type.Data,[bj]);}
;qx.ui.core.queue.Widget.add(this);}
,removeAt:function(bl){var bm=this.__fn[bl];if(bm){this.remove(bm);}
;}
,removeAll:function(){var bn=this.__fn.concat();for(var i=this.__fn.length-1;i>=0;i-- ){this.remove(this.__fn[i]);}
;return bn;}
...
(function(){var a="Boolean",b="_applyOpenMode",c="excluded",d="Space",f="Left",g="Enter",h="dblclick",j="changeOpenMode",k="_applyRootOpenClose",l="visible",m="changeSelection",n="qx.ui.tree.Tree",o="qx.event.type.Data",p="qx.ui.tree.core.AbstractTreeItem",q="tree",r="__nt",s="_applyHideRoot",t="changeRoot",u="_applyRoot",v="click",w="keypress",x="none",y="pane",z="Right";qx.Class.define(n,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IModelSelection,qx.ui.form.IForm],include:[qx.ui.core.MMultiSelectionHandling,qx.ui.core.MContentPadding,qx.ui.form.MModelSelection,qx.ui.form.MForm],construct:function(){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__nt=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({allowShrinkY:false,allowGrowX:true});this.getChildControl(y).add(this.__nt);this.initOpenMode();this.initRootOpenClose();this.addListener(m,this._onChangeSelection,this);this.addListener(w,this._onKeyPress,this);}
,events:{addItem:o,removeItem:o},properties:{openMode:{check:[v,h,x],init:h,apply:b,event:j,themeable:true},root:{check:p,init:null,nullable:true,event:t,apply:u},hideRoot:{check:a,init:false,apply:s},rootOpenClose:{check:a,init:false,apply:k},appearance:{refine:true,init:q},focusable:{refine:true,init:true}},members:{__nt:null,SELECTION_MANAGER:qx.ui.tree.selection.SelectionManager,getChildrenContainer:function(){return this.__nt;}
...
,_getContentPaddingTarget:function(){return this.__nt;}
...
(function(){var a="changeModel",b="_applyDelegate",c="qx.core.Object",d="_applyLabelPath",f="icon",g="change",h="label",k="' twice.",l="_applyTarget",m="changeTarget",n="_applyIconPath",o="Can not reverse bind the same target property '",p="_applyLabelOptions",q="_applyChildPath",r="_applyIconOptions",s="qx.data.controller.Tree",t="String",u="Can not bind the same target property '",v="_applyModel",w="get";qx.Class.define(s,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(y,z,A,x){qx.core.Object.call(this);this.__bc={};this.__Bq=[];this.__KY={};if(A!=null){this.setChildPath(A);}
;if(x!=null){this.setLabelPath(x);}
;if(y!=null){this.setModel(y);}
;if(z!=null){this.setTarget(z);}
;}
,properties:{model:{check:c,apply:v,event:a,nullable:true,dereference:true},target:{apply:l,event:m,init:null,nullable:true,dereference:true},childPath:{check:t,apply:q,nullable:true},labelPath:{check:t,apply:d,nullable:true},iconPath:{check:t,apply:n,nullable:true},labelOptions:{apply:p,nullable:true},iconOptions:{apply:r,nullable:true},delegate:{apply:b,init:null,nullable:true}},members:{__KY:null,__bc:null,__Bq:null,__La:null,_applyDelegate:function(C,B){this._setConfigureItem(C,B);this._setCreateItem(C,B);this._setBindItem(C,B);}
,_applyIconOptions:function(E,D){this.__BB();}
,_applyLabelOptions:function(G,F){this.__BB();}
,_applyTarget:function(I,H){if(H!=undefined){this.__FK(H);}
;if(this.getModel()!=null){this.__Ld();}
;this._addChangeTargetListener(I,H);}
,_applyModel:function(K,J){this.__Ld();}
,_applyChildPath:function(M,L){this.__La=L;this.__Ld();this.__La=null;}
,_applyIconPath:function(O,N){this.__BB();}
,_applyLabelPath:function(Q,P){this.__Ld();}
,__Lb:function(R){var T=R.getTarget();var S=this.__KY[T.toHashCode()].treeNode;var U=this.__KY[T.toHashCode()].modelNode;this.__Le(S,U);this._updateSelection();}
,__Lc:function(e){var X=e.getData();var bc=e.getOldData();var V=this.__KY[bc.toHashCode()];bc.removeListenerById(V.changeListenerId);delete this.__KY[bc.toHashCode()];V.modelNode.removeListenerById(V.changeChildernListenerId);var bd=V.modelNode;var bb=qx.Class.getPropertyDefinition(V.modelNode.constructor,this.getChildPath());var Y=bb.event;var ba=bd.addListener(Y,this.__Lc,this);var W=V.treeNode;this.__KY[X.toHashCode()]={modelNode:bd,treeNode:W,changeListenerId:V.changeListenerId,changeChildernListenerId:ba};this.__Le(W,bd);this._updateSelection();}
...
;return be;}
,__Ld:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__FK();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.__Ac(this.getModel(),bg);this.__Le(bg,this.getModel());this.getTarget().setRoot(bg);}
;}
,__Le:function(bn,br){if(br[w+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bj=br[w+qx.lang.String.firstUp(this.getChildPath())]();if(this.__KY[bj.toHashCode()]==undefined){var bl=bj.addListener(g,this.__Lb,this);var bp=qx.Class.getPropertyDefinition(br.constructor,this.getChildPath());var bm=bp.event;var bq=br.addListener(bm,this.__Lc,this);this.__KY[bj.toHashCode()]={modelNode:br,treeNode:bn,changeListenerId:bl,changeChildernListenerId:bq};}
...
;}
else {var bo=this._createItem();bo.setModel(bj.getItem(i));bn.addAt(bo,i);this.__Ac(bj.getItem(i),bo);this.__Le(bo,bj.getItem(i));}
;}
;}
;for(var i=bn.getChildren().length-1;i>=bj.length;i-- ){var bk=bn.getChildren()[i];this.__Lg(bk,bn);}
;}
,__FK:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__Lf(bv);var bu=bv.getModel();if(bu){this.__sp(bu);}
;bv.destroy();this.__KY={};}
;}
,__Lf:function(bw){var bx=bw.getChildren()||[];for(var i=bx.length-1;i>=0;i-- ){if(bx[i].getChildren().length>0){this.__Lf(bx[i]);}
;this.__Lg(bx[i],bw);}
;}
,__Lg:function(bz,bC){var bD=bz.getModel();var by=this.__La||this.getChildPath();var bA=w+qx.lang.String.firstUp(by);if(bD[bA]!=undefined){var bE=bD[bA]();var bB=this.__KY[bE.toHashCode()];bE.removeListenerById(bB.changeListenerId);bD.removeListenerById(bB.changeChildernListenerId);this.__Lf(bz);delete this.__KY[bE.toHashCode()];}
;this.__sp(bD);bC.remove(bz);bz.destroy();}
...
;if(!qx.lang.Array.contains(this.__Bq,bJ)){this.__Bq.push(bJ);}
;}
...
;if(!qx.lang.Array.contains(this.__Bq,bM)){this.__Bq.push(bM);}
;}
...
;}
,__BB:function(){var bW;for(var ca in this.__bc){bW=ca;break;}
;for(var bX in this.__bc[bW]){var bV=this.__bc[bW][bX].treeNode;var bY=qx.core.ObjectRegistry.fromHashCode(bX);this.__sp(bY);this.__Ac(bY,bV);}
;}
,__Ac:function(cd,cb){var cc=this.getDelegate();if(cc!=null&&cc.bindItem!=null){cc.bindItem(this,cb,cd);}
else {this.bindDefaultProperties(cb,cd);}
;}
,__sp:function(cg){for(var i=0;i<this.__Bq.length;i++ ){var ce=this.__Bq[i];var cf=this.__bc[ce][cg.toHashCode()];if(cf!=null){if(cf.id){cg.removeBinding(cf.id);cf.id=null;}
...
;this._startSelectionModification();this.__FK();this.__Ld();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cn,cm){if(cn!=null&&cn.bindItem!=null){if(cm!=null&&cm.bindItem!=null&&cn.bindItem==cm.bindItem){return;}
;this.__Ld();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__bc=this.__KY=this.__Bq=null;}
});}
)();
