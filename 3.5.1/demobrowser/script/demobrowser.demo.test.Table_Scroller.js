...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Table_Scroller.db848e79eb4f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,properties:{selectionModel:{check:a}},members:{__oQ:null,handleMouseDown:function(c,d){if(d.isLeftPressed()){var e=this.getSelectionModel();if(!e.isSelectedIndex(c)){this._handleSelectEvent(c,d);this.__oQ=true;}
else {this.__oQ=false;}
;}
else if(d.isRightPressed()&&d.getModifiers()==0){var e=this.getSelectionModel();if(!e.isSelectedIndex(c)){e.setSelectionInterval(c,c);}
;}
;}
,handleMouseUp:function(f,g){if(g.isLeftPressed()&&!this.__oQ){this._handleSelectEvent(f,g);}
;}
...
(function(){var a="..",b="changeSelection",c="Try to turn off batch mode althoug it was not turned on.",d="_applySelectionMode",f="]",g="qx.event.type.Event",h="Ranges:",k="qx.ui.table.selection.Model",l=" [";qx.Class.define(k,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__oR=[];this.__oS=-1;this.__oT=-1;this.hasBatchModeRefCount=0;this.__oU=false;}
,events:{"changeSelection":g},statics:{NO_SELECTION:1,SINGLE_SELECTION:2,SINGLE_INTERVAL_SELECTION:3,MULTIPLE_INTERVAL_SELECTION:4,MULTIPLE_INTERVAL_SELECTION_TOGGLE:5},properties:{selectionMode:{init:2,check:[1,2,3,4,5],apply:d}},members:{__oU:null,__oS:null,__oT:null,__oR:null,_applySelectionMode:function(m){this.resetSelection();}
,setBatchMode:function(n){if(n){this.hasBatchModeRefCount+=1;}
else {if(this.hasBatchModeRefCount==0){throw new Error(c);}
;this.hasBatchModeRefCount-=1;if(this.__oU){this.__oU=false;this._fireChangeSelection();}
;}
;return this.hasBatchMode();}
,hasBatchMode:function(){return this.hasBatchModeRefCount>0;}
,getAnchorSelectionIndex:function(){return this.__oS;}
,_setAnchorSelectionIndex:function(o){this.__oS=o;}
,getLeadSelectionIndex:function(){return this.__oT;}
,_setLeadSelectionIndex:function(p){this.__oT=p;}
,_getSelectedRangeArr:function(){return this.__oR;}
,resetSelection:function(){if(!this.isSelectionEmpty()){this._resetSelection();this._fireChangeSelection();}
;}
,isSelectionEmpty:function(){return this.__oR.length==0;}
,getSelectedCount:function(){var r=0;for(var i=0;i<this.__oR.length;i++ ){var q=this.__oR[i];r+=q.maxIndex-q.minIndex+1;}
;return r;}
,isSelectedIndex:function(s){for(var i=0;i<this.__oR.length;i++ ){var t=this.__oR[i];if(s>=t.minIndex&&s<=t.maxIndex){return true;}
;}
;return false;}
,getSelectedRanges:function(){var u=[];for(var i=0;i<this.__oR.length;i++ ){u.push({minIndex:this.__oR[i].minIndex,maxIndex:this.__oR[i].maxIndex});}
;return u;}
,iterateSelection:function(w,v){for(var i=0;i<this.__oR.length;i++ ){for(var j=this.__oR[i].minIndex;j<=this.__oR[i].maxIndex;j++ ){w.call(v,j);}
;}
;}
...
,removeSelectionInterval:function(G,E){this.__oS=G;this.__oT=E;var D=Math.min(G,E);var H=Math.max(G,E);for(var i=0;i<this.__oR.length;i++ ){var J=this.__oR[i];if(J.minIndex>H){break;}
else if(J.maxIndex>=D){var K=(J.minIndex>=D)&&(J.minIndex<=H);var F=(J.maxIndex>=D)&&(J.maxIndex<=H);if(K&&F){this.__oR.splice(i,1);i-- ;}
else if(K){J.minIndex=H+1;}
else if(F){J.maxIndex=D-1;}
else {var I={minIndex:H+1,maxIndex:J.maxIndex};this.__oR.splice(i+1,0,I);J.maxIndex=D-1;break;}
;}
;}
;this._fireChangeSelection();}
,_resetSelection:function(){this.__oR=[];this.__oS=-1;this.__oT=-1;}
,_addSelectionInterval:function(O,M){this.__oS=O;this.__oT=M;var L=Math.min(O,M);var P=Math.max(O,M);var N=0;for(;N<this.__oR.length;N++ ){var Q=this.__oR[N];if(Q.minIndex>L){break;}
;}
;this.__oR.splice(N,0,{minIndex:L,maxIndex:P});var R=this.__oR[0];for(var i=1;i<this.__oR.length;i++ ){var Q=this.__oR[i];if(R.maxIndex+1>=Q.minIndex){R.maxIndex=Math.max(R.maxIndex,Q.maxIndex);this.__oR.splice(i,1);i-- ;}
else {R=Q;}
;}
;}
,_dumpRanges:function(){var S=h;for(var i=0;i<this.__oR.length;i++ ){var T=this.__oR[i];S+=l+T.minIndex+a+T.maxIndex+f;}
;this.debug(S);}
,_fireChangeSelection:function(){if(this.hasBatchMode()){this.__oU=true;}
...
;}
,properties:{highlightFocusRow:{check:b,init:true}},members:{_colors:null,__oq:null,__or:null,initThemeValues:function(){this.__or=r;this.__oq={};this._colors={};this._renderFont(qx.theme.manager.Font.getInstance().resolve(d));var y=qx.theme.manager.Color.getInstance();this._colors.bgcolFocusedSelected=y.resolve(u);this._colors.bgcolFocused=y.resolve(i);this._colors.bgcolSelected=y.resolve(p);this._colors.bgcolEven=y.resolve(a);this._colors.bgcolOdd=y.resolve(l);this._colors.colSelected=y.resolve(t);this._colors.colNormal=y.resolve(h);this._colors.horLine=y.resolve(n);}
,_insetY:1,_renderFont:function(z){if(z){this.__oq=z.getStyles();this.__or=qx.bom.element.Style.compile(this.__oq);this.__or=this.__or.replace(/"/g,g);}
else {this.__or=r;this.__oq=qx.bom.Font.getDefaultStyles();}
;}
,updateDataRowElement:function(D,A){var B=this.__oq;var C=A.style;qx.bom.element.Style.setStyles(A,B);if(D.focusedRow&&this.getHighlightFocusRow()){C.backgroundColor=D.selected?this._colors.bgcolFocusedSelected:this._colors.bgcolFocused;}
...
;return e+E+v;}
,createRowStyle:function(G){var F=[];F.push(x);F.push(this.__or);F.push(q);if(G.focusedRow&&this.getHighlightFocusRow()){F.push(G.selected?this._colors.bgcolFocusedSelected:this._colors.bgcolFocused);}
...
},destruct:function(){this._colors=this.__oq=this.__or=null;if(qx.core.Environment.get(o)){qx.theme.manager.Color.getInstance().removeListener(f,this.initThemeValues,this);qx.theme.manager.Font.getInstance().removeListener(f,this.initThemeValues,this);}
;}
});}
)();
...
(function(){var a="Could not parse color: ",b="_applyTheme",c="qx.theme.manager.Color",d="Theme",e="changeTheme",f="string",g="singleton";qx.Class.define(c,{type:g,extend:qx.util.ValueManager,properties:{theme:{check:d,nullable:true,apply:b,event:e}},members:{_applyTheme:function(j){var h={};if(j){var i=j.colors;for(var name in i){h[name]=this.__dg(i,name);}
;}
;this._setDynamic(h);}
,__dg:function(l,name){var k=l[name];if(typeof k===f){if(!qx.util.ColorUtil.isCssString(k)){if(l[k]!=undefined){return this.__dg(l,k);}
;throw new Error(a+k);}
;return k;}
...
else if(this.isRgbaString(y)){return this.__di(y);}
else if(this.isRgbString(y)){return this.__dh();}
else if(this.isHex3String(y)){return this.__dj();}
else if(this.isHex6String(y)){return this.__dk();}
;throw new Error(a+y);}
,cssStringToRgb:function(z){if(this.isNamedColor(z)){return this.NAMED[z];}
else if(this.isSystemColor(z)){throw new Error(e+z);}
else if(this.isRgbString(z)){return this.__dh();}
else if(this.isRgbaString(z)){return this.__di();}
else if(this.isHex3String(z)){return this.__dj();}
else if(this.isHex6String(z)){return this.__dk();}
;throw new Error(a+z);}
...
,__dh:function(){var L=parseInt(RegExp.$1,10);var K=parseInt(RegExp.$2,10);var J=parseInt(RegExp.$3,10);return [L,K,J];}
,__di:function(){var P=parseInt(RegExp.$1,10);var O=parseInt(RegExp.$2,10);var M=parseInt(RegExp.$3,10);var N=parseInt(RegExp.$4,10);if(P===0&&O===0&M===0&&N===0){return [-1,-1,-1];}
;return [P,O,M];}
,__dj:function(){var S=parseInt(RegExp.$1,16)*17;var R=parseInt(RegExp.$2,16)*17;var Q=parseInt(RegExp.$3,16)*17;return [S,R,Q];}
,__dk:function(){var V=(parseInt(RegExp.$1,16)*16)+parseInt(RegExp.$2,16);var U=(parseInt(RegExp.$3,16)*16)+parseInt(RegExp.$4,16);var T=(parseInt(RegExp.$5,16)*16)+parseInt(RegExp.$6,16);return [V,U,T];}
,hex3StringToRgb:function(W){if(this.isHex3String(W)){return this.__dj(W);}
;throw new Error(m+W);}
,hex3StringToHex6String:function(X){if(this.isHex3String(X)){return this.rgbToHexString(this.hex3StringToRgb(X));}
;return X;}
,hex6StringToRgb:function(Y){if(this.isHex6String(Y)){return this.__dk(Y);}
;throw new Error(q+Y);}
,hexStringToRgb:function(ba){if(this.isHex3String(ba)){return this.__dj(ba);}
;if(this.isHex6String(ba)){return this.__dk(ba);}
...
;var j=this.getTheme();if(j!==null&&j.fonts[l]){var m=this.__ky(j.fonts[l]);return k[l]=(new m).set(j.fonts[l]);}
;return l;}
,isDynamic:function(q){var p=this._dynamic;if(q&&(q instanceof qx.bom.Font||p[q]!==undefined)){return true;}
;var o=this.getTheme();if(o!==null&&q&&o.fonts[q]){var n=this.__ky(o.fonts[q]);p[q]=(new n).set(o.fonts[q]);return true;}
;return false;}
,__kx:function(s,r){if(s[r].include){var t=s[s[r].include];s[r].include=null;delete s[r].include;s[r]=qx.lang.Object.mergeWith(s[r],t,false);this.__kx(s,r);}
;}
,_applyTheme:function(y){var u=this._dynamic;for(var x in u){if(u[x].themed){u[x].dispose();delete u[x];}
;}
;if(y){var v=y.fonts;for(var x in v){if(v[x].include&&v[v[x].include]){this.__kx(v,x);}
;var w=this.__ky(v[x]);u[x]=(new w).set(v[x]);u[x].themed=true;}
;}
;this._setDynamic(u);}
,__ky:function(z){if(z.sources){return qx.bom.webfonts.WebFont;}
;return qx.bom.Font;}
},destruct:function(){this._disposeMap(c);}
});}
)();
(function(){var a="Boolean",b="px",c="_applyItalic",d="_applyBold",e="underline",f="_applyTextShadow",g="Integer",h="_applyFamily",j="_applyLineHeight",k='"',m="Array",n="line-through",o="overline",p="Color",q="String",r="",s="italic",t="normal",u="qx.bom.Font",v="bold",w="Number",x="_applyDecoration",y=" ",z="_applySize",A=",",B="_applyColor";qx.Class.define(u,{extend:qx.core.Object,construct:function(D,C){qx.core.Object.call(this);this.__kz={fontFamily:r,fontSize:null,fontWeight:null,fontStyle:null,textDecoration:null,lineHeight:null,color:null,textShadow:null};if(D!==undefined){this.setSize(D);}
...
,__kA:{fontFamily:r,fontSize:r,fontWeight:r,fontStyle:r,textDecoration:r,lineHeight:1.2,color:r,textShadow:r},getDefaultStyles:function(){return this.__kA;}
},properties:{size:{check:g,nullable:true,apply:z},lineHeight:{check:w,nullable:true,apply:j},family:{check:m,nullable:true,apply:h},bold:{check:a,nullable:true,apply:d},italic:{check:a,nullable:true,apply:c},decoration:{check:[e,n,o],nullable:true,apply:x},color:{check:p,nullable:true,apply:B},textShadow:{nullable:true,check:q,apply:f}},members:{__kz:null,_applySize:function(M,L){this.__kz.fontSize=M===null?null:M+b;}
,_applyLineHeight:function(O,N){this.__kz.lineHeight=O===null?null:O;}
,_applyFamily:function(P,Q){var R=r;for(var i=0,l=P.length;i<l;i++ ){if(P[i].indexOf(y)>0){R+=k+P[i]+k;}
else {R+=P[i];}
;if(i!==l-1){R+=A;}
;}
;this.__kz.fontFamily=R;}
,_applyBold:function(T,S){this.__kz.fontWeight=T==null?null:T?v:t;}
,_applyItalic:function(V,U){this.__kz.fontStyle=V==null?null:V?s:t;}
,_applyDecoration:function(X,W){this.__kz.textDecoration=X==null?null:X;}
,_applyColor:function(ba,Y){this.__kz.color=null;if(ba){this.__kz.color=qx.theme.manager.Color.getInstance().resolve(ba);}
;}
,_applyTextShadow:function(bc,bb){this.__kz.textShadow=bc==null?null:bc;}
,getStyles:function(){return this.__kz;}
}});}
)();
...
(function(){var a="m",b="os.name",c=")",d="os.version",e="qx.bom.webfonts.Manager",f="svg",g="#",h="browser.name",k="singleton",n=",\n",o="src: ",p="mobileSafari",q="'eot)",r="');",s="qx.debug",t="interval",u="chrome",v="firefox",w="!",y="eot",z="ios",A="changeStatus",B="\.(",C="}\n",D="font-family: ",E="browser.documentmode",F="mobile safari",G="safari",H="@font-face.*?",I="",J="ttf",K=";\n",L="') format('svg')",M="') format('woff')",N="('embedded-opentype')",O="browser.version",P="opera",Q="engine.version",R="Couldn't create @font-face rule for WebFont ",S="mshtml",T="engine.name",U="url('",V="src: url('",W="('embedded-opentype)",X="\nfont-style: normal;\nfont-weight: normal;",Y="?#iefix') format('embedded-opentype')",bl="woff",bm="ie",bn=";",bh="'eot')",bi="Error while adding @font-face rule:",bj="@font-face {",bk="') format('truetype')";qx.Class.define(e,{extend:qx.core.Object,type:k,construct:function(){qx.core.Object.call(this);this.__kC=[];this.__kD={};this.__dl=[];this.__kE=this.getPreferredFormats();var bp=qx.core.Environment.get(h);var bo=parseInt(qx.core.Environment.get(O),10);if((bp==u&&bo<35)||(bp==P)&&bo<22){this.__kF=true;}
;}
,statics:{FONT_FORMATS:[y,bl,J,f],VALIDATION_TIMEOUT:5000},members:{__kC:null,__kG:null,__kD:null,__kE:null,__dl:null,__kH:null,__kF:false,require:function(bs,bt,bu,bw){var br=[];for(var i=0,l=bt.length;i<l;i++ ){var bv=bt[i].split(g);var bq=qx.util.ResourceManager.getInstance().toUri(bv[0]);if(bv.length>1){bq=bq+g+bv[1];}
;if(this.__kF){bq=qx.util.Uri.getAbsolute(bq);}
;br.push(bq);}
...
;this.__dl.push([bs,br,bu,bw]);}
else {this.__kI(bs,br,bu,bw);}
;}
,remove:function(by){var bx=null;for(var i=0,l=this.__kC.length;i<l;i++ ){if(this.__kC[i]==by){bx=i;this.__kP(by);break;}
;}
;if(bx){qx.lang.Array.removeAt(this.__kC,bx);}
;if(by in this.__kD){this.__kD[by].dispose();delete this.__kD[by];}
;}
...
;return bz;}
,removeStyleSheet:function(){this.__kC=[];if(this.__kG){qx.bom.Stylesheet.removeSheet(this.__kG);}
;this.__kG=null;}
,__kI:function(bG,bI,bF,bJ){if(!qx.lang.Array.contains(this.__kC,bG)){var bK=this.__kL(bI);var bH=this.__kM(bG,bK);if(!bH){throw new Error(R+bG+w);}
...
;}
;this.__kC.push(bG);}
;if(!this.__kD[bG]){this.__kD[bG]=new qx.bom.webfonts.Validator(bG);this.__kD[bG].setTimeout(qx.bom.webfonts.Manager.VALIDATION_TIMEOUT);this.__kD[bG].addListenerOnce(A,this.__kK,this);}
;if(bF){var bE=bJ||window;this.__kD[bG].addListenerOnce(A,bF,bE);}
;this.__kD[bG].validate();}
,__kJ:function(){if(this.__dl.length==0){this.__kH.stop();return;}
;var bM=this.__dl.shift();this.__kI.apply(this,bM);}
,__kK:function(bN){var bO=bN.getData();if(bO.valid===false){qx.event.Timer.once(function(){this.remove(bO.family);}
,this,250);}
;}
,__kL:function(bP){var bR=qx.bom.webfonts.Manager.FONT_FORMATS;var bQ={};for(var i=0,l=bP.length;i<l;i++ ){var bS=null;for(var x=0;x<bR.length;x++ ){var bT=new RegExp(B+bR[x]+c);var bU=bT.exec(bP[i]);if(bU){bS=bU[1];}
;}
;if(bS){bQ[bS]=bP[i];}
;}
;return bQ;}
,__kM:function(bX,cb){var ca=[];var bV=this.__kE.length>0?this.__kE:qx.bom.webfonts.Manager.FONT_FORMATS;for(var i=0,l=bV.length;i<l;i++ ){var bW=bV[i];if(cb[bW]){ca.push(this.__kN(bW,cb[bW]));}
;}
;var bY=o+ca.join(n)+bn;bY=D+bX+K+bY;bY=bY+X;return bY;}
,__kN:function(cd,cc){switch(cd){case y:return U+cc+r+V+cc+Y;case bl:return U+cc+M;case J:return U+cc+bk;case f:return U+cc+L;default:return null;};}
,__kO:function(cf){var ce=bj+cf+C;if(qx.core.Environment.get(h)==bm&&qx.core.Environment.get(E)<9){var cg=this.__kQ(this.__kG.cssText);cg+=ce;this.__kG.cssText=cg;}
else {this.__kG.insertRule(ce,this.__kG.cssRules.length);}
;}
,__kP:function(ch){var ck=new RegExp(H+ch,a);for(var i=0,l=document.styleSheets.length;i<l;i++ ){var ci=document.styleSheets[i];if(ci.cssText){var cj=ci.cssText.replace(/\n/g,I).replace(/\r/g,I);cj=this.__kQ(cj);if(ck.exec(cj)){cj=cj.replace(ck,I);}
;ci.cssText=cj;}
else if(ci.cssRules){for(var j=0,m=ci.cssRules.length;j<m;j++ ){var cj=ci.cssRules[j].cssText.replace(/\n/g,I).replace(/\r/g,I);if(ck.exec(cj)){this.__kG.deleteRule(j);return;}
;}
;}
;}
;}
,__kQ:function(cl){return cl.replace(q,bh).replace(W,N);}
},destruct:function(){if(this.__kH){this.__kH.stop();this.__kH.dispose();}
;delete this.__kC;this.removeStyleSheet();for(var cm in this.__kD){this.__kD[cm].dispose();}
...
,statics:{__bI:qx.$$resources||{},__ja:{}},members:{has:function(r){return !!this.self(arguments).__bI[r];}
...
;var self=this;this.__ik=function(){self._oninterval.call(self);}
;}
...
;var p=new qx.event.Timer(o);p.__il=m;p.addListener(c,function(e){p.stop();m.call(n,e);p.dispose();n=null;}
,n);p.start();return p;}
},properties:{enabled:{init:true,check:f,apply:k},interval:{check:i,init:1000,apply:b}},members:{__im:null,__ik:null,_applyInterval:function(r,q){if(this.getEnabled()){this.restart();}
;}
,_applyEnabled:function(t,s){if(s){window.clearInterval(this.__im);this.__im=null;}
else if(t){this.__im=window.setInterval(this.__ik,this.getInterval());}
;}
...
;}
)},destruct:function(){if(this.__im){window.clearInterval(this.__im);}
;this.__im=this.__ik=null;}
});}
)();
...
(function(){var a="sans-serif",b="changeStatus",c="Integer",d="auto",e="qx.event.type.Data",f="0",g="qx.bom.webfonts.Validator",h="interval",i="Georgia",j="WEei",k="visible",l="__kU",m="Times New Roman",n="Arial",o="normal",p="Helvetica",q="350px",r="_applyFontFamily",s="-1000px",t="hidden",u="serif",v="span",w="absolute",x=",";qx.Class.define(g,{extend:qx.core.Object,construct:function(y){qx.core.Object.call(this);if(y){this.setFontFamily(y);}
;this.__kR=this._getRequestedHelpers();}
,statics:{COMPARISON_FONTS:{sans:[n,p,a],serif:[m,i,u]},HELPER_CSS:{position:w,margin:f,padding:f,top:s,left:s,fontSize:q,width:d,height:d,lineHeight:o,fontVariant:o,visibility:t},COMPARISON_STRING:j,__kS:null,__kT:null,removeDefaultHelperElements:function(){var z=qx.bom.webfonts.Validator.__kT;if(z){for(var A in z){document.body.removeChild(z[A]);}
;}
;delete qx.bom.webfonts.Validator.__kT;}
},properties:{fontFamily:{nullable:true,init:null,apply:r},timeout:{check:c,init:5000}},events:{"changeStatus":e},members:{__kR:null,__kU:null,__kV:null,validate:function(){this.__kV=new Date().getTime();if(this.__kU){this.__kU.restart();}
...
,this,0);}
;}
,_reset:function(){if(this.__kR){for(var C in this.__kR){var B=this.__kR[C];document.body.removeChild(B);}
;this.__kR=null;}
;}
,_isFontValid:function(){if(!qx.bom.webfonts.Validator.__kS){this.__dp();}
;if(!this.__kR){this.__kR=this._getRequestedHelpers();}
;this.__kR.sans.style.visibility=k;this.__kR.sans.style.visibility=t;this.__kR.serif.style.visibility=k;this.__kR.serif.style.visibility=t;var E=qx.bom.element.Dimension.getWidth(this.__kR.sans);var D=qx.bom.element.Dimension.getWidth(this.__kR.serif);var F=qx.bom.webfonts.Validator;if(E!==F.__kS.sans&&D!==F.__kS.serif){return true;}
;return false;}
...
;}
,__dp:function(){var N=qx.bom.webfonts.Validator;if(!N.__kT){N.__kT={sans:this._getHelperElement(N.COMPARISON_FONTS.sans),serif:this._getHelperElement(N.COMPARISON_FONTS.serif)};}
;N.__kS={sans:qx.bom.element.Dimension.getWidth(N.__kT.sans),serif:qx.bom.element.Dimension.getWidth(N.__kT.serif)};}
,__kW:function(){if(this._isFontValid()){this.__kU.stop();this._reset();this.fireDataEvent(b,{family:this.getFontFamily(),valid:true});}
else {var O=new Date().getTime();if(O-this.__kV>=this.getTimeout()){this.__kU.stop();this._reset();this.fireDataEvent(b,{family:this.getFontFamily(),valid:false});}
;}
;}
},destruct:function(){this._reset();this.__kU.stop();this.__kU.removeListener(h,this.__kW,this);this._disposeObjects(l);}
});}
)();
...
;}
;}
,"false":null}),__cT:null,__cU:null,__cV:null,__cW:null,__cX:null,__cY:null,__da:null,getBounds:function(){return this.__cY||this.__cU||null;}
...
;if(P!=null&&P!==this.__cT){this.__cT=P;qx.ui.core.queue.Layout.add(this);return null;}
;var O=this.__cU;if(!O){O=this.__cU={};}
;var T={};if(U!==O.left||top!==O.top){T.position=true;O.left=U;O.top=top;}
;if(R!==O.width||Q!==O.height){T.size=true;O.width=R;O.height=Q;}
;if(this.__cV){T.local=true;delete this.__cV;}
;if(this.__cX){T.margin=true;delete this.__cX;}
;return T;}
,isExcluded:function(){return false;}
,hasValidLayout:function(){return !this.__cV;}
,scheduleLayoutUpdate:function(){qx.ui.core.queue.Layout.add(this);}
,invalidateLayoutCache:function(){this.__cV=true;this.__cW=null;}
,getSizeHint:function(V){var W=this.__cW;if(W){return W;}
;if(V===false){return null;}
;W=this.__cW=this._computeSizeHint();if(this._hasHeightForWidth()&&this.__cT&&this.getHeight()==null){W.height=this.__cT;}
...
,_applyMargin:function(){this.__cX=true;var parent=this.$$parent;if(parent){parent.updateLayoutProperties();}
;}
,_applyAlign:function(){var parent=this.$$parent;if(parent){parent.updateLayoutProperties();}
;}
,_applyDimension:function(){qx.ui.core.queue.Layout.add(this);}
,_applyStretching:function(){qx.ui.core.queue.Layout.add(this);}
,hasUserBounds:function(){return !!this.__cY;}
,setUserBounds:function(bi,top,bh,bj){this.__cY={left:bi,top:top,width:bh,height:bj};qx.ui.core.queue.Layout.add(this);}
,resetUserBounds:function(){delete this.__cY;qx.ui.core.queue.Layout.add(this);}
,__db:{},setLayoutProperties:function(bm){if(bm==null){return;}
;var bk=this.__da;if(!bk){bk=this.__da={};}
;var parent=this.getLayoutParent();if(parent){parent.updateLayoutProperties(bm);}
;for(var bl in bm){if(bm[bl]==null){delete bk[bl];}
else {bk[bl]=bm[bl];}
;}
;}
,getLayoutProperties:function(){return this.__da||this.__db;}
,clearLayoutProperties:function(){delete this.__da;}
...
;return null;}
,clone:function(){var bq=qx.core.Object.prototype.clone.call(this);var br=this.__da;if(br){bq.__da=qx.lang.Object.clone(br);}
;return bq;}
...
;this.$$parent=this.$$subparent=this.__da=this.__cU=this.__cY=this.__cW=null;}
});}
)();
(function(){var a="'.",b="Missing appearance: ",c="_applyTheme",d="qx.debug",e="string",f="Cannot find a matching appearance for '",g="Hint: This may be an issue with nested child controls and a missing alias definition in the appearance theme.",h="qx.theme.manager.Appearance",j=":",k="Theme",l="undefined",m="changeTheme",n="/",o="singleton";qx.Class.define(h,{type:o,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__dc={};this.__dd={};}
,properties:{theme:{check:k,nullable:true,event:m,apply:c}},members:{__de:{},__dc:null,__dd:null,_applyTheme:function(){this.__dd={};this.__dc={};}
,__df:function(D,z,q,u){var w=z.appearances;var r=w[D];if(!r){var C=n;var s=[];var v=D.split(C);var B=qx.lang.Array.clone(v);var y;while(!r&&v.length>0){s.unshift(v.pop());var t=v.join(C);r=w[t];if(r){y=r.alias||r;if(typeof y===e){var A=y+C+s.join(C);return this.__df(A,z,q,B);}
;}
;}
;for(var i=0;i<s.length-1;i++ ){s.shift();var x=s.join(C);var p=this.__df(x,z,null,B);if(p){return p;}
;}
;if(q!=null){return this.__df(q,z,null,B);}
;if(qx.core.Environment.get(d)){if(typeof u!==l){this.debug(f+u.join(n)+a);if(u.length>1){this.info(g);}
;}
;}
;return null;}
else if(typeof r===e){return this.__df(r,z,q,B);}
else if(r.include&&!r.style){return this.__df(r.include,z,q,B);}
;return D;}
,styleFrom:function(W,O,P,F){if(!P){P=this.getTheme();}
;var M=this.__dd;var E=M[W];if(!E){E=M[W]=this.__df(W,P,F);}
...
;if(J>0){U+=j+J;}
;}
;var K=this.__dc;if(K[U]!==undefined){return K[U];}
;if(!O){O=this.__de;}
...
(function(){var a="qx.ui.core.queue.Layout",b="layout";qx.Class.define(a,{statics:{__dl:{},__dm:{},remove:function(c){delete this.__dl[c.$$hash];}
,add:function(d){this.__dl[d.$$hash]=d;qx.ui.core.queue.Manager.scheduleFlush(b);}
,isScheduled:function(e){return !!this.__dl[e.$$hash];}
,flush:function(){var f=this.__do();for(var i=f.length-1;i>=0;i-- ){var g=f[i];if(g.hasValidLayout()){continue;}
...
;}
;}
,getNestingLevel:function(l){var k=this.__dm;var n=0;var parent=l;while(true){if(k[parent.$$hash]!=null){n+=k[parent.$$hash];break;}
;if(!parent.$$parent){break;}
;parent=parent.$$parent;n+=1;}
;var m=n;while(l&&l!==parent){k[l.$$hash]=m-- ;l=l.$$parent;}
;return n;}
,__dn:function(){var t=qx.ui.core.queue.Visibility;this.__dm={};var s=[];var r=this.__dl;var o,q;for(var p in r){o=r[p];if(t.isVisible(o)){q=this.getNestingLevel(o);if(!s[q]){s[q]={};}
;s[q][p]=o;delete r[p];}
;}
;return s;}
,__do:function(){var x=[];var z=this.__dn();for(var w=z.length-1;w>=0;w-- ){if(!z[w]){continue;}
...
(function(){var a="css.animation",b="translate",c="rotate",d="skew",e="scale",f="qx.bom.element.Animation";qx.Bootstrap.define(f,{statics:{animate:function(h,k,g){var j=qx.bom.element.Animation.__ee(h,k.keyFrames);if(qx.core.Environment.get(a)&&j){return qx.bom.element.AnimationCss.animate(h,k,g);}
else {return qx.bom.element.AnimationJs.animate(h,k,g);}
;}
,animateReverse:function(m,o,l){var n=qx.bom.element.Animation.__ee(m,o.keyFrames);if(qx.core.Environment.get(a)&&n){return qx.bom.element.AnimationCss.animateReverse(m,o,l);}
else {return qx.bom.element.AnimationJs.animateReverse(m,o,l);}
;}
,__ee:function(p,t){var r=[];for(var v in t){var s=t[v];for(var u in s){if(r.indexOf(u)==-1){r.push(u);}
;}
;}
...
(function(){var a="fill-mode",b="No 'keyFrames' given > 0",c="repeat",d="timing",f="start",g="end",h="Anni",i="start-event",j="keyFrames",k="Keyframe position needs to be between 0 and 100",l="alternate",m="qx.debug",n="keep",o="duration",p=":",q="delay",r="} ",s="name",t="iteration-event",u="",v="origin",w="Unknown key '",x="forwards",y="' in the animation description.",z="Some browsers will not animate elements with display==none",A="iteration",B="end-event",C="css.animation",D="ms ",E="% {",F="none",G=" ",H="linear",I=";",J="qx.bom.element.AnimationCss",K="keyframes",L="display";qx.Bootstrap.define(J,{statics:{__ef:null,__eg:h,__ci:0,__eh:{},__ei:{"scale":true,"rotate":true,"skew":true,"translate":true},__ej:qx.core.Environment.get(C),animateReverse:function(N,O,M){return this._animate(N,O,M,true);}
,animate:function(Q,R,P){return this._animate(Q,R,P,false);}
,_animate:function(S,ba,Y,U){this.__eo(ba);if(qx.core.Environment.get(m)){this.__ep(ba);}
;var W=ba.keep;if(W!=null&&(U||(ba.alternate&&ba.repeat%2==0))){W=100-W;}
;if(!this.__ef){this.__ef=qx.bom.Stylesheet.createElement();}
;var V=ba.keyFrames;if(Y==undefined){Y=ba.duration;}
;if(this.__ej!=null){var name=this.__eq(V,U);var T=name+G+Y+D+ba.timing+G+(ba.delay?ba.delay+D:u)+ba.repeat+G+(ba.alternate?l:u);qx.bom.Event.addNativeListener(S,this.__ej[i],this.__ek);qx.bom.Event.addNativeListener(S,this.__ej[t],this.__el);qx.bom.Event.addNativeListener(S,this.__ej[B],this.__em);if(qx.core.Environment.get(m)){if(qx.bom.element.Style.get(S,L)==F){qx.log.Logger.warn(z,S);}
;}
;S.style[qx.lang.String.camelCase(this.__ej[s])]=T;if(W&&W==100&&this.__ej[a]){S.style[this.__ej[a]]=x;}
;}
...
;if(this.__ej==null){window.setTimeout(function(){qx.bom.element.AnimationCss.__em({target:S});}
,0);}
;return X;}
,__ek:function(e){e.target.$$animation.emit(f,e.target);}
,__el:function(e){if(e.target!=null&&e.target.$$animation!=null){e.target.$$animation.emit(A,e.target);}
;}
,__em:function(e){var bb=e.target;var bc=bb.$$animation;if(!bc){return;}
;var be=bc.desc;if(qx.bom.element.AnimationCss.__ej!=null){var bd=qx.lang.String.camelCase(qx.bom.element.AnimationCss.__ej[s]);bb.style[bd]=u;qx.bom.Event.removeNativeListener(bb,qx.bom.element.AnimationCss.__ej[s],qx.bom.element.AnimationCss.__em);}
;if(be.origin!=null){qx.bom.element.Transform.setOrigin(bb,u);}
;qx.bom.element.AnimationCss.__en(bb,be.keyFrames[bc.keep]);bb.$$animation=null;bc.el=null;bc.ended=true;bc.emit(g,bb);}
,__en:function(bf,bg){var bi;for(var bh in bg){if(bh in qx.bom.element.AnimationCss.__ei){if(!bi){bi={};}
;bi[bh]=bg[bh];}
else {bf.style[qx.lang.String.camelCase(bh)]=bg[bh];}
;}
;if(bi){qx.bom.element.Transform.transform(bf,bi);}
;}
,__eo:function(bj){if(!bj.hasOwnProperty(l)){bj.alternate=false;}
...
;}
,__ep:qx.core.Environment.select(m,{"true":function(bl){var bk=[v,o,n,j,q,c,d,l];for(var name in bl){if(!(bk.indexOf(name)!=-1)){qx.Bootstrap.warn(w+name+y);}
;}
...
;}
;}
,"default":null}),__eq:function(frames,bo){var br=u;for(var bv in frames){br+=(bo?-(bv-100):bv)+E;var bq=frames[bv];var bt;for(var bn in bq){if(bn in this.__ei){if(!bt){bt={};}
;bt[bn]=bq[bn];}
...
;br+=r;}
;if(this.__eh[br]){return this.__eh[br];}
;var name=this.__eg+this.__ci++ ;var bs=this.__ej[K]+G+name;qx.bom.Stylesheet.addRule(this.__ef,bs,br);this.__eh[br]=name;return name;}
}});}
)();
(function(){var a="css.animation",b="Element",c="",d="qx.bom.element.AnimationHandle",e="play-state",f="paused",g="running";qx.Bootstrap.define(d,{extend:qx.event.Emitter,construct:function(){var h=qx.core.Environment.get(a);this.__er=h&&h[e];this.__es=true;}
,events:{"start":b,"end":b,"iteration":b},members:{__er:null,__es:false,__et:false,isPlaying:function(){return this.__es;}
,isEnded:function(){return this.__et;}
,isPaused:function(){return this.el.style[this.__er]==f;}
,pause:function(){if(this.el){this.el.style[this.__er]=f;this.el.$$animation.__es=false;if(this.animationId&&qx.bom.element.AnimationJs){qx.bom.element.AnimationJs.pause(this);}
;}
;}
,play:function(){if(this.el){this.el.style[this.__er]=g;this.el.$$animation.__es=true;if(this.i!=undefined&&qx.bom.element.AnimationJs){qx.bom.element.AnimationJs.play(this);}
;}
;}
,stop:function(){if(this.el&&qx.core.Environment.get(a)&&!this.jsAnimation){this.el.style[this.__er]=c;this.el.style[qx.core.Environment.get(a).name]=c;this.el.$$animation.__es=false;this.el.$$animation.__et=true;}
else if(this.jsAnimation){this.stopped=true;qx.bom.element.AnimationJs.stop(this);}
;}
}});}
)();
(function(){var c="cm",d="mm",e="0",f="pt",g="pc",h="",k="%",l="em",m="qx.bom.element.AnimationJs",n="infinite",o="#",p="in",q="px",r="start",s="end",t="ex",u=";",v="undefined",w="iteration",y="string",z=":";qx.Bootstrap.define(m,{statics:{__eu:30,__ev:[k,p,c,d,l,t,f,g,q],__ei:{"scale":true,"rotate":true,"skew":true,"translate":true},animate:function(B,C,A){return this._animate(B,C,A,false);}
...
;var L=Q.keyFrames;var J=this.__eE(L);var K=this.__eD(P,J);var N=parseInt(P/K,10);this.__ew(L,G);var O=this.__ey(N,K,J,L,P,Q.timing);var H=new qx.bom.element.AnimationHandle();H.jsAnimation=true;if(I){O.reverse();H.reverse=true;}
;H.desc=Q;H.el=G;H.delta=O;H.stepTime=K;H.steps=N;G.$$animation=H;H.i=0;H.initValues={};H.repeatSteps=this.__eB(N,Q.repeat);var M=Q.delay||0;var self=this;H.delayId=window.setTimeout(function(){H.delayId=null;self.play(H);}
,M);return H;}
,__ew:function(V,R){var Y={};for(var U in V){for(var name in V[U]){var S=qx.bom.Style.getPropertyName(name);if(S&&S!=name){var X=qx.bom.Style.getCssName(S);V[U][X]=V[U][name];delete V[U][name];name=X;}
;if(Y[name]==undefined){var W=V[U][name];if(typeof W==y){Y[name]=this.__ez(W);}
else {Y[name]=h;}
;}
;}
;}
...
;if(T[name]===h&&this.__ev.indexOf(Y[name])!=-1){T[name]=e+Y[name];}
;}
;}
;}
;}
,__ex:function(bb){bb=qx.lang.Object.clone(bb);var bc;for(var name in bb){if(name in this.__ei){if(!bc){bc={};}
...
;}
;return bb;}
,__ey:function(bw,bh,bo,bi,be,bq){var bp=new Array(bw);var bm=1;bp[0]=this.__ex(bi[0]);var bt=bi[0];var bj=bi[bo[bm]];var bf=Math.floor(bo[bm]/(bh/be*100));var bs=1;for(var i=1;i<bp.length;i++ ){if(i*bh/be*100>bo[bm]){bt=bj;bm++ ;bj=bi[bo[bm]];bf=Math.floor(bo[bm]/(bh/be*100))-bf;bs=1;}
;bp[i]={};var bd;for(var name in bj){var br=bj[name]+h;if(name in this.__ei){if(!bd){bd={};}
;if(qx.Bootstrap.isArray(bt[name])){if(!qx.Bootstrap.isArray(bj[name])){bj[name]=[bj[name]];}
;bd[name]=[];for(var j=0;j<bj[name].length;j++ ){var bu=bj[name][j]+h;var x=bs/bf;bd[name][j]=this.__eA(bu,bt[name],bq,x);}
;}
else {var x=bs/bf;bd[name]=this.__eA(br,bt[name],bq,x);}
;}
...
else if(!isNaN(parseFloat(br))){var x=bs/bf;bp[i][name]=this.__eA(br,bt[name],bq,x);}
...
;}
;bs++ ;}
;bp[bp.length-1]=this.__ex(bi[100]);return bp;}
,__ez:function(by){return by.substring((parseFloat(by)+h).length,by.length);}
,__eA:function(bC,bB,bz,x){var bA=parseFloat(bC)-parseFloat(bB);return (parseFloat(bB)+bA*qx.bom.AnimationFrame.calculateTiming(bz,x))+this.__ez(bC);}
...
;}
;}
;qx.bom.element.AnimationJs.__eC(bD.el,bF);bD.i++ ;if(bD.i%bD.steps==0){bD.emit(w,bD.el);if(bD.desc.alternate){bD.delta.reverse();}
;}
...
;this.__eC(bH,bJ.keyFrames[bL]);}
else {this.__eC(bH,bI);}
;bH.$$animation=null;bK.el=null;bK.ended=true;bK.animationId=null;bK.emit(s,bH);return bK;}
,__eB:function(bN,bM){if(bM==undefined){return bN;}
;if(bM==n){return Number.MAX_VALUE;}
;return bN*bM;}
,__eC:function(bP,bO){for(var bQ in bO){if(bO[bQ]===undefined){continue;}
...
;}
;}
,__eD:function(bT,bR){var bU=100;for(var i=0;i<bR.length-1;i++ ){bU=Math.min(bU,bR[i+1]-bR[i]);}
;var bS=bT*bU/100;while(bS>this.__eu){bS=bS/2;}
;return Math.round(bS);}
,__eE:function(bW){var bV=Object.keys(bW);for(var i=0;i<bV.length;i++ ){bV[i]=parseInt(bV[i],10);}
...
,getCss:function(J){var K=this.__eH(J);if(this.__eG!=null){var I=this.__eG[b];return qx.bom.Style.getCssName(I)+v+K+o;}
;return f;}
,setOrigin:function(L,M){if(this.__eG!=null){L.style[this.__eG[t]]=M;}
;}
,getOrigin:function(N){if(this.__eG!=null){return N.style[this.__eG[t]];}
;return f;}
,setStyle:function(O,P){if(this.__eG!=null){O.style[this.__eG[u]]=P;}
;}
,getStyle:function(Q){if(this.__eG!=null){return Q.style[this.__eG[u]];}
;return f;}
,setPerspective:function(R,S){if(this.__eG!=null){R.style[this.__eG[c]]=S+h;}
;}
,getPerspective:function(T){if(this.__eG!=null){return T.style[this.__eG[c]];}
;return f;}
,setPerspectiveOrigin:function(U,V){if(this.__eG!=null){U.style[this.__eG[p]]=V;}
;}
,getPerspectiveOrigin:function(W){if(this.__eG!=null){var X=W.style[this.__eG[p]];if(X!=f){return X;}
else {var ba=W.style[this.__eG[p]+r];var Y=W.style[this.__eG[p]+s];if(ba!=f){return ba+k+Y;}
;}
;}
;return f;}
,setBackfaceVisibility:function(bb,bc){if(this.__eG!=null){bb.style[this.__eG[a]]=bc?e:m;}
;}
,getBackfaceVisibility:function(bd){if(this.__eG!=null){return bd.style[this.__eG[a]]==e;}
;return true;}
,__eH:function(bg){var bh=f;for(var be in bg){var bf=bg[be];if(qx.Bootstrap.isArray(bf)){for(var i=0;i<bf.length;i++ ){if(bf[i]==undefined||(i==2&&!qx.core.Environment.get(d))){continue;}
;bh+=be+this.__eF[i]+g;bh+=bf[i];bh+=q;}
;}
...
(function(){var a="qx.util.DeferredCallManager",b="singleton";qx.Class.define(a,{extend:qx.core.Object,type:b,construct:function(){this.__eK={};this.__eL=qx.lang.Function.bind(this.__eP,this);this.__eM=false;}
,members:{__eN:null,__eO:null,__eK:null,__eM:null,__eL:null,schedule:function(d){if(this.__eN==null){this.__eN=window.setTimeout(this.__eL,0);}
;var c=d.toHashCode();if(this.__eO&&this.__eO[c]){return;}
;this.__eK[c]=d;this.__eM=true;}
,cancel:function(f){var e=f.toHashCode();if(this.__eO&&this.__eO[e]){this.__eO[e]=null;return;}
;delete this.__eK[e];if(qx.lang.Object.isEmpty(this.__eK)&&this.__eN!=null){window.clearTimeout(this.__eN);this.__eN=null;}
;}
...
(function(){var a="qx.util.DeferredCall",b="The context object '",c="qx.debug",d="'is already disposed.",e="' of the defered call '";qx.Class.define(a,{extend:qx.core.Object,construct:function(f,g){qx.core.Object.call(this);this.__by=f;this.__bA=g||null;this.__eQ=qx.util.DeferredCallManager.getInstance();}
,members:{__by:null,__bA:null,__eQ:null,cancel:function(){this.__eQ.cancel(this);}
...
;}
;bL.__fq();}
;var bI=this._visibility;for(var bK in bI){bL=bI[bK];var bO=bL.__fb;if(!bO){delete bI[bK];continue;}
;if(qx.core.Environment.get(A)){if(this.DEBUG){qx.log.Logger.debug(this,bt+bL.__fd);}
;}
;if(!bL.$$disposed){bO.style.display=bL.__fd?L:bq;if((qx.core.Environment.get(X)==W)){if(!(document.documentMode>=8)){bO.style.visibility=bL.__fd?C:bn;}
;}
;}
;delete bI[bK];}
;var scroll=this._scroll;for(var bK in scroll){bL=scroll[bK];var bC=bL.__fb;if(bC&&bC.offsetWidth){var bF=true;if(bL.__fg!=null){bL.__fb.scrollLeft=bL.__fg;delete bL.__fg;}
;if(bL.__fh!=null){bL.__fb.scrollTop=bL.__fh;delete bL.__fh;}
;var bP=bL.__fe;if(bP!=null){var bJ=bP.element.getDomElement();if(bJ&&bJ.offsetWidth){qx.bom.element.Scroll.intoViewX(bJ,bC,bP.align);delete bL.__fe;}
else {bF=false;}
;}
;var bQ=bL.__ff;if(bQ!=null){var bJ=bQ.element.getDomElement();if(bJ&&bJ.offsetWidth){qx.bom.element.Scroll.intoViewY(bJ,bC,bQ.align);delete bL.__ff;}
else {bF=false;}
;}
;if(bF){delete scroll[bK];}
;}
;}
;var bE={"releaseCapture":1,"blur":1,"deactivate":1};for(var i=0;i<this._actions.length;i++ ){var bR=this._actions[i];var bO=bR.element.__fb;if(!bO||!bE[bR.type]&&!bR.element.__fr()){continue;}
;var bH=bR.args;bH.unshift(bO);qx.bom.Element[bR.type].apply(qx.bom.Element,bH);}
;this._actions=[];for(var bK in this.__eU){var bA=this.__eU[bK];var bC=bA.element.__fb;if(bC){qx.bom.Selection.set(bC,bA.start,bA.end);delete this.__eU[bK];}
;}
;qx.event.handler.Appear.refresh();}
,__eX:function(){if(!this.__eV){var bT=qx.event.Registration.getManager(window);this.__eV=bT.getHandler(qx.event.handler.Focus);}
;return this.__eV;}
,__eY:function(){if(!this.__eW){var bU=qx.event.Registration.getManager(window);this.__eW=bU.getDispatcher(qx.event.dispatch.MouseCapture);}
;return this.__eW.getCaptureElement();}
,__fa:function(bV){var bW=qx.core.ObjectRegistry.fromHashCode(bV.$$element);return bW&&!bW.__fr();}
},members:{__eR:null,__fb:null,__a:false,__fc:true,__fd:true,__fe:null,__ff:null,__fg:null,__fh:null,__fi:null,__fj:null,__fk:null,__eS:null,__eT:null,__fl:null,__fm:null,__fn:null,__fo:null,__fp:null,_scheduleChildrenUpdate:function(){if(this.__fo){return;}
;this.__fo=true;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
,_createDomElement:function(){return qx.dom.Element.create(this.__eR);}
,__fq:function(){if(qx.core.Environment.get(A)){if(this.DEBUG){this.debug(r+this.getAttribute(bu));}
;}
;var length;var bX=this.__fn;if(bX){length=bX.length;var bY;for(var i=0;i<length;i++ ){bY=bX[i];if(bY.__fd&&bY.__fc&&!bY.__fb){bY.__fq();}
;}
;}
;if(!this.__fb){this.__fb=this._createDomElement();this.__fb.$$element=this.$$hash;this._copyData(false);if(bX&&length>0){this._insertChildren();}
;}
else {this._syncData();if(this.__fo){this._syncChildren();}
;}
;delete this.__fo;}
,_insertChildren:function(){var ca=this.__fn;var length=ca.length;var cc;if(length>2){var cb=document.createDocumentFragment();for(var i=0;i<length;i++ ){cc=ca[i];if(cc.__fb&&cc.__fc){cb.appendChild(cc.__fb);}
;}
;this.__fb.appendChild(cb);}
else {var cb=this.__fb;for(var i=0;i<length;i++ ){cc=ca[i];if(cc.__fb&&cc.__fc){cb.appendChild(cc.__fb);}
;}
;}
;}
,_syncChildren:function(){var cm=qx.core.ObjectRegistry;var cd=this.__fn;var ck=cd.length;var ce;var ci;var cg=this.__fb;var cl=cg.childNodes;var cf=0;var cj;if(qx.core.Environment.get(A)){var ch=0;}
;for(var i=cl.length-1;i>=0;i-- ){cj=cl[i];ci=cm.fromHashCode(cj.$$element);if(!ci||!ci.__fc||ci.__fp!==this){cg.removeChild(cj);if(qx.core.Environment.get(A)){ch++ ;}
;}
;}
;for(var i=0;i<ck;i++ ){ce=cd[i];if(ce.__fc){ci=ce.__fb;cj=cl[cf];if(!ci){continue;}
...
;}
;}
,_copyData:function(co){var cq=this.__fb;var cs=this.__eT;if(cs){var cp=qx.bom.element.Attribute;for(var cr in cs){cp.set(cq,cr,cs[cr]);}
;}
;var cs=this.__eS;if(cs){var cn=qx.bom.element.Style;if(co){cn.setStyles(cq,cs);}
else {cn.setCss(cq,cn.compile(cs));}
;}
;var cs=this.__fl;if(cs){for(var cr in cs){this._applyProperty(cr,cs[cr]);}
;}
;var cs=this.__fm;if(cs){qx.event.Registration.getManager(cq).importListeners(cq,cs);delete this.__fm;}
;}
,_syncData:function(){var cx=this.__fb;var cw=qx.bom.element.Attribute;var cu=qx.bom.element.Style;var cv=this.__fj;if(cv){var cA=this.__eT;if(cA){var cy;for(var cz in cv){cy=cA[cz];if(cy!==undefined){cw.set(cx,cz,cy);}
else {cw.reset(cx,cz);}
;}
;}
;this.__fj=null;}
;var cv=this.__fi;if(cv){var cA=this.__eS;if(cA){var ct={};for(var cz in cv){ct[cz]=cA[cz];}
;cu.setStyles(cx,ct);}
;this.__fi=null;}
;var cv=this.__fk;if(cv){var cA=this.__fl;if(cA){var cy;for(var cz in cv){this._applyProperty(cz,cA[cz]);}
;}
;this.__fk=null;}
;}
,__fr:function(){var cB=this;while(cB){if(cB.__a){return true;}
;if(!cB.__fc||!cB.__fd){return false;}
;cB=cB.__fp;}
;return false;}
,__fs:function(cC){if(cC.__fp===this){throw new Error(a+cC);}
;if(cC.__a){throw new Error(D);}
;if(cC.__fp){cC.__fp.remove(cC);}
;cC.__fp=this;if(!this.__fn){this.__fn=[];}
;if(this.__fb){this._scheduleChildrenUpdate();}
;}
,__ft:function(cD){if(cD.__fp!==this){throw new Error(br+cD);}
;if(this.__fb){this._scheduleChildrenUpdate();}
;delete cD.__fp;}
,__fu:function(cE){if(cE.__fp!==this){throw new Error(br+cE);}
;if(this.__fb){this._scheduleChildrenUpdate();}
;}
,getChildren:function(){return this.__fn||null;}
,getChild:function(cF){var cG=this.__fn;return cG&&cG[cF]||null;}
,hasChildren:function(){var cH=this.__fn;return cH&&cH[0]!==undefined;}
,indexOf:function(cJ){var cI=this.__fn;return cI?cI.indexOf(cJ):-1;}
,hasChild:function(cL){var cK=this.__fn;return cK&&cK.indexOf(cL)!==-1;}
,add:function(cM){if(arguments[1]){for(var i=0,l=arguments.length;i<l;i++ ){this.__fs(arguments[i]);}
;this.__fn.push.apply(this.__fn,arguments);}
else {this.__fs(cM);this.__fn.push(cM);}
;return this;}
,addAt:function(cO,cN){this.__fs(cO);qx.lang.Array.insertAt(this.__fn,cO,cN);return this;}
,remove:function(cP){var cQ=this.__fn;if(!cQ){return this;}
;if(arguments[1]){var cR;for(var i=0,l=arguments.length;i<l;i++ ){cR=arguments[i];this.__ft(cR);qx.lang.Array.remove(cQ,cR);}
;}
else {this.__ft(cP);qx.lang.Array.remove(cQ,cP);}
;return this;}
,removeAt:function(cS){var cT=this.__fn;if(!cT){throw new Error(E);}
;var cU=cT[cS];if(!cU){throw new Error(Q);}
;this.__ft(cU);qx.lang.Array.removeAt(this.__fn,cS);return this;}
,removeAll:function(){var cV=this.__fn;if(cV){for(var i=0,l=cV.length;i<l;i++ ){this.__ft(cV[i]);}
;cV.length=0;}
;return this;}
,getParent:function(){return this.__fp||null;}
,insertInto:function(parent,cW){parent.__fs(this);if(cW==null){parent.__fn.push(this);}
else {qx.lang.Array.insertAt(this.__fn,this,cW);}
;return this;}
,insertBefore:function(cX){var parent=cX.__fp;parent.__fs(this);qx.lang.Array.insertBefore(parent.__fn,this,cX);return this;}
,insertAfter:function(cY){var parent=cY.__fp;parent.__fs(this);qx.lang.Array.insertAfter(parent.__fn,this,cY);return this;}
,moveTo:function(da){var parent=this.__fp;parent.__fu(this);var dc=parent.__fn.indexOf(this);if(dc===da){throw new Error(I);}
else if(dc<da){da-- ;}
;qx.lang.Array.removeAt(parent.__fn,dc);qx.lang.Array.insertAt(parent.__fn,this,da);return this;}
,moveBefore:function(dd){var parent=this.__fp;return this.moveTo(parent.__fn.indexOf(dd));}
,moveAfter:function(de){var parent=this.__fp;return this.moveTo(parent.__fn.indexOf(de)+1);}
,free:function(){var parent=this.__fp;if(!parent){throw new Error(U);}
;if(!parent.__fn){return this;}
...
,useMarkup:function(dg){if(this.__fb){throw new Error(bv);}
;if(qx.core.Environment.get(X)==W){var dh=document.createElement(q);}
else {var dh=qx.dom.Element.getHelperElement();}
;dh.innerHTML=dg;this.useElement(dh.firstChild);return this.__fb;}
,useElement:function(di){if(this.__fb){throw new Error(bv);}
;this.__fb=di;this.__fb.$$element=this.$$hash;this._copyData(true);}
,isFocusable:function(){var dk=this.getAttribute(P);if(dk>=1){return true;}
;var dj=qx.event.handler.Focus.FOCUSABLE_ELEMENTS;if(dk>=0&&dj[this.__eR]){return true;}
;return false;}
...
,include:function(){if(this.__fc){return this;}
;delete this.__fc;if(this.__fp){this.__fp._scheduleChildrenUpdate();}
;return this;}
,exclude:function(){if(!this.__fc){return this;}
;this.__fc=false;if(this.__fp){this.__fp._scheduleChildrenUpdate();}
;return this;}
,isIncluded:function(){return this.__fc===true;}
,fadeIn:function(dn){var dp=qxWeb(this.__fb);if(dp.isPlaying()){dp.stop();}
;if(!this.__fb){this.__fq();dp.push(this.__fb);}
;if(this.__fb){dp.fadeIn(dn);return dp.getAnimationHandles()[0];}
;}
,fadeOut:function(dq){var dr=qxWeb(this.__fb);if(dr.isPlaying()){dr.stop();}
;if(this.__fb){dr.fadeOut(dq).once(v,function(){this.hide();qx.html.Element.flush();}
,this);return dr.getAnimationHandles()[0];}
;}
,show:function(){if(this.__fd){return this;}
;if(this.__fb){qx.html.Element._visibility[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;if(this.__fp){this.__fp._scheduleChildrenUpdate();}
;delete this.__fd;return this;}
,hide:function(){if(!this.__fd){return this;}
;if(this.__fb){qx.html.Element._visibility[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;this.__fd=false;return this;}
,isVisible:function(){return this.__fd===true;}
,scrollChildIntoViewX:function(dv,dt,dw){var ds=this.__fb;var du=dv.getDomElement();if(dw!==false&&ds&&ds.offsetWidth&&du&&du.offsetWidth){qx.bom.element.Scroll.intoViewX(du,ds,dt);}
else {this.__fe={element:dv,align:dt};qx.html.Element._scroll[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;delete this.__fg;}
,scrollChildIntoViewY:function(dA,dy,dB){var dx=this.__fb;var dz=dA.getDomElement();if(dB!==false&&dx&&dx.offsetWidth&&dz&&dz.offsetWidth){qx.bom.element.Scroll.intoViewY(dz,dx,dy);}
else {this.__ff={element:dA,align:dy};qx.html.Element._scroll[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;delete this.__fh;}
,scrollToX:function(x,dC){var dD=this.__fb;if(dC!==true&&dD&&dD.offsetWidth){dD.scrollLeft=x;delete this.__fg;}
else {this.__fg=x;qx.html.Element._scroll[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;delete this.__fe;}
,getScrollX:function(){var dE=this.__fb;if(dE){return dE.scrollLeft;}
;return this.__fg||0;}
,scrollToY:function(y,dG){var dF=this.__fb;if(dG!==true&&dF&&dF.offsetWidth){dF.scrollTop=y;delete this.__fh;}
else {this.__fh=y;qx.html.Element._scroll[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;delete this.__ff;}
,getScrollY:function(){var dH=this.__fb;if(dH){return dH.scrollTop;}
;return this.__fh||0;}
,disableScrolling:function(){this.enableScrolling();this.scrollToX(0);this.scrollToY(0);this.addListener(h,this.__fw,this);}
,enableScrolling:function(){this.removeListener(h,this.__fw,this);}
,__fv:null,__fw:function(e){if(!this.__fv){this.__fv=true;this.__fb.scrollTop=0;this.__fb.scrollLeft=0;delete this.__fv;}
;}
,getTextSelection:function(){var dI=this.__fb;if(dI){return qx.bom.Selection.get(dI);}
;return null;}
,getTextSelectionLength:function(){var dJ=this.__fb;if(dJ){return qx.bom.Selection.getLength(dJ);}
;return null;}
,getTextSelectionStart:function(){var dK=this.__fb;if(dK){return qx.bom.Selection.getStart(dK);}
;return null;}
,getTextSelectionEnd:function(){var dL=this.__fb;if(dL){return qx.bom.Selection.getEnd(dL);}
;return null;}
,setTextSelection:function(dM,dN){var dO=this.__fb;if(dO){qx.bom.Selection.set(dO,dM,dN);return;}
;qx.html.Element.__eU[this.toHashCode()]={element:this,start:dM,end:dN};qx.html.Element._scheduleFlush(K);}
,clearTextSelection:function(){var dP=this.__fb;if(dP){qx.bom.Selection.clear(dP);}
;delete qx.html.Element.__eU[this.toHashCode()];}
,__fx:function(dQ,dR){var dS=qx.html.Element._actions;dS.push({type:dQ,element:this,args:dR||[]});qx.html.Element._scheduleFlush(K);}
,focus:function(){this.__fx(k);}
,blur:function(){this.__fx(p);}
,activate:function(){this.__fx(T);}
,deactivate:function(){this.__fx(s);}
,capture:function(dT){this.__fx(B,[dT!==false]);}
,releaseCapture:function(){this.__fx(H);}
,setStyle:function(dU,dV,dW){if(!this.__eS){this.__eS={};}
;if(this.__eS[dU]==dV){return this;}
;if(dV==null){delete this.__eS[dU];}
else {this.__eS[dU]=dV;}
;if(this.__fb){if(dW){qx.bom.element.Style.set(this.__fb,dU,dV);return this;}
;if(!this.__fi){this.__fi={};}
;this.__fi[dU]=true;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;return this;}
,setStyles:function(dY,eb){var ea=qx.bom.element.Style;if(!this.__eS){this.__eS={};}
;if(this.__fb){if(!this.__fi){this.__fi={};}
;for(var dX in dY){var ec=dY[dX];if(this.__eS[dX]==ec){continue;}
;if(ec==null){delete this.__eS[dX];}
else {this.__eS[dX]=ec;}
;if(eb){ea.set(this.__fb,dX,ec);continue;}
;this.__fi[dX]=true;}
;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
else {for(var dX in dY){var ec=dY[dX];if(this.__eS[dX]==ec){continue;}
;if(ec==null){delete this.__eS[dX];}
else {this.__eS[dX]=ec;}
;}
;}
...
,getStyle:function(ef){return this.__eS?this.__eS[ef]:null;}
,getAllStyles:function(){return this.__eS||null;}
,setAttribute:function(eg,eh,ei){if(!this.__eT){this.__eT={};}
;if(this.__eT[eg]==eh){return this;}
;if(eh==null){delete this.__eT[eg];}
else {this.__eT[eg]=eh;}
;if(this.__fb){if(ei){qx.bom.element.Attribute.set(this.__fb,eg,eh);return this;}
;if(!this.__fj){this.__fj={};}
;this.__fj[eg]=true;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;return this;}
...
,getAttribute:function(ep){return this.__eT?this.__eT[ep]:null;}
...
,_setProperty:function(et,eu,ev){if(!this.__fl){this.__fl={};}
;if(this.__fl[et]==eu){return this;}
;if(eu==null){delete this.__fl[et];}
else {this.__fl[et]=eu;}
;if(this.__fb){if(ev){this._applyProperty(et,eu);return this;}
;if(!this.__fk){this.__fk={};}
;this.__fk[et]=true;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;return this;}
,_removeProperty:function(ex,ew){return this._setProperty(ex,null,ew);}
,_getProperty:function(ez){var ey=this.__fl;if(!ey){return null;}
...
;}
;if(this.__fb){return qx.event.Registration.addListener(this.__fb,eF,eC,self,eB);}
;if(!this.__fm){this.__fm={};}
;if(eB==null){eB=false;}
;var eE=qx.event.Manager.getNextUniqueId();var eG=eF+(eB?g:c)+eE;this.__fm[eG]={type:eF,listener:eC,self:self,capture:eB,unique:eE};return eG;}
...
;}
;if(this.__fb){qx.event.Registration.removeListener(this.__fb,eN,eI,self,eH);}
else {var eJ=this.__fm;var eM;if(eH==null){eH=false;}
...
;if(this.__fb){qx.event.Registration.removeListenerById(this.__fb,eO);}
else {delete this.__fm[eO];}
;return this;}
,hasListener:function(eQ,eP){if(this.$$disposed){return false;}
;if(this.__fb){return qx.event.Registration.hasListener(this.__fb,eQ,eP);}
;var eR=this.__fm;var eT;if(eP==null){eP=false;}
;for(var eS in eR){eT=eR[eS];if(eT.capture===eP&&eT.type===eQ){return true;}
;}
;return false;}
,getListeners:function(){if(this.$$disposed){return null;}
;if(this.__fb){return qx.event.Registration.getManager(this.__fb).serializeListeners(this.__fb);}
;var eU=[];for(var eW in this.__fm){var eV=this.__fm[eW];eU.push({type:eV.type,handler:eV.listener,self:eV.self,capture:eV.capture});}
;return eU;}
},defer:function(eX){eX.__fy=new qx.util.DeferredCall(eX.flush,eX);}
,destruct:function(){var eY=this.__fb;if(eY){qx.event.Registration.getManager(eY).removeAllListeners(eY);eY.$$element=L;}
;if(!qx.core.ObjectRegistry.inShutDown){var parent=this.__fp;if(parent&&!parent.$$disposed){parent.remove(this);}
;}
;this._disposeArray(bo);this.__eT=this.__eS=this.__fm=this.__fl=this.__fj=this.__fi=this.__fk=this.__fb=this.__fp=this.__fe=this.__ff=null;}
});}
)();
(function(){var a="qx.event.handler.Appear",b="engine.name",c="mshtml",d="disappear",e="appear",f="browser.documentmode";qx.Class.define(a,{extend:qx.core.Object,implement:qx.event.IEventHandler,construct:function(g){qx.core.Object.call(this);this.__eQ=g;this.__fz={};qx.event.handler.Appear.__fA[this.$$hash]=this;}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{appear:true,disappear:true},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:true,__fA:{},refresh:function(){var h=this.__fA;for(var i in h){h[i].refresh();}
;}
},members:{__eQ:null,__fz:null,canHandleEvent:function(k,j){}
,registerEvent:function(o,p,m){var n=qx.core.ObjectRegistry.toHashCode(o)+p;var l=this.__fz;if(l&&!l[n]){l[n]=o;o.$$displayed=o.offsetWidth>0;}
;}
,unregisterEvent:function(t,u,r){var s=qx.core.ObjectRegistry.toHashCode(t)+u;var q=this.__fz;if(!q){return;}
;if(q[s]){delete q[s];}
;}
,refresh:function(){var A=this.__fz;var x;var y=qx.core.Environment.get(b)==c&&qx.core.Environment.get(f)<9;for(var v in A){x=A[v];var w=x.offsetWidth>0;if(!w&&y){w=x.offsetWidth>0;}
;if((!!x.$$displayed)!==w){x.$$displayed=w;var z=qx.event.Registration.createEvent(w?e:d);this.__eQ.dispatchEvent(x,z);}
;}
;}
...
(function(){var a="os.name",b="mousedown",c="mouseout",d="gecko",e="dblclick",f="useraction",g="mousewheel",h="event.touch",j="mousemove",k="ios",l="mouseover",m="qx.event.handler.Mouse",n="click",o="on",p="engine.version",q="engine.name",r="mouseup",s="contextmenu",t="qx.emulatemouse",u="webkit",v="DOMMouseScroll",w="device.touch",x="event.mspointer";qx.Class.define(m,{extend:qx.core.Object,implement:qx.event.IEventHandler,construct:function(y){qx.core.Object.call(this);this.__eQ=y;this.__cw=y.getWindow();this.__a=this.__cw.document;if(!(qx.core.Environment.get(h)&&qx.event.handler.MouseEmulation.ON)){this._initButtonObserver();this._initMoveObserver();this._initWheelObserver();}
;if(qx.core.Environment.get(t)){qx.event.handler.MouseEmulation;}
;}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{mousemove:1,mouseover:1,mouseout:1,mousedown:1,mouseup:1,click:1,dblclick:1,contextmenu:1,mousewheel:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE+qx.event.IEventHandler.TARGET_DOCUMENT+qx.event.IEventHandler.TARGET_WINDOW,IGNORE_CAN_HANDLE:true},members:{__fB:null,__fC:null,__fD:null,__fE:null,__fF:null,__eQ:null,__cw:null,__a:null,__fG:null,canHandleEvent:function(A,z){}
...
:(function(){return null;}
),__fH:function(H,I,J){if(!J){J=qx.bom.Event.getTarget(H);}
...
,__fI:function(){var M=[this.__cw,this.__a,this.__a.body];var L=this.__cw;var K=v;for(var i=0;i<M.length;i++ ){if(qx.bom.Event.supportsEvent(M[i],g)){K=g;L=M[i];break;}
;}
;return {type:K,target:L};}
,preventNextClick:function(){this.__fG=true;}
,_initButtonObserver:function(){this.__fB=qx.lang.Function.listener(this._onButtonEvent,this);var Event=qx.bom.Event;Event.addNativeListener(this.__a,b,this.__fB);Event.addNativeListener(this.__a,r,this.__fB);if(!(qx.event.handler.MouseEmulation.ON&&qx.core.Environment.get(x)&&qx.core.Environment.get(w))){Event.addNativeListener(this.__a,n,this.__fB);}
;Event.addNativeListener(this.__a,e,this.__fB);Event.addNativeListener(this.__a,s,this.__fB);}
,_initMoveObserver:function(){this.__fC=qx.lang.Function.listener(this._onMoveEvent,this);var Event=qx.bom.Event;Event.addNativeListener(this.__a,j,this.__fC);Event.addNativeListener(this.__a,l,this.__fC);Event.addNativeListener(this.__a,c,this.__fC);}
,_initWheelObserver:function(){this.__fD=qx.lang.Function.listener(this._onWheelEvent,this);var N=this.__fI();qx.bom.Event.addNativeListener(N.target,N.type,this.__fD);}
,_stopButtonObserver:function(){var Event=qx.bom.Event;Event.removeNativeListener(this.__a,b,this.__fB);Event.removeNativeListener(this.__a,r,this.__fB);if(!(qx.event.handler.MouseEmulation.ON&&qx.core.Environment.get(x)&&qx.core.Environment.get(w))){Event.removeNativeListener(this.__a,n,this.__fB);}
;Event.removeNativeListener(this.__a,e,this.__fB);Event.removeNativeListener(this.__a,s,this.__fB);}
,_stopMoveObserver:function(){var Event=qx.bom.Event;Event.removeNativeListener(this.__a,j,this.__fC);Event.removeNativeListener(this.__a,l,this.__fC);Event.removeNativeListener(this.__a,c,this.__fC);}
,_stopWheelObserver:function(){var O=this.__fI();qx.bom.Event.removeNativeListener(O.target,O.type,this.__fD);}
,_onMoveEvent:qx.event.GlobalError.observeMethod(function(P){this.__fH(P);}
),_onButtonEvent:qx.event.GlobalError.observeMethod(function(S){var R=S.type;var T=qx.bom.Event.getTarget(S);if(R==n&&this.__fG){delete this.__fG;return;}
;if(qx.core.Environment.get(q)==d||qx.core.Environment.get(q)==u){if(T&&T.nodeType==3){T=T.parentNode;}
;}
;var Q=qx.event.handler.DragDrop&&this.__eQ.getHandler(qx.event.handler.DragDrop).isSessionActive();if(Q&&R==n){return;}
;if(this.__fJ){this.__fJ(S,R,T);}
;if(this.__fL){this.__fL(S,R,T);}
;this.__fH(S,R,T);if(this.__fK){this.__fK(S,R,T);}
;if(this.__fM&&!Q){this.__fM(S,R,T);}
;this.__fE=R;}
),_onWheelEvent:qx.event.GlobalError.observeMethod(function(U){this.__fH(U,g);}
),__fJ:qx.core.Environment.select(q,{"webkit":function(W,V,X){if(parseFloat(qx.core.Environment.get(p))<530){if(V==s){this.__fH(W,r,X);}
;}
;}
,"default":null}),__fK:qx.core.Environment.select(q,{"opera":function(Y,ba,bb){if(ba==r&&Y.button==2){this.__fH(Y,s,bb);}
;}
,"default":null}),__fL:qx.core.Environment.select(q,{"mshtml":function(bc,bd,be){if(bc.target!==undefined){return;}
;if(bd==r&&this.__fE==n){this.__fH(bc,b,be);}
else if(bd==e){this.__fH(bc,n,be);}
;}
,"default":null}),__fM:qx.core.Environment.select(q,{"mshtml":null,"default":function(bg,bf,bh){switch(bf){case b:this.__fF=bh;break;case r:if(bh!==this.__fF){var bi=qx.dom.Hierarchy.getCommonParent(bh,this.__fF);if(bi){this.__fH(bg,n,bi);}
;}
;};}
...
;this.__eQ=this.__cw=this.__a=this.__fF=null;}
...
;if(!this.__d[V]){this.__hQ=V;this.__fH(s,this.__hM,this.__hL,false);}
...
;var bc=this.__hO;var Y=this.__hP;var ba=null;if(this.__ic){if(Y.Shift&&Y.Control&&bc.alias){ba=r;}
...
;}
;var bb=this.__hR;if(ba!=bb){if(this.__hL){this.__hR=ba;this.__hX=this.__fH(x,this.__hL,this.__hM,true);if(!this.__hX){ba=null;}
;}
;if(ba!=bb){this.__hR=ba;this.__fH(x,this.__hM,this.__hL,false);}
;}
;}
,__fH:function(bi,be,bf,bg,bj){var bh=qx.event.Registration;var bd=bh.createEvent(bi,qx.event.type.Drag,[bg,bj]);if(be!==bf){bd.setRelatedTarget(bf);}
...
;bl=bl.parentNode;}
;return null;}
,__ib:function(){this.__hM=null;this.__eQ.removeListener(this.__a,q,this._onMouseMove,this,true);this.__eQ.removeListener(this.__a,A,this._onMouseUp,this,true);qx.event.Registration.removeListener(window,a,this._onWindowBlur,this);this.__hV();}
,clearSession:function(){if(this.__hS){this.__eQ.removeListener(this.__a,w,this._onMouseOver,this,true);this.__eQ.removeListener(this.__a,d,this._onMouseOut,this,true);this.__eQ.removeListener(this.__a,h,this._onKeyDown,this,true);this.__eQ.removeListener(this.__a,z,this._onKeyUp,this,true);this.__eQ.removeListener(this.__a,B,this._onKeyPress,this,true);this.__fH(D,this.__hM,this.__hL,false);this.__hS=false;}
...
,_onMouseDown:function(e){var bp=qx.event.handler.DragDrop.ALLOWED_BUTTONS.indexOf(e.getButton())!==-1;if(this.__hS||!bp){return;}
;var bq=this.__hY(e.getTarget());if(bq){this.__hT=e.getDocumentLeft();this.__hU=e.getDocumentTop();this.__hM=bq;this.__eQ.addListener(this.__a,q,this._onMouseMove,this,true);this.__eQ.addListener(this.__a,A,this._onMouseUp,this,true);qx.event.Registration.addListener(window,a,this._onWindowBlur,this);}
;}
,_onMouseUp:function(e){if(this.__ic&&this.__hX){this.__fH(j,this.__hL,this.__hM,false,e);}
;if(this.__hS&&e.getTarget()==this.__hM){e.stopPropagation();this.__fG();}
;this.clearSession();}
,_onMouseMove:function(e){if(this.__hS){if(!this.__fH(g,this.__hM,this.__hL,true,e)){this.clearSession();}
;}
else {if(Math.abs(e.getDocumentLeft()-this.__hT)>3||Math.abs(e.getDocumentTop()-this.__hU)>3){if(this.__fH(u,this.__hM,this.__hL,true,e)){this.__hS=true;this.__eQ.addListener(this.__a,w,this._onMouseOver,this,true);this.__eQ.addListener(this.__a,d,this._onMouseOut,this,true);this.__eQ.addListener(this.__a,h,this._onKeyDown,this,true);this.__eQ.addListener(this.__a,z,this._onKeyUp,this,true);this.__eQ.addListener(this.__a,B,this._onKeyPress,this,true);var br=this.__hP;br.Control=e.isCtrlPressed();br.Shift=e.isShiftPressed();br.Alt=e.isAltPressed();this.__hW();}
else {this.__fH(D,this.__hM,this.__hL,false);this.__ib();}
;}
;}
;}
...
;var bv=this.__ia(bu);if(bv&&bv!=this.__hL){this.__ic=this.__fH(E,bv,this.__hM,true,e);this.__hL=bv;this.__hW();}
;}
...
;var bz=this.__ia(e.getTarget());var by=this.__ia(e.getRelatedTarget());if(bz&&bz!==by&&bz==this.__hL){this.__fH(C,this.__hL,by,false,e);this.__hL=null;this.__ic=false;qx.event.Timer.once(this.__hW,this,0);}
;}
,__fG:function(){var bA=qx.event.Registration.getManager(window).getHandler(qx.event.handler.Mouse);bA.preventNextClick();}
},destruct:function(){this.__hM=this.__hL=this.__eQ=this.__a=this.__hN=this.__hO=this.__hP=this.__d=null;}
...
;}
,_place:function(bm){this.__ii(function(bo){var bn=qx.util.placement.Placement.compute(bo,this.getLayoutParent().getBounds(),bm,this._getPlacementOffsets(),this.getPosition(),this.getPlacementModeX(),this.getPlacementModeY());this.removeState(g);this.removeState(b);this.addState(bm.left<bn.left?b:g);this.moveTo(bn.left,bn.top);}
);}
},destruct:function(){this.__ih();}
});}
)();
(function(){var a="Number",b="interval",c="_applyTimeoutInterval",d="qx.event.type.Event",e="qx.event.Idle",f="singleton";qx.Class.define(e,{extend:qx.core.Object,type:f,construct:function(){qx.core.Object.call(this);var g=new qx.event.Timer(this.getTimeoutInterval());g.addListener(b,this._onInterval,this);g.start();this.__ij=g;}
,events:{"interval":d},properties:{timeoutInterval:{check:a,init:100,apply:c}},members:{__ij:null,_applyTimeoutInterval:function(h){this.__ij.setInterval(h);}
,_onInterval:function(){this.fireEvent(b);}
},destruct:function(){if(this.__ij){this.__ij.stop();}
;this.__ij=null;}
});}
)();
(function(){var a="-",b="best-fit",c="size",d="edge-start",e="target.bottom",f="offsets",g="size.width",h="bottom",i="offsets.bottom",j="qx.util.placement.Placement",k="Please use '",l="qx.debug",m="keep-align",n="center",o="target.right",p="direct",q="offsets.right",r="middle",s="target",t="align-start",u="Invalid 'mode' argument!'",v="left",w="align-end",x="Class",y="offsets.left",z="top",A="area",B="right",C="edge-end",D="target.top",E="area.height",F="target.left",G="align-center",H='__in',I="area.width",J="' instead!",K="size.height",L="offsets.top";qx.Class.define(j,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__in=qx.util.placement.DirectAxis;}
,properties:{axisX:{check:x},axisY:{check:x},edge:{check:[z,B,h,v],init:z},align:{check:[z,B,h,v,n,r],init:B}},statics:{__io:null,compute:function(W,P,M,N,V,Q,R){this.__io=this.__io||new qx.util.placement.Placement();var T=V.split(a);var S=T[0];var O=T[1];if(qx.core.Environment.get(l)){if(O===n||O===r){var U=r;if(S===z||S===h){U=n;}
;qx.core.Assert.assertEquals(U,O,k+S+a+U+J);}
;}
;this.__io.set({axisX:this.__is(Q),axisY:this.__is(R),edge:S,align:O});return this.__io.compute(W,P,M,N);}
,__ip:null,__iq:null,__ir:null,__is:function(X){switch(X){case p:this.__ip=this.__ip||qx.util.placement.DirectAxis;return this.__ip;case m:this.__iq=this.__iq||qx.util.placement.KeepAlignAxis;return this.__iq;case b:this.__ir=this.__ir||qx.util.placement.BestFitAxis;return this.__ir;default:throw new Error(u);};}
},members:{__in:null,compute:function(be,bb,Y,ba){if(qx.core.Environment.get(l)){this.assertObject(be,c);this.assertNumber(be.width,g);this.assertNumber(be.height,K);this.assertObject(bb,A);this.assertNumber(bb.width,I);this.assertNumber(bb.height,E);this.assertObject(Y,s);this.assertNumber(Y.top,D);this.assertNumber(Y.right,o);this.assertNumber(Y.bottom,e);this.assertNumber(Y.left,F);this.assertObject(ba,f);this.assertNumber(ba.top,L);this.assertNumber(ba.right,q);this.assertNumber(ba.bottom,i);this.assertNumber(ba.left,y);}
;var bc=this.getAxisX()||this.__in;var bf=bc.computeStart(be.width,{start:Y.left,end:Y.right},{start:ba.left,end:ba.right},bb.width,this.__it());var bd=this.getAxisY()||this.__in;var top=bd.computeStart(be.height,{start:Y.top,end:Y.bottom},{start:ba.top,end:ba.bottom},bb.height,this.__iu());return {left:bf,top:top};}
,__it:function(){var bh=this.getEdge();var bg=this.getAlign();if(bh==v){return d;}
else if(bh==B){return C;}
else if(bg==v){return t;}
else if(bg==n){return G;}
else if(bg==B){return w;}
;}
,__iu:function(){var bj=this.getEdge();var bi=this.getAlign();if(bj==z){return d;}
...
(function(){var a="backgroundColor",b="drag",c="_applyNativeContextMenu",d="div",f="_applyBackgroundColor",g="qx.event.type.Data",h="object",j="_applyFocusable",k=" requires a layout, but no one was defined!",m="qx.event.type.KeyInput",n="focused",o="disabled",p="move",q="createChildControl",r="Unsupported control: ",s="dragstart",t="__iz",u="Invalid left decorator inset detected: ",v="Font",w="qx.dynlocale",x="dragchange",y="Invalid layout data: ",z="Could not add widget to itself: ",A="_applyEnabled",B="_applySelectable",C="Number",D="_applyKeepActive",E="dragend",F="_applyVisibility",G="The 'before' widget is not a child of this widget!",H="Child control '",I="qxDraggable",J="syncAppearance",K='" while styling ',L="paddingLeft",M="' of widget ",N="qx.event.type.Mouse",O="Wrong 'left' argument. ",P="_applyPadding",Q="#",R="'Child' must be an instance of qx.ui.core.LayoutItem!",S="Remove Error: ",T="visible",U="qx.event.type.Event",V="qx.event.type.MouseWheel",W="_applyCursor",X="changeVisibility",Y="__iA",cV="_applyDraggable",cW="resize",cX="Decorator",cR="At least one child in control ",cS="zIndex",cT="changeTextColor",cU="$$widget",dd="changeContextMenu",de="on",df="paddingTop",dg="opacity",cY="This widget has no children!",da="changeSelectable",db="Invalid top decorator inset detected: ",dc="none",dk="__iC",dM="outline",eK="hidden",dl="The 'after' widget is not a child of this widget!",dh="_applyAppearance",di=" returned an invalid size hint!",eG="hovered",dj="_applyOpacity",dm="Boolean",dn="px",dp="qx.ui.core.Widget",du="default",dv="minHeight is larger than maxHeight!",dw="TabIndex property must be between 1 and 32000",dq="_applyFont",dr="cursor",ds="qxDroppable",dt="' already created!",dA="changeZIndex",dB=": ",dC="Color",dD="changeEnabled",dx="Abstract method call: _getContentHeightForWidth()!",dy="changeFont",eH="qx.event.type.Focus",dz="_applyDecorator",dH="_applyZIndex",dI="_applyTextColor",eJ=' has no themeable property "',dJ="Widget is not focusable!",dE="__iv",dF="qx.ui.menu.Menu",eI="engine.name",dG="qx.event.type.Drag",dK="Invalid right decorator inset detected: ",dL="Invalid widget to add: ",dX="qx.event.type.KeySequence",dW="excluded",dV="DOM element is not yet created!",ec="_applyToolTipText",eb="The layout of the widget",ea="Exception while creating child control '",dY="_applyDroppable",dQ=" is not a child of this widget!",dP="widget",dO="Wrong 'top' argument. ",dN="qxClass",dU="changeDecorator",dT="qx.event.type.Tap",dS="Integer",dR="changeBackgroundColor",ej="_applyTabIndex",ei="Invalid bottom decorator inset detected: ",eh="changeAppearance",eg="qx.debug",eo="shorthand",en="/",em="String",ek="border-box",ef="",ee="_applyContextMenu",ed="changeToolTipText",ez="padding",ey="tabIndex",ex="paddingBottom",eD="beforeContextmenuOpen",eC="changeNativeContextMenu",eB="undefined",eA="qx.ui.tooltip.ToolTip",es="contextmenu",er="_applyKeepFocus",eq="paddingRight",ep="minWidth is larger than maxWidth!",ew="changeLocale",ev="qxKeepFocus",eu="opera",et="qx.event.type.Touch",eF="qxKeepActive",eE="absolute";qx.Class.define(dp,{extend:qx.ui.core.LayoutItem,include:[qx.locale.MTranslation],construct:function(){qx.ui.core.LayoutItem.call(this);this.__iv=this.__iB();this.initFocusable();this.initSelectable();this.initNativeContextMenu();}
,events:{appear:U,disappear:U,createChildControl:g,resize:g,move:g,syncAppearance:g,mousemove:N,mouseover:N,mouseout:N,mousedown:N,mouseup:N,click:N,dblclick:N,contextmenu:N,beforeContextmenuOpen:g,mousewheel:V,touchstart:et,touchend:et,touchmove:et,touchcancel:et,tap:dT,longtap:dT,swipe:et,keyup:dX,keydown:dX,keypress:dX,keyinput:m,focus:eH,blur:eH,focusin:eH,focusout:eH,activate:eH,deactivate:eH,capture:U,losecapture:U,drop:dG,dragleave:dG,dragover:dG,drag:dG,dragstart:dG,dragend:dG,dragchange:dG,droprequest:dG},properties:{paddingTop:{check:dS,init:0,apply:P,themeable:true},paddingRight:{check:dS,init:0,apply:P,themeable:true},paddingBottom:{check:dS,init:0,apply:P,themeable:true},paddingLeft:{check:dS,init:0,apply:P,themeable:true},padding:{group:[df,eq,ex,L],mode:eo,themeable:true},zIndex:{nullable:true,init:10,apply:dH,event:dA,check:dS,themeable:true},decorator:{nullable:true,init:null,apply:dz,event:dU,check:cX,themeable:true},backgroundColor:{nullable:true,check:dC,apply:f,event:dR,themeable:true},textColor:{nullable:true,check:dC,apply:dI,event:cT,themeable:true,inheritable:true},font:{nullable:true,apply:dq,check:v,event:dy,themeable:true,inheritable:true,dereference:true},opacity:{check:C,apply:dj,themeable:true,nullable:true,init:null},cursor:{check:em,apply:W,themeable:true,inheritable:true,nullable:true,init:null},toolTip:{check:eA,nullable:true},toolTipText:{check:em,nullable:true,event:ed,apply:ec},toolTipIcon:{check:em,nullable:true,event:ed},blockToolTip:{check:dm,init:false},visibility:{check:[T,eK,dW],init:T,apply:F,event:X},enabled:{init:true,check:dm,inheritable:true,apply:A,event:dD},anonymous:{init:false,check:dm},tabIndex:{check:dS,nullable:true,apply:ej},focusable:{check:dm,init:false,apply:j},keepFocus:{check:dm,init:false,apply:er},keepActive:{check:dm,init:false,apply:D},draggable:{check:dm,init:false,apply:cV},droppable:{check:dm,init:false,apply:dY},selectable:{check:dm,init:false,event:da,apply:B},contextMenu:{check:dF,apply:ee,nullable:true,event:dd},nativeContextMenu:{check:dm,init:false,themeable:true,event:eC,apply:c},appearance:{check:em,init:dP,apply:dh,event:eh}},statics:{DEBUG:false,getWidgetByElement:function(eO,eM){while(eO){var eL=eO.$$widget;if(eL!=null){var eN=qx.core.ObjectRegistry.fromHashCode(eL);if(!eM||!eN.getAnonymous()){return eN;}
;}
...
;return false;}
,__iw:new qx.util.ObjectPool()},members:{__iv:null,__ix:null,__iy:null,__iz:null,_getLayout:function(){return this.__iz;}
,_setLayout:function(eQ){if(qx.core.Environment.get(eg)){if(eQ){this.assertInstance(eQ,qx.ui.layout.Abstract);}
;}
;if(this.__iz){this.__iz.connectToWidget(null);}
;if(eQ){eQ.connectToWidget(this);}
;this.__iz=eQ;qx.ui.core.queue.Layout.add(this);}
,setLayoutParent:function(parent){if(this.$$parent===parent){return;}
...
;var content=this.getContentElement();var fc=eY.size||this._updateInsets;var fa=dn;var eR={};if(eY.position){eR.left=eX+fa;eR.top=top+fa;}
;if(fc||eY.margin){eR.width=eU+fa;eR.height=eS+fa;}
;if(Object.keys(eR).length>0){content.setStyles(eR);}
;if(fc||eY.local||eY.margin){if(this.__iz&&this.hasLayoutChildren()){var eW=this.getInsets();var innerWidth=eU-eW.left-eW.right;var innerHeight=eS-eW.top-eW.bottom;var fb=this.getDecorator();var eV={left:0,right:0,top:0,bottom:0};if(fb){fb=qx.theme.manager.Decoration.getInstance().resolve(fb);eV=fb.getPadding();}
;var eT={top:this.getPaddingTop()+eV.top,right:this.getPaddingRight()+eV.right,bottom:this.getPaddingBottom()+eV.bottom,left:this.getPaddingLeft()+eV.left};this.__iz.renderLayout(innerWidth,innerHeight,eT);}
else if(this.hasLayoutChildren()){throw new Error(cR+this._findTopControl()+k);}
;}
;if(eY.position&&this.hasListener(p)){this.fireDataEvent(p,this.getBounds());}
;if(eY.size&&this.hasListener(cW)){this.fireDataEvent(cW,this.getBounds());}
;delete this._updateInsets;return eY;}
,__iA:null,clearSeparators:function(){var fe=this.__iA;if(!fe){return;}
;var ff=qx.ui.core.Widget.__iw;var content=this.getContentElement();var fd;for(var i=0,l=fe.length;i<l;i++ ){fd=fe[i];ff.poolObject(fd);content.remove(fd.getContentElement());}
;fe.length=0;}
,renderSeparator:function(fh,fi){var fg=qx.ui.core.Widget.__iw.getObject(qx.ui.core.Widget);fg.set({decorator:fh});var fk=fg.getContentElement();this.getContentElement().add(fk);var fj=fk.getDomElement();if(fj){fj.style.top=fi.top+dn;fj.style.left=fi.left+dn;fj.style.width=fi.width+dn;fj.style.height=fi.height+dn;}
else {fk.setStyles({left:fi.left+dn,top:fi.top+dn,width:fi.width+dn,height:fi.height+dn});}
...
;if(fn!==null&&fo!==null){this.assert(fn<=fo,dv);}
;}
...
,invalidateLayoutCache:function(){qx.ui.core.LayoutItem.prototype.invalidateLayoutCache.call(this);if(this.__iz){this.__iz.invalidateLayoutCache();}
;}
,_getContentHint:function(){var fw=this.__iz;if(fw){if(this.hasLayoutChildren()){var fx=fw.getSizeHint();if(qx.core.Environment.get(eg)){var fv=eb+this.toString()+di;this.assertInteger(fx.width,O+fv);this.assertInteger(fx.height,dO+fv);}
;return fx;}
...
,getInsets:function(){var top=this.getPaddingTop();var fH=this.getPaddingRight();var fI=this.getPaddingBottom();var fL=this.getPaddingLeft();if(this.getDecorator()){var fK=qx.theme.manager.Decoration.getInstance().resolve(this.getDecorator());var fJ=fK.getInsets();if(qx.core.Environment.get(eg)){this.assertNumber(fJ.top,db+fJ.top);this.assertNumber(fJ.right,dK+fJ.right);this.assertNumber(fJ.bottom,ei+fJ.bottom);this.assertNumber(fJ.left,u+fJ.left);}
...
;return false;}
,__iB:function(){var fS=this._createContentElement();fS.setAttribute(cU,this.toHashCode());if(qx.core.Environment.get(eg)){fS.setAttribute(dN,this.classname);}
...
,_createContentElement:function(){return new qx.html.Element(d,{overflowX:eK,overflowY:eK});}
,getContentElement:function(){return this.__iv;}
,__iC:null,getLayoutChildren:function(){var fU=this.__iC;if(!fU){return this.__iD;}
...
;return fV||fU;}
,scheduleLayoutUpdate:function(){qx.ui.core.queue.Layout.add(this);}
,invalidateLayoutChildren:function(){var fW=this.__iz;if(fW){fW.invalidateChildrenCache();}
;qx.ui.core.queue.Layout.add(this);}
,hasLayoutChildren:function(){var fY=this.__iC;if(!fY){return false;}
;var fX;for(var i=0,l=fY.length;i<l;i++ ){fX=fY[i];if(!fX.hasUserBounds()&&!fX.isExcluded()){return true;}
;}
;return false;}
,getChildrenContainer:function(){return this;}
,__iD:[],_getChildren:function(){return this.__iC||this.__iD;}
,_indexOf:function(gb){var ga=this.__iC;if(!ga){return -1;}
;return ga.indexOf(gb);}
,_hasChildren:function(){var gc=this.__iC;return gc!=null&&(!!gc[0]);}
,addChildrenToQueue:function(gd){var ge=this.__iC;if(!ge){return;}
...
;if(gh.getLayoutParent()==this){qx.lang.Array.remove(this.__iC,gh);}
;if(this.__iC){this.__iC.push(gh);}
else {this.__iC=[gh];}
;this.__iE(gh,gg);}
,_addAt:function(gl,gi,gk){if(!this.__iC){this.__iC=[];}
;if(gl.getLayoutParent()==this){qx.lang.Array.remove(this.__iC,gl);}
;var gj=this.__iC[gi];if(gj===gl){gl.setLayoutProperties(gk);}
;if(gj){qx.lang.Array.insertBefore(this.__iC,gl,gj);}
else {this.__iC.push(gl);}
;this.__iE(gl,gk);}
,_addBefore:function(gm,go,gn){if(qx.core.Environment.get(eg)){this.assertInArray(go,this._getChildren(),G);}
;if(gm==go){return;}
;if(!this.__iC){this.__iC=[];}
;if(gm.getLayoutParent()==this){qx.lang.Array.remove(this.__iC,gm);}
;qx.lang.Array.insertBefore(this.__iC,gm,go);this.__iE(gm,gn);}
,_addAfter:function(gr,gp,gq){if(qx.core.Environment.get(eg)){this.assertInArray(gp,this._getChildren(),dl);}
;if(gr==gp){return;}
;if(!this.__iC){this.__iC=[];}
;if(gr.getLayoutParent()==this){qx.lang.Array.remove(this.__iC,gr);}
;qx.lang.Array.insertAfter(this.__iC,gr,gp);this.__iE(gr,gq);}
,_remove:function(gs){if(!this.__iC){throw new Error(cY);}
;qx.lang.Array.remove(this.__iC,gs);this.__iF(gs);}
,_removeAt:function(gt){if(!this.__iC){throw new Error(cY);}
;var gu=this.__iC[gt];qx.lang.Array.removeAt(this.__iC,gt);this.__iF(gu);return gu;}
,_removeAll:function(){if(!this.__iC){return [];}
;var gv=this.__iC.concat();this.__iC.length=0;for(var i=gv.length-1;i>=0;i-- ){this.__iF(gv[i]);}
;qx.ui.core.queue.Layout.add(this);return gv;}
,_afterAddChild:null,_afterRemoveChild:null,__iE:function(gx,gw){if(qx.core.Environment.get(eg)){this.assertInstance(gx,qx.ui.core.LayoutItem,dL+gx);this.assertNotIdentical(gx,this,z+gx);if(gw!=null){this.assertType(gw,h,y+gw);}
;}
...
;}
,__iF:function(gy){if(qx.core.Environment.get(eg)){this.assertNotUndefined(gy);}
;if(gy.getLayoutParent()!==this){throw new Error(S+gy+dQ);}
;gy.setLayoutParent(null);if(this.__iz){this.__iz.invalidateChildrenCache();}
...
,_applyPadding:function(gB,gA,name){this._updateInsets=true;qx.ui.core.queue.Layout.add(this);this.__iG(name,gB);}
,__iG:function(gC,gF){var content=this.getContentElement();var gD=this.getDecorator();gD=qx.theme.manager.Decoration.getInstance().resolve(gD);if(gD){var gE=qx.Bootstrap.firstLow(gC.replace(ez,ef));gF+=gD.getPadding()[gE]||0;}
;content.setStyle(gC,gF+dn);}
...
;}
,_applyToolTipText:function(gK,gJ){if(qx.core.Environment.get(w)){if(this.__iy){return;}
;var gI=qx.locale.Manager.getInstance();this.__iy=gI.addListener(ew,function(){var gL=this.getToolTipText();if(gL&&gL.translate){this.setToolTipText(gL.translate());}
;}
,this);}
;}
,_applyTextColor:function(gN,gM){}
,_applyZIndex:function(gP,gO){this.getContentElement().setStyle(cS,gP==null?0:gP);}
...
,_applyOpacity:function(gT,gS){this.getContentElement().setStyle(dg,gT==1?null:gT);}
,_applyCursor:function(gV,gU){if(gV==null&&!this.isSelectable()){gV=du;}
;this.getContentElement().setStyle(dr,gV,qx.core.Environment.get(eI)==eu);}
...
;}
,__iH:null,$$stateChanges:null,_forwardStates:null,hasState:function(hf){var he=this.__iH;return !!he&&!!he[hf];}
,addState:function(hj){var hi=this.__iH;if(!hi){hi=this.__iH={};}
;if(hi[hj]){return;}
;this.__iH[hj]=true;if(hj===eG){this.syncAppearance();}
else if(!qx.ui.core.queue.Visibility.isVisible(this)){this.$$stateChanges=true;}
else {qx.ui.core.queue.Appearance.add(this);}
;var forward=this._forwardStates;var hh=this.__iK;if(forward&&forward[hj]&&hh){var hg;for(var hk in hh){hg=hh[hk];if(hg instanceof qx.ui.core.Widget){hh[hk].addState(hj);}
;}
;}
;}
,removeState:function(ho){var hn=this.__iH;if(!hn||!hn[ho]){return;}
;delete this.__iH[ho];if(ho===eG){this.syncAppearance();}
else if(!qx.ui.core.queue.Visibility.isVisible(this)){this.$$stateChanges=true;}
else {qx.ui.core.queue.Appearance.add(this);}
;var forward=this._forwardStates;var hm=this.__iK;if(forward&&forward[ho]&&hm){for(var hp in hm){var hl=hm[hp];if(hl instanceof qx.ui.core.Widget){hl.removeState(ho);}
;}
;}
;}
,replaceState:function(hr,hu){var ht=this.__iH;if(!ht){ht=this.__iH={};}
;if(!ht[hu]){ht[hu]=true;}
;if(ht[hr]){delete ht[hr];}
;if(!qx.ui.core.queue.Visibility.isVisible(this)){this.$$stateChanges=true;}
else {qx.ui.core.queue.Appearance.add(this);}
;var forward=this._forwardStates;var hs=this.__iK;if(forward&&forward[hu]&&hs){for(var hv in hs){var hq=hs[hv];if(hq instanceof qx.ui.core.Widget){hq.replaceState(hr,hu);}
;}
;}
;}
,__iI:null,__iJ:null,syncAppearance:function(){var hA=this.__iH;var hz=this.__iI;var hB=qx.theme.manager.Appearance.getInstance();var hx=qx.core.Property.$$method.setThemed;var hF=qx.core.Property.$$method.resetThemed;if(this.__iJ){delete this.__iJ;if(hz){var hw=hB.styleFrom(hz,hA,null,this.getAppearance());hz=null;}
;}
;if(!hz){var hy=this;var hC=[];do {hC.push(hy.$$subcontrol||hy.getAppearance());}
while(hy=hy.$$subparent);hz=hC.reverse().join(en).replace(/#[0-9]+/g,ef);this.__iI=hz;}
...
;}
;this.fireDataEvent(J,this.__iH);}
,_applyAppearance:function(hH,hG){this.updateAppearance();}
,checkAppearanceNeeds:function(){if(!this.__ix){qx.ui.core.queue.Appearance.add(this);this.__ix=true;}
else if(this.$$stateChanges){qx.ui.core.queue.Appearance.add(this);delete this.$$stateChanges;}
;}
,updateAppearance:function(){this.__iJ=true;qx.ui.core.queue.Appearance.add(this);var hK=this.__iK;if(hK){var hI;for(var hJ in hK){hI=hK[hJ];if(hI instanceof qx.ui.core.Widget){hI.updateAppearance();}
;}
;}
;}
...
;hP.setAttribute(ey,hR);hP.setStyle(dM,dc);}
else {if(hP.isNativelyFocusable()){hP.setAttribute(ey,-1);}
else if(hO){hP.setAttribute(ey,null);}
;}
;}
,_applyKeepFocus:function(hT){var hS=this.getFocusElement();hS.setAttribute(ev,hT?de:null);}
,_applyKeepActive:function(hV){var hU=this.getContentElement();hU.setAttribute(eF,hV?de:null);}
,_applyTabIndex:function(hW){if(hW==null){hW=1;}
else if(hW<1||hW>32000){throw new Error(dw);}
...
,_applyEnabled:function(ib,ia){if(ib===false){this.addState(o);this.removeState(eG);if(this.isFocusable()){this.removeState(n);this._applyFocusable(false,true);}
...
;this._getDragDropCursor();if(ij){this.addListener(s,this._onDragStart);this.addListener(b,this._onDrag);this.addListener(E,this._onDragEnd);this.addListener(x,this._onDragChange);}
else {this.removeListener(s,this._onDragStart);this.removeListener(b,this._onDrag);this.removeListener(E,this._onDragEnd);this.removeListener(x,this._onDragChange);}
;this.getContentElement().setAttribute(I,ij?de:null);}
,_applyDroppable:function(il,ik){if(!this.isEnabled()&&il===true){il=false;}
;this.getContentElement().setAttribute(ds,il?de:null);}
,_onDragStart:function(e){this._getDragDropCursor().placeToMouse(e);this.getApplicationRoot().setGlobalCursor(du);}
...
,hasChildControl:function(iB){if(!this.__iK){return false;}
;return !!this.__iK[iB];}
,__iK:null,_getCreatedChildControls:function(){return this.__iK;}
,getChildControl:function(iE,iD){if(!this.__iK){if(iD){return null;}
;this.__iK={};}
;var iC=this.__iK[iE];if(iC){return iC;}
...
;delete iL.$$subcontrol;delete iL.$$subparent;var iM=this.__iH;var forward=this._forwardStates;if(iM&&forward&&iL instanceof qx.ui.core.Widget){for(var iN in iM){if(forward[iN]){iL.removeState(iN);}
;}
;}
;delete this.__iK[iO];return iL;}
,_createChildControl:function(iT){if(!this.__iK){this.__iK={};}
else if(this.__iK[iT]){throw new Error(H+iT+dt);}
...
;}
catch(iU){iU.message=ea+iT+M+this.toString()+dB+iU.message;throw iU;}
;if(!iP){throw new Error(r+iT);}
;iP.$$subcontrol=iT;iP.$$subparent=this;var iR=this.__iH;var forward=this._forwardStates;if(iR&&forward&&iP instanceof qx.ui.core.Widget){for(var iS in iR){if(forward[iS]){iP.addState(iS);}
;}
;}
;this.fireDataEvent(q,iP);return this.__iK[iT]=iP;}
,_createChildControlImpl:function(iW,iV){return null;}
,_disposeChildControls:function(){var jb=this.__iK;if(!jb){return;}
...
,setDomLeft:function(jg){var jf=this.getContentElement().getDomElement();if(jf){jf.style.left=jg+dn;}
else {throw new Error(dV);}
;}
,setDomTop:function(ji){var jh=this.getContentElement().getDomElement();if(jh){jh.style.top=ji+dn;}
else {throw new Error(dV);}
;}
,setDomPosition:function(jk,top){var jj=this.getContentElement().getDomElement();if(jj){jj.style.left=jk+dn;jj.style.top=top+dn;}
else {throw new Error(dV);}
;}
...
;}
;return jl;}
},destruct:function(){if(!qx.core.ObjectRegistry.inShutDown){if(qx.core.Environment.get(w)){if(this.__iy){qx.locale.Manager.getInstance().removeListenerById(this.__iy);}
;}
;var jn=this.getContentElement();if(jn){jn.setAttribute(cU,null,true);}
...
;if(!qx.core.ObjectRegistry.inShutDown){this.clearSeparators();this.__iA=null;}
else {this._disposeArray(Y);}
;this._disposeArray(dk);this.__iH=this.__iK=null;this._disposeObjects(t,dE);}
});}
)();
(function(){var a="blur",b="activate",c="focus",d="qx.ui.core.EventHandler";qx.Class.define(d,{extend:qx.core.Object,implement:qx.event.IEventHandler,construct:function(){qx.core.Object.call(this);this.__eQ=qx.event.Registration.getManager(window);}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_FIRST,SUPPORTED_TYPES:{mousemove:1,mouseover:1,mouseout:1,mousedown:1,mouseup:1,click:1,dblclick:1,contextmenu:1,mousewheel:1,keyup:1,keydown:1,keypress:1,keyinput:1,capture:1,losecapture:1,focusin:1,focusout:1,focus:1,blur:1,activate:1,deactivate:1,appear:1,disappear:1,dragstart:1,dragend:1,dragover:1,dragleave:1,drop:1,drag:1,dragchange:1,droprequest:1,touchstart:1,touchend:1,touchmove:1,touchcancel:1,tap:1,longtap:1,swipe:1},IGNORE_CAN_HANDLE:false},members:{__eQ:null,__iL:{focusin:1,focusout:1,focus:1,blur:1},__iM:{mouseover:1,mouseout:1,appear:1,disappear:1},canHandleEvent:function(f,e){return f instanceof qx.ui.core.Widget;}
...
;if(this.__iL[h.getType()]){m=m&&m.getFocusTarget();if(!m){return;}
;}
...
;if(u){if(this.__iL[h.getType()]){u=u.getFocusTarget();}
;if(u===m){return;}
;}
;}
;var q=h.getCurrentTarget();var s=qx.ui.core.Widget.getWidgetByElement(q);if(!s||s.isAnonymous()){return;}
;if(this.__iL[h.getType()]){s=s.getFocusTarget();}
;var t=h.getType();if(!s||!(s.isEnabled()||this.__iM[t])){return;}
;var g=h.getEventPhase()==qx.event.type.Event.CAPTURING_PHASE;var p=this.__eQ.getListeners(s,t,g);if(!p||p.length===0){return;}
...
(function(){var a="Missing renderLayout() implementation!",b="abstract",c="Missing getHeightForWidth() implementation!",d="qx.debug",e="It is not possible to manually set the connected widget.",f="qx.ui.layout.Abstract";qx.Class.define(f,{type:b,extend:qx.core.Object,members:{__cW:null,_invalidChildrenCache:null,__lH:null,invalidateLayoutCache:function(){this.__cW=null;}
,renderLayout:function(h,i,g){this.warn(a);}
,getSizeHint:function(){if(this.__cW){return this.__cW;}
;return this.__cW=this._computeSizeHint();}
...
},destruct:function(){this.__lH=this.__cW=null;}
});}
)();
(function(){var a="qx.ui.core.queue.Visibility",b="visibility";qx.Class.define(a,{statics:{__dl:[],__cM:{},remove:function(c){delete this.__cM[c.$$hash];qx.lang.Array.remove(this.__dl,c);}
,isVisible:function(d){return this.__cM[d.$$hash]||false;}
,__iN:function(f){var h=this.__cM;var g=f.$$hash;var e;if(f.isExcluded()){e=false;}
else {var parent=f.$$parent;if(parent){e=this.__iN(parent);}
else {e=f.isRootWidget();}
;}
;return h[g]=e;}
,add:function(k){var j=this.__dl;if(qx.lang.Array.contains(j,k)){return;}
;j.unshift(k);qx.ui.core.queue.Manager.scheduleFlush(b);}
,flush:function(){var o=this.__dl;var p=this.__cM;for(var i=o.length-1;i>=0;i-- ){var n=o[i].$$hash;if(p[n]!=null){o[i].addChildrenToQueue(o);}
;}
;var l={};for(var i=o.length-1;i>=0;i-- ){var n=o[i].$$hash;l[n]=p[n];p[n]=null;}
;for(var i=o.length-1;i>=0;i-- ){var m=o[i];var n=m.$$hash;o.splice(i,1);if(p[n]==null){this.__iN(m);}
;if(p[n]&&p[n]!=l[n]){m.checkAppearanceNeeds();}
;}
;this.__dl=[];}
}});}
)();
(function(){var a=" due to exceptions in user code. The application has to be reloaded!",b="Error in the 'Appearance' queue:",c="Error in the 'Widget' queue:",d="\n",f="Error in the 'Layout' queue:",g="Error in the 'Visibility' queue:",h="qx.debug",i="useraction",j="qx.debug.ui.queue",k="Error while layout flush: ",l="Error in the 'Dispose' queue:",m="Stack trace: \n",n="event.touch",o="qx.ui.core.queue.Manager",p=" times in a row",q="Fatal Error: Flush terminated ";qx.Class.define(o,{statics:{__iO:false,__eI:false,__iP:{},__iQ:0,MAX_RETRIES:10,scheduleFlush:function(r){var self=qx.ui.core.queue.Manager;self.__iP[r]=true;if(!self.__iO){self.__eI=false;qx.bom.AnimationFrame.request(function(){if(self.__eI){self.__eI=false;return;}
;self.flush();}
,self);self.__iO=true;}
;}
,flush:function(){var self=qx.ui.core.queue.Manager;if(self.__iR){return;}
;self.__iR=true;self.__eI=true;var s=self.__iP;self.__iS(function(){while(s.visibility||s.widget||s.appearance||s.layout||s.element){if(s.widget){delete s.widget;if(qx.core.Environment.get(j)){try{qx.ui.core.queue.Widget.flush();}
...
;}
;}
,function(){self.__iO=false;}
);self.__iS(function(){if(s.dispose){delete s.dispose;if(qx.core.Environment.get(j)){try{qx.ui.core.queue.Dispose.flush();}
...
,__iS:qx.core.Environment.select(h,{"true":function(t,u){t();u();}
...
;self.__iO=false;self.__iR=false;self.__iQ+=1;if(self.__iQ<=self.MAX_RETRIES){self.scheduleFlush();}
else {throw new Error(q+(self.__iQ-1)+p+a);}
;throw e;}
finally{w();}
;}
}),__iT:function(e){qx.ui.core.queue.Manager.flush();}
},defer:function(x){qx.html.Element._scheduleFlush=x.scheduleFlush;qx.event.Registration.addListener(window,i,qx.core.Environment.get(n)?x.__iT:x.flush);}
});}
)();
(function(){var a="qx.ui.core.queue.Widget",b="widget",c="$$default";qx.Class.define(a,{statics:{__dl:[],__iP:{},remove:function(e,g){var d=this.__dl;if(!qx.lang.Array.contains(d,e)){return;}
;var f=e.$$hash;if(g==null){qx.lang.Array.remove(d,e);delete this.__iP[f];return;}
;if(this.__iP[f]){delete this.__iP[f][g];if(qx.lang.Object.getLength(this.__iP[f])==0){qx.lang.Array.remove(d,e);}
;}
;}
,add:function(j,l){var h=this.__dl;if(!qx.lang.Array.contains(h,j)){h.unshift(j);}
;if(l==null){l=c;}
;var k=j.$$hash;if(!this.__iP[k]){this.__iP[k]={};}
;this.__iP[k][l]=true;qx.ui.core.queue.Manager.scheduleFlush(b);}
,flush:function(){var m=this.__dl;var n,o;for(var i=m.length-1;i>=0;i-- ){n=m[i];o=this.__iP[n.$$hash];m.splice(i,1);n.syncWidget(o);}
;if(m.length!=0){return;}
;this.__dl=[];this.__iP={};}
}});}
)();
(function(){var a="appearance",b="qx.ui.core.queue.Appearance";qx.Class.define(b,{statics:{__dl:[],remove:function(c){qx.lang.Array.remove(this.__dl,c);}
,add:function(e){var d=this.__dl;if(qx.lang.Array.contains(d,e)){return;}
;d.unshift(e);qx.ui.core.queue.Manager.scheduleFlush(a);}
,has:function(f){return qx.lang.Array.contains(this.__dl,f);}
,flush:function(){var j=qx.ui.core.queue.Visibility;var g=this.__dl;var h;for(var i=g.length-1;i>=0;i-- ){h=g[i];g.splice(i,1);if(j.isVisible(h)){h.syncAppearance();}
else {h.$$stateChanges=true;}
;}
;}
}});}
)();
(function(){var a="dispose",b="qx.ui.core.queue.Dispose";qx.Class.define(b,{statics:{__dl:[],add:function(d){var c=this.__dl;if(qx.lang.Array.contains(c,d)){return;}
;c.unshift(d);qx.ui.core.queue.Manager.scheduleFlush(a);}
,isEmpty:function(){return this.__dl.length==0;}
,flush:function(){var e=this.__dl;for(var i=e.length-1;i>=0;i-- ){var f=e[i];e.splice(i,1);f.dispose();}
;if(e.length!=0){return;}
;this.__dl=[];}
}});}
)();
(function(){var a="mshtml",b="engine.name",c="_applyTheme",d="",e="'.",f="__iV",g="qx-",h="Unable to resolve decorator '",j="singleton",k=";",l="qx.theme.manager.Decoration",m=".",n="Theme",o="object",p="changeTheme",q="string",r="browser.documentmode",s=":";qx.Class.define(l,{type:j,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__eh=[];this.__iU=(qx.core.Environment.get(b)==a&&qx.core.Environment.get(r)<9);}
,properties:{theme:{check:n,nullable:true,apply:c,event:p}},members:{__iV:null,__eh:null,__iU:false,getCssClassName:function(t){if(qx.lang.Type.isString(t)){return g+t;}
...
;var D=this.__iU?z:z+(B?s:d);this.__eh.push(D+C);v.addRule(D+C,w);continue;}
;F+=C+s+u[C]+k;}
;if(F){v.addRule(z,F);this.__eh.push(z);}
;return y;}
,resolve:function(J){if(!J){return null;}
;if(typeof J===o){return J;}
;var K=this.getTheme();if(!K){return null;}
;var H=this.__iV;if(!H){H=this.__iV={};}
...
,isCached:function(R){return !this.__iV?false:qx.lang.Object.contains(this.__iV,R);}
,_applyTheme:function(U,S){var T=qx.util.AliasManager.getInstance();for(var i=0;i<this.__eh.length;i++ ){var V=this.__eh[i];qx.ui.style.Stylesheet.getInstance().removeRule(V);}
;this.__eh=[];if(S){for(var W in S.aliases){T.remove(W);}
;}
;if(U){for(var W in U.aliases){T.add(W,U.aliases[W]);}
;}
;this._disposeMap(f);this.__iV={};}
},destruct:function(){this._disposeMap(f);}
});}
)();
(function(){var a="qx.ui.style.Stylesheet",b="singleton";qx.Class.define(a,{type:b,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__ef=qx.bom.Stylesheet.createElement();this.__eh=[];}
,members:{__eh:null,__ef:null,addRule:function(d,c){if(this.hasRule(d)){return;}
;qx.bom.Stylesheet.addRule(this.__ef,d,c);this.__eh.push(d);}
,hasRule:function(e){return this.__eh.indexOf(e)!=-1;}
,removeRule:function(f){delete this.__eh[this.__eh.indexOf(f)];qx.bom.Stylesheet.removeRule(this.__ef,f);}
}});}
)();
...
(function(){var a="abstract",b="Abstract method called.",c="qx.ui.decoration.Abstract";qx.Class.define(c,{extend:qx.core.Object,implement:[qx.ui.decoration.IDecorator],type:a,members:{__iW:null,_getDefaultInsets:function(){throw new Error(b);}
,_isInitialized:function(){throw new Error(b);}
,_resetInsets:function(){this.__iW=null;}
,getInsets:function(){if(this.__iW){return this.__iW;}
...
(function(){var a="0",b="qx/static",c="http://",d="https://",e="file://",f="qx.util.AliasManager",g="singleton",h=".",i="/",j="static";qx.Class.define(f,{type:g,extend:qx.util.ValueManager,construct:function(){qx.util.ValueManager.call(this);this.__iX={};this.add(j,b);}
,members:{__iX:null,_preprocess:function(n){var m=this._getDynamic();if(m[n]===false){return n;}
else if(m[n]===undefined){if(n.charAt(0)===i||n.charAt(0)===h||n.indexOf(c)===0||n.indexOf(d)===a||n.indexOf(e)===0){m[n]=false;return n;}
;if(this.__iX[n]){return this.__iX[n];}
;var l=n.substring(0,n.indexOf(i));var k=this.__iX[l];if(k!==undefined){m[n]=k+n.substring(l.length);}
;}
;return n;}
,add:function(o,q){this.__iX[o]=q;var p=this._getDynamic();for(var r in p){if(r.substring(0,r.indexOf(i))===o){p[r]=q+r.substring(o.length);}
;}
;}
,remove:function(s){delete this.__iX[s];}
,resolve:function(t){var u=this._getDynamic();if(t!=null){t=this._preprocess(t);}
;return u[t]||t;}
,getAliases:function(){var v={};for(var w in this.__iX){v[w]=this.__iX[w];}
;return v;}
...
;var bc=this.getInnerOpacity();if(bc<1){this.__jc(bd,bc);}
...
;}
;}
,__jc:function(bh,be){if(!qx.core.Environment.get(F)){if(qx.core.Environment.get(m)){qx.log.Logger.warn(A);}
;return;}
...
;}
;}
,__jd:function(){return {top:this.getWidthTop()+this.getInnerWidthTop(),right:this.getWidthRight()+this.getInnerWidthRight(),bottom:this.getWidthBottom()+this.getInnerWidthBottom(),left:this.getWidthLeft()+this.getInnerWidthLeft()};}
}});}
)();
...
else if(qx.core.Environment.get(T)&&!qx.core.Environment.get(P)&&qx.core.Environment.get(e)){if(!this.__jf){this.__jf=document.createElement(M);}
;var br=this.getOrientation()==G;var bC=this.__jg();var bz=br?200:1;var bt=br?1:200;var bx=Math.max(100,this.getEndColorPosition()-this.getStartColorPosition());if(bJ===d){if(br){bz=Math.max(bz,this.getEndColorPosition()-this.getStartColorPosition());}
...
;}
;this.__jf.width=bt;this.__jf.height=bz;var bw=this.__jf.getContext(bm);if(br){var bI=bw.createLinearGradient(0,0,0,bz);}
...
;bw.fillStyle=bI;bw.fillRect(0,0,bt,bz);var by=v+this.__jf.toDataURL()+g;bq[h]=by;if(bJ===bn){if(br){bq[l]=Y+bx+bn;}
else {bq[l]=bx+K;}
;}
else {bq[l]=br?bz+C:Y+bt+d;}
;}
else if(qx.core.Environment.get(T)&&!qx.core.Environment.get(P)){var bC=this.__jg();var bH=this.getOrientation()==U?1:0;var bG=bC.start;var bA=bC.end;if(!qx.util.ColorUtil.isHex6String(bG)){bG=qx.util.ColorUtil.stringToRgb(bG);bG=qx.util.ColorUtil.rgbToHexString(bG);}
...
;}
;}
,__jg:function(){if(qx.core.Environment.get(A)){var bL=qx.theme.manager.Color.getInstance();var bN=bL.resolve(this.getStartColor());var bM=bL.resolve(this.getEndColor());}
else {var bN=this.getStartColor();var bM=this.getEndColor();}
;return {start:bN,end:bM};}
,_getContent:function(){if(qx.core.Environment.get(T)&&!qx.core.Environment.get(P)){var bQ=this.__jg();var bT=this.getOrientation()==U?1:0;var bS=qx.util.ColorUtil.hex3StringToHex6String(bQ.start);var bP=qx.util.ColorUtil.hex3StringToHex6String(bQ.end);bS=bS.substring(1,bS.length);bP=bP.substring(1,bP.length);var bR=H;if(this.classname.indexOf(D)!=-1){var bO={};this._styleBoxShadow(bO);bR=j+qx.bom.element.Style.compile(bO)+n;}
...
,_isInitialized:function(){return this.__jh;}
}});}
)();
(function(){var a='indexOf',b='slice',c='concat',d='toLocaleLowerCase',e="qx.type.BaseString",f="",g='trim',h='match',j="qx.debug",k='search',m='replace',n='toLowerCase',o='charCodeAt',p='split',q='substring',r='lastIndexOf',s='substr',t='toLocaleUpperCase',u='toUpperCase',v='charAt';qx.Class.define(e,{extend:Object,construct:function(w){var w=w||f;this.__ji=w;this.length=w.length;}
,members:{$$isString:true,length:0,__ji:null,toString:function(){return this.__ji;}
...
(function(){var a="qx.locale.LocalizedString";qx.Class.define(a,{extend:qx.type.BaseString,construct:function(b,d,c){qx.type.BaseString.call(this,b);this.__jj=d;this.__jk=c;}
,members:{__jj:null,__jk:null,translate:function(){return qx.locale.Manager.getInstance().translate(this.__jj,this.__jk);}
}});}
)();
(function(){var a="locale",b="_applyLocale",c="",d="changeLocale",e="_",f="Locale: ",g="C",h="locale.variant",j="qx.dynlocale",k=" not available.",l="qx.locale.Manager",m="String",n="singleton",o="qx.debug";qx.Class.define(l,{type:n,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__jl=qx.$$translations||{};this.__jm=qx.$$locales||{};var p=qx.core.Environment.get(a);var q=qx.core.Environment.get(h);if(q!==c){p+=e+q;}
;this.__jn=p;this.setLocale(p||this.__jo);}
...
},properties:{locale:{check:m,nullable:true,apply:b,event:d}},members:{__jo:g,__jp:null,__jq:null,__jl:null,__jm:null,__jn:null,getLanguage:function(){return this.__jq;}
,getTerritory:function(){return this.getLocale().split(e)[1]||c;}
,getAvailableLocales:function(F){var G=[];for(var E in this.__jm){if(E!=this.__jo){if(this.__jm[E]===null&&!F){continue;}
;G.push(E);}
;}
;return G;}
,__jr:function(H){var J;if(H==null){return null;}
;var I=H.indexOf(e);if(I==-1){J=H;}
else {J=H.substring(0,I);}
;return J;}
,_applyLocale:function(L,K){if(qx.core.Environment.get(o)){if(!(L in this.__jm||L==this.__jn)){qx.log.Logger.warn(f+L+k);}
;}
;this.__jp=L;this.__jq=this.__jr(L);}
,addTranslation:function(M,P){var N=this.__jl;if(N[M]){for(var O in P){N[M][O]=P[O];}
;}
else {N[M]=P;}
;}
,addLocale:function(T,R){var Q=this.__jm;if(Q[T]){for(var S in R){Q[T][S]=R[S];}
;}
else {Q[T]=R;}
;}
,translate:function(X,W,U){var V=this.__jl;return this.__js(V,X,W,U);}
,localize:function(bc,bb,Y){var ba=this.__jm;return this.__js(ba,bc,bb,Y);}
,__js:function(bh,bi,bf,bg){if(qx.core.Environment.get(o)){this.assertObject(bh);this.assertString(bi);this.assertArray(bf);}
;var bd;if(!bh){return bi;}
;if(bg){var be=this.__jr(bg);}
else {bg=this.__jp;be=this.__jq;}
;if(!bd&&bh[bg]){bd=bh[bg][bi];}
;if(!bd&&bh[be]){bd=bh[be][bi];}
;if(!bd&&bh[this.__jo]){bd=bh[this.__jo][bi];}
;if(!bd){bd=bi;}
...
(function(){var a="qx.bom.client.Locale",b="-",c="locale",d="",e="android",f="locale.variant";qx.Bootstrap.define(a,{statics:{getLocale:function(){var g=qx.bom.client.Locale.__jt();var h=g.indexOf(b);if(h!=-1){g=g.substr(0,h);}
;return g;}
,getVariant:function(){var i=qx.bom.client.Locale.__jt();var k=d;var j=i.indexOf(b);if(j!=-1){k=i.substr(j+1);}
;return k;}
,__jt:function(){var l=(navigator.userLanguage||navigator.language||d);if(qx.bom.client.OperatingSystem.getName()==e){var m=/(\w{2})-(\w{2})/i.exec(navigator.userAgent);if(m){l=m[0];}
;}
;return l.toLowerCase();}
},defer:function(n){qx.core.Environment.add(c,n.getLocale);qx.core.Environment.add(f,n.getVariant);}
});}
)();
(function(){var a="Image could not be loaded: ",b="Boolean",c="px",d="http",e=".png",f="background-image",g="engine.version",h="scale",i="changeSource",j="div",k="nonScaled",l="qx.ui.basic.Image",m="__ju",n="loaded",o="0 0",p=", no-repeat",q="qx.debug",r="replacement",s="backgroundImage",t="backgroundRepeat",u="-disabled.$1",v="class",w="qx.event.type.Event",x="loadingFailed",y="css.alphaimageloaderneeded",z="String",A="browser.documentmode",B="backgroundPosition",C="border-box",D="left",E="_applySource",F="$$widget",G="data:image/",H="top",I="scaled",J=", ",K="image",L="mshtml",M="engine.name",N=", 0 0",O="_applyScale",P="try to load an unmanaged relative image: ",Q="position",R="img",S="no-repeat",T="background-position",U="hidden",V="alphaScaled",W=",",X="absolute";qx.Class.define(l,{extend:qx.ui.core.Widget,construct:function(Y){this.__ju={};qx.ui.core.Widget.call(this);if(Y){this.setSource(Y);}
;}
,properties:{source:{check:z,init:null,nullable:true,event:i,apply:E,themeable:true},scale:{check:b,init:false,themeable:true,apply:O},appearance:{refine:true,init:K},allowShrinkX:{refine:true,init:false},allowShrinkY:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false}},events:{loadingFailed:w,loaded:w},members:{__jv:null,__jw:null,__fT:null,__ju:null,__jx:null,__jy:null,_onChangeTheme:function(){qx.ui.core.Widget.prototype._onChangeTheme.call(this);this._styleSource();}
,getContentElement:function(){return this.__jC();}
,_createContentElement:function(){return this.__jC();}
,_getContentHint:function(){return {width:this.__jv||0,height:this.__jw||0};}
,_applyDecorator:function(bc,bb){qx.ui.core.Widget.prototype._applyDecorator.call(this,bc,bb);var bd=this.getSource();bd=qx.util.AliasManager.getInstance().resolve(bd);var ba=this.getContentElement();if(this.__jy){ba=ba.getChild(0);}
;this.__jI(ba,bd);}
,_applyPadding:function(bf,be,name){qx.ui.core.Widget.prototype._applyPadding.call(this,bf,be,name);var bg=this.getContentElement();if(this.__jy){bg.getChild(0).setStyles({top:this.getPaddingTop()||0,left:this.getPaddingLeft()||0});}
else {bg.setPadding(this.getPaddingLeft()||0,this.getPaddingTop()||0);}
;}
,renderLayout:function(bj,top,bh,bk){qx.ui.core.Widget.prototype.renderLayout.call(this,bj,top,bh,bk);var bi=this.getContentElement();if(this.__jy){bi.getChild(0).setStyles({width:bh-(this.getPaddingLeft()||0)-(this.getPaddingRight()||0),height:bk-(this.getPaddingTop()||0)-(this.getPaddingBottom()||0),top:this.getPaddingTop()||0,left:this.getPaddingLeft()||0});}
;}
...
,__jz:function(bp){this.__fT=bp;}
,__jA:function(){if(this.__fT==null){var br=this.getSource();var bq=false;if(br!=null){bq=qx.lang.String.endsWith(br,e);}
;if(this.getScale()&&bq&&qx.core.Environment.get(y)){this.__fT=V;}
else if(this.getScale()){this.__fT=I;}
else {this.__fT=k;}
;}
;return this.__fT;}
,__jB:function(bu){var bt;var bs;if(bu==V){bt=true;bs=j;}
else if(bu==k){bt=false;bs=j;}
else {bt=true;bs=R;}
;var bw=new qx.html.Image(bs);bw.setAttribute(F,this.toHashCode());bw.setScale(bt);bw.setStyles({"overflowX":U,"overflowY":U,"boxSizing":C});if(qx.core.Environment.get(y)){var bv=this.__jy=new qx.html.Element(j);bv.setAttribute(F,this.toHashCode());bv.setStyle(Q,X);bv.add(bw);return bv;}
;return bw;}
,__jC:function(){if(this.$$disposed){return null;}
;var bx=this.__jA();if(this.__ju[bx]==null){this.__ju[bx]=this.__jB(bx);}
;var by=this.__ju[bx];if(!this.__jx){this.__jx=by;}
;return by;}
,_styleSource:function(){var bz=qx.util.AliasManager.getInstance().resolve(this.getSource());var bC=this.getContentElement();if(this.__jy){bC=bC.getChild(0);}
;if(!bz){bC.resetSource();return;}
;this.__jD(bz);if((qx.core.Environment.get(M)==L)&&(parseInt(qx.core.Environment.get(g),10)<9||qx.core.Environment.get(A)<9)){var bA=this.getScale()?h:S;bC.tagNameHint=qx.bom.element.Decoration.getTagName(bA,bz);}
;var bB=this.__jx;if(this.__jy){bB=bB.getChild(0);}
;if(qx.util.ResourceManager.getInstance().has(bz)){this.__jF(bB,bz);}
else if(qx.io.ImageLoader.isLoaded(bz)){this.__jG(bB,bz);}
else {this.__jH(bB,bz);}
;}
,__jD:qx.core.Environment.select(M,{"mshtml":function(bE){var bF=qx.core.Environment.get(y);var bD=qx.lang.String.endsWith(bE,e);if(bF&&bD){if(this.getScale()&&this.__jA()!=V){this.__jz(V);}
else if(!this.getScale()&&this.__jA()!=k){this.__jz(k);}
;}
else {if(this.getScale()&&this.__jA()!=I){this.__jz(I);}
else if(!this.getScale()&&this.__jA()!=k){this.__jz(k);}
;}
;this.__jE(this.__jC());}
,"default":function(bG){if(this.getScale()&&this.__jA()!=I){this.__jz(I);}
else if(!this.getScale()&&this.__jA(k)){this.__jz(k);}
;this.__jE(this.__jC());}
}),__jE:function(bK){var bJ=this.__jx;if(bJ!=bK){if(bJ!=null){var bV=c;var bH={};var bP=this.getBounds();if(bP!=null){bH.width=bP.width+bV;bH.height=bP.height+bV;}
;var bQ=this.getInsets();bH.left=parseInt(bJ.getStyle(D)||bQ.left)+bV;bH.top=parseInt(bJ.getStyle(H)||bQ.top)+bV;bH.zIndex=10;var bN=this.__jy?bK.getChild(0):bK;bN.setStyles(bH,true);bN.setSelectable(this.getSelectable());if(!bJ.isVisible()){bK.hide();}
;if(!bJ.isIncluded()){bK.exclude();}
;var bS=bJ.getParent();if(bS){var bI=bS.getChildren().indexOf(bJ);bS.removeAt(bI);bS.addAt(bK,bI);}
;var bM=bN.getNodeName();bN.setSource(null);var bL=this.__jy?this.__jx.getChild(0):this.__jx;bN.tagNameHint=bM;bN.setAttribute(v,bL.getAttribute(v));qx.html.Element.flush();var bU=bL.getDomElement();var bT=bK.getDomElement();var bR=bJ.getListeners()||[];bR.forEach(function(bW){bK.addListener(bW.type,bW.handler,bW.self,bW.capture);}
);if(bU&&bT){var bO=bU.$$hash;bU.$$hash=bT.$$hash;bT.$$hash=bO;}
;this.__jx=bK;}
;}
;}
,__jF:function(bY,cb){var ca=qx.util.ResourceManager.getInstance();if(!this.getEnabled()){var bX=cb.replace(/\.([a-z]+)$/,u);if(ca.has(bX)){cb=bX;this.addState(r);}
else {this.removeState(r);}
;}
;if(bY.getSource()===cb){return;}
;this.__jI(bY,cb);this.__jK(ca.getImageWidth(cb),ca.getImageHeight(cb));}
,__jG:function(cc,cg){var ce=qx.io.ImageLoader;this.__jI(cc,cg);var cf=ce.getWidth(cg);var cd=ce.getHeight(cg);this.__jK(cf,cd);}
,__jH:function(ch,ck){var cl=qx.io.ImageLoader;if(qx.core.Environment.get(q)){var cj=ck.toLowerCase();var ci=qx.lang.String.startsWith;if(!ci(cj,d)&&!ci(cj,G)){var self=this.self(arguments);if(!self.__Ob){self.__Ob={};}
;if(!self.__Ob[ck]){this.debug(P+ck);self.__Ob[ck]=true;}
;}
;}
;if(!cl.isFailed(ck)){cl.load(ck,this.__jJ,this);}
else {if(ch!=null){ch.resetSource();}
;}
;}
,__jI:function(cm,cr){if(cm.getNodeName()==j){var cu=qx.theme.manager.Decoration.getInstance().resolve(this.getDecorator());if(cu){var cs=(cu.getStartColor()&&cu.getEndColor());var cq=cu.getBackgroundImage();if(cs||cq){var cn=this.getScale()?h:S;var co=qx.bom.element.Decoration.getAttributes(cr,cn);var cp=cu.getStyles(true);var ct={"backgroundImage":co.style.backgroundImage,"backgroundPosition":(co.style.backgroundPosition||o),"backgroundRepeat":(co.style.backgroundRepeat||S)};if(cq){ct[B]+=W+cp[T]||o;ct[t]+=J+cu.getBackgroundRepeat();}
;if(cs){ct[B]+=N;ct[t]+=p;}
...
;}
;cm.setSource(cr);}
,__jJ:function(cv,cw){if(this.$$disposed===true){return;}
...
;this._styleSource();}
,__jK:function(cx,cy){if(cx!==this.__jv||cy!==this.__jw){this.__jv=cx;this.__jw=cy;qx.ui.core.queue.Layout.add(this);}
;}
},destruct:function(){delete this.__jx;this._disposeMap(m);}
});}
)();
(function(){var a="source",b="engine.name",c="",d="mshtml",e="px",f="px ",g="no-repeat",h="backgroundImage",i="scale",j="webkit",k="div",l="qx.html.Image",m="qx/static/blank.gif",n="backgroundPosition";qx.Class.define(l,{extend:qx.html.Element,members:{__jL:null,__jM:null,tagNameHint:null,setPadding:function(o,p){this.__jM=o;this.__jL=p;if(this.getNodeName()==k){this.setStyle(n,o+f+p+e);}
;}
...
;var u=this._getProperty(a);var r=this._getProperty(i);var v=r?i:g;if(u!=null){u=u||null;q.paddingTop=this.__jL;q.paddingLeft=this.__jM;qx.bom.element.Decoration.update(s,u,v,q);}
;}
;}
...
;}
,getTagName:function(bf,be){if(be&&qx.core.Environment.get(q)&&this.__jO[bf]&&qx.lang.String.endsWith(be,d)){return k;}
;return this.__jP[bf];}
...
;}
;var bl;if(qx.core.Environment.get(q)&&this.__jO[bi]&&bj===z){var bm=this.__jR(bk);this.__jQ(bh,bm.width,bm.height);bl=this.processAlphaFix(bh,bi,bk);}
else {delete bh.clip;if(bi===i){bl=this.__jS(bh,bi,bk);}
else if(bi===p||bi===P){bl=this.__jT(bh,bi,bk);}
else {bl=this.__jW(bh,bi,bk);}
;}
;return bl;}
,__jQ:function(bo,bn,bp){if(bo.width==null&&bn!=null){bo.width=bn+c;}
;if(bo.height==null&&bp!=null){bo.height=bp+c;}
;}
,__jR:function(bq){var br=qx.util.ResourceManager.getInstance().getImageWidth(bq)||qx.io.ImageLoader.getWidth(bq);var bs=qx.util.ResourceManager.getInstance().getImageHeight(bq)||qx.io.ImageLoader.getHeight(bq);return {width:br,height:bs};}
...
,__jS:function(bz,bA,by){var bB=qx.util.ResourceManager.getInstance().toUri(by);var bC=this.__jR(by);this.__jQ(bz,bC.width,bC.height);return {src:bB,style:bz};}
,__jT:function(bD,bE,bG){var bF=qx.util.ResourceManager.getInstance();var bJ=bF.getCombinedFormat(bG);var bL=this.__jR(bG);var bH;if(bJ){var bK=bF.getData(bG);var bI=bK[4];if(bJ==H){bH=bF.toDataUri(bG);}
else {bH=bF.toUri(bI);}
;if(bE===p){bD=this.__jU(bD,bK,bL.height);}
else {bD=this.__jV(bD,bK,bL.width);}
;return {src:bH,style:bD};}
else {if(qx.core.Environment.get(o)){this.__jY(bG);}
...
;}
,__jU:function(bM,bN,bP){var bO=qx.util.ResourceManager.getInstance().getImageHeight(bN[4]);bM.clip={top:-bN[6],height:bP};bM.height=bO+c;if(bM.top!=null){bM.top=(parseInt(bM.top,10)+bN[6])+c;}
else if(bM.bottom!=null){bM.bottom=(parseInt(bM.bottom,10)+bP-bO-bN[6])+c;}
;return bM;}
,__jV:function(bR,bS,bQ){var bT=qx.util.ResourceManager.getInstance().getImageWidth(bS[4]);bR.clip={left:-bS[5],width:bQ};bR.width=bT+c;if(bR.left!=null){bR.left=(parseInt(bR.left,10)+bS[5])+c;}
else if(bR.right!=null){bR.right=(parseInt(bR.right,10)+bQ-bT-bS[5])+c;}
;return bR;}
,__jW:function(bU,bV,bY){var bX=qx.util.ResourceManager.getInstance();var bW=bX.getCombinedFormat(bY);var ch=this.__jR(bY);if(bW&&bV!==b){var cg=bX.getData(bY);var ce=cg[4];if(bW==H){var cb=bX.toDataUri(bY);var ca=0;var cc=0;}
...
;return {style:bU};}
else {var top=bU.paddingTop||0;var ci=bU.paddingLeft||0;bU.backgroundPosition=ci+g+top+c;if(qx.core.Environment.get(o)){if(bV!==b){this.__jY(bY);}
;}
;this.__jQ(bU,ch.width,ch.height);this.__jX(bU,bY,bV);return {style:bU};}
;}
,__jX:function(cj,cm,ck){var top=null;var cp=null;if(cj.backgroundPosition){var cl=cj.backgroundPosition.split(K);cp=parseInt(cl[0],10);if(isNaN(cp)){cp=cl[0];}
...
;}
,__jY:function(cq){if(this.DEBUG&&qx.util.ResourceManager.getInstance().has(cq)&&cq.indexOf(a)==-1){if(!this.__jN[cq]){qx.log.Logger.debug(l+cq);this.__jN[cq]=true;}
;}
;}
}});}
)();
(function(){var a="load",b="",c="qx.io.ImageLoader",d="html.image.naturaldimensions";qx.Bootstrap.define(c,{statics:{__cM:{},__ka:{width:null,height:null},__kb:/\.(png|gif|jpg|jpeg|bmp)\b/i,__kc:/^data:image\/(png|gif|jpg|jpeg|bmp)\b/i,isLoaded:function(e){var f=this.__cM[e];return !!(f&&f.loaded);}
,isFailed:function(g){var h=this.__cM[g];return !!(h&&h.failed);}
,isLoading:function(j){var k=this.__cM[j];return !!(k&&k.loading);}
,getFormat:function(p){var o=this.__cM[p];if(!o||!o.format){var m=this.__kc.exec(p);if(m!=null){var n=(o&&qx.lang.Type.isNumber(o.width)?o.width:this.__ka.width);var q=(o&&qx.lang.Type.isNumber(o.height)?o.height:this.__ka.height);o={loaded:true,format:m[1],width:n,height:q};}
;}
;return o?o.format:null;}
,getSize:function(r){var s=this.__cM[r];return s?{width:s.width,height:s.height}:this.__ka;}
,getWidth:function(t){var u=this.__cM[t];return u?u.width:null;}
,getHeight:function(v){var w=this.__cM[v];return w?w.height:null;}
,load:function(z,y,A){var B=this.__cM[z];if(!B){B=this.__cM[z]={};}
;if(y&&!A){A=window;}
...
;var x=new Image();var C=qx.lang.Function.listener(this.__kd,this,x,z);x.onload=C;x.onerror=C;x.src=z;B.element=x;}
;}
,abort:function(D){var G=this.__cM[D];if(G&&!G.loaded){G.aborted=true;var F=G.callbacks;var E=G.element;E.onload=E.onerror=null;E.src=b;delete G.callbacks;delete G.element;delete G.loading;for(var i=0,l=F.length;i<l;i+=2){F[i].call(F[i+1],D,G);}
;}
;this.__cM[D]=null;}
,__kd:qx.event.GlobalError.observeMethod(function(event,I,H){var M=this.__cM[H];var J=function(N){return (N&&N.height!==0);}
;if(event.type===a&&J(I)){M.loaded=true;M.width=this.__ke(I);M.height=this.__kf(I);var K=this.__kb.exec(H);if(K!=null){M.format=K[1];}
;}
else {M.failed=true;}
...
;}
),__ke:function(O){return qx.core.Environment.get(d)?O.naturalWidth:O.width;}
,__kf:function(P){return qx.core.Environment.get(d)?P.naturalHeight:P.height;}
...
(function(){var a="offline",b="qx.event.handler.Offline",c="online";qx.Class.define(b,{extend:qx.core.Object,implement:qx.event.IEventHandler,construct:function(d){qx.core.Object.call(this);this.__eQ=d;this.__cw=d.getWindow();this._initObserver();}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{online:true,offline:true},TARGET_CHECK:qx.event.IEventHandler.TARGET_WINDOW,IGNORE_CAN_HANDLE:true},members:{__eQ:null,__cw:null,__fR:null,canHandleEvent:function(f,e){}
,registerEvent:function(i,h,g){}
,unregisterEvent:function(l,k,j){}
,_initObserver:function(){this.__fR=qx.lang.Function.listener(this._onNative,this);qx.bom.Event.addNativeListener(this.__cw,a,this.__fR);qx.bom.Event.addNativeListener(this.__cw,c,this.__fR);}
,_stopObserver:function(){qx.bom.Event.removeNativeListener(this.__cw,a,this.__fR);qx.bom.Event.removeNativeListener(this.__cw,c,this.__fR);}
...
,statics:{PRIORITY:qx.event.Registration.PRIORITY_FIRST},members:{__cy:null,__kk:null,__kl:true,__cw:null,_getParent:function(l){return l.parentNode;}
,canDispatchEvent:function(n,event,m){return !!(this.__kk&&this.__km[m]);}
...
;}
;if(this.__kl||!qx.dom.Hierarchy.contains(this.__kk,p)){p=this.__kk;}
;qx.event.dispatch.AbstractBubbling.prototype.dispatchEvent.call(this,p,event,o);}
,__km:{"mouseup":1,"mousedown":1,"click":1,"dblclick":1,"mousemove":1,"mouseout":1,"mouseover":1},activateCapture:function(r,q){var q=q!==false;if(this.__kk===r&&this.__kl==q){return;}
;if(this.__kk){this.releaseCapture();}
...
;this.__kl=q;this.__kk=r;this.__cy.fireEvent(r,h,qx.event.type.Event,[true,false]);}
,getCaptureElement:function(){return this.__kk;}
,releaseCapture:function(){var s=this.__kk;if(!s){return;}
;this.__kk=null;this.__cy.fireEvent(s,d,qx.event.type.Event,[true,false]);this.nativeReleaseCapture(s);}
...
,"default":(function(){}
)})},destruct:function(){this.__kk=this.__cw=this.__cy=null;}
...
;}
;return false;}
,__kn:{quicktime:{plugin:[k],control:f},wmv:{plugin:[t],control:J},divx:{plugin:[C],control:O},silverlight:{plugin:[c],control:D},pdf:{plugin:[r,g],control:K}},getQuicktimeVersion:function(){var T=qx.bom.client.Plugin.__kn[B];return qx.bom.client.Plugin.__ko(T.control,T.plugin);}
,getWindowsMediaVersion:function(){var U=qx.bom.client.Plugin.__kn[n];return qx.bom.client.Plugin.__ko(U.control,U.plugin);}
,getDivXVersion:function(){var V=qx.bom.client.Plugin.__kn[s];return qx.bom.client.Plugin.__ko(V.control,V.plugin);}
,getSilverlightVersion:function(){var W=qx.bom.client.Plugin.__kn[F];return qx.bom.client.Plugin.__ko(W.control,W.plugin);}
,getPdfVersion:function(){var X=qx.bom.client.Plugin.__kn[m];return qx.bom.client.Plugin.__ko(X.control,X.plugin);}
,getQuicktime:function(){var Y=qx.bom.client.Plugin.__kn[B];return qx.bom.client.Plugin.__kp(Y.control,Y.plugin);}
,getWindowsMedia:function(){var ba=qx.bom.client.Plugin.__kn[n];return qx.bom.client.Plugin.__kp(ba.control,ba.plugin);}
,getDivX:function(){var bb=qx.bom.client.Plugin.__kn[s];return qx.bom.client.Plugin.__kp(bb.control,bb.plugin);}
,getSilverlight:function(){var bc=qx.bom.client.Plugin.__kn[F];return qx.bom.client.Plugin.__kp(bc.control,bc.plugin);}
,getPdf:function(){var bd=qx.bom.client.Plugin.__kn[m];return qx.bom.client.Plugin.__kp(bd.control,bd.plugin);}
,__ko:function(bl,bh){var be=qx.bom.client.Plugin.__kp(bl,bh);if(!be){return u;}
...
;return u;}
;}
,__kp:function(bq,bo){if(qx.bom.client.Engine.getName()==v){var bn=window.ActiveXObject;if(!bn){return false;}
...
(function(){var a="first",b="hovered",c="__pC",d="last",e="qx.ui.table.pane.Header";qx.Class.define(e,{extend:qx.ui.core.Widget,construct:function(f){qx.ui.core.Widget.call(this);this._setLayout(new qx.ui.layout.HBox());this.__la=new qx.ui.core.Blocker(this);this.__pC=f;}
,members:{__pC:null,__pK:null,__pL:null,__la:null,getPaneScroller:function(){return this.__pC;}
,getTable:function(){return this.__pC.getTable();}
...
;}
,setMouseOverColumn:function(l){if(l!=this.__pL){if(this.__pL!=null){var k=this.getHeaderWidgetAtColumn(this.__pL);if(k!=null){k.removeState(b);}
;}
...
,showColumnMoveFeedback:function(y,x){var r=this.getContentLocation();if(this.__pK==null){var w=this.getTable();var o=this.getPaneScroller().getTablePaneModel().getX(y);var q=this._getChildren()[o];var s=w.getTableModel();var u=w.getTableColumnModel();var v={xPos:o,col:y,name:s.getColumnName(y),table:w};var t=u.getHeaderCellRenderer(y);var p=t.createHeaderCell(v);var z=q.getBounds();p.setWidth(z.width);p.setHeight(z.height);p.setZIndex(1000000);p.setOpacity(0.8);p.setLayoutProperties({top:r.top});this.getApplicationRoot().add(p);this.__pK=p;}
;this.__pK.setLayoutProperties({left:r.left+x});this.__pK.show();}
,hideColumnMoveFeedback:function(){if(this.__pK!=null){this.__pK.destroy();this.__pK=null;}
;}
,isShowingColumnMoveFeedback:function(){return this.__pK!=null;}
...
;this.__lK=y;this.__fn=B;delete this._invalidChildrenCache;}
...
;var K=this.__fn;var length=K.length;var W=qx.ui.layout.Util;var S=this.getSpacing();var Y=this.getSeparator();if(Y){var H=W.computeHorizontalSeparatorGaps(K,S,Y);}
...
;var bs=qx.ui.layout.Util;var bf=this.__fn;var bk=0,bl=0,be=0;var bi=0,bj=0;var bp,bg,br;for(var i=0,l=bf.length;i<l;i+=1){bp=bf[i];bg=bp.getSizeHint();bl+=bg.width;var bo=this.__lJ[i];var bh=this.__mq[i];if(bo){bk+=bg.minWidth;}
...
},destruct:function(){this.__mq=this.__lJ=this.__fn=null;}
});}
)();
...
(function(){var a="qx.dyntheme",b="backgroundColor",c="_applyOpacity",d="Boolean",f="px",g="keydown",h="deactivate",j="changeTheme",k="opacity",l="Tab",m="qx.event.type.Event",n="move",o="__ij",p="Color",q="resize",r="zIndex",s="__la",t="appear",u="qx.ui.root.Abstract",v="keyup",w="keypress",x="Number",y="unblocked",z="qx.ui.core.Blocker",A="disappear",B="blocked",C="_applyColor";qx.Class.define(z,{extend:qx.core.Object,events:{blocked:m,unblocked:m},construct:function(D){qx.core.Object.call(this);this._widget=D;D.addListener(q,this.__lf,this);D.addListener(n,this.__lf,this);D.addListener(A,this.__lh,this);if(qx.Class.isDefined(u)&&D instanceof qx.ui.root.Abstract){this._isRoot=true;this.setKeepBlockerActive(true);}
;if(qx.core.Environment.get(a)){qx.theme.manager.Color.getInstance().addListener(j,this._onChangeTheme,this);}
;this.__lb=[];this.__lc=[];}
,properties:{color:{check:p,init:null,nullable:true,apply:C,themeable:true},opacity:{check:x,init:1,apply:c,themeable:true},keepBlockerActive:{check:d,init:false}},members:{__la:null,__ld:0,__lb:null,__lc:null,__ij:null,_widget:null,_isRoot:false,__le:null,__lf:function(e){var E=e.getData();if(this.isBlocked()){this._updateBlockerBounds(E);}
;}
...
;}
,__lh:function(){if(this.isBlocked()){this.getBlockerElement().getParent().remove(this.getBlockerElement());this._widget.addListenerOnce(t,this.__lg,this);}
;}
...
,_applyColor:function(I,H){var G=qx.theme.manager.Color.getInstance().resolve(I);this.__li(b,G);}
,_applyOpacity:function(K,J){this.__li(k,K);}
,_onChangeTheme:qx.core.Environment.select(a,{"true":function(){this._applyColor(this.getColor());}
,"false":null}),__li:function(M,N){var L=[];this.__la&&L.push(this.__la);for(var i=0;i<L.length;i++ ){L[i].setStyle(M,N);}
;}
,_backupActiveWidget:function(){var O=qx.event.Registration.getManager(window).getHandler(qx.event.handler.Focus);this.__lb.push(O.getActive());this.__lc.push(O.getFocus());if(this._widget.isFocusable()){this._widget.focus();}
;}
,_restoreActiveWidget:function(){var R=this.__lb.length;if(R>0){var Q=this.__lb[R-1];if(Q){qx.bom.Element.activate(Q);}
;this.__lb.pop();}
;var P=this.__lc.length;if(P>0){var Q=this.__lc[P-1];if(Q){qx.bom.Element.focus(this.__lc[P-1]);}
;this.__lc.pop();}
;}
,__lj:function(){return new qx.html.Blocker(this.getColor(),this.getOpacity());}
,getBlockerElement:function(S){if(!this.__la){this.__la=this.__lj();this.__la.setStyle(r,15);if(!S){if(this._isRoot){S=this._widget;}
else {S=this._widget.getLayoutParent();}
;}
;S.getContentElement().add(this.__la);this.__la.exclude();}
;return this.__la;}
,block:function(){this._block();}
,_block:function(T,V){if(!this._isRoot&&!this._widget.getLayoutParent()){this.__le=this._widget.addListenerOnce(t,this._block.bind(this,T));return;}
...
;this.__ld++ ;if(this.__ld<2){this._backupActiveWidget();var W=this._widget.getBounds();if(W){this._updateBlockerBounds(W);}
;U.include();if(!V){U.activate();}
;U.addListener(h,this.__lm,this);U.addListener(w,this.__ll,this);U.addListener(g,this.__ll,this);U.addListener(v,this.__ll,this);this.fireEvent(B,qx.event.type.Event);}
;}
...
,__lk:function(){this._restoreActiveWidget();var X=this.getBlockerElement();X.removeListener(h,this.__lm,this);X.removeListener(w,this.__ll,this);X.removeListener(g,this.__ll,this);X.removeListener(v,this.__ll,this);X.exclude();this.fireEvent(y,qx.event.type.Event);}
,blockContent:function(Y){this._block(Y,true);}
,__ll:function(e){if(e.getKeyIdentifier()==l){e.stop();}
;}
...
;this._widget.removeListener(q,this.__lf,this);this._widget.removeListener(n,this.__lf,this);this._widget.removeListener(t,this.__lg,this);this._widget.removeListener(A,this.__lh,this);if(this.__le){this._widget.removeListenerById(this.__le);}
;this._disposeObjects(s,o);this.__lb=this.__lc=this._widget=null;}
});}
)();
...
(function(){var a='<div>',b="</div>",c="overflow: hidden;",d="qx.event.type.Data",e='</div>',f="paneReloadsData",g="div",h='style="',j="_applyMaxCacheLines",k="qx.ui.table.pane.Pane",l="width: 100%;",m="qx.event.type.Event",n="_applyVisibleRowCount",o='>',p="",q='" ',r="line-height: ",s="paneUpdated",t="appear",u='class="',v="width:100%;",w="px;",z='<div ',A="'>",B="Number",C="_applyFirstVisibleRow",D="<div style='",E=";position:relative;";qx.Class.define(k,{extend:qx.ui.core.Widget,construct:function(F){qx.ui.core.Widget.call(this);this.__pC=F;this.__pD=0;this.__ol=0;this.__pE=[];}
,events:{"paneReloadsData":d,"paneUpdated":m},properties:{firstVisibleRow:{check:B,init:0,apply:C},visibleRowCount:{check:B,init:0,apply:n},maxCacheLines:{check:B,init:1000,apply:j},allowShrinkX:{refine:true,init:false}},members:{__ol:null,__pD:null,__pC:null,__pF:null,__oi:null,__oh:null,__pE:null,__pG:0,_applyFirstVisibleRow:function(H,G){this.updateContent(false,H-G);}
,_applyVisibleRowCount:function(J,I){this.updateContent(true);}
,_getContentHint:function(){return {width:this.getPaneScroller().getTablePaneModel().getTotalWidth(),height:400};}
,getPaneScroller:function(){return this.__pC;}
,getTable:function(){return this.__pC.getTable();}
,setFocusedCell:function(M,L,K){if(M!=this.__oh||L!=this.__oi){var N=this.__oi;this.__oh=M;this.__oi=L;if(L!=N&&!K){if(N!==null){this.updateContent(false,null,N,true);}
...
,onTableModelDataChanged:function(T,V,R,Q){this.__pH();var S=this.getFirstVisibleRow();var U=this.getVisibleRowCount();if(V==-1||V>=S&&T<S+U){this.updateContent();}
;}
,onTableModelMetaDataChanged:function(){this.updateContent(true);}
,_applyMaxCacheLines:function(X,W){if(this.__pG>=X&&X!==-1){this.__pH();}
;}
,__pH:function(){this.__pE=[];this.__pG=0;}
,__pI:function(bb,Y,ba){if(!Y&&!ba&&this.__pE[bb]){return this.__pE[bb];}
else {return null;}
;}
,__pJ:function(bg,bf,bd,be){var bc=this.getMaxCacheLines();if(!bd&&!be&&!this.__pE[bg]&&bc>0){this._applyMaxCacheLines(bc);this.__pE[bg]=bf;this.__pG+=1;}
;}
,updateContent:function(bh,bk,bj,bi){if(bh){this.__pH();}
...
;bM.push(e);var bP=bM.join(p);this.__pJ(bC,bP,bF,bJ);bX.push(bP);}
...
;}
;if(!this.__pF){this.__pF=document.createElement(g);}
;var cj=a;cj+=this._getRowsHtml(ch+cd,Math.abs(cm));cj+=e;this.__pF.innerHTML=cj;var ci=this.__pF.firstChild.childNodes;if(cm>0){for(var i=ci.length-1;i>=0;i-- ){var cg=ci[0];ck.appendChild(cg);}
;}
else {for(var i=ci.length-1;i>=0;i-- ){var cg=ci[ci.length-1];ck.insertBefore(cg,ck.firstChild);}
;}
;if(this.__oi!==null){this._updateRowStyles(this.__oi-cm);this._updateRowStyles(this.__oi);}
;this.fireEvent(s);}
...
;var cA=cu.join(p);cs.innerHTML=cA;this.setWidth(cr);this.__pD=cw;this.__ol=cq;this.fireEvent(s);}
},destruct:function(){this.__pF=this.__pC=this.__pE=null;}
});}
)();
...
(function(){var a="' must be defined!",b="height",c="vAlign",d="hAlign",e="' is not supported by the Grid layout!",f="bottom",g="Invalid parameter 'column'",h="Integer",m="The property '",n="'",o="_applyLayoutChange",p="qx.debug",q="Value must be positive",r="center",s="qx.ui.layout.Grid",t="middle",u="maxHeight",v="Cannot add widget '",w="width",z=") for '",A="'!. ",B="top",C="minHeight",D="right",E="' in this cell (",F=", ",G="The layout properties 'row' and 'column' of the child widget '",H="minWidth",I="flex",J="left",K="maxWidth",L="Invalid parameter 'row'",M="There is already a widget '";qx.Class.define(s,{extend:qx.ui.layout.Abstract,construct:function(O,N){qx.ui.layout.Abstract.call(this);this.__nf=[];this.__ng=[];if(O){this.setSpacingX(O);}
;if(N){this.setSpacingY(N);}
;}
,properties:{spacingX:{check:h,init:0,apply:o},spacingY:{check:h,init:0,apply:o}},members:{__nh:null,__nf:null,__ng:null,__ni:null,__nj:null,__nk:null,__nl:null,__nm:null,__nn:null,verifyLayoutProperty:qx.core.Environment.select(p,{"true":function(P,name,Q){var R={"row":1,"column":1,"rowSpan":1,"colSpan":1};this.assert(R[name]==1,m+name+e);this.assertInteger(Q);this.assert(Q>=0,q);}
,"false":null}),__no:function(){var Y=[];var T=[];var W=[];var V=-1;var S=-1;var X=this._getLayoutChildren();for(var i=0,l=X.length;i<l;i++ ){var ba=X[i];var bb=ba.getLayoutProperties();var bc=bb.row;var U=bb.column;bb.colSpan=bb.colSpan||1;bb.rowSpan=bb.rowSpan||1;if(bc==null||U==null){throw new Error(G+ba+a);}
...
;}
;this.__nh=Y;this.__ni=T;this.__nj=W;this.__nk=V;this.__nl=S;this.__nm=null;this.__nn=null;delete this._invalidChildrenCache;}
,_setRowData:function(bg,be,bf){var bd=this.__nf[bg];if(!bd){this.__nf[bg]={};this.__nf[bg][be]=bf;}
else {bd[be]=bf;}
;}
,_setColumnData:function(bh,bj,bk){var bi=this.__ng[bh];if(!bi){this.__ng[bh]={};this.__ng[bh][bj]=bk;}
else {bi[bj]=bk;}
;}
...
,getColumnAlign:function(bp){var bq=this.__ng[bp]||{};return {vAlign:bq.vAlign||B,hAlign:bq.hAlign||J};}
...
,getRowAlign:function(bv){var bu=this.__nf[bv]||{};return {vAlign:bu.vAlign||B,hAlign:bu.hAlign||J};}
,getCellWidget:function(bx,bw){if(this._invalidChildrenCache){this.__no();}
;var bx=this.__nh[bx]||{};return bx[bw]||null;}
,getRowCount:function(){if(this._invalidChildrenCache){this.__no();}
;return this.__nk+1;}
,getColumnCount:function(){if(this._invalidChildrenCache){this.__no();}
;return this.__nl+1;}
,getCellAlign:function(bF,bz){var bE=B;var bC=J;var bD=this.__nf[bF];var bA=this.__ng[bz];var by=this.__nh[bF][bz];if(by){var bB={vAlign:by.getAlignY(),hAlign:by.getAlignX()};}
else {bB={};}
...
,getColumnFlex:function(bI){var bJ=this.__ng[bI]||{};return bJ.flex!==undefined?bJ.flex:0;}
,setRowFlex:function(bL,bK){this._setRowData(bL,I,bK);this._applyLayoutChange();return this;}
,getRowFlex:function(bO){var bM=this.__nf[bO]||{};var bN=bM.flex!==undefined?bM.flex:0;return bN;}
,setColumnMaxWidth:function(bP,bQ){this._setColumnData(bP,K,bQ);this._applyLayoutChange();return this;}
,getColumnMaxWidth:function(bR){var bS=this.__ng[bR]||{};return bS.maxWidth!==undefined?bS.maxWidth:Infinity;}
,setColumnWidth:function(bT,bU){this._setColumnData(bT,w,bU);this._applyLayoutChange();return this;}
,getColumnWidth:function(bV){var bW=this.__ng[bV]||{};return bW.width!==undefined?bW.width:null;}
,setColumnMinWidth:function(bX,bY){this._setColumnData(bX,H,bY);this._applyLayoutChange();return this;}
,getColumnMinWidth:function(ca){var cb=this.__ng[ca]||{};return cb.minWidth||0;}
,setRowMaxHeight:function(cd,cc){this._setRowData(cd,u,cc);this._applyLayoutChange();return this;}
,getRowMaxHeight:function(cf){var ce=this.__nf[cf]||{};return ce.maxHeight||Infinity;}
,setRowHeight:function(cg,ch){this._setRowData(cg,b,ch);this._applyLayoutChange();return this;}
,getRowHeight:function(cj){var ci=this.__nf[cj]||{};return ci.height!==undefined?ci.height:null;}
,setRowMinHeight:function(cl,ck){this._setRowData(cl,C,ck);this._applyLayoutChange();return this;}
,getRowMinHeight:function(cn){var cm=this.__nf[cn]||{};return cm.minHeight||0;}
,__np:function(cp){var co=cp.getSizeHint();var cr=cp.getMarginLeft()+cp.getMarginRight();var cq=cp.getMarginTop()+cp.getMarginBottom();var cs={height:co.height+cq,width:co.width+cr,minHeight:co.minHeight+cq,minWidth:co.minWidth+cr,maxHeight:co.maxHeight+cq,maxWidth:co.maxWidth+cr};return cs;}
,_fixHeightsRowSpan:function(cN){var cz=this.getSpacingY();for(var i=0,l=this.__nj.length;i<l;i++ ){var cF=this.__nj[i];var cC=this.__np(cF);var cv=cF.getLayoutProperties();var cB=cv.row;var cK=cz*(cv.rowSpan-1);var ct=cK;var cw={};for(var j=0;j<cv.rowSpan;j++ ){var cA=cv.row+j;var cJ=cN[cA];var cL=this.getRowFlex(cA);if(cL>0){cw[cA]={min:cJ.minHeight,value:cJ.height,max:cJ.maxHeight,flex:cL};}
...
;}
;}
,_fixWidthsColSpan:function(cR){var cS=this.getSpacingX();for(var i=0,l=this.__ni.length;i<l;i++ ){var cO=this.__ni[i];var cQ=this.__np(cO);var cU=cO.getLayoutProperties();var cP=cU.column;var db=cS*(cU.colSpan-1);var cT=db;var cV={};var cX;for(var j=0;j<cU.colSpan;j++ ){var cW=cU.column+j;var da=cR[cW];var cY=this.getColumnFlex(cW);if(cY>0){cV[cW]={min:da.minWidth,value:da.width,max:da.maxWidth,flex:cY};}
...
;}
;}
,_getRowHeights:function(){if(this.__nm!=null){return this.__nm;}
;var dm=[];var df=this.__nk;var de=this.__nl;for(var dn=0;dn<=df;dn++ ){var dg=0;var di=0;var dh=0;for(var dl=0;dl<=de;dl++ ){var dd=this.__nh[dn][dl];if(!dd){continue;}
;var dj=dd.getLayoutProperties().rowSpan||0;if(dj>1){continue;}
;var dk=this.__np(dd);if(this.getRowFlex(dn)>0){dg=Math.max(dg,dk.minHeight);}
...
;if(this.__nj.length>0){this._fixHeightsRowSpan(dm);}
;this.__nm=dm;return dm;}
,_getColWidths:function(){if(this.__nn!=null){return this.__nn;}
;var dt=[];var dq=this.__nl;var ds=this.__nk;for(var dy=0;dy<=dq;dy++ ){var dw=0;var dv=0;var dr=Infinity;for(var dz=0;dz<=ds;dz++ ){var dp=this.__nh[dz][dy];if(!dp){continue;}
;var du=dp.getLayoutProperties().colSpan||0;if(du>1){continue;}
;var dx=this.__np(dp);if(this.getColumnFlex(dy)>0){dv=Math.max(dv,dx.minWidth);}
...
;if(this.__ni.length>0){this._fixWidthsColSpan(dt);}
;this.__nn=dt;return dt;}
...
,renderLayout:function(ej,dO,ei){if(this._invalidChildrenCache){this.__no();}
;var ed=qx.ui.layout.Util;var dQ=this.getSpacingX();var dW=this.getSpacingY();var eh=this._getColWidths();var ek=this._getColumnFlexOffsets(ej);var dR=[];var em=this.__nl;var dP=this.__nk;var el;for(var en=0;en<=em;en++ ){el=ek[en]?ek[en].offset:0;dR[en]=eh[en].width+el;}
...
;var er=0;for(var en=0;en<=em;en++ ){var top=0;for(var dX=0;dX<=dP;dX++ ){var ef=this.__nh[dX][en];if(!ef){top+=et[dX]+dW;continue;}
...
;er+=dR[en]+dQ;}
;}
,invalidateLayoutCache:function(){qx.ui.layout.Abstract.prototype.invalidateLayoutCache.call(this);this.__nn=null;this.__nm=null;}
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__no();}
...
},destruct:function(){this.__nh=this.__nf=this.__ng=this.__ni=this.__nj=this.__nn=this.__nm=null;}
});}
)();
...
(function(){var a="blur",b="mousedown",c="qx.debug",d="qx.ui.popup.Manager",f="Object is no popup: ",g="__os",h="singleton";qx.Class.define(d,{type:h,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__os=[];qx.event.Registration.addListener(document.documentElement,b,this.__pu,this,true);qx.bom.Element.addListener(window,a,this.hideAll,this);}
,members:{__os:null,add:function(j){if(qx.core.Environment.get(c)){if(!(j instanceof qx.ui.popup.Popup)){throw new Error(f+j);}
;}
;this.__os.push(j);this.__pt();}
,remove:function(k){if(qx.core.Environment.get(c)){if(!(k instanceof qx.ui.popup.Popup)){throw new Error(f+k);}
;}
;qx.lang.Array.remove(this.__os,k);this.__pt();}
,hideAll:function(){var l=this.__os.length,m={};while(l-- ){m=this.__os[l];if(m.getAutoHide()){m.exclude();}
;}
;}
,__pt:function(){var n=1e7;for(var i=0;i<this.__os.length;i++ ){this.__os[i].setZIndex(n++ );}
;}
,__pu:function(e){var p=qx.ui.core.Widget.getWidgetByElement(e.getTarget());var q=this.__os;for(var i=0;i<q.length;i++ ){var o=q[i];if(!o.getAutoHide()||p==o||qx.ui.core.Widget.contains(o,p)){continue;}
;o.exclude();}
;}
},destruct:function(){qx.event.Registration.removeListener(document.documentElement,b,this.__pu,this,true);this._disposeArray(g);}
});}
)();
...
;qx.ui.table.cellrenderer.Abstract.__oV.stylesheet=qx.bom.Stylesheet.createElement(N);}
...
(function(){var a="-",b="Boolean",c="^",d="Number string '",e="' does not match the number format",f="Wrong argument type. String is expected.",g="0",h="NaN",i='(',j="-Infinity",k="String",l='{0,1}[0-9]{3}){0,})',m="",n='([0-9]{1,3}(?:',o="qx.util.format.NumberFormat",p='([-+]){0,1}',q="g",r="Wrong number of arguments.",s="Number",t="$",u='\\d+){0,1}',v=".",w="changeNumberFormat",x="Infinity";qx.Class.define(o,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(y){qx.core.Object.call(this);if(arguments.length>0){if(arguments.length===1){if(qx.lang.Type.isString(y)){this.__jp=y;}
else {throw new Error(f);}
;}
else {throw new Error(r);}
;}
;}
,properties:{minimumIntegerDigits:{check:s,init:0},maximumIntegerDigits:{check:s,nullable:true},minimumFractionDigits:{check:s,init:0},maximumFractionDigits:{check:s,nullable:true},groupingUsed:{check:b,init:true},prefix:{check:k,init:m,event:w},postfix:{check:k,init:m,event:w}},members:{__jp:null,format:function(E){switch(E){case Infinity:return x;case -Infinity:return j;case NaN:return h;};var C=(E<0);if(C){E=-E;}
;if(this.getMaximumFractionDigits()!=null){var K=Math.pow(10,this.getMaximumFractionDigits());E=Math.round(E*K)/K;}
;var I=String(Math.floor(E)).length;var z=m+E;var G=z.substring(0,I);while(G.length<this.getMinimumIntegerDigits()){G=g+G;}
;if(this.getMaximumIntegerDigits()!=null&&G.length>this.getMaximumIntegerDigits()){G=G.substring(G.length-this.getMaximumIntegerDigits());}
;var F=z.substring(I+1);while(F.length<this.getMinimumFractionDigits()){F+=g;}
;if(this.getMaximumFractionDigits()!=null&&F.length>this.getMaximumFractionDigits()){F=F.substring(0,this.getMaximumFractionDigits());}
;if(this.getGroupingUsed()){var B=G;G=m;var J;for(J=B.length;J>3;J-=3){G=m+qx.locale.Number.getGroupSeparator(this.__jp)+B.substring(J-3,J)+G;}
;G=B.substring(0,J)+G;}
;var D=this.getPrefix()?this.getPrefix():m;var A=this.getPostfix()?this.getPostfix():m;var H=D+(C?a:m)+G;if(F.length>0){H+=m+qx.locale.Number.getDecimalSeparator(this.__jp)+F;}
;H+=A;return H;}
,parse:function(T){var P=qx.lang.String.escapeRegexpChars(qx.locale.Number.getGroupSeparator(this.__jp)+m);var N=qx.lang.String.escapeRegexpChars(qx.locale.Number.getDecimalSeparator(this.__jp)+m);var L=new RegExp(c+qx.lang.String.escapeRegexpChars(this.getPrefix())+p+n+P+l+i+N+u+qx.lang.String.escapeRegexpChars(this.getPostfix())+t);var O=L.exec(T);if(O==null){throw new Error(d+T+e);}
;var Q=(O[1]==a);var S=O[2];var R=O[3];S=S.replace(new RegExp(P,q),m);var M=(Q?a:m)+S;if(R!=null&&R.length!=0){R=R.replace(new RegExp(N),m);M+=v+R;}
;return parseFloat(M);}
}});}
)();
...
(function(){var a="-",b="QQQ",c='W',d="qqqq",e="h",f="KK",g='L',h=" is not supported",m="Z",n="Date string '",o="GGGGG",p='abbreviated',q="Y+",r=". Wildcard ",s="eeeee",t="(\\d\\d?)",u="LL",v="abbreviated",w="EEEE",x="^",y=":",z='y',A="quoted_literal",B="weekOfYear",C='3rd quarter',D="SSS",E='a',F="day",G="(-*",H="qqq",I="locale",J="K",K='q',L="Provided date is invalid",M="pm",N="Error parsing date '",O='Anno Domini',P='G',Q="a",R='2nd quarter',S="yyyy-MM-dd'T'HH:mm:ss",T="GMT",U="HH:mm:ss",V="dd",W="+",X="qx.util.format.DateFormat",Y='weekDay',en="am",ei="eee",eo="quarter",ek="ee",el="q",eh="QQQQ",em="HHmmss",es="ww",et="H",eu='Y+',ev="wide",ep='wide',eq="(\\d\\d\\d?)",ej="(GMT[\\+\\-]\\d\\d:\\d\\d)",er="_applyLocale",ez="Q",fd="c",eA="(",eB="weekOfMonth",ew="ccc",ex="long",ge="Malformed date format: ",ey="EEEEE",eC='Y',eD="EE",eE="GGG",eJ='-',eK="w",eL="mm",eF='h',eG="S",eH='s',eI='A',eQ="yyyy-MM-dd",eR="G",eS='z',eT="GG",eM=")",eN="kk",eO="ss",eP="([\\+\\-]\\d\\d\\d\\d)",eX="default",eY='BC',gi='H',fa='S',eU='1st quarter',eV="stand-alone",gg="0",eW="MMMM",fb='c',fc="d",fo='Q',fn='Q1',fm="eeee",fs="QQ",fr="LLLLL",fq="HH",fp='k',fh="m",fg="DDD",ff="D",fe='D',fl="LLLL",fk="(\\d\\d*?)",fj='e',fi="': the value for day or month is too large",fz="literal",fy='K',fx="L",fw="hh",fD="ms",fC="(\\d\\d?\\d?)",fB="cccc",fA="e",fv='y+',fu="|",ft="W",fO="dayOfYear",fN="GGGG",fM='Q2',fS='B',fR="MM",fQ='Q4',fP="y+",fH='narrow',fG="ccccc",fF="E",fE="wildcard",fL='',fK="' does not match the date format: ",fJ="short",fI='d',fY="unkown",fX="narrow",fW="\\d?",fV="'",gd="k",gc='Before Christ',gb='Z',ga="qx.debug",fU='m',fT=" ",dQ="yyyy-MM-dd'T'HH:mm:ss'Z'",dP="String",gj='Q3',dN="z",dO="DD",dM="min",gh="",dK='M',dL="sec",dJ='isoUtcDateTime',gf='4th quarter',dH="(\\d?)",dI="SS",dG="MMM",ea="s",eb="M",dX='w',dY="EEE",dV="$",dW="?",dU='AD',dF="\\d",dS="+?",dT="hour",dR="MMMMM",eg='E',ee='month',ef="LLL",ec="qq",ed="format";qx.Class.define(X,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(gl,gk){qx.core.Object.call(this);if(!gk){this.__jp=qx.locale.Manager.getInstance().getLocale();this.__oW=qx.locale.Manager.getInstance().bind(I,this,I);}
else {this.__jp=gk;this.setLocale(gk);}
;this.__oX=this.__jp;if(gl!=null){this.__oY=gl.toString();if(this.__oY in qx.util.format.DateFormat.ISO_MASKS){if(this.__oY===dJ){this.__pa=true;}
;this.__oY=qx.util.format.DateFormat.ISO_MASKS[this.__oY];}
;}
else {this.__oY=qx.locale.Date.getDateFormat(ex,this.__jp)+fT+qx.locale.Date.getDateTimeFormat(em,U,this.__jp);}
;}
,properties:{locale:{apply:er,nullable:true,check:dP}},statics:{getDateInstance:function(){var gm=qx.util.format.DateFormat;var gn=qx.locale.Date.getDateFormat(fJ)+gh;if(gm._dateInstance==null||gm._dateInstance.__oY!=gn){gm._dateInstance=new gm(gn);}
;return gm._dateInstance;}
,getDateTimeInstance:function(){var go=qx.util.format.DateFormat;var gp=qx.locale.Date.getDateFormat(ex)+fT+qx.locale.Date.getDateTimeFormat(em,U);if(go._dateTimeInstance==null||go._dateTimeInstance.__oY!=gp){go._dateTimeInstance=new go(gp);}
;return go._dateTimeInstance;}
,ASSUME_YEAR_2000_THRESHOLD:30,ISO_MASKS:{isoDate:eQ,isoTime:U,isoDateTime:S,isoUtcDateTime:dQ},AM_MARKER:en,PM_MARKER:M},members:{__oW:null,__jp:null,__oX:null,__oY:null,__pb:null,__pc:null,__pd:null,__pa:null,__pe:function(gr,gq){var gs=gh+(gr<0?((-1)*gr):gr);while(gs.length<gq){gs=gg+gs;}
;return gr<0?a+gs:gs;}
,__pf:function(gt){var gu=new Date(gt.getTime());var gv=gu.getDate();while(gu.getMonth()!=0){gu.setDate(-1);gv+=gu.getDate()+1;}
;return gv;}
,__pg:function(gw){return new Date(gw.getTime()+(3-((gw.getDay()+6)%7))*86400000);}
,__ph:function(gx){var gz=this.__pg(gx);var gA=gz.getFullYear();var gy=this.__pg(new Date(gA,0,4));return Math.floor(1.5+(gz.getTime()-gy.getTime())/86400000/7);}
,__pi:function(gB){var gD=this.__pg(gB);var gC=this.__pg(new Date(gB.getFullYear(),gB.getMonth(),4));return Math.floor(1.5+(gD.getTime()-gC.getTime())/86400000/7);}
,__pj:function(gE){var gF=this.__pg(gE);return gF.getFullYear();}
,__pk:function(gH){var gG=new Date(gH,2,1);gG.setDate(-1);return gG.getDate()+1===29;}
,__pl:function(gM,gN){var gL=0;var gO=0;if(!gN){gN=1971;}
...
,__pm:function(gS,gQ,gP){var gR;switch(gQ){case 11:gR=gS-1;if(gS!=this.__pj(new Date(gR,gQ,gP))){gR=gS;}
;break;case 0:gR=gS+1;if(gS!=this.__pj(new Date(gR,gQ,gP))){gR=gS;}
;break;default:gR=gS;};return gR;}
,_applyLocale:function(gU,gT){this.__jp=gU===null?this.setLocale(this.__oX):gU;}
,format:function(hl){if(hl==null){return null;}
;if(isNaN(hl.getTime())){if(qx.core.Environment.get(ga)){qx.log.Logger.error(L);}
;return null;}
;if(this.__pa){hl=new Date(hl.getUTCFullYear(),hl.getUTCMonth(),hl.getUTCDate(),hl.getUTCHours(),hl.getUTCMinutes(),hl.getUTCSeconds(),hl.getUTCMilliseconds());}
;var gX=this.__jp;var hd=hl.getFullYear();var gY=hl.getMonth();var hc=hl.getDate();var hg=hl.getDay();var hj=hl.getHours();var hq=hl.getMinutes();var hh=hl.getSeconds();var ho=hl.getMilliseconds();var hf=hl.getTimezoneOffset();var hk=hf>0?1:-1;var gV=Math.floor(Math.abs(hf)/60);var hn=Math.abs(hf)%60;this.__pn();var hp=gh;for(var i=0;i<this.__pd.length;i++ ){var he=this.__pd[i];if(he.type==fz){hp+=he.text;}
else {var gW=he.character;var hi=he.size;var hr=dW;switch(gW){case z:if(hi==2){hr=this.__pe(hd%100,2);}
else {var ha=Math.abs(hd);hr=ha+gh;if(hi>hr.length){for(var j=hr.length;j<hi;j++ ){hr=gg+hr;}
;}
;if(hd<0){hr=a+hr;}
;}
;break;case eC:hr=this.__pj(hl)+gh;var ha=hr.replace(eJ,fL);if(hi>hr.length){for(var j=ha.length;j<hi;j++ ){ha=gg+ha;}
;}
;hr=hr.indexOf(a)!=-1?a+ha:ha;break;case P:if(hi>=1&&hi<=3){hr=hd>0?dU:eY;}
else if(hi==4){hr=hd>0?O:gc;}
else if(hi==5){hr=hd>0?eI:fS;}
;break;case fo:if(hi==1||hi==2){hr=this.__pe(parseInt(gY/4)+1,hi);}
;if(hi==3){hr=fo+(parseInt(gY/4)+1);}
;break;case K:if(hi==1||hi==2){hr=this.__pe(parseInt(gY/4)+1,hi);}
;if(hi==3){hr=fo+(parseInt(gY/4)+1);}
;break;case fe:hr=this.__pe(this.__pf(hl),hi);break;case fI:hr=this.__pe(hc,hi);break;case dX:hr=this.__pe(this.__ph(hl),hi);break;case c:hr=this.__pi(hl);break;case eg:if(hi>=1&&hi<=3){hr=qx.locale.Date.getDayName(v,hg,gX,ed,true);}
else if(hi==4){hr=qx.locale.Date.getDayName(ev,hg,gX,ed,true);}
else if(hi==5){hr=qx.locale.Date.getDayName(fX,hg,gX,ed,true);}
;break;case fj:var hb=qx.locale.Date.getWeekStart(gX);var hm=1+((hg-hb>=0)?(hg-hb):7+(hg-hb));if(hi>=1&&hi<=2){hr=this.__pe(hm,hi);}
...
;break;case dK:if(hi==1||hi==2){hr=this.__pe(gY+1,hi);}
...
;break;case g:if(hi==1||hi==2){hr=this.__pe(gY+1,hi);}
...
;break;case E:hr=(hj<12)?qx.locale.Date.getAmMarker(gX):qx.locale.Date.getPmMarker(gX);break;case gi:hr=this.__pe(hj,hi);break;case fp:hr=this.__pe((hj==0)?24:hj,hi);break;case fy:hr=this.__pe(hj%12,hi);break;case eF:hr=this.__pe(((hj%12)==0)?12:(hj%12),hi);break;case fU:hr=this.__pe(hq,hi);break;case eH:hr=this.__pe(hh,hi);break;case fa:hr=this.__pe(ho,3);if(hi<hr.length){hr=hr.substr(0,hi);}
else {while(hi>hr.length){hr+=gg;}
;}
;break;case eS:if(hi>=1&&hi<=4){hr=T+((hk>0)?a:W)+this.__pe(Math.abs(gV),2)+y+this.__pe(hn,2);}
;break;case gb:if(hi>=1&&hi<=3){hr=((hk>0)?a:W)+this.__pe(Math.abs(gV),2)+this.__pe(hn,2);}
else {hr=T+((hk>0)?a:W)+this.__pe(Math.abs(gV),2)+y+this.__pe(hn,2);}
;break;};hp+=hr;}
;}
;return hp;}
,parse:function(hy){this.__po();var hB=this.__pb.regex.exec(hy);if(hB==null){throw new Error(n+hy+fK+this.__oY);}
;var hs={era:1,year:1970,quarter:1,month:0,day:1,dayOfYear:1,hour:0,ispm:false,weekDay:4,weekYear:1970,weekOfMonth:1,weekOfYear:1,min:0,sec:0,ms:0};var hw=1;var ht=false;var hu=false;for(var i=0;i<this.__pb.usedRules.length;i++ ){var hv=this.__pb.usedRules[i];var hx=hB[hw];if(hv.field!=null){hs[hv.field]=parseInt(hx,10);}
else {hv.manipulator(hs,hx,hv.pattern);}
;if(hv.pattern==q){var hz=false;for(var k=0;k<this.__pb.usedRules.length;k++ ){if(this.__pb.usedRules[k].pattern==fv){hz=true;break;}
;}
;if(!hz){ht=true;}
;}
;if(hv.pattern.indexOf(ff)!=-1){var hD=false;for(var k=0;k<this.__pb.usedRules.length;k++ ){if(this.__pb.usedRules[k].pattern.indexOf(fc)!=-1){hD=true;break;}
;}
;if(!hD){hu=true;}
;}
;hw+=(hv.groups==null)?1:hv.groups;}
;if(ht){hs.year=this.__pm(hs.weekYear,hs.month,hs.day);}
;if(hu){var hC=this.__pl(hs.dayOfYear,hs.year);hs.month=hC.month;hs.day=hC.day;}
;if(hs.era<0&&(hs.year*hs.era<0)){hs.year=hs.year*hs.era;}
;var hA=new Date(hs.year,hs.month,hs.day,(hs.ispm)?(hs.hour+12):hs.hour,hs.min,hs.sec,hs.ms);if(this.__pa){hA=new Date(hA.getUTCFullYear(),hA.getUTCMonth(),hA.getUTCDate(),hA.getUTCHours(),hA.getUTCMinutes(),hA.getUTCSeconds(),hA.getUTCMilliseconds());}
;if(hs.month!=hA.getMonth()||hs.year!=hA.getFullYear()){throw new Error(N+hy+fi);}
;return hA;}
,__pn:function(){if(this.__pd!=null){return;}
;this.__pd=[];var hI;var hG=0;var hK=gh;var hE=this.__oY;var hH=eX;var i=0;while(i<hE.length){var hJ=hE.charAt(i);switch(hH){case A:if(hJ==fV){if(i+1>=hE.length){i++ ;break;}
;var hF=hE.charAt(i+1);if(hF==fV){hK+=hJ;i++ ;}
else {i++ ;hH=fY;}
;}
else {hK+=hJ;i++ ;}
;break;case fE:if(hJ==hI){hG++ ;i++ ;}
else {this.__pd.push({type:fE,character:hI,size:hG});hI=null;hG=0;hH=eX;}
;break;default:if((hJ>=E&&hJ<=eS)||(hJ>=eI&&hJ<=gb)){hI=hJ;hH=fE;}
else if(hJ==fV){if(i+1>=hE.length){hK+=hJ;i++ ;break;}
;var hF=hE.charAt(i+1);if(hF==fV){hK+=hJ;i++ ;}
;i++ ;hH=A;}
else {hH=eX;}
;if(hH!=eX){if(hK.length>0){this.__pd.push({type:fz,text:hK});hK=gh;}
;}
else {hK+=hJ;i++ ;}
;break;};}
;if(hI!=null){this.__pd.push({type:fE,character:hI,size:hG});}
else if(hK.length>0){this.__pd.push({type:fz,text:hK});}
;}
,__po:function(){if(this.__pb!=null){return;}
;var hO=this.__oY;this.__pq();this.__pn();var hU=[];var hP=x;for(var hM=0;hM<this.__pd.length;hM++ ){var hV=this.__pd[hM];if(hV.type==fz){hP+=qx.lang.String.escapeRegexpChars(hV.text);}
else {var hN=hV.character;var hQ=hV.size;var hS;for(var hW=0;hW<this.__pc.length;hW++ ){var hR=this.__pc[hW];if(this.__pp(hR,hN,hQ)){hS=hR;break;}
;}
...
;this.__pb={regex:hL,"usedRules":hU,pattern:hP};}
,__pp:function(hY,ia,ib){if(ia===z&&hY.pattern===fv){hY.regex=hY.regexFunc(ib);return true;}
else if(ia===eC&&hY.pattern===eu){hY.regex=hY.regexFunc(ib);return true;}
else {return ia==hY.pattern.charAt(0)&&ib==hY.pattern.length;}
;}
,__pq:function(){var ir=qx.util.format.DateFormat;var iu=qx.lang.String;if(this.__pc!=null){return;}
;var is=this.__pc=[];var im=qx.locale.Date.getAmMarker(this.__jp).toString()||ir.AM_MARKER;var iB=qx.locale.Date.getPmMarker(this.__jp).toString()||ir.PM_MARKER;var id=this.__jp;var ij=function(iE,iF){iF=parseInt(iF,10);if(iF>=0){if(iF<ir.ASSUME_YEAR_2000_THRESHOLD){iF+=2000;}
...
},destruct:function(){if(this.__oW!=null){qx.locale.Manager.getInstance().removeBinding(this.__oW);}
;this.__pd=this.__pb=this.__pc=null;}
});}
)();
(function(){var a='format',b="short",c="HH:mm",d='_',e="stand-alone",f="HHmmsszz",g="cldr_date_format_",h="HHmm",j="narrow",k="HHmmss",l="qx.debug",m="This case should never happen.",n="cldr_time_format_",o="sat",p="abbreviated",q="wide",r="HH:mm:ss zz",s="full",t="cldr_pm",u="cldr_day_",v="_",w="cldr_month_",x="long",y="wed",z="medium",A="fri",B="cldr_am",C="tue",D="qx.locale.Date",E="mon",F="cldr_date_time_format_",G="sun",H="format",I="thu",J="HH:mm:ss",K='stand-alone';qx.Class.define(D,{statics:{__pr:qx.locale.Manager.getInstance(),getAmMarker:function(L){return this.__pr.localize(B,[],L);}
,getPmMarker:function(M){return this.__pr.localize(t,[],M);}
...
;var Q=[G,E,C,y,I,A,o];var R=[];for(var i=0;i<Q.length;i++ ){var N=u+S+v+length+v+Q[i];R.push(P?this.__ps(S,S===a?K:a,N,O):this.__pr.localize(N,[],O));}
;return R;}
...
;var W=[G,E,C,y,I,A,o];var T=u+U+v+length+v+W[Y];return X?this.__ps(U,U===a?K:a,T,V):this.__pr.localize(T,[],V);}
...
;var bd=[];for(var i=0;i<12;i++ ){var ba=w+be+v+length+v+(i+1);bd.push(bc?this.__ps(be,be===a?K:a,ba,bb):this.__pr.localize(ba,[],bb));}
;return bd;}
...
;var bf=w+bg+v+length+v+(bi+1);return bj?this.__ps(bg,bg===a?K:a,bf,bh):this.__pr.localize(bf,[],bh);}
,getDateFormat:function(bm,bk){if(qx.core.Environment.get(l)){qx.core.Assert.assertInArray(bm,[b,z,x,s]);}
;var bl=g+bm;return this.__pr.localize(bl,[],bk);}
,getDateTimeFormat:function(br,bp,bn){var bq=F+br;var bo=this.__pr.localize(bq,[],bn);if(bo==bq){bo=bp;}
;return bo;}
,getTimeFormat:function(bv,bs){if(qx.core.Environment.get(l)){qx.core.Assert.assertInArray(bv,[b,z,x,s]);}
;var bu=n+bv;var bt=this.__pr.localize(bu,[],bs);if(bt!=bu){return bt;}
...
else {bK=this.__pr.getTerritory()||this.__pr.getLanguage();}
;return bK.toUpperCase();}
,__ps:function(bN,bM,bQ,bP){var bO=this.__pr.localize(bQ,[],bP);if(bO==bQ){var bL=bQ.replace(d+bN+d,d+bM+d);return this.__pr.localize(bL,[],bP);}
else {return bO;}
;}
}});}
)();
...
(function(){var a="__sA",b="widget",c="qx.ui.tooltip.ToolTip",d="Boolean",f="",g="__sz",h="interval",i="mouseover",j="_applyCurrent",k="mouseout",l="qx.ui.tooltip.Manager",m="mousemove",n="__sC",o="focusout",p="tooltip-error",q="singleton";qx.Class.define(l,{type:q,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);qx.event.Registration.addListener(document.body,i,this.__sH,this,true);this.__sz=new qx.event.Timer();this.__sz.addListener(h,this.__sE,this);this.__sA=new qx.event.Timer();this.__sA.addListener(h,this.__sF,this);this.__sB={left:0,top:0};}
,properties:{current:{check:c,nullable:true,apply:j},showInvalidToolTips:{check:d,init:true},showToolTips:{check:d,init:true}},members:{__sB:null,__sA:null,__sz:null,__sC:null,__sD:null,getSharedTooltip:function(){if(!this.__sC){this.__sC=new qx.ui.tooltip.ToolTip().set({rich:true});}
;return this.__sC;}
,getSharedErrorTooltip:function(){if(!this.__sD){this.__sD=new qx.ui.tooltip.ToolTip().set({appearance:p,rich:true});this.__sD.setLabel(f);this.__sD.syncAppearance();}
;return this.__sD;}
,_applyCurrent:function(t,s){if(s&&qx.ui.core.Widget.contains(s,t)){return;}
;if(s){if(!s.isDisposed()){s.exclude();}
;this.__sz.stop();this.__sA.stop();}
;var u=qx.event.Registration;var r=document.body;if(t){this.__sz.startWith(t.getShowTimeout());u.addListener(r,k,this.__sI,this,true);u.addListener(r,o,this.__sJ,this,true);u.addListener(r,m,this.__sG,this,true);}
else {u.removeListener(r,k,this.__sI,this,true);u.removeListener(r,o,this.__sJ,this,true);u.removeListener(r,m,this.__sG,this,true);}
;}
,__sE:function(e){var v=this.getCurrent();if(v&&!v.isDisposed()){this.__sA.startWith(v.getHideTimeout());if(v.getPlaceMethod()==b){v.placeToWidget(v.getOpener());}
else {v.placeToPoint(this.__sB);}
;v.show();}
;this.__sz.stop();}
,__sF:function(e){var w=this.getCurrent();if(w&&!w.isDisposed()){w.exclude();}
;this.__sA.stop();this.resetCurrent();}
,__sG:function(e){var x=this.__sB;x.left=Math.round(e.getDocumentLeft());x.top=Math.round(e.getDocumentTop());}
,__sH:function(e){var y=qx.ui.core.Widget.getWidgetByElement(e.getTarget());this.showToolTip(y);}
...
,__sI:function(e){var E=qx.ui.core.Widget.getWidgetByElement(e.getTarget());if(!E){return;}
...
;}
,__sJ:function(e){var H=qx.ui.core.Widget.getWidgetByElement(e.getTarget());if(!H){return;}
;var I=this.getCurrent();if(I&&I==H.getToolTip()){this.setCurrent(null);}
;}
},destruct:function(){qx.event.Registration.removeListener(document.body,i,this.__sH,this,true);this._disposeObjects(g,a,n);this.__sB=null;}
});}
)();
...
(function(){var a="_applyActiveWindow",b="changeModal",c="changeVisibility",d="__sK",f="changeActive",g="qx.ui.window.MDesktop",h="__eQ",i="qx.ui.window.Window";qx.Mixin.define(g,{properties:{activeWindow:{check:i,apply:a,init:null,nullable:true}},members:{__sK:null,__eQ:null,getWindowManager:function(){if(!this.__eQ){this.setWindowManager(new qx.ui.window.Window.DEFAULT_MANAGER_CLASS());}
;return this.__eQ;}
...
,getWindows:function(){if(!this.__sK){this.__sK=[];}
;return this.__sK;}
},destruct:function(){this._disposeArray(d);this._disposeObjects(h);}
});}
)();
(function(){var a="_applyBlockerColor",b="Number",c="__la",d="qx.ui.core.MBlocker",e="_applyBlockerOpacity",f="Color";qx.Mixin.define(d,{properties:{blockerColor:{check:f,init:null,nullable:true,apply:a,themeable:true},blockerOpacity:{check:b,init:1,apply:e,themeable:true}},members:{__la:null,_createBlocker:function(){return new qx.ui.core.Blocker(this);}
...
,isBlocked:function(){return this.__la&&this.__la.isBlocked();}
,unblock:function(){if(this.__la){this.__la.unblock();}
;}
,forceUnblock:function(){if(this.__la){this.__la.forceUnblock();}
;}
,blockContent:function(k){this.getBlocker().blockContent(k);}
,getBlocker:function(){if(!this.__la){this.__la=this._createBlocker();}
;return this.__la;}
},destruct:function(){this._disposeObjects(c);}
});}
)();
(function(){var a="changeGlobalCursor",b="engine.name",c="keypress",d="Boolean",f="root",g="help",h="",i="contextmenu",j=" !important",k="input",l="_applyGlobalCursor",m="Space",n="_applyNativeHelp",o=";",p="event.help",q="qx.ui.root.Abstract",r="abstract",s="textarea",t="String",u="*";qx.Class.define(q,{type:r,extend:qx.ui.core.Widget,include:[qx.ui.core.MChildrenHandling,qx.ui.core.MBlocker,qx.ui.window.MDesktop],construct:function(){qx.ui.core.Widget.call(this);qx.ui.core.FocusHandler.getInstance().addRoot(this);qx.ui.core.queue.Visibility.add(this);this.initNativeHelp();this.addListener(c,this.__sM,this);}
,properties:{appearance:{refine:true,init:f},enabled:{refine:true,init:true},focusable:{refine:true,init:true},globalCursor:{check:t,nullable:true,themeable:true,apply:l,event:a},nativeContextMenu:{refine:true,init:false},nativeHelp:{check:d,init:false,apply:n}},members:{__sL:null,isRootWidget:function(){return true;}
,getLayout:function(){return this._getLayout();}
,_applyGlobalCursor:qx.core.Environment.select(b,{"mshtml":function(w,v){}
,"default":function(A,z){var y=qx.bom.Stylesheet;var x=this.__sL;if(!x){this.__sL=x=y.createElement();}
...
;e.preventDefault();}
,__sM:function(e){if(e.getKeyIdentifier()!==m){return;}
...
;}
;}
},destruct:function(){this.__sL=null;}
...
,members:{__qX:null,__qY:null,__ra:null,__rb:null,connectTo:function(m){m.addListener(a,this.__gY,this);m.addListener(j,this._onFocusIn,this,true);m.addListener(b,this._onFocusOut,this,true);m.addListener(c,this._onActivate,this,true);m.addListener(h,this._onDeactivate,this,true);}
...
;}
,__gY:function(e){if(e.getKeyIdentifier()!=d){return;}
;if(!this.__rb){return;}
...
,members:{__cw:null,__sN:null,_createContentElement:function(){var v=this.__sN;if((qx.core.Environment.get(g)==h)){if(!v.body){alert(k);}
;}
...
},destruct:function(){this.__cw=this.__sN=null;}
});}
)();
...
(function(){var a="demobrowser.demo.test.DummyTable",b="main",c="demobrowser.demo.test.Table_Scroller",d="#FFE",e="table";qx.Class.define(c,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var f=new demobrowser.demo.test.DummyTable();this.getRoot().add(f,{left:10,top:10});}
}});qx.Class.define(a,{extend:qx.ui.container.Composite,include:[demobrowser.demo.table.MUtil],construct:function(){qx.ui.container.Composite.call(this,new qx.ui.layout.Canvas());this.setAppearance(e);this.set({decorator:b,backgroundColor:d,width:600,height:400});var h=new qx.ui.table.pane.Scroller(this.getTableMock());this.add(h,{edge:0});var g=new qx.ui.table.pane.Model(this.getColumnModelMock());h.setTablePaneModel(g);h._onPaneModelChanged();h.getTablePane().setVisibleRowCount(20);h.getTablePane()._updateAllRows();}
});}
)();
...
,_getContentHint:function(){var I=qx.bom.element.Scroll.getScrollbarWidth();return {width:this.__nb?100:I,maxWidth:this.__nb?null:I,minWidth:this.__nb?null:I,height:this.__nb?I:100,maxHeight:this.__nb?I:null,minHeight:this.__nb?I:null};}
...
,_applyPosition:function(M){var content=this.getContentElement();if(this.__nb){content.scrollToX(M);}
else {content.scrollToY(M);}
;}
,_applyOrientation:function(P,N){var O=this.__nb=P===w;this.set({allowGrowX:O,allowShrinkX:O,allowGrowY:!O,allowShrinkY:!O});if(O){this.replaceState(m,w);}
...
,_updateScrollBar:function(){var R=this.__nb;var S=this.getBounds();if(!S){return;}
...
,scrollTo:function(V,U){this.stopScrollAnimation();if(U){var W=this.getPosition();this.__ne=new qx.bom.AnimationFrame();this.__ne.on(f,function(Y){var X=parseInt(Y/U*(V-W)+W);this.updatePosition(X);}
,this);this.__ne.on(g,function(){this.setPosition(Math.max(0,Math.min(this.getMaximum(),V)));this.__ne=null;this.fireEvent(o);}
,this);this.__ne.startSequence(U);}
...
,stopScrollAnimation:function(){if(this.__ne){this.__ne.cancelSequence();this.__ne=null;}
;}
,_onScroll:function(e){var bg=this.getContentElement();var bh=this.__nb?bg.getScrollX():bg.getScrollY();this.setPosition(bh);}
...
;}
,events:{"scrollAnimationEnd":k},properties:{appearance:{refine:true,init:z},orientation:{check:[w,q],init:w,apply:r},maximum:{check:v,apply:A,init:100},position:{check:h,init:0,apply:y,event:E},singleStep:{check:f,init:20},pageStep:{check:f,init:10,apply:t},knobFactor:{check:c,apply:i,nullable:true}},members:{__AE:2,__AF:0,_computeSizeHint:function(){var G=qx.ui.core.Widget.prototype._computeSizeHint.call(this);if(this.getOrientation()===w){this.__AF=G.minWidth;G.minWidth=0;}
else {this.__AF=G.minHeight;G.minHeight=0;}
;return G;}
,renderLayout:function(L,top,I,H){var K=qx.ui.core.Widget.prototype.renderLayout.call(this,L,top,I,H);var J=this.getOrientation()===w;if(this.__AF>=(J?I:H)){this.getChildControl(m).setVisibility(n);this.getChildControl(C).setVisibility(n);}
...
,_onResizeSlider:function(e){var bf=this.getChildControl(a).getChildControl(l);var bi=bf.getSizeHint();var bg=false;var bh=this.getChildControl(a).getInnerSize();if(this.getOrientation()==q){if(bh.height<bi.minHeight+this.__AE){bg=true;}
;}
else {if(bh.width<bi.minWidth+this.__AE){bg=true;}
;}
;if(bg){bf.exclude();}
else {bf.show();}
;}
...
,_onMouseDown:function(e){if(this.__AJ){return;}
;var bu=this.__nb;var bs=this.getChildControl(R);var bt=bu?H:N;var by=bu?e.getDocumentLeft():e.getDocumentTop();var bA=this.getDecorator();bA=qx.theme.manager.Decoration.getInstance().resolve(bA);if(bu){var bx=bA?bA.getInsets().left:0;var bw=(this.getPaddingLeft()||0)+bx;}
else {var bx=bA?bA.getInsets().top:0;var bw=(this.getPaddingTop()||0)+bx;}
;var bz=this.__AG=qx.bom.element.Location.get(this.getContentElement().getDomElement())[bt];bz+=bw;var bv=this.__AH=qx.bom.element.Location.get(bs.getContentElement().getDomElement())[bt];if(e.getTarget()===bs){this.__AJ=true;if(!this.__AO){this.__AO=new qx.event.Timer(100);this.__AO.addListener(v,this._fireValue,this);}
;this.__AO.start();this.__AK=by+bz-bv;bs.addState(d);}
else {this.__AL=true;this.__AM=by<=bv?-1:1;this.__AS(e);this._onInterval();if(!this.__ij){this.__ij=new qx.event.Timer(100);this.__ij.addListener(v,this._onInterval,this);}
;this.__ij.start();}
;this.addListener(z,this._onMouseMove);this.capture();e.stopPropagation();}
,_onMouseUp:function(e){if(this.__AJ){this.releaseCapture();delete this.__AJ;this.__AO.stop();this._fireValue();delete this.__AK;this.getChildControl(R).removeState(d);if(e.getType()===U){var bC;var bD;var bB;if(this.__nb){bC=e.getDocumentLeft()-(this._valueToPosition(this.getValue())+this.__AG);bB=qx.bom.element.Location.get(this.getContentElement().getDomElement())[N];bD=e.getDocumentTop()-(bB+this.getChildControl(R).getBounds().top);}
else {bC=e.getDocumentTop()-(this._valueToPosition(this.getValue())+this.__AG);bB=qx.bom.element.Location.get(this.getContentElement().getDomElement())[H];bD=e.getDocumentLeft()-(bB+this.getChildControl(R).getBounds().left);}
;if(bD<0||bD>this.__AI||bC<0||bC>this.__AI){this.getChildControl(R).removeState(b);}
;}
;}
else if(this.__AL){this.__ij.stop();this.releaseCapture();delete this.__AL;delete this.__AM;delete this.__AN;}
;this.removeListener(z,this._onMouseMove);if(e.getType()===U){e.stopPropagation();}
;}
,_onMouseMove:function(e){if(this.__AJ){var bF=this.__nb?e.getDocumentLeft():e.getDocumentTop();var bE=bF-this.__AK;this.slideTo(this._positionToValue(bE));}
else if(this.__AL){this.__AS(e);}
;e.stopPropagation();}
,_onInterval:function(e){var bG=this.getValue()+(this.__AM*this.getPageStep());if(bG<this.getMinimum()){bG=this.getMinimum();}
else if(bG>this.getMaximum()){bG=this.getMaximum();}
;var bH=this.__AM==-1;if((bH&&bG<=this.__AN)||(!bH&&bG>=this.__AN)){bG=this.__AN;}
;this.slideTo(bG);}
,_onUpdate:function(e){var bJ=this.getInnerSize();var bK=this.getChildControl(R).getBounds();var bI=this.__nb?F:A;this._updateKnobSize();this.__AR=bJ[bI]-bK[bI];this.__AI=bK[bI];this._updateKnobPosition();}
,__nb:false,__AR:0,__AS:function(e){var bL=this.__nb;var bS=bL?e.getDocumentLeft():e.getDocumentTop();var bV=this.__AG;var bM=this.__AH;var bW=this.__AI;var bT=bS-bV;if(bS>=bM){bT-=bW;}
;var bP=this._positionToValue(bT);var bN=this.getMinimum();var bO=this.getMaximum();if(bP<bN){bP=bN;}
else if(bP>bO){bP=bO;}
else {var bU=this.getValue();var bR=this.getPageStep();var bQ=this.__AM<0?K:S;bP=bU+(Math[bQ]((bP-bU)/bR)*bR);}
;if(this.__AN==null||(this.__AM==-1&&bP<=this.__AN)||(this.__AM==1&&bP>=this.__AN)){this.__AN=bP;}
;}
,_positionToValue:function(bY){var bX=this.__AR;if(bX==null||bX==0){return 0;}
;var cb=bY/bX;if(cb<0){cb=0;}
else if(cb>1){cb=1;}
;var ca=this.getMaximum()-this.getMinimum();return this.getMinimum()+Math.round(ca*cb);}
,_valueToPosition:function(cf){var cc=this.__AR;if(cc==null){return 0;}
...
,_setKnobPosition:function(ci){var cg=this.getChildControl(R);var ch=this.getDecorator();ch=qx.theme.manager.Decoration.getInstance().resolve(ch);var content=cg.getContentElement();if(this.__nb){if(ch&&ch.getPadding()){ci+=ch.getPadding().left;}
...
;if(this.__nb){this.getChildControl(R).setWidth(Math.round(ck*cj.width));}
...
,slideTo:function(cs,cr){this.stopSlideAnimation();if(cr){this.__AU(cs,cr);}
else {this.updatePosition(cs);}
;}
,updatePosition:function(ct){this.setValue(this.__AT(ct));}
,stopSlideAnimation:function(){if(this.__ne){this.__ne.cancelSequence();this.__ne=null;}
;}
,__AT:function(cu){if(cu<this.getMinimum()){cu=this.getMinimum();}
...
;return cu;}
,__AU:function(cw,cv){cw=this.__AT(cw);var cx=this.getValue();this.__ne=new qx.bom.AnimationFrame();this.__ne.on(j,function(cy){this.setValue(parseInt(cy/cv*(cw-cx)+cx));}
,this);this.__ne.on(l,function(){this.setValue(cw);this.__ne=null;this.fireEvent(Q);}
,this);this.__ne.startSequence(cv);}
,_applyOrientation:function(cB,cA){var cz=this.getChildControl(R);this.__nb=cB===V;if(this.__nb){this.removeState(D);cz.removeState(D);this.addState(V);cz.addState(V);cz.setLayoutProperties({top:0,right:null,bottom:0});}
...
else {if(this.__nb){this.getChildControl(R).resetWidth();}
else {this.getChildControl(R).resetHeight();}
;}
;}
,_applyValue:function(cF,cE){if(cF!=null){this._updateKnobPosition();if(this.__AJ){this.__AQ=[cF,cE];}
else {this.fireEvent(h,qx.event.type.Data,[cF,cE]);}
;}
else {this.resetValue();}
;}
,_fireValue:function(){if(!this.__AQ){return;}
;var cG=this.__AQ;this.__AQ=null;this.fireEvent(h,qx.event.type.Data,cG);}
...
(function(){var a="press",b="hovered",c="qx.ui.form.RepeatButton",d="release",f="Enter",g="pressed",h="interval",i="qx.event.type.Event",j="Space",k="__ij",l="abandoned",m="Integer",n="execute";qx.Class.define(c,{extend:qx.ui.form.Button,construct:function(o,p){qx.ui.form.Button.call(this,o,p);this.__ij=new qx.event.AcceleratingTimer();this.__ij.addListener(h,this._onInterval,this);}
,events:{"execute":i,"press":i,"release":i},properties:{interval:{check:m,init:100},firstInterval:{check:m,init:500},minTimer:{check:m,init:20},timerDecrease:{check:m,init:2}},members:{__oF:null,__ij:null,press:function(){if(this.isEnabled()){if(!this.hasState(g)){this.__oG();}
...
,_applyEnabled:function(s,r){qx.ui.form.Button.prototype._applyEnabled.call(this,s,r);if(!s){this.removeState(g);this.removeState(l);this.__oH();}
;}
,_onMouseOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;}
;if(this.hasState(l)){this.removeState(l);this.addState(g);this.__ij.start();}
;this.addState(b);}
,_onMouseOut:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;}
;this.removeState(b);if(this.hasState(g)){this.removeState(g);this.addState(l);this.__ij.stop();}
;}
,_onMouseDown:function(e){if(!e.isLeftPressed()){return;}
;this.capture();this.__oG();e.stopPropagation();}
,_onMouseUp:function(e){this.releaseCapture();if(!this.hasState(l)){this.addState(b);if(this.hasState(g)&&!this.__oF){this.execute();}
;}
;this.__oH();e.stopPropagation();}
,_onKeyUp:function(e){switch(e.getKeyIdentifier()){case f:case j:if(this.hasState(g)){if(!this.__oF){this.execute();}
;this.removeState(g);this.removeState(l);e.stopPropagation();this.__oH();}
;};}
,_onKeyDown:function(e){switch(e.getKeyIdentifier()){case f:case j:this.removeState(l);this.addState(g);e.stopPropagation();this.__oG();};}
,_onInterval:function(e){this.__oF=true;this.fireEvent(n);}
,__oG:function(){this.fireEvent(a);this.__oF=false;this.__ij.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();this.removeState(l);this.addState(g);}
,__oH:function(){this.fireEvent(d);this.__ij.stop();this.removeState(l);this.removeState(g);}
},destruct:function(){this._disposeObjects(k);}
});}
)();
(function(){var a="Integer",b="interval",c="qx.event.type.Event",d="__ij",e="qx.event.AcceleratingTimer";qx.Class.define(e,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__ij=new qx.event.Timer(this.getInterval());this.__ij.addListener(b,this._onInterval,this);}
,events:{"interval":c},properties:{interval:{check:a,init:100},firstInterval:{check:a,init:500},minimum:{check:a,init:20},decrease:{check:a,init:2}},members:{__ij:null,__oI:null,start:function(){this.__ij.setInterval(this.getFirstInterval());this.__ij.start();}
,stop:function(){this.__ij.stop();this.__oI=null;}
,_onInterval:function(){this.__ij.stop();if(this.__oI==null){this.__oI=this.getInterval();}
;this.__oI=Math.max(this.getMinimum(),this.__oI-this.getDecrease());this.__ij.setInterval(this.__oI);this.__ij.start();this.fireEvent(b);}
...
;this.__lK=x;this.__fn=B;delete this._invalidChildrenCache;}
...
;var K=this.__fn;var length=K.length;var U=qx.ui.layout.Util;var T=this.getSpacing();var bb=this.getSeparator();if(bb){var H=U.computeVerticalSeparatorGaps(K,T,bb);}
...
;}
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__lL();}
;var be=qx.ui.layout.Util;var bs=this.__fn;var bh=0,bi=0,bq=0;var bj=0,bk=0;var bo,bf,br;for(var i=0,l=bs.length;i<l;i+=1){bo=bs[i];bf=bo.getSizeHint();bi+=bf.height;var bn=this.__lJ[i];var bg=this.__lI[i];if(bn){bh+=bf.minHeight;}
...
},destruct:function(){this.__lI=this.__lJ=this.__fn=null;}
});}
)();
(function(){var a="mousedown",b="Boolean",c="scroll",d="focus-indicator",g="mouseout",h="__pP",i="changeHorizontalScrollBarVisible",j="excluded",k="bottom",l="_applyScrollTimeout",m="changeScrollX",n="_applyTablePaneModel",o="Integer",p="scrollbar-y",q="qx.event.type.Data",r="dblclick",s="__pS",t="__pM",u="dataEdited",v="mousewheel",w="interval",y="qx.ui.table.pane.Scroller",z="table-scroller-focus-indicator",A="_headerClipper",B="visible",C="mousemove",D="resize-line",E="_applyShowCellFocusIndicator",F="__pN",G="__ij",H="y",I="header",J="resize",K="__pT",L="vertical",M="__pO",N="changeScrollY",O="editing",P="appear",Q="click",R="modelChanged",S="table-scroller",T="beforeSort",U="scrollbar-x",V="__pQ",W="cellDblclick",X="mouseup",Y="horizontal",bx="__pR",by="cellClick",bz="losecapture",bt="contextmenu",bu="col-resize",bv="disappear",bw="_applyVerticalScrollBarVisible",bD="_applyHorizontalScrollBarVisible",bE="os.scrollBarOverlayed",bF="pane",bH="cellContextmenu",bA="close",bB="changeTablePaneModel",bC="x",br="qx.ui.table.pane.Model",bG="qx.ui.table.pane.CellEvent",bs="changeVerticalScrollBarVisible";qx.Class.define(y,{extend:qx.ui.core.Widget,include:[qx.ui.core.scroll.MScrollBarFactory],construct:function(bI){qx.ui.core.Widget.call(this);this.__ny=bI;var bJ=new qx.ui.layout.Grid();bJ.setColumnFlex(0,1);bJ.setRowFlex(1,1);this._setLayout(bJ);this.__pM=this._showChildControl(I);this.__pN=this._showChildControl(bF);this.__pO=new qx.ui.container.Composite(new qx.ui.layout.HBox()).set({minWidth:0});this._add(this.__pO,{row:0,column:0,colSpan:2});this._headerClipper=new qx.ui.table.pane.Clipper();this._headerClipper.add(this.__pM);this._headerClipper.addListener(bz,this._onChangeCaptureHeader,this);this._headerClipper.addListener(C,this._onMousemoveHeader,this);this._headerClipper.addListener(a,this._onMousedownHeader,this);this._headerClipper.addListener(X,this._onMouseupHeader,this);this._headerClipper.addListener(Q,this._onClickHeader,this);this.__pO.add(this._headerClipper,{flex:1});this.__pP=new qx.ui.table.pane.Clipper();this.__pP.add(this.__pN);this.__pP.addListener(v,this._onMousewheel,this);this.__pP.addListener(C,this._onMousemovePane,this);this.__pP.addListener(a,this._onMousedownPane,this);this.__pP.addListener(X,this._onMouseupPane,this);this.__pP.addListener(Q,this._onClickPane,this);this.__pP.addListener(bt,this._onContextMenu,this);this.__pP.addListener(r,this._onDblclickPane,this);this.__pP.addListener(J,this._onResizePane,this);if(qx.core.Environment.get(bE)){this.__pQ=new qx.ui.container.Composite();this.__pQ.setLayout(new qx.ui.layout.Canvas());this.__pQ.add(this.__pP,{edge:0});this._add(this.__pQ,{row:1,column:0});}
else {this._add(this.__pP,{row:1,column:0});}
;this.__pR=this._showChildControl(U);this.__pS=this._showChildControl(p);this.__pT=this.getChildControl(d);this.initShowCellFocusIndicator();this.getChildControl(D).hide();this.addListener(g,this._onMouseout,this);this.addListener(P,this._onAppear,this);this.addListener(bv,this._onDisappear,this);this.__ij=new qx.event.Timer();this.__ij.addListener(w,this._oninterval,this);this.initScrollTimeout();}
,statics:{MIN_COLUMN_WIDTH:10,RESIZE_REGION_RADIUS:5,CLICK_TOLERANCE:5,HORIZONTAL_SCROLLBAR:1,VERTICAL_SCROLLBAR:2},events:{"changeScrollY":q,"changeScrollX":q,"cellClick":bG,"cellDblclick":bG,"cellContextmenu":bG,"beforeSort":q},properties:{horizontalScrollBarVisible:{check:b,init:false,apply:bD,event:i},verticalScrollBarVisible:{check:b,init:false,apply:bw,event:bs},tablePaneModel:{check:br,apply:n,event:bB},liveResize:{check:b,init:false},focusCellOnMouseMove:{check:b,init:false},selectBeforeFocus:{check:b,init:false},showCellFocusIndicator:{check:b,init:true,apply:E},contextMenuFromDataCellsOnly:{check:b,init:true},resetSelectionOnHeaderClick:{check:b,init:true},scrollTimeout:{check:o,init:100,apply:l},appearance:{refine:true,init:S}},members:{__ol:null,__ny:null,__pU:null,__pV:null,__pW:null,_moveColumn:null,__pX:null,_lastMoveTargetX:null,_lastMoveTargetScroller:null,__pY:null,__qa:null,__qb:null,__qc:null,__qd:null,__qe:false,__qf:null,__qg:null,__qh:null,__oh:null,__oi:null,__qi:null,__qj:null,__qk:null,__pR:null,__pS:null,__pM:null,_headerClipper:null,__pN:null,__pP:null,__pQ:null,__pT:null,__pO:null,__ij:null,getPaneInsetRight:function(){var bM=this.getTopRightWidget();var bN=bM&&bM.isVisible()&&bM.getBounds()?bM.getBounds().width+bM.getMarginLeft()+bM.getMarginRight():0;var bL=this.__pS;var bK=this.getVerticalScrollBarVisible()?this.getVerticalScrollBarWidth()+bL.getMarginLeft()+bL.getMarginRight():0;return Math.max(bN,bK);}
,setPaneWidth:function(bO){if(this.isVerticalScrollBarVisible()){bO+=this.getPaneInsetRight();}
;this.setWidth(bO);}
,_createChildControlImpl:function(bR,bQ){var bP;switch(bR){case I:bP=(this.getTable().getNewTablePaneHeader())(this);break;case bF:bP=(this.getTable().getNewTablePane())(this);break;case d:bP=new qx.ui.table.pane.FocusIndicator(this);bP.setUserBounds(0,0,0,0);bP.setZIndex(1000);bP.addListener(X,this._onMouseupFocusIndicator,this);this.__pP.add(bP);bP.show();bP.setDecorator(null);break;case D:bP=new qx.ui.core.Widget();bP.setUserBounds(0,0,0,0);bP.setZIndex(1000);this.__pP.add(bP);break;case U:bP=this._createScrollBar(Y).set({alignY:k});bP.addListener(c,this._onScrollX,this);if(this.__pQ!=null){bP.setMinHeight(qx.ui.core.scroll.AbstractScrollArea.DEFAULT_SCROLLBAR_WIDTH);this.__pQ.add(bP,{bottom:0,right:0,left:0});}
else {this._add(bP,{row:2,column:0});}
;break;case p:bP=this._createScrollBar(L);bP.addListener(c,this._onScrollY,this);if(this.__pQ!=null){this.__pQ.add(bP,{right:0,bottom:0,top:0});}
else {this._add(bP,{row:1,column:1});}
;break;};return bP||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,bR);}
,_applyHorizontalScrollBarVisible:function(bT,bS){this.__pR.setVisibility(bT?B:j);}
,_applyVerticalScrollBarVisible:function(bV,bU){this.__pS.setVisibility(bV?B:j);}
,_applyTablePaneModel:function(bX,bW){if(bW!=null){bW.removeListener(R,this._onPaneModelChanged,this);}
;bX.addListener(R,this._onPaneModelChanged,this);}
,_applyShowCellFocusIndicator:function(ca,bY){if(ca){this.__pT.setDecorator(z);this._updateFocusIndicator();}
else {if(this.__pT){this.__pT.setDecorator(null);}
;}
;}
,getScrollY:function(){return this.__pS.getPosition();}
,setScrollY:function(scrollY,cb){this.__pS.scrollTo(scrollY);if(cb){this._updateContent();}
;}
,getScrollX:function(){return this.__pR.getPosition();}
,setScrollX:function(scrollX){this.__pR.scrollTo(scrollX);}
...
,setColumnWidth:function(ce,cd){this.__pM.setColumnWidth(ce,cd);this.__pN.setColumnWidth(ce,cd);var cc=this.getTablePaneModel();var x=cc.getX(ce);if(x!=-1){this.updateHorScrollBarMaximum();this._updateFocusIndicator();}
;}
,onColOrderChanged:function(){this.__pM.onColOrderChanged();this.__pN.onColOrderChanged();this.updateHorScrollBarMaximum();}
,onTableModelDataChanged:function(ci,cg,cj,ch){this.__pN.onTableModelDataChanged(ci,cg,cj,ch);var cf=this.getTable().getTableModel().getRowCount();if(cf!=this.__ol){this.updateVerScrollBarMaximum();if(this.getFocusedRow()>=cf){if(cf==0){this.setFocusedCell(null,null);}
else {this.setFocusedCell(this.getFocusedColumn(),cf-1);}
;}
;this.__ol=cf;}
;}
,onSelectionChanged:function(){this.__pN.onSelectionChanged();}
,onFocusChanged:function(){this.__pN.onFocusChanged();}
,onTableModelMetaDataChanged:function(){this.__pM.onTableModelMetaDataChanged();this.__pN.onTableModelMetaDataChanged();}
,_onPaneModelChanged:function(){this.__pM.onPaneModelChanged();this.__pN.onPaneModelChanged();}
,_onResizePane:function(){this.updateHorScrollBarMaximum();this.updateVerScrollBarMaximum();this._updateContent();this.__pM._updateContent();this.__ny._updateScrollBarVisibility();}
,updateHorScrollBarMaximum:function(){var co=this.__pP.getInnerSize();if(!co){return;}
;var cl=this.getTablePaneModel().getTotalWidth();var cm=this.__pR;if(co.width<cl){var ck=Math.max(0,cl-co.width);cm.setMaximum(ck);cm.setKnobFactor(co.width/cl);var cn=cm.getPosition();cm.setPosition(Math.min(cn,ck));}
else {cm.setMaximum(0);cm.setKnobFactor(1);cm.setPosition(0);}
;}
,updateVerScrollBarMaximum:function(){var cw=this.__pP.getInnerSize();if(!cw){return;}
;var cu=this.getTable().getTableModel();var cq=cu.getRowCount();if(this.getTable().getKeepFirstVisibleRowComplete()){cq+=1;}
;var cp=this.getTable().getRowHeight();var cs=cq*cp;var cv=this.__pS;if(cw.height<cs){var cr=Math.max(0,cs-cw.height);cv.setMaximum(cr);cv.setKnobFactor(cw.height/cs);var ct=cv.getPosition();cv.setPosition(Math.min(ct,cr));}
else {cv.setMaximum(0);cv.setKnobFactor(1);cv.setPosition(0);}
;}
,onKeepFirstVisibleRowCompleteChanged:function(){this.updateVerScrollBarMaximum();this._updateContent();}
,_onAppear:function(){this._startInterval(this.getScrollTimeout());}
,_onDisappear:function(){this._stopInterval();}
,_onScrollX:function(e){var cx=e.getData();this.fireDataEvent(m,cx,e.getOldData());this._headerClipper.scrollToX(cx);this.__pP.scrollToX(cx);}
,_onScrollY:function(e){this.fireDataEvent(N,e.getData(),e.getOldData());this._postponedUpdateContent();}
,_onMousewheel:function(e){var cz=this.getTable();if(!cz.getEnabled()){return;}
;var cA=e.getWheelDelta(H);if(cA>0&&cA<1){cA=1;}
else if(cA<0&&cA>-1){cA=-1;}
;if(qx.event.handler.MouseEmulation.ON){this.__pS.scrollBy(cA);}
else {this.__pS.scrollBySteps(cA);}
;var cy=cA!=0&&!this.__ql(this.__pS,cA);cA=e.getWheelDelta(bC);if(cA>0&&cA<1){cA=1;}
else if(cA<0&&cA>-1){cA=-1;}
;if(qx.event.handler.MouseEmulation.ON){this.__pR.scrollBy(cA);}
else {this.__pR.scrollBySteps(cA);}
;if(this.__qg&&this.getFocusCellOnMouseMove()){this._focusCellAtPagePos(this.__qg,this.__qh);}
;cy=cy||(cA!=0&&!this.__ql(this.__pR,cA));if(cy){e.stop();}
;}
,__ql:function(cC,cD){var cB=cC.getPosition();return (cD<0&&cB<=0)||(cD>0&&cB>=cC.getMaximum());}
,__qm:function(cJ){var cH=this.getTable();var cK=this.__pM.getHeaderWidgetAtColumn(this.__qa);var cE=cK.getSizeHint().minWidth;var cG=Math.max(cE,this.__qc+cJ-this.__qb);if(this.getLiveResize()){var cF=cH.getTableColumnModel();cF.setColumnWidth(this.__qa,cG,true);}
else {this.__pM.setColumnWidth(this.__qa,cG,true);var cI=this.getTablePaneModel();this._showResizeLine(cI.getColumnLeft(this.__qa)+cG);}
;this.__qb+=cG-this.__qc;this.__qc=cG;}
,__qn:function(cM){var cL=qx.ui.table.pane.Scroller.CLICK_TOLERANCE;if(this.__pM.isShowingColumnMoveFeedback()||cM>this.__pY+cL||cM<this.__pY-cL){this.__pX+=cM-this.__pY;this.__pM.showColumnMoveFeedback(this._moveColumn,this.__pX);var cN=this.__ny.getTablePaneScrollerAtPageX(cM);if(this._lastMoveTargetScroller&&this._lastMoveTargetScroller!=cN){this._lastMoveTargetScroller.hideColumnMoveFeedback();}
;if(cN!=null){this._lastMoveTargetX=cN.showColumnMoveFeedback(cM);}
else {this._lastMoveTargetX=null;}
;this._lastMoveTargetScroller=cN;this.__pY=cM;}
;}
,_onMousemoveHeader:function(e){var cU=this.getTable();if(!cU.getEnabled()){return;}
;var cV=false;var cO=null;var cS=e.getDocumentLeft();var cT=e.getDocumentTop();this.__qg=cS;this.__qh=cT;if(this.__qa!=null){this.__qm(cS);cV=true;e.stopPropagation();}
else if(this._moveColumn!=null){this.__qn(cS);e.stopPropagation();}
else {var cP=this._getResizeColumnForPageX(cS);if(cP!=-1){cV=true;}
else {var cR=cU.getTableModel();var cW=this._getColumnForPageX(cS);if(cW!=null&&cR.isColumnSortable(cW)){cO=cW;}
;}
;}
;var cQ=cV?bu:null;this.getApplicationRoot().setGlobalCursor(cQ);this.setCursor(cQ);this.__pM.setMouseOverColumn(cO);}
,_onMousemovePane:function(e){var cX=this.getTable();if(!cX.getEnabled()){return;}
;var da=e.getDocumentLeft();var db=e.getDocumentTop();this.__qg=da;this.__qh=db;var cY=this._getRowForPagePos(da,db);if(cY!=null&&this._getColumnForPageX(da)!=null){if(this.getFocusCellOnMouseMove()){this._focusCellAtPagePos(da,db);}
;}
;this.__pM.setMouseOverColumn(null);}
,_onMousedownHeader:function(e){if(!this.getTable().getEnabled()){return;}
;var dd=e.getDocumentLeft();var de=this._getResizeColumnForPageX(dd);if(de!=-1){this._startResizeHeader(de,dd);e.stop();}
else {var dc=this._getColumnForPageX(dd);if(dc!=null){this._startMoveHeader(dc,dd);e.stop();}
;}
;}
,_startResizeHeader:function(dh,df){var dg=this.getTable().getTableColumnModel();this.__qa=dh;this.__qb=df;this.__qc=dg.getColumnWidth(this.__qa);this._headerClipper.capture();}
,_startMoveHeader:function(di,dj){this._moveColumn=di;this.__pY=dj;this.__pX=this.getTablePaneModel().getColumnLeft(di);this._headerClipper.capture();}
,_onMousedownPane:function(e){var dn=this.getTable();if(!dn.getEnabled()){return;}
;if(dn.isEditing()){dn.stopEditing();}
;var dl=e.getDocumentLeft();var dm=e.getDocumentTop();var dq=this._getRowForPagePos(dl,dm);var dp=this._getColumnForPageX(dl);if(dq!==null){this.__qd={row:dq,col:dp};this.__qe=false;var dk=this.getSelectBeforeFocus();if(dk){dn.getSelectionManager().handleMouseDown(dq,e);}
;if(!this.getFocusCellOnMouseMove()){this._focusCellAtPagePos(dl,dm);}
;if(!dk){dn.getSelectionManager().handleMouseDown(dq,e);}
;}
;}
,_onMouseupFocusIndicator:function(e){if(this.__qd&&!this.__qe&&!this.isEditing()&&this.__pT.getRow()==this.__qd.row&&this.__pT.getColumn()==this.__qd.col){this.fireEvent(by,qx.ui.table.pane.CellEvent,[this,e,this.__qd.row,this.__qd.col],true);this.__qe=true;}
else if(!this.isEditing()){this._onMousedownPane(e);}
;}
,_onChangeCaptureHeader:function(e){if(this.__qa!=null){this._stopResizeHeader();}
;if(this._moveColumn!=null){this._stopMoveHeader();}
;}
,_stopResizeHeader:function(){var dr=this.getTable().getTableColumnModel();if(!this.getLiveResize()){this._hideResizeLine();dr.setColumnWidth(this.__qa,this.__qc,true);}
;this.__qa=null;this._headerClipper.releaseCapture();this.getApplicationRoot().setGlobalCursor(null);this.setCursor(null);if(this.isEditing()){var ds=this.__qi.getBounds().height;this.__qi.setUserBounds(0,0,this.__qc,ds);}
;}
,_stopMoveHeader:function(){var dx=this.getTable().getTableColumnModel();var dy=this.getTablePaneModel();this.__pM.hideColumnMoveFeedback();if(this._lastMoveTargetScroller){this._lastMoveTargetScroller.hideColumnMoveFeedback();}
;if(this._lastMoveTargetX!=null){var dA=dy.getFirstColumnX()+dy.getX(this._moveColumn);var dw=this._lastMoveTargetX;if(dw!=dA&&dw!=dA+1){var dz=dx.getVisibleColumnAtX(dA);var dv=dx.getVisibleColumnAtX(dw);var du=dx.getOverallX(dz);var dt=(dv!=null)?dx.getOverallX(dv):dx.getOverallColumnCount();if(dt>du){dt-- ;}
;dx.moveColumn(du,dt);this._updateFocusIndicator();}
;}
;this._moveColumn=null;this._lastMoveTargetX=null;this._headerClipper.releaseCapture();}
,_onMouseupPane:function(e){var dB=this.getTable();if(!dB.getEnabled()){return;}
;var dC=this._getRowForPagePos(e.getDocumentLeft(),e.getDocumentTop());if(dC!=-1&&dC!=null&&this._getColumnForPageX(e.getDocumentLeft())!=null){dB.getSelectionManager().handleMouseUp(dC,e);}
;}
,_onMouseupHeader:function(e){var dD=this.getTable();if(!dD.getEnabled()){return;}
;if(this.__qa!=null){this._stopResizeHeader();this.__qf=true;e.stop();}
else if(this._moveColumn!=null){this._stopMoveHeader();e.stop();}
;}
,_onClickHeader:function(e){if(this.__qf){this.__qf=false;return;}
;var dI=this.getTable();if(!dI.getEnabled()){return;}
;var dG=dI.getTableModel();var dH=e.getDocumentLeft();var dF=this._getResizeColumnForPageX(dH);if(dF==-1){var dL=this._getColumnForPageX(dH);if(dL!=null&&dG.isColumnSortable(dL)){var dE=dG.getSortColumnIndex();var dJ=(dL!=dE)?true:!dG.isSortAscending();var dK={column:dL,ascending:dJ,clickEvent:e};if(this.fireDataEvent(T,dK,null,true)){if(dI.isEditing()){dI.stopEditing();}
;dG.sortByColumn(dL,dJ);if(this.getResetSelectionOnHeaderClick()){dI.getSelectionModel().resetSelection();}
;}
;}
;}
;e.stop();}
,_onClickPane:function(e){var dO=this.getTable();if(!dO.getEnabled()){return;}
;var dM=e.getDocumentLeft();var dN=e.getDocumentTop();var dQ=this._getRowForPagePos(dM,dN);var dP=this._getColumnForPageX(dM);if(dQ!=null&&dP!=null){dO.getSelectionManager().handleClick(dQ,e);if(this.__pT.isHidden()||(this.__qd&&!this.__qe&&!this.isEditing()&&dQ==this.__qd.row&&dP==this.__qd.col)){this.fireEvent(by,qx.ui.table.pane.CellEvent,[this,e,dQ,dP],true);this.__qe=true;}
;}
;}
,_onContextMenu:function(e){var dS=e.getDocumentLeft();var dT=e.getDocumentTop();var dV=this._getRowForPagePos(dS,dT);var dU=this._getColumnForPageX(dS);if(dV===null&&this.getContextMenuFromDataCellsOnly()){return;}
;if(!this.getShowCellFocusIndicator()||dV===null||(this.__qd&&dV==this.__qd.row&&dU==this.__qd.col)){this.fireEvent(bH,qx.ui.table.pane.CellEvent,[this,e,dV,dU],true);var dR=this.getTable().getContextMenu();if(dR){if(dR.getChildren().length>0){dR.openAtMouse(e);}
else {dR.exclude();}
;e.preventDefault();}
;}
;}
,_onContextMenuOpen:function(e){}
,_onDblclickPane:function(e){var dY=e.getDocumentLeft();var ea=e.getDocumentTop();var dX=this._getColumnForPageX(dY);if(dX!==null){this._focusCellAtPagePos(dY,ea);this.startEditing();var dW=this._getRowForPagePos(dY,ea);if(dW!=-1&&dW!=null){this.fireEvent(W,qx.ui.table.pane.CellEvent,[this,e,dW],true);}
;}
;}
,_onMouseout:function(e){var eb=this.getTable();if(!eb.getEnabled()){return;}
;if(this.__qa==null){this.setCursor(null);this.getApplicationRoot().setGlobalCursor(null);}
;this.__pM.setMouseOverColumn(null);if(this.getFocusCellOnMouseMove()){this.__ny.setFocusedCell();}
;}
,_showResizeLine:function(x){var ed=this._showChildControl(D);var ec=ed.getWidth();var ee=this.__pP.getBounds();ed.setUserBounds(x-Math.round(ec/2),0,ec,ee.height);}
,_hideResizeLine:function(){this._excludeChildControl(D);}
,showColumnMoveFeedback:function(en){var eo=this.getTablePaneModel();var eh=this.getTable().getTableColumnModel();var ei=this.__pN.getContentLocation().left;var em=eo.getColumnCount();var ej=0;var er=0;var eg=ei;for(var ef=0;ef<em;ef++ ){var eq=eo.getColumnAtX(ef);var ep=eh.getColumnWidth(eq);if(en<eg+ep/2){break;}
;eg+=ep;ej=ef+1;er=eg-ei;}
;var el=this.__pP.getContentLocation().left;var ek=this.__pP.getBounds().width;var scrollX=el-ei;er=qx.lang.Number.limit(er,scrollX+2,scrollX+ek-1);this._showResizeLine(er);return eo.getFirstColumnX()+ej;}
,hideColumnMoveFeedback:function(){this._hideResizeLine();}
,_focusCellAtPagePos:function(eu,es){var ev=this._getRowForPagePos(eu,es);if(ev!=-1&&ev!=null){var et=this._getColumnForPageX(eu);this.__ny.setFocusedCell(et,ev);}
;}
,setFocusedCell:function(ew,ex){if(!this.isEditing()){this.__pN.setFocusedCell(ew,ex,this.__pV);this.__oh=ew;this.__oi=ex;this._updateFocusIndicator();}
;}
,getFocusedColumn:function(){return this.__oh;}
,getFocusedRow:function(){return this.__oi;}
,scrollCellVisible:function(eK,eL){var eI=this.getTablePaneModel();var ey=eI.getX(eK);if(ey!=-1){var eF=this.__pP.getInnerSize();if(!eF){return;}
;var eB=this.getTable().getTableColumnModel();var eC=eI.getColumnLeft(eK);var eJ=eB.getColumnWidth(eK);var eA=this.getTable().getRowHeight();var ez=eL*eA;var scrollX=this.getScrollX();var scrollY=this.getScrollY();var eH=Math.min(eC,eC+eJ-eF.width);var eE=eC;this.setScrollX(Math.max(eH,Math.min(eE,scrollX)));var eG=ez+eA-eF.height;if(this.getTable().getKeepFirstVisibleRowComplete()){eG+=eA;}
;var eD=ez;this.setScrollY(Math.max(eG,Math.min(eD,scrollY)),true);}
;}
,isEditing:function(){return this.__qi!=null;}
,startEditing:function(){var eQ=this.getTable();var eO=eQ.getTableModel();var eS=this.__oh;if(!this.isEditing()&&(eS!=null)&&eO.isColumnEditable(eS)){var eR=this.__oi;var eM=this.getTablePaneModel().getX(eS);var eN=eO.getValue(eS,eR);this.scrollCellVisible(eM,eR);this.__qj=eQ.getTableColumnModel().getCellEditorFactory(eS);var eP={col:eS,row:eR,xPos:eM,value:eN,table:eQ};this.__qi=this.__qj.createCellEditor(eP);if(this.__qi===null){return false;}
else if(this.__qi instanceof qx.ui.window.Window){this.__qi.setModal(true);this.__qi.setShowClose(false);this.__qi.addListener(bA,this._onCellEditorModalWindowClose,this);var f=eQ.getModalCellEditorPreOpenFunction();if(f!=null){f(this.__qi,eP);}
;this.__qi.open();}
else {var eT=this.__pT.getInnerSize();this.__qi.setUserBounds(0,0,eT.width,eT.height);this.__pT.addListener(a,function(e){this.__qd={row:this.__oi,col:this.__oh};e.stopPropagation();}
,this);this.__pT.add(this.__qi);this.__pT.addState(O);this.__pT.setKeepActive(false);this.__pT.setDecorator(z);this.__qi.focus();this.__qi.activate();}
;return true;}
;return false;}
,stopEditing:function(){if(!this.getShowCellFocusIndicator()){this.__pT.setDecorator(null);}
;this.flushEditor();this.cancelEditing();}
,flushEditor:function(){if(this.isEditing()){var eU=this.__qj.getCellEditorValue(this.__qi);var eV=this.getTable().getTableModel().getValue(this.__oh,this.__oi);this.getTable().getTableModel().setValue(this.__oh,this.__oi,eU);this.__ny.focus();this.__ny.fireDataEvent(u,{row:this.__oi,col:this.__oh,oldValue:eV,value:eU});}
;}
,cancelEditing:function(){if(this.isEditing()&&!this.__qi.pendingDispose){if(this._cellEditorIsModalWindow){this.__qi.destroy();this.__qi=null;this.__qj=null;this.__qi.pendingDispose=true;}
else {this.__pT.removeState(O);this.__pT.setKeepActive(true);this.__qi.destroy();this.__qi=null;this.__qj=null;}
;}
;}
,_onCellEditorModalWindowClose:function(e){this.stopEditing();}
,_getColumnForPageX:function(eW){var fa=this.getTable().getTableColumnModel();var fb=this.getTablePaneModel();var eY=fb.getColumnCount();var eX=this.__pN.getContentLocation().left;for(var x=0;x<eY;x++ ){var fd=fb.getColumnAtX(x);var fc=fa.getColumnWidth(fd);eX+=fc;if(eW<eX){return fd;}
;}
;return null;}
,_getResizeColumnForPageX:function(ff){var fi=this.getTable().getTableColumnModel();var fj=this.getTablePaneModel();var fh=fj.getColumnCount();var fg=this.__pM.getContentLocation().left;var fe=qx.ui.table.pane.Scroller.RESIZE_REGION_RADIUS;for(var x=0;x<fh;x++ ){var fl=fj.getColumnAtX(x);var fk=fi.getColumnWidth(fl);fg+=fk;if(ff>=(fg-fe)&&ff<=(fg+fe)){return fl;}
;}
;return -1;}
,_getRowForPagePos:function(fs,ft){var fm=this.__pN.getContentLocation();if(fs<fm.left||fs>fm.right){return null;}
;if(ft>=fm.top&&ft<=fm.bottom){var fn=this.getTable().getRowHeight();var scrollY=this.__pS.getPosition();if(this.getTable().getKeepFirstVisibleRowComplete()){scrollY=Math.floor(scrollY/fn)*fn;}
;var fq=scrollY+ft-fm.top;var fu=Math.floor(fq/fn);var fo=this.getTable().getTableModel();var fr=fo.getRowCount();return (fu<fr)?fu:null;}
;var fp=this.__pM.getContentLocation();if(ft>=fp.top&&ft<=fp.bottom&&fs<=fp.right){return -1;}
;return null;}
,setTopRightWidget:function(fv){var fw=this.__qk;if(fw!=null){this.__pO.remove(fw);}
;if(fv!=null){this.__pO.add(fv);}
;this.__qk=fv;}
,getTopRightWidget:function(){return this.__qk;}
,getHeader:function(){return this.__pM;}
,getTablePane:function(){return this.__pN;}
,getVerticalScrollBarWidth:function(){var fx=this.__pS;return fx.isVisible()?(fx.getSizeHint().width||0):0;}
,getNeededScrollBars:function(fB,fC){var fN=this.__pS;var fM=fN.getSizeHint().width+fN.getMarginLeft()+fN.getMarginRight();var fH=this.__pR;var fK=fH.getSizeHint().height+fH.getMarginTop()+fH.getMarginBottom();var fE=this.__pP.getInnerSize();var fy=fE?fE.width:0;if(this.getVerticalScrollBarVisible()){fy+=fM;}
;var fI=fE?fE.height:0;if(this.getHorizontalScrollBarVisible()){fI+=fK;}
;var fF=this.getTable().getTableModel();var fG=fF.getRowCount();var fz=this.getTablePaneModel().getTotalWidth();var fJ=this.getTable().getRowHeight()*fG;var fA=false;var fO=false;if(fz>fy){fA=true;if(fJ>fI-fK){fO=true;}
;}
else if(fJ>fI){fO=true;if(!fC&&(fz>fy-fM)){fA=true;}
;}
;var fD=qx.ui.table.pane.Scroller.HORIZONTAL_SCROLLBAR;var fL=qx.ui.table.pane.Scroller.VERTICAL_SCROLLBAR;return ((fB||fA)?fD:0)|((fC||!fO)?0:fL);}
,getPaneClipper:function(){return this.__pP;}
,getScrollAreaContainer:function(){return this.getPaneClipper();}
,_applyScrollTimeout:function(fQ,fP){this._startInterval(fQ);}
,_startInterval:function(fR){this.__ij.setInterval(fR);this.__ij.start();}
,_stopInterval:function(){this.__ij.stop();}
,_postponedUpdateContent:function(){this._updateContent();}
,_oninterval:qx.event.GlobalError.observeMethod(function(){if(this.__pV&&!this.__pN._layoutPending){this.__pV=false;this._updateContent();}
;}
),_updateContent:function(){var ga=this.__pP.getInnerSize();if(!ga){return;}
;var fX=ga.height;var scrollX=this.__pR.getPosition();var scrollY=this.__pS.getPosition();var fT=this.getTable().getRowHeight();var fV=Math.floor(scrollY/fT);var fY=this.__pN.getFirstVisibleRow();this.__pN.setFirstVisibleRow(fV);var fU=Math.ceil(fX/fT);var fS=0;var fW=this.getTable().getKeepFirstVisibleRowComplete();if(!fW){fU++ ;fS=scrollY%fT;}
;this.__pN.setVisibleRowCount(fU);if(fV!=fY){this._updateFocusIndicator();}
;this.__pP.scrollToX(scrollX);if(!fW){this.__pP.scrollToY(fS);}
;}
,_updateFocusIndicator:function(){var gb=this.getTable();if(!gb.getEnabled()){return;}
;this.__pT.moveToCell(this.__oh,this.__oi);}
},destruct:function(){this._stopInterval();var gc=this.getTablePaneModel();if(gc){gc.dispose();}
;this.__qd=this.__qk=this.__ny=null;this._disposeObjects(bx,s,A,h,K,t,F,M,G,V);}
});}
)();
...
(function(){var a="x",b="Float",c="qx.ui.core.MDragDropScrolling",d="",f="). Must be: 'left', 'right', 'top' or 'bottom'",g="bottom",h="interval",i="dragend",j="scrollbar-",k="drag",l="Invalid edge type given (",m="top",n="left",o="right",p="Integer",q="y";qx.Mixin.define(c,{construct:function(){this.addListener(k,this.__mY,this);this.addListener(i,this.__na,this);this.__mV=[n,o];this.__mW=[m,g];}
,properties:{dragScrollThresholdX:{check:p,init:30},dragScrollThresholdY:{check:p,init:30},dragScrollSlowDownFactor:{check:b,init:0.1}},members:{__mX:null,__mV:null,__mW:null,_findScrollableParent:function(r){var s=r;if(s===null){return null;}
...
;}
,_getAxis:function(z){if(this.__mV.indexOf(z)!==-1){return a;}
else if(this.__mW.indexOf(z)!==-1){return q;}
else {throw new Error(l+z+f);}
;}
,_getThresholdByEdgeType:function(A){if(this.__mV.indexOf(A)!==-1){return this.getDragScrollThresholdX();}
else if(this.__mW.indexOf(A)!==-1){return this.getDragScrollThresholdY();}
;}
...
;var O=R.getBounds(),N=S===a?O.width:O.height,Q=this._calculateScrollAmount(N,P);if(this._isScrollbarExceedingMaxPos(R,S,Q)){this.__mX.stop();}
;R.scrollBy(Q);}
,__mY:function(e){if(this.__mX){this.__mX.stop();}
...
;bb=this._getAxis(V);if(this._isScrollbarVisible(T,bb)){Y=this._calculateThresholdExceedance(ba[V],this._getThresholdByEdgeType(V));if(this.__mX){this.__mX.dispose();}
;this.__mX=new qx.event.Timer(50);this.__mX.addListener(h,function(bc,be,bd){this._scrollBy(bc,be,bd);}
.bind(this,T,bb,Y));this.__mX.start();}
;}
;}
,__na:function(e){if(this.__mX){this.__mX.stop();}
;}
...
(function(){var a="Integer",b="qx.ui.table.pane.CellEvent";qx.Class.define(b,{extend:qx.event.type.Mouse,properties:{row:{check:a,nullable:true},column:{check:a,nullable:true}},members:{init:function(e,c,f,d){c.clone(this);this.setBubbles(false);if(f!=null){this.setRow(f);}
...
(function(){var a="qx.lang.Number";qx.Class.define(a,{statics:{isInRange:function(c,d,b){return c>=d&&c<=b;}
,isBetweenRange:function(f,g,e){return f>g&&f<e;}
,limit:function(i,j,h){if(h!=null&&i>h){return h;}
else if(j!=null&&i<j){return j;}
else {return i;}
;}
}});}
)();
(function(){var a="indexOf",b="addAfter",c="add",d="addBefore",e="_",f="addAt",g="hasChildren",h="removeAt",i="removeAll",j="getChildren",k="remove",l="qx.ui.core.MRemoteChildrenHandling";qx.Mixin.define(l,{members:{__oy:function(q,m,o,n){var p=this.getChildrenContainer();if(p===this){q=e+q;}
;return (p[q])(m,o,n);}
,getChildren:function(){return this.__oy(j);}
,hasChildren:function(){return this.__oy(g);}
,add:function(s,r){return this.__oy(c,s,r);}
,remove:function(t){return this.__oy(k,t);}
,removeAll:function(){return this.__oy(i);}
,indexOf:function(u){return this.__oy(a,u);}
,addAt:function(x,v,w){this.__oy(f,x,v,w);}
,addBefore:function(y,A,z){this.__oy(d,y,A,z);}
,addAfter:function(D,B,C){this.__oy(b,D,B,C);}
,removeAt:function(E){return this.__oy(h,E);}
}});}
)();
...
(function(){var a="Number",b="qx.event.type.Event",c="modelChanged",d="_applyFirstColumnX",e="headerCellRendererChanged",f="Integer",g="qx.ui.table.pane.Model",h="_applyMaxColumnCount",i="visibilityChangedPre";qx.Class.define(g,{extend:qx.core.Object,construct:function(j){qx.core.Object.call(this);this.setTableColumnModel(j);}
,events:{"modelChanged":b},statics:{EVENT_TYPE_MODEL_CHANGED:c},properties:{firstColumnX:{check:f,init:0,apply:d},maxColumnCount:{check:a,init:-1,apply:h}},members:{__rm:null,__rn:null,_applyFirstColumnX:function(l,k){this.__rm=null;this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);}
,_applyMaxColumnCount:function(n,m){this.__rm=null;this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);}
,setTableColumnModel:function(o){if(this.__rn){this.__rn.removeListener(i,this._onColVisibilityChanged,this);this.__rn.removeListener(e,this._onColVisibilityChanged,this);}
;this.__rn=o;this.__rn.addListener(i,this._onColVisibilityChanged,this);this.__rn.addListener(e,this._onHeaderCellRendererChanged,this);this.__rm=null;}
,_onColVisibilityChanged:function(p){this.__rm=null;this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);}
,_onHeaderCellRendererChanged:function(q){this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);}
,getColumnCount:function(){if(this.__rm==null){var r=this.getFirstColumnX();var t=this.getMaxColumnCount();var s=this.__rn.getVisibleColumnCount();if(t==-1||(r+t)>s){this.__rm=s-r;}
else {this.__rm=t;}
;}
;return this.__rm;}
,getColumnAtX:function(v){var u=this.getFirstColumnX();return this.__rn.getVisibleColumnAtX(u+v);}
,getX:function(z){var w=this.getFirstColumnX();var y=this.getMaxColumnCount();var x=this.__rn.getVisibleX(z)-w;if(x>=0&&(y==-1||x<y)){return x;}
else {return -1;}
;}
,getColumnLeft:function(D){var B=0;var C=this.getColumnCount();for(var x=0;x<C;x++ ){var A=this.getColumnAtX(x);if(A==D){return B;}
;B+=this.__rn.getColumnWidth(A);}
;return -1;}
,getTotalWidth:function(){var E=0;var F=this.getColumnCount();for(var x=0;x<F;x++ ){var G=this.getColumnAtX(x);E+=this.__rn.getColumnWidth(G);}
;return E;}
},destruct:function(){if(this.__rn){this.__rn.removeListener(i,this._onColVisibilityChanged,this);this.__rn.removeListener(e,this._onColVisibilityChanged,this);}
;this.__rn=null;}
});}
)();
