...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.CellSpan.136552b2c3f8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,getCellProperties:function(s,q){var r=(s+q)%2==0?c:i;this.__SU.setBackgroundColor(r);return this.__SU.getCellProperties(s+a+q);}
...
,__Kg:function(G,A,C,H,top,F,D){var B=this._cellProvider.getCellProperties(A,C);var E=B.insets||[0,0];G.push(f,j,o,H,k,b,top,k,this._getCellSizeStyle(F,D,E[0],E[1]),B.style||c,g,m,B.classes||c,g,B.attributes||c,d,B.content||c,a);}
,_fullUpdate:function(P,K,T,N){var S=[];var M=this._spanManager.findCellsInWindow(P,K,T.length,N.length);if(M.length>0){var O=this._spanManager.getCellBounds(M,P,K);var I=this._spanManager.computeCellSpanMap(M,P,K,T.length,N.length);for(var i=0,l=M.length;i<l;i++ ){var R=M[i];var J=O[i];this.__Kg(S,R.firstRow,R.firstColumn,J.left,J.top,J.width,J.height);}
;}
else {I=[];for(var i=0;i<T.length;i++ ){I[P+i]=[];}
;}
;var U=0;var top=0;var W=P;var L=K;for(var x=0;x<T.length;x++ ){var U=0;var L=K;var Q=T[x];for(var y=0;y<N.length;y++ ){var V=N[y];if(!I[W][L]){this.__Kg(S,W,L,U,top,V,Q);}
;L++ ;U+=V;}
;top+=Q;W++ ;}
...
;return this.__Gi(K,M);}
,__Gi:function(P,N){var O=[];for(var Q in P){if(N[Q]){O.push(N[Q]);}
;}
;return O;}
...
(function(){var a="cell.rowspan",b="x",c="cell.column",d="cell.colspan",e="qx.ui.virtual.layer.WidgetCellSpan",f="_spanManager",g="_cellLayer",h="cell.spanning",j="cell.row";qx.Class.define(e,{extend:qx.ui.virtual.layer.Abstract,include:[qx.ui.core.MChildrenHandling],construct:function(m,n,k){qx.ui.virtual.layer.Abstract.call(this);this.setZIndex(12);this._spanManager=new qx.ui.virtual.layer.CellSpanManager(n,k);this._cellProvider=m;this.__Ag=[];this._cellLayer=new qx.ui.virtual.layer.WidgetCell(this.__JY());this._cellLayer.setZIndex(0);this._setLayout(new qx.ui.layout.Grow());this._add(this._cellLayer);}
...
;return null;}
;return o;}
,__Ag:null,setCellSpan:function(y,u,v,w){var x=y+b+u;this._spanManager.removeCell(x);if(v>1||w>1){this._spanManager.addCell(x,y,u,v,w);}
;qx.ui.core.queue.Widget.add(this);}
,__JY:function(){var self=this;var B=this._cellProvider;var A=this.__Ag;var z={getCellWidget:function(E,C){if(!self._spanMap[E][C]){var D=B.getCellWidget(E,C);}
...
,__Ka:function(I,J,H,G){this._cells=this._spanManager.findCellsInWindow(I,J,H,G);if(this._cells.length>0){this._bounds=this._spanManager.getCellBounds(this._cells,I,J);this._spanMap=this._spanManager.computeCellSpanMap(this._cells,I,J,H,G);}
else {this._bounds=[];this._spanMap=[];for(var i=0;i<H;i++ ){this._spanMap[I+i]=[];}
;}
;}
,__Kb:function(){var O=this.getChildren();for(var i=O.length-1;i>=0;i-- ){var N=O[i];if(N!==this._cellLayer){this._cellProvider.poolCellWidget(N);this._remove(N);}
;}
...
;}
;}
,_fullUpdate:function(Q,S,R,P){this.__Ka(Q,S,R.length,P.length);this.__Kb();this._cellLayer.fullUpdate(Q,S,R,P);}
,_updateLayerWindow:function(U,W,V,T){this.__Ka(U,W,V.length,T.length);this.__Kb();this._cellLayer.updateLayerWindow(U,W,V,T);}
},destruct:function(){var X=this._getChildren();for(var i=0;i<X.length;i++ ){X[i].dispose();}
;this._disposeObjects(f,g);this._cellProvider=this.__Ag=this._cells=this._bounds=this._spanMap=null;}
});}
)();
