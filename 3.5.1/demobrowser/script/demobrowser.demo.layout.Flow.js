...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Flow.3ce100f520aa.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="7th a wide, short button",b="1. First Button",c="Flow Layout with line breaks",d="Number 6",e="icon/48/devices/computer.png",f="demobrowser.demo.layout.Flow",g="Flow Layout",h="center",j="20px Margins around the great big 5th button!",k="top",l="bottom",m="3rd really, really, really long Button",n="middle",o="Number 4",p="item #",q="2. Second longer Button...";qx.Class.define(f,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);this._createGalleryWindow(20,20);this._createLineBreakWindow(160,50);}
,_createGalleryWindow:function(u,top){var t=new qx.ui.window.Window(g).set({contentPadding:0,width:200});t.setLayout(new qx.ui.layout.Grow());var s=new qx.ui.container.Scroll();t.add(s);var r=new qx.ui.container.Composite(new qx.ui.layout.Flow()).set({allowShrinkY:false});s.add(r);for(var i=0;i<30;i++ ){r.add(new qx.ui.basic.Atom(p+(i+1),e).set({iconPosition:k,width:60,padding:5}));}
;t.moveTo(u,top);t.open();}
,_createLineBreakWindow:function(F,top){var E=new qx.ui.layout.Flow();E.setAlignX(h);var v=new qx.ui.window.Window(c).set({contentPadding:0,width:500,height:200,layout:E});var D=e;var y=new qx.ui.form.Button(b,D);v.add(y);var x=new qx.ui.form.Button(q,D);v.add(x,{lineBreak:true});var w=new qx.ui.form.Button(m,D);w.setHeight(100);v.add(w);var C=new qx.ui.form.Button(o,D);C.setAlignY(l);v.add(C);var B=new qx.ui.form.Button(j);B.setHeight(100);B.setMargin(20);v.add(B,{lineBreak:true});var A=new qx.ui.form.Button(d,D);A.setAlignY(n);v.add(A);var z=new qx.ui.form.Button(a,D);z.setMaxHeight(20);v.add(z);v.moveTo(F,top);v.open();}
}});}
)();
...
(function(){var a="lineBreak",b="' is not supported by the flow layout!",c="middle",d="_applyLayoutChange",e="stretch",f="qx.debug",g="bottom",h="qx.ui.layout.Flow",j="center",k="top",l="left",m="right",n="Integer",o="The property '",p="Boolean";qx.Class.define(h,{extend:qx.ui.layout.Abstract,construct:function(r,q,s){qx.ui.layout.Abstract.call(this);if(r){this.setSpacingX(r);}
;if(q){this.setSpacingY(q);}
;if(s){this.setAlignX(s);}
;}
,properties:{alignX:{check:[l,j,m],init:l,apply:d},alignY:{check:[k,c,g],init:k,apply:d},spacingX:{check:n,init:0,apply:d},spacingY:{check:n,init:0,apply:d},reversed:{check:p,init:false,apply:d}},members:{verifyLayoutProperty:qx.core.Environment.select(f,{"true":function(t,name,v){var u=[a,e];this.assertInArray(name,u,o+name+b);}
,"false":null}),connectToWidget:function(w){qx.ui.layout.Abstract.prototype.connectToWidget.call(this,w);if(w){w.setAllowShrinkY(false);}
;}
,renderLayout:function(A,y,z){var D=this._getLayoutChildren();if(this.getReversed()){D=D.concat().reverse();}
;var x=new qx.ui.layout.LineSizeIterator(D,this.getSpacingX());var B=z.top;while(x.hasMoreLines()){var C=x.computeNextLine(A);this.__Ij(C,B,A,z);B+=C.height+this.getSpacingY();}
;}
,__Ij:function(L,K,I,E){var H=qx.ui.layout.Util;var F=E.left;if(this.getAlignX()!=l){F=E.left+I-L.width;if(this.getAlignX()==j){F=E.left+Math.round((I-L.width)/2);}
;}
;for(var i=0;i<L.children.length;i++ ){var J=L.children[i];var M=J.getSizeHint();var N=J.getMarginTop();var G=J.getMarginBottom();var top=H.computeVerticalAlignOffset(J.getAlignY()||this.getAlignY(),N+M.height+G,L.height,N,G);var O=J.getLayoutProperties();if(O.stretch&&O.stretch){M.width+=I-L.width;}
;J.renderLayout(F+L.gapsBefore[i],K+top,M.width,M.height);F+=L.gapsBefore[i]+M.width;}
;}
,_computeSizeHint:function(){return this.__Ik(Infinity);}
,hasHeightForWidth:function(){return true;}
,getHeightForWidth:function(P){return this.__Ik(P).height;}
,getLastLineChildren:function(R){var Q=new qx.ui.layout.LineSizeIterator(this._getLayoutChildren(),this.getSpacingX());var S=[];while(Q.hasMoreLines()){S=Q.computeNextLine(R).children;}
;return S;}
,__Ik:function(T){var U=new qx.ui.layout.LineSizeIterator(this._getLayoutChildren(),this.getSpacingX());var V=0;var W=0;var X=0;while(U.hasMoreLines()){var Y=U.computeNextLine(T);X+=1;W=Math.max(W,Y.width);V+=Y.height;}
;return {width:W,height:V+this.getSpacingY()*(X-1)};}
}});}
)();
(function(){var a="No more lines to compute",b="qx.ui.layout.LineSizeIterator";qx.Class.define(b,{extend:Object,construct:function(d,c){this.__fn=d;this.__GA=c;this.__GB=d.length>0;this.__GC=0;}
,members:{__fn:null,__GA:null,__GB:null,__GC:null,computeNextLine:function(k){var k=k||Infinity;if(!this.__GB){throw new Error(a);}
;var p=this.__fn;var n=0;var f=0;var o=[];var j=[];for(var i=this.__GC;i<p.length;i++ ){var m=p[i];var q=m.getSizeHint();var g=this.__GD(i);var h=q.width+g;var e=i==this.__GC;if(!e&&f+h>k){this.__GC=i;break;}
;var l=q.height+m.getMarginTop()+m.getMarginBottom();o.push(m);j.push(g);f+=h;n=Math.max(n,l);if(m.getLayoutProperties().lineBreak){this.__GC=i+1;break;}
;}
;if(i>=p.length){this.__GB=false;}
;return {height:n,width:f,children:o,gapsBefore:j};}
,__GD:function(r){var s=r==this.__GC;if(s){return this.__fn[r].getMarginLeft();}
else {return Math.max(this.__fn[r-1].getMarginRight(),this.__fn[r].getMarginLeft(),this.__GA);}
;}
,hasMoreLines:function(){return this.__GB;}
}});}
)();
