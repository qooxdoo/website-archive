...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.DragDrop.2708cb500c1c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
);P.addListener(p,function(e){this.debug(s+e.getRelatedTarget());var W=e.getCurrentAction();var Y=e.getCurrentType();var V;switch(Y){case C:V=this.getSelection();if(W==q){var X=[];for(var i=0,l=V.length;i<l;i++ ){X[i]=V[i].clone();}
;V=X;}
;break;case k:V=this.getSelection()[0].getLabel();break;};if(W==o){var U=this.getSelection();for(var i=0,l=U.length;i<l;i++ ){this.remove(U[i]);}
;}
;e.addData(Y,V);}
);var L=new qx.ui.basic.Label(w);I.add(L,{left:140,top:20});var J=new qx.ui.form.List;J.setDroppable(true);J.setSelectionMode(u);I.add(J,{left:140,top:40});J.addListener(g,function(e){this.debug(E+e.getRelatedTarget());var ba=e.getData(C);for(var i=0,l=ba.length;i<l;i++ ){this.add(ba[i]);}
;}
);J.addListener(B,function(e){if(!e.supportsType(C)){e.preventDefault();}
;}
);var M=new qx.ui.basic.Label(j);I.add(M,{left:260,top:20});var T=new qx.ui.form.List;T.setDroppable(true);I.add(T,{left:260,top:40});T.addListener(g,function(e){this.debug(E+e.getRelatedTarget());var bb=e.getData(C);for(var i=0,l=bb.length;i<l;i++ ){this.add(bb[i]);}
;}
...
;}
);var L=new qx.ui.basic.Label(d);I.add(L,{left:380,top:20});var Q=new qx.ui.form.TextArea;Q.setDroppable(true);Q.setHeight(100);I.add(Q,{left:380,top:40});Q.addListener(g,function(e){this.debug(E+e.getRelatedTarget());this.setValue(e.getData(k).replace(/,/g,z));}
);Q.addListener(B,function(e){if(!e.supportsType(k)){e.preventDefault();}
;}
);var N=new qx.ui.basic.Label(m);I.add(N,{left:600,top:20});var G=this.__LW=new qx.ui.form.List();G.setDraggable(true);G.setDroppable(true);G.setSelectionMode(u);I.add(G,{left:600,top:40});for(var i=0;i<20;i++ ){G.add(new qx.ui.form.ListItem(D+i,h));}
...
);G.addListener(c,function(e){var bd=e.getOriginalTarget();if(bd instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__SP=bd;}
...
;}
);G.addListener(g,function(e){this.__SQ(e.getOriginalTarget());}
,this);H.addListener(g,function(e){this.__SQ(this.__SP);}
,this);}
,__SQ:function(bf){if(bf.classname!=n){return;}
;var be=this.__LW.getSortedSelection();for(var i=0,l=be.length;i<l;i++ ){this.__LW.addBefore(be[i],bf);this.__LW.addToSelection(be[i]);}
;}
