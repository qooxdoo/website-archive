...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.bom.WebWorker","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.WebWorker.482b17239468.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="demobrowser/demo/webworker/webworker.js",b="",c="Your browser doesn't support Web Workers",d="30",f="message",g="html.webworker",h="Calculate",i="Calculating...",j="demobrowser.demo.bom.WebWorker",k="Your browser supports Web Workers",l="execute",m="Calculate Fibonacci Numbers in a Web Worker";qx.Class.define(j,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var r=this.getRoot();var q=qx.util.ResourceManager.getInstance().toUri(a);var s=new qx.bom.WebWorker(q);var u=new qx.ui.basic.Label(m);var n=new qx.ui.basic.Label(b);var p=new qx.ui.form.Button(h).set({width:100});var o=new qx.ui.form.TextField(d);var t=new qx.ui.form.List().set({width:200});if(qx.core.Environment.get(g)){n.setValue(k);}
else {n.setValue(c);}
;r.add(n,{left:20,top:10});r.add(u,{left:20,top:40});r.add(o,{left:20,top:60});r.add(p,{left:120,top:58});r.add(t,{left:20,top:90});s.addListener(f,function(e){var v=e.getData();t.addAt(new qx.ui.form.ListItem(b+v.result),0);if(v.done){p.setEnabled(true);o.setEnabled(true);p.setLabel(h);}
;}
);p.addListener(l,function(e){t.removeAll();s.postMessage(parseInt(o.getValue()||0,10));p.setEnabled(false);o.setEnabled(false);p.setLabel(i);}
,this);}
}});}
)();
(function(){var a="GET",b="error",c="onmessage",d='message',f="message",g="html.webworker",h="var onmessage = null, postMessage = ",i="qx.bom.WebWorker",j=";",k="qx.event.type.Data";qx.Class.define(i,{extend:qx.core.Object,construct:function(l){qx.core.Object.call(this);this.__Hx=qx.core.Environment.get(g);this.__Hx?this.__Hz(l):this.__HA(l);}
,events:{"message":k,"error":k},members:{_worker:null,_handleErrorBound:null,_handleMessageBound:null,__Hx:true,__Hy:null,__Hz:function(m){this._worker=new window.Worker(m);this._handleMessageBound=qx.lang.Function.bind(this._handleMessage,this);this._handleErrorBound=qx.lang.Function.bind(this._handleError,this);qx.bom.Event.addNativeListener(this._worker,f,this._handleMessageBound);qx.bom.Event.addNativeListener(this._worker,b,this._handleErrorBound);}
,__HA:function(src){var that=this;var req=new qx.bom.request.Xhr();req.onload=function(){that.__Hy=(function(){var postMessage=function(e){that.fireDataEvent(d,e);}
;eval(h+postMessage+j+req.responseText);return {onmessage:eval(c),postMessage:postMessage};}
)();}
;req.open(a,src,false);req.send();}
,postMessage:function(n){var o=this;if(this.__Hx){this._worker.postMessage(n);}
else {setTimeout(function(){try{o.__Hy.onmessage&&o.__Hy.onmessage({data:n});}
catch(p){o.fireDataEvent(b,p);}
;}
,0);}
;}
,_handleMessage:function(e){this.fireDataEvent(f,e.data);}
,_handleError:function(e){this.fireDataEvent(b,e.message);}
},destruct:function(){if(this.__Hx){qx.bom.Event.removeNativeListener(this._worker,f,this._handleMessageBound);qx.bom.Event.removeNativeListener(this._worker,b,this._handleErrorBound);if(this._worker){this._worker.terminate();this._worker=null;}
;}
else {if(this.__Hy){this.__Hy=null;}
;}
;}
});}
)();
(function(){var a="Retry open native request with method: ",b="activex",c="No XHR support available.",d="If-None-Match",f="xhr",g="If-Modified-Since",h="engine.version",i="onunload",j="GET",k="-1",l="qx.debug.io",m="Open native request with method: ",n="error",o="loadend",p="load",q="Received native readyState: ",r="abort",s=", url: ",t="browser.documentmode",u="",v="readystatechange",w="Microsoft.XMLHTTP",x="Already disposed",y="browser.version",z="opera",A="qx.bom.request.Xhr",B="on",C="gecko",D="If-Match",E="mshtml",F="engine.name",G="Microsoft.XMLDOM",H="file:",I="Send native request",J="If-Range",K="Content-Type",L="io.xhr",M="Not enough arguments",N="timeout",O="undefined",P=", async: ",Q="Native XHR object doesn't support overrideMimeType.";qx.Bootstrap.define(A,{extend:Object,construct:function(){var R=qx.Bootstrap.bind(this.__uk,this);if(qx.event&&qx.event.GlobalError&&qx.event.GlobalError.observeMethod){this.__tW=qx.event.GlobalError.observeMethod(R);}
else {this.__tW=R;}
;this.__tX=qx.Bootstrap.bind(this.__uj,this);this.__tN=qx.Bootstrap.bind(this.__uo,this);this.__ui();this._emitter=new qx.event.Emitter();if(window.attachEvent){this.__tY=qx.Bootstrap.bind(this.__ur,this);window.attachEvent(i,this.__tY);}
;}
,statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},events:{"readystatechange":A,"error":A,"loadend":A,"timeout":A,"abort":A,"load":A},members:{readyState:0,responseText:u,responseXML:null,status:0,statusText:u,timeout:0,open:function(W,S,T,V,U){this.__ut();if(typeof S===O){throw new Error(M);}
else if(typeof W===O){W=j;}
;this.__tQ=false;this.__ua=false;this.__ub=false;this.__uc=S;if(typeof T==O){T=true;}
;this.__ud=T;if(!this.__us()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();this.__ui();}
;this.__uf.onreadystatechange=this.__tW;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,m+W+s+S+P+T);}
;this.__uf.open(W,S,T,V,U);}
catch(X){if(!qx.util.Request.isCrossDomain(S)){throw X;}
;if(!this.__ud){this.__ue=X;}
;if(this.__ud){if(window.XDomainRequest){this.readyState=4;this.__uf=new XDomainRequest();this.__uf.onerror=qx.Bootstrap.bind(function(){this._emit(v);this._emit(n);this._emit(o);}
,this);if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,a+W+s+S+P+T);}
;this.__uf.open(W,S,T,V,U);return;}
;window.setTimeout(qx.Bootstrap.bind(function(){if(this.__uh){return;}
;this.readyState=4;this._emit(v);this._emit(n);this._emit(o);}
,this));}
;}
;if(qx.core.Environment.get(F)===E&&qx.core.Environment.get(t)<9&&this.__uf.readyState>0){this.__uf.setRequestHeader(g,k);}
;if(qx.core.Environment.get(F)===C&&parseInt(qx.core.Environment.get(h),10)<2&&!this.__ud){this.readyState=qx.bom.request.Xhr.OPENED;this._emit(v);}
;}
,setRequestHeader:function(Y,ba){this.__ut();if(Y==D||Y==g||Y==d||Y==J){this.__ub=true;}
;this.__uf.setRequestHeader(Y,ba);return this;}
,send:function(bb){this.__ut();if(!this.__ud&&this.__ue){throw this.__ue;}
;if(qx.core.Environment.get(F)===z&&this.timeout===0){this.timeout=10000;}
;if(this.timeout>0){this.__ug=window.setTimeout(this.__tN,this.timeout);}
;bb=typeof bb==O?null:bb;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,I);}
;this.__uf.send(bb);}
catch(bd){if(!this.__ud){throw bd;}
;if(this._getProtocol()===H){this.readyState=2;this.__ul();var bc=this;window.setTimeout(function(){if(bc.__uh){return;}
;bc.readyState=3;bc.__ul();bc.readyState=4;bc.__ul();}
);}
;}
;if(qx.core.Environment.get(F)===C&&!this.__ud){this.__uk();}
;this.__ua=true;return this;}
,abort:function(){this.__ut();this.__tQ=true;this.__uf.abort();if(this.__uf){this.readyState=this.__uf.readyState;}
;return this;}
...
,getResponseHeader:function(bg){this.__ut();return this.__uf.getResponseHeader(bg);}
,getAllResponseHeaders:function(){this.__ut();return this.__uf.getAllResponseHeaders();}
,overrideMimeType:function(bh){this.__ut();if(this.__uf.overrideMimeType){this.__uf.overrideMimeType(bh);}
else {throw new Error(Q);}
;return this;}
,getRequest:function(){return this.__uf;}
,dispose:function(){if(this.__uh){return false;}
;window.clearTimeout(this.__ug);if(window.detachEvent){window.detachEvent(i,this.__tY);}
;try{this.__uf.onreadystatechange;}
catch(bj){return false;}
;var bi=function(){}
;this.__uf.onreadystatechange=bi;this.__uf.onload=bi;this.__uf.onerror=bi;this.abort();this.__uf=null;this.__uh=true;return true;}
,isDisposed:function(){return !!this.__uh;}
...
,_getProtocol:function(){var bl=this.__uc;var bm=/^(\w+:)\/\//;if(bl!==null&&bl.match){var bn=bl.match(bm);if(bn&&bn[1]){return bn[1];}
;}
;return window.location.protocol;}
,__uf:null,__ud:null,__tW:null,__tX:null,__tY:null,__tN:null,__ua:null,__uc:null,__tQ:null,__eP:null,__uh:null,__ug:null,__ue:null,__ub:null,__ui:function(){this.__uf=this._createNativeXhr();this.__uf.onreadystatechange=this.__tW;if(this.__uf.onabort){this.__uf.onabort=this.__tX;}
;this.__uh=this.__ua=this.__tQ=false;}
,__uj:function(){if(!this.__tQ){this.abort();}
;}
,__uk:function(){var bo=this.__uf,bp=true;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,q+bo.readyState);}
;if(this.readyState==bo.readyState){return;}
;this.readyState=bo.readyState;if(this.readyState===qx.bom.request.Xhr.DONE&&this.__tQ&&!this.__ua){return;}
;if(!this.__ud&&(bo.readyState==2||bo.readyState==3)){return;}
...
;if(bp){this.__up();this.__uq();}
;}
;this.__ul();if(this.readyState==qx.bom.request.Xhr.DONE){if(bo){bo.onreadystatechange=function(){}
;}
;}
;}
,__ul:function(){var br=this;if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__ug);}
;if(qx.core.Environment.get(F)==E&&qx.core.Environment.get(t)<8){if(this.__ud&&!this.__ua&&this.readyState>=qx.bom.request.Xhr.LOADING){if(this.readyState==qx.bom.request.Xhr.LOADING){return;}
;if(this.readyState==qx.bom.request.Xhr.DONE){window.setTimeout(function(){if(br.__uh){return;}
;br.readyState=3;br._emit(v);br.readyState=4;br._emit(v);br.__um();}
);return;}
;}
;}
;this._emit(v);if(this.readyState===qx.bom.request.Xhr.DONE){this.__um();}
;}
,__um:function(){if(this.__eP){this._emit(N);if(qx.core.Environment.get(F)===z){this._emit(n);}
;this.__eP=false;}
else {if(this.__tQ){this._emit(r);}
else {if(this.__un()){this._emit(n);}
else {this._emit(p);}
;}
;}
;this._emit(o);}
,__un:function(){var bs;if(this._getProtocol()===H){bs=!this.responseText;}
else {bs=!this.statusText;}
;return bs;}
,__uo:function(){var bt=this.__uf;this.readyState=qx.bom.request.Xhr.DONE;this.__eP=true;bt.abort();this.responseText=u;this.responseXML=null;this.__ul();}
,__up:function(){var bu=this.readyState===qx.bom.request.Xhr.DONE;if(this._getProtocol()===H&&this.status===0&&bu){if(!this.__un()){this.status=200;}
;}
;if(this.status===1223){this.status=204;}
;if(qx.core.Environment.get(F)===z){if(bu&&this.__ub&&!this.__tQ&&this.status===0){this.status=304;}
;}
;}
,__uq:function(){if(qx.core.Environment.get(F)==E&&(this.getResponseHeader(K)||u).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var bv=new window.ActiveXObject(G);bv.async=false;bv.validateOnParse=false;bv.loadXML(this.responseText);this.responseXML=bv;}
;}
,__ur:function(){try{if(this){this.dispose();}
;}
catch(e){}
;}
,__us:function(){var name=qx.core.Environment.get(F);var bw=qx.core.Environment.get(y);return !(name==E&&bw<9||name==C&&bw<3.5);}
,__ut:function(){if(this.__uh){throw new Error(x);}
;}
