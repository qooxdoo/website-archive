...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.7c61909bcf0a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b=" => ",c="#autoActionsBeforeOnexit",d=": Could not find friendly name for '",f="__Jd",g=": event '",h=" blocked.  Re-queuing.",j="string",k="'",l="Attempt to transition to nonexistent state ",m="Saved-state stack is empty",n="Internal error: invalid nextState",o=" not handled.  Ignoring.",p="State ",q="#autoActionsBeforeOntransition",r=" returned a value other than ",s="#",u=" already exists",v="Attempt to transition to POP_STATE_STACK ",w=":",x="Transition ",y=" does not exist",z="' on '",A="qx.util.fsm.FiniteStateMachine",B="String",C="#autoActionsAfterOntransition",D="",E="#actionsAfterOnentry",F="Saved-state stack is full",G="#autoActionsBeforeOnentry",H=": no transition found.  No state change.",I="#autoActionsAfterOnentry",J="#actionsBeforeOnentry",K=": Pushed event: ",L="Machine started with no available states",M=": Process event: ",N="#entry",O="#ontransition",P=" on ",Q="__Je",R="Invalid state: not an instance of ",S="Number",T="#exit",U="true, false, or null.",V="Explicit transition ",W="  ",X="qx.util.fsm.State",Y="while state stack is empty.",bj=".  Ignoring.",bk="#autoActionsAfterOnexit",bl=": Event '",bg="object",bh=" not handled for target ",bi=": Queued event: ";qx.Class.define(A,{extend:qx.core.Object,construct:function(bm){qx.core.Object.call(this);this.setName(bm);this.__iH={};this.__Jb=null;this.__Jc=[];this.__Jd=[];this.__Je=[];this.__Jf={};this.__Jg={};this.__Jh={};this.__Ji={};this.__Jj={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:B,nullable:true},state:{check:B,nullable:true},previousState:{check:B,nullable:true},nextState:{check:B,nullable:true},maxSavedStates:{check:S,init:2},debugFlags:{check:S,init:7}},members:{__iH:null,__Jb:null,__Jd:null,__Je:null,__Jc:null,__Jf:null,__Jg:null,__Jh:null,__Ji:null,__Jj:null,__Jk:false,addState:function(bn){if(!bn instanceof qx.util.fsm.State){throw new Error(R+X);}
;var bo=bn.getName();if(bo in this.__iH){throw new Error(p+bo+u);}
;if(this.__Jb==null){this.__Jb=bo;}
;this.__iH[bo]=bn;}
,replaceState:function(bs,br){if(!bs instanceof qx.util.fsm.State){throw new Error(R+X);}
;var bq=bs.getName();var bp=this.__iH[bq];this.__iH[bq]=bs;if(br){bp._bNeedDispose=true;}
;return bp;}
,addObject:function(bw,bt,bu){var bx=bt.toHashCode();this.__Jg[bw]=bx;this.__Jh[bx]=bw;this.__Jf[bw]=bt;if(!bu){return;}
;if(typeof (bu)==j){bu=[bu];}
;for(var i=0;i<bu.length;i++ ){var bv=bu[i];if(!this.__Ji[bv]){this.__Ji[bv]={};}
;this.__Ji[bv][bw]=true;if(!this.__Jj[bw]){this.__Jj[bw]=[];}
;this.__Jj[bw].push(bv);}
;}
,removeObject:function(bB){var by;var bz;var bA;var bC;by=this.__Jg[bB];if(this.__Jj[bB]){for(var i=0;i<this.__Jj[bB].length;i++ ){bz=this.__Jj[bB][i];delete this.__Ji[bz][bB];bC=true;for(bA in this.__Ji[bz]){bC=false;break;}
;if(bC){delete this.__Ji[bz];}
;}
;delete this.__Jj[bB];}
;delete this.__Jh[by];delete this.__Jg[bB];delete this.__Jf[bB];}
,getObject:function(bD){return this.__Jf[bD];}
,getFriendlyName:function(bE){var bF=bE?bE.toHashCode():null;return bF?this.__Jh[bF]:null;}
,getGroupObjects:function(bG){var a=[];for(var name in this.__Ji[bG]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bH in this.__Jg){var bJ=this.__Jg[bH];var bI=this.getObject(bH);this.debug(bH+b+bJ);this.debug(W+bJ+b+this.__Jh[bJ]);this.debug(W+bH+b+this.getObject(bH));this.debug(W+this.getObject(bH)+b+this.getFriendlyName(bI));}
;}
,_getInternalData:function(){return ({"states":this.__iH,"startState":this.__Jb,"eventQueue":this.__Jd,"blockedEvents":this.__Je,"savedStates":this.__Jc,"friendlyToObject":this.__Jf,"friendlyToHash":this.__Jg,"hashToFriendly":this.__Jh,"groupToFriendly":this.__Ji,"friendlyToGroups":this.__Jj});}
,start:function(){var bL=this.__Jb;if(bL==null){throw new Error(L);}
...
;this.__iH[bL].getAutoActionsBeforeOnentry()(this);if(bK){this.debug(this.getName()+s+bL+N);}
;this.__iH[bL].getOnentry()(this,null);if(bK){this.debug(this.getName()+s+bL+E);}
;this.__iH[bL].getAutoActionsAfterOnentry()(this);}
,pushState:function(bM){if(this.__Jc.length>=this.getMaxSavedStates()){throw new Error(F);}
;if(bM===true){this.__Jc.push(this.getState());}
else if(bM){this.__Jc.push(bM);}
else {this.__Jc.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Jc.length==0){throw new Error(m);}
;return this.__Jc.pop();}
,postponeEvent:function(event){this.__Je.unshift(event);}
,enqueueEvent:function(event,bN){if(bN){this.__Jd.push(event);}
else {this.__Jd.unshift(event);}
;if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){var bO=this.getFriendlyName(event.getTarget());if(bN){this.debug(this.getName()+K+event.getType()+(bO?P+bO:D));}
else {this.debug(this.getName()+bi+event.getType()+(bO?P+bO:D));}
;}
;}
...
;this.__Jk=true;while(this.__Jd.length>0){var event=this.__Jd.pop();var bW=this.__Jm(event);if(bW){event.dispose();}
;}
;this.__Jk=false;}
,__Jm:function(event){var ca;var cb;var bY;var cd;var cg;var e;var cf;var bX=this.getDebugFlags();var ck=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var ci=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var ch=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var ce=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var cj=this.getFriendlyName(event.getTarget());if(ck){this.debug(this.getName()+M+event.getType()+(cj?P+cj:D));}
;ca=this.getState();cd=this.__iH[ca];cg=cd.transitions;e=cd.getEvents()[event.getType()];if(!e){if(ck){this.debug(this.getName()+bl+event.getType()+k+o);}
;return true;}
...
;this.__Je.unshift(event);return false;default:if(typeof (cf)==j){if(cg[cf]){var cc=cg[cf];cg={};cg[cf]=cc;}
else {throw new Error(V+cf+y);}
;break;}
;};for(var t in cg){var cc=cg[t];switch(cc.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(x+ca+w+t+r+U);};cb=cc.getNextState();if(typeof (cb)==j){if(!cb in this.__iH){throw new Error(l+cb);}
;this.setNextState(cb);}
else {switch(cb){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cb=ca;this.setNextState(cb);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Jc.length==0){throw new Error(v+Y);}
;cb=this.__Jc.pop();this.setNextState(cb);break;default:throw new Error(n);};}
;if(ch){this.debug(this.getName()+s+ca+s+t+q);}
;cc.getAutoActionsBeforeOntransition()(this);if(ch){this.debug(this.getName()+s+ca+s+t+O);}
...
;cd=this.__iH[this.getNextState()];this.setPreviousState(ca);this.setState(this.getNextState());this.setNextState(null);bY=ca;ca=cb;cb=undefined;if(ch){this.debug(this.getName()+s+ca+G);}
;cd.getAutoActionsBeforeOnentry()(this);if(ch){this.debug(this.getName()+s+ca+N);}
;cd.getOnentry()(this,event);if(ch){this.debug(this.getName()+s+ca+I);}
;cd.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__Je.length;i++ ){e=this.__Je.pop();this.__Jd.unshift(e);}
;if(ci){this.debug(this.getName()+s+bY+b+this.getName()+s+ca);}
;return true;}
;if(ci){this.debug(this.getName()+s+ca+g+event.getType()+k+H);}
;return true;}
},destruct:function(){this._disposeArray(f);this._disposeArray(Q);this.__Jc=this.__iH=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="Invalid state name",k="parameters",l=")",m=" fsm.getObject('",n="groups",o="Invalid function parameters: ",p="__IY",q="  for (var i = 0; i < groupObjects.length; i++)",r="context",s="__IW",t="string",u="autoActionsAfterOnentry",v="Invalid numeric value in events object: ",w='"',x="__IR",y="__IT",z=" value: ",A="');",B="State ",C="autoActionsBeforeOnexit",D="autoActionsBeforeOnentry",E="}",F="): ",G="events must be an object",H="onexit",I="{",J="Invalid function request type: ",K=": ",L="__IV",M="expected array, found ",N="Invalid friendly name in 'objects' list: ",O="autoActionsAfterOnexit",P="  fsm.debug(ex);",Q="(",R="Invalid onentry type: ",S="    fsm.getObject(objName).",T=")(fsm)",U="events",V="undefined",W="Invalid ",X="__IS",Y="try",ba="    var objName = groupObjects[i];",bb="Invalid value in events object: ",bc="').",bd="objects",be="expected object, found ",bf="qx.util.fsm.Transition",bg="The events object must be provided in new state info",bh="catch(ex)",bi="  {",bj="__IU",bk="    fsm.getGroupObjects('",bl="Invalid value in events object ",bm="Invalid transition: not an instance of ",bn="  }",bo="__IX",bp="Invalid numeric value in events object ",bq="Invalid 'objects' list: expected array, got ",br="Invalid onexit type: ",bs="qx.util.fsm.State",bt="onentry",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(bs,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(r,bE);for(var bC in bF){switch(bC){case bt:this.setOnentry(this.__Ja(bF[bC],bE));break;case H:this.setOnexit(this.__Ja(bF[bC],bE));break;case D:this.setAutoActionsBeforeOnentry(bF[bC]);break;case u:this.setAutoActionsAfterOnentry(bF[bC]);break;case C:this.setAutoActionsBeforeOnexit(bF[bC]);break;case O:this.setAutoActionsAfterOnexit(bF[bC]);break;case U:this.setEvents(bF[bC]);break;case r:break;default:this.setUserData(bC,bF[bC]);this.debug(B+bD+K+bz+bC);break;};}
...
;for(j=0;j<g.length;j++ ){func+=d+bk+g[j]+A+q+bi+ba+S+funcFragment+bu+bn;}
;}
;}
;}
;func+=E+bh+I+P+E;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:x,nullable:true},onentry:{transform:X,nullable:true,init:function(bG,event){}
},onexit:{transform:y,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:L,nullable:true,init:function(bI,event){}
...
},autoActionsAfterOnexit:{transform:p,nullable:true,init:function(bL,event){}
},events:{transform:bj,nullable:true}},members:{__IR:function(bM){if(typeof (bM)!=t||bM.length<1){throw new Error(h);}
;return bM;}
,__IS:function(bN){switch(typeof (bN)){case V:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(r));default:throw new Error(R+typeof (bN));};}
,__IT:function(bP){switch(typeof (bP)){case V:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(r));default:throw new Error(br+typeof (bP));};}
,__IU:function(bT){if(typeof (bT)!=bB){throw new Error(G);}
;for(var e in bT){var bR=bT[e];if(typeof (bR)==bA&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(v+e+K+bR);}
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==bA&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(bp+Q+e+F+bS+K+bR[bS]);}
else if(typeof (bR[bS])!=t&&typeof (bR[bS])!=bA){throw new Error(bl+Q+e+F+bS+K+bR[bS]);}
;}
;}
else if(typeof (bR)!=t&&typeof (bR)!=bA){throw new Error(bb+e+K+bT[e]);}
;}
;return bT;}
,__IV:function(bU){return qx.util.fsm.State._commonTransformAutoActions(D,bU,this.getUserData(r));}
,__IW:function(bV){return qx.util.fsm.State._commonTransformAutoActions(u,bV,this.getUserData(r));}
,__IX:function(bW){return qx.util.fsm.State._commonTransformAutoActions(C,bW,this.getUserData(r));}
,__IY:function(bX){return qx.util.fsm.State._commonTransformAutoActions(O,bX,this.getUserData(r));}
,__Ja:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bm+bf);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="or one of the Fsm.StateChange constants",d="autoActionsAfterOntransition",e="context",g="string",h="predicate",i="__Jo",j="nextState must be an explicit state name, ",k="Transition ",l=": ",m="String",n="number",o="Invalid transition nextState type: ",p="Invalid transition nextState value: ",q="Adding user-provided field to transition: ",r="ontransition",s="Invalid transition predicate type: ",t="boolean",u="__Jn",v="nextState",w="Invalid ontransition type: ",x="__Jq",y="__Jp",z="__Jr",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(e,E);for(var C in D){switch(C){case h:this.setPredicate(this.__Ja(D[C],E));break;case v:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__Ja(D[C],E));break;case d:this.setAutoActionsAfterOntransition(this.__Ja(D[C],E));break;case r:this.setOntransition(this.__Ja(D[C],E));break;case e:break;default:this.setUserData(C,D[C]);this.debug(k+F+l+q+C);break;};}
;}
,properties:{name:{check:m,nullable:true},predicate:{init:function(G,event){return true;}
,transform:u},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:i},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:y},autoActionsAfterOntransition:{init:function(I,event){}
,transform:x},ontransition:{init:function(J,event){}
,transform:z}},members:{__Jn:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
;case t:return function(M,event){return K;}
;case a:return qx.lang.Function.bind(K,this.getUserData(e));default:throw new Error(s+typeof (K));};}
,__Jo:function(N){switch(typeof (N)){case g:return N;case n:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(p+N+l+j+c);};break;default:throw new Error(o+typeof (N));};}
,__Jp:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(e));}
,__Jq:function(P){return qx.util.fsm.State._commonTransformAutoActions(d,P,this.getUserData(e));}
,__Jr:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(e));default:throw new Error(w+typeof (Q));};}
,__Ja:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
