...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Location_StandardMode.ec0fc19b3cf8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="borderBottomWidth",b="list",c='#',d="css.boxmodel",e="px",f="mode",g="borderTopWidth",h="borderLeftWidth",j="a",k="border",m="div",n="moveable",o="borderRightWidth",p="demobrowser.demo.bom.Location_StandardMode";qx.Class.define(p,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var q=document.getElementById(b).getElementsByTagName(j);for(var i=0,l=q.length;i<l;i++ ){q[i].onclick=demobrowser.demo.bom.Location_StandardMode.jump;}
;}
...
;function r(x,w){return parseInt(qx.bom.element.Style.get(x,w,qx.bom.element.Style.COMPUTED_MODE,false),10);}
;var t=this.href;var v=t.substr(t.indexOf(c)+1);var s=document.getElementById(v);var location=qx.bom.element.Location.get(s,document.getElementById(f).value);var u=document.getElementById(n);u.style.left=location.left+e;u.style.top=location.top+e;if(qx.core.Environment.get(d)==k){u.style.width=(location.right-location.left)+e;u.style.height=(location.bottom-location.top)+e;}
else {u.style.width=(location.right-location.left-r(u,h)-r(u,o))+e;u.style.height=(location.bottom-location.top-r(u,g)-r(u,a))+e;}
;return false;}
,randomize:function(){var y=document.body.getElementsByTagName(m);for(var i=0,l=y.length;i<l;i++ ){if(y[i].id&&y[i].id!=n){y[i].style.margin=Math.round(Math.random()*10)+e;y[i].style.padding=Math.round(Math.random()*10)+e;y[i].style.borderWidth=Math.round(Math.random()*10)+e;}
;}
;}
}});}
)();
