...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Github.55c0c37416fa.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
},members:{_resource:null,__Jw:null,__Jx:null,__Jy:null,_getResource:function(v){return new qx.bom.rest.Resource(v);}
...
,map:function(G,I,H,J){this.__Jz(G,I,H,J);this._resource.map(G,I,H,J);}
,__Jz:function(L,N,K,M){this.__Jy[L]=[N,K,M];if(L==m){this[L]=undefined;}
;if(typeof this[L]!==q&&this[L]!==null&&this[L].action!==true){throw new Error(o+L+c);}
;this.__JB(L+p);this.__JB(L+f);this[L]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,L);return this.invoke.apply(this,arguments);}
,this);this[L].action=true;}
,invoke:function(O,P,Q){var P=(P==null)?{}:P;this.__Jy[O].params=P;return this._resource.invoke(O,P,Q);}
...
,poll:function(Y,U,X,ba){if(this.__Jx[Y]){this.__Jx[Y].dispose();}
;if(typeof X==q){X=this.__Jy[Y].params;}
...
;}
;var V=this.__Jx[Y]=new qx.event.Timer(U);V.addListener(i,W,this._resource);V.start();return V;}
...
;if(!bj){bi=0;}
;return false;}
;var bf=this.__Jw[bh]=this.addListener(bh+p,function bk(){if(be.isDisposed()){return;}
...
,__JA:function(bn){Object.keys(bn).forEach(function(bp){var bq=bn[bp],bs=bq.method,bo=bq.url,br=bq.check;if(qx.core.Environment.get(s)){qx.core.Assert.assertString(bs,a+bp+b);qx.core.Assert.assertString(bo,n+bp+b);}
;this.map(bp,bs,bo,br);}
,this);}
,__JB:function(bt){if(!this.constructor.$$events){this.constructor.$$events={};}
;if(!this.constructor.$$events[bt]){this.constructor.$$events[bt]=j;}
;}
},destruct:function(){var bu;if(this.__Jx){for(bu in this.__Jx){var bw=this.__Jx[bu];bw.stop();bw.dispose();}
;}
;if(this.__Jw){for(bu in this.__Jw){var bv=this.__Jw[bu];this.removeListenerById(bv);}
;}
;this._resource.destruct();this._resource=this.__Jy=this.__Jx=this.__Jw=null;}
});}
)();
...
;return S;}
},members:{__JC:null,__Jy:null,__JD:null,__Jx:null,__Jw:null,__JE:null,__JF:null,__JG:null,setRequestFactory:function(V){this.__JG=V;}
,setRequestHandler:function(W){this.__JF=W;}
,_getRequestHandler:function(){return (this.__JF===null)?{onsuccess:{callback:function(Y,X){return function(){var ba={"id":parseInt(Y.toHashCode(),10),"response":Y.getResponse(),"request":Y,"action":X};this.emit(X+G,ba);this.emit(w,ba);}
;}
...
;}
,context:this}}:this.__JF;}
,getRequestsByAction:function(bg){var bh=(this.__JC!==null&&bg in this.__JC);return bh?this.__JC[bg]:null;}
,configureRequest:function(bi){this.__JE=bi;}
,_getRequest:function(){return (this.__JG===null)?new qx.bom.request.SimpleXhr():this.__JG();}
,__JH:function(bj){var bk=this._getRequest();if(!qx.lang.Type.isArray(this.__JC[bj])){this.__JC[bj]=[];}
;this.__JC[bj].push(bk);return bk;}
,map:function(bm,bo,bl,bn){this.__Jy[bm]=[bo,bl,bn];this.__JC[bm]=[];if(bm==u){this[bm]=undefined;}
;if(typeof this[bm]!==N&&this[bm]!==null&&this[bm].action!==true){throw new Error(m+bm+y);}
;this.__JB(bm+G);this.__JB(bm+z);this[bm]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,bm);return this.invoke.apply(this,arguments);}
,this);this[bm].action=true;}
,invoke:function(bt,br,bp){var bq=this.__JH(bt),br=br==null?{}:br,bu=this._getRequestConfig(bt,br);this.__Jy[bt].params=br;this.__JI(br,bu.check);this.__Gk(bq,bu,bp);if(this.__JE){this.__JE.call(this,bq,bt,br,bp);}
;this.__JJ(bq,bu,bp);var bs=this._getRequestHandler();bq.addListenerOnce(w,bs.onsuccess.callback(bq,bt),bs.onsuccess.context);bq.addListenerOnce(s,bs.onfail.callback(bq,bt),bs.onfail.context);bq.addListenerOnce(g,bs.onloadend.callback(bq,bt),bs.onloadend.context);bq.send();return parseInt(bq.toHashCode(),10);}
,setBaseUrl:function(bv){this.__JD=bv;}
,__JI:function(bw,bx){if(typeof bx!==N){if(qx.core.Environment.get(r)){qx.core.Assert.assertObject(bx,M);}
...
;}
,__Gk:function(bA,bz,bB){bA.setUrl(bz.url);if(!bA.setMethod&&bz.method!==j){throw new Error(D+bA.classname+x);}
;if(bA.setMethod){bA.setMethod(bz.method);}
;if(bB){bA.setRequestData(bB);}
;}
,__JJ:function(bE,bC,bD){if(bD){var bF=bE.getRequestHeader(K);if(bE.getMethod&&qx.util.Request.methodAllowsRequestBody(bE.getMethod())){if((/application\/.*\+?json/).test(bF)){bD=qx.lang.Json.stringify(bD);bE.setRequestData(bD);}
;}
;}
;}
...
;}
else {var bJ=bI;var bH=this.__JC[bJ];if(this.__JC[bJ]){bH.forEach(function(bM){bM.abort();}
);}
;}
;}
,refresh:function(bN){this.invoke(bN,this.__Jy[bN].params);}
,poll:function(bO,bQ,bR,bP){if(this.__Jx[bO]){this.stopPollByAction(bO);}
;if(typeof bR==N){bR=this.__Jy[bO].params;}
;if(bP){this.invoke(bO,bR);}
;var bS=(function(bT){return function(){var bU=bT.__JC[bO][0];if(!bP&&!bU){bT.invoke(bO,bR);return;}
;if(bU.isDone()||bU.isDisposed()){bT.refresh(bO);}
;}
;}
)(this);this._startPoll(bO,bS,bQ);}
,_startPoll:function(bV,bW,bX){this.__Jx[bV]={"id":window.setInterval(bW,bX),"interval":bX,"listener":bW};}
,stopPollByAction:function(bY){if(bY in this.__Jx){var ca=this.__Jx[bY].id;window.clearInterval(ca);}
;}
,restartPollByAction:function(cb){if(cb in this.__Jx){var cc=this.__Jx[cb];this.stopPollByAction(cb);this._startPoll(cb,cc.listener,cc.interval);}
;}
...
;if(!cj){ci=0;}
;return false;}
;var cf=this.__Jw[ch]=this.addListener(ch+G,function ck(){if(ce.isDisposed()){return;}
;if(!cd()){cg=new Date();ce.refresh(ch);}
;}
);this.invoke(ch);return cf;}
,_getRequestConfig:function(cp,co){var cm=this.__Jy[cp];var co=qx.lang.Object.clone(co);if(!qx.lang.Type.isArray(cm)){throw new Error(P+cp);}
;var cn=cm[0],cl=this.__JD!==null?this.__JD+cm[1]:cm[1],cr=cm[2],cq=qx.bom.rest.Resource.placeholdersFromUrl(cl);co=co||{};cq.forEach(function(cu){var cs=new RegExp(v+cu+c),ct=cl.match(cs)[1];if(typeof co[cu]===N){if(ct){co[cu]=ct;}
else {co[cu]=A;}
;}
...
,__JA:function(cv){Object.keys(cv).forEach(function(cx){var cy=cv[cx],cA=cy.method,cw=cy.url,cz=cy.check;if(qx.core.Environment.get(r)){qx.core.Assert.assertString(cA,I+cx+o);qx.core.Assert.assertString(cw,l+cx+o);}
;this.map(cx,cA,cw,cz);}
,this);}
,__JB:function(cB){if(!this.constructor.$$events){this.constructor.$$events={};}
...
;}
;}
,destruct:function(){var cG;for(cG in this.__JC){if(this.__JC[cG]){this.__JC[cG].forEach(function(cI){cI.dispose();}
);}
;}
;if(this.__Jx){for(cG in this.__Jx){this.stopPollByAction(cG);}
;}
;if(this.__Jw){for(cG in this.__Jw){var cH=this.__Jw[cG];this.removeListenerById(cH);}
;}
;this.__JC=this.__Jy=this.__Jx=null;}
}});}
)();
...
;this.useCaching(true);this.setMethod((v!==undefined)?v:m);this._transport=this._registerTransportListener(this._createTransport());qx.core.ObjectRegistry.register(this);this.__rE={};this.__uu=this._createResponseParser();}
,members:{setRequestHeader:function(w,x){this.__rE[w]=x;return this;}
,getRequestHeader:function(y){return this.__rE[y];}
,setUrl:function(z){if(qx.lang.Type.isString(z)){this.__uc=z;}
;return this;}
,getUrl:function(){return this.__uc;}
,setMethod:function(A){if(qx.util.Request.isMethod(A)){this.__JK=A;}
;return this;}
,getMethod:function(){return this.__JK;}
,setRequestData:function(B){if(qx.lang.Type.isString(B)||qx.lang.Type.isObject(B)){this.__JL=B;}
;return this;}
,getRequestData:function(){return this.__JL;}
,getResponse:function(){if(this.__tP!==null){return this.__tP;}
...
,setParser:function(C){return this.__uu.setParser(C);}
,setTimeout:function(D){if(qx.lang.Type.isNumber(D)){this.__eP=D;}
;return this;}
,getTimeout:function(){return this.__eP;}
...
,dispose:function(){if(this._transport.dispose()){this.__uu=null;this.__uh=true;return true;}
;return false;}
...
,_setResponse:function(N){this.__tP=N;}
...
;return null;}
,__rE:null,__JL:null,__JK:d,__uc:d,__tP:null,__uu:null,__d:null,__eP:null,__uh:null,addListenerOnce:function(name,S,T){this._transport._emitter.once(name,S,T);return this;}
,_onReadyStateChange:function(){if(qx.core.Environment.get(n)){qx.Bootstrap.debug(c+this._transport.readyState);}
;if(this.isDone()){this.__tU();}
;}
,__tU:function(){if(qx.core.Environment.get(n)){qx.Bootstrap.debug(f+this._transport.status);}
;var V=this._transport.responseText;var U=this._transport.getResponseHeader(k);if(qx.util.Request.isSuccessful(this._transport.status)){if(qx.core.Environment.get(n)){qx.Bootstrap.debug(t+U+a);}
;this._setResponse(this.__uu.parse(V,U));this._transport._emit(o);}
else {try{this._setResponse(this.__uu.parse(V,U));}
catch(e){}
...
;}
,statics:{PARSER:{json:qx.lang.Json.parse,xml:qx.xml.Document.fromString}},members:{__uu:null,parse:function(h,g){var f=this._getParser(g);if(typeof f===a){if(h!==c){return f.call(this,h);}
;}
;return h;}
,setParser:function(i){if(typeof qx.util.ResponseParser.PARSER[i]===a){return this.__uu=qx.util.ResponseParser.PARSER[i];}
;if(qx.core.Environment.get(d)){qx.core.Assert.assertFunction(i);}
;return this.__uu=i;}
,_getParser:function(k){var j=this.__uu,m=c,l=c;if(j){return j;}
...
;this.__rE={};var P=this._transport=this._createTransport();this._setPhase(G);this.__tJ=qx.lang.Function.bind(this._onReadyStateChange,this);this.__tK=qx.lang.Function.bind(this._onLoad,this);this.__tL=qx.lang.Function.bind(this._onLoadEnd,this);this.__tM=qx.lang.Function.bind(this._onAbort,this);this.__tN=qx.lang.Function.bind(this._onTimeout,this);this.__tO=qx.lang.Function.bind(this._onError,this);P.onreadystatechange=this.__tJ;P.onload=this.__tK;P.onloadend=this.__tL;P.onabort=this.__tM;P.ontimeout=this.__tN;P.onerror=this.__tO;}
...
,nullable:true},authentication:{check:k,nullable:true}},members:{__tJ:null,__tK:null,__tL:null,__tM:null,__tN:null,__tO:null,__tP:null,__tQ:null,__tR:null,__rE:null,__tS:null,_transport:null,_createTransport:function(){throw new Error(E);}
...
;this.__tQ=true;this.__tR=s;this._transport.abort();}
,_setRequestHeaders:function(){var X=this._transport,W=this._getAllRequestHeaders();for(var Y in W){X.setRequestHeader(Y,W[Y]);}
;}
,_getAllRequestHeaders:function(){var ba={};qx.lang.Object.mergeWith(ba,this._getConfiguredRequestHeaders());qx.lang.Object.mergeWith(ba,this.__tT());qx.lang.Object.mergeWith(ba,this.__tS);qx.lang.Object.mergeWith(ba,this.__rE);return ba;}
,__tT:function(){var bc=this.getAuthentication(),bb={};if(bc){bc.getAuthHeaders().forEach(function(bd){bb[bd.key]=bd.value;}
);return bb;}
;}
...
,getPhase:function(){return this.__tR;}
...
,getResponse:function(){return this.__tP;}
,_setResponse:function(bl){var bk=bl;if(this.__tP!==bl){this.__tP=bl;this.fireEvent(H,qx.event.type.Data,[this.__tP,bk]);}
;}
,_onReadyStateChange:function(){var bm=this.getReadyState();if(qx.core.Environment.get(f)){this.debug(B+bm);}
;this.fireEvent(D);if(this.__tQ){return;}
;if(bm===3){this._setPhase(p);}
;if(this.isDone()){this.__tU();}
;}
,__tU:function(){if(qx.core.Environment.get(f)){this.debug(A+this.getStatus());}
...
,_setPhase:function(bo){var bp=this.__tR;if(qx.core.Environment.get(m)){qx.core.Assert.assertString(bo);qx.core.Assert.assertMatch(bo,/^(unsent)|(opened)|(sent)|(loading)|(load)|(success)|(abort)|(timeout)|(statusError)$/);}
;this.__tR=bo;this.fireDataEvent(c,bo,bp);}
...
;var A=z[h+qx.lang.String.firstUp(name)]();if(qx.lang.Type.isArray(A)){var D=qx.data&&qx.data.IListData&&qx.Class.hasInterface(A&&A.constructor,qx.data.IListData);for(var i=0;i<A.length;i++ ){var F=D?A.getItem(i):A[i];E+=this.__tV(name,F,C);}
;}
else if(qx.lang.Type.isDate(A)&&y!=null){E+=this.__tV(name,y.format(A),C);}
else {E+=this.__tV(name,A,C);}
;}
;return E.substring(0,E.length-1);}
,__tV:function(name,I,G){if(I&&I.$$type==o){I=I.classname;}
...
(function(){var a="changeModel",b="model.user.avatar_url",c="application/json",d="github.view.Gist",e="model.description",f="west",g="GET",h="/gists/{id}",i="Description",j="https://api.github.com",k="demobrowser.demo.data.Github",l="source",m="/gists",n="change",o="Content",p="center",q="value",r="get",s="#fff",t="Authorization",u="html",v="",w="model.user.login",x="model.files",y="north",z="Gists",A="Accept",B="token ",C="bold",D="model",E="github.rest.Resource",F="description",G="undefined",H="</pre>",I="<pre>";qx.Class.define(k,{extend:qx.application.Standalone,members:{__PW:null,__PX:null,__PY:null,__Qa:null,__LW:null,__Qb:null,main:function(){if(typeof GITHUB==G){return;}
;qx.application.Standalone.prototype.main.call(this);this._setUpResources();this._setUpStores();this._createGui();this._setUpBinding();this.__PW.get();this.__LW.addListener(a,function(K){var J=K.getData();this.__LW.getSelection().push(J.getItem(0));}
,this);this.__LW.getSelection().addListener(n,function(L){var M=this.__LW.getSelection().getItem(0).getId();this.__PX.get({id:M});}
,this);}
,_setUpResources:function(){this.__PW=new github.rest.Resource({"get":{method:g,url:m}});this.__PX=new github.rest.Resource({"get":{method:g,url:h}});}
,_setUpStores:function(){this.__PY=new qx.data.store.Rest(this.__PW,r);this.__Qa=new qx.data.store.Rest(this.__PX,r);}
,_createGui:function(){var N=new qx.ui.container.Composite(new qx.ui.layout.Dock());N.setPadding(10);var O=new qx.ui.basic.Label(z);O.setFont(C);O.setPaddingBottom(10);N.add(O,{edge:y});this.__LW=new qx.ui.list.List();this.__LW.setWidth(200);this.__Qb=new github.view.Gist();N.add(this.__LW,{edge:f});N.add(this.__Qb,{edge:p});this.getRoot().add(N,{edge:0});}
,_setUpBinding:function(){var Q=this.__LW,P=this.__PY,R=this.__Qa;Q.setLabelPath(F);Q.setLabelOptions({converter:function(W,U,T,V){if(W===null||!W.length){return U.getId();}
;return W;}
});P.bind(D,Q,D);var S=this.__Qb;R.bind(e,S.getDescription(),q);R.bind(w,S.getUsername(),q);R.bind(b,S.getGravatar(),l);R.bind(x,S.getContent(),u,{converter:function(Y){var X=qx.Class.getProperties(Y.constructor);var content=Y.get(X[0]).getContent();content=qx.bom.String.escape(content);return I+content+H;}
});}
...
});qx.Class.define(d,{extend:qx.ui.container.Composite,construct:function(){qx.ui.container.Composite.call(this);var bc=new qx.ui.layout.Grid(5,5);bc.setColumnFlex(0,1);bc.setRowFlex(2,1);this.setLayout(bc);this.setPadding(0,10);var bd;var bf=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));bd=new qx.ui.basic.Label(i);bd.setFont(C);this.__BJ=new qx.ui.basic.Label();bf.add(bd);bf.add(this.__BJ);this.add(bf,{row:0,column:0});var be=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));this.__Qc=new qx.ui.basic.Image();this.__Qc.set({width:40,height:40,scale:true,backgroundColor:s});this.__Qd=new qx.ui.basic.Label();be.add(this.__Qc);be.add(this.__Qd);this.add(be,{row:0,column:1});bd=new qx.ui.basic.Label(o);bd.setFont(C);var scroll=new qx.ui.container.Scroll();this.__nt=new qx.ui.embed.Html();this.__nt.setMinHeight(1000);scroll.add(this.__nt);this.add(bd,{row:1,column:0});this.add(scroll,{row:2,column:0,colSpan:2});}
,members:{__BJ:v,__Qc:v,__Qd:v,__nt:v,getDescription:function(){return this.__BJ;}
,getGravatar:function(){return this.__Qc;}
,getUsername:function(){return this.__Qd;}
,getContent:function(){return this.__nt;}
}});}
)();
...
;this._delegate=j;this._marshaler=new qx.data.marshal.Json(j);if(j&&qx.lang.Type.isFunction(j.configureRequest)){this.__Gk();}
;this.__Gj=qx.lang.Function.bind(this.__Gl,this);this.__wW();}
,properties:{resource:{check:c},actionName:{check:f},model:{nullable:true,event:a}},members:{_marshaler:null,_delegate:null,__Gj:null,__Gk:function(){var k=this.getResource(),l=this._delegate;k.configureRequest(l.configureRequest);}
,__wW:function(){var n=this.getResource(),m=this.getActionName();if(n&&m){n.addListener(this.getActionName()+d,this.__Gj);}
;}
,__Gl:function(e){var r=e.getData(),q=this._marshaler,s,o=this.getModel(),p=this._delegate;if(r){if(p&&p.manipulateData){r=p.manipulateData(r);}
...
},members:{__tr:null,__ts:function(x){return Object.keys(x).sort().join(a);}
,toClass:function(y,z){this.__tt(y,z,null,0);}
,__tt:function(L,G,A,I){if(!qx.lang.Type.isObject(L)||!!L.$$isString||L instanceof qx.core.Object){if(L instanceof Array||qx.Bootstrap.getClass(L)==h){for(var i=0;i<L.length;i++ ){this.__tt(L[i],G,null,I+1);}
;}
;return;}
;var C=this.__ts(L);if(this.__tx(C,A,I)){return;}
;for(var K in L){this.__tt(L[K],G,K,I+1);}
;if(qx.Class.isDefined(r+C)){return;}
;if(this.__tr&&this.__tr.getModelClass&&this.__tr.getModelClass(C,L)!=null){return;}
;var M={};var D={__tu:this.__tu};for(var K in L){if(this.__tr&&this.__tr.getPropertyMapping){K=this.__tr.getPropertyMapping(K,C);}
...
;if(this.__tr&&this.__tr.getValidationRule){var F=this.__tr.getValidationRule(C,K);if(F){M[K].validate=k+K;D[k+K]=F;}
;}
;}
;if(this.__tr&&this.__tr.getModelSuperClass){var J=this.__tr.getModelSuperClass(C)||qx.core.Object;}
else {var J=qx.core.Object;}
;var E=[];if(this.__tr&&this.__tr.getModelMixins){var H=this.__tr.getModelMixins(C);if(!qx.lang.Type.isArray(H)){if(H!=null){E=[H];}
;}
else {E=H;}
;}
;if(G){E.push(qx.data.marshal.MEventBubbling);}
;var B={extend:J,include:E,properties:M,members:D,destruct:this.__tv};qx.Class.define(r+C,B);}
,__tv:function(){var N=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var O in N){this.__tu(this.get(N[O].name));}
;}
,__tu:function(P){if(!(P instanceof qx.core.Object)){return;}
;if(P.isDisposed()){return;}
;P.dispose();}
,__tw:function(S,R){var T;if(this.__tr&&this.__tr.getModelClass){T=this.__tr.getModelClass(S,R);}
;if(T!=null){return (new T());}
else {var Q=r+S;var U=qx.Class.getByName(Q);if(!U){throw new Error(b+Q+l);}
;return (new U());}
;}
,__tx:function(V,Y,W){var X=this.__tr;return X&&X.ignore&&X.ignore(V,Y,W);}
,toModel:function(ba){return this.__ty(ba,null,0);}
,__ty:function(bk,bc,bh){var bg=qx.lang.Type.isObject(bk);var bb=bk instanceof Array||qx.Bootstrap.getClass(bk)==h;if((!bg&&!bb)||!!bk.$$isString||bk instanceof qx.core.Object){return bk;}
else if(this.__tx(this.__ts(bk),bc,bh)){return bk;}
else if(bb){var bl=new qx.data.Array();bl.setAutoDisposeItems(true);for(var i=0;i<bk.length;i++ ){bl.push(this.__ty(bk[i],null,bh+1));}
;return bl;}
else if(bg){var bd=this.__ts(bk);var bj=this.__tw(bd,bk);for(var bi in bk){var be=bi;if(this.__tr&&this.__tr.getPropertyMapping){be=this.__tr.getPropertyMapping(bi,bd);}
;var bm=be.replace(/-|\.|\s+/g,d);if((qx.core.Environment.get(s))&&qx.core.Environment.get(e)){if(bm!=be){this.warn(c+bi+n+be+m);}
;}
;be=bm;var bf=o+qx.lang.String.firstUp(be);if(bj[bf]){bj[bf](this.__ty(bk[bi],bi,bh+1));}
;}
;return bj;}
;throw new Error(j);}
...
,properties:{selection:{check:d,event:n,apply:h,nullable:false,deferredInit:true},selectionMode:{check:[l,f,p,a],init:l,apply:o},dragSelection:{check:b,init:false,apply:k},quickSelection:{check:b,init:false,apply:m}},events:{"changeSelection":q},members:{_manager:null,__zB:false,__zC:false,__yX:null,_initSelectionManager:function(){var self=this;var r={isItemSelectable:function(s){return self._provider.isSelectable(s);}
...
,_onChangeSelection:function(e){if(this.__zC==true){return;}
;this.__zB=true;var F=this.getSelection();var H=[];for(var i=0;i<F.getLength();i++ ){var G=F.getItem(i);var I=this._getSelectables();var E=-1;if(I!=null){E=I.indexOf(G);}
...
;this.__zB=false;}
,_onManagerChangeSelection:function(e){if(this.__zB==true){return;}
;this.__zC=true;this.__zd();this.__zC=false;}
,__zd:function(){if(this.__zE()){return;}
;var L=this._manager.getSelection();var M=[];for(var i=0;i<L.length;i++ ){var N=this._getDataFromRow(L[i]);if(N!=null){M.push(N);}
;}
;this.__zD(M);}
,__zD:function(Q){var P=this.getSelection();if(Q.length>0){var O=[0,P.getLength()];O=O.concat(Q);var R=P.splice.apply(P,O);R.dispose();}
else {P.removeAll();}
;}
,__zE:function(){var T=this.getSelection();var V=this._manager.getSelection();if(T.getLength()!==V.length){return false;}
...
(function(){var c="changeModel",d="resize",f="_applyLabelOptions",g="_applyLabelPath",h="qx.data.Array",j="_applyGroupLabelOptions",m="changeLength",n="Integer",o="_applyModel",p="changeGroups",q="Boolean",r="GroupingTypeError: You can't mix 'Objects' and 'Strings' as",s="sorter",t="row-layer",u="group",v="_applyIconPath",w="_applyDelegate",x="changeDelegate",y=" group identifier!",z="???",A="_applyRowHeight",B="qx.data.IListData",C="filter",D="virtual-list",E="String",F="_applyGroupLabelPath",G="_applyIconOptions",H="qx.ui.list.List";qx.Class.define(H,{extend:qx.ui.virtual.core.Scroller,include:[qx.ui.virtual.selection.MModel],implement:qx.data.controller.ISelection,construct:function(I){qx.ui.virtual.core.Scroller.call(this,0,1,20,100);this._init();this.__zF=new qx.data.Array();this.initGroups(this.__zF);if(I!=null){this.initModel(I);}
;this.initItemHeight();}
...
,_init:function(){this._provider=new qx.ui.list.provider.WidgetProvider(this);this.__zG=[];this.__zH=[];this.__zI={};this.__zJ=false;this.__zK=false;this.__zL=false;this.getPane().addListener(d,this._onResize,this);this._initBackground();this._initLayer();}
...
;}
,_getLookupTable:function(){return this.__zG;}
,_lookup:function(P){return this.__zG[P];}
,_lookupGroup:function(Q){return this.__zH.indexOf(Q);}
,_reverseLookup:function(R){if(R<0){return -1;}
;return this.__zG.indexOf(R);}
...
,_applyDelegate:function(bl,bk){this._provider.setDelegate(bl);this.__zN();}
,_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);}
,_onModelChange:function(e){this.__zN();this._applyDefaultSelection();}
,__zM:function(){this.getPane().getRowConfig().setItemCount(this.__zG.length);this.getPane().fullUpdate();}
,__zN:function(){this.__zG=[];this.__zH=[];this.__zI={};if(this.isAutoGrouping()){this.getGroups().removeAll();}
;var bm=this.getModel();if(bm!=null){this._runDelegateFilter(bm);this._runDelegateSorter(bm);this._runDelegateGroup(bm);}
;this._updateSelection();this.__zM();}
,_runDelegateFilter:function(bo){var bn=qx.util.Delegate.getMethod(this.getDelegate(),C);for(var i=0,l=bo.length;i<l; ++i){if(bn==null||bn(bo.getItem(i))){this.__zG.push(i);}
;}
;}
,_runDelegateSorter:function(bp){if(this.__zG.length==0){return;}
;var bq=qx.util.Delegate.getMethod(this.getDelegate(),s);if(bq!=null){this.__zG.sort(function(a,b){return bq(bp.getItem(a),bp.getItem(b));}
);}
;}
,_runDelegateGroup:function(bu){var bv=qx.util.Delegate.getMethod(this.getDelegate(),u);if(bv!=null){for(var i=0,l=this.__zG.length;i<l; ++i){var br=this.__zG[i];var bt=this.getModel().getItem(br);var bs=bv(bt);this.__zO(bs,br);}
;this.__zG=this.__zP();}
;}
,__zO:function(bx,bw){if(bx==null){this.__zL=true;bx=z;}
;var name=this.__zQ(bx);if(this.__zI[name]==null){this.__zI[name]=[];if(this.isAutoGrouping()){this.getGroups().push(bx);}
;}
;this.__zI[name].push(bw);}
,__zP:function(){this.__zR();var bB=[];var bD=0;var bz=this.getGroups();for(var i=0;i<bz.getLength();i++ ){var by=bz.getItem(i);bB.push(-1);this.__zH.push(bD);bD++ ;var bC=this.__zQ(by);var bA=this.__zI[bC];if(bA!=null){for(var k=0;k<bA.length;k++ ){bB.push(bA[k]);bD++ ;}
;}
;}
;return bB;}
,__zQ:function(bF){var name=null;if(!qx.lang.Type.isString(bF)){var bE=this.getGroups().indexOf(bF);this.__zK=true;name=u;if(bE==-1){name+=this.getGroups().getLength();}
else {name+=bE;}
;}
else {this.__zJ=true;var name=bF;}
;return name;}
,__zR:function(){if(this.__zK&&this.__zL||this.__zK&&this.__zJ){throw new Error(r+y);}
;}
...
;this._background.dispose();this._provider.dispose();this._layer.dispose();this._background=this._provider=this._layer=this.__zG=this.__zH=this.__zI=null;if(this.__zF){this.__zF.dispose();}
;}
});}
)();
...
;this.__zU={};this.__zV={};}
,properties:{colorEven:{nullable:true,check:c,apply:b,themeable:true},colorOdd:{nullable:true,check:c,apply:a,themeable:true}},members:{__zW:null,__zX:null,__zU:null,__zV:null,setColor:function(h,g){if(g){this.__zU[h]=qx.theme.manager.Color.getInstance().resolve(g);}
else {delete (this.__zU[h]);}
;}
,clearCustomColors:function(){this.__zU={};this.updateLayerData();}
,getColor:function(i){var j=this.__zU[i];if(j){return j;}
else {return i%2==0?this.__zW:this.__zX;}
;}
,_applyColorEven:function(l,k){if(l){this.__zW=qx.theme.manager.Color.getInstance().resolve(l);}
else {this.__zW=null;}
;this.updateLayerData();}
,_applyColorOdd:function(n,m){if(n){this.__zX=qx.theme.manager.Color.getInstance().resolve(n);}
else {this.__zX=null;}
;this.updateLayerData();}
,setBackground:function(o,p){if(p){this.__zV[o]=qx.theme.manager.Decoration.getInstance().resolve(p);}
else {delete (this.__zV[o]);}
;this.updateLayerData();}
,getBackground:function(q){return this.__zV[q];}
},destruct:function(){this.__zU=this.__zV=null;}
});}
)();
...
;}
,setColor:function(J,I){qx.ui.virtual.layer.AbstractBackground.prototype.setColor.call(this,J,I);if(this.__zY(J)){this.updateLayerData();}
;}
,setBackground:function(K,L){qx.ui.virtual.layer.AbstractBackground.prototype.setBackground.call(this,K,L);if(this.__zY(K)){this.updateLayerData();}
;}
,__zY:function(M){var O=this.getFirstRow();var N=O+this.getRowSizes().length-1;return M>=O&&M<=N;}
}});}
)();
...
(function(){var a="cell.type",b="model[",c="groups[",d="model",f="",g="changeDelegate",h="label",i="qx.ui.list.core.MWidgetController",j="icon",k="]",l="value",m="BindingIds",n=".",o="group",p="String";qx.Mixin.define(i,{construct:function(){this.__Aa=[];}
...
;}
,bindProperty:function(u,x,v,t,s){var y=t.getUserData(a);var w=this.__Ab(s,u,y);if(v){v.ignoreConverter=d;}
;var z=this._list.bind(w,t,x,v);this.__Ac(t,z);}
,bindPropertyReverse:function(D,B,E,F,A){var G=F.getUserData(a);var C=this.__Ab(A,D,G);var H=F.bind(B,this._list,C,E);this.__Ac(F,H);}
,removeBindings:function(){while(this.__Aa.length>0){var I=this.__Aa.pop();this._removeBindingsFrom(I);}
;}
...
;}
,_removeBindingsFrom:function(T){var U=this.__Ad(T);while(U.length>0){var V=U.pop();try{this._list.removeBinding(V);}
catch(e){T.removeBinding(V);}
;}
;if(qx.lang.Array.contains(this.__Aa,T)){qx.lang.Array.remove(this.__Aa,T);}
;}
,__Ab:function(X,ba,Y){var W=b+X+k;if(Y==o){W=c+X+k;}
;if(ba!=null&&ba!=f){W+=n+ba;}
;return W;}
,__Ac:function(bc,bd){var bb=this.__Ad(bc);if(!qx.lang.Array.contains(bb,bd)){bb.push(bd);}
;if(!qx.lang.Array.contains(this.__Aa,bc)){this.__Aa.push(bc);}
;}
,__Ad:function(be){var bf=be.getUserData(m);if(bf==null){bf=[];be.setUserData(m,bf);}
;return bf;}
},destruct:function(){this.__Aa=null;}
});}
)();
...
,styleSelectabled:function(v){var u=this.__Ae(v);this._styleSelectabled(u);}
,styleUnselectabled:function(x){var w=this.__Ae(x);this._styleUnselectabled(w);}
...
;}
,_styleSelectabled:function(A){this.__Af(A,{selected:1});}
,_styleUnselectabled:function(B){this.__Af(B,{});}
...
,__Ae:function(G){return this._list._layer.getRenderedCellWidget(G,0);}
,__Af:function(I,H){if(I==null){return;}
...
;this._cellProvider=h;this.__Ag=[];}
,properties:{anonymous:{refine:true,init:false}},events:{updated:c},members:{__Ag:null,getRenderedCellWidget:function(q,n){var j=this.getColumnSizes().length;var p=this.getRowSizes().length;var o=this.getFirstRow();var m=this.getFirstColumn();if(q<o||q>=o+p||n<m||n>=m+j){return null;}
;var l=(n-m)+(q-o)*j;var k=this._getChildren()[l];if(k.getUserData(g)){return null;}
else {return k;}
;}
,_getSpacer:function(){var r=this.__Ag.pop();if(!r){r=new qx.ui.core.Spacer();r.setUserData(g,1);}
;return r;}
...
;}
;}
,_fullUpdate:function(A,v,D,z){var B=this._cellProvider;var G=this._getChildren();for(var i=0;i<G.length;i++ ){var E=G[i];if(E.getUserData(g)){this.__Ag.push(E);}
...
;}
;}
;var J=this._cellProvider;var N=this._getChildren();for(var i=0;i<N.length;i++ ){if(!W[i]){var T=N[i];if(T.getUserData(g)){this.__Ag.push(T);}
...
(function(){var a="created",b="qx.ui.virtual.cell.AbstractWidget",c="cell.states",d="abstract method call",e="qx.event.type.Data";qx.Class.define(b,{extend:qx.core.Object,implement:[qx.ui.virtual.cell.IWidgetCell],construct:function(){qx.core.Object.call(this);this.__cL=[];}
,events:{"created":e},members:{__cL:null,_createWidget:function(){throw new Error(d);}
...
;}
;j.setUserData(c,h);}
,getCellWidget:function(o,m){var n=this.__Ah();this.updateStates(n,m);this.updateData(n,o);return n;}
,pool:function(p){this.__cL.push(p);}
,_cleanupPool:function(){var q=this.__cL.pop();while(q){q.destroy();q=this.__cL.pop();}
;}
,__Ah:function(){var r=this.__cL.pop();if(r==null){r=this._createWidget();this.fireDataEvent(a,r);}
;return r;}
},destruct:function(){this._cleanupPool();this.__cL=null;}
});}
)();
