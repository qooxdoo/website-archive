...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Element_1.c40499aa088e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="after2",c="text",d="hello",e="el3b",f="height",g="sans-serif",h="Move, remove & add",j="html",k="<b>hello</b>",m="color",n="Sort completely",o="18px",p="Remove and Add and Remove and Add",q="blue",r="before2",s="Rotation",t="el2b",u="Positioned Insert #2",v="demobrowser.demo.test.Element_1",w="el2a",x="width",y="100px",z="after1",A="Move",B="Remove",C="test",D="Basics",E="before1",F="bold",G="doc",H="Positioned Insert #1",I="Switch",J="Re-add",K="el3a",L="el1",M="id";qx.Class.define(v,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var N=new qx.html.Root(document.getElementById(C));N.setAttribute(M,G);this.info(D);var W=new qx.html.Element;var T=new qx.html.Element;var V=new qx.html.Element;var O=new qx.html.Element;var S=new qx.html.Element;W.setAttribute(M,L);T.setAttribute(M,w);V.setAttribute(M,t);O.setAttribute(M,K);S.setAttribute(M,e);W.add(T,V);T.add(O,S);N.add(W);O.setAttribute(j,k);S.setAttribute(c,d);S.setStyles({fontWeight:F,fontSize:o,fontFamily:g});W.setStyle(m,q);W.setStyle(x,y).setStyle(f,y);qx.html.Element.flush();this.info(H);var R=new qx.html.Element;R.setAttribute(M,E);R.insertBefore(W);var X=new qx.html.Element;X.setAttribute(M,z);X.insertAfter(W);qx.html.Element.flush();this.info(u);var U=new qx.html.Element;U.setAttribute(M,r);N.addAt(U,0);var Y=new qx.html.Element;Y.setAttribute(M,b);N.addAt(Y,10);qx.html.Element.flush();this.info(A);U.moveAfter(R);qx.html.Element.flush();Y.moveBefore(X);qx.html.Element.flush();this.info(I);var P=N.indexOf(U);var Q=N.indexOf(Y);U.moveTo(Q);Y.moveTo(P);qx.html.Element.flush();this.info(B);N.remove(U);N.remove(Y);qx.html.Element.flush();this.info(J);N.add(U,Y);qx.html.Element.flush();this.info(p);N.remove(U,Y);N.add(U,Y);N.remove(U,Y);N.add(U,Y);qx.html.Element.flush();this.info(h);N.remove(U);R.moveAfter(X);U.insertBefore(R);qx.html.Element.flush();this.info(n);var a=N.getChildren();for(var i=1,l=a.length;i<l;i++ ){a[i].moveTo(0);}
;qx.html.Element.flush();this.info(s);R.moveAfter(U);qx.html.Element.flush();}
}});}
)();
