...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Touch.56ac52e8ed74.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;bd.add(this.__SD(10,10,k,W));bd.add(this.__SD(110,10,Y,b));this.__SB=this.__SE(10,110);bd.add(this.__SB);this.__SC=this.__SE(110,110);bd.add(this.__SC);bd.add(this.__SF(210,10));}
,__SD:function(x,y,bh,bi){var bf={"position":X,"left":x+d,"top":y+d,"width":T,"height":T,"backgroundColor":bh,"fontFamily":J,"fontSize":s};var bg=new qx.html.Element(l,bf,{"id":bi});bg.setAttribute(D,bb+bi+P);bg.addListener(F,this.__SG,this);bg.addListener(a,this.__SG,this);bg.addListener(A,this.__SG,this);bg.addListener(S,this.__SG,this);return bg;}
,__SE:function(x,y){var bj={"position":X,"left":x+d,"top":y+d,"width":T,"fontFamily":J,"fontSize":I};return new qx.html.Element(l,bj);}
,__SF:function(x,y){var bk={"position":X,"left":x+d,"top":y+d,"width":n,"height":n,"backgroundColor":O,"fontFamily":J,"fontSize":s};var bl=new qx.html.Element(l,bk);bl.setAttribute(D,h);bl.addListener(L,function(){window.setTimeout(function(){alert(w);}
,0);}
,this);return bl;}
,__SG:function(e){var bm=[];bm.push(M+e.getType());bm.push(f);bm.push(j+e.isMultiTouch());bm.push(t+e.getAllTouches().length);bm.push(c+e.getTargetTouches().length);bm.push(g+e.getChangedTargetTouches().length);bm.push(f);bm.push(z+e.getDocumentLeft()+r+e.getDocumentTop());bm.push(m+e.getScreenLeft()+r+e.getScreenTop());bm.push(B+e.getViewportLeft()+r+e.getViewportTop());bm.push(q+e.getIdentifier());var bo=e.getTarget();var bn=false;var bq=e.getTargetTouches().length>0?e.getTargetTouches():e.getChangedTargetTouches();for(var i=0;i<bq.length;i++ ){bn=bq[0].target==bo;if(!bn){break;}
;}
