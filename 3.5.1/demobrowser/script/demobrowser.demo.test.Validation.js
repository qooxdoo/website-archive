...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Validation.5023da5414fd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="demobrowser.demo.test.Validation",b="<b>Setting the age ",c="property to 27</b><br>",d="- name: Name is validated to a minimum length of two.<br/>",e="<br>old name: ",f=")",g="age:",h="new name: ",i="new gender: ",j="fmale",k="<br><b>Setting the name to 'Hans'</b><br>",l="property to 'female'</b><br>",m="<br><b>Setting the gender ",n="must be longer than one characters. (",o="<b>Testing the gender ",p="<br>old gender: ",q="male",r="<br><b>Setting the age ",s="__SI",t="- age: Age is validated to be a number.<br/>",u="<b>Initial property values</b><br>",v="female",w="",x="<b>Setting the name to ''</b><br>",y="demobrowser.demo.test.Person",z="property to 'true'</b><br>",A="- gender: Gender is validated to be either male or female.",B="property to 'fmale'</b><br>",C="<font color='red'>GENDER NOT SET</font>",D="gender:",E="<br>old age: ",F="Hans",G="The demo creates an internal class containing three properties:<br/>",H="new age: ",I="Validation Error: Name ",J="<font color='red'>AGE NOT SET</font>",K="My name",L="<font color='red'>NAME NOT SET</font>",M="<br>",N="white",O="name:",P="<b>Description</b><br/>";qx.Class.define(a,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var Q=new demobrowser.demo.test.Person();var R=new qx.ui.embed.Html();R.setBackgroundColor(N);R.set({padding:10,width:250,height:360});this.getRoot().add(R,{left:10,top:10});R.setHtml(u);R.setHtml(R.getHtml()+O+Q.getName()+M);R.setHtml(R.getHtml()+g+Q.getAge()+M);R.setHtml(R.getHtml()+D+Q.getGender()+M);R.setHtml(R.getHtml()+k);Q.setName(F);R.setHtml(R.getHtml()+h+Q.getName()+M);R.setHtml(R.getHtml()+x);try{Q.setName(w);R.setHtml(R.getHtml()+h+Q.getName()+M);}
catch(T){R.setHtml(R.getHtml()+L+e+Q.getName()+M);}
;R.setHtml(R.getHtml()+r+c);Q.setAge(27);R.setHtml(R.getHtml()+H+Q.getAge()+M);R.setHtml(R.getHtml()+b+z);try{Q.setAge(true);R.setHtml(R.getHtml()+H+Q.getAge()+M);}
catch(U){R.setHtml(R.getHtml()+J+E+Q.getAge()+M);}
;R.setHtml(R.getHtml()+m+l);Q.setGender(v);R.setHtml(R.getHtml()+i+Q.getGender()+M);R.setHtml(R.getHtml()+o+B);try{Q.setAge(j);R.setHtml(R.getHtml()+i+Q.getGender()+M);}
catch(V){R.setHtml(R.getHtml()+C+p+Q.getGender()+M);}
;var S=new qx.ui.basic.Label();S.setRich(true);S.setWidth(250);S.setValue(P+G+d+t+A);this.getRoot().add(S,{left:280,top:10});}
}});qx.Class.define(y,{extend:qx.core.Object,properties:{name:{init:K,validate:s},age:{init:18,validate:qx.util.Validate.number()},gender:{init:q,validate:qx.util.Validate.inArray([q,v])}},members:{__SI:function(W){if(W.length<2){throw new qx.core.ValidationError(I+n+W+f);}
;}
}});}
)();
