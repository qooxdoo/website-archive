...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.ModelDebugging","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.data.ModelDebugging-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"1be407541021"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var o="loaded",n="auto",m="debugModel function in qx.dev.Debug.",l="?callback=demobrowser.demo.data.store.Twitter.saveResult",k="objects with properties. To see the properties, you can use the ",j="demobrowser.demo.data.ModelDebugging",i="failed",h="wittemann",g="Every model created by the data stores consists only of qooxdoo ",f="demobrowser.demo.data.store.Twitter",c="<b>Debugging models</b><br/>",e="white",d="main",b="http://twitter.com/statuses/user_timeline/",a=".json";
...
var r=new demobrowser.demo.data.store.Twitter(h);
var p=new qx.ui.embed.Html();
p.setBackgroundColor(e);
p.setDecorator(d);
p.setWidth(500);
p.setHeight(200);
p.setOverflow(n,n);
this.getRoot().add(p,{left:10,top:80});
r.addListener(o,function(){var s=r.getModel();
this.debug(qx.dev.Debug.debugProperties(s));
p.setHtml(qx.dev.Debug.debugProperties(s,10,true));
},this);
...
qx.Class.define(f,{extend:qx.data.store.Json,statics:{saveResult:function(t){this.__Tq=t;
}},construct:function(u){var v=b+u+a;
qx.data.store.Json.call(this,v);
},members:{_createRequest:function(w){var x=new qx.io.ScriptLoader();
w+=l;
x.load(w,function(y){this.__Tr();
},this);
},__Tr:function(){var z=demobrowser.demo.data.store.Twitter.__Tq;

if(z==undefined){this.setState(i);
return;
}this._marshaler.toClass(z);
this.setModel(this._marshaler.toModel(z));
this.fireDataEvent(o,this.getModel());
}}});
})();
...
},events:{"created":j,"configured":j,"sending":j,"receiving":j,"completed":R,"aborted":j,"failed":R,"timeout":R},statics:{__rT:0,methodAllowsRequestBody:function(V){return (V==Q)||(V==K);
...
},init:true,apply:H},crossDomain:{check:k,init:false},fileUpload:{check:k,init:false},transport:{check:m,nullable:true},useBasicHttpAuth:{check:k,init:false},parseJson:{check:k,init:true}},members:{__rP:null,__rQ:null,__rR:null,__rS:null,__rT:null,send:function(){qx.io.remote.RequestQueue.getInstance().add(this);
...
},__rU:qx.event.GlobalError.observeMethod(function(e){var W=e.clone();
W.setTarget(this);
...
},setRequestHeader:function(bh,bi){this.__rP[bh]=bi;
},removeRequestHeader:function(bj){delete this.__rP[bj];
},getRequestHeader:function(bk){return this.__rP[bk]||null;
},getRequestHeaders:function(){return this.__rP;
},setParameter:function(bl,bm,bn){if(bn){this.__rR[bl]=bm;
}else{this.__rQ[bl]=bm;
}},removeParameter:function(bo,bp){if(bp){delete this.__rR[bo];
}else{delete this.__rQ[bo];
}},getParameter:function(bq,br){if(br){return this.__rR[bq]||null;
}else{return this.__rQ[bq]||null;
}},getParameters:function(bs){return (bs?this.__rR:this.__rQ);
},setFormField:function(bt,bu){this.__rS[bt]=bu;
},removeFormField:function(bv){delete this.__rS[bv];
},getFormField:function(bw){return this.__rS[bw]||null;
...
(function(){var l="qx.ioRemoteDebug",k="Integer",j="qx.debug",h="aborted",g="_onaborted",f="_on",d="Boolean",c="singleton",b="interval",a="receiving",x="Request-Queue Progress: ",w="__rW",v="queued",u="io.maxrequests",t="_applyEnabled",s="sending",r="completed",q="failed",p="qx.io.remote.RequestQueue",o="__rY",m="timeout",n="/";
...
this.__rY.addListener(b,this._oninterval,this);
},properties:{enabled:{init:true,check:d,apply:t},maxTotalRequests:{check:k,nullable:true},maxConcurrentRequests:{check:k,init:qx.core.Environment.get(u)},defaultTimeout:{check:k,init:5000}},members:{__rV:null,__rW:null,__rX:null,__rY:null,getRequestQueue:function(){return this.__rV;
},getActiveQueue:function(){return this.__rW;
},_debug:function(){if(qx.core.Environment.get(j)){if(qx.core.Environment.get(l)){var y=this.__rW.length+n+(this.__rV.length+this.__rW.length);
...
}},_remove:function(B){qx.lang.Array.remove(this.__rW,B);
B.dispose();
this._check();
},__sa:0,_onsending:function(e){if(qx.core.Environment.get(j)){if(qx.core.Environment.get(l)){this.__sa++;
e.getTarget()._counted=true;
this.debug("ActiveCount: "+this.__sa);
}}e.getTarget().getRequest()._onsending(e);
},_onreceiving:function(e){e.getTarget().getRequest()._onreceiving(e);
},_oncompleted:function(e){if(qx.core.Environment.get(j)){if(qx.core.Environment.get(l)){if(e.getTarget()._counted){this.__sa--;
this.debug("ActiveCount: "+this.__sa);
...
D[g](event);
}}catch(F){}}},_oninterval:function(e){var M=this.__rW;

if(M.length==0){this.__rY.stop();
return;
...
}this.__rY.setEnabled(N);
},add:function(P){P.setState(v);

if(P.isAsynchronous()){this.__rV.push(P);
}else{this.__rV.unshift(P);
}this._check();
...
if(R){R.abort();
}else if(qx.lang.Array.contains(this.__rV,Q)){qx.lang.Array.remove(this.__rV,Q);
...
},__sb:function(){var bT=this.getRequest();
...
}this.__sc=qx.bom.Iframe.create({id:L,name:L,src:N});
qx.bom.element.Style.set(this.__sc,s,A);
this.__sd=qx.bom.Element.create(n,{id:M,name:M,target:L});
qx.bom.element.Style.set(this.__sd,s,A);
qx.dom.Element.insertEnd(this.__sd,qx.dom.Node.getBodyElement(document));
this.__se=qx.bom.Element.create(J,{id:I,name:I});
qx.dom.Element.insertEnd(this.__se,this.__sd);
qx.dom.Element.insertEnd(this.__sc,qx.dom.Node.getBodyElement(document));
qx.event.Registration.addListener(this.__sc,H,this._onload,this);
...
},_numericMap:{"uninitialized":1,"loading":2,"loaded":2,"interactive":3,"complete":4}},members:{__se:null,__sg:0,__sd:null,__sc:null,__sf:null,send:function(){var P=this.getMethod();
var R=this.getUrl();
...
return;
}while(this.__sg<W){this.setState(qx.io.remote.Exchange._nativeMap[++this.__sg]);
...
},destruct:function(){if(this.__sc){qx.event.Registration.removeListener(this.__sc,H,this._onload,this);
qx.bom.Event.removeNativeListener(this.__sc,E,this.__sf);
if((qx.core.Environment.get(g)==t)){this.__sc.src=qx.util.ResourceManager.getInstance().toUri(y);
...
var v=++qx.io.remote.transport.Script.__si;

if(v>=2000000000){qx.io.remote.transport.Script.__si=v=1;
}this.__sj=null;
this.__si=v;
},statics:{__si:0,_instanceRegistry:{},ScriptTransport_PREFIX:e,ScriptTransport_ID_PARAM:h,ScriptTransport_DATA_PARAM:g,handles:{synchronous:false,asynchronous:true,crossDomain:true,fileUpload:false,programaticFormFields:false,responseTypes:[o,m,q]},isSupported:function(){return true;
...
}})},members:{__sk:0,__sj:null,__si:null,send:function(){var A=this.getUrl();
A+=(A.indexOf(l)>=0?t:l)+qx.io.remote.transport.Script.ScriptTransport_ID_PARAM+s+this.__si;
...
this.__sj=document.createElement(b);
this.__sj.charset=j;
this.__sj.src=A;
...
return;
}while(this.__sk<E){this.setState(qx.io.remote.Exchange._nativeMap[++this.__sk]);
...
}},properties:{parseJson:{check:r,init:true}},members:{__sl:false,__sm:0,__sn:null,getRequest:function(){if(this.__sn===null){this.__sn=qx.io.remote.transport.XmlHttp.createRequestObject();
...
if(bn==4){if(!qx.io.remote.Exchange.wasSuccessful(this.getStatusCode(),bn,this.__sl)){if(this.getState()===N){this.setState(c);
}this.failed();
return;
}}if(bn==3&&this.__sm==1){this.setState(qx.io.remote.Exchange._nativeMap[++this.__sm]);
}while(this.__sm<bn){this.setState(qx.io.remote.Exchange._nativeMap[++this.__sm]);
...
if(qx.io.remote.Exchange.wasSuccessful(bI,bJ,this.__sl)){try{bK=this.getRequest().responseXML;
