...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.progressive.ProgressiveTable_Conditional","qx.theme":"qx.theme.Classic"};
...
for(var i=0;i<e;i++){this.__Qu[i]=new qx.ui.core.ColumnData();
}},members:{__Qu:null,getData:function(){return this.__Qu;
...
}}},setWidth:function(j,k){if(j>this.__Qu.length-1||j<0){throw new Error("Column number out of range");
}this.__Qu[j].setColumnWidth(k);
},setMinWidth:function(l,m){if(l>this.__Qu.length-1||l<0){throw new Error("Column number out of range");
}this.__Qu[l].setMinWidth(m);
},setMaxWidth:function(n,o){if(n>this.__Qu.length-1||n<0){throw new Error("Column number out of range");
}this.__Qu[n].setMaxWidth(o);
}}});
})();
...
},members:{__NN:null,join:function(c){this.__NN=c;
...
(function(){var n="progressive-progressbar-indicator-undone",m="progress",l="%",k="progressive-progressbar-indicator-done",j="__Rp",i="__Ro",h="progressive-progressbar-percent-text",g="renderStart",f="progressive-progressbar-background",d="0%",a="qx.ui.progressive.headfoot.Progress",c="progressive-progressbar-percent-background",b="renderEnd";
...
this.__Rn.background=q.resolve(f);
this.__Rn.indicatorDone=q.resolve(k);
this.__Rn.indicatorUndone=q.resolve(n);
this.__Rn.percentBackground=q.resolve(c);
this.__Rn.percentText=q.resolve(h);
this.set({backgroundColor:this.__Rn.background});
this.__Ro=new qx.ui.core.Widget();
this.__Ro.set({width:0,backgroundColor:this.__Rn.indicatorDone});
this.add(this.__Ro);
var r=new qx.ui.core.Widget();
r.set({backgroundColor:this.__Rn.indicatorUndone});
this.add(r,{flex:1});
this.__Rp=new qx.ui.basic.Atom(d);
this.__Rp.set({width:100,backgroundColor:this.__Rn.percentBackground,textColor:this.__Rn.percentText});
this.add(this.__Rp);
this.exclude();
},members:{__Rq:null,__Rn:null,__Ro:null,__Rp:null,join:function(s){qx.ui.progressive.headfoot.Abstract.prototype.join.call(this,s);
s.addListener(g,function(e){this.__Rq=e.getData().initial;
this.show();
},this);
s.addListener(m,function(e){var v=1.0-(e.getData().remaining/this.__Rq);
var w=this.getBounds();

if(w){var t=Math.floor((w.width-this.__Rp.getBounds().width)*v);
var u=Math.floor(v*100)+l;

if(!isNaN(t)){this.__Ro.setMinWidth(t);
this.__Rp.setLabel(u);
}}},this);
s.addListener(b,function(e){this.exclude();
},this);
}},destruct:function(){this.__Rn=null;
this._disposeObjects(i,j);
}});
})();
...
if(!e){this.__Ps=new qx.ui.core.Widget();
this.__Pt=this.__Ps;
}else{this.__Ps=null;
this.__Pt=e;
}this.__Pt.getContentElement().setStyle(b,a);
},members:{__Ps:null,__Pt:null,applyStructure:function(f){throw new Error("applyStructure() is abstract");
},getPane:function(){return this.__Pt;
}},destruct:function(){if(this.__Ps){this.__Ps.dispose();
}this.__Ps=this.__Pt=null;
}});
})();
...
if(!b){this.__PC=new qx.ui.progressive.headfoot.Null();
this.__PD=this.__PC;
}else{this.__PC=null;
this.__PD=b;
}if(!c){this.__PE=new qx.ui.progressive.headfoot.Null();
this.__PF=this.__PE;
}else{this.__PE=null;
this.__PF=c;
}},members:{__PD:null,__PF:null,__PC:null,__PE:null,applyStructure:function(e){this.__PD.join(e);
this.__PF.join(e);
e.add(this.__PD);
e.add(this.getPane(),{flex:1});
e.add(this.__PF);
},getHeader:function(){return this.__PD;
},getFooter:function(){return this.__PF;
}},destruct:function(){if(this.__PC){this.__PC.dispose();
this.__PC=null;
}
if(this.__PE){this.__PE.dispose();
this.__PE=null;
}this.__PD=this.__PF=null;
}});
})();
...
this.__PJ=false;
this.__PK=0;
},events:{"renderStart":k,"renderEnd":k,"progress":k,"progressDetail":k},properties:{dataModel:{check:c,apply:f},batchSize:{check:g,init:20},flushWidgetQueueAfterBatch:{check:d,init:false},interElementTimeout:{check:g,init:0}},members:{__PG:null,__PJ:null,__PL:null,__PK:null,__PI:null,__PH:null,getStructure:function(){return this.__PH;
},addRenderer:function(name,m){this.__PG[name]=m;
m.join(this,name);
},removeRenderer:function(name){if(!this.__PG[name]){throw new Error("No existing renderer named "+name);
}delete this.__PG[name];
},render:function(){if(this.__PJ){return;
}this.__PJ=true;
var n=new qx.ui.progressive.State({progressive:this,model:this.getDataModel(),pane:this.__PH.getPane(),batchSize:this.getBatchSize(),rendererData:this.__PN(),userData:{}});
this.__PL=new Date();
if(this.__PI){this.__PK=n.getModel().getElementCount();
this.fireDataEvent(h,{state:n,initial:this.__PK});
this.__PM(n);
}else{qx.event.Timer.once(function(){this.__PK=n.getModel().getElementCount();
this.fireDataEvent(h,{state:n,initial:this.__PK});
this.__PM(n);
this.__PI=true;
},this,10);
}},_applyDataModel:function(o,p){if(p){p.removeListener(j,this.__PO,this);
p.dispose();
}o.addListener(j,this.__PO,this);
},__PM:function(q){var r;
var t;
var s;

for(var i=q.getBatchSize();i>0;i--){r=q.getModel().getNextElement();

if(!r){this.debug("Render time: "+(new Date()-this.__PL)+"ms");
this.__PJ=false;
this.fireDataEvent("renderEnd",q);
q.dispose();
return ;
}t=r.element;
s=this.__PG[t.renderer];
s.render(q,t);
this.fireDataEvent("progressDetail",{initial:this.__PK,remaining:r.remaining,element:t});
}this.fireDataEvent("progress",{initial:this.__PK,remaining:r.remaining});
if(this.getFlushWidgetQueueAfterBatch()){qx.ui.core.queue.Manager.flush();
}qx.event.Timer.once(function(){this.__PM(q);
},this,this.getInterElementTimeout());
},__PN:function(){var u={};

for(var name in this.__PG){u[name]={};
}return u;
},__PO:function(e){this.__PK=e.getData();
this.render();
}},destruct:function(){for(var name in this.__PG){this.__PG[name].dispose();
}this.__PL=this.__PG=this.__PH=null;
}});
})();
...
this.__Qd=[];
},members:{__Qd:null,addElements:function(c){this.__Qd=this.__Qd.concat(c);
this.fireDataEvent(b,this.__Qd.length);
},addElement:function(d){this.__Qd.push(d);
this.fireDataEvent(b,this.__Qd.length);
},getElementCount:function(){return this.__Qd.length;
},getNextElement:function(){if(this.__Qd.length>0){return ({element:this.__Qd.shift(),remaining:this.__Qd.length});
}return null;
}},destruct:function(){this.__Qd=null;
}});
})();
...
(function(){var l=".qx-progressive-",k="-col-",j="px;",h="-row",g="}",f="width: ",d="0px ",c="qx-progressive-",b="px",a="content",G="px ",F="end",E="div",D="__OF",C="qx.tableResizeDebug",B="  width : 100%;",A=" {",z="__columnData",y="css.boxmodel",x="qx.debug",s="relative",t="-row {",q="resize",r="start",o="",p="padding: ",m="__OC",n="qx.ui.progressive.renderer.table.Row",u="progressive-table-row-background-odd",v="progressive-table-row-background-even",w="left: ";
...
this.__OA=H;
this.__OB={};
this.__OC=new qx.ui.progressive.renderer.table.cell.Default();
this.__OD=null;
this.__OE={};
var I=qx.theme.manager.Color.getInstance();
this.__OE.bgcol=[];
this.__OE.bgcol[0]=I.resolve(v);
this.__OE.bgcol[1]=I.resolve(u);
this.__OF=new qx.ui.layout.HBox();
this.__OF.connectToWidget(this);
},statics:{__OG:null,__OH:6,__OI:"  position: absolute;"+"  top: 0px;"+"  height: 100%;"+"  overflow:hidden;"+"  text-overflow:ellipsis;"+"  -o-text-overflow: ellipsis;"+"  white-space:nowrap;"+"  border-right:1px solid #f2f2f2;"+"  border-bottom:1px solid #eeeeee;"+"  padding : 0px 6px 0px 6px;"+"  cursor:default;"+"  font-size: 11px;"+"  font-family: 'Segoe UI', Corbel, Calibri, Tahoma, 'Lucida Sans Unicode', sans-serif;"+((qx.core.Environment.get("engine.name")=="mshtml")?'':';-moz-user-select:none;')},properties:{defaultRowHeight:{init:16}},members:{__OD:null,__OJ:null,__OK:null,__OA:null,__OB:null,__OC:null,__OE:null,__OF:null,join:function(J,name){if(this.__OD){throw new Error("Renderer is already joined to a Progressive.");
}this.__OD=J;
this.__OJ=name;
...
if(!L.__OG[K]){L.__OG[K]={rowstylesheet:null,cellstylesheet:[]};
var M=l+K+t+B+g;
L.__OG[K].rowstylesheet=qx.bom.Stylesheet.createElement(M);
var N=this.__OA.getData();

for(var i=0;i<N.length;i++){var M=l+K+k+i+A+L.__OI+g;
L.__OG[K].cellstylesheet[i]=qx.bom.Stylesheet.createElement(M);
}this.__OK=K;
var O=J.getStructure().getPane();
O.addListener(q,this._resizeColumns,this);
}},addRenderer:function(P,Q){var R=this.__OA.getData();

if(P<0||P>=R.length){throw new Error("Column "+P+" out of range (max: "+(R.length-1)+")");
}this.__OB[P]=Q;
},removeRenderer:function(S){var T=this.__OA.getData();

if(S<0||S>=T.length){throw new Error("Column "+S+" out of range (max: "+(T.length-1)+")");
}
if(!this.__OB[S]){throw new Error("No existing renderer for column "+S);
}delete this.__OB[S];
...
var bc;
var Y=0;
if(U.getRendererData()[this.__OJ].end===undefined){U.getRendererData()[this.__OJ]={end:0,start:1,rows:0,totalHeight:0};
}var bf=document.createElement(E);
for(var i=0;i<bg.length;i++){var ba=c+this.__OK+k+i;
bc=this.__OB[i]||this.__OC;
...
var be=U.getRendererData()[this.__OJ];
be.totalHeight+=Y;
bf.style.position=s;
bf.style.height=Y+b;
bf.className=c+this.__OK+h;
bf.innerHTML=bb.join(o);
switch(V.location){case F:var W=be.end||0;
bf.style.backgroundColor=this.__OE.bgcol[W];
be.end=(W==0?1:0);
...
},getLayoutChildren:function(){return this.__OA.getData();
},_resizeColumns:function(e){var bp=this.__OD.getStructure().getPane();
var bm=bp.getBounds().width-qx.bom.element.Overflow.getScrollbarWidth();
var bn=l+this.__OK+h;
var bj=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bj.__OG[this.__OK].rowstylesheet,bn);
var bl=f+bm+j;
qx.bom.Stylesheet.addRule(bj.__OG[this.__OK].rowstylesheet,bn,bl);
this.__OF.renderLayout(bm,100);
var bo=this.__OA.getData();
for(var i=0,bs=0;i<bo.length;i++,bs+=bm){var bn=l+this.__OK+k+i;
var bj=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bj.__OG[this.__OK].cellstylesheet[i],bn);
bm=bo[i].getComputedWidth();

if(qx.core.Environment.get(x)){if(qx.core.Environment.get(C)){this.debug("col "+i+": width="+bm);
}}var bk;

if(qx.core.Environment.get(y)==a){bk=qx.ui.progressive.renderer.table.Row.__OH*2;
}else{bk=-1;
}var br=(bm-bk)+j;
var bq=d+qx.ui.progressive.renderer.table.Row.__OH+G+d+qx.ui.progressive.renderer.table.Row.__OH+j;
var bi=bs+j;
var bl=bj.__OI+f+br+w+bi+p+bq;
qx.bom.Stylesheet.addRule(bj.__OG[this.__OK].cellstylesheet[i],bn,bl);
}}},destruct:function(){var name;

for(name in this.__OB){this.__OB[name]=null;
}var bu=qx.ui.progressive.renderer.table.Row;
var bt=this.__OD.toHashCode();

if(bu.__OG&&bu.__OG[bt]){if(bu.__OG[bt].rowstylesheet){var bw=l+this.__OK+h;
var bu=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bu.__OG[this.__OK].rowstylesheet,bw);
}if(bu.__OG[bt].cellstylesheet){for(var i=bu.__OG[bt].cellstylesheet.length-1;i>=0;i--){var bw=l+this.__OK+k+i;
var bv=bu.__OG[this.__OK].cellstylesheet[i];
var bu=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bv,bw);
}}}
if(this.__OD&&this.__OD.getRendererData){var bx=this.__OD.getRendererData();

if(bx&&bx[this.__OJ]&&bx[this.__OJ].end!==undefined){bx[this.__OJ]=null;
}}this.__OE=this.__OB=this.__OD=this.__OA=null;
this._disposeObjects(D,m,z);
}});
})();
...
this.__Qj=v||s;
this.__Qk=w||s;
},members:{__Qe:null,__Qf:null,__Qg:null,__Qh:null,__Qi:null,__Qj:null,__Qk:null,__Ql:function(x,y){if(x.align){y[h]=x.align;
}
...
}},addNumericCondition:function(z,A,B,C,D,E,F){if(!qx.lang.Array.contains(this.__Qe,z)||A==null){throw new Error("Condition not recognized or value is null!");
}this.__Qg.push({condition:z,align:B,color:C,style:D,weight:E,value1:A,target:F});
},addBetweenCondition:function(G,H,I,J,K,L,M,N){if(!qx.lang.Array.contains(this.__Qf,G)||H==null||I==null){throw new Error("Condition not recognized or value1/value2 is null!");
}this.__Qg.push({condition:G,align:J,color:K,style:L,weight:M,value1:H,value2:I,target:N});
},addRegex:function(O,P,Q,R,S,T){if(!O){throw new Error("regex cannot be null!");
}this.__Qg.push({condition:o,align:P,color:Q,style:R,weight:S,regex:O,target:T});
},_getCellStyle:function(U){if(this.__Qg.length==0){return U.style||s;
}var i;
var Y;
var V;
var X={"text-align":this.__Qh,"color":this.__Qi,"font-style":this.__Qj,"font-weight":this.__Qk};

for(i=0;i<this.__Qg.length;i++){var bc=this.__Qg[i];
Y=false;

if(qx.lang.Array.contains(this.__Qe,bc.condition)){if(bc.target==null){V=U.cellData;
...
}}else if(qx.lang.Array.contains(this.__Qf,bc.condition)){if(bc.target==null){V=U.cellData;
}else{V=U.element.data[bc.target];
}
...
}},destruct:function(){this.__Qe=this.__Qf=this.__Qg=null;
}});
})();
