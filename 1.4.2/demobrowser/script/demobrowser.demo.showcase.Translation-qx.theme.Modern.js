...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.showcase.Translation","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.showcase.Translation-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"678120b38e04"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var l='middle',k="",j="value",h="The quick brown fox",g="Orange",f="execute",d="Hello World",c="I have an apple",b="MessageId",a="The mammal fox that is.",be="changeSelection",bd="Add translation<br>at runtime for: <b>",bc="</b>",bb="I have %1 apples",ba='right',Y="MessageId:",X="<b>Locale and language:</b>",W="Choose Locale:",V='Territory: ',U="en",s="Simple Translate: this.tr('Hello World')",t='Locale: ',q="Add new Locale<br> at runtime",r="en_QX",o="A comment: 'The mammal fox that is.'<br>is added in the .po file",p="Translation",m="Singular/Plural Translate: this.trn('I have an apple', I have %1 apples, ",n="changeLocale",w="qx.debug",x="Number of apples",F="show Translation",D="Locale Name (ex: en_QX)",M='bottom',H="Mark the string 'Orange' for later<br>translation so we do not forget it",Q='top',O="demo",z="Add new Locale",T='Territory',S="Simple messageId translation",R="Mark the string 'Orange' for later translation so we do not forget it: this.marktr('Orange')",y=", applesNumber)",B="<b>Dynamic translation. This is added at runtime.</b>",C="<b>Static translation [found in .po files]. Hover the labels below to see more info.</b>",E="Add Translation",G='Language',I="Singular/Plural Translate<br>Choose a number from the select box<br>to see plural translation",N="with .po comment: this.trc('The mammal fox that is.', 'The quick brown fox')",P='Language: ',u="<i>MessageId</i>",v="Number of apples:",A='Locale',L="click 'Show Translation' button to translate<br>the messageId in the current selected locale",K="<i>Translated Text</i>",J="demobrowser.demo.showcase.Translation";
...
if((qx.core.Environment.get(w))){qx.log.appender.Native;
qx.log.appender.Console;
}var bu=new qx.ui.container.Composite(new qx.ui.layout.Grid(8,8));
this.getRoot().add(bu,{top:20,left:20});
var bA=new qx.ui.basic.Label(X);
bA.setRich(true);
bu.add(bA,{row:0,column:0,colSpan:3});
var bT=new qx.ui.basic.Label(W);
bT.setMarginTop(5);
bT.setAlignY(l);
bu.add(bT,{row:1,column:0});
var bC=qx.locale.Manager.getInstance();
var bL=bC.getAvailableLocales().sort();
var bs=bC.getLocale();
var br=new qx.ui.form.SelectBox();
br.setAllowGrowY(false);
br.setAlignY(l);
var bB=null;

for(var i=0;i<bL.length;i++){var bI=new qx.ui.form.ListItem(bL[i]);
br.add(bI);

if((!bB&&bL[i]==U)||bL[i]==bs){bB=bI;
}}br.addListener(be,function(e){var bX=e.getData()[0].getLabel();
qx.locale.Manager.getInstance().setLocale(bX);
});
bu.add(br,{row:1,column:1});
var bU=new qx.ui.basic.Label(A);
bU.setMarginTop(5);
bU.setWidth(80);
var bR=new qx.ui.basic.Label(T);
bR.setMarginTop(5);
bR.setWidth(80);
var bS=new qx.ui.basic.Label(G);
bS.setMarginTop(5);
bS.setWidth(80);
var bm=new qx.ui.container.Composite(new qx.ui.layout.VBox(3));
bm.add(bU);
bm.add(bR);
bm.add(bS);
bu.add(bm,{row:1,column:2});
qx.locale.Manager.getInstance().addListener(n,function(){var bY=qx.locale.Manager.getInstance();
bU.setValue(t+bY.getLocale());
bR.setValue(V+bY.getTerritory());
bS.setValue(P+bY.getLanguage());
bf.apply(this);
bW.setValue(bd+bC.getLocale()+bc);
},this);
function bf(){bo.setValue(this.tr(d));
bo.setToolTipText(s);
var ca=parseInt(bP.getSelection()[0].getLabel());
bn.setValue(this.trn(c,bb,ca,ca));
bn.setToolTipText(m+ca+y);
bE.setValue(this.trc(a,h));
bE.setToolTipText(N);
bl.setValue(this.marktr(g));
bl.setToolTipText(R);
var cb=bN.getValue();

if(cb&&cb.length>0){bD.setValue(bC.translate(cb,[],bC.getLocale()));
}}var bO=new qx.ui.basic.Label(C);
bO.setRich(true);
bO.setMarginTop(15);
bu.add(bO,{row:2,column:0,colSpan:4});
var bg=new qx.ui.basic.Label(u);
bg.setRich(true);
bu.add(bg,{row:3,column:1});
var bz=new qx.ui.basic.Label(K);
bz.setRich(true);
bu.add(bz,{row:3,column:2});
var bG=new qx.ui.basic.Label(d);
bu.add(bG,{row:4,column:1});
var bo=new qx.ui.basic.Label(this.tr(d));
bu.add(bo,{row:4,column:2});
var bQ=new qx.ui.basic.Label(S);
bu.add(bQ,{row:4,column:3});
var bq=new qx.ui.basic.Label(v);
bq.setAlignX(ba);
bq.setAlignY(M);
bu.add(bq,{row:5,column:0});
var bP=new qx.ui.form.SelectBox();

for(var i=1;i<5;i++){var bp=new qx.ui.form.ListItem(i+k);
bP.add(bp);
}bP.setToolTipText(x);
bP.setAlignY(Q);
bP.setAlignX(ba);
bu.add(bP,{row:6,column:0});
var bh=new qx.ui.basic.Label(c);
bh.setAlignY(l);
bu.add(bh,{row:5,column:1,rowSpan:2});
var bn=new qx.ui.basic.Label(this.trn(c,bb,1,1));
bn.setAlignY(l);
bu.add(bn,{row:5,column:2,rowSpan:2});
var bK=new qx.ui.basic.Label(I);
bK.setAlignY(l);
bK.setRich(true);
bu.add(bK,{row:5,column:3,rowSpan:2});
bP.addListener(be,function(e){bf.apply(this);
});
var bt=new qx.ui.basic.Label(h);
bu.add(bt,{row:7,column:1});
var bE=new qx.ui.basic.Label(this.trc(a,h));
bE.setWidth(140);
bu.add(bE,{row:7,column:2});
var bH=new qx.ui.basic.Label(o);
bH.setRich(true);
bu.add(bH,{row:7,column:3});
var bw=new qx.ui.basic.Label(g);
bu.add(bw,{row:8,column:1});
var bl=new qx.ui.basic.Label(this.marktr(g));
bu.add(bl,{row:8,column:2});
var bv=new qx.ui.basic.Label(H);
bv.setRich(true);
bu.add(bv,{row:8,column:3});
var bj=new qx.ui.basic.Label(B);
bj.setRich(true);
bj.setMarginTop(15);
bu.add(bj,{row:9,column:0,colSpan:3});
var bW=new qx.ui.basic.Label(bd+bC.getLocale()+bc);
bW.setRich(true);
bW.setWidth(120);
bu.add(bW,{row:10,column:0});
var bi=new qx.ui.form.TextField(k);
bi.setToolTipText(b);
bu.add(bi,{row:10,column:1});
var bF=new qx.ui.form.TextField(k);
bF.setToolTipText(p);
bu.add(bF,{row:10,column:2});
var bx=new qx.ui.form.Button(E);
bx.setAllowGrowY(false);
bu.add(bx,{row:10,column:3});
bx.addListener(f,function(e){var cc={};
cc[bi.getValue()]=bF.getValue();
bC.addTranslation(bC.getLocale(),cc);
alert(bF.getValue()+" has been added for messageId: "+bi.getValue()+" for locale: "+bC.getLocale());
},this);
var by=new qx.ui.basic.Label(Y);
bu.add(by,{row:11,column:0});
var bN=new qx.ui.form.TextField(k);
bN.setToolTipText(b);
bu.add(bN,{row:11,column:1});
var bM=new qx.ui.form.Button(F);
bu.add(bM,{row:11,column:2});
bM.setAllowGrowY(false);
bM.addListener(f,function(e){var cd=bN.getValue();
bD.setValue(bC.translate(cd,[],bC.getLocale()));
},this);
var bD=new qx.ui.basic.Label(L);
bD.setRich(true);
bD.setAlignY(l);
bu.add(bD,{row:11,column:3});
var bV=new qx.ui.basic.Label(q);
bV.setRich(true);
bu.add(bV,{row:12,column:0});
var bk=new qx.ui.form.TextField(k);
bk.setPlaceholder(r);
bk.setToolTipText(D);
bu.add(bk,{row:12,column:1});
var bJ=new qx.ui.form.Button(z);
bJ.setAllowGrowY(false);
bu.add(bJ,{row:12,column:2});
bJ.addListener(f,function(e){var ce=bk.getValue();

if(ce!=null&&ce.length<1){alert("new Locale must not be empty string");
return;
}var cf=bC.getAvailableLocales();

if(cf.indexOf(ce)!=-1){alert(ce+" already exists");
}else{bC.addTranslation(ce,{"demo":O});
alert(ce+" has been added");
br.add(new qx.ui.form.ListItem(ce));
}},this);

if(bB){br.setSelection([bB]);
}bi.bind(j,bN,j);
bN.bind(j,bi,j);
return;
}}});
