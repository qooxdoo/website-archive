...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.animation.Login","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.animation.Login-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"10a0066b30b9"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var o="top",n="right",m="resize",l="Name",k="__SO",j="Password",h="__SQ",g="appear",f="__SP",d="demobrowser.demo.animation.Login",a="50%",c="execute",b="30%";
qx.Class.define(d,{extend:qx.application.Standalone,members:{__SO:null,__SP:null,__SQ:null,checkInput:function(){this.__SO.start();
},__SR:function(){this.__SO=new qx.fx.effect.combination.Shake(this.__SP.getContainerElement().getDomElement());
},main:function(){qx.application.Standalone.prototype.main.call(this);
var q=new qx.ui.layout.Grid(9,5);
q.setColumnAlign(0,n,o);
q.setColumnAlign(2,n,o);
this.__SP=new qx.ui.groupbox.GroupBox().set({contentPadding:[16,16,16,16]});
this.__SP.setLayout(q);
this.__SP.addListener(m,function(e){var u=this.__SP.getBounds();
this.__SP.set({marginTop:Math.round(-u.height/2),marginLeft:Math.round(-u.width/2)});
},this);
this.getRoot().add(this.__SP,{left:a,top:b});
var p=[l,j];

for(var i=0;i<p.length;i++){this.__SP.add(new qx.ui.basic.Label(p[i]).set({allowShrinkX:false,paddingTop:3}),{row:i,column:0});
}var r=new qx.ui.form.TextField();
var s=new qx.ui.form.PasswordField();
this.__SP.add(r.set({allowShrinkX:false,paddingTop:3}),{row:0,column:1});
this.__SP.add(s.set({allowShrinkX:false,paddingTop:3}),{row:1,column:1});
var t=this.__SQ=new qx.ui.form.Button("Login");
t.setAllowStretchX(false);
this.__SP.add(t,{row:3,column:1});
t.addListener(c,this.checkInput,this);
this.__SP.addListener(g,this.__SR,this);
}},destruct:function(){this._disposeObjects(f,h,k);
}});
})();
...
(function(){var e="horizontal",d="vertical",c="__MY",b="qx.fx.effect.combination.Shake",a="Number";
qx.Class.define(b,{extend:qx.fx.Base,construct:function(f){qx.fx.Base.call(this,f);
this.__MY=[new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f)];
},properties:{direction:{init:e,check:[e,d]},duration:{init:0.5,refine:true},distance:{init:20,check:a}},members:{__MY:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var g=parseFloat(this.getDistance());
var h=parseFloat(this.getDuration())/10.0;

if(this.getDirection()==e){this.__MY[0].set({x:g,y:0,duration:h});
this.__MY[1].set({x:-g*2,y:0,duration:h*2});
this.__MY[2].set({x:g*2,y:0,duration:h*2});
this.__MY[3].set({x:-g*2,y:0,duration:h*2});
this.__MY[4].set({x:g*2,y:0,duration:h*2});
this.__MY[5].set({x:-g,y:0,duration:h*2});
}else if(this.getDirection()==d){this.__MY[0].set({y:g,x:0,duration:h});
this.__MY[1].set({y:-g*2,x:0,duration:h*2});
this.__MY[2].set({y:g*2,x:0,duration:h*2});
this.__MY[3].set({y:-g*2,x:0,duration:h*2});
this.__MY[4].set({y:g*2,x:0,duration:h*2});
this.__MY[5].set({y:-g,x:0,duration:h*2});
}var k=this.__MY;

for(var i=0,j=this.__MY.length;i<j;i++){this.__MY[i].id=i;

if(i<5){this.__MY[i].afterFinishInternal=function(){k[this.id+1].start();
};
}}this.__MY[0].start();
}},destruct:function(){this._disposeArray(c);
}});
...
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:g,check:[g,e]},x:{init:0,check:f},y:{init:0,check:f}},members:{__Gr:null,__Gs:null,__Gt:null,__Gu:null,__Gv:null,setup:function(){var j=this._getElement();
qx.fx.Base.prototype.setup.call(this);

if(j.parentNode){this.__Gt=qx.bom.element.Location.getLeft(j)-qx.bom.element.Location.getLeft(j.parentNode);
this.__Gu=qx.bom.element.Location.getTop(j)-qx.bom.element.Location.getTop(j.parentNode);
}else{this.__Gt=qx.bom.element.Location.getLeft(j);
this.__Gu=qx.bom.element.Location.getTop(j);
}this.__Gv=qx.bom.element.Style.get(j,i);
qx.bom.element.Style.set(j,i,e);

if(this.getMode()==b){this.__Gr=this.getX()-this.__Gt;
this.__Gs=this.getY()-this.__Gu;
}else{this.__Gr=this.getX();
this.__Gs=this.getY();
}},update:function(k){var m=this._getElement();
qx.fx.Base.prototype.update.call(this);
var l=Math.round(this.__Gr*k+this.__Gt);
var top=Math.round(this.__Gs*k+this.__Gu);
qx.bom.element.Style.set(m,c,l+h);
qx.bom.element.Style.set(m,d,top+h);
},afterFinishInternal:function(){qx.bom.element.Style.set(this._getElement(),i,this.__Gv);
}}});
})();
