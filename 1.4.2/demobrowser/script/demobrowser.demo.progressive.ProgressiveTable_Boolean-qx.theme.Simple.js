...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.progressive.ProgressiveTable_Boolean","qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.progressive.ProgressiveTable_Boolean-qx.theme.Simple.js"]],
  urisBefore : [],
  packageHashes : {"0":"5a1b9b5b9dcf"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
var x=v.resolve(j);
this.__MH=w.toUri(x);
},members:{__MH:null,getBlankImage:function(){return this.__MH;
...
var D=this.__MI(B);
C.push(m);
if((qx.core.Environment.get(h)==r)&&/\.png$/i.test(D.url)){C.push(s,this.__MH,e,a,D.url,d,t);
...
return C.join(n);
},__MI:function(E){var F=this._identifyImage(E);
if(F==null||typeof F=="string"){F={url:F,tooltip:null};
}if(F.url==null){F.url=this.__MH;
}return F;
}}});
})();
(function(){var j="'; ",i="  this.src='",h="  var filters = this.filters.item(loader); ",g="}",f="{",e="  var loader = 'DXImageTransform.Microsoft.AlphaImageLoader'; ",d="  filters.sizingMethod = 'scale'; ",c="mshtml",b="engine.name",a="  filters.src='",F="\"",E="Boolean",D="  this.attributes.setNamedItem(namedItem); ",C="} catch (e) { ",B="var node = this.attributes.getNamedItem('celldata'); ",A="qx.ui.progressive.renderer.table.cell.Boolean",z="decoration/table/boolean-true.png",y="var value = node.nodeValue; ",x="decoration/table/boolean-false.png",w="celldata='1' ",q="  node.nodeValue='0'; ",r="  this.attributes.setNamedItem(node); ",o="if (value == '0') ",p="celldata='0' ",m="  node.nodeValue='1'; ",n="onclick=\"",k="  namedItem.value = node.nodeValue; ",l="var src; ",s="try { ",t="  var namedItem = document.createAttribute('celldata'); ",v="} ",u="else ";
...
this.__PU=J.toUri(H);
this.__PV=J.toUri(G);
},properties:{allowToggle:{check:E,init:false}},members:{__PU:null,__PV:null,__PW:null,__PX:null,__PY:null,__Qa:null,__Qb:null,__Qc:null,_identifyImage:function(K){var L={imageWidth:11,imageHeight:11};

switch(K.cellData){case true:L.url=this.__PU;
L.extras=w;
break;
case false:L.url=this.__PV;
L.extras=p;
break;
default:L.url=null;
break;
...
if((qx.core.Environment.get(b)==c)&&/\.png$/i.test(this.__PU)){L.extras+=i+this.getBlankImage()+j+e+h+a+this.__PU+j+d;
}else{L.extras+=i+this.__PU+j;
}L.extras+=m+v+u+f;

if((qx.core.Environment.get(b)==c)&&/\.png$/i.test(this.__PV)){L.extras+=i+this.getBlankImage()+j+e+h+a+this.__PV+j+d;
}else{L.extras+=i+this.__PV+j;
}L.extras+=q+g;
...
return N;
}},destruct:function(){this.__PU=this.__PV=null;
}});
})();
