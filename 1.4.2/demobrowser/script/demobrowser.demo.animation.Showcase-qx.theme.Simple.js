...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.animation.Showcase","qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.animation.Showcase-qx.theme.Simple.js"]],
  urisBefore : [],
  packageHashes : {"0":"e78d04ec0738"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
this.__IT=qx.fx.effect.core.Scale.originalStyle;
this.__IU=qx.fx.effect.core.Scale.fontTypes;
},properties:{scaleX:{init:true,check:o},scaleY:{init:true,check:o},scaleContent:{init:true,check:o},scaleFromCenter:{init:true,check:o},scaleFrom:{init:100.0,check:n},scaleTo:{init:100,check:n},restoreAfterFinish:{init:false,check:o},alternateDimensions:{init:[],check:j}},statics:{originalStyle:{'top':null,'left':null,'width':null,'height':null,'fontSize':null},fontTypes:{'em':c,'px':p,'%':f,'pt':g}},members:{__IV:null,__IW:null,__IX:null,__IY:null,__Ja:null,__Jb:null,__Jc:null,__IT:null,__IU:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
var v=this._getElement();
this.__IV=qx.bom.element.Style.get(v,"position");

for(var t in this.__IT){this.__IT[t]=v.style[t];
}this.__IW=qx.bom.element.Location.getTop(v);
this.__IX=qx.bom.element.Location.getLeft(v);

try{var u=qx.bom.element.Style.get(v,m);
}catch(w){if(typeof (u)!=b){u=(qx.core.Environment.get(k)==l)?h:e;
}}
for(var r in this.__IU){if(u.indexOf(r)>0){this.__IY=parseFloat(u);
this.__Ja=r;
break;
}}this.__Jb=(this.getScaleTo()-this.getScaleFrom())/100;
var s=this.getAlternateDimensions();

if(s.length==0){this.__Jc=[v.offsetWidth,v.offsetHeight];
}else{this.__Jc=s;
}},update:function(x){var z=this._getElement();
qx.fx.Base.prototype.update.call(this);
var y=(this.getScaleFrom()/100.0)+(this.__Jb*x);

if(this.getScaleContent()&&this.__IY){qx.bom.element.Style.set(z,m,this.__IY*y+this.__Ja);
}this._setDimensions(this.__Jc[0]*y,this.__Jc[1]*y);
},finish:function(){qx.fx.Base.prototype.finish.call(this);
var C=this._getElement();

if(this.getRestoreAfterFinish()){for(var A in this.__IT){var B=this.__IT[A];
...
if(this.getScaleFromCenter()){var J=(D-this.__Jc[0])/2;
var K=(E-this.__Jc[1])/2;

if(this.__IV==a){if(H){d.top=this.__IW-K+p;
}
if(G){d.left=this.__IX-J+p;
}}else{if(H){d.top=-K+p;
}
if(G){d.left=-J+p;
}}}
for(var F in d){qx.bom.element.Style.set(I,F,d[F]);
}}},destruct:function(){this.__Jc=this.__IT=this.__IU=null;
}});
})();
...
(function(){var t="linear",s="Number",r="reverse",q="Boolean",p="none",o="wobble",n="flicker",m="pulse",l="sinodial",k="easeOutQuad",d="spring",j="easeInQuad",g="full",c="Color",b="backgroundColor",f="#ffffff",e="backgroundImage",h="__Je",a="qx.fx.effect.combination.ColorFlow",i="#ffffaa";
qx.Class.define(a,{extend:qx.fx.Base,construct:function(u){qx.fx.Base.call(this,u);
this.__Je=[new qx.fx.effect.core.Highlight(u),new qx.fx.effect.core.Highlight(u)];
},properties:{startColor:{init:f,check:c},endColor:{init:i,check:c},forwardTransition:{init:t,check:[t,j,k,l,r,n,o,m,d,p,g]},backwardTransition:{init:t,check:[t,j,k,l,r,n,o,m,d,p,g]},forwardDuration:{init:1.0,check:s},backwardDuration:{init:1.0,check:s},delayBetween:{init:0.3,check:s},restoreBackground:{init:true,check:q},keepBackgroundImage:{init:false,check:q}},members:{__Jf:null,__Je:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var v=this._getElement();
this.setDuration(this.getForwardDuration()+this.getDelayBetween()+this.getBackwardDuration());
this.__Jf={backgroundImage:qx.bom.element.Style.get(v,e),backgroundColor:qx.bom.element.Style.get(v,b)};
this.__Je[0].set({startColor:this.getStartColor(),endColor:this.getEndColor(),duration:this.getForwardDuration(),transition:this.getForwardTransition(),restoreBackground:false,keepBackgroundImage:this.getKeepBackgroundImage()});
this.__Je[1].set({startColor:this.getEndColor(),endColor:this.getStartColor(),duration:this.getBackwardDuration(),transition:this.getBackwardTransition(),restoreBackground:this.getRestoreBackground(),keepBackgroundImage:this.getKeepBackgroundImage(),delay:this.getDelayBetween()});
var self=this;
this.__Je[0].afterFinishInternal=function(){self.__Je[1].start();
};
this.__Je[0].start();
}},destruct:function(){this._disposeArray(h);
}});
})();
(function(){var e="__OM",d="qx.fx.effect.combination.Pulsate",c="opacity",b="__OO",a="sinodial";
qx.Class.define(d,{extend:qx.fx.Base,construct:function(f){qx.fx.Base.call(this,f);
var g=this.getDuration()/6;
var h=0;
this.__OM=[new qx.fx.effect.core.Fade(f),new qx.fx.effect.core.Fade(f),new qx.fx.effect.core.Fade(f),new qx.fx.effect.core.Fade(f),new qx.fx.effect.core.Fade(f),new qx.fx.effect.core.Fade(f)];

for(var i=0,l=this.__OM.length;i<l;i++){this.__OM[i].set({duration:g,to:((h%2)!=0)?1:0,from:((h%2)!=0)?0:1,transition:a,modifyDisplay:false});
h++;
}},properties:{duration:{init:2,refine:true}},members:{__ON:null,__OM:null,__OO:null,__OP:false,beforeSetup:function(){this.__ON=qx.bom.element.Style.get(this._getElement(),"opacity");
...
for(var i=0,l=this.__OM.length;i<l;i++){this.__OM[i].id=j;
this.__OO.push(this.__OM[i]);
this.__OM[i].afterFinishInternal=function(){qx.lang.Array.remove(self.__OO,this);
if(this.id<5){self.__OM[this.id+1].start();
}else{if(self.__OP){self.end();
}}};
j++;
}this.__OM[0].start();
},end:function(){if(this.__OO.length==0){qx.fx.Base.prototype.end.call(this);
}else{this.__OP=true;
}},afterFinish:function(){qx.bom.element.Style.set(this._getElement(),c,this.__ON);
},_applyDuration:function(k,m){var n=k/6;

for(var i=0,l=this.__OM.length;i<l;i++){this.__OM[i].set({duration:n});
}},cancel:function(){for(var i=0,l=this.__OO.length;i<l;i++){this.__OO[i].cancel();
}qx.fx.Base.prototype.cancel.call(this);
}},destruct:function(){this._disposeArray(e);
this._disposeArray(b);
}});
})();
(function(){var i="__PA",h="opacity",g="Boolean",f="none",e="qx.fx.effect.combination.Puff",d="_effects",c="__PB",b="__Pz",a="display";
qx.Class.define(e,{extend:qx.fx.Base,construct:function(j){qx.fx.Base.call(this,j);
this.__Pz=new qx.fx.effect.core.Scale(j);
this.__PA=new qx.fx.effect.core.Fade(j);
this.__PB=new qx.fx.effect.core.Parallel(this.__Pz,this.__PA);
},properties:{modifyDisplay:{init:true,check:g}},members:{__PA:null,__Pz:null,__PB:null,afterFinishInternal:function(){if(this.getModifyDisplay()){qx.bom.element.Style.set(this._getElement(),a,f);
}},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var l=this._getElement();
var k={opacity:qx.bom.element.Style.get(l,h)};
this.__PA.afterFinishInternal=function(){var n=this._getElement();

for(var m in k){qx.bom.element.Style.set(n,m,k[m]);
}};
this.__Pz.set({scaleTo:200,sync:true,scaleFromCenter:true,scaleContent:true,restoreAfterFinish:true});
this.__PA.set({sync:true,to:0.0,modifyDisplay:false});
this.__PB.setDuration(this.getDuration());
this.__PB.start();
}},destruct:function(){this._disposeArray(d);
this._disposeObjects(c,b,i);
}});
})();
(function(){var b="qx.fx.effect.core.Parallel",a="__Jd";
qx.Class.define(b,{extend:qx.fx.Base,construct:function(c){qx.fx.Base.call(this);
this.__Jd=arguments;
},members:{__Jd:null,finish:function(){qx.fx.Base.prototype.finish.call(this);
var d=this.__Jd;
...
(function(){var j="sinodial",i="none",h="overflow",g="flicker",f="center",e="pulse",d="spring",c="full",b="wobble",a="easeOutQuad",E="easeInQuad",D="reverse",C="linear",B="Boolean",A="px",z='bottom-left',y='top-left',x='bottom-right',w="bottom-right",v="__MW",q="__MU",r="bottom-left",o="visible",p="qx.fx.effect.combination.Shrink",m="__MV",n="top-left",k="top-right",l='top-right',s='center',t="hidden",u="display";
qx.Class.define(p,{extend:qx.fx.Base,construct:function(F){qx.fx.Base.call(this,F);
this.__MU=new qx.fx.effect.core.Move(F);
this.__MV=new qx.fx.effect.core.Scale(F);
this.__MW=new qx.fx.effect.core.Parallel(this.__MU,this.__MV);
},properties:{direction:{init:f,check:[n,k,r,w,f]},moveTransition:{init:j,check:[C,E,a,j,D,g,b,e,d,i,c]},scaleTransition:{init:j,check:[C,E,a,j,D,g,b,e,d,i,c]},modifyDisplay:{init:true,check:B}},members:{__MX:null,__MU:null,__MV:null,__MW:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
...
for(var G in this.__MX){H=this.__MX[G];
...
var J,K;
this.__MX={top:qx.bom.element.Location.getTop(L,"scroll"),left:qx.bom.element.Location.getLeft(L,"scroll"),width:qx.bom.element.Dimension.getContentWidth(L),height:qx.bom.element.Dimension.getContentHeight(L),opacity:qx.bom.element.Style.get(L,"opacity")};

switch(this.getDirection()){case y:J=K=0;
break;
case l:J=this.__MX.width;
K=0;
break;
case z:J=0;
K=this.__MX.height;
break;
case x:J=this.__MX.width;
K=this.__MX.height;
break;
case s:J=this.__MX.width/2;
K=this.__MX.height/2;
break;
}this.__MU.set({x:J,y:K,sync:true,transition:this.getMoveTransition()});
this.__MV.set({scaleTo:0,sync:true,transition:this.getScaleTransition(),restoreAfterFinish:true});
this.__MW.setDuration(this.getDuration());
this.__MW.start();
}},destruct:function(){this._disposeObjects(q,m,v);
}});
})();
(function(){var j="sinodial",i="px",h="flicker",g="0px",f="center",e="pulse",d="spring",c="full",b="wobble",a="easeOutQuad",J="easeInQuad",I="reverse",H="none",G="overflow",F="linear",E="height",D='bottom-left',C='top-left',B='bottom-right',A="bottom-right",q="qx.fx.effect.combination.Grow",r="__IR",o="bottom-left",p="visible",m="width",n="top-left",k="top",l="top-right",s='top-right',t="__IS",v="block",u='center',x="__IQ",w="hidden",z="left",y="display";
qx.Class.define(q,{extend:qx.fx.Base,construct:function(K){qx.fx.Base.call(this,K);
this.__IQ=new qx.fx.effect.core.Move(K);
this.__IR=new qx.fx.effect.core.Scale(K);
this.__IS=new qx.fx.effect.core.Parallel(this.__IQ,this.__IR);
},properties:{direction:{init:f,check:[n,l,o,A,f]},scaleTransition:{init:j,check:[F,J,a,j,I,h,b,e,d,H,c]},moveTransition:{init:j,check:[F,J,a,j,I,h,b,e,d,H,c]}},members:{__IR:null,__IQ:null,__IS:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
...
this.__IR.afterFinishInternal=function(){var S;
...
break;
}this.__IQ.set({x:P,y:Q,sync:true,transition:this.getMoveTransition()});
this.__IR.set({scaleTo:100,sync:true,scaleFrom:0,scaleFromCenter:false,transition:this.getScaleTransition(),alternateDimensions:[N.width,N.height]});
...
this.__IS.setDuration(this.getDuration());
this.__IS.start();
}},destruct:function(){this._disposeObjects(x,r,t);
}});
})();
(function(){var u="out",t="px",s="south",r="Number",q="north-west",p="west",o="in",n="north",m="south-east",l="south-west",e="east",k="north-east",h="display",c="__NT",b="__NS",g="top",f="Boolean",i="block",a="left",j="__NU",d="qx.fx.effect.combination.Drop";
qx.Class.define(d,{extend:qx.fx.Base,construct:function(v){qx.fx.Base.call(this,v);
this.__NS=new qx.fx.effect.core.Move(v);
this.__NT=new qx.fx.effect.core.Fade(v);
this.__NU=new qx.fx.effect.core.Parallel(this.__NS,this.__NT);
},properties:{direction:{init:s,check:[s,p,e,n,l,m,k,q]},xAmount:{init:100,check:r},yAmount:{init:100,check:r},mode:{init:u,check:[o,u]},modifyDisplay:{init:true,check:f}},members:{__NT:null,__NS:null,__NU:null,start:function(){qx.fx.Base.prototype.start.call(this);
...
this.__NT.afterFinishInternal=function(){for(var B in z){qx.bom.element.Style.set(A,B,z[B]+t);
}};
this.__NT.set({duration:Math.min(0.5,this.getDuration()),sync:true,from:(this.getMode()==u)?1:0,to:(this.getMode()==u)?0:1,modifyDisplay:true});
this.__NU.setDuration(this.getDuration());
this.__NU.start();
}},destruct:function(){this._disposeObjects(b,c,j);
}});
})();
(function(){var q="display",p="in",o="overflow",n="block",m="none",l="visible",k="0px",j="hidden",i="visiblity",h="Boolean",c="height",g="width",f="px",b="__Pv",a="qx.fx.effect.combination.Fold",e="__Pu",d="out";
qx.Class.define(a,{extend:qx.fx.Base,construct:function(r){qx.fx.Base.call(this,r);
},properties:{modifyDisplay:{init:true,check:h},mode:{init:p,check:[p,d]}},members:{__Pu:null,__Pv:null,__Pw:null,afterFinish:function(){var s=this._getElement();

if((this.getModifyDisplay())&&(this.getMode()==p)){qx.bom.element.Style.set(s,q,n);
}},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var t=this._getElement();
var self=this;
this.__Pu=new qx.fx.effect.core.Scale(t);
this.__Pv=new qx.fx.effect.core.Scale(t);
this.__Pu.afterFinishInternal=function(){self.__Pv.start();
};
this.__Pv.afterFinishInternal=function(){self._cleanUp();
};
this.__Pw=this._getStyle();
qx.bom.element.Style.set(t,o,j);

if(this.getMode()==p){this.__Pu.set({scaleTo:5,scaleContent:false,scaleX:false,duration:this.getDuration()/2,scaleFrom:100,scaleFromCenter:true,alternateDimensions:[]});
this.__Pv.set({scaleTo:5,scaleContent:false,scaleY:false,duration:this.getDuration()/2,scaleFrom:100,scaleFromCenter:true,alternateDimensions:[]});
}else{this.__Pu.set({scaleTo:100,scaleContent:false,scaleY:false,duration:this.getDuration()/2,scaleFrom:0,scaleFromCenter:true,alternateDimensions:[this.__Pw.width,this.__Pw.height]});
this.__Pv.set({scaleTo:100,scaleContent:false,scaleX:false,duration:this.getDuration()/2,scaleFrom:0,scaleFromCenter:false,alternateDimensions:[this.__Pw.width,this.__Pw.height]});
...
for(var u in this.__Pw){v=this.__Pw[u];
...
(function(){var j="off",i="overflow",h="Boolean",g="qx.fx.effect.combination.Switch",f="none",e="flicker",d="__Nj",c="__Nk",b="hidden",a="display";
qx.Class.define(g,{extend:qx.fx.Base,construct:function(k){qx.fx.Base.call(this,k);
this.setTransition(e);
var l=this.__Nj=new qx.fx.effect.core.Scale(k);
this.__Nj.beforeSetup=function(){qx.bom.element.Style.set(k,i,b);
};
this.__Nk=new qx.fx.effect.core.Fade(k);
this.__Nk.afterFinishInternal=function(){l.start();
};
},properties:{duration:{init:0.5,refine:true},from:{init:0.0,refine:true},to:{init:1.0,refine:true},modifyDisplay:{init:true,check:h},mode:{init:j,check:[j]}},members:{__Nj:null,__Nk:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
var n=this._getElement();
var m=qx.bom.element.Style.get(n,i);
this.__Nj.afterFinishInternal=function(){qx.bom.element.Style.set(n,i,m);
};
},afterFinish:function(){if(this.getModifyDisplay()&&(this.getMode()==j)){qx.bom.element.Style.set(this._getElement(),a,f);
}},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}
if(this.getMode()==j){this.__Nj.set({scaleTo:1.0,duration:this.getDuration()/2,scaleFromCenter:true,scaleX:false,scaleContent:false,restoreAfterFinish:true});
this.__Nk.set({duration:this.getDuration()/2,from:this.getFrom(),to:1});
}else{}this.__Nk.start();
},_applyDuration:function(o,p){this.__Nj.setDuration(o/2);
this.__Nk.setDuration(o/2);
