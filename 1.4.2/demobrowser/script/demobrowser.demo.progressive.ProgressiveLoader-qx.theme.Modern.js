...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.progressive.ProgressiveLoader","qx.theme":"qx.theme.Modern"};
...
(function(){var m="pane",k="lastTab",j="bar",h="page",g="firstTab",f="right",d="bottom",c="button",b="changeSelection",a="close",C="top",B="left",A="__sr",z="qx.event.type.Data",y="barRight",x="beforeChangeSelection",w="qx.debug",v="close-button",u="tabview",t="vertical",r="_applyBarPosition",s="barLeft",p="horizontal",q="qx.ui.tabview.TabView",n="barTop",o="barBottom";
...
this.__sq={top:n,right:y,bottom:o,left:s};
this._createChildControl(j);
this._createChildControl(m);
var E=this.__sr=new qx.ui.form.RadioGroup;
E.setWrap(false);
E.addListener(b,this._onChangeSelection,this);
if(D!=null){this.setBarPosition(D);
}else{this.initBarPosition();
}},events:{"changeSelection":z},properties:{appearance:{refine:true,init:u},barPosition:{check:[B,f,C,d],init:C,apply:r}},members:{__sr:null,_createChildControlImpl:function(F,G){var H;
...
},add:function(I){if(qx.core.Environment.get(w)){if(!(I instanceof qx.ui.tabview.Page)){throw new Error("Incompatible child for TabView: "+I);
}}var J=I.getButton();
...
M.add(I);
this.__sr.add(J);
I.addState(this.__sq[this.getBarPosition()]);
I.addState(k);
...
S.remove(N);
this.__sr.remove(P);
N.removeState(this.__sq[this.getBarPosition()]);
...
},__sq:null,_applyBarPosition:function(U,V){var W=this.getChildControl(j);
var bc=U==B||U==f;
var ba=U==f||U==d;
...
W.setOrientation(bc?t:p);
var bd=this.getChildren();
if(V){var X=this.__sq[V];
W.removeState(X);
for(var i=0,l=bd.length;i<l;i++){bd[i].removeState(X);
}}
if(U){var Y=this.__sq[U];
W.addState(Y);
for(var i=0,l=bd.length;i<l;i++){bd[i].addState(Y);
}}},getSelection:function(){var bf=this.__sr.getSelection();
var bg=[];
...
}this.__sr.setSelection(bi);
},resetSelection:function(){this.__sr.resetSelection();
},isSelected:function(bj){var bk=bj.getChildControl(c);
return this.__sr.isSelected(bk);
},isSelectionEmpty:function(){return this.__sr.isSelectionEmpty();
},getSelectables:function(bl){var bm=this.__sr.getSelectables(bl);
var bn=[];
...
var bu=bv.getButton().getChildControl(v);
bu.reset();
this.remove(bv);
