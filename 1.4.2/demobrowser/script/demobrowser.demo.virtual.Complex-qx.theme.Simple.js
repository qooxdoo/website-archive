...
this.__Vt=[];
},members:{_pool:null,__Vt:null,getCellData:function(s,t){if(!this.__Vt[s]){this.__Vt[s]=[];
}
if(!this.__Vt[s][t]){this.__Vt[s][t]={label:this.__Vu(),icon:this.__Vv()};
}return this.__Vt[s][t];
},getCellWidget:function(u,v){var w;

if(v%2==0){w=this._pool.atom.pop();

if(!w){w=new qx.ui.basic.Atom();
w.addListener(c,function(){var x=this.__Vv();
w.setIcon(x);
this.__Vt[u][v].icon=x;
},this);
...
}},__Vu:function(){var name=m;

for(var j=0;j<10;j++){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);
}return name;
},__Vv:function(){var z=n;
var B=k;
var C=[l,g,h,b,d];
var A=Math.floor(Math.random()*4);
return (z+16+e+B+C[A]);
}},destruct:function(){this._pool=this.__Vt=null;
}});
})();
...
(function(){var k="_applyInterval",j="appear",i="qx.debug",h="interval",g="qx.ui.virtual.behavior.Prefetch",f="_applyScroller",e="__Qz",d="qx.ui.virtual.core.Scroller",c="scrollbar-x",b="Integer",a="scrollbar-y";
qx.Class.define(g,{extend:qx.core.Object,construct:function(l,m){if(qx.core.Environment.get(i)){this.assertObject(m);
...
this.__Qz=new qx.event.Timer(this.getInterval());
this.__Qz.addListener(h,this._onInterval,this);

if(l){this.setScroller(l);
}},properties:{scroller:{check:d,nullable:true,init:null,apply:f},interval:{check:b,init:200,apply:k}},members:{__QA:null,__QB:null,__Qz:null,__QC:null,__QD:null,setPrefetchX:function(n,o,p,q){this.__QA=[n,o,p,q];
},setPrefetchY:function(r,s,t,u){this.__QB=[r,s,t,u];
},_onInterval:function(){var v=this.__QA;
...
qx.ui.core.queue.Manager.flush();
}},_applyScroller:function(x,y){if(y){if(this.__QC){y.getChildControl(c).removeListenerById(this.__QC);
}
if(this.__QD){y.getChildControl(a).removeListenerById(this.__QD);
}}
if(x){if(!x.getContainerElement().getDomElement()){this.__Qz.stop();
x.addListenerOnce(j,this.__Qz.start,this.__Qz);
}else{this.__Qz.restart();
}this.__QC=x.getChildControl("scrollbar-x").addListener("scroll",this.__Qz.restart,this.__Qz);
this.__QD=x.getChildControl("scrollbar-y").addListener("scroll",this.__Qz.restart,this.__Qz);
}else{this.__Qz.stop();
}},_applyInterval:function(z,A){this.__Qz.setInterval(z);
}},destruct:function(){this.setScroller(null);
this.__QA=this.__QB=null;
