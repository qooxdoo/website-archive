...
var envinfo = {"qx.allowUrlSettings":true,"qx.application":"demobrowser.Application","qx.contrib":false,"qx.revision":"28147","qx.theme":"demobrowser.Theme","qx.version":"1.4.2"};
for (var k in envinfo) qx.$$environment[k] = envinfo[k];

if (!qx.$$libraries) qx.$$libraries = {};
var libinfo = {"__out__":{"sourceUri":"script"},"demobrowser":{"resourceUri":"resource","sourceUri":"script","version":"trunk"},"qx":{"resourceUri":"resource","sourceUri":"script","version":"1.4.2"}};
...
(function(){var k="on",j="qx.debug",h="off",g="default",f="|",e="object",d="qxvariant",c="qx.client",b="qx.aspects",a=":",y="mshtml",x="qx.mobile.emulatetouch",w="qx.dynlocale",u=".",t="qx.core.Variant",s="gecko",r="opera",q="&",p="qx.mobile.nativescroll",o="$",m="qx.allowUrlVariants",n="webkit";
qx.Bootstrap.define(t,{statics:{__bt:{},__bu:{},compilerIsSet:function(){return true;
},define:function(z,A,B){if(qx.core.Variant.compilerIsSet(j,k)){qx.Bootstrap.warn("The method 'qx.core.Variant.define('"+z+"')' is deprecated: "+"Please use qx.core.Environment.add('"+z+"') instead.");
}this.defineDeprecated(z,A,B);
},defineDeprecated:function(C,D,E){if(qx.core.Variant.compilerIsSet(j,k)){if(!this.__bx(D)){throw new Error('Allowed values of variant "'+C+'" must be defined!');
}
if(E===undefined){throw new Error('Default value of variant "'+C+'" must be defined!');
}}
if(!this.__bt[C]){this.__bt[C]={};
}else if(qx.core.Variant.compilerIsSet(j,k)){if(this.__bt[C].defaultValue!==undefined){throw new Error('Variant "'+C+'" is already defined!');
}}this.__bt[C].allowedValues=D;
this.__bt[C].defaultValue=E;
},get:function(F){var G=this.__bt[F];

if(qx.core.Variant.compilerIsSet(j,k)){qx.Bootstrap.warn("The method 'qx.core.Variant.get('"+F+"')' is deprecated: "+"Please use qx.core.Environment.get('"+F+"') instead.");

if(G===undefined){throw new Error('Variant "'+F+'" is not defined.');
}}
if(G.value!==undefined){return G.value;
}return G.defaultValue;
},__bv:function(){if(window.qxvariants){for(var H in qxvariants){if(qx.core.Variant.compilerIsSet(j,k)){if((H.split(u)).length<2){throw new Error('Malformed settings key "'+H+'". Must be following the schema "namespace.key".');
}}
if(!this.__bt[H]){this.__bt[H]={};
}this.__bt[H].value=qxvariants[H];
}window.qxvariants=undefined;

try{delete window.qxvariants;
}catch(I){}this.__bw(this.__bt);
}},__bw:function(){if(qx.core.Environment.get(m)!=true){return;
}var J=document.location.search.slice(1).split(q);

for(var i=0;i<J.length;i++){var K=J[i].split(a);

if(K.length!=3||K[0]!=d){continue;
}var L=K[1];

if(!this.__bt[L]){this.__bt[L]={};
}
if(qx.core.Variant.compilerIsSet(j,k)){qx.Bootstrap.warn("URL variants are deprecated. Please use URL environment "+"variables instead. (qxvariant --> qxenv)");
}this.__bt[L].value=decodeURIComponent(K[2]);
}},select:function(M,N){if(qx.core.Variant.compilerIsSet(j,k)){qx.Bootstrap.warn("The method 'qx.core.Variant.select('"+M+"')' is deprecated: "+"Please use qx.core.Environment.select('"+M+"') instead.");
if(!this.__by(this.__bt[M])){throw new Error("Variant \""+M+"\" is not defined");
}
if(!this.__by(N)){throw new Error("the second parameter must be a map!");
}}
for(var O in N){if(this.isSet(M,O)){return N[O];
}}
if(N[g]!==undefined){return N[g];
}
if(qx.core.Variant.compilerIsSet(j,k)){throw new Error('No match for variant "'+M+'" in variants ['+qx.Bootstrap.getKeysAsString(N)+'] found, and no default ("default") given');
}},isSet:function(P,Q){if(qx.core.Variant.compilerIsSet(j,k)){qx.Bootstrap.warn("The method 'qx.core.Variant.isSet('"+P+"')' is deprecated: "+"Please use 'qx.core.Environment.get('"+P+"') ==' instead. ");
}var R=P+o+Q;

if(this.__bu[R]!==undefined){return this.__bu[R];
}var T=false;
if(Q.indexOf(f)<0){T=this.get(P)===Q;
}else{var S=Q.split(f);

for(var i=0,l=S.length;i<l;i++){if(this.get(P)===S[i]){T=true;
break;
}}}this.__bu[R]=T;
return T;
},__bx:function(v){return typeof v===e&&v!==null&&v instanceof Array;
},__by:function(v){return typeof v===e&&v!==null&&!(v instanceof Array);
},__bz:function(U,V){for(var i=0,l=U.length;i<l;i++){if(U[i]==V){return true;
}}return false;
}},defer:function(W){W.defineDeprecated(c,[s,y,r,n],qx.bom.client.Engine.getName());
W.defineDeprecated(j,[k,h],k);
W.defineDeprecated(b,[k,h],h);
W.defineDeprecated(w,[k,h],k);
W.defineDeprecated(x,[k,h],h);
W.defineDeprecated(p,[k,h],h);
W.__bv();
}});
})();
...
(function(){var cV="execute",cU="visible",cT="excluded",cS="value",cR="current",cQ="/",cP="",cO="qx.version",cN="tags",cM="category",bV="modelLink",bU="html",bT="all",bS="interval",bR="div",bQ="changeSelection",bP="node",bO="_blank",bN="auto",bM="noPlayground",dd="qx.theme.Modern",de="horizontal",db="~",dc="enabled",cY="right",da="main",cW="js",cX="Unable to access application.",df="widget",dg="visibility",cu="http://demo.qooxdoo.org/",ct="log",cx="separator-vertical",cv="request",cz="icon/22/actions/edit-clear.png",cy="Demos",cB="Simple Theme",cA="^.*",cs="filled",cr="Clear log",o="changeValue",p="Display HTML source",q="browser.name",v="icon/22/actions/media-playback-start.png",x="Open the qooxdoo Manual",y="demobrowser.DemoBrowser",z="HTML Code",A="load",B="Run previous demo",C="http://manual.qooxdoo.org/",dv="_history",du="ie",dt="icon/22/actions/help-contents.png",ds='_cmdNamespacePollution',dz="_leftComposite",dy="icon/22/apps/utilities-color-chooser.png",dx="_status",dw="Ctrl+N",dB="__Sc",dA="completed",bm="Previous",bn="Run",bk="failed",bl="_searchTextField",bq="Open demo in the playground",br="icon/22/actions/help-about.png",bo="f1",bp="white",bi="Debug",bj="Display log file",U="icon/22/actions/media-playback-stop.png",T="icon/22/apps/utilities-log-viewer.png",W="Ctrl+O",V="Log File",Q="Stop playback after current demo",P="qx.theme.Classic",S="_nextButton",R='"',O="logappender",N="_runbutton",bw="Choose theme",bx=".html",by="icon/22/apps/internet-web-browser.png",bz="#",bs="f2",bt="Ctrl+Right",bu="_stopbutton",bv="JS Code",bA="icon/22/mimetypes/executable.png",bB="ig",bf="middle",be="Unable to access namespace. Maybe no demo loaded.",bd="__Sk",bc="Stop",bb='"}',ba="_urlWindow",Y="icon/22/actions/edit-redo.png",X='_cmdDisposeSample',bh="toolbar",bg="icon/22/apps/office-spreadsheet.png",bC="To Playground",bD="demo/welcome.html",bE='_cmdSampleInOwnWindow',bF=".*",bG=" ",bH="background-splitpane",bI='demo/',bJ="__Se",bK="Open the qooxdoo API Viewer",bL="Debugging options",ca="Ctrl+P",bY="Filter...",bX="Manual",bW="app-header",ce="__Sl",cd="/playground/",cc="Dispose Demo",cb='{"code": ',cg="Classic Theme",cf="/apiviewer/",cn="__Sd",co="Global Namespace Pollution",cl="viewGroup",cm="icon/16/actions/edit-find.png",cj="_",ck="qooxdoo ",ch="Own Window",ci='_cmdObjectSummary',cp="Info",cq="_prevButton",cF="file:",cE='_cmdNextSample',cH="__Sj",cG="Demo has been disposed.",cJ="left",cI=".",cL="Run next demo",cK='_cmdPrevSample',cD="background-medium",cC="Ctrl+Left",dn="tree1",dp="Display JavaScript source",dq='_cmdRunSample',dr="Next",dj="F5",dk="_demoView",dl="icon/22/actions/application-exit.png",dm="Modern Theme",dh="Open demo in new window",di="API Viewer",n="_infosplit",l="runbutton",k="8px",h="_navPart",g="?qx.theme=",f="textfield",d="Ctrl+D",c="mainsplit",b="Could not open the Playground.",a="icon/22/actions/go-previous.png",F="Object Summary",G="_tree",D="Demo Browser",E="-",J="Run the selected demo(s)",K="monospace",H="icon/22/actions/go-next.png",I="qx.theme.Simple",L="Theme",M="_iframe";
qx.Class.define(y,{extend:qx.ui.container.Composite,construct:function(){qx.ui.container.Composite.call(this);
var dF=new qx.ui.layout.VBox;
dF.setSeparator(cx);
this.setLayout(dF);
...
this.__Sc=this.__Sx();
this.add(this.__Sc);
var dJ=new qx.ui.splitpane.Pane(de);
this.mainsplit=dJ;
var dG=new qx.ui.splitpane.Pane(de);
dG.setDecorator(null);
this._infosplit=dG;
this.add(dJ,{flex:1});
var dL=this._leftComposite=new qx.ui.container.Composite();
dL.setLayout(new qx.ui.layout.VBox(3));
dL.setBackgroundColor(bH);
dJ.add(dL,0);
var dE=new qx.ui.container.Composite();
dE.setLayout(new qx.ui.layout.HBox(3));
dE.setAppearance(f);
dL.add(dE);
var dM=new qx.ui.basic.Image(cm);
dE.add(dM);
this._searchTextField=new qx.ui.form.TextField();
this._searchTextField.setLiveUpdate(true);
this._searchTextField.setAppearance(df);
this._searchTextField.setPlaceholder(bY);
var dK=new qx.event.Timer(500);
dK.addListener(bS,function(dN){this.filter(this._searchTextField.getValue());
dK.stop();
},this);
this._searchTextField.addListener(o,function(dO){dK.restart();
},this);
dE.add(this._searchTextField,{flex:1});
this._status=new qx.ui.basic.Label(cP);
this._status.setAppearance(df);
this._status.setWidth(80);
this._status.setTextAlign(cY);
dE.add(this._status);
dJ.add(dG,1);
this._tree=this.__SC();
dL.add(this._tree,{flex:1});
this.__SD();
this._demoView=this.__Sy();
{dG.add(this._demoView,2);
};
var dC=this.__SA();
var dD=this.__SB();
var dH=this.__Sz();
var dI=new qx.ui.container.Stack;
dI.setDecorator(da);
dI.add(dC);
dI.add(dD);
dI.add(dH);
this.viewGroup.addListener(bQ,function(e){var dP=e.getData()[0];
var dQ=dP!=null?dP.getUserData(cS):cP;

switch(dQ){case bU:this.setSelection([dC]);
dI.show();
break;
case cW:this.setSelection([dD]);
dI.show();
break;
case ct:this.setSelection([dH]);
dI.show();
break;
...
this._history.addListener(cv,function(e){var dR=e.getData().replace(db,cQ);

if(this._currentSample!=dR){this.setCurrentSample(dR);
}},this);
this.__Sd=[this.__Sj,this.__Sk,this.__Sl];
{this.__Sd.push(this.__Sh);
};
this.__Se=new qx.event.Timer(250);
this.__Se.addListener(bS,this.__SF,this);
this.__Se.start();
this.__Sf=new demobrowser.InfoWindow(this.tr(cp));
this.__Sf.setAutoCenter(true);
},properties:{playDemos:{check:[bT,cM,cR],init:cR}},members:{_iframe:null,__Sb:null,__Se:null,__Sg:null,_tree:null,_status:null,_searchTextField:null,__Sh:null,__Si:null,__Sd:null,_versionFilter:null,__Sj:null,__Sk:null,__Sl:null,__Sm:null,__Sc:null,_leftComposite:null,_infosplit:null,_demoView:null,__Sn:null,_urlWindow:null,__Sf:null,defaultUrl:bD,playgroundUrl:cu+qx.core.Environment.get(cO)+cd,__So:function(){this._cmdObjectSummary=new qx.ui.core.Command(W);
this._cmdObjectSummary.addListener(cV,this.__Sp,this);
this._cmdRunSample=new qx.ui.core.Command(dj);
this._cmdRunSample.addListener(cV,this.runSample,this);
this._cmdPrevSample=new qx.ui.core.Command(cC);
this._cmdPrevSample.addListener(cV,this.playPrev,this);
this._cmdNextSample=new qx.ui.core.Command(bt);
this._cmdNextSample.addListener(cV,this.playNext,this);
this._cmdSampleInOwnWindow=new qx.ui.core.Command(dw);
this._cmdSampleInOwnWindow.addListener(cV,this.__Sq,this);
this._cmdDisposeSample=new qx.ui.core.Command(d);
this._cmdDisposeSample.addListener(cV,this.__Sv,this);
this._cmdNamespacePollution=new qx.ui.core.Command(ca);
this._cmdNamespacePollution.addListener(cV,this.__Sw,this);
},__Sp:function(){var dT=this._iframe.getWindow();
var dS;

if(dT&&dT.qx){dS=dT.qx.dev.ObjectSummary.getInfo();
}else{dS=be;
}var dU=new qx.ui.form.TextArea(dS);
dU.setDecorator(null);
dU.setAutoSize(true);
dU.setMaxHeight(qx.bom.Viewport.getHeight()-100);
this.__Sf.setContent(dU);
this.__Sf.setWidth(400);
this.__Sf.show();
},__Sq:function(){var dV=this._iframe.getSource();
window.open(dV,bO);
},__Sr:function(dW){var dX=!!dW;
var dY=this._tree.getSelection()[0].getUserData(cN);

if(dY){dX=dX&&!qx.lang.Array.contains(dY,bM);
}this.__Sh.setEnabled(dX);
this.__Si=dW;
},__Ss:function(){if(this.__Si){var ec=this.__Si;
var eb=cb+R+encodeURIComponent(ec)+bb;
var ea=this.playgroundUrl+bz+encodeURIComponent(eb);
window.open(ea,bO);
}else{var ed=new qx.ui.basic.Label(this.tr(b));
this.__Sf.setContent(ed);
this.__Sf.setWidth(200);
this.__Sf.show();
}},__St:function(){window.open(cu+qx.core.Environment.get(cO)+cf);
},__Su:function(){var ee=(qx.core.Environment.get(cO).split(E)[0]);
window.open(C+ee);
},__Sv:function(e){var eg=this._iframe.getWindow();
var ef;

if(eg&&eg.qx){eg.qx.core.ObjectRegistry.shutdown();
ef=this.tr(cG);
}else{ef=this.tr(cX);
}var eh=new qx.ui.basic.Label(ef);
eh.setRich(true);
eh.setWrap(true);
this.__Sf.setContent(eh);
this.__Sf.setWidth(350);
this.__Sf.show();
},__Sw:function(e){var ej=this._iframe.getWindow();
var ei;

if(ej&&ej.qx){ei=ej.qx.dev.Pollution.getInfo();
}else{ei=this.tr(cX);
}var ek=new qx.ui.basic.Label(ei);
ek.setRich(true);
ek.setWrap(true);
this.__Sf.setContent(ek);
this.__Sf.setWidth(350);
this.__Sf.show();
},__Sx:function(){var eF=new qx.ui.toolbar.ToolBar();
this._navPart=new qx.ui.toolbar.Part();
eF.add(this._navPart);
this._runbutton=new qx.ui.toolbar.Button(this.tr(bn),v);
this._runbutton.addListener(cV,this.runSample,this);
this._runbutton.setToolTipText(J);
this._navPart.add(this._runbutton);
this._stopbutton=new qx.ui.toolbar.Button(this.tr(bc),U);
this._stopbutton.addListener(cV,this.stopSample,this);
this._stopbutton.setToolTipText(Q);
...
var er=new qx.ui.toolbar.Button(this.tr(bm),a);
er.addListener(cV,this.playPrev,this);
er.setToolTipText(B);
this._navPart.add(er);
this._prevButton=er;
var eD=new qx.ui.toolbar.Button(this.tr(dr),H);
eD.addListener(cV,this.playNext,this);
eD.setToolTipText(cL);
this._navPart.add(eD);
this._nextButton=eD;
var et={converter:function(eM){return eM==cU;
}};
qx.data.SingleValueBinding.bind(this._runbutton,dg,er,dc,et);
qx.data.SingleValueBinding.bind(this._runbutton,dg,eD,dc,et);
var eK=new qx.ui.toolbar.Button(this.tr(ch),Y);
eK.addListener(cV,this.__Sq,this);
eK.setToolTipText(dh);
this.__Sj=eK;
this._navPart.add(eK);
{var eB=new qx.ui.toolbar.Button(this.tr(bC),dl);
eB.addListener(cV,this.__Ss,this);
eB.setToolTipText(bq);
eB.setEnabled(false);
if(qx.core.Environment.get(q)==du){eB.exclude();
}this.__Sh=eB;
this._navPart.add(eB);
var ez=new qx.ui.toolbar.Button(this.tr(di),dt);
this._navPart.add(ez);
ez.setToolTipText(this.tr(bK));
ez.addListener(cV,this.__St,this);
var em=new qx.ui.toolbar.Button(this.tr(bX),br);
this._navPart.add(em);
em.setToolTipText(this.tr(x));
em.addListener(cV,this.__Su,this);
};
var eG=new qx.ui.toolbar.Part;
this.__Sl=eG;
eF.add(eG);
{var eC=new qx.ui.menu.Menu;
this.__Sm=eC;
var eJ=new qx.ui.menu.RadioButton(dm);
var eH=new qx.ui.menu.RadioButton(cg);
var eI=new qx.ui.menu.RadioButton(cB);
eJ.setUserData(cS,dd);
eJ.setValue(true);
eH.setUserData(cS,P);
eI.setUserData(cS,I);
var eu=new qx.ui.form.RadioGroup(eJ,eH,eI);
eu.addListener(bQ,this.__SK,this);
eC.add(eJ);
eC.add(eH);
eC.add(eI);
var es=new qx.ui.toolbar.MenuButton(this.tr(L),dy,eC);
es.setToolTipText(bw);
eG.add(es);
};
var ew=new qx.ui.menu.Menu;
{var ey=new qx.ui.menu.Button(this.tr(F));
ey.setCommand(this._cmdObjectSummary);
ew.add(ey);
var eL=new qx.ui.menu.Button(this.tr(co));
eL.setCommand(this._cmdNamespacePollution);
ew.add(eL);
};
var ep=new qx.ui.menu.Button(this.tr(cc));
ep.setCommand(this._cmdDisposeSample);
ew.add(ep);
var eq=new qx.ui.toolbar.MenuButton(this.tr(bi),bg,ew);
eq.setToolTipText(bL);
eG.add(eq);
var eA=new qx.ui.toolbar.Part;
this.__Sk=eA;
eF.addSpacer();
eF.add(eA);
{var eE=new qx.ui.toolbar.RadioButton(z,by);
eE.setToolTipText(p);
var eo=new qx.ui.toolbar.RadioButton(bv,bA);
eo.setToolTipText(dp);
eE.setUserData(cS,bU);
eo.setUserData(cS,cW);
eA.add(eE);
eA.add(eo);
};
var en=new qx.ui.toolbar.RadioButton(V,T);
en.setToolTipText(bj);
en.setUserData(cS,ct);
eA.add(en);
var el=this.viewGroup=new qx.ui.form.RadioGroup;
el.setAllowEmptySelection(true);
el.add(en);
{el.add(eE,eo);
};
return eF;
},__Sy:function(){var eN=new qx.ui.embed.Iframe().set({nativeContextMenu:true});
eN.addListener(A,this.__SE,this);
this._iframe=eN;
return eN;
},__Sz:function(){var eQ=new qx.ui.layout.VBox(0,bf,da);
eQ.setAlignX(cY);
var eS=new qx.ui.container.Composite(eQ);
var eO=new qx.ui.decoration.Background().set({backgroundColor:cD});
eS.setDecorator(eO);
var eR=new qx.ui.form.Button(this.tr(cr),cz);
eR.setAllowGrowX(false);
eR.setMargin(5);
eR.addListener(cV,function(){this.logappender.clear();
},this);
eS.add(eR,{flex:0});
this.f2=new qx.ui.embed.Html();
this.f2.setOverflow(bN,bN);
this.f2.setFont(K);
this.f2.setBackgroundColor(bp);
this.logappender=new qx.log.appender.Element();
qx.log.Logger.unregister(this.logappender);
var eP=document.createElement(bR);
this.logelem=document.createElement(bR);
this.logelem.style.padding=k;
this.logappender.setElement(this.logelem);
eP.appendChild(this.logelem);
this.f2.getContentElement().useElement(eP);
eS.add(this.f2,{flex:1});
return eS;
},__SA:function(){var eT=new qx.ui.embed.Html("<div class='script'>The sample source will be displayed here.</div>");
eT.setOverflow("auto","auto");
eT.setFont("monospace");
eT.setBackgroundColor("white");
this.widgets["outputviews.sourcepage.html.page"]=eT;
eT.getContentElement().setAttribute("id","qx_srcview");
return eT;
},__SB:function(){var eU=new qx.ui.embed.Html("<div class='script'>The sample JS source will be displayed here.</div>");
eU.setOverflow("auto","auto");
eU.setFont("monospace");
eU.setBackgroundColor("white");
this.widgets["outputviews.sourcepage.js.page"]=eU;
eU.getContentElement().setAttribute("id","qx_srcview");
return eU;
},__SC:function(){var eW=new qx.ui.tree.Tree();
...
return eW;
},__SD:function(){var eX=new qx.ui.window.Window(this.tr("Demo Link"),"icon/22/mimetypes/text-html.png");
eX.setLayout(new qx.ui.layout.VBox(10));
eX.setAllowMaximize(false);
eX.setAllowMinimize(false);
var fb=new qx.ui.basic.Label("");
fb.setSelectable(true);
fb.setNativeContextMenu(true);
eX.add(fb);
this._urlWindow=eX;
var fc=Math.ceil((qx.bom.Viewport.getWidth()/2)-250);
...
this.getApplicationRoot().add(eX);
var eY=new qx.ui.menu.Menu();
var fa=new qx.ui.menu.Button(this.tr("Get Demo Link"),"icon/22/mimetypes/text-html.png");
fa.addListener("execute",function(e){var fe=this.tree.getSelection()[0];
var fg=fe.getUserData("modelLink");
var fd=this.tests.handler.getFullName(fg);
fd=fd.replace(".","~");
var ff=location.protocol+"//"+location.host+location.pathname+"#"+fd;
this._urlWindow.getChildren()[0].setValue(ff);
this._urlWindow.open();
},this);
eY.add(fa);
this._tree.setContextMenu(eY);
},treeGetSelection:function(e){var fh=this.tree.getSelection()[0];
var fi=fh.getUserData(bV);
this.tests.selected=this.tests.handler.getFullName(fi);
},leftReloadTree:function(e){this._sampleToTreeNodeMap={};
var fj=this._sampleToTreeNodeMap;
var fp=null;
var fs=null;
var fr;
{fr=/\?autorun=true/.test(location.href);
};
var fo=this._history.getState();
var fm=fo.match(/([^~]+)~/);

if(fm){fp=fm[1];
}else{var fk=fo.match(/([^~][\w]*)/);

if(fk){fp=fk[1];

if(fr){this.setPlayDemos(cM);
}}else{fp=cy;

if(fr){this.setPlayDemos(bT);
}}}function fq(ft,fu){var fy=fu.getChildren();
var t;

for(var i=0;i<fy.length;i++){var fv=fy[i];

if(fv.hasChildren()){t=new qx.ui.tree.TreeFolder(fl.polish(fv.label));
t.setUserData(cs,false);
t.setUserData(bP,fv);
fq(t,t.getUserData(bP));

if(fv.label==fp){fs=t;
t.setOpen(true);
}}else{t=new qx.ui.tree.TreeFile(fl.polish(fv.label));
var fx=fv.pwd().slice(1).join(cQ)+cQ+fv.label;

if(fv.tags){var j,m,fw;
t.setUserData(cN,fv.tags);
}fj[fx]=t;
}ft.add(t);
t.setUserData(bV,fv);
fv.widgetLinkFull=t;
}}var fn=this.tests.handler.ttree;
var fl=this;
this.tree.setUserData(bV,fn);
this.tree.getRoot().setOpen(true);
fq(this.tree.getRoot(),fn);

if(fs!=null){this.tree.setSelection([fs]);
}},runSample:function(e){if(e&&(e.getType()===cV||e.getType()===bS)){if(this.tests.selected===cP){this.setPlayDemos(bT);
}else if(this.tests.selected.indexOf(bU)>0){if(this.__Sn){this.__Sn.stop();
}this.setPlayDemos(cR);
}else{this.setPlayDemos(cM);
}}this._runbutton.setVisibility(cT);
this._stopbutton.setVisibility(cU);

if(this.tests.selected!=cP){var fz=this.tests.selected.replace(cI,cQ);
this.setCurrentSample(fz);
}else{this.playNext();
}},stopSample:function(e){this.setPlayDemos(cR);
this._nextButton.setEnabled(true);
this._prevButton.setEnabled(true);
this._stopbutton.setVisibility(cT);
this._runbutton.setVisibility(cU);
},setCurrentSample:function(fA){if(!fA){return;
}
if(!this._sampleToTreeNodeMap){return;
}var fB;
var fD=this._sampleToTreeNodeMap[fA];

if(fD){fD.getTree().setSelection([fD]);
fB=bI+fA;
{fB+=g+this.__Sb;
};
var fC=fD.getUserData(cN);

if(fC){this.__Sh.setEnabled(!qx.lang.Array.contains(fC,bM));
}}else{fB=this.defaultUrl;
}
if(this._iframe.getSource()==fB){this._iframe.reload();
}else{this.__Sg=false;
this._iframe.setSource(fB);
}if(fB==this.defaultUrl){this.disableMenuButtons();
}else{this.enableMenuButtons();
}this._currentSample=fA;
this._currentSampleUrl=fB;
},__SE:function(){var fH=this._iframe.getWindow();
var fN=this._iframe.getSource();

if(fN!=null&&fN!=this.defaultUrl){var fG;

try{fG=fH.location.href;
}catch(fP){fG=window.location.href;
var fO=fG.lastIndexOf("/");

if(fO!=-1){fG=fG.substring(0,fO+1);
}fG+=fN;
}var fL=fG.indexOf("/demo/")+6;
var fI=fG.indexOf("?");
fI=fI==-1?fG.length:fI;
var fJ=fG.substring(fL,fI).split("/");
var fM=String.fromCharCode(187);

if(fJ.length==2){var fE=fJ[0];
fE=fE.charAt(0).toUpperCase()+fE.substring(1);
var fK=fJ[1].replace(".html","").replace("_"," ");
fK=fK.charAt(0).toUpperCase()+fK.substring(1);
var fF="qooxdoo "+fM+" Demo Browser "+fM+" "+fE+" "+fM+" "+fK;
}else{var fF="qooxdoo "+fM+" Demo Browser "+fM+" Start";
}document.title=fF;
}if(this.getPlayDemos()!="current"){if(!fK){this.playNext();
}else{var self=this;
this.__Sn=qx.event.Timer.once(this.playNext,self,5000);
this._nextButton.setEnabled(false);
this._prevButton.setEnabled(false);
}}else{this._stopbutton.setVisibility("excluded");
this._runbutton.setVisibility("visible");
}},__SF:function(e){var fQ=this._iframe.getWindow();

try{if(fQ&&fQ.qx&&fQ.qx.log&&fQ.qx.log.appender){if(!this.__Sg){this.__Sg=true;
this.debug("Demo loaded: "+this._currentSample);
this.logappender.$$id=null;
this.logappender.clear();

try{fQ.qx.log.Logger.register(this.logappender);
}catch(e){return ;
}this._history.addToHistory(this._currentSample.replace("/","~"),document.title);
if(this._currentSampleUrl!=this.defaultUrl){this.__SG(this._currentSampleUrl);
}}}else{this.__Sg=false;
}}catch(fR){this.__Sg=false;
}},filter:function(fS){var gb=new RegExp(cA+fS+bF,bB);
var fY=this._tree.getRoot().getItems(true,true);
var ga=0;
var fT=0;

for(var i=0;i<fY.length;i++){var gc=fY[i];
var parent=gc.getParent();
var fX=gc.getUserData(cN);
var fU=false;

if(fX!=null){for(var j=0;j<fX.length;j++){fU=!!fX[j].match(gb);

if(fU){break;
}}}
if(gc.getChildren().length==0){fT++;
}
if((fU||(gc.getLabel().search(gb)!=-1)||(parent.getLabel().search(gb)!=-1))){if(gc.getChildren().length==0){ga++;
}gc.show();
gc.getParent().setOpen(true);
gc.getParent().show();
}else{gc.exclude();
}}if(fS==cP){var fW=this._tree.getRoot().getItems(false,true);
var fV=this._tree.getSelection();
for(var i=0;i<fW.length;i++){if(fW[i]==fV[0]||fW[i]==fV[0].getParent()){continue;
}fW[i].setOpen(false);
}}this._status.setValue(ga+cQ+fT);
},__SG:function(gd){if(typeof (gd)!="string"){return;
}var ge=new qx.io.remote.Request(gd);
ge.setTimeout(180000);
ge.setProhibitCaching(false);
ge.addListener("completed",function(gf){var content=gf.getContent();
if(content){{var gi=content.indexOf("<script",content.indexOf("<script")+7);
var go=content.indexOf("src",gi);
var gh=content.indexOf("\"",go+5);
var gj=content.substring(go+5,gh);
var gm=gj.substring(4,gj.length-3)+".src.js";
var u="script/demobrowser.demo";
var gl=gd.split('/');
var gk=gl[1];
var gn=gl[2];
gn=gn.substr(0,gn.indexOf('.html'));
u+="."+gk+"."+gn+".src.js";
gm=u;
var gg=new qx.io.remote.Request(gm);
gg.setTimeout(180000);
gg.setProhibitCaching(false);
gg.addListener("completed",function(gp){var gq=gp.getContent();
this.__Sr(gq);

if(gq){this.widgets["outputviews.sourcepage.js.page"].setHtml(this.__SH(gq,"javascript"));
}},this);
gg.addListener("failed",function(gr){this.error("Couldn't load file: "+gd);
},this);
gg.send();
this.widgets["outputviews.sourcepage.html.page"].setHtml(this.__SH(content));
};
}},this);
ge.addListener("failed",function(gs){this.error("Couldn't load file: "+gd);
},this);
ge.send();
},dataLoader:function(gt){var gu=new qx.io.remote.Request(gt);
gu.setTimeout(180000);
gu.setProhibitCaching(false);
gu.addListener(dA,function(gv){var content=gv.getContent();
if(!content){if(window.location.protocol==cF){alert("Failed to load demo data from the file system.\n\n"+"The security settings of your browser may prohibit AJAX "+"when using the file protocol. Please try the http protocol "+"instead.");
}return;
}var gw=eval(content);
qx.event.Timer.once(function(){this.tests.handler=new demobrowser.TreeDataHandler(gw);
this.leftReloadTree();
var gx=this._history.getState();

if(gx){this.setCurrentSample(gx.replace(db,cQ));
}else{this.setCurrentSample(this.defaultUrl);
}},this,0);
},this);
gu.addListener(bk,function(gy){this.error("Couldn't load file: "+gt);
},this);
gu.send();
},playPrev:function(e){this.setPlayDemos(cR);
var gC=this.tree.getSelection()[0];

if(gC){var gz=this.tree.getPreviousNodeOf(gC,false);

if(!gz||gz==this.tree.getRoot()){return;
}
while(gz.isVisible&&!gz.isVisible()){gz=this.tree.getPreviousNodeOf(gz,false);
}
if(gz.getParent()==this.tree.getRoot()){var gB=this.tree.getPreviousNodeOf(gz,false);

while(gB.isVisible&&!gB.isVisible()){gB=this.tree.getPreviousNodeOf(gB,false);
}
if(gB.getParent()==this.tree.getRoot()){gB.setOpen(true);
var gA=this.tree.getPreviousNodeOf(gz,false);

while(gA.isVisible&&!gA.isVisible()){gA=this.tree.getPreviousNodeOf(gA,false);
}
if(gB!==gA){gz=gA;
}}else{gz=gB;
}}
if(!gz||gz===gC){this._stopbutton.setVisibility(cT);
this._runbutton.setVisibility(cU);
return;
}else{this.tree.setSelection([gz]);
this.runSample();
}}},playNext:function(e){this._nextButton.setEnabled(true);
this._prevButton.setEnabled(true);
var gF=this.tree.getSelection()[0];

if(gF){var gD=this.tree.getNextNodeOf(gF);

if(!gD){this.setPlayDemos(cR);
this._stopbutton.setVisibility(cT);
this._runbutton.setVisibility(cU);
return;
}
if(gD.getParent()==this.tree.getRoot()){if(this.getPlayDemos()==cM){if(gD!=gF&&gD!=gF.getParent()){this.setPlayDemos(cR);
this._stopbutton.setVisibility(cT);
this._runbutton.setVisibility(cU);
return;
}}gD.setOpen(true);
gD=this.tree.getNextNodeOf(gD);
}
if(!gD){this._stopbutton.setVisibility(cT);
this._runbutton.setVisibility(cU);
return;
}
while(!gD.isVisible()){var gE=this.tree.getNextNodeOf(gD);

if(!gE){this._stopbutton.setVisibility(cT);
this._runbutton.setVisibility(cU);
return;
}
if(gE.getParent()==this.tree.getRoot()){gD.setOpen(true);
var gE=this.tree.getNextNodeOf(gE);
}gD=gE;
}
if(gD){this.tree.setSelection([gD]);
this.runSample();
}else{this._stopbutton.setVisibility(cT);
this._runbutton.setVisibility(cU);
}}},__SH:function(gG,gH){var gI=new qx.util.StringBuilder("<pre class='script'>");
var gK=[];
var gL=new qx.util.StringBuilder();
var gM=/^\s*<script\b[^>]*?(?!\bsrc\s*=)[^>]*?>\s*$/i;
var gJ=/^\s*<\/script>\s*$/i;
gG=gG.replace(/\r\n/g,"\n").replace(/\r/g,"\n");
var gK=gG.split('\n');
if(gH=="javascript"){return "<pre ><div class='script'>"+qx.dev.Tokenizer.javaScriptToHtml(gG)+"</div></pre>";
}
for(var i=0;i<gK.length;i++){if(gM.exec(gK[i])){gI.add(this.__SI(qx.bom.String.escape(gL.get()+gK[i])));
gL.clear();
}else if(gJ.exec(gK[i])){var gN=qx.dev.Tokenizer.javaScriptToHtml(gL.get());
gI.add('<div class="script">',gN,'</div>');
gL.clear();
gL.add(gK[i],'\n');
}else{gL.add(gK[i],'\n');
}}gI.add(this.__SI(qx.bom.String.escape(gL.get())),"</pre>");
return gI.get();
},disableMenuButtons:function(){var gO=this.__Sd;

for(var i=0;i<gO.length;i++){gO[i].setEnabled(false);
}},enableMenuButtons:function(){var gP=this.__Sd;

for(var i=0;i<gP.length;i++){gP[i].setEnabled(true);
}},__SI:function(gQ){var gS=gQ;
function gR(gT){var s=new qx.util.StringBuilder(arguments[1],'<span class="html-tag-name">',arguments[2],'</span>');
var gU;
var gV=false;
if(arguments.length-2>3){for(var i=3;i<arguments.length-2;i++){gU=arguments[i];
...
while((r=m.exec(gU))!=null){s.add(' <span class="keyword">',r[1],'</span>=<span class="string">',r[2].replace(/\s*$/,""),'</span>');
}}}s.add((gV?"/":""));
}s.add('&gt;');
return s.get();
}gS=gS.replace(/(&lt;\/?)([a-zA-Z]+)(.*?)(\/?)&gt;/g,gR);
return gS;
},polish:function(gW){return gW.replace(bx,cP).replace(cj,bG);
},__SJ:function(){var w=this._iframe.getWindow();
var gX;

if(w.qx&&w.qx.log&&w.qx.log.Logger){gX=w.qx.log.Logger;
gX.register(this.logappender);
gX.clear();
gX.unregister(this.logappender);
}},__SK:function(e){this.__Sb=e.getData()[0].getUserData("value");
this.runSample();
},_createHeader:function(){var hb=new qx.ui.layout.HBox();
var gY=new qx.ui.container.Composite(hb);
gY.setAppearance(bW);
var hc=new qx.ui.basic.Label(D);
var ha=new qx.ui.basic.Label(ck+qx.core.Environment.get(cO));
gY.add(hc);
gY.add(new qx.ui.core.Spacer,{flex:1});
gY.add(ha);
return gY;
}},destruct:function(){this.widgets=this.tests=this._sampleToTreeNodeMap=this.tree=this.logelem=null;
this._disposeObjects(c,dn,cJ,l,bh,bo,bs,dv,O,ci,dq,cK,cE,bE,X,ds,h,N,bu,cH,ce,bd,cl,dB,n,bl,dx,G,M,dk,cn,bJ,dz,ba,S,cq);
}});
})();
...
this.addListener(g,this.__SM,this);
},properties:{content:{apply:d},autoCenter:{check:e,init:false}},members:{__SL:null,_getContentContainer:function(){if(!this.__SL){this.__SL=new qx.ui.container.Composite(new qx.ui.layout.VBox());
this.__SL.setMinHeight(20);
}return this.__SL;
...
y=y>=0?y:0;
this.moveTo(x,y);
},__SM:function(){if(this.getAutoCenter()){this.center();
}}}});
})();
...
this.tmap=n;
this.ttree=this.__SN(n);
},members:{__SN:function(o){var q=o;
