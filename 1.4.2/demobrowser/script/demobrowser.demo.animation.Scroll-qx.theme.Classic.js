...
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:d,check:[d,b]},x:{init:0,check:c},y:{init:0,check:c}},members:{__Pp:null,__Pq:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var e=this._getElement();
this.__Pp={x:e.scrollLeft,y:e.scrollTop};
if(this._atEndPosition(this.__Pp.x,this.__Pp.y)){return;
}
if(this.getMode()==b){this.__Pq={left:this.getX()-this.__Pp.x,top:this.getY()-this.__Pp.y};
}},update:function(f){qx.fx.Base.prototype.update.call(this);
var g=this._getElement();

if(this.getMode()==d){if(this.getX()!=0){g.scrollLeft=this.__Pp.x+(this.getX()*f);
}
if(this.getY()!=0){g.scrollTop=this.__Pp.y+(this.getY()*f);
}}else{g.scrollLeft=this.__Pp.x+(this.__Pq.left*f);
g.scrollTop=this.__Pp.y+(this.__Pq.top*f);
