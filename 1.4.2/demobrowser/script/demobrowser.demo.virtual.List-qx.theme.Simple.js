...
qx.Class.define(v,{extend:qx.application.Standalone,properties:{showMode:{init:m,check:[p,n,m],event:bf}},members:{__VA:null,__VB:null,__VC:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var bJ=this.getRoot();
...
var bR=this.__VA=new qx.ui.list.List(ca).set({scrollbarX:"on",selectionMode:"multi",height:280,width:150,labelPath:"label",iconPath:"icon",iconOptions:{converter:function(ce){return "icon/"+ce+"/places/folder.png";
}}});
bM.add(bR,{top:20});
var bQ=this;
...
var bT=this.__VB=new qx.ui.form.CheckBox("Enable drag selection");
var bX=this.__VC=new qx.ui.form.CheckBox("Enable quick selection").set({enabled:false});
...
this.__VA.setSelectionMode(cx);

if(cx==k||cx==o){this.__VB.setEnabled(false);
this.__VC.setEnabled(true);
}else if(cx==l||cx==N){this.__VB.setEnabled(true);
this.__VC.setEnabled(false);
}},onShowModeChange:function(e){var cy=e.getData()[0].getUserData(q);
this.setShowMode(cy);
},onDragCheckChange:function(e){if(e.getData()){var cz=this.__VA.getSelectionMode();

if(cz==k||cz==o){this.debug("Drag selection is only available for the modes multi or additive");
}}this.__VA.setDragSelection(e.getData());
},onQuickCheckChange:function(e){if(e.getData()){var cA=this.__VA.getSelectionMode();

if(cA==l||cA==j){this.debug("Quick selection is only available for the modes multi or additive");
}}this.__VA.setQuickSelection(e.getData());
}}});
})();
