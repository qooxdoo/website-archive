...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Remote_Model","qx.theme":"qx.theme.Classic"};
...
this.__Ov=null;
this.__Ow=null;
},properties:{blockSize:{check:c,init:50},maxCachedBlockCount:{check:c,init:15},clearCacheOnRemove:{check:b,init:false},blockConcurrentLoadRowCount:{check:b,init:true}},members:{__On:null,__Os:null,__Oo:null,__Op:null,__Oq:null,__Or:null,__Ot:null,__Ou:null,__Ol:null,__Om:null,__Ow:null,__Ov:null,__Ox:false,_getIgnoreCurrentRequest:function(){return this.__Os;
},getRowCount:function(){if(this.__On==-1){if(!this.__Ox||!this.getBlockConcurrentLoadRowCount()){this.__Ox=true;
this._loadRowCount();
}return (this.__On==-1)?0:this.__On;
}else{return this.__On;
}},_loadRowCount:function(){throw new Error("_loadRowCount is abstract");
},_onRowCountLoaded:function(f){if(this.getBlockConcurrentLoadRowCount()){this.__Ox=false;
}if(f==null||f<0){f=0;
}this.__On=Number(f);
...
if(this.__Op!=-1){var h=this._cancelCurrentRequest();
...
this.__Ox=true;
this._loadRowCount();
},clearCache:function(){this.__Ot={};
this.__Ou=0;
},getCacheContent:function(){return {sortColumnIndex:this.__Ol,sortAscending:this.__Om,rowCount:this.__On,lruCounter:this.__Oo,rowBlockCache:this.__Ot,rowBlockCount:this.__Ou};
},restoreCacheContent:function(j){if(this.__Op!=-1){var k=this._cancelCurrentRequest();

if(k){this.__Op=-1;
this.__Os=false;
}else{this.__Os=true;
}}this.__Ol=j.sortColumnIndex;
this.__Om=j.sortAscending;
this.__On=j.rowCount;
this.__Oo=j.lruCounter;
this.__Ot=j.rowBlockCache;
this.__Ou=j.rowBlockCount;
var l={firstRow:0,lastRow:this.__On-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
for(var w=0;w<=o;w++){var q=this.__Ot[w];
...
if(r!=null){u[t]=r;
}}}}},prefetchRows:function(x,y){if(this.__Op==-1){var z=this.getBlockSize();
var E=Math.ceil(this.__On/z);
...
for(var F=D;F<=C;F++){if(this.__Ot[F]==null||this.__Ot[F].isDirty){if(B==-1){B=F;
}A=F;
}}if(B!=-1){this.__Oq=-1;
this.__Or=-1;
this.__Op=B;
...
},_onRowDataLoaded:function(I){if(I!=null&&!this.__Os){var L=this.getBlockSize();
var J=Math.ceil(I.length/L);

if(J==1){this._setRowBlockData(this.__Op,I);
}else{for(var i=0;i<J;i++){var O=i*L;
var N=[];
var K=Math.min(L,I.length-O);

for(var P=0;P<K;P++){N.push(I[O+P]);
}this._setRowBlockData(this.__Op+i,N);
}}var M={firstRow:this.__Op*L,lastRow:(this.__Op+J+1)*L-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(e,M);
}this.__Op=-1;
this.__Os=false;
if(this.__Oq!=-1){this.prefetchRows(this.__Oq,this.__Or);
}},_setRowBlockData:function(Q,R){if(this.__Ot[Q]==null){this.__Ou++;

while(this.__Ou>this.getMaxCachedBlockCount()){var V;
var U=this.__Oo;

for(var T in this.__Ot){var S=this.__Ot[T].lru;

if(S<U&&T>1){U=S;
V=T;
}}delete this.__Ot[V];
this.__Ou--;
}}this.__Ot[Q]={lru:++this.__Oo,rowDataArr:R};
...
for(var bf=bc;bf<=bb;bf++){var X=this.__Ot[bf];
...
if(bf==bb-1){if(X.rowDataArr.length==0){delete this.__Ot[bf];
}}else{var bd=this.__Ot[bf+1];

if(bd!=null){X.rowDataArr.push(bd.rowDataArr[0]);
}else{X.isDirty=true;
}}}}
if(this.__On!=-1){this.__On--;
...
if(bi.lru!=this.__Oo){bi.lru=++this.__Oo;
}return bj;
...
}}},setEditable:function(bv){this.__Ow=[];

for(var bw=0;bw<this.getColumnCount();bw++){this.__Ow[bw]=bv;
}this.fireEvent(d);
},setColumnEditable:function(bx,by){if(by!=this.isColumnEditable(bx)){if(this.__Ow==null){this.__Ow=[];
}this.__Ow[bx]=by;
this.fireEvent(d);
}},isColumnEditable:function(bz){return (this.__Ow?(this.__Ow[bz]==true):false);
},setColumnSortable:function(bA,bB){if(bB!=this.isColumnSortable(bA)){if(this.__Ov==null){this.__Ov=[];
}this.__Ov[bA]=bB;
this.fireEvent(d);
}},isColumnSortable:function(bC){return (this.__Ov?(this.__Ov[bC]!==false):true);
},sortByColumn:function(bD,bE){if(this.__Ol!=bD||this.__Om!=bE){this.__Ol=bD;
this.__Om=bE;
this.clearCache();
this.fireEvent(d);
}},getSortColumnIndex:function(){return this.__Ol;
},isSortAscending:function(){return this.__Om;
},setSortColumnIndexWithoutSortingData:function(bF){this.__Ol=bF;
},setSortAscendingWithoutSortingData:function(bG){this.__Om=bG;
}},destruct:function(){this.__Ov=this.__Ow=this.__Ot=null;
}});
})();
(function(){var f="text",d="Text",c='demobrowser.demo.table.RemoteTableModel',b="Id",a="id";
qx.Class.define(c,{extend:qx.ui.table.model.Remote,construct:function(){qx.ui.table.model.Remote.call(this);
this.setColumns([b,d],[a,f]);
},members:{__Uz:null,__UA:false,_loadRowCount:function(){this.__UB();

if(this.__UA){return;
}
if(this.__Uz){this.__UC();
}else{this.__UF();
}},_loadRowData:function(g,h){this.__UB();

if(this.__UA){return;
}
if(this.__Uz){this.__UD(g,h);
}else{this.__UG(g,h);
}},__UB:function(){if(this.__UA||this.__Uz!==null){return;
}this.__UA=true;
this.__UE("",function(j){this.__UA=false;
this.__Uz=(j=="WTF PHP");
this._loadRowCount();
});
},__UC:function(){var k="method=getRowCount";
this.__UE(k,function(l){this._onRowCountLoaded(parseInt(l));
});
},__UD:function(m,n){var o="method=getRowData&start="+m+"&end="+n;
this.__UE(o,function(p){this._onRowDataLoaded(qx.util.Json.parse(p));
});
},__UE:function(q,r){var s=new qx.io.HttpRequest("../../resource/demobrowser/backend/remote_table.php?"+q);
s.addListener("change",function(e){if(e.getData()==4){r.call(this,s.getResponseText());
}},this);
s.send();
},__UF:function(){var self=this;
window.setTimeout(function(){self._onRowCountLoaded(1000000);
},0);
},__UG:function(t,u){var self=this;
window.setTimeout(function(){var v=[];

for(var i=t;i<=u;i++){v.push({id:i,text:'Hello '+i+' Generated on:'+(new Date())});
}self._onRowDataLoaded(v);
},0);
}}});
})();
