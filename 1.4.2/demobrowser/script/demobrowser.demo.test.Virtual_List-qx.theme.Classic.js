...
this.__Vb=new qx.ui.virtual.layer.WidgetCell(this);
this.getPane().addLayer(this.__Vb);
this.getPane().addListener(z,this._onResize,this);
this.__Vc=[];
this.__Vd=[];
this.__Ve=new qx.ui.virtual.selection.Row(this.getPane());
this.__Ve.addListener(h,this._onChangeSelection,this);
this.__Ve.attachMouseEvents();
this.__Ve.attachKeyEvents(this);
this.__Ve.attachListEvents(this);
...
this.fireDataEvent(h,this.__Vi(e.getData()));
},getCellWidget:function(I,J){var L=this.__Vc[I];

if(!L){return null;
}var K=this.__Vd.pop()||new qx.ui.form.ListItem();
K.set({label:L.getLabel(),icon:L.getIcon()});

if(this.__Ve.isItemSelected(I)){K.addState(n);
}else{K.removeState(n);
}K.setUserData(g,I);
return K;
},poolCellWidget:function(M){this.__Vd.push(M);
...
},getChildren:function(){return this.__Vc;
},hasChildren:function(){return this.__Vc.length>0;
...
qx.ui.core.queue.Widget.add(this);
},add:function(W){this.__Vc.push(W);
this._addHelper(W);
return this;
},remove:function(X){qx.lang.Array.remove(this.__Vc,X);
this._removeHelper(X);
return this;
},removeAll:function(){for(var i=0,j=this.__Vc.length;i<j;i++){this._removeHelper(this.__Vc[i]);
}this.__Vc=[];
},indexOf:function(Y){return this.__Vc.indexOf(Y);
},addAt:function(ba,bb){qx.lang.Array.insertAt(this.__Vc,ba,bb);
this._addHelper(ba);
},addBefore:function(bc,bd){qx.lang.Array.insertBefore(this.__Vc,bc,bd);
this._addHelper(bc);
},addAfter:function(be,bf){qx.lang.Array.insertAfter(this.__Vc,be,bf);
this._addHelper(be);
},removeAt:function(bg){this.__Vc.splice(bg,1);
this._removeHelper(this.__Vc[bg]);
},__Vf:function(bh){var bi=this.__Vc.indexOf(bh);

if(bi<0){return null;
}else{return bi;
}},__Vg:function(bj){var bk=this.__Vc[bj];
return bk;
},__Vh:function(bl){var bm=[];

for(var i=0;i<bl.length;i++){bm.push(this.__Vf(bl[i]));
}return bm;
},__Vi:function(bn){var bo=[];

for(var i=0;i<bn.length;i++){bo.push(this.__Vc[bn[i]]);
}return bo;
},selectAll:function(){this.__Ve.selectAll();
},setSelected:function(bp){this.__Ve.selectItem(this.__Vf(bp));
},isSelected:function(bq){return this.__Ve.isItemSelected(this.__Vf(bq));
},addToSelection:function(br){this.__Ve.addItem(this.__Vf(br));
},removeFromSelection:function(bs){this.__Ve.removeItem(this.__Vf(bs));
},selectRange:function(bt,bu){this.__Ve.selectItemRange(this.__Vf(bt),this.__Vf(bu));
},resetSelection:function(){this.__Ve.clearSelection();
},setSelection:function(bv){this.__Ve.replaceSelection(this.__Vh(bv));
},getSelectedItem:function(){return this.__Vc[this.getSelected()];
},getSelected:function(){return this.__Vc[this.__Ve.getSelectedItem()];
},getSelection:function(){return this.__Vi(this.__Ve.getSelection());
},getSortedSelection:function(){return this.__Vi(this.__Ve.getSortedSelection());
},isSelectionEmpty:function(){return this.__Ve.isSelectionEmpty();
},getSelectionContext:function(){return this.__Ve.getSelectionContext();
},getSelectables:function(){return this.__Vi(this.__Ve.getSelectables());
},_applySelectionMode:function(bw,bx){this.__Ve.setMode(bw);
},_applyDragSelection:function(by,bz){this.__Ve.setDrag(by);
},_applyQuickSelection:function(bA,bB){this.__Ve.setQuick(bA);
}}});
})();
