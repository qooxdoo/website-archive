...
(function(){var j="String",h="string",g="Number",f="__Pd",d="__Pc",c="qx.util.fsm.FiniteStateMachine",b="object";
...
if(!this.__Ph[v]){this.__Ph[v]={};
}this.__Ph[v][r]=true;
if(!this.__Pi[r]){this.__Pi[r]=[];
}this.__Pi[r]=this.__Pi[r].concat(u);
}},removeObject:function(x){var z=this.__Pf[x];
if(this.__Pi[x]){for(var y in this.__Pi[x]){delete this.__Ph[y];
}delete this.__Pi[x];
}delete this.__Pg[z];
delete this.__Pf[x];
delete this.__Pe[x];
},getObject:function(A){return this.__Pe[A];
},getFriendlyName:function(B){var C=B.toHashCode();
return C?this.__Pg[C]:null;
},getGroupObjects:function(D){var a=[];

for(var name in this.__Ph[D]){a.push(name);
}return a;
},displayAllObjects:function(){for(var E in this.__Pf){var G=this.__Pf[E];
var F=this.getObject(E);
this.debug(E+" => "+G);
this.debug("  "+G+" => "+this.__Pg[G]);
this.debug("  "+E+" => "+this.getObject(E));
this.debug("  "+this.getObject(E)+" => "+this.getFriendlyName(F));
}},start:function(){var I=this.__Pa;
...
}this.__OY[I].getAutoActionsBeforeOnentry()(this);
if(H){this.debug(this.getName()+"#"+I+"#entry");
}this.__OY[I].getOnentry()(this,null);
if(H){this.debug(this.getName()+"#"+I+"#actionsAfterOnentry");
}this.__OY[I].getAutoActionsAfterOnentry()(this);
},pushState:function(J){if(this.__Pb.length>=this.getMaxSavedStates()){throw new Error("Saved-state stack is full");
}
if(J===true){this.__Pb.push(this.getState());
}else if(J){this.__Pb.push(J);
}else{this.__Pb.push(this.getPreviousState());
}},postponeEvent:function(event){this.__Pd.unshift(event);
},enqueueEvent:function(event,K){if(K){this.__Pc.push(event);
}else{this.__Pc.unshift(event);
}
...
},__Pk:function(){if(this.__Pj){return ;
}this.__Pj=true;
while(this.__Pc.length>0){var event=this.__Pc.pop();
var S=this.__Pl(event);
if(S){event.dispose();
}}this.__Pj=false;
},__Pl:function(event){var V;
var X;
var U;
var bg;
var be;
...
if(typeof (X)==h){if(!X in this.__OY){throw new Error("Attempt to transition to nonexistent state "+X);
}this.setNextState(X);
}else{switch(X){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:X=V;
this.setNextState(X);
break;
case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Pb.length==0){throw new Error("Attempt to transition to POP_STATE_STACK "+"while state stack is empty.");
}X=this.__Pb.pop();
this.setNextState(X);
break;
...
}bg=this.__OY[this.getNextState()];
...
for(var i=0;i<this.__Pd.length;i++){e=this.__Pd.pop();
this.__Pc.unshift(e);
}
...
(function(){var p="object",o="string",n='"',m="}",l="{",k="(",h=";",d="function",c="  var groupObjects = ",b="parameters",X=")",W=" fsm.getObject('",V="groups",U="  for (var i = 0; i < groupObjects.length; i++)",T="__OS",S="__OQ",R="autoActionsAfterOnentry",Q="__OT",P="__OR",O="fsm",w="');",x="__OV",u="__OX",v="autoActionsBeforeOnexit",s="autoActionsBeforeOnentry",t="onexit",q="autoActionsAfterOnexit",r="  fsm.debug(ex);",y="    fsm.getObject(objName).",z=")(fsm)",G="events",E="    var objName = groupObjects[i];",I="').",H="objects",K="catch(ex)",J="  {",B="__OU",N="    fsm.getGroupObjects('",M="  }",L="__OW",A="qx.util.fsm.State",C="onentry",D="try",F=",";
...
break;
case t:this.setOnexit(ba[bb]);
break;
case s:this.setAutoActionsBeforeOnentry(ba[bb]);
break;
case R:this.setAutoActionsAfterOnentry(ba[bb]);
break;
case v:this.setAutoActionsBeforeOnexit(ba[bb]);
break;
case q:this.setAutoActionsAfterOnexit(ba[bb]);
break;
...
for(j=0;j<g.length;j++){bj+=c+N+g[j]+w+U+J+E+y+bi+h+M;
}}}}bj+=m+K+l+r+m;
return new Function(O,bj);
}},properties:{name:{transform:S,nullable:true},onentry:{transform:P,nullable:true,init:function(bk,event){}},onexit:{transform:T,nullable:true,init:function(bl,event){}},autoActionsBeforeOnentry:{transform:B,nullable:true,init:function(bm,event){}},autoActionsAfterOnentry:{transform:x,nullable:true,init:function(bn,event){}},autoActionsBeforeOnexit:{transform:L,nullable:true,init:function(bo,event){}},autoActionsAfterOnexit:{transform:u,nullable:true,init:function(bp,event){}},events:{transform:Q,nullable:true}},members:{__OQ:function(bq){if(typeof (bq)!="string"||bq.length<1){throw new Error("Invalid state name");
}return bq;
},__OR:function(br){switch(typeof (br)){case "undefined":return function(bs,event){};
case "function":return br;
default:throw new Error("Invalid onentry type: "+typeof (br));
}},__OS:function(bt){switch(typeof (bt)){case "undefined":return function(bu,event){};
case "function":return bt;
default:throw new Error("Invalid onexit type: "+typeof (bt));
return null;
}},__OT:function(bv){if(typeof (bv)!="object"){throw new Error("events must be an object");
...
},__OU:function(by){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnentry",by);
},__OV:function(bz){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnentry",bz);
},__OW:function(bA){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnexit",bA);
},__OX:function(bB){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnexit",bB);
...
(function(){var o="function",n="undefined",m="predicate",l="nextState",k="autoActionsBeforeOntransition",j="number",i="__Po",h="boolean",g="string",f="autoActionsAfterOntransition",c="__Pn",e="qx.util.fsm.Transition",d="ontransition",b="String",a="__Pm";
qx.Class.define(e,{extend:qx.core.Object,construct:function(p,q){qx.core.Object.call(this);
this.setName(p);
for(var r in q){switch(r){case m:this.setPredicate(q[r]);
break;
case l:this.setNextState(q[r]);
break;
case k:this.setAutoActionsBeforeOntransition(q[r]);
break;
case f:this.setAutoActionsAfterOntransition(q[r]);
break;
...
},transform:a},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:c},autoActionsBeforeOntransition:{init:function(t,event){}},autoActionsAfterOntransition:{init:function(u,event){}},ontransition:{init:function(v,event){},transform:i}},members:{__Pm:function(w){switch(typeof (w)){case n:return function(x,event){return true;
};
case h:return function(y,event){return w;
};
case o:return w;
default:throw new Error("Invalid transition predicate type: "+typeof (w));
break;
}},__Pn:function(z){switch(typeof (z)){case g:return z;
case j:switch(z){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return z;
...
break;
}},__Po:function(A){switch(typeof (A)){case n:return function(B,event){};
case o:return A;
