...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Virtual_ListBinding","qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.test.Virtual_ListBinding-qx.theme.Simple.js"]],
  urisBefore : [],
  packageHashes : {"0":"48a88351b435"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var l="row-layer",k="selected",j="grid-lines",h="Boolean",g="Integer",f="cell.row",d="cell.renderer",c="_applyDelegate",b="qx.ui.virtual.form.List",a="__MN",A="Object",z="_applyCellRenderer",y="changeRowHeight",x="_applyRowCount",w="changeCellRenderer",v="_applyShowGridLines",u="resize",t="changeShowGridLines",s="changeDelegate",r="_applyUseWidgetCells",p="changeRowCount",q="_applyRowHeight",n="__MM",o="horizontal",m="virtual-list";
...
this.__MN=new qx.ui.virtual.layer.WidgetCell(B);
this.getPane().addLayer(this.__MN);

if(!this.__ML){this.setCellRenderer(qx.ui.virtual.form.ListItemCell.getInstance());
}},_initHtmlLayer:function(){var self=this;
var K={getCellProperties:function(L,M){var N={};

if(self.__MM.isItemSelected(L)){N.selected=true;
}return self._getCellRenderer(L).getCellProperties(self._getCellData(L),N);
}};
this._showChildControl(l);
this.__MN=new qx.ui.virtual.layer.HtmlCell(K);
this.getPane().addLayer(this.__MN);

if(!this.__ML){this.setCellRenderer(new qx.ui.virtual.cell.Cell());
...
},styleSelectable:function(Q,R,S){if(self.__MO){self._styleWidgetSelectable(Q,R,S);
}else{self._styleHtmlSelectable(Q,R,S);
}}};
this.__MM=new qx.ui.virtual.selection.Row(this.getPane(),O);
this.__MM.attachMouseEvents(this.getPane());
this.__MM.attachKeyEvents(this);
},getSelectionManager:function(){return this.__MM;
...
},update:function(){if(this.__MN){this.__MN.updateLayerData();
...
},_applyUseWidgetCells:function(bf,bg){if(this.__MO!==null){throw new Error("The property 'useWidgetCells' cannot be set after the list has "+"been rendered.");
}},_applyCellRenderer:function(bh,bi){this.__ML=bh;

if(this.__MN){this.__MN.fullUpdate();
}},_getCellData:function(bj){return this._delegate.getCellData?this._delegate.getCellData(bj):null;
},_getCellRenderer:function(bk){return this._delegate.getCellRenderer?this._delegate.getCellRenderer(bk):this.__ML;
...
}}},syncWidget:function(){if(this.__MO!==null){return;
}this.__MO=this.getUseWidgetCells();

if(this.__MO){this._initWidgetLayer();
}else{this._initHtmlLayer();
}},_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);
}},destruct:function(){this._delegate=this.__ML=null;
this._disposeObjects(n,a);
}});
})();
...
if(this.__NY.length==0){return;
}var E=this._getDelegate(w);

if(E!=null){this.__NY.sort(function(a,b){return E(D.getItem(a),D.getItem(b));
});
}},_runDelegateFilter:function(F){if(typeof F!==k){return;
}var G=this._getDelegate(r);

for(var i=0,l=F.length;i<l;++i){if(G==null||G(F.getItem(i))){this.__NY.push(i);
...
},__Oh:function(N){return this.__NY[N];
},_applyDelegate:function(O,P){if(this.getTarget()==null||this.getModel()==null){return;
}
if(this._containsDelegateMethod(O)){this.__Og();
}},_applyTarget:function(Q,R){if(Q!=null){Q.setDelegate(this);
this.__Od=Q.getSelectionManager().addListener(u,this._onChangeSelectionView,this);
}
if(R!=null){R.setDelegate(null);
R.getSelectionManager().removeListenerById(this.__Od);
}
if(this.getModel()==null){return;
}this.__Og();
this._syncRowCount();
},_applyModel:function(S,T){if(T!=null){T.removeListenerById(this.__Oa);
T.removeListenerById(this.__Ob);
T.removeListenerById(this.__Oc);
}
if(S!=null){this.__Og();
this.__Oa=S.addListener(t,this._onChangeLengthModel,this);
this.__Ob=S.addListener(v,this._onChangeModel,this);
this.__Oc=S.addListener(h,this._onChangeBubbleModel,this);
}
if(this.getTarget()!=null){this._syncRowCount();
}},_applySelection:function(U,V){if(U!=null){this.__Oe=U.addListener(v,this._onChangeSelectionModel,this);
this.__Of=U.addListener(t,this._onChangeSelectionModel,this);
this._syncModelSelectionToView();
}
if(V!=null){V.removeListenerById(this.__Oe);
V.removeListenerById(this.__Of);
...
},_onChangeLengthModel:function(e){this.__Og();
this._syncRowCount();
