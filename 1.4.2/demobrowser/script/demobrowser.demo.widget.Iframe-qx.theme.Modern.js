...
},properties:{appearance:{refine:true,init:i},scrollbarX:{check:[t,p,d],init:t,themeable:true,apply:l},scrollbarY:{check:[t,p,d],init:t,themeable:true,apply:l},scrollbar:{group:[g,f]}},members:{__JK:null,__JL:null,_getIframeElement:function(){return this.getChildControl(r).getContentElement();
...
e.stop();
},_startIframeObserver:function(){if(this.__JL){this._stopIframeObserver();
}var F=qx.event.Idle.getInstance();
this.__JL=F.addListener("interval",this._onIframeObserverInterval,this);
},_stopIframeObserver:function(){this.__JK=null;

if(!this.__JL){return;
}var G=qx.event.Idle.getInstance();
G.removeListenerById(this.__JL);
},_onIframeObserverInterval:function(){var H=this._getIframeSize();

if(!H){this._disableScollbars();
return;
}
if(this.__JK&&H.width==this.__JK.width&&H.height==this.__JK.height){return;
}this.__JK=H;
...
}},_updateScrollbars:function(){var K=this.__JK;
