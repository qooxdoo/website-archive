...
var settings = {"qx.application":"mobileshowcase.Application","qx.revision":"28147","qx.theme":"qx.theme.Modern","qx.version":"1.4.2"};
...
var envinfo = {"qx.application":"mobileshowcase.Application","qx.client":"webkit","qx.debug":false,"qx.mobile.emulatetouch":true,"qx.mobile.nativescroll":false,"qx.revision":"28147","qx.theme":"qx.theme.Modern","qx.version":"1.4.2"};
for (var k in envinfo) qx.$$environment[k] = envinfo[k];

if (!qx.$$libraries) qx.$$libraries = {};
var libinfo = {"__out__":{"sourceUri":"script"},"mobileshowcase":{"resourceUri":"resource","sourceUri":"script","version":"trunk"},"qx":{"resourceUri":"resource","sourceUri":"script","version":"1.4.2"}};
...
{this.__DV.setShowScrollbarY(false);
};
...
this.__fp=s;
this.__fq=s.getWindow();
this.__fr=this.__fq.document;
this._initTouchObserver();
this._initMouseObserver();
},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{touchstart:1,touchmove:1,touchend:1,touchcancel:1,tap:1,swipe:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE+qx.event.IEventHandler.TARGET_DOCUMENT,IGNORE_CAN_HANDLE:true,MOUSE_TO_TOUCH_MAPPING:{"mousedown":"touchstart","mousemove":"touchmove","mouseup":"touchend"},SWIPE_DIRECTION:{x:["left","right"],y:["up","down"]},TAP_MAX_DISTANCE:qx.core.Environment.get("os.name")!="android"?10:40,SWIPE_MIN_DISTANCE:qx.core.Environment.get("os.name")!="android"?11:41,SWIPE_MIN_VELOCITY:0},members:{__fs:null,__ft:null,__fp:null,__fq:null,__fr:null,__fu:null,__fv:null,__fw:null,__fx:null,__fy:false,__fz:null,canHandleEvent:function(t,u){},registerEvent:function(v,w,x){},unregisterEvent:function(y,z,A){},__fA:function(B){var C=qx.bom.Event.getTarget(B);
if((qx.core.Environment.get(m)==b)){if(C&&C.nodeType==3){C=C.parentNode;
}}return C;
},__fB:function(D,E,F,G){if(!F){F=this.__fA(D);
}var E=E||D.type;

if(F&&F.nodeType){qx.event.Registration.fireEvent(F,E,G||qx.event.type.Touch,[D,F,null,true,true]);
}qx.event.Registration.fireEvent(this.__fq,h,qx.event.type.Data,[E]);
},__fC:function(H,I,J){if(!J){J=this.__fA(H);
}var I=I||H.type;

if(I==q){this.__fD(H,J);
}else if(I==p){this.__fE(H,J);
}else if(I==r){this.__fF(H,J);
}},__fD:function(K,L){var M=K.changedTouches[0];
this.__fu=M.screenX;
this.__fv=M.screenY;
this.__fw=new Date().getTime();
this.__fx=K.changedTouches.length===1;
},__fE:function(N,O){if(this.__fx&&N.changedTouches.length>1){this.__fx=false;
}},__fF:function(P,Q){if(this.__fx){var R=P.changedTouches[0];
var T={x:R.screenX-this.__fu,y:R.screenY-this.__fv};
var U=qx.event.handler.Touch;

if(this.__fz==Q&&Math.abs(T.x)<=U.TAP_MAX_DISTANCE&&Math.abs(T.y)<=U.TAP_MAX_DISTANCE){this.__fB(P,a,Q,qx.event.type.Tap);
}else{var S=this.__fG(P,Q,T);

if(S){P.swipe=S;
this.__fB(P,g,Q,qx.event.type.Swipe);
}}}},__fG:function(V,W,X){var bc=qx.event.handler.Touch;
var bd=new Date().getTime()-this.__fw;
...
if(Math.abs(be)>=bc.SWIPE_MIN_VELOCITY&&Math.abs(Y)>=bc.SWIPE_MIN_DISTANCE){bb={startTime:this.__fw,duration:bd,axis:bf,direction:ba,distance:Y,velocity:be};
}return bb;
},__fH:function(bg){var bh=bg.type;
var bj=qx.event.handler.Touch.MOUSE_TO_TOUCH_MAPPING;

if(bj[bh]){bh=bj[bh];
if(bh==q&&this.__fI(bg)){this.__fy=true;
}else if(bh==r){this.__fy=false;
}var bk=this.__fJ(bg);
var bi=(bh==r?[]:[bk]);
bg.touches=bi;
bg.targetTouches=bi;
...
},__fI:function(bl){if((qx.core.Environment.get(m)==i)){var bm=1;
}else{var bm=0;
}return bl.button==bm;
},__fJ:function(bn){var bo=this.__fA(bn);
...
Event.addNativeListener(this.__fr,p,this.__fs);
Event.addNativeListener(this.__fr,r,this.__fs);
Event.addNativeListener(this.__fr,l,this.__fs);
},_initMouseObserver:function(){if(!qx.core.Environment.get(o)){this.__ft=qx.lang.Function.listener(this._onMouseEvent,this);
var Event=qx.bom.Event;
Event.addNativeListener(this.__fr,j,this.__ft);
Event.addNativeListener(this.__fr,n,this.__ft);
...
Event.removeNativeListener(this.__fr,p,this.__fs);
Event.removeNativeListener(this.__fr,r,this.__fs);
Event.removeNativeListener(this.__fr,l,this.__fs);
},_stopMouseObserver:function(){if(!qx.core.Environment.get(o)){var Event=qx.bom.Event;
Event.removeNativeListener(this.__fr,j,this.__ft);
Event.removeNativeListener(this.__fr,n,this.__ft);
...
}),_onMouseEvent:qx.event.GlobalError.observeMethod(function(bq){if(!qx.core.Environment.get(o)){if(bq.type==n&&!this.__fy){return;
}var br=this.__fH(bq);
this._commonTouchEventHandler(bq,br);
}}),_commonTouchEventHandler:function(bs,bt){var bt=bt||bs.type;

if(bt==q){this.__fz=this.__fA(bs);
}this.__fB(bs,bt);
this.__fC(bs,bt);
...
(function(){var j="os.name",h="android",g="phonegap",f="qx.event.type.Data",e="backKeyDown",d="add",c="menuKeyDown",b="qx.ui.mobile.page.manager.Simple",a="remove";
qx.Class.define(b,{extend:qx.core.Object,construct:function(k){qx.core.Object.call(this);
this.__Eu={};
this._setRoot(k||qx.core.Init.getApplication().getRoot());
this.__Ez();
},events:{add:f,remove:f},members:{__Eu:null,__Ev:null,__Ew:null,__Ex:null,__Ey:null,__Ez:function(){if(qx.core.Environment.get(g)&&qx.core.Environment.get(j)==h){this.__Ex=qx.lang.Function.bind(this._onBackButton,this);
...
}},_onBackButton:function(){if(qx.core.Environment.get(g)&&qx.core.Environment.get(j)==h){var l=true;

if(this.__Ev){l=this.__Ev.back();
}
if(l){BackButton.exitApp();
}}},_onMenuButton:function(){if(qx.core.Environment.get(g)&&qx.core.Environment.get(j)==h){if(this.__Ev){this.__Ev.menu();
}}},add:function(m){this.__Eu[m.getId()]=m;
this.fireDataEvent(d,m);
},remove:function(n){var o=this.getPage(n);

if(this.__Ev==o){throw new Error("Current page with the Id "+o.getId()+" can not be removed");
}delete this.__Eu[o.getId()];
this.fireDataEvent(a,o);
},show:function(p){var q=this.__Ev;
...
}this._setCurrentPage(p);
},_removeCurrentPage:function(){this.__Ew.remove(this.__Ev);
},_getRoot:function(){return this.__Ew;
},_setRoot:function(r){this.__Ew=r;
},getCurrentPage:function(){return this.__Ev;
},_setCurrentPage:function(s){this.__Ev=s;
},getPage:function(t){return this.__Eu[t];
},__EB:function(){var u=document.getElementsByTagName("input");

for(var i=0,length=u.length;i<length;i++){u[i].blur();
...
},events:{"initialize":o,"start":o,"stop":o,"pause":o,"resume":o,"back":o,"menu":o},properties:{defaultCssClass:{refine:true,init:h}},statics:{__EL:null,__EM:null,getManager:function(){if(!this.__EL){this.__EL=new qx.ui.mobile.page.Page.__EM();
}return qx.ui.mobile.page.Page.__EL;
},setManagerClass:function(q){qx.ui.mobile.page.Page.__EM=q;
}},members:{__EN:false,_resize:function(){{this._setStyle(j,window.innerHeight+i);
};
...
(function(){var m="delete",l="changeHash",k="put",j="get",h="post",g="any",f="$",e="^",d="([^\/]+)",c="qx.ui.mobile.navigation.Manager",a="__GB",b="singleton";
qx.Class.define(c,{extend:qx.core.Object,type:b,construct:function(){qx.core.Object.call(this);
this.__Gw={},this.__Gx=0;
this.__Gy={};
this.__Gz=[];
this.__GA=null;
this.__GB=new qx.ui.mobile.navigation.Handler(qx.ui.mobile.navigation.Manager.DEFAULT_PATH);
this.__GB.addListener(l,this.__GC,this);
this.__GB.setHash(this.__GB.getLocationHash());
},statics:{DEFAULT_PATH:"/"},members:{__GB:null,__Gw:null,__Gx:null,__Gy:null,__GA:null,__Gz:null,getCurrentGetPath:function(){return this.__GA;
...
},_addRoute:function(C,D,E,F){var G=this.__Gw[C]=this.__Gw[C]||{};
var I=this.__Gx++;
var H=[];
var J=null;
...
this.__Gy[I]=C;
this._executeRoute(C,this.__GA,G[I]);
return I;
},remove:function(L){var N=this.__Gy[L];
var M=this.__Gw[N];
delete M[L];
delete this.__Gy[L];
},__GC:function(O){var P=O.getData();

if(P!=this.__GA){this.executeGet(P,null);
}},executeGet:function(Q,R){this.__GA=Q;
var S=this.__GE(Q);
...
},__GD:function(bd,be){this.debug("Add path "+bd+" to history");
this.__Gz.unshift({path:bd,customData:be});
},__GE:function(bf){var history=this.__Gz;
...
},_getRoutes:function(){return this.__Gw;
}},destruct:function(){this.__GB.removeListener(l,this.__GC,this);
this.__Gz=this.__Gw=this.__Gy=null;
...
this.__GF=qx.lang.Function.bind(this.__GJ,this);
this.__GH();

if(g){this.setDefaultHash(g);
}},properties:{defaultHash:{check:e,init:d},hash:{check:e,event:c,init:null,apply:b}},members:{__GF:null,__GG:null,__GH:function(){qx.bom.Event.addNativeListener(window,"hashchange",this.__GF);
},__GI:function(){qx.bom.Event.removeNativeListener(window,"hashchange",this.__GF);
},__GJ:function(h){this.setHash(this.getLocationHash());
...
},members:{__GK:null,__GL:null,__GM:null,_initialize:function(){qx.ui.mobile.page.NavigationPage.prototype._initialize.call(this);
var o=this.__GK=new qx.ui.mobile.container.Composite(new qx.ui.mobile.layout.VBox().set({alignX:"center",alignY:"middle"}));
o.addCssClass(l);
...
var p=this.__GL=new qx.ui.mobile.basic.Label("Touch / Tap / Swipe this area");
o.add(p);
this.getContent().add(o,{flex:1});
},_onTap:function(q){this.__GL.setValue(this.__GL.getValue()+a);
},_onSwipe:function(r){this.__GL.setValue(this.__GL.getValue()+g);
},_onTouch:function(s){var t=s.getType();

if(t==n){this.__GL.setValue(k);
}this.__GL.setValue(this.__GL.getValue()+f+s.getType());
...
this.add(this.__GN());
},__GN:function(){var h=new qx.ui.mobile.tabbar.TabBar();
var f=new qx.ui.mobile.tabbar.TabButton("1");
h.add(f);
f.setView(this.__GO("View 1"));
var i=new qx.ui.mobile.tabbar.TabButton("2");
i.setView(this.__GO("View 2"));
h.add(i);
var g=new qx.ui.mobile.tabbar.TabButton("3");
g.setView(this.__GO("View 3"));
h.add(g);
var e=new qx.ui.mobile.tabbar.TabButton("4");
e.setView(this.__GO("View 4"));
h.add(e);
return h;
},__GO:function(j){var k=new qx.ui.mobile.basic.Label(j);
...
},members:{__GP:null,__GQ:null,__GR:null,__GS:null,__GT:null,__GU:null,_initialize:function(){qx.ui.mobile.page.NavigationPage.prototype._initialize.call(this);
var o=new qx.ui.mobile.form.Title(m);
this.getContent().add(o);
this.getContent().add(this.__GV());
...
this.getContent().add(o);
this.__GT=new qx.ui.mobile.embed.Html();
this.getContent().add(this.__GT);
},__GV:function(){var p=new qx.ui.mobile.form.Form();
var q=new qx.ui.mobile.form.Row();
this.__GQ=new qx.ui.mobile.form.TextField().set({placeholder:"Username"});
q.add(this.__GQ);
p.add(q);
var q=new qx.ui.mobile.form.Row();
p.add(q);
this.__GP=new qx.ui.mobile.form.PasswordField().set({placeholder:"Password"});
q.add(this.__GP);
var q=new qx.ui.mobile.form.Row();
p.add(q);
this.__GR=new qx.ui.mobile.form.TextArea().set({placeholder:"Some Info"});
q.add(this.__GR);
var q=new qx.ui.mobile.form.Row(new qx.ui.mobile.layout.HBox());
p.add(q);
q.add(new qx.ui.mobile.basic.Label("Save"),{flex:1});
this.__GS=new qx.ui.mobile.form.ToggleButton();
q.add(this.__GS);
var q=new qx.ui.mobile.form.Row(new qx.ui.mobile.layout.HBox());
p.add(q);
this.__GU=new qx.ui.mobile.form.Slider();
q.add(this.__GU,{flex:1});
return p;
},_onButtonTap:function(r){var s=[];
s.push(f+this.__GQ.getValue());
s.push(e+this.__GP.getValue());
s.push(l+this.__GR.getValue());
s.push(i+this.__GS.getValue());
s.push(k+this.__GU.getValue());
this.__GT.setHtml(s.join(h));
},_back:function(){qx.ui.mobile.navigation.Manager.getInstance().executeGet(c,{reverse:true});
}}});
})();
...
},events:{"input":e,"changeValue":e},properties:{liveUpdate:{check:h,init:false},placeholder:{check:a,nullable:true,init:null,apply:i},readOnly:{check:h,nullable:true,init:null,apply:i}},members:{__GW:null,_convertValue:function(k){return k||d;
},setValue:function(l){l=this._convertValue(l);

if(this.__GW!=l){this._setAttribute(f,l);
this.__GX(l);
}},getValue:function(){return this._convertValue(this._getAttribute(f));
},resetValue:function(){this.setValue(null);
},_onChangeContent:function(m){this.__GX(this._convertValue(m.getData()));
},_onInput:function(n){this.fireDataEvent(g,n.getData(),true);

if(this.getLiveUpdate()){this.setValue(n.getData());
}},__GX:function(o){if(this.__GW!=o){this.__GW=o;
...
this.__GY=this._createChild();
this._add(this.__GY);
},properties:{defaultCssClass:{refine:true,init:e},value:{check:c,init:false,apply:b,event:a}},members:{__GY:null,_getChild:function(){return this.__GY;
