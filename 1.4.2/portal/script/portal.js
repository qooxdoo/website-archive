...
var settings = {"qx.application":"portal.Application","qx.revision":"28147","qx.theme":"qx.theme.Modern","qx.version":"1.4.2"};
...
var envinfo = {"qx.application":"portal.Application","qx.debug":false,"qx.revision":"28147","qx.theme":"qx.theme.Modern","qx.version":"1.4.2"};
for (var k in envinfo) qx.$$environment[k] = envinfo[k];

if (!qx.$$libraries) qx.$$libraries = {};
var libinfo = {"__out__":{"sourceUri":"script"},"portal":{"resourceUri":"resource","sourceUri":"script","version":"trunk"},"qx":{"resourceUri":"resource","sourceUri":"script","version":"1.4.2"}};
...
this.__Fu=[];
this.__Fv={};
},statics:{dataKeyRoot:h,dataKeyGroupBoxes:m,dataKeyBoxes:d},events:{"loaded":e},properties:{activeBox:{init:null,check:g,apply:c}},members:{__Ft:null,__Fu:null,__Fv:null,_applyActiveBox:function(n,o){if(o!=null){o.setActive(false);
}n.setActive(true);
},load:function(){this.__Fw(window[portal.box.Manager.dataKeyRoot]);
qx.event.Registration.fireEvent(this,b);
},__Fw:function(p){var t,w,u,v,x,p;
...
this.__Ft.push(x);
this.__Fv[w].push(x);
}}},getBoxes:function(){return this.__Ft;
},getGroupBoxes:function(){return this.__Fu;
},updateGroupBoxMembers:function(y,z,A){var B=qx.dom.Hierarchy.getElementIndex(A.getElement());
qx.lang.Array.remove(this.__Fv[z],A);
qx.lang.Array.insertAt(this.__Fv[y],A,B);
},getGroupBoxDataOfBox:function(C){for(var i=0,j=this.__Fu.length,D;i<j;i++){D=this.__Fv[this.__Fu[i].id];

for(var k=0,l=D.length;k<l;k++){if(D[k].getId()===C){return this.__Fu[i];
}}}return null;
}},destruct:function(){this.__Ft=this.__Fu=this.__Fv=null;
}});
})();
...
this.__Fz=new portal.box.Resizable(this,o);
}if(k.draggable){this.setDraggable(true);
this.__FA=new portal.box.Draggable(this);
}this.__FC();
},properties:{element:{init:null},groupBoxId:{init:null,check:a},active:{check:j,init:false,apply:d},resizable:{check:j,init:false},draggable:{check:j,init:false}},members:{__Fy:null,__Fx:null,__FA:null,__Fz:null,_applyActive:function(p,q){if(p){qx.bom.element.Style.set(this.getElement(),i,b);
}else{qx.bom.element.Style.set(this.getElement(),i,g);
}},getId:function(){return this.getElement()?this.getElement().id:null;
},getData:function(){return this.__Fx;
},getDraggableComponent:function(){return this.__FA;
},getResizableComponent:function(){return this.__Fz;
},__FB:function(r,s){if(s){for(var t in s){qx.bom.element.Style.set(t,s[t]);
}}qx.bom.element.Style.set(r,"position","relative");
qx.bom.element.Style.set(r,"zIndex",0);
qx.bom.Element.addListener(r,"click",this.__FD,this,true);
},__FC:function(){var v="This is a demo application showing the low-level capabilities."+" It does contain no UI widget code at all.";
v=v+"<br/><br/>"+v+"<br/><br/>"+v;
var u=qx.bom.Element.create("div",{"class":"box_content"});
u.innerHTML=v;
qx.dom.Element.insertEnd(u,this.getElement());
},__FD:function(e){portal.box.Manager.getInstance().setActiveBox(this);
}},destruct:function(){this.resetElement();
this.__Fx=this.__Fz=this.__FA=null;
}});
})();
...
this.__FE=d;
this.__FF=this.__FE.getElement();
this.__FG={};
this.__FH=false;
this.__FI={orientation:null,lastCoord:{left:null,top:null}};
this.__FJ={minWidth:10,minHeight:10,maxWidth:null,maxHeight:null,autoHide:true,handles:3};

for(var name in this.__FJ){if(f[name]){this.__FJ[name]=f[name];
}}this.__FK();
},members:{__FI:null,__FE:null,__FF:null,__FG:null,__FJ:null,__FH:false,getBox:function(){return this.__FE;
},__FK:function(){switch(this.__FJ.handles){case 1:this.__FG.east=null;
break;
case 2:this.__FG.south=null;
break;
case 3:this.__FG.south=null;
this.__FG.southeast=null;
this.__FG.east=null;
break;
}for(var h in this.__FG){var g=this.__FG[h]=qx.bom.Element.create("div");
qx.bom.element.Attribute.set(g,"orientation",h);
qx.dom.Element.insertEnd(g,this.__FF);

if(this.__FF.id){g.id=this.__FF.id+"_"+h;
}this.__FL(h);
this.__FM(h);
}},__FL:function(i){qx.bom.element.Class.add(this.__FG[i],i+"Handle");
qx.bom.element.Style.set(this.__FG[i],"visibility",this.__FJ.autoHide?"hidden":"visible");
},__FM:function(j){var l=qx.event.Registration;
var k=this.__FG[j];
l.addListener(k,"mousedown",this.__FP,this);
l.addListener(document,"mouseup",this.__FQ,this);

if(this.__FJ.autoHide){l.addListener(this.__FE.getElement(),"mouseover",this.__FN,this);
l.addListener(this.__FE.getElement(),"mouseout",this.__FO,this);
}},__FN:function(e){for(var m in this.__FG){qx.bom.element.Style.set(this.__FG[m],"visibility","visible");
}},__FO:function(e){if(!this.__FH){for(var n in this.__FG){qx.bom.element.Style.set(this.__FG[n],"visibility","hidden");
}}},__FP:function(e){e.stopPropagation();
if(e.isLeftPressed()){qx.event.Registration.addListener(document,"mousemove",this.__FR,this,true);
this.__FH=true;
var o=e.getTarget();
this.__FI.orientation=qx.bom.element.Attribute.get(o,"orientation");
this.__FI.lastCoord.left=e.getDocumentLeft();
this.__FI.lastCoord.top=e.getDocumentTop();
portal.box.Util.bringToFront(this.__FF);
}},__FQ:function(e){e.stopPropagation();

if(this.__FH){qx.event.Registration.removeListener(document,"mousemove",this.__FR,this,true);
portal.box.Util.sendToBack(this.__FF);

if(this.__FJ.autoHide){for(var p in this.__FG){qx.bom.element.Style.set(this.__FG[p],"visibility","hidden");
}}this.__FH=false;
qx.bom.Selection.clear(document.body);
}},__FR:function(e){e.stopPropagation();

if(this.__FH){var q={left:null,top:null};

switch(this.__FI.orientation){case "south":q.top=e.getDocumentTop();
break;
...
break;
}this.__FS(q,this.__FI.lastCoord);
this.__FI.lastCoord.left=e.getDocumentLeft();
this.__FI.lastCoord.top=e.getDocumentTop();
}},__FS:function(r,s){var z=qx.bom.element.Dimension;
var w=portal.box.Manager.getInstance().getGroupBoxDataOfBox(this.__FE.getId());
var y=z.getContentSize(w.element);

if(r.left){var v=r.left-s.left;

if(v!==0){var u=z.getContentWidth(this.__FF)+v;
var x=z.getWidth(this.__FF)+v;

if((this.__FJ.minWidth==null||u>this.__FJ.minWidth)&&(this.__FJ.maxWidth==null||u<=this.__FJ.maxWidth)){if(y.width>=x){qx.bom.element.Style.set(this.__FF,"width",u+'px');
this.__FE.getData().width=u;
}}}}
if(r.top){var v=r.top-s.top;

if(v!==0){var t=z.getContentHeight(this.__FF)+v;

if((this.__FJ.minHeight==null||t>this.__FJ.minHeight)&&(this.__FJ.maxHeight==null||t<=this.__FJ.maxHeight)){qx.bom.element.Style.set(this.__FF,"height",t+"px");
}}}}},destruct:function(){if(this.__FJ.autoHide){qx.event.Registration.removeListener(this.__FE.getElement(),b,this.__FN,this);
qx.event.Registration.removeListener(this.__FE.getElement(),a,this.__FO,this);
}this.__FE=this.__FJ=this.__FF=this.__FG=null;
this.__FI=null;
}});
})();
...
qx.Class.define(b,{statics:{__FT:1000,bringToFront:function(d){var e=qx.bom.element.Style.get(d,c);
e=e==a?0:e;
qx.bom.element.Style.set(d,c,e+this.__FT);
},sendToBack:function(f){var g=qx.bom.element.Style.get(f,c);
qx.bom.element.Style.set(f,c,g-this.__FT);
}}});
})();
...
(function(){var n="mousemove",m="mouseup",l="dragstart",k="engine.name",j="mousedown",i="portal.box.Draggable",h="mouseover",g="drag",f="dragend",d="move",a="dragHandle",c="cursor",b="html";
qx.Class.define(i,{extend:qx.core.Object,construct:function(o){qx.core.Object.call(this);
this.__FU=o;
this.__FV=o.getElement();
this.__FW=null;
this.__FX=null;
this.__FY();
},members:{__FU:null,__FV:null,__FW:null,__FX:null,getBox:function(){return this.__FU;
},getElement:function(){return this.__FV;
},__FY:function(){this.__Ga();
this.__Gb();
},__Ga:function(){this.__FW=qx.bom.Element.create("div");
qx.bom.element.Class.add(this.__FW,a);
qx.dom.Element.insertBegin(this.__FW,this.__FV);
},__Gb:function(){qx.bom.Element.addListener(this.__FW,j,this.__Gc,this);
qx.bom.Element.addListener(this.__FW,h,function(e){qx.bom.element.Style.set(this,c,d);
},this.__FW);
qx.bom.Element.addListener(this.__FW,l,this.__Gf,this);
qx.bom.Element.addListener(this.__FW,f,portal.dragdrop.Manager.getInstance().stopSession,portal.dragdrop.Manager.getInstance());
qx.bom.Element.addListener(this.__FW,g,this.__Gg,this);
},__Gc:function(e){if(e.isLeftPressed()){this.__FX={left:e.getDocumentLeft()-qx.bom.element.Location.getLeft(this.__FV),top:e.getDocumentTop()-qx.bom.element.Location.getTop(this.__FV)};
qx.bom.Element.addListener(document.body,m,this.__Ge,this,true);
this.__Gd();
qx.event.Registration.fireEvent(this.__FW,l,qx.event.type.Event);
}},__Gd:qx.core.Environment.select(k,{"mshtml":function(){var q=this;
var p=qx.lang.Function.bind(this.__Ge,q);
document.getElementsByTagName(b)[0].onmouseleave=p;
},"default":qx.lang.Function.empty}),__Ge:function(e){try{e.stopPropagation();
}catch(r){}
if(portal.dragdrop.Manager.getInstance().isSessionActive()){qx.event.Registration.removeListener(document.body,n,this.__Gg,this,true);
portal.dragdrop.Manager.getInstance().stopSession();
}qx.bom.Element.removeListener(document.body,m,this.__Ge,this,true);
},__Gf:function(e){portal.box.Manager.getInstance().setActiveBox(this.__FU);
portal.dragdrop.Manager.getInstance().startSession(this.__FU);
qx.event.Registration.addListener(document.body,n,this.__Gg,this,true);
},__Gg:function(e){e.stopPropagation();
var s=e.getDocumentLeft()-this.__FX.left;
var top=e.getDocumentTop()-this.__FX.top;
qx.event.Timer.once(function(){this.checkGroupBox(s,parseInt(this.getActiveBox().getData().width));
this.onDragMove(top,s);
},portal.dragdrop.Manager.getInstance(),0);
}},destruct:function(){this.__FU=this.__FW=this.__FV=this.__FX=null;
}});
})();
...
this.__Gh={top:null,height:null};
this.__Gi={top:0,left:0};
},properties:{activeBox:{check:u,init:null,nullable:true},sessionActive:{check:r,init:false}},members:{__Gh:null,__Gj:null,__Gk:false,__Gi:null,__Gl:null,__Gm:null,startSession:function(y){this.setSessionActive(true);
this.setActiveBox(y);
this.__Gj=y.getGroupBoxId();
this.__Gn();
if(this.__Gl==null){this.__Gl={};
var z=portal.box.Manager.getInstance().getGroupBoxes();

for(var i=0,j=z.length;i<j;i++){this.__Gl[z[i].element.id]={left:qx.bom.element.Location.getLeft(z[i].element),right:qx.bom.element.Location.getLeft(z[i].element)+qx.bom.element.Dimension.getWidth(z[i].element)};
}}},__Gn:function(){if(!this.__Gm){this.__Gm=qx.bom.Element.create(b);
}var C=this.getActiveBox().getElement();
var A=qx.bom.element.Dimension.getContentSize(C);
this.__Go(C,this.__Gm);
qx.bom.element.Class.add(this.__Gm,qx.bom.element.Class.get(C));
var B=k+d+qx.bom.element.Location.getTop(C)+w+c+qx.bom.element.Location.getLeft(C)+w+o+A.width+w+f+A.height+x;
qx.bom.element.Style.setCss(this.__Gm,B);
qx.bom.element.Style.set(C,v,t);
qx.dom.Node.getBodyElement(C).appendChild(this.__Gm);
portal.box.Util.bringToFront(this.__Gm);
},__Go:function(D,E){while(D.firstChild){E.appendChild(D.firstChild);
}},stopSession:function(e){this.setSessionActive(false);
var G=this.getActiveBox();
var H=G.getElement();
portal.box.Manager.getInstance().updateGroupBoxMembers(G.getGroupBoxId(),this.__Gj,G);
G.setGroupBoxId(this.__Gj);
var F=new qx.fx.effect.core.Move(this.__Gm);
F.set({x:qx.bom.element.Location.getLeft(H),y:qx.bom.element.Location.getTop(H),mode:h,duration:0.5,transition:g});
F.start();
F.addListener(n,function(){this.__Go(this.__Gm,H);
qx.bom.element.Style.reset(H,v);
this.__Gm.parentNode.removeChild(this.__Gm);

if(G.isActive()){G._applyActive(true);
}else{portal.box.Manager.getInstance().setActiveBox(G);
}},this);
},onDragMove:function(top,I){if(this.__Gi.left!=I){qx.bom.element.Style.set(this.__Gm,q,I+x);
}
if(this.__Gi.top!=top){qx.bom.element.Style.set(this.__Gm,s,top+x);
}var J=this.getActiveBox().getElement();

if(this.__Gh.top==null){this.__Gh.top=qx.bom.element.Location.getTop(J);
}
if(this.__Gh.height==null){this.__Gh.height=qx.bom.element.Dimension.getHeight(J);
}if(this.__Gk==false&&top>=this.__Gh.top&&top<=(this.__Gh.top+this.__Gh.height)){return;
}var K;
if(this.__Gk){K=qx.dom.Hierarchy.getFirstDescendant(document.getElementById(this.__Gj));
while(K!=null){if(this.__Gp(true,top,J,K)){this.__Gk=false;
return;
}K=qx.dom.Hierarchy.getNextElementSibling(K);
}}else{K=J;
}if(top-this.__Gi.top>0){K=qx.dom.Hierarchy.getNextElementSibling(K);

while(K!=null){if(K){if(this.__Gp(true,top,J,K)){return;
}}K=qx.dom.Hierarchy.getNextElementSibling(K);
}}else{var K=qx.dom.Hierarchy.getPreviousElementSibling(K);

while(K!=null){if(K){if(this.__Gp(false,top,J,K)){return;
}}K=qx.dom.Hierarchy.getPreviousElementSibling(K);
}}this.__Gi.left=I;
this.__Gi.top=top;
},__Gp:function(L,top,M,N){var O=qx.bom.element.Location.getTop(N);
...
this.__Gh.top=qx.bom.element.Location.getTop(M);
}return true;
...
},checkGroupBox:function(T,U){if(this.__Gl[this.__Gj].left<=T&&this.__Gl[this.__Gj].right>=T){return;
}
for(var V in this.__Gl){if(V!=this.__Gj){if(this.__Gl[V].left<=T+U&&this.__Gl[V].right>=T){this.__Gj=V;
this.__Gk=true;
return;
}}}}},destruct:function(){this.__Gh=this.__Gi=this.__Gj=null;
this.__Gq=this.__Gm=this.__Gl=null;
}});
})();
