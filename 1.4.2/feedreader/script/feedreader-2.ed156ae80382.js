...
}},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{input:1,change:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__rX:false,__rY:null,__sa:null,__sb:null,canHandleEvent:function(B,C){var D=B.tagName.toLowerCase();

if(C===m&&(D===m||D===c)){return true;
}
if(C===j&&(D===m||D===c||D===p)){return true;
}return false;
},registerEvent:function(E,F,G){if(qx.core.Environment.get(k)==x&&(qx.core.Environment.get(f)<9||(qx.core.Environment.get(f)>=9&&qx.core.Environment.get(A)<9))){if(!E.__sc){var H=E.tagName.toLowerCase();
var I=E.type;
...
qx.bom.Event.addNativeListener(E,a,this._onKeyPressWrapped);
}}}}},__sd:qx.core.Environment.select(k,{"mshtml":function(J){if(qx.core.Environment.get(f)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.addNativeListener(J,m,this._onInputWrapper);
...
}}),unregisterEvent:function(O,P){if(qx.core.Environment.get(k)==x&&qx.core.Environment.get(f)<9&&qx.core.Environment.get(A)<9){if(O.__sc){var Q=O.tagName.toLowerCase();
var R=O.type;
...
}}}},__se:qx.core.Environment.select(k,{"mshtml":function(T){if(qx.core.Environment.get(f)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.removeNativeListener(T,m,this._onInputWrapper);
...
}}),_onKeyPress:qx.core.Environment.select(k,{"mshtml|opera":function(e,Y){if(e.keyCode===13){if(Y.value!==this.__sa){this.__sa=Y.value;
qx.event.Registration.fireEvent(Y,j,qx.event.type.Data,[Y.value]);
}}},"default":null}),_inputFix:qx.core.Environment.select(k,{"mshtml":function(e,ba){if(e.keyCode===46||e.keyCode===8){if(ba.value!==this.__sb){this.__sb=ba.value;
qx.event.Registration.fireEvent(ba,m,qx.event.type.Data,[ba.value]);
}}},"default":null}),_onKeyDown:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__rX=true;
}},"default":null}),_onKeyUp:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__rX=false;
}},"default":null}),_onBlur:qx.core.Environment.select(k,{"opera":function(e){if(this.__rY&&qx.core.Environment.get(t)<10.6){window.clearTimeout(this.__rY);
...
if(!this.__rX||bb!==m){if((qx.core.Environment.get(k)==z)&&qx.core.Environment.get(t)<10.6){this.__rY=window.setTimeout(function(){qx.event.Registration.fireEvent(bc,m,qx.event.type.Data,[bc.value]);
},0);
...
},events:{"changeValid":a,"complete":f},properties:{validator:{check:d,init:null,nullable:true},invalidMessage:{check:h,init:k},requiredFieldMessage:{check:h,init:k},context:{nullable:true}},members:{__sf:null,__sh:null,__sg:null,__si:null,add:function(l,m,n){if(!this.__sn(l)){throw new Error("Added widget not supported.");
}if(this.__so(l)){if(m!=null){throw new Error("Widgets supporting selection can only be validated "+"in the form validator");
}}var o={item:l,validator:m,valid:null,context:n};
this.__sf.push(o);
},remove:function(p){var q=this.__sf;
...
for(var i=0;i<this.__sf.length;i++){s.push(this.__sf[i].item);
}return s;
},validate:function(){var x=true;
this.__si=true;
var u=[];
for(var i=0;i<this.__sf.length;i++){var v=this.__sf[i].item;
var y=this.__sf[i].validator;
u.push(v);
if(y==null){var t=this.__sj(v);
x=x&&t;
this.__si=t&&this.__si;
continue;
}var t=this.__sk(this.__sf[i],v.getValue());
x=t&&x;

if(t!=null){this.__si=t&&this.__si;
}}var w=this.__sl(u);

if(qx.lang.Type.isBoolean(w)){this.__si=w&&this.__si;
}x=w&&x;
this.__sp(x);

if(qx.lang.Object.isEmpty(this.__sg)){this.fireEvent(j);
}return x;
},__sj:function(z){if(z.getRequired()){if(this.__so(z)){var A=!!z.getSelection()[0];
...
},__sk:function(D,E){var J=D.item;
var I=D.context;
var H=D.validator;
if(this.__sm(H)){this.__sg[J.toHashCode()]=null;
...
D.valid=G;
return G;
},__sl:function(K){var M=this.getValidator();
var N=this.getContext()||this;

if(M==null){return true;
}this.setInvalidMessage(k);

if(this.__sm(M)){this.__sg[this.toHashCode()]=null;
...
},__sm:function(P){var Q=false;

if(!qx.lang.Type.isFunction(P)){Q=qx.Class.isSubClassOf(P.constructor,qx.ui.form.validation.AsyncValidator);
}return Q;
},__sn:function(R){var S=R.constructor;
return qx.Class.hasInterface(S,qx.ui.form.IForm);
},__so:function(T){var U=T.constructor;
return qx.Class.hasInterface(U,qx.ui.core.ISingleSelection);
},__sp:function(V){var W=this.__sh;
this.__sh=V;
if(W!=V){this.fireDataEvent(g,V,W);
}},getValid:function(){return this.__sh;
},isValid:function(){return this.getValid();
},getInvalidMessages:function(){var X=[];
for(var i=0;i<this.__sf.length;i++){var Y=this.__sf[i].item;

if(!Y.getValid()){X.push(Y.getInvalidMessage());
}}if(this.getInvalidMessage()!=k){X.push(this.getInvalidMessage());
}return X;
},reset:function(){for(var i=0;i<this.__sf.length;i++){var ba=this.__sf[i];
ba.item.setValid(true);
}this.__sh=null;
},setItemValid:function(bb,bc){this.__sg[bb.toHashCode()]=bc;
bb.setValid(bc);
this.__sq();
},setFormValid:function(bd){this.__sg[this.toHashCode()]=bd;
this.__sq();
},__sq:function(){var bf=this.__si;
for(var bg in this.__sg){var be=this.__sg[bg];
bf=be&&bf;
if(be==null){return;
}}this.__sp(bf);
this.__sg={};
...
this.__sr=b;
},members:{__sr:null,__ss:null,__st:null,__su:null,validate:function(c,d,e,f){this.__su=false;
this.__ss=c;
this.__st=e;
this.__sr.call(f||this,this,d);
},validateForm:function(g,h,i){this.__su=true;
this.__st=h;
this.__sr.call(i,g,this);
},setValid:function(j,k){if(this.__su){if(k!==undefined){this.__st.setInvalidMessage(k);
}this.__st.setFormValid(j);
}else{if(k!==undefined){this.__ss.setInvalidMessage(k);
}this.__st.setItemValid(this.__ss,j);
}}},destruct:function(){this.__st=this.__ss=null;
}});
})();
...
if(bk&&this.__sv){this.__sC().setStyles({"left":bl.left+bp,"top":bl.top+bp});
}
if(bm){if(this.__sv){this.__sC().setStyles({"width":innerWidth+bp,"height":innerHeight+bp});
...
if(this.__sv){if(bs){this._showPlaceholder();
}else{this._removePlaceholder();
}}else{var bu=this.getContentElement();
bu.setAttribute(a,bs?this.getPlaceholder():j);
}},__sA:{width:16,height:16},_getContentHint:function(){return {width:this.__sA.width*10,height:this.__sA.height||16};
...
if(this.__sv){this.__sC().setStyles(bx);
}if(bv){this.__sA=qx.bom.Label.getTextSize(m,bx);
}else{delete this.__sA;
...
this.selectAllText();
},_getTextSize:function(){return this.__sA;
...
}if(bD){this.fireDataEvent(be,bE,this.__sz);
this.__sz=bE;
if(this.getLiveUpdate()){this.__sB(bE);
}}},__sB:function(bG){var bH=this.__sy;
this.__sy=bG;

if(bH!=bG){this.fireNonBubblingEvent(o,qx.event.type.Data,[bG,bH]);
}},setValue:function(bI){if(bI===null){if(this.__sw){return bI;
}bI=j;
this.__sw=true;
}else{this.__sw=false;
if(this.__sv){this._removePlaceholder();
}}
...
bK.setValue(bI);
var bJ=this.__sw?null:bI;
this.__sy=bL;
this.__sB(bJ);
}if(this.__sv){this._showPlaceholder();
}return bI;
}throw new Error("Invalid value type: "+bI);
},getValue:function(){var bM=this.getContentElement().getValue();
return this.__sw?null:bM;
},resetValue:function(){this.setValue(null);
},_onChangeContent:function(e){this.__sw=e.getData()===null;
this.__sB(e.getData());
...
}}},_removePlaceholder:function(){if(this.hasState(k)){this.__sC().setStyle(bc,X);
this.removeState(k);
}},_syncPlaceholder:function(){if(this.hasState(k)){this.__sC().setStyle(bc,L);
}},__sC:function(){if(this.__sx==null){this.__sx=new qx.html.Label();
var bR=qx.theme.manager.Color.getInstance();
this.__sx.setStyles({"visibility":X,"zIndex":6,"position":W,"color":bR.resolve(I)});
this.getContainerElement().add(this.__sx);
}return this.__sx;
},_onChangeLocale:qx.core.Environment.select(g,{"true":function(e){var content=this.getPlaceholder();

if(content&&content.translate){this.setPlaceholder(content.translate());
}},"false":null}),_applyPlaceholder:function(bS,bT){if(this.__sv){this.__sC().setValue(bS);
...
(function(){var s="",r="URL",q="Add",p="keypress",o="Title",n="__sF",m="Add a feed",l="feedreader.view.AddFeedWindow",k="user",j="this.__form",c="__sE",i="icon/16/actions/document-new.png",g="icon/16/actions/dialog-apply.png",b="right",a="__sD",f="Feed Information",d="execute",h="Enter";
...
this._addContent();
},members:{__sE:null,__sF:null,__sD:null,__sG:null,_addContent:function(){this.setLayout(new qx.ui.layout.VBox(10));
this.__sG=new qx.ui.form.Form();
this.__sG.addGroupHeader(this.tr(f));
this.__sF=new qx.ui.form.TextField().set({required:true,width:250});
this.__sG.add(this.__sF,this.tr(o));
this.__sE=new qx.ui.form.TextField().set({required:true});
this.__sG.add(this.__sE,this.tr(r),qx.util.Validate.checkUrl);
var u=new qx.ui.form.Button(this.tr(q),g);
u.set({alignX:b,allowGrowX:false});
u.addListener(d,this._addFeed,this);
this.__sG.addButton(u);
this.addListener(p,function(e){if(e.getTarget() instanceof qx.ui.form.TextField&&e.getKeyIdentifier()===h){this._addFeed();
}});
this.add(new qx.ui.form.renderer.SinglePlaceholder(this.__sG));
},_addFeed:function(e){if(this.__sG.validate()){var w=this.__sF.getValue();
var v=this.__sE.getValue();
this.__sD.addFeed(w,v,k);
this.__sF.setValue(s);
this.__sE.setValue(s);
this.close();
}}},destruct:function(){this.__sH=null;
this._disposeObjects(a,n,c,j);
}});
})();
...
this.__sI=o;
},members:{__sI:null,__sJ:null,__sK:null,_createDomElement:function(){return qx.bom.Input.create(this.__sI);
...
}},setEnabled:qx.core.Environment.select(k,{"webkit":function(u){this.__sK=u;

if(!u){this.setStyles({"userModify":d,"userSelect":j});
}else{this.setStyles({"userModify":null,"userSelect":this.__sJ?null:j});
}},"default":function(v){this.setAttribute(b,v===false);
}}),setSelectable:qx.core.Environment.select(k,{"webkit":function(w){this.__sJ=w;
qx.html.Element.prototype.setSelectable.call(this,this.__sK&&w);
...
},setWrap:function(B,C){if(this.__sI===l){this._setProperty(n,B,C);
}else{throw new Error("Text wrapping is only support by textareas!");
}return this;
},getWrap:function(){if(this.__sI===l){return this._getProperty(n);
}else{throw new Error("Text wrapping is only support by textareas!");
}}}});
})();
...
qx.Class.define(h,{statics:{__sL:{text:1,textarea:1,select:1,checkbox:1,radio:1,password:1,hidden:1,submit:1,image:1,file:1,search:1,reset:1,button:1},create:function(w,x,y){var x=x?qx.lang.Object.clone(x):{};
var z;
...
},members:{__sM:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(c,d,e,name,f,g){if(this.__sN()){this.__sM.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});
}this.__sM[this._groupCounter].items.push(c);
this.__sM[this._groupCounter].labels.push(d);
this.__sM[this._groupCounter].options.push(g);
if(name==null){name=d.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,a);
}this.__sM[this._groupCounter].names.push(name);
this._validationManager.add(c,e,f);
this._resetter.add(c);
},addGroupHeader:function(h,k){if(!this.__sN()){this._groupCounter++;
}this.__sM.push({title:h,items:[],labels:[],names:[],options:[],headerOptions:k});
},addButton:function(l,m){this._buttons.push(l);
this._buttonOptions.push(m||null);
},__sN:function(){return this.__sM.length===0;
