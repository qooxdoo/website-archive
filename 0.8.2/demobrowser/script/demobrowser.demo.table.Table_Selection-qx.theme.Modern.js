...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.table.Table_Selection-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var m="execute",l="",k="A date",j="Boolean",i="demobrowser.demo.table.Table_Selection",h="changeValue",g="icon/22/status/dialog-information.png",f="A number",e="icon/16/apps/office-calendar.png",d="Reset Cell Focus",z="middle",y="Remove 5 rows",x="Clear Selection",w="Table",v="Add 5 rows",u="icon/22/actions/list-remove.png",t=", ",s="icon/22/actions/list-add.png",r="Hide Cell Focus",q="Selected rows:<br>",o="changeChecked",p="Show selection",n="ID";
qx.Class.define(i,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return w;
},createTable:function(){var Q=this.createRandomRows(50);
var P=this._tableModel=new qx.ui.table.model.Simple();
P.setColumns([n,f,k,j]);
P.setData(Q);
...
O.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(e,k));
return R;
...
var J=qx.ui.table.selection.Model;
var E={"Single selection":J.SINGLE_SELECTION,"Single interval":J.SINGLE_INTERVAL_SELECTION,"Multiple intervals":J.MULTIPLE_INTERVAL_SELECTION,"Multiple intervals (toggle)":J.MULTIPLE_INTERVAL_SELECTION_TOGGLE,"No selection":J.NO_SELECTION};

for(var I in E){C.add(new qx.ui.form.ListItem(I).set({value:E[I]+l}));
}C.addListener(h,function(e){this._table.getSelectionModel().setSelectionMode(parseInt(C.getValue()));
},this);
G.add(C);
G=new qx.ui.toolbar.Part();
D.add(G);
F=new qx.ui.toolbar.Button(v,s);
F.addListener(m,function(a){var b=this.createRandomRows(10);
this._tableModel.addRows(b);
this.info("5 rows added");
},this);
G.add(F);
F=new qx.ui.toolbar.Button(y,u);
F.addListener(m,function(K){var L=this._tableModel.getRowCount();
this._tableModel.removeRows(L-5,5);
...
F.addListener(m,function(S){this._table.clearSelection();
},this);
G.add(F);
F=new qx.ui.toolbar.Button(d);
F.addListener(m,function(B){this._table.resetCellFocus();
},this);
G.add(F);
var H=new qx.ui.toolbar.CheckBox(r);
H.addListener(o,function(c){this._table.setShowCellFocusIndicator(!H.isChecked());
},this);
G.add(H);
F=new qx.ui.toolbar.Button(p,g);
F.addListener(m,function(M){var N=[];
this._table.getSelectionModel().iterateSelection(function(A){N.push(A+l);
});
this.showDialog(q+N.join(t));
},this);
G.add(F);
return D;
}}});
