...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.table.Table", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.table.Table-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var o="execute",n="changeChecked",m="A date",l="Boolean",k="icon/22/actions/edit-undo.png",j="Change ID sort method",h="icon/22/status/dialog-information.png",g="Change row with ID 10",f="A number",e="icon/16/apps/office-calendar.png",D="Remove 5 rows",C="Add 10 rows",B="",A="demobrowser.demo.table.Table",z="Table",y="icon/22/actions/list-remove.png",x="Whether the the first visible row should ",w=", ",v="icon/22/actions/list-add.png",u="Selected rows:<br>",s="Keep first row",t="be rendered completely when scrolling.",q="Show selection",r="Demonstrate use of alternate sorting algorithm.",p="ID";
qx.Class.define(A,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return z;
},createTable:function(){var O=this.createRandomRows(50);
var N=this._tableModel=new qx.ui.table.model.Simple();
N.setColumns([p,f,m,l]);
N.setData(O);
...
M.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(e,m));
return P;
},createControls:function(){var bb=new qx.ui.toolbar.ToolBar();
var Y,ba,bc;
ba=new qx.ui.toolbar.Part();
bb.add(ba);
Y=new qx.ui.toolbar.Button(g,k);
Y.addListener(o,function(bd){var be=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,be[0][i]);
}this.info("Row 10 changed");
},this);
ba.add(Y);
Y=new qx.ui.toolbar.Button(C,v);
Y.addListener(o,function(I){var J=this.createRandomRows(10);
this._tableModel.addRows(J);
this.info("10 rows added");
},this);
ba.add(Y);
Y=new qx.ui.toolbar.Button(D,y);
Y.addListener(o,function(a){var b=this._tableModel.getRowCount();
this._tableModel.removeRows(b-5,5);
this.info("5 rows removed");
},this);
ba.add(Y);
Y=new qx.ui.toolbar.Button(q,h);
Y.addListener(o,function(K){var L=[];
X.getSelectionModel().iterateSelection(function(c){L.push(c+B);
});
this.showDialog(u+L.join(w));
},this);
ba.add(Y);
ba=new qx.ui.toolbar.Part();
bb.add(ba);
var X=this._table;
bc=new qx.ui.toolbar.CheckBox(s);
bc.set({checked:X.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(x+t)});
bc.addListener(n,function(d){X.setKeepFirstVisibleRowComplete(this.getChecked());
},bc);
ba.add(bc);
bc=new qx.ui.toolbar.CheckBox(j);
bc.set({checked:X.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(r)});
bc.addListener(n,function(U){if(U.getData()){var W=function(E,F){var G=E[arguments.callee.columnIndex];
var H=F[arguments.callee.columnIndex];

if(G%2==1&&H%2==0){return 1;
}
if(H%2==1&&G%2==0){return -1;
}return (G>H)?1:((G==H)?0:-1);
};
var V=function(Q,R){var S=Q[arguments.callee.columnIndex];
var T=R[arguments.callee.columnIndex];

if(S%2==1&&T%2==0){return -1;
}
if(T%2==1&&S%2==0){return 1;
}return (S<T)?1:((S==T)?0:-1);
};
X.getTableModel().setSortMethods(0,{ascending:W,descending:V});
}else{X.getTableModel().setSortMethods(0,null);
}},bc);
ba.add(bc);
return bb;
}}});
