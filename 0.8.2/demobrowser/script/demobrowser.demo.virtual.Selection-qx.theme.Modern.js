...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.virtual.Selection-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var M="manager",L="multi",K=" / ",J="bold",I="white",H="selected",G="#EEE",F="horizontal",E="table-scroller-header",D="one",bh="single",bg="changeChecked",bf="table-row-background-selected",be="additive",bd="default",bc="Additive Selection",bb="changeValue",ba="Cell Line Selection",Y="addaptive",X="Options",T="Row Selection",U="Column Selection",R="Enable drag selection",S="demobrowser.demo.virtual.Selection",P="Selection Mode",Q="Single Selection",N="Cell Rectangle Selection",O="Enable quick selection",V="One Selection",W="Multi Selection";
qx.Class.define(S,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var bE=qx.theme.manager.Font.getInstance().resolve(bd).getStyles();
...
bL.add(new qx.ui.basic.Label(T).set({font:J,decorator:E,padding:3,allowGrowX:true}),{row:1,column:0});
var bD=this.createRowSelectionScroller();
bB.push(bD.getUserData(M));
bL.add(bD,{row:2,column:0});
bL.add(new qx.ui.basic.Label(U).set({font:J,decorator:E,padding:3,allowGrowX:true}),{row:1,column:1});
var bD=this.createColumnSelectionScroller();
bB.push(bD.getUserData(M));
bL.add(bD,{row:2,column:1});
bL.add(new qx.ui.basic.Label(N).set({font:J,decorator:E,padding:3,allowGrowX:true}),{row:1,column:2});
var bD=this.createCellRectangleSelectionScroller();
bB.push(bD.getUserData(M));
bL.add(bD,{row:2,column:2});
bL.add(new qx.ui.basic.Label(ba).set({font:J,decorator:E,padding:3,allowGrowX:true}),{row:1,column:3});
var bD=this.createCellLineSelectionScroller();
bB.push(bD.getUserData(M));
bL.add(bD,{row:2,column:3});
var bJ=new qx.ui.layout.Grid(20,4);
var bG=new qx.ui.container.Composite(bJ).set({padding:15,backgroundColor:I});
bL.add(bG,{row:0,column:0,colSpan:4});
bG.add(new qx.ui.basic.Label(P).set({font:J,padding:3}),{row:0,column:0});
var bF=new qx.ui.form.RadioButton(Q);
bF.setValue(bh);
...
var bC=new qx.ui.form.RadioButton(bc);
bC.setValue(be);
...
bH.addListener(bb,function(e){var c=e.getData();
bB.forEach(function(bO){bO.setMode(c);
});

if(c==bh||c==D){bM.setEnabled(false);
bK.setEnabled(true);
}else if(c==L||c==Y){bM.setEnabled(true);
bK.setEnabled(false);
}});
bG.add(new qx.ui.basic.Label(X).set({font:J,padding:3}),{row:0,column:1});
var bM=new qx.ui.form.CheckBox(R);
bG.add(bM,{row:1,column:1});
bM.addListener(bg,function(e){if(e.getData()){var s=bB[0].getMode();

if(s==bh||s==D){this.debug("Drag selection is only available for the modes multi or additive");
}}bB.forEach(function(b){b.setDrag(e.getData());
});
});
var bK=new qx.ui.form.CheckBox(O).set({enabled:false});
bG.add(bK,{row:2,column:1});
bK.addListener(bg,function(e){if(e.getData()){var bz=bB[0].getMode();

if(bz==L||bz==be){this.debug("Quick selection is only available for the modes multi or additive");
}}bB.forEach(function(a){a.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bp=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var bn=new qx.ui.virtual.layer.Row(I,G);
bp.getPane().addLayer(bn);
bp.getPane().addLayer(new qx.ui.virtual.layer.GridLines(F));
var bo=new qx.ui.virtual.cell.Cell();
var bm=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(d,f){var g={};

if(bl.isItemSelected(d)){g.selected=true;
}return bo.getCellProperties(d+K+f,g);
}});
bp.getPane().addLayer(bm);
var bl=new qx.ui.virtual.selection.Row(bp.getPane(),{styleSelectable:function(bt,bu,bv){if(bu!==H){return;
}
if(bv){bn.setDecorator(bt,H);
}else{bn.setDecorator(bt,null);
}bm.updateLayerData();
}});
...
bp.setUserData(M,bl);
return bp;
},createColumnSelectionScroller:function(){var w=new qx.ui.virtual.core.Scroller(1000,100,20,100);
w.getPane().addLayer(new qx.ui.virtual.layer.Row(I,G));
var v=new qx.ui.virtual.layer.Column();
w.getPane().addLayer(v);
w.getPane().addLayer(new qx.ui.virtual.layer.GridLines(F));
var x=new qx.ui.virtual.cell.Cell();
var u=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bi,bj){var bk={};

if(t.isItemSelected(bj)){bk.selected=true;
}return x.getCellProperties(bi+K+bj,bk);
}});
w.getPane().addLayer(u);
var t=new qx.ui.virtual.selection.Column(w.getPane(),{styleSelectable:function(bP,bQ,bR){if(bQ!==H){return;
}
if(bR){v.setDecorator(bP,H);
}else{v.setDecorator(bP,null);
}u.updateLayerData();
}});
...
w.setUserData(M,t);
return w;
},createCellRectangleSelectionScroller:function(){var B=new qx.ui.virtual.core.Scroller(1000,100,20,100);
B.getPane().addLayer(new qx.ui.virtual.layer.Row(I,G));
var A=new qx.ui.virtual.cell.Cell();
var C=new qx.ui.virtual.cell.Cell().set({backgroundColor:bf});
var z=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bw,bx){var by={};

if(y.isItemSelected({row:bw,column:bx})){by.selected=true;
}
if(by.selected){return C.getCellProperties(bw+K+bx,by);
}else{A.resetBackgroundColor();
return A.getCellProperties(bw+K+bx,by);
}}});
B.getPane().addLayer(z);
B.getPane().addLayer(new qx.ui.virtual.layer.GridLines(F));
var y=new qx.ui.virtual.selection.CellRectangle(B.getPane(),{styleSelectable:function(p,q,r){z.updateLayerData();
}});
y.attachMouseEvents(B.getPane());
y.attachKeyEvents(B);
y.set({mode:L});
y.addItem({row:0,column:0});
B.setUserData(M,y);
return B;
},createCellLineSelectionScroller:function(){var n=new qx.ui.virtual.core.Scroller(1000,100,20,100);
n.getPane().addLayer(new qx.ui.virtual.layer.Row(I,G));
var m=new qx.ui.virtual.cell.Cell();
var o=new qx.ui.virtual.cell.Cell().set({backgroundColor:bf});
var l=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bq,br){var bs={};

if(k.isItemSelected({row:bq,column:br})){bs.selected=true;
}
if(bs.selected){return o.getCellProperties(bq+K+br,bs);
}else{m.resetBackgroundColor();
return m.getCellProperties(bq+K+br,bs);
}}});
n.getPane().addLayer(l);
n.getPane().addLayer(new qx.ui.virtual.layer.GridLines(F));
var k=new qx.ui.virtual.selection.CellLines(n.getPane(),{styleSelectable:function(h,i,j){l.updateLayerData();
}});
...
(function(){var h="right",g="qx.ui.virtual.selection.Column",f="left";
qx.Class.define(g,{extend:qx.ui.virtual.selection.Row,members:{_getItemCount:function(){return this._pane.getColumnConfig().getItemCount();
},_getSelectableFromMouseEvent:function(event){var k=this._pane.getCellAtPosition(event.getDocumentLeft(),event.getDocumentTop());
return this._isSelectable(k.column)?k.column:null;
},_getRelatedSelectable:function(a,b){if(b==f){var d=a-1;
var c=0;
var e=-1;
}else if(b==h){var d=a+1;
