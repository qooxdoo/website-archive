...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.bom.Location", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.bom.Location-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
(function(){var q="px",p="moveable",o="demobrowser.demo.bom.Location",n="borderBottomWidth",m="list",k='#',j="mode",h="borderTopWidth",g="borderLeftWidth",f="a",d="div",e="borderRightWidth";
qx.Class.define(o,{extend:qx.application.Native,members:{main:function(){arguments.callee.base.call(this);
var a=document.getElementById(m).getElementsByTagName(f);

for(var i=0,l=a.length;i<l;i++){a[i].onclick=demobrowser.demo.bom.Location.jump;
}}},statics:{num:function(b,c){return parseInt(qx.bom.element.Style.get(b,c,qx.bom.element.Style.COMPUTED_MODE,false),10);
},jump:function(event){if(!event){event=window.event;
}event.returnValue=false;

if(event.preventDefault){event.preventDefault();
}function r(y,z){return parseInt(qx.bom.element.Style.get(y,z,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var u=this.href;
var w=u.substr(u.indexOf(k)+1);
var s=document.getElementById(w);
var location=qx.bom.element.Location.get(s,document.getElementById(j).value);
var v=document.getElementById(p);
v.style.left=location.left+q;
v.style.top=location.top+q;

if(qx.bom.client.Feature.BORDER_BOX){v.style.width=(location.right-location.left)+q;
v.style.height=(location.bottom-location.top)+q;
}else{var t=qx.bom.element.Location;
v.style.width=(location.right-location.left-r(v,g)-r(v,e))+q;
v.style.height=(location.bottom-location.top-r(v,h)-r(v,n))+q;
}return false;
},randomize:function(){var x=document.body.getElementsByTagName(d);

for(var i=0,l=x.length;i<l;i++){if(x[i].id&&x[i].id!=p){x[i].style.margin=Math.round(Math.random()*10)+q;
x[i].style.padding=Math.round(Math.random()*10)+q;
x[i].style.borderWidth=Math.round(Math.random()*10)+q;
}}}}});
})();
