...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.virtual.CellSpan-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
(function(){var j="yellow",i="bold",h="table-scroller-header",g="green",f=" / ",e="Widget Cells",d="demobrowser.demo.virtual.CellSpan",c="HTML Cells",b="x";
...
this.__tC=new qx.ui.virtual.cell.Cell();
...
this.__tC.setBackgroundColor(p);
return this.__tC.getCellProperties(n+f+o);
...
y.set({backgroundColor:(w+x)%2==0?j:g,content:w+b+x,padding:3});
return y;
...
(function(){var r="",q="' ",p="px;",o="</div>",n="top:",m=">",k="<div ",j="qx.ui.virtual.layer.HtmlCellSpan",h="style='",g="class='",d="x",f="left:",e="html";
qx.Class.define(j,{extend:qx.ui.virtual.layer.HtmlCell,construct:function(a,b,c){arguments.callee.base.call(this,a);
this._spanManager=new qx.ui.virtual.layer.CellSpanManager(b,c);
},members:{setCellSpan:function(L,M,N,O){var P=L+d+M;
this._spanManager.removeCell(P);

if(N>1||O>1){this._spanManager.addCell(P,L,M,N,O);
}qx.ui.core.queue.Widget.add(this);
},__tD:function(Q,R,S,T,top,U,V){var X=this._cellProvider.getCellProperties(R,S);
var W=X.insets||[0,0];
Q.push(k,h,f,T,p,n,top,p,this._getCellSizeStyle(U,V,W[0],W[1]),X.style||r,q,g,X.classes||r,q,X.attributes||r,m,X.content||r,o);
},_fullUpdate:function(s,t,u,v,w,z){var I=[];
var E=this._spanManager.findCellsInWindow(s,t,u,v);

if(E.length>0){var F=this._spanManager.getCellBounds(E,s,u);
var C=this._spanManager.computeCellSpanMap(E,s,t,u,v);
for(var i=0,l=E.length;i<l;i++){var G=E[i];
var K=F[i];
this.__tD(I,G.firstRow,G.firstColumn,K.left,K.top,K.width,K.height);
}}else{C=[];

for(var i=s;i<=t;i++){C[i]=[];
}}var J=0;
var top=0;
var B=s;
var H=u;
...
if(!C[B][H]){this.__tD(I,B,H,J,top,D,A);
}H++;
J+=D;
}top+=A;
B++;
}this.getContentElement().setAttribute(e,I.join(r));
}}});
})();
...
qx.Class.define(H,{extend:qx.core.Object,construct:function(D,E){arguments.callee.base.call(this);
{};
this._cells={};
this._invalidateSortCache();
this._invalidatePositionCache();
D.addListener(Q,this._onRowConfigChange,this);
E.addListener(Q,this._onColumnConfigChange,this);
this._rowConfig=D;
...
this._invalidateSortCache();
},removeCell:function(e){delete (this._cells[e]);
this._invalidateSortCache();
},_invalidateSortCache:function(){this._sorted={};
},_getSortedCells:function(Y){if(this._sorted[Y]){return this._sorted[Y];
}var ba=this._sorted[Y]=qx.lang.Object.getValues(this._cells);
ba.sort(function(a,b){return a[Y]<b[Y]?-1:1;
});
return ba;
},_findCellsInRange:function(bf,bg,bh,bi){var bl=this._getSortedCells(bg);

if(bl.length==0){return;
}var bk=0;
var bm=bl.length-1;
while(true){var bn=bk+((bm-bk)>>1);
var bj=bl[bn];

if(bj[bg]>=bh&&(bn==0||bl[bn-1][bg]<bh)){break;
}
if(bj[bg]>=bh){bm=bn-1;
}else{bk=bn+1;
}
...
while(bj&&bj[bg]>=bh&&bj[bg]<=bi){bf[bj.id]=bj;
bj=bl[bn++];
}},findCellsInWindow:function(v,w,x,y){var C={};
this._findCellsInRange(C,J,x,y);
this._findCellsInRange(C,O,x,y);
var A={};
this._findCellsInRange(A,I,v,w);
this._findCellsInRange(A,F,v,w);
var z=[];
for(var B in A){if(C[B]){z.push(C[B]);
}}return z;
...
return d;
},_getColumnPosition:function(W){var X=this._columnPos[W];

if(X!==undefined){return X;
}X=this._columnPos[W]=this._columnConfig.getItemPosition(W);
return X;
},_getSingleCellBounds:function(bo,bp,bq){var br={left:0,top:0,width:0,height:0};
br.height=this._getRowPosition(bo.lastRow)+this._rowConfig.getItemSize(bo.lastRow)-this._getRowPosition(bo.firstRow);
br.top=this._getRowPosition(bo.firstRow)-this._getRowPosition(bp);
br.width=this._getColumnPosition(bo.lastColumn)+this._columnConfig.getItemSize(bo.lastColumn)-this._getColumnPosition(bo.firstColumn);
br.left=this._getColumnPosition(bo.firstColumn)-this._getColumnPosition(bq);
return br;
},getCellBounds:function(bb,bc,bd){var be=[];

for(var i=0,l=bb.length;i<l;i++){be.push(this._getSingleCellBounds(bb[i],bc,bd));
}return be;
},computeCellSpanMap:function(f,g,h,j,k){var m=[];

for(var i=g;i<=h;i++){m[i]=[];
}
for(var i=0,l=f.length;i<l;i++){var q=f[i];
var o=Math.max(g,q.firstRow);
var p=Math.min(h,q.lastRow);
var u;

for(var r=o;r<=p;r++){u=m[r];
var n=Math.max(j,q.firstColumn);
var t=Math.min(k,q.lastColumn);

for(var s=n;s<=t;s++){u[s]=1;
}}}return m;
...
(function(){var z="spannedcell",y="qx.ui.virtual.layer.WidgetCellSpan",x="x";
qx.Class.define(y,{extend:qx.ui.virtual.layer.Abstract,implement:[qx.ui.virtual.core.IWidgetCellProvider],include:[qx.ui.core.MChildrenHandling],construct:function(n,o,p){arguments.callee.base.call(this);
this.setZIndex(2);
this._spanManager=new qx.ui.virtual.layer.CellSpanManager(o,p);
this._cellProvider=n;
this.__tE=[];
this._cellLayer=new qx.ui.virtual.layer.WidgetCell(this);
this._setLayout(new qx.ui.layout.Grow());
this._add(this._cellLayer);
},properties:{anonymous:{refine:true,init:false}},members:{setCellSpan:function(H,I,J,K){var L=H+x+I;
this._spanManager.removeCell(L);

if(J>1||K>1){this._spanManager.addCell(L,H,I,J,K);
}qx.ui.core.queue.Widget.add(this);
},getCellWidget:function(u,v){if(!this._spanMap[u][v]){var w=this._cellProvider.getCellWidget(u,v);
}else{var w=this.__tE.pop();

if(!w){w=new qx.ui.core.Spacer();
w.setUserData(z,1);
}}return w;
},poolCellWidget:function(M){if(M.getUserData(z)){this.__tE.push(M);
}else{this._cellProvider.poolCellWidget(M);
}},__tF:function(q,r,s,t){this._cells=this._spanManager.findCellsInWindow(q,r,s,t);

if(this._cells.length>0){this._bounds=this._spanManager.getCellBounds(this._cells,q,s);
this._spanMap=this._spanManager.computeCellSpanMap(this._cells,q,r,s,t);
}else{this._bounds=[];
this._spanMap=[];

for(var i=q;i<=r;i++){this._spanMap[i]=[];
}}},__tG:function(){var m=this.getChildren();

for(var i=m.length-1;i>=0;i--){var k=m[i];

if(k!==this._cellLayer){this._cellProvider.poolCellWidget(k);
this._remove(k);
}}
for(var i=0,l=this._cells.length;i<l;i++){var j=this._cells[i];
var g=this._bounds[i];
var h=this._cellProvider.getCellWidget(j.firstRow,j.firstColumn);

if(h){h.setUserBounds(g.left,g.top,g.width,g.height);
this._add(h);
}}},_fullUpdate:function(A,B,C,D,E,F){this.__tF(A,B,C,D);
this.__tG();
this._cellLayer.fullUpdate(A,B,C,D,E,F);
},_updateLayerWindow:function(a,b,c,d,e,f){this.__tF(a,b,c,d);
this.__tG();
this._cellLayer.updateLayerWindow(a,b,c,d,e,f);
