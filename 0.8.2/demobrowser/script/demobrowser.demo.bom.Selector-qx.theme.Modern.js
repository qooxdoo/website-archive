...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.bom.Selector", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.bom.Selector-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
var bv=qx.log.Logger;
var br=qx.bom.Collection;
br.query(bi).setStyle(e,S);
br.query(M).setStyle(bj,bh).setStyle(y,bb);
br.query(U).setStyles({"textAlign":c,"textDecoration":s,"color":p});
bv.debug("href of first link: "+br.query("a").getAttribute("href"));
br.query(a).setAttribute(b,J);
br.query(bg).addClass(Y);
br.query(bi).addListener(L,function(){alert(this.innerHTML);
});
(new qx.bom.Collection).add(g).setStyle(e,v).add(j).setStyle(bd,A).end().setStyle(C,c);
bv.debug("Found "+qx.bom.Collection.query("ul,ol").children().length+" children of ul/ol elements using children().");
bv.debug("Found "+qx.bom.Collection.query("ul,ol").children(":first-child").length+" first children of ul/ol elements using children(selector)");
bv.debug("Number of ul/ol elements which are closest to a li element: "+qx.bom.Collection.query("li").closest("ul,ol").length);
bv.debug("Number of child nodes of the body: "+new qx.bom.Collection(document.body).contents().length);
var bp=qx.bom.Collection.query(k).find(j);
bv.debug("Found "+bp.length+" li elements in uls using find()");
var bp=new qx.bom.Collection(document.body).find(bg);
bv.debug("Found "+bp.length+" headers using find()");
var bp=qx.bom.Collection.query(k).next()[0].tagName;
bv.debug("Tag following the UL element: "+bp);
var bp=qx.bom.Collection.query(k).nextAll().length;
bv.debug("Number of elements following the UL: "+bp);
var bp=qx.bom.Collection.query(j).parent().length;
bv.debug("Number of parent elements of all li elems: "+bp);
var bp=qx.bom.Collection.query(Q).parents().length;
bv.debug("Number of all parent elements of the submit button: "+bp);
var bp=qx.bom.Collection.query(k).prev()[0].tagName;
bv.debug("Tag before the UL element: "+bp);
var bp=qx.bom.Collection.query(k).prevAll().length;
bv.debug("Number of elements preceding the UL: "+bp);
var bp=qx.bom.Collection.query(O).siblings().length;
bv.debug("List Item 3 has "+bp+" siblings");
var bp=qx.bom.Collection.query(i).getOffsetParent()[0].tagName;
bv.debug("Offset parent of select box: "+bp);
var bp=qx.bom.Collection.query(i).getOffset();
bv.debug("Offset of select box: ");
bv.debug(bp);
br.query(K).eq(1).setStyle(bj,bh);
var bp=qx.bom.Collection.query(h).filter(ba);
bv.debug("Found "+bp.length+" submit buttons using filter(selector)");
var bp=qx.bom.Collection.query(h).filter(function(bm,bn,bo){return bm.type===u;
});
bv.debug("Found "+bp.length+" submit button using filter(function)");
var bp=qx.bom.Collection.query(g).hasClass(R);
bv.debug("Any H2 with form has a class 'form': "+bp);
var bp=qx.bom.Collection.query(k).find(j).is(bk);
bv.debug("Found li element with text 'List Item 1': "+bp);
var bp=qx.bom.Collection.query(k).find(j).not(bk);
bv.debug("Number of found li elements without text 'List Item 1': "+bp.length);
var bp=qx.bom.Collection.query(k).find(j).not(r);
bv.debug("Number of found li elements in the middle: "+bp.length);
...
var bs=new qx.bom.Collection(bx).setAttribute(F,d).setAttribute(z,b);
var bq=new qx.bom.Collection(bw).setAttribute(l,d).setAttribute(be,H);
qx.bom.Collection.query(i).after(bw,bx);
bs.append(o,q,t,n);
$(x).insertAfter(g);
qx.bom.Collection.id(bl).addListener(h,function(){bv.debug("Typing in e-mail field...");
});
var bt=qx.bom.Collection.id(bl);
var bu=bt.add(W).reverse();
bt.after(bu.clone(true));
...
(function(){var cN="",cM="string",cL="div",cK="previousSibling",cJ="*",cI="href",cH="e",cG="[object Array]",cF="Syntax error, unrecognized expression: ",cE="text",dv="file",du="button",dt="'/>",ds="CLASS",dr="radio",dq="script",dp="className",dn="BUTTON",dm="TAG",dl="password",cU="htmlFor",cV="reset",cS="<a href='#'></a>",cT="#",cQ="qx.bom.Selector",cR="<input name='",cO="parentNode",cP="NAME",cW="number",cX="form",dd="submit",dc="image",df="<div class='test e'></div><div class='test'></div>",de=".TEST",dh="checkbox",dg="sourceIndex",da="\\",dk="hidden",dj="<p class='TEST'></p>",di="undefined",cY="ID",db="HTML";
qx.Bootstrap.define(cQ,{statics:{query:null,matches:null}});
(function(){var dX=/((?:\((?:\([^()]+\)|[^()]+)+\)|\[(?:\[[^[\]]*\]|['"][^'"]*['"]|[^[\]'"]+)+\]|\\.|[^ >+~,(\[\\]+)+|[>+~])(\s*,\s*)?/g,ee=0,ec=Object.prototype.toString;
var dT=function(bh,bi,bj,bk){bj=bj||[];
bi=bi||document;

if(bi.nodeType!==1&&bi.nodeType!==9)return [];

if(!bh||typeof bh!==cM){return bj;
}var br=[],m,bm,bl,bt,bs,bo,bn=true;
dX.lastIndex=0;

while((m=dX.exec(bh))!==null){br.push(m[1]);

if(m[2]){bo=RegExp.rightContext;
break;
}}
if(br.length>1&&eb.exec(bh)){if(br.length===2&&dV.relative[br[0]]){bm=dU(br[0]+br[1],bi);
}else{bm=dV.relative[br[0]]?[bi]:dT(br.shift(),bi);

while(br.length){bh=br.shift();

if(dV.relative[bh])bh+=br.shift();
bm=dU(bh,bm);
}}}else{var bp=bk?
{expr:br.pop(),set:dS(bk)}:dT.find(br.pop(),br.length===1&&bi.parentNode?bi.parentNode:bi,dY(bi));
bm=dT.filter(bp.expr,bp.set);

if(br.length>0){bl=dS(bm);
}else{bn=false;
}
while(br.length){var bu=br.pop(),bq=bu;

if(!dV.relative[bu]){bu=cN;
}else{bq=br.pop();
}
if(bq==null){bq=bi;
}dV.relative[bu](bl,bq,dY(bi));
}}
if(!bl){bl=bm;
}
if(!bl){throw cF+(bu||bh);
}
if(ec.call(bl)===cG){if(!bn){bj.push.apply(bj,bl);
}else if(bi.nodeType===1){for(var i=0;bl[i]!=null;i++){if(bl[i]&&(bl[i]===true||bl[i].nodeType===1&&dW(bi,bl[i]))){bj.push(bm[i]);
}}}else{for(var i=0;bl[i]!=null;i++){if(bl[i]&&bl[i].nodeType===1){bj.push(bm[i]);
}}}}else{dS(bl,bj);
}
if(bo){dT(bo,bi,bj,bk);

if(ed){hasDuplicate=false;
bj.sort(ed);

if(hasDuplicate){for(var i=1;i<bj.length;i++){if(bj[i]===bj[i-1]){bj.splice(i--,1);
}}}}}return bj;
};
dT.matches=function(dw,dx){return dT(dw,null,null,dx);
};
dT.find=function(bG,bH,bI){var bJ,bL;

if(!bG){return [];
}
for(var i=0,l=dV.order.length;i<l;i++){var bK=dV.order[i],bL;

if((bL=dV.match[bK].exec(bG))){var bM=RegExp.leftContext;

if(bM.substr(bM.length-1)!==da){bL[1]=(bL[1]||cN).replace(/\\/g,cN);
bJ=dV.find[bK](bL,bH,bI);

if(bJ!=null){bG=bG.replace(dV.match[bK],cN);
break;
}}}}
if(!bJ){bJ=bH.getElementsByTagName(cJ);
}return {set:bJ,expr:bG};
};
dT.filter=function(L,M,N,O){var P=L,U=[],Q=M,ba,Y,R=M&&M[0]&&dY(M[0]);

while(L&&M.length){for(var X in dV.filter){if((ba=dV.match[X].exec(L))!=null){var S=dV.filter[X],W,T;
Y=false;

if(Q==U){U=[];
}
if(dV.preFilter[X]){ba=dV.preFilter[X](ba,Q,N,U,O,R);

if(!ba){Y=W=true;
}else if(ba===true){continue;
}}
if(ba){for(var i=0;(T=Q[i])!=null;i++){if(T){W=S(T,ba,i,Q);
var V=O^!!W;

if(N&&W!=null){if(V){Y=true;
}else{Q[i]=false;
}}else if(V){U.push(T);
Y=true;
}}}}
if(W!==undefined){if(!N){Q=U;
}L=L.replace(dV.match[X],cN);

if(!Y){return [];
}break;
}}}if(L==P){if(Y==null){throw cF+L;
}else{break;
}}P=L;
}return Q;
};
var dV=dT.selectors={order:[cY,cP,dm],match:{ID:/#((?:[\w\u00c0-\uFFFF_-]|\\.)+)/,CLASS:/\.((?:[\w\u00c0-\uFFFF_-]|\\.)+)/,NAME:/\[name=['"]*((?:[\w\u00c0-\uFFFF_-]|\\.)+)['"]*\]/,ATTR:/\[\s*((?:[\w\u00c0-\uFFFF_-]|\\.)+)\s*(?:(\S?=)\s*(['"]*)(.*?)\3|)\s*\]/,TAG:/^((?:[\w\u00c0-\uFFFF\*_-]|\\.)+)/,CHILD:/:(only|nth|last|first)-child(?:\((even|odd|[\dn+-]*)\))?/,POS:/:(nth|eq|gt|lt|first|last|even|odd)(?:\((\d*)\))?(?=[^-]|$)/,PSEUDO:/:((?:[\w\u00c0-\uFFFF_-]|\\.)+)(?:\((['"]*)((?:\([^\)]+\)|[^\2\(\)]*)+)\2\))?/},attrMap:{"class":dp,"for":cU},attrHandle:{href:function(r){return r.getAttribute(cI);
}},relative:{"+":function(u,v,w){var x=typeof v===cM,z=x&&!/\W/.test(v),A=x&&!z;

if(z&&!w){v=v.toUpperCase();
}
for(var i=0,l=u.length,y;i<l;i++){if((y=u[i])){while((y=y.previousSibling)&&y.nodeType!==1){}u[i]=A||y&&y.nodeName===v?y||false:y===v;
}}
if(A){dT.filter(v,u,true);
}},">":function(bv,bw,bx){var by=typeof bw===cM;

if(by&&!/\W/.test(bw)){bw=bx?bw:bw.toUpperCase();

for(var i=0,l=bv.length;i<l;i++){var bz=bv[i];

if(bz){var parent=bz.parentNode;
bv[i]=parent.nodeName===bw?parent:false;
}}}else{for(var i=0,l=bv.length;i<l;i++){var bz=bv[i];

if(bz){bv[i]=by?bz.parentNode:bz.parentNode===bw;
}}
if(by){dT.filter(bw,bv,true);
}}},"":function(fj,fk,fl){var fm=ee++,fn=ef;

if(!fk.match(/\W/)){var fo=fk=fl?fk:fk.toUpperCase();
fn=eh;
}fn(cO,fk,fm,fj,fo,fl);
},"~":function(B,C,D){var E=ee++,F=ef;

if(typeof C===cM&&!C.match(/\W/)){var G=C=D?C:C.toUpperCase();
F=eh;
}F(cK,C,E,B,G,D);
}},find:{ID:function(bA,bB,bC){if(typeof bB.getElementById!=="undefined"&&!bC){var m=bB.getElementById(bA[1]);
return m?[m]:[];
}},NAME:function(bR,bS,bT){if(typeof bS.getElementsByName!=="undefined"){var bV=[],bU=bS.getElementsByName(bR[1]);

for(var i=0,l=bU.length;i<l;i++){if(bU[i].getAttribute("name")===bR[1]){bV.push(bU[i]);
}}return bV.length===0?null:bV;
}},TAG:function(be,bf){return bf.getElementsByTagName(be[1]);
}},preFilter:{CLASS:function(dB,dC,dD,dE,dF,dG){dB=" "+dB[1].replace(/\\/g,"")+" ";

if(dG){return dB;
}
for(var i=0,dH;(dH=dC[i])!=null;i++){if(dH){if(dF^(dH.className&&(" "+dH.className+" ").indexOf(dB)>=0)){if(!dD)dE.push(dH);
}else if(dD){dC[i]=false;
}}}return false;
},ID:function(bb){return bb[1].replace(/\\/g,"");
},TAG:function(cf,cg){for(var i=0;cg[i]===false;i++){}return cg[i]&&dY(cg[i])?cf[1]:cf[1].toUpperCase();
},CHILD:function(dz){if(dz[1]=="nth"){var dA=/(-?)(\d*)n((?:\+|-)?\d*)/.exec(dz[2]=="even"&&"2n"||dz[2]=="odd"&&"2n+1"||!/\D/.test(dz[2])&&"0n+"+dz[2]||dz[2]);
dz[2]=(dA[1]+(dA[2]||1))-0;
dz[3]=dA[3]-0;
}dz[0]=ee++;
return dz;
},ATTR:function(bW,bX,bY,ca,cb,cc){var name=bW[1].replace(/\\/g,"");

if(!cc&&dV.attrMap[name]){bW[1]=dV.attrMap[name];
}
if(bW[2]==="~="){bW[4]=" "+bW[4]+" ";
}return bW;
},PSEUDO:function(eA,eB,eC,eD,eE){if(eA[1]==="not"){if(eA[3].match(dX).length>1||/^\w/.test(eA[3])){eA[3]=dT(eA[3],null,null,eB);
}else{var eF=dT.filter(eA[3],eB,eC,true^eE);

if(!eC){eD.push.apply(eD,eF);
}return false;
}}else if(dV.match.POS.test(eA[0])||dV.match.CHILD.test(eA[0])){return true;
}return eA;
},POS:function(fG){fG.unshift(true);
return fG;
}},filters:{enabled:function(fi){return fi.disabled===false&&fi.type!==dk;
},disabled:function(bF){return bF.disabled===true;
},checked:function(fd){return fd.checked===true;
},selected:function(ce){ce.parentNode.selectedIndex;
return ce.selected===true;
},parent:function(ck){return !!ck.firstChild;
},empty:function(q){return !q.firstChild;
},has:function(eO,i,eP){return !!dT(eP[3],eO).length;
},header:function(dI){return /h\d/i.test(dI.nodeName);
},text:function(eQ){return cE===eQ.type;
},radio:function(cl){return dr===cl.type;
},checkbox:function(cu){return dh===cu.type;
},file:function(fc){return dv===fc.type;
},password:function(ch){return dl===ch.type;
},submit:function(bQ){return dd===bQ.type;
},image:function(bg){return dc===bg.type;
},reset:function(cd){return cV===cd.type;
},button:function(fA){return du===fA.type||fA.nodeName.toUpperCase()===dn;
},input:function(g){return /input|select|textarea|button/i.test(g.nodeName);
}},setFilters:{first:function(fx,i){return i===0;
},last:function(bN,i,bO,bP){return i===bP.length-1;
},even:function(fb,i){return i%2===0;
},odd:function(f,i){return i%2===1;
},lt:function(fB,i,fC){return i<fC[3]-0;
},gt:function(ci,i,cj){return i>cj[3]-0;
},nth:function(fv,i,fw){return fw[3]-0==i;
},eq:function(J,i,K){return K[3]-0==i;
}},filter:{PSEUDO:function(eG,eH,i,eI){var name=eH[1],eJ=dV.filters[name];

if(eJ){return eJ(eG,i,eH,eI);
}else if(name==="contains"){return (eG.textContent||eG.innerText||"").indexOf(eH[3])>=0;
}else if(name==="not"){var eK=eH[3];

for(var i=0,l=eK.length;i<l;i++){if(eK[i]===eG){return false;
}}return true;
...
}}},ID:function(fy,fz){return fy.nodeType===1&&fy.getAttribute("id")===fz;
},TAG:function(el,em){return (em==="*"&&el.nodeType===1)||el.nodeName===em;
},CLASS:function(bD,bE){return (" "+(bD.className||bD.getAttribute("class"))+" ").indexOf(bE)>-1;
},ATTR:function(h,j){var name=j[1],p=dV.attrHandle[name]?dV.attrHandle[name](h):h[name]!=null?h[name]:h.getAttribute(name),o=p+"",n=j[2],k=j[4];
return p==null?n==="!=":n==="="?o===k:n==="*="?o.indexOf(k)>=0:n==="~="?(" "+o+" ").indexOf(k)>=0:!k?o&&p!==false:n==="!="?o!=k:n==="^="?o.indexOf(k)===0:n==="$="?o.substr(o.length-k.length)===k:n==="|="?o===k||o.substr(0,k.length+1)===k+"-":false;
},POS:function(cm,cn,i,co){var name=cn[2],cp=dV.setFilters[name];

if(cp){return cp(cm,i,cn,co);
}}}};
var eb=dV.match.POS;

for(var eg in dV.match){dV.match[eg]=RegExp(dV.match[eg].source+/(?![^\[]*\])(?![^\(]*\))/.source);
}var dS=function(ft,fu){ft=Array.prototype.slice.call(ft);
...
}catch(e){dS=function(ei,ej){var ek=ej||[];

if(ec.call(ei)===cG){Array.prototype.push.apply(ek,ei);
}else{if(typeof ei.length===cW){for(var i=0,l=ei.length;i<l;i++){ek.push(ei[i]);
}}else{for(var i=0;ei[i];i++){ek.push(ei[i]);
}}}return ek;
};
}var ed;

if(document.documentElement.compareDocumentPosition){ed=function(a,b){var H=a.compareDocumentPosition(b)&4?-1:a===b?0:1;

if(H===0){hasDuplicate=true;
}return H;
};
}else if(dg in document.documentElement){ed=function(a,b){var ew=a.sourceIndex-b.sourceIndex;

if(ew===0){hasDuplicate=true;
}return ew;
};
}else if(document.createRange){ed=function(a,b){var cr=a.ownerDocument.createRange(),cs=b.ownerDocument.createRange();
cr.selectNode(a);
cr.collapse(true);
cs.selectNode(b);
cs.collapse(true);
...
if(cq===0){hasDuplicate=true;
}return cq;
};
}(function(){var ep=document.createElement(cX),eo=dq+(new Date).getTime();
ep.innerHTML=cR+eo+dt;
var en=document.documentElement;
en.insertBefore(ep,en.firstChild);
if(!!document.getElementById(eo)){dV.find.ID=function(c,d,e){if(typeof d.getElementById!=="undefined"&&!e){var m=d.getElementById(c[1]);
return m?m.id===c[1]||typeof m.getAttributeNode!=="undefined"&&m.getAttributeNode("id").nodeValue===c[1]?[m]:undefined:[];
}};
dV.filter.ID=function(ex,ey){var ez=typeof ex.getAttributeNode!=="undefined"&&ex.getAttributeNode("id");
return ex.nodeType===1&&ez&&ez.nodeValue===ey;
};
}en.removeChild(ep);
})();
(function(){var ct=document.createElement(cL);
ct.appendChild(document.createComment(cN));
if(ct.getElementsByTagName(cJ).length>0){dV.find.TAG=function(fp,fq){var fs=fq.getElementsByTagName(fp[1]);
if(fp[1]==="*"){var fr=[];

for(var i=0;fs[i];i++){if(fs[i].nodeType===1){fr.push(fs[i]);
}}fs=fr;
}return fs;
};
}ct.innerHTML=cS;

if(ct.firstChild&&typeof ct.firstChild.getAttribute!==di&&ct.firstChild.getAttribute(cI)!==cT){dV.attrHandle.href=function(I){return I.getAttribute(cI,2);
};
}})();

if(document.querySelectorAll)(function(){var fF=dT,fE=document.createElement(cL);
fE.innerHTML=dj;
if(fE.querySelectorAll&&fE.querySelectorAll(de).length===0){return;
}dT=function(fe,ff,fg,fh){ff=ff||document;
if(!fh&&ff.nodeType===9&&!dY(ff)){try{return dS(ff.querySelectorAll(fe),fg);
}catch(e){}}return fF(fe,ff,fg,fh);
};
dT.find=fF.find;
dT.filter=fF.filter;
dT.selectors=fF.selectors;
dT.matches=fF.matches;
})();

if(document.getElementsByClassName&&document.documentElement.getElementsByClassName)(function(){var fD=document.createElement(cL);
fD.innerHTML=df;
if(fD.getElementsByClassName(cH).length===0)return;
fD.lastChild.className=cH;

if(fD.getElementsByClassName(cH).length===1)return;
dV.order.splice(1,0,ds);
dV.find.CLASS=function(eL,eM,eN){if(typeof eM.getElementsByClassName!=="undefined"&&!eN){return eM.getElementsByClassName(eL[1]);
}};
})();
function eh(cv,cw,cx,cy,cz,cA){var cB=cv==cK&&!cA;

for(var i=0,l=cy.length;i<l;i++){var cD=cy[i];

if(cD){if(cB&&cD.nodeType===1){cD.sizcache=cx;
cD.sizset=i;
}cD=cD[cv];
var cC=false;

while(cD){if(cD.sizcache===cx){cC=cy[cD.sizset];
break;
}
if(cD.nodeType===1&&!cA){cD.sizcache=cx;
cD.sizset=i;
}
if(cD.nodeName===cw){cC=cD;
break;
}cD=cD[cv];
}cy[i]=cC;
}}}function ef(dJ,dK,dL,dM,dN,dO){var dP=dJ==cK&&!dO;

for(var i=0,l=dM.length;i<l;i++){var dR=dM[i];

if(dR){if(dP&&dR.nodeType===1){dR.sizcache=dL;
dR.sizset=i;
}dR=dR[dJ];
var dQ=false;

while(dR){if(dR.sizcache===dL){dQ=dM[dR.sizset];
break;
}
if(dR.nodeType===1){if(!dO){dR.sizcache=dL;
dR.sizset=i;
}
if(typeof dK!==cM){if(dR===dK){dQ=true;
break;
}}else if(dT.filter(dK,[dR]).length>0){dQ=dR;
break;
}}dR=dR[dJ];
}dM[i]=dQ;
}}}var dW=document.compareDocumentPosition?
function(a,b){return a.compareDocumentPosition(b)&16;
}:
function(a,b){return a!==b&&(a.contains?a.contains(b):true);
};
var dY=function(dy){return dy.nodeType===9&&dy.documentElement.nodeName!==db||!!dy.ownerDocument&&dY(dy.ownerDocument);
};
var dU=function(eq,er){var et=[],es=cN,ev,eu=er.nodeType?[er]:er;
while((ev=dV.match.PSEUDO.exec(eq))){es+=ev[0];
eq=eq.replace(dV.match.PSEUDO,cN);
}eq=dV.relative[eq]?eq+cJ:eq;

for(var i=0,l=eu.length;i<l;i++){dT(eq,eu[i],et);
}return dT.filter(es,et);
};
var ea=qx.bom.Selector;
ea.query=function(s,t){return dT(s,t);
};
ea.matches=function(bc,bd){return dT(bc,null,null,bd);
};
})();
})();
...
},has:function(k,name){var l=new RegExp(f+name+g);
...
(function(){var bk="get",bj="set",bi="reset",bh=":not(",bg="getValue",bf="append",be=")",bd="getPreviousSiblings",bc="getOffsetParent",bb="qx.bom.Collection",bI="setValue",bH="prepend",bG="string",bF="getAncestors",bE="#",bD="remove",bC=">*",bB="add",bA="*",bz="",br="addListener",bs="has",bp="toggle",bq="getSiblings",bn="replace",bo="after",bl="replaceWith",bm="setCss",bt="setStyles",bu="before",bw="getNextSiblings",bv="getPosition",by="getCss",bx="removeListener";
(function(){var m=function(a,b){return function(cY,da,db,dc,dd,de){var length=this.length;

if(length>0){var df=a[b];

for(var i=0;i<length;i++){if(this[i].nodeType===1){df.call(a,this[i],cY,da,db,dc,dd,de);
}}}return this;
};
};
var k=function(bJ,bK){return function(cO,cP,cQ,cR,cS,cT){if(this.length>0){var cU=this[0].nodeType===1?bJ[bK](this[0],cO,cP,cQ,cR,cS,cT):null;

if(cU&&cU.nodeType){return this.__vX([cU]);
}else{return cU;
}}return null;
};
};
qx.Class.define(bb,{extend:qx.type.BaseArray,statics:{query:function(T,U){var V=qx.bom.Selector.query(T,U);
return qx.lang.Array.cast(V,qx.bom.Collection);
},id:function(r){var s=document.getElementById(r);
if(s&&s.id!=r){return qx.bom.Collection.query(bE+r);
}return new qx.bom.Collection(s);
},html:function(cV,cW){var cX=qx.bom.Html.clean([cV],cW);
return qx.lang.Array.cast(cX,qx.bom.Collection);
},__vU:/^[^<]*(<(.|\s)+>)[^>]*$|^#([\w-]+)$/,create:function(bT,bU){var bW=qx.bom.Collection;
if(bT.nodeType){return new bW(bT);
}else if(typeof bT===bG){var bV=bW.__vU.exec(bT);

if(bV){return bV[1]?bW.html(bV[1],bU):bW.id(bV[3].substring(1));
}else{return bW.query(bT,bU);
}}else{return qx.lang.Array.cast(bT,qx.bom.Collection);
}}},members:{setAttribute:m(qx.bom.element.Attribute,bj),resetAttribute:m(qx.bom.element.Attribute,bi),getAttribute:k(qx.bom.element.Attribute,bk),addClass:m(qx.bom.element.Class,bB),getClass:k(qx.bom.element.Class,bk),hasClass:k(qx.bom.element.Class,bs),removeClass:m(qx.bom.element.Class,bD),replaceClass:m(qx.bom.element.Class,bn),toggleClass:m(qx.bom.element.Class,bp),setValue:m(qx.bom.Input,bI),getValue:k(qx.bom.Input,bg),setStyle:m(qx.bom.element.Style,bj),setStyles:m(qx.bom.element.Style,bt),resetStyle:m(qx.bom.element.Style,bi),getStyle:k(qx.bom.element.Style,bk),setCss:m(qx.bom.element.Style,bm),getCss:m(qx.bom.element.Style,by),getOffset:k(qx.bom.element.Location,bk),getPosition:k(qx.bom.element.Location,bv),getOffsetParent:k(qx.bom.element.Location,bc),setScrollLeft:function(K){var Node=qx.dom.Node;
...
},setScrollTop:function(dn){var Node=qx.dom.Node;

for(var i=0,l=this.length,dp;i<l;i++){dp=this[i];

if(Node.isElement(dp)){dp.scrollTop=dn;
}else if(Node.isWindow(dp)){dp.scrollTo(this.getScrollLeft(dp),dn);
}else if(Node.isDocument(dp)){Node.getWindow(dp).scrollTo(this.getScrollLeft(dp),dn);
}}return this;
...
},getScrollTop:function(){var bS=this[0];

if(!bS){return null;
}var Node=qx.dom.Node;

if(Node.isWindow(bS)||Node.isDocument(bS)){return qx.bom.Viewport.getScrollTop();
}return bS.scrollTop;
},getWidth:function(){var dq=this[0];
var Node=qx.dom.Node;

if(dq){if(Node.isElement(dq)){return qx.bom.element.Dimension.getWidth(dq);
}else if(Node.isDocument(dq)){return qx.bom.Document.getWidth(Node.getWindow(dq));
}else if(Node.isWindow(dq)){return qx.bom.Viewport.getWidth(dq);
}}return null;
},getContentWidth:function(){var f=this[0];

if(qx.dom.Node.isElement(f)){return qx.bom.element.Dimension.getContentWidth(f);
}return null;
},getHeight:function(){var Y=this[0];
var Node=qx.dom.Node;

if(Y){if(Node.isElement(Y)){return qx.bom.element.Dimension.getHeight(Y);
}else if(Node.isDocument(Y)){return qx.bom.Document.getHeight(Node.getWindow(Y));
}else if(Node.isWindow(Y)){return qx.bom.Viewport.getHeight(Y);
}}return null;
},getContentHeight:function(){var t=this[0];

if(qx.dom.Node.isElement(t)){return qx.bom.element.Dimension.getContentHeight(t);
}return null;
},addListener:m(qx.bom.Element,br),removeListener:m(qx.bom.Element,bx),eq:function(cj){return this.slice(cj,+cj+1);
},filter:function(cL,cM){var cN;

if(qx.lang.Function.isFunction(cL)){cN=qx.type.BaseArray.prototype.filter.call(this,cL,cM);
}else{cN=qx.bom.Selector.matches(cL,this);
}return this.__vX(cN);
},is:function(bX){return !!bX&&qx.bom.Selector.matches(bX,this).length>0;
},__vV:/^.[^:#\[\.,]*$/,not:function(Q){if(this.__vV.test(Q)){var R=qx.bom.Selector.matches(bh+Q+be,this);
return this.__vX(R);
}var R=qx.bom.Selector.matches(Q,this);
return this.filter(function(cv){return R.indexOf(cv)===-1;
});
},add:function(n,o){var p=qx.bom.Selector.query(n,o);
var q=qx.lang.Array.unique(this.concat(p));
return this.__vX(q);
...
},closest:function(dv){var dw=new qx.bom.Collection(1);
var dy=qx.bom.Selector;
var dx=this.map(function(J){while(J&&J.ownerDocument){dw[0]=J;

if(dy.matches(dv,dw).length>0){return J;
}J=J.parentNode;
}});
return this.__vX(qx.lang.Array.unique(dx));
},contents:function(){var ci=[];
var ch=qx.lang.Array;

for(var i=0,l=this.length;i<l;i++){ci.push.apply(ci,ch.fromCollection(this[i].childNodes));
}return this.__vX(ci);
},find:function(ds){var du=qx.bom.Selector;
if(this.length===1){return this.__vX(du.query(ds,this[0]));
}else{var dt=[];

for(var i=0,l=this.length;i<l;i++){dt.push.apply(dt,du.query(ds,this[i]));
}return this.__vX(qx.lang.Array.unique(dt));
}},next:function(F){var G=qx.dom.Hierarchy;
var H=this.map(G.getNextElementSibling,G);
if(F){H=qx.bom.Selector.matches(F,H);
}return this.__vX(H);
},nextAll:function(cu){return this.__vW(bw,cu);
},prev:function(cI){var cJ=qx.dom.Hierarchy;
var cK=this.map(cJ.getPreviousElementSibling,cJ);
if(cI){cK=qx.bom.Selector.matches(cI,cK);
}return this.__vX(cK);
},prevAll:function(y){return this.__vW(bd,y);
},parent:function(z){var Element=qx.dom.Element;
var A=qx.lang.Array.unique(this.map(Element.getParentElement,Element));
if(z){A=qx.bom.Selector.matches(z,A);
}return this.__vX(A);
},parents:function(co){return this.__vW(bF,co);
},siblings:function(v){return this.__vW(bq,v);
},__vW:function(cp,cq){var cs=[];
var cr=qx.dom.Hierarchy;

for(var i=0,l=this.length;i<l;i++){cs.push.apply(cs,cr[cp](this[i]));
}var ct=qx.lang.Array.unique(cs);
if(cq){ct=qx.bom.Selector.matches(cq,ct);
}return this.__vX(ct);
},__vX:function(cm){var cn=new qx.bom.Collection;
cn.__vY=this;
cm=Array.prototype.slice.call(cm,0);
cn.push.apply(cn,cm);
return cn;
},andSelf:function(){return this.add(this.__vY);
},end:function(){return this.__vY||new qx.bom.Collection();
},__wa:function(cw,cx){var cC=this[0];
var cB=cC.ownerDocument||cC;
var cA=cB.createDocumentFragment();
var cE=qx.bom.Html.clean(cw,cB,cA);
var cG=cA.firstChild;
if(cG){var cy=this.length-1;

for(var i=0,l=cy;i<l;i++){cx.call(this,this[i],cA.cloneNode(true));
}cx.call(this,this[cy],cA);
}if(cE){var cz;
var cF=qx.io2.ScriptLoader;
var cD=qx.lang.Function;

for(var i=0,l=cE.length;i<l;i++){cz=cE[i];
if(cz.src){cF.get().load(cz.src);
}else{cD.globalEval(cz.text||cz.textContent||cz.innerHTML||bz);
}if(cz.parentNode){cz.parentNode.removeChild(cz);
}}}return this;
},__wb:function(bM,bN){var bP=qx.bom.Selector;
var bO=qx.lang.Array;
var bR=[];

for(var i=0,l=bM.length;i<l;i++){{};
bR.push.apply(bR,bP.query(bM[i]));
}bR=bO.cast(bO.unique(bR),qx.bom.Collection);
for(var i=0,bQ=this.length;i<bQ;i++){bR[bN](this[i]);
}return this;
},append:function(c){return this.__wa(arguments,this.__wc);
},prepend:function(dr){return this.__wa(arguments,this.__wd);
},__wc:function(B,C){B.appendChild(C);
},__wd:function(ck,cl){ck.insertBefore(cl,ck.firstChild);
},appendTo:function(S){return this.__wb(arguments,bf);
},prependTo:function(g){return this.__wb(arguments,bH);
},before:function(bL){return this.__wa(arguments,this.__we);
},after:function(X){return this.__wa(arguments,this.__wf);
},__we:function(cf,cg){cf.parentNode.insertBefore(cg,cf);
},__wf:function(cd,ce){cd.parentNode.insertBefore(ce,cd.nextSibling);
},insertBefore:function(bY){return this.__wb(arguments,bu);
},insertAfter:function(d){return this.__wb(arguments,bo);
},wrapAll:function(content){var cc=this[0];

if(cc){var cb=qx.bom.Collection.create(content,cc.ownerDocument).clone();
if(cc.parentNode){cc.parentNode.insertBefore(cb[0],cc);
}cb.map(this.__wg).append(this);
}return this;
},__wg:function(cH){while(cH.firstChild){cH=cH.firstChild;
}return cH;
...
},replaceAll:function(u){return this.__wb(arguments,bl);
},remove:function(M){var O=this;

if(M){O=this.filter(M);

if(O.length==0){return this;
}}for(var i=0,P=O.length,N;i<P;i++){N=O[i];

if(N.parentNode){N.parentNode.removeChild(N);
}}return O;
},destroy:function(dg){if(this.length==0){return this;
}var di=qx.bom.Selector;
var dl=this;

if(dg){dl=this.filter(dg);

if(dl.length==0){return this;
}}var dk=qx.event.Registration.getManager(this[0]);

for(var i=0,l=dl.length,dj,dm;i<l;i++){dj=dl[i];
dk.removeAllListeners(dj);
dm=di.query(bA,dj);

for(var j=0,dh=dm.length;j<dh;j++){dk.removeAllListeners(dm[j]);
}if(dj.parentNode){dj.parentNode.removeChild(dj);
}}if(dg){dl.end();
qx.lang.Array.exclude(this,dl);
}else{this.length=0;
}return this;
},empty:function(){var ba=qx.bom.Selector;

for(var i=0,l=this.length;i<l;i++){ba.query(bC,this[i]).destroy();
while(this.firstChild){this.removeChild(this.firstChild);
}}return this;
},clone:function(I){var Element=qx.bom.Element;
return I?
this.map(function(e){return Element.clone(e,true);
}):this.map(Element.clone,Element);
}},defer:function(D,E){if(window.$==null){window.$=D.create;
}}});
})();
})();
...
(function(){var v="qx.client",u="string",t="script",s="<table>",r="<fieldset>",q="<select multiple='multiple'>",p="</div>",o="</select>",n="</tr></tbody></table>",m="<col",S="div",R="<table><tbody><tr>",Q=">",P="<table><tbody></tbody><colgroup>",O="<th",N="</tbody></table>",M="<td",L="</colgroup></table>",K="<opt",J="text/javascript",C="",D="</fieldset>",A="<table><tbody>",B="div<div>",y="<table",z="mshtml",w="qx.bom.Html",x="<leg",E="tbody",F="<tr",H="</table>",G="undefined",I="></";
qx.Class.define(w,{statics:{__wh:function(T,U,V){return V.match(/^(abbr|br|col|img|input|link|meta|param|hr|area|embed)$/i)?T:U+I+V+Q;
},__wi:{opt:[1,q,o],leg:[1,r,D],table:[1,s,H],tr:[2,A,N],td:[3,R,n],col:[2,P,L],def:qx.core.Variant.select(v,{"mshtml":[1,B,p],"default":null})},__wj:function(a,b){var h=b.createElement(S);
a=a.replace(/(<(\w+)[^>]*?)\/>/g,this.__wh);
var d=a.replace(/^\s+/,C).substring(0,5).toLowerCase();
var g,c=this.__wi;

if(!d.indexOf(K)){g=c.opt;
}else if(!d.indexOf(x)){g=c.leg;
}else if(d.match(/^<(thead|tbody|tfoot|colg|cap)/)){g=c.table;
}else if(!d.indexOf(F)){g=c.tr;
}else if(!d.indexOf(M)||!d.indexOf(O)){g=c.td;
}else if(!d.indexOf(m)){g=c.col;
}else{g=c.def;
}if(g){h.innerHTML=g[1]+a+g[2];
var f=g[0];

while(f--){h=h.lastChild;
}}else{h.innerHTML=a;
}if(qx.core.Variant.isSet(v,z)){var k=/<tbody/i.test(a);
var e=!d.indexOf(y)&&!k?h.firstChild&&h.firstChild.childNodes:g[1]==s&&!k?h.childNodes:[];

for(var j=e.length-1;j>=0;--j){if(e[j].tagName.toLowerCase()===E&&!e[j].childNodes.length){e[j].parentNode.removeChild(e[j]);
}}if(/^\s/.test(a)){h.insertBefore(b.createTextNode(a.match(/^\s*/)[0]),h.firstChild);
}}return qx.lang.Array.fromCollection(h.childNodes);
},clean:function(W,X,Y){X=X||document;
if(typeof X.createElement===G){X=X.ownerDocument||X[0]&&X[0].ownerDocument||document;
}if(!Y&&W.length===1&&typeof W[0]===u){var bg=/^<(\w+)\s*\/?>$/.exec(W[0]);

if(bg){return [X.createElement(bg[1])];
}}var ba,bc=[];

for(var i=0,l=W.length;i<l;i++){ba=W[i];
if(typeof ba===u){ba=this.__wj(ba,X);
...
if(bd.nodeType==1&&bd.tagName.toLowerCase()===t&&(!bd.type||bd.type.toLowerCase()===J)){if(bd.parentNode){bd.parentNode.removeChild(bc[i]);
}bf.push(bd);
}else{if(bd.nodeType===1){bb=be.fromCollection(bd.getElementsByTagName(t));
