...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.virtual.ListBinding", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.virtual.ListBinding-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
(function(){var j="Affe ",h="demobrowser.demo.virtual.ListBinding";
...
for(var i=0;i<10000;i++){model.push(j+i);
}model=new qx.data.Array(model);
var d=new qx.ui.virtual.form.ListController(model,b);
var g=new qx.ui.virtual.form.List().set({useWidgetCells:true});
this.getRoot().add(g,{left:500,top:10});
var c=new qx.ui.virtual.form.ListController(d.getSelection(),g);
...
(function(){var L="row-layer",K="selected",J="grid-lines",I="Integer",H="cell.row",G="changeSelection",F="cell.renderer",E="_applyDelegate",D="Boolean",C="qx.ui.virtual.form.List",bc="_changeShowGridLines",bb="Object",ba="_applyCellRenderer",Y="changeRowHeight",X="_applyRowCount",W="Booelean",V="changeCellRenderer",U="qx.event.type.Data",T="resize",S="changeShowGridLines",Q="changeDelegate",R="_applyUseWidgetCells",O="changeRowCount",P="_applyRowHeight",M="horizontal",N="virtual-list";
qx.Class.define(C,{extend:qx.ui.virtual.core.Scroller,construct:function(l){arguments.callee.base.call(this,0,1,20,100);
...
this.initRowHeight();
this.initDelegate();
},events:{"changeSelection":U},properties:{appearance:{refine:true,init:N},rowCount:{check:I,event:O,init:0,apply:X},rowHeight:{check:I,event:Y,init:20,apply:P,themeable:true},showGridLines:{check:W,event:S,init:false,apply:bc,themeable:true},useWidgetCells:{check:D,init:false,apply:R,themeable:true},cellRenderer:{event:V,apply:ba,themeable:true},delegate:{check:bb,event:Q,init:null,nullable:true,apply:E}},members:{__st:null,_initWidgetLayer:function(){var self=this;
var j={getCellWidget:function(bd,be){var bi=self._getCellData(bd);

if(!bi){return null;
}var bf={};

if(self.__sv.isItemSelected(bd)){bf.selected=true;
}var bg=self._getCellRenderer(bd);
var bh=bg.getCellWidget(bi,bf);
bh.setUserData(H,bd);
bh.setUserData(F,bg);
return bh;
},poolCellWidget:function(a){var b=a.getUserData(F);
b.pool(a);
}};
this._showChildControl(L);
this.__su=new qx.ui.virtual.layer.WidgetCell(j);
this.getPane().addLayer(this.__su);

if(!this.__st){this.setCellRenderer(qx.ui.virtual.form.ListItemCell.getInstance());
}},_initHtmlLayer:function(){var self=this;
var bm={getCellProperties:function(c,d){var f={};

if(self.__sv.isItemSelected(c)){f.selected=true;
}return self._getCellRenderer(c).getCellProperties(self._getCellData(c),f);
}};
this._showChildControl(L);
this.__su=new qx.ui.virtual.layer.HtmlCell(bm);
this.getPane().addLayer(this.__su);

if(!this.__st){this.setCellRenderer(new qx.ui.virtual.cell.Cell());
}},_initSelectionManager:function(){var self=this;
var bl={isItemSelectable:function(bs){return self._delegate.isRowSelectable?self._delegate.isRowSelectable(bs):true;
},styleSelectable:function(bn,bo,bp){if(self.__sw){self._styleWidgetSelectable(bn,bo,bp);
}else{self._styleHtmlSelectable(bn,bo,bp);
}}};
this.__sv=new qx.ui.virtual.selection.Row(this.getPane(),bl);
this.__sv.attachMouseEvents(this.getPane());
this.__sv.attachKeyEvents(this);
this.__sv.addListener(G,function(e){this.fireDataEvent(G,e.getData());
},this);
},getSelectionManager:function(){return this.__sv;
},_createChildControlImpl:function(m){var n;

switch(m){case L:n=new qx.ui.virtual.layer.Row(null,null);
this.getPane().addLayer(n);
break;
case J:n=new qx.ui.virtual.layer.GridLines(M);
this.getPane().addLayer(n);
break;
}return n||arguments.callee.base.call(this,m);
},update:function(){this.__su.updateLayerData();
},_applyRowCount:function(bj,bk){this.getPane().getRowConfig().setItemCount(bj);
},_applyRowHeight:function(g,h){this.getPane().getRowConfig().setDefaultItemSize(g);
},_changeShowGridLines:function(bv,bw){if(bv){this._showChildControl(J);
}else{this._excludeChildControl(J);
}},_applyDelegate:function(A,B){this._delegate=A||{};
},_applyUseWidgetCells:function(bt,bu){if(this.__sw!==undefined){throw new Error("The property 'useWidgetCells' cannot be set after the list has "+"been rendered.");
}},_applyCellRenderer:function(bq,br){this.__st=bq;

if(this.__su){this.__su.fullUpdate();
}},_getCellData:function(k){return this._delegate.getCellData?this._delegate.getCellData(k):null;
},_getCellRenderer:function(o){return this._delegate.getCellRenderer?this._delegate.getCellRenderer(o):this.__st;
},_styleHtmlSelectable:function(p,q,r){if(q!==K){return;
}var s=this.getChildControl(L);

if(r){s.setDecorator(p,K);
}else{s.setDecorator(p,null);
}this.__su.updateLayerData();
},_styleWidgetSelectable:function(t,u,v){if(u!==K){return;
}var w=this.__su.getChildren();

for(var i=0;i<w.length;i++){var x=w[i];
var z=x.getUserData(H);
...
}}},syncWidget:function(){if(this.__sw!==undefined){return;
}this.__sw=this.getUseWidgetCells();

if(this.__sw){this._initWidgetLayer();
...
(function(){var l="cell.states",k="qx.ui.virtual.cell.AbstractWidget";
qx.Class.define(k,{extend:qx.core.Object,implement:[qx.ui.virtual.cell.IWidgetCell],construct:function(){arguments.callee.base.call(this);
this.__ta=[];
},members:{_createWidget:function(){throw new Error("abstract method call");
},updateData:function(a,b){throw new Error("abstract method call");
},updateStates:function(f,g){var j=f.getUserData(l);
if(j){var h=g||{};
...
},getCellWidget:function(c,d){var e=this.__ta.pop()||this._createWidget();
this.updateStates(e,d);
this.updateData(e,c);
return e;
},pool:function(m){this.__ta.push(m);
}}});
})();
...
(function(){var r="changeSelection",q="qx.data.IListData",p="changeModel",o="",n="qx.ui.virtual.form.ListController",m="change",l="_applyTarget",k="_applySelection",j="changeTarget",h="changeLength",g="_applyModel";
qx.Class.define(n,{extend:qx.core.Object,construct:function(x,y){arguments.callee.base.call(this);
this.setSelection(new qx.data.Array());

if(x!=null){this.setModel(x);
}
if(y!=null){this.setTarget(y);
}},properties:{target:{event:j,nullable:true,init:null,apply:l},model:{check:q,event:p,nullable:true,init:null,apply:g},selection:{check:q,event:r,apply:k}},members:{_applyTarget:function(u,v){if(u!=null){u.setDelegate(this);
this.__tu=u.addListener(r,this.__tx,this);
}
if(v!=null){v.setDelegate(null);
v.removeListenerById(this.__tu);
}
if(this.getModel()==null){return;
}this._syncRowCount();
},_applyModel:function(A,B){if(A!=null){this.__tv=A.addListener(h,this._onChangeLengthModel,this);
this.__tw=A.addListener(m,this._onChangeModel,this);
}
if(B!=null){B.removeListenerById(this.__tv);
B.removeListenerById(this.__tw);
}
if(this.getTarget()!=null){this._syncRowCount();
}},_applySelection:function(s,t){},__tx:function(e){var f=e.getData();
var d=[];

for(var i=0;i<f.length;i++){var c=this.getModel().getItem(f[i]);
d.push(c);
}d.unshift(this.getSelection().length);
d.unshift(0);
this.getSelection().splice.apply(this.getSelection(),d);
},_onChangeLengthModel:function(e){this._syncRowCount();
},_onChangeModel:function(e){var w=this.getTarget();

if(w!=null){w.update();
}},_syncRowCount:function(){var z=this.getModel();
var length=z?z.length:0;
this.getTarget().setRowCount(length);
},getCellData:function(a){var b=this.getModel();
return (b?b.getItem(a):o);
}}});
})();
...
},updateData:function(f,g){f.label.setContent(g.getName());
...
(function(){var k="offline",j="String",h="Friends",g="away",f="online",e="busy",d="Christian Schmidt",c="User #",b="qooxdoo",a="smile",N="Jonathan Weiß",M=".png",L="kiss",K="changeStatus",J="icon/22/emotes/face-smile.png",I="plain",H="smile-big",G="surprise",F="raspberry",E="sad",r="embarrassed",s="Alexander Back",p="Fabian Jakobs",q="demobrowser/demo/icons/imicons/fabian_jakobs.png",n="Daniel Wagner",o="demobrowser/demo/icons/imicons/christian_schmidt.png",l="(unnamed)",m="Andreas Ecker",t="laugh",u="Thomas Herchenröder",x="changeGroup",w="demobrowser/demo/icons/imicons/martin_wittemann.png",z="changeName",y="angel",B="demobrowser.demo.virtual.messenger.BuddyModel",A="icon/22/emotes/face-",v="demobrowser/demo/icons/imicons/jonathan_weiss.png",D="changeAvatar",C="Martin Wittemann";
qx.Class.define(B,{extend:qx.core.Object,properties:{name:{init:l,event:z,check:j},avatar:{init:J,event:D,check:j},status:{init:k,event:K,check:[g,e,f,k]},group:{init:h,event:x,check:j}},statics:{createBuddies:function(O){var Q=[{name:s,img:this.getRandomBuddy(),statusIcon:this.getRandomStatus()},{name:p,img:q,statusIcon:this.getRandomStatus()},{name:m,img:this.getRandomBuddy(),statusIcon:this.getRandomStatus()},{name:C,img:w,statusIcon:this.getRandomStatus()},{name:u,img:this.getRandomBuddy(),statusIcon:this.getRandomStatus()},{name:n,img:this.getRandomBuddy(),statusIcon:this.getRandomStatus()},{name:N,img:v,statusIcon:this.getRandomStatus()},{name:d,img:o,statusIcon:this.getRandomStatus()}];

for(var i=0;i<Q.length;i++){Q[i].group=b;
}for(var i=Q.length;i<O;i++){Q[i]={name:c+i,img:this.getRandomBuddy(),statusIcon:this.getRandomStatus(),group:h};
}var P=[];

for(var i=0;i<Q.length;i++){var R=new demobrowser.demo.virtual.messenger.BuddyModel().set({name:Q[i].name,avatar:Q[i].img,status:Q[i].statusIcon,group:Q[i].group});
P.push(R);
}return new qx.data.Array(P);
},getRandomBuddy:function(){var T=[y,r,L,t,I,F,E,H,a,G];
return A+T[Math.floor(Math.random()*T.length)]+M;
},getRandomStatus:function(){var S=[g,e,f,k];
