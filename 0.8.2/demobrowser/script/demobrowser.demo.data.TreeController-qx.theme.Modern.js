...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.data.TreeController-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var bv="model",bu="get",bt="String",bs="changeModel",br="_applyDelegate",bq="qx.core.Object",bp="_applyLabelPath",bo="icon",bn="change",bm="label",bg="_applyTarget",bl="changeTarget",bj="_applyIconPath",bf="_applyLabelOptions",be="_applyChildPath",bi="_applyIconOptions",bh="qx.data.controller.Tree",bk="_applyModel";
qx.Class.define(bh,{extend:qx.core.Object,include:qx.data.controller.MSelection,construct:function(q,r,s,t){arguments.callee.base.call(this);
this.__oj={};
this.__ok=[];
this.__ol={};
...
}},properties:{model:{check:bq,apply:bk,event:bs,nullable:true},target:{apply:bg,event:bl,init:null},childPath:{check:bt,apply:be,nullable:true},labelPath:{check:bt,apply:bp,nullable:true},iconPath:{check:bt,apply:bj,nullable:true},labelOptions:{apply:bf,nullable:true},iconOptions:{apply:bi,nullable:true},delegate:{apply:br,init:null,nullable:true}},members:{_applyDelegate:function(a,b){this._setConfigureItem(a,b);
this._setCreateItem(a,b);
this._setBindItem(a,b);
},_applyIconOptions:function(D,E){this.__os();
},_applyLabelOptions:function(L,M){this.__os();
},_applyTarget:function(I,J){if(J!=undefined){var K=J.getRoot();
J.setRoot(null);
K.destroy();
}if(this.getModel()!=null){this.__on();
}this._addChangeTargetListener(I,J);
},_applyModel:function(u,v){this.__on();
},_applyChildPath:function(bD,bE){this.__on();
},_applyIconPath:function(B,C){this.__os();
},_applyLabelPath:function(F,G){this.__os();
},__om:function(f){var h=f.getTarget();
var g=this.__ol[h.toHashCode()].treeNode;
var k=this.__ol[h.toHashCode()].modelNode;
this.__oo(g,k);
this._updateSelection();
},_createItem:function(){var bx=this.getDelegate();
if(bx!=null&&bx.createItem!=null){var bw=bx.createItem();
}else{var bw=new qx.ui.tree.TreeFolder();
...
}var P=this.getTarget().getRoot();

if(P!=null){P.destroy();
}var Q=this._createItem();
Q.setUserData(bv,this.getModel());
this.getTarget().setRoot(Q);
this.__ot(this.getModel(),Q);
this.__oo(Q,this.getModel());
},__oo:function(U,V){if(V[bu+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;
}var bc=V[bu+qx.lang.String.firstUp(this.getChildPath())]();
if(this.__ol[bc.toHashCode()]==undefined){var ba=bc.addListener(bn,this.__om,this);
this.__ol[bc.toHashCode()]={modelNode:V,treeNode:U,changeListenerId:ba};
}for(var i=0;i<bc.length;i++){if(U.getChildren()[i]==null||bc.getItem(i)!=U.getChildren()[i].getUserData(bv)){for(var j=i;j<U.getChildren().length;j++){if(U.getChildren()[j].getUserData(bv)===bc.getItem(i)){var W=j;
break;
}}if(W!=undefined){var X=U.getChildren()[W];
if(this.getTarget().isSelected(X)){var bd=true;
}U.removeAt(W);
U.addAt(X,i);
if(bd){this.getTarget().addToSelection(X);
}}else{var bb=this._createItem();
bb.setUserData(bv,bc.getItem(i));
U.addAt(bb,i);
this.__ot(bc.getItem(i),bb);
this.__oo(bb,bc.getItem(i));
}}}for(var i=U.getChildren().length-1;i>=bc.length;i--){var Y=U.getChildren()[i];
this.__or(Y,U);
...
this.__oq(H);
this.__ou(H.getUserData(bv));
H.destroy();
}},__oq:function(l){var m=l.getChildren();
for(var i=m.length-1;i>=0;i--){if(m[i].getChildren().length>0){this.__oq(m[i]);
}this.__or(m[i],l);
}},__or:function(bA,bB){var bC=bA.getUserData(bv);
if(bC[bu+qx.lang.String.firstUp(this.getChildPath())]!=undefined){delete this.__ol[bC[bu+qx.lang.String.firstUp(this.getChildPath())]().toHashCode()];
}this.__ou(bC);
bB.remove(bA);
bA.destroy();
},bindProperty:function(bF,bG,bH,bI,bJ){var bK=bJ.bind(bF,bI,bG,bH);
if(this.__oj[bG]==null){this.__oj[bG]={};
}this.__oj[bG][bJ.toHashCode()]={id:bK,treeNode:bI};
if(!qx.lang.Array.contains(this.__ok,bG)){this.__ok.push(bG);
}},__os:function(){var x;

for(var z in this.__oj){x=z;
break;
}for(var w in this.__oj[x]){var A=this.__oj[x][w].treeNode;
var y=qx.core.ObjectRegistry.fromHashCode(w);
this.__ou(y);
this.__ot(y,A);
}},__ot:function(c,d){var e=this.getDelegate();
if(e!=null&&e.bindItem!=null){e.bindItem(this,d,c);
}else{this.bindProperty(this.getLabelPath(),bm,this.getLabelOptions(),d,c);
if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),bo,this.getIconOptions(),d,c);
}}},__ou:function(n){for(var i=0;i<this.__ok.length;i++){var p=this.__ok[i];
var o=this.__oj[p][n.toHashCode()];

if(o!=null){n.removeBinding(o.id);
delete this.__oj[p][n.toHashCode()];
}}},_setConfigureItem:function(R,S){if(R!=null&&R.configureItem!=null&&this.getTarget()!=null){var T=this.getTarget().getRoot().getItems(true,true,false);

for(var i=0;i<T.length;i++){R.configureItem(T[i]);
}}},_setCreateItem:function(N,O){if(this.getTarget()==null||this.getModel()==null){return;
}
if(O&&O.createItem&&N&&N.createItem&&O.createItem==N.createTtem){return;
}this._startSelectionModification();
this.__op();
this.__on();
this._endSelectionModification();
this._updateSelection();
},_setBindItem:function(by,bz){if(by!=null&&by.bindItem!=null){if(bz!=null&&bz.bindItem!=null&&by.bindItem==bz.bindItem){return;
}this.__os();
}}}});
})();
