...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.data.ModelDebugging", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.data.ModelDebugging-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
(function(){var s="configured",r="completed",q="changeState",p="changeModel",o="qx.data.store.Json",n="GET",m="sending",l="application/json",k="changeUrl",j="failed",d="loaded",i="timeout",g="queued",c="String",b="aborted",f="_applyUrl",e="receiving",h="qx.event.type.Data";
qx.Class.define(o,{extend:qx.core.Object,construct:function(u,v){arguments.callee.base.call(this);
this._marshaler=new qx.data.marshal.Json(v);

if(u!=null){this.setUrl(u);
}},events:{"loaded":h},properties:{model:{nullable:true,event:p},state:{check:[s,g,m,e,r,b,i,j],init:s,event:q},url:{check:c,apply:f,event:k}},members:{_applyUrl:function(z,A){if(z!=null){this._createRequest(z);
}},_createRequest:function(t){this.__nT=new qx.io.remote.Request(t,n,l);
this.__nT.addListener(r,this.__nU,this);
this.__nT.addListener(q,function(y){this.setState(y.getData());
},this);
this.__nT.send();
},__nU:function(w){var x=w.getContent();
this._marshaler.jsonToClass(x,true);
this.setModel(this._marshaler.jsonToModel(x));
...
(function(){var p="loaded",o="auto",n="debugModel function in qx.dev.Debug.",m="?callback=demobrowser.demo.data.store.Twitter.saveResult",l="objects with properties. To see the properties, you can use the ",k="demobrowser.demo.data.ModelDebugging",j="failed",i="wittemann",h="Every model created by the data stores consists only of qooxdoo ",g="demobrowser.demo.data.store.Twitter",d="<b>Debugging models</b><br/>",f="white",e="main",c="http://twitter.com/statuses/user_timeline/",b=".json";
qx.Class.define(k,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var v=new demobrowser.demo.data.store.Twitter(i);
var t=new qx.ui.embed.Html();
t.setBackgroundColor(f);
t.setDecorator(e);
t.setWidth(500);
t.setHeight(200);
t.setOverflow(o,o);
this.getRoot().add(t,{left:10,top:80});
v.addListener(p,function(){var y=v.getModel();
this.debug(qx.dev.Debug.debugProperties(y));
t.setHtml(qx.dev.Debug.debugProperties(y,10,true));
},this);
var u=new qx.ui.basic.Label();
u.setRich(true);
u.setWidth(500);
u.setContent(d+h+l+n);
this.getRoot().add(u,{left:10,top:10});
}}});
qx.Class.define(g,{extend:qx.data.store.Json,statics:{saveResult:function(z){this.__nV=z;
}},construct:function(q){var r=c+q+b;
arguments.callee.base.call(this,r);
},members:{_createRequest:function(w){var x=new qx.io2.ScriptLoader();
w+=m;
x.load(w,function(a){this.__nW();
},this);
},__nW:function(){var s=demobrowser.demo.data.store.Twitter.__nV;

if(s==undefined){this.setState(j);
return;
}this._marshaler.jsonToClass(s);
this.setModel(this._marshaler.jsonToModel(s));
this.fireDataEvent(p,this.getModel());
}}});
})();
...
(function(){var o="qx.data.model.",n="Boolean",m="Array",l="Number",k="String",j="set",h="change",g="qx.data.marshal.Json",f=" ",e="_applyEventPropagation",d="Object";
qx.Class.define(g,{extend:qx.core.Object,construct:function(p){arguments.callee.base.call(this);
this.__nX=p;
},members:{__nY:function(a){var b=[];

for(var c in a){b.push(c);
}return b.sort().join(f);
},jsonToClass:function(q,r){var y=Object.prototype.toString.call(q).slice(8,-1);
if(y==k||y==l||y==n||q==null){return;
}var t=this.__nY(q);
if(this.__nX&&this.__nX.getModelClass&&this.__nX.getModelClass(t)!=null){return;
}if(qx.Class.isDefined(o+t)){return;
}var z={};

for(var x in q){if(q[x] instanceof Array){for(var i=0;i<q[x].length;i++){this.jsonToClass(q[x][i],r);
}}else if(q[x] instanceof Object){this.jsonToClass(q[x],r);
}z[x]={};
z[x].nullable=true;
z[x].event=h+qx.lang.String.firstUp(x);

if(r){z[x].apply=e;
}}if(this.__nX&&this.__nX.getModelSuperClass){var w=this.__nX.getModelSuperClass(t)||qx.core.Object;
}else{var w=qx.core.Object;
}var u=[];

if(this.__nX&&this.__nX.getModelMixins){var v=this.__nX.getModelMixins(t);
if(Object.prototype.toString.call(v).slice(8,-1)!=m){if(v!=null){u=[v];
}}}if(r){u.push(qx.data.marshal.MEventBubbling);
}var s={extend:w,include:u,properties:z};
qx.Class.define(o+t,s);
},__oa:function(G){var H;
if(this.__nX&&this.__nX.getModelClass){H=this.__nX.getModelClass(G);
}
if(H!=null){return (new H());
}else{var I=qx.Class.getByName(o+G);
return (new I());
}},jsonToModel:function(A){var E=Object.prototype.toString.call(A).slice(8,-1);

if(E==l||E==k||E==n||A==null){return A;
}else if(E==m){var D=new qx.data.Array();

for(var i=0;i<A.length;i++){D.push(this.jsonToModel(A[i]));
}return D;
}else if(E==d){var B=this.__nY(A);
var F=this.__oa(B);
for(var C in A){F[j+qx.lang.String.firstUp(C)](this.jsonToModel(A[C]));
}return F;
}throw new Error("Unsupported type!");
}}});
})();
(function(){var q="Boolean",p="qx.event.type.Event",o="queued",n="String",m="sending",l="qx.io.remote.Response",k="receiving",j="aborted",i="failed",h="nocache",ba="completed",Y="POST",X="configured",W="timeout",V="GET",U="Pragma",T="no-url-params-on-post",S="no-cache",R="Cache-Control",Q="Content-Type",y="text/plain",z="application/xml",w="application/json",x="text/html",t="application/x-www-form-urlencoded",u="qx.io.remote.Exchange",r="Integer",s="X-Qooxdoo-Response-Type",A="HEAD",B="qx.io.remote.Request",I="__kR",G="__kS",K="_applyResponseType",J="_applyState",M="text/javascript",L="__kT",D="changeState",P="PUT",O="_applyProhibitCaching",N="",C="_applyMethod",E="DELETE",F="boolean",H="__kQ";
...
},events:{"created":p,"configured":p,"sending":p,"receiving":p,"completed":l,"aborted":l,"failed":l,"timeout":l},statics:{__kU:0},properties:{url:{check:n,init:N},method:{check:[V,Y,P,A,E],apply:C,init:V},asynchronous:{check:q,init:true},data:{check:n,nullable:true},username:{check:n,nullable:true},password:{check:n,nullable:true},state:{check:[X,o,m,k,ba,j,W,i],init:X,apply:J,event:D},responseType:{check:[y,M,w,z,x],init:y,apply:K},timeout:{check:r,nullable:true},prohibitCaching:{check:function(v){return typeof v==F||v===T;
},init:true,apply:O},crossDomain:{check:q,init:false},fileUpload:{check:q,init:false},transport:{check:u,nullable:true},useBasicHttpAuth:{check:q,init:false}},members:{__kQ:null,__kR:null,__kS:null,__kT:null,__kU:null,send:function(){qx.io.remote.RequestQueue.getInstance().add(this);
},abort:function(){qx.io.remote.RequestQueue.getInstance().abort(this);
},reset:function(){switch(this.getState()){case m:case k:this.error("Aborting already sent request!");
case o:this.abort();
break;
}},isConfigured:function(){return this.getState()===X;
},isQueued:function(){return this.getState()===o;
},isSending:function(){return this.getState()===m;
},isReceiving:function(){return this.getState()===k;
},isCompleted:function(){return this.getState()===ba;
},isAborted:function(){return this.getState()===j;
},isTimeout:function(){return this.getState()===W;
},isFailed:function(){return this.getState()===i;
},__kV:function(e){var bl=e.clone();
bl.setTarget(this);
...
this.__kV(e);
this.dispose();
},_onaborted:function(e){this.setState(j);
this.__kV(e);
this.dispose();
},_ontimeout:function(e){this.setState(W);
this.__kV(e);
this.dispose();
},_onfailed:function(e){this.setState(i);
this.__kV(e);
this.dispose();
},_applyState:function(d,f){{};
},_applyProhibitCaching:function(bw,bx){if(!bw){this.removeParameter(h);
this.removeRequestHeader(U);
this.removeRequestHeader(R);
return;
}if(bw!==T||this.getMethod()!=Y){this.setParameter(h,new Date().valueOf());
}else{this.removeParameter(h);
}this.setRequestHeader(U,S);
this.setRequestHeader(R,S);
},_applyMethod:function(bt,bu){if(bt===Y){this.setRequestHeader(Q,t);
}else{this.removeRequestHeader(Q);
}var bv=this.getProhibitCaching();
this._applyProhibitCaching(bv,bv);
},_applyResponseType:function(bi,bj){this.setRequestHeader(s,bi);
},setRequestHeader:function(bm,bn){this.__kQ[bm]=bn;
},removeRequestHeader:function(bh){delete this.__kQ[bh];
},getRequestHeader:function(bs){return this.__kQ[bs]||null;
},getRequestHeaders:function(){return this.__kQ;
},setParameter:function(be,bf,bg){if(bg){this.__kS[be]=bf;
}else{this.__kR[be]=bf;
}},removeParameter:function(bq,br){if(br){delete this.__kS[bq];
}else{delete this.__kR[bq];
}},getParameter:function(b,c){if(c){return this.__kS[b]||null;
}else{return this.__kR[b]||null;
}},getParameters:function(a){return (a?this.__kS:this.__kR);
},setFormField:function(bo,bp){this.__kT[bo]=bp;
},removeFormField:function(bk){delete this.__kT[bk];
},getFormField:function(g){return this.__kT[g]||null;
},getFormFields:function(){return this.__kT;
},getSequenceNumber:function(){return this.__kU;
}},destruct:function(){this.setTransport(null);
this._disposeFields(H,I,G,L);
}});
})();
(function(){var x="Integer",w="sending",v="failed",u="timeout",t="completed",s="aborted",r="_applyEnabled",q="Boolean",p="interval",o="__kW",k="qx.io.remote.RequestQueue",n="__kX",m="queued",j="__la",h="receiving",l="singleton";
...
this.__la.addListener(p,this._oninterval,this);
},properties:{enabled:{init:true,check:q,apply:r},maxTotalRequests:{check:x,nullable:true},maxConcurrentRequests:{check:x,init:3},defaultTimeout:{check:x,init:5000}},members:{__kW:null,__kX:null,__kY:null,__la:null,_debug:function(){var A;
...
this.__kX.push(b);
this._debug();
b.addListener(w,a._onsending,a);
b.addListener(h,a._onreceiving,a);
b.addListener(t,a._oncompleted,a);
b.addListener(s,a._onaborted,a);
b.addListener(u,a._ontimeout,a);
b.addListener(v,a._onfailed,a);
b.addListener(w,this._onsending,this);
b.addListener(t,this._oncompleted,this);
b.addListener(s,this._oncompleted,this);
b.addListener(u,this._oncompleted,this);
b.addListener(v,this._oncompleted,this);
b._start=(new Date).valueOf();
b.send();
if(this.__kW.length>0){this._check();
}},_remove:function(f){qx.lang.Array.remove(this.__kX,f);
f.dispose();
this._check();
...
this.warn(B+"ms > "+E+"ms");
F.timeout();
}}}},_applyEnabled:function(y,z){if(y){this._check();
}this.__la.setEnabled(y);
},add:function(g){g.setState(m);
this.__kW.push(g);
this._check();
...
if(d){d.abort();
}else if(qx.lang.Array.contains(this.__kW,c)){qx.lang.Array.remove(this.__kW,c);
}}},destruct:function(){this._disposeArray(n);
this._disposeObjects(j);
this._disposeFields(o);
}});
})();
(function(){var D="sending",C="completed",B="receiving",A="aborted",z="failed",y="timeout",x="qx.io.remote.Response",w="Connection dropped",v="configured",u="qx.event.type.Event",bz="Proxy authentication required",by="qx.io.remote.transport.Abstract",bx="MSHTML-specific HTTP status code",bw="Not available",bv="Precondition failed",bu="Server error",bt="Moved temporarily",bs="qx.io.remote.Exchange",br="Bad gateway",bq="Gone",K="See other",L="Partial content",I="Server timeout",J="qx.io.remote.transport.Script",G="HTTP version not supported",H="Unauthorized",E="Multiple choices",F="Payment required",O="Not implemented",P="Request-URL too large",X="Length required",V="_applyState",bg="changeState",bb="Not modified",bm="qx.io.remote.Request",bk="Connection closed by server",R="Moved permanently",bp="_applyImplementation",bo="Method not allowed",bn="Forbidden",Q="Use proxy",T="Ok",U="Not found",W="Not acceptable",Y="Request time-out",bc="Bad request",bh="Conflict",bl="No content",M="qx.io.remote.transport.XmlHttp",N="qx.io.remote.transport.Iframe",S="Request entity too large",bf="Unknown status code",be="Unsupported media type",bd="Gateway time-out",bj="created",bi="Out of resources",ba="undefined";
qx.Class.define(bs,{extend:qx.core.Object,construct:function(k){arguments.callee.base.call(this);
this.setRequest(k);
k.setTransport(this);
},events:{"sending":u,"receiving":u,"completed":x,"aborted":x,"failed":x,"timeout":x},statics:{typesOrder:[M,N,J],typesReady:false,typesAvailable:{},typesSupported:{},registerType:function(bK,bL){qx.io.remote.Exchange.typesAvailable[bL]=bK;
},initTypes:function(){if(qx.io.remote.Exchange.typesReady){return;
}
for(var bE in qx.io.remote.Exchange.typesAvailable){var bD=qx.io.remote.Exchange.typesAvailable[bE];

if(bD.isSupported()){qx.io.remote.Exchange.typesSupported[bE]=bD;
...
},_nativeMap:{0:bj,1:v,2:D,3:B,4:C},wasSuccessful:function(f,g,h){if(h){switch(f){case null:case 0:return true;
case -1:return g<4;
default:return typeof f===ba;
}}else{switch(f){case -1:{};
return g<4;
...
return false;
default:if(f>206&&f<300){return true;
}qx.log.Logger.debug(this,"Unknown status code: "+f+" ("+g+")");
return false;
}}},statusCodeToString:function(e){switch(e){case -1:return bw;
case 200:return T;
case 304:return bb;
case 206:return L;
case 204:return bl;
...
case 12030:return w;
case 12031:return w;
case 12152:return bk;
case 13030:return bx;
default:return bf;
}}},properties:{request:{check:bm,nullable:true},implementation:{check:by,nullable:true,apply:bp},state:{check:[v,D,B,C,A,y,z],init:v,event:bg,apply:V}},members:{send:function(){var p=this.getRequest();

if(!p){return this.error("Please attach a request object first");
}qx.io.remote.Exchange.initTypes();
var n=qx.io.remote.Exchange.typesOrder;
var m=qx.io.remote.Exchange.typesSupported;
var r=p.getResponseType();
var s={};

if(p.getAsynchronous()){s.asynchronous=true;
}else{s.synchronous=true;
}
if(p.getCrossDomain()){s.crossDomain=true;
}
if(p.getFileUpload()){s.fileUpload=true;
}for(var q in p.getFormFields()){s.programaticFormFields=true;
break;
}var t,o;

for(var i=0,l=n.length;i<l;i++){t=m[n[i]];

if(t){if(!qx.io.remote.Exchange.canHandle(t,s,r)){continue;
}
try{{};
o=new t;
this.setImplementation(o);
o.setUseBasicHttpAuth(p.getUseBasicHttpAuth());
o.send();
return true;
}catch(bN){this.error("Request handler throws error");
this.error(bN);
return;
}}}this.error("There is no transport implementation available to handle this request: "+p);
...
},_applyImplementation:function(bA,bB){if(bB){bB.removeListener(D,this._onsending,this);
bB.removeListener(B,this._onreceiving,this);
bB.removeListener(C,this._oncompleted,this);
bB.removeListener(A,this._onabort,this);
bB.removeListener(y,this._ontimeout,this);
bB.removeListener(z,this._onfailed,this);
}
if(bA){var bC=this.getRequest();
bA.setUrl(bC.getUrl());
bA.setMethod(bC.getMethod());
bA.setAsynchronous(bC.getAsynchronous());
bA.setUsername(bC.getUsername());
bA.setPassword(bC.getPassword());
bA.setParameters(bC.getParameters());
bA.setFormFields(bC.getFormFields());
bA.setRequestHeaders(bC.getRequestHeaders());
bA.setData(bC.getData());
bA.setResponseType(bC.getResponseType());
bA.addListener(D,this._onsending,this);
bA.addListener(B,this._onreceiving,this);
bA.addListener(C,this._oncompleted,this);
bA.addListener(A,this._onabort,this);
bA.addListener(y,this._ontimeout,this);
bA.addListener(z,this._onfailed,this);
}},_applyState:function(bF,bG){{};

switch(bF){case D:this.fireEvent(D);
break;
case B:this.fireEvent(B);
break;
case C:case A:case y:case z:var bI=this.getImplementation();

if(!bI){break;
}
if(this.hasListener(bF)){var bJ=qx.event.Registration.createEvent(bF,qx.io.remote.Response);

if(bF==C){var bH=bI.getResponseContent();
bJ.setContent(bH);
if(bH===null){{};
bF=z;
...
switch(t){case n:this.fireEvent(n);
break;
...
(function(){var I="completed",H="failed",G="=",F="aborted",E="",D="&",C="timeout",B="application/xml",A="qx.io.remote.transport.XmlHttp",z="application/json",bf="text/html",be="qx.client",bd="receiving",bc="text/plain",bb="text/javascript",ba="sending",Y="configured",X="?",W="created",V='Referer',P='Basic ',Q="\n</pre>",N="string",O='Authorization',L="<pre>Could not execute json: \n",M="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",J=':',K="parseerror",R="file:",S="webkit",U="__lc",T="object";
...
},statics:{handles:{synchronous:true,asynchronous:true,crossDomain:false,fileUpload:false,programaticFormFields:false,responseTypes:[bc,bb,z,B,bf]},requestObjects:[],requestObjectCount:0,createRequestObject:qx.core.Variant.select(be,{"default":function(){return new XMLHttpRequest;
...
var bv=this.getRequest();
var br=this.getMethod();
var by=this.getAsynchronous();
var bx=this.getUrl();
var bt=(window.location.protocol===R&&!(/^http(s){0,1}\:/.test(bx)));
this.__le=bt;
var bB=this.getParameters(false);
var bz=[];

for(var bs in bB){var bw=bB[bs];

if(bw instanceof Array){for(var i=0;i<bw.length;i++){bz.push(encodeURIComponent(bs)+G+encodeURIComponent(bw[i]));
}}else{bz.push(encodeURIComponent(bs)+G+encodeURIComponent(bw));
}}
if(bz.length>0){bx+=(bx.indexOf(X)>=0?D:X)+bz.join(D);
...
if(bz.length>0){this.setData(bz.join(D));
}}var bA=function(bE){var bJ=M;
var bN=E;
var bH,bG,bF;
var bK,bL,bM,bI;
var i=0;
...
bv.onreadystatechange=qx.lang.Function.bind(this._onreadystatechange,this);
try{if(this.getUsername()){if(this.getUseBasicHttpAuth()){bv.open(br,bx,by);
bv.setRequestHeader(O,P+bA(this.getUsername()+J+this.getPassword()));
}else{bv.open(br,bx,by,this.getUsername(),this.getPassword());
}}else{bv.open(br,bx,by);
}}catch(bO){this.error("Failed with exception: "+bO);
this.failed();
return;
}if(!qx.core.Variant.isSet(be,S)){bv.setRequestHeader(V,window.location.href);
}var bu=this.getRequestHeaders();

for(var bs in bu){bv.setRequestHeader(bs,bu[bs]);
}try{{};
bv.send(this.getData());
}catch(p){if(bt){this.failedLocally();
}else{this.error("Failed to send data: "+p,"send");
this.failed();
}return;
}if(!by){this._onreadystatechange();
}},failedLocally:function(){if(this.getState()===H){return;
}this.warn("Could not load from file: "+this.getUrl());
this.failed();
},_onreadystatechange:function(e){switch(this.getState()){case I:case F:case H:case C:{};
return;
}var e=this.getReadyState();

if(e==4){if(!qx.io.remote.Exchange.wasSuccessful(this.getStatusCode(),e,this.__le)){return this.failed();
}}while(this.__lf<e){this.setState(qx.io.remote.Exchange._nativeMap[++this.__lf]);
}},getReadyState:function(){var d=null;

try{d=this.__lc.readyState;
}catch(bg){}return d;
},setRequestHeader:function(bl,bm){this.__lc.setRequestHeader(bl,bm);
},getResponseHeader:function(u){var v=null;

try{this.getRequest().getResponseHeader(u)||null;
}catch(bp){}return v;
},getStringResponseHeaders:function(){var bQ=null;

try{var bP=this.__lc.getAllResponseHeaders();

if(bP){bQ=bP;
}}catch(bk){}return bQ;
},getResponseHeaders:function(){var h=this.getStringResponseHeaders();
var j={};

if(h){var f=h.split(/[\r\n]+/g);

for(var i=0,l=f.length;i<l;i++){var g=f[i].match(/^([^:]+)\s*:\s*(.+)$/i);
...
},getStatusText:function(){var bD=E;

try{bD=this.getRequest().statusText;
}catch(o){}return bD;
},getResponseText:function(){var n=null;
var k=this.getStatusCode();
var m=this.getReadyState();

if(qx.io.remote.Exchange.wasSuccessful(k,m,this.__le)){try{n=this.getRequest().responseText;
}catch(bq){}}return n;
},getResponseXml:function(){var t=null;
var q=this.getStatusCode();
var r=this.getReadyState();

if(qx.io.remote.Exchange.wasSuccessful(q,r,this.__le)){try{t=this.getRequest().responseXML;
}catch(c){}}if(typeof t==T&&t!=null){if(!t.documentElement){var s=String(this.getRequest().responseText).replace(/<\?xml[^\?]*\?>/,E);
t.loadXML(s);
}if(!t.documentElement){throw new Error("Missing Document Element!");
}
if(t.documentElement.tagName==K){throw new Error("XML-File is not well-formed!");
}}else{throw new Error("Response was not a valid xml document ["+this.getRequest().responseText+"]");
}return t;
},getFetchedLength:function(){var bo=this.getResponseText();
return typeof bo==N?bo.length:0;
},getResponseContent:function(){if(this.getState()!==I){{};
return null;
}{};
var y=this.getResponseText();

switch(this.getResponseType()){case bc:case bf:{};
return y;
case z:{};

try{if(y&&y.length>0){var x=qx.util.Json.parseQx(y);
return (x===0?0:(x||null));
}else{return null;
}}catch(bC){this.error("Could not execute json: ["+y+"]",bC);
return L+y+Q;
}case bb:{};

try{if(y&&y.length>0){var x=window.eval(y);
return (x===0?0:(x||null));
}else{return null;
}}catch(bn){this.error("Could not execute javascript: ["+y+"]",bn);
return null;
}case B:y=this.getResponseXml();
{};
return (y===0?0:(y||null));
...
switch(bh){case W:this.fireEvent(W);
break;
...
this.fireEvent(C);
break;
}}},defer:function(a,b){qx.io.remote.Exchange.registerType(qx.io.remote.transport.XmlHttp,A);
},destruct:function(){var bR=this.getRequest();

if(bR){bR.onreadystatechange=qx.io.remote.transport.XmlHttp.__ld;
switch(bR.readyState){case 1:case 2:case 3:bR.abort();
}}this._disposeFields(U);
}});
})();
(function(){var A=",",z="",y="string",x="null",w='"',v="qx.jsonDebugging",u='\\u00',t="new Date(Date.UTC(",s=")",r='\\\\',bb='\\f',ba="Object",Y='\\"',X="))",W="}",V='(',U=":",T="{",S='\\r',R="__bN",H="__bM",I="__bX",F='\\t',G="(",D="]",E="qx.jsonEncodeUndefined",B="[",C="__bP",J="__bW",K='\\b',M="qx.util.Json",L=')',O='\\n',N="Date",Q="__bO",P="Array";
qx.Class.define(M,{statics:{BEAUTIFYING_INDENT:"  ",BEAUTIFYING_LINE_END:"\n",__bL:{"function":H,"boolean":R,"number":Q,"string":C,"object":J,"undefined":I},__bM:function(bc){return String(bc);
},__bN:function(n){return String(n);
},__bO:function(p){return isFinite(p)?String(p):x;
},__bP:function(d){var e;

if(/["\\\x00-\x1f]/.test(d)){e=d.replace(/([\x00-\x1f\\"])/g,qx.util.Json.__bR);
}else{e=d;
}return w+e+w;
},__bQ:{'\b':K,'\t':F,'\n':O,'\f':bb,'\r':S,'"':Y,'\\':r},__bR:function(a,b){var bd=qx.util.Json.__bQ[b];

if(bd){return bd;
}bd=b.charCodeAt();
return u+Math.floor(bd/16).toString(16)+(bd%16).toString(16);
},__bS:function(f){var h=[],m=true,k,g;
...
for(var i=0,l=f.length;i<l;i++){g=f[i];
k=this.__bL[typeof g];

if(k){g=this[k](g);

if(typeof g==y){if(!m){h.push(A);
...
return h.join(z);
},__bU:function(a){var c=a.getUTCFullYear()+A+a.getUTCMonth()+A+a.getUTCDate()+A+a.getUTCHours()+A+a.getUTCMinutes()+A+a.getUTCSeconds()+A+a.getUTCMilliseconds();
return t+c+X;
},__bV:function(bk){var bn=[],bq=true,bm,bl;
var bo=qx.util.Json.__bY;
bn.push(T);

if(bo){qx.util.Json.__bT+=qx.util.Json.BEAUTIFYING_INDENT;
bn.push(qx.util.Json.__bT);
}
for(var bp in bk){bl=bk[bp];
bm=this.__bL[typeof bl];

if(bm){bl=this[bm](bl);

if(typeof bl==y){if(!bq){bn.push(A);

if(bo){bn.push(qx.util.Json.__bT);
}}bn.push(this.__bP(bp),U,bl);
bq=false;
}}}
if(bo){qx.util.Json.__bT=qx.util.Json.__bT.substring(0,qx.util.Json.__bT.length-qx.util.Json.BEAUTIFYING_INDENT.length);
bn.push(qx.util.Json.__bT);
}bn.push(W);
return bn.join(z);
},__bW:function(br){if(br){var bs=br.constructor.name;

if(br instanceof Array||bs==P){return this.__bS(br);
}else if(br instanceof Date||bs==N){return this.__bU(br);
}else if(br instanceof Object||bs==ba){return this.__bV(br);
}return z;
}return x;
},__bX:function(q){if(qx.core.Setting.get(E)){return x;
}},stringify:function(be,bf){this.__bY=bf;
this.__bT=this.BEAUTIFYING_LINE_END;
var bg=this[this.__bL[typeof be]](be);

if(typeof bg!=y){bg=null;
}if(qx.core.Setting.get(v)){qx.log.Logger.debug(this,"JSON request: "+bg);
}return bg;
},parse:function(bh){if(/[^,:{}\[\]0-9.\-+Eaeflnr-u \n\r\t]/.test(bh.replace(/"(\\.|[^"\\])*"/g,z))){throw new Error("Could not parse JSON string!");
}
try{return eval(G+bh+s);
}catch(o){throw new Error("Could not evaluate JSON string: "+o.message);
}},parseQx:function(bi){if(qx.core.Setting.get(v)){qx.log.Logger.debug(this,"JSON response: "+bi);
}var bj=(bi&&bi.length>0)?eval(V+bi+L):null;
return bj;
}},settings:{"qx.jsonEncodeUndefined":true,"qx.jsonDebugging":false}});
})();
(function(){var o="=",n="&",m="application/xml",l="application/json",k="text/html",j="qx.client",h="textarea",g="none",f="text/plain",e="text/javascript",K="",J="completed",I="?",H="qx.io.remote.transport.Iframe",G="__lh",F="gecko",E="frame_",D="aborted",C="_data_",B="pre",v="__lg",w="javascript:void(0)",t="sending",u="form",r="failed",s='<iframe name="',p="mshtml",q="form_",x='"></iframe>',y="iframe",A="timeout",z="qx/static/blank.gif";
qx.Class.define(H,{extend:qx.io.remote.transport.Abstract,construct:function(){arguments.callee.base.call(this);
var a=(new Date).valueOf();
var b=E+a;
var c=q+a;
if(qx.core.Variant.isSet(j,p)){this.__lg=document.createElement(s+b+x);
}else{this.__lg=document.createElement(y);
}this.__lg.src=w;
this.__lg.id=this.__lg.name=b;
this.__lg.onload=qx.lang.Function.bind(this._onload,this);
this.__lg.style.display=g;
document.body.appendChild(this.__lg);
this.__lh=document.createElement(u);
this.__lh.target=b;
this.__lh.id=this.__lh.name=c;
this.__lh.style.display=g;
document.body.appendChild(this.__lh);
this.__li=document.createElement(h);
this.__li.id=this.__li.name=C;
this.__lh.appendChild(this.__li);
this.__lg.onreadystatechange=qx.lang.Function.bind(this._onreadystatechange,this);
},statics:{handles:{synchronous:false,asynchronous:true,crossDomain:false,fileUpload:true,programaticFormFields:true,responseTypes:[f,e,l,m,k]},isSupported:function(){return true;
},_numericMap:{"uninitialized":1,"loading":2,"loaded":2,"interactive":3,"complete":4}},members:{__li:null,__lj:0,__lh:null,__lg:null,send:function(){var O=this.getMethod();
var Q=this.getUrl();
...
if(T.length>0){Q+=(Q.indexOf(I)>=0?n:I)+T.join(n);
...
},_switchReadyState:function(V){switch(this.getState()){case J:case D:case r:case A:this.warn("Ignore Ready State Change");
return;
}while(this.__lj<V){this.setState(qx.io.remote.Exchange._nativeMap[++this.__lj]);
}},setRequestHeader:function(L,M){},getResponseHeader:function(be){return null;
...
}if(W.firstChild.tagName&&W.firstChild.tagName.toLowerCase()==B){return W.firstChild.innerHTML;
}else{return W.innerHTML;
}},getIframeHtmlContent:function(){var bc=this.getIframeBody();
return bc?bc.innerHTML:null;
},getFetchedLength:function(){return 0;
},getResponseContent:function(){if(this.getState()!==J){{};
return null;
}{};
...
return bd;
break;
case k:bd=this.getIframeHtmlContent();
{};
return bd;
break;
case l:bd=this.getIframeHtmlContent();
{};

try{return bd&&bd.length>0?qx.util.Json.parseQx(bd):null;
}catch(X){return this.error("Could not execute json: ("+bd+")",X);
}case e:bd=this.getIframeHtmlContent();
{};

try{return bd&&bd.length>0?window.eval(bd):null;
}catch(d){return this.error("Could not execute javascript: ("+bd+")",d);
}case m:bd=this.getIframeDocument();
{};
return bd;
default:this.warn("No valid responseType specified ("+this.getResponseType()+")!");
return null;
}}},defer:function(Y,ba,bb){qx.io.remote.Exchange.registerType(qx.io.remote.transport.Iframe,H);
},destruct:function(){if(this.__lg){this.__lg.onload=null;
this.__lg.onreadystatechange=null;
if(qx.core.Variant.isSet(j,F)){this.__lg.src=qx.util.ResourceManager.toUri(z);
}document.body.removeChild(this.__lg);
}
if(this.__lh){document.body.removeChild(this.__lh);
}this._disposeFields(v,G);
}});
})();
(function(){var e="qx.event.handler.Iframe",d="load",c="iframe";
qx.Class.define(e,{extend:qx.core.Object,implement:qx.event.IEventHandler,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{load:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false,onevent:function(b){qx.event.Registration.fireEvent(b,d);
}},members:{canHandleEvent:function(l,m){return l.tagName.toLowerCase()===c;
},registerEvent:function(i,j,k){},unregisterEvent:function(f,g,h){}},defer:function(a){qx.event.Registration.addHandler(a);
}});
})();
(function(){var n="0",m="qx.client",l="qx.bom.Iframe",k="qx.event.handler.Iframe.onevent(this)",j="true",i="iframe",h="body";
qx.Class.define(l,{statics:{create:function(A,B){var A=A?qx.lang.Object.clone(A):{};
A.onload=k;
A.frameBorder=n;
A.frameSpacing=n;
A.marginWidth=n;
...
A.vspace=n;
A.border=n;
A.allowTransparency=j;
return qx.bom.Element.create(i,A,B);
},getWindow:qx.core.Variant.select(m,{"mshtml|gecko":function(b){try{return b.contentWindow;
}catch(e){return null;
...
}}}),getBody:function(u){var v=this.getDocument(u);
return v?v.getElementsByTagName(h)[0]:null;
},setSource:function(f,g){try{if(this.getWindow(f)){try{this.getWindow(f).location.replace(g);
}catch(y){f.src=g;
}}else{f.src=g;
...
(function(){var z="&",y="=",x="?",w="application/json",v="completed",u="text/plain",t="text/javascript",s="qx.io.remote.transport.Script",r="",q="_ScriptTransport_data",k="_responseContent",p="script",n="timeout",j="_ScriptTransport_",h="_ScriptTransport_id",m="__ll",l="aborted",o="utf-8",g="failed";
qx.Class.define(s,{extend:qx.io.remote.transport.Abstract,construct:function(){arguments.callee.base.call(this);
var B=++qx.io.remote.transport.Script.__lk;

if(B>=2000000000){qx.io.remote.transport.Script.__lk=B=1;
}this.__ll=null;
this.__lk=B;
},statics:{__lk:0,_instanceRegistry:{},ScriptTransport_PREFIX:j,ScriptTransport_ID_PARAM:h,ScriptTransport_DATA_PARAM:q,handles:{synchronous:false,asynchronous:true,crossDomain:true,fileUpload:false,programaticFormFields:false,responseTypes:[u,t,w]},isSupported:function(){return true;
},_numericMap:{"uninitialized":1,"loading":2,"loaded":2,"interactive":3,"complete":4},_requestFinished:function(b,content){var c=qx.io.remote.transport.Script._instanceRegistry[b];

if(c==null){{};
}else{c._responseContent=content;
c._switchReadyState(qx.io.remote.transport.Script._numericMap.complete);
}}},members:{__lm:0,__ll:null,__lk:null,send:function(){var H=this.getUrl();
H+=(H.indexOf(x)>=0?z:x)+qx.io.remote.transport.Script.ScriptTransport_ID_PARAM+y+this.__lk;
...
if(I instanceof Array){for(var i=0;i<I.length;i++){J.push(encodeURIComponent(G)+y+encodeURIComponent(I[i]));
}}else{J.push(encodeURIComponent(G)+y+encodeURIComponent(I));
}}
if(J.length>0){H+=z+J.join(z);
}var F=this.getData();

if(F!=null){H+=z+qx.io.remote.transport.Script.ScriptTransport_DATA_PARAM+y+encodeURIComponent(F);
}qx.io.remote.transport.Script._instanceRegistry[this.__lk]=this;
this.__ll=document.createElement(p);
this.__ll.charset=o;
this.__ll.src=H;
{};
document.body.appendChild(this.__ll);
},_switchReadyState:function(E){switch(this.getState()){case v:case l:case g:case n:this.warn("Ignore Ready State Change");
return;
}while(this.__lm<E){this.setState(qx.io.remote.Exchange._nativeMap[++this.__lm]);
}},setRequestHeader:function(C,D){},getResponseHeader:function(a){return null;
},getResponseHeaders:function(){return {};
},getStatusCode:function(){return 200;
...
default:this.warn("No valid responseType specified ("+this.getResponseType()+")!");
return null;
}}},defer:function(d,e,f){qx.io.remote.Exchange.registerType(qx.io.remote.transport.Script,s);
qx.io.remote.ScriptTransport=d;
...
(function(){var r="",q="color",p="String",o="none",n="padding",m="0px",l="changeHtml",k="_applyCssClass",j="class",i="qx.ui.embed.Html",f="_applyHtml",h="border",g="html";
qx.Class.define(i,{extend:qx.ui.core.Widget,include:[qx.ui.core.MNativeOverflow],construct:function(s){arguments.callee.base.call(this);

if(s!=null){this.setHtml(s);
}},properties:{html:{check:p,apply:f,event:l,nullable:true},cssClass:{check:p,init:r,apply:k},selectable:{refine:true,init:true},focusable:{refine:true,init:true}},members:{getFocusElement:function(){return this.getContentElement();
},_applyHtml:function(a,b){var c=this.getContentElement();
c.setAttribute(g,a||r);
c.setStyle(n,m);
c.setStyle(h,o);
},_applyCssClass:function(t,u){this.getContentElement().setAttribute(j,t);
...
(function(){var o=": ",n="",m="construct",l="object",k="-",h=":",g=") ***",f="Boolean",e="\r\n",d="px;'>",M="): ",L="</span><br>",K="*** EXCEPTION (",J="============================================================",I="Object",H="<br>",G="null",F="Array",E="members",D="String",v="statics",w="get",t="------------------------------------------------------------",u="qx.dev.Debug",r="\n",s="*** TOO MUCH RECURSION: not displaying ***",p="Number",q="Object, count=",x="  ",y="<span style='padding-left:",A=" ",z="Array, length=",C="undefined",B="index(";
qx.Class.define(u,{statics:{debugObject:function(bo,bp,bq){qx.log.Logger.debug(this,qx.dev.Debug.debugObjectToString(bo,bp,bq,false));
},debugObjectToString:function(be,bf,bg,bh){if(!bg){bg=10;
}var bn=(bh?L:r);
var bk=function(N){var O;

if(!bh){O=n;

for(var i=0;i<N;i++){O+=x;
}}else{O=y+(N*8)+d;
}return O;
};
var bm=n;
var bj=function(Q,R,S){if(R>S){bm+=(bk(R)+s+bn);
return;
}if(typeof (Q)!=l){bm+=bk(R)+Q+bn;
return;
}for(var T in Q){if(typeof (Q[T])==l){if(Q[T] instanceof Array){bm+=bk(R)+T+o+F+bn;
}else if(Q[T]===null){bm+=bk(R)+T+o+G+bn;
continue;
}else if(Q[T]===undefined){bm+=bk(R)+T+o+C+bn;
continue;
}else{bm+=bk(R)+T+o+I+bn;
}bj(Q[T],R+1,S);
}else{bm+=bk(R)+T+o+Q[T]+bn;
}}};
if(bf){bm+=bk(0)+bf+bn;
}
if(be instanceof Array){bm+=bk(0)+z+be.length+h+bn;
}else if(typeof (be)==l){var bi=0;
...
try{bj(be,0,bg);
}catch(P){bm+=bk(0)+K+P+g+bn;
}bm+=bk(0)+J+bn;
return bm;
},getFunctionName:function(a,b){var c=a.self;

if(!c){return null;
}while(a.wrapper){a=a.wrapper;
}
switch(b){case m:return a==c?m:null;
case E:return qx.lang.Object.getKeyFromValue(c,a);
case v:return qx.lang.Object.getKeyFromValue(c.prototype,a);
default:if(a==c){return m;
}return (qx.lang.Object.getKeyFromValue(c.prototype,a)||qx.lang.Object.getKeyFromValue(c,a)||null);
}},debugProperties:function(U,V,W,X){if(V==null){V=10;
}if(X==null){X=1;
}var Y=n;
W?Y=H:Y=e;
var bb=n;
var bc=Object.prototype.toString.call(U).slice(8,-1);

if(bc==p||bc==D||bc==f||U==null||V<=0){return U;
}else if(qx.Class.hasInterface(U.constructor,qx.data.IListData)){for(var i=0;i<U.length;i++){for(var j=0;j<X;j++){bb+=k;
}bb+=B+i+M+this.debugProperties(U.getItem(i),V-1,W,X+1)+Y;
}return bb+Y;
}else if(U.constructor!=null){var bd=U.constructor.$$properties;

for(var ba in bd){bb+=Y;
for(var j=0;j<X;j++){bb+=k;
}bb+=A+ba+o+this.debugProperties(U[w+qx.lang.String.firstUp(ba)](),V-1,W,X+1);
}return bb;
}return n;
}}});
})();
...
},members:{__od:null,__oe:null,__of:null,__ob:null,__oc:null,load:function(u,v,w){if(this.__od){throw new Error("Another request is still running!");
}this.__od=true;
...
this.__of=w||window;
y.type=f;
y.onerror=y.onload=y.onreadystatechange=this.__ob;
y.src=u;
x.appendChild(y);
},abort:function(){if(this.__od){this.__og(h);
}},__og:function(status){var t=this.__oc;
...
},__oh:qx.core.Variant.select(a,{"mshtml":function(){var z=this.__oc.readyState;

if(z==p){this.__og(s);
}else if(z==q){this.__og(s);
}else{return;
