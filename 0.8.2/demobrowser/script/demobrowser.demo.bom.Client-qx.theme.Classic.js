...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.bom.Client-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
for(var r in q[i]){if(/^[A-Z_0-9]+$/.test(r)){t.add(n,r,o,q[i][r],l);
}}t.add(g);
...
(function(){var n="$1",m="Shockwave Flash",l=".",k="undefined",j="object",i=",",h="$version",g="always",f="ShockwaveFlash.ShockwaveFlash.6",e="qx.bom.client.Flash",b="ShockwaveFlash.ShockwaveFlash.7",d="qx.client",c="ShockwaveFlash.ShockwaveFlash";
qx.Bootstrap.define(e,{statics:{AVAILABLE:false,FULLVERSION:"0.0.0",VERSION:0.0,EXPRESSINSTALL:false,supportsVersion:function(p){var p=p.split(l);
var q=this.FULLVERSION.split(l);
return (q[0]>p[0]||(q[0]==p[0]&&q[1]>p[1])||(q[0]==p[0]&&q[1]==p[1]&&q[2]>=p[2]))?true:false;
},__wJ:qx.core.Variant.select(d,{"mshtml":function(){if(!window.ActiveXObject){return;
}var x=[0,0,0];
var w=false;

try{var y=new ActiveXObject(b);
}catch(o){try{var y=new ActiveXObject(f);
x=[6,0,21];
y.AllowScriptAccess=g;
}catch(a){if(x[0]==6){w=true;
}}
if(!w){try{y=new ActiveXObject(c);
}catch(t){}}}
if(!w&&typeof y==j){var v=y.GetVariable(h);

if(typeof v!=k){v=v.split(" ")[1].split(i);
x[0]=parseInt(v[0],10);
x[1]=parseInt(v[1],10);
x[2]=parseInt(v[2],10);
}}this.__wK(x);
},"default":function(){if(!navigator.plugins||typeof navigator.plugins[m]!==j){return;
}var z=[0,0,0];
var A=navigator.plugins[m].description;

if(typeof A!=k){A=A.replace(/^.*\s+(\S+\s+\S+$)/,n);
z[0]=parseInt(A.replace(/^(.*)\..*$/,n),10);
z[1]=parseInt(A.replace(/^.*\.(.*)\s.*$/,n),10);
z[2]=/r/.test(A)?parseInt(A.replace(/^.*r(.*)$/,n),10):0;
}this.__wK(z);
}}),__wK:function(r){this.FULLVERSION=r.join(".");
