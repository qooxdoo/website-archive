...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.ui.DragDrop", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.ui.DragDrop-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var r="value",q="items",p="drop",o="move",n="multi",m="dragover",k="icon/16/places/folder.png",j="dragstart",h="Item ",g="copy",D="drag",C="TextArea Target",B="solid",A="Reorderable",z="droprequest",y="#33508D",x="Source",w="demobrowser.demo.ui.DragDrop",v="Simple Target",u="Enable drag",s="\n",t="dragend";
...
var H=new qx.ui.container.Composite(E).set({allowStretchY:false,allowStretchX:false});
J.add(H);
P.add(J,{edge:0});
var O=new qx.ui.basic.Label(x);
H.add(O,{left:20,top:20});
var M=new qx.ui.form.List;
M.setDraggable(true);
M.setSelectionMode(n);
H.add(M,{left:20,top:40});

for(var i=0;i<20;i++){M.add(new qx.ui.form.ListItem(h+i,k));
}var Q=new qx.ui.form.CheckBox(u);
Q.setChecked(true);
H.add(Q,{left:20,top:260});
M.addListener(j,function(e){if(!Q.isChecked()){e.preventDefault();
}e.addType(r);
e.addType(q);
e.addAction(g);
e.addAction(o);
});
M.addListener(z,function(e){this.debug("Related of droprequest: "+e.getRelatedTarget());
var T=e.getCurrentAction();
var V=e.getCurrentType();
var S;

switch(V){case q:S=this.getSelection();

if(T==g){var U=[];

for(var i=0,l=S.length;i<l;i++){U[i]=S[i].clone();
}S=U;
}break;
case r:S=this.getValue();
break;
}if(T==o){var R=this.getSelection();

for(var i=0,l=R.length;i<l;i++){this.remove(R[i]);
}}e.addData(V,S);
});
var K=new qx.ui.basic.Label(v);
H.add(K,{left:140,top:20});
var I=new qx.ui.form.List;
I.setDroppable(true);
I.setSelectionMode(n);
H.add(I,{left:140,top:40});
I.addListener(p,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
var f=e.getData(q);

for(var i=0,l=f.length;i<l;i++){this.add(f[i]);
}});
I.addListener(m,function(e){if(!e.supportsType(q)){e.preventDefault();
}});
var K=new qx.ui.basic.Label(C);
H.add(K,{left:260,top:20});
var N=new qx.ui.form.TextArea;
N.setDroppable(true);
N.setHeight(100);
H.add(N,{left:260,top:40});
N.addListener(p,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
this.setValue(e.getData(r).replace(/,/g,s));
});
N.addListener(m,function(e){if(!e.supportsType(r)){e.preventDefault();
}});
var L=new qx.ui.basic.Label(A);
H.add(L,{left:500,top:20});
var F=new qx.ui.form.List;
F.setDraggable(true);
F.setDroppable(true);
F.setSelectionMode(n);
H.add(F,{left:500,top:40});

for(var i=0;i<20;i++){F.add(new qx.ui.form.ListItem(h+i,k));
}var G=new qx.ui.core.Widget;
G.setDecorator(new qx.ui.decoration.Single().set({top:[1,B,y]}));
G.setHeight(0);
G.setOpacity(0.5);
G.setZIndex(100);
G.setLayoutProperties({left:-1000,top:-1000});
this.getRoot().add(G);
F.addListener(j,function(e){e.addAction(o);
});
F.addListener(t,function(e){G.setDomPosition(-1000,-1000);
});
F.addListener(D,function(e){var d=e.getOriginalTarget();

if(!qx.ui.core.Widget.contains(this,d)){return;
}var c=d.getContainerLocation();
G.setWidth(d.getBounds().width);
G.setDomPosition(c.left,c.top);
});
F.addListener(m,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
F.addListener(p,function(e){var a=this.getSortedSelection();
var b=e.getOriginalTarget();

for(var i=0,l=a.length;i<l;i++){this.addBefore(a[i],b);
this.addToSelection(a[i]);
}});
}}});
})();
...
(function(){var v="Boolean",u="changeSelection",t="single",s="qx.ui.core.MSelectionHandling",r="mousedown",q="losecapture",p="one",o="multi",n="_applyQuickSelection",m="mouseover",f="_applySelectionMode",l="_applyDragSelection",i="mouseup",d="mousemove",c="removeItem",h="keypress",g="__jG",j="addItem",b="additive",k="qx.event.type.Data";
qx.Mixin.define(s,{construct:function(){var H=this.SELECTION_MANAGER;
var G=this.__jG=new H(this);
this.addListener(r,G.handleMouseDown,G);
this.addListener(i,G.handleMouseUp,G);
this.addListener(m,G.handleMouseOver,G);
this.addListener(d,G.handleMouseMove,G);
this.addListener(q,G.handleLoseCapture,G);
this.addListener(h,G.handleKeyPress,G);
this.addListener(j,G.handleAddItem,G);
this.addListener(c,G.handleRemoveItem,G);
G.addListener(u,this._onSelectionChange,this);
},events:{"changeSelection":k},properties:{selectionMode:{check:[t,o,b,p],init:t,apply:f},dragSelection:{check:v,init:false,apply:l},quickSelection:{check:v,init:false,apply:n}},members:{__jG:null,selectAll:function(){this.__jG.selectAll();
},select:function(a){this.setSelected(a);
},setSelected:function(w){this.__jG.selectItem(w);
},isSelected:function(x){return this.__jG.isItemSelected(x);
},addToSelection:function(D){this.__jG.addItem(D);
},removeFromSelection:function(y){this.__jG.removeItem(y);
},selectRange:function(E,F){this.__jG.selectItemRange(E,F);
},clearSelection:function(){this.resetSelection();
},resetSelection:function(){this.__jG.clearSelection();
},replaceSelection:function(K){this.setSelection(K);
},setSelection:function(L){this.__jG.replaceSelection(L);
...
},_applySelectionMode:function(z,A){this.__jG.setMode(z);
},_applyDragSelection:function(I,J){this.__jG.setDrag(I);
},_applyQuickSelection:function(B,C){this.__jG.setQuick(B);
...
(function(){var v="__kg",u="vertical",t="under",s="above",r="qx.ui.core.selection.Widget",q="left",p="right";
qx.Class.define(r,{extend:qx.ui.core.selection.Abstract,construct:function(d){arguments.callee.base.call(this);
this.__kg=d;
},members:{__kg:null,_isSelectable:function(f){return f.isEnabled()&&f.getLayoutParent()===this.__kg;
...
},_getLocation:function(){var m=this.__kg.getContentElement().getDomElement();
...
},_scrollBy:function(D,E){},_scrollItemIntoView:function(k){this.__kg.scrollChildIntoView(k);
},getSelectables:function(){var I=this.__kg.getChildren();
var J=[];
var H;

for(var i=0,l=I.length;i<l;i++){H=I[i];

if(H.isEnabled()){J.push(H);
}}return J;
},_getSelectableRange:function(K,L){if(K===L){return [K];
}var P=this.__kg.getChildren();
var M=[];
var O=false;
var N;
...
if(N===K||N===L){if(O){M.push(N);
break;
}else{O=true;
}}
if(O&&N.isEnabled()){M.push(N);
}}return M;
},_getFirstSelectable:function(){var e=this.__kg.getChildren();

for(var i=0,l=e.length;i<l;i++){if(e[i].isEnabled()){return e[i];
}}return null;
},_getLastSelectable:function(){var C=this.__kg.getChildren();

for(var i=C.length-1;i>0;i--){if(C[i].isEnabled()){return C[i];
}}return null;
},_getRelatedSelectable:function(w,x){var A=this.__kg.getOrientation()===u;
var z=this.__kg.getChildren();
var y=z.indexOf(w);
var B;

if((A&&x===s)||(!A&&x===q)){for(var i=y-1;i>=0;i--){B=z[i];

if(B.isEnabled()){return B;
}}}else if((A&&x===t)||(!A&&x===p)){for(var i=y+1;i<z.length;i++){B=z[i];
...
qx.Class.define(a,{extend:qx.ui.core.selection.Widget,members:{_isSelectable:function(b){return (b.isEnabled()&&b.getLayoutParent()===this._getWidget().getChildrenContainer());
...
(function(){var x="qx.event.type.Data",w="horizontal",v="changeValue",u="vertical",t="",s=",",r="qx.ui.form.List",q="Boolean",p="one",o="action",O="addChildWidget",N="__mu",M="_applySpacing",L="Enter",K="Integer",J="list",I="keyinput",H="changeSelection",G="addItem",F="removeChildWidget",D="_applyOrientation",E="single",B="keypress",C="changeName",z="String",A="pane",y="removeItem";
qx.Class.define(r,{extend:qx.ui.core.AbstractScrollArea,implement:[qx.ui.form.IFormElement],include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MSelectionHandling],construct:function(j){arguments.callee.base.call(this);
this.__mu=new qx.ui.container.Composite();
this.__mu.addListener(O,this._onAddChild,this);
this.__mu.addListener(F,this._onRemoveChild,this);
this.getChildControl(A).add(this.__mu);
if(j){this.setOrientation(w);
}else{this.initOrientation();
}this.addListener(B,this._onKeyPress);
this.addListener(I,this._onKeyInput);
this.addListener(H,this._onChangeSelection);
this.__mv=t;
},events:{addItem:x,removeItem:x,changeValue:x},properties:{appearance:{refine:true,init:J},focusable:{refine:true,init:true},orientation:{check:[w,u],init:u,apply:D},spacing:{check:K,init:0,apply:M,themeable:true},enableInlineFind:{check:q,init:true},name:{check:z,nullable:true,event:C}},members:{__mv:null,__mw:null,__mu:null,SELECTION_MANAGER:qx.ui.core.selection.ScrollArea,getChildrenContainer:function(){return this.__mu;
},_onAddChild:function(e){this.fireDataEvent(G,e.getData());
},_onRemoveChild:function(e){this.fireDataEvent(y,e.getData());
},getValue:function(){var k=this.getSelection();
var m=[];
var n;

for(var i=0,l=k.length;i<l;i++){n=k[i].getValue();
if(n==null){n=k[i].getLabel();
}m.push(n);
}return m.join(s);
},setValue:function(U){var X=U.split(s);
var W=[];
var V;
...
}},_applyOrientation:function(d,f){var g=d===w;
...
},_applySpacing:function(R,S){this.__mu.getLayout().setSpacing(R);
},_onKeyPress:function(e){if(e.getKeyIdentifier()==L&&!e.isAltPressed()){var T=this.getSelection();

for(var i=0;i<T.length;i++){T[i].fireEvent(o);
}return true;
}return false;
},_onChangeSelection:function(){if(this.hasListener(v)){this.fireDataEvent(v,this.getValue());
}},_onKeyInput:function(e){if(!this.getEnableInlineFind()){return;
}var P=this.getSelectionMode();

if(!(P===E||P===p)){return;
}if(((new Date).valueOf()-this.__mw)>1000){this.__mv=t;
}this.__mv+=e.getChar();
var Q=this.findItemByLabelFuzzy(this.__mv);
...
},findItem:function(Y){Y=Y.toLowerCase();
var ba=this.getChildren();
var bb;
for(var i=0,l=ba.length;i<l;i++){bb=ba[i];

if(bb.getFormValue().toLowerCase()==Y){return bb;
}}return null;
...
this.initWrap();
},properties:{wrap:{check:d,init:true,apply:e},appearance:{refine:true,init:g}},members:{_createInputElement:function(){var i=new qx.html.Input(g);
return i;
},_applyWrap:function(a,b){this.getContentElement().setWrap(a);
},_getContentHint:function(){var c=arguments.callee.base.call(this);
c.height=c.height*4;
c.width=this._getTextSize().width*20;
return c;
}}});
})();
