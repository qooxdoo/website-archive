...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.test.Table_CellRenderer-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var k="",j="</div>",h="px;top:",g="px'>",f="<div style='position:absolute;height:",e="px;width:",d="<div style='position:absolute;height:20px;width:",c="Hello",b="icon/16/actions/go-down.png",a="World",M="bold",L="Juhu",K="icon/16/actions/go-next.png",J="$ ",I="string",H="border: 1px dotted green",G="inactive",F="decoration",E="multi<br>line",D="between",r="<",s="icon/16/actions/go-previous.png",p="icon",q="qx/decoration/Classic",n="number",o="<i>italic</i>",l="0$",m="qx/icon/Tango",t="<b>bold</b>",u="icon/16/actions/go-up.png",x="boolean",w="demobrowser.demo.test.Table_CellRenderer",z="waiting",y="red",B="left",A=">=",v="active",C="object";
qx.Class.define(w,{extend:qx.application.Native,include:[demobrowser.demo.table.MUtil],members:{main:function(){arguments.callee.base.call(this);
this.setUp();
...
qx.util.AliasManager.getInstance().add(F,q);
qx.util.AliasManager.getInstance().add(p,m);
},testDefaultRenderer:function(bv){var bw=100;
var by={value:[null,L,32,new Date()],styleLeft:[0],styleWidth:[bw],styleHeight:[20],style:[k]};
var bx=this._getNewTableDiv(bw);
var top=0;
this.permute(by,function(bL){var bM=[d,bw,h,top,g];
top+=20;
bv.createDataCellHtml(bL,bM);
bM.push(j);
bx.innerHTML+=bM.join(k);
});
},testBooleanRenderer:function(){var bk=50;
var bm=new qx.ui.table.cellrenderer.Boolean();
var bn={value:[null,true,false],styleLeft:[0],styleWidth:[bk],styleHeight:[20],style:[k]};
var bl=this._getNewTableDiv(bk);
var top=0;
this.permute(bn,function(cj){var ck=[d,bk,h,top,g];
top+=20;
bm.createDataCellHtml(cj,ck);
ck.push(j);
bl.innerHTML+=ck.join(k);
});
},testDateRenderer:function(){var ce=180;
var cg=new qx.ui.table.cellrenderer.Date();
var ci={value:new Date(),styleLeft:[0],styleWidth:[ce],styleHeight:[20],style:[k]};
var cd=qx.util.format.DateFormat;
var ch=[cd.getDateTimeInstance(),cd.getDateInstance()];
var cb=this._getNewTableDiv(ce);
var top=0;

for(var i=0;i<ch.length;i++){var cc=ch[i];
cg.setDateFormat(cc);
var cf=[d,ce,h,top,g];
top+=20;
cg.createDataCellHtml(ci,cf);
cf.push(j);
cb.innerHTML+=cf.join(k);
}},testNumberRenderer:function(){var bX=new qx.ui.table.cellrenderer.Number();
var bV=100;
var ca={value:Math.PI,styleLeft:[0],styleWidth:[bV],styleHeight:[20],style:[k]};
var bU=qx.util.format.NumberFormat;
var bY=[bU.getIntegerInstance(),new bU().set({minimumFractionDigits:3,maximumFractionDigits:5,prefix:J})];
var bS=this._getNewTableDiv(bV);
var top=0;

for(var i=0;i<bY.length;i++){var bT=bY[i];
bX.setNumberFormat(bT);
var bW=[d,bV,h,top,g];
top+=20;
bX.createDataCellHtml(ca,bW);
bW.push(j);
bS.innerHTML+=bW.join(k);
}},testImageRenderer:function(){var R=new qx.ui.table.cellrenderer.Image();
var P=50;
var S={value:[null,b,u,s,K],styleLeft:[0],styleWidth:[P],styleHeight:[20],style:[k]};
var Q=this._getNewTableDiv(P);
var top=0;
this.permute(S,function(bi){var bj=[d,P,h,top,g];
top+=20;
R.createDataCellHtml(bi,bj);
bj.push(j);
Q.innerHTML+=bj.join(k);
});
},testStringRenderer:function(){var bR=new qx.ui.table.cellrenderer.String();
var bQ=100;
var bP=20;
var bN={value:[null,c,a],styleLeft:[0],styleWidth:[bQ],styleHeight:[bP],style:[k]};
var bO=this._getNewTableDiv(bQ);
var top=0;
this.permute(bN,function(bJ){var bK=[f,bP,e,bQ,h,top,g];
top+=bP;
bR.createDataCellHtml(bJ,bK);
bK.push(j);
bO.innerHTML+=bK.join(k);
});
},testPasswordRenderer:function(){var bI=new qx.ui.table.cellrenderer.Password();
var bH=100;
var bG=20;
var bE={value:[null,c,a],styleLeft:[0],styleWidth:[bH],styleHeight:[bG],style:[k]};
var bF=this._getNewTableDiv(bH);
var top=0;
this.permute(bE,function(N){var O=[f,bG,e,bH,h,top,g];
top+=bG;
bI.createDataCellHtml(N,O);
O.push(j);
bF.innerHTML+=O.join(k);
});
},testHtmlRenderer:function(){var bc=new qx.ui.table.cellrenderer.Html();
var bb=100;
var ba=50;
var X={value:[null,t,o,E],styleLeft:[0],styleWidth:[bb],styleHeight:[ba],style:[k]};
var Y=this._getNewTableDiv(bb);
var top=0;
this.permute(X,function(bo){var bp=[f,ba,e,bb,h,top,g];
top+=ba;
bc.createDataCellHtml(bo,bp);
bp.push(j);
Y.innerHTML+=bp.join(k);
});
},testConditionalRenderer:function(){var bh=new qx.ui.table.cellrenderer.Conditional();
bh.addNumericCondition(r,0,null,y,null,null);
bh.addNumericCondition(A,100,B,null,null,null);
bh.addBetweenCondition(D,0,2,null,null,null,M,null);
bh.addRegex(l,null,null,M,H);
var bg=100;
var bf=20;
var bd={value:[null,1,2,100,200,-100,0],styleLeft:[0],styleWidth:[bg],table:[this.tableMock],styleHeight:[bf],style:[k]};
var be=this._getNewTableDiv(bg);
var top=0;
this.permute(bd,function(V){var W=[f,bf,e,bg,h,top,g];
top+=bf;
bh.createDataCellHtml(V,W);
W.push(j);
be.innerHTML+=W.join(k);
});
},testDynamicRenderer:function(){var bu=new qx.ui.table.cellrenderer.Dynamic();
bu.setCellRendererFactoryFunction(function(cl){var cn=cl.value;
var cm=qx.ui.table.cellrenderer;

switch(typeof cn){case I:if(cn.match(/^icon\//)){return new cm.Image();
}else{return new cm.String();
}case x:return new cm.Boolean();
case n:return new cm.Number();
case C:if(cn instanceof Date){return new cm.Date();
}return new cm.Default();
}});
var bt=100;
var bs=20;
var bq={value:[null,L,new Date(),30,b,false],styleLeft:[0],styleWidth:[bt],styleHeight:[bs],table:[this.tableMock],style:[k]};
var br=this._getNewTableDiv(bt);
var top=0;
this.permute(bq,function(T){var U=[f,bs,e,bt,h,top,g];
top+=bs;
bu.createDataCellHtml(T,U);
U.push(j);
br.innerHTML+=U.join(k);
});
},testReplaceRenderer:function(){var bD=new qx.ui.table.cellrenderer.Replace();
bD.setReplaceMap({1:v,2:G,3:z});
bD.addReversedReplaceMap();
var bC=100;
var bB=20;
var bz={value:[1,2,3],styleLeft:[0],styleWidth:[bC],table:[this.tableMock],styleHeight:[bB],style:[k]};
var bA=this._getNewTableDiv(bC);
var top=0;
this.permute(bz,function(co){var cp=[f,bB,e,bC,h,top,g];
top+=bB;
bD.createDataCellHtml(co,cp);
cp.push(j);
bA.innerHTML+=cp.join(k);
});
}}});
})();
