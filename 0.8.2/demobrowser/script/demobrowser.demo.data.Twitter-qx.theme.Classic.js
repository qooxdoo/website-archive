...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.data.Twitter", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.data.Twitter-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
(function(){var B="content",A="icon",z="model",y="text",x="qx.client",w="execute",v="source",u="wittemann",t="mshtml",s=".json",ba="user.profile_image_url",Y="http://twitter.com/statuses/user_timeline/",X="Enter",W="<span style='font-size: 20px'>Twitter</span>",V="<a target='_blank' href='http://twitter.com/home?status=Check%20out%20the%20%23qooxdoo%20twitter%20demo%20at%20http://bit.ly/Mc4Of'>Tweet This</a></span>",U="demobrowser.demo.data.store.Twitter",T="keydown",S="Friends",R="loaded",Q="demobrowser.demo.data.Twitter",I="selection[0]",J="<span style='font-size: 13px'>",G="Location: ",H="Details",E="Name: ",F="?callback=demobrowser.demo.data.store.Twitter.saveResult",C="User",D="http://twitter.com/statuses/friends_timeline.json",K="Postet with: ",L="failed",N="user.name",M="user.location",P="Avatar: ",O="Message: ";
...
k.setDelegate(this);
k.setLabelPath(y);
if(!qx.core.Variant.isSet(x,t)){k.setIconPath(ba);
}var o=new demobrowser.demo.data.store.Twitter(u);
o.bind(z,k,z);
var g=new qx.ui.form.Button(S);
this.getRoot().add(g,{left:10,top:55});
g.addListener(w,function(){o.setUrl(D);
},this);
var l=new qx.ui.form.Button(C);
this.getRoot().add(l,{left:90,top:55});
var d=new qx.ui.form.TextField(u);
this.getRoot().add(d,{left:140,top:55});
l.addListener(w,function(){var bc=Y+d.getValue()+s;

if(o.getUrl()==bc){o.reload();
}else{o.setUrl(bc);
}},this);
d.addListener(T,function(bf){if(bf.getKeyIdentifier()==X){l.execute();
}},this);
var h=new qx.ui.groupbox.GroupBox(H);
this.getRoot().add(h,{left:320,top:60});
h.setWidth(300);
h.setHeight(220);
h.setLayout(new qx.ui.layout.Grid(0,5));
h.add(new qx.ui.basic.Label(E),{row:0,column:0});
h.add(new qx.ui.basic.Label(G),{row:1,column:0});
h.add(new qx.ui.basic.Label(O),{row:2,column:0});
h.add(new qx.ui.basic.Label(K),{row:3,column:0});
...
var e=new qx.data.controller.Object();
e.addTarget(name,B,N);
e.addTarget(location,B,M);
e.addTarget(m,B,y);
e.addTarget(p,B,v);

if(!qx.core.Variant.isSet(x,t)){h.add(new qx.ui.basic.Label(P),{row:4,column:0});
...
bd.getChildControl(A).setWidth(50);
bd.getChildControl(A).setHeight(50);
bd.getChildControl(A).setScale(true);
}}});
qx.Class.define(U,{extend:qx.data.store.Json,statics:{saveResult:function(bb){this.__oK=bb;
}},construct:function(q){var r=Y+q+s;
arguments.callee.base.call(this,r);
},members:{_createRequest:function(b){var c=new qx.io2.ScriptLoader();
b+=F;
c.load(b,function(a){this.__oL();
},this);
},__oL:function(){var be=demobrowser.demo.data.store.Twitter.__oK;

if(be==undefined){this.setState(L);
return;
}this._marshaler.jsonToClass(be);
this.setModel(this._marshaler.jsonToModel(be));
this.fireDataEvent(R,this.getModel());
}}});
})();
...
}},properties:{model:{check:d,event:e,apply:a,nullable:true}},members:{_applyModel:function(j,k){for(var i=0;i<this.__oH.length;i++){var q=this.__oH[i][0];
var n=this.__oH[i][1];
var m=this.__oH[i][2];
var o=this.__oH[i][3];
var p=this.__oH[i][4];
var l=this.__oH[i][5];
if(k!=undefined){this.__oJ(q,n,m,k);
}if(j!=undefined){this.__oI(q,n,m,o,p,l);
}else{q[c+qx.lang.String.firstUp(n)]();
}}},addTarget:function(H,I,J,K,L,M){this.__oH.push([H,I,J,K,L,M]);
this.__oI(H,I,J,K,L,M);
},__oI:function(r,s,t,u,v,w){if(this.getModel()==null){return;
}var x=this.getModel().bind(t,r,s,v);
var y=null;

if(u){y=r.bind(s,this.getModel(),t,w);
}var z=r.toHashCode();

if(this.__oG[z]==undefined){this.__oG[z]=[];
}this.__oG[z].push([x,y,s,t,v,w]);
},removeTarget:function(f,g,h){this.__oJ(f,g,h,this.getModel());
for(var i=0;i<this.__oH.length;i++){if(this.__oH[i][0]==f&&this.__oH[i][1]==g&&this.__oH[i][2]==h){this.__oH.splice(i,1);
}}},__oJ:function(A,B,C,D){if(!(A instanceof qx.core.Object)){return ;
}var E=this.__oG[A.toHashCode()];
