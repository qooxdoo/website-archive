...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.widget.SplitPane", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.widget.SplitPane-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var B="horizontal",A="first",z="both",y="Trash",x="second",w="main",v="Show first pane",u="Documents (D:)",t="Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",s="Windows (C:)",T="Toggle Splitpane Orientation",S="Inbox",R="changeValue",Q="Desktop",P="execute",O="Presets",N="Network",M="Sent",L="/",K="Files",I="vertical",J="Workspace",G="Junk #",H="Data",E="Show both panes",F="Show second pane",C="demobrowser.demo.widget.SplitPane",D="Edit";
qx.Class.define(C,{extend:qx.application.Standalone,members:{__oU:null,main:function(){arguments.callee.base.call(this);
var b=new qx.ui.container.Scroll();
var a=new qx.ui.container.Composite(new qx.ui.layout.Basic());
a.setAllowStretchX(false);
b.add(a);
var j=new qx.ui.splitpane.Pane(B).set({width:450,height:300});
this.__oU=j;
var m=new qx.ui.container.Composite(new qx.ui.layout.Grow).set({width:200,height:100,decorator:w});
var p=new qx.ui.container.Composite(new qx.ui.layout.VBox(10)).set({padding:10,maxWidth:450,decorator:w});
var f=this.__oV();
var d=new qx.ui.form.Button(T).set({allowGrowX:false,allowGrowY:false});
d.addListener(P,this._toggle,this);
var g=new qx.ui.basic.Label(t);
g.setRich(true);
m.add(f);
p.add(d);
p.add(g);
...
j.add(m,0);
j.add(p,1);
a.add(j,{left:20,top:40});
this.getRoot().add(b,{edge:0});
var c=new qx.ui.layout.VBox(5);
var k=new qx.ui.container.Composite(c);
var n=new qx.ui.form.RadioButton(v);
var o=new qx.ui.form.RadioButton(F);
var l=new qx.ui.form.RadioButton(E);
n.setValue(A);
o.setValue(x);
l.setValue(z);
l.setChecked(true);
...
k.add(o);
k.add(l);
var h=new qx.ui.form.RadioGroup(n,o,l);
h.addListener(R,this._changeVisiblity,this);
a.add(k,{left:490,top:42});
d.focus();
},_toggle:function(){var r=this.__oU.getOrientation();
this.__oU.setOrientation(r==B?I:B);
},_changeVisiblity:function(e){var q=e.getData();

if(q==z){this._container1.show();
this._container2.show();
}else if(q==A){this._container1.show();
this._container2.exclude();
}else if(q==x){this._container1.exclude();
this._container2.show();
}},__oV:function(){var be=new qx.ui.tree.Tree();
be.setDecorator(null);
var bj=new qx.ui.tree.TreeFolder(L);
bj.setOpen(true);
be.setRoot(bj);
var bh=new qx.ui.tree.TreeFolder(Q);
bh.setOpen(true);
bj.add(bh);
var Y=new qx.ui.tree.TreeFolder(K);
var W=new qx.ui.tree.TreeFolder(J);
var X=new qx.ui.tree.TreeFolder(N);
var V=new qx.ui.tree.TreeFolder(y);
bh.add(Y,W,X,V);
var bf=new qx.ui.tree.TreeFile(s);
var U=new qx.ui.tree.TreeFile(u);
W.add(bf,U);
var bi=new qx.ui.tree.TreeFolder(S);
var ba=new qx.ui.tree.TreeFolder(O);
var bd=new qx.ui.tree.TreeFolder(M);
var bc=new qx.ui.tree.TreeFolder(y);

for(var i=0;i<100;i++){bc.add(new qx.ui.tree.TreeFile(G+i));
}var bg=new qx.ui.tree.TreeFolder(H);
var bb=new qx.ui.tree.TreeFolder(D);
...
(function(){var j="right",i="above",h="left",g="under",f="qx.ui.tree.SelectionManager";
qx.Class.define(f,{extend:qx.ui.core.selection.ScrollArea,members:{_getSelectableLocationY:function(q){var r=q.getBounds();

if(r){var top=this._getWidget().getItemTop(q);
return {top:top,bottom:top+r.height};
}},_isSelectable:function(k){return k instanceof qx.ui.tree.AbstractTreeItem&&k.isEnabled();
},_getSelectableFromMouseEvent:function(event){return this._getWidget().getTreeItem(event.getTarget());
},getSelectables:function(){var l=this._getWidget();
return l.getRoot().getItems(true,false,l.getHideRoot());
...
switch(o){case i:return p.getPreviousSiblingOf(n,false);
case g:return p.getNextSiblingOf(n,false);
case h:if(n.isOpenable()&&n.isOpen()){n.setOpen(false);
}break;
case j:if(n.isOpenable()&&!n.isOpen()){n.setOpen(true);
}break;
}return null;
}}});
})();
...
},_applyRoot:function(C,D){var E=this.getChildrenContainer();
...
if(C.hasChildren()){E.add(C.getChildrenContainer());
}C.setVisibility(this.getHideRoot()?x:z);
C.recursiveAddToWidgetQueue();
}},_applyHideRoot:function(F,G){var H=this.getRoot();

if(!H){return;
}H.setVisibility(F?x:z);
...
},getNextSiblingOf:function(g,h){if((h!==false||g.isOpen())&&g.hasChildren()){return g.getChildren()[0];
}
while(g){var parent=g.getParent();

if(!parent){return null;
}var j=parent.getChildren();
var i=j.indexOf(g);

if(i>-1&&i<j.length-1){return j[i+1];
}g=parent;
}return null;
},getPreviousSiblingOf:function(O,P){var parent=O.getParent();
...
while((P!==false||R.isOpen())&&R.hasChildren()){var T=R.getChildren();
R=T[T.length-1];
}return R;
}else{return parent;
}},getItems:function(a,b){return this.getRoot().getItems(a,b,this.getHideRoot());
},scrollChildIntoViewY:function(L,M,N){if(!this.getNextSiblingOf(L,false)){this.scrollToY(1000000);
}else{arguments.callee.base.call(this,L,M,N);
}},getTreeItem:function(c){while(c){if(c==this){return null;
}
if(c instanceof qx.ui.tree.AbstractTreeItem){return c;
}c=c.getLayoutParent();
}return null;
},_applyOpenMode:function(d,f){if(f==A){this.removeListener(A,this._onOpen,this);
}else if(f==B){this.removeListener(B,this._onOpen,this);
}
if(d==A){this.addListener(A,this._onOpen,this);
}else if(d==B){this.addListener(B,this._onOpen,this);
}},_onOpen:function(e){var U=this.getTreeItem(e.getTarget());

if(!U||!U.isOpenable()){return;
}U.setOpen(!U.isOpen());
e.stopPropagation();
}},destruct:function(){this._disposeObjects(m);
}});
})();
(function(){var s="open",r="auto",q="icon",p="middle",o="label",n="changeOpen",m="excluded",k="String",j="visible",h="opened",N="always",M="qx.ui.tree.AbstractTreeItem",L="addItem",K="Boolean",J="__kO",I="Integer",H="_applyIndent",G="changeOpenSymbolMode",F="_applyOpenSymbolMode",E="-label",z="-icon",A="resize",x="",y="removeItem",v="__kL",w="abstract",t="never",u="_applyIcon",B="__kK",C="_applyOpen",D="_applyLabel";
qx.Class.define(M,{extend:qx.ui.core.Widget,type:w,construct:function(){arguments.callee.base.call(this);
this.__kK=[];
this._setLayout(new qx.ui.layout.HBox());
this._addWidgets();
this.initOpen();
},properties:{open:{check:K,init:false,event:n,apply:C},openSymbolMode:{check:[N,t,r],init:r,event:G,apply:F},indent:{check:I,init:19,apply:H,themeable:true},parent:{check:M,nullable:true},icon:{check:k,apply:u,nullable:true,themeable:true},label:{check:k,apply:D,init:x,dispose:true}},members:{__kK:null,__kL:null,__kM:null,__kN:null,__kO:null,_addWidgets:function(){throw new Error("Abstract method call.");
},_createChildControlImpl:function(f){var g;

switch(f){case o:g=new qx.ui.basic.Label().set({appearance:this.getAppearance()+E,alignY:p});
break;
case q:g=new qx.ui.basic.Image().set({appearance:this.getAppearance()+z,alignY:p});
break;
case s:g=new qx.ui.tree.FolderOpenButton().set({alignY:p});
g.addListener(n,this._onChangeOpen,this);
g.addListener(A,this._updateIndent,this);
break;
}return g||arguments.callee.base.call(this,f);
...
if(P&&P instanceof qx.ui.core.ScrollPane){return P.getLayoutParent();
}return null;
},addWidget:function(T,U){this._add(T,U);
},addSpacer:function(){if(!this.__kO){this.__kO=new qx.ui.core.Spacer();
}else{this._remove(this.__kO);
}this._add(this.__kO);
},addOpenButton:function(){this._add(this.getChildControl(s));
},_onChangeOpen:function(e){if(this.isOpenable()){this.setOpen(e.getData());
}},addIcon:function(){var bw=this.getChildControl(q);

if(this.__kN){this._remove(bw);
}this._add(bw);
this.__kN=true;
},addLabel:function(bJ){var bK=this.getChildControl(o);

if(this.__kM){this._remove(bK);
}
if(bJ){this.setLabel(bJ);
}else{bK.setContent(this.getLabel());
}this._add(bK);
this.__kM=true;
...
}}},_applyIcon:function(bL,bM){this.getChildControl(q).setSource(bL);
},_applyLabel:function(bu,bv){this.getChildControl(o).setContent(bu);
},_applyOpen:function(bq,br){if(this.hasChildren()){this.getChildrenContainer().setVisibility(bq?j:m);
}var open=this.getChildControl(s,true);

if(open){open.setOpen(bq);
}bq?this.addState(h):this.removeState(h);
},isOpenable:function(){var W=this.getOpenSymbolMode();
return (W===N||W===r&&this.hasChildren());
},_shouldShowOpenSymbol:function(){var open=this.getChildControl(s,true);
...
}}this.__kO.setWidth((this.getLevel()+1)*this.getIndent()-bt);
},_applyIndent:function(bN,bO){this._updateIndent();
...
},getChildrenContainer:function(){if(!this.__kL){this.__kL=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?j:m});
}return this.__kL;
...
},getItems:function(bg,bh,bi){if(bi!==false){var bj=[];
}else{var bj=[this];
}var bm=this.hasChildren()&&(bh!==false||this.isOpen());

if(bm){var bl=this.getChildren();

if(bg===false){bj=bj.concat(bl);
}else{for(var i=0,bk=bl.length;i<bk;i++){bj=bj.concat(bl[i].getItems(bg,bh,false));
}}}return bj;
},recursiveAddToWidgetQueue:function(){var V=this.getItems(true,true,false);

for(var i=0,l=V.length;i<l;i++){qx.ui.core.queue.Widget.add(V[i]);
...
bH.fireNonBubblingEvent(L,qx.event.type.Data,[bI]);
}}
if(bH){qx.ui.core.queue.Widget.add(this);
}},addAt:function(ba,bb){{};

if(bb==this.__kK.length){this.add(ba);
return;
}var bf=ba.getParent();
...
if(ba.hasChildren()){bd.addAfter(ba.getChildrenContainer(),ba);
}qx.lang.Array.insertAt(this.__kK,ba,bb);

if(!be){this.__kP();
}
if(this.getTree()){ba.recursiveAddToWidgetQueue();
qx.ui.core.queue.Widget.add(this);
}},addBefore:function(bP,bQ){{};
this.addAt(bP,this.__kK.indexOf(bQ));
},addAfter:function(X,Y){{};
this.addAt(X,this.__kK.indexOf(Y)+1);
},addAtBegin:function(O){this.addAt(O,0);
},remove:function(bx){for(var i=0,l=arguments.length;i<l;i++){var bA=arguments[i];

if(this.__kK.indexOf(bA)==-1){this.warn("Cannot remove treeitem '"+bA+"'. It is not a child of this tree item.");
return;
}var bz=this.getChildrenContainer();

if(bA.hasChildren()){bz.remove(bA.getChildrenContainer());
}qx.lang.Array.remove(this.__kK,bA);
bA.setParent(null);
bz.remove(bA);
}var by=this.getTree();

if(by){by.fireNonBubblingEvent(y,qx.event.type.Data,[bA]);
}qx.ui.core.queue.Widget.add(this);
},removeAt:function(bB){var bC=this.__kK[bB];

if(bC){this.remove(bC);
}},removeAll:function(){for(var i=this.__kK.length-1;i>=0;i--){this.remove(this.__kK[i]);
}}},destruct:function(){this._disposeArray(B);
this._disposeObjects(J,v);
}});
})();
