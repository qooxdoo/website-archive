...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.progressive.ProgressiveLoader", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.progressive.ProgressiveLoader-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var r="white",q="100%",p="func",n="demobrowser.demo.progressive.ProgressiveLoader",m="black",l="Red",k="http://resources.qooxdoo.org/images/logo.gif",j="green",i="yellow",h="col ",D="gray",C="blue",B="Green",A="Yellow",z="Blue",y="renderStart",x=", ",w="purple",v="red",u="renderEnd",s="row ",t="Purple";
qx.Class.define(n,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var M=new qx.ui.progressive.renderer.table.Widths(1);
M.setWidth(0,q);
var F=new qx.ui.progressive.headfoot.Progress(M);
var K=new qx.ui.progressive.structure.Default(null,F);
var E=new qx.ui.progressive.Progressive(K);
E.setFlushWidgetQueueAfterBatch(true);
var J=function(S){var T={renderer:p,data:S};
return T;
};
var Q=new qx.ui.progressive.model.Default();
var I=[];
var N=new qx.ui.progressive.renderer.FunctionCaller();
E.addRenderer(p,N);
var L=k;
var O=new qx.ui.basic.Image(L,q,q);
E.add(O);
E.set({height:100,width:272,zIndex:99999,backgroundColor:D,opacity:0.86,batchSize:10});
this.getRoot().add(E,{top:-1000,left:-1000});
var P=[{text:l,background:v,color:r},{text:B,background:j,color:r},{text:z,background:C,color:r},{text:t,background:w,color:r},{text:A,background:i,color:m}];
this.context={document:this.getRoot(),pages:P};
E.addListener(y,function(e){var Y=e.getData().state;
var U=e.getData().initial;
var X=this.getRoot().getBounds();
var V=e.getData().state.getProgressive();
var W=V.getBounds();
var ba=Math.floor((X.width-W.width)/2);
var top=Math.floor((X.height-W.height)/2);
V.setLayoutProperties({left:ba<0?0:ba,top:top<0?0:top});
Y.getUserData().context=this.context;
Y.getUserData().initialNum=U;
},this);
E.addListener(u,function(e){qx.event.Timer.once(function(){this.getLayoutParent().remove(this);
this.dispose();
},this,0);
});
Q.addElement(J(function(a){var b=a.context;
b.tabview=new qx.ui.tabview.TabView();
b.tabview.set({});
b.document.add(b.tabview,{left:20,right:20,top:20,bottom:20});
b.page=0;
b.currentPage=null;
}));

for(var H=0;H<P.length;H++){Q.addElement(J(function(d){var g=d.context;
var f=g.pages[g.page];
g.currentPage=new qx.ui.tabview.Page(f.text);
g.currentPage.setLayout(new qx.ui.layout.VBox(4));
g.tabview.add(g.currentPage);
g.row=0;
g.col=0;
if(g.page==0){g.firstPage=g.currentPage;
}}));
for(var R=0;R<20;R++){Q.addElement(J(function(bh){var bi=bh.context;
bi.hBox=new qx.ui.container.Composite(new qx.ui.layout.HBox(4));
bi.currentPage.add(bi.hBox);
bi.tabview.setSelected(bi.currentPage);
bi.col=0;
}));

for(var G=0;G<10;G++){Q.addElement(J(function(be){var bg=be.context;
var bf=bg.pages[bg.page];
var o=new qx.ui.basic.Atom(s+bg.row+x+h+bg.col);
o.set({width:90,backgroundColor:bf.background,textColor:bf.color});
bg.hBox.add(o);
}));
Q.addElement(J(function(bb){++bb.context.col;
}));
}Q.addElement(J(function(bd){++bd.context.row;
}));
}Q.addElement(J(function(c){++c.context.page;
}));
}Q.addElement(J(function(bc){bc.context.tabview.setSelected(bc.context.firstPage);
}));
E.setDataModel(Q);
E.render();
}}});
...
for(var i=0;i<g;i++){this.__xg[i]=new qx.ui.core.ColumnData();
}},members:{__xg:null,getData:function(){return this.__xg;
},set:function(k,l){for(var m in l){switch(m){case a:this.setWidth(k,l[m]);
break;
case b:this.setMinWidth(k,l[m]);
break;
case d:this.setMaxWidth(k,l[m]);
break;
default:throw new Error("Unrecognized key: "+m);
}}},setWidth:function(n,o){if(n>this.__xg.length-1||n<0){throw new Error("Column number out of range");
}this.__xg[n].setColumnWidth(o);
},setMinWidth:function(e,f){if(e>this.__xg.length-1||e<0){throw new Error("Column number out of range");
}this.__xg[e].setMinWidth(f);
},setMaxWidth:function(h,j){if(h>this.__xg.length-1||h<0){throw new Error("Column number out of range");
}this.__xg[h].setMaxWidth(j);
}}});
})();
...
},members:{__sl:null,renderLayout:function(j,top,k,l){this.__sl=k;
},getComputedWidth:function(){return this.__sl;
},setColumnWidth:function(f){var g=null;
var h=null;
...
(function(){var c="__xh",b="qx.ui.progressive.headfoot.Abstract",a="abstract";
...
},members:{__xh:null,join:function(d){this.__xh=d;
}},destruct:function(){this._disposeFields(c);
}});
})();
(function(){var q="progressive-progressbar-indicator-undone",p="__xn",o="progress",n="%",m="progressive-progressbar-indicator-done",l="__xm",k="progressive-progressbar-percent-text",j="renderStart",i="__xo",h="progressive-progressbar-background",d="0%",g="__progressive",f="qx.ui.progressive.headfoot.Progress",c="progressive-progressbar-percent-background",b="renderEnd";
qx.Class.define(f,{extend:qx.ui.progressive.headfoot.Abstract,construct:function(v,w){arguments.callee.base.call(this);
this.setHeight(16);
this.setPadding(0);
this.__xm={};
var x=qx.theme.manager.Color.getInstance();
this.__xm.background=x.resolve(h);
this.__xm.indicatorDone=x.resolve(m);
this.__xm.indicatorUndone=x.resolve(q);
this.__xm.percentBackground=x.resolve(c);
this.__xm.percentText=x.resolve(k);
this.set({backgroundColor:this.__xm.background});
this.__xn=new qx.ui.core.Widget();
this.__xn.set({width:0,backgroundColor:this.__xm.indicatorDone});
this.add(this.__xn);
var y=new qx.ui.core.Widget();
y.set({backgroundColor:this.__xm.indicatorUndone});
this.add(y,{flex:1});
this.__xo=new qx.ui.basic.Atom(d);
this.__xo.set({width:100,backgroundColor:this.__xm.percentBackground,textColor:this.__xm.percentText});
this.add(this.__xo);
this.exclude();
},members:{__xp:null,__xm:null,__xn:null,__xo:null,join:function(a){arguments.callee.base.call(this,a);
a.addListener(j,function(e){this.__xp=e.getData().initial;
this.show();
},this);
a.addListener(o,function(e){var t=1.0-(e.getData().remaining/this.__xp);
var u=this.getBounds();

if(u){var r=Math.floor((u.width-this.__xo.getBounds().width)*t);
var s=Math.floor(t*100)+n;

if(!isNaN(r)){this.__xn.setMinWidth(r);
this.__xo.setLabel(s);
}}},this);
a.addListener(b,function(e){this.exclude();
},this);
}},destruct:function(){this._disposeFields(l,g);
this._disposeObjects(p,i);
}});
})();
...
(function(){var g="abstract",f="qx.ui.progressive.structure.Abstract",e="__xq",d="overflowY",c="auto",b="__xr";
qx.Class.define(f,{type:g,extend:qx.core.Object,construct:function(h){arguments.callee.base.call(this);
if(!h){this.__xq=new qx.ui.core.Widget();
this.__xr=this.__xq;
}else{this.__xq=null;
this.__xr=h;
}this.__xr.getContentElement().setStyle(d,c);
},members:{__xq:null,__xr:null,applyStructure:function(a){throw new Error("applyStructure() is abstract");
},getPane:function(){return this.__xr;
}},destruct:function(){if(this.__xq){this.__xq.dispose();
}this._disposeFields(e,b);
}});
})();
(function(){var d="__xv",c="qx.ui.progressive.structure.Default",b="__xt";
qx.Class.define(c,{extend:qx.ui.progressive.structure.Abstract,construct:function(e,f,g){arguments.callee.base.call(this,g);
if(!e){this.__xs=new qx.ui.progressive.headfoot.Null();
this.__xt=this.__xs;
}else{this.__xs=null;
this.__xt=e;
}if(!f){this.__xu=new qx.ui.progressive.headfoot.Null();
this.__xv=this.__xu;
}else{this.__xu=null;
this.__xv=f;
}},members:{__xt:null,__xv:null,__xs:null,__xu:null,applyStructure:function(a){this.__xt.join(a);
this.__xv.join(a);
a.add(this.__xt);
a.add(this.getPane(),{flex:1});
a.add(this.__xv);
},getHeader:function(){return this.__xt;
},getFooter:function(){return this.__xv;
}},destruct:function(){if(this.__xs){this.__xs.dispose();
this.__xs=null;
}
if(this.__xu){this.__xu.dispose();
this.__xu=null;
}this._disposeFields(b,d);
}});
})();
...
(function(){var o="qx.event.type.Data",n="dataAvailable",m="renderStart",l="_applyDataModel",k="__xB",j="progress",h="__xw",g="white",f="qx.ui.progressive.Progressive",d="progressDetail",b="__xx",c="renderEnd";
qx.Class.define(f,{extend:qx.ui.container.Composite,construct:function(p){arguments.callee.base.call(this,new qx.ui.layout.VBox());
this.__xw={};
this.set({backgroundColor:g});
...
this.__xz=false;
this.__xA=0;
},events:{"renderStart":o,"renderEnd":o,"progress":o,"progressDetail":o},properties:{dataModel:{apply:l},batchSize:{init:20},flushWidgetQueueAfterBatch:{init:false}},members:{__xw:null,__xz:null,__xB:null,__xA:null,__xy:null,__xx:null,getStructure:function(){return this.__xx;
},addRenderer:function(name,a){this.__xw[name]=a;
a.join(this,name);
},removeRenderer:function(name){if(!this.__xw[name]){throw new Error("No existing renderer named "+name);
}delete this.__xw[name];
},render:function(){if(this.__xz){return;
}this.__xz=true;
var q=new qx.ui.progressive.State({progressive:this,model:this.getDataModel(),pane:this.__xx.getPane(),batchSize:this.getBatchSize(),rendererData:this.__xD(),userData:{}});
this.__xB=new Date();
if(this.__xy){this.__xA=q.getModel().getElementCount();
this.fireDataEvent(m,{state:q,initial:this.__xA});
this.__xC(q);
}else{qx.event.Timer.once(function(){this.__xA=q.getModel().getElementCount();
this.fireDataEvent(m,{state:q,initial:this.__xA});
this.__xC(q);
this.__xy=true;
},this,10);
}},_applyDataModel:function(v,w){if(w){w.removeListener(n,this.__xE,this);
w.dispose();
}v.addListener(n,this.__xE,this);
},__xC:function(r){var s;
var u;
var t;

for(var i=r.getBatchSize();i>0;i--){s=r.getModel().getNextElement();

if(!s){this.debug("Render time: "+(new Date()-this.__xB)+"ms");
this.__xz=false;
this.fireDataEvent(c,r);
r.dispose();
return ;
}u=s.element;
t=this.__xw[u.renderer];
t.render(r,u);
this.fireDataEvent(d,{initial:this.__xA,remaining:s.remaining,element:u});
}this.fireDataEvent(j,{initial:this.__xA,remaining:s.remaining});
if(this.getFlushWidgetQueueAfterBatch()){qx.ui.core.queue.Manager.flush();
}qx.event.Timer.once(function(){this.__xC(r);
},this,0);
},__xD:function(){var x={};

for(var name in this.__xw){x[name]={};
}return x;
},__xE:function(e){this.__xA=e.getData();
this.render();
}},destruct:function(){for(var name in this.__xw){this.__xw[name].dispose();
}this._disposeFields(k,h,b);
}});
})();
...
(function(){var d="dataAvailable",c="__xF",b="qx.ui.progressive.model.Default";
qx.Class.define(b,{extend:qx.ui.progressive.model.Abstract,construct:function(){arguments.callee.base.call(this);
this.__xF=[];
},members:{__xF:null,addElements:function(e){this.__xF=this.__xF.concat(e);
this.fireDataEvent(d,this.__xF.length);
},addElement:function(a){this.__xF.push(a);
this.fireDataEvent(d,this.__xF.length);
},getElementCount:function(){return this.__xF.length;
},getNextElement:function(){if(this.__xF.length>0){return ({element:this.__xF.shift(),remaining:this.__xF.length});
}return null;
}},destruct:function(){this._disposeFields(c);
}});
})();
...
(function(){var t="pane",s="lastTab",r="bar",q="firstTab",p="right",o="bottom",n="top",m="left",k="changeValue",j="changeSelected",F="barRight",E="tabview",D="vertical",C="_applyBarPosition",B="barLeft",A="_applySelected",z="horizontal",y="qx.ui.tabview.TabView",x="__lC",w="barTop",u="barBottom",v="qx.ui.tabview.Page";
qx.Class.define(y,{extend:qx.ui.core.Widget,include:[qx.ui.core.MContentPadding],construct:function(a){arguments.callee.base.call(this);
this._createChildControl(r);
this._createChildControl(t);
var b=this.__lC=new qx.ui.form.RadioGroup;
b.setWrap(false);
b.addListener(k,this._onRadioChangeValue,this);
if(a!=null){this.setBarPosition(a);
}else{this.initBarPosition();
}},properties:{appearance:{refine:true,init:E},barPosition:{check:[m,p,n,o],init:n,apply:C},selected:{check:v,apply:A,event:j,nullable:true}},members:{__lC:null,_createChildControlImpl:function(bc){var bd;

switch(bc){case r:bd=new qx.ui.container.SlideBar();
bd.setZIndex(10);
this._add(bd);
break;
case t:bd=new qx.ui.container.Stack;
bd.setZIndex(5);
this._add(bd,{flex:1});
break;
...
O.add(N);
Q.add(M);
M.addState(this.__lD[this.getBarPosition()]);
M.addState(s);
var P=this.getChildren();

if(P[0]==M){M.addState(q);
}else{P[P.length-2].removeState(s);
}},remove:function(G){var L=this.getChildControl(t);
var J=this.getChildControl(r);
var I=G.getButton();
var K=L.getChildren();
if(this.getSelected()==G){var H=K.indexOf(G);

if(H==0){if(K[1]){this.setSelected(K[1]);
}else{this.resetSelected();
}}else{this.setSelected(K[H-1]);
}}J.remove(I);
L.remove(G);
this.__lC.remove(I);
G.removeState(this.__lD[this.getBarPosition()]);
if(G.hasState(q)){G.removeState(q);

if(K[0]){K[0].addState(q);
}}
if(G.hasState(s)){G.removeState(s);

if(K.length>0){K[K.length-1].addState(s);
}}},getChildren:function(){return this.getChildControl(t).getChildren();
},indexOf:function(be){return this.getChildControl(t).indexOf(be);
},__lD:{top:w,right:F,bottom:u,left:B},_applyBarPosition:function(R,S){var T=this.getChildControl(r);
var Y=R==m||R==p;
var W=R==p||R==o;
var X=Y?qx.ui.layout.HBox:qx.ui.layout.VBox;
var bb=this._getLayout();

if(bb&&bb instanceof X){}else{this._setLayout(bb=new X);
}bb.setReversed(W);
T.setOrientation(Y?D:z);
var ba=this.getChildren();
if(S){var U=this.__lD[S];
T.removeState(U);
for(var i=0,l=ba.length;i<l;i++){ba[i].removeState(U);
}}
if(R){var V=this.__lD[R];
T.addState(V);
for(var i=0,l=ba.length;i<l;i++){ba[i].addState(V);
}}},_applySelected:function(c,d){var h=this.getChildControl(t);
var g=this.__lC;

if(c){var f=c.getButton();
h.setSelected(c);
g.setSelected(f);
f.focus();
this.scrollChildIntoView(f,null,null,false);
}else{h.resetSelected();
g.resetSelected();
...
qx.Class.define(g,{extend:qx.ui.core.Widget,include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MRemoteLayoutHandling],construct:function(q){arguments.callee.base.call(this);
var r=this.getChildControl(m);

if(q!=null){this.setOrientation(q);
}else{this.initOrientation();
}this._add(r,{flex:1});
},properties:{appearance:{refine:true,init:c},orientation:{check:[k,d],init:k,apply:e}},members:{__lE:null,getChildrenContainer:function(){return this.getChildControl(i);
},_createChildControlImpl:function(u){var v;

switch(u){case j:v=new qx.ui.form.RepeatButton;
v.addListener(h,this._onExecuteForward,this);
v.setFocusable(false);
this._add(v);
break;
case l:v=new qx.ui.form.RepeatButton;
v.addListener(h,this._onExecuteBackward,this);
v.setFocusable(false);
this._addAt(v,0);
break;
case i:v=new qx.ui.container.Composite();
this.getChildControl(m).add(v);
break;
case m:v=new qx.ui.core.ScrollPane();
v.addListener(f,this._onResize,this);
break;
}return v||arguments.callee.base.call(this,u);
},_forwardStates:{barLeft:true,barTop:true,barRight:true,barBottom:true},scrollBy:function(w){var x=this.getChildControl(m);

if(this.__lE){x.scrollByX(w);
}else{x.scrollByY(w);
}},scrollTo:function(s){var t=this.getChildControl(m);

if(this.__lE){t.scrollToX(s);
}else{t.scrollToY(s);
...
var o=this.__lE?p.width>n.width:p.height>n.height;
...
(function(){var o="Boolean",n="qx.ui.container.Stack",m="change",k="_applyDynamic",j="qx.ui.core.Widget",h="_applySelected";
qx.Class.define(n,{extend:qx.ui.core.Widget,construct:function(){arguments.callee.base.call(this);
this._setLayout(new qx.ui.layout.Grow);
},properties:{dynamic:{check:o,init:false,apply:k},selected:{check:j,apply:h,event:m,nullable:true}},members:{_applyDynamic:function(r){var t=this._getChildren();
...
}}}},_applySelected:function(p,q){if(q){if(this.isDynamic()){q.exclude();
}else{q.hide();
}}
if(p){p.show();
...
this.setSelected(y);
},next:function(){var d=this.getSelected();
...
(function(){var k="button",j="",i="String",h="_applyIcon",g="qx.ui.tabview.Page",f="tabview-page",e="_applyLabel";
qx.Class.define(g,{extend:qx.ui.container.Composite,construct:function(l,m){arguments.callee.base.call(this);
this._createChildControl(k);
if(l!=null){this.setLabel(l);
}
if(m!=null){this.setIcon(m);
}},properties:{appearance:{refine:true,init:f},label:{check:i,init:j,apply:e},icon:{check:i,init:j,apply:h}},members:{_forwardStates:{barTop:1,barRight:1,barBottom:1,barLeft:1,firstTab:1,lastTab:1},_applyIcon:function(q,r){this.getChildControl(k).setIcon(q);
},_applyLabel:function(c,d){this.getChildControl(k).setLabel(c);
},_applyEnabled:function(n,o){arguments.callee.base.call(this,n,o);
var p=this.getChildControl(k);
n==null?p.resetEnabled():p.setEnabled(n);
},_createChildControlImpl:function(a){var b;

switch(a){case k:b=new qx.ui.form.RadioButton;
