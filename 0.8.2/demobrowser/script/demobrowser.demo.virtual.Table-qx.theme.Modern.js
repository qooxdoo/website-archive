...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.virtual.Table", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.virtual.Table-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var f=" / ",e="horizontal",d="demobrowser.demo.virtual.Table",c="white",b="#EEE",a="vertical";
qx.Class.define(d,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var g=new qx.ui.virtual.core.Scroller(10000,10000,20,100);
for(var i=0;i<10000;i++){g.getPane().getRowConfig().setItemSize(i,20+Math.round(Math.random()*40));
g.getPane().getColumnConfig().setItemSize(i,50+Math.round(Math.random()*80));
}this.getRoot().add(g,{edge:20});
g.getPane().addLayer(new qx.ui.virtual.layer.Row(c,b));
g.getPane().addLayer(new qx.ui.virtual.layer.GridLines(e));
g.getPane().addLayer(new qx.ui.virtual.layer.GridLines(a));
g.getPane().addLayer(new qx.ui.virtual.layer.HtmlCell(this));
this.__ug=new qx.ui.virtual.cell.Cell();
},getCellProperties:function(h,j){return this.__ug.getCellProperties(h+f+j);
}}});
})();
...
(function(){var D="",C="px;",B="' ",A="qx.ui.virtual.layer.HtmlCell",z="width:",w="top:",v=">",u="</div>",t="<div ",s="style='",p="height:",r="class='",q="left:",o="html";
qx.Class.define(A,{extend:qx.ui.virtual.layer.Abstract,construct:function(E){arguments.callee.base.call(this);
this.setZIndex(2);
{};
this._cellProvider=E;
},members:{_getCellSizeStyle:function(F,G,H,I){var J=D;
...
J+=p+G+C;
return J;
},_fullUpdate:function(a,b,c,d,e,f){var l=[];
var m=0;
var top=0;
var n=a;
var h=c;

for(var x=0;x<e.length;x++){var m=0;
var h=c;
var i=e[x];

for(var y=0;y<f.length;y++){var k=f[y];
var g=this._cellProvider.getCellProperties(n,h);
var j=g.insets||[0,0];
l.push(t,s,q,m,C,w,top,C,this._getCellSizeStyle(k,i,j[0],j[1]),g.style||D,B,r,g.classes||D,B,g.attributes||D,v,g.content||D,u);
h++;
m+=k;
}top+=i;
n++;
}this.getContentElement().setAttribute(o,l.join(D));
}}});
})();
...
},members:{getCssClasses:function(c,d){return e;
},getAttributes:function(m,n){return h;
},getStyles:function(o,p){return h;
},getInsets:function(i,j){return [0,0];
...
(function(){var t="Integer",s="_applyPadding",r="paddingLeft",q="Color",p="qx-cell ",o="paddingBottom",n="paddingTop",m="paddingRight",l=";",k="-",T="backgroundColor",S="_applyTextAlign",R="px",Q="_applyFont",P="_applyBackgroundColor",O="cell",N="center",M="font",L=":",K="shorthand",A="String",B="_applyAppearance",y="background-color:",z="",w="textAlign",x="left",u="textColor",v="text-align:",C="Font",D="right",F="_applyTextColor",E="justify",H="color:",G=" ",J="qx.ui.virtual.cell.Cell",I="__tb";
qx.Class.define(J,{extend:qx.ui.virtual.cell.Abstract,construct:function(){arguments.callee.base.call(this);
this.__tb=qx.ui.virtual.cell.CellStylesheet.getInstance();
this.__tc={};
this.__td={};
this.__te={};
this.__tf={};
this.initAppearance();
},properties:{appearance:{check:A,init:O,apply:B},backgroundColor:{nullable:true,check:q,apply:P,themeable:true},textColor:{nullable:true,check:q,apply:F,themeable:true},textAlign:{check:[x,N,D,E],nullable:true,themeable:true,apply:S},font:{nullable:true,apply:Q,check:C,themeable:true},paddingTop:{check:t,init:0,apply:s,themeable:true},paddingRight:{check:t,nullable:true,apply:s,themeable:true},paddingBottom:{check:t,nullable:true,apply:s,themeable:true},paddingLeft:{check:t,nullable:true,apply:s,themeable:true},padding:{group:[n,m,o,r],mode:K,themeable:true}},members:{_getCssProperties:function(){return [T,u,M,w,n,m,o,r];
},_applyAppearance:function(bA,bB){if(bB){this.__td={};
}},_getValue:function(bn){if(this.__tg){return qx.util.PropertyUtil.getThemeValue(this,bn);
}else{return qx.util.PropertyUtil.getUserValue(this,bn);
}},_storeStyle:function(bg,bh){if(this.__tg){var bi=this.__td;
}else{var bi=this.__tc;
}
if(bh===null){delete bi[bg];
}else{bi[bg]=bh;
}},_applyBackgroundColor:function(h,j,name){var h=this._getValue(name);

if(!h){this._storeStyle(name,null);
}else{this._storeStyle(name,y+qx.theme.manager.Color.getInstance().resolve(h));
}},_applyTextColor:function(bo,bp,name){var bo=this._getValue(name);

if(!bo){this._storeStyle(name,null);
}else{this._storeStyle(name,H+qx.theme.manager.Color.getInstance().resolve(bo));
}},_applyTextAlign:function(d,e,name){var d=this._getValue(name);

if(!d){this._storeStyle(name,null);
}else{this._storeStyle(name,v+d);
}},_applyFont:function(a,b,name){var a=this._getValue(name);

if(!a){this._storeStyle(name,null);
}else{var c=qx.theme.manager.Font.getInstance().resolve(a);
this._storeStyle(name,qx.bom.element.Style.compile(c.getStyles()));
}},_applyPadding:function(bc,bd,name){var bc=this._getValue(name);

if(this.__tg){var bf=this.__tf;
}else{bf=this.__te;
}
if(bc===null){delete bf[name];
}else{bf[name]=bc;
}
if(bc===null){this._storeStyle(name,null);
}else{var be=qx.lang.String.hyphenate(name);
this._storeStyle(name,be+L+bc+R);
}},getCellProperties:function(bC,bD){return {classes:this.getCssClasses(bC,bD),style:this.getStyles(bC,bD),attributes:this.getAttributes(bC,bD),content:this.getContent(bC,bD),insets:this.getInsets(bC,bD)};
},getAttributes:function(f,g){return z;
},getContent:function(bl,bm){return bl;
},getCssClasses:function(bq,br){var bv=this.getAppearance();

if(!br){br={};
}var bt=bv+k+qx.lang.Object.getKeys(br).sort().join(G);
var bz=this.__tb.getCssClass(bt);
...
for(var i=0;i<by.length;i++){bu.deleteThemeValue(this,by[i]);
}this.__tg=true;
var bs=qx.theme.manager.Appearance.getInstance().styleFrom(bv,br);

for(var bw in bs){if(bs[bw]!==undefined){bu.setThemed(this,bw,bs[bw]);
}}this.__tg=false;
var bx=qx.lang.Object.getValues(this.__td).join(l);
return p+this.__tb.computeClassForStyles(bv,bx);
},getStyles:function(bj,bk){return qx.lang.Object.getValues(this.__tc).join(l);
},getInsets:function(U,V){var ba=this.__te;
var Y=this.__tf;
var top=(ba.paddingTop!==undefined?ba.paddingTop:Y.paddingTop)||0;
var W=(ba.paddingRight!==undefined?ba.paddingRight:Y.paddingRight)||0;
var X=(ba.paddingBottom!==undefined?ba.paddingBottom:Y.paddingBottom)||0;
var bb=(ba.paddingLeft!==undefined?ba.paddingLeft:Y.paddingLeft)||0;
return [bb+W,top+X];
}},destruct:function(){this._disposeFields(I);
}});
})();
(function(){var v=".qx-cell {",u="-",t="mshtml",s="qx.ui.virtual.cell.CellStylesheet",r="none",q="__th",p="qx-cell-",o="__ti",n="default",m="ellipsis",g="__tj",l="}",j="hidden",f="} ",e=".",i="qx.client",h="content-box",k="singleton",d="absolute";
qx.Class.define(s,{extend:qx.core.Object,type:k,construct:function(){arguments.callee.base.call(this);
var x=v+
qx.bom.element.Style.compile({position:d,overflow:j,cursor:n,textOverflow:m,userSelect:r})+f;

if(!qx.core.Variant.isSet(i,t)){x+=v+qx.bom.element.BoxSizing.compile(h)+l;
}this.__th=qx.bom.Stylesheet.createElement(x);
this.__ti={};
this.__tj={};
},members:{getStylesheet:function(){return this.__th;
},getCssClass:function(w){return this.__ti[w];
},computeClassForStyles:function(a,b){var c=this.__tj[b];

if(!c){var c=this.__tk();
qx.bom.Stylesheet.addRule(this.__th,e+c,b);
this.__tj[b]=c;
}this.__ti[a]=c;
return c;
},__tk:function(){return p+this.toHashCode()+u+(this.__tl++);
},__tl:0},destruct:function(){this._disposeFields(q,o,g);
}});
})();
(function(){var m="$$theme_",l="$$user_",k="$$init_",j="qx.util.PropertyUtil";
qx.Class.define(j,{statics:{getUserValue:function(z,A){return z[l+A];
},setUserValue:function(D,E,F){D[l+E]=F;
},deleteUserValue:function(p,q){delete (p[l+q]);
},getInitValue:function(a,b){return a[k+b];
},setInitValue:function(u,v,w){u[k+v]=w;
},deleteInitValue:function(n,o){delete (n[k+o]);
},getThemeValue:function(B,C){return B[m+C];
},setThemeValue:function(g,h,i){g[m+h]=i;
},deleteThemeValue:function(x,y){delete (x[m+y]);
