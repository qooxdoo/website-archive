...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.progressive.ProgressiveTable_Conditional-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var m="",l=">",k="between",j="<",h="regex",g="!between",f="!=",e="==",d=">=",c="<=",B="color",A="__ym",z="__yk",y="text-align",x=":",w="__yi",v="font-weight",u="__yg",t='g',s="qx.ui.progressive.renderer.table.cell.Conditional",q="__yj",r="__yl",o="__yh",p=";",n="font-style";
qx.Class.define(s,{extend:qx.ui.progressive.renderer.table.cell.Abstract,construct:function(bg,bh,bi,bj){arguments.callee.base.call(this);
this.__yg=[e,f,l,j,d,c];
this.__yh=[k,g];
this.__yi=[];
this.__yj=bg||m;
this.__yk=bh||m;
this.__yl=bi||m;
this.__ym=bj||m;
},members:{__yg:null,__yh:null,__yi:null,__yj:null,__yk:null,__yl:null,__ym:null,__yn:function(a,b){if(a.align){b[y]=a.align;
}
if(a.color){b[B]=a.color;
}
if(a.style){b[n]=a.style;
}
if(a.weight){b[v]=a.weight;
}},addNumericCondition:function(Q,R,S,T,U,V,W){if(!qx.lang.Array.contains(this.__yg,Q)||R==null){throw new Error("Condition not recognized or value is null!");
}this.__yi.push({condition:Q,align:S,color:T,style:U,weight:V,value1:R,target:W});
},addBetweenCondition:function(C,D,E,F,G,H,I,J){if(!qx.lang.Array.contains(this.__yh,C)||D==null||E==null){throw new Error("Condition not recognized or value1/value2 is null!");
}this.__yi.push({condition:C,align:F,color:G,style:H,weight:I,value1:D,value2:E,target:J});
},addRegex:function(K,L,M,N,O,P){if(!K){throw new Error("regex cannot be null!");
}this.__yi.push({condition:h,align:L,color:M,style:N,weight:O,regex:K,target:P});
},_getCellStyle:function(X){if(this.__yi.length==0){return X.style||m;
}var i;
var bc;
var Y;
var bb={"text-align":this.__yj,"color":this.__yk,"font-style":this.__yl,"font-weight":this.__ym};

for(i=0;i<this.__yi.length;i++){var bf=this.__yi[i];
bc=false;

if(qx.lang.Array.contains(this.__yg,bf.condition)){if(bf.target==null){Y=X.cellData;
}else{Y=X.element.data[bf.target];
}
switch(bf.condition){case e:if(Y==bf.value1){bc=true;
}break;
case f:if(Y!=bf.value1){bc=true;
}break;
case l:if(Y>bf.value1){bc=true;
}break;
case j:if(Y<bf.value1){bc=true;
}break;
case d:if(Y>=bf.value1){bc=true;
}break;
case c:if(Y<=bf.value1){bc=true;
}break;
}}else if(qx.lang.Array.contains(this.__yh,bf.condition)){if(bf.target==null){Y=X.cellData;
}else{Y=X.element.data[bf.target];
}
switch(bf.condition){case k:if(Y>=bf.value1&&Y<=bf.value2){bc=true;
}break;
case g:if(Y<bf.value1&&Y>bf.value2){bc=true;
}break;
}}else if(bf.condition==h){if(bf.target==null){Y=X.cellData;
}else{Y=X.element.data[bf.target];
}var ba=new RegExp(bf.value1,t);
bc=ba.test(Y);
}if(bc){this.__yn(bf,bb);
}var bd=[];

for(var be in bb){if(bb[be]){bd.push(be,x,bb[be],p);
}}}return bd.join(m);
}},destruct:function(){this._disposeFields(u,o,w,q,z,r,A);
}});
})();
