...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.virtual.Gallery-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
(function(){var N="",M="selected",L="cell",K="icon",J="#f3f3f3",I="label",H="Gallery (HTML - divs)",G="default",F="demobrowser.demo.virtual.Gallery",E="text-align: center;",bp="auto",bo="color: white; background-color: #00398D;",bn="user-desktop.png",bm="<img src='",bl="'></img>",bk="demo.WidgetGallery",bj="group",bi="resize",bh="folder.png",bg="demo.AbstractGallery",U="vertical",V="gallery.GalleryCell",S="multi",T="network-server.png",Q="user-trash.png",R="off",O="top",P="icon/32/places/",W="horizontal",X="abstract",bb="position: absolute;",ba="<br>",bd="Icon #",bc="network-workgroup.png",bf="text-selected",be="Gallery (widgets)",Y="demo.HtmlGallery";
qx.Class.define(F,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var p=new demo.WidgetGallery(be);
var o=new demo.HtmlGallery(H);
o.moveTo(400,50);
}}});
qx.Class.define(bg,{extend:qx.ui.window.Window,type:X,construct:function(u){arguments.callee.base.call(this,u);
...
var v=this._createScroller();
v.set({scrollbarX:R,scrollbarY:bp});
v.getPane().addListener(bi,this._onPaneResize,this);
this.add(v);
this.manager=new qx.ui.virtual.selection.CellRectangle(v.getPane(),this).set({mode:S,drag:true});
this.manager.attachMouseEvents();
this.manager.attachKeyEvents(v);
},members:{getItemData:function(C,D){return this.items[C*this.itemPerLine+D];
},_createScroller:function(){},isItemSelectable:function(B){return !!this.getItemData(B.row,B.column);
},styleSelectable:function(a,b,c){},_onPaneResize:function(e){var s=e.getTarget();
var q=e.getData().width;
var r=Math.floor(q/this.itemWidth);

if(r==this.itemsPerLine){return;
}this.itemPerLine=r;
var t=Math.ceil(this.itemCount/r);
s.getColumnConfig().setItemCount(r);
s.getRowConfig().setItemCount(t);
},_generateItems:function(bF){var bH=[];
var bJ=[bh,Q,T,bc,bn];
var bI=qx.util.AliasManager.getInstance();
var bG=qx.util.ResourceManager;

for(var i=0;i<bF;i++){var bK=P+bJ[Math.floor(Math.random()*bJ.length)];
resolved=bI.resolve(bK);
url=bG.toUri(resolved);
bH[i]={label:bd+(i+1),icon:bK,resolvedIcon:url};
}return bH;
}}});
qx.Class.define(V,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var bO=new qx.ui.basic.Atom().set({iconPosition:O});
bO.getChildControl(I).set({padding:[0,4]});
bO.getChildControl(K).set({padding:4});
return bO;
},updateData:function(bP,bQ){bP.set({icon:bQ.icon,label:bQ.label});
},updateStates:function(bB,bC){var bE=bB.getChildControl(I);
var bD=bB.getChildControl(K);

if(bC.selected){bE.setDecorator(M);
bE.setTextColor(bf);
bD.setDecorator(bj);
}else{bE.resetDecorator();
bE.resetTextColor();
bD.resetDecorator();
}}}});
qx.Class.define(bk,{extend:demo.AbstractGallery,construct:function(bN){arguments.callee.base.call(this,bN);
this.__tW=new gallery.GalleryCell();
},members:{_createScroller:function(){var z=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.WidgetCell(this);
z.getPane().addLayer(this.layer);
var y=new qx.ui.virtual.behavior.Prefetch(z,0,0,0,0,200,300,600,800).set({interval:500});
return z;
},styleSelectable:function(bq,br,bs){if(br!==M){return;
}var bt=this.layer.getChildren();

for(var i=0;i<bt.length;i++){var bu=bt[i];
var bv=bu.getUserData(L);

if(bq.row!==bv.row||bq.column!==bv.column){continue;
}
if(bs){this.__tW.updateStates(bu,{selected:1});
}else{this.__tW.updateStates(bu,{});
}}},getCellWidget:function(h,j){var n=this.getItemData(h,j);

if(!n){return null;
}var k={row:h,column:j};
var l={};

if(this.manager.isItemSelected(k)){l.selected=true;
}var m=this.__tW.getCellWidget(n,l);
m.setUserData(L,k);
return m;
},poolCellWidget:function(A){this.__tW.pool(A);
}}});
qx.Class.define(Y,{extend:demo.AbstractGallery,construct:function(bL){arguments.callee.base.call(this,bL);
var bM=qx.theme.manager.Font.getInstance().resolve(G).getStyles();
this._fontCss=qx.bom.element.Style.compile(bM);
...
var w=new qx.ui.virtual.layer.GridLines(W,J);
x.getPane().addLayer(w);
var w=new qx.ui.virtual.layer.GridLines(U,J);
x.getPane().addLayer(w);
return x;
},_onPaneResize:function(e){arguments.callee.base.call(this,e);
this.manager.clearSelection();
},styleSelectable:function(d,f,g){this.layer.updateLayerData();
},getCellProperties:function(bw,bx){var bA=this.getItemData(bw,bx);

if(!bA){return N;
}var bz=this.manager.isItemSelected({row:bw,column:bx});
var by=bz?bo:N;
return {style:[bb,E,this._fontCss,by].join(N),content:[bm,bA.resolvedIcon,bl,ba,bA.label].join(N)};
}}});
})();
...
(function(){var o="under",n="above",m="left",l="right",k="x",j="qx.ui.virtual.selection.CellRectangle";
qx.Class.define(j,{extend:qx.ui.virtual.selection.Abstract,members:{_getItemCount:function(){return this._pane.getRowConfig().getItemCount()*this._pane.getColumnConfig().getItemCount();
...
},getSelectables:function(){var e=[];
var h=this._pane.getRowConfig().getItemCount();
var d=this._pane.getColumnConfig().getItemCount();

for(var i=0;i<h;i++){for(var f=0;f<d;f++){var g={row:i,column:f};

if(this._isSelectable(g)){e.push(g);
}}}return e;
},_getSelectableRange:function(F,G){var J=[];
var K=Math.min(F.row,G.row);
var L=Math.max(F.row,G.row);
var I=Math.min(F.column,G.column);
var N=Math.max(F.column,G.column);

for(var O=K;O<=L;O++){for(var M=I;M<=N;M++){var H={row:O,column:M};

if(this._isSelectable(H)){J.push(H);
}}}return J;
...
switch(v){case n:for(var A=u.row-1;A>=0;A--){w.row=A;

if(this._isSelectable(w)){return w;
}}break;
case o:var x=this._pane.getRowConfig().getItemCount();

for(var A=u.row+1;A<x;A++){w.row=A;

if(this._isSelectable(w)){return w;
}}break;
case m:for(var z=u.column-1;z>=0;z--){w.column=z;

if(this._isSelectable(w)){return w;
}}break;
case l:var y=this._pane.getColumnConfig().getItemCount();

for(var z=u.column+1;z<y;z++){w.column=z;
...
}},_selectableToHashCode:function(P){return P.column+k+P.row;
...
var s=t.getItemPosition(q.row);
var r=s+t.getItemSize(q.row)-1;
