...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.test.Table_RowRenderer", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.test.Table_RowRenderer-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
qx.Class.define(e,{extend:qx.application.Native,include:[demobrowser.demo.table.MUtil],members:{main:function(){arguments.callee.base.call(this);
qx.theme.manager.Meta.getInstance().initialize();
this.setUp();
this.testDefaultRenderer();
},setUp:function(){this.tableMock=this.getTableMock();
},testDefaultRenderer:function(A){var A=new qx.ui.table.rowrenderer.Default();
var E=300;
var D=20;
var C={focusedRow:[true,false],selected:[true,false],row:[1,2],table:[this.tableMock]};
var B=this._getNewTableDiv(E);
var top=0;
var self=this;
this.permute(C,function(s){var content=[];

for(var v in s){if(v!==m){content.push(v+a+s[v]);
}}content=content.join(d);
var w=self._createRowElement(B,E,D);
var u=A.getRowClass(s);

if(u){w.className=u;
}w.innerHTML=content+j;
A.updateDataRowElement(s,w);
rowHtml=[];
rowHtml.push(g);

if(u){rowHtml.push(k,u,q);
}var t=A.createRowStyle(s);
t+=n+D+b+E+f;

if(t){rowHtml.push(h,t,q);
}rowHtml.push(p,content,l);
B.innerHTML+=rowHtml.join(o);
self._rowTop+=D;
});
},_rowTop:0,_createRowElement:function(parent,x,y){var z=qx.bom.Element.create(c);
qx.bom.element.Style.setStyles(z,{position:i,width:x+r,height:y+r});
this._rowTop+=20;
