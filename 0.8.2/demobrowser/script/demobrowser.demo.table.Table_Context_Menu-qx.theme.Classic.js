...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.table.Table_Context_Menu", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.table.Table_Context_Menu-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
(function(){var C="execute",B="value",A="changeChecked",z="icon/22/actions/edit-undo.png",y="Change ID sort method",x="Disabled ctx menu",w="icon/22/status/dialog-information.png",v="On",u="Change row with ID 10",t="Default ctx menu #2",T="Remove 5 rows",S="Add 10 rows",R="Context-menu selectable",Q="",P="Table",O="icon/22/actions/list-remove.png",N="Whether the the first visible row should ",M=", ",L="demobrowser.demo.table.Table_Context_Menu",K="Default ctx menu #1",I="icon/22/actions/list-add.png",J="Selected rows:<br>",G="Keep first row",H="Off",E="be rendered completely when scrolling.",F="Show selection",D="Demonstrate use of alternate sorting algorithm.";
qx.Class.define(L,{extend:demobrowser.demo.table.TableDemo,members:{main:function(){qx.Class.include(qx.ui.table.Table,qx.ui.table.MTableContextMenu);
arguments.callee.base.call(this);
this.getRoot().setNativeContextMenu(true);
},getCaption:function(){return P;
},createTable:function(){var bm=this.createRandomRows(50);
var bl=this._tableModel=new qx.ui.table.model.Simple();
bl.setColumns([K,t,x,R]);
bl.setData(bm);
var bn=new qx.ui.table.Table(bl);
bn.set({width:600,height:400,decorator:null});
var bk=bn.getTableColumnModel();
bk.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
bn.setContextMenuHandler(3,this._contextMenuHandlerBoolean);
bn.setContextMenuHandler(2,this._contextMenuHandlerNull);
return bn;
},_contextMenuHandlerBoolean:function(m,n,o,p,q){var r;

for(var i=0;i<=1;i++){r=new qx.ui.menu.Button(i?v:H);
r.setUserData(B,i?true:false);
r.addListener(C,function(e){p.setValue(m,n,this.getUserData(B));
});
q.add(r);
}return true;
},_contextMenuHandlerNull:function(a,b,c,d,e){return true;
},createControls:function(){var bg=new qx.ui.toolbar.ToolBar();
var be,bf,bh;
bf=new qx.ui.toolbar.Part();
bg.add(bf);
be=new qx.ui.toolbar.Button(u,z);
be.addListener(C,function(g){var h=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,h[0][i]);
}this.info("Row 10 changed");
},this);
bf.add(be);
be=new qx.ui.toolbar.Button(S,I);
be.addListener(C,function(bi){var bj=this.createRandomRows(10);
this._tableModel.addRows(bj);
this.info("10 rows added");
},this);
bf.add(be);
be=new qx.ui.toolbar.Button(T,O);
be.addListener(C,function(bo){var bp=this._tableModel.getRowCount();
this._tableModel.removeRows(bp-5,5);
this.info("5 rows removed");
},this);
bf.add(be);
be=new qx.ui.toolbar.Button(F,w);
be.addListener(C,function(bq){var br=[];
bd.getSelectionModel().iterateSelection(function(s){br.push(s+Q);
});
this.showDialog(J+br.join(M));
},this);
bf.add(be);
bf=new qx.ui.toolbar.Part();
bg.add(bf);
var bd=this._table;
bh=new qx.ui.toolbar.CheckBox(G);
bh.set({checked:bd.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(N+E)});
bh.addListener(A,function(f){bd.setKeepFirstVisibleRowComplete(this.getChecked());
},bh);
bf.add(bh);
bh=new qx.ui.toolbar.CheckBox(y);
bh.set({checked:bd.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(D)});
bh.addListener(A,function(j){if(j.getData()){var l=function(Y,ba){var bb=Y[arguments.callee.columnIndex];
var bc=ba[arguments.callee.columnIndex];

if(bb%2==1&&bc%2==0){return 1;
}
if(bc%2==1&&bb%2==0){return -1;
}return (bb>bc)?1:((bb==bc)?0:-1);
};
var k=function(U,V){var W=U[arguments.callee.columnIndex];
var X=V[arguments.callee.columnIndex];

if(W%2==1&&X%2==0){return -1;
}
if(X%2==1&&W%2==0){return 1;
}return (W<X)?1:((W==X)?0:-1);
};
bd.getTableModel().setSortMethods(0,{ascending:l,descending:k});
}else{bd.getTableModel().setSortMethods(0,null);
}},bh);
bf.add(bh);
return bg;
}}});
...
(function(){var i="contextmenu",h="qx.ui.table.MTableContextMenu",g="cellContextmenu";
qx.Mixin.define(h,{construct:function(){this.addListener(g,function(e){if(this._contextMenuTimer){this._contextMenuTimer.dispose();
...
var b=this.getContextMenuHandler(d);
if(b==null){return;
}var c=this.getTableModel();
this._contextMenu=new qx.ui.menu.Menu();
this._contextMenu.addListener(i,function(e){e.preventDefault();
});
var f=b(d,e.getRow(),this,c,this._contextMenu);
if(!f){this._contextMenu.dispose();
return;
}this.setContextMenu(this._contextMenu);
},this);
this.__vE=[];
},members:{setContextMenuHandler:function(j,k){this.__vE[j]=k;
},getContextMenuHandler:function(a){return this.__vE[a];
}}});
})();
