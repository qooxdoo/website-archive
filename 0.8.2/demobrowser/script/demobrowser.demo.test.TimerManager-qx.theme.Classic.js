...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.test.TimerManager", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.test.TimerManager-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
(function(){var l="Stopping recurrent 2-second timer",k="Recurrent 2-second timer with limit 3:",j="\n",i="One-shot 1-second timer: ",h=" ",g="demobrowser.demo.test.TimerManager",f="Recurrent 5-second timer: ",e="Start timers",d="execute";
qx.Class.define(g,{extend:qx.application.Standalone,members:{startTimers:function(){var c=qx.util.TimerManager.getInstance();
c.start(function(t,u){this.__xa(f+u);
},5000,this,null,0);
c.start(function(r,s){this.__xa(i+s);
},0,this,null,1000);
c.start(function(a,b){this.__xa(k+b);

if(++a.count==3){this.__xa(l);
c.stop(b);
}},2000,this,{count:0},2000);
},__xa:function(q){q=Date.parse(new Date)+h+q+j;
this.__xb.setValue(this.__xb.getValue()+q);
},main:function(){arguments.callee.base.call(this);
var n=new qx.ui.layout.VBox(5);
var m=new qx.ui.container.Composite(n);
var o=new qx.ui.form.Button(e);
o.set({allowGrowX:false,allowGrowY:false});
o.addListener(d,function(){o.setEnabled(false);
this.startTimers();
},this);
...
qx.Class.define(b,{extend:qx.core.Object,type:a,statics:{__oy:[],__oz:{},__oA:0},members:{start:function(q,r,s,t,u){if(!u){u=r||0;
}var v=(new Date()).getTime()+u;
arguments.callee.self.__oz[++arguments.callee.self.__oA]={callback:q,userData:t||null,expireAt:v,recurTime:r,context:s||this};
this.__oB(v,arguments.callee.self.__oA);
return arguments.callee.self.__oA;
},stop:function(d){var e=arguments.callee.self.__oy;
var length=e.length;

for(var i=0;i<length;i++){if(e[i]==d){e.splice(i,1);
break;
}}delete arguments.callee.self.__oz[d];
if(e.length==0){qx.event.Idle.getInstance().removeListener(c,this.__oC);
}},__oB:function(m,n){var p=arguments.callee.self.__oy;
var o=arguments.callee.self.__oz;
var length=p.length;

for(var i=0;i<length;i++){if(o[p[i]].expireAt>m){p.splice(i,0,n);
break;
}}if(p.length==length){p.push(n);
}if(p.length==1){qx.event.Idle.getInstance().addListener(c,this.__oC,this);
}},__oC:function(){var j=(new Date()).getTime();
var g=arguments.callee.self.__oy;
var h=arguments.callee.self.__oz;
...
this.__oB(f.expireAt,l);
}else{delete h[l];
}}if(g.length==0){qx.event.Idle.getInstance().removeListener(c,this.__oC);
}}}});
})();
