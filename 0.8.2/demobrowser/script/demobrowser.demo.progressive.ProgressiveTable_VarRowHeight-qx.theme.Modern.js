...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.progressive.ProgressiveTable_VarRowHeight", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.progressive.ProgressiveTable_VarRowHeight-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var D="row",C="small icon",B="This, too, is tall.",A="No attributes specified.",z="1*",y="demobrowser/demo/icons/",x="",w="TALL ICON",v="I am tall.",u="red",n="demobrowser.demo.progressive.ProgressiveTable_VarRowHeight",t="format-justify-left.png",q="I am small.",m="end",l="multimedia-player.png",p="This, too, is small.",o="Id",s="Image",k="Note variable row heights in this table.";
qx.Class.define(n,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var e=y;
this.imageData=[{url:e+t,height:4,tooltip:C},{url:e+l,height:4,tooltip:w},x];
this.__xf=0;
...
}a=c.resolve(this.imageData[i].url);
d=b.toUri(a);
this.imageData[i].url=d;
++this.__xf;
qx.io2.ImageLoader.load(d,function(R,S){this.info("Searching for ["+R+"]");
for(var j=0;j<this.imageData.length;j++){if(this.imageData[j].url==R){this.imageData[j].height=S.height;
--this.__xf;
this.debug("Found ["+this.imageData[j].url+"].  loadCount="+this.__xf+", end="+this.imageData.length);
if(this.__xf==0){this.doLoad();
}return;
}}throw new Error("Image data for "+R+" not found");
},this);
}if(this.__xf==0){this.doLoad();
}},doLoad:function(){var O=0;
var F=this;
var N=function(f,g){var h={renderer:D,location:m,data:[g,F.imageData[f]]};
return h;
};
var M=new qx.ui.progressive.renderer.table.Widths(2);
M.setWidth(0,200);
M.setWidth(1,z);
var H=[o,s];
var J=new qx.ui.progressive.headfoot.TableHeading(M,H);
var G=new qx.ui.progressive.headfoot.Progress(M,H);
var Q=new qx.ui.progressive.structure.Default(J,G);
var E=new qx.ui.progressive.Progressive(Q);
var P=new qx.ui.basic.Atom(k);
P.setHeight(16);
P.setTextColor(u);
E.add(P);
...
L.addElements(K);
E.setDataModel(L);
var I=new qx.ui.progressive.renderer.table.Row(M);
E.addRenderer(D,I);
var r=new qx.ui.progressive.renderer.table.cell.Image();
I.addRenderer(1,r);
E.set({width:400,maxWidth:400});
this.getRoot().add(E,{left:50,top:50,bottom:50});
E.render();
}}});
})();
...
var F=qx.util.ResourceManager;
var G=E.resolve(o);
this.__xR=F.toUri(G);
},members:{__xR:null,getBlankImage:function(){return this.__xR;
},_identifyImage:function(H){throw new Error("_identifyImage() is abstract");
},_getCellStyle:function(z){var A=arguments.callee.base.call(this,z)+x+f;
return A;
},_getContentHtml:function(B){var C=[];
var D=this.__xS(B);
C.push(d);
if(qx.core.Variant.isSet(h,s)&&/\.png$/i.test(D.url)){C.push(k,this.__xR,p,e,D.url,m,l);
}else{C.push(k,D.url,l);
}if(D.imageWidth){C.push(r,D.imageWidth,j);
}if(D.imageHeight){C.push(n,D.imageHeight,j);
}C.push(y);
if(D.tooltip){C.push(v,D.tooltip,u);
}if(D.extras){C.push(D.extras);
}C.push(c);
return C.join(w);
},__xS:function(a){var b=this._identifyImage(a);
if(b==null||typeof b==g){b={url:b,tooltip:null};
}if(b.url==null){b.url=this.__xR;
}return b;
}},destruct:function(){this._disposeFields(t);
}});
})();
(function(){var e="qx.ui.progressive.renderer.table.cell.Image",d="__xT",c="string",b="",a="__xU";
qx.Class.define(e,{extend:qx.ui.progressive.renderer.table.cell.Icon,construct:function(l,m){arguments.callee.base.call(this);

if(l===undefined){this.__xT=l;
}else{this.__xT=16;
}
if(m===undefined){this.__xU=m;
}else{this.__xU=16;
}},members:{__xT:null,__xU:null,_identifyImage:function(f){var j={imageWidth:this.__xT,imageHeight:this.__xU};
var k;
if(typeof (f.cellData)==c){j.url=f.cellData;
}else{j.url=f.cellData.url;
j.tooltip=f.cellData.tooltip;
k=f.cellData.height;
}
if(j.url==b){j.url=this._imageBlank;
}else{var i=qx.util.AliasManager.getInstance();
var h=qx.util.ResourceManager;
var g=i.resolve(j.url);
j.url=h.toUri(g);
}if(k){f.height=k;
}return j;
}},destruct:function(){this._disposeFields(d,a);
}});
})();
