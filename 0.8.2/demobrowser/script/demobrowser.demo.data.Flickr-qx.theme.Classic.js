...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.data.Flickr", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.data.Flickr-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
(function(){var q="qooxdoo",p="/",o=".static.flickr.com/",n="http://api.flickr.com/services/rest/?tags=",m="",l="_",k="http://farm",j="Enter",i="content",h="keydown",M="model.photos.photo",L=".jpg",K="loaded",J="execute",I="source",H="selection[0]",G="_s.jpg",F="<span style='font-size: 20px'>Flickr</span>",E="failed",D="loading",x="demobrowser.demo.data.store.Flickr",y="demobrowser.demo.data.Flickr",v="title",w="icon",t="state",u="completed",r="Search",s="receiving",z="horizontal",A="model",C="&method=flickr.photos.search&api_key=63a8042eead205f7e0040f488c02afd9&format=json",B="id";
qx.Class.define(y,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var V=new demobrowser.demo.data.store.Flickr(q);
var N=new qx.ui.form.TextField(q);
this.getRoot().add(N,{left:30,top:50});
var O=new qx.ui.form.Button(r);
this.getRoot().add(O,{left:140,top:49});
O.addListener(J,function(){V.searchForTag(N.getValue());
},this);
N.addListener(h,function(e){if(e.getKeyIdentifier()==j){V.searchForTag(N.getValue());
}},this);
var status=new qx.ui.basic.Label(D);
this.getRoot().add(status,{left:210,top:52});
V.bind(t,status,i);
var R=new qx.ui.form.List();
R.setWidth(700);
R.setHeight(110);
R.setOrientation(z);
this.getRoot().add(R,{left:30,right:30,top:80});
var T=new qx.data.controller.List(null,R);
T.setLabelPath(v);
T.setDelegate({configureItem:function(g){g.setShow(w);
}});
var P={converter:function(Y,ba){return (k+ba.getFarm()+o+ba.getServer()+p+Y+l+ba.getSecret()+G);
}};
T.setIconOptions(P);
T.setIconPath(B);
V.bind(M,T,A);
var Q=new qx.ui.basic.Image();
this.getRoot().add(Q,{left:30,top:200});
var U={converter:function(f){if(f){return (k+f.getFarm()+o+f.getServer()+p+f.getId()+l+f.getSecret()+L);
}return m;
}};
T.bind(H,Q,I,U);
...
qx.Class.define(x,{extend:qx.data.store.Json,statics:{RESULT:null},construct:function(b){jsonFlickrApi=function(W){demobrowser.demo.data.store.Flickr.RESULT=W;
};
var c=n+b;
arguments.callee.base.call(this,c);
},members:{searchForTag:function(d){if(d!=m){this.setUrl(n+d);
}},_createRequest:function(bb){var bc=new qx.io2.ScriptLoader();
this.setState(s);
bb+=C;
bc.load(bb,function(a){this.__ov();
},this);
},__ov:function(){this.setState(u);
var X=demobrowser.demo.data.store.Flickr.RESULT;

if(X==undefined){this.setState(E);
return;
}this._marshaler.jsonToClass(X);
this.setModel(this._marshaler.jsonToModel(X));
...
(function(){var m="model",l="changeSelection",k="change",j="qx.data.Array",h="qx.data.controller.MSelection",g="_applySelection",f="qx.ui.form.SelectBox",e="changeSelected",d="target";
qx.Mixin.define(h,{construct:function(){if(!qx.Class.hasProperty(this.constructor,d)){throw new Error("Target property is needed.");
}this.__nz=0;
this.setSelection(new qx.data.Array());
},properties:{selection:{check:j,event:l,apply:g,init:null}},members:{_applySelection:function(r,s){if(this._selectionArrayListenerId!=undefined&&s!=undefined){s.removeListenerById(this._selectionArrayListenerId);
}this._selectionArrayListenerId=r.addListener(k,this.__nA,this);
},__nA:function(){this._updateSelection();
},__nB:function(){if(this.__nz>0||this.getTarget()==null){return;
}
if(this.__nD()){var o=this.getTarget().getSelection();
}else if(this.__nE()){var o=[this.getTarget().getSelected()];
}for(var i=0;i<o.length;i++){var n=o[i].getUserData(m);

if(!this.getSelection().contains(n)){this.getSelection().splice(this.getSelection().length,0,n);
}}var p=[];

for(var i=0;i<o.length;i++){p[i]=o[i].getUserData(m);
}for(var i=this.getSelection().length-1;i>=0;i--){if(!qx.lang.Array.contains(p,this.getSelection().getItem(i))){this.getSelection().splice(i,1);
}}this.fireDataEvent(l,this.getSelection());
},_addChangeTargetListener:function(t,u){if(this.__nC!=undefined&&u!=undefined){u.removeListenerById(this.__nC);
}if(this.__nD()){this.__nC=t.addListener(l,this.__nB,this);
}else if(this.__nE()){this.__nC=t.addListener(e,this.__nB,this);
...
for(var i=0;i<v.length;i++){w[i]=v[i].getUserData(m);
...
},__nD:function(){var q=this.getTarget().constructor;
return qx.Class.implementsInterface(q,qx.ui.core.IMultiSelection);
},__nE:function(){var a=this.getTarget().constructor;
return a.classname==f;
},__nF:function(b){var c=this.getTarget().getSelectables();
for(var i=0;i<c.length;i++){if(c[i].getUserData(m)==b){if(this.__nD()){this.getTarget().addToSelection(c[i]);
}else if(this.__nE()){this.getTarget().setSelected(c[i]);
}return;
}}},_startSelectionModification:function(){this.__nz++;
},_endSelectionModification:function(){this.__nz>0?this.__nz--:null;
}}});
})();
(function(){var bk="model",bj="labelBindingId",bi="BindingId",bh="String",bg="iconBindingId",bf="changeModel",be="_applyLabelOptions",bd="_applyLabelPath",bc="changeTarget",bb="changeLength",by="_applyModel",bx="change",bw="icon",bv="qx.data.controller.List",bu="_applyIconPath",bt="_applyDelegate",bs="changeDelegate",br="_applyTarget",bq="]",bp="qx.data.IListData",bn="model[",bo="label",bl="_applyIconOptions",bm=".";
qx.Class.define(bv,{extend:qx.core.Object,include:qx.data.controller.MSelection,construct:function(bD,bE,bF){arguments.callee.base.call(this);
this.__nG=[];
this.__nH=[];
this.__nI={};
...
}},properties:{model:{check:bp,apply:by,event:bf,nullable:true},target:{apply:br,event:bc,nullable:true,init:null},labelPath:{check:bh,apply:bd,nullable:true},iconPath:{check:bh,apply:bu,nullable:true},labelOptions:{apply:be,nullable:true},iconOptions:{apply:bl,nullable:true},delegate:{apply:bt,event:bs,init:null,nullable:true}},members:{update:function(){this.__nR();
this.__nM();
this.__nQ();
this._updateSelection();
},_applyDelegate:function(J,K){this._setConfigureItem(J,K);
this._setFilter(J,K);
this._setCreateItem(J,K);
this._setBindItem(J,K);
},_applyIconOptions:function(X,Y){this.__nQ();
},_applyLabelOptions:function(bz,bA){this.__nQ();
},_applyIconPath:function(E,F){this.__nQ();
},_applyLabelPath:function(M,N){this.__nQ();
},_applyModel:function(y,z){if(z!=undefined){if(this.__nJ!=undefined){z.removeListenerById(this.__nJ);
}
if(this.__changeModelListenerId!=undefined){z.removeListenerById(this.__changeModelListenerId);
}}if(y!=null){this.__nJ=y.addListener(bb,this.__nM,this);
this.__nK=y.addListener(bx,this.__nL,this);
this.__nR();
this.__nM();
if(this.getTarget()!=null){var B=this.getModel();
var C=this.getTarget().getChildren();

for(var i=0,l=this.__nG.length;i<l;i++){var D=B.getItem(this.__nS(i));
var A=C[i];
A.setUserData(bk,D);
...
var q=this.__nG.length;
var p=this.getTarget().getChildren().length;
if(q>p){for(var j=p;j<q;j++){this.__nN(this.__nS(j));
}}else if(q<p){for(var j=p;j>q;j--){this.__nO();
}}},_createItem:function(){var k=this.getDelegate();
if(k!=null&&k.createItem!=null){var h=k.createItem();
}else{var h=new qx.ui.form.ListItem();
}if(k!=null&&k.configureItem!=null){k.configureItem(h);
}return h;
},__nN:function(bB){var bC=this._createItem();
bC.setUserData(bk,this.getModel().getItem(this.__nS(bB)));
...
this.getTarget().removeAt(e);
g.destroy();
this.__nP=false;
this._endSelectionModification();
},_bindListItem:function(m,n){var o=this.getDelegate();
if(o!=null&&o.bindItem!=null){o.bindItem(this,m,n);
}else{this.bindProperty(this.getLabelPath(),bo,this.getLabelOptions(),m,n);
if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),bw,this.getIconOptions(),m,n);
}}},bindProperty:function(bK,bL,bM,bN,bO){bN.setUserData(bk,this.getModel().getItem(bO));
var bM=qx.lang.Object.clone(bM);

if(bM!=null){this.__nI[bL]=bM.onSetOk;
delete bM.onSetOk;
}else{bM={};
this.__nI[bL]=null;
}bM.onSetOk=qx.lang.Function.bind(this._onBindingSet,this,bO);
var bP=bn+bO+bq;

if(bK!=null){bP+=bm+bK;
}var bQ=this.bind(bP,bN,bL,bM);
bN.setUserData(bL+bi,bQ);
if(!qx.lang.Array.contains(this.__nH,bL)){this.__nH.push(bL);
}},_onBindingSet:function(r,s,t){for(var i=0;i<this.__nH.length;i++){if(this.__nI[this.__nH[i]]!=null){this.__nI[this.__nH[i]]();
}}var u=this.getModel().getItem(this.__nS(r));
t.setUserData(bk,u);
this._updateSelection();
},_removeBindingsFrom:function(bG){for(var i=0;i<this.__nH.length;i++){var bH=bG.getUserData(this.__nH[i]+bi);

if(bH!=null){this.removeBinding(bH);
}}},__nQ:function(){if(this.getTarget()==null){return;
}var ba=this.getTarget().getChildren();
for(var i=0;i<ba.length;i++){this._removeBindingsFrom(ba[i]);
this._bindListItem(ba[i],this.__nS(i));
}},_setConfigureItem:function(v,w){if(v!=null&&v.configureItem!=null&&this.getTarget()!=null){var x=this.getTarget().getChildren();
...
}},_setCreateItem:function(a,b){if(this.getTarget()==null||this.getModel()==null){return;
}this._startSelectionModification();
var d=this.getTarget().getChildren();

for(var i=0,l=this.getTarget().length;i<l;i++){var c=d[i].getUserData(bj);
this.removeBinding(c);
c=d[i].getUserData(bg);

if(c!=null){this.removeBinding(c);
}}this.getTarget().removeAll();
this.update();
this._endSelectionModification();
this._updateSelection();
},_setFilter:function(O,P){if(this.getTarget()==null||this.getModel()==null){return;
}this._startSelectionModification();
var S=this.getTarget().getChildren();

for(var i=0,l=this.getTarget().length;i<l;i++){var U=S[i].getUserData(bj);
this.removeBinding(U);
U=S[i].getUserData(bg);

if(U!=null){this.removeBinding(U);
}}var R=this.__nG;
this.__nR();
if(R.length>this.__nG.length){for(var j=R.length;j>this.__nG.length;j--){this.getTarget().removeAt(j-1);
}}else if(R.length<this.__nG.length){for(var j=R.length;j<this.__nG.length;j++){var T=this._createItem();
this.getTarget().add(T);
}}var Q=this.getTarget().getChildren();

for(var i=0;i<Q.length;i++){this._bindListItem(Q[i],this.__nS(i));
}this._endSelectionModification();
this._updateSelection();
},__nR:function(){var H=this.getModel();

if(H==null){return;
}var I=this.getDelegate();

if(I!=null){var G=I.filter;
}this.__nG=[];

for(var i=0;i<H.length;i++){if(G==null||G(H.getItem(i))){this.__nG.push(i);
}}},__nS:function(L){return this.__nG[L];
}}});
})();
