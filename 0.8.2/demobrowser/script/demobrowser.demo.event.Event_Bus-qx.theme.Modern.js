...
(function(){var r="",q="X",p="start",o="finished",n="loading",m="_anotherStartCallback",l="_finishedCallback",k="_startCallback",j="_anotherFinishedCallback",i="_loadingCallback",d="logger",h="Callback",g="Dispatch",c="Message",b="Receiving",f="Subscribe",e="demobrowser.demo.event.Event_Bus";
qx.Class.define(e,{extend:demobrowser.demo.event.EventDemo,members:{main:function(){arguments.callee.base.call(this);
this._initLogger([f,g,b,c,h],document.getElementById(d),50);
var a=qx.event.message.Bus;
a.subscribe(p,this._startCallback,this);
this._log([q,r,r,p,k]);
a.subscribe(p,this._anotherStartCallback,this);
this._log([q,r,r,p,m]);
a.subscribe(n,this._loadingCallback,this);
this._log([q,r,r,n,i]);
a.subscribe(o,this._finishedCallback,this);
this._log([q,r,r,o,l]);
a.subscribe(o,this._anotherFinishedCallback,this);
this._log([q,r,r,o,j]);
...
(function(){var n="function",m="qx.event.message.Bus",l="*",k="undefined",j="string",h="singleton";
qx.Class.define(m,{type:h,extend:qx.core.Object,statics:{getSubscriptions:function(){return this.getInstance().getSubscriptions();
...
},dispatch:function(o){return this.getInstance().dispatch.apply(this,arguments);
}},construct:function(){this.__nu={};
},members:{getSubscriptions:function(){return this.__nu;
},subscribe:function(w,x,y){if(!w||typeof x!=n){this.error("Invalid parameters! "+[w,x,y]);
return false;
}var z=this.getSubscriptions();

if(this.checkSubscription(w)){if(this.checkSubscription(w,x,y)){this.warn("Object method already subscribed to "+w);
return false;
}z[w].push({subscriber:x,context:y||null});
return true;
}else{z[w]=[{subscriber:x,context:y||null}];
return true;
...
},unsubscribe:function(s,t,u){var v=this.getSubscriptions();

if(!v[s]){return false;
}
if(t){for(var i=0;i<v[s].length;i++){if(v[s][i].subscriber==t&&v[s][i].context==(u||null)){v[s].splice(i,1);
return true;
}}}v[s]=null;
return true;
},dispatch:function(D){if(typeof D==j){var I=typeof arguments[1]!=k?arguments[1]:true;
D=new qx.event.message.Message(D,I);
}var F=this.getSubscriptions();
var G=D.getName();

for(var K in F){var H=K.indexOf(l);

if(H>-1){if(H==1||K.substr(0,H)==G.substr(0,H)){for(var i=0;i<F[K].length;i++){var J=F[K][i].subscriber;
var E=F[K][i].context;
J.call(E,D);
}}}else{if(K==G){for(var i=0;i<F[G].length;i++){var J=F[G][i].subscriber;
var E=F[G][i].context;
J.call(E,D);
}return true;
}}}}}});
