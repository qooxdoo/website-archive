...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.table.Table_Cell_Editor", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.table.Table_Cell_Editor-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var s="checkbox",r='Property',q="options",p="password",o="type",n='email',m='password',l='options',k='type',j='0',bi='Use only numbers!',bh='regExp',bg='[0-9]+',bf='newsletter',be='admin',bd='editable combobox',bc='secret',bb="demobrowser.demo.table.Table_Cell_Editor",ba='status',Y="validationFunc",z='1',A="regExp",x="required",y='Doubleclick on cell to edit value',v='Newsletter',w='Inactive',t='User Name',u='telephone',D="dataChanged",E='Status',M='Active',K='validationFunc',Q='enter website',O="editable",U='role',S='username',G='jdoe',X='Telephone',W='manager',V='enter valid telephone number',F='Waiting',I='Email',J='Role',L="Property editor table",N='2',P='user',R='website',T='Password',B="email",C='Website',H='enter valid email';
qx.Class.define(bb,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return L;
},main:function(){arguments.callee.base.call(this);
this._container.resetWidth();
this._container.setResizable(false);
},createTable:function(){var bD=[[S,t,G,{'required':true}],[m,T,bc,{'type':m,'required':true}],[U,J,bd,{'options':[be,W,P],'editable':true}],[ba,E,j,{'options':[[w,null,j],[M,null,z],[F,null,N]]}],[n,I,H,{'type':n}],[u,X,V,{'regExp':bg,'failMsg':bi}],[R,C,Q,{'validationFunc':function(bH){if(bH.search(/^http:/)!=-1){window.open(bH);
}return bH;
}}],[bf,v,true,{'type':s}]];
var bC=function(b){var h=b.table;
var e=h.getTableModel();
var f=e.getRowData(b.row);
var i=f[3];
var g;

for(var d in i){switch(d){case o:switch(i[k]){case s:return new qx.ui.table.cellrenderer.Boolean;
case p:return new qx.ui.table.cellrenderer.Password;
}break;
case q:var g=new qx.ui.table.cellrenderer.Replace;
var c={};
i[l].forEach(function(bG){if(bG instanceof Array){c[bG[0]]=bG[2];
}});
g.setReplaceMap(c);
g.addReversedReplaceMap();
return g;
}}return new qx.ui.table.cellrenderer.Default();
};
propertyCellRendererFactory=new qx.ui.table.cellrenderer.Dynamic(bC);
var bA=function(bp){var bu=bp.table;
var bs=bu.getTableModel();
var bt=bs.getRowData(bp.row);
var bw=bt[3];
var br=new qx.ui.table.celleditor.TextField;
var bv=null;

for(var bq in bw){switch(bq){case q:if(bw.editable){br=new qx.ui.table.celleditor.ComboBox();
}else{br=new qx.ui.table.celleditor.SelectBox();
}br.setListData(bw[l]);
break;
case O:break;
case o:switch(bw[k]){case p:br=new qx.ui.table.celleditor.PasswordField;
break;
case s:br=new qx.ui.table.celleditor.CheckBox;
break;
case B:br.setValidationFunction(function(bm,bn){var bo=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{2}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/;
...
case A:br.setValidationFunction(function(bj,bk){var bl=new RegExp(bw[bh]);
...
break;
case Y:br.setValidationFunction(bw[K]);
break;
case x:bv=function(bE,bF){if(!bE){alert("You need to supply a value here");
return bF;
}return bE;
};
break;
...
propertyCellEditorFactory=new qx.ui.table.celleditor.Dynamic(bA);
var by=new qx.ui.table.model.Simple();
by.setColumns([r,r,y]);
var bx={tableColumnModel:function(a){return new qx.ui.table.columnmodel.Resize(a);
}};
var bz=new qx.ui.table.Table(by,bx);
bz.setDecorator(null);
bz.setColumnVisibilityButtonVisible(false);
bz.setKeepFirstVisibleRowComplete(true);
bz.setStatusBarVisible(false);
bz.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var bB=bz.getTableColumnModel();
bB.setColumnVisible(0,false);
bB.getBehavior().setWidth(1,100);
bB.getBehavior().setWidth(2,300);
by.setColumnEditable(2,true);
bB.setDataCellRenderer(2,propertyCellRendererFactory);
bB.setCellEditorFactory(2,propertyCellEditorFactory);
bz.getTableModel().setData(bD);
bz.getTableModel().addListener(D,function(event){if(!(event instanceof qx.event.type.Data)){return;
}var bL=event.getData();
var bI=this.getTableModel();
var bK=bI.getValue(0,bL.firstRow);
var bJ=bI.getValue(bL.firstColumn,bL.firstRow);
this.info("User edited property '"+bK+"' and entered value '"+bJ+"'.");
},bz);
return bz;
}}});
})();
...
if(!g){this.error("No function provided! Aborting.");
}var f=g(d);
...
(function(){var t="",s="Function",r="qx.ui.table.celleditor.ComboBox",q="number",p="Array",o="table-editor-combobox",n="appear";
qx.Class.define(r,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,construct:function(){arguments.callee.base.call(this);
},properties:{validationFunction:{check:s,nullable:true,init:null},listData:{check:p,init:null,nullable:true}},members:{__vF:null,createCellEditor:function(d){var f=new qx.ui.form.ComboBox().set({appearance:o});
var g=d.value;
var k=d.table.getTableColumnModel().getDataCellRenderer(d.col);
var h=k._getContentHtml(d);
...
for(var i=0,l=e.length;i<l;i++){var m=e[i];

if(m instanceof Array){j=new qx.ui.form.ListItem(m[0],m[1],m[2]);
}else{j=new qx.ui.form.ListItem(m,null,m);
}f.add(j);
}}f.setValue(t+g);
f.addListener(n,function(){f.selectAll();
});
return f;
},getCellEditorValue:function(a){var c=a.getValue()||t;
var b=this.getValidationFunction();

if(!this.__vF&&b){c=b(c,a.originalValue);
this.__vF=true;
}
if(typeof a.originalValue==q){c=parseFloat(c);
}return c;
}}});
})();
...
(function(){var q="",p="Function",o="qx.ui.table.celleditor.SelectBox",n="number",m="table-editor-selectbox",k="appear",j="Array";
qx.Class.define(o,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,construct:function(){arguments.callee.base.call(this);
},properties:{validationFunction:{check:p,nullable:true,init:null},listData:{check:j,init:null,nullable:true}},members:{__vG:null,createCellEditor:function(a){var c=new qx.ui.form.SelectBox().set({appearance:m});
var d=a.value;
c.originalValue=d;
var g=a.table.getTableColumnModel().getDataCellRenderer(a.col);
var e=g._getContentHtml(a);

if(d!=e){d=e;
}if(d===null){d=q;
}var b=this.getListData();

if(b){var f;

for(var i=0,l=b.length;i<l;i++){var h=b[i];

if(h instanceof Array){f=new qx.ui.form.ListItem(h[0],h[1],h[2]);
}else{f=new qx.ui.form.ListItem(h,null,h);
}c.add(f);
}}c.setValue(q+d);
c.addListener(k,function(){c.open();
});
return c;
},getCellEditorValue:function(r){var t=r.getValue()||q;
var s=this.getValidationFunction();

if(!this.__vG&&s){t=s(t,r.originalValue);
this.__vG=true;
}
if(typeof r.originalValue==n){t=parseFloat(t);
}return t;
}}});
})();
...
},properties:{validationFunction:{check:g,nullable:true,init:null}},members:{__vH:null,createCellEditor:function(a){var b=new qx.ui.form.PasswordField();
b.setAppearance(d);
...
if(!this.__vH&&j){k=j(k,i.originalValue);
this.__vH=true;
}
...
},members:{createCellEditor:function(j){var l=new qx.ui.container.Composite(new qx.ui.layout.HBox().set({alignX:d,alignY:b})).set({focusable:true});
var k=new qx.ui.form.CheckBox().set({checked:j.value});
l.add(k);
...
return l;
},getCellEditorValue:function(o){return o.getChildren()[0].getChecked();
}}});
})();
...
(function(){var c="Function",b="qx.ui.table.celleditor.Dynamic";
qx.Class.define(b,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,construct:function(a){arguments.callee.base.call(this);

if(a){this.setCellEditorFactoryFunction(a);
}},properties:{cellEditorFactoryFunction:{check:c,nullable:true,init:null}},members:{__vI:null,createCellEditor:function(d){var e=this.getCellEditorFactoryFunction();
{};
this.__vI=e(d);
var f=this.__vI.createCellEditor(d);
return f;
},getCellEditorValue:function(g){var h=this.getCellEditorFactoryFunction();
{};
var i=this.__vI.getCellEditorValue(g);
return i;
}}});
})();
