...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.data.Gears-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
(function(){var o="value",n='undefined',m="",l="model",k="application/x-googlegears",j="demobrowser.demo.data.store.Gears",i="<span style='font-size: 20px'>google Gears</span>",h="loaded",g="Age: ",f="http://code.google.com/apis/gears/install.html",I="lastname",H="firstname",G="age",F="demobrowser.demo.data.Gears",E="First Name: ",D='qooxdoo',C='Gears.Factory',B='select * from Persons',A="delete from Persons",z="LastName: ",v=' (Firstname varchar(255), Lastname varchar(255), Age int)',w="Person",t='ie_mobile',u="none",r='create table if not exists Persons',s="This demo requires Gears to be installed. Install now?",p='insert into Persons values (?, ?, ?)',q='beta.database',x="changeBubble",y="object";
...
this.getRoot().add(X,{left:10,top:50});
X.setLayout(new qx.ui.layout.Grid(0,3));
X.add(new qx.ui.basic.Label(E),{row:0,column:0});
X.add(new qx.ui.basic.Label(z),{row:1,column:0});
X.add(new qx.ui.basic.Label(g),{row:2,column:0});
var R=new qx.ui.form.TextField();
X.add(R,{row:0,column:1});
var T=new qx.ui.form.TextField();
X.add(T,{row:1,column:1});
var U=new qx.ui.form.Spinner(0,0,1000);
X.add(U,{row:2,column:1});
var V=new qx.data.controller.Object();
V.addTarget(R,o,H,true);
V.addTarget(T,o,I,true);
V.addTarget(U,o,G,true);
var W=new demobrowser.demo.data.store.Gears();
W.bind(l,V,l);
...
qx.Class.define(j,{extend:qx.data.store.Json,construct:function(){arguments.callee.base.call(this);
this.initGears();
if(!window.google||!google.gears){if(confirm(s)){location.href=f;
return;
}return;
}this.__oD=this.initDB();
this.__oE();
},members:{__oE:function(K){var L=this.getPerson();
this._marshaler.jsonToClass(L,true);
this.setModel(this._marshaler.jsonToModel(L));
this.getModel().addListener(x,this.__oF,this);
this.fireDataEvent(h,this.getModel());
},__oF:function(M){var N=this.getModel();
this.update(N.getFirstname(),N.getLastname(),N.getAge());
},update:function(a,b,c){if(!google.gears.factory||!this.__oD){return;
}this.__oD.execute(A);
this.__oD.execute(p,[a,b,c]);
},getPerson:function(){var O={};
O.firstname=m;
O.lastname=m;
O.age=0;

if(!google.gears.factory||!this.__oD){return O;
}var Q=this.__oD.execute(B);
var P=0;

if(Q.isValidRow()){O.firstname=Q.field(0);
O.lastname=Q.field(1);
O.age=parseInt(Q.field(2));
}Q.close();
return O;
},initDB:function(){if(window.google&&google.gears){try{var J=google.gears.factory.create(q);

if(J){J.open(D);
J.execute(r+v);
}}catch(e){this.error('Could not create database: '+e.message);
}}return J;
},initGears:function(){if(window.google&&google.gears){return;
}var d=null;
if(typeof GearsFactory!=n){d=new GearsFactory();
}else{try{d=new ActiveXObject(C);
if(d.getBuildInfo().indexOf(t)!=-1){d.privateSetGlobalObject(this);
}}catch(e){if((typeof navigator.mimeTypes!=n)&&navigator.mimeTypes[k]){d=document.createElement(y);
d.style.display=u;
d.width=0;
d.height=0;
d.type=k;
