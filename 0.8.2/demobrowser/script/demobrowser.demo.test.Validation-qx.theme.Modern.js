...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.test.Validation", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.test.Validation-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var k="<br>",j="male",i="new gender: ",h="new age: ",g="new name: ",f="female",e="demobrowser.demo.test.Validation",d="<b>Setting the age ",c="property to 27</b><br>",b="<br>old name: ",Q=")",P="age:",O="qx.Person",N="<b>Testing the gender ",M="fmale",L="<br><b>Setting the name to 'Hans'</b><br>",K="property to 'female'</b><br>",J="<br><b>Setting the gender ",I="must be longer than one characters. (",H="<font color='red'>GENDER NOT SET</font>",r="<br>old gender: ",s="- name: Name is validated to a minimum length of two.<br/>",p="__wY",q="<br><b>Setting the age ",n="- age: Age is validated to be a number.<br/>",o="<b>Initial property values</b><br>",l="",m="<b>Setting the name to ''</b><br>",t="property to 'true'</b><br>",u="- gender: Gender is validated to be either male or female.",z="property to 'fmale'</b><br>",y="gender:",B="<br>old age: ",A="Hans",D="The demo creates an internal class containing three properties:<br/>",C="Validation Error: Name ",w="<font color='red'>AGE NOT SET</font>",G="<font color='red'>NAME NOT SET</font>",F="My name",E="white",v="name:",x="<b>Description</b><br/>";
qx.Class.define(e,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
qx.Class.define(O,{extend:qx.core.Object,properties:{name:{init:F,validate:p},age:{init:18,validate:qx.util.Validate.number()},gender:{init:j,validate:qx.util.Validate.inArray([j,f])}},members:{__wY:function(a){if(a.length<2){throw new qx.core.ValidationError(C+I+a+Q);
}}}});
var R=new qx.Person();
var S=new qx.ui.embed.Html();
S.setBackgroundColor(E);
S.set({padding:10,width:250,height:360});
this.getRoot().add(S,{left:10,top:10});
S.setHtml(o);
S.setHtml(S.getHtml()+v+R.getName()+k);
S.setHtml(S.getHtml()+P+R.getAge()+k);
S.setHtml(S.getHtml()+y+R.getGender()+k);
S.setHtml(S.getHtml()+L);
R.setName(A);
S.setHtml(S.getHtml()+g+R.getName()+k);
S.setHtml(S.getHtml()+m);
try{R.setName(l);
S.setHtml(S.getHtml()+g+R.getName()+k);
}catch(U){S.setHtml(S.getHtml()+G+b+R.getName()+k);
}S.setHtml(S.getHtml()+q+c);
R.setAge(27);
S.setHtml(S.getHtml()+h+R.getAge()+k);
S.setHtml(S.getHtml()+d+t);
try{R.setAge(true);
S.setHtml(S.getHtml()+h+R.getAge()+k);
}catch(V){S.setHtml(S.getHtml()+w+B+R.getAge()+k);
}S.setHtml(S.getHtml()+J+K);
R.setGender(f);
S.setHtml(S.getHtml()+i+R.getGender()+k);
S.setHtml(S.getHtml()+N+z);
try{R.setAge(M);
S.setHtml(S.getHtml()+i+R.getGender()+k);
}catch(W){S.setHtml(S.getHtml()+H+r+R.getGender()+k);
}var T=new qx.ui.basic.Label();
T.setRich(true);
T.setWidth(250);
T.setContent(x+D+s+n+u);
this.getRoot().add(T,{left:280,top:10});
}}});
})();
...
(function(){var u="Validation Error",t=" is not a color! ",s="]!",r=" is not an url!",q="number",p="Validation Error: ",o="qx.util.Validate",n=" is not in ",m=" is not a String!",l="!",g=" is not a Number!",k=" is not in the range from [",j=" is not a E-Mail address!",f=".",d=" does not fit ",i=", ",h="string";
qx.Class.define(o,{statics:{number:function(){return qx.util.Validate.checkNumber;
},checkNumber:function(B){if((typeof B!==q&&(!(B instanceof Number)))||(!(isFinite(B)))){throw new qx.core.ValidationError(p,B+g);
}},email:function(){return qx.util.Validate.checkEmail;
},checkEmail:function(x){var y=/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;

if(y.test(x)===false){throw new qx.core.ValidationError(u,x+j);
}},string:function(){return qx.util.Validate.checkString;
},checkString:function(D){if(typeof D!==h&&(!(D instanceof String))){throw new qx.core.ValidationError(u,D+m);
}},url:function(){return qx.util.Validate.checkUrl;
},checkUrl:function(v){var w=/([A-Za-z0-9])+:\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;

if(w.test(v)===false){throw new qx.core.ValidationError(u,v+r);
}},color:function(){return qx.util.Validate.checkColor;
},checkColor:function(b){try{qx.util.ColorUtil.stringToRgb(b);
}catch(e){throw new qx.core.ValidationError(u,b+t+e);
}},range:function(E,F){return function(a){if(a<E||a>F){throw new qx.core.ValidationError(u,a+k+E+i+F+s);
}};
},inArray:function(C){return function(c){if(C.indexOf(c)===-1){throw new qx.core.ValidationError(u,c+n+C+l);
}};
},regExp:function(z){return function(A){if(z.test(A)===false){throw new qx.core.ValidationError(u,A+d+z+f);
}};
}}});
})();
