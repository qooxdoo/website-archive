...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.table.Table_Filtered_Model", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.table.Table_Filtered_Model-qx.theme.Modern-0.js"]],
  boot : "boot"
};  
...
(function(){var r="execute",q="A boolean",p="icon/22/actions/list-remove.png",o="A number",n="<b>HTML-Test <i>",m="icon/22/actions/edit-undo.png",l="between",k=", additional Status. All Values are shown.",j="Hide 1k-5k",i="Table with data filtering",F=", additional Status. 1k - 5k Values are hidden.",E="Show All",D=", additional Status. False Values are hidden.",C="Hide True",B="demobrowser.demo.table.Table_Filtered_Model",A=", additional Status. True Values are hidden.",z="!=",y="==",x="Hide False",w="ID",u="A date",v="HTML",s="</i></b>",t=", additional Status.";
qx.Class.define(B,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return i;
},createTable:function(){var c=this._tableModel=new qx.ui.table.model.Filtered();
c.setColumns([w,o,u,q,v]);
var d=[];
...
f.setAdditionalStatusBarText(t);
return f;
},createControls:function(){var H=new qx.ui.toolbar.ToolBar();
var J,N,G;
N=new qx.ui.toolbar.Part();
H.add(N);
var P=this._table;
var L=this._tableModel;
var M=new qx.ui.toolbar.Button(C,p);
N.add(M);
M.addListener(r,function(e){L.addNumericFilter(y,1,q);
L.applyFilters();
P.setAdditionalStatusBarText(A);
});
var I=new qx.ui.toolbar.Button(x,p);
N.add(I);
I.addListener(r,function(e){L.addNumericFilter(z,1,q);
L.applyFilters();
P.setAdditionalStatusBarText(D);
});
var K=new qx.ui.toolbar.Button(j,p);
N.add(K);
K.addListener(r,function(e){L.addBetweenFilter(l,1000,5000,o);
L.applyFilters();
P.setAdditionalStatusBarText(F);
});
var O=new qx.ui.toolbar.Button(E,m);
N.add(O);
O.addListener(r,function(e){L.resetHiddenRows();
P.setAdditionalStatusBarText(k);
});
return H;
}}});
})();
...
(function(){var L="==",K=">",J="between",I="<",H='g',G="regex",F="!between",E=">=",D="!=",C="<=",y="qx.ui.table.model.Filtered",B="Filters",A='',x="betweenAllowed",w="__vT",z="numericAllowed";
qx.Class.define(y,{extend:qx.ui.table.model.Simple,construct:function(){arguments.callee.base.call(this);
this.numericAllowed=new Array(L,D,K,I,E,C);
this.betweenAllowed=new Array(J,F);
this.__vS=false;
this.Filters=new Array();
},members:{__vT:null,__vS:null,_js_in_array:function(P,Q){var R=Q.toString();

if(R==A){return false;
}var T=new RegExp(P,H);
var S=T.test(Q);
return S;
...
}},addNumericFilter:function(h,j,k){var l=null;

if(this._js_in_array(h,this.numericAllowed)&&k!=null){if(j!=null){l=[h,j,k];
}}
if(l!=null){this.Filters.push(l);
}else{throw new Error("Filter not recognized: value or target is null!");
}},addRegex:function(M,N){if(M!=null&&N!=null){var O=new Array(G,M,N);
}
if(O!=null){this.Filters.push(O);
...
switch(this.Filters[i][0]){case L:if(a==this.Filters[i][1]){e=true;
}break;
case D:if(a!=this.Filters[i][1]){e=true;
}break;
case K:if(a>this.Filters[i][1]){e=true;
}break;
case I:if(a<this.Filters[i][1]){e=true;
}break;
case E:if(a>=this.Filters[i][1]){e=true;
}break;
case C:if(a<=this.Filters[i][1]){e=true;
}break;
}}else if(this._js_in_array(this.Filters[i][0],this.betweenAllowed)&&e==false){a=this.getValueById(this.Filters[i][3],g);

switch(this.Filters[i][0]){case J:if(a>=this.Filters[i][1]&&a<=this.Filters[i][2]){e=true;
}break;
case F:if(a<this.Filters[i][1]&&a>this.Filters[i][2]){e=true;
}break;
}}else if(this.Filters[i][0]==G&&e==false){a=this.getValueById(this.Filters[i][2],g);
var c=new RegExp(this.Filters[i][1],H);
e=c.test(a);
...
this.fireDataEvent(qx.ui.table.ITableModel.EVENT_TYPE_DATA_CHANGED,f);
},hideRows:function(m,n,dispatchEvent){var p=this.getData();
dispatchEvent=(dispatchEvent!=null?dispatchEvent:true);

if(!this.__vS){this.__vT=p.slice();
this.__vS=true;
}
if(n==null||n<1){n=1;
}
for(var o=m;o<(p.length-n);o++){p[o]=p[o+n];
}this.removeRows(o,n);
if(dispatchEvent){var q={firstRow:0,lastRow:p.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(qx.ui.table.ITableModel.EVENT_TYPE_DATA_CHANGED,q);
}},resetHiddenRows:function(){if(!this.__vT){return ;
}this.Filters=[];
this.setData(qx.lang.Array.clone(this.__vT));
},setData:function(U,V){this.__vT=qx.lang.Array.clone(U);
this.Filters=[];
arguments.callee.base.call(this,U,V);
}},destruct:function(){this._disposeFields(w,z,x,B);
}});
})();
...
(function(){var R="",Q="==",P=">",O="between",N="<",M="regex",L="!between",K=">=",J="!=",I="<=",C="font-weight",H=";",F="text-align",B='g',A=":",E="qx.ui.table.cellrenderer.Conditional",D="color",G="font-style";
qx.Class.define(E,{extend:qx.ui.table.cellrenderer.Default,construct:function(bc,bd,be,bf){arguments.callee.base.call(this);
this.numericAllowed=[Q,J,P,N,K,I];
this.betweenAllowed=[O,L];
this.conditions=[];
this.__vJ=bc||R;
this.__vK=bd||R;
this.__vL=be||R;
this.__vM=bf||R;
},members:{__vJ:null,__vK:null,__vL:null,__vM:null,__vN:function(j,k){if(j[1]!=null){k[F]=j[1];
}
if(j[2]!=null){k[D]=j[2];
}
if(j[3]!=null){k[G]=j[3];
...
}},addBetweenCondition:function(S,T,U,V,W,X,Y,ba){if(qx.lang.Array.contains(this.betweenAllowed,S)){if(T!=null&&U!=null){var bb=[S,V,W,X,Y,T,U,ba];
}}
if(bb!=null){this.conditions.push(bb);
}else{throw new Error("Condition not recognized or value1/value2 is null!");
}},addRegex:function(l,m,n,o,p,q){if(l!=null){var r=[M,m,n,o,p,l,q];
}
if(r!=null){this.conditions.push(r);
}else{throw new Error("regex cannot be null!");
}},_getCellStyle:function(s){if(!this.conditions.length){return s.style||R;
}var x=s.table.getTableModel();
var i;
var z;
var t;
var v={"text-align":this.__vJ,"color":this.__vK,"font-style":this.__vL,"font-weight":this.__vM};

for(i in this.conditions){z=false;
...
switch(this.conditions[i][0]){case Q:if(t==this.conditions[i][5]){z=true;
}break;
case J:if(t!=this.conditions[i][5]){z=true;
}break;
case P:if(t>this.conditions[i][5]){z=true;
}break;
case N:if(t<this.conditions[i][5]){z=true;
}break;
case K:if(t>=this.conditions[i][5]){z=true;
}break;
case I:if(t<=this.conditions[i][5]){z=true;
}break;
...
switch(this.conditions[i][0]){case O:if(t>=this.conditions[i][5]&&t<=this.conditions[i][6]){z=true;
}break;
case L:if(t<this.conditions[i][5]&&t>this.conditions[i][6]){z=true;
}break;
}}else if(this.conditions[i][0]==M){if(this.conditions[i][6]==null){t=s.value;
}else{t=x.getValueById(this.conditions[i][6],s.row);
}var u=new RegExp(this.conditions[i][5],B);
z=u.test(t);
}if(z==true){this.__vN(this.conditions[i],v);
}}var y=[];

for(var w in v){if(v[w]){y.push(w,A,v[w],H);
}}return y.join(R);
}}});
})();
(function(){var d="qooxdoo-table-cell",c="qx.ui.table.cellrenderer.Html",b="";
qx.Class.define(c,{extend:qx.ui.table.cellrenderer.Conditional,members:{_getContentHtml:function(e){return (e.value||b);
},_getCellClass:function(a){return d;
}}});
})();
