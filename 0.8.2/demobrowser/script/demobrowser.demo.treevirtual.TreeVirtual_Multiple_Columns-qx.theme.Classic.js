...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.treevirtual.TreeVirtual_Multiple_Columns", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.treevirtual.TreeVirtual_Multiple_Columns-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
(function(){var l="changeChecked",k="2007-01-30 22:54:03",j="Trash",h="-rwxr-xr-x",g="Permissions",f="Documents (D:)",d="Windows (C:)",c="Inbox",b="Desktop",a="execute",O="color: white;",N="font-weight: bold;",M="Disable the tree?",L="1*",K="Network",J="Control",I="Last Accessed",H="changeSelection",G="Sent",F="Files",t="demobrowser.demo.treevirtual.TreeVirtual_Multiple_Columns",u="background-color: red; ",r="Workspace",s="Open/close click selects row?",p="Remove",q="treeOpenWhileEmpty",m="Tree",n="Spam",v="background-color: cyan;",w="Data",z="Always show open/close symbol?",y="Current Selection: ",B='/',A="Remove open/close if found empty?",D="Exclude first-level tree lines?",C="Spam Message #",x="Use tree lines if theme supports them?",E="Edit";
...
ba.add(U);
var Y=U.getDataModel();
var W=Y.addBranch(null,b,true);
U.nodeSetLabelStyle(W,u+O+N);
var S;
Y.addBranch(W,F,true);
S=Y.addBranch(W,r,true);
te=Y.addLeaf(S,d);
Y.setColumnData(te,1,h);
Y.setColumnData(te,2,k);
te=Y.addLeaf(S,f);
Y.setColumnData(te,1,h);
Y.setColumnData(te,2,k);
Y.addBranch(W,K,true);
te=Y.addBranch(W,j,true);
U.nodeSetCellStyle(te,v);
var X=Y.addBranch(null,c,true);
te=Y.addBranch(X,n,false);

for(var i=1;i<3000;i++){Y.addLeaf(te,C+i);
}Y.addBranch(X,G,false);
Y.addBranch(X,j,false);
Y.addBranch(X,w,false);
Y.addBranch(X,E,false);
Y.setData();
...
R.addListener(a,function(e){selectedNodes=U.getSelectedNodes();

for(var i=0;i<selectedNodes.length;i++){Y.prune(selectedNodes[i].nodeId,true);
Y.setData();
}});
...
U.addListener(q,function(e){if(this.getChecked()){var P=e.getData();
