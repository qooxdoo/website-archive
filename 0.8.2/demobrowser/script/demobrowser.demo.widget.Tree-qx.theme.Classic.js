...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.widget.Tree", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["../../script/demobrowser.demo.widget.Tree-qx.theme.Classic-0.js"]],
  boot : "boot"
};  
...
(function(){var n="changeChecked",m="execute",l="multi",k="Trash",j="single",h="Windows (C:)",g="Documents (D:)",f="Inbox",d="changeValue",c="Desktop",P="root",O="Open mode:",N="Presets",M="Hide Root Node",L="Network",K="Control",J=" items",I="changeSelection",H="Sent",G="Files",u="",v="Workspace",s="Enable drag selection",t="Selection: ",q="Junk #",r="demobrowser.demo.widget.Tree",o="Data",p="click",w="Show root open button",x="none",A="Enable multi selection",z="double click",C="Selection:",B="Show Selected Items",E="Root node:",D="Toggle Height",y="Edit",F="Show Items";
...
bA.set({spacing:20});
var bz=new qx.ui.container.Composite(bA);
this.getRoot().add(bz);
var bx=this.getTree();
bz.add(bx);
var by=this.getCommandFrame(bx);
bz.add(by);
},getTree:function(){var br=new qx.ui.tree.Tree().set({width:200,height:400});
var bw=new qx.ui.tree.TreeFolder(P);
bw.setOpen(true);
br.setRoot(bw);
var bu=new qx.ui.tree.TreeFolder(c);
bu.setOpen(true);
bw.add(bu);
var bm=new qx.ui.tree.TreeFolder(G);
var bk=new qx.ui.tree.TreeFolder(v);
var bl=new qx.ui.tree.TreeFolder(L);
var bj=new qx.ui.tree.TreeFolder(k);
bu.add(bm,bk,bl,bj);
var bs=new qx.ui.tree.TreeFile(h);
var bi=new qx.ui.tree.TreeFile(g);
bk.add(bs,bi);
var bv=new qx.ui.tree.TreeFolder(f);
var bn=new qx.ui.tree.TreeFolder(N);
var bq=new qx.ui.tree.TreeFolder(H);
var bp=new qx.ui.tree.TreeFolder(k);

for(var i=0;i<30;i++){bp.add(new qx.ui.tree.TreeFile(q+i));
}var bt=new qx.ui.tree.TreeFolder(o);
var bo=new qx.ui.tree.TreeFolder(y);
bv.add(bn,bq,bp,bt,bo);
bw.add(bv);
return br;
},getCommandFrame:function(R){var ba=new qx.ui.groupbox.GroupBox(K);
var Y=4;
ba.setLayout(new qx.ui.layout.Grid(5,3));
var bh=0;
ba.add(new qx.ui.basic.Label(t).set({paddingTop:4}),{row:bh,column:0});
var be=new qx.ui.form.TextField().set({readOnly:true});
ba.add(be,{row:bh++,column:1});
R.addListener(I,function(e){var Q=e.getData();

if(Q.length>0){if(this.getSelectionMode()===l){be.setValue(Q.length+J);
}else{be.setValue(Q[0].getLabel());
}}else{be.setValue(u);
}});
ba.add(new qx.ui.core.Spacer(Y,Y),{row:bh++,column:0});
ba.add(new qx.ui.basic.Label(O),{row:bh,column:0});
var U={"click":p,"dblclick":z,"none":x};
var S=new qx.ui.form.RadioGroup();

for(var bc in U){var T=new qx.ui.form.RadioButton(U[bc]).set({value:bc,checked:bc==R.getOpenMode()});
S.add(T);
ba.add(T,{row:bh++,column:1});
}S.addListener(d,function(e){R.setOpenMode(e.getData());
});
ba.add(new qx.ui.core.Spacer(Y,Y),{row:bh++,column:0});
ba.add(new qx.ui.basic.Label(C),{row:bh,column:0});
var W=new qx.ui.form.CheckBox(A);
W.setChecked(R.getSelectionMode()==l);
ba.add(W,{row:bh++,column:1});
W.addListener(n,function(e){var b=e.getData();
R.setSelectionMode(b?l:j);

if(!b){X.setChecked(false);
}});
var X=new qx.ui.form.CheckBox(s);
X.setChecked(R.getDragSelection());
ba.add(X,{row:bh++,column:1});
X.addListener(n,function(e){var a=e.getData();
R.setDragSelection(a);

if(a){W.setChecked(true);
}});
ba.add(new qx.ui.core.Spacer(Y,Y),{row:bh++,column:0});
ba.add(new qx.ui.basic.Label(E),{row:bh,column:0});
var bf=new qx.ui.form.CheckBox(M);
bf.setChecked(R.getHideRoot());
ba.add(bf,{row:bh++,column:1});
bf.addListener(n,function(e){R.setHideRoot(e.getData());
});
var V=new qx.ui.form.CheckBox(w);
V.setChecked(R.getRootOpenClose());
ba.add(V,{row:bh++,column:1});
V.addListener(n,function(e){R.setRootOpenClose(e.getData());
});
ba.add(new qx.ui.core.Spacer(Y,Y),{row:bh++,column:0});
var bd=new qx.ui.form.Button(F);
ba.add(bd,{row:bh++,column:1});
bd.addListener(m,function(e){alert((""+R.getItems()).replace(",","\n","g"));
});
var bb=new qx.ui.form.Button(B);
ba.add(bb,{row:bh++,column:1});
bb.addListener(m,function(e){if(this.getSelectionMode()===j){alert(this.getSelection());
}else{alert(this.getSelection().toString().replace(",","\n","g"));
}},R);
var bb=new qx.ui.form.Button(D);
ba.add(bb,{row:bh++,column:1});
var bg=true;
bb.addListener(m,function(e){R.setHeight(bg?600:400);
bg=!bg;
});
return ba;
}}});
