...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:fce.9b48251e6351.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="Feature Configuration Editor",k="\.",j="fce.Application",h="^",g="qx.debug",f="__LM",e="changeFeatures",d="app-header",c="icon/16/actions/help-contents.png",b="Help",a="execute";
qx.Class.define(j,{extend:qx.application.Standalone,members:{__LM:null,__LN:null,__LO:null,main:function(){qx.application.Standalone.prototype.main.call(this);
if(qx.core.Environment.get(g)){qx.log.appender.Native;
qx.log.appender.Console;
}this.__LN=qx.core.Environment.get("qx.application").split(".")[0];
...
o.add(n);
this.__LM=new fce.view.FeatureSelector();
this.__LM.setMargin(20);
o.add(this.__LM,{flex:1});
...
this.__LM.setFeatureData(s);
},this);
p.check();
},_stripOwnSettings:function(v){var x=new RegExp(h+this.__LN+k);
var w=[];
...
z.add(A);
return z;
},__LP:function(){var F=qx.core.Environment.get("fce.reportServerHost");
...
},_getHelpWindow:function(){if(!this.__LO){this.__LO=new fce.view.HelpWindow();
this.__LO.center();
}return this.__LO;
}},destruct:function(){this._disposeObjects(f);
}});
...
(function(){var k="list",j="table",h="execute",g="bigger",f="set",e="changeValue",d="filter",c="",b="change",a="name",Q="jsonField",P="changeBubble",O="icon/16/actions/go-next.png",N="changeFeatureMap",M="__LR",L="sourceProperty",K="Selected Features",J="fce.view.FeatureSelector",I="bottom",H="_applyModel",s="changeFilter",t="null",q="interval",r="Available Features",o="icon/16/actions/edit-find.png",p="Filter by feature name (RegEx)",m="middle",n="distinctValues",u="icon/16/actions/go-previous.png",v="icon/16/actions/window-new.png",z="_",y="Import Feature Map",B="textfield",A="__LS",D="__LT",C="JSON",x="cellDblclick",G="widget",F="featureselector",E="modelValueProperty",w="_applyFeatureData";
qx.Class.define(J,{extend:qx.ui.container.Composite,construct:function(){var R=new qx.ui.layout.HBox(20);
...
this.__LQ={};
},properties:{featureData:{init:{},nullable:true,apply:w},model:{apply:H},filter:{init:c,event:s}},statics:{sanitizeId:function(S){if(/^[$A-Za-z_][0-9A-Za-z_]*$/.test(S)){return S;
}S=S.replace(/[^0-9a-z_]/gi,c);

if(S.length==0){S=z+new Date().getTime();
}return S;
}},members:{__LR:null,__LS:null,__LT:null,__LU:null,__LQ:null,__LV:null,_createChildControlImpl:function(T,U){var V;

switch(T){case j:V=new fce.view.Table();
V.setMinWidth(330);
this.bind(d,V,d);
V.addListener(x,this.__Ma,this);
qx.data.SingleValueBinding.bind(V,L,this.getChildControl(k),E);
break;
case k:V=new fce.view.List();
V.setMinWidth(330);
V.setMinHeight(75);
V.getSelectedItems().addListener(b,this.__JU,this);
break;
case Q:V=new qx.ui.form.TextArea();
V.setMinWidth(330);
V.setMinHeight(75);
V.setReadOnly(true);
}return V;
...
Y.add(ba);
var bb=new qx.ui.basic.Label(r);
bb.setFont(g);
ba.add(bb);
bb.setAlignY(I);
...
bl.setAppearance(B);
var bj=this.__LR=new qx.ui.form.TextField();
bj.setLiveUpdate(true);
bj.setAppearance(G);
bj.setPlaceholder(p);
bl.add(bj,{flex:1});
var bm=new qx.ui.basic.Image(o);
bl.add(bm);
var bk=new qx.event.Timer(500);
bk.addListener(q,function(bn){this.setFilter(this.__LR.getValue());
bk.stop();
},this);
bj.addListener(e,function(bo){bk.restart();
},this);
return bl;
},_getImportWindow:function(){if(!this.__LS){this.__LS=new fce.view.ImportWindow();
this.__LS.center();
this.__LS.addListener(N,this.__LW,this);
}return this.__LS;
},__LW:function(bp){var bq=bp.getData();
...
},_saveModifiedData:function(){this.__LV=[];
var bC=this.getChildControl(k);
var bD=bC.getChildren();

for(var i=0,l=bD.length;i<l;i++){var bG=bD[i];
var bF=bG.getModelItem();
var bH=bF.getName();
var bJ=bG.getValueProperty();
var bE=bF.get(bJ);
var bI={key:bH,value:bE,valueProp:bJ};
this.__LV.push(bI);
}},_restoreModifiedData:function(){for(var i=0,l=this.__LV.length;i<l;i++){var bK=this.__LV[i];
var bL=this.__LX(a,bK.key);

if(bL){bL.set(bK.valueProp,bK.value);
this.getChildControl(k).addItemsUnique(new qx.data.Array([bL]));
}}},__LX:function(bM,bN){var bP=this.getModel();

for(var i=0,l=bP.length;i<l;i++){var bO=bP.getItem(i);

if(bO.get(bM)===bN){return bO;
}}return null;
},addFeatureSet:function(bQ){var bR=this.getFeatureData();

for(var bS in bQ){var bT=fce.view.FeatureSelector.sanitizeId(bS);
bR[bT]=bQ[bS];
}this.setFeatureData(null);
this.setFeatureData(bR);
},_applyFeatureData:function(bU,bV){if(bU){var bX=this._getData(bU);
this.__LY(bX);
var bW=qx.data.marshal.Json.createModel(bX,true);
this.setModel(bW);
}},_applyModel:function(bY,ca){if(ca){ca.removeListener(P,this.__JU,this);
}bY.addListener(P,this.__JU,this);
this.getChildControl(j).setModel(bY);
},__LY:function(cb){for(var i=0,l=cb.length;i<l;i++){var cc=cb[i];

for(var ce in cc){var cd=typeof cc[ce];

if(!(cd=="boolean"||cd=="number"||cd=="string")){cc[ce]=qx.lang.Json.stringify(cc[ce]);
}}}},__JU:function(){var cf=this.getChildControl("list").getModelValueProperty();
var ch=this.__Mb(this.getChildControl("list").getSelectedItems(),cf);
var cg=this._getJson(ch);
this.getChildControl("jsonField").setValue(cg);
},__Ma:function(ci){var cj=this.getChildControl(j);
var cn=cj.getTableModel();
var ck=ci.getRow();
var cl=cn.getColumnCount()-1;
var cm=cn.getValue(cl,ck);
this.getChildControl(k).addItemsUnique(new qx.data.Array([cm]));
},__Mb:function(co,cp){var cr={};

for(var i=0,l=co.length;i<l;i++){var cq=co.getItem(i);

if(!qx.Class.hasProperty(cq.constructor,cp)){for(var cs in qx.util.PropertyUtil.getAllProperties(cq.constructor)){if(cs!==a&&cs!==n){cp=cs;
break;
}}}cr[cq.getName()]=cq.get(cp);
}return cr;
},_getJson:function(ct){for(var cu in ct){if(ct[cu]===t||/^(?:\{|\[)".*(?:\}|\])$/.test(ct[cu])){ct[cu]=qx.lang.Json.parse(ct[cu]);
}}return qx.lang.Json.stringify(ct,null,2);
},_getSetsMenu:function(cv){if(!this.__LU){this.__LU=[];
}
for(var i=0,l=cv.length;i<l;i++){if(!qx.lang.Array.contains(this.__LU,cv[i])){var cw=new qx.ui.menu.CheckBox(cv[i]);
cw.setValue(true);
cw.setUserData(f,cv[i]);
cw.addListener(e,function(cx){var cy=cx.getTarget().getUserData(f);
var cz=cx.getData();

if(cz){this._unstashSet(cy);
}else{this._stashSet(cy);
}},this);
this.__LT.add(cw);
this.__LU.push(cv[i]);
}}},_stashSet:function(cA){var cB=this.getFeatureData();

if(cB[cA]){this.__LQ[cA]=cB[cA];
delete cB[cA];
this.setFeatureData(null);
this.setFeatureData(cB);
}},_unstashSet:function(cC){var cD=this.getFeatureData();

if(this.__LQ[cC]){cD[cC]=this.__LQ[cC];
delete this.__LQ[cC];
this.setFeatureData(null);
this.setFeatureData(cD);
}},__Mc:function(){var cE=this.getChildControl(j).getSelectedItems();
this.getChildControl(k).addItemsUnique(cE);
},__Md:function(){this.getChildControl(k).removeSelected();
}},destruct:function(){this.getChildControl(k).getSelectedItems().removeListener(b,this.__JU,this);
this._disposeObjects(M,A,D);
}});
})();
...
B.addListener(w,this.__Mg,this);
this.__Me=[];
this._setUpDragDrop();
this.setDecorator(p);
...
},properties:{model:{apply:A},selectedItems:{init:new qx.data.Array(),event:a},sourceProperty:{init:null,nullable:true,check:u,event:q},filter:{apply:o}},members:{__Me:null,__Mf:function(F){var G=F.col;
...
},__Mg:function(){var K=this.getTableModel().getColumnCount()-1;
...
this.__Me=this._getModelProperties(N);

if(this.__Me.indexOf(m)>=0){this.__Me.push(qx.lang.Array.removeAt(this.__Me,this.__Me.indexOf(m)));
}this.__Me.push(k);
P.setColumns(this.__Me);
...
for(var x=0,y=this.__Me.length;x<y;x++){var W=this.__Me[x];
...
var bc=new qx.ui.table.cellrenderer.Dynamic(this.__Mf);
...
(function(){var j="keypress",h="change",g="items",f="fce.view.List",e="multi",d="dragover",c="changeSelectedItems",b="Delete",a="drop";
qx.Class.define(f,{extend:qx.ui.form.List,construct:function(k){qx.ui.form.List.call(this,k);
this.set({enableInlineFind:true,selectionMode:e});
this._setUpDragDrop();
this.addListener(j,function(o){if(o.getKeyIdentifier()===b){this.removeSelected();
}},this);
var n=new qx.data.Array();
n.addListener(h,this.__JU,this);
this.setSelectedItems(n);
this.__Mh=[];
var m=this._getManager();
this.removeListener(j,m.handleKeyPress,m);
},properties:{selectedItems:{event:c},modelValueProperty:{init:null,nullable:true}},members:{__Mh:null,_setUpDragDrop:function(){this.setDroppable(true);
this.addListener(a,function(p){var q=p.getData(g);
this.addItemsUnique(q);
},this);
this.addListener(d,function(r){if(!r.supportsType(g)){r.preventDefault();
}});
},__JU:function(){if(!this.getModelValueProperty()){return;
}var s=this.getSelectedItems();

for(var i=0,l=s.length;i<l;i++){var t=s.getItem(i);

if(!this.isItemListed(t)){var u=new fce.view.ListItem(t,this.getModelValueProperty(),"name");
this.add(u);
this.__Mh.push(t);
}}},isItemListed:function(v){if(qx.lang.Array.contains(this.__Mh,v)){return true;
}var w=v.getName();

for(var i=0,l=this.__Mh.length;i<l;i++){if(this.__Mh[i].getName()===w){return true;
}}return false;
},removeSelected:function(){var y=this.getSelection();

for(var i=0,l=y.length;i<l;i++){var x=y[i].getModelItem();
this.getSelectedItems().remove(x);
y[i].destroy();
qx.lang.Array.remove(this.__Mh,x);
}},removeAll:function(){var A=this.getChildren().concat();

for(var i=0,l=A.length;i<l;i++){var z=A[i].getModelItem();
this.getSelectedItems().remove(z);
A[i].destroy();
qx.lang.Array.remove(this.__Mh,z);
}},addItemsUnique:function(B){for(var i=0,l=B.length;i<l;i++){var C=this.getSelectedItems();

if(!C.contains(B.getItem(i))){C.push(B.getItem(i));
}}}},destruct:function(){this.getSelectedItems().removeListener(h,this.__JU,this);
this.__Mh=null;
}});
})();
...
this.add(o);
var s=this.__Mi=new qx.ui.form.TextField();
this.add(s);
o.setBuddy(s);
var q=new qx.ui.basic.Label(b);
this.add(q);
var p=this.__Mj=new qx.ui.form.TextArea();
this.add(p,{flex:1});
q.setBuddy(p);
...
},members:{__Mi:null,__Mj:null,__Mk:null,reset:function(){this.__Mi.setValue(n);
this.__Mj.setValue(n);
this.__Mk.setValue(n);
this.__Mk.exclude();
},_onImport:function(){var name=this.__Mi.getValue();
var v=this.__Mj.getValue();
...
},_showError:function(A){this.__Mk.setValue(A);
this.__Mk.setVisibility(l);
}}});
})();
...
qx.Class.define(b,{extend:qx.core.Object,properties:{features:{nullable:true,init:null,event:a}},members:{__gk:null,check:function(){var g=qx.core.Environment.getChecks();
...
}},properties:{server:{init:null,nullable:true},addUrl:{init:null,nullable:true},getUrl:{init:null,nullable:true}},members:{__Ml:null,_sendReport:function(o){if(this.getServer()===null||this.getAddUrl()===null){throw new Error("Report server host or URL not specified!");
}
...
q.send();
},compare:function(w){this.__Ml=w;

if(this.getGetUrl()){this.getFeaturesFromServer();
}else{this._sendReport(this.__Ml);
...
y.send();
},_compareFeatureSets:function(E){if(!E[g]||!this.__Ml[g]){return;
}var H=[];

for(var I in this.__Ml){if(this.isIgnored(I)){continue;
}
if(!E.hasOwnProperty(I)){H.push(I);
continue;
}var G=typeof E[I];
var L=typeof this.__Ml[I];
...
this.error(I,"expected",E[I],"found",this.__Ml[I]);
