...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:feedreader.4caed7300b44.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
if(qx.core.Environment.get(a)==false){this.__dB.setShowScrollbarY(false);
...
},__eU:function(I,J,K){if(!K){K=this.__eT(I);
}var J=J||I.type;

if(J==r){this.__eV(I,K);
}else if(J==q){this.__eW(I,K);
}else if(J==s){this.__eX(I,K);
}},__eV:function(L,M){var N=L.changedTouches[0];
this.__eN=N.screenX;
this.__eO=N.screenY;
this.__eP=new Date().getTime();
this.__eQ=L.changedTouches.length===1;
},__eW:function(O,P){if(this.__eQ&&O.changedTouches.length>1){this.__eQ=false;
}},__eX:function(Q,R){if(this.__eQ){var S=Q.changedTouches[0];
var U={x:S.screenX-this.__eN,y:S.screenY-this.__eO};
var V=qx.event.handler.Touch;

if(this.__eS==R&&Math.abs(U.x)<=V.TAP_MAX_DISTANCE&&Math.abs(U.y)<=V.TAP_MAX_DISTANCE){this.__dH(Q,f,R,qx.event.type.Tap);
}else{var T=this.__eY(Q,R,U);

if(T){Q.swipe=T;
this.__dH(Q,g,R,qx.event.type.Swipe);
}}}},__eY:function(W,X,Y){var bd=qx.event.handler.Touch;
var be=new Date().getTime()-this.__eP;
...
if(Math.abs(bf)>=bd.SWIPE_MIN_VELOCITY&&Math.abs(ba)>=bd.SWIPE_MIN_DISTANCE){bc={startTime:this.__eP,duration:be,axis:bg,direction:bb,distance:ba,velocity:bf};
}return bc;
},__fa:function(bh){var bi=bh.type;
var bk=qx.event.handler.Touch.MOUSE_TO_TOUCH_MAPPING;

if(bk[bi]){bi=bk[bi];
if(bi==r&&this.__fb(bh)){this.__eR=true;
}else if(bi==s){this.__eR=false;
}var bl=this.__fc(bh);
var bj=(bi==s?[]:[bl]);
bh.touches=bj;
bh.targetTouches=bj;
...
},__fb:function(bm){if((qx.core.Environment.get(n)==j)){var bn=1;
}else{var bn=0;
}return bm.button==bn;
},__fc:function(bo){var bp=this.__eT(bo);
...
},_initMouseObserver:function(){if(!qx.core.Environment.get(p)){this.__eM=qx.lang.Function.listener(this._onMouseEvent,this);
var Event=qx.bom.Event;
...
}),_onMouseEvent:qx.event.GlobalError.observeMethod(function(br){if(!qx.core.Environment.get(p)){if(br.type==o&&!this.__eR){return;
}var bs=this.__fa(br);
this._commonTouchEventHandler(br,bs);
}}),_commonTouchEventHandler:function(bt,bu){var bu=bu||bt.type;

if(bu==r){this.__eS=this.__eT(bt);
}this.__dH(bt,bu);
this.__eU(bt,bu);
...
qx.Class.define(f,{extend:qx.application.Mobile,members:{__pE:null,main:function(){qx.application.Mobile.prototype.main.call(this);
var j=new feedreader.model.Model();
var k=feedreader.io.FeedLoader.getInstance();
this.__pE=j.getFeedFolder();
k.loadAll(this.__pE);
this.buildUpGui();
...
(function(){var j="static",i="user",h="Surfin' Safari",g="Heise",f="http://blogs.msdn.com/ie/rss.xml",e="changeStaticFeedFolder",d="http://blogs.msdn.com/jscript/rss.xml",c="http://functionsource.com/feeds/entries",b="The Mozilla Blog",a="http://webkit.org/blog/feed/",A="http://feeds2.feedburner.com/qooxdoo/news/content",z="http://blog.mozilla.com/feed/",y="changeFeedFolder",x="FunctionSource",w="http://www.heise.de/newsticker/heise-atom.xml",v="http://my.opera.com/desktopteam/xml/rss/blog/",u="JScript Team Blog",t="Feeds",s="feedreader.model.Model",r="IEBlog",p="User Feeds",q="http://daringfireball.net/index.xml",n="Static Feeds",o="qooxdoo News",l="Opera Desktop Blog",m="Daring Fireball",k="changeUserFeedFolder";
qx.Class.define(s,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this._initializeModel();
},properties:{feedFolder:{init:null,event:y,nullable:true},staticFeedFolder:{init:null,event:e,nullable:true},userFeedFolder:{init:null,event:k,nullable:true}},members:{_initializeModel:function(){var B=new feedreader.model.FeedFolder(t);
var C=new feedreader.model.FeedFolder(qx.locale.Manager.tr(n));
B.getFeeds().push(C);
C.getFeeds().push(new feedreader.model.Feed(o,A,j));
C.getFeeds().push(new feedreader.model.Feed(u,d,j));
C.getFeeds().push(new feedreader.model.Feed(m,q,j));
C.getFeeds().push(new feedreader.model.Feed(h,a,j));
C.getFeeds().push(new feedreader.model.Feed(x,c,j));
var D=new feedreader.model.FeedFolder(qx.locale.Manager.tr(p));
B.getFeeds().push(D);
D.getFeeds().push(new feedreader.model.Feed(g,w,i));
D.getFeeds().push(new feedreader.model.Feed(r,f,i));
D.getFeeds().push(new feedreader.model.Feed(b,z,i));
D.getFeeds().push(new feedreader.model.Feed(l,v,i));
...
(function(){var q="failed",p="html.storage.local",o="error",n="qx-feeds-",m="loaded",l="select * from feed where url='",k="'",j="io.ssl",h="timeout",g="feedreader.io.FeedLoader",c="loading",f="changeState",d="aborted",b="singleton",a="cached";
qx.Class.define(g,{extend:qx.core.Object,type:b,members:{loadAll:function(r){var t=r.getFeeds().getItem(0).getFeeds();
...
}},load:function(u){u.setState(c);
var v=l+u.getUrl()+k;
var w=new qx.data.store.Yql(v,{manipulateData:function(x){try{x=x.query.results.item||x.query.results.entry;
for(var i=0;i<x.length;i++){if(!qx.lang.Type.isString(x[i].title)){x[i].title=x[i].title.content;
}}if(qx.core.Environment.get(p)){var y=n+u.getUrl();
qx.bom.storage.Local.getInstance().setItem(y,x);
}return x;
}catch(e){return q;
}},configureRequest:function(z){z.setTimeout(10000);
}},qx.core.Environment.get(j));
w.addListener(m,this.__oM(u),this);
w.addListener(f,qx.lang.Function.bind(this.__oL,this,u),this);
},__oL:function(A,e){if(e.getData()==d||e.getData()==h||e.getData()==q){var D=o;
if(qx.core.Environment.get(p)){var E=n+A.getUrl();
var B=qx.bom.storage.Local.getInstance().getItem(E);

if(B){var C=qx.data.marshal.Json.createModel(B);
A.setArticles(C);
D=a;
}}A.setState(D);
}},__oM:function(F){return function(e){var G=e.getData();
if(G==q){F.setState(o);
return;
}F.setArticles(G);
F.setState(m);
...
this._setRoot(m);
this.__rX();
},events:{add:g,remove:g},members:{__rT:null,__rU:null,__dB:null,__rV:null,__rW:null,__rX:function(){if(qx.core.Environment.get(j)&&qx.core.Environment.get(l)==k){this.__rV=qx.lang.Function.bind(this._onBackButton,this);
...
},_setRoot:function(t){this.__dB=t;
...
},events:{"initialize":q,"start":q,"stop":q,"pause":q,"resume":q,"back":q,"menu":q},properties:{defaultCssClass:{refine:true,init:i}},statics:{__dy:null,getManager:function(){return qx.ui.mobile.page.Page.__dy;
},setManager:function(s){qx.ui.mobile.page.Page.__dy=s;
}},members:{__sm:false,_resize:function(){if(qx.core.Environment.get(k)){this._setStyle(a,window.innerHeight+o);
...
(function(){var o="resize",n="orientationchange",m="domupdated",l="qx.mobile.nativescroll",k='TEXTAREA',j="qx/mobile/js/iscroll.min.js",i='INPUT',h="iscroll",g="scrollbar",f='SELECT',b="div",d="qx.ui.mobile.container.MIScroll",c="success",a='iscrollstart';
qx.Mixin.define(d,{construct:function(){this.__sw();
this.__rX();
},members:{__sv:null,_createScrollElement:function(){var scroll=qx.bom.Element.create(b);
qx.bom.element.Class.add(scroll,h);
return scroll;
},_getScrollContentElement:function(){return this.getContainerElement().childNodes[0];
},__sw:function(){if(!window.iScroll){{var q=j;
};
var p=qx.util.ResourceManager.getInstance().toUri(q);
var r=new qx.io.ScriptLoader();
r.load(p,this.__sz,this);
}else{this._setScroll(this.__sx());
}},__sx:function(){var scroll=new iScroll(this.getContainerElement(),{hideScrollbar:true,fadeScrollbar:true,hScrollbar:false,scrollbarClass:g,onBeforeScrollStart:function(e){var t=e.target;

while(t.nodeType!=1){t=t.parentNode;
}
if(t.tagName!=f&&t.tagName!=i&&t.tagName!=k){e.preventDefault();
}if(qx.core.Environment.get(l)==false){var s=new qx.event.message.Message(a);
qx.event.message.Bus.getInstance().dispatch(s);
}}});
return scroll;
},__rX:function(){qx.event.Registration.addListener(window,n,this._refresh,this);
qx.event.Registration.addListener(window,o,this._refresh,this);
this.addListener(m,this._refresh,this);
},__sy:function(){qx.event.Registration.removeListener(window,n,this._refresh,this);
qx.event.Registration.removeListener(window,o,this._refresh,this);
this.removeListener(m,this._refresh,this);
...
this.setTitle(a);
},properties:{feeds:{event:d,init:null,apply:c},selectedFeed:{event:f,init:null,nullable:true}},members:{__rm:null,__sH:null,_initialize:function(){qx.ui.mobile.page.NavigationPage.prototype._initialize.call(this);
this.__rm=new qx.ui.mobile.list.List();
this.__rm.setDelegate({configureItem:function(h,i){h.setTitle(i.getTitle());
h.setShowArrow(true);
}});
this.__rm.addListener(b,function(e){var j=this.__sH.getItem(e.getData());
this.setSelectedFeed(j);
},this);
this.getContent().add(this.__rm);
},_applyFeeds:function(k,l){this.__sH=k.getFeeds().getItem(0).getFeeds();
this.__rm.setModel(this.__sH);
}}});
})();
...
},properties:{feed:{event:c,init:null,nullable:true,apply:g},selectedArticle:{event:a,init:null,nullable:true}},members:{__rm:null,__sQ:null,_initialize:function(){qx.ui.mobile.page.NavigationPage.prototype._initialize.call(this);
this.__rm=new qx.ui.mobile.list.List();
this.__rm.setDelegate({configureItem:function(h,i){h.setTitle(i.getTitle());
h.setShowArrow(true);
}});
this.__rm.addListener(b,function(e){var j=this.__sQ.getItem(e.getData());
this.setSelectedArticle(j);
},this);
this.getContent().add(this.__rm);
},_applyFeed:function(k,l){if(k!=null){this.__sQ=k.getArticles();
this.__rm.setModel(this.__sQ);
...
},properties:{article:{event:b,init:null,nullable:true,apply:c}},members:{__sR:null,_initialize:function(){qx.ui.mobile.page.NavigationPage.prototype._initialize.call(this);
this.__sR=new qx.ui.mobile.embed.Html();
this.__sR.addCssClass(d);
this.getContent().add(this.__sR);
