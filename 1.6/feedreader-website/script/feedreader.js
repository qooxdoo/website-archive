...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:feedreader.88747869a232.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var s="list",r="loaded",q="",p="stateModified",o="html",n="1.6",m="click",l="error",k="User Feeds",h="Sorry, unable to load the feed.",c="Static Feeds",g="qooxdoo ",f="selectedFeed",b="qxTag",a="feedreader.WebsiteApplication",d="tree";
...
qx.bom.Collection.id(b).setAttribute(o,g+n);
var u=new feedreader.model.Model();
var v=qx.bom.Collection.id(d);
var t=qx.bom.Collection.id(s);
this.fillTree(v,u);
var self=this;
qx.bom.Event.addNativeListener(v[0],m,function(e){var x=qx.bom.Event.getTarget(e).feed;
if(x.getState()==r){self.fillList(t,x);
}else if(x.getState()==l){qx.bom.Collection.id(s).setAttribute(o,h);
}else{x.addListener(p,function(e){if(e.getData()==r){self.fillList(t,x);
}});
}});
...
qx.bom.element.Animation.animate(y[0],A).onEnd(function(){y[0].innerHTML=q;
var C=z.getArticles();
...
qx.bom.element.Animation.animate(E[0],G).onEnd(function(){E[0].innerHTML=q;
...
}else{J.addListener(p,function(e){if(e.getData()==r){this.fillList(qx.bom.Collection.id(s),e.getTarget());
}},this);
}}E.append(K);
...
qx.Bootstrap.define(c,{statics:{__ti:null,__tj:f,__cq:0,__tk:{},__tl:{"scale":true,"rotate":true,"skew":true,"translate":true},__tm:qx.core.Environment.get(p),animate:function(w,x){this.__tp(x);

if(!this.__ti){this.__ti=qx.bom.Stylesheet.createElement();
}var B=x.keyFrames;
if(this.__tm!=null){var name=this.__tr(B,x.reverse);
var z=name+u+x.duration+o+x.repeat+u+x.timing+u+(x.alternate?s:v);
var y=this.__tm[k];
qx.bom.Event.addNativeListener(w,y,this.__tn);
w.style[this.__tm[t]]=z;
}var A=new qx.bom.element.AnimationHandle();
A.desc=x;
A.el=w;
w.$$animation=A;
if(x.origin!=null){qx.bom.element.Transform.setOrigin(w,x.origin);
}if(this.__tm==null){window.setTimeout(function(){qx.bom.element.Animation.__tn({target:w});
},0);
}return A;
},__tn:function(e){var C=e.target;
var E=C.$$animation;
if(!E){return;
}var F=E.desc;

if(qx.bom.element.Animation.__tm!=null){C.style[qx.bom.element.Animation.__tm[t]]=v;
qx.bom.Event.removeNativeListener(C,qx.bom.element.Animation.__tm[t],qx.bom.element.Animation.__tn);
}
if(F.origin!=null){qx.bom.element.Transform.setOrigin(C,v);
}
if(F.keep!=null){qx.bom.element.Animation.__to(C,F.keyFrames[F.keep]);
}C.$$animation=null;
E.el=null;
E.ended=true;
var D=E.getOnEnd();
...
}},__to:function(G,H){var J;

for(var I in H){if(I in qx.bom.element.Animation.__tl){if(!J){J={};
}J[I]=H[I];
}else{G.style[I]=H[I];
}}if(J){qx.bom.element.Transform.transform(G,J);
}},__tp:function(K){if(!K.hasOwnProperty(s)){K.alternate=false;
}
...
}},__tq:null,__tr:function(frames,L){var O=v;
for(var R in frames){O+=(L?-(R-100):R)+n;
var N=frames[R];
var Q;
for(var M in N){if(M in this.__tl){if(!Q){Q={};
}Q[M]=N[M];
}else{O+=M+a+N[M]+b;
}}if(Q){O+=qx.bom.element.Transform.getCss(Q);
}O+=g;
}if(this.__tk[O]){return this.__tk[O];
}var name=this.__tj+this.__cq++;
var P=this.__tm[j]+u+name;
qx.bom.Stylesheet.addRule(this.__ti,P,O);
this.__tk[O]=name;
return name;
}}});
})();
