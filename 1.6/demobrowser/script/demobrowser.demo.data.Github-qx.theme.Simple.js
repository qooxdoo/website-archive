...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Github-qx.theme.Simple.c38a46945429.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var p="qx.event.type.Rest",o="undefined",n="Success",m="Error",l="success",k="qx.debug",j="",i="loadEnd",h="qx.io.rest.Resource",g="interval",c="fail",f="get",d="{",b="=?(\\w+)?}",a="error";
...
try{if(typeof q!==o){if(qx.core.Environment.get(k)){qx.core.Assert.assertMap(q);
}this.__IT(q);
}}catch(e){this.dispose();
throw e;
}},events:{"success":p,"actionSuccess":p,"error":p,"actionError":p},statics:{POLL_THROTTLE_LIMIT:100,POLL_THROTTLE_COUNT:30,REQUIRED:true},members:{__IK:null,__uD:null,__IN:null,__IL:null,__IM:null,__IO:null,configureRequest:function(r){this.__IO=r;
},_getRequest:function(){return new qx.io.request.Xhr();
},__IP:function(s){var t=this._getRequest();

if(!qx.lang.Type.isArray(this.__IK[s])){this.__IK[s]=[];
}this.__IK[s].push(t);
return t;
},map:function(u,v,w,x){this.__uD[u]=[v,w,x];
this.__IK[u]=[];
if(u==f){this[u]=undefined;
}if(typeof this[u]!==o&&this[u].action!==true){if(this[u]!==qx.lang.Function.empty){throw new Error("Method with name of action ("+u+") already exists");
}}this.__IU(u+n);
this.__IU(u+m);
this[u]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,u);
return this.invoke.apply(this,arguments);
},this);
this[u].action=true;
},invoke:function(y,z){var C=this.__IP(y),F=this._getRequestConfig(y,z),z=z==null?{}:z,B=qx.lang.Object.clone(z);
this.__uD[y].params=z;
if(z){this.__IS(this.__uD[y][1]).forEach(function(G){delete B[G];
});
}this.__IQ(z,F.check);

if(this.__IO){this.__IO.call(this,C,y,z);
}this.__IR(C,B,y,F);
C.addListenerOnce(l,function E(){var H=[C.getResponse(),null,false,C,y,C.getPhase()];
this.fireEvent(y+n,qx.event.type.Rest,H);
this.fireEvent(l,qx.event.type.Rest,H);
},this);
C.addListenerOnce(c,function A(){var I=[C.getResponse(),null,false,C,y,C.getPhase()];
this.fireEvent(y+m,qx.event.type.Rest,I);
this.fireEvent(a,qx.event.type.Rest,I);
},this);
C.addListenerOnce(i,function D(){C.dispose();
},this);
C.send();
return parseInt(C.toHashCode(),10);
},setBaseUrl:function(J){this.__IN=J;
},__IQ:function(K,L){if(typeof L!=="undefined"){if(qx.core.Environment.get("qx.debug")){qx.core.Assert.assertObject(L,"Check must be object with params as keys");
}qx.lang.Object.getKeys(L).forEach(function(M){if(qx.core.Environment.get("qx.debug")){if(L[M]!==true){if(qx.core.Environment.get("qx.debug")){qx.core.Assert.assertRegExp(L[M]);
}}}if(L[M]===qx.io.rest.Resource.REQUIRED&&typeof K[M]==="undefined"){throw new Error("Missing parameter '"+M+"'");
}if(!(L[M]&&typeof L[M].test=="function")){return;
}if(!L[M].test(K[M])){throw new Error("Parameter '"+M+"' is invalid");
}});
}},__IR:function(N,O,P,Q){N.set({method:Q.method,url:Q.url});

if(O){var R=N.getRequestHeader("Content-Type");

if(qx.util.Request.methodAllowsRequestBody(N.getMethod())){if((/application\/.*\+?json/).test(R)){O=qx.lang.Json.stringify(O);
}}N.setRequestData(O);
}},abort:function(S){if(qx.lang.Type.isNumber(S)){var W=S;
var U=qx.core.ObjectRegistry.getPostId();
var V=qx.core.ObjectRegistry.fromHashCode(W+U);

if(V){V.abort();
}}else{var T=S;
var X=this.__IK[T];

if(this.__IK[T]){X.forEach(function(Y){Y.abort();
});
}}},refresh:function(ba){this.invoke(ba,this.__uD[ba].params);
},poll:function(bb,bc,bd){if(this.__IL[bb]){this.__IL[bb].dispose();
}if(typeof bd==o){bd=this.__uD[bb].params;
}this.invoke(bb,bd);
var be=this.__IL[bb]=new qx.event.Timer(bc);
be.addListener(g,function bf(){var bg=this.__IK[bb][0];

if(bg.isDone()||bg.isDisposed()){this.refresh(bb);
}},this);
be.start();
return be;
},longPoll:function(bh){var bj=this,bl=true,bm,bn=0;
function bi(){var bp=bm&&((new Date())-bm)<bj._getThrottleLimit();

if(bp){bn+=1;

if(bn>bj._getThrottleCount()){if(qx.core.Environment.get(k)){bj.debug("Received successful response more than "+bj._getThrottleCount()+" times subsequently, each within "+bj._getThrottleLimit()+" ms. Throttling.");
}return true;
}}if(!bp){bn=0;
}return false;
}var bk=this.__IM[bh]=this.addListener(bh+n,function bo(){if(bj.isDisposed()){return;
}
if(!bi()){bm=new Date();
bj.refresh(bh);
}},bl);
this.invoke(bh);
return bk;
},_getRequestConfig:function(bq,br){var bt=this.__uD[bq];

if(!qx.lang.Type.isArray(bt)){throw new Error("No route for action "+bq);
}var bv=bt[0],bs=this.__IN!==null?this.__IN+bt[1]:bt[1],bw=bt[2],bu=this.__IS(bs);
br=br||{};
bu.forEach(function(bx){var by=new RegExp(d+bx+b),bz=bs.match(by)[1];
if(typeof br[bx]===o){if(bz){br[bx]=bz;
}else{br[bx]=j;
}}bs=bs.replace(by,br[bx]);
});
return {method:bv,url:bs,check:bw};
...
},__IS:function(bA){var bC=/\{(\w+)(=\w+)?\}/g,bD,bB=[];
while((bD=bC.exec(bA))){bB.push(bD[1]);
}return bB;
},__IT:function(bE){qx.lang.Object.getKeys(bE).forEach(function(bF){var bH=bE[bF],bI=bH.method,bG=bH.url,bJ=bH.check;

if(qx.core.Environment.get("qx.debug")){qx.core.Assert.assertString(bI,"Method must be string for route '"+bF+"'");
qx.core.Assert.assertString(bG,"URL must be string for route '"+bF+"'");
}this.map(bF,bI,bG,bJ);
},this);
},__IU:function(bK){if(!this.constructor.$$events){this.constructor.$$events={};
}
if(!this.constructor.$$events[bK]){this.constructor.$$events[bK]="qx.event.type.Rest";
}}},destruct:function(){var bL;

for(bL in this.__IK){if(this.__IK[bL]){this.__IK[bL].forEach(function(bO){bO.dispose();
});
}}
if(this.__IL){for(bL in this.__IL){var bN=this.__IL[bL];
bN.stop();
bN.dispose();
}}
if(this.__IM){for(bL in this.__IM){var bM=this.__IM[bL];
this.removeListenerById(bM);
}}this.__IK=this.__uD=this.__IL=null;
}});
})();
...
qx.Class.define(D,{extend:qx.application.Standalone,members:{__No:null,__Np:null,__Nq:null,__Nr:null,__rm:null,__Ns:null,main:function(){if(typeof GITHUB==u){return;
...
this._setUpBinding();
this.__No.get();
this.__rm.addListener(e,function(I){var J=I.getData();
this.__rm.getSelection().push(J.getItem(0));
},this);
this.__rm.getSelection().addListener(A,function(K){var L=this.__rm.getSelection().getItem(0).getId();
this.__Np.get({id:L});
},this);
},_setUpResources:function(){this.__No=new github.rest.Resource({"get":{method:i,url:B}});
this.__Np=new github.rest.Resource({"get":{method:i,url:G}});
},_setUpStores:function(){this.__Nq=new qx.data.store.Rest(this.__No,g);
this.__Nr=new qx.data.store.Rest(this.__Np,g);
...
M.add(N,{edge:n});
this.__rm=new qx.ui.list.List();
this.__rm.setWidth(200);
this.__Ns=new github.view.Gist();
M.add(this.__rm,{edge:H});
M.add(this.__Ns,{edge:y});
this.getRoot().add(M,{edge:0});
},_setUpBinding:function(){var P=this.__rm,O=this.__Nq,Q=this.__Nr;
P.setLabelPath(v);
...
this.__Nt=new qx.ui.basic.Image();
this.__Nt.set({width:40,height:40,scale:true,backgroundColor:q});
this.__Nu=new qx.ui.basic.Label();
bd.add(this.__Nt);
bd.add(this.__Nu);
...
this.__oK=new qx.ui.embed.Html();
this.__oK.setMinHeight(1000);
scroll.add(this.__oK);
this.add(be,{row:1,column:0});
this.add(scroll,{row:2,column:0,colSpan:2});
},members:{getDescription:function(){return this.__Lm;
},getGravatar:function(){return this.__Nt;
},getUsername:function(){return this.__Nu;
},getContent:function(){return this.__oK;
}}});
})();
...
if(j&&qx.lang.Type.isFunction(j.configureRequest)){this.__IR();
}this.__JK=qx.lang.Function.bind(this.__JL,this);
this.__Ec();
},properties:{resource:{check:d},actionName:{check:b},model:{nullable:true,event:g}},members:{_marshaler:null,_delegate:null,__JK:null,__IR:function(){var k=this.getResource(),l=this._delegate;
k.configureRequest(l.configureRequest);
},__Ec:function(){var n=this.getResource(),m=this.getActionName();

if(n&&m){n.addListener(this.getActionName()+c,this.__JK);
}},__JL:function(e){var p=e.getData(),o=this._marshaler,q,r=this.getModel(),s=this._delegate;
