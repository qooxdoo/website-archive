...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.animation.Scroll-qx.theme.Simple.358759df0334.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:d,check:[d,b]},x:{init:0,check:c},y:{init:0,check:c}},members:{__IC:null,__ID:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var e=this._getElement();
this.__IC={x:e.scrollLeft,y:e.scrollTop};
if(this._atEndPosition(this.__IC.x,this.__IC.y)){return;
}
if(this.getMode()==b){this.__ID={left:this.getX()-this.__IC.x,top:this.getY()-this.__IC.y};
}},update:function(f){qx.fx.Base.prototype.update.call(this);
var g=this._getElement();

if(this.getMode()==d){if(this.getX()!=0){g.scrollLeft=this.__IC.x+(this.getX()*f);
}
if(this.getY()!=0){g.scrollTop=this.__IC.y+(this.getY()*f);
}}else{g.scrollLeft=this.__IC.x+(this.__ID.left*f);
g.scrollTop=this.__IC.y+(this.__ID.top*f);
