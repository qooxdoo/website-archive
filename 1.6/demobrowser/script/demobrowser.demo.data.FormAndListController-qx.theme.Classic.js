...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.FormAndListController","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.FormAndListController-qx.theme.Classic.0d3a4d0b0a89.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="modelSelection[0]",k="value",j="changeModel",h="qx.core.Object",g="qx.debug",f="_applyTarget",e="qx.data.controller.Form",d="changeTarget",c=".",b="qx.ui.form.Form",a="_applyModel";
qx.Class.define(e,{extend:qx.core.Object,construct:function(m,n,o){qx.core.Object.call(this);
this._selfUpdate=!!o;
this.__Df={};
...
}},properties:{model:{check:h,apply:a,event:j,nullable:true,dereference:true},target:{check:b,apply:f,event:d,nullable:true,init:null,dereference:true}},members:{__Dg:null,__Df:null,addBindingOptions:function(name,p,q){this.__Df[name]=[p,q];
if(this.getModel()==null||this.getTarget()==null){return;
}var r=this.getTarget().getItems()[name];
var s=this.__Dj(r)?l:k;
this.__Dg.removeTarget(r,s,name);
this.__Dg.addTarget(r,s,name,!this._selfUpdate,p,q);
},createModel:function(t){var v=this.getTarget();
if(v==null){throw new Error("No target is set.");
}var w=v.getItems();
var x={};

for(var name in w){var y=name.split(c);
var B=x;

for(var i=0;i<y.length;i++){if(i+1==y.length){var A=w[name].constructor;
var u=null;

if(qx.Class.hasInterface(A,qx.ui.core.ISingleSelection)){u=w[name].getModelSelection().getItem(0)||null;
}else{u=w[name].getValue();
}if(this.__Df[name]&&this.__Df[name][1]){u=this.__Df[name][1].converter(u);
}B[y[i]]=u;
}else{if(!B[y[i]]){B[y[i]]={};
}B=B[y[i]];
}}}var z=qx.data.marshal.Json.createModel(x,t);
this.setModel(z);
return z;
},updateModel:function(){if(!this._selfUpdate||!this.getModel()||!this.getTarget()){return;
}var C=this.getTarget().getItems();

for(var name in C){var F=C[name];
var E=this.__Dj(F)?l:k;
var D=this.__Df[name];
D=D&&this.__Df[name][1];
qx.data.SingleValueBinding.updateTarget(F,E,this.getModel(),name,D);
}},_applyTarget:function(G,H){if(H!=null){this.__Di(H);
}if(this.getModel()==null){return;
}if(G!=null){this.__Dh();
}},_applyModel:function(I,J){if(this.__Dg!=null){var K=this.getTarget().getItems();

for(var name in K){var M=K[name];
var L=this.__Dj(M)?l:k;
this.__Dg.removeTarget(M,L,name);
}}if(this.__Dg!=null){this.__Dg.setModel(I);
}if(this.getTarget()==null){return;
}if(I!=null){this.__Dh();
}},__Dh:function(){if(this.__Dg==null){this.__Dg=new qx.data.controller.Object(this.getModel());
}var N=this.getTarget().getItems();
for(var name in N){var Q=N[name];
var O=this.__Dj(Q)?l:k;
var P=this.__Df[name];
try{if(P==null){this.__Dg.addTarget(Q,O,name,!this._selfUpdate);
}else{this.__Dg.addTarget(Q,O,name,!this._selfUpdate,P[0],P[1]);
}}catch(R){if(qx.core.Environment.get(g)){this.warn("Could not bind property "+name+" of "+this.getModel());
}}}},__Di:function(S){if(this.__Dg==null){return;
}var T=S.getItems();
for(var name in T){var V=T[name];
var U=this.__Dj(V)?l:k;
this.__Dg.removeTarget(V,U,name);
}},__Dj:function(W){return qx.Class.hasInterface(W.constructor,qx.ui.core.ISingleSelection)&&qx.Class.hasInterface(W.constructor,qx.ui.form.IModelSelection);
}},destruct:function(){if(this.__Dg){this.__Dg.dispose();
}}});
})();
