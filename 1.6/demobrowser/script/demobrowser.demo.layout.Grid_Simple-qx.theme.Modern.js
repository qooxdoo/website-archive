...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.layout.Grid_Simple","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Grid_Simple-qx.theme.Modern.d41167965774.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="_applyLayoutChange",j="top",h="left",g="west",f="Integer",e="auto",d="center",c="y",b="Decorator",a="north",z="east",y="edge",x="south",w="flex",v="x",u="Boolean",t="height",s="bottom",r="The property '",q="qx.debug",o="width",p="' is not supported by the Dock layout!",m="qx.ui.layout.Dock",n="_applySort",l="right";
qx.Class.define(m,{extend:qx.ui.layout.Abstract,construct:function(A,B,C,D){qx.ui.layout.Abstract.call(this);
...
}},properties:{sort:{check:[e,c,v],init:e,apply:n},separatorX:{check:b,nullable:true,apply:k},separatorY:{check:b,nullable:true,apply:k},connectSeparators:{check:u,init:false,apply:k},spacingX:{check:f,init:0,apply:k},spacingY:{check:f,init:0,apply:k}},members:{__kv:null,__pj:null,verifyLayoutProperty:qx.core.Environment.select(q,{"true":function(E,name,F){this.assertInArray(name,[w,y,t,o],r+name+p);

if(name===y){this.assertInArray(F,[a,x,g,z,d]);
}else if(name===w){this.assertNumber(F);
this.assert(F>=0);
...
},__pk:{north:1,south:2,west:3,east:4,center:5},__pl:{1:j,2:s,3:h,4:l},__lg:function(){var G=this._getLayoutChildren();
var O,I;
var length=G.length;
var K=[];
var N=[];
var L=[];
var H=this.getSort()===c;
var M=this.getSort()===v;

for(var i=0;i<length;i++){O=G[i];
L=O.getLayoutProperties().edge;

if(L===d){if(I){throw new Error("It is not allowed to have more than one child aligned to 'center'!");
}I=O;
}else if(M||H){if(L===a||L===x){H?K.push(O):N.push(O);
}else if(L===g||L===z){H?N.push(O):K.push(O);
}}else{K.push(O);
}}var P=K.concat(N);
...
for(var i=0;i<length;i++){L=P[i].getLayoutProperties().edge;
J[i]=this.__pk[L]||5;
}this.__pj=J;
delete this._invalidChildrenCache;
},renderLayout:function(Q,R){if(this._invalidChildrenCache){this.__lg();
}var bl=qx.ui.layout.Util;
var X=this.__kv;
var bm=this.__pj;
var length=X.length;
var bb,T,ba,bg,bh,br,be,U,bo;
var bv=[];
var bd=[];
var bu=this._getSeparatorWidths();
var bz=this.getSpacingX();
...
for(var i=0;i<length;i++){T=X[i];
bg=T.getLayoutProperties();
ba=T.getSizeHint();
be=ba.width;
U=ba.height;

if(bg.width!=null){be=Math.floor(Q*parseFloat(bg.width)/100);

if(be<ba.minWidth){be=ba.minWidth;
}else if(be>ba.maxWidth){be=ba.maxWidth;
}}
if(bg.height!=null){U=Math.floor(R*parseFloat(bg.height)/100);

if(U<ba.minHeight){U=ba.minHeight;
}else if(U>ba.maxHeight){U=ba.maxHeight;
}}bv[i]=be;
bd[i]=U;
switch(bm[i]){case 1:case 2:bn+=U+T.getMarginTop()+T.getMarginBottom()+by;

if(bu.y){bn+=bu.y+by;
}break;
case 3:case 4:Y+=be+T.getMarginLeft()+T.getMarginRight()+bz;

if(bu.x){Y+=bu.x+bz;
}break;
default:Y+=be+T.getMarginLeft()+T.getMarginRight()+bz;
bn+=U+T.getMarginTop()+T.getMarginBottom()+by;
...
switch(bm[i]){case 3:case 4:case 5:bh=T.getLayoutProperties().flex;
if(bh==null&&bm[i]==5){bh=1;
}
if(bh>0){ba=T.getSizeHint();
bb[i]={min:ba.minWidth,value:bv[i],max:ba.maxWidth,flex:bh};
}}}var V=bl.computeFlexOffsets(bb,Q,Y);

for(var i in V){bo=V[i].offset;
bv[i]+=bo;
Y+=bo;
}}if(bn!=R){bb={};
br=bn<R;
...
switch(bm[i]){case 1:case 2:case 5:bh=T.getLayoutProperties().flex;
if(bh==null&&bm[i]==5){bh=1;
}
if(bh>0){ba=T.getSizeHint();
bb[i]={min:ba.minHeight,value:bd[i],max:ba.maxHeight,flex:bh};
}}}var V=bl.computeFlexOffsets(bb,R,bn);

for(var i in V){bo=V[i].offset;
bd[i]+=bo;
bn+=bo;
}}this._clearSeparators();
var bs=this.getSeparatorX(),bt=this.getSeparatorY();
var bw=this.getConnectSeparators();
var bj=0,bq=0;
var bD,top,be,U,bi,bA;
var bk,bx,bC,S;
var bB,bc,bf,W;
var bp=this.__pl;
...
bf=T.getMarginLeft();
W=T.getMarginRight();
switch(bA){case 1:case 2:be=Q-bf-W;
if(be<ba.minWidth){be=ba.minWidth;
}else if(be>ba.maxWidth){be=ba.maxWidth;
}U=bd[i];
top=bj+bl.computeVerticalAlignOffset(bp[bA],U,R,bB,bc);
bD=bq+bl.computeHorizontalAlignOffset(T.getAlignX()||h,be,Q,bf,W);
if(bu.y){if(bA==1){bx=bj+U+bB+by+bc;
...
case 3:case 4:U=R-bB-bc;
if(U<ba.minHeight){U=ba.minHeight;
}else if(U>ba.maxHeight){U=ba.maxHeight;
}be=bv[i];
bD=bq+bl.computeHorizontalAlignOffset(bp[bA],be,Q,bf,W);
top=bj+bl.computeVerticalAlignOffset(T.getAlignY()||j,U,R,bB,bc);
if(bu.x){if(bA==3){bk=bq+be+bf+bz+W;
...
top=bj+bl.computeVerticalAlignOffset(T.getAlignY()||j,U,R,bB,bc);
}T.renderLayout(bD,top,be,U);
}},_getSeparatorWidths:function(){var bF=this.getSeparatorX(),bE=this.getSeparatorY();

if(bF||bE){var bK=qx.theme.manager.Decoration.getInstance();
}
if(bF){var bL=bK.resolve(bF);
var bH=bL.getInsets();
var bI=bH.left+bH.right;
}
if(bE){var bM=bK.resolve(bE);
var bG=bM.getInsets();
var bJ=bG.top+bG.bottom;
...
var bY=0,ci=0;
var bO=0,cd=0;
var cf=this._getSeparatorWidths();
var ch=this.getSpacingX(),cg=this.getSpacingY();
var bS=-ch,bR=-cg;

if(cf.x){bS-=cf.x+ch;
}
if(cf.y){bR-=cf.y+cg;
}for(var i=0;i<length;i++){cc=bQ[i];
bU=cc.getSizeHint();
bW=cc.getMarginLeft()+cc.getMarginRight();
bV=cc.getMarginTop()+cc.getMarginBottom();
switch(cb[i]){case 1:case 2:bY=Math.max(bY,bU.width+bX+bW);
ci=Math.max(ci,bU.minWidth+cj+bW);
bO+=bU.height+bV;
cd+=bU.minHeight+bV;
bR+=cg;
...
ce=Math.max(ce,bU.minHeight+cd+bV);
bX+=bU.width+bW;
cj+=bU.minWidth+bW;
bS+=ch;
...
if(cf.y){bR+=cf.y+cg;
}}}var bT=Math.max(cj,ci)+bS;
var ck=Math.max(bX,bY)+bS;
var ca=Math.max(ce,cd)+bR;
var bP=Math.max(bN,bO)+bR;
return {minWidth:bT,width:ck,minHeight:ca,height:bP};
...
(function(){var k="main",j="green",i="yellow",h="click me",g="click",f="center",d="top",c="middle",b="right",a="demobrowser.demo.layout.Grid_Simple";
...
var l=new qx.ui.container.Composite(new qx.ui.layout.VBox(20)).set({padding:20});
scroll.add(l);
l.add(this.getGrid1());
l.add(this.getGrid2());
l.add(this.getGrid3());
},getGrid1:function(){var m=new qx.ui.container.Composite(new qx.ui.layout.Grid(10,20)).set({decorator:k,backgroundColor:i,allowGrowX:false,allowGrowY:false});
m.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j}),{row:0,column:0});
m.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j}),{row:0,column:1});
m.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j}),{row:1,column:0});
m.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j}),{row:1,column:1});
return m;
},getGrid2:function(){var p=new qx.ui.layout.Grid();
p.setColumnAlign(1,f,d);
p.setColumnAlign(0,b,c);
p.setSpacing(5);
var o=new qx.ui.container.Composite(p).set({decorator:k,backgroundColor:i,allowGrowX:false});
o.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j,width:150}),{row:0,column:0});
var n=new qx.ui.basic.Label(h).set({decorator:k,backgroundColor:j,allowGrowX:false,width:50,height:50});
o.add(n,{row:0,column:1});
o.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j,maxHeight:60,allowGrowX:false}),{row:1,column:0});
o.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j,height:80}),{row:1,column:1});
o.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j}),{row:2,column:0});
o.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j}),{row:2,column:1});
var q=10;
n.addListener(g,function(e){n.setWidth(50+q);
q=-q;
});
return o;
},getGrid3:function(){var s=new qx.ui.container.Composite().set({decorator:k,backgroundColor:i,width:400,height:300,allowShrinkX:false,allowShrinkY:false});
var r=new qx.ui.layout.Grid();
r.setColumnFlex(1,2);
r.setRowFlex(1,3);
r.setSpacing(5);
s.setLayout(r);
s.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j}),{row:0,column:0});
s.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j}),{row:0,column:1});
s.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j}),{row:1,column:0});
s.add(new qx.ui.core.Widget().set({decorator:k,backgroundColor:j}),{row:1,column:1});
return s;
}}});
})();
