...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine-qx.theme.Modern.06157a885252.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j="String",h="string",g="Number",f="__HQ",d="__HR",c="qx.util.fsm.FiniteStateMachine",b="object";
...
if(!this.__HV[v]){this.__HV[v]={};
}this.__HV[v][r]=true;
if(!this.__HW[r]){this.__HW[r]=[];
}this.__HW[r]=this.__HW[r].concat(u);
}},removeObject:function(x){var z=this.__HT[x];
if(this.__HW[x]){for(var y in this.__HW[x]){delete this.__HV[y];
}delete this.__HW[x];
}delete this.__HU[z];
delete this.__HT[x];
delete this.__HS[x];
},getObject:function(A){return this.__HS[A];
},getFriendlyName:function(B){var C=B?B.toHashCode():null;
return C?this.__HU[C]:null;
},getGroupObjects:function(D){var a=[];

for(var name in this.__HV[D]){a.push(name);
}return a;
},displayAllObjects:function(){for(var E in this.__HT){var G=this.__HT[E];
var F=this.getObject(E);
this.debug(E+" => "+G);
this.debug("  "+G+" => "+this.__HU[G]);
this.debug("  "+E+" => "+this.getObject(E));
this.debug("  "+this.getObject(E)+" => "+this.getFriendlyName(F));
}},start:function(){var I=this.__HO;
...
}this.__kX[I].getAutoActionsBeforeOnentry()(this);
if(H){this.debug(this.getName()+"#"+I+"#entry");
}this.__kX[I].getOnentry()(this,null);
if(H){this.debug(this.getName()+"#"+I+"#actionsAfterOnentry");
}this.__kX[I].getAutoActionsAfterOnentry()(this);
},pushState:function(J){if(this.__HP.length>=this.getMaxSavedStates()){throw new Error("Saved-state stack is full");
}
if(J===true){this.__HP.push(this.getState());
}else if(J){this.__HP.push(J);
}else{this.__HP.push(this.getPreviousState());
}},popState:function(){if(this.__HP.length==0){throw new Error("Saved-state stack is empty");
}return this.__HP.pop();
},postponeEvent:function(event){this.__HR.unshift(event);
},enqueueEvent:function(event,K){if(K){this.__HQ.push(event);
}else{this.__HQ.unshift(event);
}
if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){var L=this.getFriendlyName(event.getTarget());

if(K){this.debug(this.getName()+": Pushed event: "+event.getType()+(L?" on "+L:""));
}else{this.debug(this.getName()+": Queued event: "+event.getType()+(L?" on "+L:""));
}}},eventListener:function(event){var e=event.clone();
this.enqueueEvent(e,false);
this.__HY();
},fireImmediateEvent:function(M,N,O){if(O){var event=qx.event.Registration.createEvent(M,qx.event.type.Data,[O,null,false]);
}else{var event=qx.event.Registration.createEvent(M,qx.event.type.Event,[false,false]);
}event.setTarget(N);
this.eventListener(event);
},scheduleEvent:function(P,Q,R,S){qx.event.Timer.once(function(){this.fireImmediateEvent(P,Q,R);
},this,S||20);
},__HY:function(){if(this.__HX){return ;
}this.__HX=true;
while(this.__HQ.length>0){var event=this.__HQ.pop();
var T=this.__Ia(event);
if(T){event.dispose();
}}this.__HX=false;
},__Ia:function(event){var W;
var Y;
var V;
var bh;
var bf;
var e;
var bd;
var U=this.getDebugFlags();
var bg=U&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;
var ba=U&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;
var be=U&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;
var bc=U&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;
var X=this.getFriendlyName(event.getTarget());

if(bg){this.debug(this.getName()+": Process event: "+event.getType()+(X?" on "+X:""));
}W=this.getState();
bh=this.__kX[W];
bf=bh.transitions;
e=bh.getEvents()[event.getType()];
if(!e){if(bg){this.debug(this.getName()+": Event '"+event.getType()+"'"+" not handled.  Ignoring.");
}return true;
}if(typeof (e)==b){if(!X){if(bc){this.debug(this.getName()+": Could not find friendly name for '"+event.getType()+"' on '"+event.getTarget()+"'");
}return true;
}bd=e[X];
if(!bd){if(bg){this.debug(this.getName()+": Event '"+event.getType()+"'"+" not handled for target "+X+".  Ignoring.");
}return true;
}}else{bd=e;
}
switch(bd){case qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE:break;
case qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED:if(bg){this.debug(this.getName()+": Event '"+event.getType()+"'"+" blocked.  Re-queuing.");
}this.__HR.unshift(event);
return false;
default:if(typeof (bd)==h){if(bf[bd]){var bb=bf[bd];
bf={};
bf[bd]=bb;
}else{throw new Error("Explicit transition "+bd+" does not exist");
}break;
}}for(var t in bf){var bb=bf[t];
switch(bb.getPredicate()(this,event)){case true:break;
case false:continue;
case null:return true;
default:throw new Error("Transition "+W+":"+t+" returned a value other than "+"true, false, or null.");
}Y=bb.getNextState();

if(typeof (Y)==h){if(!Y in this.__kX){throw new Error("Attempt to transition to nonexistent state "+Y);
}this.setNextState(Y);
}else{switch(Y){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:Y=W;
this.setNextState(Y);
break;
case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__HP.length==0){throw new Error("Attempt to transition to POP_STATE_STACK "+"while state stack is empty.");
}Y=this.__HP.pop();
this.setNextState(Y);
break;
default:throw new Error("Internal error: invalid nextState");
break;
}}if(be){this.debug(this.getName()+"#"+W+"#"+t+"#autoActionsBeforeOntransition");
}bb.getAutoActionsBeforeOntransition()(this);
if(be){this.debug(this.getName()+"#"+W+"#"+t+"#ontransition");
}bb.getOntransition()(this,event);
if(be){this.debug(this.getName()+"#"+W+"#"+t+"#autoActionsAfterOntransition");
}bb.getAutoActionsAfterOntransition()(this);
if(be){this.debug(this.getName()+"#"+W+"#autoActionsBeforeOnexit");
}bh.getAutoActionsBeforeOnexit()(this);
if(be){this.debug(this.getName()+"#"+W+"#exit");
}bh.getOnexit()(this,event);
if(be){this.debug(this.getName()+"#"+W+"#autoActionsAfterOnexit");
}bh.getAutoActionsAfterOnexit()(this);
if(bh._bNeedDispose){bh.dispose();
}bh=this.__kX[this.getNextState()];
this.setPreviousState(W);
this.setState(this.getNextState());
this.setNextState(null);
V=W;
W=Y;
Y=undefined;
if(be){this.debug(this.getName()+"#"+W+"#autoActionsBeforeOnentry");
}bh.getAutoActionsBeforeOnentry()(this);
if(be){this.debug(this.getName()+"#"+W+"#entry");
}bh.getOnentry()(this,event);
if(be){this.debug(this.getName()+"#"+W+"#autoActionsAfterOnentry");
}bh.getAutoActionsAfterOnentry()(this);
for(var i=0;i<this.__HR.length;i++){e=this.__HR.pop();
this.__HQ.unshift(e);
}
if(ba){this.debug(this.getName()+"#"+V+" => "+this.getName()+"#"+W);
}return true;
}
if(ba){this.debug(this.getName()+"#"+W+": event '"+event.getType()+"'"+": no transition found.  No state change.");
}return true;
}},destruct:function(){this._disposeArray(f);
this._disposeArray(d);
this.__HP=this.__kX=null;
}});
})();
(function(){var p="object",o="context",n="string",m='"',l="}",k="{",h="(",d=";",c="function",b="  var groupObjects = ",Y="__HJ",X="__HK",W="parameters",V="__HF",U=")",T=" fsm.getObject('",S="groups",R="  for (var i = 0; i < groupObjects.length; i++)",Q="autoActionsAfterOnentry",P="fsm",w="');",x="autoActionsBeforeOnexit",u="autoActionsBeforeOnentry",v="__HL",s="onexit",t="__HH",q="autoActionsAfterOnexit",r="  fsm.debug(ex);",y="    fsm.getObject(objName).",z=")(fsm)",G="events",E="__HG",J="__HI",I="    var objName = groupObjects[i];",L="').",K="objects",B="catch(ex)",O="  {",N="    fsm.getGroupObjects('",M="  }",A="qx.util.fsm.State",C="onentry",D="__HM",F="try",H=",";
...
for(var bc in bb){switch(bc){case C:this.setOnentry(this.__HN(bb[bc],bd));
break;
case s:this.setOnexit(this.__HN(bb[bc],bd));
break;
...
for(j=0;j<g.length;j++){bm+=b+N+g[j]+w+R+O+I+y+bl+d+M;
}}}}bm+=l+B+k+r+l;
return qx.lang.Function.bind(new Function(P,bm),bg);
}},properties:{name:{transform:V,nullable:true},onentry:{transform:E,nullable:true,init:function(bn,event){}},onexit:{transform:t,nullable:true,init:function(bo,event){}},autoActionsBeforeOnentry:{transform:Y,nullable:true,init:function(bp,event){}},autoActionsAfterOnentry:{transform:X,nullable:true,init:function(bq,event){}},autoActionsBeforeOnexit:{transform:v,nullable:true,init:function(br,event){}},autoActionsAfterOnexit:{transform:D,nullable:true,init:function(bs,event){}},events:{transform:J,nullable:true}},members:{__HF:function(bt){if(typeof (bt)!="string"||bt.length<1){throw new Error("Invalid state name");
}return bt;
},__HG:function(bu){switch(typeof (bu)){case "undefined":return function(bv,event){};
...
}},__HH:function(bw){switch(typeof (bw)){case "undefined":return function(bx,event){};
...
return null;
}},__HI:function(by){if(typeof (by)!="object"){throw new Error("events must be an object");
...
},__HJ:function(bB){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnentry",bB,this.getUserData("context"));
},__HK:function(bC){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnentry",bC,this.getUserData("context"));
},__HL:function(bD){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnexit",bD,this.getUserData("context"));
},__HM:function(bE){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnexit",bE,this.getUserData("context"));
},__HN:function(f,bF){if(typeof (f)=="function"){f=qx.lang.Function.bind(f,bF);
}return f;
...
(function(){var s="context",r="undefined",q="autoActionsBeforeOntransition",p="function",o="autoActionsAfterOntransition",n="string",m="predicate",l="nextState",k="number",j="__Ie",c="String",i="__Ic",g="__Ib",b="__Id",a="qx.util.fsm.Transition",e="__If",d="ontransition",h="boolean";
qx.Class.define(a,{extend:qx.core.Object,construct:function(t,u){var w;
qx.core.Object.call(this);
this.setName(t);
w=u.context||window;
this.setUserData(s,w);
for(var v in u){switch(v){case m:this.setPredicate(this.__HN(u[v],w));
break;
case l:this.setNextState(u[v]);
break;
case q:this.setAutoActionsBeforeOntransition(this.__HN(u[v],w));
break;
case o:this.setAutoActionsAfterOntransition(this.__HN(u[v],w));
break;
case d:this.setOntransition(this.__HN(u[v],w));
break;
case s:break;
default:this.setUserData(v,u[v]);
this.debug("Transition "+t+": "+"Adding user-provided field to transition: "+v);
break;
}}},properties:{name:{check:c,nullable:true},predicate:{init:function(x,event){return true;
},transform:g},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:i},autoActionsBeforeOntransition:{init:function(y,event){},transform:b},autoActionsAfterOntransition:{init:function(z,event){},transform:j},ontransition:{init:function(A,event){},transform:e}},members:{__Ib:function(B){switch(typeof (B)){case r:return function(C,event){return true;
};
...
break;
}},__Ic:function(E){switch(typeof (E)){case n:return E;
case k:switch(E){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return E;
...
break;
}},__Id:function(F){return qx.util.fsm.State._commonTransformAutoActions(q,F,this.getUserData(s));
},__Ie:function(G){return qx.util.fsm.State._commonTransformAutoActions(o,G,this.getUserData(s));
},__If:function(H){switch(typeof (H)){case r:return function(I,event){};
case p:return qx.lang.Function.bind(H,this.getUserData(s));
default:throw new Error("Invalid ontransition type: "+typeof (H));
break;
}},__HN:function(f,J){if(typeof (f)=="function"){f=qx.lang.Function.bind(f,J);
}return f;
}}});
})();
