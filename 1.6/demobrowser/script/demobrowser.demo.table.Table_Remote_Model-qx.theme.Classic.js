...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Remote_Model","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Remote_Model-qx.theme.Classic.ac3be0da134f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
this.__AS=null;
this.__zq=null;
},properties:{blockSize:{check:c,init:50},maxCachedBlockCount:{check:c,init:15},clearCacheOnRemove:{check:b,init:false},blockConcurrentLoadRowCount:{check:b,init:true}},members:{__Ig:null,__Il:null,__Ih:null,__Ii:null,__Ij:null,__Ik:null,__Im:null,__In:null,__AQ:null,__AT:null,__zq:null,__AS:null,__Io:false,_getIgnoreCurrentRequest:function(){return this.__Il;
},getRowCount:function(){if(this.__Ig==-1){if(!this.__Io||!this.getBlockConcurrentLoadRowCount()){this.__Io=true;
this._loadRowCount();
}return (this.__Ig==-1)?0:this.__Ig;
}else{return this.__Ig;
}},_loadRowCount:function(){throw new Error("_loadRowCount is abstract");
},_onRowCountLoaded:function(f){if(this.getBlockConcurrentLoadRowCount()){this.__Io=false;
}if(f==null||f<0){f=0;
}this.__Ig=Number(f);
...
if(this.__Ii!=-1){var h=this._cancelCurrentRequest();
...
this.__Io=true;
this._loadRowCount();
},clearCache:function(){this.__Im={};
this.__In=0;
},getCacheContent:function(){return {sortColumnIndex:this.__AQ,sortAscending:this.__AT,rowCount:this.__Ig,lruCounter:this.__Ih,rowBlockCache:this.__Im,rowBlockCount:this.__In};
},restoreCacheContent:function(j){if(this.__Ii!=-1){var k=this._cancelCurrentRequest();

if(k){this.__Ii=-1;
this.__Il=false;
}else{this.__Il=true;
}}this.__AQ=j.sortColumnIndex;
this.__AT=j.sortAscending;
this.__Ig=j.rowCount;
this.__Ih=j.lruCounter;
this.__Im=j.rowBlockCache;
this.__In=j.rowBlockCount;
var l={firstRow:0,lastRow:this.__Ig-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
for(var w=0;w<=o;w++){var q=this.__Im[w];
...
if(r!=null){u[t]=r;
}}}}},prefetchRows:function(x,y){if(this.__Ii==-1){var z=this.getBlockSize();
var E=Math.ceil(this.__Ig/z);
...
for(var F=D;F<=C;F++){if(this.__Im[F]==null||this.__Im[F].isDirty){if(B==-1){B=F;
}A=F;
}}if(B!=-1){this.__Ij=-1;
this.__Ik=-1;
this.__Ii=B;
...
},_onRowDataLoaded:function(I){if(I!=null&&!this.__Il){var L=this.getBlockSize();
var J=Math.ceil(I.length/L);

if(J==1){this._setRowBlockData(this.__Ii,I);
}else{for(var i=0;i<J;i++){var O=i*L;
var N=[];
var K=Math.min(L,I.length-O);

for(var P=0;P<K;P++){N.push(I[O+P]);
}this._setRowBlockData(this.__Ii+i,N);
}}var M={firstRow:this.__Ii*L,lastRow:(this.__Ii+J+1)*L-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(e,M);
}this.__Ii=-1;
this.__Il=false;
if(this.__Ij!=-1){this.prefetchRows(this.__Ij,this.__Ik);
}},_setRowBlockData:function(Q,R){if(this.__Im[Q]==null){this.__In++;

while(this.__In>this.getMaxCachedBlockCount()){var V;
var U=this.__Ih;

for(var T in this.__Im){var S=this.__Im[T].lru;

if(S<U&&T>1){U=S;
V=T;
}}delete this.__Im[V];
this.__In--;
}}this.__Im[Q]={lru:++this.__Ih,rowDataArr:R};
...
for(var bf=bc;bf<=bb;bf++){var X=this.__Im[bf];
...
if(bf==bb-1){if(X.rowDataArr.length==0){delete this.__Im[bf];
}}else{var bd=this.__Im[bf+1];

if(bd!=null){X.rowDataArr.push(bd.rowDataArr[0]);
}else{X.isDirty=true;
}}}}
if(this.__Ig!=-1){this.__Ig--;
...
if(bi.lru!=this.__Ih){bi.lru=++this.__Ih;
}return bj;
...
}}},setEditable:function(bv){this.__zq=[];

for(var bw=0;bw<this.getColumnCount();bw++){this.__zq[bw]=bv;
}this.fireEvent(d);
},setColumnEditable:function(bx,by){if(by!=this.isColumnEditable(bx)){if(this.__zq==null){this.__zq=[];
}this.__zq[bx]=by;
this.fireEvent(d);
}},isColumnEditable:function(bz){return (this.__zq?(this.__zq[bz]==true):false);
},setColumnSortable:function(bA,bB){if(bB!=this.isColumnSortable(bA)){if(this.__AS==null){this.__AS=[];
}this.__AS[bA]=bB;
this.fireEvent(d);
}},isColumnSortable:function(bC){return (this.__AS?(this.__AS[bC]!==false):true);
},sortByColumn:function(bD,bE){if(this.__AQ!=bD||this.__AT!=bE){this.__AQ=bD;
this.__AT=bE;
this.clearCache();
this.fireEvent(d);
}},getSortColumnIndex:function(){return this.__AQ;
},isSortAscending:function(){return this.__AT;
},setSortColumnIndexWithoutSortingData:function(bF){this.__AQ=bF;
},setSortAscendingWithoutSortingData:function(bG){this.__AT=bG;
}},destruct:function(){this.__AS=this.__zq=this.__Im=null;
}});
})();
(function(){var o="text",n="&end=",m='demobrowser.demo.table.RemoteTableModel',l="Text",k=' Generated on:',j="method=checkphp",h="id",g="success",f="../../resource/demobrowser/backend/remote_table.php?",e="method=getRowData&start=",b="WTF PHP",d="method=getRowCount",c="Id",a='Hello ';
qx.Class.define(m,{extend:qx.ui.table.model.Remote,construct:function(){qx.ui.table.model.Remote.call(this);
this.setColumns([c,l],[h,o]);
},members:{__Od:null,__Oe:false,_loadRowCount:function(){this.__Of();

if(this.__Oe){return;
}
if(this.__Od){this.__Og();
}else{this.__Oj();
}},_loadRowData:function(p,q){this.__Of();

if(this.__Oe){return;
}
if(this.__Od){this.__Oh(p,q);
}else{this.__Ok(p,q);
}},__Of:function(){if(this.__Oe||this.__Od!==null){return;
}this.__Oe=true;
this.__Oi(j,function(r){this.__Oe=false;
this.__Od=(r==b);
this._loadRowCount();
});
},__Og:function(){var s=d;
this.__Oi(s,function(t){this._onRowCountLoaded(parseInt(t));
});
},__Oh:function(u,v){var w=e+u+n+v;
this.__Oi(w,function(x){this._onRowDataLoaded(qx.lang.Json.parse(x));
});
},__Oi:function(y,z){var A=new qx.io.request.Xhr();
A.setUrl(f+y);
A.addListener(g,function(){z.call(this,A.getResponseText());
},this);
A.send();
},__Oj:function(){var self=this;
window.setTimeout(function(){self._onRowCountLoaded(1000000);
},0);
},__Ok:function(B,C){var self=this;
window.setTimeout(function(){var D=[];

for(var i=B;i<=C;i++){D.push({id:i,text:a+i+k+(new Date())});
}self._onRowDataLoaded(D);
},0);
}}});
})();
