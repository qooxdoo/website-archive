...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Window-qx.theme.Classic.3c3393f245dd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
qx.Class.define(m,{extend:qx.application.Native,members:{__mo:null,main:function(){qx.application.Native.prototype.main.call(this);
var r=qx.util.ResourceManager.getInstance();
this.__mo=[g,k,h,r.toUri(f)];
...
this.window1=qx.bom.Window.open(this.__mo[0],j,x);
},this);
...
this.window2=qx.bom.Window.open(this.__mo[3],a,y,true);
},this);
...
this.window3=qx.bom.Window.open(this.__mo[0],i,A,true,false);
},this);
...
(function(){var k='load',j="dialogHeight",i="qx.bom.Window",h="interval",g="dialogWidth",f="resizable",e="dialogLeft",d="javascript:/",c="dialogTop",b="qxNativeWindow",a="scroll";
qx.Class.define(i,{statics:{__na:null,__Gw:null,__lh:null,__Gx:{"top":c,left:e,width:g,height:j,scrollbars:a,resizable:f},__Gy:{"top":1,left:1,width:1,height:1,dependent:1,resizable:1,status:1,location:1,menubar:1,scrollbars:1,toolbar:1},__Gz:function(){return window.showModalDialog!=null;
},open:function(l,name,m,n,o,p,self){var t=null;
...
if(o==null){o=true;
}var q=this.__GA(m,n&&o);

if(n){if(this.__Gz()&&o){t=window.showModalDialog(l,[window.self],q);
}else{this.getBlocker().block();

if(this.__lh==null){this.__lh=new qx.event.Timer(500);
this.__lh.addListener(h,this.__GB,this);
}this.__Gw=window.open(l,name,q);
this.__lh.restart();
t=this.__Gw;
}}else{t=window.open(l,name,q);
}
if(t&&p&&(p instanceof Function)){var s=self||t;
var r=qx.lang.Function.bind(p,s);
qx.bom.Event.addNativeListener(t,k,function(){r();
qx.bom.Event.removeNativeListener(t,k,arguments.callee);
});
}return t;
},__GA:function(u,v){var w;
var z;
var x=[];

if(v&&this.__Gz()){for(var y in u){if(qx.bom.Window.__Gx[y]){var A="";

if(y!="scrollbars"&&y!="resizable"){A="px";
}z=qx.bom.Window.__Gx[y]+":"+u[y]+A;
x.push(z);
}else{qx.log.Logger.warn("Option '"+y+"' is not supported for modal windows.");
}}w=x.join(";");
}else{for(var y in u){if(qx.bom.Window.__Gy[y]){if(qx.lang.Type.isBoolean(u[y])){z=y+"="+(u[y]?"yes":"no");
}else{z=y+"="+u[y];
}x.push(z);
}else{qx.log.Logger.warn("Option '"+y+"' is not supported for native windows.");
}}w=x.join(",");
}return w;
},__GB:function(){if(this.isClosed(this.__Gw)){this.getBlocker().unblock();
this.__lh.stop();
}},getBlocker:function(){if(this.__na==null){this.__na=new qx.bom.Blocker;
}return this.__na;
},close:function(B){if(B){return B.close();
}},isClosed:function(C){var closed=true;

if(C){try{closed=C.closed;
}catch(D){}}return closed;
},moveTo:function(E,top,F){if(!qx.bom.Window.isClosed(E)){try{E.moveTo(F,top);
}catch(G){qx.log.Logger.error("Cross-Domain Scripting problem: Could not move window!",G);
}}},resizeTo:function(H,I,J){if(!qx.bom.Window.isClosed(H)){try{H.resizeTo(I,J);
}catch(K){qx.log.Logger.error("Cross-Domain Scripting problem: Could not resize window!",K);
}}}}});
})();
