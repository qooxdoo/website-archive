...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.animation.Login","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.animation.Login-qx.theme.Simple.d71cd2ddbb8b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var p="top",o="right",n="resize",m="Name",l="__sO",k="Password",j="Login",h="__Nc",g="__Ll",f="appear",b="demobrowser.demo.animation.Login",d="50%",c="execute",a="30%";
qx.Class.define(b,{extend:qx.application.Standalone,members:{__Ll:null,__sO:null,__Nc:null,checkInput:function(){this.__Ll.start();
},__Nd:function(){this.__Ll=new qx.fx.effect.combination.Shake(this.__sO.getContainerElement().getDomElement());
...
this.__sO.setLayout(r);
this.__sO.addListener(n,function(e){var v=this.__sO.getBounds();
this.__sO.set({marginTop:Math.round(-v.height/2),marginLeft:Math.round(-v.width/2)});
},this);
this.getRoot().add(this.__sO,{left:d,top:a});
var q=[m,k];

for(var i=0;i<q.length;i++){this.__sO.add(new qx.ui.basic.Label(q[i]).set({allowShrinkX:false,paddingTop:3}),{row:i,column:0});
}var s=new qx.ui.form.TextField();
var t=new qx.ui.form.PasswordField();
this.__sO.add(s.set({allowShrinkX:false,paddingTop:3}),{row:0,column:1});
this.__sO.add(t.set({allowShrinkX:false,paddingTop:3}),{row:1,column:1});
var u=this.__Nc=new qx.ui.form.Button(j);
u.setAllowStretchX(false);
this.__sO.add(u,{row:3,column:1});
u.addListener(c,this.checkInput,this);
this.__sO.addListener(f,this.__Nd,this);
}},destruct:function(){this._disposeObjects(l,h,g);
}});
})();
...
this.__uo=qx.fx.Base.EffectState.IDLE;
this.__kj=u;
},events:{"setup":s,"update":s,"finish":s},properties:{duration:{init:0.5,check:t,apply:g},fps:{init:100,check:t},sync:{init:false,check:o},from:{init:0,check:t},to:{init:1,check:t},delay:{init:0.0,check:t},queue:{check:i,dereference:true},transition:{init:q,check:[q,f,c,k,p,d,h,j,b,m,e]}},statics:{EffectState:{IDLE:'idle',PREPARING:'preparing',RUNNING:'running'}},members:{__uo:null,__up:null,__uq:null,__ur:null,__us:null,__ut:null,__uu:null,__uv:null,__kj:null,_getElement:function(){return this.__kj;
},_setElement:function(v){this.__kj=v;
},_applyDuration:function(w,x){},init:function(){this.__uo=qx.fx.Base.EffectState.PREPARING;
this.__up=0;
...
},start:function(){if(this.__uo!=qx.fx.Base.EffectState.IDLE){return false;
}this.init();
...
this.afterFinish();
},render:function(z){if(this.__uo==qx.fx.Base.EffectState.PREPARING){this.__uo=qx.fx.Base.EffectState.RUNNING;
...
if(this.__uo==qx.fx.Base.EffectState.RUNNING){this.__uv=qx.fx.Transition.get(this.getTransition())(z)*this.__us+this.getFrom();
...
},resetState:function(){this.__uo=qx.fx.Base.EffectState.IDLE;
},isActive:function(){return this.__uo===qx.fx.Base.EffectState.RUNNING||this.__uo===qx.fx.Base.EffectState.PREPARING;
}},destruct:function(){this.__kj=this.__uo=null;
}});
})();
(function(){var e="__default",d="qx.fx.queue.Manager",c="__dA",b="singleton",a="object";
qx.Class.define(d,{extend:qx.core.Object,type:b,construct:function(){qx.core.Object.call(this);
this.__dA={};
},members:{__dA:null,getQueue:function(f){if(typeof (this.__dA[f])==a){return this.__dA[f];
}else{return this.__dA[f]=new qx.fx.queue.Queue;
...
this.__uw=[];
},properties:{limit:{init:Infinity,check:a}},members:{__ux:null,__uw:null,add:function(c){var d=new Date().getTime();
c._startOn+=d;
c._finishOn+=d;

if(this.__uw.length<this.getLimit()){this.__uw.push(c);
}else{c.resetState();
}
if(!this.__ux){this.__ux=qx.lang.Function.periodical(this.loop,15,this);
}},remove:function(e){qx.lang.Array.remove(this.__uw,e);

if(this.__uw.length==0){window.clearInterval(this.__ux);
delete this.__ux;
}},loop:function(){var f=new Date().getTime();

for(var i=0,g=this.__uw.length;i<g;i++){this.__uw[i]&&this.__uw[i].loop(f);
...
(function(){var e="horizontal",d="vertical",c="__uw",b="qx.fx.effect.combination.Shake",a="Number";
qx.Class.define(b,{extend:qx.fx.Base,construct:function(f){qx.fx.Base.call(this,f);
this.__uw=[new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f)];
},properties:{direction:{init:e,check:[e,d]},duration:{init:0.5,refine:true},distance:{init:20,check:a}},members:{__uw:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var g=parseFloat(this.getDistance());
var h=parseFloat(this.getDuration())/10.0;

if(this.getDirection()==e){this.__uw[0].set({x:g,y:0,duration:h});
this.__uw[1].set({x:-g*2,y:0,duration:h*2});
this.__uw[2].set({x:g*2,y:0,duration:h*2});
this.__uw[3].set({x:-g*2,y:0,duration:h*2});
this.__uw[4].set({x:g*2,y:0,duration:h*2});
this.__uw[5].set({x:-g,y:0,duration:h*2});
}else if(this.getDirection()==d){this.__uw[0].set({y:g,x:0,duration:h});
this.__uw[1].set({y:-g*2,x:0,duration:h*2});
this.__uw[2].set({y:g*2,x:0,duration:h*2});
this.__uw[3].set({y:-g*2,x:0,duration:h*2});
this.__uw[4].set({y:g*2,x:0,duration:h*2});
this.__uw[5].set({y:-g,x:0,duration:h*2});
}var k=this.__uw;

for(var i=0,j=this.__uw.length;i<j;i++){this.__uw[i].id=i;

if(i<5){this.__uw[i].afterFinishInternal=function(){k[this.id+1].start();
};
}}this.__uw[0].start();
...
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:g,check:[g,e]},x:{init:0,check:f},y:{init:0,check:f}},members:{__uy:null,__uz:null,__uA:null,__uB:null,__uC:null,setup:function(){var j=this._getElement();
qx.fx.Base.prototype.setup.call(this);

if(j.parentNode){this.__uA=qx.bom.element.Location.getLeft(j)-qx.bom.element.Location.getLeft(j.parentNode);
this.__uB=qx.bom.element.Location.getTop(j)-qx.bom.element.Location.getTop(j.parentNode);
}else{this.__uA=qx.bom.element.Location.getLeft(j);
this.__uB=qx.bom.element.Location.getTop(j);
}this.__uC=qx.bom.element.Style.get(j,i);
qx.bom.element.Style.set(j,i,e);

if(this.getMode()==b){this.__uy=this.getX()-this.__uA;
this.__uz=this.getY()-this.__uB;
}else{this.__uy=this.getX();
this.__uz=this.getY();
}},update:function(k){var m=this._getElement();
qx.fx.Base.prototype.update.call(this);
var l=Math.round(this.__uy*k+this.__uA);
var top=Math.round(this.__uz*k+this.__uB);
qx.bom.element.Style.set(m,c,l+h);
qx.bom.element.Style.set(m,d,top+h);
},afterFinishInternal:function(){qx.bom.element.Style.set(this._getElement(),i,this.__uC);
}}});
})();
