...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.FormValidator","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormValidator-qx.theme.Classic.05c385b72b29.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var o="password",n="Send",m="All fields are required. Some by a custom validator, some by a ",l="complete",k="Please enter a password at with least 3 characters.",j="username",i="Server said no!",h="Passwords must be equal.",g="<b>Client side form validation</b><br/>",f="Validating...",c="email address",e="demobrowser.demo.ui.FormValidator",d="Accept",b="execute",a="predefined validator and some by the required flag.";
...
p.setWidth(150);
p.setRequired(true);
this.getRoot().add(p,{left:20,top:70});
var y=new qx.ui.form.TextField();
y.setPlaceholder(c);
y.setWidth(150);
this.getRoot().add(y,{left:20,top:100});
var q=new qx.ui.form.TextField();
q.setPlaceholder(o);
q.setWidth(150);
this.getRoot().add(q,{left:20,top:130});
var r=new qx.ui.form.TextField();
r.setPlaceholder(o);
r.setWidth(150);
this.getRoot().add(r,{left:20,top:160});
var x=new qx.ui.form.CheckBox(d);
x.setRequired(true);
this.getRoot().add(x,{left:20,top:190});
var u=new qx.ui.form.validation.Manager();
var s=function(z,A){var B=z!=null&&z.length>2;

if(!B){A.setInvalidMessage(k);
}return B;
};
var v=new qx.ui.form.validation.AsyncValidator(function(C,D){window.setTimeout(function(){if(D==null||D.length==0){C.setValid(false,i);
}else{C.setValid(true);
}},1000);
});
u.add(p,v);
u.add(y,qx.util.Validate.email());
u.add(q,s);
u.add(r,s);
u.add(x);
u.setValidator(function(E){var G=q.getValue()==r.getValue();

if(!G){var F=h;
q.setInvalidMessage(F);
r.setInvalidMessage(F);
q.setValid(false);
r.setValid(false);
}return G;
});
var t=new qx.ui.form.Button(n);
this.getRoot().add(t,{left:20,top:215});
t.addListener(b,function(){t.setEnabled(false);
t.setLabel(f);
u.validate();
},this);
u.addListener(l,function(){t.setEnabled(true);
t.setLabel(n);
if(u.getValid()){alert("You can send...");
}else{alert(u.getInvalidMessages().join("\n"));
}},this);
var w=new qx.ui.basic.Label();
w.setRich(true);
w.setWidth(320);
w.setValue(g+m+a);
this.getRoot().add(w,{left:20,top:10});
}}});
})();
