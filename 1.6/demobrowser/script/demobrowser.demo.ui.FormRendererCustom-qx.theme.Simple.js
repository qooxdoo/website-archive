...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormRendererCustom-qx.theme.Simple.d06251a3a2b0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
qx.Class.define(r,{extend:qx.ui.form.renderer.Single,members:{__OD:0,__OE:0,addItems:function(C,D,E,F){var I=0;
if(E!=null){this._add(this._createHeader(E),{row:I,column:this.__OD,colSpan:2});
I++;
}for(var i=0;i<C.length;i++){var H=this._createLabel(D[i],C[i]);
this._add(H,{row:I,column:this.__OD});
var G=C[i];
H.setBuddy(G);

if(G instanceof qx.ui.form.RadioGroup){G=this._createWidgetForRadioGroup(G);
}this._add(G,{row:I,column:this.__OD+1});
if(F[i]&&F[i].placeholder){G.setPlaceholder(F[i].placeholder);
}I++;
}this.__OD+=2;
this.__OE<I?this.__OE=I:null;
...
this._add(this._buttonRow,{row:this.__OE,column:0,colSpan:this.__OD});
