...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.progressive.ProgressiveTable_Conditional","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.progressive.ProgressiveTable_Conditional-qx.theme.Classic.687c66deaaf4.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var o="",n="==",m=">",l="between",k="regex",j="<",h="!between",g=">=",f="!=",e="<=",b='g',d="qx.ui.progressive.renderer.table.cell.Conditional",c=":",a=";";
...
this.__CB=r||"";
this.__CC=s||"";
},members:{__Jr:null,__Jt:null,__Js:null,__Cz:null,__CA:null,__CB:null,__CC:null,__CD:function(t,u){if(t.align){u["text-align"]=t.align;
}
...
}},addNumericCondition:function(v,w,x,y,z,A,B){if(!qx.lang.Array.contains(this.__Jr,v)||w==null){throw new Error("Condition not recognized or value is null!");
}this.__Js.push({condition:v,align:x,color:y,style:z,weight:A,value1:w,target:B});
},addBetweenCondition:function(C,D,E,F,G,H,I,J){if(!qx.lang.Array.contains(this.__Jt,C)||D==null||E==null){throw new Error("Condition not recognized or value1/value2 is null!");
}this.__Js.push({condition:C,align:F,color:G,style:H,weight:I,value1:D,value2:E,target:J});
},addRegex:function(K,L,M,N,O,P){if(!K){throw new Error("regex cannot be null!");
}this.__Js.push({condition:k,align:L,color:M,style:N,weight:O,regex:K,target:P});
},_getCellStyle:function(Q){if(this.__Js.length==0){return Q.style||o;
}var i;
var U;
var R;
var T={"text-align":this.__Cz,"color":this.__CA,"font-style":this.__CB,"font-weight":this.__CC};

for(i=0;i<this.__Js.length;i++){var X=this.__Js[i];
U=false;

if(qx.lang.Array.contains(this.__Jr,X.condition)){if(X.target==null){R=Q.cellData;
}else{R=Q.element.data[X.target];
}
switch(X.condition){case n:if(R==X.value1){U=true;
}break;
case f:if(R!=X.value1){U=true;
}break;
case m:if(R>X.value1){U=true;
}break;
case j:if(R<X.value1){U=true;
}break;
case g:if(R>=X.value1){U=true;
}break;
case e:if(R<=X.value1){U=true;
}break;
}}else if(qx.lang.Array.contains(this.__Jt,X.condition)){if(X.target==null){R=Q.cellData;
}else{R=Q.element.data[X.target];
}
switch(X.condition){case l:if(R>=X.value1&&R<=X.value2){U=true;
}break;
case h:if(R<X.value1&&R>X.value2){U=true;
}break;
}}else if(X.condition==k){if(X.target==null){R=Q.cellData;
}else{R=Q.element.data[X.target];
}var S=new RegExp(X.value1,b);
U=S.test(R);
}if(U){this.__CD(X,T);
}var V=[];

for(var W in T){if(T[W]){V.push(W,c,T[W],a);
}}}return V.join(o);
}},destruct:function(){this.__Jr=this.__Jt=this.__Js=null;
}});
})();
