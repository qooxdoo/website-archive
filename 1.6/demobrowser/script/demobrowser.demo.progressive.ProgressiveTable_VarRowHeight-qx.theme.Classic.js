...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.progressive.ProgressiveTable_VarRowHeight","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.progressive.ProgressiveTable_VarRowHeight-qx.theme.Classic.ed5cde4b49d1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
qx.Class.define(d,{extend:qx.application.Standalone,members:{__Ob:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var A=p;
this.imageData=[{url:A+k,height:4,tooltip:u},{url:A+b,height:4,tooltip:n},o];
this.__Ob=0;
...
}w=y.resolve(this.imageData[i].url);
z=x.toUri(w);
this.imageData[i].url=z;
++this.__Ob;
qx.io.ImageLoader.load(z,function(B,C){this.info("Searching for ["+B+"]");
for(var j=0;j<this.imageData.length;j++){if(this.imageData[j].url==B){this.imageData[j].height=C.height;
--this.__Ob;
this.debug("Found ["+this.imageData[j].url+"].  loadCount="+this.__Ob+", end="+this.imageData.length);
if(this.__Ob==0){this.doLoad();
}return;
}}throw new Error("Image data for "+B+" not found");
},this);
}if(this.__Ob==0){this.doLoad();
}},doLoad:function(){var E=this;
var L=function(P,Q){var R={renderer:v,location:c,data:[Q,E.imageData[P]]};
return R;
};
var K=new qx.ui.progressive.renderer.table.Widths(2);
K.setWidth(0,200);
K.setWidth(1,q);
var G=[e,h];
var H=new qx.ui.progressive.headfoot.TableHeading(K,G);
var F=new qx.ui.progressive.headfoot.Progress(K,G);
var O=new qx.ui.progressive.structure.Default(H,F);
var D=new qx.ui.progressive.Progressive(O);
var M=new qx.ui.basic.Atom(a);
M.setHeight(16);
M.setTextColor(l);
D.add(M);
...
J.addElements(I);
D.setDataModel(J);
var N=new qx.ui.progressive.renderer.table.Row(K);
D.addRenderer(v,N);
var r=new qx.ui.progressive.renderer.table.cell.Image();
N.addRenderer(1,r);
D.set({width:400,maxWidth:400});
this.getRoot().add(D,{left:50,top:50,bottom:50});
D.render();
}}});
})();
...
for(var i=0;i<e;i++){this.__Ke[i]=new qx.ui.core.ColumnData();
}},members:{__Ke:null,getData:function(){return this.__Ke;
...
}}},setWidth:function(j,k){if(j>this.__Ke.length-1||j<0){throw new Error("Column number out of range");
}this.__Ke[j].setColumnWidth(k);
},setMinWidth:function(l,m){if(l>this.__Ke.length-1||l<0){throw new Error("Column number out of range");
}this.__Ke[l].setMinWidth(m);
},setMaxWidth:function(n,o){if(n>this.__Ke.length-1||n<0){throw new Error("Column number out of range");
}this.__Ke[n].setMaxWidth(o);
}}});
})();
...
},members:{__Hs:null,join:function(c){this.__Hs=c;
...
},properties:{appearance:{refine:true,init:c}},members:{__Ht:null,__Hu:null,__vG:null,__Be:null,join:function(m){qx.ui.progressive.headfoot.Abstract.prototype.join.call(this,m);
},getLayoutChildren:function(){if(this.__Hu){return this.__Ht.getData();
...
this.__Hu=true;
this.__Be.renderLayout(n,100);
this.__Hu=false;
var o=this.__Ht.getData();
for(var i=0;i<o.length;i++){this.__vG[i].setWidth(o[i].getComputedWidth());
}}},destruct:function(){this.__Ht=this.__vG=null;
this._disposeObjects(b);
}});
})();
(function(){var m="progressive-progressbar-indicator-undone",l="progress",k="%",j="progressive-progressbar-indicator-done",i="__IE",h="progressive-progressbar-percent-text",g="renderStart",f="__IF",d="progressive-progressbar-background",c="qx.ui.progressive.headfoot.Progress",a="progressive-progressbar-percent-background",b="renderEnd";
...
this.__Iu.background=p.resolve(d);
this.__Iu.indicatorDone=p.resolve(j);
this.__Iu.indicatorUndone=p.resolve(m);
this.__Iu.percentBackground=p.resolve(a);
this.__Iu.percentText=p.resolve(h);
this.set({backgroundColor:this.__Iu.background});
this.__IE=new qx.ui.core.Widget();
this.__IE.set({width:0,backgroundColor:this.__Iu.indicatorDone});
this.add(this.__IE);
var q=new qx.ui.core.Widget();
q.set({backgroundColor:this.__Iu.indicatorUndone});
this.add(q,{flex:1});
this.__IF=new qx.ui.basic.Atom("0%");
this.__IF.set({width:100,backgroundColor:this.__Iu.percentBackground,textColor:this.__Iu.percentText});
this.add(this.__IF);
this.exclude();
},members:{__IG:null,__Iu:null,__IE:null,__IF:null,join:function(r){qx.ui.progressive.headfoot.Abstract.prototype.join.call(this,r);
r.addListener(g,function(e){this.__IG=e.getData().initial;
this.show();
},this);
r.addListener(l,function(e){var u=1.0-(e.getData().remaining/this.__IG);
var v=this.getBounds();

if(v){var s=Math.floor((v.width-this.__IF.getBounds().width)*u);
var t=Math.floor(u*100)+k;

if(!isNaN(s)){this.__IE.setMinWidth(s);
this.__IF.setLabel(t);
}}},this);
...
if(!e){this.__sO=new qx.ui.core.Widget();
this.__wP=this.__sO;
}else{this.__sO=null;
this.__wP=e;
}this.__wP.getContentElement().setStyle(b,a);
},members:{__sO:null,__wP:null,applyStructure:function(f){throw new Error("applyStructure() is abstract");
},getPane:function(){return this.__wP;
}},destruct:function(){if(this.__sO){this.__sO.dispose();
}this.__sO=this.__wP=null;
}});
})();
...
if(!b){this.__Jo=new qx.ui.progressive.headfoot.Null();
this.__pw=this.__Jo;
}else{this.__Jo=null;
this.__pw=b;
}if(!c){this.__Jp=new qx.ui.progressive.headfoot.Null();
this.__Jq=this.__Jp;
}else{this.__Jp=null;
this.__Jq=c;
}},members:{__pw:null,__Jq:null,__Jo:null,__Jp:null,applyStructure:function(e){this.__pw.join(e);
this.__Jq.join(e);
e.add(this.__pw);
e.add(this.getPane(),{flex:1});
e.add(this.__Jq);
},getHeader:function(){return this.__pw;
},getFooter:function(){return this.__Jq;
}},destruct:function(){if(this.__Jo){this.__Jo.dispose();
this.__Jo=null;
}
if(this.__Jp){this.__Jp.dispose();
this.__Jp=null;
}this.__pw=this.__Jq=null;
}});
})();
...
this.__Ji=false;
this.__Jj=0;
},events:{"renderStart":n,"renderEnd":n,"progress":n,"progressDetail":n},properties:{dataModel:{check:g,apply:j},batchSize:{check:k,init:20},flushWidgetQueueAfterBatch:{check:h,init:false},interElementTimeout:{check:k,init:0}},members:{__vH:null,__Ji:null,__Jk:null,__Jj:null,__Jh:null,__Jg:null,getStructure:function(){return this.__Jg;
},addRenderer:function(name,p){this.__vH[name]=p;
p.join(this,name);
},removeRenderer:function(name){if(!this.__vH[name]){throw new Error("No existing renderer named "+name);
}delete this.__vH[name];
},render:function(){if(this.__Ji){return;
}this.__Ji=true;
var q=new qx.ui.progressive.State({progressive:this,model:this.getDataModel(),pane:this.__Jg.getPane(),batchSize:this.getBatchSize(),rendererData:this.__Jm(),userData:{}});
this.__Jk=new Date();
if(this.__Jh){this.__Jj=q.getModel().getElementCount();
this.fireDataEvent(l,{state:q,initial:this.__Jj});
this.__Jl(q);
}else{qx.event.Timer.once(function(){this.__Jj=q.getModel().getElementCount();
this.fireDataEvent(l,{state:q,initial:this.__Jj});
this.__Jl(q);
this.__Jh=true;
},this,10);
}},_applyDataModel:function(r,s){if(s){s.removeListener(m,this.__Jn,this);
s.dispose();
}r.addListener(m,this.__Jn,this);
},__Jl:function(t){var u;
var w;
var v;

for(var i=t.getBatchSize();i>0;i--){u=t.getModel().getNextElement();

if(!u){this.debug("Render time: "+(new Date()-this.__Jk)+"ms");
this.__Ji=false;
this.fireDataEvent(b,t);
t.dispose();
return ;
}w=u.element;
v=this.__vH[w.renderer];
v.render(t,w);
this.fireDataEvent(a,{initial:this.__Jj,remaining:u.remaining,element:w});
}this.fireDataEvent(f,{initial:this.__Jj,remaining:u.remaining});
if(this.getFlushWidgetQueueAfterBatch()){qx.ui.core.queue.Manager.flush();
}qx.event.Timer.once(function(){this.__Jl(t);
},this,this.getInterElementTimeout());
},__Jm:function(){var x={};

for(var name in this.__vH){x[name]={};
}return x;
},__Jn:function(e){this.__Jj=e.getData();
this.render();
}},destruct:function(){for(var name in this.__vH){this.__vH[name].dispose();
}this.__Jk=this.__vH=this.__Jg=null;
}});
})();
...
this.__vV=[];
},members:{__vV:null,addElements:function(c){this.__vV=this.__vV.concat(c);
this.fireDataEvent(b,this.__vV.length);
},addElement:function(d){this.__vV.push(d);
this.fireDataEvent(b,this.__vV.length);
},getElementCount:function(){return this.__vV.length;
},getNextElement:function(){if(this.__vV.length>0){return ({element:this.__vV.shift(),remaining:this.__vV.length});
}return null;
...
(function(){var l=".qx-progressive-",k="-col-",j="px;",h="-row",g="css.userselect",f="css.textoverflow",d="}",c="",b="width: ",a="0px ",Y="qx-progressive-",X="px",W="  font-size: 11px;",V="content",U="px ",T=';',S='',R="padding: ",Q="end",P="div",s="__Be",t="  position: absolute;",q="qx.tableResizeDebug",r="  width : 100%;",o="__columnData",p="css.boxmodel",m="  top: 0px;",n="qx.debug",v="  overflow:hidden;",w=" {",D="  border-right:1px solid #f2f2f2;",B="  cursor:default;",H="  font-family: 'Segoe UI', Corbel, Calibri, Tahoma, 'Lucida Sans Unicode', sans-serif;",F="-row {",L="resize",J="start",y="relative",O=':ellipsis;',N=':',M="  white-space:nowrap;",x="  padding : 0px 6px 0px 6px;",z="qx.ui.progressive.renderer.table.Row",A="progressive-table-row-background-odd",C="  height: 100%;",E="progressive-table-row-background-even",G="__GW",I="  border-bottom:1px solid #eeeeee;",K="left: ",u="css.userselect.none";
qx.Class.define(z,{extend:qx.ui.progressive.renderer.Abstract,construct:function(ba){qx.ui.progressive.renderer.Abstract.call(this);
this.__Ht=ba;
this.__It={};
this.__GW=new qx.ui.progressive.renderer.table.cell.Default();
this.__Hs=null;
this.__Iu={};
var bb=qx.theme.manager.Color.getInstance();
this.__Iu.bgcol=[];
this.__Iu.bgcol[0]=bb.resolve(E);
this.__Iu.bgcol[1]=bb.resolve(A);
this.__Be=new qx.ui.layout.HBox();
this.__Be.connectToWidget(this);
},statics:{__zQ:null,__Iv:6,__Iw:t+m+C+v+(qx.core.Environment.get(f)?qx.lang.String.hyphenate(qx.core.Environment.get(f))+O:c)+M+D+I+x+B+W+H+(qx.core.Environment.get(g)?qx.lang.String.hyphenate(qx.core.Environment.get(g))+N+qx.core.Environment.get(u)+T:S)},properties:{defaultRowHeight:{init:16}},members:{__Hs:null,__ml:null,__Ix:null,__Ht:null,__It:null,__GW:null,__Iu:null,__Be:null,join:function(bc,name){if(this.__Hs){throw new Error("Renderer is already joined to a Progressive.");
}this.__Hs=bc;
this.__ml=name;
var be=qx.ui.progressive.renderer.table.Row;

if(!be.__zQ){be.__zQ={};
}var bd=bc.toHashCode();

if(!be.__zQ[bd]){be.__zQ[bd]={rowstylesheet:null,cellstylesheet:[]};
var bf=l+bd+F+r+d;
be.__zQ[bd].rowstylesheet=qx.bom.Stylesheet.createElement(bf);
var bg=this.__Ht.getData();

for(var i=0;i<bg.length;i++){var bf=l+bd+k+i+w+be.__Iw+d;
be.__zQ[bd].cellstylesheet[i]=qx.bom.Stylesheet.createElement(bf);
}this.__Ix=bd;
var bh=bc.getStructure().getPane();
bh.addListener(L,this._resizeColumns,this);
}},addRenderer:function(bi,bj){var bk=this.__Ht.getData();

if(bi<0||bi>=bk.length){throw new Error("Column "+bi+" out of range (max: "+(bk.length-1)+")");
}this.__It[bi]=bj;
},removeRenderer:function(bl){var bm=this.__Ht.getData();

if(bl<0||bl>=bm.length){throw new Error("Column "+bl+" out of range (max: "+(bm.length-1)+")");
}
if(!this.__It[bl]){throw new Error("No existing renderer for column "+bl);
}delete this.__It[bl];
},render:function(bn,bo){var by=bo.data;
var bt=[];
var bv;
var bu;
var br=0;
if(bn.getRendererData()[this.__ml].end===undefined){bn.getRendererData()[this.__ml]={end:0,start:1,rows:0,totalHeight:0};
}var bx=document.createElement(P);
for(var i=0;i<by.length;i++){var bs=Y+this.__Ix+k+i;
bu=this.__It[i]||this.__GW;
bv={state:bn,rowDiv:bx,stylesheet:bs,element:bo,dataIndex:i,cellData:by[i],height:br};
bt.push(bu.render(bv));
if(bv.height>br){br=bv.height;
}}br=(br>0?br:this.getDefaultRowHeight());
var bw=bn.getRendererData()[this.__ml];
bw.totalHeight+=br;
bx.style.position=y;
bx.style.height=br+X;
bx.className=Y+this.__Ix+h;
bx.innerHTML=bt.join(c);
switch(bo.location){case Q:var bp=bw.end||0;
bx.style.backgroundColor=this.__Iu.bgcol[bp];
bw.end=(bp==0?1:0);
bn.getPane().getContentElement().getDomElement().appendChild(bx);
break;
case J:var bq=bn.getPane().getContentElement().getDomElement();
var bz=bq.childNodes;
if(bz.length>0){var bp=bw.start;
bx.style.backgroundColor=this.__Iu.bgcol[bp];
bw.start=(bp==0?1:0);
bq.insertBefore(bx,bz[0]);
break;
}else{bq.appendChild(bx);
}break;
default:throw new Error("Invalid location: "+bo.location);
}++bw.rows;
},getLayoutChildren:function(){return this.__Ht.getData();
},_resizeColumns:function(e){var bH=this.__Hs.getStructure().getPane();
var bE=bH.getBounds().width-qx.bom.element.Overflow.getScrollbarWidth();
var bF=l+this.__Ix+h;
var bB=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bB.__zQ[this.__Ix].rowstylesheet,bF);
var bD=b+bE+j;
qx.bom.Stylesheet.addRule(bB.__zQ[this.__Ix].rowstylesheet,bF,bD);
this.__Be.renderLayout(bE,100);
var bG=this.__Ht.getData();
for(var i=0,bK=0;i<bG.length;i++,bK+=bE){var bF=l+this.__Ix+k+i;
var bB=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bB.__zQ[this.__Ix].cellstylesheet[i],bF);
bE=bG[i].getComputedWidth();

if(qx.core.Environment.get(n)){if(qx.core.Environment.get(q)){this.debug("col "+i+": width="+bE);
}}var bC;

if(qx.core.Environment.get(p)==V){bC=qx.ui.progressive.renderer.table.Row.__Iv*2;
}else{bC=-1;
}var bJ=(bE-bC)+j;
var bI=a+qx.ui.progressive.renderer.table.Row.__Iv+U+a+qx.ui.progressive.renderer.table.Row.__Iv+j;
var bA=bK+j;
var bD=bB.__Iw+b+bJ+K+bA+R+bI;
qx.bom.Stylesheet.addRule(bB.__zQ[this.__Ix].cellstylesheet[i],bF,bD);
}}},destruct:function(){var name;

for(name in this.__It){this.__It[name]=null;
}var bM=qx.ui.progressive.renderer.table.Row;
var bL=this.__Hs.toHashCode();

if(bM.__zQ&&bM.__zQ[bL]){if(bM.__zQ[bL].rowstylesheet){var bO=l+this.__Ix+h;
var bM=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bM.__zQ[this.__Ix].rowstylesheet,bO);
}if(bM.__zQ[bL].cellstylesheet){for(var i=bM.__zQ[bL].cellstylesheet.length-1;i>=0;i--){var bO=l+this.__Ix+k+i;
var bN=bM.__zQ[this.__Ix].cellstylesheet[i];
var bM=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bN,bO);
}}}
if(this.__Hs&&this.__Hs.getRendererData){var bP=this.__Hs.getRendererData();

if(bP&&bP[this.__ml]&&bP[this.__ml].end!==undefined){bP[this.__ml]=null;
}}this.__Iu=this.__It=this.__Hs=this.__Ht=null;
this._disposeObjects(s,G,o);
}});
})();
...
(function(){var t="px'",s='" ',r='src="',q=" height='",p="vertical-align:middle;",o="text-align:center;",n="abstract",m="",l='<img ',k='" style="filter:',d=" title='",j=" width='",g="qx.ui.progressive.renderer.table.cell.Icon",c="'",b=" style='padding-top:2px;'",f=">",e="progid:DXImageTransform.Microsoft.AlphaImageLoader(src='",h="qx/static/blank.gif",a="string",i="',sizingMethod='scale')";
qx.Class.define(g,{type:n,extend:qx.ui.progressive.renderer.table.cell.Abstract,construct:function(){qx.ui.progressive.renderer.table.cell.Abstract.call(this);
var u=qx.util.AliasManager.getInstance();
var v=qx.util.ResourceManager.getInstance();
var w=u.resolve(h);
this.__Hf=v.toUri(w);
},members:{__Hf:null,getBlankImage:function(){return this.__Hf;
},_identifyImage:function(x){throw new Error("_identifyImage() is abstract");
},_getCellStyle:function(y){var z=qx.ui.progressive.renderer.table.cell.Abstract.prototype._getCellStyle.call(this,y)+o+p;
return z;
},_getContentHtml:function(A){var B=[];
var C=this.__Hg(A);
B.push(l);
if(qx.bom.element.Decoration.isAlphaImageLoaderEnabled()&&/\.png$/i.test(C.url)){B.push(r,this.__Hf,k,e,C.url,i,s);
}else{B.push(r,C.url,s);
}if(C.imageWidth){B.push(j,C.imageWidth,t);
}if(C.imageHeight){B.push(q,C.imageHeight,t);
}B.push(b);
if(C.tooltip){B.push(d,C.tooltip,c);
}if(C.extras){B.push(C.extras);
}B.push(f);
return B.join(m);
},__Hg:function(D){var E=this._identifyImage(D);
if(E==null||typeof E==a){E={url:E,tooltip:null};
}if(E.url==null){E.url=this.__Hf;
}return E;
}}});
})();
...
if(d===undefined){this.__Hh=d;
}else{this.__Hh=16;
}
if(e===undefined){this.__Hi=e;
}else{this.__Hi=16;
}},members:{__Hh:null,__Hi:null,_identifyImage:function(f){var j={imageWidth:this.__Hh,imageHeight:this.__Hi};
var k;
