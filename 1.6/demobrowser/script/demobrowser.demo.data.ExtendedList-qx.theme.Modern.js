...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.ExtendedList","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ExtendedList-qx.theme.Modern.64337306e4ff.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
for(var i=0;i<20;i++){var F=new demobrowser.demo.data.model.Person();
F.setName(K[i%K.length]);
F.setEmote(D[i%D.length]);
F.setOnline(i%3==0);
I.push(F);
}var J=new qx.data.Array(I);
var C=new qx.ui.form.List();
C.setWidth(150);
this.getRoot().add(C,{left:10,top:80});
var G=new qx.data.controller.List(null,C);
var B={configureItem:function(L){L.setPadding(3);
},createItem:function(){return new qx.ui.form.CheckBox();
},bindItem:function(M,N,O){M.bindProperty(x,o,null,N,O);
M.bindProperty(h,j,null,N,O);
M.bindPropertyReverse(h,j,null,N,O);
}};
G.setDelegate(B);
G.setModel(J);
var A=new qx.ui.form.Button(r);
A.setWidth(120);
this.getRoot().add(A,{left:180,top:80});
A.addListener(k,function(){for(var i=0;i<J.length;i++){J.getItem(i).toggleOnline();
}},this);
...
},_applyIconOptions:function(F,G){this.__ob();
},_applyLabelOptions:function(H,I){this.__ob();
},_applyIconPath:function(J,K){this.__ob();
},_applyLabelPath:function(L,M){this.__ob();
},_applyModel:function(N,O){if(O!=undefined){if(this.__dv!=undefined){O.removeListenerById(this.__dv);
}}if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();
}if(N!=null){this.__dv=N.addListener(k,this.__nV,this);
this.__od();
this.__nW();
...
for(var i=0;i<length;i++){this.__oa();
...
if(Q!=null){if(this.getModel()!=null){for(var i=0;i<this.__nP.length;i++){this.__nY(this.__oe(i));
}}}},__nV:function(){this.__nU=true;
qx.ui.core.queue.Widget.add(this);
if(this.__nP.length!=this.getModel().getLength()){this.update();
}},syncWidget:function(){if(this.__nT){this._changeTargetSelection();
}
if(this.__nU){this._updateSelection();
}this.__nU=this.__nT=null;
},__nW:function(){if(this.getTarget()==null){return;
}this.__od();
var U=this.__nP.length;
var T=this.getTarget().getChildren().length;
if(U>T){for(var j=T;j<U;j++){this.__nY(this.__oe(j));
}}else if(U<T){for(var j=T;j>U;j--){this.__oa();
}}this.__nT=true;
qx.ui.core.queue.Widget.add(this);
},__nX:function(){var V=this.getModel();
if(V!=null){V.removeListenerById(this.__dv);
this.__dv=V.addListener(k,this.__nV,this);
...
},__nY:function(Y){var ba=this._createItem();
this._bindListItem(ba,Y);
this.getTarget().add(ba);
},__oa:function(){this._startSelectionModification();
...
if(!qx.lang.Array.contains(this.__nQ,bn)){this.__nQ.push(bn);
...
if(!qx.lang.Array.contains(this.__nR,bt)){this.__nR.push(bt);
}},_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;
}for(var i=0;i<this.__nQ.length;i++){if(this.__nS[this.__nQ[i]]!=null){this.__nS[this.__nQ[i]]();
}}},_removeBindingsFrom:function(bD){for(var i=0;i<this.__nQ.length;i++){var bE=bD.getUserData(this.__nQ[i]+g);

if(bE!=null){this.removeBinding(bE);
}}for(var i=0;i<this.__nR.length;i++){var bE=bD.getUserData(this.__nR[i]+h);

if(bE!=null){bD.removeBinding(bE);
}}},__ob:function(){if(this.getTarget()==null||this.getModel()==null){return;
}var bF=this.getTarget().getChildren();
for(var i=0;i<bF.length;i++){this._removeBindingsFrom(bF[i]);
this._bindListItem(bF[i],this.__oe(i));
}this.__nX();
...
this._endSelectionModification();
this._updateSelection();
},_setFilter:function(bP,bQ){if((bP==null||bP.filter==null)&&(bQ!=null&&bQ.filter!=null)){this.__oc();
...
this.__od();
if(bS.length>this.__nP.length){for(var j=bS.length;j>this.__nP.length;j--){this.getTarget().removeAt(j-1).destroy();
}}else if(bS.length<this.__nP.length){for(var j=bS.length;j<this.__nP.length;j++){var bT=this._createItem();
this.getTarget().add(bT);
}}var bR=this.getTarget().getChildren();

for(var i=0;i<bR.length;i++){this._bindListItem(bR[i],this.__oe(i));
}this.__nX();
this._endSelectionModification();
this._updateSelection();
},__oc:function(){this.__od();
this.__nW();
this.__ob();
this.__nU=true;
qx.ui.core.queue.Widget.add(this);
},__od:function(){var bW=this.getModel();
...
for(var i=0;i<bW.getLength();i++){if(bV==null||bV(bW.getItem(i))){this.__nP.push(i);
}}},__oe:function(bY){return this.__nP[bY];
}},destruct:function(){this.__nP=this.__nS=this.__nQ=null;
this.__nR=null;
qx.ui.core.queue.Widget.remove(this);
}});
})();
