...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.Offline","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Offline-qx.theme.Simple.cef772759eef.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="model",l="item ",k="execute",j="demobrowser.demo.data.Offline",i="html.storage.local",h="Remove item",g="The added data will be keept on reload. Just add or remove strings ",f="and reload to see the result.",e="Add item",d="initial entry",a="Offline storage (local) not supported.",c="qx-offline-demo",b="<b>Offline data store</b><br/>";
...
if(!qx.core.Environment.get(i)){var q=new qx.ui.basic.Label(a);
this.getRoot().add(q,{left:30,top:30});
return;
...
content.add(p,{row:0,column:0,rowSpan:2});
var s=new qx.data.controller.List(null,p);
var w=new qx.data.store.Offline(c);
if(w.getModel()==null){var v=qx.data.marshal.Json.createModel([d]);
w.setModel(v);
}w.bind(m,s,m);
var n=new qx.ui.form.Button(e);
var u=new qx.ui.form.Button(h);
var r=new qx.ui.form.TextField();
r.setValue(l+w.getModel().length);
content.add(r,{row:0,column:1,rowSpan:2});
content.add(n,{row:0,column:2});
content.add(u,{row:1,column:2});
n.setAllowGrowY(false);
u.setAllowGrowY(false);
t.setRowFlex(1,1);
n.addListener(k,function(){w.getModel().push(r.getValue());
r.setValue(l+w.getModel().length);
},this);
u.addListener(k,function(){w.getModel().remove(s.getSelection().getItem(0));
},this);
...
(function(){var g="qx.data.store.Offline",f="session",d="qx.debug",c="changeModel",b="changeBubble",a="_applyModel";
qx.Class.define(g,{extend:qx.core.Object,construct:function(h,i){qx.core.Object.call(this);

try{if(qx.core.Environment.get(d)){this.assertNotUndefined(h);
}}catch(e){this.dispose();
throw e;
}
if(i==f){this._storage=qx.bom.storage.Session.getInstance();
}else{this._storage=qx.bom.storage.Local.getInstance();
}this._marshaler=new qx.data.marshal.Json();
this._key=h;
this._initializeModel();
},properties:{model:{nullable:true,event:c,apply:a}},members:{_storage:null,__dg:null,_applyModel:function(j,k){if(k){k.removeListenerById(this.__dg);
k.dispose();
this.__dg=null;
}
if(j){this.__dg=j.addListener(b,this.__dh,this);
this.__dh();
}},__dh:function(){var l=qx.util.Serializer.toNativeObject(this.getModel());
this._storage.setItem(this._key,l);
},_initializeModel:function(){this._setModel(this._storage.getItem(this._key));
},_setModel:function(m){this._marshaler.toClass(m,true);
