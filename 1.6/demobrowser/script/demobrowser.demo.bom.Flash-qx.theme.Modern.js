...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.bom.Flash","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Flash-qx.theme.Modern.5389eaf0c99a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="engine.name",j="object",h='</object>',g="100%",f="movie",e="beforeunload",d="=",c="Invalid parameter 'element'.",b="function",a="application/x-shockwave-flash",F='<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">',E="param",D="Invalid parameter 'attributes'.",C='" />',B="Invalid attribute 'movie'.",A="qx.bom.Flash",z='<param name="',y="qx.debug",x="type",w="load",r="name",s="data",p="Invalid attribute 'id'.",q="",n="mshtml",o="value",l="&",m='<object id="',t='" value="',u='" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">',v="undefined";
qx.Class.define(A,{statics:{_flashObjects:{},create:function(G,H,I,J,K){if(!K){K=window;
}if(qx.core.Environment.get(y)){qx.core.Assert.assertElement(G,c);
qx.core.Assert.assertMap(H,D);
qx.core.Assert.assertString(H.movie,B);
qx.core.Assert.assertString(H.id,p);

if(!qx.dom.Element.isInDom(G,K)){qx.log.Logger.warn(this,"The parent DOM element isn't in DOM! The External Interface doesn't work in IE!");
}}
if(!H.width){H.width=g;
}
if(!H.height){H.height=g;
}J=J?qx.lang.Object.clone(J):{};

if(!J[f]){J[f]=H.movie;
}H[s]=H.movie;
delete H.movie;
if(I){for(var name in I){if(typeof J.flashvars!=v){J.flashvars+=l+name+d+I[name];
}else{J.flashvars=name+d+I[name];
}}}var L=this.__Jf(G,H,J,K);
this._flashObjects[H.id]=L;
return L;
},destroy:qx.core.Environment.select(k,{"mshtml":function(M,N){M=this.__Ja(M);

if(M.readyState==4){this.__Jb(M);
}else{if(!N){N=window;
}qx.bom.Event.addNativeListener(N,w,function(){qx.bom.Flash.__Jb(M);
});
}},"default":function(O,P){O=this.__Ja(O);

if(O.parentNode){O.parentNode.removeChild(O);
}delete this._flashObjects[O.id];
}}),__Ja:function(Q){if(!Q){throw new Error("DOM element is null or undefined!");
}
if(Q.tagName.toLowerCase()!==j){Q=Q.firstChild;
}
if(!Q||Q.tagName.toLowerCase()!==j){throw new Error("DOM element has or is not a flash object!");
}return Q;
},__Jb:qx.core.Environment.select(k,{"mshtml":qx.event.GlobalError.observeMethod(function(R){for(var i in R){if(typeof R[i]==b){R[i]=null;
}}
if(R.parentNode){R.parentNode.removeChild(R);
}delete this._flashObjects[R.id];
}),"default":null}),__Jc:qx.event.GlobalError.observeMethod(function(){for(var S in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[S]);
}window.__Jd=function(){};
window.__Je=function(){};
qx.bom.Event.removeNativeListener(window,e,qx.bom.Flash.__Jc);
}),__Jf:qx.core.Environment.select(k,{"mshtml":function(T,U,V,W){V.movie=U.data;
delete U.data;
delete U.classid;
var X=q;

for(name in V){X+=z+name+t+V[name]+C;
}if(U.id){T.innerHTML=m+U.id+u+X+h;
delete U.id;
}else{T.innerHTML=F+X+h;
}for(var name in U){T.firstChild.setAttribute(name,U[name]);
}return T.firstChild;
},"default":function(Y,ba,bb,bc){delete ba.classid;
delete bb.movie;
var be=qx.bom.Element.create(j,ba,bc);
be.setAttribute(x,a);
var bd;

for(var name in bb){bd=qx.bom.Element.create(E,{},bc);
bd.setAttribute(r,name);
bd.setAttribute(o,bb[name]);
be.appendChild(bd);
}Y.appendChild(be);
return be;
}})},defer:function(bf){if((qx.core.Environment.get(k)==n)){qx.bom.Event.addNativeListener(window,e,bf.__Jc);
}}});
})();
