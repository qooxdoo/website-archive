...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.JsonToTree","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.JsonToTree-qx.theme.Classic.39241f333fd4.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="model",l="name",k="Loading...",j="tree.json</a> and bind the items to the tree widget.",i="Loading the json file <a href='",h="state",g="kids",f="demobrowser/demo/data/tree.json",e="value",d="demobrowser.demo.data.JsonToTree",a="' target='_blank'>",c="<b>Tree bound to data in a json file</b><br/>",b="loaded";
...
var p=new qx.ui.tree.Tree();
this.getRoot().add(p,{left:10,top:80});
p.setWidth(200);
p.setHeight(300);
var q=new qx.data.controller.Tree(null,p,g,l);
var o=qx.util.ResourceManager.getInstance().toUri(f);
var r=new qx.data.store.Json(o);
var status=new qx.ui.basic.Label(k);
this.getRoot().add(status,{left:220,top:80});
r.bind(m,q,m);
r.bind(h,status,e);
r.addListener(b,function(s){p.getRoot().setOpen(true);
},this);
var n=new qx.ui.basic.Label();
n.setRich(true);
n.setWidth(260);
n.setValue(c+i+o+a+j);
this.getRoot().add(n,{left:10,top:10});
}}});
})();
...
(function(){var m="qx.debug",k="visible",j="excluded",h="qx.ui.tree.core.AbstractTreeItem",g="open",f="abstract",e="__kv",d="__ri",c="addItem",b="removeItem",a="Invalid child index: ";
...
this.__kv=[];
},properties:{parent:{check:h,nullable:true}},members:{__kv:null,__ri:null,getTree:function(){var p=this;
...
}}},getChildrenContainer:function(){if(!this.__ri){this.__ri=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?k:j});
}return this.__ri;
...
}},addAt:function(Q,R){if(qx.core.Environment.get(m)){this.assert(R<=this.__kv.length&&R>=0,a+R);
}
if(R==this.__kv.length){this.add(Q);
return;
}var V=Q.getParent();
...
}qx.lang.Array.insertAt(this.__kv,Q,R);

if(!U){this.__rj();
}
if(this.getTree()){Q.recursiveAddToWidgetQueue();
qx.ui.core.queue.Widget.add(this);
}},addBefore:function(W,X){if(qx.core.Environment.get(m)){this.assert(this.__kv.indexOf(X)>=0);
}var Y=W.getParent();

if(Y){Y.remove(W);
}this.addAt(W,this.__kv.indexOf(X));
},addAfter:function(ba,bb){if(qx.core.Environment.get(m)){this.assert(this.__kv.indexOf(bb)>=0);
}var bc=ba.getParent();

if(bc){bc.remove(ba);
}this.addAt(ba,this.__kv.indexOf(bb)+1);
},addAtBegin:function(bd){this.addAt(bd,0);
},remove:function(be){for(var i=0,l=arguments.length;i<l;i++){var bi=arguments[i];

if(this.__kv.indexOf(bi)==-1){this.warn("Cannot remove treeitem '"+bi+"'. It is not a child of this tree item.");
return;
...
}}qx.lang.Array.remove(this.__kv,bi);
bi.setParent(null);
bf.remove(bi);
}var bg=this.getTree();
...
},removeAt:function(bj){var bk=this.__kv[bj];

if(bk){this.remove(bk);
}},removeAll:function(){var bl=this.__kv.concat();
...
}if(this.getSelection()==null){this.__ro=new qx.data.Array();
this.setSelection(this.__ro);
}},properties:{selection:{check:d,event:f,apply:b,init:null}},members:{_modifingSelection:0,__rp:null,__rq:null,__ro:null,_applySelection:function(g,h){if(this.__rq!=undefined&&h!=undefined){h.removeListenerById(this.__rq);
}this.__rq=g.addListener(e,this.__rr,this);
this._updateSelection();
},__rr:function(){this._updateSelection();
},_changeTargetSelection:function(){if(this.getTarget()==null){return;
}if(!this.__rs()&&!this.__rt()){return;
...
},_addChangeTargetListener:function(m,n){if(this.__rp!=undefined&&n!=undefined){n.removeListenerById(this.__rp);
}
if(m!=null){if(this.__rs()||this.__rt()){this.__rp=m.addListener(f,this._changeTargetSelection,this);
...
}}}else if(this.__rt()){var p=this.getSelection().getItem(this.getSelection().length-1);
...
},__rs:function(){var t=this.getTarget().constructor;
return qx.Class.implementsInterface(t,qx.ui.core.IMultiSelection);
},__rt:function(){var u=this.getTarget().constructor;
return qx.Class.implementsInterface(u,qx.ui.core.ISingleSelection);
},__ru:function(v){var w=this.__rv(v);
if(w==null){return;
}if(this.__rs()){this.getTarget().addToSelection(w);
}else if(this.__rt()){this.getTarget().setSelection([w]);
}},__rv:function(x){var y=this.getTarget().getSelectables(true);
...
}},destruct:function(){if(this.__ro){this.__ro.dispose();
}}});
})();
...
qx.Class.define(f,{extend:qx.core.Object,include:qx.data.controller.MSelection,construct:function(u,v,w,x){qx.core.Object.call(this);
this.__bg={};
this.__nQ=[];
this.__rw={};
...
}},properties:{model:{check:p,apply:d,event:r,nullable:true,dereference:true},target:{apply:k,event:c,init:null,nullable:true,dereference:true},childPath:{check:t,apply:b,nullable:true},labelPath:{check:t,apply:o,nullable:true},iconPath:{check:t,apply:h,nullable:true},labelOptions:{apply:g,nullable:true},iconOptions:{apply:a,nullable:true},delegate:{apply:q,init:null,nullable:true}},members:{__rw:null,__bg:null,__nQ:null,__rx:null,_applyDelegate:function(y,z){this._setConfigureItem(y,z);
this._setCreateItem(y,z);
this._setBindItem(y,z);
},_applyIconOptions:function(A,B){this.__ob();
},_applyLabelOptions:function(C,D){this.__ob();
},_applyTarget:function(E,F){if(F!=undefined){this.__dx(F);
}if(this.getModel()!=null){this.__rA();
}this._addChangeTargetListener(E,F);
},_applyModel:function(G,H){this.__rA();
},_applyChildPath:function(I,J){this.__rx=J;
this.__rA();
this.__rx=null;
},_applyIconPath:function(K,L){this.__ob();
},_applyLabelPath:function(M,N){this.__rA();
},__ry:function(O){var Q=O.getTarget();
var P=this.__rw[Q.toHashCode()].treeNode;
var R=this.__rw[Q.toHashCode()].modelNode;
this.__rB(P,R);
this._updateSelection();
},__rz:function(e){var Y=e.getData();
var U=e.getOldData();
var V=this.__rw[U.toHashCode()];
U.removeListenerById(V.changeListenerId);
delete this.__rw[U.toHashCode()];
...
var S=X.event;
var W=ba.addListener(S,this.__rz,this);
var T=V.treeNode;
this.__rw[Y.toHashCode()]={modelNode:ba,treeNode:T,changeListenerId:V.changeListenerId,changeChildernListenerId:W};
this.__rB(T,ba);
this._updateSelection();
...
},__rA:function(){if(this.getTarget()==null||this.getChildPath()==null){return;
...
this.__rE(this.getModel(),bd);
this.__rB(bd,this.getModel());
}},__rB:function(be,bf){if(bf[s+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;
}var bo=bf[s+qx.lang.String.firstUp(this.getChildPath())]();
if(this.__rw[bo.toHashCode()]==undefined){var bj=bo.addListener(m,this.__ry,this);
var bn=qx.Class.getPropertyDefinition(bf.constructor,this.getChildPath());
var bk=bn.event;
var bl=bf.addListener(bk,this.__rz,this);
this.__rw[bo.toHashCode()]={modelNode:bf,treeNode:be,changeListenerId:bj,changeChildernListenerId:bl};
...
be.addAt(bm,i);
this.__rE(bo.getItem(i),bm);
this.__rB(bm,bo.getItem(i));
}}}for(var i=be.getChildren().length-1;i>=bo.length;i--){var bi=be.getChildren()[i];
this.__rD(bi,be);
}},__dx:function(bq){if(bq==null){bq=this.getTarget();
}if(bq==null){return;
}var bs=bq.getRoot();
...
this.__rw={};
}},__rC:function(bt){var bu=bt.getChildren()||[];
for(var i=bu.length-1;i>=0;i--){if(bu[i].getChildren().length>0){this.__rC(bu[i]);
}this.__rD(bu[i],bt);
}},__rD:function(bv,bw){var by=bv.getModel();
var bA=this.__rx||this.getChildPath();
var bB=s+qx.lang.String.firstUp(bA);
if(by[bB]!=undefined){var bz=by[bB]();
var bx=this.__rw[bz.toHashCode()];
bz.removeListenerById(bx.changeListenerId);
by.removeListenerById(bx.changeChildernListenerId);
this.__rC(bv);
delete this.__rw[bz.toHashCode()];
}this.__rF(by);
bw.remove(bv);
bv.destroy();
...
}if(!qx.lang.Array.contains(this.__nQ,bD)){this.__nQ.push(bD);
...
}if(!qx.lang.Array.contains(this.__nQ,bK)){this.__nQ.push(bK);
...
this.__rF(bU);
this.__rE(bU,bW);
}},__rE:function(bX,bY){var ca=this.getDelegate();
if(ca!=null&&ca.bindItem!=null){ca.bindItem(this,bY,bX);
}else{this.bindDefaultProperties(bY,bX);
}},__rF:function(cb){for(var i=0;i<this.__nQ.length;i++){var cd=this.__nQ[i];
...
this.setModel(null);
this.__bg=this.__rw=this.__nQ=null;
}});
})();
...
}},members:{__dm:null,__dn:function(r){return qx.Bootstrap.getKeys(r).sort().join(h);
...
}if(this.__dm&&this.__dm.getModelClass&&this.__dm.getModelClass(v)!=null){return;
}var C={};
var B={__do:this.__do};
...
if(t){C[w].apply=c;
}if(this.__dm&&this.__dm.getValidationRule){var y=this.__dm.getValidationRule(v,w);

if(y){C[w].validate=k+w;
B[k+w]=y;
}}}if(this.__dm&&this.__dm.getModelSuperClass){var A=this.__dm.getModelSuperClass(v)||qx.core.Object;
}else{var A=qx.core.Object;
}var x=[];

if(this.__dm&&this.__dm.getModelMixins){var z=this.__dm.getModelMixins(v);
if(!qx.lang.Type.isArray(z)){if(z!=null){x=[z];
}}}if(t){x.push(qx.data.marshal.MEventBubbling);
}var u={extend:A,include:x,properties:C,members:B,destruct:this.__dp};
qx.Class.define(n+v,u);
},__dp:function(){var D=qx.util.PropertyUtil.getAllProperties(this.constructor);

for(var E in D){this.__do(this.get(D[E].name));
}},__do:function(F){if(!(F instanceof qx.core.Object)){return ;
}if(F.isDisposed()){return;
}F.dispose();
},__dq:function(G){var H;
if(this.__dm&&this.__dm.getModelClass){H=this.__dm.getModelClass(G);
}
...
H.onreadystatechange=this.__nE;
H.onload=this.__nF;
H.onloadend=this.__nG;
H.onabort=this.__nH;
H.ontimeout=this.__ni;
H.onerror=this.__nI;
...
}},_getAllRequestHeaders:function(){var R=qx.lang.Object.merge({},this._getConfiguredRequestHeaders(),this.__nM(),this.__nL,this.__nD);
return R;
},__nM:function(){var T=this.getAuthentication(),S={};

if(T){T.getAuthHeaders().forEach(function(U){S[U.key]=U.value;
});
return S;
}},setRequestHeader:function(V,W){this.__nD[V]=W;
},getRequestHeader:function(X){return this.__nD[X];
},removeRequestHeader:function(Y){if(this.__nD[Y]){delete this.__nD[Y];
...
if(this.__nJ!==bc){this.__nJ=bc;
this.fireEvent(l,qx.event.type.Data,[this.__nJ,bd]);
...
this.fireEvent(bg);
},_setPhase:function(bh){var bi=this.__nK;
...
(function(){var r="qx.event.type.Event",q="",p="function",o="xml",n="GET",m="null",l="XMLHttpRequest",k="Boolean",j="X-Requested-With",i="qx.debug",c="application/x-www-form-urlencoded",h="Cache-Control",f="Content-Type",b="qx.debug.io",a="qx.io.request.Xhr",e="json",d="Accept",g="String";
qx.Class.define(a,{extend:qx.io.request.AbstractRequest,construct:function(s,t){if(t!==undefined){this.setMethod(t);
}qx.io.request.AbstractRequest.call(this,s);
},events:{"readyStateChange":r,"success":r,"load":r,"statusError":r},statics:{PARSER:{json:qx.lang.Json.parse,xml:qx.xml.Document.fromString}},properties:{method:{init:n},async:{check:k,init:true},accept:{check:g,nullable:true},cache:{check:function(u){return qx.lang.Type.isBoolean(u)||qx.lang.Type.isString(u);
},init:true}},members:{__ow:null,_createTransport:function(){return new qx.bom.request.Xhr();
},_getConfiguredUrl:function(){var v=this.getUrl(),w;

if(this.getMethod()===n&&this.getRequestData()){w=this._serializeData(this.getRequestData());
v=qx.util.Uri.appendParamsToUrl(v,w);
}
if(this.getCache()===false){v=qx.util.Uri.appendParamsToUrl(v,{nocache:new Date().valueOf()});
}return v;
},_getConfiguredRequestHeaders:function(){var x={},y=qx.util.Request.methodAllowsRequestBody(this.getMethod());
if(!qx.util.Request.isCrossDomain(this.getUrl())){x[j]=l;
}if(qx.lang.Type.isString(this.getCache())){x[h]=this.getCache();
}if(this.getRequestData()!==m&&y){x[f]=c;
}if(this.getAccept()){if(qx.core.Environment.get(b)){this.debug("Accepting: '"+this.getAccept()+"'");
}x[d]=this.getAccept();
}return x;
},_getMethod:function(){return this.getMethod();
},_isAsync:function(){return this.isAsync();
},_getParsedResponse:function(){var A=this._transport.responseText,z=this._getParser();

if(typeof z===p){if(A!==q){return z.call(this,A);
}}return A;
},setParser:function(B){var C=qx.io.request.Xhr;
if(typeof C.PARSER[B]===p){return this.__ow=C.PARSER[B];
}if(qx.core.Environment.get(i)){qx.core.Assert.assertFunction(B);
}return this.__ow=B;
},_getParser:function(){var D=this.__ow,E;
if(D){return D;
}if(!this.isDone()){return;
}E=this.getResponseContentType()||q;
E=E.replace(/;.*$/,q);

if((/^application\/(\w|\.)*\+?json$/).test(E)){D=qx.io.request.Xhr.PARSER[e];
}
if((/^application\/xml$/).test(E)){D=qx.io.request.Xhr.PARSER[o];
}if((/[^\/]+\/[^\+]+\+xml$/).test(this.getResponseContentType())){D=qx.io.request.Xhr.PARSER[o];
}return D;
}}});
})();
