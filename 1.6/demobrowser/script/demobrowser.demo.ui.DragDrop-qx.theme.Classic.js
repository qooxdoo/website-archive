...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.DragDrop","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.DragDrop-qx.theme.Classic.6d3a2ce1badd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="drop",k="value",j="items",h="move",g="multi",f="dragover",d="icon/16/places/folder.png",c="dragstart",b="Item ",a="copy",z="__rm",y="drag",x="TextArea Target",w="solid",v="Reorderable",u="droprequest",t="#33508D",s="Source",r="demobrowser.demo.ui.DragDrop",q="Simple Target",o="Enable drag",p="\n",n="dragend";
qx.Class.define(r,{extend:qx.application.Standalone,members:{__rm:null,__OB:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var L=this.getRoot();
var F=new qx.ui.container.Scroll();
var A=new qx.ui.layout.Basic();
var D=new qx.ui.container.Composite(A).set({allowStretchY:false,allowStretchX:false});
F.add(D);
L.add(F,{edge:0});
var K=new qx.ui.basic.Label(s);
D.add(K,{left:20,top:20});
var I=new qx.ui.form.List;
I.setDraggable(true);
I.setSelectionMode(g);
D.add(I,{left:20,top:40});

for(var i=0;i<20;i++){I.add(new qx.ui.form.ListItem(b+i,d));
}var M=new qx.ui.form.CheckBox(o);
M.setValue(true);
D.add(M,{left:20,top:260});
I.addListener(c,function(e){if(!M.isValue()){e.preventDefault();
}e.addType(k);
e.addType(j);
e.addAction(a);
e.addAction(h);
});
I.addListener(u,function(e){this.debug("Related of droprequest: "+e.getRelatedTarget());
var P=e.getCurrentAction();
var R=e.getCurrentType();
var O;

switch(R){case j:O=this.getSelection();

if(P==a){var Q=[];

for(var i=0,l=O.length;i<l;i++){Q[i]=O[i].clone();
}O=Q;
}break;
case k:O=this.getSelection()[0].getLabel();
break;
}if(P==h){var N=this.getSelection();

for(var i=0,l=N.length;i<l;i++){this.remove(N[i]);
}}e.addData(R,O);
});
var G=new qx.ui.basic.Label(q);
D.add(G,{left:140,top:20});
var E=new qx.ui.form.List;
E.setDroppable(true);
E.setSelectionMode(g);
D.add(E,{left:140,top:40});
E.addListener(m,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
var S=e.getData(j);

for(var i=0,l=S.length;i<l;i++){this.add(S[i]);
}});
E.addListener(f,function(e){if(!e.supportsType(j)){e.preventDefault();
}});
var G=new qx.ui.basic.Label(x);
D.add(G,{left:260,top:20});
var J=new qx.ui.form.TextArea;
J.setDroppable(true);
J.setHeight(100);
D.add(J,{left:260,top:40});
J.addListener(m,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
this.setValue(e.getData(k).replace(/,/g,p));
});
J.addListener(f,function(e){if(!e.supportsType(k)){e.preventDefault();
}});
var H=new qx.ui.basic.Label(v);
D.add(H,{left:500,top:20});
var B=this.__rm=new qx.ui.form.List;
B.setDraggable(true);
B.setDroppable(true);
B.setSelectionMode(g);
D.add(B,{left:500,top:40});

for(var i=0;i<20;i++){B.add(new qx.ui.form.ListItem(b+i,d));
}var C=new qx.ui.core.Widget;
C.setDecorator(new qx.ui.decoration.Single().set({top:[1,w,t]}));
C.setHeight(0);
C.setOpacity(0.5);
C.setZIndex(100);
C.setLayoutProperties({left:-1000,top:-1000});
C.setDroppable(true);
this.getRoot().add(C);
B.addListener(c,function(e){e.addAction(h);
});
B.addListener(n,function(e){C.setDomPosition(-1000,-1000);
});
B.addListener(y,function(e){var U=e.getOriginalTarget();
if(U instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__OB=U;
}
if(!qx.ui.core.Widget.contains(this,U)&&U!=C){return;
}var T=U.getContainerLocation();
C.setWidth(U.getBounds().width);
C.setDomPosition(T.left,T.top);
});
B.addListener(f,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
B.addListener(m,function(e){this.__OC(e.getOriginalTarget());
},this);
C.addListener(m,function(e){this.__OC(this.__OB);
},this);
},__OC:function(V){if(V.classname!="qx.ui.form.ListItem"){return;
}var W=this.__rm.getSortedSelection();

for(var i=0,l=W.length;i<l;i++){this.__rm.addBefore(W[i],V);
this.__rm.addToSelection(W[i]);
