...
},setSource:function(g){if(this.__IY){throw new Error("The source cannot be modified after initial creation");
}this.setAttribute(c,g);
},setId:function(h){if(this.__IY){throw new Error("The id cannot be modified after initial creation");
}this.setAttribute(b,h);
},getVariables:function(){return this.__IW;
},setVariables:function(i){if(this.__IY){throw new Error("The variables cannot be modified after initial creation");
}this.__IW=i;
},getAttributes:function(){return this.__IX;
},setAttribute:function(j,k){if(this.__IY){throw new Error("The attributes cannot be modified after initial creation");
}
if(k===null||k===undefined){delete this.__IX[j];
}else{this.__IX[j]=k;
}},getParams:function(){return this.__IV;
},setParam:function(l,m){if(this.__IY){throw new Error("The params cannot be modified after initial creation");
}
if(m===null||m===undefined){delete this.__IV[l];
}else{this.__IV[l]=m;
...
}}}var G=this.__Jf(B,C,E,F);
this._flashObjects[C.id]=G;
return G;
},destroy:qx.core.Environment.select(k,{"mshtml":function(H,I){H=this.__Ja(H);

if(H.readyState==4){this.__Jb(H);
}else{if(!I){I=window;
}qx.bom.Event.addNativeListener(I,w,function(){qx.bom.Flash.__Jb(H);
});
}},"default":function(J,K){J=this.__Ja(J);

if(J.parentNode){J.parentNode.removeChild(J);
}delete this._flashObjects[J.id];
}}),__Ja:function(L){if(!L){throw new Error("DOM element is null or undefined!");
}
...
},__Jb:qx.core.Environment.select(k,{"mshtml":qx.event.GlobalError.observeMethod(function(M){for(var i in M){if(typeof M[i]==c){M[i]=null;
}}
if(M.parentNode){M.parentNode.removeChild(M);
}delete this._flashObjects[M.id];
}),"default":null}),__Jc:qx.event.GlobalError.observeMethod(function(){for(var N in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[N]);
}window.__Jd=function(){};
window.__Je=function(){};
qx.bom.Event.removeNativeListener(window,e,qx.bom.Flash.__Jc);
}),__Jf:qx.core.Environment.select(k,{"mshtml":function(O,P,Q,R){Q.movie=P.data;
delete P.data;
delete P.classid;
var S=s;
...
return Y;
}})},defer:function(ba){if((qx.core.Environment.get(k)==r)){qx.bom.Event.addNativeListener(window,e,ba.__Jc);
}}});
})();
...
if(be!=e&&be!=null&&be!=c){if(!this.isLoaded()){if(!this.__JX){this.__JX=new Date().getTime();
}var bf=new Date().getTime()-this.__JX;
...
qx.ui.core.queue.Layout.add(this);
},_applyQuality:function(bq,br){this.__JY(b,bq);
},_applyScale:function(bs,bt){this.__JY(V,bs);
},_applyWmode:function(bu,bv){this.__JY(R,bu);
},_applyPlay:function(bw,bx){this.__JY(E,bw);
},_applyLoop:function(by,bz){this.__JY(r,by);
},_applyMenu:function(bA,bB){this.__JY(m,bA);
},_applyAllowScriptAccess:function(bC,bD){this.__JY(o,bC);
},_applyLiveConnect:function(bE,bF){this.__JY(l,bE);
},__JY:function(bG,bH){this.getContentElement().setParam(bG,bH);
qx.ui.core.queue.Layout.add(this);
}}});
})();
...
G.setColumnFlex(1,1);
this.__ju=new qx.ui.container.Composite(G);
this.add(this.__ju,{width:j,height:j});
this.initWidgets();
},members:{__ju:null,initWidgets:function(){var U=this._widgets;
var O;
O=new qx.ui.basic.Label(o);
this.__ju.add(O,{row:0,column:0});
...
U.push(N);
this.__ju.add(N,{row:1,column:0,colSpan:2});
O=new qx.ui.basic.Label(n);
this.__ju.add(O,{row:2,column:0});
...
H.addListener(g,this.__LK,this);
U.push(H);
this.__ju.add(H,{row:3,column:0});
}else{this.__ju.add(new qx.ui.basic.Label(y),{row:3,column:0});
}O=new qx.ui.basic.Label(v);
this.__ju.add(O,{row:2,column:1});
var S=new qx.ui.container.Composite(new qx.ui.layout.VBox(10));
this.__ju.add(S,{row:3,column:1});
var P=w+F+a+B+b+d+h;
...
this.__ju.add(O,{row:4,column:0});
...
this.__ju.add(R,{row:5,column:0});
U.push(R);
},__LK:function(e){var X=e.getData();
var W=X.context;
