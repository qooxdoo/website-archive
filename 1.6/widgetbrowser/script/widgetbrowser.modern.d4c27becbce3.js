...
if(!dt){this.__jH=qx.locale.Manager.getInstance().getLocale();
this.__yP=qx.locale.Manager.getInstance().bind(dn,this,dn);
}else{this.__jH=dt;
this.setLocale(dt);
}this.__yQ=this.__jH;

if(ds!=null){this.__yR=ds.toString();

if(this.__yR in qx.util.format.DateFormat.ISO_MASKS){if(this.__yR===d){this.__yS=true;
}this.__yR=qx.util.format.DateFormat.ISO_MASKS[this.__yR];
}}else{this.__yR=qx.locale.Date.getDateFormat(bd,this.__jH)+N+qx.locale.Date.getDateTimeFormat(dq,dl,this.__jH);
...
if(dv._dateInstance==null||dv._dateInstance.__yR!=du){dv._dateTimeInstance=new dv();
...
if(dx._dateInstance==null||dx._dateInstance.__yR!=dw){dx._dateInstance=new dx(dw);
}return dx._dateInstance;
},ASSUME_YEAR_2000_THRESHOLD:30,LOGGING_DATE_TIME__format:da,ISO_MASKS:{isoDate:"yyyy-MM-dd",isoTime:"HH:mm:ss",isoDateTime:"yyyy-MM-dd'T'HH:mm:ss",isoUtcDateTime:"yyyy-MM-dd'T'HH:mm:ss'Z'"},AM_MARKER:"am",PM_MARKER:"pm"},members:{__yP:null,__jH:null,__yQ:null,__yR:null,__yT:null,__yU:null,__yV:null,__yS:null,__yW:function(dy,dz){var dA=cG+(dy<0?((-1)*dy):dy);

while(dA.length<dz){dA=bH+dA;
}return dy<0?cF+dA:dA;
},__yX:function(dB){var dC=new Date(dB.getTime());
var dD=dC.getDate();

while(dC.getMonth()!=0){dC.setDate(-1);
dD+=dC.getDate()+1;
}return dD;
},__yY:function(dE){return new Date(dE.getTime()+(3-((dE.getDay()+6)%7))*86400000);
},__za:function(dF){var dH=this.__yY(dF);
var dI=dH.getFullYear();
var dG=this.__yY(new Date(dI,0,4));
return Math.floor(1.5+(dH.getTime()-dG.getTime())/86400000/7);
},__zb:function(dJ){var dL=this.__yY(dJ);
var dK=this.__yY(new Date(dJ.getFullYear(),dJ.getMonth(),4));
return Math.floor(1.5+(dL.getTime()-dK.getTime())/86400000/7);
},__zc:function(dM){var dN=this.__yY(dM);
return dN.getFullYear();
},__zd:function(dO){var dP=new Date(dO,2,1);
dP.setDate(-1);
return dP.getDate()+1===29;
},__ze:function(dQ,dR){var dV=0;
var dW=0;
if(!dR){dR=1971;
}var dS=0;
...
},__zf:function(dX,dY,ea){var eb;

switch(dY){case 11:eb=dX-1;

if(dX!=this.__zc(new Date(eb,dY,ea))){eb=dX;
}break;
case 0:eb=dX+1;
...
},_applyLocale:function(ec,ed){this.__jH=ec===null?this.setLocale(this.__yQ):ec;
},format:function(ee){if(ee==null){return null;
}
if(isNaN(ee.getTime())){return null;
}
if(this.__yS){ee=new Date(ee.getUTCFullYear(),ee.getUTCMonth(),ee.getUTCDate(),ee.getUTCHours(),ee.getUTCMinutes(),ee.getUTCSeconds(),ee.getUTCMilliseconds());
}var eh=this.__jH;
...
for(var i=0;i<this.__yV.length;i++){var en=this.__yV[i];
...
switch(eg){case x:if(er==2){ez=this.__yW(em%100,2);
...
case cl:if(er==1||er==2){ez=this.__yW(parseInt(ei/4)+1,er);
}
if(er==3){ez=cl+(parseInt(ei/4)+1);
}break;
case U:if(er==1||er==2){ez=this.__yW(parseInt(ei/4)+1,er);
}
if(er==3){ez=cl+(parseInt(ei/4)+1);
}break;
case cz:ez=this.__yW(this.__yX(ee),er);
break;
case dc:ez=this.__yW(el,er);
break;
case A:ez=this.__yW(this.__za(ee),er);
break;
case E:ez=this.__zb(ee);
break;
...
if(er>=1&&er<=2){ez=this.__yW(eu,er);
...
case H:if(er==1||er==2){ez=this.__yW(ei+1,er);
...
case t:if(er==1||er==2){ez=this.__yW(ei+1,er);
...
break;
case ca:ez=this.__yW(es,er);
break;
case cu:ez=this.__yW((es==0)?24:es,er);
break;
case cy:ez=this.__yW(es%12,er);
break;
case bT:ez=this.__yW(((es%12)==0)?12:(es%12),er);
break;
case m:ez=this.__yW(ey,er);
break;
case bR:ez=this.__yW(eq,er);
break;
case cb:ez=ew+cG;
...
case bb:if(er>=1&&er<=4){ez=dm+((et>0)?cF:cO)+this.__yW(Math.abs(ef),2)+w+this.__yW(ev,2);
}break;
case K:if(er>=1&&er<=3){ez=((et>0)?cF:cO)+this.__yW(Math.abs(ef),2)+this.__yW(ev,2);
}else{ez=dm+((et>0)?cF:cO)+this.__yW(Math.abs(ef),2)+w+this.__yW(ev,2);
}break;
}ex+=ez;
}}return ex;
},parse:function(eA){this.__zh();
var eJ=this.__yT.regex.exec(eA);

if(eJ==null){throw new Error("Date string '"+eA+"' does not match the date format: "+this.__yR);
...
for(var i=0;i<this.__yT.usedRules.length;i++){var eF=this.__yT.usedRules[i];
var eG=eJ[eD];
...
for(var k=0;k<this.__yT.usedRules.length;k++){if(this.__yT.usedRules[k].pattern==dr){eH=true;
break;
}}
if(!eH){eC=true;
}}
...
for(var k=0;k<this.__yT.usedRules.length;k++){if(this.__yT.usedRules[k].pattern.indexOf(bi)!=-1){eL=true;
break;
}}
if(!eL){eE=true;
}}eD+=(eF.groups==null)?1:eF.groups;
}
if(eC){eB.year=this.__zf(eB.weekYear,eB.month,eB.day);
}
if(eE){var eK=this.__ze(eB.dayOfYear,eB.year);
eB.month=eK.month;
eB.day=eK.day;
}
...
if(this.__yS){eI=new Date(eI.getUTCFullYear(),eI.getUTCMonth(),eI.getUTCDate(),eI.getUTCHours(),eI.getUTCMinutes(),eI.getUTCSeconds(),eI.getUTCMilliseconds());
}
...
case bF:if(eR==eQ){eO++;
i++;
}else{this.__yV.push({type:bF,character:eQ,size:eO});
eQ=null;
eO=0;
eP=bI;
}break;
...
if(eP!=bI){if(eS.length>0){this.__yV.push({type:bG,text:eS});
eS=cG;
}}else{eS+=eR;
i++;
}break;
}}if(eQ!=null){this.__yV.push({type:bF,character:eQ,size:eO});
}else if(eS.length>0){this.__yV.push({type:bG,text:eS});
...
for(var eU=0;eU<this.__yV.length;eU++){var fe=this.__yV[eU];
...
for(var ff=0;ff<this.__yU.length;ff++){var fb=this.__yU[ff];

if(this.__zi(fb,eV,fa)){eX=fb;
break;
...
}this.__yT={regex:eT,"usedRules":fd,pattern:eY};
},__zi:function(fh,fi,fj){if(fi===x&&fh.pattern===dr){fh.regex=fh.regexFunc(fj);
return true;
}else if(fi===be&&fh.pattern===bw){fh.regex=fh.regexFunc(fj);
return true;
}else{return fi==fh.pattern.charAt(0)&&fj==fh.pattern.length;
}},__zj:function(){var fx=qx.util.format.DateFormat;
var fz=qx.lang.String;

if(this.__yU!=null){return ;
}var fy=this.__yU=[];
var ft=qx.locale.Date.getAmMarker(this.__jH).toString()||fx.AM_MARKER;
var fG=qx.locale.Date.getPmMarker(this.__jH).toString()||fx.PM_MARKER;
var fl=this.__jH;
...
qx.Class.define(l,{statics:{__zk:qx.locale.Manager.getInstance(),getAmMarker:function(F){return this.__zk.localize(o,[],F);
},getPmMarker:function(G){return this.__zk.localize(p,[],G);
},getDayNames:function(length,H,I,J){var I=I?I:h;
var L=[B,C,D,a,e,E,d];
var M=[];

for(var i=0;i<L.length;i++){var K=c+I+k+length+k+L[i];
M.push(J?this.__zl(I,I===j?f:j,K,H):this.__zk.localize(K,[],H));
}return M;
},getDayName:function(length,N,O,P,Q){var P=P?P:h;
var S=[B,C,D,a,e,E,d];
var R=c+P+k+length+k+S[N];
return Q?this.__zl(P,P===j?f:j,R,O):this.__zk.localize(R,[],O);
},getMonthNames:function(length,T,U,V){var U=U?U:h;
var X=[];

for(var i=0;i<12;i++){var W=b+U+k+length+k+(i+1);
X.push(V?this.__zl(U,U===j?f:j,W,T):this.__zk.localize(W,[],T));
}return X;
},getMonthName:function(length,Y,ba,bb,bc){var bb=bb?bb:h;
var bd=b+bb+k+length+k+(Y+1);
return bc?this.__zl(bb,bb===j?f:j,bd,ba):this.__zk.localize(bd,[],ba);
},getDateFormat:function(be,bf){var bg=v+be;
return this.__zk.localize(bg,[],bf);
},getDateTimeFormat:function(bh,bi,bj){var bl=m+bh;
var bk=this.__zk.localize(bl,[],bj);

if(bk==bl){bk=bi;
}return bk;
},getTimeFormat:function(bm,bn){var bp=t+bm;
var bo=this.__zk.localize(bp,[],bn);
...
}else{bE=this.__zk.getTerritory()||this.__zk.getLanguage();
}return bE.toUpperCase();
},__zl:function(bF,bG,bH,bI){var bK=this.__zk.localize(bH,[],bI);

if(bK==bH){var bJ=bH.replace(g+bF+g,g+bG+g);
return this.__zk.localize(bJ,[],bI);
}else{return bK;
}}}});
})();
