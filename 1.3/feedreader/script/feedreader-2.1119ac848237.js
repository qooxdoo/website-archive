qx.$$packageData['1119ac848237']={"locales":{},"resources":{"feedreader/images/combined/icons16.png":[16,96,"png","feedreader"],"qx/icon/Tango/16/actions/dialog-apply.png":[16,16,"png","qx","feedreader/images/combined/icons16.png",0,-80],"qx/icon/Tango/16/actions/document-new.png":[16,16,"png","qx","feedreader/images/combined/icons16.png",0,-16]},"translations":{}};

qx.Part.$$notifyLoad("1119ac848237", function() {
...
}}},__N:function(bB){if(this.__K!=bB){this.fireNonBubblingEvent("changeValue",qx.event.type.Data,[bB,this.__K]);
this.__K=bB;
}},setValue:function(bC){if(bC===null){if(this.__I){return bC;
}bC=j;
this.__I=true;
}else{this.__I=false;
if(this.__H){this._removePlaceholder();
}}
if(qx.lang.Type.isString(bC)){var bE=this.getContentElement();

if(bC.length>this.getMaxLength()){bC=bC.substr(0,this.getMaxLength());
}
if(bE.getValue()!=bC){var bF=bE.getValue();
bE.setValue(bC);
var bD=this.__I?null:bC;
this.__K=bF;
this.__N(bD);
}if(this.__H){this._showPlaceholder();
}return bC;
}throw new Error("Invalid value type: "+bC);
},getValue:function(){var bG=this.getContentElement().getValue();
return this.__I?null:bG;
...
},_showPlaceholder:function(){var bK=this.getValue()||j;
var bJ=this.getPlaceholder();

if(bJ!=null&&bK==j&&!this.hasState(o)&&!this.hasState(v)){if(this.hasState(k)){this._syncPlaceholder();
...
}},"off":null}),_applyPlaceholder:function(bL,bM){if(this.__H){this.__O().setValue(bL);

if(bL!=null){this.addListener(X,this._removePlaceholder,this);
...
}}else{if(this.getEnabled()){this.getContentElement().setAttribute(a,bL);
}}},_applyTextAlign:function(bN,bO){this.getContentElement().setStyle(n,bN);
},_applyReadOnly:function(bP,bQ){var bR=this.getContentElement();
bR.setAttribute(S,bP);

if(bP){this.addState(b);
