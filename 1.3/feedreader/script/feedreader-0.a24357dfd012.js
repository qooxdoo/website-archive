...
this.__fg[L]=K;
K.$$id=L;
var M=this.__fi.getAllLogEvents();
...
delete N.$$id;
},debug:function(P,Q){qx.log.Logger.__fk(A,arguments);
},info:function(R,S){qx.log.Logger.__fk(e,arguments);
},warn:function(T,U){qx.log.Logger.__fk(r,arguments);
},error:function(V,W){qx.log.Logger.__fk(j,arguments);
},trace:function(X){qx.log.Logger.__fk(e,[X,qx.dev.StackTrace.getStackTrace().join(u)]);
},deprecatedMethodWarning:function(Y,ba){var bb;
{};
...
{};
},deprecateMethodOverriding:function(bp,bq,br,bs){var bt;
{};
},clear:function(){this.__fi.clearHistory();
},__fi:new qx.log.appender.RingBuffer(50),__fj:{debug:0,info:1,warn:2,error:3},__fk:function(bu,bv){var bA=this.__fj;

if(bA[bu]<bA[this.__ff]){return;
}var bx=bv.length<2?null:bv[0];
var bz=bx?1:0;
var bw=[];

for(var i=bz,l=bv.length;i<l;i++){bw.push(this.__fm(bv[i],true));
}var bB=new Date;
var bC={time:bB,offset:bB-qx.Bootstrap.LOADSTART,level:bu,items:bw,win:window};
if(bx){if(bx.$$hash!==undefined){bC.object=bx.$$hash;
}else if(bx.$$type){bC.clazz=bx;
}}this.__fi.process(bC);
var bD=this.__fg;

for(var by in bD){bD[by].process(bC);
}},__fl:function(bE){if(bE===undefined){return y;
}else if(bE===null){return b;
}
if(bE.$$type){return a;
}var bF=typeof bE;

if(bF===C||bF==c||bF===H||bF===B){return bF;
...
}else if(bE instanceof Date){return E;
}else{return z;
}}
if(bE.toString){return G;
}return D;
},__fm:function(bG,bH){var bO=this.__fl(bG);
var bK=D;
var bJ=[];

switch(bO){case b:case y:bK=bO;
break;
case c:case H:case B:case E:bK=bG;
break;
case k:if(bG.nodeType===9){bK=o;
}else if(bG.nodeType===3){bK=m+bG.nodeValue+F;
}else if(bG.nodeType===1){bK=bG.nodeName.toLowerCase();

if(bG.id){bK+=q+bG.id;
}}else{bK=k;
}break;
case C:bK=qx.lang.Function.getName(bG)||bO;
break;
case d:bK=bG.basename+t+bG.$$hash+F;
break;
case a:case G:bK=bG.toString();
break;
case j:bJ=qx.dev.StackTrace.getStackTraceFromError(bG);
bK=bG.toString();
break;
case g:if(bH){bK=[];

for(var i=0,l=bG.length;i<l;i++){if(bK.length>20){bK.push(h+(l-i)+f);
break;
}bK.push(this.__fm(bG[i],false));
}}else{bK=n+bG.length+x;
}break;
case z:if(bH){var bI;
var bN=[];
...
for(var i=0,l=bN.length;i<l;i++){if(bK.length>20){bK.push(h+(l-i)+f);
break;
}bM=bN[i];
bI=this.__fm(bG[bM],false);
bI.key=bM;
bK.push(bI);
}}else{var bL=0;
...
for(var i=0;i<bQ.length;i++){bP.__fk(bQ[i][0],bQ[i][1]);
...
}}}),getStart:qx.core.Variant.select(k,{"mshtml":function(A){if(this.getLength(A)<1){return -1;
}
if(this.__lf(A)){var F=qx.bom.Range.get();
...
}},"gecko|webkit":function(J){if(this.getLength(J)<1){return -1;
}
if(this.__lf(J)){return J.selectionStart;
...
}}},"default":function(M){if(this.getLength(M)<1){return -1;
}
if(this.__lf(M)){return M.selectionStart;
}else{return qx.bom.Selection.getSelectionObject(qx.dom.Node.getDocument(M)).anchorOffset;
}}}),getEnd:qx.core.Variant.select(k,{"mshtml":function(N){if(this.getLength(N)<1){return -1;
}
if(this.__lf(N)){var S=qx.bom.Range.get();
...
}},"gecko|webkit":function(W){if(this.getLength(W)<1){return -1;
}
if(this.__lf(W)){return W.selectionEnd;
...
}}},"default":function(ba){if(this.getLength(ba)<1){return -1;
}
if(this.__lf(ba)){return ba.selectionEnd;
...
var r=j+q.getUrl()+h;
var s=new qx.data.store.Yql(r,{manipulateData:function(t){if(t.query.results){t=t.query.results.item||t.query.results.entry;
for(var i=0;i<t.length;i++){if(!qx.lang.Type.isString(t[i].title)){t[i].title=t[i].title.content;
}}return t;
}return m;
}},qx.bom.client.Feature.SSL);
s.addListener(k,this.__ns(q),this);
s.addListener(c,qx.lang.Function.bind(this.__nr,this,q),this);
},__nr:function(u,e){if(e.getData()==b||e.getData()==g||e.getData()==m){u.setState(l);
}},__ns:function(v){return function(e){var w=e.getData();
if(w==m){v.setState(l);
return;
}v.setArticles(w);
v.setState(k);
...
(function(){var o="static",n="execute",m="user",l="settings",k="loading",j="addfeed",h="selection[0]",g="title",f="north",d="main",bw="new",bv="Surfin' Safari",bu="changeModel",bt="__pD",bs="selection[0].state",br="Heise",bq="__pJ",bp="__pG",bo="__pP",bn="F1",v="article",w="partLoaded",t="Control+A",u="__pE",r="http://blogs.msdn.com/ie/rss.xml",s="__pO",p="http://my.opera.com/desktopteam/xml/rss/blog/",q="feedreader/images/loading22.gif",D="error",E="change",Q="http://blogs.msdn.com/jscript/rss.xml",M="loaded",Y="feeds",T="__pH",bj="icon/22/places/folder.png",be="The Mozilla Blog",I="enabled",bm="__pN",bl="http://webkit.org/blog/feed/",bk="__pL",H="separator-vertical",K="http://feeds2.feedburner.com/qooxdoo/news/content",L="http://blog.mozilla.com/feed/",O="__pI",R="vertical",U="http://www.heise.de/newsticker/heise-atom.xml",bb="__pR",bg="state",x="JScript Team Blog",y="Feeds",J="http://feeds2.feedburner.com/ajaxian",X="Control+D",W="icon/22/actions/process-stop.png",V="selection[0].articles",bd="white",bc="IEBlog",S="Ajaxian",ba="User Feeds",a="icon/22/apps/internet-feed-reader.png",bf="model",z="feedreader.Application",A="http://daringfireball.net/index.xml",N="Control+P",b="Static Feeds",c="__pQ",G="Control+R",B="qooxdoo News",C="Opera Desktop Blog",F="feedreader.model.Feed",P="Daring Fireball",bi="__pK",bh="__pM";
qx.Class.define(z,{extend:qx.application.Standalone,members:{__pD:null,__pE:null,__pF:null,__pG:null,__pH:null,__pI:null,__pJ:null,__pK:null,__pL:null,__pM:null,__pN:null,__pO:null,__pP:null,__pQ:null,__pR:null,main:function(){qx.application.Standalone.prototype.main.call(this);
{};
qx.io.PartLoader.getInstance().addListener(w,function(e){this.debug("part loaded: "+e.getData().getName());
},this);
var by=qx.locale.Manager.getInstance().getLanguage();
var bx=qx.Part.getInstance().getParts();
if(bx[by]){qx.io.PartLoader.require([by],function(){qx.locale.Manager.getInstance().setLocale(by);
this.buildUpGui();
},this);
}else{this.warn("Cannot load locale part for current language "+by+", falling back to English.");
this.buildUpGui();
}},buildUpGui:function(){this._initializeCommands();
this._createLayout();
this._initializeModel();
this._setUpBinding();
this.__pG.getRoot().setOpen(true);
this.__pG.getRoot().getChildren()[0].setOpen(true);
this.__pG.getRoot().getChildren()[1].setOpen(true);
this.__pG.setHideRoot(true);
this.__pD.getSelection().push(this.__pR.getFeeds().getItem(0));
this.reload();
},_initializeModel:function(){this.__pP=new feedreader.model.FeedFolder(y);
this.__pR=new feedreader.model.FeedFolder(this.tr(b));
this.__pP.getFeeds().push(this.__pR);
this.__pR.getFeeds().push(new feedreader.model.Feed(B,K,o));
this.__pR.getFeeds().push(new feedreader.model.Feed(x,Q,o));
this.__pR.getFeeds().push(new feedreader.model.Feed(P,A,o));
this.__pR.getFeeds().push(new feedreader.model.Feed(bv,bl,o));
this.__pR.getFeeds().push(new feedreader.model.Feed(S,J,o));
this.__pQ=new feedreader.model.FeedFolder(this.tr(ba));
this.__pP.getFeeds().push(this.__pQ);
this.__pQ.getFeeds().push(new feedreader.model.Feed(br,U,m));
this.__pQ.getFeeds().push(new feedreader.model.Feed(bc,r,m));
this.__pQ.getFeeds().push(new feedreader.model.Feed(be,L,m));
this.__pQ.getFeeds().push(new feedreader.model.Feed(C,p,m));
},_setUpBinding:function(){var bz={converter:this._state2iconConverter};
this.__pD=new qx.data.controller.Tree(this.__pP,this.__pG,Y,g);
this.__pD.setIconOptions(bz);
this.__pD.setIconPath(bg);
this.__pE=new qx.data.controller.List(null,this.__pJ.getList(),g);
this.__pD.bind(V,this.__pE,bf);
this.__pE.bind(h,this.__pN,v);
this.__pE.getSelection().addListener(E,this._listControllerChange,this);
this.__pE.addListener(bu,this._treeControllerChange,this);
var bA={converter:this._state2loadingConverter};
this.__pD.bind(bs,this.__pJ,k,bA);
bA={converter:this._category2enabledConverter};
this.__pD.bind(h,this.__pI.getRemoveButton(),I,bA);
},_category2enabledConverter:function(bB){return !!bB&&bB.getCategory()==m;
},_state2iconConverter:function(bC){if(bC==bw||bC==k){return q;
}else if(bC==M){return a;
}else if(bC==D){return W;
}return bj;
},_state2loadingConverter:function(bD){if(bD==bw||bD==k){return true;
}return false;
},_listControllerChange:function(bE){var bF=this.__pD.getSelection().getItem(0);
var bG=this.__pE.getSelection().getItem(0);
if(bG!=undefined){bF.setSelectedArticle(bG);
}},_treeControllerChange:function(bH){var bI=this.__pD.getSelection().getItem(0);
if(bI!=null&&bI.classname==F&&bI.getSelectedArticle()!=null&&bI.getArticles().contains(bI.getSelectedArticle())){qx.event.Timer.once(function(){this.__pE.getSelection().push(bI.getSelectedArticle());
},this,0);
}else{this.__pJ.getList().scrollToY(0);
}},_createLayout:function(){var bJ=new qx.ui.layout.Dock();
bJ.setSeparatorY(H);
var bK=new qx.ui.container.Composite(bJ);
this.getRoot().add(bK,{edge:0});
this.__pH=new feedreader.view.Header();
bK.add(this.__pH,{edge:f});
this.__pI=new feedreader.view.ToolBar(this);
bK.add(this.__pI,{edge:f});
this.__pK=new qx.ui.splitpane.Pane();
bK.add(this.__pK);
this.__pG=new qx.ui.tree.Tree();
this.__pG.setWidth(250);
this.__pG.setBackgroundColor(bd);
this.__pK.add(this.__pG,0);
this.__pL=new qx.ui.splitpane.Pane(R);
this.__pL.setDecorator(null);
this.__pK.add(this.__pL,1);
this.__pJ=new feedreader.view.List(this.__pP);
this.__pJ.setHeight(200);
this.__pJ.setDecorator(d);
this.__pL.add(this.__pJ,0);
this.__pN=new feedreader.view.Article();
this.__pN.setDecorator(d);
this.__pL.add(this.__pN,1);
},_initializeCommands:function(){var bL={};
bL.reload=new qx.ui.core.Command(G);
bL.reload.addListener(n,this.reload,this);
bL.about=new qx.ui.core.Command(bn);
bL.about.addListener(n,this.showAbout,this);
bL.preferences=new qx.ui.core.Command(N);
bL.preferences.addListener(n,this.showPreferences,this);
bL.addFeed=new qx.ui.core.Command(t);
bL.addFeed.addListener(n,this.showAddFeed,this);
bL.removeFeed=new qx.ui.core.Command(X);
bL.removeFeed.addListener(n,this.removeFeed,this);
this.__pF=bL;
},getCommand:function(bM){return this.__pF[bM];
},addFeed:function(bN,bO,bP){var bQ=new feedreader.model.Feed(bN,bO,bP);
this.__pQ.getFeeds().push(bQ);
var bR=feedreader.io.FeedLoader.getInstance();
bR.load(bQ);
},removeFeed:function(){var bS=this.__pD.getSelection().getItem(0);
if(bS&&bS.getCategory&&bS.getCategory()!==o){var bT=this.__pQ.getFeeds();
for(var i=0;i<bT.length;i++){if(bS===bT.getItem(i)){bT.splice(i,1);
return;
}}}},reload:function(){var bU=feedreader.io.FeedLoader.getInstance();
bU.loadAll(this.__pP);
},showPreferences:function(){this.__pI.signalLoading(l,true);
qx.io.PartLoader.require([l],function(){if(!this.__pM){this.__pM=new feedreader.view.PreferenceWindow();
this.getRoot().add(this.__pM);
}this.__pM.center();
this.__pM.open();
this.__pI.signalLoading(l,false);
},this);
},showAbout:function(){alert(this.tr("FeedReader (qooxdoo powered)"));
},showAddFeed:function(){this.__pI.signalLoading(j,true);
qx.io.PartLoader.require([j],function(){if(!this.__pO){this.__pO=new feedreader.view.AddFeedWindow(this);
this.getRoot().add(this.__pO);
}this.__pO.center();
this.__pO.open();
this.__pI.signalLoading(j,false);
},this);
}},destruct:function(){this.__pF=null;
this._disposeObjects(O,bq,bm,bp,bo,bi,bk,T,bb,c,bt,u,bh,s);
}});
})();
...
(function(){var k="enabled",j="originalIcon",i=")",h="Remove feed",g="Preferences",f="excluded",d="addFeed",c="removeFeed",b="icon/22/actions/help-about.png",a="Reload the feeds. (%1)",G="Help",F="visible",E="feedreader.view.ToolBar",D="icon/22/actions/media-seek-forward.png",C="toolbar-button",B="Add feed",A="addfeed",z="icon/22/actions/view-refresh.png",y="(",x="reload",r="showItem",s="settings",p="about",q="icon/22/actions/dialog-cancel.png",n="Reload",o="preferences",l="icon/22/actions/dialog-ok.png",m="icon/22/apps/preferences-theme.png",t="__qs",u="hideItem",w="Open preferences window.",v="feedreader/images/loading22.gif";
qx.Class.define(E,{extend:qx.ui.toolbar.ToolBar,construct:function(H){qx.ui.toolbar.ToolBar.call(this);
this.__qq={};
this.__qr=new qx.ui.toolbar.Button(this.tr(B),l);
this.__qr.setCommand(H.getCommand(d));
this.add(this.__qr);
this.__qs=new qx.ui.toolbar.Button(this.tr(h),q);
this.__qs.setCommand(H.getCommand(c));
this.__qs.setEnabled(false);
this.add(this.__qs);
this.addSeparator();
var K=new qx.ui.toolbar.Button(this.tr(n),z);
var I=H.getCommand(x);
K.setCommand(I);
K.setToolTipText(this.tr(a,I.toString()));
this.add(K);
this.addSeparator();
this.__qt=new qx.ui.toolbar.Button(this.tr(g),m);
this.__qt.setCommand(H.getCommand(o));
this.__qt.setToolTipText(this.tr(w));
this.add(this.__qt);
this.addSpacer();
...
this.__qu=new qx.ui.menu.Menu();
M.setMenu(this.__qu);
this.addListener(u,this._onHideItem,this);
this.addListener(r,this._onShowItem,this);
},members:{__qs:null,__qu:null,__qq:null,__qr:null,__qt:null,getRemoveButton:function(){return this.__qs;
...
P.setVisibility(f);
},_getMenuItem:function(R){var S=this.__qq[R.toHashCode()];
...
this.__qq[R.toHashCode()]=S;
}return S;
},signalLoading:function(T,U){if(T==A){var V=this.__qr;
}else if(T==s){var V=this.__qt;
}else{return ;
...
(function(){var n="separator-vertical",m="background-medium",l="Posts",k="bold",j="_applyLoading",i="__rv",h="center",g="feedreader.view.List",f="__rw",e="single",b="__ru",d="middle",c="feedreader/images/loading66.gif",a="Boolean";
...
q.setFont(k);
this._add(q);
this.__ru=new qx.ui.container.Stack();
this._add(this.__ru,{flex:1});
this.__rv=new qx.ui.form.List();
this.__rv.setDecorator(null);
this.__rv.setSelectionMode(e);
this.__ru.add(this.__rv);
this.__rw=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,h));
var o=new qx.ui.basic.Image(c);
o.setAlignY(d);
this.__rw.add(o);
this.__ru.add(this.__rw);
},properties:{loading:{check:a,init:false,apply:j}},members:{__ru:null,__rv:null,__rw:null,_applyLoading:function(r,s){if(r){this.__ru.setSelection([this.__rw]);
}else{this.__ru.setSelection([this.__rv]);
}},getList:function(){return this.__rv;
}},destruct:function(){this._disposeObjects(i,b,f);
}});
})();
...
if(!feedreader.view.Article.__rR){feedreader.view.Article.__rR=true;
...
w.setLineHeight(1.8);
this.setFont(w);
},properties:{article:{apply:e,nullable:true,check:d}},members:{_applyArticle:function(x,y){var z=this.__rS();
this.setHtml(z);
qx.html.Element.flush();
var B=this.getContentElement().getDomElement();
var A=B.getElementsByTagName(b);
for(var i=0;i<A.length;i++){A[i].target=t;
}},__rS:function(){var D=this.getArticle();
if(!D){return v;
