...
this.__bL[L]=K;
K.$$id=L;
var M=this.__bN.getAllLogEvents();
...
delete N.$$id;
},debug:function(P,Q){qx.log.Logger.__bP(A,arguments);
},info:function(R,S){qx.log.Logger.__bP(e,arguments);
},warn:function(T,U){qx.log.Logger.__bP(r,arguments);
},error:function(V,W){qx.log.Logger.__bP(j,arguments);
},trace:function(X){qx.log.Logger.__bP(e,[X,qx.dev.StackTrace.getStackTrace().join(u)]);
...
},__bN:new qx.log.appender.RingBuffer(50),__bO:{debug:0,info:1,warn:2,error:3},__bP:function(bu,bv){var bA=this.__bO;

if(bA[bu]<bA[this.__bK]){return;
}var bx=bv.length<2?null:bv[0];
var bz=bx?1:0;
var bw=[];

for(var i=bz,l=bv.length;i<l;i++){bw.push(this.__bR(bv[i],true));
}var bB=new Date;
...
}}this.__bN.process(bC);
var bD=this.__bL;

for(var by in bD){bD[by].process(bC);
}},__bQ:function(bE){if(bE===undefined){return y;
...
},__bR:function(bG,bH){var bO=this.__bQ(bG);
var bK=D;
var bJ=[];
...
for(var i=0;i<bQ.length;i++){bP.__bP(bQ[i][0],bQ[i][1]);
...
(function(){var k="keypress",j="focusout",h="__kD",g="activate",f="Tab",d="singleton",c="deactivate",b="focusin",a="qx.ui.core.FocusHandler";
qx.Class.define(a,{extend:qx.core.Object,type:d,construct:function(){qx.core.Object.call(this);
this.__kD={};
},members:{__kD:null,__kE:null,__kF:null,__kG:null,connectTo:function(m){m.addListener(k,this.__kH,this);
...
if(s!=this.__kG){this.__kG=s;
}},_onDeactivate:function(e){var u=e.getTarget();

if(this.__kE==u){this.__kE=null;
}},_onFocusIn:function(e){var v=e.getTarget();

if(v!=this.__kF){this.__kF=v;
v.visualizeFocus();
}},_onFocusOut:function(e){var w=e.getTarget();

if(w==this.__kF){this.__kF=null;
w.visualizeBlur();
}},__kH:function(e){if(e.getKeyIdentifier()!=f){return;
}
if(!this.__kG){return;
}e.stopPropagation();
e.preventDefault();
...
if(!e.isShiftPressed()){var y=x?this.__kM(x):this.__kK();
}else{var y=x?this.__kN(x):this.__kL();
}if(y){y.tabFocus();
}},__kI:function(z){var A=this.__kD;

while(z){if(A[z.$$hash]){return z;
}z=z.getLayoutParent();
}return null;
},__kJ:function(B,C){if(B===C){return 0;
...
},__kL:function(){return this.__kR(this.__kG,null);
},__kM:function(M){var N=this.__kG;

if(N==M){return this.__kK();
}
...
O.sort(this.__kJ);
var P=O.length;
return P>0?O[0]:this.__kK();
},__kN:function(Q){var R=this.__kG;

if(R==Q){return this.__kL();
}
...
S.sort(this.__kJ);
var T=S.length;
return T>0?S[T-1]:this.__kL();
},__kO:function(parent,U,V){var W=parent.getLayoutChildren();
var X;

for(var i=0,l=W.length;i<l;i++){X=W[i];
if(!(X instanceof qx.ui.core.Widget)){continue;
}
if(!this.isFocusRoot(X)&&X.isEnabled()&&X.isVisible()){if(X.isTabable()&&this.__kJ(U,X)<0){V.push(X);
}this.__kO(X,U,V);
}}},__kP:function(parent,Y,ba){var bb=parent.getLayoutChildren();
var bc;

for(var i=0,l=bb.length;i<l;i++){bc=bb[i];
if(!(bc instanceof qx.ui.core.Widget)){continue;
}
if(!this.isFocusRoot(bc)&&bc.isEnabled()&&bc.isVisible()){if(bc.isTabable()&&this.__kJ(Y,bc)>0){ba.push(bc);
}this.__kP(bc,Y,ba);
}}},__kQ:function(parent,bd){var be=parent.getLayoutChildren();
var bf;

for(var i=0,l=be.length;i<l;i++){bf=be[i];
if(!(bf instanceof qx.ui.core.Widget)){continue;
}if(!this.isFocusRoot(bf)&&bf.isEnabled()&&bf.isVisible()){if(bf.isTabable()){if(bd==null||this.__kJ(bf,bd)<0){bd=bf;
}}bd=this.__kQ(bf,bd);
}}return bd;
},__kR:function(parent,bg){var bh=parent.getLayoutChildren();
var bi;

for(var i=0,l=bh.length;i<l;i++){bi=bh[i];
if(!(bi instanceof qx.ui.core.Widget)){continue;
}if(!this.isFocusRoot(bi)&&bi.isEnabled()&&bi.isVisible()){if(bi.isTabable()){if(bg==null||this.__kJ(bi,bg)>0){bg=bi;
}}bg=this.__kR(bi,bg);
}}return bg;
}},destruct:function(){this._disposeMap(h);
this.__kF=this.__kE=this.__kG=null;
}});
})();
...
},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{appear:true,disappear:true},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:true,__hF:{},refresh:function(){var e=this.__hF;

for(var f in e){e[f].refresh();
}}},members:{__hD:null,__hE:null,canHandleEvent:function(g,h){},registerEvent:function(i,j,k){var l=qx.core.ObjectRegistry.toHashCode(i)+j;
var m=this.__hE;
...
if(r[q]){delete r[q];
}},refresh:function(){var v=this.__hE;
var w;
...
qx.Bootstrap.define(g,{statics:{STANDARD_MODE:false,QUIRKS_MODE:false,CONTENT_BOX:false,BORDER_BOX:false,SVG:false,CANVAS:!!window.CanvasRenderingContext2D,VML:false,XPATH:!!document.evaluate,AIR:navigator.userAgent.indexOf("adobeair")!==-1,GEARS:!!(window.google&&window.google.gears),SSL:window.location.protocol==="https:",ECMA_OBJECT_COUNT:(({}).__count__==0),CSS_POINTER_EVENTS:false,XUL:false,CSS_TEXT_OVERFLOW:("textOverflow" in document.documentElement.style||"OTextOverflow" in document.documentElement.style),HTML5_CLASSLIST:!!(document.documentElement.classList&&qx.Bootstrap.getClass(document.documentElement.classList)==="DOMTokenList"),TOUCH:("ontouchstart" in window),PLACEHOLDER:false,__dh:function(){this.QUIRKS_MODE=this.__di();
...
}}},__di:function(){if(qx.bom.client.Engine.MSHTML&&qx.bom.client.Engine.VERSION>=8){return qx.bom.client.Engine.DOCUMENT_MODE===5;
}else{return document.compatMode!==f;
}}},defer:function(h){h.__dh();
}});
})();
(function(){var i="",h="/",g="mshtml",f="qx.client",e="//",d="?",c="string",b="qx.util.ResourceManager",a="singleton";
qx.Class.define(b,{extend:qx.core.Object,type:a,statics:{__iL:qx.$$resources||{},__iM:{}},members:{has:function(j){return !!this.self(arguments).__iL[j];
},getData:function(k){return this.self(arguments).__iL[k]||null;
},getImageWidth:function(l){var m=this.self(arguments).__iL[l];
return m?m[0]:null;
},getImageHeight:function(n){var o=this.self(arguments).__iL[n];
return o?o[1]:null;
},getImageFormat:function(p){var q=this.self(arguments).__iL[p];
return q?q[2]:null;
},isClippedImage:function(r){var s=this.self(arguments).__iL[r];
return s&&s.length>4;
},toUri:function(t){if(t==null){return t;
}var u=this.self(arguments).__iL[t];
...
if(qx.core.Variant.isSet(f,g)&&qx.bom.client.Feature.SSL){v=this.self(arguments).__iM[w];
...
continue;
}if(z.match(/^\/\//)!=null){x.__iM[B]=window.location.protocol;
}else if(z.match(/^\//)!=null){x.__iM[B]=window.location.protocol+e+window.location.host;
}else if(z.match(/^\.\//)!=null){var y=document.URL;
x.__iM[B]=y.substring(0,y.lastIndexOf(h)+1);
}else if(z.match(/^http/)!=null){x.__iM[B]=i;
}else{var C=window.location.href.indexOf(d);
var A;

if(C==-1){A=window.location.href;
}else{A=window.location.href.substring(0,C);
}x.__iM[B]=A.substring(0,A.lastIndexOf(h)+1);
}}}}}});
})();
...
(function(){var h="object",g="__cW",f="_applyTheme",e="qx.theme.manager.Decoration",d="Theme",c="changeTheme",b="string",a="singleton";
qx.Class.define(e,{type:a,extend:qx.core.Object,properties:{theme:{check:d,nullable:true,apply:f,event:c}},members:{__cW:null,resolve:function(i){if(!i){return null;
}
...
qx.Class.define(j,{statics:{__eF:qx.core.Variant.select(q,{"mshtml":null,"webkit":[o,h,f],"gecko":[a],"opera":[o]}),__eG:qx.core.Variant.select(q,{"mshtml":null,"webkit":[n,e,b],"gecko":[g],"opera":[n]}),__eH:{tags:{button:true,select:true},types:{search:true,button:true,submit:true,reset:true,checkbox:true,radio:true}},__eI:function(r){var s=this.__eH;
return s.tags[r.tagName.toLowerCase()]||s.types[r.type];
},compile:qx.core.Variant.select(q,{"mshtml":function(t){{};
},"default":function(u){var w=this.__eG;
var v=p;

if(w){for(var i=0,l=w.length;i<l;i++){v+=w[i]+m+u+c;
}}return v;
}}),get:qx.core.Variant.select(q,{"mshtml":function(x){if(qx.bom.Document.isStandardMode(qx.dom.Node.getDocument(x))){if(!this.__eI(x)){return d;
}}return k;
},"default":function(y){var A=this.__eF;
var z;
...
},"default":function(D,E){var F=this.__eF;
...
qx.Class.define(l,{statics:{__eJ:null,getScrollbarWidth:function(){if(this.__eJ!==null){return this.__eJ;
...
qx.Class.define(c,{statics:{__eK:qx.core.Variant.select(e,{"mshtml":{"cursor":a,"ew-resize":j,"ns-resize":k,"nesw-resize":h,"nwse-resize":i},"opera":{"col-resize":j,"row-resize":k,"ew-resize":j,"ns-resize":k,"nesw-resize":h,"nwse-resize":i},"default":{}}),compile:function(l){return f+(this.__eK[l]||l)+d;
},get:function(m,n){return qx.bom.element.Style.get(m,b,n,false);
},set:function(o,p){o.style.cursor=this.__eK[p]||p;
...
qx.Class.define(y,{statics:{__eL:function(){var G=[a,h,q,c];
var K={};
...
this.__eM[n]=qx.core.Variant.select(k,{"gecko":f,"webkit":v,"default":h});
this.__eN={};

for(var I in K){this.__eN[I]=this.__eR(K[I]);
}this.__eM[d]=qx.core.Variant.select(k,{"mshtml":b,"default":C});
},__eO:{width:r,height:F,left:A,right:t,top:B,bottom:s},__eP:{clip:qx.bom.element.Clip,cursor:qx.bom.element.Cursor,opacity:qx.bom.element.Opacity,boxSizing:qx.bom.element.BoxSizing,overflowX:{set:qx.lang.Function.bind(qx.bom.element.Overflow.setX,qx.bom.element.Overflow),get:qx.lang.Function.bind(qx.bom.element.Overflow.getX,qx.bom.element.Overflow),reset:qx.lang.Function.bind(qx.bom.element.Overflow.resetX,qx.bom.element.Overflow),compile:qx.lang.Function.bind(qx.bom.element.Overflow.compileX,qx.bom.element.Overflow)},overflowY:{set:qx.lang.Function.bind(qx.bom.element.Overflow.setY,qx.bom.element.Overflow),get:qx.lang.Function.bind(qx.bom.element.Overflow.getY,qx.bom.element.Overflow),reset:qx.lang.Function.bind(qx.bom.element.Overflow.resetY,qx.bom.element.Overflow),compile:qx.lang.Function.bind(qx.bom.element.Overflow.compileY,qx.bom.element.Overflow)}},compile:function(O){var Q=[];
var S=this.__eP;
var R=this.__eN;
var name,P;
...
}else{Q.push(this.__eR(name),D,P,p);
}}return Q.join(m);
},__eQ:{},__eR:function(T){var U=this.__eQ;
var V=U[T];
...
}}),isPropertySupported:function(bd){return (this.__eP[bd]||this.__eM[bd]||bd in document.documentElement.style);
},COMPUTED_MODE:1,CASCADED_MODE:2,LOCAL_MODE:3,set:function(be,name,bf,bg){{};
name=this.__eM[name]||name;
if(bg!==false&&this.__eP[name]){return this.__eP[name].set(be,bf);
...
}}}},reset:function(bp,name,bq){name=this.__eM[name]||name;
if(bq!==false&&this.__eP[name]){return this.__eP[name].reset(bp);
}else{bp.style[name]=m;
}},get:qx.core.Variant.select(k,{"mshtml":function(br,name,bs,bt){name=this.__eM[name]||name;
if(bt!==false&&this.__eP[name]){return this.__eP[name].get(br,bs);
...
}},"default":function(by,name,bz,bA){name=this.__eM[name]||name;
if(bA!==false&&this.__eP[name]){return this.__eP[name].get(by,bz);
...
G.set(F);
return G;
},__da:{fontFamily:k,fontSize:k,fontWeight:k,fontStyle:k,textDecoration:k,lineHeight:1.2},getDefaultStyles:function(){return this.__da;
}},properties:{size:{check:x,nullable:true,apply:m},lineHeight:{check:q,nullable:true,apply:v},family:{check:u,nullable:true,apply:w},bold:{check:h,nullable:true,apply:a},italic:{check:h,nullable:true,apply:b},decoration:{check:[j,s,t],nullable:true,apply:p}},members:{__db:null,__dc:null,__dd:null,__de:null,__df:null,__dg:null,_applySize:function(H,I){this.__db=H===null?null:H+g;
},_applyLineHeight:function(J,K){this.__dg=J===null?null:J;
...
},_applyBold:function(O,P){this.__dd=O===null?null:O?c:d;
},_applyItalic:function(Q,R){this.__de=Q===null?null:Q?e:d;
},_applyDecoration:function(S,T){this.__df=S===null?null:S;
},getStyles:function(){return {fontFamily:this.__dc,fontSize:this.__db,fontWeight:this.__dd,fontStyle:this.__de,textDecoration:this.__df,lineHeight:this.__dg};
}}});
})();
...
},__hL:function(n){n=n==null?0:n;
return this.__hM()[n];
},__hM:function(){var o=(this._isTouchEnd()?this.getChangedTargetTouches():this.getTargetTouches());
return o;
...
j.target=i.target;
return j;
},__hN:{0:h,2:g,1:f},__hO:{1:h,2:g,4:f},stop:function(){this.stopPropagation();
},getButton:function(){switch(this._type){case c:return g;
case d:if(this.__hP){return this.__hP();
}default:if(this._native.target!==undefined){return this.__hN[this._native.button]||e;
}else{return this.__hO[this._native.button]||e;
}}},__hP:qx.core.Variant.select(a,{"mshtml":function(){return h;
...
}},properties:{alignX:{check:[m,k,c],init:m,apply:n},alignY:{check:[j,g,e],init:j,apply:n},spacing:{check:a,init:0,apply:n},separator:{check:h,nullable:true,apply:n},reversed:{check:d,init:false,apply:f}},members:{__lX:null,__lY:null,__ma:null,__mb:null,_applyReversed:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__mc:function(){var w=this._getLayoutChildren();
var length=w.length;
var t=false;
...
},verifyLayoutProperty:null,renderLayout:function(x,y){if(this._invalidChildrenCache){this.__mc();
}var E=this.__mb;
var length=E.length;
...
U+=K;
D=z.getMarginRight();
}},_computeSizeHint:function(){if(this._invalidChildrenCache){this.__mc();
...
}},destruct:function(){this.__lX=this.__lY=this.__mb=null;
}});
})();
(function(){var b="qx.ui.core.DecoratorFactory",a="$$nopool$$";
qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__eb={};
},statics:{MAX_SIZE:15,__ec:a},members:{__eb:null,getDecoratorElement:function(c){var h=qx.ui.core.DecoratorFactory;
...
}},destruct:function(){if(!qx.core.ObjectRegistry.inShutDown){var q=this.__eb;
...
this.__cr=c;
this.__cs=d||b;
this.__ct=e===undefined?-1:e;
},members:{__cr:null,__cs:null,__ct:null,toString:function(){return this.__cr;
},getUri:function(){return this.__cs;
},getLineNumber:function(){return this.__ct;
}}});
})();
(function(){var m="number",l="0",k="px",j=";",i="background-image:url(",h=");",g="",f=")",e="background-repeat:",d=" ",a="qx.bom.element.Background",c="url(",b="background-position:";
qx.Class.define(a,{statics:{__ju:[i,null,h,b,null,j,e,null,j],__jv:{backgroundImage:null,backgroundPosition:null,backgroundRepeat:null},__jw:function(n,top){var o=qx.bom.client.Engine;
...
},compile:function(r,s,t,top){var u=this.__jw(t,top);
...
w[7]=s;
return w.join(g);
},getStyles:function(x,y,z,top){if(!x){return this.__jv;
}var A=this.__jw(z,top);
...
this.__fg=qx.lang.Function.bind(this.__fk,this);
this.__fh=false;
...
this.__fi=null;
}},__fk:qx.event.GlobalError.observeMethod(function(){this.__fi=null;
...
this.__fl=b;
this.__fm=c||null;
this.__fn=qx.util.DeferredCallManager.getInstance();
},members:{__fl:null,__fm:null,__fn:null,cancel:function(){this.__fn.cancel(this);
},schedule:function(){this.__fn.schedule(this);
},call:function(){this.__fm?this.__fl.apply(this.__fm):this.__fl();
}},destruct:function(d,e){this.cancel();
this.__fm=this.__fl=this.__fn=null;
}});
})();
(function(){var m="element",k="qx.client",j="qxSelectable",h="off",g="on",f="text",d="div",c="",b="mshtml",a="none",F="scroll",E="qx.html.Element",D="|capture|",C="__fK",B="activate",A="blur",z="deactivate",w="capture",v="userSelect",u="-moz-none",s="visible",t="releaseCapture",q="|bubble|",r="tabIndex",o="focus",p="MozUserSelect",n="hidden";
qx.Class.define(E,{extend:qx.core.Object,construct:function(G,H,I){qx.core.Object.call(this);
this.__fo=G||d;
this.__fp=H||null;
this.__fq=I||null;
},statics:{DEBUG:false,_modified:{},_visibility:{},_scroll:{},_actions:[],__fr:{},_scheduleFlush:function(J){qx.html.Element.__fV.schedule();
},flush:function(){var U;
{};
var M=this.__fs();
var L=M.getFocus();

if(L&&this.__fw(L)){M.blur(L);
}var bc=M.getActive();

if(bc&&this.__fw(bc)){qx.bom.Element.deactivate(bc);
}var P=this.__fu();

if(P&&this.__fw(P)){qx.bom.Element.releaseCapture(P);
}var V=[];
var W=this._modified;

for(var T in W){U=W[T];
if(U.__fO()){if(U.__fx&&qx.dom.Hierarchy.isRendered(U.__fx)){V.push(U);
}else{{};
U.__fN();
}delete W[T];
}}
...
if(!U.$$disposed){X.style.display=U.__fA?c:a;
if(qx.core.Variant.isSet(k,b)){if(!(document.documentMode>=8)){X.style.visibility=U.__fA?s:n;
}}}delete R[T];
...
if(U.__fD!=null){U.__fx.scrollLeft=U.__fD;
delete U.__fD;
}if(U.__fE!=null){U.__fx.scrollTop=U.__fE;
delete U.__fE;
}var Y=U.__fB;
...
var X=bb.element.__fx;

if(!X||!N[bb.type]&&!bb.element.__fO()){continue;
}var Q=bb.args;
Q.unshift(X);
qx.bom.Element[bb.type].apply(qx.bom.Element,Q);
}this._actions=[];
for(var T in this.__fr){var K=this.__fr[T];
var bd=K.element.__fx;

if(bd){qx.bom.Selection.set(bd,K.start,K.end);
delete this.__fr[T];
}}qx.event.handler.Appear.refresh();
},__fs:function(){if(!this.__ft){var be=qx.event.Registration.getManager(window);
this.__ft=be.getHandler(qx.event.handler.Focus);
}return this.__ft;
},__fu:function(){if(!this.__fv){var bf=qx.event.Registration.getManager(window);
this.__fv=bf.getDispatcher(qx.event.dispatch.MouseCapture);
}return this.__fv.getCaptureElement();
},__fw:function(bg){var bh=qx.core.ObjectRegistry.fromHashCode(bg.$$element);
return bh&&!bh.__fO();
}},members:{__fo:null,__fx:null,__fy:false,__fz:true,__fA:true,__fB:null,__fC:null,__fD:null,__fE:null,__fF:null,__fG:null,__fH:null,__fp:null,__fq:null,__fI:null,__fJ:null,__fK:null,__fL:null,__fM:null,_scheduleChildrenUpdate:function(){if(this.__fL){return;
}this.__fL=true;
qx.html.Element._modified[this.$$hash]=this;
qx.html.Element._scheduleFlush(m);
},_createDomElement:function(){return qx.bom.Element.create(this.__fo);
},__fN:function(){{};
var length;
var bi=this.__fK;
...
if(bj.__fA&&bj.__fz&&!bj.__fx){bj.__fN();
}}}
if(!this.__fx){this.__fx=this._createDomElement();
this.__fx.$$element=this.$$hash;
...
},_insertChildren:function(){var bk=this.__fK;
...
if(bm.__fx&&bm.__fz){bl.appendChild(bm.__fx);
}}this.__fx.appendChild(bl);
}else{var bl=this.__fx;
...
if(!bs||!bs.__fz||bs.__fM!==this){bq.removeChild(bv);
{};
}}for(var i=0;i<bu;i++){bo=bn[i];
if(bo.__fz){bs=bo.__fx;
bv=bt[bp];
...
},_copyData:function(bx){var bB=this.__fx;
var bA=this.__fq;
...
}},__fO:function(){var bL=this;
while(bL){if(bL.__fy){return true;
}
if(!bL.__fz||!bL.__fA){return false;
}bL=bL.__fM;
}return false;
},__fP:function(bM){if(bM.__fM===this){throw new Error("Child is already in: "+bM);
}
if(bM.__fy){throw new Error("Root elements could not be inserted into other ones.");
}if(bM.__fM){bM.__fM.remove(bM);
}bM.__fM=this;
if(!this.__fK){this.__fK=[];
}if(this.__fx){this._scheduleChildrenUpdate();
}},__fQ:function(bN){if(bN.__fM!==this){throw new Error("Has no child: "+bN);
}if(this.__fx){this._scheduleChildrenUpdate();
}delete bN.__fM;
},__fR:function(bO){if(bO.__fM!==this){throw new Error("Has no child: "+bO);
}if(this.__fx){this._scheduleChildrenUpdate();
}},getChildren:function(){return this.__fK||null;
},getChild:function(bP){var bQ=this.__fK;
return bQ&&bQ[bP]||null;
},hasChildren:function(){var bR=this.__fK;
return bR&&bR[0]!==undefined;
},indexOf:function(bS){var bT=this.__fK;
return bT?bT.indexOf(bS):-1;
},hasChild:function(bU){var bV=this.__fK;
return bV&&bV.indexOf(bU)!==-1;
},add:function(bW){if(arguments[1]){for(var i=0,l=arguments.length;i<l;i++){this.__fP(arguments[i]);
}this.__fK.push.apply(this.__fK,arguments);
}else{this.__fP(bW);
this.__fK.push(bW);
}return this;
},addAt:function(bX,bY){this.__fP(bX);
qx.lang.Array.insertAt(this.__fK,bX,bY);
return this;
},remove:function(ca){var cb=this.__fK;
...
},removeAt:function(cd){var ce=this.__fK;
...
qx.lang.Array.removeAt(this.__fK,cd);
return this;
},removeAll:function(){var cg=this.__fK;

if(cg){for(var i=0,l=cg.length;i<l;i++){this.__fQ(cg[i]);
}cg.length=0;
}return this;
},getParent:function(){return this.__fM||null;
},insertInto:function(parent,ch){parent.__fP(this);

if(ch==null){parent.__fK.push(this);
}else{qx.lang.Array.insertAt(this.__fK,this,ch);
}return this;
},insertBefore:function(ci){var parent=ci.__fM;
parent.__fP(this);
qx.lang.Array.insertBefore(parent.__fK,this,ci);
return this;
},insertAfter:function(cj){var parent=cj.__fM;
parent.__fP(this);
qx.lang.Array.insertAfter(parent.__fK,this,cj);
return this;
},moveTo:function(ck){var parent=this.__fM;
parent.__fR(this);
var cl=parent.__fK.indexOf(this);

if(cl===ck){throw new Error("Could not move to same index!");
}else if(cl<ck){ck--;
}qx.lang.Array.removeAt(parent.__fK,cl);
qx.lang.Array.insertAt(parent.__fK,this,ck);
return this;
},moveBefore:function(cm){var parent=this.__fM;
return this.moveTo(parent.__fK.indexOf(cm));
},moveAfter:function(cn){var parent=this.__fM;
return this.moveTo(parent.__fK.indexOf(cn)+1);
...
},setRoot:function(co){this.__fy=co;
},useMarkup:function(cp){if(this.__fx){throw new Error("Could not overwrite existing element!");
...
return this.__fx;
},useElement:function(cr){if(this.__fx){throw new Error("Could not overwrite existing element!");
}this.__fx=cr;
this.__fx.$$element=this.$$hash;
this._copyData(true);
...
if(ct>=0&&cs[this.__fo]){return true;
}return false;
...
},include:function(){if(this.__fz){return;
}delete this.__fz;

if(this.__fM){this.__fM._scheduleChildrenUpdate();
}return this;
},exclude:function(){if(!this.__fz){return;
}this.__fz=false;

if(this.__fM){this.__fM._scheduleChildrenUpdate();
}return this;
},isIncluded:function(){return this.__fz===true;
},show:function(){if(this.__fA){return;
}
if(this.__fx){qx.html.Element._visibility[this.$$hash]=this;
qx.html.Element._scheduleFlush(m);
}if(this.__fM){this.__fM._scheduleChildrenUpdate();
}delete this.__fA;
...
},getScrollX:function(){var cJ=this.__fx;

if(cJ){return cJ.scrollLeft;
}return this.__fD||0;
},scrollToY:function(y,cK){var cL=this.__fx;
...
},getScrollY:function(){var cM=this.__fx;
...
this.scrollToX(0);
this.scrollToY(0);
this.addListener(F,this.__fT,this);
},enableScrolling:function(){this.removeListener(F,this.__fT,this);
},__fS:null,__fT:function(e){if(!this.__fS){this.__fS=true;
this.__fx.scrollTop=0;
this.__fx.scrollLeft=0;
delete this.__fS;
}},getTextSelection:function(){var cN=this.__fx;

if(cN){return qx.bom.Selection.get(cN);
}return null;
},getTextSelectionLength:function(){var cO=this.__fx;

if(cO){return qx.bom.Selection.getLength(cO);
}return null;
},getTextSelectionStart:function(){var cP=this.__fx;

if(cP){return qx.bom.Selection.getStart(cP);
}return null;
},getTextSelectionEnd:function(){var cQ=this.__fx;

if(cQ){return qx.bom.Selection.getEnd(cQ);
}return null;
},setTextSelection:function(cR,cS){var cT=this.__fx;

if(cT){qx.bom.Selection.set(cT,cR,cS);
return;
}qx.html.Element.__fr[this.toHashCode()]={element:this,start:cR,end:cS};
qx.html.Element._scheduleFlush(m);
},clearTextSelection:function(){var cU=this.__fx;

if(cU){qx.bom.Selection.clear(cU);
}delete qx.html.Element.__fr[this.toHashCode()];
},__fU:function(cV,cW){var cX=qx.html.Element._actions;
cX.push({type:cV,element:this,args:cW||[]});
qx.html.Element._scheduleFlush(m);
},focus:function(){this.__fU(o);
},blur:function(){this.__fU(A);
},activate:function(){this.__fU(B);
},deactivate:function(){this.__fU(z);
},capture:function(cY){this.__fU(w,[cY!==false]);
},releaseCapture:function(){this.__fU(t);
},setStyle:function(da,dc,dd){if(!this.__fp){this.__fp={};
}
if(this.__fp[da]==dc){return;
}
if(dc==null){delete this.__fp[da];
}else{this.__fp[da]=dc;
}if(this.__fx){if(dd){qx.bom.element.Style.set(this.__fx,da,dc);
return this;
...
if(this.__fx){if(!this.__fF){this.__fF={};
}
for(var di in de){var dh=de[di];

if(this.__fp[di]==dh){continue;
}
if(dh==null){delete this.__fp[di];
}else{this.__fp[di]=dh;
}if(df){dg.set(this.__fx,di,dh);
continue;
}this.__fF[di]=true;
}qx.html.Element._modified[this.$$hash]=this;
qx.html.Element._scheduleFlush(m);
...
},getStyle:function(dl){return this.__fp?this.__fp[dl]:null;
},getAllStyles:function(){return this.__fp||null;
},setAttribute:function(dm,dn,dp){if(!this.__fq){this.__fq={};
}
if(this.__fq[dm]==dn){return;
}
if(dn==null){delete this.__fq[dm];
}else{this.__fq[dm]=dn;
}if(this.__fx){if(dp){qx.bom.element.Attribute.set(this.__fx,dm,dn);
return this;
...
},getAttribute:function(dv){return this.__fq?this.__fq[dv]:null;
},_applyProperty:function(name,dw){},_setProperty:function(dx,dy,dz){if(!this.__fI){this.__fI={};
}
if(this.__fI[dx]==dy){return;
}
if(dy==null){delete this.__fI[dx];
}else{this.__fI[dx]=dy;
}if(this.__fx){if(dz){this._applyProperty(dx,dy);
return this;
...
},_getProperty:function(dC){var dD=this.__fI;
...
if(this.__fx){return qx.event.Registration.addListener(this.__fx,dF,dG,self,dH);
}
...
var dK=dF+(dH?D:q)+dJ;
this.__fJ[dK]={type:dF,listener:dG,self:self,capture:dH,unique:dJ};
return dK;
...
if(this.__fx){qx.event.Registration.removeListener(this.__fx,dL,dM,self,dN);
}else{var dQ=this.__fJ;
var dP;
...
if(this.__fx){qx.event.Registration.removeListenerById(this.__fx,dS);
}else{delete this.__fJ[dS];
}return this;
},hasListener:function(dT,dU){if(this.$$disposed){return false;
}
if(this.__fx){return qx.event.Registration.hasListener(this.__fx,dT,dU);
}var dW=this.__fJ;
var dV;
...
}},defer:function(dY){dY.__fV=new qx.util.DeferredCall(dY.flush,dY);
},destruct:function(){var ea=this.__fx;
...
this.__fq=this.__fp=this.__fJ=this.__fI=this.__fG=this.__fF=this.__fH=this.__fx=this.__fM=this.__fB=this.__fC=null;
}});
})();
...
this.__dj={};
this.__dk={};
},properties:{theme:{check:d,nullable:true,event:c,apply:g}},members:{__dl:{},__dj:null,__dk:null,_applyTheme:function(j,k){this.__dk={};
this.__dj={};
},__dm:function(l,m,n){var s=m.appearances;
var v=s[l];
...
return this.__dm(r,m,n);
}}}for(var i=0;i<p.length-1;i++){p.shift();
var q=p.join(w);
var o=this.__dm(q,m);

if(o){return o;
}}if(n!=null){return this.__dm(n,m);
}return null;
}else if(typeof v===h){return this.__dm(v,m,n);
}else if(v.include&&!v.style){return this.__dm(v.include,m,n);
}return l;
},styleFrom:function(x,y,z,A){if(!z){z=this.getTheme();
}var G=this.__dk;
var B=G[x];

if(!B){B=G[x]=this.__dm(x,z,A);
...
qx.Class.define(a,{extend:qx.html.Element,members:{__jX:null,_applyProperty:function(name,c){qx.html.Element.prototype._applyProperty.call(this,name,c);

if(name==b){var d=this.getDomElement();
qx.bom.Label.setValue(d,c);
}},_createDomElement:function(){var f=this.__jX;
...
if(E!=null&&E!==this.__dS){this.__dS=E;
...
G.width=B;
G.height=C;
}if(this.__dU){F.local=true;
delete this.__dU;
}
if(this.__dW){F.margin=true;
delete this.__dW;
}return F;
},isExcluded:function(){return false;
},hasValidLayout:function(){return !this.__dU;
},scheduleLayoutUpdate:function(){qx.ui.core.queue.Layout.add(this);
},invalidateLayoutCache:function(){this.__dU=true;
this.__dV=null;
},getSizeHint:function(H){var I=this.__dV;

if(I){return I;
}
if(H===false){return null;
}I=this.__dV=this._computeSizeHint();
if(this._hasHeightForWidth()&&this.__dS&&this.getHeight()==null){I.height=this.__dS;
...
},_applyMargin:function(){this.__dW=true;
...
},hasUserBounds:function(){return !!this.__dX;
},setUserBounds:function(S,top,T,U){this.__dX={left:S,top:top,width:T,height:U};
qx.ui.core.queue.Layout.add(this);
},resetUserBounds:function(){delete this.__dX;
qx.ui.core.queue.Layout.add(this);
},__ea:{},setLayoutProperties:function(V){if(V==null){return;
}var W=this.__dY;
...
}}},getLayoutProperties:function(){return this.__dY||this.__ea;
},clearLayoutProperties:function(){delete this.__dY;
...
if(bd){bc.__dY=qx.lang.Object.clone(bd);
}return bc;
}},destruct:function(){this.$$parent=this.$$subparent=this.__dY=this.__dT=this.__dX=this.__dV=null;
}});
})();
...
(function(){var bV="px",bU="Boolean",bT="qx.event.type.Drag",bS="qx.event.type.Mouse",bR="visible",bQ="qx.event.type.Focus",bP="on",bO="Integer",bN="qx.event.type.Touch",bM="excluded",bx="qx.event.type.Data",bw="_applyPadding",bv="qx.event.type.Event",bu="hidden",bt="contextmenu",bs="String",br="tabIndex",bq="focused",bp="changeVisibility",bo="mshtml",cd="hovered",ce="qx.event.type.KeySequence",cb="qx.client",cc="absolute",bY="backgroundColor",ca="drag",bW="div",bX="disabled",cf="move",cg="dragstart",bF="qx.dynlocale",bE="dragchange",bH="dragend",bG="resize",bJ="Decorator",bI="zIndex",bL="opacity",bK="default",bD="Color",bC="changeToolTipText",c="beforeContextmenuOpen",d="_applyNativeContextMenu",f="_applyBackgroundColor",g="_applyFocusable",h="changeShadow",j="__ej",k="qx.event.type.KeyInput",m="createChildControl",n="Font",o="_applyShadow",ck="__ei",cj="_applyEnabled",ci="_applySelectable",ch="Number",co="_applyKeepActive",cn="_applyVisibility",cm="repeat",cl="qxDraggable",cq="syncAppearance",cp="paddingLeft",N="_applyDroppable",O="__eq",L="__eh",M="#",R="qx.event.type.MouseWheel",S="_applyCursor",P="_applyDraggable",Q="changeTextColor",J="$$widget",K="changeContextMenu",w="paddingTop",v="changeSelectable",y="hideFocus",x="none",s="outline",r="_applyAppearance",u="_applyOpacity",t="url(",q=")",p="qx.ui.core.Widget",X="_applyFont",Y="cursor",ba="qxDroppable",bb="changeZIndex",T="changeEnabled",U="changeFont",V="__ee",W="_applyDecorator",bc="_applyZIndex",bd="_applyTextColor",G="qx.ui.menu.Menu",F="__eo",E="__em",D="_applyToolTipText",C="__ed",B="true",A="widget",z="changeDecorator",I="_applyTabIndex",H="changeAppearance",be="shorthand",bf="/",bg="",bh="_applyContextMenu",bi="paddingBottom",bj="changeNativeContextMenu",bk="qx.ui.tooltip.ToolTip",bl="qxKeepActive",bm="_applyKeepFocus",bn="paddingRight",bB="changeBackgroundColor",bA="changeLocale",bz="qxKeepFocus",by="qx/static/blank.gif";
...
if(cM.size){var cL=this.__ej;

if(cL){cL.setStyles({width:cC+bV,height:cD+bV});
}}
if(cM.size||this._updateInsets){if(this.__eh){this.__eh.resize(cC,cD);
}}
if(cM.size){if(this.__ei){var cE=this.__ei.getInsets();
var cI=cC+cE.left+cE.right;
var cG=cD+cE.top+cE.bottom;
this.__ei.resize(cI,cG);
}}
if(cJ||cM.local||cM.margin){if(this.__em&&this.hasLayoutChildren()){this.__em.renderLayout(innerWidth,innerHeight);
...
return cM;
},__eo:null,clearSeparators:function(){var cP=this.__eo;

if(!cP){return;
}var cQ=qx.ui.core.Widget.__ef;
var content=this.getContentElement();
var cO;
...
},renderSeparator:function(cR,cS){var cT=qx.ui.core.Widget.__ef.getDecoratorElement(cR);
this.getContentElement().add(cT);
cT.resize(cS.width,cS.height);
cT.setStyles({left:cS.left+bV,top:cS.top+bV});
if(!this.__eo){this.__eo=[cT];
}else{this.__eo.push(cT);
...
}},_getContentHint:function(){var dg=this.__em;
...
if(this.__eh){var ds=this.__eh.getInsets();
{};
top+=ds.top;
dt+=ds.right;
dv+=ds.bottom;
...
},__ep:function(){var dC=this._createContentElement();
{};
...
},__eq:null,getLayoutChildren:function(){var dE=this.__eq;

if(!dE){return this.__er;
}var dF;
...
},invalidateLayoutChildren:function(){var dG=this.__em;

if(dG){dG.invalidateChildrenCache();
}qx.ui.core.queue.Layout.add(this);
},hasLayoutChildren:function(){var dH=this.__eq;
...
},_indexOf:function(dJ){var dK=this.__eq;

if(!dK){return -1;
}return dK.indexOf(dJ);
},_hasChildren:function(){var dL=this.__eq;
return dL!=null&&(!!dL[0]);
},addChildrenToQueue:function(dM){var dN=this.__eq;
...
}},_add:function(dP,dQ){if(dP.getLayoutParent()==this){qx.lang.Array.remove(this.__eq,dP);
}
if(this.__eq){this.__eq.push(dP);
}else{this.__eq=[dP];
}this.__es(dP,dQ);
},_addAt:function(dR,dS,dT){if(!this.__eq){this.__eq=[];
}if(dR.getLayoutParent()==this){qx.lang.Array.remove(this.__eq,dR);
}var dU=this.__eq[dS];

if(dU===dR){dR.setLayoutProperties(dT);
}
if(dU){qx.lang.Array.insertBefore(this.__eq,dR,dU);
}else{this.__eq.push(dR);
}this.__es(dR,dT);
...
}if(dV.getLayoutParent()==this){qx.lang.Array.remove(this.__eq,dV);
}qx.lang.Array.insertBefore(this.__eq,dV,dW);
this.__es(dV,dX);
...
if(!this.__eq){this.__eq=[];
}if(dY.getLayoutParent()==this){qx.lang.Array.remove(this.__eq,dY);
}qx.lang.Array.insertAfter(this.__eq,dY,ea);
this.__es(dY,eb);
},_remove:function(ec){if(!this.__eq){throw new Error("This widget has no children!");
}qx.lang.Array.remove(this.__eq,ec);
this.__et(ec);
},_removeAt:function(ed){if(!this.__eq){throw new Error("This widget has no children!");
}var ee=this.__eq[ed];
qx.lang.Array.removeAt(this.__eq,ed);
this.__et(ee);
return ee;
},_removeAll:function(){if(!this.__eq){return;
}var ef=this.__eq.concat();
this.__eq.length=0;

for(var i=ef.length-1;i>=0;i--){this.__et(ef[i]);
}qx.ui.core.queue.Layout.add(this);
},_afterAddChild:null,_afterRemoveChild:null,__es:function(eg,eh){{};
...
}var en=this.__ej=new qx.html.Element;
{};
en.setStyles({position:cc,top:0,left:0,zIndex:7});
var eo=this.getBounds();

if(eo){this.__ej.setStyles({width:eo.width+bV,height:eo.height+bV});
...
var et=qx.ui.core.Widget.__ef;
var er=this.getContainerElement();
if(!this.__ej&&!qx.bom.client.Feature.CSS_POINTER_EVENTS){this._createProtectorElement();
}if(eq){er.remove(this.__eh);
et.poolDecorator(this.__eh);
}if(ep){var es=this.__eh=et.getDecoratorElement(ep);
es.setStyle(bI,5);
er.add(es);
}else{delete this.__eh;
}this._applyBackgroundColor(this.getBackgroundColor());
if(this.__en(eq,ep)){this._updateInsets=true;
qx.ui.core.queue.Layout.add(this);
}else if(ep){var eu=this.getBounds();

if(eu){es.resize(eu.width,eu.height);
this.__ej&&
this.__ej.setStyles({width:eu.width+bV,height:eu.height+bV});
}}},_applyShadow:function(ev,ew){var eD=qx.ui.core.Widget.__eg;
var ey=this.getContainerElement();
if(ew){ey.remove(this.__ei);
eD.poolDecorator(this.__ei);
}if(ev){var eA=this.__ei=eD.getDecoratorElement(ev);
ey.add(eA);
var eC=eA.getInsets();
...
}},_applyToolTipText:function(eE,eF){if(qx.core.Variant.isSet(bF,bP)){if(this.__el){return;
}var eG=qx.locale.Manager.getInstance();
this.__el=eG.addListener(bA,function(){if(eE&&eE.translate){this.setToolTipText(eE.translate());
}},this);
...
if(this.__eh){this.__eh.tint(eV);
eX.setStyle(bY,null);
}else{var eW=qx.theme.manager.Color.getInstance().resolve(eV);
eX.setStyle(bY,eW);
}},_applyFont:function(eY,fa){},__eu:null,$$stateChanges:null,_forwardStates:null,hasState:function(fb){var fc=this.__eu;
return !!fc&&!!fc[fb];
},addState:function(fd){var fe=this.__eu;
...
}}}},removeState:function(fi){var fj=this.__eu;
...
}}}},replaceState:function(fn,fo){var fp=this.__eu;
...
}}}},__ev:null,__ew:null,syncAppearance:function(){var fx=this.__eu;
var fw=this.__ev;
...
fw=fB.reverse().join(bf).replace(/#[0-9]+/g,bg);
this.__ev=fw;
...
}}this.fireDataEvent(cq,this.__eu);
...
if(qx.core.Variant.isSet(cb,bo)){fM.setAttribute(y,B);
...
},hasChildControl:function(gs){if(!this.__ex){return false;
}return !!this.__ex[gs];
},__ex:null,_getCreatedChildControls:function(){return this.__ex;
},getChildControl:function(gt,gu){if(!this.__ex){if(gu){return null;
}this.__ex={};
}var gv=this.__ex[gt];
...
},_createChildControl:function(gC){if(!this.__ex){this.__ex={};
}else if(this.__ex[gC]){throw new Error("Child control '"+gC+"' already created!");
}var gG=gC.indexOf(M);

if(gG==-1){var gD=this._createChildControlImpl(gC);
}else{var gD=this._createChildControlImpl(gC.substring(0,gG),gC.substring(gG+1,gC.length));
}
...
}}}this.fireDataEvent(m,gD);
return this.__ex[gC]=gD;
},_createChildControlImpl:function(gH,gI){return null;
},_disposeChildControls:function(){var gM=this.__ex;
...
},getContainerLocation:function(gO){var gP=this.getContainerElement().getDomElement();
return gP?qx.bom.element.Location.get(gP,gO):null;
},getContentLocation:function(gQ){var gR=this.getContentElement().getDomElement();
return gR?qx.bom.element.Location.get(gR,gQ):null;
},setDomLeft:function(gS){var gT=this.getContainerElement().getDomElement();

if(gT){gT.style.left=gS+bV;
}else{throw new Error("DOM element is not yet created!");
}},setDomTop:function(gU){var gV=this.getContainerElement().getDomElement();

if(gV){gV.style.top=gU+bV;
}else{throw new Error("DOM element is not yet created!");
}},setDomPosition:function(gW,top){var gX=this.getContainerElement().getDomElement();

if(gX){gX.style.left=gW+bV;
gX.style.top=top+bV;
}else{throw new Error("DOM element is not yet created!");
...
for(var i=0,l=ha.length;i<l;i++){gY.add(ha[i].clone());
}}return gY;
}},destruct:function(){if(!qx.core.ObjectRegistry.inShutDown){if(qx.core.Variant.isSet(bF,bP)){if(this.__el){qx.locale.Manager.getInstance().removeListenerById(this.__el);
...
if(this.__eh){hb.remove(this.__eh);
hc.__ef.poolDecorator(this.__eh);
}
if(this.__ei){hb.remove(this.__ei);
hc.__eg.poolDecorator(this.__ei);
}this.clearSeparators();
this.__eh=this.__ei=this.__eo=null;
}else{this._disposeArray(F);
this._disposeObjects(L,ck);
}this._disposeArray(O);
this.__eu=this.__ex=null;
this._disposeObjects(E,C,V,j);
}});
})();
...
},statics:{PRIORITY:qx.event.Registration.PRIORITY_FIRST,SUPPORTED_TYPES:{mousemove:1,mouseover:1,mouseout:1,mousedown:1,mouseup:1,click:1,dblclick:1,contextmenu:1,mousewheel:1,keyup:1,keydown:1,keypress:1,keyinput:1,capture:1,losecapture:1,focusin:1,focusout:1,focus:1,blur:1,activate:1,deactivate:1,appear:1,disappear:1,dragstart:1,dragend:1,dragover:1,dragleave:1,drop:1,drag:1,dragchange:1,droprequest:1,touchstart:1,touchend:1,touchmove:1,touchcancel:1,tap:1,swipe:1},IGNORE_CAN_HANDLE:false},members:{__iI:null,__iJ:{focusin:1,focusout:1,focus:1,blur:1},__iK:{mouseover:1,mouseout:1,appear:1,disappear:1},canHandleEvent:function(g,h){return g instanceof qx.ui.core.Widget;
...
}if(this.__iJ[j.getType()]){o=o&&o.getFocusTarget();
if(!o){return;
...
if(w){if(this.__iJ[j.getType()]){w=w.getFocusTarget();
}if(w===o){return;
}}}var s=j.getCurrentTarget();
var u=qx.ui.core.Widget.getWidgetByElement(s);

if(!u||u.isAnonymous()){return;
}if(this.__iJ[j.getType()]){u=u.getFocusTarget();
}var v=j.getType();

if(!u||!(u.isEnabled()||this.__iK[v])){return;
}var k=j.getEventPhase()==qx.event.type.Event.CAPTURING_PHASE;
var r=this.__iI.getListeners(u,v,k);
...
},members:{$$isString:true,length:0,__iO:null,toString:function(){return this.__iO;
...
qx.Mixin.define(b,{members:{__nr:function(m,n,o,p){var q=this.getChildrenContainer();

if(q===this){m=h+m;
}return (q[m])(n,o,p);
},getChildren:function(){return this.__nr(c);
},hasChildren:function(){return this.__nr(f);
},add:function(r,s){return this.__nr(j,r,s);
},remove:function(t){return this.__nr(a,t);
},removeAll:function(){return this.__nr(d);
},indexOf:function(u){return this.__nr(l,u);
},addAt:function(v,w,x){this.__nr(g,v,w,x);
},addBefore:function(y,z,A){this.__nr(i,y,z,A);
},addAfter:function(B,C,D){this.__nr(k,B,C,D);
},removeAt:function(E){this.__nr(e,E);
}}});
})();
...
m.start();
return m;
}},properties:{enabled:{init:true,check:c,apply:d},interval:{check:a,init:1000,apply:f}},members:{__dK:null,__dI:null,_applyInterval:function(n,o){if(this.getEnabled()){this.restart();
}},_applyEnabled:function(p,q){if(q){window.clearInterval(this.__dK);
this.__dK=null;
}else if(p){this.__dK=window.setInterval(this.__dI,this.getInterval());
...
qx.Class.define(b,{extend:qx.event.type.Event,members:{__hJ:null,__hK:null,init:function(d,e){qx.event.type.Event.prototype.init.call(this,false,false);
this.__hJ=d;
this.__hK=e;
return this;
...
g.__hK=this.__hK;
return g;
},getOrientation:function(){return this.__hJ;
},isLandscape:function(){return this.__hK==c;
},isPortrait:function(){return this.__hK==a;
}}});
})();
...
qx.Class.define(e,{statics:{NAME:"",WIN:false,MAC:false,UNIX:false,UNKNOWN_PLATFORM:false,__cP:function(){var o=navigator.platform;
...
qx.Bootstrap.define(w,{statics:{NAME:"",SP1:false,SP2:false,WIN95:false,WIN98:false,WINME:false,WINNT4:false,WIN2000:false,WINXP:false,WIN2003:false,WINVISTA:false,WIN7:false,WINCE:false,LINUX:false,SUNOS:false,FREEBSD:false,NETBSD:false,OPENBSD:false,OSX:false,OS9:false,SYMBIAN:false,NINTENDODS:false,PSP:false,IPHONE:false,IPAD:false,UNKNOWN_SYSTEM:false,__cQ:{"Windows NT 6.1":u,"Windows NT 6.0":n,"Windows NT 5.2":s,"Windows NT 5.1":H,"Windows NT 5.0":e,"Windows 2000":e,"Windows NT 4.0":B,"Win 9x 4.90":z,"Windows CE":A,"Windows 98":j,"Win98":j,"Windows 95":f,"Win95":f,"Linux":p,"FreeBSD":G,"NetBSD":m,"OpenBSD":k,"SunOS":F,"Symbian System":v,"Nitro":C,"PSP":"sonypsp","Mac OS X 10_5":c,"Mac OS X 10.5":c,"Mac OS X 10_4":g,"Mac OS X 10.4":g,"Mac OS X 10_3":b,"Mac OS X 10.3":b,"Mac OS X 10_2":i,"Mac OS X 10.2":i,"Mac OS X 10_1":d,"Mac OS X 10.1":d,"Mac OS X 10_0":h,"Mac OS X 10.0":h,"Mac OS X":o,"Mac OS 9":q},__cR:function(){var L=navigator.userAgent;
var K=[];

for(var J in this.__cQ){K.push(J);
...
this.__gH=q;
this.__gI=q.getWindow();
this.__gJ=this.__gI.document;
this._initButtonObserver();
this._initMoveObserver();
this._initWheelObserver();
},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{mousemove:1,mouseover:1,mouseout:1,mousedown:1,mouseup:1,click:1,dblclick:1,contextmenu:1,mousewheel:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:true},members:{__gK:null,__gL:null,__gM:null,__gN:null,__gO:null,__gH:null,__gI:null,__gJ:null,canHandleEvent:function(r,s){},registerEvent:qx.bom.client.System.IPHONE?
function(t,u,v){t[c+u]=qx.lang.Function.returnNull;
}:qx.lang.Function.returnNull,unregisterEvent:qx.bom.client.System.IPHONE?
function(w,x,y){w[c+x]=undefined;
}:qx.lang.Function.returnNull,__gP:function(z,A,B){if(!B){B=qx.bom.Event.getTarget(z);
...
}qx.event.Registration.fireEvent(this.__gI,e,qx.event.type.Data,[A||z.type]);
},__gQ:function(){var D=[this.__gI,this.__gJ,this.__gJ.body];
var E=this.__gI;
var C=d;
...
},_initButtonObserver:function(){this.__gK=qx.lang.Function.listener(this._onButtonEvent,this);
var Event=qx.bom.Event;
Event.addNativeListener(this.__gJ,m,this.__gK);
Event.addNativeListener(this.__gJ,p,this.__gK);
Event.addNativeListener(this.__gJ,o,this.__gK);
Event.addNativeListener(this.__gJ,j,this.__gK);
Event.addNativeListener(this.__gJ,l,this.__gK);
},_initMoveObserver:function(){this.__gL=qx.lang.Function.listener(this._onMoveEvent,this);
var Event=qx.bom.Event;
Event.addNativeListener(this.__gJ,f,this.__gL);
Event.addNativeListener(this.__gJ,h,this.__gL);
Event.addNativeListener(this.__gJ,g,this.__gL);
},_initWheelObserver:function(){this.__gM=qx.lang.Function.listener(this._onWheelEvent,this);
var F=this.__gQ();
qx.bom.Event.addNativeListener(F.target,F.type,this.__gM);
},_stopButtonObserver:function(){var Event=qx.bom.Event;
Event.removeNativeListener(this.__gJ,m,this.__gK);
Event.removeNativeListener(this.__gJ,p,this.__gK);
Event.removeNativeListener(this.__gJ,o,this.__gK);
Event.removeNativeListener(this.__gJ,j,this.__gK);
Event.removeNativeListener(this.__gJ,l,this.__gK);
},_stopMoveObserver:function(){var Event=qx.bom.Event;
Event.removeNativeListener(this.__gJ,f,this.__gL);
Event.removeNativeListener(this.__gJ,h,this.__gL);
Event.removeNativeListener(this.__gJ,g,this.__gL);
},_stopWheelObserver:function(){var G=this.__gQ();
qx.bom.Event.removeNativeListener(G.target,G.type,this.__gM);
},_onMoveEvent:qx.event.GlobalError.observeMethod(function(H){this.__gP(H);
...
if(this.__gR){this.__gR(I,J,K);
}
if(this.__gT){this.__gT(I,J,K);
}this.__gP(I,J,K);

if(this.__gS){this.__gS(I,J,K);
}
if(this.__gU){this.__gU(I,J,K);
}this.__gN=J;
}),_onWheelEvent:qx.event.GlobalError.observeMethod(function(L){this.__gP(L,k);
}),__gR:qx.core.Variant.select(n,{"webkit":function(M,N,O){if(qx.bom.client.Engine.VERSION<530){if(N==l){this.__gP(M,p,O);
}}},"default":null}),__gS:qx.core.Variant.select(n,{"opera":function(P,Q,R){if(Q==p&&P.button==2){this.__gP(P,l,R);
}},"default":null}),__gT:qx.core.Variant.select(n,{"mshtml":function(S,T,U){if(S.target!==undefined){return;
}
if(T==p&&this.__gN==o){this.__gP(S,m,U);
}else if(T==j){this.__gP(S,o,U);
}},"default":null}),__gU:qx.core.Variant.select(n,{"mshtml":null,"default":function(V,W,X){switch(W){case m:this.__gO=X;
break;
case p:if(X!==this.__gO){var Y=qx.dom.Hierarchy.getCommonParent(X,this.__gO);
this.__gP(V,o,Y);
}}}})},destruct:function(){this._stopButtonObserver();
this._stopMoveObserver();
this._stopWheelObserver();
this.__gH=this.__gI=this.__gJ=this.__gO=null;
...
if(q){this.setSeparator(q);
}},properties:{alignY:{check:[m,j,e],init:m,apply:n},alignX:{check:[k,g,a],init:k,apply:n},spacing:{check:c,init:0,apply:n},separator:{check:h,nullable:true,apply:n},reversed:{check:b,init:false,apply:f}},members:{__lF:null,__lG:null,__lH:null,__lI:null,_applyReversed:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__lJ:function(){var w=this._getLayoutChildren();
var length=w.length;
var s=false;
var r=this.__lF&&this.__lF.length!=length&&this.__lG&&this.__lF;
var u;
var t=r?this.__lF:new Array(length);
var v=r?this.__lG:new Array(length);
if(this.getReversed()){w=w.concat().reverse();
}for(var i=0;i<length;i++){u=w[i].getLayoutProperties();
...
this.__lI=w;
delete this._invalidChildrenCache;
},verifyLayoutProperty:null,renderLayout:function(x,y){if(this._invalidChildrenCache){this.__lJ();
}var F=this.__lI;
var length=F.length;
...
for(i=0;i<length;i+=1){J=this.__lF[i];
B=J!=null?Math.floor((y-C)*J):F[i].getSizeHint().height;
K.push(B);
Q+=B;
}if(this.__lH&&Q!=y){var H={};
var N,R;

for(i=0;i<length;i+=1){N=this.__lG[i];
...
top+=B;
I=A.getMarginBottom();
}},_computeSizeHint:function(){if(this._invalidChildrenCache){this.__lJ();
}var bc=qx.ui.layout.Util;
var bk=this.__lI;
var X=0,bb=0,ba=0;
var V=0,bd=0;
...
}},destruct:function(){this.__lF=this.__lG=this.__lI=null;
}});
})();
...
(function(){var j="nonScaled",i="scaled",h="alphaScaled",g=".png",f="qx.client",e="div",d="replacement",c="qx.event.type.Event",b="hidden",a="Boolean",y="px",x="scale",w="changeSource",v="qx.ui.basic.Image",u="loaded",t="-disabled.$1",s="loadingFailed",r="String",q="_applySource",p="img",n="image",o="mshtml",l="_applyScale",m="__jx",k="no-repeat";
qx.Class.define(v,{extend:qx.ui.core.Widget,construct:function(z){this.__jx={};
qx.ui.core.Widget.call(this);

if(z){this.setSource(z);
}},properties:{source:{check:r,init:null,nullable:true,event:w,apply:q,themeable:true},scale:{check:a,init:false,themeable:true,apply:l},appearance:{refine:true,init:n},allowShrinkX:{refine:true,init:false},allowShrinkY:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false}},events:{loadingFailed:c,loaded:c},members:{__jy:null,__jz:null,__jA:null,__jx:null,getContentElement:function(){return this.__jE();
},_createContentElement:function(){return this.__jE();
},_getContentHint:function(){return {width:this.__jy||0,height:this.__jz||0};
...
},__jB:function(E){this.__jA=E;
},__jC:function(){if(this.__jA==null){var G=this.getSource();
var F=false;

if(G!=null){F=qx.lang.String.endsWith(G,g);
}
if(this.getScale()&&F&&qx.bom.element.Decoration.isAlphaImageLoaderEnabled()){this.__jA=h;
}else if(this.getScale()){this.__jA=i;
}else{this.__jA=j;
}}return this.__jA;
},__jD:function(H){var I;
var J;
...
return K;
},__jE:function(){var L=this.__jC();

if(this.__jx[L]==null){this.__jx[L]=this.__jD(L);
}return this.__jx[L];
...
}if(qx.util.ResourceManager.getInstance().has(M)){this.__jH(this.getContentElement(),M);
}else if(qx.io.ImageLoader.isLoaded(M)){this.__jI(this.getContentElement(),M);
}else{this.__jJ(this.getContentElement(),M);
}},__jF:qx.core.Variant.select(f,{"mshtml":function(O){var Q=qx.bom.element.Decoration.isAlphaImageLoaderEnabled();
var P=qx.lang.String.endsWith(O,g);

if(Q&&P){if(this.getScale()&&this.__jC()!=h){this.__jB(h);
}else if(!this.getScale()&&this.__jC()!=j){this.__jB(j);
}}else{if(this.getScale()&&this.__jC()!=i){this.__jB(i);
}else if(!this.getScale()&&this.__jC()!=j){this.__jB(j);
}}this.__jG(this.__jE());
},"default":function(R){if(this.getScale()&&this.__jC()!=i){this.__jB(i);
}else if(!this.getScale()&&this.__jC(j)){this.__jB(j);
}this.__jG(this.__jE());
}}),__jG:function(S){var V=this.getContainerElement();
var W=V.getChild(0);
...
V.addAt(S,0);
}},__jH:function(ba,bb){var bd=qx.util.ResourceManager.getInstance();
...
this.__jL(bd.getImageWidth(bb),bd.getImageHeight(bb));
},__jI:function(be,bf){var bh=qx.io.ImageLoader;
be.setSource(bf);
var bg=bh.getWidth(bf);
var bi=bh.getHeight(bf);
this.__jL(bg,bi);
},__jJ:function(bj,bk){var self;
var bl=qx.io.ImageLoader;
{};
if(!bl.isFailed(bk)){bl.load(bk,this.__jK,this);
}else{if(bj!=null){bj.resetSource();
}}},__jK:function(bm,bn){if(this.$$disposed===true){return;
...
},__jL:function(bo,bp){if(bo!==this.__jy||bp!==this.__jz){this.__jy=bo;
this.__jz=bp;
qx.ui.core.queue.Layout.add(this);
...
qx.Mixin.define(w,{statics:{__dL:null,setVisibleElement:function(y){this.__dL=y;
},getVisibleElement:function(){return this.__dL;
}},properties:{position:{check:[r,o,d,a,v,l,k,n],init:d,themeable:true},placeMethod:{check:[x,e],init:e,themeable:true},domMove:{check:b,init:false},placementModeX:{check:[c,h,f],init:h,themeable:true},placementModeY:{check:[c,h,f],init:h,themeable:true},offsetLeft:{check:j,init:0,themeable:true},offsetTop:{check:j,init:0,themeable:true},offsetRight:{check:j,init:0,themeable:true},offsetBottom:{check:j,init:0,themeable:true},offset:{group:[m,u,p,s],mode:t,themeable:true}},members:{__dM:null,__dN:null,__dO:null,getLayoutLocation:function(z){var C,B,D,top;
B=z.getBounds();
D=B.left;
top=B.top;
var E=B;
...
}},placeToWidget:function(L,M){if(M){this.__dP();
this.__dM=qx.lang.Function.bind(this.placeToWidget,this,L,false);
qx.event.Idle.getInstance().addListener(i,this.__dM);
this.__dO=function(){this.__dP();
};
this.addListener(g,this.__dO,this);
}var N=L.getContainerLocation()||this.getLayoutLocation(L);
this.__dR(N);
},__dP:function(){if(this.__dM){qx.event.Idle.getInstance().removeListener(i,this.__dM);
this.__dM=null;
}
if(this.__dO){this.removeListener(g,this.__dO,this);
this.__dO=null;
...
if(R){this.__dM=qx.lang.Function.bind(this.placeToElement,this,Q,false);
qx.event.Idle.getInstance().addListener(i,this.__dM);
this.addListener(g,function(){if(this.__dM){qx.event.Idle.getInstance().removeListener(i,this.__dM);
this.__dM=null;
}},this);
}this.__dR(S);
...
},__dQ:function(V){var W=null;

if(this._computePlacementSize){var W=this._computePlacementSize();
}else if(this.isVisible()){var W=this.getBounds();
}
if(W==null){this.addListenerOnce(q,function(){this.__dQ(V);
},this);
}else{V.call(this,W);
}},__dR:function(X){this.__dQ(function(Y){var ba=qx.util.placement.Placement.compute(Y,this.getLayoutParent().getBounds(),X,this._getPlacementOffsets(),this.getPosition(),this.getPlacementModeX(),this.getPlacementModeY());
this.moveTo(ba.left,ba.top);
});
}},destruct:function(){this.__dP();
}});
})();
...
qx.Class.define(G,{statics:{__cM:{"Array":[f,p,E,q,z,d,a,y,g,h,k,j,n,v,b,D,o],"String":[H,A,w,t,C,r,k,j,x,e,F,I,m,u,B,c,g,h]},__cN:function(J,K){return function(s){return J.prototype[K].apply(s,Array.prototype.slice.call(arguments,1));
};
},__cO:function(){var L=qx.lang.Generics.__cM;
...
if(!N[O]){N[O]=qx.lang.Generics.__cN(N,O);
...
qx.Mixin.define(d,{construct:function(){if(qx.core.Variant.isSet(m,i)){qx.locale.Manager.getInstance().addListener(k,this.__md,this);
...
},__md:qx.core.Variant.select(m,{"on":function(e){var p=this.getInvalidMessage();
...
}},"off":null})},destruct:function(){if(qx.core.Variant.isSet(m,i)){qx.locale.Manager.getInstance().removeListener(k,this.__md,this);
}}});
})();
...
}},properties:{size:{check:a,init:Infinity}},members:{__cu:null,getObject:function(d){if(this.$$disposed){return new d;
}
...
},poolObject:function(g){if(!this.__cu){return;
}var h=g.classname;
var j=this.__cu[h];
...
qx.Class.define(v,{statics:{DEBUG_ON:false,__cv:{},bind:function(E,F,G,H,I){var S=this.__cx(E,F,G,H,I);
var N=F.split(d);
var K=this.__cE(N);
var R=[];
var O=[];
...
var Q=qx.lang.Function.bind(this.__cw,this,T);
O.push(Q);
...
this.__cG(U,E,F,G,H);
return U;
},__cw:function(W){if(W.options&&W.options.onUpdate){W.options.onUpdate(W.sources[W.index],W.targetObject);
...
if(!bb){this.__cA(W.targetObject,W.targetPropertyChain);
break;
...
var Y=bb.getItem(bc);
this.__cD(Y,W.targetObject,W.targetPropertyChain,W.options,W.sources[W.index]);
W.listenerIds[j]=this.__cF(bb,g,W.targetObject,W.targetPropertyChain,W.options,W.arrayIndexValues[j]);
...
this.__cD(Y,W.targetObject,W.targetPropertyChain,W.options,W.sources[W.index]);
}var ba=this.__cz(bb,W.propertyNames[j]);
W.listenerIds[j]=this.__cF(bb,ba,W.targetObject,W.targetPropertyChain,W.options);
}}else{if(W.listeners[j]==null){var X=qx.lang.Function.bind(this.__cw,this,W);
W.listeners.push(X);
}if(qx.Class.implementsInterface(bb,qx.data.IListData)){var ba=g;
}else{var ba=this.__cz(bb,W.propertyNames[j]);
}W.listenerIds[j]=bb.addListener(ba,W.listeners[j]);
}}},__cx:function(bd,be,bf,bg,bh){var bl=bg.split(d);
var bj=this.__cE(bl);
var bq=[];
var bp=[];
var bn=[];
...
}else{try{bm.push(this.__cz(bk,bl[i]));
}catch(e){break;
}}bq[i]=bk;
...
}else{try{var bs=qx.data.SingleValueBinding.__cz(bt,bl[j]);
}catch(e){break;
...
},updateTarget:function(bv,bw,bx,by,bz){var bD=this.__cC(bv,bw);
...
}}bB=qx.data.SingleValueBinding.__cH(bB,bx,by,bz);
this.__cB(bx,by,bB);
},__cz:function(bG,bH){var bI=this.__cI(bG,bH);
...
}}},__cC:function(bV,bW){var ca=bW.split(d);
var cb=bV;
...
},__cD:function(cd,ce,cf,cg,ch){cd=this.__cH(cd,ce,cf,cg);
if(cd===undefined){this.__cA(ce,cf);
}if(cd!==undefined){try{this.__cB(ce,cf,cd);
...
},__cF:function(cl,cm,cn,co,cp,cq){var cr;
{};
var ct=function(cu,e){if(cu!==l){if(cu===h){cu=cl.length-1;
}var cx=cl.getItem(cu);
if(cx===undefined){qx.data.SingleValueBinding.__cA(cn,co);
...
}cx=qx.data.SingleValueBinding.__cH(cx,cn,co,cp);
if(qx.data.SingleValueBinding.DEBUG_ON){qx.log.Logger.debug("Data after conversion: "+cx);
}try{if(cx!==undefined){qx.data.SingleValueBinding.__cB(cn,co,cx);
}else{qx.data.SingleValueBinding.__cA(cn,co);
...
return cs;
},__cG:function(cy,cz,cA,cB,cC){if(this.__cv[cz.toHashCode()]===undefined){this.__cv[cz.toHashCode()]=[];
}this.__cv[cz.toHashCode()].push([cy,cz,cA,cB,cC]);
},__cH:function(cD,cE,cF,cG){if(cG&&cG.converter){var cI;

if(cE.getModel){cI=cE.getModel();
}return cG.converter(cD,cI);
}else{var cK=this.__cC(cE,cF);
...
return this.__cJ(cD,cH);
}},__cI:function(cM,cN){var cO=qx.Class.getPropertyDefinition(cM.constructor,cN);

if(cO==null){return null;
}return cO.event;
},__cJ:function(cP,cQ){var cR=qx.lang.Type.getClass(cP);
...
}var cU=this.__cv[cS.toHashCode()];
...
var cW=this.__cv[cV.toHashCode()];

if(cW!=undefined){for(var i=cW.length-1;i>=0;i--){this.removeBindingFromObject(cV,cW[i][0]);
}}},getAllBindingsForObject:function(cX){if(this.__cv[cX.toHashCode()]===undefined){this.__cv[cX.toHashCode()]=[];
}return this.__cv[cX.toHashCode()];
},removeAllBindings:function(){for(var da in this.__cv){var cY=qx.core.ObjectRegistry.fromHashCode(da);
if(cY==null){delete this.__cv[da];
continue;
}this.removeAllBindingsForObject(cY);
}this.__cv={};
},getAllBindings:function(){return this.__cv;
},showBindingInLog:function(db,dc){var de;
for(var i=0;i<this.__cv[db.toHashCode()].length;i++){if(this.__cv[db.toHashCode()][i][0]==dc){de=this.__cv[db.toHashCode()][i];
break;
}}
if(de===undefined){var dd=r;
}else{var dd=s+de[1]+A+de[2]+y+de[3]+A+de[4]+o;
}qx.log.Logger.debug(dd);
},showAllBindingsInLog:function(){for(var dg in this.__cv){var df=qx.core.ObjectRegistry.fromHashCode(dg);

for(var i=0;i<this.__cv[dg].length;i++){this.showBindingInLog(df,this.__cv[dg][i][0]);
}}}}});
})();
...
}else if(this.isRgbString(m)){return this.__cS();
}else if(this.isHex3String(m)){return this.__cU();
}else if(this.isHex6String(m)){return this.__cV();
}throw new Error("Could not parse color: "+m);
},cssStringToRgb:function(n){if(this.isNamedColor(n)){return this.NAMED[n];
}else if(this.isSystemColor(n)){throw new Error("Could not convert system colors to RGB: "+n);
}else if(this.isRgbString(n)){return this.__cS();
}else if(this.isRgbaString(n)){return this.__cT();
...
},__cS:function(){var D=parseInt(RegExp.$1,10);
var C=parseInt(RegExp.$2,10);
var B=parseInt(RegExp.$3,10);
return [D,C,B];
},__cT:function(){var G=parseInt(RegExp.$1,10);
var F=parseInt(RegExp.$2,10);
var E=parseInt(RegExp.$3,10);
return [G,F,E];
},__cU:function(){var J=parseInt(RegExp.$1,16)*17;
...
return [M,L,K];
},hex3StringToRgb:function(N){if(this.isHex3String(N)){return this.__cU(N);
}throw new Error("Invalid hex3 value: "+N);
},hex6StringToRgb:function(O){if(this.isHex6String(O)){return this.__cV(O);
}throw new Error("Invalid hex6 value: "+O);
},hexStringToRgb:function(P){if(this.isHex3String(P)){return this.__cU(P);
}
if(this.isHex6String(P)){return this.__cV(P);
...
},add:function(o,p){this.__cX[o]=p;
var r=this._getDynamic();
for(var q in r){if(q.substring(0,q.indexOf(j))===o){r[q]=p+q.substring(o.length);
}}},remove:function(s){delete this.__cX[s];
...
}},properties:{rich:{check:j,init:false,event:s,apply:q},wrap:{check:j,init:true,apply:C},value:{check:u,apply:x,event:m,nullable:true},buddy:{check:c,apply:v,nullable:true,init:null,dereference:true},textAlign:{check:[l,w,r],nullable:true,themeable:true,apply:d,event:a},appearance:{refine:true,init:o},selectable:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false},allowShrinkY:{refine:true,init:false}},members:{__jR:null,__jS:null,__jT:null,__jU:null,_getContentHint:function(){if(this.__jS){this.__jV=this.__jW();
delete this.__jS;
...
}},__jV:{width:0,height:0},_applyFont:function(K,L){var M;

if(K){this.__jR=qx.theme.manager.Font.getInstance().resolve(K);
M=this.__jR.getStyles();
}else{this.__jR=null;
M=qx.bom.Font.getDefaultStyles();
}this.getContentElement().setStyles(M);
this.__jS=true;
qx.ui.core.queue.Layout.add(this);
},__jW:function(N){var R=qx.bom.Label;
var P=this.getFont();
var O=P?this.__jR.getStyles():qx.bom.Font.getDefaultStyles();
var content=this.getValue()||B;
var Q=this.getRich();
return Q?R.getHtmlSize(content,O,N):R.getTextSize(content,O);
},_applyBuddy:function(S,T){if(T!=null){T.removeBinding(this.__jT);
this.__jT=null;
this.removeListenerById(this.__jU);
this.__jU=null;
}
if(S!=null){this.__jT=S.bind(g,this,g);
this.__jU=this.addListener(n,function(){if(S.isFocusable()){S.focus.apply(S);
}},this);
...
}if(this.__jT!=null){var bb=this.getBuddy();

if(bb!=null&&!bb.isDisposed()){bb.removeBinding(this.__jT);
...
this.__gV=bv;
this.__gW=bv.getWindow();
if(qx.core.Variant.isSet(l,bs)){this.__gX=this.__gW;
}else{this.__gX=this.__gW.document.documentElement;
}this.__gY={};
...
}}},members:{__ha:null,__gV:null,__gW:null,__gX:null,__gY:null,__hb:null,__hc:null,__hd:null,canHandleEvent:function(bx,by){},registerEvent:function(bz,bA,bB){},unregisterEvent:function(bC,bD,bE){},_fireInputEvent:function(bF,bG){var bH=this.__he();
if(bH&&bH.offsetWidth!=0){var event=qx.event.Registration.createEvent(s,qx.event.type.KeyInput,[bF,bH,bG]);
this.__gV.dispatchEvent(bH,event);
}if(this.__gW){qx.event.Registration.fireEvent(this.__gW,u,qx.event.type.Data,[s]);
}},_fireSequenceEvent:function(bI,bJ,bK){var bL=this.__he();
var bM=bI.keyCode;
var event=qx.event.Registration.createEvent(bJ,qx.event.type.KeySequence,[bI,bL,bK]);
this.__gV.dispatchEvent(bL,event);
...
}}}if(this.__gW){qx.event.Registration.fireEvent(this.__gW,u,qx.event.type.Data,[bJ]);
}},__he:function(){var bN=this.__gV.getHandler(qx.event.handler.Focus);
var bO=bN.getActive();
if(!bO||bO.offsetWidth==0){bO=bN.getFocus();
}if(!bO||bO.offsetWidth==0){bO=this.__gV.getWindow().document.body;
}return bO;
},_initKeyObserver:function(){this.__ha=qx.lang.Function.listener(this.__hf,this);
this.__hd=qx.lang.Function.listener(this.__hh,this);
var Event=qx.bom.Event;
Event.addNativeListener(this.__gX,i,this.__ha);
Event.addNativeListener(this.__gX,m,this.__ha);
Event.addNativeListener(this.__gX,k,this.__hd);
},_stopKeyObserver:function(){var Event=qx.bom.Event;
Event.removeNativeListener(this.__gX,i,this.__ha);
Event.removeNativeListener(this.__gX,m,this.__ha);
Event.removeNativeListener(this.__gX,k,this.__hd);

for(var bQ in (this.__hc||{})){var bP=this.__hc[bQ];
Event.removeNativeListener(bP.target,k,bP.callback);
}delete (this.__hc);
},__hf:qx.event.GlobalError.observeMethod(qx.core.Variant.select(l,{"mshtml":function(bR){bR=window.event||bR;
var bU=bR.keyCode;
var bS=0;
var bT=bR.type;
if(!(this.__gY[bU]==m&&bT==m)){this._idealKeyHandler(bU,bS,bT,bR);
...
if(!(this.__gY[bW]==m&&bY==m)){this._idealKeyHandler(ca,bX,bY,bV);
}this.__gY[bW]=bY;
}else{this._idealKeyHandler(ca,bX,bY,bV);
}this.__hg(bV.target,bY,ca);
...
}},"opera":function(cf){this.__hb=cf.keyCode;
this._idealKeyHandler(cf.keyCode,0,cf.type,cf);
}})),__hg:qx.core.Variant.select(l,{"gecko":function(cg,ch,ci){if(ch===m&&(ci==33||ci==34||ci==38||ci==40)&&cg.type==J&&cg.tagName.toLowerCase()===H&&cg.getAttribute(N)!==x){if(!this.__hc){this.__hc={};
}var ck=qx.core.ObjectRegistry.toHashCode(cg);

if(this.__hc[ck]){return;
}var self=this;
this.__hc[ck]={target:cg,callback:function(cl){qx.bom.Event.stopPropagation(cl);
self.__hh(cl);
}};
var cj=qx.event.GlobalError.observeMethod(this.__hc[ck].callback);
qx.bom.Event.addNativeListener(cg,k,cj);
}},"default":null}),__hh:qx.event.GlobalError.observeMethod(qx.core.Variant.select(l,{"mshtml":function(cm){cm=window.event||cm;
...
var cw=cv.type;
if(cx!=this.__hb){this._idealKeyHandler(0,this.__hb,cw,cv);
...
this.__hb=this.__gV=this.__gW=this.__gX=this.__gY=null;
...
qx.Class.define(e,{statics:{__iu:{names:{"class":b,"for":x,html:q,text:qx.core.Variant.isSet(h,l)?r:a,colspan:k,rowspan:d,valign:c,datetime:n,accesskey:f,tabindex:p,maxlength:o,readonly:g,longdesc:w,cellpadding:m,cellspacing:v,frameborder:u,usemap:s},runtime:{"html":1,"text":1},bools:{compact:1,nowrap:1,ismap:1,declare:1,noshade:1,checked:1,disabled:1,readOnly:1,multiple:1,selected:1,noresize:1,defer:1,allowTransparency:1},property:{$$html:1,$$widget:1,disabled:1,checked:1,readOnly:1,multiple:1,selected:1,value:1,maxLength:1,className:1,innerHTML:1,innerText:1,textContent:1,htmlFor:1,tabIndex:1},qxProperties:{$$widget:1,$$html:1},propertyDefault:{disabled:false,checked:false,readOnly:false,multiple:false,selected:false,value:j,className:j,innerHTML:j,innerText:j,textContent:j,htmlFor:j,tabIndex:0,maxLength:qx.core.Variant.select(h,{"mshtml":2147483647,"webkit":524288,"default":-1})},removeableProperties:{disabled:1,multiple:1,maxLength:1},original:{href:1,src:1,type:1}},compile:function(z){var A=[];
var C=this.__iu.runtime;

for(var B in z){if(!C[B]){A.push(B,t,z[B],y);
}}return A.join(j);
},get:qx.core.Variant.select(h,{"mshtml":function(D,name){var F=this.__iu;
var E;
...
},"default":function(G,name){var I=this.__iu;
var H;
...
this.__gh=f;
this.__gi=f.getWindow();
this._initObserver();
},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{orientationchange:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_WINDOW,IGNORE_CAN_HANDLE:true},members:{__gh:null,__gi:null,__gj:null,__gk:null,__gl:null,canHandleEvent:function(g,h){},registerEvent:function(i,j,k){},unregisterEvent:function(l,m,n){},_initObserver:function(){this.__gl=qx.lang.Function.listener(this._onNative,this);
this.__gj=qx.bom.Event.supportsEvent(this.__gi,e)?e:d;
var Event=qx.bom.Event;
Event.addNativeListener(this.__gi,this.__gj,this.__gl);
},_stopObserver:function(){var Event=qx.bom.Event;
Event.removeNativeListener(this.__gi,this.__gj,this.__gl);
},_onNative:qx.event.GlobalError.observeMethod(function(o){var q=qx.bom.Viewport;
var p=q.getOrientation();

if(this.__gk!=p){this.__gk=p;
var r=q.isLandscape()?c:b;
qx.event.Registration.fireEvent(this.__gi,e,qx.event.type.Orientation,[p,r]);
}})},destruct:function(){this._stopObserver();
this.__gh=this.__gi=null;
...
this.__gm=u;
this.__gn=u.getWindow();
this.__go=this.__gn.document;
this._initTouchObserver();
this._initMouseObserver();
},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{touchstart:1,touchmove:1,touchend:1,touchcancel:1,tap:1,swipe:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE+qx.event.IEventHandler.TARGET_DOCUMENT,IGNORE_CAN_HANDLE:true,MOUSE_TO_TOUCH_MAPPING:{"mousedown":"touchstart","mousemove":"touchmove","mouseup":"touchend"},SWIPE_DIRECTION:{x:["left","right"],y:["up","down"]},TAP_MAX_DISTANCE:10,SWIPE_MIN_DISTANCE:11,SWIPE_MIN_VELOCITY:0},members:{__gp:null,__gq:null,__gm:null,__gn:null,__go:null,__gr:null,__gs:null,__gt:null,__gu:null,__gv:false,__gw:null,canHandleEvent:function(v,w){},registerEvent:function(x,y,z){},unregisterEvent:function(A,B,C){},__gx:function(D){var E=qx.bom.Event.getTarget(D);
if(qx.core.Variant.isSet(l,b)){if(E&&E.nodeType==3){E=E.parentNode;
}}return E;
},__gy:function(F,G,H,I){if(!H){H=this.__gx(F);
}var G=G||F.type;

if(H&&H.nodeType){qx.event.Registration.fireEvent(H,G,I||qx.event.type.Touch,[F,H,null,true,true]);
}qx.event.Registration.fireEvent(this.__gn,j,qx.event.type.Data,[G]);
},__gz:function(J,K,L){if(!L){L=this.__gx(J);
}var K=K||J.type;

if(K==r){this.__gA(J,L);
}else if(K==q){this.__gB(J,L);
}else if(K==s){this.__gC(J,L);
}},__gA:function(M,N){var O=M.changedTouches[0];
this.__gr=O.screenX;
this.__gs=O.screenY;
this.__gt=new Date().getTime();
this.__gu=M.changedTouches.length===1;
},__gB:function(P,Q){if(this.__gu&&P.changedTouches.length>1){this.__gu=false;
}},__gC:function(R,S){if(this.__gu){var T=R.changedTouches[0];
var V={x:T.screenX-this.__gr,y:T.screenY-this.__gs};
var W=qx.event.handler.Touch;

if(this.__gw==S&&Math.abs(V.x)<=W.TAP_MAX_DISTANCE&&Math.abs(V.y)<=W.TAP_MAX_DISTANCE){this.__gy(R,f,S,qx.event.type.Tap);
}else{var U=this.__gD(R,S,V);

if(U){R.swipe=U;
this.__gy(R,h,S,qx.event.type.Swipe);
}}}},__gD:function(X,Y,ba){var be=qx.event.handler.Touch;
var bf=new Date().getTime()-this.__gt;
...
if(Math.abs(bg)>=be.SWIPE_MIN_VELOCITY&&Math.abs(bb)>=be.SWIPE_MIN_DISTANCE){bd={startTime:this.__gt,duration:bf,axis:bh,direction:bc,distance:bb,velocity:bg};
}return bd;
},__gE:qx.core.Variant.select(t,{"on":function(bi){var bj=bi.type;
var bl=qx.event.handler.Touch.MOUSE_TO_TOUCH_MAPPING;

if(bl[bj]){bj=bl[bj];
if(bj==r&&this.__gF(bi)){this.__gv=true;
}else if(bj==s){this.__gv=false;
}var bm=this.__gG(bi);
var bk=(bj==s?[]:[bm]);
bi.touches=bk;
bi.targetTouches=bk;
...
},"default":qx.lang.Function.empty}),__gF:qx.core.Variant.select(t,{"on":function(bn){if(qx.core.Variant.isSet(l,k)){var bo=1;
}else{var bo=0;
}return bn.button==bo;
},"default":qx.lang.Function.empty}),__gG:qx.core.Variant.select(t,{"on":function(bp){var bq=this.__gx(bp);
...
},"default":qx.lang.Function.empty}),_initTouchObserver:function(){this.__gp=qx.lang.Function.listener(this._onTouchEvent,this);
var Event=qx.bom.Event;
Event.addNativeListener(this.__go,r,this.__gp);
Event.addNativeListener(this.__go,q,this.__gp);
Event.addNativeListener(this.__go,s,this.__gp);
Event.addNativeListener(this.__go,o,this.__gp);
},_initMouseObserver:qx.core.Variant.select(t,{"on":function(){if(!qx.bom.client.Feature.TOUCH){this.__gq=qx.lang.Function.listener(this._onMouseEvent,this);
var Event=qx.bom.Event;
Event.addNativeListener(this.__go,m,this.__gq);
Event.addNativeListener(this.__go,p,this.__gq);
Event.addNativeListener(this.__go,n,this.__gq);
}},"default":qx.lang.Function.empty}),_stopTouchObserver:function(){var Event=qx.bom.Event;
Event.removeNativeListener(this.__go,r,this.__gp);
Event.removeNativeListener(this.__go,q,this.__gp);
Event.removeNativeListener(this.__go,s,this.__gp);
Event.removeNativeListener(this.__go,o,this.__gp);
},_stopMouseObserver:qx.core.Variant.select(t,{"on":function(){if(!qx.bom.client.Feature.TOUCH){var Event=qx.bom.Event;
Event.removeNativeListener(this.__go,m,this.__gq);
Event.removeNativeListener(this.__go,p,this.__gq);
Event.removeNativeListener(this.__go,n,this.__gq);
...
}),_onMouseEvent:qx.core.Variant.select(t,{"on":qx.event.GlobalError.observeMethod(function(bs){if(!qx.bom.client.Feature.TOUCH){if(bs.type==p&&!this.__gv){return;
}var bt=this.__gE(bs);
...
if(bv==r){this.__gw=this.__gx(bu);
}this.__gy(bu,bv);
this.__gz(bu,bv);
}},destruct:function(){this._stopTouchObserver();
this._stopMouseObserver();
this.__gm=this.__gn=this.__go=this.__gw=null;
...
qx.Class.define(y,{statics:{DEBUG:false,__ja:{},__jb:qx.core.Variant.isSet(j,c)&&qx.bom.client.Engine.VERSION<9,__jc:qx.core.Variant.select(j,{"mshtml":{"scale-x":true,"scale-y":true,"scale":true,"no-repeat":true},"default":null}),__jd:{"scale-x":h,"scale-y":h,"scale":h,"repeat":i,"no-repeat":i,"repeat-x":i,"repeat-y":i},update:function(H,I,J,K){var M=this.getTagName(J,I);
...
if(this.__jb){try{H.filters[u].apply();
...
}},getTagName:function(U,V){if(qx.core.Variant.isSet(j,c)){if(V&&this.__jb&&this.__jc[U]&&qx.lang.String.endsWith(V,E)){return i;
}}return this.__jd[U];
...
{};
var ba;
if(this.__jb&&this.__jc[X]&&bb===r){ba=this.__jg(Y,X,W);
}else{if(X===b){ba=this.__jh(Y,X,W);
}else if(X===d||X===a){ba=this.__ji(Y,X,W);
}else{ba=this.__jl(Y,X,W);
}}return ba;
},__je:function(bc,bd,be){if(bc.width==null&&bd!=null){bc.width=bd+k;
}
if(bc.height==null&&be!=null){bc.height=be+k;
}return bc;
},__jf:function(bf){var bh=qx.util.ResourceManager.getInstance().getImageWidth(bf)||qx.io.ImageLoader.getWidth(bf);
var bi=qx.util.ResourceManager.getInstance().getImageHeight(bf)||qx.io.ImageLoader.getHeight(bf);
return {width:bh,height:bi};
},__jg:function(bj,bk,bl){var bo=this.__jf(bl);
bj=this.__je(bj,bo.width,bo.height);
var bn=bk==f?D:b;
var bm=C+qx.util.ResourceManager.getInstance().toUri(bl)+x+bn+s;
bj.filter=bm;
bj.backgroundImage=bj.backgroundRepeat=g;
return {style:bj};
},__jh:function(bp,bq,br){var bs=qx.util.ResourceManager.getInstance().toUri(br);
var bt=this.__jf(br);
bp=this.__je(bp,bt.width,bt.height);
return {src:bs,style:bp};
},__ji:function(bu,bv,bw){var bA=qx.util.ResourceManager.getInstance();
...
if(bv===d){bu=this.__jj(bu,by,bB.height);
}else{bu=this.__jk(bu,by,bB.width);
...
return {src:bx,style:bu};
}},__jj:function(bC,bD,bE){var bF=qx.util.ResourceManager.getInstance().getImageHeight(bD[4]);
...
},__jk:function(bG,bH,bI){var bJ=qx.util.ResourceManager.getInstance().getImageWidth(bH[4]);
...
},__jl:function(bK,bL,bM){var bR=qx.util.ResourceManager.getInstance().isClippedImage(bM);
var bQ=this.__jf(bM);
...
bK=this.__je(bK,bQ.width,bQ.height);
bK=this.__jm(bK,bM,bL);
return {style:bK};
}},__jm:function(bS,bT,bU){var top=null;
var bY=null;
...
},__jn:function(ca){if(this.DEBUG&&qx.util.ResourceManager.getInstance().has(ca)&&ca.indexOf(G)==-1){if(!this.__ja[ca]){qx.log.Logger.debug("Potential clipped image candidate: "+ca);
this.__ja[ca]=true;
}}},isAlphaImageLoaderEnabled:qx.core.Variant.select(j,{"mshtml":function(){return qx.bom.element.Decoration.__jb;
},"default":function(){return false;
}})}});
})();
(function(){var b="appearance",a="qx.ui.core.queue.Appearance";
qx.Class.define(a,{statics:{__iE:{},remove:function(c){delete this.__iE[c.$$hash];
},add:function(d){var e=this.__iE;

if(e[d.$$hash]){return;
}e[d.$$hash]=d;
qx.ui.core.queue.Manager.scheduleFlush(b);
},has:function(f){return !!this.__iE[f.$$hash];
...
(function(){var o="top",n="right",m="bottom",l="left",k="align-start",j="qx.util.placement.AbstractAxis",i="edge-start",h="align-end",g="edge-end",f="-",c="best-fit",e="qx.util.placement.Placement",d='__eS',b="keep-align",a="direct";
qx.Class.define(e,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__eS=new qx.util.placement.DirectAxis();
},properties:{axisX:{check:j},axisY:{check:j},edge:{check:[o,n,m,l],init:o},align:{check:[o,n,m,l],init:n}},statics:{__eT:null,compute:function(p,q,r,s,t,u,v){this.__eT=this.__eT||new qx.util.placement.Placement();
var y=t.split(f);
var x=y[0];
var w=y[1];
this.__eT.set({axisX:this.__eX(u),axisY:this.__eX(v),edge:x,align:w});
return this.__eT.compute(p,q,r,s);
},__eU:null,__eV:null,__eW:null,__eX:function(z){switch(z){case a:this.__eU=this.__eU||new qx.util.placement.DirectAxis();
return this.__eU;
case b:this.__eV=this.__eV||new qx.util.placement.KeepAlignAxis();
return this.__eV;
case c:this.__eW=this.__eW||new qx.util.placement.BestFitAxis();
return this.__eW;
default:throw new Error("Invalid 'mode' argument!'");
}}},members:{__eS:null,compute:function(A,B,C,D){{};
var E=this.getAxisX()||this.__eS;
var G=E.computeStart(A.width,{start:C.left,end:C.right},{start:D.left,end:D.right},B.width,this.__eY());
var F=this.getAxisY()||this.__eS;
var top=F.computeStart(A.height,{start:C.top,end:C.bottom},{start:D.top,end:D.bottom},B.height,this.__fa());
return {left:G,top:top};
},__eY:function(){var I=this.getEdge();
var H=this.getAlign();
...
}},__fa:function(){var K=this.getEdge();
var J=this.getAlign();
...
qx.Class.define(b,{statics:{__fb:{},remove:function(c){delete this.__fb[c.$$hash];
},add:function(d){this.__fb[d.$$hash]=d;
qx.ui.core.queue.Manager.scheduleFlush(a);
},flush:function(){var e=this.__fe();
...
}}},getNestingLevel:function(j){var k=this.__fd;
var m=0;
var parent=j;
...
},__fc:function(){var s=qx.ui.core.queue.Visibility;
this.__fd={};
var r=[];
var q=this.__fb;
var n,p;
...
},__cY:function(o,p){if(o[p].include){var q=o[o[p].include];
o[p].include=null;
delete o[p].include;
...
for(var v in t){if(t[v].include&&t[t[v].include]){this.__cY(t,v);
}s[v]=(new u).set(t[v]);
s[v].themed=true;
}}this._setDynamic(s);
}}});
...
},__kf:{visible:true,hidden:true},getContentWidth:function(q){var s=qx.bom.element.Style;
var t=qx.bom.element.Overflow.getX(q);
var u=parseInt(s.get(q,d)||i,10);
var x=parseInt(s.get(q,e)||i,10);

if(this.__kf[t]){var w=q.clientWidth;
...
if(this.__kf[C]){return y.clientHeight-D-B;
...
this.__iv=i.getWindow();
this.__iw=j;
i.addListener(this.__iv,f,this.releaseCapture,this);
i.addListener(this.__iv,e,this.releaseCapture,this);
i.addListener(this.__iv,a,this.releaseCapture,this);
},statics:{PRIORITY:qx.event.Registration.PRIORITY_FIRST},members:{__iw:null,__ix:null,__iy:true,__iv:null,_getParent:function(k){return k.parentNode;
},canDispatchEvent:function(l,event,m){return (this.__ix&&this.__iz[m]);
},dispatchEvent:function(n,event,o){if(o==d){event.stopPropagation();
this.releaseCapture();
return;
}
if(this.__iy||!qx.dom.Hierarchy.contains(this.__ix,n)){n=this.__ix;
}qx.event.dispatch.AbstractBubbling.prototype.dispatchEvent.call(this,n,event,o);
},__iz:{"mouseup":1,"mousedown":1,"click":1,"dblclick":1,"mousemove":1,"mouseout":1,"mouseover":1},activateCapture:function(p,q){var q=q!==false;

if(this.__ix===p&&this.__iy==q){return;
}
if(this.__ix){this.releaseCapture();
...
this.__ix=p;
this.__iw.fireEvent(p,b,qx.event.type.Event,[true,false]);
},getCaptureElement:function(){return this.__ix;
},releaseCapture:function(){var r=this.__ix;

if(!r){return;
}this.__ix=null;
this.__iw.fireEvent(r,h,qx.event.type.Event,[true,false]);
...
},"default":qx.lang.Function.empty})},destruct:function(){this.__ix=this.__iv=this.__iw=null;
...
qx.Class.define(b,{statics:{__iB:{},__iC:{},remove:function(c){var d=c.$$hash;
delete this.__iC[d];
delete this.__iB[d];
},isVisible:function(e){return this.__iC[e.$$hash]||false;
},__iD:function(f){var h=this.__iC;
var g=f.$$hash;
var i;
if(f.isExcluded()){i=false;
}else{var parent=f.$$parent;

if(parent){i=this.__iD(parent);
...
}},properties:{spacingX:{check:h,init:0,apply:p},spacingY:{check:h,init:0,apply:p}},members:{__oW:null,__oU:null,__oV:null,__oX:null,__oY:null,__pa:null,__pb:null,__pc:null,__pd:null,verifyLayoutProperty:null,__pe:function(){var B=[];
var A=[];
var C=[];
var w=-1;
var v=-1;
...
this.__pd=null;
delete this._invalidChildrenCache;
},_setRowData:function(G,H,I){var J=this.__oU[G];

if(!J){this.__oU[G]={};
this.__oU[G][H]=I;
}else{J[H]=I;
}},_setColumnData:function(K,L,M){var N=this.__oV[K];
...
this._applyLayoutChange();
return this;
},getColumnAlign:function(S){var T=this.__oV[S]||{};
...
this._applyLayoutChange();
return this;
},getRowAlign:function(X){var Y=this.__oU[X]||{};
return {vAlign:Y.vAlign||q,hAlign:Y.hAlign||r};
},getCellWidget:function(ba,bb){if(this._invalidChildrenCache){this.__pe();
}var ba=this.__oW[ba]||{};
return ba[bb]||null;
...
for(var i=0,l=this.__oY.length;i<l;i++){var ca=this.__oY[i];
var cc=this.__pf(ca);
...
for(var i=0,l=this.__oX.length;i<l;i++){var co=this.__oX[i];
var cq=this.__pf(co);
...
for(var cJ=0;cJ<=cC;cJ++){var cB=this.__oW[cL][cJ];
...
if(this.__oY.length>0){this._fixHeightsRowSpan(cK);
}this.__pc=cK;
return cK;
...
for(var cW=0;cW<=cP;cW++){var cM=this.__oW[cW][cV];
...
if(this.__oX.length>0){this._fixWidthsColSpan(cQ);
}this.__pd=cQ;
return cQ;
...
},renderLayout:function(dm,dn){if(this._invalidChildrenCache){this.__pe();
...
for(var dx=0;dx<=dp;dx++){var dE=this.__oW[dx][dK];
if(!dE){top+=dQ[dx]+dw;
continue;
...
return dV;
}},destruct:function(){this.__oW=this.__oU=this.__oV=this.__oX=this.__oY=this.__pd=this.__pc=null;
}});
})();
...
qx.Class.define(x,{statics:{__ez:function(z,A){return qx.bom.element.Style.get(z,A,qx.bom.element.Style.COMPUTED_MODE,false);
},__eA:function(B,C){return parseInt(qx.bom.element.Style.get(B,C,qx.bom.element.Style.COMPUTED_MODE,false),10)||0;
},__eB:function(D){var G=0,top=0;
...
},__eC:qx.core.Variant.select(e,{"mshtml":function(H){var J=qx.dom.Node.getDocument(H);
var I=J.body;
var K=0;
var top=0;
...
if(qx.bom.client.Feature.STANDARD_MODE){K+=this.__eA(I,i);
top+=this.__eA(I,j);
...
var O=M.offsetLeft;
var top=M.offsetTop;
if(qx.bom.client.Engine.VERSION<530.17){O+=this.__eA(M,i);
top+=this.__eA(M,j);
}return {left:O,top:top};
},"gecko":function(P){var Q=qx.dom.Node.getDocument(P).body;
var R=Q.offsetLeft;
var top=Q.offsetTop;
if(qx.bom.client.Engine.VERSION<1.9){R+=this.__eA(Q,g);
top+=this.__eA(Q,h);
}if(qx.bom.element.BoxSizing.get(Q)!==d){R+=this.__eA(Q,i);
top+=this.__eA(Q,j);
...
}}),__eD:qx.core.Variant.select(e,{"mshtml|webkit":function(V){var X=qx.dom.Node.getDocument(V);
...
if(bd.get(bb)!==d){bf-=this.__eA(bb,i);
top-=this.__eA(bb,j);
}
...
if(bd.get(bb)!==d){bf+=this.__eA(bb,i);
top+=this.__eA(bb,j);
}if(bb.parentNode&&this.__ez(bb.parentNode,l)!=t){bf+=this.__eA(bb.parentNode,i);
top+=this.__eA(bb.parentNode,j);
}bb=bb.offsetParent;
...
}}),get:function(bj,bk){if(bj.tagName==s){var location=this.__eE(bj);
var br=location.left;
var top=location.top;
}else{var bl=this.__eC(bj);
var bq=this.__eD(bj);
var scroll=this.__eB(bj);
...
if(bo==f||bo==a){bm+=bj.scrollWidth-bj.offsetWidth+this.__eA(bj,i)+this.__eA(bj,b);
}var bp=qx.bom.element.Overflow.getY(bj);

if(bp==f||bp==a){bn+=bj.scrollHeight-bj.offsetHeight+this.__eA(bj,j)+this.__eA(bj,c);
}}
switch(bk){case y:br+=this.__eA(bj,w);
top+=this.__eA(bj,q);
bm-=this.__eA(bj,m);
bn-=this.__eA(bj,r);
...
bm-=bj.scrollLeft;
bn-=bj.scrollTop;
case k:br+=this.__eA(bj,i);
top+=this.__eA(bj,j);
bm-=this.__eA(bj,b);
bn-=this.__eA(bj,c);
break;
case o:br-=this.__eA(bj,g);
top-=this.__eA(bj,h);
bm+=this.__eA(bj,p);
bn+=this.__eA(bj,u);
break;
}}return {left:br,top:top,right:bm,bottom:bn};
},__eE:qx.core.Variant.select(e,{"default":function(bs){var top=bs.offsetTop+this.__eA(bs,h);
var bt=bs.offsetLeft+this.__eA(bs,g);
return {left:bt,top:top};
},"mshtml":function(bu){var top=bu.offsetTop;
var bv=bu.offsetLeft;

if(!((qx.bom.client.Engine.VERSION<8||qx.bom.client.Engine.DOCUMENT_MODE<8)&&!qx.bom.client.Feature.QUIRKS_MODE)){top+=this.__eA(bu,h);
bv+=this.__eA(bu,g);
}return {left:bv,top:top};
},"gecko":function(bw){var top=bw.offsetTop+this.__eA(bw,h)+this.__eA(bw,i);
var bx=bw.offsetLeft+this.__eA(bw,g)+this.__eA(bw,j);
...
qx.Class.define(c,{statics:{LOCALE:"",VARIANT:"",__iN:function(){var d=(navigator.userLanguage||navigator.language).toLowerCase();
var f=a;
var e=d.indexOf(b);
...
}},properties:{locale:{check:b,nullable:true,apply:h,event:g}},members:{__iU:f,__iV:null,__iW:null,__iR:null,__iS:null,__iT:null,getLanguage:function(){return this.__iW;
},getTerritory:function(){return this.getLocale().split(k)[1]||j;
},getAvailableLocales:function(){var C=[];

for(var B in this.__iS){if(B!=this.__iU){C.push(B);
}}return C;
},__iX:function(D){var F;
var E=D.indexOf(k);
...
this.__iV=G;
this.__iW=this.__iX(G);
},addTranslation:function(I,J){var K=this.__iR;

if(K[I]){for(var L in J){K[I][L]=J[L];
}}else{K[I]=J;
}},addLocale:function(M,N){var O=this.__iS;

if(O[M]){for(var P in N){O[M][P]=N[P];
}}else{O[M]=N;
}},translate:function(Q,R,S){var T=this.__iR;
return this.__iY(T,Q,R,S);
},localize:function(U,V,W){var X=this.__iS;
return this.__iY(X,U,V,W);
},__iY:function(Y,ba,bb,bc){var bd;

if(!Y){return ba;
}
if(bc){var bf=this.__iX(bc);
}else{bc=this.__iV;
bf=this.__iW;
}if(!bd&&Y[bc]){bd=Y[bc][ba];
}if(!bd&&Y[bf]){bd=Y[bf][ba];
}if(!bd&&Y[this.__iU]){bd=Y[this.__iU][ba];
}
if(!bd){bd=ba;
}
...
}},destruct:function(){this.__iR=this.__iS=null;
}});
})();
...
qx.Class.define(o,{statics:{__jY:{fontFamily:1,fontSize:1,fontWeight:1,fontStyle:1,lineHeight:1},__ka:function(){var A=this.__kc(false);
document.body.insertBefore(A,document.body.firstChild);
return this._textElement=A;
},__kb:function(){var B=this.__kc(true);
document.body.insertBefore(B,document.body.firstChild);
return this._htmlElement=B;
},__kc:function(C){var D=qx.bom.Element.create(j);
var E=D.style;
E.width=E.height=a;
E.left=E.top=k;
E.visibility=d;
...
for(var G in this.__jY){E[G]=i;
}D.appendChild(F);
}}return D;
},__kd:function(H){var I={};
...
M.maxWidth=r;
M.padding=z;
for(var O in this.__jY){N.style[O]=i;
}N.setAttribute(u,s);
L.appendChild(N);
}else{var L=K.document.createElement(j);
qx.bom.element.Style.setStyles(L,this.__kd(J));
}
...
}},getHtmlSize:function(content,S,T){var U=this._htmlElement||this.__kb();
U.style.width=T!==undefined?T+v:a;
U.innerHTML=content;
return this.__ke(U,S);
},getTextSize:function(V,W){var X=this._textElement||this.__ka();
...
}return this.__ke(X,W);
},__ke:function(Y,ba){var bb=this.__jY;
...
this.__hi=B;
this.__hj=B.getWindow().document.documentElement;
this.__hi.addListener(this.__hj,v,this._onMouseDown,this);
this.__hv();
},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{dragstart:1,dragend:1,dragover:1,dragleave:1,drop:1,drag:1,dragchange:1,droprequest:1},IGNORE_CAN_HANDLE:true},members:{__hi:null,__hj:null,__hk:null,__hl:null,__hm:null,__hn:null,__ho:null,__hp:null,__hq:null,__hr:null,__hs:false,__ht:0,__hu:0,canHandleEvent:function(C,D){},registerEvent:function(E,F,G){},unregisterEvent:function(H,I,J){},addType:function(K){this.__hm[K]=true;
},addAction:function(L){this.__hn[L]=true;
},supportsType:function(M){return !!this.__hm[M];
},supportsAction:function(N){return !!this.__hn[N];
},getData:function(O){if(!this.__hC||!this.__hk){throw new Error("This method must not be used outside the drop event listener!");
}
if(!this.__hm[O]){throw new Error("Unsupported data type: "+O+"!");
}
if(!this.__hp[O]){this.__hq=O;
this.__hx(q,this.__hl,this.__hk,false);
}
if(!this.__hp[O]){throw new Error("Please use a droprequest listener to the drag source to fill the manager with data!");
}return this.__hp[O]||null;
},getCurrentAction:function(){return this.__hr;
},addData:function(P,Q){this.__hp[P]=Q;
},getCurrentType:function(){return this.__hq;
},isSessionActive:function(){return this.__hs;
},__hv:function(){this.__hm={};
this.__hn={};
this.__ho={};
this.__hp={};
},__hw:function(){if(this.__hl==null){return;
}var T=this.__hn;
var R=this.__ho;
var S=null;

if(this.__hC){if(R.Shift&&R.Ctrl&&T.alias){S=k;
...
if(S!=this.__hr){this.__hr=S;
this.__hx(o,this.__hl,this.__hk,false);
}},__hx:function(U,V,W,X,Y){var bb=qx.event.Registration;
var ba=bb.createEvent(U,qx.event.type.Drag,[X,Y]);

if(V!==W){ba.setRelatedTarget(W);
}return bb.dispatchEvent(V,ba);
},__hy:function(bc){while(bc&&bc.nodeType==1){if(bc.getAttribute(u)==w){return bc;
}bc=bc.parentNode;
}return null;
},__hz:function(bd){while(bd&&bd.nodeType==1){if(bd.getAttribute(r)==w){return bd;
}bd=bd.parentNode;
}return null;
},__hA:function(){this.__hl=null;
this.__hi.removeListener(this.__hj,c,this._onMouseMove,this,true);
this.__hi.removeListener(this.__hj,y,this._onMouseUp,this,true);
qx.event.Registration.removeListener(window,i,this._onWindowBlur,this);
this.__hv();
},__hB:function(){if(this.__hs){this.__hi.removeListener(this.__hj,a,this._onMouseOver,this,true);
this.__hi.removeListener(this.__hj,h,this._onMouseOut,this,true);
this.__hi.removeListener(this.__hj,g,this._onKeyDown,this,true);
this.__hi.removeListener(this.__hj,z,this._onKeyUp,this,true);
this.__hx(x,this.__hl,this.__hk,false);
this.__hs=false;
}this.__hC=false;
this.__hk=null;
this.__hA();
...
switch(be){case A:case f:case d:if(!this.__ho[be]){this.__ho[be]=true;
this.__hw();
}}},_onKeyUp:function(e){var bf=e.getKeyIdentifier();

switch(bf){case A:case f:case d:if(this.__ho[bf]){this.__ho[bf]=false;
this.__hw();
}}},_onMouseDown:function(e){if(this.__hs){return;
}var bg=this.__hy(e.getTarget());

if(bg){this.__ht=e.getDocumentLeft();
this.__hu=e.getDocumentTop();
this.__hl=bg;
this.__hi.addListener(this.__hj,c,this._onMouseMove,this,true);
this.__hi.addListener(this.__hj,y,this._onMouseUp,this,true);
qx.event.Registration.addListener(window,i,this._onWindowBlur,this);
}},_onMouseUp:function(e){if(this.__hC){this.__hx(s,this.__hk,this.__hl,false,e);
}if(this.__hs){e.stopPropagation();
}this.__hB();
},_onMouseMove:function(e){if(this.__hs){if(!this.__hx(t,this.__hl,this.__hk,true,e)){this.__hB();
}}else{if(Math.abs(e.getDocumentLeft()-this.__ht)>3||Math.abs(e.getDocumentTop()-this.__hu)>3){if(this.__hx(n,this.__hl,this.__hk,true,e)){this.__hs=true;
this.__hi.addListener(this.__hj,a,this._onMouseOver,this,true);
this.__hi.addListener(this.__hj,h,this._onMouseOut,this,true);
this.__hi.addListener(this.__hj,g,this._onKeyDown,this,true);
this.__hi.addListener(this.__hj,z,this._onKeyUp,this,true);
var bh=this.__ho;
bh.Ctrl=e.isCtrlPressed();
bh.Shift=e.isShiftPressed();
bh.Alt=e.isAltPressed();
this.__hw();
}else{this.__hx(x,this.__hl,this.__hk,false);
this.__hA();
}}}},_onMouseOver:function(e){var bi=e.getTarget();
var bj=this.__hz(bi);

if(bj&&bj!=this.__hk){this.__hC=this.__hx(m,bj,this.__hl,true,e);
this.__hk=bj;
this.__hw();
}},_onMouseOut:function(e){var bl=this.__hz(e.getTarget());
var bk=this.__hz(e.getRelatedTarget());

if(bl&&bl!==bk&&bl==this.__hk){this.__hx(l,this.__hk,bk,false,e);
this.__hk=null;
this.__hC=false;
qx.event.Timer.once(this.__hw,this,0);
}}},destruct:function(){this.__hl=this.__hk=this.__hi=this.__hj=this.__hm=this.__hn=this.__ho=this.__hp=null;
...
}},tryActivate:function(S){var T=this.__iq(S);

if(T){this.setActive(T);
}},__ic:function(U,V,W,X){var ba=qx.event.Registration;
var Y=ba.createEvent(W,qx.event.type.Focus,[U,V,X]);
ba.dispatchEvent(U,Y);
},_windowFocused:true,__id:function(){if(this._windowFocused){this._windowFocused=false;
this.__ic(this._window,null,g,false);
}},__ie:function(){if(!this._windowFocused){this._windowFocused=true;
this.__ic(this._window,null,f,false);
}},_initObserver:qx.core.Variant.select(j,{"gecko":function(){this.__hR=qx.lang.Function.listener(this.__ik,this);
this.__hS=qx.lang.Function.listener(this.__il,this);
this.__hT=qx.lang.Function.listener(this.__ij,this);
this.__hU=qx.lang.Function.listener(this.__ii,this);
this.__hV=qx.lang.Function.listener(this.__if,this);
qx.bom.Event.addNativeListener(this._document,i,this.__hR,true);
qx.bom.Event.addNativeListener(this._document,h,this.__hS,true);
qx.bom.Event.addNativeListener(this._window,f,this.__hT,true);
qx.bom.Event.addNativeListener(this._window,g,this.__hU,true);
qx.bom.Event.addNativeListener(this._window,y,this.__hV,true);
},"mshtml":function(){this.__hR=qx.lang.Function.listener(this.__ik,this);
this.__hS=qx.lang.Function.listener(this.__il,this);
this.__hX=qx.lang.Function.listener(this.__ig,this);
this.__hY=qx.lang.Function.listener(this.__ih,this);
this.__hW=qx.lang.Function.listener(this.__in,this);
qx.bom.Event.addNativeListener(this._document,i,this.__hR);
qx.bom.Event.addNativeListener(this._document,h,this.__hS);
qx.bom.Event.addNativeListener(this._document,b,this.__hX);
qx.bom.Event.addNativeListener(this._document,a,this.__hY);
qx.bom.Event.addNativeListener(this._document,d,this.__hW);
},"webkit":function(){this.__hR=qx.lang.Function.listener(this.__ik,this);
this.__hS=qx.lang.Function.listener(this.__il,this);
this.__hY=qx.lang.Function.listener(this.__ih,this);
this.__hT=qx.lang.Function.listener(this.__ij,this);
this.__hU=qx.lang.Function.listener(this.__ii,this);
...
qx.bom.Event.addNativeListener(this._document,i,this.__hR,true);
qx.bom.Event.addNativeListener(this._document,h,this.__hS,true);
qx.bom.Event.addNativeListener(this._document,d,this.__hW,false);
qx.bom.Event.addNativeListener(this._window,c,this.__hY,true);
qx.bom.Event.addNativeListener(this._window,f,this.__hT,true);
qx.bom.Event.addNativeListener(this._window,g,this.__hU,true);
},"opera":function(){this.__hR=qx.lang.Function.listener(this.__ik,this);
this.__hS=qx.lang.Function.listener(this.__il,this);
this.__hX=qx.lang.Function.listener(this.__ig,this);
this.__hY=qx.lang.Function.listener(this.__ih,this);
qx.bom.Event.addNativeListener(this._document,i,this.__hR,true);
qx.bom.Event.addNativeListener(this._document,h,this.__hS,true);
qx.bom.Event.addNativeListener(this._window,z,this.__hX,true);
qx.bom.Event.addNativeListener(this._window,c,this.__hY,true);
}}),_stopObserver:qx.core.Variant.select(j,{"gecko":function(){qx.bom.Event.removeNativeListener(this._document,i,this.__hR,true);
qx.bom.Event.removeNativeListener(this._document,h,this.__hS,true);
qx.bom.Event.removeNativeListener(this._window,f,this.__hT,true);
qx.bom.Event.removeNativeListener(this._window,g,this.__hU,true);
qx.bom.Event.removeNativeListener(this._window,y,this.__hV,true);
},"mshtml":function(){qx.bom.Event.removeNativeListener(this._document,i,this.__hR);
qx.bom.Event.removeNativeListener(this._document,h,this.__hS);
qx.bom.Event.removeNativeListener(this._document,b,this.__hX);
qx.bom.Event.removeNativeListener(this._document,a,this.__hY);
qx.bom.Event.removeNativeListener(this._document,d,this.__hW);
},"webkit":function(){qx.bom.Event.removeNativeListener(this._document,i,this.__hR,true);
qx.bom.Event.removeNativeListener(this._document,h,this.__hS,true);
qx.bom.Event.removeNativeListener(this._document,d,this.__hW,false);
qx.bom.Event.removeNativeListener(this._window,c,this.__hY,true);
qx.bom.Event.removeNativeListener(this._window,f,this.__hT,true);
qx.bom.Event.removeNativeListener(this._window,g,this.__hU,true);
},"opera":function(){qx.bom.Event.removeNativeListener(this._document,i,this.__hR,true);
qx.bom.Event.removeNativeListener(this._document,h,this.__hS,true);
qx.bom.Event.removeNativeListener(this._window,z,this.__hX,true);
qx.bom.Event.removeNativeListener(this._window,c,this.__hY,true);
}}),__if:qx.event.GlobalError.observeMethod(qx.core.Variant.select(j,{"gecko":function(bb){var bc=qx.bom.Event.getTarget(bb);

if(!this.__ir(bc)){qx.bom.Event.preventDefault(bb);
}},"default":null})),__ig:qx.event.GlobalError.observeMethod(qx.core.Variant.select(j,{"mshtml":function(bd){this.__ie();
...
if(bh==this._document||bh==this._window){this.__ie();

if(this.__ia){this.setFocus(this.__ia);
delete this.__ia;
}
if(this.__ib){this.setActive(this.__ib);
delete this.__ib;
}}else{this.setFocus(bh);
this.tryActivate(bh);
if(!this.__ir(bh)){bh.selectionStart=0;
bh.selectionEnd=0;
}}},"default":null})),__ih:qx.event.GlobalError.observeMethod(qx.core.Variant.select(j,{"mshtml":function(bi){if(!bi.toElement){this.__id();
this.resetFocus();
this.resetActive();
...
if(bm==this._document){this.__id();
this.__ia=this.getFocus();
this.__ib=this.getActive();
this.resetFocus();
this.resetActive();
}else{if(bm===this.getFocus()){this.resetFocus();
}
if(bm===this.getActive()){this.resetActive();
}}},"default":null})),__ii:qx.event.GlobalError.observeMethod(qx.core.Variant.select(j,{"gecko":function(bn){var bo=qx.bom.Event.getTarget(bn);

if(bo===this._window||bo===this._document){this.__id();
this.resetActive();
this.resetFocus();
...
this.resetActive();
this.resetFocus();
}},"default":null})),__ij:qx.event.GlobalError.observeMethod(qx.core.Variant.select(j,{"gecko":function(br){var bs=qx.bom.Event.getTarget(br);

if(bs===this._window||bs===this._document){this.__ie();
bs=this._body;
}this.setFocus(bs);
this.tryActivate(bs);
},"webkit":function(bt){var bu=qx.bom.Event.getTarget(bt);

if(bu===this._window||bu===this._document){this.__ie();
...
this.tryActivate(bu);
}},"default":null})),__ik:qx.event.GlobalError.observeMethod(qx.core.Variant.select(j,{"gecko":function(bv){var bx=qx.bom.Event.getTarget(bv);
var bw=this.__ip(bx);
...
if(bz){if(!this.__ir(bA)){bA.unselectable=e;
try{document.selection.empty();
}catch(bB){}try{bz.focus();
}catch(bC){}}}else{qx.bom.Event.preventDefault(by);
if(!this.__ir(bA)){bA.unselectable=e;
...
if(!this.__ir(bJ)){qx.bom.Event.preventDefault(bG);
...
}},"default":null})),__il:qx.event.GlobalError.observeMethod(qx.core.Variant.select(j,{"mshtml":function(bK){var bL=qx.bom.Event.getTarget(bK);

if(bL.unselectable){bL.unselectable=p;
}this.tryActivate(this.__im(bL));
...
this.tryActivate(this.__im(bP));
},"default":null})),__im:qx.event.GlobalError.observeMethod(qx.core.Variant.select(j,{"mshtml|webkit":function(bQ){var bR=this.getFocus();

if(bR&&bQ!=bR&&(bR.nodeName.toLowerCase()===r||bR.nodeName.toLowerCase()===u)){bQ=bR;
}return bQ;
},"default":function(bS){return bS;
}})),__in:qx.event.GlobalError.observeMethod(qx.core.Variant.select(j,{"mshtml|webkit":function(bT){var bU=qx.bom.Event.getTarget(bT);

if(!this.__ir(bU)){qx.bom.Event.preventDefault(bT);
}},"default":null})),__io:function(bV){var bW=qx.bom.element.Attribute.get(bV,o);
...
},__ip:function(bY){while(bY&&bY.nodeType===1){if(bY.getAttribute(k)==e){return null;
}
if(this.__io(bY)){return bY;
}bY=bY.parentNode;
}return this._body;
},__iq:function(ca){var cb=ca;

while(ca&&ca.nodeType===1){if(ca.getAttribute(l)==e){return null;
}ca=ca.parentNode;
}return cb;
},__ir:function(cc){while(cc&&cc.nodeType===1){var cd=cc.getAttribute(q);

if(cd!=null){return cd===e;
}cc=cc.parentNode;
}return true;
},_applyActive:function(ce,cf){if(cf){this.__ic(cf,ce,v,true);
}
if(ce){this.__ic(ce,cf,m,true);
}},_applyFocus:function(cg,ch){if(ch){this.__ic(ch,cg,a,true);
}
if(cg){this.__ic(cg,ch,b,true);
}if(ch){this.__ic(ch,cg,g,false);
}
if(cg){this.__ic(cg,ch,f,false);
}}},destruct:function(){this._stopObserver();
this._manager=this._window=this._document=this._root=this._body=this.__is=null;
...
qx.Class.define(d,{statics:{__hG:{"onload":true,"onpropertychange":true,"oninput":true,"onchange":true,"name":true,"type":true,"checked":true,"disabled":true},__hH:{},__hI:{},allowCreationWithMarkup:function(r){if(!r){r=window;
}var s=r.location.href;

if(qx.bom.Element.__hI[s]==undefined){try{r.document.createElement(f);
qx.bom.Element.__hI[s]=true;
}catch(e){qx.bom.Element.__hI[s]=false;
}}return qx.bom.Element.__hI[s];
},getHelperElement:function(t){if(!t){t=window;
}var v=t.location.href;

if(!qx.bom.Element.__hH[v]){var u=qx.bom.Element.__hH[v]=t.document.createElement(c);
if(qx.bom.client.Engine.WEBKIT){u.style.display=g;
t.document.body.appendChild(u);
}}return qx.bom.Element.__hH[v];
...
g.start();
this.__ey=g;
},events:{"interval":c},properties:{timeoutInterval:{check:e,init:100,apply:d}},members:{__ey:null,_applyTimeoutInterval:function(h){this.__ey.setInterval(h);
...
qx.Mixin.define(j,{events:{"execute":g},properties:{command:{check:b,apply:f,event:a,nullable:true}},members:{__ng:null,__nh:false,__ni:null,_bindableProperties:[d,k,l,m,h,c],execute:function(){var o=this.getCommand();

if(o){if(this.__nh){this.__nh=false;
}else{this.__nh=true;
o.execute(this);
}}this.fireEvent(n);
},__nj:function(e){if(this.__nh){this.__nh=false;
return;
}this.__nh=true;
this.execute();
},_applyCommand:function(p,q){if(q!=null){q.removeListenerById(this.__ni);
}
if(p!=null){this.__ni=p.addListener(n,this.__nj,this);
}var t=this.__ng;

if(t==null){this.__ng=t={};
}
...
qx.Mixin.define(k,{properties:{contentPaddingTop:{check:p,init:0,apply:o,themeable:true},contentPaddingRight:{check:p,init:0,apply:o,themeable:true},contentPaddingBottom:{check:p,init:0,apply:o,themeable:true},contentPaddingLeft:{check:p,init:0,apply:o,themeable:true},contentPadding:{group:[e,a,d,c],mode:b,themeable:true}},members:{__oa:{contentPaddingTop:i,contentPaddingRight:h,contentPaddingBottom:m,contentPaddingLeft:f},__ob:{contentPaddingTop:l,contentPaddingRight:n,contentPaddingBottom:g,contentPaddingLeft:j},_applyContentPadding:function(q,r,name){var s=this._getContentPaddingTarget();

if(q==null){var t=this.__ob[name];
s[t]();
}else{var u=this.__oa[name];
s[u](q);
}}}});
})();
...
return o.text;
},"default":function(p){if(this.__it(p)){return p.value.substring(p.selectionStart,p.selectionEnd);
...
if(this.__it(t)){var x=t.selectionStart;
...
return w-(u.length-1);
},"default":function(z){if(this.__it(z)){return z.selectionEnd-z.selectionStart;
}else{return this.get(z).length;
}}}),getStart:qx.core.Variant.select(k,{"mshtml":function(A){if(this.__it(A)){var F=qx.bom.Range.get();
...
}},"gecko|webkit":function(J){if(this.__it(J)){return J.selectionStart;
...
}}},"default":function(M){if(this.__it(M)){return M.selectionStart;
}else{return qx.bom.Selection.getSelectionObject(qx.dom.Node.getDocument(M)).anchorOffset;
}}}),getEnd:qx.core.Variant.select(k,{"mshtml":function(N){if(this.__it(N)){var S=qx.bom.Range.get();
...
}},"gecko|webkit":function(W){if(this.__it(W)){return W.selectionEnd;
...
}}},"default":function(ba){if(this.__it(ba)){return ba.selectionEnd;
}else{return qx.bom.Selection.getSelectionObject(qx.dom.Node.getDocument(ba)).focusOffset;
}}}),__it:function(bb){return qx.dom.Node.isElement(bb)&&(bb.nodeName.toLowerCase()==h||bb.nodeName.toLowerCase()==g);
...
qx.Class.define(c,{statics:{__fY:false,__ga:{},__gb:0,MAX_RETRIES:10,scheduleFlush:function(f){var self=qx.ui.core.queue.Manager;
self.__ga[f]=true;

if(!self.__fY){self.__gg.schedule();
self.__fY=true;
...
self.__gg.cancel();
var g=self.__ga;
self.__gd(function(){while(g.visibility||g.widget||g.appearance||g.layout||g.element){if(g.widget){delete g.widget;
...
self.__gd(function(){if(g.dispose){delete g.dispose;
qx.ui.core.queue.Dispose.flush();
}},function(){self.__gc=false;
});
self.__gb=0;
},__gd:function(h,i){var self=qx.ui.core.queue.Manager;
...
if(self.__gb<=self.MAX_RETRIES){self.scheduleFlush();
}else{throw new Error("Fatal Error: Flush terminated "+(self.__gb-1)+" times in a row"+" due to exceptions in user code. The application has to be reloaded!");
}throw e;
}finally{i();
}},__ge:function(e){var j=qx.ui.core.queue.Manager;
if(e.getData()==a){j.PAUSE=true;

if(j.__gf){window.clearTimeout(j.__gf);
}j.__gf=window.setTimeout(function(){j.PAUSE=false;
j.__gf=null;
j.flush();
},500);
}else{j.flush();
}}},defer:function(k){k.__gg=new qx.util.DeferredCall(k.flush);
qx.html.Element._scheduleFlush=k.scheduleFlush;
qx.event.Registration.addListener(window,b,qx.bom.client.Feature.TOUCH?k.__ge:k.flush);
}});
})();
(function(){var c="qx.client",b="load",a="qx.io.ImageLoader";
qx.Bootstrap.define(a,{statics:{__jo:{},__jp:{width:null,height:null},__jq:/\.(png|gif|jpg|jpeg|bmp)\b/i,isLoaded:function(d){var e=this.__jo[d];
return !!(e&&e.loaded);
},isFailed:function(f){var g=this.__jo[f];
return !!(g&&g.failed);
},isLoading:function(h){var j=this.__jo[h];
return !!(j&&j.loading);
},getFormat:function(k){var m=this.__jo[k];
return m?m.format:null;
},getSize:function(n){var o=this.__jo[n];
return o?
{width:o.width,height:o.height}:this.__jp;
},getWidth:function(p){var q=this.__jo[p];
return q?q.width:null;
},getHeight:function(r){var s=this.__jo[r];
return s?s.height:null;
},load:function(t,u,v){var w=this.__jo[t];
...
var x=qx.lang.Function.listener(this.__jr,this,y,t);
y.onload=x;
y.onerror=x;
y.src=t;
w.element=y;
}},abort:function(z){var A=this.__jo[z];
...
},__jr:qx.event.GlobalError.observeMethod(function(event,D,E){var F=this.__jo[E];
if(event.type===b){F.loaded=true;
F.width=this.__js(D);
F.height=this.__jt(D);
var G=this.__jq.exec(E);
...
}}),__js:qx.core.Variant.select(c,{"gecko":function(I){return I.naturalWidth;
},"default":function(J){return J.width;
}}),__jt:qx.core.Variant.select(c,{"gecko":function(K){return K.naturalHeight;
...
qx.Bootstrap.define(u,{statics:{UNKNOWN:true,NAME:"unknown",TITLE:"unknown 0.0",VERSION:0.0,FULLVERSION:"0.0.0",__hQ:function(y){var z=navigator.userAgent;
...
this.UNKNOWN=false;
}},defer:qx.core.Variant.select(j,{"webkit":function(D){D.__hQ(o);
},"gecko":function(E){E.__hQ(k);
},"mshtml":function(F){F.__hQ(v);
},"opera":function(G){G.__hQ(p);
}})});
})();
(function(){var i="error",h="initialized",g="loading",f="complete",e="webkit",d="qx.client",c="success",b="qx.io.part.Package",a="cached";
qx.Bootstrap.define(b,{construct:function(j,k,l){this.__rL=l?f:h;
this.__rM=j;
this.__rN=k;
},members:{__rL:null,__rM:null,__rN:null,__rO:null,__rP:null,__rQ:null,__rR:null,getId:function(){return this.__rN;
},getReadyState:function(){return this.__rL;
},getUrls:function(){return this.__rM;
},saveClosure:function(m){if(this.__rL==i){return;
}this.__rO=m;

if(!this.__rP){this.execute();
}else{clearTimeout(this.__rQ);
this.__rL=a;
this.__rR(this);
}},execute:function(){if(this.__rO){this.__rO();
delete this.__rO;
}
if(qx.$$packageData[this.__rN]){this.__rT(qx.$$packageData[this.__rN]);
delete qx.$$packageData[this.__rN];
}this.__rL=f;
},loadClosure:function(n,self){if(this.__rL!==h){return;
}this.__rP=true;
this.__rL=g;
this.__rR=qx.Bootstrap.bind(n,self);
this.__rS(this.__rM,function(){},function(){this.__rL=i;
n.call(self,this);
},this);
var o=this;
this.__rQ=setTimeout(function(){o.__rL=i;
n.call(self,o);
},qx.Part.TIMEOUT);
},load:function(p,self){if(this.__rL!==h){return;
}this.__rP=false;
this.__rL=g;
this.__rS(this.__rM,function(){this.__rL=f;
this.execute();
p.call(self,this);
},function(){this.__rL=i;
p.call(self,this);
},this);
},__rS:function(q,r,s,self){if(q.length==0){r.call(self);
return;
}var u=0;
var self=this;
...
if(status!==c){if(self.__rL==g){clearTimeout(self.__rQ);
return s.call(self);
}}
...
t(q.concat());
},__rT:qx.$$loader.importPackageData}});
})();
...
},_registerEventChaining:function(k,l,name){if((k instanceof qx.core.Object)&&qx.Class.hasMixin(k.constructor,qx.data.marshal.MEventBubbling)){var m=qx.lang.Function.bind(this.__nL,this,name);
var n=k.addListener(c,m,this);
k.setUserData(d,n);
}if(l!=null&&l.getUserData&&l.getUserData(d)!=null){l.removeListenerById(l.getUserData(d));
}},__nL:function(name,e){var v=e.getData();
var r=v.value;
var p=v.old;
...
if(m==undefined){this.__nM=[];
}else if(arguments.length>1){this.__nM=[];

for(var i=0;i<arguments.length;i++){this.__nM.push(arguments[i]);
}}else if(typeof m==b){this.__nM=new Array(m);
}else if(m instanceof Array){this.__nM=qx.lang.Array.clone(m);
}else{this.__nM=[];
throw new Error("Type of the parameter not supported!");
}for(var i=0;i<this.__nM.length;i++){this._applyEventPropagation(this.__nM[i],null,i);
}this.__nN();
},events:{"change":g,"changeLength":g},members:{__nM:null,concat:function(n){if(n){var o=this.__nM.concat(n);
}else{var o=this.__nM.concat();
}return new qx.data.Array(o);
},join:function(p){return this.__nM.join(p);
},pop:function(){var q=this.__nM.pop();
this.__nN();
...
return q;
},push:function(r){for(var i=0;i<arguments.length;i++){this.__nM.push(arguments[i]);
this.__nN();
this._applyEventPropagation(arguments[i],null,this.length-1);
this.fireDataEvent(l,{start:this.length-1,end:this.length-1,type:k,items:[arguments[i]]},null);
}return this.length;
},reverse:function(){this.__nM.reverse();
this.fireDataEvent(l,{start:0,end:this.length-1,type:h,items:null},null);
},shift:function(){var s=this.__nM.shift();
this.__nN();
this._applyEventPropagation(null,s,this.length-1);
this.fireDataEvent(l,{start:0,end:this.length-1,type:j,items:[s]},null);
return s;
},slice:function(t,u){return new qx.data.Array(this.__nM.slice(t,u));
},splice:function(v,w,x){var D=this.__nM.length;
var A=this.__nM.splice.apply(this.__nM,arguments);
if(this.__nM.length!=D){this.__nN();
}var B=w>0;
var y=arguments.length>2;
var z=null;

if(B||y){if(this.__nM.length>D){var C=k;
}else if(this.__nM.length<D){var C=j;
z=A;
}else{var C=h;
...
},sort:function(E){this.__nM.sort.apply(this.__nM,arguments);
this.fireDataEvent(l,{start:0,end:this.length-1,type:h,items:null},null);
},unshift:function(F){for(var i=arguments.length-1;i>=0;i--){this.__nM.unshift(arguments[i]);
this.__nN();
this._applyEventPropagation(arguments[i],null,0);
this.fireDataEvent(l,{start:0,end:this.length-1,type:k,items:[arguments[i]]},null);
}return this.length;
},toArray:function(){return this.__nM;
},getItem:function(G){return this.__nM[G];
},setItem:function(H,I){var J=this.__nM[H];
if(J===I){return;
}this.__nM[H]=I;
this._applyEventPropagation(I,J,H);
if(this.length!=this.__nM.length){this.__nN();
}this.fireDataEvent(l,{start:H,end:H,type:k,items:[I]},null);
},getLength:function(){return this.length;
},indexOf:function(K){return this.__nM.indexOf(K);
},toString:function(){if(this.__nM!=null){return this.__nM.toString();
}return f;
},contains:function(L){return this.__nM.indexOf(L)!==-1;
...
},removeAll:function(){for(var i=0;i<this.__nM.length;i++){this._applyEventPropagation(null,this.__nM[i],i);
}var W=this.getLength();
var V=this.__nM.concat();
this.__nM.length=0;
this.__nN();
this.fireDataEvent(l,{start:0,end:W-1,type:j,items:V},null);
},append:function(X){if(X instanceof qx.data.Array){X=X.toArray();
}{};
for(var i=0;i<X.length;i++){this._applyEventPropagation(X[i],null,this.__nM.length+i);
}Array.prototype.push.apply(this.__nM,X);
var Y=this.length;
this.__nN();
...
},forEach:function(bg,bh){for(var i=0;i<this.__nM.length;i++){bg.call(bh,this.__nM[i]);
}},__nN:function(){var bi=this.length;
this.length=this.__nM.length;
this.fireDataEvent(a,this.length,bi);
}},destruct:function(){for(var i=0;i<this.__nM.length;i++){this._applyEventPropagation(null,this.__nM[i],i);
}this.__nM=null;
}});
})();
...
qx.Bootstrap.define(a,{construct:function(name,f,g){this.__rU=name;
this._readyState=e;
this._packages=f;
this._loader=g;
...
break;
}}},members:{_readyState:null,_loader:null,_packages:null,__rU:null,getReadyState:function(){return this._readyState;
},getName:function(){return this.__rU;
...
(function(){var e="change",d="qx.event.type.Data",c="__nF",b="qx.ui.form.MModelSelection",a="changeSelection";
qx.Mixin.define(b,{construct:function(){this.__nF=new qx.data.Array();
this.__nF.addListener(e,this.__nI,this);
this.addListener(a,this.__nH,this);
},events:{changeModelSelection:d},members:{__nF:null,__nG:false,__nH:function(){if(this.__nG){return;
...
}},__nI:function(){this.__nG=true;
var m=this.getSelectables(true);
var o=[];
var n=this.__nF.toArray();
...
if(!qx.lang.Array.equals(p,o)){this.__nH();
}},getModelSelection:function(){return this.__nF;
},setModelSelection:function(s){if(!s){this.__nF.removeAll();
return;
}{};
s.unshift(this.__nF.getLength());
s.unshift(0);
var t=this.__nF.splice.apply(this.__nF,s);
t.dispose();
...
(function(){var f="__mw",d="qx.ui.core.MSingleSelectionHandling",c="changeSelection",b="changeSelected",a="qx.event.type.Data";
qx.Mixin.define(d,{events:{"changeSelection":a},members:{__mw:null,getSelection:function(){var g=this.__mx().getSelected();

if(g){return [g];
}else{return [];
}},setSelection:function(h){switch(h.length){case 0:this.resetSelection();
break;
case 1:this.__mx().setSelected(h[0]);
break;
default:throw new Error("Could only select one item, but the selection "+" array contains "+h.length+" items!");
}},resetSelection:function(){this.__mx().resetSelected();
},isSelected:function(i){return this.__mx().isSelected(i);
},isSelectionEmpty:function(){return this.__mx().isSelectionEmpty();
},getSelectables:function(j){return this.__mx().getSelectables(j);
},_onChangeSelected:function(e){var l=e.getData();
var k=e.getOldData();
l==null?l=[]:l=[l];
k==null?k=[]:k=[k];
this.fireDataEvent(c,l,k);
},__mx:function(){if(this.__mw==null){var m=this;
this.__mw=new qx.ui.core.SingleSelectionManager({getItems:function(){return m._getItems();
},isItemSelectable:function(n){if(m._isItemSelectable){return m._isItemSelectable(n);
}else{return n.isVisible();
}}});
this.__mw.addListener(b,this._onChangeSelected,this);
}this.__mw.setAllowEmptySelection(this._isAllowEmptySelection());
return this.__mw;
...
qx.Bootstrap.define(f,{construct:function(){this.__rV=qx.Bootstrap.bind(this.__sc,this);
this.__rW=document.createElement(d);
},members:{__rX:null,__rY:null,__sa:null,__sb:null,__rV:null,__rW:null,load:function(p,q,r){if(this.__rX){throw new Error("Another request is still running!");
}this.__rX=true;
this.__rY=false;
...
this.__sb=r||window;
t.type=c;
t.onerror=t.onload=t.onreadystatechange=this.__rV;
t.src=p;
setTimeout(function(){s.appendChild(t);
},0);
},abort:function(){if(this.__rX){this.dispose(a);
}},dispose:function(status){if(this.__rY){return;
}this.__rY=true;
var v=this.__rW;
...
if(this.__sa){if(qx.core.Variant.isSet(j,b)){var self=this;
setTimeout(qx.event.GlobalError.observeMethod(function(){self.__sa.call(self.__sb,status);
delete self.__sa;
}),0);
}else{this.__sa.call(this.__sb,status);
delete this.__sa;
}}},__sc:qx.event.GlobalError.observeMethod(qx.core.Variant.select(j,{"mshtml":function(e){var w=this.__rW.readyState;
...
for(var i=0;i<g.length;i++){l.push(this.__sh[g[i]]);
...
}},members:{__sd:null,__sh:null,__si:null,__sg:null,addToPackage:function(t){this.__sh.push(t);
},addClosurePackageListener:function(u,v){var w=u.getId();

if(!this.__sg[w]){this.__sg[w]=[];
}this.__sg[w].push(v);
},saveClosure:function(x,y){var A;

for(var i=0;i<this.__sh.length;i++){if(this.__sh[i].getId()==x){A=this.__sh[i];
break;
...
},getParts:function(){return this.__si;
...
for(var i=0;i<B.length;i++){F.push(this.__si[B[i]]);
}var E=0;
var D=function(){E+=1;
if(E>=F.length){var G=[];
...
for(var i=0;i<H.length;i++){this.__si[H[i]].preload(function(){J++;

if(J>=H.length){var K=[];

for(var i=0;i<H.length;i++){K.push(this.__si[H[i]].getReadyState());
}
if(I){I.call(self,K);
}}},this);
}},__sj:function(){var L=this.__sd.uris;
var M=[];

for(var i=0;i<L.length;i++){M.push(this.__sk(L[i]));
}return M;
},__sk:qx.$$loader.decodeUris,__se:null,addPartListener:function(N,O){var P=N.getName();

if(!this.__se[P]){this.__se[P]=[];
}this.__se[P].push(O);
...
}},__sf:null,addPackageListener:function(T,U){var V=T.getId();

if(!this.__sf[V]){this.__sf[V]=[];
}this.__sf[V].push(U);
...
},members:{__sl:0,preload:function(h,self){var j=0;
var l=this;
...
}this.__sl=this._packages.length;
...
}}if(this.__sl<=0){this.__sm();
}},__sm:function(){for(var i=0;i<this._packages.length;i++){this._packages[i].execute();
...
(function(){var i="qx.ui.window.Window",h="changeModal",g="changeVisibility",f="changeActive",d="_applyActiveWindow",c="__kg",b="__kh",a="qx.ui.window.MDesktop";
qx.Mixin.define(a,{properties:{activeWindow:{check:i,apply:d,init:null,nullable:true}},members:{__kg:null,__kh:null,getWindowManager:function(){if(!this.__kh){this.setWindowManager(new qx.ui.window.Window.DEFAULT_MANAGER_CLASS());
}return this.__kh;
},supportsMaximize:function(){return true;
},setWindowManager:function(j){if(this.__kh){this.__kh.setDesktop(null);
}j.setDesktop(this);
this.__kh=j;
...
this.getWindowManager().updateStack();
},getWindows:function(){if(!this.__kg){this.__kg=[];
}return this.__kg;
}},destruct:function(){this._disposeArray(c);
this._disposeObjects(b);
}});
})();
(function(){var f="__ki",e="_applyBlockerColor",d="Number",c="qx.ui.core.MBlocker",b="_applyBlockerOpacity",a="Color";
qx.Mixin.define(c,{construct:function(){this.__ki=new qx.ui.core.Blocker(this);
},properties:{blockerColor:{check:a,init:null,nullable:true,apply:e,themeable:true},blockerOpacity:{check:d,init:1,apply:b,themeable:true}},members:{__ki:null,_applyBlockerColor:function(g,h){this.__ki.setColor(g);
},_applyBlockerOpacity:function(i,j){this.__ki.setOpacity(i);
},block:function(){this.__ki.block();
},isBlocked:function(){return this.__ki.isBlocked();
},unblock:function(){this.__ki.unblock();
},forceUnblock:function(){this.__ki.forceUnblock();
},blockContent:function(k){this.__ki.blockContent(k);
},isContentBlocked:function(){return this.__ki.isContentBlocked();
},unblockContent:function(){this.__ki.unblockContent();
},forceUnblockContent:function(){this.__ki.forceUnblockContent();
},getBlocker:function(){return this.__ki;
}},destruct:function(){this._disposeObjects(f);
}});
})();
...
qx.ui.core.queue.Visibility.add(this);
this.initNativeHelp();
},properties:{appearance:{refine:true,init:j},enabled:{refine:true,init:true},focusable:{refine:true,init:true},globalCursor:{check:b,nullable:true,themeable:true,apply:g,event:m},nativeContextMenu:{refine:true,init:false},nativeHelp:{check:k,init:false,apply:c}},members:{__kj:null,isRootWidget:function(){return true;
},getLayout:function(){return this._getLayout();
},_applyGlobalCursor:qx.core.Variant.select(n,{"mshtml":function(q,r){},"default":function(s,t){var u=qx.bom.Stylesheet;
var v=this.__kj;

if(!v){this.__kj=v=u.createElement();
}u.removeAllRules(v);
...
}},"default":function(){}})},destruct:function(){this.__kj=null;
...
}this.__lS.addListener(d,this._onMouseDown,this);
this.__lS.addListener(r,this._onMouseUp,this);
this.__lS.addListener(D,this._onMouseMove,this);
this.__lS.addListener(c,this._onMouseOut,this);
this.__lS.addListener(m,this._onMouseUp,this);
},properties:{appearance:{refine:true,init:u},offset:{check:b,init:6,apply:v},orientation:{init:i,check:[i,g],apply:w}},members:{__lL:null,__lM:false,__lN:null,__lO:null,__lP:null,__lQ:null,__lR:null,__lK:null,__lS:null,_createChildControlImpl:function(F,G){var H;
...
this._add(H,{type:F});
H.addListener(C,this.__lT,this);
break;
}return H||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,F);
},__lT:function(e){this.__lV(e.getData());
},__lU:function(I){this.__lS=new qx.ui.splitpane.Blocker(I);
this.getContentElement().add(this.__lS);
var J=this.getChildControl(j);
var K=J.getWidth();

if(!K){J.addListenerOnce(p,function(){this.__lV();
},this);
}J.addListener(A,function(e){var L=e.getData();

if(L.height==0||L.width==0){this.__lS.hide();
}else{this.__lS.show();
}},this);
},getBlocker:function(){return this.__lS;
},_applyOrientation:function(M,N){var O=this.getChildControl(k);
var R=this.getChildControl(j);
this.__lP=M===i;

if(!this.__lS){this.__lU(M);
}this.__lS.setOrientation(M);
...
this.__lV();
},_applyOffset:function(S,T){this.__lV();
},__lV:function(U){var V=this.getChildControl(j);
var ba=this.getOffset();
var bb=V.getBounds();
var Y=V.getContainerElement().getDomElement();
if(!Y){return;
}if(this.__lP){var X=null;
...
}this.__lS.setWidth(ba,X);
this.__lS.setLeft(ba,bc);
}}else{var W=null;
...
}this.__lS.setHeight(ba,W);
this.__lS.setTop(ba,top);
}}},add:function(bd,be){if(be==null){this._add(bd);
}else{this._add(bd,{flex:be});
}this.__lK.push(bd);
},remove:function(bf){this._remove(bf);
qx.lang.Array.remove(this.__lK,bf);
},getChildren:function(){return this.__lK;
},_onMouseDown:function(e){if(!e.isLeftPressed()){return;
}var bg=this.getChildControl(j);
var bi=bg.getContainerLocation();
var bh=this.getContentLocation();
this.__lL=this.__lP?e.getDocumentLeft()-bi.left+bh.left:e.getDocumentTop()-bi.top+bh.top;
...
if(this.__lM){this.__lW();
var bl=this.getChildControl(k);
var bm=this.__lQ;

if(this.__lP){bl.setDomLeft(bm);
this.__lS.setStyle(n,(bm-this.getOffset())+h);
}else{bl.setDomTop(bm);
this.__lS.setStyle(s,(bm-this.getOffset())+h);
}e.stop();
}},_onMouseOut:function(e){this._setLastMousePosition(e.getDocumentLeft(),e.getDocumentTop());
},_onMouseUp:function(e){if(!this.__lM){return;
}this._finalizeSizes();
...
this.releaseCapture();
e.stop();
},_finalizeSizes:function(){var br=this.__lQ;
var bo=this.__lR;
...
}else{if(this.__lP){bs.setWidth(br);
bp.setWidth(bo);
}else{bs.setHeight(br);
bp.setHeight(bo);
}}},__lW:function(){if(this.__lP){var bx=o,bE=z,by=l,bC=this.__lN;
}else{var bx=q,bE=a,by=B,bC=this.__lO;
...
this.__lR=bw;
},_isActiveDragSession:function(){return this.__lM;
},_setLastMousePosition:function(x,y){this.__lN=x;
this.__lO=y;
...
},__pB:function(){var bR=this.getRequest();
...
}},properties:{parseJson:{check:A,init:true}},members:{__qc:false,__qd:0,__qe:null,getRequest:function(){if(this.__qe===null){this.__qe=qx.io.remote.transport.XmlHttp.createRequestObject();
this.__qe.onreadystatechange=qx.lang.Function.bind(this._onreadystatechange,this);
}return this.__qe;
},send:function(){this.__qd=0;
...
if(bi==4){if(!qx.io.remote.Exchange.wasSuccessful(this.getStatusCode(),bi,this.__qc)){if(this.getState()===c){this.setState(f);
}this.failed();
return;
}}while(this.__qd<bi){this.setState(qx.io.remote.Exchange._nativeMap[++this.__qd]);
...
if(qx.io.remote.Exchange.wasSuccessful(bD,bE,this.__qc)){try{bF=this.getRequest().responseXML;
...
this.__sn=j;
},statics:{__so:null,createModel:function(k,l){if(this.__so===null){this.__so=new qx.data.marshal.Json();
}this.__so.toClass(k,l);
return this.__so.toModel(k);
}},members:{__sn:null,__sp:function(m){var n=[];
...
if(this.__sn&&this.__sn.getModelClass&&this.__sn.getModelClass(s)!=null){return;
}for(var t in p){this.toClass(p[t],q);
}if(qx.Class.isDefined(h+s)){return;
}var z={};
var y={__sq:this.__sq};
...
if(q){z[t].apply=a;
}if(this.__sn&&this.__sn.getValidationRule){var v=this.__sn.getValidationRule(s,t);

if(v){z[t].validate=f+t;
y[f+t]=v;
}}}if(this.__sn&&this.__sn.getModelSuperClass){var x=this.__sn.getModelSuperClass(s)||qx.core.Object;
}else{var x=qx.core.Object;
}var u=[];

if(this.__sn&&this.__sn.getModelMixins){var w=this.__sn.getModelMixins(s);
if(!qx.lang.Type.isArray(w)){if(w!=null){u=[w];
}}}if(q){u.push(qx.data.marshal.MEventBubbling);
}var r={extend:x,include:u,properties:z,members:y,destruct:this.__sr};
qx.Class.define(h+s,r);
},__sr:function(){var A=qx.util.PropertyUtil.getAllProperties(this.constructor);

for(var B in A){this.__sq(this.get(A[B].name));
}},__sq:function(C){if(!(C instanceof qx.core.Object)){return ;
}if(C.isDisposed()){return;
}if(qx.Class.implementsInterface(C,qx.data.IListData)){for(var i=0;i<C.getLength();i++){this.__sq(C.getItem(i));
}}C.dispose();
},__ss:function(D){var E;
if(this.__sn&&this.__sn.getModelClass){E=this.__sn.getModelClass(D);
}
...
}else if(qx.lang.Type.isObject(G)){var H=this.__sp(G);
var L=this.__ss(H);
...
s.addListener(k,this.__su(q),this);
s.addListener(c,qx.lang.Function.bind(this.__st,this,q),this);
},__st:function(u,e){if(e.getData()==b||e.getData()==g||e.getData()==m){u.setState(l);
}},__su:function(v){return function(e){var w=e.getData();
if(w==m){v.setState(l);
return;
}v.setArticles(w);
v.setState(k);
...
(function(){var v="single",u="Boolean",t="one",s="changeSelection",r="__oc",q="mouseup",p="mousedown",o="losecapture",n="multi",m="_applyQuickSelection",d="mouseover",l="_applySelectionMode",h="_applyDragSelection",c="qx.ui.core.MMultiSelectionHandling",b="removeItem",g="keypress",f="qx.event.type.Data",j="addItem",a="additive",k="mousemove";
qx.Mixin.define(c,{construct:function(){var x=this.SELECTION_MANAGER;
var w=this.__oc=new x(this);
...
},events:{"changeSelection":f},properties:{selectionMode:{check:[v,n,a,t],init:v,apply:l},dragSelection:{check:u,init:false,apply:h},quickSelection:{check:u,init:false,apply:m}},members:{__oc:null,selectAll:function(){this.__oc.selectAll();
...
}return this.__oc.isItemSelected(y);
...
}this.__oc.removeItem(A);
},selectRange:function(B,C){this.__oc.selectItemRange(B,C);
},resetSelection:function(){this.__oc.clearSelection();
...
if(!qx.lang.Array.equals(E,D)){this.__oc.replaceSelection(D);
}}},getSelection:function(){return this.__oc.getSelection();
},getSortedSelection:function(){return this.__oc.getSortedSelection();
},isSelectionEmpty:function(){return this.__oc.isSelectionEmpty();
},getSelectionContext:function(){return this.__oc.getSelectionContext();
},_getManager:function(){return this.__oc;
},getSelectables:function(F){return this.__oc.getSelectables(F);
},invertSelection:function(){this.__oc.invertSelection();
},_getLeadItem:function(){var G=this.__oc.getMode();

if(G===v||G===t){return this.__oc.getSelectedItem();
}else{return this.__oc.getLeadItem();
}},_applySelectionMode:function(H,I){this.__oc.setMode(H);
},_applyDragSelection:function(J,K){this.__oc.setDrag(J);
},_applyQuickSelection:function(L,M){this.__oc.setQuick(L);
...
this.addListener(c,this.__my,this);
...
},__my:function(e){var m=e.getOldData()[0];
var n=e.getData()[0];
...
qx.Class.define(c,{extend:qx.core.Object,implement:[qx.ui.decoration.IDecorator],type:g,properties:{insetLeft:{check:i,nullable:true,apply:h},insetRight:{check:i,nullable:true,apply:h},insetBottom:{check:i,nullable:true,apply:h},insetTop:{check:i,nullable:true,apply:h},insets:{group:[e,f,d,a],mode:b}},members:{__nQ:null,_getDefaultInsets:function(){throw new Error("Abstract method called.");
},_isInitialized:function(){throw new Error("Abstract method called.");
},_resetInsets:function(){this.__nQ=null;
},getInsets:function(){if(this.__nQ){return this.__nQ;
}var j=this._getDefaultInsets();
return this.__nQ={left:this.getInsetLeft()==null?j.left:this.getInsetLeft(),right:this.getInsetRight()==null?j.right:this.getInsetRight(),bottom:this.getInsetBottom()==null?j.bottom:this.getInsetBottom(),top:this.getInsetTop()==null?j.top:this.getInsetTop()};
...
}},statics:{IS_SUPPORTED:qx.bom.element.Style.isPropertySupported("borderImage")},properties:{borderImage:{check:t,nullable:true,apply:j},sliceTop:{check:h,init:0,apply:j},sliceRight:{check:h,init:0,apply:j},sliceBottom:{check:h,init:0,apply:j},sliceLeft:{check:h,init:0,apply:j},slice:{group:[n,v,a,w],mode:c},repeatX:{check:[i,e,d],init:i,apply:j},repeatY:{check:[i,e,d],init:i,apply:j},repeat:{group:[u,m],mode:c}},members:{__qx:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__qx;
},getMarkup:function(){if(this.__qx){return this.__qx;
...
this.__qx=[x,qx.bom.element.Style.compile({"borderImage":o+B+p+C.join(f)+f+D,position:k,lineHeight:0,fontSize:0,overflow:l,boxSizing:r,borderWidth:C.join(b)+g}),y].join(q);
return this.__qx;
...
},_onMouseDown:function(e){if(this.__oI){return;
}var bg=this.__oS;
var be=this.getChildControl(k);
var bf=bg?f:d;
var bi=bg?e.getDocumentLeft():e.getDocumentTop();
var bj=this.__oF=qx.bom.element.Location.get(this.getContentElement().getDomElement())[bf];
var bh=this.__oG=qx.bom.element.Location.get(be.getContainerElement().getDomElement())[bf];

if(e.getTarget()===be){this.__oI=true;

if(!this.__oO){this.__oO=new qx.event.Timer(100);
this.__oO.addListener(U,this._fireValue,this);
}this.__oO.start();
this.__oJ=bi+bj-bh;
be.addState(b);
}else{this.__oK=true;
...
this.__oN.addListener(U,this._onInterval,this);
}this.__oN.start();
}this.addListener(T,this._onMouseMove);
this.capture();
e.stopPropagation();
},_onMouseUp:function(e){if(this.__oI){this.releaseCapture();
delete this.__oI;
this.__oO.stop();
this._fireValue();
delete this.__oJ;
...
if(this.__oS){bl=e.getDocumentLeft()-(this._valueToPosition(this.getValue())+this.__oF);
...
}else{bl=e.getDocumentTop()-(this._valueToPosition(this.getValue())+this.__oF);
...
if(bm<0||bm>this.__oH||bl<0||bl>this.__oH){this.getChildControl(k).removeState(g);
}}}else if(this.__oK){this.__oN.stop();
this.releaseCapture();
delete this.__oK;
delete this.__oL;
delete this.__oM;
...
}},_onMouseMove:function(e){if(this.__oI){var bo=this.__oS?e.getDocumentLeft():e.getDocumentTop();
var bn=bo-this.__oJ;
this.slideTo(this._positionToValue(bn));
}else if(this.__oK){this.__oT(e);
}e.stopPropagation();
},_onInterval:function(e){var bp=this.getValue()+(this.__oL*this.getPageStep());
if(bp<this.getMinimum()){bp=this.getMinimum();
}else if(bp>this.getMaximum()){bp=this.getMaximum();
}var bq=this.__oL==-1;

if((bq&&bp<=this.__oM)||(!bq&&bp>=this.__oM)){bp=this.__oM;
}this.slideTo(bp);
},_onUpdate:function(e){var bs=this.getInnerSize();
var bt=this.getChildControl(k).getBounds();
var br=this.__oS?z:u;
this._updateKnobSize();
this.__oR=bs[br]-bt[br];
this.__oH=bt[br];
this._updateKnobPosition();
},__oS:false,__oR:0,__oT:function(e){var bu=this.__oS;
...
var bE=this.__oL<0?H:v;
bz=bA+(Math[bE]((bz-bA)/by)*by);
}if(this.__oM==null||(this.__oL==-1&&bz<=this.__oM)||(this.__oL==1&&bz>=this.__oM)){this.__oM=bz;
}},_positionToValue:function(bG){var bH=this.__oR;
...
},_valueToPosition:function(bK){var bL=this.__oR;
...
if(this.__oS){bP.setStyle(f,bO+a,true);
...
if(bQ==null){return;
}if(this.__oS){this.getChildControl(k).setWidth(Math.round(bR*bQ.width));
...
this.__oS=bU===j;
if(this.__oS){this.removeState(i);
bW.removeState(i);
this.addState(j);
bW.addState(j);
...
}},properties:{baseImage:{check:p,nullable:true,apply:r}},members:{__qy:null,__qz:null,__qA:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__qy;
},getMarkup:function(){if(this.__qy){return this.__qy;
...
D.push(b);
return this.__qy=D.join(o);
},resize:function(E,F,G){var H=this.__qA;
...
var O=Q[1];
var N=Q[2];
var M=this.__qz={tl:O+u+N,t:O+v+N,tr:O+d+N,bl:O+m+N,b:O+q+N,br:O+w+N,l:O+c+N,c:O+k+N,r:O+t+N};
this.__qA=this._computeEdgeSizes(M);
...
this.addListener(p,function(){this.__nW={"x":0,"y":0};
},this);
this.__nW={};
this.__nX={};
}},properties:{appearance:{refine:true,init:v},width:{refine:true,init:100},height:{refine:true,init:200},scrollbarX:{check:[h,d,D],init:h,themeable:true,apply:c},scrollbarY:{check:[h,d,D],init:h,themeable:true,apply:c},scrollbar:{group:[w,m]}},members:{__nW:null,__nX:null,_createChildControlImpl:function(F,G){var H;
...
if(ba&&Y){if(this.__nW[V]==0){var X=0;
}else{var X=-(e[b+W]()-this.__nW[V]);
}this.__nW[V]=e[b+W]();
Y.scrollBy(X);
if(this.__nX[V]){clearTimeout(this.__nX[V]);
this.__nX[V]=null;
}this.__nX[V]=setTimeout(qx.lang.Function.bind(function(bb){this.__nY(bb,V);
},this,X),100);
}},__nY:function(bc,bd){this.__nX[bd]=null;
var bf=this._isChildControlVisible(f+bd);

if(bc==0||!bf){return;
}if(bc>0){bc=Math.max(0,bc-3);
}else{bc=Math.min(0,bc+3);
}this.__nX[bd]=setTimeout(qx.lang.Function.bind(function(bg,bh){this.__nY(bg,bh);
},this,bc,bd),20);
...
this.__oA=g;
},members:{__oA:null,_isSelectable:function(h){return this._isItemSelectable(h)&&h.getLayoutParent()===this.__oA;
},_selectableToHashCode:function(j){return j.$$hash;
},_styleSelectable:function(k,m,n){n?k.addState(m):k.removeState(m);
},_capture:function(){this.__oA.capture();
},_releaseCapture:function(){this.__oA.releaseCapture();
},_isItemSelectable:function(o){if(this._userInteraction){return o.isVisible()&&o.isEnabled();
}else{return o.isVisible();
}},_getWidget:function(){return this.__oA;
},_getLocation:function(){var p=this.__oA.getContentElement().getDomElement();
return p?qx.bom.element.Location.get(p):null;
},_getDimension:function(){return this.__oA.getInnerSize();
...
},_scrollBy:function(u,v){},_scrollItemIntoView:function(w){this.__oA.scrollChildIntoView(w);
},getSelectables:function(x){var y=false;

if(!x){y=this._userInteraction;
this._userInteraction=true;
}var B=this.__oA.getChildren();
var z=[];
var A;
...
},_getFirstSelectable:function(){var I=this.__oA.getChildren();

for(var i=0,l=I.length;i<l;i++){if(this._isItemSelectable(I[i])){return I[i];
}}return null;
},_getLastSelectable:function(){var J=this.__oA.getChildren();

for(var i=J.length-1;i>0;i--){if(this._isItemSelectable(J[i])){return J[i];
}}return null;
},_getRelatedSelectable:function(K,L){var O=this.__oA.getOrientation()===f;
var N=this.__oA.getChildren();
var M=N.indexOf(K);
var P;
...
(function(){var m="horizontal",k="qx.event.type.Data",j="vertical",h="",g="qx.ui.form.List",f="__qp",d="Boolean",c="one",b="addChildWidget",a="_applySpacing",y="Enter",x="Integer",w="action",v="keyinput",u="addItem",t="removeChildWidget",s="_applyOrientation",r="single",q="keypress",p="list",n="pane",o="removeItem";
...
if(!(H===r||H===c)){return;
}if(((new Date).valueOf()-this.__qr)>1000){this.__qq=h;
}this.__qq+=e.getChar();
var I=this.findItemByLabelFuzzy(this.__qq);
if(I){this.setSelection([I]);
}this.__qr=(new Date).valueOf();
...
(function(){var m="open",k="icon",j="auto",h="middle",g="String",f="label",d="changeOpen",c="excluded",b="visible",a="opened",H="always",G="qx.ui.tree.AbstractTreeItem",F="_applyIconOpened",E="Boolean",D="__pg",C="Integer",B="_applyIndent",A="__ph",z="changeOpenSymbolMode",y="_applyOpenSymbolMode",t="resize",u="",r="removeItem",s="addItem",p="iconOpened",q="abstract",n="never",o="_applyIcon",v="__pk",w="_applyOpen",x="_applyLabel";
...
this._addWidgets();
this.initOpen();
},properties:{open:{check:E,init:false,event:d,apply:w},openSymbolMode:{check:[H,n,j],init:j,event:z,apply:y},indent:{check:C,init:19,apply:B,themeable:true},parent:{check:G,nullable:true},icon:{check:g,apply:o,nullable:true,themeable:true},iconOpened:{check:g,apply:F,nullable:true,themeable:true},label:{check:g,apply:x,init:u}},members:{__pg:null,__ph:null,__pi:null,__pj:null,__pk:null,__pl:null,_addWidgets:function(){throw new Error("Abstract method call.");
...
}}},_applyIcon:function(Y,ba){if(!this.__pn()){this.__po(Y);
}else if(!this.isOpen()){this.__po(Y);
}},_applyIconOpened:function(bb,bc){if(this.isOpen()){if(this.__pm()&&this.__pn()){this.__po(bb);
}else if(!this.__pm()&&this.__pn()){this.__po(bb);
...
if(bg){bi=this.__pn()?this.getIconOpened():null;
}else{bi=this.getIcon();
}
if(bi){this.__po(bi);
}bg?this.addState(a):this.removeState(a);
},__pm:function(){return qx.util.PropertyUtil.getUserValue(this,k);
},__pn:function(){return qx.util.PropertyUtil.getUserValue(this,p);
},__po:function(bj){var bk=this.getChildControl(k,true);
...
if(this.__pk){this.__pk.setWidth((this.getLevel()+1)*this.getIndent()-bq);
...
},getChildrenContainer:function(){if(!this.__ph){this.__ph=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?b:c});
}return this.__ph;
...
},hasChildren:function(){return this.__pg?this.__pg.length>0:false;
...
if(bL==this.__pg.length){this.add(bK);
return;
}var bP=bK.getParent();
...
}qx.lang.Array.insertAt(this.__pg,bK,bL);
...
if(bS){bS.remove(bQ);
}this.addAt(bQ,this.__pg.indexOf(bR));
},addAfter:function(bT,bU){{};
var bV=bT.getParent();

if(bV){bV.remove(bT);
}this.addAt(bT,this.__pg.indexOf(bU)+1);
},addAtBegin:function(bW){this.addAt(bW,0);
},remove:function(bX){for(var i=0,l=arguments.length;i<l;i++){var cc=arguments[i];

if(this.__pg.indexOf(cc)==-1){this.warn("Cannot remove treeitem '"+cc+"'. It is not a child of this tree item.");
return;
...
}}qx.lang.Array.remove(this.__pg,cc);
cc.setParent(null);
bY.remove(cc);
}var ca=this.getTree();
...
},removeAt:function(cd){var ce=this.__pg[cd];

if(ce){this.remove(ce);
}},removeAll:function(){for(var i=this.__pg.length-1;i>=0;i--){this.remove(this.__pg[i]);
}}},destruct:function(){this._disposeArray(D);
this._disposeObjects(v,A);
}});
})();
...
(function(){var n="pressed",m="abandoned",l="Integer",k="hovered",j="qx.event.type.Event",i="Enter",h="Space",g="press",f="qx.ui.form.RepeatButton",d="release",a="__ny",c="interval",b="execute";
qx.Class.define(f,{extend:qx.ui.form.Button,construct:function(o,p){qx.ui.form.Button.call(this,o,p);
this.__ny=new qx.event.AcceleratingTimer();
this.__ny.addListener(c,this._onInterval,this);
},events:{"execute":j,"press":j,"release":j},properties:{interval:{check:l,init:100},firstInterval:{check:l,init:500},minTimer:{check:l,init:20},timerDecrease:{check:l,init:2}},members:{__nz:null,__ny:null,press:function(){if(this.isEnabled()){if(!this.hasState(n)){this.__nA();
}this.removeState(m);
this.addState(n);
}},release:function(q){if(!this.isEnabled()){return;
}if(this.hasState(n)){if(!this.__nz){this.execute();
}}this.removeState(n);
this.removeState(m);
this.__nB();
...
if(this.hasState(n)&&!this.__nz){this.execute();
}}this.__nB();
e.stopPropagation();
},_onKeyUp:function(e){switch(e.getKeyIdentifier()){case i:case h:if(this.hasState(n)){if(!this.__nz){this.execute();
}this.removeState(n);
this.removeState(m);
e.stopPropagation();
this.__nB();
...
e.stopPropagation();
this.__nA();
}},_onInterval:function(e){this.__nz=true;
this.fireEvent(b);
},__nA:function(){this.fireEvent(g);
this.__nz=false;
this.__ny.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();
this.removeState(m);
this.addState(n);
...
(function(){var t="configured",s="completed",r="changeState",q="changeModel",p="__sv",o="qx.data.store.Json",n="GET",m="_marshaler",l="sending",k="application/json",d="changeUrl",j="failed",g="loaded",c="timeout",b="queued",f="String",e="aborted",h="_applyUrl",a="receiving",i="qx.event.type.Data";
qx.Class.define(o,{extend:qx.core.Object,construct:function(u,v){qx.core.Object.call(this);
this._marshaler=new qx.data.marshal.Json(v);
this._delegate=v;

if(u!=null){this.setUrl(u);
}},events:{"loaded":i},properties:{model:{nullable:true,event:q},state:{check:[t,b,l,a,s,e,c,j],init:t,event:r},url:{check:f,apply:h,event:d,nullable:true}},members:{__sv:null,_delegate:null,_applyUrl:function(w,x){if(w!=null){this._createRequest(w);
}},_createRequest:function(y){this.__sv=new qx.io.remote.Request(y,n,k);
this.__sv.addListener(s,this.__sw,this);
var z=this._delegate;

if(z&&qx.lang.Type.isFunction(z.configureRequest)){this._delegate.configureRequest(this.__sv);
}this.__sv.addListener(r,function(A){this.setState(A.getData());
},this);
this.__sv.send();
},__sw:function(B){var D=B.getContent();
...
}}},destruct:function(){this._disposeObjects(m,p);
this._delegate=null;
}});
})();
...
}},__nx:function(t){while(!(t instanceof qx.ui.menu.Menu)){t=t.getLayoutParent();
}return t;
...
},properties:{callbackParam:{check:a,nullable:false}},members:{__sx:null,_createRequest:function(m){if(this.__sx){this.__sx.dispose();
}this.__sx=new qx.io.ScriptLoader();
var o=this._delegate;

if(o&&qx.lang.Type.isFunction(o.configureRequest)){this._delegate.configureRequest(this.__sx);
}var n=m.indexOf(i)==-1?i:h;
m+=n+this.getCallbackParam()+c;
var p=parseInt(this.toHashCode(),10);
qx.data.store.Jsonp[p]=this;
m+=f+p+e;
this.__sx.load(m,function(q){delete this[p];
},this);
},callback:function(r){if(this.isDisposed()){return;
}this.__sy(r);
},__sy:function(s){if(s==undefined){this.setState(d);
return;
...
this.fireDataEvent(g,this.getModel());
}},destruct:function(){if(this.__sx){this.__sx.dispose();
}this.__sx=null;
}});
})();
...
}},properties:{backgroundColor:{check:b,nullable:true,apply:c}},members:{__nR:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__nR;
},getMarkup:function(){if(this.__nR){return this.__nR;
...
}this.__pC=qx.bom.Iframe.create({id:I,name:I,src:K});
qx.bom.element.Style.set(this.__pC,z,A);
this.__pD=qx.bom.Element.create(o,{id:J,name:J,target:I});
qx.bom.element.Style.set(this.__pD,z,A);
qx.dom.Element.insertEnd(this.__pD,qx.dom.Node.getBodyElement(document));
this.__pE=qx.bom.Element.create(c,{id:b,name:b});
qx.dom.Element.insertEnd(this.__pE,this.__pD);
qx.dom.Element.insertEnd(this.__pC,qx.dom.Node.getBodyElement(document));
qx.event.Registration.addListener(this.__pC,a,this._onload,this);
this.__pF=qx.lang.Function.listener(this._onreadystatechange,this);
qx.bom.Event.addNativeListener(this.__pC,D,this.__pF);
...
},_numericMap:{"uninitialized":1,"loading":2,"loaded":2,"interactive":3,"complete":4}},members:{__pE:null,__pG:0,__pD:null,__pC:null,__pF:null,send:function(){var M=this.getMethod();
var O=this.getUrl();
...
this.__pD.appendChild(Q);
}this.__pD.action=O;
this.__pD.method=M;
this.__pE.appendChild(document.createTextNode(this.getData()));
this.__pD.submit();
this.setState(r);
},_onload:qx.event.GlobalError.observeMethod(function(e){if(qx.bom.client.Engine.NAME==u&&this.getIframeHtmlContent()==k){return;
}
if(this.__pD.src){return;
}this._switchReadyState(qx.io.remote.transport.Iframe._numericMap.complete);
}),_onreadystatechange:qx.event.GlobalError.observeMethod(function(e){this._switchReadyState(qx.io.remote.transport.Iframe._numericMap[this.__pC.readyState]);
}),_switchReadyState:function(T){switch(this.getState()){case E:case s:case p:case v:this.warn("Ignore Ready State Change");
return;
}while(this.__pG<T){this.setState(qx.io.remote.Exchange._nativeMap[++this.__pG]);
}},setRequestHeader:function(U,V){},getResponseHeader:function(W){return null;
},getResponseHeaders:function(){return {};
},getStatusCode:function(){return 200;
},getStatusText:function(){return k;
},getIframeWindow:function(){return qx.bom.Iframe.getWindow(this.__pC);
},getIframeDocument:function(){return qx.bom.Iframe.getDocument(this.__pC);
},getIframeBody:function(){return qx.bom.Iframe.getBody(this.__pC);
...
},destruct:function(){if(this.__pC){qx.event.Registration.removeListener(this.__pC,a,this._onload,this);
qx.bom.Event.removeNativeListener(this.__pC,D,this.__pF);
if(qx.core.Variant.isSet(d,y)){this.__pC.src=qx.util.ResourceManager.getInstance().toUri(w);
}qx.dom.Element.remove(this.__pC);
}
if(this.__pD){qx.dom.Element.remove(this.__pD);
}this.__pC=this.__pD=this.__pE=null;
}});
})();
(function(){var t="keypress",s="interval",r="keydown",q="mousedown",p="keyup",o="blur",n="Enter",m="__nk",l="__nl",k="__nm",c="Up",j="Escape",g="qx.ui.menu.Manager",b="Left",a="Down",f="Right",d="singleton",h="Space";
...
this.__nl.addListener(s,this._onOpenInterval,this);
this.__nm=new qx.event.Timer;
this.__nm.addListener(s,this._onCloseInterval,this);
},members:{__nn:null,__no:null,__nl:null,__nm:null,__nk:null,_getChild:function(w,x,y,z){var A=w.getChildren();
var length=A.length;
var B;
...
}},hideAll:function(){var I=this.__nk;

if(I){for(var i=I.length-1;i>=0;i--){I[i].exclude();
}}},getActiveMenu:function(){var J=this.__nk;
return J.length>0?J[J.length-1]:null;
},scheduleOpen:function(K){this.cancelClose(K);
if(K.isVisible()){if(this.__nn){this.cancelOpen(this.__nn);
}}else if(this.__nn!=K){this.__nn=K;
this.__nl.restartWith(K.getOpenInterval());
}},scheduleClose:function(L){this.cancelOpen(L);
if(!L.isVisible()){if(this.__no){this.cancelClose(this.__no);
}}else if(this.__no!=L){this.__no=L;
this.__nm.restartWith(L.getCloseInterval());
}},cancelOpen:function(M){if(this.__nn==M){this.__nl.stop();
this.__nn=null;
}},cancelClose:function(N){if(this.__no==N){this.__nm.stop();
this.__no=null;
}},_onOpenInterval:function(e){this.__nl.stop();
this.__nn.open();
this.__nn=null;
},_onCloseInterval:function(e){this.__nm.stop();
this.__no.exclude();
this.__no=null;
...
}if(this.__nk.length>0&&!this._isInMenu(O)){this.hideAll();
}},__np:{"Enter":1,"Space":1},__nq:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var P=this.getActiveMenu();

if(!P){return;
}var Q=e.getKeyIdentifier();

if(this.__nq[Q]||(this.__np[Q]&&P.getSelectedButton())){e.stopPropagation();
...
}},properties:{selection:{check:d,event:f,apply:b,init:null}},members:{_modifingSelection:0,__sz:null,__sA:null,_applySelection:function(g,h){if(this.__sA!=undefined&&h!=undefined){h.removeListenerById(this.__sA);
}this.__sA=g.addListener(e,this.__sB,this);
this._updateSelection();
},__sB:function(){this._updateSelection();
},_changeTargetSelection:function(){if(this.getTarget()==null){return;
}if(!this.__sC()&&!this.__sD()){return;
...
},_addChangeTargetListener:function(m,n){if(this.__sz!=undefined&&n!=undefined){n.removeListenerById(this.__sz);
}
if(m!=null){if(this.__sC()||this.__sD()){this.__sz=m.addListener(f,this._changeTargetSelection,this);
...
}}}else if(this.__sD()){var p=this.getSelection().getItem(this.getSelection().length-1);

if(p){this.__sE(p);
this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();
}else{this.getTarget().resetSelection();
}}this._endSelectionModification();
},__sC:function(){var t=this.getTarget().constructor;
return qx.Class.implementsInterface(t,qx.ui.core.IMultiSelection);
},__sD:function(){var u=this.getTarget().constructor;
return qx.Class.implementsInterface(u,qx.ui.core.ISingleSelection);
},__sE:function(v){var w=this.__sF(v);
if(w==null){return;
}if(this.__sC()){this.getTarget().addToSelection(w);
}else if(this.__sD()){this.getTarget().setSelection([w]);
}},__sF:function(x){var y=this.getTarget().getSelectables(true);
...
if(qx.ui.decoration.css3.BorderImage.IS_SUPPORTED){this.__qv=new qx.ui.decoration.css3.BorderImage();

if(n){this.__qw(n);
}}else{this.__qv=new qx.ui.decoration.GridDiv(n);
}
if(o!=null){this.__qv.setInsets(o);
}},properties:{baseImage:{check:c,nullable:true,apply:h},insetLeft:{check:m,nullable:true,apply:l},insetRight:{check:m,nullable:true,apply:l},insetBottom:{check:m,nullable:true,apply:l},insetTop:{check:m,nullable:true,apply:l},insets:{group:[i,j,g,a],mode:e}},members:{__qv:null,getMarkup:function(){return this.__qv.getMarkup();
},resize:function(p,q,r){this.__qv.resize(p,q,r);
},tint:function(s,t){},getInsets:function(){return this.__qv.getInsets();
},_applyInsets:function(u,v,name){var w=f+qx.lang.String.firstUp(name);
this.__qv[w](u);
},_applyBaseImage:function(x,y){if(this.__qv instanceof qx.ui.decoration.GridDiv){this.__qv.setBaseImage(x);
}else{this.__qw(x);
}},__qw:function(z){var B,D;
this.__qv.setBorderImage(z);
...
}q.include=this.__dn(q.include);
q.patch=this.__dn(q.patch);
{};
...
}},__dn:function(s){if(!s){return [];
}
if(qx.Bootstrap.isArray(s)){return s;
}else{return [s];
}},__do:function(t,u){var v=u.aliases||{};
...
},__dp:function(w){for(var i=0,x=this.__dr,l=x.length;i<l;i++){if(w[x[i]]){return x[i];
}}},__dq:function(y,z){var C=this.__dp(z);
...
y[C]=new E;
},$$registry:{},__dr:[g,d,f,n,c,o,m,b],__ds:null,__dt:null,__du:function(){},patch:function(F,G){var I=this.__dp(G);

if(I!==this.__dp(F)){throw new Error("The mixins '"+F.name+"' are not compatible '"+G.name+"'!");
}var H=G[I];
...
if(this.getOrientation()==h){if(R.height<S.minHeight+this.__oE){Q=true;
}}else{if(R.width<S.minWidth+this.__oE){Q=true;
}}
if(Q){P.exclude();
}else{P.show();
}}}});
...
(function(){var s="Integer",r="aborted",q="_onaborted",p="_on",o="_applyEnabled",n="Boolean",m="__px",l="sending",k="interval",j="__pz",c="failed",h="qx.io.remote.RequestQueue",g="timeout",b="completed",a="queued",f="receiving",d="singleton";
qx.Class.define(h,{type:d,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__pw=[];
this.__px=[];
this.__py=0;
this.__pz=new qx.event.Timer(500);
this.__pz.addListener(k,this._oninterval,this);
},properties:{enabled:{init:true,check:n,apply:o},maxTotalRequests:{check:s,nullable:true},maxConcurrentRequests:{check:s,init:qx.bom.client.Transport.getMaxConcurrentRequestCount()},defaultTimeout:{check:s,init:5000}},members:{__pw:null,__px:null,__py:null,__pz:null,getRequestQueue:function(){return this.__pw;
},getActiveQueue:function(){return this.__px;
},_debug:function(){var t;
{};
},_check:function(){this._debug();
if(this.__px.length==0&&this.__pw.length==0){this.__pz.stop();
}if(!this.getEnabled()){return;
}if(this.__pw.length==0||(this.__pw[0].isAsynchronous()&&this.__px.length>=this.getMaxConcurrentRequests())){return;
}if(this.getMaxTotalRequests()!=null&&this.__py>=this.getMaxTotalRequests()){return;
}var u=this.__pw.shift();
...
v.send();
if(this.__pw.length>0){this._check();
}},_remove:function(w){qx.lang.Array.remove(this.__px,w);
w.dispose();
this._check();
},__pA:0,_onsending:function(e){{};
...
}},_oninterval:function(e){var H=this.__px;

if(H.length==0){this.__pz.stop();
return;
...
}this.__pz.setEnabled(I);
},add:function(K){K.setState(a);

if(K.isAsynchronous()){this.__pw.push(K);
}else{this.__pw.unshift(K);
}this._check();

if(this.getEnabled()){this.__pz.start();
}},abort:function(L){var M=L.getTransport();

if(M){M.abort();
}else if(qx.lang.Array.contains(this.__pw,L)){qx.lang.Array.remove(this.__pw,L);
...
var s=++qx.io.remote.transport.Script.__pY;

if(s>=2000000000){qx.io.remote.transport.Script.__pY=s=1;
}this.__qa=null;
this.__pY=s;
},statics:{__pY:0,_instanceRegistry:{},ScriptTransport_PREFIX:f,ScriptTransport_ID_PARAM:b,ScriptTransport_DATA_PARAM:h,handles:{synchronous:false,asynchronous:true,crossDomain:true,fileUpload:false,programaticFormFields:false,responseTypes:[m,l,o]},isSupported:function(){return true;
...
}})},members:{__qb:0,__qa:null,__pY:null,send:function(){var x=this.getUrl();
x+=(x.indexOf(p)>=0?r:p)+qx.io.remote.transport.Script.ScriptTransport_ID_PARAM+q+this.__pY;
...
this.__qa=document.createElement(c);
this.__qa.charset=e;
this.__qa.src=x;
{};
...
return;
}while(this.__qb<B){this.setState(qx.io.remote.Exchange._nativeMap[++this.__qb]);
...
},events:{"execute":R},properties:{enabled:{init:true,check:l,event:o,apply:I},shortcut:{check:n,apply:Q,nullable:true},autoRepeat:{check:l,init:false}},members:{__mT:g,__mU:g,execute:function(U){this.fireDataEvent(L,U);
},__mV:function(event){if(this.getEnabled()&&this.__mX(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},__mW:function(event){if(this.getEnabled()&&this.__mX(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},_applyEnabled:function(V,W){if(V){qx.event.Registration.addListener(document.documentElement,h,this.__mV,this);
qx.event.Registration.addListener(document.documentElement,d,this.__mW,this);
}else{qx.event.Registration.removeListener(document.documentElement,h,this.__mV,this);
qx.event.Registration.removeListener(document.documentElement,d,this.__mW,this);
}},_applyShortcut:function(X,Y){if(X){if(X.search(/[\s]+/)!=-1){var bc=y;
this.error(bc);
throw new Error(bc);
}this.__mT={"Control":false,"Shift":false,"Meta":false,"Alt":false};
this.__mU=null;
var ba;
var a=[];
...
for(var i=0;i<bb;i++){var bd=this.__na(a[i]);

switch(bd){case f:case u:case K:case C:this.__mT[bd]=true;
break;
case m:var bc=F+a[i];
this.error(bc);
throw bc;
default:if(this.__mU){var bc=r;
this.error(bc);
throw bc;
}this.__mU=bd;
}}}return true;
},__mX:function(e){var be=this.__mU;

if(!be){return ;
}if((!this.__mT.Shift&&e.isShiftPressed())||(this.__mT.Shift&&!e.isShiftPressed())||(!this.__mT.Control&&e.isCtrlPressed())||(this.__mT.Control&&!e.isCtrlPressed())||(!this.__mT.Meta&&e.isMetaPressed())||(this.__mT.Meta&&!e.isMetaPressed())||(!this.__mT.Alt&&e.isAltPressed())||(this.__mT.Alt&&!e.isAltPressed())){return false;
}
if(be==e.getKeyIdentifier()){return true;
}return false;
},__mY:{esc:S,ctrl:f,print:P,del:q,pageup:b,pagedown:E,numlock:O,numpad_0:J,numpad_1:D,numpad_2:t,numpad_3:s,numpad_4:A,numpad_5:N,numpad_6:B,numpad_7:H,numpad_8:M,numpad_9:z,numpad_divide:p,numpad_multiply:v,numpad_minus:c,numpad_plus:k},__na:function(bf){var bg=qx.event.handler.Keyboard;
var bh=m;
...
var bh=this.__mY[bf]||qx.lang.String.firstUp(bf);

if(bg.isValidKeyIdentifier(bh)){return bh;
}else{return m;
}},toString:function(){var bk=this.__mU;
var bj=[];

for(var bi in this.__mT){if(this.__mT[bi]){bj.push(qx.locale.Key.getKeyName(j,bi));
}}
if(bk){bj.push(qx.locale.Key.getKeyName(j,bk));
}return bj.join(k);
}},destruct:function(){this.setEnabled(false);
this.__mT=this.__mU=null;
}});
})();
...
(function(){var o="static",n="execute",m="user",l="settings",k="loading",j="addfeed",h="selection[0]",g="title",f="north",d="main",bw="new",bv="Surfin' Safari",bu="changeModel",bt="selection[0].state",bs="Heise",br="__sN",bq="__sT",bp="F1",bo="article",bn="partLoaded",v="Control+A",w="__sS",t="http://blogs.msdn.com/ie/rss.xml",u="http://my.opera.com/desktopteam/xml/rss/blog/",r="feedreader/images/loading22.gif",s="error",p="__sL",q="change",D="http://blogs.msdn.com/jscript/rss.xml",E="__sH",Q="__sU",M="loaded",Y="__sQ",T="feeds",bj="icon/22/places/folder.png",be="The Mozilla Blog",I="enabled",bm="__sK",bl="http://webkit.org/blog/feed/",bk="separator-vertical",H="http://feeds2.feedburner.com/qooxdoo/news/content",K="http://blog.mozilla.com/feed/",L="vertical",O="http://www.heise.de/newsticker/heise-atom.xml",R="__sO",U="state",bb="JScript Team Blog",bg="Feeds",x="http://feeds2.feedburner.com/ajaxian",y="Control+D",J="icon/22/actions/process-stop.png",X="selection[0].articles",W="__sJ",V="white",bd="IEBlog",bc="Ajaxian",S="User Feeds",ba="__sM",a="icon/22/apps/internet-feed-reader.png",bf="model",z="feedreader.Application",A="http://daringfireball.net/index.xml",N="Control+P",b="Static Feeds",c="Control+R",G="qooxdoo News",B="__sP",C="Opera Desktop Blog",F="feedreader.model.Feed",P="Daring Fireball",bi="__sR",bh="__sG";
qx.Class.define(z,{extend:qx.application.Standalone,members:{__sG:null,__sH:null,__sI:null,__sJ:null,__sK:null,__sL:null,__sM:null,__sN:null,__sO:null,__sP:null,__sQ:null,__sR:null,__sS:null,__sT:null,__sU:null,main:function(){qx.application.Standalone.prototype.main.call(this);
{};
qx.io.PartLoader.getInstance().addListener(bn,function(e){this.debug("part loaded: "+e.getData().getName());
},this);
...
this._setUpBinding();
this.__sJ.getRoot().setOpen(true);
this.__sJ.getRoot().getChildren()[0].setOpen(true);
this.__sJ.getRoot().getChildren()[1].setOpen(true);
this.__sJ.setHideRoot(true);
this.__sG.getSelection().push(this.__sU.getFeeds().getItem(0));
this.reload();
},_initializeModel:function(){this.__sS=new feedreader.model.FeedFolder(bg);
this.__sU=new feedreader.model.FeedFolder(this.tr(b));
this.__sS.getFeeds().push(this.__sU);
this.__sU.getFeeds().push(new feedreader.model.Feed(G,H,o));
this.__sU.getFeeds().push(new feedreader.model.Feed(bb,D,o));
this.__sU.getFeeds().push(new feedreader.model.Feed(P,A,o));
this.__sU.getFeeds().push(new feedreader.model.Feed(bv,bl,o));
this.__sU.getFeeds().push(new feedreader.model.Feed(bc,x,o));
this.__sT=new feedreader.model.FeedFolder(this.tr(S));
this.__sS.getFeeds().push(this.__sT);
this.__sT.getFeeds().push(new feedreader.model.Feed(bs,O,m));
this.__sT.getFeeds().push(new feedreader.model.Feed(bd,t,m));
this.__sT.getFeeds().push(new feedreader.model.Feed(be,K,m));
this.__sT.getFeeds().push(new feedreader.model.Feed(C,u,m));
},_setUpBinding:function(){var bz={converter:this._state2iconConverter};
this.__sG=new qx.data.controller.Tree(this.__sS,this.__sJ,T,g);
this.__sG.setIconOptions(bz);
this.__sG.setIconPath(U);
this.__sH=new qx.data.controller.List(null,this.__sM.getList(),g);
this.__sG.bind(X,this.__sH,bf);
this.__sH.bind(h,this.__sQ,bo);
this.__sH.getSelection().addListener(q,this._listControllerChange,this);
this.__sH.addListener(bu,this._treeControllerChange,this);
var bA={converter:this._state2loadingConverter};
this.__sG.bind(bt,this.__sM,k,bA);
bA={converter:this._category2enabledConverter};
this.__sG.bind(h,this.__sL.getRemoveButton(),I,bA);
},_category2enabledConverter:function(bB){return !!bB&&bB.getCategory()==m;
},_state2iconConverter:function(bC){if(bC==bw||bC==k){return r;
}else if(bC==M){return a;
}else if(bC==s){return J;
}return bj;
},_state2loadingConverter:function(bD){if(bD==bw||bD==k){return true;
}return false;
},_listControllerChange:function(bE){var bF=this.__sG.getSelection().getItem(0);
var bG=this.__sH.getSelection().getItem(0);
if(bG!=undefined){bF.setSelectedArticle(bG);
}},_treeControllerChange:function(bH){var bI=this.__sG.getSelection().getItem(0);
if(bI!=null&&bI.classname==F&&bI.getSelectedArticle()!=null&&bI.getArticles().contains(bI.getSelectedArticle())){qx.event.Timer.once(function(){this.__sH.getSelection().push(bI.getSelectedArticle());
},this,0);
}else{this.__sM.getList().scrollToY(0);
}},_createLayout:function(){var bJ=new qx.ui.layout.Dock();
bJ.setSeparatorY(bk);
var bK=new qx.ui.container.Composite(bJ);
this.getRoot().add(bK,{edge:0});
this.__sK=new feedreader.view.Header();
bK.add(this.__sK,{edge:f});
this.__sL=new feedreader.view.ToolBar(this);
bK.add(this.__sL,{edge:f});
this.__sN=new qx.ui.splitpane.Pane();
bK.add(this.__sN);
this.__sJ=new qx.ui.tree.Tree();
this.__sJ.setWidth(250);
this.__sJ.setBackgroundColor(V);
this.__sN.add(this.__sJ,0);
this.__sO=new qx.ui.splitpane.Pane(L);
this.__sO.setDecorator(null);
this.__sN.add(this.__sO,1);
this.__sM=new feedreader.view.List(this.__sS);
this.__sM.setHeight(200);
this.__sM.setDecorator(d);
this.__sO.add(this.__sM,0);
this.__sQ=new feedreader.view.Article();
this.__sQ.setDecorator(d);
this.__sO.add(this.__sQ,1);
},_initializeCommands:function(){var bL={};
bL.reload=new qx.ui.core.Command(c);
bL.reload.addListener(n,this.reload,this);
bL.about=new qx.ui.core.Command(bp);
...
bL.addFeed=new qx.ui.core.Command(v);
bL.addFeed.addListener(n,this.showAddFeed,this);
bL.removeFeed=new qx.ui.core.Command(y);
bL.removeFeed.addListener(n,this.removeFeed,this);
this.__sI=bL;
},getCommand:function(bM){return this.__sI[bM];
},addFeed:function(bN,bO,bP){var bQ=new feedreader.model.Feed(bN,bO,bP);
this.__sT.getFeeds().push(bQ);
var bR=feedreader.io.FeedLoader.getInstance();
bR.load(bQ);
},removeFeed:function(){var bS=this.__sG.getSelection().getItem(0);
if(bS&&bS.getCategory&&bS.getCategory()!==o){var bT=this.__sT.getFeeds();
for(var i=0;i<bT.length;i++){if(bS===bT.getItem(i)){bT.splice(i,1);
return;
}}}},reload:function(){var bU=feedreader.io.FeedLoader.getInstance();
bU.loadAll(this.__sS);
},showPreferences:function(){this.__sL.signalLoading(l,true);
qx.io.PartLoader.require([l],function(){if(!this.__sP){this.__sP=new feedreader.view.PreferenceWindow();
this.getRoot().add(this.__sP);
}this.__sP.center();
this.__sP.open();
this.__sL.signalLoading(l,false);
},this);
},showAbout:function(){alert(this.tr("FeedReader (qooxdoo powered)"));
},showAddFeed:function(){this.__sL.signalLoading(j,true);
qx.io.PartLoader.require([j],function(){if(!this.__sR){this.__sR=new feedreader.view.AddFeedWindow(this);
this.getRoot().add(this.__sR);
}this.__sR.center();
this.__sR.open();
this.__sL.signalLoading(j,false);
},this);
}},destruct:function(){this.__sI=null;
this._disposeObjects(p,ba,Y,W,w,br,R,bm,Q,bq,bh,E,B,bi);
}});
})();
(function(){var k="horizontal",j="px",i="scroll",h="vertical",g="-1px",f="qx.client",d="0",c="hidden",b="mousedown",a="qx.ui.core.scroll.NativeScrollBar",z="PositiveNumber",y="Integer",x="mousemove",w="_applyMaximum",v="_applyOrientation",u="appear",t="opera",s="PositiveInteger",r="mshtml",q="mouseup",o="Number",p="_applyPosition",m="scrollbar",n="__oD",l="native";
...
return {width:this.__oC?100:F,maxWidth:this.__oC?null:F,minWidth:this.__oC?null:F,height:this.__oC?F:100,maxHeight:this.__oC?F:null,minHeight:this.__oC?F:null};
...
if(this.__oC){content.scrollToX(J);
}else{content.scrollToY(J);
}},_applyOrientation:function(K,L){var M=this.__oC=K===k;
...
qx.ui.core.queue.Layout.add(this);
},_updateScrollBar:function(){var O=this.__oC;
...
var V=this.__oC?W.getScrollX():W.getScrollY();
this.setPosition(V);
...
},_applyIconOptions:function(G,H){this.__tj();
},_applyLabelOptions:function(I,J){this.__tj();
},_applyIconPath:function(K,L){this.__tj();
},_applyLabelPath:function(M,N){this.__tj();
},_applyModel:function(O,P){if(P!=undefined){if(this.__ta!=undefined){P.removeListenerById(this.__ta);
}
if(this.__tb!=undefined){P.removeListenerById(this.__tb);
}}if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length);
}if(O!=null){this.__ta=O.addListener(y,this.__tf,this);
this.__tb=O.addListener(k,this.__te,this);
this.__tl();
this.__tf();
...
for(var i=0;i<length;i++){this.__ti();
}}}},_applyTarget:function(R,S){this._addChangeTargetListener(R,S);
if(S!=undefined){S.removeAll();
this.removeAllBindings();
}
if(R!=null){if(this.getModel()!=null){for(var i=0;i<this.__sV.length;i++){this.__th(this.__tm(i));
}}}},__te:function(){this.__td=true;
qx.ui.core.queue.Widget.add(this);
if(this.__sV.length!=this.getModel().getLength()){this.update();
}},syncWidget:function(){if(this.__tc){this._changeTargetSelection();
}
if(this.__td){this._updateSelection();
}this.__td=this.__tc=null;
},__tf:function(){if(this.getTarget()==null){return;
}this.__tl();
var U=this.__sV.length;
var T=this.getTarget().getChildren().length;
if(U>T){for(var j=T;j<U;j++){this.__th(this.__tm(j));
}}else if(U<T){for(var j=T;j>U;j--){this.__ti();
}}},__tg:function(){var V=this.getModel();
if(V!=null){V.removeListenerById(this.__tb);
this.__tb=V.addListener(k,this.__te,this);
...
},__th:function(Y){var ba=this._createItem();
this._bindListItem(ba,Y);
this.getTarget().add(ba);
},__ti:function(){this._startSelectionModification();
...
if(!qx.lang.Array.contains(this.__sW,bn)){this.__sW.push(bn);
...
if(!qx.lang.Array.contains(this.__sX,bt)){this.__sX.push(bt);
}},_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;
}for(var i=0;i<this.__sW.length;i++){if(this.__sY[this.__sW[i]]!=null){this.__sY[this.__sW[i]]();
}}},_removeBindingsFrom:function(bD){for(var i=0;i<this.__sW.length;i++){var bE=bD.getUserData(this.__sW[i]+g);

if(bE!=null){this.removeBinding(bE);
}}for(var i=0;i<this.__sX.length;i++){var bE=bD.getUserData(this.__sX[i]+h);
...
this._bindListItem(bF[i],this.__tm(i));
}this.__tg();
...
this._endSelectionModification();
this._updateSelection();
},_setFilter:function(bO,bP){if((bO==null||bO.filter==null)&&(bP!=null&&bP.filter!=null)){this.__tk();
...
this.__tl();
if(bR.length>this.__sV.length){for(var j=bR.length;j>this.__sV.length;j--){this.getTarget().removeAt(j-1);
}}else if(bR.length<this.__sV.length){for(var j=bR.length;j<this.__sV.length;j++){var bS=this._createItem();
this.getTarget().add(bS);
}}var bQ=this.getTarget().getChildren();

for(var i=0;i<bQ.length;i++){this._bindListItem(bQ[i],this.__tm(i));
}this.__tg();
this._endSelectionModification();
this._updateSelection();
},__tk:function(){this.__tl();
this.__tf();
this.__tj();
this.__td=true;
qx.ui.core.queue.Widget.add(this);
},__tl:function(){var bV=this.getModel();
...
for(var i=0;i<bV.getLength();i++){if(bU==null||bU(bV.getItem(i))){this.__sV.push(i);
}}},__tm:function(bX){return this.__sV[bX];
}},destruct:function(){this.__sV=this.__sY=this.__sW=null;
this.__sX=null;
qx.ui.core.queue.Widget.remove(this);
}});
})();
(function(){var c="Integer",b="_applyLayoutChange",a="qx.ui.menu.Layout";
qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:c,init:0,apply:b},spanColumn:{check:c,init:1,nullable:true,apply:b},iconColumnWidth:{check:c,init:0,themeable:true,apply:b},arrowColumnWidth:{check:c,init:0,themeable:true,apply:b}},members:{__nw:null,_computeSizeHint:function(){var q=this._getLayoutChildren();
var o,g,j;
var e=this.getSpanColumn();
var h=this.__nw=[0,0,0,0];
...
},getColumnSizes:function(){return this.__nw||null;
...
(function(){var e="Integer",d="interval",c="__nC",b="qx.event.type.Event",a="qx.event.AcceleratingTimer";
qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__nC=new qx.event.Timer(this.getInterval());
this.__nC.addListener(d,this._onInterval,this);
},events:{"interval":b},properties:{interval:{check:e,init:100},firstInterval:{check:e,init:500},minimum:{check:e,init:20},decrease:{check:e,init:2}},members:{__nC:null,__nD:null,start:function(){this.__nC.setInterval(this.getFirstInterval());
this.__nC.start();
...
this.addListener(s,this.__kC,this);
...
}else{do{var I=this.__nb[0];
if(I){var H=I.getMarginLeft()+I.getMarginRight();
var D=I.getSizeHint().width+H;
if(A>C+D+this.getSpacing()||(this.__nb.length==1&&A>C+D-F+this.getSpacing())){this.__nd(I);
A+=D;
if(B&&this.__nb.length==0){B.setVisibility(v);
}}else{return;
}}}while(A>=C&&this.__nb.length>0);
}},__nd:function(J){J.setVisibility(w);
this.__nb.shift();
this.fireDataEvent(p,J);
},__ne:function(K){if(!K){return;
}this.__nb.unshift(K);
K.setVisibility(v);
this.fireDataEvent(j,K);
},_getNextToHide:function(){for(var i=this.__nc.length-1;i>=0;i--){var L=this.__nc[i];
...
}}},setRemovePriority:function(O,P,Q){if(!Q&&this.__nc[P]!=undefined){throw new Error("Priority already in use!");
}this.__nc[P]=O;
...
}for(var i=0;i<this.__nb.length;i++){this.__nb[i].setVisibility(w);
}this.__nb=[];
...
}},__nf:false,_setAllowMenuOpenHover:function(X){this.__nf=X;
},_isAllowMenuOpenHover:function(){return this.__nf;
...
this.add(K);
this.addSeparator();
this.__tq=new qx.ui.toolbar.Button(this.tr(g),m);
this.__tq.setCommand(H.getCommand(o));
this.__tq.setToolTipText(this.tr(w));
this.add(this.__tq);
this.addSpacer();
...
this.__tr=new qx.ui.menu.Menu();
M.setMenu(this.__tr);
this.addListener(u,this._onHideItem,this);
this.addListener(r,this._onShowItem,this);
},members:{__tp:null,__tr:null,__tn:null,__to:null,__tq:null,getRemoveButton:function(){return this.__tp;
...
P.setVisibility(f);
},_getMenuItem:function(R){var S=this.__tn[R.toHashCode()];
...
this.__tn[R.toHashCode()]=S;
}return S;
},signalLoading:function(T,U){if(T==A){var V=this.__to;
}else if(T==s){var V=this.__tq;
}else{return ;
...
},__nu:function(){if(this.isVisible()){if(this.getBlockBackground()){var R=this.getZIndex();
...
},__nv:function(){var bj=this._getMenuBounds();

if(!bj){this.addListenerOnce(j,this.__nv,this);
return;
...
if(this.__nt){this.__nt.call(this);
delete this.__nt;
...
if(this.__ns){bo.cancelOpen(this.__ns);
this.__ns=null;
...
}if(this.__ns){br.cancelOpen(this.__ns);
...
(function(){var n=",",m="",k="string",j="null",h='"',g=':',f="qx.jsonDebugging",e='-',d='\\u00',c="new Date(Date.UTC(",N="__pJ",M='\\\\',L='\\f',K='\\"',J='Z',I="))",H='T',G="}",F='(',E='.',u="{",v='\\r',s="__pL",t=":",q='\\t',r="__pK",o="]",p="[",w="__pU",x="qx.jsonEncodeUndefined",z='\\b',y="qx.util.Json",B=')',A='\\n',D="__pT",C="__pM";
qx.Class.define(y,{statics:{__pH:null,BEAUTIFYING_INDENT:"  ",BEAUTIFYING_LINE_END:"\n",CONVERT_DATES:null,__pI:{"function":N,"boolean":r,"number":s,"string":C,"object":D,"undefined":w},__pJ:function(O,P){return String(O);
},__pK:function(Q,R){return String(Q);
},__pL:function(S,T){return isFinite(S)?String(S):j;
},__pM:function(U,V){var W;

if(/["\\\x00-\x1f]/.test(U)){W=U.replace(/([\x00-\x1f\\"])/g,qx.util.Json.__pO);
}else{W=U;
}return h+W+h;
},__pN:{'\b':z,'\t':q,'\n':A,'\f':L,'\r':v,'"':K,'\\':M},__pO:function(a,b){var X=qx.util.Json.__pN[b];

if(X){return X;
}X=b.charCodeAt();
return d+Math.floor(X/16).toString(16)+(X%16).toString(16);
},__pP:function(Y,ba){var bc=[],bf=true,be,bb;
var bd=qx.util.Json.__pW;
bc.push(p);

if(bd){qx.util.Json.__pQ+=qx.util.Json.BEAUTIFYING_INDENT;
bc.push(qx.util.Json.__pQ);
}
for(var i=0,l=Y.length;i<l;i++){bb=Y[i];
be=this.__pI[typeof bb];

if(be){bb=this[be](bb,i+m);

if(typeof bb==k){if(!bf){bc.push(n);

if(bd){bc.push(qx.util.Json.__pQ);
}}bc.push(bb);
bf=false;
}}}
if(bd){qx.util.Json.__pQ=qx.util.Json.__pQ.substring(0,qx.util.Json.__pQ.length-qx.util.Json.BEAUTIFYING_INDENT.length);
bc.push(qx.util.Json.__pQ);
}bc.push(o);
return bc.join(m);
},__pR:function(bg,bh){if(!qx.util.Json.CONVERT_DATES){if(bg.toJSON&&!qx.bom.client.Engine.OPERA){return bg.toJSON();
}var bi=qx.util.format.NumberFormat.getInstance();
bi.setMinimumIntegerDigits(2);
var bk=bg.getUTCFullYear()+e+bi.format(bg.getUTCMonth()+1)+e+bi.format(bg.getUTCDate())+H+bi.format(bg.getUTCHours())+g+bi.format(bg.getUTCMinutes())+g+bi.format(bg.getUTCSeconds())+E;
...
return c+bj+I;
}},__pS:function(bl,bm){var bp=[],br=true,bo,bn;
var bq=qx.util.Json.__pW;
bp.push(u);

if(bq){qx.util.Json.__pQ+=qx.util.Json.BEAUTIFYING_INDENT;
bp.push(qx.util.Json.__pQ);
}
for(var bm in bl){bn=bl[bm];
bo=this.__pI[typeof bn];

if(bo){bn=this[bo](bn,bm);

if(typeof bn==k){if(!br){bp.push(n);

if(bq){bp.push(qx.util.Json.__pQ);
}}bp.push(this.__pM(bm),t,bn);
br=false;
}}}
if(bq){qx.util.Json.__pQ=qx.util.Json.__pQ.substring(0,qx.util.Json.__pQ.length-qx.util.Json.BEAUTIFYING_INDENT.length);
bp.push(qx.util.Json.__pQ);
}bp.push(G);
return bp.join(m);
},__pT:function(bs,bt){if(bs){if(qx.lang.Type.isFunction(bs.toJSON)&&bs.toJSON!==this.__pH){return this.__pV(bs.toJSON(bt),bt);
}else if(qx.lang.Type.isDate(bs)){return this.__pR(bs,bt);
}else if(qx.lang.Type.isArray(bs)){return this.__pP(bs,bt);
}else if(qx.lang.Type.isObject(bs)){return this.__pS(bs,bt);
}return m;
}return j;
},__pU:function(bu,bv){if(qx.core.Setting.get(x)){return j;
}},__pV:function(bw,bx){return this[this.__pI[typeof bw]](bw,bx);
},stringify:function(by,bz){this.__pW=bz;
this.__pQ=this.BEAUTIFYING_LINE_END;
var bA=this.__pV(by,m);
...
try{var bD=(bB&&bB.length>0)?eval(F+bB+B):null;
return bD;
}catch(bE){throw new Error("Could not evaluate JSON string: "+bE.message);
}}},settings:{"qx.jsonEncodeUndefined":true,"qx.jsonDebugging":false},defer:function(bF){bF.__pH=Date.prototype.toJSON;
}});
})();
...
}},properties:{innerColor:{check:h,nullable:true,apply:j},innerOpacity:{check:o,init:1,apply:j},outerColor:{check:h,nullable:true,apply:j},backgroundColor:{check:h,nullable:true,apply:j}},members:{__qt:null,_getDefaultInsets:function(){return {top:2,right:2,bottom:2,left:2};
...
}},properties:{width:{check:b,init:0,apply:g},style:{nullable:true,check:[k,j,h,i],init:k,apply:o},color:{nullable:true,check:m,apply:o},backgroundColor:{check:m,nullable:true,apply:o}},members:{__qu:null,_getDefaultInsets:function(){var s=this.getWidth();
return {top:s,right:s,bottom:s,left:s};
},_isInitialized:function(){return !!this.__qu;
},getMarkup:function(){if(this.__qu){return this.__qu;
...
(function(){var dM="button-frame",dL="atom",dK="widget",dJ="main",dI="button",dH="text-selected",dG="image",dF="bold",dE="middle",dD="selected",co="background-light",cn="cell",cm="text-disabled",cl="groupbox",ck="decoration/arrows/down.png",cj="border-invalid",ci="input",ch="input-disabled",cg="menu-button",cf="input-focused-invalid",dT="toolbar-button",dU="spinner",dR="input-focused",dS="white",dP="popup",dQ="tooltip",dN="label",dO="list",dV="tree-item",dW="treevirtual-contract",dk="scrollbar",dj="datechooser/nav-button",dm="text-hovered",dl="center",dp="treevirtual-expand",dn="textfield",dr="decoration/arrows/right.png",dq="background-application",di="radiobutton",dh="invalid",k="combobox",l="right-top",m="checkbox",n="text-title",o="icon/16/places/folder-open.png",p="qx/static/blank.gif",q="scrollbar/button",r="right",s="combobox/button",t="icon/16/places/folder.png",el="text-label",ek="decoration/tree/closed.png",ej="scrollbar-slider-horizontal",ei="checkbox-checked",ep="decoration/arrows/left.png",eo="button-focused",en="text-light",em="menu-slidebar-button",er="checkbox-undetermined",eq="text-input",bm="slidebar/button-forward",bn="background-splitpane",bk=".png",bl="decoration/tree/open.png",bq="default",br="decoration/arrows/down-small.png",bo="datechooser",bp="slidebar/button-backward",bi="selectbox",bj="treevirtual-folder",N="shadow-popup",M="icon/16/mimetypes/office-document.png",P="background-medium",O="icon/32/places/folder-open.png",J="icon/22/places/folder-open.png",I="table",L="decoration/arrows/up.png",K="decoration/form/",H="",G="-invalid",bx="button-checked",by="decoration/window/maximize-active-hovered.png",bz="radiobutton-hovered",bA="keyboard-focus",bt="decoration/cursors/",bu="slidebar",bv="tooltip-error-arrow",bw="table-scroller-focus-indicator",bB="move-frame",bC="nodrop",bb="decoration/table/boolean-true.png",ba="table-header-cell",Y="menu",X="app-header",W="row-layer",V="text-inactive",U="move",T="radiobutton-checked-focused",bf="decoration/window/restore-active-hovered.png",be="shadow-window",bD="table-column-button",bE="right.png",bF="checkbox-undetermined-hovered",bG="tabview-page-button-bottom-inactive",bH="tooltip-error",bI="window-statusbar",bJ="button-hovered",bK="decoration/scrollbar/scrollbar-",bL="background-tip",bM="scrollbar-slider-horizontal-disabled",cw="table-scroller-header",cv="button-pressed",cu="table-pane",ct="decoration/window/close-active.png",cA="native",cz="checkbox-hovered",cy="button-invalid-shadow",cx="decoration/window/minimize-active-hovered.png",cE="menubar",cD="icon/16/actions/dialog-cancel.png",dc="tabview-page-button-top-inactive",dd="tabview-page-button-left-inactive",da="menu-slidebar",db="toolbar-button-checked",cX="decoration/tree/open-selected.png",cY="radiobutton-checked",cV="decoration/window/minimize-inactive.png",cW="icon/16/apps/office-calendar.png",de="group",df="tabview-page-button-right-inactive",dv="decoration/window/minimize-active.png",du="decoration/window/restore-inactive.png",dx="checkbox-checked-focused",dw="splitpane",dz="combobox/textfield",dy="button-preselected-focused",dB="decoration/window/close-active-hovered.png",dA="qx/icon/Tango/16/actions/window-close.png",dt="checkbox-pressed",ds="button-disabled",ee="selected-dragover",ef="border-separator",eg="decoration/window/maximize-inactive.png",eh="dragover",ea="scrollarea",eb="scrollbar-vertical",ec="decoration/menu/checkbox-invert.gif",ed="decoration/toolbar/toolbar-handle-knob.gif",dX="icon/22/mimetypes/office-document.png",dY="button-preselected",j="button-checked-focused",i="up.png",h="best-fit",g="decoration/tree/closed-selected.png",f="qx.theme.modern.Appearance",e="text-active",d="toolbar-button-hovered",c="progressive-table-header",b="decoration/table/select-column-order.png",a="decoration/menu/radiobutton.gif",w="decoration/arrows/forward.png",x="decoration/table/descending.png",u="progressbar",v="window-captionbar-active",A="checkbox-checked-hovered",B="scrollbar-slider-vertical",y="toolbar",z="alias",D="decoration/window/restore-active.png",E="decoration/table/boolean-false.png",cI="icon/32/mimetypes/office-document.png",cC="radiobutton-checked-disabled",cP="tabview-pane",cL="decoration/arrows/rewind.png",cr="checkbox-focused",cp="top",R="icon/16/actions/dialog-ok.png",cs="radiobutton-checked-hovered",bd="table-header-cell-hovered",bc="window",bW="text-gray",bX="decoration/menu/radiobutton-invert.gif",bY="text-placeholder",ca="slider",cb="keep-align",cc="down.png",cd="tabview-page-button-top-active",ce="icon/22/places/folder.png",bT="decoration/window/maximize-active.png",bU="checkbox-checked-pressed",cq="decoration/window/close-inactive.png",cO="tabview-page-button-left-active",cN="toolbar-part",cM="decoration/splitpane/knob-vertical.png",cT=".gif",cS="radiobutton-checked-pressed",cR="table-statusbar",cQ="radiobutton-pressed",cK="window-captionbar-inactive",cJ="copy",C="radiobutton-focused",bh="decoration/arrows/down-invert.png",bg="decoration/menu/checkbox.gif",cB="decoration/splitpane/knob-horizontal.png",bs="icon/32/places/folder.png",cH="toolbar-separator",cG="tabview-page-button-bottom-active",cF="decoration/arrows/up-small.png",Q="decoration/table/ascending.png",cU="decoration/arrows/up-invert.png",F="small",S="tabview-page-button-right-active",bN="-disabled",bO="scrollbar-horizontal",bP="checkbox-undetermined-focused",bQ="progressive-table-header-cell",bR="menu-separator",bS="pane",dg="decoration/arrows/right-invert.png",bV="left.png",dC="icon/16/actions/view-refresh.png";
qx.Theme.define(f,{appearances:{"widget":{},"root":{style:function(es){return {backgroundColor:dq,textColor:el,font:bq};
}},"label":{style:function(et){return {textColor:et.disabled?cm:undefined};
}},"move-frame":{style:function(eu){return {decorator:dJ};
}},"resize-frame":bB,"dragdrop-cursor":{style:function(ev){var ew=bC;

if(ev.copy){ew=cJ;
}else if(ev.move){ew=U;
}else if(ev.alias){ew=z;
}return {source:bt+ew+cT,position:l,offset:[2,16,2,6]};
}},"image":{style:function(ex){return {opacity:!ex.replacement&&ex.disabled?0.3:1};
}},"atom":{},"atom/label":dN,"atom/icon":dG,"popup":{style:function(ey){return {decorator:dJ,backgroundColor:co,shadow:N};
}},"button-frame":{alias:dL,style:function(ez){var eB,eA;

if(ez.checked&&ez.focused&&!ez.inner){eB=j;
eA=undefined;
}else if(ez.disabled){eB=ds;
eA=undefined;
}else if(ez.pressed){eB=cv;
eA=dm;
}else if(ez.checked){eB=bx;
eA=undefined;
}else if(ez.hovered){eB=bJ;
eA=dm;
}else if(ez.preselected&&ez.focused&&!ez.inner){eB=dy;
eA=dm;
}else if(ez.preselected){eB=dY;
eA=dm;
}else if(ez.focused&&!ez.inner){eB=eo;
eA=undefined;
}else{eB=dI;
eA=undefined;
}return {decorator:eB,textColor:eA,shadow:ez.invalid&&!ez.disabled?cy:undefined};
}},"button-frame/image":{style:function(eC){return {opacity:!eC.replacement&&eC.disabled?0.5:1};
}},"button":{alias:dM,include:dM,style:function(eD){return {padding:[2,8],center:true};
}},"hover-button":{alias:dL,include:dL,style:function(eE){return {decorator:eE.hovered?dD:undefined,textColor:eE.hovered?dH:undefined};
}},"splitbutton":{},"splitbutton/button":dI,"splitbutton/arrow":{alias:dI,include:dI,style:function(eF){return {icon:ck,padding:2,marginLeft:1};
}},"checkbox":{alias:dL,style:function(eG){var eI;
if(eG.checked){if(eG.disabled){eI=ei;
}else if(eG.focused){eI=dx;
}else if(eG.pressed){eI=bU;
}else if(eG.hovered){eI=A;
}else{eI=ei;
}}else if(eG.undetermined){if(eG.disabled){eI=er;
}else if(eG.focused){eI=bP;
}else if(eG.hovered){eI=bF;
}else{eI=er;
}}else if(!eG.disabled){if(eG.focused){eI=cr;
}else if(eG.pressed){eI=dt;
}else if(eG.hovered){eI=cz;
}}eI=eI||m;
var eH=eG.invalid&&!eG.disabled?G:H;
return {icon:K+eI+eH+bk,gap:6};
}},"radiobutton":{alias:dL,style:function(eJ){var eL;

if(eJ.checked&&eJ.focused){eL=T;
}else if(eJ.checked&&eJ.disabled){eL=cC;
}else if(eJ.checked&&eJ.pressed){eL=cS;
}else if(eJ.checked&&eJ.hovered){eL=cs;
}else if(eJ.checked){eL=cY;
}else if(eJ.focused){eL=C;
}else if(eJ.pressed){eL=cQ;
}else if(eJ.hovered){eL=bz;
}else{eL=di;
}var eK=eJ.invalid&&!eJ.disabled?G:H;
return {icon:K+eL+eK+bk,gap:6};
}},"textfield":{style:function(eM){var eR;
var eP=!!eM.focused;
var eQ=!!eM.invalid;
var eN=!!eM.disabled;

if(eP&&eQ&&!eN){eR=cf;
}else if(eP&&!eQ&&!eN){eR=dR;
}else if(eN){eR=ch;
}else if(!eP&&eQ&&!eN){eR=cj;
}else{eR=ci;
}var eO;

if(eM.disabled){eO=cm;
}else if(eM.showingPlaceholder){eO=bY;
}else{eO=eq;
}return {decorator:eR,padding:[2,4,1],textColor:eO};
}},"textarea":{include:dn,style:function(eS){return {padding:4};
}},"spinner":{style:function(eT){var eX;
var eV=!!eT.focused;
var eW=!!eT.invalid;
var eU=!!eT.disabled;

if(eV&&eW&&!eU){eX=cf;
}else if(eV&&!eW&&!eU){eX=dR;
}else if(eU){eX=ch;
}else if(!eV&&eW&&!eU){eX=cj;
}else{eX=ci;
}return {decorator:eX};
}},"spinner/textfield":{style:function(eY){return {marginRight:2,padding:[2,4,1],textColor:eY.disabled?cm:eq};
}},"spinner/upbutton":{alias:dM,include:dM,style:function(fa){return {icon:cF,padding:fa.pressed?[2,2,0,4]:[1,3,1,3],shadow:undefined};
}},"spinner/downbutton":{alias:dM,include:dM,style:function(fb){return {icon:br,padding:fb.pressed?[2,2,0,4]:[1,3,1,3],shadow:undefined};
}},"datefield":k,"datefield/button":{alias:s,include:s,style:function(fc){return {icon:cW,padding:[0,3],decorator:undefined};
}},"datefield/textfield":dz,"datefield/list":{alias:bo,include:bo,style:function(fd){return {decorator:undefined};
}},"groupbox":{style:function(fe){return {legendPosition:cp};
}},"groupbox/legend":{alias:dL,style:function(ff){return {padding:[1,0,1,4],textColor:ff.invalid?dh:n,font:dF};
}},"groupbox/frame":{style:function(fg){return {padding:12,decorator:de};
}},"check-groupbox":cl,"check-groupbox/legend":{alias:m,include:m,style:function(fh){return {padding:[1,0,1,4],textColor:fh.invalid?dh:n,font:dF};
}},"radio-groupbox":cl,"radio-groupbox/legend":{alias:di,include:di,style:function(fi){return {padding:[1,0,1,4],textColor:fi.invalid?dh:n,font:dF};
}},"scrollarea":{style:function(fj){return {minWidth:50,minHeight:50};
}},"scrollarea/corner":{style:function(fk){return {backgroundColor:dq};
}},"scrollarea/pane":dK,"scrollarea/scrollbar-x":dk,"scrollarea/scrollbar-y":dk,"scrollbar":{style:function(fl){if(fl[cA]){return {};
}return {width:fl.horizontal?undefined:16,height:fl.horizontal?16:undefined,decorator:fl.horizontal?bO:eb,padding:1};
}},"scrollbar/slider":{alias:ca,style:function(fm){return {padding:fm.horizontal?[0,1,0,1]:[1,0,1,0]};
}},"scrollbar/slider/knob":{include:dM,style:function(fn){var fo=fn.horizontal?ej:B;

if(fn.disabled){fo+=bN;
}return {decorator:fo,minHeight:fn.horizontal?undefined:9,minWidth:fn.horizontal?9:undefined};
}},"scrollbar/button":{alias:dM,include:dM,style:function(fp){var fq=bK;

if(fp.left){fq+=bV;
}else if(fp.right){fq+=bE;
}else if(fp.up){fq+=i;
}else{fq+=cc;
}
if(fp.left||fp.right){return {padding:[0,0,0,fp.left?3:4],icon:fq,width:15,height:14};
}else{return {padding:[0,0,0,2],icon:fq,width:14,height:15};
}}},"scrollbar/button-begin":q,"scrollbar/button-end":q,"slider":{style:function(fr){var fv;
var ft=!!fr.focused;
var fu=!!fr.invalid;
var fs=!!fr.disabled;

if(ft&&fu&&!fs){fv=cf;
}else if(ft&&!fu&&!fs){fv=dR;
}else if(fs){fv=ch;
}else if(!ft&&fu&&!fs){fv=cj;
}else{fv=ci;
}return {decorator:fv};
}},"slider/knob":{include:dM,style:function(fw){return {decorator:fw.disabled?bM:ej,shadow:undefined,height:14,width:14};
}},"list":{alias:ea,style:function(fx){var fB;
var fz=!!fx.focused;
var fA=!!fx.invalid;
var fy=!!fx.disabled;

if(fz&&fA&&!fy){fB=cf;
}else if(fz&&!fA&&!fy){fB=dR;
}else if(fy){fB=ch;
}else if(!fz&&fA&&!fy){fB=cj;
}else{fB=ci;
}return {backgroundColor:co,decorator:fB};
}},"list/pane":dK,"listitem":{alias:dL,style:function(fC){var fD;

if(fC.dragover){fD=fC.selected?ee:eh;
}else{fD=fC.selected?dD:undefined;
}return {padding:fC.dragover?[4,4,2,4]:4,textColor:fC.selected?dH:undefined,decorator:fD};
}},"slidebar":{},"slidebar/scrollpane":{},"slidebar/content":{},"slidebar/button-forward":{alias:dM,include:dM,style:function(fE){return {padding:5,center:true,icon:fE.vertical?ck:dr};
}},"slidebar/button-backward":{alias:dM,include:dM,style:function(fF){return {padding:5,center:true,icon:fF.vertical?L:ep};
}},"tabview":{style:function(fG){return {contentPadding:16};
}},"tabview/bar":{alias:bu,style:function(fH){var fI={marginBottom:fH.barTop?-1:0,marginTop:fH.barBottom?-4:0,marginLeft:fH.barRight?-3:0,marginRight:fH.barLeft?-1:0,paddingTop:0,paddingRight:0,paddingBottom:0,paddingLeft:0};

if(fH.barTop||fH.barBottom){fI.paddingLeft=5;
fI.paddingRight=7;
}else{fI.paddingTop=5;
fI.paddingBottom=7;
}return fI;
}},"tabview/bar/button-forward":{include:bm,alias:bm,style:function(fJ){if(fJ.barTop||fJ.barBottom){return {marginTop:2,marginBottom:2};
}else{return {marginLeft:2,marginRight:2};
}}},"tabview/bar/button-backward":{include:bp,alias:bp,style:function(fK){if(fK.barTop||fK.barBottom){return {marginTop:2,marginBottom:2};
}else{return {marginLeft:2,marginRight:2};
}}},"tabview/bar/scrollpane":{},"tabview/pane":{style:function(fL){return {decorator:cP,minHeight:100,marginBottom:fL.barBottom?-1:0,marginTop:fL.barTop?-1:0,marginLeft:fL.barLeft?-1:0,marginRight:fL.barRight?-1:0};
}},"tabview-page":dK,"tabview-page/button":{alias:dL,style:function(fM){var fS,fO=0;
var fR=0,fN=0,fP=0,fQ=0;

if(fM.checked){if(fM.barTop){fS=cd;
fO=[6,14];
fP=fM.firstTab?0:-5;
fQ=fM.lastTab?0:-5;
}else if(fM.barBottom){fS=cG;
fO=[6,14];
fP=fM.firstTab?0:-5;
fQ=fM.lastTab?0:-5;
}else if(fM.barRight){fS=S;
fO=[6,13];
fR=fM.firstTab?0:-5;
fN=fM.lastTab?0:-5;
}else{fS=cO;
...
fR=fM.firstTab?0:-5;
fN=fM.lastTab?0:-5;
}}else{if(fM.barTop){fS=dc;
fO=[4,10];
fR=4;
fP=fM.firstTab?5:1;
fQ=1;
}else if(fM.barBottom){fS=bG;
fO=[4,10];
fN=4;
fP=fM.firstTab?5:1;
fQ=1;
}else if(fM.barRight){fS=df;
fO=[4,10];
fQ=5;
fR=fM.firstTab?5:1;
fN=1;
...
fN=1;
fQ=1;
}}return {zIndex:fM.checked?10:5,decorator:fS,padding:fO,marginTop:fR,marginBottom:fN,marginLeft:fP,marginRight:fQ,textColor:fM.checked?e:V};
}},"tabview-page/button/label":{alias:dN,style:function(fT){return {padding:[0,1,0,1],margin:fT.focused?0:1,decorator:fT.focused?bA:undefined};
}},"tabview-page/button/close-button":{alias:dL,style:function(fU){return {icon:dA};
}},"toolbar":{style:function(fV){return {decorator:y,spacing:2};
}},"toolbar/part":{style:function(fW){return {decorator:cN,spacing:2};
}},"toolbar/part/container":{style:function(fX){return {paddingLeft:2,paddingRight:2};
}},"toolbar/part/handle":{style:function(fY){return {source:ed,marginLeft:3,marginRight:3};
}},"toolbar-button":{alias:dL,style:function(ga){return {marginTop:2,marginBottom:2,padding:(ga.pressed||ga.checked||ga.hovered)&&!ga.disabled||(ga.disabled&&ga.checked)?3:5,decorator:ga.pressed||(ga.checked&&!ga.hovered)||(ga.checked&&ga.disabled)?db:ga.hovered&&!ga.disabled?d:undefined};
}},"toolbar-menubutton":{alias:dT,include:dT,style:function(gb){return {showArrow:true};
}},"toolbar-menubutton/arrow":{alias:dG,include:dG,style:function(gc){return {source:br};
}},"toolbar-splitbutton":{style:function(gd){return {marginTop:2,marginBottom:2};
}},"toolbar-splitbutton/button":{alias:dT,include:dT,style:function(ge){return {icon:ck,marginTop:undefined,marginBottom:undefined};
}},"toolbar-splitbutton/arrow":{alias:dT,include:dT,style:function(gf){if(gf.pressed||gf.checked||(gf.hovered&&!gf.disabled)){var gg=1;
}else{var gg=3;
}return {padding:gg,icon:ck,marginTop:undefined,marginBottom:undefined};
}},"toolbar-separator":{style:function(gh){return {decorator:cH,margin:7};
}},"tree":dO,"tree-item":{style:function(gi){return {padding:[2,6],textColor:gi.selected?dH:undefined,decorator:gi.selected?dD:undefined};
}},"tree-item/icon":{include:dG,style:function(gj){return {paddingRight:5};
}},"tree-item/label":dN,"tree-item/open":{include:dG,style:function(gk){var gl;

if(gk.selected&&gk.opened){gl=cX;
}else if(gk.selected&&!gk.opened){gl=g;
}else if(gk.opened){gl=bl;
}else{gl=ek;
}return {padding:[0,5,0,2],source:gl};
}},"tree-folder":{include:dV,alias:dV,style:function(gm){var go,gn;

if(gm.small){go=gm.opened?o:t;
gn=o;
}else if(gm.large){go=gm.opened?O:bs;
gn=O;
}else{go=gm.opened?J:ce;
gn=J;
}return {icon:go,iconOpened:gn};
}},"tree-file":{include:dV,alias:dV,style:function(gp){return {icon:gp.small?M:gp.large?cI:dX};
}},"treevirtual":I,"treevirtual-folder":{style:function(gq){return {icon:gq.opened?o:t};
}},"treevirtual-file":{include:bj,alias:bj,style:function(gr){return {icon:M};
}},"treevirtual-line":{style:function(gs){return {icon:p};
}},"treevirtual-contract":{style:function(gt){return {icon:bl,paddingLeft:5,paddingTop:2};
}},"treevirtual-expand":{style:function(gu){return {icon:ek,paddingLeft:5,paddingTop:2};
}},"treevirtual-only-contract":dW,"treevirtual-only-expand":dp,"treevirtual-start-contract":dW,"treevirtual-start-expand":dp,"treevirtual-end-contract":dW,"treevirtual-end-expand":dp,"treevirtual-cross-contract":dW,"treevirtual-cross-expand":dp,"treevirtual-end":{style:function(gv){return {icon:p};
}},"treevirtual-cross":{style:function(gw){return {icon:p};
}},"tooltip":{include:dP,style:function(gx){return {backgroundColor:bL,padding:[1,3,2,3],offset:[15,5,5,5]};
}},"tooltip/atom":dL,"tooltip-error":{include:dQ,style:function(gy){return {textColor:dH,placeMethod:dK,offset:[0,0,0,14],marginTop:-2,position:l,showTimeout:100,hideTimeout:10000,decorator:bH,shadow:bv,font:dF};
}},"tooltip-error/atom":dL,"window":{style:function(gz){return {shadow:be,contentPadding:[10,10,10,10]};
}},"window/pane":{style:function(gA){return {decorator:bc};
}},"window/captionbar":{style:function(gB){return {decorator:gB.active?v:cK,textColor:gB.active?dS:bW,minHeight:26,paddingRight:2};
}},"window/icon":{style:function(gC){return {margin:[5,0,3,6]};
}},"window/title":{style:function(gD){return {alignY:dE,font:dF,marginLeft:6,marginRight:12};
}},"window/minimize-button":{alias:dL,style:function(gE){return {icon:gE.active?gE.hovered?cx:dv:cV,margin:[4,8,2,0]};
}},"window/restore-button":{alias:dL,style:function(gF){return {icon:gF.active?gF.hovered?bf:D:du,margin:[5,8,2,0]};
}},"window/maximize-button":{alias:dL,style:function(gG){return {icon:gG.active?gG.hovered?by:bT:eg,margin:[4,8,2,0]};
}},"window/close-button":{alias:dL,style:function(gH){return {icon:gH.active?gH.hovered?dB:ct:cq,margin:[4,8,2,0]};
}},"window/statusbar":{style:function(gI){return {padding:[2,6],decorator:bI,minHeight:18};
}},"window/statusbar-text":{style:function(gJ){return {font:F};
}},"iframe":{style:function(gK){return {decorator:dJ};
}},"resizer":{style:function(gL){return {decorator:bS};
}},"splitpane":{style:function(gM){return {decorator:dw};
}},"splitpane/splitter":{style:function(gN){return {width:gN.horizontal?3:undefined,height:gN.vertical?3:undefined,backgroundColor:bn};
}},"splitpane/splitter/knob":{style:function(gO){return {source:gO.horizontal?cB:cM};
}},"splitpane/slider":{style:function(gP){return {width:gP.horizontal?3:undefined,height:gP.vertical?3:undefined,backgroundColor:bn};
}},"selectbox":{alias:dM,include:dM,style:function(gQ){return {padding:[2,8]};
}},"selectbox/atom":dL,"selectbox/popup":dP,"selectbox/list":{alias:dO},"selectbox/arrow":{include:dG,style:function(gR){return {source:ck,paddingLeft:5};
...
if(gU&&gV&&!gT){gW=cf;
}else if(gU&&!gV&&!gT){gW=dR;
}else if(gT){gW=ch;
}else if(!gU&&gV&&!gT){gW=cj;
}else{gW=ci;
}return {padding:2,decorator:gW,backgroundColor:co};
}},"datechooser/navigation-bar":{},"datechooser/nav-button":{include:dM,alias:dM,style:function(gX){var gY={padding:[2,4],shadow:undefined};

if(gX.lastYear){gY.icon=cL;
gY.marginRight=1;
}else if(gX.lastMonth){gY.icon=ep;
}else if(gX.nextYear){gY.icon=w;
gY.marginLeft=1;
}else if(gX.nextMonth){gY.icon=dr;
}return gY;
}},"datechooser/last-year-button-tooltip":dQ,"datechooser/last-month-button-tooltip":dQ,"datechooser/next-year-button-tooltip":dQ,"datechooser/next-month-button-tooltip":dQ,"datechooser/last-year-button":dj,"datechooser/last-month-button":dj,"datechooser/next-month-button":dj,"datechooser/next-year-button":dj,"datechooser/month-year-label":{style:function(ha){return {font:dF,textAlign:dl,textColor:ha.disabled?cm:undefined};
}},"datechooser/date-pane":{style:function(hb){return {textColor:hb.disabled?cm:undefined,marginTop:2};
}},"datechooser/weekday":{style:function(hc){return {textColor:hc.disabled?cm:hc.weekend?en:undefined,textAlign:dl,paddingTop:2,backgroundColor:P};
}},"datechooser/week":{style:function(hd){return {textAlign:dl,padding:[2,4],backgroundColor:P};
}},"datechooser/day":{style:function(he){return {textAlign:dl,decorator:he.disabled?undefined:he.selected?dD:undefined,textColor:he.disabled?cm:he.selected?dH:he.otherMonth?en:undefined,font:he.today?dF:undefined,padding:[2,4]};
}},"combobox":{style:function(hf){var hj;
var hh=!!hf.focused;
var hi=!!hf.invalid;
var hg=!!hf.disabled;

if(hh&&hi&&!hg){hj=cf;
}else if(hh&&!hi&&!hg){hj=dR;
}else if(hg){hj=ch;
}else if(!hh&&hi&&!hg){hj=cj;
}else{hj=ci;
}return {decorator:hj};
}},"combobox/popup":dP,"combobox/list":{alias:dO},"combobox/button":{include:dM,alias:dM,style:function(hk){var hl={icon:ck,padding:2};

if(hk.selected){hl.decorator=eo;
}return hl;
}},"combobox/textfield":{include:dn,style:function(hm){return {decorator:undefined};
}},"menu":{style:function(hn){var ho={decorator:Y,shadow:N,spacingX:6,spacingY:1,iconColumnWidth:16,arrowColumnWidth:4,placementModeY:hn.submenu||hn.contextmenu?h:cb};

if(hn.submenu){ho.position=l;
ho.offset=[-2,-3];
}return ho;
}},"menu/slidebar":da,"menu-slidebar":dK,"menu-slidebar-button":{style:function(hp){return {decorator:hp.hovered?dD:undefined,padding:7,center:true};
}},"menu-slidebar/button-backward":{include:em,style:function(hq){return {icon:hq.hovered?cU:L};
}},"menu-slidebar/button-forward":{include:em,style:function(hr){return {icon:hr.hovered?bh:ck};
}},"menu-separator":{style:function(hs){return {height:0,decorator:bR,margin:[4,2]};
}},"menu-button":{alias:dL,style:function(ht){return {decorator:ht.selected?dD:undefined,textColor:ht.selected?dH:undefined,padding:[4,6]};
}},"menu-button/icon":{include:dG,style:function(hu){return {alignY:dE};
}},"menu-button/label":{include:dN,style:function(hv){return {alignY:dE,padding:1};
}},"menu-button/shortcut":{include:dN,style:function(hw){return {alignY:dE,marginLeft:14,padding:1};
}},"menu-button/arrow":{include:dG,style:function(hx){return {source:hx.selected?dg:dr,alignY:dE};
}},"menu-checkbox":{alias:cg,include:cg,style:function(hy){return {icon:!hy.checked?undefined:hy.selected?ec:bg};
}},"menu-radiobutton":{alias:cg,include:cg,style:function(hz){return {icon:!hz.checked?undefined:hz.selected?bX:a};
}},"menubar":{style:function(hA){return {decorator:cE};
}},"menubar-button":{alias:dL,style:function(hB){return {decorator:(hB.pressed||hB.hovered)&&!hB.disabled?dD:undefined,textColor:hB.pressed||hB.hovered?dH:undefined,padding:[3,8]};
}},"colorselector":dK,"colorselector/control-bar":dK,"colorselector/control-pane":dK,"colorselector/visual-pane":cl,"colorselector/preset-grid":dK,"colorselector/colorbucket":{style:function(hC){return {decorator:dJ,width:16,height:16};
}},"colorselector/preset-field-set":cl,"colorselector/input-field-set":cl,"colorselector/preview-field-set":cl,"colorselector/hex-field-composite":dK,"colorselector/hex-field":dn,"colorselector/rgb-spinner-composite":dK,"colorselector/rgb-spinner-red":dU,"colorselector/rgb-spinner-green":dU,"colorselector/rgb-spinner-blue":dU,"colorselector/hsb-spinner-composite":dK,"colorselector/hsb-spinner-hue":dU,"colorselector/hsb-spinner-saturation":dU,"colorselector/hsb-spinner-brightness":dU,"colorselector/preview-content-old":{style:function(hD){return {decorator:dJ,width:50,height:10};
}},"colorselector/preview-content-new":{style:function(hE){return {decorator:dJ,backgroundColor:co,width:50,height:10};
}},"colorselector/hue-saturation-field":{style:function(hF){return {decorator:dJ,margin:5};
}},"colorselector/brightness-field":{style:function(hG){return {decorator:dJ,margin:[5,7]};
}},"colorselector/hue-saturation-pane":dK,"colorselector/hue-saturation-handle":dK,"colorselector/brightness-pane":dK,"colorselector/brightness-handle":dK,"colorpopup":{alias:dP,include:dP,style:function(hH){return {padding:5,backgroundColor:dq};
}},"colorpopup/field":{style:function(hI){return {decorator:dJ,margin:2,width:14,height:14,backgroundColor:co};
}},"colorpopup/selector-button":dI,"colorpopup/auto-button":dI,"colorpopup/preview-pane":cl,"colorpopup/current-preview":{style:function(hJ){return {height:20,padding:4,marginLeft:4,decorator:dJ,allowGrowX:true};
}},"colorpopup/selected-preview":{style:function(hK){return {height:20,padding:4,marginRight:4,decorator:dJ,allowGrowX:true};
}},"colorpopup/colorselector-okbutton":{alias:dI,include:dI,style:function(hL){return {icon:R};
}},"colorpopup/colorselector-cancelbutton":{alias:dI,include:dI,style:function(hM){return {icon:cD};
}},"table":{alias:dK,style:function(hN){return {decorator:I};
}},"table-header":{},"table/statusbar":{style:function(hO){return {decorator:cR,padding:[0,2]};
}},"table/column-button":{alias:dM,style:function(hP){return {decorator:bD,padding:3,icon:b};
}},"table-column-reset-button":{include:cg,alias:cg,style:function(){return {icon:dC};
}},"table-scroller":dK,"table-scroller/scrollbar-x":dk,"table-scroller/scrollbar-y":dk,"table-scroller/header":{style:function(hQ){return {decorator:cw};
}},"table-scroller/pane":{style:function(hR){return {backgroundColor:cu};
}},"table-scroller/focus-indicator":{style:function(hS){return {decorator:bw};
}},"table-scroller/resize-line":{style:function(hT){return {backgroundColor:ef,width:2};
}},"table-header-cell":{alias:dL,style:function(hU){return {minWidth:13,minHeight:20,padding:hU.hovered?[3,4,2,4]:[3,4],decorator:hU.hovered?bd:ba,sortIcon:hU.sorted?(hU.sortedAscending?Q:x):undefined};
}},"table-header-cell/label":{style:function(hV){return {minWidth:0,alignY:dE,paddingRight:5};
}},"table-header-cell/sort-icon":{style:function(hW){return {alignY:dE,alignX:r};
}},"table-header-cell/icon":{style:function(hX){return {minWidth:0,alignY:dE,paddingRight:5};
}},"table-editor-textfield":{include:dn,style:function(hY){return {decorator:undefined,padding:[2,2],backgroundColor:co};
}},"table-editor-selectbox":{include:bi,alias:bi,style:function(ia){return {padding:[0,2],backgroundColor:co};
}},"table-editor-combobox":{include:k,alias:k,style:function(ib){return {decorator:undefined,backgroundColor:co};
}},"progressive-table-header":{alias:dK,style:function(ic){return {decorator:c};
}},"progressive-table-header-cell":{alias:dL,style:function(id){return {minWidth:40,minHeight:25,paddingLeft:6,decorator:bQ};
}},"app-header":{style:function(ie){return {font:dF,textColor:dH,padding:[8,12],decorator:X};
}},"virtual-list":dO,"virtual-list/row-layer":W,"row-layer":{style:function(ig){return {colorEven:dS,colorOdd:dS};
}},"column-layer":dK,"cell":{style:function(ih){return {textColor:ih.selected?dH:el,padding:[3,6],font:bq};
}},"cell-string":cn,"cell-number":{include:cn,style:function(ii){return {textAlign:r};
}},"cell-image":cn,"cell-boolean":{include:cn,style:function(ij){return {iconTrue:bb,iconFalse:E};
}},"cell-atom":cn,"cell-date":cn,"cell-html":cn,"htmlarea":{"include":dK,style:function(ik){return {backgroundColor:dS};
}},"progressbar":{style:function(il){return {decorator:u,padding:[1],backgroundColor:dS};
}},"progressbar/progress":{style:function(im){return {decorator:dD};
}}}});
})();
...
(function(){var p="Boolean",o="focusout",n="interval",m="mouseover",l="mouseout",k="mousemove",j="widget",i="__dv",h="qx.ui.tooltip.ToolTip",g="_applyCurrent",c="qx.ui.tooltip.Manager",f="__dy",d="__dw",b="tooltip-error",a="singleton";
qx.Class.define(c,{type:a,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
qx.event.Registration.addListener(document.body,m,this.__dF,this,true);
this.__dv=new qx.event.Timer();
this.__dv.addListener(n,this.__dC,this);
this.__dw=new qx.event.Timer();
this.__dw.addListener(n,this.__dD,this);
this.__dx={left:0,top:0};
},properties:{current:{check:h,nullable:true,apply:g},showInvalidToolTips:{check:p,init:true},showToolTips:{check:p,init:true}},members:{__dx:null,__dw:null,__dv:null,__dy:null,__dz:null,__dA:function(){if(!this.__dy){this.__dy=new qx.ui.tooltip.ToolTip().set({rich:true});
}return this.__dy;
},__dB:function(){if(!this.__dz){this.__dz=new qx.ui.tooltip.ToolTip().set({appearance:b});
...
var s=document.body;
if(q){this.__dv.startWith(q.getShowTimeout());
t.addListener(s,l,this.__dG,this,true);
t.addListener(s,o,this.__dH,this,true);
t.addListener(s,k,this.__dE,this,true);
}else{t.removeListener(s,l,this.__dG,this,true);
t.removeListener(s,o,this.__dH,this,true);
t.removeListener(s,k,this.__dE,this,true);
}},__dC:function(e){var u=this.getCurrent();

if(u&&!u.isDisposed()){this.__dw.startWith(u.getHideTimeout());

if(u.getPlaceMethod()==j){u.placeToWidget(u.getOpener());
}else{u.placeToPoint(this.__dx);
}u.show();
}this.__dv.stop();
},__dD:function(e){var v=this.getCurrent();

if(v&&!v.isDisposed()){v.exclude();
}this.__dw.stop();
this.resetCurrent();
},__dE:function(e){var w=this.__dx;
w.left=e.getDocumentLeft();
w.top=e.getDocumentTop();
},__dF:function(e){var z=qx.ui.core.Widget.getWidgetByElement(e.getTarget());
...
if(x){A=this.__dB().set({label:x});
}
if(!A){A=this.__dA().set({label:B,icon:y});
}this.setCurrent(A);
A.setOpener(z);
},__dG:function(e){var C=qx.ui.core.Widget.getWidgetByElement(e.getTarget());
...
}},__dH:function(e){var F=qx.ui.core.Widget.getWidgetByElement(e.getTarget());

if(!F){return;
}var G=this.getCurrent();
if(G&&G==F.getToolTip()){this.setCurrent(null);
}}},destruct:function(){qx.event.Registration.removeListener(document.body,m,this.__dF,this,true);
this._disposeObjects(i,d,f);
this.__dx=null;
}});
})();
...
for(F=z.length;F>3;F-=3){D=t+qx.locale.Number.getGroupSeparator(this.__pX)+z.substring(F-3,F)+D;
}D=z.substring(0,F)+D;
}var B=this.getPrefix()?this.getPrefix():t;
var y=this.getPostfix()?this.getPostfix():t;
var E=B+(A?r:t)+D;

if(C.length>0){E+=t+qx.locale.Number.getDecimalSeparator(this.__pX)+C;
}E+=y;
return E;
},parse:function(I){var N=qx.lang.String.escapeRegexpChars(qx.locale.Number.getGroupSeparator(this.__pX)+t);
var L=qx.lang.String.escapeRegexpChars(qx.locale.Number.getDecimalSeparator(this.__pX)+t);
...
(function(){var n="separator-vertical",m="__tt",l="background-medium",k="Posts",j="bold",i="__ts",h="_applyLoading",g="center",f="feedreader.view.List",e="__tu",b="single",d="middle",c="feedreader/images/loading66.gif",a="Boolean";
...
q.setFont(j);
this._add(q);
this.__ts=new qx.ui.container.Stack();
this._add(this.__ts,{flex:1});
this.__tt=new qx.ui.form.List();
this.__tt.setDecorator(null);
this.__tt.setSelectionMode(b);
this.__ts.add(this.__tt);
this.__tu=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,g));
var o=new qx.ui.basic.Image(c);
o.setAlignY(d);
this.__tu.add(o);
this.__ts.add(this.__tu);
},properties:{loading:{check:a,init:false,apply:h}},members:{__ts:null,__tt:null,__tu:null,_applyLoading:function(r,s){if(r){this.__ts.setSelection([this.__tu]);
}else{this.__ts.setSelection([this.__tt]);
}},getList:function(){return this.__tt;
}},destruct:function(){this._disposeObjects(m,i,e);
}});
})();
...
(function(){var f="mousedown",d="__jM",c="blur",b="singleton",a="qx.ui.popup.Manager";
qx.Class.define(a,{type:b,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__jM=[];
qx.event.Registration.addListener(document.documentElement,f,this.__jO,this,true);
qx.bom.Element.addListener(window,c,this.hideAll,this);
},members:{__jM:null,add:function(g){{};
this.__jM.push(g);
this.__jN();
},remove:function(h){{};

if(this.__jM){qx.lang.Array.remove(this.__jM,h);
this.__jN();
}},hideAll:function(){if(this.__jM){for(var i=0;i<this.__jM.length;i++){this.__jM[i].exclude();
}}},__jN:function(){var j=1e7;

for(var i=0;i<this.__jM.length;i++){this.__jM[i].setZIndex(j++);
}},__jO:function(e){var l=qx.ui.core.Widget.getWidgetByElement(e.getTarget());
var m=this.__jM;
...
}}},destruct:function(){qx.event.Registration.removeListener(document.documentElement,f,this.__jO,this,true);
...
var T=Y.event;
var X=bb.addListener(T,this.__tz,this);
var U=W.treeNode;
this.__tx[ba.toHashCode()]={modelNode:bb,treeNode:U,changeListenerId:W.changeListenerId,changeChildernListenerId:X};
this.__tB(U,bb);
this._updateSelection();
...
},__tA:function(){if(this.getTarget()==null||this.getChildPath()==null){return;
...
this.__tG(this.getModel(),bf);
this.__tB(bf,this.getModel());
}},__tB:function(bg,bh){if(bh[s+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;
}var bq=bh[s+qx.lang.String.firstUp(this.getChildPath())]();
if(this.__tx[bq.toHashCode()]==undefined){var bl=bq.addListener(m,this.__ty,this);
var bp=qx.Class.getPropertyDefinition(bh.constructor,this.getChildPath());
var bm=bp.event;
var bn=bh.addListener(bm,this.__tz,this);
this.__tx[bq.toHashCode()]={modelNode:bh,treeNode:bg,changeListenerId:bl,changeChildernListenerId:bn};
...
}},__tF:function(){var bS;

for(var bU in this.__tv){bS=bU;
break;
}for(var bR in this.__tv[bS]){var bV=this.__tv[bS][bR].treeNode;
var bT=qx.core.ObjectRegistry.fromHashCode(bR);
this.__tH(bT);
this.__tG(bT,bV);
}},__tG:function(bW,bX){var bY=this.getDelegate();
if(bY!=null&&bY.bindItem!=null){bY.bindItem(this,bX,bW);
}else{this.bindDefaultProperties(bX,bW);
}},__tH:function(ca){for(var i=0;i<this.__tw.length;i++){var cc=this.__tw[i];
var cb=this.__tv[cc][ca.toHashCode()];

if(cb!=null){if(cb.id){ca.removeBinding(cb.id);
}
if(cb.reverseId){cb.treeNode.removeBinding(cb.reverseId);
}delete this.__tv[cc][ca.toHashCode()];
...
}}},destruct:function(){this.__tv=this.__tx=this.__tw=null;
}});
})();
...
qx.Class.define(h,{extend:qx.ui.root.Abstract,construct:function(o){this.__kk=qx.dom.Node.getWindow(o);
this.__kl=o;
...
if(!feedreader.view.Article.__tI){feedreader.view.Article.__tI=true;
...
w.setLineHeight(1.8);
this.setFont(w);
},properties:{article:{apply:e,nullable:true,check:d}},members:{_applyArticle:function(x,y){var z=this.__tJ();
this.setHtml(z);
qx.html.Element.flush();
var B=this.getContentElement().getDomElement();
var A=B.getElementsByTagName(b);
for(var i=0;i<A.length;i++){A[i].target=t;
}},__tJ:function(){var D=this.getArticle();
if(!D){return v;
...
}},properties:{sort:{check:[l,m,n],init:l,apply:j},separatorX:{check:p,nullable:true,apply:s},separatorY:{check:p,nullable:true,apply:s},connectSeparators:{check:e,init:false,apply:s},spacingX:{check:o,init:0,apply:s},spacingY:{check:o,init:0,apply:s}},members:{__tK:null,__tL:null,verifyLayoutProperty:null,_applySort:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__tM:{north:1,south:2,west:3,east:4,center:5},__tN:{1:q,2:d,3:r,4:g},__tO:function(){var x=this._getLayoutChildren();
var F,z;
var length=x.length;
var B=[];
var E=[];
...
delete this._invalidChildrenCache;
},renderLayout:function(H,I){if(this._invalidChildrenCache){this.__tO();
}var bc=qx.ui.layout.Util;
var O=this.__tK;
var bd=this.__tL;
var length=O.length;
...
},_computeSizeHint:function(){if(this._invalidChildrenCache){this.__tO();
}var bH=this.__tK;
var bR=this.__tL;
...
(function(){var l="zIndex",k="px",j="keydown",h="deactivate",g="resize",f="keyup",d="keypress",c="backgroundColor",b="_applyOpacity",a="Boolean",x="__kt",w="__kp",v="opacity",u="interval",t="Tab",s="Color",r="qx.ui.root.Page",q="__kr",p="Number",o="qx.ui.core.Blocker",m="qx.ui.root.Application",n="_applyColor";
...
this.__kv(c,C);
},_applyOpacity:function(D,E){this.__kv(v,D);
...
M.include();
M.activate();
M.addListener(h,this.__kB,this);
M.addListener(d,this.__kA,this);
M.addListener(j,this.__kA,this);
M.addListener(f,this.__kA,this);
...
N.removeListener(h,this.__kB,this);
N.removeListener(d,this.__kA,this);
N.removeListener(j,this.__kA,this);
N.removeListener(f,this.__kA,this);
N.exclude();
...
this.__kt.addListener(u,this.__kz,this);
}this.__kt.start();
this.__kz();
...
}},__kz:function(){var T=this._widget.getContainerElement().getDomElement();
...
},__kA:function(e){if(e.getKeyIdentifier()==t){e.stop();
}},__kB:function(){if(this.getKeepBlockerActive()){this.getBlockerElement().activate();
}}},destruct:function(){if(this._isPageRoot){this._widget.removeListener(g,this.__ku,this);
}this._disposeObjects(q,w,x);
...
this.__pu=++qx.io.remote.Request.__pu;
},events:{"created":j,"configured":j,"sending":j,"receiving":j,"completed":P,"aborted":j,"failed":P,"timeout":P},statics:{__pu:0,methodAllowsRequestBody:function(T){return (T==O)||(T==I);
...
},init:true,apply:B},crossDomain:{check:k,init:false},fileUpload:{check:k,init:false},transport:{check:l,nullable:true},useBasicHttpAuth:{check:k,init:false},parseJson:{check:k,init:true}},members:{__pq:null,__pr:null,__ps:null,__pt:null,__pu:null,send:function(){qx.io.remote.RequestQueue.getInstance().add(this);
...
},setRequestHeader:function(bf,bg){this.__pq[bf]=bg;
},removeRequestHeader:function(bh){delete this.__pq[bh];
},getRequestHeader:function(bi){return this.__pq[bi]||null;
},getRequestHeaders:function(){return this.__pq;
},setParameter:function(bj,bk,bl){if(bl){this.__ps[bj]=bk;
}else{this.__pr[bj]=bk;
}},removeParameter:function(bm,bn){if(bn){delete this.__ps[bm];
}else{delete this.__pr[bm];
}},getParameter:function(bo,bp){if(bp){return this.__ps[bo]||null;
}else{return this.__pr[bo]||null;
}},getParameters:function(bq){return (bq?this.__ps:this.__pr);
},setFormField:function(br,bs){this.__pt[br]=bs;
},removeFormField:function(bt){delete this.__pt[bt];
},getFormField:function(bu){return this.__pt[bu]||null;
},getFormFields:function(){return this.__pt;
},getSequenceNumber:function(){return this.__pu;
}},destruct:function(){this.setTransport(null);
this.__pq=this.__pr=this.__ps=this.__pt=null;
}});
})();
(function(){var l="dblclick",k="click",j="Boolean",h="excluded",g="visible",f="qx.event.type.Data",d="_applyOpenMode",c="Space",b="Left",a="Enter",z="changeOpenMode",y="_applyRootOpenClose",x="changeSelection",w="qx.ui.tree.Tree",v="tree",u="_applyHideRoot",t="changeRoot",s="_applyRoot",r="keypress",q="none",o="pane",p="__oB",m="Right",n="qx.ui.tree.AbstractTreeItem";
