...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.bom.Client","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.bom.Client-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"9ca48239de46"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var c=".",b="boolean",a="qx.bom.client.Flash";
qx.Class.define(a,{statics:{AVAILABLE:false,FULLVERSION:"0.0.0",REVISION:"0",VERSION:0.0,EXPRESSINSTALL:false,STRICT_SECURITY_MODEL:false,_cachedSupportsVersion:{},supportsVersion:function(d){if(typeof this._cachedSupportsVersion[d]===b){return this._cachedSupportsVersion[d];
}else{var g=d.split(c);
var f=this.FULLVERSION.split(c);

for(var i=0;i<g.length;i++){var e=parseInt(f[i],10)-parseInt(g[i],10);
...
}},__AL:qx.core.Variant.select("qx.client",{"mshtml":function(){if(!window.ActiveXObject){return;
}var k=[0,0,0];
var j=false;

try{var l=new ActiveXObject("ShockwaveFlash.ShockwaveFlash.7");
}catch(m){try{var l=new ActiveXObject("ShockwaveFlash.ShockwaveFlash.6");
k=[6,0,21];
l.AllowScriptAccess="always";
}catch(n){if(k[0]==6){j=true;
}}
if(!j){try{l=new ActiveXObject("ShockwaveFlash.ShockwaveFlash");
}catch(o){}}}
if(!j&&typeof l=="object"){var h=l.GetVariable("$version");

if(typeof h!="undefined"){h=h.split(" ")[1].split(",");
k[0]=parseInt(h[0],10);
k[1]=parseInt(h[1],10);
k[2]=parseInt(h[2],10);
}}this.__AM(k);
...
var q=navigator.plugins["Shockwave Flash"].description;

if(typeof q!="undefined"){q=q.replace(/^.*\s+(\S+\s+\S+$)/,"$1");
p[0]=parseInt(q.replace(/^(.*)\..*$/,"$1"),10);
p[1]=parseInt(q.replace(/^.*\.(.*)\s.*$/,"$1"),10);
p[2]=/r/.test(q)?parseInt(q.replace(/^.*r(.*)$/,"$1"),10):0;
}this.__AM(p);
}}),__AM:function(r){this.FULLVERSION=r.join(".");
this.VERSION=parseFloat(r);
this.AVAILABLE=this.VERSION>0;
this.REVISION=r[r.length-1];

if(r[0]<10){this.STRICT_SECURITY_MODEL=this.supportsVersion("9.0.151");
}else{this.STRICT_SECURITY_MODEL=this.supportsVersion("10.0.12");
}var s=qx.bom.client.Platform;
this.EXPRESSINSTALL=(s.WIN||s.MAC)&&this.supportsVersion("6.0.65");
}},defer:function(t){t.__AL();
}});
})();
(function(){var a="qx.bom.client.Multimedia";
qx.Class.define(a,{statics:{__JJ:{quicktime:{plugin:"QuickTime",control:"QuickTimeCheckObject.QuickTimeCheck.1"},wmv:{plugin:"Windows Media",control:"WMPlayer.OCX.7"},divx:{plugin:"DivX Web Player",control:"npdivx.DivXBrowserPlugin.1"},silverlight:{plugin:"Silverlight",control:"AgControl.AgControl"}},has:function(b,c){if(this.__JJ[b]){var d=this.__JJ[b];
if(d.installed){if(c==null||(d.version>=parseFloat(c))){return true;
}}}return false;
},__JK:qx.core.Variant.select("qx.client",{"mshtml":function(){var e=window.ActiveXObject;

if(!e){return;
}var g=this.__JJ;
var f,h;

for(var j in g){f=g[j];

try{h=new ActiveXObject(f.control);
}catch(k){continue;
}switch(j){case "quicktime":f.version=0;
break;
case "wmv":f.version=h.versionInfo;
break;
case "divx":f.version=0;
break;
case "silverlight":try{f.version=h.version===undefined?0:h.version;
}catch(l){try{f.version=h.settings.version===undefined?0:h.settings.version;
}catch(m){f.version=0;
}}break;
}f.installed=true;
}},"default":function(){var r=navigator.plugins;
...
for(var i=0,q=r.length;i<q;i++){n=r[i];
name=n.name;

for(var t in o){s=o[t];

if(!s.installed&&name.indexOf(s.plugin)!==-1){s.installed=true;

if(p.test(n.name)||p.test(n.description)){s.version=parseFloat(RegExp.$1,10);
}else{s.version=0;
}break;
}}}}})},defer:function(u){u.__JK();
}});
})();
