...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.ui.FiniteStateMachine","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.ui.FiniteStateMachine-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"4e33b578ec9d"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var h="String",g="Number",f="string",d="__GA",c="qx.util.fsm.FiniteStateMachine",b="__GB";
...
if(!this.__GF[u]){this.__GF[u]={};
}this.__GF[u][q]=true;
if(!this.__GG[q]){this.__GG[q]=[];
}this.__GG[q]=this.__GG[q].concat(s);
}},removeObject:function(w){var y=this.__GD[w];
if(this.__GG[w]){for(var x in this.__GG[w]){delete this.__GF[x];
}delete this.__GG[w];
}delete this.__GE[y];
delete this.__GD[w];
delete this.__GC[w];
},getObject:function(z){return this.__GC[z];
},getFriendlyName:function(A){var B=A.toHashCode();
return B?this.__GE[B]:null;
},getGroupObjects:function(C){var a=[];

for(var name in this.__GF[C]){a.push(name);
}return a;
},displayAllObjects:function(){for(var D in this.__GD){var F=this.__GD[D];
var E=this.getObject(D);
this.debug(D+" => "+F);
this.debug("  "+F+" => "+this.__GE[F]);
this.debug("  "+D+" => "+this.getObject(D));
this.debug("  "+this.getObject(D)+" => "+this.getFriendlyName(E));
}},start:function(){var H=this.__Gy;
...
}this.__Gx[H].getAutoActionsBeforeOnentry()(this);
if(G){this.debug(this.getName()+"#"+H+"#entry");
}this.__Gx[H].getOnentry()(this,null);
if(G){this.debug(this.getName()+"#"+H+"#actionsAfterOnentry");
}this.__Gx[H].getAutoActionsAfterOnentry()(this);
},pushState:function(I){if(this.__Gz.length>=this.getMaxSavedStates()){throw new Error("Saved-state stack is full");
}
if(I===true){this.__Gz.push(this.getState());
}else if(I){this.__Gz.push(I);
}else{this.__Gz.push(this.getPreviousState());
}},postponeEvent:function(event){this.__GB.unshift(event);
},enqueueEvent:function(event,J){if(J){this.__GA.push(event);
}else{this.__GA.unshift(event);
}
...
},__GI:function(){if(this.__GH){return ;
}this.__GH=true;
while(this.__GA.length>0){var event=this.__GA.pop();
var R=this.__GJ(event);
if(R){event.dispose();
}}this.__GH=false;
},__GJ:function(event){var U;
var W;
var T;
var bf;
var bd;
...
if(typeof (W)=="string"){if(!W in this.__Gx){throw new Error("Attempt to transition to nonexistent state "+W);
}this.setNextState(W);
}else{switch(W){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:W=U;
this.setNextState(W);
break;
case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Gz.length==0){throw new Error("Attempt to transition to POP_STATE_STACK "+"while state stack is empty.");
}W=this.__Gz.pop();
this.setNextState(W);
break;
...
}bf=this.__Gx[this.getNextState()];
...
for(var i=0;i<this.__GB.length;i++){e=this.__GB.pop();
this.__GA.unshift(e);
}
...
(function(){var p="string",o="object",n="number",m="function",l="autoActionsAfterOnentry",k='"',h="autoActionsBeforeOnexit",d="autoActionsBeforeOnentry",c="}",b="{",ba="undefined",Y="autoActionsAfterOnexit",X="(",W=";",V="  var groupObjects = ",U="__Gp",T="parameters",S="__Gt",R=")",Q="__Gr",w=" fsm.getObject('",x="__Gv",u="groups",v="  for (var i = 0; i < groupObjects.length; i++)",s="fsm",t="');",q="__Gu",r="onexit",y="  fsm.debug(ex);",z="__Gq",G="    fsm.getObject(objName).",E=")(fsm)",K="events",I="__Gs",M="    var objName = groupObjects[i];",L="__Gw",B="').",P="objects",O="catch(ex)",N="  {",A="    fsm.getGroupObjects('",C="  }",D="qx.util.fsm.State",F="onentry",H="try",J=",";
...
for(j=0;j<g.length;j++){bl+=V+A+g[j]+t+v+N+M+G+bk+W+C;
}}}}bl+=c+O+b+y+c;
return new Function(s,bl);
}},properties:{name:{transform:U,nullable:true},onentry:{transform:z,nullable:true,init:function(bm,event){}},onexit:{transform:Q,nullable:true,init:function(bn,event){}},autoActionsBeforeOnentry:{transform:S,nullable:true,init:function(bo,event){}},autoActionsAfterOnentry:{transform:q,nullable:true,init:function(bp,event){}},autoActionsBeforeOnexit:{transform:x,nullable:true,init:function(bq,event){}},autoActionsAfterOnexit:{transform:L,nullable:true,init:function(br,event){}},events:{transform:I,nullable:true}},members:{__Gp:function(bs){if(typeof (bs)!=p||bs.length<1){throw new Error("Invalid state name");
}return bs;
},__Gq:function(bt){switch(typeof (bt)){case ba:return function(bu,event){};
case m:return bt;
default:throw new Error("Invalid onentry type: "+typeof (bt));
}},__Gr:function(bv){switch(typeof (bv)){case ba:return function(bw,event){};
case m:return bv;
default:throw new Error("Invalid onexit type: "+typeof (bv));
return null;
}},__Gs:function(bx){if(typeof (bx)!=o){throw new Error("events must be an object");
...
},__Gt:function(bA){return qx.util.fsm.State._commonTransformAutoActions(d,bA);
},__Gu:function(bB){return qx.util.fsm.State._commonTransformAutoActions(l,bB);
},__Gv:function(bC){return qx.util.fsm.State._commonTransformAutoActions(h,bC);
},__Gw:function(bD){return qx.util.fsm.State._commonTransformAutoActions(Y,bD);
...
(function(){var j="predicate",i="nextState",h="autoActionsBeforeOntransition",g="__GK",f="__GM",e="__GL",d="autoActionsAfterOntransition",c="qx.util.fsm.Transition",b="ontransition",a="String";
...
break;
case h:this.setAutoActionsBeforeOntransition(l[m]);
break;
...
},transform:g},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:e},autoActionsBeforeOntransition:{init:function(o,event){}},autoActionsAfterOntransition:{init:function(p,event){}},ontransition:{init:function(q,event){},transform:f}},members:{__GK:function(r){switch(typeof (r)){case "undefined":return function(s,event){return true;
};
...
break;
}},__GL:function(u){switch(typeof (u)){case "string":return u;
...
break;
}},__GM:function(v){switch(typeof (v)){case "undefined":return function(w,event){};
