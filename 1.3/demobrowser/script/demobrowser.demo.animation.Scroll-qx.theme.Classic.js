...
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:d,check:[d,b]},x:{init:0,check:c},y:{init:0,check:c}},members:{__GN:null,__GO:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var e=this._getElement();
this.__GN={x:e.scrollLeft,y:e.scrollTop};
if(this._atEndPosition(this.__GN.x,this.__GN.y)){return;
}
if(this.getMode()==b){this.__GO={left:this.getX()-this.__GN.x,top:this.getY()-this.__GN.y};
}},update:function(f){qx.fx.Base.prototype.update.call(this);
var g=this._getElement();

if(this.getMode()==d){if(this.getX()!=0){g.scrollLeft=this.__GN.x+(this.getX()*f);
}
if(this.getY()!=0){g.scrollTop=this.__GN.y+(this.getY()*f);
}}else{g.scrollLeft=this.__GN.x+(this.__GO.left*f);
g.scrollTop=this.__GN.y+(this.__GO.top*f);
