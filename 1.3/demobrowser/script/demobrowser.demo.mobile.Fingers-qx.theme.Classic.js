...
qx.Class.define(t,{extend:qx.application.Native,members:{__PF:null,__PG:null,main:function(){qx.application.Native.prototype.main.call(this);
{};
...
this.__PF[i]=parseInt(J.target.style.left)-J.pageX;
this.__PG[i]=parseInt(J.target.style.top)-J.pageY;
}},_onTouchMove:function(e){var M=e.getAllTouches();

for(var i=0;i<M.length;i++){var L=M[i];

if(M.target==document.body){continue;
}qx.bom.element.Style.setStyles(L.target,{"left":(L.pageX+this.__PF[i])+l,"top":(L.pageY+this.__PG[i])+l});
}e.stop();
}}});
})();
