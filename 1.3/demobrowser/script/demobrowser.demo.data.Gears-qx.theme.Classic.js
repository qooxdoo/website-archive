...
}this.__OC=this.initDB();
this.__OD();
},members:{__OD:function(K){var L=this.getPerson();
this._marshaler.toClass(L,true);
this.setModel(this._marshaler.toModel(L));
this.getModel().addListener("changeBubble",this.__OE,this);
this.fireDataEvent("loaded",this.getModel());
},__OE:function(M){var N=this.getModel();
this.update(N.getFirstname(),N.getLastname(),N.getAge());
},update:function(O,P,Q){if(!google.gears.factory||!this.__OC){return;
}this.__OC.execute(v);
this.__OC.execute(o,[O,P,Q]);
},getPerson:function(){var R={};
R.firstname=i;
R.lastname=i;
R.age=0;

if(!google.gears.factory||!this.__OC){return R;
}var S=this.__OC.execute(w);

if(S.isValidRow()){R.firstname=S.field(0);
R.lastname=S.field(1);
R.age=parseInt(S.field(2));
}S.close();
return R;
},initDB:function(){if(window.google&&google.gears){try{var T=google.gears.factory.create(l);
