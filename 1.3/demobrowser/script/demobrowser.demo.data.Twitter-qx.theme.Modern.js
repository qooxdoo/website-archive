...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.data.Twitter","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.data.Twitter-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"093529d51e00"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var j="value",i="icon",h="model",g="text",f="qx.client",e="source",d="wittemann",c="mshtml",b=".json",a="user.profile_image_url",D="http://twitter.com/statuses/user_timeline/",C="Enter",B="<span style='font-size: 20px'>Twitter</span>",A="<a target='_blank' href='http://twitter.com/home?status=Check%20out%20the%20%23qooxdoo%20twitter%20demo%20at%20http://bit.ly/Mc4Of'>Tweet This</a></span>",z="demobrowser.demo.data.store.Twitter",y="keydown",x="execute",w="demobrowser.demo.data.Twitter",v="selection[0]",u="<span style='font-size: 13px'>",q="Location: ",r="Details",o="Name: ",p="?callback=demobrowser.demo.data.store.Twitter.saveResult",m="User",n="Postet with: ",k="user.name",l="user.location",s="Avatar: ",t="Message: ";
...
this.getRoot().add(I,{left:10,top:85});
I.setWidth(300);
var K=new qx.data.controller.List(null,I);
K.setDelegate(this);
K.setLabelPath(g);
if(!qx.core.Variant.isSet(f,c)){K.setIconPath(a);
}var O=new demobrowser.demo.data.store.Twitter(d);
O.bind(h,K,h);
var L=new qx.ui.form.Button(m);
this.getRoot().add(L,{left:10,top:55});
var E=new qx.ui.form.TextField(d);
this.getRoot().add(E,{left:60,top:55});
L.addListener(x,function(){var Q=D+E.getValue()+b;

if(O.getUrl()==Q){O.reload();
}else{O.setUrl(Q);
}},this);
E.addListener(y,function(R){if(R.getKeyIdentifier()==C){L.execute();
}},this);
var H=new qx.ui.groupbox.GroupBox(r);
this.getRoot().add(H,{left:320,top:60});
H.setWidth(300);
H.setHeight(220);
H.setLayout(new qx.ui.layout.Grid(0,5));
H.add(new qx.ui.basic.Label(o),{row:0,column:0});
H.add(new qx.ui.basic.Label(q),{row:1,column:0});
H.add(new qx.ui.basic.Label(t),{row:2,column:0});
H.add(new qx.ui.basic.Label(n),{row:3,column:0});
var name=new qx.ui.basic.Label();
H.add(name,{row:0,column:1});
var location=new qx.ui.basic.Label();
H.add(location,{row:1,column:1});
var M=new qx.ui.basic.Label();
M.setRich(true);
M.setWidth(150);
H.add(M,{row:2,column:1});
var P=new qx.ui.basic.Label();
P.setRich(true);
H.add(P,{row:3,column:1});
var F=new qx.data.controller.Object();
F.addTarget(name,j,k);
F.addTarget(location,j,l);
F.addTarget(M,j,g);
F.addTarget(P,j,e);

if(!qx.core.Variant.isSet(f,c)){H.add(new qx.ui.basic.Label(s),{row:4,column:0});
var J=new qx.ui.basic.Image();
H.add(J,{row:4,column:1});
F.addTarget(J,e,a);
}K.bind(v,F,h);
...
N.setWidth(260);
N.setValue(u+A);
this.getRoot().add(N,{left:10,top:290});
},configureItem:function(S){S.setRich(true);
S.getChildControl(i).setWidth(50);
S.getChildControl(i).setHeight(50);
S.getChildControl(i).setScale(true);
}}});
qx.Class.define(z,{extend:qx.data.store.Json,statics:{saveResult:function(T){this.__OF=T;
}},construct:function(U){var V=D+U+b;
qx.data.store.Json.call(this,V);
},members:{_createRequest:function(W){var X=new qx.io.ScriptLoader();
W+=p;
X.load(W,function(Y){this.__OG();
},this);
},__OG:function(){var ba=demobrowser.demo.data.store.Twitter.__OF;

if(ba==undefined){this.setState("failed");
return;
}this._marshaler.toClass(ba);
this.setModel(this._marshaler.toModel(ba));
