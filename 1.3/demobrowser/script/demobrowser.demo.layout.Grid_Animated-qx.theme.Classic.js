...
l.setSpacing(20);
var k=new qx.ui.container.Composite(l);
k.setPadding(20);
this.getRoot().add(k,{left:0,top:0});
k.add(this.getAnimGrid(),{row:0,column:0});
},getNewWidget:function(){var m=new qx.ui.core.Widget().set({width:50,height:50,backgroundColor:j});
m.addListener(h,function(e){if(this._active==m){return;
}var p=[];
var n=0.3;

if(this._active){this._active.set({backgroundColor:j,width:50,height:50});
}m.set({backgroundColor:b});
var q=m.getBounds();
p.push(new demobrowser.demo.layout.Grid_Animated_Property(m,f).set({from:q.width,to:200,duration:n,transition:i}));
p.push(new demobrowser.demo.layout.Grid_Animated_Property(m,d).set({from:q.height,to:200,duration:n,transition:i}));
var o=new qx.fx.effect.core.Parallel(p[0],p[1]);
o.start();
this._active=m;
},this);
return m;
},getAnimGrid:function(){var r=new qx.ui.container.Composite().set({decorator:a,backgroundColor:g,width:500,height:500});
var s=new qx.ui.layout.Grid();
s.setSpacing(3);
r.setLayout(s);
this._active=null;

for(var x=0;x<7;x++){s.setColumnFlex(x,1);
s.setRowFlex(x,1);

for(var y=0;y<7;y++){r.add(this.getNewWidget(),{row:y,column:x});
}}return r;
}}});
})();
...
(function(){var b="qx.fx.effect.core.Parallel",a="__yA";
qx.Class.define(b,{extend:qx.fx.Base,construct:function(c){qx.fx.Base.call(this);
this.__yA=arguments;
},members:{__yA:null,finish:function(){qx.fx.Base.prototype.finish.call(this);
var d=this.__yA;
