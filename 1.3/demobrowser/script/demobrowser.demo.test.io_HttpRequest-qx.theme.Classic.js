...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.test.io_HttpRequest","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.test.io_HttpRequest-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"e3a998d883eb"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var j="qx.event.type.Event",i="Boolean",h="String",g="basic",f="GET",e="text/plain",d="http",c="POST",b="application/xml",a="X-Requested-With",H="application/json",G="text/html",F="If-Modified-Since",E="Last-Modified",D='Basic ',C="Integer",B="qx.event.type.Data",A="__IR",z="HEAD",y="XMLHttpRequest",q='Authorization',r="Accept",o="OPTIONS",p="text/javascript",m="",n="PUT",k=':',l="application/x-www-form-urlencoded",s="DELETE",t="qx.io.HttpRequest",v="no-cache",u="Cache-Control",x="Content-Type",w="Thu, 01 Jan 1970 00:00:00 GMT";
qx.Class.define(t,{extend:qx.core.Object,construct:function(I){qx.core.Object.call(this);
this.__IP={};
if(I!=null){this.setUrl(I);
}},statics:{__IQ:{}},events:{"change":B,"timeout":j,"load":j,"error":j,"abort":j},properties:{refresh:{check:i,init:false},data:{nullable:true},method:{check:[f,c,n,z,s,o],init:f},async:{check:i,init:true},mime:{check:[e,p,H,b,G],init:e},url:{check:h,init:m},username:{check:h,nullable:true},password:{check:h,nullable:true},auth:{check:[d,g],init:d},timeout:{check:C,nullable:true},cache:{check:i,init:false}},members:{__IR:null,__IP:null,setRequestHeader:function(J,K){this.__IP[J]=K;
},removeRequestHeader:function(L){delete this.__IP[L];
},getRequestHeader:function(M){var N=this.__IP[M];

if(N===undefined){N=null;
}return N;
},getResponseText:function(){var O=this.__IR;

if(O){return O.responseText;
}},getResponseXml:function(){var P=this.__IR;

if(P){return P.responseXML;
}},getResponseHeader:function(Q){var R=this.__IR;

if(R){return R.getResponseHeader(Q);
}},getAllResponseHeaders:function(){var S=this.__IR;

if(S){return S.getAllResponseHeaders();
}},isNotModified:function(){var T=this.__IR;

if(!T){return false;
}var U=T.getResponseHeader(E);
return T.status===304||qx.io.HttpRequest.__IQ[this.getUrl()]===U;
},isSuccessful:function(){var V=this.__IR;
return !V||V.isSuccessful();
},getStatusCode:function(){var W=this.__IR;

if(W){return W.status;
}},getStatusText:function(){var X=this.__IR;

if(X){return X.statusText;
}},getReadyState:function(){var Y=this.__IR;

if(Y){return Y.readyState;
}},send:function(){if(this.__IR){if(this.getReadyState()!==4){throw new Error("Request is still pending at ready state: "+this.getReadyState());
}this.__IR.dispose();
}var bc=this.__IR=new qx.bom.Request;
bc.onreadystatechange=qx.lang.Function.bind(this.__IS,this);
bc.ontimeout=qx.lang.Function.bind(this.__IT,this);
bc.onload=qx.lang.Function.bind(this.__IU,this);
bc.onerror=qx.lang.Function.bind(this.__IV,this);
bc.onabort=qx.lang.Function.bind(this.__IW,this);
...
}if(this.getRefresh()){bc.setRequestHeader(F,qx.io.HttpRequest.__IQ[bb]||w);
...
},abort:function(){if(this.__IR){this.__IR.abort();
}},__IS:qx.event.GlobalError.observeMethod(function(){this.fireDataEvent("change",this.getReadyState());
if(this.getRefresh()&&this.getReadyState()===4&&this.isSuccessful()){var bf=this.getResponseHeader("Last-Modified");

if(bf){qx.io.HttpRequest.__IQ[this.getUrl()]=bf;
}}}),__IT:qx.event.GlobalError.observeMethod(function(){if(this.hasListener("timeout")){this.fireEvent("timeout");
}}),__IU:qx.event.GlobalError.observeMethod(function(){if(this.hasListener("load")){this.fireEvent("load");
}}),__IV:qx.event.GlobalError.observeMethod(function(){if(this.hasListener("error")){this.fireEvent("error");
}}),__IW:qx.event.GlobalError.observeMethod(function(){if(this.hasListener("abort")){this.fireEvent("abort");
...
qx.Class.define(c,{extend:qx.core.Object,construct:function(){this.__Ee={};
this.__Ef=this.__Ep();
},statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},members:{__Eg:null,__Eh:null,__Ef:null,__Ei:null,__Ej:null,__Ek:null,__El:null,__Ee:null,readyState:0,responseText:f,responseXML:null,status:0,statusText:f,timeout:0,onreadystatechange:function(){},ontimeout:function(){},onload:function(){},onerror:function(){},onabort:function(){},open:function(j,k,l,m,n){if(l==null){l=true;
}this.__Eg=l;
this.__Eh=qx.lang.Function.bind(this.__Em,this);
this.__Ei=qx.lang.Function.bind(this.__En,this);
this.__Ef.onreadystatechange=this.__Eh;
if(arguments.length>4){this.__Ef.open(j,k,l,m,n);
}else if(arguments.length>3){this.__Ef.open(j,k,l,m);
}else{this.__Ef.open(j,k,l);
}if(qx.core.Variant.isSet(i,h)){if(!l){this.readyState=qx.bom.Request.OPENED;
this.__Eq();
}}},send:function(o){var p=this.__Ee;
if(o&&o.nodeType){o=window.XMLSerializer?new XMLSerializer().serializeToString(o):o.xml;

if(!p[g]){p[g]=d;
}}for(var q in p){this.__Ef.setRequestHeader(q,p[q]);
}if(this.timeout!=null&&this.timeout>0){this.__Ej=window.setTimeout(this.__Ei,this.timeout);
}this.__Ef.send(o);
if(qx.core.Variant.isSet(i,h)){if(!this.__Eg){this.readyState=qx.bom.Request.OPENED;
this.__Es(this);
...
},abort:function(){if(this.__Ej){window.clearTimeout(this.__Ej);
}this.__Eo();
this.onabort();
this.dispose();
},__Em:qx.event.GlobalError.observeMethod(function(){if(qx.core.Variant.isSet(i,h)){if(!this.__Eg){return;
}}this.readyState=this.__Ef.readyState;
this.__Es();
if(this.__Ek){this.readyState=qx.bom.Request.UNSENT;
return ;
}if(this.readyState==qx.bom.Request.DONE&&this.__Ej){window.clearTimeout(this.__Ej);
}this.__Eq();
if(this.readyState==qx.bom.Request.DONE){this.dispose();
}}),__En:qx.event.GlobalError.observeMethod(function(){this.__Eo();
this.ontimeout();
this.dispose();
}),getAllResponseHeaders:function(){try{return this.__Ef.getAllResponseHeaders();
}catch(r){return null;
}},getResponseHeader:function(s){try{return this.__Ef.getResponseHeader(s);
}catch(t){return null;
}},setRequestHeader:function(u,v){if(v==null){delete this.__Ee[u];
}else{this.__Ee[u]=v;
}},removeRequestHeader:function(w,x){delete this.__Ee[w];
},getRequestHeader:function(y){return this.__Ee[y]||null;
},__Eo:function(){this.__Es();
if(this.readyState>qx.bom.Request.UNSENT){this.__Ek=true;
}this.__Ef.abort();
},__Ep:qx.core.Variant.select(i,{"default":function(){return new XMLHttpRequest;
},"mshtml":function(){if(window.ActiveXObject&&qx.xml.Document.XMLHTTP){return new ActiveXObject(qx.xml.Document.XMLHTTP);
}
if(window.XMLHttpRequest){return new XMLHttpRequest;
}}}),__Eq:function(){if(this.__El===this.readyState){return;
}this.onreadystatechange();
this.__El=this.readyState;
if(this.readyState===4){if(this.isSuccessful()){this.onload();
}else{this.onerror();
}}},__Er:function(){var z=this.__Ef.responseXML;

if(qx.core.Variant.isSet(i,e)){if(z&&!z.documentElement&&this.__Ef.getResponseHeader(g).match(/[^\/]+\/[^\+]+\+xml/)){z=new ActiveXObject(qx.xml.Document.DOMDOC);
z.loadXML(this.__Ef.responseText);
}if(z&&z.parseError!=0){return null;
}}else if(!z.documentElement||z.documentElement.tagName==a){return null;
}return z;
},__Es:function(){var A=this.__Ef;

try{this.responseText=A.responseText;
}catch(B){}
try{this.responseXML=this.__Er();
}catch(C){}
try{this.status=A.status;
}catch(D){}
try{this.statusText=A.statusText;
}catch(E){}if(qx.core.Variant.isSet(i,e)){if(this.status===1223){this.status=204;
}}if(!this.status&&location.protocol===b){this.status=204;
}}},destruct:function(){if(this.__Ej){window.clearTimeout(this.__Ej);
this.__Ej=null;
}if(this.__Ef){this.__Ef.onreadystatechange=qx.lang.Function.empty;
this.__Ef=null;
...
qx.Class.define(a,{statics:{__IX:"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",encode:function(c){var h=this.__IX;
var m=b;
var f,e,d;
var j,k,l,g;
var i=0;
