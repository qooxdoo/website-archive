...
qx.Class.define(m,{extend:qx.application.Native,members:{__Pl:null,main:function(){qx.application.Native.prototype.main.call(this);
var r=qx.util.ResourceManager.getInstance();
this.__Pl=[g,k,h,r.toUri(f)];
...
this.window1=qx.bom.Window.open(this.__Pl[0],j,x);
},this);
...
this.window2=qx.bom.Window.open(this.__Pl[3],a,y,true);
},this);
...
this.window3=qx.bom.Window.open(this.__Pl[0],i,A,true,false);
},this);
...
(function(){var d="qx.bom.Window",c="interval",b="javascript:/",a="qxNativeWindow";
qx.Class.define(d,{statics:{__JL:null,__JM:null,__JN:null,__JO:{"top":"dialogTop",left:"dialogLeft",width:"dialogWidth",height:"dialogHeight",scrollbars:"scroll",resizable:"resizable"},__JP:{"top":1,left:1,width:1,height:1,dependent:1,resizable:1,status:1,location:1,menubar:1,scrollbars:1,toolbar:1},__JQ:function(){return window.showModalDialog!=null;
},open:function(e,name,f,g,h){if(e==null){e=b;
}
if(name==null){name=a+new Date().getTime();
}
if(h==null){h=true;
}var i=this.__JR(f,g&&h);

if(g){if(this.__JQ()&&h){return window.showModalDialog(e,[window.self],i);
}else{this.getBlocker().block();

if(this.__JN==null){this.__JN=new qx.event.Timer(500);
this.__JN.addListener(c,this.__JS,this);
}this.__JM=window.open(e,name,i);
this.__JN.restart();
return this.__JM;
}}else{return window.open(e,name,i);
}},__JR:function(j,k){var l;
var o;
var m=[];

if(k&&this.__JQ()){for(var n in j){if(qx.bom.Window.__JO[n]){var p="";

if(n!="scrollbars"&&n!="resizable"){p="px";
}o=qx.bom.Window.__JO[n]+":"+j[n]+p;
m.push(o);
}else{qx.log.Logger.warn("Option '"+n+"' is not supported for modal windows.");
}}l=m.join(";");
}else{for(var n in j){if(qx.bom.Window.__JP[n]){if(qx.lang.Type.isBoolean(j[n])){o=n+"="+(j[n]?"yes":"no");
}else{o=n+"="+j[n];
}m.push(o);
}else{qx.log.Logger.warn("Option '"+n+"' is not supported for native windows.");
}}l=m.join(",");
}return l;
},__JS:function(){if(this.isClosed(this.__JM)){this.getBlocker().unblock();
this.__JN.stop();
}},getBlocker:function(){if(this.__JL==null){this.__JL=new qx.bom.Blocker;
}return this.__JL;
},close:function(q){if(q){return q.close();
}},isClosed:function(r){var closed=true;

if(r){try{closed=r.closed;
}catch(s){}}return closed;
...
}}},resizeTo:function(w,x,y){if(!qx.bom.Window.isClosed(w)){try{w.resizeTo(x,y);
}catch(z){qx.log.Logger.error("Cross-Domain Scripting problem: Could not resize window!",z);
}}}}});
})();
