...
var variants = {"qx.aspects":"off","qx.debug":"off","qx.treevirtual":"on"};
...
(function(){var s="Trash",r="Windows (C:)",q="Data",p="Documents (D:)",o="background-color: cyan;",n="demobrowser.demo.treevirtual.TreeVirtual",m="Edit",l="Workspace",k="Network",j="Spam Message #",c="icon/22/apps/internet-mail.png",h="Inbox",f="Desktop",b="Sent",a="background-color: red; color: white;",e="Files",d="Tree",g="Spam";
qx.Class.define(n,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);
qx.Class.include(qx.ui.treevirtual.TreeVirtual,qx.ui.treevirtual.MNode);
var t=new qx.ui.treevirtual.TreeVirtual(d);
t.set({width:400,rowHeight:22});
t.setColumnWidth(0,400);
t.setAlwaysShowOpenCloseSymbol(true);
this.getRoot().add(t,{left:10,top:30,bottom:30});
var x=t.getDataModel();
var u=x.addBranch(null,f,true);
t.nodeSetLabelStyle(u,a);
var w;
x.addBranch(u,e,true);
w=x.addBranch(u,l,true);
x.addLeaf(w,r);
x.addLeaf(w,p);
x.addBranch(u,k,true);
w=x.addBranch(u,s,true);
t.nodeSetCellStyle(w,o);
var v=x.addBranch(null,h,true,false,c);
w=x.addBranch(v,g,false);

for(var i=1;i<3000;i++){x.addLeaf(w,j+i);
}x.addBranch(v,b,true);
x.addBranch(v,s,true);
x.addBranch(v,q,true);
x.addBranch(v,m,true);
x.setData();
}}});
})();
...
if(!be.bHideOpenClose){bf.setState(be,{bOpened:!be.bOpened});
}bi=true;
}break;
...
(function(){var l="number",k="dataChanged",h="object",g="metaDataChanged",f="treeOpenWhileEmpty",e="treeOpenWithContent",d="bSelected",c="qx.ui.treevirtual.SimpleTreeDataModel",b="bOpened",a="treeClose";
...
this.__vQ=null;
},members:{__vR:null,__vQ:null,_rowArr:null,_nodeArr:null,_nodeRowMap:null,_treeColumn:null,_selections:null,setTree:function(m){this.__vR=m;
},getTree:function(){return this.__vR;
},setEditable:function(n){this.__vQ=[];

for(var o=0;o<this.getColumnCount();o++){this.__vQ[o]=n;
}this.fireEvent(g);
},setColumnEditable:function(p,q){if(q!=this.isColumnEditable(p)){if(this.__vQ==null){this.__vQ=[];
}this.__vQ[p]=q;
this.fireEvent(g);
}},isColumnEditable:function(r){if(r==this._treeColumn){return false;
}return (this.__vQ?this.__vQ[r]==true:false);
},isColumnSortable:function(s){return false;
},sortByColumn:function(t,u){throw new Error("Trees can not be sorted by column");
},getSortColumnIndex:function(){return -1;
},setTreeColumn:function(v){this._treeColumn=v;
...
},getValue:function(x,y){if(y<0||y>=this._rowArr.length){throw new Error("this._rowArr row "+"("+y+") out of bounds: "+this._rowArr+" (0.."+(this._rowArr.length-1)+")");
}
if(x<0||x>=this._rowArr[y].length){throw new Error("this._rowArr column "+"("+x+") out of bounds: "+this._rowArr[y]+" (0.."+(this._rowArr[y].length-1)+")");
}return this._rowArr[y][x];
},setValue:function(z,A,B){if(z==this._treeColumn){return ;
}var C=this.getNodeFromRow(A);

if(C.columnData[z]!=B){C.columnData[z]=B;
this.setData();
if(this.hasListener(k)){var D={firstRow:C.nodeId,lastRow:C.nodeId,firstColumn:z,lastColumn:z};
this.fireDataEvent(k,D);
}}},getNode:function(E){if(E<0||E>=this._rowArr.length){throw new Error("this._rowArr row "+"("+E+") out of bounds: "+this._rowArr+" (0.."+(this._rowArr.length-1)+")");
}return this._rowArr[E][this._treeColumn];
},addBranch:function(F,G,H,I,J,K){return qx.ui.treevirtual.MTreePrimitive._addNode(this._nodeArr,F,G,H,I,qx.ui.treevirtual.MTreePrimitive.Type.BRANCH,J,K);
},addLeaf:function(L,M,N,O){return qx.ui.treevirtual.MTreePrimitive._addNode(this._nodeArr,L,M,false,false,qx.ui.treevirtual.MTreePrimitive.Type.LEAF,N,O);
},prune:function(P,Q){var R;
var S;

if(typeof (P)==h){R=P;
S=R.nodeId;
}else if(typeof (P)==l){S=P;
}else{throw new Error("Expected node object or node id");
}for(var i=this._nodeArr[S].children.length-1;i>=0;i--){this.prune(this._nodeArr[S].children[i],true);
}if(Q&&S!=0){R=this._nodeArr[S];
qx.lang.Array.remove(this._nodeArr[R.parentNodeId].children,S);
if(this._selections[S]){delete this._selections[S];
}this._nodeArr[S]=null;
}},move:function(T,U){var ba;
var V;
var Y;
var W;
U=U||0;
...
if(typeof (T)==h){ba=T;
V=ba.nodeId;
}else if(typeof (T)==l){V=T;
ba=this._nodeArr[V];
}else{throw new Error("Expected move node object or node id");
}
if(typeof (U)==h){Y=U;
W=Y.nodeId;
}else if(typeof (U)==l){W=U;
Y=this._nodeArr[W];
}else{throw new Error("Expected parent node object or node id");
}if(Y.type==qx.ui.treevirtual.MTreePrimitive.Type.LEAF){throw new Error("Sorry, a LEAF may not have children.");
}var X=this._nodeArr[ba.parentNodeId];
qx.lang.Array.remove(X.children,V);
Y.children.push(V);
this._nodeArr[V].parentNodeId=W;
},setData:function(bb){var bc=this;
function bd(){var bj=function(bk,bl){var bp=null;
var bo;
var bm=bc._nodeArr[bk].children.length;

for(var i=0;i<bm;i++){bo=bc._nodeArr[bk].children[i];
bp=bc._nodeArr[bo];
if(bp==null){continue;
}bp.level=bl;
bp.bFirstChild=(i==0);
bp.lastChild=[i==bm-1];
var parent=bc._nodeArr[bp.parentNodeId];
while(parent.nodeId){var bq=parent.lastChild[parent.lastChild.length-1];
bp.lastChild.unshift(bq);
parent=bc._nodeArr[parent.parentNodeId];
}if(!bp.columnData){bp.columnData=[];
}
if(bp.columnData.length<bc.getColumnCount()){bp.columnData[bc.getColumnCount()-1]=null;
}var bn=[];
if(bp.columnData){for(var j=0;j<bp.columnData.length;j++){if(j==bc._treeColumn){bn.push(bp);
}else{bn.push(bp.columnData[j]);
}}}else{bn.push(bp);
}bc._nodeRowMap[bp.nodeId]=bc._rowArr.length;
bc._rowArr.push(bn);
if(bp.bSelected){bn.selected=true;
bc._selections[bp.nodeId]=true;
}if(bp.bOpened){bj(bo,bl+1);
}}};
bc._rowArr=[];
bc._nodeRowMap=[];
bc._selections={};
bj(0,1);
if(bc.hasListener(k)){var bi={firstRow:0,lastRow:bc._rowArr.length-1,firstColumn:0,lastColumn:bc.getColumnCount()-1};
bc.fireDataEvent(k,bi);
}}
if(bb instanceof Array){this._nodeArr=bb;
}else if(bb!==null&&bb!==undefined){throw new Error("Expected array of node objects or null/undefined; "+"got "+typeof (bb));
}bd();
var bf=bc.getTree().getSelectionModel();
var be=this._selections;

for(var bg in be){var bh=this.getRowFromNodeId(bg);
bf.setSelectionInterval(bh,bh);
...
},setColumnData:function(br,bs,bt){this._nodeArr[br].columnData[bs]=bt;
},getColumnData:function(bu,bv){return this._nodeArr[bu].columnData[bv];
},setState:function(bw,bx){var by;
var bD;

if(typeof (bw)==h){by=bw;
bD=by.nodeId;
}else if(typeof (bw)==l){bD=bw;
by=this._nodeArr[bD];
}else{throw new Error("Expected node object or node id");
}
for(var bB in bx){switch(bB){case d:var bE=this.getRowFromNodeId(bD);
var bz=this.getTree().getSelectionModel();
var bA=qx.ui.treevirtual.TreeVirtual;
var bF=(typeof (bE)===l&&this.getTree().getSelectionMode()!=bA.SelectionMode.NONE);
if(bx[bB]){this._selections[bD]=true;
if(bF&&!bz.isSelectedIndex(bE)){bz.setSelectionInterval(bE,bE);
}}else{delete this._selections[bD];
if(bF&&bz.isSelectedIndex(bE)){bz.removeSelectionInterval(bE,bE);
}}break;
case b:if(bx[bB]==by.bOpened){break;
}var bC=this.__vR;
if(by.bOpened){bC.fireDataEvent(a,by);
}else{if(by.children.length>0){bC.fireDataEvent(e,by);
}else{bC.fireDataEvent(f,by);
}}if(!by.bHideOpenClose){by.bOpened=!by.bOpened;
bC.getSelectionModel()._resetSelection();
}this.setData();
break;
default:break;
}by[bB]=bx[bB];
}},getNodeRowMap:function(){return this._nodeRowMap;
},getRowFromNodeId:function(bG){return this._nodeRowMap[bG];
},getNodeFromRow:function(bH){return this._nodeArr[this._rowArr[bH][this._treeColumn].nodeId];
},_clearSelections:function(){for(var bI in this._selections){this._nodeArr[bI].bSelected=false;
}this._selections={};
},getSelectedNodes:function(){var bJ=[];

for(var bK in this._selections){bJ.push(this._nodeArr[bK]);
}return bJ;
}},destruct:function(){this._rowArr=this._nodeArr=this._nodeRowMap=this._selections=this.__vR=null;
},defer:function(bL){bL.Type=qx.ui.treevirtual.MTreePrimitive.Type;
}});
})();
...
(function(){var l='px;',k="",j="Boolean",h='',g='px',e='</div>',d='<div style="position:absolute;',c=";",b='left:',a='">',bb='</span>',ba='background-image:url(',Y="treevirtual-start-contract",X='">&nbsp;</div>',W="treevirtual-file",V="qx.ui.treevirtual.SimpleTreeDataCellRenderer",U="qx.client",T="treevirtual-only-expand",S='style="',R='top:0;',t='background-repeat:no-repeat;',u='>',r='right:',s='<div style="',p='" title="',q="treevirtual-end",m="treevirtual-cross",n=');',x='<span',y="treevirtual-end-contract",F=';width:',D=';"',J="treevirtual-end-expand",H="treevirtual-only-contract",N="qx.dynlocale",L="treevirtual-contract",A='top:',Q="content-box",P='bottom:',O="treevirtual-start-expand",z="on",B="mshtml",C="treevirtual-cross-contract",E="treevirtual-folder",G='width:',I="treevirtual-expand",K="treevirtual-cross-expand",M="treevirtual-line",v=';height:',w='height:';
qx.Class.define(V,{extend:qx.ui.table.cellrenderer.Abstract,construct:function(){var bc=qx.ui.treevirtual.SimpleTreeDataCellRenderer;
if(bc.__xT){bc.__xY();
bc.__xT=false;
}qx.ui.table.cellrenderer.Abstract.call(this);
this.__xU=qx.util.AliasManager.getInstance();
this.__xV=qx.util.ResourceManager.getInstance();
this.__xW=qx.theme.manager.Appearance.getInstance();
this.BLANK=this.__xV.toUri(this.__xU.resolve("static/blank.gif"));
},statics:{__xX:{},__xT:true,__xY:function(){qx.theme.manager.Meta.getInstance().initialize();
var bd=qx.ui.treevirtual.SimpleTreeDataCellRenderer;
var bg=qx.io.ImageLoader;
var be=qx.util.AliasManager.getInstance();
var bh=qx.util.ResourceManager.getInstance();
var bf=qx.theme.manager.Appearance.getInstance();
var bi=function(f){bg.load(bh.toUri(be.resolve(f)));
};
bd.__xX.line=bf.styleFrom(M);
bi(bd.__xX.line.icon);
bd.__xX.contract=bf.styleFrom(L);
bi(bd.__xX.contract.icon);
bd.__xX.expand=bf.styleFrom(I);
bi(bd.__xX.expand.icon);
bd.__xX.onlyContract=bf.styleFrom(H);
bi(bd.__xX.onlyContract.icon);
bd.__xX.onlyExpand=bf.styleFrom(T);
bi(bd.__xX.onlyExpand.icon);
bd.__xX.startContract=bf.styleFrom(Y);
bi(bd.__xX.startContract.icon);
bd.__xX.startExpand=bf.styleFrom(O);
bi(bd.__xX.startExpand.icon);
bd.__xX.endContract=bf.styleFrom(y);
bi(bd.__xX.endContract.icon);
bd.__xX.endExpand=bf.styleFrom(J);
bi(bd.__xX.endExpand.icon);
bd.__xX.crossContract=bf.styleFrom(C);
bi(bd.__xX.crossContract.icon);
bd.__xX.crossExpand=bf.styleFrom(K);
bi(bd.__xX.crossExpand.icon);
bd.__xX.end=bf.styleFrom(q);
bi(bd.__xX.end.icon);
bd.__xX.cross=bf.styleFrom(m);
bi(bd.__xX.cross.icon);
}},properties:{useTreeLines:{check:j,init:true},excludeFirstLevelTreeLines:{check:j,init:false},alwaysShowOpenCloseSymbol:{check:j,init:false}},members:{__xU:null,__xW:null,__xV:null,_getCellStyle:function(bj){var bk=bj.value;
var bl=qx.ui.table.cellrenderer.Abstract.prototype._getCellStyle.call(this,bj)+(bk.cellStyle?bk.cellStyle+c:k);
return bl;
},_getContentHtml:function(bm){var bo=k;
var bp=0;
var br=this._addExtraContentBeforeIndentation(bm,bp);
bo+=br.html;
bp=br.pos;
var bq=this._addIndentation(bm,bp);
bo+=bq.html;
bp=bq.pos;
br=this._addExtraContentBeforeIcon(bm,bp);
bo+=br.html;
bp=br.pos;
var bn=this._addIcon(bm,bp);
bo+=bn.html;
bp=bn.pos;
br=this._addExtraContentBeforeLabel(bm,bp);
bo+=br.html;
bp=br.pos;
bo+=this._addLabel(bm,bp);
return bo;
},_addImage:function(bs){var bu=[];
var bt=this.__xV.toUri(this.__xU.resolve(bs.url));
if(bs.position){var bv=bs.position;
bu.push(d);

if(!qx.core.Variant.isSet(U,B)){bu.push(qx.bom.element.BoxSizing.compile(Q));
}
if(bv.top!==undefined){bu.push(A+bv.top+l);
}
if(bv.right!==undefined){bu.push(r+bv.right+l);
}
if(bv.bottom!==undefined){bu.push(P+bv.bottom+l);
}
if(bv.left!==undefined){bu.push(b+bv.left+l);
}
if(bv.width!==undefined){bu.push(G+bv.width+l);
}
if(bv.height!==undefined){bu.push(w+bv.height+l);
}bu.push(a);
}bu.push(s);
bu.push(ba+bt+n);
bu.push(t);

if(bs.imageWidth&&bs.imageHeight){bu.push(F+bs.imageWidth+g+v+bs.imageHeight+g);
}var bw=bs.tooltip;

if(bw!=null){bu.push(p+bw);
}bu.push(X);

if(bs.position){bu.push(e);
}return bu.join(k);
},_addIndentation:function(bx,by){var bz=bx.value;
var bB;
var bC=k;
var bF=this.getUseTreeLines();
var bD=this.getExcludeFirstLevelTreeLines();
var bE=this.getAlwaysShowOpenCloseSymbol();

for(var i=0;i<bz.level;i++){bB=this._getIndentSymbol(i,bz,bF,bE,bD);
var bA=bx.table.getRowHeight();
bC+=this._addImage({url:bB.icon,position:{top:0+(bB.paddingTop||0),left:by+(bB.paddingLeft||0),width:bA+3,height:bA},imageWidth:bA,imageHeight:bA});
by+=bA+3;
}return ({html:bC,pos:by});
},_addIcon:function(bG,bH){var bI=bG.value;
var bK=(bI.bSelected?bI.iconSelected:bI.icon);

if(!bK){if(bI.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.LEAF){var o=this.__xW.styleFrom(W);
}else{var bL={opened:bI.bOpened};
var o=this.__xW.styleFrom(E,bL);
}bK=o.icon;
}var bJ=bG.table.getRowHeight();
var bM=this._addImage({url:bK,position:{top:0,left:bH,width:bJ+3,height:bJ},imageWidth:bJ,imageHeight:bJ});
return ({html:bM,pos:bH+bJ+3});
},_addLabel:function(bN,bO){var bP=bN.value;
var bR=bP.label;

if(qx.core.Variant.isSet(N,z)){if(bR&&bR.translate){bR=bR.translate();
}}var bQ=d+b+bO+l+R+(bP.labelStyle?bP.labelStyle+c:k)+a+x+(bN.labelSpanStyle?S+bN.labelSpanStyle+D:k)+u+bR+bb+e;
return bQ;
},_addExtraContentBeforeIndentation:function(bS,bT){return {html:h,pos:bT};
},_addExtraContentBeforeIcon:function(bU,bV){return {html:h,pos:bV};
},_addExtraContentBeforeLabel:function(bW,bX){return {html:h,pos:bX};
},_getIndentSymbol:function(bY,ca,cb,cc,cd){var ce=qx.ui.treevirtual.SimpleTreeDataCellRenderer;
if(bY==0&&cd){cb=false;
}if(bY<ca.level-1){return (cb&&!ca.lastChild[bY]?ce.__xX.line:{icon:this.BLANK});
}var cf=ca.lastChild[ca.lastChild.length-1];
if(ca.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH&&!ca.bHideOpenClose){if(ca.children.length>0||cc){if(!cb){return (ca.bOpened?ce.__xX.contract:ce.__xX.expand);
}if(bY==0&&ca.bFirstChild){if(cf){return (ca.bOpened?ce.__xX.onlyContract:ce.__xX.onlyExpand);
}else{return (ca.bOpened?ce.__xX.startContract:ce.__xX.startExpand);
}}if(cf){return (ca.bOpened?ce.__xX.endContract:ce.__xX.endExpand);
}return (ca.bOpened?ce.__xX.crossContract:ce.__xX.crossExpand);
}}if(cb){if(ca.parentNodeId==0){if(cf&&ca.bFirstChild){return {icon:this.BLANK};
}if(cf){return ce.__xX.end;
}if(ca.bFirstChild){return (ca.bOpened?ce.__xX.startContract:ce.__xX.startExpand);
}}return (cf?ce.__xX.end:ce.__xX.cross);
}return {icon:this.BLANK};
}},destruct:function(){this.__xU=this.__xV=this.__xW=null;
}});
})();
...
this.__ya=d;
},members:{__ya:null,getTable:function(){return this.__ya;
...
case a:if(!n.bHideOpenClose){y.setState(n,{bOpened:!n.bOpened});
}return k.getOpenCloseClickSelectsRow()?false:true;
default:return true;
}}}var h=g(this.__ya,e,f);
