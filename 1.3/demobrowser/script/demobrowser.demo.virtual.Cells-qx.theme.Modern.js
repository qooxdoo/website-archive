...
(function(){var j="",i="<b>HTML</b>",h="px;",g="Juhu",f="testImageCell",e="qx.theme",d="testNumberCell",c="width:",b="testHtmlCell",a="testDateCell",w="height:",v="testStringCell",u="testBooleanCellCustomImage",t="demobrowser.demo.virtual.Cells",s="testBooleanCell",r="foobar!",q="qx.theme.Modern",p="icon/32/apps/utilities-notes.png",o="medium",n="icon/16/emotes/face-sad.png",l="icon/16/apps/internet-feed-reader.png",m="icon/16/emotes/face-smile.png",k="This is a feed reader!";
...
var x=new qx.ui.layout.HBox(5);
this.topContainer=new qx.ui.container.Composite(x);
this.runTest(v);
this.runTest(d);
this.runTest(a);
this.runTest(b);
if(qx.core.Setting.get(e)==q){this.runTest(f);
this.runTest(s);
this.runTest(u);
}this.getRoot().add(this.topContainer,{edge:5});
},runTest:function(y){this.setUp();
this[y]();
...
},testStringCell:function(){var A={value:[g,i,j,null],states:[{},{selected:1}]};
var z=new qx.ui.virtual.cell.String;
qx.util.Permutation.permute(A,function(B){var C=this.__NW(z,B.value,B.states);
this.container.add(C);
},this);
},testNumberCell:function(){var D={value:[-1.666666,0,null],format:[null,new qx.util.format.NumberFormat().set({maximumFractionDigits:2})],states:[{},{selected:1}]};
var E=new qx.ui.virtual.cell.Number();
qx.util.Permutation.permute(D,function(F){if(F.format){E.setNumberFormat(F.format);
}else{E.resetNumberFormat();
}var G=this.__NW(E,F.value,F.states);
this.container.add(G);
},this);
},testDateCell:function(){var H={value:[new Date(),null],format:[null,new qx.util.format.DateFormat(qx.locale.Date.getDateFormat(o))],states:[{},{selected:1}]};
var I=new qx.ui.virtual.cell.Date();
qx.util.Permutation.permute(H,function(J){if(J.format){I.setDateFormat(J.format);
}else{I.resetDateFormat();
}var K=this.__NW(I,J.value,J.states);
this.container.add(K);
},this);
},testHtmlCell:function(){var L={value:[g,i,j,null],states:[{},{selected:1}]};
var M=new qx.ui.virtual.cell.Html;
qx.util.Permutation.permute(L,function(N){var O=this.__NW(M,N.value,N.states);
this.container.add(O);
},this);
},testImageCell:function(){var P={value:[{url:l,tooltip:k},{url:p,tooltip:r},null],states:[{},{selected:1}]};
var Q=new qx.ui.virtual.cell.Image;
qx.util.Permutation.permute(P,function(R){var S=this.__NW(Q,R.value,R.states);
this.container.add(S);
},this);
},testBooleanCell:function(){var T={value:[true,false,null],states:[{},{selected:1}]};
var U=new qx.ui.virtual.cell.Boolean;
qx.util.Permutation.permute(T,function(V){var W=this.__NW(U,V.value,V.states);
this.container.add(W);
},this);
},testBooleanCellCustomImage:function(){var X={value:[true,false,null],states:[{},{selected:1}]};
var Y=new qx.ui.virtual.cell.Boolean;
Y.setIconFalse(n);
Y.setIconTrue(m);
qx.util.Permutation.permute(X,function(ba){var bb=this.__NW(Y,ba.value,ba.states);
this.container.add(bb);
},this);
},__NW:function(bc,bd,be){var bi=100;
var bg=30;
var bk=new qx.ui.embed.Html().set({width:bi,height:bg,decorator:be.selected?null:"main"});
var bf=bc.getCellProperties(bd,be);
var bh=bf.insets;
var bj=["<div ","style='",this._getCellSizeStyle(bi,bg,bh[0],bh[1]),bf.style,"' ","class='",bf.classes,"' ",bf.attributes,">",bf.content,"</div>"].join("");
bk.setHtml(bj);
return bk;
},_getCellSizeStyle:function(bl,bm,bn,bo){var bp=j;

if(qx.bom.client.Feature.CONTENT_BOX){bl-=bn;
bm-=bo;
}bp+=c+bl+h;
bp+=w+bm+h;
return bp;
...
(function(){var s="",r="px",q='" ',p="no-repeat",o='"',n='" style="',m='title="',l='<div style="',k='></div>',j="qx.ui.virtual.cell.AbstractImage",d="img",i="inline-block",g='/>',c="static",b="top",f='<img src="',e="abstract",h="string",a="-moz-inline-box";
...
},members:{__zN:16,__zO:16,_aliasManager:null,__zP:function(t){var w=qx.util.ResourceManager.getInstance();
var v=qx.io.ImageLoader;
var u,x;
if(w.has(t)){u=w.getImageWidth(t),x=w.getImageHeight(t);
}else if(v.isLoaded(t)){u=v.getWidth(t);
x=v.getHeight(t);
}else{u=this.__zN;
x=this.__zO;
}return {width:u,height:x};
},__zQ:function(y){if(typeof (y)==h){y={url:y};
}var z=this._aliasManager.resolve(y.url||null);
var A;

if(y.width&&y.height){A={width:y.width,height:y.height};
}else{A=this.__zP(z);
}return {width:A.width,height:A.height,url:z,tooltip:y.tooltip};
},_identifyImage:function(B){throw new Error("_identifyImage is abstract");
},getContent:function(C,D){if(C===null){return s;
}var content=s;
var F=this.__zQ(this._identifyImage(C));
var J=qx.bom.client.Engine.GECKO&&qx.bom.client.Engine.VERSION<1.9;
var G=F.tooltip?m+F.tooltip+o:s;
var E={width:F.width+r,height:F.height+r,display:J?a:i,verticalAlign:b,position:c};
var H=qx.bom.element.Decoration.getTagName(p,F.url);
var I=qx.bom.element.Decoration.getAttributes(F.url,p,E);
var K=qx.bom.element.Style.compile(I.style);

if(H===d){content=f+I.src+n+K+q;
content+=G+g;
}else{content=l+K+q;
content+=G+k;
}return content;
...
},properties:{appearance:{refine:true,init:a},iconTrue:{check:e,themeable:true,apply:b},iconFalse:{check:e,themeable:true,apply:d}},members:{__Fj:null,__Fk:null,__Fi:null,_applyIconTrue:function(f){this.__Fj=this.__Fi.resolve(f);
},_applyIconFalse:function(g){this.__Fk=this.__Fi.resolve(g);
},_identifyImage:function(h){return h==true?this.__Fj:this.__Fk;
}}});
})();
