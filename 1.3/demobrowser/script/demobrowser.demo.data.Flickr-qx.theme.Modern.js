...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.data.Flickr","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.data.Flickr-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"d24496d8cebd"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
F.addListener(C,function(){M.searchForTag(E.getValue());
},this);
E.addListener(b,function(e){if(e.getKeyIdentifier()==c){M.searchForTag(E.getValue());
}},this);
...
var G={converter:function(O,P){return (d+P.getFarm()+i+P.getServer()+j+O+f+P.getSecret()+z);
}};
K.setIconOptions(G);
K.setIconPath(t);
M.bind(a,K,l);
...
var L={converter:function(Q){if(Q){return (d+Q.getFarm()+i+Q.getServer()+j+Q.getId()+f+Q.getSecret()+D);
}return g;
}};
K.bind(A,H,B,L);
...
qx.Class.define(v,{extend:qx.data.store.Json,statics:{RESULT:null},construct:function(R){jsonFlickrApi=function(T){demobrowser.demo.data.store.Flickr.RESULT=T;
};
var S=h+R;
qx.data.store.Json.call(this,S);
},members:{searchForTag:function(U){if(U!=g){this.setUrl(h+U);
}},_createRequest:function(V){var W=new qx.io.ScriptLoader();
this.setState(n);
V+=m;
W.load(V,function(X){this.__OH();
},this);
},__OH:function(){this.setState("completed");
var Y=demobrowser.demo.data.store.Flickr.RESULT;
