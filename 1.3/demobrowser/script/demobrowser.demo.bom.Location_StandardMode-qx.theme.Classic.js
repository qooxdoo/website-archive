...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.bom.Location_StandardMode","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.bom.Location_StandardMode-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"db94f6186b43"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var n="px",m="moveable",k="borderBottomWidth",j="list",h='#',g="mode",f="borderTopWidth",e="borderLeftWidth",d="a",c="div",a="borderRightWidth",b="demobrowser.demo.bom.Location_StandardMode";
qx.Class.define(b,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);
var o=document.getElementById(j).getElementsByTagName(d);

for(var i=0,l=o.length;i<l;i++){o[i].onclick=demobrowser.demo.bom.Location_StandardMode.jump;
}}},statics:{jump:function(event){if(!event){event=window.event;
}event.returnValue=false;

if(event.preventDefault){event.preventDefault();
}function p(u,v){return parseInt(qx.bom.element.Style.get(u,v,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var r=this.href;
var t=r.substr(r.indexOf(h)+1);
var q=document.getElementById(t);
var location=qx.bom.element.Location.get(q,document.getElementById(g).value);
var s=document.getElementById(m);
s.style.left=location.left+n;
s.style.top=location.top+n;

if(qx.bom.client.Feature.BORDER_BOX){s.style.width=(location.right-location.left)+n;
s.style.height=(location.bottom-location.top)+n;
}else{s.style.width=(location.right-location.left-p(s,e)-p(s,a))+n;
s.style.height=(location.bottom-location.top-p(s,f)-p(s,k))+n;
}return false;
},randomize:function(){var w=document.body.getElementsByTagName(c);

for(var i=0,l=w.length;i<l;i++){if(w[i].id&&w[i].id!=m){w[i].style.margin=Math.round(Math.random()*10)+n;
w[i].style.padding=Math.round(Math.random()*10)+n;
w[i].style.borderWidth=Math.round(Math.random()*10)+n;
}}}}});
})();
