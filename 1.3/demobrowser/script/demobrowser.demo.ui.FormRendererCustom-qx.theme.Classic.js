...
qx.Class.define(r,{extend:qx.ui.form.renderer.Single,members:{__Pm:0,__Pn:0,addItems:function(C,D,E,F){var I=0;
if(E!=null){this._add(this._createHeader(E),{row:I,column:this.__Pm,colSpan:2});
I++;
}for(var i=0;i<C.length;i++){var H=this._createLabel(D[i],C[i]);
this._add(H,{row:I,column:this.__Pm});
var G=C[i];
H.setBuddy(G);

if(G instanceof qx.ui.form.RadioGroup){G=this._createWidgetForRadioGroup(G);
}this._add(G,{row:I,column:this.__Pm+1});
if(F[i]&&F[i].placeholder){G.setPlaceholder(F[i].placeholder);
}I++;
}this.__Pm+=2;
this.__Pn<I?this.__Pn=I:null;
...
this._add(this._buttonRow,{row:this.__Pn,column:0,colSpan:this.__Pm});
