...
var C=0;
var y=function(F){var I=[];
var H=new Date().getTime();
var J=400*24*60*60*1000;

for(var K=0;K<F;K++){var G=new Date(H+Math.random()*J-J/2);
I.push({renderer:s,location:g,data:[C++,Math.random()*10000,G]});
}return I;
};
var B=new qx.ui.progressive.renderer.table.Widths(3);
B.setWidth(0,100);
B.setWidth(1,l);
B.setWidth(2,300);
var v=[h,n,c];
var x=new qx.ui.progressive.headfoot.TableHeading(B,v);
var u=new qx.ui.progressive.headfoot.Progress(B,v);
var E=new qx.ui.progressive.structure.Default(x,u);
var t=new qx.ui.progressive.Progressive(E);
var D=new qx.ui.basic.Atom(b);
D.setTextColor(m);
D.setHeight(16);
D.setRich(true);
t.add(D);
var A=new qx.ui.progressive.model.Default();
var z=y(500);
A.addElements(z);
t.setDataModel(A);
var w=new qx.ui.progressive.renderer.table.Row(B);
t.addRenderer(s,w);
w.addRenderer(2,new demobrowser.demo.progressive.DateCellRenderer());
this.getRoot().add(t,{edge:50});
t.render();
}}});
qx.Class.define(f,{extend:qx.ui.progressive.renderer.table.cell.Default,statics:{onmouseover:function(L){L.__Ou=qx.bom.element.Style.get(L,e);
L.style.color=q;
L.parentNode.style.height=32;
L.__Ov=L.innerHTML;
var N=Date.parse(L.attributes.getNamedItem(p).nodeValue);
var M=new Date(N);
L.innerHTML=M.toUTCString()+d+M.toString();
},onmouseout:function(O){O.style.color=O.__Ou;
O.parentNode.style.height=16;
O.innerHTML=O.__Ov;
}},members:{_getCellExtras:function(P){var Q=[];
Q.push(qx.ui.progressive.renderer.table.cell.Default.prototype._getCellExtras.call(this));
Q.push(k+P.cellData.toUTCString()+j);
Q.push(o,a);
return Q.join(r);
}}});
})();
