...
}}},statics:{num:function(p,q){return parseInt(qx.bom.element.Style.get(p,q,qx.bom.element.Style.COMPUTED_MODE,false),10);
...
var v=t.substr(t.indexOf(g)+1);
var s=document.getElementById(v);
var location=qx.bom.element.Location.get(s,document.getElementById(f).value);
var u=document.getElementById(m);
u.style.left=location.left+n;
u.style.top=location.top+n;

if(qx.bom.client.Feature.BORDER_BOX){u.style.width=(location.right-location.left)+n;
u.style.height=(location.bottom-location.top)+n;
}else{u.style.width=(location.right-location.left-r(u,d)-r(u,b))+n;
u.style.height=(location.bottom-location.top-r(u,e)-r(u,j))+n;
}return false;
},randomize:function(){var y=document.body.getElementsByTagName(a);

for(var i=0,l=y.length;i<l;i++){if(y[i].id&&y[i].id!=m){y[i].style.margin=Math.round(Math.random()*10)+n;
y[i].style.padding=Math.round(Math.random()*10)+n;
y[i].style.borderWidth=Math.round(Math.random()*10)+n;
}}}}});
})();
