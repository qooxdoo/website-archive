...
this.__ML(D,C);
},__ML:function(E,F){var G=new F.clazz();
...
if(J!=null){for(var i=0;i<J.length;i++){if(G instanceof qx.ui.form.Form){this.__MM(G,J[i]);
}else{this.__ML(G,J[i]);
...
}}},__MM:function(L,M){if(M.type==="Header"){L.addGroupHeader(M.label);
return;
