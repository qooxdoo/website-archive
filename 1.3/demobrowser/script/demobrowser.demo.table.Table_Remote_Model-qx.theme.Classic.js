...
this.__FU=null;
this.__FV=null;
},properties:{blockSize:{check:c,init:50},maxCachedBlockCount:{check:c,init:15},clearCacheOnRemove:{check:b,init:false},blockConcurrentLoadRowCount:{check:b,init:true}},members:{__FM:null,__FR:null,__FN:null,__FO:null,__FP:null,__FQ:null,__FS:null,__FT:null,__FK:null,__FL:null,__FV:null,__FU:null,__FW:false,_getIgnoreCurrentRequest:function(){return this.__FR;
},getRowCount:function(){if(this.__FM==-1){if(!this.__FW||!this.getBlockConcurrentLoadRowCount()){this.__FW=true;
this._loadRowCount();
}return (this.__FM==-1)?0:this.__FM;
}else{return this.__FM;
}},_loadRowCount:function(){throw new Error("_loadRowCount is abstract");
},_onRowCountLoaded:function(f){if(this.getBlockConcurrentLoadRowCount()){this.__FW=false;
}if(f==null||f<0){f=0;
}this.__FM=Number(f);
...
if(this.__FO!=-1){var h=this._cancelCurrentRequest();
...
this.__FW=true;
this._loadRowCount();
},clearCache:function(){this.__FS={};
this.__FT=0;
},getCacheContent:function(){return {sortColumnIndex:this.__FK,sortAscending:this.__FL,rowCount:this.__FM,lruCounter:this.__FN,rowBlockCache:this.__FS,rowBlockCount:this.__FT};
},restoreCacheContent:function(j){if(this.__FO!=-1){var k=this._cancelCurrentRequest();

if(k){this.__FO=-1;
this.__FR=false;
}else{this.__FR=true;
}}this.__FK=j.sortColumnIndex;
this.__FL=j.sortAscending;
this.__FM=j.rowCount;
this.__FN=j.lruCounter;
this.__FS=j.rowBlockCache;
this.__FT=j.rowBlockCount;
var l={firstRow:0,lastRow:this.__FM-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
for(var w=0;w<=o;w++){var q=this.__FS[w];
...
if(r!=null){u[t]=r;
}}}}},prefetchRows:function(x,y){if(this.__FO==-1){var z=this.getBlockSize();
var E=Math.ceil(this.__FM/z);
...
for(var F=D;F<=C;F++){if(this.__FS[F]==null||this.__FS[F].isDirty){if(B==-1){B=F;
}A=F;
}}if(B!=-1){this.__FP=-1;
this.__FQ=-1;
this.__FO=B;
...
},_onRowDataLoaded:function(I){if(I!=null&&!this.__FR){var L=this.getBlockSize();
var J=Math.ceil(I.length/L);

if(J==1){this._setRowBlockData(this.__FO,I);
}else{for(var i=0;i<J;i++){var O=i*L;
var N=[];
var K=Math.min(L,I.length-O);

for(var P=0;P<K;P++){N.push(I[O+P]);
}this._setRowBlockData(this.__FO+i,N);
}}var M={firstRow:this.__FO*L,lastRow:(this.__FO+J+1)*L-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(e,M);
}this.__FO=-1;
this.__FR=false;
if(this.__FP!=-1){this.prefetchRows(this.__FP,this.__FQ);
}},_setRowBlockData:function(Q,R){if(this.__FS[Q]==null){this.__FT++;

while(this.__FT>this.getMaxCachedBlockCount()){var V;
var U=this.__FN;

for(var T in this.__FS){var S=this.__FS[T].lru;

if(S<U&&T>1){U=S;
V=T;
}}delete this.__FS[V];
this.__FT--;
}}this.__FS[Q]={lru:++this.__FN,rowDataArr:R};
...
for(var bf=bc;bf<=bb;bf++){var X=this.__FS[bf];
...
if(bf==bb-1){if(X.rowDataArr.length==0){delete this.__FS[bf];
}}else{var bd=this.__FS[bf+1];

if(bd!=null){X.rowDataArr.push(bd.rowDataArr[0]);
}else{X.isDirty=true;
}}}}
if(this.__FM!=-1){this.__FM--;
...
if(bi.lru!=this.__FN){bi.lru=++this.__FN;
}return bj;
...
}}},setEditable:function(bv){this.__FV=[];

for(var bw=0;bw<this.getColumnCount();bw++){this.__FV[bw]=bv;
}this.fireEvent(d);
},setColumnEditable:function(bx,by){if(by!=this.isColumnEditable(bx)){if(this.__FV==null){this.__FV=[];
}this.__FV[bx]=by;
this.fireEvent(d);
}},isColumnEditable:function(bz){return (this.__FV?(this.__FV[bz]==true):false);
},setColumnSortable:function(bA,bB){if(bB!=this.isColumnSortable(bA)){if(this.__FU==null){this.__FU=[];
}this.__FU[bA]=bB;
this.fireEvent(d);
}},isColumnSortable:function(bC){return (this.__FU?(this.__FU[bC]!==false):true);
},sortByColumn:function(bD,bE){if(this.__FK!=bD||this.__FL!=bE){this.__FK=bD;
this.__FL=bE;
this.clearCache();
this.fireEvent(d);
}},getSortColumnIndex:function(){return this.__FK;
},isSortAscending:function(){return this.__FL;
},setSortColumnIndexWithoutSortingData:function(bF){this.__FK=bF;
},setSortAscendingWithoutSortingData:function(bG){this.__FL=bG;
}},destruct:function(){this.__FU=this.__FV=this.__FS=null;
}});
})();
