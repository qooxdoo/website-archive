...
var x=v.resolve(j);
this.__ET=w.toUri(x);
},members:{__ET:null,getBlankImage:function(){return this.__ET;
},_identifyImage:function(y){throw new Error("_identifyImage() is abstract");
},_getCellStyle:function(z){var A=qx.ui.progressive.renderer.table.cell.Abstract.prototype._getCellStyle.call(this,z)+p+q;
return A;
},_getContentHtml:function(B){var C=[];
var D=this.__EU(B);
C.push(m);
if(qx.core.Variant.isSet(g,r)&&/\.png$/i.test(D.url)){C.push(s,this.__ET,e,a,D.url,d,t);
}else{C.push(s,D.url,t);
}if(D.imageWidth){C.push(h,D.imageWidth,u);
}if(D.imageHeight){C.push(l,D.imageHeight,u);
}C.push(b);
if(D.tooltip){C.push(k,D.tooltip,f);
}if(D.extras){C.push(D.extras);
}C.push(i);
return C.join(n);
},__EU:function(E){var F=this._identifyImage(E);
if(F==null||typeof F=="string"){F={url:F,tooltip:null};
}if(F.url==null){F.url=this.__ET;
}return F;
}}});
})();
...
this.__HM=J.toUri(H);
this.__HN=J.toUri(G);
},properties:{allowToggle:{check:E,init:false}},members:{__HM:null,__HN:null,__HO:null,__HP:null,__HQ:null,__HR:null,__HS:null,__HT:null,_identifyImage:function(K){var L={imageWidth:11,imageHeight:11};

switch(K.cellData){case true:L.url=this.__HM;
L.extras=w;
break;
case false:L.url=this.__HN;
L.extras=p;
break;
default:L.url=null;
break;
}
if(this.getAllowToggle()){L.extras+=n+B+y+l+o+e;

if(qx.core.Variant.isSet(g,c)&&/\.png$/i.test(this.__HM)){L.extras+=i+this.getBlankImage()+j+d+h+a+this.__HM+j+b;
}else{L.extras+=i+this.__HM+j;
}L.extras+=m+v+u+e;

if(qx.core.Variant.isSet(g,c)&&/\.png$/i.test(this.__HN)){L.extras+=i+this.getBlankImage()+j+d+h+a+this.__HN+j+b;
}else{L.extras+=i+this.__HN+j;
}L.extras+=q+f;
L.extras+=s+r+C+t+k+D+f+F;
}return L;
