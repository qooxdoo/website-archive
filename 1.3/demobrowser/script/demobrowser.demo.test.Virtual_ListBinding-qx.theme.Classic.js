...
(function(){var l="row-layer",k="selected",j="grid-lines",h="Boolean",g="Integer",f="cell.row",d="cell.renderer",c="_applyDelegate",b="qx.ui.virtual.form.List",a="Object",A="_applyCellRenderer",z="__EI",y="changeRowHeight",x="_applyRowCount",w="changeCellRenderer",v="_applyShowGridLines",u="resize",t="changeShowGridLines",s="changeDelegate",r="_applyUseWidgetCells",p="changeRowCount",q="_applyRowHeight",n="horizontal",o="virtual-list",m="__EH";
...
this.initRowHeight();
this.initDelegate();
},properties:{appearance:{refine:true,init:o},rowCount:{check:g,event:p,init:0,apply:x},rowHeight:{check:g,event:y,init:20,apply:q,themeable:true},showGridLines:{check:h,event:t,init:false,apply:v,themeable:true},useWidgetCells:{check:h,init:false,apply:r,themeable:true},cellRenderer:{event:w,apply:A,themeable:true},delegate:{check:a,event:s,init:null,nullable:true,apply:c}},members:{__EG:null,__EH:null,__EI:null,__EJ:null,_initWidgetLayer:function(){var self=this;
var B={getCellWidget:function(C,D){var H=self._getCellData(C);

if(!H){return null;
}var E={};

if(self.__EH.isItemSelected(C)){E.selected=true;
...
this.__EI=new qx.ui.virtual.layer.WidgetCell(B);
...
if(self.__EH.isItemSelected(L)){N.selected=true;
}return self._getCellRenderer(L).getCellProperties(self._getCellData(L),N);
}};
this._showChildControl(l);
this.__EI=new qx.ui.virtual.layer.HtmlCell(K);
this.getPane().addLayer(this.__EI);
...
},styleSelectable:function(Q,R,S){if(self.__EJ){self._styleWidgetSelectable(Q,R,S);
}else{self._styleHtmlSelectable(Q,R,S);
}}};
this.__EH=new qx.ui.virtual.selection.Row(this.getPane(),O);
...
},_applyUseWidgetCells:function(bf,bg){if(this.__EJ!==null){throw new Error("The property 'useWidgetCells' cannot be set after the list has "+"been rendered.");
}},_applyCellRenderer:function(bh,bi){this.__EG=bh;
...
},_getCellRenderer:function(bk){return this._delegate.getCellRenderer?this._delegate.getCellRenderer(bk):this.__EG;
...
if(this.__Fy.length==0){return;
}var C=this._getDelegate(u);

if(C!=null){this.__Fy.sort(function(a,b){return C(B.getItem(a),B.getItem(b));
});
}},_runDelegateFilter:function(D){if(typeof D!==g){return;
}var E=this._getDelegate(s);

for(var i=0,l=D.length;i<l;++i){if(E==null||E(D.getItem(i))){this.__Fy.push(i);
...
},__FG:function(L){return this.__Fy[L];
},_applyDelegate:function(M,N){if(this.getTarget()==null||this.getModel()==null){return;
}
if(this._containsDelegateMethod(M)){this.__FF();
}},_applyTarget:function(O,P){if(O!=null){O.setDelegate(this);
this.__FC=O.getSelectionManager().addListener("changeSelection",this._onChangeSelectionView,this);
}
if(P!=null){P.setDelegate(null);
P.getSelectionManager().removeListenerById(this.__FC);
}
if(this.getModel()==null){return;
}this.__FF();
this._syncRowCount();
},_applyModel:function(Q,R){if(R!=null){R.removeListenerById(this.__Fz);
R.removeListenerById(this.__FA);
R.removeListenerById(this.__FB);
}
if(Q!=null){this.__FF();
this.__Fz=Q.addListener(d,this._onChangeLengthModel,this);
this.__FA=Q.addListener("change",this._onChangeModel,this);
this.__FB=Q.addListener("changeBubble",this._onChangeBubbleModel,this);
}
if(this.getTarget()!=null){this._syncRowCount();
}},_applySelection:function(S,T){if(S!=null){this.__FD=S.addListener("change",this._onChangeSelectionModel,this);
this.__FE=S.addListener("changeLength",this._onChangeSelectionModel,this);
this._syncModelSelectionToView();
}
if(T!=null){T.removeListenerById(this.__FD);
T.removeListenerById(this.__FE);
...
},_onChangeLengthModel:function(e){this.__FF();
this._syncRowCount();
