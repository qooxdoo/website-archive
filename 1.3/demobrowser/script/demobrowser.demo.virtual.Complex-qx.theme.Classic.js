...
this.__Oj=[];
},members:{_pool:null,__Oj:null,getCellData:function(s,t){if(!this.__Oj[s]){this.__Oj[s]=[];
}
if(!this.__Oj[s][t]){this.__Oj[s][t]={label:this.__Ok(),icon:this.__Ol()};
}return this.__Oj[s][t];
},getCellWidget:function(u,v){var w;

if(v%2==0){w=this._pool.atom.pop();

if(!w){w=new qx.ui.basic.Atom();
w.addListener(c,function(){var x=this.__Ol();
w.setIcon(x);
this.__Oj[u][v].icon=x;
},this);
...
}},__Ok:function(){var name=m;

for(var j=0;j<10;j++){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);
}return name;
},__Ol:function(){var z=n;
var B=k;
var C=[l,g,h,b,d];
var A=Math.floor(Math.random()*4);
return (z+16+e+B+C[A]);
}},destruct:function(){this._pool=this.__Oj=null;
}});
})();
