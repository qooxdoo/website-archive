...
var settings = {"qx.application":"portal.Application","qx.theme":"qx.theme.Modern","qx.version":"1.3"};
...
var libinfo = {"__out__":{"sourceUri":"script"},"portal":{"resourceUri":"resource","sourceUri":"script","version":"trunk"},"qx":{"resourceUri":"resource","sourceUri":"script","version":"1.3"}};
...
this.__sx=[];
this.__sy={};
},statics:{dataKeyRoot:h,dataKeyGroupBoxes:m,dataKeyBoxes:d},events:{"loaded":e},properties:{activeBox:{init:null,check:g,apply:c}},members:{__sw:null,__sx:null,__sy:null,_applyActiveBox:function(n,o){if(o!=null){o.setActive(false);
}n.setActive(true);
},load:function(){this.__sz(window[portal.box.Manager.dataKeyRoot]);
qx.event.Registration.fireEvent(this,b);
},__sz:function(p){var t,w,u,v,x,p;
...
this.__sw.push(x);
this.__sy[w].push(x);
}}},getBoxes:function(){return this.__sw;
},getGroupBoxes:function(){return this.__sx;
},updateGroupBoxMembers:function(y,z,A){var B=qx.dom.Hierarchy.getElementIndex(A.getElement());
qx.lang.Array.remove(this.__sy[z],A);
qx.lang.Array.insertAt(this.__sy[y],A,B);
},getGroupBoxDataOfBox:function(C){for(var i=0,j=this.__sx.length,D;i<j;i++){D=this.__sy[this.__sx[i].id];

for(var k=0,l=D.length;k<l;k++){if(D[k].getId()===C){return this.__sx[i];
}}}return null;
}},destruct:function(){this.__sw=this.__sx=this.__sy=null;
}});
})();
...
this.__sC=new portal.box.Resizable(this,x);
}if(t.draggable){this.setDraggable(true);
this.__sD=new portal.box.Draggable(this);
}this.__sF();
},properties:{element:{init:null},groupBoxId:{init:null,check:h},active:{check:s,init:false,apply:j},resizable:{check:s,init:false},draggable:{check:s,init:false}},members:{__sB:null,__sA:null,__sD:null,__sC:null,_applyActive:function(y,z){if(y){qx.bom.element.Style.set(this.getElement(),q,a);
}else{qx.bom.element.Style.set(this.getElement(),q,n);
}},getId:function(){return this.getElement()?this.getElement().id:null;
},getData:function(){return this.__sA;
},getDraggableComponent:function(){return this.__sD;
},getResizableComponent:function(){return this.__sC;
},__sE:function(A,B){if(B){for(var C in B){qx.bom.element.Style.set(C,B[C]);
}}qx.bom.element.Style.set(A,l,b);
qx.bom.element.Style.set(A,k,0);
qx.bom.Element.addListener(A,o,this.__sG,this,true);
},__sF:function(){var E=f+g;
E=E+r+E+r+E;
var D=qx.bom.Element.create(d,{"class":i});
D.innerHTML=E;
qx.dom.Element.insertEnd(D,this.getElement());
},__sG:function(e){portal.box.Manager.getInstance().setActiveBox(this);
}},destruct:function(){this.resetElement();
this.__sA=this.__sC=this.__sD=null;
}});
})();
...
this.__sH=v;
this.__sI=this.__sH.getElement();
this.__sJ={};
this.__sK=false;
this.__sL={orientation:null,lastCoord:{left:null,top:null}};
this.__sM={minWidth:10,minHeight:10,maxWidth:null,maxHeight:null,autoHide:true,handles:3};

for(var name in this.__sM){if(w[name]){this.__sM[name]=w[name];
}}this.__sN();
},members:{__sL:null,__sH:null,__sI:null,__sJ:null,__sM:null,__sK:false,getBox:function(){return this.__sH;
},__sN:function(){switch(this.__sM.handles){case 1:this.__sJ.east=null;
break;
case 2:this.__sJ.south=null;
break;
case 3:this.__sJ.south=null;
this.__sJ.southeast=null;
this.__sJ.east=null;
break;
}for(var y in this.__sJ){var x=this.__sJ[y]=qx.bom.Element.create(f);
qx.bom.element.Attribute.set(x,o,y);
qx.dom.Element.insertEnd(x,this.__sI);

if(this.__sI.id){x.id=this.__sI.id+h+y;
}this.__sO(y);
this.__sP(y);
}},__sO:function(z){qx.bom.element.Class.add(this.__sJ[z],z+j);
qx.bom.element.Style.set(this.__sJ[z],u,this.__sM.autoHide?t:q);
},__sP:function(A){var C=qx.event.Registration;
var B=this.__sJ[A];
C.addListener(B,i,this.__sS,this);
C.addListener(document,m,this.__sT,this);

if(this.__sM.autoHide){C.addListener(this.__sH.getElement(),s,this.__sQ,this);
C.addListener(this.__sH.getElement(),r,this.__sR,this);
}},__sQ:function(e){for(var D in this.__sJ){qx.bom.element.Style.set(this.__sJ[D],u,q);
}},__sR:function(e){if(!this.__sK){for(var E in this.__sJ){qx.bom.element.Style.set(this.__sJ[E],u,t);
}}},__sS:function(e){e.stopPropagation();
if(e.isLeftPressed()){qx.event.Registration.addListener(document,p,this.__sU,this,true);
this.__sK=true;
var F=e.getTarget();
this.__sL.orientation=qx.bom.element.Attribute.get(F,o);
this.__sL.lastCoord.left=e.getDocumentLeft();
this.__sL.lastCoord.top=e.getDocumentTop();
portal.box.Util.bringToFront(this.__sI);
}},__sT:function(e){e.stopPropagation();

if(this.__sK){qx.event.Registration.removeListener(document,p,this.__sU,this,true);
portal.box.Util.sendToBack(this.__sI);

if(this.__sM.autoHide){for(var G in this.__sJ){qx.bom.element.Style.set(this.__sJ[G],u,t);
}}this.__sK=false;
qx.bom.Selection.clear(document.body);
}},__sU:function(e){e.stopPropagation();

if(this.__sK){var H={left:null,top:null};

switch(this.__sL.orientation){case n:H.top=e.getDocumentTop();
break;
...
break;
}this.__sV(H,this.__sL.lastCoord);
this.__sL.lastCoord.left=e.getDocumentLeft();
this.__sL.lastCoord.top=e.getDocumentTop();
}},__sV:function(I,J){var Q=qx.bom.element.Dimension;
var N=portal.box.Manager.getInstance().getGroupBoxDataOfBox(this.__sH.getId());
var P=Q.getContentSize(N.element);

if(I.left){var M=I.left-J.left;

if(M!==0){var L=Q.getContentWidth(this.__sI)+M;
var O=Q.getWidth(this.__sI)+M;

if((this.__sM.minWidth==null||L>this.__sM.minWidth)&&(this.__sM.maxWidth==null||L<=this.__sM.maxWidth)){if(P.width>=O){qx.bom.element.Style.set(this.__sI,k,L+d);
this.__sH.getData().width=L;
}}}}
if(I.top){var M=I.top-J.top;

if(M!==0){var K=Q.getContentHeight(this.__sI)+M;

if((this.__sM.minHeight==null||K>this.__sM.minHeight)&&(this.__sM.maxHeight==null||K<=this.__sM.maxHeight)){qx.bom.element.Style.set(this.__sI,c,K+b);
}}}}},destruct:function(){if(this.__sM.autoHide){qx.event.Registration.removeListener(this.__sH.getElement(),s,this.__sQ,this);
qx.event.Registration.removeListener(this.__sH.getElement(),r,this.__sR,this);
}this.__sH=this.__sM=this.__sI=this.__sJ=null;
this.__sL=null;
}});
})();
...
qx.Class.define(b,{statics:{__sY:1000,bringToFront:function(d){var e=qx.bom.element.Style.get(d,c);
e=e==a?0:e;
qx.bom.element.Style.set(d,c,e+this.__sY);
},sendToBack:function(f){var g=qx.bom.element.Style.get(f,c);
qx.bom.element.Style.set(f,c,g-this.__sY);
}}});
})();
...
this.__ta=p;
this.__tb=p.getElement();
this.__tc=null;
this.__td=null;
this.__te();
},members:{__ta:null,__tb:null,__tc:null,__td:null,getBox:function(){return this.__ta;
},getElement:function(){return this.__tb;
},__te:function(){this.__tf();
this.__tg();
},__tf:function(){this.__tc=qx.bom.Element.create(g);
qx.bom.element.Class.add(this.__tc,b);
qx.dom.Element.insertBegin(this.__tc,this.__tb);
},__tg:function(){qx.bom.Element.addListener(this.__tc,l,this.__th,this);
qx.bom.Element.addListener(this.__tc,j,function(e){qx.bom.element.Style.set(this,c,f);
},this.__tc);
qx.bom.Element.addListener(this.__tc,m,this.__tk,this);
qx.bom.Element.addListener(this.__tc,h,portal.dragdrop.Manager.getInstance().stopSession,portal.dragdrop.Manager.getInstance());
qx.bom.Element.addListener(this.__tc,i,this.__tl,this);
},__th:function(e){if(e.isLeftPressed()){this.__td={left:e.getDocumentLeft()-qx.bom.element.Location.getLeft(this.__tb),top:e.getDocumentTop()-qx.bom.element.Location.getTop(this.__tb)};
qx.bom.Element.addListener(document.body,n,this.__tj,this,true);
this.__ti();
qx.event.Registration.fireEvent(this.__tc,m,qx.event.type.Event);
}},__ti:qx.core.Variant.select(a,{"mshtml":function(){var r=this;
var q=qx.lang.Function.bind(this.__tj,r);
document.getElementsByTagName(d)[0].onmouseleave=q;
},"default":qx.lang.Function.empty}),__tj:function(e){try{e.stopPropagation();
}catch(s){}
if(portal.dragdrop.Manager.getInstance().isSessionActive()){qx.event.Registration.removeListener(document.body,o,this.__tl,this,true);
portal.dragdrop.Manager.getInstance().stopSession();
}qx.bom.Element.removeListener(document.body,n,this.__tj,this,true);
},__tk:function(e){portal.box.Manager.getInstance().setActiveBox(this.__ta);
portal.dragdrop.Manager.getInstance().startSession(this.__ta);
qx.event.Registration.addListener(document.body,o,this.__tl,this,true);
},__tl:function(e){e.stopPropagation();
var t=e.getDocumentLeft()-this.__td.left;
var top=e.getDocumentTop()-this.__td.top;
...
}},destruct:function(){this.__ta=this.__tc=this.__tb=this.__td=null;
}});
})();
...
this.__tm={top:null,height:null};
this.__tn={top:0,left:0};
},properties:{activeBox:{check:u,init:null,nullable:true},sessionActive:{check:r,init:false}},members:{__tm:null,__to:null,__tp:false,__tn:null,__tq:null,__tr:null,startSession:function(y){this.setSessionActive(true);
this.setActiveBox(y);
this.__to=y.getGroupBoxId();
this.__ts();
if(this.__tq==null){this.__tq={};
var z=portal.box.Manager.getInstance().getGroupBoxes();

for(var i=0,j=z.length;i<j;i++){this.__tq[z[i].element.id]={left:qx.bom.element.Location.getLeft(z[i].element),right:qx.bom.element.Location.getLeft(z[i].element)+qx.bom.element.Dimension.getWidth(z[i].element)};
}}},__ts:function(){if(!this.__tr){this.__tr=qx.bom.Element.create(b);
}var C=this.getActiveBox().getElement();
var A=qx.bom.element.Dimension.getContentSize(C);
this.__tt(C,this.__tr);
qx.bom.element.Class.add(this.__tr,qx.bom.element.Class.get(C));
var B=k+d+qx.bom.element.Location.getTop(C)+w+c+qx.bom.element.Location.getLeft(C)+w+o+A.width+w+f+A.height+x;
qx.bom.element.Style.setCss(this.__tr,B);
qx.bom.element.Style.set(C,v,t);
qx.dom.Node.getBodyElement(C).appendChild(this.__tr);
portal.box.Util.bringToFront(this.__tr);
},__tt:function(D,E){while(D.firstChild){E.appendChild(D.firstChild);
}},stopSession:function(e){this.setSessionActive(false);
var G=this.getActiveBox();
var H=G.getElement();
portal.box.Manager.getInstance().updateGroupBoxMembers(G.getGroupBoxId(),this.__to,G);
G.setGroupBoxId(this.__to);
var F=new qx.fx.effect.core.Move(this.__tr);
F.set({x:qx.bom.element.Location.getLeft(H),y:qx.bom.element.Location.getTop(H),mode:h,duration:0.5,transition:g});
F.start();
F.addListener(n,function(){this.__tt(this.__tr,H);
qx.bom.element.Style.reset(H,v);
this.__tr.parentNode.removeChild(this.__tr);

if(G.isActive()){G._applyActive(true);
}else{portal.box.Manager.getInstance().setActiveBox(G);
}},this);
},onDragMove:function(top,I){if(this.__tn.left!=I){qx.bom.element.Style.set(this.__tr,q,I+x);
}
if(this.__tn.top!=top){qx.bom.element.Style.set(this.__tr,s,top+x);
}var J=this.getActiveBox().getElement();

if(this.__tm.top==null){this.__tm.top=qx.bom.element.Location.getTop(J);
}
if(this.__tm.height==null){this.__tm.height=qx.bom.element.Dimension.getHeight(J);
}if(this.__tp==false&&top>=this.__tm.top&&top<=(this.__tm.top+this.__tm.height)){return;
}var K;
if(this.__tp){K=qx.dom.Hierarchy.getFirstDescendant(document.getElementById(this.__to));
while(K!=null){if(this.__tu(true,top,J,K)){this.__tp=false;
return;
}K=qx.dom.Hierarchy.getNextElementSibling(K);
}}else{K=J;
}if(top-this.__tn.top>0){K=qx.dom.Hierarchy.getNextElementSibling(K);

while(K!=null){if(K){if(this.__tu(true,top,J,K)){return;
}}K=qx.dom.Hierarchy.getNextElementSibling(K);
}}else{var K=qx.dom.Hierarchy.getPreviousElementSibling(K);

while(K!=null){if(K){if(this.__tu(false,top,J,K)){return;
}}K=qx.dom.Hierarchy.getPreviousElementSibling(K);
}}this.__tn.left=I;
this.__tn.top=top;
},__tu:function(L,top,M,N){var O=qx.bom.element.Location.getTop(N);
...
this.__tm.top=qx.bom.element.Location.getTop(M);
}return true;
...
},checkGroupBox:function(T,U){if(this.__tq[this.__to].left<=T&&this.__tq[this.__to].right>=T){return;
}
for(var V in this.__tq){if(V!=this.__to){if(this.__tq[V].left<=T+U&&this.__tq[V].right>=T){this.__to=V;
this.__tp=true;
return;
}}}}},destruct:function(){this.__tm=this.__tn=this.__to=null;
this.__tv=this.__tr=this.__tq=null;
}});
})();
