...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.event.Event.8ab966cce635.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="-",l="PageUp",k="Escape",j="qx.event.type.Data",h="_applyShortcut",g="PrintScreen",f="NumLock",d="5",c="8",b="execute",S="Meta",R="0",Q="2",P="Shift",O="You can only specify one non modifier key!",N="3",M="/",L="Delete",K="String",J="changeEnabled",t="*",u="qx.bom.Shortcut",r="6",s="4",p="1",q="Alt",n="Not a valid key name for a shortcut: ",o="PageDown",v="Whitespaces are not allowed within shortcuts",w="_applyEnabled",B="7",A="a",D="z",C="9",F="Boolean",E="+",y="short",I="keydown",H="",G="Control",x="keypress",z="Unidentified";qx.Class.define(u,{extend:qx.core.Object,construct:function(T){qx.core.Object.call(this);this.__zC={};this.__zD=null;if(T!=null){this.setShortcut(T);}
;this.initEnabled();}
,events:{"execute":j},properties:{enabled:{init:true,check:F,event:J,apply:w},shortcut:{check:K,apply:h,nullable:true},autoRepeat:{check:F,init:false}},members:{__zC:H,__zD:H,execute:function(U){this.fireDataEvent(b,U);}
,__zE:function(event){if(this.getEnabled()&&this.__zF(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,__gW:function(event){if(this.getEnabled()&&this.__zF(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,_applyEnabled:function(V,W){if(V){qx.event.Registration.addListener(document.documentElement,I,this.__zE,this);qx.event.Registration.addListener(document.documentElement,x,this.__gW,this);}
else {qx.event.Registration.removeListener(document.documentElement,I,this.__zE,this);qx.event.Registration.removeListener(document.documentElement,x,this.__gW,this);}
;}
,_applyShortcut:function(X,Y){if(X){if(X.search(/[\s]+/)!=-1){var bc=v;this.error(bc);throw new Error(bc);}
;this.__zC={"Control":false,"Shift":false,"Meta":false,"Alt":false};this.__zD=null;var ba;var a=[];while(X.length>0&&ba!=-1){ba=X.search(/[-+]+/);a.push((X.length==1||ba==-1)?X:X.substring(0,ba));X=X.substring(ba+1);}
;var bb=a.length;for(var i=0;i<bb;i++){var bd=this.__zH(a[i]);switch(bd){case G:case P:case S:case q:this.__zC[bd]=true;break;case z:var bc=n+a[i];this.error(bc);throw bc;default:if(this.__zD){var bc=O;this.error(bc);throw bc;}
;this.__zD=bd;};}
;}
;return true;}
,__zF:function(e){var be=this.__zD;if(!be){return false;}
;if((!this.__zC.Shift&&e.isShiftPressed())||(this.__zC.Shift&&!e.isShiftPressed())||(!this.__zC.Control&&e.isCtrlPressed())||(this.__zC.Control&&!e.isCtrlPressed())||(!this.__zC.Meta&&e.isMetaPressed())||(this.__zC.Meta&&!e.isMetaPressed())||(!this.__zC.Alt&&e.isAltPressed())||(this.__zC.Alt&&!e.isAltPressed())){return false;}
;if(be==e.getKeyIdentifier()){return true;}
;return false;}
,__zG:{esc:k,ctrl:G,print:g,del:L,pageup:l,pagedown:o,numlock:f,numpad_0:R,numpad_1:p,numpad_2:Q,numpad_3:N,numpad_4:s,numpad_5:d,numpad_6:r,numpad_7:B,numpad_8:c,numpad_9:C,numpad_divide:M,numpad_multiply:t,numpad_minus:m,numpad_plus:E},__zH:function(bf){var bg=qx.event.util.Keyboard;var bh=z;if(bg.isValidKeyIdentifier(bf)){return bf;}
;if(bf.length==1&&bf>=A&&bf<=D){return bf.toUpperCase();}
;bf=bf.toLowerCase();var bh=this.__zG[bf]||qx.lang.String.firstUp(bf);if(bg.isValidKeyIdentifier(bh)){return bh;}
else {return z;}
;}
,toString:function(){var bk=this.__zD;var bj=[];for(var bi in this.__zC){if(this.__zC[bi]){bj.push(qx.locale.Key.getKeyName(y,bi));}
;}
;if(bk){bj.push(qx.locale.Key.getKeyName(y,bk));}
;return bj.join(E);}
},destruct:function(){this.setEnabled(false);this.__zC=this.__zD=null;}
});}
)();
