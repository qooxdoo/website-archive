...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Input.fc4abc827451.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var q="val1",p="text",o="file",n="val2",m="demobrowser.demo.bom.Input",l="textarea",k="checkbox",j="radio1",h="input",g="radio",c="option",f="select",d="VAL",b="Value ",a="change";qx.Class.define(m,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var y=qx.bom.Input.create(p);qx.event.Registration.addListener(y,h,function(e){this.debug("Input (Single): "+e.getData());}
,this);qx.event.Registration.addListener(y,a,function(e){this.debug("Change (Single): "+e.getData());}
,this);var t=qx.bom.Input.create(l);qx.event.Registration.addListener(t,h,function(e){this.debug("Input (Multi): "+e.getData());}
,this);qx.event.Registration.addListener(t,a,function(e){this.debug("Change (Multi): "+e.getData());}
,this);document.body.appendChild(y);document.body.appendChild(t);var x=qx.bom.Input.create(k);var B=qx.bom.Input.create(k,{checked:true});qx.event.Registration.addListener(x,a,function(e){this.debug("Changed checkbox 1: "+e.getData());}
,this);qx.event.Registration.addListener(B,a,function(e){this.debug("Changed checkbox 2: "+e.getData());}
,this);document.body.appendChild(x);document.body.appendChild(B);var A=qx.bom.Input.create(g,{name:j,value:q,checked:true});var C=qx.bom.Input.create(g,{name:j,value:n});function s(e){this.debug("Changed radio: "+e.getTarget().name+" = "+e.getData());}
;qx.event.Registration.addListener(A,a,s,this);qx.event.Registration.addListener(C,a,s,this);document.body.appendChild(A);document.body.appendChild(C);var r=qx.bom.Input.create(f);var z;for(var i=0;i<10;i++){z=document.createElement(c);z.value=d+i;z.appendChild(document.createTextNode(b+i));r.appendChild(z);}
;qx.event.Registration.addListener(r,a,function(e){this.debug("Changed selectbox1: "+e.getData());}
,this);document.body.appendChild(r);var v=qx.bom.Input.create(f,{size:3});var z;for(var i=0;i<10;i++){z=document.createElement(c);z.value=d+i;z.appendChild(document.createTextNode(b+i));v.appendChild(z);}
;qx.event.Registration.addListener(v,a,function(e){this.debug("Changed selectbox2: "+e.getData());}
,this);document.body.appendChild(v);var w=qx.bom.Input.create(f,{size:3,multiple:true});var z;for(var i=0;i<10;i++){z=document.createElement(c);z.value=d+i;z.appendChild(document.createTextNode(b+i));w.appendChild(z);}
;qx.event.Registration.addListener(w,a,function(e){this.debug("Changed selectbox3: "+e.getData());}
,this);document.body.appendChild(w);var u=qx.bom.Input.create(o);qx.event.Registration.addListener(u,a,function(e){this.debug("Changed file select: "+e.getData());}
,this);document.body.appendChild(u);}
}});}
)();
(function(){var k="text",j="Unsupported input type.",h="nowrap",g="radio",f="qx.debug",e="input",d="option",c="value",b="number",a="qx.bom.Input",x="normal",w="mshtml",v="wrap",u="checkbox",t="select-one",s="textarea",r="auto",q="soft",p="off",o="engine.name",m="select",n="";qx.Bootstrap.define(a,{statics:{__ij:{text:1,textarea:1,select:1,checkbox:1,radio:1,password:1,hidden:1,submit:1,image:1,file:1,search:1,reset:1,button:1},create:function(y,z,A){if(qx.core.Environment.get(f)){qx.core.Assert.assertKeyInMap(y,this.__ij,j);}
