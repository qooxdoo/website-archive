...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Element_1.7a83536bace9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="after2",k="text",j="hello",h="el3b",g="height",f="sans-serif",e="<b>hello</b>",d="color",c="demobrowser.demo.test.Element_1",b="18px",B="blue",A="before2",z="el2b",y="html",x="el2a",w="width",v="after1",u="test",t="el3a",s="bold",q="doc",r="before1",o="el1",p="100px",n="id";qx.Class.define(c,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var D=new qx.html.Root(document.getElementById(u));D.setAttribute(n,q);this.info("Basics");var L=new qx.html.Element;var I=new qx.html.Element;var K=new qx.html.Element;var E=new qx.html.Element;var C=new qx.html.Element;L.setAttribute(n,o);I.setAttribute(n,x);K.setAttribute(n,z);E.setAttribute(n,t);C.setAttribute(n,h);L.add(I,K);I.add(E,C);D.add(L);E.setAttribute(y,e);C.setAttribute(k,j);C.setStyles({fontWeight:s,fontSize:b,fontFamily:f});L.setStyle(d,B);L.setStyle(w,p).setStyle(g,p);qx.html.Element.flush();this.info("Positioned Insert #1");var M=new qx.html.Element;M.setAttribute(n,r);M.insertBefore(L);var H=new qx.html.Element;H.setAttribute(n,v);H.insertAfter(L);qx.html.Element.flush();this.info("Positioned Insert #2");var N=new qx.html.Element;N.setAttribute(n,A);D.addAt(N,0);var J=new qx.html.Element;J.setAttribute(n,m);D.addAt(J,10);qx.html.Element.flush();this.info("Move");N.moveAfter(M);qx.html.Element.flush();J.moveBefore(H);qx.html.Element.flush();this.info("Switch");var F=D.indexOf(N);var G=D.indexOf(J);N.moveTo(G);J.moveTo(F);qx.html.Element.flush();this.info("Remove");D.remove(N);D.remove(J);qx.html.Element.flush();this.info("Re-add");D.add(N,J);qx.html.Element.flush();this.info("Remove and Add and Remove and Add");D.remove(N,J);D.add(N,J);D.remove(N,J);D.add(N,J);qx.html.Element.flush();this.info("Move, remove & add");D.remove(N);M.moveAfter(H);N.insertBefore(M);qx.html.Element.flush();this.info("Sort completely");var a=D.getChildren();for(var i=1,l=a.length;i<l;i++){a[i].moveTo(0);}
;qx.html.Element.flush();this.info("Rotation");M.moveAfter(N);qx.html.Element.flush();}
}});}
)();
