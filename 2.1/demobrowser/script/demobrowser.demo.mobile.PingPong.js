...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.mobile.PingPong.12edf5e26ef5.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="#333",j="border-left",i="green",h="demobrowser.demo.mobile.PingPong",g="0",f="qx.debug",d="interval",c="0px",b="12px",a="15em",L="center",K='Lucida Grande',J="Arial",I="<b>This demo is supposed to be run in a WebKit-based browser.</b>",H="engine.name",G="webkit",F="border-right",E="touchmove",D="black",C="200px",r="30px",s="100px",p="1px solid white",q="top",n="right",o="50%",l="none",m="white",t="px",u="innerHTML",w="20px",v="absolute",A="100%",z="div",B="left";qx.Class.define(h,{extend:qx.application.Native,members:{__QX:null,__QY:null,__Ra:null,__Rb:null,__Rc:5,__Rd:1,__Re:1,__Rf:null,__Rg:null,__Rh:null,main:function(){qx.application.Native.prototype.main.call(this);if(qx.core.Environment.get(f)){qx.log.appender.Native;qx.log.appender.Console;}
;var O={"width":A,"height":A,"backgroundColor":D,"margin":c,"userSelect":l};var R=new qx.html.Element(z,O);R.useElement(document.body);R.setRoot(true);if(qx.core.Environment.get(H)!=G){var Q={"color":i,"font-family":K,"font-size":b,"position":v,"left":r,"top":w};var P=new qx.html.Element(z,Q);R.add(P);P.setAttribute(u,I);return;}
;var M=this.__Rl(B);R.add(M);var N=this.__Rl(n);R.add(N);this.__QY=this.__Rm(B);M.add(this.__QY);M.addListener(E,qx.lang.Function.bind(this.__oo,this,this.__QY),this);this.__QX=this.__Rm(n);N.add(this.__QX);N.addListener(E,qx.lang.Function.bind(this.__oo,this,this.__QX),this);this.__Ra=this.__Rn();R.add(this.__Ra);this.__Rg=this.__Ro();M.add(this.__Rg);this.__Rh=this.__Ro();N.add(this.__Rh);this.__Rf=[0,0];this.__Ri();}
,__Ri:function(){this.__Ra.setStyles({"left":C,"top":C});this.__Rb=new qx.event.Timer(40);this.__Rb.addListener(d,function(){var x=(parseInt(this.__Ra.getStyle(B))+this.__Rc*this.__Rd);var y=(parseInt(this.__Ra.getStyle(q))+this.__Rc*this.__Re);this.__Ra.setStyle(B,x+t);this.__Ra.setStyle(q,y+t);this.__Rk();}
,this);this.__Rb.start();}
,__Rj:function(S){this.__Rb.stop();this.__Rb.dispose();this.__Rc=5;this.__Rf[S]=this.__Rf[S]+1;this.__Rg.setAttribute(u,this.__Rf[0]);this.__Rh.setAttribute(u,this.__Rf[1]);this.__Ri();}
,__Rk:function(){var U=this.getBoundsFor(this.__Ra);if(U.top<=0){this.__Re=1;}
;if(U.bottom>=window.innerHeight-5){this.__Re=-1;}
;if(U.left<=0||U.right>=window.innerWidth){this.__Rj(U.left<=0?1:0);}
;var V=this.getBoundsFor(this.__QY);if(U.left<=V.right&&U.bottom>=V.top&&U.top<=V.bottom){this.__Rd=1;this.__Rc=Math.min(this.__Rc+1,20);}
;var T=this.getBoundsFor(this.__QX);if(U.right>=T.left&&U.bottom>=T.top&&U.top<=T.bottom){this.__Rd=-1;this.__Rc=Math.min(this.__Rc+1,20);}
;}
,__oo:function(W,e){W.setStyles({"top":(e.getDocumentTop()-50)+t});}
,getBoundsFor:function(X){return qx.bom.element.Location.get(X.getDomElement());}
,__Rl:function(Y){var ba={"width":o,"height":A,"position":v,"backgroundColor":D};if(Y==B){ba[F]=p;}
else {ba[B]=o;ba[j]=p;}
;return new qx.html.Element(z,ba);}
,__Rm:function(bb){var bc={"width":r,"height":s,"top":s,"position":v,"backgroundColor":m};bc[bb]=w;return new qx.html.Element(z,bc);}
,__Rn:function(){var bd={"width":w,"height":w,"position":v,"backgroundColor":m,"userSelect":l};return new qx.html.Element(z,bd);}
,__Ro:function(){var be={"width":A,"height":A,"textAlign":L,"fontSize":a,"color":k,"fontFamily":J};var bf=new qx.html.Element(z,be);bf.setAttribute(u,g);return bf;}
}});}
)();
