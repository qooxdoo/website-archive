...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Flash.f31fecf58cec.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
;}
;var w=this.__FS(r,s,u,v);this._flashObjects[s.id]=w;return w;}
,destroy:qx.core.Environment.select(d,{"mshtml":function(x,y){x=this.__FN(x);if(x.readyState==4){this.__FO(x);}
else {if(!y){y=window;}
;qx.bom.Event.addNativeListener(y,j,function(){qx.bom.Flash.__FO(x);}
);}
;}
,"default":function(z,A){z=this.__FN(z);if(z.parentNode){z.parentNode.removeChild(z);}
...
),__FS:qx.core.Environment.select("engine.name",{"mshtml":function(E,F,G,H){G.movie=F.data;delete F.data;delete F.classid;var I="";for(name in G){I+='<param name="'+name+'" value="'+G[name]+'" />';}
...
})},defer:function(P){if((qx.core.Environment.get(d)==n)){qx.bom.Event.addNativeListener(window,l,P.__FP);}
;}
});}
)();
