...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Location.37ecb05a2bfd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var p="demobrowser.demo.bom.Location",o="borderBottomWidth",n="list",m='#',k="css.boxmodel",j="mode",h="borderTopWidth",g="borderLeftWidth",f="a",e="div",b="borderRightWidth",d="border",c="moveable",a="px";qx.Class.define(p,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var q=document.getElementById(n).getElementsByTagName(f);for(var i=0,l=q.length;i<l;i++){q[i].onclick=demobrowser.demo.bom.Location.jump;}
;}
...
;var v=this.href;var x=v.substr(v.indexOf(m)+1);var u=document.getElementById(x);var location=qx.bom.element.Location.get(u,document.getElementById(j).value);var w=document.getElementById(c);w.style.left=location.left+a;w.style.top=location.top+a;if(qx.core.Environment.get(k)==d){w.style.width=(location.right-location.left)+a;w.style.height=(location.bottom-location.top)+a;}
else {w.style.width=(location.right-location.left-t(w,g)-t(w,b))+a;w.style.height=(location.bottom-location.top-t(w,h)-t(w,o))+a;}
;return false;}
,randomize:function(){var A=document.body.getElementsByTagName(e);for(var i=0,l=A.length;i<l;i++){if(A[i].id&&A[i].id!=c){A[i].style.margin=Math.round(Math.random()*10)+a;A[i].style.padding=Math.round(Math.random()*10)+a;A[i].style.borderWidth=Math.round(Math.random()*10)+a;}
;}
;}
}});}
)();
