...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Placeholder.d6d103c88296.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;o=o.concat(n);}
;return qxWeb.$init(o);}
,getAncestors:function(p){return this.__lO(null,p);}
,getAncestorsUntil:function(q,r){return this.__lO(q,r);}
,__lO:function(s,t){var v=[];for(var i=0;i<this.length;i++){var parent=qx.dom.Element.getParentElement(this[i]);while(parent){var u=[parent];if(s&&qx.bom.Selector.matches(s,u).length>0){break;}
...
;return K;}
,getNextAll:function(L){var M=qx.module.Traversing.__lP(this,a,L);return qxWeb.$init(M);}
...
;return T;}
,getPrevAll:function(U){var V=qx.module.Traversing.__lP(this,d,U);return qxWeb.$init(V);}
...
,getSiblings:function(bc){var bd=qx.module.Traversing.__lP(this,c,bc);return qxWeb.$init(bd);}
...
;return null;}
,setHtml:function(c){for(var i=0;i<this.length;i++){qx.bom.element.Attribute.set(this[i],a,c);}
;return this;}
...
;}
;return this;}
,appendTo:function(parent){parent=qx.module.Manipulating.__lQ(parent);for(var i=0,l=parent.length;i<l;i++){for(var j=0,m=this.length;j<m;j++){if(i==0){qx.dom.Element.insertEnd(this[j],parent[i]);}
else {qx.dom.Element.insertEnd(this.eq(j).clone(true)[0],parent[i]);}
;}
;}
;return this;}
,insertBefore:function(p){p=qx.module.Manipulating.__lQ(p);for(var i=0,l=p.length;i<l;i++){for(var j=0,m=this.length;j<m;j++){if(i==0){qx.dom.Element.insertBefore(this[j],p[i]);}
else {qx.dom.Element.insertBefore(this.eq(j).clone(true)[0],p[i]);}
;}
;}
;return this;}
,insertAfter:function(q){q=qx.module.Manipulating.__lQ(q);for(var i=0,l=q.length;i<l;i++){for(var j=this.length-1;j>=0;j--){if(i==0){qx.dom.Element.insertAfter(this[j],q[i]);}
else {qx.dom.Element.insertAfter(this.eq(j).clone(true)[0],q[i]);}
;}
;}
;return this;}
,__lQ:function(r){if(!qx.lang.Type.isArray(r)){var s=qxWeb(r);r=s.length>0?s:[r];}
;return r;}
,wrap:function(t){var t=qx.module.Manipulating.__lR(t);if(t.length==0||!qx.dom.Node.isElement(t[0])){return this;}
;for(var i=0,l=this.length;i<l;i++){var u=t.eq(0).clone(true);qx.dom.Element.insertAfter(u[0],this[i]);var v=qx.module.Manipulating.__lS(u[0]);qx.dom.Element.insertEnd(this[i],v);}
;return this;}
,__lR:function(w){var x;if(qx.lang.Type.isArray(w)){x=qxWeb(w);}
else {var y=qx.bom.Html.clean([w]);if(y.length>0&&qx.dom.Node.isElement(y[0])){x=qxWeb(y);}
else {x=qxWeb(w);}
;}
;return x;}
,__lS:function(z){if(z.childNodes.length==0){return z;}
;for(var i=0,l=z.childNodes.length;i<l;i++){if(z.childNodes[i].nodeType===1){return this.__lS(z.childNodes[i]);}
;}
;return z;}
...
;return this;}
,before:function(A){if(!qx.lang.Type.isArray(A)){A=[A];}
;var B=document.createDocumentFragment();qx.bom.Html.clean(A,document,B);this.forEach(function(C,D){var E=qx.lang.Array.cast(B.childNodes,Array);for(var i=0,l=E.length;i<l;i++){var F;if(D<this.length-1){F=E[i].cloneNode(true);}
else {F=E[i];}
;C.parentNode.insertBefore(F,C);}
;}
,this);return this;}
,after:function(G){if(!qx.lang.Type.isArray(G)){G=[G];}
;var H=document.createDocumentFragment();qx.bom.Html.clean(G,document,H);this.forEach(function(I,J){var K=qx.lang.Array.cast(H.childNodes,Array);for(var i=K.length-1;i>=0;i--){var L;if(J<this.length-1){L=K[i].cloneNode(true);}
else {L=K[i];}
;I.parentNode.insertBefore(L,I.nextSibling);}
;}
,this);return this;}
,getScrollLeft:function(){var M=this[0];if(!M){return null;}
;var Node=qx.dom.Node;if(Node.isWindow(M)||Node.isDocument(M)){return qx.bom.Viewport.getScrollLeft();}
;return M.scrollLeft;}
,getScrollTop:function(){var N=this[0];if(!N){return null;}
;var Node=qx.dom.Node;if(Node.isWindow(N)||Node.isDocument(N)){return qx.bom.Viewport.getScrollTop();}
;return N.scrollTop;}
,_animationDescription:{scrollLeft:{duration:700,timing:a,keep:100,keyFrames:{'0':{},'100':{scrollLeft:1}}},scrollTop:{duration:700,timing:a,keep:100,keyFrames:{'0':{},'100':{scrollTop:1}}}},__lT:function(O,P,Q){var R=qx.lang.Object.clone(qx.module.Manipulating._animationDescription[O],true);R.keyFrames[100][O]=P;return this.animate(R,Q);}
,setScrollLeft:function(S,T){var Node=qx.dom.Node;if(T&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__lT.bind(this,e,S,T)();}
;for(var i=0,l=this.length,U;i<l;i++){U=this[i];if(Node.isElement(U)){if(!(T&&qx.bom.element&&qx.bom.element.AnimationJs)){U.scrollLeft=S;}
;}
else if(Node.isWindow(U)){U.scrollTo(S,this.getScrollTop(U));}
else if(Node.isDocument(U)){Node.getWindow(U).scrollTo(S,this.getScrollTop(U));}
;}
;return this;}
,setScrollTop:function(V,W){var Node=qx.dom.Node;if(W&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__lT.bind(this,b,V,W)();}
;for(var i=0,l=this.length,X;i<l;i++){X=this[i];if(Node.isElement(X)){if(!(W&&qx.bom.element&&qx.bom.element.AnimationJs)){X.scrollTop=V;}
;}
else if(Node.isWindow(X)){X.scrollTo(this.getScrollLeft(X),V);}
else if(Node.isDocument(X)){Node.getWindow(X).scrollTo(this.getScrollLeft(X),V);}
;}
;return this;}
,focus:function(){try{this[0].focus();}
catch(Y){}
;return this;}
,blur:function(){this.forEach(function(ba,bb){try{ba.blur();}
catch(bc){}
;}
);return this;}
},defer:function(bd){qxWeb.$attachStatic({"create":bd.create});qxWeb.$attach({"append":bd.append,"appendTo":bd.appendTo,"remove":bd.remove,"empty":bd.empty,"before":bd.before,"insertBefore":bd.insertBefore,"after":bd.after,"insertAfter":bd.insertAfter,"wrap":bd.wrap,"clone":bd.clone,"getScrollLeft":bd.getScrollLeft,"setScrollLeft":bd.setScrollLeft,"getScrollTop":bd.getScrollTop,"setScrollTop":bd.setScrollTop,"focus":bd.focus,"blur":bd.blur});}
});}
)();
(function(){var m="<fieldset>",k="<select multiple='multiple'>",h="</div>",g="</select>",f="</tr></tbody></table>",e="<col",d="div",c="<table><tbody><tr>",b=">",a="<table><tbody></tbody><colgroup>",J="<th",I="</tbody></table>",H="<td",G="</colgroup></table>",F="<opt",E="text/javascript",D="",C="</fieldset>",B="<table><tbody>",A="div<div>",t="<table",u="mshtml",r="qx.bom.Html",s="<leg",p="tbody",q="<tr",n="</table>",o="></",v="string",w="script",y="<table>",x="engine.name",z="undefined";qx.Bootstrap.define(r,{statics:{__lU:function(K,L,M){return M.match(/^(abbr|br|col|img|input|link|meta|param|hr|area|embed)$/i)?K:L+o+M+b;}
,__lV:{opt:[1,k,g],leg:[1,m,C],table:[1,y,n],tr:[2,B,I],td:[3,c,f],col:[2,a,G],def:qx.core.Environment.select(x,{"mshtml":[1,A,h],"default":null})},__lW:function(N,O){var U=O.createElement(d);N=N.replace(/(<(\w+)[^>]*?)\/>/g,this.__lU);var Q=N.replace(/^\s+/,D).substring(0,5).toLowerCase();var T,P=this.__lV;if(!Q.indexOf(F)){T=P.opt;}
...
;}
;var bb,bc=[];for(var i=0,l=W.length;i<l;i++){bb=W[i];if(typeof bb===v){bb=this.__lW(bb,X);}
...
;var h=k.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);if(!h){h=qx.module.Placeholder.__HJ(k);}
;var j=k.isRendered();var l=h.isRendered();if(j&&!l){k.before(h);}
else if(!j&&l){h.remove();return this;}
;qx.module.Placeholder.__HI(k);}
;}
;return this;}
,__HI:function(o){var r=o.getAttribute("placeholder");var q=o.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);var p=o.getStyle("z-index");var s=parseInt(o.getStyle("padding-left"))+2*parseInt(o.getStyle("padding-right"));var t=parseInt(o.getStyle("padding-top"))+2*parseInt(o.getStyle("padding-bottom"));q.setHtml(r).setStyles({display:o.getValue()==""?"inline":"none",zIndex:p=="auto"?1:p+1,textAlign:o.getStyle("text-align"),width:(o.getWidth()-s-4)+"px",height:(o.getHeight()-t-4)+"px",left:o.getOffset().left+"px",top:o.getOffset().top+"px",fontFamily:o.getStyle("font-family"),fontStyle:o.getStyle("font-style"),fontVariant:o.getStyle("font-variant"),fontWeight:o.getStyle("font-weight"),fontSize:o.getStyle("font-size"),paddingTop:(parseInt(o.getStyle("padding-top"))+2)+"px",paddingRight:(parseInt(o.getStyle("padding-right"))+2)+"px",paddingBottom:(parseInt(o.getStyle("padding-bottom"))+2)+"px",paddingLeft:(parseInt(o.getStyle("padding-left"))+2)+"px"});}
,__HJ:function(u){var v=qxWeb.create("<label>").setStyles({position:"absolute",color:"#989898",overflow:"hidden",pointerEvents:"none"});u.setProperty(qx.module.Placeholder.PLACEHOLDER_NAME,v);u.on("keyup",function(w){var x=w.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);x.setStyle("display",w.getValue()==""?"inline":"none");}
