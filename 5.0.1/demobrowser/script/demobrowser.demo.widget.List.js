...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.List","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.List.499a4d66818c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="Julian",b="Show Icon",c="one",d="Kevin",f="changeValue",g="Marvin",h="Marc",j="Configurable",k="icon/16/places/folder.png",l="computer.png",m="changeSelection",n="Jason",o="Jakob",p="Lukas",q="Julius",r="network-wired.png",s="additive",t="Jan",u="bold",v="Tobias",w="Mika",x="Nils",y="Noah",z="Quick selection is only available for the modes multi or additive",A="Tom",B="Drag selection is only available for the modes multi or additive",C="Multi Selection",D="battery.png",E="Felix",F="Leon",G="input-keyboard.png",H="camera-web.png",I="Enable drag selection",J="icon",K="Show Both",L="48",M="icon/48/devices/",N="Fabian",O="Nico",P="camera-photo.png",Q="media-flash.png",R="input-mouse.png",S="Joel",T="Dominic",U="Show Label",V="demobrowser.demo.widget.List",W="Enable quick selection",X="Item No ",Y="Finn",cl="One as selection mode",cm="Jannis",cn="audio-card.png",ch="Alexander",ci="Additive Selection",cj="display.png",ck="Erik",cs="addaptive",ct="Johannes",cu="pda.png",cv="16",co="Jonas",cp="icon/",cq="David",cr="Maximilian",cz="Tim",cY="Justin",da="Single Selection",cA="Linus",cw="Daniel",cx="Paul",dj="Philipp",cy="Niclas",cB="One Selection",cC="Nick",cD="Jonathan",cH="Ben",dk="on",cI="Value: ",cE="Vincent",cF="/places/folder.png",di="Robin",cG="both",cM="Jannick",cN="Luis",cO="multimedia-player.png",cP="drive-harddisk.png",cJ="value",cK="Lennard",dl="Moritz",cL="Florian",cT="drive-optical.png",cU="media-optical.png",dn="multi",cV="Marlon",cQ="Luca",cR="single",dm="network-wireless.png",cS="audio-input-microphone.png",cW="phone.png",cX="Simon",dd="Colin",dc="label",db="printer.png",dh="Max",dg="Additive selection",df="Horizontal, Icons only",de="Elias";qx.Class.define(V,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var dp=new qx.ui.container.Scroll();var ds=new qx.ui.container.Composite(new qx.ui.layout.Basic());ds.setAllowStretchX(false);dp.add(ds);this.getRoot().add(dp,{edge:0});var dz=new qx.ui.basic.Label(j);dz.setFont(u);ds.add(dz,{left:20,top:20});var dH=new qx.ui.form.List;dH.setScrollbarX(dk);dH.set({height:280,width:150,selectionMode:dn});var dI;for(var i=1;i<=25;i++ ){dI=new qx.ui.form.ListItem(X+i,cp+((i%4)?cv:L)+cF);dH.add(dI);!(i%9)&&(dI.setEnabled(false));if(i==20){dH.setSelection([dI]);}
;}
;dH.addListener(m,function(e){var dO=e.getData();if(dO[0]){this.debug(cI+dO[0].getLabel());}
;}
);ds.add(dH,{left:20,top:40});var dv=new qx.ui.form.RadioButton(da);var dF=new qx.ui.form.RadioButton(C);var dG=new qx.ui.form.RadioButton(ci);var dr=new qx.ui.form.RadioButton(cB);dv.setUserData(cJ,cR);dF.setUserData(cJ,dn);dG.setUserData(cJ,s);dr.setUserData(cJ,c);dF.setValue(true);ds.add(dv,{left:180,top:40});ds.add(dF,{left:180,top:60});ds.add(dG,{left:180,top:80});ds.add(dr,{left:180,top:100});var du=new qx.ui.form.RadioGroup(dv,dF,dG,dr);var dL=new qx.ui.form.RadioButton(U);var dM=new qx.ui.form.RadioButton(b);var dN=new qx.ui.form.RadioButton(K);dL.setUserData(cJ,dc);dM.setUserData(cJ,J);dN.setUserData(cJ,cG);dN.setValue(true);ds.add(dL,{left:180,top:140});ds.add(dM,{left:180,top:160});ds.add(dN,{left:180,top:180});var dt=new qx.ui.form.RadioGroup(dL,dM,dN);dt.addListener(m,function(e){for(var i=0;i<dH.getChildren().length;i++ ){dH.getChildren()[i].setShow(e.getData()[0].getUserData(cJ));}
;}
);var dD=new qx.ui.form.CheckBox(I);var dA=new qx.ui.form.CheckBox(W).set({enabled:false});ds.add(dD,{left:180,top:220});ds.add(dA,{left:180,top:240});dD.addListener(f,function(e){if(e.getData()){var dP=dH.getSelectionMode();if(dP==cR||dP==c){this.debug(B);}
;}
;dH.setDragSelection(e.getData());}
);dA.addListener(f,function(e){if(e.getData()){var dQ=dH.getSelectionMode();if(dQ==dn||dQ==s){this.debug(z);}
;}
;dH.setQuickSelection(e.getData());}
);du.addListener(m,function(e){var dR=e.getData()[0].getUserData(cJ);dH.setSelectionMode(dR);if(dR==cR||dR==c){dD.setEnabled(false);dA.setEnabled(true);}
else if(dR==dn||dR==cs){dD.setEnabled(true);dA.setEnabled(false);}
;}
);var dK=new qx.ui.basic.Label(cl);dK.setFont(u);ds.add(dK,{left:330,top:20});var dy=new qx.ui.form.List();dy.set({height:280,width:150,selectionMode:c});var dI;for(var i=1;i<=25;i++ ){dI=new qx.ui.form.ListItem(X+i,k);dy.add(dI);if(i==16){dy.setSelection([dI]);}
;}
;ds.add(dy,{left:330,top:40});var dz=new qx.ui.basic.Label(dg);dz.setFont(u);ds.add(dz,{left:520,top:20});var dq=new qx.ui.form.List;var dw;dq.set({width:150,selectionMode:s});var dC=[F,p,cQ,Y,cz,E,co,cN,cr,a,dh,cx,cy,t,cH,de,cM,dj,y,A,dl,O,cq,x,cX,N,ck,cY,ch,o,cL,cC,cA,w,n,cw,cK,g,cm,v,T,cV,h,ct,cD,q,dd,S,d,cE,di];for(var i=0;i<dC.length;i++ ){dw=new qx.ui.form.ListItem(dC[i]);dq.add(dw);if(i==10||i==12||i==16){dq.addToSelection(dw);}
;}
;ds.add(dq,{left:520,top:40});var dz=new qx.ui.basic.Label(df);dz.setFont(u);ds.add(dz,{left:20,top:350});var dJ=new qx.ui.form.List(true);var dx;dJ.set({width:550,selectionMode:dn,height:null});var dB=[cn,cS,D,P,H,l,cj,cP,cT,G,R,Q,cU,cO,r,dm,cu,cW,db];var dE=M;for(var i=0;i<dB.length;i++ ){dx=new qx.ui.form.ListItem(null,dE+dB[i]);dJ.add(dx);if(i==12){dJ.setSelection([dx]);}
;}
;ds.add(dJ,{left:20,top:370});}
}});}
)();
...
(function(){var a="qx.ui.form.List",b="text",c="Boolean",d="one",f="addChildWidget",g="_applySpacing",h="Enter",j="Integer",k="action",m="keyinput",n="__nM",o="qx.event.type.Data",p="addItem",q="vertical",r="removeChildWidget",s="",t="_applyOrientation",u="single",v="keypress",w="list",x="horizontal",y="label",z="pane",A="removeItem";qx.Class.define(a,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MMultiSelectionHandling,qx.ui.form.MForm,qx.ui.form.MModelSelection],construct:function(B){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__nM=this._createListItemContainer();this.__nM.addListener(f,this._onAddChild,this);this.__nM.addListener(r,this._onRemoveChild,this);this.getChildControl(z).add(this.__nM);if(B){this.setOrientation(x);}
else {this.initOrientation();}
;this.addListener(v,this._onKeyPress);this.addListener(m,this._onKeyInput);this.__uG=s;}
,events:{addItem:o,removeItem:o},properties:{appearance:{refine:true,init:w},focusable:{refine:true,init:true},orientation:{check:[x,q],init:q,apply:t},spacing:{check:j,init:0,apply:g,themeable:true},enableInlineFind:{check:c,init:true}},members:{__uG:null,__uH:null,__nM:null,SELECTION_MANAGER:qx.ui.core.selection.ScrollArea,getChildrenContainer:function(){return this.__nM;}
...
,_applyOrientation:function(F,C){var D=F===x;var E=D?new qx.ui.layout.HBox():new qx.ui.layout.VBox();var content=this.__nM;content.setLayout(E);content.setAllowGrowX(!D);content.setAllowGrowY(D);this._applySpacing(this.getSpacing());}
,_applySpacing:function(H,G){this.__nM.getLayout().setSpacing(H);}
...
;if(((new Date).valueOf()-this.__uH)>1000){this.__uG=s;}
;this.__uG+=e.getChar();var K=this.findItemByLabelFuzzy(this.__uG);if(K){this.setSelection([K]);}
;this.__uH=(new Date).valueOf();}
...
;}
,remove:function(w){var x=this.__mf;if(qx.lang.Array.contains(x,w)){qx.lang.Array.remove(x,w);if(w.getGroup()===this){w.resetGroup();}
...
;}
,_applyInvalidMessage:function(A,z){for(var i=0;i<this.__mf.length;i++ ){this.__mf[i].setInvalidMessage(A);}
;}
,_applyValid:function(C,B){for(var i=0;i<this.__mf.length;i++ ){this.__mf[i].setValid(C);}
;}
,_applyEnabled:function(F,E){var D=this.__mf;if(F==null){for(var i=0,l=D.length;i<l;i++ ){D[i].resetEnabled();}
;}
...
;}
,selectNext:function(){var J=this.getSelection()[0];var K=this.__mf;var I=K.indexOf(J);if(I==-1){return;}
...
,selectPrevious:function(){var M=this.getSelection()[0];var N=this.__mf;var L=N.indexOf(M);if(L==-1){return;}
...
,_isItemSelectable:function(O){return this.__mf.indexOf(O)!=-1;}
,__lP:function(e){var Q=e.getData()[0];var P=e.getOldData()[0];if(P){P.setValue(false);}
