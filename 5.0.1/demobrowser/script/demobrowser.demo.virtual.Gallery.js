...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Gallery.860e9a64a8af.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="__rJ",b="qx.ui.window.Manager";qx.Class.define(b,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__rJ:null,setDesktop:function(c){this.__rJ=c;this.updateStack();}
,getDesktop:function(){return this.__rJ;}
,changeActiveWindow:function(d,e){if(d){this.bringToFront(d);d.setActive(true);}
;if(e){e.resetActive();}
;}
,_minZIndex:1e5,updateStack:function(){qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(){this.__rJ.forceUnblock();var h=this.__rJ.getWindows();var g=this._minZIndex;var m=g+h.length*2;var j=g+h.length*4;var k=null;for(var i=0,l=h.length;i<l;i++ ){var f=h[i];if(!f.isVisible()){continue;}
;k=k||f;if(f.isModal()){f.setZIndex(j);this.__rJ.blockContent(j-1);j+=2;k=f;}
else if(f.isAlwaysOnTop()){f.setZIndex(m);m+=2;}
else {f.setZIndex(g);g+=2;}
;if(!k.isModal()&&f.isActive()||f.getZIndex()>k.getZIndex()){k=f;}
;}
;this.__rJ.setActiveWindow(k);}
,bringToFront:function(o){var n=this.__rJ.getWindows();var p=qx.lang.Array.remove(n,o);if(p){n.push(o);this.updateStack();}
;}
,sendToBack:function(r){var q=this.__rJ.getWindows();var s=qx.lang.Array.remove(q,r);if(s){q.unshift(r);this.updateStack();}
;}
},destruct:function(){this._disposeObjects(a);}
});}
)();
(function(){var a="pointerup",b="Boolean",c="__rK",d="losecapture",f="move",g="qx.ui.core.MMovable",h="The move handle could not be redefined!",i="pointerdown",j="__rL",k="maximized",l="pointermove",m="roll",n="move-frame";qx.Mixin.define(g,{properties:{movable:{check:b,init:true},useMoveFrame:{check:b,init:false}},members:{__rK:null,__rL:null,__rM:null,__rN:null,__rO:null,__rP:null,__rQ:null,__rR:false,__rS:null,__rT:0,_activateMoveHandle:function(o){if(this.__rK){throw new Error(h);}
;this.__rK=o;o.addListener(i,this._onMovePointerDown,this);o.addListener(a,this._onMovePointerUp,this);o.addListener(l,this._onMovePointerMove,this);o.addListener(d,this.__rX,this);}
,__rU:function(){var p=this.__rL;if(!p){p=this.__rL=new qx.ui.core.Widget();p.setAppearance(n);p.exclude();qx.core.Init.getApplication().getRoot().add(p);}
;return p;}
,__rV:function(){var location=this.getContentLocation();var r=this.getBounds();var q=this.__rU();q.setUserBounds(location.left,location.top,r.width,r.height);q.show();q.setZIndex(this.getZIndex()+1);}
,__rW:function(e){var t=this.__rM;var s=Math.max(t.left,Math.min(t.right,e.getDocumentLeft()));var v=Math.max(t.top,Math.min(t.bottom,e.getDocumentTop()));var u=this.__rN+s;var w=this.__rO+v;return {viewportLeft:parseInt(u,10),viewportTop:parseInt(w,10),parentLeft:parseInt(u-this.__rP,10),parentTop:parseInt(w-this.__rQ,10)};}
,_onMoveRoll:function(e){e.stop();}
,_onMovePointerDown:function(e){if(!this.getMovable()||this.hasState(k)){return;}
;this.addListener(m,this._onMoveRoll,this);var parent=this.getLayoutParent();var x=parent.getContentLocation();var z=parent.getBounds();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isBlocked()){this.__rS=parent.getBlockerColor();this.__rT=parent.getBlockerOpacity();parent.setBlockerColor(null);parent.setBlockerOpacity(1);parent.blockContent(this.getZIndex()-1);this.__rR=true;}
;}
;this.__rM={left:x.left,top:x.top,right:x.left+z.width,bottom:x.top+z.height};var y=this.getContentLocation();this.__rP=x.left;this.__rQ=x.top;this.__rN=y.left-e.getDocumentLeft();this.__rO=y.top-e.getDocumentTop();this.addState(f);this.__rK.capture();if(this.getUseMoveFrame()){this.__rV();}
;e.stop();}
,_onMovePointerMove:function(e){if(!this.hasState(f)){return;}
;var B=this.__rW(e);if(this.getUseMoveFrame()){this.__rU().setDomPosition(B.viewportLeft,B.viewportTop);}
...
;this.removeState(f);var parent=this.getLayoutParent();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__rR){parent.unblock();parent.setBlockerColor(this.__rS);parent.setBlockerOpacity(this.__rT);this.__rS=null;this.__rT=0;this.__rR=false;}
;}
;this.__rK.releaseCapture();var D=this.__rW(e);var C=this.getLayoutParent().getInsets();this.setLayoutProperties({left:D.parentLeft-(C.left||0),top:D.parentTop-(C.top||0)});if(this.getUseMoveFrame()){this.__rU().exclude();}
;e.stopPropagation();}
,__rX:function(e){if(!this.hasState(f)){return;}
;this.removeState(f);if(this.getUseMoveFrame()){this.__rU().exclude();}
;}
},destruct:function(){this._disposeObjects(j,c);this.__rM=null;}
});}
)();
...
(function(){var a="Boolean",b="w-resize",c="touch",d="sw-resize",f="n-resize",g="resizableRight",h="ne-resize",i="se-resize",j="Integer",k="e-resize",l="resizableLeft",m="move",n="shorthand",o="maximized",p="resize",q="nw-resize",r="pointerup",s="pointerdown",t="qx.ui.core.MResizable",u="losecapture",v="resize-frame",w="resizableBottom",x="s-resize",y="pointermove",z="resizableTop",A="pointerout";qx.Mixin.define(t,{construct:function(){var content=this.getContentElement();content.addListener(s,this.__sl,this,true);content.addListener(r,this.__sm,this);content.addListener(y,this.__so,this);content.addListener(A,this.__sp,this);content.addListener(u,this.__sn,this);var B=content.getDomElement();if(B==null){B=window;}
;this.__rY=qx.event.Registration.getManager(B).getHandler(qx.event.handler.DragDrop);}
,properties:{resizableTop:{check:a,init:true},resizableRight:{check:a,init:true},resizableBottom:{check:a,init:true},resizableLeft:{check:a,init:true},resizable:{group:[z,g,w,l],mode:n},resizeSensitivity:{check:j,init:5},useResizeFrame:{check:a,init:true}},members:{__rY:null,__sa:null,__sb:null,__sc:null,__sd:null,__se:null,__sf:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__sa;if(!C){C=this.__sa=new qx.ui.core.Widget();C.setAppearance(v);C.exclude();qx.core.Init.getApplication().getRoot().add(C);}
;return C;}
,__sg:function(){var location=this.getContentLocation();var D=this._getResizeFrame();D.setUserBounds(location.left,location.top,location.right-location.left,location.bottom-location.top);D.show();D.setZIndex(this.getZIndex()+1);}
,__sh:function(e){var F=this.__sb;var E=this.getSizeHint();var I=this.__sf;var H=this.__se;var J=H.width;var G=H.height;var L=H.left;var top=H.top;var K;if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){K=Math.max(I.top,Math.min(I.bottom,e.getDocumentTop()))-this.__sd;if(F&this.RESIZE_TOP){G-=K;}
else {G+=K;}
;if(G<E.minHeight){G=E.minHeight;}
else if(G>E.maxHeight){G=E.maxHeight;}
;if(F&this.RESIZE_TOP){top+=H.height-G;}
;}
;if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){K=Math.max(I.left,Math.min(I.right,e.getDocumentLeft()))-this.__sc;if(F&this.RESIZE_LEFT){J-=K;}
else {J+=K;}
...
,__si:{'1':f,'2':x,'4':b,'8':k,'5':q,'6':d,'9':h,'10':i},__sj:function(e){var location=this.getContentLocation();var O=this.getResizeSensitivity();var N=e.getDocumentLeft();var P=e.getDocumentTop();var M=this.__sk(location,N,P,O);if(M>0){M=M|this.__sk(location,N,P,O*2);}
;this.__sb=M;}
,__sk:function(location,T,R,S){var Q=0;if(this.getResizableTop()&&Math.abs(location.top-R)<S&&T>location.left-S&&T<location.right+S){Q+=this.RESIZE_TOP;}
...
;return Q;}
,__sl:function(e){if(!this.__sb||!this.getEnabled()||e.getPointerType()==c){return;}
;this.addState(p);this.__sc=e.getDocumentLeft();this.__sd=e.getDocumentTop();var location=this.getContentLocation();var V=this.getBounds();this.__se={top:location.top,left:location.left,width:location.right-location.left,height:location.bottom-location.top,bounds:qx.lang.Object.clone(V)};var parent=this.getLayoutParent();var W=parent.getContentLocation();var U=parent.getBounds();this.__sf={left:W.left,top:W.top,right:W.left+U.width,bottom:W.top+U.height};if(this.getUseResizeFrame()){this.__sg();}
;this.capture();e.stop();}
,__sm:function(e){if(!this.hasState(p)||!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.getUseResizeFrame()){this._getResizeFrame().exclude();}
;var X=this.__sh(e);this.setWidth(X.width);this.setHeight(X.height);if(this.getResizableLeft()||this.getResizableTop()){this.setLayoutProperties({left:X.parentLeft,top:X.parentTop});}
;this.__sb=0;this.removeState(p);this.resetCursor();this.getApplicationRoot().resetGlobalCursor();this.releaseCapture();e.stopPropagation();}
,__sn:function(e){if(!this.__sb){return;}
...
;}
,__so:function(e){if(!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.hasState(p)){var ba=this.__sh(e);if(this.getUseResizeFrame()){var Y=this._getResizeFrame();Y.setUserBounds(ba.viewportLeft,ba.viewportTop,ba.width,ba.height);}
...
;}
;e.stopPropagation();}
else if(!this.hasState(o)&&!this.__rY.isSessionActive()){this.__sj(e);var bd=this.__sb;var bc=this.getApplicationRoot();if(bd){var bb=this.__si[bd];this.setCursor(bb);bc.setGlobalCursor(bb);}
else if(this.getCursor()){this.resetCursor();bc.resetGlobalCursor();}
;}
;}
,__sp:function(e){if(e.getPointerType()==c){return;}
;if(this.getCursor()&&!this.hasState(p)){this.resetCursor();this.getApplicationRoot().resetGlobalCursor();}
;}
},destruct:function(){if(this.__sa!=null&&!qx.core.ObjectRegistry.inShutDown){this.__sa.destroy();this.__sa=null;}
;this.__rY=null;}
});}
)();
...
;var br=this.getLayoutProperties();this.__sr=br.left===undefined?0:br.left;this.__sq=br.top===undefined?0:br.top;this.setLayoutProperties({left:null,top:null,edge:0});this.addState(F);this._updateCaptionBar();this.fireEvent(Q);}
;}
;}
,minimize:function(){if(!this.isVisible()){return;}
;if(this.fireNonBubblingEvent(b,qx.event.type.Event,[false,true])){var bs=this.getLayoutProperties();this.__sr=bs.left===undefined?0:bs.left;this.__sq=bs.top===undefined?0:bs.top;this.removeState(F);this.hide();this.fireEvent(v);}
;}
...
;var bt=this.__sr;var top=this.__sq;this.setLayoutProperties({edge:null,left:bt,top:top});this.removeState(F);this._updateCaptionBar();this.fireEvent(S);}
;}
...
(function(){var a="Gallery (HTML - divs)",b="manager",c="background",d="demobrowser.demo.virtual.Gallery",f="demobrowser.demo.virtual.WidgetGallery",g="selected",h="text-align: center;",j="background-selected",k="auto",l="color: white; background-color: #00398D;",m="user-desktop.png",n="cell",o="<img src='",p="'></img>",q="layer",r="resize",s="folder.png",t="vertical",u="",v="icon",w="multi",x="network-server.png",y="user-trash.png",z="off",A="white-box",B="top",C="icon/32/places/",D="horizontal",E="demobrowser.demo.virtual.AbstractGallery",F="default",G="abstract",H="__PC",I="position: absolute;",J="label",K="#f3f3f3",L="demobrowser.demo.virtual.HtmlGallery",M="Gallery (widgets)",N="<br>",O="Icon #",P="network-workgroup.png",Q="text-selected",R="demobrowser.demo.virtual.GalleryCell";qx.Class.define(d,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);new demobrowser.demo.virtual.WidgetGallery(M);var S=new demobrowser.demo.virtual.HtmlGallery(a);S.moveTo(400,50);}
}});qx.Class.define(E,{extend:qx.ui.window.Window,type:G,construct:function(U){qx.ui.window.Window.call(this,U);this.set({contentPadding:0,showClose:false,showMinimize:false,width:320,height:400});this.setLayout(new qx.ui.layout.Grow());this.moveTo(30,50);this.open();this.itemHeight=65;this.itemWidth=70;this.itemCount=431;this.itemPerLine=1;this.items=this._generateItems(this.itemCount);var T=this._createScroller();T.set({scrollbarX:z,scrollbarY:k});T.getPane().addListener(r,this._onPaneResize,this);this.add(T);this.manager=new qx.ui.virtual.selection.CellRectangle(T.getPane(),this).set({mode:w,drag:true});this.manager.attachPointerEvents();this.manager.attachKeyEvents(T);}
,members:{getItemData:function(W,V){return this.items[W*this.itemPerLine+V];}
...
,_onPaneResize:function(e){var be=e.getTarget();var bf=e.getData().width;var bd=Math.max(1,Math.floor(bf/this.itemWidth));if(bd==this.itemsPerLine){return;}
;this.itemPerLine=bd;var bc=Math.ceil(this.itemCount/bd);be.getColumnConfig().setItemCount(bd);be.getRowConfig().setItemCount(bc);}
,_generateItems:function(bg){var bj=[];var bm=[s,y,x,P,m];var bk=qx.util.AliasManager.getInstance();var bi=qx.util.ResourceManager.getInstance();for(var i=0;i<bg;i++ ){var bn=C+bm[Math.floor(Math.random()*bm.length)];var bh=bk.resolve(bn);var bl=bi.toUri(bh);bj[i]={label:O+(i+1),icon:bn,resolvedIcon:bl};}
;return bj;}
},destruct:function(){this.items=null;this._disposeObjects(b);}
});qx.Class.define(R,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var bo=new qx.ui.basic.Atom().set({iconPosition:B});bo.getChildControl(J).set({padding:[0,4]});bo.getChildControl(v).set({padding:4});return bo;}
,updateData:function(bp,bq){bp.set({icon:bq.icon,label:bq.label});}
,updateStates:function(bs,br){var bt=bs.getChildControl(J);var bu=bs.getChildControl(v);if(br.selected){bt.setBackgroundColor(j);bt.setTextColor(Q);bu.setDecorator(A);bu.setBackgroundColor(c);}
else {bt.resetBackgroundColor();bt.resetTextColor();bu.resetDecorator();bu.resetBackgroundColor();}
;}
}});qx.Class.define(f,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bv){demobrowser.demo.virtual.AbstractGallery.call(this,bv);this.__PC=new demobrowser.demo.virtual.GalleryCell();}
,members:{__PC:null,_createScroller:function(){var bw=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.WidgetCell(this);bw.getPane().addLayer(this.layer);new qx.ui.virtual.behavior.Prefetch(bw,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:200,maxAbove:300,minBelow:600,maxBelow:800}).set({interval:500});return bw;}
,styleSelectable:function(bA,bC,by){if(bC!==g){return;}
;var bB=this.layer.getChildren();for(var i=0;i<bB.length;i++ ){var bx=bB[i];var bz=bx.getUserData(n);if(bA.row!==bz.row||bA.column!==bz.column){continue;}
;if(by){this.__PC.updateStates(bx,{selected:1});}
else {this.__PC.updateStates(bx,{});}
;}
;}
,getCellWidget:function(bI,bG){var bF=this.getItemData(bI,bG);if(!bF){return null;}
;var bH={row:bI,column:bG};var bD={};if(this.manager.isItemSelected(bH)){bD.selected=true;}
;var bE=this.__PC.getCellWidget(bF,bD);bE.setUserData(n,bH);return bE;}
,poolCellWidget:function(bJ){this.__PC.pool(bJ);}
},destruct:function(){this._disposeObjects(H,q);}
});qx.Class.define(L,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bK){demobrowser.demo.virtual.AbstractGallery.call(this,bK);var bL=qx.theme.manager.Font.getInstance().resolve(F).getStyles();this._fontCss=qx.bom.element.Style.compile(bL);}
,members:{_createScroller:function(){var bN=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.HtmlCell(this);bN.getPane().addLayer(this.layer);var bM=new qx.ui.virtual.layer.GridLines(D,K);bN.getPane().addLayer(bM);var bM=new qx.ui.virtual.layer.GridLines(t,K);bN.getPane().addLayer(bM);return bN;}
...
,getCellProperties:function(bV,bR){var bT=this.getItemData(bV,bR);if(!bT){return u;}
;var bS=this.manager.isItemSelected({row:bV,column:bR});var bU=bS?l:u;return {style:[I,h,this._fontCss,bU].join(u),content:[o,bT.resolvedIcon,p,N,bT.label].join(u)};}
...
(function(){var a="under",b="above",c="left",d="right",e="x",f="qx.ui.virtual.selection.CellRectangle";qx.Class.define(f,{extend:qx.ui.virtual.selection.Abstract,members:{_getItemCount:function(){return this._pane.getRowConfig().getItemCount()*this._pane.getColumnConfig().getItemCount();}
,_getSelectableFromPointerEvent:function(event){var g=this._pane.getCellAtPosition(event.getDocumentLeft(),event.getDocumentTop());if(!g){return null;}
;return this._isSelectable(g)?g:null;}
,getSelectables:function(i){var k=[];var j=this._pane.getRowConfig().getItemCount();var h=this._pane.getColumnConfig().getItemCount();for(var n=0;n<j;n++ ){for(var m=0;m<h;m++ ){var l={row:n,column:m};if(this._isSelectable(l)){k.push(l);}
;}
;}
;return k;}
,_getSelectableRange:function(r,q){var w=[];var s=Math.min(r.row,q.row);var o=Math.max(r.row,q.row);var p=Math.min(r.column,q.column);var v=Math.max(r.column,q.column);for(var x=s;x<=o;x++ ){for(var u=p;u<=v;u++ ){var t={row:x,column:u};if(this._isSelectable(t)){w.push(t);}
;}
;}
;return w;}
,_getFirstSelectable:function(){var C=this._pane.getRowConfig().getItemCount();var y=this._pane.getColumnConfig().getItemCount();for(var A=0;A<C;A++ ){for(var z=0;z<y;z++ ){var B={row:A,column:z};if(this._isSelectable(B)){return B;}
;}
;}
;return null;}
,_getLastSelectable:function(){var H=this._pane.getRowConfig().getItemCount();var D=this._pane.getColumnConfig().getItemCount();for(var E=D-1;E>=0;E-- ){for(var F=H-1;F>=0;F-- ){var G={row:F,column:E};if(this._isSelectable(G)){return G;}
;}
;}
;return null;}
,_getRelatedSelectable:function(J,N){var L={row:J.row,column:J.column};switch(N){case b:for(var O=J.row-1;O>=0;O-- ){L.row=O;if(this._isSelectable(L)){return L;}
;}
;break;case a:var M=this._pane.getRowConfig().getItemCount();for(var O=J.row+1;O<M;O++ ){L.row=O;if(this._isSelectable(L)){return L;}
;}
;break;case c:for(var K=J.column-1;K>=0;K-- ){L.column=K;if(this._isSelectable(L)){return L;}
;}
;break;case d:var I=this._pane.getColumnConfig().getItemCount();for(var K=J.column+1;K<I;K++ ){L.column=K;if(this._isSelectable(L)){return L;}
;}
;break;};return null;}
,_getPage:function(Q,P){if(P){return this._getFirstSelectable();}
else {return this._getLastSelectable();}
;}
,_selectableToHashCode:function(R){return R.column+e+R.row;}
,_scrollItemIntoView:function(S){if(this._autoScrollIntoView){this._pane.scrollCellIntoView(S.column,S.row);}
;}
,_getSelectableLocationX:function(U){var T=this._pane.getColumnConfig();var V=T.getItemPosition(U.column);var W=V+T.getItemSize(U.column)-1;return {left:V,right:W};}
...
(function(){var a="__eg",b="_applyInterval",c="appear",d="qx.debug",e="interval",f="qx.ui.virtual.behavior.Prefetch",g="scroll",h="_applyScroller",i="qx.ui.virtual.core.Scroller",j="scrollbar-x",k="Integer",l="scrollbar-y";qx.Class.define(f,{extend:qx.core.Object,construct:function(m,n){if(qx.core.Environment.get(d)){this.assertObject(n);this.assertPositiveInteger(n.minLeft);this.assertPositiveInteger(n.maxLeft);this.assertPositiveInteger(n.minRight);this.assertPositiveInteger(n.maxRight);this.assertPositiveInteger(n.minAbove);this.assertPositiveInteger(n.maxAbove);this.assertPositiveInteger(n.minBelow);this.assertPositiveInteger(n.maxBelow);}
;qx.core.Object.call(this);this.setPrefetchX(n.minLeft,n.maxLeft,n.minRight,n.maxRight);this.setPrefetchY(n.minAbove,n.maxAbove,n.minBelow,n.maxBelow);this.__eg=new qx.event.Timer(this.getInterval());this.__eg.addListener(e,this._onInterval,this);if(m){this.setScroller(m);}
;}
,properties:{scroller:{check:i,nullable:true,init:null,apply:h},interval:{check:k,init:200,apply:b}},members:{__Hh:null,__Hi:null,__eg:null,__Hj:null,__Hk:null,setPrefetchX:function(o,r,p,q){this.__Hh=[o,r,p,q];}
,setPrefetchY:function(v,u,t,s){this.__Hi=[v,u,t,s];}
,_onInterval:function(){var w=this.__Hh;if(w[1]&&w[3]){this.getScroller().getPane().prefetchX(w[0],w[1],w[2],w[3]);qx.ui.core.queue.Manager.flush();}
;var x=this.__Hi;if(x[1]&&x[3]){this.getScroller().getPane().prefetchY(x[0],x[1],x[2],x[3]);qx.ui.core.queue.Manager.flush();}
;}
,_applyScroller:function(z,y){if(y){if(this.__Hj){y.getChildControl(j).removeListenerById(this.__Hj);}
;if(this.__Hk){y.getChildControl(l).removeListenerById(this.__Hk);}
;}
;if(z){if(!z.getContentElement().getDomElement()){this.__eg.stop();z.addListenerOnce(c,this.__eg.start,this.__eg);}
else {this.__eg.restart();}
;this.__Hj=z.getChildControl(j).addListener(g,this.__eg.restart,this.__eg);this.__Hk=z.getChildControl(l).addListener(g,this.__eg.restart,this.__eg);}
else {this.__eg.stop();}
;}
,_applyInterval:function(B,A){this.__eg.setInterval(B);}
},destruct:function(){this.setScroller(null);this.__Hh=this.__Hi=null;this._disposeObjects(a);}
});}
)();
...
;this.__zk=[];this.__zl=[];this._isHorizontal=(w||c)==c;}
,properties:{defaultLineColor:{init:h,check:s,themeable:true},defaultLineSize:{init:1,check:t,themeable:true}},members:{__zk:null,__zl:null,isHorizontal:function(){return this._isHorizontal;}
...
;this.__zk[B]=A;if(this.__zm(B)){this.updateLayerData();}
;}
...
;this.__zl[C]=D;if(this.__zm(C)){this.updateLayerData();}
;}
,__zm:function(E){if(this._isHorizontal){var I=this.getFirstColumn();var G=I+this.getColumnSizes().length-1;return E>=I&&E<=G;}
else {var H=this.getFirstRow();var F=H+this.getRowSizes().length-1;return E>=H&&E<=F;}
;}
,getLineSize:function(J){return this.__zl[J]||this.getDefaultLineSize();}
,getLineColor:function(K){return this.__zk[K]||this.getDefaultLineColor();}
,__zn:function(P,M,O){var top=0;var L,N;for(var y=0;y<O.length-1;y++ ){L=this.getLineColor(M+y);N=this.getLineSize(M+y);top+=O[y];P.push(m,g,f+N+q,j,d,top-((N>1)?Math.floor(N/2):1),q,b,L,n,a);}
;}
,__zo:function(T,Q,S){var V=0;var R,U;for(var x=0;x<S.length-1;x++ ){R=this.getLineColor(Q+x);U=this.getLineSize(Q+x);V+=S[x];T.push(m,g,k+U+q,l,u,p,V-((U>1)?Math.floor(U/2):1),q,b,R,n,a);}
;}
,_fullUpdate:function(ba,bb,X,W){var Y=[];if(this._isHorizontal){this.__zn(Y,ba,X);}
else {this.__zo(Y,bb,W);}
...
;}
},destruct:function(){this.__zk=this.__zl=null;}
});}
)();
