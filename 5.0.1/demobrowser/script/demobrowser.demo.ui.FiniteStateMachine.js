...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.b68f0f4a1eae.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="TERMINATED",c=" => ",d="#autoActionsBeforeOnexit",f=": Could not find friendly name for '",g=": event '",h=" blocked.  Re-queuing.",j="string",k="qx.event.type.Data",l="'",m="Attempt to transition to nonexistent state ",n="__FO",o="Saved-state stack is empty",p="Internal error: invalid nextState",q=" not handled.  Ignoring.",r="State ",s="#autoActionsBeforeOntransition",u=" returned a value other than ",v="#",w=" already exists",x="Attempt to transition to POP_STATE_STACK ",y=":",z="Transition ",A=" does not exist",B="' on '",C="qx.util.fsm.FiniteStateMachine",D="String",E="__FP",F="terminated",G="#autoActionsAfterOntransition",H="",I="#actionsAfterOnentry",J="Saved-state stack is full",K="#autoActionsBeforeOnentry",L=": no transition found.  No state change.",M="#autoActionsAfterOnentry",N="#actionsBeforeOnentry",O="', because the finite state machine is not running.",P=": Pushed event: ",Q=": Cannot listen to event '",R="Machine started with no available states",S=": Process event: ",T="#entry",U="#ontransition",V=" on ",W="Invalid state: not an instance of ",X="Number",Y="#exit",bp="true, false, or null.",bq="Explicit transition ",br="  ",bl="qx.util.fsm.State",bm="while state stack is empty.",bn=".  Ignoring.",bo="#autoActionsAfterOnexit",bs=": Event '",bt="object",bu=" not handled for target ",bv=": Queued event: ";qx.Class.define(C,{extend:qx.core.Object,construct:function(bw){qx.core.Object.call(this);this.setName(bw);this.__jz={};this.__FM=null;this.__FN=[];this.__FO=[];this.__FP=[];this.__FQ={};this.__FR={};this.__FS={};this.__FT={};this.__FU={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},events:{"terminated":k},properties:{name:{check:D,nullable:true},state:{check:D,nullable:true},previousState:{check:D,nullable:true},nextState:{check:D,nullable:true},maxSavedStates:{check:X,init:2},debugFlags:{check:X,init:7}},members:{__jz:null,__FM:null,__FO:null,__FP:null,__FN:null,__FQ:null,__FR:null,__FS:null,__FT:null,__FU:null,__FV:false,__FW:true,isTerminated:function(){return this.__FW;}
,addState:function(bx){if(!bx instanceof qx.util.fsm.State){throw new Error(W+bl);}
;var by=bx.getName();if(by in this.__jz){throw new Error(r+by+w);}
;if(this.__FM==null){this.__FM=by;}
;this.__jz[by]=bx;}
,replaceState:function(bC,bB){if(!bC instanceof qx.util.fsm.State){throw new Error(W+bl);}
;var bA=bC.getName();var bz=this.__jz[bA];this.__jz[bA]=bC;if(bB){bz._bNeedDispose=true;}
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__FR[bG]=bH;this.__FS[bH]=bG;this.__FQ[bG]=bD;if(!bE){return;}
;if(typeof (bE)==j){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__FT[bF]){this.__FT[bF]={};}
;this.__FT[bF][bG]=true;if(!this.__FU[bG]){this.__FU[bG]=[];}
;this.__FU[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__FR[bL];if(this.__FU[bL]){for(var i=0;i<this.__FU[bL].length;i++ ){bJ=this.__FU[bL][i];delete this.__FT[bJ][bL];bM=true;for(bK in this.__FT[bJ]){bM=false;break;}
;if(bM){delete this.__FT[bJ];}
;}
;delete this.__FU[bL];}
;delete this.__FS[bI];delete this.__FR[bL];delete this.__FQ[bL];}
,getObject:function(bN){return this.__FQ[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__FS[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__FT[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__FR){var bT=this.__FR[bR];var bS=this.getObject(bR);this.debug(bR+c+bT);this.debug(br+bT+c+this.__FS[bT]);this.debug(br+bR+c+this.getObject(bR));this.debug(br+this.getObject(bR)+c+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__jz,"startState":this.__FM,"eventQueue":this.__FO,"blockedEvents":this.__FP,"savedStates":this.__FN,"friendlyToObject":this.__FQ,"friendlyToHash":this.__FR,"hashToFriendly":this.__FS,"groupToFriendly":this.__FT,"friendlyToGroups":this.__FU});}
,start:function(){this.__FW=false;var bV=this.__FM;if(bV==null){throw new Error(R);}
...
;this.__jz[bV].getAutoActionsBeforeOnentry()(this);if(bU){this.debug(this.getName()+v+bV+T);}
;this.__jz[bV].getOnentry()(this,null);if(bU){this.debug(this.getName()+v+bV+I);}
;this.__jz[bV].getAutoActionsAfterOnentry()(this);}
,pushState:function(bW){if(this.__FN.length>=this.getMaxSavedStates()){throw new Error(J);}
;if(bW===true){this.__FN.push(this.getState());}
else if(bW){this.__FN.push(bW);}
else {this.__FN.push(this.getPreviousState());}
;}
,popState:function(){if(this.__FN.length==0){throw new Error(o);}
;return this.__FN.pop();}
,postponeEvent:function(event){this.__FP.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__FO.push(event);}
else {this.__FO.unshift(event);}
;if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){var bY=this.getFriendlyName(event.getTarget());if(bX){this.debug(this.getName()+P+event.getType()+(bY?V+bY:H));}
else {this.debug(this.getName()+bv+event.getType()+(bY?V+bY:H));}
;}
;}
,eventListener:function(event){if(this.__FW){this.debug(this.getName()+Q+event.getType()+O);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__FX();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__FW){this.debug(this.getName()+Q+cb+O);return;}
...
,this,cg||20);}
,__FX:function(){if(this.__FV){return;}
;this.__FV=true;while(this.__FO.length>0){var event=this.__FO.pop();var ch=this.__FY(event);if(ch){event.dispose();}
;}
;this.__FV=false;}
,__FY:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+S+event.getType()+(ct?V+ct:H));}
;ck=this.getState();cn=this.__jz[ck];cq=cn.transitions;e=cn.getEvents()[event.getType()];if(!e){if(cu){this.debug(this.getName()+bs+event.getType()+l+q);}
;return true;}
;if(typeof (e)==bt){if(!ct){if(co){this.debug(this.getName()+f+event.getType()+B+event.getTarget()+l);}
;return true;}
;cp=e[ct];if(!cp){if(cu){this.debug(this.getName()+bs+event.getType()+l+bu+ct+bn);}
;return true;}
;}
else {cp=e;}
;switch(cp){case qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE:break;case qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED:if(cu){this.debug(this.getName()+bs+event.getType()+l+h);}
;this.__FP.unshift(event);return false;default:if(typeof (cp)==j){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bq+cp+A);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(z+ck+y+t+u+bp);};cl=cm.getNextState();if(typeof (cl)==j){if(!cl in this.__jz){throw new Error(m+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__FN.length==0){throw new Error(x+bm);}
;cl=this.__FN.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__FW=true;this.setNextState(null);break;default:throw new Error(p);};}
...
;cm.getOntransition()(this,event);if(cr){this.debug(this.getName()+v+ck+v+t+G);}
;cm.getAutoActionsAfterOntransition()(this);if(cr){this.debug(this.getName()+v+ck+d);}
...
;if(this.__FW){if(cr){this.debug(this.getName()+v+b);}
;this.fireDataEvent(F,ck);return true;}
;cn=this.__jz[this.getNextState()];this.setPreviousState(ck);this.setState(this.getNextState());this.setNextState(null);cj=ck;ck=cl;cl=undefined;if(cr){this.debug(this.getName()+v+ck+K);}
...
;cn.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__FP.length;i++ ){e=this.__FP.pop();this.__FO.unshift(e);}
;if(cs){this.debug(this.getName()+v+cj+c+this.getName()+v+ck);}
;return true;}
;if(cs){this.debug(this.getName()+v+ck+g+event.getType()+l+L);}
;return true;}
},destruct:function(){this._disposeArray(n);this._disposeArray(E);this.__FN=this.__jz=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="__FK",h="  var groupObjects = ",k="__FE",l="parameters",m="__FG",n=")",o=" fsm.getObject('",p="groups",q="Invalid function parameters: ",r="Invalid onexit type: ",s="  for (var i = 0; i < groupObjects.length; i++)",t="qx.util.fsm.Transition",u="context",v="string",w="autoActionsAfterOnentry",x="Invalid numeric value in events object: ",y='"',z="Invalid state name",A=" value: ",B="');",C="State ",D="autoActionsBeforeOnexit",E="autoActionsBeforeOnentry",F="}",G="__FI",H="events must be an object",I="onexit",J="{",K="Invalid function request type: ",L="): ",M=": ",N="expected array, found ",O="Invalid friendly name in 'objects' list: ",P="autoActionsAfterOnexit",Q="  fsm.debug(ex);",R="(",S="__FH",T="Invalid onentry type: ",U="    fsm.getObject(objName).",V=")(fsm)",W="events",X="undefined",Y="Invalid ",ba="try",bb="    var objName = groupObjects[i];",bc="Invalid value in events object: ",bd="').",be="objects",bf="expected object, found ",bg="__FD",bh="The events object must be provided in new state info",bi="catch(ex)",bj="  {",bk="    fsm.getGroupObjects('",bl="Invalid value in events object ",bm="Invalid transition: not an instance of ",bn="  }",bo="__FJ",bp="Invalid 'objects' list: expected array, got ",bq="Invalid numeric value in events object ",br="qx.util.fsm.State",bs="onentry",bt=";",bu="Invalid function request parameter type: ",bv="Invalid 'groups' list: expected array, got ",bw="fsm",bx=",",by="__FF",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(br,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(u,bE);for(var bC in bF){switch(bC){case bs:this.setOnentry(this.__FL(bF[bC],bE));break;case I:this.setOnexit(this.__FL(bF[bC],bE));break;case E:this.setAutoActionsBeforeOnentry(bF[bC]);break;case w:this.setAutoActionsAfterOnentry(bF[bC]);break;case D:this.setAutoActionsBeforeOnexit(bF[bC]);break;case P:this.setAutoActionsAfterOnexit(bF[bC]);break;case W:this.setEvents(bF[bC]);break;case u:break;default:this.setUserData(bC,bF[bC]);this.debug(C+bD+M+bz+bC);break;};}
;if(!this.getEvents()){throw new Error(bh);}
;this.transitions={};}
...
;var funcFragment;var func=ba+J;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(K+N+typeof (functionRequest));}
;for(var i=0;i<functionRequest.length;i++ ){objectAndGroupList=functionRequest[i];if(typeof (objectAndGroupList)!=bB){throw new Error(bu+bf+typeof (functionRequest[param]));}
;var params=objectAndGroupList[l];if(!params){params=[];}
else {if(!params instanceof Array){throw new Error(q+N+typeof (params));}
;}
;funcFragment=f+R;for(var j=0;j<params.length;j++ ){if(j!=0){funcFragment+=bx;}
;if(typeof (params[j])==c){funcFragment+=R+params[j]+V;}
else if(typeof (params[j])==v){funcFragment+=y+params[j]+y;}
else {funcFragment+=params[j];}
;}
;funcFragment+=n;var a=objectAndGroupList[be];if(!a){a=[];}
else if(!a instanceof Array){throw new Error(bp+typeof (a));}
;for(var j=0;j<a.length;j++ ){if(typeof (a[j])!=v){throw new Error(O+a[j]);}
;func+=o+a[j]+bd+funcFragment+bt;}
;var g=objectAndGroupList[p];if(g){if(!g instanceof Array){throw new Error(bv+typeof (g));}
;for(j=0;j<g.length;j++ ){func+=h+bk+g[j]+B+s+bj+bb+U+funcFragment+bt+bn;}
;}
;}
;}
;func+=F+bi+J+Q+F;return qx.lang.Function.bind(new Function(bw,func),context);}
},properties:{name:{transform:bg,nullable:true},onentry:{transform:k,nullable:true,init:function(bG,event){}
},onexit:{transform:by,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:S,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:G,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:bo,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:d,nullable:true,init:function(bL,event){}
},events:{transform:m,nullable:true}},members:{__FD:function(bM){if(typeof (bM)!=v||bM.length<1){throw new Error(z);}
;return bM;}
,__FE:function(bN){switch(typeof (bN)){case X:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(u));default:throw new Error(T+typeof (bN));};}
,__FF:function(bP){switch(typeof (bP)){case X:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(u));default:throw new Error(r+typeof (bP));};}
,__FG:function(bT){if(typeof (bT)!=bB){throw new Error(H);}
...
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==bA&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(bq+R+e+L+bS+M+bR[bS]);}
else if(typeof (bR[bS])!=v&&typeof (bR[bS])!=bA){throw new Error(bl+R+e+L+bS+M+bR[bS]);}
;}
;}
else if(typeof (bR)!=v&&typeof (bR)!=bA){throw new Error(bc+e+M+bT[e]);}
;}
;return bT;}
,__FH:function(bU){return qx.util.fsm.State._commonTransformAutoActions(E,bU,this.getUserData(u));}
,__FI:function(bV){return qx.util.fsm.State._commonTransformAutoActions(w,bV,this.getUserData(u));}
,__FJ:function(bW){return qx.util.fsm.State._commonTransformAutoActions(D,bW,this.getUserData(u));}
,__FK:function(bX){return qx.util.fsm.State._commonTransformAutoActions(P,bX,this.getUserData(u));}
,__FL:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bm+t);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="or one of the Fsm.StateChange constants",d="autoActionsAfterOntransition",e="context",g="string",h="predicate",i="__Gc",j="nextState must be an explicit state name, ",k="__Ga",l="__Ge",m=": ",n="String",o="number",p="Invalid transition nextState type: ",q="Transition ",r="Adding user-provided field to transition: ",s="ontransition",t="Invalid transition predicate type: ",u="boolean",v="__Gd",w="nextState",x="Invalid transition nextState value: ",y="Invalid ontransition type: ",z="qx.util.fsm.Transition",A="undefined",B="__Gb";qx.Class.define(z,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(e,E);for(var C in D){switch(C){case h:this.setPredicate(this.__FL(D[C],E));break;case w:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__FL(D[C],E));break;case d:this.setAutoActionsAfterOntransition(this.__FL(D[C],E));break;case s:this.setOntransition(this.__FL(D[C],E));break;case e:break;default:this.setUserData(C,D[C]);this.debug(q+F+m+r+C);break;};}
;}
,properties:{name:{check:n,nullable:true},predicate:{init:function(G,event){return true;}
,transform:k},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:B},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:i},autoActionsAfterOntransition:{init:function(I,event){}
,transform:v},ontransition:{init:function(J,event){}
,transform:l}},members:{__Ga:function(K){switch(typeof (K)){case A:return function(L,event){return true;}
...
,__Gb:function(N){switch(typeof (N)){case g:return N;case o:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(x+N+m+j+c);};break;default:throw new Error(p+typeof (N));};}
,__Gc:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(e));}
,__Gd:function(P){return qx.util.fsm.State._commonTransformAutoActions(d,P,this.getUserData(e));}
,__Ge:function(Q){switch(typeof (Q)){case A:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(e));default:throw new Error(y+typeof (Q));};}
,__FL:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
