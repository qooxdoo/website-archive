...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.mobile.PingPong.f9699f1c6a2a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;this.__Pa=this.__Pe(L);U.add(this.__Pa);var Q=this.__Pe(A);U.add(Q);this.__OQ=this.__Pf(L);this.__Pa.add(this.__OQ);this.__Pa.addListener(M,qx.lang.Function.bind(this.__va,this),this);this.__OP=this.__Pf(A);Q.add(this.__OP);Q.addListener(M,qx.lang.Function.bind(this.__va,this),this);this.__OR=this.__Pg();U.add(this.__OR);this.__OX=this.__Ph();this.__Pa.add(this.__OX);this.__OY=this.__Ph();Q.add(this.__OY);this.__OW=[0,0];this.__Pb();}
,__Pb:function(){this.__OR.setStyles({"left":k,"top":k});this.__OS=new qx.event.Timer(40);this.__OS.addListener(n,function(){var x=(parseInt(this.__OR.getStyle(L))+this.__OT*this.__OU);var y=(parseInt(this.__OR.getStyle(z))+this.__OT*this.__OV);this.__OR.setStyle(L,x+c);this.__OR.setStyle(z,y+c);this.__Pd();}
,this);this.__OS.start();}
,__Pc:function(V){this.__OS.stop();this.__OS.dispose();this.__OT=5;this.__OW[V]=this.__OW[V]+1;this.__OX.setAttribute(w,this.__OW[0]);this.__OY.setAttribute(w,this.__OW[1]);this.__Pb();}
,__Pd:function(){var X=this.getBoundsFor(this.__OR);if(X.top<=0){this.__OV=1;}
;if(X.bottom>=window.innerHeight-5){this.__OV=-1;}
;if(X.left<=0||X.right>=window.innerWidth){this.__Pc(X.left<=0?1:0);}
;var Y=this.getBoundsFor(this.__OQ);if(X.left<=Y.right&&X.bottom>=Y.top&&X.top<=Y.bottom){this.__OU=1;this.__OT=Math.min(this.__OT+1,20);}
;var W=this.getBoundsFor(this.__OP);if(X.right>=W.left&&X.bottom>=W.top&&X.top<=W.bottom){this.__OU=-1;this.__OT=Math.min(this.__OT+1,20);}
;}
,__va:function(e){var ba;if(qx.dom.Hierarchy.contains(this.__Pa.getDomElement(),e.getTarget())){ba=this.__OQ;}
else {ba=this.__OP;}
...
,__Pe:function(bd){var bc={"width":C,"height":h,"position":N,"backgroundColor":a};if(bd==L){bc[J]=u;}
else {bc[L]=C;bc[d]=u;}
;return new qx.html.Element(g,bc);}
,__Pf:function(bf){var be={"width":l,"height":t,"top":t,"position":N,"backgroundColor":K};be[bf]=F;return new qx.html.Element(g,be);}
,__Pg:function(){var bg={"width":F,"height":F,"position":N,"backgroundColor":K,"userSelect":I};return new qx.html.Element(g,bg);}
,__Ph:function(){var bh={"width":h,"height":h,"textAlign":r,"fontSize":H,"color":b,"fontFamily":B};var bi=new qx.html.Element(g,bh);bi.setAttribute(w,j);return bi;}
}});}
)();
