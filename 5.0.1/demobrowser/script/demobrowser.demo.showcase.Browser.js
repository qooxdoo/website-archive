...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.showcase.Browser.6274e7062cb0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var P=K.getMarginLeft()+K.getMarginRight();P=Math.max(P,this.getSpacing());var J=K.getSizeHint().width+P;this.__qm(K);G-=J;if(E&&E.getVisibility()!=o){E.setVisibility(o);G+=L;var H=E.getMarginLeft()+E.getMarginRight();G+=Math.max(H,this.getSpacing());}
;}
while(G>F);}
else if(this.__qj.length>0){do {var Q=this.__qj[0];if(Q){var P=Q.getMarginLeft()+Q.getMarginRight();P=Math.max(P,this.getSpacing());if(Q.getContentElement().getDomElement()==null){Q.syncAppearance();Q.invalidateLayoutCache();}
;var I=Q.getSizeHint().width;var O=false;if(this.__qj.length==1&&L>0){var N=P-this.getSpacing();var M=G-L+I+N;O=F>M;}
;if(F>G+I+P||O){this.__ql(Q);G+=I;if(E&&this.__qj.length==0){E.setVisibility(y);}
;}
else {return;}
;}
;}
while(F>=G&&this.__qj.length>0);}
;}
,__ql:function(R){R.setVisibility(o);this.__qj.shift();this.fireDataEvent(m,R);}
,__qm:function(S){if(!S){return;}
;this.__qj.unshift(S);S.setVisibility(y);this.fireDataEvent(A,S);}
,_getNextToHide:function(){for(var i=this.__qk.length-1;i>=0;i-- ){var T=this.__qk[i];if(T&&T.getVisibility&&T.getVisibility()==o){return T;}
;}
...
;}
;}
,setRemovePriority:function(X,W,Y){if(!Y&&this.__qk[W]!=undefined){throw new Error(v);}
;this.__qk[W]=X;}
...
;for(var i=0;i<this.__qj.length;i++ ){this.__qj[i].setVisibility(o);}
;this.__qj=[];}
;}
...
;bf.setVisibility(y);}
;}
,__qn:false,_setAllowMenuOpenHover:function(bg){this.__qn=bg;}
,_isAllowMenuOpenHover:function(){return this.__qn;}
...
(function(){var a="keyup",b="blur",c="Enter",d="Escape",f="Space",g="keydown",h="Left",j="Down",k="Up",l="singleton",m="qx.debug",n="interval",o="roll",p="event.touch",q="Object is no menu: ",r="pointerdown",s="qx.ui.menu.Manager",t="__pW",u="keypress",v="contextmenu",w="__pX",x="Right",y="__oC";qx.Class.define(s,{type:l,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__oC=[];var z=document.body;var A=qx.event.Registration;A.addListener(window.document.documentElement,r,this._onPointerDown,this,true);A.addListener(z,o,this._onRoll,this,true);A.addListener(z,g,this._onKeyUpDown,this,true);A.addListener(z,a,this._onKeyUpDown,this,true);A.addListener(z,u,this._onKeyPress,this,true);if(!qx.core.Environment.get(p)){qx.bom.Element.addListener(window,b,this.hideAll,this);}
;this.__pW=new qx.event.Timer();this.__pW.addListener(n,this._onOpenInterval,this);this.__pX=new qx.event.Timer();this.__pX.addListener(n,this._onCloseInterval,this);}
,members:{__pY:null,__qa:null,__pW:null,__pX:null,__oC:null,_getChild:function(B,D,C,G){var F=B.getChildren();var length=F.length;var E;for(var i=D;i<length&&i>=0;i+=C){E=F[i];if(E.isEnabled()&&!E.isAnonymous()&&E.isVisible()){return E;}
;}
...
;H=H.getLayoutParent();}
;return false;}
,_isMenuOpener:function(J){var I=this.__oC;for(var i=0;i<I.length;i++ ){if(I[i].getOpener()===J){return true;}
;}
;return false;}
,_getMenuButton:function(K){while(K){if(K instanceof qx.ui.menu.AbstractButton){return K;}
;K=K.getLayoutParent();}
;return null;}
,add:function(L){if(qx.core.Environment.get(m)){if(!(L instanceof qx.ui.menu.Menu)){throw new Error(q+L);}
;}
;var M=this.__oC;M.push(L);L.setZIndex(1e6+M.length);}
,remove:function(N){if(qx.core.Environment.get(m)){if(!(N instanceof qx.ui.menu.Menu)){throw new Error(q+N);}
;}
;var O=this.__oC;if(O){qx.lang.Array.remove(O,N);}
;}
,hideAll:function(){var P=this.__oC;if(P){for(var i=P.length-1;i>=0;i-- ){P[i].exclude();}
;}
;}
,getActiveMenu:function(){var Q=this.__oC;return Q.length>0?Q[Q.length-1]:null;}
,scheduleOpen:function(R){this.cancelClose(R);if(R.isVisible()){if(this.__pY){this.cancelOpen(this.__pY);}
;}
else if(this.__pY!=R){this.__pY=R;this.__pW.restartWith(R.getOpenInterval());}
;}
,scheduleClose:function(S){this.cancelOpen(S);if(!S.isVisible()){if(this.__qa){this.cancelClose(this.__qa);}
;}
else if(this.__qa!=S){this.__qa=S;this.__pX.restartWith(S.getCloseInterval());}
;}
,cancelOpen:function(T){if(this.__pY==T){this.__pW.stop();this.__pY=null;}
;}
,cancelClose:function(U){if(this.__qa==U){this.__pX.stop();this.__qa=null;}
;}
,_onOpenInterval:function(e){this.__pW.stop();this.__pY.open();this.__pY=null;}
,_onCloseInterval:function(e){this.__pX.stop();this.__qa.exclude();this.__qa=null;}
,preventContextMenuOnce:function(){qx.event.Registration.addListener(document.body,v,this.__qb,this,true);}
,__qb:function(e){var V=e.getTarget();V=qx.ui.core.Widget.getWidgetByElement(V,true);if(this._isInMenu(V)){e.stopPropagation();e.preventDefault();}
;qx.event.Registration.removeListener(document.body,v,this.__qb,this,true);}
...
;if(this.__oC.length>0&&!this._isInMenu(W)){this.hideAll();}
;}
,__qc:{"Enter":1,"Space":1},__nx:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var X=this.getActiveMenu();if(!X){return;}
;var Y=e.getKeyIdentifier();if(this.__nx[Y]||(this.__qc[Y]&&X.getSelectedButton())){e.stopPropagation();}
;}
,_onKeyPress:function(e){var bc=this.getActiveMenu();if(!bc){return;}
;var ba=e.getKeyIdentifier();var be=this.__nx[ba];var bb=this.__qc[ba];if(be){switch(ba){case k:this._onKeyPressUp(bc);break;case j:this._onKeyPressDown(bc);break;case h:this._onKeyPressLeft(bc);break;case x:this._onKeyPressRight(bc);break;case d:this.hideAll();break;};e.stopPropagation();e.preventDefault();}
...
;}
,_onRoll:function(e){var bK=e.getTarget();bK=qx.ui.core.Widget.getWidgetByElement(bK,true);if(this.__oC.length>0&&!this._isInMenu(bK)&&!this._isMenuOpener(bK)&&!e.getMomentum()){this.hideAll();}
;}
},destruct:function(){var bM=qx.event.Registration;var bL=document.body;bM.removeListener(window.document.documentElement,r,this._onPointerDown,this,true);bM.removeListener(bL,g,this._onKeyUpDown,this,true);bM.removeListener(bL,a,this._onKeyUpDown,this,true);bM.removeListener(bL,u,this._onKeyPress,this,true);this._disposeObjects(t,w);this._disposeArray(y);}
});}
)();
...
;this.__qf();}
,__qf:function(){if(this.isVisible()){if(this.getBlockBackground()){var V=this.getZIndex();this._blocker.blockContent(V-1);}
;}
...
,__qg:function(){var bn=this._getMenuBounds();if(!bn){this.addListenerOnce(n,this.__qg,this);return;}
...
;this.__qe=bp;qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(bq){this.getChildControl(h);if(this.__qe){this.__qe.call(this);delete this.__qe;}
;}
...
;this.__qg();}
;}
,_onPointerOver:function(e){var bt=qx.ui.menu.Manager.getInstance();bt.cancelClose(this);var bu=e.getTarget();if(bu.isEnabled()&&bu instanceof qx.ui.menu.AbstractButton){this.setSelectedButton(bu);var bs=bu.getMenu&&bu.getMenu();if(bs){bs.setOpener(bu);bt.scheduleOpen(bs);this.__qd=bs;}
else {var bv=this.getOpenedButton();if(bv){bt.scheduleClose(bv.getMenu());}
;if(this.__qd){bt.cancelOpen(this.__qd);this.__qd=null;}
;}
;}
...
;if(this.__qd){bw.cancelOpen(this.__qd);}
;}
;}
...
(function(){var a="qx.ui.menu.Layout",b="Integer",c="_applyLayoutChange";qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:b,init:0,apply:c},spanColumn:{check:b,init:1,nullable:true,apply:c},iconColumnWidth:{check:b,init:0,themeable:true,apply:c},arrowColumnWidth:{check:b,init:0,themeable:true,apply:c}},members:{__qh:null,_computeSizeHint:function(){var q=this._getLayoutChildren();var o,g,k;var e=this.getSpanColumn();var j=this.__qh=[0,0,0,0];var n=this.getColumnSpacing();var m=0;var f=0;for(var i=0,l=q.length;i<l;i++ ){o=q[i];if(o.isAnonymous()){continue;}
...
,getColumnSizes:function(){return this.__qh||null;}
...
;var x=this.__qi(y[0]);var w=x.getColumnSizes();var s=x.getSpacingX();var z=qx.lang.Array.sum(w)+s*(w.length-1);if(z<o){w[1]+=o-z;}
...
;}
;}
,__qi:function(B){while(!(B instanceof qx.ui.menu.Menu)){B=B.getLayoutParent();}
;return B;}
...
,__qo:function(){var y=this.getChildrenContainer().getChildren();y=y.filter(function(z){return z.getVisibility()==k;}
