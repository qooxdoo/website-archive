...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Flash.93e8d23efaa0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,createFlashDemo1:function(){var N=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));var L={flashVarText:z};var M=new qx.ui.embed.Flash(y).set({scale:E,variables:L});M.getContentElement().setParam(k,F);N.add(M,{flex:1});var O=new qx.ui.form.Button(D);O.addListener(h,function(){this.__cz.open();}
,this);N.add(O);return N;}
,__DM:null,__PM:null,__PN:null,__PO:null,createFlashDemo2:function(){this.self(arguments).setCallBackInstance(this);var R=new qx.ui.container.Composite(new qx.ui.layout.HBox(8));var Q=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));R.add(Q,{flex:1});Q.add(new qx.ui.basic.Label(t).set({rich:true}));Q.add(new qx.ui.basic.Label(g));this.__PM=new qx.ui.form.TextArea(r);this.__PM.setEnabled(false);Q.add(this.__PM,{flex:1});Q.add(new qx.ui.basic.Label(C));this.__PN=new qx.ui.form.TextArea(f);Q.add(this.__PN,{flex:1});var S=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,d));this.__PO=new qx.ui.form.Button(p);this.__PO.setEnabled(false);this.__PO.addListener(h,function(e){this.__DM.getFlashElement().sendMessage(this.__PN.getValue());}
,this);S.add(this.__PO);Q.add(S);var P=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));R.add(P,{flex:1});P.add(new qx.ui.basic.Label(s).set({rich:true}));this.__DM=new qx.ui.embed.Flash(n);this.__DM.setVariables({init:v});P.add(this.__DM,{flex:1});return R;}
,sendMessage:function(T){this.__PM.setValue(T);}
,initFlash:function(){if(this.__DM.getFlashElement().setup){this.__DM.getFlashElement().setup(u);this.__PO.setEnabled(true);}
else {alert(x+o+a);}
;}
},statics:{__PP:null,setCallBackInstance:function(U){demobrowser.demo.widget.Flash.__PP=U;}
,sendMessage:function(V){demobrowser.demo.widget.Flash.__PP.sendMessage(V);}
,flashReady:function(){demobrowser.demo.widget.Flash.__PP.initFlash();}
},destruct:function(){this._disposeObjects(i,w,B,b,l);}
});}
)();
...
,_checkLoading:function(){var bo=this.getSource();if(bo!=F&&bo!=null&&bo!=bf){if(!this.isLoaded()){if(!this.__DC){this.__DC=new Date().getTime();}
;var bp=new Date().getTime()-this.__DC;if(this.getLoadTimeout()>bp){var bq=qx.util.TimerManager.getInstance();bq.start(this._checkLoading,0,this,null,10);this.fireEvent(x);}
else {if(qx.core.Environment.get(u)){this.debug(A+bp);}
;this.fireEvent(P);this.__DC=null;}
;}
else {this.fireEvent(p);this.__DC=null;}
;}
;}
...
,_applyQuality:function(bB,bA){this.__DD(c,bB);}
,_applyScale:function(bD,bC){this.__DD(h,bD);}
,_applyWmode:function(bF,bE){this.__DD(m,bF);}
,_applyPlay:function(bH,bG){this.__DD(I,bH);}
,_applyLoop:function(bJ,bI){this.__DD(s,bJ);}
,_applyMenu:function(bL,bK){this.__DD(v,bL);}
,_applyAllowScriptAccess:function(bN,bM){this.__DD(t,bN);}
,_applyLiveConnect:function(bP,bO){this.__DD(z,bP);}
,__DD:function(bQ,bR){this.getContentElement().setParam(bQ,bR);qx.ui.core.queue.Layout.add(this);}
}});}
)();
(function(){var a="$$",b="Invalid attribute 'value'.",c="The id cannot be modified after initial creation",d="The variables cannot be modified after initial creation",e="qx.debug",f="$$widget",g="Invalid attribute 'key'.",h="movie",i="div",j="The attributes cannot be modified after initial creation",k="The source cannot be modified after initial creation",l="The params cannot be modified after initial creation",m="Invalid attribute 'value' expected String, Boolean or null.",n="id",o="qx.html.Flash";qx.Class.define(o,{extend:qx.html.Element,construct:function(p,q){qx.html.Element.call(this,i,p,q);this.__DJ={};this.__DK={};this.__DL={};}
,members:{__DJ:null,__DL:null,__DK:null,__DM:null,_createDomElement:function(){return qx.dom.Element.create(i);}
,createFlash:function(){this.__DM=qx.bom.Flash.create(this.getDomElement(),this.getAttributes(),this.__DK,this.__DJ);}
,setSource:function(r){if(qx.core.Environment.get(e)){qx.core.Assert.assertString(r,b);}
;if(this.__DM){throw new Error(k);}
;this.setAttribute(h,r);}
,setId:function(s){if(qx.core.Environment.get(e)){qx.core.Assert.assertString(s,b);}
;if(this.__DM){throw new Error(c);}
;this.setAttribute(n,s);}
,getVariables:function(){return this.__DK;}
,setVariables:function(t){if(qx.core.Environment.get(e)){qx.core.Assert.assertMap(t,b);}
;if(this.__DM){throw new Error(d);}
;this.__DK=t;}
,getAttributes:function(){return this.__DL;}
...
else if(this.__DM){throw new Error(j);}
;if(v===null||v===undefined){delete this.__DL[u];}
else {this.__DL[u]=v;}
;}
,getParams:function(){return this.__DJ;}
...
;}
;}
;if(this.__DM){throw new Error(l);}
;if(x===null||x===undefined){delete this.__DJ[w];}
else {this.__DJ[w]=x;}
;}
,getFlashElement:function(){return this.__DM;}
},destruct:function(){if(this.__DM){qx.bom.Flash.destroy(this.__DM);}
;this.__DJ=this.__DK=this.__DL=null;}
});}
)();
