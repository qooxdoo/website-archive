...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.DragDrop.4b0a478265c0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
);var M=new qx.ui.basic.Label(j);I.add(M,{left:260,top:20});var T=new qx.ui.form.List();T.setDroppable(true);I.add(T,{left:260,top:40});T.addListener(f,function(e){this.debug(E+e.getRelatedTarget());var bc=e.getData(C);for(var i=0,l=bc.length;i<l;i++ ){this.add(bc[i]);}
;}
...
;}
);L=new qx.ui.basic.Label(c);I.add(L,{left:380,top:20});var Q=new qx.ui.form.TextArea();Q.setDroppable(true);Q.setHeight(100);I.add(Q,{left:380,top:40});Q.addListener(f,function(e){this.debug(E+e.getRelatedTarget());this.setValue(e.getData(k).replace(/,/g,z));}
);Q.addListener(B,function(e){if(!e.supportsType(k)){e.preventDefault();}
;}
);var N=new qx.ui.basic.Label(m);I.add(N,{left:600,top:20});var G=this.__Jj=new qx.ui.form.List();G.setDraggable(true);G.setDroppable(true);G.setSelectionMode(u);I.add(G,{left:600,top:40});for(var i=0;i<20;i++ ){G.add(new qx.ui.form.ListItem(D+i,g));}
...
);G.addListener(b,function(e){var bf=e.getOriginalTarget();if(bf instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__Pt=bf;}
...
;}
);G.addListener(f,function(e){this.__Pu(e.getOriginalTarget(),e.getDragTarget());}
,this);H.addListener(f,function(e){this.__Pu(this.__Pt);}
,this);}
,__Pu:function(bi,bg){if(bi.classname!=n){return;}
;var bh=this.__Jj.getSortedSelection();if(bh.length===0){bh=[bg];}
else if(bh.indexOf(bg)==-1){bh=[bg];}
;for(var i=0,l=bh.length;i<l;i++ ){this.__Jj.addBefore(bh[i],bi);}
;}
