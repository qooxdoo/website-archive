...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.JsonToTree.8b9a80a07af9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="__ou",b="qx.ui.tooltip.ToolTip",c="Boolean",d="",f="mouse",g="pointerover",h="interval",i="_applyCurrent",j="widget",k="__or",l="qx.ui.tooltip.Manager",m="pointermove",n="focusout",o="__os",p="tooltip-error",q="singleton",r="pointerout";qx.Class.define(l,{type:q,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);qx.event.Registration.addListener(document.body,g,this.__oz,this,true);this.__or=new qx.event.Timer();this.__or.addListener(h,this.__ow,this);this.__os=new qx.event.Timer();this.__os.addListener(h,this.__ox,this);this.__ot={left:0,top:0};}
,properties:{current:{check:b,nullable:true,apply:i},showInvalidToolTips:{check:c,init:true},showToolTips:{check:c,init:true}},members:{__ot:null,__os:null,__or:null,__ou:null,__ov:null,getSharedTooltip:function(){if(!this.__ou){this.__ou=new qx.ui.tooltip.ToolTip().set({rich:true});}
;return this.__ou;}
,getSharedErrorTooltip:function(){if(!this.__ov){this.__ov=new qx.ui.tooltip.ToolTip().set({appearance:p,rich:true});this.__ov.setLabel(d);this.__ov.syncAppearance();}
;return this.__ov;}
,_applyCurrent:function(u,t){if(t&&qx.ui.core.Widget.contains(t,u)){return;}
;if(t){if(!t.isDisposed()){t.exclude();}
;this.__or.stop();this.__os.stop();}
;var v=qx.event.Registration;var s=document.body;if(u){this.__or.startWith(u.getShowTimeout());v.addListener(s,r,this.__oA,this,true);v.addListener(s,n,this.__oB,this,true);v.addListener(s,m,this.__oy,this,true);}
else {v.removeListener(s,r,this.__oA,this,true);v.removeListener(s,n,this.__oB,this,true);v.removeListener(s,m,this.__oy,this,true);}
;}
,__ow:function(e){var w=this.getCurrent();if(w&&!w.isDisposed()){this.__os.startWith(w.getHideTimeout());if(w.getPlaceMethod()==j){w.placeToWidget(w.getOpener());}
else {w.placeToPoint(this.__ot);}
;w.show();}
;this.__or.stop();}
,__ox:function(e){var x=this.getCurrent();if(x&&!x.isDisposed()){x.exclude();}
;this.__os.stop();this.resetCurrent();}
,__oy:function(e){var y=this.__ot;y.left=Math.round(e.getDocumentLeft());y.top=Math.round(e.getDocumentTop());}
,__oz:function(e){var z=qx.ui.core.Widget.getWidgetByElement(e.getTarget());this.__oy(e);this.showToolTip(z);}
...
,__oA:function(e){var F=qx.ui.core.Widget.getWidgetByElement(e.getTarget());if(!F){return;}
...
;}
,__oB:function(e){var I=qx.ui.core.Widget.getWidgetByElement(e.getTarget());if(!I){return;}
;var J=this.getCurrent();if(J&&J==I.getToolTip()){this.setCurrent(null);}
;}
},destruct:function(){qx.event.Registration.removeListener(document.body,g,this.__oz,this,true);this._disposeObjects(k,o,a);this.__ot=null;}
});}
)();
...
;var C=qx.module.Event.__cS;for(var i=0,l=E.length;i<l;i++ ){var D=E[i];if(qx.lang.Type.isFunction(B)){if(!C[D]){C[D]=[];}
;C[D].push(B);}
;}
;}
,$unregisterEventNormalization:function(I,F){if(!qx.lang.Type.isArray(I)){I=[I];}
;var G=qx.module.Event.__cS;for(var i=0,l=I.length;i<l;i++ ){var H=I[i];if(G[H]){qx.lang.Array.remove(G[H],F);}
;}
;}
...
;var M=qx.module.Event.__cT.on;for(var i=0,l=O.length;i<l;i++ ){var N=O[i];if(qx.lang.Type.isFunction(L)){if(!M[N]){M[N]=[];}
;M[N].push(L);}
;}
;if(!K){return;}
;var J=qx.module.Event.__cT.off;for(var i=0,l=O.length;i<l;i++ ){var N=O[i];if(qx.lang.Type.isFunction(K)){if(!J[N]){J[N]=[];}
;J[N].push(K);}
;}
;}
,$unregisterEventHook:function(U,R,Q){if(!qx.lang.Type.isArray(U)){U=[U];}
;var S=qx.module.Event.__cT.on;for(var i=0,l=U.length;i<l;i++ ){var T=U[i];if(S[T]){qx.lang.Array.remove(S[T],R);}
;}
;if(!Q){return;}
;var P=qx.module.Event.__cT.off;for(var i=0,l=U.length;i<l;i++ ){var T=U[i];if(P[T]){qx.lang.Array.remove(P[T],Q);}
;}
;}
,$getEventHookRegistry:function(){return qx.module.Event.__cT;}
},members:{on:function(bd,bb,bc,W){for(var i=0;i<this.length;i++ ){var V=this[i];var Y=bc||qxWeb(V);var X=qx.module.Event.__cT.on;var be=X[t]||[];if(X[bd]){be=be.concat(X[bd]);}
;for(var j=0,m=be.length;j<m;j++ ){be[j](V,bd,bb,bc);}
;var ba=function(bf,event){var bh=qx.module.Event.__cS;var bg=bh[t]||[];if(bh[bd]){bg=bg.concat(bh[bd]);}
...
;V.$$lastlistenerId=V.$$emitter.on(bd,ba,Y);V.$$emitter.getEntryById(V.$$lastlistenerId).useCapture=!!W;if(!V.__cV){V.__cV={};}
;if(!V.__cV[bd]){V.__cV[bd]={};}
;V.__cV[bd][V.$$lastlistenerId]=ba;if(!bc){if(!V.__cW){V.__cW={};}
;V.__cW[V.$$lastlistenerId]=Y;}
;}
;return this;}
,off:function(bt,bo,bq,bj){var br=(bo===null&&bq===null);for(var j=0;j<this.length;j++ ){var bi=this[j];if(!bi.__cV){continue;}
;var bv=[];if(bt!==null){bv.push(bt);}
else {for(var bm in bi.__cV){bv.push(bm);}
;}
;for(var i=0,l=bv.length;i<l;i++ ){for(var bu in bi.__cV[bv[i]]){var bs=bi.__cV[bv[i]][bu];if(br||bs==bo||bs.original==bo){var bl=typeof bi.__cW!==q&&bi.__cW[bu];var bw;if(!bq&&bl){bw=bi.__cW[bu];}
;var bp=bi.$$emitter.off(bv[i],bs,bw||bq);if(br||bs.original==bo){qx.bom.Event.removeNativeListener(bi,bv[i],bs,bj);}
;if(bp!==null){delete bi.__cV[bv[i]][bu];}
;if(bl){delete bi.__cW[bu];}
;}
;}
;var bk=qx.module.Event.__cT.off;var bn=bk[t]||[];if(bk[bt]){bn=bn.concat(bk[bt]);}
...
,__kz:function(cN,cQ){var cR=qx.io.ImageLoader;if(qx.core.Environment.get(w)){var cP=cQ.toLowerCase();var cO=qx.lang.String.startsWith;if(!cO(cP,e)&&!cO(cP,P)){var self=this.self(arguments);if(!self.__Ln){self.__Ln={};}
;if(!self.__Ln[cQ]){this.debug(bq+cQ);self.__Ln[cQ]=true;}
;}
;}
...
(function(){var a="mshtml",b='img',c="",d="qx.globalErrorHandling",e="load",f="qx.io.ImageLoader";qx.Bootstrap.define(f,{statics:{__cP:{},__kD:{width:null,height:null},__kE:/\.(png|gif|jpg|jpeg|bmp)\b/i,__kF:/^data:image\/(png|gif|jpg|jpeg|bmp)\b/i,isLoaded:function(g){var h=this.__cP[g];return !!(h&&h.loaded);}
,isFailed:function(j){var k=this.__cP[j];return !!(k&&k.failed);}
,isLoading:function(m){var n=this.__cP[m];return !!(n&&n.loading);}
,getFormat:function(r){var q=this.__cP[r];if(!q||!q.format){var o=this.__kF.exec(r);if(o!=null){var p=(q&&qx.lang.Type.isNumber(q.width)?q.width:this.__kD.width);var s=(q&&qx.lang.Type.isNumber(q.height)?q.height:this.__kD.height);q={loaded:true,format:o[1],width:p,height:s};}
;}
;return q?q.format:null;}
,getSize:function(t){var u=this.__cP[t];return u?{width:u.width,height:u.height}:this.__kD;}
...
;var z=document.createElement(b);var E=qx.lang.Function.listener(this.__kG,this,z,B);z.onload=E;z.onerror=E;z.src=B;D.element=z;}
;}
...
;}
;this.__cP[F]=null;}
,__kG:function(){var J=qx.core.Environment.select(d,{"true":qx.event.GlobalError.observeMethod(this.__kH),"false":this.__kH});J.apply(this,arguments);}
,__kH:function(event,L,K){var P=this.__cP[K];if(qx.bom.client.Engine.getName()==a&&parseFloat(qx.bom.client.Engine.getVersion())===11){document.body.appendChild(L);}
;var M=function(Q){return (Q&&Q.height!==0);}
;if(event.type===e&&M(L)){P.loaded=true;P.width=L.width;P.height=L.height;var N=this.__kE.exec(K);if(N!=null){P.format=N[1];}
;}
else {P.failed=true;}
...
(function(){var a="source",b="engine.name",c="",d="mshtml",e="px",f="px ",g="no-repeat",h="backgroundImage",i="scale",j="webkit",k="div",l="qx.html.Image",m="qx/static/blank.gif",n="backgroundPosition";qx.Class.define(l,{extend:qx.html.Element,members:{__kI:null,__kJ:null,tagNameHint:null,setPadding:function(o,p){this.__kJ=o;this.__kI=p;if(this.getNodeName()==k){this.setStyle(n,o+f+p+e);}
;}
...
;var u=this._getProperty(a);var r=this._getProperty(i);var v=r?i:g;if(u!=null){u=u||null;q.paddingTop=this.__kI;q.paddingLeft=this.__kJ;qx.bom.element.Decoration.update(s,u,v,q);}
;}
;}
...
;}
,getTagName:function(bf,be){if(be&&qx.core.Environment.get(q)&&this.__kL[bf]&&qx.lang.String.endsWith(be,d)){return k;}
;return this.__kM[bf];}
...
;}
;var bl;if(qx.core.Environment.get(q)&&this.__kL[bi]&&bj===z){var bm=this.__kO(bk);this.__kN(bh,bm.width,bm.height);bl=this.processAlphaFix(bh,bi,bk);}
else {delete bh.clip;if(bi===i){bl=this.__kP(bh,bi,bk);}
else if(bi===p||bi===P){bl=this.__kQ(bh,bi,bk);}
else {bl=this.__kT(bh,bi,bk);}
;}
;return bl;}
,__kN:function(bo,bn,bp){if(bo.width==null&&bn!=null){bo.width=bn+c;}
;if(bo.height==null&&bp!=null){bo.height=bp+c;}
;}
,__kO:function(bq){var br=qx.util.ResourceManager.getInstance().getImageWidth(bq)||qx.io.ImageLoader.getWidth(bq);var bs=qx.util.ResourceManager.getInstance().getImageHeight(bq)||qx.io.ImageLoader.getHeight(bq);return {width:br,height:bs};}
...
,__kP:function(bz,bA,by){var bB=qx.util.ResourceManager.getInstance().toUri(by);var bC=this.__kO(by);this.__kN(bz,bC.width,bC.height);return {src:bB,style:bz};}
,__kQ:function(bD,bE,bG){var bF=qx.util.ResourceManager.getInstance();var bJ=bF.getCombinedFormat(bG);var bL=this.__kO(bG);var bH;if(bJ){var bK=bF.getData(bG);var bI=bK[4];if(bJ==H){bH=bF.toDataUri(bG);}
else {bH=bF.toUri(bI);}
;if(bE===p){bD=this.__kR(bD,bK,bL.height);}
else {bD=this.__kS(bD,bK,bL.width);}
;return {src:bH,style:bD};}
else {if(qx.core.Environment.get(o)){this.__kV(bG);}
...
;}
,__kR:function(bM,bN,bP){var bO=qx.util.ResourceManager.getInstance().getImageHeight(bN[4]);bM.clip={top:-bN[6],height:bP};bM.height=bO+c;if(bM.top!=null){bM.top=(parseInt(bM.top,10)+bN[6])+c;}
else if(bM.bottom!=null){bM.bottom=(parseInt(bM.bottom,10)+bP-bO-bN[6])+c;}
;return bM;}
,__kS:function(bR,bS,bQ){var bT=qx.util.ResourceManager.getInstance().getImageWidth(bS[4]);bR.clip={left:-bS[5],width:bQ};bR.width=bT+c;if(bR.left!=null){bR.left=(parseInt(bR.left,10)+bS[5])+c;}
else if(bR.right!=null){bR.right=(parseInt(bR.right,10)+bQ-bT-bS[5])+c;}
;return bR;}
,__kT:function(bU,bV,bY){var bX=qx.util.ResourceManager.getInstance();var bW=bX.getCombinedFormat(bY);var ch=this.__kO(bY);if(bW&&bV!==b){var cg=bX.getData(bY);var ce=cg[4];if(bW==H){var cb=bX.toDataUri(bY);var ca=0;var cc=0;}
...
;return {style:bU};}
else {var top=bU.paddingTop||0;var ci=bU.paddingLeft||0;bU.backgroundPosition=ci+g+top+c;if(qx.core.Environment.get(o)){if(bV!==b){this.__kV(bY);}
;}
;this.__kN(bU,ch.width,ch.height);this.__kU(bU,bY,bV);return {style:bU};}
;}
,__kU:function(cj,cm,ck){var top=null;var cp=null;if(cj.backgroundPosition){var cl=cj.backgroundPosition.split(K);cp=parseInt(cl[0],10);if(isNaN(cp)){cp=cl[0];}
...
;}
,__kV:function(cq){if(this.DEBUG&&qx.util.ResourceManager.getInstance().has(cq)&&cq.indexOf(a)==-1){if(!this.__kK[cq]){qx.log.Logger.debug(l+cq);this.__kK[cq]=true;}
;}
;}
}});}
)();
(function(){var a="')",b="gecko",c="background-image:url(",d="0",e=");",f="",g="px",h="number",i=")",j="background-repeat:",k="engine.version",l="data:",m=" ",n="qx.bom.element.Background",o=";",p="url(",q="background-position:",r="base64",s="url('",t="engine.name",u="'";qx.Class.define(n,{statics:{__kW:[c,null,e,q,null,o,j,null,o],__kX:{backgroundImage:null,backgroundPosition:null,backgroundRepeat:null},__kY:function(z,top){var v=qx.core.Environment.get(t);var x=qx.core.Environment.get(k);if(v==b&&x<1.9&&z==top&&typeof z==h){top+=0.01;}
;if(z){var y=(typeof z==h)?z+g:z;}
else {y=d;}
;if(top){var w=(typeof top==h)?top+g:top;}
else {w=d;}
;return y+m+w;}
,__la:function(A){var String=qx.lang.String;var B=A.substr(0,50);return String.startsWith(B,l)&&String.contains(B,r);}
,compile:function(F,D,H,top){var G=this.__kY(H,top);var E=qx.util.ResourceManager.getInstance().toUri(F);if(this.__la(E)){E=u+E+u;}
;var C=this.__kW;C[1]=E;C[4]=G;C[7]=D;return C.join(f);}
,getStyles:function(L,J,N,top){if(!L){return this.__kX;}
;var M=this.__kY(N,top);var K=qx.util.ResourceManager.getInstance().toUri(L);var O;if(this.__la(K)){O=s+K+a;}
else {O=p+K+i;}
...
,"false":null}),_clearSeparators:function(){var m=this.__mk;if(m instanceof qx.ui.core.LayoutItem){m.clearSeparators();}
;}
,_renderSeparator:function(n,o){this.__mk.renderSeparator(n,o);}
,connectToWidget:function(p){if(p&&this.__mk){throw new Error(e);}
;this.__mk=p;this.invalidateChildrenCache();}
,_getWidget:function(){return this.__mk;}
,_applyLayoutChange:function(){if(this.__mk){this.__mk.scheduleLayoutUpdate();}
;}
,_getLayoutChildren:function(){return this.__mk.getLayoutChildren();}
},destruct:function(){this.__mk=this.__eb=null;}
});}
)();
...
(function(){var a="locale",b="_applyLocale",c="",d="changeLocale",e="_",f="Locale: ",g="C",h="locale.variant",j="qx.dynlocale",k=" not available.",l="qx.locale.Manager",m="String",n="singleton",o="qx.debug";qx.Class.define(l,{type:n,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__le=qx.$$translations||{};this.__lf=qx.$$locales||{};var p=qx.core.Environment.get(a);var q=qx.core.Environment.get(h);if(q!==c){p+=e+q;}
;this.__lg=p;this.setLocale(p||this.__lh);}
...
},properties:{locale:{check:m,nullable:true,apply:b,event:d}},members:{__lh:g,__li:null,__lj:null,__le:null,__lf:null,__lg:null,getLanguage:function(){return this.__lj;}
...
(function(){var a="qx.bom.client.Locale",b="-",c="locale",d="",e="android",f="locale.variant";qx.Bootstrap.define(a,{statics:{getLocale:function(){var g=qx.bom.client.Locale.__lm();var h=g.indexOf(b);if(h!=-1){g=g.substr(0,h);}
;return g;}
,getVariant:function(){var i=qx.bom.client.Locale.__lm();var k=d;var j=i.indexOf(b);if(j!=-1){k=i.substr(j+1);}
;return k;}
,__lm:function(){var l=(navigator.userLanguage||navigator.language||d);if(qx.bom.client.OperatingSystem.getName()==e){var m=/(\w{2})-(\w{2})/i.exec(navigator.userAgent);if(m){l=m[0];}
;}
...
(function(){var a="blur",b="qx.debug",c="qx.ui.popup.Manager",d="Object is no popup: ",f="pointerdown",g="__oC",h="singleton";qx.Class.define(c,{type:h,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__oC=[];qx.event.Registration.addListener(document.documentElement,f,this.__oE,this,true);qx.bom.Element.addListener(window,a,this.hideAll,this);}
,members:{__oC:null,add:function(j){if(qx.core.Environment.get(b)){if(!(j instanceof qx.ui.popup.Popup)){throw new Error(d+j);}
;}
;this.__oC.push(j);this.__oD();}
,remove:function(k){if(qx.core.Environment.get(b)){if(!(k instanceof qx.ui.popup.Popup)){throw new Error(d+k);}
;}
;qx.lang.Array.remove(this.__oC,k);this.__oD();}
,hideAll:function(){var l=this.__oC.length,m={};while(l-- ){m=this.__oC[l];if(m.getAutoHide()){m.exclude();}
;}
;}
,__oD:function(){var n=1e7;for(var i=0;i<this.__oC.length;i++ ){this.__oC[i].setZIndex(n++ );}
;}
,__oE:function(e){var p=qx.ui.core.Widget.getWidgetByElement(e.getTarget());var q=this.__oC;for(var i=0;i<q.length;i++ ){var o=q[i];if(!o.getAutoHide()||p==o||qx.ui.core.Widget.contains(o,p)){continue;}
;o.exclude();}
;}
},destruct:function(){qx.event.Registration.removeListener(document.documentElement,f,this.__oE,this,true);this._disposeArray(g);}
});}
)();
...
;}
,properties:{alignX:{check:[o,j,p],init:o,apply:c},alignY:{check:[n,b,g],init:n,apply:c},spacing:{check:q,init:0,apply:c},separator:{check:a,nullable:true,apply:c},reversed:{check:k,init:false,apply:e}},members:{__nF:null,__nG:null,__nH:null,__gW:null,_applyReversed:function(){this._invalidChildrenCache=true;this._applyLayoutChange();}
,__nI:function(){var B=this._getLayoutChildren();var length=B.length;var y=false;var w=this.__nF&&this.__nF.length!=length&&this.__nG&&this.__nF;var z;var x=w?this.__nF:new Array(length);var A=w?this.__nG:new Array(length);if(this.getReversed()){B=B.concat().reverse();}
...
;}
;if(!w){this.__nF=x;this.__nG=A;}
;this.__nH=y;this.__gW=B;delete this._invalidChildrenCache;}
...
;}
,"false":null}),renderLayout:function(U,O,T){if(this._invalidChildrenCache){this.__nI();}
...
;var i,V,Q,P;var X=[];var L=H;for(i=0;i<length;i+=1){P=this.__nF[i];Q=P!=null?Math.floor((U-H)*P):K[i].getSizeHint().width;X.push(Q);L+=Q;}
;if(this.__nH&&L!=U){var N={};var R,F;for(i=0;i<length;i+=1){R=this.__nG[i];if(R>0){M=K[i].getSizeHint();N[i]={min:M.minWidth,value:X[i],max:M.maxWidth,flex:R};}
;}
...
;}
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__nI();}
;var bs=qx.ui.layout.Util;var bf=this.__gW;var bk=0,bl=0,be=0;var bi=0,bj=0;var bp,bg,br;for(var i=0,l=bf.length;i<l;i+=1){bp=bf[i];bg=bp.getSizeHint();bl+=bg.width;var bo=this.__nG[i];var bh=this.__nF[i];if(bo){bk+=bg.minWidth;}
...
},destruct:function(){this.__nF=this.__nG=this.__gW=null;}
});}
)();
...
;}
,_applyValue:function(br,bq){this.getContentElement().setValue(br);this.__mm=true;qx.ui.core.queue.Layout.add(this);}
...
(function(){var a="value",b="qx.html.Label",c="The label mode cannot be modified after initial creation",d='hidden';qx.Class.define(b,{extend:qx.html.Element,members:{__ms:null,_applyProperty:function(name,e){qx.html.Element.prototype._applyProperty.call(this,name,e);if(name==a){var f=this.getDomElement();qx.bom.Label.setValue(f,e);}
;}
,_createDomElement:function(){var h=this.__ms;var g=qx.bom.Label.create(this._content,h);g.style.overflow=d;return g;}
...
;j=!!j;if(this.__ms==j){return this;}
;this.__ms=j;return this;}
,setValue:function(l){this._setProperty(a,l);return this;}
,getValue:function(){return this._getProperty(a);}
}});}
)();
(function(){var a="text",b="function",c="px",d="http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul",e="crop",f="nowrap",g="end",h="div",i="100%",j="auto",k="0",l="css.textoverflow",m="qx.debug",n="html.xul",o="value",p="visible",q="qx.bom.Label",r="",s="ellipsis",t="normal",u="inherit",v="block",w="label",x="-1000px",y="hidden",z="absolute";qx.Bootstrap.define(q,{statics:{__mt:{fontFamily:1,fontSize:1,fontWeight:1,fontStyle:1,lineHeight:1},__mu:function(){var A=this.__mw(false);document.body.insertBefore(A,document.body.firstChild);return this._textElement=A;}
,__mv:function(){var B=this.__mw(true);document.body.insertBefore(B,document.body.firstChild);return this._htmlElement=B;}
,__mw:function(E){var C=qx.dom.Element.create(h);var D=C.style;D.width=D.height=j;D.left=D.top=x;D.visibility=y;D.position=z;D.overflow=p;D.display=v;if(E){D.whiteSpace=t;}
else {D.whiteSpace=f;if(!qx.core.Environment.get(l)&&qx.core.Environment.get(n)){var F=document.createElementNS(d,w);var D=F.style;D.padding=k;D.margin=k;D.width=j;for(var G in this.__mt){D[G]=u;}
;C.appendChild(F);}
;}
;return C;}
,__mx:function(I){var H={};if(I){H.whiteSpace=t;}
...
else if(!qx.core.Environment.get(l)&&qx.core.Environment.get(n)){var N=L.document.createElementNS(d,w);var K=N.style;K.cursor=u;K.color=u;K.overflow=y;K.maxWidth=i;K.padding=k;K.margin=k;K.width=j;for(var O in this.__mt){N.style[O]=u;}
;N.setAttribute(e,g);J.appendChild(N);}
else {qx.bom.element.Style.setStyles(J,this.__mx(M));}
;if(content){this.setValue(J,content);}
;return J;}
,__my:null,setSanitizer:function(P){if(qx.core.Environment.get(m)){if(P){qx.core.Assert.assertFunction(P);}
;}
;qx.bom.Label.__my=P;}
,setValue:function(R,Q){Q=Q||r;if(R.useHtml){if(qx.bom.Label.__my&&typeof (qx.bom.Label.__my)===b){Q=qx.bom.Label.__my(Q);}
;R.innerHTML=Q;}
...
;}
,getHtmlSize:function(content,T,U){var V=this._htmlElement||this.__mv();V.style.width=U!=undefined?U+c:j;V.innerHTML=content;return this.__mz(V,T);}
,getTextSize:function(X,W){var Y=this._textElement||this.__mu();if(!qx.core.Environment.get(l)&&qx.core.Environment.get(n)){Y.firstChild.setAttribute(o,X);}
else {qx.bom.element.Attribute.set(Y,a,X);}
;return this.__mz(Y,W);}
,__mz:function(be,ba){var bb=this.__mt;if(!ba){ba={};}
...
(function(){var a="__lp",b="_applyBlockerColor",c="Number",d="qx.ui.core.MBlocker",e="_applyBlockerOpacity",f="Color";qx.Mixin.define(d,{properties:{blockerColor:{check:f,init:null,nullable:true,apply:b,themeable:true},blockerOpacity:{check:c,init:1,apply:e,themeable:true}},members:{__lp:null,_createBlocker:function(){return new qx.ui.core.Blocker(this);}
...
,isBlocked:function(){return this.__lp&&this.__lp.isBlocked();}
,unblock:function(){if(this.__lp){this.__lp.unblock();}
;}
,forceUnblock:function(){if(this.__lp){this.__lp.forceUnblock();}
;}
,blockContent:function(k){this.getBlocker().blockContent(k);}
,getBlocker:function(){if(!this.__lp){this.__lp=this._createBlocker();}
;return this.__lp;}
},destruct:function(){this._disposeObjects(a);}
});}
)();
(function(){var a="qx.dyntheme",b="backgroundColor",c="_applyOpacity",d="Boolean",f="px",g="keydown",h="deactivate",j="changeTheme",k="opacity",l="__eg",m="Tab",n="qx.event.type.Event",o="move",p="Color",q="resize",r="__lp",s="zIndex",t="appear",u="qx.ui.root.Abstract",v="keyup",w="keypress",x="Number",y="unblocked",z="qx.ui.core.Blocker",A="disappear",B="blocked",C="_applyColor";qx.Class.define(z,{extend:qx.core.Object,events:{blocked:n,unblocked:n},construct:function(D){qx.core.Object.call(this);this._widget=D;D.addListener(q,this.__lu,this);D.addListener(o,this.__lu,this);D.addListener(A,this.__lw,this);if(qx.Class.isDefined(u)&&D instanceof qx.ui.root.Abstract){this._isRoot=true;this.setKeepBlockerActive(true);}
;if(qx.core.Environment.get(a)){qx.theme.manager.Meta.getInstance().addListener(j,this._onChangeTheme,this);}
;this.__lq=[];this.__lr=[];}
,properties:{color:{check:p,init:null,nullable:true,apply:C,themeable:true},opacity:{check:x,init:1,apply:c,themeable:true},keepBlockerActive:{check:d,init:false}},members:{__lp:null,__ls:0,__lq:null,__lr:null,__eg:null,_widget:null,_isRoot:false,__lt:null,__lu:function(e){var E=e.getData();if(this.isBlocked()){this._updateBlockerBounds(E);}
;}
,__lv:function(){this._updateBlockerBounds(this._widget.getBounds());if(this._widget.isRootWidget()){this._widget.getContentElement().add(this.getBlockerElement());}
else {this._widget.getLayoutParent().getContentElement().add(this.getBlockerElement());}
;}
,__lw:function(){if(this.isBlocked()){this.getBlockerElement().getParent().remove(this.getBlockerElement());this._widget.addListenerOnce(t,this.__lv,this);}
;}
...
,_applyColor:function(I,H){var G=qx.theme.manager.Color.getInstance().resolve(I);this.__lx(b,G);}
,_applyOpacity:function(K,J){this.__lx(k,K);}
,_onChangeTheme:qx.core.Environment.select(a,{"true":function(){this._applyColor(this.getColor());}
,"false":null}),__lx:function(M,N){var L=[];this.__lp&&L.push(this.__lp);for(var i=0;i<L.length;i++ ){L[i].setStyle(M,N);}
;}
,_backupActiveWidget:function(){var O=qx.event.Registration.getManager(window).getHandler(qx.event.handler.Focus);this.__lq.push(qx.ui.core.Widget.getWidgetByElement(O.getActive()));this.__lr.push(qx.ui.core.Widget.getWidgetByElement(O.getFocus()));if(this._widget.isFocusable()){this._widget.focus();}
;}
,_restoreActiveWidget:function(){var Q;var P=this.__lr.length;if(P>0){Q=this.__lr.pop();if(Q&&!Q.isDisposed()&&Q.isFocusable()){Q.focus();}
;}
;var R=this.__lq.length;if(R>0){Q=this.__lq.pop();if(Q&&!Q.isDisposed()){Q.activate();}
;}
;}
,__ly:function(){return new qx.html.Blocker(this.getColor(),this.getOpacity());}
,getBlockerElement:function(S){if(!this.__lp){this.__lp=this.__ly();this.__lp.setStyle(s,15);if(!S){if(this._isRoot){S=this._widget;}
else {S=this._widget.getLayoutParent();}
;}
;S.getContentElement().add(this.__lp);this.__lp.exclude();}
;return this.__lp;}
,block:function(){this._block();}
,_block:function(T,V){if(!this._isRoot&&!this._widget.getLayoutParent()){this.__lt=this._widget.addListenerOnce(t,this._block.bind(this,T));return;}
...
;this.__ls++ ;if(this.__ls<2){this._backupActiveWidget();var W=this._widget.getBounds();if(W){this._updateBlockerBounds(W);}
;U.include();if(!V){U.activate();}
;U.addListener(h,this.__lB,this);U.addListener(w,this.__lA,this);U.addListener(g,this.__lA,this);U.addListener(v,this.__lA,this);this.fireEvent(B,qx.event.type.Event);}
;}
,isBlocked:function(){return this.__ls>0;}
,unblock:function(){if(this.__lt){this._widget.removeListenerById(this.__lt);}
;if(!this.isBlocked()){return;}
;this.__ls-- ;if(this.__ls<1){this.__lz();this.__ls=0;}
;}
,forceUnblock:function(){if(!this.isBlocked()){return;}
;this.__ls=0;this.__lz();}
,__lz:function(){this._restoreActiveWidget();var X=this.getBlockerElement();X.removeListener(h,this.__lB,this);X.removeListener(w,this.__lA,this);X.removeListener(g,this.__lA,this);X.removeListener(v,this.__lA,this);X.exclude();this.fireEvent(y,qx.event.type.Event);}
,blockContent:function(Y){this._block(Y,true);}
,__lA:function(e){if(e.getKeyIdentifier()==m){e.stop();}
;}
...
;this._widget.removeListener(q,this.__lu,this);this._widget.removeListener(o,this.__lu,this);this._widget.removeListener(t,this.__lv,this);this._widget.removeListener(A,this.__lw,this);if(this.__lt){this._widget.removeListenerById(this.__lt);}
;this._disposeObjects(r,l);this.__lq=this.__lr=this._widget=null;}
});}
)();
...
;e.preventDefault();}
,__oH:function(e){if(e.getKeyIdentifier()!==m){return;}
...
(function(){var a="keypress",b="focusout",c="activate",d="Tab",f="__oI",g="singleton",h="deactivate",j="focusin",k="qx.ui.core.FocusHandler";qx.Class.define(k,{extend:qx.core.Object,type:g,construct:function(){qx.core.Object.call(this);this.__oI={};}
,members:{__oI:null,__oJ:null,__oK:null,__oL:null,connectTo:function(m){m.addListener(a,this.__hJ,this);m.addListener(j,this._onFocusIn,this,true);m.addListener(b,this._onFocusOut,this,true);m.addListener(c,this._onActivate,this,true);m.addListener(h,this._onDeactivate,this,true);}
,addRoot:function(n){this.__oI[n.$$hash]=n;}
,removeRoot:function(o){delete this.__oI[o.$$hash];}
,getActiveWidget:function(){return this.__oJ;}
,isActive:function(p){return this.__oJ==p;}
,getFocusedWidget:function(){return this.__oK;}
,isFocused:function(q){return this.__oK==q;}
,isFocusRoot:function(r){return !!this.__oI[r.$$hash];}
,_onActivate:function(e){var t=e.getTarget();this.__oJ=t;var s=this.__oM(t);if(s!=this.__oL){this.__oL=s;}
;}
,_onDeactivate:function(e){var u=e.getTarget();if(this.__oJ==u){this.__oJ=null;}
;}
,_onFocusIn:function(e){var v=e.getTarget();if(v!=this.__oK){this.__oK=v;v.visualizeFocus();}
;}
,_onFocusOut:function(e){var w=e.getTarget();if(w==this.__oK){this.__oK=null;w.visualizeBlur();}
;}
,__hJ:function(e){if(e.getKeyIdentifier()!=d){return;}
;if(!this.__oL){return;}
;e.stopPropagation();e.preventDefault();var x=this.__oK;if(!e.isShiftPressed()){var y=x?this.__oQ(x):this.__oO();}
else {var y=x?this.__oR(x):this.__oP();}
;if(y){y.tabFocus();}
;}
,__oM:function(z){var A=this.__oI;while(z){if(A[z.$$hash]){return z;}
;z=z.getLayoutParent();}
;return null;}
,__oN:function(I,H){if(I===H){return 0;}
...
;return 0;}
,__oO:function(){return this.__oU(this.__oL,null);}
,__oP:function(){return this.__oV(this.__oL,null);}
,__oQ:function(M){var N=this.__oL;if(N==M){return this.__oO();}
;while(M&&M.getAnonymous()){M=M.getLayoutParent();}
;if(M==null){return [];}
;var O=[];this.__oS(N,M,O);O.sort(this.__oN);var P=O.length;return P>0?O[0]:this.__oO();}
,__oR:function(Q){var R=this.__oL;if(R==Q){return this.__oP();}
;while(Q&&Q.getAnonymous()){Q=Q.getLayoutParent();}
;if(Q==null){return [];}
;var S=[];this.__oT(R,Q,S);S.sort(this.__oN);var T=S.length;return T>0?S[T-1]:this.__oP();}
,__oS:function(parent,U,V){var X=parent.getLayoutChildren();var W;for(var i=0,l=X.length;i<l;i++ ){W=X[i];if(!(W instanceof qx.ui.core.Widget)){continue;}
;if(!this.isFocusRoot(W)&&W.isEnabled()&&W.isVisible()){if(W.isTabable()&&this.__oN(U,W)<0){V.push(W);}
;this.__oS(W,U,V);}
;}
;}
,__oT:function(parent,Y,ba){var bc=parent.getLayoutChildren();var bb;for(var i=0,l=bc.length;i<l;i++ ){bb=bc[i];if(!(bb instanceof qx.ui.core.Widget)){continue;}
;if(!this.isFocusRoot(bb)&&bb.isEnabled()&&bb.isVisible()){if(bb.isTabable()&&this.__oN(Y,bb)>0){ba.push(bb);}
;this.__oT(bb,Y,ba);}
;}
;}
,__oU:function(parent,bd){var bf=parent.getLayoutChildren();var be;for(var i=0,l=bf.length;i<l;i++ ){be=bf[i];if(!(be instanceof qx.ui.core.Widget)){continue;}
;if(!this.isFocusRoot(be)&&be.isEnabled()&&be.isVisible()){if(be.isTabable()){if(bd==null||this.__oN(be,bd)<0){bd=be;}
;}
;bd=this.__oU(be,bd);}
;}
;return bd;}
,__oV:function(parent,bg){var bi=parent.getLayoutChildren();var bh;for(var i=0,l=bi.length;i<l;i++ ){bh=bi[i];if(!(bh instanceof qx.ui.core.Widget)){continue;}
;if(!this.isFocusRoot(bh)&&bh.isEnabled()&&bh.isVisible()){if(bh.isTabable()){if(bg==null||this.__oN(bh,bg)>0){bg=bh;}
;}
;bg=this.__oV(bh,bg);}
;}
;return bg;}
},destruct:function(){this._disposeMap(f);this.__oK=this.__oJ=this.__oL=null;}
});}
)();
(function(){var a="touchmove",b="os.name",c="-webkit-overflow-scrolling",d="touch",f="paddingLeft",g="div",h="100%",i="The root widget does not support 'left', or 'top' paddings!",j="0px",k="The application could not be started due to a missing body tag in the HTML file!",l="ios",m="overflowY",n="resize",o="",p="$$widget",q="paddingTop",r="engine.name",s="none",t="webkit",u="-webkit-backface-visibility",v="touch-action",w="qx.ui.root.Application",x="hidden",y="tap",z="overflowX",A="absolute";qx.Class.define(w,{extend:qx.ui.root.Abstract,construct:function(B){this.__cz=qx.dom.Node.getWindow(B);this.__oW=B;qx.ui.root.Abstract.call(this);qx.event.Registration.addListener(this.__cz,n,this._onResize,this);this._setLayout(new qx.ui.layout.Canvas());qx.ui.core.queue.Layout.add(this);qx.ui.core.FocusHandler.getInstance().connectTo(this);this.getContentElement().disableScrolling();this.getContentElement().setStyle(u,x);this.addListener(a,this.__oX,this);if(qx.core.Environment.get(b)==l){this.getContentElement().addListener(y,function(e){var C=qx.ui.core.Widget.getWidgetByElement(e.getTarget());while(C&&!C.isFocusable()){C=C.getLayoutParent();}
;if(C&&C.isFocusable()){C.getContentElement().focus();}
;}
,this,true);}
;}
,members:{__cz:null,__oW:null,_createContentElement:function(){var D=this.__oW;if((qx.core.Environment.get(r)==t)){if(!D.body){alert(k);}
;}
...
,__oX:function(e){var M=e.getOriginalTarget();while(M&&M.style){var Q=qx.bom.element.Style.get(M,v)!==s&&qx.bom.element.Style.get(M,v)!==o;var P=qx.bom.element.Style.get(M,c)===d;var O=qx.bom.element.Style.get(M,z)!=x;var N=qx.bom.element.Style.get(M,m)!=x;if(Q||P||N||O){return;}
;M=M.parentNode;}
;e.preventDefault();}
},destruct:function(){this.__cz=this.__oW=null;}
});}
)();
...
(function(){var a="name",b="Loading...",c="model",d="tree.json</a> and bind the items to the tree widget.",e="Loading the json file <a href='",f="state",g="kids",h="demobrowser/demo/data/tree.json",i="value",j="demobrowser.demo.data.JsonToTree",k="' target='_blank'>",l="<b>Tree bound to data in a json file</b><br/>",m="loaded";qx.Class.define(j,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var p=new qx.ui.tree.Tree();this.getRoot().add(p,{left:10,top:80});p.setWidth(200);p.setHeight(300);var q=new qx.data.controller.Tree(null,p,g,a);var o=qx.util.ResourceManager.getInstance().toUri(h);var r=new qx.data.store.Json(o);var status=new qx.ui.basic.Label(b);this.getRoot().add(status,{left:220,top:80});r.bind(c,q,c);r.bind(f,status,i);r.addListener(m,function(s){p.getRoot().setOpen(true);}
,this);var n=new qx.ui.basic.Label();n.setRich(true);n.setWidth(260);n.setValue(l+e+o+k+d);this.getRoot().add(n,{left:10,top:10});}
}});}
)();
...
(function(){var a="",b="qx.ui.form.MForm",c="Boolean",d="_applyValid",f="changeLocale",g="changeRequired",h="changeValid",i="qx.dynlocale",j="changeInvalidMessage",k="String",l="invalid";qx.Mixin.define(b,{construct:function(){if(qx.core.Environment.get(i)){qx.locale.Manager.getInstance().addListener(f,this.__mA,this);}
;}
...
;this._fireChange();}
,_setLeadItem:function(cb){var ca=this.__nt;if(ca!==null){this._styleSelectable(ca,u,false);}
;if(cb!==null){this._styleSelectable(cb,u,true);}
;this.__nt=cb;}
,getLeadItem:function(){return this.__nt;}
,_setAnchorItem:function(cd){var cc=this.__nu;if(cc!=null){this._styleSelectable(cc,I,false);}
;if(cd!=null){this._styleSelectable(cd,I,true);}
;this.__nu=cd;}
,_getAnchorItem:function(){return this.__nu!==null?this.__nu:null;}
...
;this._fireChange();}
,handlePointerOver:function(event){if(this.__nw!=null&&this.__nw!=this._getScroll().top){this.__nw=null;return;}
...
;var cD=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cA=event.isShiftPressed();if(this.isItemSelected(cB)&&!cA&&!cD&&!this.getDrag()){this.__nv=cB;this._userInteraction=false;return;}
else {this.__nv=null;}
;this._scrollItemIntoView(cB);var cC=this.getMode();if(this.getDrag()&&cC!==bt&&cC!==h&&!cA&&!cD&&event.getPointerType()==F){this._setAnchorItem(cB);this._setLeadItem(cB);this.__nj=this._getLocation();this.__ng=this._getScroll();this.__nk=event.getDocumentLeft()+this.__ng.left;this.__nl=event.getDocumentTop()+this.__ng.top;this.__nm=true;this._capture();}
;this._fireChange(bD);this._userInteraction=false;}
,handleTap:function(event){this._userInteraction=true;var cH=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cE=event.isShiftPressed();if(!cH&&!cE&&this.__nv!=null){this._userInteraction=false;var cF=this._getSelectableFromPointerEvent(event);if(cF===null||!this.isItemSelected(cF)){return;}
;}
...
,handlePointerMove:function(event){if(!this.__nm){return;}
;this.__nn=event.getDocumentLeft();this.__no=event.getDocumentTop();this._userInteraction=true;var cI=this.__nn+this.__ng.left;if(cI>this.__nk){this.__np=1;}
else if(cI<this.__nk){this.__np=-1;}
else {this.__np=0;}
;var cJ=this.__no+this.__ng.top;if(cJ>this.__nl){this.__nq=1;}
else if(cJ<this.__nl){this.__nq=-1;}
else {this.__nq=0;}
;var location=this.__nj;if(this.__nn<location.left){this.__nd=this.__nn-location.left;}
else if(this.__nn>location.right){this.__nd=this.__nn-location.right;}
else {this.__nd=0;}
;if(this.__no<location.top){this.__ne=this.__no-location.top;}
else if(this.__no>location.bottom){this.__ne=this.__no-location.bottom;}
else {this.__ne=0;}
;if(!this.__nf){this.__nf=new qx.event.Timer(100);this.__nf.addListener(G,this._onInterval,this);}
;this.__nf.start();this._autoSelect();event.stopPropagation();this._userInteraction=false;}
,handleAddItem:function(e){var cK=e.getData();if(this.getMode()===h&&this.isSelectionEmpty()){this.addItem(cK);}
;}
,handleRemoveItem:function(e){this.removeItem(e.getData());}
,_cleanup:function(){if(!this.getDrag()&&this.__nm){return;}
;if(this.__nr){this._fireChange(bD);}
;delete this.__nm;delete this.__nh;delete this.__ni;this._releaseCapture();if(this.__nf){this.__nf.stop();}
;}
,_onInterval:function(e){this._scrollBy(this.__nd,this.__ne);this.__ng=this._getScroll();this._autoSelect();}
,_autoSelect:function(){var cT=this._getDimension();var cM=Math.max(0,Math.min(this.__nn-this.__nj.left,cT.width))+this.__ng.left;var cL=Math.max(0,Math.min(this.__no-this.__nj.top,cT.height))+this.__ng.top;if(this.__nh===cM&&this.__ni===cL){return;}
;this.__nh=cM;this.__ni=cL;var cQ=this._getAnchorItem();var cO=cQ;var cR=this.__np;var cU,cN;while(cR!==0){cU=cR>0?this._getRelatedSelectable(cO,t):this._getRelatedSelectable(cO,V);if(cU!==null){cN=this._getSelectableLocationX(cU);if((cR>0&&cN.left<=cM)||(cR<0&&cN.right>=cM)){cO=cU;continue;}
;}
;break;}
;var cS=this.__nq;var cV,cP;while(cS!==0){cV=cS>0?this._getRelatedSelectable(cO,z):this._getRelatedSelectable(cO,bA);if(cV!==null){cP=this._getSelectableLocationY(cV);if((cS>0&&cP.top<=cL)||(cS<0&&cP.bottom>=cL)){cO=cV;continue;}
;}
;break;}
...
;this._fireChange(bw);}
,__nx:{Home:1,Down:1,Right:1,PageDown:1,End:1,Up:1,Left:1,PageUp:1},handleKeyPress:function(event){this._userInteraction=true;var dd,dc;var df=event.getKeyIdentifier();var de=this.getMode();var cY=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var da=event.isShiftPressed();var db=false;if(df===A&&cY){if(de!==bt&&de!==h){this._selectAllItems();db=true;}
;}
...
;db=true;}
;}
else if(this.__nx[df]){db=true;if(de===bt||de==h){dd=this._getSelectedItem();}
else {dd=this.getLeadItem();}
;if(dd!==null){switch(df){case X:dc=this._getFirstSelectable();break;case P:dc=this._getLastSelectable();break;case x:dc=this._getRelatedSelectable(dd,bA);break;case v:dc=this._getRelatedSelectable(dd,z);break;case k:dc=this._getRelatedSelectable(dd,V);break;case bu:dc=this._getRelatedSelectable(dd,t);break;case bx:dc=this._getPage(dd,true);break;case C:dc=this._getPage(dd,false);break;};}
...
;}
;break;};this.__nw=this._getScroll().top;this._scrollItemIntoView(dc);}
;}
...
;this.__gD={};}
,_selectItemRange:function(dk,dp,dm){var dq=this._getSelectableRange(dk,dp);if(!dm){var dn=this.__gD;var dr=this.__ny(dq);for(var dl in dn){if(!dr[dl]){this._removeFromSelection(dn[dl]);}
;}
;}
...
;}
,__ny:function(dw){var dx={};var dv;for(var i=0,l=dw.length;i<l;i++ ){dv=dw[i];dx[this._selectableToHashCode(dv)]=dv;}
;return dx;}
...
;}
;}
,_addToSelection:function(dC){var dD=this._selectableToHashCode(dC);if(this.__gD[dD]==null&&this._isSelectable(dC)){this.__gD[dD]=dC;this._styleSelectable(dC,R,true);this.__nr=true;}
;}
...
;this.__nr=true;}
,_removeFromSelection:function(dG){var dH=this._selectableToHashCode(dG);if(this.__gD[dH]!=null){delete this.__gD[dH];this._styleSelectable(dG,R,false);this.__nr=true;}
;}
...
;if(!dL){return false;}
;this._scrollItemIntoView(dI);this._setLeadItem(dP);this._setAnchorItem(dP);this.__nr=true;this._fireChange();}
,_fireChange:function(dQ){if(this.__nr){this.__ns=dQ||null;this.fireDataEvent(bs,this.getSelection());delete this.__nr;}
;}
...
;return dS;}
;return null;}
},destruct:function(){this._disposeObjects(bB);this.__gD=this.__nv=this.__nu=null;this.__nt=null;}
});}
)();
(function(){var a="vertical",b="under",c="above",d="qx.ui.core.selection.Widget",e="left",f="right";qx.Class.define(d,{extend:qx.ui.core.selection.Abstract,construct:function(g){qx.ui.core.selection.Abstract.call(this);this.__mk=g;}
,members:{__mk:null,_isSelectable:function(h){return this._isItemSelectable(h)&&h.getLayoutParent()===this.__mk;}
,_selectableToHashCode:function(j){return j.$$hash;}
,_styleSelectable:function(k,m,n){n?k.addState(m):k.removeState(m);}
,_capture:function(){this.__mk.capture();}
,_releaseCapture:function(){this.__mk.releaseCapture();}
,_isItemSelectable:function(o){if(this._userInteraction){return o.isVisible()&&o.isEnabled();}
else {return o.isVisible();}
;}
,_getWidget:function(){return this.__mk;}
,_getLocation:function(){var p=this.__mk.getContentElement().getDomElement();return p?qx.bom.element.Location.get(p):null;}
,_getDimension:function(){return this.__mk.getInnerSize();}
...
,_scrollItemIntoView:function(w){this.__mk.scrollChildIntoView(w);}
,getSelectables:function(x){var y=false;if(!x){y=this._userInteraction;this._userInteraction=true;}
;var B=this.__mk.getChildren();var z=[];var A;for(var i=0,l=B.length;i<l;i++ ){A=B[i];if(this._isItemSelectable(A)){z.push(A);}
;}
;this._userInteraction=y;return z;}
,_getSelectableRange:function(C,D){if(C===D){return [C];}
;var H=this.__mk.getChildren();var E=[];var G=false;var F;for(var i=0,l=H.length;i<l;i++ ){F=H[i];if(F===C||F===D){if(G){E.push(F);break;}
else {G=true;}
;}
;if(G&&this._isItemSelectable(F)){E.push(F);}
;}
;return E;}
,_getFirstSelectable:function(){var I=this.__mk.getChildren();for(var i=0,l=I.length;i<l;i++ ){if(this._isItemSelectable(I[i])){return I[i];}
;}
;return null;}
,_getLastSelectable:function(){var J=this.__mk.getChildren();for(var i=J.length-1;i>0;i-- ){if(this._isItemSelectable(J[i])){return J[i];}
;}
;return null;}
,_getRelatedSelectable:function(N,O){var L=this.__mk.getOrientation()===a;var P=this.__mk.getChildren();var K=P.indexOf(N);var M;if((L&&O===c)||(!L&&O===e)){for(var i=K-1;i>=0;i-- ){M=P[i];if(this._isItemSelectable(M)){return M;}
;}
;}
...
(function(){var a="_applyIconOpened",b="changeOpen",c="Boolean",d="changeIcon",f="changeIconOpened",g="changeLabel",h="__nB",i="Integer",j="auto",k="Abstract method call.",l="_applyIndent",m="open",n="changeOpenSymbolMode",o="_applyOpenSymbolMode",p="middle",q="String",r="resize",s="",t="icon",u="opened",v="iconOpened",w="abstract",x="never",y="_applyIcon",z="always",A="_applyOpen",B="changeIndent",C="qx.ui.tree.core.AbstractItem",D="label",E="_applyLabel";qx.Class.define(C,{extend:qx.ui.core.Widget,type:w,include:[qx.ui.form.MModelProperty],implement:[qx.ui.form.IModel],construct:function(F){qx.ui.core.Widget.call(this);if(F!=null){this.setLabel(F);}
;this._setLayout(new qx.ui.layout.HBox());this._addWidgets();this.initOpen();}
,properties:{open:{check:c,init:false,event:b,apply:A},openSymbolMode:{check:[z,x,j],init:j,event:n,apply:o},indent:{check:i,init:19,apply:l,event:B,themeable:true},icon:{check:q,apply:y,event:d,nullable:true,themeable:true},iconOpened:{check:q,apply:a,event:f,nullable:true,themeable:true},label:{check:q,apply:E,event:g,init:s}},members:{__nz:null,__nA:null,__nB:null,_addWidgets:function(){throw new Error(k);}
...
,addSpacer:function(){if(!this.__nB){this.__nB=new qx.ui.core.Spacer();}
else {this._remove(this.__nB);}
;this._add(this.__nB);}
,addOpenButton:function(){this._add(this.getChildControl(m));}
,_onChangeOpen:function(e){if(this.isOpenable()){this.setOpen(e.getData());}
;}
,addIcon:function(){var L=this.getChildControl(t);if(this.__nA){this._remove(L);}
;this._add(L);this.__nA=true;}
,addLabel:function(M){var N=this.getChildControl(D);if(this.__nz){this._remove(N);}
;if(M){this.setLabel(M);}
else {N.setValue(this.getLabel());}
;this._add(N);this.__nz=true;}
,_applyIcon:function(P,O){if(!this.__nD()){this.__nE(P);}
else if(!this.isOpen()){this.__nE(P);}
;}
,_applyIconOpened:function(R,Q){if(this.isOpen()){if(this.__nC()&&this.__nD()){this.__nE(R);}
else if(!this.__nC()&&this.__nD()){this.__nE(R);}
;}
;}
...
;var X;if(W){X=this.__nD()?this.getIconOpened():null;}
else {X=this.getIcon();}
;if(X){this.__nE(X);}
;W?this.addState(u):this.removeState(u);}
,__nC:function(){return qx.util.PropertyUtil.getUserValue(this,t);}
,__nD:function(){return qx.util.PropertyUtil.getUserValue(this,v);}
,__nE:function(Y){var ba=this.getChildControl(t,true);if(ba){ba.setSource(Y);}
;}
...
;}
;if(this.__nB){this.__nB.setWidth((this.getLevel()+1)*this.getIndent()-bf);}
;}
...
(function(){var a="open",b="abstract",c="qx.debug",d="'. It is not a child of this tree item.",e="addItem",f="__nJ",g="visible",h="excluded",j="__gW",k="removeItem",m="Invalid child index: ",n="Cannot remove treeitem '",o="qx.ui.tree.core.AbstractTreeItem";qx.Class.define(o,{extend:qx.ui.tree.core.AbstractItem,type:b,construct:function(p){qx.ui.tree.core.AbstractItem.call(this,p);this.__gW=[];}
,properties:{parent:{check:o,nullable:true}},members:{__gW:null,__nJ:null,getTree:function(){var r=this;while(r.getParent()){r=r.getParent();}
...
;}
;}
,getChildrenContainer:function(){if(!this.__nJ){this.__nJ=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?g:h});}
;return this.__nJ;}
,hasChildrenContainer:function(){return this.__nJ;}
,getParentChildrenContainer:function(){if(this.getParent()){return this.getParent().getChildrenContainer();}
else if(this.getLayoutParent()){return this.getLayoutParent();}
else {return null;}
;}
,getChildren:function(){return this.__gW;}
,hasChildren:function(){return this.__gW?this.__gW.length>0:false;}
...
;}
,__nK:function(){if(this.getParentChildrenContainer()){this.getParentChildrenContainer()._addAfter(this.getChildrenContainer(),this);}
;}
...
;this.__gW.push(Q);if(!N){this.__nK();}
;if(P){Q.recursiveAddToWidgetQueue();P.fireNonBubblingEvent(e,qx.event.type.Data,[Q]);}
;}
;if(P){qx.ui.core.queue.Widget.add(this);}
;}
,addAt:function(X,T){if(qx.core.Environment.get(c)){this.assert(T<=this.__gW.length&&T>=0,m+T);}
;if(T==this.__gW.length){this.add(X);return;}
;var W=X.getParent();if(W){W.remove(X);}
;var U=this.getChildrenContainer();X.setParent(this);var V=this.hasChildren();var S=this.__gW[T];U.addBefore(X,S);if(X.hasChildren()){U.addAfter(X.getChildrenContainer(),X);}
;qx.lang.Array.insertAt(this.__gW,X,T);if(!V){this.__nK();}
;if(this.getTree()){X.recursiveAddToWidgetQueue();qx.ui.core.queue.Widget.add(this);}
;}
,addBefore:function(Y,bb){if(qx.core.Environment.get(c)){this.assert(this.__gW.indexOf(bb)>=0);}
;var ba=Y.getParent();if(ba){ba.remove(Y);}
;this.addAt(Y,this.__gW.indexOf(bb));}
,addAfter:function(bd,bc){if(qx.core.Environment.get(c)){this.assert(this.__gW.indexOf(bc)>=0);}
;var be=bd.getParent();if(be){be.remove(bd);}
;this.addAt(bd,this.__gW.indexOf(bc)+1);}
,addAtBegin:function(bf){this.addAt(bf,0);}
,remove:function(bk){for(var i=0,l=arguments.length;i<l;i++ ){var bj=arguments[i];if(this.__gW.indexOf(bj)==-1){this.warn(n+bj+d);return;}
...
;}
;qx.lang.Array.remove(this.__gW,bj);bj.setParent(null);bg.remove(bj);}
;var bh=this.getTree();if(bh){bh.fireNonBubblingEvent(k,qx.event.type.Data,[bj]);}
;qx.ui.core.queue.Widget.add(this);}
,removeAt:function(bl){var bm=this.__gW[bl];if(bm){this.remove(bm);}
;}
,removeAll:function(){var bn=this.__gW.concat();for(var i=this.__gW.length-1;i>=0;i-- ){this.remove(this.__gW[i]);}
;return bn;}
},destruct:function(){this._disposeArray(j);this._disposeObjects(f);}
});}
)();
...
,events:{update:m,scrollAnimationEnd:m},properties:{scrollX:{check:j,apply:g,event:f,init:0},scrollY:{check:l,apply:h,event:b,init:0}},members:{__mX:null,add:function(r){var s=this._getChildren()[0];if(s){this._remove(s);s.removeListener(a,this._onUpdate,this);}
...
;this.stopScrollAnimation();if(L){var N=this.getScrollX();this.__mX=new qx.bom.AnimationFrame();this.__mX.on(o,function(){this.setScrollX(O);this.__mX=null;this.fireEvent(c);}
,this);this.__mX.on(i,function(Q){var P=parseInt(Q/L*(O-N)+N);this.setScrollX(P);}
,this);this.__mX.startSequence(L);}
else {this.setScrollX(O);}
;}
,scrollToY:function(U,R){var S=this.getScrollMaxY();if(U<0){U=0;}
else if(U>S){U=S;}
;this.stopScrollAnimation();if(R){var T=this.getScrollY();this.__mX=new qx.bom.AnimationFrame();this.__mX.on(o,function(){this.setScrollY(U);this.__mX=null;this.fireEvent(c);}
,this);this.__mX.on(i,function(W){var V=parseInt(W/R*(U-T)+T);this.setScrollY(V);}
,this);this.__mX.startSequence(R);}
...
,stopScrollAnimation:function(){if(this.__mX){this.__mX.cancelSequence();this.__mX=null;}
;}
...
;if(v){this.setSeparator(v);}
;}
,properties:{alignY:{check:[k,o,f],init:k,apply:b},alignX:{check:[m,c,r],init:m,apply:b},spacing:{check:p,init:0,apply:b},separator:{check:a,nullable:true,apply:b},reversed:{check:s,init:false,apply:d}},members:{__nL:null,__nG:null,__nH:null,__gW:null,_applyReversed:function(){this._invalidChildrenCache=true;this._applyLayoutChange();}
,__nI:function(){var B=this._getLayoutChildren();var length=B.length;var x=false;var w=this.__nL&&this.__nL.length!=length&&this.__nG&&this.__nL;var z;var y=w?this.__nL:new Array(length);var A=w?this.__nG:new Array(length);if(this.getReversed()){B=B.concat().reverse();}
...
;}
;if(!w){this.__nL=y;this.__nG=A;}
;this.__nH=x;this.__gW=B;delete this._invalidChildrenCache;}
...
;}
,"false":null}),renderLayout:function(V,N,Y){if(this._invalidChildrenCache){this.__nI();}
...
;var i,ba,G,O;var P=[];var W=H;for(i=0;i<length;i+=1){O=this.__nL[i];G=O!=null?Math.floor((N-H)*O):K[i].getSizeHint().height;P.push(G);W+=G;}
;if(this.__nH&&W!=N){var M={};var S,F;for(i=0;i<length;i+=1){S=this.__nG[i];if(S>0){L=K[i].getSizeHint();M[i]={min:L.minHeight,value:P[i],max:L.maxHeight,flex:S};}
;}
...
;}
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__nI();}
;var be=qx.ui.layout.Util;var bs=this.__gW;var bh=0,bi=0,bq=0;var bj=0,bk=0;var bo,bf,br;for(var i=0,l=bs.length;i<l;i+=1){bo=bs[i];bf=bo.getSizeHint();bi+=bf.height;var bn=this.__nG[i];var bg=this.__nL[i];if(bn){bh+=bf.minHeight;}
...
},destruct:function(){this.__nL=this.__nG=this.__gW=null;}
});}
)();
(function(){var a="Could not set the model selection. Maybe your models are not unique? ",b="qx.ui.form.MModelSelection",c="Please use an array as parameter.",d="change",f="qx.debug",g="__lW",h="changeSelection",k="qx.event.type.Data";qx.Mixin.define(b,{construct:function(){this.__lW=new qx.data.Array();this.__lW.addListener(d,this.__ma,this);this.addListener(h,this.__lY,this);}
,events:{changeModelSelection:k},members:{__lW:null,__lX:false,__lY:function(){if(this.__lX){return;}
...
;}
;}
,__ma:function(){this.__lX=true;var r=this.getSelectables(true);var s=[];var q=this.__lW.toArray();for(var i=0;i<q.length;i++ ){var u=q[i];for(var j=0;j<r.length;j++ ){var v=r[j];var p=v.getModel?v.getModel():null;if(u===p){s.push(v);break;}
;}
;}
;this.setSelection(s);this.__lX=false;var t=this.getSelection();if(!qx.lang.Array.equals(t,s)){this.__lY();}
;}
,getModelSelection:function(){return this.__lW;}
,setModelSelection:function(w){if(!w){this.__lW.removeAll();return;}
;if(qx.core.Environment.get(f)){this.assertArray(w,c);}
;w.unshift(this.__lW.getLength());w.unshift(0);var x=this.__lW.splice.apply(this.__lW,w);x.dispose();}
...
;if((n instanceof qx.core.Object)&&qx.Class.hasMixin(n.constructor,qx.data.marshal.MEventBubbling)){var o=qx.lang.Function.bind(this.__mb,this,name);var q=n.addListener(g,o,this);var p=n.getUserData(d+this.$$hash);if(p==null){p=[];n.setUserData(d+this.$$hash,p);}
;p.push(q);}
;}
,__mb:function(name,e){var y=e.getData();var u=y.value;var s=y.old;if(qx.Class.hasInterface(e.getTarget().constructor,qx.data.IListData)){if(y.name.indexOf){var x=y.name.indexOf(h)!=-1?y.name.indexOf(h):y.name.length;var v=y.name.indexOf(f)!=-1?y.name.indexOf(f):y.name.length;if(v==0){var t=name+y.name;}
...
else if(arguments.length>1){this.__mc=[];for(var i=0;i<arguments.length;i++ ){this.__mc.push(arguments[i]);}
;}
else if(typeof u==q){this.__mc=new Array(u);}
else if(u instanceof Array){this.__mc=qx.lang.Array.clone(u);}
else {this.__mc=[];this.dispose();throw new Error(l);}
;for(var i=0;i<this.__mc.length;i++ ){this._applyEventPropagation(this.__mc[i],null,i);}
;this.__md();if(qx.core.Environment.get(k)){this[0]=h;}
;}
,properties:{autoDisposeItems:{check:e,init:false}},events:{"change":t,"changeLength":t},members:{__mc:null,concat:function(v){if(v){var w=this.__mc.concat(v);}
else {var w=this.__mc.concat();}
;return new qx.data.Array(w);}
,join:function(x){return this.__mc.join(x);}
,pop:function(){var y=this.__mc.pop();this.__md();this._registerEventChaining(null,y,this.length-1);this.fireDataEvent(r,{value:[],name:this.length+f,old:[y],item:this});this.fireDataEvent(o,{start:this.length-1,end:this.length-1,type:g,removed:[y],added:[]},null);return y;}
,push:function(z){for(var i=0;i<arguments.length;i++ ){this.__mc.push(arguments[i]);this.__md();this._registerEventChaining(arguments[i],null,this.length-1);this.fireDataEvent(r,{value:[arguments[i]],name:(this.length-1)+f,old:[],item:this});this.fireDataEvent(o,{start:this.length-1,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,reverse:function(){if(this.length==0){return;}
;var A=this.__mc.concat();this.__mc.reverse();this.__me(0,this.length);this.fireDataEvent(o,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(r,{value:this.__mc,name:n+(this.__mc.length-1),old:A,item:this});}
,shift:function(){if(this.length==0){return;}
;var B=this.__mc.shift();this.__md();this._registerEventChaining(null,B,this.length-1);this.__me(0,this.length);this.fireDataEvent(r,{value:[],name:p,old:[B],item:this});this.fireDataEvent(o,{start:0,end:this.length-1,type:g,removed:[B],added:[]},null);return B;}
,slice:function(D,C){return new qx.data.Array(this.__mc.slice(D,C));}
,splice:function(I,K,M){var Q=this.__mc.length;var L=this.__mc.splice.apply(this.__mc,arguments);if(this.__mc.length!=Q){this.__md();}
...
;this.__me(I+(arguments.length-2)-K,this.length);if(O||G){var J=[];for(var i=2;i<arguments.length;i++ ){J[i-2]=arguments[i];}
...
;var R=this.__mc.concat();this.__mc.sort.apply(this.__mc,arguments);if(qx.lang.Array.equals(this.__mc,R)===true){return;}
;this.__me(0,this.length);this.fireDataEvent(o,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(r,{value:this.__mc,name:n+(this.length-1),old:R,item:this});}
,unshift:function(T){for(var i=arguments.length-1;i>=0;i-- ){this.__mc.unshift(arguments[i]);this.__md();this.__me(0,this.length);this.fireDataEvent(r,{value:[this.__mc[0]],name:p,old:[this.__mc[1]],item:this});this.fireDataEvent(o,{start:0,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,toArray:function(){return this.__mc;}
,getItem:function(U){return this.__mc[U];}
,setItem:function(V,X){var W=this.__mc[V];if(W===X){return;}
;this.__mc[V]=X;this._registerEventChaining(X,W,V);if(this.length!=this.__mc.length){this.__md();}
...
,indexOf:function(Y){return this.__mc.indexOf(Y);}
,lastIndexOf:function(ba){return this.__mc.lastIndexOf(ba);}
,toString:function(){if(this.__mc!=null){return this.__mc.toString();}
;return f;}
,contains:function(bb){return this.__mc.indexOf(bb)!==-1;}
...
,removeAll:function(){for(var i=0;i<this.__mc.length;i++ ){this._registerEventChaining(null,this.__mc[i],i);}
;if(this.getLength()==0){return [];}
;var bo=this.getLength();var bn=this.__mc.concat();this.__mc.length=0;this.__md();this.fireDataEvent(r,{value:[],name:n+(bo-1),old:bn,item:this});this.fireDataEvent(o,{start:0,end:bo-1,type:g,removed:bn,added:[]},null);return bn;}
...
;var bq=this.__mc.length;Array.prototype.push.apply(this.__mc,bp);for(var i=0;i<bp.length;i++ ){this._registerEventChaining(bp[i],null,bq+i);}
;var bq=this.length;this.__md();var name=bq==(this.length-1)?bq:bq+a+(this.length-1);this.fireDataEvent(r,{value:bp,name:name+f,old:[],item:this});this.fireDataEvent(o,{start:bq,end:this.length-1,type:b,added:bp,removed:[]},null);}
...
,forEach:function(bx,by){for(var i=0;i<this.__mc.length;i++ ){bx.call(by,this.__mc[i],i,this);}
;}
,filter:function(bz,self){return new qx.data.Array(this.__mc.filter(bz,self));}
,map:function(bA,self){return new qx.data.Array(this.__mc.map(bA,self));}
,some:function(bB,self){return this.__mc.some(bB,self);}
,every:function(bC,self){return this.__mc.every(bC,self);}
,reduce:function(bE,bD){return this.__mc.reduce(bE,bD);}
,reduceRight:function(bG,bF){return this.__mc.reduceRight(bG,bF);}
,__md:function(){var bH=this.length;this.length=this.__mc.length;this.fireDataEvent(s,this.length,bH);}
,__me:function(bJ,bI){for(var i=bJ;i<bI;i++ ){this._registerEventChaining(this.__mc[i],this.__mc[i],i);}
;}
},destruct:function(){for(var i=0;i<this.__mc.length;i++ ){var bK=this.__mc[i];this._applyEventPropagation(null,bK,i);if(this.isAutoDisposeItems()&&bK&&bK instanceof qx.core.Object){bK.dispose();}
;}
;this.__mc=null;}
});}
)();
...
(function(){var a="mouse",b="scrollbar-y",c="qx.ui.core.scroll.MRoll",d="pointerdown",f="scrollbar-x",g="roll";qx.Mixin.define(c,{members:{__mO:null,_addRollHandling:function(){this.addListener(g,this._onRoll,this);this.addListener(d,this._onPointerDownForRoll,this);}
...
,_onPointerDownForRoll:function(e){this.__mO=e.getPointerId();}
,_onRoll:function(e){if(e.getPointerType()==a){return;}
;if(this.__mO&&e.getMomentum()){qx.event.Registration.getManager(e.getOriginalTarget()).getHandler(qx.event.handler.Gesture).gestureCancel(this.__mO);e.stopMomentum();this.__mO=null;return;}
;this.__mO=null;var k=this._isChildControlVisible(f);var l=this._isChildControlVisible(b);var q=l?this.getChildControl(b,true):null;var p=k?this.getChildControl(f,true):null;var m=e.getDelta().y;var h=e.getDelta().x;var j=!l;var o=!k;if(q){if(m!==0){q.scrollBy(parseInt(m,10));}
...
;r.addListener(k,this.__mS,this);r.addListener(i,this.__mT,this);this.__mP=[n,o];this.__mQ=[m,g];}
,properties:{dragScrollThresholdX:{check:p,init:30},dragScrollThresholdY:{check:p,init:30},dragScrollSlowDownFactor:{check:b,init:0.1}},members:{__mR:null,__mP:null,__mQ:null,_findScrollableParent:function(s){var t=s;if(t===null){return null;}
...
;}
,_getAxis:function(A){if(this.__mP.indexOf(A)!==-1){return a;}
else if(this.__mQ.indexOf(A)!==-1){return q;}
else {throw new Error(l+A+f);}
;}
,_getThresholdByEdgeType:function(B){if(this.__mP.indexOf(B)!==-1){return this.getDragScrollThresholdX();}
else if(this.__mQ.indexOf(B)!==-1){return this.getDragScrollThresholdY();}
;}
...
;var P=S.getBounds(),O=T===a?P.width:P.height,R=this._calculateScrollAmount(O,Q);if(this._isScrollbarExceedingMaxPos(S,T,R)){this.__mR.stop();}
;S.scrollBy(R);}
,__mS:function(e){if(this.__mR){this.__mR.stop();}
...
;bd=this._getAxis(X);if(this._isScrollbarVisible(U,bd)){bb=this._calculateThresholdExceedance(bc[X],this._getThresholdByEdgeType(X));if(this.__mR){this.__mR.dispose();}
;this.__mR=new qx.event.Timer(50);this.__mR.addListener(h,function(be,bg,bf){this._scrollBy(be,bg,bf);}
.bind(this,U,bd,bb));this.__mR.start();e.stopPropagation();return;}
else {U=this._findScrollableParent(U);}
;}
;}
,__mT:function(e){if(this.__mR){this.__mR.stop();}
;}
},destruct:function(){if(this.__mR){this.__mR.dispose();}
;}
});}
)();
...
,_getContentHint:function(){var L=qx.bom.element.Scroll.getScrollbarWidth();return {width:this.__mU?100:L,maxWidth:this.__mU?null:L,minWidth:this.__mU?null:L,height:this.__mU?L:100,maxHeight:this.__mU?L:null,minHeight:this.__mU?L:null};}
...
,_applyPosition:function(P){var content=this.getContentElement();if(this.__mU){content.scrollToX(P);}
else {content.scrollToY(P);}
;}
,_applyOrientation:function(S,Q){var R=this.__mU=S===x;this.set({allowGrowX:R,allowShrinkX:R,allowGrowY:!R,allowShrinkY:!R});if(R){this.replaceState(o,x);}
...
,_updateScrollBar:function(){var U=this.__mU;var V=this.getBounds();if(!V){return;}
...
,scrollTo:function(Y,X){this.stopScrollAnimation();if(X){var ba=this.getPosition();this.__mW=new qx.bom.AnimationFrame();this.__mW.on(d,function(bc){var bb=parseInt(bc/X*(Y-ba)+ba);this.updatePosition(bb);}
,this);this.__mW.on(f,function(){this.setPosition(Math.max(0,Math.min(this.getMaximum(),Y)));this.__mW=null;this.fireEvent(q);}
,this);this.__mW.startSequence(X);}
...
,stopScrollAnimation:function(){if(this.__mW){this.__mW.cancelSequence();this.__mW=null;}
;}
,_onScroll:function(e){var bj=this.getContentElement();var bk=this.__mU?bj.getScrollX():bj.getScrollY();this.setPosition(bk);}
...
,this);}
,events:{"scrollAnimationEnd":l},properties:{appearance:{refine:true,init:A},orientation:{check:[x,r],init:x,apply:s},maximum:{check:w,apply:B,init:100},position:{check:i,init:0,apply:z,event:F},singleStep:{check:f,init:20},pageStep:{check:f,init:10,apply:u},knobFactor:{check:c,apply:j,nullable:true}},members:{__yP:2,__yQ:0,_computeSizeHint:function(){var H=qx.ui.core.Widget.prototype._computeSizeHint.call(this);if(this.getOrientation()===x){this.__yQ=H.minWidth;H.minWidth=0;}
else {this.__yQ=H.minHeight;H.minHeight=0;}
;return H;}
,renderLayout:function(M,top,J,I){var L=qx.ui.core.Widget.prototype.renderLayout.call(this,M,top,J,I);var K=this.getOrientation()===x;if(this.__yQ>=(K?J:I)){this.getChildControl(n).setVisibility(o);this.getChildControl(D).setVisibility(o);}
...
,_onResizeSlider:function(e){var bg=this.getChildControl(a).getChildControl(m);var bj=bg.getSizeHint();var bh=false;var bi=this.getChildControl(a).getInnerSize();if(this.getOrientation()==r){if(bi.height<bj.minHeight+this.__yP){bh=true;}
;}
else {if(bi.width<bj.minWidth+this.__yP){bh=true;}
;}
;if(bh){bg.exclude();}
else {bg.show();}
;}
...
,_onPointerDown:function(e){if(this.__yU){return;}
;var bw=this.__mU;var bu=this.getChildControl(P);var bv=bw?G:L;var bA=bw?e.getDocumentLeft():e.getDocumentTop();var bC=this.getDecorator();bC=qx.theme.manager.Decoration.getInstance().resolve(bC);if(bw){var bz=bC?bC.getInsets().left:0;var by=(this.getPaddingLeft()||0)+bz;}
else {var bz=bC?bC.getInsets().top:0;var by=(this.getPaddingTop()||0)+bz;}
;var bB=this.__yR=qx.bom.element.Location.get(this.getContentElement().getDomElement())[bv];bB+=by;var bx=this.__yS=qx.bom.element.Location.get(bu.getContentElement().getDomElement())[bv];if(e.getTarget()===bu){this.__yU=true;if(!this.__za){this.__za=new qx.event.Timer(100);this.__za.addListener(r,this._fireValue,this);}
;this.__za.start();this.__yV=bA+bB-bx;bu.addState(c);}
else {this.__yW=true;this.__yX=bA<=bx?-1:1;this.__ze(e);this._onInterval();if(!this.__eg){this.__eg=new qx.event.Timer(100);this.__eg.addListener(r,this._onInterval,this);}
;this.__eg.start();}
;this.addListener(w,this._onPointerMove);this.capture();e.stopPropagation();}
,_onPointerUp:function(e){if(this.__yU){this.releaseCapture();delete this.__yU;this.__za.stop();this._fireValue();delete this.__yV;this.getChildControl(P).removeState(c);if(e.getType()===B){var bE;var bF;var bD;if(this.__mU){bE=e.getDocumentLeft()-(this._valueToPosition(this.getValue())+this.__yR);bD=qx.bom.element.Location.get(this.getContentElement().getDomElement())[L];bF=e.getDocumentTop()-(bD+this.getChildControl(P).getBounds().top);}
else {bE=e.getDocumentTop()-(this._valueToPosition(this.getValue())+this.__yR);bD=qx.bom.element.Location.get(this.getContentElement().getDomElement())[G];bF=e.getDocumentLeft()-(bD+this.getChildControl(P).getBounds().left);}
;if(bF<0||bF>this.__yT||bE<0||bE>this.__yT){this.getChildControl(P).removeState(b);}
;}
;}
else if(this.__yW){this.__eg.stop();this.releaseCapture();delete this.__yW;delete this.__yX;delete this.__yY;}
;this.removeListener(w,this._onPointerMove);if(e.getType()===B){e.stopPropagation();}
;}
,_onPointerMove:function(e){if(this.__yU){var bH=this.__mU?e.getDocumentLeft():e.getDocumentTop();var bG=bH-this.__yV;this.slideTo(this._positionToValue(bG));}
else if(this.__yW){this.__ze(e);}
;e.stopPropagation();}
,_onInterval:function(e){var bI=this.getValue()+(this.__yX*this.getPageStep());if(bI<this.getMinimum()){bI=this.getMinimum();}
else if(bI>this.getMaximum()){bI=this.getMaximum();}
;var bJ=this.__yX==-1;if((bJ&&bI<=this.__yY)||(!bJ&&bI>=this.__yY)){bI=this.__yY;}
;this.slideTo(bI);}
,_onUpdate:function(e){var bL=this.getInnerSize();var bM=this.getChildControl(P).getBounds();var bK=this.__mU?E:x;this._updateKnobSize();this.__zd=bL[bK]-bM[bK];this.__yT=bM[bK];this._updateKnobPosition();}
,__mU:false,__zd:0,__ze:function(e){var bN=this.__mU;var bU=bN?e.getDocumentLeft():e.getDocumentTop();var bX=this.__yR;var bO=this.__yS;var bY=this.__yT;var bV=bU-bX;if(bU>=bO){bV-=bY;}
;var bR=this._positionToValue(bV);var bP=this.getMinimum();var bQ=this.getMaximum();if(bR<bP){bR=bP;}
else if(bR>bQ){bR=bQ;}
else {var bW=this.getValue();var bT=this.getPageStep();var bS=this.__yX<0?I:Q;bR=bW+(Math[bS]((bR-bW)/bT)*bT);}
;if(this.__yY==null||(this.__yX==-1&&bR<=this.__yY)||(this.__yX==1&&bR>=this.__yY)){this.__yY=bR;}
;}
,_positionToValue:function(cb){var ca=this.__zd;if(ca==null||ca==0){return 0;}
;var cd=cb/ca;if(cd<0){cd=0;}
else if(cd>1){cd=1;}
;var cc=this.getMaximum()-this.getMinimum();return this.getMinimum()+Math.round(cc*cd);}
,_valueToPosition:function(ch){var ce=this.__zd;if(ce==null){return 0;}
...
,_setKnobPosition:function(ck){var ci=this.getChildControl(P);var cj=this.getDecorator();cj=qx.theme.manager.Decoration.getInstance().resolve(cj);var content=ci.getContentElement();if(this.__mU){if(cj&&cj.getPadding()){ck+=cj.getPadding().left;}
...
;if(this.__mU){this.getChildControl(P).setWidth(Math.round(cm*cl.width));}
...
,slideTo:function(cu,ct){this.stopSlideAnimation();if(ct){this.__zg(cu,ct);}
else {this.updatePosition(cu);}
;}
,updatePosition:function(cv){this.setValue(this.__zf(cv));}
,stopSlideAnimation:function(){if(this.__mW){this.__mW.cancelSequence();this.__mW=null;}
;}
,__zf:function(cw){if(cw<this.getMinimum()){cw=this.getMinimum();}
...
;return cw;}
,__zg:function(cy,cx){cy=this.__zf(cy);var cz=this.getValue();this.__mW=new qx.bom.AnimationFrame();this.__mW.on(i,function(cA){this.setValue(parseInt(cA/cx*(cy-cz)+cz));}
,this);this.__mW.on(j,function(){this.setValue(cy);this.__mW=null;this.fireEvent(O);}
,this);this.__mW.startSequence(cx);}
,_applyOrientation:function(cD,cC){var cB=this.getChildControl(P);this.__mU=cD===N;if(this.__mU){this.removeState(C);cB.removeState(C);this.addState(N);cB.addState(N);cB.setLayoutProperties({top:0,right:null,bottom:0});}
...
else {if(this.__mU){this.getChildControl(P).resetWidth();}
else {this.getChildControl(P).resetHeight();}
;}
;}
,_applyValue:function(cH,cG){if(cH!=null){this._updateKnobPosition();if(this.__yU){this.__zc=[cH,cG];}
else {this.fireEvent(h,qx.event.type.Data,[cH,cG]);}
;}
else {this.resetValue();}
;}
,_fireValue:function(){if(!this.__zc){return;}
;var cI=this.__zc;this.__zc=null;this.fireEvent(h,qx.event.type.Data,cI);}
...
,events:{"execute":j,"press":j,"release":j},properties:{interval:{check:m,init:100},firstInterval:{check:m,init:500},minTimer:{check:m,init:20},timerDecrease:{check:m,init:2}},members:{__nS:null,__eg:null,press:function(){if(this.isEnabled()){if(!this.hasState(h)){this.__nT();}
;this.removeState(l);this.addState(h);}
;}
,release:function(q){if(!this.isEnabled()){return;}
;if(this.hasState(h)){if(!this.__nS){this.execute();}
;}
...
;}
;this.__nU();e.stopPropagation();}
,_onTap:function(e){}
,_onKeyUp:function(e){switch(e.getKeyIdentifier()){case g:case k:if(this.hasState(h)){if(!this.__nS){this.execute();}
;this.removeState(h);this.removeState(l);e.stopPropagation();this.__nU();}
;};}
,_onKeyDown:function(e){switch(e.getKeyIdentifier()){case g:case k:this.removeState(l);this.addState(h);e.stopPropagation();this.__nT();};}
,_onInterval:function(e){this.__nS=true;this.fireEvent(n);}
,__nT:function(){this.fireEvent(a);this.__nS=false;this.__eg.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();this.removeState(l);this.addState(h);}
,__nU:function(){this.fireEvent(f);this.__eg.stop();this.removeState(l);this.removeState(h);}
...
,events:{"interval":d},properties:{interval:{check:a,init:100},firstInterval:{check:a,init:500},minimum:{check:a,init:20},decrease:{check:a,init:2}},members:{__eg:null,__nV:null,start:function(){this.__eg.setInterval(this.getFirstInterval());this.__eg.start();}
,stop:function(){this.__eg.stop();this.__nV=null;}
,_onInterval:function(){this.__eg.stop();if(this.__nV==null){this.__nV=this.getInterval();}
;this.__nV=Math.max(this.getMinimum(),this.__nV-this.getDecrease());this.__eg.setInterval(this.__nV);this.__eg.start();this.fireEvent(c);}
},destruct:function(){this._disposeObjects(b);}
});}
)();
...
(function(){var a="' must be defined!",b="height",c="vAlign",d="hAlign",e="' is not supported by the Grid layout!",f="bottom",g="Invalid parameter 'column'",h="Integer",m="The property '",n="'",o="_applyLayoutChange",p="qx.debug",q="Value must be positive",r="center",s="qx.ui.layout.Grid",t="middle",u="maxHeight",v="Cannot add widget '",w="width",z=") for '",A="'!. ",B="top",C="minHeight",D="right",E="' in this cell (",F=", ",G="The layout properties 'row' and 'column' of the child widget '",H="minWidth",I="flex",J="left",K="maxWidth",L="Invalid parameter 'row'",M="There is already a widget '";qx.Class.define(s,{extend:qx.ui.layout.Abstract,construct:function(O,N){qx.ui.layout.Abstract.call(this);this.__mB=[];this.__mC=[];if(O){this.setSpacingX(O);}
;if(N){this.setSpacingY(N);}
;}
,properties:{spacingX:{check:h,init:0,apply:o},spacingY:{check:h,init:0,apply:o}},members:{__mD:null,__mB:null,__mC:null,__mE:null,__mF:null,__mG:null,__mH:null,__mI:null,__mJ:null,verifyLayoutProperty:qx.core.Environment.select(p,{"true":function(P,name,Q){var R={"row":1,"column":1,"rowSpan":1,"colSpan":1};this.assert(R[name]==1,m+name+e);this.assertInteger(Q);this.assert(Q>=0,q);}
,"false":null}),__mK:function(){var Y=[];var T=[];var W=[];var V=-1;var S=-1;var X=this._getLayoutChildren();for(var i=0,l=X.length;i<l;i++ ){var ba=X[i];var bb=ba.getLayoutProperties();var bc=bb.row;var U=bb.column;bb.colSpan=bb.colSpan||1;bb.rowSpan=bb.rowSpan||1;if(bc==null||U==null){throw new Error(G+ba+a);}
...
;}
;this.__mD=Y;this.__mE=T;this.__mF=W;this.__mG=V;this.__mH=S;this.__mI=null;this.__mJ=null;delete this._invalidChildrenCache;}
,_setRowData:function(bg,be,bf){var bd=this.__mB[bg];if(!bd){this.__mB[bg]={};this.__mB[bg][be]=bf;}
else {bd[be]=bf;}
;}
,_setColumnData:function(bh,bj,bk){var bi=this.__mC[bh];if(!bi){this.__mC[bh]={};this.__mC[bh][bj]=bk;}
else {bi[bj]=bk;}
;}
...
,getColumnAlign:function(bp){var bq=this.__mC[bp]||{};return {vAlign:bq.vAlign||B,hAlign:bq.hAlign||J};}
...
,getRowAlign:function(bv){var bu=this.__mB[bv]||{};return {vAlign:bu.vAlign||B,hAlign:bu.hAlign||J};}
,getCellWidget:function(bx,bw){if(this._invalidChildrenCache){this.__mK();}
;var bx=this.__mD[bx]||{};return bx[bw]||null;}
,getRowCount:function(){if(this._invalidChildrenCache){this.__mK();}
;return this.__mG+1;}
,getColumnCount:function(){if(this._invalidChildrenCache){this.__mK();}
;return this.__mH+1;}
,getCellAlign:function(bF,bz){var bE=B;var bC=J;var bD=this.__mB[bF];var bA=this.__mC[bz];var by=this.__mD[bF][bz];if(by){var bB={vAlign:by.getAlignY(),hAlign:by.getAlignX()};}
else {bB={};}
...
,getColumnFlex:function(bI){var bJ=this.__mC[bI]||{};return bJ.flex!==undefined?bJ.flex:0;}
,setRowFlex:function(bL,bK){this._setRowData(bL,I,bK);this._applyLayoutChange();return this;}
,getRowFlex:function(bO){var bM=this.__mB[bO]||{};var bN=bM.flex!==undefined?bM.flex:0;return bN;}
,setColumnMaxWidth:function(bP,bQ){this._setColumnData(bP,K,bQ);this._applyLayoutChange();return this;}
,getColumnMaxWidth:function(bR){var bS=this.__mC[bR]||{};return bS.maxWidth!==undefined?bS.maxWidth:Infinity;}
,setColumnWidth:function(bT,bU){this._setColumnData(bT,w,bU);this._applyLayoutChange();return this;}
,getColumnWidth:function(bV){var bW=this.__mC[bV]||{};return bW.width!==undefined?bW.width:null;}
,setColumnMinWidth:function(bX,bY){this._setColumnData(bX,H,bY);this._applyLayoutChange();return this;}
,getColumnMinWidth:function(ca){var cb=this.__mC[ca]||{};return cb.minWidth||0;}
,setRowMaxHeight:function(cd,cc){this._setRowData(cd,u,cc);this._applyLayoutChange();return this;}
,getRowMaxHeight:function(cf){var ce=this.__mB[cf]||{};return ce.maxHeight||Infinity;}
,setRowHeight:function(cg,ch){this._setRowData(cg,b,ch);this._applyLayoutChange();return this;}
,getRowHeight:function(cj){var ci=this.__mB[cj]||{};return ci.height!==undefined?ci.height:null;}
,setRowMinHeight:function(cl,ck){this._setRowData(cl,C,ck);this._applyLayoutChange();return this;}
,getRowMinHeight:function(cn){var cm=this.__mB[cn]||{};return cm.minHeight||0;}
,__mL:function(cp){var co=cp.getSizeHint();var cr=cp.getMarginLeft()+cp.getMarginRight();var cq=cp.getMarginTop()+cp.getMarginBottom();var cs={height:co.height+cq,width:co.width+cr,minHeight:co.minHeight+cq,minWidth:co.minWidth+cr,maxHeight:co.maxHeight+cq,maxWidth:co.maxWidth+cr};return cs;}
,_fixHeightsRowSpan:function(cN){var cz=this.getSpacingY();for(var i=0,l=this.__mF.length;i<l;i++ ){var cF=this.__mF[i];var cC=this.__mL(cF);var cv=cF.getLayoutProperties();var cB=cv.row;var cK=cz*(cv.rowSpan-1);var ct=cK;var cw={};for(var j=0;j<cv.rowSpan;j++ ){var cA=cv.row+j;var cJ=cN[cA];var cL=this.getRowFlex(cA);if(cL>0){cw[cA]={min:cJ.minHeight,value:cJ.height,max:cJ.maxHeight,flex:cL};}
...
;}
;}
,_fixWidthsColSpan:function(cR){var cS=this.getSpacingX();for(var i=0,l=this.__mE.length;i<l;i++ ){var cO=this.__mE[i];var cQ=this.__mL(cO);var cU=cO.getLayoutProperties();var cP=cU.column;var db=cS*(cU.colSpan-1);var cT=db;var cV={};var cX;for(var j=0;j<cU.colSpan;j++ ){var cW=cU.column+j;var da=cR[cW];var cY=this.getColumnFlex(cW);if(cY>0){cV[cW]={min:da.minWidth,value:da.width,max:da.maxWidth,flex:cY};}
...
;}
;}
,_getRowHeights:function(){if(this.__mI!=null){return this.__mI;}
;var dm=[];var df=this.__mG;var de=this.__mH;for(var dn=0;dn<=df;dn++ ){var dg=0;var di=0;var dh=0;for(var dl=0;dl<=de;dl++ ){var dd=this.__mD[dn][dl];if(!dd){continue;}
;var dj=dd.getLayoutProperties().rowSpan||0;if(dj>1){continue;}
;var dk=this.__mL(dd);if(this.getRowFlex(dn)>0){dg=Math.max(dg,dk.minHeight);}
...
;if(this.__mF.length>0){this._fixHeightsRowSpan(dm);}
;this.__mI=dm;return dm;}
,_getColWidths:function(){if(this.__mJ!=null){return this.__mJ;}
;var dt=[];var dq=this.__mH;var ds=this.__mG;for(var dy=0;dy<=dq;dy++ ){var dw=0;var dv=0;var dr=Infinity;for(var dz=0;dz<=ds;dz++ ){var dp=this.__mD[dz][dy];if(!dp){continue;}
;var du=dp.getLayoutProperties().colSpan||0;if(du>1){continue;}
;var dx=this.__mL(dp);if(this.getColumnFlex(dy)>0){dv=Math.max(dv,dx.minWidth);}
...
;if(this.__mE.length>0){this._fixWidthsColSpan(dt);}
;this.__mJ=dt;return dt;}
...
,renderLayout:function(ej,dO,ei){if(this._invalidChildrenCache){this.__mK();}
;var ed=qx.ui.layout.Util;var dQ=this.getSpacingX();var dW=this.getSpacingY();var eh=this._getColWidths();var ek=this._getColumnFlexOffsets(ej);var dR=[];var em=this.__mH;var dP=this.__mG;var el;for(var en=0;en<=em;en++ ){el=ek[en]?ek[en].offset:0;dR[en]=eh[en].width+el;}
...
;var er=0;for(var en=0;en<=em;en++ ){var top=0;for(var dX=0;dX<=dP;dX++ ){var ef=this.__mD[dX][en];if(!ef){top+=et[dX]+dW;continue;}
...
;er+=dR[en]+dQ;}
;}
,invalidateLayoutCache:function(){qx.ui.layout.Abstract.prototype.invalidateLayoutCache.call(this);this.__mJ=null;this.__mI=null;}
...
},destruct:function(){this.__mD=this.__mB=this.__mC=this.__mE=this.__mF=this.__mJ=this.__mI=null;}
});}
)();
...
,_getContentPaddingTarget:function(){return this.__nM;}
...
;if(this.getSelection()==null){this.__CJ=new qx.data.Array();this.setSelection(this.__CJ);}
;}
,properties:{selection:{check:b,event:f,apply:d,init:null}},events:{"changeSelection":h},members:{_modifingSelection:0,__CK:null,__CL:null,__CJ:null,_applySelection:function(k,j){if(this.__CL!=undefined&&j!=undefined){j.removeListenerById(this.__CL);}
;this.__CL=k.addListener(a,this.__CM,this);this._updateSelection();}
,__CM:function(){this._updateSelection();}
,_changeTargetSelection:function(){if(this.getTarget()==null){return;}
;if(!this.__CN()&&!this.__CO()){return;}
;if(this._inSelectionModification()){return;}
;var m=this.getTarget().getSelection();var l=this.getSelection();if(l==null){l=new qx.data.Array();this.__CJ=l;this.setSelection(l);}
...
,_addChangeTargetListener:function(p,o){if(this.__CK!=undefined&&o!=undefined){o.removeListenerById(this.__CK);}
;if(p!=null){if(this.__CN()||this.__CO()){this.__CK=p.addListener(f,this._changeTargetSelection,this);}
;}
;}
,_updateSelection:function(){if(!this.getTarget()){return;}
;this._startSelectionModification();if(this.__CN()){var s=[];for(var i=0;i<this.getSelection().length;i++ ){var t=this.getSelection().getItem(i);var u=this.__CP(t);if(u!=null){s.push(u);}
;}
...
;}
;}
else if(this.__CO()){var r=this.getSelection().getItem(this.getSelection().length-1);if(r!==undefined){this.__tc(r);this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();}
else {this.getTarget().resetSelection();}
;}
;this._endSelectionModification();}
,__CN:function(){var v=this.getTarget().constructor;return qx.Class.implementsInterface(v,qx.ui.core.IMultiSelection);}
,__CO:function(){var w=this.getTarget().constructor;return qx.Class.implementsInterface(w,qx.ui.core.ISingleSelection);}
,__tc:function(x){var y=this.__CP(x);if(y==null){return;}
;if(this.__CN()){this.getTarget().addToSelection(y);}
else if(this.__CO()){this.getTarget().setSelection([y]);}
;}
,__CP:function(z){var A=this.getTarget().getSelectables(true);for(var i=0;i<A.length;i++ ){if(A[i].getModel()==z){return A[i];}
;}
;return null;}
...
},destruct:function(){if(this.__CJ){this.__CJ.dispose();}
;}
});}
)();
...
(function(){var a="changeModel",b="_applyDelegate",c="qx.core.Object",d="_applyLabelPath",f="icon",g="change",h="label",k="' twice.",l="_applyTarget",m="changeTarget",n="_applyIconPath",o="Can not reverse bind the same target property '",p="_applyLabelOptions",q="_applyChildPath",r="_applyIconOptions",s="qx.data.controller.Tree",t="String",u="Can not bind the same target property '",v="_applyModel",w="get";qx.Class.define(s,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(y,z,A,x){qx.core.Object.call(this);this.__bb={};this.__CQ=[];this.__Ij={};if(A!=null){this.setChildPath(A);}
;if(x!=null){this.setLabelPath(x);}
;if(y!=null){this.setModel(y);}
;if(z!=null){this.setTarget(z);}
;}
,properties:{model:{check:c,apply:v,event:a,nullable:true,dereference:true},target:{apply:l,event:m,init:null,nullable:true,dereference:true},childPath:{check:t,apply:q,nullable:true},labelPath:{check:t,apply:d,nullable:true},iconPath:{check:t,apply:n,nullable:true},labelOptions:{apply:p,nullable:true},iconOptions:{apply:r,nullable:true},delegate:{apply:b,init:null,nullable:true}},members:{__Ij:null,__bb:null,__CQ:null,__Ik:null,_applyDelegate:function(C,B){this._setConfigureItem(C,B);this._setCreateItem(C,B);this._setBindItem(C,B);}
,_applyIconOptions:function(E,D){this.__Db();}
,_applyLabelOptions:function(G,F){this.__Db();}
,_applyTarget:function(I,H){if(H!=undefined){this.__Ap(H);}
;if(this.getModel()!=null){this.__In();}
;this._addChangeTargetListener(I,H);}
,_applyModel:function(K,J){this.__In();}
,_applyChildPath:function(M,L){this.__Ik=L;this.__In();this.__Ik=null;}
,_applyIconPath:function(O,N){this.__Db();}
,_applyLabelPath:function(Q,P){this.__In();}
,__Il:function(R){var T=R.getTarget();var S=this.__Ij[T.toHashCode()].treeNode;var U=this.__Ij[T.toHashCode()].modelNode;this.__Io(S,U);this._updateSelection();}
,__Im:function(e){var X=e.getData();var bc=e.getOldData();var V=this.__Ij[bc.toHashCode()];bc.removeListenerById(V.changeListenerId);delete this.__Ij[bc.toHashCode()];V.modelNode.removeListenerById(V.changeChildernListenerId);var bd=V.modelNode;var bb=qx.Class.getPropertyDefinition(V.modelNode.constructor,this.getChildPath());var Y=bb.event;var ba=bd.addListener(Y,this.__Im,this);var W=V.treeNode;this.__Ij[X.toHashCode()]={modelNode:bd,treeNode:W,changeListenerId:V.changeListenerId,changeChildernListenerId:ba};this.__Io(W,bd);this._updateSelection();}
...
;return be;}
,__In:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__Ap();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.__zt(this.getModel(),bg);this.__Io(bg,this.getModel());this.getTarget().setRoot(bg);}
;}
,__Io:function(bn,br){if(br[w+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bj=br[w+qx.lang.String.firstUp(this.getChildPath())]();if(this.__Ij[bj.toHashCode()]==undefined){var bl=bj.addListener(g,this.__Il,this);var bp=qx.Class.getPropertyDefinition(br.constructor,this.getChildPath());var bm=bp.event;var bq=br.addListener(bm,this.__Im,this);this.__Ij[bj.toHashCode()]={modelNode:br,treeNode:bn,changeListenerId:bl,changeChildernListenerId:bq};}
...
;}
else {var bo=this._createItem();bo.setModel(bj.getItem(i));bn.addAt(bo,i);this.__zt(bj.getItem(i),bo);this.__Io(bo,bj.getItem(i));}
;}
;}
;for(var i=bn.getChildren().length-1;i>=bj.length;i-- ){var bk=bn.getChildren()[i];this.__Iq(bk,bn);}
;}
,__Ap:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__Ip(bv);var bu=bv.getModel();if(bu){this.__mN(bu);}
;bv.destroy();this.__Ij={};}
;}
,__Ip:function(bw){var bx=bw.getChildren()||[];for(var i=bx.length-1;i>=0;i-- ){if(bx[i].getChildren().length>0){this.__Ip(bx[i]);}
;this.__Iq(bx[i],bw);}
;}
,__Iq:function(bz,bC){var bD=bz.getModel();var by=this.__Ik||this.getChildPath();var bA=w+qx.lang.String.firstUp(by);if(bD[bA]!=undefined){var bE=bD[bA]();var bB=this.__Ij[bE.toHashCode()];bE.removeListenerById(bB.changeListenerId);bD.removeListenerById(bB.changeChildernListenerId);this.__Ip(bz);delete this.__Ij[bE.toHashCode()];}
;this.__mN(bD);bC.remove(bz);bz.destroy();}
,bindProperty:function(bG,bJ,bI,bF,bL){var bK=bL.bind(bG,bF,bJ,bI);if(this.__bb[bJ]==null){this.__bb[bJ]={};}
;var bH=this.__bb[bJ];if(bH[bL.toHashCode()]){if(bH[bL.toHashCode()].id){throw new Error(u+bJ+k);}
;bH[bL.toHashCode()].id=bK;}
else {bH[bL.toHashCode()]={id:bK,reverseId:null,treeNode:bF};}
;if(!qx.lang.Array.contains(this.__CQ,bJ)){this.__CQ.push(bJ);}
;}
,bindPropertyReverse:function(bQ,bM,bO,bP,bS){var bR=bP.bind(bM,bS,bQ,bO);if(this.__bb[bM]==null){this.__bb[bM]={};}
;var bN=this.__bb[bM];if(bN[bS.toHashCode()]){if(bN[bS.toHashCode()].reverseId){throw new Error(o+bQ+k);}
;bN[bS.toHashCode()].reverseId=bR;}
else {bN[bS.toHashCode()]={id:null,reverseId:bR,treeNode:bP};}
;if(!qx.lang.Array.contains(this.__CQ,bM)){this.__CQ.push(bM);}
;}
...
;}
,__Db:function(){var bW;for(var ca in this.__bb){bW=ca;break;}
;for(var bX in this.__bb[bW]){var bV=this.__bb[bW][bX].treeNode;var bY=qx.core.ObjectRegistry.fromHashCode(bX);this.__mN(bY);this.__zt(bY,bV);}
;}
,__zt:function(cd,cb){var cc=this.getDelegate();if(cc!=null&&cc.bindItem!=null){cc.bindItem(this,cb,cd);}
else {this.bindDefaultProperties(cb,cd);}
;}
,__mN:function(cg){for(var i=0;i<this.__CQ.length;i++ ){var ce=this.__CQ[i];var cf=this.__bb[ce][cg.toHashCode()];if(cf!=null){if(cf.id){cg.removeBinding(cf.id);cf.id=null;}
;if(cf.reverseId){cf.treeNode.removeBinding(cf.reverseId);cf.reverseId=null;}
;delete this.__bb[ce][cg.toHashCode()];}
;}
;}
...
;this._startSelectionModification();this.__Ap();this.__In();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cn,cm){if(cn!=null&&cn.bindItem!=null){if(cm!=null&&cm.bindItem!=null&&cn.bindItem==cm.bindItem){return;}
;this.__In();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__bb=this.__Ij=this.__CQ=null;}
});}
)();
...
(function(){var a="changeModel",b="application/json",c="json",d="aborted",e="loaded",f="qx.event.type.Data",g="error",h="fail",i="receiving",j="queued",k="changeState",l="success",m="String",n="changePhase",o="sending",p="_marshaler",q="completed",r="failed",s="__op",t="qx.data.store.Json",u="configured",v="changeUrl",w="timeout",x="_applyUrl";qx.Class.define(t,{extend:qx.core.Object,construct:function(y,z){qx.core.Object.call(this);this._marshaler=new qx.data.marshal.Json(z);this._delegate=z;if(y!=null){this.setUrl(y);}
;}
,events:{"loaded":f,"error":f},properties:{model:{nullable:true,event:a},state:{check:[u,j,o,i,q,d,w,r],init:u,event:k},url:{check:m,apply:x,event:v,nullable:true}},members:{_marshaler:null,_delegate:null,__op:null,_applyUrl:function(B,A){if(B!=null){B=qx.util.AliasManager.getInstance().resolve(B);B=qx.util.ResourceManager.getInstance().toUri(B);this._createRequest(B);}
;}
,_getRequest:function(){return this.__op;}
,_setRequest:function(C){this.__op=C;}
,_createRequest:function(D){if(this.__op){this.__op.dispose();this.__op=null;}
;var E=new qx.io.request.Xhr(D);this._setRequest(E);E.setAccept(b);E.setParser(c);E.addListener(l,this._onSuccess,this);var F=this._delegate;if(F&&qx.lang.Type.isFunction(F.configureRequest)){this._delegate.configureRequest(E);}
;E.addListener(n,this._onChangePhase,this);E.addListener(h,this._onFail,this);E.send();}
,_onChangePhase:function(I){var G=I.getData(),J={},H;J={"opened":u,"sent":o,"loading":i,"success":q,"abort":d,"timeout":w,"statusError":r};H=J[G];if(H){this.setState(H);}
;}
...
;this.fireDataEvent(e,this.getModel());if(this.__op){this.__op.dispose();this.__op=null;}
;}
,reload:function(){var R=this.getUrl();if(R!=null){this._createRequest(R);}
;}
},destruct:function(){if(this.__op!=null){this._disposeObjects(s);}
;this._disposeSingletonObjects(p);this._delegate=null;}
});}
)();
...
},members:{__Aq:null,__Ar:function(z){return Object.keys(z).sort().join(a);}
,toClass:function(A,B){this.__As(A,B,null,0);}
,__As:function(N,I,C,K){if(!qx.lang.Type.isObject(N)||!!N.$$isString||N instanceof qx.core.Object){if(N instanceof Array||qx.Bootstrap.getClass(N)==h){for(var i=0;i<N.length;i++ ){this.__As(N[i],I,C+o+i+m,K+1);}
;}
;return;}
;var E=this.__Ar(N);if(this.__Aw(E,C,K)){return;}
;for(var M in N){this.__As(N[M],I,M,K+1);}
;if(qx.Class.isDefined(s+E)){return;}
;if(this.__Aq&&this.__Aq.getModelClass&&this.__Aq.getModelClass(E,N,C,K)!=null){return;}
;var O={};var F={__At:this.__At};for(var M in N){if(this.__Aq&&this.__Aq.getPropertyMapping){M=this.__Aq.getPropertyMapping(M,E);}
...
;if(this.__Aq&&this.__Aq.getValidationRule){var H=this.__Aq.getValidationRule(E,M);if(H){O[M].validate=k+M;F[k+M]=H;}
;}
;}
;if(this.__Aq&&this.__Aq.getModelSuperClass){var L=this.__Aq.getModelSuperClass(E,C,K)||qx.core.Object;}
else {var L=qx.core.Object;}
;var G=[];if(this.__Aq&&this.__Aq.getModelMixins){var J=this.__Aq.getModelMixins(E,C,K);if(!qx.lang.Type.isArray(J)){if(J!=null){G=[J];}
;}
else {G=J;}
;}
;if(I){G.push(qx.data.marshal.MEventBubbling);}
;var D={extend:L,include:G,properties:O,members:F,destruct:this.__Au};qx.Class.define(s+E,D);}
,__Au:function(){var P=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var Q in P){this.__At(this.get(P[Q].name));}
;}
,__At:function(R){if(!(R instanceof qx.core.Object)){return;}
;if(R.isDisposed()){return;}
;R.dispose();}
,__Av:function(S,W,X,V){var Y;if(this.__Aq&&this.__Aq.getModelClass){Y=this.__Aq.getModelClass(S,W,X,V);}
;if(Y!=null){return (new Y());}
else {var U=s+S;var T=qx.Class.getByName(U);if(!T){throw new Error(b+U+l);}
;return (new T());}
;}
,__Aw:function(ba,bd,bb){var bc=this.__Aq;return bc&&bc.ignore&&bc.ignore(ba,bd,bb);}
,toModel:function(be){return this.__Ax(be,null,0);}
,__Ax:function(bq,bg,bm){var bk=qx.lang.Type.isObject(bq);var bf=bq instanceof Array||qx.Bootstrap.getClass(bq)==h;if((!bk&&!bf)||!!bq.$$isString||bq instanceof qx.core.Object){return bq;}
else if(this.__Aw(this.__Ar(bq),bg,bm)){return bq;}
else if(bf){var bo=qx.data.Array;if(this.__Aq&&this.__Aq.getArrayClass){var bl=this.__Aq.getArrayClass(bg,bm);bo=bl||bo;}
;var br=new bo();br.setAutoDisposeItems(true);for(var i=0;i<bq.length;i++ ){br.push(this.__Ax(bq[i],bg+o+i+m,bm+1));}
;return br;}
else if(bk){var bh=this.__Ar(bq);var bp=this.__Av(bh,bq,bg,bm);for(var bn in bq){var bi=bn;if(this.__Aq&&this.__Aq.getPropertyMapping){bi=this.__Aq.getPropertyMapping(bn,bh);}
;var bs=bi.replace(/-|\.|\s+/g,d);if((qx.core.Environment.get(t))&&qx.core.Environment.get(e)){if(bs!=bi){this.warn(c+bn+n+bi+v);}
;}
;bi=bs;var bj=p+qx.lang.String.firstUp(bi);if(bp[bj]){bp[bj](this.__Ax(bq[bn],bn,bm+1));}
;}
;return bp;}
;throw new Error(j);}
...
(function(){var a="loadEnd",b="qx.io.request.AbstractRequest",c="changePhase",d="GET",f="qx.debug.io",g="sent",h="Abort request",i="qx.event.type.Data",j="'",k="qx.io.request.authentication.IAuthentication",l="error",m="Blob",n="qx.debug",o="fail",p="Send low-level request",q="loading",r="load",s="qx.event.type.Event",t="abort",u=", url: ",v="String",w="",x="opened",y="Response is of type: '",z="POST",A="success",B="Request completed with HTTP status: ",C="Fire readyState: ",D="statusError",E="readyStateChange",F="Abstract method call",G="abstract",H="unsent",I="changeResponse",J="Number",K="FormData",L="timeout",M="Content-Type",N="ArrayBuffer",O="undefined",P=", async: ",Q="Open low-level request with method: ";qx.Class.define(b,{type:G,extend:qx.core.Object,construct:function(R){qx.core.Object.call(this);if(R!==undefined){this.setUrl(R);}
;this.__nY={};var S=this._transport=this._createTransport();this._setPhase(H);this.__wr=qx.lang.Function.bind(this._onReadyStateChange,this);this.__ws=qx.lang.Function.bind(this._onLoad,this);this.__wt=qx.lang.Function.bind(this._onLoadEnd,this);this.__wu=qx.lang.Function.bind(this._onAbort,this);this.__tU=qx.lang.Function.bind(this._onTimeout,this);this.__wv=qx.lang.Function.bind(this._onError,this);S.onreadystatechange=this.__wr;S.onload=this.__ws;S.onloadend=this.__wt;S.onabort=this.__wu;S.ontimeout=this.__tU;S.onerror=this.__wv;}
...
,nullable:true},authentication:{check:k,nullable:true}},members:{__wr:null,__ws:null,__wt:null,__wu:null,__tU:null,__wv:null,__ww:null,__tX:null,__wx:null,__nY:null,__wy:null,_transport:null,_createTransport:function(){throw new Error(F);}
...
;this.__tX=true;this.__wx=t;this._transport.abort();}
...
;}
,_getAllRequestHeaders:function(){var bd={};qx.lang.Object.mergeWith(bd,this._getConfiguredRequestHeaders());qx.lang.Object.mergeWith(bd,this.__wz());qx.lang.Object.mergeWith(bd,this.__wy);qx.lang.Object.mergeWith(bd,this.__nY);return bd;}
,__wz:function(){var bf=this.getAuthentication(),be={};if(bf){bf.getAuthHeaders().forEach(function(bg){be[bg.key]=bg.value;}
);return be;}
;}
,setRequestHeader:function(bh,bi){this.__nY[bh]=bi;}
,getRequestHeader:function(bj){return this.__nY[bj];}
,removeRequestHeader:function(bk){if(this.__nY[bk]){delete this.__nY[bk];}
;}
,getTransport:function(){return this._transport;}
,getReadyState:function(){return this._transport.readyState;}
,getPhase:function(){return this.__wx;}
...
,getResponse:function(){return this.__ww;}
,_setResponse:function(bo){var bn=bo;if(this.__ww!==bo){this.__ww=bo;this.fireEvent(I,qx.event.type.Data,[this.__ww,bn]);}
;}
,_onReadyStateChange:function(){var bp=this.getReadyState();if(qx.core.Environment.get(f)){this.debug(C+bp);}
;this.fireEvent(E);if(this.__tX){return;}
;if(bp===3){this._setPhase(q);}
;if(this.isDone()){this.__wA();}
;}
,__wA:function(){if(qx.core.Environment.get(f)){this.debug(B+this.getStatus());}
...
,_setPhase:function(br){var bs=this.__wx;if(qx.core.Environment.get(n)){qx.core.Assert.assertString(br);qx.core.Assert.assertMatch(br,/^(unsent)|(opened)|(sent)|(loading)|(load)|(success)|(abort)|(timeout)|(statusError)$/);}
;this.__wx=br;this.fireDataEvent(c,br,bs);}
...
;var A=z[h+qx.lang.String.firstUp(name)]();if(qx.lang.Type.isArray(A)){var D=qx.data&&qx.data.IListData&&qx.Class.hasInterface(A&&A.constructor,qx.data.IListData);for(var i=0;i<A.length;i++ ){var F=D?A.getItem(i):A[i];E+=this.__wB(name,F,C);}
;}
else if(qx.lang.Type.isDate(A)&&y!=null){E+=this.__wB(name,y.format(A),C);}
else {E+=this.__wB(name,A,C);}
;}
;return E.substring(0,E.length-1);}
,__wB:function(name,I,G){if(I&&I.$$type==o){I=I.classname;}
...
(function(){var a="Retry open native request with method: ",b="activex",c="No XHR support available.",d="If-None-Match",f="xhr",g="If-Modified-Since",h="engine.version",i="onunload",j="GET",k="-1",l="qx.debug.io",m="HTMLDocument",n="Open native request with method: ",o="error",p="loadend",q="Blob",r="load",s="Received native readyState: ",t="abort",u=", url: ",v="String",w="browser.documentmode",x="",y="readystatechange",z="Microsoft.XMLHTTP",A="Already disposed",B="browser.version",C="opera",D="qx.bom.request.Xhr",E="on",F="timeout",G="gecko",H="If-Match",I="mshtml",J="engine.name",K="Microsoft.XMLDOM",L="file:",M="Send native request",N="FormData",O="If-Range",P="Content-Type",Q="io.xhr",R="Not enough arguments",S="ArrayBuffer",T="undefined",U=", async: ",V="Native XHR object doesn't support overrideMimeType.";qx.Bootstrap.define(D,{extend:Object,construct:function(){var W=qx.Bootstrap.bind(this.__wP,this);if(qx.event&&qx.event.GlobalError&&qx.event.GlobalError.observeMethod){this.__wD=qx.event.GlobalError.observeMethod(W);}
else {this.__wD=W;}
;this.__wE=qx.Bootstrap.bind(this.__wO,this);this.__tU=qx.Bootstrap.bind(this.__wT,this);this.__wN();this._emitter=new qx.event.Emitter();if(window.attachEvent){this.__wF=qx.Bootstrap.bind(this.__wW,this);window.attachEvent(i,this.__wF);}
;}
,statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},events:{"readystatechange":D,"error":D,"loadend":D,"timeout":D,"abort":D,"load":D},members:{readyState:0,responseText:x,responseXML:null,status:0,statusText:x,timeout:0,open:function(bc,X,Y,bb,ba){this.__wY();if(typeof X===T){throw new Error(R);}
else if(typeof bc===T){bc=j;}
;this.__tX=false;this.__wG=false;this.__wH=false;this.__tY=X;if(typeof Y==T){Y=true;}
;this.__wI=Y;if(!this.__wX()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();this.__wN();}
;this.__wK.onreadystatechange=this.__wD;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,n+bc+u+X+U+Y);}
;this.__wK.open(bc,X,Y,bb,ba);}
catch(bd){if(!qx.util.Request.isCrossDomain(X)){throw bd;}
;if(!this.__wI){this.__wJ=bd;}
;if(this.__wI){if(window.XDomainRequest){this.readyState=4;this.__wK=new XDomainRequest();this.__wK.onerror=qx.Bootstrap.bind(function(){this._emit(y);this._emit(o);this._emit(p);}
,this);if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,a+bc+u+X+U+Y);}
;this.__wK.open(bc,X,Y,bb,ba);return;}
;window.setTimeout(qx.Bootstrap.bind(function(){if(this.__ua){return;}
;this.readyState=4;this._emit(y);this._emit(o);this._emit(p);}
,this));}
;}
;if(qx.core.Environment.get(J)===I&&qx.core.Environment.get(w)<9&&this.__wK.readyState>0){this.__wK.setRequestHeader(g,k);}
;if(qx.core.Environment.get(J)===G&&parseInt(qx.core.Environment.get(h),10)<2&&!this.__wI){this.readyState=qx.bom.request.Xhr.OPENED;this._emit(y);}
;}
,setRequestHeader:function(be,bf){this.__wY();if(be==H||be==g||be==d||be==O){this.__wH=true;}
;this.__wK.setRequestHeader(be,bf);return this;}
,send:function(bh){this.__wY();if(!this.__wI&&this.__wJ){throw this.__wJ;}
;if(qx.core.Environment.get(J)===C&&this.timeout===0){this.timeout=10000;}
;if(this.timeout>0){this.__wL=window.setTimeout(this.__tU,this.timeout);}
;bh=typeof bh==T?null:bh;var bg=qx.Bootstrap.getClass(bh);bh=(bh!==null&&this.__wM.indexOf(bg)===-1)?bh.toString():bh;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,M);}
;this.__wK.send(bh);}
catch(bj){if(!this.__wI){throw bj;}
;if(this._getProtocol()===L){this.readyState=2;this.__wQ();var bi=this;window.setTimeout(function(){if(bi.__ua){return;}
;bi.readyState=3;bi.__wQ();bi.readyState=4;bi.__wQ();}
);}
;}
;if(qx.core.Environment.get(J)===G&&!this.__wI){this.__wP();}
;this.__wG=true;return this;}
,abort:function(){this.__wY();this.__tX=true;this.__wK.abort();if(this.__wK){this.readyState=this.__wK.readyState;}
;return this;}
...
,getResponseHeader:function(bm){this.__wY();if(qx.core.Environment.get(w)===9&&this.__wK.aborted){return x;}
;return this.__wK.getResponseHeader(bm);}
,getAllResponseHeaders:function(){this.__wY();if(qx.core.Environment.get(w)===9&&this.__wK.aborted){return x;}
;return this.__wK.getAllResponseHeaders();}
,overrideMimeType:function(bn){this.__wY();if(this.__wK.overrideMimeType){this.__wK.overrideMimeType(bn);}
else {throw new Error(V);}
;return this;}
,getRequest:function(){return this.__wK;}
,dispose:function(){if(this.__ua){return false;}
;window.clearTimeout(this.__wL);if(window.detachEvent){window.detachEvent(i,this.__wF);}
;try{this.__wK.onreadystatechange;}
catch(bp){return false;}
;var bo=function(){}
;this.__wK.onreadystatechange=bo;this.__wK.onload=bo;this.__wK.onerror=bo;this.abort();this.__wK=null;this.__ua=true;return true;}
,isDisposed:function(){return !!this.__ua;}
...
,_getProtocol:function(){var br=this.__tY;var bs=/^(\w+:)\/\//;if(br!==null&&br.match){var bt=br.match(bs);if(bt&&bt[1]){return bt[1];}
;}
;return window.location.protocol;}
,__wK:null,__wI:null,__wD:null,__wE:null,__wF:null,__tU:null,__wG:null,__tY:null,__tX:null,__gy:null,__ua:null,__wL:null,__wJ:null,__wH:null,__wM:null,__wN:function(){this.__wK=this._createNativeXhr();this.__wK.onreadystatechange=this.__wD;if(this.__wK.onabort){this.__wK.onabort=this.__wE;}
;this.__ua=this.__wG=this.__tX=false;this.__wM=[S,q,m,v,N];}
,__wO:function(){if(!this.__tX){this.abort();}
;}
,__wP:function(){var bu=this.__wK,bv=true;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,s+bu.readyState);}
;if(this.readyState==bu.readyState){return;}
;this.readyState=bu.readyState;if(this.readyState===qx.bom.request.Xhr.DONE&&this.__tX&&!this.__wG){return;}
;if(!this.__wI&&(bu.readyState==2||bu.readyState==3)){return;}
...
;if(bv){this.__wU();this.__wV();}
;}
;this.__wQ();if(this.readyState==qx.bom.request.Xhr.DONE){if(bu){bu.onreadystatechange=function(){}
;}
;}
;}
,__wQ:function(){if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__wL);}
;this._emit(y);if(this.readyState===qx.bom.request.Xhr.DONE){this.__wR();}
;}
,__wR:function(){if(this.__gy){this._emit(F);if(qx.core.Environment.get(J)===C){this._emit(o);}
;this.__gy=false;}
else {if(this.__tX){this._emit(t);}
else {if(this.__wS()){this._emit(o);}
else {this._emit(r);}
;}
;}
;this._emit(p);}
,__wS:function(){var bx;if(this._getProtocol()===L){bx=!this.responseText;}
else {bx=!this.statusText&&this.status!==204;}
;return bx;}
,__wT:function(){var by=this.__wK;this.readyState=qx.bom.request.Xhr.DONE;this.__gy=true;by.aborted=true;by.abort();this.responseText=x;this.responseXML=null;this.__wQ();}
,__wU:function(){var bz=this.readyState===qx.bom.request.Xhr.DONE;if(this._getProtocol()===L&&this.status===0&&bz){if(!this.__wS()){this.status=200;}
;}
;if(this.status===1223){this.status=204;}
;if(qx.core.Environment.get(J)===C){if(bz&&this.__wH&&!this.__tX&&this.status===0){this.status=304;}
;}
;}
,__wV:function(){if(qx.core.Environment.get(J)==I&&(this.getResponseHeader(P)||x).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var bA=new window.ActiveXObject(K);bA.async=false;bA.validateOnParse=false;bA.loadXML(this.responseText);this.responseXML=bA;}
;}
,__wW:function(){try{if(this){this.dispose();}
;}
catch(e){}
;}
,__wX:function(){var name=qx.core.Environment.get(J);var bB=qx.core.Environment.get(B);return !(name==I&&bB<9||name==G&&bB<3.5);}
,__wY:function(){if(this.__ua){throw new Error(A);}
;}
...
;}
,statics:{PARSER:{json:qx.lang.Json.parse,xml:qx.xml.Document.fromString}},members:{__wC:null,parse:function(h,g){var f=this._getParser(g);if(typeof f===a){if(h!==c){return f.call(this,h);}
;}
;return h;}
,setParser:function(i){if(typeof qx.util.ResponseParser.PARSER[i]===a){return this.__wC=qx.util.ResponseParser.PARSER[i];}
;if(qx.core.Environment.get(d)){qx.core.Assert.assertFunction(i);}
;return this.__wC=i;}
,_getParser:function(k){var j=this.__wC,m=c,l=c;if(j){return j;}
...
;return Y+ba;}
,escapeHTML:function(bd){return String(bd).replace(/[<>&"']/g,this.__Bf);}
,__Bf:function(bf){var be={"<":m,">":e,"&":s,"'":t,'"':D};return be[bf]||L;}
...
,process:function(bd){this.__co.appendChild(qx.log.appender.Util.toHtml(bd));this.__Bc();}
,__Bc:function(){this.__co.scrollTop=this.__co.scrollHeight;}
,__gM:true,toggle:function(){if(!this.__AY){this.init();}
else if(this.__AY.style.display==p){this.show();}
else {this.__AY.style.display=p;}
;}
,show:function(){if(!this.__AY){this.init();}
else {this.__AY.style.display=P;this.__co.scrollTop=this.__co.scrollHeight;}
;}
,__Bd:[],execute:function(){var bf=this.__Ba.value;if(bf==x){return;}
;if(bf==F){this.clear();return;}
;var be=document.createElement(j);be.innerHTML=qx.log.appender.Util.escapeHTML(V+bf);be.className=k;this.__Bd.push(bf);this.__Bb=this.__Bd.length;this.__co.appendChild(be);this.__Bc();try{var bg=window.eval(bf);}
catch(bh){qx.log.Logger.error(bh);}
;if(bg!==undefined){qx.log.Logger.debug(bg);}
;}
,__Be:function(e){this.__co.style.height=(this.__AY.clientHeight-this.__AY.firstChild.offsetHeight-this.__AY.lastChild.offsetHeight)+c;}
...
;if(!this.__AY){return;}
;if(!qx.dom.Hierarchy.contains(this.__AY,e.getTarget())){return;}
;if(bj==b&&this.__Ba.value!=x){this.execute();this.__Ba.value=x;}
;if(bj==m||bj==W){this.__Bb+=bj==m?-1:1;this.__Bb=Math.min(Math.max(0,this.__Bb),this.__Bd.length);var bi=this.__Bd[this.__Bb];this.__Ba.value=bi||x;this.__Ba.select();}
;}
