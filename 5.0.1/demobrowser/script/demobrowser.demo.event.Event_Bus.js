...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.event.Event_Bus.485c30d04b71.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,_initLogger:function(p,n,o){this.__OH=f+p.join(c)+a;this.__OI=o||50;this.__OJ=n;this._clearLog();}
,_clearLog:function(){this.__OJ.innerHTML=this.__OH+k;this.__OK=[];}
,_log:function(q){this.__OK.unshift(q);this.__OK=this.__OK.slice(0,this.__OI);var r=[this.__OH];for(var i=0;i<this.__OK.length;i++ ){r.push(g,this.__OK[i].join(e),l);}
;r.push(k);this.__OJ.innerHTML=r.join(d);this.__OJ.scrollTop=0;}
,__OH:null,__OI:null,__OJ:null,__OK:null},destruct:function(){this.__OJ=this.__OK=null;}
});}
)();
...
},construct:function(){this.__xs={};}
,members:{__xs:null,getSubscriptions:function(){return this.__xs;}
...
;}
;return false;}
,dispatch:function(L){var G=this.getSubscriptions();var H=L.getName();var I=false;for(var K in G){var J=K.indexOf(c);if(J>-1){if(J===0||K.substr(0,J)===H.substr(0,J)){this.__xt(G[K],L);I=true;}
;}
else {if(K===H){this.__xt(G[H],L);I=true;}
;}
;}
;return I;}
,dispatchByName:function(name,N){var M=new qx.event.message.Message(name,N);return this.dispatch(M);}
,__xt:function(P,O){for(var i=0;i<P.length;i++ ){var R=P[i].subscriber;var Q=P[i].context;if(Q&&Q.isDisposed){if(Q.isDisposed()){P.splice(i,1);i-- ;}
else {R.call(Q,O);}
;}
else {R.call(Q,O);}
;}
