...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Columns.6f415dc15d9c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,openNodeAndParents:function(bk){this.__AU(this.getModel(),bk);this.buildLookupTable();}
,closeNode:function(bl){if(qx.lang.Array.contains(this.__AO,bl)){qx.lang.Array.remove(this.__AO,bl);this.fireDataEvent(G,bl);this.buildLookupTable();}
;}
...
,isNodeOpen:function(bo){return qx.lang.Array.contains(this.__AO,bo);}
,_init:function(){this.__zQ=new qx.data.Array();this.__AO=[];this.__AP=[];this._initLayer();}
...
,this);}
,getLookupTable:function(){return this.__zQ;}
,_reverseLookup:function(bp){return bp;}
,_getDataFromRow:function(bq){return this.__zQ.getItem(bq);}
,_getSelectables:function(){return this.__zQ;}
,getOpenNodes:function(){return this.__AO;}
,isNode:function(br){return qx.ui.tree.core.Util.isNode(br,this.getChildProperty());}
,getLevel:function(bs){return this.__AP[bs];}
...
,_applyModel:function(bQ,bP){this.__AO=[];if(bQ!=null){if(qx.core.Environment.get(w)){if(!qx.Class.hasMixin(bQ.constructor,qx.data.marshal.MEventBubbling)){this.warn(H+h+k);}
;}
;bQ.addListener(J,this._onChangeBubble,this);this.__AT(bQ);}
...
;}
;if(this.__zQ.indexOf(bV)!=-1){this.__AR();}
;}
;}
,_onUpdated:function(event){if(this.__hi==null){this.__hi=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__hi.schedule();}
,_onOpen:function(event){var ca=event.getRow();var bY=this.__zQ.getItem(ca);if(this.isNode(bY)){if(this.isNodeOpen(bY)){this.closeNode(bY);}
...
;break;};}
;}
,_beforeApplySelection:function(cg){if(cg.length===0&&this.getSelectionMode()===g){var cf=this.__AW();var ch=this.getLookupTable().indexOf(cf);if(ch>=0){cg.push(ch);}
;}
;}
,_afterApplySelection:function(){var ci=this.getSelection();if(ci.getLength()>0&&this.getSelectionMode()===g){this.__AV(ci.getItem(0));}
else {this.__AQ=[];}
;}
,__AR:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(Q+f);}
;this._itemWidth=0;var cm=[];this.__AP=[];var ck=-1;var cl=this.getModel();if(cl!=null){if(!this.isHideRoot()){ck++ ;cm.push(cl);this.__AP.push(ck);}
;if(this.isNodeOpen(cl)){var cj=this.__AS(cl,ck);cm=cm.concat(cj);}
;}
;if(!qx.lang.Array.equals(this.__zQ.toArray(),cm)){this._provider.removeBindings();this.__zQ.removeAll();this.__zQ.append(cm);this.__zW();this._updateSelection();}
;}
,__AS:function(cn,cs){var cr=[];cs++ ;if(!this.isNode(cn)){return cr;}
...
;if(this.isNode(cu)){this.__AP.push(cs);cr.push(cu);if(this.isNodeOpen(cu)){var co=this.__AS(cu,cs);cr=cr.concat(co);}
;}
else {if(this.isShowLeafs()){this.__AP.push(cs);cr.push(cu);}
;}
;}
;cv.dispose();return cr;}
,__AT:function(cw){if(!qx.lang.Array.contains(this.__AO,cw)){this.__AO.push(cw);this.fireDataEvent(t,cw);}
;}
,__AU:function(cx,cy){if(cx===cy){this.__AT(cy);return true;}
;if(!this.isNode(cx)){return false;}
;var cB=cx.get(this.getChildProperty());if(cB==null){return false;}
;for(var i=0;i<cB.getLength();i++ ){var cA=cB.getItem(i);var cz=this.__AU(cA,cy);if(cz===true){this.__AT(cA);return true;}
;}
;return false;}
,__zW:function(){this.getPane().getRowConfig().setItemCount(this.__zQ.getLength());this.getPane().fullUpdate();}
,getParent:function(cD){var cE=this.__zQ.indexOf(cD);if(cE<0){return null;}
;var cF=this.__AP[cE];while(cE>0){cE-- ;var cC=this.__AP[cE];if(cC<cF){return this.__zQ.getItem(cE);}
;}
;return null;}
,__AV:function(cG){this.__AQ=[];var parent=this.getParent(cG);while(parent!=null){this.__AQ.unshift(parent);parent=this.getParent(parent);}
;}
,__AW:function(){if(this.__AQ==null){return this.getModel();}
;var cH=this.getLookupTable();var parent=this.__AQ.pop();while(parent!=null){if(cH.contains(parent)){return parent;}
;parent=this.__AQ.pop();}
...
;this._layer.removeListener(R,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__zQ.dispose();this._layer=this._provider=this.__zQ=this.__AO=this.__hi=null;}
});}
)();
...
(function(){var a="BindingIds",b="model",c="",d="changeDelegate",f="virtual-tree-file",g="label",h="appearance",i="icon",j="bindItem",k=".length",l="]",m="virtual-tree-folder",n="[",o=".",p="String",q="qx.ui.tree.core.MWidgetController";qx.Mixin.define(q,{construct:function(){this.__zr=[];}
,properties:{labelPath:{check:p,nullable:true},iconPath:{check:p,nullable:true},labelOptions:{nullable:true},iconOptions:{nullable:true},childProperty:{check:p,nullable:true},delegate:{event:d,init:null,nullable:true}},members:{__zr:null,bindDefaultProperties:function(u,s){this.bindProperty(c,b,null,u,s);this.bindProperty(this.getLabelPath(),g,this.getLabelOptions(),u,s);var r=this.__zs(s);var t=this._tree.getLookupTable();t=qx.data.SingleValueBinding.resolvePropertyChain(t,r);if(qx.util.OOUtil.hasProperty(t.constructor,this.getChildProperty())){this.bindProperty(this.getChildProperty()+k,h,{converter:function(){return m;}
},u,s);}
else {u.setAppearance(f);}
;if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),i,this.getIconOptions(),u,s);}
;}
,bindProperty:function(y,B,z,x,v){var A=this.__zs(v,y);var w=this._tree.getLookupTable();var C=w.bind(A,x,B,z);this.__zt(x,C);}
,bindPropertyReverse:function(J,F,H,I,D){var G=this.__zs(D,J);var E=this._tree.getLookupTable();var K=I.bind(F,E,G,H);this.__zt(I,K);}
,removeBindings:function(){while(this.__zr.length>0){var L=this.__zr.pop();this._removeBindingsFrom(L);}
;}
...
;}
,_removeBindingsFrom:function(P){var Q=this.__zu(P);while(Q.length>0){var R=Q.pop();try{this._tree.getLookupTable().removeBinding(R);}
catch(e){P.removeBinding(R);}
;}
;if(qx.lang.Array.contains(this.__zr,P)){qx.lang.Array.remove(this.__zr,P);}
;}
,__zs:function(T,U){var S=n+T+l;if(U!=null&&U!=c){S+=o+U;}
;return S;}
,__zt:function(W,X){var V=this.__zu(W);if(!qx.lang.Array.contains(V,X)){V.push(X);}
;if(!qx.lang.Array.contains(this.__zr,W)){this.__zr.push(W);}
;}
,__zu:function(Y){var ba=Y.getUserData(a);if(ba==null){ba=[];Y.setUserData(a,ba);}
;return ba;}
...
;var o=this._renderer.getCellWidget();o.setOpen(p&&this._tree.isNodeOpen(s));o.addListener(a,this.__AX,this);o.setUserData(b,this.getChildProperty());o.setUserData(g,this._tree.isShowLeafs());if(this._tree.getSelection().contains(s)){this._styleSelectabled(o);}
...
,poolCellWidget:function(u){u.removeListener(a,this.__AX,this);this._removeBindingsFrom(u);this._renderer.pool(u);this._onPool(u);}
...
,__AX:function(event){var K=event.getTarget();var L=K.getUserData(k);var J=this._tree.getLookupTable().getItem(L);if(event.getData()){this._tree.openNodeWithoutScrolling(J);}
