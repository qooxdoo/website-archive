...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.CommandGroupManager.975c4dbf6291.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="qx.ui.command.GroupManager",b="Group must be an instance of qx.ui.command.Group",c="qx.debug",d="Group was not added before. Please use ",e="'add()' method to add the group.",f="Given group is not an instance of qx.ui.command.Group",g="Group was not added before! You have to use 'addCommand()' method before activating!",h="Group is already added!";qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__GB=[];}
,members:{__GB:null,__Ie:null,add:function(j){if(qx.core.Environment.get(c)){this.assertInstance(j,qx.ui.command.Group,f);}
;if(qx.lang.Array.contains(this.__GB,j)){if(qx.core.Environment.get(c)){this.debug(h);}
;return false;}
;this.__GB.push(j);j.setActive(false);return true;}
...
;var l=this.__GB.indexOf(m);if(l===-1){if(qx.core.Environment.get(c)){this.debug(d+e);}
;}
;if(this.getActive()===m){this.__Ie=null;}
;this.__GB.splice(l,1);return m;}
...
;return false;}
;for(var i=0;i<this.__GB.length;i++ ){var n=this.__GB[i];if(n==o){n.setActive(true);this.__Ie=n;continue;}
;n.setActive(false);}
;return true;}
,getActive:function(){return this.__Ie;}
,block:function(){if(this.__Ie){this.__Ie.setActive(false);}
;}
,unblock:function(){if(this.__Ie){this.__Ie.setActive(true);}
;}
,_getGroup:function(q){var p=this.__GB.indexOf(q);if(p===-1){return null;}
;return this.__GB[p];}
},destruct:function(){this.__GB=this.__Ie=null;}
});}
)();
...
(function(){var b="-",c="PageUp",d="Escape",f="Boolean",g="+",h="qx.event.type.Data",j="_applyShortcut",k="PrintScreen",l="NumLock",m="short",n="keydown",o="5",p="8",q="execute",r="Meta",s="0",t="",u="2",v="Control",w="Shift",x="You can only specify one non modifier key!",y="3",z="Unidentified",A="/",B="Delete",C="String",D="changeEnabled",E="*",F="qx.bom.Shortcut",G="6",H="4",I="1",J="Alt",K="Not a valid key name for a shortcut: ",L="PageDown",M="Whitespaces are not allowed within shortcuts",N="_applyEnabled",O="keypress",P="7",Q="a",R="z",S="9";qx.Class.define(F,{extend:qx.core.Object,construct:function(T){qx.core.Object.call(this);this.__Dn={};this.__Do=null;if(T!=null){this.setShortcut(T);}
;this.initEnabled();}
,events:{"execute":h},properties:{enabled:{init:true,check:f,event:D,apply:N},shortcut:{check:C,apply:j,nullable:true},autoRepeat:{check:f,init:false}},members:{__Dn:t,__Do:t,execute:function(U){this.fireDataEvent(q,U);}
,__Dp:function(event){if(this.getEnabled()&&this.__Dq(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,__hJ:function(event){if(this.getEnabled()&&this.__Dq(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,_applyEnabled:function(W,V){if(W){qx.event.Registration.addListener(document.documentElement,n,this.__Dp,this);qx.event.Registration.addListener(document.documentElement,O,this.__hJ,this);}
else {qx.event.Registration.removeListener(document.documentElement,n,this.__Dp,this);qx.event.Registration.removeListener(document.documentElement,O,this.__hJ,this);}
;}
,_applyShortcut:function(bb,Y){if(bb){if(bb.search(/[\s]+/)!=-1){var bc=M;this.error(bc);throw new Error(bc);}
;this.__Dn={"Control":false,"Shift":false,"Meta":false,"Alt":false};this.__Do=null;var X;var a=[];while(bb.length>0&&X!=-1){X=bb.search(/[-+]+/);a.push((bb.length==1||X==-1)?bb:bb.substring(0,X));bb=bb.substring(X+1);}
;var ba=a.length;for(var i=0;i<ba;i++ ){var bd=this.__Ds(a[i]);switch(bd){case v:case w:case r:case J:this.__Dn[bd]=true;break;case z:var bc=K+a[i];this.error(bc);throw bc;default:if(this.__Do){var bc=x;this.error(bc);throw bc;}
;this.__Do=bd;};}
;}
;return true;}
,__Dq:function(e){var be=this.__Do;if(!be){return false;}
;if((!this.__Dn.Shift&&e.isShiftPressed())||(this.__Dn.Shift&&!e.isShiftPressed())||(!this.__Dn.Control&&e.isCtrlPressed())||(this.__Dn.Control&&!e.isCtrlPressed())||(!this.__Dn.Meta&&e.isMetaPressed())||(this.__Dn.Meta&&!e.isMetaPressed())||(!this.__Dn.Alt&&e.isAltPressed())||(this.__Dn.Alt&&!e.isAltPressed())){return false;}
;if(be==e.getKeyIdentifier()){return true;}
;return false;}
,__Dr:{esc:d,ctrl:v,print:k,del:B,pageup:c,pagedown:L,numlock:l,numpad_0:s,numpad_1:I,numpad_2:u,numpad_3:y,numpad_4:H,numpad_5:o,numpad_6:G,numpad_7:P,numpad_8:p,numpad_9:S,numpad_divide:A,numpad_multiply:E,numpad_minus:b,numpad_plus:g},__Ds:function(bg){var bf=qx.event.util.Keyboard;var bh=z;if(bf.isValidKeyIdentifier(bg)){return bg;}
;if(bg.length==1&&bg>=Q&&bg<=R){return bg.toUpperCase();}
;bg=bg.toLowerCase();var bh=this.__Dr[bg]||qx.lang.String.firstUp(bg);if(bf.isValidKeyIdentifier(bh)){return bh;}
else {return z;}
;}
,toString:function(){var bj=this.__Do;var bk=[];for(var bi in this.__Dn){if(this.__Dn[bi]){bk.push(qx.locale.Key.getKeyName(m,bi));}
;}
;if(bj){bk.push(qx.locale.Key.getKeyName(m,bj));}
;return bk.join(g);}
},destruct:function(){this.setEnabled(false);this.__Dn=this.__Do=null;}
});}
)();
