...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ListControllerWith3Widgets.6d6f19e50ba1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,this);}
,properties:{appearance:{refine:true,init:t},placeholder:{check:n,nullable:true,apply:w}},events:{"changeValue":j},members:{__uI:null,__Bo:null,_applyPlaceholder:function(A,z){this.getChildControl(p).setPlaceholder(A);}
...
,_setPreselectedItem:function(){if(this.__uI){var J=this.__uI.getLabel();if(this.getFormat()!=null){J=this.getFormat().call(this,this.__uI);}
;if(J&&J.translate){J=J.translate();}
;this.setValue(J);this.__uI=null;}
;}
,_onListChangeSelection:function(e){var K=e.getData();if(K.length>0){var M=this.getChildControl(o);var L=M.getSelectionContext();if(L==m||L==f){this.__uI=K[0];}
else {var N=K[0].getLabel();if(this.getFormat()!=null){N=this.getFormat().call(this,K[0]);}
;if(N&&N.translate){N=N.translate();}
;this.setValue(N);this.__uI=null;}
;}
;}
...
(function(){var a="changeModel",b="_applyLabelOptions",c="_applyLabelPath",d="changeTarget",e="_applyModel",f="change",g="icon",h="ReverseBindingId",k="qx.data.controller.List",m="_applyIconPath",n="BindingId",o="_applyDelegate",p="",q="changeDelegate",r="_applyTarget",s="]",t="qx.data.IListData",u="model[",v="model",w="label",x="String",y="_applyIconOptions",z=".";qx.Class.define(k,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(A,B,C){qx.core.Object.call(this);this.__zQ=[];this.__CQ=[];this.__CR=[];this.__CS={};if(C!=null){this.setLabelPath(C);}
;if(A!=null){this.setModel(A);}
;if(B!=null){this.setTarget(B);}
;}
,properties:{model:{check:t,apply:e,event:a,nullable:true,dereference:true},target:{apply:r,event:d,nullable:true,init:null,dereference:true},labelPath:{check:x,apply:c,nullable:true},iconPath:{check:x,apply:m,nullable:true},labelOptions:{apply:b,nullable:true},iconOptions:{apply:y,nullable:true},delegate:{apply:o,event:q,init:null,nullable:true}},members:{__An:null,__zQ:null,__CS:null,__CQ:null,__CR:null,__CT:null,__CU:null,update:function(){this.__CW();this.__Db();this._updateSelection();}
,_applyDelegate:function(E,D){this._setConfigureItem(E,D);this._setFilter(E,D);this._setCreateItem(E,D);this._setBindItem(E,D);}
,_applyIconOptions:function(G,F){this.__Db();}
,_applyLabelOptions:function(I,H){this.__Db();}
,_applyIconPath:function(K,J){this.__Db();}
,_applyLabelPath:function(M,L){this.__Db();}
,_applyModel:function(P,N){if(N!=undefined){if(this.__An!=undefined){N.removeListenerById(this.__An);}
;}
;if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();}
;if(P!=null){this.__An=P.addListener(f,this.__CV,this);this.__zY();this.__CW();if(N==null){this._changeTargetSelection();}
else {this.__CT=true;qx.ui.core.queue.Widget.add(this);}
;}
else {var O=this.getTarget();if(O!=null){var length=O.getChildren().length;for(var i=0;i<length;i++ ){this.__Da();}
;}
;}
;}
...
;if(S!=null){if(this.getModel()!=null){for(var i=0;i<this.__zQ.length;i++ ){this.__CY(this.__Dd(i));}
;}
;}
;}
,__CV:function(){this.__CU=true;qx.ui.core.queue.Widget.add(this);if(this.__zQ.length!=this.getModel().getLength()){this.update();}
;}
,syncWidget:function(){if(this.__CT){this._changeTargetSelection();}
;if(this.__CU){this._updateSelection();}
;this.__CU=this.__CT=null;}
,__CW:function(){if(this.getTarget()==null){return;}
;this.__zY();var U=this.__zQ.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++ ){this.__CY(this.__Dd(j));}
;}
else if(U<T){for(var j=T;j>U;j-- ){this.__Da();}
;}
;this.__CT=true;qx.ui.core.queue.Widget.add(this);}
,__CX:function(){var V=this.getModel();if(V!=null){V.removeListenerById(this.__An);this.__An=V.addListener(f,this.__CV,this);}
;}
...
;return W;}
,__CY:function(Y){var ba=this._createItem();this._bindListItem(ba,Y);this.getTarget().add(ba);}
,__Da:function(){this._startSelectionModification();var bc=this.getTarget().getChildren();var bb=bc.length-1;var bd=bc[bb];this._removeBindingsFrom(bd);this.getTarget().removeAt(bb);bd.destroy();this._endSelectionModification();}
...
;}
,bindProperty:function(bo,br,bp,bn,bm){if(bp!=null){var bp=qx.lang.Object.clone(bp);this.__CS[br]=bp.onUpdate;delete bp.onUpdate;}
else {bp={};this.__CS[br]=null;}
;bp.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,bm);bp.ignoreConverter=v;var bq=u+bm+s;if(bo!=null&&bo!=p){bq+=z+bo;}
;var bs=this.bind(bq,bn,br,bp);bn.setUserData(br+n,bs);if(!qx.lang.Array.contains(this.__CQ,br)){this.__CQ.push(br);}
;}
,bindPropertyReverse:function(by,bv,bw,bx,bt){var bu=u+bt+s;if(by!=null&&by!=p){bu+=z+by;}
;var bz=bx.bind(bv,this,bu,bw);bx.setUserData(by+h,bz);if(!qx.lang.Array.contains(this.__CR,by)){this.__CR.push(by);}
;}
,_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;}
;for(var i=0;i<this.__CQ.length;i++ ){if(this.__CS[this.__CQ[i]]!=null){this.__CS[this.__CQ[i]]();}
;}
;}
,_removeBindingsFrom:function(bD){for(var i=0;i<this.__CQ.length;i++ ){var bE=bD.getUserData(this.__CQ[i]+n);if(bE!=null){this.removeBinding(bE);}
;}
;for(var i=0;i<this.__CR.length;i++ ){var bE=bD.getUserData(this.__CR[i]+h);if(bE!=null){bD.removeBinding(bE);}
;}
;}
,__Db:function(){if(this.getTarget()==null||this.getModel()==null){return;}
;var bF=this.getTarget().getChildren();for(var i=0;i<bF.length;i++ ){this._removeBindingsFrom(bF[i]);this._bindListItem(bF[i],this.__Dd(i));}
;this.__CX();}
...
,_setFilter:function(bR,bP){if((bR==null||bR.filter==null)&&(bP!=null&&bP.filter!=null)){this.__Dc();}
...
;var bQ=this.__zQ;this.__zY();if(bQ.length>this.__zQ.length){for(var j=bQ.length;j>this.__zQ.length;j-- ){this.getTarget().removeAt(j-1).destroy();}
;}
else if(bQ.length<this.__zQ.length){for(var j=bQ.length;j<this.__zQ.length;j++ ){var bT=this._createItem();this.getTarget().add(bT);}
;}
;var bS=this.getTarget().getChildren();for(var i=0;i<bS.length;i++ ){this._bindListItem(bS[i],this.__Dd(i));}
;this.__CX();this._endSelectionModification();this._updateSelection();}
,__Dc:function(){this.__zY();this.__CW();this.__Db();this.__CU=true;qx.ui.core.queue.Widget.add(this);}
,__zY:function(){var bW=this.getModel();if(bW==null){return;}
;var bX=this.getDelegate();if(bX!=null){var bV=bX.filter;}
;this.__zQ=[];for(var i=0;i<bW.getLength();i++ ){if(bV==null||bV(bW.getItem(i))){this.__zQ.push(i);}
;}
;}
,__Dd:function(bY){return this.__zQ[bY];}
},destruct:function(){this.__zQ=this.__CS=this.__CQ=null;this.__CR=null;qx.ui.core.queue.Widget.remove(this);}
});}
)();
