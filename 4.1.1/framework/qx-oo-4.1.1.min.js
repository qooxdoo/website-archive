...
;R=/@(.+):(\d+)$/gm;while((S=R.exec(bd.stack))!=null){bb=S[1];Q=S[2];ba=this.__bS(bb);T.push(ba+m+Q);}
;if(T.length>0){return this.__bU(T);}
;R=/at (.*)/gm;var be=/\((.*?)(:[^\/].*)\)/;var Y=/(.*?)(:[^\/].*)/;while((S=R.exec(bd.stack))!=null){var X=be.exec(S[1]);if(!X){X=Y.exec(S[1]);}
;if(X){ba=this.__bS(X[1]);T.push(ba+X[2]);}
else {T.push(S[1]);}
;}
;}
else if(bc===q){var U=bd.stacktrace;if(!U){return T;}
;if(U.indexOf(h)>=0){U=U.split(h)[0];}
;R=/line\ (\d+?),\ column\ (\d+?)\ in\ (?:.*?)\ in\ (.*?):[^\/]/gm;while((S=R.exec(U))!=null){Q=S[1];P=S[2];bb=S[3];ba=this.__bS(bb);T.push(ba+m+Q+m+P);}
;if(T.length>0){return this.__bU(T);}
;R=/Line\ (\d+?)\ of\ linked\ script\ (.*?)$/gm;while((S=R.exec(U))!=null){Q=S[1];bb=S[2];ba=this.__bS(bb);T.push(ba+m+Q);}
;}
else if(bd.message&&bd.message.indexOf(k)>=0){var W=bd.message.split(k)[1].trim();var V=W.split(e);for(var i=0;i<V.length;i++ ){var O=V[i].match(/\s*Line ([0-9]+) of.* (\S.*)/);if(O&&O.length>=2){Q=O[1];bf=this.__bS(O[2]);T.push(bf+m+Q);}
;}
;}
else if(bd.sourceURL&&bd.line){T.push(this.__bS(bd.sourceURL)+m+bd.line);}
;return this.__bU(T);}
,__bS:function(bh){if(typeof qx.dev.StackTrace.FILENAME_TO_CLASSNAME==o){var bg=qx.dev.StackTrace.FILENAME_TO_CLASSNAME(bh);{}
;return bg;}
;return qx.dev.StackTrace.__bT(bh);}
,__bT:function(bk){var bl=g;var bi=bk.indexOf(bl);var bm=bk.indexOf(f);if(bm>=0){bk=bk.substring(0,bm);}
;var bj=(bi==-1)?bk:bk.substring(bi+bl.length).replace(/\//g,n).replace(/\.js$/,d);return bj;}
,__bU:function(bn){if(typeof qx.dev.StackTrace.FORMAT_STACKTRACE==o){bn=qx.dev.StackTrace.FORMAT_STACKTRACE(bn);{}
;}
;return bn;}
...
(function(){var c="-",d="",e="qx.core.ObjectRegistry",f="Disposed ",g="$$hash",h="-0",j=" objects",k="Could not dispose object ",m=": ";qx.Bootstrap.define(e,{statics:{inShutDown:false,__bV:{},__bW:0,__bX:[],__bY:d,__ca:{},register:function(n){var q=this.__bV;if(!q){return;}
;var p=n.$$hash;if(p==null){var o=this.__bX;if(o.length>0&&true){p=o.pop();}
else {p=(this.__bW++ )+this.__bY;}
;n.$$hash=p;{}
;}
;{}
;q[p]=n;}
,unregister:function(r){var s=r.$$hash;if(s==null){return;}
;var t=this.__bV;if(t&&t[s]){delete t[s];this.__bX.push(s);}
;try{delete r.$$hash;}
...
;var w=this.__bX;if(w.length>0){x=w.pop();}
else {x=(this.__bW++ )+this.__bY;}
;return v.$$hash=x;}
,clearHashCode:function(y){{}
;var z=y.$$hash;if(z!=null){this.__bX.push(z);try{delete y.$$hash;}
catch(A){if(y.removeAttribute){y.removeAttribute(g);}
;}
;}
;}
,fromHashCode:function(B){return this.__bV[B]||null;}
,shutdown:function(){this.inShutDown=true;var D=this.__bV;var F=[];for(var C in D){F.push(C);}
...
;}
;break;}
;qx.Bootstrap.debug(this,f+l+j);delete this.__bV;}
...
},defer:function(H){if(window&&window.top){var frames=window.top.frames;for(var i=0;i<frames.length;i++ ){if(frames[i]===window){H.__bY=c+(i+1);return;}
;}
;}
;H.__bY=h;}
});}
...
;{qx.bom.Event.addNativeListener(s,j,t);}
;}
...
;}
;qx.core.ObjectRegistry.unregister(this);this.__cM=null;if(qx.core.Environment.get(d)){var F=this.constructor;var J;var K=this.__K.$$store;var H=K.user;var I=K.theme;var D=K.inherit;var G=K.useinit;var E=K.init;while(F){J=F.$$properties;if(J){for(var name in J){if(J[name].dereference){this[H[name]]=this[I[name]]=this[D[name]]=this[G[name]]=this[E[name]]=undefined;}
;}
;}
;F=F.superclass;}
;}
...
(function(){var a='"',b="Class '",c="",d="Unsupported type!",e="change",f="qx.data.marshal.Json",g="Array",h="_validate",j="' could not be found.",k="]",l="[",m="set",n="_applyEventPropagation",o="qx.data.model.";qx.Class.define(f,{extend:qx.core.Object,implement:[qx.data.marshal.IMarshaler],construct:function(p){qx.core.Object.call(this);this.__tF=p;}
...
},members:{__tF:null,__tG:function(s){return Object.keys(s).sort().join(a);}
,toClass:function(t,u){this.__tH(t,u,null,0);}
,__tH:function(G,B,v,D){if(!qx.lang.Type.isObject(G)||!!G.$$isString||G instanceof qx.core.Object){if(G instanceof Array||qx.Bootstrap.getClass(G)==g){for(var i=0;i<G.length;i++ ){this.__tH(G[i],B,v+l+i+k,D+1);}
;}
;return;}
;var x=this.__tG(G);if(this.__tL(x,v,D)){return;}
;for(var F in G){this.__tH(G[F],B,F,D+1);}
;if(qx.Class.isDefined(o+x)){return;}
;if(this.__tF&&this.__tF.getModelClass&&this.__tF.getModelClass(x,G,v,D)!=null){return;}
;var H={};var y={__tI:this.__tI};for(var F in G){if(this.__tF&&this.__tF.getPropertyMapping){F=this.__tF.getPropertyMapping(F,x);}
;F=F.replace(/-|\.|\s+/g,c);{}
;H[F]={};H[F].nullable=true;H[F].event=e+qx.lang.String.firstUp(F);if(B){H[F].apply=n;}
;if(this.__tF&&this.__tF.getValidationRule){var A=this.__tF.getValidationRule(x,F);if(A){H[F].validate=h+F;y[h+F]=A;}
;}
;}
;if(this.__tF&&this.__tF.getModelSuperClass){var E=this.__tF.getModelSuperClass(x,v,D)||qx.core.Object;}
else {var E=qx.core.Object;}
;var z=[];if(this.__tF&&this.__tF.getModelMixins){var C=this.__tF.getModelMixins(x,v,D);if(!qx.lang.Type.isArray(C)){if(C!=null){z=[C];}
;}
else {z=C;}
;}
;if(B){z.push(qx.data.marshal.MEventBubbling);}
;var w={extend:E,include:z,properties:H,members:y,destruct:this.__tJ};qx.Class.define(o+x,w);}
,__tJ:function(){var I=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var J in I){this.__tI(this.get(I[J].name));}
;}
,__tI:function(K){if(!(K instanceof qx.core.Object)){return;}
;if(K.isDisposed()){return;}
;K.dispose();}
,__tK:function(L,P,Q,O){var R;if(this.__tF&&this.__tF.getModelClass){R=this.__tF.getModelClass(L,P,Q,O);}
;if(R!=null){return (new R());}
else {var N=o+L;var M=qx.Class.getByName(N);if(!M){throw new Error(b+N+j);}
;return (new M());}
;}
,__tL:function(S,V,T){var U=this.__tF;return U&&U.ignore&&U.ignore(S,V,T);}
,toModel:function(W){return this.__tM(W,null,0);}
,__tM:function(bj,Y,bf){var bd=qx.lang.Type.isObject(bj);var X=bj instanceof Array||qx.Bootstrap.getClass(bj)==g;if((!bd&&!X)||!!bj.$$isString||bj instanceof qx.core.Object){return bj;}
else if(this.__tL(this.__tG(bj),Y,bf)){return bj;}
else if(X){var bh=qx.data.Array;if(this.__tF&&this.__tF.getArrayClass){var be=this.__tF.getArrayClass(Y,bf);bh=be||bh;}
;var bk=new bh();bk.setAutoDisposeItems(true);for(var i=0;i<bj.length;i++ ){bk.push(this.__tM(bj[i],Y+l+i+k,bf+1));}
;return bk;}
else if(bd){var ba=this.__tG(bj);var bi=this.__tK(ba,bj,Y,bf);for(var bg in bj){var bb=bg;if(this.__tF&&this.__tF.getPropertyMapping){bb=this.__tF.getPropertyMapping(bg,ba);}
;var bl=bb.replace(/-|\.|\s+/g,c);{}
;bb=bl;var bc=m+qx.lang.String.firstUp(bb);if(bi[bc]){bi[bc](this.__tM(bj[bg],bg,bf+1));}
;}
;return bi;}
;throw new Error(d);}
...
;}
else if(typeof r==n){this.__me=new Array(r);}
else if(r instanceof Array){this.__me=qx.lang.Array.clone(r);}
else {this.__me=[];this.dispose();throw new Error(j);}
;for(var i=0;i<this.__me.length;i++ ){this._applyEventPropagation(this.__me[i],null,i);}
;this.__mf();{}
;}
,properties:{autoDisposeItems:{check:e,init:false}},events:{"change":q,"changeLength":q},members:{__me:null,concat:function(s){if(s){var t=this.__me.concat(s);}
else {var t=this.__me.concat();}
;return new qx.data.Array(t);}
,join:function(u){return this.__me.join(u);}
,pop:function(){var v=this.__me.pop();this.__mf();this._registerEventChaining(null,v,this.length-1);this.fireDataEvent(o,{value:[],name:this.length+f,old:[v],item:this});this.fireDataEvent(g,{start:this.length-1,end:this.length-1,type:l,removed:[v],added:[]},null);return v;}
...
;var x=this.__me.concat();this.__me.reverse();this.__mg(0,this.length);this.fireDataEvent(g,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(o,{value:this.__me,name:k+(this.__me.length-1),old:x,item:this});}
,shift:function(){if(this.length==0){return;}
;var y=this.__me.shift();this.__mf();this._registerEventChaining(null,y,this.length-1);this.__mg(0,this.length);this.fireDataEvent(o,{value:[],name:m,old:[y],item:this});this.fireDataEvent(g,{start:0,end:this.length-1,type:l,removed:[y],added:[]},null);return y;}
,slice:function(A,z){return new qx.data.Array(this.__me.slice(A,z));}
,splice:function(F,H,J){var N=this.__me.length;var I=this.__me.splice.apply(this.__me,arguments);if(this.__me.length!=N){this.__mf();}
...
else {var M=d;var E=F+Math.max(B.length,I.length)-1;}
...
;this.__mg(F+(arguments.length-2)-H,this.length);if(L||D){var G=[];for(var i=2;i<arguments.length;i++ ){G[i-2]=arguments[i];}
...
;var O=this.__me.concat();this.__me.sort.apply(this.__me,arguments);if(qx.lang.Array.equals(this.__me,O)===true){return;}
;this.__mg(0,this.length);this.fireDataEvent(g,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(o,{value:this.__me,name:k+(this.length-1),old:O,item:this});}
,unshift:function(Q){for(var i=arguments.length-1;i>=0;i-- ){this.__me.unshift(arguments[i]);this.__mf();this.__mg(0,this.length);this.fireDataEvent(o,{value:[this.__me[0]],name:m,old:[this.__me[1]],item:this});this.fireDataEvent(g,{start:0,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,toArray:function(){return this.__me;}
,getItem:function(R){return this.__me[R];}
,setItem:function(S,U){var T=this.__me[S];if(T===U){return;}
;this.__me[S]=U;this._registerEventChaining(U,T,S);if(this.length!=this.__me.length){this.__mf();}
...
,indexOf:function(V){return this.__me.indexOf(V);}
,lastIndexOf:function(W){return this.__me.lastIndexOf(W);}
,toString:function(){if(this.__me!=null){return this.__me.toString();}
;return f;}
,contains:function(X){return this.__me.indexOf(X)!==-1;}
...
;var bl=this.getLength();var bk=this.__me.concat();this.__me.length=0;this.__mf();this.fireDataEvent(o,{value:[],name:k+(bl-1),old:bk,item:this});this.fireDataEvent(g,{start:0,end:bl-1,type:l,removed:bk,added:[]},null);return bk;}
,append:function(bm){if(bm instanceof qx.data.Array){bm=bm.toArray();}
;{}
;var bn=this.__me.length;Array.prototype.push.apply(this.__me,bm);for(var i=0;i<bm.length;i++ ){this._registerEventChaining(bm[i],null,bn+i);}
;var bn=this.length;this.__mf();var name=bn==(this.length-1)?bn:bn+a+(this.length-1);this.fireDataEvent(o,{value:bm,name:name+f,old:[],item:this});this.fireDataEvent(g,{start:bn,end:this.length-1,type:b,added:bm,removed:[]},null);}
...
,forEach:function(bu,bv){for(var i=0;i<this.__me.length;i++ ){bu.call(bv,this.__me[i],i,this);}
;}
,filter:function(bw,self){return new qx.data.Array(this.__me.filter(bw,self));}
,map:function(bx,self){return new qx.data.Array(this.__me.map(bx,self));}
,some:function(by,self){return this.__me.some(by,self);}
,every:function(bz,self){return this.__me.every(bz,self);}
,reduce:function(bB,bA){return this.__me.reduce(bB,bA);}
,reduceRight:function(bD,bC){return this.__me.reduceRight(bD,bC);}
,__mf:function(){var bE=this.length;this.length=this.__me.length;this.fireDataEvent(p,this.length,bE);}
,__mg:function(bG,bF){for(var i=bG;i<bF;i++ ){this._registerEventChaining(this.__me[i],this.__me[i],i);}
;}
},destruct:function(){for(var i=0;i<this.__me.length;i++ ){var bH=this.__me[i];this._applyEventPropagation(null,bH,i);if(this.isAutoDisposeItems()&&bH&&bH instanceof qx.core.Object){bH.dispose();}
;}
;this.__me=null;}
});}
)();
