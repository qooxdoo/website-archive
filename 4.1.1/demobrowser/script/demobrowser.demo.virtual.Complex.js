...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Complex.817fb1cb6278.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,properties:{scroller:{check:i,nullable:true,init:null,apply:h},interval:{check:k,init:200,apply:b}},members:{__wl:null,__wm:null,__eg:null,__wn:null,__wo:null,setPrefetchX:function(o,r,p,q){this.__wl=[o,r,p,q];}
,setPrefetchY:function(v,u,t,s){this.__wm=[v,u,t,s];}
,_onInterval:function(){var w=this.__wl;if(w[1]&&w[3]){this.getScroller().getPane().prefetchX(w[0],w[1],w[2],w[3]);qx.ui.core.queue.Manager.flush();}
;var x=this.__wm;if(x[1]&&x[3]){this.getScroller().getPane().prefetchY(x[0],x[1],x[2],x[3]);qx.ui.core.queue.Manager.flush();}
;}
,_applyScroller:function(z,y){if(y){if(this.__wn){y.getChildControl(j).removeListenerById(this.__wn);}
;if(this.__wo){y.getChildControl(l).removeListenerById(this.__wo);}
;}
...
;this.__wn=z.getChildControl(j).addListener(g,this.__eg.restart,this.__eg);this.__wo=z.getChildControl(l).addListener(g,this.__eg.restart,this.__eg);}
else {this.__eg.stop();}
;}
,_applyInterval:function(B,A){this.__eg.setInterval(B);}
},destruct:function(){this.setScroller(null);this.__wl=this.__wm=null;this._disposeObjects(a);}
});}
)();
...
(function(){var a="icon/",b=" / ",c="",d="folder.png",e="places/",f="demobrowser.demo.virtual.DemoLayer",g="pointerover",h="network-server.png",i="row",k="user-trash.png",l="foobar!",m="changeValue",n="column",o="network-workgroup.png",p="qx.ui.basic.Atom",q="/",r="user-desktop.png";qx.Class.define(f,{extend:qx.ui.virtual.layer.WidgetCell,construct:function(){qx.ui.virtual.layer.WidgetCell.call(this,this);this._pool={atom:[],checkbox:[]};this.__mJ=[];}
,members:{_pool:null,__mJ:null,getCellData:function(t,s){if(!this.__mJ[t]){this.__mJ[t]=[];}
;if(!this.__mJ[t][s]){this.__mJ[t][s]={label:this.__PW(),icon:this.__PX()};}
;return this.__mJ[t][s];}
,getCellWidget:function(w,u){var v;if(u%2==0){v=this._pool.atom.pop();if(!v){v=new qx.ui.basic.Atom();v.addListener(g,function(){var x=this.__PX();v.setIcon(x);this.__mJ[w][u].icon=x;}
,this);}
...
;}
,__PW:function(){var name=c;for(var j=0;j<10;j++ ){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);}
;return name;}
,__PX:function(){var z=a;var B=e;var A=[d,k,h,o,r];var C=Math.floor(Math.random()*4);return (z+16+q+B+A[C]);}
},destruct:function(){this._pool=this.__mJ=null;}
});}
)();
