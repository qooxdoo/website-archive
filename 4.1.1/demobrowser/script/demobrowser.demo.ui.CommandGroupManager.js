...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.CommandGroupManager.7cae84631f4b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="qx.ui.command.GroupManager",b="Group must be an instance of qx.ui.command.Group",c="qx.debug",d="Group was not added before. Please use ",e="'add()' method to add the group.",f="Given group is not an instance of qx.ui.command.Group",g="Group was not added before! You have to use 'addCommand()' method before activating!",h="Group is already added!";qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__vs=[];}
,members:{__vs:null,__Jb:null,add:function(j){if(qx.core.Environment.get(c)){this.assertInstance(j,qx.ui.command.Group,f);}
;if(qx.lang.Array.contains(this.__vs,j)){if(qx.core.Environment.get(c)){this.debug(h);}
;return false;}
;this.__vs.push(j);j.setActive(false);return true;}
...
;var l=this.__vs.indexOf(m);if(l===-1){if(qx.core.Environment.get(c)){this.debug(d+e);}
;}
;if(this.getActive()===m){this.__Jb=null;}
;this.__vs.splice(l,1);return m;}
...
;return false;}
;for(var i=0;i<this.__vs.length;i++ ){var n=this.__vs[i];if(n==o){n.setActive(true);this.__Jb=n;continue;}
;n.setActive(false);}
;return true;}
,getActive:function(){return this.__Jb;}
,block:function(){if(this.__Jb){this.__Jb.setActive(false);}
;}
,unblock:function(){if(this.__Jb){this.__Jb.setActive(true);}
;}
,_getGroup:function(q){var p=this.__vs.indexOf(q);if(p===-1){return null;}
;return this.__vs[p];}
},destruct:function(){this.__vs=this.__Jb=null;}
});}
)();
...
(function(){var a="__nY",b="bottom",c="button",d="qx.event.type.Data",f="lastTab",g="barRight",h="beforeChangeSelection",j="qx.debug",k="horizontal",m="changeSelection",n="bar",o="close-button",p="Index should be less than : ",q="tabview",r="vertical",s="page",t="Incompatible child for TabView: ",u="close",v="_applyBarPosition",w="barLeft",x="top",y="right",z="qx.ui.tabview.TabView",A="barTop",B="barBottom",C="pane",D="left",E="firstTab";qx.Class.define(z,{extend:qx.ui.core.Widget,implement:qx.ui.core.ISingleSelection,include:[qx.ui.core.MContentPadding],construct:function(G){qx.ui.core.Widget.call(this);this.__nX={top:A,right:g,bottom:B,left:w};this._createChildControl(n);this._createChildControl(C);var F=this.__nY=new qx.ui.form.RadioGroup;F.setWrap(false);F.addListener(m,this._onChangeSelection,this);if(G!=null){this.setBarPosition(G);}
else {this.initBarPosition();}
;}
,events:{"changeSelection":d},properties:{appearance:{refine:true,init:q},barPosition:{check:[D,y,x,b],init:x,apply:v}},members:{__nY:null,_createChildControlImpl:function(J,I){var H;switch(J){case n:H=new qx.ui.container.SlideBar();H.setZIndex(10);this._add(H);break;case C:H=new qx.ui.container.Stack;H.setZIndex(5);this._add(H,{flex:1});break;};return H||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,J);}
...
;}
;var K=O.getButton();var L=this.getChildControl(n);var N=this.getChildControl(C);O.exclude();L.add(K);N.add(O);this.__nY.add(K);O.addState(this.__nX[this.getBarPosition()]);O.addState(f);var M=this.getChildren();if(M[0]==O){O.addState(E);}
...
;var R=U.getButton();var Q=this.getChildControl(n);var T=this.getChildControl(C);U.exclude();Q.addAt(R,P);T.addAt(U,P);this.__nY.add(R);U.addState(this.__nX[this.getBarPosition()]);S=this.getChildren();if(P==S.length-1){U.addState(f);}
...
;}
;W.remove(X);ba.remove(Y);this.__nY.remove(X);Y.removeState(this.__nX[this.getBarPosition()]);if(Y.hasState(E)){Y.removeState(E);if(bb[0]){bb[0].addState(E);}
;}
...
,__nX:null,_applyBarPosition:function(bg,bi){var bd=this.getChildControl(n);var bj=this.getChildControl(C);var bl=bg==D||bg==y;var bm=bg==y||bg==b;var bk=bl?qx.ui.layout.HBox:qx.ui.layout.VBox;var bn=this._getLayout();if(bn&&bn instanceof bk){}
else {this._setLayout(bn=new bk);}
;bn.setReversed(bm);bd.setOrientation(bl?r:k);var bh=this.getChildren();if(bi){var bf=this.__nX[bi];bd.removeState(bf);bj.removeState(bf);for(var i=0,l=bh.length;i<l;i++ ){bh[i].removeState(bf);}
;}
;if(bg){var be=this.__nX[bg];bd.addState(be);bj.addState(be);for(var i=0,l=bh.length;i<l;i++ ){bh[i].addState(be);}
;}
;}
,getSelection:function(){var bo=this.__nY.getSelection();var bp=[];for(var i=0;i<bo.length;i++ ){bp.push(bo[i].getUserData(s));}
;return bp;}
,setSelection:function(br){var bq=[];for(var i=0;i<br.length;i++ ){bq.push(br[i].getChildControl(c));}
;this.__nY.setSelection(bq);}
,resetSelection:function(){this.__nY.resetSelection();}
,isSelected:function(bs){var bt=bs.getChildControl(c);return this.__nY.isSelected(bt);}
,isSelectionEmpty:function(){return this.__nY.isSelectionEmpty();}
,getSelectables:function(bv){var bu=this.__nY.getSelectables(bv);var bw=[];for(var i=0;i<bu.length;i++ ){bw.push(bu[i].getUserData(s));}
;return bw;}
...
},destruct:function(){this._disposeObjects(a);this.__nX=null;}
});}
)();
