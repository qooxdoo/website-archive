...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Spinner.8638177d7e16.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="-",b="Boolean",c='{0,1}[0-9]{3}){0,}){0,1}',d=")?$",e="Number string '",f="' does not match the number format",g="Wrong argument type. String is expected.",h="0",i="^(",j="NaN",k=')?([-+]){0,1}',l='(',m="-Infinity",n="String",o="",p='\\d+){0,1}(',q='([0-9]{1,3}(?:',r="qx.util.format.NumberFormat",s="g",t="Wrong number of arguments.",u="Number",v="changeNumberFormat",w=".",x="Infinity";qx.Class.define(r,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(y){qx.core.Object.call(this);if(arguments.length>0){if(arguments.length===1){if(qx.lang.Type.isString(y)){this.__lk=y;}
else {throw new Error(g);}
;}
else {throw new Error(t);}
;}
;}
,properties:{minimumIntegerDigits:{check:u,init:0},maximumIntegerDigits:{check:u,nullable:true},minimumFractionDigits:{check:u,init:0},maximumFractionDigits:{check:u,nullable:true},groupingUsed:{check:b,init:true},prefix:{check:n,init:o,event:v},postfix:{check:n,init:o,event:v}},members:{__lk:null,format:function(E){if(isNaN(E)){return j;}
;switch(E){case Infinity:return x;case -Infinity:return m;};var C=(E<0);if(C){E=-E;}
;if(this.getMaximumFractionDigits()!=null){var K=Math.pow(10,this.getMaximumFractionDigits());E=Math.round(E*K)/K;}
;var I=String(Math.floor(E)).length;var z=o+E;var G=z.substring(0,I);while(G.length<this.getMinimumIntegerDigits()){G=h+G;}
...
;if(this.getGroupingUsed()){var B=G;G=o;var J;for(J=B.length;J>3;J-=3){G=o+qx.locale.Number.getGroupSeparator(this.__lk)+B.substring(J-3,J)+G;}
;G=B.substring(0,J)+G;}
;var D=this.getPrefix()?this.getPrefix():o;var A=this.getPostfix()?this.getPostfix():o;var H=D+(C?a:o)+G;if(F.length>0){H+=o+qx.locale.Number.getDecimalSeparator(this.__lk)+F;}
;H+=A;return H;}
,parse:function(T){var P=qx.lang.String.escapeRegexpChars(qx.locale.Number.getGroupSeparator(this.__lk)+o);var N=qx.lang.String.escapeRegexpChars(qx.locale.Number.getDecimalSeparator(this.__lk)+o);var L=new RegExp(i+qx.lang.String.escapeRegexpChars(this.getPrefix())+k+q+P+c+l+N+p+qx.lang.String.escapeRegexpChars(this.getPostfix())+d);var O=L.exec(T);if(O==null){throw new Error(e+T+f);}
;var Q=(O[2]==a);var S=O[3]||h;var R=O[4];S=S.replace(new RegExp(P,s),o);var M=(Q?a:o)+S;if(R!=null&&R.length!=0){R=R.replace(new RegExp(N),o);M+=w+R;}
;return parseFloat(M);}
}});}
)();
