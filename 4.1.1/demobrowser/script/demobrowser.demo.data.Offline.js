...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Offline.9d3efebfb1b2.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
},construct:function(g){this.__pE=g;}
,members:{__pE:null,getStorage:function(){return window[this.__pE+c];}
,getLength:function(){return this.getStorage(this.__pE).length;}
,setItem:function(h,j){j=qx.lang.Json.stringify(j);try{this.getStorage(this.__pE).setItem(h,j);}
catch(e){throw new Error(f);}
;}
,getItem:function(l){var k=this.getStorage(this.__pE).getItem(l);if(qx.lang.Type.isString(k)){k=qx.lang.Json.parse(k);}
else if(k&&k.value&&qx.lang.Type.isString(k.value)){k=qx.lang.Json.parse(k.value);}
;return k;}
,removeItem:function(m){this.getStorage(this.__pE).removeItem(m);}
,clear:function(){var n=this.getStorage(this.__pE);if(!n.clear){for(var i=n.length-1;i>=0;i-- ){n.removeItem(n.key(i));}
;}
else {n.clear();}
;}
,getKey:function(o){return this.getStorage(this.__pE).key(o);}
