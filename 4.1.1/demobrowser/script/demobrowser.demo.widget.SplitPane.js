...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.SplitPane.181305931a35.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,properties:{appearance:{refine:true,init:o},offset:{check:d,init:6,apply:n},orientation:{init:u,check:[u,k],apply:m}},members:{__sS:null,__sT:false,__sU:null,__sV:null,__nd:null,__sW:null,__sX:null,__gW:null,__lr:null,_createChildControlImpl:function(H,G){var F;switch(H){case a:F=new qx.ui.splitpane.Slider(this);F.exclude();this._add(F,{type:H});break;case b:F=new qx.ui.splitpane.Splitter(this);this._add(F,{type:H});F.addListener(g,this.__sY,this);break;};return F||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,H);}
,__sY:function(e){this.__tb(e.getData());}
,__ta:function(K){this.__lr=new qx.ui.splitpane.Blocker(K);this.getContentElement().add(this.__lr);var I=this.getChildControl(b);var J=I.getWidth();if(!J){I.addListenerOnce(w,function(){this.__tb();}
,this);}
...
,_applyOrientation:function(Q,N){var R=this.getChildControl(a);var M=this.getChildControl(b);this.__nd=Q===u;if(!this.__lr){this.__ta(Q);}
;this.__lr.setOrientation(Q);var P=this._getLayout();if(P){P.dispose();}
;var O=Q===k?new qx.ui.splitpane.VLayout:new qx.ui.splitpane.HLayout;this._setLayout(O);M.removeState(N);M.addState(Q);M.getChildControl(t).removeState(N);M.getChildControl(t).addState(Q);R.removeState(N);R.addState(Q);qx.ui.core.queue.Manager.flush();this.__tb();}
,_applyOffset:function(T,S){this.__tb();}
,__tb:function(V){var U=this.getChildControl(b);var ba=this.getOffset();var bb=U.getBounds();var Y=U.getContentElement().getDomElement();if(!Y){return;}
;if(this.__nd){var X=null;if(V){X=V.width;}
...
,_onPointerDown:function(e){if(!e.isLeftPressed()){return;}
;var bg=this.getChildControl(b);var bj=bg.getContentLocation();var bh=this.getContentLocation();this.__sS=this.__nd?e.getDocumentLeft()-bj.left+bh.left:e.getDocumentTop()-bj.top+bh.top;var bi=this.getChildControl(a);var bk=bg.getBounds();bi.setUserBounds(bk.left,bk.top,bk.width,bk.height);bi.setZIndex(bg.getZIndex()+1);bi.show();this.__sT=true;this.__lr.capture();e.stop();}
,_onPointerMove:function(e){this._setLastPointerPosition(e.getDocumentLeft(),e.getDocumentTop());if(this.__sT){this.__tc();var bl=this.getChildControl(a);var bm=this.__sW;if(this.__nd){bl.setDomLeft(bm);this.__lr.setStyle(A,(bm-this.getOffset())+c);}
...
,_onPointerUp:function(e){if(!this.__sT){return;}
;this._finalizeSizes();var bn=this.getChildControl(a);bn.exclude();this.__sT=false;this.releaseCapture();e.stop();}
,_finalizeSizes:function(){var br=this.__sW;var bo=this.__sX;if(br==null){return;}
...
else {if(this.__nd){bs.setWidth(br);bq.setWidth(bo);}
else {bs.setHeight(br);bq.setHeight(bo);}
;}
;}
,__tc:function(){if(this.__nd){var bx=v,bE=l,by=C,bC=this.__sU;}
else {var bx=s,bE=f,by=h,bC=this.__sV;}
;var bD=this._getChildren();var bv=bD[2].getSizeHint();var bA=bD[3].getSizeHint();var bB=bD[2].getBounds()[bE]+bD[3].getBounds()[bE];var bz=bC-this.__sS;var bw=bB-bz;if(bz<bv[bx]){bw-=bv[bx]-bz;bz=bv[bx];}
else if(bw<bA[bx]){bz-=bA[bx]-bw;bw=bA[bx];}
;if(bz>bv[by]){bw+=bz-bv[by];bz=bv[by];}
else if(bw>bA[by]){bz+=bw-bA[by];bw=bA[by];}
;this.__sW=bz;this.__sX=bw;}
,_isActiveDragSession:function(){return this.__sT;}
,_setLastPointerPosition:function(x,y){this.__sU=x;this.__sV=y;}
...
,addSpacer:function(){if(!this.__nJ){this.__nJ=new qx.ui.core.Spacer();}
else {this._remove(this.__nJ);}
;this._add(this.__nJ);}
,addOpenButton:function(){this._add(this.getChildControl(l));}
,_onChangeOpen:function(e){if(this.isOpenable()){this.setOpen(e.getData());}
;}
,addIcon:function(){var L=this.getChildControl(s);if(this.__nI){this._remove(L);}
;this._add(L);this.__nI=true;}
,addLabel:function(M){var N=this.getChildControl(D);if(this.__nH){this._remove(N);}
;if(M){this.setLabel(M);}
else {N.setValue(this.getLabel());}
;this._add(N);this.__nH=true;}
,_applyIcon:function(P,O){if(!this.__nL()){this.__nM(P);}
else if(!this.isOpen()){this.__nM(P);}
;}
,_applyIconOpened:function(R,Q){if(this.isOpen()){if(this.__nK()&&this.__nL()){this.__nM(R);}
else if(!this.__nK()&&this.__nL()){this.__nM(R);}
;}
;}
...
;var X;if(W){X=this.__nL()?this.getIconOpened():null;}
else {X=this.getIcon();}
;if(X){this.__nM(X);}
;W?this.addState(t):this.removeState(t);}
,__nK:function(){return qx.util.PropertyUtil.getUserValue(this,s);}
,__nL:function(){return qx.util.PropertyUtil.getUserValue(this,u);}
,__nM:function(Y){var ba=this.getChildControl(s,true);if(ba){ba.setSource(Y);}
;}
...
;}
;if(this.__nJ){this.__nJ.setWidth((this.getLevel()+1)*this.getIndent()-bf);}
;}
...
(function(){var a="open",b="abstract",c="__nR",d="qx.debug",e="'. It is not a child of this tree item.",f="addItem",g="visible",h="excluded",j="__gW",k="removeItem",m="Invalid child index: ",n="Cannot remove treeitem '",o="qx.ui.tree.core.AbstractTreeItem";qx.Class.define(o,{extend:qx.ui.tree.core.AbstractItem,type:b,construct:function(p){qx.ui.tree.core.AbstractItem.call(this,p);this.__gW=[];}
,properties:{parent:{check:o,nullable:true}},members:{__gW:null,__nR:null,getTree:function(){var r=this;while(r.getParent()){r=r.getParent();}
...
;}
;}
,getChildrenContainer:function(){if(!this.__nR){this.__nR=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?g:h});}
;return this.__nR;}
,hasChildrenContainer:function(){return this.__nR;}
...
;this.__gW.push(Q);if(!N){this.__nS();}
;if(P){Q.recursiveAddToWidgetQueue();P.fireNonBubblingEvent(f,qx.event.type.Data,[Q]);}
;}
;if(P){qx.ui.core.queue.Widget.add(this);}
;}
,addAt:function(X,T){if(qx.core.Environment.get(d)){this.assert(T<=this.__gW.length&&T>=0,m+T);}
...
;qx.lang.Array.insertAt(this.__gW,X,T);if(!V){this.__nS();}
;if(this.getTree()){X.recursiveAddToWidgetQueue();qx.ui.core.queue.Widget.add(this);}
;}
,addBefore:function(Y,bb){if(qx.core.Environment.get(d)){this.assert(this.__gW.indexOf(bb)>=0);}
;var ba=Y.getParent();if(ba){ba.remove(Y);}
;this.addAt(Y,this.__gW.indexOf(bb));}
,addAfter:function(bd,bc){if(qx.core.Environment.get(d)){this.assert(this.__gW.indexOf(bc)>=0);}
;var be=bd.getParent();if(be){be.remove(bd);}
;this.addAt(bd,this.__gW.indexOf(bc)+1);}
,addAtBegin:function(bf){this.addAt(bf,0);}
,remove:function(bk){for(var i=0,l=arguments.length;i<l;i++ ){var bj=arguments[i];if(this.__gW.indexOf(bj)==-1){this.warn(n+bj+e);return;}
...
;return bn;}
},destruct:function(){this._disposeArray(j);this._disposeObjects(c);}
});}
)();
...
(function(){var a="Enter",b="_applyOpenMode",c="excluded",d="Space",f="Left",g="Boolean",h="changeOpenMode",j="_applyRootOpenClose",k="visible",l="changeSelection",m="qx.ui.tree.Tree",n="qx.event.type.Data",o="qx.ui.tree.core.AbstractTreeItem",p="tree",q="dbltap",r="_applyHideRoot",s="changeRoot",t="_applyRoot",u="keypress",v="none",w="__nU",x="pane",y="tap",z="Right";qx.Class.define(m,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IModelSelection,qx.ui.form.IForm],include:[qx.ui.core.MMultiSelectionHandling,qx.ui.core.MContentPadding,qx.ui.form.MModelSelection,qx.ui.form.MForm],construct:function(){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__nU=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({allowShrinkY:false,allowGrowX:true});this.getChildControl(x).add(this.__nU);this.initOpenMode();this.initRootOpenClose();this.addListener(l,this._onChangeSelection,this);this.addListener(u,this._onKeyPress,this);}
,events:{addItem:n,removeItem:n},properties:{openMode:{check:[y,q,v],init:q,apply:b,event:h,themeable:true},root:{check:o,init:null,nullable:true,event:s,apply:t},hideRoot:{check:g,init:false,apply:r},rootOpenClose:{check:g,init:false,apply:j},appearance:{refine:true,init:p},focusable:{refine:true,init:true}},members:{__nU:null,SELECTION_MANAGER:qx.ui.tree.selection.SelectionManager,getChildrenContainer:function(){return this.__nU;}
...
,_getContentPaddingTarget:function(){return this.__nU;}
