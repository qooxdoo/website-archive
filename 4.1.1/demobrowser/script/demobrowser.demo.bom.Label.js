...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Label.00b03de9f673.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="This is <b>bold</b> text",b="Preferred Label2 Size (HeightForWidth): ",c="Preferred Label2 Size: ",d="foo bar",e="hello <b>world</b>",f="80px",g="foo <i>bar</i>",h="Runtime to measure 500 html blocks: ",j="ms",k="hello world",l="demobrowser.demo.bom.Label",m="Quite a long label text",n="Preferred Label1 Size: ",o="Runtime to measure 500 text blocks: ",p="1px solid red",q="x";qx.Class.define(l,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var y=qx.bom.Label.create(m);y.style.border=p;y.style.width=f;document.body.appendChild(y);var u=qx.bom.Label.getTextSize(m);this.debug(n+u.width+q+u.height);var t=qx.bom.Label.create(a,true);t.style.border=p;t.style.width=f;document.body.appendChild(t);var u=qx.bom.Label.getHtmlSize(a);this.debug(c+u.width+q+u.height);var u=qx.bom.Label.getHtmlSize(a,null,80);this.debug(b+u.width+q+u.height);var z=k;var v=d;var w=qx.bom.Label;var s=new Date;for(var i=0;i<500;i++ ){w.getTextSize(i%2?z:v);}
;var stop=new Date;this.debug(o+(stop-s)+j);var r=e;var x=g;var w=qx.bom.Label;var s=new Date;for(var i=0;i<500;i++ ){w.getHtmlSize(i%2?r:x);}
;var stop=new Date;this.debug(h+(stop-s)+j);}
}});}
)();
