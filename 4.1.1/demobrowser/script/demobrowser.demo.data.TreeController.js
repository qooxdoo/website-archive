...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.TreeController.b7f02d4cfce3.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;if(this.getModel()!=null){this.__Jk();}
;this._addChangeTargetListener(I,H);}
,_applyModel:function(K,J){this.__Jk();}
,_applyChildPath:function(M,L){this.__Jh=L;this.__Jk();this.__Jh=null;}
,_applyIconPath:function(O,N){this.__yN();}
,_applyLabelPath:function(Q,P){this.__Jk();}
,__Ji:function(R){var T=R.getTarget();var S=this.__Jg[T.toHashCode()].treeNode;var U=this.__Jg[T.toHashCode()].modelNode;this.__Jl(S,U);this._updateSelection();}
,__Jj:function(e){var X=e.getData();var bc=e.getOldData();var V=this.__Jg[bc.toHashCode()];bc.removeListenerById(V.changeListenerId);delete this.__Jg[bc.toHashCode()];V.modelNode.removeListenerById(V.changeChildernListenerId);var bd=V.modelNode;var bb=qx.Class.getPropertyDefinition(V.modelNode.constructor,this.getChildPath());var Y=bb.event;var ba=bd.addListener(Y,this.__Jj,this);var W=V.treeNode;this.__Jg[X.toHashCode()]={modelNode:bd,treeNode:W,changeListenerId:V.changeListenerId,changeChildernListenerId:ba};this.__Jl(W,bd);this._updateSelection();}
...
;return be;}
,__Jk:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__CU();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.__xE(this.getModel(),bg);this.__Jl(bg,this.getModel());this.getTarget().setRoot(bg);}
;}
,__Jl:function(bn,br){if(br[w+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bj=br[w+qx.lang.String.firstUp(this.getChildPath())]();if(this.__Jg[bj.toHashCode()]==undefined){var bl=bj.addListener(g,this.__Ji,this);var bp=qx.Class.getPropertyDefinition(br.constructor,this.getChildPath());var bm=bp.event;var bq=br.addListener(bm,this.__Jj,this);this.__Jg[bj.toHashCode()]={modelNode:br,treeNode:bn,changeListenerId:bl,changeChildernListenerId:bq};}
...
;}
else {var bo=this._createItem();bo.setModel(bj.getItem(i));bn.addAt(bo,i);this.__xE(bj.getItem(i),bo);this.__Jl(bo,bj.getItem(i));}
;}
;}
;for(var i=bn.getChildren().length-1;i>=bj.length;i-- ){var bk=bn.getChildren()[i];this.__Jn(bk,bn);}
;}
,__CU:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__Jm(bv);var bu=bv.getModel();if(bu){this.__mV(bu);}
;bv.destroy();this.__Jg={};}
;}
,__Jm:function(bw){var bx=bw.getChildren()||[];for(var i=bx.length-1;i>=0;i-- ){if(bx[i].getChildren().length>0){this.__Jm(bx[i]);}
;this.__Jn(bx[i],bw);}
;}
,__Jn:function(bz,bC){var bD=bz.getModel();var by=this.__Jh||this.getChildPath();var bA=w+qx.lang.String.firstUp(by);if(bD[bA]!=undefined){var bE=bD[bA]();var bB=this.__Jg[bE.toHashCode()];bE.removeListenerById(bB.changeListenerId);bD.removeListenerById(bB.changeChildernListenerId);this.__Jm(bz);delete this.__Jg[bE.toHashCode()];}
;this.__mV(bD);bC.remove(bz);bz.destroy();}
...
;for(var bX in this.__bb[bW]){var bV=this.__bb[bW][bX].treeNode;var bY=qx.core.ObjectRegistry.fromHashCode(bX);this.__mV(bY);this.__xE(bY,bV);}
;}
...
;}
,__mV:function(cg){for(var i=0;i<this.__yC.length;i++ ){var ce=this.__yC[i];var cf=this.__bb[ce][cg.toHashCode()];if(cf!=null){if(cf.id){cg.removeBinding(cf.id);cf.id=null;}
...
;this._startSelectionModification();this.__CU();this.__Jk();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cn,cm){if(cn!=null&&cn.bindItem!=null){if(cm!=null&&cm.bindItem!=null&&cn.bindItem==cm.bindItem){return;}
;this.__Jk();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__bb=this.__Jg=this.__yC=null;}
});}
)();
...
(function(){var a="changeNames",b="changeName",c="Validation Error: ",d="- ",e="",f="changeChild",g="qx.data.Array",h="changeColor",j="Marge",k="AFFE",l="is no number (parseFloat says so!).",m="Homer",n="\n",o="__Pb",p="demobrowser.demo.data.model.Node",q="String",r="changeName2",s="changeChildren";qx.Class.define(p,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.setChildren(new qx.data.Array());}
,properties:{children:{check:g,event:s},child:{check:p,event:f,nullable:true},names:{check:g,event:a,init:new qx.data.Array(m,j)},name:{check:q,init:k,event:b},name2:{check:q,event:r},number:{init:10,validate:o},color:{event:h,nullable:true}},members:{__Pb:function(t){if(!isNaN(parseFloat(t))){if(/^\d+$/.test(t)){return;}
;}
