...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.Finder","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Finder.119cd5325877.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
;return cn;}
,assert:function(cq,cp){cq==true||this.__bM(cp||Q,v);}
,fail:function(cr,cs){var ct=cs?Q:bS;this.__bM(cr||Q,ct);}
,assertTrue:function(cv,cu){(cv===true)||this.__bM(cu||Q,m,cv,x);}
,assertFalse:function(cx,cw){(cx===false)||this.__bM(cw||Q,W,cx,x);}
,assertEquals:function(cy,cz,cA){cy==cz||this.__bM(cA||Q,D,cy,z,cz,O);}
,assertNotEquals:function(cB,cC,cD){cB!=cC||this.__bM(cD||Q,D,cB,h,cC,O);}
,assertIdentical:function(cE,cF,cG){cE===cF||this.__bM(cG||Q,D,cE,T,cF,O);}
,assertNotIdentical:function(cH,cI,cJ){cH!==cI||this.__bM(cJ||Q,D,cH,bH,cI,O);}
,assertNotUndefined:function(cL,cK){cL!==undefined||this.__bM(cK||Q,J);}
,assertUndefined:function(cN,cM){cN===undefined||this.__bM(cM||Q,p,cN,I);}
,assertNotNull:function(cP,cO){cP!==null||this.__bM(cO||Q,bN);}
,assertNull:function(cR,cQ){cR===null||this.__bM(cQ||Q,t,cR,I);}
,assertJsonEquals:function(cS,cT,cU){this.assertEquals(qx.lang.Json.stringify(cS),qx.lang.Json.stringify(cT),cU);}
,assertMatch:function(cX,cW,cV){this.assertString(cX);this.assert(qx.lang.Type.isRegExp(cW)||qx.lang.Type.isString(cW),cf);cX.search(cW)>=0||this.__bM(cV||Q,E,cX,bG,cW.toString(),O);}
,assertArgumentsCount:function(db,dc,dd,cY){var da=db.length;(da>=dc&&da<=dd)||this.__bM(cY||Q,cd,dc,j,dd,bP,da,bK);}
...
;}
;df===true||this.__bM(dj||Q,G,event,N);}
,assertEventNotFired:function(dn,event,dr,ds){var dp=false;var dq=function(e){dp=true;}
;var dt=dn.addListener(event,dq,dn);dr.call();dp===false||this.__bM(ds||Q,G,event,f);dn.removeListenerById(dt);}
,assertException:function(dx,dw,dv,du){var dw=dw||Error;var dy;try{this.__bL=false;dx();}
catch(dz){dy=dz;}
finally{this.__bL=true;}
;if(dy==null){this.__bM(du||Q,o);}
;dy instanceof dw||this.__bM(du||Q,M,dw,a,dy);if(dv){this.assertMatch(dy.toString(),dv,du);}
;}
,assertInArray:function(dC,dB,dA){dB.indexOf(dC)!==-1||this.__bM(dA||Q,bU,dC,U,dB,x);}
...
;}
;}
,assertKeyInMap:function(dI,dH,dG){dH[dI]!==undefined||this.__bM(dG||Q,bU,dI,C,dH,x);}
,assertFunction:function(dK,dJ){qx.lang.Type.isFunction(dK)||this.__bM(dJ||Q,bW,dK,I);}
,assertString:function(dM,dL){qx.lang.Type.isString(dM)||this.__bM(dL||Q,F,dM,I);}
,assertBoolean:function(dO,dN){qx.lang.Type.isBoolean(dO)||this.__bM(dN||Q,bM,dO,I);}
,assertNumber:function(dQ,dP){(qx.lang.Type.isNumber(dQ)&&isFinite(dQ))||this.__bM(dP||Q,V,dQ,I);}
,assertPositiveNumber:function(dS,dR){(qx.lang.Type.isNumber(dS)&&isFinite(dS)&&dS>=0)||this.__bM(dR||Q,bY,dS,I);}
,assertInteger:function(dU,dT){(qx.lang.Type.isNumber(dU)&&isFinite(dU)&&dU%1===0)||this.__bM(dT||Q,cb,dU,I);}
,assertPositiveInteger:function(dX,dV){var dW=(qx.lang.Type.isNumber(dX)&&isFinite(dX)&&dX%1===0&&dX>=0);dW||this.__bM(dV||Q,g,dX,I);}
,assertInRange:function(eb,ec,ea,dY){(eb>=ec&&eb<=ea)||this.__bM(dY||Q,qx.lang.String.format(bL,[eb,ec,ea]));}
,assertObject:function(ee,ed){var ef=ee!==null&&(qx.lang.Type.isObject(ee)||typeof ee===ce);ef||this.__bM(ed||Q,R,(ee),I);}
,assertArray:function(eh,eg){qx.lang.Type.isArray(eh)||this.__bM(eg||Q,c,eh,I);}
,assertMap:function(ej,ei){qx.lang.Type.isObject(ej)||this.__bM(ei||Q,n,ej,I);}
,assertRegExp:function(el,ek){qx.lang.Type.isRegExp(el)||this.__bM(ek||Q,r,el,I);}
,assertType:function(eo,en,em){this.assertString(en,u);typeof (eo)===en||this.__bM(em||Q,bE,en,S,eo,I);}
,assertInstance:function(er,es,ep){var eq=es.classname||es+Q;er instanceof es||this.__bM(ep||Q,ca,eq,S,er,I);}
,assertInterface:function(ev,eu,et){qx.Class&&qx.Class.implementsInterface(ev,eu)||this.__bM(et||Q,k,ev,s,eu,O);}
...
catch(eE){this.__bM(eB||Q,H,eC,d,ey.join(B),bT,ez,bQ);}
;var eD=ey[0]==eA[0]&&ey[1]==eA[1]&&ey[2]==eA[2];eD||this.__bM(eB||Q,H,ey,d,ey.join(B),bT,ez,d,eA.join(B),bO);}
,assertElement:function(eG,eF){!!(eG&&eG.nodeType===1)||this.__bM(eF||Q,q,eG,O);}
,assertQxObject:function(eI,eH){this.__bO(eI,b)||this.__bM(eH||Q,bJ,eI,I);}
,assertQxWidget:function(eK,eJ){this.__bO(eK,X)||this.__bM(eJ||Q,bD,eK,I);}
,__bO:function(eM,eL){if(!eM){return false;}
...
(function(){var a="qx.core.AssertionError";qx.Bootstrap.define(a,{extend:qx.type.BaseError,construct:function(b,c){qx.type.BaseError.call(this,b,c);this.__bR=qx.dev.StackTrace.getStackTrace();}
...
;U=/@(.+):(\d+)$/gm;while((V=U.exec(bg.stack))!=null){be=V[1];T=V[2];bd=this.__bS(be);W.push(bd+p+T);}
;if(W.length>0){return this.__bU(W);}
;U=/at (.*)/gm;var bh=/\((.*?)(:[^\/].*)\)/;var bc=/(.*?)(:[^\/].*)/;while((V=U.exec(bg.stack))!=null){var bb=bh.exec(V[1]);if(!bb){bb=bc.exec(V[1]);}
;if(bb){bd=this.__bS(bb[1]);W.push(bd+bb[2]);}
else {W.push(V[1]);}
;}
;}
else if(bf===t){var X=bg.stacktrace;if(!X){return W;}
;if(X.indexOf(j)>=0){X=X.split(j)[0];}
;U=/line\ (\d+?),\ column\ (\d+?)\ in\ (?:.*?)\ in\ (.*?):[^\/]/gm;while((V=U.exec(X))!=null){T=V[1];S=V[2];be=V[3];bd=this.__bS(be);W.push(bd+p+T+p+S);}
;if(W.length>0){return this.__bU(W);}
;U=/Line\ (\d+?)\ of\ linked\ script\ (.*?)$/gm;while((V=U.exec(X))!=null){T=V[1];be=V[2];bd=this.__bS(be);W.push(bd+p+T);}
;}
else if(bg.message&&bg.message.indexOf(m)>=0){var ba=bg.message.split(m)[1].trim();var Y=ba.split(f);for(var i=0;i<Y.length;i++ ){var R=Y[i].match(/\s*Line ([0-9]+) of.* (\S.*)/);if(R&&R.length>=2){T=R[1];bi=this.__bS(R[2]);W.push(bi+p+T);}
;}
;}
else if(bg.sourceURL&&bg.line){W.push(this.__bS(bg.sourceURL)+p+bg.line);}
;return this.__bU(W);}
,__bS:function(bk){if(typeof qx.dev.StackTrace.FILENAME_TO_CLASSNAME==r){var bj=qx.dev.StackTrace.FILENAME_TO_CLASSNAME(bk);if(qx.core.Environment.get(g)&&!qx.lang.Type.isString(bj)){throw new Error(k);}
;return bj;}
;return qx.dev.StackTrace.__bT(bk);}
,__bT:function(bn){var bo=h;var bl=bn.indexOf(bo);var bp=bn.indexOf(a);if(bp>=0){bn=bn.substring(0,bp);}
;var bm=(bl==-1)?bn:bn.substring(bl+bo.length).replace(/\//g,q).replace(/\.js$/,e);return bm;}
,__bU:function(bq){if(typeof qx.dev.StackTrace.FORMAT_STACKTRACE==r){bq=qx.dev.StackTrace.FORMAT_STACKTRACE(bq);if(qx.core.Environment.get(g)&&!qx.lang.Type.isArray(bq)){throw new Error(o);}
;}
;return bq;}
...
;}
;}
else if(qx.bom.client.Engine.__bv()){if(/Opera[\s\/]([0-9]+)\.([0-9])([0-9]*)/.test(B)){if(B.indexOf(g)!=-1){var E=B.match(/Version\/(\d+)\.(\d+)/);C=E[1]+x+E[2].charAt(0)+x+E[2].substring(1,E[2].length);}
else {C=RegExp.$1+x+RegExp.$2;if(RegExp.$3!=p){C+=x+RegExp.$3;}
;}
;}
;}
else if(qx.bom.client.Engine.__bw()){if(/AppleWebKit\/([^ ]+)/.test(B)){C=RegExp.$1;var D=RegExp(c).exec(C);if(D){C=C.slice(0,D.index);}
;}
;}
else if(qx.bom.client.Engine.__by()||qx.bom.client.Engine.__bx()){if(/rv\:([^\);]+)(\)|;)/.test(B)){C=RegExp.$1;}
;}
else {var z=window.qxFail;if(z&&typeof z===b){C=z().FULLVERSION;}
else {C=f;qx.Bootstrap.warn(o+B+s);}
;}
;return C;}
,getName:function(){var name;if(qx.bom.client.Engine.__bz()){name=t;}
else if(qx.bom.client.Engine.__bv()){name=q;}
else if(qx.bom.client.Engine.__bw()){name=v;}
...
,__bw:function(){return window.navigator.userAgent.indexOf(m)!=-1;}
,__bx:function(){return window.navigator.userAgent.indexOf(l)!=-1;}
,__by:function(){return window.navigator.mozApps&&window.navigator.product===k&&window.navigator.userAgent.indexOf(l)==-1&&window.navigator.userAgent.indexOf(n)==-1;}
,__bz:function(){if(window.navigator.cpuClass&&(/MSIE\s+([^\);]+)(\)|;)/.test(window.navigator.userAgent)||/Trident\/\d+?\.\d+?/.test(window.navigator.userAgent))){return true;}
;if(qx.bom.client.Engine.__bA()){return true;}
;return false;}
,__bA:function(){return window.navigator.userAgent.indexOf(r)>-1;}
...
(function(){var a="qx.globalErrorHandling",b="qx.event.GlobalError";qx.Bootstrap.define(b,{statics:{__bB:null,__bC:null,__bD:null,__bE:function(){if(qx.core&&qx.core.Environment){return qx.core.Environment.get(a);}
else {return !!qx.Bootstrap.getEnvironmentSetting(a);}
;}
,setErrorHandler:function(c,d){this.__bB=c||null;this.__bD=d||window;if(this.__bE()){if(c&&window.onerror){var e=qx.Bootstrap.bind(this.__bF,this);if(this.__bC==null){this.__bC=window.onerror;}
;var self=this;window.onerror=function(f,g,h){self.__bC(f,g,h);e(f,g,h);}
;}
;if(c&&!window.onerror){window.onerror=qx.Bootstrap.bind(this.__bF,this);}
;if(this.__bB==null){if(this.__bC!=null){window.onerror=this.__bC;this.__bC=null;}
else {window.onerror=null;}
;}
;}
;}
,__bF:function(i,j,k){if(this.__bB){this.handleError(new qx.core.WindowError(i,j,k));}
;}
,observeMethod:function(l){if(this.__bE()){var self=this;return function(){if(!self.__bB){return l.apply(this,arguments);}
...
;}
,handleError:function(n){if(this.__bB){this.__bB.call(this.__bD,n);}
;}
...
;this.__bG=c;this.__bH=e||a;this.__bI=f===undefined?-1:f;}
...
;this.__bG=a+(e&&e.message?e.message:e);var d=Error.call(this,this.__bG);if(d.stack){this.stack=d.stack;}
;if(d.stacktrace){this.stacktrace=d.stacktrace;}
;this.__bJ=c;this.__bK=e;}
...
},destruct:function(){this.__bK=null;this.__bJ=null;this.__bG=null;}
});}
)();
(function(){var c="-",d="qx.debug.dispose",e="",f="qx.core.ObjectRegistry",g="qx.debug",h="$$hash",j="-0",k="Invalid object: ",m="Could not dispose object ",n=" objects",o=": ",p="Disposed ";qx.Bootstrap.define(f,{statics:{inShutDown:false,__bV:{},__bW:0,__bX:[],__bY:e,__ca:{},register:function(q){var t=this.__bV;if(!t){return;}
;var s=q.$$hash;if(s==null){var r=this.__bX;if(r.length>0&&!qx.core.Environment.get(d)){s=r.pop();}
else {s=(this.__bW++ )+this.__bY;}
;q.$$hash=s;if(qx.core.Environment.get(d)){if(qx.dev&&qx.dev.Debug&&qx.dev.Debug.disposeProfilingActive){this.__ca[s]=qx.dev.StackTrace.getStackTrace();}
;}
;}
...
;var w=this.__bV;if(w&&w[v]){delete w[v];this.__bX.push(v);}
;try{delete u.$$hash;}
...
;var z=this.__bX;if(z.length>0){y=z.pop();}
else {y=(this.__bW++ )+this.__bY;}
;return A.$$hash=y;}
,clearHashCode:function(C){if(qx.core.Environment.get(g)){if(C==null){throw new Error(k+C);}
;}
;var B=C.$$hash;if(B!=null){this.__bX.push(B);try{delete C.$$hash;}
catch(D){if(C.removeAttribute){C.removeAttribute(h);}
;}
;}
;}
,fromHashCode:function(E){return this.__bV[E]||null;}
,shutdown:function(){this.inShutDown=true;var G=this.__bV;var I=[];for(var F in G){I.push(F);}
...
;}
;break;}
;qx.Bootstrap.debug(this,p+l+n);delete this.__bV;}
,getRegistry:function(){return this.__bV;}
,getNextHash:function(){return this.__bW;}
,getPostId:function(){return this.__bY;}
,getStackTraces:function(){return this.__ca;}
},defer:function(K){if(window&&window.top){var frames=window.top.frames;for(var i=0;i<frames.length;i++ ){if(frames[i]===window){K.__bY=c+(i+1);return;}
;}
;}
;K.__bY=j;}
});}
...
;}
;if(!(cG={}.hasOwnProperty)){cG=function(dp){var dq={},dr;if((dq.__cb=null,dq.__cb={"toString":1},dq).toString!=cr){cG=function(ds){var dt=this.__cb,du=ds in (this.__cb=null,this);this.__cb=dt;return du;}
;}
...
,__cW:null,__it:null,__iu:null,__iv:null,__iw:function(){if(!this.__cW&&this.__it&&qx.$$loader.scriptLoaded){if((qx.core.Environment.get(c)==b)){if(qx.event.Registration.hasListener(this._window,a)){this.__cW=true;qx.event.Registration.fireEvent(this._window,a);}
;}
...
,statics:{__bV:qx.$$resources||{},__et:{}},members:{has:function(r){return !!this.self(arguments).__bV[r];}
,getData:function(s){return this.self(arguments).__bV[s]||null;}
,getImageWidth:function(u){var t=this.self(arguments).__bV[u];return t?t[0]:null;}
,getImageHeight:function(w){var v=this.self(arguments).__bV[w];return v?v[1]:null;}
,getImageFormat:function(y){var x=this.self(arguments).__bV[y];return x?x[2]:null;}
,getCombinedFormat:function(D){var A=e;var C=this.self(arguments).__bV[D];var z=C&&C.length>4&&typeof (C[4])==i&&this.constructor.__bV[C[4]];if(z){var E=C[4];var B=this.constructor.__bV[E];A=B[2];}
;return A;}
,toUri:function(I){if(I==null){return I;}
;var F=this.self(arguments).__bV[I];if(!F){return I;}
...
,toDataUri:function(L){var K=this.constructor.__bV[L];var N=this.constructor.__bV[K[4]];var M;if(N){var J=N[4][L];M=k+J[j]+l+J[f]+q+J[h];}
else {M=this.toUri(L);}
;return M;}
...
(function(){var a="cursor:",b="engine.name",c="",d="mshtml",e="nw-resize",f="engine.version",g="nesw-resize",h="browser.documentmode",i=";",j="nwse-resize",k="qx.bom.element.Cursor",l="ne-resize",m="browser.quirksmode",n="cursor";qx.Bootstrap.define(k,{statics:{__fj:{},compile:function(o){return a+(this.__fj[o]||o)+i;}
,get:function(q,p){return qx.bom.element.Style.get(q,n,p,false);}
,set:function(s,r){s.style.cursor=this.__fj[r]||r;}
,reset:function(t){t.style.cursor=c;}
},defer:function(u){if(qx.core.Environment.get(b)==d&&((parseFloat(qx.core.Environment.get(f))<9||qx.core.Environment.get(h)<9)&&!qx.core.Environment.get(m))){u.__fj[g]=l;u.__fj[j]=e;}
;}
});}
)();
(function(){var a="This client does not support the boxSizing value",b="border-box",c="qx.bom.element.BoxSizing",d="css.boxsizing",e="",f="This client does not support dynamic modification of the boxSizing property.",g="qx.debug",h="boxSizing",i="content-box",j=":",k=";";qx.Bootstrap.define(c,{statics:{__fk:{tags:{button:true,select:true},types:{search:true,button:true,submit:true,reset:true,checkbox:true,radio:true}},__fl:function(m){var l=this.__fk;return l.tags[m.tagName.toLowerCase()]||l.types[m.type];}
...
;if(qx.bom.Document.isStandardMode(qx.dom.Node.getWindow(p))){if(!this.__fl(p)){return i;}
;}
;return b;}
...
(function(){var a="qx.ui.tooltip.ToolTip",b="Boolean",c="",d="mouse",f="pointerover",g="interval",h="__oA",i="__oz",j="_applyCurrent",k="widget",l="qx.ui.tooltip.Manager",m="pointermove",n="focusout",o="tooltip-error",p="__oC",q="singleton",r="pointerout";qx.Class.define(l,{type:q,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);qx.event.Registration.addListener(document.body,f,this.__oH,this,true);this.__oz=new qx.event.Timer();this.__oz.addListener(g,this.__oE,this);this.__oA=new qx.event.Timer();this.__oA.addListener(g,this.__oF,this);this.__oB={left:0,top:0};}
,properties:{current:{check:a,nullable:true,apply:j},showInvalidToolTips:{check:b,init:true},showToolTips:{check:b,init:true}},members:{__oB:null,__oA:null,__oz:null,__oC:null,__oD:null,getSharedTooltip:function(){if(!this.__oC){this.__oC=new qx.ui.tooltip.ToolTip().set({rich:true});}
;return this.__oC;}
,getSharedErrorTooltip:function(){if(!this.__oD){this.__oD=new qx.ui.tooltip.ToolTip().set({appearance:o,rich:true});this.__oD.setLabel(c);this.__oD.syncAppearance();}
;return this.__oD;}
,_applyCurrent:function(u,t){if(t&&qx.ui.core.Widget.contains(t,u)){return;}
;if(t){if(!t.isDisposed()){t.exclude();}
;this.__oz.stop();this.__oA.stop();}
;var v=qx.event.Registration;var s=document.body;if(u){this.__oz.startWith(u.getShowTimeout());v.addListener(s,r,this.__oI,this,true);v.addListener(s,n,this.__oJ,this,true);v.addListener(s,m,this.__oG,this,true);}
else {v.removeListener(s,r,this.__oI,this,true);v.removeListener(s,n,this.__oJ,this,true);v.removeListener(s,m,this.__oG,this,true);}
;}
,__oE:function(e){var w=this.getCurrent();if(w&&!w.isDisposed()){this.__oA.startWith(w.getHideTimeout());if(w.getPlaceMethod()==k){w.placeToWidget(w.getOpener());}
else {w.placeToPoint(this.__oB);}
;w.show();}
;this.__oz.stop();}
,__oF:function(e){var x=this.getCurrent();if(x&&!x.isDisposed()){x.exclude();}
;this.__oA.stop();this.resetCurrent();}
,__oG:function(e){var y=this.__oB;y.left=Math.round(e.getDocumentLeft());y.top=Math.round(e.getDocumentTop());}
,__oH:function(e){var z=qx.ui.core.Widget.getWidgetByElement(e.getTarget());this.__oG(e);this.showToolTip(z);}
...
,__oI:function(e){var F=qx.ui.core.Widget.getWidgetByElement(e.getTarget());if(!F){return;}
...
;}
,__oJ:function(e){var I=qx.ui.core.Widget.getWidgetByElement(e.getTarget());if(!I){return;}
;var J=this.getCurrent();if(J&&J==I.getToolTip()){this.setCurrent(null);}
;}
},destruct:function(){qx.event.Registration.removeListener(document.body,f,this.__oH,this,true);this._disposeObjects(i,h,p);this.__oB=null;}
});}
)();
...
;var s=qx.lang.Array.cast(p,o);for(var i=0;i<qxWeb.__cR.length;i++ ){qxWeb.__cR[i].call(s);}
;return s;}
...
;qxWeb[name]=w[name];}
;}
,$attachInit:function(x){this.__cR.push(x);}
...
;if(!A){A=[];}
else if(qx.Bootstrap.isString(A)){if(z instanceof qxWeb&&z.length!=0){z=z[0];}
;if(z instanceof qxWeb){A=[];}
else {A=qx.bom.Selector.query(A,z);}
;}
...
(function(){var a="mshtml",b="engine.name",c="*",d="Array",f="pointerout",g="pointerover",h="load",n="left",o="qx.module.Event",p="undefined",q="DOMContentLoaded",r="browser.documentmode",s="complete";qx.Bootstrap.define(o,{statics:{__cS:{},__cT:{on:{},off:{}},on:function(C,A,B,u){for(var i=0;i<this.length;i++ ){var t=this[i];var w=B||qxWeb(t);var v=qx.module.Event.__cT.on;var D=v[c]||[];if(v[C]){D=D.concat(v[C]);}
;for(var j=0,m=D.length;j<m;j++ ){D[j](t,C,A,B);}
;var z=function(E,event){var G=qx.module.Event.__cS;var F=G[c]||[];if(G[C]){F=F.concat(G[C]);}
...
;t.$$lastlistenerId=t.$$emitter.on(C,z,w);t.$$emitter.getEntryById(t.$$lastlistenerId).useCapture=!!u;if(!t.__cU){t.__cU={};}
;if(!t.__cU[C]){t.__cU[C]={};}
;t.__cU[C][t.$$lastlistenerId]=z;if(!B){if(!t.__cV){t.__cV={};}
;t.__cV[t.$$lastlistenerId]=w;}
;}
;return this;}
,off:function(R,J,O,I){var N=(J===null&&O===null);for(var j=0;j<this.length;j++ ){var H=this[j];if(!H.__cU){continue;}
;var T=[];if(R!==null){T.push(R);}
else {for(var L in H.__cU){T.push(L);}
;}
;for(var i=0,l=T.length;i<l;i++ ){for(var M in H.__cU[T[i]]){var Q=H.__cU[T[i]][M];if(N||Q==J||Q.original==J){var K=typeof H.__cV!==p&&H.__cV[M];var U;if(!O&&K){U=H.__cV[M];}
;H.$$emitter.off(T[i],Q,U||O);if(N||Q.original==J){qx.bom.Event.removeNativeListener(H,T[i],Q,I);}
;delete H.__cU[T[i]][M];if(K){delete H.__cV[M];}
;}
;}
;var P=qx.module.Event.__cT.off;var S=P[c]||[];if(P[R]){S=S.concat(P[R]);}
...
;}
;}
,__cW:false,ready:function(bu){if(document.readyState===s){window.setTimeout(bu,1);return;}
;var bv=function(){qx.module.Event.__cW=true;bu();}
...
else {var bx=function(){if(qx.module.Event.__cW){return;}
...
;var bR=qx.module.Event.__cS;for(var i=0,l=bT.length;i<l;i++ ){var bS=bT[i];if(qx.lang.Type.isFunction(bQ)){if(!bR[bS]){bR[bS]=[];}
;bR[bS].push(bQ);}
;}
;}
,$unregisterNormalization:function(bX,bU){if(!qx.lang.Type.isArray(bX)){bX=[bX];}
;var bV=qx.module.Event.__cS;for(var i=0,l=bX.length;i<l;i++ ){var bW=bX[i];if(bV[bW]){qx.lang.Array.remove(bV[bW],bU);}
;}
;}
,$getRegistry:function(){return qx.module.Event.__cS;}
,$registerEventHook:function(ce,cb,ca){if(!qx.lang.Type.isArray(ce)){ce=[ce];}
;var cc=qx.module.Event.__cT.on;for(var i=0,l=ce.length;i<l;i++ ){var cd=ce[i];if(qx.lang.Type.isFunction(cb)){if(!cc[cd]){cc[cd]=[];}
;cc[cd].push(cb);}
;}
;if(!ca){return;}
;var bY=qx.module.Event.__cT.off;for(var i=0,l=ce.length;i<l;i++ ){var cd=ce[i];if(qx.lang.Type.isFunction(ca)){if(!bY[cd]){bY[cd]=[];}
;bY[cd].push(ca);}
;}
;}
,$unregisterEventHook:function(ck,ch,cg){if(!qx.lang.Type.isArray(ck)){ck=[ck];}
;var ci=qx.module.Event.__cT.on;for(var i=0,l=ck.length;i<l;i++ ){var cj=ck[i];if(ci[cj]){qx.lang.Array.remove(ci[cj],ch);}
;}
;if(!cg){return;}
;var cf=qx.module.Event.__cT.off;for(var i=0,l=ck.length;i<l;i++ ){var cj=ck[i];if(cf[cj]){qx.lang.Array.remove(cf[cj],cg);}
;}
;}
,$getHookRegistry:function(){return qx.module.Event.__cT;}
...
(function(){var a="qx.event.Emitter",b="*";qx.Bootstrap.define(a,{extend:Object,statics:{__cX:[]},members:{__cU:null,__cY:null,on:function(name,c,d){var e=qx.event.Emitter.__cX.length;this.__da(name).push({listener:c,ctx:d,id:e,name:name});qx.event.Emitter.__cX.push({name:name,listener:c,ctx:d});return e;}
...
;}
,getListeners:function(){return this.__cU;}
,getEntryById:function(F){for(var name in this.__cU){var E=this.__cU[name];for(var i=0,j=E.length;i<j;i++ ){if(E[i].id===F){return E[i];}
;}
;}
;}
,__da:function(name){if(this.__cU==null){this.__cU={};}
;if(this.__cU[name]==null){this.__cU[name]=[];}
;return this.__cU[name];}
}});}
)();
...
(function(){var a="qx.util.DeferredCall",b="The context object '",c="qx.debug",d="'is already disposed.",e="' of the defered call '";qx.Class.define(a,{extend:qx.core.Object,construct:function(f,g){qx.core.Object.call(this);this.__bB=f;this.__bD=g||null;this.__gz=qx.util.DeferredCallManager.getInstance();}
,members:{__bB:null,__bD:null,__gz:null,cancel:function(){this.__gz.cancel(this);}
,schedule:function(){this.__gz.schedule(this);}
,call:function(){if(qx.core.Environment.get(c)){var h=this.__bD;if(h&&h.isDisposed&&h.isDisposed()){this.warn(b+h+e+this+d);}
;}
;this.__bD?this.__bB.apply(this.__bD):this.__bB();}
},destruct:function(){this.cancel();this.__bD=this.__bB=this.__gz=null;}
});}
)();
...
,_stopObserver:function(){qx.event.handler.GestureCore.GESTURE_EVENTS.forEach(function(M){qxWeb(this.__dc).off(M,this.checkAndFireGesture,this);}
...
;}
,gestureBegin:function(Q,R){if(this.__du[Q.pointerId]){this.__dK(this.__du[Q.pointerId]);delete this.__du[Q.pointerId];}
;this.__du[Q.pointerId]={"startTime":new Date().getTime(),"lastEventTime":new Date().getTime(),"startX":Q.clientX,"startY":Q.clientY,"clientX":Q.clientX,"clientY":Q.clientY,"velocityX":0,"velocityY":0,"target":R,"isTap":true,"isPrimary":Q.isPrimary,"longTapTimer":window.setTimeout(this.__dJ.bind(this,Q,R),qx.event.handler.GestureCore.LONGTAP_TIME)};if(Q.isPrimary){this.__dz=false;this.__dy=R;this.__dG(l,Q,R);}
else {this.__dz=true;if(Object.keys(this.__du).length===2){this.__dA=this._calcAngle();this.__dC=this._calcDistance();}
;}
;}
,gestureMove:function(T,U){var V=this.__du[T.pointerId];if(V){var S=V.clientX;var W=V.clientY;V.clientX=T.clientX;V.clientY=T.clientY;V.lastEventTime=new Date().getTime();if(S){V.velocityX=V.clientX-S;}
;if(W){V.velocityY=V.clientY-W;}
;if(Object.keys(this.__du).length===2){this.__dH(T,V.target);this.__dI(T,V.target);}
;if(!this.__dz){this.__dG(w,T,V.target);this._fireRoll(T,e,V.target);}
;if(V.isTap){V.isTap=this._isBelowTapMaxDistance(T);if(!V.isTap){this.__dK(V);}
;}
;}
;}
,_hasIntermediaryHandler:function(X){while(X&&X!==this.__dc){if(X.$$gestureHandler){return true;}
;X=X.parentNode;}
;return false;}
,gestureFinish:function(ba,bb){if(!this.__du[ba.pointerId]){return;}
;var bh=this.__du[ba.pointerId];this.__dK(bh);if(this._hasIntermediaryHandler(bb)){return;}
;this.__dD(bh.velocityX,bh.velocityY,ba,bh.target);this.__dG(z,ba,bh.target);if(bh.isTap){if(bb!==bh.target){delete this.__du[ba.pointerId];return;}
;this._fireEvent(ba,B,ba.target||bb);var bc=false;if(Object.keys(this.__dv).length>0){var bg=Date.now()-qx.event.handler.GestureCore.DOUBLETAP_TIME;for(var bi in this.__dv){if(bi<bg){delete this.__dv[bi];}
else {var Y=this.__dv[bi];var be=this.__dE(Y.x,Y.y,ba.clientX,ba.clientY,ba.getPointerType());var bf=Y.target===(ba.target||bb);var bj=Y.button===ba.button;if(be&&bj&&bf){bc=true;delete this.__dv[bi];this._fireEvent(ba,p,ba.target||bb);}
;}
;}
;}
;if(!bc){this.__dv[Date.now()]={x:ba.clientX,y:ba.clientY,target:ba.target||bb,button:ba.button};}
;}
else if(!this._isBelowTapMaxDistance(ba)){var bd=this.__dF(ba,bb);if(bd){ba.swipe=bd;this._fireEvent(ba,a,bh.target||bb);}
;}
;delete this.__du[ba.pointerId];}
,stopMomentum:function(bk){this.__dw[bk]=true;}
,gestureCancel:function(bl){if(this.__du[bl]){this.__dK(this.__du[bl]);delete this.__du[bl];}
;}
,updateGestureTarget:function(bm,bn){this.__du[bm].target=bn;}
,__dD:function(bs,bt,bp,bq,bv){var bu=bp.timeoutId;if((Math.abs(bt)<1&&Math.abs(bs)<1)||this.__dw[bu]){delete this.__dw[bu];return;}
;if(!bv){bv=1;var br=2.8;bt=bt/br;bs=bs/br;}
;bv+=0.0006;bt=bt/bv;bs=bs/bv;var bo=qx.bom.AnimationFrame.request(qx.lang.Function.bind(function(bw,bx,by,bz,bA){this.__dD(bw,bx,by,bz,bA);}
...
,_calcAngle:function(){var bC=null;var bD=null;for(var bB in this.__du){var bE=this.__du[bB];if(bC===null){bC=bE;}
else {bD=bE;}
;}
;var x=bC.clientX-bD.clientX;var y=bC.clientY-bD.clientY;return (360+Math.atan2(y,x)*(180/Math.PI))%360;}
,_calcDistance:function(){var bF=null;var bG=null;for(var bI in this.__du){var bJ=this.__du[bI];if(bF===null){bF=bJ;}
else {bG=bJ;}
;}
...
,__dE:function(bN,bR,bS,bT,bU){var bQ=qx.event.handler.GestureCore;var bO=Math.abs(bN-bS)<bQ.TAP_MAX_DISTANCE[bU];var bP=Math.abs(bR-bT)<bQ.TAP_MAX_DISTANCE[bU];return bO&&bP;}
,_getDeltaCoordinates:function(bX){var bY=this.__du[bX.pointerId];if(!bY){return null;}
;var bV=bX.clientX-bY.startX;var bW=bX.clientY-bY.startY;var ca=E;if(Math.abs(bV/bW)<1){ca=n;}
;return {"x":bV,"y":bW,"axis":ca};}
,_fireEvent:function(cc,ce,cd){if(!this.__dc){return;}
...
else if(this.__dd){cb=new qx.event.type.dom.Custom(ce,cc,{target:this.__dc,currentTarget:this.__dc,srcElement:this.__dc,swipe:cc.swipe,scale:cc.scale,angle:cc.angle,delta:cc.delta,pointerType:cc.pointerType,momentum:cc.momentum});this.__dd.emit(ce,cc);}
;}
,_onDblClick:function(cf){var cg=qx.bom.Event.getTarget(cf);this._fireEvent(cf,B,cg);this._fireEvent(cf,p,cg);}
,__dF:function(cj,ck){var cq=this.__du[cj.pointerId];if(!cq){return null;}
...
,__dG:function(cs,ct,cu){ct.delta=this._getDeltaCoordinates(ct);this._fireEvent(ct,cs,ct.target||cu);}
...
else {var cy=this.__du[cw.pointerId];cw.delta={x:-cy.velocityX,y:-cy.velocityY,axis:Math.abs(cy.velocityX/cy.velocityY)<1?n:E};}
;this._fireEvent(cw,h,cw.target||cx);}
,__dH:function(cz,cB){if(!cz.isPrimary){var cA=this._calcAngle();cz.angle=Math.round((cA-this.__dA)%360);this._fireEvent(cz,k,this.__dy);}
;}
,__dI:function(cE,cF){if(!cE.isPrimary){var cC=this._calcDistance();var cD=cC/this.__dC;cE.scale=(Math.round(cD*100)/100);this._fireEvent(cE,b,this.__dy);}
;}
,__dJ:function(cG,cH){var cI=this.__du[cG.pointerId];if(cI){this._fireEvent(cG,f,cG.target||cH);cI.longTapTimer=null;cI.isTap=false;}
;}
,__dK:function(cJ){if(cJ.longTapTimer){window.clearTimeout(cJ.longTapTimer);cJ.longTapTimer=null;}
;}
...
,dispose:function(){for(var cM in this.__du){this.__dK(cM);}
...
,members:{__gz:null,__cz:null,__gK:null,__cU:null,__hp:null,__hq:null,canHandleEvent:function(o,n){}
,registerEvent:function(r,q,p){}
,unregisterEvent:function(u,t,s){}
,_initObserver:function(){this.__cU=qx.lang.Function.listener(this.checkAndFireGesture,this);qx.event.handler.Gesture.GESTURE_EVENTS.forEach(function(w){qx.event.Registration.addListener(this.__gK,w,this.__cU,this);}
...
,_stopObserver:function(){qx.event.handler.Gesture.GESTURE_EVENTS.forEach(function(B){qx.event.Registration.removeListener(this.__gK,B,this.__cU);}
...
(function(){var a="Image could not be loaded: ",b="Boolean",c="px",d="__kj",e="http",f=".png",g="background-image",h="engine.version",i="scale",j="changeSource",k="div",l="aborted",m="nonScaled",n="qx.ui.basic.Image",o="0 0",p=", no-repeat",q="qx.debug",r="loaded",s="backgroundImage",t="backgroundRepeat",u="-disabled.$1",v="class",w="qx.event.type.Event",x="loadingFailed",y="css.alphaimageloaderneeded",z="String",A="browser.documentmode",B="backgroundPosition",C="border-box",D="left",E="_applySource",F="$$widget",G="data:image/",H="top",I="scaled",J=", ",K="image",L="mshtml",M="engine.name",N=", 0 0",O="_applyScale",P="try to load an unmanaged relative image: ",Q="position",R="replacement",S="img",T="no-repeat",U="background-position",V="hidden",W="alphaScaled",X=",",Y="absolute";qx.Class.define(n,{extend:qx.ui.core.Widget,construct:function(ba){this.__kj={};qx.ui.core.Widget.call(this);if(ba){this.setSource(ba);}
;}
,properties:{source:{check:z,init:null,nullable:true,event:j,apply:E,themeable:true},scale:{check:b,init:false,themeable:true,apply:O},appearance:{refine:true,init:K},allowShrinkX:{refine:true,init:false},allowShrinkY:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false}},events:{loadingFailed:w,loaded:w,aborted:w},members:{__kk:null,__kl:null,__iN:null,__kj:null,__km:null,__kn:null,__ko:0,_onChangeTheme:function(){qx.ui.core.Widget.prototype._onChangeTheme.call(this);this._styleSource();}
...
,_applyDecorator:function(bd,bc){qx.ui.core.Widget.prototype._applyDecorator.call(this,bd,bc);var be=this.getSource();be=qx.util.AliasManager.getInstance().resolve(be);var bb=this.getContentElement();if(this.__kn){bb=bb.getChild(0);}
;this.__kA(bb,be);}
,_applyPadding:function(bg,bf,name){qx.ui.core.Widget.prototype._applyPadding.call(this,bg,bf,name);var bh=this.getContentElement();if(this.__kn){bh.getChild(0).setStyles({top:this.getPaddingTop()||0,left:this.getPaddingLeft()||0});}
else {bh.setPadding(this.getPaddingLeft()||0,this.getPaddingTop()||0);}
;}
,renderLayout:function(bk,top,bi,bl){qx.ui.core.Widget.prototype.renderLayout.call(this,bk,top,bi,bl);var bj=this.getContentElement();if(this.__kn){bj.getChild(0).setStyles({width:bi-(this.getPaddingLeft()||0)-(this.getPaddingRight()||0),height:bl-(this.getPaddingTop()||0)-(this.getPaddingBottom()||0),top:this.getPaddingTop()||0,left:this.getPaddingLeft()||0});}
;}
...
,__kp:function(br){this.__iN=br;}
,__kq:function(){if(this.__iN==null){var bt=this.getSource();var bs=false;if(bt!=null){bs=qx.lang.String.endsWith(bt,f);}
;if(this.getScale()&&bs&&qx.core.Environment.get(y)){this.__iN=W;}
else if(this.getScale()){this.__iN=I;}
else {this.__iN=m;}
;}
;return this.__iN;}
,__kr:function(bw){var bv;var bu;if(bw==W){bv=true;bu=k;}
else if(bw==m){bv=false;bu=k;}
else {bv=true;bu=S;}
;var by=new qx.html.Image(bu);by.setAttribute(F,this.toHashCode());by.setScale(bv);by.setStyles({"overflowX":V,"overflowY":V,"boxSizing":C});if(qx.core.Environment.get(y)){var bx=this.__kn=new qx.html.Element(k);bx.setAttribute(F,this.toHashCode());bx.setStyle(Q,Y);bx.add(by);return bx;}
;return by;}
,__ks:function(){if(this.$$disposed){return null;}
;var bz=this.__kq();if(this.__kj[bz]==null){this.__kj[bz]=this.__kr(bz);}
;var bA=this.__kj[bz];if(!this.__km){this.__km=bA;}
;return bA;}
,_styleSource:function(){var bB=qx.util.AliasManager.getInstance().resolve(this.getSource());var bE=this.getContentElement();if(this.__kn){bE=bE.getChild(0);}
;if(!bB){bE.resetSource();return;}
;this.__kv(bB);if((qx.core.Environment.get(M)==L)&&(parseInt(qx.core.Environment.get(h),10)<9||qx.core.Environment.get(A)<9)){var bC=this.getScale()?i:T;bE.tagNameHint=qx.bom.element.Decoration.getTagName(bC,bB);}
;var bD=this.__ku();if(qx.util.ResourceManager.getInstance().has(bB)){this.__kx(bD,bB);this.__kt();}
else if(qx.io.ImageLoader.isLoaded(bB)){this.__ky(bD,bB);this.__kt();}
else {this.__kz(bD,bB);}
;}
,__kt:function(){this.__ko++ ;qx.bom.AnimationFrame.request(function(bF){if(bF===this.__ko){this.fireEvent(r);}
else {this.fireEvent(l);}
;}
.bind(this,this.__ko));}
,__ku:function(){var bG=this.__km;if(this.__kn){bG=bG.getChild(0);}
;return bG;}
,__kv:qx.core.Environment.select(M,{"mshtml":function(bI){var bJ=qx.core.Environment.get(y);var bH=qx.lang.String.endsWith(bI,f);if(bJ&&bH){if(this.getScale()&&this.__kq()!=W){this.__kp(W);}
else if(!this.getScale()&&this.__kq()!=m){this.__kp(m);}
;}
else {if(this.getScale()&&this.__kq()!=I){this.__kp(I);}
else if(!this.getScale()&&this.__kq()!=m){this.__kp(m);}
;}
;this.__kw(this.__ks());}
,"default":function(bK){if(this.getScale()&&this.__kq()!=I){this.__kp(I);}
else if(!this.getScale()&&this.__kq(m)){this.__kp(m);}
;this.__kw(this.__ks());}
}),__kw:function(bO){var bN=this.__km;if(bN!=bO){if(bN!=null){var ca=c;var bL={};var bT=this.getBounds();if(bT!=null){bL.width=bT.width+ca;bL.height=bT.height+ca;}
;var bU=this.getInsets();bL.left=parseInt(bN.getStyle(D)||bU.left)+ca;bL.top=parseInt(bN.getStyle(H)||bU.top)+ca;bL.zIndex=10;var bR=this.__kn?bO.getChild(0):bO;bR.setStyles(bL,true);bR.setSelectable(this.getSelectable());if(!bN.isVisible()){bO.hide();}
;if(!bN.isIncluded()){bO.exclude();}
;var bW=bN.getParent();if(bW){var bM=bW.getChildren().indexOf(bN);bW.removeAt(bM);bW.addAt(bO,bM);}
;var bQ=bR.getNodeName();bR.setSource(null);var bP=this.__ku();bR.tagNameHint=bQ;bR.setAttribute(v,bP.getAttribute(v));qx.html.Element.flush();var bY=bP.getDomElement();var bX=bO.getDomElement();var bV=bN.getListeners()||[];bV.forEach(function(cb){bO.addListener(cb.type,cb.handler,cb.self,cb.capture);}
);if(bY&&bX){var bS=bY.$$hash;bY.$$hash=bX.$$hash;bX.$$hash=bS;}
;this.__km=bO;}
;}
;}
,__kx:function(cd,cf){var ce=qx.util.ResourceManager.getInstance();if(!this.getEnabled()){var cc=cf.replace(/\.([a-z]+)$/,u);if(ce.has(cc)){cf=cc;this.addState(R);}
else {this.removeState(R);}
;}
;if(cd.getSource()===cf){return;}
;this.__kA(cd,cf);this.__kC(ce.getImageWidth(cf),ce.getImageHeight(cf));}
,__ky:function(cg,ck){var ci=qx.io.ImageLoader;this.__kA(cg,ck);var cj=ci.getWidth(ck);var ch=ci.getHeight(ck);this.__kC(cj,ch);}
,__kz:function(cl,co){var cp=qx.io.ImageLoader;if(qx.core.Environment.get(q)){var cn=co.toLowerCase();var cm=qx.lang.String.startsWith;if(!cm(cn,e)&&!cm(cn,G)){var self=this.self(arguments);if(!self.__Mh){self.__Mh={};}
;if(!self.__Mh[co]){this.debug(P+co);self.__Mh[co]=true;}
;}
;}
;if(!cp.isFailed(co)){cp.load(co,this.__kB,this);}
else {if(cl!=null){cl.resetSource();}
;}
;}
,__kA:function(cq,cv){if(cq.getNodeName()==k){var cy=qx.theme.manager.Decoration.getInstance().resolve(this.getDecorator());if(cy){var cw=(cy.getStartColor()&&cy.getEndColor());var cu=cy.getBackgroundImage();if(cw||cu){var cr=this.getScale()?i:T;var cs=qx.bom.element.Decoration.getAttributes(cv,cr);var ct=cy.getStyles(true);var cx={"backgroundImage":cs.style.backgroundImage,"backgroundPosition":(cs.style.backgroundPosition||o),"backgroundRepeat":(cs.style.backgroundRepeat||T)};if(cu){cx[B]+=X+ct[U]||o;cx[t]+=J+cy.getBackgroundRepeat();}
;if(cw){cx[B]+=N;cx[t]+=p;}
;cx[s]+=X+ct[g];cq.setStyles(cx);return;}
;}
else {cq.setSource(null);}
;}
;cq.setSource(cv);}
,__kB:function(cz,cA){if(this.$$disposed===true){return;}
;if(cz!==qx.util.AliasManager.getInstance().resolve(this.getSource())){this.fireEvent(l);return;}
;if(cA.failed){this.warn(a+cz);this.fireEvent(x);}
else if(cA.aborted){this.fireEvent(l);return;}
else {this.fireEvent(r);}
;this.__ky(this.__ku(),cz);}
,__kC:function(cB,cC){if(cB!==this.__kk||cC!==this.__kl){this.__kk=cB;this.__kl=cC;qx.ui.core.queue.Layout.add(this);}
;}
},destruct:function(){for(var cD in this.__kj){if(this.__kj.hasOwnProperty(cD)){this.__kj[cD].setAttribute(F,null,true);}
;}
...
;}
;this.__cP[E]=null;}
,__kG:function(event){var I=qx.core.Environment.select(a,{"true":qx.event.GlobalError.observeMethod(this.__kH),"false":this.__kH});I.apply(this,arguments);}
...
,"false":null}),_clearSeparators:function(){var m=this.__ms;if(m instanceof qx.ui.core.LayoutItem){m.clearSeparators();}
;}
,_renderSeparator:function(n,o){this.__ms.renderSeparator(n,o);}
,connectToWidget:function(p){if(p&&this.__ms){throw new Error(e);}
;this.__ms=p;this.invalidateChildrenCache();}
,_getWidget:function(){return this.__ms;}
,_applyLayoutChange:function(){if(this.__ms){this.__ms.scheduleLayoutUpdate();}
;}
,_getLayoutChildren:function(){return this.__ms.getLayoutChildren();}
},destruct:function(){this.__ms=this.__eb=null;}
});}
)();
...
(function(){var a="blur",b="qx.debug",c="__oK",d="qx.ui.popup.Manager",f="Object is no popup: ",g="pointerdown",h="singleton";qx.Class.define(d,{type:h,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__oK=[];qx.event.Registration.addListener(document.documentElement,g,this.__oM,this,true);qx.bom.Element.addListener(window,a,this.hideAll,this);}
,members:{__oK:null,add:function(j){if(qx.core.Environment.get(b)){if(!(j instanceof qx.ui.popup.Popup)){throw new Error(f+j);}
;}
;this.__oK.push(j);this.__oL();}
,remove:function(k){if(qx.core.Environment.get(b)){if(!(k instanceof qx.ui.popup.Popup)){throw new Error(f+k);}
;}
;qx.lang.Array.remove(this.__oK,k);this.__oL();}
,hideAll:function(){var l=this.__oK.length,m={};while(l-- ){m=this.__oK[l];if(m.getAutoHide()){m.exclude();}
;}
;}
,__oL:function(){var n=1e7;for(var i=0;i<this.__oK.length;i++ ){this.__oK[i].setZIndex(n++ );}
;}
,__oM:function(e){var p=qx.ui.core.Widget.getWidgetByElement(e.getTarget());var q=this.__oK;for(var i=0;i<q.length;i++ ){var o=q[i];if(!o.getAutoHide()||p==o||qx.ui.core.Widget.contains(o,p)){continue;}
;o.exclude();}
;}
},destruct:function(){qx.event.Registration.removeListener(document.documentElement,g,this.__oM,this,true);this._disposeArray(c);}
});}
)();
...
;}
,properties:{alignX:{check:[o,j,p],init:o,apply:c},alignY:{check:[n,b,g],init:n,apply:c},spacing:{check:q,init:0,apply:c},separator:{check:a,nullable:true,apply:c},reversed:{check:k,init:false,apply:e}},members:{__nN:null,__nO:null,__nP:null,__gW:null,_applyReversed:function(){this._invalidChildrenCache=true;this._applyLayoutChange();}
,__nQ:function(){var B=this._getLayoutChildren();var length=B.length;var y=false;var w=this.__nN&&this.__nN.length!=length&&this.__nO&&this.__nN;var z;var x=w?this.__nN:new Array(length);var A=w?this.__nO:new Array(length);if(this.getReversed()){B=B.concat().reverse();}
...
;}
;if(!w){this.__nN=x;this.__nO=A;}
;this.__nP=y;this.__gW=B;delete this._invalidChildrenCache;}
...
;}
,"false":null}),renderLayout:function(U,O,T){if(this._invalidChildrenCache){this.__nQ();}
...
;var i,V,Q,P;var X=[];var L=H;for(i=0;i<length;i+=1){P=this.__nN[i];Q=P!=null?Math.floor((U-H)*P):K[i].getSizeHint().width;X.push(Q);L+=Q;}
;if(this.__nP&&L!=U){var N={};var R,F;for(i=0;i<length;i+=1){R=this.__nO[i];if(R>0){M=K[i].getSizeHint();N[i]={min:M.minWidth,value:X[i],max:M.maxWidth,flex:R};}
;}
...
;var bs=qx.ui.layout.Util;var bf=this.__gW;var bk=0,bl=0,be=0;var bi=0,bj=0;var bp,bg,br;for(var i=0,l=bf.length;i<l;i+=1){bp=bf[i];bg=bp.getSizeHint();bl+=bg.width;var bo=this.__nO[i];var bh=this.__nN[i];if(bo){bk+=bg.minWidth;}
...
},destruct:function(){this.__nN=this.__nO=this.__gW=null;}
});}
)();
...
;}
,properties:{rich:{check:d,init:false,event:J,apply:L},wrap:{check:d,init:true,apply:j},value:{check:y,apply:t,event:k,nullable:true},buddy:{check:f,apply:w,nullable:true,init:null,dereference:true},textAlign:{check:[P,u,G,I],nullable:true,themeable:true,apply:c,event:i},appearance:{refine:true,init:N},selectable:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false},allowShrinkY:{refine:true,init:false}},members:{__mt:null,__mu:null,__mv:null,__mw:null,_getContentHint:function(){if(this.__mu){this.__mx=this.__my();delete this.__mu;}
;return {width:this.__mx.width,height:this.__mx.height};}
...
;}
,__mx:{width:0,height:0},_applyFont:function(bc,bb){if(bb&&this.__mt&&this.__mw){this.__mt.removeListenerById(this.__mw);this.__mw=null;}
;var ba;if(bc){this.__mt=qx.theme.manager.Font.getInstance().resolve(bc);if(this.__mt instanceof qx.bom.webfonts.WebFont){this.__mw=this.__mt.addListener(h,this._onWebFontStatusChange,this);}
;ba=this.__mt.getStyles();}
else {this.__mt=null;ba=qx.bom.Font.getDefaultStyles();}
;if(this.getTextColor()!=null){delete ba[m];}
;this.getContentElement().setStyles(ba);this.__mu=true;qx.ui.core.queue.Layout.add(this);}
,__my:function(bf){var be=qx.bom.Label;var bh=this.getFont();var bd=bh?this.__mt.getStyles():qx.bom.Font.getDefaultStyles();var content=this.getValue()||R;var bg=this.getRich();if(this.__mw){this.__mz();}
;return bg?be.getHtmlSize(content,bd,bf):be.getTextSize(content,bd);}
,__mz:function(){if(!this.getContentElement()){return;}
...
;}
;}
,_applyBuddy:function(bk,bj){if(bj!=null){this.removeRelatedBindings(bj);this.removeListenerById(this.__mv);this.__mv=null;}
;if(bk!=null){bk.bind(x,this,x);this.__mv=this.addListener(Q,function(){if(bk.isFocusable()){bk.focus.apply(bk);}
;if(z in bk&&typeof bk.toggleValue===C){bk.toggleValue();}
;}
,this);}
;}
,_applyRich:function(bl){this.getContentElement().setRich(bl);this.__mu=true;qx.ui.core.queue.Layout.add(this);}
...
;}
,"false":null}),_onWebFontStatusChange:function(bp){if(bp.getData().valid===true){if(qx.core.Environment.get(l)==a&&parseFloat(qx.core.Environment.get(D))>=8){window.setTimeout(function(){this.__mu=true;qx.ui.core.queue.Layout.add(this);}
.bind(this),0);}
;this.__mu=true;qx.ui.core.queue.Layout.add(this);}
;}
,_applyValue:function(br,bq){this.getContentElement().setValue(br);this.__mu=true;qx.ui.core.queue.Layout.add(this);}
},destruct:function(){if(qx.core.Environment.get(E)){qx.locale.Manager.getInstance().removeListener(O,this._onChangeLocale,this);}
;if(this.__mt&&this.__mw){this.__mt.removeListenerById(this.__mw);}
;this.__mt=null;}
});}
)();
(function(){var a="value",b="qx.html.Label",c="The label mode cannot be modified after initial creation",d='hidden';qx.Class.define(b,{extend:qx.html.Element,members:{__mA:null,_applyProperty:function(name,e){qx.html.Element.prototype._applyProperty.call(this,name,e);if(name==a){var f=this.getDomElement();qx.bom.Label.setValue(f,e);}
;}
,_createDomElement:function(){var h=this.__mA;var g=qx.bom.Label.create(this._content,h);g.style.overflow=d;return g;}
...
;j=!!j;if(this.__mA==j){return this;}
;this.__mA=j;return this;}
,setValue:function(l){this._setProperty(a,l);return this;}
,getValue:function(){return this._getProperty(a);}
}});}
)();
(function(){var a="text",b="function",c="px",d="http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul",e="crop",f="nowrap",g="end",h="div",i="100%",j="auto",k="0",l="css.textoverflow",m="qx.debug",n="html.xul",o="value",p="visible",q="qx.bom.Label",r="",s="ellipsis",t="normal",u="inherit",v="block",w="label",x="-1000px",y="hidden",z="absolute";qx.Bootstrap.define(q,{statics:{__mB:{fontFamily:1,fontSize:1,fontWeight:1,fontStyle:1,lineHeight:1},__mC:function(){var A=this.__mE(false);document.body.insertBefore(A,document.body.firstChild);return this._textElement=A;}
,__mD:function(){var B=this.__mE(true);document.body.insertBefore(B,document.body.firstChild);return this._htmlElement=B;}
,__mE:function(E){var C=qx.dom.Element.create(h);var D=C.style;D.width=D.height=j;D.left=D.top=x;D.visibility=y;D.position=z;D.overflow=p;D.display=v;if(E){D.whiteSpace=t;}
else {D.whiteSpace=f;if(!qx.core.Environment.get(l)&&qx.core.Environment.get(n)){var F=document.createElementNS(d,w);var D=F.style;D.padding=k;D.margin=k;D.width=j;for(var G in this.__mB){D[G]=u;}
;C.appendChild(F);}
;}
;return C;}
,__mF:function(I){var H={};if(I){H.whiteSpace=t;}
...
else if(!qx.core.Environment.get(l)&&qx.core.Environment.get(n)){var N=L.document.createElementNS(d,w);var K=N.style;K.cursor=u;K.color=u;K.overflow=y;K.maxWidth=i;K.padding=k;K.margin=k;K.width=j;for(var O in this.__mB){N.style[O]=u;}
;N.setAttribute(e,g);J.appendChild(N);}
else {qx.bom.element.Style.setStyles(J,this.__mF(M));}
;if(content){this.setValue(J,content);}
;return J;}
,__mG:null,setSanitizer:function(P){if(qx.core.Environment.get(m)){if(P){qx.core.Assert.assertFunction(P);}
;}
;qx.bom.Label.__mG=P;}
,setValue:function(R,Q){Q=Q||r;if(R.useHtml){if(qx.bom.Label.__mG&&typeof (qx.bom.Label.__mG)===b){Q=qx.bom.Label.__mG(Q);}
;R.innerHTML=Q;}
...
;}
,getHtmlSize:function(content,T,U){var V=this._htmlElement||this.__mD();V.style.width=U!=undefined?U+c:j;V.innerHTML=content;return this.__mH(V,T);}
,getTextSize:function(X,W){var Y=this._textElement||this.__mC();if(!qx.core.Environment.get(l)&&qx.core.Environment.get(n)){Y.firstChild.setAttribute(o,X);}
else {qx.bom.element.Attribute.set(Y,a,X);}
;return this.__mH(Y,W);}
,__mH:function(be,ba){var bb=this.__mB;if(!ba){ba={};}
...
(function(){var a="_applyActiveWindow",b="changeModal",c="__gz",d="__oN",f="changeVisibility",g="changeActive",h="qx.ui.window.MDesktop",i="qx.ui.window.Window";qx.Mixin.define(h,{properties:{activeWindow:{check:i,apply:a,init:null,nullable:true}},members:{__oN:null,__gz:null,getWindowManager:function(){if(!this.__gz){this.setWindowManager(new qx.ui.window.Window.DEFAULT_MANAGER_CLASS());}
;return this.__gz;}
...
,getWindows:function(){if(!this.__oN){this.__oN=[];}
;return this.__oN;}
...
;e.preventDefault();}
,__oP:function(e){if(e.getKeyIdentifier()!==m){return;}
...
(function(){var a="keypress",b="focusout",c="activate",d="Tab",f="singleton",g="deactivate",h="__oQ",j="focusin",k="qx.ui.core.FocusHandler";qx.Class.define(k,{extend:qx.core.Object,type:f,construct:function(){qx.core.Object.call(this);this.__oQ={};}
,members:{__oQ:null,__oR:null,__oS:null,__oT:null,connectTo:function(m){m.addListener(a,this.__hJ,this);m.addListener(j,this._onFocusIn,this,true);m.addListener(b,this._onFocusOut,this,true);m.addListener(c,this._onActivate,this,true);m.addListener(g,this._onDeactivate,this,true);}
,addRoot:function(n){this.__oQ[n.$$hash]=n;}
,removeRoot:function(o){delete this.__oQ[o.$$hash];}
,getActiveWidget:function(){return this.__oR;}
,isActive:function(p){return this.__oR==p;}
,getFocusedWidget:function(){return this.__oS;}
,isFocused:function(q){return this.__oS==q;}
,isFocusRoot:function(r){return !!this.__oQ[r.$$hash];}
,_onActivate:function(e){var t=e.getTarget();this.__oR=t;var s=this.__oU(t);if(s!=this.__oT){this.__oT=s;}
;}
,_onDeactivate:function(e){var u=e.getTarget();if(this.__oR==u){this.__oR=null;}
;}
,_onFocusIn:function(e){var v=e.getTarget();if(v!=this.__oS){this.__oS=v;v.visualizeFocus();}
;}
,_onFocusOut:function(e){var w=e.getTarget();if(w==this.__oS){this.__oS=null;w.visualizeBlur();}
;}
,__hJ:function(e){if(e.getKeyIdentifier()!=d){return;}
;if(!this.__oT){return;}
;e.stopPropagation();e.preventDefault();var x=this.__oS;if(!e.isShiftPressed()){var y=x?this.__oY(x):this.__oW();}
else {var y=x?this.__pa(x):this.__oX();}
;if(y){y.tabFocus();}
;}
,__oU:function(z){var A=this.__oQ;while(z){if(A[z.$$hash]){return z;}
;z=z.getLayoutParent();}
;return null;}
,__oV:function(I,H){if(I===H){return 0;}
...
;return 0;}
,__oW:function(){return this.__pd(this.__oT,null);}
,__oX:function(){return this.__pe(this.__oT,null);}
,__oY:function(M){var N=this.__oT;if(N==M){return this.__oW();}
;while(M&&M.getAnonymous()){M=M.getLayoutParent();}
;if(M==null){return [];}
;var O=[];this.__pb(N,M,O);O.sort(this.__oV);var P=O.length;return P>0?O[0]:this.__oW();}
,__pa:function(Q){var R=this.__oT;if(R==Q){return this.__oX();}
;while(Q&&Q.getAnonymous()){Q=Q.getLayoutParent();}
;if(Q==null){return [];}
;var S=[];this.__pc(R,Q,S);S.sort(this.__oV);var T=S.length;return T>0?S[T-1]:this.__oX();}
,__pb:function(parent,U,V){var X=parent.getLayoutChildren();var W;for(var i=0,l=X.length;i<l;i++ ){W=X[i];if(!(W instanceof qx.ui.core.Widget)){continue;}
;if(!this.isFocusRoot(W)&&W.isEnabled()&&W.isVisible()){if(W.isTabable()&&this.__oV(U,W)<0){V.push(W);}
;this.__pb(W,U,V);}
;}
;}
,__pc:function(parent,Y,ba){var bc=parent.getLayoutChildren();var bb;for(var i=0,l=bc.length;i<l;i++ ){bb=bc[i];if(!(bb instanceof qx.ui.core.Widget)){continue;}
;if(!this.isFocusRoot(bb)&&bb.isEnabled()&&bb.isVisible()){if(bb.isTabable()&&this.__oV(Y,bb)>0){ba.push(bb);}
;this.__pc(bb,Y,ba);}
;}
;}
,__pd:function(parent,bd){var bf=parent.getLayoutChildren();var be;for(var i=0,l=bf.length;i<l;i++ ){be=bf[i];if(!(be instanceof qx.ui.core.Widget)){continue;}
;if(!this.isFocusRoot(be)&&be.isEnabled()&&be.isVisible()){if(be.isTabable()){if(bd==null||this.__oV(be,bd)<0){bd=be;}
;}
;bd=this.__pd(be,bd);}
;}
;return bd;}
,__pe:function(parent,bg){var bi=parent.getLayoutChildren();var bh;for(var i=0,l=bi.length;i<l;i++ ){bh=bi[i];if(!(bh instanceof qx.ui.core.Widget)){continue;}
;if(!this.isFocusRoot(bh)&&bh.isEnabled()&&bh.isVisible()){if(bh.isTabable()){if(bg==null||this.__oV(bh,bg)>0){bg=bh;}
;}
;bg=this.__pe(bh,bg);}
;}
;return bg;}
},destruct:function(){this._disposeMap(h);this.__oS=this.__oR=this.__oT=null;}
});}
)();
(function(){var a="touchmove",b="os.name",c="-webkit-overflow-scrolling",d="touch",f="paddingLeft",g="div",h="100%",i="The root widget does not support 'left', or 'top' paddings!",j="0px",k="The application could not be started due to a missing body tag in the HTML file!",l="ios",m="overflowY",n="resize",o="",p="$$widget",q="paddingTop",r="engine.name",s="none",t="webkit",u="-webkit-backface-visibility",v="touch-action",w="qx.ui.root.Application",x="hidden",y="tap",z="overflowX",A="absolute";qx.Class.define(w,{extend:qx.ui.root.Abstract,construct:function(B){this.__cz=qx.dom.Node.getWindow(B);this.__pf=B;qx.ui.root.Abstract.call(this);qx.event.Registration.addListener(this.__cz,n,this._onResize,this);this._setLayout(new qx.ui.layout.Canvas());qx.ui.core.queue.Layout.add(this);qx.ui.core.FocusHandler.getInstance().connectTo(this);this.getContentElement().disableScrolling();this.getContentElement().setStyle(u,x);this.addListener(a,this.__pg,this);if(qx.core.Environment.get(b)==l){this.getContentElement().addListener(y,function(e){var C=qx.ui.core.Widget.getWidgetByElement(e.getTarget());while(C&&!C.isFocusable()){C=C.getLayoutParent();}
;if(C&&C.isFocusable()){C.getContentElement().focus();}
;}
,this,true);}
;}
,members:{__cz:null,__pf:null,_createContentElement:function(){var D=this.__pf;if((qx.core.Environment.get(r)==t)){if(!D.body){alert(k);}
;}
...
,__pg:function(e){var M=e.getOriginalTarget();while(M&&M.style){var Q=qx.bom.element.Style.get(M,v)!==s&&qx.bom.element.Style.get(M,v)!==o;var P=qx.bom.element.Style.get(M,c)===d;var O=qx.bom.element.Style.get(M,z)!=x;var N=qx.bom.element.Style.get(M,m)!=x;if(Q||P||N||O){return;}
;M=M.parentNode;}
;e.preventDefault();}
},destruct:function(){this.__cz=this.__pf=null;}
});}
)();
...
(function(){var a="Finder",b="name",c="model",d="selection[0].files",e="model.files",f="demobrowser/demo/data/finder.json",g="demobrowser.demo.data.Finder";qx.Class.define(g,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var i=new qx.ui.window.Window(a);i.setLayout(new qx.ui.layout.HBox());i.setContentPadding(0);i.setShowMaximize(false);i.setShowMinimize(false);i.setShowClose(false);i.open();var j=new qx.ui.form.List();i.add(j);var l=new qx.ui.form.List();i.add(l);var k=new qx.ui.form.List();i.add(k);var o=new qx.data.controller.List(null,j);o.setLabelPath(b);var n=new qx.data.controller.List(null,l);n.setLabelPath(b);var m=new qx.data.controller.List(null,k);m.setLabelPath(b);var h=qx.util.ResourceManager.getInstance().toUri(f);var p=new qx.data.store.Json(h);p.bind(e,o,c);o.bind(d,n,c);n.bind(d,m,c);}
}});}
)();
(function(){var a="indexOf",b="addAfter",c="add",d="addBefore",e="_",f="addAt",g="hasChildren",h="removeAt",i="removeAll",j="getChildren",k="remove",l="qx.ui.core.MRemoteChildrenHandling";qx.Mixin.define(l,{members:{__oa:function(q,m,o,n){var p=this.getChildrenContainer();if(p===this){q=e+q;}
;return (p[q])(m,o,n);}
,getChildren:function(){return this.__oa(j);}
,hasChildren:function(){return this.__oa(g);}
,add:function(s,r){return this.__oa(c,s,r);}
,remove:function(t){return this.__oa(k,t);}
,removeAll:function(){return this.__oa(i);}
,indexOf:function(u){return this.__oa(a,u);}
,addAt:function(x,v,w){this.__oa(f,x,v,w);}
,addBefore:function(y,A,z){this.__oa(d,y,A,z);}
,addAfter:function(D,B,C){this.__oa(b,D,B,C);}
,removeAt:function(E){return this.__oa(h,E);}
}});}
)();
...
(function(){var a="__rR",b="qx.ui.window.Manager";qx.Class.define(b,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__rR:null,setDesktop:function(c){this.__rR=c;this.updateStack();}
...
,syncWidget:function(){this.__rR.forceUnblock();var h=this.__rR.getWindows();var g=this._minZIndex;var m=g+h.length*2;var j=g+h.length*4;var k=null;for(var i=0,l=h.length;i<l;i++ ){var f=h[i];if(!f.isVisible()){continue;}
;k=k||f;if(f.isModal()){f.setZIndex(j);this.__rR.blockContent(j-1);j+=2;k=f;}
...
,bringToFront:function(o){var n=this.__rR.getWindows();var p=qx.lang.Array.remove(n,o);if(p){n.push(o);this.updateStack();}
;}
,sendToBack:function(r){var q=this.__rR.getWindows();var s=qx.lang.Array.remove(q,r);if(s){q.unshift(r);this.updateStack();}
;}
},destruct:function(){this._disposeObjects(a);}
});}
)();
(function(){var a="pointerup",b="Boolean",c="losecapture",d="move",f="qx.ui.core.MMovable",g="__rS",h="The move handle could not be redefined!",i="pointerdown",j="maximized",k="pointermove",l="roll",m="__rT",n="move-frame";qx.Mixin.define(f,{properties:{movable:{check:b,init:true},useMoveFrame:{check:b,init:false}},members:{__rS:null,__rT:null,__rU:null,__rV:null,__rW:null,__rX:null,__rY:null,__sa:false,__sb:null,__sc:0,_activateMoveHandle:function(o){if(this.__rS){throw new Error(h);}
;this.__rS=o;o.addListener(i,this._onMovePointerDown,this);o.addListener(a,this._onMovePointerUp,this);o.addListener(k,this._onMovePointerMove,this);o.addListener(c,this.__sg,this);}
,__sd:function(){var p=this.__rT;if(!p){p=this.__rT=new qx.ui.core.Widget();p.setAppearance(n);p.exclude();qx.core.Init.getApplication().getRoot().add(p);}
;return p;}
,__se:function(){var location=this.getContentLocation();var r=this.getBounds();var q=this.__sd();q.setUserBounds(location.left,location.top,r.width,r.height);q.show();q.setZIndex(this.getZIndex()+1);}
,__sf:function(e){var t=this.__rU;var s=Math.max(t.left,Math.min(t.right,e.getDocumentLeft()));var v=Math.max(t.top,Math.min(t.bottom,e.getDocumentTop()));var u=this.__rV+s;var w=this.__rW+v;return {viewportLeft:parseInt(u,10),viewportTop:parseInt(w,10),parentLeft:parseInt(u-this.__rX,10),parentTop:parseInt(w-this.__rY,10)};}
,_onMoveRoll:function(e){e.stop();}
,_onMovePointerDown:function(e){if(!this.getMovable()||this.hasState(j)){return;}
;this.addListener(l,this._onMoveRoll,this);var parent=this.getLayoutParent();var x=parent.getContentLocation();var z=parent.getBounds();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isBlocked()){this.__sb=parent.getBlockerColor();this.__sc=parent.getBlockerOpacity();parent.setBlockerColor(null);parent.setBlockerOpacity(1);parent.blockContent(this.getZIndex()-1);this.__sa=true;}
;}
;this.__rU={left:x.left,top:x.top,right:x.left+z.width,bottom:x.top+z.height};var y=this.getContentLocation();this.__rX=x.left;this.__rY=x.top;this.__rV=y.left-e.getDocumentLeft();this.__rW=y.top-e.getDocumentTop();this.addState(d);this.__rS.capture();if(this.getUseMoveFrame()){this.__se();}
;e.stop();}
,_onMovePointerMove:function(e){if(!this.hasState(d)){return;}
;var B=this.__sf(e);if(this.getUseMoveFrame()){this.__sd().setDomPosition(B.viewportLeft,B.viewportTop);}
else {var A=this.getLayoutParent().getInsets();this.setDomPosition(B.parentLeft-(A.left||0),B.parentTop-(A.top||0));}
;e.stopPropagation();}
,_onMovePointerUp:function(e){if(this.hasListener(l,this._onMoveRoll,this)){this.removeListener(l,this._onMoveRoll,this);}
;if(!this.hasState(d)){return;}
;this.removeState(d);var parent=this.getLayoutParent();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__sa){parent.unblock();parent.setBlockerColor(this.__sb);parent.setBlockerOpacity(this.__sc);this.__sb=null;this.__sc=0;this.__sa=false;}
;}
;this.__rS.releaseCapture();var D=this.__sf(e);var C=this.getLayoutParent().getInsets();this.setLayoutProperties({left:D.parentLeft-(C.left||0),top:D.parentTop-(C.top||0)});if(this.getUseMoveFrame()){this.__sd().exclude();}
;e.stopPropagation();}
,__sg:function(e){if(!this.hasState(d)){return;}
;this.removeState(d);if(this.getUseMoveFrame()){this.__sd().exclude();}
;}
},destruct:function(){this._disposeObjects(m,g);this.__rU=null;}
});}
)();
(function(){var a="resetPaddingRight",b="setPaddingTop",c="_applyContentPadding",d="setPaddingBottom",e="resetThemed",f="contentPaddingRight",g="Integer",h="contentPaddingLeft",i="setThemedPaddingLeft",j="resetPaddingTop",k="shorthand",l="setThemedPaddingRight",m="setThemed",n="setPaddingRight",o="contentPaddingBottom",p="resetPaddingBottom",q="qx.ui.core.MContentPadding",r="resetPaddingLeft",s="setThemedPaddingTop",t="setPaddingLeft",u="setThemedPaddingBottom",v="contentPaddingTop";qx.Mixin.define(q,{properties:{contentPaddingTop:{check:g,init:0,apply:c,themeable:true},contentPaddingRight:{check:g,init:0,apply:c,themeable:true},contentPaddingBottom:{check:g,init:0,apply:c,themeable:true},contentPaddingLeft:{check:g,init:0,apply:c,themeable:true},contentPadding:{group:[v,f,o,h],mode:k,themeable:true}},members:{__nh:{contentPaddingTop:b,contentPaddingRight:n,contentPaddingBottom:d,contentPaddingLeft:t},__ni:{contentPaddingTop:s,contentPaddingRight:l,contentPaddingBottom:u,contentPaddingLeft:i},__nj:{contentPaddingTop:j,contentPaddingRight:a,contentPaddingBottom:p,contentPaddingLeft:r},_applyContentPadding:function(z,w,name,y){var A=this._getContentPaddingTarget();if(z==null){var x=this.__nj[name];A[x]();}
else {if(y==m||y==e){var B=this.__ni[name];A[B](z);}
else {var B=this.__nh[name];A[B](z);}
;}
;}
}});}
)();
(function(){var a="Boolean",b="w-resize",c="touch",d="sw-resize",f="n-resize",g="resizableRight",h="ne-resize",i="se-resize",j="Integer",k="e-resize",l="resizableLeft",m="move",n="shorthand",o="maximized",p="resize",q="nw-resize",r="pointerup",s="pointerdown",t="qx.ui.core.MResizable",u="losecapture",v="resize-frame",w="resizableBottom",x="s-resize",y="pointermove",z="resizableTop",A="pointerout";qx.Mixin.define(t,{construct:function(){var content=this.getContentElement();content.addListener(s,this.__st,this,true);content.addListener(r,this.__su,this);content.addListener(y,this.__sw,this);content.addListener(A,this.__sx,this);content.addListener(u,this.__sv,this);var B=content.getDomElement();if(B==null){B=window;}
;this.__sh=qx.event.Registration.getManager(B).getHandler(qx.event.handler.DragDrop);}
,properties:{resizableTop:{check:a,init:true},resizableRight:{check:a,init:true},resizableBottom:{check:a,init:true},resizableLeft:{check:a,init:true},resizable:{group:[z,g,w,l],mode:n},resizeSensitivity:{check:j,init:5},useResizeFrame:{check:a,init:true}},members:{__sh:null,__si:null,__sj:null,__sk:null,__sl:null,__sm:null,__sn:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__si;if(!C){C=this.__si=new qx.ui.core.Widget();C.setAppearance(v);C.exclude();qx.core.Init.getApplication().getRoot().add(C);}
;return C;}
,__so:function(){var location=this.getContentLocation();var D=this._getResizeFrame();D.setUserBounds(location.left,location.top,location.right-location.left,location.bottom-location.top);D.show();D.setZIndex(this.getZIndex()+1);}
,__sp:function(e){var F=this.__sj;var E=this.getSizeHint();var I=this.__sn;var H=this.__sm;var J=H.width;var G=H.height;var L=H.left;var top=H.top;var K;if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){K=Math.max(I.top,Math.min(I.bottom,e.getDocumentTop()))-this.__sl;if(F&this.RESIZE_TOP){G-=K;}
else {G+=K;}
;if(G<E.minHeight){G=E.minHeight;}
else if(G>E.maxHeight){G=E.maxHeight;}
;if(F&this.RESIZE_TOP){top+=H.height-G;}
;}
;if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){K=Math.max(I.left,Math.min(I.right,e.getDocumentLeft()))-this.__sk;if(F&this.RESIZE_LEFT){J-=K;}
else {J+=K;}
...
,__sq:{'1':f,'2':x,'4':b,'8':k,'5':q,'6':d,'9':h,'10':i},__sr:function(e){var location=this.getContentLocation();var O=this.getResizeSensitivity();var N=e.getDocumentLeft();var P=e.getDocumentTop();var M=this.__ss(location,N,P,O);if(M>0){M=M|this.__ss(location,N,P,O*2);}
;this.__sj=M;}
,__ss:function(location,T,R,S){var Q=0;if(this.getResizableTop()&&Math.abs(location.top-R)<S&&T>location.left-S&&T<location.right+S){Q+=this.RESIZE_TOP;}
...
;return Q;}
,__st:function(e){if(!this.__sj||!this.getEnabled()||e.getPointerType()==c){return;}
;this.addState(p);this.__sk=e.getDocumentLeft();this.__sl=e.getDocumentTop();var location=this.getContentLocation();var V=this.getBounds();this.__sm={top:location.top,left:location.left,width:location.right-location.left,height:location.bottom-location.top,bounds:qx.lang.Object.clone(V)};var parent=this.getLayoutParent();var W=parent.getContentLocation();var U=parent.getBounds();this.__sn={left:W.left,top:W.top,right:W.left+U.width,bottom:W.top+U.height};if(this.getUseResizeFrame()){this.__so();}
;this.capture();e.stop();}
,__su:function(e){if(!this.hasState(p)||!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.getUseResizeFrame()){this._getResizeFrame().exclude();}
;var X=this.__sp(e);this.setWidth(X.width);this.setHeight(X.height);if(this.getResizableLeft()||this.getResizableTop()){this.setLayoutProperties({left:X.parentLeft,top:X.parentTop});}
;this.__sj=0;this.removeState(p);this.resetCursor();this.getApplicationRoot().resetGlobalCursor();this.releaseCapture();e.stopPropagation();}
,__sv:function(e){if(!this.__sj){return;}
...
;}
,__sw:function(e){if(!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.hasState(p)){var ba=this.__sp(e);if(this.getUseResizeFrame()){var Y=this._getResizeFrame();Y.setUserBounds(ba.viewportLeft,ba.viewportTop,ba.width,ba.height);}
...
;}
;e.stopPropagation();}
else if(!this.hasState(o)&&!this.__sh.isSessionActive()){this.__sr(e);var bd=this.__sj;var bc=this.getApplicationRoot();if(bd){var bb=this.__sq[bd];this.setCursor(bb);bc.setGlobalCursor(bb);}
else if(this.getCursor()){this.resetCursor();bc.resetGlobalCursor();}
;}
;}
,__sx:function(e){if(e.getPointerType()==c){return;}
;if(this.getCursor()&&!this.hasState(p)){this.resetCursor();this.getApplicationRoot().resetGlobalCursor();}
;}
},destruct:function(){if(this.__si!=null&&!qx.core.ObjectRegistry.inShutDown){this.__si.destroy();this.__si=null;}
;this.__sh=null;}
});}
)();
(function(){var a="beforeClose",b="beforeMinimize",c="Boolean",d="window-resize-frame",f="changeStatus",g="changeIcon",h="excluded",i="_applyModal",j="execute",k="statusbar",l="qx.debug",m="_applyActive",n="active",o="minimize-button",p="qx.event.type.Event",q="close-button",r="beforeRestore",s="qx.ui.window.IDesktop. All root widgets implement this interface.",t="captionbar",u="String",v="minimize",w="dbltap",x="modal",y="changeModal",z="title",A="icon",B="showStatusbar",C="changeAlwaysOnTop",D="_applyShowStatusbar",E="pointerdown",F="maximized",G="_applyStatus",H="mshtml",I="qx.ui.window.Window",J="normal",K="Windows can only be added to widgets, which implement the interface ",L="changeCaption",M="engine.name",N="statusbar-text",O="focusout",P="beforeMaximize",Q="maximize",R="maximize-button",S="restore",T="window",U="pane",V="close",W="changeActive",X="restore-button",Y="_applyCaptionBarChange",bc="Centering depends on parent bounds!",bd="minimized";qx.Class.define(I,{extend:qx.ui.core.Widget,include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MRemoteLayoutHandling,qx.ui.core.MResizable,qx.ui.core.MMovable,qx.ui.core.MContentPadding],construct:function(be,bf){qx.ui.core.Widget.call(this);this._setLayout(new qx.ui.layout.VBox());this._createChildControl(t);this._createChildControl(U);if(bf!=null){this.setIcon(bf);}
;if(be!=null){this.setCaption(be);}
;this._updateCaptionBar();this.addListener(E,this._onWindowPointerDown,this,true);this.addListener(O,this._onWindowFocusOut,this);qx.core.Init.getApplication().getRoot().add(this);this.initVisibility();qx.ui.core.FocusHandler.getInstance().addRoot(this);this._getResizeFrame().setAppearance(d);}
,statics:{DEFAULT_MANAGER_CLASS:qx.ui.window.Manager},events:{"beforeClose":p,"close":p,"beforeMinimize":p,"minimize":p,"beforeMaximize":p,"maximize":p,"beforeRestore":p,"restore":p},properties:{appearance:{refine:true,init:T},visibility:{refine:true,init:h},focusable:{refine:true,init:true},active:{check:c,init:false,apply:m,event:W},alwaysOnTop:{check:c,init:false,event:C},modal:{check:c,init:false,event:y,apply:i},caption:{apply:Y,event:L,nullable:true},icon:{check:u,nullable:true,apply:Y,event:g,themeable:true},status:{check:u,nullable:true,apply:G,event:f},showClose:{check:c,init:true,apply:Y,themeable:true},showMaximize:{check:c,init:true,apply:Y,themeable:true},showMinimize:{check:c,init:true,apply:Y,themeable:true},allowClose:{check:c,init:true,apply:Y},allowMaximize:{check:c,init:true,apply:Y},allowMinimize:{check:c,init:true,apply:Y},showStatusbar:{check:c,init:false,apply:D}},members:{__sy:null,__sz:null,getChildrenContainer:function(){return this.getChildControl(U);}
,_forwardStates:{active:true,maximized:true,showStatusbar:true,modal:true},setLayoutParent:function(parent){if(qx.core.Environment.get(l)){parent&&this.assertInterface(parent,qx.ui.window.IDesktop,K+s);}
;qx.ui.core.Widget.prototype.setLayoutParent.call(this,parent);}
,_createChildControlImpl:function(bk,bj){var bg;switch(bk){case k:bg=new qx.ui.container.Composite(new qx.ui.layout.HBox());this._add(bg);bg.add(this.getChildControl(N));break;case N:bg=new qx.ui.basic.Label();bg.setValue(this.getStatus());break;case U:bg=new qx.ui.container.Composite();this._add(bg,{flex:1});break;case t:var bh=new qx.ui.layout.Grid();bh.setRowFlex(0,1);bh.setColumnFlex(1,1);bg=new qx.ui.container.Composite(bh);this._add(bg);bg.addListener(w,this._onCaptionPointerDblTap,this);this._activateMoveHandle(bg);break;case A:bg=new qx.ui.basic.Image(this.getIcon());this.getChildControl(t).add(bg,{row:0,column:0});break;case z:bg=new qx.ui.basic.Label(this.getCaption());bg.setWidth(0);bg.setAllowGrowX(true);var bi=this.getChildControl(t);bi.add(bg,{row:0,column:1});break;case o:bg=new qx.ui.form.Button();bg.setFocusable(false);bg.addListener(j,this._onMinimizeButtonTap,this);this.getChildControl(t).add(bg,{row:0,column:2});break;case X:bg=new qx.ui.form.Button();bg.setFocusable(false);bg.addListener(j,this._onRestoreButtonTap,this);this.getChildControl(t).add(bg,{row:0,column:3});break;case R:bg=new qx.ui.form.Button();bg.setFocusable(false);bg.addListener(j,this._onMaximizeButtonTap,this);this.getChildControl(t).add(bg,{row:0,column:4});break;case q:bg=new qx.ui.form.Button();bg.setFocusable(false);bg.addListener(j,this._onCloseButtonTap,this);this.getChildControl(t).add(bg,{row:0,column:6});break;};return bg||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,bk);}
,_updateCaptionBar:function(){var bm;var bn=this.getIcon();if(bn){this.getChildControl(A).setSource(bn);this._showChildControl(A);}
else {this._excludeChildControl(A);}
;var bl=this.getCaption();if(bl){this.getChildControl(z).setValue(bl);this._showChildControl(z);}
else {this._excludeChildControl(z);}
;if(this.getShowMinimize()){this._showChildControl(o);bm=this.getChildControl(o);this.getAllowMinimize()?bm.resetEnabled():bm.setEnabled(false);}
...
;if(this.getShowClose()){this._showChildControl(q);bm=this.getChildControl(q);this.getAllowClose()?bm.resetEnabled():bm.setEnabled(false);}
...
;}
;if(qx.core.Environment.get(l)){this.warn(bc);}
;}
...
;var br=this.getLayoutProperties();this.__sz=br.left===undefined?0:br.left;this.__sy=br.top===undefined?0:br.top;this.setLayoutProperties({left:null,top:null,edge:0});this.addState(F);this._updateCaptionBar();this.fireEvent(Q);}
;}
;}
,minimize:function(){if(!this.isVisible()){return;}
;if(this.fireNonBubblingEvent(b,qx.event.type.Event,[false,true])){var bs=this.getLayoutProperties();this.__sz=bs.left===undefined?0:bs.left;this.__sy=bs.top===undefined?0:bs.top;this.removeState(F);this.hide();this.fireEvent(v);}
;}
...
;var bt=this.__sz;var top=this.__sy;this.setLayoutProperties({edge:null,left:bt,top:top});this.removeState(F);this._updateCaptionBar();this.fireEvent(S);}
;}
...
;}
;}
,_applyActive:function(bw,bv){if(bv){this.removeState(n);}
else {this.addState(n);}
;}
,_applyModal:function(by,bx){if(bx){this.removeState(x);}
else {this.addState(x);}
;}
,_getContentPaddingTarget:function(){return this.getChildControl(U);}
,_applyShowStatusbar:function(bB,bz){var bA=this._getResizeFrame();if(bB){this.addState(B);bA.addState(B);}
else {this.removeState(B);bA.removeState(B);}
;if(bB){this._showChildControl(k);}
...
;}
,_onMinimizeButtonTap:function(e){this.minimize();this.getChildControl(o).reset();}
,_onRestoreButtonTap:function(e){this.restore();this.getChildControl(X).reset();}
,_onMaximizeButtonTap:function(e){this.maximize();this.getChildControl(R).reset();}
,_onCloseButtonTap:function(e){this.close();this.getChildControl(q).reset();}
}});}
)();
...
;if(v){this.setSeparator(v);}
;}
,properties:{alignY:{check:[k,o,f],init:k,apply:b},alignX:{check:[m,c,r],init:m,apply:b},spacing:{check:p,init:0,apply:b},separator:{check:a,nullable:true,apply:b},reversed:{check:s,init:false,apply:d}},members:{__nT:null,__nO:null,__nP:null,__gW:null,_applyReversed:function(){this._invalidChildrenCache=true;this._applyLayoutChange();}
,__nQ:function(){var B=this._getLayoutChildren();var length=B.length;var x=false;var w=this.__nT&&this.__nT.length!=length&&this.__nO&&this.__nT;var z;var y=w?this.__nT:new Array(length);var A=w?this.__nO:new Array(length);if(this.getReversed()){B=B.concat().reverse();}
...
;}
;if(!w){this.__nT=y;this.__nO=A;}
;this.__nP=x;this.__gW=B;delete this._invalidChildrenCache;}
...
;}
,"false":null}),renderLayout:function(V,N,Y){if(this._invalidChildrenCache){this.__nQ();}
...
;if(this.__nP&&W!=N){var M={};var S,F;for(i=0;i<length;i+=1){S=this.__nO[i];if(S>0){L=K[i].getSizeHint();M[i]={min:L.minHeight,value:P[i],max:L.maxHeight,flex:S};}
;}
...
;}
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__nQ();}
;var be=qx.ui.layout.Util;var bs=this.__gW;var bh=0,bi=0,bq=0;var bj=0,bk=0;var bo,bf,br;for(var i=0,l=bs.length;i<l;i+=1){bo=bs[i];bf=bo.getSizeHint();bi+=bf.height;var bn=this.__nO[i];var bg=this.__nT[i];if(bn){bh+=bf.minHeight;}
...
},destruct:function(){this.__nT=this.__nO=this.__gW=null;}
});}
)();
...
,apply:j,event:m,nullable:true}},members:{__mi:null,__mj:false,__mk:null,_bindableProperties:[k,c,b,a,f,l],execute:function(){var o=this.getCommand();if(o){if(this.__mj){this.__mj=false;}
else {this.__mj=true;o.execute(this);}
;}
;this.fireEvent(h);}
,__ml:function(e){if(this.__mj){this.__mj=false;return;}
;this.__mj=true;this.execute();}
,_applyCommand:function(r,p){if(p!=null){p.removeListenerById(this.__mk);}
;if(r!=null){this.__mk=r.addListener(h,this.__ml,this);}
...
(function(){var b="-",c="PageUp",d="Escape",f="Boolean",g="+",h="qx.event.type.Data",j="_applyShortcut",k="PrintScreen",l="NumLock",m="short",n="keydown",o="5",p="8",q="execute",r="Meta",s="0",t="",u="2",v="Control",w="Shift",x="You can only specify one non modifier key!",y="3",z="Unidentified",A="/",B="Delete",C="String",D="changeEnabled",E="*",F="qx.bom.Shortcut",G="6",H="4",I="1",J="Alt",K="Not a valid key name for a shortcut: ",L="PageDown",M="Whitespaces are not allowed within shortcuts",N="_applyEnabled",O="keypress",P="7",Q="a",R="z",S="9";qx.Class.define(F,{extend:qx.core.Object,construct:function(T){qx.core.Object.call(this);this.__mm={};this.__mn=null;if(T!=null){this.setShortcut(T);}
;this.initEnabled();}
,events:{"execute":h},properties:{enabled:{init:true,check:f,event:D,apply:N},shortcut:{check:C,apply:j,nullable:true},autoRepeat:{check:f,init:false}},members:{__mm:t,__mn:t,execute:function(U){this.fireDataEvent(q,U);}
,__mo:function(event){if(this.getEnabled()&&this.__mp(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,__hJ:function(event){if(this.getEnabled()&&this.__mp(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,_applyEnabled:function(W,V){if(W){qx.event.Registration.addListener(document.documentElement,n,this.__mo,this);qx.event.Registration.addListener(document.documentElement,O,this.__hJ,this);}
else {qx.event.Registration.removeListener(document.documentElement,n,this.__mo,this);qx.event.Registration.removeListener(document.documentElement,O,this.__hJ,this);}
;}
,_applyShortcut:function(bb,Y){if(bb){if(bb.search(/[\s]+/)!=-1){var bc=M;this.error(bc);throw new Error(bc);}
;this.__mm={"Control":false,"Shift":false,"Meta":false,"Alt":false};this.__mn=null;var X;var a=[];while(bb.length>0&&X!=-1){X=bb.search(/[-+]+/);a.push((bb.length==1||X==-1)?bb:bb.substring(0,X));bb=bb.substring(X+1);}
;var ba=a.length;for(var i=0;i<ba;i++ ){var bd=this.__mr(a[i]);switch(bd){case v:case w:case r:case J:this.__mm[bd]=true;break;case z:var bc=K+a[i];this.error(bc);throw bc;default:if(this.__mn){var bc=x;this.error(bc);throw bc;}
;this.__mn=bd;};}
;}
;return true;}
,__mp:function(e){var be=this.__mn;if(!be){return false;}
;if((!this.__mm.Shift&&e.isShiftPressed())||(this.__mm.Shift&&!e.isShiftPressed())||(!this.__mm.Control&&e.isCtrlPressed())||(this.__mm.Control&&!e.isCtrlPressed())||(!this.__mm.Meta&&e.isMetaPressed())||(this.__mm.Meta&&!e.isMetaPressed())||(!this.__mm.Alt&&e.isAltPressed())||(this.__mm.Alt&&!e.isAltPressed())){return false;}
;if(be==e.getKeyIdentifier()){return true;}
;return false;}
,__mq:{esc:d,ctrl:v,print:k,del:B,pageup:c,pagedown:L,numlock:l,numpad_0:s,numpad_1:I,numpad_2:u,numpad_3:y,numpad_4:H,numpad_5:o,numpad_6:G,numpad_7:P,numpad_8:p,numpad_9:S,numpad_divide:A,numpad_multiply:E,numpad_minus:b,numpad_plus:g},__mr:function(bg){var bf=qx.event.util.Keyboard;var bh=z;if(bf.isValidKeyIdentifier(bg)){return bg;}
;if(bg.length==1&&bg>=Q&&bg<=R){return bg.toUpperCase();}
;bg=bg.toLowerCase();var bh=this.__mq[bg]||qx.lang.String.firstUp(bg);if(bf.isValidKeyIdentifier(bh)){return bh;}
else {return z;}
;}
,toString:function(){var bj=this.__mn;var bk=[];for(var bi in this.__mm){if(this.__mm[bi]){bk.push(qx.locale.Key.getKeyName(m,bi));}
;}
;if(bj){bk.push(qx.locale.Key.getKeyName(m,bj));}
;return bk.join(g);}
},destruct:function(){this.setEnabled(false);this.__mm=this.__mn=null;}
});}
)();
...
,getSortedSelection:function(){var bU=this.getSelectables();var bT=qx.lang.Object.getValues(this.__gD);bT.sort(function(a,b){return bU.indexOf(a)-bU.indexOf(b);}
);return bT;}
,isItemSelected:function(bV){var bW=this._selectableToHashCode(bV);return this.__gD[bW]!==undefined;}
...
;this._fireChange();}
,_setLeadItem:function(cb){var ca=this.__nB;if(ca!==null){this._styleSelectable(ca,u,false);}
;if(cb!==null){this._styleSelectable(cb,u,true);}
;this.__nB=cb;}
,getLeadItem:function(){return this.__nB;}
,_setAnchorItem:function(cd){var cc=this.__nC;if(cc!=null){this._styleSelectable(cc,I,false);}
...
;this._fireChange();}
,handlePointerOver:function(event){if(this.__nE!=null&&this.__nE!=this._getScroll().top){this.__nE=null;return;}
...
;var cD=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cA=event.isShiftPressed();if(this.isItemSelected(cB)&&!cA&&!cD&&!this.getDrag()){this.__nD=cB;this._userInteraction=false;return;}
else {this.__nD=null;}
;this._scrollItemIntoView(cB);var cC=this.getMode();if(this.getDrag()&&cC!==bu&&cC!==h&&!cA&&!cD&&event.getPointerType()==F){this._setAnchorItem(cB);this._setLeadItem(cB);this.__nr=this._getLocation();this.__no=this._getScroll();this.__ns=event.getDocumentLeft()+this.__no.left;this.__nt=event.getDocumentTop()+this.__no.top;this.__nu=true;this._capture();}
;this._fireChange(bD);this._userInteraction=false;}
,handleTap:function(event){this._userInteraction=true;var cH=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cE=event.isShiftPressed();if(!cH&&!cE&&this.__nD!=null){this._userInteraction=false;var cF=this._getSelectableFromPointerEvent(event);if(cF===null||!this.isItemSelected(cF)){return;}
;}
...
,handlePointerMove:function(event){if(!this.__nu){return;}
;this.__nv=event.getDocumentLeft();this.__nw=event.getDocumentTop();this._userInteraction=true;var cI=this.__nv+this.__no.left;if(cI>this.__ns){this.__nx=1;}
else if(cI<this.__ns){this.__nx=-1;}
else {this.__nx=0;}
;var cJ=this.__nw+this.__no.top;if(cJ>this.__nt){this.__ny=1;}
else if(cJ<this.__nt){this.__ny=-1;}
else {this.__ny=0;}
;var location=this.__nr;if(this.__nv<location.left){this.__nl=this.__nv-location.left;}
...
;if(!this.__nn){this.__nn=new qx.event.Timer(100);this.__nn.addListener(G,this._onInterval,this);}
;this.__nn.start();this._autoSelect();event.stopPropagation();this._userInteraction=false;}
...
;if(this.__nz){this._fireChange(bD);}
;delete this.__nu;delete this.__np;delete this.__nq;this._releaseCapture();if(this.__nn){this.__nn.stop();}
;}
,_onInterval:function(e){this._scrollBy(this.__nl,this.__nm);this.__no=this._getScroll();this._autoSelect();}
,_autoSelect:function(){var cT=this._getDimension();var cM=Math.max(0,Math.min(this.__nv-this.__nr.left,cT.width))+this.__no.left;var cL=Math.max(0,Math.min(this.__nw-this.__nr.top,cT.height))+this.__no.top;if(this.__np===cM&&this.__nq===cL){return;}
;this.__np=cM;this.__nq=cL;var cQ=this._getAnchorItem();var cO=cQ;var cR=this.__nx;var cU,cN;while(cR!==0){cU=cR>0?this._getRelatedSelectable(cO,t):this._getRelatedSelectable(cO,W);if(cU!==null){cN=this._getSelectableLocationX(cU);if((cR>0&&cN.left<=cM)||(cR<0&&cN.right>=cM)){cO=cU;continue;}
;}
;break;}
;var cS=this.__ny;var cV,cP;while(cS!==0){cV=cS>0?this._getRelatedSelectable(cO,z):this._getRelatedSelectable(cO,bB);if(cV!==null){cP=this._getSelectableLocationY(cV);if((cS>0&&cP.top<=cL)||(cS<0&&cP.bottom>=cL)){cO=cV;continue;}
;}
;break;}
...
;this._fireChange(bw);}
,__nF:{Home:1,Down:1,Right:1,PageDown:1,End:1,Up:1,Left:1,PageUp:1},handleKeyPress:function(event){this._userInteraction=true;var dd,dc;var df=event.getKeyIdentifier();var de=this.getMode();var cY=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var da=event.isShiftPressed();var db=false;if(df===A&&cY){if(de!==bu&&de!==h){this._selectAllItems();db=true;}
;}
...
;db=true;}
;}
else if(this.__nF[df]){db=true;if(de===bu||de==h){dd=this._getSelectedItem();}
...
;}
;break;};this.__nE=this._getScroll().top;this._scrollItemIntoView(dc);}
;}
...
;}
,_clearSelection:function(){var di=this.__gD;for(var dj in di){this._removeFromSelection(di[dj]);}
;this.__gD={};}
,_selectItemRange:function(dk,dp,dm){var dq=this._getSelectableRange(dk,dp);if(!dm){var dn=this.__gD;var dr=this.__nG(dq);for(var dl in dn){if(!dr[dl]){this._removeFromSelection(dn[dl]);}
;}
;}
...
;}
,__nG:function(dw){var dx={};var dv;for(var i=0,l=dw.length;i<l;i++ ){dv=dw[i];dx[this._selectableToHashCode(dv)]=dv;}
;return dx;}
,_getSelectedItem:function(){for(var dy in this.__gD){return this.__gD[dy];}
;return null;}
,_setSelectedItem:function(dA){if(this._isSelectable(dA)){var dz=this.__gD;var dB=this._selectableToHashCode(dA);if(!dz[dB]||(dz.length>=2)){this._clearSelection();this._addToSelection(dA);}
;}
;}
,_addToSelection:function(dC){var dD=this._selectableToHashCode(dC);if(this.__gD[dD]==null&&this._isSelectable(dC)){this.__gD[dD]=dC;this._styleSelectable(dC,R,true);this.__nz=true;}
;}
,_toggleInSelection:function(dE){var dF=this._selectableToHashCode(dE);if(this.__gD[dF]==null){this.__gD[dF]=dE;this._styleSelectable(dE,R,true);}
else {delete this.__gD[dF];this._styleSelectable(dE,R,false);}
;this.__nz=true;}
,_removeFromSelection:function(dG){var dH=this._selectableToHashCode(dG);if(this.__gD[dH]!=null){delete this.__gD[dH];this._styleSelectable(dG,R,false);this.__nz=true;}
;}
...
;}
;var dP=dK[0];var dI=dO;var dM=this.__gD;for(var dN in dM){if(dJ[dN]){delete dJ[dN];}
...
;if(!dL){return false;}
;this._scrollItemIntoView(dI);this._setLeadItem(dP);this._setAnchorItem(dP);this.__nz=true;this._fireChange();}
,_fireChange:function(dQ){if(this.__nz){this.__nA=dQ||null;this.fireDataEvent(bA,this.getSelection());delete this.__nz;}
;}
...
;return dS;}
;return null;}
},destruct:function(){this._disposeObjects(U);this.__gD=this.__nD=this.__nC=null;this.__nB=null;}
});}
)();
(function(){var a="vertical",b="under",c="above",d="qx.ui.core.selection.Widget",e="left",f="right";qx.Class.define(d,{extend:qx.ui.core.selection.Abstract,construct:function(g){qx.ui.core.selection.Abstract.call(this);this.__ms=g;}
,members:{__ms:null,_isSelectable:function(h){return this._isItemSelectable(h)&&h.getLayoutParent()===this.__ms;}
,_selectableToHashCode:function(j){return j.$$hash;}
,_styleSelectable:function(k,m,n){n?k.addState(m):k.removeState(m);}
,_capture:function(){this.__ms.capture();}
,_releaseCapture:function(){this.__ms.releaseCapture();}
,_isItemSelectable:function(o){if(this._userInteraction){return o.isVisible()&&o.isEnabled();}
else {return o.isVisible();}
;}
,_getWidget:function(){return this.__ms;}
,_getLocation:function(){var p=this.__ms.getContentElement().getDomElement();return p?qx.bom.element.Location.get(p):null;}
,_getDimension:function(){return this.__ms.getInnerSize();}
...
,_scrollItemIntoView:function(w){this.__ms.scrollChildIntoView(w);}
,getSelectables:function(x){var y=false;if(!x){y=this._userInteraction;this._userInteraction=true;}
;var B=this.__ms.getChildren();var z=[];var A;for(var i=0,l=B.length;i<l;i++ ){A=B[i];if(this._isItemSelectable(A)){z.push(A);}
;}
;this._userInteraction=y;return z;}
,_getSelectableRange:function(C,D){if(C===D){return [C];}
;var H=this.__ms.getChildren();var E=[];var G=false;var F;for(var i=0,l=H.length;i<l;i++ ){F=H[i];if(F===C||F===D){if(G){E.push(F);break;}
else {G=true;}
;}
;if(G&&this._isItemSelectable(F)){E.push(F);}
;}
;return E;}
,_getFirstSelectable:function(){var I=this.__ms.getChildren();for(var i=0,l=I.length;i<l;i++ ){if(this._isItemSelectable(I[i])){return I[i];}
;}
;return null;}
,_getLastSelectable:function(){var J=this.__ms.getChildren();for(var i=J.length-1;i>0;i-- ){if(this._isItemSelectable(J[i])){return J[i];}
;}
;return null;}
,_getRelatedSelectable:function(N,O){var L=this.__ms.getOrientation()===a;var P=this.__ms.getChildren();var K=P.indexOf(N);var M;if((L&&O===c)||(!L&&O===e)){for(var i=K-1;i>=0;i-- ){M=P[i];if(this._isItemSelectable(M)){return M;}
;}
;}
...
;this.__gz.removeItem(I);}
,selectRange:function(J,K){this.__gz.selectItemRange(J,K);}
,resetSelection:function(){this.__gz.clearSelection();}
...
else {var M=this.getSelection();if(!qx.lang.Array.equals(M,L)){this.__gz.replaceSelection(L);}
;}
;}
,getSelection:function(){return this.__gz.getSelection();}
,getSortedSelection:function(){return this.__gz.getSortedSelection();}
,isSelectionEmpty:function(){return this.__gz.isSelectionEmpty();}
,getSelectionContext:function(){return this.__gz.getSelectionContext();}
,_getManager:function(){return this.__gz;}
,getSelectables:function(N){return this.__gz.getSelectables(N);}
,invertSelection:function(){this.__gz.invertSelection();}
,_getLeadItem:function(){var O=this.__gz.getMode();if(O===q||O===b){return this.__gz.getSelectedItem();}
else {return this.__gz.getLeadItem();}
;}
,_applySelectionMode:function(Q,P){this.__gz.setMode(Q);}
,_applyDragSelection:function(S,R){this.__gz.setDrag(S);}
,_applyQuickSelection:function(U,T){this.__gz.setQuick(U);}
,_onSelectionChange:function(e){this.fireDataEvent(k,e.getData());}
},destruct:function(){this._disposeObjects(u);}
});}
)();
(function(){var a="",b="qx.ui.form.MForm",c="Boolean",d="_applyValid",f="changeLocale",g="changeRequired",h="changeValid",i="qx.dynlocale",j="changeInvalidMessage",k="String",l="invalid";qx.Mixin.define(b,{construct:function(){if(qx.core.Environment.get(i)){qx.locale.Manager.getInstance().addListener(f,this.__mI,this);}
;}
...
,__mI:qx.core.Environment.select(i,{"true":function(e){var o=this.getInvalidMessage();if(o&&o.translate){this.setInvalidMessage(o.translate());}
;var p=this.getRequiredInvalidMessage();if(p&&p.translate){this.setRequiredInvalidMessage(p.translate());}
;}
,"false":null})},destruct:function(){if(qx.core.Environment.get(i)){qx.locale.Manager.getInstance().removeListener(f,this.__mI,this);}
;}
});}
)();
...
(function(){var a="mouse",b="scrollbar-y",c="qx.ui.core.scroll.MRoll",d="pointerdown",f="scrollbar-x",g="roll";qx.Mixin.define(c,{members:{__mW:null,_addRollHandling:function(){this.addListener(g,this._onRoll,this);this.addListener(d,this._onPointerDownForRoll,this);}
...
,_onPointerDownForRoll:function(e){this.__mW=e.getPointerId();}
,_onRoll:function(e){if(e.getPointerType()==a){return;}
;if(this.__mW&&e.getMomentum()){qx.event.Registration.getManager(e.getOriginalTarget()).getHandler(qx.event.handler.Gesture).gestureCancel(this.__mW);e.stopMomentum();this.__mW=null;return;}
;this.__mW=null;var k=this._isChildControlVisible(f);var l=this._isChildControlVisible(b);var q=l?this.getChildControl(b,true):null;var p=k?this.getChildControl(f,true):null;var m=e.getDelta().y;var h=e.getDelta().x;var j=!l;var o=!k;if(q){if(m!==0){q.scrollBy(parseInt(m,10));}
...
;r.addListener(k,this.__nb,this);r.addListener(i,this.__nc,this);this.__mX=[n,o];this.__mY=[m,g];}
,properties:{dragScrollThresholdX:{check:p,init:30},dragScrollThresholdY:{check:p,init:30},dragScrollSlowDownFactor:{check:b,init:0.1}},members:{__na:null,__mX:null,__mY:null,_findScrollableParent:function(s){var t=s;if(t===null){return null;}
...
;}
,_getAxis:function(A){if(this.__mX.indexOf(A)!==-1){return a;}
else if(this.__mY.indexOf(A)!==-1){return q;}
else {throw new Error(l+A+f);}
;}
,_getThresholdByEdgeType:function(B){if(this.__mX.indexOf(B)!==-1){return this.getDragScrollThresholdX();}
else if(this.__mY.indexOf(B)!==-1){return this.getDragScrollThresholdY();}
;}
...
;var P=S.getBounds(),O=T===a?P.width:P.height,R=this._calculateScrollAmount(O,Q);if(this._isScrollbarExceedingMaxPos(S,T,R)){this.__na.stop();}
;S.scrollBy(R);}
,__nb:function(e){if(this.__na){this.__na.stop();}
...
;bd=this._getAxis(X);if(this._isScrollbarVisible(U,bd)){bb=this._calculateThresholdExceedance(bc[X],this._getThresholdByEdgeType(X));if(this.__na){this.__na.dispose();}
;this.__na=new qx.event.Timer(50);this.__na.addListener(h,function(be,bg,bf){this._scrollBy(be,bg,bf);}
.bind(this,U,bd,bb));this.__na.start();e.stopPropagation();return;}
else {U=this._findScrollableParent(U);}
;}
;}
,__nc:function(e){if(this.__na){this.__na.stop();}
;}
...
(function(){var a="qx.ui.core.scroll.NativeScrollBar",b="px",c="PositiveNumber",d="frame",f="end",g="box-sizing",h="Integer",i="track",j="hidden",k="content-box",l="pointerup",m="vertical",n="_applyOrientation",o="scrollAnimationEnd",p="appear",q="pointerdown",r="PositiveInteger",s='qx.event.type.Event',t="mshtml",u="engine.name",v="horizontal",w="Number",x="__ne",y="_applyPosition",z="scrollbar",A="_applyMaximum",B="pointermove",C="native",D="scroll";qx.Class.define(a,{extend:qx.ui.core.Widget,implement:qx.ui.core.scroll.IScrollBar,construct:function(E){qx.ui.core.Widget.call(this);this.addState(C);this.getContentElement().addListener(D,this._onScroll,this);this.addListener(q,this._stopPropagation,this);this.addListener(l,this._stopPropagation,this);this.addListener(B,this._stopPropagation,this);this.addListener(p,this._onAppear,this);this.getContentElement().add(this._getScrollPaneElement());this.getContentElement().setStyle(g,k);if(E!=null){this.setOrientation(E);}
else {this.initOrientation();}
;this.addListener(i,function(e){e.stopPropagation();}
,this);}
,events:{scrollAnimationEnd:s},properties:{appearance:{refine:true,init:z},orientation:{check:[v,m],init:v,apply:n},maximum:{check:r,apply:A,init:100},position:{check:w,init:0,apply:y,event:D},singleStep:{check:h,init:20},knobFactor:{check:c,nullable:true}},members:{__nd:null,__ne:null,__ko:null,__nf:null,_getScrollPaneElement:function(){if(!this.__ne){this.__ne=new qx.html.Element();}
;return this.__ne;}
...
,_getContentHint:function(){var J=qx.bom.element.Scroll.getScrollbarWidth();return {width:this.__nd?100:J,maxWidth:this.__nd?null:J,minWidth:this.__nd?null:J,height:this.__nd?J:100,maxHeight:this.__nd?J:null,minHeight:this.__nd?J:null};}
...
,_applyPosition:function(N){var content=this.getContentElement();if(this.__nd){content.scrollToX(N);}
else {content.scrollToY(N);}
;}
,_applyOrientation:function(Q,O){var P=this.__nd=Q===v;this.set({allowGrowX:P,allowShrinkX:P,allowGrowY:!P,allowShrinkY:!P});if(P){this.replaceState(m,v);}
...
,_updateScrollBar:function(){var S=this.__nd;var T=this.getBounds();if(!T){return;}
...
,scrollTo:function(W,V){this.stopScrollAnimation();if(V){var X=this.getPosition();this.__nf=new qx.bom.AnimationFrame();this.__nf.on(d,function(ba){var Y=parseInt(ba/V*(W-X)+X);this.updatePosition(Y);}
,this);this.__nf.on(f,function(){this.setPosition(Math.max(0,Math.min(this.getMaximum(),W)));this.__nf=null;this.fireEvent(o);}
,this);this.__nf.startSequence(V);}
...
,stopScrollAnimation:function(){if(this.__nf){this.__nf.cancelSequence();this.__nf=null;}
;}
,_onScroll:function(e){var bh=this.getContentElement();var bi=this.__nd?bh.getScrollX():bh.getScrollY();this.setPosition(bi);}
...
;var bw=this.__nd;var bu=this.getChildControl(P);var bv=bw?G:L;var bA=bw?e.getDocumentLeft():e.getDocumentTop();var bC=this.getDecorator();bC=qx.theme.manager.Decoration.getInstance().resolve(bC);if(bw){var bz=bC?bC.getInsets().left:0;var by=(this.getPaddingLeft()||0)+bz;}
...
,_onPointerUp:function(e){if(this.__xV){this.releaseCapture();delete this.__xV;this.__yb.stop();this._fireValue();delete this.__xW;this.getChildControl(P).removeState(c);if(e.getType()===B){var bE;var bF;var bD;if(this.__nd){bE=e.getDocumentLeft()-(this._valueToPosition(this.getValue())+this.__xS);bD=qx.bom.element.Location.get(this.getContentElement().getDomElement())[L];bF=e.getDocumentTop()-(bD+this.getChildControl(P).getBounds().top);}
...
;}
,_onPointerMove:function(e){if(this.__xV){var bH=this.__nd?e.getDocumentLeft():e.getDocumentTop();var bG=bH-this.__xW;this.slideTo(this._positionToValue(bG));}
...
;this.slideTo(bI);}
,_onUpdate:function(e){var bL=this.getInnerSize();var bM=this.getChildControl(P).getBounds();var bK=this.__nd?E:x;this._updateKnobSize();this.__ye=bL[bK]-bM[bK];this.__xU=bM[bK];this._updateKnobPosition();}
,__nd:false,__ye:0,__yf:function(e){var bN=this.__nd;var bU=bN?e.getDocumentLeft():e.getDocumentTop();var bX=this.__xS;var bO=this.__xT;var bY=this.__xU;var bV=bU-bX;if(bU>=bO){bV-=bY;}
...
,_setKnobPosition:function(ck){var ci=this.getChildControl(P);var cj=this.getDecorator();cj=qx.theme.manager.Decoration.getInstance().resolve(cj);var content=ci.getContentElement();if(this.__nd){if(cj&&cj.getPadding()){ck+=cj.getPadding().left;}
...
;if(this.__nd){this.getChildControl(P).setWidth(Math.round(cm*cl.width));}
...
,stopSlideAnimation:function(){if(this.__nf){this.__nf.cancelSequence();this.__nf=null;}
;}
...
;return cw;}
,__yh:function(cy,cx){cy=this.__yg(cy);var cz=this.getValue();this.__nf=new qx.bom.AnimationFrame();this.__nf.on(i,function(cA){this.setValue(parseInt(cA/cx*(cy-cz)+cz));}
,this);this.__nf.on(j,function(){this.setValue(cy);this.__nf=null;this.fireEvent(O);}
,this);this.__nf.startSequence(cx);}
,_applyOrientation:function(cD,cC){var cB=this.getChildControl(P);this.__nd=cD===N;if(this.__nd){this.removeState(C);cB.removeState(C);this.addState(N);cB.addState(N);cB.setLayoutProperties({top:0,right:null,bottom:0});}
...
else {if(this.__nd){this.getChildControl(P).resetWidth();}
...
,events:{"execute":j,"press":j,"release":j},properties:{interval:{check:m,init:100},firstInterval:{check:m,init:500},minTimer:{check:m,init:20},timerDecrease:{check:m,init:2}},members:{__ob:null,__eg:null,press:function(){if(this.isEnabled()){if(!this.hasState(h)){this.__oc();}
;this.removeState(l);this.addState(h);}
;}
,release:function(q){if(!this.isEnabled()){return;}
;if(this.hasState(h)){if(!this.__ob){this.execute();}
;}
;this.removeState(h);this.removeState(l);this.__od();}
...
,_onPointerUp:function(e){this.releaseCapture();if(!this.hasState(l)){this.addState(c);if(this.hasState(h)&&!this.__ob){this.execute();}
;}
;this.__od();e.stopPropagation();}
,_onTap:function(e){}
,_onKeyUp:function(e){switch(e.getKeyIdentifier()){case g:case k:if(this.hasState(h)){if(!this.__ob){this.execute();}
;this.removeState(h);this.removeState(l);e.stopPropagation();this.__od();}
;};}
,_onKeyDown:function(e){switch(e.getKeyIdentifier()){case g:case k:this.removeState(l);this.addState(h);e.stopPropagation();this.__oc();};}
,_onInterval:function(e){this.__ob=true;this.fireEvent(n);}
,__oc:function(){this.fireEvent(a);this.__ob=false;this.__eg.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();this.removeState(l);this.addState(h);}
,__od:function(){this.fireEvent(f);this.__eg.stop();this.removeState(l);this.removeState(h);}
...
,events:{"interval":d},properties:{interval:{check:a,init:100},firstInterval:{check:a,init:500},minimum:{check:a,init:20},decrease:{check:a,init:2}},members:{__eg:null,__oe:null,start:function(){this.__eg.setInterval(this.getFirstInterval());this.__eg.start();}
,stop:function(){this.__eg.stop();this.__oe=null;}
,_onInterval:function(){this.__eg.stop();if(this.__oe==null){this.__oe=this.getInterval();}
;this.__oe=Math.max(this.getMinimum(),this.__oe-this.getDecrease());this.__eg.setInterval(this.__oe);this.__eg.start();this.fireEvent(c);}
},destruct:function(){this._disposeObjects(b);}
});}
)();
...
,events:{update:m,scrollAnimationEnd:m},properties:{scrollX:{check:j,apply:g,event:f,init:0},scrollY:{check:l,apply:h,event:b,init:0}},members:{__ng:null,add:function(r){var s=this._getChildren()[0];if(s){this._remove(s);s.removeListener(a,this._onUpdate,this);}
...
;this.stopScrollAnimation();if(L){var N=this.getScrollX();this.__ng=new qx.bom.AnimationFrame();this.__ng.on(o,function(){this.setScrollX(O);this.__ng=null;this.fireEvent(c);}
,this);this.__ng.on(i,function(Q){var P=parseInt(Q/L*(O-N)+N);this.setScrollX(P);}
,this);this.__ng.startSequence(L);}
else {this.setScrollX(O);}
;}
,scrollToY:function(U,R){var S=this.getScrollMaxY();if(U<0){U=0;}
else if(U>S){U=S;}
;this.stopScrollAnimation();if(R){var T=this.getScrollY();this.__ng=new qx.bom.AnimationFrame();this.__ng.on(o,function(){this.setScrollY(U);this.__ng=null;this.fireEvent(c);}
,this);this.__ng.on(i,function(W){var V=parseInt(W/R*(U-T)+T);this.setScrollY(V);}
,this);this.__ng.startSequence(R);}
...
,stopScrollAnimation:function(){if(this.__ng){this.__ng.cancelSequence();this.__ng=null;}
;}
...
(function(){var a="qx.ui.form.List",b="text",c="Boolean",d="one",f="addChildWidget",g="_applySpacing",h="Enter",j="Integer",k="action",m="keyinput",n="qx.event.type.Data",o="addItem",p="vertical",q="removeChildWidget",r="",s="_applyOrientation",t="single",u="keypress",v="list",w="horizontal",x="label",y="__nU",z="pane",A="removeItem";qx.Class.define(a,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MMultiSelectionHandling,qx.ui.form.MForm,qx.ui.form.MModelSelection],construct:function(B){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__nU=this._createListItemContainer();this.__nU.addListener(f,this._onAddChild,this);this.__nU.addListener(q,this._onRemoveChild,this);this.getChildControl(z).add(this.__nU);if(B){this.setOrientation(w);}
else {this.initOrientation();}
;this.addListener(u,this._onKeyPress);this.addListener(m,this._onKeyInput);this.__yt=r;}
,events:{addItem:n,removeItem:n},properties:{appearance:{refine:true,init:v},focusable:{refine:true,init:true},orientation:{check:[w,p],init:p,apply:s},spacing:{check:j,init:0,apply:g,themeable:true},enableInlineFind:{check:c,init:true}},members:{__yt:null,__yu:null,__nU:null,SELECTION_MANAGER:qx.ui.core.selection.ScrollArea,getChildrenContainer:function(){return this.__nU;}
...
,_applyOrientation:function(F,C){var D=F===w;var E=D?new qx.ui.layout.HBox():new qx.ui.layout.VBox();var content=this.__nU;content.setLayout(E);content.setAllowGrowX(!D);content.setAllowGrowY(D);this._applySpacing(this.getSpacing());}
,_applySpacing:function(H,G){this.__nU.getLayout().setSpacing(H);}
...
;if(((new Date).valueOf()-this.__yu)>1000){this.__yt=r;}
;this.__yt+=e.getChar();var K=this.findItemByLabelFuzzy(this.__yt);if(K){this.setSelection([K]);}
;this.__yu=(new Date).valueOf();}
...
;if(this.getSelection()==null){this.__yv=new qx.data.Array();this.setSelection(this.__yv);}
;}
,properties:{selection:{check:b,event:f,apply:d,init:null}},events:{"changeSelection":h},members:{_modifingSelection:0,__yw:null,__yx:null,__yv:null,_applySelection:function(k,j){if(this.__yx!=undefined&&j!=undefined){j.removeListenerById(this.__yx);}
;this.__yx=k.addListener(a,this.__yy,this);this._updateSelection();}
,__yy:function(){this._updateSelection();}
,_changeTargetSelection:function(){if(this.getTarget()==null){return;}
;if(!this.__yz()&&!this.__yA()){return;}
;if(this._inSelectionModification()){return;}
;var m=this.getTarget().getSelection();var l=this.getSelection();if(l==null){l=new qx.data.Array();this.__yv=l;this.setSelection(l);}
...
,_addChangeTargetListener:function(p,o){if(this.__yw!=undefined&&o!=undefined){o.removeListenerById(this.__yw);}
;if(p!=null){if(this.__yz()||this.__yA()){this.__yw=p.addListener(f,this._changeTargetSelection,this);}
;}
;}
,_updateSelection:function(){if(!this.getTarget()){return;}
;this._startSelectionModification();if(this.__yz()){var s=[];for(var i=0;i<this.getSelection().length;i++ ){var t=this.getSelection().getItem(i);var u=this.__yB(t);if(u!=null){s.push(u);}
;}
...
;}
;}
else if(this.__yA()){var r=this.getSelection().getItem(this.getSelection().length-1);if(r!==undefined){this.__tk(r);this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();}
else {this.getTarget().resetSelection();}
;}
;this._endSelectionModification();}
,__yz:function(){var v=this.getTarget().constructor;return qx.Class.implementsInterface(v,qx.ui.core.IMultiSelection);}
,__yA:function(){var w=this.getTarget().constructor;return qx.Class.implementsInterface(w,qx.ui.core.ISingleSelection);}
,__tk:function(x){var y=this.__yB(x);if(y==null){return;}
;if(this.__yz()){this.getTarget().addToSelection(y);}
else if(this.__yA()){this.getTarget().setSelection([y]);}
;}
,__yB:function(z){var A=this.getTarget().getSelectables(true);for(var i=0;i<A.length;i++ ){if(A[i].getModel()==z){return A[i];}
;}
;return null;}
...
},destruct:function(){if(this.__yv){this.__yv.dispose();}
;}
});}
)();
...
(function(){var a="changeModel",b="_applyLabelOptions",c="_applyLabelPath",d="changeTarget",e="_applyModel",f="change",g="icon",h="ReverseBindingId",k="qx.data.controller.List",m="_applyIconPath",n="BindingId",o="_applyDelegate",p="",q="changeDelegate",r="_applyTarget",s="]",t="qx.data.IListData",u="model[",v="model",w="label",x="String",y="_applyIconOptions",z=".";qx.Class.define(k,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(A,B,C){qx.core.Object.call(this);this.__xj=[];this.__yC=[];this.__yD=[];this.__yE={};if(C!=null){this.setLabelPath(C);}
;if(A!=null){this.setModel(A);}
;if(B!=null){this.setTarget(B);}
;}
,properties:{model:{check:t,apply:e,event:a,nullable:true,dereference:true},target:{apply:r,event:d,nullable:true,init:null,dereference:true},labelPath:{check:x,apply:c,nullable:true},iconPath:{check:x,apply:m,nullable:true},labelOptions:{apply:b,nullable:true},iconOptions:{apply:y,nullable:true},delegate:{apply:o,event:q,init:null,nullable:true}},members:{__yF:null,__xj:null,__yE:null,__yC:null,__yD:null,__yG:null,__yH:null,update:function(){this.__yJ();this.__yN();this._updateSelection();}
,_applyDelegate:function(E,D){this._setConfigureItem(E,D);this._setFilter(E,D);this._setCreateItem(E,D);this._setBindItem(E,D);}
,_applyIconOptions:function(G,F){this.__yN();}
,_applyLabelOptions:function(I,H){this.__yN();}
,_applyIconPath:function(K,J){this.__yN();}
,_applyLabelPath:function(M,L){this.__yN();}
,_applyModel:function(P,N){if(N!=undefined){if(this.__yF!=undefined){N.removeListenerById(this.__yF);}
;}
;if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();}
;if(P!=null){this.__yF=P.addListener(f,this.__yI,this);this.__xq();this.__yJ();if(N==null){this._changeTargetSelection();}
else {this.__yG=true;qx.ui.core.queue.Widget.add(this);}
;}
else {var O=this.getTarget();if(O!=null){var length=O.getChildren().length;for(var i=0;i<length;i++ ){this.__yM();}
;}
;}
;}
...
;if(S!=null){if(this.getModel()!=null){for(var i=0;i<this.__xj.length;i++ ){this.__yL(this.__yP(i));}
;}
;}
;}
,__yI:function(){this.__yH=true;qx.ui.core.queue.Widget.add(this);if(this.__xj.length!=this.getModel().getLength()){this.update();}
;}
,syncWidget:function(){if(this.__yG){this._changeTargetSelection();}
;if(this.__yH){this._updateSelection();}
;this.__yH=this.__yG=null;}
,__yJ:function(){if(this.getTarget()==null){return;}
;this.__xq();var U=this.__xj.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++ ){this.__yL(this.__yP(j));}
;}
else if(U<T){for(var j=T;j>U;j-- ){this.__yM();}
;}
;this.__yG=true;qx.ui.core.queue.Widget.add(this);}
,__yK:function(){var V=this.getModel();if(V!=null){V.removeListenerById(this.__yF);this.__yF=V.addListener(f,this.__yI,this);}
;}
...
;return W;}
,__yL:function(Y){var ba=this._createItem();this._bindListItem(ba,Y);this.getTarget().add(ba);}
,__yM:function(){this._startSelectionModification();var bc=this.getTarget().getChildren();var bb=bc.length-1;var bd=bc[bb];this._removeBindingsFrom(bd);this.getTarget().removeAt(bb);bd.destroy();this._endSelectionModification();}
...
;}
,bindProperty:function(bo,br,bp,bn,bm){if(bp!=null){var bp=qx.lang.Object.clone(bp);this.__yE[br]=bp.onUpdate;delete bp.onUpdate;}
else {bp={};this.__yE[br]=null;}
;bp.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,bm);bp.ignoreConverter=v;var bq=u+bm+s;if(bo!=null&&bo!=p){bq+=z+bo;}
;var bs=this.bind(bq,bn,br,bp);bn.setUserData(br+n,bs);if(!qx.lang.Array.contains(this.__yC,br)){this.__yC.push(br);}
;}
,bindPropertyReverse:function(by,bv,bw,bx,bt){var bu=u+bt+s;if(by!=null&&by!=p){bu+=z+by;}
;var bz=bx.bind(bv,this,bu,bw);bx.setUserData(by+h,bz);if(!qx.lang.Array.contains(this.__yD,by)){this.__yD.push(by);}
;}
,_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;}
;for(var i=0;i<this.__yC.length;i++ ){if(this.__yE[this.__yC[i]]!=null){this.__yE[this.__yC[i]]();}
;}
;}
,_removeBindingsFrom:function(bD){for(var i=0;i<this.__yC.length;i++ ){var bE=bD.getUserData(this.__yC[i]+n);if(bE!=null){this.removeBinding(bE);}
;}
;for(var i=0;i<this.__yD.length;i++ ){var bE=bD.getUserData(this.__yD[i]+h);if(bE!=null){bD.removeBinding(bE);}
;}
;}
,__yN:function(){if(this.getTarget()==null||this.getModel()==null){return;}
;var bF=this.getTarget().getChildren();for(var i=0;i<bF.length;i++ ){this._removeBindingsFrom(bF[i]);this._bindListItem(bF[i],this.__yP(i));}
;this.__yK();}
...
,_setFilter:function(bR,bP){if((bR==null||bR.filter==null)&&(bP!=null&&bP.filter!=null)){this.__yO();}
...
;var bQ=this.__xj;this.__xq();if(bQ.length>this.__xj.length){for(var j=bQ.length;j>this.__xj.length;j-- ){this.getTarget().removeAt(j-1).destroy();}
;}
else if(bQ.length<this.__xj.length){for(var j=bQ.length;j<this.__xj.length;j++ ){var bT=this._createItem();this.getTarget().add(bT);}
;}
;var bS=this.getTarget().getChildren();for(var i=0;i<bS.length;i++ ){this._bindListItem(bS[i],this.__yP(i));}
;this.__yK();this._endSelectionModification();this._updateSelection();}
,__yO:function(){this.__xq();this.__yJ();this.__yN();this.__yH=true;qx.ui.core.queue.Widget.add(this);}
,__xq:function(){var bW=this.getModel();if(bW==null){return;}
;var bX=this.getDelegate();if(bX!=null){var bV=bX.filter;}
;this.__xj=[];for(var i=0;i<bW.getLength();i++ ){if(bV==null||bV(bW.getItem(i))){this.__xj.push(i);}
;}
;}
,__yP:function(bY){return this.__xj[bY];}
},destruct:function(){this.__xj=this.__yE=this.__yC=null;this.__yD=null;qx.ui.core.queue.Widget.remove(this);}
});}
)();
...
(function(){var a="changeModel",b="application/json",c="json",d="aborted",e="loaded",f="qx.event.type.Data",g="error",h="__ox",i="fail",j="receiving",k="queued",l="changeState",m="success",n="String",o="changePhase",p="sending",q="_marshaler",r="completed",s="failed",t="qx.data.store.Json",u="configured",v="changeUrl",w="timeout",x="_applyUrl";qx.Class.define(t,{extend:qx.core.Object,construct:function(y,z){qx.core.Object.call(this);this._marshaler=new qx.data.marshal.Json(z);this._delegate=z;if(y!=null){this.setUrl(y);}
;}
,events:{"loaded":f,"error":f},properties:{model:{nullable:true,event:a},state:{check:[u,k,p,j,r,d,w,s],init:u,event:l},url:{check:n,apply:x,event:v,nullable:true}},members:{_marshaler:null,_delegate:null,__ox:null,_applyUrl:function(B,A){if(B!=null){B=qx.util.AliasManager.getInstance().resolve(B);B=qx.util.ResourceManager.getInstance().toUri(B);this._createRequest(B);}
;}
,_getRequest:function(){return this.__ox;}
,_setRequest:function(C){this.__ox=C;}
,_createRequest:function(D){if(this.__ox){this.__ox.dispose();this.__ox=null;}
...
;this.fireDataEvent(e,this.getModel());if(this.__ox){this.__ox.dispose();this.__ox=null;}
;}
,reload:function(){var R=this.getUrl();if(R!=null){this._createRequest(R);}
;}
},destruct:function(){if(this.__ox!=null){this._disposeObjects(h);}
...
},members:{__tF:null,__tG:function(z){return Object.keys(z).sort().join(a);}
,toClass:function(A,B){this.__tH(A,B,null,0);}
,__tH:function(N,I,C,K){if(!qx.lang.Type.isObject(N)||!!N.$$isString||N instanceof qx.core.Object){if(N instanceof Array||qx.Bootstrap.getClass(N)==h){for(var i=0;i<N.length;i++ ){this.__tH(N[i],I,C+o+i+m,K+1);}
;}
;return;}
;var E=this.__tG(N);if(this.__tL(E,C,K)){return;}
;for(var M in N){this.__tH(N[M],I,M,K+1);}
;if(qx.Class.isDefined(s+E)){return;}
;if(this.__tF&&this.__tF.getModelClass&&this.__tF.getModelClass(E,N,C,K)!=null){return;}
;var O={};var F={__tI:this.__tI};for(var M in N){if(this.__tF&&this.__tF.getPropertyMapping){M=this.__tF.getPropertyMapping(M,E);}
...
;if(this.__tF&&this.__tF.getValidationRule){var H=this.__tF.getValidationRule(E,M);if(H){O[M].validate=k+M;F[k+M]=H;}
;}
;}
;if(this.__tF&&this.__tF.getModelSuperClass){var L=this.__tF.getModelSuperClass(E,C,K)||qx.core.Object;}
else {var L=qx.core.Object;}
;var G=[];if(this.__tF&&this.__tF.getModelMixins){var J=this.__tF.getModelMixins(E,C,K);if(!qx.lang.Type.isArray(J)){if(J!=null){G=[J];}
;}
else {G=J;}
;}
;if(I){G.push(qx.data.marshal.MEventBubbling);}
;var D={extend:L,include:G,properties:O,members:F,destruct:this.__tJ};qx.Class.define(s+E,D);}
,__tJ:function(){var P=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var Q in P){this.__tI(this.get(P[Q].name));}
;}
,__tI:function(R){if(!(R instanceof qx.core.Object)){return;}
;if(R.isDisposed()){return;}
;R.dispose();}
,__tK:function(S,W,X,V){var Y;if(this.__tF&&this.__tF.getModelClass){Y=this.__tF.getModelClass(S,W,X,V);}
;if(Y!=null){return (new Y());}
else {var U=s+S;var T=qx.Class.getByName(U);if(!T){throw new Error(b+U+l);}
;return (new T());}
;}
,__tL:function(ba,bd,bb){var bc=this.__tF;return bc&&bc.ignore&&bc.ignore(ba,bd,bb);}
,toModel:function(be){return this.__tM(be,null,0);}
,__tM:function(bq,bg,bm){var bk=qx.lang.Type.isObject(bq);var bf=bq instanceof Array||qx.Bootstrap.getClass(bq)==h;if((!bk&&!bf)||!!bq.$$isString||bq instanceof qx.core.Object){return bq;}
else if(this.__tL(this.__tG(bq),bg,bm)){return bq;}
else if(bf){var bo=qx.data.Array;if(this.__tF&&this.__tF.getArrayClass){var bl=this.__tF.getArrayClass(bg,bm);bo=bl||bo;}
;var br=new bo();br.setAutoDisposeItems(true);for(var i=0;i<bq.length;i++ ){br.push(this.__tM(bq[i],bg+o+i+m,bm+1));}
;return br;}
else if(bk){var bh=this.__tG(bq);var bp=this.__tK(bh,bq,bg,bm);for(var bn in bq){var bi=bn;if(this.__tF&&this.__tF.getPropertyMapping){bi=this.__tF.getPropertyMapping(bn,bh);}
;var bs=bi.replace(/-|\.|\s+/g,d);if((qx.core.Environment.get(t))&&qx.core.Environment.get(e)){if(bs!=bi){this.warn(c+bn+n+bi+v);}
;}
;bi=bs;var bj=p+qx.lang.String.firstUp(bi);if(bp[bj]){bp[bj](this.__tM(bq[bn],bn,bm+1));}
;}
;return bp;}
;throw new Error(j);}
},destruct:function(){this.__tF=null;}
});}
)();
...
;this.__oh={};var P=this._transport=this._createTransport();this._setPhase(G);this.__ud=qx.lang.Function.bind(this._onReadyStateChange,this);this.__ue=qx.lang.Function.bind(this._onLoad,this);this.__uf=qx.lang.Function.bind(this._onLoadEnd,this);this.__ug=qx.lang.Function.bind(this._onAbort,this);this.__uh=qx.lang.Function.bind(this._onTimeout,this);this.__ui=qx.lang.Function.bind(this._onError,this);P.onreadystatechange=this.__ud;P.onload=this.__ue;P.onloadend=this.__uf;P.onabort=this.__ug;P.ontimeout=this.__uh;P.onerror=this.__ui;}
,events:{"readyStateChange":r,"success":r,"load":r,"loadEnd":r,"abort":r,"timeout":r,"error":r,"statusError":r,"fail":r,"changeResponse":i,"changePhase":i},properties:{url:{check:u},timeout:{check:I,nullable:true,init:0},requestData:{check:function(Q){return qx.lang.Type.isString(Q)||qx.Class.isSubClassOf(Q.constructor,qx.core.Object)||qx.lang.Type.isObject(Q)||qx.lang.Type.isArray(Q);}
,nullable:true},authentication:{check:k,nullable:true}},members:{__ud:null,__ue:null,__uf:null,__ug:null,__uh:null,__ui:null,__uj:null,__uk:null,__ul:null,__oh:null,__um:null,_transport:null,_createTransport:function(){throw new Error(E);}
...
;this.__uk=true;this.__ul=s;this._transport.abort();}
,_setRequestHeaders:function(){var X=this._transport,W=this._getAllRequestHeaders();for(var Y in W){X.setRequestHeader(Y,W[Y]);}
;}
,_getAllRequestHeaders:function(){var ba={};qx.lang.Object.mergeWith(ba,this._getConfiguredRequestHeaders());qx.lang.Object.mergeWith(ba,this.__un());qx.lang.Object.mergeWith(ba,this.__um);qx.lang.Object.mergeWith(ba,this.__oh);return ba;}
,__un:function(){var bc=this.getAuthentication(),bb={};if(bc){bc.getAuthHeaders().forEach(function(bd){bb[bd.key]=bd.value;}
);return bb;}
;}
,setRequestHeader:function(be,bf){this.__oh[be]=bf;}
,getRequestHeader:function(bg){return this.__oh[bg];}
,removeRequestHeader:function(bh){if(this.__oh[bh]){delete this.__oh[bh];}
;}
,getTransport:function(){return this._transport;}
,getReadyState:function(){return this._transport.readyState;}
,getPhase:function(){return this.__ul;}
...
,getResponse:function(){return this.__uj;}
,_setResponse:function(bl){var bk=bl;if(this.__uj!==bl){this.__uj=bl;this.fireEvent(H,qx.event.type.Data,[this.__uj,bk]);}
;}
,_onReadyStateChange:function(){var bm=this.getReadyState();if(qx.core.Environment.get(f)){this.debug(B+bm);}
;this.fireEvent(D);if(this.__uk){return;}
;if(bm===3){this._setPhase(p);}
;if(this.isDone()){this.__uo();}
;}
,__uo:function(){if(qx.core.Environment.get(f)){this.debug(A+this.getStatus());}
...
,_setPhase:function(bo){var bp=this.__ul;if(qx.core.Environment.get(m)){qx.core.Assert.assertString(bo);qx.core.Assert.assertMatch(bo,/^(unsent)|(opened)|(sent)|(loading)|(load)|(success)|(abort)|(timeout)|(statusError)$/);}
;this.__ul=bo;this.fireDataEvent(c,bo,bp);}
...
;var A=z[h+qx.lang.String.firstUp(name)]();if(qx.lang.Type.isArray(A)){var D=qx.data&&qx.data.IListData&&qx.Class.hasInterface(A&&A.constructor,qx.data.IListData);for(var i=0;i<A.length;i++ ){var F=D?A.getItem(i):A[i];E+=this.__up(name,F,C);}
;}
else if(qx.lang.Type.isDate(A)&&y!=null){E+=this.__up(name,y.format(A),C);}
else {E+=this.__up(name,A,C);}
;}
;return E.substring(0,E.length-1);}
,__up:function(name,I,G){if(I&&I.$$type==o){I=I.classname;}
...
(function(){var a="Retry open native request with method: ",b="activex",c="No XHR support available.",d="If-None-Match",f="xhr",g="If-Modified-Since",h="engine.version",i="onunload",j="GET",k="-1",l="qx.debug.io",m="HTMLDocument",n="Open native request with method: ",o="error",p="loadend",q="Blob",r="load",s="Received native readyState: ",t="abort",u=", url: ",v="String",w="browser.documentmode",x="",y="readystatechange",z="Microsoft.XMLHTTP",A="Already disposed",B="browser.version",C="opera",D="qx.bom.request.Xhr",E="on",F="timeout",G="gecko",H="If-Match",I="mshtml",J="engine.name",K="Microsoft.XMLDOM",L="file:",M="Send native request",N="FormData",O="If-Range",P="Content-Type",Q="io.xhr",R="Not enough arguments",S="ArrayBuffer",T="undefined",U=", async: ",V="Native XHR object doesn't support overrideMimeType.";qx.Bootstrap.define(D,{extend:Object,construct:function(){var W=qx.Bootstrap.bind(this.__uF,this);if(qx.event&&qx.event.GlobalError&&qx.event.GlobalError.observeMethod){this.__ur=qx.event.GlobalError.observeMethod(W);}
else {this.__ur=W;}
;this.__us=qx.Bootstrap.bind(this.__uE,this);this.__uh=qx.Bootstrap.bind(this.__uJ,this);this.__uD();this._emitter=new qx.event.Emitter();if(window.attachEvent){this.__ut=qx.Bootstrap.bind(this.__uM,this);window.attachEvent(i,this.__ut);}
;}
,statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},events:{"readystatechange":D,"error":D,"loadend":D,"timeout":D,"abort":D,"load":D},members:{readyState:0,responseText:x,responseXML:null,status:0,statusText:x,timeout:0,open:function(bc,X,Y,bb,ba){this.__uO();if(typeof X===T){throw new Error(R);}
else if(typeof bc===T){bc=j;}
;this.__uk=false;this.__uu=false;this.__uv=false;this.__uw=X;if(typeof Y==T){Y=true;}
;this.__ux=Y;if(!this.__uN()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();this.__uD();}
;this.__uz.onreadystatechange=this.__ur;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,n+bc+u+X+U+Y);}
;this.__uz.open(bc,X,Y,bb,ba);}
catch(bd){if(!qx.util.Request.isCrossDomain(X)){throw bd;}
;if(!this.__ux){this.__uy=bd;}
;if(this.__ux){if(window.XDomainRequest){this.readyState=4;this.__uz=new XDomainRequest();this.__uz.onerror=qx.Bootstrap.bind(function(){this._emit(y);this._emit(o);this._emit(p);}
,this);if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,a+bc+u+X+U+Y);}
;this.__uz.open(bc,X,Y,bb,ba);return;}
;window.setTimeout(qx.Bootstrap.bind(function(){if(this.__uB){return;}
;this.readyState=4;this._emit(y);this._emit(o);this._emit(p);}
,this));}
;}
;if(qx.core.Environment.get(J)===I&&qx.core.Environment.get(w)<9&&this.__uz.readyState>0){this.__uz.setRequestHeader(g,k);}
;if(qx.core.Environment.get(J)===G&&parseInt(qx.core.Environment.get(h),10)<2&&!this.__ux){this.readyState=qx.bom.request.Xhr.OPENED;this._emit(y);}
;}
,setRequestHeader:function(be,bf){this.__uO();if(be==H||be==g||be==d||be==O){this.__uv=true;}
;this.__uz.setRequestHeader(be,bf);return this;}
,send:function(bh){this.__uO();if(!this.__ux&&this.__uy){throw this.__uy;}
;if(qx.core.Environment.get(J)===C&&this.timeout===0){this.timeout=10000;}
;if(this.timeout>0){this.__uA=window.setTimeout(this.__uh,this.timeout);}
;bh=typeof bh==T?null:bh;var bg=qx.Bootstrap.getClass(bh);bh=(bh!==null&&this.__uC.indexOf(bg)===-1)?bh.toString():bh;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,M);}
;this.__uz.send(bh);}
catch(bj){if(!this.__ux){throw bj;}
;if(this._getProtocol()===L){this.readyState=2;this.__uG();var bi=this;window.setTimeout(function(){if(bi.__uB){return;}
;bi.readyState=3;bi.__uG();bi.readyState=4;bi.__uG();}
);}
;}
;if(qx.core.Environment.get(J)===G&&!this.__ux){this.__uF();}
;this.__uu=true;return this;}
,abort:function(){this.__uO();this.__uk=true;this.__uz.abort();if(this.__uz){this.readyState=this.__uz.readyState;}
;return this;}
,_emit:function(event){if(this[E+event]){this[E+event]();}
...
,onabort:function(){}
,ontimeout:function(){}
,on:function(name,bk,bl){this._emitter.on(name,bk,bl);return this;}
,getResponseHeader:function(bm){this.__uO();if(qx.core.Environment.get(w)===9&&this.__uz.aborted){return x;}
;return this.__uz.getResponseHeader(bm);}
,getAllResponseHeaders:function(){this.__uO();if(qx.core.Environment.get(w)===9&&this.__uz.aborted){return x;}
;return this.__uz.getAllResponseHeaders();}
,overrideMimeType:function(bn){this.__uO();if(this.__uz.overrideMimeType){this.__uz.overrideMimeType(bn);}
else {throw new Error(V);}
;return this;}
,getRequest:function(){return this.__uz;}
,dispose:function(){if(this.__uB){return false;}
;window.clearTimeout(this.__uA);if(window.detachEvent){window.detachEvent(i,this.__ut);}
;try{this.__uz.onreadystatechange;}
catch(bp){return false;}
;var bo=function(){}
;this.__uz.onreadystatechange=bo;this.__uz.onload=bo;this.__uz.onerror=bo;this.abort();this.__uz=null;this.__uB=true;return true;}
,isDisposed:function(){return !!this.__uB;}
,_createNativeXhr:function(){var bq=qx.core.Environment.get(Q);if(bq===f){return new XMLHttpRequest();}
;if(bq==b){return new window.ActiveXObject(z);}
;qx.Bootstrap.error(this,c);}
,_getProtocol:function(){var br=this.__uw;var bs=/^(\w+:)\/\//;if(br!==null&&br.match){var bt=br.match(bs);if(bt&&bt[1]){return bt[1];}
;}
;return window.location.protocol;}
,__uz:null,__ux:null,__ur:null,__us:null,__ut:null,__uh:null,__uu:null,__uw:null,__uk:null,__gy:null,__uB:null,__uA:null,__uy:null,__uv:null,__uC:null,__uD:function(){this.__uz=this._createNativeXhr();this.__uz.onreadystatechange=this.__ur;if(this.__uz.onabort){this.__uz.onabort=this.__us;}
;this.__uB=this.__uu=this.__uk=false;this.__uC=[S,q,m,v,N];}
,__uE:function(){if(!this.__uk){this.abort();}
;}
,__uF:function(){var bu=this.__uz,bv=true;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,s+bu.readyState);}
;if(this.readyState==bu.readyState){return;}
;this.readyState=bu.readyState;if(this.readyState===qx.bom.request.Xhr.DONE&&this.__uk&&!this.__uu){return;}
;if(!this.__ux&&(bu.readyState==2||bu.readyState==3)){return;}
;this.status=0;this.statusText=this.responseText=x;this.responseXML=null;if(this.readyState>=qx.bom.request.Xhr.HEADERS_RECEIVED){try{this.status=bu.status;this.statusText=bu.statusText;this.responseText=bu.responseText;this.responseXML=bu.responseXML;}
catch(bw){bv=false;}
;if(bv){this.__uK();this.__uL();}
;}
;this.__uG();if(this.readyState==qx.bom.request.Xhr.DONE){if(bu){bu.onreadystatechange=function(){}
;}
;}
;}
,__uG:function(){if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__uA);}
;this._emit(y);if(this.readyState===qx.bom.request.Xhr.DONE){this.__uH();}
;}
,__uH:function(){if(this.__gy){this._emit(F);if(qx.core.Environment.get(J)===C){this._emit(o);}
;this.__gy=false;}
else {if(this.__uk){this._emit(t);}
else {if(this.__uI()){this._emit(o);}
else {this._emit(r);}
;}
;}
;this._emit(p);}
,__uI:function(){var bx;if(this._getProtocol()===L){bx=!this.responseText;}
else {bx=!this.statusText;}
;return bx;}
,__uJ:function(){var by=this.__uz;this.readyState=qx.bom.request.Xhr.DONE;this.__gy=true;by.aborted=true;by.abort();this.responseText=x;this.responseXML=null;this.__uG();}
,__uK:function(){var bz=this.readyState===qx.bom.request.Xhr.DONE;if(this._getProtocol()===L&&this.status===0&&bz){if(!this.__uI()){this.status=200;}
;}
;if(this.status===1223){this.status=204;}
;if(qx.core.Environment.get(J)===C){if(bz&&this.__uv&&!this.__uk&&this.status===0){this.status=304;}
;}
;}
,__uL:function(){if(qx.core.Environment.get(J)==I&&(this.getResponseHeader(P)||x).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var bA=new window.ActiveXObject(K);bA.async=false;bA.validateOnParse=false;bA.loadXML(this.responseText);this.responseXML=bA;}
;}
,__uM:function(){try{if(this){this.dispose();}
;}
catch(e){}
;}
,__uN:function(){var name=qx.core.Environment.get(J);var bB=qx.core.Environment.get(B);return !(name==I&&bB<9||name==G&&bB<3.5);}
,__uO:function(){if(this.__uB){throw new Error(A);}
;}
...
;}
,statics:{PARSER:{json:qx.lang.Json.parse,xml:qx.xml.Document.fromString}},members:{__uq:null,parse:function(h,g){var f=this._getParser(g);if(typeof f===a){if(h!==c){return f.call(this,h);}
;}
;return h;}
,setParser:function(i){if(typeof qx.util.ResponseParser.PARSER[i]===a){return this.__uq=qx.util.ResponseParser.PARSER[i];}
;if(qx.core.Environment.get(d)){qx.core.Assert.assertFunction(i);}
;return this.__uq=i;}
,_getParser:function(k){var j=this.__uq,m=c,l=c;if(j){return j;}
