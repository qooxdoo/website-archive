...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.WebWorker.4bd931bbde37.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="GET",b="error",c="onmessage",d='message',f="message",g="html.webworker",h="var onmessage = null, postMessage = ",i="qx.bom.WebWorker",j=";",k="qx.event.type.Data";qx.Class.define(i,{extend:qx.core.Object,construct:function(l){qx.core.Object.call(this);this.__EI=qx.core.Environment.get(g);this.__EI?this.__EK(l):this.__EL(l);}
,events:{"message":k,"error":k},members:{_worker:null,_handleErrorBound:null,_handleMessageBound:null,__EI:true,__EJ:null,__EK:function(m){this._worker=new window.Worker(m);this._handleMessageBound=qx.lang.Function.bind(this._handleMessage,this);this._handleErrorBound=qx.lang.Function.bind(this._handleError,this);qx.bom.Event.addNativeListener(this._worker,f,this._handleMessageBound);qx.bom.Event.addNativeListener(this._worker,b,this._handleErrorBound);}
,__EL:function(src){var that=this;var req=new qx.bom.request.Xhr();req.onload=function(){that.__EJ=(function(){var postMessage=function(e){that.fireDataEvent(d,e);}
;eval(h+postMessage+j+req.responseText);return {onmessage:eval(c),postMessage:postMessage};}
)();}
;req.open(a,src,false);req.send();}
,postMessage:function(n){var o=this;if(this.__EI){this._worker.postMessage(n);}
else {setTimeout(function(){try{o.__EJ.onmessage&&o.__EJ.onmessage({data:n});}
...
},destruct:function(){if(this.__EI){qx.bom.Event.removeNativeListener(this._worker,f,this._handleMessageBound);qx.bom.Event.removeNativeListener(this._worker,b,this._handleErrorBound);if(this._worker){this._worker.terminate();this._worker=null;}
;}
else {if(this.__EJ){this.__EJ=null;}
;}
;}
});}
)();
...
;var cB=this.getContentElement();if(cw&&this.__pA){if(this.__pA){var cy=this.getInsets();this._getPlaceholderElement().setStyles({paddingTop:cy.top+cD,paddingRight:cy.right+cD,paddingBottom:cy.bottom+cD,paddingLeft:cy.left+cD});}
;}
;if(cA||cC.margin){if(this.__pA){var cy=this.getInsets();this._getPlaceholderElement().setStyles({"width":(innerWidth-cy.left-cy.right)+cD,"height":(innerHeight-cy.top-cy.bottom)+cD});}
;cB.setStyles({"width":innerWidth+cD,"height":innerHeight+cD});this._renderContentElement(innerHeight,cB);}
;if(cC.position){if(this.__pA){this._getPlaceholderElement().setStyles({"left":cE+cD,"top":top+cD});}
;}
;}
...
;return cG;}
,_applyEnabled:function(cJ,cI){qx.ui.core.Widget.prototype._applyEnabled.call(this,cJ,cI);this.getContentElement().setEnabled(cJ);if(this.__pA){if(cJ){this._showPlaceholder();}
else {this._removePlaceholder();}
;}
else {var cH=this.getContentElement();cH.setAttribute(C,cJ?this.getPlaceholder():R);}
;}
,__pC:{width:16,height:16},_getContentHint:function(){return {width:this.__pC.width*10,height:this.__pC.height||16};}
,_applyFont:function(cM,cL){if(cL&&this.__mt&&this.__mw){this.__mt.removeListenerById(this.__mw);this.__mw=null;}
;var cK;if(cM){this.__mt=qx.theme.manager.Font.getInstance().resolve(cM);if(this.__mt instanceof qx.bom.webfonts.WebFont){this.__mw=this.__mt.addListener(B,this._onWebFontStatusChange,this);}
;cK=this.__mt.getStyles();}
...
;if(this.__pA){delete cK[r];this._getPlaceholderElement().setStyles(cK);}
;if(cM){this.__pC=qx.bom.Label.getTextSize(m,cK);}
else {delete this.__pC;}
;qx.ui.core.queue.Layout.add(this);}
...
,_getTextSize:function(){return this.__pC;}
,_onHtmlInput:function(e){var cT=e.getData();var cS=true;this.__pB=false;if(this.__iU&&this.__iU===cT){cS=false;}
...
;}
;if(cS){this.fireDataEvent(X,cT,this.__iU);this.__iU=cT;if(this.getLiveUpdate()){this.__pD(cT);}
;}
;}
,_onWebFontStatusChange:function(cX){if(cX.getData().valid===true){var cW=this.__mt.getStyles();this.__pC=qx.bom.Label.getTextSize(m,cW);qx.ui.core.queue.Layout.add(this);}
;}
,__pD:function(da){var cY=this.__iT;this.__iT=da;if(cY!=da){this.fireNonBubblingEvent(cm,qx.event.type.Data,[da,cY]);}
;}
,setValue:function(de){if(de===null){if(this.__pB){return de;}
;de=R;this.__pB=true;}
else {this.__pB=false;if(this.__pA){this._removePlaceholder();}
;}
;if(qx.lang.Type.isString(de)){var dd=this.getContentElement();if(dd.getValue()!=de){var db=dd.getValue();dd.setValue(de);var dc=this.__pB?null:de;this.__iT=db;this.__pD(dc);this.__iU=this.__iT;}
;if(this.__pA){this._showPlaceholder();}
;return de;}
;throw new Error(l+de);}
,getValue:function(){var df=this.getContentElement().getValue();return this.__pB?null:df;}
,resetValue:function(){this.setValue(null);}
,_onChangeContent:function(e){this.__pB=e.getData()===null;this.__pD(e.getData());}
...
,setLayoutParent:function(parent){qx.ui.core.Widget.prototype.setLayoutParent.call(this,parent);if(this.__pA){if(parent){this.getLayoutParent().getContentElement().add(this._getPlaceholderElement());}
...
.bind(this),0);}
,_removePlaceholder:function(){if(this.hasState(w)){if(this.__pA){this._getPlaceholderElement().setStyle(T,cg);}
;this.removeState(w);}
;}
,_syncPlaceholder:function(){if(this.hasState(w)&&this.__pA){this._getPlaceholderElement().setStyle(T,G);}
;}
...
;if(!this.__pA&&qx.ui.form.AbstractField.__dP){qx.bom.Stylesheet.removeSheet(qx.ui.form.AbstractField.__dP);qx.ui.form.AbstractField.__dP=null;qx.ui.form.AbstractField.__pz();}
;}
,_applyPlaceholder:function(dn,dm){if(this.__pA){this._getPlaceholderElement().setValue(dn);if(dn!=null){this.addListener(u,this._removePlaceholder,this);this.addListener(bH,this._showPlaceholder,this);this._showPlaceholder();}
...
;this._placeholder=this.__mt=null;if(qx.core.Environment.get(bL)){qx.locale.Manager.getInstance().removeListener(ce,this._onChangeLocale,this);}
;if(this.__mt&&this.__mw){this.__mt.removeListenerById(this.__mw);}
...
;qx.html.Element.call(this,t,q,r);this.__pE=s;}
,members:{__pE:null,__pF:null,__pG:null,_createDomElement:function(){return qx.bom.Input.create(this.__pE);}
...
;}
,setEnabled:function(w){this.__pG=w;this.setAttribute(l,w===false);if(qx.core.Environment.get(a)==g){if(!w){this.setStyles({"userModify":m,"userSelect":d});}
else {this.setStyles({"userModify":null,"userSelect":this.__pF?null:d});}
;}
;}
,setSelectable:qx.core.Environment.select(a,{"webkit":function(x){this.__pF=x;qx.html.Element.prototype.setSelectable.call(this,this.__pG&&x);}
...
,setWrap:function(C,D){if(this.__pE===n){this._setProperty(c,C,D);}
else {throw new Error(e);}
;return this;}
,getWrap:function(){if(this.__pE===n){return this._getProperty(c);}
else {throw new Error(e);}
;}
}});}
)();
