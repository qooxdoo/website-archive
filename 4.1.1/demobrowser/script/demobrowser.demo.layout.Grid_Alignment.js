...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Grid_Alignment.7cbbef5bec89.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,__HX:{north:1,south:2,west:3,east:4,center:5},__HY:{'1':t,'2':e,'3':z,'4':u},__nQ:function(){var H=this._getLayoutChildren();var P,J;var length=H.length;var L=[];var O=[];var K=[];var I=this.getSort()===o;var N=this.getSort()===A;for(var i=0;i<length;i++ ){P=H[i];K=P.getLayoutProperties().edge;if(K===n){if(J){throw new Error(b);}
;J=P;}
...
;this.__gW=Q;var M=[];for(var i=0;i<length;i++ ){K=Q[i].getLayoutProperties().edge;M[i]=this.__HX[K]||5;}
;this.__HW=M;delete this._invalidChildrenCache;}
,renderLayout:function(bl,bb,bt){if(this._invalidChildrenCache){this.__nQ();}
;var bk=qx.ui.layout.Util;var W=this.__gW;var bm=this.__HW;var length=W.length;var ba,bu,Y,bf,bg,br,bd,T,S;var bx=[];var bc=[];var bs=this._getSeparatorWidths();var bB=this.getSpacingX();var bA=this.getSpacingY();var X=-bB;var bn=-bA;if(bs.x){X-=bs.x+bB;}
;if(bs.y){bn-=bs.y+bA;}
...
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__nQ();}
;var bS=this.__gW;var cd=this.__HW;var length=bS.length;var bW,ce;var bY,bX;var ca=0,cl=0;var bP=0,cg=0;var cb=0,ck=0;var bQ=0,cf=0;var ch=this._getSeparatorWidths();var cj=this.getSpacingX(),ci=this.getSpacingY();var bU=-cj,bT=-ci;if(ch.x){bU-=ch.x+cj;}
;if(ch.y){bT-=ch.y+ci;}
...
},destruct:function(){this.__HW=this.__gW=null;}
});}
)();
...
,_applyValue:function(bh,bg){var bf=this.getChildControl(t);this._updateButtons();this.__wb=bh;if(bh!==null){if(this.getNumberFormat()){bf.setValue(this.getNumberFormat().format(bh));}
...
,_applyNumberFormat:function(bp,bo){var bn=this.getChildControl(t);bn.setFilter(this._getFilterRegExp());var bq=this.getNumberFormat();if(bq!==null){bq.addListener(F,this._onChangeNumberFormat,this);}
;this._applyValue(this.__wb,undefined);}
,_getContentPaddingTarget:function(){return this.getChildControl(t);}
,_updateButtons:function(){var bs=this.getChildControl(y);var br=this.getChildControl(q);var bt=this.getValue();if(!this.getEnabled()){bs.setEnabled(false);br.setEnabled(false);}
else {if(this.getWrap()){bs.setEnabled(true);br.setEnabled(true);}
else {if(bt!==null&&bt<this.getMaximum()){bs.setEnabled(true);}
else {bs.setEnabled(false);}
;if(bt!==null&&bt>this.getMinimum()){br.setEnabled(true);}
else {br.setEnabled(false);}
;}
;}
;}
,_onKeyDown:function(e){switch(e.getKeyIdentifier()){case a:this.__wc=true;case p:this.getChildControl(y).press();break;case m:this.__wd=true;case h:this.getChildControl(q).press();break;default:return;};e.stopPropagation();e.preventDefault();}
,_onKeyUp:function(e){switch(e.getKeyIdentifier()){case a:this.getChildControl(y).release();this.__wc=false;break;case p:this.getChildControl(y).release();break;case m:this.getChildControl(q).release();this.__wd=false;break;case h:this.getChildControl(q).release();break;};}
,_onRoll:function(e){if(e.getPointerType()!=D){return;}
;var bu=e.getDelta().y;if(bu<0){this._countUp();}
else if(bu>0){this._countDown();}
;e.stop();}
,_onTextChange:function(e){var bv=this.getChildControl(t);var bw;if(this.getNumberFormat()){try{bw=this.getNumberFormat().parse(bv.getValue());}
catch(bx){}
;}
;if(bw===undefined){bw=parseFloat(bv.getValue());}
;if(!isNaN(bw)){if(bw>this.getMaximum()){bw=this.getMaximum();}
else if(bw<this.getMinimum()){bw=this.getMinimum();}
;if(bw===this.__wb){this._applyValue(this.__wb);}
else {this.setValue(bw);}
;}
else {this._applyValue(this.__wb,undefined);}
;}
,_onChangeLocale:function(bz){if(this.getNumberFormat()!==null){this.setNumberFormat(this.getNumberFormat());var by=this.getChildControl(t);by.setFilter(this._getFilterRegExp());by.setValue(this.getNumberFormat().format(this.getValue()));}
;}
,_onChangeNumberFormat:function(bB){var bA=this.getChildControl(t);bA.setFilter(this._getFilterRegExp());bA.setValue(this.getNumberFormat().format(this.getValue()));}
,_countUp:function(){if(this.__wc){var bD=this.getValue()+this.getPageStep();}
else {var bD=this.getValue()+this.getSingleStep();}
;if(this.getWrap()){if(bD>this.getMaximum()){var bC=this.getMaximum()-bD;bD=this.getMinimum()-bC-1;}
;}
;this.gotoValue(bD);}
,_countDown:function(){if(this.__wd){var bF=this.getValue()-this.getPageStep();}
else {var bF=this.getValue()-this.getSingleStep();}
;if(this.getWrap()){if(bF<this.getMinimum()){var bE=this.getMinimum()+bF;bF=this.getMaximum()+bE+1;}
;}
;this.gotoValue(bF);}
,gotoValue:function(bG){return this.setValue(Math.min(this.getMaximum(),Math.max(this.getMinimum(),bG)));}
...
(function(){var a="yellow",b="gray",c="bottom",d="center",e="green",f="top",g="left",h="middle",j="demobrowser.demo.layout.Grid_Alignment",k="main",l="orange",m="right";qx.Class.define(j,{extend:demobrowser.demo.util.LayoutApplication,members:{main:function(){demobrowser.demo.util.LayoutApplication.prototype.main.call(this);var scroll=new qx.ui.container.Scroll();this.getRoot().add(scroll,{edge:0});var n=new qx.ui.container.Composite(new qx.ui.layout.Grid(20)).set({padding:20});scroll.add(n);n.add(this.getCellAlignGrid(),{row:0,column:0});n.add(this.getRowColumnAlignGrid(),{row:0,column:1});n.add(this.getColumnAlignGrid(),{row:0,column:2});n.add(this.getRowAlignGrid(),{row:1,column:0});}
,getNewWidget:function(){var o=new qx.ui.core.Widget().set({decorator:k,backgroundColor:e,allowGrowX:false,allowGrowY:false,height:Math.round((Math.random()*30)+30),width:Math.round((Math.random()*30)+30)});return o;}
,getColumnAlignGrid:function(){var p=new qx.ui.container.Composite().set({decorator:k,backgroundColor:a,allowGrowX:false,allowGrowY:false});var q=new qx.ui.layout.Grid();q.setSpacing(3);q.setColumnAlign(0,g,f);q.setColumnAlign(1,d,h);q.setColumnAlign(2,m,c);p.setLayout(q);for(var x=0;x<3;x++ ){for(var y=0;y<6;y++ ){p.add(this.getNewWidget(),{row:y,column:x});}
;}
;return p;}
,getRowAlignGrid:function(){var r=new qx.ui.container.Composite().set({decorator:k,backgroundColor:a,allowGrowX:false,allowGrowY:false});var s=new qx.ui.layout.Grid();s.setSpacing(3);s.setRowAlign(0,g,f);s.setRowAlign(1,d,h);s.setRowAlign(2,m,c);r.setLayout(s);for(var x=0;x<6;x++ ){for(var y=0;y<3;y++ ){r.add(this.getNewWidget(),{row:y,column:x});}
;}
;return r;}
,getCellAlignGrid:function(){var t=new qx.ui.container.Composite().set({decorator:k,backgroundColor:a,allowGrowX:false,allowGrowY:false});var v=new qx.ui.layout.Grid();v.setSpacing(3);t.setLayout(v);for(var x=0;x<5;x++ ){v.setColumnAlign(x,d,h);for(var y=0;y<5;y++ ){t.add(this.getNewWidget(),{row:y,column:x});}
;}
;var u=v.getCellWidget(0,0);u.set({alignX:g,alignY:f});u.set({backgroundColor:l,width:20,height:20});var u=v.getCellWidget(4,4);u.set({alignX:m,alignY:c});u.set({backgroundColor:l,width:20,height:20});return t;}
,getRowColumnAlignGrid:function(){var w=new qx.ui.container.Composite().set({decorator:k,backgroundColor:a,allowGrowX:false,allowGrowY:false});var A=new qx.ui.layout.Grid();A.setSpacing(3);w.setLayout(A);for(var x=0;x<5;x++ ){for(var y=0;y<5;y++ ){w.add(this.getNewWidget(),{row:y,column:x});}
;}
;A.setColumnAlign(2,g,f);A.setRowAlign(2,m,c);for(var i=0;i<5;i++ ){A.getCellWidget(2,i).setBackgroundColor(b);A.getCellWidget(i,2).setBackgroundColor(b);}
;var z=A.getCellWidget(2,2);z.set({backgroundColor:l,width:20,height:20});return w;}
}});}
)();
