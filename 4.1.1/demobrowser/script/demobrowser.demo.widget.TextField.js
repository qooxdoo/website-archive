...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.TextField","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.TextField.fd6e57d060bd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="text\narea\nmonospace",b="Use the following buttons to interact with the ",c="changeValue",d="Send content",f="execute",g="20px sans-serif",h="ChangeValue: ",i="Send end of selection",j="demobrowser.demo.widget.TextField",k="Toggle live update",l="Web 2.0",m="Toggle enabled",n="Sending content: ",o="Sending selection: ",p="<b>first textfield</b> widget.",q="text\narea\nnowrap",r="max15",s="Send selection",t="monospace",u="read only",v="Send start of selection",w="text\narea";qx.Class.define(j,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var I=new qx.ui.container.Composite(new qx.ui.layout.HBox());this.getRoot().add(I);var x=new qx.ui.layout.VBox();x.setSpacing(10);var A=new qx.ui.container.Composite(x);A.setPadding(20);I.add(A);var z=new qx.ui.form.TextField(r).set({maxLength:15});z.focus();z.addListener(c,function(e){this.debug(h+e.getData());}
,this);A.add(z);var B=new qx.ui.form.TextField(l).set({font:qx.bom.Font.fromString(g),padding:6});A.add(B);var D=new qx.ui.form.TextField(u).set({readOnly:true});A.add(D);var E=new qx.ui.form.TextArea(w);A.add(E);var L=new qx.ui.form.TextArea(q);L.setWrap(false);A.add(L);var y=new qx.ui.form.TextArea(a);y.setFont(t);A.add(y);var G=new qx.ui.container.Composite(new qx.ui.layout.VBox(8));G.setPadding(20);var F=new qx.ui.form.Button(k);F.addListener(f,function(){z.toggleLiveUpdate();}
);G.add(F);var K=new qx.ui.form.Button(m);var J=false;K.addListener(f,function(){A.setEnabled(J);J=!J;}
);G.add(K);G.add(new qx.ui.core.Spacer(null,20));var N=b+p;var P=new qx.ui.basic.Label(N);P.setRich(true);G.add(P);var H=new qx.ui.form.Button(d);H.addListener(f,function(){this.debug(n+z.getValue());}
);G.add(H);var C=new qx.ui.form.Button(s);C.setFocusable(false);C.setKeepFocus(true);C.addListener(f,function(){this.debug(o+z.getTextSelection());}
);G.add(C);var O=new qx.ui.form.Button(v);O.setFocusable(false);O.setKeepFocus(true);O.addListener(f,function(){this.debug(o+z.getTextSelectionStart());}
);G.add(O);var M=new qx.ui.form.Button(i);M.setFocusable(false);M.setKeepFocus(true);M.addListener(f,function(){this.debug(o+z.getTextSelectionEnd());}
);G.add(M);I.add(G);}
}});}
)();
...
,properties:{wrap:{check:b,init:true,apply:f},appearance:{refine:true,init:j},singleStep:{check:i,init:20},minimalLineHeight:{check:i,apply:h,init:4},autoSize:{check:b,apply:w,init:false}},members:{__vM:null,__vN:null,__vO:null,setValue:function(M){M=qx.ui.form.AbstractField.prototype.setValue.call(this,M);this.__vP();return M;}
...
;}
,__vP:function(){if(this.isAutoSize()){var R=this.__vQ();if(R&&this.getBounds()){this.__vO=this.__vO||this._getAreaHeight();var T=this._getScrolledAreaHeight();if(this.getMaxHeight()){var P=this.getInsets();var Q=-P.top+this.getMaxHeight()-P.bottom;if(T>Q){this.getContentElement().setStyle(I,k);}
else {this.getContentElement().setStyle(I,H);}
;}
;var S=Math.max(T,this.__vO);this._setAreaHeight(S);}
else {this.getContentElement().addListenerOnce(x,function(){this.__vP();}
,this);}
;}
;}
,_getAreaHeight:function(){return this.getInnerSize().height;}
,_setAreaHeight:function(U){if(this._getAreaHeight()!==U){this.__vN=U;qx.ui.core.queue.Layout.add(this);qx.ui.core.queue.Manager.flush();this.__vT();}
;}
,_getScrolledAreaHeight:function(){var W=this.__vQ();var V=W.getDomElement();if(V){if(!V.parentNode){qx.html.Element.flush();return this._getScrolledAreaHeight();}
...
;this.__vS(W);if(qx.core.Environment.get(C)==B&&qx.core.Environment.get(s)==8){if(!V.scrollTop){qx.html.Element.flush();}
;}
;return V.scrollTop;}
;}
,__vQ:function(){this.__vM=this.__vM||this.__vR();return this.__vM;}
,__vR:function(){var ba,Y,bb,X;ba=this.getContentElement();if(!ba.getDomElement()){return null;}
;bb=qx.bom.Element.clone(ba.getDomElement());X=new qx.html.Input(j);X.useElement(bb);Y=X;Y.setStyles({position:K,top:0,left:o,height:0,overflow:H},true);Y.removeAttribute(G);Y.removeAttribute(z);Y.setAttribute(y,g);Y.setValue(ba.getValue()||t);Y.insertBefore(ba);this.__vS(Y);return Y;}
,__vS:function(bc){bc=bc.getDomElement();if(bc){bc.scrollTop=10000;}
;}
...
,_applyAutoSize:function(bh,bg){if(qx.core.Environment.get(m)){this.__vU();}
;if(bh){this.__vP();this.addListener(a,this.__vP,this);if(!this.getBounds()){this.addListenerOnce(x,function(){this.getContentElement().setStyle(I,H);}
);}
else {this.getContentElement().setStyle(I,H);}
;}
else {this.removeListener(a,this.__vP);this.getContentElement().setStyle(I,k);}
;}
,_applyDimension:function(bi){qx.ui.form.AbstractField.prototype._applyDimension.call(this);if(qx.core.Environment.get(m)){this.__vU();}
;if(bi===this.getMaxHeight()){this.__vP();}
;}
,__vT:function(){var content=this.getContentElement();var bk=content.getDomElement();var bj=content.getStyle(u);content.setStyle(u,parseInt(bj,10)+1000+c,true);if(bk){qx.bom.element.Dimension.getWidth(bk);}
;content.setStyle(u,bj,true);}
,__vU:function(){if(this.isAutoSize()&&this.getHeight()){this.warn(n+J+v);}
;}
,_getContentHint:function(){var bl=qx.ui.form.AbstractField.prototype._getContentHint.call(this);bl.height=bl.height*this.getMinimalLineHeight();bl.width=this._getTextSize().width*20;if(this.isAutoSize()){bl.height=this.__vN||bl.height;}
;return bl;}
},destruct:function(){this.setAutoSize(false);if(this.__vM){this.__vM.dispose();}
;}
});}
)();
