...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.7817381dcc8a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="TERMINATED",c="__Hd",d=" => ",f="#autoActionsBeforeOnexit",g=": Could not find friendly name for '",h=": event '",j=" blocked.  Re-queuing.",k="string",l="qx.event.type.Data",m="'",n="Attempt to transition to nonexistent state ",o="Saved-state stack is empty",p="Internal error: invalid nextState",q=" not handled.  Ignoring.",r="State ",s="#autoActionsBeforeOntransition",u=" returned a value other than ",v="#",w="__He",x=" already exists",y="Attempt to transition to POP_STATE_STACK ",z=":",A="Transition ",B=" does not exist",C="' on '",D="qx.util.fsm.FiniteStateMachine",E="String",F="terminated",G="#autoActionsAfterOntransition",H="",I="#actionsAfterOnentry",J="Saved-state stack is full",K="#autoActionsBeforeOnentry",L=": no transition found.  No state change.",M="#autoActionsAfterOnentry",N="#actionsBeforeOnentry",O="', because the finite state machine is not running.",P=": Pushed event: ",Q=": Cannot listen to event '",R="Machine started with no available states",S=": Process event: ",T="#entry",U="#ontransition",V=" on ",W="Invalid state: not an instance of ",X="Number",Y="#exit",bp="true, false, or null.",bq="Explicit transition ",br="  ",bl="qx.util.fsm.State",bm="while state stack is empty.",bn=".  Ignoring.",bo="#autoActionsAfterOnexit",bs=": Event '",bt="object",bu=" not handled for target ",bv=": Queued event: ";qx.Class.define(D,{extend:qx.core.Object,construct:function(bw){qx.core.Object.call(this);this.setName(bw);this.__jz={};this.__Hb=null;this.__Hc=[];this.__Hd=[];this.__He=[];this.__Hf={};this.__Hg={};this.__Hh={};this.__Hi={};this.__Hj={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},events:{"terminated":l},properties:{name:{check:E,nullable:true},state:{check:E,nullable:true},previousState:{check:E,nullable:true},nextState:{check:E,nullable:true},maxSavedStates:{check:X,init:2},debugFlags:{check:X,init:7}},members:{__jz:null,__Hb:null,__Hd:null,__He:null,__Hc:null,__Hf:null,__Hg:null,__Hh:null,__Hi:null,__Hj:null,__Hk:false,__Hl:true,isTerminated:function(){return this.__Hl;}
,addState:function(bx){if(!bx instanceof qx.util.fsm.State){throw new Error(W+bl);}
;var by=bx.getName();if(by in this.__jz){throw new Error(r+by+x);}
;if(this.__Hb==null){this.__Hb=by;}
;this.__jz[by]=bx;}
...
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__Hg[bG]=bH;this.__Hh[bH]=bG;this.__Hf[bG]=bD;if(!bE){return;}
;if(typeof (bE)==k){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__Hi[bF]){this.__Hi[bF]={};}
;this.__Hi[bF][bG]=true;if(!this.__Hj[bG]){this.__Hj[bG]=[];}
;this.__Hj[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__Hg[bL];if(this.__Hj[bL]){for(var i=0;i<this.__Hj[bL].length;i++ ){bJ=this.__Hj[bL][i];delete this.__Hi[bJ][bL];bM=true;for(bK in this.__Hi[bJ]){bM=false;break;}
;if(bM){delete this.__Hi[bJ];}
;}
;delete this.__Hj[bL];}
;delete this.__Hh[bI];delete this.__Hg[bL];delete this.__Hf[bL];}
,getObject:function(bN){return this.__Hf[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__Hh[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__Hi[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__Hg){var bT=this.__Hg[bR];var bS=this.getObject(bR);this.debug(bR+d+bT);this.debug(br+bT+d+this.__Hh[bT]);this.debug(br+bR+d+this.getObject(bR));this.debug(br+this.getObject(bR)+d+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__jz,"startState":this.__Hb,"eventQueue":this.__Hd,"blockedEvents":this.__He,"savedStates":this.__Hc,"friendlyToObject":this.__Hf,"friendlyToHash":this.__Hg,"hashToFriendly":this.__Hh,"groupToFriendly":this.__Hi,"friendlyToGroups":this.__Hj});}
,start:function(){this.__Hl=false;var bV=this.__Hb;if(bV==null){throw new Error(R);}
...
,pushState:function(bW){if(this.__Hc.length>=this.getMaxSavedStates()){throw new Error(J);}
;if(bW===true){this.__Hc.push(this.getState());}
else if(bW){this.__Hc.push(bW);}
else {this.__Hc.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Hc.length==0){throw new Error(o);}
;return this.__Hc.pop();}
,postponeEvent:function(event){this.__He.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__Hd.push(event);}
else {this.__Hd.unshift(event);}
...
;}
;}
,eventListener:function(event){if(this.__Hl){this.debug(this.getName()+Q+event.getType()+O);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__Hm();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__Hl){this.debug(this.getName()+Q+cb+O);return;}
...
,this,cg||20);}
,__Hm:function(){if(this.__Hk){return;}
;this.__Hk=true;while(this.__Hd.length>0){var event=this.__Hd.pop();var ch=this.__Hn(event);if(ch){event.dispose();}
;}
;this.__Hk=false;}
,__Hn:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+S+event.getType()+(ct?V+ct:H));}
;ck=this.getState();cn=this.__jz[ck];cq=cn.transitions;e=cn.getEvents()[event.getType()];if(!e){if(cu){this.debug(this.getName()+bs+event.getType()+m+q);}
;return true;}
;if(typeof (e)==bt){if(!ct){if(co){this.debug(this.getName()+g+event.getType()+C+event.getTarget()+m);}
;return true;}
...
;this.__He.unshift(event);return false;default:if(typeof (cp)==k){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bq+cp+B);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(A+ck+z+t+u+bp);};cl=cm.getNextState();if(typeof (cl)==k){if(!cl in this.__jz){throw new Error(n+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Hc.length==0){throw new Error(y+bm);}
;cl=this.__Hc.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__Hl=true;this.setNextState(null);break;default:throw new Error(p);};}
...
;if(this.__Hl){if(cr){this.debug(this.getName()+v+b);}
...
;cn.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__He.length;i++ ){e=this.__He.pop();this.__Hd.unshift(e);}
;if(cs){this.debug(this.getName()+v+cj+d+this.getName()+v+ck);}
;return true;}
;if(cs){this.debug(this.getName()+v+ck+h+event.getType()+m+L);}
;return true;}
},destruct:function(){this._disposeArray(c);this._disposeArray(w);this.__Hc=this.__jz=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="parameters",k=")",l=" fsm.getObject('",m="__GU",n="groups",o="Invalid function parameters: ",p="Invalid onexit type: ",q="  for (var i = 0; i < groupObjects.length; i++)",r="context",s="Invalid value in events object: ",t="string",u="autoActionsAfterOnentry",v="Invalid numeric value in events object: ",w='"',x="__GS",y="Invalid state name",z=" value: ",A="');",B="State ",C="autoActionsBeforeOnexit",D="autoActionsBeforeOnentry",E="}",F="): ",G="__GX",H="onexit",I="{",J="Invalid function request type: ",K="__GV",L=": ",M="__GY",N="expected array, found ",O="Invalid friendly name in 'objects' list: ",P="autoActionsAfterOnexit",Q="  fsm.debug(ex);",R="(",S="__GW",T="Invalid onentry type: ",U="    fsm.getObject(objName).",V=")(fsm)",W="events",X="undefined",Y="Invalid ",ba="try",bb="    var objName = groupObjects[i];",bc="events must be an object",bd="').",be="objects",bf="expected object, found ",bg="qx.util.fsm.Transition",bh="The events object must be provided in new state info",bi="catch(ex)",bj="  {",bk="    fsm.getGroupObjects('",bl="Invalid value in events object ",bm="Invalid transition: not an instance of ",bn="  }",bo="__GT",bp="__GR",bq="Invalid 'objects' list: expected array, got ",br="Invalid numeric value in events object ",bs="qx.util.fsm.State",bt="onentry",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(bs,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(r,bE);for(var bC in bF){switch(bC){case bt:this.setOnentry(this.__Ha(bF[bC],bE));break;case H:this.setOnexit(this.__Ha(bF[bC],bE));break;case D:this.setAutoActionsBeforeOnentry(bF[bC]);break;case u:this.setAutoActionsAfterOnentry(bF[bC]);break;case C:this.setAutoActionsBeforeOnexit(bF[bC]);break;case P:this.setAutoActionsAfterOnexit(bF[bC]);break;case W:this.setEvents(bF[bC]);break;case r:break;default:this.setUserData(bC,bF[bC]);this.debug(B+bD+L+bz+bC);break;};}
;if(!this.getEvents()){throw new Error(bh);}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(actionType,value,context){if(typeof (value)!=bB){throw new Error(Y+actionType+z+typeof (value));}
;var funcFragment;var func=ba+I;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(J+N+typeof (functionRequest));}
;for(var i=0;i<functionRequest.length;i++ ){objectAndGroupList=functionRequest[i];if(typeof (objectAndGroupList)!=bB){throw new Error(bv+bf+typeof (functionRequest[param]));}
...
else if(typeof (params[j])==t){funcFragment+=w+params[j]+w;}
else {funcFragment+=params[j];}
;}
;funcFragment+=k;var a=objectAndGroupList[be];if(!a){a=[];}
else if(!a instanceof Array){throw new Error(bq+typeof (a));}
;for(var j=0;j<a.length;j++ ){if(typeof (a[j])!=t){throw new Error(O+a[j]);}
;func+=l+a[j]+bd+funcFragment+bu;}
;var g=objectAndGroupList[n];if(g){if(!g instanceof Array){throw new Error(bw+typeof (g));}
;for(j=0;j<g.length;j++ ){func+=d+bk+g[j]+A+q+bj+bb+U+funcFragment+bu+bn;}
;}
;}
;}
;func+=E+bi+I+Q+E;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:bp,nullable:true},onentry:{transform:x,nullable:true,init:function(bG,event){}
},onexit:{transform:bo,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:K,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:S,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:G,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:M,nullable:true,init:function(bL,event){}
},events:{transform:m,nullable:true}},members:{__GR:function(bM){if(typeof (bM)!=t||bM.length<1){throw new Error(y);}
;return bM;}
,__GS:function(bN){switch(typeof (bN)){case X:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(r));default:throw new Error(T+typeof (bN));};}
,__GT:function(bP){switch(typeof (bP)){case X:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(r));default:throw new Error(p+typeof (bP));};}
,__GU:function(bT){if(typeof (bT)!=bB){throw new Error(bc);}
;for(var e in bT){var bR=bT[e];if(typeof (bR)==bA&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(v+e+L+bR);}
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==bA&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(br+R+e+F+bS+L+bR[bS]);}
else if(typeof (bR[bS])!=t&&typeof (bR[bS])!=bA){throw new Error(bl+R+e+F+bS+L+bR[bS]);}
;}
;}
else if(typeof (bR)!=t&&typeof (bR)!=bA){throw new Error(s+e+L+bT[e]);}
;}
;return bT;}
,__GV:function(bU){return qx.util.fsm.State._commonTransformAutoActions(D,bU,this.getUserData(r));}
,__GW:function(bV){return qx.util.fsm.State._commonTransformAutoActions(u,bV,this.getUserData(r));}
,__GX:function(bW){return qx.util.fsm.State._commonTransformAutoActions(C,bW,this.getUserData(r));}
,__GY:function(bX){return qx.util.fsm.State._commonTransformAutoActions(P,bX,this.getUserData(r));}
,__Ha:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bm+bg);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="__Hp",b="function",c="autoActionsBeforeOntransition",d="__Hr",e="or one of the Fsm.StateChange constants",g="autoActionsAfterOntransition",h="context",i="string",j="predicate",k="nextState must be an explicit state name, ",l="number",m="Transition ",n=": ",o="String",p="__Hq",q="__Hs",r="Invalid transition nextState type: ",s="Invalid transition nextState value: ",t="Adding user-provided field to transition: ",u="ontransition",v="__Ho",w="Invalid transition predicate type: ",x="boolean",y="nextState",z="Invalid ontransition type: ",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(h,E);for(var C in D){switch(C){case j:this.setPredicate(this.__Ha(D[C],E));break;case y:this.setNextState(D[C]);break;case c:this.setAutoActionsBeforeOntransition(this.__Ha(D[C],E));break;case g:this.setAutoActionsAfterOntransition(this.__Ha(D[C],E));break;case u:this.setOntransition(this.__Ha(D[C],E));break;case h:break;default:this.setUserData(C,D[C]);this.debug(m+F+n+t+C);break;};}
;}
,properties:{name:{check:o,nullable:true},predicate:{init:function(G,event){return true;}
,transform:v},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:a},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:p},autoActionsAfterOntransition:{init:function(I,event){}
,transform:d},ontransition:{init:function(J,event){}
,transform:q}},members:{__Ho:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
;case x:return function(M,event){return K;}
;case b:return qx.lang.Function.bind(K,this.getUserData(h));default:throw new Error(w+typeof (K));};}
,__Hp:function(N){switch(typeof (N)){case i:return N;case l:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(s+N+n+k+e);};break;default:throw new Error(r+typeof (N));};}
,__Hq:function(O){return qx.util.fsm.State._commonTransformAutoActions(c,O,this.getUserData(h));}
,__Hr:function(P){return qx.util.fsm.State._commonTransformAutoActions(g,P,this.getUserData(h));}
,__Hs:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case b:return qx.lang.Function.bind(Q,this.getUserData(h));default:throw new Error(z+typeof (Q));};}
,__Ha:function(f,S){if(typeof (f)==b){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
