...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Columns.856c158475a6.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,_init:function(){this.__xj=new qx.data.Array();this.__Db=[];this.__Dc=[];this._initLayer();}
...
,_getDataFromRow:function(bq){return this.__xj.getItem(bq);}
...
;}
;if(this.__xj.indexOf(bV)!=-1){this.__De();}
;}
;}
,_onUpdated:function(event){if(this.__hi==null){this.__hi=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__hi.schedule();}
,_onOpen:function(event){var ca=event.getRow();var bY=this.__xj.getItem(ca);if(this.isNode(bY)){if(this.isNodeOpen(bY)){this.closeNode(bY);}
...
;}
;if(!qx.lang.Array.equals(this.__xj.toArray(),cm)){this._provider.removeBindings();this.__xj.removeAll();this.__xj.append(cm);this.__xp();this._updateSelection();}
;}
...
,getParent:function(cD){var cE=this.__xj.indexOf(cD);if(cE<0){return null;}
;var cF=this.__Dc[cE];while(cE>0){cE-- ;var cC=this.__Dc[cE];if(cC<cF){return this.__xj.getItem(cE);}
;}
;return null;}
...
;this._layer.removeListener(R,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__xj.dispose();this._layer=this._provider=this.__xj=this.__Db=this.__hi=null;}
});}
)();
...
;}
,bindProperty:function(x,A,y,w,u){var z=this.__xD(u,x);var v=this._tree.getLookupTable();var B=v.bind(z,w,A,y);this.__xE(w,B);}
,bindPropertyReverse:function(I,E,G,H,C){var F=this.__xD(C,I);var D=this._tree.getLookupTable();var J=H.bind(E,D,F,G);this.__xE(H,J);}
,removeBindings:function(){while(this.__xC.length>0){var K=this.__xC.pop();this._removeBindingsFrom(K);}
;}
...
;}
,_removeBindingsFrom:function(O){var P=this.__xF(O);while(P.length>0){var Q=P.pop();try{this._tree.getLookupTable().removeBinding(Q);}
catch(e){O.removeBinding(Q);}
;}
;if(qx.lang.Array.contains(this.__xC,O)){qx.lang.Array.remove(this.__xC,O);}
;}
,__xD:function(S,T){var R=n+S+l;if(T!=null&&T!=c){R+=o+T;}
;return R;}
,__xE:function(V,W){var U=this.__xF(V);if(!qx.lang.Array.contains(U,W)){U.push(W);}
;if(!qx.lang.Array.contains(this.__xC,V)){this.__xC.push(V);}
;}
,__xF:function(X){var Y=X.getUserData(a);if(Y==null){Y=[];X.setUserData(a,Y);}
;return Y;}
