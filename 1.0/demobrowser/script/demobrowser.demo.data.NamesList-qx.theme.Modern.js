...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.data.NamesList","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.data.NamesList-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"fdae5ed3bf4e"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var q="persons.json</a> and bind the items to the list widget. In the ",p="converter for the label, the names will be set to both, the first ",o="Loading...",n="model.persons",m="' target='_blank'>",l="model",k="state",j="and last name.",i="value",h="demobrowser.demo.data.NamesList",e="firstname",g="<b>List bound to data in a json file</b><br/>",f="json/persons.json",d=", ",c="Loading the json file <a href='";
qx.Class.define(h,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var t=new qx.ui.form.List();
this.getRoot().add(t,{left:10,top:80});
var u=new qx.data.controller.List(null,t);
u.setLabelPath(e);
u.setLabelOptions({converter:function(a,b){return b.getLastname()+d+a;
}});
var s=f;
var v=new qx.data.store.Json(s);
var status=new qx.ui.basic.Label(o);
this.getRoot().add(status,{left:120,top:80});
v.bind(n,u,l);
v.bind(k,status,i);
var r=new qx.ui.basic.Label();
r.setRich(true);
r.setWidth(300);
r.setValue(g+c+s+m+q+p+j);
this.getRoot().add(r,{left:10,top:10});
}}});
})();
...
(function(){var K="single",J="Boolean",I="one",H="changeSelection",G="mouseup",F="mousedown",E="losecapture",D="multi",C="_applyQuickSelection",B="mouseover",u="_applySelectionMode",A="_applyDragSelection",x="__kV",t="qx.ui.core.MMultiSelectionHandling",s="removeItem",w="keypress",v="qx.event.type.Data",y="addItem",r="additive",z="mousemove";
qx.Mixin.define(t,{construct:function(){var q=this.SELECTION_MANAGER;
var p=this.__kV=new q(this);
this.addListener(F,p.handleMouseDown,p);
this.addListener(G,p.handleMouseUp,p);
this.addListener(B,p.handleMouseOver,p);
this.addListener(z,p.handleMouseMove,p);
this.addListener(E,p.handleLoseCapture,p);
this.addListener(w,p.handleKeyPress,p);
this.addListener(y,p.handleAddItem,p);
this.addListener(s,p.handleRemoveItem,p);
p.addListener(H,this._onSelectionChange,this);
},events:{"changeSelection":v},properties:{selectionMode:{check:[K,D,r,I],init:K,apply:u},dragSelection:{check:J,init:false,apply:A},quickSelection:{check:J,init:false,apply:C}},members:{__kV:null,selectAll:function(){this.__kV.selectAll();
...
}return this.__kV.isItemSelected(L);
...
},selectRange:function(c,d){this.__kV.selectItemRange(c,d);
...
if(!qx.lang.Array.equals(g,f)){this.__kV.replaceSelection(f);
...
},_getLeadItem:function(){var k=this.__kV.getMode();

if(k===K||k===I){return this.__kV.getSelectedItem();
}else{return this.__kV.getLeadItem();
}},_applySelectionMode:function(l,m){this.__kV.setMode(l);
},_applyDragSelection:function(a,b){this.__kV.setDrag(a);
},_applyQuickSelection:function(n,o){this.__kV.setQuick(n);
...
(function(){var bd="one",bc="single",bb="selected",ba="additive",Y="multi",X="PageUp",W="under",V="Left",U="lead",T="Down",bB="Up",bA="Boolean",bz="PageDown",by="anchor",bx="End",bw="Home",bv="Right",bu="right",bt="click",bs="above",bk="left",bl="Escape",bi="A",bj="Space",bg="_applyMode",bh="interval",be="changeSelection",bf="qx.event.type.Data",bm="quick",bn="key",bp="__mo",bo="abstract",br="drag",bq="qx.ui.core.selection.Abstract";
qx.Class.define(bq,{type:bo,extend:qx.core.Object,construct:function(){arguments.callee.base.call(this);
this.__ml={};
},events:{"changeSelection":bf},properties:{mode:{check:[bc,Y,ba,bd],init:bc,apply:bg},drag:{check:bA,init:false},quick:{check:bA,init:false}},members:{__mm:0,__mn:0,__mo:null,__mp:null,__mq:null,__mr:null,__ms:null,__mt:null,__mu:null,__mv:null,__mw:null,__mx:null,__my:null,__mz:null,__mA:null,__mB:null,__mC:null,__ml:null,__mD:null,__mE:null,getSelectionContext:function(){return this.__mB;
},selectAll:function(){var cc=this.getMode();

if(cc==bc||cc==bd){throw new Error("Can not select all items in selection mode: "+cc);
}this._selectAllItems();
this._fireChange();
},selectItem:function(ch){this._setSelectedItem(ch);
var ci=this.getMode();

if(ci!==bc&&ci!==bd){this._setLeadItem(ch);
this._setAnchorItem(ch);
}this._scrollItemIntoView(ch);
this._fireChange();
},addItem:function(k){var m=this.getMode();

if(m===bc||m===bd){this._setSelectedItem(k);
}else{if(!this._getAnchorItem()){this._setAnchorItem(k);
}this._setLeadItem(k);
this._addToSelection(k);
}this._scrollItemIntoView(k);
this._fireChange();
},removeItem:function(g){this._removeFromSelection(g);

if(this.getMode()===bd&&this.isSelectionEmpty()){var h=this._getFirstSelectable();
...
if(bU==bc||bU==bd){throw new Error("Can not select multiple items in selection mode: "+bU);
...
this._fireChange();
},replaceSelection:function(bC){var bD=this.getMode();

if(bD==bd||bD===bc){if(bC.length>1){throw new Error("Could not select more than one items in mode: "+bD+"!");
}
if(bC.length==1){this.selectItem(bC[0]);
}else{this.clearSelection();
}return;
}else{this._replaceMultiSelection(bC);
}},getSelectedItem:function(){var P=this.getMode();

if(P===bc||P===bd){return this._getSelectedItem()||null;
}throw new Error("The method getSelectedItem() is only supported in 'single' and 'one' selection mode!");
},getSelection:function(){return qx.lang.Object.getValues(this.__ml);
},getSortedSelection:function(){var f=this.getSelectables();
var d=qx.lang.Object.getValues(this.__ml);
d.sort(function(a,b){return f.indexOf(a)-f.indexOf(b);
});
return d;
},isItemSelected:function(bQ){var bR=this._selectableToHashCode(bQ);
return this.__ml[bR]!==undefined;
},isSelectionEmpty:function(){return qx.lang.Object.isEmpty(this.__ml);
},invertSelection:function(){var O=this.getMode();

if(O===bc||O===bd){throw new Error("The method invertSelection() is only supported in 'multi' and 'additive' selection mode!");
}var N=this.getSelectables();

for(var i=0;i<N.length;i++){this._toggleInSelection(N[i]);
}this._fireChange();
},_setLeadItem:function(cM){var cN=this.__mC;

if(cN!==null){this._styleSelectable(cN,U,false);
}
if(cM!==null){this._styleSelectable(cM,U,true);
}this.__mC=cM;
},_getLeadItem:function(){{};
return this.getLeadItem();
},getLeadItem:function(){return this.__mC!==null?this.__mC:null;
},_setAnchorItem:function(cS){var cT=this.__mD;

if(cT){this._styleSelectable(cT,by,false);
}
if(cS){this._styleSelectable(cS,by,true);
}this.__mD=cS;
...
},_getSelectableFromMouseEvent:function(event){var cg=event.getTarget();
return this._isSelectable(cg)?cg:null;
...
},_getSelectableLocationX:function(cO){throw new Error("Abstract method call: _getSelectableLocationX()");
},_getSelectableLocationY:function(t){throw new Error("Abstract method call: _getSelectableLocationY()");
},_getScroll:function(){throw new Error("Abstract method call: _getScroll()");
},_scrollBy:function(bX,bY){throw new Error("Abstract method call: _scrollBy()");
},_scrollItemIntoView:function(cL){throw new Error("Abstract method call: _scrollItemIntoView()");
...
this._setAnchorItem(null);
this._clearSelection();
if(z===bd){var B=this._getFirstSelectable();
...
if(cb!==bd&&cb!==bc){return;
}var ca=this._getSelectableFromMouseEvent(event);

if(ca===null){return;
}this._setSelectedItem(ca);
this._fireChange(bm);
},handleMouseDown:function(event){var cr=this._getSelectableFromMouseEvent(event);

if(cr===null){return;
}var ct=event.isCtrlPressed()||(qx.bom.client.Platform.MAC&&event.isMetaPressed());
var cq=event.isShiftPressed();
if(this.isItemSelected(cr)&&!cq&&!ct&&!this.getDrag()){this.__mE=cr;
return;
}else{this.__mE=null;
}this._scrollItemIntoView(cr);
switch(this.getMode()){case bc:case bd:this._setSelectedItem(cr);
break;
case ba:this._setLeadItem(cr);
this._setAnchorItem(cr);
this._toggleInSelection(cr);
break;
case Y:this._setLeadItem(cr);
if(cq){var cs=this._getAnchorItem();

if(cs===null){cs=this._getFirstSelectable();
this._setAnchorItem(cs);
}this._selectItemRange(cs,cr,ct);
}else if(ct){this._setAnchorItem(cr);
this._toggleInSelection(cr);
}else{this._setAnchorItem(cr);
this._setSelectedItem(cr);
}break;
}var cu=this.getMode();

if(this.getDrag()&&cu!==bc&&cu!==bd&&!cq&&!ct){this.__ms=this._getLocation();
...
if(!q&&!n&&this.__mE){var o=this._getSelectableFromMouseEvent(event);

if(o===null||!this.isItemSelected(o)){return;
}var p=this.getMode();

if(p===ba){this._removeFromSelection(o);
}else{this._setSelectedItem(o);

if(this.getMode()===Y){this._setLeadItem(o);
...
var M=this.__mw+this.__mp.left;

if(M>this.__mt){this.__my=1;
}else if(M<this.__mt){this.__my=-1;
}else{this.__my=0;
}var L=this.__mx+this.__mp.top;

if(L>this.__mu){this.__mz=1;
}else if(L<this.__mu){this.__mz=-1;
}else{this.__mz=0;
...
this.__mo.addListener(bh,this._onInterval,this);
}this.__mo.start();
this._autoSelect();
event.stopPropagation();
},handleAddItem:function(e){var R=e.getData();

if(this.getMode()===bd&&this.isSelectionEmpty()){this.addItem(R);
}},handleRemoveItem:function(e){this.removeItem(e.getData());
},_cleanup:function(){if(!this.getDrag()&&this.__mv){return;
}if(this.__mA){this._fireChange(bt);
}delete this.__mv;
delete this.__mq;
delete this.__mr;
...
this.__mp=this._getScroll();
this._autoSelect();
},_autoSelect:function(){var cF=this._getDimension();
var cy=Math.max(0,Math.min(this.__mw-this.__ms.left,cF.width))+this.__mp.left;
var cx=Math.max(0,Math.min(this.__mx-this.__ms.top,cF.height))+this.__mp.top;
if(this.__mq===cy&&this.__mr===cx){return;
}this.__mq=cy;
this.__mr=cx;
var cH=this._getAnchorItem();
var cA=cH;
var cD=this.__my;
var cG,cz;

while(cD!==0){cG=cD>0?this._getRelatedSelectable(cA,bu):this._getRelatedSelectable(cA,bk);
if(cG!==null){cz=this._getSelectableLocationX(cG);
if((cD>0&&cz.left<=cy)||(cD<0&&cz.right>=cy)){cA=cG;
continue;
}}break;
}var cE=this.__mz;
var cC,cB;

while(cE!==0){cC=cE>0?this._getRelatedSelectable(cA,W):this._getRelatedSelectable(cA,bs);
if(cC!==null){cB=this._getSelectableLocationY(cC);
if((cE>0&&cB.top<=cx)||(cE<0&&cB.bottom>=cx)){cA=cC;
continue;
}}break;
}var cI=this.getMode();

if(cI===Y){this._selectItemRange(cH,cA);
}else if(cI===ba){if(this.isItemSelected(cH)){this._selectItemRange(cH,cA,true);
}else{this._deselectItemRange(cH,cA);
}this._setAnchorItem(cA);
}this._fireChange(br);
},__mF:{Home:1,Down:1,Right:1,PageDown:1,End:1,Up:1,Left:1,PageUp:1},handleKeyPress:function(event){var H,G;
...
if(J===bi&&D){if(I!==bc&&I!==bd){this._selectAllItems();
F=true;
}}else if(J===bl){if(I!==bc&&I!==bd){this._clearSelection();
F=true;
}}else if(J===bj){var C=this.getLeadItem();

if(C&&!E){if(D||I===ba){this._toggleInSelection(C);
}else{this._setSelectedItem(C);
}F=true;
}}else if(this.__mF[J]){F=true;

if(I===bc||I==bd){H=this._getSelectedItem();
}else{H=this.getLeadItem();
}
if(H!==null){switch(J){case bw:G=this._getFirstSelectable();
break;
case bx:G=this._getLastSelectable();
break;
case bB:G=this._getRelatedSelectable(H,bs);
break;
case T:G=this._getRelatedSelectable(H,W);
break;
case V:G=this._getRelatedSelectable(H,bk);
break;
case bv:G=this._getRelatedSelectable(H,bu);
break;
case X:G=this._getPage(H,true);
break;
case bz:G=this._getPage(H,false);
break;
}}else{switch(J){case bw:case T:case bv:case bz:G=this._getFirstSelectable();
break;
case bx:case bB:case V:case X:G=this._getLastSelectable();
break;
}}if(G!==null){switch(I){case bc:case bd:this._setSelectedItem(G);
break;
case ba:this._setLeadItem(G);
break;
case Y:if(E){var K=this._getAnchorItem();
...
if(F){event.stop();
this._fireChange(bn);
}},_selectAllItems:function(){var S=this.getSelectables();

for(var i=0,l=S.length;i<l;i++){this._addToSelection(S[i]);
}},_clearSelection:function(){var cJ=this.__ml;

for(var cK in cJ){this._removeFromSelection(cJ[cK]);
}this.__ml={};
},_selectItemRange:function(bJ,bK,bL){var bO=this._getSelectableRange(bJ,bK);
if(!bL){var bN=this.__ml;
var bP=this.__mG(bO);

for(var bM in bN){if(!bP[bM]){this._removeFromSelection(bN[bM]);
}}}for(var i=0,l=bO.length;i<l;i++){this._addToSelection(bO[i]);
...
}},__mG:function(w){var y={};
var x;

for(var i=0,l=w.length;i<l;i++){x=w[i];
y[this._selectableToHashCode(x)]=x;
}return y;
},_getSelectedItem:function(){for(var j in this.__ml){return this.__ml[j];
}return null;
},_setSelectedItem:function(bG){if(this._isSelectable(bG)){var bH=this.__ml;
var bI=this._selectableToHashCode(bG);

if(!bH[bI]||qx.lang.Object.hasMinLength(bH,2)){this._clearSelection();
this._addToSelection(bG);
}}},_addToSelection:function(cQ){var cR=this._selectableToHashCode(cQ);

if(!this.__ml[cR]&&this._isSelectable(cQ)){this.__ml[cR]=cQ;
this._styleSelectable(cQ,bb,true);
this.__mA=true;
}},_toggleInSelection:function(cv){var cw=this._selectableToHashCode(cv);

if(!this.__ml[cw]){this.__ml[cw]=cv;
this._styleSelectable(cv,bb,true);
}else{delete this.__ml[cw];
this._styleSelectable(cv,bb,false);
}this.__mA=true;
},_removeFromSelection:function(u){var v=this._selectableToHashCode(u);

if(this.__ml[v]!=null){delete this.__ml[v];
this._styleSelectable(u,bb,false);
this.__mA=true;
...
delete cn[co];
this._styleSelectable(cp,bb,false);
cm=true;
}}for(var co in ck){cp=cn[co]=ck[co];
this._styleSelectable(cp,bb,true);
cm=true;
}if(!cm){return false;
...
this.__mA=true;
this._fireChange();
},_fireChange:function(cP){if(this.__mA){this.__mB=cP||null;
this.fireDataEvent(be,this.getSelection());
delete this.__mA;
...
(function(){var C="vertical",B="under",A="above",z="qx.ui.core.selection.Widget",y="left",x="right";
qx.Class.define(z,{extend:qx.ui.core.selection.Abstract,construct:function(k){arguments.callee.base.call(this);
this.__lg=k;
},members:{__lg:null,_isSelectable:function(q){return q.isEnabled()&&q.isVisible()&&q.getLayoutParent()===this.__lg;
...
},_getLocation:function(){var p=this.__lg.getContentElement().getDomElement();
...
}},_getSelectableLocationY:function(r){var s=r.getBounds();

if(s){return {top:s.top,bottom:s.top+s.height};
}},_getScroll:function(){return {left:0,top:0};
},_scrollBy:function(t,u){},_scrollItemIntoView:function(v){this.__lg.scrollChildIntoView(v);
},getSelectables:function(){var K=this.__lg.getChildren();
var L=[];
var J;
...
},_getFirstSelectable:function(){var M=this.__lg.getChildren();

for(var i=0,l=M.length;i<l;i++){if(M[i].isEnabled()&&M[i].isVisible()){return M[i];
}}return null;
},_getLastSelectable:function(){var g=this.__lg.getChildren();

for(var i=g.length-1;i>0;i--){if(g[i].isEnabled()&&g[i].isVisible()){return g[i];
}}return null;
},_getRelatedSelectable:function(a,b){var e=this.__lg.getOrientation()===C;
var d=this.__lg.getChildren();
var c=d.indexOf(a);
var f;

if((e&&b===A)||(!e&&b===y)){for(var i=c-1;i>=0;i--){f=d[i];

if(f.isEnabled()&&f.isVisible()){return f;
}}}else if((e&&b===B)||(!e&&b===x)){for(var i=c+1;i<d.length;i++){f=d[i];
...
(function(){var p="horizontal",o="qx.event.type.Data",n="vertical",m="",k="qx.ui.form.List",j="Enter",h="one",g="addChildWidget",f="_applySpacing",d="Boolean",B="Integer",A="action",z="keyinput",y="addItem",x="removeChildWidget",w="_applyOrientation",v="__kY",u="single",t="keypress",s="list",q="pane",r="removeItem";
...
}},_applyOrientation:function(G,H){var I=G===p;
...
},_applySpacing:function(a,b){this.__kY.getLayout().setSpacing(a);
},_onKeyPress:function(e){if(e.getKeyIdentifier()==j&&!e.isAltPressed()){var F=this.getSelection();

for(var i=0;i<F.length;i++){F[i].fireEvent(A);
}return true;
}return false;
...
if(!(P===u||P===h)){return;
}if(((new Date).valueOf()-this.__lb)>1000){this.__la=m;
}this.__la+=e.getChar();
var Q=this.findItemByLabelFuzzy(this.__la);
if(Q){this.setSelection([Q]);
}this.__lb=(new Date).valueOf();
},findItemByLabelFuzzy:function(C){C=C.toLowerCase();
var D=this.getChildren();
for(var i=0,l=D.length;i<l;i++){var E=D[i].getLabel();
if(E&&E.toLowerCase().indexOf(C)==0){return D[i];
}}return null;
...
if(O.getLabel()!=null){N=O.getLabel();

if(N.translate){N=N.translate();
}
...
(function(){var o="changeSelection",n="change",m="qx.data.Array",l="qx.data.controller.MSelection",k="_applySelection",j="target";
qx.Mixin.define(l,{construct:function(){if(!qx.Class.hasProperty(this.constructor,j)){throw new Error("Target property is needed.");
}if(this.getSelection()==null){this.setSelection(new qx.data.Array());
}},properties:{selection:{check:m,event:o,apply:k,init:null}},members:{_modifingSelection:0,__ly:null,__lz:null,_applySelection:function(g,h){if(this.__lz!=undefined&&h!=undefined){h.removeListenerById(this.__lz);
}this.__lz=g.addListener(n,this.__lA,this);
...
}}this.fireDataEvent(o,this.getSelection());
},_addChangeTargetListener:function(c,d){if(this.__ly!=undefined&&d!=undefined){d.removeListenerById(this.__ly);
}
if(c!=null){if(this.__lB()||this.__lC()){this.__ly=c.addListener(o,this._changeTargetSelection,this);
...
},__lB:function(){var a=this.getTarget().constructor;
return qx.Class.implementsInterface(a,qx.ui.core.IMultiSelection);
},__lC:function(){var b=this.getTarget().constructor;
return qx.Class.implementsInterface(b,qx.ui.core.ISingleSelection);
},__lD:function(p){var q=this.__lE(p);
if(q==null){return;
}if(this.__lB()){this.getTarget().addToSelection(q);
}else if(this.__lC()){this.getTarget().setSelection([q]);
}},__lE:function(e){var f=this.getTarget().getSelectables();
for(var i=0;i<f.length;i++){if(f[i].getModel()==e){return f[i];
}}return null;
...
(function(){var bN="change",bM="ReverseBindingId",bL="BindingId",bK="",bJ="]",bI="model[",bH="String",bG=".",bF="changeModel",bE="_applyLabelOptions",cb="_applyLabelPath",ca="changeTarget",bY="changeLength",bX="_applyModel",bW="icon",bV="qx.data.controller.List",bU="_applyIconPath",bT="_applyDelegate",bS="changeDelegate",bR="_applyTarget",bP="qx.data.IListData",bQ="label",bO="_applyIconOptions";
qx.Class.define(bV,{extend:qx.core.Object,include:qx.data.controller.MSelection,construct:function(bz,bA,bB){arguments.callee.base.call(this);
this.__nM=[];
this.__nN=[];
this.__nO=[];
this.__nP={};

if(bB!=null){this.setLabelPath(bB);
}
if(bz!=null){this.setModel(bz);
}
if(bA!=null){this.setTarget(bA);
}},properties:{model:{check:bP,apply:bX,event:bF,nullable:true},target:{apply:bR,event:ca,nullable:true,init:null},labelPath:{check:bH,apply:cb,nullable:true},iconPath:{check:bH,apply:bU,nullable:true},labelOptions:{apply:bE,nullable:true},iconOptions:{apply:bO,nullable:true},delegate:{apply:bT,event:bS,init:null,nullable:true}},members:{__nQ:null,__nR:null,__nM:null,__nP:null,__nN:null,__nO:null,update:function(){this.__nT();
this.__nX();
this._updateSelection();
},_applyDelegate:function(bm,bn){this._setConfigureItem(bm,bn);
this._setFilter(bm,bn);
this._setCreateItem(bm,bn);
this._setBindItem(bm,bn);
},_applyIconOptions:function(h,k){this.__nX();
},_applyLabelOptions:function(d,e){this.__nX();
},_applyIconPath:function(bC,bD){this.__nX();
},_applyLabelPath:function(u,v){this.__nX();
},_applyModel:function(w,x){if(x!=undefined){if(this.__nQ!=undefined){x.removeListenerById(this.__nQ);
}
if(this.__nR!=undefined){x.removeListenerById(this.__nR);
}}if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length);
}if(w!=null){this.__nQ=w.addListener(bY,this.__nT,this);
this.__nR=w.addListener(bN,this.__nS,this);
this.__oa();
this.__nT();
if(this.getTarget()!=null){var z=this.getModel();
var A=this.getTarget().getChildren();

for(var i=0,l=this.__nM.length;i<l;i++){var B=z.getItem(this.__ob(i));
var y=A[i];
y.setModel(B);
}}this._changeTargetSelection();
}},_applyTarget:function(bi,bj){this._addChangeTargetListener(bi,bj);
if(bj!=undefined){bj.removeAll();
this.removeAllBindings();
}
if(bi!=null){if(this.getModel()!=null){for(var i=0;i<this.__nM.length;i++){this.__nV(this.__ob(i));
...
var bp=this.__nM.length;
var bo=this.getTarget().getChildren().length;
if(bp>bo){for(var j=bo;j<bp;j++){this.__nV(this.__ob(j));
}}else if(bp<bo){for(var j=bo;j>bp;j--){this.__nW();
}}},__nU:function(){var t=this.getModel();
if(t!=null){t.removeListenerById(this.__nR);
this.__nR=t.addListener(bN,this.__nS,this);
...
},__nV:function(f){var g=this._createItem();
g.setModel(this.getModel().getItem(f)||null);
this._bindListItem(g,f);
this.getTarget().add(g);
...
be.destroy();
this._endSelectionModification();
},getVisibleModels:function(){var J=[];
var K=this.getTarget();

if(K!=null){var L=K.getChildren();

for(var i=0;i<L.length;i++){J.push(L[i].getModel());
...
}},bindDefaultProperties:function(bu,bv){this.bindProperty(this.getLabelPath(),bQ,this.getLabelOptions(),bu,bv);
if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),bW,this.getIconOptions(),bu,bv);
}},bindProperty:function(m,n,o,p,q){p.setModel(this.getModel().getItem(q));
if(o!=null){var o=qx.lang.Object.clone(o);
this.__nP[n]=o.onUpdate;
delete o.onUpdate;
}else{o={};
this.__nP[n]=null;
...
if(m!=null&&m!=bK){r+=bG+m;
}var s=this.bind(r,p,n,o);
p.setUserData(n+bL,s);
if(!qx.lang.Array.contains(this.__nN,n)){this.__nN.push(n);
}},bindPropertyReverse:function(C,D,E,F,G){var H=bI+G+bJ;

if(C!=null&&C!=bK){H+=bG+C;
}var I=F.bind(D,this,H,E);
F.setUserData(C+bM,I);
if(!qx.lang.Array.contains(this.__nO,C)){this.__nO.push(C);
}},_onBindingSet:function(bq,br,bs){if(this.getModel()==null||this._inSelectionModification()){return;
}for(var i=0;i<this.__nN.length;i++){if(this.__nP[this.__nN[i]]!=null){this.__nP[this.__nN[i]]();
}}var bt=this.getModel().getItem(bq);
bs.setModel(bt==undefined?null:bt);
},_removeBindingsFrom:function(V){for(var i=0;i<this.__nN.length;i++){var W=V.getUserData(this.__nN[i]+bL);

if(W!=null){this.removeBinding(W);
}}for(var i=0;i<this.__nO.length;i++){var W=V.getUserData(this.__nO[i]+bM);
...
for(var i=0;i<O.length;i++){this._removeBindingsFrom(O[i]);
this._bindListItem(O[i],this.__ob(i));
}this.__nU();
},_setConfigureItem:function(bw,bx){if(bw!=null&&bw.configureItem!=null&&this.getTarget()!=null){var by=this.getTarget().getChildren();

for(var i=0;i<by.length;i++){bw.configureItem(by[i]);
}}},_setBindItem:function(bk,bl){if(bk!=null&&bk.bindItem!=null){if(bl!=null&&bl.bindItem!=null&&bk.bindItem==bl.bindItem){return;
}this.__nX();
}},_setCreateItem:function(X,Y){if(this.getTarget()==null||this.getModel()==null||X==null||X.createItem==null){return;
...
this._endSelectionModification();
this._updateSelection();
},_setFilter:function(P,Q){if((P==null||P.filter==null)&&(Q!=null&&Q.filter!=null)){this.__nY();
...
this.__oa();
if(S.length>this.__nM.length){for(var j=S.length;j>this.__nM.length;j--){this.getTarget().removeAt(j-1);
}}else if(S.length<this.__nM.length){for(var j=S.length;j<this.__nM.length;j++){var T=this._createItem();
this.getTarget().add(T);
}}var R=this.getTarget().getChildren();

for(var i=0;i<R.length;i++){this._bindListItem(R[i],this.__ob(i));
}this.__nU();
this._endSelectionModification();
this._updateSelection();
...
this.__nX();
qx.ui.core.queue.Widget.add(this);
},__oa:function(){var b=this.getModel();
...
for(var i=0;i<b.getLength();i++){if(a==null||a(b.getItem(i))){this.__nM.push(i);
}}},__ob:function(bb){return this.__nM[bb];
...
(function(){var x="configured",w="completed",v="changeState",u="changeModel",t="qx.data.store.Json",s="GET",r="_marshaler",q="sending",p="application/json",o="changeUrl",h="failed",n="loaded",k="timeout",g="queued",f="String",j="aborted",i="_applyUrl",l="receiving",e="__rS",m="qx.event.type.Data";
qx.Class.define(t,{extend:qx.core.Object,construct:function(y,z){arguments.callee.base.call(this);
this._marshaler=new qx.data.marshal.Json(z);
this._delegate=z;

if(y!=null){this.setUrl(y);
}},events:{"loaded":m},properties:{model:{nullable:true,event:u},state:{check:[x,g,q,l,w,j,k,h],init:x,event:v},url:{check:f,apply:i,event:o}},members:{__rS:null,_delegate:null,_applyUrl:function(D,E){if(D!=null){this._createRequest(D);
}},_createRequest:function(B){this.__rS=new qx.io.remote.Request(B,s,p);
var C=this._delegate;

if(C&&qx.lang.Type.isFunction(C.configureRequest)){this._delegate.configureRequest(this.__rS);
}this.__rS.addListener(w,this.__rT,this);
this.__rS.addListener(v,function(d){this.setState(d.getData());
},this);
this.__rS.send();
},__rT:function(a){var c=a.getContent();
...
this.fireDataEvent(n,this.getModel());
},reload:function(){var A=this.getUrl();

if(A!=null){this._createRequest(A);
}}},destruct:function(){this._disposeObjects(r,e);
this._delegate=null;
}});
})();
...
(function(){var x="qx.data.model.",w="-",v="",u='"',t="change",s="qx.data.marshal.Json",r="set",q="_applyEventPropagation";
qx.Class.define(s,{extend:qx.core.Object,implement:[qx.data.marshal.IMarshaler],construct:function(y){arguments.callee.base.call(this);
this.__rU=y;
},statics:{__rV:null,createModel:function(z,A){if(this.__rV===null){this.__rV=new qx.data.marshal.Json();
}this.__rV.toClass(z,A);
return this.__rV.toModel(z);
}},members:{__rU:null,__rW:function(B){var C=[];

for(var D in B){C.push(D);
}return C.sort().join(u);
...
if(this.__rU&&this.__rU.getModelClass&&this.__rU.getModelClass(d)!=null){return;
}for(var h in a){this.toClass(a[h],b);
}if(qx.Class.isDefined(x+d)){return;
}var j={};

for(var h in a){h=h.replace(w,v);
j[h]={};
j[h].nullable=true;
j[h].event=t+qx.lang.String.firstUp(h);

if(b){j[h].apply=q;
}}if(this.__rU&&this.__rU.getModelSuperClass){var g=this.__rU.getModelSuperClass(d)||qx.core.Object;
}else{var g=qx.core.Object;
}var e=[];

if(this.__rU&&this.__rU.getModelMixins){var f=this.__rU.getModelMixins(d);
...
},__rX:function(E){var F;
if(this.__rU&&this.__rU.getModelClass){F=this.__rU.getModelClass(E);
}
if(F!=null){return (new F());
}else{var G=qx.Class.getByName(x+E);
return (new G());
}},toModel:function(k){if(qx.lang.Type.isNumber(k)||qx.lang.Type.isString(k)||qx.lang.Type.isBoolean(k)||qx.lang.Type.isDate(k)||k==null){return k;
}else if(qx.lang.Type.isArray(k)){var o=new qx.data.Array();

for(var i=0;i<k.length;i++){o.push(this.toModel(k[i]));
}return o;
}else if(qx.lang.Type.isObject(k)){var l=this.__rW(k);
var p=this.__rX(l);
for(var n in k){var m=n.replace(w,v);
p[r+qx.lang.String.firstUp(m)](this.toModel(k[n]));
}return p;
}throw new Error("Unsupported type!");
}},destruct:function(){this.__rU=null;
}});
})();
(function(){var D="Boolean",C="qx.event.type.Event",B="queued",A="String",z="sending",y="qx.io.remote.Response",x="receiving",w="aborted",u="failed",t="nocache",bi="completed",bh="POST",bg="configured",bf="timeout",be="GET",bd="Pragma",bc="no-url-params-on-post",bb="no-cache",ba="Cache-Control",Y="Content-Type",K="text/plain",L="application/xml",I="application/json",J="text/html",G="application/x-www-form-urlencoded",H="qx.io.remote.Exchange",E="Integer",F="X-Qooxdoo-Response-Type",M="HEAD",N="qx.io.remote.Request",Q="_applyResponseType",P="_applyState",S="text/javascript",R="changeState",U="PUT",T="_applyProhibitCaching",O="",X="_applyMethod",W="DELETE",V="boolean";
qx.Class.define(N,{extend:qx.core.Object,construct:function(bp,bq,br){arguments.callee.base.call(this);
this.__pJ={};
this.__pK={};
this.__pL={};
this.__pM={};
...
},events:{"created":C,"configured":C,"sending":C,"receiving":C,"completed":y,"aborted":y,"failed":y,"timeout":y},statics:{__pN:0},properties:{url:{check:A,init:O},method:{check:[be,bh,U,M,W],apply:X,init:be},asynchronous:{check:D,init:true},data:{check:A,nullable:true},username:{check:A,nullable:true},password:{check:A,nullable:true},state:{check:[bg,B,z,x,bi,w,bf,u],init:bg,apply:P,event:R},responseType:{check:[K,S,I,L,J],init:K,apply:Q},timeout:{check:E,nullable:true},prohibitCaching:{check:function(v){return typeof v==V||v===bc;
},init:true,apply:T},crossDomain:{check:D,init:false},fileUpload:{check:D,init:false},transport:{check:H,nullable:true},useBasicHttpAuth:{check:D,init:false}},members:{__pJ:null,__pK:null,__pL:null,__pM:null,__pN:null,send:function(){qx.io.remote.RequestQueue.getInstance().add(this);
},abort:function(){qx.io.remote.RequestQueue.getInstance().abort(this);
},reset:function(){switch(this.getState()){case z:case x:this.error("Aborting already sent request!");
case B:this.abort();
break;
...
},isReceiving:function(){return this.getState()===x;
},isCompleted:function(){return this.getState()===bi;
},isAborted:function(){return this.getState()===w;
},isTimeout:function(){return this.getState()===bf;
},isFailed:function(){return this.getState()===u;
},__pO:function(e){var bo=e.clone();
bo.setTarget(this);
...
this.__pO(e);
},_onreceiving:function(e){this.setState(x);
this.__pO(e);
},_oncompleted:function(e){this.setState(bi);
this.__pO(e);
this.dispose();
},_onaborted:function(e){this.setState(w);
this.__pO(e);
this.dispose();
},_ontimeout:function(e){this.setState(bf);
this.__pO(e);
this.dispose();
},_onfailed:function(e){this.setState(u);
this.__pO(e);
this.dispose();
},_applyState:function(b,c){{};
},_applyProhibitCaching:function(bs,bt){if(!bs){this.removeParameter(t);
this.removeRequestHeader(bd);
this.removeRequestHeader(ba);
return;
}if(bs!==bc||this.getMethod()!=bh){this.setParameter(t,new Date().valueOf());
}else{this.removeParameter(t);
}this.setRequestHeader(bd,bb);
this.setRequestHeader(ba,bb);
},_applyMethod:function(h,i){if(h===bh){this.setRequestHeader(Y,G);
}else{this.removeRequestHeader(Y);
}var j=this.getProhibitCaching();
this._applyProhibitCaching(j,j);
},_applyResponseType:function(k,l){this.setRequestHeader(F,k);
},setRequestHeader:function(bm,bn){this.__pJ[bm]=bn;
},removeRequestHeader:function(a){delete this.__pJ[a];
},getRequestHeader:function(m){return this.__pJ[m]||null;
},getRequestHeaders:function(){return this.__pJ;
},setParameter:function(bj,bk,bl){if(bl){this.__pL[bj]=bk;
}else{this.__pK[bj]=bk;
}},removeParameter:function(r,s){if(s){delete this.__pL[r];
}else{delete this.__pK[r];
}},getParameter:function(o,p){if(p){return this.__pL[o]||null;
}else{return this.__pK[o]||null;
}},getParameters:function(n){return (n?this.__pL:this.__pK);
},setFormField:function(d,f){this.__pM[d]=f;
},removeFormField:function(g){delete this.__pM[g];
},getFormField:function(q){return this.__pM[q]||null;
...
(function(){var x="Integer",w="aborted",v="_onaborted",u="_on",t="_applyEnabled",s="Boolean",r="sending",q="__mI",p="interval",o="__mK",j="failed",n="qx.io.remote.RequestQueue",m="timeout",h="completed",g="queued",l="receiving",k="singleton";
...
this.__mK.addListener(p,this._oninterval,this);
},properties:{enabled:{init:true,check:s,apply:t},maxTotalRequests:{check:x,nullable:true},maxConcurrentRequests:{check:x,init:qx.bom.client.Transport.getMaxConcurrentRequestCount()},defaultTimeout:{check:x,init:5000}},members:{__mH:null,__mI:null,__mJ:null,__mK:null,getRequestQueue:function(){return this.__mH;
...
try{if(M[v]){var event=qx.event.Registration.createEvent(w,qx.event.type.Event);
M[v](event);
...
}this.__mK.setEnabled(a);
},add:function(f){f.setState(g);

if(f.isAsynchronous()){this.__mH.push(f);
}else{this.__mH.unshift(f);
}this._check();

if(this.getEnabled()){this.__mK.start();
}},abort:function(c){var d=c.getTransport();

if(d){d.abort();
}else if(qx.lang.Array.contains(this.__mH,c)){qx.lang.Array.remove(this.__mH,c);
}}},destruct:function(){this._disposeArray(q);
this._disposeObjects(o);
this.__mH=null;
}});
})();
(function(){var U="failed",T="sending",S="completed",R="receiving",Q="aborted",P="timeout",O="qx.io.remote.Response",N="Connection dropped",M="configured",L="qx.event.type.Event",bQ="Proxy authentication required",bP="qx.io.remote.transport.Abstract",bO="MSHTML-specific HTTP status code",bN="Not available",bM="Precondition failed",bL="Server error",bK="Moved temporarily",bJ="qx.io.remote.Exchange",bI="Bad gateway",bH="Gone",bc="See other",bd="Partial content",ba="Server timeout",bb="qx.io.remote.transport.Script",X="HTTP version not supported",Y="Unauthorized",V="Multiple choices",W="Payment required",bg="Not implemented",bh="Request-URL too large",bp="Length required",bn="_applyState",bx="changeState",bs="Not modified",bD="qx.io.remote.Request",bB="Connection closed by server",bj="Moved permanently",bG="_applyImplementation",bF="Method not allowed",bE="Forbidden",bi="Use proxy",bl="Ok",bm="Not found",bo="Not acceptable",bq="Request time-out",bt="Bad request",by="Conflict",bC="No content",be="qx.io.remote.transport.XmlHttp",bf="qx.io.remote.transport.Iframe",bk="Request entity too large",bw="Unknown status code",bv="Unsupported media type",bu="Gateway time-out",bA="created",bz="Out of resources",br="undefined";
qx.Class.define(bJ,{extend:qx.core.Object,construct:function(k){arguments.callee.base.call(this);
this.setRequest(k);
k.setTransport(this);
},events:{"sending":L,"receiving":L,"completed":O,"aborted":O,"failed":O,"timeout":O},statics:{typesOrder:[be,bf,bb],typesReady:false,typesAvailable:{},typesSupported:{},registerType:function(m,n){qx.io.remote.Exchange.typesAvailable[n]=m;
},initTypes:function(){if(qx.io.remote.Exchange.typesReady){return;
}
for(var K in qx.io.remote.Exchange.typesAvailable){var J=qx.io.remote.Exchange.typesAvailable[K];

if(J.isSupported()){qx.io.remote.Exchange.typesSupported[K]=J;
...
},_nativeMap:{0:bA,1:M,2:T,3:R,4:S},wasSuccessful:function(s,t,u){if(u){switch(s){case null:case 0:return true;
case -1:return t<4;
default:return typeof s===br;
...
return false;
}}},statusCodeToString:function(q){switch(q){case -1:return bN;
case 200:return bl;
case 304:return bs;
case 206:return bd;
case 204:return bC;
...
default:return bw;
}}},properties:{request:{check:bD,nullable:true},implementation:{check:bP,nullable:true,apply:bG},state:{check:[M,T,R,S,Q,P,U],init:M,event:bx,apply:bn}},members:{send:function(){var y=this.getRequest();
...
},abort:function(){var D=this.getImplementation();

if(D){{};
D.abort();
}else{{};
this.setState(Q);
}},timeout:function(){var E=this.getImplementation();

if(E){this.warn("Timeout: implementation "+E.toHashCode());
E.timeout();
}else{this.warn("Timeout: forcing state to timeout");
this.setState(P);
}this.__oN();
},__oN:function(){var r=this.getRequest();

if(r){r.setTimeout(0);
}},_onsending:function(e){this.setState(T);
},_onreceiving:function(e){this.setState(R);
},_oncompleted:function(e){this.setState(S);
},_onabort:function(e){this.setState(Q);
},_onfailed:function(e){this.setState(U);
},_ontimeout:function(e){this.setState(P);
},_applyImplementation:function(a,b){if(b){b.removeListener(T,this._onsending,this);
b.removeListener(R,this._onreceiving,this);
b.removeListener(S,this._oncompleted,this);
b.removeListener(Q,this._onabort,this);
b.removeListener(P,this._ontimeout,this);
b.removeListener(U,this._onfailed,this);
}
...
a.addListener(T,this._onsending,this);
a.addListener(R,this._onreceiving,this);
a.addListener(S,this._oncompleted,this);
a.addListener(Q,this._onabort,this);
a.addListener(P,this._ontimeout,this);
a.addListener(U,this._onfailed,this);
}},_applyState:function(d,f){{};

switch(d){case T:this.fireEvent(T);
break;
case R:this.fireEvent(R);
break;
case S:case Q:case P:case U:var h=this.getImplementation();
...
if(d==S){var g=h.getResponseContent();
j.setContent(g);
if(g===null){{};
d=U;
}}else if(d==U){j.setContent(h.getResponseContent());
...
h.dispose();
break;
}}},settings:{"qx.ioRemoteDebug":false,"qx.ioRemoteDebugData":false},destruct:function(){var o=this.getImplementation();

if(o){this.setImplementation(null);
o.dispose();
...
qx.Class.define(e,{type:g,extend:qx.core.Object,construct:function(){arguments.callee.base.call(this);
...
(function(){var m="failed",k="completed",j="=",h="aborted",g="",f="sending",d="&",c="configured",b="timeout",a="application/xml",I="qx.io.remote.transport.XmlHttp",H="application/json",G="text/html",F="qx.client",E="receiving",D="text/plain",C="text/javascript",B="?",A="created",z='Referer',u='Basic ',v="\n</pre>",r="string",t='Authorization',p="<pre>Could not execute json: \n",q="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",n=':',o="parseerror",w="file:",x="webkit",y="object";
qx.Class.define(I,{extend:qx.io.remote.transport.Abstract,statics:{handles:{synchronous:true,asynchronous:true,crossDomain:false,fileUpload:false,programaticFormFields:false,responseTypes:[D,C,H,a,G]},requestObjects:[],requestObjectCount:0,createRequestObject:qx.core.Variant.select(F,{"default":function(){return new XMLHttpRequest;
...
var bo=this.getRequest();
var bk=this.getMethod();
var br=this.getAsynchronous();
var bq=this.getUrl();
var bm=(window.location.protocol===w&&!(/^http(s){0,1}\:/.test(bq)));
this.__pC=bm;
var bu=this.getParameters(false);
var bs=[];

for(var bl in bu){var bp=bu[bl];

if(bp instanceof Array){for(var i=0;i<bp.length;i++){bs.push(encodeURIComponent(bl)+j+encodeURIComponent(bp[i]));
}}else{bs.push(encodeURIComponent(bl)+j+encodeURIComponent(bp));
}}
if(bs.length>0){bq+=(bq.indexOf(B)>=0?d:B)+bs.join(d);
}if(this.getData()===null){var bu=this.getParameters(true);
var bs=[];
...
if(bs.length>0){this.setData(bs.join(d));
}}var bt=function(T){var Y=q;
var bd=g;
var W,V,U;
var ba,bb,bc,X;
var i=0;
...
try{if(this.getUsername()){if(this.getUseBasicHttpAuth()){bo.open(bk,bq,br);
bo.setRequestHeader(t,u+bt(this.getUsername()+n+this.getPassword()));
}else{bo.open(bk,bq,br,this.getUsername(),this.getPassword());
}}else{bo.open(bk,bq,br);
}}catch(K){this.error("Failed with exception: "+K);
this.failed();
return;
}if(!qx.core.Variant.isSet(F,x)){bo.setRequestHeader(z,window.location.href);
}var bn=this.getRequestHeaders();

for(var bl in bn){bo.setRequestHeader(bl,bn[bl]);
}try{{};
bo.send(this.getData());
}catch(be){if(bm){this.failedLocally();
}else{this.error("Failed to send data: "+be,"send");
this.failed();
}return;
}if(!br){this._onreadystatechange();
...
if(bf==4){if(!qx.io.remote.Exchange.wasSuccessful(this.getStatusCode(),bf,this.__pC)){if(this.getState()===c){this.setState(f);
}return this.failed();
}}while(this.__pD<bf){this.setState(qx.io.remote.Exchange._nativeMap[++this.__pD]);
}}),getReadyState:function(){var bI=null;

try{bI=this.getRequest().readyState;
}catch(O){}return bI;
},setRequestHeader:function(bg,bh){this.getRequestHeaders()[bg]=bh;
...
},getResponseHeaders:function(){var R=this.getStringResponseHeaders();
var S={};

if(R){var P=R.split(/[\r\n]+/g);

for(var i=0,l=P.length;i<l;i++){var Q=P[i].match(/^([^:]+)\s*:\s*(.+)$/i);

if(Q){S[Q[1]]=Q[2];
}}}return S;
},getStatusCode:function(){var L=-1;

try{L=this.getRequest().status;
}catch(bJ){}return L;
},getStatusText:function(){var bF=g;
...
if(qx.io.remote.Exchange.wasSuccessful(bO,bP,this.__pC)){try{bQ=this.getRequest().responseXML;
}catch(N){}}if(typeof bQ==y&&bQ!=null){if(!bQ.documentElement){var s=String(this.getRequest().responseText).replace(/<\?xml[^\?]*\?>/,g);
bQ.loadXML(s);
}if(!bQ.documentElement){throw new Error("Missing Document Element!");
}
if(bQ.documentElement.tagName==o){throw new Error("XML-File is not well-formed!");
}}else{throw new Error("Response was not a valid xml document ["+this.getRequest().responseText+"]");
}return bQ;
},getFetchedLength:function(){var bz=this.getResponseText();
return typeof bz==r?bz.length:0;
},getResponseContent:function(){var bB=this.getState();

if(bB!==k&&bB!=m){{};
return null;
}{};
var bD=this.getResponseText();

if(bB==m){{};
return bD;
}
switch(this.getResponseType()){case D:case G:{};
return bD;
case H:{};

try{if(bD&&bD.length>0){var bC=qx.util.Json.parse(bD,false);
return (bC===0?0:(bC||null));
}else{return null;
}}catch(bK){this.error("Could not execute json: ["+bD+"]",bK);
return p+bD+v;
}case C:{};

try{if(bD&&bD.length>0){var bC=window.eval(bD);
return (bC===0?0:(bC||null));
}else{return null;
}}catch(bv){this.error("Could not execute javascript: ["+bD+"]",bv);
return null;
}case a:bD=this.getResponseXml();
{};
...
switch(bi){case A:this.fireEvent(A);
break;
...
this.fireEvent(b);
break;
}}},defer:function(bM,bN){qx.io.remote.Exchange.registerType(qx.io.remote.transport.XmlHttp,I);
},destruct:function(){var bA=this.getRequest();

if(bA){bA.onreadystatechange=qx.lang.Function.empty;
switch(bA.readyState){case 1:case 2:case 3:bA.abort();
}}this.__pE=null;
}});
})();
...
(function(){var o="=",n="&",m="application/xml",l="application/json",k="text/html",j="qx.client",h="textarea",g="none",f="text/plain",d="text/javascript",I="",H="completed",G="?",F="qx.io.remote.transport.Iframe",E="gecko",D="frame_",C="aborted",B="_data_",A="pre",z="javascript:void(0)",v="sending",w="form",t="failed",u='<iframe name="',r="mshtml",s="form_",p='"></iframe>',q="iframe",x="timeout",y="qx/static/blank.gif";
qx.Class.define(F,{extend:qx.io.remote.transport.Abstract,construct:function(){arguments.callee.base.call(this);
var a=(new Date).valueOf();
var b=D+a;
var c=s+a;
if(qx.core.Variant.isSet(j,r)){this.__ln=document.createElement(u+b+p);
}else{this.__ln=document.createElement(q);
}this.__ln.src=z;
this.__ln.id=this.__ln.name=b;
this.__ln.onload=qx.lang.Function.bind(this._onload,this);
this.__ln.style.display=g;
document.body.appendChild(this.__ln);
this.__lo=document.createElement(w);
this.__lo.target=b;
this.__lo.id=this.__lo.name=c;
this.__lo.style.display=g;
document.body.appendChild(this.__lo);
this.__lp=document.createElement(h);
this.__lp.id=this.__lp.name=B;
this.__lo.appendChild(this.__lp);
this.__ln.onreadystatechange=qx.lang.Function.bind(this._onreadystatechange,this);
},statics:{handles:{synchronous:false,asynchronous:true,crossDomain:false,fileUpload:true,programaticFormFields:true,responseTypes:[f,d,l,m,k]},isSupported:function(){return true;
},_numericMap:{"uninitialized":1,"loading":2,"loaded":2,"interactive":3,"complete":4}},members:{__lp:null,__lq:0,__lo:null,__ln:null,send:function(){var O=this.getMethod();
var Q=this.getUrl();
...
if(R instanceof Array){for(var i=0;i<R.length;i++){T.push(encodeURIComponent(P)+o+encodeURIComponent(R[i]));
}}else{T.push(encodeURIComponent(P)+o+encodeURIComponent(R));
}}
if(T.length>0){Q+=(Q.indexOf(G)>=0?n:G)+T.join(n);
...
if(T.length>0){this.setData(T.join(n));
}}var N=this.getFormFields();

for(var P in N){var S=document.createElement(h);
S.name=P;
S.appendChild(document.createTextNode(N[P]));
this.__lo.appendChild(S);
}this.__lo.action=Q;
this.__lo.method=O;
...
}),_switchReadyState:function(J){switch(this.getState()){case H:case C:case t:case x:this.warn("Ignore Ready State Change");
return;
}while(this.__lq<J){this.setState(qx.io.remote.Exchange._nativeMap[++this.__lq]);
}},setRequestHeader:function(X,Y){},getResponseHeader:function(bd){return null;
...
if(!K.firstChild){return I;
}if(K.firstChild.tagName&&K.firstChild.tagName.toLowerCase()==A){return K.firstChild.innerHTML;
...
return L;
break;
case k:L=this.getIframeHtmlContent();
{};
return L;
break;
case l:L=this.getIframeHtmlContent();
{};

try{return L&&L.length>0?qx.util.Json.parse(L,false):null;
}catch(W){return this.error("Could not execute json: ("+L+")",W);
}case d:L=this.getIframeHtmlContent();
{};

try{return L&&L.length>0?window.eval(L):null;
}catch(V){return this.error("Could not execute javascript: ("+L+")",V);
}case m:L=this.getIframeDocument();
{};
return L;
default:this.warn("No valid responseType specified ("+this.getResponseType()+")!");
return null;
}}},defer:function(ba,bb,bc){qx.io.remote.Exchange.registerType(qx.io.remote.transport.Iframe,F);
},destruct:function(){if(this.__ln){this.__ln.onload=null;
this.__ln.onreadystatechange=null;
if(qx.core.Variant.isSet(j,E)){this.__ln.src=qx.util.ResourceManager.getInstance().toUri(y);
...
})},members:{canHandleEvent:function(e,f){return e.tagName.toLowerCase()===g;
},registerEvent:function(k,l,m){},unregisterEvent:function(b,c,d){}},defer:function(a){qx.event.Registration.addHandler(a);
}});
})();
(function(){var k="qx.client",j="webkit",i="body",h="iframe",g="qx.bom.Iframe";
qx.Class.define(g,{statics:{DEFAULT_ATTRIBUTES:{onload:"qx.event.handler.Iframe.onevent(this)",frameBorder:0,frameSpacing:0,marginWidth:0,marginHeight:0,hspace:0,vspace:0,border:0,allowTransparency:true},create:function(A,B){var A=A?qx.lang.Object.clone(A):{};
var C=qx.bom.Iframe.DEFAULT_ATTRIBUTES;

for(var D in C){if(A[D]==null){A[D]=C[D];
}}return qx.bom.Element.create(h,A,B);
},getWindow:qx.core.Variant.select(k,{"mshtml|gecko":function(r){try{return r.contentWindow;
}catch(e){return null;
}},"default":function(l){try{var m=this.getDocument(l);
return m?m.defaultView:null;
}catch(a){return null;
}}}),getDocument:qx.core.Variant.select(k,{"mshtml":function(v){try{var w=this.getWindow(v);
return w?w.document:null;
}catch(x){return null;
}},"default":function(s){try{return s.contentDocument;
}catch(q){return null;
}}}),getBody:function(y){try{var z=this.getDocument(y);
return z?z.getElementsByTagName(i)[0]:null;
}catch(u){return null;
}},setSource:function(n,o){try{if(this.getWindow(n)&&qx.dom.Hierarchy.isRendered(n)){try{if(qx.core.Variant.isSet(k,j)&&qx.bom.client.Platform.MAC){var p=this.getContentWindow();
...
var I=++qx.io.remote.transport.Script.__lF;

if(I>=2000000000){qx.io.remote.transport.Script.__lF=I=1;
}this.__lG=null;
this.__lF=I;
},statics:{__lF:0,_instanceRegistry:{},ScriptTransport_PREFIX:r,ScriptTransport_ID_PARAM:n,ScriptTransport_DATA_PARAM:t,handles:{synchronous:false,asynchronous:true,crossDomain:true,fileUpload:false,programaticFormFields:false,responseTypes:[x,w,z]},isSupported:function(){return true;
...
}})},members:{__lH:0,__lG:null,__lF:null,send:function(){var g=this.getUrl();
g+=(g.indexOf(A)>=0?C:A)+qx.io.remote.transport.Script.ScriptTransport_ID_PARAM+B+this.__lF;
...
if(h instanceof Array){for(var i=0;i<h.length;i++){j.push(encodeURIComponent(f)+B+encodeURIComponent(h[i]));
}}else{j.push(encodeURIComponent(f)+B+encodeURIComponent(h));
}}
if(j.length>0){g+=C+j.join(C);
}var e=this.getData();

if(e!=null){g+=C+qx.io.remote.transport.Script.ScriptTransport_DATA_PARAM+B+encodeURIComponent(e);
}qx.io.remote.transport.Script._instanceRegistry[this.__lF]=this;
this.__lG=document.createElement(o);
this.__lG.charset=q;
this.__lG.src=g;
{};
document.body.appendChild(this.__lG);
},_switchReadyState:function(D){switch(this.getState()){case y:case m:case p:case s:this.warn("Ignore Ready State Change");
return;
}while(this.__lH<D){this.setState(qx.io.remote.Exchange._nativeMap[++this.__lH]);
}},setRequestHeader:function(E,F){},getResponseHeader:function(l){return null;
},getResponseHeaders:function(){return {};
},getStatusCode:function(){return 200;
...
default:this.warn("No valid responseType specified ("+this.getResponseType()+")!");
return null;
}}},defer:function(a,b,c){qx.io.remote.Exchange.registerType(qx.io.remote.transport.Script,v);
