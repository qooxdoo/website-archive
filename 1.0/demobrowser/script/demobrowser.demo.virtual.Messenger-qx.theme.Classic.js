...
(function(){var m="execute",l="middle",k="",j="away",i="offline",h="value",g="online",f="icon",d="busy",c="selection[0].status",W="right",V="selection[0]",U="separator-vertical",T="Escape",S="Cancel",R="qooxdoo",Q="Group: ",P="Friends",O="Name:",N="Enter",t="source",u="Group:",r="demobrowser/demo/icons/imicons/user_delete.png",s="center",p="enabled",q="icon/16/actions/list-add.png",n="group",o="Name: ",v="name",w="keypress",D="modelSelection",B="top",H="Contacts",F="Status: ",J="selection",I="Add",y="Contact Details",M="Add user",L="model",K="appear",x="New Group",z="avatar",A="left",C="Avatar: ",E="demobrowser.demo.virtual.Messenger",G="demobrowser/demo/icons/imicons/user_add.png";
qx.Class.define(E,{extend:qx.application.Standalone,members:{__JB:null,messenger:null,__JC:null,main:function(){arguments.callee.base.call(this);
var bc=this.getRoot();
var bf=new qx.ui.window.Window(H).set({contentPadding:0,showClose:false,showMinimize:false});
...
var bd=demobrowser.demo.virtual.messenger.BuddyModel.createBuddies(200);
this.messenger.setModel(bd);
bf.add(this.createToolbar());
bf.add(this.messenger,{flex:1});
bc.add(this.createDetailsView(),{left:20,top:20});
},createToolbar:function(){var toolbar=new qx.ui.toolbar.ToolBar();
var ba=new qx.ui.toolbar.Part();
toolbar.add(ba);
var X=new qx.ui.toolbar.Button(k,G).set({show:f});
X.addListener(m,this.showAddContactWindow,this);
ba.add(X);
var Y=new qx.ui.toolbar.Button(k,r).set({show:f});
this.messenger.bind(V,Y,p,{converter:function(bb){return !!bb;
}});
Y.addListener(m,function(){this.messenger.getModel().remove(this.messenger.getSelection().getItem(0));
},this);
ba.add(Y);
return ba;
},createDetailsView:function(){var bk=new qx.data.controller.Object();
this.messenger.bind(V,bk,L);
var bh=new qx.ui.groupbox.GroupBox(y);
var bl=new qx.ui.layout.Grid(5,5);
bl.setColumnAlign(0,W,l);
bh.setLayout(bl);
bh.add(new qx.ui.basic.Label(o),{row:0,column:0});
var bn=new qx.ui.form.TextField();
bk.addTarget(bn,h,v,true);
bh.add(bn,{row:0,column:1});
bh.add(new qx.ui.basic.Label(Q),{row:1,column:0});
var bi=new qx.ui.form.TextField();
bk.addTarget(bi,h,n,true);
bh.add(bi,{row:1,column:1});
bh.add(new qx.ui.basic.Label(F),{row:2,column:0});
var bm=new qx.ui.form.SelectBox();
bm.add(new qx.ui.form.ListItem(g).set({model:g}));
bm.add(new qx.ui.form.ListItem(j).set({model:j}));
bm.add(new qx.ui.form.ListItem(d).set({model:d}));
bm.add(new qx.ui.form.ListItem(i).set({model:i}));
this.messenger.bind(c,bm,D,{converter:function(bg){return [bg];
}});
bm.bind(J,this.messenger,c,{converter:function(b){return b[0].getModel();
}});
bh.add(bm,{row:2,column:1});
bh.add(new qx.ui.basic.Label(C).set({alignY:B}),{row:3,column:0});
var bj=new qx.ui.basic.Image().set({alignX:s});
bk.addTarget(bj,t,z);
bh.add(bj,{row:3,column:1});
return bh;
},showAddContactWindow:function(){if(this.__JB==null){var bp=new qx.ui.layout.Grid(5,10);
var br=new qx.ui.window.Window(M,q);
br.set({width:165,height:100,showMinimize:false,showClose:false,showMaximize:false});
br.setLayout(bp);
var bu=new qx.ui.basic.Label(O);
var bv=new qx.ui.form.TextField();
var bs=new qx.ui.form.ComboBox(x);
bs.add(new qx.ui.form.ListItem(R));
bs.add(new qx.ui.form.ListItem(P));
var bq=new qx.ui.basic.Label(u);
var bx=new qx.ui.form.Button(I);
var bw=new qx.ui.form.Button(S);
this.__JC=bv;
bx.setAllowGrowX(false);
bw.setAllowGrowX(false);
br.addListener(K,bv.focus,bv);
bw.addListener(m,br.close,br);
var bt=function(){var bo=new demobrowser.demo.virtual.messenger.BuddyModel();
bo.setName(bv.getValue());
bo.setGroup(bs.getValue());
this.messenger.getModel().push(bo);
this.messenger.getSelection().setItem(0,bo);
br.close();
};
bx.addListener(m,bt,this);
br.addListener(w,function(e){var a=e.getKeyIdentifier();

if(a==N){bt.call(this);
}else if(a==T){br.close();
}},this);
br.add(bu,{row:0,column:0});
br.add(bv,{row:0,column:1,colSpan:2});
br.add(bq,{row:1,column:0});
br.add(bs,{row:1,column:1,colSpan:2});
br.add(bx,{row:2,column:1});
br.add(bw,{row:2,column:2});
bp.setColumnAlign(0,A,l);
bp.setColumnAlign(1,W,l);
this.getRoot().add(br,{left:20,top:20});
this.__JB=br;
}this.__JC.setValue(k);
this.__JB.center();
this.__JB.open();
...
(function(){var m="model",l="changeModel",k="demobrowser.demo.virtual.messenger.Roster",j="qx.data.Array",i="rgb(238, 243, 255)",h="row-layer",g="off",f="white",e="changeSelection",d="rgb(60, 97, 226)",c="auto";
qx.Class.define(k,{extend:qx.ui.virtual.form.List,construct:function(){arguments.callee.base.call(this);
this.set({scrollbarX:g,scrollbarY:c,width:200,height:300,rowHeight:28,useWidgetCells:true,decorator:null});
var o=new demobrowser.demo.virtual.messenger.Controller(null,this);
this.bind(m,o,m);
this.setSelection(o.getSelection());
this.rowLayer=this.getChildControl(h);
this.rowLayer.set({colorEven:f,colorOdd:i});
this.getPane().addLayer(this.rowLayer);
new qx.ui.virtual.behavior.Prefetch(this,0,0,0,0,600,800,600,800).set({interval:500});
},properties:{model:{event:l,check:j,nullable:true},selection:{event:e}},members:{styleGroup:function(a){var b=d;
this.getPane().getRowConfig().setItemSize(a,15);
this.rowLayer.setColor(a,b);
...
(function(){var y="name",x="changeOpen",w="group",v="demobrowser.demo.virtual.messenger.Controller",u=".",t="cell.row";
qx.Class.define(v,{extend:qx.ui.virtual.form.ListController,construct:function(I,J){arguments.callee.base.call(this,I,J);
this.__JD=new demobrowser.demo.virtual.messenger.BuddyCell();
this.__JE=new demobrowser.demo.virtual.messenger.GroupCell();
this.__JE.addListener(x,this._onChangeOpenGroup,this);
this.__JF=[];
this.__JG={};
this.__JH=[];
this.__JI=new qx.util.DeferredCall(this._updateGrouping,this);
},members:{__JH:null,__JG:null,__JI:null,__JF:null,__JD:null,__JE:null,_getRowData:function(C){return this.__JH[C];
},_getModelRow:function(s){return this.__JH.indexOf(s);
},_getRowCount:function(){return this.__JH.length;
},_isGroupHeader:function(F){return this.__JH[F] instanceof demobrowser.demo.virtual.messenger.GroupModel;
},_getGroupModel:function(name,c){var d=this.__JG[name];

if(!d){var d=new demobrowser.demo.virtual.messenger.GroupModel();
d.setName(name);
this.__JG[name]=d;
}d.setRow(c);
return d;
},_onChangeOpenGroup:function(e){var B=this.__JH[e.getData().getUserData(t)];
B.toggleOpen();
this.__JI.schedule();
},getCellRenderer:function(z){if(this._isGroupHeader(z)){return this.__JE;
...
if(g){for(var i=0;i<this.__JF.length;i++){var h=this.__JF[i].getOldRow();
...
},_updateGrouping:function(){var p=this.getModel();
this.__JH=[];
var q=[];
var m=[];

if(p&&p.length>0){q=qx.lang.Array.clone(p.toArray());
q.sort(function(a,b){var G=a.getGroup();
var H=b.getGroup();

if(G==H){return a.getName()<b.getName()?-1:1;
}return G>H?-1:1;
});
var o=q[0];
var l=this._getGroupModel(o.getGroup(),0);
this.__JH.push(l);
m.push(l);
var r=0;

for(var i=0;i<q.length;i++){var n=q[i];

if(n.getGroup()!==l.getName()){l.setItemCount(r);
r=0;
var l=this._getGroupModel(n.getGroup(),this.__JH.length);
this.__JH.push(l);
m.push(l);
}
if(l.isOpen()){this.__JH.push(n);
}else{this.getSelection().remove(n);
}r+=1;
}}l.setItemCount(r);
this._visualizeGrouping(m);
this._syncModelSelectionToView();
this._syncRowCount();
},_applyModel:function(j,k){arguments.callee.base.call(this,j,k);
this.__JI.schedule();
},_onChangeModel:function(e){arguments.callee.base.call(this,e);
this.__JI.schedule();
},_onChangeBubbleModel:function(e){var D=e.getData();

if(D.name){var E=D.name.toString().split(u)[1];

if(E==y||E==w){this.__JI.schedule();
}}arguments.callee.base.call(this);
}}});
})();
...
}},properties:{name:{init:c,event:g,check:a},row:{event:d,check:j,apply:b},open:{check:h,init:true,event:i},itemCount:{check:j,init:0,event:e}},members:{__JJ:null,_applyRow:function(k,l){this.__JJ=l;
},getOldRow:function(){return this.__JJ;
}}});
})();
