...
(function(){var A="execute",z="A boolean",y="icon/22/actions/list-remove.png",x="A number",w="<b>HTML-Test <i>",v="icon/22/actions/edit-undo.png",u="between",t=", additional Status. All Values are shown.",s="Hide 1k-5k",r="Table with data filtering",P=", additional Status. 1k - 5k Values are hidden.",O="Show All",N=", additional Status. False Values are hidden.",M="Hide True",L="_tableModel",K="demobrowser.demo.table.Table_Filtered_Model",J=", additional Status. True Values are hidden.",I="!=",H="==",G="Hide False",E="ID",F="A date",C="HTML",D="</i></b>",B=", additional Status.";
qx.Class.define(K,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return r;
},createTable:function(){var l=this._tableModel=new qx.ui.table.model.Filtered();
l.setColumns([E,x,F,z,C]);
var m=[];
var p=new Date().getTime();
var j=400*24*60*60*1000;

for(var q=0;q<20;q++){var n=new Date(p+Math.random()*j-j/2);
var k=w+q+D;
m.push([q,Math.random()*10000,n,(Math.random()>0.5),k]);
}l.setData(m);
l.setColumnEditable(1,true);
l.setColumnEditable(2,true);
var o=this._table=new qx.ui.table.Table(l);
o.setMetaColumnCounts([1,-1]);
o.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
o.getTableColumnModel().setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
o.getTableColumnModel().setDataCellRenderer(4,new qx.ui.table.cellrenderer.Html());
o.setAdditionalStatusBarText(B);
return o;
...
var f=new qx.ui.toolbar.Button(M,y);
g.add(f);
f.addListener(A,function(e){b.addNumericFilter(H,1,z);
b.applyFilters();
i.setAdditionalStatusBarText(J);
});
var d=new qx.ui.toolbar.Button(G,y);
g.add(d);
d.addListener(A,function(e){b.addNumericFilter(I,1,z);
b.applyFilters();
i.setAdditionalStatusBarText(N);
});
var c=new qx.ui.toolbar.Button(s,y);
g.add(c);
c.addListener(A,function(e){b.addBetweenFilter(u,1000,5000,x);
b.applyFilters();
i.setAdditionalStatusBarText(P);
});
var h=new qx.ui.toolbar.Button(O,v);
g.add(h);
h.addListener(A,function(e){b.resetHiddenRows();
i.setAdditionalStatusBarText(t);
});
return a;
...
qx.Class.define(t,{extend:qx.ui.table.model.Simple,construct:function(){arguments.callee.base.call(this);
this.numericAllowed=new Array(F,w,E,B,x,v);
this.betweenAllowed=new Array(C,y);
this.__GT=false;
this.Filters=new Array();
},members:{__GU:null,__GT:null,_js_in_array:function(X,Y){var ba=Y.toString();

if(ba==s){return false;
}var bc=new RegExp(X,G);
var bb=bc.test(Y);
return bb;
},addBetweenFilter:function(h,j,k,l){if(this._js_in_array(h,this.betweenAllowed)&&l!=null){if(j!=null&&k!=null){var m=new Array(h,j,k,l);
}}
if(m!=null){this.Filters.push(m);
}else{throw new Error("Filter not recognized or value1/value2 is null!");
}},addNumericFilter:function(T,U,V){var W=null;

if(this._js_in_array(T,this.numericAllowed)&&V!=null){if(U!=null){W=[T,U,V];
}}
if(W!=null){this.Filters.push(W);
}else{throw new Error("Filter not recognized: value or target is null!");
}},addRegex:function(n,o,p){var r;

if(p){r=z;
}else{r=G;
}
if(n!=null&&o!=null){var q=new Array(A,n,o,r);
}
...
if(H!=null&&I!=null){var K=new Array(D,H,I,L);
}
...
switch(this.Filters[i][0]){case F:if(a==this.Filters[i][1]){e=true;
}break;
case w:if(a!=this.Filters[i][1]){e=true;
}break;
case E:if(a>this.Filters[i][1]){e=true;
}break;
case B:if(a<this.Filters[i][1]){e=true;
}break;
case x:if(a>=this.Filters[i][1]){e=true;
}break;
case v:if(a<=this.Filters[i][1]){e=true;
}break;
}}else if(this._js_in_array(this.Filters[i][0],this.betweenAllowed)&&e==false){a=this.getValueById(this.Filters[i][3],g);

switch(this.Filters[i][0]){case C:if(a>=this.Filters[i][1]&&a<=this.Filters[i][2]){e=true;
}break;
case y:if(a<this.Filters[i][1]&&a>this.Filters[i][2]){e=true;
}break;
}}else if(this.Filters[i][0]==A&&e==false){a=this.getValueById(this.Filters[i][2],g);
var c=new RegExp(this.Filters[i][1],this.Filters[i][3]);
e=c.test(a);
}else if(this.Filters[i][0]==D&&e==false){a=this.getValueById(this.Filters[i][2],g);
var c=new RegExp(this.Filters[i][1],this.Filters[i][3]);
e=!c.test(a);
...
},hideRows:function(O,P,dispatchEvent){var R=this.getData();
dispatchEvent=(dispatchEvent!=null?dispatchEvent:true);

if(!this.__GT){this.__GU=R.slice(0);
this.__GT=true;
}
if(P==null||P<1){P=1;
}
for(var Q=O;Q<(R.length-P);Q++){R[Q]=R[Q+P];
}this.removeRows(Q,P);
if(dispatchEvent){var S={firstRow:0,lastRow:R.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(u,S);
}},resetHiddenRows:function(){if(!this.__GU){return ;
}this.Filters=[];
this.setData(qx.lang.Array.clone(this.__GU));
},setData:function(M,N){this.__GU=qx.lang.Array.clone(M);
this.Filters=[];
arguments.callee.base.call(this,M,N);
}},destruct:function(){this.__GU=this.numericAllowed=this.betweenAllowed=this.Filters=null;
}});
})();
