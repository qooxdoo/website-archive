...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.ui.FormRendererDouble-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"52328fc4b800"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var v="execute",u="Female",t="Name",s="Registration",r="Male",q="Send",p="Bio",o="demobrowser.demo.ui.FormRendererDouble",n="Gender",m="Country",j="Personal Information",l="Save?",k="Password",i="Reset",h="Age";
qx.Class.define(o,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var b=new qx.ui.form.Form();
b.addGroupHeader(s);
...
g.setRequired(true);
b.add(g,k);
b.add(new qx.ui.form.CheckBox(),l);
b.addGroupHeader(j);
b.add(new qx.ui.form.Spinner(),h);
b.add(new qx.ui.form.TextField(),m);
var e=new qx.ui.form.RadioButtonGroup();
e.add(new qx.ui.form.RadioButton(r));
e.add(new qx.ui.form.RadioButton(u));
b.add(e,n);
b.add(new qx.ui.form.TextArea(),p);
var d=new qx.ui.form.Button(q);
d.addListener(v,function(){if(b.validate()){alert("send...");
}},this);
b.addButton(d);
var c=new qx.ui.form.Button(i);
c.addListener(v,function(){b.reset();
},this);
b.addButton(c);
var f=new qx.ui.form.renderer.Double(b);
...
this.__tt=[];
this.__tu=[];
this.__tv=new qx.ui.form.validation.Manager();
this.__tw=new qx.ui.form.Resetter();
},members:{__tt:null,__tv:null,__tx:0,__tu:null,__tw:null,add:function(f,g,h,name){if(this.__ty()){this.__tt.push({title:null,items:[],labels:[],names:[]});
}this.__tt[this.__tx].items.push(f);
this.__tt[this.__tx].labels.push(g);
if(name==null){name=g.replace(/\s+/g,a);
}this.__tt[this.__tx].names.push(name);
this.__tv.add(f,h);
this.__tw.add(f);
},addGroupHeader:function(k){if(!this.__ty()){this.__tx++;
}this.__tt.push({title:k,items:[],labels:[],names:[]});
},addButton:function(d){this.__tu.push(d);
},__ty:function(){return this.__tt.length===0;
},reset:function(){this.__tw.reset();
this.__tv.reset();
},redefineResetter:function(){this.__tw.redefine();
},validate:function(){return this.__tv.validate();
},getValidationManager:function(){return this.__tv;
},getGroups:function(){return this.__tt;
},getButtons:function(){return this.__tu;
...
for(var i=0;i<this.__tt.length;i++){var m=this.__tt[i];
for(var j=0;j<m.names.length;j++){var name=m.names[j];
l[name]=m.items[j];
}}return l;
}}});
})();
(function(){var L="",K="complete",J="changeValid",I="qx.event.type.Event",H="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",G="qx.ui.form.validation.Manager",F="This field is required",E="String",D="qx.event.type.Data";
qx.Class.define(G,{extend:qx.core.Object,construct:function(){arguments.callee.base.call(this);
this.__sV=[];
this.__sW={};
},events:{"changeValid":D,"complete":I},properties:{validator:{check:H,init:null,nullable:true},invalidMessage:{check:E,init:L}},members:{__sV:null,__sX:null,__sW:null,__sY:null,add:function(q,r){if(!this.__te(q)){throw new Error("Added widget not supported.");
}if(this.__tf(q)){if(r!=null){throw new Error("Widgets suporting selection can only be validated "+"in the form validator");
}}var s={item:q,validator:r,valid:null};
this.__sV.push(s);
},validate:function(){var U=true;
this.__sY=true;
var R=[];
for(var i=0;i<this.__sV.length;i++){var S=this.__sV[i].item;
var V=this.__sV[i].validator;
R.push(S);
if(V==null){var Q=this.__ta(S);
U=U&&Q;
this.__sY=Q&&this.__sY;
continue;
}var Q=this.__tb(this.__sV[i],S.getValue());
U=Q&&U;

if(Q!=null){this.__sY=Q&&this.__sY;
}}var T=this.__tc(R);

if(qx.lang.Type.isBoolean(T)){this.__sY=T&&this.__sY;
}U=T&&U;
this.__th(U);

if(qx.lang.Object.isEmpty(this.__sW)){this.fireEvent(K);
}return U;
},__ta:function(B){if(B.getRequired()){if(this.__tf(B)){var C=!!B.getSelection()[0];
}else{var C=!!B.getValue();
}B.setValid(C);
B.setInvalidMessage(F);
return C;
}return true;
},__tb:function(v,w){var A=v.item;
var z=v.validator;
if(this.__td(z)){this.__sW[A.toHashCode()]=null;
z.validate(A,A.getValue(),this);
return null;
}var y=null;
...
}A.setInvalidMessage(x);
}else{throw e;
}}A.setValid(y);
v.valid=y;
return y;
},__tc:function(a){var c=this.getValidator();

if(c==null){return true;
}this.setInvalidMessage(L);

if(this.__td(c)){this.__sW[this.toHashCode()]=null;
c.validateForm(a,this);
return null;
}
try{var d=c(a,this);

if(d===undefined){d=true;
}}catch(e){if(e instanceof qx.core.ValidationError){d=false;
...
},__td:function(f){var g=false;

if(!qx.lang.Type.isFunction(f)){g=qx.Class.isSubClassOf(f.constructor,qx.ui.form.validation.AsyncValidator);
}return g;
},__te:function(t){var u=t.constructor;
return qx.Class.hasInterface(u,qx.ui.form.IForm);
},__tf:function(k){var l=k.constructor;
return qx.Class.hasInterface(l,qx.ui.core.ISingleSelection);
},__tg:function(o){var p=o.constructor;
...
},__th:function(m){var n=this.__sX;
this.__sX=m;
if(n!=m){this.fireDataEvent(J,m,n);
}},getValid:function(){return this.__sX;
},isValid:function(){return this.getValid();
},getInvalidMessages:function(){var h=[];
for(var i=0;i<this.__sV.length;i++){var j=this.__sV[i].item;

if(!j.getValid()){h.push(j.getInvalidMessage());
}}if(this.getInvalidMessage()!=L){h.push(this.getInvalidMessage());
}return h;
},reset:function(){for(var i=0;i<this.__sV.length;i++){var W=this.__sV[i];
W.item.setValid(true);
}this.__sX=null;
},setItemValid:function(X,Y){this.__sW[X.toHashCode()]=Y;
X.setValid(Y);
this.__ti();
},setFormValid:function(P){this.__sW[this.toHashCode()]=P;
this.__ti();
},__ti:function(){var N=this.__sY;
for(var O in this.__sW){var M=this.__sW[O];
N=M&&N;
if(M==null){return;
}}this.__th(N);
this.__sW={};
...
(function(){var h="qx.ui.form.validation.AsyncValidator";
qx.Class.define(h,{extend:qx.core.Object,construct:function(i){arguments.callee.base.call(this);
this.__tj=i;
},members:{__tj:null,__tk:null,__tl:null,__tm:null,validate:function(a,b,c){this.__tm=false;
this.__tk=a;
this.__tl=c;
this.__tj(this,b);
},validateForm:function(d,e){this.__tm=true;
this.__tl=e;
this.__tj(d,this);
},setValid:function(f,g){if(this.__tm){if(g!==undefined){this.__tl.setInvalidMessage(g);
}this.__tl.setFormValid(f);
}else{if(g!==undefined){this.__tk.setInvalidMessage(g);
}this.__tl.setItemValid(this.__tk,f);
}}},destruct:function(){this.__tl=this.__tk=null;
}});
})();
...
qx.Class.define(c,{extend:qx.core.Object,construct:function(){arguments.callee.base.call(this);
this.__tz=[];
},members:{__tz:null,add:function(d){if(this.__tD(d)){var e=d.getValue();
}else if(this.__tC(d)){var e=d.getSelection();
}else{throw new Error("Item "+d+" not supported for reseting.");
}this.__tz.push({item:d,init:e});
},reset:function(){for(var i=0;i<this.__tz.length;i++){var o=this.__tz[i];
this.__tA(o.item,o.init);
}},resetItem:function(p){var q;

for(var i=0;i<this.__tz.length;i++){var r=this.__tz[i];

if(r.item===p){q=r.init;
break;
}}if(q===undefined){throw new Error("The given item has not been added.");
}this.__tA(p,q);
},__tA:function(a,b){if(this.__tD(a)){a.setValue(b);
}else if(this.__tC(a)){a.setSelection(b);
}},redefine:function(){for(var i=0;i<this.__tz.length;i++){var j=this.__tz[i].item;
this.__tz[i].init=this.__tB(j);
}},redefineItem:function(m){var n;

for(var i=0;i<this.__tz.length;i++){if(this.__tz[i].item===m){n=this.__tz[i];
break;
}}if(n===undefined){throw new Error("The given item has not been added.");
}n.init=this.__tB(n.item);
},__tB:function(h){if(this.__tD(h)){return h.getValue();
}else if(this.__tC(h)){return h.getSelection();
}},__tC:function(k){var l=k.constructor;
return qx.Class.hasInterface(l,qx.ui.core.ISingleSelection);
},__tD:function(f){var g=f.constructor;
...
}},members:{addItems:function(g,h,j){throw new Error("Abstract method call");
},addButton:function(k){throw new Error("Abstract method call");
}}});
})();
(function(){var m="",l="right",k=" <span style='color:red'>*</span> ",j="bold",h="qx.ui.form.renderer.Single",g="top",f="left",e=" :";
qx.Class.define(h,{extend:qx.ui.form.renderer.AbstractRenderer,construct:function(c){var d=new qx.ui.layout.Grid();
d.setSpacing(6);
d.setColumnFlex(0,1);
d.setColumnAlign(0,l,g);
this._setLayout(d);
...
},_createLabel:function(name,u){var v=m;

if(u.getRequired()){v=k;
}var w=name.length>0||u.getRequired()?e:m;
...
(function(){var q="top",p="left",o="",n=" <span style='color:red'>*</span> ",m="bold",l="qx.ui.form.renderer.Double",k="right",j=" :";
qx.Class.define(l,{extend:qx.ui.form.renderer.AbstractRenderer,construct:function(a){var b=new qx.ui.layout.Grid();
b.setSpacing(6);
b.setColumnAlign(0,p,q);
b.setColumnAlign(1,p,q);
b.setColumnAlign(2,p,q);
b.setColumnAlign(3,p,q);
this._setLayout(b);
arguments.callee.base.call(this,a);
},members:{_row:0,_buttonRow:null,addItems:function(t,u,v){if(v!=null){this._add(this._createHeader(v),{row:this._row,column:0,colSpan:4});
this._row++;
}for(var i=0;i<t.length;i++){var x=this._createLabel(u[i],t[i]);
this._add(x,{row:this._row,column:(i*2)%4});
var w=t[i];
x.setBuddy(w);
this._add(w,{row:this._row,column:((i*2)%4)+1});

if(i%2==1){this._row++;
}}
if(i%2==1){this._row++;
}},addButton:function(r){if(this._buttonRow==null){this._buttonRow=new qx.ui.container.Composite();
this._buttonRow.setMarginTop(5);
var s=new qx.ui.layout.HBox();
s.setAlignX(k);
s.setSpacing(5);
this._buttonRow.setLayout(s);
this._add(this._buttonRow,{row:this._row,column:0,colSpan:4});
this._row++;
}this._buttonRow.add(r);
},getLayout:function(){return this._getLayout();
},_createLabel:function(name,e){var f=o;

if(e.getRequired()){f=n;
}var g=name.length>0||e.getRequired()?j:o;
var h=new qx.ui.basic.Label(name+f+g);
h.setRich(true);
return h;
},_createHeader:function(c){var d=new qx.ui.basic.Label(c);
d.setFont(m);

if(this._row!=0){d.setMarginTop(10);
}return d;
}}});
})();
