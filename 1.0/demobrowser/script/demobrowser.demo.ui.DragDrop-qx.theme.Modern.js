...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.ui.DragDrop-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"e3e73ec260b3"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var v="drop",u="value",t="items",s="move",r="multi",q="dragover",p="icon/16/places/folder.png",o="dragstart",n="Item ",m="copy",I="drag",H="TextArea Target",G="solid",F="__JX",E="Reorderable",D="droprequest",C="#33508D",B="Source",A="demobrowser.demo.ui.DragDrop",z="Simple Target",x="Enable drag",y="\n",w="dragend";
qx.Class.define(A,{extend:qx.application.Standalone,members:{__JX:null,__JY:null,main:function(){arguments.callee.base.call(this);
var V=this.getRoot();
var P=new qx.ui.container.Scroll();
var K=new qx.ui.layout.Basic();
var N=new qx.ui.container.Composite(K).set({allowStretchY:false,allowStretchX:false});
P.add(N);
V.add(P,{edge:0});
var U=new qx.ui.basic.Label(B);
N.add(U,{left:20,top:20});
var S=new qx.ui.form.List;
S.setDraggable(true);
S.setSelectionMode(r);
N.add(S,{left:20,top:40});

for(var i=0;i<20;i++){S.add(new qx.ui.form.ListItem(n+i,p));
}var W=new qx.ui.form.CheckBox(x);
W.setValue(true);
N.add(W,{left:20,top:260});
S.addListener(o,function(e){if(!W.isValue()){e.preventDefault();
}e.addType(u);
e.addType(t);
e.addAction(m);
e.addAction(s);
});
S.addListener(D,function(e){this.debug("Related of droprequest: "+e.getRelatedTarget());
var h=e.getCurrentAction();
var k=e.getCurrentType();
var g;

switch(k){case t:g=this.getSelection();

if(h==m){var j=[];

for(var i=0,l=g.length;i<l;i++){j[i]=g[i].clone();
}g=j;
}break;
case u:g=this.getSelection()[0].getLabel();
break;
}if(h==s){var f=this.getSelection();

for(var i=0,l=f.length;i<l;i++){this.remove(f[i]);
}}e.addData(k,g);
});
var Q=new qx.ui.basic.Label(z);
N.add(Q,{left:140,top:20});
var O=new qx.ui.form.List;
O.setDroppable(true);
O.setSelectionMode(r);
N.add(O,{left:140,top:40});
O.addListener(v,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
var J=e.getData(t);

for(var i=0,l=J.length;i<l;i++){this.add(J[i]);
}});
O.addListener(q,function(e){if(!e.supportsType(t)){e.preventDefault();
}});
var Q=new qx.ui.basic.Label(H);
N.add(Q,{left:260,top:20});
var T=new qx.ui.form.TextArea;
T.setDroppable(true);
T.setHeight(100);
N.add(T,{left:260,top:40});
T.addListener(v,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
this.setValue(e.getData(u).replace(/,/g,y));
});
T.addListener(q,function(e){if(!e.supportsType(u)){e.preventDefault();
}});
var R=new qx.ui.basic.Label(E);
N.add(R,{left:500,top:20});
var L=this.__JX=new qx.ui.form.List;
L.setDraggable(true);
L.setDroppable(true);
L.setSelectionMode(r);
N.add(L,{left:500,top:40});

for(var i=0;i<20;i++){L.add(new qx.ui.form.ListItem(n+i,p));
}var M=new qx.ui.core.Widget;
M.setDecorator(new qx.ui.decoration.Single().set({top:[1,G,C]}));
M.setHeight(0);
M.setOpacity(0.5);
M.setZIndex(100);
M.setLayoutProperties({left:-1000,top:-1000});
M.setDroppable(true);
this.getRoot().add(M);
L.addListener(o,function(e){e.addAction(s);
});
L.addListener(w,function(e){M.setDomPosition(-1000,-1000);
});
L.addListener(I,function(e){var b=e.getOriginalTarget();
if(b instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__JY=b;
}
if(!qx.ui.core.Widget.contains(this,b)&&b!=M){return;
}var a=b.getContainerLocation();
M.setWidth(b.getBounds().width);
M.setDomPosition(a.left,a.top);
});
L.addListener(q,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
L.addListener(v,function(e){this.__Ka(e.getOriginalTarget());
},this);
M.addListener(v,function(e){this.__Ka(this.__JY);
},this);
},__Ka:function(c){var d=this.__JX.getSortedSelection();

for(var i=0,l=d.length;i<l;i++){this.__JX.addBefore(d[i],c);
this.__JX.addToSelection(d[i]);
