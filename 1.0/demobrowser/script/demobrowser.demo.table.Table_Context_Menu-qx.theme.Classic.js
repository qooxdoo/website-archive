...
for(var i=0;i<=1;i++){bt=new qx.ui.menu.Button(i?r:B);
bt.setUserData(w,i?true:false);
bt.addListener(y,function(e){br.setValue(bo,bp,this.getUserData(w));
});
bs.add(bt);
}return true;
},_contextMenuHandlerNull:function(bc,bd,be,bf,bg){return true;
},createControls:function(){var U=new qx.ui.toolbar.ToolBar();
var S,T,V;
T=new qx.ui.toolbar.Part();
U.add(T);
S=new qx.ui.toolbar.Button(q,v);
S.addListener(y,function(k){var l=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,l[0][i]);
}this.info("Row 10 changed");
},this);
T.add(S);
S=new qx.ui.toolbar.Button(P,G);
S.addListener(y,function(bh){var bi=this.createRandomRows(10);
this._tableModel.addRows(bi);
this.info("10 rows added");
},this);
T.add(S);
S=new qx.ui.toolbar.Button(Q,K);
S.addListener(y,function(n){var o=this._tableModel.getRowCount();
this._tableModel.removeRows(o-5,5);
this.info("5 rows removed");
},this);
T.add(S);
S=new qx.ui.toolbar.Button(z,s);
S.addListener(y,function(bj){var bk=[];
R.getSelectionModel().iterateSelection(function(m){bk.push(m+N);
});
this.showDialog(D+bk.join(I));
},this);
T.add(S);
T=new qx.ui.toolbar.Part();
U.add(T);
var R=this._table;
V=new qx.ui.toolbar.CheckBox(E);
V.set({value:R.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(J+C)});
V.addListener(x,function(W){R.setKeepFirstVisibleRowComplete(this.getValue());
},V);
T.add(V);
V=new qx.ui.toolbar.CheckBox(u);
V.set({value:R.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(A)});
V.addListener(x,function(bl){if(bl.getData()){var bn=function(f,g){var h=f[arguments.callee.columnIndex];
...
var bm=function(a,b){var c=a[arguments.callee.columnIndex];
...
R.getTableModel().setSortMethods(0,{ascending:bn,descending:bm});
...
this.__GI=[];
},members:{__GI:null,setContextMenuHandler:function(a,b){this.__GI[a]=b;
},getContextMenuHandler:function(g){return this.__GI[g];
