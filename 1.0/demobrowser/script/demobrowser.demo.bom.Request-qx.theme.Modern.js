...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.bom.Request","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.bom.Request-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"1a31fb357025"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var f="Loading...",e="",d="content",c="GET",b="demobrowser.demo.bom.Request",a="Request.html";
...
g.value=f;
var h=new qx.bom.Request;
h.open(c,a,true);
h.onreadystatechange=function(){qx.log.Logger.debug("Event: readystatechange: "+h.readyState);

if(h.readyState==4){g.value=h.responseText;
}};
h.onload=function(){qx.log.Logger.debug("Event: load");
};
h.onerror=function(){qx.log.Logger.debug("Event: error");
};
h.ontimeout=function(){qx.log.Logger.debug("Event: timeout");
};
h.onabort=function(){qx.log.Logger.debug("Event: abort");
};
h.send(e);
}}});
})();
...
(function(){var n="",m="gecko",l="Content-Type",k="qx.client",j="application/xml",i="qx.bom.Request";
qx.Class.define(i,{extend:qx.core.Object,construct:function(){arguments.callee.base.call(this);
this.__Cv={};
this.__Cw=this.__CG();
},statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},members:{__Cx:null,__Cy:null,__Cw:null,__Cz:null,__CA:null,__CB:null,__CC:null,__Cv:null,readyState:0,responseText:n,responseXML:null,status:0,statusText:n,timeout:0,onreadystatechange:function(){},ontimeout:function(){},onload:function(){},onerror:function(){},onabort:function(){},open:function(v,w,x,y,z){if(x==null){x=true;
}this.__Cx=x;
this.__Cy=qx.lang.Function.bind(this.__CD,this);
this.__Cz=qx.lang.Function.bind(this.__CE,this);
this.__Cw.onreadystatechange=this.__Cy;
if(arguments.length>4){this.__Cw.open(v,w,x,y,z);
}else if(arguments.length>3){this.__Cw.open(v,w,x,y);
}else{this.__Cw.open(v,w,x);
}if(qx.core.Variant.isSet(k,m)){if(!x){this.readyState=qx.bom.Request.OPENED;
this.__CH();
}}},send:function(b){var c=this.__Cv;
if(b&&b.nodeType){b=window.XMLSerializer?new XMLSerializer().serializeToString(b):b.xml;

if(!c[l]){c[l]=j;
}}for(var d in c){this.__Cw.setRequestHeader(d,c[d]);
}if(this.timeout!=null&&this.timeout>0){this.__CA=window.setTimeout(this.__Cz,this.timeout);
}this.__Cw.send(b);
if(qx.core.Variant.isSet(k,m)){if(!this.__Cx){this.readyState=qx.bom.Request.OPENED;
this.__CJ(this);
...
},abort:function(){if(this.__CA){window.clearTimeout(this.__CA);
}this.__CF();
this.onabort();
this.dispose();
},__CD:qx.event.GlobalError.observeMethod(function(){if(qx.core.Variant.isSet("qx.client","gecko")){if(!this.__Cx){return;
}}this.readyState=this.__Cw.readyState;
this.__CJ();
if(this.__CB){this.readyState=qx.bom.Request.UNSENT;
return ;
}if(this.readyState==qx.bom.Request.DONE&&this.__CA){window.clearTimeout(this.__CA);
}this.__CH();
if(this.readyState==qx.bom.Request.DONE){this.dispose();
}}),__CE:qx.event.GlobalError.observeMethod(function(){this.__CF();
this.ontimeout();
this.dispose();
}),getAllResponseHeaders:function(){try{return this.__Cw.getAllResponseHeaders();
}catch(r){return null;
}},getResponseHeader:function(B){try{return this.__Cw.getResponseHeader(B);
}catch(h){return null;
}},setRequestHeader:function(s,t){if(t==null){delete this.__Cv[s];
}else{this.__Cv[s]=t;
}},removeRequestHeader:function(o,p){delete this.__Cv[o];
},getRequestHeader:function(e){return this.__Cv[e]||null;
},__CF:function(){this.__CJ();
if(this.readyState>qx.bom.Request.UNSENT){this.__CB=true;
}this.__Cw.abort();
},__CG:qx.core.Variant.select("qx.client",{"default":function(){return new XMLHttpRequest;
},"mshtml":function(){if(window.ActiveXObject&&qx.xml.Document.XMLHTTP){return new ActiveXObject(qx.xml.Document.XMLHTTP);
}
if(window.XMLHttpRequest){return new XMLHttpRequest;
}}}),__CH:function(){if(this.__CC===this.readyState){return;
}this.onreadystatechange();
this.__CC=this.readyState;
if(this.readyState===4){if(this.isSuccessful()){this.onload();
}else{this.onerror();
}}},__CI:function(){var f=this.__Cw.responseXML;

if(qx.core.Variant.isSet("qx.client","mshtml")){if(f&&!f.documentElement&&this.__Cw.getResponseHeader("Content-Type").match(/[^\/]+\/[^\+]+\+xml/)){f=new ActiveXObject(qx.xml.Document.DOMDOC);
f.loadXML(this.__Cw.responseText);
}if(f&&f.parseError!=0){return null;
}}else if(!f.documentElement||f.documentElement.tagName=="parsererror"){return null;
}return f;
},__CJ:function(){var q=this.__Cw;

try{this.responseText=q.responseText;
}catch(g){}
try{this.responseXML=this.__CI();
}catch(a){}
try{this.status=q.status;
}catch(u){}
try{this.statusText=q.statusText;
}catch(A){}if(qx.core.Variant.isSet("qx.client","mshtml")){if(this.status===1223){this.status=204;
}}if(!this.status&&location.protocol==="file:"){this.status=204;
}}},destruct:function(){if(this.__CA){window.clearTimeout(this.__CA);
this.__CA=null;
}if(this.__Cw){this.__Cw.onreadystatechange=qx.lang.Function.empty;
this.__Cw=null;
