...
(function(){var m="error",l="Loading...",k="change",j="demobrowser.demo.test.io_HttpRequest",i="content",h="load",g="io_HttpRequest.html",f="abort",d="timeout";
...
b.setUrl(g);
b.addListener(k,function(e){var c=e.getData();
qx.log.Logger.debug("Event: readystatechange: "+c);

if(c==4){a.value=this.getResponseText();
}});
b.addListener(h,function(){qx.log.Logger.debug("Event: load");
});
b.addListener(m,function(){qx.log.Logger.debug("Event: error");
});
b.addListener(d,function(){qx.log.Logger.debug("Event: timeout");
});
b.addListener(f,function(){qx.log.Logger.debug("Event: abort");
});
b.send();
}}});
})();
...
(function(){var s="qx.event.type.Event",r="Boolean",q="String",p="Last-Modified",o="basic",n="GET",m="load",l="text/plain",k="abort",j="http",V="POST",U="error",T="timeout",S="application/xml",R="X-Requested-With",Q="application/json",P="text/html",O="If-Modified-Since",N='Basic ',M="Integer",z="qx.event.type.Data",A="HEAD",x="XMLHttpRequest",y='Authorization',v="change",w="Accept",t="OPTIONS",u="text/javascript",B="",C="PUT",F=':',E="application/x-www-form-urlencoded",H="DELETE",G="__FY",J="qx.io.HttpRequest",I="no-cache",D="Cache-Control",L="Content-Type",K="Thu, 01 Jan 1970 00:00:00 GMT";
qx.Class.define(J,{extend:qx.core.Object,construct:function(X){arguments.callee.base.call(this);
this.__FW={};
if(X!=null){this.setUrl(X);
}},statics:{__FX:{}},events:{"change":z,"timeout":s,"load":s,"error":s,"abort":s},properties:{refresh:{check:r,init:false},data:{nullable:true},method:{check:[n,V,C,A,H,t],init:n},async:{check:r,init:true},mime:{check:[l,u,Q,S,P],init:l},url:{check:q,init:B},username:{check:q,nullable:true},password:{check:q,nullable:true},auth:{check:[j,o],init:j},timeout:{check:M,nullable:true},cache:{check:r,init:false}},members:{__FY:null,__FW:null,setRequestHeader:function(bj,bk){this.__FW[bj]=bk;
},removeRequestHeader:function(bc){delete this.__FW[bc];
},getRequestHeader:function(Y){var ba=this.__FW[Y];

if(ba===undefined){ba=null;
}return ba;
},getResponseText:function(){var bd=this.__FY;

if(bd){return bd.responseText;
}},getResponseXml:function(){var f=this.__FY;

if(f){return f.responseXML;
}},getResponseHeader:function(be){var bf=this.__FY;

if(bf){return bf.getResponseHeader(be);
}},getAllResponseHeaders:function(){var bh=this.__FY;

if(bh){return bh.getAllResponseHeaders();
}},isNotModified:function(){var h=this.__FY;

if(!h){return false;
}var i=h.getResponseHeader(p);
return h.status===304||qx.io.HttpRequest.__FX[this.getUrl()]===i;
},isSuccessful:function(){var bb=this.__FY;
return !bb||bb.isSuccessful();
},getStatusCode:function(){var g=this.__FY;

if(g){return g.status;
}},getStatusText:function(){var bi=this.__FY;

if(bi){return bi.statusText;
}},getReadyState:function(){var bg=this.__FY;

if(bg){return bg.readyState;
}},send:function(){if(this.__FY){if(this.getReadyState()!==4){throw new Error("Request is still pending at ready state: "+this.getReadyState());
}this.__FY.dispose();
}var c=this.__FY=new qx.bom.Request;
c.onreadystatechange=qx.lang.Function.bind(this.__Ga,this);
c.ontimeout=qx.lang.Function.bind(this.__Gb,this);
c.onload=qx.lang.Function.bind(this.__Gc,this);
c.onerror=qx.lang.Function.bind(this.__Gd,this);
c.onabort=qx.lang.Function.bind(this.__Ge,this);
var a=this.getUsername();
var e=this.getPassword();

if(this.getAuth()==o){c.setRequestHeader(y,N+qx.util.Base64.encode(a+F+e));
a=e=null;
}var b=this.getUrl();
c.timeout=this.getTimeout();
c.open(this.getMethod(),b,this.getAsync(),a,e);
if(!this.getCache()){c.setRequestHeader(D,I);
}if(this.getRefresh()){c.setRequestHeader(O,qx.io.HttpRequest.__FX[b]||K);
}c.setRequestHeader(R,x);
if(this.getMethod()===V){c.setRequestHeader(L,E);
}c.setRequestHeader(w,this.getMime());
var d=this.__FW;

for(var name in d){c.setRequestHeader(name,d[name]);
}c.send(this.getData());
},abort:function(){if(this.__FY){this.__FY.abort();
}},__Ga:qx.event.GlobalError.observeMethod(function(){this.fireDataEvent(v,this.getReadyState());
if(this.getRefresh()&&this.getReadyState()===4&&this.isSuccessful()){var W=this.getResponseHeader(p);

if(W){qx.io.HttpRequest.__FX[this.getUrl()]=W;
}}}),__Gb:qx.event.GlobalError.observeMethod(function(){if(this.hasListener(T)){this.fireEvent(T);
}}),__Gc:qx.event.GlobalError.observeMethod(function(){if(this.hasListener(m)){this.fireEvent(m);
}}),__Gd:qx.event.GlobalError.observeMethod(function(){if(this.hasListener(U)){this.fireEvent(U);
}}),__Ge:qx.event.GlobalError.observeMethod(function(){if(this.hasListener(k)){this.fireEvent(k);
...
qx.Class.define(b,{statics:{__Gf:a,encode:function(d){var j=this.__Gf;
var n=c;
var g,f,e;
var k,l,m,h;
var i=0;
