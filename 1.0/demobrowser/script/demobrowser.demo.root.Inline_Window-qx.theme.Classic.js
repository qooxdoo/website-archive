...
var o=new qx.ui.root.Inline(document.getElementById(d)).set({decorator:a,padding:10,backgroundColor:b});
o.setLayout(new qx.ui.layout.HBox(10));
var l=new qx.ui.form.Button(g);
var n=1;
l.addListener(j,function(e){var k=new qx.ui.window.Window(f+n++).set({width:200,height:150});
k.moveTo(n*15,n*15);
k.open();
});
o.add(l);
...
(function(){var r="div",q="resize",p="qx.ui.root.Page",o="gecko",n="paddingLeft",m="$$widget",l="qx.client",k="left",j="paddingTop",i="qxIsRootPage",h="absolute";
qx.Class.define(p,{extend:qx.ui.root.Abstract,construct:function(g){this.__Au=g;
arguments.callee.base.call(this);
this._setLayout(new qx.ui.layout.Basic());
this.setZIndex(10000);
qx.ui.core.queue.Layout.add(this);
this.addListener(q,this.__Aw,this);
qx.ui.core.FocusHandler.getInstance().connectTo(this);
},members:{__Av:null,__Au:null,_createContainerElement:function(){var v=this.__Au.createElement(r);
this.__Au.body.appendChild(v);
var u=new qx.html.Root(v);
u.setStyles({position:h,textAlign:k});
u.setAttribute(m,this.toHashCode());
if(qx.core.Variant.isSet(l,o)){u.setAttribute(i,1);
}return u;
},_createContentElement:function(){return new qx.html.Element(r);
},_computeSizeHint:function(){var d=qx.bom.Document.getWidth(this._window);
var f=qx.bom.Document.getHeight(this._window);
return {minWidth:d,width:d,maxWidth:d,minHeight:f,height:f,maxHeight:f};
},__Aw:function(e){this.getContainerElement().setStyles({width:0,height:0});
this.getContentElement().setStyles({width:0,height:0});
},supportsMaximize:function(){return false;
},_applyPadding:function(s,t,name){if(s&&(name==j||name==n)){throw new Error("The root widget does not support 'left', or 'top' paddings!");
}arguments.callee.base.call(this,s,t,name);
...
(function(){var C="relative",B="resize",A="mshtml",z="position",y="qx.ui.root.Inline",x="$$widget",w="hidden",v="qx.client",u="div",t="left";
qx.Class.define(y,{extend:qx.ui.root.Abstract,include:[qx.ui.core.MLayoutHandling],construct:function(F,G,H){this.__Ax=F;
F.style.overflow=w;
F.style.textAlign=t;
this.__Ay=G||false;
this.__Az=H||false;
...
qx.ui.core.queue.Layout.add(this);
qx.ui.core.FocusHandler.getInstance().connectTo(this);
},members:{__Ay:false,__Az:false,__Ax:null,__AA:function(){if(this.__Ay||this.__Az){var E=qx.bom.element.Dimension.getSize(this.__Ax);

if(this.__Ay&&E.width<1){throw new Error("The root element "+this.__Ax+" of "+this+" needs a width when its width size should be used!");
}
if(this.__Az){if(E.height<1){throw new Error("The root element "+this.__Ax+" of "+this+" needs a height when its height size should be used!");
}if(E.height>=1&&qx.bom.element.Style.get(this.__Ax,"height",3)==""){qx.bom.element.Style.set(this.__Ax,"height",E.height+"px");
}}qx.event.Registration.addListener(this.__Ax,"resize",this._onResize,this);
}},_createContainerElement:function(){var a=this.__Ax;

if(this.__Ay||this.__Az){var f=document.createElement(u);
a.appendChild(f);
if(qx.core.Variant.isSet(v,A)&&qx.bom.client.Engine.VERSION==6){var d=qx.dom.Node.getBodyElement(a);
var c;
var h;
var g=false;
var b=qx.dom.Hierarchy.getAncestors(a);

for(var i=0,j=b.length;i<j;i++){c=b[i];

if(c!=d){h=qx.bom.element.Style.get(c,z);

if(h==C){g=true;
break;
}}else{break;
}}
if(g){a.style.position=C;
}}}else{f=a;
}var k=new qx.html.Root(f);
f.style.position=C;
k.setAttribute(x,this.toHashCode());
return k;
},_onResize:function(e){var D=e.getData();

if((D.oldWidth!==D.width)&&this.__Ay||(D.oldHeight!==D.height)&&this.__Az){qx.ui.core.queue.Layout.add(this);
}},_computeSizeHint:function(){var r=this.__Ay;
var o=this.__Az;

if(!r||!o){var n=arguments.callee.base.call(this);
}else{n={};
}var s=qx.bom.element.Dimension;

if(r){var q=s.getContentWidth(this.__Ax);
n.width=q;
n.minWidth=q;
n.maxWidth=q;
}
if(o){var p=s.getContentHeight(this.__Ax);
n.height=p;
n.minHeight=p;
n.maxHeight=p;
}return n;
}},defer:function(l,m){qx.ui.core.MLayoutHandling.remap(m);
},destruct:function(){qx.event.Registration.removeListener(this.__Ax,B,this._onResize,this);
this.__Ax=null;
}});
})();
(function(){var l="resize",k="interval",j="__AD",i="body",h="qx.event.handler.ElementResize";
...
this.__AB=x;
this.__AC={};
this.__AD=new qx.event.Timer(200);
this.__AD.addListener(k,this._onInterval,this);
},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{resize:true},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__AC:null,__AB:null,__AD:null,canHandleEvent:function(y,z){return y.tagName.toLowerCase()!==i;
},registerEvent:function(s,t,u){var w=qx.core.ObjectRegistry.toHashCode(s);
var v=this.__AC;

if(!v[w]){v[w]={element:s,width:qx.bom.element.Dimension.getWidth(s),height:qx.bom.element.Dimension.getHeight(s)};
this.__AD.start();
}},unregisterEvent:function(m,n,o){var q=qx.core.ObjectRegistry.toHashCode(m);
var p=this.__AC;

if(p[q]){delete p[q];

if(qx.lang.Object.isEmpty(p)){this.__AD.stop();
}}},_onInterval:function(e){var b=this.__AC;
