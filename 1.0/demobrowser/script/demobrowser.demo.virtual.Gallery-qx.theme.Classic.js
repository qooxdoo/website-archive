...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.virtual.Gallery-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"6d33625d3d15"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var s="",r="selected",q="cell",p="layer",o="icon",n="#f3f3f3",m="label",l="Gallery (HTML - divs)",k="__JV",j="manager",W="default",V="demobrowser.demo.virtual.Gallery",U="demobrowser.demo.virtual.WidgetGallery",T="text-align: center;",S="auto",R="color: white; background-color: #00398D;",Q="user-desktop.png",P="<img src='",O="'></img>",N="group",z="resize",A="folder.png",x="vertical",y="multi",v="network-server.png",w="user-trash.png",t="off",u="top",B="icon/32/places/",C="horizontal",F="demobrowser.demo.virtual.AbstractGallery",E="abstract",H="position: absolute;",G="demobrowser.demo.virtual.HtmlGallery",J="Gallery (widgets)",I="<br>",D="Icon #",M="network-workgroup.png",L="text-selected",K="demobrowser.demo.virtual.GalleryCell";
...
var bm=new demobrowser.demo.virtual.HtmlGallery(l);
bm.moveTo(400,50);
}}});
qx.Class.define(F,{extend:qx.ui.window.Window,type:E,construct:function(bS){arguments.callee.base.call(this,bS);
...
bT.set({scrollbarX:t,scrollbarY:S});
bT.getPane().addListener(z,this._onPaneResize,this);
this.add(bT);
this.manager=new qx.ui.virtual.selection.CellRectangle(bT.getPane(),this).set({mode:y,drag:true});
this.manager.attachMouseEvents();
this.manager.attachKeyEvents(bT);
},members:{getItemData:function(bp,bq){return this.items[bp*this.itemPerLine+bq];
},_createScroller:function(){},isItemSelectable:function(bu){return !!this.getItemData(bu.row,bu.column);
},styleSelectable:function(bF,bG,bH){},_onPaneResize:function(e){var bQ=e.getTarget();
...
},_generateItems:function(bv){var by=[];
var bB=[A,w,v,M,Q];
var bz=qx.util.AliasManager.getInstance();
var bx=qx.util.ResourceManager.getInstance();

for(var i=0;i<bv;i++){var bC=B+bB[Math.floor(Math.random()*bB.length)];
var bw=bz.resolve(bC);
var bA=bx.toUri(bw);
by[i]={label:D+(i+1),icon:bC,resolvedIcon:bA};
}return by;
}},destruct:function(){this.items=null;
this._disposeObjects(j);
}});
qx.Class.define(K,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var br=new qx.ui.basic.Atom().set({iconPosition:u});
br.getChildControl(m).set({padding:[0,4]});
br.getChildControl(o).set({padding:4});
return br;
},updateData:function(bM,bN){bM.set({icon:bN.icon,label:bN.label});
},updateStates:function(bI,bJ){var bL=bI.getChildControl(m);
var bK=bI.getChildControl(o);
...
qx.Class.define(U,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(a){arguments.callee.base.call(this,a);
this.__JV=new demobrowser.demo.virtual.GalleryCell();
...
return bt;
},styleSelectable:function(bg,bh,bi){if(bh!==r){return;
}var bj=this.layer.getChildren();

for(var i=0;i<bj.length;i++){var bk=bj[i];
var bl=bk.getUserData(q);

if(bg.row!==bl.row||bg.column!==bl.column){continue;
}
if(bi){this.__JV.updateStates(bk,{selected:1});
}else{this.__JV.updateStates(bk,{});
}}},getCellWidget:function(b,c){var h=this.getItemData(b,c);

if(!h){return null;
}var d={row:b,column:c};
var f={};

if(this.manager.isItemSelected(d)){f.selected=true;
}var g=this.__JV.getCellWidget(h,f);
g.setUserData(q,d);
return g;
},poolCellWidget:function(bs){this.__JV.pool(bs);
}},destruct:function(){this._disposeObjects(k,p);
}});
qx.Class.define(G,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bD){arguments.callee.base.call(this,bD);
var bE=qx.theme.manager.Font.getInstance().resolve(W).getStyles();
this._fontCss=qx.bom.element.Style.compile(bE);
},members:{_createScroller:function(){var bo=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.HtmlCell(this);
bo.getPane().addLayer(this.layer);
var bn=new qx.ui.virtual.layer.GridLines(C,n);
bo.getPane().addLayer(bn);
var bn=new qx.ui.virtual.layer.GridLines(x,n);
bo.getPane().addLayer(bn);
return bo;
},_onPaneResize:function(e){arguments.callee.base.call(this,e);
this.manager.clearSelection();
},styleSelectable:function(X,Y,ba){this.layer.updateLayerData();
},getCellProperties:function(bb,bc){var bf=this.getItemData(bb,bc);

if(!bf){return s;
}var be=this.manager.isItemSelected({row:bb,column:bc});
var bd=be?R:s;
return {style:[H,T,this._fontCss,bd].join(s),content:[P,bf.resolvedIcon,O,I,bf.label].join(s)};
