...
var bp=new qx.ui.layout.HBox(5);
this.topContainer=new qx.ui.container.Composite(bp);
this.runTest(v);
this.runTest(d);
this.runTest(a);
this.runTest(b);
...
},runTest:function(bo){this.setUp();
this[bo]();
...
},testStringCell:function(){var E={value:[g,i,j,null],states:[{},{selected:1}]};
var D=new qx.ui.virtual.cell.String;
qx.util.Permutation.permute(E,function(x){var y=this.__JK(D,x.value,x.states);
...
}var K=this.__JK(I,J.value,J.states);
this.container.add(K);
},this);
},testDateCell:function(){var R={value:[new Date(),null],format:[null,new qx.util.format.DateFormat(qx.locale.Date.getDateFormat(o))],states:[{},{selected:1}]};
var S=new qx.ui.virtual.cell.Date();
qx.util.Permutation.permute(R,function(bf){if(bf.format){S.setDateFormat(bf.format);
}else{S.resetDateFormat();
}var bg=this.__JK(S,bf.value,bf.states);
this.container.add(bg);
},this);
},testHtmlCell:function(){var F={value:[g,i,j,null],states:[{},{selected:1}]};
var G=new qx.ui.virtual.cell.Html;
qx.util.Permutation.permute(F,function(z){var A=this.__JK(G,z.value,z.states);
...
qx.util.Permutation.permute(P,function(bm){var bn=this.__JK(Q,bm.value,bm.states);
this.container.add(bn);
},this);
},testBooleanCell:function(){var L={value:[true,false,null],states:[{},{selected:1}]};
var M=new qx.ui.virtual.cell.Boolean;
qx.util.Permutation.permute(L,function(N){var O=this.__JK(M,N.value,N.states);
this.container.add(O);
},this);
},testBooleanCellCustomImage:function(){var B={value:[true,false,null],states:[{},{selected:1}]};
var C=new qx.ui.virtual.cell.Boolean;
C.setIconFalse(n);
C.setIconTrue(m);
qx.util.Permutation.permute(B,function(bd){var be=this.__JK(C,bd.value,bd.states);
this.container.add(be);
},this);
},__JK:function(T,U,V){var ba=100;
var X=30;
var bc=new qx.ui.embed.Html().set({width:ba,height:X,decorator:V.selected?null:"main"});
var W=T.getCellProperties(U,V);
var Y=W.insets;
var bb=["<div ","style='",this._getCellSizeStyle(ba,X,Y[0],Y[1]),W.style,"' ","class='",W.classes,"' ",W.attributes,">",W.content,"</div>"].join("");
bc.setHtml(bb);
return bc;
},_getCellSizeStyle:function(bh,bi,bj,bk){var bl=j;

if(qx.bom.client.Feature.CONTENT_BOX){bh-=bj;
bi-=bk;
}bl+=c+bh+h;
bl+=w+bi+h;
return bl;
...
(function(){var f="qx.ui.virtual.cell.Number",e="qx.util.format.NumberFormat",d="",c="cell-number";
qx.Class.define(f,{extend:qx.ui.virtual.cell.Cell,construct:function(g){arguments.callee.base.call(this);

if(g){this.setNumberFormat(g);
}},properties:{numberFormat:{check:e,init:qx.util.format.NumberFormat.getInstance()},appearance:{refine:true,init:c}},members:{getContent:function(a,b){return a!==null?this.getNumberFormat().format(a):d;
}}});
})();
...
(function(){var B="",A="px",z='" ',y="no-repeat",x='"',w='" style="',v='title="',u='<div style="',t='></div>',s="qx.ui.virtual.cell.AbstractImage",m='/>',r="img",p="inline-block",l="static",k="top",o='<img src="',n="abstract",q="-moz-inline-box";
...
},members:{__JL:16,__JM:16,_aliasManager:null,__JN:function(F){var I=qx.util.ResourceManager.getInstance();
...
},__JO:function(C){if(typeof (C)=="string"){C={url:C};
}var D=this._aliasManager.resolve(C.url||null);
var E;

if(C.width&&C.height){E={width:C.width,height:C.height};
}else{E=this.__JN(D);
...
},getContent:function(b,c){if(b===null){return B;
}var content=B;
var e=this.__JO(this._identifyImage(b));
var i=qx.bom.client.Engine.GECKO&&qx.bom.client.Engine.VERSION<1.9;
var f=e.tooltip?v+e.tooltip+x:B;
var d={width:e.width+A,height:e.height+A,display:i?q:p,verticalAlign:k,position:l};
var g=qx.bom.element.Decoration.getTagName(y,e.url);
var h=qx.bom.element.Decoration.getAttributes(e.url,y,d);
var j=qx.bom.element.Style.compile(h.style);

if(g===r){content=o+h.src+w+j+z;
content+=f+m;
}else{content=u+j+z;
content+=f+t;
}return content;
...
(function(){var c="cell-image",b="qx.ui.virtual.cell.Image";
qx.Class.define(b,{extend:qx.ui.virtual.cell.AbstractImage,properties:{appearance:{refine:true,init:c}},members:{_identifyImage:function(a){return a;
}}});
})();
(function(){var g="String",f="_applyIconFalse",e="qx.ui.virtual.cell.Boolean",d="_applyIconTrue",c="cell-boolean";
qx.Class.define(e,{extend:qx.ui.virtual.cell.AbstractImage,construct:function(){arguments.callee.base.call(this);
this.__JP=qx.util.AliasManager.getInstance();
},properties:{appearance:{refine:true,init:c},iconTrue:{check:g,themeable:true,apply:d},iconFalse:{check:g,themeable:true,apply:f}},members:{__JQ:null,__JR:null,__JP:null,_applyIconTrue:function(h){this.__JQ=this.__JP.resolve(h);
},_applyIconFalse:function(a){this.__JR=this.__JP.resolve(a);
},_identifyImage:function(b){return b==true?this.__JQ:this.__JR;
}}});
})();
