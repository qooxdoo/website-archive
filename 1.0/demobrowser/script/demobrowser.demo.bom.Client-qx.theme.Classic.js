...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.bom.Client-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"5f46eab44736"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var u="</td><td>",t="<tr><td>",s="<tr><td colspan='2'><b>",r="</td></tr>",q="</b></td></tr>",p="silverlight",o="wmv",n="<tr><td colspan='2'>&nbsp;</td></tr>",m="output",l="</table>",h="quicktime",k="demobrowser.demo.bom.Client",j="<table border='0'>",g="divx";
qx.Class.define(k,{extend:qx.application.Native,members:{main:function(){arguments.callee.base.call(this);
var e=new qx.util.StringBuilder();
e.add(j);
var b=[qx.bom.client.Engine,qx.bom.client.Browser,qx.bom.client.Version,qx.bom.client.System,qx.bom.client.Platform,qx.bom.client.Feature,qx.bom.client.Locale,qx.bom.client.Flash];

for(var i=0;i<b.length;i++){e.add(s,b[i].classname,q);

for(var c in b[i]){if(/^[A-Z_0-9]+$/.test(c)){e.add(t,c,u,new String(b[i][c]).toString(),r);
}}e.add(n);
}var a=qx.bom.client.Multimedia;
var d=[h,o,g,p];
e.add(s,a.classname,q);

for(var i=0;i<d.length;i++){e.add(t,d[i],u,a.has(d[i]),r);
}e.add(l);
...
(function(){var q="presto",p="1.7",o="1.9.2",n="1.8",m="[^\\.0-9]",l="1.9.1",k="webkit",j="1.9",i="gecko",h=".",e="qx.bom.client.Version",g="trident",f="1.8.1";
qx.Bootstrap.define(e,{statics:{NAME:"unknown",TITLE:"unknown 0.0",VERSION:0.0},defer:function(a){var c=navigator.userAgent;
var name,b;
if(/Presto[\s\/]([0-9]+\.[0-9\.]+)/.test(c)){name=q;
b=parseFloat(RegExp.$1,10);

if(b>=2.4){b=4;
}else if(b>=2.2){b=3;
}else if(b>=2.1){b=2;
}else{b=1;
}}else if(/Opera[\s\/]([0-9]+)\.([0-9])/.test(c)){name=q;
b=parseFloat(RegExp.$1+h+RegExp.$2,10);

if(b>=9.7){b=3;
}else if(b>=9.5){b=2;
}else{b=1;
}}else if(/AppleWebKit\/([^ ]+)/.test(c)){name=k;
b=RegExp.$1;
var d=RegExp(m).exec(b);

if(d){b=b.slice(0,d.index);
}b=parseFloat(b,10);
...
(function(){var j=".",h="boolean",g="qx.bom.client.Flash";
qx.Bootstrap.define(g,{statics:{AVAILABLE:false,FULLVERSION:"0.0.0",REVISION:"0",VERSION:0.0,EXPRESSINSTALL:false,STRICT_SECURITY_MODEL:false,_cachedSupportsVersion:{},supportsVersion:function(l){if(typeof this._cachedSupportsVersion[l]===h){return this._cachedSupportsVersion[l];
}else{var o=l.split(j);
var n=this.FULLVERSION.split(j);

for(var i=0;i<o.length;i++){var m=parseInt(n[i])-parseInt(o[i]);

if(m>0){return (this._cachedSupportsVersion[l]=true);
}else if(m<0){return (this._cachedSupportsVersion[l]=false);
}}return (this._cachedSupportsVersion[l]=true);
}},__BS:qx.core.Variant.select("qx.client",{"mshtml":function(){if(!window.ActiveXObject){return;
}var e=[0,0,0];
var d=false;

try{var f=new ActiveXObject("ShockwaveFlash.ShockwaveFlash.7");
}catch(s){try{var f=new ActiveXObject("ShockwaveFlash.ShockwaveFlash.6");
e=[6,0,21];
f.AllowScriptAccess="always";
}catch(t){if(e[0]==6){d=true;
}}
if(!d){try{f=new ActiveXObject("ShockwaveFlash.ShockwaveFlash");
}catch(p){}}}
if(!d&&typeof f=="object"){var c=f.GetVariable("$version");

if(typeof c!="undefined"){c=c.split(" ")[1].split(",");
e[0]=parseInt(c[0],10);
e[1]=parseInt(c[1],10);
e[2]=parseInt(c[2],10);
}}this.__BT(e);
...
var r=navigator.plugins["Shockwave Flash"].description;

if(typeof r!="undefined"){r=r.replace(/^.*\s+(\S+\s+\S+$)/,"$1");
q[0]=parseInt(r.replace(/^(.*)\..*$/,"$1"),10);
q[1]=parseInt(r.replace(/^.*\.(.*)\s.*$/,"$1"),10);
q[2]=/r/.test(r)?parseInt(r.replace(/^.*r(.*)$/,"$1"),10):0;
}this.__BT(q);
}}),__BT:function(a){this.FULLVERSION=a.join(".");
