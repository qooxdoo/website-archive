...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.widget.List-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"eda55f69894b"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var bK="value",bJ="multi",bI="one",bH="bold",bG="changeSelection",bF="additive",bE="single",bD="changeValue",bC="Item No ",bB="Julian",bq="Show Icon",bp="Kevin",bo="Marvin",bn="Marc",bm="Configurable",bl="icon/16/places/folder.png",bk="computer.png",bj="Jason",bi="Jakob",bh="Lukas",bR="Julius",bS="network-wired.png",bP="Jan",bQ="Tobias",bN="Mika",bO="Nils",bL="Noah",bM="Tom",bT="Nico",bU="Multi Selection",bu="battery.png",bt="Felix",bw="Leon",bv="input-keyboard.png",by="camera-web.png",bx="Enable drag selection",bA="icon",bz="Show Both",bs="48",br="icon/48/devices/",c="Fabian",d="multimedia-player.png",f="camera-photo.png",g="media-flash.png",h="input-mouse.png",j="Joel",k="Dominic",l="Show Label",m="demobrowser.demo.widget.List",n="Enable quick selection",bY="Finn",bX="One as selection mode",bW="Jannis",bV="audio-card.png",cd="Alexander",cc="Additive Selection",cb="display.png",ca="Erik",cf="addaptive",ce="Johannes",M="pda.png",N="16",K="Jonas",L="icon/",Q="David",R="Maximilian",O="Tim",P="Justin",I="Single Selection",J="Linus",v="Daniel",u="Paul",x="Philipp",w="Niclas",r="One Selection",q="Nick",t="Jonathan",s="Ben",p="on",o="Vincent",W="/places/folder.png",X="Robin",Y="both",ba="Jannick",S="Luis",T="drive-harddisk.png",U="Lennard",V="Moritz",bb="Florian",bc="drive-optical.png",F="media-optical.png",E="Marlon",D="Luca",C="network-wireless.png",B="audio-input-microphone.png",A="phone.png",z="Simon",y="Colin",H="label",G="printer.png",bd="Max",be="Additive selection",bf="Horizontal, Icons only",bg="Elias";
qx.Class.define(m,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var ci=new qx.ui.container.Scroll();
var cy=new qx.ui.container.Composite(new qx.ui.layout.Basic());
cy.setAllowStretchX(false);
ci.add(cy);
this.getRoot().add(ci,{edge:0});
var cs=new qx.ui.basic.Label(bm);
cs.setFont(bH);
cy.add(cs,{left:20,top:20});
var cA=new qx.ui.form.List;
cA.setScrollbarX(p);
cA.set({height:280,width:150,selectionMode:bJ});
var cB;

for(var i=1;i<=25;i++){cB=new qx.ui.form.ListItem(bC+i,L+((i%4)?N:bs)+W);
cA.add(cB);
!(i%9)&&(cB.setEnabled(false));
if(i==20){cA.setSelection([cB]);
}}cA.addListener(bG,function(e){var ch=e.getData();

if(ch[0]){this.debug("Value: "+ch[0].getLabel());
}});
cy.add(cA,{left:20,top:40});
var co=new qx.ui.form.RadioButton(I);
var cl=new qx.ui.form.RadioButton(bU);
var cm=new qx.ui.form.RadioButton(cc);
var ck=new qx.ui.form.RadioButton(r);
co.setUserData(bK,bE);
cl.setUserData(bK,bJ);
cm.setUserData(bK,bF);
ck.setUserData(bK,bI);
cl.setValue(true);
cy.add(co,{left:180,top:40});
cy.add(cl,{left:180,top:60});
cy.add(cm,{left:180,top:80});
cy.add(ck,{left:180,top:100});
var cn=new qx.ui.form.RadioGroup(co,cl,cm,ck);
var cE=new qx.ui.form.RadioButton(l);
var cD=new qx.ui.form.RadioButton(bq);
var cG=new qx.ui.form.RadioButton(bz);
cE.setUserData(bK,H);
cD.setUserData(bK,bA);
cG.setUserData(bK,Y);
cG.setValue(true);
cy.add(cE,{left:180,top:140});
cy.add(cD,{left:180,top:160});
cy.add(cG,{left:180,top:180});
var cz=new qx.ui.form.RadioGroup(cE,cD,cG);
cz.addListener(bG,function(e){for(var i=0;i<cA.getChildren().length;i++){cA.getChildren()[i].setShow(e.getData()[0].getUserData(bK));
}});
var cw=new qx.ui.form.CheckBox(bx);
var ct=new qx.ui.form.CheckBox(n).set({enabled:false});
cy.add(cw,{left:180,top:220});
cy.add(ct,{left:180,top:240});
cw.addListener(bD,function(e){if(e.getData()){var cg=cA.getSelectionMode();

if(cg==bE||cg==bI){this.debug("Drag selection is only available for the modes multi or additive");
}}cA.setDragSelection(e.getData());
});
ct.addListener(bD,function(e){if(e.getData()){var a=cA.getSelectionMode();

if(a==bJ||a==bF){this.debug("Quick selection is only available for the modes multi or additive");
}}cA.setQuickSelection(e.getData());
});
cn.addListener(bG,function(e){var b=e.getData()[0].getUserData(bK);
cA.setSelectionMode(b);

if(b==bE||b==bI){cw.setEnabled(false);
ct.setEnabled(true);
}else if(b==bJ||b==cf){cw.setEnabled(true);
ct.setEnabled(false);
}});
var cF=new qx.ui.basic.Label(bX);
cF.setFont(bH);
cy.add(cF,{left:330,top:20});
var cr=new qx.ui.form.List();
cr.set({height:280,width:150,selectionMode:bI});
var cB;

for(var i=1;i<=25;i++){cB=new qx.ui.form.ListItem(bC+i,bl);
cr.add(cB);
if(i==16){cr.setSelection([cB]);
}}cy.add(cr,{left:330,top:40});
var cs=new qx.ui.basic.Label(be);
cs.setFont(bH);
cy.add(cs,{left:520,top:20});
var cj=new qx.ui.form.List;
var cp;
cj.set({width:150,selectionMode:bF});
var cv=[bw,bh,D,bY,O,bt,K,S,R,bB,bd,u,w,bP,s,bg,ba,x,bL,bM,V,bT,Q,bO,z,c,ca,P,cd,bi,bb,q,J,bN,bj,v,U,bo,bW,bQ,k,E,bn,ce,t,bR,y,j,bp,o,X];

for(var i=0;i<cv.length;i++){cp=new qx.ui.form.ListItem(cv[i]);
cj.add(cp);

if(i==10||i==12||i==16){cj.addToSelection(cp);
}}cy.add(cj,{left:520,top:40});
var cs=new qx.ui.basic.Label(bf);
cs.setFont(bH);
cy.add(cs,{left:20,top:350});
var cC=new qx.ui.form.List(true);
var cq;
cC.set({width:550,selectionMode:bJ,height:null});
var cu=[bV,B,bu,f,by,bk,cb,T,bc,bv,h,g,F,d,bS,C,M,A,G];
var cx=br;

for(var i=0;i<cu.length;i++){cq=new qx.ui.form.ListItem(null,cx+cu[i]);
cC.add(cq);

if(i==12){cC.setSelection([cq]);
}}cy.add(cC,{left:20,top:370});
}}});
})();
