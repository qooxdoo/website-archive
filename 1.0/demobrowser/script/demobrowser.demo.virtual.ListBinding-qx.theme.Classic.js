...
var f=new qx.ui.virtual.form.List();
this.getRoot().add(f,{left:10,top:10});
var j=[];

for(var i=0;i<10000;i++){j.push(b+i);
}j=new qx.data.Array(j);
var g=new qx.ui.virtual.form.ListController(j,f);
g.setDelegate({filter:function(c){var d;
parseInt(c[c.length-1])%2?d=false:d=true;
return d;
},sorter:function(l,m){return l>m;
}});
var k=new qx.ui.virtual.form.List().set({useWidgetCells:true});
this.getRoot().add(k,{left:500,top:10});
new qx.ui.virtual.form.ListController(g.getSelection(),k);
var e=new qx.ui.virtual.form.List().set({useWidgetCells:true,cellRenderer:new demobrowser.demo.virtual.messenger.BuddyCell(),rowHeight:28});
var h=demobrowser.demo.virtual.messenger.BuddyModel.createBuddies(200);
new qx.ui.virtual.form.ListController(h,e);
this.getRoot().add(e,{left:10,top:320});
}}});
})();
...
(function(){var n="row-layer",m="selected",l="grid-lines",k="Boolean",j="Integer",h="cell.row",g="cell.renderer",f="_applyDelegate",d="qx.ui.virtual.form.List",c="Object",C="_applyCellRenderer",B="changeRowHeight",A="__Jm",z="_applyRowCount",y="changeCellRenderer",x="_applyShowGridLines",w="resize",v="changeShowGridLines",u="changeDelegate",t="_applyUseWidgetCells",r="changeRowCount",s="_applyRowHeight",p="__Jn",q="horizontal",o="virtual-list";
qx.Class.define(d,{extend:qx.ui.virtual.core.Scroller,construct:function(){arguments.callee.base.call(this,0,1,20,100);
qx.ui.core.queue.Widget.add(this);
this.getPane().addListener(w,this._onResize,this);
this._initSelectionManager();
this.initRowHeight();
this.initDelegate();
},properties:{appearance:{refine:true,init:o},rowCount:{check:j,event:r,init:0,apply:z},rowHeight:{check:j,event:B,init:20,apply:s,themeable:true},showGridLines:{check:k,event:v,init:false,apply:x,themeable:true},useWidgetCells:{check:k,init:false,apply:t,themeable:true},cellRenderer:{event:y,apply:C,themeable:true},delegate:{check:c,event:u,init:null,nullable:true,apply:f}},members:{__Jl:null,__Jm:null,__Jn:null,__Jo:null,_initWidgetLayer:function(){var self=this;
var bs={getCellWidget:function(bm,bn){var br=self._getCellData(bm);

if(!br){return null;
}var bo={};

if(self.__Jm.isItemSelected(bm)){bo.selected=true;
}var bp=self._getCellRenderer(bm);
var bq=bp.getCellWidget(br,bo);
bq.setUserData(h,bm);
bq.setUserData(g,bp);
return bq;
},poolCellWidget:function(I){var J=I.getUserData(g);
J.pool(I);
}};
this._showChildControl(n);
this.__Jn=new qx.ui.virtual.layer.WidgetCell(bs);
this.getPane().addLayer(this.__Jn);

if(!this.__Jl){this.setCellRenderer(qx.ui.virtual.form.ListItemCell.getInstance());
}},_initHtmlLayer:function(){var self=this;
var V={getCellProperties:function(P,Q){var R={};

if(self.__Jm.isItemSelected(P)){R.selected=true;
}return self._getCellRenderer(P).getCellProperties(self._getCellData(P),R);
}};
this._showChildControl(n);
this.__Jn=new qx.ui.virtual.layer.HtmlCell(V);
this.getPane().addLayer(this.__Jn);

if(!this.__Jl){this.setCellRenderer(new qx.ui.virtual.cell.Cell());
}},_initSelectionManager:function(){var self=this;
var Y={isItemSelectable:function(K){return self._delegate.isRowSelectable?self._delegate.isRowSelectable(K):true;
},styleSelectable:function(bj,bk,bl){if(self.__Jo){self._styleWidgetSelectable(bj,bk,bl);
}else{self._styleHtmlSelectable(bj,bk,bl);
}}};
this.__Jm=new qx.ui.virtual.selection.Row(this.getPane(),Y);
this.__Jm.attachMouseEvents(this.getPane());
this.__Jm.attachKeyEvents(this);
},getSelectionManager:function(){return this.__Jm;
},_createChildControlImpl:function(a){var b;

switch(a){case n:b=new qx.ui.virtual.layer.Row(null,null);
this.getPane().addLayer(b);
break;
case l:b=new qx.ui.virtual.layer.GridLines(q);
this.getPane().addLayer(b);
break;
}return b||arguments.callee.base.call(this,a);
},update:function(){if(this.__Jn){this.__Jn.updateLayerData();
}},_applyRowCount:function(N,O){this.getPane().getRowConfig().setItemCount(N);
},_applyRowHeight:function(W,X){this.getPane().getRowConfig().setDefaultItemSize(W);
},_applyShowGridLines:function(L,M){if(L){this._showChildControl(l);
}else{this._excludeChildControl(l);
}},_applyDelegate:function(S,T){this._delegate=S||{};
},_applyUseWidgetCells:function(bh,bi){if(this.__Jo!==null){throw new Error("The property 'useWidgetCells' cannot be set after the list has "+"been rendered.");
}},_applyCellRenderer:function(bt,bu){this.__Jl=bt;

if(this.__Jn){this.__Jn.fullUpdate();
}},_getCellData:function(U){return this._delegate.getCellData?this._delegate.getCellData(U):null;
},_getCellRenderer:function(H){return this._delegate.getCellRenderer?this._delegate.getCellRenderer(H):this.__Jl;
},_styleHtmlSelectable:function(D,E,F){if(E!==m){return;
}var G=this.getChildControl(n);

if(F){G.setBackground(D,m);
}else{G.setBackground(D,null);
}this.__Jn.updateLayerData();
},_styleWidgetSelectable:function(ba,bb,bc){if(bb!==m){return;
}var bd=this.__Jn.getChildren();

for(var i=0;i<bd.length;i++){var be=bd[i];
var bg=be.getUserData(h);

if(ba!==bg){continue;
}
if(this.getPane().isUpdatePending()){continue;
}var bf=this._getCellRenderer(ba);

if(bc){bf.updateStates(be,{selected:1});
}else{bf.updateStates(be,{});
}}},syncWidget:function(){if(this.__Jo!==null){return;
}this.__Jo=this.getUseWidgetCells();

if(this.__Jo){this._initWidgetLayer();
}else{this._initHtmlLayer();
}},_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);
}},destruct:function(){this._delegate=this.__Jl=null;
this._disposeObjects(A,p);
}});
})();
...
(function(){var o="cell.empty",n="cell.row",m="cell.column",l="qx.ui.virtual.layer.WidgetCell";
qx.Class.define(l,{extend:qx.ui.virtual.layer.Abstract,include:[qx.ui.core.MChildrenHandling],construct:function(b){arguments.callee.base.call(this);
this.setZIndex(2);
{};
...
var k=this.getRowSizes().length;
var j=this.getFirstRow();
var h=this.getFirstColumn();

if(c<j||c>=j+k||d<h||d>=h+e){return null;
}var g=(d-h)+(c-j)*e;
var f=this._getChildren()[g];

if(f.getUserData(o)){return null;
}else{return f;
}},__vz:null,_getSpacer:function(){var p=this.__vz.pop();

if(!p){p=new qx.ui.core.Spacer();
p.setUserData(o,1);
}return p;
...
if(R.getUserData(o)){this.__vz.push(R);
...
U=0;
}},_updateLayerWindow:function(q,r,s,t){var H=q+s.length-1;
var w=r+t.length-1;
var D={firstRow:Math.max(q,this.getFirstRow()),lastRow:Math.min(H,this._lastRow),firstColumn:Math.max(r,this.getFirstColumn()),lastColumn:Math.min(w,this._lastColumn)};
this._lastColumn=w;
this._lastRow=H;

if(D.firstRow>D.lastRow||D.firstColumn>D.lastColumn){return this._fullUpdate(q,r,s,t);
}var I=this._getChildren();
var u=this.getColumnSizes().length;
var F=[];
var C={};

for(var J=q;J<=H;J++){F[J]=[];

for(var B=r;B<=w;B++){if(J>=D.firstRow&&J<=D.lastRow&&B>=D.firstColumn&&B<=D.lastColumn){var x=B-this.getFirstColumn();
var y=J-this.getFirstRow();
var v=y*u+x;
F[J][B]=I[v];
C[v]=true;
}}}var E=this._cellProvider;
var I=this._getChildren();

for(var i=0;i<I.length;i++){if(!C[i]){var G=I[i];

if(G.getUserData(o)){this.__vz.push(G);
}else{E.poolCellWidget(G);
}}}this._removeAll();
var top=0;
var z=0;

for(var y=0;y<s.length;y++){for(var x=0;x<t.length;x++){var J=q+y;
var B=r+x;
var A=F[J][B]||E.getCellWidget(J,B)||this._getSpacer();
A.setUserBounds(z,top,t[x],s[y]);
A.setUserData(n,J);
A.setUserData(m,B);
this._add(A);
z+=t[x];
...
}this._cellProvider=this.__vz=null;
}});
})();
...
qx.Interface.define(a,{members:{getCellWidget:function(e,f){},pool:function(d){},updateStates:function(g,h){},updateData:function(b,c){}}});
})();
...
this.__Jp=[];
},members:{__Jp:null,_createWidget:function(){throw new Error("abstract method call");
},updateData:function(c,d){throw new Error("abstract method call");
},updateStates:function(e,f){var i=e.getUserData(b);
if(i){var g=f||{};

for(var h in i){if(!g[h]){e.removeState(h);
}}}else{i={};
}if(f){for(var h in f){if(!i.state){e.addState(h);
}}}e.setUserData(b,f);
},getCellWidget:function(k,l){var m=this.__Jp.pop()||this._createWidget();
this.updateStates(m,l);
this.updateData(m,k);
return m;
},pool:function(j){this.__Jp.push(j);
...
qx.Class.define(k,{extend:qx.ui.virtual.layer.Abstract,construct:function(t){arguments.callee.base.call(this);
this.setZIndex(2);
{};
...
},members:{getCssClasses:function(a,b){return e;
},getAttributes:function(i,j){return h;
},getStyles:function(c,d){return h;
...
(function(){var P="Integer",O="_applyPadding",N="paddingLeft",M="Color",L="",K="paddingBottom",J="paddingTop",I="paddingRight",H=";",G="-",bp="backgroundColor",bo="_applyTextAlign",bn="px",bm="_applyFont",bl="_applyBackgroundColor",bk="cell",bj="center",bi="font",bh=":",bg="shorthand",W="String",X="_applyAppearance",U="background-color:",V="textAlign",S="qx-cell ",T="left",Q="textColor",R="text-align:",Y="Font",ba="right",bc="_applyTextColor",bb="justify",be="color:",bd=" ",bf="qx.ui.virtual.cell.Cell";
qx.Class.define(bf,{extend:qx.ui.virtual.cell.Abstract,construct:function(){arguments.callee.base.call(this);
this.__vR=qx.ui.virtual.cell.CellStylesheet.getInstance();
this.__vS={};
this.__vT={};
this.__vU={};
this.__vV={};
...
this.initAppearance();
this.__wc();
},properties:{appearance:{check:W,init:bk,apply:X},backgroundColor:{nullable:true,check:M,apply:bl,themeable:true},textColor:{nullable:true,check:M,apply:bc,themeable:true},textAlign:{check:[T,bj,ba,bb],nullable:true,themeable:true,apply:bo},font:{nullable:true,apply:bm,check:Y,themeable:true},paddingTop:{check:P,init:0,apply:O,themeable:true},paddingRight:{check:P,nullable:true,apply:O,themeable:true},paddingBottom:{check:P,nullable:true,apply:O,themeable:true},paddingLeft:{check:P,nullable:true,apply:O,themeable:true},padding:{group:[J,I,K,N],mode:bg,themeable:true}},members:{__vY:null,__wa:null,__vW:null,__vX:null,__vT:null,__vS:null,__vU:null,__vV:null,__wb:false,__vR:null,__wc:function(){var bS=qx.util.PropertyUtil;
var bU=qx.lang.Object.fromArray(this._getCssProperties());
this.__vY=[];
var bR=this.constructor;

while(bR){var bT=bS.getProperties(bR);

for(var bQ in bT){if(!bU[bQ]){this.__vY.push(bQ);
}}bR=bR.superclass;
}},_getCssProperties:function(){return [bp,Q,bi,V,J,I,K,N];
},_applyAppearance:function(E,F){if(F){this.__vT={};
}},_getValue:function(bC){if(this.__wb){return qx.util.PropertyUtil.getThemeValue(this,bC);
}else{return qx.util.PropertyUtil.getUserValue(this,bC);
}},_storeStyle:function(bJ,bK){var bL;

if(this.__wb){bL=this.__vT;
}else{bL=this.__vS;
}
if(bK===null){delete bL[bJ];
}else{bL[bJ]=bK;
}},_applyBackgroundColor:function(bD,bE,name){var bD=this._getValue(name);

if(!bD){this._storeStyle(name,null);
}else{this._storeStyle(name,U+qx.theme.manager.Color.getInstance().resolve(bD));
}},_applyTextColor:function(A,B,name){var A=this._getValue(name);

if(!A){this._storeStyle(name,null);
}else{this._storeStyle(name,be+qx.theme.manager.Color.getInstance().resolve(A));
}},_applyTextAlign:function(j,k,name){var j=this._getValue(name);

if(!j){this._storeStyle(name,null);
}else{this._storeStyle(name,R+j);
...
}},_applyPadding:function(bM,bN,name){var bM=this._getValue(name);

if(this.__wb){var bP=this.__vV;
}else{bP=this.__vU;
}
if(bM===null){delete bP[name];
}else{bP[name]=bM;
}
if(bM===null){this._storeStyle(name,null);
}else{var bO=qx.lang.String.hyphenate(name);
this._storeStyle(name,bO+bh+bM+bn);
}},getCellProperties:function(a,b){this.__wd(b);
return {classes:this.getCssClasses(a,b),style:this.getStyles(a,b),attributes:this.getAttributes(a,b),content:this.getContent(a,b),insets:this.getInsets(a,b)};
},getAttributes:function(C,D){return L;
},getContent:function(bv,bw){return bv;
},getCssClasses:function(bx,by){var bz=this.__vR.getCssClass(this.__wa)||L;
return S+bz;
},__wd:function(c){if(!c){c={};
}var e=this.getAppearance();
var d=e+G+qx.lang.Object.getKeys(c).sort().join(bd);

if(this.__wa==d){return;
}this.__wa=d;
var f=this.__vW[this.__wa];

if(!f){this.__we();
this.__wf(c);
this.__wg(c);
this.__wh();
var f=qx.lang.Object.clone(this.__vT);
this.__vW[this.__wa]=1;
}this.__wi();
},__we:function(){var bA=qx.util.PropertyUtil;
var bB=this._getCssProperties();

for(var i=0;i<bB.length;i++){bA.deleteThemeValue(this,bB[i]);
}},__wf:function(bq){this.__vT={};
this.__wb=true;
var bt=this.getAppearance();
var bs=qx.util.PropertyUtil;
var br=qx.theme.manager.Appearance.getInstance().styleFrom(bt,bq);

for(var bu in br){if(br[bu]!==undefined){bs.setThemed(this,bu,br[bu]);
}}this.__wb=false;
},__wg:function(){var bF=qx.lang.Object.getValues(this.__vT).join(H);
this.__vR.computeClassForStyles(this.__wa,bF);
},__wh:function(){var z=this.__vY;
var v=qx.util.PropertyUtil;
var x={};

for(var i=0;i<z.length;i++){var y=z[i];
var w=v.getThemeValue(this,y);

if(w!==undefined){x[y]=w;
}}this.__vX[this.__wa]=x;
},__wi:function(){var bH=qx.util.PropertyUtil;
var bG=this.__vX[this.__wa]||{};

for(var bI in bG){bH.setThemed(this,bI,bG[bI]);
}},getStyles:function(g,h){return qx.lang.Object.getValues(this.__vS).join(H);
},getInsets:function(l,m){var q=this.__vU;
var p=this.__vV;
...
return [r+n,top+o];
}},destruct:function(){this.__vR=this.__vS=this.__vT=this.__vU=this.__vV=this.__vW=this.__vX=this.__vY=null;
}});
})();
...
qx.Class.define(q,{extend:qx.core.Object,type:e,construct:function(){arguments.callee.base.call(this);
var a=t+
qx.bom.element.Style.compile({position:h,overflow:k,cursor:n,textOverflow:m,userSelect:p})+g;

if(!qx.core.Variant.isSet(i,r)){a+=t+qx.bom.element.BoxSizing.compile(f)+l;
}this.__wj=qx.bom.Stylesheet.createElement(a);
this.__wk={};
this.__wl={};
},members:{__wj:null,__wk:null,__wl:null,getStylesheet:function(){return this.__wj;
},getCssClass:function(u){return this.__wk[u]||null;
},computeClassForStyles:function(b,c){var d=this.__wl[c];

if(!d){var d=this.__wm();
qx.bom.Stylesheet.addRule(this.__wj,j+d,c);
this.__wl[c]=d;
}this.__wk[b]=d;
return d;
},__wm:function(){return o+this.toHashCode()+s+(this.__wn++);
},__wn:0},destruct:function(){this.__wj=this.__wk=this.__wl=null;
}});
})();
(function(){var q="$$theme_",p="$$user_",o="$$init_",n="qx.util.PropertyUtil";
qx.Class.define(n,{statics:{getProperties:function(f){return f.$$properties;
},getAllProperties:function(g){var j={};
var k=g;
while(k!=qx.core.Object){var i=this.getProperties(k);

for(var h in i){j[h]=i[h];
}k=k.superclass;
}return j;
},getUserValue:function(a,b){return a[p+b];
},setUserValue:function(z,A,B){z[p+A]=B;
},deleteUserValue:function(r,s){delete (r[p+s]);
},getInitValue:function(x,y){return x[o+y];
},setInitValue:function(G,H,I){G[o+H]=I;
},deleteInitValue:function(l,m){delete (l[o+m]);
},getThemeValue:function(C,D){return C[q+D];
},setThemeValue:function(c,d,e){c[q+d]=e;
},deleteThemeValue:function(E,F){delete (E[q+F]);
...
(function(){var r="mouseup",q="mousedown",p="losecapture",o="mouseover",n="mousemove",m="removeItem",l="keypress",k="addItem",j="qx.ui.virtual.selection.Abstract";
qx.Class.define(j,{extend:qx.ui.core.selection.Abstract,construct:function(e,f){arguments.callee.base.call(this);
{};
this._pane=e;
this._delegate=f||{};
},members:{_isSelectable:function(x){return this._delegate.isItemSelectable?this._delegate.isItemSelectable(x):true;
},_styleSelectable:function(g,h,i){if(this._delegate.styleSelectable){this._delegate.styleSelectable(g,h,i);
}},attachMouseEvents:function(){var a=this._pane.getContainerElement();
a.addListener(q,this.handleMouseDown,this);
a.addListener(r,this.handleMouseUp,this);
a.addListener(o,this.handleMouseOver,this);
a.addListener(n,this.handleMouseMove,this);
a.addListener(p,this.handleLoseCapture,this);
},detatchMouseEvents:function(){var d=this._pane.getContainerElement();
d.removeListener(q,this.handleMouseDown,this);
d.removeListener(r,this.handleMouseUp,this);
d.removeListener(o,this.handleMouseOver,this);
d.removeListener(n,this.handleMouseMove,this);
d.removeListener(p,this.handleLoseCapture,this);
},attachKeyEvents:function(v){v.addListener(l,this.handleKeyPress,this);
},detachKeyEvents:function(w){w.removeListener(l,this.handleKeyPress,this);
},attachListEvents:function(t){t.addListener(k,this.handleAddItem,this);
t.addListener(m,this.handleRemoveItem,this);
},detachListEvents:function(u){u.removeListener(k,this.handleAddItem,this);
u.removeListener(m,this.handleRemoveItem,this);
...
},_scrollBy:function(b,c){this._pane.setScrollX(this._pane.getScrollX()+b);
this._pane.setScrollY(this._pane.getScrollY()+c);
...
(function(){var e="qx.ui.virtual.selection.Row",d="above",c="under";
qx.Class.define(e,{extend:qx.ui.virtual.selection.Abstract,members:{_getItemCount:function(){return this._pane.getRowConfig().getItemCount();
},_getSelectableFromMouseEvent:function(event){var o=this._pane.getCellAtPosition(event.getDocumentLeft(),event.getDocumentTop());

if(!o){return null;
}return this._isSelectable(o.row)?o.row:null;
},getSelectables:function(){var b=[];

for(var i=0,l=this._getItemCount();i<l;i++){if(this._isSelectable(i)){b.push(i);
}}return b;
},_getSelectableRange:function(q,r){var s=[];
var u=Math.min(q,r);
var t=Math.max(q,r);

for(var i=u;i<=t;i++){if(this._isSelectable(i)){s.push(i);
}}return s;
...
},_getRelatedSelectable:function(x,y){if(y==d){var A=x-1;
var z=0;
var B=-1;
}else if(y==c){var A=x+1;
...
},_scrollItemIntoView:function(v){this._pane.scrollRowIntoView(v);
},_getSelectableLocationX:function(p){return {left:0,right:this._pane.getColumnConfig().getTotalSize()-1};
},_getSelectableLocationY:function(g){var k=this._pane.getRowConfig();
var j=k.getItemPosition(g);
var h=j+k.getItemSize(g)-1;
return {top:j,bottom:h};
}}});
})();
...
if(this.__ru(a)){this.updateLayerData();
}},setBackground:function(f,g){arguments.callee.base.call(this,f,g);

if(this.__ru(f)){this.updateLayerData();
}},__ru:function(c){var e=this.getFirstRow();
var d=e+this.getRowSizes().length-1;
return c>=e&&c<=d;
}}});
})();
(function(){var A="px;",z="</div>",w="background-color:",v="horizontal",u="position: absolute;",t="<div style='",s="'>",r="top:",q="",p="height: ",i="gray",o="width: 100%;",l="width: ",h="height: 100%;",g="qx.ui.virtual.layer.GridLines",k="left:",j="html",m="Color",f="PositiveInteger",n="top: 0px;";
qx.Class.define(g,{extend:qx.ui.virtual.layer.Abstract,construct:function(X,Y,ba){arguments.callee.base.call(this);
this.setZIndex(1);
...
this.__sD=[];
this._isHorizontal=(X||v)==v;
},properties:{defaultLineColor:{init:i,check:m,themeable:true},defaultLineSize:{init:1,check:f,themeable:true}},members:{__sC:null,__sD:null,isHorizontal:function(){return this._isHorizontal;
},setLineColor:function(B,C){{};
this.__sC[B]=C;

if(this.__sE(B)){this.updateLayerData();
}},setLineSize:function(Q,R){{};
this.__sD[Q]=R;

if(this.__sE(Q)){this.updateLayerData();
}},__sE:function(S){if(this._isHorizontal){var W=this.getFirstColumn();
var U=W+this.getColumnSizes().length-1;
return S>=W&&S<=U;
}else{var V=this.getFirstRow();
var T=V+this.getRowSizes().length-1;
return S>=V&&S<=T;
}},getLineSize:function(D){return this.__sD[D]||this.getDefaultLineSize();
},getLineColor:function(bg){return this.__sC[bg]||this.getDefaultLineColor();
},__sF:function(bb,bc,bd){var top=0;
var be,bf;

for(var y=0;y<bd.length-1;y++){be=this.getLineColor(bc+y);
bf=this.getLineSize(bc+y);
top+=bd[y];
bb.push(t,u,p+bf+A,o,r,top-((bf>1)?Math.floor(bf/2):1),A,w,be,s,z);
}},__sG:function(E,F,G){var J=0;
var H,I;

for(var x=0;x<G.length-1;x++){H=this.getLineColor(F+x);
I=this.getLineSize(F+x);
J+=G[x];
E.push(t,u,l+I+A,h,n,k,J-((I>1)?Math.floor(I/2):1),A,w,H,s,z);
}},_fullUpdate:function(a,b,c,d){var e=[];

if(this._isHorizontal){this.__sF(e,a,c);
}else{this.__sG(e,b,d);
}this.getContentElement().setAttribute(j,e.join(q));
},_updateLayerWindow:function(K,L,M,N){var O=K!==this.getFirstRow()||M.length!==this.getRowSizes().length;
var P=L!==this.getFirstColumn()||N.length!==this.getColumnSizes().length;

if((this._isHorizontal&&O)||(!this._isHorizontal&&P)){this._fullUpdate(K,L,M,N);
...
(function(){var bg="sorter",bf="change",be="changeSelection",bd="changeLength",bc="qx.data.IListData",bb="filter",ba="changeModel",Y="_applyDelegate",X="changeDelegate",W="",Q="qx.ui.virtual.form.ListController",V="_applyTarget",T="qx.ui.virtual.form.List",P="_applySelection",O="changeTarget",S="changeBubble",R="_applyModel",U="object";
qx.Class.define(Q,{extend:qx.core.Object,construct:function(p,q){arguments.callee.base.call(this);
this.setSelection(new qx.data.Array());

if(p!=null){this.setModel(p);
}
if(q!=null){this.setTarget(q);
}},properties:{target:{check:T,event:O,nullable:true,init:null,apply:V},model:{check:bc,event:ba,nullable:true,init:null,apply:R},selection:{check:bc,event:be,apply:P},delegate:{apply:Y,event:X,init:null,nullable:true}},members:{__Jq:null,__Jr:null,__Js:null,__Jt:null,__Ju:null,__Jv:null,__Jw:null,_validDelegates:[bg,bb],_getRowData:function(r){var s=this.getModel();
return s?s.getItem(this.__Jy(r)):null;
},_getModelRow:function(L){return this.getModel().indexOf(L);
},getRowCount:function(){return this.__Jw.length;
},update:function(){this.__Jx();
this._syncViewSelectionToModel();
},__Jx:function(){var g=this.getModel();

if(g==null){return;
}this.__Jw=[];
this._runDelegateFilter(g);
this._runDelegateSorter(g);

if(this.getTarget()!=null){this._syncRowCount();
}},_runDelegateSorter:function(M){if(!this._containsDelegateMethod(this.getDelegate(),bg)||qx.lang.Type.isObject(M)){return;
}
if(this.__Jw.length==0){return;
}var N=this._getDelegate(bg);

if(N!=null){this.__Jw.sort(function(a,b){return N(M.getItem(a),M.getItem(b));
});
}},_runDelegateFilter:function(B){if(typeof B!==U){return;
}var C=this._getDelegate(bb);

for(var i=0,l=B.length;i<l;++i){if(C==null||C(B.getItem(i))){this.__Jw.push(i);
}}},_containsDelegateMethod:function(v,w){var y=qx.lang.Type;

if(y.isObject(v)){if(y.isString(w)){return y.isFunction(v[w]);
}else{for(var x in this._validDelegates){if(y.isFunction(v[x])){return true;
}}}}return false;
},_getDelegate:function(c){var d=this.getDelegate();

if(this._containsDelegateMethod(d,c)){return d[c];
}return null;
},__Jy:function(bh){return this.__Jw[bh];
},_applyDelegate:function(h,j){if(this.getTarget()==null||this.getModel()==null){return;
}
if(this._containsDelegateMethod(h)){this.__Jx();
}},_applyTarget:function(z,A){if(z!=null){z.setDelegate(this);
this.__Jt=z.getSelectionManager().addListener(be,this._onChangeSelectionView,this);
}
if(A!=null){A.setDelegate(null);
A.getSelectionManager().removeListenerById(this.__Jt);
}
if(this.getModel()==null){return;
}this.__Jx();
this._syncRowCount();
},_applyModel:function(t,u){if(u!=null){u.removeListenerById(this.__Jq);
u.removeListenerById(this.__Jr);
u.removeListenerById(this.__Js);
}
if(t!=null){this.__Jx();
this.__Jq=t.addListener(bd,this._onChangeLengthModel,this);
this.__Jr=t.addListener(bf,this._onChangeModel,this);
this.__Js=t.addListener(S,this._onChangeBubbleModel,this);
}
if(this.getTarget()!=null){this._syncRowCount();
}},_applySelection:function(n,o){if(n!=null){this.__Ju=n.addListener(bf,this._onChangeSelectionModel,this);
this.__Jv=n.addListener(bd,this._onChangeSelectionModel,this);
this._syncModelSelectionToView();
}
if(o!=null){o.removeListenerById(this.__Ju);
o.removeListenerById(this.__Jv);
...
},_onChangeLengthModel:function(e){this.__Jx();
this._syncRowCount();
},_onChangeModel:function(e){var m=this.getTarget();
...
if(!G){this.getSelection().removaeAll();
return;
}var F=G.getSelectionManager().getSelection();
var E=[];

for(var i=0;i<F.length;i++){var D=this._getRowData(F[i]);
E.push(D);
}E.unshift(this.getSelection().length);
E.unshift(0);
this._ignoreSelectionChange=true;
this.getSelection().splice.apply(this.getSelection(),E);
...
for(var i=H.length;i>=0;i--){var K=this._getModelRow(H.getItem(i));

if(K!==-1){I.push(K);
}else{H.removeAt(i);
}}J.getSelectionManager().replaceSelection(I);
this._ignoreSelectionChange=false;
},_syncRowCount:function(){var length=this.getRowCount();
this.getTarget().setRowCount(length);
},getCellData:function(f){return this._getRowData(f)||W;
...
},getRandomBuddy:function(){var Q=[B,s,L,u,I,F,r,H,O,G];
return v+Q[Math.floor(Math.random()*Q.length)]+M;
},getRandomStatus:function(){var P=[f,d,e,j];
