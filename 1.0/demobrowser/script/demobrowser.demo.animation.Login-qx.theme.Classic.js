...
(function(){var o="top",n="right",m="resize",l="Name",k="Password",j="__BQ",h="appear",g="__BO",f="__BP",d="demobrowser.demo.animation.Login",a="50%",c="execute",b="30%";
qx.Class.define(d,{extend:qx.application.Standalone,members:{__BO:null,__BP:null,__BQ:null,checkInput:function(){this.__BO.start();
},__BR:function(){this.__BO=new qx.fx.effect.combination.Shake(this.__BP.getContainerElement().getDomElement());
},main:function(){arguments.callee.base.call(this);
var r=new qx.ui.layout.Grid(9,5);
r.setColumnAlign(0,n,o);
r.setColumnAlign(2,n,o);
this.__BP=new qx.ui.groupbox.GroupBox().set({contentPadding:[16,16,16,16]});
this.__BP.setLayout(r);
this.__BP.addListener(m,function(e){var p=this.__BP.getBounds();
this.__BP.set({marginTop:Math.round(-p.height/2),marginLeft:Math.round(-p.width/2)});
},this);
this.getRoot().add(this.__BP,{left:a,top:b});
var q=[l,k];

for(var i=0;i<q.length;i++){this.__BP.add(new qx.ui.basic.Label(q[i]).set({allowShrinkX:false,paddingTop:3}),{row:i,column:0});
}var s=new qx.ui.form.TextField();
var t=new qx.ui.form.PasswordField();
this.__BP.add(s.set({allowShrinkX:false,paddingTop:3}),{row:0,column:1});
this.__BP.add(t.set({allowShrinkX:false,paddingTop:3}),{row:1,column:1});
var u=this.__BQ=new qx.ui.form.Button("Login");
u.setAllowStretchX(false);
this.__BP.add(u,{row:3,column:1});
u.addListener(c,this.checkInput,this);
this.__BP.addListener(h,this.__BR,this);
}},destruct:function(){this._disposeObjects(f,j,g);
}});
})();
...
(function(){var j="horizontal",h="vertical",g="__Bj",f="qx.fx.effect.combination.Shake",e="Number";
qx.Class.define(f,{extend:qx.fx.Base,construct:function(k){arguments.callee.base.call(this,k);
this.__Bj=[new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k)];
},properties:{direction:{init:j,check:[j,h]},duration:{init:0.5,refine:true},distance:{init:20,check:e}},members:{__Bj:null,start:function(){if(!arguments.callee.base.call(this)){return;
}var a=parseFloat(this.getDistance());
var b=parseFloat(this.getDuration())/10.0;

if(this.getDirection()==j){this.__Bj[0].set({x:a,y:0,duration:b});
this.__Bj[1].set({x:-a*2,y:0,duration:b*2});
this.__Bj[2].set({x:a*2,y:0,duration:b*2});
this.__Bj[3].set({x:-a*2,y:0,duration:b*2});
this.__Bj[4].set({x:a*2,y:0,duration:b*2});
this.__Bj[5].set({x:-a,y:0,duration:b*2});
}else if(this.getDirection()==h){this.__Bj[0].set({y:a,x:0,duration:b});
this.__Bj[1].set({y:-a*2,x:0,duration:b*2});
this.__Bj[2].set({y:a*2,x:0,duration:b*2});
this.__Bj[3].set({y:-a*2,x:0,duration:b*2});
this.__Bj[4].set({y:a*2,x:0,duration:b*2});
this.__Bj[5].set({y:-a,x:0,duration:b*2});
}var d=this.__Bj;

for(var i=0,c=this.__Bj.length;i<c;i++){this.__Bj[i].id=i;

if(i<5){this.__Bj[i].afterFinishInternal=function(){d[this.id+1].start();
};
}}this.__Bj[0].start();
}},destruct:function(){this._disposeArray(g);
}});
...
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:g,check:[g,e]},x:{init:0,check:f},y:{init:0,check:f}},members:{__AN:null,__AO:null,__AP:null,__AQ:null,__AR:null,setup:function(){var m=this._getElement();
arguments.callee.base.call(this);

if(m.parentNode){this.__AP=qx.bom.element.Location.getLeft(m)-qx.bom.element.Location.getLeft(m.parentNode);
this.__AQ=qx.bom.element.Location.getTop(m)-qx.bom.element.Location.getTop(m.parentNode);
}else{this.__AP=qx.bom.element.Location.getLeft(m);
this.__AQ=qx.bom.element.Location.getTop(m);
}this.__AR=qx.bom.element.Style.get(m,"position");
qx.bom.element.Style.set(m,h,e);

if(this.getMode()==b){this.__AN=this.getX()-this.__AP;
this.__AO=this.getY()-this.__AQ;
}else{this.__AN=this.getX();
this.__AO=this.getY();
}},update:function(j){var l=this._getElement();
arguments.callee.base.call(this);
var k=Math.round(this.__AN*j+this.__AP);
var top=Math.round(this.__AO*j+this.__AQ);
qx.bom.element.Style.set(l,c,k+i);
qx.bom.element.Style.set(l,d,top+i);
},afterFinishInternal:function(){qx.bom.element.Style.set(this._getElement(),h,this.__AR);
}}});
})();
