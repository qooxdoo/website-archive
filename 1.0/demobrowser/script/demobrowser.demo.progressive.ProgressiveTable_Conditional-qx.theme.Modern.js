...
(function(){var K="",J="regex",I="==",H=">",G="!between",F="between",E="<",D='g',C=":",B=";",y=">=",A="qx.ui.progressive.renderer.table.cell.Conditional",z="!=",x="<=";
qx.Class.define(A,{extend:qx.ui.progressive.renderer.table.cell.Abstract,construct:function(L,M,N,O){arguments.callee.base.call(this);
this.__HP=["==","!=",">","<",">=","<="];
this.__HQ=["between","!between"];
this.__HR=[];
this.__HS=L||"";
this.__HT=M||"";
this.__HU=N||"";
this.__HV=O||"";
},members:{__HP:null,__HQ:null,__HR:null,__HS:null,__HT:null,__HU:null,__HV:null,__HW:function(W,X){if(W.align){X["text-align"]=W.align;
}
...
}},addNumericCondition:function(P,Q,R,S,T,U,V){if(!qx.lang.Array.contains(this.__HP,P)||Q==null){throw new Error("Condition not recognized or value is null!");
}this.__HR.push({condition:P,align:R,color:S,style:T,weight:U,value1:Q,target:V});
},addBetweenCondition:function(a,b,c,d,e,f,g,h){if(!qx.lang.Array.contains(this.__HQ,a)||b==null||c==null){throw new Error("Condition not recognized or value1/value2 is null!");
}this.__HR.push({condition:a,align:d,color:e,style:f,weight:g,value1:b,value2:c,target:h});
},addRegex:function(j,k,l,m,n,o){if(!j){throw new Error("regex cannot be null!");
}this.__HR.push({condition:J,align:k,color:l,style:m,weight:n,regex:j,target:o});
},_getCellStyle:function(p){if(this.__HR.length==0){return p.style||K;
}var i;
var t;
var q;
var s={"text-align":this.__HS,"color":this.__HT,"font-style":this.__HU,"font-weight":this.__HV};

for(i=0;i<this.__HR.length;i++){var w=this.__HR[i];
t=false;

if(qx.lang.Array.contains(this.__HP,w.condition)){if(w.target==null){q=p.cellData;
}else{q=p.element.data[w.target];
}
switch(w.condition){case I:if(q==w.value1){t=true;
}break;
case z:if(q!=w.value1){t=true;
}break;
case H:if(q>w.value1){t=true;
}break;
case E:if(q<w.value1){t=true;
}break;
case y:if(q>=w.value1){t=true;
}break;
case x:if(q<=w.value1){t=true;
}break;
}}else if(qx.lang.Array.contains(this.__HQ,w.condition)){if(w.target==null){q=p.cellData;
}else{q=p.element.data[w.target];
}
switch(w.condition){case F:if(q>=w.value1&&q<=w.value2){t=true;
}break;
case G:if(q<w.value1&&q>w.value2){t=true;
}break;
}}else if(w.condition==J){if(w.target==null){q=p.cellData;
}else{q=p.element.data[w.target];
}var r=new RegExp(w.value1,D);
t=r.test(q);
}if(t){this.__HW(w,s);
}var u=[];

for(var v in s){if(s[v]){u.push(v,C,s[v],B);
}}}return u.join(K);
}},destruct:function(){this.__HP=this.__HQ=this.__HR=null;
}});
})();
