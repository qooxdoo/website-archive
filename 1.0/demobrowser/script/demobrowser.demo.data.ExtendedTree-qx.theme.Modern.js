...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.data.ExtendedTree","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.data.ExtendedTree-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"25d479939f73"},
  boot : "boot",
  bootIsInline : true,
...
qx.Class.define(v,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var d=[s,h,k,A,t,n];
var f=[];
...
(function(){var q="qx.data.Array",p="demobrowser.demo.data.model.Node",o="String",n="changeNames",m="changeName",l="__FC",k="- ",j="",h="changeChild",g="changeColor",c="Marge",f="AFFE",e="Homer",b="\n",a="changeName2",d="changeChildren";
...
},properties:{children:{check:q,event:d},child:{check:p,event:h,nullable:true},names:{check:q,event:n,init:new qx.data.Array(e,c)},name:{check:o,init:f,event:m},name2:{check:o,event:a},number:{init:10,validate:l},color:{event:g,nullable:true}},members:{__FC:function(t){if(!isNaN(parseFloat(t))){if(/^\d+$/gi.test(t)){return;
...
qx.Class.define(d,{extend:qx.ui.core.selection.ScrollArea,members:{_getSelectableLocationY:function(k){var l=k.getBounds();

if(l){var top=this._getWidget().getItemTop(k);
return {top:top,bottom:top+l.height};
}},_isSelectable:function(j){return j instanceof qx.ui.tree.AbstractTreeItem&&j.isEnabled()&&j.isVisible();
...
switch(s){case g:u=t.getPreviousSiblingOf(r,false);
break;
case e:u=t.getNextSiblingOf(r,false);
break;
case f:case h:break;
}
if(!u){return null;
...
},_applyRoot:function(bb,bc){var bd=this.getChildrenContainer();

if(bc){bd.remove(bc);

if(bc.hasChildren()){bd.remove(bc.getChildrenContainer());
}}
if(bb){bd.add(bb);

if(bb.hasChildren()){bd.add(bb.getChildrenContainer());
}bb.setVisibility(this.getHideRoot()?s:r);
bb.recursiveAddToWidgetQueue();
}},_applyHideRoot:function(R,S){var T=this.getRoot();

if(!T){return;
}T.setVisibility(R?s:r);
T.recursiveAddToWidgetQueue();
},_applyRootOpenClose:function(W,X){var Y=this.getRoot();

if(!Y){return;
}Y.recursiveAddToWidgetQueue();
},_getContentPaddingTarget:function(){return this.__pP;
},getNextSiblingOf:function(a,b,c){if((b!==false||a.isOpen())&&!(c==true)&&a.hasChildren()){return a.getChildren()[0];
}
...
if(d>-1&&d<f.length-1){return f[d+1];
}a=parent;
}return null;
},getPreviousSiblingOf:function(K,L,M){var parent=K.getParent();
...
while((L!==false||O.isOpen())&&!(M==true)&&O.hasChildren()){var Q=O.getChildren();
O=Q[Q.length-1];
}return O;
}else{return parent;
}},getItems:function(g,h){if(this.getRoot()!=null){return this.getRoot().getItems(g,h,this.getHideRoot());
}else{return [];
...
},_applyOpenMode:function(k,l){if(l==u){this.removeListener(u,this._onOpen,this);
}else if(l==v){this.removeListener(v,this._onOpen,this);
}
if(k==u){this.addListener(u,this._onOpen,this);
}else if(k==v){this.addListener(v,this._onOpen,this);
}},_onOpen:function(e){var be=this.getTreeItem(e.getTarget());

if(!be||!be.isOpenable()){return;
}be.setOpen(!be.isOpen());
e.stopPropagation();
},_onChangeSelection:function(e){var V=e.getData();
for(var i=0;i<V.length;i++){var U=V[i];
while(U.getParent()!=null){U=U.getParent();
U.setOpen(true);
}}},_onKeyPress:function(e){var ba=this._getLeadItem();

if(ba!==null){switch(e.getKeyIdentifier()){case n:if(ba.isOpenable()&&ba.isOpen()){ba.setOpen(false);
}break;
case w:if(ba.isOpenable()&&!ba.isOpen()){ba.setOpen(true);
}break;
case J:case o:if(ba.isOpenable()){ba.toggleOpen();
}break;
...
(function(){var G="open",F="auto",E="middle",D="icon",C="label",B="changeOpen",A="excluded",z="visible",y="String",x="opened",ba="always",Y="qx.ui.tree.AbstractTreeItem",X="addItem",W="Boolean",V="__nq",U="Integer",T="_applyIndent",S="changeOpenSymbolMode",R="_applyOpenSymbolMode",Q="resize",N="__nt",O="",L="removeItem",M="__np",J="abstract",K="never",H="_applyIcon",I="_applyOpen",P="_applyLabel";
qx.Class.define(Y,{extend:qx.ui.core.Widget,type:J,include:[qx.ui.form.MModelProperty],implement:[qx.ui.form.IModel],construct:function(){arguments.callee.base.call(this);
this.__np=[];
this._setLayout(new qx.ui.layout.HBox());
this._addWidgets();
this.initOpen();
},properties:{open:{check:W,init:false,event:B,apply:I},openSymbolMode:{check:[ba,K,F],init:F,event:S,apply:R},indent:{check:U,init:19,apply:T,themeable:true},parent:{check:Y,nullable:true},icon:{check:y,apply:H,nullable:true,themeable:true},label:{check:y,apply:P,init:O}},members:{__np:null,__nq:null,__nr:null,__ns:null,__nt:null,_addWidgets:function(){throw new Error("Abstract method call.");
},_createChildControlImpl:function(v){var w;

switch(v){case C:w=new qx.ui.basic.Label().set({alignY:E,value:this.getLabel()});
break;
case D:w=new qx.ui.basic.Image().set({alignY:E,source:this.getIcon()});
break;
case G:w=new qx.ui.tree.FolderOpenButton().set({alignY:E});
w.addListener(B,this._onChangeOpen,this);
w.addListener(Q,this._updateIndent,this);
break;
}return w||arguments.callee.base.call(this,v);
},getTree:function(){var q=this;

while(q.getParent()){q=q.getParent();
}var p=q.getLayoutParent()?q.getLayoutParent().getLayoutParent():0;

if(p&&p instanceof qx.ui.core.scroll.ScrollPane){return p.getLayoutParent();
}return null;
...
},addOpenButton:function(){this._add(this.getChildControl(G));
},_onChangeOpen:function(e){if(this.isOpenable()){this.setOpen(e.getData());
}},addIcon:function(){var m=this.getChildControl(D);

if(this.__ns){this._remove(m);
}this._add(m);
this.__ns=true;
},addLabel:function(bb){var bc=this.getChildControl(C);

if(this.__nr){this._remove(bc);
}
if(bb){this.setLabel(bb);
}else{bc.setValue(this.getLabel());
}this._add(bc);
this.__nr=true;
},addState:function(bS){arguments.callee.base.call(this,bS);
var bU=this._getChildren();

for(var i=0,l=bU.length;i<l;i++){var bT=bU[i];

if(bT.addState){bU[i].addState(bS);
...
}}},_applyIcon:function(bs,bt){var bu=this.getChildControl(D,true);

if(bu){bu.setSource(bs);
}},_applyLabel:function(bL,bM){var bN=this.getChildControl(C,true);

if(bN){bN.setValue(bL);
}},_applyOpen:function(n,o){if(this.hasChildren()){this.getChildrenContainer().setVisibility(n?z:A);
}var open=this.getChildControl(G,true);

if(open){open.setOpen(n);
}n?this.addState(x):this.removeState(x);
},isOpenable:function(){var bB=this.getOpenSymbolMode();
return (bB===ba||bB===F&&this.hasChildren());
...
},_applyOpenSymbolMode:function(j,k){this._updateIndent();
},_updateIndent:function(){if(!this.getTree()){return;
}var s=0;
var open=this.getChildControl(G,true);

if(open){if(this._shouldShowOpenSymbol()){open.show();
var r=open.getBounds();

if(r){s=r.width;
}else{return;
}}else{open.exclude();
}}
if(this.__nt){this.__nt.setWidth((this.getLevel()+1)*this.getIndent()-s);
}},_applyIndent:function(bq,br){this._updateIndent();
...
},getChildrenContainer:function(){if(!this.__nq){this.__nq=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?z:A});
}return this.__nq;
...
}},add:function(bC){var bD=this.getChildrenContainer();
var bG=this.getTree();

for(var i=0,l=arguments.length;i<l;i++){var bH=arguments[i];
var bF=bH.getParent();

if(bF){bF.remove(bH);
}bH.setParent(this);
var bE=this.hasChildren();
bD.add(bH);

if(bH.hasChildren()){bD.add(bH.getChildrenContainer());
}this.__np.push(bH);

if(!bE){this.__nu();
}
if(bG){bH.recursiveAddToWidgetQueue();
bG.fireNonBubblingEvent(X,qx.event.type.Data,[bH]);
}}
if(bG){qx.ui.core.queue.Widget.add(this);
}},addAt:function(bv,bw){{};

if(bw==this.__np.length){this.add(bv);
return;
}var bA=bv.getParent();
...
bv.setParent(this);
var bz=this.hasChildren();
var bx=this.__np[bw];
by.addBefore(bv,bx);

if(bv.hasChildren()){by.addAfter(bv.getChildrenContainer(),bv);
}qx.lang.Array.insertAt(this.__np,bv,bw);

if(!bz){this.__nu();
}
if(this.getTree()){bv.recursiveAddToWidgetQueue();
qx.ui.core.queue.Widget.add(this);
}},addBefore:function(bd,be){{};
this.addAt(bd,this.__np.indexOf(be));
},addAfter:function(t,u){{};
this.addAt(t,this.__np.indexOf(u)+1);
},addAtBegin:function(bm){this.addAt(bm,0);
},remove:function(c){for(var i=0,l=arguments.length;i<l;i++){var h=arguments[i];

if(this.__np.indexOf(h)==-1){this.warn("Cannot remove treeitem '"+h+"'. It is not a child of this tree item.");
return;
}var d=this.getChildrenContainer();

if(h.hasChildrenContainer()){var g=h.getChildrenContainer();

if(d.getChildren().indexOf(g)>=0){d.remove(g);
}}qx.lang.Array.remove(this.__np,h);
h.setParent(null);
d.remove(h);
}var f=this.getTree();

if(f){f.fireNonBubblingEvent(L,qx.event.type.Data,[h]);
}qx.ui.core.queue.Widget.add(this);
},removeAt:function(bO){var bP=this.__np[bO];
...
(function(){var t="get",s="String",r="changeModel",q="_applyDelegate",p="qx.core.Object",o="_applyLabelPath",n="icon",m="change",l="label",k="_applyTarget",c="changeTarget",h="_applyIconPath",g="_applyLabelOptions",b="_applyChildPath",a="_applyIconOptions",f="qx.data.controller.Tree",d="_applyModel";
qx.Class.define(f,{extend:qx.core.Object,include:qx.data.controller.MSelection,construct:function(bA,bB,bC,bD){arguments.callee.base.call(this);
this.__pb={};
this.__pc=[];
this.__pd={};

if(bC!=null){this.setChildPath(bC);
}
...
}},properties:{model:{check:p,apply:d,event:r,nullable:true},target:{apply:k,event:c,init:null},childPath:{check:s,apply:b,nullable:true},labelPath:{check:s,apply:o,nullable:true},iconPath:{check:s,apply:h,nullable:true},labelOptions:{apply:g,nullable:true},iconOptions:{apply:a,nullable:true},delegate:{apply:q,init:null,nullable:true}},members:{__pd:null,__pb:null,__pc:null,_applyDelegate:function(bE,bF){this._setConfigureItem(bE,bF);
this._setCreateItem(bE,bF);
this._setBindItem(bE,bF);
},_applyIconOptions:function(bG,bH){this.__pl();
},_applyLabelOptions:function(L,M){this.__pl();
},_applyTarget:function(P,Q){if(Q!=undefined){var R=Q.getRoot();
Q.setRoot(null);
R.destroy();
}if(this.getModel()!=null){this.__pg();
}this._addChangeTargetListener(P,Q);
},_applyModel:function(br,bs){this.__pg();
},_applyChildPath:function(cd,ce){this.__pg();
},_applyIconPath:function(bp,bq){this.__pl();
},_applyLabelPath:function(bt,bu){this.__pl();
},__pe:function(bg){var bi=bg.getTarget();
var bh=this.__pd[bi.toHashCode()].treeNode;
var bj=this.__pd[bi.toHashCode()].modelNode;
this.__ph(bh,bj);
this._updateSelection();
},__pf:function(e){var cb=e.getData();
var bW=e.getOldData();
var bX=this.__pd[bW.toHashCode()];
delete this.__pd[bW.toHashCode()];
bX.modelNode.removeListenerById(bX.changeChildernListenerId);
var cc=bX.modelNode;
var ca=qx.Class.getPropertyDefinition(bX.modelNode.constructor,this.getChildPath());
var bU=ca.event;
var bY=cc.addListener(bU,this.__pf,this);
var bV=bX.treeNode;
this.__pd[cb.toHashCode()]={modelNode:cc,treeNode:bV,changeListenerId:bX.changeListenerId,changeChildernListenerId:bY};
this.__ph(bV,cc);
this._updateSelection();
},_createItem:function(){var O=this.getDelegate();
if(O!=null&&O.createItem!=null){var N=O.createItem();
}else{var N=new qx.ui.tree.TreeFolder();
...
if(bS!=null){this.getTarget().resetRoot();
bS.destroy();
}if(this.getModel()!=null){var bT=this._createItem();
bT.setModel(this.getModel());
this.getTarget().setRoot(bT);
this.__pm(this.getModel(),bT);
this.__ph(bT,this.getModel());
}},__ph:function(u,v){if(v[t+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;
}var E=v[t+qx.lang.String.firstUp(this.getChildPath())]();
if(this.__pd[E.toHashCode()]==undefined){var z=E.addListener(m,this.__pe,this);
var D=qx.Class.getPropertyDefinition(v.constructor,this.getChildPath());
var A=D.event;
var B=v.addListener(A,this.__pf,this);
this.__pd[E.toHashCode()]={modelNode:v,treeNode:u,changeListenerId:z,changeChildernListenerId:B};
}for(var i=0;i<E.length;i++){if(u.getChildren()[i]==null||E.getItem(i)!=u.getChildren()[i].getModel()){for(var j=i;j<u.getChildren().length;j++){if(u.getChildren()[j].getModel()===E.getItem(i)){var w=j;
break;
}}if(w!=undefined){var x=u.getChildren()[w];
if(this.getTarget().isSelected(x)){var F=true;
}u.removeAt(w);
u.addAt(x,i);
if(F){this.getTarget().addToSelection(x);
}}else{var C=this._createItem();
C.setModel(E.getItem(i));
u.addAt(C,i);
this.__pm(E.getItem(i),C);
this.__ph(C,E.getItem(i));
}}}for(var i=u.getChildren().length-1;i>=E.length;i--){var y=u.getChildren()[i];
this.__pk(y,u);
...
this.__pj(bf);
this.__pn(bf.getModel());
bf.destroy();
this.__pd={};
}},__pj:function(cf){var cg=cf.getChildren();
for(var i=cg.length-1;i>=0;i--){if(cg[i].getChildren().length>0){this.__pj(cg[i]);
}this.__pk(cg[i],cf);
}},__pk:function(G,H){var I=G.getModel();
if(I[t+qx.lang.String.firstUp(this.getChildPath())]!=undefined){delete this.__pd[I[t+qx.lang.String.firstUp(this.getChildPath())]().toHashCode()];
}this.__pn(I);
H.remove(G);
G.destroy();
},bindProperty:function(S,T,U,V,W){var Y=W.bind(S,V,T,U);
if(this.__pb[T]==null){this.__pb[T]={};
}var X=this.__pb[T];

if(X[W.toHashCode()]){X[W.toHashCode()].reverseId=Y;
}else{X[W.toHashCode()]={id:Y,reverseId:null,treeNode:V};
}if(!qx.lang.Array.contains(this.__pc,T)){this.__pc.push(T);
}},bindPropertyReverse:function(bL,bM,bN,bO,bP){var bR=bO.bind(bM,bP,bL,bN);
if(this.__pb[bM]==null){this.__pb[bM]={};
}var bQ=this.__pb[bM];

if(bQ[bP.toHashCode()]){bQ[bP.toHashCode()].reverseId=bR;
}else{bQ[bP.toHashCode()]={id:null,reverseId:bR,treeNode:bO};
}if(!qx.lang.Array.contains(this.__pc,bM)){this.__pc.push(bM);
}},bindDefaultProperties:function(bd,be){this.bindProperty(this.getLabelPath(),l,this.getLabelOptions(),bd,be);
if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),n,this.getIconOptions(),bd,be);
}},__pl:function(){var bw;

for(var by in this.__pb){bw=by;
break;
}for(var bv in this.__pb[bw]){var bz=this.__pb[bw][bv].treeNode;
var bx=qx.core.ObjectRegistry.fromHashCode(bv);
this.__pn(bx);
this.__pm(bx,bz);
}},__pm:function(bI,bJ){var bK=this.getDelegate();
if(bK!=null&&bK.bindItem!=null){bK.bindItem(this,bJ,bI);
}else{this.bindDefaultProperties(bJ,bI);
}},__pn:function(bm){for(var i=0;i<this.__pc.length;i++){var bo=this.__pc[i];
var bn=this.__pb[bo][bm.toHashCode()];

if(bn!=null){if(bn.id){bm.removeBinding(bn.id);
}
if(bn.reverseId){bn.treeNode.removeBinding(bn.reverseId);
}delete this.__pb[bo][bm.toHashCode()];
...
}}},_setCreateItem:function(bk,bl){if(this.getTarget()==null||this.getModel()==null){return;
}if(bk==null||bk.createItem==null){return;
}if(bl&&bl.createItem&&bk&&bk.createItem&&bl.createItem==bk.createTtem){return;
}this._startSelectionModification();
this.__pi();
this.__pg();
this._endSelectionModification();
this._updateSelection();
},_setBindItem:function(J,K){if(J!=null&&J.bindItem!=null){if(K!=null&&K.bindItem!=null&&J.bindItem==K.bindItem){return;
}this.__pg();
