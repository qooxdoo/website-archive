...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.bom.Flash-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"f30718aecaed"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var s="20px",r="demobrowser/demo/flash/fo_tester.swf",q="this is passed in via FlashVars",p="div",o="position",n="demobrowser.demo.bom.Flash",m="left",l="opaque",k="movie",j="top",g="bottom",i="right",h="noscale",f="#FF6600",e="absolute";
qx.Class.define(n,{extend:qx.application.Native,members:{main:function(){arguments.callee.base.call(this);
var d=new qx.bom.Element.create(p);
qx.bom.element.Style.set(d,o,e);
qx.bom.element.Style.set(d,j,s);
qx.bom.element.Style.set(d,g,s);
qx.bom.element.Style.set(d,i,s);
qx.bom.element.Style.set(d,m,s);
document.body.appendChild(d);
var a=qx.util.ResourceManager.getInstance().toUri(r);
var c={flashVarText:q};
var b={bgcolor:f,scale:h,wmode:l};
qx.bom.Flash.create(d,{movie:a,id:k},c,b);
}}});
})();
...
(function(){var C="qx.client",B="object",A="100%",z='</object>',y="movie",x="=",w="function",v="application/x-shockwave-flash",u='<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">',t="param",T='" />',S="qx.bom.Flash",R='<param name="',Q="load",P="type",O="onbeforeunload",N="name",M="data",L="",K="mshtml",I="value",J="&",G="beforeunload",H='<object id="',E='" value="',F='" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">',D="undefined";
qx.Class.define(S,{statics:{_flashObjects:{},create:function(h,j,k,l,m){if(!m){m=window;
}{};

if(!j.width){j.width=A;
}
if(!j.height){j.height=A;
}l=l?qx.lang.Object.clone(l):{};

if(!l[y]){l[y]=j.movie;
}j[M]=j.movie;
delete j.movie;
if(k){for(var name in k){if(typeof l.flashvars!=D){l.flashvars+=J+name+x+k[name];
}else{l.flashvars=name+x+k[name];
}}}var n=this.__yJ(h,j,l,m);
this._flashObjects[j.id]=n;
return n;
},destroy:qx.core.Variant.select(C,{"mshtml":function(W,X){W=this.__yE(W);

if(W.readyState==4){this.__yF(W);
}else{if(!X){X=window;
}qx.bom.Event.addNativeListener(X,Q,function(){qx.bom.Flash.__yF(W);
});
}},"default":function(Y,ba){Y=this.__yE(Y);
Y.parentNode.removeChild(Y);
delete this._flashObjects[Y.id];
}}),__yE:function(a){if(!a){throw new Error("DOM element is null or undefined!");
}
if(a.tagName.toLowerCase()!==B){a=a.firstChild;
}
if(!a||a.tagName.toLowerCase()!==B){throw new Error("DOM element has or is not a flash object!");
}return a;
},__yF:qx.core.Variant.select(C,{"mshtml":qx.event.GlobalError.observeMethod(function(V){for(var i in V){if(typeof V[i]==w){V[i]=null;
}}V.parentNode.removeChild(V);
delete this._flashObjects[V.id];
}),"default":null}),__yG:qx.event.GlobalError.observeMethod(function(){for(var U in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[U]);
}window.__yH=function(){};
window.__yI=function(){};
window.detachEvent(O,qx.bom.Flash.__yG);
}),__yJ:qx.core.Variant.select(C,{"mshtml":function(o,p,q,r){q.movie=p.data;
delete p.data;
delete p.classid;
var s=L;

for(name in q){s+=R+name+E+q[name]+T;
}if(p.id){o.innerHTML=H+p.id+F+s+z;
delete p.id;
}else{o.innerHTML=u+s+z;
}for(var name in p){o.firstChild.setAttribute(name,p[name]);
}return o.firstChild;
},"default":function(b,c,d,e){delete c.classid;
delete d.movie;
var g=qx.bom.Element.create(B,c,e);
g.setAttribute(P,v);
var f;

for(var name in d){f=qx.bom.Element.create(t,{},e);
f.setAttribute(N,name);
f.setAttribute(I,d[name]);
g.appendChild(f);
}b.appendChild(g);
return g;
}})},defer:function(bb){if(qx.core.Variant.isSet(C,K)){qx.bom.Event.addNativeListener(window,G,bb.__yG);
}}});
})();
