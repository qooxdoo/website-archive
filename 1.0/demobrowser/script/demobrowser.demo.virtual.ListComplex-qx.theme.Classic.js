...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.virtual.ListComplex-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"20d617d324b0"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var i=" / ",h="row",g="foobar!",f="column",e="demobrowser.demo.virtual.DemoLayer",d="mouseover",c="changeValue",b="qx.ui.basic.Atom";
qx.Class.define(e,{extend:qx.ui.virtual.layer.WidgetCell,construct:function(){arguments.callee.base.call(this,this);
this._pool={atom:[],checkbox:[]};
this.__JS=[];
},members:{_pool:null,__JS:null,getCellData:function(n,o){if(!this.__JS[n]){this.__JS[n]=[];
}
if(!this.__JS[n][o]){this.__JS[n][o]={label:this.__JT(),icon:this.__JU()};
}return this.__JS[n][o];
},getCellWidget:function(k,l){var m;

if(l%2==0){m=this._pool.atom.pop();

if(!m){m=new qx.ui.basic.Atom();
m.addListener(d,function(){var t=this.__JU();
m.setIcon(t);
this.__JS[k][l].icon=t;
},this);
}m.set(this.getCellData(k,l));
}else{m=this._pool.checkbox.pop();

if(!m){m=new qx.ui.form.CheckBox();
m.addListener(c,function(){this.setLabel(this.getLabel()==g?m.getUserData(h)+i+m.getUserData(f):g);
},m);
}m.set({value:k%2==0,label:k+i+l});
}m.setUserData(h,k);
m.setUserData(f,l);
return m;
},poolCellWidget:function(a){if(a.classname==b){this._pool.atom.push(a);
}else{this._pool.checkbox.push(a);
}},__JT:function(){var name="";

for(var j=0;j<10;j++){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);
}return name;
},__JU:function(){var p="icon/";
var r="places/";
var s=["folder.png","user-trash.png","network-server.png","network-workgroup.png","user-desktop.png"];
var q=Math.floor(Math.random()*4);
return (p+16+"/"+r+s[q]);
}},destruct:function(){this._pool=this.__JS=null;
}});
})();
...
(function(){var o="scroll",n="scrollbar-x",m="scrollbar-y",l="_applyInterval",k="__Jg",j="appear",i="interval",h="qx.ui.virtual.behavior.Prefetch",g="_applyScroller",f="qx.ui.virtual.core.Scroller",e="Integer";
qx.Class.define(h,{extend:qx.core.Object,construct:function(v,w,x,y,z,A,B,C,D){arguments.callee.base.call(this);
this.setPrefetchX(w,x,y,z);
this.setPrefetchY(A,B,C,D);
this.__Jg=new qx.event.Timer(this.getInterval());
this.__Jg.addListener(i,this._onInterval,this);

if(v){this.setScroller(v);
}},properties:{scroller:{check:f,nullable:true,init:null,apply:g},interval:{check:e,init:200,apply:l}},members:{__Jh:null,__Ji:null,__Jg:null,__Jj:null,__Jk:null,setPrefetchX:function(a,b,c,d){this.__Jh=[a,b,c,d];
},setPrefetchY:function(r,s,t,u){this.__Ji=[r,s,t,u];
},_onInterval:function(){var G=this.__Jh;

if(G[1]&&G[3]){this.getScroller().getPane().prefetchX(G[0],G[1],G[2],G[3]);
qx.ui.core.queue.Manager.flush();
}var H=this.__Ji;

if(H[1]&&H[3]){this.getScroller().getPane().prefetchY(H[0],H[1],H[2],H[3]);
qx.ui.core.queue.Manager.flush();
}},_applyScroller:function(E,F){if(F){if(this.__Jj){F.getChildControl(n).removeListenerById(this.__Jj);
}
if(this.__Jk){F.getChildControl(m).removeListenerById(this.__Jk);
}}
if(E){if(!E.getContainerElement().getDomElement()){this.__Jg.stop();
E.addListenerOnce(j,this.__Jg.start,this.__Jg);
}else{this.__Jg.restart();
}this.__Jj=E.getChildControl(n).addListener(o,this.__Jg.restart,this.__Jg);
this.__Jk=E.getChildControl(m).addListener(o,this.__Jg.restart,this.__Jg);
}else{this.__Jg.stop();
}},_applyInterval:function(p,q){this.__Jg.setInterval(p);
}},destruct:function(){this.setScroller(null);
this.__Jh=this.__Ji=null;
