...
s.setRowFlex(0,1);
s.setSpacing(10);
var t=new qx.ui.container.Composite(s).set({backgroundColor:g,decorator:q,padding:10,allowGrowX:false,allowGrowY:false,allowStretchX:false,allowStretchY:false});
u.add(t);
this.getRoot().add(u,{edge:0});
var y=this.getPane();
t.add(y,{row:0,column:0,colSpan:3});
var w=new qx.ui.basic.Label(h).set({backgroundColor:n,decorator:q,padding:[4,8]});
t.add(w,{row:1,column:1});
var v=false;
w.addListener(o,function(){var r=y.getChildren()[0];
r.setValue(v?p:m);
v=!v;
});
var x=new qx.ui.basic.Label(f).set({backgroundColor:n,decorator:q,padding:[4,8]});
t.add(x,{row:1,column:2});
var z=false;
x.addListener(o,function(e){if(z){y.setWidth(600);
}else{y.setWidth(300);
}z=!z;
});
