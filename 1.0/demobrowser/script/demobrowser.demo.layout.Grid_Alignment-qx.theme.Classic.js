...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.layout.Grid_Alignment-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"b68efd13ca78"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var n="main",m="yellow",l="bottom",k="top",j="left",h="right",g="center",f="middle",e="orange",d="gray",b="green",c="demobrowser.demo.layout.Grid_Alignment";
...
},getNewWidget:function(){var a=new qx.ui.core.Widget().set({decorator:n,backgroundColor:b,allowGrowX:false,allowGrowY:false,height:Math.round((Math.random()*30)+30),width:Math.round((Math.random()*30)+30)});
return a;
},getColumnAlignGrid:function(){var u=new qx.ui.container.Composite().set({decorator:n,backgroundColor:m,allowGrowX:false,allowGrowY:false});
var v=new qx.ui.layout.Grid();
v.setSpacing(3);
v.setColumnAlign(0,j,k);
v.setColumnAlign(1,g,f);
v.setColumnAlign(2,h,l);
u.setLayout(v);

for(var x=0;x<3;x++){for(var y=0;y<6;y++){u.add(this.getNewWidget(),{row:y,column:x});
}}return u;
},getRowAlignGrid:function(){var s=new qx.ui.container.Composite().set({decorator:n,backgroundColor:m,allowGrowX:false,allowGrowY:false});
...
for(var x=0;x<6;x++){for(var y=0;y<3;y++){s.add(this.getNewWidget(),{row:y,column:x});
}}return s;
},getCellAlignGrid:function(){var w=new qx.ui.container.Composite().set({decorator:n,backgroundColor:m,allowGrowX:false,allowGrowY:false});
var A=new qx.ui.layout.Grid();
A.setSpacing(3);
w.setLayout(A);

for(var x=0;x<5;x++){A.setColumnAlign(x,g,f);

for(var y=0;y<5;y++){w.add(this.getNewWidget(),{row:y,column:x});
}}var z=A.getCellWidget(0,0);
z.set({alignX:j,alignY:k});
z.set({backgroundColor:e,width:20,height:20});
var z=A.getCellWidget(4,4);
z.set({alignX:h,alignY:l});
z.set({backgroundColor:e,width:20,height:20});
return w;
},getRowColumnAlignGrid:function(){var o=new qx.ui.container.Composite().set({decorator:n,backgroundColor:m,allowGrowX:false,allowGrowY:false});
var q=new qx.ui.layout.Grid();
q.setSpacing(3);
o.setLayout(q);

for(var x=0;x<5;x++){for(var y=0;y<5;y++){o.add(this.getNewWidget(),{row:y,column:x});
}}q.setColumnAlign(2,j,k);
q.setRowAlign(2,h,l);

for(var i=0;i<5;i++){q.getCellWidget(2,i).setBackgroundColor(d);
q.getCellWidget(i,2).setBackgroundColor(d);
}var p=q.getCellWidget(2,2);
p.set({backgroundColor:e,width:20,height:20});
return o;
}}});
})();
