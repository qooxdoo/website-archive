...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.virtual.Selection","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.virtual.Selection-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"c3094e4182d4"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var H="manager",G="multi",F=" / ",E="bold",D="value",C="white",B="selected",A="#EEE",z="horizontal",y="table-scroller-header",bd="one",bc="single",bb="changeValue",ba="table-row-background-selected",Y="additive",X="default",W="Additive Selection",V="Cell Line Selection",U="addaptive",T="Options",O="Row Selection",P="changeSelection",M="Column Selection",N="Enable drag selection",K="demobrowser.demo.virtual.Selection",L="Selection Mode",I="Single Selection",J="Cell Rectangle Selection",Q="Enable quick selection",R="One Selection",S="Multi Selection";
qx.Class.define(K,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var bi=qx.theme.manager.Font.getInstance().resolve(X).getStyles();
this._fontCss=qx.bom.element.Style.compile(bi);
var br=new qx.ui.layout.Grid(5,0);
br.setRowFlex(2,1);
br.setColumnFlex(0,1);
br.setColumnFlex(1,1);
br.setColumnFlex(2,1);
br.setColumnFlex(3,1);
var bf=[];
var bp=new qx.ui.container.Composite(br);
this.getRoot().add(bp,{edge:5});
bp.add(new qx.ui.basic.Label(O).set({font:E,decorator:y,padding:3,allowGrowX:true}),{row:1,column:0});
var bh=this.createRowSelectionScroller();
bf.push(bh.getUserData(H));
bp.add(bh,{row:2,column:0});
bp.add(new qx.ui.basic.Label(M).set({font:E,decorator:y,padding:3,allowGrowX:true}),{row:1,column:1});
var bh=this.createColumnSelectionScroller();
bf.push(bh.getUserData(H));
bp.add(bh,{row:2,column:1});
bp.add(new qx.ui.basic.Label(J).set({font:E,decorator:y,padding:3,allowGrowX:true}),{row:1,column:2});
var bh=this.createCellRectangleSelectionScroller();
bf.push(bh.getUserData(H));
bp.add(bh,{row:2,column:2});
bp.add(new qx.ui.basic.Label(V).set({font:E,decorator:y,padding:3,allowGrowX:true}),{row:1,column:3});
var bh=this.createCellLineSelectionScroller();
bf.push(bh.getUserData(H));
bp.add(bh,{row:2,column:3});
var bn=new qx.ui.layout.Grid(20,4);
var bk=new qx.ui.container.Composite(bn).set({padding:15,backgroundColor:C});
bp.add(bk,{row:0,column:0,colSpan:4});
bk.add(new qx.ui.basic.Label(L).set({font:E,padding:3}),{row:0,column:0});
var bj=new qx.ui.form.RadioButton(I);
bj.setUserData(D,bc);
bk.add(bj,{row:1,column:0});
var bm=new qx.ui.form.RadioButton(S);
bm.setUserData(D,G);
bm.setValue(true);
bk.add(bm,{row:2,column:0});
var bg=new qx.ui.form.RadioButton(W);
bg.setUserData(D,Y);
bk.add(bg,{row:3,column:0});
var be=new qx.ui.form.RadioButton(R);
be.setUserData(D,bd);
bk.add(be,{row:4,column:0});
var bl=new qx.ui.form.RadioGroup(bj,bm,bg,be);
bl.addListener(P,function(e){var bH=e.getData()[0].getUserData(D);
bf.forEach(function(s){s.setMode(bH);
});

if(bH==bc||bH==bd){bq.setEnabled(false);
bo.setEnabled(true);
}else if(bH==G||bH==U){bq.setEnabled(true);
bo.setEnabled(false);
}});
bk.add(new qx.ui.basic.Label(T).set({font:E,padding:3}),{row:0,column:1});
var bq=new qx.ui.form.CheckBox(N);
bk.add(bq,{row:1,column:1});
bq.addListener(bb,function(e){if(e.getData()){var bG=bf[0].getMode();

if(bG==bc||bG==bd){this.debug("Drag selection is only available for the modes multi or additive");
}}bf.forEach(function(bI){bI.setDrag(e.getData());
});
});
var bo=new qx.ui.form.CheckBox(Q).set({enabled:false});
bk.add(bo,{row:2,column:1});
bo.addListener(bb,function(e){if(e.getData()){var r=bf[0].getMode();

if(r==G||r==Y){this.debug("Quick selection is only available for the modes multi or additive");
}}bf.forEach(function(bS){bS.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bz=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var bx=new qx.ui.virtual.layer.Row(C,A);
bz.getPane().addLayer(bx);
bz.getPane().addLayer(new qx.ui.virtual.layer.GridLines(z));
var by=new qx.ui.virtual.cell.Cell();
var bw=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bA,bB){var bC={};

if(bv.isItemSelected(bA)){bC.selected=true;
}return by.getCellProperties(bA+F+bB,bC);
}});
bz.getPane().addLayer(bw);
var bv=new qx.ui.virtual.selection.Row(bz.getPane(),{styleSelectable:function(bJ,bK,bL){if(bK!==B){return;
}
if(bL){bx.setDecorator(B);
}else{bx.setDecorator(null);
}bw.updateLayerData();
}});
bv.attachMouseEvents(bz.getPane());
bv.attachKeyEvents(bz);
bv.set({mode:G});
bv.addItem(0);
bz.setUserData(H,bv);
return bz;
},createColumnSelectionScroller:function(){var j=new qx.ui.virtual.core.Scroller(1000,100,20,100);
j.getPane().addLayer(new qx.ui.virtual.layer.Row(C,A));
var i=new qx.ui.virtual.layer.Column();
j.getPane().addLayer(i);
j.getPane().addLayer(new qx.ui.virtual.layer.GridLines(z));
var k=new qx.ui.virtual.cell.Cell();
var h=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bP,bQ){var bR={};

if(g.isItemSelected(bQ)){bR.selected=true;
}return k.getCellProperties(bP+F+bQ,bR);
}});
j.getPane().addLayer(h);
var g=new qx.ui.virtual.selection.Column(j.getPane(),{styleSelectable:function(bM,bN,bO){if(bN!==B){return;
}
if(bO){i.setDecorator(B);
}else{i.setDecorator(null);
}h.updateLayerData();
}});
g.attachMouseEvents(j.getPane());
g.attachKeyEvents(j);
g.set({mode:G});
g.addItem(0);
j.setUserData(H,g);
return j;
},createCellRectangleSelectionScroller:function(){var w=new qx.ui.virtual.core.Scroller(1000,100,20,100);
w.getPane().addLayer(new qx.ui.virtual.layer.Row(C,A));
var v=new qx.ui.virtual.cell.Cell();
var x=new qx.ui.virtual.cell.Cell().set({backgroundColor:ba});
var u=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bD,bE){var bF={};

if(t.isItemSelected({row:bD,column:bE})){bF.selected=true;
}
if(bF.selected){return x.getCellProperties(bD+F+bE,bF);
}else{v.resetBackgroundColor();
return v.getCellProperties(bD+F+bE,bF);
}}});
w.getPane().addLayer(u);
w.getPane().addLayer(new qx.ui.virtual.layer.GridLines(z));
var t=new qx.ui.virtual.selection.CellRectangle(w.getPane(),{styleSelectable:function(o,p,q){u.updateLayerData();
}});
t.attachMouseEvents(w.getPane());
t.attachKeyEvents(w);
t.set({mode:G});
t.addItem({row:0,column:0});
w.setUserData(H,t);
return w;
},createCellLineSelectionScroller:function(){var d=new qx.ui.virtual.core.Scroller(1000,100,20,100);
d.getPane().addLayer(new qx.ui.virtual.layer.Row(C,A));
var c=new qx.ui.virtual.cell.Cell();
var f=new qx.ui.virtual.cell.Cell().set({backgroundColor:ba});
var b=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bs,bt){var bu={};

if(a.isItemSelected({row:bs,column:bt})){bu.selected=true;
}
if(bu.selected){return f.getCellProperties(bs+F+bt,bu);
}else{c.resetBackgroundColor();
return c.getCellProperties(bs+F+bt,bu);
}}});
d.getPane().addLayer(b);
d.getPane().addLayer(new qx.ui.virtual.layer.GridLines(z));
var a=new qx.ui.virtual.selection.CellLines(d.getPane(),{styleSelectable:function(l,m,n){b.updateLayerData();
}});
a.attachMouseEvents(d.getPane());
a.attachKeyEvents(d);
a.set({mode:G});
a.addItem({row:0,column:0});
d.setUserData(H,a);
return d;
}}});
})();
...
},_getSelectableFromMouseEvent:function(event){var h=this._pane.getCellAtPosition(event.getDocumentLeft(),event.getDocumentTop());

if(!h){return null;
}return this._isSelectable(h.column)?h.column:null;
},_getRelatedSelectable:function(j,k){if(k==a){var m=j-1;
var l=0;
var n=-1;
}else if(k==c){var m=j+1;
...
},_scrollItemIntoView:function(p){this._pane.scrollColumnIntoView(p);
},_getSelectableLocationX:function(d){var e=this._pane.getColumnConfig();
var g=e.getItemPosition(d);
var f=g+e.getItemSize(d)-1;
return {left:g,right:f};
},_getSelectableLocationY:function(o){return {top:0,bottom:this._pane.getRowConfig().getTotalSize()-1};
}}});
})();
(function(){var n="under",m="above",l="left",k="right",j="x",i="qx.ui.virtual.selection.CellRectangle";
qx.Class.define(i,{extend:qx.ui.virtual.selection.Abstract,members:{_getItemCount:function(){return this._pane.getRowConfig().getItemCount()*this._pane.getColumnConfig().getItemCount();
...
var W=this._pane.getRowConfig().getItemCount();
var S=this._pane.getColumnConfig().getItemCount();

for(var X=0;X<W;X++){for(var U=0;U<S;U++){var V={row:X,column:U};

if(this._isSelectable(V)){T.push(V);
}}}return T;
},_getSelectableRange:function(I,J){var M=[];
var N=Math.min(I.row,J.row);
var O=Math.max(I.row,J.row);
var L=Math.min(I.column,J.column);
var Q=Math.max(I.column,J.column);

for(var R=N;R<=O;R++){for(var P=L;P<=Q;P++){var K={row:R,column:P};

if(this._isSelectable(K)){M.push(K);
}}}return M;
},_getFirstSelectable:function(){var v=this._pane.getRowConfig().getItemCount();
var u=this._pane.getColumnConfig().getItemCount();

for(var y=0;y<v;y++){for(var x=0;x<u;x++){var w={row:y,column:x};

if(this._isSelectable(w)){return w;
}}}return null;
},_getLastSelectable:function(){var p=this._pane.getRowConfig().getItemCount();
var o=this._pane.getColumnConfig().getItemCount();

for(var r=o-1;r>=0;r--){for(var s=p-1;s>=0;s--){var q={row:s,column:r};

if(this._isSelectable(q)){return q;
}}}return null;
},_getRelatedSelectable:function(A,B){var C={row:A.row,column:A.column};

switch(B){case m:for(var G=A.row-1;G>=0;G--){C.row=G;

if(this._isSelectable(C)){return C;
}}break;
case n:var D=this._pane.getRowConfig().getItemCount();

for(var G=A.row+1;G<D;G++){C.row=G;

if(this._isSelectable(C)){return C;
}}break;
case l:for(var F=A.column-1;F>=0;F--){C.column=F;

if(this._isSelectable(C)){return C;
}}break;
case k:var E=this._pane.getColumnConfig().getItemCount();
...
},_getPage:function(Y,ba){if(ba){return this._getFirstSelectable();
}else{return this._getLastSelectable();
}},_selectableToHashCode:function(z){return z.column+j+z.row;
},_scrollItemIntoView:function(H){this._pane.scrollCellIntoView(H.column,H.row);
},_getSelectableLocationX:function(e){var f=this._pane.getColumnConfig();
var h=f.getItemPosition(e.column);
var g=h+f.getItemSize(e.column)-1;
return {left:h,right:g};
