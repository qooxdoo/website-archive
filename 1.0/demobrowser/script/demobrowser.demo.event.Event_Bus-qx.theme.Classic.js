...
(function(){var q="function",p="qx.event.message.Bus",o="*",n="undefined",m="string",l="singleton";
qx.Class.define(p,{type:l,extend:qx.core.Object,statics:{getSubscriptions:function(){return this.getInstance().getSubscriptions();
},subscribe:function(C,D,E){return this.getInstance().subscribe(C,D,E);
},checkSubscription:function(e,f,g){return this.getInstance().checkSubscription(e,f,g);
},unsubscribe:function(h,j,k){return this.getInstance().unsubscribe(h,j,k);
},dispatch:function(L){return this.getInstance().dispatch.apply(this.getInstance(),arguments);
}},construct:function(){this.__rJ={};
},members:{__rJ:null,getSubscriptions:function(){return this.__rJ;
},subscribe:function(M,N,O){if(!M||typeof N!=q){this.error("Invalid parameters! "+[M,N,O]);
return false;
...
return true;
}},checkSubscription:function(r,s,t){var u=this.getSubscriptions();

if(!u[r]||u[r].length===0){return false;
}
if(s){for(var i=0;i<u[r].length;i++){if(u[r][i].subscriber===s&&u[r][i].context===(t||null)){return true;
}}return false;
}return true;
},unsubscribe:function(F,G,H){var J=this.getSubscriptions();
var I=J[F];
...
if(K.subscriber===G&&K.context===H){I.splice(i,1);

if(I.length===0){J[F]=null;
delete J[F];
}return true;
}}while(i);
}return false;
},dispatch:function(v){if(typeof v==m){var A=typeof arguments[1]!=n?arguments[1]:true;
v=new qx.event.message.Message(v,A);
}var w=this.getSubscriptions();
var x=v.getName();
var y=false;

for(var B in w){var z=B.indexOf(o);

if(z>-1){if(z===0||B.substr(0,z)===x.substr(0,z)){this.__rK(w[B],v);
y=true;
}}else{if(B===x){this.__rK(w[x],v);
y=true;
}}}return y;
},__rK:function(a,b){for(var i=0;i<a.length;i++){var c=a[i].subscriber;
var d=a[i].context;
if(d&&d.isDisposed){if(d.isDisposed()){a.splice(i,1);
i--;
}else{c.call(d,b);
}}else{c.call(d,b);
}}}}});
