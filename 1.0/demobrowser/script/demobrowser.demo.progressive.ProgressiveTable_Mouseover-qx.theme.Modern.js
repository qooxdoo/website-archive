...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.progressive.ProgressiveTable_Mouseover-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"78b27290c818"},
  boot : "boot",
  bootIsInline : true,
...
var L=0;
var H=function(s){var v=[];
var u=new Date().getTime();
var w=400*24*60*60*1000;

for(var x=0;x<s;x++){var t=new Date(u+Math.random()*w-w/2);
v.push({renderer:r,location:h,data:[L++,Math.random()*10000,t]});
}return v;
};
var K=new qx.ui.progressive.renderer.table.Widths(3);
K.setWidth(0,100);
K.setWidth(1,l);
K.setWidth(2,300);
var E=[d,n,f];
var G=new qx.ui.progressive.headfoot.TableHeading(K,E);
var D=new qx.ui.progressive.headfoot.Progress(K,E);
var N=new qx.ui.progressive.structure.Default(G,D);
var C=new qx.ui.progressive.Progressive(N);
var M=new qx.ui.basic.Atom(b);
M.setTextColor(m);
M.setHeight(16);
M.setRich(true);
C.add(M);
...
J.addElements(I);
C.setDataModel(J);
var F=new qx.ui.progressive.renderer.table.Row(K);
C.addRenderer(r,F);
F.addRenderer(2,new demobrowser.demo.progressive.DateCellRenderer());
this.getRoot().add(C,{edge:50});
C.render();
}}});
qx.Class.define(a,{extend:qx.ui.progressive.renderer.table.cell.Default,statics:{onmouseover:function(y){y.__HN=qx.bom.element.Style.get(y,"color");
y.style.color=q;
y.parentNode.style.height=32;
y.__HO=y.innerHTML;
var A=Date.parse(y.attributes.getNamedItem(p).nodeValue);
var z=new Date(A);
y.innerHTML=z.toUTCString()+i+z.toString();
},onmouseout:function(B){B.style.color=B.__HN;
B.parentNode.style.height=16;
B.innerHTML=B.__HO;
}},members:{_getCellExtras:function(O){var P=[];
P.push(arguments.callee.base.call(this));
