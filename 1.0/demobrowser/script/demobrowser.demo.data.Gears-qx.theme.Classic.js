...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.data.Gears","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.data.Gears-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"61c5b157f1d0"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var t="value",s='undefined',r="",q="model",p="application/x-googlegears",o="demobrowser.demo.data.store.Gears",n="<span style='font-size: 20px'>google Gears</span>",m="Age: ",l="http://code.google.com/apis/gears/install.html",k="lastname",L="firstname",K="age",J="demobrowser.demo.data.Gears",I="First Name: ",H='qooxdoo',G='Gears.Factory',F='select * from Persons',E="delete from Persons",D="LastName: ",C=' (Firstname varchar(255), Lastname varchar(255), Age int)',A="Person",B='ie_mobile',y="none",z='create table if not exists Persons',w="This demo requires Gears to be installed. Install now?",x='insert into Persons values (?, ?, ?)',u='beta.database',v="object";
qx.Class.define(J,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var h=new qx.ui.groupbox.GroupBox(A);
this.getRoot().add(h,{left:10,top:50});
h.setLayout(new qx.ui.layout.Grid(0,3));
h.add(new qx.ui.basic.Label(I),{row:0,column:0});
h.add(new qx.ui.basic.Label(D),{row:1,column:0});
h.add(new qx.ui.basic.Label(m),{row:2,column:0});
var a=new qx.ui.form.TextField();
h.add(a,{row:0,column:1});
var c=new qx.ui.form.TextField();
h.add(c,{row:1,column:1});
var d=new qx.ui.form.Spinner(0,0,1000);
h.add(d,{row:2,column:1});
var f=new qx.data.controller.Object();
f.addTarget(a,t,L,true);
f.addTarget(c,t,k,true);
f.addTarget(d,t,K,true);
var g=new demobrowser.demo.data.store.Gears();
g.bind(q,f,q);
...
qx.Class.define(o,{extend:qx.data.store.Json,construct:function(){arguments.callee.base.call(this);
this.initGears();
if(!window.google||!google.gears){if(confirm(w)){location.href=l;
return;
}return;
}this.__FG=this.initDB();
this.__FH();
},members:{__FH:function(M){var N=this.getPerson();
this._marshaler.toClass(N,true);
this.setModel(this._marshaler.toModel(N));
this.getModel().addListener("changeBubble",this.__FI,this);
this.fireDataEvent("loaded",this.getModel());
},__FI:function(R){var S=this.getModel();
this.update(S.getFirstname(),S.getLastname(),S.getAge());
},update:function(O,P,Q){if(!google.gears.factory||!this.__FG){return;
}this.__FG.execute(E);
this.__FG.execute(x,[O,P,Q]);
},getPerson:function(){var i={};
i.firstname=r;
i.lastname=r;
i.age=0;

if(!google.gears.factory||!this.__FG){return i;
}var j=this.__FG.execute(F);

if(j.isValidRow()){i.firstname=j.field(0);
i.lastname=j.field(1);
i.age=parseInt(j.field(2));
}j.close();
return i;
},initDB:function(){if(window.google&&google.gears){try{var V=google.gears.factory.create(u);

if(V){V.open(H);
V.execute(z+C);
}}catch(U){this.error('Could not create database: '+U.message);
}}return V;
},initGears:function(){if(window.google&&google.gears){return;
}var T=null;
if(typeof GearsFactory!=s){T=new GearsFactory();
}else{try{T=new ActiveXObject(G);
if(T.getBuildInfo().indexOf(B)!=-1){T.privateSetGlobalObject(this);
}}catch(e){if((typeof navigator.mimeTypes!=s)&&navigator.mimeTypes[p]){T=document.createElement(v);
T.style.display=y;
T.width=0;
T.height=0;
T.type=p;
...
if(!google.gears){google.gears={factory:T};
}}}});
})();
...
qx.Class.define(f,{extend:qx.ui.core.Widget,include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MRemoteLayoutHandling,qx.ui.core.MContentPadding,qx.ui.form.MForm],implement:[qx.ui.form.IForm],construct:function(a,b){arguments.callee.base.call(this);
this._setLayout(new qx.ui.layout.Canvas);
...
}},_repositionFrame:function(){var r=this.getChildControl(k);
var q=this.getChildControl(j);
var s=r.getBounds().height;
if(this.getLegendPosition()==i){q.setLayoutProperties({"top":Math.round(s/2)});
}else if(this.getLegendPosition()==h){q.setLayoutProperties({"top":s});
...
},setIcon:function(p){this.getChildControl(k).setIcon(p);
...
(function(){var t="changeModel",s="qx.core.Object",r="reset",q="qx.data.controller.Object",p="_applyModel";
qx.Class.define(q,{extend:qx.core.Object,construct:function(A){arguments.callee.base.call(this);
this.__qR={};
this.__qS=[];

if(A!=null){this.setModel(A);
}},properties:{model:{check:s,event:t,apply:p,nullable:true}},members:{__qS:null,__qR:null,_applyModel:function(g,h){for(var i=0;i<this.__qS.length;i++){var o=this.__qS[i][0];
var l=this.__qS[i][1];
var k=this.__qS[i][2];
var m=this.__qS[i][3];
var n=this.__qS[i][4];
var j=this.__qS[i][5];
if(h!=undefined){this.__qU(o,l,k,h);
}if(g!=undefined){this.__qT(o,l,k,m,n,j);
}else{o[r+qx.lang.String.firstUp(l)]();
}}},addTarget:function(u,v,w,x,y,z){this.__qS.push([u,v,w,x,y,z]);
this.__qT(u,v,w,x,y,z);
},__qT:function(E,F,G,H,I,J){if(this.getModel()==null){return;
}var K=this.getModel().bind(G,E,F,I);
var L=null;

if(H){L=E.bind(F,this.getModel(),G,J);
}var M=E.toHashCode();

if(this.__qR[M]==undefined){this.__qR[M]=[];
}this.__qR[M].push([K,L,F,G,I,J]);
},removeTarget:function(B,C,D){this.__qU(B,C,D,this.getModel());
for(var i=0;i<this.__qS.length;i++){if(this.__qS[i][0]==B&&this.__qS[i][1]==C&&this.__qS[i][2]==D){this.__qS.splice(i,1);
}}},__qU:function(a,b,c,d){if(!(a instanceof qx.core.Object)){return ;
}var e=this.__qR[a.toHashCode()];
...
return;
}}}},destruct:function(){this.__qR=this.__qS=null;
}});
})();
