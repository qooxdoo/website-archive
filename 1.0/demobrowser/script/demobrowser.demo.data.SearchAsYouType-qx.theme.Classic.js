...
(function(){var q="searchString",p="Bart",o="<b>Search As You Type List</b><br/>",n="Marge",m="Maggy",l="Skinner",k="Moe",j="Smithers",h="Lisa",g=" ",c="Homer",f="Nelson",e="Flanders",b="changeValue",a="demobrowser.demo.data.SearchAsYouType",d="Type a name and the list will show only the fitting results.";
qx.Class.define(a,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var v=[p,h,c,n,k,j,f,e,m,l];
var y=[];

for(var i=0;i<60;i++){y.push(v[i%10]+g+i);
...
qx.Class.define(c,{extend:qx.core.Object,construct:function(e){arguments.callee.base.call(this);
this.__FJ=e;
this.filter=qx.lang.Function.bind(function(i){return i.search(this.getSearchString())!=-1;
},this);
this.__FK=null;
},properties:{searchString:{check:b,apply:d,init:a}},members:{__FK:null,__FJ:null,_applySearchString:function(f,g){var h=qx.util.TimerManager.getInstance();
if(this.__FK!=null){h.stop(this.__FK);
this.__FK=null;
}this.__FK=h.start(function(){this.__FJ.update();
this.__FK=null;
},0,this,null,200);
},filter:null},destruct:function(){this.__FJ=null;
}});
})();
(function(){var l="interval",k="qx.util.TimerManager",j="singleton";
qx.Class.define(k,{extend:qx.core.Object,type:j,statics:{__zf:[],__zg:{},__zh:0},members:{start:function(q,r,s,t,u){if(!u){u=r||0;
}var v=(new Date()).getTime()+u;
arguments.callee.self.__zg[++arguments.callee.self.__zh]={callback:q,userData:t||null,expireAt:v,recurTime:r,context:s||this};
this.__zi(v,arguments.callee.self.__zh);
return arguments.callee.self.__zh;
},stop:function(g){var h=arguments.callee.self.__zf;
var length=h.length;

for(var i=0;i<length;i++){if(h[i]==g){h.splice(i,1);
break;
}}delete arguments.callee.self.__zg[g];
if(h.length==0){qx.event.Idle.getInstance().removeListener(l,this.__zj);
}},__zi:function(m,n){var p=arguments.callee.self.__zf;
var o=arguments.callee.self.__zg;
var length=p.length;

for(var i=0;i<length;i++){if(o[p[i]].expireAt>m){p.splice(i,0,n);
break;
}}if(p.length==length){p.push(n);
}if(p.length==1){qx.event.Idle.getInstance().addListener(l,this.__zj,this);
}},__zj:function(){var d=(new Date()).getTime();
var b=arguments.callee.self.__zf;
var c=arguments.callee.self.__zg;
...
this.__zi(a.expireAt,f);
}else{delete c[f];
}}if(b.length==0){qx.event.Idle.getInstance().removeListener(l,this.__zj);
}}}});
})();
