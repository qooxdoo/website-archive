...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.data.Form-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"c35da127051e"},
  boot : "boot",
  bootIsInline : true,
...
G.setSpacing(5);
G.setColumnAlign(0,f,a);
J.setLayout(G);
var w=new qx.ui.basic.Label(b);
J.add(w,{row:0,column:0});
var z=new qx.ui.form.TextField();
J.add(z,{row:0,column:1});
var M=new qx.ui.basic.Label(d);
J.add(M,{row:1,column:0});
var H=new qx.ui.form.SelectBox();
var I=new qx.ui.form.ListItem(i);
H.add(I);
var E=new qx.ui.form.ListItem(n);
H.add(E);
var C=new qx.ui.form.ListItem(p);
H.add(C);
J.add(H,{row:1,column:1});
var O=new qx.ui.basic.Label(k);
J.add(O,{row:2,column:0});
var L=new qx.ui.form.CheckBox();
J.add(L,{row:2,column:1});
var D=new qx.ui.form.Button(m);
J.add(D,{row:3,column:0});
var y={name:null,gender:null,ok:0,hidden:e};
var K=qx.data.marshal.Json.createModel(y);
var B=new qx.data.controller.Object(K);
B.addTarget(z,r,g,true);
B.addTarget(H,j,l,true);
var x={converter:function(P){return P===1;
}};
var A={converter:function(v){return v?1:0;
}};
B.addTarget(L,r,h,true,x,A);
z.setRequired(true);
L.setRequired(true);
var N=new qx.ui.form.validation.Manager();
N.add(z,qx.util.Validate.required);
N.add(L);
N.setValidator(function(s){var t=H.getSelection()[0]!==I;
H.setValid(t);
return t;
});
var F=function(u){if(u instanceof qx.ui.form.ListItem){return u.getLabel();
}};
D.addListener(c,function(){if(N.validate()){alert("You are sending: "+qx.util.Serializer.toUriParameter(K,F));
}},this);
}}});
})();
...
(function(){var x="Validation Error",w="",v="'",u=" is not a color! ",t=" is not an url.",s="number",r="Validation Error: ",q=" is not in ",p="qx.util.Validate",o="string",i=" is not a string.",n=" does not fit ",l=" is not in the range from [",h=" is not a number.",g=".",k="].",j=", ",m="' is not an email address.";
qx.Class.define(p,{statics:{number:function(){return qx.util.Validate.checkNumber;
},checkNumber:function(B){if((typeof B!==s&&(!(B instanceof Number)))||(!(isFinite(B)))){throw new qx.core.ValidationError(r,B+h);
}},email:function(){return qx.util.Validate.checkEmail;
},checkEmail:function(y){var z=/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;

if(z.test(y)===false){throw new qx.core.ValidationError(x,v+(y||w)+m);
}},string:function(){return qx.util.Validate.checkString;
},checkString:function(D){if(typeof D!==o&&(!(D instanceof String))){throw new qx.core.ValidationError(x,D+i);
}},url:function(){return qx.util.Validate.checkUrl;
},checkUrl:function(d){var f=/([A-Za-z0-9])+:\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;

if(f.test(d)===false){throw new qx.core.ValidationError(x,d+t);
}},color:function(){return qx.util.Validate.checkColor;
},checkColor:function(E){try{qx.util.ColorUtil.stringToRgb(E);
}catch(e){throw new qx.core.ValidationError(x,E+u+e);
}},range:function(b,c){return function(A){if(A<b||A>c){throw new qx.core.ValidationError(x,A+l+b+j+c+k);
}};
},inArray:function(G){return function(F){if(G.indexOf(F)===-1){throw new qx.core.ValidationError(x,F+q+G+w);
}};
},regExp:function(a){return function(C){if(a.test(C)===false){throw new qx.core.ValidationError(x,C+n+a+g);
}};
}}});
})();
(function(){var u='"',t="{",s="[",r=",",q="",p="get",o="}",n="]",m='":',l="&",d="null",k='\\t',g='\\"',c='\\n',b='\\b',f="=",e="qx.util.Serializer",h='\\r',a='\\\\',j='\\f';
qx.Class.define(e,{statics:{toUriParameter:function(v,w){var y=q;
var z=qx.util.PropertyUtil.getProperties(v.constructor);

for(var name in z){var x=v[p+qx.lang.String.firstUp(name)]();
if(qx.lang.Type.isArray(x)){for(var i=0;i<x.length;i++){y+=this.__ye(name,x[i],w);
}}else{y+=this.__ye(name,x,w);
}}return y.substring(0,y.length-1);
},__ye:function(name,H,I){if(H instanceof qx.core.Object&&I!=null){var J=encodeURIComponent(I(H));
...
},toJson:function(A,B){var E=q;
if(A==null){return d;
}else if(qx.Class.hasInterface(A.constructor,qx.data.IListData)){E+=s;

for(var i=0;i<A.getLength();i++){E+=qx.util.Serializer.toJson(A.getItem(i),B)+r;
}
if(E!=s){E=E.substring(0,E.length-1);
}return E+n;
}else if(qx.lang.Type.isArray(A)){E+=s;

for(var i=0;i<A.length;i++){E+=qx.util.Serializer.toJson(A[i],B)+r;
}
if(E!=s){E=E.substring(0,E.length-1);
}return E+n;
}else if(A instanceof qx.core.Object){if(B!=null){var F=B(A);
if(F!=undefined){return u+F+u;
}}E+=t;
var G=qx.util.PropertyUtil.getProperties(A.constructor);

for(var name in G){if(G[name].group!=undefined){continue;
}var D=A[p+qx.lang.String.firstUp(name)]();
E+=u+name+m+qx.util.Serializer.toJson(D,B)+r;
}
if(E!=t){E=E.substring(0,E.length-1);
}return E+o;
}else if(qx.lang.Type.isObject(A)){E+=t;

for(var C in A){E+=u+C+m+qx.util.Serializer.toJson(A[C],B)+r;
}
if(E!=t){E=E.substring(0,E.length-1);
}return E+o;
}else if(qx.lang.Type.isString(A)){A=A.replace(/([\\])/g,a);
A=A.replace(/(["])/g,g);
A=A.replace(/([\r])/g,h);
A=A.replace(/([\f])/g,j);
A=A.replace(/([\n])/g,c);
...
A=A.replace(/([\b])/g,b);
return u+A+u;
}else if(qx.lang.Type.isDate(A)||qx.lang.Type.isRegExp(A)){return u+A+u;
}return A+q;
}}});
})();
