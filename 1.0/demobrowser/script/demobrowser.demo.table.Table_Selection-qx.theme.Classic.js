...
(function(){var q="execute",p="value",o="",n="A date",m="Boolean",l="demobrowser.demo.table.Table_Selection",k="changeValue",j="icon/22/status/dialog-information.png",i="A number",h="icon/16/apps/office-calendar.png",F="Reset Cell Focus",E="changeSelection",D="middle",C="Remove 5 rows",B="Reset Selection",A="_tableModel",z="Table",y="Add 5 rows",x="icon/22/actions/list-remove.png",w=", ",u="icon/22/actions/list-add.png",v="Hide Cell Focus",s="Selected rows:<br>",t="Show selection",r="ID";
qx.Class.define(l,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return z;
},createTable:function(){var I=this.createRandomRows(50);
var H=this._tableModel=new qx.ui.table.model.Simple();
H.setColumns([r,i,n,m]);
H.setData(I);
H.setColumnEditable(1,true);
H.setColumnEditable(2,true);
H.setColumnSortable(3,false);
var J=new qx.ui.table.Table(H);
J.set({width:600,height:400});
J.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var G=J.getTableColumnModel();
G.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
G.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(h,n));
return J;
},createControls:function(){var O=new qx.ui.toolbar.ToolBar();
var Q,S;
S=new qx.ui.toolbar.Part();
O.add(S);
var N=new qx.ui.form.SelectBox().set({allowGrowY:false,alignY:D});
var V=qx.ui.table.selection.Model;
var P={"Single selection":V.SINGLE_SELECTION,"Single interval":V.SINGLE_INTERVAL_SELECTION,"Multiple intervals":V.MULTIPLE_INTERVAL_SELECTION,"Multiple intervals (toggle)":V.MULTIPLE_INTERVAL_SELECTION_TOGGLE,"No selection":V.NO_SELECTION};

for(var U in P){var R=new qx.ui.form.ListItem(U);
R.setUserData(p,P[U]+o);
N.add(R);
}N.addListener(E,function(e){this._table.getSelectionModel().setSelectionMode(parseInt(e.getData()[0].getUserData(p)));
},this);
S.add(N);
S=new qx.ui.toolbar.Part();
O.add(S);
Q=new qx.ui.toolbar.Button(y,u);
Q.addListener(q,function(L){var M=this.createRandomRows(10);
this._tableModel.addRows(M);
this.info("5 rows added");
},this);
S.add(Q);
Q=new qx.ui.toolbar.Button(C,x);
Q.addListener(q,function(c){var d=this._tableModel.getRowCount();
this._tableModel.removeRows(d-5,5);
this.info("5 rows removed");
},this);
S.add(Q);
S=new qx.ui.toolbar.Part();
O.add(S);
Q=new qx.ui.toolbar.Button(B);
Q.addListener(q,function(a){this._table.resetSelection();
},this);
S.add(Q);
Q=new qx.ui.toolbar.Button(F);
Q.addListener(q,function(K){this._table.resetCellFocus();
},this);
S.add(Q);
var T=new qx.ui.toolbar.CheckBox(v);
T.addListener(k,function(W){this._table.setShowCellFocusIndicator(!T.isValue());
},this);
S.add(T);
Q=new qx.ui.toolbar.Button(t,j);
Q.addListener(q,function(f){var g=[];
this._table.getSelectionModel().iterateSelection(function(b){g.push(b+o);
});
this.showDialog(s+g.join(w));
},this);
S.add(Q);
return O;
