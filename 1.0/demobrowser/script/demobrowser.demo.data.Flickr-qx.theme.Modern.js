...
(function(){var m="qooxdoo",l="/",k=".static.flickr.com/",j="http://api.flickr.com/services/rest/?tags=",i="",h="_",g="http://farm",f="Enter",d="keydown",c="model.photos.photo",F=".jpg",E="execute",D="source",C="selection[0]",B="_s.jpg",A="<span style='font-size: 20px'>Flickr</span>",z="loading",y="value",x="demobrowser.demo.data.store.Flickr",w="demobrowser.demo.data.Flickr",t="title",u="icon",r="state",s="Search",p="receiving",q="horizontal",n="model",o="&method=flickr.photos.search&api_key=63a8042eead205f7e0040f488c02afd9&format=json",v="id";
qx.Class.define(w,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var R=new demobrowser.demo.data.store.Flickr(m);
var J=new qx.ui.form.TextField(m);
this.getRoot().add(J,{left:30,top:50});
var K=new qx.ui.form.Button(s);
this.getRoot().add(K,{left:140,top:49});
K.addListener(E,function(){R.searchForTag(J.getValue());
},this);
J.addListener(d,function(e){if(e.getKeyIdentifier()==f){R.searchForTag(J.getValue());
}},this);
...
N.setHeight(110);
N.setOrientation(q);
this.getRoot().add(N,{left:30,right:30,top:80});
var P=new qx.data.controller.List(null,N);
P.setLabelPath(t);
P.setDelegate({configureItem:function(b){b.setShow(u);
}});
var L={converter:function(S,T){return (g+T.getFarm()+k+T.getServer()+l+S+h+T.getSecret()+B);
}};
P.setIconOptions(L);
P.setIconPath(v);
R.bind(c,P,n);
var M=new qx.ui.basic.Image();
this.getRoot().add(M,{left:30,top:200});
var Q={converter:function(U){if(U){return (g+U.getFarm()+k+U.getServer()+l+U.getId()+h+U.getSecret()+F);
}return i;
}};
P.bind(C,M,D,Q);
...
qx.Class.define(x,{extend:qx.data.store.Json,statics:{RESULT:null},construct:function(X){jsonFlickrApi=function(W){demobrowser.demo.data.store.Flickr.RESULT=W;
};
var Y=j+X;
arguments.callee.base.call(this,Y);
},members:{searchForTag:function(V){if(V!=i){this.setUrl(j+V);
}},_createRequest:function(H){var I=new qx.io.ScriptLoader();
this.setState(p);
H+=o;
I.load(H,function(a){this.__FD();
},this);
},__FD:function(){this.setState("completed");
var G=demobrowser.demo.data.store.Flickr.RESULT;

if(G==undefined){this.setState("failed");
return;
}this._marshaler.toClass(G);
