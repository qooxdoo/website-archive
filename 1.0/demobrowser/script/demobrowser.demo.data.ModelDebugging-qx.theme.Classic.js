...
(function(){var r="auto",q="debugModel function in qx.dev.Debug.",p="?callback=demobrowser.demo.data.store.Twitter.saveResult",o="objects with properties. To see the properties, you can use the ",n="demobrowser.demo.data.ModelDebugging",m="wittemann",l="loaded",k="Every model created by the data stores consists only of qooxdoo ",j="demobrowser.demo.data.store.Twitter",i="<b>Debugging models</b><br/>",f="white",h="main",g="http://twitter.com/statuses/user_timeline/",e=".json";
qx.Class.define(n,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var u=new demobrowser.demo.data.store.Twitter(m);
var s=new qx.ui.embed.Html();
...
u.addListener(l,function(){var d=u.getModel();
this.debug(qx.dev.Debug.debugProperties(d));
s.setHtml(qx.dev.Debug.debugProperties(d,10,true));
},this);
...
qx.Class.define(j,{extend:qx.data.store.Json,statics:{saveResult:function(y){this.__FE=y;
}},construct:function(w){var x=g+w+e;
arguments.callee.base.call(this,x);
},members:{_createRequest:function(b){var c=new qx.io.ScriptLoader();
b+=p;
c.load(b,function(a){this.__FF();
},this);
},__FF:function(){var v=demobrowser.demo.data.store.Twitter.__FE;

if(v==undefined){this.setState("failed");
return;
}this._marshaler.toClass(v);
this.setModel(this._marshaler.toModel(v));
...
(function(){var w=": ",v="",u="construct",t="object",s="-",r=":",q=") ***",p="\r\n",o="px;'>",n="): ",R="</span><br>",Q="*** EXCEPTION (",P="============================================================",O="Object",N="<br>",M="null",L="Array",K="members",J="statics",I="get",D="------------------------------------------------------------",E="qx.dev.Debug",B="\n",C="*** TOO MUCH RECURSION: not displaying ***",z="Object, count=",A="  ",x="<span style='padding-left:",y=" ",F="Array, length=",G="undefined",H="index(";
qx.Class.define(E,{statics:{debugObject:function(U,V,W){qx.log.Logger.debug(this,qx.dev.Debug.debugObjectToString(U,V,W,false));
},debugObjectToString:function(bd,be,bf,bg){if(!bf){bf=10;
}var bm=(bg?R:B);
var bj=function(S){var T;

if(!bg){T=v;

for(var i=0;i<S;i++){T+=A;
}}else{T=x+(S*8)+o;
}return T;
};
var bl=v;
var bi=function(Y,ba,bb){if(ba>bb){bl+=(bj(ba)+C+bm);
return;
}if(typeof (Y)!=t){bl+=bj(ba)+Y+bm;
return;
}for(var bc in Y){if(typeof (Y[bc])==t){if(Y[bc] instanceof Array){bl+=bj(ba)+bc+w+L+bm;
}else if(Y[bc]===null){bl+=bj(ba)+bc+w+M+bm;
continue;
}else if(Y[bc]===undefined){bl+=bj(ba)+bc+w+G+bm;
continue;
}else{bl+=bj(ba)+bc+w+O+bm;
}bi(Y[bc],ba+1,bb);
}else{bl+=bj(ba)+bc+w+Y[bc]+bm;
}}};
if(be){bl+=bj(0)+be+bm;
}
if(bd instanceof Array){bl+=bj(0)+F+bd.length+r+bm;
}else if(typeof (bd)==t){var bh=0;
...
switch(b){case u:return a==c?u:null;
case K:return qx.lang.Object.getKeyFromValue(c,a);
case J:return qx.lang.Object.getKeyFromValue(c.prototype,a);
default:if(a==c){return u;
}return (qx.lang.Object.getKeyFromValue(c.prototype,a)||qx.lang.Object.getKeyFromValue(c,a)||null);
}},debugProperties:function(d,e,f,g){if(e==null){e=10;
}if(g==null){g=1;
}var h=v;
f?h=N:h=p;
var l=v;

if(qx.lang.Type.isNumber(d)||qx.lang.Type.isString(d)||qx.lang.Type.isBoolean(d)||d==null||e<=0){return d;
}else if(qx.Class.hasInterface(d.constructor,qx.data.IListData)){for(var i=0;i<d.length;i++){for(var j=0;j<g;j++){l+=s;
}l+=H+i+n+this.debugProperties(d.getItem(i),e-1,f,g+1)+h;
}return l+h;
}else if(d.constructor!=null){var m=d.constructor.$$properties;

for(var k in m){l+=h;
for(var j=0;j<g;j++){l+=s;
}l+=y+k+w+this.debugProperties(d[I+qx.lang.String.firstUp(k)](),e-1,f,g+1);
}return l;
}return v;
}}});
})();
