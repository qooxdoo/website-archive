...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.progressive.ProgressiveLoader-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"cba1be9cc029"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var H="white",G="100%",F="func",E="demobrowser.demo.progressive.ProgressiveLoader",D="black",C="Red",B="http://resources.qooxdoo.org/images/logo.gif",A="green",z="yellow",y="col ",T="gray",S="blue",R="Green",Q="Yellow",P="Blue",O="renderStart",N=", ",M="purple",L="red",K="renderEnd",I="row ",J="Purple";
qx.Class.define(E,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var s=new qx.ui.progressive.renderer.table.Widths(1);
s.setWidth(0,G);
var l=new qx.ui.progressive.headfoot.Progress(s);
var q=new qx.ui.progressive.structure.Default(null,l);
var k=new qx.ui.progressive.Progressive(q);
k.setFlushWidgetQueueAfterBatch(true);
var p=function(g){var h={renderer:F,data:g};
return h;
};
var w=new qx.ui.progressive.model.Default();
var t=new qx.ui.progressive.renderer.FunctionCaller();
k.addRenderer(F,t);
var r=B;
var u=new qx.ui.basic.Image(r,G,G);
k.add(u);
k.set({height:100,width:272,zIndex:99999,backgroundColor:T,opacity:0.86,batchSize:10});
this.getRoot().add(k,{top:-1000,left:-1000});
var v=[{text:C,background:L,color:H},{text:R,background:A,color:H},{text:P,background:S,color:H},{text:J,background:M,color:H},{text:Q,background:z,color:D}];
this.context={document:this.getRoot(),pages:v};
k.addListener(O,function(e){var bd=e.getData().state;
var Y=e.getData().initial;
var bc=this.getRoot().getBounds();
var ba=e.getData().state.getProgressive();
var bb=ba.getBounds();
var be=Math.floor((bc.width-bb.width)/2);
var top=Math.floor((bc.height-bb.height)/2);
ba.setLayoutProperties({left:be<0?0:be,top:top<0?0:top});
bd.getUserData().context=this.context;
bd.getUserData().initialNum=Y;
},this);
k.addListener(K,function(e){qx.event.Timer.once(function(){this.getLayoutParent().remove(this);
this.dispose();
},this,0);
});
w.addElement(p(function(i){var j=i.context;
j.tabview=new qx.ui.tabview.TabView();
j.tabview.set({});
j.document.add(j.tabview,{left:20,right:20,top:20,bottom:20});
j.page=0;
j.currentPage=null;
}));

for(var m=0;m<v.length;m++){w.addElement(p(function(bf){var bh=bf.context;
var bg=bh.pages[bh.page];
bh.currentPage=new qx.ui.tabview.Page(bg.text);
bh.currentPage.setLayout(new qx.ui.layout.VBox(4));
bh.tabview.add(bh.currentPage);
bh.row=0;
bh.col=0;
if(bh.page==0){bh.firstPage=bh.currentPage;
}}));
for(var x=0;x<20;x++){w.addElement(p(function(d){var f=d.context;
f.hBox=new qx.ui.container.Composite(new qx.ui.layout.HBox(4));
f.currentPage.add(f.hBox);
f.tabview.setSelection([f.currentPage]);
f.col=0;
}));

for(var n=0;n<10;n++){w.addElement(p(function(a){var c=a.context;
var b=c.pages[c.page];
var o=new qx.ui.basic.Atom(I+c.row+N+y+c.col);
o.set({width:90,backgroundColor:b.background,textColor:b.color});
c.hBox.add(o);
}));
w.addElement(p(function(X){++X.context.col;
}));
}w.addElement(p(function(V){++V.context.row;
}));
}w.addElement(p(function(W){++W.context.page;
}));
}w.addElement(p(function(U){U.context.tabview.setSelection([U.context.firstPage]);
}));
k.setDataModel(w);
k.render();
}}});
...
(function(){var g="maxWidth",f="qx.ui.progressive.renderer.table.Widths",e="minWidth",d="width";
qx.Class.define(f,{extend:qx.core.Object,construct:function(m){arguments.callee.base.call(this);
this.__GW=[];

for(var i=0;i<m;i++){this.__GW[i]=new qx.ui.core.ColumnData();
}},members:{__GW:null,getData:function(){return this.__GW;
},set:function(a,b){for(var c in b){switch(c){case d:this.setWidth(a,b[c]);
break;
case e:this.setMinWidth(a,b[c]);
break;
case g:this.setMaxWidth(a,b[c]);
break;
default:throw new Error("Unrecognized key: "+c);
}}},setWidth:function(h,j){if(h>this.__GW.length-1||h<0){throw new Error("Column number out of range");
}this.__GW[h].setColumnWidth(j);
},setMinWidth:function(k,l){if(k>this.__GW.length-1||k<0){throw new Error("Column number out of range");
}this.__GW[k].setMinWidth(l);
},setMaxWidth:function(n,o){if(n>this.__GW.length-1||n<0){throw new Error("Column number out of range");
}this.__GW[n].setMaxWidth(o);
}}});
})();
...
},members:{__GX:null,join:function(c){this.__GX=c;
}},destruct:function(){this.__GX=null;
}});
})();
(function(){var r="progressive-progressbar-indicator-undone",q="progress",p="%",o="progressive-progressbar-indicator-done",n="progressive-progressbar-percent-text",m="renderStart",l="progressive-progressbar-background",k="0%",j="__Ha",i="qx.ui.progressive.headfoot.Progress",f="__Hb",h="progressive-progressbar-percent-background",g="renderEnd";
qx.Class.define(i,{extend:qx.ui.progressive.headfoot.Abstract,construct:function(a,b){arguments.callee.base.call(this);
this.setHeight(16);
this.setPadding(0);
this.__GY={};
var c=qx.theme.manager.Color.getInstance();
this.__GY.background=c.resolve(l);
this.__GY.indicatorDone=c.resolve(o);
this.__GY.indicatorUndone=c.resolve(r);
this.__GY.percentBackground=c.resolve(h);
this.__GY.percentText=c.resolve(n);
this.set({backgroundColor:this.__GY.background});
this.__Ha=new qx.ui.core.Widget();
this.__Ha.set({width:0,backgroundColor:this.__GY.indicatorDone});
this.add(this.__Ha);
var d=new qx.ui.core.Widget();
d.set({backgroundColor:this.__GY.indicatorUndone});
this.add(d,{flex:1});
this.__Hb=new qx.ui.basic.Atom(k);
this.__Hb.set({width:100,backgroundColor:this.__GY.percentBackground,textColor:this.__GY.percentText});
this.add(this.__Hb);
this.exclude();
},members:{__Hc:null,__GY:null,__Ha:null,__Hb:null,join:function(s){arguments.callee.base.call(this,s);
s.addListener(m,function(e){this.__Hc=e.getData().initial;
this.show();
},this);
s.addListener(q,function(e){var v=1.0-(e.getData().remaining/this.__Hc);
var w=this.getBounds();

if(w){var t=Math.floor((w.width-this.__Hb.getBounds().width)*v);
var u=Math.floor(v*100)+p;

if(!isNaN(t)){this.__Ha.setMinWidth(t);
this.__Hb.setLabel(u);
}}},this);
s.addListener(g,function(e){this.exclude();
},this);
}},destruct:function(){this.__GY=null;
this._disposeObjects(j,f);
}});
})();
...
qx.Class.define(c,{type:d,extend:qx.core.Object,construct:function(f){arguments.callee.base.call(this);
if(!f){this.__Hd=new qx.ui.core.Widget();
this.__He=this.__Hd;
}else{this.__Hd=null;
this.__He=f;
}this.__He.getContentElement().setStyle(b,a);
},members:{__Hd:null,__He:null,applyStructure:function(e){throw new Error("applyStructure() is abstract");
},getPane:function(){return this.__He;
}},destruct:function(){if(this.__Hd){this.__Hd.dispose();
}this.__Hd=this.__He=null;
}});
})();
(function(){var b="qx.ui.progressive.structure.Default";
qx.Class.define(b,{extend:qx.ui.progressive.structure.Abstract,construct:function(c,d,e){arguments.callee.base.call(this,e);
if(!c){this.__Hf=new qx.ui.progressive.headfoot.Null();
this.__Hg=this.__Hf;
}else{this.__Hf=null;
this.__Hg=c;
}if(!d){this.__Hh=new qx.ui.progressive.headfoot.Null();
this.__Hi=this.__Hh;
}else{this.__Hh=null;
this.__Hi=d;
}},members:{__Hg:null,__Hi:null,__Hf:null,__Hh:null,applyStructure:function(a){this.__Hg.join(a);
this.__Hi.join(a);
a.add(this.__Hg);
a.add(this.getPane(),{flex:1});
a.add(this.__Hi);
},getHeader:function(){return this.__Hg;
},getFooter:function(){return this.__Hi;
}},destruct:function(){if(this.__Hf){this.__Hf.dispose();
this.__Hf=null;
}
if(this.__Hh){this.__Hh.dispose();
this.__Hh=null;
}this.__Hg=this.__Hi=null;
}});
})();
...
(function(){var s="qx.event.type.Data",r="dataAvailable",q="renderStart",p="Integer",o="_applyDataModel",n="Boolean",m="qx.ui.progressive.model.Abstract",l="progress",k="white",j="qx.ui.progressive.Progressive",g="progressDetail",h="renderEnd";
qx.Class.define(j,{extend:qx.ui.container.Composite,construct:function(x){arguments.callee.base.call(this,new qx.ui.layout.VBox());
this.__Hj={};
this.set({backgroundColor:k});
...
this.__Hm=false;
this.__Hn=0;
},events:{"renderStart":s,"renderEnd":s,"progress":s,"progressDetail":s},properties:{dataModel:{check:m,apply:o},batchSize:{check:p,init:20},flushWidgetQueueAfterBatch:{check:n,init:false},interElementTimeout:{check:p,init:0}},members:{__Hj:null,__Hm:null,__Ho:null,__Hn:null,__Hl:null,__Hk:null,getStructure:function(){return this.__Hk;
},addRenderer:function(name,f){this.__Hj[name]=f;
f.join(this,name);
},removeRenderer:function(name){if(!this.__Hj[name]){throw new Error("No existing renderer named "+name);
}delete this.__Hj[name];
},render:function(){if(this.__Hm){return;
}this.__Hm=true;
var w=new qx.ui.progressive.State({progressive:this,model:this.getDataModel(),pane:this.__Hk.getPane(),batchSize:this.getBatchSize(),rendererData:this.__Hq(),userData:{}});
this.__Ho=new Date();
if(this.__Hl){this.__Hn=w.getModel().getElementCount();
this.fireDataEvent(q,{state:w,initial:this.__Hn});
this.__Hp(w);
}else{qx.event.Timer.once(function(){this.__Hn=w.getModel().getElementCount();
this.fireDataEvent(q,{state:w,initial:this.__Hn});
this.__Hp(w);
this.__Hl=true;
},this,10);
}},_applyDataModel:function(u,v){if(v){v.removeListener(r,this.__Hr,this);
v.dispose();
}u.addListener(r,this.__Hr,this);
},__Hp:function(a){var b;
var d;
var c;

for(var i=a.getBatchSize();i>0;i--){b=a.getModel().getNextElement();

if(!b){this.debug("Render time: "+(new Date()-this.__Ho)+"ms");
this.__Hm=false;
this.fireDataEvent(h,a);
a.dispose();
return ;
}d=b.element;
c=this.__Hj[d.renderer];
c.render(a,d);
this.fireDataEvent(g,{initial:this.__Hn,remaining:b.remaining,element:d});
}this.fireDataEvent(l,{initial:this.__Hn,remaining:b.remaining});
if(this.getFlushWidgetQueueAfterBatch()){qx.ui.core.queue.Manager.flush();
}qx.event.Timer.once(function(){this.__Hp(a);
},this,this.getInterElementTimeout());
},__Hq:function(){var t={};

for(var name in this.__Hj){t[name]={};
}return t;
},__Hr:function(e){this.__Hn=e.getData();
this.render();
}},destruct:function(){for(var name in this.__Hj){this.__Hj[name].dispose();
}this.__Ho=this.__Hj=this.__Hk=null;
}});
})();
...
this.__Hs=[];
},members:{__Hs:null,addElements:function(d){this.__Hs=this.__Hs.concat(d);
this.fireDataEvent(b,this.__Hs.length);
},addElement:function(c){this.__Hs.push(c);
this.fireDataEvent(b,this.__Hs.length);
},getElementCount:function(){return this.__Hs.length;
},getNextElement:function(){if(this.__Hs.length>0){return ({element:this.__Hs.shift(),remaining:this.__Hs.length});
}return null;
...
(function(){var v="pane",u="lastTab",t="bar",s="page",r="firstTab",q="right",p="bottom",o="button",n="changeSelection",m="close",J="top",I="left",H="qx.event.type.Data",G="barRight",F="__rN",E="beforeChangeSelection",D="tabview",C="vertical",B="_applyBarPosition",A="barLeft",y="horizontal",z="qx.ui.tabview.TabView",w="barTop",x="barBottom";
qx.Class.define(z,{extend:qx.ui.core.Widget,implement:qx.ui.core.ISingleSelection,include:[qx.ui.core.MContentPadding],construct:function(c){arguments.callee.base.call(this);
this.__rM={top:w,right:G,bottom:x,left:A};
this._createChildControl(t);
this._createChildControl(v);
var d=this.__rN=new qx.ui.form.RadioGroup;
d.setWrap(false);
d.addListener(n,this._onChangeSelection,this);
if(c!=null){this.setBarPosition(c);
}else{this.initBarPosition();
}},events:{"changeSelection":H},properties:{appearance:{refine:true,init:D},barPosition:{check:[I,q,J,p],init:J,apply:B}},members:{__rN:null,_createChildControlImpl:function(R){var S;

switch(R){case t:S=new qx.ui.container.SlideBar();
S.setZIndex(10);
this._add(S);
break;
case v:S=new qx.ui.container.Stack;
S.setZIndex(5);
this._add(S,{flex:1});
break;
...
k.add(f);
this.__rN.add(g);
f.addState(this.__rM[this.getBarPosition()]);
f.addState(u);
var j=this.getChildren();

if(j[0]==f){f.addState(r);
}else{j[j.length-2].removeState(u);
}f.addListener(m,this._onPageClose,this);
},remove:function(Y){var be=this.getChildControl(v);
var bc=this.getChildControl(t);
var bb=Y.getButton();
var bd=be.getChildren();
if(this.getSelection()[0]==Y){var ba=bd.indexOf(Y);

if(ba==0){if(bd[1]){this.setSelection([bd[1]]);
}else{this.resetSelection();
}}else{this.setSelection([bd[ba-1]]);
}}bc.remove(bb);
be.remove(Y);
this.__rN.remove(bb);
Y.removeState(this.__rM[this.getBarPosition()]);
if(Y.hasState(r)){Y.removeState(r);

if(bd[0]){bd[0].addState(r);
}}
if(Y.hasState(u)){Y.removeState(u);

if(bd.length>0){bd[bd.length-1].addState(u);
}}Y.removeListener(m,this._onPageClose,this);
},getChildren:function(){return this.getChildControl(v).getChildren();
},indexOf:function(X){return this.getChildControl(v).indexOf(X);
},__rM:null,_applyBarPosition:function(bf,bg){var bh=this.getChildControl(t);
var bm=bf==I||bf==q;
var bk=bf==q||bf==p;
var bl=bm?qx.ui.layout.HBox:qx.ui.layout.VBox;
var bo=this._getLayout();

if(bo&&bo instanceof bl){}else{this._setLayout(bo=new bl);
}bo.setReversed(bk);
bh.setOrientation(bm?C:y);
var bn=this.getChildren();
if(bg){var bi=this.__rM[bg];
bh.removeState(bi);
for(var i=0,l=bn.length;i<l;i++){bn[i].removeState(bi);
}}
if(bf){var bj=this.__rM[bf];
bh.addState(bj);
for(var i=0,l=bn.length;i<l;i++){bn[i].addState(bj);
}}},getSelection:function(){var T=this.__rN.getSelection();
var U=[];

for(var i=0;i<T.length;i++){U.push(T[i].getUserData(s));
}return U;
},setSelection:function(a){var b=[];

for(var i=0;i<a.length;i++){b.push(a[i].getChildControl(o));
}this.__rN.setSelection(b);
},resetSelection:function(){this.__rN.resetSelection();
},isSelected:function(O){var P=O.getChildControl(o);
return this.__rN.isSelected(P);
},isSelectionEmpty:function(){return this.__rN.isSelectionEmpty();
},getSelectables:function(){var V=this.__rN.getSelectables();
var W=[];

for(var i=0;i<V.length;i++){W.push(V[i].getUserData(s));
}return W;
},_onChangeSelection:function(e){var N=this.getChildControl(v);
var K=e.getData()[0];

if(K){N.setSelection([K.getUserData(s)]);
K.focus();
this.scrollChildIntoView(K,null,null,false);
}else{N.resetSelection();
}var M=N.getSelection();
var L=e.getOldData();
this.fireDataEvent(n,M,L);
},_onBeforeChangeSelection:function(e){if(!this.fireNonBubblingEvent(E,qx.event.type.Event,[false,true])){e.preventDefault();
}},_onRadioChangeSelection:function(e){var Q=e.getData()[0];

if(Q){this.setSelection([Q.getUserData(s)]);
}else{this.resetSelection();
...
this._setLayout(new qx.ui.layout.Grow);
this.addListener(s,this.__kW,this);
},properties:{dynamic:{check:r,init:false,apply:t}},members:{_applyDynamic:function(u){var w=this._getChildren();
...
},_isItemSelectable:function(p){return p.isEnabled();
},__kW:function(e){var a=e.getOldData()[0];
var b=e.getData()[0];
...
(function(){var s="button",r="",q="close",p="String",o="Boolean",n="_applyIcon",m="page",l="qx.ui.tabview.Page",k="tabview-page",j="_applyShowCloseButton",h="_applyLabel",i="qx.event.type.Data";
qx.Class.define(l,{extend:qx.ui.container.Composite,construct:function(f,g){arguments.callee.base.call(this);
this._createChildControl(s);
if(f!=null){this.setLabel(f);
}
if(g!=null){this.setIcon(g);
}},events:{"close":i},properties:{appearance:{refine:true,init:k},label:{check:p,init:r,apply:h},icon:{check:p,init:r,apply:n},showCloseButton:{check:o,init:false,apply:j}},members:{_forwardStates:{barTop:1,barRight:1,barBottom:1,barLeft:1,firstTab:1,lastTab:1},_applyIcon:function(x,y){this.getChildControl(s).setIcon(x);
},_applyLabel:function(t,u){this.getChildControl(s).setLabel(t);
...
switch(a){case s:b=new qx.ui.tabview.TabButton;
b.setAllowGrowX(true);
b.setAllowGrowY(true);
b.setUserData(m,this);
b.addListener(q,this._onButtonClose,this);
break;
}return b||arguments.callee.base.call(this,a);
},_applyShowCloseButton:function(v,w){this.getChildControl(s).setShowCloseButton(v);
...
(function(){var u="close-button",t="middle",s="left",r="icon",q="label",p="right",o="click",n="Boolean",m="bottom",l="qx.ui.tabview.TabButton",i="center",k="_applyShowCloseButton",j="top",h="close",g="qx.event.type.Data";
qx.Class.define(l,{extend:qx.ui.form.RadioButton,implement:qx.ui.form.IRadioItem,construct:function(){arguments.callee.base.call(this);
var v=new qx.ui.layout.Grid(2,0);
v.setRowAlign(0,s,t);
v.setColumnAlign(0,p,t);
this._getLayout().dispose();
this._setLayout(v);
this.initShowCloseButton();
},events:{"close":g},properties:{showCloseButton:{check:n,init:false,apply:k}},members:{_applyIconPosition:function(c,d){var e={icon:this.getChildControl(r),label:this.getChildControl(q),closeButton:this.getShowCloseButton()?this.getChildControl(u):null};
for(var f in e){if(e[f]){this._remove(e[f]);
}}
switch(c){case j:this._add(e.label,{row:3,column:2});
this._add(e.icon,{row:1,column:2});

if(e.closeButton){this._add(e.closeButton,{row:0,column:4});
}break;
case m:this._add(e.label,{row:1,column:2});
this._add(e.icon,{row:3,column:2});

if(e.closeButton){this._add(e.closeButton,{row:0,column:4});
}break;
case s:this._add(e.label,{row:0,column:2});
this._add(e.icon,{row:0,column:0});

if(e.closeButton){this._add(e.closeButton,{row:0,column:4});
}break;
case p:this._add(e.label,{row:0,column:0});
this._add(e.icon,{row:0,column:2});

if(e.closeButton){this._add(e.closeButton,{row:0,column:4});
}break;
}},_createChildControlImpl:function(w){var x;

switch(w){case q:var x=new qx.ui.basic.Label(this.getLabel());
x.setAnonymous(true);
this._add(x,{row:0,column:2});
...
break;
case u:x=new qx.ui.form.Button();
x.addListener(o,this._onCloseButtonClick,this);
this._add(x,{row:0,column:4});

if(!this.getShowCloseButton()){x.exclude();
}break;
}return x||arguments.callee.base.call(this,w);
},_onCloseButtonClick:function(){this.fireDataEvent(h,this);
},_applyShowCloseButton:function(a,b){if(a){this._showChildControl(u);
}else{this._excludeChildControl(u);
}},_applyCenter:function(y){var z=this._getLayout();

if(y){z.setColumnAlign(2,i,t);
}else{z.setColumnAlign(2,s,t);
}}}});
})();
