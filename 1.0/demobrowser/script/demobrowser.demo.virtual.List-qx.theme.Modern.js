...
(function(){var F="value",E="one",D="multi",C="changeValue",B="single",A="additive",z="bold",y="Item No ",x="Julian",w="Moritz",bS="Jannis",bR="Kevin",bQ="Robin",bP="Felix",bO="Vincent",bN="demobrowser.demo.virtual.List",bM="Additive Selection",bL="/places/folder.png",bK="Nils",bJ="Nick",M="Leon",N="Jannick",K="Linus",L="Configurable",I="Marc",J="icon/16/places/folder.png",G="Erik",H="addaptive",U="Colin",V="Lennard",bl="changeSelection",bh="Jason",bt="Tobias",bo="Florian",bF="16",bz="Jonas",bc="icon/",bI="Jakob",bH="Enable drag selection",bG="Joel",ba="David",be="Maximilian",bg="48",bj="Marlon",bm="Lukas",bp="Luca",bv="Julius",bB="Fabian",O="Tim",P="Justin",bd="Single Selection",bs="Simon",br="Luis",bq="Johannes",bx="Nico",bw="Alexander",bn="Jan",bu="Dominic",t="Noah",bA="Enable quick selection",Q="Daniel",R="Paul",bi="Philipp",u="Niclas",v="Max",Y="One Selection",S="Additive selection",T="Jonathan",X="Tom",bk="Finn",bD="Ben",bC="on",bf="One as selection mode",bE="Elias",bb="Mika",by="Multi Selection",W="Marvin";
qx.Class.define(bN,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
...
for(var i=1;i<=2500;i++){l=new demobrowser.demo.virtual.list.ListItem(y+i,bc+((i%4)?bF:bg)+bL);

if(i%4==0){l.setHeight(56);
}p.add(l);
if(i==20){p.setSelected(l);
}}p.addListener(C,function(e){this.debug("Value: "+e.getData());
});
this.getRoot().add(p,{left:20,top:40});
var g=new qx.ui.form.RadioButton(bd);
var c=new qx.ui.form.RadioButton(by);
var d=new qx.ui.form.RadioButton(bM);
...
var k=new qx.ui.form.CheckBox(bH);
var n=new qx.ui.form.CheckBox(bA).set({enabled:false});
this.getRoot().add(k,{left:180,top:220});
this.getRoot().add(n,{left:180,top:240});
k.addListener(C,function(e){if(e.getData()){var s=p.getSelectionMode();

if(s==B||s==E){this.debug("Drag selection is only available for the modes multi or additive");
}}p.setDragSelection(e.getData());
});
n.addListener(C,function(e){if(e.getData()){var bT=p.getSelectionMode();

if(bT==D||bT==A){this.debug("Quick selection is only available for the modes multi or additive");
}}p.setQuickSelection(e.getData());
});
f.addListener(bl,function(e){var a=e.getData()[0].getUserData(F);
p.setSelectionMode(a);

if(a==B||a==E){k.setEnabled(false);
n.setEnabled(true);
}else if(a==D||a==H){k.setEnabled(true);
n.setEnabled(false);
}});
var r=new qx.ui.basic.Label(bf);
r.setFont(z);
...
for(var i=1;i<=25;i++){l=new demobrowser.demo.virtual.list.ListItem(y+i,J);
m.add(l);
if(i==16){m.setSelected(l);
}}this.getRoot().add(m,{left:330,top:40});
var o=new qx.ui.basic.Label(S);
o.setFont(z);
this.getRoot().add(o,{left:520,top:20});
var q=new demobrowser.demo.virtual.list.List;
var h;
q.set({width:150,selectionMode:A});
var j=[M,bm,bp,bk,O,bP,bz,br,be,x,v,R,u,bn,bD,bE,N,bi,t,X,w,bx,ba,bK,bs,bB,G,P,bw,bI,bo,bJ,K,bb,bh,Q,V,W,bS,bt,bu,bj,I,bq,T,bv,U,bG,bR,bO,bQ];
...
(function(){var D="selected",C="qx.event.type.Data",B="Boolean",A="changeSelection",z="row",y="single",x="changeHeight",w=",",v="one",u="_applySpacing",S="Integer",R="_applyDragSelection",Q="String",P="resize",O="demobrowser.demo.virtual.list.List",N="list",M="multi",L="_applyQuickSelection",K="_applyItemHeight",J="_applySelectionMode",H="additive",I="changeName",F="Number",G="addItem",E="removeItem";
qx.Class.define(O,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider],construct:function(){arguments.callee.base.call(this,0,1,this.getItemHeight(),10);
this.__IX=new qx.ui.virtual.layer.WidgetCell(this);
this.getPane().addLayer(this.__IX);
this.getPane().addListener(P,this._onResize,this);
this.__IY=[];
this.__Ja=[];
this.__Jb=new qx.ui.virtual.selection.Row(this.getPane());
this.__Jb.addListener(A,this._onChangeSelection,this);
this.__Jb.attachMouseEvents();
this.__Jb.attachKeyEvents(this);
this.__Jb.attachListEvents(this);
new qx.ui.virtual.behavior.Prefetch(this,0,0,0,0,400,600,400,600).set({interval:500});
},events:{addItem:C,removeItem:C,changeValue:C,changeSelection:C},properties:{appearance:{refine:true,init:N},focusable:{refine:true,init:true},spacing:{check:S,init:0,apply:u,themeable:true},enableInlineFind:{check:B,init:true},name:{check:Q,nullable:true,event:I},itemHeight:{init:24,themeable:true,check:F,apply:K},selectionMode:{check:[y,M,H,v],init:y,apply:J},dragSelection:{check:B,init:false,apply:R},quickSelection:{check:B,init:false,apply:L}},members:{__IY:null,__Ja:null,__Jb:null,__IX:null,syncWidget:function(){this.update();
},update:function(){var U=this.getPane().getRowConfig();
U.setItemCount(this.__IY.length);
U.resetItemSizes();

for(var i=0;i<this.__IY.length;i++){var T=this.__IY[i].getHeight();

if(T!==null){U.setItemSize(i,T);
}}},updateSelection:function(){var bq=this.__IX.getChildren();

for(var i=0;i<bq.length;i++){var br=bq[i];
var bs=br.getUserData(z);

if(this.__Jb.isItemSelected(bs)){br.addState(D);
}else{br.removeState(D);
...
this.fireDataEvent(A,this.__Jf(e.getData()));
},getCellWidget:function(V,W){var Y=this.__IY[V];

if(!Y){return null;
}var X=this.__Ja.pop()||new qx.ui.form.ListItem();
X.set({label:Y.getLabel(),icon:Y.getIcon()});

if(this.__Jb.isItemSelected(V)){X.addState(D);
}else{X.removeState(D);
}X.setUserData(z,V);
return X;
},poolCellWidget:function(m){this.__Ja.push(m);
},getValue:function(){var r=this.getSelection();
var s=[];
var t;

for(var i=0,l=r.length;i<l;i++){t=r[i].getValue();
if(t==null){t=r[i].getLabel();
}s.push(t);
}return s.join(w);
},setValue:function(a){var d=a.split(w);
var c=[];
var b;

for(var i=0,l=d.length;i<l;i++){b=this.findItem(d[i]);

if(b){c.push(b);
}}this.setSelection(c);
},getChildren:function(){return this.__IY;
},hasChildren:function(){return this.__IY.length>0;
},_addHelper:function(bh){this.fireDataEvent(G,bh);
bh.addListener(x,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},_removeHelper:function(bB){this.fireDataEvent(E,bB);
bB.removeListener(x,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},add:function(n){this.__IY.push(n);
this._addHelper(n);
return this;
},remove:function(be){qx.lang.Array.remove(this.__IY,be);
this._removeHelper(be);
return this;
},removeAll:function(){for(var i=0,j=this.__IY.length;i<j;i++){this._removeHelper(this.__IY[i]);
}this.__IY=[];
},indexOf:function(o){return this.__IY.indexOf(o);
},addAt:function(bw,bx){qx.lang.Array.insertAt(this.__IY,bw,bx);
this._addHelper(bw);
},addBefore:function(ba,bb){qx.lang.Array.insertBefore(this.__IY,ba,bb);
this._addHelper(ba);
},addAfter:function(h,k){qx.lang.Array.insertAfter(this.__IY,h,k);
this._addHelper(h);
},removeAt:function(f){this.__IY.splice(f,1);
this._removeHelper(this.__IY[f]);
},__Jc:function(p){var q=this.__IY.indexOf(p);

if(q<0){return null;
}else{return q;
}},__Jd:function(bz){var bA=this.__IY[bz];
return bA;
},__Je:function(bf){var bg=[];

for(var i=0;i<bf.length;i++){bg.push(this.__Jc(bf[i]));
}return bg;
},__Jf:function(bm){var bn=[];

for(var i=0;i<bm.length;i++){bn.push(this.__IY[bm[i]]);
}return bn;
},selectAll:function(){this.__Jb.selectAll();
},setSelected:function(bd){this.__Jb.selectItem(this.__Jc(bd));
},isSelected:function(by){return this.__Jb.isItemSelected(this.__Jc(by));
},addToSelection:function(bc){this.__Jb.addItem(this.__Jc(bc));
},removeFromSelection:function(bv){this.__Jb.removeItem(this.__Jc(bv));
},selectRange:function(bo,bp){this.__Jb.selectItemRange(this.__Jc(bo),this.__Jc(bp));
},resetSelection:function(){this.__Jb.clearSelection();
},setSelection:function(g){this.__Jb.replaceSelection(this.__Je(g));
},getSelectedItem:function(){return this.__IY[this.getSelected()];
},getSelected:function(){return this.__IY[this.__Jb.getSelectedItem()];
},getSelection:function(){return this.__Jf(this.__Jb.getSelection());
},getSortedSelection:function(){return this.__Jf(this.__Jb.getSortedSelection());
},isSelectionEmpty:function(){return this.__Jb.isSelectionEmpty();
},getSelectionContext:function(){return this.__Jb.getSelectionContext();
},getSelectables:function(){return this.__Jf(this.__Jb.getSelectables());
},_applySelectionMode:function(bt,bu){this.__Jb.setMode(bt);
},_applyDragSelection:function(bi,bj){this.__Jb.setDrag(bi);
},_applyQuickSelection:function(bk,bl){this.__Jb.setQuick(bk);
}}});
})();
...
(function(){var l="String",k="Integer",j="changeWidth",i="qx.ui.form.RadioGroup",h="_applyManager",g="changeHeight",f="qx.event.type.Event",e="changeValue",d="demobrowser.demo.virtual.list.ListItem";
...
}},events:{"action":f},properties:{label:{nullable:true,dispose:true,check:l},icon:{check:l,nullable:true},manager:{check:i,nullable:true,apply:h},value:{check:l,nullable:true,event:e},width:{check:k,nullable:true,event:j,init:null},height:{check:k,nullable:true,event:g,init:null}},members:{_applyManager:function(m,n){if(n){n.remove(this);
}
if(m){m.add(this);
