...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.bom.Window","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.bom.Window-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"ae36ca44e40c"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var o="click",n="button",m="Open Native Window",l="Open Native Modal Window",k="demobrowser.demo.bom.Window",j="Modal Window closed?",i="window1",h="Window closed?",g="window2";
qx.Class.define(k,{extend:qx.application.Native,members:{__BW:null,main:function(){arguments.callee.base.call(this);
this.__BW=["http://qooxdoo.org","http://sourceforge.net","http://slashdot.org"];
var d=qx.bom.Input.create(n);
qx.bom.Input.setValue(d,m);
document.body.appendChild(d);
qx.event.Registration.addListener(d,o,function(e){var p={width:400,height:200,top:200,left:100,scrollbars:false,menubar:true,status:false};
this.window1=qx.bom.Window.open(this.__BW[0],i,p);
},this);
var c=qx.bom.Input.create(n);
qx.bom.Input.setValue(c,h);
document.body.appendChild(c);
qx.event.Registration.addListener(c,o,function(e){alert(qx.bom.Window.isClosed(this.window1));
},this);
var b=qx.bom.Input.create(n);
qx.bom.Input.setValue(b,l);
document.body.appendChild(b);
qx.event.Registration.addListener(b,o,function(e){var a={width:400,height:200,top:200,left:100,scrollbars:false};
this.window2=qx.bom.Window.open(this.__BW[1],g,a,true);
},this);
var f=qx.bom.Input.create(n);
qx.bom.Input.setValue(f,j);
document.body.appendChild(f);
qx.event.Registration.addListener(f,o,function(e){alert(qx.bom.Window.isClosed(this.window2));
},this);
...
(function(){var C="resizable",B="=",A="dialogHeight",z="no",y="yes",x="",w="px",v="qx.bom.Window",u="interval",t="dialogWidth",n="dialogLeft",s="javascript:/",q=":",m=";",l="dialogTop",p="qxNativeWindow",o="scrollbars",r="scroll",k=",";
qx.Class.define(v,{statics:{__BX:null,__BY:null,__Ca:null,__Cb:{"top":l,left:n,width:t,height:A,scrollbars:r,resizable:C},__Cc:{"top":1,left:1,width:1,height:1,dependent:1,resizable:1,status:1,location:1,menubar:1,scrollbars:1,toolbar:1},__Cd:function(){return window.showModalDialog!=null;
},open:function(H,name,I,J){if(H==null){H=s;
}
if(name==null){name=p+new Date().getTime();
}var K=this.__Ce(I,J);

if(J){if(this.__Cd()){return window.showModalDialog(H,null,K);
}else{if(this.__BX==null){this.__BX=new qx.bom.Blocker;
}this.__BX.block();

if(this.__Ca==null){this.__Ca=new qx.event.Timer(200);
this.__Ca.addListener(u,this.__Cf,this);
this.__Ca.start();
}else{this.__Ca.restart();
}this.__BY=window.open(H,name,K);
return this.__BY;
}}else{return window.open(H,name,K);
}},__Ce:function(d,e){var f;
var i;
var g=[];

if(e&&this.__Cd()){for(var h in d){if(qx.bom.Window.__Cb[h]){var j=x;

if(h!=o&&h!=C){j=w;
}i=qx.bom.Window.__Cb[h]+q+d[h]+j;
g.push(i);
}else{qx.log.Logger.warn("Option '"+h+"' is not supported for modal windows.");
}}f=g.join(m);
}else{for(var h in d){if(qx.bom.Window.__Cc[h]){if(qx.lang.Type.isBoolean(d[h])){i=h+B+d[h]?y:z;
}else{i=h+B+d[h];
}g.push(i);
}else{qx.log.Logger.warn("Option '"+h+"' is not supported for native windows.");
}}f=g.join(k);
}return f;
},__Cf:function(){if(this.isClosed(this.__BY)){this.__BX.unblock();
this.__Ca.stop();
}},close:function(D){if(D){return D.close();
}},isClosed:function(c){var closed=true;

if(c){try{closed=c.closed;
}catch(N){}}return closed;
},moveTo:function(L,top,M){if(!qx.bom.Window.isClosed(L)){try{L.moveTo(M,top);
}catch(b){qx.log.Logger.error("Cross-Domain Scripting problem: Could not move window!",b);
}}},resizeTo:function(E,F,G){if(!qx.bom.Window.isClosed(E)){try{E.resizeTo(F,G);
}catch(a){qx.log.Logger.error("Cross-Domain Scripting problem: Could not resize window!",a);
}}}}});
})();
...
(function(){var x="px",w="mshtml",v="qx.client",u="0px",t="backgroundColor",s="block",r="zIndex",q="resize",p="repeat",o="transparent",i="url(",n="src",l=")",h="allowTransparency",g="div",k="javascript:false;",j="qx/static/blank.gif",m="qx.bom.Blocker",f="absolute";
qx.Class.define(m,{extend:qx.core.Object,construct:function(){arguments.callee.base.call(this);
this.__xA();
},members:{__xt:null,__xu:null,__xv:null,__xw:false,__xx:10000,__xy:0,__xz:o,block:function(C){if(!this.__xw){this.__xv=C;
var D=this.__xD();
this.__xE(D);
this.__xw=true;
}},unblock:function(){if(this.__xw){this.__xF();
this.__xw=false;
}},isBlocked:function(){return this.__xw;
},getBlockerElement:function(){return this.__xu;
},setBlockerColor:function(z){qx.bom.element.Style.set(this.__xu,t,z);
},getBlockerColor:function(){return qx.bom.element.Style.get(this.__xu,t);
},setBlockerOpacity:function(c){qx.bom.element.Opacity.set(this.__xu,c);
},getBlockerOpacity:function(){return qx.bom.element.Opacity.get(this.__xu);
},setBlockerZIndex:function(d){qx.bom.element.Style.set(this.__xu,r,d);
},getBlockerZIndex:function(){return qx.bom.element.Style.get(this.__xu,r);
},__xA:function(){this.__xB();

if(qx.core.Variant.isSet(v,w)){this.__xC();
}qx.event.Registration.addListener(window,q,this.__xG,this);
},__xB:function(){this.__xu=qx.bom.Element.create(g);
qx.bom.element.Style.setStyles(this.__xu,{display:s,opacity:this.__xy,backgroundColor:this.__xz});
this.setBlockerZIndex(this.__xx);
if(qx.core.Variant.isSet(v,w)){qx.bom.element.Style.setStyles(this.__xu,{backgroundImage:i+qx.util.ResourceManager.getInstance().toUri(j)+l,backgroundRepeat:p});
}},__xC:function(){this.__xt=qx.bom.Iframe.create();
qx.bom.element.Attribute.set(this.__xt,h,false);
qx.bom.element.Attribute.set(this.__xt,n,k);
qx.bom.element.Style.setStyles(this.__xt,{display:s,opacity:this.__xy});
},__xD:function(){var a={position:f};

if(this.__xI()){a.left=u;
a.top=u;
a.right=null;
a.bottom=null;
a.width=qx.bom.Document.getWidth()+x;
a.height=qx.bom.Document.getHeight()+x;
}else{a.width=qx.bom.element.Dimension.getWidth(this.__xv)+x;
a.height=qx.bom.element.Dimension.getHeight(this.__xv)+x;
a.left=qx.bom.element.Location.getLeft(this.__xv)+x;
a.top=qx.bom.element.Location.getTop(this.__xv)+x;
}return a;
},__xE:function(A){var B=document.body;
qx.bom.element.Style.setStyles(this.__xu,A);
qx.dom.Element.insertEnd(this.__xu,B);

if(qx.core.Variant.isSet(v,w)){A.zIndex=this.getBlockerZIndex()-1;
qx.bom.element.Style.setStyles(this.__xt,A);
qx.dom.Element.insertEnd(this.__xt,document.body);
}},__xF:function(){qx.dom.Element.remove(this.__xu);

if(qx.core.Variant.isSet(v,w)){qx.dom.Element.remove(this.__xt);
}},__xG:function(e){if(this.__xI()){this.__xH({width:u,height:u});
qx.event.Timer.once(function(){var y={width:qx.bom.Document.getWidth()+x,height:qx.bom.Document.getHeight()+x};
this.__xH(y);
},this,0);
}},__xH:function(b){qx.bom.element.Style.setStyles(this.__xu,b);

if(qx.core.Variant.isSet(v,w)){qx.bom.element.Style.setStyles(this.__xt,b);
}},__xI:function(){return (this.__xv==null||qx.dom.Node.isWindow(this.__xv)||qx.dom.Node.isDocument(this.__xv));
}}});
})();
