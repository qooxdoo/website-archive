...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.ui.Placement","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.ui.Placement-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"53683e7cb669"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var q="mousedown",p="changeSelection",o="demobrowser.demo.ui.Placement",n="DOM Popup",m="Open Popup",l="Content",k="top-left",j="bottom-right",h="right-top",g="bottom-left",c="left-bottom",f="top-right",d="Open DOM-Popup",b="domanchor",a="DOM align control";
qx.Class.define(o,{extend:qx.application.Standalone,members:{__JW:null,main:function(){arguments.callee.base.call(this);
var r=this.getRoot();
this.__JW=["bottom-left","bottom-right","top-left","top-right","right-top","right-bottom","left-top","left-bottom"];
r.add(this.createTestField(g),{left:20,top:20});
r.add(this.createTestField(j),{right:20,top:20});
r.add(this.createTestField(k),{right:20,bottom:20});
r.add(this.createTestField(f),{left:20,bottom:20});
r.add(this.createTestField(h),{left:20,top:250});
r.add(this.createTestField(c),{right:20,top:250});
r.add(this.createDomClick(),{left:200,top:100});
},createTestField:function(s){var w=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));
var u=new qx.ui.popup.Popup(new qx.ui.layout.Grow);
u.add(new qx.ui.basic.Atom(l));
u.setPadding(20);
var t=new qx.ui.form.SelectBox;
var x=null;

for(var i=0;i<this.__JW.length;i++){var v=new qx.ui.form.ListItem(this.__JW[i]);
t.add(v);

if(this.__JW[i]==s){x=v;
}}w.add(t);
t.addListener(p,function(e){u.setPosition(e.getData()[0].getLabel());
});
t.setSelection([x]);
var y=new qx.ui.form.Button(m);
w.add(y);
y.addListener(q,function(e){u.placeToWidget(y);
u.show();
});
return w;
},createDomClick:function(){var B=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));
var D=new qx.ui.basic.Label(a);
B.add(D);
var A=new qx.ui.popup.Popup(new qx.ui.layout.Grow);
A.add(new qx.ui.basic.Atom(n));
A.setPadding(20);
var z=new qx.ui.form.SelectBox;

for(var i=0;i<this.__JW.length;i++){z.add(new qx.ui.form.ListItem(this.__JW[i]));
}B.add(z);
z.addListener(p,function(e){A.setPosition(e.getData()[0].getLabel());
});
var C=new qx.ui.form.Button(d);
B.add(C);
C.addListener(q,function(e){A.placeToElement(document.getElementById(b));
A.show();
});
return B;
}},destruct:function(){this.__JW=null;
}});
})();
...
d.addListener(x,this._onMousewheel,this,true);
this.addListener(w,this._onResize,this);
},properties:{focusable:{refine:true,init:true},width:{refine:true,init:120},maxListHeight:{check:r,apply:n,nullable:true,init:200},format:{check:v,init:function(c){return this._defaultFormat(c);
...
},_applyMaxListHeight:function(H,I){this.getChildControl(z).setMaxHeight(H);
},getChildrenContainer:function(){return this.getChildControl(z);
},open:function(){var D=this.getChildControl(A);
...
}},_defaultFormat:function(J){var K=J?J.getLabel():y;
var L=J?J.getRich():false;

if(L){K=K.replace(/<[^>]+?>/g,y);
...
this.addListener(s,this.__rD,this);
},properties:{appearance:{refine:true,init:n}},members:{__rC:null,_createChildControlImpl:function(B){var C;

switch(B){case A:C=new qx.ui.core.Spacer();
this._add(C,{flex:1});
break;
case k:C=new qx.ui.basic.Atom(m);
C.setCenter(false);
C.setAnonymous(true);
this._add(C,{flex:1});
break;
...
},__rD:function(e){var E=e.getData()[0];
var D=this.getChildControl(l);

if(D.getSelection()[0]!=E){D.setSelection([E]);
}this.__rE();
this.__rF();
},__rE:function(){var Y=this.getChildControl(l).getSelection()[0];
var ba=this.getChildControl(k);
var X=Y?Y.getIcon():b;
X==null?ba.resetIcon():ba.setIcon(X);
},__rF:function(){var be=this.getChildControl(l).getSelection()[0];
var bf=this.getChildControl(k);
var bd=be?be.getLabel():b;
...
}this.setSelection([N[K]]);
e.stopPropagation();
e.preventDefault();
},_onKeyPress:function(e){var bb=e.getKeyIdentifier();

if(bb==z||bb==w){if(this.__rC){this.setSelection([this.__rC]);
this.__rC=null;
}this.toggle();
...
},_onListMouseDown:function(e){if(this.__rC){this.setSelection([this.__rC]);
this.__rC=null;
}},_onListChangeSelection:function(e){var F=e.getData();
var I=e.getOldData();
if(I&&I.length>0){I[0].removeListener(d,this.__rE,this);
I[0].removeListener(f,this.__rF,this);
}
if(F.length>0){var H=this.getChildControl(h);
var G=this.getChildControl(l);
var J=G.getSelectionContext();

if(H.isVisible()&&(J==r||J==v)){this.__rC=F[0];
}else{this.setSelection([F[0]]);
this.__rC=null;
}F[0].addListener(d,this.__rE,this);
F[0].addListener(f,this.__rF,this);
}else{this.resetSelection();
}},_onPopupChangeVisibility:function(e){var Q=this.getChildControl(h);

if(!Q.isVisible()){var S=this.getChildControl(l);
if(S.hasChildren()){S.setSelection(this.getSelection());
}}else{var P=Q.getLayoutLocation(this);
var U=qx.bom.Viewport.getHeight();
var T=P.top;
var V=U-P.bottom;
var R=T>V?T:V;
var W=this.getMaxListHeight();
var S=this.getChildControl(l);

if(W==null||W>R){S.setMaxHeight(R);
}else if(W<R){S.setMaxHeight(W);
