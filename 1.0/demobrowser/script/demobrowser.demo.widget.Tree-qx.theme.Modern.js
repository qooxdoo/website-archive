...
var bC=new qx.ui.layout.HBox();
bC.set({spacing:20});
var bB=new qx.ui.container.Composite(bC);
this.getRoot().add(bB);
var bz=this.getTree();
bB.add(bz);
var bA=this.getCommandFrame(bz);
bB.add(bA);
...
r.add(q);
return m;
},getCommandFrame:function(bh){var br=new qx.ui.groupbox.GroupBox(Y);
var bp=4;
br.setLayout(new qx.ui.layout.Grid(5,3));
var by=0;
br.add(new qx.ui.basic.Label(E).set({paddingTop:4}),{row:by,column:0});
var bv=new qx.ui.form.TextField().set({readOnly:true});
br.add(bv,{row:by++,column:1});
bh.addListener(y,function(e){var a=e.getData();

if(a.length>0){if(this.getSelectionMode()===w){bv.setValue(a.length+X);
}else{bv.setValue(a[0].getLabel());
}}else{bv.setValue(J);
}});
br.add(new qx.ui.core.Spacer(bp,bp),{row:by++,column:0});
br.add(new qx.ui.basic.Label(bd),{row:by,column:0});
var bl={"click":K,"dblclick":Q,"none":O};
var bj=new qx.ui.form.RadioGroup();

for(var bt in bl){var bk=new qx.ui.form.RadioButton(bl[bt]).set({value:bt==bh.getOpenMode()});
bk.setUserData(x,bt);
bj.add(bk);
br.add(bk,{row:by++,column:1});
}bj.addListener(y,function(e){bh.setOpenMode(e.getData()[0].getUserData(x));
});
br.add(new qx.ui.core.Spacer(bp,bp),{row:by++,column:0});
br.add(new qx.ui.basic.Label(P),{row:by,column:0});
var bn=new qx.ui.form.CheckBox(N);
br.add(bn,{row:by++,column:1});
bn.addListener(B,function(e){var bD=e.getData();
bh.setSelectionMode(bD?w:v);
});
var bo=new qx.ui.form.CheckBox(H);
br.add(bo,{row:by++,column:1});
bo.addListener(B,function(e){var bg=e.getData();
bh.setDragSelection(bg);

if(!bn.getValue()){bn.setValue(true);
}});
br.add(new qx.ui.core.Spacer(bp,bp),{row:by++,column:0});
br.add(new qx.ui.basic.Label(R),{row:by,column:0});
var bw=new qx.ui.form.CheckBox(bb);
br.add(bw,{row:by++,column:1});
bw.addListener(B,function(e){bh.setHideRoot(e.getData());
});
var bm=new qx.ui.form.CheckBox(L);
br.add(bm,{row:by++,column:1});
bm.addListener(B,function(e){bh.setRootOpenClose(e.getData());
});
br.add(new qx.ui.core.Spacer(bp,bp),{row:by++,column:0});
var bu=new qx.ui.form.Button(T);
br.add(bu,{row:by++,column:1});
bu.addListener(A,function(e){alert((""+bh.getItems()).replace(",","\n","g"));
});
var bi=new qx.ui.form.Button(I);
br.add(bi,{row:by++,column:1});
bi.addListener(A,function(e){alert((""+bh.getItems(true,false)).replace(",","\n","g"));
});
var bs=new qx.ui.form.Button(S);
br.add(bs,{row:by++,column:1});
bs.addListener(A,function(e){if(this.getSelectionMode()===v){alert(this.getSelection());
}else{alert(this.getSelection().toString().replace(",","\n","g"));
}},bh);
var bq=new qx.ui.form.Button(M);
br.add(bq,{row:by++,column:1});
var bx=true;
bq.addListener(A,function(e){bh.setHeight(bx?600:400);
bx=!bx;
});
return br;
}}});
