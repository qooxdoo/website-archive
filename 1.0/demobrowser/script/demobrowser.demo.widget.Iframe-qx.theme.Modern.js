...
qx.Class.define(i,{extend:qx.ui.embed.AbstractIframe,construct:function(C){arguments.callee.base.call(this,C);
var D=new qx.ui.layout.Grid();
D.setColumnFlex(0,1);
D.setRowFlex(0,1);
this._setLayout(D);
this._showChildControl(u);
},properties:{appearance:{refine:true,init:c},scrollbarX:{check:[w,s,g],init:w,themeable:true,apply:o},scrollbarY:{check:[w,s,g],init:w,themeable:true,apply:o},scrollbar:{group:[h,k]}},members:{__Ii:null,__Ij:null,_getIframeElement:function(){return this.getChildControl(u).getContentElement();
},_createChildControlImpl:function(a){var b;

switch(a){case u:b=new qx.ui.embed.Iframe(this.getSource());
b.addListener(n,this._onIframeLoad,this);
b.addListener(m,this._onIframeResize,this);
this._add(b,{row:0,column:0});
break;
case r:b=new qx.ui.core.scroll.ScrollBar(f);
b.setMinWidth(0);
b.exclude();
b.addListener(q,this._onScrollBarX,this);
this._add(b,{row:1,column:0});
break;
case v:b=new qx.ui.core.scroll.ScrollBar(l);
b.setMinHeight(0);
b.exclude();
b.addListener(q,this._onScrollBarY,this);
this._add(b,{row:0,column:1});
break;
case t:b=new qx.ui.core.Widget();
b.setWidth(0);
b.setHeight(0);
b.exclude();
...
var K=this._getIframeElement().getBody();

if(K){this._startIframeObserver();
...
},_addWheelListener:function(){try{var L=this._getIframeElement().getBody();
qx.bom.Element.addListener(L,d,this._onMouseWheel,this);
}catch(e){this._disableScollbars();
}},_onMouseWheel:function(e){var O=this._isChildControlVisible(v)&&this.getChildControl(v).isEnabled();

if(!O){return;
}var N=this.getChildControl(v,true);
N.scrollBySteps(e.getWheelDelta());
e.stop();
},_startIframeObserver:function(){if(this.__Ij){this._stopIframeObserver();
}var M=qx.event.Idle.getInstance();
this.__Ij=M.addListener(j,this._onIframeObserverInterval,this);
},_stopIframeObserver:function(){this.__Ii=null;

if(!this.__Ij){return;
}var X=qx.event.Idle.getInstance();
X.removeListenerById(this.__Ij);
},_onIframeObserverInterval:function(){var W=this._getIframeSize();

if(!W){this._disableScollbars();
return;
}
if(this.__Ii&&W.width==this.__Ii.width&&W.height==this.__Ii.height){return;
}this.__Ii=W;
this._preventIframeScrolling();
this._updateScrollbars();
},_preventIframeScrolling:function(){try{var z=this._getIframeElement().getDocument();
z.documentElement.style.overflow=p;
z.body.style.overflow=p;
}catch(e){this._disableScollbars();
}},_updateScrollbars:function(){var P=this.__Ii;
var S=this.getChildControl(u).getBounds();
var Q=this.getChildControl(u).getInnerSize();
...
if(T===w&&U===w){var R=P.width>Q.width;
var V=P.height>Q.height;
if((R||V)&&!(R&&V)){if(R){V=P.height>S.height;
}else if(V){R=P.width>S.width;
}}}else{var R=T===s;
var V=U===s;
if(P.width>(R?S.width:Q.width)&&T===w){R=true;
}
if(P.height>(R?S.height:Q.height)&&U===w){V=true;
}}this._configureScrollbar(r,R,Q.width,P.width);
this._configureScrollbar(v,V,Q.height,P.height);
this._updateCornerWidget();
},_getIframeSize:function(){try{var A=this._getIframeElement().getWindow();
var B={width:qx.bom.Document.getWidth(A),height:qx.bom.Document.getHeight(A)};
return B;
}catch(e){return null;
...
}},_configureScrollbar:function(E,F,G,H){if(!F){this._excludeChildControl(E);
return;
}var I=this._showChildControl(E);
