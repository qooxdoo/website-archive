...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.table.Table","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.table.Table-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"f3652b3c5871"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var v="_controls",u="icon/32/status/dialog-information.png",t="",s="_table",r="center",q="en",p="icon/16/apps/office-spreadsheet.png",o="OK",n="demobrowser.demo.table.TableDemo",m="_container",l="execute";
qx.Class.define(n,{extend:qx.application.Standalone,include:demobrowser.demo.table.MUtil,members:{__GH:null,main:function(){arguments.callee.base.call(this);
qx.locale.Manager.getInstance().setLocale(q);
this._container=new qx.ui.window.Window(this.getCaption(),p).set({width:600,height:400,contentPadding:[0,0,0,0],showClose:false,showMinimize:false});
...
},nextId:0,createRandomRows:function(f){var i=[];
var h=new Date().getTime();
var j=400*24*60*60*1000;

for(var k=0;k<f;k++){var g=new Date(h+Math.random()*j-j/2);
i.push([this.nextId++,Math.random()*10000,g,(Math.random()>0.5)]);
}return i;
},getCaption:function(){return t;
},createTable:function(){throw new Error("abstract method call");
},createControls:function(){return null;
},showDialog:function(a){if(!this.__GH){var d=this.__GH=new qx.ui.window.Window().set({modal:true,showMinimize:false,showMaximize:false,width:180,contentPadding:[10,10,10,10]});
d.moveTo(315,100);
var b=new qx.ui.layout.Grid(15,15);
b.setRowFlex(0,1);
b.setColumnFlex(1,1);
d.setLayout(b);
d.add(new qx.ui.basic.Image(u),{row:0,column:0});
d.add(new qx.ui.basic.Label().set({rich:true,allowGrowY:true}),{row:0,column:1});
var c=new qx.ui.form.Button(o).set({alignX:r,allowGrowX:false,padding:[2,10]});
c.addListener(l,function(e){d.close();
},this);
d.add(c,{row:1,column:0,colSpan:2});
}this.__GH.getChildren()[1].setValue(a);
this.__GH.open();
this.__GH.getChildren()[2].focus();
}},destruct:function(){this._disposeObjects(s,v,m);
}});
})();
(function(){var s="execute",r="changeValue",q="A date",p="Boolean",o="icon/22/actions/edit-undo.png",n="Change ID sort method",m="icon/22/status/dialog-information.png",l="Change row with ID 10",k="A number",j="icon/16/apps/office-calendar.png",I="Remove 5 rows",H="Add 10 rows",G="",F="demobrowser.demo.table.Table",E="_tableModel",D="Table",C="icon/22/actions/list-remove.png",B="Whether the the first visible row should ",A=", ",z="icon/22/actions/list-add.png",x="Selected rows:<br>",y="Keep first row",v="be rendered completely when scrolling.",w="Show selection",t="Demonstrate use of alternate sorting algorithm.",u="ID";
qx.Class.define(F,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return D;
},createTable:function(){var L=this.createRandomRows(50);
var K=this._tableModel=new qx.ui.table.model.Simple();
K.setColumns([u,k,q,p]);
K.setData(L);
K.setColumnEditable(1,true);
K.setColumnEditable(2,true);
K.setColumnSortable(3,false);
var M=new qx.ui.table.Table(K);
M.set({width:600,height:400,decorator:null});
M.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
var J=M.getTableColumnModel();
J.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
J.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(j,q));
return M;
},createControls:function(){var f=new qx.ui.toolbar.ToolBar();
var d,e,g;
e=new qx.ui.toolbar.Part();
f.add(e);
d=new qx.ui.toolbar.Button(l,o);
d.addListener(s,function(T){var U=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,U[0][i]);
}this.info("Row 10 changed");
},this);
e.add(d);
d=new qx.ui.toolbar.Button(H,z);
d.addListener(s,function(be){var bf=this.createRandomRows(10);
this._tableModel.addRows(bf);
this.info("10 rows added");
},this);
e.add(d);
d=new qx.ui.toolbar.Button(I,C);
d.addListener(s,function(N){var O=this._tableModel.getRowCount();
this._tableModel.removeRows(O-5,5);
this.info("5 rows removed");
},this);
e.add(d);
d=new qx.ui.toolbar.Button(w,m);
d.addListener(s,function(a){var b=[];
c.getSelectionModel().iterateSelection(function(P){b.push(P+G);
});
this.showDialog(x+b.join(A));
},this);
e.add(d);
e=new qx.ui.toolbar.Part();
f.add(e);
var c=this._table;
g=new qx.ui.toolbar.CheckBox(y);
g.set({value:c.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(B+v)});
g.addListener(r,function(h){c.setKeepFirstVisibleRowComplete(this.getValue());
},g);
e.add(g);
g=new qx.ui.toolbar.CheckBox(n);
g.set({value:c.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(t)});
g.addListener(r,function(Q){if(Q.getData()){var S=function(ba,bb){var bc=ba[arguments.callee.columnIndex];
...
var R=function(V,W){var X=V[arguments.callee.columnIndex];
var Y=W[arguments.callee.columnIndex];

if(X%2==1&&Y%2==0){return -1;
}
if(Y%2==1&&X%2==0){return 1;
}return (X<Y)?1:((X==Y)?0:-1);
};
c.getTableModel().setSortMethods(0,{ascending:S,descending:R});
}else{c.getTableModel().setSortMethods(0,null);
}},g);
e.add(g);
return f;
...
(function(){var z="px",y=".qooxdoo-table-cell-icon {",x="abstract",w="",v="qx.ui.table.cellrenderer.AbstractImage",u=" qooxdoo-table-cell-icon",t="<div></div>",s="'",r="no-repeat",q="}",k="  text-align:center;",p="inline-block",n="static",j="top",i="  padding-top:1px;",m="title='",l="string",o="-moz-inline-box";
...
if(!c.stylesheet){c.stylesheet=qx.bom.Stylesheet.createElement(y+k+i+q);
}},members:{__Gp:16,__Gq:16,_insetY:2,__Gr:null,_identifyImage:function(A){throw new Error("_identifyImage is abstract");
},_getImageInfos:function(f){var g=this._identifyImage(f);
if(g==null||typeof f==l){g={url:g,tooltip:null};
}
if(f.width&&f.height){var h={width:f.imageWidth,height:f.imageHeight};
}else{h=this.__Gs(g.url);
}g.width=h.width;
g.height=h.height;
return g;
},__Gs:function(C){var F=qx.util.ResourceManager.getInstance();
...
},createDataCellHtml:function(a,b){this.__Gr=this._getImageInfos(a);
return arguments.callee.base.call(this,a,b);
},_getCellClass:function(B){return arguments.callee.base.call(this)+u;
},_getContentHtml:function(H){var content=t;
if(this.__Gr.url){var content=qx.bom.element.Decoration.create(this.__Gr.url,r,{width:this.__Gr.width+z,height:this.__Gr.height+z,display:qx.bom.client.Engine.GECKO&&qx.bom.client.Engine.VERSION<1.9?o:p,verticalAlign:j,position:n});
}return content;
},_getCellAttributes:function(d){var e=this.__Gr.tooltip;
...
this.__Gt=qx.util.AliasManager.getInstance();
this.initIconTrue();
this.initIconFalse();
},properties:{iconTrue:{check:h,init:f,apply:g},iconFalse:{check:h,init:c,apply:b}},members:{__Gu:null,__Gv:false,__Gt:null,_applyIconTrue:function(l){this.__Gu=this.__Gt.resolve(l);
},_applyIconFalse:function(a){this.__Gv=this.__Gt.resolve(a);
...
switch(j.value){case true:k.url=this.__Gu;
break;
case false:k.url=this.__Gv;
break;
default:k.url=null;
break;
}return k;
