...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.data.Twitter-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"2213c8dcc9da"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var A="value",z="icon",y="model",x="text",w="qx.client",v="execute",u="source",t="wittemann",s="mshtml",r=".json",W="user.profile_image_url",V="http://twitter.com/statuses/user_timeline/",U="Enter",T="<span style='font-size: 20px'>Twitter</span>",S="<a target='_blank' href='http://twitter.com/home?status=Check%20out%20the%20%23qooxdoo%20twitter%20demo%20at%20http://bit.ly/Mc4Of'>Tweet This</a></span>",R="demobrowser.demo.data.store.Twitter",Q="keydown",P="Friends",O="demobrowser.demo.data.Twitter",N="selection[0]",H="<span style='font-size: 13px'>",I="Location: ",F="Details",G="Name: ",D="?callback=demobrowser.demo.data.store.Twitter.saveResult",E="User",B="http://twitter.com/statuses/friends_timeline.json",C="Postet with: ",J="user.name",K="user.location",M="Avatar: ",L="Message: ";
...
this.getRoot().add(g,{left:10,top:85});
g.setWidth(300);
var i=new qx.data.controller.List(null,g);
i.setDelegate(this);
i.setLabelPath(x);
if(!qx.core.Variant.isSet(w,s)){i.setIconPath(W);
}var m=new demobrowser.demo.data.store.Twitter(t);
m.bind(y,i,y);
var e=new qx.ui.form.Button(P);
this.getRoot().add(e,{left:10,top:55});
e.addListener(v,function(){m.setUrl(B);
},this);
var j=new qx.ui.form.Button(E);
this.getRoot().add(j,{left:90,top:55});
var b=new qx.ui.form.TextField(t);
this.getRoot().add(b,{left:140,top:55});
j.addListener(v,function(){var a=V+b.getValue()+r;

if(m.getUrl()==a){m.reload();
}else{m.setUrl(a);
}},this);
b.addListener(Q,function(o){if(o.getKeyIdentifier()==U){j.execute();
}},this);
var f=new qx.ui.groupbox.GroupBox(F);
this.getRoot().add(f,{left:320,top:60});
f.setWidth(300);
f.setHeight(220);
f.setLayout(new qx.ui.layout.Grid(0,5));
f.add(new qx.ui.basic.Label(G),{row:0,column:0});
f.add(new qx.ui.basic.Label(I),{row:1,column:0});
f.add(new qx.ui.basic.Label(L),{row:2,column:0});
f.add(new qx.ui.basic.Label(C),{row:3,column:0});
var name=new qx.ui.basic.Label();
f.add(name,{row:0,column:1});
var location=new qx.ui.basic.Label();
f.add(location,{row:1,column:1});
var k=new qx.ui.basic.Label();
k.setRich(true);
k.setWidth(150);
f.add(k,{row:2,column:1});
var n=new qx.ui.basic.Label();
n.setRich(true);
f.add(n,{row:3,column:1});
var c=new qx.data.controller.Object();
c.addTarget(name,A,J);
c.addTarget(location,A,K);
c.addTarget(k,A,x);
c.addTarget(n,A,u);

if(!qx.core.Variant.isSet(w,s)){f.add(new qx.ui.basic.Label(M),{row:4,column:0});
var h=new qx.ui.basic.Image();
f.add(h,{row:4,column:1});
c.addTarget(h,u,W);
}i.bind(N,c,y);
var d=new qx.ui.basic.Label();
d.setRich(true);
d.setWidth(260);
d.setValue(T);
...
l.setWidth(260);
l.setValue(H+S);
this.getRoot().add(l,{left:10,top:290});
},configureItem:function(ba){ba.setRich(true);
ba.getChildControl(z).setWidth(50);
ba.getChildControl(z).setHeight(50);
ba.getChildControl(z).setScale(true);
}}});
qx.Class.define(R,{extend:qx.data.store.Json,statics:{saveResult:function(bb){this.__FL=bb;
}},construct:function(X){var Y=V+X+r;
arguments.callee.base.call(this,Y);
},members:{_createRequest:function(p){var q=new qx.io.ScriptLoader();
p+=D;
q.load(p,function(bd){this.__FM();
},this);
},__FM:function(){var bc=demobrowser.demo.data.store.Twitter.__FL;

if(bc==undefined){this.setState("failed");
return;
