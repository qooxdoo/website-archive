...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.table.Table_Cell_Editor-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"38e73b9429ff"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var t="checkbox",s='Property',r="options",q="password",p="type",o='email',n='password',m='options',l='type',k='0',bj='Use only numbers!',bi='regExp',bh='[0-9]+',bg='newsletter',bf='admin',be='editable combobox',bd='secret',bc="demobrowser.demo.table.Table_Cell_Editor",bb='status',ba="validationFunc",A='1',B="regExp",y="required",z='Doubleclick on cell to edit value',w='Newsletter',x='Inactive',u='User Name',v='telephone',E="dataChanged",F='Status',N='Active',L='validationFunc',R='enter website',P="editable",V='role',T='username',H='jdoe',Y='Telephone',X='manager',W='enter valid telephone number',G='Waiting',J='Email',K='Role',M="Property editor table",O='2',Q='user',S='website',U='Password',C="email",D='Website',I='enter valid email';
qx.Class.define(bc,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return M;
},main:function(){arguments.callee.base.call(this);
this._container.resetWidth();
this._container.setResizable(false);
},createTable:function(){var bs=[[T,u,H,{'required':true}],[n,U,bd,{'type':n,'required':true}],[V,K,be,{'options':[bf,X,Q],'editable':true}],[bb,F,k,{'options':[[x,null,k],[N,null,A],[G,null,O]]}],[o,J,I,{'type':o}],[v,Y,W,{'regExp':bh,'failMsg':bj}],[S,D,R,{'validationFunc':function(a){if(a.search(/^http:/)!=-1){window.open(a);
}return a;
}}],[bg,w,true,{'type':t}]];
var br=function(c){var i=c.table;
var f=i.getTableModel();
var g=f.getRowData(c.row);
var j=g[3];
var h;

for(var e in j){switch(e){case p:switch(j[l]){case t:return new qx.ui.table.cellrenderer.Boolean;
case q:return new qx.ui.table.cellrenderer.Password;
}break;
case r:var h=new qx.ui.table.cellrenderer.Replace;
var d={};
j[m].forEach(function(bG){if(bG instanceof Array){d[bG[0]]=bG[2];
}});
h.setReplaceMap(d);
h.addReversedReplaceMap();
return h;
}}return new qx.ui.table.cellrenderer.Default();
};
var bq=new qx.ui.table.cellrenderer.Dynamic(br);
var bo=function(bv){var bA=bv.table;
var by=bA.getTableModel();
var bz=by.getRowData(bv.row);
var bC=bz[3];
var bx=new qx.ui.table.celleditor.TextField;
var bB=null;

for(var bw in bC){switch(bw){case r:if(bC.editable){bx=new qx.ui.table.celleditor.ComboBox();
}else{bx=new qx.ui.table.celleditor.SelectBox();
}bx.setListData(bC[m]);
break;
case P:break;
case p:switch(bC[l]){case q:bx=new qx.ui.table.celleditor.PasswordField;
break;
case t:bx=new qx.ui.table.celleditor.CheckBox;
break;
case C:bx.setValidationFunction(function(bH,bI){var bJ=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{2}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/;

if(bJ.test(bH)){return bH;
}alert("You did not enter a valid email address");
return bI;
});
break;
}break;
case B:bx.setValidationFunction(function(bD,bE){var bF=new RegExp(bC[bi]);
...
break;
case ba:bx.setValidationFunction(bC[L]);
break;
case y:bB=function(bt,bu){if(!bt){alert("You need to supply a value here");
return bu;
}return bt;
};
break;
...
var bn=new qx.ui.table.celleditor.Dynamic(bo);
var bl=new qx.ui.table.model.Simple();
bl.setColumns([s,s,z]);
var bk={tableColumnModel:function(b){return new qx.ui.table.columnmodel.Resize(b);
}};
var bm=new qx.ui.table.Table(bl,bk);
bm.setDecorator(null);
bm.setColumnVisibilityButtonVisible(false);
bm.setKeepFirstVisibleRowComplete(true);
bm.setStatusBarVisible(false);
bm.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var bp=bm.getTableColumnModel();
bp.setColumnVisible(0,false);
bp.getBehavior().setWidth(1,100);
bp.getBehavior().setWidth(2,300);
bl.setColumnEditable(2,true);
bp.setDataCellRenderer(2,bq);
bp.setCellEditorFactory(2,bn);
bm.getTableModel().setData(bs);
bm.getTableModel().addListener(E,function(event){if(!(event instanceof qx.event.type.Data)){return;
