...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.widget.HtmlArea-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"1af967fe6b64"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var w="demobrowser.demo.widget.HtmlArea",v="As an application developer you can <b>use the same API.</b></p>",u='<h1>About</h1><p>qooxdoo (pronounced [ku:ksdu:]) is a comprehensive and innovative Ajax application framework. Leveraging object-oriented JavaScript allows developers to build impressive cross-browser applications. No <acronym title="HyperText Markup Language">HTML</acronym>, <acronym title="Cascading Style Sheets">CSS</acronym> nor <acronym title="Document Object Model">DOM</acronym> knowledge is needed. qooxdoo includes a platform-independent development tool chain, a state-of-the-art <acronym title="Graphical User Interface">GUI</acronym> toolkit and an advanced client-server communication layer. It is Open Source under an <acronym title="GNU Lesser General Public License">LGPL</acronym>/<acronym title="Eclipse Public License">EPL</acronym> dual <a href="http://qooxdoo.org/license" class="wikilink1" title="license">license</a>.</p>',t="solid",s="<p>This UI-level widget can be used at RIA (Rich ",r="<h1>HtmlArea UI-level widget</h1>",q="Internet Applications) and inline applications.<br/>",p="border-main",o="so code-duplication is avoided.<br/>",n="The UI widget does use the low-level component internally ",l="<p>You can play around with this widget by hitting the ",m="buttons at the toolbar.</p>";
qx.Class.define(w,{extend:qx.application.Standalone,members:{__IA:null,main:function(){arguments.callee.base.call(this);
var H=r+s+q+n+o+v+l+m;
var C=new qx.ui.basic.Label(H);
C.set({rich:true,marginBottom:10});
var G=u;
var F=this.getRoot();
var E=new qx.ui.decoration.Single(1,t,p);
E.setWidthTop(0);
this.__IA=new qx.ui.embed.HtmlArea(G,null,"blank.html");
this.__IA.set({width:600,height:400,decorator:E});
var toolbar=this.__IK();
var B=new qx.ui.layout.VBox(0);
var D=new qx.ui.container.Composite(B);
D.add(C);
D.add(toolbar);
D.add(this.__IA);
F.add(D,{left:20,top:20});
},__IB:function(e){var k=window.prompt("Color (Hex): ","#");
this.setTextColor(k);
},__IC:function(e){var A=window.prompt("BgColor (Hex): ","#");
this.setTextBackgroundColor(A);
},__ID:function(e){var S={src:qx.util.ResourceManager.getInstance().toUri("demobrowser/demo/icons/htmlarea/qooxdoo_logo.png"),border:0,title:"qooxdoo logo",alt:"qooxdoo logo"};
this.insertImage(S);
},__IE:function(e){var I="<table border='1'>"+"<tbody>"+"<tr>"+"<td>First Row, First cell</td>"+"<td>First Row, Second cell</td>"+"</tr>"+"<tr>"+"<td>Second Row, First cell</td>"+"<td>Second Row, Second cell</td>"+"</tr>"+"</tbody>"+"</table>";
this.insertHtml(I);
},__IF:function(e){var b=new qx.ui.window.Window("Insert Hyperlink");
b.setLayout(new qx.ui.layout.VBox(20));
b.set({width:400,showMaximize:false,showMinimize:false});
var g=new qx.ui.form.TextField("http://");
b.add(g);
...
a.setWidth(60);
a.addListener("execute",function(e){this.insertHyperLink(g.getValue());
b.close();
},this);
f.add(a);
...
b.open();
this.saveRange();
},__IG:function(e){var R=window.prompt("HTML Code:","");
this.insertHtml(R);
},__IH:function(){var K=new qx.ui.form.SelectBox;
K.set({toolTipText:"Change Font Family",focusable:false,keepFocus:true,width:120,height:16,margin:[4,0]});
K.add(new qx.ui.form.ListItem(""));
var L=["Tahoma","Verdana","Times New Roman","Arial","Arial Black","Courier New","Courier","Georgia","Impact","Comic Sans MS","Lucida Console"];
var J;

for(var i=0,j=L.length;i<j;i++){J=new qx.ui.form.ListItem(L[i]);
J.set({focusable:false,keepFocus:true,font:qx.bom.Font.fromString("12px "+L[i])});
K.add(J);
}K.addListener("changeSelection",function(e){var x=e.getData()[0].getLabel();

if(x!=""){this.setFontFamily(x);
K.setSelection([K.getChildren()[0]]);
}},this.__IA);
return K;
},__II:function(){var z=new qx.ui.form.SelectBox;
z.set({toolTipText:"Change Font Size",focusable:false,keepFocus:true,width:50,height:16,margin:[4,0]});
z.add(new qx.ui.form.ListItem(""));
var y;

for(var i=1;i<=7;i++){y=new qx.ui.form.ListItem(i+"");
y.set({focusable:false,keepFocus:true});
z.add(y);
}z.addListener("changeSelection",function(e){var h=e.getData()[0].getLabel();

if(h!=""){this.setFontSize(h);
z.setSelection([z.getChildren()[0]]);
}},this.__IA);
return z;
},__IJ:function(){return [{bold:{text:"Format Bold",image:"qx/icon/Oxygen/16/actions/format-text-bold.png",action:this.__IA.setBold},italic:{text:"Format Italic",image:"qx/icon/Oxygen/16/actions/format-text-italic.png",action:this.__IA.setItalic},underline:{text:"Format Underline",image:"qx/icon/Oxygen/16/actions/format-text-underline.png",action:this.__IA.setUnderline},strikethrough:{text:"Format Strikethrough",image:"qx/icon/Oxygen/16/actions/format-text-strikethrough.png",action:this.__IA.setStrikeThrough},removeFormat:{text:"Remove Format",image:"qx/icon/Oxygen/16/actions/edit-clear.png",action:this.__IA.removeFormat}},{alignLeft:{text:"Align Left",image:"qx/icon/Oxygen/16/actions/format-justify-left.png",action:this.__IA.setJustifyLeft},alignCenter:{text:"Align Center",image:"qx/icon/Oxygen/16/actions/format-justify-center.png",action:this.__IA.setJustifyCenter},alignRight:{text:"Align Right",image:"qx/icon/Oxygen/16/actions/format-justify-right.png",action:this.__IA.setJustifyRight},alignJustify:{text:"Align Justify",image:"qx/icon/Oxygen/16/actions/format-justify-fill.png",action:this.__IA.setJustifyFull}},{fontFamily:{custom:this.__IH},fontSize:{custom:this.__II},fontColor:{text:"Set Text Color",image:"demobrowser/demo/icons/htmlarea/format-text-color.png",action:this.__IB},textBackgroundColor:{text:"Set Text Background Color",image:"demobrowser/demo/icons/htmlarea/format-fill-color.png",action:this.__IC}},{indent:{text:"Indent More",image:"qx/icon/Oxygen/16/actions/format-indent-more.png",action:this.__IA.insertIndent},outdent:{text:"Indent Less",image:"qx/icon/Oxygen/16/actions/format-indent-less.png",action:this.__IA.insertOutdent}},{insertImage:{text:"Insert Image",image:"qx/icon/Oxygen/16/actions/insert-image.png",action:this.__ID},insertTable:{text:"Insert Table",image:"demobrowser/demo/icons/htmlarea/insert-table.png",action:this.__IE},insertLink:{text:"Insert Link",image:"qx/icon/Oxygen/16/actions/insert-link.png",action:this.__IF},insertHTML:{text:"Insert HTML Code",image:"demobrowser/demo/icons/htmlarea/insert-text.png",action:this.__IG},insertHR:{text:"Insert Horizontal Ruler",image:"demobrowser/demo/icons/htmlarea/insert-horizontal-rule.png",action:this.__IA.insertHorizontalRuler}},{ol:{text:"Insert Ordered List",image:"demobrowser/demo/icons/htmlarea/format-list-ordered.png",action:this.__IA.insertOrderedList},ul:{text:"Inserted Unordered List",image:"demobrowser/demo/icons/htmlarea/format-list-unordered.png",action:this.__IA.insertUnorderedList}},{undo:{text:"Undo Last Change",image:"qx/icon/Oxygen/16/actions/edit-undo.png",action:this.__IA.undo},redo:{text:"Redo Last Undo Step",image:"qx/icon/Oxygen/16/actions/edit-redo.png",action:this.__IA.redo}}];
},__IK:function(){var toolbar=new qx.ui.toolbar.ToolBar;
toolbar.setDecorator("main");
var M;
var N=this.__IJ();

for(var i=0,j=N.length;i<j;i++){var O=new qx.ui.toolbar.Part;
toolbar.add(O);

for(var P in N[i]){var Q=N[i][P];

if(Q.custom){M=Q.custom.call(this);
}else{M=new qx.ui.toolbar.Button(null,Q.image);
M.set({focusable:false,keepFocus:true,center:true,toolTipText:Q.text?Q.text:""});
M.addListener("execute",Q.action,this.__IA);
}O.add(M);
}}return toolbar;
}}});
})();
...
(function(){var B="qx.event.type.Data",A="Boolean",z="qx.event.type.Event",y="display",x="mousedown",w="none",v="mouseup",u="losecapture",t="xhtml",s="_applyUseUndoRedo",bj="height",bi="_applyMessengerMode",bh=")",bg="InsertParagraphOnLinebreak",bf="repeat",be="qx.client",bd="div",bc="Function",bb="_applyInsertParagraphOnLinebreak",ba="PostProcess",I="width",J="_applyInsertLinebreakOnCtrlEnter",G="ContentType",H="px",E="url(",F="String",C="htmlarea",D="MessengerMode",K="zIndex",L="UseUndoRedo",R="top",Q="_applyContentType",T="qx.ui.embed.HtmlArea",S="mshtml",V="block",U="position",N="InsertLinebreakOnCtrlEnter",Y="__IM",X="__IO",W="left",M="qx/static/blank.gif",O="_applyPostProcess",P="absolute";
qx.Class.define(T,{extend:qx.ui.core.Widget,construct:function(bm,bn,bo){arguments.callee.base.call(this);
this._setLayout(new qx.ui.layout.Grow);
this.__IP();
this.__IL={content:bm,styleInfo:bn,source:bo};
qx.event.Registration.addListener(document.body,x,this.block,this,true);
qx.event.Registration.addListener(document.body,v,this.release,this,true);
qx.event.Registration.addListener(document.body,u,this.release,this,true);
this.__IM=this._createBlockerElement();
this.getContainerElement().add(this.__IM);
this.__IN={};
},events:{"loadingError":B,"messengerContent":B,"cursorContext":B,"ready":z,"readyAfterInvalid":z,"focused":z,"focusOut":z,"contextmenu":B,"undoRedoState":B},statics:{isBlockNode:function(bJ){return qx.bom.htmlarea.HtmlArea.isBlockNode(bJ);
},isParagraphParent:function(bI){return qx.bom.htmlarea.HtmlArea.isParagraphParent(bI);
}},properties:{contentType:{check:F,init:t,apply:Q},messengerMode:{check:A,init:false,apply:bi},insertParagraphOnLinebreak:{check:A,init:true,apply:bb},insertLinebreakOnCtrlEnter:{check:A,init:true,apply:J},postProcess:{check:bc,nullable:true,init:null,apply:O},useUndoRedo:{check:A,init:true,apply:s},appearance:{refine:true,init:C}},members:{__IO:null,__IN:null,__IM:null,__IL:null,_applyContentType:function(bx,by){if(this.__IO!=null){this.__IO.setContentType(bx);
}else{this.__IN[G]=bx;
}},_applyMessengerMode:function(i,j){if(this.__IO!=null){this.__IO.setMessengerMode(i);
}else{this.__IN[D]=i;
}},_applyInsertParagraphOnLinebreak:function(bp,bq){if(this.__IO!=null){this.__IO.setInsertParagraphOnLinebreak(bp);
}else{this.__IN[bg]=bp;
}},_applyInsertLinebreakOnCtrlEnter:function(bu,bv){if(this.__IO!=null){this.__IO.setInsertLinebreakOnCtrlEnter(bu);
}else{this.__IN[N]=bu;
}},_applyPostProcess:function(b,c){if(this.__IO!=null){this.__IO.setPostProcess(b);
}else{this.__IN[ba]=b;
}},_applyUseUndoRedo:function(q,r){if(this.__IO!=null){this.__IO.setUseUndoRedo(q);
}else{this.__IN[L]=q;
}},_createBlockerElement:function(){var bK=new qx.html.Element(bd);
bK.setStyle(K,20);
bK.setStyle(U,P);
bK.setStyle(y,w);
if(qx.core.Variant.isSet(be,S)){bK.setStyles({backgroundImage:E+qx.util.ResourceManager.getInstance().toUri(M)+bh,backgroundRepeat:bf});
}return bK;
},__IP:function(){this.addListenerOnce("appear",this.__IQ);
},__IQ:function(){var bw=this.getContentElement().getDomElement();
this.__IO=new qx.bom.htmlarea.HtmlArea(bw,this.__IL.content,this.__IL.styleInfo,this.__IL.source);
this.__IR();
this.__IS();

if(qx.core.Variant.isSet("qx.client","gecko")){this.__IV();
}this.addListener("appear",this.forceEditable);
},__IR:function(){for(var o in this.__IN){this.__IO["set"+o](this.__IN[o]);
}},__IS:function(){this.__IO.addListener("ready",this.__IT,this);
this.__IO.addListener("readyAfterInvalid",this.__IT,this);
this.__IO.addListener("focused",this.__IT,this);
this.__IO.addListener("focusOut",this.__IT,this);
this.__IO.addListener("loadingError",this.__IU,this);
this.__IO.addListener("cursorContext",this.__IU,this);
this.__IO.addListener("contextmenu",this.__IU,this);
this.__IO.addListener("undoRedoState",this.__IU,this);
this.__IO.addListener("messengerContent",this.__IU,this);
},__IT:function(e){var a=e.clone();
this.fireEvent(a.getType());
},__IU:function(e){var k=e.clone();
this.fireDataEvent(k.getType(),e.getData());
},__IV:function(){var self=this;
var bH=this.getContainerElement().getDomElement();
qx.bom.Event.addNativeListener(bH,"DOMNodeRemoved",qx.event.GlobalError.observeMethod(function(e){self.__IO.invalidateEditor();
}));
},renderLayout:function(bA,top,bB,bC){arguments.callee.base.call(this,bA,top,bB,bC);
var bE=H;
var bD=this.getInsets();
this.__IM.setStyle(W,bD.left+bE);
this.__IM.setStyle(R,bD.top+bE);
this.__IM.setStyle(I,(bB-bD.left-bD.right)+bE);
this.__IM.setStyle(bj,(bC-bD.top-bD.bottom)+bE);
},getIframeObject:function(){return this.__IO!=null?this.__IO.getIframeObject():null;
},getCommandManager:function(){return this.__IO!=null?this.__IO.getCommandManager():null;
},setValue:function(bz){if(this.__IO!=null){this.__IO.setValue(bz);
}},getValue:function(){return this.__IO!=null?this.__IO.getValue():null;
},getComputedValue:function(l){return this.__IO!=null?this.__IO.getHtml(l):null;
},getCompleteHtml:function(){return this.__IO!=null?this.__IO.getCompleteHtml():null;
},getContentDocument:function(){return this.__IO!=null?this.__IO.getContentDocument():null;
},getContentBody:function(){return this.__IO!=null?this.__IO.getContentBody():null;
},getContentWindow:function(){return this.__IO!=null?this.__IO.getContentWindow():null;
},getWords:function(bt){return this.__IO!=null?this.__IO.getWords(bt):null;
},getWordsWithElement:function(){return this.__IO!=null?this.__IO.getWordsWithElement():null;
},getTextNodes:function(){return this.__IO!=null?this.__IO.getTextNodes():null;
},isReady:function(){return this.__IO!=null?this.__IO.isReady():false;
},forceEditable:function(){if(this.__IO!=null){this.__IO.forceEditable();
}},isLoaded:function(){return this.__IO!=null?this.__IO.isLoaded():false;
},setEditable:function(h){if(this.__IO!=null){this.__IO.setEditable(h);
}},getEditable:function(){return this.__IO!=null?this.__IO.getEditable():false;
},isEditable:function(){return this.__IO!=null?this.__IO.isEditable():false;
},insertHtml:function(bG){return this.__IO!=null?this.__IO.insertHtml(bG):false;
},removeFormat:function(){return this.__IO!=null?this.__IO.removeFormat():false;
},setBold:function(){return this.__IO!=null?this.__IO.setBold():false;
},setItalic:function(){return this.__IO!=null?this.__IO.setItalic():false;
},setUnderline:function(){return this.__IO!=null?this.__IO.setUnderline():false;
},setStrikeThrough:function(){return this.__IO!=null?this.__IO.setStrikeThrough():false;
},setFontSize:function(bF){return this.__IO!=null?this.__IO.setFontSize(bF):false;
},setFontFamily:function(p){return this.__IO!=null?this.__IO.setFontFamily(p):false;
},setTextColor:function(m){return this.__IO!=null?this.__IO.setTextColor(m):false;
},setTextBackgroundColor:function(br){return this.__IO!=null?this.__IO.setTextBackgroundColor(br):false;
},setJustifyLeft:function(){return this.__IO!=null?this.__IO.setJustifyLeft():false;
},setJustifyCenter:function(){return this.__IO!=null?this.__IO.setJustifyCenter():false;
},setJustifyRight:function(){return this.__IO!=null?this.__IO.setJustifyRight():false;
},setJustifyFull:function(){return this.__IO!=null?this.__IO.setJustifyFull():false;
},insertIndent:function(){return this.__IO!=null?this.__IO.insertIndent():false;
},insertOutdent:function(){return this.__IO!=null?this.__IO.insertOutdent():false;
},insertOrderedList:function(){return this.__IO!=null?this.__IO.insertOrderedList():false;
},insertUnorderedList:function(){return this.__IO!=null?this.__IO.insertUnorderedList():false;
},insertHorizontalRuler:function(){return this.__IO!=null?this.__IO.insertHorizontalRuler():false;
},insertImage:function(n){return this.__IO!=null?this.__IO.insertImage(n):false;
},insertHyperLink:function(bl){return this.__IO!=null?this.__IO.insertHyperLink(bl):false;
},removeBackgroundColor:function(){return this.__IO!=null?this.__IO.removeBackgroundColor():false;
},setBackgroundColor:function(bk){return this.__IO!=null?this.__IO.setBackgroundColor(bk):false;
},removeBackgroundImage:function(){return this.__IO!=null?this.__IO.removeBackgroundImage():false;
},setBackgroundImage:function(d,f,g){return this.__IO!=null?this.__IO.setBackgroundImage(d,f,g):false;
},selectAll:function(){return this.__IO!=null?this.__IO.selectAll():false;
},undo:function(){return this.__IO!=null?this.__IO.undo():false;
},redo:function(){return this.__IO!=null?this.__IO.redo():false;
},resetHtml:function(){if(this.__IO!=null){this.__IO.resetHtml();
}},getHtml:function(bs){return this.__IO!=null?this.__IO.getHtml(bs):null;
},containsOnlyPlaceholder:function(){return this.__IO!=null?this.__IO.containsOnlyPlaceHolder():false;
},getContextInformation:function(){return this.__IO!=null?this.__IO.getContextInformation():null;
},getSelection:function(){return this.__IO!=null?this.__IO.getSelection():null;
},getSelectedText:function(){return this.__IO!=null?this.__IO.getSelectedText():null;
},getSelectedHtml:function(){return this.__IO!=null?this.__IO.getSelectedHtml():null;
},clearSelection:function(){if(this.__IO!=null){this.__IO.clearSelection();
}},getRange:function(){return this.__IO.getRange();
},saveRange:function(){this.__IO.saveRange();
},getSavedRange:function(){return this.__IO.getSavedRange();
},resetSavedRange:function(){this.__IO.resetSavedRange();
},getFocusNode:function(){return this.__IO!=null?this.__IO.getFocusNode():null;
},block:function(){this.__IM.setStyle(y,V);
},release:function(){this.__IM.setStyle(y,w);
}},destruct:function(){this._disposeObjects(Y,X);
this.__IN=this.__IL=null;
qx.event.Registration.removeListener(document.body,x,this.block,this,true);
qx.event.Registration.removeListener(document.body,v,this.release,this,true);
qx.event.Registration.removeListener(document.body,u,this.release,this,true);
}});
})();
(function(){var eV="qx.client",eU="",eT="mshtml",eS="qx.event.type.Data",eR="enter",eQ="qx.event.type.Event",eP=" ",eO="gecko",eN='"',eM="Boolean",dU="qx.bom.htmlarea.HtmlArea.debug",dT="on",dS="old_id",dR="end",dQ="redo",dP="i",dO="opera",dN="backspace",dM="inserthtml",dL="down",fd="100%",fe="string",fb="k",fc=":",eY='redo',fa="y",eW="g",eX=";",ff="undo",fg="pageup",eu="pagedown",et="u",ew="right",ev="backgroundimage",ez="left",ey="up",eB="None",eA="control",es="delete",er="'",cB=">",cC="b",cD="backgroundcolor",cE="contextmenu",cF="webkit",cG="z",cH="-",cI="li",cJ="xhtml",cK='undo',fu="div",ft="textbackgroundcolor",fs="Function",fr="br",fy="home",fx="<",fw="focused",fv="load",fA="setUnderline",fz="<br /><div id='placeholder'></div>",dk="<!--",dl="transparent",di="'>",dj="insertimage",dp='bold',dq="&copy;",dm="bold",dn="mouseup",dg=" />",dh="id",cS="style",cR="setBold",cU="P",cT="<P>&nbsp;</P>",cO="</",cN="qx/static/blank.html",cQ="&gt;",cP='="',cM="X",cL='</head>',dv='<head>',dw="#",dx="textcolor",dy="a",dr="&amp;",ds="removeformat",dt="ready",du="<p>&nbsp;</p>",dz="__Di",dA="italic",dd="outline",dc="insertorderedlist",db="> ",da="hideFocus",cY="keypress",cX="none",cW="S",cV="fontsize",df="setItalic",de="body",dB="focusOut",dC="justifyleft",dD="click",dE="underline",dF="placeholder",dG="Control",dH=" { ",dI='italic',dJ="styleWithCSS",dK='</style>',dY="off",dX="focus",dW="&quot;",dV="STYLE",ed="justifyfull",ec='underline',eb="inserthyperlink",ea="<br />",ef='<style type="text/css">',ee="<br>",en="justifycenter",eo="useCSS",ek="object",em="strikethrough",ei="blur",ej="indent",eg="true",eh="1.9",ep=' style="',eq="messengerContent",eF="insertunorderedlist",eE="Text",eH="&lt;",eG="inserthorizontalrule",eJ='</body></html>',eI='<br/><div class="placeholder"></div>',eL="selectall",eK="String",eD=" SCRIPT STYLE DIV SPAN TR TD TBODY TABLE EM STRONG FONT A P B I U STRIKE H1 H2 H3 H4 H5 H6 ",eC="justifyright",fn="//text()[string-length(normalize-space(.))>0]",fo="outdent",fp="fontfamily",fq="<body style='",fj="qx.bom.htmlarea.HtmlArea",fk="keydown",fl="keyup",fm="loadingError",fh="focusout",fi="$",cA="-->",cz=" }",cy="Off",cx="mshtml|opera";
qx.Class.define(fj,{extend:qx.core.Object,construct:function(bp,bq,br,bs){arguments.callee.base.call(this);
var bu=bs||qx.util.ResourceManager.getInstance().toUri(cN);
this.__DC(bp);
this.__DD();
this._createAndAddIframe(bu);
this._addIframeLoadListener();
this.__Da=qx.bom.htmlarea.HtmlArea.__Dj(br);
this.__Db=qx.lang.Function.bind(this._handleFocusEvent,this);
this.__Dc=qx.lang.Function.bind(this._handleBlurEvent,this);
this.__Dd=qx.lang.Function.bind(this._handleFocusOutEvent,this);
this.__De=qx.lang.Function.bind(this._handleMouseUpOnBody,this);
this.__Df=qx.lang.Function.bind(this._handleMouseUpOnDocument,this);
this.__Dg=qx.lang.Function.bind(this._handleContextMenuEvent,this);
if(typeof bq===fe){this.__Dh=bq;
}if(qx.core.Variant.isSet(eV,dO)){var bt=qx.dom.Node.getWindow(bp);
var bv=qx.event.Registration.getManager(bt).getHandler(qx.event.handler.Keyboard);
bv._keyCodeToIdentifierMap[35]=dw;
bv._keyCodeToIdentifierMap[45]=cH;
bv._keyCodeToIdentifierMap[112]=cU;
bv._keyCodeToIdentifierMap[115]=cW;
bv._keyCodeToIdentifierMap[120]=cM;
}this.__Di=this.__DM();
},events:{"load":eQ,"loadingError":eS,"messengerContent":eS,"cursorContext":eS,"ready":eQ,"readyAfterInvalid":eQ,"focused":eQ,"focusOut":eQ,"contextmenu":eS,"undoRedoState":eS},statics:{simpleLinebreak:ee,EMPTY_DIV:"<div></div>",GetWordsRegExp:/([^\u0000-\u0040\u005b-\u005f\u007b-\u007f]|['])+/g,CleanupWordsRegExp:/[\u0000-\u0040]/gi,hotkeyInfo:{bold:{method:cR},italic:{method:df},underline:{method:fA},undo:{method:ff},redo:{method:dQ}},__Dj:function(U){if(U==null||U==eU){return eU;
}else if(typeof U==ek){var V=eU;

for(var i in U){V+=i+dH+U[i]+cz;
}return V;
}else{return U;
}},__Dk:function(bc){var bd={};
var a=bc.split(eX);
var i;

for(i=0;i<a.length;i++){var be=a[i],bf=be.indexOf(fc);

if(bf===-1){continue;
}var name=qx.lang.String.trim(be.substring(0,bf));
var bg=qx.lang.String.trim(be.substring(bf+1,be.length));

if(name&&bg){bd[name]=bg;
}}return bd;
},__Dl:function(I,J,K,L){var Q=[];

switch(I.nodeType){case 1:case 11:var i;
var O=I.tagName.toLowerCase();
var S={};
var M={};
var closed=(!(I.hasChildNodes()||qx.bom.htmlarea.HtmlArea.__Dm(I)));

if(J){if(qx.core.Variant.isSet(eV,eT)){if(O==fu&&I.className&&I.className==dF){for(i=I.firstChild;i;i=i.nextSibling){Q.push(qx.bom.htmlarea.HtmlArea.__Dl(i,true,K,L));
}return Q.join(eU);
}}var R=I.attributes;
var P=R.length;
var a;
...
if(name!=cS){if(qx.bom.client.Engine.MSHTML){if(name==dh&&I.getAttribute(dS)){T=I.getAttribute(dS);
}else if(!isNaN(T)){T=I.getAttribute(name);
}else{T=a.nodeValue;
}}else{T=a.nodeValue;
}}else{T=I.style.cssText;
}
if(/(_moz|^$)/.test(T)){continue;
}if(name==dS){continue;
}if(!T){continue;
}if(name.charAt(0)===fi){continue;
}S[name]=T;
}if(S.style!==undefined){M=qx.bom.htmlarea.HtmlArea.__Dk(S.style);
delete S.style;
}if(L){var N={domElement:I,tag:O,attributes:S,styles:M};
L(N);
N.domElement=null;
O=N.tag;
}if(O){Q.push(fx,O);
for(var name in S){var T=S[name];
Q.push(eP,name,cP,T.toString().replace(new RegExp(eN,eW),er),eN);
}if(!qx.lang.Object.isEmpty(M)){Q.push(ep);

for(var name in M){var T=M[name];
Q.push(name,fc,T.toString().replace(new RegExp(eN,eW),er),eX);
}Q.push(eN);
}Q.push(closed?dg:cB);
}}for(i=I.firstChild;i;i=i.nextSibling){Q.push(qx.bom.htmlarea.HtmlArea.__Dl(i,true,K,L));
}if(J&&!closed&&O){Q.push(cO,O,cB);
}break;
case 3:Q.push(K?I.data:qx.bom.htmlarea.HtmlArea.__Dn(I.data));
break;
case 8:Q.push(dk,I.data,cA);
break;
}return Q.join(eU);
},closingTags:eD,__Dm:function(H){return (qx.bom.htmlarea.HtmlArea.closingTags.indexOf(eP+H.tagName+eP)!=-1);
},__Dn:function(s){s=s.replace(/&/ig,dr);
s=s.replace(/</ig,eH);
s=s.replace(/>/ig,cQ);
s=s.replace(/\x22/ig,dW);
s=s.replace(/\xA9/ig,dq);
return s;
},isBlockNode:function(G){if(!qx.dom.Node.isElement(G)){return false;
}G=qx.dom.Node.getName(G);
return /^(body|form|textarea|fieldset|ul|ol|dl|li|div|p|h[1-6]|quote|pre|table|thead|tbody|tfoot|tr|td|th|iframe|address|blockquote)$/.test(G);
},isParagraphParent:function(cc){if(!qx.dom.Node.isElement(cc)){return false;
}cc=qx.dom.Node.getName(cc);
return /^(body|td|th|caption|fieldset|div)$/.test(cc);
}},properties:{contentType:{check:eK,init:cJ},messengerMode:{check:eM,init:false},insertParagraphOnLinebreak:{check:eM,init:true},insertLinebreakOnCtrlEnter:{check:eM,init:true},postProcess:{check:fs,nullable:true,init:null},useUndoRedo:{check:eM,init:true}},members:{__Do:null,__Dp:false,__Dq:false,__Dr:false,__Ds:false,__Dt:false,__Di:null,__Du:null,__Dv:null,__Dw:null,__Dx:null,__Db:null,__Dc:null,__Dd:null,__De:null,__Df:null,__Dg:null,__Da:null,__Dy:null,__Dz:null,__DA:false,__DB:false,__DC:function(gE){if(qx.dom.Node.isElement(gE)&&qx.dom.Node.isNodeName(gE,"div")){this.__Do=gE;
}},_createAndAddIframe:function(go){this.__Dx=qx.bom.Iframe.create();
qx.bom.Iframe.setSource(this.__Dx,go);
if(qx.core.Variant.isSet(eV,eT)){qx.bom.element.Attribute.set(this.__Dx,da,eg);
}else{qx.bom.element.Style.set(this.__Dx,dd,cX);
}qx.bom.element.Style.setStyles(this.__Dx,{width:fd,height:fd});
qx.dom.Element.insertBegin(this.__Dx,this.__Do);
},_getIframeDocument:function(){return qx.bom.Iframe.getDocument(this.__Dx);
},_getIframeWindow:function(){return qx.bom.Iframe.getWindow(this.__Dx);
},_addIframeLoadListener:function(){qx.event.Registration.addListener(this.__Dx,fv,this._loaded,this);
},__DD:function(){this.__Dy={"xhtml":{doctype:'<!'+'DOCTYPE html PUBLIC "-/'+'/W3C/'+'/DTD XHTML 1.0 Transitional/'+'/EN" "http:/'+'/www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">',html:'<html xmlns="http:/'+'/www.w3.org/1999/xhtml" xml:lang="en" lang="en">',meta:'<title></title><meta http-equiv="Content-type" content="text/html; charset=UTF-8" />',style:qx.core.Variant.select("qx.client",{"mshtml":'html { margin:0px; padding:0px; } '+'body { font-size: 100.01%; font-family:Verdana, Geneva, Arial, Helvetica, sans-serif; width:100%; height:100%; background-color:transparent; overflow:auto; background-image:none; margin:0px; padding:5px; } ',"default":'html { width:100%; height:100%; margin:0px; padding:0px; overflow-y:auto; overflow-x:auto; } '+'body { font-size:100.01%; font-family:Verdana, Geneva, Arial, Helvetica, sans-serif; background-color:transparent; overflow:visible; background-image:none; margin:0px; padding:5px; } '}),contentStyle:'p { margin:0px; padding:0px; }',body:'<body>',footer:'</body></html>'}};
},__Dh:eU,getIframeObject:function(){return this.__Dx;
},getCommandManager:function(){return this.__Di;
},setValue:function(bA){if(typeof bA===fe){this.__Dh=bA;
var bB=this._getIframeDocument();

if(bB&&bB.body){bB.body.innerHTML=bA;
}}},getValue:function(){return this.__Dh;
},getComputedValue:function(fD){return this.getHtml(fD);
},getCompleteHtml:function(){var gi=this.__Dy[this.getContentType()];
var gh=gi.html+dv+gi.meta+ef+gi.contentStyle+dK+cL;
gh+=fq+this.__DE()+di;
gh+=this.getHtml()+eJ;
return gh;
},__DE:function(){var d=["backgroundColor","backgroundImage","backgroundRepeat","backgroundPosition","fontFamily","fontSize","marginTop","marginBottom","marginLeft","marginRight","paddingTop","paddingBottom","paddingLeft","paddingRight"];
var c=qx.bom.element.Style;
var b=this.getContentBody();
var h={};
var g,k;
var f=qx.bom.client.Engine.MSHTML?2:1;

for(var i=0,j=d.length;i<j;i++){g=d[i];
k=c.get(b,g,f);

if(k!==undefined&&k!=""){h[g]=k;
}}return qx.bom.element.Style.compile(h);
},getContentDocument:function(){if(this.__Dp){return this._getIframeDocument();
}},getContentBody:function(){if(this.__Dp){return this._getIframeDocument().body;
}},getContentWindow:function(){if(this.__Dp){return this._getIframeWindow();
}},getWords:function(fO){if(!fO){fO=this.getContentBody();
}
if(!fO){return [];
}var fQ=fO.cloneNode(true);
fQ.innerHTML=fQ.innerHTML.replace(/>/gi,db);
fQ.innerHTML=fQ.innerHTML.replace(/\n/gi,eP);
fQ.innerHTML=fQ.innerHTML.replace(/<!--.*-->/gi,eU);
var fP=qx.core.Variant.isSet(eV,cx)?fQ.innerText:fQ.textContent;
var fR=fP.match(qx.bom.htmlarea.HtmlArea.GetWordsRegExp);
return !fR?[]:fR;
},getWordsWithElement:function(){var bR=this.getTextNodes();
var bV={};
var i,j,bW,bS,bQ;

for(var i=0,bU=bR.length;i<bU;++i){bS=bR[i];
bW=bS.nodeValue.split(eP);

for(var j=0,bT=bW.length;j<bT;++j){bQ=this._cleanupWord(bW[j]);

if(bQ!=null&&bQ.length>1){if(!bV[bQ]){bV[bQ]=[];
}bV[bQ].push(bS);
}}}return bV;
},_cleanupWord:function(cb){if(!cb){return null;
}return cb.replace(qx.bom.htmlarea.HtmlArea.CleanupWordsRegExp,eU);
},getTextNodes:function(){return this._fetchTextNodes(this.getContentBody());
},_fetchTextNodes:function(y){var A=[];
var z;
if(y.hasChildNodes){for(var i=0;i<y.childNodes.length;i++){z=this._fetchTextNodes(y.childNodes[i]);
qx.lang.Array.append(A,z);
}}if(y.nodeType==3){if(y.nodeValue.length>1){A.push(y);
}}return A;
},__DF:0,__DG:function(){var gn=this._getIframeDocument();
if(!gn){this.__DF++;

if(this.__DF>5){this.error('cant load HtmlArea. Document is not available. '+gn);
this.fireDataEvent("loadingError");
}else{{};
qx.event.Timer.once(function(){this.__DG();
},this,0);
}}else{this.__DF=0;
this._onDocumentIsReady();
}},_loaded:function(e){if(this.__Dr){return;
}
if(this.__Dq){this.__DH();
return;
}if(qx.core.Variant.isSet(eV,eT)){this.__DG();
}else{this._onDocumentIsReady();
}},isReady:function(){return this.__Dp;
},_onDocumentIsReady:function(){var fC=new qx.bom.htmlarea.manager.Command(this);

if(this.getUseUndoRedo()){fC=new qx.bom.htmlarea.manager.UndoRedo(fC,this);
}this.__Dr=true;
if(qx.core.Variant.isSet(eV,eT)){this.setEditable(true);
}this.__DK();
this.__DL();
if(!qx.core.Variant.isSet(eV,eT)){this.setEditable(true);
}this.__Dp=true;
this.__Di=fC;
fC.setContentDocument(this._getIframeDocument());
this.__DN();
this.fireEvent(dt);
},forceEditable:qx.core.Variant.select(eV,{"gecko":function(){var ba=this._getIframeDocument();

if(ba){if(qx.bom.client.Engine.VERSION>=eh){ba.designMode=cy;
ba.body.contentEditable=false;
ba.body.contentEditable=true;
}else{ba.body.contentEditable=true;
this.__DO(true);
}}},"default":qx.lang.Function.empty}),invalidateEditor:qx.core.Variant.select(eV,{"gecko":function(){this.__Dr=false;
this.__Dp=false;
this.__Dq=true;
},"default":function(){}}),__DH:qx.core.Variant.select("qx.client",{"gecko":function(){this.__DK();
this.__DL();
this.__Di.setContentDocument(this._getIframeDocument());
...
},"default":function(){}}),__DI:function(gp){var gr="";

if(!gp){return gr;
}
try{var gq=gp.getAttribute("style");

if(!gq){return gr;
}if(qx.core.Variant.isSet("qx.client","mshtml")){gr=gq.cssText;
}else{gr=gq;
}}catch(X){this.error("can't extract style from elem. ");
}return gr;
},__DJ:function(fE){var fF=qx.bom.client.Engine.GECKO?" html, body {overflow-x: visible; } ":"";
var fG=this.__Dy[this.getContentType()];
var fH='<head>'+fG.meta+'<style type="text/css">'+fF+fG.style+fG.contentStyle+this.__Da+'</style>'+'</head>';
var content=fG.body+fE;
return fG.html+fH+content+fG.footer;
},__DK:function(){var fV=this.getValue();

if(typeof fV=="string"){var fU=this._getIframeDocument();

try{fU.open("text/html",true);
fU.write(this.__DJ(fV));
fU.close();
}catch(e){this.error("cant open document on source '"+qx.bom.Iframe.queryCurrentUrl(this.__Dx)+"'",e);
this.fireDataEvent("loadingError",e);
}}},__DL:function(){var u=qx.event.Registration;
var r=this._getIframeDocument();
u.addListener(r.body,"keypress",this._handleKeyPress,this);
u.addListener(r.body,"keyup",this._handleKeyUp,this);
u.addListener(r.body,"keydown",this._handleKeyDown,this);
var t=qx.bom.client.Engine.WEBKIT?this._getIframeWindow():r.body;
u.addListener(t,"focus",this.__Db,this);
u.addListener(t,"blur",this.__Dc,this);
u.addListener(r,"focusout",this.__Dd,this);
var q=qx.bom.client.Engine.MSHTML?"click":"mouseup";
u.addListener(r.body,q,this.__De,this);
u.addListener(r.documentElement,q,this.__Df,this);
u.addListener(r.documentElement,"contextmenu",this.__Dg,this);
},__DM:function(){if(this.__Du==null){this.__Du={execute:function(B,C){this.stackedCommands=true;
this.commandStack.push({command:B,value:C});
},commandStack:[],stackedCommands:false};
}this.__Du.stackedCommands=false;
return this.__Du;
},__DN:function(){var E=this.__Du;

if(E!=null&&E.stackedCommands){var F=E.commandStack;

if(F!=null){for(var i=0,j=F.length;i<j;i++){this.__Di.execute(F[i].command,F[i].value);
}}}},__DO:function(bO){var bP=this._getIframeDocument();

if(this.__Dr&&bP){try{if(qx.core.Variant.isSet("qx.client","gecko")){bP.designMode=(bO!==false)?'Off':'On';
}bP.designMode=(bO!==false)?'On':'Off';
}catch(e){}}},setEditable:function(fY){if(this.__Dr){this.__DO(true);
if(qx.core.Variant.isSet(eV,eO)){try{var ga=this._getIframeDocument();
ga.execCommand(dJ,false,true);
}catch(bi){try{var ga=this._getIframeDocument();
ga.execCommand(eo,false,false);
}catch(fT){if(!this.__Dp){this.error("Failed to enable rich edit functionality");
this.fireDataEvent(fm,fT);
}else{throw new Error("Failed to enable rich edit functionality");
}}}}this.__Ds=fY;
}},getEditable:function(){return this.__Ds;
},isEditable:function(){return this.__Ds;
},__DP:false,_handleKeyUp:function(e){var gj=e.getKeyIdentifier().toLowerCase();
var gl=e.isCtrlPressed();
this.__Dv=e;

if(false&&qx.core.Setting.get(dU)==dT){this.debug(e.getType()+" | "+gj);
}if(qx.core.Variant.isSet(eV,eT)){if(this.__DP){switch(gj){case eR:if(this.getInsertLinebreakOnCtrlEnter()){var gk=this.__DX(this.getSelection());

if(gk){gk.collapse(true);
gk.pasteHTML(eI);
}this.__DS();
}break;
case eA:this.__DP=false;
return;
break;
}}}else if(qx.core.Variant.isSet(eV,eO)){switch(gj){case ez:case ew:case ey:case dL:case fg:case eu:case es:case dR:case dN:this.__Dt=(this.getFocusNode()==this.getContentBody().firstChild);
break;
}}else if(qx.core.Variant.isSet(eV,cF)){if(gl){if(this.getInsertLinebreakOnCtrlEnter()&&gj==eR){this.__DQ();
e.preventDefault();
e.stopPropagation();
this.__DS();
}this.__DP=false;
}}},__DQ:function(){var gH=this.getSelection();
var gI="";
if(gH&&(gH.focusNode.textContent==""||gH.focusNode.parentElement.tagName=="LI")){gI="<br class='webkit-block-placeholder' />";
}this.__Di.execute("inserthtml",gI+qx.bom.htmlarea.HtmlArea.simpleLinebreak);
},_handleKeyDown:qx.core.Variant.select(eV,{"mshtml|webkit":function(e){var D=e.getKeyIdentifier().toLowerCase();

if(false&&qx.core.Setting.get(dU)==dT){}if(this.__DP&&(D==cG||D==fa||D==cC||D==et||D==dP||D==fb)){e.preventDefault();
e.stopPropagation();
}if(D==eA){this.__DP=true;
}},"default":function(e){}}),_handleKeyPress:function(e){var gw=this.getContentDocument();
var gs=e.getKeyIdentifier().toLowerCase();
var gu=e.isCtrlPressed();
var gv=e.isShiftPressed();
this.__Dv=e;

if(false&&qx.core.Setting.get(dU)==dT){this.debug(e.getType()+" | "+gs);
}if(this.__DA){var gD=!(qx.core.Variant.isSet(eV,eT)&&gs==eR)||!(qx.core.Variant.isSet(eV,eO)&&gs==eR);

if(gD){this.__DS();
this.__DA=false;
}}
switch(gs){case eR:if(!gv&&!gu&&this.getMessengerMode()){e.preventDefault();
e.stopPropagation();
var gB=new qx.event.type.Data(eq,this.getComputedValue());
this.dispatchEvent(gB,true);
this.resetHtml();
}if(gu){if(!this.getInsertLinebreakOnCtrlEnter()){return;
}e.preventDefault();
e.stopPropagation();

if(qx.core.Variant.isSet(eV,eO)){if(this.__DV()){this.insertHtml(ea);
this.__DS();
return;
}this.insertHtml(fz);
}else if(qx.core.Variant.isSet(eV,dO)){var gC=this.getSelection();
var gx=this.__DX(gC);

if(gC&&gx){var gy=gw.createElement(fr);
gx.collapse(true);
gx.insertNode(gy);
gx.collapse(true);
gx.selectNode(gy);
...
}if(qx.core.Variant.isSet(eV,eT)){if(!this.getInsertParagraphOnLinebreak()){if(this.__Di.execute(dM,qx.bom.htmlarea.HtmlArea.simpleLinebreak)){this.__DS();
e.preventDefault();
e.stopPropagation();
}}}else if(qx.core.Variant.isSet(eV,eO)){if(this.getInsertParagraphOnLinebreak()&&!gv&&!gu){var gC=this.getSelection();

if(gC){var gt=gC.focusNode;
if(this.__DV()){this.__DS();
return;
}if(this.__DW()){this.__DS();
return;
}while(!qx.dom.Node.isNodeName(gt,de)){if(qx.dom.Node.isNodeName(gt,cI)){this.__DS();
return;
}gt=gt.parentNode;
}}this.__Di.insertParagraphOnLinebreak();
e.preventDefault();
e.stopPropagation();
this.__DS();
this.__DA=true;
}}else if(qx.core.Variant.isSet(eV,cF)){if(this.getInsertParagraphOnLinebreak()&&gv){this.__DQ();
e.preventDefault();
e.stopPropagation();
this.__DS();
}}break;
case ey:if(qx.bom.client.Engine.GECKO&&qx.bom.client.Engine.FULLVERSION<1.9&&gv){var gC=this.getSelection();
if(gC&&gC.focusNode==gw.body.firstChild){if(this.__Dt){if(gC.focusOffset!=0){gC.extend(gC.focusNode,0);
}}}}break;
case fy:if(qx.bom.client.Engine.GECKO&&qx.bom.client.Engine.FULLVERSION<1.9){if(gu){var gC=this.getSelection();
if(gv){if(gC&&(gC.focusOffset!=0)||(gC.focusNode!=gw.body.firstChild)){gC.extend(gw.body.firstChild,0);
}}else{var gz=null;
var gA;
if(gw){gz=gw.evaluate(fn,gw.body,null,XPathResult.ANY_TYPE,null);
}
if(gz&&gC){while(gA=gz.iterateNext()){if(gA&&gA.parentNode&&gA.parentNode.tagName!=dV){try{gC.extend(gA,0);

if(!this.isSelectionCollapsed()){gC.collapseToStart();
}}catch(e){}break;
}}}}}}this.__DS();
break;
case ez:case ew:case dL:case fg:case eu:case es:case dR:case dN:this.__DS();
break;
case cC:if(gu){this.__DR(dp,true);
}break;
case dP:case fb:if(gu){this.__DR(dI,true);
}break;
case et:if(gu){this.__DR(ec,true);
}break;
case cG:if(gu&&!gv){this.__DR(cK,true);
}else if(gu&&gv){this.__DR(eY,true);
}break;
case fa:if(gu){this.__DR(eY,true);
}break;
case dy:if(gu){this.selectAll();
}break;
}this.__Dv=null;
},__DR:function(bw,bx){var bz=null;
var by=qx.bom.htmlarea.HtmlArea.hotkeyInfo;

if(by[bw]){bz=by[bw].method;
}
if(bz!=null&&this[bz]){this[bz]();

if(bx){this.__Dv.preventDefault();
this.__Dv.stopPropagation();
}
if(this.isSelectionCollapsed()){this.__DA=true;
}this.__DS();
}},_handleFocusEvent:function(e){this.__Dw=null;
this.fireEvent(fw);
},_handleBlurEvent:function(e){this.__Dh=this.getComputedValue();
},_handleFocusOutEvent:function(e){if(this.__Dw==null){this.__Dw=this.getSelectedHtml();
}this.fireEvent(dB);
},_handleMouseUpOnBody:function(e){if(false&&qx.core.Setting.get(dU)==dT){this.debug("handleMouse "+e.getType());
}this.__DB=true;
this.__DS();
},_handleMouseUpOnDocument:qx.core.Variant.select(eV,{"mshtml":function(e){},"default":function(e){if(!this.__DB){qx.bom.Element.activate(this.getContentBody());
}this.__DB=false;
}}),_handleContextMenuEvent:function(e){var bL=e.getViewportLeft();
var bK=e.getViewportTop();
var bM=qx.bom.element.Location.getLeft(this.__Do)+bL;
var bN=qx.bom.element.Location.getTop(this.__Do)+bK;
var bJ={x:bM,y:bN,relX:bL,relY:bK,target:e.getTarget()};
e.preventDefault();
e.stopPropagation();
qx.event.Timer.once(function(){this.fireDataEvent(cE,bJ);
},this,0);
},isLoaded:function(){return this.__Dr;
},insertHtml:function(bh){return this.__Di.execute(dM,bh);
},removeFormat:function(){return this.__Di.execute(ds);
},setBold:function(){return this.__Di.execute(dm);
},setItalic:function(){return this.__Di.execute(dA);
},setUnderline:function(){return this.__Di.execute(dE);
},setStrikeThrough:function(){return this.__Di.execute(em);
},setFontSize:function(bl){return this.__Di.execute(cV,bl);
},setFontFamily:function(gG){return this.__Di.execute(fp,gG);
},setTextColor:function(bY){return this.__Di.execute(dx,bY);
},setTextBackgroundColor:function(fB){return this.__Di.execute(ft,fB);
},setJustifyLeft:function(){return this.__Di.execute(dC);
},setJustifyCenter:function(){return this.__Di.execute(en);
},setJustifyRight:function(){return this.__Di.execute(eC);
},setJustifyFull:function(){return this.__Di.execute(ed);
},insertIndent:function(){return this.__Di.execute(ej);
},insertOutdent:function(){return this.__Di.execute(fo);
},insertOrderedList:function(){return this.__Di.execute(dc);
},insertUnorderedList:function(){return this.__Di.execute(eF);
},insertHorizontalRuler:function(){return this.__Di.execute(eG);
},insertImage:function(W){return this.__Di.execute(dj,W);
},insertHyperLink:function(gb){return this.__Di.execute(eb,gb);
},removeBackgroundColor:function(){this.__Di.execute(cD,dl);
},setBackgroundColor:function(n){this.__Di.execute(cD,n);
},removeBackgroundImage:function(){this.__Di.execute(ev);
},setBackgroundImage:function(gd,ge,gf){return this.__Di.execute(ev,[gd,ge,gf]);
},selectAll:function(){return this.__Di.execute(eL);
},undo:function(){if(this.getUseUndoRedo()){return this.__Di.execute(ff);
}else{return true;
}},redo:function(){if(this.getUseUndoRedo()){return this.__Di.execute(dQ);
}else{return true;
}},resetHtml:function(){var bG=this._getIframeDocument();
while(bG.body.firstChild){bG.body.removeChild(bG.body.firstChild);
}if(qx.bom.client.Engine.GECKO){bG.body.innerHTML=du;
}else if(qx.bom.client.Engine.WEBKIT){var bH=this.getSelection();
var bI=bG.createRange();

if(bI&&bH){bH.addRange(bI);
}}},getHtml:function(fW){var fX=this._getIframeDocument();

if(fX==null){return null;
}return qx.bom.htmlarea.HtmlArea.__Dl(fX.body,false,fW,this.getPostProcess());
},containsOnlyPlaceholder:qx.core.Variant.select(eV,{"mshtml":function(){var bX=this._getIframeDocument();
return (bX.body.innerHTML==cT);
},"default":function(){return false;
}}),getContextInformation:function(){return this.__DT();
},__DS:function(){qx.event.Timer.once(function(e){var Y=this.__DT();
this.fireDataEvent("cursorContext",Y);
},this,200);
},__DT:function(){if(this._processingExamineCursorContext||this.getEditable()==false){return;
}this._processingExamineCursorContext=true;
var ck=this._getIframeDocument();
var cg=this.getFocusNode();

if(qx.dom.Node.isText(cg)){cg=cg.parentNode;
}var cu=qx.core.Variant.isSet("qx.client","mshtml")?cg.currentStyle:ck.defaultView.getComputedStyle(cg,null);
var co=qx.core.Variant.isSet("qx.client","mshtml|opera")?cu.fontWeight==700:cu.getPropertyValue("font-weight")=="bold"||qx.dom.Node.isNodeName(cg,"b");
var cd=qx.core.Variant.isSet("qx.client","mshtml")?cu.fontStyle=="italic":cu.getPropertyValue("font-style")=="italic";
var ci=qx.core.Variant.isSet("qx.client","mshtml")?cu.textDecoration.indexOf("underline")!==-1:cu.getPropertyValue("text-decoration").indexOf("underline")!==-1;
var ch=qx.core.Variant.isSet("qx.client","mshtml")?cu.textDecoration.indexOf("line-through")!==-1:cu.getPropertyValue("text-decoration").indexOf("line-through")!==-1;
var cr=qx.core.Variant.isSet("qx.client","mshtml")?cu.fontSize:cu.getPropertyValue("font-size");
var cw=null;
var cp=qx.core.Variant.isSet("qx.client","mshtml")?cu.fontFamily:cu.getPropertyValue("font-family");
var cn=false;
var ct=false;
var ce=cg;
if(ce!=null&&ce.parentNode!=null&&!qx.dom.Node.isDocument(ce.parentNode)){while(ce!=null&&!qx.dom.Node.isNodeName(ce,"body")){var cl=qx.dom.Node.getName(ce);

if(cl=="ol"){ct=true;
break;
}else if(cl=="ul"){cn=true;
break;
}
if(cw==null||cw==""){cw=this._getAttribute(ce,'size');
}ce=ce.parentNode;
}}var cj=qx.core.Variant.isSet("qx.client","mshtml")?cu.textAlign=="left":cu.getPropertyValue("text-align")=="left";
var cv=qx.core.Variant.isSet("qx.client","mshtml")?cu.textAlign=="center":cu.getPropertyValue("text-align")=="center";
var cf=qx.core.Variant.isSet("qx.client","mshtml")?cu.textAlign=="right":cu.getPropertyValue("text-align")=="right";
var cs=qx.core.Variant.isSet("qx.client","mshtml")?cu.textAlign=="justify":cu.getPropertyValue("text-align")=="justify";
var cq={bold:co?1:0,italic:cd?1:0,underline:ci?1:0,strikethrough:ch?1:0,fontSize:(cw==null)?cr:cw,fontFamily:cp,insertUnorderedList:cn?1:0,insertOrderedList:ct?1:0,justifyLeft:cj?1:0,justifyCenter:cv?1:0,justifyRight:cf?1:0,justifyFull:cs?1:0};
this._processingExamineCursorContext=false;
return cq;
},_getAttribute:qx.core.Variant.select(eV,{"mshtml":function(l,m){try{return l[m];
}catch(e){return null;
}},"default":function(fI,fJ){try{return fI.getAttribute(fJ);
}catch(e){return null;
}}}),getSelection:qx.core.Variant.select(eV,{"mshtml":function(){return this._getIframeDocument()?this._getIframeDocument().selection:null;
},"default":function(){return this._getIframeWindow()?this._getIframeWindow().getSelection():null;
}}),isSelectionCollapsed:qx.core.Variant.select(eV,{"mshtml":function(){return this.getSelection()&&this.getSelection().type==eB;
},"default":function(){return this.getSelection()&&this.getSelection().isCollapsed;
}}),getSelectedText:qx.core.Variant.select(eV,{"mshtml":function(){return this.getRange()?this.getRange().text:eU;
},"default":function(){return this.getRange()?this.getRange().toString():eU;
}}),getSelectedHtml:function(){if(this.__Dw!=null){return this.__Dw;
}var bm=this.getRange();

if(!bm){return eU;
}else{return this.__DU(bm);
}},__DU:qx.core.Variant.select("qx.client",{"mshtml":function(gc){if(!gc){return "";
}return gc.item?gc.item(0).outerHTML:gc.htmlText;
},"default":function(fK){var fL=this._getIframeDocument();

if(fL&&fK){try{var fM=fL.createElement("body");
fM.appendChild(fK.cloneContents());
return fM.innerHTML;
}catch(ca){}return fK+"";
}return "";
}}),clearSelection:qx.core.Variant.select(eV,{"mshtml":function(){var x=this.getSelection();

if(x){x.empty();
}},"default":function(){var gF=this.getSelection();

if(gF){gF.collapseToStart();
}}}),__DV:qx.core.Variant.select("qx.client",{"gecko":function(){var v=this.getSelection();
var w=this.getFocusNode();
return v&&this.isSelectionCollapsed()&&qx.dom.Node.isText(w)&&v.anchorOffset<w.length;
},"default":function(){return false;
}}),__DW:qx.core.Variant.select("qx.client",{"gecko":function(){return qx.dom.Node.isElement(this.getFocusNode());
},"default":function(){return false;
}}),getRange:function(){return this.__DX(this.getSelection());
},__DX:qx.core.Variant.select("qx.client",{"mshtml":function(o){var p=this._getIframeDocument();

if(o){try{return o.createRange();
}catch(fS){return p.body.createTextRange();
}}else{return p.body.createTextRange();
}},"default":function(bj){var bk=this._getIframeDocument();

if(bj){try{return bj.getRangeAt(0);
}catch(gm){return bk.createRange();
}}else{return bk.createRange();
}}}),saveRange:qx.core.Variant.select(eV,{"mshtml":function(){this.__Dz=this.getRange();
},"default":function(){}}),getSavedRange:qx.core.Variant.select(eV,{"mshtml":function(){return this.__Dz;
},"default":function(){}}),resetSavedRange:qx.core.Variant.select(eV,{"mshtml":function(){this.__Dz=null;
},"default":function(){}}),getFocusNode:qx.core.Variant.select(eV,{"mshtml":function(){var bn=this.getSelection();
var bo;

switch(bn.type){case eE:case eB:bo=this.__DX(bn);
bo.collapse(false);
return bo.parentElement();
case dG:bo=this.__DX(bn);

try{bo.collapse(false);
}catch(fN){}return bo.item(0);
default:return this._getIframeDocument().body;
}},"default":function(){var bb=this.getSelection();

if(bb&&bb.focusNode){return bb.focusNode;
}return this._getIframeDocument().body;
}})},settings:{"qx.bom.htmlarea.HtmlArea.debug":dY},destruct:function(){try{var bD=this._getIframeDocument();
var bF=qx.event.Registration;
bF.removeListener(bD.body,cY,this._handleKeyPress,this);
bF.removeListener(bD.body,fl,this._handleKeyUp,this);
bF.removeListener(bD.body,fk,this._handleKeyDown,this);
var bE=qx.bom.client.Engine.WEBKIT?this._getIframeWindow():bD.body;
bF.removeListener(bE,dX,this.__Db);
bF.removeListener(bE,ei,this.__Dc);
bF.removeListener(bD,fh,this.__Dd);
var bC=qx.bom.client.Engine.MSHTML?dD:dn;
bF.removeListener(bD.body,bC,this.__De,this);
bF.removeListener(bD.body,bC,this.__Df,this);
bF.removeListener(bD.documentElement,cE,this.__Dg,this);
}catch(gg){}this._disposeObjects(dz);
this.__Db=this.__Dc=this.__Dd=null;
this.handleMouseUpOnBody=this.__Df=this.__Dy=null;
this.__Dx=this.__Do=null;
}});
})();
...
(function(){var cj="qx.client",ci="",ch="legacy-font-size",cg="inserthtml",cf='color',ce="__Eq",cd="mshtml",cc=';',cb="span",ca='">',bM="child",bL="text-align",bK='text-decoration',bJ="text-decoration",bI="</font>",bH="__Er",bG="p",bF=':',bE="id",bD="font",cq=":",cr="InsertHtml",co=";",cp="Control",cm="__paragraph__",cn="qx.bom.htmlarea.HtmlArea.debug",ck="on",cl="size",cs="_moz",ct="/>",bS="formatBlock",bR="insertParagraph",bU="li",bT='<span style="',bW="__EI",bV=' <a href="',bY="|top|bottom|center|left|right|right top|left top|left bottom|right bottom|",bX="__placeholder__",bQ="JustifyCenter",bP="<p style='",u="outdent",v='</a> ',w="InsertImage",x="__resetHtml",y="FontName",z="gecko",A="blockquote",B="__EH",C="style",D="__EG",cx="P",cw="qx.bom.htmlarea.manager.Command",cv="'><br class='webkit-block-placeholder' />",cu="a",cB='"',cA="<span></span>",cz="type",cy="CreateLink",cD="__EF",cC="='",bd="<img ",be="qx_link",bb='"></span>',bc="Italic",bh="InsertUnorderedList",bi='text-align:',bf="Bold",bg="ForeColor",Y='" ',ba="repeat repeat-x repeat-y no-repeat",L="__EE",K='</p>',N="__Ew",M="<hr />",H='<font style="',G="IMG",J='<p id="',I="StrikeThrough",F="__getHtml",E='>',bn="__Ev",bo='" size="',bp="Indent",bq="__Em",bj="br",bk="Copy",bl="</span>",bm="Paste",br="RemoveFormat",bs="JustifyRight",V="FontSize",U="indent",T="' ",S='',R='style="',Q="__EK",P="_moz_dirty",O="JustifyLeft",X="Underline",W="SelectAll",bt="Cut",bu="BODY",bv="href",bw='<span id="',bx="__Et",by="JustifyFull",bz="__EJ",bA="Outdent",bB="webkit",bC="selectall",bO="InsertOrderedList",bN="undefined";
qx.Class.define(cw,{extend:qx.core.Object,construct:function(eR){arguments.callee.base.call(this);
this.__DY=eR;
this.__Ea=null;
this._commands=null;
this.__Eh();
this.__Eb={"Bold":true,"Italic":true,"Underline":true,"StrikeThrough":true};
this.__Ec=[10,12,16,18,24,32,48];
this.__Ed=0;
},statics:{__Ee:bY,__Ef:ba},members:{__Ea:null,__DY:null,__Eg:false,__Eb:null,__Ec:null,__Ed:null,setContentDocument:function(fN){this.__Ea=fN;
},getCommandObject:function(ed){if(this._commands[ed]){return this._commands[ed];
}else{return null;
}},__Eh:function(){this._commands={bold:{useBuiltin:true,identifier:bf,method:null},italic:{useBuiltin:true,identifier:bc,method:null},underline:{useBuiltin:true,identifier:X,method:null},strikethrough:{useBuiltin:true,identifier:I,method:null},fontfamily:{useBuiltin:true,identifier:y,method:null},fontsize:{useBuiltin:false,identifier:V,method:L},textcolor:{useBuiltin:true,identifier:bg,method:null},textbackgroundcolor:{useBuiltin:false,identifier:null,method:cD},backgroundcolor:{useBuiltin:false,identifier:null,method:D},backgroundimage:{useBuiltin:false,identifier:null,method:B},justifyleft:{useBuiltin:false,identifier:O,method:ce},justifyright:{useBuiltin:false,identifier:bs,method:ce},justifycenter:{useBuiltin:false,identifier:bQ,method:ce},justifyfull:{useBuiltin:false,identifier:by,method:ce},indent:{useBuiltin:true,identifier:bp,method:null},outdent:{useBuiltin:true,identifier:bA,method:null},copy:{useBuiltin:true,identifier:bk,method:null},cut:{useBuiltin:true,identifier:bt,method:null},paste:{useBuiltin:true,identifier:bm,method:null},insertorderedlist:{useBuiltin:false,identifier:bO,method:bH},insertunorderedlist:{useBuiltin:false,identifier:bh,method:bH},inserthorizontalrule:{useBuiltin:false,identifier:cr,method:N},insertimage:{useBuiltin:false,identifier:w,method:bx},inserthyperlink:{useBuiltin:false,identifier:cy,method:bn},selectall:{useBuiltin:false,identifier:W,method:bW},selectedtext:{useBuiltin:false,identifier:null,method:bz},selectedhtml:{useBuiltin:false,identifier:null,method:Q},inserthtml:{useBuiltin:false,identifier:cr,method:bq},resethtml:{useBuiltin:false,identifier:null,method:x},gethtml:{useBuiltin:false,identifier:null,method:F},removeformat:{useBuiltin:true,identifier:br,method:null}};
},execute:function(gz,gA){if(!this.__DY.isReady()){this.error("editor not ready! '"+gz+"':'"+gA+"'");
return false;
}gz=gz.toLowerCase();
gA=gA!=null?gA:null;
if(this._commands[gz]){var gB;
var gC=this._commands[gz];
if(!(qx.bom.client.Engine.WEBKIT&&(gz==U||gz==u))){if(this.__Ei()){this.__Ej();
}}if(gC.useBuiltin){gB=this.__Ek(gC.identifier,false,gA);
}else{if(gC.method!=null&&this[gC.method]){gB=this[gC.method].call(this,gA,gC);
}else{this.error("The method '"+gC.method+"' you calling to execute the command '"+gz+"' is not available!");
}}this.__DY.resetSavedRange();
return gB;
}else{this.error("Command "+gz+" is currently not supported!");
}},__Ei:function(){var fO=this.__DY.getFocusNode();
var fR=false;
var fP=false;

if(fO){if(fO.nodeType==3){var fQ=qx.dom.Hierarchy.getAncestors(fO);

for(var i=0,j=fQ.length;i<j;i++){if(fQ[i].tagName==cx){fR=true;
break;
}}}else if(fO.nodeType==1&&fO.tagName==bu){fP=true;
}}return fP||!fR;
},__Ej:function(){this.__Ek(bS,false,bG);
},__Ek:function(fn,fo,fp){try{var fs=this.__Ea;
var fq=false;
var fr=this.__DY.getRange();
this.__Ea.body.focus();
if(qx.core.Variant.isSet(cj,cd)){if(fn!=bC){fr.select();
if(((fr.text)&&(fr.text.length>0))||((fr.length==1)&&(fr.item(0))&&(fr.item(0).tagName==G))){fs=fr;
}else{fs=this.__Ea;
}}if((qx.core.Variant.isSet(cj,cd))&&(this.__Eb[fn])){if(fr.text==ci){fq=true;
}}}var ft=fs.execCommand(fn,fo,fp);

if(fq&&fr.text!=ci){fr.collapse();
}
if(false&&qx.core.Setting.get(cn)==ck){this.debug("execCommand "+fn+" with value "+fp+" succeded");
}this.__Eg=false;
}catch(ge){if(false&&qx.core.Setting.get(cn)==ck){this.debug("execCommand "+fn+" with value "+fp+" failed");
}return false;
}return ft;
},__El:qx.core.Variant.select(cj,{"mshtml":function(){var eJ=this.__DY;
var eI=eJ.getSavedRange()!=null?eJ.getSavedRange():eJ.getRange();
return eI;
},"default":function(){return this.__DY.getRange();
}}),__Em:qx.core.Variant.select(cj,{"mshtml":function(gl,gm){var gp;
if(gl==qx.bom.htmlarea.HtmlArea.simpleLinebreak){return this.__Ep();
}else{this.__Ea.body.focus();
var go=this.__DY.getSelection();
var gn=this.__El();
if(gn&&go&&go.type!=cp){try{gn.pasteHTML(gl);
gn.collapse(false);
gn.select();
gp=true;
}catch(e){}}else{gp=false;
}this.__DY.resetSavedRange();
return gp;
}},"default":function(dR,dS){this.__Ea.body.focus();
return this.__Ea.execCommand(dS.identifier,false,dR);
}}),insertParagraphOnLinebreak:qx.core.Variant.select(cj,{"gecko":function(){var fG=this.__Ez();
var fK=this.__En(fG);
var fC=this.__Ex(fG);
var fH=bX+Date.parse(new Date());
var fI=cm+Date.parse(new Date());
var fA=S;
var fE=bw+fH+bb;
fA+=fE;
fA+=J+fI+Y+fK+E;
fA+=fC+K;
this.__DY.getCommandManager().addUndoStep(cg,bR,this.getCommandObject(cg));
this.execute(cg,fA);
this.__Eo();
this.__Ea.getElementById(fH).removeAttribute(bE);
var fF=this.__Ea.getElementById(fI);

if(fF.previousSibling.innerHTML==fE){var fz=this.__Ey();
var fD=this.__Ea.createElement(bj);
var fB=this.__Ea.createAttribute(P);
fB.nodeValue=ci;
fD.setAttributeNode(fB);
var fJ=this.__Ea.createAttribute(cz);
fJ.nodeValue=cs;
fD.setAttributeNode(fJ);
fF.previousSibling.appendChild(fz);
fF.previousSibling.appendChild(fD);
}return true;
},"webkit":function(){var cM=this.getCurrentStyles();
var cN=ci;
var cP={"background-color":true,"text-align":true};
for(var cO in cM){if(cP[cO]){cN+=cO+cq+cM[cO]+co;
}}this.__DY.insertHtml(bP+cN+cv);
},"default":qx.lang.Function.empty}),__En:qx.core.Variant.select(cj,{"gecko":function(cE){var cJ=R;
var cI=cE.child;

if(cI[bL]){cJ+=bi+cI[bL]+cc;
delete cE.child[bL];
}var cH={"padding-top":true,"padding-bottom":true,"padding-left":true,"padding-right":true};
var cF={"margin-top":true,"margin-bottom":true,"margin-left":true,"margin-right":true};

for(var cG in cI){if(cH[cG]||cF[cG]){cJ+=cG+bF+cI[cG]+cc;
delete cE.child[cG];
}}cJ+=cB;
return cJ;
},"default":function(){return ci;
}}),__Eo:qx.core.Variant.select(cj,{"gecko":function(){var s=this.__DY.getSelection();

if(!s||!s.focusNode){return;
}var h=s.focusNode;
var q=s.focusNode;

while(!qx.dom.Node.isNodeName(q,bG)){q=q.parentNode;
}var p=q.previousSibling.id;
var t=q.nextSibling?q.nextSibling.id:null;

if(qx.lang.String.startsWith(p,cm)&&p==t){var r=q.nextSibling;
var n=this.__DY.getRange();
n.selectNode(r);
s.addRange(n);
var m=qx.bom.htmlarea.HtmlArea.EMPTY_DIV;
this.__DY.getCommandManager().addUndoStep(cg,m,this.getCommandObject(cg));
this.execute(cg,m);
var o=this.__DY.getRange();

if(h){while(h&&h.firstChild&&qx.dom.Node.isElement(h.firstChild)){h=h.firstChild;
}}o.selectNode(h);
s.addRange(o);
o.collapse(true);
}},"default":qx.lang.Function.empty}),__Ep:qx.core.Variant.select(cj,{"mshtml":function(){var gd=this.__DY.getRange();
if(gd&&!qx.dom.Node.isNodeName(gd.parentElement(),bU)){gd.pasteHTML(qx.bom.htmlarea.HtmlArea.simpleLinebreak);
gd.collapse(false);
gd.select();
return true;
}return false;
},"default":function(){return false;
}}),__Eq:function(gw,gx){var gy=qx.core.Variant.isSet(cj,cd)?this.__DY.getRange():this.__Ea;
return gy.execCommand(gx.identifier,false,gw);
},__Er:function(fS,fT){if(qx.core.Variant.isSet(cj,cd)){var fU=this.__DY.getFocusNode();
this.__Es(fU);
}this.__Ea.body.focus();
var fV=this.__Ea.execCommand(fT.identifier,false,fS);

if(qx.core.Variant.isSet(cj,bB)){var fU=this.__DY.getFocusNode();
this.__Es(fU.parentNode);
}return fV;
},__Es:function(c){var d=[];
var parent=c.parentNode;

while(qx.dom.Node.isNodeName(parent,A)){d.push(parent);
parent=parent.parentNode;
}if(d.length>0){parent.appendChild(c);
parent.removeChild(d[d.length-1]);
}},__Et:qx.core.Variant.select(cj,{"gecko":function(cQ,cR){if(cQ.src){this.__Ea.execCommand(cR.identifier,false,cQ.src);
delete cQ.src;
var df=this.__DY.getSelection();
if(df){var de=df.anchorNode;
var cY=df.anchorOffset;
var cV=de.childNodes[cY-1];
var dc;

for(var cX in cQ){dc=this.__Ea.createAttribute(cX);
dc.nodeValue=cQ[cX];
cV.setAttributeNode(dc);
}var cW={"font":true,"span":true};
var cT=null;
var db=true;
if(qx.dom.Node.isElement(cV.previousSibling)&&cW[qx.dom.Node.getName(cV.previousSibling)]){cT=cV.previousSibling;
}else if(cW[qx.dom.Node.getName(cV.parentNode)]){cT=cV.parentNode;
db=false;
}var cU=this.__Ea.createDocumentFragment();
var dg;

if(db&&cT!=null){var cW=this.__Eu(cT);
cU.appendChild(cW.root);
dg=cW.inline;
}else{dg=this.__Ea.createElement(cb);
cU.appendChild(dg);
}var dd=this.__Ea.createTextNode(ci);
dg.appendChild(dd);
var da=cV.parentNode;
if(cV==da.lastChild){da.appendChild(cU);
}else{da.insertBefore(cU,cV.nextSibling);
}var cS=this.__DY.getRange();
cS.selectNodeContents(dg);
}return true;
}else{return false;
}},"mshtml":function(ev,ew){var eB;
var eC=bd;

for(var ey in ev){eC+=ey+cC+ev[ey]+T;
}eC+=ct;
var ez=this.__DY.getSelection();
var eA=this.__El();
if(ez&&ez.type!=cp){eA.select();
eA.pasteHTML(eC);
eB=true;
}else{eB=false;
}this.__DY.resetSavedRange();
return eB;
},"default":function(a,b){return this.__Ea.execCommand(b.identifier,false,a.src);
}}),__Eu:function(dv){while(dv.firstChild&&dv.firstChild.nodeType==1){dv=dv.firstChild;
}var dz=this.__Ez(dv);
var dD,dx,dB;
var dw=ci;
var parent=null;
...
while(dA){dC=this.__Ea.createElement(dA[ch]?bD:cb);
dx=this.__Ea.createAttribute(C);
dC.setAttributeNode(dx);
for(var dy in dA){if(dy!=bM&&dy!=ch){dw+=dy+cq+dA[dy]+co;
}else if(dy==ch){dB=this.__Ea.createAttribute(cl);
dB.nodeValue=dA[dy];
dC.setAttributeNode(dB);
}}dx.nodeValue=dw;

if(parent!=null){parent.appendChild(dC);
}else{dD=dC;
}parent=dC;
dA=dA.child;
dw=ci;
}return {root:dD,inline:dC};
},__Ev:qx.core.Variant.select(cj,{"gecko":function(dT,dU){var eb=this.__DY.getSelection();
var dX=this.__DY.getRange();
if(eb.isCollapsed){var dW=be+(++this.__Ed);
var ec=this.__Ea.createElement(cu);
var dY=this.__Ea.createAttribute(bv);
var dV=this.__Ea.createAttribute(bE);
var ea=this.__Ea.createTextNode(dT);
dV.nodeValue=dW;
...
eb.collapseToEnd();
return true;
}else{return this.__Ea.execCommand(dU.identifier,false,dT);
}},"mshtml":function(eS,eT){try{var eV;
var eU=this.__El();
var eW=this.__DY;
var eU=eW.getSavedRange()!=null?eW.getSavedRange():eW.getRange();

if(eU!=null&&eU.text!=ci){eV=eU.execCommand(eT.identifier,false,eS);
}else{eV=this.__Em(bV+eS+ca+eS+v,eT);
}this.__DY.resetSavedRange();
return eV;
}catch(e){{};
return false;
}},"default":function(cK,cL){return this.__Ea.execCommand(cL.identifier,false,cK);
}}),__Ew:function(es,et){var eu=M;
if(qx.core.Variant.isSet(cj,z)){eu+=this.__Ex();
}return this.__Em(eu,et);
},__Ex:function(fd){var fh=ci;
var fg=bT;
var ff=[];
var fk=typeof fd!==bN?fd:this.__Ez();
var fi=fk.child;
var fj=false;

while(fi){fj=fi[ch]!=null;
fh+=fj?H:fg;

for(var fe in fi){fh+=(fe!=bM&&fe!=ch)?fe+bF+fi[fe]+cc:ci;
}fh+=fj?bo+fi[ch]+ca:ca;
ff.unshift(fj?bI:bl);
fi=fi.child;
}if(ff.length==1&&ff[0]==bI){fh+=cA;
}for(var i=0,j=ff.length;i<j;i++){fh+=ff[i];
}return fh;
},__Ey:function(){var eL=this.__Ea.createDocumentFragment();
var eQ=this.__Ez();
var parent=eL;
var eO=eQ.child;
var eM;
...
while(eO){eP=eO[ch]!=null;
eM=this.__Ea.createElement(eP?bD:cb);
parent.appendChild(eM);
for(var eK in eO){if(eK!=bM&&eK!=ch){qx.bom.element.Style.set(eM,eK,eO[eK]);
}}
if(eP){var eN=this.__Ea.createAttribute(cl);
eN.nodeValue=eO[ch];
eM.setAttributeNode(eN);
}parent=eM;
eO=eO.child;
}return eL;
},__Ez:function(gf){var gh={};
var gi=null;
var gj=this.getCurrentStyles(gf);
gi=gh.child={};

for(var gg in gj){if(gg!=bJ){gi[gg]=gj[gg];
}}if(gj[bJ]){var gk=gj[bJ];
for(var i=0,j=gk.length;i<j;i++){if(gi==null){gi=gh.child={};
}else{gi=gi.child={};
}gi[cf]=gk[i][cf];
gi[bK]=gk[i][bK];
}}if(gj[cf]&&gj[bK]){gi=gi.child={};
gi[cf]=gj[cf];
}return gh;
},getCurrentStyles:function(dh){if(dh==null){var dj=this.__DY.getSelection();

if(!dj||dj.focusNode==null){return {};
}dh=(dj.focusNode.nodeType==3)?dj.focusNode.parentNode:dj.focusNode;
}var dk=qx.dom.Hierarchy.getAncestors(dh);
var dm=qx.lang.Array.insertBefore(dk,dh,dk[0]);
var di=this.__EA(dm);
var dl=this.__EB(di,dm);
return dl;
},__EA:function(fY){var gc={};
var ga,gb;
...
if(ga.length>0&&typeof gc[ga]==="undefined"){gc[ga]=gb.style.getPropertyValue(ga);
}}if(gb.tagName.toUpperCase()=="FONT"&&gb.size&&gc["legacy-font-size"]===undefined){gc["legacy-font-size"]=gb.size;
}}if(gc["legacy-font-size"]&&gc["font-size"]){delete gc["font-size"];
}return gc;
},__EB:function(dn,dp){var du=dp[0];
var ds=this.__DY.getContentWindow().getComputedStyle(du,null);
var dq;
var dt={};

for(var dr in dn){if(dr!="legacy-font-size"){dq=ds.getPropertyValue(dr);
}else{dq=dn[dr];
}if(dr=="background-color"&&dq=="transparent"){dt[dr]=this.__ED(dp);
}else if(dr=="text-decoration"){dt[dr]=this.__EC(dp);
}else{dt[dr]=dq;
}}return dt;
},__EC:function(gq){var gt,gv,gr;
var gu=[];
var gs=this.__DY.getContentWindow();

for(var i=0,j=gq.length;i<j;i++){gr=gs.getComputedStyle(gq[i],null);
gt=gr.getPropertyValue("text-decoration");
gv=gr.getPropertyValue("color");
if(gt!="none"){gu.push({'text-decoration':gt,'color':gv});
}}return gu;
},__ED:function(dN){var dP,dO,dQ;

for(var i=0;i<dN.length;i++){dP=dN[i];
dO=this.__DY.getContentWindow().getComputedStyle(dP,null);
dQ=dO.getPropertyValue("background-color");
if(dQ!="transparent"){return dQ;
}}},__EE:function(ee,ef){var er=this.__DY.getSelection();
var em=(qx.core.Variant.isSet("qx.client","mshtml"))?this.__DY.getRange():em=er.getRangeAt(0);
var en=[];
var eq;
var el=["OL","UL"];
var eg,i,j,eo;
eo=(qx.core.Variant.isSet("qx.client","mshtml"))?em.parentElement():em.commonAncestorContainer;
if(eo.tagName=="BODY"){for(var i=0;i<el.length;i++){eg=eo.getElementsByTagName(el[i]);

for(var j=0;j<eg.length;j++){if(eg[j]){en.push(eg[j]);
}}}}else if(qx.lang.Array.contains(el,eo.tagName)){en.push(eo);
}
if(en.length>0){for(var i=0;i<en.length;i++){var ei=en[i];
eq=(qx.core.Variant.isSet("qx.client","mshtml"))?
((ei==eo)||(eo.tagName=="BODY")):
er.containsNode(ei,false);
for(j=0;j<ei.childNodes.length;j++){var ek=ei.childNodes[j];
if(eq||
(er.focusNode.nodeValue&&qx.dom.Hierarchy.contains(ek,er.focusNode)&&(er.focusOffset==er.focusNode.nodeValue.length))||
(qx.dom.Hierarchy.contains(ek,er.anchorNode)&&(er.anchorOffset==0))||
(er.containsNode(ek,false))){ek.style.fontSize=(this.__Ec[ee]||ee)+"px";
}}}}else{var ej=(qx.core.Variant.isSet("qx.client","mshtml"))?eo:er.focusNode;
var ep=qx.dom.Hierarchy.getAncestors(ej);

for(i=0;i<ep.length;i++){if(ep[i].tagName=="LI"){if(((qx.core.Variant.isSet("qx.client","gecko"))&&
((er.anchorOffset==0)&&
(er.focusNode.nodeValue&&(er.focusOffset==er.focusNode.nodeValue.length))&&
qx.dom.Hierarchy.contains(ep[i].firstChild,er.anchorNode)&&
qx.dom.Hierarchy.contains(ep[i].lastChild,er.focusNode)))||
((qx.core.Variant.isSet("qx.client","mshtml"))&&(em.htmlText==ep[i].innerHTML))){ep[i].style.fontSize=(this.__Ec[ee]||ee)+"px";
}break;
}}}if(qx.core.Variant.isSet("qx.client","mshtml")){this.__Ea.body.focus();
this.__DY.getRange().select();
return this.__Ea.execCommand("FontSize",false,ee);
}else if(qx.core.Variant.isSet("qx.client","gecko")){var parent=em.commonAncestorContainer;
if(parent.nodeType===1){var eh=parent.getElementsByTagName("span");

for(i=0;i<eh.length;i++){if(eh[i].style.fontSize){eh[i].style.fontSize=null;
}}}}return this.__Ea.execCommand("FontSize",false,ee);
},__EF:qx.core.Variant.select("qx.client",{"mshtml":function(dL,dM){this.__Ea.body.focus();
return this.__Ea.execCommand("BackColor",false,dL);
},"gecko|opera":function(fW,fX){this.__Ea.body.focus();
return this.__Ea.execCommand("HiliteColor",false,fW);
},"webkit":function(dE,dF){var dG=this.__DY.getSelection();
var dH=this.__DY.getRange();
if(!dG||!dG.isCollapsed){this.__Ea.body.focus();
this.__Ea.execCommand("BackColor",false,dE);
if(dG){dG.collapseToEnd();
}return true;
}else{var dJ=dG.anchorOffset;
var dK=dG.anchorOffset;
var dH=dG.getRangeAt(0);
var dI=dG.anchorNode;
while(dK>0){if(dI.nodeValue.charCodeAt(dK)==160||dI.nodeValue.charCodeAt(dK)==32){break;
}else{dK--;
}}while(dJ<dI.nodeValue.length){if(dI.nodeValue.charCodeAt(dJ)==160||dI.nodeValue.charCodeAt(dJ)==32){break;
}else{dJ++;
}}dH.setStart(dG.anchorNode,dG.anchorNode.nodeValue.charAt(dK)==" "?dK+1:dK);
dH.setEnd(dG.anchorNode,dJ);
dG.addRange(dH);
this.__Ea.body.focus();
this.__Ea.execCommand("BackColor",false,dE);
dG.collapseToEnd();
return true;
}}}),__EG:function(fL,fM){fL=fL!=null&&typeof fL=="string"?fL:"transparent";
qx.bom.element.Style.set(this.__Ea.body,"backgroundColor",fL);
return true;
},__EH:function(fu,fv){var fw,fy,fx;

if(fu==null){fw=null;
}else{fw=fu[0];
fy=fu[1];
fx=fu[2];
}if(fw==null||typeof fw!="string"){qx.bom.element.Style.set(this.__Ea.body,"backgroundImage","");
qx.bom.element.Style.set(this.__Ea.body,"backgroundRepeat","");
qx.bom.element.Style.set(this.__Ea.body,"backgroundPosition","");
return true;
}else{if(fw.search(/^url.*\(/)==-1){fw="url("+fw+")";
}}if(fy!=null&&qx.bom.htmlarea.manager.Command.__Ef.indexOf(fy)<0){{};
return false;
}else{fy="no-repeat";
}if(fx!=null&&qx.bom.htmlarea.manager.Command.__Ee.indexOf('|'+fx+'|')<0){{};
return false;
}else{if(!fx){fx="top";
}}qx.bom.element.Style.set(this.__Ea.body,"backgroundImage",fw);
qx.bom.element.Style.set(this.__Ea.body,"backgroundRepeat",fy);
qx.bom.element.Style.set(this.__Ea.body,"backgroundPosition",fx);
return true;
},__EI:qx.core.Variant.select("qx.client",{"mshtml":function(eD,eE){var eF=this.__Ea.body.createTextRange();
eF.select();
return true;
},"default":function(fl,fm){return this.__Ek(fm.identifier,false,fl);
}}),__EJ:function(){return this.__DY.getSelectedText();
},__EK:function(){return this.__DY.getSelectedHtml();
},__EL:qx.core.Variant.select("qx.client",{"webkit":function(gD,gE){var gF=this.__DY.getContextInformation();
var gH=this.__DY.getFocusNode();
if(gF.underline){gH.style.textDecoration="none";
}else{if(gF.strikethrough){var gI=this.__Ea.createElement("span");
qx.bom.element.Style.set(gI,"textDecoration","underline");
gH.appendChild(gI);
var gG=this.__DY.getSelection();

if(gG){gG.extend(gI,0);

if(!gG.isCollapsed){gG.collapseToEnd();
}}}else{gH.style.textDecoration="underline";
}}return true;
},"default":function(eG,eH){return this.__Ek(eH.identifier,false,eG);
}}),__EM:qx.core.Variant.select("qx.client",{"webkit":function(eX,eY){var fb=this.__DY.getFocusNode();
var fc=this.__Ea.createElement("span");
qx.bom.element.Style.set(fc,"textDecoration","line-through");
fb.appendChild(fc);
var fa=this.__DY.getSelection();

if(fa){fa.extend(fc,0);

if(!fa.isCollapsed){fa.collapseToEnd();
}}return true;
},"default":function(f,g){return this.__Ek(g.identifier,false,f);
}})},destruct:function(){this.__Ea=this.__DY=this._commands=this.__Eb=this.__Ec=null;
}});
})();
(function(){var bT="qx.client",bS="Command",bR="mshtml",bQ="gecko",bP="Custom",bO="Content",bN="inserthtml",bM="backgroundimage",bL="backgroundcolor",bK="webkit",cN="backgroundColor",cM="mousedown",cL="table",cK="backgroundImage",cJ="backgroundRepeat",cI="p",cH="backgroundPosition",cG="td",cF="img",cE="Internal",cb="inserthyperlink",cc="mouseup",bY="keypress",ca="qx.bom.htmlarea.HtmlArea.debug",bW="on",bX="undo",bU="pageup",bV="b",cf="control",cg="end",co="right",cm="qx_link",cv="insertunorderedlist",cq="Undo",cA="pagedown",cy="k",ci="home",cD="a",cC="insertorderedlist",cB="y",ch="justifyright",ck="redo",cl="insertParagraph",cn="qx.bom.htmlarea.manager.UndoRedo",cp="left",cr="i",cw="Redo",cz="u",cd="enter",ce="__EN",cj="insertimage",cu="z",ct="down",cs="up",cx="body";
qx.Class.define(cn,{extend:qx.core.Object,construct:function(bh,bi){arguments.callee.base.call(this);
this.__EN=bh;
this.__EO=bi;
this.__EP=[];
this.__EQ=[];
...
this.__ER=null;
this.__ES={};
this.__ET={command:true,content:true,custom:true};
this.__Fs();
this.__EU=qx.lang.Function.bind(this._handleKeyPress,this);
this.__EV=qx.lang.Function.bind(this._handleMouseUp,this);

if(qx.core.Variant.isSet(bT,bR)){this.__EW=qx.lang.Function.bind(this._handleMouseDown,this);
}},members:{__EX:false,__EY:false,__Fa:false,__ET:null,__ES:null,__EN:null,__ER:null,__EP:null,__EQ:null,__EO:null,__EU:null,__EV:null,__EW:null,__Fb:null,setContentDocument:function(bf){this.__ER=bf;
this.__EN.setContentDocument(bf);
qx.event.Registration.addListener(bf.body,bY,this.__EU,this);
qx.event.Registration.addListener(bf.body,cc,this.__EV,this);

if(qx.core.Variant.isSet(bT,bR)){qx.event.Registration.addListener(bf.body,cM,this.__EW,this,true);
}},insertParagraphOnLinebreak:function(){return this.__EN.insertParagraphOnLinebreak();
},execute:function(J,K){var L;
J=J.toLowerCase();
if(this._commands[J]){if(this._commands[J].passthrough){L=this.__EN.execute(J,K);
}else{L=this[J].call(this);
}}else{if(qx.core.Variant.isSet(bT,bR)||qx.core.Variant.isSet(bT,bK)){this.__Ft(J,K,this.__EN.getCommandObject(J));
L=this.__EN.execute(J,K);
if(!L){this.__EP.pop();
}}else{L=this.__EN.execute(J,K);

if(L){this.__Ft(J,K,this.__EN.getCommandObject(J));
}}
if(J==bX&&this.__EP.length==0){this.__EY=false;
this.__FB();
}else if(J==ck&&this.__EQ.length==0){this.__EX=false;
this.__FB();
}}this.__Fa=false;
return L;
},addUndoStep:function(cO,cP,cQ){this.__Ft(cO,cP,cQ);
},registerHandler:function(bb,bc,bd,be){this.__ES[bb]={undo:bc,redo:bd,context:be};
},isUndoPossible:function(){return this.__EY;
},undo:function(){var bJ;

if(this.__Fa){this.__Fv();
}if(this.__EP.length>0){var bI=this.__EP.pop();

if(this.__ET[bI.actionType.toLowerCase()]){switch(bI.actionType){case bS:bJ=this.__Fd(bI);
break;
case bO:bJ=this.__Fh(bI);
break;
case cE:bJ=this.__Fg(bI);
break;
case bP:bJ=this.__Fc(bI);
break;
}}else if(this.__ES[bI.actionType]){var bH=this.__ES[bI.actionType];
bJ=bH.undo.call(bH.context?bH.context:this,bI);
this.__Fx(bI);
}else{this.error("actionType "+bI.actionType+" is not managed! Please provide a handler method!");
}this.__EX=true;
this.__FB();
return bJ;
}},__Fc:qx.core.Variant.select(bT,{"mshtml|webkit":function(B){var C=this.__ER.body.innerHTML;
var E=B.content;
this.__ER.body.innerHTML=E;
var D=B;
D.content=C;
this.__Fx(D);
return true;
},"default":function(bq){var bu=bq;
var bt=qx.bom.element.Style;
switch(bq.command){case bL:bu.parameter=[bt.get(this.__ER.body,cN)];
break;
case bM:bu.parameter=[bt.get(this.__ER.body,cK),bt.get(this.__ER.body,cJ),bt.get(this.__ER.body,cH)];
break;
}this.__Fx(bu);
if(bq.command==cb){if(qx.core.Variant.isSet(bT,bQ)){var bs=cm+this.__EN.__hyperLinkId;
var br=this.__ER.getElementById(bs);

if(br){br.parentNode.removeChild(br);
return true;
}else{return false;
}}}else{return this.__EN.execute(bq.command,bq.value);
}}}),__Fd:qx.core.Variant.select(bT,{"mshtml|webkit":function(o){},"default":function(bv){this.__Fx(bv);

if(qx.core.Variant.isSet(bT,bQ)){if(bv.command==bN&&bv.value==qx.bom.htmlarea.HtmlArea.EMPTY_DIV&&this.__Fe(bN,cl)){this.__Ff();
}}return this.__Fi();
}}),__Fe:function(bw,bx){if(this.__EP.length>0){var by=this.__EP[this.__EP.length-1];
return (by.command==bw&&by.value==bx);
}return false;
},__Ff:function(){this.__Fi();

if(this.__EP.length>0){var bg=this.__EP.pop();
this.__Fx(bg);
}},__Fg:function(bG){this.__Fx(bG);
return this.__Fi();
},__Fh:qx.core.Variant.select(bT,{"gecko":function(A){this.__Fx(A);

try{return this.__Fi();
}catch(I){{};
}},"mshtml|webkit":function(k){},"default":function(bF){this.__Fx(bF);
return this.__Fi();
}}),__Fi:function(){try{return this.__ER.execCommand(cq,false,null);
}catch(e){return false;
}},isRedoPossible:function(){return this.__EX;
},redo:function(){if(this.__EX){var bD;
if(this.__EQ.length>0){var bE=this.__EQ.pop();

if(this.__ET[bE.actionType.toLowerCase()]){switch(bE.actionType){case bS:bD=this.__Fk(bE);
break;
case bO:bD=this.__Fq(bE);
break;
case cE:bD=this.__Fp(bE);
break;
case bP:bD=this.__Fj(bE);
break;
}}else if(this.__ES[bE.actionType]){var bC=this.__ES[bE.actionType];
bD=bC.redo.call(bC.context?bC.context:this,bE);
this.__Fw(bE);
}else{this.error("actionType "+bE.actionType+" is not managed! Please provide a handler method!");
}this.__EY=true;
this.__FB();
}return bD;
}},__Fj:qx.core.Variant.select(bT,{"mshtml|webkit":function(d){var f=this.__ER.body.innerHTML;
var h=d.content;
this.__ER.body.innerHTML=h;
var g=d;
g.content=f;
this.__Fw(g);
return true;
},"default":function(M){this.__Fw(M);
return this.__Fr();
}}),__Fk:qx.core.Variant.select(bT,{"mshtml|webkit":function(P){},"default":function(bz){this.__Fw(bz);
var bA=this.__Fr();

if(qx.core.Variant.isSet(bT,bQ)){if(this.__Fl(bN,qx.bom.htmlarea.HtmlArea.EMPTY_DIV)){var bB=this.__Fm();
this.__Fn();

if(bB!=null){this.__Fo(bB);
}}}return bA;
}}),__Fl:function(R,S){if(this.__EQ.length>0){var T=this.__EQ[this.__EQ.length-1];
return (T.command==R&&T.value==S);
}return false;
},__Fm:function(){if(this.__EO==null){return null;
}var y=this.__EO.getSelection();
var z=y?y.focusNode:null;

if(z==null){return null;
}
try{while(z.nodeName.toLowerCase()!=cI){z=z.parentNode;

if(!z||qx.dom.Node.isNodeName(z,cx)){return null;
}}}catch(N){return null;
}
if(z!=null&&qx.dom.Node.isNodeName(z,cI)){return z;
}else{return null;
}},__Fn:function(){var a=this.__EQ.pop();
this.__Fw(a);
this.__Fr();
},__Fo:qx.core.Variant.select(bT,{"gecko":function(t){if(t==this.__EO.getContentBody().lastChild){return;
}var w=t.firstChild;

while(w.firstChild){w=w.firstChild;
}var u=this.__EO.getSelection();
var v=this.__EO.getRange();

if(u&&v){v.selectNode(w);
u.addRange(v);
v.collapse(true);
}},"default":qx.lang.Function.empty}),__Fp:function(O){this.__Fw(O);
return this.__Fr();
},__Fq:qx.core.Variant.select(bT,{"mshtml|webkit":function(V){},"default":function(x){this.__Fw(x);
return this.__Fr();
}}),__Fr:function(){try{return this.__ER.execCommand(cw,false,null);
}catch(e){return false;
}},__Fs:function(){this._commands={undo:{passthrough:false},redo:{passthrough:false}};
this.__EN.getCommandObject(bL).customUndo=true;
this.__EN.getCommandObject(bM).customUndo=true;

if(qx.core.Variant.isSet(bT,bQ)){}},__Ft:qx.core.Variant.select(bT,{"mshtml|webkit":function(p,q,r){var s=this.getUndoRedoObject();
s.commandObject=r;
s.command=p;
s.value=q;
s.actionType=bP;
s.content=this.__ER.body.innerHTML;
this.__Fu(s);
},"default":function(bk,bl,bm){if(this.__EO==null){return;
}var bo=this.getUndoRedoObject();
bo.commandObject=bm;
bo.command=bk;
bo.value=bl;
bo.actionType=bP;
var bn=this.__EO.getSelection();

if(bm.customUndo){var bp=[];

switch(bk){case bL:bp.push(qx.bom.element.Style.get(this.__ER.body,cN));
break;
case bM:bp.push(qx.bom.element.Style.get(this.__ER.body,cK),qx.bom.element.Style.get(this.__ER.body,cJ),qx.bom.element.Style.get(this.__ER.body,cH));
break;
case cb:if(bn&&!bn.isCollapsed){bo.actionType=bS;
}break;
}bo.parameter=bp;
}else{if(qx.core.Variant.isSet(bT,bQ)){if(bn&&bn.isCollapsed){switch(bk){case cC:case cv:case ch:case bN:case cj:bo.actionType=bS;
break;
default:return;
}}else{bo.actionType=bS;
}}else{bo.actionType=bS;
}}this.__Fu(bo);
}}),__Fu:function(F){if(this.__Fa){this.__Fv();
}this.__Fw(F);
this.__EX=false;
this.__EQ=[];
this.__FB();
},__Fv:function(){var G=this.__EP[this.__EP.length-1];

if(G==null||G.actionType!=bO){var H=this.getUndoRedoObject();
H.actionType=bO;

if(qx.core.Variant.isSet(bT,bR)||qx.core.Variant.isSet(bT,bK)){H.content=this.__Fb;
H.actionType=bP;
this.__Fb=null;
}this.__Fw(H);
this.__Fa=false;
...
},__Fw:function(U){if(false&&qx.core.Setting.get(ca)==bW){this.debug("ADD TO UNDO STACK");
this.debug(U.actionType+" "+U.command+" "+U.value);
}this.__EP.push(U);
},__Fx:function(Q){if(false&&qx.core.Setting.get(ca)==bW){this.debug("ADD TO REDO STACK");
this.debug(Q.actionType+" "+Q.command+" "+Q.value);
}this.__EQ.push(Q);
},_handleKeyPress:function(e){var m=e.getKeyIdentifier().toLowerCase();
var n=e.isCtrlPressed();

switch(m){case cf:case cp:case co:case cs:case ct:case bU:case cA:case ci:case cg:case cd:break;
case cD:case bV:case cr:case cz:case cy:case cB:case cu:if(!n){this.__Fy();
}break;
default:this.__EX=false;
this.__EQ=[];
this.__Fy();
}},__Fy:function(){if(!this.__Fa){this.__Fa=true;
this.__EY=true;
if(qx.core.Variant.isSet(bT,bR)||qx.core.Variant.isSet(bT,bK)){this.__Fb=this.__ER.body.innerHTML;
}this.__FB();
}},__Fz:null,_handleMouseDown:qx.core.Variant.select(bT,{"mshtml":function(e){var b=e.getOriginalTarget();

if(qx.dom.Node.isElement(b)&&(qx.dom.Node.isNodeName(b,cF)||qx.dom.Node.isNodeName(b,cL))){this.__Fz={node:b,content:b.outerHTML};
}else{this.__Fz=null;
}},"default":function(e){return true;
}}),_handleMouseUp:qx.core.Variant.select(bT,{"gecko":function(e){if(this.__EO==null){return;
}var X=this.__EO.getSelection();

if(!X){this.__Fz=null;
return;
}var W=X.anchorNode;
var Y=W.childNodes[X.anchorOffset];
if(qx.dom.Node.isNodeName(Y,cF)){if(this.__Fz==null){this.__Fz=Y.cloneNode(true);
}else{if(this.__Fz.style.width!=Y.style.width||this.__Fz.style.height!=Y.style.height){this.__FA();
this.__Fz=Y.cloneNode(true);
}}}else if(qx.dom.Node.isNodeName(W,cG)||qx.dom.Node.isNodeName(W.parentNode,cG)){var ba=W.parentNode;

while(qx.dom.Node.isNodeName(ba,cL)){ba=ba.parentNode;
}if(this.__Fz==null){this.__Fz=ba.cloneNode(true);
}else{qx.event.Timer.once(function(){if(ba.style.width!=this.__Fz.style.width||ba.style.height!=this.__Fz.style.height||ba.innerHTML!=this.__Fz.innerHTML){this.__FA();
this.__Fz=ba.cloneNode(true);
}},this,0);
}}else{this.__Fz=null;
}},"default":function(e){var bj=e.srcElement;

if(this.__Fz!=null){if(bj.nodeType==1){if(bj==this.__Fz.node){if(bj.outerHTML!=this.__Fz.content){this.__Fz.content=bj.outerHTML;
this.__FA();
}}else{for(var i=0,j=bj.childNodes.length;i<j;i++){if(bj.childNodes[i]==this.__Fz.node){if(bj.childNodes[i].outerHTML!=this.__Fz.content){this.__Fz.content=bj.childNodes[i].outerHTML;
this.__FA();
}}}}}else{this.__Fz=null;
}}}}),__FA:qx.core.Variant.select("qx.client",{"mshtml|webkit":function(){this.__Ft("Internal",null,null);
},"default":function(){var l=this.getUndoRedoObject();
l.actionType="Internal";
this.__Fw(l);
}}),__FB:function(){qx.event.Timer.once(function(e){if(this.__EO!=null){var c={undo:this.isUndoPossible()?0:-1,redo:this.isRedoPossible()?0:-1};
this.__EO.fireDataEvent("undoRedoState",c);
}},this,200);
}},destruct:function(){try{qx.event.Registration.removeListener(this.__ER.body,bY,this.__EU);
qx.event.Registration.removeListener(this.__ER,cc,this.__EV);

if(qx.core.Variant.isSet(bT,bR)){qx.event.Registration.removeListener(this.__ER,cM,this.__EW);
}}catch(e){}this._disposeObjects(ce);
this.__EO=this.__EP=this.__EQ=this._commands=this.__ER=null;
