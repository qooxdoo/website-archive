...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.data.FormController-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"0f657f67ae9b"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var v="modelSelection[0]",u="value",t="changeModel",s="qx.core.Object",r="_applyTarget",q="qx.data.controller.Form",p="changeTarget",o="qx.ui.form.Form",n="_applyModel";
qx.Class.define(q,{extend:qx.core.Object,construct:function(y,z){arguments.callee.base.call(this);
this.__tE={};

if(y!=null){this.setModel(y);
}
if(z!=null){this.setTarget(z);
}},properties:{model:{check:s,apply:n,event:t,nullable:true},target:{check:o,apply:r,event:p,nullable:true,init:null}},members:{__tF:null,__tE:null,addBindingOptions:function(name,b,c){this.__tE[name]=[b,c];
if(this.getModel()==null||this.getTarget()==null){return;
}var d=this.getTarget().getItems()[name];
var e=this.__tI(d)?v:u;
this.__tF.removeTarget(d,e,name);
this.__tF.addTarget(d,e,name,true,b,c);
},createModel:function(f){var g=this.getTarget();
if(g==null){throw new Error("No target is set.");
}var h=g.getItems();
var k={};

for(var name in h){var i=h[name].constructor;

if(qx.Class.hasInterface(i,qx.ui.core.ISingleSelection)){k[name]=h[name].getModelSelection();
}else{k[name]=h[name].getValue();
}}var j=qx.data.marshal.Json.createModel(k,f);
this.setModel(j);
return j;
},_applyTarget:function(l,m){if(m!=null){this.__tH(m);
}if(this.getModel()==null){return;
}if(l!=null){this.__tG();
}},_applyModel:function(w,x){if(this.__tF!=null){this.__tF.setModel(w);
}if(this.getTarget()==null){return;
}if(w!=null){this.__tG();
}},__tG:function(){if(this.__tF==null){this.__tF=new qx.data.controller.Object(this.getModel());
}var A=this.getTarget().getItems();
for(var name in A){var D=A[name];
var B=this.__tI(D)?v:u;
var C=this.__tE[name];

if(C==null){this.__tF.addTarget(D,B,name,true);
}else{this.__tF.addTarget(D,B,name,true,C[0],C[1]);
}}},__tH:function(E){if(this.__tF==null){return;
}var F=E.getItems();
for(var name in F){var H=F[name];
var G=this.__tI(H)?v:u;
this.__tF.removeTarget(H,G,name);
}},__tI:function(a){return qx.Class.hasInterface(a.constructor,qx.ui.core.ISingleSelection)&&qx.Class.hasInterface(a.constructor,qx.ui.form.IModelSelection);
}}});
})();
