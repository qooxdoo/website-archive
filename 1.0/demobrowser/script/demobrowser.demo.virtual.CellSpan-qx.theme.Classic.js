...
(function(){var q="yellow",p="bold",o="table-scroller-header",n="green",m=" / ",l="Widget Cells",k="__Jz",j="demobrowser.demo.virtual.CellSpan",i="HTML Cells",h="x";
qx.Class.define(j,{extend:qx.application.Standalone,members:{__Jz:null,main:function(){arguments.callee.base.call(this);
var v=new qx.ui.layout.Grid(5,0);
v.setRowFlex(1,1);
v.setColumnFlex(0,1);
v.setColumnFlex(1,1);
var u=new qx.ui.container.Composite(v);
this.getRoot().add(u,{edge:0});
u.add(new qx.ui.basic.Label(i).set({font:p,decorator:o,padding:3,allowGrowX:true}),{row:0,column:0});
var w=this.getHtmlCellSpanScroller();
u.add(w,{row:1,column:0});
u.add(new qx.ui.basic.Label(l).set({font:p,decorator:o,padding:3,allowGrowX:true}),{row:0,column:1});
var w=this.getWidgetCellSpanScroller();
u.add(w,{row:1,column:1});
this.__Jz=new qx.ui.virtual.cell.Cell();
...
return z;
},getCellProperties:function(e,f){var g=(e+f)%2==0?q:n;
this.__Jz.setBackgroundColor(g);
return this.__Jz.getCellProperties(e+m+f);
},getWidgetCellSpanScroller:function(){var t=new qx.ui.virtual.core.Scroller(1000,100,50,120);
var s=t.getPane();
var r=new qx.ui.virtual.layer.WidgetCellSpan(this,s.getRowConfig(),s.getColumnConfig());
...
this._pool=[];
return t;
},getCellWidget:function(b,c){var d=this._pool.pop()||new qx.ui.basic.Label().set({allowGrowX:true});
d.set({backgroundColor:(b+c)%2==0?q:n,value:b+h+c,padding:3});
return d;
},poolCellWidget:function(a){this._pool.push(a);
...
qx.Class.define(f,{extend:qx.ui.virtual.layer.HtmlCell,construct:function(q,r,s){arguments.callee.base.call(this,q);
...
},__zJ:function(A,B,C,D,top,E,F){var H=this._cellProvider.getCellProperties(B,C);
var G=H.insets||[0,0];
...
var W=R[i];
this.__zJ(U,S.firstRow,S.firstColumn,W.left,W.top,W.width,W.height);
}}else{O=[];
...
if(!O[N][T]){this.__zJ(U,N,T,V,top,P,M);
}T++;
V+=P;
}top+=M;
N++;
...
(function(){var w="change",v="lastColumn",u="firstColumn",t="firstRow",s="qx.ui.virtual.layer.CellSpanManager",r="lastRow";
qx.Class.define(s,{extend:qx.core.Object,construct:function(N,O){arguments.callee.base.call(this);
{};
this._cells={};
this._invalidateSortCache();
this._invalidatePositionCache();
N.addListener(w,this._onRowConfigChange,this);
O.addListener(w,this._onColumnConfigChange,this);
this._rowConfig=N;
this._columnConfig=O;
},members:{addCell:function(be,bf,bg,bh,bi){this._cells[be]={firstRow:bf,lastRow:bf+bh-1,firstColumn:bg,lastColumn:bg+bi-1,id:be};
this._invalidateSortCache();
},removeCell:function(bd){delete (this._cells[bd]);
this._invalidateSortCache();
},_invalidateSortCache:function(){this._sorted={};
},_getSortedCells:function(p){if(this._sorted[p]){return this._sorted[p];
}var q=this._sorted[p]=qx.lang.Object.getValues(this._cells);
q.sort(function(a,b){return a[p]<b[p]?-1:1;
});
return q;
},_findCellsInRange:function(P,Q,R){var T=this._getSortedCells(P);

if(T.length==0){return {};
}var V=0;
var S=T.length-1;
while(true){var X=V+((S-V)>>1);
var U=T[X];

if(U[P]>=Q&&(X==0||T[X-1][P]<Q)){break;
}
if(U[P]>=Q){S=X-1;
}else{V=X+1;
}
...
while(U&&U[P]>=Q&&U[P]<=R){W[U.id]=U;
U=T[X++];
}return W;
},findCellsInWindow:function(bl,bm,bn,bo){var bs={};

if(bn>0){var bq=bl+bn-1;
qx.lang.Object.merge(bs,this._findCellsInRange(t,bl,bq),this._findCellsInRange(r,bl,bq));
}var bp={};

if(bo>0){var br=bm+bo-1;
qx.lang.Object.merge(bp,this._findCellsInRange(u,bm,br),this._findCellsInRange(v,bm,br));
}return this.__xq(bp,bs);
},__xq:function(c,d){var f=[];
...
this._columnPos=[];
},_getRowPosition:function(bj){var bk=this._rowPos[bj];

if(bk!==undefined){return bk;
}bk=this._rowPos[bj]=this._rowConfig.getItemPosition(bj);
return bk;
},_getColumnPosition:function(n){var o=this._columnPos[n];

if(o!==undefined){return o;
}o=this._columnPos[n]=this._columnConfig.getItemPosition(n);
return o;
},_getSingleCellBounds:function(h,j,k){var m={left:0,top:0,width:0,height:0};
m.height=this._getRowPosition(h.lastRow)+this._rowConfig.getItemSize(h.lastRow)-this._getRowPosition(h.firstRow);
m.top=this._getRowPosition(h.firstRow)-this._getRowPosition(j);
m.width=this._getColumnPosition(h.lastColumn)+this._columnConfig.getItemSize(h.lastColumn)-this._getColumnPosition(h.firstColumn);
m.left=this._getColumnPosition(h.firstColumn)-this._getColumnPosition(k);
return m;
},getCellBounds:function(Y,ba,bb){var bc=[];

for(var i=0,l=Y.length;i<l;i++){bc.push(this._getSingleCellBounds(Y[i],ba,bb));
}return bc;
},computeCellSpanMap:function(x,y,z,A,B){var C=[];

if(A<=0){return C;
}var L=y+A-1;

for(var i=y;i<=L;i++){C[i]=[];
}
if(B<=0){return C;
}var E=z+B-1;

for(var i=0,l=x.length;i<l;i++){var H=x[i];
var F=Math.max(y,H.firstRow);
var G=Math.min(L,H.lastRow);
var M;

for(var I=F;I<=G;I++){M=C[I];
var D=Math.max(z,H.firstColumn);
var K=Math.min(E,H.lastColumn);

for(var J=D;J<=K;J++){M[J]=1;
}}}return C;
}},destruct:function(){this._rowConfig.removeListener(w,this._onRowConfigChange,this);
this._columnConfig.removeListener(w,this._onColumnConfigChange,this);
...
qx.Class.define(g,{extend:qx.ui.virtual.layer.Abstract,include:[qx.ui.core.MChildrenHandling],construct:function(u,v,w){arguments.callee.base.call(this);
this.setZIndex(2);
this._spanManager=new qx.ui.virtual.layer.CellSpanManager(v,w);
this._cellProvider=u;
this.__xm=[];
this._cellLayer=new qx.ui.virtual.layer.WidgetCell(this.__xn());
...
if(!G||G.getUserData(o)){var J=this._getChildren();

for(var i=0,l=J.length;i<l;i++){var I=J[i];

if(I==this._cellLayer){continue;
}var H={row:I.getUserData(j),column:I.getUserData(m),rowSpan:I.getUserData(n),colSpan:I.getUserData(k)};

if(H.row<=E&&E<H.row+H.rowSpan&&H.column<=F&&F<H.column+H.colSpan){return I;
}}return null;
}return G;
},__xm:null,setCellSpan:function(O,P,Q,R){var S=O+h+P;
this._spanManager.removeCell(S);

if(Q>1||R>1){this._spanManager.addCell(S,O,P,Q,R);
}qx.ui.core.queue.Widget.add(this);
},__xn:function(){var self=this;
var c=this._cellProvider;
var b=this.__xm;
var a={getCellWidget:function(x,y){if(!self._spanMap[x][y]){var z=c.getCellWidget(x,y);
}else{var z=b.pop();

if(!z){z=new qx.ui.core.Spacer();
z.setUserData(o,1);
}}return z;
},poolCellWidget:function(T){if(T.getUserData(o)){b.push(T);
}else{c.poolCellWidget(T);
}}};
return a;
},__xo:function(U,V,W,X){this._cells=this._spanManager.findCellsInWindow(U,V,W,X);
...
}}},__xp:function(){var t=this.getChildren();
...
q.setUserData(j,r.firstRow);
q.setUserData(m,r.firstColumn);
q.setUserData(n,r.lastRow-r.firstRow+1);
q.setUserData(k,r.lastColumn-r.firstColumn+1);
this._add(q);
}}},_fullUpdate:function(K,L,M,N){this.__xo(K,L,M.length,N.length);
this.__xp();
this._cellLayer.fullUpdate(K,L,M,N);
},_updateLayerWindow:function(A,B,C,D){this.__xo(A,B,C.length,D.length);
this.__xp();
this._cellLayer.updateLayerWindow(A,B,C,D);
}},destruct:function(){var d=this._getChildren();

for(var i=0;i<d.length;i++){d[i].dispose();
}this._disposeObjects(f,e);
this._cellProvider=this.__xm=this._cells=this._bounds=this._spanMap=null;
}});
})();
