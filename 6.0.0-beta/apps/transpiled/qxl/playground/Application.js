  ...
  qx.Class.define("qxl.playground.Application", {
    ...
    */
    members: {
      ...
      __afterReg: null,
      __oldCode: null,
      __errorMsg: qx.locale.Manager.tr("Unfortunately, an unrecoverable internal error was caused by your code. This may prevent the playground application to run properly.||"),
      __mode: null,
      __maximized: null,
      ...
      main: function main() {
        ...
        qx.event.GlobalError.setErrorHandler(this.__onGlobalError, this); // container layout
        ...
        this.__header.addListener("changeMode", this._onChangeMode, this); // toolbar


        this.__toolbar = new qxl.playground.view.Toolbar();
        mainContainer.add(this.__toolbar, {
          flex: 0
        }); // toolbar listener
        ...
        this.__toolbar.addListener("openDemoBrowser", this.__onDemoBrowser, this); // mainsplit, contains the editor splitpane and the info splitpane


        this.__mainsplit = new qx.ui.splitpane.Pane("horizontal");
        mainContainer.add(this.__mainsplit, {
          flex: 1
        });

        this.__mainsplit.setAppearance("app-splitpane"); // editor split (left side of main split)
        ...
        this.__store = new qx.data.store.Offline("qooxdoo-playground-samples"); // if the local storage is not empty
        ...
        this.__store.bind("model", this.__samplesPane, "model"); // need to split up the creation process
       ...
       */
      init: function init() {
        this.__editor.init(); // check if mobile chould be used
        ...
        this.__initBookmarkSupport(); // check for the highlight and examples cookie
      ...
      _applyCurrentSample: function _applyCurrentSample(newSample, old) {
        ...
        this.setOriginCode(this.__editor.getCode()); // only add static samples to the url as name
      ...
      // MODE HANDLING
      // ***************************************************
      __enableWebsiteMode: function __enableWebsiteMode(enabled) {
        if (enabled) {
        ...
        if (!this.__websiteContent && enabled) {
          ...
          this.getRoot().getChildren()[0].add(this.__websiteContent, {
            flex: 1
          });
        }
       ...
       */
      __supportsMode: function __supportsMode(mode) {
        if (mode == "mobile") {
      ...
      setMode: function setMode(mode) {
        ...
        this.__enableWebsiteMode(mode == "website"); // update the views (changes the play application)
        ...
        this.__samplesPane.setMode(mode); // erase the code
      ...
      __onSave: function __onSave() {
        ...
        if (!current || current.getCategory() == "static") {
          this.__onSaveAs(); // if its a user sample which is selected, we just store the new code
       ...
       */
      __onSaveAs: function __onSaveAs() {
        ...
        this.__store.getModel().push(sample); // store the origin code and select the new sample
       ...
       */
      __onDelete: function __onDelete() {
        var current = this.getCurrentSample(); // if we have a sample selected and its not a static one

        if (current || current.getCategory() != "static") {
          // remove the selection
          this.__samplesPane.select(null); // delete the current sample


          this.__store.getModel().remove(current); // reset the current selected sample
       ...
       */
      __onRename: function __onRename() {
       ...
       */
      __onSampleChange: function __onSampleChange(e) {
       ...
       */
      __onHighlightChange: function __onHighlightChange(e) {
       ...
       *   be shown.
       */
      __onLogChange: function __onLogChange(e) {
       ...
       */
      __onUrlShorten: function __onUrlShorten() {
      ...
      __onApiOpen: function __onApiOpen() {
        window.open(qx.core.Environment.get("/apps/") + "qxl.apiviewer/");
      },
       ...
       */
      __onDemoBrowser: function __onDemoBrowser() {
        window.open("/apps/" + "qxl.widgetbrowser/");
      },
      // ***************************************************
      // HISTORY SUPPORT
      // ***************************************************
       ...
       * Back button and bookmark support
       */
      __initBookmarkSupport: function __initBookmarkSupport() {
        this.__history = qx.bom.History.getInstance();

        this.__history.addListener("changeState", this.__onHistoryChanged, this); // Handle bookmarks
        ...
        if (state && this.__samples.isAvailable(state)) {
          ...
          code = this.__parseURLCode(state); // need to get the code from the editor in case he changes something
          // in the code

          this.__editor.setCode(code);

          this.setOriginCode(this.__editor.getCode()); // try to select a custom sample
       ...
       */
      __onHistoryChanged: function __onHistoryChanged(e) {
       ...
       */
      __parseURLCode: function __parseURLCode(state) {
        try {
       ...
       * @lint ignoreDeprecated(confirm)
       */
      __addCodeToHistory: function __addCodeToHistory(code) {
        var codeJson = "{\"code\":\"" + encodeURIComponent(code) + '", "mode":"' + this.__mode + '"}';

        if (qx.core.Environment.get("engine.name") == "mshtml" && codeJson.length > 1300) {
          if (!this.__ignoreSaveFaults && confirm(this.tr("Cannot append sample code to URL, as it is too long. Disable this warning in the future?"))) {
       ...
       */
      __discardChanges: function __discardChanges() {
       ...
       */
      __isCodeNotEqual: function __isCodeNotEqual(code1, code2) {
       ...
       */
      __updateTitle: function __updateTitle(label) {
       ...
       */
      __updatePlayground: function __updatePlayground() {
        this.__log.clear();

        this.__playArea.reset(this.__beforeReg, this.__afterReg, this.__oldCode);

        var reg = qx.Class.$$registry;
        delete reg[this.__currentStandalone]; // build the code to run

        var code = this.__editor.getCode(); // special replacement for unicode "zero width space" [BUG #3635]
        ...
        try {
          ...
          this.__beforeReg = qx.lang.Object.clone(qx.core.ObjectRegistry.getRegistry()); // run the application
       ...
       */
      __onGlobalError: function __onGlobalError(e) {
        this.error(e);
      },
      // ***************************************************
      // STANDALONE SUPPORT
       ...
       */
      __isAppClass: function __isAppClass(name) {
       ...
       */
      __executeStandaloneApp: function __executeStandaloneApp(name) {
        var self = this;
    ...
    destruct: function destruct() {
      ...
      this._disposeObjects("__currentStandalone", "__samples", "__toolbar", "__editor", "__playArea", "__log");
    }
  });
  qxl.playground.Application.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=Application.js.map?dt=1562187674185