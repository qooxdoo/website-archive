(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.playground.Samples", {
     ...
     *   added.
     */
    construct: function construct(array) {
      qx.core.Object.constructor.call(this); // initialize the model

      if (!array) {
        array = new qx.data.Array();
      }

      this.setModel(array); // remove all stored static samples
    ...
    members: {
       ...
       */
      get: function get(nameandmode) {
        // split the name and mode into separate parts
        var name = nameandmode.split("-")[0];
        var mode = nameandmode.split("-")[1]; // get the data

        var model = this.getModel();

        for (var i = 0; i < model.length; i++) {
          var sample = model.getItem(i); // check if name AND mode are fitting
       ...
       */
      getFirstSample: function getFirstSample(mode) {
        var model = this.getModel();
       ...
       */
      isAvailable: function isAvailable(nameandmode) {
        var name = nameandmode.split("-")[0];
...
//# sourceMappingURL=Samples.js.map?dt=1562512206390