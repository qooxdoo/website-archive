(function () {
  ...
  ************************************************************************ */
   ...
   */
  qx.Class.define("qxl.demobrowser.demo.bom.Selector", {
    ...
    members: {
       ...
       */
      main: function main() {
        qxl.demobrowser.demo.bom.Selector.prototype.main.base.call(this);
        ...
        q("a").setAttribute("title", "Tap to follow the link"); // Change classes

        q("h1,h2").addClass("header"); // Add some events
        ...
        q().concat(q("h2")).setStyle("color", "orange").concat(q("li")).setStyle("backgroundColor", "#eee"); // CHILDREN
        // Select all "li" items from all ul/ol elements.
        // Yes this could be done with purely selector magic as well :)

        Logger.debug("Found " + q("ul,ol").getChildren().length + " children of ul/ol elements using children()."); // It is even possible to filter them

        Logger.debug("Found " + q("ul,ol").getChildren(":first-child").length + " first children of ul/ol elements using children(selector)"); // CLOSEST
        // All even 'li' elements and their next ul or ol parent
        // Do not make a lot of sense using this way, though

        Logger.debug("Number of ul/ol elements which are closest to a li element: " + q("li").getClosest("ul,ol").length); // CONTENTS
        // Detect children number (incl. text nodes)

        Logger.debug("Number of child nodes of the body: " + q(document.body).getContents().length); // FIND
        // Fast path

        var res = q("ul").find("li");
        Logger.debug("Found " + res.length + " li elements in uls using find()"); // Testing find() for complexer cases

        var res = q(document.body).find("h1,h2");
        Logger.debug("Found " + res.length + " headers using find()"); // NEXT

        var res = q("ul").getNext()[0].tagName;
        Logger.debug("Tag following the UL element: " + res); // NEXT ALL

        var res = q("ul").getNextAll().length;
        Logger.debug("Number of elements following the UL: " + res); // PARENT

        var res = q("li").getParents().length;
        Logger.debug("Number of parent elements of all li elems: " + res); // PARENTS

        var res = q("input[type=submit]").getAncestors().length;
        Logger.debug("Number of all parent elements of the submit button: " + res); // PREV

        var res = q("ul").getPrev()[0].tagName;
        Logger.debug("Tag before the UL element: " + res); // PREV ALL

        var res = q("ul").getPrevAll().length;
        Logger.debug("Number of elements preceding the UL: " + res); // SIBLINGS
        ...
        Logger.debug("Offset parent of select box: " + res); // OFFSET
        ...
        q("li a").eq(1).setStyle("fontWeight", "bold"); // FILTER
        // Filtering the collection using a selector

        var res = q("input").filter("[type=submit]");
        Logger.debug("Found " + res.length + " submit buttons using filter(selector)"); // or a function

        var res = q("input").filter(function (item, index, array) {
          return item.type === "submit";
        });
        Logger.debug("Found " + res.length + " submit button using filter(function)"); // HASCLASS

        var res = q("h2").hasClass("form");
        Logger.debug("Any H2 with form has a class 'form': " + res); // IS

        var res = q("ul").find("li").is(":contains('List Item 1')");
        Logger.debug("Found li element with text 'List Item 1': " + res); // NOT
        ...
        q.create("<option>Mr</option><option>Mrs</option><option>Dr</option><option>Prof</option>").appendTo(select); // testing noConflict alias
  ...
  qxl.demobrowser.demo.bom.Selector.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=Selector.js.map?dt=1562187675133