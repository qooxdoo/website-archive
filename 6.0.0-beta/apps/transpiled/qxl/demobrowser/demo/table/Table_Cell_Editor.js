  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.demobrowser.demo.table.Table_Cell_Editor", {
    ...
    members: {
      ...
      main: function main() {
        qxl.demobrowser.demo.table.Table_Cell_Editor.prototype.main.base.call(this);
      ...
      createTable: function createTable() {
        ...
        var tableData = [['username', 'User Name', 'jdoe', {
          'required': true
        }], ['password', 'Password', 'secret', {
          'type': 'password',
          'required': true
        }], ['role', 'Role', 'editable combobox', {
          'options': ['admin', 'manager', 'user'],
          'editable': true
        }], ['status', 'Status', '0', {
          'options': [['Inactive', null, '0'], ['Active', null, '1'], ['Waiting', null, '2']]
        }], ['email', 'Email', 'enter valid email', {
          'type': 'email'
        }], ['telephone', 'Telephone', 'enter valid telephone number', {
          'regExp': '[0-9]+',
          'failMsg': 'Use only numbers!'
        }], ['website', 'Website', 'enter website', {
          'validationFunc': function validationFunc(newValue) {
        ...
        var propertyCellRendererFactoryFunc = function propertyCellRendererFactoryFunc(cellInfo) {
        ...
        var propertyCellRendererFactory = new qx.ui.table.cellrenderer.Dynamic(propertyCellRendererFactoryFunc); // cell editor factory function
        // returns a cellEditorFactory instance based on data in the row itself

        var propertyCellEditorFactoryFunc = function propertyCellEditorFactoryFunc(cellInfo) {
          var table = cellInfo.table;
          var tableModel = table.getTableModel();
          var rowData = tableModel.getRowData(cellInfo.row);
          var metaData = rowData[3];
          ...
          for (var cmd in metaData) {
            switch (cmd) {
              ...
              case "required":
                validationFunc = function validationFunc(newValue, oldValue) {
                  if (!newValue) {
        ...
        var propertyCellEditorFactory = new qx.ui.table.celleditor.Dynamic(propertyCellEditorFactoryFunc); // create table

        var propertyEditor_tableModel = new qx.ui.table.model.Simple();
        propertyEditor_tableModel.setColumns(['Property', 'Property', 'Doubletap on cell to edit value']);
        var propertyEditor_resizeBehaviour = {
          tableColumnModel: function tableColumnModel(obj) {
            return new qx.ui.table.columnmodel.Resize(obj);
          }
        };
        var propertyEditor = new qx.ui.table.Table(propertyEditor_tableModel, propertyEditor_resizeBehaviour); // remove decor

        propertyEditor.setDecorator(null); // layout

        propertyEditor.setColumnVisibilityButtonVisible(false);
        propertyEditor.setKeepFirstVisibleRowComplete(true);
        propertyEditor.setStatusBarVisible(false); // selection mode

        propertyEditor.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION); // Get the table column model

        var tcm = propertyEditor.getTableColumnModel(); // first table columns is not visible, has the key

        tcm.setColumnVisible(0, false); // second column has the label

        tcm.getBehavior().setWidth(1, 100); // third column for editing the value and has special cell renderers
        // and cell editors
        ...
        tcm.setCellEditorFactory(2, propertyCellEditorFactory); // fourth column is not visible, has the metadata
        // set data

        propertyEditor.getTableModel().setData(tableData); // create event listener for data change event. this would normally
        ...
        propertyEditor.getTableModel().addListener("dataChanged", function (event) {
          ...
          var changedData = event.getData(); // get changed data
...
//# sourceMappingURL=Table_Cell_Editor.js.map?dt=1562140824913