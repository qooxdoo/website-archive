(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.demobrowser.demo.widget.ToolBar", {
    ...
    members: {
      main: function main() {
        qxl.demobrowser.demo.widget.ToolBar.prototype.main.base.call(this); // create a container for the main layout and set the main layout

        var mainContainer = new qx.ui.container.Composite(new qx.ui.layout.VBox(20));
        mainContainer.setPadding(20); // add the scroll container to the root
        ...
        mainContainer.add(toolbar); // create and add Part 1 to the toolbar
        ...
        toolbar.add(part1); // create and add Part 2 to the toolbar

        var part2 = new qx.ui.toolbar.Part();
        var checkBtn = new qx.ui.toolbar.CheckBox("Toggle", "icon/22/actions/format-text-underline.png");
        part2.add(checkBtn);
        toolbar.add(part2); // create and add Part 3 to the toolbar
        ...
        toolbar.add(part3); // Manager for part 3 (Radio example)

        var radioGroup = new qx.ui.form.RadioGroup(radioButton1, radioButton2, radioButton3);
        radioGroup.setAllowEmptySelection(true);
        toolbar.addSpacer(); // add a widget which signals that something has been removed

        var overflow = new qx.ui.toolbar.MenuButton("More...");
        toolbar.add(overflow);
        toolbar.setOverflowIndicator(overflow); // create Help Button and add it to the toolbar

        var helpButton = new qx.ui.toolbar.Button("Help", "icon/22/actions/help-contents.png");
        toolbar.add(helpButton);
        var buttons = [newButton, copyButton, cutButton, pasteButton, checkBtn, radioButton1, radioButton2, radioButton3, helpButton]; // set priorities for overflow handling

        toolbar.setRemovePriority(part1, 4);
        toolbar.setRemovePriority(part3, 3);
        toolbar.setRemovePriority(part2, 2); // create overflow menu
        ...
        overflowMenu.add(radioButton3); // hide all menu items
        ...
        var showHideHandler = function showHideHandler(item, visibility) {
          var items = [];
        ...
        mainContainer.add(controlContainer); //////////////////////// icon size stuff
        // create the buttons
        ...
        var size48Button = new qx.ui.form.RadioButton("48px"); // create the radio manager and add the buttons

        var sizeManager = new qx.ui.form.RadioGroup();
        sizeManager.add(size22Button, size32Button, size48Button); // add the buttons to the grid

        controlContainer.add(new qx.ui.basic.Label("Icon Size:"), {
          row: 0,
          column: 0
        });
        controlContainer.add(size22Button, {
          row: 0,
          column: 1
        });
        controlContainer.add(size32Button, {
          row: 0,
          column: 2
        });
        controlContainer.add(size48Button, {
          row: 0,
          column: 3
        ...
        var showLabelButton = new qx.ui.form.RadioButton("Label only"); // create the radio manager and add the buttons

        var showManager = new qx.ui.form.RadioGroup();
        showManager.add(showBothButton, showIconButton, showLabelButton); // add the buttons to the grid

        controlContainer.add(new qx.ui.basic.Label("Show:"), {
          row: 1,
          column: 0
        });
        controlContainer.add(showBothButton, {
          row: 1,
          column: 1
        });
        controlContainer.add(showIconButton, {
          row: 1,
          column: 2
        });
        controlContainer.add(showLabelButton, {
          row: 1,
          column: 3
        }); // register the handler
        ...
        }, this); //////////////////////// Overflow handling stuff
        // create the checkbox

        var enabledOverflowBox = new qx.ui.form.CheckBox("on"); // add the buttons to the grid

        controlContainer.add(new qx.ui.basic.Label("Overflow handling:"), {
          row: 2,
          column: 0
        });
        controlContainer.add(enabledOverflowBox, {
          row: 2,
          column: 1
        });
...
//# sourceMappingURL=ToolBar.js.map?dt=1562184216411