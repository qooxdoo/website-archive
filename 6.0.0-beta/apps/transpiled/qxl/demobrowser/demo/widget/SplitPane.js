  ...
  ************************************************************************ */
  qx.Class.define("qxl.demobrowser.demo.widget.SplitPane", {
    ...
    members: {
      __pane: null,
      main: function main() {
        qxl.demobrowser.demo.widget.SplitPane.prototype.main.base.call(this); // Create a scroll container and an outer container

        var scroller = new qx.ui.container.Scroll();
        var outerContainer = new qx.ui.container.Composite(new qx.ui.layout.Basic());
        outerContainer.setAllowStretchX(false);
        scroller.add(outerContainer); // Create a horizontal split pane

        var pane = new qx.ui.splitpane.Pane("horizontal").set({
          width: 450,
          height: 300
        });
        this.__pane = pane; // Create container with fixed dimensions for the left:
        ...
        label.setRich(true); // ... and add it to the containers

        container1.add(tree);
        container2.add(button);
        container2.add(label);
        this._container1 = container1;
        this._container2 = container2; // Add the first container to the pane. Flex = 0 means that this child should not grow

        pane.add(container1, 0); // Add the second container. Flex = 1 means that this child should consume all available space.

        pane.add(container2, 1); // Add the pane to the outer container.
        ...
        outerContainer.add(controlContainer, {
          left: 490,
          top: 42
        });
        button.focus(); // offset controll

        var offsetLabel = new qx.ui.basic.Label();
        this.getRoot().add(offsetLabel, {
          left: 490,
          top: 120
        });
       ...
       */
      _changeVisiblity: function _changeVisiblity(e) {
       ...
       */
      __createDummyTree: function __createDummyTree() {
...
//# sourceMappingURL=SplitPane.js.map?dt=1562184216264