(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.demobrowser.demo.data.Flickr", {
    ...
    members: {
      main: function main() {
        qxl.demobrowser.demo.data.Flickr.prototype.main.base.call(this); // fetch some data from Flickr
        ...
        this.getRoot().add(search, {
          left: 30,
          top: 50
        });
        var searchGo = new qx.ui.form.Button("Search");
        this.getRoot().add(searchGo, {
          left: 140,
          top: 49
        });
        ...
        var iconOptions = {
          converter: function converter(data, model) {
            return "http://farm" + model.getFarm() + ".static.flickr.com/" + model.getServer() + "/" + data + "_" + model.getSecret() + "_s.jpg";
          }
        };
        ...
        var detailOptions = {
          converter: function converter(data) {
            if (data) {
              return "http://farm" + data.getFarm() + ".static.flickr.com/" + data.getServer() + "/" + data.getId() + "_" + data.getSecret() + ".jpg";
            }
  ...
  qx.Class.define("qxl.demobrowser.demo.data.store.Flickr", {
    ...
    construct: function construct(tag) {
      this.setCallbackName("jsonFlickrApi");
      qx.data.store.Jsonp.constructor.call(this, this.__generateUrl(tag));
    },
    members: {
      searchForTag: function searchForTag(tag) {
        if (tag != "") {
          this.setUrl(this.__generateUrl(tag));
        }
      },
      __generateUrl: function __generateUrl(tag) {
        if (!tag) {
          return;
        }
...
//# sourceMappingURL=Flickr.js.map?dt=1562266611259