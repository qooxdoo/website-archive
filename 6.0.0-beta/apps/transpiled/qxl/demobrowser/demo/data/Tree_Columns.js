(function () {
  var $$dbClassInfo = {
    "dependsOn": {
      ...
      "qxl.demobrowser.demo.data.TreeColumn": {}
    }
  };
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.demobrowser.demo.data.Tree_Columns", {
    ...
    members: {
      main: function main() {
        qxl.demobrowser.demo.data.Tree_Columns.prototype.main.base.call(this);
        ...
        for (var i = 0; i < 50; i++) {
          data.children[1].children[7].children[i] = {
            label: "Item " + i
          };
        }

        this.extendData(data);
        var model = qx.data.marshal.Json.createModel(data); // data binding

        var treeController = new qx.data.controller.Tree(null, tree, "children", "label");
        treeController.setDelegate(this);
        treeController.setModel(model); // make sure the root node is open

        tree.getRoot().setOpen(true);
      },
      extendData: function extendData(data) {
...
//# sourceMappingURL=Tree_Columns.js.map?dt=1562235029387