(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.demobrowser.demo.ui.FormValidator", {
    ...
    members: {
      main: function main() {
        qxl.demobrowser.demo.ui.FormValidator.prototype.main.base.call(this); // create a field for a usernamen

        var username = new qx.ui.form.TextField();
        username.setPlaceholder("username");
        username.setWidth(150);
        username.setRequired(true);
        this.getRoot().add(username, {
          left: 20,
          top: 70
        ...
        this.getRoot().add(accepted, {
          left: 20,
          top: 190
        }); // create the form manager

        var manager = new qx.ui.form.validation.Manager(); // create a validator function

        var passwordLengthValidator = function passwordLengthValidator(value, item) {
        ...
        var userNameValidator = new qx.ui.form.validation.AsyncValidator(function (validator, value) {
        ...
        manager.add(username, userNameValidator); // add the email with a predefined email validator

        manager.add(email, qx.util.Validate.email()); // add the password fields with the notEmpty validator

        manager.add(password1, passwordLengthValidator);
        manager.add(password2, passwordLengthValidator); // add the checkbox (required flag is set)

        manager.add(accepted); // add a validator to the manager itself (passwords mut be equal)
        ...
        send.addListener("execute", function () {
          ...
          send.setLabel("Validating..."); // return type can not be used because of async validation

          manager.validate();
        }, this); // add a listener to the form manager for the validation complete

        manager.addListener("complete", function () {
          // configure the send button
          send.setEnabled(true);
          send.setLabel("Send"); // check the validation status
        ...
        notEmptyDescription.setValue("<b>Client side form validation</b><br/>All fields are required. Some by a custom validator, some by a predefined validator and some by the required flag.");
        this.getRoot().add(notEmptyDescription, {
          left: 20,
          top: 10
        });
      }
...
//# sourceMappingURL=FormValidator.js.map?dt=1562184213805