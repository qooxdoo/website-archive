(function () {
  var $$dbClassInfo = {
    "dependsOn": {
      ...
      "qx.data.Array": {},
      "qxl.demobrowser.demo.virtual.model.Item": {}
    }
  };
  ...
  ************************************************************************ */
   ...
   * @tag noPlayground
   */
  qx.Class.define("qxl.demobrowser.demo.virtual.List", {
    ...
    members: {
      ...
      __quickCheck: null,
      main: function main() {
        qxl.demobrowser.demo.virtual.List.prototype.main.base.call(this);
        var container = this.getRoot();
        container.add(this.createConfigurableList(), {
          left: 20,
          top: 20
        });
        container.add(this.createOneSelectionList(), {
          left: 330,
          top: 20
        });
        container.add(this.createAdditiveSelectionList(), {
          left: 520,
          top: 20
        });
      },
      createConfigurableList: function createConfigurableList() {
        var container = new qx.ui.container.Composite(new qx.ui.layout.Canvas());
        var title = new qx.ui.basic.Label("Configurable").set({
          font: "bold"
        });
        container.add(title); // Creates model data
        ...
        var model = qx.data.marshal.Json.createModel(rawData); // Creates the list and configures it
        ...
        list.setDelegate(delegate); // Pre-Select "Item No 20"

        list.getSelection().push(model.getItem(20)); // log all changes on the selection
        ...
        }, this); // Sets the list item size to all items.
        ...
        var quickCheck = this.__quickCheck = new qx.ui.form.CheckBox("Enable quick selection").set({
          enabled: false
        });
        container.add(dragCheck, {
          left: 160,
          top: 200
        });
        container.add(quickCheck, {
          left: 160,
          top: 220
        });
      ...
      createOneSelectionList: function createOneSelectionList() {
        var container = new qx.ui.container.Composite(new qx.ui.layout.Canvas());
        var title = new qx.ui.basic.Label("One as selection mode").set({
          font: "bold"
        });
        container.add(title); // Creates the mode data
      ...
      onSelectionModeChange: function onSelectionModeChange(e) {
      ...
      onShowModeChange: function onShowModeChange(e) {
        var value = e.getData()[0].getUserData("value");
        this.setShowMode(value);
      },
      onDragCheckChange: function onDragCheckChange(e) {
        if (e.getData()) {
      ...
      onQuickCheckChange: function onQuickCheckChange(e) {
        if (e.getData()) {
          var mode = this.__configList.getSelectionMode();
...
//# sourceMappingURL=List.js.map?dt=1562266615299