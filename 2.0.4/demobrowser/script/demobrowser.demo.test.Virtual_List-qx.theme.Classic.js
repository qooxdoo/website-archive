...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Virtual_List-qx.theme.Classic.73a5ce58e743.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var n="one",m="_applySpacing",k="Integer",h="_applyDragSelection",g="String",f="resize",d="demobrowser.demo.virtual.list.List",c="list",b="multi",a="_applyQuickSelection",C="_applyItemHeight",B="_applySelectionMode",A="additive",z="changeName",y="Number",x="addItem",w="removeItem",v="changeSelection",u="row",t="single",r="changeHeight",s=",",p="Boolean",q="selected",o="qx.event.type.Data";qx.Class.define(d,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider],construct:function(){qx.ui.virtual.core.Scroller.call(this,0,1,this.getItemHeight(),10);this.__Rj=new qx.ui.virtual.layer.WidgetCell(this);this.getPane().addLayer(this.__Rj);this.getPane().addListener(f,this._onResize,this);this.__mS=[];this.__cQ=[];this.__nW=new qx.ui.virtual.selection.Row(this.getPane());this.__nW.addListener(v,this._onChangeSelection,this);this.__nW.attachMouseEvents();this.__nW.attachKeyEvents(this);this.__nW.attachListEvents(this);new qx.ui.virtual.behavior.Prefetch(this,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:400,maxAbove:600,minBelow:400,maxBelow:600}).set({interval:500});}
,events:{addItem:o,removeItem:o,changeValue:o,changeSelection:o},properties:{appearance:{refine:true,init:c},focusable:{refine:true,init:true},spacing:{check:k,init:0,apply:m,themeable:true},enableInlineFind:{check:p,init:true},name:{check:g,nullable:true,event:z},itemHeight:{init:24,themeable:true,check:y,apply:C},selectionMode:{check:[t,b,A,n],init:t,apply:B},dragSelection:{check:p,init:false,apply:h},quickSelection:{check:p,init:false,apply:a}},members:{__mS:null,__cQ:null,__nW:null,__Rj:null,syncWidget:function(){this.update();}
,update:function(){var E=this.getPane().getRowConfig();E.setItemCount(this.__mS.length);E.resetItemSizes();for(var i=0;i<this.__mS.length;i++){var D=this.__mS[i].getHeight();if(D!==null){E.setItemSize(i,D);}
;}
;}
,updateSelection:function(){var F=this.__Rj.getChildren();for(var i=0;i<F.length;i++){var G=F[i];var H=G.getUserData(u);if(this.__nW.isItemSelected(H)){G.addState(q);}
...
,_onChangeSelection:function(e){this.updateSelection();this.fireDataEvent(v,this.__Rn(e.getData()));}
,getCellWidget:function(I,J){var L=this.__mS[I];if(!L){return null;}
;var K=this.__cQ.pop()||new qx.ui.form.ListItem();K.set({label:L.getLabel(),icon:L.getIcon()});if(this.__nW.isItemSelected(I)){K.addState(q);}
...
,hasChildren:function(){return this.__mS.length>0;}
...
,add:function(W){this.__mS.push(W);this._addHelper(W);return this;}
,remove:function(X){qx.lang.Array.remove(this.__mS,X);this._removeHelper(X);return this;}
,removeAll:function(){for(var i=0,j=this.__mS.length;i<j;i++){this._removeHelper(this.__mS[i]);}
;this.__mS=[];}
,indexOf:function(Y){return this.__mS.indexOf(Y);}
,addAt:function(ba,bb){qx.lang.Array.insertAt(this.__mS,ba,bb);this._addHelper(ba);}
,addBefore:function(bc,bd){qx.lang.Array.insertBefore(this.__mS,bc,bd);this._addHelper(bc);}
,addAfter:function(be,bf){qx.lang.Array.insertAfter(this.__mS,be,bf);this._addHelper(be);}
,removeAt:function(bg){this.__mS.splice(bg,1);this._removeHelper(this.__mS[bg]);}
,__Rk:function(bh){var bi=this.__mS.indexOf(bh);if(bi<0){return null;}
else {return bi;}
;}
,__Rl:function(bj){var bk=this.__mS[bj];return bk;}
,__Rm:function(bl){var bm=[];for(var i=0;i<bl.length;i++){bm.push(this.__Rk(bl[i]));}
;return bm;}
,__Rn:function(bn){var bo=[];for(var i=0;i<bn.length;i++){bo.push(this.__mS[bn[i]]);}
;return bo;}
,selectAll:function(){this.__nW.selectAll();}
,setSelected:function(bp){this.__nW.selectItem(this.__Rk(bp));}
,isSelected:function(bq){return this.__nW.isItemSelected(this.__Rk(bq));}
,addToSelection:function(br){this.__nW.addItem(this.__Rk(br));}
,removeFromSelection:function(bs){this.__nW.removeItem(this.__Rk(bs));}
,selectRange:function(bt,bu){this.__nW.selectItemRange(this.__Rk(bt),this.__Rk(bu));}
,resetSelection:function(){this.__nW.clearSelection();}
,setSelection:function(bv){this.__nW.replaceSelection(this.__Rm(bv));}
,getSelectedItem:function(){return this.__mS[this.getSelected()];}
,getSelected:function(){return this.__mS[this.__nW.getSelectedItem()];}
,getSelection:function(){return this.__Rn(this.__nW.getSelection());}
,getSortedSelection:function(){return this.__Rn(this.__nW.getSortedSelection());}
,isSelectionEmpty:function(){return this.__nW.isSelectionEmpty();}
,getSelectionContext:function(){return this.__nW.getSelectionContext();}
,getSelectables:function(){return this.__Rn(this.__nW.getSelectables());}
,_applySelectionMode:function(bw,bx){this.__nW.setMode(bw);}
,_applyDragSelection:function(by,bz){this.__nW.setDrag(by);}
,_applyQuickSelection:function(bA,bB){this.__nW.setQuick(bA);}
}});}
)();
...
(function(){var l="_applyInterval",k="__iI",j="appear",i="qx.debug",h="interval",g="qx.ui.virtual.behavior.Prefetch",f="_applyScroller",e="qx.ui.virtual.core.Scroller",d="Integer",c="scroll",a="scrollbar-x",b="scrollbar-y";qx.Class.define(g,{extend:qx.core.Object,construct:function(m,n){if(qx.core.Environment.get(i)){this.assertObject(n);this.assertPositiveInteger(n.minLeft);this.assertPositiveInteger(n.maxLeft);this.assertPositiveInteger(n.minRight);this.assertPositiveInteger(n.maxRight);this.assertPositiveInteger(n.minAbove);this.assertPositiveInteger(n.maxAbove);this.assertPositiveInteger(n.minBelow);this.assertPositiveInteger(n.maxBelow);}
;qx.core.Object.call(this);this.setPrefetchX(n.minLeft,n.maxLeft,n.minRight,n.maxRight);this.setPrefetchY(n.minAbove,n.maxAbove,n.minBelow,n.maxBelow);this.__iI=new qx.event.Timer(this.getInterval());this.__iI.addListener(h,this._onInterval,this);if(m){this.setScroller(m);}
;}
,properties:{scroller:{check:e,nullable:true,init:null,apply:f},interval:{check:d,init:200,apply:l}},members:{__uV:null,__uW:null,__iI:null,__uX:null,__uY:null,setPrefetchX:function(o,p,q,r){this.__uV=[o,p,q,r];}
,setPrefetchY:function(s,t,u,v){this.__uW=[s,t,u,v];}
,_onInterval:function(){var w=this.__uV;if(w[1]&&w[3]){this.getScroller().getPane().prefetchX(w[0],w[1],w[2],w[3]);qx.ui.core.queue.Manager.flush();}
;var x=this.__uW;if(x[1]&&x[3]){this.getScroller().getPane().prefetchY(x[0],x[1],x[2],x[3]);qx.ui.core.queue.Manager.flush();}
;}
,_applyScroller:function(y,z){if(z){if(this.__uX){z.getChildControl(a).removeListenerById(this.__uX);}
;if(this.__uY){z.getChildControl(b).removeListenerById(this.__uY);}
;}
;if(y){if(!y.getContainerElement().getDomElement()){this.__iI.stop();y.addListenerOnce(j,this.__iI.start,this.__iI);}
else {this.__iI.restart();}
;this.__uX=y.getChildControl(a).addListener(c,this.__iI.restart,this.__iI);this.__uY=y.getChildControl(b).addListener(c,this.__iI.restart,this.__iI);}
else {this.__iI.stop();}
;}
,_applyInterval:function(A,B){this.__iI.setInterval(A);}
},destruct:function(){this.setScroller(null);this.__uV=this.__uW=null;this._disposeObjects(k);}
});}
)();
