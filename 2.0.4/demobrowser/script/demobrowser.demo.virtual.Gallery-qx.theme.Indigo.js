...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Gallery-qx.theme.Indigo.54934c47b6fb.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,_generateItems:function(bg){var bj=[];var bm=[M,I,J,B,Q];var bk=qx.util.AliasManager.getInstance();var bi=qx.util.ResourceManager.getInstance();for(var i=0;i<bg;i++){var bn=r+bm[Math.floor(Math.random()*bm.length)];var bh=bk.resolve(bn);var bl=bi.toUri(bh);bj[i]={label:C+(i+1),icon:bn,resolvedIcon:bl};}
;return bj;}
...
,updateStates:function(br,bs){var bu=br.getChildControl(w);var bt=br.getChildControl(G);if(bs.selected){bu.setBackgroundColor(b);bu.setTextColor(E);bt.setDecorator(t);bt.setBackgroundColor(j);}
else {bu.resetBackgroundColor();bu.resetTextColor();bt.resetDecorator();bt.resetBackgroundColor();}
;}
}});qx.Class.define(f,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bv){demobrowser.demo.virtual.AbstractGallery.call(this,bv);this.__Rx=new demobrowser.demo.virtual.GalleryCell();}
...
,styleSelectable:function(bx,by,bz){if(by!==d){return;}
...
;if(bz){this.__Rx.updateStates(bB,{selected:1});}
else {this.__Rx.updateStates(bB,{});}
;}
;}
...
;var bH=this.__Rx.getCellWidget(bI,bG);bH.setUserData(x,bF);return bH;}
,poolCellWidget:function(bJ){this.__Rx.pool(bJ);}
...
;var bU=this.manager.isItemSelected({row:bR,column:bS});var bT=bU?R:y;return {style:[u,c,this._fontCss,bT].join(y),content:[P,bV.resolvedIcon,O,z,bV.label].join(y)};}
