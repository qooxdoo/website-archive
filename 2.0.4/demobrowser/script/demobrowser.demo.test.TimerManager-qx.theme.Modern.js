...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.TimerManager","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.TimerManager-qx.theme.Modern.b3aa9be2529e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j="Stopping recurrent 2-second timer",i="Recurrent 2-second timer with limit 3:",h="\n",g="One-shot 1-second timer: ",f=" ",e="demobrowser.demo.test.TimerManager",d="Recurrent 5-second timer: ",c="__HC",b="Start timers",a="execute";qx.Class.define(e,{extend:qx.application.Standalone,members:{__HC:null,startTimers:function(){var k=qx.util.TimerManager.getInstance();k.start(function(l,m){this.__Ra(d+m);}
,5000,this,null,0);k.start(function(n,o){this.__Ra(g+o);}
,0,this,null,1000);k.start(function(p,q){this.__Ra(i+q);if(++p.count==3){this.__Ra(j);k.stop(q);}
;}
,2000,this,{count:0},2000);}
,__Ra:function(r){r=Date.parse(new Date)+f+r+h;this.__HC.setValue(this.__HC.getValue()+r);}
,main:function(){qx.application.Standalone.prototype.main.call(this);var t=new qx.ui.layout.VBox(5);var s=new qx.ui.container.Composite(t);var u=new qx.ui.form.Button(b);u.set({allowGrowX:false,allowGrowY:false});u.addListener(a,function(){u.setEnabled(false);this.startTimers();}
,this);var v=new qx.ui.form.TextArea();v.set({width:400,height:300});this.__HC=v;s.add(v);s.add(u);this.getRoot().add(s,{left:20,top:10});}
},destruct:function(){this._disposeObjects(c);}
});}
)();
(function(){var c="qx.util.TimerManager",b="interval",a="singleton";qx.Class.define(c,{extend:qx.core.Object,type:a,statics:{__KN:[],__KO:{},__ts:0},members:{__KP:false,start:function(d,e,f,g,h){if(!h){h=e||0;}
;var j=(new Date()).getTime()+h;this.self(arguments).__KO[++this.self(arguments).__ts]={callback:d,userData:g||null,expireAt:j,recurTime:e,context:f||this};this.__KQ(j,this.self(arguments).__ts);return this.self(arguments).__ts;}
,stop:function(k){var l=this.self(arguments).__KN;var length=l.length;for(var i=0;i<length;i++){if(l[i]==k){l.splice(i,1);break;}
;}
;delete this.self(arguments).__KO[k];if(l.length==0&&this.__KP){qx.event.Idle.getInstance().removeListener(b,this.__KR,this);this.__KP=false;}
;}
,__KQ:function(m,n){var p=this.self(arguments).__KN;var o=this.self(arguments).__KO;var length=p.length;for(var i=0;i<length;i++){if(o[p[i]].expireAt>m){p.splice(i,0,n);break;}
;}
;if(p.length==length){p.push(n);}
;if(!this.__KP){qx.event.Idle.getInstance().addListener("interval",this.__KR,this);this.__KP=true;}
;}
,__KR:function(){var t=(new Date()).getTime();var r=this.self(arguments).__KN;var s=this.self(arguments).__KO;while(r.length>0&&s[r[0]].expireAt<=t){var v=r.shift();var q=s[v];q.callback.call(q.context,q.userData,v);if(q.recurTime&&s[v]){var u=(new Date()).getTime();q.expireAt=u+q.recurTime;this.__KQ(q.expireAt,v);}
else {delete s[v];}
;}
;if(r.length==0&&this.__KP){qx.event.Idle.getInstance().removeListener("interval",this.__KR,this);this.__KP=false;}
;}
}});}
)();
