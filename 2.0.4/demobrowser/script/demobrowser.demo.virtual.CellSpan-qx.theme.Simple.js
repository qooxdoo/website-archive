...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.virtual.CellSpan","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.CellSpan-qx.theme.Simple.654adf7f6722.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j=" / ",i="Widget Cells",h="demobrowser.demo.virtual.CellSpan",g="HTML Cells",f="__Ru",e="x",d="yellow",c="bold",b="table-scroller-header",a="green";qx.Class.define(h,{extend:qx.application.Standalone,members:{__Ru:null,main:function(){qx.application.Standalone.prototype.main.call(this);var l=new qx.ui.layout.Grid(5,0);l.setRowFlex(1,1);l.setColumnFlex(0,1);l.setColumnFlex(1,1);var k=new qx.ui.container.Composite(l);this.getRoot().add(k,{edge:0});k.add(new qx.ui.basic.Label(g).set({font:c,decorator:b,padding:3,allowGrowX:true}),{row:0,column:0});var m=this.getHtmlCellSpanScroller();k.add(m,{row:1,column:0});k.add(new qx.ui.basic.Label(i).set({font:c,decorator:b,padding:3,allowGrowX:true}),{row:0,column:1});var m=this.getWidgetCellSpanScroller();k.add(m,{row:1,column:1});this.__Ru=new qx.ui.virtual.cell.Cell();}
...
,getCellProperties:function(q,r){var s=(q+r)%2==0?d:a;this.__Ru.setBackgroundColor(s);return this.__Ru.getCellProperties(q+j+r);}
...
},destruct:function(){this._disposeObjects(f);this._pool=null;}
});}
)();
...
,__LA:function(p,q,r,s,top,t,u){var w=this._cellProvider.getCellProperties(q,r);var v=w.insets||[0,0];p.push("<div ","style='","left:",s,"px;","top:",top,"px;",this._getCellSizeStyle(t,u,v[0],v[1]),w.style||"","' ","class='",w.classes||"","' ",w.attributes||"",">",w.content||"","</div>");}
,_fullUpdate:function(z,A,B,C){var L=[];var H=this._spanManager.findCellsInWindow(z,A,B.length,C.length);if(H.length>0){var I=this._spanManager.getCellBounds(H,z,A);var F=this._spanManager.computeCellSpanMap(H,z,A,B.length,C.length);for(var i=0,l=H.length;i<l;i++){var J=H[i];var N=I[i];this.__LA(L,J.firstRow,J.firstColumn,N.left,N.top,N.width,N.height);}
;}
else {F=[];for(var i=0;i<B.length;i++){F[z+i]=[];}
;}
;var M=0;var top=0;var E=z;var K=A;for(var x=0;x<B.length;x++){var M=0;var K=A;var D=B[x];for(var y=0;y<C.length;y++){var G=C[y];if(!F[E][K]){this.__LA(L,E,K,M,top,G,D);}
;K++;M+=G;}
;top+=D;E++;}
...
;return this.__Jr(J,M);}
,__Jr:function(N,O){var P=[];for(var Q in N){if(O[Q]){P.push(O[Q]);}
;}
;return P;}
...
(function(){var j="cell.rowspan",h="x",g="cell.column",f="cell.colspan",e="qx.ui.virtual.layer.WidgetCellSpan",d="_spanManager",c="_cellLayer",b="cell.row",a="cell.spanning";qx.Class.define(e,{extend:qx.ui.virtual.layer.Abstract,include:[qx.ui.core.MChildrenHandling],construct:function(k,m,n){qx.ui.virtual.layer.Abstract.call(this);this.setZIndex(2);this._spanManager=new qx.ui.virtual.layer.CellSpanManager(m,n);this._cellProvider=k;this.__Al=[];this._cellLayer=new qx.ui.virtual.layer.WidgetCell(this.__Mz());this._cellLayer.setZIndex(0);this._setLayout(new qx.ui.layout.Grow());this._add(this._cellLayer);}
...
;var r={row:s.getUserData(b),column:s.getUserData(g),rowSpan:s.getUserData(j),colSpan:s.getUserData(f)};if(r.row<=o&&o<r.row+r.rowSpan&&r.column<=p&&p<r.column+r.colSpan){return s;}
;}
;return null;}
;return q;}
,__Al:null,setCellSpan:function(u,v,w,x){var y=u+h+v;this._spanManager.removeCell(y);if(w>1||x>1){this._spanManager.addCell(y,u,v,w,x);}
;qx.ui.core.queue.Widget.add(this);}
,__Mz:function(){var self=this;var B=this._cellProvider;var A=this.__Al;var z={getCellWidget:function(C,D){if(!self._spanMap[C][D]){var E=B.getCellWidget(C,D);}
...
,__MA:function(G,H,I,J){this._cells=this._spanManager.findCellsInWindow(G,H,I,J);if(this._cells.length>0){this._bounds=this._spanManager.getCellBounds(this._cells,G,H);this._spanMap=this._spanManager.computeCellSpanMap(this._cells,G,H,I,J);}
else {this._bounds=[];this._spanMap=[];for(var i=0;i<I;i++){this._spanMap[G+i]=[];}
;}
;}
,__MB:function(){var O=this.getChildren();for(var i=O.length-1;i>=0;i--){var N=O[i];if(N!==this._cellLayer){this._cellProvider.poolCellWidget(N);this._remove(N);}
;}
...
;}
;}
,_fullUpdate:function(P,Q,R,S){this.__MA(P,Q,R.length,S.length);this.__MB();this._cellLayer.fullUpdate(P,Q,R,S);}
,_updateLayerWindow:function(T,U,V,W){this.__MA(T,U,V.length,W.length);this.__MB();this._cellLayer.updateLayerWindow(T,U,V,W);}
},destruct:function(){var X=this._getChildren();for(var i=0;i<X.length;i++){X[i].dispose();}
;this._disposeObjects(d,c);this._cellProvider=this.__Al=this._cells=this._bounds=this._spanMap=null;}
});}
)();
