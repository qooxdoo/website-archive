...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.virtual.Tree_Columns","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Columns-qx.theme.Classic.ae1dd5ca6122.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="changeModel",k="or 'labelPath' is 'null'!",j="_applyLabelOptions",h="marshal.MEventBubbling'. Therefore the tree can not update ",g="_applyOpenMode",f="the view automatically on model changes.",d="qx.core.Object",c="Space",b="Left",a="Integer",T="_applyModel",S="_applyShowTopLevelOpenCloseIcons",R="_applyLabelPath",Q="open",P="changeOpenMode",O="qx.debug",N="changeDelegate",M="virtual-tree",L="_applyChildProperty",K="_applyIconPath",s="_applyDelegate",t="_applyHideRoot",q="close",r="The model item doesn't support the Mixin 'qx.data.",o="_applyRowHeight",p="Right",m="Enter",n="qx.ui.tree.VirtualTree",u="keypress",v="none",C="Could not build tree, because 'childProperty' and/",A="_applyShowLeafs",E="_applyIconOptions",D=".",G="one",F="qx.event.type.Data",x="updated",J="Boolean",I="click",H="String",w="changeBubble",y="dblclick",z="cellDblclick",B="cellClick";qx.Class.define(n,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.tree.core.IVirtualTree,qx.data.controller.ISelection],include:[qx.ui.virtual.selection.MModel,qx.ui.core.MContentPadding],construct:function(U,V,W){qx.ui.virtual.core.Scroller.call(this,0,1,20,100);this._init();if(V!=null){this.setLabelPath(V);}
;if(W!=null){this.setChildProperty(W);}
;if(U!=null){this.initModel(U);}
;this.initItemHeight();this.initOpenMode();this.addListener(u,this._onKeyPress,this);}
,events:{open:F,close:F},properties:{appearance:{refine:true,init:M},focusable:{refine:true,init:true},width:{refine:true,init:100},height:{refine:true,init:200},itemHeight:{check:a,init:25,apply:o,themeable:true},openMode:{check:[I,y,v],init:y,apply:g,event:P,themeable:true},hideRoot:{check:J,init:false,apply:t},showTopLevelOpenCloseIcons:{check:J,init:false,apply:S},showLeafs:{check:J,init:true,apply:A},childProperty:{check:H,apply:L,nullable:true},labelPath:{check:H,apply:R,nullable:true},iconPath:{check:H,apply:K,nullable:true},labelOptions:{apply:j,nullable:true},iconOptions:{apply:E,nullable:true},model:{check:d,apply:T,event:l,nullable:true,deferredInit:true},delegate:{event:N,apply:s,init:null,nullable:true}},members:{_provider:null,_layer:null,__zD:null,__IW:null,__IX:null,__ek:null,__IY:0,__Ja:null,syncWidget:function(X){var bc=this._layer.getFirstRow();var Y=this._layer.getRowSizes().length;for(var bd=bc;bd<bc+Y;bd++){var ba=this._layer.getRenderedCellWidget(bd,0);if(ba!=null){this.__IY=Math.max(this.__IY,ba.getSizeHint().width);}
;}
;var bb=this.getPane().getInnerSize().width;this.getPane().getColumnConfig().setItemSize(0,Math.max(this.__IY,bb));}
,openNode:function(be){this.__Jd(be);this.buildLookupTable();}
,refresh:function(){this.buildLookupTable();}
,openNodeAndParents:function(bf){this.__Je(this.getModel(),bf);this.buildLookupTable();}
,closeNode:function(bg){if(qx.lang.Array.contains(this.__IW,bg)){qx.lang.Array.remove(this.__IW,bg);this.fireDataEvent(q,bg);this.buildLookupTable();}
;}
,isNodeOpen:function(bh){return qx.lang.Array.contains(this.__IW,bh);}
,_init:function(){this.__zD=new qx.data.Array();this.__IW=[];this.__IX=[];this._initLayer();}
,_initLayer:function(){this._provider=new qx.ui.tree.provider.WidgetProvider(this);this._layer=this._provider.createLayer();this._layer.addListener(x,this._onUpdated,this);this.getPane().addLayer(this._layer);}
,getLookupTable:function(){return this.__zD;}
,_reverseLookup:function(bi){return bi;}
,_getDataFromRow:function(bj){return this.__zD.getItem(bj);}
,_getSelectables:function(){return this.__zD;}
,getOpenNodes:function(){return this.__IW;}
,isNode:function(bk){return qx.Class.hasProperty(bk.constructor,this.getChildProperty());}
,getLevel:function(bl){return this.__IX[bl];}
,hasChildren:function(bm){var bn=bm.get(this.getChildProperty());if(bn==null){return false;}
;if(this.isShowLeafs()){return bn.length>0;}
else {for(var i=0;i<bn.getLength();i++){var bo=bn.getItem(i);if(this.isNode(bo)){return true;}
;}
;}
;return false;}
,_getContentPaddingTarget:function(){return this.getPane();}
,_applyRowHeight:function(bp,bq){this.getPane().getRowConfig().setDefaultItemSize(bp);}
,_applyOpenMode:function(br,bs){var bt=this.getPane();if(br===y){bt.addListener(z,this._onOpen,this);}
else if(br===I){bt.addListener(B,this._onOpen,this);}
;if(bs===y){bt.removeListener(z,this._onOpen,this);}
else if(bs===I){bt.removeListener(B,this._onOpen,this);}
;}
,_applyHideRoot:function(bu,bv){this.buildLookupTable();}
,_applyShowTopLevelOpenCloseIcons:function(bw,bx){this.buildLookupTable();}
,_applyShowLeafs:function(by,bz){this.buildLookupTable();}
,_applyChildProperty:function(bA,bB){this._provider.setChildProperty(bA);}
,_applyLabelPath:function(bC,bD){this._provider.setLabelPath(bC);}
,_applyIconPath:function(bE,bF){this._provider.setIconPath(bE);}
,_applyLabelOptions:function(bG,bH){this._provider.setLabelOptions(bG);}
,_applyIconOptions:function(bI,bJ){this._provider.setIconOptions(bI);}
,_applyModel:function(bK,bL){this.__IW=[];if(bK!=null){if(qx.core.Environment.get(O)){if(!qx.Class.hasMixin(bK.constructor,qx.data.marshal.MEventBubbling)){this.warn(r+h+f);}
;}
;bK.addListener(w,this._onChangeBubble,this);this.__Jd(bK);}
;if(bL!=null){bL.removeListener(w,this._onChangeBubble,this);}
;this.__Jb();}
,_applyDelegate:function(bM,bN){this._provider.setDelegate(bM);this.buildLookupTable();}
,_onChangeBubble:function(event){var bP=event.getData().name;var bO=bP.lastIndexOf(D);if(bO!=-1){bP=bP.substr(bO+1,bP.length);}
;if(qx.lang.String.startsWith(bP,this.getChildProperty())){this.__Jb();}
;}
,_onUpdated:function(event){if(this.__ek==null){this.__ek=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__ek.schedule();}
,_onOpen:function(event){var bR=event.getRow();var bQ=this.__zD.getItem(bR);if(this.isNode(bQ)){if(this.isNodeOpen(bQ)){this.closeNode(bQ);}
else {this.openNode(bQ);}
;}
;}
,_onKeyPress:function(e){var bU=this.getSelection();if(bU.getLength()>0){var bS=bU.getItem(0);var bT=this.isNode(bS);switch(e.getKeyIdentifier()){case b:if(bT&&this.isNodeOpen(bS)){this.closeNode(bS);}
else {var parent=this.getParent(bS);if(parent!=null){bU.splice(0,1,parent);}
;}
;break;case p:if(bT&&!this.isNodeOpen(bS)){this.openNode(bS);}
else {if(bT){var bV=bS.get(this.getChildProperty());if(bV!=null&&bV.getLength()>0){bU.splice(0,1,bV.getItem(0));}
;}
;}
;break;case m:case c:if(!bT){return;}
;if(this.isNodeOpen(bS)){this.closeNode(bS);}
else {this.openNode(bS);}
;break;};}
;}
,_beforeApplySelection:function(bW){if(bW.length===0&&this.getSelectionMode()===G){var bX=this.__Jg();var bY=this.getLookupTable().indexOf(bX);if(bY>=0){bW.push(bY);}
;}
;}
,_afterApplySelection:function(){var ca=this.getSelection();if(ca.getLength()>0&&this.getSelectionMode()===G){this.__Jf(ca.getItem(0));}
else {this.__Ja=[];}
;}
,__Jb:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(C+k);}
;this.__IY=0;var ce=[];this.__IX=[];var cc=-1;var cd=this.getModel();if(cd!=null){if(!this.isHideRoot()){cc++;ce.push(cd);this.__IX.push(cc);}
;if(this.isNodeOpen(cd)){var cb=this.__Jc(cd,cc);ce=ce.concat(cb);}
;}
;this._provider.removeBindings();this.__zD.removeAll();this.__zD.append(ce);this.__zJ();this._updateSelection();}
,__Jc:function(cf,cg){var ch=[];cg++;if(!this.isNode(cf)){return ch;}
;var cj=cf.get(this.getChildProperty());if(cj==null){return ch;}
;for(var i=0;i<cj.getLength();i++){var ck=cj.getItem(i);if(this.isNode(ck)){this.__IX.push(cg);ch.push(ck);if(this.isNodeOpen(ck)){var ci=this.__Jc(ck,cg);ch=ch.concat(ci);}
;}
else {if(this.isShowLeafs()){this.__IX.push(cg);ch.push(ck);}
;}
;}
;return ch;}
,__Jd:function(cl){if(!qx.lang.Array.contains(this.__IW,cl)){this.__IW.push(cl);this.fireDataEvent(Q,cl);}
;}
,__Je:function(cm,cn){if(cm===cn){this.__Jd(cn);return true;}
;if(!this.isNode(cm)){return false;}
;var cp=cm.get(this.getChildProperty());if(cp==null){return false;}
;for(var i=0;i<cp.getLength();i++){var cq=cp.getItem(i);var co=this.__Je(cq,cn);if(co===true){this.__Jd(cq);return true;}
;}
;return false;}
,__zJ:function(){this.getPane().getRowConfig().setItemCount(this.__zD.getLength());this.getPane().fullUpdate();}
,getParent:function(cr){var ct=this.__zD.indexOf(cr);if(ct<0){return null;}
;var cu=this.__IX[ct];while(ct>0){ct--;var cs=this.__IX[ct];if(cs<cu){return this.__zD.getItem(ct);}
;}
;return null;}
,__Jf:function(cv){this.__Ja=[];var parent=this.getParent(cv);while(parent!=null){this.__Ja.unshift(parent);parent=this.getParent(parent);}
;}
,__Jg:function(){if(this.__Ja==null){return this.getModel();}
;var cw=this.getLookupTable();var parent=this.__Ja.pop();while(parent!=null){if(cw.contains(parent)){return parent;}
;parent=this.__Ja.pop();}
;return this.getModel();}
},destruct:function(){var cy=this.getPane();if(cy!=null){if(cy.hasListener(z)){cy.removeListener(z,this._onOpen,this);}
;if(cy.hasListener(B)){cy.removeListener(B,this._onOpen,this);}
;}
;if(!qx.core.ObjectRegistry.inShutDown&&this.__ek!=null){this.__ek.cancel();this.__ek.dispose();}
;var cx=this.getModel();if(cx!=null){cx.removeListener(w,this._onChangeBubble,this);}
;this._layer.removeListener(x,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__zD.dispose();this._layer=this._provider=this.__zD=this.__IW=this.__ek=null;}
});}
)();
...
(function(){var q="model",p="changeDelegate",o="virtual-tree-file",n="label",m="appearance",l="icon",k="bindItem",j=".length",i="]",h="virtual-tree-folder",c="[",g=".",f="qx.ui.tree.core.MWidgetController",b="BindingIds",a="",d="String";qx.Mixin.define(f,{construct:function(){this.__zP=[];}
...
;}
,bindProperty:function(u,v,w,x,y){var z=this.__zQ(y,u);var A=this._tree.getLookupTable();var B=A.bind(z,x,v,w);this.__zR(x,B);}
,bindPropertyReverse:function(C,D,E,F,G){var H=this.__zQ(G,C);var I=this._tree.getLookupTable();var J=F.bind(D,I,H,E);this.__zR(F,J);}
,removeBindings:function(){while(this.__zP.length>0){var K=this.__zP.pop();this._removeBindingsFrom(K);}
;}
...
;}
,_removeBindingsFrom:function(O){var P=this.__zS(O);while(P.length>0){var Q=P.pop();try{this._tree.getLookupTable().removeBinding(Q);}
catch(e){O.removeBinding(Q);}
;}
;if(qx.lang.Array.contains(this.__zP,O)){qx.lang.Array.remove(this.__zP,O);}
;}
,__zQ:function(R,S){var T=c+R+i;if(S!=null&&S!=a){T+=g+S;}
;return T;}
,__zR:function(U,V){var W=this.__zS(U);if(!qx.lang.Array.contains(W,V)){W.push(V);}
;if(!qx.lang.Array.contains(this.__zP,U)){this.__zP.push(U);}
;}
,__zS:function(X){var Y=X.getUserData(b);if(Y==null){Y=[];X.setUserData(b,Y);}
;return Y;}
...
;var q=this._renderer.getCellWidget();q.setOpen(r&&this._tree.isNodeOpen(p));q.setUserData(g,r);q.addListener(b,this.__Jh,this);if(this._tree.getSelection().contains(p)){this._styleSelectabled(q);}
...
,poolCellWidget:function(t){t.removeListener(b,this.__Jh,this);this._removeBindingsFrom(t);this._renderer.pool(t);this._onPool(t);}
...
,__Jh:function(event){var J=event.getTarget();var K=J.getUserData(c);var I=this._tree.getLookupTable().getItem(K);if(event.getData()){this._tree.openNode(I);}
