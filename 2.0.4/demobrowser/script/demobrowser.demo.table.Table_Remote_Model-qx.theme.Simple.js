...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Remote_Model-qx.theme.Simple.6469724ffa98.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="icon/22/actions/view-refresh.png",j='90%',i="interval",h="demobrowser.demo.table.Table_Remote_Model",g='10%',f="value",e='Reload',d='execute',c="enabled",b='Poll',a="execute";qx.Class.define(h,{extend:demobrowser.demo.table.TableDemo,construct:function(){demobrowser.demo.table.TableDemo.call(this);this.__iI=new qx.event.Timer(1000);}
,members:{__iI:null,createTable:function(){var l=this._tableModel=new demobrowser.demo.table.RemoteTableModel();var o={tableColumnModel:function(p){return new qx.ui.table.columnmodel.Resize(p);}
};this.__iI.addListener(i,l.reloadData,l);var m=new qx.ui.table.Table(l,o);var n=m.getTableColumnModel().getBehavior();n.setWidth(0,g);n.setWidth(1,j);return m;}
,createControls:function(){var t=new qx.ui.toolbar.ToolBar();var s=new qx.ui.toolbar.Part();t.add(s);var r=new qx.ui.toolbar.Button(e,k);r.addListener(d,function(){this._tableModel.reloadData();}
,this);s.add(r);var q=new qx.ui.toolbar.CheckBox(b);q.addListener(a,function(){if(q.getValue()){this.__iI.start();}
else {this.__iI.stop();}
;}
,this);s.add(q);q.bind(f,r,c,{converter:function(u){return !u;}
});return t;}
}});}
)();
...
(function(){var f="qx.ui.table.model.Remote",e="Boolean",d="Integer",c="_loadRowCount is abstract",b="metaDataChanged",a="dataChanged";qx.Class.define(f,{extend:qx.ui.table.model.Abstract,construct:function(){qx.ui.table.model.Abstract.call(this);this.__na=-1;this.__ne=true;this.__KS=-1;this.__KT=0;this.__KU=-1;this.__KV=-1;this.__KW=-1;this.__KX=false;this.__KY={};this.__La=0;this.__nd=null;this.__nc=null;}
,properties:{blockSize:{check:d,init:50},maxCachedBlockCount:{check:d,init:15},clearCacheOnRemove:{check:e,init:false},blockConcurrentLoadRowCount:{check:e,init:true}},members:{__KS:null,__KX:null,__KT:null,__KU:null,__KV:null,__KW:null,__KY:null,__La:null,__na:null,__ne:null,__nc:null,__nd:null,__Lb:false,_clearCache:false,_getIgnoreCurrentRequest:function(){return this.__KX;}
,getRowCount:function(){if(this.__KS==-1){if(!this.__Lb||!this.getBlockConcurrentLoadRowCount()){this.__Lb=true;this._loadRowCount();}
;return (this.__KS==-1)?0:this.__KS;}
else {return this.__KS;}
;}
,_loadRowCount:function(){throw new Error(c);}
,_onRowCountLoaded:function(g){if(this.getBlockConcurrentLoadRowCount()){this.__Lb=false;}
;if(g==null||g<0){g=0;}
;this.__KS=Number(g);var h={firstRow:0,lastRow:g-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,h);}
,reloadData:function(){if(this.__KU!=-1){var j=this._cancelCurrentRequest();if(j){this.__KU=-1;this.__KX=false;}
else {this.__KX=true;}
;}
;this._clearCache=true;this.__KV=-1;this.__KW=-1;this.__Lb=true;this._loadRowCount();}
,clearCache:function(){this.__KY={};this.__La=0;}
,getCacheContent:function(){return {sortColumnIndex:this.__na,sortAscending:this.__ne,rowCount:this.__KS,lruCounter:this.__KT,rowBlockCache:this.__KY,rowBlockCount:this.__La};}
,restoreCacheContent:function(k){if(this.__KU!=-1){var m=this._cancelCurrentRequest();if(m){this.__KU=-1;this.__KX=false;}
else {this.__KX=true;}
;}
;this.__na=k.sortColumnIndex;this.__ne=k.sortAscending;this.__KS=k.rowCount;this.__KT=k.lruCounter;this.__KY=k.rowBlockCache;this.__La=k.rowBlockCount;var l={firstRow:0,lastRow:this.__KS-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,l);}
,_cancelCurrentRequest:function(){return false;}
,iterateCachedRows:function(n,o){var q=this.getBlockSize();var p=Math.ceil(this.getRowCount()/q);for(var x=0;x<=p;x++){var r=this.__KY[x];if(r!=null){var w=x*q;var v=r.rowDataArr;for(var u=0;u<v.length;u++){var t=v[u];var s=n.call(o,w+u,t);if(s!=null){v[u]=s;}
;}
;}
;}
;}
,prefetchRows:function(y,z){if(this.__KU==-1||this._clearCache){var A=this.getBlockSize();var F=Math.ceil(this.__KS/A);var E=parseInt(y/A,10)-1;if(E<0){E=0;}
;var D=parseInt(z/A,10)+1;if(D>=F){D=F-1;}
;var C=-1;var B=-1;for(var G=E;G<=D;G++){if(this._clearCache||this.__KY[G]==null||this.__KY[G].isDirty){if(C==-1){C=G;}
;B=G;}
;}
;if(C!=-1){this.__KV=-1;this.__KW=-1;this.__KU=C;this._loadRowData(C*A,(B+1)*A-1);}
;}
else {this.__KV=y;this.__KW=z;}
;}
,_loadRowData:function(H,I){throw new Error(c);}
,_onRowDataLoaded:function(J){if(this._clearCache){this.clearCache();this._clearCache=false;}
;if(J!=null&&!this.__KX){var M=this.getBlockSize();var K=Math.ceil(J.length/M);if(K==1){this._setRowBlockData(this.__KU,J);}
else {for(var i=0;i<K;i++){var P=i*M;var O=[];var L=Math.min(M,J.length-P);for(var Q=0;Q<L;Q++){O.push(J[P+Q]);}
;this._setRowBlockData(this.__KU+i,O);}
;}
;var N={firstRow:this.__KU*M,lastRow:(this.__KU+K+1)*M-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,N);}
;this.__KU=-1;this.__KX=false;if(this.__KV!=-1){this.prefetchRows(this.__KV,this.__KW);}
;}
,_setRowBlockData:function(R,S){if(this.__KY[R]==null){this.__La++;while(this.__La>this.getMaxCachedBlockCount()){var W;var V=this.__KT;for(var U in this.__KY){var T=this.__KY[U].lru;if(T<V&&U>1){V=T;W=U;}
;}
;delete this.__KY[W];this.__La--;}
;}
;this.__KY[R]={lru:++this.__KT,rowDataArr:S};}
...
else {var bb=this.getBlockSize();var bc=Math.ceil(this.getRowCount()/bb);var bd=parseInt(X/bb,10);for(var bg=bd;bg<=bc;bg++){var Y=this.__KY[bg];if(Y!=null){var ba=0;if(bg==bd){ba=X-bg*bb;}
;Y.rowDataArr.splice(ba,1);if(bg==bc-1){if(Y.rowDataArr.length==0){delete this.__KY[bg];}
;}
else {var be=this.__KY[bg+1];if(be!=null){Y.rowDataArr.push(be.rowDataArr[0]);}
else {Y.isDirty=true;}
;}
;}
;}
;if(this.__KS!=-1){this.__KS--;}
...
;}
;}
,getRowData:function(bh){var bi=this.getBlockSize();var bl=parseInt(bh/bi,10);var bj=this.__KY[bl];if(bj==null){return null;}
else {var bk=bj.rowDataArr[bh-(bl*bi)];if(bj.lru!=this.__KT){bj.lru=++this.__KT;}
;return bk;}
;}
...
;}
;}
,setEditable:function(bw){this.__nc=[];for(var bx=0;bx<this.getColumnCount();bx++){this.__nc[bx]=bw;}
;this.fireEvent(b);}
,setColumnEditable:function(by,bz){if(bz!=this.isColumnEditable(by)){if(this.__nc==null){this.__nc=[];}
;this.__nc[by]=bz;this.fireEvent(b);}
;}
,isColumnEditable:function(bA){return (this.__nc?(this.__nc[bA]==true):false);}
,setColumnSortable:function(bB,bC){if(bC!=this.isColumnSortable(bB)){if(this.__nd==null){this.__nd=[];}
;this.__nd[bB]=bC;this.fireEvent(b);}
;}
,isColumnSortable:function(bD){return (this.__nd?(this.__nd[bD]!==false):true);}
,sortByColumn:function(bE,bF){if(this.__na!=bE||this.__ne!=bF){this.__na=bE;this.__ne=bF;this.clearCache();this.fireEvent(b);}
;}
,getSortColumnIndex:function(){return this.__na;}
,isSortAscending:function(){return this.__ne;}
,setSortColumnIndexWithoutSortingData:function(bG){this.__na=bG;}
,setSortAscendingWithoutSortingData:function(bH){this.__ne=bH;}
},destruct:function(){this.__nd=this.__nc=this.__KY=null;}
});}
)();
...
,members:{__QQ:null,__QR:false,_loadRowCount:function(){this.__QS();if(this.__QR){return;}
;if(this.__QQ){this.__QT();}
else {this.__QW();}
;}
,_loadRowData:function(f,g){this.__QS();if(this.__QR){return;}
;if(this.__QQ){this.__QU(f,g);}
else {this.__QX(f,g);}
;}
,__QS:function(){if(this.__QR||this.__QQ!==null){return;}
;this.__QR=true;this.__QV("method=checkphp",function(h){this.__QR=false;this.__QQ=(h=="WTF PHP");this._loadRowCount();}
);}
,__QT:function(){var j="method=getRowCount";this.__QV(j,function(k){this._onRowCountLoaded(parseInt(k));}
);}
,__QU:function(l,m){var n="method=getRowData&start="+l+"&end="+m;this.__QV(n,function(o){this._onRowDataLoaded(qx.lang.Json.parse(o));}
);}
,__QV:function(p,q){var r=new qx.io.request.Xhr();r.setUrl("../../resource/demobrowser/backend/remote_table.php?"+p);r.addListener("success",function(){q.call(this,r.getResponseText());}
,this);r.send();}
,__QW:function(){var self=this;window.setTimeout(function(){self._onRowCountLoaded(1000000);}
,0);}
,__QX:function(s,t){var self=this;window.setTimeout(function(){var u=[];for(var i=s;i<=t;i++){u.push({id:i,text:'Hello '+i+' Generated on:'+(new Date())});}
...
(function(){var v="columnVisibilityMenuCreateEnd",u="tableWidthChanged",t="ontablewidthchanged",s="verticalScrollBarChanged",r="oncolumnwidthchanged",q="onvisibilitychanged",p="qx.ui.table.columnmodel.resizebehavior.Abstract",n="qx.ui.table.columnmodel.Resize",m="_applyBehavior",l="separator",e="visibilityChanged",k="Reset column widths",h="changeBehavior",c="user-button",b="onverticalscrollbarchanged",g="onappear",f="widthChanged",i="execute",a="appear",j="qx.tableResizeDebug",d="qx.debug";qx.Class.define(n,{extend:qx.ui.table.columnmodel.Basic,include:qx.locale.MTranslation,construct:function(){qx.ui.table.columnmodel.Basic.call(this);this.__nM=false;this.__nN=false;}
,properties:{behavior:{check:p,init:null,nullable:true,apply:m,event:h}},members:{__nN:null,__nM:null,__my:null,_applyBehavior:function(w,x){if(x!=null){x.dispose();x=null;}
;w._setNumColumns(this.getOverallColumnCount());w.setTableColumnModel(this);}
,init:function(y,z){qx.ui.table.columnmodel.Basic.prototype.init.call(this,y,z);if(this.__my==null){this.__my=z;z.addListener(a,this._onappear,this);z.addListener(u,this._onTableWidthChanged,this);z.addListener(s,this._onverticalscrollbarchanged,this);z.addListener(v,this._addResetColumnWidthButton,this);this.addListener(f,this._oncolumnwidthchanged,this);this.addListener(e,this._onvisibilitychanged,this);}
...
,_onappear:function(event){if(this.__nM){return;}
;this.__nM=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(g);}
;}
;this.getBehavior().onAppear(event,event.getType()!==a);this.__my._updateScrollerWidths();this.__my._updateScrollBarVisibility();this.__nM=false;this.__nN=true;}
,_onTableWidthChanged:function(event){if(this.__nM||!this.__nN){return;}
;this.__nM=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(t);}
;}
;this.getBehavior().onTableWidthChanged(event);this.__nM=false;}
,_onverticalscrollbarchanged:function(event){if(this.__nM||!this.__nN){return;}
;this.__nM=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(b);}
;}
;this.getBehavior().onVerticalScrollBarChanged(event);qx.event.Timer.once(function(){if(this.__my&&!this.__my.isDisposed()){this.__my._updateScrollerWidths();this.__my._updateScrollBarVisibility();}
;}
,this,0);this.__nM=false;}
,_oncolumnwidthchanged:function(event){if(this.__nM||!this.__nN){return;}
;this.__nM=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(r);}
;}
;this.getBehavior().onColumnWidthChanged(event);this.__nM=false;}
,_onvisibilitychanged:function(event){if(this.__nM||!this.__nN){return;}
;this.__nM=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(q);}
;}
;this.getBehavior().onVisibilityChanged(event);this.__nM=false;}
...
(function(){var m="Function",k="Boolean",j="minWidth",h="width",g="__nR",f="qx.ui.table.columnmodel.Resize",e="qx.ui.table.columnmodel.resizebehavior.Default",d="__nQ",c="Unknown property: ",b="maxWidth",a="Column number out of range";qx.Class.define(e,{extend:qx.ui.table.columnmodel.resizebehavior.Abstract,construct:function(){qx.ui.table.columnmodel.resizebehavior.Abstract.call(this);this.__nP=[];this.__nQ=new qx.ui.layout.HBox();this.__nQ.connectToWidget(this);this.__nR=new qx.util.DeferredCall(this._computeColumnsFlexWidth,this);}
,properties:{newResizeBehaviorColumnData:{check:m,init:function(n){return new qx.ui.core.ColumnData();}
},initializeWidthsOnEveryAppear:{check:k,init:false},tableColumnModel:{check:f}},members:{__nQ:null,__nS:null,__nP:null,__nR:null,__nT:false,setWidth:function(o,p,q){if(o>=this.__nP.length){throw new Error(a);}
;this.__nP[o].setColumnWidth(p,q);this.__nR.schedule();}
,setMinWidth:function(r,s){if(r>=this.__nP.length){throw new Error(a);}
;this.__nP[r].setMinWidth(s);this.__nR.schedule();}
,setMaxWidth:function(t,u){if(t>=this.__nP.length){throw new Error(a);}
;this.__nP[t].setMaxWidth(u);this.__nR.schedule();}
...
;}
,onAppear:function(event,y){if(y===true||!this.__nT||this.getInitializeWidthsOnEveryAppear()){this._computeColumnsFlexWidth();this.__nT=true;}
;}
...
,_setNumColumns:function(A){var B=this.__nP;if(A<=B.length){B.splice(A,B.length);return;}
;for(var i=B.length;i<A;i++){B[i]=this.getNewResizeBehaviorColumnData()();B[i].columnNumber=i;}
;}
,getLayoutChildren:function(){return this.__nS;}
,_computeColumnsFlexWidth:function(){this.__nR.cancel();var G=this._getAvailableWidth();if(G===null){return;}
;var C=this.getTableColumnModel();var E=C.getVisibleColumns();var F=E.length;var D=this.__nP;var i,l;if(F===0){return;}
;var I=[];for(i=0;i<F;i++){I.push(D[E[i]]);}
;this.__nS=I;this.__nU();this.__nQ.renderLayout(G,100);for(i=0,l=I.length;i<l;i++){var H=I[i].getComputedWidth();C.setColumnWidth(E[i],H);}
;}
,__nU:function(){this.__nQ.invalidateChildrenCache();var J=this.__nS;for(var i=0,l=J.length;i<l;i++){J[i].invalidateLayoutCache();}
;}
...
;}
,_getResizeColumnData:function(){return this.__nP;}
},destruct:function(){this.__nP=this.__nS=null;this._disposeObjects(d,g);}
});}
)();
