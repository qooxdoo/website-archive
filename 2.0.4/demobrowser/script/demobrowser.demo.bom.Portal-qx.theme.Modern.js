...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Portal-qx.theme.Modern.6408ce4c0fe2.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="groupBoxes",h="demobrowser.demo.bom.portal.box.Manager",g="boxData",f="qx.event.type.Event",e="boxes",d="demobrowser.demo.bom.portal.box.Box",c="_applyActiveBox",b="loaded",a="singleton";qx.Class.define(h,{type:a,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__Pl=[];this.__Pm=[];this.__Pn={};}
,statics:{dataKeyRoot:g,dataKeyGroupBoxes:m,dataKeyBoxes:e},events:{"loaded":f},properties:{activeBox:{init:null,check:d,apply:c}},members:{__Pl:null,__Pm:null,__Pn:null,_applyActiveBox:function(n,o){if(o!=null){o.setActive(false);}
;n.setActive(true);}
...
,__bf:function(p){var t,w,u,v,x,p;var q=p[demobrowser.demo.bom.portal.box.Manager.dataKeyGroupBoxes];var s=p[demobrowser.demo.bom.portal.box.Manager.dataKeyBoxes];for(var w in q){t=q[w];u=t.boxes;w=t.id;v={id:w,element:document.getElementById(w)};this.__Pm.push(v);this.__Pn[w]=[];for(var k=0,l=u.length;k<l;k++){p=s[u[k]];var r=document.getElementById(p.id);var x=new demobrowser.demo.bom.portal.box.Box(p,r,null,w);this.__Pl.push(x);this.__Pn[w].push(x);}
;}
;}
,getBoxes:function(){return this.__Pl;}
,getGroupBoxes:function(){return this.__Pm;}
,updateGroupBoxMembers:function(y,z,A){var B=qx.dom.Hierarchy.getElementIndex(A.getElement());qx.lang.Array.remove(this.__Pn[z],A);qx.lang.Array.insertAt(this.__Pn[y],A,B);}
,getGroupBoxDataOfBox:function(C){for(var i=0,j=this.__Pm.length,D;i<j;i++){D=this.__Pn[this.__Pm[i].id];for(var k=0,l=D.length;k<l;k++){if(D[k].getId()===C){return this.__Pm[i];}
;}
;}
;return null;}
},destruct:function(){this.__Pl=this.__Pm=this.__Pn=null;}
});}
)();
(function(){var s="click",r="1px solid #444444",q="height",p="position",o="zIndex",n="_applyActive",m="width",l="box_content",k=" It does contain no UI widget code at all.",j="relative",c="demobrowser.demo.bom.portal.box.Box",i="1px solid orange",g="This is a demo application showing the low-level capabilities.",b="div",a="String",f="<br/><br/>",d="border",h="Boolean";qx.Class.define(c,{extend:qx.core.Object,construct:function(t,u,v,w){qx.core.Object.call(this);this.__Po=t;this.__bV=t.id;if(u!=null){qx.bom.element.Style.set(u,m,t.width);qx.bom.element.Style.set(u,q,t.height);this.__Pr(u,v);}
;this.setElement(u);this.setGroupBoxId(w);this.__Pp=null;this.__Pq=null;if(t.resizable){this.setResizable(true);var x={minWidth:parseInt(t.minWidth),minHeight:parseInt(t.minHeight),maxWidth:parseInt(t.maxWidth),maxHeight:parseInt(t.maxHeight),handles:t.resizeHandles};this.__Pp=new demobrowser.demo.bom.portal.box.Resizable(this,x);}
;if(t.draggable){this.setDraggable(true);this.__Pq=new demobrowser.demo.bom.portal.box.Draggable(this);}
;this.__Ps();}
,properties:{element:{init:null},groupBoxId:{init:null,check:a},active:{check:h,init:false,apply:n},resizable:{check:h,init:false},draggable:{check:h,init:false}},members:{__bV:null,__Po:null,__Pq:null,__Pp:null,_applyActive:function(y,z){if(y){qx.bom.element.Style.set(this.getElement(),d,i);}
else {qx.bom.element.Style.set(this.getElement(),d,r);}
;}
,getId:function(){return this.getElement()?this.getElement().id:null;}
,getData:function(){return this.__Po;}
,getDraggableComponent:function(){return this.__Pq;}
,getResizableComponent:function(){return this.__Pp;}
,__Pr:function(A,B){if(B){for(var C in B){qx.bom.element.Style.set(C,B[C]);}
;}
;qx.bom.element.Style.set(A,p,j);qx.bom.element.Style.set(A,o,0);qx.bom.Element.addListener(A,s,this.__Pt,this,true);}
,__Ps:function(){var E=g+k;E=E+f+E+f+E;var D=qx.dom.Element.create(b,{"class":l});D.innerHTML=E;qx.dom.Element.insertEnd(D,this.getElement());}
,__Pt:function(e){demobrowser.demo.bom.portal.box.Manager.getInstance().setActiveBox(this);}
},destruct:function(){this.resetElement();this.__Po=this.__Pp=this.__Pq=null;}
});}
)();
...
(function(){var k="visibility",j="_",i="mouseover",h="mouseout",g="visible",f="hidden",d="div",c="orientation",b="demobrowser.demo.bom.portal.box.Resizable",a="Handle";qx.Class.define(b,{extend:qx.core.Object,construct:function(l,m){qx.core.Object.call(this);this.__Pu=l;this.__dL=this.__Pu.getElement();this.__Pv={};this.__Pw=false;this.__Px={orientation:null,lastCoord:{left:null,top:null}};this.__Py={minWidth:10,minHeight:10,maxWidth:null,maxHeight:null,autoHide:true,handles:3};for(var name in this.__Py){if(m[name]){this.__Py[name]=m[name];}
;}
;this.__Pr();}
,members:{__Px:null,__Pu:null,__dL:null,__Pv:null,__Py:null,__Pw:false,getBox:function(){return this.__Pu;}
,__Pr:function(){switch(this.__Py.handles){case 1:this.__Pv.east=null;break;case 2:this.__Pv.south=null;break;case 3:this.__Pv.south=null;this.__Pv.southeast=null;this.__Pv.east=null;break;};for(var o in this.__Pv){var n=this.__Pv[o]=qx.dom.Element.create(d);qx.bom.element.Attribute.set(n,c,o);qx.dom.Element.insertEnd(n,this.__dL);if(this.__dL.id){n.id=this.__dL.id+j+o;}
;this.__Pz(o);this.__PA(o);}
;}
,__Pz:function(p){qx.bom.element.Class.add(this.__Pv[p],p+a);qx.bom.element.Style.set(this.__Pv[p],k,this.__Py.autoHide?f:g);}
,__PA:function(q){var s=qx.event.Registration;var r=this.__Pv[q];s.addListener(r,"mousedown",this.__PD,this);s.addListener(document,"mouseup",this.__PE,this);if(this.__Py.autoHide){s.addListener(this.__Pu.getElement(),"mouseover",this.__PB,this);s.addListener(this.__Pu.getElement(),"mouseout",this.__PC,this);}
;}
,__PB:function(e){for(var t in this.__Pv){qx.bom.element.Style.set(this.__Pv[t],"visibility","visible");}
;}
,__PC:function(e){if(!this.__Pw){for(var u in this.__Pv){qx.bom.element.Style.set(this.__Pv[u],"visibility","hidden");}
;}
;}
,__PD:function(e){e.stopPropagation();if(e.isLeftPressed()){qx.event.Registration.addListener(document,"mousemove",this.__PF,this,true);this.__Pw=true;var v=e.getTarget();this.__Px.orientation=qx.bom.element.Attribute.get(v,"orientation");this.__Px.lastCoord.left=e.getDocumentLeft();this.__Px.lastCoord.top=e.getDocumentTop();demobrowser.demo.bom.portal.box.Util.bringToFront(this.__dL);}
;}
,__PE:function(e){e.stopPropagation();if(this.__Pw){qx.event.Registration.removeListener(document,"mousemove",this.__PF,this,true);demobrowser.demo.bom.portal.box.Util.sendToBack(this.__dL);if(this.__Py.autoHide){for(var w in this.__Pv){qx.bom.element.Style.set(this.__Pv[w],"visibility","hidden");}
;}
;this.__Pw=false;qx.bom.Selection.clear(document.body);}
;}
,__PF:function(e){e.stopPropagation();if(this.__Pw){var x={left:null,top:null};switch(this.__Px.orientation){case "south":x.top=e.getDocumentTop();break;case "southeast":x.top=e.getDocumentTop();x.left=e.getDocumentLeft();break;case "east":x.left=e.getDocumentLeft();break;};this.__PG(x,this.__Px.lastCoord);this.__Px.lastCoord.left=e.getDocumentLeft();this.__Px.lastCoord.top=e.getDocumentTop();}
;}
,__PG:function(y,z){var G=qx.bom.element.Dimension;var D=demobrowser.demo.bom.portal.box.Manager.getInstance().getGroupBoxDataOfBox(this.__Pu.getId());var F=G.getContentSize(D.element);if(y.left){var C=y.left-z.left;if(C!==0){var B=G.getContentWidth(this.__dL)+C;var E=G.getWidth(this.__dL)+C;if((this.__Py.minWidth==null||B>this.__Py.minWidth)&&(this.__Py.maxWidth==null||B<=this.__Py.maxWidth)){if(F.width>=E){qx.bom.element.Style.set(this.__dL,"width",B+'px');this.__Pu.getData().width=B;}
;}
;}
;}
;if(y.top){var C=y.top-z.top;if(C!==0){var A=G.getContentHeight(this.__dL)+C;if((this.__Py.minHeight==null||A>this.__Py.minHeight)&&(this.__Py.maxHeight==null||A<=this.__Py.maxHeight)){qx.bom.element.Style.set(this.__dL,"height",A+"px");}
;}
;}
;}
},destruct:function(){if(this.__Py.autoHide){qx.event.Registration.removeListener(this.__Pu.getElement(),i,this.__PB,this);qx.event.Registration.removeListener(this.__Pu.getElement(),h,this.__PC,this);}
;this.__Pu=this.__Py=this.__dL=this.__Pv=null;this.__Px=null;}
});}
)();
...
(function(){var c="auto",b="demobrowser.demo.bom.portal.box.Util",a="zIndex";qx.Class.define(b,{statics:{__PH:1000,bringToFront:function(d){var e=qx.bom.element.Style.get(d,a);e=e==c?0:e;qx.bom.element.Style.set(d,a,e+this.__PH);}
,sendToBack:function(f){var g=qx.bom.element.Style.get(f,a);qx.bom.element.Style.set(f,a,g-this.__PH);}
}});}
)();
...
(function(){var d="mousemove",c="demobrowser.demo.bom.portal.box.Draggable",b="mouseup",a="dragstart";qx.Class.define(c,{extend:qx.core.Object,construct:function(f){qx.core.Object.call(this);this.__Pu=f;this.__dL=f.getElement();this.__PI=null;this.__PJ=null;this.__Pr();}
,members:{__Pu:null,__dL:null,__PI:null,__PJ:null,getBox:function(){return this.__Pu;}
,getElement:function(){return this.__dL;}
,__Pr:function(){this.__PK();this.__PA();}
,__PK:function(){this.__PI=qx.dom.Element.create("div");qx.bom.element.Class.add(this.__PI,"dragHandle");qx.dom.Element.insertBegin(this.__PI,this.__dL);}
,__PA:function(){qx.bom.Element.addListener(this.__PI,"mousedown",this.__nE,this);qx.bom.Element.addListener(this.__PI,"mouseover",function(e){qx.bom.element.Style.set(this,"cursor","move");}
,this.__PI);qx.bom.Element.addListener(this.__PI,"dragstart",this.__uo,this);qx.bom.Element.addListener(this.__PI,"dragend",demobrowser.demo.bom.portal.dragdrop.Manager.getInstance().stopSession,demobrowser.demo.bom.portal.dragdrop.Manager.getInstance());qx.bom.Element.addListener(this.__PI,"drag",this.__PN,this);}
,__nE:function(e){if(e.isLeftPressed()){this.__PJ={left:e.getDocumentLeft()-qx.bom.element.Location.getLeft(this.__dL),top:e.getDocumentTop()-qx.bom.element.Location.getTop(this.__dL)};qx.bom.Element.addListener(document.body,b,this.__PM,this,true);this.__PL();qx.event.Registration.fireEvent(this.__PI,a,qx.event.type.Event);}
;}
,__PL:qx.core.Environment.select("engine.name",{"mshtml":function(){var h=this;var g=qx.lang.Function.bind(this.__PM,h);document.getElementsByTagName("html")[0].onmouseleave=g;}
,"default":qx.lang.Function.empty}),__PM:function(e){try{e.stopPropagation();}
catch(i){}
;if(demobrowser.demo.bom.portal.dragdrop.Manager.getInstance().isSessionActive()){qx.event.Registration.removeListener(document.body,"mousemove",this.__PN,this,true);demobrowser.demo.bom.portal.dragdrop.Manager.getInstance().stopSession();}
;qx.bom.Element.removeListener(document.body,"mouseup",this.__PM,this,true);}
,__uo:function(e){if(demobrowser.demo.bom.portal.dragdrop.Manager.getInstance().isSessionActive()){return;}
;demobrowser.demo.bom.portal.box.Manager.getInstance().setActiveBox(this.__Pu);demobrowser.demo.bom.portal.dragdrop.Manager.getInstance().startSession(this.__Pu);qx.event.Registration.addListener(document.body,d,this.__PN,this,true);}
,__PN:function(e){e.stopPropagation();var j=e.getDocumentLeft()-this.__PJ.left;var top=e.getDocumentTop()-this.__PJ.top;qx.event.Timer.once(function(){this.checkGroupBox(j,parseInt(this.getActiveBox().getData().width));this.onDragMove(top,j);}
,demobrowser.demo.bom.portal.dragdrop.Manager.getInstance(),0);}
},destruct:function(){this.__Pu=this.__PI=this.__dL=this.__PJ=null;}
});}
)();
(function(){var m="Boolean",l="left",k="demobrowser.demo.bom.portal.dragdrop.Manager",h="end",g="border",f="top",d="demobrowser.demo.bom.portal.box.Box",c="ease-out",b="singleton",a="px";qx.Class.define(k,{type:b,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__PO={top:null,height:null};this.__PP={top:0,left:0};}
,properties:{activeBox:{check:d,init:null,nullable:true},sessionActive:{check:m,init:false}},members:{__PO:null,__PQ:null,__PR:false,__PP:null,__PS:null,__PT:null,startSession:function(n){this.setSessionActive(true);this.setActiveBox(n);this.__PQ=n.getGroupBoxId();this.__PU();if(this.__PS==null){this.__PS={};var o=demobrowser.demo.bom.portal.box.Manager.getInstance().getGroupBoxes();for(var i=0,j=o.length;i<j;i++){this.__PS[o[i].element.id]={left:qx.bom.element.Location.getLeft(o[i].element),right:qx.bom.element.Location.getLeft(o[i].element)+qx.bom.element.Dimension.getWidth(o[i].element)};}
;}
;}
,__PU:function(){if(!this.__PT){this.__PT=qx.dom.Element.create("div");}
;var r=this.getActiveBox().getElement();var p=qx.bom.element.Dimension.getContentSize(r);this.__PV(r,this.__PT);qx.bom.element.Class.add(this.__PT,qx.bom.element.Class.get(r));var q="opacity:0.5;margin:0;position:absolute;"+"top:"+qx.bom.element.Location.getTop(r)+"px;"+"left:"+qx.bom.element.Location.getLeft(r)+"px;"+"width:"+p.width+"px;"+"height:"+p.height+"px";qx.bom.element.Style.setCss(this.__PT,q);qx.bom.element.Style.set(r,"border","1px dashed red");qx.dom.Node.getBodyElement(r).appendChild(this.__PT);demobrowser.demo.bom.portal.box.Util.bringToFront(this.__PT);}
,__PV:function(s,t){while(s.firstChild){t.appendChild(s.firstChild);}
;}
,stopSession:function(e){this.setSessionActive(false);var y=this.getActiveBox();var v=y.getElement();demobrowser.demo.bom.portal.box.Manager.getInstance().updateGroupBoxMembers(y.getGroupBoxId(),this.__PQ,y);y.setGroupBoxId(this.__PQ);var x={left:qx.bom.element.Location.getLeft(this.__PT)+a,top:qx.bom.element.Location.getTop(this.__PT)+a};var w={left:qx.bom.element.Location.getLeft(v)+a,top:qx.bom.element.Location.getTop(v)+a};var z={timing:c,duration:300,keep:100,keyFrames:{'0':x,'100':w}};var u=qx.bom.element.Animation.animate(this.__PT,z);u.addListener(h,function(){this.__PV(this.__PT,v);qx.bom.element.Style.reset(v,g);this.__PT.parentNode.removeChild(this.__PT);if(y.isActive()){y._applyActive(true);}
else {demobrowser.demo.bom.portal.box.Manager.getInstance().setActiveBox(y);}
;}
,this);}
,onDragMove:function(top,A){if(this.__PP.left!=A){qx.bom.element.Style.set(this.__PT,l,A+a);}
;if(this.__PP.top!=top){qx.bom.element.Style.set(this.__PT,f,top+a);}
;var B=this.getActiveBox().getElement();if(this.__PO.top==null){this.__PO.top=qx.bom.element.Location.getTop(B);}
;if(this.__PO.height==null){this.__PO.height=qx.bom.element.Dimension.getHeight(B);}
;if(this.__PR==false&&top>=this.__PO.top&&top<=(this.__PO.top+this.__PO.height)){return;}
;var C;if(this.__PR){C=qx.dom.Hierarchy.getFirstDescendant(document.getElementById(this.__PQ));while(C!=null){if(this.__PW(true,top,B,C)){this.__PR=false;return;}
;C=qx.dom.Hierarchy.getNextElementSibling(C);}
;}
else {C=B;}
;if(top-this.__PP.top>0){C=qx.dom.Hierarchy.getNextElementSibling(C);while(C!=null){if(C){if(this.__PW(true,top,B,C)){return;}
;}
;C=qx.dom.Hierarchy.getNextElementSibling(C);}
;}
else {var C=qx.dom.Hierarchy.getPreviousElementSibling(C);while(C!=null){if(C){if(this.__PW(false,top,B,C)){return;}
;}
;C=qx.dom.Hierarchy.getPreviousElementSibling(C);}
;}
;this.__PP.left=A;this.__PP.top=top;}
,__PW:function(D,top,E,F){var G=qx.bom.element.Location.getTop(F);var I=parseInt(qx.bom.element.Style.get(F,"paddingTop"));var J=parseInt(qx.bom.element.Style.get(F,"paddingBottom"));var H=qx.bom.element.Dimension.getContentHeight(F)+J+I;var K;if(D){if(top>=(G+(H/3))){K=qx.dom.Hierarchy.getNextElementSibling(F);if(K!=E||K==null){qx.dom.Element.insertAfter(E,F);this.__PO.top=qx.bom.element.Location.getTop(E);}
;return true;}
;}
else {if(top<=(G+H*2/3)){K=qx.dom.Hierarchy.getPreviousElementSibling(F);if(K!=E||K==null){qx.dom.Element.insertBefore(E,F);this.__PO.top=qx.bom.element.Location.getTop(E);}
;return true;}
;}
;return false;}
,checkGroupBox:function(L,M){if(this.__PS[this.__PQ].left<=L&&this.__PS[this.__PQ].right>=L){return;}
;for(var N in this.__PS){if(N!=this.__PQ){if(this.__PS[N].left<=L+M&&this.__PS[N].right>=L){this.__PQ=N;this.__PR=true;return;}
;}
;}
;}
},destruct:function(){this.__PO=this.__PP=this.__PQ=null;this.__PX=this.__PT=this.__PS=null;}
});}
)();
...
(function(){var k="repeat",j="timing",i="end",h="Anni",g="The key 'reverse' is deprecated: Please use the method ",f="keep",d=":",c="} ",b="origin",a="forwards",A="end-event",z="reverse",y="css.animation",x="ms ",w="% {",v="'animateReverse' instead.",u="linear",t=";",s="qx.bom.element.AnimationCss",r="keyframes",p="fill-mode",q="alternate",n="qx.debug",o="name",l="",m=" ";qx.Bootstrap.define(s,{statics:{__sx:null,__sy:h,__bV:0,__sz:{},__sA:{"scale":true,"rotate":true,"skew":true,"translate":true},__sB:qx.core.Environment.get(y),animateReverse:function(B,C,D){return this._animate(B,C,D,true);}
,animate:function(E,F,G){return this._animate(E,F,G,false);}
,_animate:function(H,I,J,K){this.__sE(I);if(I.hasOwnProperty(z)){K=I.reverse;if(qx.core.Environment.get(n)){qx.log.Logger.warn(g+v);qx.log.Logger.trace();}
;}
;if(qx.core.Environment.get(n)){this.__BU(I);}
;var N=I.keep;if(N!=null&&(K||(I.alternate&&I.repeat%2==0))){N=100-N;}
;if(!this.__sx){this.__sx=qx.bom.Stylesheet.createElement();}
;var M=I.keyFrames;if(J==undefined){J=I.duration;}
;if(this.__sB!=null){var name=this.__sF(M,K);var L=name+m+J+x+I.repeat+m+I.timing+m+(I.alternate?q:l);var O=this.__sB[A];qx.bom.Event.addNativeListener(H,O,this.__sC);H.style[qx.lang.String.camelCase(this.__sB[o])]=L;if(N&&N==100&&this.__sB[p]){H.style[this.__sB[p]]=a;}
;}
...
;if(this.__sB==null){window.setTimeout(function(){qx.bom.element.AnimationCss.__sC({target:H});}
,0);}
;return P;}
,__sC:function(e){var Q=e.target;var R=Q.$$animation;if(!R){return;}
;var T=R.desc;if(qx.bom.element.AnimationCss.__sB!=null){var S=qx.lang.String.camelCase(qx.bom.element.AnimationCss.__sB[o]);Q.style[S]=l;qx.bom.Event.removeNativeListener(Q,qx.bom.element.AnimationCss.__sB[o],qx.bom.element.AnimationCss.__sC);}
;if(T.origin!=null){qx.bom.element.Transform.setOrigin(Q,l);}
;qx.bom.element.AnimationCss.__sD(Q,T.keyFrames[R.keep]);Q.$$animation=null;R.el=null;R.ended=true;R.emit(i,Q);}
,__sD:function(U,V){var X;for(var W in V){if(W in qx.bom.element.AnimationCss.__sA){if(!X){X={};}
;X[W]=V[W];}
else {U.style[qx.lang.String.camelCase(W)]=V[W];}
;}
;if(X){qx.bom.element.Transform.transform(U,X);}
;}
,__sE:function(Y){if(!Y.hasOwnProperty(q)){Y.alternate=false;}
...
;}
,__BU:qx.core.Environment.select("qx.debug",{"true":function(ba){var bb=["origin","duration","keep","keyFrames","repeat","timing","alternate","reverse"];for(var name in ba){if(!(bb.indexOf(name)!=-1)){qx.Bootstrap.warn("Unknown key '"+name+"' in the animation description.");}
;}
...
;}
;}
,"default":null}),__sF:function(frames,bd){var bg=l;for(var bj in frames){bg+=(bd?-(bj-100):bj)+w;var bf=frames[bj];var bi;for(var be in bf){if(be in this.__sA){if(!bi){bi={};}
;bi[be]=bf[be];}
else {bg+=be+d+bf[be]+t;}
;}
;if(bi){bg+=qx.bom.element.Transform.getCss(bi);}
;bg+=c;}
;if(this.__sz[bg]){return this.__sz[bg];}
;var name=this.__sy+this.__bV++;var bh=this.__sB[r]+m+name;qx.bom.Stylesheet.addRule(this.__sx,bh,bg);this.__sz[bg]=name;return name;}
}});}
)();
...
(function(){var g="Element",f="qx.bom.element.AnimationHandle",e="play-state",d="paused",c="running",b="",a="css.animation";qx.Bootstrap.define(f,{extend:qx.event.Emitter,construct:function(){var h=qx.core.Environment.get(a);this.__sG=h&&h[e];this.__sH=true;}
,events:{"end":g},members:{__sG:null,__sH:false,__sI:false,isPlaying:function(){return this.__sH;}
,isEnded:function(){return this.__sI;}
,pause:function(){if(this.el){this.el.style[this.__sG]=d;this.el.$$animation.__sH=false;if(this.animationId&&qx.bom.element.AnimationJs){qx.bom.element.AnimationJs.pause(this);}
;}
;}
,play:function(){if(this.el){this.el.style[this.__sG]=c;this.el.$$animation.__sH=true;if(this.i!=undefined&&qx.bom.element.AnimationJs){qx.bom.element.AnimationJs.play(this);}
;}
;}
,stop:function(){if(this.el&&qx.core.Environment.get(a)){this.el.style[this.__sG]=b;this.el.style[qx.core.Environment.get(a).name]=b;this.el.$$animation.__sH=false;this.el.$$animation.__sI=true;}
...
,getCss:function(H){var J=this.__sv(H);if(this.__su!=null){var I=this.__su[l];return qx.lang.String.hyphenate(I)+n+J+o;}
;return j;}
,setOrigin:function(K,L){if(this.__su!=null){K.style[this.__su[e]]=L;}
;}
,getOrigin:function(M){if(this.__su!=null){return M.style[this.__su[e]];}
;return j;}
,setStyle:function(N,O){if(this.__su!=null){N.style[this.__su[h]]=O;}
;}
,getStyle:function(P){if(this.__su!=null){return P.style[this.__su[h]];}
;return j;}
,setPerspective:function(Q,R){if(this.__su!=null){Q.style[this.__su[d]]=R+u;}
;}
,getPerspective:function(S){if(this.__su!=null){return S.style[this.__su[d]];}
;return j;}
,setPerspectiveOrigin:function(T,U){if(this.__su!=null){T.style[this.__su[a]]=U;}
;}
,getPerspectiveOrigin:function(V){if(this.__su!=null){var X=V.style[this.__su[a]];if(X!=j){return X;}
else {var Y=V.style[this.__su[a]+b];var W=V.style[this.__su[a]+f];if(Y!=j){return Y+s+W;}
;}
;}
;return j;}
,setBackfaceVisibility:function(ba,bb){if(this.__su!=null){ba.style[this.__su[m]]=bb?k:q;}
;}
,getBackfaceVisibility:function(bc){if(this.__su!=null){return bc.style[this.__su[m]]==k;}
;return true;}
,__sv:function(bd){var bg=j;for(var bf in bd){var be=bd[bf];if(qx.Bootstrap.isArray(be)){for(var i=0;i<be.length;i++){if(be[i]==undefined){continue;}
;bg+=bf+this.__st[i]+g;bg+=be[i];bg+=c;}
;}
...
;var R=L.keyFrames;var P=this.__sR(R);var Q=this.__sQ(M,P);var S=parseInt(M/Q,10);this.__sL(R,K);var T=this.__sM(S,Q,P,R,M,L.timing);var O=new qx.bom.element.AnimationHandle();if(N){T.reverse();O.reverse=true;}
;O.desc=L;O.el=K;O.delta=T;O.stepTime=Q;O.steps=S;K.$$animation=O;O.i=0;O.initValues={};O.repeatSteps=this.__sO(S,L.repeat);return this.play(O);}
,__sL:function(U,V){var W={};for(var ba in U){for(var name in U[ba]){if(W[name]==undefined){var X=U[ba][name];if(typeof X==g){W[name]=X.substring((parseInt(X,10)+p).length,X.length);}
else {W[name]=p;}
;}
;}
;}
...
;if(Y[name]==p&&this.__sK.indexOf(W[name])!=-1){Y[name]=e+W[name];}
;}
;}
;}
;}
,__sM:function(bb,bc,bd,be,bf,bg){var bo=new Array(bb);var bq=1;bo[0]=be[0];var bi=be[0];var bk=be[bd[bq]];for(var i=1;i<bo.length;i++){if(i*bc/bf*100>bd[bq]){bi=bk;bq++;bk=be[bd[bq]];}
;bo[i]={};for(var name in bk){var bp=bk[name]+p;if(bp.charAt(0)==C){var bj=qx.util.ColorUtil.cssStringToRgb(bi[name]);var bn=qx.util.ColorUtil.cssStringToRgb(bp);var bh=[];for(var j=0;j<bj.length;j++){var bl=bj[j]-bn[j];bh[j]=parseInt(bj[j]-bl*this.__sN(bg,i/bb),10);}
;bo[i][name]=qx.util.ColorUtil.rgbToHexString(bh);}
else if(!isNaN(parseInt(bp,10))){var bm=bp.substring((parseInt(bp,10)+p).length,bp.length);var bl=parseFloat(bp)-parseFloat(bi[name]);bo[i][name]=(parseFloat(bi[name])+bl*this.__sN(bg,i/bb))+bm;}
...
;}
;}
;qx.bom.element.AnimationJs.__sP(br.el,bt);br.i++;if(br.i%br.steps==0){if(br.desc.alternate){br.delta.reverse();}
;}
...
;this.__sP(bw,bz.keyFrames[by]);}
else {this.__sP(bw,bx);}
;bw.$$animation=null;bv.el=null;bv.ended=true;bv.animationId=null;bv.emit(h,bw);}
,__sN:function(bA,x){if(bA==B){var a=[3.1223e-7,0.0757,1.2646,-0.167,-0.4387,0.2654];}
...
;return y;}
,__sO:function(bB,bC){if(bC==undefined){return bB;}
;if(bC==w){return Number.MAX_VALUE;}
;return bB*bC;}
,__sP:function(bD,bE){for(var bF in bE){if(bE[bF]===undefined){continue;}
...
;}
;}
,__sQ:function(bG,bH){var bJ=100;for(var i=0;i<bH.length-1;i++){bJ=Math.min(bJ,bH[i+1]-bH[i]);}
;var bI=bG*bJ/100;while(bI>this.__sJ){bI=bI/2;}
;return Math.round(bI);}
,__sR:function(bK){var bL=qx.Bootstrap.getKeys(bK);for(var i=0;i<bL.length;i++){bL[i]=parseInt(bL[i],10);}
