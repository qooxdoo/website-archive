...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.Label","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Label-qx.theme.Indigo.5859c50b4a8a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var t="Big Long Label Cutted",s="My First Long Label Cutted",r="rich label",q="top",p="native context menu",o="text label",n="configure rich label",m="demobrowser.demo.widget.Label",l="nativeContextMenu",k="right",d="configure text label",j="A long label text with auto-wrapping. This also may contain <b style='color:red'>rich HTML</b> markup.",g="Big Long Label",c="execute",b="Verdana",f="sans-serif",e="selectable",h="rich",a="value",i="main";qx.Class.define(m,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var u=new qx.ui.container.Composite(new qx.ui.layout.VBox(20)).set({padding:20});this.getRoot().add(u);var v=new qx.ui.basic.Label(o).set({decorator:i,width:200});u.add(v);this.getRoot().add(this.createConfigureButton(v,d),{left:240,top:20});var w=new qx.ui.basic.Label(r).set({decorator:i,rich:true});u.add(w);this.getRoot().add(this.createConfigureButton(w,n),{left:240,top:60});var x=new qx.ui.basic.Label(s).set({decorator:i,width:80});u.add(x);var y=new qx.ui.basic.Label().set({value:j,decorator:i,rich:true,width:120});u.add(y);var z=new qx.ui.basic.Label(g).set({decorator:i,font:new qx.bom.Font(28,[b,f])});u.add(z);var A=new qx.ui.basic.Label(t).set({decorator:i,font:new qx.bom.Font(28,[b,f]),width:150});u.add(A);}
,createConfigureButton:function(B,C){var E=new qx.ui.form.Button(C);var D;E.addListener(c,function(){if(!D){D=this.createEditDialog(B,C);}
;D.open();}
,this);return E;}
,createEditDialog:function(F,G){var H=new qx.ui.window.Window(G).set({allowMinimize:false,showMinimize:false,allowMaximize:false,showMaximize:false});H.moveTo(350,20);var J=new qx.ui.layout.Grid(10,10);J.setColumnFlex(1,1);J.setColumnAlign(0,k,q);H.setLayout(J);var I=new qx.data.controller.Object(F);H.add(new qx.ui.basic.Label(a).set({paddingTop:5}),{row:0,column:0});var K=new qx.ui.form.TextArea().set({liveUpdate:true});I.addTarget(K,a,a,true);H.add(K,{row:0,column:1});H.add(new qx.ui.basic.Label(e),{row:1,column:0});var L=new qx.ui.form.CheckBox();I.addTarget(L,a,e,true);H.add(L,{row:1,column:1});H.add(new qx.ui.basic.Label(p),{row:2,column:0});var L=new qx.ui.form.CheckBox();I.addTarget(L,a,l,true);H.add(L,{row:2,column:1});H.add(new qx.ui.basic.Label(h),{row:3,column:0});var L=new qx.ui.form.CheckBox().set({enabled:false});I.addTarget(L,a,h);H.add(L,{row:3,column:1});return H;}
}});}
)();
...
(function(){var h="changeModel",g="qx.core.Object",f="qx.data.controller.Object",e="get",d="reset",c="_applyModel",b="last",a="[";qx.Class.define(f,{extend:qx.core.Object,construct:function(j){qx.core.Object.call(this);this.__bg={};this.__el=[];if(j!=null){this.setModel(j);}
;}
,properties:{model:{check:g,event:h,apply:c,nullable:true,dereference:true}},members:{__el:null,__bg:null,_applyModel:function(k,l){for(var i=0;i<this.__el.length;i++){var t=this.__el[i][0];var q=this.__el[i][1];var o=this.__el[i][2];var r=this.__el[i][3];var s=this.__el[i][4];var n=this.__el[i][5];if(l!=undefined&&!l.isDisposed()){this.__zz(t,q,o,l);}
;if(k!=undefined){this.__zy(t,q,o,r,s,n);}
...
;}
;}
,addTarget:function(u,v,w,x,y,z){this.__el.push([u,v,w,x,y,z]);this.__zy(u,v,w,x,y,z);}
,__zy:function(A,B,C,D,E,F){if(this.getModel()==null){return;}
...
,removeTarget:function(J,K,L){this.__zz(J,K,L,this.getModel());for(var i=0;i<this.__el.length;i++){if(this.__el[i][0]==J&&this.__el[i][1]==K&&this.__el[i][2]==L){this.__el.splice(i,1);}
;}
;}
,__zz:function(M,N,O,P){if(!(M instanceof qx.core.Object)){return;}
