...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.FormRenderer","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormRenderer-qx.theme.Classic.4d8c7887591d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var p="female",o="Name",n="Registration",m="male",l="Send",k="send...",j="Bio",i="Gender",h="Country",g="Personal Information",c="Save?",f="Password",e="demobrowser.demo.ui.FormRenderer",b="Reset",a="Age",d="execute";qx.Class.define(e,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var r=new qx.ui.form.Form();r.addGroupHeader(n);var q=new qx.ui.form.TextField();q.setRequired(true);r.add(q,o);var v=new qx.ui.form.PasswordField();v.setRequired(true);r.add(v,f);r.add(new qx.ui.form.CheckBox(),c);r.addGroupHeader(g);r.add(new qx.ui.form.Spinner(),a);r.add(new qx.ui.form.TextField(),h);var u=new qx.ui.form.SelectBox();u.add(new qx.ui.form.ListItem(m));u.add(new qx.ui.form.ListItem(p));r.add(u,i);r.add(new qx.ui.form.TextArea(),j);var t=new qx.ui.form.Button(l);t.addListener(d,function(){if(r.validate()){alert(k);}
;}
,this);r.addButton(t);var s=new qx.ui.form.Button(b);s.addListener(d,function(){r.reset();}
,this);r.addButton(s);this.getRoot().add(new qx.ui.form.renderer.Single(r),{left:10,top:10});}
}});}
)();
(function(){var b="qx.ui.form.Form",a="";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__vV=[];this._buttons=[];this._buttonOptions=[];this._validationManager=new qx.ui.form.validation.Manager();this._resetter=this._createResetter();}
,members:{__vV:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(c,d,e,name,f,g){if(this.__vW()){this.__vV.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
;this.__vV[this._groupCounter].items.push(c);this.__vV[this._groupCounter].labels.push(d);this.__vV[this._groupCounter].options.push(g);if(name==null){name=d.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,a);}
;this.__vV[this._groupCounter].names.push(name);this._validationManager.add(c,e,f);this._resetter.add(c);}
,addGroupHeader:function(h,k){if(!this.__vW()){this._groupCounter++;}
;this.__vV.push({title:h,items:[],labels:[],names:[],options:[],headerOptions:k});}
,addButton:function(l,m){this._buttons.push(l);this._buttonOptions.push(m||null);}
,__vW:function(){return this.__vV.length===0;}
...
,getGroups:function(){return this.__vV;}
,getButtons:function(){return this._buttons;}
,getButtonOptions:function(){return this._buttonOptions;}
,getItems:function(){var n={};for(var i=0;i<this.__vV.length;i++){var o=this.__vV[i];for(var j=0;j<o.names.length;j++){var name=o.names[j];n[name]=o.items[j];}
;}
;return n;}
,_createResetter:function(){return new qx.ui.form.Resetter();}
},destruct:function(){this.__vV=this._buttons=this._buttonOptions=null;this._validationManager.dispose();this._resetter.dispose();}
});}
)();
(function(){var n="Widgets supporting selection can only be validated ",m="Added widget not supported.",l="in the form validator",k="changeValid",j="qx.event.type.Event",h="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",g="qx.ui.form.validation.Manager",f="This field is required",d="qx.event.type.Data",c="complete",a="String",b="";qx.Class.define(g,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__vi=[];this.__vj={};this.setRequiredFieldMessage(qx.locale.Manager.tr(f));}
,events:{"changeValid":d,"complete":j},properties:{validator:{check:h,init:null,nullable:true},invalidMessage:{check:a,init:b},requiredFieldMessage:{check:a,init:b},context:{nullable:true}},members:{__vi:null,__vk:null,__vj:null,__vl:null,add:function(o,p,q){if(!this.__vq(o)){throw new Error(m);}
;if(this.__vg(o)&&!o.getValue){if(p!=null){throw new Error(n+l);}
;}
;var r={item:o,validator:p,valid:null,context:q};this.__vi.push(r);}
,remove:function(s){var t=this.__vi;for(var i=0,u=t.length;i<u;i++){if(s===t[i].item){t.splice(i,1);return s;}
;}
;return null;}
,getItems:function(){var v=[];for(var i=0;i<this.__vi.length;i++){v.push(this.__vi[i].item);}
;return v;}
,validate:function(){var A=true;this.__vl=true;var x=[];for(var i=0;i<this.__vi.length;i++){var y=this.__vi[i].item;var B=this.__vi[i].validator;x.push(y);if(B==null){var w=this.__vm(y);A=A&&w;this.__vl=w&&this.__vl;continue;}
;var w=this.__vn(this.__vi[i],y.getValue());A=w&&A;if(w!=null){this.__vl=w&&this.__vl;}
;}
;var z=this.__vo(x);if(qx.lang.Type.isBoolean(z)){this.__vl=z&&this.__vl;}
;A=z&&A;this.__vr(A);if(qx.lang.Object.isEmpty(this.__vj)){this.fireEvent(c);}
;return A;}
,__vm:function(C){if(C.getRequired()){if(this.__vg(C)){var D=!!C.getSelection()[0];}
...
;return true;}
,__vn:function(G,H){var M=G.item;var L=G.context;var K=G.validator;if(this.__vp(K)){this.__vj[M.toHashCode()]=null;K.validate(M,M.getValue(),this,L);return null;}
...
,__vo:function(N){var P=this.getValidator();var Q=this.getContext()||this;if(P==null){return true;}
;this.setInvalidMessage(b);if(this.__vp(P)){this.__vj[this.toHashCode()]=null;P.validateForm(N,this,Q);return null;}
...
;}
;return R;}
,__vp:function(S){var T=false;if(!qx.lang.Type.isFunction(S)){T=qx.Class.isSubClassOf(S.constructor,qx.ui.form.validation.AsyncValidator);}
;return T;}
,__vq:function(U){var V=U.constructor;return qx.Class.hasInterface(V,qx.ui.form.IForm);}
,__vg:function(W){var X=W.constructor;return qx.Class.hasInterface(X,qx.ui.core.ISingleSelection);}
,__vr:function(Y){var ba=this.__vk;this.__vk=Y;if(ba!=Y){this.fireDataEvent(k,Y,ba);}
;}
,getValid:function(){return this.__vk;}
,isValid:function(){return this.getValid();}
,getInvalidMessages:function(){var bb=[];for(var i=0;i<this.__vi.length;i++){var bc=this.__vi[i].item;if(!bc.getValid()){bb.push(bc.getInvalidMessage());}
;}
;if(this.getInvalidMessage()!=b){bb.push(this.getInvalidMessage());}
;return bb;}
,getInvalidFormItems:function(){var bd=[];for(var i=0;i<this.__vi.length;i++){var be=this.__vi[i].item;if(!be.getValid()){bd.push(be);}
;}
;return bd;}
,reset:function(){for(var i=0;i<this.__vi.length;i++){var bf=this.__vi[i];bf.item.setValid(true);}
;this.__vk=null;}
,setItemValid:function(bg,bh){this.__vj[bg.toHashCode()]=bh;bg.setValid(bh);this.__vs();}
,setFormValid:function(bi){this.__vj[this.toHashCode()]=bi;this.__vs();}
,__vs:function(){var bk=this.__vl;for(var bl in this.__vj){var bj=this.__vj[bl];bk=bj&&bk;if(bj==null){return;}
;}
;this.__vr(bk);this.__vj={};this.fireEvent(c);}
},destruct:function(){this.__vi=null;}
});}
)();
(function(){var a="qx.ui.form.validation.AsyncValidator";qx.Class.define(a,{extend:qx.core.Object,construct:function(b){qx.core.Object.call(this);this.__vt=b;}
,members:{__vt:null,__vu:null,__dB:null,__vv:null,validate:function(c,d,e,f){this.__vv=false;this.__vu=c;this.__dB=e;this.__vt.call(f||this,this,d);}
,validateForm:function(g,h,i){this.__vv=true;this.__dB=h;this.__vt.call(i,g,this);}
,setValid:function(j,k){if(this.__vv){if(k!==undefined){this.__dB.setInvalidMessage(k);}
;this.__dB.setFormValid(j);}
else {if(k!==undefined){this.__vu.setInvalidMessage(k);}
;this.__dB.setItemValid(this.__vu,j);}
;}
},destruct:function(){this.__dB=this.__vu=null;}
});}
)();
...
(function(){var d="qx.ui.form.Resetter",c=" not supported for reseting.",b="Item ",a="The given item has not been added.";qx.Class.define(d,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__mS=[];}
,members:{__mS:null,add:function(e){if(this._supportsValue(e)){var f=e.getValue();}
else if(this.__vg(e)){var f=e.getSelection();}
else if(this.__vh(e)){var f=e.getSelection().concat();}
else {throw new Error(b+e+c);}
;;this.__mS.push({item:e,init:f});}
,reset:function(){for(var i=0;i<this.__mS.length;i++){var g=this.__mS[i];this.__ve(g.item,g.init);}
;}
,resetItem:function(h){var j;for(var i=0;i<this.__mS.length;i++){var k=this.__mS[i];if(k.item===h){j=k.init;break;}
;}
;if(j===undefined){throw new Error(a);}
;this.__ve(h,j);}
,__ve:function(l,m){if(this._supportsValue(l)){l.setValue(m);}
else if(this.__vg(l)||this.__vh(l)){l.setSelection(m);}
;}
,redefine:function(){for(var i=0;i<this.__mS.length;i++){var n=this.__mS[i].item;this.__mS[i].init=this.__vf(n);}
;}
,redefineItem:function(o){var p;for(var i=0;i<this.__mS.length;i++){if(this.__mS[i].item===o){p=this.__mS[i];break;}
;}
;if(p===undefined){throw new Error(a);}
;p.init=this.__vf(p.item);}
,__vf:function(q){if(this._supportsValue(q)){return q.getValue();}
else if(this.__vg(q)||this.__vh(q)){return q.getSelection();}
;}
,__vg:function(r){var s=r.constructor;return qx.Class.hasInterface(s,qx.ui.core.ISingleSelection);}
,__vh:function(t){var u=t.constructor;return qx.Class.hasInterface(u,qx.data.controller.ISelection);}
...
,properties:{wrap:{check:r,init:true,apply:i},appearance:{refine:true,init:m},singleStep:{check:s,init:20},minimalLineHeight:{check:s,apply:g,init:4},autoSize:{check:r,apply:F,init:false}},members:{__vM:null,__vN:null,__vO:null,setValue:function(J){J=qx.ui.form.AbstractField.prototype.setValue.call(this,J);this.__vP();return J;}
...
;}
,__vP:function(){if(this.isAutoSize()){var O=this.__vQ();if(O){this.__vO=this.__vO||this._getAreaHeight();var Q=this._getScrolledAreaHeight();if(this.getMaxHeight()){var M=this.getInsets();var N=-M.top+this.getMaxHeight()-M.bottom;if(Q>N){this.getContentElement().setStyle(x,u);}
else {this.getContentElement().setStyle(x,v);}
;}
;var P=Math.max(Q,this.__vO);this._setAreaHeight(P);}
else {this.getContentElement().addListenerOnce(q,function(){this.__vP();}
,this);}
;}
;}
,_getAreaHeight:function(){return this.getInnerSize().height;}
,_setAreaHeight:function(R){if(this._getAreaHeight()!==R){this.__vN=R;qx.ui.core.queue.Layout.add(this);qx.ui.core.queue.Manager.flush();this.__vT();}
;}
,_getScrolledAreaHeight:function(){var T=this.__vQ();var S=T.getDomElement();if(S){if(!S.parentNode){qx.html.Element.flush();return this._getScrolledAreaHeight();}
...
;T.setValue(this.getValue());this.__vS(T);if(qx.core.Environment.get(w)==n){if(!S.scrollTop){qx.html.Element.flush();}
;return S.scrollTop+this._getTextSize().height;}
;return S.scrollTop;}
;}
,__vQ:function(){this.__vM=this.__vM||this.__vR();return this.__vM;}
,__vR:function(){var X,V,U,W;X=this.getContentElement();if(!X.getDomElement()){return;}
;U=qx.bom.Collection.create(X.getDomElement()).clone()[0];W=new qx.html.Input(m);W.useElement(U);V=W;V.setStyles({position:y,top:0,left:-9999,height:0,overflow:v},true);V.removeAttribute(A);V.removeAttribute(D);V.setAttribute(E,h);V.setValue(X.getValue());V.insertBefore(X);this.__vS(V);return V;}
,__vS:function(Y){Y=Y.getDomElement();if(Y){Y.scrollTop=10000;}
;}
...
,_applyAutoSize:function(bd,be){if(qx.core.Environment.get(l)){this.__vU();}
;if(bd){this.__vP();this.addListener(p,this.__vP,this);this.addListenerOnce(q,function(){this.getContentElement().setStyle(x,v);}
);}
else {this.removeListener(p,this.__vP);this.getContentElement().setStyle(x,u);}
;}
,_applyDimension:function(bf){qx.ui.form.AbstractField.prototype._applyDimension.call(this);if(qx.core.Environment.get(l)){this.__vU();}
;if(bf===this.getMaxHeight()){this.__vP();}
;}
,__vT:function(){var content=this.getContentElement();var bh=content.getDomElement();var bg=content.getStyle(t);content.setStyle(t,parseInt(bg,10)+1000+k,true);if(bh){qx.bom.element.Dimension.getWidth(bh);}
;content.setStyle(t,bg,true);}
,__vU:function(){if(this.isAutoSize()&&this.getHeight()){this.warn(d+z+G);}
;}
,_getContentHint:function(){var bi=qx.ui.form.AbstractField.prototype._getContentHint.call(this);bi.height=bi.height*this.getMinimalLineHeight();bi.width=this._getTextSize().width*20;if(this.isAutoSize()){bi.height=this.__vN||bi.height;}
;return bi;}
}});}
)();
...
;return z.className;}
,"default":function(B,C){var D={};var F;var E=B.className;if(E){F=E.split(this.__kD);for(var i=0,l=F.length;i<l;i++){D[F[i]]=true;}
...
;return O.className;}
,"default":function(Q,R){var S=new RegExp(g+R.join(u)+g,p);return Q.className=Q.className.replace(S,f).replace(this.__kE,f).replace(this.__kD,j);}
...
;return this;}
;}
;var O=function(ba,bb){return function(bc,bd,be,bf,bg,bh){if(this.length>0){var bi=this[0].nodeType===1?ba[bb](this[0],bc,bd,be,bf,bg,bh):null;if(bi&&bi.nodeType){return this.__vE([bi]);}
else {return bi;}
;}
;return null;}
;}
...
,__vB:/^[^<]*(<(.|\s)+>)[^>]*$|^#([\w-]+)$/,create:function(bs,bt){var bv=qx.bom.Collection;if(bs.nodeType){return new bv(bs);}
else if(typeof bs===C){var bu=bv.__vB.exec(bs);if(bu){return bu[1]?bv.html(bu[1],bt):bv.id(bu[3]);}
...
,__vD:/^.[^:#\[\.,]*$/,not:function(bL){if(this.__vD.test(bL)){var bM=qx.bom.Selector.matches(m+bL+g,this);return this.__vE(bM);}
;var bM=qx.bom.Selector.matches(bL,this);return this.filter(function(bN){return bM.indexOf(bN)===-1;}
);}
,add:function(bO,bP){var bQ=qx.bom.Selector.query(bO,bP);var bR=qx.lang.Array.unique(this.concat(bQ));return this.__vE(bR);}
...
;bY=bY.parentNode;}
;}
);return this.__vE(qx.lang.Array.unique(bW));}
...
;return this.__vE(cb);}
,find:function(cc){var ce=qx.bom.Selector;if(this.length===1){return this.__vE(ce.query(cc,this[0]));}
else {var cd=[];for(var i=0,l=this.length;i<l;i++){cd.push.apply(cd,ce.query(cc,this[i]));}
;return this.__vE(qx.lang.Array.unique(cd));}
;}
...
;return this.__vE(ch);}
,nextAll:function(ci){return this.__sU(v,ci);}
...
;return this.__vE(cl);}
,prevAll:function(cm){return this.__sU(f,cm);}
...
;return this.__vE(co);}
,parents:function(cp){return this.__sU(a,cp);}
,siblings:function(cq){return this.__sU(u,cq);}
,__sU:function(cr,cs){var cu=[];var ct=qx.dom.Hierarchy;for(var i=0,l=this.length;i<l;i++){cu.push.apply(cu,ct[cr](this[i]));}
;var cv=qx.lang.Array.unique(cu);if(cs){cv=qx.bom.Selector.matches(cs,cv);}
;return this.__vE(cv);}
,__vE:function(cw){var cx=new qx.bom.Collection;cx.__vC=this;cw=Array.prototype.slice.call(cw,0);cx.push.apply(cx,cw);return cx;}
,andSelf:function(){return this.add(this.__vC);}
,end:function(){return this.__vC||new qx.bom.Collection();}
,__vF:function(cy,cz){var cE=this[0];var cD=cE.ownerDocument||cE;var cC=cD.createDocumentFragment();var cG=qx.bom.Html.clean(cy,cD,cC);var cI=cC.firstChild;if(cI){var cA=this.length-1;for(var i=0,l=cA;i<l;i++){cz.call(this,this[i],cC.cloneNode(true));}
...
;}
;return this;}
,__vG:function(cJ,cK){var cM=qx.bom.Selector;var cL=qx.lang.Array;var cO=[];for(var i=0,l=cJ.length;i<l;i++){if(qx.core.Environment.get(N)){if(typeof cJ[i]!==C){throw new Error(M+cJ[i]);}
;}
;cO.push.apply(cO,cM.query(cJ[i]));}
;cO=cL.cast(cL.unique(cO),qx.bom.Collection);for(var i=0,cN=this.length;i<cN;i++){cO[cK](this[i]);}
;return this;}
,append:function(cP){return this.__vF(arguments,this.__vH);}
,prepend:function(cQ){return this.__vF(arguments,this.__vI);}
,__vH:function(cR,cS){cR.appendChild(cS);}
,__vI:function(cT,cU){cT.insertBefore(cU,cT.firstChild);}
,appendTo:function(cV){return this.__vG(arguments,h);}
,prependTo:function(cW){return this.__vG(arguments,b);}
,before:function(cX){return this.__vF(arguments,this.__vJ);}
,after:function(cY){return this.__vF(arguments,this.__vK);}
,__vJ:function(da,db){da.parentNode.insertBefore(db,da);}
,__vK:function(dc,dd){dc.parentNode.insertBefore(dd,dc.nextSibling);}
,insertBefore:function(de){return this.__vG(arguments,o);}
,insertAfter:function(df){return this.__vG(arguments,s);}
...
;dg.map(this.__vL).append(this);}
;return this;}
,__vL:function(di){while(di.firstChild){di=di.firstChild;}
;return di;}
...
,replaceAll:function(dl){return this.__vG(arguments,p);}
...
;}
;var ba,bc=[];for(var i=0,l=W.length;i<l;i++){ba=W[i];if(typeof ba===w){ba=this.__tb(ba,X);}
...
(function(){var j="url: ",i="Invalid state",h="head",g="script",f="load",e="Unknown response headers",d="abort",c="Received native readyState: loaded",b="readystatechange",a="Response header cannot be determined for ",D="opera",C="unknown",B="Open native request with ",A="Response headers cannot be determined for",z="Detected error",y="Send native request",x="on",w="timeout",v="Unknown environment key at this phase",u="Received native load",q="error",r="qx.debug",o="requests made with script transport.",p="loadend",m="",n="mshtml",k="browser.documentmode",l="engine.name",s="qx.debug.io",t="qx.bom.request.Script";qx.Bootstrap.define(t,{construct:function(){this.__tL();this.__tG=qx.Bootstrap.bind(this._onNativeLoad,this);this.__tH=qx.Bootstrap.bind(this._onNativeError,this);this.__tj=qx.Bootstrap.bind(this._onTimeout,this);this.__tI=document.head||document.getElementsByTagName(h)[0]||document.documentElement;this._emitter=new qx.event.Emitter();this.timeout=this.__tN()?0:15000;}
,events:{"readystatechange":t,"error":t,"loadend":t,"timeout":t,"abort":t,"load":t},members:{readyState:null,status:null,statusText:null,timeout:null,__tJ:null,on:function(name,E,F){this._emitter.on(name,E,F);return this;}
,open:function(G,H){if(this.__tt){return;}
;this.__tL();this.__tl=null;this.__to=H;if(this.__tQ(s)){qx.Bootstrap.debug(qx.bom.request.Script,B+j+H);}
;this._readyStateChange(1);}
,setRequestHeader:function(I,J){if(this.__tt){return;}
;var K={};if(this.readyState!==1){throw new Error(i);}
;K[I]=J;this.__to=qx.util.Uri.appendParamsToUrl(this.__to,K);return this;}
,send:function(){if(this.__tt){return;}
;var N=this.__tO(),L=this.__tI,M=this;if(this.timeout>0){this.__dy=window.setTimeout(this.__tj,this.timeout);}
;if(this.__tQ(s)){qx.Bootstrap.debug(qx.bom.request.Script,y);}
;L.insertBefore(N,L.firstChild);window.setTimeout(function(){M._readyStateChange(2);M._readyStateChange(3);}
);return this;}
,abort:function(){if(this.__tt){return;}
;this.__tl=true;this.__tP();this._emit(d);return this;}
...
,onabort:function(){}
,ontimeout:function(){}
,getResponseHeader:function(O){if(this.__tt){return;}
;if(this.__tQ(r)){qx.Bootstrap.debug(a+o);}
;return C;}
,getAllResponseHeaders:function(){if(this.__tt){return;}
;if(this.__tQ(r)){qx.Bootstrap.debug(A+o);}
;return e;}
,setDetermineSuccess:function(P){this.__tJ=P;}
,dispose:function(){var Q=this.__tK;if(!this.__tt){if(Q){Q.onload=Q.onreadystatechange=null;this.__tP();}
;if(this.__dy){window.clearTimeout(this.__dy);}
;this.__tt=true;}
;}
,_getUrl:function(){return this.__to;}
,_getScriptElement:function(){return this.__tK;}
,_onTimeout:function(){this.__tM();if(!this.__tN()){this._emit(q);}
;this._emit(w);if(!this.__tN()){this._emit(p);}
;}
,_onNativeLoad:function(){var T=this.__tK,R=this.__tJ,S=this;if(this.__tl){return;}
;if(this.__tQ(l)===n&&this.__tQ(k)<9){if(!(/loaded|complete/).test(T.readyState)){return;}
else {if(this.__tQ(s)){qx.Bootstrap.debug(qx.bom.request.Script,c);}
;}
;}
;if(this.__tQ(s)){qx.Bootstrap.debug(qx.bom.request.Script,u);}
;if(R){if(!this.status){this.status=R()?200:500;}
;}
;if(this.status===500){if(this.__tQ(s)){qx.Bootstrap.debug(qx.bom.request.Script,z);}
;}
;if(this.__dy){window.clearTimeout(this.__dy);}
;window.setTimeout(function(){S._success();S._readyStateChange(4);S._emit(f);S._emit(p);}
);}
,_onNativeError:function(){this.__tM();this._emit(q);this._emit(p);}
,__tK:null,__tI:null,__to:m,__tG:null,__tH:null,__tj:null,__dy:null,__tl:null,__tt:null,__tL:function(){this.readyState=0;this.status=0;this.statusText=m;}
,_readyStateChange:function(U){this.readyState=U;this._emit(b);}
,_success:function(){this.__tP();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=m+this.status;}
,__tM:function(){this.__tP();this.readyState=4;this.status=0;this.statusText=null;}
,__tN:function(){var W=this.__tQ(l)===n&&this.__tQ(k)<9;var V=this.__tQ(l)===D;return !(W||V);}
,__tO:function(){var X=this.__tK=document.createElement(g);X.src=this.__to;X.onerror=this.__tH;X.onload=this.__tG;if(this.__tQ(l)===n&&this.__tQ(k)<9){X.onreadystatechange=this.__tG;}
;return X;}
,__tP:function(){var Y=this.__tK;if(Y&&Y.parentNode){this.__tI.removeChild(Y);}
;}
,__tQ:function(ba){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(ba);}
