...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.HtmlArea-qx.theme.Classic.f62ab7f72dea.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,this.__yO);qx.dom.Element.insertBegin(cI,cH);return cH;}
,__yW:function(){var cP=qx.dom.Element.create(bE);qx.bom.element.Style.setStyles(cP,{"marginRight":bx,"float":by});var cR;var cQ=qx.dom.Element.create(bj,{title:m});if(qx.core.Environment.get(bA)==bD){qx.bom.element.Attribute.set(cQ,bC,bF);}
...
;}
,this.__yO);qx.dom.Element.insertBegin(cQ,cP);return cP;}
,__yY:function(){var cW=[{bold:{text:cn,image:s,action:this.__yO.setBold},italic:{text:bu,image:U,action:this.__yO.setItalic},underline:{text:B,image:bt,action:this.__yO.setUnderline},strikethrough:{text:C,image:ct,action:this.__yO.setStrikeThrough},removeFormat:{text:M,image:h,action:this.__yO.removeFormat}},{alignLeft:{text:cc,image:t,action:this.__yO.setJustifyLeft},alignCenter:{text:X,image:cd,action:this.__yO.setJustifyCenter},alignRight:{text:bv,image:p,action:this.__yO.setJustifyRight},alignJustify:{text:cl,image:bb,action:this.__yO.setJustifyFull}},{fontFamily:{custom:this.__yV},fontSize:{custom:this.__yW},fontColor:{text:b,image:c,action:this.__yP},textBackgroundColor:{text:bN,image:bR,action:this.__yQ}},{indent:{text:bT,image:bn,action:this.__yO.insertIndent},outdent:{text:D,image:n,action:this.__yO.insertOutdent}},{insertImage:{text:o,image:K,action:this.__yR},insertTable:{text:ca,image:f,action:this.__yS},insertLink:{text:bQ,image:H,action:this.__yT},insertHTML:{text:O,image:bK,action:this.__yU},insertHR:{text:bJ,image:bM,action:this.__yO.insertHorizontalRuler}},{ol:{text:I,image:bo,action:this.__yO.insertOrderedList},ul:{text:z,image:bL,action:this.__yO.insertUnorderedList}},{undo:{text:T,image:g,action:this.__yO.undo},redo:{text:co,image:Y,action:this.__yO.redo}}];var cY,dd,cV,db;for(var i=0,j=cW.length;i<j;i++){dd={};dd[cr]=i==j-1?bG:bx;dd[cg]=by;cY=qx.dom.Element.create(bE);qx.bom.element.Style.setStyles(cY,dd);for(var da in cW[i]){var dc=cW[i][da];if(dc.custom){cV=dc.custom.call(this);}
...
;qx.event.Registration.addListener(cV,cu,dc.action,this.__yO);}
;qx.dom.Element.insertEnd(cV,cY);}
;qx.dom.Element.insertEnd(cY,this.__Pi);}
;qx.dom.Element.insertEnd(qx.dom.Element.create(cs,{style:cb}),this.__Pi);}
}});}
)();
...
;}
;return eL;}
,__xi:function(eP,eQ,eR,eS){var eX=[];switch(eP.nodeType){case 1:case 11:var i;var eV=eP.tagName.toLowerCase();var fa={};var eT={};var closed=(!(eP.hasChildNodes()||qx.bom.htmlarea.HtmlArea.__xj(eP)));if(eQ){if((qx.core.Environment.get(dK)==dI)){if(eV==eq&&eP.className&&eP.className==L){for(i=eP.firstChild;i;i=i.nextSibling){eX.push(qx.bom.htmlarea.HtmlArea.__xi(i,true,eR,eS));}
;return eX.join(dL);}
;}
...
;fa[name]=fb;}
;if(fa.style!==undefined){eT=qx.bom.htmlarea.HtmlArea.__xh(fa.style);delete fa.style;}
;if(eS){var eU={domElement:eP,tag:eV,attributes:fa,styles:eT};eS(eU);eU.domElement=null;eV=eU.tag;}
;if(eV){eX.push(ct,eV);for(var name in fa){var fb=fa[name];eX.push(bV,name,dn,fb.toString().replace(new RegExp(cL,z),bm),cL);}
...
;}
;for(i=eP.firstChild;i;i=i.nextSibling){eX.push(qx.bom.htmlarea.HtmlArea.__xi(i,true,eR,eS));}
;if(eQ&&!closed&&eV){eX.push(dx,eV,cg);}
;break;case 3:eX.push(eR?eP.data:qx.bom.htmlarea.HtmlArea.__xk(eP.data));break;case 8:eX.push(ed,eP.data,dA);break;};return eX.join(dL);}
,closingTags:cJ,__xj:function(fc){return (qx.bom.htmlarea.HtmlArea.closingTags.indexOf(bV+fc.tagName+bV)!=-1);}
,__xk:function(s){s=s.replace(/&/ig,y);s=s.replace(/</ig,cG);s=s.replace(/>/ig,dp);s=s.replace(/\x22/ig,bg);s=s.replace(/\xA9/ig,dW);return s;}
...
},properties:{contentType:{check:cj,init:bn},messengerMode:{check:bW,init:false},insertParagraphOnLinebreak:{check:bW,init:true},insertLinebreakOnCtrlEnter:{check:bW,init:true},postProcess:{check:dP,nullable:true,init:null},useUndoRedo:{check:bW,init:true},nativeContextMenu:{check:bW,init:false},defaultFontFamily:{check:cj,init:dl},defaultFontSize:{check:cB,init:4}},members:{__ks:null,__hg:false,__xl:false,__xm:false,__xn:false,__xo:false,__wq:null,__xp:null,__xq:null,__xr:null,__rP:null,__xe:null,__xs:null,__xt:null,__xu:false,__xv:false,__xw:function(fj){if(qx.dom.Node.isElement(fj)&&qx.dom.Node.isNodeName(fj,eq)){this.__ks=fj;}
;}
,_createAndAddIframe:function(fk){this.__rP=qx.bom.Iframe.create();qx.bom.Iframe.setSource(this.__rP,fk);if((qx.core.Environment.get(dK)==dI)){qx.bom.element.Attribute.set(this.__rP,br,bN);}
else {qx.bom.element.Style.set(this.__rP,bv,bs);}
;qx.bom.element.Style.setStyles(this.__rP,{width:l,height:l});qx.dom.Element.insertBegin(this.__rP,this.__ks);}
,_getIframeDocument:function(){return qx.bom.Iframe.getDocument(this.__rP);}
,_getIframeWindow:function(){return qx.bom.Iframe.getWindow(this.__rP);}
,_addIframeLoadListener:function(){qx.event.Registration.addListener(this.__rP,cq,this._loaded,this);}
,__xx:function(){this.__xs={"xhtml":{doctype:bG+eA+M+bp+bk+dj,html:bq+eb,meta:N,style:qx.core.Environment.select(dK,{"mshtml":ba+dG,"default":dF+eB}),contentStyle:dH,body:ew,footer:ch}};}
,__xf:dL,getIframeObject:function(){return this.__rP;}
,getCommandManager:function(){return this.__wq;}
,setValue:function(fl){if(qx.lang.Type.isString(fl)){this.__xf=fl;var fm=this._getIframeDocument();if(fm&&fm.body){fm.body.innerHTML=this.__xy(fl);}
;}
;}
,__xy:function(fn){var fq=dL;if((qx.core.Environment.get(dK)==dJ)){if(qx.core.Environment.get(ez)<=2){fq+=cF;}
;}
;var fr=fn.length==0?bH:dL;var fp=qx.core.Environment.get(dK)==dJ||qx.core.Environment.get(dK)==cb?cn:bP;var fo=t+P+this.getDefaultFontFamily()+b+co+fp+ee+this.getDefaultFontSize()+b+fq+fn+fr+u+dU+U;return fo;}
,getValue:function(){return this.__xf;}
,getComputedValue:function(fs){return this.getHtml(fs);}
,getCompleteHtml:function(){var fu=this.__xs[this.getContentType()];var ft=fu.html+h+fu.meta+cy+fu.contentStyle+cI+k;ft+=dc+this.__xz()+ec;ft+=this.getHtml()+ch;return ft;}
,__xz:function(){var fx=[dT,bD,v,ck,cr,dO,bE,bu,eu,bf,T,dk,p,dC];var fw=qx.bom.element.Style;var fv=this.getContentBody();var fA={};var fz,fB;var fy=qx.core.Environment.get(dK)==dI?2:1;for(var i=0,j=fx.length;i<j;i++){fz=fx[i];fB=fw.get(fv,fz,fy);if(fB!==undefined&&fB!=dL){fA[fz]=fB;}
;}
;return qx.bom.element.Style.compile(fA);}
,getContentDocument:function(){if(this.__hg){return this._getIframeDocument();}
;}
,getContentBody:function(){if(this.__hg){return this._getIframeDocument().body;}
;}
,getContentWindow:function(){if(this.__hg){return this._getIframeWindow();}
;}
...
;}
;return fR;}
,__xA:0,__xB:function(){var fS=this._getIframeDocument();if(!fS){this.__xA++;if(this.__xA>5){this.error(bC+fS);this.fireDataEvent(cH);}
else {if(qx.core.Environment.get(dm)){this.error(cp);}
;qx.event.Timer.once(function(){this.__xB();}
,this,0);}
;}
else {this.__xA=0;this._onDocumentIsReady();}
;}
,_loaded:function(e){if(this.__xm){return;}
;if(this.__xl){this.__xC();return;}
;if((qx.core.Environment.get(dK)==dI)){this.__xB();}
else {this._onDocumentIsReady();}
;}
,isReady:function(){return this.__hg;}
...
;this.__xm=true;if((qx.core.Environment.get(dK)==dI)){this.setEditable(true);}
;this.__xF();if(!(qx.core.Environment.get(dK)==bS)){this.__xG();}
;if(!(qx.core.Environment.get(dK)==dI)){this.setEditable(true);}
;this.__hg=true;this.__wq=fT;fT.setContentDocument(this._getIframeDocument());this.__xL();if((qx.core.Environment.get(dK)==bS)){this.__xG();}
;this.fireEvent(et);}
...
else {fU.body.contentEditable=true;this.__xM(true);}
;}
;}
,"default":qx.lang.Function.empty}),invalidateEditor:qx.core.Environment.select(dK,{"gecko":function(){this.__xm=false;this.__hg=false;this.__xl=true;}
,"default":function(){}
}),__xC:qx.core.Environment.select(dK,{"gecko":function(){this.__xF();this.__xG();this.__wq.setContentDocument(this._getIframeDocument());this.setEditable(true);this.forceEditable();this.__xm=true;this.__hg=true;this.__xl=false;this.fireEvent(eg);}
,"default":function(){}
}),__xD:function(fV){var fX=dL;if(!fV){return fX;}
;try{var fW=fV.getAttribute(n);if(!fW){return fX;}
;if((qx.core.Environment.get(dK)==dI)){fX=fW.cssText;}
else {fX=fW;}
;}
catch(fY){this.error(o);}
;return fX;}
,__xE:function(ga){var gb=qx.core.Environment.get(dK)==dJ?ey:dL;var gc=this.__xs[this.getContentType()];var gd=h+gc.meta+cy+gb+gc.style+gc.contentStyle+this.__xe+cI+k;var content=gc.body+ga;return gc.html+gd+content+gc.footer;}
,__xF:function(){var gf=this.__xy(this.getValue());if(qx.lang.Type.isString(gf)){var ge=this._getIframeDocument();try{ge.open(bM,true);ge.write(this.__xE(gf));ge.close();}
catch(e){this.error(dM+qx.bom.Iframe.queryCurrentUrl(this.__rP)+bm,e);this.fireDataEvent(cH,e);}
;}
;}
,__xG:function(){this.__xH();this.__xJ();this.__xI();}
,__xH:function(){var gh=qx.event.Registration;var gg=this._getIframeDocument();gh.addListener(gg.body,A,this._handleKeyPress,this);gh.addListener(gg.body,cT,this._handleKeyUp,this);gh.addListener(gg.body,cU,this._handleKeyDown,this);}
,__xI:function(){var gk=qx.event.Registration;var gi=this._getIframeDocument();var gj=qx.core.Environment.get(dK)==cb?this._getIframeWindow():gi.body;gk.addListener(gj,cx,this._handleFocusEvent,this);gk.addListener(gj,bh,this._handleBlurEvent,this);gk.addListener(gi,cS,this._handleFocusOutEvent,this);}
,__xJ:function(){var gn=qx.event.Registration;var gm=this._getIframeDocument();var gl=qx.core.Environment.get(dK)==dI?W:eh;gn.addListener(gm.body,gl,this._handleMouseUpOnBody,this);gn.addListener(gm.documentElement,gl,this._handleMouseUpOnDocument,this);gn.addListener(gm.documentElement,bx,this._handleContextMenuEvent,this);}
,__xK:function(){if(this.__xp==null){this.__xp={execute:function(go,gp){this.stackedCommands=true;this.commandStack.push({command:go,value:gp});}
,commandStack:[],stackedCommands:false};}
;this.__xp.stackedCommands=false;return this.__xp;}
,__xL:function(){var gq=this.__xp;if(gq!=null&&gq.stackedCommands){var gr=gq.commandStack;if(gr!=null){for(var i=0,j=gr.length;i<j;i++){this.__wq.execute(gr[i].command,gr[i].value);}
;}
;}
;}
,__xM:function(gs){var gt=this._getIframeDocument();if(this.__xm&&gt){try{if((qx.core.Environment.get(dK)==dJ)){gt.designMode=(gs!==false)?c:er;}
;gt.designMode=(gs!==false)?er:c;}
catch(e){}
;}
;}
,setEditable:function(gu){if(this.__xm){this.__xM(true);if((qx.core.Environment.get(dK)==dJ)){try{var gv=this._getIframeDocument();gv.execCommand(dr,false,true);}
catch(gw){try{var gv=this._getIframeDocument();gv.execCommand(bJ,false,false);}
catch(gx){if(!this.__hg){this.error(eo);this.fireDataEvent(cH,gx);}
...
;this.__xn=gu;}
;}
,getEditable:function(){return this.__xn;}
,isEditable:function(){return this.__xn;}
,__xN:false,_handleKeyUp:function(e){var gy=e.getKeyIdentifier().toLowerCase();this.__xq=e;if((qx.core.Environment.get(dm))&&qx.core.Environment.get(K)){this.debug(e.getType()+cR+gy);}
;if(qx.core.Environment.get(dK)==dI||qx.core.Environment.get(dK)==cb){if(this.__xN){switch(gy){case cM:if(this.getInsertLinebreakOnCtrlEnter()){if((qx.core.Environment.get(dK)==cb)){this.__xO();e.preventDefault();e.stopPropagation();}
else {var gz=this.__xY(this.getSelection());if(gz){gz.collapse(true);gz.pasteHTML(cE);}
;}
;this.__xT();}
;break;case cv:this.__xN=false;return;break;};}
;}
else if((qx.core.Environment.get(dK)==dJ)){switch(gy){case V:case cN:case bi:case B:case dz:case E:case ce:case ep:case em:this.__xo=(this.getFocusNode()==this.getContentBody().firstChild);break;};}
;}
,__xO:function(){var gA=this.getSelection();var gB=dL;if(gA&&(gA.focusNode.textContent==dL||gA.focusNode.parentElement.tagName==bA)){gB=dQ;}
;this.__wq.execute(cY,gB+qx.bom.htmlarea.HtmlArea.simpleLinebreak);}
...
;if(this.__xN&&(gC==bY||gC==ei||gC==bl||gC==cV||gC==ek||gC==g)){e.preventDefault();e.stopPropagation();}
;if(gC==cv){this.__xN=true;}
;}
,"default":function(e){}
}),_handleKeyPress:function(e){var gH=this.getContentDocument();var gD=e.getKeyIdentifier().toLowerCase();var gF=e.isCtrlPressed();var gG=e.isShiftPressed();this.__xq=e;if((qx.core.Environment.get(dm))&&qx.core.Environment.get(K)){this.debug(e.getType()+cR+gD);}
;if(this.__xu){var gN=!((qx.core.Environment.get(dK)==dI)&&gD==cM)||!((qx.core.Environment.get(dK)==dJ)&&gD==cM);if(gN){this.__xT();this.__xu=false;}
;}
...
;e.preventDefault();e.stopPropagation();if((qx.core.Environment.get(dK)==dJ)){if(this.__xW()){this.insertHtml(bb);this.__xT();return;}
;this.insertHtml(bI);}
else if((qx.core.Environment.get(dK)==bS)){var gM=this.getSelection();var gI=this.__xY(gM);if(gM&&gI){var gJ=gH.createElement(cP);gI.collapse(true);gI.insertNode(gJ);gI.collapse(true);gI.selectNode(gJ);gM.addRange(gI);gI.collapse(true);}
;}
;this.__xT();}
;if((qx.core.Environment.get(dK)==dI)){if(!this.getInsertParagraphOnLinebreak()){if(this.__wq.execute(cY,qx.bom.htmlarea.HtmlArea.simpleLinebreak)){this.__xT();e.preventDefault();e.stopPropagation();}
;}
;}
else if((qx.core.Environment.get(dK)==dJ)){if(this.getInsertParagraphOnLinebreak()&&!gG&&!gF){var gM=this.getSelection();if(gM){var gE=gM.focusNode;if(this.__xW()){this.__xT();return;}
;if(this.__xX()){this.__xT();return;}
;while(!qx.dom.Node.isNodeName(gE,cW)){if(qx.dom.Node.isNodeName(gE,dS)){this.__xT();return;}
;gE=gE.parentNode;}
;}
;this.__wq.insertParagraphOnLinebreak();e.preventDefault();e.stopPropagation();this.__xT();this.__xu=true;}
;}
else if((qx.core.Environment.get(dK)==cb)){if(this.getInsertParagraphOnLinebreak()&&gG){this.__xO();e.preventDefault();e.stopPropagation();this.__xT();}
;}
;;break;case bi:if(qx.core.Environment.get(dK)==dJ&&qx.core.Environment.get(da)<1.9&&gG){var gM=this.getSelection();if(gM&&gM.focusNode==gH.body.firstChild){if(this.__xo){if(gM.focusOffset!=0){gM.extend(gM.focusNode,0);}
;}
;}
;}
;this.__xT();break;case cu:if(qx.core.Environment.get(dK)==dJ&&qx.core.Environment.get(da)<1.9){if(gF){var gM=this.getSelection();if(gG){if(gM&&(gM.focusOffset!=0)||(gM.focusNode!=gH.body.firstChild)){gM.extend(gH.body.firstChild,0);}
;}
...
;}
;}
;this.__xT();break;case V:case cN:case B:case dz:case E:case ce:case ep:case em:this.__xT();break;case bl:if(gF){this.__xP(ea,true);}
;break;case ek:case g:if(gF){this.__xP(bB,true);}
;break;case cV:if(gF){this.__xP(dY,true);}
;break;case bY:if(gF&&!gG){this.__xP(dR,true);}
else if(gF&&gG){this.__xP(d,true);}
;break;case ei:if(gF){this.__xP(d,true);}
;break;case x:if(gF){this.selectAll();}
;break;};this.__xq=null;}
,__xP:function(gO,gP){var gR=null;var gQ=qx.bom.htmlarea.HtmlArea.hotkeyInfo;if(gQ[gO]){gR=gQ[gO].method;}
;if(gR!=null&&this[gR]){this[gR]();if(gP){this.__xq.preventDefault();this.__xq.stopPropagation();}
;if(this.isSelectionCollapsed()){this.__xu=true;}
;this.__xT();}
;}
,_handleFocusEvent:function(e){this.__xr=null;if(qx.core.Environment.get(dK)==dJ||qx.core.Environment.get(dK)==cb){var gS=this.getContentDocument().getElementById(bX);if(gS){qx.bom.element.Attribute.reset(gS,bT);}
;}
;this.fireEvent(cs);}
,_handleBlurEvent:function(e){this.__xf=this.getComputedValue();}
,_handleFocusOutEvent:function(e){this.__xN=false;if(this.__xr==null){this.__xr=this.getSelectedHtml();}
;this.fireEvent(bo);}
,_handleMouseUpOnBody:function(e){if((qx.core.Environment.get(dm))&&qx.core.Environment.get(K)){this.debug(R+e.getType());}
;this.__xv=true;this.__xT();}
,_handleMouseUpOnDocument:qx.core.Environment.select(dK,{"mshtml":qx.lang.Function.empty,"default":function(e){if(!this.__xv){qx.bom.Element.activate(this.getContentBody());}
;this.__xv=false;}
}),_handleContextMenuEvent:function(e){if(!this.getNativeContextMenu()){var gV=e.getViewportLeft();var gU=e.getViewportTop();var gW=qx.bom.element.Location.getLeft(this.__ks)+gV;var gX=qx.bom.element.Location.getTop(this.__ks)+gU;var gT={x:gW,y:gX,relX:gV,relY:gU,target:e.getTarget()};e.preventDefault();e.stopPropagation();qx.event.Timer.once(function(){this.fireDataEvent(bx,gT);}
,this,0);}
;}
,isLoaded:function(){return this.__xm;}
,insertHtml:function(gY){return this.__wq.execute(cY,gY);}
,removeFormat:function(){var ha=this.__wq.execute(ev);this.__wq.execute(F,this.getDefaultFontSize());this.__wq.execute(cw,this.getDefaultFontFamily());return ha;}
,setBold:function(){return this.__wq.execute(en);}
,setItalic:function(){return this.__wq.execute(cX);}
,setUnderline:function(){return this.__wq.execute(cQ);}
,setStrikeThrough:function(){return this.__wq.execute(bL);}
,setFontSize:function(hb){return this.__wq.execute(F,hb);}
,setFontFamily:function(hc){return this.__wq.execute(cw,hc);}
,setTextColor:function(hd){return this.__wq.execute(dN,hd);}
,setTextBackgroundColor:function(he){return this.__wq.execute(dw,he);}
,setJustifyLeft:function(){return this.__wq.execute(S);}
,setJustifyCenter:function(){return this.__wq.execute(bj);}
,setJustifyRight:function(){return this.__wq.execute(di);}
,setJustifyFull:function(){return this.__wq.execute(bd);}
,insertIndent:function(){return this.__wq.execute(bR);}
,insertOutdent:function(){return this.__wq.execute(de);}
,insertOrderedList:function(){return this.__wq.execute(eC);}
,insertUnorderedList:function(){return this.__wq.execute(cA);}
,insertHorizontalRuler:function(){return this.__wq.execute(r);}
,insertImage:function(hf){return this.__wq.execute(w,hf);}
,insertHyperLink:function(hg){return this.__wq.execute(bc,hg);}
,removeBackgroundColor:function(){this.__wq.execute(dy,cK);}
,setBackgroundColor:function(hh){this.__wq.execute(dy,hh);}
,removeBackgroundImage:function(){this.__wq.execute(m);}
,setBackgroundImage:function(hi,hj,hk){return this.__wq.execute(m,[hi,hj,hk]);}
,selectAll:function(){return this.__wq.execute(cD);}
,undo:function(){if(this.getUseUndoRedo()){return this.__wq.execute(G);}
else {return true;}
;}
,redo:function(){if(this.getUseUndoRedo()){return this.__wq.execute(ej);}
else {return true;}
;}
...
;return qx.bom.htmlarea.HtmlArea.__xi(hp.body,false,ho,this.getPostProcess());}
,containsOnlyPlaceholder:qx.core.Environment.select(dK,{"mshtml":function(){var hq=this._getIframeDocument();return (hq.body.innerHTML==du);}
...
;this.__xQ();}
,"opera":function(){qx.bom.Element.focus(this.getContentWindow());qx.bom.Element.focus(this.getContentBody());this.__xQ();}
,"default":function(){qx.bom.Element.focus(this.getContentBody());this.__xQ();}
}),__xQ:function(){if(!this.__xR()){this.__xS();}
;}
,__xR:qx.core.Environment.select(dK,{"gecko":function(){var hx=this.getContentBody().childNodes;if(hx.length==0){return false;}
...
;}
}),__xS:qx.core.Environment.select(dK,{"gecko|webkit":function(){this.getContentDocument().body.innerHTML=this.__xy(dL);var hA=this.getContentDocument().getElementById(bX);qx.bom.element.Attribute.reset(hA,bT);this._selectElement(hA);}
...
;}
}),getContextInformation:function(){return this.__xU();}
,__xT:function(){qx.event.Timer.once(function(e){var hC=this.__xU();this.fireDataEvent(ef,hC);}
,this,200);}
,__xU:function(){if(this._processingExamineCursorContext||this.getEditable()==false){return;}
;this._processingExamineCursorContext=true;if(!this.__xR()){this.__xS();}
...
;if(hV==null||hV==dL){hV=qx.bom.element.Attribute.get(hE,q);}
;hE=hE.parentNode;}
;}
...
}),getSelectedHtml:function(){if(this.__xr!=null){return this.__xr;}
;var hW=this.getRange();if(!hW){return dL;}
else {return this.__xV(hW);}
;}
,__xV:qx.core.Environment.select(dK,{"mshtml":function(hX){if(!hX){return dL;}
...
;}
}),__xW:qx.core.Environment.select(dK,{"gecko":function(){var ig=this.getSelection();var ih=this.getFocusNode();return ig&&this.isSelectionCollapsed()&&ih!=null&&qx.dom.Node.isText(ih)&&ig.anchorOffset<ih.length;}
,"default":function(){return false;}
}),__xX:qx.core.Environment.select(dK,{"gecko":function(){return qx.dom.Node.isElement(this.getFocusNode());}
,"default":function(){return false;}
}),getRange:function(){return this.__xY(this.getSelection());}
,__xY:qx.core.Environment.select(dK,{"mshtml":function(ii){var ij=this._getIframeDocument();if(ii){try{return ii.createRange();}
...
;}
}),saveRange:qx.core.Environment.select(dK,{"mshtml":function(){this.__xt=this.getRange();}
,"default":function(){}
}),getSavedRange:qx.core.Environment.select(dK,{"mshtml":function(){return this.__xt;}
,"default":function(){}
}),resetSavedRange:qx.core.Environment.select(dK,{"mshtml":function(){this.__xt=null;}
,"default":function(){}
}),getFocusNode:qx.core.Environment.select(dK,{"mshtml":function(){var ip=this.getSelection();var iq;switch(ip.type){case cz:case cd:iq=this.__xY(ip);if(iq){iq.collapse(false);return iq.parentElement();}
else {return null;}
;break;case Q:iq=this.__xY(ip);if(iq){try{iq.collapse(false);}
catch(ir){}
;return iq.item(0);}
else {return null;}
...
})},environment:{"qx.bom.htmlarea.HtmlArea.debug":false},destruct:function(){try{var iu=this._getIframeDocument();var iw=qx.event.Registration;iw.removeListener(iu.body,A,this._handleKeyPress,this);iw.removeListener(iu.body,cT,this._handleKeyUp,this);iw.removeListener(iu.body,cU,this._handleKeyDown,this);var iv=qx.core.Environment.get(dK)==cb?this._getIframeWindow():iu.body;iw.removeListener(iv,cx,this._handleFocusEvent);iw.removeListener(iv,bh,this._handleBlurEvent);iw.removeListener(iu,cS,this._handleFocusOutEvent);var it=qx.core.Environment.get(dK)==dI?W:eh;iw.removeListener(iu.body,it,this._handleMouseUpOnBody,this);iw.removeListener(iu.documentElement,it,this._handleMouseUpOnDocument,this);iw.removeListener(iu.documentElement,bx,this._handleContextMenuEvent,this);if((qx.core.Environment.get(dK)==dI)){qx.bom.Iframe.setSource(this.__rP,df);}
;}
catch(ix){}
;if(this.__wq instanceof qx.core.Object){this._disposeObjects(ds);}
else {this.__wq=null;}
;this.__xs=this.__rP=this.__ks=this.__xp=null;}
});}
)();
...
(function(){var cL="backgroundColor",cK="_moz",cJ="top",cI="formatBlock",cH="insertParagraph",cG=" succeded",cF="li",cE='<span style="',cD="underline",cC="__yC",bM="FONT",bL="no-repeat",bK=' <a href="',bJ="__yr",bI="inserthyperlink failed!",bH="|top|bottom|center|left|right|right top|left top|left bottom|right bottom|",bG="__placeholder__",bF="JustifyCenter",bE="Cut",bD="outdent",cS='</a> ',cT="InsertImage",cQ="__yI",cR="__resetHtml",cO="FontName",cP="bold",cM="blockquote",cN="If an array is provided for parameter 'position' it has to define two elements!",cU="style",cV="The method '",cl="qx.bom.htmlarea.manager.Command",ck="__yH",cn="'><br class='webkit-block-placeholder' />",cm="a",cp='"',co="type",cr="CreateLink",cq="':'",cj="italic",ci="' you calling to execute the command '",p="__yG",q="qx_link",r="none",s="' is not allowed for parameter 'position'. Possible values are '",t='"></span>',u="Italic",v="fontStyle",w=" failed",x="InsertUnorderedList",y="OL",dk='text-align:',dj="Bold",di="ForeColor",dh="__yN",dp="url(",dn='" ',dm="repeat repeat-x repeat-y no-repeat",dl='</p>',dr="<hr />",dq="UL",bd="__yF",be="editor not ready! '",bb='<font style="',bc="Command ",bh="IMG",bi='<p id="',bf="StrikeThrough",bg="__yM",Y="LI",ba="__getHtml",L='>',K='font-family:',N="' is not allowed for parameter 'repeat'. Possible values are '",M='" size="',H="Indent",G="HiliteColor",J="__yE",I="br",F="fontWeight",E="Copy",bn="</span>",bo="Paste",bp="__yu",bq="RemoveFormat",bj="JustifyRight",bk="__yL",bl='',bm='style="',br="' is not available!",bs="_moz_dirty",V="__yk",U="color",T="JustifyLeft",S=" is currently not supported!",R="Underline",Q="SelectAll",P="__yt",O="<p style='",X="line-through",W=")",bt="href",bu="__yK",bv='<span id="',bw="<span></span>",bx="JustifyFull",by="__yD",bz="Outdent",bA="indent",bB="selectall",bC="InsertOrderedList",bQ="/>",bP="<img ",bO="backgroundRepeat",bN="</font>",bU="backgroundPosition",bT='|',bS="font-size",bR="string",bW="background-color",bV="font",ce=":",cf="__yp",cc="='",cd=" with value ",ca=" ",cb=";",bX="body",bY="px",cg="backgroundImage",ch="Control",cv="__paragraph__",cu=':',cx="The value '",cw="qx.bom.htmlarea.HtmlArea.debug",cz="size",cy="' ",cB="webkit",cA="execCommand ",ct="textDecoration",cs="BODY",dd="undefined",de="transparent",df="id",dg="InsertHtml",cY='">',da="font-family",db="gecko",dc="text-align",cW='text-decoration',cX="BackColor",o="FontSize",n="'",m="child",h=';',g="p",f="span",d="__yo",c='color',b="text-decoration",a="inserthtml",B="qx.debug",C="mshtml",z="legacy-font-size",A="",D="engine.name";qx.Class.define(cl,{extend:qx.core.Object,construct:function(ds){qx.core.Object.call(this);this.__wr=ds;this.__kr=null;this._commands=null;this.__wT();this.__ya={"Bold":true,"Italic":true,"Underline":true,"StrikeThrough":true};this.__yb=[10,12,16,18,24,32,48];this.__yc=0;}
,statics:{__yd:bH,__ye:dm},members:{__kr:null,__wr:null,__yf:false,__ya:null,__yb:null,__yc:null,setContentDocument:function(dt){this.__kr=dt;}
,getCommandObject:function(du){if(this._commands[du]){return this._commands[du];}
else {return null;}
;}
,__wT:function(){this._commands={bold:{useBuiltin:false,identifier:dj,method:bu},italic:{useBuiltin:false,identifier:u,method:bk},underline:{useBuiltin:false,identifier:R,method:bg},strikethrough:{useBuiltin:false,identifier:bf,method:dh},fontfamily:{useBuiltin:true,identifier:cO,method:null},fontsize:{useBuiltin:false,identifier:o,method:cC},textcolor:{useBuiltin:true,identifier:di,method:null},textbackgroundcolor:{useBuiltin:false,identifier:null,method:by},backgroundcolor:{useBuiltin:false,identifier:null,method:J},backgroundimage:{useBuiltin:false,identifier:null,method:bd},justifyleft:{useBuiltin:false,identifier:T,method:d},justifyright:{useBuiltin:false,identifier:bj,method:d},justifycenter:{useBuiltin:false,identifier:bF,method:d},justifyfull:{useBuiltin:false,identifier:bx,method:d},indent:{useBuiltin:true,identifier:H,method:null},outdent:{useBuiltin:true,identifier:bz,method:null},copy:{useBuiltin:true,identifier:E,method:null},cut:{useBuiltin:true,identifier:bE,method:null},paste:{useBuiltin:true,identifier:bo,method:null},insertorderedlist:{useBuiltin:false,identifier:bC,method:cf},insertunorderedlist:{useBuiltin:false,identifier:x,method:cf},inserthorizontalrule:{useBuiltin:false,identifier:dg,method:bp},insertimage:{useBuiltin:false,identifier:cT,method:bJ},inserthyperlink:{useBuiltin:false,identifier:cr,method:P},selectall:{useBuiltin:false,identifier:Q,method:p},selectedtext:{useBuiltin:false,identifier:null,method:ck},selectedhtml:{useBuiltin:false,identifier:null,method:cQ},inserthtml:{useBuiltin:false,identifier:dg,method:V},resethtml:{useBuiltin:false,identifier:null,method:cR},gethtml:{useBuiltin:false,identifier:null,method:ba},removeformat:{useBuiltin:true,identifier:bq,method:null}};}
,execute:function(dv,dw){if(!this.__wr.isReady()){this.error(be+dv+cq+dw+n);return false;}
;dv=dv.toLowerCase();dw=dw!=null?dw:null;if(this._commands[dv]){var dx;var dy=this._commands[dv];if(!((qx.core.Environment.get(D)==cB)&&(dv==bA||dv==bD))){if(this.__yg()){this.__yh();}
;}
;if(dy.useBuiltin){dx=this.__yi(dy.identifier,false,dw);}
else {if(dy.method!=null&&this[dy.method]){dx=this[dy.method].call(this,dw,dy);}
else {this.error(cV+dy.method+ci+dv+br);}
;}
;this.__wr.resetSavedRange();return dx;}
else {this.error(bc+dv+S);}
;}
,__yg:function(){var Node=qx.dom.Node;var dz=this.__wr.getFocusNode();var dC=false;var dA=false;if(dz){if(Node.isText(dz)){var dB=qx.dom.Hierarchy.getAncestors(dz);for(var i=0,j=dB.length;i<j;i++){if(Node.isNodeName(dB[i],g)||qx.bom.htmlarea.HtmlArea.isHeadlineNode(dB[i])){dC=true;break;}
;}
;}
else if(Node.isNodeName(dz,bX)){dA=true;}
;}
;return dA||!dC;}
,__yh:function(){this.__yi(cI,false,g);}
,__yi:function(dD,dE,dF){try{var dI=this.__kr;var dG=false;var dH=this.__wr.getRange();this.__kr.body.focus();if((qx.core.Environment.get(D)==C)){if(dD!=bB){dH.select();if(((dH.text)&&(dH.text.length>0))||((dH.length==1)&&(dH.item(0))&&(dH.item(0).tagName==bh))){dI=dH;}
else {dI=this.__kr;}
;}
;if(((qx.core.Environment.get(D)==C))&&(this.__ya[dD])){if(dH.text==A){dG=true;}
;}
;}
;var dJ=dI.execCommand(dD,dE,dF);if(dG&&dH.text!=A){dH.collapse();}
;if((qx.core.Environment.get(B))&&qx.core.Environment.get(cw)){this.debug(cA+dD+cd+dF+cG);}
;this.__yf=false;}
catch(dK){if((qx.core.Environment.get(B))&&qx.core.Environment.get(cw)){this.debug(cA+dD+cd+dF+w);}
;return false;}
;return dJ;}
,__yj:qx.core.Environment.select(D,{"mshtml":function(){var dM=this.__wr;var dL=dM.getSavedRange()!=null?dM.getSavedRange():dM.getRange();return dL;}
,"default":function(){return this.__wr.getRange();}
}),__yk:qx.core.Environment.select(D,{"mshtml":function(dN,dO){var dR;if(dN==qx.bom.htmlarea.HtmlArea.simpleLinebreak){return this.__yn();}
else {this.__kr.body.focus();var dQ=this.__wr.getSelection();var dP=this.__yj();if(dP&&dQ&&dQ.type!=ch){try{dP.pasteHTML(dN);dP.collapse(false);dP.select();dR=true;}
catch(e){}
;}
else {dR=false;}
;this.__wr.resetSavedRange();return dR;}
;}
,"default":function(dS,dT){this.__kr.body.focus();return this.__kr.execCommand(dT.identifier,false,dS);}
}),insertParagraphOnLinebreak:qx.core.Environment.select(D,{"gecko":function(){var ec=this.__yx();var eg=this.__yl(ec);var dX=this.__yv(ec);var ed=bG+Date.parse(new Date());var ee=cv+Date.parse(new Date());var dV=bl;var ea=bv+ed+t;dV+=ea;dV+=bi+ee+dn+eg+L;dV+=dX+dl;this.__wr.getCommandManager().addUndoStep(a,cH,this.getCommandObject(a));this.execute(a,dV);this.__ym();qx.bom.element.Attribute.reset(this.__kr.getElementById(ed),df);var eb=this.__kr.getElementById(ee);if(eb.previousSibling.innerHTML==ea){var dU=this.__yw();var dY=this.__kr.createElement(I);var dW=this.__kr.createAttribute(bs);dW.nodeValue=A;dY.setAttributeNode(dW);var ef=this.__kr.createAttribute(co);ef.nodeValue=cK;dY.setAttributeNode(ef);eb.previousSibling.appendChild(dU);eb.previousSibling.appendChild(dY);}
;qx.bom.element.Attribute.reset(eb,df);return true;}
,"webkit":function(){var eh=this.getCurrentStyles();var ei=A;var ek={"background-color":true,"text-align":true};for(var ej in eh){if(ek[ej]){ei+=ej+ce+eh[ej]+cb;}
;}
;this.__wr.insertHtml(O+ei+cn);}
,"default":qx.lang.Function.empty}),__yl:qx.core.Environment.select(D,{"gecko":function(el){var eq=bm;var ep=el.child;if(ep[dc]){eq+=dk+ep[dc]+h;delete el.child[dc];}
;if(ep[da]){eq+=K+ep[da]+h;delete el.child[da];}
...
}),__ym:qx.core.Environment.select(D,{"gecko":function(){var ez=this.__wr.getSelection();if(!ez||!ez.focusNode){return;}
;var er=ez.focusNode;var ew=ez.focusNode;while(!qx.dom.Node.isNodeName(ew,g)){ew=ew.parentNode;}
;var ev=ew.previousSibling.id;var eA=ew.nextSibling?ew.nextSibling.id:null;if(qx.lang.String.startsWith(ev,cv)&&ev==eA){var ey=ew.nextSibling;var et=this.__wr.getRange();et.selectNode(ey);ez.addRange(et);var es=qx.bom.htmlarea.HtmlArea.EMPTY_DIV;this.__wr.getCommandManager().addUndoStep(a,es,this.getCommandObject(a));this.execute(a,es);var eu=this.__wr.getRange();if(er){while(er&&er.firstChild&&qx.dom.Node.isElement(er.firstChild)){er=er.firstChild;}
;}
;eu.selectNode(er);ez.addRange(eu);eu.collapse(true);}
;}
,"default":qx.lang.Function.empty}),__yn:qx.core.Environment.select(D,{"mshtml":function(){var eB=this.__wr.getRange();if(eB&&!qx.dom.Node.isNodeName(eB.parentElement(),cF)){eB.pasteHTML(qx.bom.htmlarea.HtmlArea.simpleLinebreak);eB.collapse(false);eB.select();return true;}
;return false;}
,"default":function(){return false;}
}),__yo:function(eC,eD){var eE=(qx.core.Environment.get(D)==C)?this.__wr.getRange():this.__kr;return eE.execCommand(eD.identifier,false,eC);}
,__yp:function(eF,eG){if((qx.core.Environment.get(D)==C)){var eH=this.__wr.getFocusNode();this.__yq(eH);}
;this.__kr.body.focus();var eI=this.__kr.execCommand(eG.identifier,false,eF);if((qx.core.Environment.get(D)==cB)){var eH=this.__wr.getFocusNode();this.__yq(eH.parentNode);}
;return eI;}
,__yq:function(eJ){var eK=[];var parent=eJ.parentNode;while(qx.dom.Node.isNodeName(parent,cM)){eK.push(parent);parent=parent.parentNode;}
;if(eK.length>0){parent.appendChild(eJ);parent.removeChild(eK[eK.length-1]);}
;}
,__yr:qx.core.Environment.select(D,{"gecko":function(eL,eM){if(eL.src){this.__kr.execCommand(eM.identifier,false,eL.src);delete eL.src;var fa=this.__wr.getSelection();if(fa){var eY=fa.anchorNode;var eT=fa.anchorOffset;var eQ=eY.childNodes[eT-1];var eW;for(var eS in eL){eW=this.__kr.createAttribute(eS);eW.nodeValue=eL[eS];eQ.setAttributeNode(eW);}
...
;var eP=this.__kr.createDocumentFragment();var fb;if(eV&&eO!=null){var eR=this.__ys(eO);eP.appendChild(eR.root);fb=eR.inline;}
else {fb=this.__kr.createElement(f);eP.appendChild(fb);}
;var eX=this.__kr.createTextNode(A);fb.appendChild(eX);var eU=eQ.parentNode;if(eQ==eU.lastChild){eU.appendChild(eP);}
else {eU.insertBefore(eP,eQ.nextSibling);}
;var eN=this.__wr.getRange();eN.selectNodeContents(fb);}
;return true;}
else {return false;}
;}
,"mshtml":function(fc,fd){var fh=false;var fi=bP;for(var fe in fc){fi+=fe+cc+fc[fe]+cy;}
;fi+=bQ;var ff=this.__wr.getSelection();var fg=this.__yj();if(ff&&ff.type!=ch){try{fg.select();fg.pasteHTML(fi);fh=true;}
catch(e){}
;}
;this.__wr.resetSavedRange();return fh;}
,"default":function(fj,fk){if(fj.src){var fm=bP;for(var fl in fj){fm+=fl+cc+fj[fl]+cy;}
;fm+=bQ;return this.__kr.execCommand(dg,false,fm);}
else {return false;}
;}
}),__ys:function(fn){while(fn.firstChild&&fn.firstChild.nodeType==1){fn=fn.firstChild;}
;var fr=this.__yx(fn);var fv,fp,ft;var fo=A;var parent=null;var fu=null;var fs=fr.child;while(fs){fu=this.__kr.createElement(fs[z]?bV:f);fp=this.__kr.createAttribute(cU);fu.setAttributeNode(fp);for(var fq in fs){if(fq!=m&&fq!=z){fo+=fq+ce+fs[fq]+cb;}
else if(fq==z){ft=this.__kr.createAttribute(cz);ft.nodeValue=fs[fq];fu.setAttributeNode(ft);}
;}
;fp.nodeValue=fo;if(parent!=null){parent.appendChild(fu);}
else {fv=fu;}
;parent=fu;fs=fs.child;fo=A;}
;return {root:fv,inline:fu};}
,__yt:qx.core.Environment.select(D,{"gecko|opera":function(fw,fx){var fD=this.__wr.getSelection();var fA=this.__wr.getRange();if(fD.isCollapsed){var fz=q+(++this.__yc);var fE=this.__kr.createElement(cm);var fB=this.__kr.createAttribute(bt);var fy=this.__kr.createAttribute(df);var fC=this.__kr.createTextNode(fw);fy.nodeValue=fz;fE.setAttributeNode(fy);fB.nodeValue=fw;fE.setAttributeNode(fB);fE.appendChild(fC);fA.insertNode(fE);fA.selectNode(fE);fD.collapseToEnd();return true;}
else {return this.__kr.execCommand(fx.identifier,false,fw);}
;}
,"mshtml":function(fF,fG){try{var fI;var fH=this.__yj();var fJ=this.__wr;var fH=fJ.getSavedRange()!=null?fJ.getSavedRange():fJ.getRange();if(fH!=null&&fH.text!=A){fI=fH.execCommand(fG.identifier,false,fF);}
else {fI=this.__yk(bK+fF+cY+fF+cS,fG);}
;this.__wr.resetSavedRange();return fI;}
catch(e){if(qx.core.Environment.get(B)){this.error(bI);}
;return false;}
;}
,"default":function(fK,fL){return this.__kr.execCommand(fL.identifier,false,fK);}
}),__yu:function(fM,fN){var fO=dr;if((qx.core.Environment.get(D)==db)){fO+=this.__yv();}
;return this.__yk(fO,fN);}
,__yv:function(fP){var fT=A;var fS=cE;var fR=[];var fW=typeof fP!==dd?fP:this.__yx();var fU=fW.child;var fV=false;if(qx.lang.Object.isEmpty(fU)){return A;}
;while(fU){fV=fU[z]!=null;fT+=fV?bb:fS;for(var fQ in fU){fT+=(fQ!=m&&fQ!=z)?fQ+cu+fU[fQ]+h:A;}
;fT+=fV?M+fU[z]+cY:cY;fR.unshift(fV?bN:bn);fU=fU.child;}
;if(fR.length==1&&fR[0]==bN){fT+=bw;}
;for(var i=0,j=fR.length;i<j;i++){fT+=fR[i];}
;return fT;}
,__yw:function(){var fY=this.__kr.createDocumentFragment();var ge=this.__yx();var parent=fY;var gc=ge.child;var ga;var gd=false;while(gc){gd=gc[z]!=null;ga=this.__kr.createElement(gd?bV:f);parent.appendChild(ga);for(var fX in gc){if(fX!=m&&fX!=z){qx.bom.element.Style.set(ga,fX,gc[fX]);}
;}
;if(gd){var gb=this.__kr.createAttribute(cz);gb.nodeValue=gc[z];ga.setAttributeNode(gb);}
;parent=ga;gc=gc.child;}
;return fY;}
,__yx:function(gf){var gh={};var gi=null;var gj=this.getCurrentStyles(gf);gi=gh.child={};for(var gg in gj){if(gg!=b){gi[gg]=gj[gg];}
;}
;if(gj[b]){var gk=gj[b];for(var i=0,j=gk.length;i<j;i++){if(gi==null){gi=gh.child={};}
else {gi=gi.child={};}
;gi[c]=gk[i][c];gi[cW]=gk[i][cW];}
;}
;if(gj[c]&&gj[cW]){gi=gi.child={};gi[c]=gj[c];}
;return gh;}
,getCurrentStyles:function(gl){if(gl==null){var gn=this.__wr.getSelection();if(!gn||gn.focusNode==null){return {};}
;gl=(gn.focusNode.nodeType==3)?gn.focusNode.parentNode:gn.focusNode;}
;var go=qx.dom.Hierarchy.getAncestors(gl);var gq=qx.lang.Array.insertBefore(go,gl,go[0]);var gm=this.__yy(gq);var gp=this.__yz(gm,gq);return gp;}
,__yy:function(gr){var gu={};var gs,gt;for(var i=0,j=gr.length;i<j;i++){gt=gr[i];for(var k=0,l=gt.style.length;k<l;k++){gs=gt.style[k];if(gs.length>0&&typeof gu[gs]===dd){gu[gs]=gt.style.getPropertyValue(gs);}
;}
;if(gt.tagName.toUpperCase()==bM&&gt.size&&gu[z]===undefined){gu[z]=gt.size;}
;}
;if(gu[z]&&gu[bS]){delete gu[bS];}
;return gu;}
,__yz:function(gv,gw){var gB=gw[0];var gz=this.__wr.getContentWindow().getComputedStyle(gB,null);var gx;var gA={};for(var gy in gv){if(gy!=z){gx=gz.getPropertyValue(gy);}
else {gx=gv[gy];}
;if(gy==bW&&gx==de){gA[gy]=this.__yB(gw);}
else if(gy==b){gA[gy]=this.__yA(gw);}
else {gA[gy]=gx;}
;}
;return gA;}
,__yA:function(gC){var gF,gH,gD;var gG=[];var gE=this.__wr.getContentWindow();for(var i=0,j=gC.length;i<j;i++){gD=gE.getComputedStyle(gC[i],null);gF=gD.getPropertyValue(b);gH=gD.getPropertyValue(U);if(gF!=r){gG.push({'text-decoration':gF,'color':gH});}
;}
;return gG;}
,__yB:function(gI){var gK,gJ,gL;for(var i=0;i<gI.length;i++){gK=gI[i];gJ=this.__wr.getContentWindow().getComputedStyle(gK,null);gL=gJ.getPropertyValue(bW);if(gL!=de){return gL;}
;}
;}
,__yC:function(gM,gN){var ha=this.__wr.getSelection();var gU=((qx.core.Environment.get(D)==C))?this.__wr.getRange():gU=ha.getRangeAt(0);var gV=[];var gY;var gT=[y,dq];var gO,i,j,gW;gW=((qx.core.Environment.get(D)==C))?gU.parentElement():gU.commonAncestorContainer;if(gW.tagName==cs){for(var i=0;i<gT.length;i++){gO=gW.getElementsByTagName(gT[i]);for(var j=0;j<gO.length;j++){if(gO[j]){gV.push(gO[j]);}
;}
;}
;}
else if(qx.lang.Array.contains(gT,gW.tagName)){gV.push(gW);}
;if(gV.length>0){for(var i=0;i<gV.length;i++){var gQ=gV[i];gY=((qx.core.Environment.get(D)==C))?((gQ==gW)||(gW.tagName==cs)):ha.containsNode(gQ,false);for(j=0;j<gQ.childNodes.length;j++){var gS=gQ.childNodes[j];if(gY||(ha.focusNode.nodeValue&&qx.dom.Hierarchy.contains(gS,ha.focusNode)&&(ha.focusOffset==ha.focusNode.nodeValue.length))||(qx.dom.Hierarchy.contains(gS,ha.anchorNode)&&(ha.anchorOffset==0))||(ha.containsNode(gS,false))){gS.style.fontSize=(this.__yb[gM]||gM)+bY;}
;}
;}
;}
else {var gR=((qx.core.Environment.get(D)==C))?gW:ha.focusNode;var gX=qx.dom.Hierarchy.getAncestors(gR);for(i=0;i<gX.length;i++){if(gX[i].tagName==Y){if((((qx.core.Environment.get(D)==db))&&((ha.anchorOffset==0)&&(ha.focusNode.nodeValue&&(ha.focusOffset==ha.focusNode.nodeValue.length))&&qx.dom.Hierarchy.contains(gX[i].firstChild,ha.anchorNode)&&qx.dom.Hierarchy.contains(gX[i].lastChild,ha.focusNode)))||(((qx.core.Environment.get(D)==C))&&(gU.htmlText==gX[i].innerHTML))){gX[i].style.fontSize=(this.__yb[gM]||gM)+bY;}
;break;}
;}
;}
;if((qx.core.Environment.get(D)==C)){this.__kr.body.focus();this.__wr.getRange().select();return this.__kr.execCommand(o,false,gM);}
else if((qx.core.Environment.get(D)==db)){var parent=gU.commonAncestorContainer;if(parent.nodeType===1){var gP=parent.getElementsByTagName(f);for(i=0;i<gP.length;i++){if(gP[i].style.fontSize){gP[i].style.fontSize=null;}
;}
;}
;}
;return this.__kr.execCommand(o,false,gM);}
,__yD:qx.core.Environment.select(D,{"mshtml":function(hb,hc){this.__kr.body.focus();return this.__kr.execCommand(cX,false,hb);}
,"gecko|opera":function(hd,he){this.__kr.body.focus();return this.__kr.execCommand(G,false,hd);}
,"webkit":function(hf,hg){var hh=this.__wr.getSelection();var hi=this.__wr.getRange();if(!hh||!hh.isCollapsed){this.__kr.body.focus();this.__kr.execCommand(cX,false,hf);if(hh){hh.collapseToEnd();}
;return true;}
...
;}
;hi.setStart(hh.anchorNode,hh.anchorNode.nodeValue.charAt(hl)==ca?hl+1:hl);hi.setEnd(hh.anchorNode,hk);hh.addRange(hi);this.__kr.body.focus();this.__kr.execCommand(cX,false,hf);hh.collapseToEnd();return true;}
;}
}),__yE:function(hm,hn){hm=hm!=null&&typeof hm==bR?hm:de;qx.bom.element.Style.set(this.__kr.body,cL,hm);return true;}
,__yF:function(ho,hp){var hq,hs,hr;var ht=qx.bom.htmlarea.manager.Command;if(ho==null){hq=null;}
else {hq=ho[0];hs=ho[1];hr=ho[2];}
;if(hq==null||typeof hq!=bR){qx.bom.element.Style.set(this.__kr.body,cg,A);qx.bom.element.Style.set(this.__kr.body,bO,A);qx.bom.element.Style.set(this.__kr.body,bU,A);return true;}
else {if(hq.search(/^url.*\(/)==-1){hq=dp+hq+W;}
;}
;if(hs!=null&&!qx.lang.String.contains(ht.__ye,hs)){if(qx.core.Environment.get(B)){this.error(cx+hs+N+ht.__ye+n);}
;return false;}
else {hs=bL;}
;if(hr!=null){if(qx.lang.Type.isString(hr)&&!qx.lang.String.contains(ht.__yd,bT+hr+bT)){if(qx.core.Environment.get(B)){this.error(cx+hr+s+ht.__yd+n);}
;return false;}
else {if(qx.lang.Type.isArray(hr)&&hr.length==2){hr=hr[0]+ca+hr[1];}
else {if(qx.core.Environment.get(B)){this.error(cN);}
;return false;}
;}
;}
else {hr=cJ;}
;qx.bom.element.Style.set(this.__kr.body,cg,hq);qx.bom.element.Style.set(this.__kr.body,bO,hs);qx.bom.element.Style.set(this.__kr.body,bU,hr);return true;}
,__yG:qx.core.Environment.select(D,{"mshtml":function(hu,hv){var hw=this.__kr.body.createTextRange();hw.select();return true;}
,"default":function(hx,hy){return this.__yi(hy.identifier,false,hx);}
}),__yH:function(){return this.__wr.getSelectedText();}
,__yI:function(){return this.__wr.getSelectedHtml();}
,__yJ:function(hz,hA){var hE=this.__wr.getFocusNode();if(hE.textContent==A){var hC=qx.dom.Hierarchy.getAncestors(hE);hC.unshift(hE);var Node=qx.dom.Node;var hB=qx.bom.element.Style;var hD=hC.shift();while(hC.length>0){if(Node.getName(hD)==g||Node.getName(hD)==bX){break;}
;if(hB.get(hD,hz,hB.LOCAL_MODE)==hA){hB.reset(hD,hz);return true;}
;hD=hC.shift();}
;}
;return false;}
,__yK:qx.core.Environment.select(D,{"gecko":function(hF,hG){if(this.__yJ(F,cP)){return true;}
else {return this.__yi(hG.identifier,false,hF);}
;}
,"default":function(hH,hI){return this.__yi(hI.identifier,false,hH);}
}),__yL:qx.core.Environment.select(D,{"gecko":function(hJ,hK){if(this.__yJ(v,cj)){return true;}
else {return this.__yi(hK.identifier,false,hJ);}
;}
,"default":function(hL,hM){return this.__yi(hM.identifier,false,hL);}
}),__yM:qx.core.Environment.select(D,{"gecko":function(hN,hO){if(this.__yJ(ct,cD)){return true;}
else {return this.__yi(hO.identifier,false,hN);}
;}
,"default":function(hP,hQ){return this.__yi(hQ.identifier,false,hP);}
}),__yN:qx.core.Environment.select(D,{"gecko":function(hR,hS){if(this.__yJ(ct,X)){return true;}
else {return this.__yi(hS.identifier,false,hR);}
;}
,"default":function(hT,hU){return this.__yi(hU.identifier,false,hT);}
})},destruct:function(){this.__kr=this.__wr=this._commands=null;this.__ya=this.__yb=null;}
});}
)();
(function(){var n="undo",m="pageup",l="ADD TO UNDO STACK",k="control",h="shift",g="end",f="right",d="qx_link",c="b",b="insertunorderedlist",bo="__wq",bn="Undo",bm="pagedown",bl="k",bk="home",bj="Redo",bi="a",bh="insertorderedlist",bg="y",bf="justifyright",u="redo",v="insertParagraph",s="qx.bom.htmlarea.manager.UndoRedo",t="left",q="i",r="u",o="enter",p="execCommand failed! Details: ",A="ADD TO REDO STACK",B="insertimage",K="z",H="undoRedoState",S="down",N="up",bb="body",X="backgroundColor",D="actionType ",be="mousedown",bd=" is not managed! Please provide a handler method!",bc="table",C="backgroundImage",F="backgroundRepeat",G="p",J="backgroundPosition",L="td",O="img",U="inserthyperlink",ba="mouseup",w="keypress",x="qx.bom.htmlarea.HtmlArea.debug",E="backgroundimage",R="qx.debug",Q="backgroundcolor",P="webkit",W=" ",V="Content",M="Internal",T="inserthtml",a="gecko",Y="Custom",y="Command",z="mshtml",I="engine.name";qx.Class.define(s,{extend:qx.core.Object,construct:function(bp,bq){qx.core.Object.call(this);this.__wq=bp;this.__wr=bq;this.__ws=[];this.__wt=[];this._commands=null;this.__kr=null;this.__wu={};this.__wv={command:true,content:true,custom:true};this.__wT();this.__ww=qx.lang.Function.bind(this._handleKeyPress,this);this.__wx=qx.lang.Function.bind(this._handleMouseUp,this);if((qx.core.Environment.get(I)==z)){this.__wy=qx.lang.Function.bind(this._handleMouseDown,this);}
;}
,members:{__wz:false,__wA:false,__wB:false,__wv:null,__wu:null,__wq:null,__kr:null,__ws:null,__wt:null,__wr:null,__ww:null,__wx:null,__wy:null,__wC:null,setContentDocument:function(br){this.__kr=br;this.__wq.setContentDocument(br);qx.event.Registration.addListener(br.body,w,this.__ww,this);qx.event.Registration.addListener(br.body,ba,this.__wx,this);if((qx.core.Environment.get(I)==z)){qx.event.Registration.addListener(br.body,be,this.__wy,this,true);}
;}
,insertParagraphOnLinebreak:function(){return this.__wq.insertParagraphOnLinebreak();}
,execute:function(bs,bt){var bu;bs=bs.toLowerCase();if(this._commands[bs]){if(this._commands[bs].passthrough){bu=this.__wq.execute(bs,bt);}
else {bu=this[bs].call(this);}
;}
else {if((qx.core.Environment.get(I)==z)||(qx.core.Environment.get(I)==P)){this.__wU(bs,bt,this.__wq.getCommandObject(bs));bu=this.__wq.execute(bs,bt);if(!bu){this.__ws.pop();}
;}
else {bu=this.__wq.execute(bs,bt);if(bu){this.__wU(bs,bt,this.__wq.getCommandObject(bs));}
;}
;if(bs==n&&this.__ws.length==0){this.__wA=false;this.__xd();}
else if(bs==u&&this.__wt.length==0){this.__wz=false;this.__xd();}
;}
;this.__wB=false;return bu;}
,addUndoStep:function(bv,bw,bx){this.__wU(bv,bw,bx);}
,registerHandler:function(by,bz,bA,bB){this.__wu[by]={undo:bz,redo:bA,context:bB};}
,isUndoPossible:function(){return this.__wA;}
,undo:function(){var bE;if(this.__wB){this.__wW();}
;if(this.__ws.length>0){var bD=this.__ws.pop();if(this.__wv[bD.actionType.toLowerCase()]){switch(bD.actionType){case y:bE=this.__wE(bD);break;case V:bE=this.__wI(bD);break;case M:bE=this.__wH(bD);break;case Y:bE=this.__wD(bD);break;};}
else if(this.__wu[bD.actionType]){var bC=this.__wu[bD.actionType];bE=bC.undo.call(bC.context?bC.context:this,bD);this.__wY(bD);}
else {this.error(D+bD.actionType+bd);}
;this.__wz=true;this.__xd();return bE;}
;}
,__wD:qx.core.Environment.select(I,{"mshtml|webkit":function(bF){var bG=this.__kr.body.innerHTML;var bI=bF.content;this.__kr.body.innerHTML=bI;var bH=bF;bH.content=bG;this.__wY(bH);return true;}
,"default":function(bJ){var bN=bJ;var bM=qx.bom.element.Style;switch(bJ.command){case Q:bN.parameter=[bM.get(this.__kr.body,X)];break;case E:bN.parameter=[bM.get(this.__kr.body,C),bM.get(this.__kr.body,F),bM.get(this.__kr.body,J)];break;};this.__wY(bN);if(bJ.command==U){if((qx.core.Environment.get(I)==a)){var bL=d+this.__wq.__hyperLinkId;var bK=this.__kr.getElementById(bL);if(bK){bK.parentNode.removeChild(bK);return true;}
else {return false;}
;}
;}
else {return this.__wq.execute(bJ.command,bJ.value);}
;}
}),__wE:qx.core.Environment.select(I,{"mshtml|webkit":function(bO){}
,"default":function(bP){this.__wY(bP);if((qx.core.Environment.get(I)==a)){if(bP.command==T&&bP.value==qx.bom.htmlarea.HtmlArea.EMPTY_DIV&&this.__wF(T,v)){this.__wG();}
;}
;return this.__wJ();}
}),__wF:function(bQ,bR){if(this.__ws.length>0){var bS=this.__ws[this.__ws.length-1];return (bS.command==bQ&&bS.value==bR);}
;return false;}
,__wG:function(){this.__wJ();if(this.__ws.length>0){var bT=this.__ws.pop();this.__wY(bT);}
;}
,__wH:function(bU){this.__wY(bU);return this.__wJ();}
,__wI:qx.core.Environment.select(I,{"gecko":function(bV){this.__wY(bV);try{return this.__wJ();}
catch(bW){if(qx.core.Environment.get(R)){this.error(p+bW);}
;}
;}
,"mshtml|webkit":function(bX){}
,"default":function(bY){this.__wY(bY);return this.__wJ();}
}),__wJ:function(){try{return this.__kr.execCommand(bn,false,null);}
catch(e){return false;}
;}
,isRedoPossible:function(){return this.__wz;}
,redo:function(){if(this.__wz){var cb;if(this.__wt.length>0){var cc=this.__wt.pop();if(this.__wv[cc.actionType.toLowerCase()]){switch(cc.actionType){case y:cb=this.__wL(cc);break;case V:cb=this.__wR(cc);break;case M:cb=this.__wQ(cc);break;case Y:cb=this.__wK(cc);break;};}
else if(this.__wu[cc.actionType]){var ca=this.__wu[cc.actionType];cb=ca.redo.call(ca.context?ca.context:this,cc);this.__wX(cc);}
else {this.error(D+cc.actionType+bd);}
;this.__wA=true;this.__xd();}
;return cb;}
;}
,__wK:qx.core.Environment.select(I,{"mshtml|webkit":function(cd){var ce=this.__kr.body.innerHTML;var cg=cd.content;this.__kr.body.innerHTML=cg;var cf=cd;cf.content=ce;this.__wX(cf);return true;}
,"default":function(ch){this.__wX(ch);return this.__wS();}
}),__wL:qx.core.Environment.select(I,{"mshtml|webkit":function(ci){}
,"default":function(cj){this.__wX(cj);var ck=this.__wS();if((qx.core.Environment.get(I)==a)){if(this.__wM(T,qx.bom.htmlarea.HtmlArea.EMPTY_DIV)){var cl=this.__wN();this.__wO();if(cl!=null){this.__wP(cl);}
;}
;}
;return ck;}
}),__wM:function(cm,cn){if(this.__wt.length>0){var co=this.__wt[this.__wt.length-1];return (co.command==cm&&co.value==cn);}
;return false;}
,__wN:function(){if(this.__wr==null){return null;}
;var cp=this.__wr.getSelection();var cq=cp?cp.focusNode:null;if(cq==null){return null;}
...
;}
,__wO:function(){var cs=this.__wt.pop();this.__wX(cs);this.__wS();}
,__wP:qx.core.Environment.select(I,{"gecko":function(ct){if(ct==this.__wr.getContentBody().lastChild){return;}
;var cw=ct.firstChild;while(cw.firstChild){cw=cw.firstChild;}
;var cu=this.__wr.getSelection();var cv=this.__wr.getRange();if(cu&&cv){cv.selectNode(cw);cu.addRange(cv);cv.collapse(true);}
;}
,"default":qx.lang.Function.empty}),__wQ:function(cx){this.__wX(cx);return this.__wS();}
,__wR:qx.core.Environment.select(I,{"mshtml|webkit":function(cy){}
,"default":function(cz){this.__wX(cz);return this.__wS();}
}),__wS:function(){try{return this.__kr.execCommand(bj,false,null);}
catch(e){return false;}
;}
,__wT:function(){this._commands={undo:{passthrough:false},redo:{passthrough:false}};this.__wq.getCommandObject(Q).customUndo=true;this.__wq.getCommandObject(E).customUndo=true;if((qx.core.Environment.get(I)==a)){}
;}
,__wU:qx.core.Environment.select(I,{"mshtml|webkit":function(cA,cB,cC){var cD=this.getUndoRedoObject();cD.commandObject=cC;cD.command=cA;cD.value=cB;cD.actionType=Y;cD.content=this.__kr.body.innerHTML;this.__wV(cD);}
,"default":function(cE,cF,cG){if(this.__wr==null){return;}
;var cI=this.getUndoRedoObject();cI.commandObject=cG;cI.command=cE;cI.value=cF;cI.actionType=Y;var cH=this.__wr.getSelection();if(cG.customUndo){var cJ=[];switch(cE){case Q:cJ.push(qx.bom.element.Style.get(this.__kr.body,X));break;case E:cJ.push(qx.bom.element.Style.get(this.__kr.body,C),qx.bom.element.Style.get(this.__kr.body,F),qx.bom.element.Style.get(this.__kr.body,J));break;case U:if(cH&&!cH.isCollapsed){cI.actionType=y;}
...
;}
;this.__wV(cI);}
}),__wV:function(cK){if(this.__wB){this.__wW();}
;this.__wX(cK);this.__wz=false;this.__wt=[];this.__xd();}
,__wW:function(){var cL=this.__ws[this.__ws.length-1];if(cL==null||cL.actionType!=V){var cM=this.getUndoRedoObject();cM.actionType=V;if((qx.core.Environment.get(I)==z)||(qx.core.Environment.get(I)==P)){cM.content=this.__wC;cM.actionType=Y;this.__wC=null;}
;this.__wX(cM);this.__wB=false;}
;}
...
,__wX:function(cN){if((qx.core.Environment.get(R))&&qx.core.Environment.get(x)){this.debug(l);this.debug(cN.actionType+W+cN.command+W+cN.value);}
;this.__ws.push(cN);}
,__wY:function(cO){if((qx.core.Environment.get(R))&&qx.core.Environment.get(x)){this.debug(A);this.debug(cO.actionType+W+cO.command+W+cO.value);}
;this.__wt.push(cO);}
,_handleKeyPress:function(e){var cP=e.getKeyIdentifier().toLowerCase();var cQ=e.isCtrlPressed();switch(cP){case k:case h:case t:case f:case N:case S:case m:case bm:case bk:case g:case o:break;case bi:case c:case q:case r:case bl:case bg:case K:if(!cQ){this.__xa();}
;break;default:this.__wz=false;this.__wt=[];this.__xa();};}
,__xa:function(){if(!this.__wB){this.__wB=true;this.__wA=true;if((qx.core.Environment.get(I)==z)||(qx.core.Environment.get(I)==P)){this.__wC=this.__kr.body.innerHTML;}
;this.__xd();}
;}
,__xb:null,_handleMouseDown:qx.core.Environment.select(I,{"mshtml":function(e){var cR=e.getOriginalTarget();if(qx.dom.Node.isElement(cR)&&(qx.dom.Node.isNodeName(cR,O)||qx.dom.Node.isNodeName(cR,bc))){this.__xb={node:cR,content:cR.outerHTML};}
else {this.__xb=null;}
;}
,"default":function(e){return true;}
}),_handleMouseUp:qx.core.Environment.select(I,{"gecko":function(e){if(this.__wr==null){return;}
;var cT=this.__wr.getSelection();if(!cT){this.__xb=null;return;}
;var cS=cT.anchorNode;var cU=cS.childNodes[cT.anchorOffset];if(qx.dom.Node.isNodeName(cU,O)){if(this.__xb==null){this.__xb=cU.cloneNode(true);}
else {if(this.__xb.style.width!=cU.style.width||this.__xb.style.height!=cU.style.height){this.__xc();this.__xb=cU.cloneNode(true);}
;}
;}
...
;if(this.__xb==null){this.__xb=cV.cloneNode(true);}
else {qx.event.Timer.once(function(){if(cV.style.width!=this.__xb.style.width||cV.style.height!=this.__xb.style.height||cV.innerHTML!=this.__xb.innerHTML){this.__xc();this.__xb=cV.cloneNode(true);}
;}
,this,0);}
;}
else {this.__xb=null;}
;}
,"default":function(e){var cW=qx.bom.Event.getTarget(e);if(this.__xb!=null){if(cW.nodeType==1){if(cW==this.__xb.node){if(cW.outerHTML!=this.__xb.content){this.__xb.content=cW.outerHTML;this.__xc();}
;}
else {for(var i=0,j=cW.childNodes.length;i<j;i++){if(cW.childNodes[i]==this.__xb.node){if(cW.childNodes[i].outerHTML!=this.__xb.content){this.__xb.content=cW.childNodes[i].outerHTML;this.__xc();}
;}
;}
;}
;}
...
;}
;}
}),__xc:qx.core.Environment.select(I,{"mshtml|webkit":function(){this.__wU(M,null,null);}
,"default":function(){var cX=this.getUndoRedoObject();cX.actionType=M;this.__wX(cX);}
}),__xd:function(){qx.event.Timer.once(function(e){if(this.__wr!=null){var cY={undo:this.isUndoPossible()?0:-1,redo:this.isRedoPossible()?0:-1};this.__wr.fireDataEvent(H,cY);}
;}
,this,200);}
},destruct:function(){try{qx.event.Registration.removeListener(this.__kr.body,w,this.__ww);qx.event.Registration.removeListener(this.__kr,ba,this.__wx);if((qx.core.Environment.get(I)==z)){qx.event.Registration.removeListener(this.__kr,be,this.__wy);}
;}
catch(e){}
;this._disposeObjects(bo);this.__wr=this.__ws=this.__wt=this._commands=this.__kr=null;this.__wv=this.__wu=null;}
});}
)();
