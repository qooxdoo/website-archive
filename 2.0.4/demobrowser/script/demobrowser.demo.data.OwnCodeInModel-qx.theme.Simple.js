...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.OwnCodeInModel","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.OwnCodeInModel-qx.theme.Simple.5e88c1e7c1bf.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;if(this.getSelection()==null){this.__Bx=new qx.data.Array();this.setSelection(this.__Bx);}
;}
,properties:{selection:{check:f,event:a,apply:d,init:null}},members:{_modifingSelection:0,__By:null,__Bz:null,__Bx:null,_applySelection:function(h,j){if(this.__Bz!=undefined&&j!=undefined){j.removeListenerById(this.__Bz);}
;this.__Bz=h.addListener(g,this.__BA,this);this._updateSelection();}
,__BA:function(){this._updateSelection();}
,_changeTargetSelection:function(){if(this.getTarget()==null){return;}
;if(!this.__BB()&&!this.__BC()){return;}
;if(this._inSelectionModification()){return;}
;var l=this.getTarget().getSelection();var k=this.getSelection();if(k==null){k=new qx.data.Array();this.__Bx=k;this.setSelection(k);}
...
,_addChangeTargetListener:function(n,o){if(this.__By!=undefined&&o!=undefined){o.removeListenerById(this.__By);}
;if(n!=null){if(this.__BB()||this.__BC()){this.__By=n.addListener(a,this._changeTargetSelection,this);}
;}
;}
,_updateSelection:function(){if(!this.getTarget()){return;}
;this._startSelectionModification();if(this.__BB()){var r=[];for(var i=0;i<this.getSelection().length;i++){var s=this.getSelection().getItem(i);var t=this.__BD(s);if(t!=null){r.push(t);}
;}
...
;}
;}
else if(this.__BC()){var q=this.getSelection().getItem(this.getSelection().length-1);if(q!==undefined){this.__rG(q);this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();}
else {this.getTarget().resetSelection();}
;}
;this._endSelectionModification();}
,__BB:function(){var u=this.getTarget().constructor;return qx.Class.implementsInterface(u,qx.ui.core.IMultiSelection);}
,__BC:function(){var v=this.getTarget().constructor;return qx.Class.implementsInterface(v,qx.ui.core.ISingleSelection);}
,__rG:function(w){var x=this.__BD(w);if(x==null){return;}
;if(this.__BB()){this.getTarget().addToSelection(x);}
else if(this.__BC()){this.getTarget().setSelection([x]);}
;}
,__BD:function(y){var z=this.getTarget().getSelectables(true);for(var i=0;i<z.length;i++){if(z[i].getModel()==y){return z[i];}
;}
;return null;}
...
},destruct:function(){if(this.__Bx){this.__Bx.dispose();}
;}
});}
)();
...
(function(){var m="changeModel",k="_applyLabelOptions",h="_applyLabelPath",g="changeTarget",f="_applyModel",e="icon",d="qx.data.controller.List",c="_applyIconPath",b="_applyDelegate",a="changeDelegate",z="_applyTarget",y="qx.data.IListData",x="model",w="label",v="_applyIconOptions",u="change",t="ReverseBindingId",s="BindingId",r="]",q="model[",o="String",p=".",n="";qx.Class.define(d,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(A,B,C){qx.core.Object.call(this);this.__zD=[];this.__Bj=[];this.__Bk=[];this.__Bl={};if(C!=null){this.setLabelPath(C);}
;if(A!=null){this.setModel(A);}
;if(B!=null){this.setTarget(B);}
;}
,properties:{model:{check:y,apply:f,event:m,nullable:true,dereference:true},target:{apply:z,event:g,nullable:true,init:null,dereference:true},labelPath:{check:o,apply:h,nullable:true},iconPath:{check:o,apply:c,nullable:true},labelOptions:{apply:k,nullable:true},iconOptions:{apply:v,nullable:true},delegate:{apply:b,event:a,init:null,nullable:true}},members:{__Bm:null,__zD:null,__Bl:null,__Bj:null,__Bk:null,__Bn:null,__Bo:null,update:function(){this.__Bq();this.__Bu();this._updateSelection();}
,_applyDelegate:function(D,E){this._setConfigureItem(D,E);this._setFilter(D,E);this._setCreateItem(D,E);this._setBindItem(D,E);}
,_applyIconOptions:function(F,G){this.__Bu();}
,_applyLabelOptions:function(H,I){this.__Bu();}
,_applyIconPath:function(J,K){this.__Bu();}
,_applyLabelPath:function(L,M){this.__Bu();}
,_applyModel:function(N,O){if(O!=undefined){if(this.__Bm!=undefined){O.removeListenerById(this.__Bm);}
;}
;if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();}
;if(N!=null){this.__Bm=N.addListener(u,this.__Bp,this);this.__zK();this.__Bq();if(O==null){this._changeTargetSelection();}
else {this.__Bn=true;qx.ui.core.queue.Widget.add(this);}
;}
else {var P=this.getTarget();if(P!=null){var length=P.getChildren().length;for(var i=0;i<length;i++){this.__Bt();}
;}
;}
;}
...
;if(Q!=null){if(this.getModel()!=null){for(var i=0;i<this.__zD.length;i++){this.__Bs(this.__Bw(i));}
;}
;}
;}
,__Bp:function(){this.__Bo=true;qx.ui.core.queue.Widget.add(this);if(this.__zD.length!=this.getModel().getLength()){this.update();}
;}
,syncWidget:function(){if(this.__Bn){this._changeTargetSelection();}
;if(this.__Bo){this._updateSelection();}
;this.__Bo=this.__Bn=null;}
,__Bq:function(){if(this.getTarget()==null){return;}
;this.__zK();var U=this.__zD.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++){this.__Bs(this.__Bw(j));}
;}
else if(U<T){for(var j=T;j>U;j--){this.__Bt();}
;}
;this.__Bn=true;qx.ui.core.queue.Widget.add(this);}
,__Br:function(){var V=this.getModel();if(V!=null){V.removeListenerById(this.__Bm);this.__Bm=V.addListener(u,this.__Bp,this);}
;}
...
;return W;}
,__Bs:function(Y){var ba=this._createItem();this._bindListItem(ba,Y);this.getTarget().add(ba);}
,__Bt:function(){this._startSelectionModification();var bc=this.getTarget().getChildren();var bb=bc.length-1;var bd=bc[bb];this._removeBindingsFrom(bd);this.getTarget().removeAt(bb);bd.destroy();this._endSelectionModification();}
...
;}
,bindProperty:function(bm,bn,bo,bp,bq){if(bo!=null){var bo=qx.lang.Object.clone(bo);this.__Bl[bn]=bo.onUpdate;delete bo.onUpdate;}
else {bo={};this.__Bl[bn]=null;}
;bo.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,bq);var br=q+bq+r;if(bm!=null&&bm!=n){br+=p+bm;}
;var bs=this.bind(br,bp,bn,bo);bp.setUserData(bn+s,bs);if(!qx.lang.Array.contains(this.__Bj,bn)){this.__Bj.push(bn);}
;}
,bindPropertyReverse:function(bt,bu,bv,bw,bx){var by=q+bx+r;if(bt!=null&&bt!=n){by+=p+bt;}
;var bz=bw.bind(bu,this,by,bv);bw.setUserData(bt+t,bz);if(!qx.lang.Array.contains(this.__Bk,bt)){this.__Bk.push(bt);}
;}
,_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;}
;for(var i=0;i<this.__Bj.length;i++){if(this.__Bl[this.__Bj[i]]!=null){this.__Bl[this.__Bj[i]]();}
;}
;}
,_removeBindingsFrom:function(bD){for(var i=0;i<this.__Bj.length;i++){var bE=bD.getUserData(this.__Bj[i]+s);if(bE!=null){this.removeBinding(bE);}
;}
;for(var i=0;i<this.__Bk.length;i++){var bE=bD.getUserData(this.__Bk[i]+t);if(bE!=null){bD.removeBinding(bE);}
;}
;}
,__Bu:function(){if(this.getTarget()==null||this.getModel()==null){return;}
;var bF=this.getTarget().getChildren();for(var i=0;i<bF.length;i++){this._removeBindingsFrom(bF[i]);this._bindListItem(bF[i],this.__Bw(i));}
;this.__Br();}
...
,_setFilter:function(bP,bQ){if((bP==null||bP.filter==null)&&(bQ!=null&&bQ.filter!=null)){this.__Bv();}
...
;var bS=this.__zD;this.__zK();if(bS.length>this.__zD.length){for(var j=bS.length;j>this.__zD.length;j--){this.getTarget().removeAt(j-1).destroy();}
;}
else if(bS.length<this.__zD.length){for(var j=bS.length;j<this.__zD.length;j++){var bT=this._createItem();this.getTarget().add(bT);}
;}
;var bR=this.getTarget().getChildren();for(var i=0;i<bR.length;i++){this._bindListItem(bR[i],this.__Bw(i));}
;this.__Br();this._endSelectionModification();this._updateSelection();}
,__Bv:function(){this.__zK();this.__Bq();this.__Bu();this.__Bo=true;qx.ui.core.queue.Widget.add(this);}
,__zK:function(){var bW=this.getModel();if(bW==null){return;}
;var bX=this.getDelegate();if(bX!=null){var bV=bX.filter;}
;this.__zD=[];for(var i=0;i<bW.getLength();i++){if(bV==null||bV(bW.getItem(i))){this.__zD.push(i);}
;}
;}
,__Bw:function(bY){return this.__zD[bY];}
},destruct:function(){this.__zD=this.__Bl=this.__Bj=null;this.__Bk=null;qx.ui.core.queue.Widget.remove(this);}
});}
)();
...
(function(){var j="changeModel",i="application/json",h="json",g="loaded",f="error",e="fail",d="queued",c="changeState",b="success",a="String",x="__qN",w="changePhase",v="_marshaler",u="qx.data.store.Json",t="changeUrl",s="_applyUrl",r="aborted",q="qx.event.type.Data",p="receiving",o="sending",m="completed",n="failed",k="timeout",l="configured";qx.Class.define(u,{extend:qx.core.Object,construct:function(y,z){qx.core.Object.call(this);this._marshaler=new qx.data.marshal.Json(z);this._delegate=z;if(y!=null){this.setUrl(y);}
;}
,events:{"loaded":q,"error":q},properties:{model:{nullable:true,event:j},state:{check:[l,d,o,p,m,r,k,n],init:l,event:c},url:{check:a,apply:s,event:t,nullable:true}},members:{_marshaler:null,_delegate:null,__qN:null,_applyUrl:function(A,B){if(A!=null){A=qx.util.AliasManager.getInstance().resolve(A);A=qx.util.ResourceManager.getInstance().toUri(A);this._createRequest(A);}
;}
,_getRequest:function(){return this.__qN;}
,_setRequest:function(C){this.__qN=C;}
,_createRequest:function(D){if(this.__qN){this.__qN.dispose();this.__qN=null;}
...
;this.fireDataEvent(g,this.getModel());if(this.__qN){this.__qN.dispose();this.__qN=null;}
;}
,reload:function(){var R=this.getUrl();if(R!=null){this._createRequest(R);}
;}
},destruct:function(){if(this.__qN!=null){this._disposeObjects(x);}
