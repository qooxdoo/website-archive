...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.StackContainer-qx.theme.Indigo.c9015210c9f1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var w="resize",v=")",u="Resize to: ",t=" (",s="demobrowser.demo.widget.StackContainer",r="x",q="Selected: ",p="yellow",o="teal",n="gray",f="red",m="Next",j="Previous",c="green",b="icon/22/actions/go-next.png",h="changeSelection",g="icon/22/actions/go-previous.png",k="orange",a="main",l="blue",d="execute";qx.Class.define(s,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);this.addSimpleStack();this.addDynamicStack();}
,addSimpleStack:function(){var z=new qx.ui.container.Stack();z.setDecorator(a);z.setWidth(200);z.setHeight(120);this.getRoot().add(z,{left:20,top:20});var x=new qx.ui.container.Composite((new qx.ui.layout.HBox).set({spacing:4}));var C=new qx.ui.form.Button(j,g);var A=new qx.ui.form.Button(m,b);x.add(C);x.add(A);this.getRoot().add(x,{left:20,top:150});C.addListener(d,z.previous,z);A.addListener(d,z.next,z);var B=[f,n,l,k,o,p,c];var y;for(var i=0;i<B.length;i++){y=new qx.ui.core.Widget;y.setBackgroundColor(B[i]);z.add(y);}
;z.addListener(h,function(e){this.debug(q+e.getData()[0].getBackgroundColor());}
);}
,addDynamicStack:function(){var F=new qx.ui.container.Stack();F.setDecorator(a);F.setDynamic(true);this.getRoot().add(F,{left:250,top:20});var D=new qx.ui.container.Composite((new qx.ui.layout.HBox).set({spacing:4}));var I=new qx.ui.form.Button(j,g);var G=new qx.ui.form.Button(m,b);D.add(I);D.add(G);this.getRoot().add(D,{left:250,top:150});I.addListener(d,F.previous,F);G.addListener(d,F.next,F);var H=[f,n,l,k,o,p,c];var E;for(var i=0;i<H.length;i++){E=new qx.ui.core.Widget;E.setBackgroundColor(H[i]);E.setWidth((Math.round(i/2)+1)*50);E.setHeight((Math.round(i/3)+1)*40);F.add(E);}
;F.addListener(h,function(e){var J=e.getData()[0];this.debug(q+J.getBackgroundColor()+t+J.getWidth()+r+J.getHeight()+v);}
);F.addListener(w,function(e){var K=this.getBounds();this.debug(u+K.width+r+K.height);}
);}
}});}
)();
