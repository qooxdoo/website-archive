...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Validation-qx.theme.Indigo.3687963ad75b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j="demobrowser.demo.test.Validation",i="<b>Setting the age ",h="property to 27</b><br>",g="- name: Name is validated to a minimum length of two.<br/>",f="<br>old name: ",e=")",d="age:",c="fmale",b="<br><b>Setting the name to 'Hans'</b><br>",a="property to 'female'</b><br>",P="<br><b>Setting the gender ",O="must be longer than one characters. (",N="<b>Testing the gender ",M="<br>old gender: ",L="<br><b>Setting the age ",K="- age: Age is validated to be a number.<br/>",J="<b>Initial property values</b><br>",I="",H="<b>Setting the name to ''</b><br>",G="demobrowser.demo.test.Person",q="property to 'true'</b><br>",r="- gender: Gender is validated to be either male or female.",o="property to 'fmale'</b><br>",p="<font color='red'>GENDER NOT SET</font>",m="gender:",n="<br>old age: ",k="Hans",l="The demo creates an internal class containing three properties:<br/>",s="Validation Error: Name ",t="<font color='red'>AGE NOT SET</font>",y="My name",x="__Ri",A="<font color='red'>NAME NOT SET</font>",z="white",C="name:",B="<b>Description</b><br/>",v="new name: ",F="new gender: ",E="male",D="female",u="new age: ",w="<br>";qx.Class.define(j,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var Q=new demobrowser.demo.test.Person();var R=new qx.ui.embed.Html();R.setBackgroundColor(z);R.set({padding:10,width:250,height:360});this.getRoot().add(R,{left:10,top:10});R.setHtml(J);R.setHtml(R.getHtml()+C+Q.getName()+w);R.setHtml(R.getHtml()+d+Q.getAge()+w);R.setHtml(R.getHtml()+m+Q.getGender()+w);R.setHtml(R.getHtml()+b);Q.setName(k);R.setHtml(R.getHtml()+v+Q.getName()+w);R.setHtml(R.getHtml()+H);try{Q.setName(I);R.setHtml(R.getHtml()+v+Q.getName()+w);}
...
catch(U){R.setHtml(R.getHtml()+t+n+Q.getAge()+w);}
;R.setHtml(R.getHtml()+P+a);Q.setGender(D);R.setHtml(R.getHtml()+F+Q.getGender()+w);R.setHtml(R.getHtml()+N+o);try{Q.setAge(c);R.setHtml(R.getHtml()+F+Q.getGender()+w);}
catch(V){R.setHtml(R.getHtml()+p+M+Q.getGender()+w);}
;var S=new qx.ui.basic.Label();S.setRich(true);S.setWidth(250);S.setValue(B+l+g+K+r);this.getRoot().add(S,{left:280,top:10});}
}});qx.Class.define(G,{extend:qx.core.Object,properties:{name:{init:y,validate:x},age:{init:18,validate:qx.util.Validate.number()},gender:{init:E,validate:qx.util.Validate.inArray([E,D])}},members:{__Ri:function(W){if(W.length<2){throw new qx.core.ValidationError(s+O+W+e);}
;}
}});}
)();
