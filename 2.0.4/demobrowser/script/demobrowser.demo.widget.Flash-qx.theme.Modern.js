...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Flash-qx.theme.Modern.89a6733dc0f5.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="2) this html page is not loaded from the file system, but a webserver.",j="Flash Player Version",i="__RI",h="right",g="qooxdoo is the best!",f="Message from Flash:",d="__KG",c="Simple demo with FlashVars",b="__ce",a="bgcolor",F="demobrowser.demo.widget.Flash",E="demobrowser/demo/flash/TestFlash.swf",D="1) no pop-up or advertising blocker is activated.\n",C="Send to Flash",B="demobrowser/demo/flash/FlashVersion.swf",A="No message.",z="Flash:",y="__RH",x="__RJ",w="<b>qooxdoo:</b>",r="demobrowser.demo.widget.Flash.sendMessage",s="demobrowser.demo.widget.Flash.flashReady",p="Couldn't connect to Flash Player! Please make sure that:\n",q="demobrowser/demo/flash/fo_tester.swf",n="this is passed in via FlashVars",o="Two way communication (ActionScript <-> JavaScript)",l="Message to Flash:",m="Show Flash Player version",t="noscale",u="#FF6600",v="execute";qx.Class.define(F,{extend:qx.application.Standalone,members:{__ce:null,main:function(){qx.application.Standalone.prototype.main.call(this);var G=new qx.ui.tabview.TabView();G.setContentPadding([8,8,8,8]);var J=this.getRoot();J.add(G,{edge:0});var H=new qx.ui.tabview.Page(c);H.setLayout(new qx.ui.layout.Canvas());H.add(this.createFlashDemo1(),{edge:0});G.add(H);var I=new qx.ui.tabview.Page(o);I.setLayout(new qx.ui.layout.Canvas());I.add(this.createFlashDemo2(),{edge:0});G.add(I);this.__ce=this.createWindow();J.add(this.__ce,{top:20,left:20});}
...
,createFlashDemo1:function(){var N=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));var L={flashVarText:n};var M=new qx.ui.embed.Flash(q).set({scale:t,variables:L});M.getContentElement().setParam(a,u);N.add(M,{flex:1});var O=new qx.ui.form.Button(m);O.addListener(v,function(){this.__ce.open();}
,this);N.add(O);return N;}
,__KG:null,__RH:null,__RI:null,__RJ:null,createFlashDemo2:function(){this.self(arguments).setCallBackInstance(this);var R=new qx.ui.container.Composite(new qx.ui.layout.HBox(8));var Q=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));R.add(Q,{flex:1});Q.add(new qx.ui.basic.Label(w).set({rich:true}));Q.add(new qx.ui.basic.Label(f));this.__RH=new qx.ui.form.TextArea(A);this.__RH.setEnabled(false);Q.add(this.__RH,{flex:1});Q.add(new qx.ui.basic.Label(l));this.__RI=new qx.ui.form.TextArea(g);Q.add(this.__RI,{flex:1});var S=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,h));this.__RJ=new qx.ui.form.Button(C);this.__RJ.setEnabled(false);this.__RJ.addListener(v,function(e){this.__KG.getFlashElement().sendMessage(this.__RI.getValue());}
,this);S.add(this.__RJ);Q.add(S);var P=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));R.add(P,{flex:1});P.add(new qx.ui.basic.Label(z).set({rich:true}));this.__KG=new qx.ui.embed.Flash(E);this.__KG.setVariables({init:s});P.add(this.__KG,{flex:1});return R;}
,sendMessage:function(T){this.__RH.setValue(T);}
,initFlash:function(){if(this.__KG.getFlashElement().setup){this.__KG.getFlashElement().setup(r);this.__RJ.setEnabled(true);}
else {alert(p+D+k);}
;}
},statics:{__RK:null,setCallBackInstance:function(U){demobrowser.demo.widget.Flash.__RK=U;}
,sendMessage:function(V){demobrowser.demo.widget.Flash.__RK.sendMessage(V);}
,flashReady:function(){demobrowser.demo.widget.Flash.__RK.initFlash();}
},destruct:function(){this._disposeObjects(y,i,x,d,b);}
});}
)();
...
,_checkLoading:function(){var bi=this.getSource();if(bi!=t&&bi!=null&&bi!=J){if(!this.isLoaded()){if(!this.__KB){this.__KB=new Date().getTime();}
;var bj=new Date().getTime()-this.__KB;if(this.getLoadTimeout()>bj){var bk=qx.util.TimerManager.getInstance();bk.start(this._checkLoading,0,this,null,10);this.fireEvent(S);}
else {if(qx.core.Environment.get(N)){this.debug(r+bj);}
;this.fireEvent(F);this.__KB=null;}
;}
else {this.fireEvent(ba);this.__KB=null;}
;}
;}
...
,_applyQuality:function(bu,bv){this.__KC(j,bu);}
,_applyScale:function(bw,bx){this.__KC(f,bw);}
,_applyWmode:function(by,bz){this.__KC(a,by);}
,_applyPlay:function(bA,bB){this.__KC(l,bA);}
,_applyLoop:function(bC,bD){this.__KC(W,bC);}
,_applyMenu:function(bE,bF){this.__KC(U,bE);}
,_applyAllowScriptAccess:function(bG,bH){this.__KC(V,bG);}
,_applyLiveConnect:function(bI,bJ){this.__KC(q,bI);}
,__KC:function(bK,bL){this.getContentElement().setParam(bK,bL);qx.ui.core.queue.Layout.add(this);}
}});}
)();
(function(){var m="The id cannot be modified after initial creation",l="The variables cannot be modified after initial creation",k="movie",j="The attributes cannot be modified after initial creation",i="The source cannot be modified after initial creation",h="The params cannot be modified after initial creation",g="id",f="qx.html.Flash",e="Invalid attribute 'key'.",d="div",a="Invalid attribute 'value' expected String, Boolean or null.",c="Invalid attribute 'value'.",b="qx.debug";qx.Class.define(f,{extend:qx.html.Element,construct:function(n,o){qx.html.Element.call(this,d,n,o);this.__KD={};this.__KE={};this.__KF={};}
,members:{__KD:null,__KF:null,__KE:null,__KG:null,_createDomElement:function(){return qx.dom.Element.create(d);}
,createFlash:function(){this.__KG=qx.bom.Flash.create(this.getDomElement(),this.getAttributes(),this.__KE,this.__KD);}
,setSource:function(p){if(qx.core.Environment.get(b)){qx.core.Assert.assertString(p,c);}
;if(this.__KG){throw new Error(i);}
;this.setAttribute(k,p);}
,setId:function(q){if(qx.core.Environment.get(b)){qx.core.Assert.assertString(q,c);}
;if(this.__KG){throw new Error(m);}
;this.setAttribute(g,q);}
,getVariables:function(){return this.__KE;}
,setVariables:function(r){if(qx.core.Environment.get(b)){qx.core.Assert.assertMap(r,c);}
;if(this.__KG){throw new Error(l);}
;this.__KE=r;}
,getAttributes:function(){return this.__KF;}
...
;}
;}
;if(this.__KG){throw new Error(j);}
;if(t===null||t===undefined){delete this.__KF[s];}
else {this.__KF[s]=t;}
;}
,getParams:function(){return this.__KD;}
...
;}
;}
;if(this.__KG){throw new Error(h);}
;if(v===null||v===undefined){delete this.__KD[u];}
else {this.__KD[u]=v;}
;}
,getFlashElement:function(){return this.__KG;}
},destruct:function(){if(this.__KG){qx.bom.Flash.destroy(this.__KG);}
;this.__KD=this.__KE=this.__KF=null;}
});}
)();
