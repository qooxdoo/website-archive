...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.FiniteStateMachine","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine-qx.theme.Simple.d36c165e0055.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var v="FadingOut, and Zombie (about to dispose).",u="which it was instantiated); when it ",t="Each mouse is driven by (and in fact, in this example, is a ",s="The initial mouse is a bit special, in that it looks for ",r="This is a simple example of using a finite state machine, in ",q="behind it, as well as to its left and right.",p="qx.debug",n="Each mouse is quite dumb, having in ",m="arrives at a new cell, it looks left and right and creates a ",l="<u>The finite state diagram is below the maze.</u>",e="its arsenal of knowledge only these characteristics: it knows ",k="subclass of) a finite state machine.  The FSM has five states: ",h="clone of itself to go each available direction; upon hitting a ",c="only how to travel in a single direction (the direction in ",b="demobrowser.demo.ui.FiniteStateMachine",g="wall, it dies.",f="FadingIn, CellArrival (look for cloning opportunities), Moving, ",i="which virtual mice solve a maze. ",a="the possibility to clone a mouse to travel in the direction ",j="demobrowser/demo/ui/FsmMiceMaze.png",d="<p>";qx.Class.define(b,{extend:qx.application.Standalone,construct:function(){qx.application.Standalone.call(this);}
,members:{main:function(){qx.application.Standalone.prototype.main.call(this);if((qx.core.Environment.get(p))){qx.log.appender.Native;qx.log.appender.Console;}
;var x=r+i+d+n+e+c+u+m+h+g+d+s+a+q+d+t+k+f+v+d+l;var scroll=new qx.ui.container.Scroll();this.getRoot().add(scroll,{edge:0});var w=new qx.ui.container.Composite(new qx.ui.layout.Canvas());scroll.add(w);var o;o=new qx.ui.basic.Image(j);o=null;o=new qx.ui.basic.Label(x);o.set({width:700,rich:true,wrap:true});w.add(o,{left:50});var A=new demobrowser.demo.util.FSMMaze(10,10,50,240);w.add(A,{left:50,top:240});o=new qx.ui.basic.Image(j,800);w.add(o,{left:50,top:800});var z;var y=Math.floor(Math.random()*4);switch(y){case 0:z=demobrowser.demo.util.FSMMaze.Direction.NORTH;break;case 1:z=demobrowser.demo.util.FSMMaze.Direction.EAST;break;case 2:z=demobrowser.demo.util.FSMMaze.Direction.SOUTH;break;case 3:z=demobrowser.demo.util.FSMMaze.Direction.WEST;break;};new demobrowser.demo.util.FSMMouse(null,A,z,w);}
...
(function(){var g="#b0ffb0",f="&bull;",e="demobrowser.demo.util.FSMMaze",d="#ffb0b0",c="html",b="black",a="solid";qx.Class.define(e,{extend:qx.ui.container.Composite,statics:{Direction:{WEST:0x8,SOUTH:0x4,EAST:0x2,NORTH:0x1}},construct:function(h,i,x,y,j){qx.ui.container.Composite.call(this);this.setLayout(new qx.ui.layout.Grid());this.numRows=(h===undefined?10:h);this.numCols=(i===undefined?10:i);this.x=(x===undefined?50:x);this.y=(y===undefined?50:y);this.cellSize=(j===undefined?50:j);this.totalHeight=this.numRows*this.cellSize;this.totalWidth=this.numCols*this.cellSize;this.setWidth(this.totalWidth);this.setHeight(this.totalHeight);for(var C=0;C<this.numRows;C++){this.getLayout().setRowHeight(C,this.cellSize);}
;for(var A=0;A<this.numCols;A++){this.getLayout().setColumnWidth(A,this.cellSize);}
;this.cells=[];this.mazeInfo=[];for(C=0;C<this.numRows;C++){this.cells[C]=[];this.mazeInfo[C]=[];for(A=0;A<this.numCols;A++){this.cells[C][A]=new qx.ui.container.Composite();this.cells[C][A].setLayout(new qx.ui.layout.HBox());this.add(this.cells[C][A],{row:C,column:A});this.cells[C][A].setDecorator(new qx.ui.decoration.Single(1,a,b));this.mazeInfo[C][A]=0xf;}
;}
;var s=[];var n=1;var m=this.numRows*this.numCols;var z;var k;var t;z={row:Math.floor(Math.random()*this.numRows),col:Math.floor(Math.random()*this.numCols)};while(n<m){k=[];if(z.col>0&&(this.mazeInfo[z.row][z.col-1]==0xf)){k.push({row:z.row,col:z.col-1,currentCellWall:demobrowser.demo.util.FSMMaze.Direction.WEST,neighborWall:demobrowser.demo.util.FSMMaze.Direction.EAST});}
;if(z.row<this.numRows-1&&(this.mazeInfo[z.row+1][z.col]==0xf)){k.push({row:z.row+1,col:z.col,currentCellWall:demobrowser.demo.util.FSMMaze.Direction.SOUTH,neighborWall:demobrowser.demo.util.FSMMaze.Direction.NORTH});}
;if(z.col<this.numCols-1&&(this.mazeInfo[z.row][z.col+1]==0xf)){k.push({row:z.row,col:z.col+1,currentCellWall:demobrowser.demo.util.FSMMaze.Direction.EAST,neighborWall:demobrowser.demo.util.FSMMaze.Direction.WEST});}
;if(z.row>0&&(this.mazeInfo[z.row-1][z.col]==0xf)){k.push({row:z.row-1,col:z.col,currentCellWall:demobrowser.demo.util.FSMMaze.Direction.NORTH,neighborWall:demobrowser.demo.util.FSMMaze.Direction.SOUTH});}
;if(k.length>0){var r=Math.floor(Math.random()*k.length);t=k[r];this.mazeInfo[z.row][z.col]&=~t.currentCellWall;this.mazeInfo[t.row][t.col]&=~t.neighborWall;var p=new qx.ui.decoration.Single(1,a,b);var w=this.cells[z.row][z.col].getDecorator();p.set({widthLeft:w.getWidthLeft(),widthBottom:w.getWidthBottom(),widthRight:w.getWidthRight(),widthTop:w.getWidthTop()});switch(t.currentCellWall){case demobrowser.demo.util.FSMMaze.Direction.WEST:p.setWidthLeft(0);break;case demobrowser.demo.util.FSMMaze.Direction.SOUTH:p.setWidthBottom(0);break;case demobrowser.demo.util.FSMMaze.Direction.EAST:p.setWidthRight(0);break;case demobrowser.demo.util.FSMMaze.Direction.NORTH:p.setWidthTop(0);break;};this.cells[z.row][z.col].setDecorator(p);var v=new qx.ui.decoration.Single(1,a,b);var l=this.cells[t.row][t.col].getDecorator();v.set({widthLeft:l.getWidthLeft(),widthBottom:l.getWidthBottom(),widthRight:l.getWidthRight(),widthTop:l.getWidthTop()});switch(t.neighborWall){case demobrowser.demo.util.FSMMaze.Direction.WEST:v.setWidthLeft(0);break;case demobrowser.demo.util.FSMMaze.Direction.SOUTH:v.setWidthBottom(0);break;case demobrowser.demo.util.FSMMaze.Direction.EAST:v.setWidthRight(0);break;case demobrowser.demo.util.FSMMaze.Direction.NORTH:v.setWidthTop(0);break;};this.cells[t.row][t.col].setDecorator(v);s.push({row:z.row,col:z.col});z={row:t.row,col:t.col};n++;}
else {var q=s.pop();z={row:q.row,col:q.col};}
;}
;this.startCell={row:Math.floor(Math.random()*this.numRows),col:Math.floor(Math.random()*this.numCols)};var u=this.cells[this.startCell.row][this.startCell.col];u.setBackgroundColor(g);do {this.endCell={row:Math.floor(Math.random()*this.numRows),col:Math.floor(Math.random()*this.numCols)};}
while((Math.abs(this.startCell.row-this.endCell.row)<this.numRows/2)||(Math.abs(this.startCell.col-this.endCell.col)<this.numCols/2));var B=this.cells[this.endCell.row][this.endCell.col];B.setBackgroundColor(d);}
...
;return null;}
,markCell:function(M){var N=Math.ceil(this.cellSize/5);var o=new qx.ui.basic.Label(f,null,c);o.set({height:N,width:N,paddingTop:(this.cellSize-N)/2,paddingLeft:(this.cellSize-N)/2,rich:true});this.cells[M.row][M.col].add(o);}
}});}
)();
...
(function(){var m="string",l="__LN",k="Saved-state stack is empty",j="State ",h=" already exists",g="qx.util.fsm.FiniteStateMachine",f="#actionsAfterOnentry",d="Saved-state stack is full",c="__LM",b="#actionsBeforeOnentry",A=": Pushed event: ",z="Machine started with no available states",y="#entry",x=": Queued event: ",w="",v=" on ",u="Invalid state: not an instance of ",s="Number",r="qx.util.fsm.State",q="#",o="  ",p=" => ",n="String";qx.Class.define(g,{extend:qx.core.Object,construct:function(B){qx.core.Object.call(this);this.setName(B);this.__hA={};this.__LK=null;this.__LL=[];this.__LM=[];this.__LN=[];this.__LO={};this.__LP={};this.__LQ={};this.__LR={};this.__LS={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:n,nullable:true},state:{check:n,nullable:true},previousState:{check:n,nullable:true},nextState:{check:n,nullable:true},maxSavedStates:{check:s,init:2},debugFlags:{check:s,init:7}},members:{__hA:null,__LK:null,__LM:null,__LN:null,__LL:null,__LO:null,__LP:null,__LQ:null,__LR:null,__LS:null,__LT:false,addState:function(C){if(!C instanceof qx.util.fsm.State){throw new Error(u+r);}
;var D=C.getName();if(D in this.__hA){throw new Error(j+D+h);}
;if(this.__LK==null){this.__LK=D;}
;this.__hA[D]=C;}
,replaceState:function(E,F){if(!E instanceof qx.util.fsm.State){throw new Error(u+r);}
;var H=E.getName();var G=this.__hA[H];this.__hA[H]=E;if(F){G._bNeedDispose=true;}
;return G;}
,addObject:function(I,J,K){var M=J.toHashCode();this.__LP[I]=M;this.__LQ[M]=I;this.__LO[I]=J;if(!K){return;}
;if(typeof (K)==m){K=[K];}
;for(var i=0;i<K.length;i++){var L=K[i];if(!this.__LR[L]){this.__LR[L]={};}
;this.__LR[L][I]=true;if(!this.__LS[I]){this.__LS[I]=[];}
;this.__LS[I].push(L);}
;}
,removeObject:function(N){var P;var O;var Q;var R;P=this.__LP[N];if(this.__LS[N]){for(var i=0;i<this.__LS[N].length;i++){O=this.__LS[N][i];delete this.__LR[O][N];R=true;for(Q in this.__LR[O]){R=false;break;}
;if(R){delete this.__LR[O];}
;}
;delete this.__LS[N];}
;delete this.__LQ[P];delete this.__LP[N];delete this.__LO[N];}
,getObject:function(S){return this.__LO[S];}
,getFriendlyName:function(T){var U=T?T.toHashCode():null;return U?this.__LQ[U]:null;}
,getGroupObjects:function(V){var a=[];for(var name in this.__LR[V]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var W in this.__LP){var Y=this.__LP[W];var X=this.getObject(W);this.debug(W+p+Y);this.debug(o+Y+p+this.__LQ[Y]);this.debug(o+W+p+this.getObject(W));this.debug(o+this.getObject(W)+p+this.getFriendlyName(X));}
;}
,_getInternalData:function(){return ({"states":this.__hA,"startState":this.__LK,"eventQueue":this.__LM,"blockedEvents":this.__LN,"savedStates":this.__LL,"friendlyToObject":this.__LO,"friendlyToHash":this.__LP,"hashToFriendly":this.__LQ,"groupToFriendly":this.__LR,"friendlyToGroups":this.__LS});}
,start:function(){var bb=this.__LK;if(bb==null){throw new Error(z);}
;this.setState(bb);this.setPreviousState(null);this.setNextState(null);var ba=(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL);if(ba){this.debug(this.getName()+q+bb+b);}
;this.__hA[bb].getAutoActionsBeforeOnentry()(this);if(ba){this.debug(this.getName()+q+bb+y);}
;this.__hA[bb].getOnentry()(this,null);if(ba){this.debug(this.getName()+q+bb+f);}
;this.__hA[bb].getAutoActionsAfterOnentry()(this);}
,pushState:function(bc){if(this.__LL.length>=this.getMaxSavedStates()){throw new Error(d);}
;if(bc===true){this.__LL.push(this.getState());}
else if(bc){this.__LL.push(bc);}
else {this.__LL.push(this.getPreviousState());}
;}
,popState:function(){if(this.__LL.length==0){throw new Error(k);}
;return this.__LL.pop();}
,postponeEvent:function(event){this.__LN.unshift(event);}
,enqueueEvent:function(event,bd){if(bd){this.__LM.push(event);}
else {this.__LM.unshift(event);}
;if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){var be=this.getFriendlyName(event.getTarget());if(bd){this.debug(this.getName()+A+event.getType()+(be?v+be:w));}
else {this.debug(this.getName()+x+event.getType()+(be?v+be:w));}
;}
;}
,eventListener:function(event){var e=event.clone();this.enqueueEvent(e,false);this.__LU();}
,fireImmediateEvent:function(bf,bg,bh){if(bh){var event=qx.event.Registration.createEvent(bf,qx.event.type.Data,[bh,null,false]);}
else {var event=qx.event.Registration.createEvent(bf,qx.event.type.Event,[false,false]);}
;event.setTarget(bg);this.eventListener(event);}
,scheduleEvent:function(bi,bj,bk,bl){qx.event.Timer.once(function(){this.fireImmediateEvent(bi,bj,bk);}
,this,bl||20);}
,__LU:function(){if(this.__LT){return;}
;this.__LT=true;while(this.__LM.length>0){var event=this.__LM.pop();var bm=this.__LV(event);if(bm){event.dispose();}
;}
;this.__LT=false;}
,__LV:function(event){var bp;var br;var bo;var bz;var bx;var e;var bv;var bn=this.getDebugFlags();var by=bn&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var bs=bn&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var bw=bn&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var bu=bn&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var bq=this.getFriendlyName(event.getTarget());if(by){this.debug(this.getName()+": Process event: "+event.getType()+(bq?" on "+bq:""));}
;bp=this.getState();bz=this.__hA[bp];bx=bz.transitions;e=bz.getEvents()[event.getType()];if(!e){if(by){this.debug(this.getName()+": Event '"+event.getType()+"'"+" not handled.  Ignoring.");}
;return true;}
;if(typeof (e)=="object"){if(!bq){if(bu){this.debug(this.getName()+": Could not find friendly name for '"+event.getType()+"' on '"+event.getTarget()+"'");}
;return true;}
;bv=e[bq];if(!bv){if(by){this.debug(this.getName()+": Event '"+event.getType()+"'"+" not handled for target "+bq+".  Ignoring.");}
;return true;}
;}
else {bv=e;}
;switch(bv){case qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE:break;case qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED:if(by){this.debug(this.getName()+": Event '"+event.getType()+"'"+" blocked.  Re-queuing.");}
;this.__LN.unshift(event);return false;default:if(typeof (bv)=="string"){if(bx[bv]){var bt=bx[bv];bx={};bx[bv]=bt;}
else {throw new Error("Explicit transition "+bv+" does not exist");}
;break;}
;};for(var t in bx){var bt=bx[t];switch(bt.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error("Transition "+bp+":"+t+" returned a value other than "+"true, false, or null.");};br=bt.getNextState();if(typeof (br)=="string"){if(!br in this.__hA){throw new Error("Attempt to transition to nonexistent state "+br);}
;this.setNextState(br);}
else {switch(br){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:br=bp;this.setNextState(br);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__LL.length==0){throw new Error("Attempt to transition to POP_STATE_STACK "+"while state stack is empty.");}
;br=this.__LL.pop();this.setNextState(br);break;default:throw new Error("Internal error: invalid nextState");break;};}
;if(bw){this.debug(this.getName()+"#"+bp+"#"+t+"#autoActionsBeforeOntransition");}
;bt.getAutoActionsBeforeOntransition()(this);if(bw){this.debug(this.getName()+"#"+bp+"#"+t+"#ontransition");}
;bt.getOntransition()(this,event);if(bw){this.debug(this.getName()+"#"+bp+"#"+t+"#autoActionsAfterOntransition");}
;bt.getAutoActionsAfterOntransition()(this);if(bw){this.debug(this.getName()+"#"+bp+"#autoActionsBeforeOnexit");}
;bz.getAutoActionsBeforeOnexit()(this);if(bw){this.debug(this.getName()+"#"+bp+"#exit");}
;bz.getOnexit()(this,event);if(bw){this.debug(this.getName()+"#"+bp+"#autoActionsAfterOnexit");}
;bz.getAutoActionsAfterOnexit()(this);if(bz._bNeedDispose){bz.dispose();}
;bz=this.__hA[this.getNextState()];this.setPreviousState(bp);this.setState(this.getNextState());this.setNextState(null);bo=bp;bp=br;br=undefined;if(bw){this.debug(this.getName()+"#"+bp+"#autoActionsBeforeOnentry");}
;bz.getAutoActionsBeforeOnentry()(this);if(bw){this.debug(this.getName()+"#"+bp+"#entry");}
;bz.getOnentry()(this,event);if(bw){this.debug(this.getName()+"#"+bp+"#autoActionsAfterOnentry");}
;bz.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__LN.length;i++){e=this.__LN.pop();this.__LM.unshift(e);}
;if(bs){this.debug(this.getName()+"#"+bo+" => "+this.getName()+"#"+bp);}
;return true;}
;if(bs){this.debug(this.getName()+"#"+bp+": event '"+event.getType()+"'"+": no transition found.  No state change.");}
;return true;}
},destruct:function(){this._disposeArray(c);this._disposeArray(l);this.__LL=this.__hA=null;}
});}
)();
(function(){var q="State info must be an object",p="function",o="  var groupObjects = ",n="__LF",m="__LG",l="parameters",k=")",h=" fsm.getObject('",d="groups",c="Invalid function parameters: ",bq="  for (var i = 0; i < groupObjects.length; i++)",bp="__LH",bo="autoActionsAfterOnentry",bn=" value: ",bm="');",bl="State ",bk="autoActionsBeforeOnexit",bj="autoActionsBeforeOnentry",bi="__LB",bh="onexit",x="Invalid function request type: ",y="__LD",v=": ",w="Invalid friendly name in 'objects' list: ",t="autoActionsAfterOnexit",u="  fsm.debug(ex);",r="__LE",s="    fsm.getObject(objName).",D=")(fsm)",E="events",M="Invalid ",K="__LI",U="try",P="    var objName = groupObjects[i];",bd="').",ba="objects",G="expected object, found ",bg="qx.util.fsm.Transition",bf="The events object must be provided in new state info",be="catch(ex)",F="  {",I="    fsm.getGroupObjects('",J="Invalid transition: not an instance of ",L="  }",N="Invalid 'objects' list: expected array, got ",Q="__LC",W="qx.util.fsm.State",bc="onentry",z="Invalid function request parameter type: ",A="Invalid 'groups' list: expected array, got ",H="fsm",T=",",S="Adding user-provided field to state: ",R="context",Y="string",X='"',O="}",V="{",b="expected array, found ",bb="(",B=";",C="object";qx.Class.define(W,{extend:qx.core.Object,construct:function(br,bs){var bu;qx.core.Object.call(this);this.setName(br);if(typeof (bs)!=C){throw new Error(q);}
;bu=bs.context||window;this.setUserData(R,bu);for(var bt in bs){switch(bt){case bc:this.setOnentry(this.__LJ(bs[bt],bu));break;case bh:this.setOnexit(this.__LJ(bs[bt],bu));break;case bj:this.setAutoActionsBeforeOnentry(bs[bt]);break;case bo:this.setAutoActionsAfterOnentry(bs[bt]);break;case bk:this.setAutoActionsBeforeOnexit(bs[bt]);break;case t:this.setAutoActionsAfterOnexit(bs[bt]);break;case E:this.setEvents(bs[bt]);break;case R:break;default:this.setUserData(bt,bs[bt]);this.debug(bl+br+v+S+bt);break;};}
;if(!this.getEvents()){throw new Error(bf);}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(bv,bw,bx){if(typeof (bw)!=C){throw new Error(M+bv+bn+typeof (bw));}
;var bC;var bD=U+V;var bA;var by;for(var f in bw){var bz=bw[f];if(!bz instanceof Array){throw new Error(x+b+typeof (bz));}
;for(var i=0;i<bz.length;i++){by=bz[i];if(typeof (by)!=C){throw new Error(z+G+typeof (bz[bA]));}
;var bB=by[l];if(!bB){bB=[];}
else {if(!bB instanceof Array){throw new Error(c+b+typeof (bB));}
;}
;bC=f+bb;for(var j=0;j<bB.length;j++){if(j!=0){bC+=T;}
;if(typeof (bB[j])==p){bC+=bb+bB[j]+D;}
else if(typeof (bB[j])==Y){bC+=X+bB[j]+X;}
else {bC+=bB[j];}
;}
;bC+=k;var a=by[ba];if(!a){a=[];}
else if(!a instanceof Array){throw new Error(N+typeof (a));}
;for(var j=0;j<a.length;j++){if(typeof (a[j])!=Y){throw new Error(w+a[j]);}
;bD+=h+a[j]+bd+bC+B;}
;var g=by[d];if(g){if(!g instanceof Array){throw new Error(A+typeof (g));}
;for(j=0;j<g.length;j++){bD+=o+I+g[j]+bm+bq+F+P+s+bC+B+L;}
;}
;}
;}
;bD+=O+be+V+u+O;return qx.lang.Function.bind(new Function(H,bD),bx);}
},properties:{name:{transform:bi,nullable:true},onentry:{transform:Q,nullable:true,init:function(bE,event){}
},onexit:{transform:y,nullable:true,init:function(bF,event){}
},autoActionsBeforeOnentry:{transform:n,nullable:true,init:function(bG,event){}
},autoActionsAfterOnentry:{transform:m,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnexit:{transform:bp,nullable:true,init:function(bI,event){}
},autoActionsAfterOnexit:{transform:K,nullable:true,init:function(bJ,event){}
},events:{transform:r,nullable:true}},members:{__LB:function(bK){if(typeof (bK)!="string"||bK.length<1){throw new Error("Invalid state name");}
;return bK;}
,__LC:function(bL){switch(typeof (bL)){case "undefined":return function(bM,event){}
;case "function":return qx.lang.Function.bind(bL,this.getUserData("context"));default:throw new Error("Invalid onentry type: "+typeof (bL));};}
,__LD:function(bN){switch(typeof (bN)){case "undefined":return function(bO,event){}
;case "function":return qx.lang.Function.bind(bN,this.getUserData("context"));default:throw new Error("Invalid onexit type: "+typeof (bN));return null;};}
,__LE:function(bP){if(typeof (bP)!="object"){throw new Error("events must be an object");}
;for(var e in bP){var bQ=bP[e];if(typeof (bQ)=="number"&&bQ!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bQ!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error("Invalid numeric value in events object: "+e+": "+bQ);}
else if(typeof (bQ)=="object"){for(var bR in bQ){if(typeof (bQ[bR])=="number"&&bQ[bR]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bQ[bR]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error("Invalid numeric value in events object "+"("+e+"): "+bR+": "+bQ[bR]);}
else if(typeof (bQ[bR])!="string"&&typeof (bQ[bR])!="number"){throw new Error("Invalid value in events object "+"("+e+"): "+bR+": "+bQ[bR]);}
;}
;}
else if(typeof (bQ)!="string"&&typeof (bQ)!="number"){throw new Error("Invalid value in events object: "+e+": "+bP[e]);}
;;}
;return bP;}
,__LF:function(bS){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnentry",bS,this.getUserData("context"));}
,__LG:function(bT){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnentry",bT,this.getUserData("context"));}
,__LH:function(bU){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnexit",bU,this.getUserData("context"));}
,__LI:function(bV){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnexit",bV,this.getUserData("context"));}
,__LJ:function(f,bW){if(typeof (f)=="function"){f=qx.lang.Function.bind(f,bW);}
;return f;}
,addTransition:function(bX){if(!bX instanceof qx.util.fsm.Transition){throw new Error(J+bg);}
;this.transitions[bX.getName()]=bX;}
}});}
)();
(function(){var t="predicate",s="nextState",r="__LW",q="autoActionsBeforeOntransition",p="__LX",o=": ",n="__Ma",m="Invalid ontransition type: ",l="String",k="Transition ",d="Adding user-provided field to transition: ",j="function",h="__Mb",c="qx.util.fsm.Transition",b="undefined",g="ontransition",e="__LY",i="autoActionsAfterOntransition",a="context";qx.Class.define(c,{extend:qx.core.Object,construct:function(u,v){var x;qx.core.Object.call(this);this.setName(u);x=v.context||window;this.setUserData(a,x);for(var w in v){switch(w){case t:this.setPredicate(this.__LJ(v[w],x));break;case s:this.setNextState(v[w]);break;case q:this.setAutoActionsBeforeOntransition(this.__LJ(v[w],x));break;case i:this.setAutoActionsAfterOntransition(this.__LJ(v[w],x));break;case g:this.setOntransition(this.__LJ(v[w],x));break;case a:break;default:this.setUserData(w,v[w]);this.debug(k+u+o+d+w);break;};}
;}
,properties:{name:{check:l,nullable:true},predicate:{init:function(y,event){return true;}
,transform:r},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:p},autoActionsBeforeOntransition:{init:function(z,event){}
,transform:e},autoActionsAfterOntransition:{init:function(A,event){}
,transform:n},ontransition:{init:function(B,event){}
,transform:h}},members:{__LW:function(C){switch(typeof (C)){case "undefined":return function(D,event){return true;}
;case "boolean":return function(E,event){return C;}
;case "function":return qx.lang.Function.bind(C,this.getUserData("context"));default:throw new Error("Invalid transition predicate type: "+typeof (C));break;};}
,__LX:function(F){switch(typeof (F)){case "string":return F;case "number":switch(F){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return F;default:throw new Error("Invalid transition nextState value: "+F+": "+"nextState must be an explicit state name, "+"or one of the Fsm.StateChange constants");};break;default:throw new Error("Invalid transition nextState type: "+typeof (F));break;};}
,__LY:function(G){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOntransition",G,this.getUserData("context"));}
,__Ma:function(H){return qx.util.fsm.State._commonTransformAutoActions(i,H,this.getUserData(a));}
,__Mb:function(I){switch(typeof (I)){case b:return function(J,event){}
;case j:return qx.lang.Function.bind(I,this.getUserData(a));default:throw new Error(m+typeof (I));break;};}
,__LJ:function(f,K){if(typeof (f)=="function"){f=qx.lang.Function.bind(f,K);}
;return f;}
}});}
)();
(function(){var k="Transition_FadingOut_to_Zombie_via_Oneshot",j="demobrowser/demo/ui/mouse-west.gif",h="Transition_FadingIn_to_CellArrival_via_Oneshot",g="Transition_FadingOut_to_FadingOut_via_Oneshot",f="Transition_FadingIn_to_FadingIn_via_Oneshot",e="interval",d="demobrowser/demo/ui/mouse-east.gif",c="demobrowser/demo/ui/mouse-north.gif",b="Transition_Moving_to_Moving_via_Oneshot",a="Transition_Moving_to_CellArrival_via_Oneshot",w="oneshot",v="demobrowser.demo.util.FSMMouse",u="demobrowser/demo/ui/mouse-south.gif",t="State_Zombie",s="State_FadingIn",r="Transition_CellArrival_to_Moving_via_KickInTheAss",q="Transition_CellArrival_to_FadingOut_via_Die",p="State_Moving",o="State_CellArrival",n="State_FadingOut",l="KickInTheAss",m="Die";qx.Class.define(v,{extend:qx.util.fsm.FiniteStateMachine,statics:{nextMouse:1,ticksPerSec:20,fadeSeconds:0.5,moveSeconds:0.2},construct:function(x,y,z,A){var B=demobrowser.demo.util.FSMMouse.nextMouse.toString();qx.util.fsm.FiniteStateMachine.call(this,B);this.setDebugFlags(0);++demobrowser.demo.util.FSMMouse.nextMouse;this.facing=z;if(x===null){this.currentCell=y.getStartCell();this.traversed=[];this.traversed.push(this.currentCell);this.bLookBehind=true;this.bLookSideways=true;}
else {this.currentCell=x.currentCell;this.traversed=x.traversed.concat();this.bLookBehind=false;this.bLookSideways=false;}
;this.currentLocation=y.getCellTopLeft(this.currentCell);switch(z){case demobrowser.demo.util.FSMMaze.Direction.NORTH:this.mouseImage=new qx.ui.basic.Image(c);break;case demobrowser.demo.util.FSMMaze.Direction.EAST:this.mouseImage=new qx.ui.basic.Image(d);break;case demobrowser.demo.util.FSMMaze.Direction.SOUTH:this.mouseImage=new qx.ui.basic.Image(u);break;case demobrowser.demo.util.FSMMaze.Direction.WEST:this.mouseImage=new qx.ui.basic.Image(j);break;};this.mouseImage.set({opacity:0.0,zIndex:9999});A.add(this.mouseImage,{top:this.currentLocation.top,left:this.currentLocation.left});var C=new qx.util.fsm.State(s,{"onentry":function(E,F){E.oneshot(1000/demobrowser.demo.util.FSMMouse.ticksPerSec);}
,"events":{"oneshot":qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE}});this.addState(C);var D=new qx.util.fsm.Transition(f,{"predicate":function(G,event){return G.opacity<1.0;}
,"nextState":s,"ontransition":function(H,event){H.opacity+=1/(demobrowser.demo.util.FSMMouse.fadeSeconds*demobrowser.demo.util.FSMMouse.ticksPerSec);H.mouseImage.setOpacity(H.opacity);}
});C.addTransition(D);var D=new qx.util.fsm.Transition(h,{"nextState":o});C.addTransition(D);var C=new qx.util.fsm.State(o,{"onentry":function(I,J){var K=y.getEndCell();if(I.currentCell.row==K.row&&I.currentCell.col==K.col){for(var i=I.traversed.length-2;i>=0;i--){y.markCell(I.traversed[i]);}
;return;}
;switch(I.facing){case demobrowser.demo.util.FSMMaze.Direction.WEST:if(I.bLookSideways){if(y.getSouthCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.SOUTH,A);}
;if(y.getNorthCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.NORTH,A);}
;}
;I.bLookSideways=true;if(I.bLookBehind&&y.getEastCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.EAST,A);I.bLookBehind=false;}
;I.destinationCell=y.getWestCell(I.currentCell);if(I.destinationCell!==null){I.destinationLocation=y.getCellTopLeft(I.destinationCell);var event=new qx.event.type.Event();event.setType(l);I.eventListener(event);}
else {var event=new qx.event.type.Event();event.setType(m);I.eventListener(event);}
;break;case demobrowser.demo.util.FSMMaze.Direction.EAST:if(I.bLookSideways){if(y.getSouthCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.SOUTH,A);}
;if(y.getNorthCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.NORTH,A);}
;}
;I.bLookSideways=true;if(I.bLookBehind&&y.getWestCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.WEST,A);I.bLookBehind=false;}
;I.destinationCell=y.getEastCell(I.currentCell);if(I.destinationCell!==null){I.destinationLocation=y.getCellTopLeft(I.destinationCell);var event=new qx.event.type.Event();event.setType(l);I.eventListener(event);}
else {var event=new qx.event.type.Event();event.setType(m);I.eventListener(event);}
;break;case demobrowser.demo.util.FSMMaze.Direction.SOUTH:if(I.bLookSideways){if(y.getWestCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.WEST,A);}
;if(y.getEastCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.EAST,A);}
;}
;I.bLookSideways=true;if(I.bLookBehind&&y.getNorthCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.NORTH,A);I.bLookBehind=false;}
;I.destinationCell=y.getSouthCell(I.currentCell);if(I.destinationCell!==null){I.destinationLocation=y.getCellTopLeft(I.destinationCell);var event=new qx.event.type.Event();event.setType(l);I.eventListener(event);}
else {var event=new qx.event.type.Event();event.setType(m);I.eventListener(event);}
;break;case demobrowser.demo.util.FSMMaze.Direction.NORTH:if(I.bLookSideways){if(y.getWestCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.WEST,A);}
;if(y.getEastCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.EAST,A);}
;}
;I.bLookSideways=true;if(I.bLookBehind&&y.getSouthCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.SOUTH,A);I.bLookBehind=false;}
;I.destinationCell=y.getNorthCell(I.currentCell);if(I.destinationCell!==null){I.destinationLocation=y.getCellTopLeft(I.destinationCell);var event=new qx.event.type.Event();event.setType(l);I.eventListener(event);}
else {var event=new qx.event.type.Event();event.setType(m);I.eventListener(event);}
;break;};I.bLookBehind=false;}
,"events":{"Die":q,"KickInTheAss":r}});this.addState(C);var D=new qx.util.fsm.Transition(q,{"nextState":n});C.addTransition(D);var D=new qx.util.fsm.Transition(r,{"nextState":p});C.addTransition(D);var C=new qx.util.fsm.State(n,{"onentry":function(L,M){L.oneshot(1000/demobrowser.demo.util.FSMMouse.ticksPerSec);}
,"events":{"oneshot":qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE}});this.addState(C);var D=new qx.util.fsm.Transition(g,{"predicate":function(N,event){return N.opacity>0.0;}
,"nextState":n,"ontransition":function(O,event){O.opacity-=1/(demobrowser.demo.util.FSMMouse.fadeSeconds*demobrowser.demo.util.FSMMouse.ticksPerSec);if(O.opacity<0.0){O.opacity=0.0;}
;O.mouseImage.setOpacity(O.opacity);}
});C.addTransition(D);var D=new qx.util.fsm.Transition(k,{"nextState":t});C.addTransition(D);var C=new qx.util.fsm.State(p,{"onentry":function(P,Q){P.oneshot(1000/demobrowser.demo.util.FSMMouse.ticksPerSec);}
,"events":{"oneshot":qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE}});this.addState(C);var D=new qx.util.fsm.Transition(b,{"predicate":function(R,event){switch(R.facing){case demobrowser.demo.util.FSMMaze.Direction.WEST:return R.currentLocation.left>R.destinationLocation.left;case demobrowser.demo.util.FSMMaze.Direction.SOUTH:return R.currentLocation.top<R.destinationLocation.top;case demobrowser.demo.util.FSMMaze.Direction.EAST:return R.currentLocation.left<R.destinationLocation.left;case demobrowser.demo.util.FSMMaze.Direction.NORTH:return R.currentLocation.top>R.destinationLocation.top;};}
,"nextState":p,"ontransition":function(S,event){var T=Math.ceil(y.cellSize/(demobrowser.demo.util.FSMMouse.moveSeconds*demobrowser.demo.util.FSMMouse.ticksPerSec));switch(S.facing){case demobrowser.demo.util.FSMMaze.Direction.WEST:S.currentLocation.left=Math.round(S.currentLocation.left-T);S.mouseImage.setLayoutProperties({left:S.currentLocation.left,top:S.currentLocation.top});break;case demobrowser.demo.util.FSMMaze.Direction.SOUTH:S.currentLocation.top=Math.round(S.currentLocation.top+T);S.mouseImage.setLayoutProperties({left:S.currentLocation.left,top:S.currentLocation.top});break;case demobrowser.demo.util.FSMMaze.Direction.EAST:S.currentLocation.left=Math.round(S.currentLocation.left+T);S.mouseImage.setLayoutProperties({left:S.currentLocation.left,top:S.currentLocation.top});break;case demobrowser.demo.util.FSMMaze.Direction.NORTH:S.currentLocation.top=Math.round(S.currentLocation.top-T);S.mouseImage.setLayoutProperties({left:S.currentLocation.left,top:S.currentLocation.top});break;};}
});C.addTransition(D);var D=new qx.util.fsm.Transition(a,{"nextState":o,"ontransition":function(U,event){U.currentCell=U.destinationCell;U.traversed.push(U.currentCell);}
});C.addTransition(D);var C=new qx.util.fsm.State(t,{"onentry":function(V,W){qx.event.Timer.once(function(){A.remove(this.mouseImage);this.mouseImage.dispose();this.mouseImage=null;this.dispose();}
,V,0);}
,"events":{}});this.addState(C);this.start();}
,members:{opacity:0.0,oneshot:function(X){var Y=new qx.event.Timer(X);Y.addListener(e,function(){Y.dispose();var event=new qx.event.type.Event();event.setType(w);this.eventListener(event);}
,this);Y.start();}
}});}
)();
