...
(function(){var p="failed",o="error",n="loaded",m="select * from feed where url='",l="'",k="timeout",j="feedreader.io.FeedLoader",h="loading",g="changeState",f="aborted",d="singleton";
qx.Class.define(j,{extend:qx.core.Object,type:d,members:{loadAll:function(r){var t=r.getFeeds().getItem(0).getFeeds();

for(var i=0;i<t.length;i++){this.load(t.getItem(i));
}var s=r.getFeeds().getItem(1).getFeeds();

for(i=0;i<s.length;i++){this.load(s.getItem(i));
}},load:function(a){a.setState(h);
var b=m+a.getUrl()+l;
var c=new qx.data.store.Yql(b,{manipulateData:function(q){if(q.query.results){q=q.query.results.item||q.query.results.entry;
for(var i=0;i<q.length;i++){if(!qx.lang.Type.isString(q[i].title)){q[i].title=q[i].title.content;
}}return q;
}return p;
}},qx.bom.client.Feature.SSL);
c.addListener(n,this.__mF(a),this);
c.addListener(g,qx.lang.Function.bind(this.__mE,this,a),this);
},__mE:function(u,e){if(e.getData()==f||e.getData()==k||e.getData()==p){u.setState(o);
}},__mF:function(w){return function(e){var v=e.getData();
if(v==p){w.setState(o);
return;
}w.setArticles(v);
w.setState(n);
...
(function(){var D="static",C="user",B="execute",A="loading",z="selection[0]",y="title",x="north",w="main",v="new",u="Surfin' Safari",bK="changeModel",bJ="__pD",bI="__pp",bH="selection[0].state",bG="Heise",bF="__px",bE="F1",bD="article",bC="partLoaded",bB="Control+A",K="http://blogs.msdn.com/ie/rss.xml",L="__pA",I="http://my.opera.com/desktopteam/xml/rss/blog/",J="settings",G="feedreader/images/loading22.gif",H="error",E="change",F="http://blogs.msdn.com/jscript/rss.xml",S="__pz",T="loaded",bg="feeds",bc="__pt",bo="__pB",bj="The Mozilla Blog",bx="enabled",bt="http://webkit.org/blog/feed/",X="separator-vertical",bA="http://feeds2.feedburner.com/qooxdoo/news/content",bz="http://blog.mozilla.com/feed/",by="__ps",W="vertical",ba="http://www.heise.de/newsticker/heise-atom.xml",bb="__pu",be="state",bh="JScript Team Blog",bk="Feeds",bq="http://feeds2.feedburner.com/ajaxian",bv="Control+D",M="icon/22/actions/process-stop.png",N="selection[0].articles",Y="white",bn="IEBlog",bm="Ajaxian",bl="User Feeds",bs="icon/22/apps/internet-feed-reader.png",br="model",bi="feedreader.Application",bp="http://daringfireball.net/index.xml",r="Control+P",bu="__pv",O="Static Feeds",P="Control+R",bd="qooxdoo News",s="__pC",t="__py",V="Opera Desktop Blog",Q="feedreader.model.Feed",R="Daring Fireball",U="__pq",bf="__pw",bw="addfeed";
qx.Class.define(bi,{extend:qx.application.Standalone,members:{__pp:null,__pq:null,__pr:null,__ps:null,__pt:null,__pu:null,__pv:null,__pw:null,__px:null,__py:null,__pz:null,__pA:null,__pB:null,__pC:null,__pD:null,main:function(){qx.application.Standalone.prototype.main.call(this);
{};
qx.io.PartLoader.getInstance().addListener(bC,function(e){this.debug("part loaded: "+e.getData().getName());
},this);
var bS=qx.locale.Manager.getInstance().getLanguage();
var bR=qx.Part.getInstance().getParts();
if(bR[bS]){qx.io.PartLoader.require([bS],function(){qx.locale.Manager.getInstance().setLocale(bS);
this.buildUpGui();
},this);
}else{this.warn("Cannot load locale part for current language "+bS+", falling back to English.");
this.buildUpGui();
}},buildUpGui:function(){this._initializeCommands();
this._createLayout();
this._initializeModel();
this._setUpBinding();
this.__ps.getRoot().setOpen(true);
this.__ps.getRoot().getChildren()[0].setOpen(true);
this.__ps.getRoot().getChildren()[1].setOpen(true);
this.__ps.setHideRoot(true);
this.__pp.getSelection().push(this.__pD.getFeeds().getItem(0));
this.reload();
},_initializeModel:function(){this.__pB=new feedreader.model.FeedFolder(bk);
this.__pD=new feedreader.model.FeedFolder(this.tr(O));
this.__pB.getFeeds().push(this.__pD);
this.__pD.getFeeds().push(new feedreader.model.Feed(bd,bA,D));
this.__pD.getFeeds().push(new feedreader.model.Feed(bh,F,D));
this.__pD.getFeeds().push(new feedreader.model.Feed(R,bp,D));
this.__pD.getFeeds().push(new feedreader.model.Feed(u,bt,D));
this.__pD.getFeeds().push(new feedreader.model.Feed(bm,bq,D));
this.__pC=new feedreader.model.FeedFolder(this.tr(bl));
this.__pB.getFeeds().push(this.__pC);
this.__pC.getFeeds().push(new feedreader.model.Feed(bG,ba,C));
this.__pC.getFeeds().push(new feedreader.model.Feed(bn,K,C));
this.__pC.getFeeds().push(new feedreader.model.Feed(bj,bz,C));
this.__pC.getFeeds().push(new feedreader.model.Feed(V,I,C));
},_setUpBinding:function(){var j={converter:this._state2iconConverter};
this.__pp=new qx.data.controller.Tree(this.__pB,this.__ps,bg,y);
this.__pp.setIconOptions(j);
this.__pp.setIconPath(be);
this.__pq=new qx.data.controller.List(null,this.__pv.getList(),y);
this.__pp.bind(N,this.__pq,br);
this.__pq.bind(z,this.__pz,bD);
this.__pq.getSelection().addListener(E,this._listControllerChange,this);
this.__pq.addListener(bK,this._treeControllerChange,this);
var k={converter:this._state2loadingConverter};
this.__pp.bind(bH,this.__pv,A,k);
k={converter:this._category2enabledConverter};
this.__pp.bind(z,this.__pu.getRemoveButton(),bx,k);
},_category2enabledConverter:function(bQ){return !!bQ&&bQ.getCategory()==C;
},_state2iconConverter:function(m){if(m==v||m==A){return G;
}else if(m==T){return bs;
}else if(m==H){return M;
}return null;
},_state2loadingConverter:function(l){if(l==v||l==A){return true;
}return false;
},_listControllerChange:function(d){var f=this.__pp.getSelection().getItem(0);
var g=this.__pq.getSelection().getItem(0);
if(g!=undefined){f.setSelectedArticle(g);
}},_treeControllerChange:function(a){var b=this.__pp.getSelection().getItem(0);
if(b!=null&&b.classname==Q&&b.getSelectedArticle()!=null&&b.getArticles().contains(b.getSelectedArticle())){this.__pq.getSelection().push(b.getSelectedArticle());
}else{this.__pv.getList().scrollToY(0);
}},_createLayout:function(){var p=new qx.ui.layout.Dock();
p.setSeparatorY(X);
var q=new qx.ui.container.Composite(p);
this.getRoot().add(q,{edge:0});
this.__pt=new feedreader.view.Header();
q.add(this.__pt,{edge:x});
this.__pu=new feedreader.view.ToolBar(this);
q.add(this.__pu,{edge:x});
this.__pw=new qx.ui.splitpane.Pane();
q.add(this.__pw);
this.__ps=new qx.ui.tree.Tree();
this.__ps.setWidth(250);
this.__ps.setBackgroundColor(Y);
this.__pw.add(this.__ps,0);
this.__px=new qx.ui.splitpane.Pane(W);
this.__px.setDecorator(null);
this.__pw.add(this.__px,1);
this.__pv=new feedreader.view.List(this.__pB);
this.__pv.setHeight(200);
this.__pv.setDecorator(w);
this.__px.add(this.__pv,0);
this.__pz=new feedreader.view.Article();
this.__pz.setDecorator(w);
this.__px.add(this.__pz,1);
},_initializeCommands:function(){var c={};
c.reload=new qx.ui.core.Command(P);
c.reload.addListener(B,this.reload,this);
c.about=new qx.ui.core.Command(bE);
c.about.addListener(B,this.showAbout,this);
c.preferences=new qx.ui.core.Command(r);
c.preferences.addListener(B,this.showPreferences,this);
c.addFeed=new qx.ui.core.Command(bB);
c.addFeed.addListener(B,this.showAddFeed,this);
c.removeFeed=new qx.ui.core.Command(bv);
c.removeFeed.addListener(B,this.removeFeed,this);
this.__pr=c;
},getCommand:function(bT){return this.__pr[bT];
},addFeed:function(bL,bM,bN){var bO=new feedreader.model.Feed(bL,bM,bN);
this.__pC.getFeeds().push(bO);
var bP=feedreader.io.FeedLoader.getInstance();
bP.load(bO);
},removeFeed:function(){var n=this.__pp.getSelection().getItem(0);
if(n&&n.getCategory&&n.getCategory()!==D){var o=this.__pC.getFeeds();
for(var i=0;i<o.length;i++){if(n===o.getItem(i)){o.splice(i,1);
return;
}}}},reload:function(){var h=feedreader.io.FeedLoader.getInstance();
h.loadAll(this.__pB);
},showPreferences:function(){qx.io.PartLoader.require([J],function(){if(!this.__py){this.__py=new feedreader.view.PreferenceWindow();
this.getRoot().add(this.__py);
}this.__py.center();
this.__py.open();
},this);
},showAbout:function(){alert(this.tr("FeedReader (qooxdoo powered)"));
},showAddFeed:function(){qx.io.PartLoader.require([bw],function(){if(!this.__pA){this.__pA=new feedreader.view.AddFeedWindow(this);
this.getRoot().add(this.__pA);
}this.__pA.center();
this.__pA.open();
},this);
}},destruct:function(){this.__pr=null;
this._disposeObjects(bb,bu,S,by,bo,bf,bF,bc,bJ,s,bI,U,t,L);
}});
})();
...
B.add(A);
},members:{__qa:null,getRemoveButton:function(){return this.__qa;
...
(function(){var n="separator-vertical",m="background-medium",l="Posts",k="bold",j="_applyLoading",i="center",h="feedreader.view.List",g="__qz",f="single",e="__qA",b="middle",d="feedreader/images/loading66.gif",c="__qy",a="Boolean";
qx.Class.define(h,{extend:qx.ui.core.Widget,construct:function(){qx.ui.core.Widget.call(this);
var p=new qx.ui.layout.VBox();
p.setSeparator(n);
this._setLayout(p);
var q=new qx.ui.basic.Label(this.tr(l));
q.setBackgroundColor(m);
q.setPadding(5);
q.setAllowGrowX(true);
q.setFont(k);
this._add(q);
this.__qy=new qx.ui.container.Stack();
this._add(this.__qy,{flex:1});
this.__qz=new qx.ui.form.List();
this.__qz.setDecorator(null);
this.__qz.setSelectionMode(f);
this.__qy.add(this.__qz);
this.__qA=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,i));
var o=new qx.ui.basic.Image(d);
o.setAlignY(b);
this.__qA.add(o);
this.__qy.add(this.__qA);
},properties:{loading:{check:a,init:false,apply:j}},members:{__qy:null,__qz:null,__qA:null,_applyLoading:function(r,s){if(r){this.__qy.setSelection([this.__qA]);
}else{this.__qy.setSelection([this.__qz]);
}},getList:function(){return this.__qz;
}},destruct:function(){this._disposeObjects(g,c,e);
}});
})();
...
if(!feedreader.view.Article.__qX){feedreader.view.Article.__qX=true;
qx.bom.Stylesheet.includeFile(l);
}this.setCssClass(h);
this.setOverflowY(a);
this.setBackgroundColor(p);
var B=new qx.bom.Font(14,[g,s,r]);
B.setLineHeight(1.8);
this.setFont(B);
},properties:{article:{apply:e,nullable:true,check:d}},members:{_applyArticle:function(w,x){var y=this.__qY();
this.setHtml(y);
qx.html.Element.flush();
var A=this.getContentElement().getDomElement();
var z=A.getElementsByTagName(b);
for(var i=0;i<z.length;i++){z[i].target=t;
}},__qY:function(){var D=this.getArticle();
if(!D){return v;
}var E=new qx.util.StringBuilder();
var C=(D.getPubDate&&D.getPubDate())||v;
var G=v;

if(D.getDescription){G=D.getDescription();
}else if(D.getContent){G=D.getContent().getContent&&D.getContent().getContent();
}var F=D.getLink();

if(F.getHref){F=F.getHref();
}else if(F instanceof qx.data.Array){F=F.getItem(0).getHref();
}E.add(c);
E.add(f,D.getTitle(),j);
E.add(n,C,u);
E.add(q,G,u);
E.add(o,F,k);
E.add(u);
