...
qx.Class.define(k,{extend:qx.application.Standalone,members:{__Nz:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var F=this.getRoot();
this.__Nz=[n,p,q,l,o,b,d,m];
F.add(this.createTestField(n),{left:20,top:20});
F.add(this.createTestField(p),{right:20,top:20});
F.add(this.createTestField(q),{right:20,bottom:20});
F.add(this.createTestField(l),{left:20,bottom:20});
F.add(this.createTestField(o),{left:20,top:250});
F.add(this.createTestField(m),{right:20,top:250});
...
A.add(new qx.ui.basic.Atom(g));
A.setPadding(20);
var z=new qx.ui.form.SelectBox;
var D=null;

for(var i=0;i<this.__Nz.length;i++){var B=new qx.ui.form.ListItem(this.__Nz[i]);
z.add(B);

if(this.__Nz[i]==y){D=B;
}}C.add(z);
z.addListener(r,function(e){A.setPosition(e.getData()[0].getLabel());
});
z.setSelection([D]);
var E=new qx.ui.form.Button(h);
C.add(E);
E.addListener(s,function(e){A.placeToWidget(E);
A.show();
});
return C;
},createDomClick:function(){var v=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));
var x=new qx.ui.basic.Label(a);
v.add(x);
var u=new qx.ui.popup.Popup(new qx.ui.layout.Grow);
u.add(new qx.ui.basic.Atom(c));
u.setPadding(20);
var t=new qx.ui.form.SelectBox;

for(var i=0;i<this.__Nz.length;i++){t.add(new qx.ui.form.ListItem(this.__Nz[i]));
}v.add(t);
t.addListener(r,function(e){u.setPosition(e.getData()[0].getLabel());
});
var w=new qx.ui.form.Button(j);
v.add(w);
w.addListener(s,function(e){u.placeToElement(document.getElementById(f));
u.show();
});
return v;
