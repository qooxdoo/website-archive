...
var T=new qx.ui.treevirtual.TreeVirtual([m,g,I]);
T.set({width:400});
T.setAlwaysShowOpenCloseSymbol(true);
var S=T.getTableColumnModel().getBehavior();
S.set(0,{width:L,minWidth:180});
ba.add(T);
var Y=T.getDataModel();
var V=Y.addBranch(null,b,true);
T.nodeSetLabelStyle(V,u+O+N);
var R;
Y.addBranch(V,F,true);
R=Y.addBranch(V,r,true);
var X=Y.addLeaf(R,d);
Y.setColumnData(X,1,h);
Y.setColumnData(X,2,k);
X=Y.addLeaf(R,f);
Y.setColumnData(X,1,h);
Y.setColumnData(X,2,k);
Y.addBranch(V,K,true);
X=Y.addBranch(V,j,true);
T.nodeSetCellStyle(X,v);
var W=Y.addBranch(null,c,true);
X=Y.addBranch(W,n,false);

for(var i=1;i<3000;i++){Y.addLeaf(X,C+i);
}Y.addBranch(W,G,false);
Y.addBranch(W,j,false);
Y.addBranch(W,w,false);
Y.addBranch(W,E,false);
Y.setData();
var U=new qx.ui.groupbox.GroupBox(J);
...
T.addListener(H,function(e){var bc=e.getData();
this.setValue(T.getHierarchy(bc[0].nodeId).join(B));
Q.setEnabled(true);
},o);
var Q=new qx.ui.form.Button(p);
Q.set({enabled:false});
U.add(Q,{top:50,left:0});
Q.addListener(a,function(e){var P=T.getSelectedNodes();

for(var i=0;i<P.length;i++){Y.prune(P[i].nodeId,true);
Y.setData();
}});
o=new qx.ui.form.CheckBox(x);
o.set({value:true});
U.add(o,{top:80,left:0});
o.addListener(l,function(e){T.setUseTreeLines(e.getData());
});
o=new qx.ui.form.CheckBox(D);
o.set({value:false});
U.add(o,{top:100,left:0});
o.addListener(l,function(e){T.setExcludeFirstLevelTreeLines(e.getData());
});
o=new qx.ui.form.CheckBox(z);
o.set({value:true});
U.add(o,{top:120,left:0});
o.addListener(l,function(e){T.setAlwaysShowOpenCloseSymbol(e.getData());
});
o=new qx.ui.form.CheckBox(A);
o.set({value:true});
U.add(o,{top:140,left:0});
T.addListener(q,function(e){if(this.getValue()){var bb=e.getData();
T.nodeSetHideOpenClose(bb,true);
}},o);
o=new qx.ui.form.CheckBox(s);
o.set({value:false});
U.add(o,{top:160,left:0});
o.addListener(l,function(e){T.setOpenCloseClickSelectsRow(e.getData());
});
o=new qx.ui.form.CheckBox(M);
o.set({value:false});
U.add(o,{top:180,left:0});
o.addListener(l,function(e){T.setEnabled(!e.getData());
});
}}});
})();
