...
}qx.bom.Element.addListener(F,l,this.__LB,this);
}var D=document.getElementById(a);
D.checked=false;
qx.bom.Element.addListener(D,l,function(e){var H=e.getTarget().checked;

if(H){qx.bom.Element.capture(C);
}else{qx.bom.Element.releaseCapture(C);
}},this);
qx.bom.Element.addListener(C,o,function(e){D.checked=false;
},this);
},__LB:function(e){var K=e.getTarget().id.split("_")[1];
var I=e.getTarget().checked;
var J=document.getElementById("mouse");
...
}},logMouseEvent:function(G){G.preventDefault();
this._log([G.getTarget().id,G.getType(),G.getButton(),G.getDocumentLeft(),G.getDocumentTop(),G.getScreenLeft(),G.getScreenTop(),G.getViewportLeft(),G.getViewportTop(),G.getRelatedTarget()?G.getRelatedTarget().id:x]);
}}});
})();
