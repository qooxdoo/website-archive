...
(function(){var k="value",j='undefined',i="",h="model",g="application/x-googlegears",f="demobrowser.demo.data.store.Gears",d="<span style='font-size: 20px'>google Gears</span>",c="Age: ",b="http://code.google.com/apis/gears/install.html",a="lastname",C="firstname",B="age",A="demobrowser.demo.data.Gears",z="First Name: ",y='qooxdoo',x='Gears.Factory',w='select * from Persons',v="delete from Persons",u="LastName: ",t=' (Firstname varchar(255), Lastname varchar(255), Age int)',r="Person",s='ie_mobile',p="none",q='create table if not exists Persons',n="This demo requires Gears to be installed. Install now?",o='insert into Persons values (?, ?, ?)',l='beta.database',m="object";
...
var S=new qx.ui.groupbox.GroupBox(r);
this.getRoot().add(S,{left:10,top:50});
S.setLayout(new qx.ui.layout.Grid(0,3));
S.add(new qx.ui.basic.Label(z),{row:0,column:0});
S.add(new qx.ui.basic.Label(u),{row:1,column:0});
S.add(new qx.ui.basic.Label(c),{row:2,column:0});
...
Q.addTarget(M,k,C,true);
Q.addTarget(O,k,a,true);
Q.addTarget(P,k,B,true);
...
qx.Class.define(f,{extend:qx.data.store.Json,construct:function(){qx.data.store.Json.call(this);
this.initGears();
if(!window.google||!google.gears){if(confirm(n)){location.href=b;
return;
}return;
}this.__MK=this.initDB();
this.__ML();
},members:{__ML:function(G){var H=this.getPerson();
this._marshaler.toClass(H,true);
this.setModel(this._marshaler.toModel(H));
this.getModel().addListener("changeBubble",this.__MM,this);
this.fireDataEvent("loaded",this.getModel());
},__MM:function(D){var E=this.getModel();
this.update(E.getFirstname(),E.getLastname(),E.getAge());
},update:function(I,J,K){if(!google.gears.factory||!this.__MK){return;
}this.__MK.execute(v);
this.__MK.execute(o,[I,J,K]);
},getPerson:function(){var T={};
T.firstname=i;
T.lastname=i;
T.age=0;

if(!google.gears.factory||!this.__MK){return T;
}var U=this.__MK.execute(w);

if(U.isValidRow()){T.firstname=U.field(0);
T.lastname=U.field(1);
T.age=parseInt(U.field(2));
}U.close();
return T;
},initDB:function(){if(window.google&&google.gears){try{var L=google.gears.factory.create(l);

if(L){L.open(y);
L.execute(q+t);
}}catch(F){this.error('Could not create database: '+F.message);
}}return L;
},initGears:function(){if(window.google&&google.gears){return;
}var V=null;
if(typeof GearsFactory!=j){V=new GearsFactory();
}else{try{V=new ActiveXObject(x);
if(V.getBuildInfo().indexOf(s)!=-1){V.privateSetGlobalObject(this);
}}catch(e){if((typeof navigator.mimeTypes!=j)&&navigator.mimeTypes[g]){V=document.createElement(m);
V.style.display=p;
V.width=0;
V.height=0;
V.type=g;
...
if(!google.gears){google.gears={factory:V};
}}}});
})();
