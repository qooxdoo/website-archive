...
var E=this._tableModel=new qx.ui.table.model.Simple();
E.setColumns([n,d,j,i]);
E.setData(F);
...
G.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var D=G.getTableColumnModel();
D.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
D.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(c,j));
return G;
},createControls:function(){var N=new qx.ui.toolbar.ToolBar();
var P,R;
R=new qx.ui.toolbar.Part();
N.add(R);
var M=new qx.ui.form.SelectBox().set({allowGrowY:false,alignY:z});
var U=qx.ui.table.selection.Model;
var O={"Single selection":U.SINGLE_SELECTION,"Single interval":U.SINGLE_INTERVAL_SELECTION,"Multiple intervals":U.MULTIPLE_INTERVAL_SELECTION,"Multiple intervals (toggle)":U.MULTIPLE_INTERVAL_SELECTION_TOGGLE,"No selection":U.NO_SELECTION};

for(var T in O){var Q=new qx.ui.form.ListItem(T);
Q.setUserData(l,O[T]+k);
M.add(Q);
}M.addListener(A,function(e){this._table.getSelectionModel().setSelectionMode(parseInt(e.getData()[0].getUserData(l)));
},this);
R.add(M);
R=new qx.ui.toolbar.Part();
N.add(R);
P=new qx.ui.toolbar.Button(u,q);
P.addListener(m,function(a){var b=this.createRandomRows(10);
this._tableModel.addRows(b);
this.info("5 rows added");
},this);
R.add(P);
P=new qx.ui.toolbar.Button(y,t);
P.addListener(m,function(V){var W=this._tableModel.getRowCount();
this._tableModel.removeRows(W-5,5);
this.info("5 rows removed");
},this);
R.add(P);
R=new qx.ui.toolbar.Part();
N.add(R);
P=new qx.ui.toolbar.Button(x);
P.addListener(m,function(C){this._table.resetSelection();
},this);
R.add(P);
P=new qx.ui.toolbar.Button(B);
P.addListener(m,function(I){this._table.resetCellFocus();
},this);
R.add(P);
var S=new qx.ui.toolbar.CheckBox(r);
S.addListener(g,function(L){this._table.setShowCellFocusIndicator(!S.isValue());
},this);
R.add(S);
P=new qx.ui.toolbar.Button(p,f);
P.addListener(m,function(J){var K=[];
this._table.getSelectionModel().iterateSelection(function(H){K.push(H+k);
});
this.showDialog(o+K.join(s));
},this);
R.add(P);
return N;
