...
var O=this._eventsWin=new qx.ui.window.Window(l).set({height:400,width:290,showClose:false,showMinimize:false,contentPadding:0});
O.setLayout(new qx.ui.layout.Canvas());
O.open();
this.getRoot().add(O,{left:670,top:10});
this._events=new qx.ui.table.model.Simple();
this._events.setColumns([q,w,d]);
this._events.setData([]);
var N=new qx.ui.table.Table(this._events);
N.getTableColumnModel().setColumnWidth(0,150);
N.getTableColumnModel().setColumnWidth(1,60);
N.getTableColumnModel().setColumnWidth(2,60);
O.add(N,{edge:0});
},createTable:function(){var X=this.createRandomRows(50);
var T=this._tableModel=new qx.ui.table.model.Simple();
T.setColumns([x,g,v,i]);
T.setData(X);
T.setColumnEditable(1,true);
T.setColumnEditable(2,true);
var Y=new qx.ui.table.Table(T);
Y.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
var U=Y.getTableColumnModel();
U.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
var ba=function(e){this._events.addRows([[e.getType(),k,k,k]],0);
};
var W=function(e){this._events.addRows([[e.getType(),e.getRow(),e.getColumn()]],0);
};
var V=function(e){var P=e.getData();
this._events.addRows([[e.getType(),P.ascending?B:n,P.columnIndex]],0);
};
Y.addListener(z,ba,this);
Y.addListener(j,ba,this);
Y.addListener(b,ba,this);
Y.addListener(a,ba,this);
Y.addListener(m,W,this);
Y.addListener(r,W,this);
Y.addListener(G,W,this);
T.addListener(c,V,this);
T.addListener(t,ba,this);
Y.setContextMenu(this.getContextMenu());
return Y;
},createControls:function(){var S=new qx.ui.toolbar.ToolBar();
var Q,R;
R=new qx.ui.toolbar.Part();
S.add(R);
Q=new qx.ui.toolbar.CheckBox(y,H).set({value:true});
R.add(Q);
Q.addListener(f,function(e){if(Q.isValue()){this._events.setData([]);
this._eventsWin.open();
}else{this._eventsWin.close();
}},this);
return S;
},getContextMenu:function(){var J=new qx.ui.menu.Menu();
var L=new qx.ui.menu.Button(E,h);
var M=new qx.ui.menu.Button(p,I);
var K=new qx.ui.menu.Button(o,A);
J.add(L);
J.add(M);
J.add(K);
return J;
