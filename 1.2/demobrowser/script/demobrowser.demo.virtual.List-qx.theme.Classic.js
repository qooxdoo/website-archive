...
(function(){var bu="value",bt="label",bs="icon",br="one",bq="multi",bp="additive",bo="bold",bn="both",bm="single",bl="changeValue",ba="changeSelection",Y="Item No ",X="Julian",W="Show Icon",V="Kevin",U="demobrowser.demo.virtual.List",T="Marvin",S="Marc",R="Configurable",Q="icon/16/places/folder.png",bB="Jason",bC="show",bz="Jakob",bA="Lukas",bx="Julius",by="Jan",bv="Tobias",bw="Mika",bD="Nils",bE="Noah",be="Tom",bd="Nico",bg="Multi Selection",bf="Felix",bi="Leon",bh="Enable drag selection",bk="Show Both",bj="48",bc="changeShowMode",bb="Fabian",b="Joel",c="Dominic",d="Show Label",f="Enable quick selection",g="Finn",h="One as selection mode",j="Jannis",k="Alexander",l="Additive Selection",m="change",bI="Erik",bH="addaptive",bG="Johannes",bF="16",bM="Jonas",bL="icon/",bK="David",bJ="Maximilian",bO="enabled",bN="Tim",B="Justin",C="Single Selection",z="Linus",A="Daniel",F="Paul",G="Philipp",D="Niclas",E="One Selection",x="Nick",y="Jonathan",u="Ben",t="on",w="showMode",v="Vincent",q="/places/folder.png",p="Robin",s="Jannick",r="Luis",o="Lennard",n="Moritz",L="Florian",M="",N="Marlon",O="Luca",H="Simon",I="Colin",J="Max",K="Additive selection",P="Elias";
qx.Class.define(U,{extend:qx.application.Standalone,properties:{showMode:{init:bn,check:[bt,bs,bn],event:bc}},members:{__Mx:null,__My:null,__Mz:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var bY=this.getRoot();
bY.add(this.createConfigurableList(),{left:20,top:20});
bY.add(this.createOneSelectionList(),{left:330,top:20});
bY.add(this.createAdditiveSelectionList(),{left:520,top:20});
},createConfigurableList:function(){var ci=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
var ck=new qx.ui.basic.Label(R).set({font:bo});
ci.add(ck);
var cs=[];

for(var i=0;i<2500;i++){cs[i]={label:Y+i,icon:(i%4)?bF:bj};
}var cv=qx.data.marshal.Json.createModel(cs);
var cn=this.__Mx=new qx.ui.list.List(cv).set({scrollbarX:t,selectionMode:bq,height:280,width:150,labelPath:bt,iconPath:bs,iconOptions:{converter:function(cB){return bL+cB+q;
}}});
var cm=this;
var cy={configureItem:function(bP){cm.bind(w,bP,bC);
},bindItem:function(cD,cE,cF){cD.bindDefaultProperties(cE,cF);
cD.bindProperty(bt,bO,{converter:function(cA){return parseInt(cA.replace(/Item No /g,M))%9!=0;
}},cE,cF);
}};
cn.setDelegate(cy);
ci.add(cn,{top:20});
cn.getSelection().push(cv.getItem(20));
cn.getSelection().addListener(m,function(e){var cz=cn.getSelection();

for(var i=0;i<cz.getLength();i++){this.debug("Selection: "+cz.getItem(i).getLabel());
}},this);
var cq=cn.getPane().getRowConfig();

for(var i=0;i<2500;i++){if(i%4==0){cq.setItemSize(i,56);
}else{cq.setItemSize(i,25);
}}var cr=new qx.ui.form.RadioButton(C);
var ch=new qx.ui.form.RadioButton(bg);
var cu=new qx.ui.form.RadioButton(l);
var cw=new qx.ui.form.RadioButton(E);
cr.setUserData(bu,bm);
ch.setUserData(bu,bq);
cu.setUserData(bu,bp);
cw.setUserData(bu,br);
ci.add(cr,{left:160,top:20});
ci.add(ch,{left:160,top:40});
ci.add(cu,{left:160,top:60});
ci.add(cw,{left:160,top:80});
var co=new qx.ui.form.RadioButton(d);
var cj=new qx.ui.form.RadioButton(W);
var cg=new qx.ui.form.RadioButton(bk);
co.setUserData(bu,bt);
cj.setUserData(bu,bs);
cg.setUserData(bu,bn);
ci.add(co,{left:160,top:120});
ci.add(cj,{left:160,top:140});
ci.add(cg,{left:160,top:160});
var cp=this.__My=new qx.ui.form.CheckBox(bh);
var ct=this.__Mz=new qx.ui.form.CheckBox(f).set({enabled:false});
ci.add(cp,{left:160,top:200});
ci.add(ct,{left:160,top:220});
var cx=new qx.ui.form.RadioGroup(cr,ch,cu,cw);
cx.setSelection([ch]);
cx.addListener(ba,this.onSelectionModeChange,this);
var cl=new qx.ui.form.RadioGroup(co,cj,cg);
cl.setSelection([cg]);
cl.addListener(ba,this.onShowModeChange,this);
cp.addListener(bl,this.onDragCheckChange,this);
ct.addListener(bl,this.onQuickCheckChange,this);
return ci;
},createOneSelectionList:function(){var ca=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
var cb=new qx.ui.basic.Label(h).set({font:bo});
ca.add(cb);
var ce=new qx.data.Array();

for(var i=0;i<250;i++){var cd=new demobrowser.demo.virtual.model.Item(Y+i,Q);
ce.push(cd);
}var cc=new qx.ui.list.List(ce).set({selectionMode:br,height:280,width:150,labelPath:bt,iconPath:bs});
ca.add(cc,{top:20});
cc.getSelection().push(ce.getItem(16));
return ca;
},createAdditiveSelectionList:function(){var bR=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
var bS=new qx.ui.basic.Label(K).set({font:bo});
bR.add(bS);
var bT=[bi,bA,O,g,bN,bf,bM,r,bJ,X,J,F,D,by,u,P,s,G,bE,be,n,bd,bK,bD,H,bb,bI,B,k,bz,L,x,z,bw,bB,A,o,T,j,bv,c,N,S,bG,y,bx,I,b,V,v,p];
var bW=new qx.data.Array();
var bQ=new qx.data.Array();

for(var i=0;i<bT.length;i++){var bV=new demobrowser.demo.virtual.model.Item(bT[i]);
bW.push(bV);

if(i==10||i==12||i==16){bQ.push(bV);
}}var bU=new qx.ui.list.List(bW).set({selectionMode:bp,height:200,width:150,labelPath:bt,iconPath:bs});
bR.add(bU,{top:20});
bU.setSelection(bQ);
return bR;
},onSelectionModeChange:function(e){var bX=e.getData()[0].getUserData(bu);
this.__Mx.setSelectionMode(bX);

if(bX==bm||bX==br){this.__My.setEnabled(false);
this.__Mz.setEnabled(true);
}else if(bX==bq||bX==bH){this.__My.setEnabled(true);
this.__Mz.setEnabled(false);
}},onShowModeChange:function(e){var cf=e.getData()[0].getUserData(bu);
this.setShowMode(cf);
},onDragCheckChange:function(e){if(e.getData()){var a=this.__Mx.getSelectionMode();

if(a==bm||a==br){this.debug("Drag selection is only available for the modes multi or additive");
}}this.__Mx.setDragSelection(e.getData());
},onQuickCheckChange:function(e){if(e.getData()){var cC=this.__Mx.getSelectionMode();

if(cC==bq||cC==bp){this.debug("Quick selection is only available for the modes multi or additive");
}}this.__Mx.setQuickSelection(e.getData());
}}});
})();
