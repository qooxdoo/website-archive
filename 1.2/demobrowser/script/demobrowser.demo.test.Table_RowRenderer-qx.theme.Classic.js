...
},testDefaultRenderer:function(y){var y=new qx.ui.table.rowrenderer.Default();
var C=300;
var B=20;
var A={focusedRow:[true,false],selected:[true,false],row:[1,2],table:[this.tableMock]};
var z=this._getNewTableDiv(C);
var self=this;
qx.util.Permutation.permute(A,function(s){var content=[];

for(var w in s){if(w!==m){content.push(w+a+s[w]);
}}content=content.join(d);
var x=self._createRowElement(z,C,B);
var t=y.getRowClass(s);

if(t){x.className=t;
}x.innerHTML=content+j;
y.updateDataRowElement(s,x);
var v=[];
v.push(g);

if(t){v.push(k,t,q);
}var u=y.createRowStyle(s);
u+=n+B+b+C+f;

if(u){v.push(h,u,q);
}v.push(p,content,l);
z.innerHTML+=v.join(o);
self._rowTop+=B;
});
