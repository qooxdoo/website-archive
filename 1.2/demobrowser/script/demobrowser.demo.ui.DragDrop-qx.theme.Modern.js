...
qx.Class.define(s,{extend:qx.application.Standalone,members:{__Nw:null,__Nx:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var P=this.getRoot();
...
J.add(H);
P.add(J,{edge:0});
var O=new qx.ui.basic.Label(t);
H.add(O,{left:20,top:20});
var M=new qx.ui.form.List;
M.setDraggable(true);
M.setSelectionMode(g);
H.add(M,{left:20,top:40});

for(var i=0;i<20;i++){M.add(new qx.ui.form.ListItem(b+i,d));
}var Q=new qx.ui.form.CheckBox(p);
Q.setValue(true);
H.add(Q,{left:20,top:260});
M.addListener(c,function(e){if(!Q.isValue()){e.preventDefault();
}e.addType(k);
e.addType(j);
e.addAction(a);
e.addAction(h);
});
M.addListener(v,function(e){this.debug("Related of droprequest: "+e.getRelatedTarget());
var V=e.getCurrentAction();
var X=e.getCurrentType();
var U;

switch(X){case j:U=this.getSelection();
...
case k:U=this.getSelection()[0].getLabel();
break;
}if(V==h){var T=this.getSelection();

for(var i=0,l=T.length;i<l;i++){this.remove(T[i]);
}}e.addData(X,U);
});
var K=new qx.ui.basic.Label(r);
H.add(K,{left:140,top:20});
var I=new qx.ui.form.List;
I.setDroppable(true);
I.setSelectionMode(g);
H.add(I,{left:140,top:40});
I.addListener(m,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
var D=e.getData(j);

for(var i=0,l=D.length;i<l;i++){this.add(D[i]);
}});
I.addListener(f,function(e){if(!e.supportsType(j)){e.preventDefault();
}});
var K=new qx.ui.basic.Label(z);
H.add(K,{left:260,top:20});
var N=new qx.ui.form.TextArea;
N.setDroppable(true);
N.setHeight(100);
H.add(N,{left:260,top:40});
N.addListener(m,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
this.setValue(e.getData(k).replace(/,/g,q));
});
N.addListener(f,function(e){if(!e.supportsType(k)){e.preventDefault();
}});
var L=new qx.ui.basic.Label(x);
H.add(L,{left:500,top:20});
var F=this.__Nw=new qx.ui.form.List;
F.setDraggable(true);
F.setDroppable(true);
F.setSelectionMode(g);
H.add(F,{left:500,top:40});

for(var i=0;i<20;i++){F.add(new qx.ui.form.ListItem(b+i,d));
}var G=new qx.ui.core.Widget;
G.setDecorator(new qx.ui.decoration.Single().set({top:[1,y,u]}));
G.setHeight(0);
G.setOpacity(0.5);
G.setZIndex(100);
...
G.setDroppable(true);
this.getRoot().add(G);
F.addListener(c,function(e){e.addAction(h);
});
F.addListener(o,function(e){G.setDomPosition(-1000,-1000);
});
F.addListener(A,function(e){var S=e.getOriginalTarget();
if(S instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__Nx=S;
}
if(!qx.ui.core.Widget.contains(this,S)&&S!=G){return;
}var R=S.getContainerLocation();
G.setWidth(S.getBounds().width);
G.setDomPosition(R.left,R.top);
});
F.addListener(f,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
F.addListener(m,function(e){this.__Ny(e.getOriginalTarget());
},this);
G.addListener(m,function(e){this.__Ny(this.__Nx);
},this);
},__Ny:function(B){if(B.classname!=w){return;
}var C=this.__Nw.getSortedSelection();

for(var i=0,l=C.length;i<l;i++){this.__Nw.addBefore(C[i],B);
this.__Nw.addToSelection(C[i]);
