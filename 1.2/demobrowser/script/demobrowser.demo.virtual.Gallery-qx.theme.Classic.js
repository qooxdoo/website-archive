...
var bj=new demobrowser.demo.virtual.HtmlGallery(c);
bj.moveTo(400,50);
}}});
qx.Class.define(y,{extend:qx.ui.window.Window,type:x,construct:function(bD){qx.ui.window.Window.call(this,bD);
...
var bE=this._createScroller();
bE.set({scrollbarX:m,scrollbarY:M});
bE.getPane().addListener(G,this._onPaneResize,this);
this.add(bE);
this.manager=new qx.ui.virtual.selection.CellRectangle(bE.getPane(),this).set({mode:q,drag:true});
this.manager.attachMouseEvents();
this.manager.attachKeyEvents(bE);
},members:{getItemData:function(bS,bT){return this.items[bS*this.itemPerLine+bT];
},_createScroller:function(){},isItemSelectable:function(bR){return !!this.getItemData(bR.row,bR.column);
},styleSelectable:function(bk,bl,bm){},_onPaneResize:function(e){var bd=e.getTarget();
var bb=e.getData().width;
var bc=Math.floor(bb/this.itemWidth);

if(bc==this.itemsPerLine){return;
}this.itemPerLine=bc;
var be=Math.ceil(this.itemCount/bc);
bd.getColumnConfig().setItemCount(bc);
bd.getRowConfig().setItemCount(be);
},_generateItems:function(bF){var bI=[];
var bL=[s,o,r,F,K];
var bJ=qx.util.AliasManager.getInstance();
var bH=qx.util.ResourceManager.getInstance();

for(var i=0;i<bF;i++){var bM=u+bL[Math.floor(Math.random()*bL.length)];
var bG=bJ.resolve(bM);
var bK=bH.toUri(bG);
bI[i]={label:w+(i+1),icon:bM,resolvedIcon:bK};
}return bI;
}},destruct:function(){this.items=null;
this._disposeObjects(b);
}});
qx.Class.define(D,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var bf=new qx.ui.basic.Atom().set({iconPosition:n});
bf.getChildControl(d).set({padding:[0,4]});
bf.getChildControl(g).set({padding:4});
return bf;
},updateData:function(bn,bo){bn.set({icon:bo.icon,label:bo.label});
},updateStates:function(bN,bO){var bQ=bN.getChildControl(d);
var bP=bN.getChildControl(g);

if(bO.selected){bQ.setDecorator(k);
bQ.setTextColor(E);
bP.setDecorator(H);
}else{bQ.resetDecorator();
bQ.resetTextColor();
bP.resetDecorator();
}}}});
qx.Class.define(O,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bq){demobrowser.demo.virtual.AbstractGallery.call(this,bq);
this.__Mg=new demobrowser.demo.virtual.GalleryCell();
},members:{_createScroller:function(){var Q=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.WidgetCell(this);
Q.getPane().addLayer(this.layer);
new qx.ui.virtual.behavior.Prefetch(Q,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:200,maxAbove:300,minBelow:600,maxBelow:800}).set({interval:500});
return Q;
},styleSelectable:function(br,bs,bt){if(bs!==k){return;
}var bu=this.layer.getChildren();

for(var i=0;i<bu.length;i++){var bv=bu[i];
var bw=bv.getUserData(j);

if(br.row!==bw.row||br.column!==bw.column){continue;
}
if(bt){this.__Mg.updateStates(bv,{selected:1});
}else{this.__Mg.updateStates(bv,{});
}}},getCellWidget:function(bx,by){var bC=this.getItemData(bx,by);

if(!bC){return null;
}var bz={row:bx,column:by};
var bA={};

if(this.manager.isItemSelected(bz)){bA.selected=true;
}var bB=this.__Mg.getCellWidget(bC,bA);
bB.setUserData(j,bz);
return bB;
},poolCellWidget:function(bp){this.__Mg.pool(bp);
}},destruct:function(){this._disposeObjects(p,h);
}});
qx.Class.define(z,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(Y){demobrowser.demo.virtual.AbstractGallery.call(this,Y);
var ba=qx.theme.manager.Font.getInstance().resolve(a).getStyles();
this._fontCss=qx.bom.element.Style.compile(ba);
},members:{_createScroller:function(){var X=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.HtmlCell(this);
X.getPane().addLayer(this.layer);
var W=new qx.ui.virtual.layer.GridLines(v,f);
X.getPane().addLayer(W);
var W=new qx.ui.virtual.layer.GridLines(t,f);
X.getPane().addLayer(W);
return X;
},_onPaneResize:function(e){demobrowser.demo.virtual.AbstractGallery.prototype._onPaneResize.call(this,e);
this.manager.clearSelection();
},styleSelectable:function(bg,bh,bi){this.layer.updateLayerData();
},getCellProperties:function(R,S){var V=this.getItemData(R,S);

if(!V){return l;
}var U=this.manager.isItemSelected({row:R,column:S});
var T=U?L:l;
return {style:[A,N,this._fontCss,T].join(l),content:[J,V.resolvedIcon,I,B,V.label].join(l)};
