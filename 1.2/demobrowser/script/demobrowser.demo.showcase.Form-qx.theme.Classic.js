...
var bg=new qx.ui.groupbox.GroupBox(z);
...
bm.setWidth(130);
bh=new qx.ui.basic.Label(v);
bh.setBuddy(bm);
br.add(bh,{row:1,column:0});
br.add(bm,{row:1,column:1});
},createItems:function(bB){for(var i=0;i<this.self(arguments).ITEM_SIZE;i++){var bC=new qx.ui.form.ListItem(y+i);
bB.add(bC);
}},createMenuForMenuButton:function(){var T=new qx.ui.menu.Menu;

for(var i=0;i<3;i++){T.add(new qx.ui.menu.RadioButton(t+i));
}var W=new qx.ui.form.RadioGroup;
W.add.apply(W,T.getChildren());
var V=new qx.ui.menu.Menu();

for(var i=0;i<3;i++){var X=new qx.ui.menu.Button(j+i);
V.add(X);
}var U=new qx.ui.menu.Button(N,q,null,T);
V.addSeparator();
V.add(U);
return V;
},createMenuForSplitButton:function(){var bx=new qx.ui.menu.Menu;
var bA=new qx.ui.menu.Button(G);
var by=new qx.ui.menu.Button(c);
var bz=new qx.ui.menu.Button(I);
bx.add(bA);
bx.add(by);
bx.add(bz);
return bx;
