...
T.setHtml(T.getHtml()+y+S.getGender()+j);
...
try{S.setName(p);
T.setHtml(T.getHtml()+i+S.getName()+j);
}catch(W){T.setHtml(T.getHtml()+E+P+S.getName()+j);
...
try{S.setAge(true);
T.setHtml(T.getHtml()+e+S.getAge()+j);
}catch(Q){T.setHtml(T.getHtml()+B+x+S.getAge()+j);
...
try{S.setAge(M);
T.setHtml(T.getHtml()+h+S.getGender()+j);
}catch(V){T.setHtml(T.getHtml()+t+G+S.getGender()+j);
}var U=new qx.ui.basic.Label();
U.setRich(true);
U.setWidth(250);
U.setValue(w+z+a+r+l);
this.getRoot().add(U,{left:280,top:10});
}}});
qx.Class.define(n,{extend:qx.core.Object,properties:{name:{init:v,validate:F},age:{init:18,validate:qx.util.Validate.number()},gender:{init:g,validate:qx.util.Validate.inArray([g,f])}},members:{__Lh:function(R){if(R.length<2){throw new qx.core.ValidationError(C+I+R+O);
}}}});
})();
