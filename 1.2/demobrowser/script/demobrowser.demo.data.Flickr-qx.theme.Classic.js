...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.data.Flickr","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.data.Flickr-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"c1ed89caa394"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var k="qooxdoo",j="/",i=".static.flickr.com/",h="http://api.flickr.com/services/rest/?tags=",g="",f="_",d="http://farm",c="Enter",b="keydown",a="model.photos.photo",D=".jpg",C="execute",B="source",A="selection[0]",z="_s.jpg",y="<span style='font-size: 20px'>Flickr</span>",x="loading",w="value",v="demobrowser.demo.data.store.Flickr",u="demobrowser.demo.data.Flickr",r="title",s="icon",p="state",q="Search",n="receiving",o="horizontal",l="model",m="&method=flickr.photos.search&api_key=63a8042eead205f7e0040f488c02afd9&format=json",t="id";
qx.Class.define(u,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);
var N=new demobrowser.demo.data.store.Flickr(k);
var F=new qx.ui.form.TextField(k);
this.getRoot().add(F,{left:30,top:50});
var G=new qx.ui.form.Button(q);
this.getRoot().add(G,{left:140,top:49});
G.addListener(C,function(){N.searchForTag(F.getValue());
},this);
F.addListener(b,function(e){if(e.getKeyIdentifier()==c){N.searchForTag(F.getValue());
}},this);
var status=new qx.ui.basic.Label(x);
...
J.setHeight(110);
J.setOrientation(o);
this.getRoot().add(J,{left:30,right:30,top:80});
var L=new qx.data.controller.List(null,J);
L.setLabelPath(r);
L.setDelegate({configureItem:function(O){O.setShow(s);
}});
var H={converter:function(V,W){return (d+W.getFarm()+i+W.getServer()+j+V+f+W.getSecret()+z);
}};
L.setIconOptions(H);
L.setIconPath(t);
N.bind(a,L,l);
var I=new qx.ui.basic.Image();
this.getRoot().add(I,{left:30,top:200});
var M={converter:function(P){if(P){return (d+P.getFarm()+i+P.getServer()+j+P.getId()+f+P.getSecret()+D);
}return g;
}};
L.bind(A,I,B,M);
var K=new qx.ui.basic.Label();
K.setRich(true);
K.setWidth(260);
K.setValue(y);
this.getRoot().add(K,{left:10,top:10});
}}});
qx.Class.define(v,{extend:qx.data.store.Json,statics:{RESULT:null},construct:function(T){jsonFlickrApi=function(S){demobrowser.demo.data.store.Flickr.RESULT=S;
};
var U=h+T;
qx.data.store.Json.call(this,U);
},members:{searchForTag:function(Y){if(Y!=g){this.setUrl(h+Y);
}},_createRequest:function(Q){var R=new qx.io.ScriptLoader();
this.setState(n);
Q+=m;
R.load(Q,function(E){this.__MP();
},this);
},__MP:function(){this.setState("completed");
var X=demobrowser.demo.data.store.Flickr.RESULT;

if(X==undefined){this.setState("failed");
return;
}this._marshaler.toClass(X);
