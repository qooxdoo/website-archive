...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.ui.FormValidator","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.ui.FormValidator-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"b2075fca18f2"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
x.setWidth(150);
x.setRequired(true);
this.getRoot().add(x,{left:20,top:70});
var G=new qx.ui.form.TextField();
G.setPlaceholder(c);
G.setWidth(150);
this.getRoot().add(G,{left:20,top:100});
var y=new qx.ui.form.TextField();
y.setPlaceholder(o);
y.setWidth(150);
this.getRoot().add(y,{left:20,top:130});
var z=new qx.ui.form.TextField();
z.setPlaceholder(o);
z.setWidth(150);
this.getRoot().add(z,{left:20,top:160});
var F=new qx.ui.form.CheckBox(d);
F.setRequired(true);
this.getRoot().add(F,{left:20,top:190});
var C=new qx.ui.form.validation.Manager();
var A=function(u,v){var w=u!=null&&u.length>2;

if(!w){v.setInvalidMessage(k);
}return w;
};
var D=new qx.ui.form.validation.AsyncValidator(function(p,q){window.setTimeout(function(){if(q==null||q.length==0){p.setValid(false,i);
}else{p.setValid(true);
}},1000);
});
C.add(x,D);
C.add(G,qx.util.Validate.email());
C.add(y,A);
C.add(z,A);
C.add(F);
C.setValidator(function(r){var t=y.getValue()==z.getValue();

if(!t){var s=h;
y.setInvalidMessage(s);
z.setInvalidMessage(s);
y.setValid(false);
z.setValid(false);
}return t;
});
var B=new qx.ui.form.Button(n);
this.getRoot().add(B,{left:20,top:215});
B.addListener(b,function(){B.setEnabled(false);
B.setLabel(f);
C.validate();
},this);
C.addListener(l,function(){B.setEnabled(true);
B.setLabel(n);
if(C.getValid()){alert("You can send...");
}else{alert(C.getInvalidMessages().join("\n"));
}},this);
