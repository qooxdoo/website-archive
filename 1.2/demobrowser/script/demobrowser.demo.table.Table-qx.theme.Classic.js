...
},createTable:function(){var be=this.createRandomRows(50);
var bd=this._tableModel=new qx.ui.table.model.Simple();
bd.setColumns([m,b,h,g]);
bd.setData(be);
bd.setColumnEditable(1,true);
bd.setColumnEditable(2,true);
bd.setColumnSortable(3,false);
var bf=new qx.ui.table.Table(bd);
bf.set({width:600,height:400,decorator:null});
bf.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
var bc=bf.getTableColumnModel();
bc.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
bc.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(a,h));
return bf;
},createControls:function(){var M=new qx.ui.toolbar.ToolBar();
var K,L,N;
L=new qx.ui.toolbar.Part();
M.add(L);
K=new qx.ui.toolbar.Button(c,f);
K.addListener(k,function(S){var T=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,T[0][i]);
}this.info("Row 10 changed");
},this);
L.add(K);
K=new qx.ui.toolbar.Button(z,r);
K.addListener(k,function(P){var Q=this.createRandomRows(10);
this._tableModel.addRows(Q);
this.info("10 rows added");
},this);
L.add(K);
K=new qx.ui.toolbar.Button(A,u);
K.addListener(k,function(U){var V=this._tableModel.getRowCount();
this._tableModel.removeRows(V-5,5);
this.info("5 rows removed");
},this);
L.add(K);
K=new qx.ui.toolbar.Button(o,d);
K.addListener(k,function(W){var X=[];
J.getSelectionModel().iterateSelection(function(R){X.push(R+y);
});
this.showDialog(p+X.join(s));
},this);
L.add(K);
L=new qx.ui.toolbar.Part();
M.add(L);
var J=this._table;
N=new qx.ui.toolbar.CheckBox(q);
N.set({value:J.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(t+n)});
N.addListener(j,function(O){J.setKeepFirstVisibleRowComplete(this.getValue());
},N);
L.add(N);
N=new qx.ui.toolbar.CheckBox(e);
N.set({value:J.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(l)});
N.addListener(j,function(Y){if(Y.getData()){var bb=function(F,G){var H=F[arguments.callee.columnIndex];
...
var ba=function(B,C){var D=B[arguments.callee.columnIndex];
var E=C[arguments.callee.columnIndex];

if(D%2==1&&E%2==0){return -1;
}
if(E%2==1&&D%2==0){return 1;
