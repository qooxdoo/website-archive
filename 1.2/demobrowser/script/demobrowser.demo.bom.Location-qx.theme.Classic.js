...
(function(){var p="px",o="moveable",n="demobrowser.demo.bom.Location",m="borderBottomWidth",k="list",j='#',h="mode",g="borderTopWidth",f="borderLeftWidth",e="a",c="div",d="borderRightWidth";
qx.Class.define(n,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);
var q=document.getElementById(k).getElementsByTagName(e);

for(var i=0,l=q.length;i<l;i++){q[i].onclick=demobrowser.demo.bom.Location.jump;
}}},statics:{num:function(a,b){return parseInt(qx.bom.element.Style.get(a,b,qx.bom.element.Style.COMPUTED_MODE,false),10);
},jump:function(event){if(!event){event=window.event;
}event.returnValue=false;

if(event.preventDefault){event.preventDefault();
}function t(r,s){return parseInt(qx.bom.element.Style.get(r,s,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var v=this.href;
var x=v.substr(v.indexOf(j)+1);
var u=document.getElementById(x);
var location=qx.bom.element.Location.get(u,document.getElementById(h).value);
var w=document.getElementById(o);
w.style.left=location.left+p;
w.style.top=location.top+p;

if(qx.bom.client.Feature.BORDER_BOX){w.style.width=(location.right-location.left)+p;
w.style.height=(location.bottom-location.top)+p;
}else{w.style.width=(location.right-location.left-t(w,f)-t(w,d))+p;
w.style.height=(location.bottom-location.top-t(w,g)-t(w,m))+p;
}return false;
},randomize:function(){var y=document.body.getElementsByTagName(c);
