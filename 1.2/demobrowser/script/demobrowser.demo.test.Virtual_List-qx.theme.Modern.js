...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.test.Virtual_List","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.test.Virtual_List-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"ad8a94f7dd66"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
var bO=new qx.ui.basic.Label(u);
bO.setFont(h);
this.getRoot().add(bO,{left:20,top:20});
var bP=new demobrowser.demo.virtual.list.List();
bP.setScrollbarX(bl);
bP.set({height:280,width:150,selectionMode:m});
var bL;

for(var i=1;i<=2500;i++){bL=new demobrowser.demo.virtual.list.ListItem(g+i,K+((i%4)?bo:O)+bu);

if(i%4==0){bL.setHeight(56);
}bP.add(bL);
if(i==20){bP.setSelected(bL);
}}bP.addListener(l,function(e){this.debug("Value: "+e.getData());
});
this.getRoot().add(bP,{left:20,top:40});
var bH=new qx.ui.form.RadioButton(L);
var bE=new qx.ui.form.RadioButton(bh);
var bF=new qx.ui.form.RadioButton(bv);
var bD=new qx.ui.form.RadioButton(H);
bH.setUserData(o,k);
bE.setUserData(o,m);
bF.setUserData(o,j);
bD.setUserData(o,n);
bE.setValue(true);
this.getRoot().add(bH,{left:180,top:40});
this.getRoot().add(bE,{left:180,top:60});
this.getRoot().add(bF,{left:180,top:80});
this.getRoot().add(bD,{left:180,top:100});
var bG=new qx.ui.form.RadioGroup(bH,bE,bF,bD);
var bK=new qx.ui.form.CheckBox(bq);
var bN=new qx.ui.form.CheckBox(bj).set({enabled:false});
this.getRoot().add(bK,{left:180,top:220});
this.getRoot().add(bN,{left:180,top:240});
bK.addListener(l,function(e){if(e.getData()){var bS=bP.getSelectionMode();

if(bS==k||bS==n){this.debug("Drag selection is only available for the modes multi or additive");
}}bP.setDragSelection(e.getData());
});
bN.addListener(l,function(e){if(e.getData()){var bC=bP.getSelectionMode();

if(bC==m||bC==j){this.debug("Quick selection is only available for the modes multi or additive");
}}bP.setQuickSelection(e.getData());
});
bG.addListener(T,function(e){var bT=e.getData()[0].getUserData(o);
bP.setSelectionMode(bT);

if(bT==k||bT==n){bK.setEnabled(false);
bN.setEnabled(true);
}else if(bT==m||bT==q){bK.setEnabled(true);
bN.setEnabled(false);
}});
var bR=new qx.ui.basic.Label(N);
bR.setFont(h);
this.getRoot().add(bR,{left:330,top:20});
var bM=new demobrowser.demo.virtual.list.List();
bM.set({height:280,width:150,selectionMode:n});
var bL;

for(var i=1;i<=25;i++){bL=new demobrowser.demo.virtual.list.ListItem(g+i,s);
bM.add(bL);
if(i==16){bM.setSelected(bL);
}}this.getRoot().add(bM,{left:330,top:40});
var bO=new qx.ui.basic.Label(B);
bO.setFont(h);
this.getRoot().add(bO,{left:520,top:20});
var bQ=new demobrowser.demo.virtual.list.List;
var bI;
bQ.set({width:150,selectionMode:j});
var bJ=[v,U,X,S,x,by,bi,ba,M,f,c,A,b,V,bm,bn,w,Q,a,G,d,bg,I,bt,bb,bk,bw,y,bf,br,W,bs,t,J,P,z,E,F,bB,bc,bd,R,r,Y,C,be,D,bp,bA,bx,bz];

for(var i=0;i<bJ.length;i++){bI=new demobrowser.demo.virtual.list.ListItem(bJ[i]);
bQ.add(bI);

if(i==10||i==12||i==16){bQ.addToSelection(bI);
}}this.getRoot().add(bQ,{left:520,top:40});
}}});
})();
...
(function(){var ba="selected",Y="qx.event.type.Data",X="Boolean",W="changeSelection",V="row",U="single",T="changeHeight",S=",",R="one",Q="_applySpacing",bp="Integer",bo="_applyDragSelection",bn="String",bm="resize",bl="demobrowser.demo.virtual.list.List",bk="list",bj="multi",bi="_applyQuickSelection",bh="_applyItemHeight",bg="_applySelectionMode",be="additive",bf="changeName",bc="Number",bd="addItem",bb="removeItem";
qx.Class.define(bl,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider],construct:function(){qx.ui.virtual.core.Scroller.call(this,0,1,this.getItemHeight(),10);
this.__Lp=new qx.ui.virtual.layer.WidgetCell(this);
this.getPane().addLayer(this.__Lp);
this.getPane().addListener(bm,this._onResize,this);
this.__Lq=[];
this.__Lr=[];
this.__Ls=new qx.ui.virtual.selection.Row(this.getPane());
this.__Ls.addListener(W,this._onChangeSelection,this);
this.__Ls.attachMouseEvents();
this.__Ls.attachKeyEvents(this);
this.__Ls.attachListEvents(this);
...
},events:{addItem:Y,removeItem:Y,changeValue:Y,changeSelection:Y},properties:{appearance:{refine:true,init:bk},focusable:{refine:true,init:true},spacing:{check:bp,init:0,apply:Q,themeable:true},enableInlineFind:{check:X,init:true},name:{check:bn,nullable:true,event:bf},itemHeight:{init:24,themeable:true,check:bc,apply:bh},selectionMode:{check:[U,bj,be,R],init:U,apply:bg},dragSelection:{check:X,init:false,apply:bo},quickSelection:{check:X,init:false,apply:bi}},members:{__Lq:null,__Lr:null,__Ls:null,__Lp:null,syncWidget:function(){this.update();
},update:function(){var br=this.getPane().getRowConfig();
br.setItemCount(this.__Lq.length);
br.resetItemSizes();

for(var i=0;i<this.__Lq.length;i++){var bq=this.__Lq[i].getHeight();

if(bq!==null){br.setItemSize(i,bq);
}}},updateSelection:function(){var H=this.__Lp.getChildren();

for(var i=0;i<H.length;i++){var I=H[i];
var J=I.getUserData(V);

if(this.__Ls.isItemSelected(J)){I.addState(ba);
}else{I.removeState(ba);
...
this.fireDataEvent(W,this.__Lw(e.getData()));
},getCellWidget:function(a,b){var d=this.__Lq[a];

if(!d){return null;
}var c=this.__Lr.pop()||new qx.ui.form.ListItem();
c.set({label:d.getLabel(),icon:d.getIcon()});

if(this.__Ls.isItemSelected(a)){c.addState(ba);
}else{c.removeState(ba);
}c.setUserData(V,a);
return c;
},poolCellWidget:function(x){this.__Lr.push(x);
},getValue:function(){var bv=this.getSelection();
var bw=[];
var bx;

for(var i=0,l=bv.length;i<l;i++){bx=bv[i].getValue();
if(bx==null){bx=bv[i].getLabel();
}bw.push(bx);
}return bw.join(S);
},setValue:function(n){var q=n.split(S);
var p=[];
var o;

for(var i=0,l=q.length;i<l;i++){o=this.findItem(q[i]);

if(o){p.push(o);
}}this.setSelection(p);
},getChildren:function(){return this.__Lq;
},hasChildren:function(){return this.__Lq.length>0;
},_addHelper:function(f){this.fireDataEvent(bd,f);
f.addListener(T,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},_removeHelper:function(G){this.fireDataEvent(bb,G);
G.removeListener(T,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},add:function(bz){this.__Lq.push(bz);
this._addHelper(bz);
return this;
},remove:function(F){qx.lang.Array.remove(this.__Lq,F);
this._removeHelper(F);
return this;
},removeAll:function(){for(var i=0,j=this.__Lq.length;i<j;i++){this._removeHelper(this.__Lq[i]);
}this.__Lq=[];
},indexOf:function(m){return this.__Lq.indexOf(m);
},addAt:function(B,C){qx.lang.Array.insertAt(this.__Lq,B,C);
this._addHelper(B);
},addBefore:function(t,u){qx.lang.Array.insertBefore(this.__Lq,t,u);
this._addHelper(t);
},addAfter:function(D,E){qx.lang.Array.insertAfter(this.__Lq,D,E);
this._addHelper(D);
},removeAt:function(A){this.__Lq.splice(A,1);
this._removeHelper(this.__Lq[A]);
},__Lt:function(y){var z=this.__Lq.indexOf(y);

if(z<0){return null;
}else{return z;
}},__Lu:function(r){var s=this.__Lq[r];
return s;
},__Lv:function(K){var L=[];

for(var i=0;i<K.length;i++){L.push(this.__Lt(K[i]));
}return L;
},__Lw:function(bA){var bB=[];

for(var i=0;i<bA.length;i++){bB.push(this.__Lq[bA[i]]);
}return bB;
},selectAll:function(){this.__Ls.selectAll();
},setSelected:function(by){this.__Ls.selectItem(this.__Lt(by));
},isSelected:function(v){return this.__Ls.isItemSelected(this.__Lt(v));
},addToSelection:function(bs){this.__Ls.addItem(this.__Lt(bs));
},removeFromSelection:function(w){this.__Ls.removeItem(this.__Lt(w));
},selectRange:function(O,P){this.__Ls.selectItemRange(this.__Lt(O),this.__Lt(P));
},resetSelection:function(){this.__Ls.clearSelection();
},setSelection:function(g){this.__Ls.replaceSelection(this.__Lv(g));
},getSelectedItem:function(){return this.__Lq[this.getSelected()];
},getSelected:function(){return this.__Lq[this.__Ls.getSelectedItem()];
},getSelection:function(){return this.__Lw(this.__Ls.getSelection());
},getSortedSelection:function(){return this.__Lw(this.__Ls.getSortedSelection());
},isSelectionEmpty:function(){return this.__Ls.isSelectionEmpty();
},getSelectionContext:function(){return this.__Ls.getSelectionContext();
},getSelectables:function(){return this.__Lw(this.__Ls.getSelectables());
},_applySelectionMode:function(M,N){this.__Ls.setMode(M);
},_applyDragSelection:function(h,k){this.__Ls.setDrag(h);
},_applyQuickSelection:function(bt,bu){this.__Ls.setQuick(bt);
}}});
})();
...
(function(){var j="_applyInterval",i="__GT",h="appear",g="interval",f="qx.ui.virtual.behavior.Prefetch",e="_applyScroller",d="qx.ui.virtual.core.Scroller",c="scrollbar-x",b="Integer",a="scrollbar-y";
qx.Class.define(f,{extend:qx.core.Object,construct:function(y,z){{};
...
this.__GT=new qx.event.Timer(this.getInterval());
this.__GT.addListener(g,this._onInterval,this);

if(y){this.setScroller(y);
}},properties:{scroller:{check:d,nullable:true,init:null,apply:e},interval:{check:b,init:200,apply:j}},members:{__GU:null,__GV:null,__GT:null,__GW:null,__GX:null,setPrefetchX:function(o,p,q,r){this.__GU=[o,p,q,r];
},setPrefetchY:function(k,l,m,n){this.__GV=[k,l,m,n];
},_onInterval:function(){var u=this.__GU;
...
qx.ui.core.queue.Manager.flush();
}},_applyScroller:function(s,t){if(t){if(this.__GW){t.getChildControl(c).removeListenerById(this.__GW);
}
if(this.__GX){t.getChildControl(a).removeListenerById(this.__GX);
}}
if(s){if(!s.getContainerElement().getDomElement()){this.__GT.stop();
s.addListenerOnce(h,this.__GT.start,this.__GT);
}else{this.__GT.restart();
}this.__GW=s.getChildControl("scrollbar-x").addListener("scroll",this.__GT.restart,this.__GT);
this.__GX=s.getChildControl("scrollbar-y").addListener("scroll",this.__GT.restart,this.__GT);
}else{this.__GT.stop();
}},_applyInterval:function(w,x){this.__GT.setInterval(w);
}},destruct:function(){this.setScroller(null);
this.__GU=this.__GV=null;
...
qx.Class.define(a,{extend:qx.core.Object,construct:function(k,l,m){qx.core.Object.call(this);
