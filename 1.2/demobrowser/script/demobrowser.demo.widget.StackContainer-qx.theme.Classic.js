...
(function(){var x="execute",w="yellow",v="teal",u="gray",t="red",s="Next",r="Previous",q="green",p="icon/22/actions/go-next.png",o="changeSelection",k="icon/22/actions/go-previous.png",n="orange",m="main",j="blue",h="resize",l="demobrowser.demo.widget.StackContainer";
...
var g=new qx.ui.form.Button(r,k);
var d=new qx.ui.form.Button(s,p);
a.add(g);
a.add(d);
this.getRoot().add(a,{left:20,top:150});
g.addListener(x,c.previous,c);
d.addListener(x,c.next,c);
var f=[t,u,j,n,v,w,q];
var b;

for(var i=0;i<f.length;i++){b=new qx.ui.core.Widget;
b.setBackgroundColor(f[i]);
c.add(b);
}c.addListener(o,function(e){this.debug("Selected: "+e.getData()[0].getBackgroundColor());
});
},addDynamicStack:function(){var C=new qx.ui.container.Stack();
C.setDecorator(m);
C.setDynamic(true);
this.getRoot().add(C,{left:250,top:20});
var A=new qx.ui.container.Composite((new qx.ui.layout.HBox).set({spacing:4}));
var F=new qx.ui.form.Button(r,k);
var D=new qx.ui.form.Button(s,p);
A.add(F);
A.add(D);
this.getRoot().add(A,{left:250,top:150});
F.addListener(x,C.previous,C);
D.addListener(x,C.next,C);
var E=[t,u,j,n,v,w,q];
var B;
...
B.setWidth((Math.round(i/2)+1)*50);
B.setHeight((Math.round(i/3)+1)*40);
C.add(B);
}C.addListener(o,function(e){var z=e.getData()[0];
this.debug("Selected: "+z.getBackgroundColor()+" ("+z.getWidth()+"x"+z.getHeight()+")");
});
C.addListener(h,function(e){var y=this.getBounds();
this.debug("Resize to: "+y.width+"x"+y.height);
});
}}});
})();
