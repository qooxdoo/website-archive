...
qx.Class.define(D,{extend:qx.application.Standalone,members:{__Ml:null,messenger:null,__Mm:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var bk=this.getRoot();
var bn=new qx.ui.window.Window(G).set({contentPadding:0,showClose:false,showMinimize:false});
...
var bl=demobrowser.demo.virtual.messenger.BuddyModel.createBuddies(200);
this.messenger.setModel(bl);
bn.add(this.createToolbar());
bn.add(this.messenger,{flex:1});
bk.add(this.createDetailsView(),{left:20,top:20});
},createToolbar:function(){var toolbar=new qx.ui.toolbar.ToolBar();
var bx=new qx.ui.toolbar.Part();
toolbar.add(bx);
var bv=new qx.ui.toolbar.Button(j,F).set({show:d});
bv.addListener(l,this.showAddContactWindow,this);
bx.add(bv);
var bw=new qx.ui.toolbar.Button(j,q).set({show:d});
this.messenger.bind(U,bw,o,{converter:function(a){return !!a;
}});
bw.addListener(l,function(){this.messenger.getModel().remove(this.messenger.getSelection().getItem(0));
},this);
bx.add(bw);
return bx;
},createDetailsView:function(){var br=new qx.data.controller.Object();
this.messenger.bind(U,br,K);
var bo=new qx.ui.groupbox.GroupBox(x);
var bs=new qx.ui.layout.Grid(5,5);
bs.setColumnAlign(0,V,k);
bo.setLayout(bs);
bo.add(new qx.ui.basic.Label(n),{row:0,column:0});
var bu=new qx.ui.form.TextField();
br.addTarget(bu,g,u,true);
bo.add(bu,{row:0,column:1});
bo.add(new qx.ui.basic.Label(P),{row:1,column:0});
var bp=new qx.ui.form.TextField();
br.addTarget(bp,g,m,true);
bo.add(bp,{row:1,column:1});
bo.add(new qx.ui.basic.Label(E),{row:2,column:0});
var bt=new qx.ui.form.SelectBox();
bt.add(new qx.ui.form.ListItem(f).set({model:f}));
bt.add(new qx.ui.form.ListItem(i).set({model:i}));
bt.add(new qx.ui.form.ListItem(c).set({model:c}));
bt.add(new qx.ui.form.ListItem(h).set({model:h}));
this.messenger.bind(b,bt,C,{converter:function(bj){return [bj];
}});
bt.bind(I,this.messenger,b,{converter:function(bh){return bh[0].getModel();
}});
bo.add(bt,{row:2,column:1});
bo.add(new qx.ui.basic.Label(B).set({alignY:A}),{row:3,column:0});
var bq=new qx.ui.basic.Image().set({alignX:r});
br.addTarget(bq,s,y);
bo.add(bq,{row:3,column:1});
return bo;
},showAddContactWindow:function(){if(this.__Ml==null){var X=new qx.ui.layout.Grid(5,10);
var ba=new qx.ui.window.Window(L,p);
ba.set({width:165,height:100,showMinimize:false,showClose:false,showMaximize:false});
ba.setLayout(X);
var bd=new qx.ui.basic.Label(N);
var be=new qx.ui.form.TextField();
var bb=new qx.ui.form.ComboBox(w);
bb.add(new qx.ui.form.ListItem(Q));
bb.add(new qx.ui.form.ListItem(O));
var Y=new qx.ui.basic.Label(t);
var bg=new qx.ui.form.Button(H);
var bf=new qx.ui.form.Button(R);
this.__Mm=be;
bg.setAllowGrowX(false);
bf.setAllowGrowX(false);
ba.addListener(J,be.focus,be);
bf.addListener(l,ba.close,ba);
var bc=function(){var W=new demobrowser.demo.virtual.messenger.BuddyModel();
W.setName(be.getValue());
W.setGroup(bb.getValue());
this.messenger.getModel().push(W);
this.messenger.getSelection().setItem(0,W);
ba.close();
};
bg.addListener(l,bc,this);
ba.addListener(v,function(e){var bi=e.getKeyIdentifier();

if(bi==M){bc.call(this);
}else if(bi==S){ba.close();
}},this);
ba.add(bd,{row:0,column:0});
ba.add(be,{row:0,column:1,colSpan:2});
ba.add(Y,{row:1,column:0});
ba.add(bb,{row:1,column:1,colSpan:2});
ba.add(bg,{row:2,column:1});
ba.add(bf,{row:2,column:2});
X.setColumnAlign(0,z,k);
X.setColumnAlign(1,V,k);
this.getRoot().add(ba,{left:20,top:20});
this.__Ml=ba;
}this.__Mm.setValue(j);
this.__Ml.center();
this.__Ml.open();
...
qx.Class.define(g,{extend:qx.ui.virtual.form.ListController,construct:function(v,w){qx.ui.virtual.form.ListController.call(this,v,w);
this.__Mn=new demobrowser.demo.virtual.messenger.BuddyCell();
this.__Mo=new demobrowser.demo.virtual.messenger.GroupCell();
this.__Mo.addListener(j,this._onChangeOpenGroup,this);
this.__Mp=[];
this.__Mq={};
this.__Mr=[];
this.__Ms=new qx.util.DeferredCall(this._updateGrouping,this);
},members:{__Mr:null,__Mq:null,__Ms:null,__Mp:null,__Mn:null,__Mo:null,_getRowData:function(D){return this.__Mr[D];
},_getModelRow:function(c){return this.__Mr.indexOf(c);
},_getRowCount:function(){return this.__Mr.length;
},_isGroupHeader:function(E){return this.__Mr[E] instanceof demobrowser.demo.virtual.messenger.GroupModel;
},_getGroupModel:function(name,G){var H=this.__Mq[name];

if(!H){var H=new demobrowser.demo.virtual.messenger.GroupModel();
H.setName(name);
this.__Mq[name]=H;
}H.setRow(G);
return H;
},_onChangeOpenGroup:function(e){var l=this.__Mr[e.getData().getUserData(d)];
l.toggleOpen();
this.__Ms.schedule();
},getCellRenderer:function(F){if(this._isGroupHeader(F)){return this.__Mo;
}else{return this.__Mn;
}},isRowSelectable:function(C){return !this._isGroupHeader(C);
},_visualizeGrouping:function(z){var A=this.getTarget();

if(A){for(var i=0;i<this.__Mp.length;i++){var B=this.__Mp[i].getOldRow();
...
},_updateGrouping:function(){var q=this.getModel();
this.__Mr=[];
var r=[];
var n=[];

if(q&&q.length>0){r=qx.lang.Array.clone(q.toArray());
r.sort(function(a,b){var t=a.getGroup();
var u=b.getGroup();

if(t==u){return a.getName()<b.getName()?-1:1;
}return t>u?-1:1;
});
var p=r[0];
var m=this._getGroupModel(p.getGroup(),0);
this.__Mr.push(m);
n.push(m);
var s=0;
...
if(o.getGroup()!==m.getName()){m.setItemCount(s);
s=0;
var m=this._getGroupModel(o.getGroup(),this.__Mr.length);
this.__Mr.push(m);
n.push(m);
}
if(m.isOpen()){this.__Mr.push(o);
}else{this.getSelection().remove(o);
}s+=1;
}}m.setItemCount(s);
this._visualizeGrouping(n);
this._syncModelSelectionToView();
this._syncRowCount();
},_applyModel:function(x,y){qx.ui.virtual.form.ListController.prototype._applyModel.call(this,x,y);
this.__Ms.schedule();
...
if(I.name){var J=I.name.toString().split(f)[1];

if(J==k||J==h){this.__Ms.schedule();
...
}},properties:{name:{init:c,event:g,check:a},row:{event:d,check:j,apply:b},open:{check:h,init:true,event:i},itemCount:{check:j,init:0,event:e}},members:{__Mt:null,_applyRow:function(k,l){this.__Mt=l;
},getOldRow:function(){return this.__Mt;
}}});
})();
