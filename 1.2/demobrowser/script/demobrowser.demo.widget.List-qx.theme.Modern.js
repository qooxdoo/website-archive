...
var cg=new qx.ui.container.Scroll();
var cw=new qx.ui.container.Composite(new qx.ui.layout.Basic());
cw.setAllowStretchX(false);
cg.add(cw);
this.getRoot().add(cg,{edge:0});
var cq=new qx.ui.basic.Label(bk);
cq.setFont(bF);
cw.add(cq,{left:20,top:20});
var cy=new qx.ui.form.List;
cy.setScrollbarX(n);
cy.set({height:280,width:150,selectionMode:bH});
var cz;

for(var i=1;i<=25;i++){cz=new qx.ui.form.ListItem(bA+i,J+((i%4)?L:bq)+U);
cy.add(cz);
!(i%9)&&(cz.setEnabled(false));
if(i==20){cy.setSelection([cz]);
}}cy.addListener(bE,function(e){var cG=e.getData();

if(cG[0]){this.debug("Value: "+cG[0].getLabel());
}});
cw.add(cy,{left:20,top:40});
var cm=new qx.ui.form.RadioButton(G);
var cj=new qx.ui.form.RadioButton(bS);
var ck=new qx.ui.form.RadioButton(ca);
var ci=new qx.ui.form.RadioButton(p);
cm.setUserData(bI,bC);
cj.setUserData(bI,bH);
ck.setUserData(bI,bD);
ci.setUserData(bI,bG);
cj.setValue(true);
cw.add(cm,{left:180,top:40});
cw.add(cj,{left:180,top:60});
cw.add(ck,{left:180,top:80});
cw.add(ci,{left:180,top:100});
var cl=new qx.ui.form.RadioGroup(cm,cj,ck,ci);
var cC=new qx.ui.form.RadioButton(j);
var cB=new qx.ui.form.RadioButton(bo);
var cE=new qx.ui.form.RadioButton(bx);
cC.setUserData(bI,F);
cB.setUserData(bI,by);
cE.setUserData(bI,W);
cE.setValue(true);
cw.add(cC,{left:180,top:140});
cw.add(cB,{left:180,top:160});
cw.add(cE,{left:180,top:180});
var cx=new qx.ui.form.RadioGroup(cC,cB,cE);
cx.addListener(bE,function(e){for(var i=0;i<cy.getChildren().length;i++){cy.getChildren()[i].setShow(e.getData()[0].getUserData(bI));
}});
var cu=new qx.ui.form.CheckBox(bv);
var cr=new qx.ui.form.CheckBox(l).set({enabled:false});
cw.add(cu,{left:180,top:220});
cw.add(cr,{left:180,top:240});
cu.addListener(bB,function(e){if(e.getData()){var cf=cy.getSelectionMode();

if(cf==bC||cf==bG){this.debug("Drag selection is only available for the modes multi or additive");
}}cy.setDragSelection(e.getData());
});
cr.addListener(bB,function(e){if(e.getData()){var ce=cy.getSelectionMode();

if(ce==bH||ce==bD){this.debug("Quick selection is only available for the modes multi or additive");
}}cy.setQuickSelection(e.getData());
});
cl.addListener(bE,function(e){var cF=e.getData()[0].getUserData(bI);
cy.setSelectionMode(cF);

if(cF==bC||cF==bG){cu.setEnabled(false);
cr.setEnabled(true);
}else if(cF==bH||cF==cd){cu.setEnabled(true);
cr.setEnabled(false);
}});
var cD=new qx.ui.basic.Label(bV);
cD.setFont(bF);
cw.add(cD,{left:330,top:20});
var cp=new qx.ui.form.List();
cp.set({height:280,width:150,selectionMode:bG});
var cz;

for(var i=1;i<=25;i++){cz=new qx.ui.form.ListItem(bA+i,bj);
cp.add(cz);
if(i==16){cp.setSelection([cz]);
}}cw.add(cp,{left:330,top:40});
var cq=new qx.ui.basic.Label(bc);
cq.setFont(bF);
cw.add(cq,{left:520,top:20});
var ch=new qx.ui.form.List;
var cn;
ch.set({width:150,selectionMode:bD});
var ct=[bu,bf,B,bW,M,br,I,Q,P,bz,bb,s,u,bN,q,be,X,v,bJ,bK,T,bR,O,bM,x,a,bX,N,cb,bg,Y,o,H,bL,bh,t,S,bm,bU,bO,h,C,bl,cc,r,bP,w,g,bn,m,V];

for(var i=0;i<ct.length;i++){cn=new qx.ui.form.ListItem(ct[i]);
ch.add(cn);

if(i==10||i==12||i==16){ch.addToSelection(cn);
}}cw.add(ch,{left:520,top:40});
var cq=new qx.ui.basic.Label(bd);
cq.setFont(bF);
cw.add(cq,{left:20,top:350});
var cA=new qx.ui.form.List(true);
var co;
cA.set({width:550,selectionMode:bH,height:null});
var cs=[bT,z,bs,c,bw,bi,bY,R,ba,bt,f,d,D,b,bQ,A,K,y,E];
var cv=bp;

for(var i=0;i<cs.length;i++){co=new qx.ui.form.ListItem(null,cv+cs[i]);
cA.add(co);

if(i==12){cA.setSelection([co]);
}}cw.add(cA,{left:20,top:370});
}}});
})();
