...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.virtual.Selection-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"a1554cdf8414"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var l="manager",k="multi",j=" / ",i="bold",h="value",g="white",f="selected",d="#EEE",c="horizontal",b="table-scroller-header",G="one",F="single",E="changeValue",D="table-row-background-selected",C="additive",B="default",A="Additive Selection",z="Cell Line Selection",y="addaptive",x="Options",s="Row Selection",t="changeSelection",q="Column Selection",r="Enable drag selection",o="demobrowser.demo.virtual.Selection",p="Selection Mode",m="Single Selection",n="Cell Rectangle Selection",u="Enable quick selection",v="One Selection",w="Multi Selection";
...
var M=qx.theme.manager.Font.getInstance().resolve(B).getStyles();
this._fontCss=qx.bom.element.Style.compile(M);
var V=new qx.ui.layout.Grid(5,0);
V.setRowFlex(2,1);
V.setColumnFlex(0,1);
V.setColumnFlex(1,1);
V.setColumnFlex(2,1);
...
this.getRoot().add(T,{edge:5});
T.add(new qx.ui.basic.Label(s).set({font:i,decorator:b,padding:3,allowGrowX:true}),{row:1,column:0});
var L=this.createRowSelectionScroller();
J.push(L.getUserData(l));
T.add(L,{row:2,column:0});
T.add(new qx.ui.basic.Label(q).set({font:i,decorator:b,padding:3,allowGrowX:true}),{row:1,column:1});
var L=this.createColumnSelectionScroller();
J.push(L.getUserData(l));
T.add(L,{row:2,column:1});
T.add(new qx.ui.basic.Label(n).set({font:i,decorator:b,padding:3,allowGrowX:true}),{row:1,column:2});
var L=this.createCellRectangleSelectionScroller();
J.push(L.getUserData(l));
T.add(L,{row:2,column:2});
T.add(new qx.ui.basic.Label(z).set({font:i,decorator:b,padding:3,allowGrowX:true}),{row:1,column:3});
var L=this.createCellLineSelectionScroller();
J.push(L.getUserData(l));
T.add(L,{row:2,column:3});
var R=new qx.ui.layout.Grid(20,4);
var O=new qx.ui.container.Composite(R).set({padding:15,backgroundColor:g});
T.add(O,{row:0,column:0,colSpan:4});
O.add(new qx.ui.basic.Label(p).set({font:i,padding:3}),{row:0,column:0});
var N=new qx.ui.form.RadioButton(m);
N.setUserData(h,F);
O.add(N,{row:1,column:0});
var Q=new qx.ui.form.RadioButton(w);
Q.setUserData(h,k);
Q.setValue(true);
O.add(Q,{row:2,column:0});
var K=new qx.ui.form.RadioButton(A);
K.setUserData(h,C);
O.add(K,{row:3,column:0});
var I=new qx.ui.form.RadioButton(v);
I.setUserData(h,G);
O.add(I,{row:4,column:0});
var P=new qx.ui.form.RadioGroup(N,Q,K,I);
P.addListener(t,function(e){var bK=e.getData()[0].getUserData(h);
J.forEach(function(a){a.setMode(bK);
});

if(bK==F||bK==G){U.setEnabled(false);
S.setEnabled(true);
}else if(bK==k||bK==y){U.setEnabled(true);
S.setEnabled(false);
}});
O.add(new qx.ui.basic.Label(x).set({font:i,padding:3}),{row:0,column:1});
var U=new qx.ui.form.CheckBox(r);
O.add(U,{row:1,column:1});
U.addListener(E,function(e){if(e.getData()){var H=J[0].getMode();

if(H==F||H==G){this.debug("Drag selection is only available for the modes multi or additive");
}}J.forEach(function(bJ){bJ.setDrag(e.getData());
});
});
var S=new qx.ui.form.CheckBox(u).set({enabled:false});
O.add(S,{row:2,column:1});
S.addListener(E,function(e){if(e.getData()){var bD=J[0].getMode();

if(bD==k||bD==C){this.debug("Quick selection is only available for the modes multi or additive");
}}J.forEach(function(bC){bC.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bP=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var bN=new qx.ui.virtual.layer.Row(g,d);
bP.getPane().addLayer(bN);
bP.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var bO=new qx.ui.virtual.cell.Cell();
var bM=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(ba,bb){var bc={};

if(bL.isItemSelected(ba)){bc.selected=true;
}return bO.getCellProperties(ba+j+bb,bc);
}});
bP.getPane().addLayer(bM);
var bL=new qx.ui.virtual.selection.Row(bP.getPane(),{styleSelectable:function(bz,bA,bB){if(bA!==f){return;
}
if(bB){bN.setBackground(bz,f);
}else{bN.setBackground(bz,null);
}bM.updateLayerData();
}});
bL.attachMouseEvents(bP.getPane());
bL.attachKeyEvents(bP);
bL.set({mode:k});
bL.addItem(0);
bP.setUserData(l,bL);
return bP;
},createColumnSelectionScroller:function(){var bg=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bg.getPane().addLayer(new qx.ui.virtual.layer.Row(g,d));
var bf=new qx.ui.virtual.layer.Column();
bg.getPane().addLayer(bf);
bg.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var bh=new qx.ui.virtual.cell.Cell();
var be=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bQ,bR){var bS={};

if(bd.isItemSelected(bR)){bS.selected=true;
}return bh.getCellProperties(bQ+j+bR,bS);
}});
bg.getPane().addLayer(be);
var bd=new qx.ui.virtual.selection.Column(bg.getPane(),{styleSelectable:function(bn,bo,bp){if(bo!==f){return;
}
if(bp){bf.setBackground(bn,f);
}else{bf.setBackground(bn,null);
}be.updateLayerData();
}});
bd.attachMouseEvents(bg.getPane());
bd.attachKeyEvents(bg);
bd.set({mode:k});
bd.addItem(0);
bg.setUserData(l,bd);
return bg;
},createCellRectangleSelectionScroller:function(){var bl=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bl.getPane().addLayer(new qx.ui.virtual.layer.Row(g,d));
var bk=new qx.ui.virtual.cell.Cell();
var bm=new qx.ui.virtual.cell.Cell().set({backgroundColor:D});
var bj=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bt,bu){var bv={};

if(bi.isItemSelected({row:bt,column:bu})){bv.selected=true;
}
if(bv.selected){return bm.getCellProperties(bt+j+bu,bv);
}else{bk.resetBackgroundColor();
return bk.getCellProperties(bt+j+bu,bv);
}}});
bl.getPane().addLayer(bj);
bl.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var bi=new qx.ui.virtual.selection.CellRectangle(bl.getPane(),{styleSelectable:function(bq,br,bs){bj.updateLayerData();
}});
...
bl.setUserData(l,bi);
return bl;
},createCellLineSelectionScroller:function(){var bH=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bH.getPane().addLayer(new qx.ui.virtual.layer.Row(g,d));
var bG=new qx.ui.virtual.cell.Cell();
var bI=new qx.ui.virtual.cell.Cell().set({backgroundColor:D});
var bF=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bw,bx){var by={};

if(bE.isItemSelected({row:bw,column:bx})){by.selected=true;
}
if(by.selected){return bI.getCellProperties(bw+j+bx,by);
}else{bG.resetBackgroundColor();
return bG.getCellProperties(bw+j+bx,by);
}}});
bH.getPane().addLayer(bF);
bH.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var bE=new qx.ui.virtual.selection.CellLines(bH.getPane(),{styleSelectable:function(W,X,Y){bF.updateLayerData();
}});
bE.attachMouseEvents(bH.getPane());
bE.attachKeyEvents(bH);
bE.set({mode:k});
bE.addItem({row:0,column:0});
bH.setUserData(l,bE);
return bH;
}}});
})();
...
if(!j){return null;
}return this._isSelectable(j.column)?j.column:null;
...
var n=l.getItemPosition(k);
var m=n+l.getItemSize(k)-1;
return {left:n,right:m};
},_getSelectableLocationY:function(p){return {top:0,bottom:this._pane.getRowConfig().getTotalSize()-1};
}}});
})();
(function(){var m="under",l="above",k="left",j="right",i="x",h="qx.ui.virtual.selection.CellRectangle";
qx.Class.define(h,{extend:qx.ui.virtual.selection.Abstract,members:{_getItemCount:function(){return this._pane.getRowConfig().getItemCount()*this._pane.getColumnConfig().getItemCount();
...
},getSelectables:function(U){var W=[];
var ba=this._pane.getRowConfig().getItemCount();
var V=this._pane.getColumnConfig().getItemCount();

for(var bb=0;bb<ba;bb++){for(var X=0;X<V;X++){var Y={row:bb,column:X};

if(this._isSelectable(Y)){W.push(Y);
}}}return W;
},_getSelectableRange:function(E,F){var I=[];
var J=Math.min(E.row,F.row);
var K=Math.max(E.row,F.row);
var H=Math.min(E.column,F.column);
var M=Math.max(E.column,F.column);

for(var N=J;N<=K;N++){for(var L=H;L<=M;L++){var G={row:N,column:L};

if(this._isSelectable(G)){I.push(G);
}}}return I;
...
switch(b){case l:for(var g=a.row-1;g>=0;g--){c.row=g;

if(this._isSelectable(c)){return c;
}}break;
case m:var d=this._pane.getRowConfig().getItemCount();

for(var g=a.row+1;g<d;g++){c.row=g;

if(this._isSelectable(c)){return c;
}}break;
case k:for(var f=a.column-1;f>=0;f--){c.column=f;

if(this._isSelectable(c)){return c;
}}break;
case j:var e=this._pane.getColumnConfig().getItemCount();
...
}},_selectableToHashCode:function(t){return t.column+i+t.row;
},_scrollItemIntoView:function(z){this._pane.scrollCellIntoView(z.column,z.row);
