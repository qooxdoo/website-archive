...
qx.Class.define(g,{extend:qx.ui.progressive.renderer.table.cell.Default,statics:{onmouseover:function(N){N.__MC=qx.bom.element.Style.get(N,"color");
N.style.color=w;
N.parentNode.style.height=32;
N.__MD=N.innerHTML;
var P=Date.parse(N.attributes.getNamedItem(v).nodeValue);
var O=new Date(P);
N.innerHTML=O.toUTCString()+o+O.toString();
},onmouseout:function(M){M.style.color=M.__MC;
M.parentNode.style.height=16;
M.innerHTML=M.__MD;
}},members:{_getCellExtras:function(K){var L=[];
L.push(qx.ui.progressive.renderer.table.cell.Default.prototype._getCellExtras.call(this));
L.push(q+K.cellData.toUTCString()+p);
L.push(k,m);
return L.join(u);
}}});
})();
