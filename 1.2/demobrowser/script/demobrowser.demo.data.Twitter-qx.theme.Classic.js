...
(function(){var j="value",i="icon",h="model",g="text",f="qx.client",e="execute",d="source",c="wittemann",b="mshtml",a=".json",F="user.profile_image_url",E="http://twitter.com/statuses/user_timeline/",D="Enter",C="<span style='font-size: 20px'>Twitter</span>",B="<a target='_blank' href='http://twitter.com/home?status=Check%20out%20the%20%23qooxdoo%20twitter%20demo%20at%20http://bit.ly/Mc4Of'>Tweet This</a></span>",A="demobrowser.demo.data.store.Twitter",z="keydown",y="Friends",x="demobrowser.demo.data.Twitter",w="selection[0]",q="<span style='font-size: 13px'>",r="Location: ",o="Details",p="Name: ",m="?callback=demobrowser.demo.data.store.Twitter.saveResult",n="User",k="http://twitter.com/statuses/friends_timeline.json",l="Postet with: ",s="user.name",t="user.location",v="Avatar: ",u="Message: ";
...
U.setDelegate(this);
U.setLabelPath(g);
if(!qx.core.Variant.isSet(f,b)){U.setIconPath(F);
}var Y=new demobrowser.demo.data.store.Twitter(c);
Y.bind(h,U,h);
var Q=new qx.ui.form.Button(y);
this.getRoot().add(Q,{left:10,top:55});
Q.addListener(e,function(){Y.setUrl(k);
},this);
var V=new qx.ui.form.Button(n);
this.getRoot().add(V,{left:90,top:55});
var N=new qx.ui.form.TextField(c);
this.getRoot().add(N,{left:140,top:55});
V.addListener(e,function(){var H=E+N.getValue()+a;

if(Y.getUrl()==H){Y.reload();
}else{Y.setUrl(H);
}},this);
N.addListener(z,function(M){if(M.getKeyIdentifier()==D){V.execute();
}},this);
var R=new qx.ui.groupbox.GroupBox(o);
this.getRoot().add(R,{left:320,top:60});
R.setWidth(300);
R.setHeight(220);
R.setLayout(new qx.ui.layout.Grid(0,5));
R.add(new qx.ui.basic.Label(p),{row:0,column:0});
R.add(new qx.ui.basic.Label(r),{row:1,column:0});
R.add(new qx.ui.basic.Label(u),{row:2,column:0});
R.add(new qx.ui.basic.Label(l),{row:3,column:0});
...
O.addTarget(name,j,s);
O.addTarget(location,j,t);
O.addTarget(W,j,g);
O.addTarget(ba,j,d);

if(!qx.core.Variant.isSet(f,b)){R.add(new qx.ui.basic.Label(v),{row:4,column:0});
...
},configureItem:function(G){G.setRich(true);
G.getChildControl(i).setWidth(50);
G.getChildControl(i).setHeight(50);
G.getChildControl(i).setScale(true);
}}});
qx.Class.define(A,{extend:qx.data.store.Json,statics:{saveResult:function(I){this.__MN=I;
}},construct:function(K){var L=E+K+a;
qx.data.store.Json.call(this,L);
},members:{_createRequest:function(bb){var bc=new qx.io.ScriptLoader();
bb+=m;
bc.load(bb,function(J){this.__MO();
},this);
},__MO:function(){var bd=demobrowser.demo.data.store.Twitter.__MN;
