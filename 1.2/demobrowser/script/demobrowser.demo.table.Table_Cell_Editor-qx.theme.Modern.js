...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.table.Table_Cell_Editor","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.table.Table_Cell_Editor-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"197ee1b2fdcf"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
},createTable:function(){var bw=[[J,k,x,{'required':true}],[d,K,S,{'type':d,'required':true}],[L,A,T,{'options':[U,N,G],'editable':true}],[Q,v,a,{'options':[[n,null,a],[D,null,q],[w,null,E]]}],[e,z,y,{'type':e}],[l,O,M,{'regExp':W,'failMsg':Y}],[I,t,H,{'validationFunc':function(bN){if(bN.search(/^http:/)!=-1){window.open(bN);
}return bN;
}}],[V,m,true,{'type':j}]];
var bv=function(bg){var bm=bg.table;
var bj=bm.getTableModel();
var bk=bj.getRowData(bg.row);
var bn=bk[3];
var bl;

for(var bi in bn){switch(bi){case f:switch(bn[b]){case j:return new qx.ui.table.cellrenderer.Boolean;
case g:return new qx.ui.table.cellrenderer.Password;
}break;
case h:var bl=new qx.ui.table.cellrenderer.Replace;
var bh={};
bn[c].forEach(function(bc){if(bc instanceof Array){bh[bc[0]]=bc[2];
}});
bl.setReplaceMap(bh);
bl.addReversedReplaceMap();
return bl;
}}return new qx.ui.table.cellrenderer.Default();
};
var bu=new qx.ui.table.cellrenderer.Dynamic(bv);
var bs=function(bF){var bK=bF.table;
var bI=bK.getTableModel();
var bJ=bI.getRowData(bF.row);
var bM=bJ[3];
var bH=new qx.ui.table.celleditor.TextField;
var bL=null;

for(var bG in bM){switch(bG){case h:if(bM.editable){bH=new qx.ui.table.celleditor.ComboBox();
}else{bH=new qx.ui.table.celleditor.SelectBox();
}bH.setListData(bM[c]);
break;
case F:break;
case f:switch(bM[b]){case g:bH=new qx.ui.table.celleditor.PasswordField;
break;
case j:bH=new qx.ui.table.celleditor.CheckBox;
break;
case s:bH.setValidationFunction(function(bd,be){var bf=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{2}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/;

if(bf.test(bd)){return bd;
}alert("You did not enter a valid email address");
return be;
});
break;
}break;
case r:bH.setValidationFunction(function(bC,bD){var bE=new RegExp(bM[X]);

if(bE.test(bC)){return bC;
}alert(bM['failMsg']);
return bD;
});
break;
case P:bH.setValidationFunction(bM[B]);
break;
case o:bL=function(ba,bb){if(!ba){alert("You need to supply a value here");
return bb;
}return ba;
};
break;
...
var br=new qx.ui.table.celleditor.Dynamic(bs);
var bp=new qx.ui.table.model.Simple();
bp.setColumns([i,i,p]);
var bo={tableColumnModel:function(bB){return new qx.ui.table.columnmodel.Resize(bB);
}};
var bq=new qx.ui.table.Table(bp,bo);
bq.setDecorator(null);
bq.setColumnVisibilityButtonVisible(false);
bq.setKeepFirstVisibleRowComplete(true);
bq.setStatusBarVisible(false);
bq.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var bt=bq.getTableColumnModel();
bt.setColumnVisible(0,false);
bt.getBehavior().setWidth(1,100);
bt.getBehavior().setWidth(2,300);
bp.setColumnEditable(2,true);
bt.setDataCellRenderer(2,bu);
bt.setCellEditorFactory(2,br);
bq.getTableModel().setData(bw);
bq.getTableModel().addListener(u,function(event){if(!(event instanceof qx.event.type.Data)){return;
}var bA=event.getData();
var bx=this.getTableModel();
var bz=bx.getValue(0,bA.firstRow);
var by=bx.getValue(bA.firstColumn,bA.firstRow);
this.info("User edited property '"+bz+"' and entered value '"+by+"'.");
},bq);
return bq;
}}});
})();
...
qx.Class.define(g,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,members:{createCellEditor:function(k){var m=new qx.ui.container.Composite(new qx.ui.layout.HBox().set({alignX:d,alignY:b})).set({focusable:true});
var l=new qx.ui.form.CheckBox().set({value:k.value});
m.add(l);
m.addListener(f,function(){l.focus();
});
m.addListener(a,function(){l.activate();
});
l.addListener(c,function(e){if(e.getKeyIdentifier()==h){var n=qx.event.Pool.getInstance().getObject(qx.event.type.KeySequence);
var o=m.getContainerElement().getDomElement();
...
}},properties:{cellEditorFactoryFunction:{check:b,nullable:true,init:null}},members:{__GY:null,createCellEditor:function(c){var d=this.getCellEditorFactoryFunction();
{};
this.__GY=d(c);
var e=this.__GY.createCellEditor(c);
return e;
},getCellEditorValue:function(g){var h=this.getCellEditorFactoryFunction();
{};
var i=this.__GY.getCellEditorValue(g);
return i;
