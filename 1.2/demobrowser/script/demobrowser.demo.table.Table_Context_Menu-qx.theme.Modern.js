...
},createTable:function(){var bc=this.createRandomRows(50);
var bb=this._tableModel=new qx.ui.table.model.Simple();
bb.setColumns([s,a,f,B]);
bb.setData(bc);
var bd=new qx.ui.table.Table(bb);
bd.set({width:600,height:400,decorator:null});
var ba=bd.getTableColumnModel();
ba.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
bd.setContextMenuHandler(3,this._contextMenuHandlerBoolean);
bd.setContextMenuHandler(2,this._contextMenuHandlerNull);
return bd;
},_contextMenuHandlerBoolean:function(bf,bg,bh,bi,bj){var bk;

for(var i=0;i<=1;i++){bk=new qx.ui.menu.Button(i?c:o);
bk.setUserData(j,i?true:false);
bk.addListener(l,function(e){bi.setValue(bf,bg,this.getUserData(j));
});
bj.add(bk);
}return true;
},_contextMenuHandlerNull:function(Q,R,S,T,U){return true;
},createControls:function(){var bq=new qx.ui.toolbar.ToolBar();
var bo,bp,br;
bp=new qx.ui.toolbar.Part();
bq.add(bp);
bo=new qx.ui.toolbar.Button(b,h);
bo.addListener(l,function(I){var J=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,J[0][i]);
}this.info("Row 10 changed");
},this);
bp.add(bo);
bo=new qx.ui.toolbar.Button(C,t);
bo.addListener(l,function(bl){var bm=this.createRandomRows(10);
this._tableModel.addRows(bm);
this.info("10 rows added");
},this);
bp.add(bo);
bo=new qx.ui.toolbar.Button(D,x);
bo.addListener(l,function(bs){var bt=this._tableModel.getRowCount();
this._tableModel.removeRows(bt-5,5);
this.info("5 rows removed");
},this);
bp.add(bo);
bo=new qx.ui.toolbar.Button(m,d);
bo.addListener(l,function(K){var L=[];
bn.getSelectionModel().iterateSelection(function(H){L.push(H+A);
});
this.showDialog(q+L.join(v));
},this);
bp.add(bo);
bp=new qx.ui.toolbar.Part();
bq.add(bp);
var bn=this._table;
br=new qx.ui.toolbar.CheckBox(r);
br.set({value:bn.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(w+p)});
br.addListener(k,function(be){bn.setKeepFirstVisibleRowComplete(this.getValue());
},br);
bp.add(br);
br=new qx.ui.toolbar.CheckBox(g);
br.set({value:bn.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(n)});
br.addListener(k,function(E){if(E.getData()){var G=function(M,N){var O=M[arguments.callee.columnIndex];
var P=N[arguments.callee.columnIndex];

if(O%2==1&&P%2==0){return 1;
}
if(P%2==1&&O%2==0){return -1;
}return (O>P)?1:((O==P)?0:-1);
};
var F=function(V,W){var X=V[arguments.callee.columnIndex];
...
bn.getTableModel().setSortMethods(0,{ascending:G,descending:F});
}else{bn.getTableModel().setSortMethods(0,null);
}},br);
bp.add(br);
return bq;
...
this.__DN=[];
},members:{__DN:null,setContextMenuHandler:function(j,k){this.__DN[j]=k;
},getContextMenuHandler:function(i){return this.__DN[i];
