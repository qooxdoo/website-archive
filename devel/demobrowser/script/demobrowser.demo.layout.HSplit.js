...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.HSplit.4c99e498f8f8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,properties:{appearance:{refine:true,init:o},offset:{check:d,init:6,apply:n},orientation:{init:u,check:[u,k],apply:m}},members:{__pv:null,__pw:false,__px:null,__py:null,__pz:null,__pA:null,__pB:null,__hd:null,__mf:null,_createChildControlImpl:function(H,G){var F;switch(H){case a:F=new qx.ui.splitpane.Slider(this);F.exclude();this._add(F,{type:H});break;case b:F=new qx.ui.splitpane.Splitter(this);this._add(F,{type:H});F.addListener(g,this.__pC,this);break;};return F||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,H);}
,__pC:function(e){this.__pE(e.getData());}
,__pD:function(K){this.__mf=new qx.ui.splitpane.Blocker(K);this.getContentElement().add(this.__mf);var I=this.getChildControl(b);var J=I.getWidth();if(!J){I.addListenerOnce(w,function(){this.__pE();}
,this);}
...
,_applyOrientation:function(Q,N){var R=this.getChildControl(a);var M=this.getChildControl(b);this.__pz=Q===u;if(!this.__mf){this.__pD(Q);}
;this.__mf.setOrientation(Q);var P=this._getLayout();if(P){P.dispose();}
;var O=Q===k?new qx.ui.splitpane.VLayout:new qx.ui.splitpane.HLayout;this._setLayout(O);M.removeState(N);M.addState(Q);M.getChildControl(t).removeState(N);M.getChildControl(t).addState(Q);R.removeState(N);R.addState(Q);qx.ui.core.queue.Manager.flush();this.__pE();}
,_applyOffset:function(T,S){this.__pE();}
,__pE:function(V){var U=this.getChildControl(b);var ba=this.getOffset();var bb=U.getBounds();var Y=U.getContentElement().getDomElement();if(!Y){return;}
;if(this.__pz){var X=null;if(V){X=V.width;}
...
;var bg=this.getChildControl(b);var bj=bg.getContentLocation();var bh=this.getContentLocation();this.__pv=this.__pz?e.getDocumentLeft()-bj.left+bh.left:e.getDocumentTop()-bj.top+bh.top;var bi=this.getChildControl(a);var bk=bg.getBounds();bi.setUserBounds(bk.left,bk.top,bk.width,bk.height);bi.setZIndex(bg.getZIndex()+1);bi.show();this.__pw=true;this.__mf.capture();e.stop();}
,_onPointerMove:function(e){this._setLastPointerPosition(e.getDocumentLeft(),e.getDocumentTop());if(this.__pw){this.__pF();var bl=this.getChildControl(a);var bm=this.__pA;if(this.__pz){bl.setDomLeft(bm);this.__mf.setStyle(A,(bm-this.getOffset())+c);}
...
,_onPointerUp:function(e){if(!this.__pw){return;}
;this._finalizeSizes();var bn=this.getChildControl(a);bn.exclude();this.__pw=false;this.releaseCapture();e.stop();}
,_finalizeSizes:function(){var br=this.__pA;var bo=this.__pB;if(br==null){return;}
...
else {if(this.__pz){bs.setWidth(br);bq.setWidth(bo);}
else {bs.setHeight(br);bq.setHeight(bo);}
;}
;}
,__pF:function(){if(this.__pz){var bx=v,bE=l,by=C,bC=this.__px;}
else {var bx=s,bE=f,by=h,bC=this.__py;}
;var bD=this._getChildren();var bv=bD[2].getSizeHint();var bA=bD[3].getSizeHint();var bB=bD[2].getBounds()[bE]+bD[3].getBounds()[bE];var bz=bC-this.__pv;var bw=bB-bz;if(bz<bv[bx]){bw-=bv[bx]-bz;bz=bv[bx];}
else if(bw<bA[bx]){bz-=bA[bx]-bw;bw=bA[bx];}
;if(bz>bv[by]){bw+=bz-bv[by];bz=bv[by];}
else if(bw>bA[by]){bz+=bw-bA[by];bw=bA[by];}
;this.__pA=bz;this.__pB=bw;}
,_isActiveDragSession:function(){return this.__pw;}
,_setLastPointerPosition:function(x,y){this.__px=x;this.__py=y;}
