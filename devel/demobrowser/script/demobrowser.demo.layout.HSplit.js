...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.HSplit.34550f5c1d35.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;this.__mk.addListener(q,this._onPointerDown,this);this.__mk.addListener(j,this._onPointerUp,this);this.__mk.addListener(B,this._onPointerMove,this);this.__mk.addListener(D,this._onPointerOut,this);this.__mk.addListener(z,this._onPointerUp,this);}
,properties:{appearance:{refine:true,init:o},offset:{check:d,init:6,apply:n},orientation:{init:u,check:[u,k],apply:m}},members:{__pA:null,__pB:false,__pC:null,__pD:null,__pE:null,__pF:null,__pG:null,__hi:null,__mk:null,_createChildControlImpl:function(H,G){var F;switch(H){case a:F=new qx.ui.splitpane.Slider(this);F.exclude();this._add(F,{type:H});break;case b:F=new qx.ui.splitpane.Splitter(this);this._add(F,{type:H});F.addListener(g,this.__pH,this);break;};return F||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,H);}
,__pH:function(e){this.__pJ(e.getData());}
,__pI:function(K){this.__mk=new qx.ui.splitpane.Blocker(K);this.getContentElement().add(this.__mk);var I=this.getChildControl(b);var J=I.getWidth();if(!J){I.addListenerOnce(w,function(){this.__pJ();}
,this);}
;I.addListener(i,function(e){var L=e.getData();if(L.height==0||L.width==0){this.__mk.hide();}
else {this.__mk.show();}
;}
,this);}
,getBlocker:function(){return this.__mk;}
,_applyOrientation:function(Q,N){var R=this.getChildControl(a);var M=this.getChildControl(b);this.__pE=Q===u;if(!this.__mk){this.__pI(Q);}
;this.__mk.setOrientation(Q);var P=this._getLayout();if(P){P.dispose();}
;var O=Q===k?new qx.ui.splitpane.VLayout:new qx.ui.splitpane.HLayout;this._setLayout(O);M.removeState(N);M.addState(Q);M.getChildControl(t).removeState(N);M.getChildControl(t).addState(Q);R.removeState(N);R.addState(Q);qx.ui.core.queue.Manager.flush();this.__pJ();}
,_applyOffset:function(T,S){this.__pJ();}
,__pJ:function(V){var U=this.getChildControl(b);var ba=this.getOffset();var bb=U.getBounds();var Y=U.getContentElement().getDomElement();if(!Y){return;}
;if(this.__pE){var X=null;if(V){X=V.width;}
else if(bb){X=bb.width;}
;var bc=V&&V.left;if(X){if(isNaN(bc)){bc=qx.bom.element.Location.getPosition(Y).left;}
;this.__mk.setWidth(ba,X);this.__mk.setLeft(ba,bc);}
;}
...
;this.__mk.setHeight(ba,W);this.__mk.setTop(ba,top);}
;}
;}
...
;var bg=this.getChildControl(b);var bj=bg.getContentLocation();var bh=this.getContentLocation();this.__pA=this.__pE?e.getDocumentLeft()-bj.left+bh.left:e.getDocumentTop()-bj.top+bh.top;var bi=this.getChildControl(a);var bk=bg.getBounds();bi.setUserBounds(bk.left,bk.top,bk.width,bk.height);bi.setZIndex(bg.getZIndex()+1);bi.show();this.__pB=true;this.__mk.capture();e.stop();}
,_onPointerMove:function(e){this._setLastPointerPosition(e.getDocumentLeft(),e.getDocumentTop());if(this.__pB){this.__pK();var bl=this.getChildControl(a);var bm=this.__pF;if(this.__pE){bl.setDomLeft(bm);this.__mk.setStyle(A,(bm-this.getOffset())+c);}
else {bl.setDomTop(bm);this.__mk.setStyle(r,(bm-this.getOffset())+c);}
;e.stop();}
;}
,_onPointerOut:function(e){this._setLastPointerPosition(e.getDocumentLeft(),e.getDocumentTop());}
,_onPointerUp:function(e){if(!this.__pB){return;}
;this._finalizeSizes();var bn=this.getChildControl(a);bn.exclude();this.__pB=false;this.releaseCapture();e.stop();}
,_finalizeSizes:function(){var br=this.__pF;var bo=this.__pG;if(br==null){return;}
...
else {if(this.__pE){bs.setWidth(br);bq.setWidth(bo);}
else {bs.setHeight(br);bq.setHeight(bo);}
;}
;}
,__pK:function(){if(this.__pE){var bx=v,bE=l,by=C,bC=this.__pC;}
else {var bx=s,bE=f,by=h,bC=this.__pD;}
;var bD=this._getChildren();var bv=bD[2].getSizeHint();var bA=bD[3].getSizeHint();var bB=bD[2].getBounds()[bE]+bD[3].getBounds()[bE];var bz=bC-this.__pA;var bw=bB-bz;if(bz<bv[bx]){bw-=bv[bx]-bz;bz=bv[bx];}
else if(bw<bA[bx]){bz-=bA[bx]-bw;bw=bA[bx];}
;if(bz>bv[by]){bw+=bz-bv[by];bz=bv[by];}
else if(bw>bA[by]){bz+=bw-bA[by];bw=bA[by];}
;this.__pF=bz;this.__pG=bw;}
,_isActiveDragSession:function(){return this.__pB;}
,_setLastPointerPosition:function(x,y){this.__pC=x;this.__pD=y;}
