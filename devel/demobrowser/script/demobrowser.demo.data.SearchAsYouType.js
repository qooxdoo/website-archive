...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType.8c26e7777c11.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="_applySearchString",b="demobrowser.demo.data.filter.SearchAsYouTypeFilter",c="String",d="";qx.Class.define(b,{extend:qx.core.Object,construct:function(e){qx.core.Object.call(this);this.__P=e;this.filter=qx.lang.Function.bind(function(f){return f.search(this.getSearchString())!=-1;}
,this);this.__Q=null;}
,properties:{searchString:{check:c,apply:a,init:d}},members:{__Q:null,__P:null,_applySearchString:function(h,g){var i=qx.util.TimerManager.getInstance();if(this.__Q!=null){i.stop(this.__Q);this.__Q=null;}
;this.__Q=i.start(function(){this.__P.update();this.__Q=null;}
,0,this,null,200);}
,filter:null},destruct:function(){this.__P=null;}
});}
)();
(function(){var a="number",b="qx.util.TimerManager",c="interval",d="singleton";qx.Class.define(b,{extend:qx.core.Object,type:d,statics:{__yg:[],__yh:{},__sG:0},members:{__yi:false,start:function(h,g,j,e,k){if(typeof k!=a){k=g||0;}
;var f=(new Date()).getTime()+k;this.self(arguments).__yh[ ++this.self(arguments).__sG]={callback:h,userData:e||null,expireAt:f,recurTime:g,context:j||this};this.__yj(f,this.self(arguments).__sG);return this.self(arguments).__sG;}
,stop:function(l){var m=this.self(arguments).__yg;var length=m.length;for(var i=0;i<length;i++ ){if(m[i]==l){m.splice(i,1);break;}
;}
;delete this.self(arguments).__yh[l];if(m.length==0&&this.__yi){qx.event.Idle.getInstance().removeListener(c,this.__yk,this);this.__yi=false;}
;}
,__yj:function(n,q){var p=this.self(arguments).__yg;var o=this.self(arguments).__yh;var length=p.length;for(var i=0;i<length;i++ ){if(o[p[i]].expireAt>n){p.splice(i,0,q);break;}
;}
;if(p.length==length){p.push(q);}
;if(!this.__yi){qx.event.Idle.getInstance().addListener(c,this.__yk,this);this.__yi=true;}
;}
,__yk:function(){var s=(new Date()).getTime();var u=this.self(arguments).__yg;var t=this.self(arguments).__yh;while(u.length>0&&t[u[0]].expireAt<=s){var w=u.shift();var r=t[w];r.callback.call(r.context,r.userData,w);if(r.recurTime&&t[w]){var v=(new Date()).getTime();r.expireAt=v+r.recurTime;this.__yj(r.expireAt,w);}
else {delete t[w];}
;}
;if(u.length==0&&this.__yi){qx.event.Idle.getInstance().removeListener(c,this.__yk,this);this.__yi=false;}
;}
}});}
)();
