...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType.1b6c1778bddc.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="number",b="qx.util.TimerManager",c="interval",d="singleton";qx.Class.define(b,{extend:qx.core.Object,type:d,statics:{__xW:[],__xX:{},__sE:0},members:{__xY:false,start:function(h,g,j,e,k){if(typeof k!=a){k=g||0;}
;var f=(new Date()).getTime()+k;this.self(arguments).__xX[ ++this.self(arguments).__sE]={callback:h,userData:e||null,expireAt:f,recurTime:g,context:j||this};this.__ya(f,this.self(arguments).__sE);return this.self(arguments).__sE;}
,stop:function(l){var m=this.self(arguments).__xW;var length=m.length;for(var i=0;i<length;i++ ){if(m[i]==l){m.splice(i,1);break;}
;}
;delete this.self(arguments).__xX[l];if(m.length==0&&this.__xY){qx.event.Idle.getInstance().removeListener(c,this.__yb,this);this.__xY=false;}
;}
,__ya:function(n,q){var p=this.self(arguments).__xW;var o=this.self(arguments).__xX;var length=p.length;for(var i=0;i<length;i++ ){if(o[p[i]].expireAt>n){p.splice(i,0,q);break;}
;}
;if(p.length==length){p.push(q);}
;if(!this.__xY){qx.event.Idle.getInstance().addListener(c,this.__yb,this);this.__xY=true;}
;}
,__yb:function(){var s=(new Date()).getTime();var u=this.self(arguments).__xW;var t=this.self(arguments).__xX;while(u.length>0&&t[u[0]].expireAt<=s){var w=u.shift();var r=t[w];r.callback.call(r.context,r.userData,w);if(r.recurTime&&t[w]){var v=(new Date()).getTime();r.expireAt=v+r.recurTime;this.__ya(r.expireAt,w);}
else {delete t[w];}
;}
;if(u.length==0&&this.__xY){qx.event.Idle.getInstance().removeListener(c,this.__yb,this);this.__xY=false;}
;}
}});}
)();
