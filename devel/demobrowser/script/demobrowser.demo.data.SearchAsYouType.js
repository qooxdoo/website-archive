...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType.65454d0e2d13.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var f=(new Date()).getTime()+k;this.self(arguments).__xM[ ++this.self(arguments).__st]={callback:h,userData:e||null,expireAt:f,recurTime:g,context:j||this};this.__xO(f,this.self(arguments).__st);return this.self(arguments).__st;}
,stop:function(l){var m=this.self(arguments).__xL;var length=m.length;for(var i=0;i<length;i++ ){if(m[i]==l){m.splice(i,1);break;}
;}
;delete this.self(arguments).__xM[l];if(m.length==0&&this.__xN){qx.event.Idle.getInstance().removeListener(c,this.__xP,this);this.__xN=false;}
;}
,__xO:function(n,q){var p=this.self(arguments).__xL;var o=this.self(arguments).__xM;var length=p.length;for(var i=0;i<length;i++ ){if(o[p[i]].expireAt>n){p.splice(i,0,q);break;}
;}
;if(p.length==length){p.push(q);}
;if(!this.__xN){qx.event.Idle.getInstance().addListener(c,this.__xP,this);this.__xN=true;}
;}
,__xP:function(){var s=(new Date()).getTime();var u=this.self(arguments).__xL;var t=this.self(arguments).__xM;while(u.length>0&&t[u[0]].expireAt<=s){var w=u.shift();var r=t[w];r.callback.call(r.context,r.userData,w);if(r.recurTime&&t[w]){var v=(new Date()).getTime();r.expireAt=v+r.recurTime;this.__xO(r.expireAt,w);}
else {delete t[w];}
;}
;if(u.length==0&&this.__xN){qx.event.Idle.getInstance().removeListener(c,this.__xP,this);this.__xN=false;}
;}
}});}
)();
