...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType.740ff93c5c99.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="_applySearchString",b="demobrowser.demo.data.filter.SearchAsYouTypeFilter",c="String",d="";qx.Class.define(b,{extend:qx.core.Object,construct:function(e){qx.core.Object.call(this);this.__bS=e;this.filter=qx.lang.Function.bind(function(f){return f.search(this.getSearchString())!=-1;}
,this);this.__bT=null;}
,properties:{searchString:{check:c,apply:a,init:d}},members:{__bT:null,__bS:null,_applySearchString:function(h,g){var i=qx.util.TimerManager.getInstance();if(this.__bT!=null){i.stop(this.__bT);this.__bT=null;}
;this.__bT=i.start(function(){this.__bS.update();this.__bT=null;}
,0,this,null,200);}
,filter:null},destruct:function(){this.__bS=null;}
});}
)();
(function(){var a="number",b="qx.util.TimerManager",c="interval",d="singleton";qx.Class.define(b,{extend:qx.core.Object,type:d,statics:{__Bc:[],__Bd:{},__sG:0},members:{__Be:false,start:function(h,g,j,e,k){if(typeof k!=a){k=g||0;}
;var f=(new Date()).getTime()+k;this.self(arguments).__Bd[ ++this.self(arguments).__sG]={callback:h,userData:e||null,expireAt:f,recurTime:g,context:j||this};this.__Bf(f,this.self(arguments).__sG);return this.self(arguments).__sG;}
,stop:function(l){var m=this.self(arguments).__Bc;var length=m.length;for(var i=0;i<length;i++ ){if(m[i]==l){m.splice(i,1);break;}
;}
;delete this.self(arguments).__Bd[l];if(m.length==0&&this.__Be){qx.event.Idle.getInstance().removeListener(c,this.__Bg,this);this.__Be=false;}
;}
,__Bf:function(n,q){var p=this.self(arguments).__Bc;var o=this.self(arguments).__Bd;var length=p.length;for(var i=0;i<length;i++ ){if(o[p[i]].expireAt>n){p.splice(i,0,q);break;}
;}
;if(p.length==length){p.push(q);}
;if(!this.__Be){qx.event.Idle.getInstance().addListener(c,this.__Bg,this);this.__Be=true;}
;}
,__Bg:function(){var s=(new Date()).getTime();var u=this.self(arguments).__Bc;var t=this.self(arguments).__Bd;while(u.length>0&&t[u[0]].expireAt<=s){var w=u.shift();var r=t[w];r.callback.call(r.context,r.userData,w);if(r.recurTime&&t[w]){var v=(new Date()).getTime();r.expireAt=v+r.recurTime;this.__Bf(r.expireAt,w);}
else {delete t[w];}
;}
;if(u.length==0&&this.__Be){qx.event.Idle.getInstance().removeListener(c,this.__Bg,this);this.__Be=false;}
;}
}});}
)();
