...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType.089b90389f14.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="number",b="qx.util.TimerManager",c="interval",d="singleton";qx.Class.define(b,{extend:qx.core.Object,type:d,statics:{__xR:[],__xS:{},__sA:0},members:{__xT:false,start:function(h,g,j,e,k){if(typeof k!=a){k=g||0;}
;var f=(new Date()).getTime()+k;this.self(arguments).__xS[ ++this.self(arguments).__sA]={callback:h,userData:e||null,expireAt:f,recurTime:g,context:j||this};this.__xU(f,this.self(arguments).__sA);return this.self(arguments).__sA;}
,stop:function(l){var m=this.self(arguments).__xR;var length=m.length;for(var i=0;i<length;i++ ){if(m[i]==l){m.splice(i,1);break;}
;}
;delete this.self(arguments).__xS[l];if(m.length==0&&this.__xT){qx.event.Idle.getInstance().removeListener(c,this.__xV,this);this.__xT=false;}
;}
,__xU:function(n,q){var p=this.self(arguments).__xR;var o=this.self(arguments).__xS;var length=p.length;for(var i=0;i<length;i++ ){if(o[p[i]].expireAt>n){p.splice(i,0,q);break;}
;}
;if(p.length==length){p.push(q);}
;if(!this.__xT){qx.event.Idle.getInstance().addListener(c,this.__xV,this);this.__xT=true;}
;}
,__xV:function(){var s=(new Date()).getTime();var u=this.self(arguments).__xR;var t=this.self(arguments).__xS;while(u.length>0&&t[u[0]].expireAt<=s){var w=u.shift();var r=t[w];r.callback.call(r.context,r.userData,w);if(r.recurTime&&t[w]){var v=(new Date()).getTime();r.expireAt=v+r.recurTime;this.__xU(r.expireAt,w);}
else {delete t[w];}
;}
;if(u.length==0&&this.__xT){qx.event.Idle.getInstance().removeListener(c,this.__xV,this);this.__xT=false;}
;}
}});}
)();
