...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType.a0f74dc90579.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="_applySearchString",b="demobrowser.demo.data.filter.SearchAsYouTypeFilter",c="String",d="";qx.Class.define(b,{extend:qx.core.Object,construct:function(e){qx.core.Object.call(this);this.__ul=e;this.filter=qx.lang.Function.bind(function(f){return f.search(this.getSearchString())!=-1;}
,this);this.__va=null;}
,properties:{searchString:{check:c,apply:a,init:d}},members:{__va:null,__ul:null,_applySearchString:function(h,g){var i=qx.util.TimerManager.getInstance();if(this.__va!=null){i.stop(this.__va);this.__va=null;}
;this.__va=i.start(function(){this.__ul.update();this.__va=null;}
,0,this,null,200);}
,filter:null},destruct:function(){this.__ul=null;}
});}
)();
(function(){var a="number",b="qx.util.TimerManager",c="interval",d="singleton";qx.Class.define(b,{extend:qx.core.Object,type:d,statics:{__xY:[],__ya:{},__sH:0},members:{__yb:false,start:function(h,g,j,e,k){if(typeof k!=a){k=g||0;}
;var f=(new Date()).getTime()+k;this.self(arguments).__ya[ ++this.self(arguments).__sH]={callback:h,userData:e||null,expireAt:f,recurTime:g,context:j||this};this.__yc(f,this.self(arguments).__sH);return this.self(arguments).__sH;}
,stop:function(l){var m=this.self(arguments).__xY;var length=m.length;for(var i=0;i<length;i++ ){if(m[i]==l){m.splice(i,1);break;}
;}
;delete this.self(arguments).__ya[l];if(m.length==0&&this.__yb){qx.event.Idle.getInstance().removeListener(c,this.__yd,this);this.__yb=false;}
;}
,__yc:function(n,q){var p=this.self(arguments).__xY;var o=this.self(arguments).__ya;var length=p.length;for(var i=0;i<length;i++ ){if(o[p[i]].expireAt>n){p.splice(i,0,q);break;}
;}
;if(p.length==length){p.push(q);}
;if(!this.__yb){qx.event.Idle.getInstance().addListener(c,this.__yd,this);this.__yb=true;}
;}
,__yd:function(){var s=(new Date()).getTime();var u=this.self(arguments).__xY;var t=this.self(arguments).__ya;while(u.length>0&&t[u[0]].expireAt<=s){var w=u.shift();var r=t[w];r.callback.call(r.context,r.userData,w);if(r.recurTime&&t[w]){var v=(new Date()).getTime();r.expireAt=v+r.recurTime;this.__yc(r.expireAt,w);}
else {delete t[w];}
;}
;if(u.length==0&&this.__yb){qx.event.Idle.getInstance().removeListener(c,this.__yd,this);this.__yb=false;}
;}
}});}
)();
