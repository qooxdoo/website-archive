...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType.16c19df90fe3.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="number",b="qx.util.TimerManager",c="interval",d="singleton";qx.Class.define(b,{extend:qx.core.Object,type:d,statics:{__xJ:[],__xK:{},__ss:0},members:{__xL:false,start:function(h,g,j,e,k){if(typeof k!=a){k=g||0;}
;var f=(new Date()).getTime()+k;this.self(arguments).__xK[ ++this.self(arguments).__ss]={callback:h,userData:e||null,expireAt:f,recurTime:g,context:j||this};this.__xM(f,this.self(arguments).__ss);return this.self(arguments).__ss;}
,stop:function(l){var m=this.self(arguments).__xJ;var length=m.length;for(var i=0;i<length;i++ ){if(m[i]==l){m.splice(i,1);break;}
;}
;delete this.self(arguments).__xK[l];if(m.length==0&&this.__xL){qx.event.Idle.getInstance().removeListener(c,this.__xN,this);this.__xL=false;}
;}
,__xM:function(n,q){var p=this.self(arguments).__xJ;var o=this.self(arguments).__xK;var length=p.length;for(var i=0;i<length;i++ ){if(o[p[i]].expireAt>n){p.splice(i,0,q);break;}
;}
;if(p.length==length){p.push(q);}
;if(!this.__xL){qx.event.Idle.getInstance().addListener(c,this.__xN,this);this.__xL=true;}
;}
,__xN:function(){var s=(new Date()).getTime();var u=this.self(arguments).__xJ;var t=this.self(arguments).__xK;while(u.length>0&&t[u[0]].expireAt<=s){var w=u.shift();var r=t[w];r.callback.call(r.context,r.userData,w);if(r.recurTime&&t[w]){var v=(new Date()).getTime();r.expireAt=v+r.recurTime;this.__xM(r.expireAt,w);}
else {delete t[w];}
;}
;if(u.length==0&&this.__xL){qx.event.Idle.getInstance().removeListener(c,this.__xN,this);this.__xL=false;}
;}
}});}
)();
