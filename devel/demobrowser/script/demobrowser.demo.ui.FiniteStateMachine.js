...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.fc4e0b43b667.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="TERMINATED",c=" => ",d="#autoActionsBeforeOnexit",f=": Could not find friendly name for '",g=": event '",h=" blocked.  Re-queuing.",j="string",k="qx.event.type.Data",l="'",m="Attempt to transition to nonexistent state ",n="Saved-state stack is empty",o="Internal error: invalid nextState",p=" not handled.  Ignoring.",q="State ",r="#autoActionsBeforeOntransition",s=" returned a value other than ",u="#",v=" already exists",w="Attempt to transition to POP_STATE_STACK ",x=":",y="Transition ",z=" does not exist",A="' on '",B="qx.util.fsm.FiniteStateMachine",C="String",D="terminated",E="#autoActionsAfterOntransition",F="",G="#actionsAfterOnentry",H="Saved-state stack is full",I="#autoActionsBeforeOnentry",J="__Bf",K=": no transition found.  No state change.",L="#autoActionsAfterOnentry",M="#actionsBeforeOnentry",N="', because the finite state machine is not running.",O=": Pushed event: ",P=": Cannot listen to event '",Q="Machine started with no available states",R=": Process event: ",S="#entry",T="#ontransition",U=" on ",V="Invalid state: not an instance of ",W="Number",X="#exit",Y="true, false, or null.",bp="Explicit transition ",bq="  ",br="qx.util.fsm.State",bl="__Bg",bm="while state stack is empty.",bn=".  Ignoring.",bo="#autoActionsAfterOnexit",bs=": Event '",bt="object",bu=" not handled for target ",bv=": Queued event: ";qx.Class.define(B,{extend:qx.core.Object,construct:function(bw){qx.core.Object.call(this);this.setName(bw);this.__jO={};this.__Bd=null;this.__Be=[];this.__Bf=[];this.__Bg=[];this.__Bh={};this.__Bi={};this.__Bj={};this.__Bk={};this.__Bl={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},events:{"terminated":k},properties:{name:{check:C,nullable:true},state:{check:C,nullable:true},previousState:{check:C,nullable:true},nextState:{check:C,nullable:true},maxSavedStates:{check:W,init:2},debugFlags:{check:W,init:7}},members:{__jO:null,__Bd:null,__Bf:null,__Bg:null,__Be:null,__Bh:null,__Bi:null,__Bj:null,__Bk:null,__Bl:null,__Bm:false,__Bn:true,isTerminated:function(){return this.__Bn;}
,addState:function(bx){if(!bx instanceof qx.util.fsm.State){throw new Error(V+br);}
;var by=bx.getName();if(by in this.__jO){throw new Error(q+by+v);}
;if(this.__Bd==null){this.__Bd=by;}
;this.__jO[by]=bx;}
,replaceState:function(bC,bB){if(!bC instanceof qx.util.fsm.State){throw new Error(V+br);}
;var bA=bC.getName();var bz=this.__jO[bA];this.__jO[bA]=bC;if(bB){bz._bNeedDispose=true;}
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__Bi[bG]=bH;this.__Bj[bH]=bG;this.__Bh[bG]=bD;if(!bE){return;}
;if(typeof (bE)==j){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__Bk[bF]){this.__Bk[bF]={};}
;this.__Bk[bF][bG]=true;if(!this.__Bl[bG]){this.__Bl[bG]=[];}
;this.__Bl[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__Bi[bL];if(this.__Bl[bL]){for(var i=0;i<this.__Bl[bL].length;i++ ){bJ=this.__Bl[bL][i];delete this.__Bk[bJ][bL];bM=true;for(bK in this.__Bk[bJ]){bM=false;break;}
;if(bM){delete this.__Bk[bJ];}
;}
;delete this.__Bl[bL];}
;delete this.__Bj[bI];delete this.__Bi[bL];delete this.__Bh[bL];}
,getObject:function(bN){return this.__Bh[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__Bj[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__Bk[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__Bi){var bT=this.__Bi[bR];var bS=this.getObject(bR);this.debug(bR+c+bT);this.debug(bq+bT+c+this.__Bj[bT]);this.debug(bq+bR+c+this.getObject(bR));this.debug(bq+this.getObject(bR)+c+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__jO,"startState":this.__Bd,"eventQueue":this.__Bf,"blockedEvents":this.__Bg,"savedStates":this.__Be,"friendlyToObject":this.__Bh,"friendlyToHash":this.__Bi,"hashToFriendly":this.__Bj,"groupToFriendly":this.__Bk,"friendlyToGroups":this.__Bl});}
,start:function(){this.__Bn=false;var bV=this.__Bd;if(bV==null){throw new Error(Q);}
...
;this.__jO[bV].getAutoActionsBeforeOnentry()(this);if(bU){this.debug(this.getName()+u+bV+S);}
;this.__jO[bV].getOnentry()(this,null);if(bU){this.debug(this.getName()+u+bV+G);}
;this.__jO[bV].getAutoActionsAfterOnentry()(this);}
,pushState:function(bW){if(this.__Be.length>=this.getMaxSavedStates()){throw new Error(H);}
;if(bW===true){this.__Be.push(this.getState());}
else if(bW){this.__Be.push(bW);}
else {this.__Be.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Be.length==0){throw new Error(n);}
;return this.__Be.pop();}
,postponeEvent:function(event){this.__Bg.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__Bf.push(event);}
else {this.__Bf.unshift(event);}
;if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){var bY=this.getFriendlyName(event.getTarget());if(bX){this.debug(this.getName()+O+event.getType()+(bY?U+bY:F));}
else {this.debug(this.getName()+bv+event.getType()+(bY?U+bY:F));}
;}
;}
,eventListener:function(event){if(this.__Bn){this.debug(this.getName()+P+event.getType()+N);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__Bo();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__Bn){this.debug(this.getName()+P+cb+N);return;}
...
,this,cg||20);}
,__Bo:function(){if(this.__Bm){return;}
;this.__Bm=true;while(this.__Bf.length>0){var event=this.__Bf.pop();var ch=this.__Bp(event);if(ch){event.dispose();}
;}
;this.__Bm=false;}
,__Bp:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+R+event.getType()+(ct?U+ct:F));}
;ck=this.getState();cn=this.__jO[ck];cq=cn.transitions;e=cn.getEvents()[event.getType()];if(!e){if(cu){this.debug(this.getName()+bs+event.getType()+l+p);}
;return true;}
;if(typeof (e)==bt){if(!ct){if(co){this.debug(this.getName()+f+event.getType()+A+event.getTarget()+l);}
;return true;}
...
;this.__Bg.unshift(event);return false;default:if(typeof (cp)==j){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bp+cp+z);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(y+ck+x+t+s+Y);};cl=cm.getNextState();if(typeof (cl)==j){if(!cl in this.__jO){throw new Error(m+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Be.length==0){throw new Error(w+bm);}
;cl=this.__Be.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__Bn=true;this.setNextState(null);break;default:throw new Error(o);};}
...
;cm.getOntransition()(this,event);if(cr){this.debug(this.getName()+u+ck+u+t+E);}
...
;cn.getOnexit()(this,event);if(cr){this.debug(this.getName()+u+ck+bo);}
;cn.getAutoActionsAfterOnexit()(this);if(cn._bNeedDispose){cn.dispose();}
;if(this.__Bn){if(cr){this.debug(this.getName()+u+b);}
;this.fireDataEvent(D,ck);return true;}
;cn=this.__jO[this.getNextState()];this.setPreviousState(ck);this.setState(this.getNextState());this.setNextState(null);cj=ck;ck=cl;cl=undefined;if(cr){this.debug(this.getName()+u+ck+I);}
...
;cn.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__Bg.length;i++ ){e=this.__Bg.pop();this.__Bf.unshift(e);}
;if(cs){this.debug(this.getName()+u+cj+c+this.getName()+u+ck);}
;return true;}
;if(cs){this.debug(this.getName()+u+ck+g+event.getType()+l+K);}
;return true;}
},destruct:function(){this._disposeArray(J);this._disposeArray(bl);this.__Be=this.__jO=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="Invalid state name",k="parameters",l=")",m="__Bx",n=" fsm.getObject('",o="groups",p="Invalid function parameters: ",q="Invalid onexit type: ",r="  for (var i = 0; i < groupObjects.length; i++)",s="context",t="__Br",u="string",v="autoActionsAfterOnentry",w="Invalid numeric value in events object: ",x='"',y="__Bt",z="__Bw",A="__Bq",B=" value: ",C="');",D="__Bu",E="State ",F="autoActionsBeforeOnexit",G="autoActionsBeforeOnentry",H="}",I="): ",J="events must be an object",K="onexit",L="{",M="Invalid function request type: ",N="__Bv",O=": ",P="__Bs",Q="expected array, found ",R="Invalid friendly name in 'objects' list: ",S="autoActionsAfterOnexit",T="  fsm.debug(ex);",U="(",V="Invalid onentry type: ",W="    fsm.getObject(objName).",X=")(fsm)",Y="events",ba="undefined",bb="Invalid ",bc="try",bd="    var objName = groupObjects[i];",be="Invalid value in events object: ",bf="').",bg="objects",bh="expected object, found ",bi="qx.util.fsm.Transition",bj="The events object must be provided in new state info",bk="catch(ex)",bl="  {",bm="    fsm.getGroupObjects('",bn="Invalid value in events object ",bo="Invalid transition: not an instance of ",bp="  }",bq="Invalid 'objects' list: expected array, got ",br="Invalid numeric value in events object ",bs="qx.util.fsm.State",bt="onentry",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(bs,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(s,bE);for(var bC in bF){switch(bC){case bt:this.setOnentry(this.__By(bF[bC],bE));break;case K:this.setOnexit(this.__By(bF[bC],bE));break;case G:this.setAutoActionsBeforeOnentry(bF[bC]);break;case v:this.setAutoActionsAfterOnentry(bF[bC]);break;case F:this.setAutoActionsBeforeOnexit(bF[bC]);break;case S:this.setAutoActionsAfterOnexit(bF[bC]);break;case Y:this.setEvents(bF[bC]);break;case s:break;default:this.setUserData(bC,bF[bC]);this.debug(E+bD+O+bz+bC);break;};}
;if(!this.getEvents()){throw new Error(bj);}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(actionType,value,context){if(typeof (value)!=bB){throw new Error(bb+actionType+B+typeof (value));}
;var funcFragment;var func=bc+L;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(M+Q+typeof (functionRequest));}
;for(var i=0;i<functionRequest.length;i++ ){objectAndGroupList=functionRequest[i];if(typeof (objectAndGroupList)!=bB){throw new Error(bv+bh+typeof (functionRequest[param]));}
;var params=objectAndGroupList[k];if(!params){params=[];}
else {if(!params instanceof Array){throw new Error(p+Q+typeof (params));}
;}
;funcFragment=f+U;for(var j=0;j<params.length;j++ ){if(j!=0){funcFragment+=by;}
;if(typeof (params[j])==c){funcFragment+=U+params[j]+X;}
else if(typeof (params[j])==u){funcFragment+=x+params[j]+x;}
else {funcFragment+=params[j];}
;}
;funcFragment+=l;var a=objectAndGroupList[bg];if(!a){a=[];}
else if(!a instanceof Array){throw new Error(bq+typeof (a));}
;for(var j=0;j<a.length;j++ ){if(typeof (a[j])!=u){throw new Error(R+a[j]);}
;func+=n+a[j]+bf+funcFragment+bu;}
;var g=objectAndGroupList[o];if(g){if(!g instanceof Array){throw new Error(bw+typeof (g));}
;for(j=0;j<g.length;j++ ){func+=d+bm+g[j]+C+r+bl+bd+W+funcFragment+bu+bp;}
;}
;}
;}
;func+=H+bk+L+T+H;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:A,nullable:true},onentry:{transform:t,nullable:true,init:function(bG,event){}
},onexit:{transform:P,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:D,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:N,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:z,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:m,nullable:true,init:function(bL,event){}
},events:{transform:y,nullable:true}},members:{__Bq:function(bM){if(typeof (bM)!=u||bM.length<1){throw new Error(h);}
;return bM;}
,__Br:function(bN){switch(typeof (bN)){case ba:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(s));default:throw new Error(V+typeof (bN));};}
,__Bs:function(bP){switch(typeof (bP)){case ba:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(s));default:throw new Error(q+typeof (bP));};}
,__Bt:function(bT){if(typeof (bT)!=bB){throw new Error(J);}
;for(var e in bT){var bR=bT[e];if(typeof (bR)==bA&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(w+e+O+bR);}
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==bA&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(br+U+e+I+bS+O+bR[bS]);}
else if(typeof (bR[bS])!=u&&typeof (bR[bS])!=bA){throw new Error(bn+U+e+I+bS+O+bR[bS]);}
;}
;}
else if(typeof (bR)!=u&&typeof (bR)!=bA){throw new Error(be+e+O+bT[e]);}
;}
;return bT;}
,__Bu:function(bU){return qx.util.fsm.State._commonTransformAutoActions(G,bU,this.getUserData(s));}
,__Bv:function(bV){return qx.util.fsm.State._commonTransformAutoActions(v,bV,this.getUserData(s));}
,__Bw:function(bW){return qx.util.fsm.State._commonTransformAutoActions(F,bW,this.getUserData(s));}
,__Bx:function(bX){return qx.util.fsm.State._commonTransformAutoActions(S,bX,this.getUserData(s));}
,__By:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bo+bi);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="__Bz",c="autoActionsBeforeOntransition",d="or one of the Fsm.StateChange constants",e="autoActionsAfterOntransition",g="context",h="string",i="predicate",j="__BB",k="nextState must be an explicit state name, ",l="Transition ",m=": ",n="String",o="__BD",p="number",q="Invalid transition nextState type: ",r="Invalid transition nextState value: ",s="Adding user-provided field to transition: ",t="ontransition",u="Invalid transition predicate type: ",v="boolean",w="nextState",x="Invalid ontransition type: ",y="__BC",z="__BA",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(g,E);for(var C in D){switch(C){case i:this.setPredicate(this.__By(D[C],E));break;case w:this.setNextState(D[C]);break;case c:this.setAutoActionsBeforeOntransition(this.__By(D[C],E));break;case e:this.setAutoActionsAfterOntransition(this.__By(D[C],E));break;case t:this.setOntransition(this.__By(D[C],E));break;case g:break;default:this.setUserData(C,D[C]);this.debug(l+F+m+s+C);break;};}
;}
,properties:{name:{check:n,nullable:true},predicate:{init:function(G,event){return true;}
,transform:b},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:z},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:j},autoActionsAfterOntransition:{init:function(I,event){}
,transform:y},ontransition:{init:function(J,event){}
,transform:o}},members:{__Bz:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
...
,__BA:function(N){switch(typeof (N)){case h:return N;case p:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(r+N+m+k+d);};break;default:throw new Error(q+typeof (N));};}
,__BB:function(O){return qx.util.fsm.State._commonTransformAutoActions(c,O,this.getUserData(g));}
,__BC:function(P){return qx.util.fsm.State._commonTransformAutoActions(e,P,this.getUserData(g));}
,__BD:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(g));default:throw new Error(x+typeof (Q));};}
,__By:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
