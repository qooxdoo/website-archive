...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.a41d5babb6bd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="TERMINATED",c="__Bo",d=" => ",f="#autoActionsBeforeOnexit",g=": Could not find friendly name for '",h=": event '",j="__Bn",k=" blocked.  Re-queuing.",l="string",m="qx.event.type.Data",n="'",o="Attempt to transition to nonexistent state ",p="Saved-state stack is empty",q="Internal error: invalid nextState",r=" not handled.  Ignoring.",s="State ",u="#autoActionsBeforeOntransition",v=" returned a value other than ",w="#",x=" already exists",y="Attempt to transition to POP_STATE_STACK ",z=":",A="Transition ",B=" does not exist",C="' on '",D="qx.util.fsm.FiniteStateMachine",E="String",F="terminated",G="#autoActionsAfterOntransition",H="",I="#actionsAfterOnentry",J="Saved-state stack is full",K="#autoActionsBeforeOnentry",L=": no transition found.  No state change.",M="#autoActionsAfterOnentry",N="#actionsBeforeOnentry",O="', because the finite state machine is not running.",P=": Pushed event: ",Q=": Cannot listen to event '",R="Machine started with no available states",S=": Process event: ",T="#entry",U="#ontransition",V=" on ",W="Invalid state: not an instance of ",X="Number",Y="#exit",bp="true, false, or null.",bq="Explicit transition ",br="  ",bl="qx.util.fsm.State",bm="while state stack is empty.",bn=".  Ignoring.",bo="#autoActionsAfterOnexit",bs=": Event '",bt="object",bu=" not handled for target ",bv=": Queued event: ";qx.Class.define(D,{extend:qx.core.Object,construct:function(bw){qx.core.Object.call(this);this.setName(bw);this.__jQ={};this.__Bl=null;this.__Bm=[];this.__Bn=[];this.__Bo=[];this.__Bp={};this.__Bq={};this.__Br={};this.__Bs={};this.__Bt={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},events:{"terminated":m},properties:{name:{check:E,nullable:true},state:{check:E,nullable:true},previousState:{check:E,nullable:true},nextState:{check:E,nullable:true},maxSavedStates:{check:X,init:2},debugFlags:{check:X,init:7}},members:{__jQ:null,__Bl:null,__Bn:null,__Bo:null,__Bm:null,__Bp:null,__Bq:null,__Br:null,__Bs:null,__Bt:null,__Bu:false,__Bv:true,isTerminated:function(){return this.__Bv;}
,addState:function(bx){if(!bx instanceof qx.util.fsm.State){throw new Error(W+bl);}
;var by=bx.getName();if(by in this.__jQ){throw new Error(s+by+x);}
;if(this.__Bl==null){this.__Bl=by;}
;this.__jQ[by]=bx;}
,replaceState:function(bC,bB){if(!bC instanceof qx.util.fsm.State){throw new Error(W+bl);}
;var bA=bC.getName();var bz=this.__jQ[bA];this.__jQ[bA]=bC;if(bB){bz._bNeedDispose=true;}
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__Bq[bG]=bH;this.__Br[bH]=bG;this.__Bp[bG]=bD;if(!bE){return;}
;if(typeof (bE)==l){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__Bs[bF]){this.__Bs[bF]={};}
;this.__Bs[bF][bG]=true;if(!this.__Bt[bG]){this.__Bt[bG]=[];}
;this.__Bt[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__Bq[bL];if(this.__Bt[bL]){for(var i=0;i<this.__Bt[bL].length;i++ ){bJ=this.__Bt[bL][i];delete this.__Bs[bJ][bL];bM=true;for(bK in this.__Bs[bJ]){bM=false;break;}
;if(bM){delete this.__Bs[bJ];}
;}
;delete this.__Bt[bL];}
;delete this.__Br[bI];delete this.__Bq[bL];delete this.__Bp[bL];}
,getObject:function(bN){return this.__Bp[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__Br[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__Bs[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__Bq){var bT=this.__Bq[bR];var bS=this.getObject(bR);this.debug(bR+d+bT);this.debug(br+bT+d+this.__Br[bT]);this.debug(br+bR+d+this.getObject(bR));this.debug(br+this.getObject(bR)+d+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__jQ,"startState":this.__Bl,"eventQueue":this.__Bn,"blockedEvents":this.__Bo,"savedStates":this.__Bm,"friendlyToObject":this.__Bp,"friendlyToHash":this.__Bq,"hashToFriendly":this.__Br,"groupToFriendly":this.__Bs,"friendlyToGroups":this.__Bt});}
,start:function(){this.__Bv=false;var bV=this.__Bl;if(bV==null){throw new Error(R);}
...
;this.__jQ[bV].getAutoActionsBeforeOnentry()(this);if(bU){this.debug(this.getName()+w+bV+T);}
;this.__jQ[bV].getOnentry()(this,null);if(bU){this.debug(this.getName()+w+bV+I);}
;this.__jQ[bV].getAutoActionsAfterOnentry()(this);}
,pushState:function(bW){if(this.__Bm.length>=this.getMaxSavedStates()){throw new Error(J);}
;if(bW===true){this.__Bm.push(this.getState());}
else if(bW){this.__Bm.push(bW);}
else {this.__Bm.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Bm.length==0){throw new Error(p);}
;return this.__Bm.pop();}
,postponeEvent:function(event){this.__Bo.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__Bn.push(event);}
else {this.__Bn.unshift(event);}
...
;}
;}
,eventListener:function(event){if(this.__Bv){this.debug(this.getName()+Q+event.getType()+O);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__Bw();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__Bv){this.debug(this.getName()+Q+cb+O);return;}
...
,this,cg||20);}
,__Bw:function(){if(this.__Bu){return;}
;this.__Bu=true;while(this.__Bn.length>0){var event=this.__Bn.pop();var ch=this.__Bx(event);if(ch){event.dispose();}
;}
;this.__Bu=false;}
,__Bx:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+S+event.getType()+(ct?V+ct:H));}
;ck=this.getState();cn=this.__jQ[ck];cq=cn.transitions;e=cn.getEvents()[event.getType()];if(!e){if(cu){this.debug(this.getName()+bs+event.getType()+n+r);}
;return true;}
...
;this.__Bo.unshift(event);return false;default:if(typeof (cp)==l){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bq+cp+B);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(A+ck+z+t+v+bp);};cl=cm.getNextState();if(typeof (cl)==l){if(!cl in this.__jQ){throw new Error(o+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Bm.length==0){throw new Error(y+bm);}
;cl=this.__Bm.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__Bv=true;this.setNextState(null);break;default:throw new Error(q);};}
...
;if(this.__Bv){if(cr){this.debug(this.getName()+w+b);}
;this.fireDataEvent(F,ck);return true;}
;cn=this.__jQ[this.getNextState()];this.setPreviousState(ck);this.setState(this.getNextState());this.setNextState(null);cj=ck;ck=cl;cl=undefined;if(cr){this.debug(this.getName()+w+ck+K);}
...
;var funcFragment;var func=ba+I;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(J+M+typeof (functionRequest));}
...
;for(j=0;j<g.length;j++ ){func+=d+bk+g[j]+z+p+bj+bb+T+funcFragment+bt+bn;}
;}
;}
;}
;func+=E+bi+I+Q+E;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:Y,nullable:true},onentry:{transform:q,nullable:true,init:function(bG,event){}
},onexit:{transform:bs,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:bp,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:N,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:L,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:bu,nullable:true,init:function(bL,event){}
},events:{transform:B,nullable:true}},members:{__By:function(bM){if(typeof (bM)!=s||bM.length<1){throw new Error(x);}
;return bM;}
,__Bz:function(bN){switch(typeof (bN)){case W:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(r));default:throw new Error(S+typeof (bN));};}
,__BA:function(bP){switch(typeof (bP)){case W:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(r));default:throw new Error(o+typeof (bP));};}
,__BB:function(bT){if(typeof (bT)!=bB){throw new Error(G);}
...
else if(typeof (bR[bS])!=s&&typeof (bR[bS])!=bA){throw new Error(bl+R+e+F+bS+K+bR[bS]);}
;}
;}
else if(typeof (bR)!=s&&typeof (bR)!=bA){throw new Error(bc+e+K+bT[e]);}
;}
;return bT;}
,__BC:function(bU){return qx.util.fsm.State._commonTransformAutoActions(D,bU,this.getUserData(r));}
,__BD:function(bV){return qx.util.fsm.State._commonTransformAutoActions(t,bV,this.getUserData(r));}
,__BE:function(bW){return qx.util.fsm.State._commonTransformAutoActions(C,bW,this.getUserData(r));}
,__BF:function(bX){return qx.util.fsm.State._commonTransformAutoActions(P,bX,this.getUserData(r));}
,__BG:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bm+bg);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="or one of the Fsm.StateChange constants",d="__BK",e="autoActionsAfterOntransition",g="context",h="string",i="predicate",j="__BL",k="__BI",l="nextState must be an explicit state name, ",m="Transition ",n=": ",o="String",p="number",q="Invalid transition nextState type: ",r="Invalid transition nextState value: ",s="__BH",t="__BJ",u="ontransition",v="Invalid transition predicate type: ",w="boolean",x="nextState",y="Adding user-provided field to transition: ",z="Invalid ontransition type: ",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(g,E);for(var C in D){switch(C){case i:this.setPredicate(this.__BG(D[C],E));break;case x:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__BG(D[C],E));break;case e:this.setAutoActionsAfterOntransition(this.__BG(D[C],E));break;case u:this.setOntransition(this.__BG(D[C],E));break;case g:break;default:this.setUserData(C,D[C]);this.debug(m+F+n+y+C);break;};}
;}
,properties:{name:{check:o,nullable:true},predicate:{init:function(G,event){return true;}
,transform:s},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:k},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:t},autoActionsAfterOntransition:{init:function(I,event){}
,transform:d},ontransition:{init:function(J,event){}
,transform:j}},members:{__BH:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
...
,__BI:function(N){switch(typeof (N)){case h:return N;case p:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(r+N+n+l+c);};break;default:throw new Error(q+typeof (N));};}
,__BJ:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(g));}
,__BK:function(P){return qx.util.fsm.State._commonTransformAutoActions(e,P,this.getUserData(g));}
,__BL:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(g));default:throw new Error(z+typeof (Q));};}
,__BG:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
