...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.374103e8c985.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="TERMINATED",c="__Bm",d=" => ",f="#autoActionsBeforeOnexit",g=": Could not find friendly name for '",h=": event '",j="__Bn",k=" blocked.  Re-queuing.",l="string",m="qx.event.type.Data",n="'",o="Attempt to transition to nonexistent state ",p="Saved-state stack is empty",q="Internal error: invalid nextState",r=" not handled.  Ignoring.",s="State ",u="#autoActionsBeforeOntransition",v=" returned a value other than ",w="#",x=" already exists",y="Attempt to transition to POP_STATE_STACK ",z=":",A="Transition ",B=" does not exist",C="' on '",D="qx.util.fsm.FiniteStateMachine",E="String",F="terminated",G="#autoActionsAfterOntransition",H="",I="#actionsAfterOnentry",J="Saved-state stack is full",K="#autoActionsBeforeOnentry",L=": no transition found.  No state change.",M="#autoActionsAfterOnentry",N="#actionsBeforeOnentry",O="', because the finite state machine is not running.",P=": Pushed event: ",Q=": Cannot listen to event '",R="Machine started with no available states",S=": Process event: ",T="#entry",U="#ontransition",V=" on ",W="Invalid state: not an instance of ",X="Number",Y="#exit",bp="true, false, or null.",bq="Explicit transition ",br="  ",bl="qx.util.fsm.State",bm="while state stack is empty.",bn=".  Ignoring.",bo="#autoActionsAfterOnexit",bs=": Event '",bt="object",bu=" not handled for target ",bv=": Queued event: ";qx.Class.define(D,{extend:qx.core.Object,construct:function(bw){qx.core.Object.call(this);this.setName(bw);this.__jP={};this.__Bk=null;this.__Bl=[];this.__Bm=[];this.__Bn=[];this.__Bo={};this.__Bp={};this.__Bq={};this.__Br={};this.__Bs={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},events:{"terminated":m},properties:{name:{check:E,nullable:true},state:{check:E,nullable:true},previousState:{check:E,nullable:true},nextState:{check:E,nullable:true},maxSavedStates:{check:X,init:2},debugFlags:{check:X,init:7}},members:{__jP:null,__Bk:null,__Bm:null,__Bn:null,__Bl:null,__Bo:null,__Bp:null,__Bq:null,__Br:null,__Bs:null,__Bt:false,__Bu:true,isTerminated:function(){return this.__Bu;}
,addState:function(bx){if(!bx instanceof qx.util.fsm.State){throw new Error(W+bl);}
;var by=bx.getName();if(by in this.__jP){throw new Error(s+by+x);}
;if(this.__Bk==null){this.__Bk=by;}
;this.__jP[by]=bx;}
,replaceState:function(bC,bB){if(!bC instanceof qx.util.fsm.State){throw new Error(W+bl);}
;var bA=bC.getName();var bz=this.__jP[bA];this.__jP[bA]=bC;if(bB){bz._bNeedDispose=true;}
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__Bp[bG]=bH;this.__Bq[bH]=bG;this.__Bo[bG]=bD;if(!bE){return;}
;if(typeof (bE)==l){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__Br[bF]){this.__Br[bF]={};}
;this.__Br[bF][bG]=true;if(!this.__Bs[bG]){this.__Bs[bG]=[];}
;this.__Bs[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__Bp[bL];if(this.__Bs[bL]){for(var i=0;i<this.__Bs[bL].length;i++ ){bJ=this.__Bs[bL][i];delete this.__Br[bJ][bL];bM=true;for(bK in this.__Br[bJ]){bM=false;break;}
;if(bM){delete this.__Br[bJ];}
;}
;delete this.__Bs[bL];}
;delete this.__Bq[bI];delete this.__Bp[bL];delete this.__Bo[bL];}
,getObject:function(bN){return this.__Bo[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__Bq[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__Br[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__Bp){var bT=this.__Bp[bR];var bS=this.getObject(bR);this.debug(bR+d+bT);this.debug(br+bT+d+this.__Bq[bT]);this.debug(br+bR+d+this.getObject(bR));this.debug(br+this.getObject(bR)+d+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__jP,"startState":this.__Bk,"eventQueue":this.__Bm,"blockedEvents":this.__Bn,"savedStates":this.__Bl,"friendlyToObject":this.__Bo,"friendlyToHash":this.__Bp,"hashToFriendly":this.__Bq,"groupToFriendly":this.__Br,"friendlyToGroups":this.__Bs});}
,start:function(){this.__Bu=false;var bV=this.__Bk;if(bV==null){throw new Error(R);}
...
;this.__jP[bV].getAutoActionsBeforeOnentry()(this);if(bU){this.debug(this.getName()+w+bV+T);}
;this.__jP[bV].getOnentry()(this,null);if(bU){this.debug(this.getName()+w+bV+I);}
;this.__jP[bV].getAutoActionsAfterOnentry()(this);}
,pushState:function(bW){if(this.__Bl.length>=this.getMaxSavedStates()){throw new Error(J);}
;if(bW===true){this.__Bl.push(this.getState());}
else if(bW){this.__Bl.push(bW);}
else {this.__Bl.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Bl.length==0){throw new Error(p);}
;return this.__Bl.pop();}
,postponeEvent:function(event){this.__Bn.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__Bm.push(event);}
...
;}
;}
,eventListener:function(event){if(this.__Bu){this.debug(this.getName()+Q+event.getType()+O);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__Bv();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__Bu){this.debug(this.getName()+Q+cb+O);return;}
...
,this,cg||20);}
,__Bv:function(){if(this.__Bt){return;}
;this.__Bt=true;while(this.__Bm.length>0){var event=this.__Bm.pop();var ch=this.__Bw(event);if(ch){event.dispose();}
;}
;this.__Bt=false;}
,__Bw:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+S+event.getType()+(ct?V+ct:H));}
;ck=this.getState();cn=this.__jP[ck];cq=cn.transitions;e=cn.getEvents()[event.getType()];if(!e){if(cu){this.debug(this.getName()+bs+event.getType()+n+r);}
;return true;}
...
;this.__Bn.unshift(event);return false;default:if(typeof (cp)==l){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bq+cp+B);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(A+ck+z+t+v+bp);};cl=cm.getNextState();if(typeof (cl)==l){if(!cl in this.__jP){throw new Error(o+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Bl.length==0){throw new Error(y+bm);}
;cl=this.__Bl.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__Bu=true;this.setNextState(null);break;default:throw new Error(q);};}
...
;if(this.__Bu){if(cr){this.debug(this.getName()+w+b);}
;this.fireDataEvent(F,ck);return true;}
;cn=this.__jP[this.getNextState()];this.setPreviousState(ck);this.setState(this.getNextState());this.setNextState(null);cj=ck;ck=cl;cl=undefined;if(cr){this.debug(this.getName()+w+ck+K);}
...
;cn.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__Bn.length;i++ ){e=this.__Bn.pop();this.__Bm.unshift(e);}
;if(cs){this.debug(this.getName()+w+cj+d+this.getName()+w+ck);}
;return true;}
;if(cs){this.debug(this.getName()+w+ck+h+event.getType()+n+L);}
;return true;}
},destruct:function(){this._disposeArray(c);this._disposeArray(j);this.__Bl=this.__jP=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="parameters",k=")",l="__Bx",m=" fsm.getObject('",n="groups",o="Invalid function parameters: ",p="Invalid onexit type: ",q="  for (var i = 0; i < groupObjects.length; i++)",r="__Bz",s="context",t="string",u="autoActionsAfterOnentry",v="Invalid numeric value in events object: ",w='"',x="Invalid numeric value in events object ",y="Invalid state name",z=" value: ",A="');",B="State ",C="__BB",D="autoActionsBeforeOnexit",E="autoActionsBeforeOnentry",F="}",G="): ",H="events must be an object",I="onexit",J="{",K="Invalid function request type: ",L=": ",M="__BE",N="expected array, found ",O="__BD",P="Invalid friendly name in 'objects' list: ",Q="autoActionsAfterOnexit",R="  fsm.debug(ex);",S="(",T="Invalid onentry type: ",U="    fsm.getObject(objName).",V=")(fsm)",W="events",X="undefined",Y="Invalid ",ba="__By",bb="try",bc="    var objName = groupObjects[i];",bd="Invalid value in events object: ",be="').",bf="objects",bg="expected object, found ",bh="qx.util.fsm.Transition",bi="The events object must be provided in new state info",bj="catch(ex)",bk="  {",bl="    fsm.getGroupObjects('",bm="Invalid value in events object ",bn="Invalid transition: not an instance of ",bo="  }",bp="Invalid 'objects' list: expected array, got ",bq="__BC",br="qx.util.fsm.State",bs="onentry",bt="__BA",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(br,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(s,bE);for(var bC in bF){switch(bC){case bs:this.setOnentry(this.__BF(bF[bC],bE));break;case I:this.setOnexit(this.__BF(bF[bC],bE));break;case E:this.setAutoActionsBeforeOnentry(bF[bC]);break;case u:this.setAutoActionsAfterOnentry(bF[bC]);break;case D:this.setAutoActionsBeforeOnexit(bF[bC]);break;case Q:this.setAutoActionsAfterOnexit(bF[bC]);break;case W:this.setEvents(bF[bC]);break;case s:break;default:this.setUserData(bC,bF[bC]);this.debug(B+bD+L+bz+bC);break;};}
...
;func+=m+a[j]+be+funcFragment+bu;}
;var g=objectAndGroupList[n];if(g){if(!g instanceof Array){throw new Error(bw+typeof (g));}
;for(j=0;j<g.length;j++ ){func+=d+bl+g[j]+A+q+bk+bc+U+funcFragment+bu+bo;}
;}
;}
;}
;func+=F+bj+J+R+F;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:l,nullable:true},onentry:{transform:ba,nullable:true,init:function(bG,event){}
},onexit:{transform:r,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:C,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:bq,nullable:true,init:function(bJ,event){}
...
},events:{transform:bt,nullable:true}},members:{__Bx:function(bM){if(typeof (bM)!=t||bM.length<1){throw new Error(y);}
;return bM;}
,__By:function(bN){switch(typeof (bN)){case X:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(s));default:throw new Error(T+typeof (bN));};}
,__Bz:function(bP){switch(typeof (bP)){case X:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(s));default:throw new Error(p+typeof (bP));};}
,__BA:function(bT){if(typeof (bT)!=bB){throw new Error(H);}
...
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==bA&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(x+S+e+G+bS+L+bR[bS]);}
else if(typeof (bR[bS])!=t&&typeof (bR[bS])!=bA){throw new Error(bm+S+e+G+bS+L+bR[bS]);}
;}
;}
else if(typeof (bR)!=t&&typeof (bR)!=bA){throw new Error(bd+e+L+bT[e]);}
;}
;return bT;}
,__BB:function(bU){return qx.util.fsm.State._commonTransformAutoActions(E,bU,this.getUserData(s));}
,__BC:function(bV){return qx.util.fsm.State._commonTransformAutoActions(u,bV,this.getUserData(s));}
,__BD:function(bW){return qx.util.fsm.State._commonTransformAutoActions(D,bW,this.getUserData(s));}
,__BE:function(bX){return qx.util.fsm.State._commonTransformAutoActions(Q,bX,this.getUserData(s));}
,__BF:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bn+bh);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="or one of the Fsm.StateChange constants",d="__BK",e="autoActionsAfterOntransition",g="context",h="string",i="predicate",j="__BI",k="nextState must be an explicit state name, ",l="Transition ",m=": ",n="String",o="number",p="Invalid transition nextState type: ",q="Invalid transition nextState value: ",r="__BH",s="__BJ",t="ontransition",u="Invalid transition predicate type: ",v="boolean",w="nextState",x="Adding user-provided field to transition: ",y="Invalid ontransition type: ",z="__BG",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(g,E);for(var C in D){switch(C){case i:this.setPredicate(this.__BF(D[C],E));break;case w:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__BF(D[C],E));break;case e:this.setAutoActionsAfterOntransition(this.__BF(D[C],E));break;case t:this.setOntransition(this.__BF(D[C],E));break;case g:break;default:this.setUserData(C,D[C]);this.debug(l+F+m+x+C);break;};}
;}
,properties:{name:{check:n,nullable:true},predicate:{init:function(G,event){return true;}
,transform:z},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:r},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:j},autoActionsAfterOntransition:{init:function(I,event){}
,transform:s},ontransition:{init:function(J,event){}
,transform:d}},members:{__BG:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
...
,__BH:function(N){switch(typeof (N)){case h:return N;case o:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(q+N+m+k+c);};break;default:throw new Error(p+typeof (N));};}
,__BI:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(g));}
,__BJ:function(P){return qx.util.fsm.State._commonTransformAutoActions(e,P,this.getUserData(g));}
,__BK:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(g));default:throw new Error(y+typeof (Q));};}
,__BF:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
