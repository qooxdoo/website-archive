...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.aad51ee9e7b7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="TERMINATED",c=" => ",d="#autoActionsBeforeOnexit",f=": Could not find friendly name for '",g=": event '",h=" blocked.  Re-queuing.",j="string",k="qx.event.type.Data",l="'",m="Attempt to transition to nonexistent state ",n="Saved-state stack is empty",o="Internal error: invalid nextState",p=" not handled.  Ignoring.",q="State ",r="#autoActionsBeforeOntransition",s=" returned a value other than ",u="#",v=" already exists",w="Attempt to transition to POP_STATE_STACK ",x=":",y="__Bk",z="Transition ",A=" does not exist",B="' on '",C="qx.util.fsm.FiniteStateMachine",D="String",E="terminated",F="#autoActionsAfterOntransition",G="",H="#actionsAfterOnentry",I="Saved-state stack is full",J="#autoActionsBeforeOnentry",K=": no transition found.  No state change.",L="__Bl",M="#autoActionsAfterOnentry",N="#actionsBeforeOnentry",O="', because the finite state machine is not running.",P=": Pushed event: ",Q=": Cannot listen to event '",R="Machine started with no available states",S=": Process event: ",T="#entry",U="#ontransition",V=" on ",W="Invalid state: not an instance of ",X="Number",Y="#exit",bp="true, false, or null.",bq="Explicit transition ",br="  ",bl="qx.util.fsm.State",bm="while state stack is empty.",bn=".  Ignoring.",bo="#autoActionsAfterOnexit",bs=": Event '",bt="object",bu=" not handled for target ",bv=": Queued event: ";qx.Class.define(C,{extend:qx.core.Object,construct:function(bw){qx.core.Object.call(this);this.setName(bw);this.__jO={};this.__Bi=null;this.__Bj=[];this.__Bk=[];this.__Bl=[];this.__Bm={};this.__Bn={};this.__Bo={};this.__Bp={};this.__Bq={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},events:{"terminated":k},properties:{name:{check:D,nullable:true},state:{check:D,nullable:true},previousState:{check:D,nullable:true},nextState:{check:D,nullable:true},maxSavedStates:{check:X,init:2},debugFlags:{check:X,init:7}},members:{__jO:null,__Bi:null,__Bk:null,__Bl:null,__Bj:null,__Bm:null,__Bn:null,__Bo:null,__Bp:null,__Bq:null,__Br:false,__Bs:true,isTerminated:function(){return this.__Bs;}
...
;if(this.__Bi==null){this.__Bi=by;}
;this.__jO[by]=bx;}
...
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__Bn[bG]=bH;this.__Bo[bH]=bG;this.__Bm[bG]=bD;if(!bE){return;}
;if(typeof (bE)==j){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__Bp[bF]){this.__Bp[bF]={};}
;this.__Bp[bF][bG]=true;if(!this.__Bq[bG]){this.__Bq[bG]=[];}
;this.__Bq[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__Bn[bL];if(this.__Bq[bL]){for(var i=0;i<this.__Bq[bL].length;i++ ){bJ=this.__Bq[bL][i];delete this.__Bp[bJ][bL];bM=true;for(bK in this.__Bp[bJ]){bM=false;break;}
;if(bM){delete this.__Bp[bJ];}
;}
;delete this.__Bq[bL];}
;delete this.__Bo[bI];delete this.__Bn[bL];delete this.__Bm[bL];}
,getObject:function(bN){return this.__Bm[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__Bo[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__Bp[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__Bn){var bT=this.__Bn[bR];var bS=this.getObject(bR);this.debug(bR+c+bT);this.debug(br+bT+c+this.__Bo[bT]);this.debug(br+bR+c+this.getObject(bR));this.debug(br+this.getObject(bR)+c+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__jO,"startState":this.__Bi,"eventQueue":this.__Bk,"blockedEvents":this.__Bl,"savedStates":this.__Bj,"friendlyToObject":this.__Bm,"friendlyToHash":this.__Bn,"hashToFriendly":this.__Bo,"groupToFriendly":this.__Bp,"friendlyToGroups":this.__Bq});}
,start:function(){this.__Bs=false;var bV=this.__Bi;if(bV==null){throw new Error(R);}
...
;this.__jO[bV].getAutoActionsBeforeOnentry()(this);if(bU){this.debug(this.getName()+u+bV+T);}
;this.__jO[bV].getOnentry()(this,null);if(bU){this.debug(this.getName()+u+bV+H);}
;this.__jO[bV].getAutoActionsAfterOnentry()(this);}
,pushState:function(bW){if(this.__Bj.length>=this.getMaxSavedStates()){throw new Error(I);}
;if(bW===true){this.__Bj.push(this.getState());}
else if(bW){this.__Bj.push(bW);}
else {this.__Bj.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Bj.length==0){throw new Error(n);}
;return this.__Bj.pop();}
,postponeEvent:function(event){this.__Bl.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__Bk.push(event);}
else {this.__Bk.unshift(event);}
...
;}
;}
,eventListener:function(event){if(this.__Bs){this.debug(this.getName()+Q+event.getType()+O);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__Bt();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__Bs){this.debug(this.getName()+Q+cb+O);return;}
...
,this,cg||20);}
,__Bt:function(){if(this.__Br){return;}
;this.__Br=true;while(this.__Bk.length>0){var event=this.__Bk.pop();var ch=this.__Bu(event);if(ch){event.dispose();}
;}
;this.__Br=false;}
,__Bu:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+S+event.getType()+(ct?V+ct:G));}
...
;this.__Bl.unshift(event);return false;default:if(typeof (cp)==j){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bq+cp+A);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(z+ck+x+t+s+bp);};cl=cm.getNextState();if(typeof (cl)==j){if(!cl in this.__jO){throw new Error(m+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Bj.length==0){throw new Error(w+bm);}
;cl=this.__Bj.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__Bs=true;this.setNextState(null);break;default:throw new Error(o);};}
...
;if(this.__Bs){if(cr){this.debug(this.getName()+u+b);}
...
;cn.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__Bl.length;i++ ){e=this.__Bl.pop();this.__Bk.unshift(e);}
;if(cs){this.debug(this.getName()+u+cj+c+this.getName()+u+ck);}
;return true;}
;if(cs){this.debug(this.getName()+u+ck+g+event.getType()+l+K);}
;return true;}
},destruct:function(){this._disposeArray(y);this._disposeArray(L);this.__Bj=this.__jO=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="parameters",k=")",l="__Bx",m=" fsm.getObject('",n="groups",o="Invalid function parameters: ",p="Invalid onexit type: ",q="  for (var i = 0; i < groupObjects.length; i++)",r="__Bz",s="context",t="string",u="autoActionsAfterOnentry",v="Invalid numeric value in events object: ",w='"',x="__Bw",y="Invalid state name",z=" value: ",A="');",B="State ",C="__BB",D="autoActionsBeforeOnexit",E="autoActionsBeforeOnentry",F="}",G="): ",H="events must be an object",I="onexit",J="{",K="Invalid function request type: ",L="__Bv",M=": ",N="expected array, found ",O="Invalid friendly name in 'objects' list: ",P="autoActionsAfterOnexit",Q="  fsm.debug(ex);",R="(",S="Invalid onentry type: ",T="    fsm.getObject(objName).",U=")(fsm)",V="events",W="undefined",X="Invalid ",Y="__By",ba="try",bb="    var objName = groupObjects[i];",bc="Invalid value in events object: ",bd="').",be="objects",bf="expected object, found ",bg="qx.util.fsm.Transition",bh="The events object must be provided in new state info",bi="catch(ex)",bj="  {",bk="    fsm.getGroupObjects('",bl="Invalid value in events object ",bm="Invalid transition: not an instance of ",bn="  }",bo="Invalid numeric value in events object ",bp="Invalid 'objects' list: expected array, got ",bq="__BC",br="qx.util.fsm.State",bs="onentry",bt="__BA",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(br,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(s,bE);for(var bC in bF){switch(bC){case bs:this.setOnentry(this.__BD(bF[bC],bE));break;case I:this.setOnexit(this.__BD(bF[bC],bE));break;case E:this.setAutoActionsBeforeOnentry(bF[bC]);break;case u:this.setAutoActionsAfterOnentry(bF[bC]);break;case D:this.setAutoActionsBeforeOnexit(bF[bC]);break;case P:this.setAutoActionsAfterOnexit(bF[bC]);break;case V:this.setEvents(bF[bC]);break;case s:break;default:this.setUserData(bC,bF[bC]);this.debug(B+bD+M+bz+bC);break;};}
;if(!this.getEvents()){throw new Error(bh);}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(actionType,value,context){if(typeof (value)!=bB){throw new Error(X+actionType+z+typeof (value));}
...
;for(j=0;j<g.length;j++ ){func+=d+bk+g[j]+A+q+bj+bb+T+funcFragment+bu+bn;}
;}
;}
;}
;func+=F+bi+J+Q+F;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:L,nullable:true},onentry:{transform:x,nullable:true,init:function(bG,event){}
...
},autoActionsAfterOnexit:{transform:bq,nullable:true,init:function(bL,event){}
},events:{transform:Y,nullable:true}},members:{__Bv:function(bM){if(typeof (bM)!=t||bM.length<1){throw new Error(y);}
;return bM;}
,__Bw:function(bN){switch(typeof (bN)){case W:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(s));default:throw new Error(S+typeof (bN));};}
,__Bx:function(bP){switch(typeof (bP)){case W:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(s));default:throw new Error(p+typeof (bP));};}
,__By:function(bT){if(typeof (bT)!=bB){throw new Error(H);}
;for(var e in bT){var bR=bT[e];if(typeof (bR)==bA&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(v+e+M+bR);}
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==bA&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(bo+R+e+G+bS+M+bR[bS]);}
else if(typeof (bR[bS])!=t&&typeof (bR[bS])!=bA){throw new Error(bl+R+e+G+bS+M+bR[bS]);}
;}
;}
else if(typeof (bR)!=t&&typeof (bR)!=bA){throw new Error(bc+e+M+bT[e]);}
;}
;return bT;}
,__Bz:function(bU){return qx.util.fsm.State._commonTransformAutoActions(E,bU,this.getUserData(s));}
,__BA:function(bV){return qx.util.fsm.State._commonTransformAutoActions(u,bV,this.getUserData(s));}
,__BB:function(bW){return qx.util.fsm.State._commonTransformAutoActions(D,bW,this.getUserData(s));}
,__BC:function(bX){return qx.util.fsm.State._commonTransformAutoActions(P,bX,this.getUserData(s));}
,__BD:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bm+bg);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="or one of the Fsm.StateChange constants",d="autoActionsAfterOntransition",e="context",g="string",h="predicate",i="__BI",j="__BF",k="nextState must be an explicit state name, ",l="Transition ",m=": ",n="String",o="number",p="Invalid transition nextState type: ",q="Invalid transition nextState value: ",r="__BH",s="ontransition",t="Invalid transition predicate type: ",u="boolean",v="nextState",w="Adding user-provided field to transition: ",x="Invalid ontransition type: ",y="__BG",z="__BE",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(e,E);for(var C in D){switch(C){case h:this.setPredicate(this.__BD(D[C],E));break;case v:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__BD(D[C],E));break;case d:this.setAutoActionsAfterOntransition(this.__BD(D[C],E));break;case s:this.setOntransition(this.__BD(D[C],E));break;case e:break;default:this.setUserData(C,D[C]);this.debug(l+F+m+w+C);break;};}
;}
,properties:{name:{check:n,nullable:true},predicate:{init:function(G,event){return true;}
,transform:z},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:j},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:y},autoActionsAfterOntransition:{init:function(I,event){}
,transform:r},ontransition:{init:function(J,event){}
,transform:i}},members:{__BE:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
...
,__BF:function(N){switch(typeof (N)){case g:return N;case o:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(q+N+m+k+c);};break;default:throw new Error(p+typeof (N));};}
,__BG:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(e));}
,__BH:function(P){return qx.util.fsm.State._commonTransformAutoActions(d,P,this.getUserData(e));}
,__BI:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(e));default:throw new Error(x+typeof (Q));};}
,__BD:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
