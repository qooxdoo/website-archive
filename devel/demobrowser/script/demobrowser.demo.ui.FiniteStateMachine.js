...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.e86adc6a22aa.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="TERMINATED",c=" => ",d="#autoActionsBeforeOnexit",f=": Could not find friendly name for '",g=": event '",h=" blocked.  Re-queuing.",j="string",k="qx.event.type.Data",l="'",m="__Bw",n="Attempt to transition to nonexistent state ",o="Saved-state stack is empty",p="Internal error: invalid nextState",q=" not handled.  Ignoring.",r="State ",s="#autoActionsBeforeOntransition",u=" returned a value other than ",v="#",w=" already exists",x="Attempt to transition to POP_STATE_STACK ",y=":",z="Transition ",A="__Bv",B=" does not exist",C="' on '",D="qx.util.fsm.FiniteStateMachine",E="String",F="terminated",G="#autoActionsAfterOntransition",H="",I="#actionsAfterOnentry",J="Saved-state stack is full",K="#autoActionsBeforeOnentry",L=": no transition found.  No state change.",M="#autoActionsAfterOnentry",N="#actionsBeforeOnentry",O="', because the finite state machine is not running.",P=": Pushed event: ",Q=": Cannot listen to event '",R="Machine started with no available states",S=": Process event: ",T="#entry",U="#ontransition",V=" on ",W="Invalid state: not an instance of ",X="Number",Y="#exit",bp="true, false, or null.",bq="Explicit transition ",br="  ",bl="qx.util.fsm.State",bm="while state stack is empty.",bn=".  Ignoring.",bo="#autoActionsAfterOnexit",bs=": Event '",bt="object",bu=" not handled for target ",bv=": Queued event: ";qx.Class.define(D,{extend:qx.core.Object,construct:function(bw){qx.core.Object.call(this);this.setName(bw);this.__kd={};this.__Bt=null;this.__Bu=[];this.__Bv=[];this.__Bw=[];this.__Bx={};this.__By={};this.__Bz={};this.__BA={};this.__BB={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},events:{"terminated":k},properties:{name:{check:E,nullable:true},state:{check:E,nullable:true},previousState:{check:E,nullable:true},nextState:{check:E,nullable:true},maxSavedStates:{check:X,init:2},debugFlags:{check:X,init:7}},members:{__kd:null,__Bt:null,__Bv:null,__Bw:null,__Bu:null,__Bx:null,__By:null,__Bz:null,__BA:null,__BB:null,__BC:false,__BD:true,isTerminated:function(){return this.__BD;}
,addState:function(bx){if(!bx instanceof qx.util.fsm.State){throw new Error(W+bl);}
;var by=bx.getName();if(by in this.__kd){throw new Error(r+by+w);}
;if(this.__Bt==null){this.__Bt=by;}
;this.__kd[by]=bx;}
...
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__By[bG]=bH;this.__Bz[bH]=bG;this.__Bx[bG]=bD;if(!bE){return;}
;if(typeof (bE)==j){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__BA[bF]){this.__BA[bF]={};}
;this.__BA[bF][bG]=true;if(!this.__BB[bG]){this.__BB[bG]=[];}
;this.__BB[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__By[bL];if(this.__BB[bL]){for(var i=0;i<this.__BB[bL].length;i++ ){bJ=this.__BB[bL][i];delete this.__BA[bJ][bL];bM=true;for(bK in this.__BA[bJ]){bM=false;break;}
;if(bM){delete this.__BA[bJ];}
;}
;delete this.__BB[bL];}
;delete this.__Bz[bI];delete this.__By[bL];delete this.__Bx[bL];}
,getObject:function(bN){return this.__Bx[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__Bz[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__BA[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__By){var bT=this.__By[bR];var bS=this.getObject(bR);this.debug(bR+c+bT);this.debug(br+bT+c+this.__Bz[bT]);this.debug(br+bR+c+this.getObject(bR));this.debug(br+this.getObject(bR)+c+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__kd,"startState":this.__Bt,"eventQueue":this.__Bv,"blockedEvents":this.__Bw,"savedStates":this.__Bu,"friendlyToObject":this.__Bx,"friendlyToHash":this.__By,"hashToFriendly":this.__Bz,"groupToFriendly":this.__BA,"friendlyToGroups":this.__BB});}
,start:function(){this.__BD=false;var bV=this.__Bt;if(bV==null){throw new Error(R);}
...
;this.__kd[bV].getAutoActionsBeforeOnentry()(this);if(bU){this.debug(this.getName()+v+bV+T);}
;this.__kd[bV].getOnentry()(this,null);if(bU){this.debug(this.getName()+v+bV+I);}
;this.__kd[bV].getAutoActionsAfterOnentry()(this);}
,pushState:function(bW){if(this.__Bu.length>=this.getMaxSavedStates()){throw new Error(J);}
;if(bW===true){this.__Bu.push(this.getState());}
else if(bW){this.__Bu.push(bW);}
else {this.__Bu.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Bu.length==0){throw new Error(o);}
;return this.__Bu.pop();}
,postponeEvent:function(event){this.__Bw.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__Bv.push(event);}
else {this.__Bv.unshift(event);}
...
;}
;}
,eventListener:function(event){if(this.__BD){this.debug(this.getName()+Q+event.getType()+O);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__BE();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__BD){this.debug(this.getName()+Q+cb+O);return;}
...
,this,cg||20);}
,__BE:function(){if(this.__BC){return;}
;this.__BC=true;while(this.__Bv.length>0){var event=this.__Bv.pop();var ch=this.__BF(event);if(ch){event.dispose();}
;}
;this.__BC=false;}
,__BF:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+S+event.getType()+(ct?V+ct:H));}
;ck=this.getState();cn=this.__kd[ck];cq=cn.transitions;e=cn.getEvents()[event.getType()];if(!e){if(cu){this.debug(this.getName()+bs+event.getType()+l+q);}
;return true;}
;if(typeof (e)==bt){if(!ct){if(co){this.debug(this.getName()+f+event.getType()+C+event.getTarget()+l);}
;return true;}
...
;this.__Bw.unshift(event);return false;default:if(typeof (cp)==j){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bq+cp+B);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(z+ck+y+t+u+bp);};cl=cm.getNextState();if(typeof (cl)==j){if(!cl in this.__kd){throw new Error(n+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Bu.length==0){throw new Error(x+bm);}
;cl=this.__Bu.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__BD=true;this.setNextState(null);break;default:throw new Error(p);};}
...
;if(this.__BD){if(cr){this.debug(this.getName()+v+b);}
;this.fireDataEvent(F,ck);return true;}
;cn=this.__kd[this.getNextState()];this.setPreviousState(ck);this.setState(this.getNextState());this.setNextState(null);cj=ck;ck=cl;cl=undefined;if(cr){this.debug(this.getName()+v+ck+K);}
...
;for(j=0;j<g.length;j++ ){func+=d+bl+g[j]+z+q+bj+ba+R+funcFragment+bu+bo;}
;}
;}
;}
;func+=E+bi+I+O+E;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:bt,nullable:true},onentry:{transform:X,nullable:true,init:function(bG,event){}
},onexit:{transform:D,nullable:true,init:function(bH,event){}
...
},autoActionsBeforeOnexit:{transform:bk,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:bd,nullable:true,init:function(bL,event){}
},events:{transform:Y,nullable:true}},members:{__BG:function(bM){if(typeof (bM)!=s||bM.length<1){throw new Error(w);}
;return bM;}
,__BH:function(bN){switch(typeof (bN)){case U:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(r));default:throw new Error(Q+typeof (bN));};}
,__BI:function(bP){switch(typeof (bP)){case U:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(r));default:throw new Error(p+typeof (bP));};}
,__BJ:function(bT){if(typeof (bT)!=bB){throw new Error(G);}
...
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==bA&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(bq+P+e+F+bS+K+bR[bS]);}
else if(typeof (bR[bS])!=s&&typeof (bR[bS])!=bA){throw new Error(bm+P+e+F+bS+K+bR[bS]);}
;}
;}
else if(typeof (bR)!=s&&typeof (bR)!=bA){throw new Error(bb+e+K+bT[e]);}
;}
;return bT;}
,__BK:function(bU){return qx.util.fsm.State._commonTransformAutoActions(C,bU,this.getUserData(r));}
,__BL:function(bV){return qx.util.fsm.State._commonTransformAutoActions(t,bV,this.getUserData(r));}
,__BM:function(bW){return qx.util.fsm.State._commonTransformAutoActions(B,bW,this.getUserData(r));}
,__BN:function(bX){return qx.util.fsm.State._commonTransformAutoActions(N,bX,this.getUserData(r));}
,__BO:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bn+bg);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="__BQ",c="autoActionsBeforeOntransition",d="or one of the Fsm.StateChange constants",e="autoActionsAfterOntransition",g="context",h="string",i="predicate",j="nextState must be an explicit state name, ",k="Transition ",l=": ",m="String",n="__BS",o="number",p="Invalid transition nextState type: ",q="__BT",r="Invalid transition nextState value: ",s="Adding user-provided field to transition: ",t="ontransition",u="qx.util.fsm.Transition",v="Invalid transition predicate type: ",w="boolean",x="nextState",y="Invalid ontransition type: ",z="__BP",A="undefined",B="__BR";qx.Class.define(u,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(g,E);for(var C in D){switch(C){case i:this.setPredicate(this.__BO(D[C],E));break;case x:this.setNextState(D[C]);break;case c:this.setAutoActionsBeforeOntransition(this.__BO(D[C],E));break;case e:this.setAutoActionsAfterOntransition(this.__BO(D[C],E));break;case t:this.setOntransition(this.__BO(D[C],E));break;case g:break;default:this.setUserData(C,D[C]);this.debug(k+F+l+s+C);break;};}
;}
,properties:{name:{check:m,nullable:true},predicate:{init:function(G,event){return true;}
,transform:z},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:b},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:B},autoActionsAfterOntransition:{init:function(I,event){}
,transform:n},ontransition:{init:function(J,event){}
,transform:q}},members:{__BP:function(K){switch(typeof (K)){case A:return function(L,event){return true;}
...
,__BQ:function(N){switch(typeof (N)){case h:return N;case o:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(r+N+l+j+d);};break;default:throw new Error(p+typeof (N));};}
,__BR:function(O){return qx.util.fsm.State._commonTransformAutoActions(c,O,this.getUserData(g));}
,__BS:function(P){return qx.util.fsm.State._commonTransformAutoActions(e,P,this.getUserData(g));}
,__BT:function(Q){switch(typeof (Q)){case A:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(g));default:throw new Error(y+typeof (Q));};}
,__BO:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
