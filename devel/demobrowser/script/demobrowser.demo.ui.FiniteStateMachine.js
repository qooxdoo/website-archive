...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.a0746934094f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;if(this.__Bu==null){this.__Bu=by;}
;this.__ke[by]=bx;}
...
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__Bz[bG]=bH;this.__BA[bH]=bG;this.__By[bG]=bD;if(!bE){return;}
;if(typeof (bE)==k){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__BB[bF]){this.__BB[bF]={};}
;this.__BB[bF][bG]=true;if(!this.__BC[bG]){this.__BC[bG]=[];}
;this.__BC[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__Bz[bL];if(this.__BC[bL]){for(var i=0;i<this.__BC[bL].length;i++ ){bJ=this.__BC[bL][i];delete this.__BB[bJ][bL];bM=true;for(bK in this.__BB[bJ]){bM=false;break;}
;if(bM){delete this.__BB[bJ];}
;}
;delete this.__BC[bL];}
;delete this.__BA[bI];delete this.__Bz[bL];delete this.__By[bL];}
,getObject:function(bN){return this.__By[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__BA[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__BB[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__Bz){var bT=this.__Bz[bR];var bS=this.getObject(bR);this.debug(bR+d+bT);this.debug(br+bT+d+this.__BA[bT]);this.debug(br+bR+d+this.getObject(bR));this.debug(br+this.getObject(bR)+d+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__ke,"startState":this.__Bu,"eventQueue":this.__Bw,"blockedEvents":this.__Bx,"savedStates":this.__Bv,"friendlyToObject":this.__By,"friendlyToHash":this.__Bz,"hashToFriendly":this.__BA,"groupToFriendly":this.__BB,"friendlyToGroups":this.__BC});}
,start:function(){this.__BE=false;var bV=this.__Bu;if(bV==null){throw new Error(R);}
...
,pushState:function(bW){if(this.__Bv.length>=this.getMaxSavedStates()){throw new Error(J);}
;if(bW===true){this.__Bv.push(this.getState());}
else if(bW){this.__Bv.push(bW);}
else {this.__Bv.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Bv.length==0){throw new Error(p);}
;return this.__Bv.pop();}
,postponeEvent:function(event){this.__Bx.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__Bw.push(event);}
else {this.__Bw.unshift(event);}
...
;}
;}
,eventListener:function(event){if(this.__BE){this.debug(this.getName()+Q+event.getType()+O);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__BF();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__BE){this.debug(this.getName()+Q+cb+O);return;}
...
,this,cg||20);}
,__BF:function(){if(this.__BD){return;}
;this.__BD=true;while(this.__Bw.length>0){var event=this.__Bw.pop();var ch=this.__BG(event);if(ch){event.dispose();}
;}
;this.__BD=false;}
,__BG:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+S+event.getType()+(ct?V+ct:H));}
;ck=this.getState();cn=this.__ke[ck];cq=cn.transitions;e=cn.getEvents()[event.getType()];if(!e){if(cu){this.debug(this.getName()+bs+event.getType()+m+r);}
;return true;}
...
;this.__Bx.unshift(event);return false;default:if(typeof (cp)==k){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bq+cp+B);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(A+ck+z+t+v+bp);};cl=cm.getNextState();if(typeof (cl)==k){if(!cl in this.__ke){throw new Error(o+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Bv.length==0){throw new Error(y+bm);}
;cl=this.__Bv.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__BE=true;this.setNextState(null);break;default:throw new Error(q);};}
...
;if(this.__BE){if(cr){this.debug(this.getName()+w+b);}
...
;cn.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__Bx.length;i++ ){e=this.__Bx.pop();this.__Bw.unshift(e);}
;if(cs){this.debug(this.getName()+w+cj+d+this.getName()+w+ck);}
;return true;}
;if(cs){this.debug(this.getName()+w+ck+h+event.getType()+m+L);}
;return true;}
},destruct:function(){this._disposeArray(n);this._disposeArray(c);this.__Bv=this.__ke=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="parameters",k="__BK",l=")",m=" fsm.getObject('",n="groups",o="Invalid function parameters: ",p="Invalid onexit type: ",q="  for (var i = 0; i < groupObjects.length; i++)",r="context",s="__BO",t="string",u="autoActionsAfterOnentry",v="Invalid numeric value in events object: ",w='"',x="Invalid state name",y="__BL",z=" value: ",A="');",B="State ",C="autoActionsBeforeOnexit",D="autoActionsBeforeOnentry",E="__BI",F="}",G="): ",H="events must be an object",I="onexit",J="{",K="Invalid function request type: ",L=": ",M="expected array, found ",N="Invalid friendly name in 'objects' list: ",O="autoActionsAfterOnexit",P="  fsm.debug(ex);",Q="(",R="Invalid onentry type: ",S="    fsm.getObject(objName).",T=")(fsm)",U="events",V="undefined",W="Invalid ",X="try",Y="__BH",ba="__BJ",bb="    var objName = groupObjects[i];",bc="Invalid value in events object: ",bd="').",be="__BN",bf="objects",bg="expected object, found ",bh="qx.util.fsm.Transition",bi="The events object must be provided in new state info",bj="catch(ex)",bk="  {",bl="__BM",bm="    fsm.getGroupObjects('",bn="Invalid value in events object ",bo="Invalid transition: not an instance of ",bp="  }",bq="Invalid 'objects' list: expected array, got ",br="Invalid numeric value in events object ",bs="qx.util.fsm.State",bt="onentry",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(bs,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(r,bE);for(var bC in bF){switch(bC){case bt:this.setOnentry(this.__BP(bF[bC],bE));break;case I:this.setOnexit(this.__BP(bF[bC],bE));break;case D:this.setAutoActionsBeforeOnentry(bF[bC]);break;case u:this.setAutoActionsAfterOnentry(bF[bC]);break;case C:this.setAutoActionsBeforeOnexit(bF[bC]);break;case O:this.setAutoActionsAfterOnexit(bF[bC]);break;case U:this.setEvents(bF[bC]);break;case r:break;default:this.setUserData(bC,bF[bC]);this.debug(B+bD+L+bz+bC);break;};}
...
;var funcFragment;var func=X+J;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(K+M+typeof (functionRequest));}
...
;for(j=0;j<g.length;j++ ){func+=d+bm+g[j]+A+q+bk+bb+S+funcFragment+bu+bp;}
;}
;}
;}
;func+=F+bj+J+P+F;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:Y,nullable:true},onentry:{transform:E,nullable:true,init:function(bG,event){}
...
},autoActionsAfterOnentry:{transform:bl,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:be,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:s,nullable:true,init:function(bL,event){}
},events:{transform:k,nullable:true}},members:{__BH:function(bM){if(typeof (bM)!=t||bM.length<1){throw new Error(x);}
;return bM;}
,__BI:function(bN){switch(typeof (bN)){case V:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(r));default:throw new Error(R+typeof (bN));};}
,__BJ:function(bP){switch(typeof (bP)){case V:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(r));default:throw new Error(p+typeof (bP));};}
,__BK:function(bT){if(typeof (bT)!=bB){throw new Error(H);}
...
else if(typeof (bR[bS])!=t&&typeof (bR[bS])!=bA){throw new Error(bn+Q+e+G+bS+L+bR[bS]);}
;}
;}
else if(typeof (bR)!=t&&typeof (bR)!=bA){throw new Error(bc+e+L+bT[e]);}
;}
;return bT;}
,__BL:function(bU){return qx.util.fsm.State._commonTransformAutoActions(D,bU,this.getUserData(r));}
,__BM:function(bV){return qx.util.fsm.State._commonTransformAutoActions(u,bV,this.getUserData(r));}
,__BN:function(bW){return qx.util.fsm.State._commonTransformAutoActions(C,bW,this.getUserData(r));}
,__BO:function(bX){return qx.util.fsm.State._commonTransformAutoActions(O,bX,this.getUserData(r));}
,__BP:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bo+bh);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="__BQ",c="autoActionsBeforeOntransition",d="__BU",e="or one of the Fsm.StateChange constants",g="autoActionsAfterOntransition",h="context",i="string",j="predicate",k="nextState must be an explicit state name, ",l="Transition ",m=": ",n="String",o="__BS",p="number",q="Invalid transition nextState type: ",r="__BT",s="Invalid transition nextState value: ",t="Adding user-provided field to transition: ",u="ontransition",v="Invalid transition predicate type: ",w="boolean",x="nextState",y="Invalid ontransition type: ",z="qx.util.fsm.Transition",A="undefined",B="__BR";qx.Class.define(z,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(h,E);for(var C in D){switch(C){case j:this.setPredicate(this.__BP(D[C],E));break;case x:this.setNextState(D[C]);break;case c:this.setAutoActionsBeforeOntransition(this.__BP(D[C],E));break;case g:this.setAutoActionsAfterOntransition(this.__BP(D[C],E));break;case u:this.setOntransition(this.__BP(D[C],E));break;case h:break;default:this.setUserData(C,D[C]);this.debug(l+F+m+t+C);break;};}
;}
,properties:{name:{check:n,nullable:true},predicate:{init:function(G,event){return true;}
,transform:b},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:B},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:o},autoActionsAfterOntransition:{init:function(I,event){}
,transform:r},ontransition:{init:function(J,event){}
,transform:d}},members:{__BQ:function(K){switch(typeof (K)){case A:return function(L,event){return true;}
;case w:return function(M,event){return K;}
;case a:return qx.lang.Function.bind(K,this.getUserData(h));default:throw new Error(v+typeof (K));};}
,__BR:function(N){switch(typeof (N)){case i:return N;case p:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(s+N+m+k+e);};break;default:throw new Error(q+typeof (N));};}
,__BS:function(O){return qx.util.fsm.State._commonTransformAutoActions(c,O,this.getUserData(h));}
,__BT:function(P){return qx.util.fsm.State._commonTransformAutoActions(g,P,this.getUserData(h));}
,__BU:function(Q){switch(typeof (Q)){case A:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(h));default:throw new Error(y+typeof (Q));};}
,__BP:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
