...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.138ef9895107.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var by=bx.getName();if(by in this.__ku){throw new Error(r+by+w);}
;if(this.__Bv==null){this.__Bv=by;}
;this.__ku[by]=bx;}
,replaceState:function(bC,bB){if(!bC instanceof qx.util.fsm.State){throw new Error(W+bl);}
;var bA=bC.getName();var bz=this.__ku[bA];this.__ku[bA]=bC;if(bB){bz._bNeedDispose=true;}
;return bz;}
...
;}
,_getInternalData:function(){return ({"states":this.__ku,"startState":this.__Bv,"eventQueue":this.__Bx,"blockedEvents":this.__By,"savedStates":this.__Bw,"friendlyToObject":this.__Bz,"friendlyToHash":this.__BA,"hashToFriendly":this.__BB,"groupToFriendly":this.__BC,"friendlyToGroups":this.__BD});}
...
;this.__ku[bV].getAutoActionsBeforeOnentry()(this);if(bU){this.debug(this.getName()+v+bV+T);}
;this.__ku[bV].getOnentry()(this,null);if(bU){this.debug(this.getName()+v+bV+H);}
;this.__ku[bV].getAutoActionsAfterOnentry()(this);}
...
;ck=this.getState();cn=this.__ku[ck];cq=cn.transitions;e=cn.getEvents()[event.getType()];if(!e){if(cu){this.debug(this.getName()+bs+event.getType()+m+q);}
;return true;}
...
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(z+ck+y+t+u+bp);};cl=cm.getNextState();if(typeof (cl)==k){if(!cl in this.__ku){throw new Error(n+cl);}
...
;cn=this.__ku[this.getNextState()];this.setPreviousState(ck);this.setState(this.getNextState());this.setNextState(null);cj=ck;ck=cl;cl=undefined;if(cr){this.debug(this.getName()+v+ck+J);}
...
;return true;}
},destruct:function(){this._disposeArray(c);this._disposeArray(K);this.__Bw=this.__ku=null;}
});}
)();
