...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.TreeController.748d92b34b85.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="dblclick",b="os.name",c="mouseup",d="mousedown",e="useraction",f="webkit",g="contextmenu",h="mousewheel",i="engine.name",j="mouseover",k="mouseout",l="gecko",m="ios",n="click",o="mousemove",p="qx.event.handler.Mouse",q="on";qx.Class.define(p,{extend:qx.core.Object,implement:[qx.event.IEventHandler,qx.core.IDisposable],construct:function(r){qx.core.Object.call(this);this.__cW=r;this.__cq=r.getWindow();this.__dD=this.__cq.document;this._initButtonObserver();this._initMoveObserver();this._initWheelObserver();}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{mousemove:1,mouseover:1,mouseout:1,mousedown:1,mouseup:1,click:1,dblclick:1,contextmenu:1,mousewheel:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE+qx.event.IEventHandler.TARGET_DOCUMENT+qx.event.IEventHandler.TARGET_WINDOW,IGNORE_CAN_HANDLE:true},members:{__hU:null,__hV:null,__hW:null,__hX:null,__hY:null,__cW:null,__cq:null,__dD:null,__ia:null,canHandleEvent:function(t,s){}
...
:(function(){return null;}
),__hb:function(A,B,C){if(!C){C=qx.bom.Event.getTarget(A);}
...
;qx.event.Registration.fireEvent(this.__cq,e,qx.event.type.Data,[B||A.type]);}
,preventNextClick:function(){this.__ia=true;}
,_initButtonObserver:function(){this.__hU=qx.lang.Function.listener(this._onButtonEvent,this);var Event=qx.bom.Event;Event.addNativeListener(this.__dD,d,this.__hU);Event.addNativeListener(this.__dD,c,this.__hU);Event.addNativeListener(this.__dD,n,this.__hU);Event.addNativeListener(this.__dD,a,this.__hU);Event.addNativeListener(this.__dD,g,this.__hU);}
...
,_initWheelObserver:function(){this.__hW=qx.lang.Function.listener(this._onWheelEvent,this);var D=qx.bom.client.Event.getMouseWheel(this.__cq);qx.bom.Event.addNativeListener(D.target,D.type,this.__hW);}
...
,_stopWheelObserver:function(){var E=qx.bom.client.Event.getMouseWheel(this.__cq);qx.bom.Event.removeNativeListener(E.target,E.type,this.__hW);}
,_onMoveEvent:qx.event.GlobalError.observeMethod(function(F){this.__hb(F);}
),_onButtonEvent:qx.event.GlobalError.observeMethod(function(I){var H=I.type;var J=qx.bom.Event.getTarget(I);if(H==n&&this.__ia){delete this.__ia;return;}
;if(qx.core.Environment.get(i)==l||qx.core.Environment.get(i)==f){if(J&&J.nodeType==3){J=J.parentNode;}
;}
;var G=qx.event.handler.DragDrop&&this.__cW.getHandler(qx.event.handler.DragDrop).isSessionActive();if(G&&H==n){return;}
;if(this.__ic){this.__ic(I,H,J);}
;this.__hb(I,H,J);if(this.__ib){this.__ib(I,H,J);}
;if(this.__id&&!G){this.__id(I,H,J);}
;this.__hX=H;}
),_onWheelEvent:qx.event.GlobalError.observeMethod(function(K){this.__hb(K,h);}
),__ib:qx.core.Environment.select(i,{"opera":function(L,M,N){if(M==c&&L.button==2){this.__hb(L,g,N);}
;}
,"default":null}),__ic:qx.core.Environment.select(i,{"mshtml":function(O,P,Q){if(O.target!==undefined){return;}
;if(P==c&&this.__hX==n){this.__hb(O,d,Q);}
else if(P==a){this.__hb(O,n,Q);}
;}
,"default":null}),__id:qx.core.Environment.select(i,{"mshtml":null,"default":function(S,R,T){switch(R){case d:this.__hY=T;break;case c:if(T!==this.__hY){var U=qx.dom.Hierarchy.getCommonParent(T,this.__hY);if(U){this.__hb(S,n,U);}
;}
;};}
...
;throw new Error(n);}
,getSelection:function(){return qx.lang.Object.getValues(this.__gh);}
,getSortedSelection:function(){var bU=this.getSelectables();var bT=qx.lang.Object.getValues(this.__gh);bT.sort(function(a,b){return bU.indexOf(a)-bU.indexOf(b);}
);return bT;}
,isItemSelected:function(bV){var bW=this._selectableToHashCode(bV);return this.__gh[bW]!==undefined;}
,isSelectionEmpty:function(){return qx.lang.Object.isEmpty(this.__gh);}
...
;this._fireChange(bw);}
,__oA:{Home:1,Down:1,Right:1,PageDown:1,End:1,Up:1,Left:1,PageUp:1},handleKeyPress:function(event){this._userInteraction=true;var dd,dc;var df=event.getKeyIdentifier();var de=this.getMode();var cY=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var da=event.isShiftPressed();var db=false;if(df===A&&cY){if(de!==bt&&de!==h){this._selectAllItems();db=true;}
;}
...
;db=true;}
;}
else if(this.__oA[df]){db=true;if(de===bt||de==h){dd=this._getSelectedItem();}
...
;}
,_clearSelection:function(){var di=this.__gh;for(var dj in di){this._removeFromSelection(di[dj]);}
;this.__gh={};}
,_selectItemRange:function(dk,dp,dm){var dq=this._getSelectableRange(dk,dp);if(!dm){var dn=this.__gh;var dr=this.__ry(dq);for(var dl in dn){if(!dr[dl]){this._removeFromSelection(dn[dl]);}
;}
;}
...
;return dx;}
,_getSelectedItem:function(){for(var dy in this.__gh){return this.__gh[dy];}
;return null;}
,_setSelectedItem:function(dA){if(this._isSelectable(dA)){var dz=this.__gh;var dB=this._selectableToHashCode(dA);if(!dz[dB]||(dz.length>=2)){this._clearSelection();this._addToSelection(dA);}
;}
;}
,_addToSelection:function(dC){var dD=this._selectableToHashCode(dC);if(this.__gh[dD]==null&&this._isSelectable(dC)){this.__gh[dD]=dC;this._styleSelectable(dC,R,true);this.__rs=true;}
;}
,_toggleInSelection:function(dE){var dF=this._selectableToHashCode(dE);if(this.__gh[dF]==null){this.__gh[dF]=dE;this._styleSelectable(dE,R,true);}
else {delete this.__gh[dF];this._styleSelectable(dE,R,false);}
;this.__rs=true;}
,_removeFromSelection:function(dG){var dH=this._selectableToHashCode(dG);if(this.__gh[dH]!=null){delete this.__gh[dH];this._styleSelectable(dG,R,false);this.__rs=true;}
;}
...
;}
;var dP=dK[0];var dI=dO;var dM=this.__gh;for(var dN in dM){if(dJ[dN]){delete dJ[dN];}
...
;return dS;}
;return null;}
},destruct:function(){this._disposeObjects(br);this.__gh=this.__rw=this.__rv=null;this.__ru=null;}
});}
)();
(function(){var a="vertical",b="under",c="above",d="qx.ui.core.selection.Widget",e="left",f="right";qx.Class.define(d,{extend:qx.ui.core.selection.Abstract,construct:function(g){qx.ui.core.selection.Abstract.call(this);this.__go=g;}
,members:{__go:null,_isSelectable:function(h){return this._isItemSelectable(h)&&h.getLayoutParent()===this.__go;}
,_selectableToHashCode:function(j){return j.$$hash;}
,_styleSelectable:function(k,m,n){n?k.addState(m):k.removeState(m);}
,_capture:function(){this.__go.capture();}
,_releaseCapture:function(){this.__go.releaseCapture();}
,_isItemSelectable:function(o){if(this._userInteraction){return o.isVisible()&&o.isEnabled();}
else {return o.isVisible();}
;}
,_getWidget:function(){return this.__go;}
,_getLocation:function(){var p=this.__go.getContentElement().getDomElement();return p?qx.bom.element.Location.get(p):null;}
,_getDimension:function(){return this.__go.getInnerSize();}
...
,_scrollItemIntoView:function(w){this.__go.scrollChildIntoView(w);}
,getSelectables:function(x){var y=false;if(!x){y=this._userInteraction;this._userInteraction=true;}
;var B=this.__go.getChildren();var z=[];var A;for(var i=0,l=B.length;i<l;i++ ){A=B[i];if(this._isItemSelectable(A)){z.push(A);}
;}
;this._userInteraction=y;return z;}
,_getSelectableRange:function(C,D){if(C===D){return [C];}
;var H=this.__go.getChildren();var E=[];var G=false;var F;for(var i=0,l=H.length;i<l;i++ ){F=H[i];if(F===C||F===D){if(G){E.push(F);break;}
else {G=true;}
;}
;if(G&&this._isItemSelectable(F)){E.push(F);}
;}
;return E;}
,_getFirstSelectable:function(){var I=this.__go.getChildren();for(var i=0,l=I.length;i<l;i++ ){if(this._isItemSelectable(I[i])){return I[i];}
;}
;return null;}
,_getLastSelectable:function(){var J=this.__go.getChildren();for(var i=J.length-1;i>0;i-- ){if(this._isItemSelectable(J[i])){return J[i];}
;}
;return null;}
,_getRelatedSelectable:function(N,O){var L=this.__go.getOrientation()===a;var P=this.__go.getChildren();var K=P.indexOf(N);var M;if((L&&O===c)||(!L&&O===e)){for(var i=K-1;i>=0;i-- ){M=P[i];if(this._isItemSelectable(M)){return M;}
;}
;}
...
;}
},destruct:function(){this.__go=null;}
});}
)();
...
(function(){var a="__rF",b="open",c="abstract",d="__gz",e="qx.debug",f="'. It is not a child of this tree item.",g="addItem",h="visible",j="excluded",k="removeItem",m="Invalid child index: ",n="Cannot remove treeitem '",o="qx.ui.tree.core.AbstractTreeItem";qx.Class.define(o,{extend:qx.ui.tree.core.AbstractItem,type:c,construct:function(p){qx.ui.tree.core.AbstractItem.call(this,p);this.__gz=[];}
,properties:{parent:{check:o,nullable:true}},members:{__gz:null,__rF:null,getTree:function(){var r=this;while(r.getParent()){r=r.getParent();}
...
;}
,getChildren:function(){return this.__gz;}
,hasChildren:function(){return this.__gz?this.__gz.length>0:false;}
...
;this.__gz.push(Q);if(!N){this.__rG();}
;if(P){Q.recursiveAddToWidgetQueue();P.fireNonBubblingEvent(g,qx.event.type.Data,[Q]);}
;}
;if(P){qx.ui.core.queue.Widget.add(this);}
;}
,addAt:function(X,T){if(qx.core.Environment.get(e)){this.assert(T<=this.__gz.length&&T>=0,m+T);}
;if(T==this.__gz.length){this.add(X);return;}
;var W=X.getParent();if(W){W.remove(X);}
;var U=this.getChildrenContainer();X.setParent(this);var V=this.hasChildren();var S=this.__gz[T];U.addBefore(X,S);if(X.hasChildren()){U.addAfter(X.getChildrenContainer(),X);}
;qx.lang.Array.insertAt(this.__gz,X,T);if(!V){this.__rG();}
;if(this.getTree()){X.recursiveAddToWidgetQueue();qx.ui.core.queue.Widget.add(this);}
;}
,addBefore:function(Y,bb){if(qx.core.Environment.get(e)){this.assert(this.__gz.indexOf(bb)>=0);}
;var ba=Y.getParent();if(ba){ba.remove(Y);}
;this.addAt(Y,this.__gz.indexOf(bb));}
,addAfter:function(bd,bc){if(qx.core.Environment.get(e)){this.assert(this.__gz.indexOf(bc)>=0);}
;var be=bd.getParent();if(be){be.remove(bd);}
;this.addAt(bd,this.__gz.indexOf(bc)+1);}
,addAtBegin:function(bf){this.addAt(bf,0);}
,remove:function(bk){for(var i=0,l=arguments.length;i<l;i++ ){var bj=arguments[i];if(this.__gz.indexOf(bj)==-1){this.warn(n+bj+f);return;}
...
;}
;qx.lang.Array.remove(this.__gz,bj);bj.setParent(null);bg.remove(bj);}
;var bh=this.getTree();if(bh){bh.fireNonBubblingEvent(k,qx.event.type.Data,[bj]);}
;qx.ui.core.queue.Widget.add(this);}
,removeAt:function(bl){var bm=this.__gz[bl];if(bm){this.remove(bm);}
;}
,removeAll:function(){var bn=this.__gz.concat();for(var i=this.__gz.length-1;i>=0;i-- ){this.remove(this.__gz[i]);}
;return bn;}
},destruct:function(){this._disposeArray(d);this._disposeObjects(a);}
});}
)();
...
;this.__cW.removeItem(I);}
,selectRange:function(J,K){this.__cW.selectItemRange(J,K);}
,resetSelection:function(){this.__cW.clearSelection();}
...
else {var M=this.getSelection();if(!qx.lang.Array.equals(M,L)){this.__cW.replaceSelection(L);}
;}
;}
,getSelection:function(){return this.__cW.getSelection();}
,getSortedSelection:function(){return this.__cW.getSortedSelection();}
,isSelectionEmpty:function(){return this.__cW.isSelectionEmpty();}
,getSelectionContext:function(){return this.__cW.getSelectionContext();}
,_getManager:function(){return this.__cW;}
,getSelectables:function(N){return this.__cW.getSelectables(N);}
,invertSelection:function(){this.__cW.invertSelection();}
,_getLeadItem:function(){var O=this.__cW.getMode();if(O===r||O===b){return this.__cW.getSelectedItem();}
else {return this.__cW.getLeadItem();}
;}
,_applySelectionMode:function(Q,P){this.__cW.setMode(Q);}
,_applyDragSelection:function(S,R){this.__cW.setDrag(S);}
,_applyQuickSelection:function(U,T){this.__cW.setQuick(U);}
...
,bindProperty:function(bN,bQ,bP,bM,bS){var bR=bS.bind(bN,bM,bQ,bP);if(this.__cX[bQ]==null){this.__cX[bQ]={};}
;var bO=this.__cX[bQ];qx.core.ObjectRegistry.register(bS);if(bO[bS.toHashCode()]){if(bO[bS.toHashCode()].id){throw new Error(D+bQ+z);}
...
;}
,bindPropertyReverse:function(bX,bT,bV,bW,ca){var bY=bW.bind(bT,ca,bX,bV);if(this.__cX[bT]==null){this.__cX[bT]={};}
;var bU=this.__cX[bT];qx.core.ObjectRegistry.register(ca);if(bU[ca.toHashCode()]){if(bU[ca.toHashCode()].reverseId){throw new Error(o+bX+z);}
...
;}
,__uA:function(){var ce;for(var ch in this.__cX){ce=ch;break;}
;for(var cf in this.__cX[ce]){var cd=this.__cX[ce][cf].treeNode;var cg=qx.core.ObjectRegistry.fromHashCode(cf);this.__uN(cg);this.__uM(cg,cd);}
;}
...
;}
,__uN:function(cn){for(var i=0;i<this.__uo.length;i++ ){var cl=this.__uo[i];var cm=this.__cX[cl][cn.toHashCode()];if(cm!=null){if(cm.id){cn.removeBinding(cm.id);cm.id=null;}
;if(cm.reverseId){cm.treeNode.removeBinding(cm.reverseId);cm.reverseId=null;}
;delete this.__cX[cl][cn.toHashCode()];}
;}
;}
...
;this.__cX=this.__uD=this.__uo=null;}
});}
)();
...
;if(S!=null){if(this.getModel()!=null){for(var i=0;i<this.__un.length;i++ ){this.__uy(this.__eN(i));}
;}
;}
;}
...
;this.__uC();var U=this.__un.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++ ){this.__uy(this.__eN(j));}
;}
...
;var bF=this.getTarget().getChildren();for(var i=0;i<bF.length;i++ ){this._removeBindingsFrom(bF[i]);this._bindListItem(bF[i],this.__eN(i));}
;this.__ux();}
...
;}
;var bS=this.getTarget().getChildren();for(var i=0;i<bS.length;i++ ){this._bindListItem(bS[i],this.__eN(i));}
...
;}
;}
,__eN:function(bY){return this.__un[bY];}
...
(function(){var a="changeNames",b="changeName",c="Validation Error: ",d="- ",e="",f="changeChild",g="qx.data.Array",h="changeColor",j="Marge",k="AFFE",l="is no number (parseFloat says so!).",m="__Ap",n="Homer",o="\n",p="demobrowser.demo.data.model.Node",q="String",r="changeName2",s="changeChildren";qx.Class.define(p,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.setChildren(new qx.data.Array());}
,properties:{children:{check:g,event:s},child:{check:p,event:f,nullable:true},names:{check:g,event:a,init:new qx.data.Array(n,j)},name:{check:q,init:k,event:b},name2:{check:q,event:r},number:{init:10,validate:m},color:{event:h,nullable:true}},members:{__Ap:function(t){if(!isNaN(parseFloat(t))){if(/^\d+$/.test(t)){return;}
;}
