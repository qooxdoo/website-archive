...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.TreeController.b231d01c43b2.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;this._fireChange();}
,_setLeadItem:function(cb){var ca=this.__ru;if(ca!==null){this._styleSelectable(ca,u,false);}
;if(cb!==null){this._styleSelectable(cb,u,true);}
;this.__ru=cb;}
,getLeadItem:function(){return this.__ru;}
,_setAnchorItem:function(cd){var cc=this.__rv;if(cc!=null){this._styleSelectable(cc,I,false);}
;if(cd!=null){this._styleSelectable(cd,I,true);}
;this.__rv=cd;}
,_getAnchorItem:function(){return this.__rv!==null?this.__rv:null;}
...
;this._fireChange();}
,handlePointerOver:function(event){if(this.__rx!=null&&this.__rx!=this._getScroll().top){this.__rx=null;return;}
...
;var cD=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cA=event.isShiftPressed();if(this.isItemSelected(cB)&&!cA&&!cD&&!this.getDrag()){this.__rw=cB;this._userInteraction=false;return;}
else {this.__rw=null;}
;this._scrollItemIntoView(cB);var cC=this.getMode();if(this.getDrag()&&cC!==bt&&cC!==h&&!cA&&!cD&&event.getPointerType()==F){this._setAnchorItem(cB);this._setLeadItem(cB);this.__rk=this._getLocation();this.__rh=this._getScroll();this.__rl=event.getDocumentLeft()+this.__rh.left;this.__rm=event.getDocumentTop()+this.__rh.top;this.__rn=true;this._capture();}
;this._fireChange(bD);this._userInteraction=false;}
,handleTap:function(event){this._userInteraction=true;var cH=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cE=event.isShiftPressed();if(!cH&&!cE&&this.__rw!=null){this._userInteraction=false;var cF=this._getSelectableFromPointerEvent(event);if(cF===null||!this.isItemSelected(cF)){return;}
;}
...
,handlePointerMove:function(event){if(!this.__rn){return;}
;this.__ro=event.getDocumentLeft();this.__rp=event.getDocumentTop();this._userInteraction=true;var cI=this.__ro+this.__rh.left;if(cI>this.__rl){this.__rq=1;}
else if(cI<this.__rl){this.__rq=-1;}
else {this.__rq=0;}
;var cJ=this.__rp+this.__rh.top;if(cJ>this.__rm){this.__rr=1;}
else if(cJ<this.__rm){this.__rr=-1;}
else {this.__rr=0;}
;var location=this.__rk;if(this.__ro<location.left){this.__re=this.__ro-location.left;}
else if(this.__ro>location.right){this.__re=this.__ro-location.right;}
else {this.__re=0;}
;if(this.__rp<location.top){this.__rf=this.__rp-location.top;}
else if(this.__rp>location.bottom){this.__rf=this.__rp-location.bottom;}
else {this.__rf=0;}
;if(!this.__rg){this.__rg=new qx.event.Timer(100);this.__rg.addListener(G,this._onInterval,this);}
;this.__rg.start();this._autoSelect();event.stopPropagation();this._userInteraction=false;}
,handleAddItem:function(e){var cK=e.getData();if(this.getMode()===h&&this.isSelectionEmpty()){this.addItem(cK);}
;}
,handleRemoveItem:function(e){this.removeItem(e.getData());}
,_cleanup:function(){if(!this.getDrag()&&this.__rn){return;}
;if(this.__rs){this._fireChange(bD);}
;delete this.__rn;delete this.__ri;delete this.__rj;this._releaseCapture();if(this.__rg){this.__rg.stop();}
;}
,_onInterval:function(e){this._scrollBy(this.__re,this.__rf);this.__rh=this._getScroll();this._autoSelect();}
,_autoSelect:function(){var cT=this._getDimension();var cM=Math.max(0,Math.min(this.__ro-this.__rk.left,cT.width))+this.__rh.left;var cL=Math.max(0,Math.min(this.__rp-this.__rk.top,cT.height))+this.__rh.top;if(this.__ri===cM&&this.__rj===cL){return;}
;this.__ri=cM;this.__rj=cL;var cQ=this._getAnchorItem();var cO=cQ;var cR=this.__rq;var cU,cN;while(cR!==0){cU=cR>0?this._getRelatedSelectable(cO,t):this._getRelatedSelectable(cO,V);if(cU!==null){cN=this._getSelectableLocationX(cU);if((cR>0&&cN.left<=cM)||(cR<0&&cN.right>=cM)){cO=cU;continue;}
;}
;break;}
;var cS=this.__rr;var cV,cP;while(cS!==0){cV=cS>0?this._getRelatedSelectable(cO,z):this._getRelatedSelectable(cO,bB);if(cV!==null){cP=this._getSelectableLocationY(cV);if((cS>0&&cP.top<=cL)||(cS<0&&cP.bottom>=cL)){cO=cV;continue;}
;}
;break;}
...
;this._fireChange(bw);}
,__oy:{Home:1,Down:1,Right:1,PageDown:1,End:1,Up:1,Left:1,PageUp:1},handleKeyPress:function(event){this._userInteraction=true;var dd,dc;var df=event.getKeyIdentifier();var de=this.getMode();var cY=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var da=event.isShiftPressed();var db=false;if(df===A&&cY){if(de!==bt&&de!==h){this._selectAllItems();db=true;}
;}
...
;db=true;}
;}
else if(this.__oy[df]){db=true;if(de===bt||de==h){dd=this._getSelectedItem();}
...
;}
;break;};this.__rx=this._getScroll().top;this._scrollItemIntoView(dc);}
;}
...
;this.__gV={};}
,_selectItemRange:function(dk,dp,dm){var dq=this._getSelectableRange(dk,dp);if(!dm){var dn=this.__gV;var dr=this.__ry(dq);for(var dl in dn){if(!dr[dl]){this._removeFromSelection(dn[dl]);}
;}
;}
...
;}
,__ry:function(dw){var dx={};var dv;for(var i=0,l=dw.length;i<l;i++ ){dv=dw[i];dx[this._selectableToHashCode(dv)]=dv;}
;return dx;}
...
;}
;}
,_addToSelection:function(dC){var dD=this._selectableToHashCode(dC);if(this.__gV[dD]==null&&this._isSelectable(dC)){this.__gV[dD]=dC;this._styleSelectable(dC,R,true);this.__rs=true;}
;}
...
;this.__rs=true;}
,_removeFromSelection:function(dG){var dH=this._selectableToHashCode(dG);if(this.__gV[dH]!=null){delete this.__gV[dH];this._styleSelectable(dG,R,false);this.__rs=true;}
;}
...
;if(!dL){return false;}
;this._scrollItemIntoView(dI);this._setLeadItem(dP);this._setAnchorItem(dP);this.__rs=true;this._fireChange();}
,_fireChange:function(dQ){if(this.__rs){this.__rt=dQ||null;this.fireDataEvent(bA,this.getSelection());delete this.__rs;}
;}
...
;return dS;}
;return null;}
},destruct:function(){this._disposeObjects(br);this.__gV=this.__rw=this.__rv=null;this.__ru=null;}
});}
)();
...
,addSpacer:function(){if(!this.__rB){this.__rB=new qx.ui.core.Spacer();}
else {this._remove(this.__rB);}
;this._add(this.__rB);}
,addOpenButton:function(){this._add(this.getChildControl(l));}
,_onChangeOpen:function(e){if(this.isOpenable()){this.setOpen(e.getData());}
;}
,addIcon:function(){var L=this.getChildControl(t);if(this.__rA){this._remove(L);}
;this._add(L);this.__rA=true;}
,addLabel:function(M){var N=this.getChildControl(D);if(this.__rz){this._remove(N);}
;if(M){this.setLabel(M);}
else {N.setValue(this.getLabel());}
;this._add(N);this.__rz=true;}
,_applyIcon:function(P,O){if(!this.__rD()){this.__rE(P);}
else if(!this.isOpen()){this.__rE(P);}
;}
,_applyIconOpened:function(R,Q){if(this.isOpen()){if(this.__rC()&&this.__rD()){this.__rE(R);}
else if(!this.__rC()&&this.__rD()){this.__rE(R);}
;}
;}
...
;var X;if(W){X=this.__rD()?this.getIconOpened():null;}
else {X=this.getIcon();}
;if(X){this.__rE(X);}
;W?this.addState(u):this.removeState(u);}
,__rC:function(){return qx.util.PropertyUtil.getUserValue(this,t);}
,__rD:function(){return qx.util.PropertyUtil.getUserValue(this,v);}
,__rE:function(Y){var ba=this.getChildControl(t,true);if(ba){ba.setSource(Y);}
;}
...
;}
;if(this.__rB){this.__rB.setWidth((this.getLevel()+1)*this.getIndent()-bf);}
;}
...
(function(){var a="__hp",b="__rF",c="open",d="abstract",e="qx.debug",f="'. It is not a child of this tree item.",g="addItem",h="visible",j="excluded",k="removeItem",m="Invalid child index: ",n="Cannot remove treeitem '",o="qx.ui.tree.core.AbstractTreeItem";qx.Class.define(o,{extend:qx.ui.tree.core.AbstractItem,type:d,construct:function(p){qx.ui.tree.core.AbstractItem.call(this,p);this.__hp=[];}
,properties:{parent:{check:o,nullable:true}},members:{__hp:null,__rF:null,getTree:function(){var r=this;while(r.getParent()){r=r.getParent();}
...
,_shouldShowOpenSymbol:function(){var open=this.getChildControl(c,true);if(!open){return false;}
...
;}
;}
,getChildrenContainer:function(){if(!this.__rF){this.__rF=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?h:j});}
;return this.__rF;}
,hasChildrenContainer:function(){return this.__rF;}
...
;}
,__rG:function(){if(this.getParentChildrenContainer()){this.getParentChildrenContainer()._addAfter(this.getChildrenContainer(),this);}
;}
...
;this.__hp.push(Q);if(!N){this.__rG();}
...
;qx.lang.Array.insertAt(this.__hp,X,T);if(!V){this.__rG();}
...
;return bn;}
},destruct:function(){this._disposeArray(a);this._disposeObjects(b);}
});}
)();
...
(function(){var a="Enter",b="_applyOpenMode",c="excluded",d="Space",f="Left",g="Boolean",h="changeOpenMode",j="_applyRootOpenClose",k="visible",l="changeSelection",m="qx.ui.tree.Tree",n="qx.event.type.Data",o="qx.ui.tree.core.AbstractTreeItem",p="tree",q="dbltap",r="_applyHideRoot",s="changeRoot",t="__sh",u="_applyRoot",v="keypress",w="none",x="pane",y="tap",z="Right";qx.Class.define(m,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IModelSelection,qx.ui.form.IForm],include:[qx.ui.core.MMultiSelectionHandling,qx.ui.core.MContentPadding,qx.ui.form.MModelSelection,qx.ui.form.MForm],construct:function(){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__sh=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({allowShrinkY:false,allowGrowX:true});this.getChildControl(x).add(this.__sh);this.initOpenMode();this.initRootOpenClose();this.addListener(l,this._onChangeSelection,this);this.addListener(v,this._onKeyPress,this);}
,events:{addItem:n,removeItem:n},properties:{openMode:{check:[y,q,w],init:q,apply:b,event:h,themeable:true},root:{check:o,init:null,nullable:true,event:s,apply:u},hideRoot:{check:g,init:false,apply:r},rootOpenClose:{check:g,init:false,apply:j},appearance:{refine:true,init:p},focusable:{refine:true,init:true}},members:{__sh:null,SELECTION_MANAGER:qx.ui.tree.selection.SelectionManager,getChildrenContainer:function(){return this.__sh;}
...
,_getContentPaddingTarget:function(){return this.__sh;}
...
;if(this.getSelection()==null){this.__Bk=new qx.data.Array();this.setSelection(this.__Bk);}
;}
,properties:{selection:{check:b,event:f,apply:d,init:null}},events:{"changeSelection":h},members:{_modifingSelection:0,__Bl:null,__Bm:null,__Bk:null,_applySelection:function(k,j){if(this.__Bm!=undefined&&j!=undefined){j.removeListenerById(this.__Bm);this.__Bm=null;}
;if(k){this.__Bm=k.addListener(a,this.__Bn,this);}
;this._updateSelection();}
,__Bn:function(){this._updateSelection();}
,_changeTargetSelection:function(){if(this.getTarget()==null){return;}
;if(!this.__Bo()&&!this.__Bp()){return;}
;if(this._inSelectionModification()){return;}
;var m=this.getTarget().getSelection();var l=this.getSelection();if(l==null){l=new qx.data.Array();this.__Bk=l;this.setSelection(l);}
...
,_addChangeTargetListener:function(p,o){if(this.__Bl!=undefined&&o!=undefined){o.removeListenerById(this.__Bl);}
;if(p!=null){if(this.__Bo()||this.__Bp()){this.__Bl=p.addListener(f,this._changeTargetSelection,this);}
;}
;}
,_updateSelection:function(){if(!this.getTarget()||!this.getSelection()){return;}
;this._startSelectionModification();if(this.__Bo()){var s=[];for(var i=0;i<this.getSelection().length;i++ ){var t=this.getSelection().getItem(i);var u=this.__Br(t);if(u!=null){s.push(u);}
;}
...
;}
;}
else if(this.__Bp()){var r=this.getSelection().getItem(this.getSelection().length-1);if(r!==undefined){this.__Bq(r);this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();}
else {this.getTarget().resetSelection();}
;}
;this._endSelectionModification();}
,__Bo:function(){var v=this.getTarget().constructor;return qx.Class.implementsInterface(v,qx.ui.core.IMultiSelection);}
,__Bp:function(){var w=this.getTarget().constructor;return qx.Class.implementsInterface(w,qx.ui.core.ISingleSelection);}
,__Bq:function(x){var y=this.__Br(x);if(y==null){return;}
;if(this.__Bo()){this.getTarget().addToSelection(y);}
else if(this.__Bp()){this.getTarget().setSelection([y]);}
;}
,__Br:function(z){var A=this.getTarget().getSelectables(true);for(var i=0;i<A.length;i++ ){if(A[i].getModel()==z){return A[i];}
;}
;return null;}
...
},destruct:function(){if(this.__Bk){this.__Bk.dispose();}
;}
});}
)();
...
(function(){var a="changeModel",b="qx.core.Object",c="_applyLabelPath",d="changeTarget",f="_applyLabelOptions",g="qx.data.controller.Tree",h="2: removing children=",k="_applyModel",l="1: adding children=",m="change",n=" to this=",o="Can not reverse bind the same target property '",p="get",q="1: removing children=",r="_applyIconPath",s="_applyDelegate",t=" from this=",u="icon",v="erasing all children from this=",w="_applyTarget",x="label",y="String",z="' twice.",A="_applyChildPath",B="_applyIconOptions",C="2: adding children=",D="Can not bind the same target property '";qx.Class.define(g,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:[qx.data.controller.ISelection],construct:function(F,G,H,E){qx.core.Object.call(this);this.__bt={};this.__Bs=[];this.__BG={a:1};if(H!=null){this.setChildPath(H);}
;if(E!=null){this.setLabelPath(E);}
;if(F!=null){this.setModel(F);}
;if(G!=null){this.setTarget(G);}
;}
,properties:{model:{check:b,apply:k,event:a,nullable:true,dereference:true},target:{apply:w,event:d,init:null,nullable:true,dereference:true},childPath:{check:y,apply:A,nullable:true},labelPath:{check:y,apply:c,nullable:true},iconPath:{check:y,apply:r,nullable:true},labelOptions:{apply:f,nullable:true},iconOptions:{apply:B,nullable:true},delegate:{apply:s,init:null,nullable:true}},members:{__BG:null,__bt:null,__Bs:null,__BH:null,_applyDelegate:function(J,I){this._setConfigureItem(J,I);this._setCreateItem(J,I);this._setBindItem(J,I);}
,_applyIconOptions:function(L,K){this.__BE();}
,_applyLabelOptions:function(N,M){this.__BE();}
,_applyTarget:function(P,O){if(O!=undefined){this.__BM(O);}
;if(this.getModel()!=null){this.__BK();}
;this._addChangeTargetListener(P,O);}
,_applyModel:function(R,Q){this.__BK();}
,_applyChildPath:function(T,S){this.__BH=S;this.__BK();this.__BH=null;}
,_applyIconPath:function(V,U){this.__BE();}
,_applyLabelPath:function(X,W){this.__BK();}
,__BI:function(Y){var bb=Y.getTarget();qx.core.ObjectRegistry.register(bb);var ba=this.__BG[bb.toHashCode()].treeNode;var bc=this.__BG[bb.toHashCode()].modelNode;this.__BL(ba,bc);this._updateSelection();}
,__BJ:function(e){var bf=e.getData();var bj=e.getOldData();var bd=this.__BG[bj.toHashCode()];bj.removeListenerById(bd.changeListenerId);this.debug(q+bj.toHashCode()+t+this.toHashCode());delete this.__BG[bj.toHashCode()];bd.modelNode.removeListenerById(bd.changeChildernListenerId);var bk=bd.modelNode;var bi=qx.Class.getPropertyDefinition(bd.modelNode.constructor,this.getChildPath());var bg=bi.event;var bh=bk.addListener(bg,this.__BJ,this);var be=bd.treeNode;this.debug(l+bf.toHashCode()+n+this.toHashCode());this.__BG[bf.toHashCode()]={modelNode:bk,treeNode:be,changeListenerId:bd.changeListenerId,changeChildernListenerId:bh};this.__BL(be,bk);this._updateSelection();}
...
;return bl;}
,__BK:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__BM();if(this.getModel()!=null){var bn=this._createItem();bn.setModel(this.getModel());this.__zX(this.getModel(),bn);this.__BL(bn,this.getModel());this.getTarget().setRoot(bn);}
;}
,__BL:function(bu,by){if(by[p+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bq=by[p+qx.lang.String.firstUp(this.getChildPath())]();if(this.__BG[bq.toHashCode()]==undefined){var bs=bq.addListener(m,this.__BI,this);var bw=qx.Class.getPropertyDefinition(by.constructor,this.getChildPath());var bt=bw.event;var bx=by.addListener(bt,this.__BJ,this);this.debug(C+bq.toHashCode()+n+this.toHashCode());this.__BG[bq.toHashCode()]={modelNode:by,treeNode:bu,changeListenerId:bs,changeChildernListenerId:bx};}
...
;}
else {var bv=this._createItem();bv.setModel(bq.getItem(i));bu.addAt(bv,i);this.__zX(bq.getItem(i),bv);this.__BL(bv,bq.getItem(i));}
;}
;}
;for(var i=bu.getChildren().length-1;i>=bq.length;i-- ){var br=bu.getChildren()[i];this.__BO(br,bu);}
;}
,__BM:function(bA){if(bA==null){bA=this.getTarget();}
;if(bA==null){return;}
;var bC=bA.getRoot();if(bC!=null){bA.setRoot(null);this.__BN(bC);var bB=bC.getModel();if(bB){this.__BP(bB);}
;bC.destroy();this.debug(v+this.toHashCode());this.__BG={b:2};}
;}
,__BN:function(bD){var bE=bD.getChildren()||[];for(var i=bE.length-1;i>=0;i-- ){if(bE[i].getChildren().length>0){this.__BN(bE[i]);}
;this.__BO(bE[i],bD);}
;}
,__BO:function(bG,bJ){var bK=bG.getModel();var bF=this.__BH||this.getChildPath();var bH=p+qx.lang.String.firstUp(bF);if(bK[bH]!=undefined){var bL=bK[bH]();this.debug(h+bL.toHashCode()+t+this.toHashCode());var bI=this.__BG[bL.toHashCode()];bL.removeListenerById(bI.changeListenerId);bK.removeListenerById(bI.changeChildernListenerId);this.__BN(bG);delete this.__BG[bL.toHashCode()];}
;this.__BP(bK);bJ.remove(bG);bG.destroy();}
...
;if(!qx.lang.Array.contains(this.__Bs,bQ)){this.__Bs.push(bQ);}
;}
...
;if(!qx.lang.Array.contains(this.__Bs,bT)){this.__Bs.push(bT);}
;}
...
;}
,__BE:function(){var ce;for(var ch in this.__bt){ce=ch;break;}
;for(var cf in this.__bt[ce]){var cd=this.__bt[ce][cf].treeNode;var cg=qx.core.ObjectRegistry.fromHashCode(cf);this.__BP(cg);this.__zX(cg,cd);}
;}
,__zX:function(ck,ci){var cj=this.getDelegate();if(cj!=null&&cj.bindItem!=null){cj.bindItem(this,ci,ck);}
else {this.bindDefaultProperties(ci,ck);}
;}
,__BP:function(cn){for(var i=0;i<this.__Bs.length;i++ ){var cl=this.__Bs[i];var cm=this.__bt[cl][cn.toHashCode()];if(cm!=null){if(cm.id){cn.removeBinding(cm.id);cm.id=null;}
...
;this._startSelectionModification();this.__BM();this.__BK();this._endSelectionModification();this._updateSelection();}
...
;this.__bt=this.__BG=this.__Bs=null;}
});}
)();
...
(function(){var a="indexOf",b="addAfter",c="add",d="addBefore",e="_",f="addAt",g="hasChildren",h="removeAt",i="removeAll",j="getChildren",k="remove",l="qx.ui.core.MRemoteChildrenHandling";qx.Mixin.define(l,{members:{__oz:function(q,m,o,n){var p=this.getChildrenContainer();if(p===this){q=e+q;}
;return (p[q])(m,o,n);}
,getChildren:function(){return this.__oz(j);}
,hasChildren:function(){return this.__oz(g);}
,add:function(s,r){return this.__oz(c,s,r);}
,remove:function(t){return this.__oz(k,t);}
,removeAll:function(){return this.__oz(i);}
,indexOf:function(u){return this.__oz(a,u);}
,addAt:function(x,v,w){this.__oz(f,x,v,w);}
,addBefore:function(y,A,z){this.__oz(d,y,A,z);}
,addAfter:function(D,B,C){this.__oz(b,D,B,C);}
,removeAt:function(E){return this.__oz(h,E);}
}});}
)();
(function(){var a="qx.ui.form.List",b="text",c="Boolean",d="one",f="addChildWidget",g="_applySpacing",h="Enter",j="Integer",k="action",m="keyinput",n="qx.event.type.Data",o="addItem",p="vertical",q="removeChildWidget",r="",s="_applyOrientation",t="__sh",u="single",v="keypress",w="list",x="horizontal",y="label",z="pane",A="removeItem";qx.Class.define(a,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MMultiSelectionHandling,qx.ui.form.MForm,qx.ui.form.MModelSelection],construct:function(B){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__sh=this._createListItemContainer();this.__sh.addListener(f,this._onAddChild,this);this.__sh.addListener(q,this._onRemoveChild,this);this.getChildControl(z).add(this.__sh);if(B){this.setOrientation(x);}
else {this.initOrientation();}
;this.addListener(v,this._onKeyPress);this.addListener(m,this._onKeyInput);this.__tW=r;}
,events:{addItem:n,removeItem:n},properties:{appearance:{refine:true,init:w},focusable:{refine:true,init:true},orientation:{check:[x,p],init:p,apply:s},spacing:{check:j,init:0,apply:g,themeable:true},enableInlineFind:{check:c,init:true}},members:{__tW:null,__tX:null,__sh:null,SELECTION_MANAGER:qx.ui.core.selection.ScrollArea,getChildrenContainer:function(){return this.__sh;}
...
,_applyOrientation:function(F,C){var D=F===x;var E=D?new qx.ui.layout.HBox():new qx.ui.layout.VBox();var content=this.__sh;content.setLayout(E);content.setAllowGrowX(!D);content.setAllowGrowY(D);this._applySpacing(this.getSpacing());}
,_applySpacing:function(H,G){this.__sh.getLayout().setSpacing(H);}
...
;if(((new Date).valueOf()-this.__tX)>1000){this.__tW=r;}
;this.__tW+=e.getChar();var K=this.findItemByLabelFuzzy(this.__tW);if(K){this.setSelection([K]);}
;this.__tX=(new Date).valueOf();}
...
(function(){var a="changeModel",b="_applyLabelOptions",c="_applyLabelPath",d="changeTarget",e="_applyModel",f="change",g="icon",h="ReverseBindingId",k="qx.data.controller.List",m="_applyIconPath",n="BindingId",o="_applyDelegate",p="",q="changeDelegate",r="_applyTarget",s="]",t="qx.data.IListData",u="model[",v="model",w="label",x="String",y="_applyIconOptions",z=".";qx.Class.define(k,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(A,B,C){qx.core.Object.call(this);this.__zC=[];this.__Bs=[];this.__Bt=[];this.__Bu={};if(C!=null){this.setLabelPath(C);}
;if(A!=null){this.setModel(A);}
;if(B!=null){this.setTarget(B);}
;}
,properties:{model:{check:t,apply:e,event:a,nullable:true,dereference:true},target:{apply:r,event:d,nullable:true,init:null,dereference:true},labelPath:{check:x,apply:c,nullable:true},iconPath:{check:x,apply:m,nullable:true},labelOptions:{apply:b,nullable:true},iconOptions:{apply:y,nullable:true},delegate:{apply:o,event:q,init:null,nullable:true}},members:{__Bv:null,__zC:null,__Bu:null,__Bs:null,__Bt:null,__Bw:null,__Bx:null,update:function(){this.__BA();this.__BE();this._updateSelection();}
,_applyDelegate:function(E,D){this._setConfigureItem(E,D);this._setFilter(E,D);this._setCreateItem(E,D);this._setBindItem(E,D);}
,_applyIconOptions:function(G,F){this.__BE();}
,_applyLabelOptions:function(I,H){this.__BE();}
,_applyIconPath:function(K,J){this.__BE();}
,_applyLabelPath:function(M,L){this.__BE();}
,_applyModel:function(P,N){if(N!=undefined){if(this.__Bv!=undefined){N.removeListenerById(this.__Bv);}
;}
;if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();}
;if(P!=null){this.__Bv=P.addListener(f,this.__Bz,this);this.__zL();this.__BA();if(N==null){this._changeTargetSelection();}
else {this.__Bw=true;qx.ui.core.queue.Widget.add(this);}
;}
else {var O=this.getTarget();if(O!=null){var length=O.getChildren().length;for(var i=0;i<length;i++ ){this.__BD();}
;}
;}
;}
...
;if(S!=null){if(this.getModel()!=null){for(var i=0;i<this.__zC.length;i++ ){this.__BC(this.__jP(i));}
;}
;}
;}
,__By:false,__Bz:function(){if(this.__By){return;}
;this.__By=true;this.__Bx=true;qx.ui.core.queue.Widget.add(this);this.update();this.__By=false;}
,syncWidget:function(){if(this.__Bw){this._changeTargetSelection();}
;if(this.__Bx){this._updateSelection();}
;this.__Bx=this.__Bw=null;}
,__BA:function(){if(this.getTarget()==null){return;}
;this.__zL();var U=this.__zC.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++ ){this.__BC(this.__jP(j));}
;}
else if(U<T){for(var j=T;j>U;j-- ){this.__BD();}
;}
;this.__zL();this.__Bw=true;qx.ui.core.queue.Widget.add(this);}
,__BB:function(){var V=this.getModel();if(V!=null){V.removeListenerById(this.__Bv);this.__Bv=V.addListener(f,this.__Bz,this);}
;}
...
;return W;}
,__BC:function(Y){var ba=this._createItem();this._bindListItem(ba,Y);this.getTarget().add(ba);}
,__BD:function(){this._startSelectionModification();var bc=this.getTarget().getChildren();var bb=bc.length-1;var bd=bc[bb];this._removeBindingsFrom(bd);this.getTarget().removeAt(bb);bd.destroy();this._endSelectionModification();}
...
;}
,bindProperty:function(bo,br,bp,bn,bm){if(bp!=null){var bp=qx.lang.Object.clone(bp);this.__Bu[br]=bp.onUpdate;delete bp.onUpdate;}
else {bp={};this.__Bu[br]=null;}
;bp.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,bm);bp.ignoreConverter=v;var bq=u+bm+s;if(bo!=null&&bo!=p){bq+=z+bo;}
;var bs=this.bind(bq,bn,br,bp);bn.setUserData(br+n,bs);if(!qx.lang.Array.contains(this.__Bs,br)){this.__Bs.push(br);}
;}
,bindPropertyReverse:function(by,bv,bw,bx,bt){var bu=u+bt+s;if(by!=null&&by!=p){bu+=z+by;}
;var bz=bx.bind(bv,this,bu,bw);bx.setUserData(by+h,bz);if(!qx.lang.Array.contains(this.__Bt,by)){this.__Bt.push(by);}
;}
,_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;}
;for(var i=0;i<this.__Bs.length;i++ ){if(this.__Bu[this.__Bs[i]]!=null){this.__Bu[this.__Bs[i]]();}
;}
;}
,_removeBindingsFrom:function(bD){for(var i=0;i<this.__Bs.length;i++ ){var bE=bD.getUserData(this.__Bs[i]+n);if(bE!=null){this.removeBinding(bE);}
;}
;for(var i=0;i<this.__Bt.length;i++ ){var bE=bD.getUserData(this.__Bt[i]+h);if(bE!=null){bD.removeBinding(bE);}
;}
;}
,__BE:function(){if(this.getTarget()==null||this.getModel()==null){return;}
...
,_setFilter:function(bR,bP){if((bR==null||bR.filter==null)&&(bP!=null&&bP.filter!=null)){this.__BF();}
...
;var bQ=this.__zC;this.__zL();if(bQ.length>this.__zC.length){for(var j=bQ.length;j>this.__zC.length;j-- ){this.getTarget().removeAt(j-1).destroy();}
;}
else if(bQ.length<this.__zC.length){for(var j=bQ.length;j<this.__zC.length;j++ ){var bT=this._createItem();this.getTarget().add(bT);}
;}
;var bS=this.getTarget().getChildren();for(var i=0;i<bS.length;i++ ){this._bindListItem(bS[i],this.__jP(i));}
;this.__BB();this._endSelectionModification();this._updateSelection();}
,__BF:function(){this.__zL();this.__BA();this.__BE();this.__Bx=true;qx.ui.core.queue.Widget.add(this);}
,__zL:function(){var bW=this.getModel();if(bW==null){return;}
;var bX=this.getDelegate();if(bX!=null){var bV=bX.filter;}
;this.__zC=[];for(var i=0;i<bW.getLength();i++ ){if(bV==null||bV(bW.getItem(i))){this.__zC.push(i);}
;}
;}
,__jP:function(bY){return this.__zC[bY];}
},destruct:function(){this.__zC=this.__Bu=this.__Bs=null;this.__Bt=null;qx.ui.core.queue.Widget.remove(this);}
});}
)();
...
(function(){var a="changeNames",b="changeName",c="Validation Error: ",d="- ",e="",f="changeChild",g="qx.data.Array",h="changeColor",j="Marge",k="AFFE",l="is no number (parseFloat says so!).",m="__bH",n="Homer",o="\n",p="demobrowser.demo.data.model.Node",q="String",r="changeName2",s="changeChildren";qx.Class.define(p,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.setChildren(new qx.data.Array());}
,properties:{children:{check:g,event:s},child:{check:p,event:f,nullable:true},names:{check:g,event:a,init:new qx.data.Array(n,j)},name:{check:q,init:k,event:b},name2:{check:q,event:r},number:{init:10,validate:m},color:{event:h,nullable:true}},members:{__bH:function(t){if(!isNaN(parseFloat(t))){if(/^\d+$/.test(t)){return;}
;}
