...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.TreeController.25340b8df9d7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="open",b="abstract",c="__hd",d="qx.debug",e="'. It is not a child of this tree item.",f="addItem",g="__rr",h="visible",j="excluded",k="removeItem",m="Invalid child index: ",n="Cannot remove treeitem '",o="qx.ui.tree.core.AbstractTreeItem";qx.Class.define(o,{extend:qx.ui.tree.core.AbstractItem,type:b,construct:function(p){qx.ui.tree.core.AbstractItem.call(this,p);this.__hd=[];}
,properties:{parent:{check:o,nullable:true}},members:{__hd:null,__rr:null,getTree:function(){var r=this;while(r.getParent()){r=r.getParent();}
...
;}
;}
,getChildrenContainer:function(){if(!this.__rr){this.__rr=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?h:j});}
;return this.__rr;}
,hasChildrenContainer:function(){return this.__rr;}
...
;}
,__rs:function(){if(this.getParentChildrenContainer()){this.getParentChildrenContainer()._addAfter(this.getChildrenContainer(),this);}
;}
...
;this.__hd.push(Q);if(!N){this.__rs();}
...
;qx.lang.Array.insertAt(this.__hd,X,T);if(!V){this.__rs();}
...
(function(){var a="Enter",b="_applyOpenMode",c="excluded",d="Space",f="Left",g="Boolean",h="changeOpenMode",j="_applyRootOpenClose",k="__rS",l="visible",m="changeSelection",n="qx.ui.tree.Tree",o="qx.event.type.Data",p="qx.ui.tree.core.AbstractTreeItem",q="tree",r="dbltap",s="_applyHideRoot",t="changeRoot",u="_applyRoot",v="keypress",w="none",x="pane",y="tap",z="Right";qx.Class.define(n,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IModelSelection,qx.ui.form.IForm],include:[qx.ui.core.MMultiSelectionHandling,qx.ui.core.MContentPadding,qx.ui.form.MModelSelection,qx.ui.form.MForm],construct:function(){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__rS=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({allowShrinkY:false,allowGrowX:true});this.getChildControl(x).add(this.__rS);this.initOpenMode();this.initRootOpenClose();this.addListener(m,this._onChangeSelection,this);this.addListener(v,this._onKeyPress,this);}
,events:{addItem:o,removeItem:o},properties:{openMode:{check:[y,r,w],init:r,apply:b,event:h,themeable:true},root:{check:p,init:null,nullable:true,event:t,apply:u},hideRoot:{check:g,init:false,apply:s},rootOpenClose:{check:g,init:false,apply:j},appearance:{refine:true,init:q},focusable:{refine:true,init:true}},members:{__rS:null,SELECTION_MANAGER:qx.ui.tree.selection.SelectionManager,getChildrenContainer:function(){return this.__rS;}
...
,_getContentPaddingTarget:function(){return this.__rS;}
...
;if(this.getSelection()==null){this.__tQ=new qx.data.Array();this.setSelection(this.__tQ);}
;}
,properties:{selection:{check:b,event:f,apply:d,init:null}},events:{"changeSelection":h},members:{_modifingSelection:0,__tR:null,__tS:null,__tQ:null,_applySelection:function(k,j){if(this.__tS!=undefined&&j!=undefined){j.removeListenerById(this.__tS);this.__tS=null;}
;if(k){this.__tS=k.addListener(a,this.__tT,this);}
;this._updateSelection();}
,__tT:function(){this._updateSelection();}
,_changeTargetSelection:function(){if(this.getTarget()==null){return;}
;if(!this.__tU()&&!this.__tV()){return;}
;if(this._inSelectionModification()){return;}
;var m=this.getTarget().getSelection();var l=this.getSelection();if(l==null){l=new qx.data.Array();this.__tQ=l;this.setSelection(l);}
...
,_addChangeTargetListener:function(p,o){if(this.__tR!=undefined&&o!=undefined){o.removeListenerById(this.__tR);}
;if(p!=null){if(this.__tU()||this.__tV()){this.__tR=p.addListener(f,this._changeTargetSelection,this);}
;}
;}
,_updateSelection:function(){if(!this.getTarget()||!this.getSelection()){return;}
;this._startSelectionModification();if(this.__tU()){var s=[];for(var i=0;i<this.getSelection().length;i++ ){var t=this.getSelection().getItem(i);var u=this.__tX(t);if(u!=null){s.push(u);}
;}
...
;}
;}
else if(this.__tV()){var r=this.getSelection().getItem(this.getSelection().length-1);if(r!==undefined){this.__tW(r);this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();}
else {this.getTarget().resetSelection();}
;}
;this._endSelectionModification();}
,__tU:function(){var v=this.getTarget().constructor;return qx.Class.implementsInterface(v,qx.ui.core.IMultiSelection);}
,__tV:function(){var w=this.getTarget().constructor;return qx.Class.implementsInterface(w,qx.ui.core.ISingleSelection);}
,__tW:function(x){var y=this.__tX(x);if(y==null){return;}
;if(this.__tU()){this.getTarget().addToSelection(y);}
else if(this.__tV()){this.getTarget().setSelection([y]);}
;}
,__tX:function(z){var A=this.getTarget().getSelectables(true);for(var i=0;i<A.length;i++ ){if(A[i].getModel()==z){return A[i];}
;}
;return null;}
...
},destruct:function(){if(this.__tQ){this.__tQ.dispose();}
;}
});}
)();
...
(function(){var a="changeModel",b="_applyDelegate",c="qx.core.Object",d="_applyLabelPath",f="icon",g="change",h="label",k="' twice.",l="_applyTarget",m="changeTarget",n="_applyIconPath",o="Can not reverse bind the same target property '",p="_applyLabelOptions",q="_applyChildPath",r="_applyIconOptions",s="qx.data.controller.Tree",t="String",u="Can not bind the same target property '",v="_applyModel",w="get";qx.Class.define(s,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(y,z,A,x){qx.core.Object.call(this);this.__bd={};this.__ua=[];this.__uq={};if(A!=null){this.setChildPath(A);}
;if(x!=null){this.setLabelPath(x);}
;if(y!=null){this.setModel(y);}
;if(z!=null){this.setTarget(z);}
;}
,properties:{model:{check:c,apply:v,event:a,nullable:true,dereference:true},target:{apply:l,event:m,init:null,nullable:true,dereference:true},childPath:{check:t,apply:q,nullable:true},labelPath:{check:t,apply:d,nullable:true},iconPath:{check:t,apply:n,nullable:true},labelOptions:{apply:p,nullable:true},iconOptions:{apply:r,nullable:true},delegate:{apply:b,init:null,nullable:true}},members:{__uq:null,__bd:null,__ua:null,__ur:null,_applyDelegate:function(C,B){this._setConfigureItem(C,B);this._setCreateItem(C,B);this._setBindItem(C,B);}
,_applyIconOptions:function(E,D){this.__um();}
,_applyLabelOptions:function(G,F){this.__um();}
,_applyTarget:function(I,H){if(H!=undefined){this.__uw(H);}
;if(this.getModel()!=null){this.__uu();}
;this._addChangeTargetListener(I,H);}
,_applyModel:function(K,J){this.__uu();}
,_applyChildPath:function(M,L){this.__ur=L;this.__uu();this.__ur=null;}
,_applyIconPath:function(O,N){this.__um();}
,_applyLabelPath:function(Q,P){this.__uu();}
,__us:function(R){var T=R.getTarget();var S=this.__uq[T.toHashCode()].treeNode;var U=this.__uq[T.toHashCode()].modelNode;this.__uv(S,U);this._updateSelection();}
,__ut:function(e){var X=e.getData();var bc=e.getOldData();var V=this.__uq[bc.toHashCode()];bc.removeListenerById(V.changeListenerId);delete this.__uq[bc.toHashCode()];V.modelNode.removeListenerById(V.changeChildernListenerId);var bd=V.modelNode;var bb=qx.Class.getPropertyDefinition(V.modelNode.constructor,this.getChildPath());var Y=bb.event;var ba=bd.addListener(Y,this.__ut,this);var W=V.treeNode;this.__uq[X.toHashCode()]={modelNode:bd,treeNode:W,changeListenerId:V.changeListenerId,changeChildernListenerId:ba};this.__uv(W,bd);this._updateSelection();}
...
;return be;}
,__uu:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__uw();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.__uz(this.getModel(),bg);this.__uv(bg,this.getModel());this.getTarget().setRoot(bg);}
;}
,__uv:function(bn,br){if(br[w+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bj=br[w+qx.lang.String.firstUp(this.getChildPath())]();if(this.__uq[bj.toHashCode()]==undefined){var bl=bj.addListener(g,this.__us,this);var bp=qx.Class.getPropertyDefinition(br.constructor,this.getChildPath());var bm=bp.event;var bq=br.addListener(bm,this.__ut,this);this.__uq[bj.toHashCode()]={modelNode:br,treeNode:bn,changeListenerId:bl,changeChildernListenerId:bq};}
...
;}
else {var bo=this._createItem();bo.setModel(bj.getItem(i));bn.addAt(bo,i);this.__uz(bj.getItem(i),bo);this.__uv(bo,bj.getItem(i));}
;}
;}
;for(var i=bn.getChildren().length-1;i>=bj.length;i-- ){var bk=bn.getChildren()[i];this.__uy(bk,bn);}
;}
,__uw:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__ux(bv);var bu=bv.getModel();if(bu){this.__uA(bu);}
;bv.destroy();this.__uq={};}
;}
,__ux:function(bw){var bx=bw.getChildren()||[];for(var i=bx.length-1;i>=0;i-- ){if(bx[i].getChildren().length>0){this.__ux(bx[i]);}
;this.__uy(bx[i],bw);}
;}
,__uy:function(bz,bC){var bD=bz.getModel();var by=this.__ur||this.getChildPath();var bA=w+qx.lang.String.firstUp(by);if(bD[bA]!=undefined){var bE=bD[bA]();var bB=this.__uq[bE.toHashCode()];bE.removeListenerById(bB.changeListenerId);bD.removeListenerById(bB.changeChildernListenerId);this.__ux(bz);delete this.__uq[bE.toHashCode()];}
;this.__uA(bD);bC.remove(bz);bz.destroy();}
...
;if(!qx.lang.Array.contains(this.__ua,bJ)){this.__ua.push(bJ);}
;}
...
;if(!qx.lang.Array.contains(this.__ua,bM)){this.__ua.push(bM);}
;}
...
;}
,__um:function(){var bW;for(var ca in this.__bd){bW=ca;break;}
;for(var bX in this.__bd[bW]){var bV=this.__bd[bW][bX].treeNode;var bY=qx.core.ObjectRegistry.fromHashCode(bX);this.__uA(bY);this.__uz(bY,bV);}
;}
,__uz:function(cd,cb){var cc=this.getDelegate();if(cc!=null&&cc.bindItem!=null){cc.bindItem(this,cb,cd);}
else {this.bindDefaultProperties(cb,cd);}
;}
,__uA:function(cg){for(var i=0;i<this.__ua.length;i++ ){var ce=this.__ua[i];var cf=this.__bd[ce][cg.toHashCode()];if(cf!=null){if(cf.id){cg.removeBinding(cf.id);cf.id=null;}
...
;this._startSelectionModification();this.__uw();this.__uu();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cn,cm){if(cn!=null&&cn.bindItem!=null){if(cm!=null&&cm.bindItem!=null&&cn.bindItem==cm.bindItem){return;}
;this.__uu();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__bd=this.__uq=this.__ua=null;}
});}
)();
...
(function(){var a="changeModel",b="_applyLabelOptions",c="_applyLabelPath",d="changeTarget",e="_applyModel",f="change",g="icon",h="ReverseBindingId",k="qx.data.controller.List",m="_applyIconPath",n="BindingId",o="_applyDelegate",p="",q="changeDelegate",r="_applyTarget",s="]",t="qx.data.IListData",u="model[",v="model",w="label",x="String",y="_applyIconOptions",z=".";qx.Class.define(k,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(A,B,C){qx.core.Object.call(this);this.__tY=[];this.__ua=[];this.__ub=[];this.__uc={};if(C!=null){this.setLabelPath(C);}
;if(A!=null){this.setModel(A);}
;if(B!=null){this.setTarget(B);}
;}
,properties:{model:{check:t,apply:e,event:a,nullable:true,dereference:true},target:{apply:r,event:d,nullable:true,init:null,dereference:true},labelPath:{check:x,apply:c,nullable:true},iconPath:{check:x,apply:m,nullable:true},labelOptions:{apply:b,nullable:true},iconOptions:{apply:y,nullable:true},delegate:{apply:o,event:q,init:null,nullable:true}},members:{__ud:null,__tY:null,__uc:null,__ua:null,__ub:null,__ue:null,__uf:null,update:function(){this.__ui();this.__um();this._updateSelection();}
,_applyDelegate:function(E,D){this._setConfigureItem(E,D);this._setFilter(E,D);this._setCreateItem(E,D);this._setBindItem(E,D);}
,_applyIconOptions:function(G,F){this.__um();}
,_applyLabelOptions:function(I,H){this.__um();}
,_applyIconPath:function(K,J){this.__um();}
,_applyLabelPath:function(M,L){this.__um();}
,_applyModel:function(P,N){if(N!=undefined){if(this.__ud!=undefined){N.removeListenerById(this.__ud);}
;}
;if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();}
;if(P!=null){this.__ud=P.addListener(f,this.__uh,this);this.__uo();this.__ui();if(N==null){this._changeTargetSelection();}
else {this.__ue=true;qx.ui.core.queue.Widget.add(this);}
;}
else {var O=this.getTarget();if(O!=null){var length=O.getChildren().length;for(var i=0;i<length;i++ ){this.__ul();}
;}
;}
;}
...
;if(S!=null){if(this.getModel()!=null){for(var i=0;i<this.__tY.length;i++ ){this.__uk(this.__up(i));}
;}
;}
;}
,__ug:false,__uh:function(){if(this.__ug){return;}
;this.__ug=true;this.__uf=true;qx.ui.core.queue.Widget.add(this);this.update();this.__ug=false;}
,syncWidget:function(){if(this.__ue){this._changeTargetSelection();}
;if(this.__uf){this._updateSelection();}
;this.__uf=this.__ue=null;}
,__ui:function(){if(this.getTarget()==null){return;}
;this.__uo();var U=this.__tY.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++ ){this.__uk(this.__up(j));}
;}
else if(U<T){for(var j=T;j>U;j-- ){this.__ul();}
;}
;this.__uo();this.__ue=true;qx.ui.core.queue.Widget.add(this);}
,__uj:function(){var V=this.getModel();if(V!=null){V.removeListenerById(this.__ud);this.__ud=V.addListener(f,this.__uh,this);}
;}
...
;return W;}
,__uk:function(Y){var ba=this._createItem();this._bindListItem(ba,Y);this.getTarget().add(ba);}
,__ul:function(){this._startSelectionModification();var bc=this.getTarget().getChildren();var bb=bc.length-1;var bd=bc[bb];this._removeBindingsFrom(bd);this.getTarget().removeAt(bb);bd.destroy();this._endSelectionModification();}
...
;}
,bindProperty:function(bo,br,bp,bn,bm){if(bp!=null){var bp=qx.lang.Object.clone(bp);this.__uc[br]=bp.onUpdate;delete bp.onUpdate;}
else {bp={};this.__uc[br]=null;}
;bp.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,bm);bp.ignoreConverter=v;var bq=u+bm+s;if(bo!=null&&bo!=p){bq+=z+bo;}
;var bs=this.bind(bq,bn,br,bp);bn.setUserData(br+n,bs);if(!qx.lang.Array.contains(this.__ua,br)){this.__ua.push(br);}
;}
,bindPropertyReverse:function(by,bv,bw,bx,bt){var bu=u+bt+s;if(by!=null&&by!=p){bu+=z+by;}
;var bz=bx.bind(bv,this,bu,bw);bx.setUserData(by+h,bz);if(!qx.lang.Array.contains(this.__ub,by)){this.__ub.push(by);}
;}
,_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;}
;for(var i=0;i<this.__ua.length;i++ ){if(this.__uc[this.__ua[i]]!=null){this.__uc[this.__ua[i]]();}
;}
;}
,_removeBindingsFrom:function(bD){for(var i=0;i<this.__ua.length;i++ ){var bE=bD.getUserData(this.__ua[i]+n);if(bE!=null){this.removeBinding(bE);}
;}
;for(var i=0;i<this.__ub.length;i++ ){var bE=bD.getUserData(this.__ub[i]+h);if(bE!=null){bD.removeBinding(bE);}
;}
;}
,__um:function(){if(this.getTarget()==null||this.getModel()==null){return;}
;var bF=this.getTarget().getChildren();for(var i=0;i<bF.length;i++ ){this._removeBindingsFrom(bF[i]);this._bindListItem(bF[i],this.__up(i));}
;this.__uj();}
...
,_setFilter:function(bR,bP){if((bR==null||bR.filter==null)&&(bP!=null&&bP.filter!=null)){this.__un();}
...
;var bQ=this.__tY;this.__uo();if(bQ.length>this.__tY.length){for(var j=bQ.length;j>this.__tY.length;j-- ){this.getTarget().removeAt(j-1).destroy();}
;}
else if(bQ.length<this.__tY.length){for(var j=bQ.length;j<this.__tY.length;j++ ){var bT=this._createItem();this.getTarget().add(bT);}
;}
;var bS=this.getTarget().getChildren();for(var i=0;i<bS.length;i++ ){this._bindListItem(bS[i],this.__up(i));}
;this.__uj();this._endSelectionModification();this._updateSelection();}
,__un:function(){this.__uo();this.__ui();this.__um();this.__uf=true;qx.ui.core.queue.Widget.add(this);}
,__uo:function(){var bW=this.getModel();if(bW==null){return;}
;var bX=this.getDelegate();if(bX!=null){var bV=bX.filter;}
;this.__tY=[];for(var i=0;i<bW.getLength();i++ ){if(bV==null||bV(bW.getItem(i))){this.__tY.push(i);}
;}
;}
,__up:function(bY){return this.__tY[bY];}
},destruct:function(){this.__tY=this.__uc=this.__ua=null;this.__ub=null;qx.ui.core.queue.Widget.remove(this);}
});}
)();
