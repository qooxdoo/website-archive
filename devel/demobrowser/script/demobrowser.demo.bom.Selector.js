...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Selector.69b812472ff7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,getWindow:function(N){return qx.dom.Node.getWindow(qx.module.Traversing.__sQ(N));}
...
,getDocument:function(Q){return qx.dom.Node.getDocument(qx.module.Traversing.__sQ(Q));}
,getNodeName:function(R){return qx.dom.Node.getName(qx.module.Traversing.__sQ(R));}
,getNodeText:function(S){return qx.dom.Node.getText(qx.module.Traversing.__sQ(S));}
,isBlockNode:function(T){return qx.dom.Node.isBlockNode(qx.module.Traversing.__sQ(T));}
,equalNodes:function(bd,bf){bd=qx.module.Traversing.__sR(bd);bf=qx.module.Traversing.__sR(bf);if(!bd||!bf){return false;}
...
;}
;if(bc){var U=qx.module.Traversing.__sS(bd);var be=qx.module.Traversing.__sS(bf);for(var W in U){if(U[W]!==be[W]){return false;}
;}
;}
...
;return true;}
;}
},members:{__sP:null,add:function(bg){if(bg instanceof qxWeb){bg=bg[0];}
...
;}
;return bq;}
,getAncestors:function(bt){return this.__sP(null,bt);}
,getAncestorsUntil:function(bv,bu){return this.__sP(bv,bu);}
...
;return bO;}
,getNextAll:function(bR){var bQ=qx.module.Traversing.__sT(this,f,bR);return qxWeb.$init(bQ,qxWeb);}
...
;return bX;}
,getPrevAll:function(cb){var ca=qx.module.Traversing.__sT(this,k,cb);return qxWeb.$init(ca,qxWeb);}
...
,getSiblings:function(ci){var ch=qx.module.Traversing.__sT(this,n,ci);return qxWeb.$init(ch,qxWeb);}
...
},defer:function(cp){qxWeb.$attachAll(this);qxWeb.$attach({"__sP":cp.__sP});}
});}
)();
...
;}
;var Y,bb=[];for(var i=0,l=X.length;i<l;i++ ){Y=X[i];if(typeof Y===k){Y=this.__sW(Y,bc);}
...
;}
;return k;}
,__ta:function(p){if(p.childNodes.length==0){return p;}
;for(var i=0,l=p.childNodes.length;i<l;i++ ){if(p.childNodes[i].nodeType===1){return this.__ta(p.childNodes[i]);}
;}
;return p;}
,__tb:function(r){if(!qx.lang.Type.isArray(r)){var q=qxWeb(r);r=q.length>0?q:[r];}
...
,appendTo:function(parent){parent=qx.module.Manipulating.__tb(parent);for(var i=0,l=parent.length;i<l;i++ ){this._forEachElement(function(C,j){if(i==0){qx.dom.Element.insertEnd(this[j],parent[i]);}
else {qx.dom.Element.insertEnd(this.eq(j).clone(true)[0],parent[i]);}
;}
);}
;return this;}
,insertBefore:function(D){D=qx.module.Manipulating.__tb(D);for(var i=0,l=D.length;i<l;i++ ){this._forEachElement(function(E,F){if(i==0){qx.dom.Element.insertBefore(E,D[i]);}
else {qx.dom.Element.insertBefore(this.eq(F).clone(true)[0],D[i]);}
;}
);}
;return this;}
,insertAfter:function(G){G=qx.module.Manipulating.__tb(G);for(var i=0,l=G.length;i<l;i++ ){for(var j=this.length-1;j>=0;j-- ){if(!this[j]||this[j].nodeType!==1){continue;}
...
;this._forEachElement(function(I){var J=H.eq(0).clone(true);qx.dom.Element.insertAfter(J[0],I);var K=qx.module.Manipulating.__ta(J[0]);qx.dom.Element.insertEnd(I,K);}
);return this;}
