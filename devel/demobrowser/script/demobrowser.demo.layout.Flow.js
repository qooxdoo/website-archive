...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Flow.30f8fe677fdd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var x=new qx.ui.layout.LineSizeIterator(D,this.getSpacingX());var B=z.top;while(x.hasMoreLines()){var C=x.computeNextLine(A);this.__yb(C,B,A,z);B+=C.height+this.getSpacingY();}
;}
,__yb:function(L,K,I,E){var H=qx.ui.layout.Util;var F=E.left;if(this.getAlignX()!=l){F=E.left+I-L.width;if(this.getAlignX()==j){F=E.left+Math.round((I-L.width)/2);}
;}
...
;}
,_computeSizeHint:function(){return this.__yc(Infinity);}
,hasHeightForWidth:function(){return true;}
,getHeightForWidth:function(P){return this.__yc(P).height;}
...
;return S;}
,__yc:function(T){var U=new qx.ui.layout.LineSizeIterator(this._getLayoutChildren(),this.getSpacingX());var V=0;var W=0;var X=0;while(U.hasMoreLines()){var Y=U.computeNextLine(T);X+=1;W=Math.max(W,Y.width);V+=Y.height;}
;return {width:W,height:V+this.getSpacingY()*(X-1)};}
}});}
)();
(function(){var a="No more lines to compute",b="qx.ui.layout.LineSizeIterator";qx.Class.define(b,{extend:Object,construct:function(d,c){this.__hi=d;this.__yd=c;this.__ye=d.length>0;this.__yf=0;}
,members:{__hi:null,__yd:null,__ye:null,__yf:null,computeNextLine:function(k){var k=k||Infinity;if(!this.__ye){throw new Error(a);}
;var p=this.__hi;var n=0;var f=0;var o=[];var j=[];for(var i=this.__yf;i<p.length;i++ ){var m=p[i];var q=m.getSizeHint();var g=this.__yg(i);var h=q.width+g;var e=i==this.__yf;if(!e&&f+h>k){this.__yf=i;break;}
;var l=q.height+m.getMarginTop()+m.getMarginBottom();o.push(m);j.push(g);f+=h;n=Math.max(n,l);if(m.getLayoutProperties().lineBreak){this.__yf=i+1;break;}
;}
;if(i>=p.length){this.__ye=false;}
;return {height:n,width:f,children:o,gapsBefore:j};}
,__yg:function(r){var s=r==this.__yf;if(s){return this.__hi[r].getMarginLeft();}
else {return Math.max(this.__hi[r-1].getMarginRight(),this.__hi[r].getMarginLeft(),this.__yd);}
;}
,hasMoreLines:function(){return this.__ye;}
}});}
)();
