...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Flow.4c0f521976a9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var x=new qx.ui.layout.LineSizeIterator(D,this.getSpacingX());var B=z.top;while(x.hasMoreLines()){var C=x.computeNextLine(A);this.__ye(C,B,A,z);B+=C.height+this.getSpacingY();}
;}
,__ye:function(L,K,I,E){var H=qx.ui.layout.Util;var F=E.left;if(this.getAlignX()!=l){F=E.left+I-L.width;if(this.getAlignX()==j){F=E.left+Math.round((I-L.width)/2);}
;}
...
;}
,_computeSizeHint:function(){return this.__yf(Infinity);}
,hasHeightForWidth:function(){return true;}
,getHeightForWidth:function(P){return this.__yf(P).height;}
...
;return S;}
,__yf:function(T){var U=new qx.ui.layout.LineSizeIterator(this._getLayoutChildren(),this.getSpacingX());var V=0;var W=0;var X=0;while(U.hasMoreLines()){var Y=U.computeNextLine(T);X+=1;W=Math.max(W,Y.width);V+=Y.height;}
;return {width:W,height:V+this.getSpacingY()*(X-1)};}
}});}
)();
(function(){var a="No more lines to compute",b="qx.ui.layout.LineSizeIterator";qx.Class.define(b,{extend:Object,construct:function(d,c){this.__gK=d;this.__yg=c;this.__yh=d.length>0;this.__yi=0;}
,members:{__gK:null,__yg:null,__yh:null,__yi:null,computeNextLine:function(k){var k=k||Infinity;if(!this.__yh){throw new Error(a);}
;var p=this.__gK;var n=0;var f=0;var o=[];var j=[];for(var i=this.__yi;i<p.length;i++ ){var m=p[i];var q=m.getSizeHint();var g=this.__yj(i);var h=q.width+g;var e=i==this.__yi;if(!e&&f+h>k){this.__yi=i;break;}
;var l=q.height+m.getMarginTop()+m.getMarginBottom();o.push(m);j.push(g);f+=h;n=Math.max(n,l);if(m.getLayoutProperties().lineBreak){this.__yi=i+1;break;}
;}
;if(i>=p.length){this.__yh=false;}
;return {height:n,width:f,children:o,gapsBefore:j};}
,__yj:function(r){var s=r==this.__yi;if(s){return this.__gK[r].getMarginLeft();}
else {return Math.max(this.__gK[r-1].getMarginRight(),this.__gK[r].getMarginLeft(),this.__yg);}
;}
,hasMoreLines:function(){return this.__yh;}
}});}
)();
