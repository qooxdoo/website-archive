...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.FormController.39be50cb71da.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="The given item has not been added.",b="qx.ui.form.Resetter",c=" not supported for reseting.",d="Item ";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__qQ=[];}
,members:{__qQ:null,add:function(e){if(this._supportsValue(e)){var f=e.getValue();}
else if(this.__vv(e)){var f=e.getSelection();}
else if(this.__vw(e)){var f=e.getSelection().concat();}
else {throw new Error(d+e+c);}
;this.__qQ.push({item:e,init:f});}
,remove:function(g){for(var i=0;i<this.__qQ.length;i++ ){var h=this.__qQ[i];if(h.item===g){this.__qQ.splice(i,1);return true;}
;}
;return false;}
,reset:function(){for(var i=0;i<this.__qQ.length;i++ ){var j=this.__qQ[i];this.__vD(j.item,j.init);}
;}
,resetItem:function(k){var l;for(var i=0;i<this.__qQ.length;i++ ){var m=this.__qQ[i];if(m.item===k){l=m.init;break;}
;}
...
;}
,redefine:function(){for(var i=0;i<this.__qQ.length;i++ ){var p=this.__qQ[i].item;this.__qQ[i].init=this.__vE(p);}
;}
,redefineItem:function(q){var r;for(var i=0;i<this.__qQ.length;i++ ){if(this.__qQ[i].item===q){r=this.__qQ[i];break;}
;}
...
,ASSUME_YEAR_2000_THRESHOLD:30,ISO_MASKS:{isoDate:eT,isoTime:V,isoDateTime:T,isoUtcDateTime:dQ},AM_MARKER:ek,PM_MARKER:N},members:{__vR:null,__lI:null,__vS:null,__vT:null,__vV:null,__vW:null,__vX:null,__vU:null,__vY:function(gt,gs){var gu=dL+(gt<0?((-1)*gt):gt);while(gu.length<gs){gu=eY+gu;}
;return gt<0?a+gu:gu;}
...
,_applyLocale:function(gW,gV){this.__lI=gW===null?this.setLocale(this.__vS):gW;}
...
;var iu=this.__vW=[];var ip=qx.locale.Date.getAmMarker(this.__lI).toString()||it.AM_MARKER;var iD=qx.locale.Date.getPmMarker(this.__lI).toString()||it.PM_MARKER;var ig=this.__lI;var il=function(iG,iH){iH=parseInt(iH,10);if(iH>=0){if(iH<it.ASSUME_YEAR_2000_THRESHOLD){iH+=2000;}
...
;var A=z[h+qx.lang.String.firstUp(name)]();if(qx.lang.Type.isArray(A)){var D=qx.data&&qx.data.IListData&&qx.Class.hasInterface(A&&A.constructor,qx.data.IListData);for(var i=0;i<A.length;i++ ){var F=D?A.getItem(i):A[i];E+=this.__sm(name,F,C);}
;}
else if(qx.lang.Type.isDate(A)&&y!=null){E+=this.__sm(name,y.format(A),C);}
else {E+=this.__sm(name,A,C);}
;}
;return E.substring(0,E.length-1);}
,__sm:function(name,I,G){if(I&&I.$$type==o){I=I.classname;}
