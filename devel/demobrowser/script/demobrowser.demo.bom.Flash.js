...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Flash.52cb755a175d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
;}
;var N=this.__uf(O,Q,P,L);this._flashObjects[Q.id]=N;return N;}
,destroy:function(S,R){if(qx.core.Environment.get(A)==u&&qx.core.Environment.get(p)<11){S=this.__ua(S);if(S.readyState==4){this.__ub(S);}
else {if(!R){R=window;}
;qx.bom.Event.addNativeListener(R,o,function(){qx.bom.Flash.__ub(S);}
);}
;}
else {S=this.__ua(S);if(S.parentNode){S.parentNode.removeChild(S);}
;delete this._flashObjects[S.id];}
;}
,__ua:function(T){if(!T){throw new Error(w);}
;if(T.tagName.toLowerCase()!==K){T=T.firstChild;}
;if(!T||T.tagName.toLowerCase()!==K){throw new Error(x);}
;return T;}
,__ub:qx.core.Environment.select(A,{"mshtml":qx.event.GlobalError.observeMethod(function(U){for(var i in U){if(typeof U[i]==b){U[i]=null;}
;}
;if(U.parentNode){U.parentNode.removeChild(U);}
;delete this._flashObjects[U.id];}
),"default":null}),__uc:qx.event.GlobalError.observeMethod(function(){for(var V in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[V]);}
;window.__ud=function(){}
;window.__ue=function(){}
;qx.bom.Event.removeNativeListener(window,D,qx.bom.Flash.__uc);}
),__uf:function(ba,bc,bb,W){if(qx.core.Environment.get(A)==u&&qx.core.Environment.get(p)<11){bb.movie=bc.data;delete bc.data;delete bc.classid;var X=t;for(var name in bb){X+=l+name+H+bb[name]+C;}
...
},defer:function(be){if(qx.core.Environment.get(A)==u){qx.bom.Event.addNativeListener(window,D,be.__uc);}
;}
});}
)();
