...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.ToolBar.c973e72730a6.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var P=K.getMarginLeft()+K.getMarginRight();P=Math.max(P,this.getSpacing());var J=K.getSizeHint().width+P;this.__oc(K);G-=J;if(E&&E.getVisibility()!=o){E.setVisibility(o);G+=L;var H=E.getMarginLeft()+E.getMarginRight();G+=Math.max(H,this.getSpacing());}
;}
while(G>F);}
else if(this.__nY.length>0){do {var Q=this.__nY[0];if(Q){var P=Q.getMarginLeft()+Q.getMarginRight();P=Math.max(P,this.getSpacing());if(Q.getContentElement().getDomElement()==null){Q.syncAppearance();Q.invalidateLayoutCache();}
;var I=Q.getSizeHint().width;var O=false;if(this.__nY.length==1&&L>0){var N=P-this.getSpacing();var M=G-L+I+N;O=F>M;}
;if(F>G+I+P||O){this.__ob(Q);G+=I;if(E&&this.__nY.length==0){E.setVisibility(y);}
;}
else {return;}
;}
;}
while(F>=G&&this.__nY.length>0);}
;}
,__ob:function(R){R.setVisibility(o);this.__nY.shift();this.fireDataEvent(m,R);}
,__oc:function(S){if(!S){return;}
;this.__nY.unshift(S);S.setVisibility(y);this.fireDataEvent(A,S);}
,_getNextToHide:function(){for(var i=this.__oa.length-1;i>=0;i-- ){var T=this.__oa[i];if(T&&T.getVisibility&&T.getVisibility()==o){return T;}
;}
...
;}
;}
,setRemovePriority:function(X,W,Y){if(!Y&&this.__oa[W]!=undefined){throw new Error(v);}
;this.__oa[W]=X;}
...
;for(var i=0;i<this.__nY.length;i++ ){this.__nY[i].setVisibility(o);}
;this.__nY=[];}
;}
...
;bf.setVisibility(y);}
;}
,__od:false,_setAllowMenuOpenHover:function(bg){this.__od=bg;}
...
(function(){var a="blur",b="Enter",c="Escape",d="Space",f="keydown",g="Left",h="Down",j="Up",k="singleton",l="qx.debug",m="interval",n="roll",o="event.touch",p="__oj",q="Object is no menu: ",r="pointerdown",s="qx.ui.menu.Manager",t="keyup",u="keypress",v="contextmenu",w="__oi",x="Right",y="__lN";qx.Class.define(s,{type:k,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__lN=[];var z=document.body;var A=qx.event.Registration;A.addListener(window.document.documentElement,r,this._onPointerDown,this,true);A.addListener(z,n,this._onRoll,this,true);A.addListener(z,f,this._onKeyUpDown,this,true);A.addListener(z,t,this._onKeyUpDown,this,true);A.addListener(z,u,this._onKeyPress,this,true);if(!qx.core.Environment.get(o)){qx.bom.Element.addListener(window,a,this.hideAll,this);}
;this.__oi=new qx.event.Timer();this.__oi.addListener(m,this._onOpenInterval,this);this.__oj=new qx.event.Timer();this.__oj.addListener(m,this._onCloseInterval,this);}
,members:{__ok:null,__ol:null,__oi:null,__oj:null,__lN:null,_getChild:function(B,D,C,G){var F=B.getChildren();var length=F.length;var E;for(var i=D;i<length&&i>=0;i+=C){E=F[i];if(E.isEnabled()&&!E.isAnonymous()&&E.isVisible()){return E;}
;}
...
;H=H.getLayoutParent();}
;return false;}
,_isMenuOpener:function(J){var I=this.__lN;for(var i=0;i<I.length;i++ ){if(I[i].getOpener()===J){return true;}
;}
;return false;}
...
;}
;var M=this.__lN;M.push(L);L.setZIndex(1e6+M.length);}
,remove:function(N){if(qx.core.Environment.get(l)){if(!(N instanceof qx.ui.menu.Menu)){throw new Error(q+N);}
;}
;var O=this.__lN;if(O){qx.lang.Array.remove(O,N);}
;}
,hideAll:function(){var P=this.__lN;if(P){for(var i=P.length-1;i>=0;i-- ){P[i].exclude();}
;}
;}
,getActiveMenu:function(){var Q=this.__lN;return Q.length>0?Q[Q.length-1]:null;}
,scheduleOpen:function(R){this.cancelClose(R);if(R.isVisible()){if(this.__ok){this.cancelOpen(this.__ok);}
;}
else if(this.__ok!=R){this.__ok=R;this.__oi.restartWith(R.getOpenInterval());}
;}
,scheduleClose:function(S){this.cancelOpen(S);if(!S.isVisible()){if(this.__ol){this.cancelClose(this.__ol);}
;}
else if(this.__ol!=S){this.__ol=S;this.__oj.restartWith(S.getCloseInterval());}
;}
,cancelOpen:function(T){if(this.__ok==T){this.__oi.stop();this.__ok=null;}
;}
,cancelClose:function(U){if(this.__ol==U){this.__oj.stop();this.__ol=null;}
;}
,_onOpenInterval:function(e){this.__oi.stop();this.__ok.open();this.__ok=null;}
,_onCloseInterval:function(e){this.__oj.stop();this.__ol.exclude();this.__ol=null;}
,preventContextMenuOnce:function(){qx.event.Registration.addListener(document.body,v,this.__om,this,true);}
,__om:function(e){var V=e.getTarget();V=qx.ui.core.Widget.getWidgetByElement(V,true);if(this._isInMenu(V)){e.stopPropagation();e.preventDefault();}
;qx.event.Registration.removeListener(document.body,v,this.__om,this,true);}
...
;if(this.__lN.length>0&&!this._isInMenu(W)){this.hideAll();}
;}
,__on:{"Enter":1,"Space":1},__oo:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var X=this.getActiveMenu();if(!X){return;}
;var Y=e.getKeyIdentifier();if(this.__oo[Y]||(this.__on[Y]&&X.getSelectedButton())){e.stopPropagation();}
;}
,_onKeyPress:function(e){var bc=this.getActiveMenu();if(!bc){return;}
;var ba=e.getKeyIdentifier();var be=this.__oo[ba];var bb=this.__on[ba];if(be){switch(ba){case j:this._onKeyPressUp(bc);break;case h:this._onKeyPressDown(bc);break;case g:this._onKeyPressLeft(bc);break;case x:this._onKeyPressRight(bc);break;case c:this.hideAll();break;};e.stopPropagation();e.preventDefault();}
...
;this.__os();}
,__os:function(){if(this.isVisible()){if(this.getBlockBackground()){var V=this.getZIndex();this._blocker.blockContent(V-1);}
;}
...
,__ot:function(){var bn=this._getMenuBounds();if(!bn){this.addListenerOnce(n,this.__ot,this);return;}
...
;this.__or=bp;qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(bq){this.getChildControl(h);if(this.__or){this.__or.call(this);delete this.__or;}
;}
...
;this.__ot();}
;}
,_onPointerOver:function(e){var bt=qx.ui.menu.Manager.getInstance();bt.cancelClose(this);var bu=e.getTarget();if(bu.isEnabled()&&bu instanceof qx.ui.menu.AbstractButton){this.setSelectedButton(bu);var bs=bu.getMenu&&bu.getMenu();if(bs){bs.setOpener(bu);bt.scheduleOpen(bs);this.__oq=bs;}
else {var bv=this.getOpenedButton();if(bv){bt.scheduleClose(bv.getMenu());}
;if(this.__oq){bt.cancelOpen(this.__oq);this.__oq=null;}
;}
;}
...
;if(this.__oq){bw.cancelOpen(this.__oq);}
;}
;}
...
(function(){var a="qx.ui.menu.Layout",b="Integer",c="_applyLayoutChange";qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:b,init:0,apply:c},spanColumn:{check:b,init:1,nullable:true,apply:c},iconColumnWidth:{check:b,init:0,themeable:true,apply:c},arrowColumnWidth:{check:b,init:0,themeable:true,apply:c}},members:{__ou:null,_computeSizeHint:function(){var q=this._getLayoutChildren();var o,g,k;var e=this.getSpanColumn();var j=this.__ou=[0,0,0,0];var n=this.getColumnSpacing();var m=0;var f=0;for(var i=0,l=q.length;i<l;i++ ){o=q[i];if(o.isAnonymous()){continue;}
...
,getColumnSizes:function(){return this.__ou||null;}
...
;var x=this.__ov(y[0]);var w=x.getColumnSizes();var s=x.getSpacingX();var z=qx.lang.Array.sum(w)+s*(w.length-1);if(z<o){w[1]+=o-z;}
...
;}
;}
,__ov:function(B){while(!(B instanceof qx.ui.menu.Menu)){B=B.getLayoutParent();}
;return B;}
...
(function(){var a="hovered",b="hover-button",c="pointerover",d="interval",f="__dy",g="qx.ui.form.HoverButton",h="Integer",i="pointerout";qx.Class.define(g,{extend:qx.ui.basic.Atom,include:[qx.ui.core.MExecutable],implement:[qx.ui.form.IExecutable],construct:function(j,k){qx.ui.basic.Atom.call(this,j,k);this.addListener(c,this._onPointerOver,this);this.addListener(i,this._onPointerOut,this);this.__dy=new qx.event.AcceleratingTimer();this.__dy.addListener(d,this._onInterval,this);}
,properties:{appearance:{refine:true,init:b},interval:{check:h,init:80},firstInterval:{check:h,init:200},minTimer:{check:h,init:20},timerDecrease:{check:h,init:2}},members:{__dy:null,_onPointerOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;}
;this.__dy.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();this.addState(a);}
,_onPointerOut:function(e){this.__dy.stop();this.removeState(a);if(!this.isEnabled()||e.getTarget()!==this){return;}
;}
...
,__oB:function(){var y=this.getChildrenContainer().getChildren();y=y.filter(function(z){return z.getVisibility()==k;}
