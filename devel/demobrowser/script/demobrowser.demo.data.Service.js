...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Service.7767f55a67b8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="url: ",b="qx.debug.io",c="qx.bom.request.Script",d="Invalid state",e="head",f="error",g="loadend",h="qx.debug",i="script",j="load",k="Unknown response headers",l="browser.documentmode",m="abort",n="",o="Received native readyState: loaded",p="readystatechange",q="Response header cannot be determined for ",r="requests made with script transport.",s="opera",t="unknown",u="Open native request with ",v="Response headers cannot be determined for",w="mshtml",x="engine.name",y="Detected error",z="Send native request",A="on",B="timeout",C="Unknown environment key at this phase",D="Received native load";qx.Bootstrap.define(c,{construct:function(){this.__nD();this.__nu=qx.Bootstrap.bind(this._onNativeLoad,this);this.__nv=qx.Bootstrap.bind(this._onNativeError,this);this.__nw=qx.Bootstrap.bind(this._onTimeout,this);this.__nx=document.head||document.getElementsByTagName(e)[0]||document.documentElement;this._emitter=new qx.event.Emitter();this.timeout=this.__nF()?0:15000;}
,events:{"readystatechange":c,"error":c,"loadend":c,"timeout":c,"abort":c,"load":c},members:{readyState:null,status:null,statusText:null,timeout:null,__ny:null,on:function(name,E,F){this._emitter.on(name,E,F);return this;}
,open:function(H,G){if(this.__nB){return;}
;this.__nD();this.__nz=null;this.__nA=G;if(this.__nI(b)){qx.Bootstrap.debug(qx.bom.request.Script,u+a+G);}
;this._readyStateChange(1);}
,setRequestHeader:function(I,J){if(this.__nB){return null;}
;var K={};if(this.readyState!==1){throw new Error(d);}
;K[I]=J;this.__nA=qx.util.Uri.appendParamsToUrl(this.__nA,K);return this;}
,send:function(){if(this.__nB){return null;}
;var M=this.__nG(),L=this.__nx,N=this;if(this.timeout>0){this.__dd=window.setTimeout(this.__nw,this.timeout);}
;if(this.__nI(b)){qx.Bootstrap.debug(qx.bom.request.Script,z);}
;L.insertBefore(M,L.firstChild);window.setTimeout(function(){N._readyStateChange(2);N._readyStateChange(3);}
);return this;}
,abort:function(){if(this.__nB){return null;}
;this.__nz=true;this.__nH();this._emit(m);return this;}
...
,onabort:function(){}
,ontimeout:function(){}
,getResponseHeader:function(O){if(this.__nB){return null;}
;if(this.__nI(h)){qx.Bootstrap.debug(q+r);}
;return t;}
,getAllResponseHeaders:function(){if(this.__nB){return null;}
;if(this.__nI(h)){qx.Bootstrap.debug(v+r);}
;return k;}
,setDetermineSuccess:function(P){this.__ny=P;}
,dispose:function(){var Q=this.__nC;if(!this.__nB){if(Q){Q.onload=Q.onreadystatechange=null;this.__nH();}
;if(this.__dd){window.clearTimeout(this.__dd);}
;this.__nB=true;}
;}
,isDisposed:function(){return !!this.__nB;}
,_getUrl:function(){return this.__nA;}
,_getScriptElement:function(){return this.__nC;}
,_onTimeout:function(){this.__nE();if(!this.__nF()){this._emit(f);}
;this._emit(B);if(!this.__nF()){this._emit(g);}
;}
,_onNativeLoad:function(){var S=this.__nC,R=this.__ny,T=this;if(this.__nz){return;}
;if(this.__nI(x)===w&&this.__nI(l)<9){if(!(/loaded|complete/).test(S.readyState)){return;}
else {if(this.__nI(b)){qx.Bootstrap.debug(qx.bom.request.Script,o);}
;}
;}
;if(this.__nI(b)){qx.Bootstrap.debug(qx.bom.request.Script,D);}
;if(R){if(!this.status){this.status=R()?200:500;}
;}
;if(this.status===500){if(this.__nI(b)){qx.Bootstrap.debug(qx.bom.request.Script,y);}
;}
;if(this.__dd){window.clearTimeout(this.__dd);}
;window.setTimeout(function(){T._success();T._readyStateChange(4);T._emit(j);T._emit(g);}
);}
,_onNativeError:function(){this.__nE();this._emit(f);this._emit(g);}
,__nC:null,__nx:null,__nA:n,__nu:null,__nv:null,__nw:null,__dd:null,__nz:null,__nB:null,__nD:function(){this.readyState=0;this.status=0;this.statusText=n;}
,_readyStateChange:function(U){this.readyState=U;this._emit(p);}
,_success:function(){this.__nH();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=n+this.status;}
,__nE:function(){this.__nH();this.readyState=4;this.status=0;this.statusText=null;}
,__nF:function(){var W=this.__nI(x)===w&&this.__nI(l)<9;var V=this.__nI(x)===s;return !(W||V);}
,__nG:function(){var X=this.__nC=document.createElement(i);X.src=this.__nA;X.onerror=this.__nv;X.onload=this.__nu;if(this.__nI(x)===w&&this.__nI(l)<9){X.onreadystatechange=this.__nu;}
;return X;}
,__nH:function(){var Y=this.__nC;if(Y&&Y.parentNode){this.__nx.removeChild(Y);}
;}
,__nI:function(ba){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(ba);}
...
(function(){var a="qx.bom.request.Jsonp",b="callback",c="open",d="dispose",e="",f="qx",g="qx.debug",h="Expecting JavaScript response to call: ",i="_onNativeLoad",j=" already exists",k="Callback ",l="qx.debug.io",m=".callback",n="qx.bom.request.Jsonp.";qx.Bootstrap.define(a,{extend:qx.bom.request.Script,construct:function(){qx.bom.request.Script.apply(this);this.__AH();}
,members:{responseJson:null,__cr:null,__AA:null,__AB:null,__AC:null,__AD:null,__AE:null,__nB:null,__AF:e,open:function(t,p){if(this.__nB){return;}
;var r={},q,s,o=this;this.responseJson=null;this.__AC=false;q=this.__AA||b;s=this.__AB||this.__AF+n+this.__cr+m;if(!this.__AB){this.constructor[this.__cr]=this;}
else {if(!window[this.__AB]){this.__AD=true;window[this.__AB]=function(u){o.callback(u);}
;}
else {if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,k+this.__AB+j);}
;}
;}
;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,h+s);}
;r[q]=s;this.__AE=p=qx.util.Uri.appendParamsToUrl(p,r);this.__hu(c,[t,p]);}
,callback:function(v){if(this.__nB){return;}
;this.__AC=true;if(qx.core.Environment.get(g)){v=qx.lang.Json.stringify(v);v=qx.lang.Json.parse(v);}
;this.responseJson=v;this.constructor[this.__cr]=undefined;this.__AG();}
,setCallbackParam:function(w){this.__AA=w;return this;}
,setCallbackName:function(name){this.__AB=name;return this;}
,setPrefix:function(x){this.__AF=x;}
,getGeneratedUrl:function(){return this.__AE;}
,dispose:function(){this.__AG();this.__hu(d);}
,_onNativeLoad:function(){this.status=this.__AC?200:500;this.__hu(i);}
,__AG:function(){if(this.__AD&&window[this.__AB]){window[this.__AB]=undefined;this.__AD=false;}
;}
,__hu:function(z,y){qx.bom.request.Script.prototype[z].apply(this,y||[]);}
,__AH:function(){this.__cr=f+(new Date().valueOf())+(e+Math.random()).substring(2,5);}
}});}
)();
