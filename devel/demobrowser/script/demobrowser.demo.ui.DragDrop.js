...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.DragDrop.17ff9e194de6.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a=" array contains ",b="__dl",c="qx.ui.core.MSingleSelectionHandling",d=" items!",f="changeSelection",g="changeSelected",h="Could only select one item, but the selection",i="qx.event.type.Data";qx.Mixin.define(c,{events:{"changeSelection":i},members:{__dl:null,getSelection:function(){var j=this.__qa().getSelected();if(j){return [j];}
else {return [];}
;}
,setSelection:function(k){switch(k.length){case 0:this.resetSelection();break;case 1:this.__qa().setSelected(k[0]);break;default:throw new Error(h+a+k.length+d);};}
,resetSelection:function(){this.__qa().resetSelected();}
,isSelected:function(l){return this.__qa().isSelected(l);}
,isSelectionEmpty:function(){return this.__qa().isSelectionEmpty();}
,getSelectables:function(m){return this.__qa().getSelectables(m);}
,_onChangeSelected:function(e){var o=e.getData();var n=e.getOldData();o==null?o=[]:o=[o];n==null?n=[]:n=[n];this.fireDataEvent(f,o,n);}
,__qa:function(){if(this.__dl==null){var p=this;this.__dl=new qx.ui.core.SingleSelectionManager({getItems:function(){return p._getItems();}
...
(function(){var a=", because it is not a child element!",b="Boolean",c="__qc",d="qx.ui.core.SingleSelectionManager",e="qx.debug",f="__qb",g="__qd",h="Invalid selectionProvider!",j="Could not check if ",k=" is selected,",l="Could not select ",m="changeSelected",n=" because it is not a child element!",o="qx.event.type.Data";qx.Class.define(d,{extend:qx.core.Object,construct:function(p){qx.core.Object.call(this);if(qx.core.Environment.get(e)){qx.core.Assert.assertInterface(p,qx.ui.core.ISingleSelectionProvider,h);}
;this.__qb=p;}
,events:{"changeSelected":o},properties:{allowEmptySelection:{check:b,init:true,apply:g}},members:{__qc:null,__qb:null,getSelected:function(){return this.__qc;}
,setSelected:function(q){if(!this.__qf(q)){throw new Error(l+q+a);}
;this.__qe(q);}
,resetSelected:function(){this.__qe(null);}
,isSelected:function(r){if(!this.__qf(r)){throw new Error(j+r+k+n);}
;return this.__qc===r;}
,isSelectionEmpty:function(){return this.__qc==null;}
,getSelectables:function(t){var s=this.__qb.getItems();var u=[];for(var i=0;i<s.length;i++ ){if(this.__qb.isItemSelectable(s[i])){u.push(s[i]);}
;}
;if(!t){for(var i=u.length-1;i>=0;i-- ){if(!u[i].getEnabled()){u.splice(i,1);}
;}
;}
;return u;}
,__qd:function(w,v){if(!w){this.__qe(this.__qc);}
;}
,__qe:function(x){var A=this.__qc;var y=x;if(y!=null&&A===y){return;}
;if(!this.isAllowEmptySelection()&&y==null){var z=this.getSelectables(true)[0];if(z){y=z;}
;}
;this.__qc=y;this.fireDataEvent(m,y,A);}
,__qf:function(B){var C=this.__qb.getItems();for(var i=0;i<C.length;i++ ){if(C[i]===B){return true;}
;}
;return false;}
},destruct:function(){if(this.__qb.toHashCode){this._disposeObjects(f);}
else {this.__qb=null;}
...
,__qg:function(e){var Q=e.getData()[0];var P=e.getOldData()[0];if(P){P.setValue(false);}
...
,_applyFont:function(cV,cU){if(cU&&this.__lY&&this.__mc){this.__lY.removeListenerById(this.__mc);this.__mc=null;}
;var cT;if(cV){this.__lY=qx.theme.manager.Font.getInstance().resolve(cV);if(this.__lY instanceof qx.bom.webfonts.WebFont){this.__mc=this.__lY.addListener(C,this._onWebFontStatusChange,this);}
...
,_onHtmlInput:function(e){var dd=e.getData();var dc=true;this.__pT=false;if(this.__jt&&this.__jt===dd){dc=false;}
...
;}
;if(dc){this.fireDataEvent(r,dd,this.__jt);this.__jt=dd;if(this.getLiveUpdate()){this.__pV(dd);}
;}
;}
...
;}
,__pV:function(dj){var di=this.__js;this.__js=dj;if(di!=dj){this.fireNonBubblingEvent(c,qx.event.type.Data,[dj,di]);}
;}
,setValue:function(dn){if(this.isDisposed()){return null;}
;if(dn===null){if(this.__pT){return dn;}
;dn=cm;this.__pT=true;}
else {this.__pT=false;if(this.__pS){this._removePlaceholder();}
;}
;if(qx.lang.Type.isString(dn)){var dm=this.getContentElement();if(dm.getValue()!=dn){var dk=dm.getValue();dm.setValue(dn);var dl=this.__pT?null:dn;this.__js=dk;this.__pV(dl);this.__jt=this.__js;}
;if(this.__pS){this._showPlaceholder();}
;return dn;}
;throw new Error(J+dn);}
,getValue:function(){return (this.isDisposed()||this.__pT)?null:this.getContentElement().getValue();}
...
;if(!this.__pS&&qx.ui.form.AbstractField.__dN){qx.bom.Stylesheet.removeSheet(qx.ui.form.AbstractField.__dN);qx.ui.form.AbstractField.__dN=null;qx.ui.form.AbstractField.__pR();}
;}
,_applyPlaceholder:function(dw,dv){if(this.__pS){this._getPlaceholderElement().setValue(dw);if(dw!=null){this.addListener(V,this._removePlaceholder,this);this.addListener(ct,this._showPlaceholder,this);this._showPlaceholder();}
...
,properties:{wrap:{check:b,init:true,apply:f},appearance:{refine:true,init:j},singleStep:{check:i,init:20},minimalLineHeight:{check:i,apply:h,init:4},autoSize:{check:b,apply:w,init:false}},members:{__qE:null,__qF:null,__qG:null,setValue:function(M){M=qx.ui.form.AbstractField.prototype.setValue.call(this,M);this.__qH();return M;}
...
;}
,__qH:function(){if(this.isAutoSize()){var R=this.__qI();if(R&&this.getBounds()){this.__qG=this.__qG||this._getAreaHeight();var T=this._getScrolledAreaHeight();if(this.getMaxHeight()){var P=this.getInsets();var Q=-P.top+this.getMaxHeight()-P.bottom;if(T>Q){this.getContentElement().setStyle(I,k);}
else {this.getContentElement().setStyle(I,H);}
;}
;var S=Math.max(T,this.__qG);this._setAreaHeight(S);}
else {this.getContentElement().addListenerOnce(x,function(){this.__qH();}
,this);}
;}
;}
,_getAreaHeight:function(){return this.getInnerSize().height;}
,_setAreaHeight:function(U){if(this._getAreaHeight()!==U){this.__qF=U;qx.ui.core.queue.Layout.add(this);qx.ui.core.queue.Manager.flush();this.__qL();}
;}
,_getScrolledAreaHeight:function(){var W=this.__qI();var V=W.getDomElement();if(V){if(!V.parentNode){qx.html.Element.flush();return this._getScrolledAreaHeight();}
...
;this.__qK(W);if(qx.core.Environment.get(C)==B&&qx.core.Environment.get(s)==8){if(!V.scrollTop){qx.html.Element.flush();}
;}
;return V.scrollTop;}
;}
,__qI:function(){this.__qE=this.__qE||this.__qJ();return this.__qE;}
,__qJ:function(){var ba,Y,bb,X;ba=this.getContentElement();if(!ba.getDomElement()){return null;}
;bb=qx.bom.Element.clone(ba.getDomElement());X=new qx.html.Input(j);X.useElement(bb);Y=X;Y.setStyles({position:K,top:0,left:o,height:0,overflow:H},true);Y.removeAttribute(G);Y.removeAttribute(z);Y.setAttribute(y,g);Y.setValue(ba.getValue()||t);Y.insertBefore(ba);this.__qK(Y);return Y;}
,__qK:function(bc){bc=bc.getDomElement();if(bc){bc.scrollTop=10000;}
;}
...
,_applyAutoSize:function(bh,bg){if(qx.core.Environment.get(m)){this.__qM();}
;if(bh){this.__qH();this.addListener(a,this.__qH,this);if(!this.getBounds()){this.addListenerOnce(x,function(){this.getContentElement().setStyle(I,H);}
);}
else {this.getContentElement().setStyle(I,H);}
;}
else {this.removeListener(a,this.__qH);this.getContentElement().setStyle(I,k);}
;}
,_applyDimension:function(bi){qx.ui.form.AbstractField.prototype._applyDimension.call(this);if(qx.core.Environment.get(m)){this.__qM();}
;if(bi===this.getMaxHeight()){this.__qH();}
;}
,__qL:function(){var content=this.getContentElement();var bk=content.getDomElement();var bj=content.getStyle(u);content.setStyle(u,parseInt(bj,10)+1000+c,true);if(bk){qx.bom.element.Dimension.getWidth(bk);}
;content.setStyle(u,bj,true);}
,__qM:function(){if(this.isAutoSize()&&this.getHeight()){this.warn(n+J+v);}
;}
,_getContentHint:function(){var bl=qx.ui.form.AbstractField.prototype._getContentHint.call(this);bl.height=bl.height*this.getMinimalLineHeight();bl.width=this._getTextSize().width*20;if(this.isAutoSize()){bl.height=this.__qF||bl.height;}
;return bl;}
},destruct:function(){this.setAutoSize(false);if(this.__qE){this.__qE.dispose();}
;}
});}
)();
