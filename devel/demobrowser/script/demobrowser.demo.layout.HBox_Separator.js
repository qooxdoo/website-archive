...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.HBox_Separator.02975a160381.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,properties:{sort:{check:[m,o,A],init:m,apply:r},separatorX:{check:p,nullable:true,apply:h},separatorY:{check:p,nullable:true,apply:h},connectSeparators:{check:a,init:false,apply:h},spacingX:{check:f,init:0,apply:h},spacingY:{check:f,init:0,apply:h}},members:{__gK:null,__wM:null,verifyLayoutProperty:qx.core.Environment.select(j,{"true":function(F,name,G){this.assertInArray(name,[y,w,c,k],g+name+l);if(name===w){this.assertInArray(G,[s,x,d,v,n]);}
...
,__wN:{north:1,south:2,west:3,east:4,center:5},__wO:{'1':t,'2':e,'3':z,'4':u},__mb:function(){var H=this._getLayoutChildren();var P,J;var length=H.length;var L=[];var O=[];var K=[];var I=this.getSort()===o;var N=this.getSort()===A;for(var i=0;i<length;i++ ){P=H[i];K=P.getLayoutProperties().edge;if(K===n){if(J){throw new Error(b);}
;J=P;}
...
;this.__gK=Q;var M=[];for(var i=0;i<length;i++ ){K=Q[i].getLayoutProperties().edge;M[i]=this.__wN[K]||5;}
;this.__wM=M;delete this._invalidChildrenCache;}
,renderLayout:function(bl,bb,bt){if(this._invalidChildrenCache){this.__mb();}
;var bk=qx.ui.layout.Util;var W=this.__gK;var bm=this.__wM;var length=W.length;var ba,bu,Y,bf,bg,br,bd,T,S;var bx=[];var bc=[];var bs=this._getSeparatorWidths();var bB=this.getSpacingX();var bA=this.getSpacingY();var X=-bB;var bn=-bA;if(bs.x){X-=bs.x+bB;}
;if(bs.y){bn-=bs.y+bA;}
...
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__mb();}
;var bS=this.__gK;var cd=this.__wM;var length=bS.length;var bW,ce;var bY,bX;var ca=0,cl=0;var bP=0,cg=0;var cb=0,ck=0;var bQ=0,cf=0;var ch=this._getSeparatorWidths();var cj=this.getSpacingX(),ci=this.getSpacingY();var bU=-cj,bT=-ci;if(ch.x){bU-=ch.x+cj;}
;if(ch.y){bT-=ch.y+ci;}
...
},destruct:function(){this.__wM=this.__gK=null;}
});}
)();
...
,_applyValue:function(bi,bh){var bg=this.getChildControl(t);this._updateButtons();this.__yb=bi;if(bi!==null){if(this.getNumberFormat()){bg.setValue(this.getNumberFormat().format(bi));}
...
;this._applyValue(this.__yb,undefined);}
...
;}
;}
,_onKeyDown:function(e){switch(e.getKeyIdentifier()){case a:this.__yc=true;case p:this.getChildControl(y).press();break;case m:this.__yd=true;case h:this.getChildControl(q).press();break;default:return;};e.stopPropagation();e.preventDefault();}
,_onKeyUp:function(e){switch(e.getKeyIdentifier()){case a:this.getChildControl(y).release();this.__yc=false;break;case p:this.getChildControl(y).release();break;case m:this.getChildControl(q).release();this.__yd=false;break;case h:this.getChildControl(q).release();break;};}
...
;if(bx===this.__yb){this._applyValue(this.__yb);}
else {this.setValue(bx);}
;}
else {this._applyValue(this.__yb,undefined);}
;}
...
,_countUp:function(){if(this.__yc){var bE=this.getValue()+this.getPageStep();}
...
;}
;this.gotoValue(bE);}
,_countDown:function(){if(this.__yd){var bG=this.getValue()-this.getPageStep();}
