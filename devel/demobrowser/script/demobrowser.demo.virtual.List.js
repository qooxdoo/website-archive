...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.List.45404108c7b7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
};cQ.setDelegate(cX);cQ.getSelection().push(dc.getItem(20));cQ.getSelection().addListener(X,function(e){var dk=cQ.getSelection();for(var i=0;i<dk.getLength();i++ ){this.debug(s+dk.getItem(i).getLabel());}
;}
...
(function(){var a="one",b="Boolean",c="qx.ui.virtual.selection.MModel",d="qx.data.Array",f="multi",g="selected",h="_applySelection",j="change",k="_applyDragSelection",l="single",m="_applyQuickSelection",n="changeSelection",o="_applySelectionMode",p="additive",q="qx.event.type.Data";qx.Mixin.define(c,{construct:function(){this._initSelectionManager();this.__xg=new qx.data.Array();this.initSelection(this.__xg);}
,properties:{selection:{check:d,event:n,apply:h,nullable:false,deferredInit:true},selectionMode:{check:[l,f,p,a],init:l,apply:o},dragSelection:{check:b,init:false,apply:k},quickSelection:{check:b,init:false,apply:m}},events:{"changeSelection":q},members:{_manager:null,__xh:false,__xi:false,__xg:null,_initSelectionManager:function(){var self=this;var r={isItemSelectable:function(s){return self._provider.isSelectable(s);}
...
;this.__xh=true;var G=this.getSelection();var I=[];for(var i=0;i<G.getLength();i++ ){var H=G.getItem(i);var J=this._getSelectables();var F=-1;if(J!=null){F=J.indexOf(H);}
...
;this.__xj();if(this._afterApplySelection!=null&&qx.lang.Type.isFunction(this._afterApplySelection)){this._afterApplySelection();}
;this.__xh=false;}
,_onManagerChangeSelection:function(e){if(this.__xh==true){return;}
;this.__xi=true;this.__xj();this.__xi=false;}
...
;}
;this.__xk(N);}
,__xk:function(R){var Q=this.getSelection();if(R.length>0){var P=[0,Q.getLength()];P=P.concat(R);var S=Q.splice.apply(Q,P);S.dispose();}
else {Q.removeAll();}
;}
,__xl:function(){var U=this.getSelection();var W=this._manager.getSelection();if(U.getLength()!==W.length){return false;}
...
;}
},destruct:function(){this._manager.dispose();this._manager=null;if(this.__xg){this.__xg.dispose();}
;}
});}
)();
...
,_init:function(){this._provider=new qx.ui.list.provider.WidgetProvider(this);this.__uf=[];this.__xn=[];this.__xo={};this.__xp=false;this.__xq=false;this.__xr=false;this.getPane().addListener(d,this._onResize,this);this._initBackground();this._initLayer();}
...
;}
,_getLookupTable:function(){return this.__uf;}
,_lookup:function(Q){return this.__uf[Q];}
,_lookupGroup:function(R){return this.__xn.indexOf(R);}
,_reverseLookup:function(S){if(S<0){return -1;}
;return this.__uf.indexOf(S);}
...
,_applyGroupRowHeight:function(ba,Y){this.__xt();}
...
,_applyDelegate:function(bo,bn){this._provider.setDelegate(bo);this.__uu();}
,_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);}
,_onModelChange:function(e){this._provider.removeBindings();this.__uu();this._applyDefaultSelection();}
,__xs:function(){this.getPane().getRowConfig().setItemCount(this.__uf.length);this.getPane().fullUpdate();}
,__xt:function(){var bp=this.getPane().getRowConfig();var bq=this.getGroupItemHeight();bp.resetItemSizes();if(bq){for(var i=0,l=this.__uf.length;i<l; ++i){if(this.__uf[i]==-1){bp.setItemSize(i,bq);}
;}
;}
;}
,__uu:function(){this.__uf=[];this.__xn=[];this.__xo={};if(this.isAutoGrouping()){this.getGroups().removeAll();}
;var br=this.getModel();if(br!=null){this._runDelegateFilter(br);this._runDelegateSorter(br);this._runDelegateGroup(br);}
;this._updateSelection();this.__xt();this.__xs();}
,_runDelegateFilter:function(bt){var bs=qx.util.Delegate.getMethod(this.getDelegate(),D);for(var i=0,l=bt.length;i<l; ++i){if(bs==null||bs(bt.getItem(i))){this.__uf.push(i);}
;}
;}
,_runDelegateSorter:function(bu){if(this.__uf.length==0){return;}
;var bv=qx.util.Delegate.getMethod(this.getDelegate(),t);if(bv!=null){this.__uf.sort(function(a,b){return bv(bu.getItem(a),bu.getItem(b));}
);}
;}
,_runDelegateGroup:function(bz){var bA=qx.util.Delegate.getMethod(this.getDelegate(),v);if(bA!=null){for(var i=0,l=this.__uf.length;i<l; ++i){var bw=this.__uf[i];var by=this.getModel().getItem(bw);var bx=bA(by);this.__xu(bx,bw);}
;this.__uf=this.__xv();}
;}
,__xu:function(bC,bB){if(bC==null){this.__xr=true;bC=A;}
;var name=this.__xw(bC);if(this.__xo[name]==null){this.__xo[name]=[];if(this.isAutoGrouping()){this.getGroups().push(bC);}
;}
;this.__xo[name].push(bB);}
,__xv:function(){this.__xx();var bG=[];var bI=0;var bE=this.getGroups();for(var i=0;i<bE.getLength();i++ ){var bD=bE.getItem(i);bG.push(-1);this.__xn.push(bI);bI++ ;var bH=this.__xw(bD);var bF=this.__xo[bH];if(bF!=null){for(var k=0;k<bF.length;k++ ){bG.push(bF[k]);bI++ ;}
;}
;}
;return bG;}
,__xw:function(bK){var name=null;if(!qx.lang.Type.isString(bK)){var bJ=this.getGroups().indexOf(bK);this.__xq=true;name=v;if(bJ==-1){name+=this.getGroups().getLength();}
else {name+=bJ;}
;}
else {this.__xp=true;var name=bK;}
;return name;}
,__xx:function(){if(this.__xq&&this.__xr||this.__xq&&this.__xp){throw new Error(r+z);}
;}
...
;this._background.dispose();this._provider.dispose();this._layer.dispose();this._background=this._provider=this._layer=this.__uf=this.__xn=this.__xo=null;if(this.__xm){this.__xm.dispose();}
;}
});}
)();
...
(function(){var a="cell.type",b="model[",c="groups[",d="model",f="",g="changeDelegate",h="label",i="qx.ui.list.core.MWidgetController",j="icon",k="]",l="value",m="BindingIds",n=".",o="group",p="String";qx.Mixin.define(i,{construct:function(){this.__xF=[];}
,properties:{labelPath:{check:p,nullable:true},iconPath:{check:p,nullable:true},groupLabelPath:{check:p,nullable:true},labelOptions:{nullable:true},iconOptions:{nullable:true},groupLabelOptions:{nullable:true},delegate:{event:g,init:null,nullable:true}},members:{__xF:null,bindDefaultProperties:function(q,r){if(q.getUserData(a)!=o){this.bindProperty(f,d,null,q,r);this.bindProperty(this.getLabelPath(),h,this.getLabelOptions(),q,r);if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),j,this.getIconOptions(),q,r);}
;}
else {this.bindProperty(this.getGroupLabelPath(),l,this.getGroupLabelOptions(),q,r);}
;}
,bindProperty:function(u,x,v,t,s){var y=t.getUserData(a);var w=this.__xG(s,u,y);if(v){v.ignoreConverter=d;}
;var z=this._list.bind(w,t,x,v);this.__uE(t,z);}
,bindPropertyReverse:function(D,B,E,F,A){var G=F.getUserData(a);var C=this.__xG(A,D,G);var H=F.bind(B,this._list,C,E);this.__uE(F,H);}
,removeBindings:function(){while(this.__xF.length>0){var I=this.__xF.pop();this._removeBindingsFrom(I);}
;}
...
;}
,_removeBindingsFrom:function(T){var U=this.__xH(T);while(U.length>0){var V=U.pop();try{this._list.removeBinding(V);}
catch(e){T.removeBinding(V);}
;}
;if(qx.lang.Array.contains(this.__xF,T)){qx.lang.Array.remove(this.__xF,T);}
;}
,__xG:function(X,ba,Y){var W=b+X+k;if(Y==o){W=c+X+k;}
;if(ba!=null&&ba!=f){W+=n+ba;}
;return W;}
,__uE:function(bc,bd){var bb=this.__xH(bc);if(!qx.lang.Array.contains(bb,bd)){bb.push(bd);}
;if(!qx.lang.Array.contains(this.__xF,bc)){this.__xF.push(bc);}
;}
,__xH:function(be){var bf=be.getUserData(m);if(bf==null){bf=[];be.setUserData(m,bf);}
;return bf;}
...
,styleSelectabled:function(v){var u=this.__xI(v);this._styleSelectabled(u);}
,styleUnselectabled:function(x){var w=this.__xI(x);this._styleUnselectabled(w);}
...
;}
,_styleSelectabled:function(A){this.__xJ(A,{selected:1});}
,_styleUnselectabled:function(B){this.__xJ(B,{});}
...
,__xI:function(G){return this._list._layer.getRenderedCellWidget(G,0);}
,__xJ:function(I,H){if(I==null){return;}
...
;this._cellProvider=h;this.__xK=[];}
,properties:{anonymous:{refine:true,init:false}},events:{updated:c},members:{__xK:null,getRenderedCellWidget:function(q,n){if(this._getChildren().length===0){return null;}
...
;}
,_getSpacer:function(){var r=this.__xK.pop();if(!r){r=new qx.ui.core.Spacer();r.setUserData(g,1);}
;return r;}
...
;}
;}
,_fullUpdate:function(B,v,A,z){var C=this._cellProvider;var H=this._getChildren().concat();for(var i=0;i<H.length;i++ ){var F=H[i];if(F.getUserData(g)){this.__xK.push(F);}
...
;}
;}
;var L=this._cellProvider;var P=this._getChildren().concat();for(var i=0;i<P.length;i++ ){if(!bb[i]){var W=P[i];if(W.getUserData(g)){this.__xK.push(W);}
...
;this._cellProvider=this.__xK=null;}
});}
)();
...
(function(){var a="created",b="qx.ui.virtual.cell.AbstractWidget",c="cell.states",d="abstract method call",e="qx.event.type.Data";qx.Class.define(b,{extend:qx.core.Object,implement:[qx.ui.virtual.cell.IWidgetCell],construct:function(){qx.core.Object.call(this);this.__db=[];}
,events:{"created":e},members:{__db:null,_createWidget:function(){throw new Error(d);}
...
;}
;j.setUserData(c,h);}
,getCellWidget:function(o,m){var n=this.__xL();this.updateStates(n,m);this.updateData(n,o);return n;}
,pool:function(p){this.__db.push(p);}
,_cleanupPool:function(){var q=this.__db.pop();while(q){q.destroy();q=this.__db.pop();}
;}
,__xL:function(){var r=this.__db.shift();if(r==null){r=this._createWidget();this.fireDataEvent(a,r);}
;return r;}
},destruct:function(){this._cleanupPool();this.__db=null;}
});}
)();
...
(function(){var a="demobrowser.demo.virtual.model.Item",b="changeIcon",c="String",d="changeLabel";qx.Class.define(a,{extend:qx.core.Object,construct:function(e,f){qx.core.Object.call(this);if(e!=null){this.setLabel(e);}
;if(f!=null){this.setIcon(f);}
;}
,properties:{label:{check:c,event:d,nullable:true},icon:{check:c,event:b,nullable:true}}});}
)();
