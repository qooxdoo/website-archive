...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.List.7204db8d8d11.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="one",b="Boolean",c="qx.ui.virtual.selection.MModel",d="qx.data.Array",f="multi",g="selected",h="_applySelection",j="change",k="_applyDragSelection",l="single",m="_applyQuickSelection",n="changeSelection",o="_applySelectionMode",p="additive",q="qx.event.type.Data";qx.Mixin.define(c,{construct:function(){this._initSelectionManager();this.__xc=new qx.data.Array();this.initSelection(this.__xc);}
,properties:{selection:{check:d,event:n,apply:h,nullable:false,deferredInit:true},selectionMode:{check:[l,f,p,a],init:l,apply:o},dragSelection:{check:b,init:false,apply:k},quickSelection:{check:b,init:false,apply:m}},events:{"changeSelection":q},members:{_manager:null,__xd:false,__xe:false,__xc:null,_initSelectionManager:function(){var self=this;var r={isItemSelectable:function(s){return self._provider.isSelectable(s);}
...
;this.__xf();if(this._afterApplySelection!=null&&qx.lang.Type.isFunction(this._afterApplySelection)){this._afterApplySelection();}
;this.__xd=false;}
,_onManagerChangeSelection:function(e){if(this.__xd==true){return;}
;this.__xe=true;this.__xf();this.__xe=false;}
,__xf:function(){if(this.__xh()){return;}
;var M=this._manager.getSelection();var N=[];for(var i=0;i<M.length;i++ ){var O=this._getDataFromRow(M[i]);if(O!=null){N.push(O);}
;}
;this.__xg(N);}
,__xg:function(R){var Q=this.getSelection();if(R.length>0){var P=[0,Q.getLength()];P=P.concat(R);var S=Q.splice.apply(Q,P);S.dispose();}
else {Q.removeAll();}
;}
,__xh:function(){var U=this.getSelection();var W=this._manager.getSelection();if(U.getLength()!==W.length){return false;}
...
;}
},destruct:function(){this._manager.dispose();this._manager=null;if(this.__xc){this.__xc.dispose();}
;}
});}
)();
...
,_init:function(){this._provider=new qx.ui.list.provider.WidgetProvider(this);this.__ua=[];this.__xj=[];this.__xk={};this.__xl=false;this.__xm=false;this.__xn=false;this.getPane().addListener(d,this._onResize,this);this._initBackground();this._initLayer();}
...
;}
,_getLookupTable:function(){return this.__ua;}
,_lookup:function(Q){return this.__ua[Q];}
,_lookupGroup:function(R){return this.__xj.indexOf(R);}
,_reverseLookup:function(S){if(S<0){return -1;}
;return this.__ua.indexOf(S);}
...
,_applyDelegate:function(bo,bn){this._provider.setDelegate(bo);this.__up();}
,_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);}
,_onModelChange:function(e){this._provider.removeBindings();this.__up();this._applyDefaultSelection();}
,__xo:function(){this.getPane().getRowConfig().setItemCount(this.__ua.length);this.getPane().fullUpdate();}
,__xp:function(){var bp=this.getPane().getRowConfig();var bq=this.getGroupItemHeight();bp.resetItemSizes();if(bq){for(var i=0,l=this.__ua.length;i<l; ++i){if(this.__ua[i]==-1){bp.setItemSize(i,bq);}
;}
;}
;}
,__up:function(){this.__ua=[];this.__xj=[];this.__xk={};if(this.isAutoGrouping()){this.getGroups().removeAll();}
...
,_runDelegateFilter:function(bt){var bs=qx.util.Delegate.getMethod(this.getDelegate(),D);for(var i=0,l=bt.length;i<l; ++i){if(bs==null||bs(bt.getItem(i))){this.__ua.push(i);}
;}
;}
,_runDelegateSorter:function(bu){if(this.__ua.length==0){return;}
;var bv=qx.util.Delegate.getMethod(this.getDelegate(),t);if(bv!=null){this.__ua.sort(function(a,b){return bv(bu.getItem(a),bu.getItem(b));}
);}
;}
,_runDelegateGroup:function(bz){var bA=qx.util.Delegate.getMethod(this.getDelegate(),v);if(bA!=null){for(var i=0,l=this.__ua.length;i<l; ++i){var bw=this.__ua[i];var by=this.getModel().getItem(bw);var bx=bA(by);this.__xq(bx,bw);}
;this.__ua=this.__xr();}
;}
,__xq:function(bC,bB){if(bC==null){this.__xn=true;bC=A;}
;var name=this.__xs(bC);if(this.__xk[name]==null){this.__xk[name]=[];if(this.isAutoGrouping()){this.getGroups().push(bC);}
;}
;this.__xk[name].push(bB);}
,__xr:function(){this.__xt();var bG=[];var bI=0;var bE=this.getGroups();for(var i=0;i<bE.getLength();i++ ){var bD=bE.getItem(i);bG.push(-1);this.__xj.push(bI);bI++ ;var bH=this.__xs(bD);var bF=this.__xk[bH];if(bF!=null){for(var k=0;k<bF.length;k++ ){bG.push(bF[k]);bI++ ;}
;}
;}
;return bG;}
,__xs:function(bK){var name=null;if(!qx.lang.Type.isString(bK)){var bJ=this.getGroups().indexOf(bK);this.__xm=true;name=v;if(bJ==-1){name+=this.getGroups().getLength();}
else {name+=bJ;}
;}
else {this.__xl=true;var name=bK;}
;return name;}
,__xt:function(){if(this.__xm&&this.__xn||this.__xm&&this.__xl){throw new Error(r+z);}
;}
...
;this._background.dispose();this._provider.dispose();this._layer.dispose();this._background=this._provider=this._layer=this.__ua=this.__xj=this.__xk=null;if(this.__xi){this.__xi.dispose();}
;}
});}
)();
...
(function(){var a="cell.type",b="model[",c="groups[",d="model",f="",g="changeDelegate",h="label",i="qx.ui.list.core.MWidgetController",j="icon",k="]",l="value",m="BindingIds",n=".",o="group",p="String";qx.Mixin.define(i,{construct:function(){this.__xB=[];}
,properties:{labelPath:{check:p,nullable:true},iconPath:{check:p,nullable:true},groupLabelPath:{check:p,nullable:true},labelOptions:{nullable:true},iconOptions:{nullable:true},groupLabelOptions:{nullable:true},delegate:{event:g,init:null,nullable:true}},members:{__xB:null,bindDefaultProperties:function(q,r){if(q.getUserData(a)!=o){this.bindProperty(f,d,null,q,r);this.bindProperty(this.getLabelPath(),h,this.getLabelOptions(),q,r);if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),j,this.getIconOptions(),q,r);}
;}
else {this.bindProperty(this.getGroupLabelPath(),l,this.getGroupLabelOptions(),q,r);}
;}
,bindProperty:function(u,x,v,t,s){var y=t.getUserData(a);var w=this.__xC(s,u,y);if(v){v.ignoreConverter=d;}
;var z=this._list.bind(w,t,x,v);this.__uA(t,z);}
,bindPropertyReverse:function(D,B,E,F,A){var G=F.getUserData(a);var C=this.__xC(A,D,G);var H=F.bind(B,this._list,C,E);this.__uA(F,H);}
,removeBindings:function(){while(this.__xB.length>0){var I=this.__xB.pop();this._removeBindingsFrom(I);}
;}
...
;}
,_removeBindingsFrom:function(T){var U=this.__xD(T);while(U.length>0){var V=U.pop();try{this._list.removeBinding(V);}
catch(e){T.removeBinding(V);}
;}
;if(qx.lang.Array.contains(this.__xB,T)){qx.lang.Array.remove(this.__xB,T);}
;}
,__xC:function(X,ba,Y){var W=b+X+k;if(Y==o){W=c+X+k;}
;if(ba!=null&&ba!=f){W+=n+ba;}
;return W;}
,__uA:function(bc,bd){var bb=this.__xD(bc);if(!qx.lang.Array.contains(bb,bd)){bb.push(bd);}
;if(!qx.lang.Array.contains(this.__xB,bc)){this.__xB.push(bc);}
;}
,__xD:function(be){var bf=be.getUserData(m);if(bf==null){bf=[];be.setUserData(m,bf);}
;return bf;}
...
,styleSelectabled:function(v){var u=this.__xE(v);this._styleSelectabled(u);}
,styleUnselectabled:function(x){var w=this.__xE(x);this._styleUnselectabled(w);}
...
;}
,_styleSelectabled:function(A){this.__xF(A,{selected:1});}
,_styleUnselectabled:function(B){this.__xF(B,{});}
...
,__xE:function(G){return this._list._layer.getRenderedCellWidget(G,0);}
,__xF:function(I,H){if(I==null){return;}
...
;this._cellProvider=h;this.__xG=[];}
,properties:{anonymous:{refine:true,init:false}},events:{updated:c},members:{__xG:null,getRenderedCellWidget:function(q,n){if(this._getChildren().length===0){return null;}
...
;}
,_getSpacer:function(){var r=this.__xG.pop();if(!r){r=new qx.ui.core.Spacer();r.setUserData(g,1);}
;return r;}
...
;}
;}
,_fullUpdate:function(B,v,A,z){var C=this._cellProvider;var H=this._getChildren().concat();for(var i=0;i<H.length;i++ ){var F=H[i];if(F.getUserData(g)){this.__xG.push(F);}
...
;}
;}
;var L=this._cellProvider;var P=this._getChildren().concat();for(var i=0;i<P.length;i++ ){if(!bb[i]){var W=P[i];if(W.getUserData(g)){this.__xG.push(W);}
...
;this._cellProvider=this.__xG=null;}
});}
)();
...
(function(){var a="created",b="qx.ui.virtual.cell.AbstractWidget",c="cell.states",d="abstract method call",e="qx.event.type.Data";qx.Class.define(b,{extend:qx.core.Object,implement:[qx.ui.virtual.cell.IWidgetCell],construct:function(){qx.core.Object.call(this);this.__cX=[];}
,events:{"created":e},members:{__cX:null,_createWidget:function(){throw new Error(d);}
...
;}
;j.setUserData(c,h);}
,getCellWidget:function(o,m){var n=this.__xH();this.updateStates(n,m);this.updateData(n,o);return n;}
,pool:function(p){this.__cX.push(p);}
,_cleanupPool:function(){var q=this.__cX.pop();while(q){q.destroy();q=this.__cX.pop();}
;}
,__xH:function(){var r=this.__cX.shift();if(r==null){r=this._createWidget();this.fireDataEvent(a,r);}
;return r;}
},destruct:function(){this._cleanupPool();this.__cX=null;}
});}
)();
