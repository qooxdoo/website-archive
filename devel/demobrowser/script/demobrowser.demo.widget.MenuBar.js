...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.MenuBar.9547aa1efdd5.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var P=K.getMarginLeft()+K.getMarginRight();P=Math.max(P,this.getSpacing());var J=K.getSizeHint().width+P;this.__nV(K);G-=J;if(E&&E.getVisibility()!=o){E.setVisibility(o);G+=L;var H=E.getMarginLeft()+E.getMarginRight();G+=Math.max(H,this.getSpacing());}
;}
while(G>F);}
else if(this.__nS.length>0){do {var Q=this.__nS[0];if(Q){var P=Q.getMarginLeft()+Q.getMarginRight();P=Math.max(P,this.getSpacing());if(Q.getContentElement().getDomElement()==null){Q.syncAppearance();Q.invalidateLayoutCache();}
;var I=Q.getSizeHint().width;var O=false;if(this.__nS.length==1&&L>0){var N=P-this.getSpacing();var M=G-L+I+N;O=F>M;}
;if(F>G+I+P||O){this.__nU(Q);G+=I;if(E&&this.__nS.length==0){E.setVisibility(y);}
;}
else {return;}
;}
;}
while(F>=G&&this.__nS.length>0);}
;}
,__nU:function(R){R.setVisibility(o);this.__nS.shift();this.fireDataEvent(m,R);}
,__nV:function(S){if(!S){return;}
;this.__nS.unshift(S);S.setVisibility(y);this.fireDataEvent(A,S);}
,_getNextToHide:function(){for(var i=this.__nT.length-1;i>=0;i-- ){var T=this.__nT[i];if(T&&T.getVisibility&&T.getVisibility()==o){return T;}
;}
...
;}
;}
,setRemovePriority:function(X,W,Y){if(!Y&&this.__nT[W]!=undefined){throw new Error(v);}
;this.__nT[W]=X;}
...
;for(var i=0;i<this.__nS.length;i++ ){this.__nS[i].setVisibility(o);}
;this.__nS=[];}
;}
...
;bf.setVisibility(y);}
;}
,__nW:false,_setAllowMenuOpenHover:function(bg){this.__nW=bg;}
,_isAllowMenuOpenHover:function(){return this.__nW;}
...
(function(){var a="blur",b="Enter",c="Escape",d="Space",f="keydown",g="Left",h="Down",j="Up",k="singleton",l="qx.debug",m="interval",n="roll",o="event.touch",p="Object is no menu: ",q="__oc",r="pointerdown",s="qx.ui.menu.Manager",t="keyup",u="keypress",v="contextmenu",w="__lH",x="__od",y="Right";qx.Class.define(s,{type:k,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__lH=[];var z=document.body;var A=qx.event.Registration;A.addListener(window.document.documentElement,r,this._onPointerDown,this,true);A.addListener(z,n,this._onRoll,this,true);A.addListener(z,f,this._onKeyUpDown,this,true);A.addListener(z,t,this._onKeyUpDown,this,true);A.addListener(z,u,this._onKeyPress,this,true);if(!qx.core.Environment.get(o)){qx.bom.Element.addListener(window,a,this.hideAll,this);}
;this.__oc=new qx.event.Timer();this.__oc.addListener(m,this._onOpenInterval,this);this.__od=new qx.event.Timer();this.__od.addListener(m,this._onCloseInterval,this);}
,members:{__oe:null,__of:null,__oc:null,__od:null,__lH:null,_getChild:function(B,D,C,G){var F=B.getChildren();var length=F.length;var E;for(var i=D;i<length&&i>=0;i+=C){E=F[i];if(E.isEnabled()&&!E.isAnonymous()&&E.isVisible()){return E;}
;}
...
;H=H.getLayoutParent();}
;return false;}
,_isMenuOpener:function(J){var I=this.__lH;for(var i=0;i<I.length;i++ ){if(I[i].getOpener()===J){return true;}
;}
;return false;}
...
;}
;var M=this.__lH;M.push(L);L.setZIndex(1e6+M.length);}
,remove:function(N){if(qx.core.Environment.get(l)){if(!(N instanceof qx.ui.menu.Menu)){throw new Error(p+N);}
;}
;var O=this.__lH;if(O){qx.lang.Array.remove(O,N);}
;}
,hideAll:function(){var P=this.__lH;if(P){for(var i=P.length-1;i>=0;i-- ){P[i].exclude();}
;}
;}
,getActiveMenu:function(){var Q=this.__lH;return Q.length>0?Q[Q.length-1]:null;}
,scheduleOpen:function(R){this.cancelClose(R);if(R.isVisible()){if(this.__oe){this.cancelOpen(this.__oe);}
;}
else if(this.__oe!=R){this.__oe=R;this.__oc.restartWith(R.getOpenInterval());}
;}
,scheduleClose:function(S){this.cancelOpen(S);if(!S.isVisible()){if(this.__of){this.cancelClose(this.__of);}
;}
else if(this.__of!=S){this.__of=S;this.__od.restartWith(S.getCloseInterval());}
;}
,cancelOpen:function(T){if(this.__oe==T){this.__oc.stop();this.__oe=null;}
;}
,cancelClose:function(U){if(this.__of==U){this.__od.stop();this.__of=null;}
;}
,_onOpenInterval:function(e){this.__oc.stop();this.__oe.open();this.__oe=null;}
,_onCloseInterval:function(e){this.__od.stop();this.__of.exclude();this.__of=null;}
,preventContextMenuOnce:function(){qx.event.Registration.addListener(document.body,v,this.__og,this,true);}
,__og:function(e){var V=e.getTarget();V=qx.ui.core.Widget.getWidgetByElement(V,true);if(this._isInMenu(V)){e.stopPropagation();e.preventDefault();}
;qx.event.Registration.removeListener(document.body,v,this.__og,this,true);}
...
;if(this.__lH.length>0&&!this._isInMenu(W)){this.hideAll();}
;}
,__oh:{"Enter":1,"Space":1},__oi:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var X=this.getActiveMenu();if(!X){return;}
;var Y=e.getKeyIdentifier();if(this.__oi[Y]||(this.__oh[Y]&&X.getSelectedButton())){e.stopPropagation();}
;}
,_onKeyPress:function(e){var bc=this.getActiveMenu();if(!bc){return;}
;var ba=e.getKeyIdentifier();var be=this.__oi[ba];var bb=this.__oh[ba];if(be){switch(ba){case j:this._onKeyPressUp(bc);break;case h:this._onKeyPressDown(bc);break;case g:this._onKeyPressLeft(bc);break;case y:this._onKeyPressRight(bc);break;case c:this.hideAll();break;};e.stopPropagation();e.preventDefault();}
...
;this.__om();}
,__om:function(){if(this.isVisible()){if(this.getBlockBackground()){var V=this.getZIndex();this._blocker.blockContent(V-1);}
;}
...
,__on:function(){var bn=this._getMenuBounds();if(!bn){this.addListenerOnce(n,this.__on,this);return;}
...
;this.__ol=bp;qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(bq){this.getChildControl(h);if(this.__ol){this.__ol.call(this);delete this.__ol;}
;}
...
;this.__on();}
;}
,_onPointerOver:function(e){var bt=qx.ui.menu.Manager.getInstance();bt.cancelClose(this);var bu=e.getTarget();if(bu.isEnabled()&&bu instanceof qx.ui.menu.AbstractButton){this.setSelectedButton(bu);var bs=bu.getMenu&&bu.getMenu();if(bs){bs.setOpener(bu);bt.scheduleOpen(bs);this.__ok=bs;}
else {var bv=this.getOpenedButton();if(bv){bt.scheduleClose(bv.getMenu());}
;if(this.__ok){bt.cancelOpen(this.__ok);this.__ok=null;}
;}
;}
...
;if(this.__ok){bw.cancelOpen(this.__ok);}
;}
;}
...
(function(){var a="qx.ui.menu.Layout",b="Integer",c="_applyLayoutChange";qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:b,init:0,apply:c},spanColumn:{check:b,init:1,nullable:true,apply:c},iconColumnWidth:{check:b,init:0,themeable:true,apply:c},arrowColumnWidth:{check:b,init:0,themeable:true,apply:c}},members:{__oo:null,_computeSizeHint:function(){var q=this._getLayoutChildren();var o,g,k;var e=this.getSpanColumn();var j=this.__oo=[0,0,0,0];var n=this.getColumnSpacing();var m=0;var f=0;for(var i=0,l=q.length;i<l;i++ ){o=q[i];if(o.isAnonymous()){continue;}
...
,getColumnSizes:function(){return this.__oo||null;}
...
;var x=this.__op(y[0]);var w=x.getColumnSizes();var s=x.getSpacingX();var z=qx.lang.Array.sum(w)+s*(w.length-1);if(z<o){w[1]+=o-z;}
...
;}
;}
,__op:function(B){while(!(B instanceof qx.ui.menu.Menu)){B=B.getLayoutParent();}
;return B;}
...
(function(){var a="hovered",b="hover-button",c="pointerover",d="interval",f="__du",g="qx.ui.form.HoverButton",h="Integer",i="pointerout";qx.Class.define(g,{extend:qx.ui.basic.Atom,include:[qx.ui.core.MExecutable],implement:[qx.ui.form.IExecutable],construct:function(j,k){qx.ui.basic.Atom.call(this,j,k);this.addListener(c,this._onPointerOver,this);this.addListener(i,this._onPointerOut,this);this.__du=new qx.event.AcceleratingTimer();this.__du.addListener(d,this._onInterval,this);}
,properties:{appearance:{refine:true,init:b},interval:{check:h,init:80},firstInterval:{check:h,init:200},minTimer:{check:h,init:20},timerDecrease:{check:h,init:2}},members:{__du:null,_onPointerOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;}
;this.__du.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();this.addState(a);}
,_onPointerOut:function(e){this.__du.stop();this.removeState(a);if(!this.isEnabled()||e.getTarget()!==this){return;}
;}
...
,__ov:function(){var y=this.getChildrenContainer().getChildren();y=y.filter(function(z){return z.getVisibility()==k;}
