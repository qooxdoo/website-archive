...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.MenuBar.5ed3af6266ea.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var P=K.getMarginLeft()+K.getMarginRight();P=Math.max(P,this.getSpacing());var J=K.getSizeHint().width+P;this.__nW(K);G-=J;if(E&&E.getVisibility()!=o){E.setVisibility(o);G+=L;var H=E.getMarginLeft()+E.getMarginRight();G+=Math.max(H,this.getSpacing());}
;}
while(G>F);}
else if(this.__nT.length>0){do {var Q=this.__nT[0];if(Q){var P=Q.getMarginLeft()+Q.getMarginRight();P=Math.max(P,this.getSpacing());if(Q.getContentElement().getDomElement()==null){Q.syncAppearance();Q.invalidateLayoutCache();}
;var I=Q.getSizeHint().width;var O=false;if(this.__nT.length==1&&L>0){var N=P-this.getSpacing();var M=G-L+I+N;O=F>M;}
;if(F>G+I+P||O){this.__nV(Q);G+=I;if(E&&this.__nT.length==0){E.setVisibility(y);}
;}
else {return;}
;}
;}
while(F>=G&&this.__nT.length>0);}
;}
,__nV:function(R){R.setVisibility(o);this.__nT.shift();this.fireDataEvent(m,R);}
,__nW:function(S){if(!S){return;}
;this.__nT.unshift(S);S.setVisibility(y);this.fireDataEvent(A,S);}
,_getNextToHide:function(){for(var i=this.__nU.length-1;i>=0;i-- ){var T=this.__nU[i];if(T&&T.getVisibility&&T.getVisibility()==o){return T;}
;}
...
;}
;}
,setRemovePriority:function(X,W,Y){if(!Y&&this.__nU[W]!=undefined){throw new Error(v);}
;this.__nU[W]=X;}
...
;for(var i=0;i<this.__nT.length;i++ ){this.__nT[i].setVisibility(o);}
;this.__nT=[];}
;}
...
;bf.setVisibility(y);}
;}
,__nX:false,_setAllowMenuOpenHover:function(bg){this.__nX=bg;}
,_isAllowMenuOpenHover:function(){return this.__nX;}
...
(function(){var a="blur",b="Enter",c="Escape",d="Space",f="keydown",g="Left",h="Down",j="__lI",k="Up",l="singleton",m="qx.debug",n="interval",o="roll",p="event.touch",q="Object is no menu: ",r="__oe",s="pointerdown",t="qx.ui.menu.Manager",u="keyup",v="keypress",w="contextmenu",x="__od",y="Right";qx.Class.define(t,{type:l,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__lI=[];var z=document.body;var A=qx.event.Registration;A.addListener(window.document.documentElement,s,this._onPointerDown,this,true);A.addListener(z,o,this._onRoll,this,true);A.addListener(z,f,this._onKeyUpDown,this,true);A.addListener(z,u,this._onKeyUpDown,this,true);A.addListener(z,v,this._onKeyPress,this,true);if(!qx.core.Environment.get(p)){qx.bom.Element.addListener(window,a,this.hideAll,this);}
;this.__od=new qx.event.Timer();this.__od.addListener(n,this._onOpenInterval,this);this.__oe=new qx.event.Timer();this.__oe.addListener(n,this._onCloseInterval,this);}
,members:{__of:null,__og:null,__od:null,__oe:null,__lI:null,_getChild:function(B,D,C,G){var F=B.getChildren();var length=F.length;var E;for(var i=D;i<length&&i>=0;i+=C){E=F[i];if(E.isEnabled()&&!E.isAnonymous()&&E.isVisible()){return E;}
;}
...
;H=H.getLayoutParent();}
;return false;}
,_isMenuOpener:function(J){var I=this.__lI;for(var i=0;i<I.length;i++ ){if(I[i].getOpener()===J){return true;}
;}
;return false;}
...
;}
;var M=this.__lI;M.push(L);L.setZIndex(1e6+M.length);}
,remove:function(N){if(qx.core.Environment.get(m)){if(!(N instanceof qx.ui.menu.Menu)){throw new Error(q+N);}
;}
;var O=this.__lI;if(O){qx.lang.Array.remove(O,N);}
;}
,hideAll:function(){var P=this.__lI;if(P){for(var i=P.length-1;i>=0;i-- ){P[i].exclude();}
;}
;}
,getActiveMenu:function(){var Q=this.__lI;return Q.length>0?Q[Q.length-1]:null;}
,scheduleOpen:function(R){this.cancelClose(R);if(R.isVisible()){if(this.__of){this.cancelOpen(this.__of);}
;}
else if(this.__of!=R){this.__of=R;this.__od.restartWith(R.getOpenInterval());}
;}
,scheduleClose:function(S){this.cancelOpen(S);if(!S.isVisible()){if(this.__og){this.cancelClose(this.__og);}
;}
else if(this.__og!=S){this.__og=S;this.__oe.restartWith(S.getCloseInterval());}
;}
,cancelOpen:function(T){if(this.__of==T){this.__od.stop();this.__of=null;}
;}
,cancelClose:function(U){if(this.__og==U){this.__oe.stop();this.__og=null;}
;}
...
,preventContextMenuOnce:function(){qx.event.Registration.addListener(document.body,w,this.__oh,this,true);}
,__oh:function(e){var V=e.getTarget();V=qx.ui.core.Widget.getWidgetByElement(V,true);if(this._isInMenu(V)){e.stopPropagation();e.preventDefault();}
;qx.event.Registration.removeListener(document.body,w,this.__oh,this,true);}
...
;if(this.__lI.length>0&&!this._isInMenu(W)){this.hideAll();}
;}
,__oi:{"Enter":1,"Space":1},__oj:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var X=this.getActiveMenu();if(!X){return;}
;var Y=e.getKeyIdentifier();if(this.__oj[Y]||(this.__oi[Y]&&X.getSelectedButton())){e.stopPropagation();}
;}
,_onKeyPress:function(e){var bc=this.getActiveMenu();if(!bc){return;}
;var ba=e.getKeyIdentifier();var be=this.__oj[ba];var bb=this.__oi[ba];if(be){switch(ba){case k:this._onKeyPressUp(bc);break;case h:this._onKeyPressDown(bc);break;case g:this._onKeyPressLeft(bc);break;case y:this._onKeyPressRight(bc);break;case c:this.hideAll();break;};e.stopPropagation();e.preventDefault();}
...
;this.__on();}
,__on:function(){if(this.isVisible()){if(this.getBlockBackground()){var V=this.getZIndex();this._blocker.blockContent(V-1);}
;}
...
,__oo:function(){var bn=this._getMenuBounds();if(!bn){this.addListenerOnce(n,this.__oo,this);return;}
...
;this.__om=bp;qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(bq){this.getChildControl(h);if(this.__om){this.__om.call(this);delete this.__om;}
;}
...
;this.__oo();}
;}
,_onPointerOver:function(e){var bt=qx.ui.menu.Manager.getInstance();bt.cancelClose(this);var bu=e.getTarget();if(bu.isEnabled()&&bu instanceof qx.ui.menu.AbstractButton){this.setSelectedButton(bu);var bs=bu.getMenu&&bu.getMenu();if(bs){bs.setOpener(bu);bt.scheduleOpen(bs);this.__ol=bs;}
else {var bv=this.getOpenedButton();if(bv){bt.scheduleClose(bv.getMenu());}
;if(this.__ol){bt.cancelOpen(this.__ol);this.__ol=null;}
;}
;}
...
;if(this.__ol){bw.cancelOpen(this.__ol);}
;}
;}
...
(function(){var a="qx.ui.menu.Layout",b="Integer",c="_applyLayoutChange";qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:b,init:0,apply:c},spanColumn:{check:b,init:1,nullable:true,apply:c},iconColumnWidth:{check:b,init:0,themeable:true,apply:c},arrowColumnWidth:{check:b,init:0,themeable:true,apply:c}},members:{__op:null,_computeSizeHint:function(){var q=this._getLayoutChildren();var o,g,k;var e=this.getSpanColumn();var j=this.__op=[0,0,0,0];var n=this.getColumnSpacing();var m=0;var f=0;for(var i=0,l=q.length;i<l;i++ ){o=q[i];if(o.isAnonymous()){continue;}
...
,getColumnSizes:function(){return this.__op||null;}
...
;var x=this.__oq(y[0]);var w=x.getColumnSizes();var s=x.getSpacingX();var z=qx.lang.Array.sum(w)+s*(w.length-1);if(z<o){w[1]+=o-z;}
...
;}
;}
,__oq:function(B){while(!(B instanceof qx.ui.menu.Menu)){B=B.getLayoutParent();}
;return B;}
...
,__ow:function(){var y=this.getChildrenContainer().getChildren();y=y.filter(function(z){return z.getVisibility()==k;}
