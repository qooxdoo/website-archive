...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Complex.1de3ad6136a3.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,properties:{scroller:{check:i,nullable:true,init:null,apply:h},interval:{check:k,init:200,apply:b}},members:{__Cu:null,__Cv:null,__dD:null,__Cw:null,__Cx:null,setPrefetchX:function(o,r,p,q){this.__Cu=[o,r,p,q];}
,setPrefetchY:function(v,u,t,s){this.__Cv=[v,u,t,s];}
,_onInterval:function(){var w=this.__Cu;if(w[1]&&w[3]){this.getScroller().getPane().prefetchX(w[0],w[1],w[2],w[3]);qx.ui.core.queue.Manager.flush();}
;var x=this.__Cv;if(x[1]&&x[3]){this.getScroller().getPane().prefetchY(x[0],x[1],x[2],x[3]);qx.ui.core.queue.Manager.flush();}
;}
,_applyScroller:function(z,y){if(y){if(this.__Cw){y.getChildControl(j).removeListenerById(this.__Cw);}
;if(this.__Cx){y.getChildControl(l).removeListenerById(this.__Cx);}
;}
...
;this.__Cw=z.getChildControl(j).addListener(g,this.__dD.restart,this.__dD);this.__Cx=z.getChildControl(l).addListener(g,this.__dD.restart,this.__dD);}
else {this.__dD.stop();}
;}
,_applyInterval:function(B,A){this.__dD.setInterval(B);}
},destruct:function(){this.setScroller(null);this.__Cu=this.__Cv=null;this._disposeObjects(a);}
});}
)();
