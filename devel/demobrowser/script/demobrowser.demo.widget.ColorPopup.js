...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.ColorPopup.16729fd46ca7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="__oH",b="pointerup",c="Boolean",d="losecapture",f="__oI",g="move",h="qx.ui.core.MMovable",i="The move handle could not be redefined!",j="pointerdown",k="maximized",l="pointermove",m="roll",n="move-frame";qx.Mixin.define(h,{properties:{movable:{check:c,init:true},useMoveFrame:{check:c,init:false}},members:{__oH:null,__oI:null,__oJ:null,__oK:null,__oL:null,__oM:null,__oN:null,__oO:false,__oP:null,__oQ:0,_activateMoveHandle:function(o){if(this.__oH){throw new Error(i);}
;this.__oH=o;o.addListener(j,this._onMovePointerDown,this);o.addListener(b,this._onMovePointerUp,this);o.addListener(l,this._onMovePointerMove,this);o.addListener(d,this.__oU,this);}
,__oR:function(){var p=this.__oI;if(!p){p=this.__oI=new qx.ui.core.Widget();p.setAppearance(n);p.exclude();qx.core.Init.getApplication().getRoot().add(p);}
;return p;}
,__oS:function(){var location=this.getContentLocation();var r=this.getBounds();var q=this.__oR();q.setUserBounds(location.left,location.top,r.width,r.height);q.show();q.setZIndex(this.getZIndex()+1);}
,__oT:function(e){var t=this.__oJ;var s=Math.max(t.left,Math.min(t.right,e.getDocumentLeft()));var v=Math.max(t.top,Math.min(t.bottom,e.getDocumentTop()));var u=this.__oK+s;var w=this.__oL+v;return {viewportLeft:parseInt(u,10),viewportTop:parseInt(w,10),parentLeft:parseInt(u-this.__oM,10),parentTop:parseInt(w-this.__oN,10)};}
,_onMoveRoll:function(e){e.stop();}
,_onMovePointerDown:function(e){if(!this.getMovable()||this.hasState(k)){return;}
;this.addListener(m,this._onMoveRoll,this);var parent=this.getLayoutParent();var x=parent.getContentLocation();var z=parent.getBounds();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isBlocked()){this.__oP=parent.getBlockerColor();this.__oQ=parent.getBlockerOpacity();parent.setBlockerColor(null);parent.setBlockerOpacity(1);parent.blockContent(this.getZIndex()-1);this.__oO=true;}
;}
;this.__oJ={left:x.left,top:x.top,right:x.left+z.width,bottom:x.top+z.height};var y=this.getContentLocation();this.__oM=x.left;this.__oN=x.top;this.__oK=y.left-e.getDocumentLeft();this.__oL=y.top-e.getDocumentTop();this.addState(g);this.__oH.capture();if(this.getUseMoveFrame()){this.__oS();}
;e.stop();}
,_onMovePointerMove:function(e){if(!this.hasState(g)){return;}
;var B=this.__oT(e);if(this.getUseMoveFrame()){this.__oR().setDomPosition(B.viewportLeft,B.viewportTop);}
...
;this.removeState(g);var parent=this.getLayoutParent();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__oO){parent.unblock();parent.setBlockerColor(this.__oP);parent.setBlockerOpacity(this.__oQ);this.__oP=null;this.__oQ=0;this.__oO=false;}
;}
;this.__oH.releaseCapture();var D=this.__oT(e);var C=this.getLayoutParent().getInsets();this.setLayoutProperties({left:D.parentLeft-(C.left||0),top:D.parentTop-(C.top||0)});if(this.getUseMoveFrame()){this.__oR().exclude();}
;e.stopPropagation();}
,__oU:function(e){if(!this.hasState(g)){return;}
;this.removeState(g);if(this.getUseMoveFrame()){this.__oR().exclude();}
;}
},destruct:function(){this._disposeObjects(f,a);this.__oJ=null;}
});}
)();
(function(){var a="Boolean",b="w-resize",c="touch",d="sw-resize",f="n-resize",g="resizableRight",h="ne-resize",i="se-resize",j="Integer",k="e-resize",l="resizableLeft",m="move",n="shorthand",o="maximized",p="resize",q="nw-resize",r="pointerup",s="pointerdown",t="qx.ui.core.MResizable",u="losecapture",v="resize-frame",w="resizableBottom",x="s-resize",y="pointermove",z="resizableTop",A="pointerout";qx.Mixin.define(t,{construct:function(){var content=this.getContentElement();content.addListener(s,this.__pl,this,true);content.addListener(r,this.__pm,this);content.addListener(y,this.__po,this);content.addListener(A,this.__pp,this);content.addListener(u,this.__pn,this);var B=content.getDomElement();if(B==null){B=window;}
;this.__oY=qx.event.Registration.getManager(B).getHandler(qx.event.handler.DragDrop);}
,properties:{resizableTop:{check:a,init:true},resizableRight:{check:a,init:true},resizableBottom:{check:a,init:true},resizableLeft:{check:a,init:true},resizable:{group:[z,g,w,l],mode:n},resizeSensitivity:{check:j,init:5},useResizeFrame:{check:a,init:true}},members:{__oY:null,__pa:null,__pb:null,__pc:null,__pd:null,__pe:null,__pf:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__pa;if(!C){C=this.__pa=new qx.ui.core.Widget();C.setAppearance(v);C.exclude();qx.core.Init.getApplication().getRoot().add(C);}
;return C;}
,__pg:function(){var location=this.getContentLocation();var D=this._getResizeFrame();D.setUserBounds(location.left,location.top,location.right-location.left,location.bottom-location.top);D.show();D.setZIndex(this.getZIndex()+1);}
,__ph:function(e){var F=this.__pb;var E=this.getSizeHint();var I=this.__pf;var H=this.__pe;var J=H.width;var G=H.height;var L=H.left;var top=H.top;var K;if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){K=Math.max(I.top,Math.min(I.bottom,e.getDocumentTop()))-this.__pd;if(F&this.RESIZE_TOP){G-=K;}
else {G+=K;}
;if(G<E.minHeight){G=E.minHeight;}
else if(G>E.maxHeight){G=E.maxHeight;}
;if(F&this.RESIZE_TOP){top+=H.height-G;}
;}
;if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){K=Math.max(I.left,Math.min(I.right,e.getDocumentLeft()))-this.__pc;if(F&this.RESIZE_LEFT){J-=K;}
else {J+=K;}
...
,__pi:{'1':f,'2':x,'4':b,'8':k,'5':q,'6':d,'9':h,'10':i},__pj:function(e){var location=this.getContentLocation();var O=this.getResizeSensitivity();var N=e.getDocumentLeft();var P=e.getDocumentTop();var M=this.__pk(location,N,P,O);if(M>0){M=M|this.__pk(location,N,P,O*2);}
;this.__pb=M;}
,__pk:function(location,T,R,S){var Q=0;if(this.getResizableTop()&&Math.abs(location.top-R)<S&&T>location.left-S&&T<location.right+S){Q+=this.RESIZE_TOP;}
...
;return Q;}
,__pl:function(e){if(!this.__pb||!this.getEnabled()||e.getPointerType()==c){return;}
;this.addState(p);this.__pc=e.getDocumentLeft();this.__pd=e.getDocumentTop();var location=this.getContentLocation();var V=this.getBounds();this.__pe={top:location.top,left:location.left,width:location.right-location.left,height:location.bottom-location.top,bounds:qx.lang.Object.clone(V)};var parent=this.getLayoutParent();var W=parent.getContentLocation();var U=parent.getBounds();this.__pf={left:W.left,top:W.top,right:W.left+U.width,bottom:W.top+U.height};if(this.getUseResizeFrame()){this.__pg();}
;this.capture();e.stop();}
,__pm:function(e){if(!this.hasState(p)||!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.getUseResizeFrame()){this._getResizeFrame().exclude();}
;var X=this.__ph(e);this.setWidth(X.width);this.setHeight(X.height);if(this.getResizableLeft()||this.getResizableTop()){this.setLayoutProperties({left:X.parentLeft,top:X.parentTop});}
;this.__pb=0;this.removeState(p);this.resetCursor();this.getApplicationRoot().resetGlobalCursor();this.releaseCapture();e.stopPropagation();}
,__pn:function(e){if(!this.__pb){return;}
...
;}
,__po:function(e){if(!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.hasState(p)){var ba=this.__ph(e);if(this.getUseResizeFrame()){var Y=this._getResizeFrame();Y.setUserBounds(ba.viewportLeft,ba.viewportTop,ba.width,ba.height);}
...
;}
;e.stopPropagation();}
else if(!this.hasState(o)&&!this.__oY.isSessionActive()){this.__pj(e);var bd=this.__pb;var bc=this.getApplicationRoot();if(bd){var bb=this.__pi[bd];this.setCursor(bb);bc.setGlobalCursor(bb);}
else if(this.getCursor()){this.resetCursor();bc.resetGlobalCursor();}
;}
;}
,__pp:function(e){if(e.getPointerType()==c){return;}
;if(this.getCursor()&&!this.hasState(p)){this.resetCursor();this.getApplicationRoot().resetGlobalCursor();}
;}
},destruct:function(){if(this.__pa!=null&&!qx.core.ObjectRegistry.inShutDown){this.__pa.destroy();this.__pa=null;}
;this.__oY=null;}
});}
)();
...
;var br=this.getLayoutProperties();this.__pr=br.left===undefined?0:br.left;this.__pq=br.top===undefined?0:br.top;this.setLayoutProperties({left:null,top:null,edge:0});this.addState(F);this._updateCaptionBar();this.fireEvent(Q);}
;}
;}
,minimize:function(){if(!this.isVisible()){return;}
;if(this.fireNonBubblingEvent(b,qx.event.type.Event,[false,true])){var bs=this.getLayoutProperties();this.__pr=bs.left===undefined?0:bs.left;this.__pq=bs.top===undefined?0:bs.top;this.removeState(F);this.hide();this.fireEvent(v);}
;}
...
;var bt=this.__pr;var top=this.__pq;this.setLayoutProperties({edge:null,left:bt,top:top});this.removeState(F);this._updateCaptionBar();this.fireEvent(S);}
;}
...
,setValue:function(da){var cY;if(da==null){this.__pT=true;cY=[255,255,255];}
else {cY=qx.util.ColorUtil.stringToRgb(da);this.__pT=false;}
;this.__yW=true;this.setRed(cY[0]);this.setGreen(cY[1]);this.__yW=false;this.setBlue(cY[2]);}
,getValue:function(){return this.__pT?null:qx.util.ColorUtil.rgbToHexString([this.getRed(),this.getGreen(),this.getBlue()]);}
,resetValue:function(){this.__pT=true;this.__yW=true;this.setRed(255);this.setGreen(255);this.__yW=false;this.setBlue(255);}
,__pV:function(){if(!this.__yW){this.__pT=false;this.fireDataEvent(f,this.getValue());}
;}
...
;switch(this.__yQ){case cK:case n:case bU:this._setHueFromRgb();};this._setPreviewFromRgb();this.__pV();if(this.__yQ===bU){this.__yQ=null;}
;}
,_applyGreen:function(de,dd){if(this.__yQ===null){this.__yQ=cq;}
;if(this.__yQ!==cK){this.getChildControl(O).setValue(de);}
;if(this.__yQ!==n){this._setHexFromRgb();}
;switch(this.__yQ){case cK:case n:case cq:this._setHueFromRgb();};this._setPreviewFromRgb();this.__pV();if(this.__yQ===cq){this.__yQ=null;}
;}
,_applyBlue:function(dg,df){if(this.__yQ===null){this.__yQ=d;}
;if(this.__yQ!==cK){this.getChildControl(cG).setValue(dg);}
;if(this.__yQ!==n){this._setHexFromRgb();}
;switch(this.__yQ){case cK:case n:case d:this._setHueFromRgb();};this._setPreviewFromRgb();this.__pV();if(this.__yQ===d){this.__yQ=null;}
;}
