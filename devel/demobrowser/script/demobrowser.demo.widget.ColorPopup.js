...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.ColorPopup.15764c8c19dd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var bn=this._tables[this.__yK].values;var bo=this.__yG[this.__yK];if(!bn){return;}
;var bp=this.getValue();if(!bp){return;}
;var bm=bn.indexOf(bp);if(bm!=-1){qx.lang.Array.removeAt(bn,bm);}
else if(bn.length==this.__yL){bn.shift();}
...
;this.__yI.setRed(bx);this.__yI.setGreen(bw);this.__yI.setBlue(bv);this.__yH.open();}
,_onColorSelectorOk:function(){var by=this.__yI;this.setValue(qx.util.ColorUtil.rgbToRgbString([by.getRed(),by.getGreen(),by.getBlue()]));this.__yH.close();}
,_onColorSelectorCancel:function(){this.__yH.close();}
...
;}
,_tables:{core:{label:qx.locale.Manager.tr(a),values:[n,k,g,J,w,F,U,T,I,o,b,d]},recent:{label:qx.locale.Manager.tr(Q),values:[]}}},destruct:function(){if(this.__yH){this.__yH.destroy();this.__yI.destroy();this.__yJ.destroy();}
;this._tables=this.__yG=null;}
});}
)();
...
(function(){var a="qx.ui.window.Manager",b="__ox";qx.Class.define(a,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__ox:null,setDesktop:function(c){this.__ox=c;this.updateStack();}
,getDesktop:function(){return this.__ox;}
,changeActiveWindow:function(d,e){if(d){this.bringToFront(d);d.setActive(true);}
;if(e){e.resetActive();}
;}
,_minZIndex:1e5,updateStack:function(){qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(){this.__ox.forceUnblock();var h=this.__ox.getWindows();var g=this._minZIndex;var m=g+h.length*2;var j=g+h.length*4;var k=null;for(var i=0,l=h.length;i<l;i++ ){var f=h[i];if(!f.isVisible()){continue;}
;k=k||f;if(f.isModal()){f.setZIndex(j);this.__ox.blockContent(j-1);j+=2;k=f;}
else if(f.isAlwaysOnTop()){f.setZIndex(m);m+=2;}
else {f.setZIndex(g);g+=2;}
;if(!k.isModal()&&f.isActive()||f.getZIndex()>k.getZIndex()){k=f;}
;}
;this.__ox.setActiveWindow(k);}
,bringToFront:function(o){var n=this.__ox.getWindows();var p=qx.lang.Array.remove(n,o);if(p){n.push(o);this.updateStack();}
;}
,sendToBack:function(r){var q=this.__ox.getWindows();var s=qx.lang.Array.remove(q,r);if(s){q.unshift(r);this.updateStack();}
;}
...
(function(){var a="Boolean",b="w-resize",c="touch",d="sw-resize",f="n-resize",g="resizableRight",h="ne-resize",i="se-resize",j="Integer",k="e-resize",l="resizableLeft",m="move",n="shorthand",o="maximized",p="resize",q="nw-resize",r="pointerup",s="pointerdown",t="qx.ui.core.MResizable",u="losecapture",v="resize-frame",w="resizableBottom",x="s-resize",y="pointermove",z="resizableTop",A="pointerout";qx.Mixin.define(t,{construct:function(){var content=this.getContentElement();content.addListener(s,this.__pc,this,true);content.addListener(r,this.__pd,this);content.addListener(y,this.__pf,this);content.addListener(A,this.__pg,this);content.addListener(u,this.__pe,this);var B=content.getDomElement();if(B==null){B=window;}
;this.__oP=qx.event.Registration.getManager(B).getHandler(qx.event.handler.DragDrop);}
,properties:{resizableTop:{check:a,init:true},resizableRight:{check:a,init:true},resizableBottom:{check:a,init:true},resizableLeft:{check:a,init:true},resizable:{group:[z,g,w,l],mode:n},resizeSensitivity:{check:j,init:5},useResizeFrame:{check:a,init:true}},members:{__oP:null,__oQ:null,__oR:null,__oS:null,__oT:null,__oU:null,__oV:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__oQ;if(!C){C=this.__oQ=new qx.ui.core.Widget();C.setAppearance(v);C.exclude();qx.core.Init.getApplication().getRoot().add(C);}
;return C;}
,__oW:function(){var location=this.getContentLocation();var D=this._getResizeFrame();D.setUserBounds(location.left,location.top,location.right-location.left,location.bottom-location.top);D.show();D.setZIndex(this.getZIndex()+1);}
,__oX:function(e){var F=this.__oR;var E=this.getSizeHint();var I=this.__oV;var H=this.__oU;var J=H.width;var G=H.height;var L=H.left;var top=H.top;var K;if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){K=Math.max(I.top,Math.min(I.bottom,e.getDocumentTop()))-this.__oT;if(F&this.RESIZE_TOP){G-=K;}
else {G+=K;}
;if(G<E.minHeight){G=E.minHeight;}
else if(G>E.maxHeight){G=E.maxHeight;}
;if(F&this.RESIZE_TOP){top+=H.height-G;}
;}
;if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){K=Math.max(I.left,Math.min(I.right,e.getDocumentLeft()))-this.__oS;if(F&this.RESIZE_LEFT){J-=K;}
else {J+=K;}
...
,__oY:{'1':f,'2':x,'4':b,'8':k,'5':q,'6':d,'9':h,'10':i},__pa:function(e){var location=this.getContentLocation();var O=this.getResizeSensitivity();var N=e.getDocumentLeft();var P=e.getDocumentTop();var M=this.__pb(location,N,P,O);if(M>0){M=M|this.__pb(location,N,P,O*2);}
;this.__oR=M;}
,__pb:function(location,T,R,S){var Q=0;if(this.getResizableTop()&&Math.abs(location.top-R)<S&&T>location.left-S&&T<location.right+S){Q+=this.RESIZE_TOP;}
...
;return Q;}
,__pc:function(e){if(!this.__oR||!this.getEnabled()||e.getPointerType()==c){return;}
;this.addState(p);this.__oS=e.getDocumentLeft();this.__oT=e.getDocumentTop();var location=this.getContentLocation();var V=this.getBounds();this.__oU={top:location.top,left:location.left,width:location.right-location.left,height:location.bottom-location.top,bounds:qx.lang.Object.clone(V)};var parent=this.getLayoutParent();var W=parent.getContentLocation();var U=parent.getBounds();this.__oV={left:W.left,top:W.top,right:W.left+U.width,bottom:W.top+U.height};if(this.getUseResizeFrame()){this.__oW();}
;this.capture();e.stop();}
,__pd:function(e){if(!this.hasState(p)||!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.getUseResizeFrame()){this._getResizeFrame().exclude();}
;var X=this.__oX(e);this.setWidth(X.width);this.setHeight(X.height);if(this.getResizableLeft()||this.getResizableTop()){this.setLayoutProperties({left:X.parentLeft,top:X.parentTop});}
;this.__oR=0;this.removeState(p);this.resetCursor();this.getApplicationRoot().resetGlobalCursor();this.releaseCapture();e.stopPropagation();}
,__pe:function(e){if(!this.__oR){return;}
...
;}
,__pf:function(e){if(!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.hasState(p)){var ba=this.__oX(e);if(this.getUseResizeFrame()){var Y=this._getResizeFrame();Y.setUserBounds(ba.viewportLeft,ba.viewportTop,ba.width,ba.height);}
...
;}
;e.stopPropagation();}
else if(!this.hasState(o)&&!this.__oP.isSessionActive()){this.__pa(e);var bd=this.__oR;var bc=this.getApplicationRoot();if(bd){var bb=this.__oY[bd];this.setCursor(bb);bc.setGlobalCursor(bb);}
else if(this.getCursor()){this.resetCursor();bc.resetGlobalCursor();}
;}
;}
,__pg:function(e){if(e.getPointerType()==c){return;}
;if(this.getCursor()&&!this.hasState(p)){this.resetCursor();this.getApplicationRoot().resetGlobalCursor();}
;}
},destruct:function(){if(this.__oQ!=null&&!qx.core.ObjectRegistry.inShutDown){this.__oQ.destroy();this.__oQ=null;}
;this.__oP=null;}
});}
)();
...
;var br=this.getLayoutProperties();this.__pi=br.left===undefined?0:br.left;this.__ph=br.top===undefined?0:br.top;this.setLayoutProperties({left:null,top:null,edge:0});this.addState(F);this._updateCaptionBar();this.fireEvent(Q);}
;}
;}
,minimize:function(){if(!this.isVisible()){return;}
;if(this.fireNonBubblingEvent(b,qx.event.type.Event,[false,true])){var bs=this.getLayoutProperties();this.__pi=bs.left===undefined?0:bs.left;this.__ph=bs.top===undefined?0:bs.top;this.removeState(F);this.hide();this.fireEvent(v);}
;}
...
;var bt=this.__pi;var top=this.__ph;this.setLayoutProperties({edge:null,left:bt,top:top});this.removeState(F);this._updateCaptionBar();this.fireEvent(S);}
;}
...
,setValue:function(da){var cY;if(da==null){this.__pK=true;cY=[255,255,255];}
else {cY=qx.util.ColorUtil.stringToRgb(da);this.__pK=false;}
;this.__yS=true;this.setRed(cY[0]);this.setGreen(cY[1]);this.__yS=false;this.setBlue(cY[2]);}
,getValue:function(){return this.__pK?null:qx.util.ColorUtil.rgbToHexString([this.getRed(),this.getGreen(),this.getBlue()]);}
,resetValue:function(){this.__pK=true;this.__yS=true;this.setRed(255);this.setGreen(255);this.__yS=false;this.setBlue(255);}
,__pM:function(){if(!this.__yS){this.__pK=false;this.fireDataEvent(f,this.getValue());}
;}
,_applyRed:function(dc,db){if(this.__yM===null){this.__yM=bU;}
;if(this.__yM!==cK){this.getChildControl(m).setValue(dc);}
;if(this.__yM!==n){this._setHexFromRgb();}
;switch(this.__yM){case cK:case n:case bU:this._setHueFromRgb();};this._setPreviewFromRgb();this.__pM();if(this.__yM===bU){this.__yM=null;}
;}
,_applyGreen:function(de,dd){if(this.__yM===null){this.__yM=cq;}
;if(this.__yM!==cK){this.getChildControl(O).setValue(de);}
;if(this.__yM!==n){this._setHexFromRgb();}
;switch(this.__yM){case cK:case n:case cq:this._setHueFromRgb();};this._setPreviewFromRgb();this.__pM();if(this.__yM===cq){this.__yM=null;}
;}
,_applyBlue:function(dg,df){if(this.__yM===null){this.__yM=d;}
;if(this.__yM!==cK){this.getChildControl(cG).setValue(dg);}
;if(this.__yM!==n){this._setHexFromRgb();}
;switch(this.__yM){case cK:case n:case d:this._setHueFromRgb();};this._setPreviewFromRgb();this.__pM();if(this.__yM===d){this.__yM=null;}
;}
,_applyHue:function(di,dh){if(this.__yM===null){this.__yM=cr;}
;if(this.__yM!==F){this.getChildControl(cu).setValue(di);}
;if(this.__yM!==R){if(this.getChildControl(V).getBounds()){this.getChildControl(V).setDomLeft(Math.round(di/1.40625)+this.getChildControl(cc).getPaddingLeft());}
else {this.getChildControl(V).setLayoutProperties({left:Math.round(di/1.40625)});}
;}
;switch(this.__yM){case F:case R:case cr:this._setRgbFromHue();};this._setBrightnessGradiant();if(this.__yM===cr){this.__yM=null;}
;}
,_applySaturation:function(dk,dj){if(this.__yM===null){this.__yM=s;}
;if(this.__yM!==F){this.getChildControl(cx).setValue(dk);}
;if(this.__yM!==R){this._setBrightnessGradiant();if(this.getChildControl(V).getBounds()){this.getChildControl(V).setDomTop(256-Math.round(dk*2.56)+this.getChildControl(cc).getPaddingTop());}
else {this.getChildControl(V).setLayoutProperties({top:256-Math.round(dk*2.56)});}
;}
;switch(this.__yM){case F:case R:case s:this._setRgbFromHue();};if(this.__yM===s){this.__yM=null;}
;}
,_applyBrightness:function(dn,dm){if(this.__yM===null){this.__yM=c;}
;if(this.__yM!==F){this.getChildControl(bT).setValue(dn);}
;if(this.__yM!==P){var dl=256-Math.round(dn*2.56);if(this.getChildControl(cM).getBounds()){this.getChildControl(cM).setDomTop(dl+this.getChildControl(cE).getPaddingTop());}
else {this.getChildControl(cM).setLayoutProperties({top:dl});}
;}
;switch(this.__yM){case F:case P:case c:this._setRgbFromHue();};if(this.__yM===c){this.__yM=null;}
;}
,_onBrightnessHandlePointerDown:function(e){this.getChildControl(cM).capture();this.__yO=cM;var dr=this.getChildControl(cf).getContentLocation();var dq=this.getChildControl(cM).getContentLocation();var dp=this.getChildControl(cf).getBounds();this.__yP=dr.top+(e.getDocumentTop()-dq.top)-dp.top;e.stopPropagation();}
,_onBrightnessHandlePointerUp:function(e){this.getChildControl(cM).releaseCapture();this.__yO=null;}
,_onBrightnessHandlePointerMove:function(e){if(this.__yO===cM){this._setBrightnessOnFieldEvent(e);e.stopPropagation();}
;}
,_onBrightnessFieldPointerDown:function(e){var location=this.getChildControl(cf).getContentLocation();var ds=this.getChildControl(cM).getBounds();this.__yP=location.top+(ds.height/2);this._setBrightnessOnFieldEvent(e);this.getChildControl(cM).capture();this.__yO=cM;}
...
,_setBrightnessOnFieldEvent:function(e){var dt=qx.lang.Number.limit(e.getDocumentTop()-this.__yP,0,256);this.__yM=P;if(this.getChildControl(cM).getBounds()){this.getChildControl(cM).setDomTop(dt);}
else {this.getChildControl(cM).setLayoutProperties({top:dt});}
;this.setBrightness(100-Math.round(dt/2.56));this.__yM=null;}
,_onHueSaturationHandlePointerUp:function(e){if(this.__yO){e.stopPropagation();this.getChildControl(V).releaseCapture();this.__yO=null;}
;}
,_onHueSaturationHandlePointerMove:function(e){if(this.__yO===V){this._setHueSaturationOnFieldEvent(e);e.stopPropagation();}
;}
,_onHueSaturationFieldPointerDown:function(e){var location=this.getChildControl(ca).getContentLocation();var du=this.getChildControl(V).getBounds();var dv=this.getChildControl(ca).getBounds();this.__yQ=location.top+(du.height/2)-dv.top;this.__yR=location.left+(du.width/2)-dv.left;this._setHueSaturationOnFieldEvent(e);this.getChildControl(V).capture();this.__yO=V;}
...
,_setHueSaturationOnFieldEvent:function(e){var dy=qx.lang.Number.limit(e.getDocumentTop()-this.__yQ,0,256);var dx=qx.lang.Number.limit(e.getDocumentLeft()-this.__yR,0,256);this.getChildControl(V).setDomPosition(dx,dy);this.__yM=R;this.setSaturation(100-Math.round(dy/2.56));this.setHue(Math.round(dx*1.40625));this.__yM=null;}
,_setRedFromSpinner:function(){if(this.__yM!==null){return;}
;this.__yM=cK;this.setRed(this.getChildControl(m).getValue());this.__yM=null;}
,_setGreenFromSpinner:function(){if(this.__yM!==null){return;}
;this.__yM=cK;this.setGreen(this.getChildControl(O).getValue());this.__yM=null;}
,_setBlueFromSpinner:function(){if(this.__yM!==null){return;}
;this.__yM=cK;this.setBlue(this.getChildControl(cG).getValue());this.__yM=null;}
,_setHueFromSpinner:function(){if(this.__yM!==null){return;}
;this.__yM=F;this.setHue(this.getChildControl(cu).getValue());this.__yM=null;}
,_setSaturationFromSpinner:function(){if(this.__yM!==null){return;}
;this.__yM=F;this.setSaturation(this.getChildControl(cx).getValue());this.__yM=null;}
,_setBrightnessFromSpinner:function(){if(this.__yM!==null){return;}
;this.__yM=F;this.setBrightness(this.getChildControl(bT).getValue());this.__yM=null;}
,_onHexFieldChange:function(e){if(this.__yM!==null){return;}
;try{var dA=this.getChildControl(cD);var dz=qx.util.ColorUtil.hexStringToRgb(J+dA.getValue());}
catch(dB){return;}
;this.__yM=n;this.setRed(dz[0]);this.setGreen(dz[1]);this.setBlue(dz[2]);this.__yM=null;}
...
,_setHueFromRgb:function(){switch(this.__yM){case F:case R:case P:break;default:var dF=qx.util.ColorUtil.rgbToHsb([this.getRed(),this.getGreen(),this.getBlue()]);this.setHue(dF[0]);this.setSaturation(dF[1]);this.setBrightness(dF[2]);};}
,_setRgbFromHue:function(){switch(this.__yM){case cK:case n:break;default:var dG=qx.util.ColorUtil.hsbToRgb([this.getHue(),this.getSaturation(),this.getBrightness()]);this.setRed(dG[0]);this.setGreen(dG[1]);this.setBlue(dG[2]);};}
