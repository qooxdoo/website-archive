...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.ColorPopup.b820743299a7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="qx.ui.window.Manager",b="__oH";qx.Class.define(a,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__oH:null,setDesktop:function(c){this.__oH=c;this.updateStack();}
...
,syncWidget:function(){this.__oH.forceUnblock();var h=this.__oH.getWindows();var g=this._minZIndex;var m=g+h.length*2;var j=g+h.length*4;var k=null;for(var i=0,l=h.length;i<l;i++ ){var f=h[i];if(!f.isVisible()){continue;}
;k=k||f;if(f.isModal()){f.setZIndex(j);this.__oH.blockContent(j-1);j+=2;k=f;}
...
,bringToFront:function(o){var n=this.__oH.getWindows();var p=qx.lang.Array.remove(n,o);if(p){n.push(o);this.updateStack();}
;}
,sendToBack:function(r){var q=this.__oH.getWindows();var s=qx.lang.Array.remove(q,r);if(s){q.unshift(r);this.updateStack();}
;}
},destruct:function(){this._disposeObjects(b);}
});}
)();
(function(){var a="pointerup",b="Boolean",c="losecapture",d="__oI",f="move",g="qx.ui.core.MMovable",h="__oJ",i="The move handle could not be redefined!",j="pointerdown",k="maximized",l="pointermove",m="roll",n="move-frame";qx.Mixin.define(g,{properties:{movable:{check:b,init:true},useMoveFrame:{check:b,init:false}},members:{__oI:null,__oJ:null,__oK:null,__oL:null,__oM:null,__oN:null,__oO:null,__oP:false,__oQ:null,__oR:0,_activateMoveHandle:function(o){if(this.__oI){throw new Error(i);}
;this.__oI=o;o.addListener(j,this._onMovePointerDown,this);o.addListener(a,this._onMovePointerUp,this);o.addListener(l,this._onMovePointerMove,this);o.addListener(c,this.__oV,this);}
,__oS:function(){var p=this.__oJ;if(!p){p=this.__oJ=new qx.ui.core.Widget();p.setAppearance(n);p.exclude();qx.core.Init.getApplication().getRoot().add(p);}
;return p;}
,__oT:function(){var location=this.getContentLocation();var r=this.getBounds();var q=this.__oS();q.setUserBounds(location.left,location.top,r.width,r.height);q.show();q.setZIndex(this.getZIndex()+1);}
,__oU:function(e){var t=this.__oK;var s=Math.max(t.left,Math.min(t.right,e.getDocumentLeft()));var v=Math.max(t.top,Math.min(t.bottom,e.getDocumentTop()));var u=this.__oL+s;var w=this.__oM+v;return {viewportLeft:parseInt(u,10),viewportTop:parseInt(w,10),parentLeft:parseInt(u-this.__oN,10),parentTop:parseInt(w-this.__oO,10)};}
,_onMoveRoll:function(e){e.stop();}
,_onMovePointerDown:function(e){if(!this.getMovable()||this.hasState(k)){return;}
;this.addListener(m,this._onMoveRoll,this);var parent=this.getLayoutParent();var x=parent.getContentLocation();var z=parent.getBounds();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isBlocked()){this.__oQ=parent.getBlockerColor();this.__oR=parent.getBlockerOpacity();parent.setBlockerColor(null);parent.setBlockerOpacity(1);parent.blockContent(this.getZIndex()-1);this.__oP=true;}
;}
;this.__oK={left:x.left,top:x.top,right:x.left+z.width,bottom:x.top+z.height};var y=this.getContentLocation();this.__oN=x.left;this.__oO=x.top;this.__oL=y.left-e.getDocumentLeft();this.__oM=y.top-e.getDocumentTop();this.addState(f);this.__oI.capture();if(this.getUseMoveFrame()){this.__oT();}
;e.stop();}
,_onMovePointerMove:function(e){if(!this.hasState(f)){return;}
;var B=this.__oU(e);if(this.getUseMoveFrame()){this.__oS().setDomPosition(B.viewportLeft,B.viewportTop);}
...
;this.removeState(f);var parent=this.getLayoutParent();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__oP){parent.unblock();parent.setBlockerColor(this.__oQ);parent.setBlockerOpacity(this.__oR);this.__oQ=null;this.__oR=0;this.__oP=false;}
;}
;this.__oI.releaseCapture();var D=this.__oU(e);var C=this.getLayoutParent().getInsets();this.setLayoutProperties({left:D.parentLeft-(C.left||0),top:D.parentTop-(C.top||0)});if(this.getUseMoveFrame()){this.__oS().exclude();}
;e.stopPropagation();}
,__oV:function(e){if(!this.hasState(f)){return;}
;this.removeState(f);if(this.getUseMoveFrame()){this.__oS().exclude();}
;}
},destruct:function(){this._disposeObjects(h,d);this.__oK=null;}
});}
)();
(function(){var a="Boolean",b="w-resize",c="touch",d="sw-resize",f="n-resize",g="resizableRight",h="ne-resize",i="se-resize",j="Integer",k="e-resize",l="resizableLeft",m="move",n="shorthand",o="maximized",p="resize",q="nw-resize",r="pointerup",s="pointerdown",t="qx.ui.core.MResizable",u="losecapture",v="resize-frame",w="resizableBottom",x="s-resize",y="pointermove",z="resizableTop",A="pointerout";qx.Mixin.define(t,{construct:function(){var content=this.getContentElement();content.addListener(s,this.__pm,this,true);content.addListener(r,this.__pn,this);content.addListener(y,this.__pp,this);content.addListener(A,this.__pq,this);content.addListener(u,this.__po,this);var B=content.getDomElement();if(B==null){B=window;}
;this.__pa=qx.event.Registration.getManager(B).getHandler(qx.event.handler.DragDrop);}
,properties:{resizableTop:{check:a,init:true},resizableRight:{check:a,init:true},resizableBottom:{check:a,init:true},resizableLeft:{check:a,init:true},resizable:{group:[z,g,w,l],mode:n},resizeSensitivity:{check:j,init:5},useResizeFrame:{check:a,init:true}},members:{__pa:null,__pb:null,__pc:null,__pd:null,__pe:null,__pf:null,__pg:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__pb;if(!C){C=this.__pb=new qx.ui.core.Widget();C.setAppearance(v);C.exclude();qx.core.Init.getApplication().getRoot().add(C);}
;return C;}
,__ph:function(){var location=this.getContentLocation();var D=this._getResizeFrame();D.setUserBounds(location.left,location.top,location.right-location.left,location.bottom-location.top);D.show();D.setZIndex(this.getZIndex()+1);}
,__pi:function(e){var F=this.__pc;var E=this.getSizeHint();var I=this.__pg;var H=this.__pf;var J=H.width;var G=H.height;var L=H.left;var top=H.top;var K;if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){K=Math.max(I.top,Math.min(I.bottom,e.getDocumentTop()))-this.__pe;if(F&this.RESIZE_TOP){G-=K;}
else {G+=K;}
;if(G<E.minHeight){G=E.minHeight;}
else if(G>E.maxHeight){G=E.maxHeight;}
;if(F&this.RESIZE_TOP){top+=H.height-G;}
;}
;if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){K=Math.max(I.left,Math.min(I.right,e.getDocumentLeft()))-this.__pd;if(F&this.RESIZE_LEFT){J-=K;}
else {J+=K;}
...
,__pj:{'1':f,'2':x,'4':b,'8':k,'5':q,'6':d,'9':h,'10':i},__pk:function(e){var location=this.getContentLocation();var O=this.getResizeSensitivity();var N=e.getDocumentLeft();var P=e.getDocumentTop();var M=this.__pl(location,N,P,O);if(M>0){M=M|this.__pl(location,N,P,O*2);}
;this.__pc=M;}
,__pl:function(location,T,R,S){var Q=0;if(this.getResizableTop()&&Math.abs(location.top-R)<S&&T>location.left-S&&T<location.right+S){Q+=this.RESIZE_TOP;}
...
;return Q;}
,__pm:function(e){if(!this.__pc||!this.getEnabled()||e.getPointerType()==c){return;}
;this.addState(p);this.__pd=e.getDocumentLeft();this.__pe=e.getDocumentTop();var location=this.getContentLocation();var V=this.getBounds();this.__pf={top:location.top,left:location.left,width:location.right-location.left,height:location.bottom-location.top,bounds:qx.lang.Object.clone(V)};var parent=this.getLayoutParent();var W=parent.getContentLocation();var U=parent.getBounds();this.__pg={left:W.left,top:W.top,right:W.left+U.width,bottom:W.top+U.height};if(this.getUseResizeFrame()){this.__ph();}
;this.capture();e.stop();}
,__pn:function(e){if(!this.hasState(p)||!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.getUseResizeFrame()){this._getResizeFrame().exclude();}
;var X=this.__pi(e);this.setWidth(X.width);this.setHeight(X.height);if(this.getResizableLeft()||this.getResizableTop()){this.setLayoutProperties({left:X.parentLeft,top:X.parentTop});}
;this.__pc=0;this.removeState(p);this.resetCursor();this.getApplicationRoot().resetGlobalCursor();this.releaseCapture();e.stopPropagation();}
,__po:function(e){if(!this.__pc){return;}
...
;}
,__pp:function(e){if(!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.hasState(p)){var ba=this.__pi(e);if(this.getUseResizeFrame()){var Y=this._getResizeFrame();Y.setUserBounds(ba.viewportLeft,ba.viewportTop,ba.width,ba.height);}
...
;}
;e.stopPropagation();}
else if(!this.hasState(o)&&!this.__pa.isSessionActive()){this.__pk(e);var bd=this.__pc;var bc=this.getApplicationRoot();if(bd){var bb=this.__pj[bd];this.setCursor(bb);bc.setGlobalCursor(bb);}
else if(this.getCursor()){this.resetCursor();bc.resetGlobalCursor();}
;}
;}
,__pq:function(e){if(e.getPointerType()==c){return;}
;if(this.getCursor()&&!this.hasState(p)){this.resetCursor();this.getApplicationRoot().resetGlobalCursor();}
;}
},destruct:function(){if(this.__pb!=null&&!qx.core.ObjectRegistry.inShutDown){this.__pb.destroy();this.__pb=null;}
;this.__pa=null;}
});}
)();
...
;var br=this.getLayoutProperties();this.__ps=br.left===undefined?0:br.left;this.__pr=br.top===undefined?0:br.top;this.setLayoutProperties({left:null,top:null,edge:0});this.addState(F);this._updateCaptionBar();this.fireEvent(Q);}
;}
;}
,minimize:function(){if(!this.isVisible()){return;}
;if(this.fireNonBubblingEvent(b,qx.event.type.Event,[false,true])){var bs=this.getLayoutProperties();this.__ps=bs.left===undefined?0:bs.left;this.__pr=bs.top===undefined?0:bs.top;this.removeState(F);this.hide();this.fireEvent(v);}
;}
...
;var bt=this.__ps;var top=this.__pr;this.setLayoutProperties({edge:null,left:bt,top:top});this.removeState(F);this._updateCaptionBar();this.fireEvent(S);}
;}
...
,setValue:function(da){var cY;if(da==null){this.__pU=true;cY=[255,255,255];}
else {cY=qx.util.ColorUtil.stringToRgb(da);this.__pU=false;}
;this.__yW=true;this.setRed(cY[0]);this.setGreen(cY[1]);this.__yW=false;this.setBlue(cY[2]);}
,getValue:function(){return this.__pU?null:qx.util.ColorUtil.rgbToHexString([this.getRed(),this.getGreen(),this.getBlue()]);}
,resetValue:function(){this.__pU=true;this.__yW=true;this.setRed(255);this.setGreen(255);this.__yW=false;this.setBlue(255);}
,__pW:function(){if(!this.__yW){this.__pU=false;this.fireDataEvent(f,this.getValue());}
;}
...
;switch(this.__yQ){case cK:case n:case bU:this._setHueFromRgb();};this._setPreviewFromRgb();this.__pW();if(this.__yQ===bU){this.__yQ=null;}
;}
,_applyGreen:function(de,dd){if(this.__yQ===null){this.__yQ=cq;}
;if(this.__yQ!==cK){this.getChildControl(O).setValue(de);}
;if(this.__yQ!==n){this._setHexFromRgb();}
;switch(this.__yQ){case cK:case n:case cq:this._setHueFromRgb();};this._setPreviewFromRgb();this.__pW();if(this.__yQ===cq){this.__yQ=null;}
;}
,_applyBlue:function(dg,df){if(this.__yQ===null){this.__yQ=d;}
;if(this.__yQ!==cK){this.getChildControl(cG).setValue(dg);}
;if(this.__yQ!==n){this._setHexFromRgb();}
;switch(this.__yQ){case cK:case n:case d:this._setHueFromRgb();};this._setPreviewFromRgb();this.__pW();if(this.__yQ===d){this.__yQ=null;}
;}
