...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ModelDebugging.f30edc06a6b6.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var M=this.__BY(),L=this.__BS,N=this;if(this.timeout>0){this.__dj=window.setTimeout(this.__sn,this.timeout);}
;if(this.__Cb(b)){qx.Bootstrap.debug(qx.bom.request.Script,z);}
...
;this.__sq=true;this.__Ca();this._emit(m);return this;}
...
;if(this.__Cb(h)){qx.Bootstrap.debug(q+r);}
;return t;}
,getAllResponseHeaders:function(){if(this.__sH){return null;}
;if(this.__Cb(h)){qx.Bootstrap.debug(v+r);}
;return k;}
,setDetermineSuccess:function(P){this.__BT=P;}
,dispose:function(){var Q=this.__BU;if(!this.__sH){if(Q){Q.onload=Q.onreadystatechange=null;this.__Ca();}
;if(this.__dj){window.clearTimeout(this.__dj);}
;this.__sH=true;}
;}
,isDisposed:function(){return !!this.__sH;}
,_getUrl:function(){return this.__sC;}
,_getScriptElement:function(){return this.__BU;}
,_onTimeout:function(){this.__BW();if(!this.__BX()){this._emit(f);}
;this._emit(B);if(!this.__BX()){this._emit(g);}
;}
,_onNativeLoad:function(){var S=this.__BU,R=this.__BT,T=this;if(this.__sq){return;}
;if(this.__Cb(x)===w&&this.__Cb(l)<9){if(!(/loaded|complete/).test(S.readyState)){return;}
else {if(this.__Cb(b)){qx.Bootstrap.debug(qx.bom.request.Script,o);}
;}
;}
;if(this.__Cb(b)){qx.Bootstrap.debug(qx.bom.request.Script,D);}
;if(R){if(!this.status){this.status=R()?200:500;}
;}
;if(this.status===500){if(this.__Cb(b)){qx.Bootstrap.debug(qx.bom.request.Script,y);}
;}
;if(this.__dj){window.clearTimeout(this.__dj);}
;window.setTimeout(function(){T._success();T._readyStateChange(4);T._emit(j);T._emit(g);}
);}
,_onNativeError:function(){this.__BW();this._emit(f);this._emit(g);}
,__BU:null,__BS:null,__sC:n,__BQ:null,__BR:null,__sn:null,__dj:null,__sq:null,__sH:null,__BV:function(){this.readyState=0;this.status=0;this.statusText=n;}
,_readyStateChange:function(U){this.readyState=U;this._emit(p);}
,_success:function(){this.__Ca();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=n+this.status;}
,__BW:function(){this.__Ca();this.readyState=4;this.status=0;this.statusText=null;}
,__BX:function(){var W=this.__Cb(x)===w&&this.__Cb(l)<9;var V=this.__Cb(x)===s;return !(W||V);}
,__BY:function(){var X=this.__BU=document.createElement(i);X.src=this.__sC;X.onerror=this.__BR;X.onload=this.__BQ;if(this.__Cb(x)===w&&this.__Cb(l)<9){X.onreadystatechange=this.__BQ;}
;return X;}
,__Ca:function(){var Y=this.__BU;if(Y&&Y.parentNode){this.__BS.removeChild(Y);}
;}
,__Cb:function(ba){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(ba);}
...
(function(){var a="qx.bom.request.Jsonp",b="callback",c="open",d="dispose",e="",f="qx",g="qx.debug",h="Expecting JavaScript response to call: ",i="_onNativeLoad",j=" already exists",k="Callback ",l="qx.debug.io",m=".callback",n="qx.bom.request.Jsonp.";qx.Bootstrap.define(a,{extend:qx.bom.request.Script,construct:function(){qx.bom.request.Script.apply(this);this.__Cj();}
,members:{responseJson:null,__cj:null,__Cc:null,__Cd:null,__Ce:null,__Cf:null,__Cg:null,__sH:null,__Ch:e,open:function(t,p){if(this.__sH){return;}
;var r={},q,s,o=this;this.responseJson=null;this.__Ce=false;q=this.__Cc||b;s=this.__Cd||this.__Ch+n+this.__cj+m;if(!this.__Cd){this.constructor[this.__cj]=this;}
else {if(!window[this.__Cd]){this.__Cf=true;window[this.__Cd]=function(u){o.callback(u);}
;}
else {if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,k+this.__Cd+j);}
;}
;}
;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,h+s);}
;r[q]=s;this.__Cg=p=qx.util.Uri.appendParamsToUrl(p,r);this.__hH(c,[t,p]);}
,callback:function(v){if(this.__sH){return;}
;this.__Ce=true;if(qx.core.Environment.get(g)){v=qx.lang.Json.stringify(v);v=qx.lang.Json.parse(v);}
;this.responseJson=v;this.constructor[this.__cj]=undefined;this.__Ci();}
,setCallbackParam:function(w){this.__Cc=w;return this;}
,setCallbackName:function(name){this.__Cd=name;return this;}
,setPrefix:function(x){this.__Ch=x;}
,getGeneratedUrl:function(){return this.__Cg;}
,dispose:function(){this.__Ci();this.__hH(d);}
,_onNativeLoad:function(){this.status=this.__Ce?200:500;this.__hH(i);}
,__Ci:function(){if(this.__Cf&&window[this.__Cd]){window[this.__Cd]=undefined;this.__Cf=false;}
;}
,__hH:function(z,y){qx.bom.request.Script.prototype[z].apply(this,y||[]);}
,__Cj:function(){this.__cj=f+(new Date().valueOf())+(e+Math.random()).substring(2,5);}
}});}
)();
...
;return br;}
;return A;}
,startDisposeProfiling:qx.core.Environment.select(c,{"true":function(){this.disposeProfilingActive=true;this.__Cp=qx.core.ObjectRegistry.getNextHash();}
,"default":(function(){}
)}),stopDisposeProfiling:qx.core.Environment.select(c,{"true":function(bA){if(!this.__Cp){qx.log.Logger.error(s+this.classname+F);return [];}
...
;}
;var bB=qx.core.ObjectRegistry.getNextHash();var bD=qx.core.ObjectRegistry.getPostId();var bE=qx.core.ObjectRegistry.getStackTraces();for(var bC=this.__Cp;bC<bB;bC++ ){var by=qx.core.ObjectRegistry.fromHashCode(bC+bD);if(by&&by.isDisposed&&!by.isDisposed()){if(bA&&typeof bA==h&&!bA(by)){continue;}
