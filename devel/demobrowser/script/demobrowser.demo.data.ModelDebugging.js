...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ModelDebugging.dad04be0a3d7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var M=this.__va(),L=this.__uT,N=this;if(this.timeout>0){this.__di=window.setTimeout(this.__sh,this.timeout);}
;if(this.__vc(b)){qx.Bootstrap.debug(qx.bom.request.Script,z);}
...
;this.__sk=true;this.__vb();this._emit(m);return this;}
...
;if(this.__vc(h)){qx.Bootstrap.debug(q+r);}
;return t;}
,getAllResponseHeaders:function(){if(this.__sB){return null;}
;if(this.__vc(h)){qx.Bootstrap.debug(v+r);}
;return k;}
,setDetermineSuccess:function(P){this.__uU=P;}
,dispose:function(){var Q=this.__uV;if(!this.__sB){if(Q){Q.onload=Q.onreadystatechange=null;this.__vb();}
;if(this.__di){window.clearTimeout(this.__di);}
;this.__sB=true;}
;}
,isDisposed:function(){return !!this.__sB;}
,_getUrl:function(){return this.__sw;}
,_getScriptElement:function(){return this.__uV;}
,_onTimeout:function(){this.__uX();if(!this.__uY()){this._emit(f);}
;this._emit(B);if(!this.__uY()){this._emit(g);}
;}
,_onNativeLoad:function(){var S=this.__uV,R=this.__uU,T=this;if(this.__sk){return;}
;if(this.__vc(x)===w&&this.__vc(l)<9){if(!(/loaded|complete/).test(S.readyState)){return;}
else {if(this.__vc(b)){qx.Bootstrap.debug(qx.bom.request.Script,o);}
;}
;}
;if(this.__vc(b)){qx.Bootstrap.debug(qx.bom.request.Script,D);}
;if(R){if(!this.status){this.status=R()?200:500;}
;}
;if(this.status===500){if(this.__vc(b)){qx.Bootstrap.debug(qx.bom.request.Script,y);}
;}
;if(this.__di){window.clearTimeout(this.__di);}
;window.setTimeout(function(){T._success();T._readyStateChange(4);T._emit(j);T._emit(g);}
);}
,_onNativeError:function(){this.__uX();this._emit(f);this._emit(g);}
,__uV:null,__uT:null,__sw:n,__uR:null,__uS:null,__sh:null,__di:null,__sk:null,__sB:null,__uW:function(){this.readyState=0;this.status=0;this.statusText=n;}
,_readyStateChange:function(U){this.readyState=U;this._emit(p);}
,_success:function(){this.__vb();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=n+this.status;}
,__uX:function(){this.__vb();this.readyState=4;this.status=0;this.statusText=null;}
,__uY:function(){var W=this.__vc(x)===w&&this.__vc(l)<9;var V=this.__vc(x)===s;return !(W||V);}
,__va:function(){var X=this.__uV=document.createElement(i);X.src=this.__sw;X.onerror=this.__uS;X.onload=this.__uR;if(this.__vc(x)===w&&this.__vc(l)<9){X.onreadystatechange=this.__uR;}
;return X;}
,__vb:function(){var Y=this.__uV;if(Y&&Y.parentNode){this.__uT.removeChild(Y);}
;}
,__vc:function(ba){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(ba);}
...
(function(){var a="qx.bom.request.Jsonp",b="callback",c="open",d="dispose",e="",f="qx",g="qx.debug",h="Expecting JavaScript response to call: ",i="_onNativeLoad",j=" already exists",k="Callback ",l="qx.debug.io",m=".callback",n="qx.bom.request.Jsonp.";qx.Bootstrap.define(a,{extend:qx.bom.request.Script,construct:function(){qx.bom.request.Script.apply(this);this.__vk();}
,members:{responseJson:null,__ci:null,__vd:null,__ve:null,__vf:null,__vg:null,__vh:null,__sB:null,__vi:e,open:function(t,p){if(this.__sB){return;}
;var r={},q,s,o=this;this.responseJson=null;this.__vf=false;q=this.__vd||b;s=this.__ve||this.__vi+n+this.__ci+m;if(!this.__ve){this.constructor[this.__ci]=this;}
else {if(!window[this.__ve]){this.__vg=true;window[this.__ve]=function(u){o.callback(u);}
;}
else {if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,k+this.__ve+j);}
;}
;}
;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,h+s);}
;r[q]=s;this.__vh=p=qx.util.Uri.appendParamsToUrl(p,r);this.__hG(c,[t,p]);}
,callback:function(v){if(this.__sB){return;}
;this.__vf=true;if(qx.core.Environment.get(g)){v=qx.lang.Json.stringify(v);v=qx.lang.Json.parse(v);}
;this.responseJson=v;this.constructor[this.__ci]=undefined;this.__vj();}
,setCallbackParam:function(w){this.__vd=w;return this;}
,setCallbackName:function(name){this.__ve=name;return this;}
,setPrefix:function(x){this.__vi=x;}
,getGeneratedUrl:function(){return this.__vh;}
,dispose:function(){this.__vj();this.__hG(d);}
,_onNativeLoad:function(){this.status=this.__vf?200:500;this.__hG(i);}
,__vj:function(){if(this.__vg&&window[this.__ve]){window[this.__ve]=undefined;this.__vg=false;}
;}
,__hG:function(z,y){qx.bom.request.Script.prototype[z].apply(this,y||[]);}
,__vk:function(){this.__ci=f+(new Date().valueOf())+(e+Math.random()).substring(2,5);}
}});}
)();
...
;return br;}
;return A;}
,startDisposeProfiling:qx.core.Environment.select(c,{"true":function(){this.disposeProfilingActive=true;this.__vN=qx.core.ObjectRegistry.getNextHash();}
,"default":(function(){}
)}),stopDisposeProfiling:qx.core.Environment.select(c,{"true":function(bA){if(!this.__vN){qx.log.Logger.error(s+this.classname+F);return [];}
...
;}
;var bB=qx.core.ObjectRegistry.getNextHash();var bD=qx.core.ObjectRegistry.getPostId();var bE=qx.core.ObjectRegistry.getStackTraces();for(var bC=this.__vN;bC<bB;bC++ ){var by=qx.core.ObjectRegistry.fromHashCode(bC+bD);if(by&&by.isDisposed&&!by.isDisposed()){if(bA&&typeof bA==h&&!bA(by)){continue;}
