...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ModelDebugging.fd557cb9de5b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="url: ",b="qx.debug.io",c="qx.bom.request.Script",d="Invalid state",e="head",f="error",g="loadend",h="qx.debug",i="script",j="load",k="Unknown response headers",l="browser.documentmode",m="abort",n="",o="Received native readyState: loaded",p="readystatechange",q="Response header cannot be determined for ",r="requests made with script transport.",s="opera",t="unknown",u="Open native request with ",v="Response headers cannot be determined for",w="mshtml",x="engine.name",y="Detected error",z="Send native request",A="on",B="timeout",C="Unknown environment key at this phase",D="Received native load";qx.Bootstrap.define(c,{implement:[qx.core.IDisposable],construct:function(){this.__vb();this.__uV=qx.Bootstrap.bind(this._onNativeLoad,this);this.__uW=qx.Bootstrap.bind(this._onNativeError,this);this.__sl=qx.Bootstrap.bind(this._onTimeout,this);this.__uX=document.head||document.getElementsByTagName(e)[0]||document.documentElement;this._emitter=new qx.event.Emitter();this.timeout=this.__vd()?0:15000;}
,events:{"readystatechange":c,"error":c,"loadend":c,"timeout":c,"abort":c,"load":c},members:{readyState:null,status:null,statusText:null,timeout:null,__uY:null,on:function(name,E,F){this._emitter.on(name,E,F);return this;}
,open:function(H,G){if(this.__sF){return;}
;this.__vb();this.__so=null;this.__sA=G;if(this.__vg(b)){qx.Bootstrap.debug(qx.bom.request.Script,u+a+G);}
;this._readyStateChange(1);}
,setRequestHeader:function(I,J){if(this.__sF){return null;}
;var K={};if(this.readyState!==1){throw new Error(d);}
;K[I]=J;this.__sA=qx.util.Uri.appendParamsToUrl(this.__sA,K);return this;}
,send:function(){if(this.__sF){return null;}
;var M=this.__ve(),L=this.__uX,N=this;if(this.timeout>0){this.__dj=window.setTimeout(this.__sl,this.timeout);}
;if(this.__vg(b)){qx.Bootstrap.debug(qx.bom.request.Script,z);}
;L.insertBefore(M,L.firstChild);window.setTimeout(function(){N._readyStateChange(2);N._readyStateChange(3);}
);return this;}
,abort:function(){if(this.__sF){return null;}
;this.__so=true;this.__vf();this._emit(m);return this;}
...
,onabort:function(){}
,ontimeout:function(){}
,getResponseHeader:function(O){if(this.__sF){return null;}
;if(this.__vg(h)){qx.Bootstrap.debug(q+r);}
;return t;}
,getAllResponseHeaders:function(){if(this.__sF){return null;}
;if(this.__vg(h)){qx.Bootstrap.debug(v+r);}
;return k;}
,setDetermineSuccess:function(P){this.__uY=P;}
,dispose:function(){var Q=this.__va;if(!this.__sF){if(Q){Q.onload=Q.onreadystatechange=null;this.__vf();}
;if(this.__dj){window.clearTimeout(this.__dj);}
;this.__sF=true;}
;}
,isDisposed:function(){return !!this.__sF;}
,_getUrl:function(){return this.__sA;}
,_getScriptElement:function(){return this.__va;}
,_onTimeout:function(){this.__vc();if(!this.__vd()){this._emit(f);}
;this._emit(B);if(!this.__vd()){this._emit(g);}
;}
,_onNativeLoad:function(){var S=this.__va,R=this.__uY,T=this;if(this.__so){return;}
;if(this.__vg(x)===w&&this.__vg(l)<9){if(!(/loaded|complete/).test(S.readyState)){return;}
else {if(this.__vg(b)){qx.Bootstrap.debug(qx.bom.request.Script,o);}
;}
;}
;if(this.__vg(b)){qx.Bootstrap.debug(qx.bom.request.Script,D);}
;if(R){if(!this.status){this.status=R()?200:500;}
;}
;if(this.status===500){if(this.__vg(b)){qx.Bootstrap.debug(qx.bom.request.Script,y);}
;}
;if(this.__dj){window.clearTimeout(this.__dj);}
;window.setTimeout(function(){T._success();T._readyStateChange(4);T._emit(j);T._emit(g);}
);}
,_onNativeError:function(){this.__vc();this._emit(f);this._emit(g);}
,__va:null,__uX:null,__sA:n,__uV:null,__uW:null,__sl:null,__dj:null,__so:null,__sF:null,__vb:function(){this.readyState=0;this.status=0;this.statusText=n;}
,_readyStateChange:function(U){this.readyState=U;this._emit(p);}
,_success:function(){this.__vf();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=n+this.status;}
,__vc:function(){this.__vf();this.readyState=4;this.status=0;this.statusText=null;}
,__vd:function(){var W=this.__vg(x)===w&&this.__vg(l)<9;var V=this.__vg(x)===s;return !(W||V);}
,__ve:function(){var X=this.__va=document.createElement(i);X.src=this.__sA;X.onerror=this.__uW;X.onload=this.__uV;if(this.__vg(x)===w&&this.__vg(l)<9){X.onreadystatechange=this.__uV;}
;return X;}
,__vf:function(){var Y=this.__va;if(Y&&Y.parentNode){this.__uX.removeChild(Y);}
;}
,__vg:function(ba){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(ba);}
...
(function(){var a="qx.bom.request.Jsonp",b="callback",c="open",d="dispose",e="",f="qx",g="qx.debug",h="Expecting JavaScript response to call: ",i="_onNativeLoad",j=" already exists",k="Callback ",l="qx.debug.io",m=".callback",n="qx.bom.request.Jsonp.";qx.Bootstrap.define(a,{extend:qx.bom.request.Script,construct:function(){qx.bom.request.Script.apply(this);this.__vo();}
,members:{responseJson:null,__cj:null,__vh:null,__vi:null,__vj:null,__vk:null,__vl:null,__sF:null,__vm:e,open:function(t,p){if(this.__sF){return;}
;var r={},q,s,o=this;this.responseJson=null;this.__vj=false;q=this.__vh||b;s=this.__vi||this.__vm+n+this.__cj+m;if(!this.__vi){this.constructor[this.__cj]=this;}
else {if(!window[this.__vi]){this.__vk=true;window[this.__vi]=function(u){o.callback(u);}
;}
else {if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,k+this.__vi+j);}
;}
;}
;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,h+s);}
;r[q]=s;this.__vl=p=qx.util.Uri.appendParamsToUrl(p,r);this.__hH(c,[t,p]);}
,callback:function(v){if(this.__sF){return;}
;this.__vj=true;if(qx.core.Environment.get(g)){v=qx.lang.Json.stringify(v);v=qx.lang.Json.parse(v);}
;this.responseJson=v;this.constructor[this.__cj]=undefined;this.__vn();}
,setCallbackParam:function(w){this.__vh=w;return this;}
,setCallbackName:function(name){this.__vi=name;return this;}
,setPrefix:function(x){this.__vm=x;}
,getGeneratedUrl:function(){return this.__vl;}
,dispose:function(){this.__vn();this.__hH(d);}
,_onNativeLoad:function(){this.status=this.__vj?200:500;this.__hH(i);}
,__vn:function(){if(this.__vk&&window[this.__vi]){window[this.__vi]=undefined;this.__vk=false;}
;}
,__hH:function(z,y){qx.bom.request.Script.prototype[z].apply(this,y||[]);}
,__vo:function(){this.__cj=f+(new Date().valueOf())+(e+Math.random()).substring(2,5);}
}});}
)();
...
,"default":(function(){}
)}),stopDisposeProfiling:qx.core.Environment.select(c,{"true":function(bA){if(!this.__vR){qx.log.Logger.error(s+this.classname+F);return [];}
...
;}
;var bB=qx.core.ObjectRegistry.getNextHash();var bD=qx.core.ObjectRegistry.getPostId();var bE=qx.core.ObjectRegistry.getStackTraces();for(var bC=this.__vR;bC<bB;bC++ ){var by=qx.core.ObjectRegistry.fromHashCode(bC+bD);if(by&&by.isDisposed&&!by.isDisposed()){if(bA&&typeof bA==h&&!bA(by)){continue;}
