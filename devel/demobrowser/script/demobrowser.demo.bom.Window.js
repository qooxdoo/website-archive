...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Window.312bb4af11cf.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="no",b="px",c="qx.bom.Window",d="Cross-Domain Scripting problem: Could not resize window!",e="' is not supported for native windows.",f="interval",g="dialogWidth",h="resizable",i=":",j="yes",k="dialogTop",l="qxNativeWindow",m="Cross-Domain Scripting problem: Could not move window!",n="",o="Option '",p="javascript:/",q="dialogLeft",r="dialogHeight",s="' is not supported for modal windows.",t='load',u="scrollbars",v="=",w=";",x="scroll",y=",";qx.Class.define(c,{statics:{__mu:null,__tX:null,__dE:null,__tY:{"top":k,left:q,width:g,height:r,scrollbars:x,resizable:h},__ua:{"top":1,left:1,width:1,height:1,dependent:1,resizable:1,status:1,location:1,menubar:1,scrollbars:1,toolbar:1},__ub:function(){return window.showModalDialog!=null;}
,open:function(z,name,H,E,A,D,self){var C=null;if(z==null){z=p;}
;if(name==null){name=l+new Date().getTime();}
;if(A==null){A=true;}
;var G=this.__uc(H,E&&A);if(E){if(this.__ub()&&A){C=window.showModalDialog(z,[window.self],G);}
else {this.getBlocker().block();if(this.__dE==null){this.__dE=new qx.event.Timer(500);this.__dE.addListener(f,this.__ud,this);}
;this.__tX=window.open(z,name,G);this.__dE.restart();C=this.__tX;}
;}
...
;return C;}
,__uc:function(K,O){var M;var J;var N=[];if(O&&this.__ub()){for(var L in K){if(qx.bom.Window.__tY[L]){var I=n;if(L!=u&&L!=h){I=b;}
;J=qx.bom.Window.__tY[L]+i+K[L]+I;N.push(J);}
else {qx.log.Logger.warn(o+L+s);}
;}
;M=N.join(w);}
else {for(var L in K){if(qx.bom.Window.__ua[L]){if(qx.lang.Type.isBoolean(K[L])){J=L+v+(K[L]?j:a);}
else {J=L+v+K[L];}
;N.push(J);}
...
;M=N.join(y);}
;return M;}
,__ud:function(){if(this.isClosed(this.__tX)){this.getBlocker().unblock();this.__dE.stop();}
;}
,getBlocker:function(){if(this.__mu==null){this.__mu=new qx.bom.Blocker;}
;return this.__mu;}
...
,members:{__tl:null,__rc:null,__tm:null,__tn:false,__to:10000,__tp:0,__tq:f,block:function(r){if(!this.__tn){qx.event.Registration.addListener(window,a,this.__nh,this);this.__tm=r;var q=this.__tt();this.__tu(q);this.__tn=true;}
;}
,unblock:function(){if(this.__tn){this.__tv();qx.event.Registration.removeListener(window,a,this.__nh,this);this.__tn=false;}
;}
,isBlocked:function(){return this.__tn;}
,getBlockerElement:function(){return this.__rc;}
,setBlockerColor:function(s){qx.bom.element.Style.set(this.__rc,c,s);}
,getBlockerColor:function(){return qx.bom.element.Style.get(this.__rc,c);}
,setBlockerOpacity:function(t){qx.bom.element.Opacity.set(this.__rc,t);}
,getBlockerOpacity:function(){return qx.bom.element.Opacity.get(this.__rc);}
,setBlockerZIndex:function(u){qx.bom.element.Style.set(this.__rc,h,u);}
,getBlockerZIndex:function(){return qx.bom.element.Style.get(this.__rc,h);}
,__eP:function(){this.__tr();if((qx.core.Environment.get(j)==b)){this.__ts();}
;}
,__tr:function(){this.__rc=qx.dom.Element.create(m);qx.bom.element.Style.setStyles(this.__rc,{display:d,opacity:this.__tp,backgroundColor:this.__tq});this.setBlockerZIndex(this.__to);}
,__ts:function(){this.__tl=qx.bom.Iframe.create();qx.bom.element.Attribute.set(this.__tl,l,false);qx.bom.element.Attribute.set(this.__tl,i,n);qx.bom.element.Style.setStyles(this.__tl,{display:d,opacity:this.__tp});}
,__tt:function(){var v={position:p};if(this.__tx()){v.left=k;v.top=k;v.right=null;v.bottom=null;v.width=qx.bom.Document.getWidth()+g;v.height=qx.bom.Document.getHeight()+g;}
else {v.width=qx.bom.element.Dimension.getWidth(this.__tm)+g;v.height=qx.bom.element.Dimension.getHeight(this.__tm)+g;v.left=qx.bom.element.Location.getLeft(this.__tm)+g;v.top=qx.bom.element.Location.getTop(this.__tm)+g;}
;return v;}
,__tu:function(w){var x=document.body;qx.bom.element.Style.setStyles(this.__rc,w);qx.dom.Element.insertEnd(this.__rc,x);if((qx.core.Environment.get(j)==b)){w.zIndex=this.getBlockerZIndex()-1;qx.bom.element.Style.setStyles(this.__tl,w);qx.dom.Element.insertEnd(this.__tl,document.body);}
;}
,__tv:function(){qx.dom.Element.remove(this.__rc);if((qx.core.Environment.get(j)==b)){qx.dom.Element.remove(this.__tl);}
;}
,__nh:function(e){if(this.__tx()){this.__tw({width:k,height:k});qx.event.Timer.once(function(){var y={width:qx.bom.Document.getWidth()+g,height:qx.bom.Document.getHeight()+g};this.__tw(y);}
,this,0);}
;}
,__tw:function(z){qx.bom.element.Style.setStyles(this.__rc,z);if((qx.core.Environment.get(j)==b)){qx.bom.element.Style.setStyles(this.__tl,z);}
;}
,__tx:function(){return (this.__tm==null||qx.dom.Node.isWindow(this.__tm)||qx.dom.Node.isDocument(this.__tm));}
}});}
)();
