...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.event.Event.c2662d345b26.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,__R:function(clazz,config,name){var upname=qx.Bootstrap.firstUp(name);var members=clazz.prototype;if(qx.core.Environment.get(eg)){if(qx.core.Environment.get(dN)>1){qx.Bootstrap.debug(ec+name);}
;}
;if(config.dereference===undefined&&typeof config.check===bA){config.dereference=this.__S(config.check);}
...
;members[method.get[name]].$$install=function(){qx.core.Property.__W(clazz,name,bN,arguments);if(config.async){qx.core.Property.__W(clazz,name,bg,arguments);}
;}
...
;method.setImpl[name]=M+upname+el;members[setName].$$install=function(){qx.core.Property.__Y(clazz,name,f);qx.core.Property.__Y(clazz,name,co);if(config.async){qx.core.Property.__Y(clazz,name,fh);}
;}
...
;members[method.reset[name]].$$install=function(){qx.core.Property.__Y(clazz,name,bz);}
...
;}
,__S:function(fA){return !!this.__M[fA];}
,__T:{'0':ff,'1':ck,'2':J,'3':cR,'4':ej,'5':bd},error:function(fB,fH,fG,fC,fD){var fE=fB.constructor.classname;var fF=u+fG+br+fE+G+this.$$method[fC][fG]+eU+fD+bq;throw new Error(fF+(this.__T[fH]||dR+fH));}
,__U:function(fL,fM,name,fK,fI,fJ){var fN=this.__V(fL.constructor,name,fK,fI,fJ);if(fJ===undefined){return fN.call(fL);}
else if(qx.core.Environment.get(eg)){return fN.apply(fL,fJ);}
else {return fN.call(fL,fJ[0]);}
;}
,__V:function(clazz,name,variant,code,args){var store=this.$$method[variant][name];if(qx.core.Environment.get(eg)){if(qx.core.Environment.get(dN)>1){qx.Bootstrap.debug(bF+this.$$method[variant][name]+V+code.join(ez));}
...
,executeOptimizedGetter:function(fS,fR,name,fQ){var fP=this.__X(fR,name,fQ);var fT=fR.prototype;return this.__U(fS,fT,name,fQ,fP);}
,__W:function(fW,name,fV){var fU=this.__X(fW,name,fV);this.__V(fW,name,fV,fU);}
,__X:function(ga,name,fY){var gb=ga.$$properties[name];var fX=[];var gc=this.$$store;if(fY==bg){fX.push(N+this.$$method.get[name]+cw);return fX;}
...
;return fX;}
,executeOptimizedSetter:function(gh,gg,name,gf,ge){var gd=this.__ba(gg,name,gf);var gi=gg.prototype;return this.__U(gh,gi,name,gf,gd,ge);}
,__Y:function(gl,name,gk){var gj=this.__ba(gl,name,gk);return this.__V(gl,name,gk,gj);}
,__ba:function(gr,name,gq){var go=gr.$$properties[name];var gs=gr.prototype;var gn=[];var gt=qx.lang.String.firstUp(name);if(gq==fh){gn.push(eb+gt+ef);return gn;}
else if(gq==f){gn.push(de+gt+eW,cK);return gn;}
;var gm=gq===co||gq===ce||gq===cJ||(gq===dV&&go.init===undefined);var gp=go.apply||go.event||go.inheritable;var gu=this.__bb(gq,name);this.__bc(gn,gr,go,name);this.__bd(gn,go,name,gq,gm);if(gm||gp){this.__bj(gn,go,name);}
;if(gm){this.__be(gn,gr,go,name);}
;if(gp){this.__bf(gn,gm,gu,gq);}
;if(go.inheritable){gn.push(bP);}
;if(qx.core.Environment.get(eg)){if(gm){this.__bg(gn,go,gr,name,gq);}
;}
;if(!gp){this.__bh(gn,name,gq,gm);}
else {this.__bi(gn,go,name,gq,gm);}
;if(go.inheritable){this.__bk(gn,go,name,gq);}
else if(gp){this.__bl(gn,go,name,gq);}
;if(gp){this.__bm(gn,go,name,gq,!!(go.inheritable&&gs._getChildren));}
...
;}
;return gn;}
,__bb:function(gv,name){if(gv===cJ||gv===bo){var gw=this.$$store.runtime[name];}
else if(gv===ce||gv===j){gw=this.$$store.theme[name];}
else if(gv===dV){gw=this.$$store.init[name];}
else {gw=this.$$store.user[name];}
;return gw;}
,__bc:function(gx,gA,gy,name){gx.push(bB);if(typeof gy.isEqual===g){gx.push(by,gA.classname,cY,name,bm);}
...
;}
,__bd:function(gD,gB,name,gE,gC){if(qx.core.Environment.get(eg)){gD.push(eK);if(gE===dV){gD.push(cm,name,bI,gE,cf);}
;if(gE===eI){}
...
;}
;}
,__be:function(gF,gH,gG,name){if(gG.transform){gF.push(bG,gG.transform,dn);}
...
;}
;}
,__bf:function(gJ,gI,gL,gK){var gM=(gK===bz||gK===j||gK===bo);if(gI){gJ.push(cb,gL,W);}
else if(gM){gJ.push(Y,gL,bu);}
;}
,__bg:qx.core.Environment.select(eg,{"true":function(gO,gN,gQ,name,gP){if(!gN.nullable){gO.push(cr,name,bI,gP,cf);}
;if(gN.check!==undefined){gO.push(c+name+dQ+gQ.classname+cS);if(gN.nullable){gO.push(db);}
;if(gN.inheritable){gO.push(fa);}
;gO.push(L);if(this.__L[gN.check]!==undefined){gO.push(ea,this.__L[gN.check],eY);}
...
;}
,"false":undefined}),__bh:function(gS,name,gT,gR){if(gT===cJ){gS.push(cd,this.$$store.runtime[name],dj);}
...
;}
,__bi:function(gW,gU,name,gX,gV){gW.push(eo);gW.push(Y,this.$$store.runtime[name],dp);if(gX===cJ){gW.push(es,this.$$store.runtime[name],dj);}
...
;gW.push(dG);}
;}
,__bj:function(gY,ha,name){if(ha.inheritable){gY.push(dt,this.$$store.inherit[name],h);}
else {gY.push(v);}
...
;}
,__bk:function(hc,hb,name,hd){hc.push(dh);if(hd===eI){hc.push(cB);}
...
;hc.push(fe);}
,__bl:function(hf,he,name,hg){if(hg!==co&&hg!==cJ&&hg!==ce){hf.push(dP);}
;hf.push(eT);if(he.init!==undefined&&hg!==dV){hf.push(S,this.$$store.init[name],cx);}
else {hf.push(eO);}
;}
,__bm:function(hi,hh,name,hj,hk){hi.push(Q);if(hh.apply){hi.push(er,hh.apply,cI,name,bh,hj,ds);}
...
;}
,dispatchEvent:function(de,event){if(qx.core.Environment.get(n)){var di=v+event+t+de.classname+b;qx.core.Assert.assertNotUndefined(de,di+A);qx.core.Assert.assertNotNull(de,di+A);qx.core.Assert.assertInstance(event,qx.event.type.Event,di+P);}
...
;var dh=this.__cv.getDispatchers();var dg;var dd=false;for(var i=0,l=dh.length;i<l;i++ ){dg=this.getDispatcher(dh[i]);if(dg.canDispatchEvent(de,event,dj)){dg.dispatchEvent(de,event,dj);dd=true;break;}
;}
;if(!dd){if(qx.core.Environment.get(n)){qx.log.Logger.error(this,B+dj+F+de);}
;return true;}
...
,__cG:function(dk){if(this.__cB===null){this.__cB={};this.__cA.schedule();}
;this.__cB[dk]=true;}
,isBlacklisted:function(dl){return (this.__cB!==null&&this.__cB[dl]===true);}
}});}
)();
...
,fireEvent:function(bb,ba,bc,Y){var X=this.__cM.apply(this,arguments);return !X.getDefaultPrevented();}
,fireEventAsync:function(bg,bf,bh,be){var bd=this.__cM.apply(this,arguments);return bd.promise();}
,__cN:function(bi,bn,bl,bk){if(qx.core.Environment.get(g)){if(arguments.length>2&&bl===undefined&&bk!==undefined){throw new Error(c+bn+h);}
;}
...
,fireNonBubblingEvent:function(br,bq,bs,bp){var bo=this.__cN.apply(this,arguments);if(bo===null){return true;}
;return !bo.getDefaultPrevented();}
,fireNonBubblingEventAsync:qx.core.Environment.select(k,{"true":function(bw,bv,bx,bu){var bt=this.__cN.apply(this,arguments);if(bt===null){return qx.Promise.resolve(true);}
;return bt.promise();}
,"false":function(){throw new Error(this.classname+i);}
}),PRIORITY_FIRST:-32000,PRIORITY_NORMAL:0,PRIORITY_LAST:32000,__cx:[],addHandler:function(by){if(qx.core.Environment.get(g)){qx.core.Assert.assertInterface(by,qx.event.IEventHandler,f);}
;this.__cx.push(by);this.__cx.sort(function(a,b){return a.PRIORITY-b.PRIORITY;}
);}
,getHandlers:function(){return this.__cx;}
,__cy:[],addDispatcher:function(bA,bz){if(qx.core.Environment.get(g)){qx.core.Assert.assertInterface(bA,qx.event.IEventDispatcher,d);}
;this.__cy.push(bA);this.__cy.sort(function(a,b){return a.PRIORITY-b.PRIORITY;}
);}
...
(function(){var b='./timers.js',c='cancel',g="promiseFulfilled",h="A catch statement predicate ",k="caller",m="constructor",q=" Use ",w="                                                         \n\
        ...
        ",cg="(not from exception)",ch="ret = callback.apply(receiver, args);\n",ci="cancellation is disabled",cj="./promise",ck='./each.js',cl="ensureMethod",cm='./nodeify.js',cn="./cancel",co="Unhandled promise rejection: ",cp=":",cq='./call_get.js',cs='reject',ct="var promise;\n",cu='catch',cv="join pop push shift unshift slice filter forEach some ",cw="__de",cx="\u001b[33m",cy="Promise.using",cz="new Promise",cA="promise",cB="BLUEBIRD_W_FORGOTTEN_RETURN",cC="qx.promise.longStackTraces",cD="promiseChained",cE="expecting a positive integer\u000a\u000a    See http://goo.gl/MqrFmX\u000a",cF="Cannot promisify an API that has normal methods with '%s'-suffix\u000a\u000a    See http://goo.gl/MqrFmX\u000a",cG=".",cH="./async",cI="a promise was created in a ",cJ="withAppended",cK="you must pass at least 2 arguments to Promise.using",cL="./finally",cM="a promise was rejected with a non-error: ",cN="CustomEvent",cO=".all() was passed arguments but it does not take any",cP="circular promise resolution chain\u000a\u000a    See http://goo.gl/MqrFmX\u000a",cQ="color: darkorange",cR="A value %s was yielded that could not be treated as a promise\u000a\u000a    See http://goo.gl/MqrFmX\u000a\u000a",cS=" ",cT="testingtheevent",cU="__stackCleaned__",cV="BLUEBIRD_DEBUG",cW="foo",cX="function",cY="./direct_resolve",da="maybeWrapAsError",db="expecting a function but got ",dc="message",dd="async",de="From coroutine:\u000a",df="./nodeback",dg="return function(tryCatch, errorObj, Promise, async) {    \n\
            'use strict';                                                    \n\
            function [TheName](fn) {                                         \n\
                [TheProperties]                                              \n\
                this.fn = fn;                                                \n\
        ...
        ",dh="./promise_array",di="AggregateError of:",dj="late cancellation observer",dk=".settle()",dl="return",dm=">, no stack trace)",dn="aggregate error",dp="RangeError",dq="notEnumerableProp",dr="Parameters",ds="the promise constructor cannot be invoked directly\u000a\u000a    See http://goo.gl/MqrFmX\u000a",dt="Unhandled rejection ",du='./filter.js',dv=".reflect()",dw="[object Promise]",dx="unhandledRejection",dy="MODULE_NOT_FOUND",dz='props',dA="throw",dB="break",dC="ret = callback({{args}}, nodeback); break;\n",dD="type error",dE="the target of promisifyAll must be an object or a function\u000a\u000a    See http://goo.gl/MqrFmX\u000a",dF="ret = callback.call(this, {{args}}, nodeback); break;\n",dG="arguments",dH='some',dI='map',dJ="timeout error",dK="INTERNAL",dL="_",dM="catch",dN=" is deprecated and will be removed in a future version.",dO="Promise.method",dP="[no string representation]",dQ="[object process]",dR="promiseCancelled",dS='./using.js',dT=", ",dU=";                                      \n\
                if (promise instanceof Promise) {                            \n\
                    promise.cancel();                                        \n\
                }                                                            \n\
            ",dV="Promise.coroutine()",dW=" = ",dX="anonymous",dY="./util",ea="'concurrency' must be a number but it is ",eb="\n",ec="operation timed out",ed='reduce',ee='race',ef="generatorFunction must be a function\u000a\u000a    See http://goo.gl/MqrFmX\u000a",eg="Promise",eh="./errors",ei='./generators.js',ej="development",ek="generator .return() sentinel",el="__BluebirdErrorTypes__",em="./synchronous_inspection",en="qx.Promise",eo="_async",ep="try",eq="on",er="./join",es=")$",et="onCancel must be a function, got: ",eu="object",ev="Promises are installed and initialised but disabled from properties because qx.promise==false; this may cause unexpected behaviour",ew="OperationalError",ex=");   \n\
            var ret;                                                         \n\
...
,race:function(gc){return this._callIterableMethod(ee,arguments);}
,any:function(gd){return this._callIterableMethod(A,arguments);}
,some:function(gf,ge){return this._callIterableMethod(dH,arguments);}
...
,map:function(gl,gm,gn){return this._callIterableMethod(dI,arguments);}
,mapSeries:function(go,gp){return this._callIterableMethod(X,arguments);}
,reduce:function(gs,gq,gr){return this._callIterableMethod(ed,arguments);}
,__cT:function(gt,gu){this.__cS={resolve:gt,reject:gu,complete:false};}
,__cU:function(){if(!this.__cS){throw new Error(fm);}
;if(this.__cS.complete){throw new Error(V);}
;this.__cS.complete=true;return this.__cS;}
,resolve:function(gv){this.__cU().resolve(gv);}
,reject:function(gw){this.__cU().reject(gw);}
,_callIterableMethod:function(gy,gx){gx=qx.Promise.__db(gx);return qx.Promise.__da(this.__cR.then(function(gA){var gz=qx.Promise.Bluebird.resolve(gA instanceof qx.data.Array?gA.toArray():gA);return qx.Promise.__da(gz[gy].apply(gz,gx));}
));}
,_callMethod:function(gC,gB){gB=qx.Promise.__db(gB);return qx.Promise.__da(this.__cR[gC].apply(this.__cR,gB));}
...
else {gD=this.__da(qx.Promise.Bluebird.resolve(gF));}
;if(gE!==undefined){gD=gD.bind(gE);}
;return gD;}
,reject:function(gG,gI){var gH=this.__dc(cs,arguments,0);if(gI!==undefined){gH=gH.bind(gI);}
;return gH;}
,all:function(gJ){return this.__dc(Q,arguments);}
,race:function(gK){return this.__dc(ee,arguments);}
,any:function(gL){return this.__dc(A,arguments);}
,some:function(gN,gM){return this.__dc(dH,arguments);}
,forEach:function(gO,gP){return this.__dc(L,arguments);}
,filter:function(gQ,gR,gS){return this.__dc(bn,arguments);}
,map:function(gT,gU,gV){return this.__dc(dI,arguments);}
,mapSeries:function(gW,gX){return this.__dc(X,arguments);}
,reduce:function(hb,gY,ha){return this.__dc(ed,arguments);}
,method:function(hc){var hd=qx.Promise.Bluebird.method(hc);return function(){return qx.Promise.__da(hd.apply(this,arguments));}
;}
,props:function(he){return this.__dc(dz,arguments);}
,__cV:function(hf){this.Bluebird=hf;hf.config({warnings:qx.core.Environment.get(fL),longStackTraces:qx.core.Environment.get(cC),cancellation:true});}
,__cW:false,__cX:function(){if(this.__cW){return;}
;this.__cW=true;qx.bom.Event.addNativeListener(window,bh,this.__cY.bind(this));if(!qx.core.Environment.get(eJ)){qx.log.Logger.error(this,ev);}
;}
,__cY:function(e){e.preventDefault();var hg=null;if(e instanceof Error){hg=e;}
...
,__da:function(hh){if(hh instanceof qx.Promise.Bluebird){if(hh.$$qxPromise){hh=hh.$$qxPromise;}
else {hh=new qx.Promise(hh);}
;}
;return hh;}
,__db:function(hi,hj){hi=qx.lang.Array.fromArguments(hi);if(hj===undefined){hj=1;}
;if(hi.length>hj){var hk=hi[hi.length-1];if(hk instanceof qx.core.Object||qx.Class.isClass(hk)){hi.pop();for(var i=0;i<hi.length;i++ ){if(typeof hi[i]==cX){hi[i]=hi[i].bind(hk);}
;}
;}
;}
;return hi;}
,__dc:function(hl,hm,hn){hm=qx.Promise.__db(hm,hn);return this.__da(qx.Promise.Bluebird[hl].apply(qx.Promise.Bluebird,this.__dd(hm)));}
,__dd:function(hp){var ho=[];hp.forEach(function(hq){if(hq instanceof qx.data.Array){ho.push(hq.toArray());}
else if(hq instanceof qx.Promise){ho.push(hq.toPromise());}
else {ho.push(hq);}
;}
);return ho;}
},defer:function(hs,ht){hs.Promise=hs.Native=window.Promise;var hr=qx.core.Environment.get(eT);qx.core.Environment.add(fL,hr);qx.core.Environment.add(cC,false);}
});(function(){!function(e){qx.Promise.__cV(e());}
(function(){var define,module,exports;return (function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof _dereq_==cX&&_dereq_;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error(fg+o+eP);throw f.code=dy,f;}
...
;var hJ=hO(bs);var hE=hO(fO);var hL=hO(dY);function hN(){this._customScheduler=false;this._isTickUsed=false;this._lateQueue=new hE(16);this._normalQueue=new hE(16);this._haveDrainedQueues=false;this._trampolineEnabled=true;var self=this;this.drainQueues=function(){self._drainQueues();}
;this._schedule=hJ;}
...
;hN.prototype.fatalError=function(e,hR){if(hR){process.stderr.write(eF+(e instanceof Error?e.stack:e)+eb);process.exit(2);}
...
;hN.prototype._drainQueue=function(io){while(io.length()>0){var iq=io.shift();if(typeof iq!==cX){iq._settlePromises();continue;}
...
;module.exports=function(Promise){var util=_dereq_(dY);var canEvaluate=util.canEvaluate;var isIdentifier=util.isIdentifier;var getMethodCaller;var getGetter;if(!true){var makeMethodCaller=function(methodName){return new Function(cl,fz.replace(/methodName/g,methodName))(ensureMethod);}
;var makeGetter=function(propertyName){return new Function(bo,cf.replace(bt,propertyName));}
;var getCompiled=function(name,jb,iY){var ja=iY[name];if(typeof ja!==cX){if(!isIdentifier(name)){return null;}
...
;}
;function ensureMethod(jc,je){var jf;if(jc!=null)jf=jc[je];if(typeof jf!==cX){var jd=bk+util.classString(jc)+K+util.toString(je)+eP;throw new Promise.TypeError(jd);}
;return jf;}
...
;}
;}
,{"./util":36}],'6':[function(jt,jv,ju){B;jv.exports=function(jz,jw,jy,jB){var jx=jt(dY);var jD=jx.tryCatch;var jA=jx.errorObj;var jC=jz._async;jz.prototype[dB]=jz.prototype.cancel=function(){if(!jB.cancellation())return this._warn(ci);var jE=this;var jF=jE;while(jE._isCancellable()){if(!jE._cancelBy(jF)){if(jF._isFollowing()){jF._followee().cancel();}
...
;}
else if(jH!==undefined){if(typeof jH===cX){if(!jI){var e=jD(jH).call(this._boundValue());if(e===jA){this._attachExtraTrace(e.e);jC.throwLater(e.e);}
;}
;}
...
;}
;}
,{"./util":36}],'7':[function(jK,jM,jL){B;jM.exports=function(jO){var jQ=jK(dY);var jP=jK(fB).keys;var jR=jQ.tryCatch;var jS=jQ.errorObj;function jN(jT,jV,jU){return function(e){var jY=jU._boundValue();predicateLoop:for(var i=0;i<jT.length; ++i){var jX=jT[i];if(jX===Error||(jX!=null&&jX.prototype instanceof Error)){if(e instanceof jX){return jR(jV).call(jY,e);}
;}
else if(typeof jX===cX){var jW=jR(jX).call(jY,e);if(jW===jS){return jW;}
...
;return ki;}
;}
,{}],'9':[function(ku,kw,kv){B;kw.exports=function(kU,lj){var lh=kU._getDomain;var lB=kU._async;var lx=ku(eh).Warning;var li=ku(dY);var kJ=li.canAttachTrace;var kF;var kP;var ky=/[\\\/]bluebird[\\\/]js[\\\/](release|debug|instrumented)/;var lz=/\((?:timers\.js):\d+:\d+\)/;var kV=/[\/<\(](.+?):(\d+):(\d+)\)?\s*$/;var kB=null;var lc=null;var kT=false;var le;var kx=!!(li.env(cV)!=0&&(true||li.env(cV)||li.env(ba)===ej));var kQ=!!(li.env(z)!=0&&(kx||li.env(z)));var kX=!!(li.env(fb)!=0&&(kx||li.env(fb)));var kR=li.env(cB)!=0&&(kQ||!!li.env(cB));kU.prototype.suppressUnhandledRejections=function(){var lF=this._target();lF._bitField=((lF._bitField&(~1048576))|524288);}
...
;kU.prototype._notifyUnhandledRejection=function(){if(this._isRejectionUnhandled()){var lG=this._settledValue();this._setUnhandledRejectionIsNotified();lr(dx,kP,lG,this);}
;}
...
;kU.onPossiblyUnhandledRejection=function(lL){var lK=lh();kP=typeof lL===cX?(lK===null?lL:li.domainBind(lK,lL)):undefined;}
;kU.onUnhandledRejectionHandled=function(lN){var lM=lh();kF=typeof lN===cX?(lM===null?lN:li.domainBind(lM,lN)):undefined;}
;var lg=function(){}
...
;var lD=(function(){try{if(typeof CustomEvent===cX){var event=new CustomEvent(cN);li.global.dispatchEvent(event);return function(name,event){var lQ=new CustomEvent(name.toLowerCase(),{detail:event,cancelable:true});return !li.global.dispatchEvent(lQ);}
;}
else if(typeof Event===cX){var event=new Event(cN);li.global.dispatchEvent(event);return function(name,event){var lR=new Event(name.toLowerCase(),{cancelable:true});lR.detail=event;return !li.global.dispatchEvent(lR);}
;}
else {var event=document.createEvent(cN);event.initCustomEvent(cT,false,true,{});li.global.dispatchEvent(event);return function(name,event){var lS=document.createEvent(cN);lS.initCustomEvent(name.toLowerCase(),false,true,event);return !li.global.dispatchEvent(lS);}
;}
;}
catch(e){}
...
;}
;return function(name){var lT=eq+name.toLowerCase();var lU=li.global[lT];if(!lU)return false;lU.apply(li.global,[].slice.call(arguments,1));return true;}
;}
;}
...
;}
;if(ey in mf){var me=mf.warnings;kW.warnings=!!me;kR=kW.warnings;if(li.isObject(me)){if(eD in me){kR=!!me.wForgottenReturn;}
;}
;}
...
;function lv(mo,mm,mp){var mn=this;try{mo(mm,mp,function(mq){if(typeof mq!==cX){throw new TypeError(et+li.toString(mq));}
...
else if(!mC.__stackCleaned__){var mz=lE(mC);li.notEnumerableProp(mC,I,mz.message+eb+mz.stack.join(eb));li.notEnumerableProp(mC,cU,true);}
;}
;}
;function ld(mJ,mF,name,mH,parent){if(mJ===undefined&&mF!==null&&kR){if(parent!==undefined&&parent._returnedNonUndefined())return;if((mH._bitField&65535)===0)return;if(name)name=name+cS;var mD=fi;var mI=fi;if(mF._trace){var mG=mF._trace.stack.split(eb);var mN=kG(mG);for(var i=mN.length-1;i>=0; --i){var mM=mN[i];if(!lz.test(mM)){var mE=mM.match(kV);if(mE){mD=bx+mE[1]+cp+mE[2]+cp+mE[3]+cS;}
;break;}
;}
;if(mN.length>0){var mL=mN[0];for(var i=0;i<mG.length; ++i){if(mG[i]===mL){if(i>0){mI=eb+mG[i-1];}
;break;}
;}
;}
;}
;var mK=cI+name+bO+mD+bJ+fA+mI;mH._warn(mK,true,mF);}
;}
;function ls(name,mP){var mO=name+dN;if(mP)mO+=q+mP+O;return lu(mO);}
...
else {var mV=lE(mR);mR.stack=mV.message+eb+mV.stack.join(eb);}
;if(!kz(H,mR)){kY(mR,fi,true);}
;}
;function lq(mW,mX){for(var i=0;i<mX.length-1; ++i){mX[i].push(bL);mX[i]=mX[i].join(eb);}
;if(i<mX.length){mX[i]=mX[i].join(eb);}
;return mW+eb+mX.join(eb);}
...
;nb=nd;}
;}
;function kG(nl){var ni=[];for(var i=0;i<nl.length; ++i){var nk=nl[i];var nj=fa===nk||kB.test(nk);var nh=nj&&lb(nk);if(nj&&!nh){if(kT&&nk.charAt(0)!==cS){nk=fC+nk;}
;ni.push(nk);}
;}
;return ni;}
;function lC(no){var nn=no.stack.replace(/\s+$/g,fi).split(eb);for(var i=0;i<nn.length; ++i){var nm=nn[i];if(fa===nm||kB.test(nm)){break;}
;}
...
;if(typeof le===cX){le(ns,nv);}
else if(typeof console.log===cX||typeof console.log===eu){console.log(ns);}
;}
;}
;function lr(name,ny,nx,nz){var nA=false;try{if(typeof ny===cX){nA=true;if(name===M){ny(nz);}
else {ny(nx,nz);}
;}
;}
catch(e){lB.throwLater(e);}
;if(name===dx){if(!kz(name,nx,nz)&&!nA){kY(nx,dt);}
;}
else {kz(name,nz);}
;}
;function lw(nC){var nE;if(typeof nC===cX){nE=fx+(nC.name||dX)+fq;}
else {nE=nC&&typeof nC.toString===cX?nC.toString():li.toString(nC);var nB=/\[object [a-zA-Z0-9$_]+\]/;if(nB.test(nE)){try{var nD=JSON.stringify(nC);nE=nD;}
catch(e){}
;}
...
;function kK(){return typeof kE===cX;}
...
;}
;function la(nN,nJ){if(!kK())return;var nQ=nN.stack.split(eb);var nM=nJ.stack.split(eb);var nR=-1;var nL=-1;var nO;var nK;for(var i=0;i<nQ.length; ++i){var nP=kD(nQ[i]);if(nP){nO=nP.fileName;nR=nP.line;break;}
;}
...
;}
;}
;ll.prototype.attachExtraTrace=function(oh){if(oh.__stackCleaned__)return;this.uncycle();var of=lE(oh);var od=of.message;var oe=[of.stack];var og=this;while(og!==undefined){oe.push(kG(og.stack.split(eb)));og=og._parent;}
;ln(oe);lk(oe);li.notEnumerableProp(oh,I,lq(od,oe));li.notEnumerableProp(oh,cU,true);}
...
;return lw(op);}
;if(typeof Error.stackTraceLimit===bf&&typeof Error.captureStackTrace===cX){Error.stackTraceLimit+=6;kB=oi;lc=ol;var ok=Error.captureStackTrace;lb=function(oq){return ky.test(oq);}
;return function(os,or){Error.stackTraceLimit+=6;ok(os,or);Error.stackTraceLimit-=6;}
;}
;var oj=new Error();if(typeof oj.stack===bY&&oj.stack.split(eb)[0].indexOf(cc)>=0){kB=/@/;lc=ol;kT=true;return function ot(o){o.stack=new Error().stack;}
;}
...
;}
;lc=function(ov,ow){if(typeof ov===bY)return ov;if((typeof ow===eu||typeof ow===cX)&&ow.name!==undefined&&ow.message!==undefined){return ow.toString();}
;return lw(ow);}
;return null;}
)([]);if(typeof console!==fQ&&typeof console.warn!==fQ){le=function(ox){console.warn(ox);}
;if(li.isNode&&process.stderr.isTTY){le=function(oz,oA){var oy=oA?cx:fr;console.warn(oy+oz+be);}
;}
else if(!li.isNode&&typeof (new Error().stack)===bY){le=function(oB,oC){console.warn(eK+oB,oC?cQ:S);}
;}
;}
...
;oI.prototype[dl]=oI.prototype.thenReturn=function(oJ){if(oJ instanceof oI)oJ.suppressUnhandledRejections();return this._then(oH,undefined,undefined,{value:oJ},undefined);}
;oI.prototype[dA]=oI.prototype.thenThrow=function(oK){return this._then(oG,undefined,undefined,{reason:oK},undefined);}
...
;pa.mapSeries=oU;}
;}
,{}],'12':[function(pw,pp,pj){B;var po=pw(fB);var pv=po.freeze;var pr=pw(dY);var ph=pr.inherits;var pq=pr.notEnumerableProp;function pk(pA,pC){function pB(pD){if(!(this instanceof pB))return new pB(pD);pq(this,dc,typeof pD===bY?pD:pC);pq(this,ez,pA);if(Error.captureStackTrace){Error.captureStackTrace(this,this.constructor);}
else {Error.call(this);}
;}
;ph(pB,Error);return pB;}
;var pi,pz;var ps=pk(Y,H);var pu=pk(bU,bi);var pt=pk(eL,dJ);var pm=pk(bP,dn);try{pi=TypeError;pz=RangeError;}
catch(e){pi=pk(fv,dD);pz=pk(dp,y);}
;var pl=(cv+bS).split(cS);for(var i=0;i<pl.length; ++i){if(typeof Array.prototype[pl[i]]===cX){pm.prototype[pl[i]]=Array.prototype[pl[i]];}
;}
;po.defineProperty(pm.prototype,eH,{value:0,configurable:false,writable:true,enumerable:true});pm.prototype[fS]=true;var pn=0;pm.prototype.toString=function(){var pE=Array(pn*4+1).join(cS);var pG=eb+pE+di+eb;pn++ ;pE=Array(pn*4+1).join(cS);for(var i=0;i<this.length; ++i){var pH=this[i]===this?fK:this[i]+fi;var pF=pH.split(eb);for(var j=0;j<pF.length; ++j){pF[j]=pE+pF[j];}
;pH=pF.join(eb);pG+=pH+eb;}
;pn-- ;return pG;}
;function py(pI){if(!(this instanceof py))return new py(pI);pq(this,ez,ew);pq(this,dc,pI);this.cause=pI;this[fS]=true;if(pI instanceof Error){pq(this,dc,pI.message);pq(this,I,pI.stack);}
else if(Error.captureStackTrace){Error.captureStackTrace(this,this.constructor);}
;}
;ph(py,Error);var px=Error[el];if(!px){px=pv({CancellationError:pu,TimeoutError:pt,OperationalError:py,RejectionError:py,AggregateError:pm});po.defineProperty(Error,el,{value:px,writable:false,enumerable:false,configurable:false});}
...
;}
;}
,{}],'15':[function(qt,qv,qu){B;qv.exports=function(qD,qE){var qw=qt(dY);var qA=qD.CancellationError;var qG=qw.errorObj;function qB(qI,qK,qJ){this.promise=qI;this.type=qK;this.handler=qJ;this.called=false;this.cancelPromise=null;}
...
;function qy(qU){var qT=this.promise;var qS=this.handler;if(!this.called){this.called=true;var qQ=this.isFinallyHandler()?qS.call(qT._boundValue()):qS.call(qT._boundValue(),qU);if(qQ!==undefined){qT._setReturnedNonUndefined();var qP=qE(qQ,qT);if(qP instanceof qD){if(this.cancelPromise!=null){if(qP._isCancelled()){var qR=new qA(dj);qT._attachExtraTrace(qR);qG.e=qR;return qG;}
...
;}
;qD.prototype._passThrough=function(qW,qX,qY,qV){if(typeof qW!==cX)return this.then();return this._then(qY,qV,undefined,new qB(this,qX,qW),undefined);}
...
;return qB;}
;}
,{"./util":36}],'16':[function(rc,re,rd){B;re.exports=function(rn,ri,rm,ro,rl,rp){var rh=rc(eh);var TypeError=rh.TypeError;var rk=rc(dY);var rq=rk.errorObj;var rr=rk.tryCatch;var rj=[];function rg(rt,rs,rw){for(var i=0;i<rs.length; ++i){rw._pushContext();var rx=rr(rs[i])(rt);rw._popContext();if(rx===rq){rw._pushContext();var rv=rn.reject(rq.e);rw._popContext();return rv;}
...
;this._stack=rE;this._generatorFunction=rC;this._receiver=rB;this._generator=undefined;this._yieldHandlers=typeof ry===cX?[ry].concat(rj):rj;this._yieldedPromise=null;this._cancellationPhase=false;}
...
;}
;rf.prototype._promiseCancelled=function(){if(this._isResolved())return;var rF=typeof this._generator[dl]!==fQ;var rH;if(!rF){var rG=new rn.CancellationError(ek);rn.coroutine.returnSentinel=rG;this._promise._attachExtraTrace(rG);this._promise._pushContext();rH=rr(this._generator[dA]).call(this._generator,rG);this._promise._popContext();}
else {this._promise._pushContext();rH=rr(this._generator[dl]).call(this._generator,undefined);this._promise._popContext();}
...
;rf.prototype._promiseRejected=function(rK){this._yieldedPromise=null;this._promise._attachExtraTrace(rK);this._promise._pushContext();var rL=rr(this._generator[dA]).call(this._generator,rK);this._promise._popContext();this._continue(rL);}
...
;}
else {var rN=ro(rQ,this._promise);if(!(rN instanceof rn)){rN=rg(rN,this._yieldHandlers,this._promise);if(rN===null){this._promiseRejected(new TypeError(cR.replace(fk,rQ)+de+this._stack.split(eb).slice(1,-7).join(eb)));return;}
;}
...
;}
;}
;rn.coroutine=function(rU,rW){if(typeof rU!==cX){throw new TypeError(ef);}
...
;}
;rn.coroutine.addYieldHandler=function(sb){if(typeof sb!==cX){throw new TypeError(db+rk.classString(sb));}
;rj.push(sb);}
;rn.spawn=function(sd){rp.deprecated(bg,dV);if(typeof sd!==cX){return ri(ef);}
;var sc=new rf(sd,this);var se=sc.promise();sc._run(rn.spawn);return se;}
;}
;}
,{"./errors":12,"./util":36}],'17':[function(_dereq_,module,exports){B;module.exports=function(Promise,PromiseArray,tryConvertToPromise,INTERNAL,async,getDomain){var util=_dereq_(dY);var canEvaluate=util.canEvaluate;var tryCatch=util.tryCatch;var errorObj=util.errorObj;var reject;if(!true){if(canEvaluate){var thenCallback=function(i){return new Function(eY,bT,fe.replace(/Index/g,i));}
;var promiseSetter=function(i){return new Function(cA,bT,ce.replace(/Index/g,i));}
;var generateHolderClass=function(total){var props=new Array(total);for(var i=0;i<props.length; ++i){props[i]=fI+(i+1);}
;var assignment=props.join(dW)+eU;var cancellationCode=ct+props.map(function(sf){return w+sf+dU;}
).join(eb);var passedArguments=props.join(dT);var name=bj+total;var code=dg;code=code.replace(/\[TheName\]/g,name).replace(/\[TheTotal\]/g,total).replace(/\[ThePassedArguments\]/g,passedArguments).replace(/\[TheProperties\]/g,assignment).replace(/\[CancellationCode\]/g,cancellationCode);return new Function(bI,by,eg,dd,code)(tryCatch,errorObj,Promise,async);}
...
;}
;}
;Promise.join=function(){var sj=arguments.length-1;var sq;if(sj>0&&typeof arguments[sj]===cX){sq=arguments[sj];if(!true){if(sj<=8&&canEvaluate){var sl=new Promise(INTERNAL);sl._captureStackTrace();var sh=holderClasses[sj-1];var sp=new sh(sq);var sn=thenCallbacks;for(var i=0;i<sj; ++i){var so=tryConvertToPromise(arguments[i],sl);if(so instanceof Promise){so=so._target();var sm=so._bitField;;if(((sm&50397184)===0)){so._then(sn[i],reject,undefined,sl,sp);promiseSetters[i](so,sp);sp.asyncNeeded=false;}
...
;function sv(th,tk,tl,tj){if(typeof tk!==cX){return sy(db+sx.classString(tk));}
;var ti=0;if(tl!==undefined){if(typeof tl===eu&&tl!==null){if(typeof tl.concurrency!==bf){return sA.reject(new TypeError(ea+sx.classString(tl.concurrency)));}
;ti=tl.concurrency;}
...
;}
;}
,{"./util":36}],'19':[function(tt,tv,tu){B;tv.exports=function(tz,ty,tA,tx,tB){var tw=tt(dY);var tC=tw.tryCatch;tz.method=function(tD){if(typeof tD!==cX){throw new tz.TypeError(db+tw.classString(tD));}
;return function(){var tF=new tz(ty);tF._captureStackTrace();tF._pushContext();var tG=tC(tD).apply(this,arguments);var tE=tF._popContext();tB.checkForgottenReturns(tG,tE,dO,tF);tF._resolveFromSyncValue(tG);return tF;}
;}
;tz.attempt=tz[ep]=function(tM){if(typeof tM!==cX){return tx(db+tw.classString(tM));}
...
;}
;}
,{"./util":36}],'20':[function(tX,tT,tS){B;var tU=tX(dY);var tO=tU.maybeWrapAsError;var tP=tX(eh);var ua=tP.OperationalError;var tW=tX(fB);function tQ(ub){return ub instanceof Error&&tW.getPrototypeOf(ub)===Error.prototype;}
...
;}
;tT.exports=tV;}
,{"./errors":12,"./es5":13,"./util":36}],'21':[function(um,uo,un){B;uo.exports=function(us){var ur=um(dY);var ut=us._async;var uu=ur.tryCatch;var uv=ur.errorObj;function uw(uz,uy){var ux=this;if(!ur.isArray(uz))return up.call(ux,uz,uy);var uA=uu(uy).apply(ux._boundValue(),[null].concat(uz));if(uA===uv){ut.throwLater(uA.e);}
;}
...
;}
;us.prototype.asCallback=us.prototype.nodeify=function(uN,uM){if(typeof uN==cX){var uL=up;if(uM!==undefined&&Object(uM).spread){uL=uw;}
;this._then(uL,uq,undefined,this,uN);}
;return this;}
;}
;}
,{"./util":36}],'22':[function(uO,uQ,uP){B;uQ.exports=function(){var uT=function(){return new TypeError(cP);}
;var vq=function(){return new vu.PromiseInspection(this._target());}
;var vr=function(vx){return vu.reject(new TypeError(vx));}
;function vi(){}
;var vj={};var vh=uO(dY);var vo;if(vh.isNode){vo=function(){var vy=process.domain;if(vy===undefined)vy=null;return vy;}
;}
else {vo=function(){return null;}
;}
;vh.notEnumerableProp(vu,U,vo);var vp=uO(fB);var vn=uO(cH);var vw=new vn();vp.defineProperty(vu,eo,{value:vw});var uX=uO(eh);var TypeError=vu.TypeError=uX.TypeError;vu.RangeError=uX.RangeError;var vm=vu.CancellationError=uX.CancellationError;vu.TimeoutError=uX.TimeoutError;vu.OperationalError=uX.OperationalError;vu.RejectionError=uX.OperationalError;vu.AggregateError=uX.AggregateError;var vt=function(){}
;var vd={};var vg={};var vk=uO(bR)(vu,vt);var uY=uO(dh)(vu,vt,vk,vr,vi);var vl=uO(eN)(vu);var uR=vl.create;var vv=uO(bE)(vu,vl);var uS=vv.CapturedTrace;var vs=uO(cL)(vu,vk);var ve=uO(bN)(vg);var uW=uO(df);var uU=vh.errorObj;var vc=vh.tryCatch;function uV(self,vz){if(typeof vz!==cX){throw new TypeError(db+vh.classString(vz));}
;if(self.constructor!==vu){throw new TypeError(ds);}
;}
...
;vu.prototype.toString=function(){return dw;}
;vu.prototype.caught=vu.prototype[dM]=function(vE){var vC=arguments.length;if(vC>1){var vB=new Array(vC-1),j=0,i;for(i=0;i<vC-1; ++i){var vD=arguments[i];if(vh.isObject(vD)){vB[j++ ]=vD;}
...
;vu.prototype.then=function(vG,vH){if(vv.warnings()&&arguments.length>0&&typeof vG!==cX&&typeof vH!==cX){var vF=eA+vh.classString(vG);if(arguments.length>1){vF+=dT+vh.classString(vH);}
;this._warn(vF);}
...
;vu.prototype.spread=function(vL){if(typeof vL!==cX){return vr(db+vh.classString(vL));}
...
;return vM;}
;vu.prototype.all=function(){if(arguments.length>0){this._warn(cO);}
...
;vu.setScheduler=function(vY){if(typeof vY!==cX){throw new TypeError(db+vh.classString(vY));}
...
else {wd=wl._settlePromiseLateCancellationObserver;wf=new vm(dj);wl._attachExtraTrace(wf);wi=wc;}
;vw.invoke(wd,wl,{handler:wa===null?wi:(typeof wi===cX&&vh.domainBind(wa,wi)),promise:wj,receiver:wk,value:wf});}
...
;vu.prototype._setCancelled=function(){this._bitField=this._bitField|65536;this._fireEvent(dR,this);}
...
;if(wE===0){this._promise0=wI;this._receiver0=wJ;if(typeof wK===cX){this._fulfillmentHandler0=wF===null?wK:vh.domainBind(wF,wK);}
;if(typeof wG===cX){this._rejectionHandler0=wF===null?wG:vh.domainBind(wF,wG);}
;}
else {var wH=wE*4-4;this[wH+2]=wI;this[wH+3]=wJ;if(typeof wK===cX){this[wH+0]=wF===null?wK:vh.domainBind(wF,wK);}
;if(typeof wG===cX){this[wH+1]=wF===null?wG:vh.domainBind(wF,wG);}
;}
...
else {var wP=new vm(dj);wR._attachExtraTrace(wP);this._reject(wP);}
;}
;vu.prototype._rejectCallback=function(wX,wV,wW){var wU=vh.ensureErrorObject(wX);var wY=wU===wX;if(!wY&&!wW&&vv.warnings()){var xa=cM+vh.classString(wX);this._warn(xa,true);}
...
;}
else if(typeof xs===cX){if(!xr){xs.call(xu,xp,xt);}
...
;}
;}
;vu.prototype._settlePromiseLateCancellationObserver=function(xx){var xw=xx.handler;var xv=xx.promise;var xz=xx.receiver;var xy=xx.value;if(typeof xw===cX){if(!(xv instanceof vu)){xw.call(xz,xy,xv);}
...
;vu.defer=vu.pending=function(){vv.deprecated(eO,cz);var yd=new vu(vt);return {promise:yd,resolve:va,reject:vb};}
;vh.notEnumerableProp(vu,bc,uT);uO(eS)(vu,vt,vk,vr,vv);uO(ft)(vu,vt,vk,vv);uO(cn)(vu,uY,vr,vv);uO(cY)(vu);uO(em)(vu);uO(er)(vu,uY,vk,vt,vw,vo);vu.Promise=vu;vu.version=fH;uO(eG)(vu,uY,vr,vk,vt,vv);uO(cq)(vu);uO(dS)(vu,vr,vk,uR,vt,vv);uO(b)(vu,vt,vv);uO(ei)(vu,vr,vt,vk,vi,vv);uO(cm)(vu);uO(bM)(vu,vt);uO(fN)(vu,uY,vk,vr);uO(bX)(vu,vt,vk,vr);uO(eQ)(vu,uY,vr,vk,vt,vv);uO(fl)(vu,uY,vv);uO(fJ)(vu,uY,vr);uO(du)(vu,vt);uO(ck)(vu,vt);uO(bu)(vu);vh.toFastProperties(vu);vh.toFastProperties(vu.prototype);function vf(ye){var p=new vu(vt);p._fulfillmentHandler0=ye;p._rejectionHandler0=ye;p._promise0=ye;p._receiver0=ye;}
...
;return yl;}
;}
,{"./util":36}],'24':[function(_dereq_,module,exports){B;module.exports=function(Promise,INTERNAL){var THIS={};var util=_dereq_(dY);var nodebackForPromise=_dereq_(df);var withAppended=util.withAppended;var maybeWrapAsError=util.maybeWrapAsError;var canEvaluate=util.canEvaluate;var TypeError=_dereq_(eh).TypeError;var defaultSuffix=eW;var defaultPromisified={__de:true};var noCopyProps=[eR,eH,ez,dG,k,cd,F,cw];var noCopyPropsPattern=new RegExp(eX+noCopyProps.join(bW)+es);var defaultFilter=function(name){return util.isIdentifier(name)&&name.charAt(0)!==dL&&name!==m;}
;function propsFilter(yN){return !noCopyPropsPattern.test(yN);}
;function isPromisified(yO){try{return yO.__de===true;}
catch(e){return false;}
;}
...
;function checkValid(yV,yT,yW){for(var i=0;i<yV.length;i+=2){var yX=yV[i];if(yW.test(yX)){var yU=yX.replace(yW,fi);for(var j=0;j<yV.length;j+=2){if(yV[j]===yU){throw new TypeError(cF.replace(fk,yT));}
;}
;}
;}
;}
;function promisifiableMethods(za,zb,yY,ze){var zc=util.inheritedDataKeys(za);var zd=[];for(var i=0;i<zc.length; ++i){var zg=zc[i];var zf=za[zg];var zh=ze===defaultFilter?true:defaultFilter(zg,zf,za);if(typeof zf===cX&&!isPromisified(zf)&&!hasPromisified(za,zg,zb)&&ze(zg,zf,za,zh)){zd.push(zg,zf);}
;}
...
;return 0;}
;makeNodePromisifiedEval=function(callback,receiver,originalName,fn,_,multiArgs){var newParameterCount=Math.max(0,parameterCount(fn)-1);var argumentOrder=switchCaseArgumentOrder(newParameterCount);var shouldProxyThis=typeof callback===bY||receiver===THIS;function generateCallForArgumentCount(zp){var zq=argumentSequence(zp).join(dT);var zr=zp>0?dT:fi;var zs;if(shouldProxyThis){zs=dF;}
else {zs=receiver===undefined?dC:fo;}
;return zs.replace(fp,zq).replace(dT,zr);}
...
;var getFunctionCode=typeof callback===bY?(bm+callback+eI):bw;var body=E+multiArgs+ex.replace(eB,generateArgumentSwitchCase()).replace(fy,getFunctionCode);body=body.replace(dr,parameterDeclaration(newParameterCount));return new Function(eg,bw,fh,cJ,da,ff,bI,by,dq,dK,body)(Promise,fn,receiver,withAppended,maybeWrapAsError,nodebackForPromise,util.tryCatch,util.errorObj,util.notEnumerableProp,INTERNAL);}
;}
...
;util.notEnumerableProp(zv,cw,true);return zv;}
...
);util.notEnumerableProp(zL,cw,true);zH[zQ]=zL;}
;}
;util.toFastProperties(zH);return zH;}
;function promisify(zS,zU,zT){return makeNodePromisified(zS,zU,undefined,zS,null,zT);}
;Promise.promisify=function(zV,zX){if(typeof zV!==cX){throw new TypeError(db+util.classString(zV));}
...
;Promise.promisifyAll=function(Ai,Ag){if(typeof Ai!==cX&&typeof Ai!==eu){throw new TypeError(dE);}
;Ag=Object(Ag);var Ae=!!Ag.multiArgs;var Ab=Ag.suffix;if(typeof Ab!==bY)Ab=defaultSuffix;var Af=Ag.filter;if(typeof Af!==cX)Af=defaultFilter;var Ah=Ag.promisifier;if(typeof Ah!==cX)Ah=makeNodePromisified;if(!util.isIdentifier(Ab)){throw new RangeError(fw);}
...
;}
;}
,{"./errors":12,"./nodeback":20,"./util":36}],'25':[function(Aj,Al,Ak){B;Al.exports=function(As,An,At,Ar){var Aq=Aj(dY);var Ax=Aq.isObject;var Am=Aj(fB);var Au;if(typeof Map===cX)Au=Map;var Ap=(function(){var Ay=0;var Az=0;function AA(AB,AC){this[Ay]=AB;this[Ay+Az]=AC;Ay++ ;}
...
;Bf.exports=Bc;}
,{}],'27':[function(BG,BI,BH){B;BI.exports=function(BN,BJ,BO,BL){var BK=BG(dY);var BP=function(BQ){return BQ.then(function(BR){return BM(BR,BQ);}
);}
...
;}
;}
,{"./util":36}],'28':[function(Cb,Cd,Cc){B;Cd.exports=function(Cm,Cg,Ck,Cn,Cl,Co){var Ce=Cm._getDomain;var Cj=Cb(dY);var Cp=Cj.tryCatch;function Cf(Cw,Cs,Cu,Cv){this.constructor$(Cw);var Ct=Ce();this._fn=Ct===null?Cs:Cj.domainBind(Ct,Cs);if(Cu!==undefined){Cu=Cm.resolve(Cu);Cu._attachCancellationCallback(this);}
...
;}
;function Ch(CM,CP,CN,CQ){if(typeof CP!==cX){return Ck(db+Cj.classString(CP));}
...
;}
;}
,{"./util":36}],'29':[function(Di,De,Db){B;var Da=Di(dY);var Dd;var Dg=function(){throw new Error(bd);}
...
;}
else if(typeof Dc===cX&&typeof Dc.resolve===cX){var Dj=Dc.resolve();Dd=function(Dm){Dj.then(Dm);}
;}
else if((typeof MutationObserver!==fQ)&&!(typeof window!==fQ&&window.navigator&&(window.navigator.standalone||window.cordova))){Dd=(function(){var Dq=document.createElement(bK);var Ds={attributes:true};var Dn=false;var Dr=document.createElement(bK);var Do=new MutationObserver(function(){Dq.classList.toggle(cW);Dn=false;}
);Do.observe(Dr,Ds);var Dp=function(){if(Dn)return;Dn=true;Dr.classList.toggle(cW);}
...
;De.exports=Dd;}
,{"./util":36}],'30':[function(Dx,Dz,Dy){B;Dz.exports=function(DE,DB,DF){var DA=DE.PromiseInspection;var DD=Dx(dY);function DC(DG){this.constructor$(DG);}
...
;DE.settle=function(DQ){DF.deprecated(dk,dv);return new DC(DQ).promise();}
;DE.prototype.settle=function(){return DE.settle(this);}
;}
;}
,{"./util":36}],'31':[function(DR,DT,DS){B;DT.exports=function(Eb,DW,Ea){var DY=DR(dY);var RangeError=DR(eh).RangeError;var DV=DR(eh).AggregateError;var DU=DY.isArray;var Ec={};function Ed(Ee){this.constructor$(Ee);this._howMany=0;this._unwrap=false;this._initialized=false;}
...
;function DX(Eo,En){if((En|0)!==En||En<0){return Ea(cE);}
...
;EC.prototype.__df=function(){return (this._bitField&65536)===65536;}
;EC.prototype._isCancelled=function(){return this._target().__df();}
...
;}
,{}],'33':[function(EH,EJ,EI){B;EJ.exports=function(ET,EO){var EN=EH(dY);var ER=EN.errorObj;var EU=EN.isObject;function EQ(EW,EX){if(EU(EW)){if(EW instanceof ET)return EW;var EV=EK(EW);if(EV===ER){if(EX)EX._pushContext();var EY=ET.reject(EV.e);if(EX)EX._popContext();return EY;}
else if(typeof EV===cX){if(EM(EW)){var EY=new ET(EO);EW._then(EY._fulfill,EY._reject,undefined,EY,null);return EY;}
;return ES(EW,EV,EX);}
;}
;return EW;}
...
;return EQ;}
;}
,{"./util":36}],'34':[function(Fn,Fp,Fo){B;Fp.exports=function(Fx,Fw,Fv){var Fq=Fn(dY);var Fr=Fx.TimeoutError;function Fs(FB){this.handle=FB;}
...
;}
;}
,{"./util":36}],'35':[function(FS,FU,FT){B;FU.exports=function(Gh,Ge,Gi,FV,Gg,Ga){var Gd=FS(dY);var TypeError=FS(eh).TypeError;var FW=FS(dY).inherits;var Gk=Gd.errorObj;var Gn=Gd.tryCatch;var FX={};function Gf(e){setTimeout(function(){throw e;}
,0);}
;function Gl(Go){var Gp=Gi(Go);if(Gp!==Go&&typeof Go._isDisposable===cX&&typeof Go._getDisposer===cX&&Go._isDisposable()){Gp._setDisposable(Go._getDisposer());}
;return Gp;}
...
;Gm.isDisposer=function(d){return (d!=null&&typeof d.resource===cX&&typeof d.tryDispose===cX);}
...
;}
;}
;Gh.using=function(){var GO=arguments.length;if(GO<2)return Ge(cK);var GR=arguments[GO-1];if(typeof GR!==cX){return Ge(db+Gd.classString(GR));}
...
;GX[i]=Ha.value();}
;GQ._pushContext();GR=Gn(GR);var Hb=GN?GR.apply(undefined,GX):GR(GX);var GY=GQ._popContext();Ga.checkForgottenReturns(Hb,GY,cy,GQ);return Hb;}
...
;Gh.prototype.disposer=function(He){if(typeof He===cX){return new Gb(He,this,FV());}
;throw new TypeError();}
;}
;}
...
;function isObject(Hm){return typeof Hm===cX||typeof Hm===eu&&Hm!==null;}
...
;}
;}
)();var thisAssignmentPattern=/this\s*\.\s*\S+\s*=/;function isClass(HQ){try{if(typeof HQ===cX){var HN=es5.names(HQ.prototype);var HO=es5.isES5&&HN.length>1;var HP=HN.length>0&&!(HN.length===1&&HN[0]===m);var HR=thisAssignmentPattern.test(HQ+fi)&&es5.names(HQ).length>0;if(HO||HP||HR){return true;}
;}
;return false;}
catch(e){return false;}
;}
...
;}
;function isError(HY){return HY!==null&&typeof HY===eu&&typeof HY.message===bY&&typeof HY.name===bY;}
;function markAsOriginatingFromRejection(e){try{notEnumerableProp(e,fS,true);}
catch(Ia){}
;}
;function originatesFromRejection(e){if(e==null)return false;return ((e instanceof Error[el].OperationalError)||e[fS]===true);}
...
;return null;}
;if(typeof Symbol!==fQ&&Symbol.iterator){var ArrayFrom=typeof Array.from===cX?function(v){return Array.from(v);}
...
else if(arguments.length>1){this.__dg=[];for(var i=0;i<arguments.length;i++ ){this.__dg.push(arguments[i]);}
;}
else if(typeof u==q){this.__dg=new Array(u);}
else if(u instanceof Array){this.__dg=qx.lang.Array.clone(u);}
else {this.__dg=[];this.dispose();throw new Error(l);}
;for(var i=0;i<this.__dg.length;i++ ){this._applyEventPropagation(this.__dg[i],null,i);}
;this.__dh();if(qx.core.Environment.get(k)){this[0]=h;}
;}
,properties:{autoDisposeItems:{check:e,init:false}},events:{"change":t,"changeLength":t},members:{__dg:null,concat:function(v){v=qx.lang.Array.toNativeArray(v);if(v){var w=this.__dg.concat(v);}
else {var w=this.__dg.concat();}
;return new qx.data.Array(w);}
,join:function(x){return this.__dg.join(x);}
,pop:function(){var y=this.__dg.pop();this.__dh();this._registerEventChaining(null,y,this.length-1);this.fireDataEvent(r,{value:[],name:this.length+f,old:[y],item:this});this.fireDataEvent(o,{start:this.length-1,end:this.length-1,type:g,removed:[y],added:[]},null);return y;}
,push:function(z){for(var i=0;i<arguments.length;i++ ){this.__dg.push(arguments[i]);this.__dh();this._registerEventChaining(arguments[i],null,this.length-1);this.fireDataEvent(r,{value:[arguments[i]],name:(this.length-1)+f,old:[],item:this});this.fireDataEvent(o,{start:this.length-1,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,reverse:function(){if(this.length==0){return;}
;var A=this.__dg.concat();this.__dg.reverse();this.__di(0,this.length);this.fireDataEvent(o,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(r,{value:this.__dg,name:n+(this.__dg.length-1),old:A,item:this});}
,shift:function(){if(this.length==0){return;}
;var B=this.__dg.shift();this.__dh();this._registerEventChaining(null,B,this.length-1);this.__di(0,this.length);this.fireDataEvent(r,{value:[],name:p,old:[B],item:this});this.fireDataEvent(o,{start:0,end:this.length-1,type:g,removed:[B],added:[]},null);return B;}
,slice:function(D,C){return new qx.data.Array(this.__dg.slice(D,C));}
,splice:function(I,K,M){var Q=this.__dg.length;var L=this.__dg.splice.apply(this.__dg,arguments);if(this.__dg.length!=Q){this.__dh();}
...
;this.__di(I+(arguments.length-2)-K,this.length);if(O||G){var J=[];for(var i=2;i<arguments.length;i++ ){J[i-2]=arguments[i];}
...
;var U=this.__dg.concat();this.__dg.sort.apply(this.__dg,arguments);if(qx.lang.Array.equals(this.__dg,U)===true){return;}
;this.__di(0,this.length);this.fireDataEvent(o,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(r,{value:this.__dg,name:n+(this.length-1),old:U,item:this});}
,unshift:function(W){for(var i=arguments.length-1;i>=0;i-- ){this.__dg.unshift(arguments[i]);this.__dh();this.__di(0,this.length);this.fireDataEvent(r,{value:[this.__dg[0]],name:p,old:[this.__dg[1]],item:this});this.fireDataEvent(o,{start:0,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,toArray:function(){return this.__dg;}
,getItem:function(X){return this.__dg[X];}
,setItem:function(Y,bb){var ba=this.__dg[Y];if(ba===bb){return;}
;this.__dg[Y]=bb;this._registerEventChaining(bb,ba,Y);if(this.length!=this.__dg.length){this.__dh();}
...
,indexOf:function(bc){return this.__dg.indexOf(bc);}
,lastIndexOf:function(bd){return this.__dg.lastIndexOf(bd);}
,toString:function(){if(this.__dg!=null){return this.__dg.toString();}
;return f;}
,contains:function(be){return this.includes(be);}
,includes:function(bf){return this.__dg.indexOf(bf)!==-1;}
...
,removeAll:function(){for(var i=0;i<this.__dg.length;i++ ){this._registerEventChaining(null,this.__dg[i],i);}
;if(this.getLength()==0){return [];}
;var bs=this.getLength();var br=this.__dg.concat();this.__dg.length=0;this.__dh();this.fireDataEvent(r,{value:[],name:n+(bs-1),old:br,item:this});this.fireDataEvent(o,{start:0,end:bs-1,type:g,removed:br,added:[]},null);return br;}
,append:function(bt){bt=qx.lang.Array.toNativeArray(bt);if(qx.core.Environment.get(k)){qx.core.Assert.assertArray(bt,m);}
;var bu=this.__dg.length;Array.prototype.push.apply(this.__dg,bt);for(var i=0;i<bt.length;i++ ){this._registerEventChaining(bt[i],null,bu+i);}
;var bu=this.length;this.__dh();var name=bu==(this.length-1)?bu:bu+a+(this.length-1);this.fireDataEvent(r,{value:bt,name:name+f,old:[],item:this});this.fireDataEvent(o,{start:bu,end:this.length-1,type:b,added:bt,removed:[]},null);}
...
;}
;return bC===undefined?null:bC;}
,forEach:function(bD,bE){for(var i=0;i<this.__dg.length;i++ ){bD.call(bE,this.__dg[i],i,this);}
;}
,filter:function(bF,self){return new qx.data.Array(this.__dg.filter(bF,self));}
,map:function(bG,self){return new qx.data.Array(this.__dg.map(bG,self));}
,some:function(bH,self){return this.__dg.some(bH,self);}
,every:function(bI,self){return this.__dg.every(bI,self);}
,reduce:function(bK,bJ){return this.__dg.reduce(bK,bJ);}
,reduceRight:function(bM,bL){return this.__dg.reduceRight(bM,bL);}
,__dh:function(){var bN=this.length;this.length=this.__dg.length;this.fireDataEvent(s,this.length,bN);}
,__di:function(bP,bO){for(var i=bP;i<bO;i++ ){this._registerEventChaining(this.__dg[i],this.__dg[i],i);}
;}
},destruct:function(){for(var i=0;i<this.__dg.length;i++ ){var bQ=this.__dg[i];this._applyEventPropagation(null,bQ,i);if(this.isAutoDisposeItems()&&bQ&&bQ instanceof qx.core.Object){bQ.dispose();}
;}
;this.__dg=null;}
});}
)();
...
(function(){var a="qx.core.AssertionError";qx.Bootstrap.define(a,{extend:qx.type.BaseError,construct:function(b,c){qx.type.BaseError.call(this,b,c);this.__dl=qx.dev.StackTrace.getStackTrace();}
,members:{__dl:null,getStackTrace:function(){return this.__dl;}
}});}
)();
...
(function(){var a="qx.util.DeferredCallManager",b="singleton";qx.Class.define(a,{extend:qx.core.Object,type:b,implement:[qx.core.IDisposable],construct:function(){this.__dm={};this.__dn=qx.lang.Function.bind(this.__dr,this);this.__do=false;}
,members:{__dp:null,__dq:null,__dm:null,__do:null,__dn:null,schedule:function(d){if(this.__dp==null){this.__dp=window.setTimeout(this.__dn,0);}
;var c=d.toHashCode();if(this.__dq&&this.__dq[c]){return;}
;this.__dm[c]=d;this.__do=true;}
,refreshTimeout:function(){if(this.__dp!==null){this.__dp=window.setTimeout(this.__dn,0);}
;}
,cancel:function(f){var e=f.toHashCode();if(this.__dq&&this.__dq[e]){this.__dq[e]=null;return;}
;delete this.__dm[e];if(qx.lang.Object.isEmpty(this.__dm)&&this.__dp!=null){window.clearTimeout(this.__dp);this.__dp=null;}
;}
,__dr:qx.event.GlobalError.observeMethod(function(){this.__dp=null;while(this.__do){this.__dq=qx.lang.Object.clone(this.__dm);this.__dm={};this.__do=false;for(var h in this.__dq){var g=this.__dq[h];if(g){this.__dq[h]=null;g.call();}
;}
;}
;this.__dq=null;}
)},destruct:function(){if(this.__dp!=null){window.clearTimeout(this.__dp);}
;this.__dn=this.__dm=null;}
});}
)();
...
;return A;}
;return z;}
,equals:function(D,E){return qx.lang.Object.__ds(D,E,[],[]);}
,__ds:function(L,H,F,G){if(L===H){return L!==0||1/L==1/H;}
...
;F.push(L);G.push(H);var O=0,M=true;if(K==j){O=L.length;M=O==H.length;if(M){while(O-- ){if(!(M=qx.lang.Object.__ds(L[O],H[O],F,G))){break;}
;}
;}
;}
else {for(var N in L){if(Object.prototype.hasOwnProperty.call(L,N)){O++ ;if(!(M=Object.prototype.hasOwnProperty.call(H,N)&&qx.lang.Object.__ds(L[N],H[N],F,G))){break;}
;}
;}
...
(function(){var a="qx.util.DeferredCall",b="The context object '",c="qx.debug",d="'is already disposed.",e="' of the defered call '";qx.Class.define(a,{extend:qx.core.Object,construct:function(f,g){qx.core.Object.call(this);this.__s=f;this.__u=g||null;this.__dt=qx.util.DeferredCallManager.getInstance();}
,members:{__s:null,__u:null,__dt:null,cancel:function(){this.__dt.cancel(this);}
...
},destruct:function(){this.cancel();this.__u=this.__s=this.__dt=null;}
});}
)();
(function(){var a="qx.debug.databinding",b=". Error message: ",c="No number or 'last' value has been given",d="Data after conversion: ",f="set",g="deepBinding",h=")",k=") to the object '",l="item",m="Can not remove the bindings for null object!",n="Please use only one array at a time: ",p="Binding executed from ",q="Integer",r="reset",s=" of object ",t="qx.event.type.Data",u="qx.data.SingleValueBinding",v="Binding property ",w="Failed so set value ",x=").",y="change",z="qx.debug",A="targetObject",B="targetPropertyChain",C="get",D="^",E="Binding could not be found!",F="sourceObject",G="String",H=" to ",I="Binding from '",J="",K="sourcePropertyChain",L="PositiveNumber",M="Data before conversion: ",N="]",O="[",P=".",Q="PositiveInteger",R="' (",S=" on ",T="Binding does not exist!",U=" does not work.",V=" in an array binding: ",W=" is not an data (qx.event.type.Data) event on ",X=".[",Y=" (",bj="Boolean",bk=" by ",bl="Date",bg="Number",bh=" not possible: No event available. ",bi="last";qx.Class.define(u,{statics:{__du:{},__dv:{},bind:function(by,bs,bD,bA,bH){if(qx.core.Environment.get(z)){qx.core.Assert.assertObject(by,F);qx.core.Assert.assertString(bs,K);qx.core.Assert.assertObject(bD,A);qx.core.Assert.assertString(bA,B);}
;var bC=this.__dx(by,bs,bD,bA,bH);var bo=bs.split(P);var bn=this.__dF(bo);var bq=[];var bF=[];var bp=[];var br=[];var bw=by;var bE=null;try{for(var i=0;i<bo.length;i++ ){if(bn[i]!==J){br.push(y);}
else {var bv=this.__dy(bw,bo[i]);if(!bv){if(i==0){throw new qx.core.AssertionError(v+bo[i]+s+bw+bh);}
;bE=this.__dE(undefined,bD,bA,bH,by);break;}
;br.push(bv);}
;bq[i]=bw;if(i==bo.length-1){if(bn[i]!==J){var bx=bn[i]===bi?bw.length-1:bn[i];var bm=bw.getItem(bx);bE=this.__dE(bm,bD,bA,bH,by);bp[i]=this.__dG(bw,br[i],bD,bA,bH,bn[i]);}
else {if(bo[i]!=null&&bw[C+qx.lang.String.firstUp(bo[i])]!=null){var bm=bw[C+qx.lang.String.firstUp(bo[i])]();bE=this.__dE(bm,bD,bA,bH,by);}
;bp[i]=this.__dG(bw,br[i],bD,bA,bH);}
;}
else {var bG={index:i,propertyNames:bo,sources:bq,listenerIds:bp,arrayIndexValues:bn,targetObject:bD,targetPropertyChain:bA,options:bH,listeners:bF};var bB=qx.lang.Function.bind(this.__dw,this,bG);bF.push(bB);bp[i]=bw.addListener(br[i],bB);}
...
;}
;if(!bw){this.__dE(bw,bD,bA,bH,by);break;}
;}
;}
...
;}
;throw bI;}
;var bu={type:g,listenerIds:bp,sources:bq,targetListenerIds:bC.listenerIds,targets:bC.targets,initialPromise:bE};this.__dH(bu,by,bs,bD,bA);return bu;}
,__dw:function(bP){if(bP.options&&bP.options.onUpdate){bP.options.onUpdate(bP.sources[bP.index],bP.targetObject);}
...
;if(!bJ){this.__dA(bP.targetObject,bP.targetPropertyChain,bP.options.converter());}
else {this.__dz(bP.targetObject,bP.targetPropertyChain);}
;}
else {this.__dz(bP.targetObject,bP.targetPropertyChain);}
;break;}
;if(j==bP.propertyNames.length-1){if(qx.Class.implementsInterface(bN,qx.data.IListData)){var bR=bP.arrayIndexValues[j]===bi?bN.length-1:bP.arrayIndexValues[j];var bK=bN.getItem(bR);this.__dE(bK,bP.targetObject,bP.targetPropertyChain,bP.options,bP.sources[bP.index]);bP.listenerIds[j]=this.__dG(bN,y,bP.targetObject,bP.targetPropertyChain,bP.options,bP.arrayIndexValues[j]);}
else {if(bP.propertyNames[j]!=null&&bN[C+qx.lang.String.firstUp(bP.propertyNames[j])]!=null){var bK=bN[C+qx.lang.String.firstUp(bP.propertyNames[j])]();this.__dE(bK,bP.targetObject,bP.targetPropertyChain,bP.options,bP.sources[bP.index]);}
;var bL=this.__dy(bN,bP.propertyNames[j]);if(!bL){this.__dz(bP.targetObject,bP.targetPropertyChain);break;}
;bP.listenerIds[j]=this.__dG(bN,bL,bP.targetObject,bP.targetPropertyChain,bP.options);}
;}
else {if(bP.listeners[j]==null){var bM=qx.lang.Function.bind(this.__dw,this,bP);bP.listeners.push(bM);}
;if(qx.Class.implementsInterface(bN,qx.data.IListData)){var bL=y;}
else {var bL=this.__dy(bN,bP.propertyNames[j]);}
;if(!bL){this.__dz(bP.targetObject,bP.targetPropertyChain);return;}
;bP.listenerIds[j]=bN.addListener(bL,bP.listeners[j]);}
;}
;}
,__dx:function(bU,cd,ch,bY,cb){var bX=bY.split(P);var bV=this.__dF(bX);var cg=[];var cf=[];var ca=[];var ce=[];var bW=ch;for(var i=0;i<bX.length-1;i++ ){if(bV[i]!==J){ce.push(y);}
else {var bT=this.__dy(bW,bX[i]);if(!bT){break;}
;ce.push(bT);}
...
else {var cj=qx.data.SingleValueBinding.__dy(ck,bX[j]);if(!cj){break;}
;}
...
,updateTarget:function(cm,cp,cr,cn,cq){var co=this.resolvePropertyChain(cm,cp);co=qx.data.SingleValueBinding.__dI(co,cr,cn,cq,cm);this.__dA(cr,cn,co);}
,resolvePropertyChain:function(o,cs){var ct=this.__dC(cs);return this.__dD(o,ct,ct.length);}
,__dy:function(cv,cw){var cu=this.__dJ(cv,cw);if(cu==null){if(qx.Class.supportsEvent(cv.constructor,cw)){cu=cw;}
else if(qx.Class.supportsEvent(cv.constructor,y+qx.lang.String.firstUp(cw))){cu=y+qx.lang.String.firstUp(cw);}
else {return null;}
;}
;return cu;}
,__dz:function(cB,cz){var cA=this.__dC(cz);var cy=this.__dD(cB,cA);if(cy!=null){var cC=cA[cA.length-1];var cx=this.__dB(cC);if(cx){this.__dA(cB,cz,null);return;}
;if(cy[r+qx.lang.String.firstUp(cC)]!=undefined){cy[r+qx.lang.String.firstUp(cC)]();}
else {cy[f+qx.lang.String.firstUp(cC)](null);}
;}
;}
,__dA:function(cI,cF,cG){var cH=this.__dC(cF);var cE=this.__dD(cI,cH);if(cE){var cJ=cH[cH.length-1];var cD=this.__dB(cJ);if(cD){if(cD===bi){cD=cE.length-1;}
;cE.setItem(cD,cG);}
else {return cE[f+qx.lang.String.firstUp(cJ)](cG);}
;}
;}
,__dB:function(cM){var cK=/^\[(\d+|last)\]$/;var cL=cM.match(cK);if(cL){return cL[1];}
;return null;}
,__dC:function(cN){return cN.replace(/\[/g,X).split(P).filter(function(cO){return cO!==J;}
);}
,__dD:function(cU,cP,cQ){cQ=cQ||cP.length-1;var cS=cU;for(var i=0;cS!==null&&i<cQ;i++ ){try{var cT=cP[i];var cR=this.__dB(cT);if(cR){if(cR===bi){cR=cS.length-1;}
;cS=cS.getItem(cR);}
...
;}
;return cS;}
,__dE:function(cY,dc,cX,db,cW){cY=this.__dI(cY,dc,cX,db,cW);if(cY===undefined){this.__dz(dc,cX);}
;if(cY!==undefined){try{var da=this.__dA(dc,cX,cY);if(db&&db.onUpdate){db.onUpdate(cW,dc,cY);}
;return da;}
...
;}
;}
,__dF:function(dd){var de=[];for(var i=0;i<dd.length;i++ ){var name=dd[i];if(name.endsWith(N)){var df=name.substring(name.indexOf(O)+1,name.indexOf(N));if(name.indexOf(N)!=name.length-1){throw new Error(n+name+U);}
...
;}
;return de;}
,__dG:function(dg,dj,dp,dm,dk,di){if(qx.core.Environment.get(z)){var dh=qx.Class.getEventType(dg.constructor,dj);qx.core.Assert.assertEquals(t,dh,dj+W+dg+P);}
;var dl=function(dr,e){if(dr!==J){if(dr===bi){dr=dg.length-1;}
;var du=dg.getItem(dr);if(du===undefined){qx.data.SingleValueBinding.__dz(dp,dm);}
...
;du=qx.data.SingleValueBinding.__dI(du,dp,dm,dk,dg);if(qx.core.Environment.get(a)){qx.log.Logger.debug(d+du);}
;var dt;try{if(du!==undefined){dt=qx.data.SingleValueBinding.__dA(dp,dm,du);}
else {dt=qx.data.SingleValueBinding.__dz(dp,dm);}
...
,__dH:function(dB,dw,dz,dC,dA){var dx;dx=dw.toHashCode();if(this.__du[dx]===undefined){this.__du[dx]=[];}
;var dy=[dB,dw,dz,dC,dA];this.__du[dx].push(dy);dx=dC.toHashCode();if(this.__dv[dx]===undefined){this.__dv[dx]=[];}
;this.__dv[dx].push(dy);}
,__dI:function(dG,dM,dF,dI,dD){if(dI&&dI.converter){var dJ;if(dM.getModel){dJ=dM.getModel();}
;return dI.converter(dG,dJ,dD,dM);}
else {var dH=this.__dC(dF);var dE=this.__dD(dM,dH);var dN=dF.substring(dF.lastIndexOf(P)+1,dF.length);if(dE==null){return dG;}
;var dK=qx.Class.getPropertyDefinition(dE.constructor,dN);var dL=dK==null?J:dK.check;return this.__dK(dG,dL);}
;}
,__dJ:function(dO,dQ){var dP=qx.Class.getPropertyDefinition(dO.constructor,dQ);if(dP==null){return null;}
;return dP.event;}
,__dK:function(dT,dS){var dR=qx.lang.Type.getClass(dT);if((dR==bg||dR==G)&&(dS==q||dS==Q)){dT=parseInt(dT,10);}
...
;var dX=this.getAllBindingsForObject(dU);if(dX!=undefined){for(var i=0;i<dX.length;i++ ){if(dX[i][0]==dY){var dV=dX[i][3];if(this.__dv[dV.toHashCode()]){qx.lang.Array.remove(this.__dv[dV.toHashCode()],dX[i]);}
;var dW=dX[i][1];if(this.__du[dW.toHashCode()]){qx.lang.Array.remove(this.__du[dW.toHashCode()],dX[i]);}
;return;}
;}
;}
;throw new Error(E);}
...
;}
;}
,getAllBindingsForObject:function(ei){var ej=ei.toHashCode();if(this.__du[ej]===undefined){this.__du[ej]=[];}
;var ek=this.__du[ej];var eh=this.__dv[ej]?this.__dv[ej]:[];return qx.lang.Array.unique(ek.concat(eh));}
,removeAllBindings:function(){for(var em in this.__du){var el=qx.core.ObjectRegistry.fromHashCode(em);if(el==null){delete this.__du[em];continue;}
;this.removeAllBindingsForObject(el);}
;this.__du={};}
,getAllBindings:function(){return this.__du;}
,showBindingInLog:function(eo,eq){var ep;for(var i=0;i<this.__du[eo.toHashCode()].length;i++ ){if(this.__du[eo.toHashCode()][i][0]==eq){ep=this.__du[eo.toHashCode()][i];break;}
;}
;if(ep===undefined){var en=T;}
else {var en=I+ep[1]+R+ep[2]+k+ep[3]+R+ep[4]+x;}
;qx.log.Logger.debug(en);}
,showAllBindingsInLog:function(){for(var es in this.__du){var er=qx.core.ObjectRegistry.fromHashCode(es);if(er){for(var i=0;i<this.__du[es].length;i++ ){this.showBindingInLog(er,this.__du[es][i][0]);}
;}
;}
;}
}});}
...
(function(){var a="ready",b="mshtml",c="engine.name",d="qx.event.handler.Application",f="complete",g="webkit",h="gecko",i="qx.globalErrorHandling",j="load",k="unload",l="opera",m="left",n="DOMContentLoaded",o="shutdown",p="browser.documentmode";qx.Class.define(d,{extend:qx.core.Object,implement:[qx.event.IEventHandler,qx.core.IDisposable],construct:function(q){qx.core.Object.call(this);this._window=q.getWindow();this.__dM=false;this.__dN=false;this.__dO=false;this.__dP=false;this._initObserver();qx.event.handler.Application.$$instance=this;}
...
;qx.log.Logger.debug(this,j+(new Date-qx.Bootstrap.LOADSTART)+k);var q=qx.core.Environment.get(f);var r=qx.Class.getByName(q);if(r){this.__dT=new r;var p=new Date;this.__dT.main();qx.log.Logger.debug(this,c+(new Date-p)+k);var p=new Date;this.__dT.finalize();qx.log.Logger.debug(this,m+(new Date-p)+k);}
...
;}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{input:1,change:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__mp:false,__mq:null,__mr:null,__ms:null,canHandleEvent:function(C,B){var A=C.tagName.toLowerCase();if(B===m&&(A===m||A===f)){return true;}
;if(B===k&&(A===m||A===f||A===p)){return true;}
;return false;}
,registerEvent:function(H,G,E){if(qx.core.Environment.get(v)==u&&(qx.core.Environment.get(b)<9||(qx.core.Environment.get(b)>=9&&qx.core.Environment.get(q)<9))){if(!H.__mt){var F=H.tagName.toLowerCase();var D=H.type;if(D===a||D===g||F===f||D===x||D===d){qx.bom.Event.addNativeListener(H,h,this._onPropertyWrapper);}
...
;}
;}
,__mu:qx.core.Environment.select(v,{"mshtml":function(I){if(qx.core.Environment.get(b)>=9&&qx.core.Environment.get(q)>=9){qx.bom.Event.addNativeListener(I,m,this._onInputWrapper);if(I.type===a||I.type===g||I.type===f){this._inputFixWrapper=qx.lang.Function.listener(this._inputFix,this,I);qx.bom.Event.addNativeListener(I,t,this._inputFixWrapper);}
;}
;}
...
}),unregisterEvent:function(Q,P){if(qx.core.Environment.get(v)==u&&qx.core.Environment.get(b)<9&&qx.core.Environment.get(q)<9){if(Q.__mt){var O=Q.tagName.toLowerCase();var N=Q.type;if(N===a||N===g||O===f||N===x||N===d){qx.bom.Event.removeNativeListener(Q,h,this._onPropertyWrapper);}
...
;}
;}
,__mv:qx.core.Environment.select(v,{"mshtml":function(S){if(qx.core.Environment.get(b)>=9&&qx.core.Environment.get(q)>=9){qx.bom.Event.removeNativeListener(S,m,this._onInputWrapper);if(S.type===a||S.type===g||S.type===f){qx.bom.Event.removeNativeListener(S,t,this._inputFixWrapper);}
;}
;}
...
}),_onKeyPress:qx.core.Environment.select(v,{"mshtml":function(e,X){if(e.keyCode===13){if(X.value!==this.__mr){this.__mr=X.value;qx.event.Registration.fireEvent(X,k,qx.event.type.Data,[X.value]);}
;}
;}
,"opera":function(e,Y){if(e.keyCode===13){if(Y.value!==this.__mr){this.__mr=Y.value;qx.event.Registration.fireEvent(Y,k,qx.event.type.Data,[Y.value]);}
;}
;}
,"default":null}),_inputFix:qx.core.Environment.select(v,{"mshtml":function(e,ba){if(e.keyCode===46||e.keyCode===8){if(ba.value!==this.__ms){this.__ms=ba.value;qx.event.Registration.fireEvent(ba,m,qx.event.type.Data,[ba.value]);}
;}
;}
,"default":null}),_onKeyDown:qx.core.Environment.select(v,{"opera":function(e){if(e.keyCode===13){this.__mp=true;}
;}
,"default":null}),_onKeyUp:qx.core.Environment.select(v,{"opera":function(e){if(e.keyCode===13){this.__mp=false;}
;}
,"default":null}),_onInput:qx.event.GlobalError.observeMethod(function(e){var bc=qx.bom.Event.getTarget(e);var bb=bc.tagName.toLowerCase();if(!this.__mp||bb!==m){if((qx.core.Environment.get(v)==s)&&qx.core.Environment.get(r)<10.6){this.__mq=window.setTimeout(function(){qx.event.Registration.fireEvent(bc,m,qx.event.type.Data,[bc.value]);}
,0);}
...
,__kJ:{'0':i,'2':j,'1':d},__kK:{'0':e,'1':i,'2':j,'4':d},__kL:{'1':i,'2':j,'4':d},stop:function(){this.stopPropagation();}
...
;default:if(!(qx.core.Environment.get(b)==a&&qx.core.Environment.get(h)<=8)){if(this._native.button===-1){return this.__kK[this._native.buttons]||e;}
;return this.__kJ[this._native.button]||e;}
else {return this.__kL[this._native.button]||e;}
;};}
...
(function(){var a="qx.event.handler.UserAction";qx.Class.define(a,{extend:qx.core.Object,implement:qx.event.IEventHandler,construct:function(b){qx.core.Object.call(this);this.__dt=b;this.__ct=b.getWindow();}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{useraction:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_WINDOW,IGNORE_CAN_HANDLE:true},members:{__dt:null,__ct:null,canHandleEvent:function(d,c){}
,registerEvent:function(g,f,e){}
,unregisterEvent:function(j,i,h){}
},destruct:function(){this.__dt=this.__ct=null;}
,defer:function(k){qx.event.Registration.addHandler(k);}
});}
)();
(function(){var a="touchmove",b="os.name",c="MSPointerDown",d="android",e="engine.version",f="pointercancel",g="qx.event.handler.TouchCore",h="event.mspointer",j="MSPointerCancel",k="y",l="pointer-events",m="pointerup",n="touchend",o="pointerdown",p="MSPointerUp",q="right",r="engine.name",s="undefined",t="touchcancel",u="MSPointerMove",v="webkit",w="none",z="left",A="pointermove",B="down",C="x",D="up",E="touchstart";qx.Bootstrap.define(g,{extend:Object,implement:[qx.core.IDisposable],statics:{TAP_MAX_DISTANCE:qx.core.Environment.get(b)!=d?10:40,SWIPE_DIRECTION:{x:[z,q],y:[D,B]},SWIPE_MIN_DISTANCE:qx.core.Environment.get(b)!=d?11:41,SWIPE_MIN_VELOCITY:0,LONGTAP_TIME:500},construct:function(F,G){this.__lP=F;this.__fq=G;this._initTouchObserver();this.__lQ=[];this.__lR={};}
,members:{__lP:null,__fq:null,__lS:null,__lT:null,__lR:null,__lU:null,__lV:null,__lW:null,__lQ:null,__lX:null,_initTouchObserver:function(){this.__lS=qx.lang.Function.listener(this._onTouchEvent,this);this.__lX=[E,a,n,t];if(qx.core.Environment.get(h)){var H=parseInt(qx.core.Environment.get(e),10);if(H==10){this.__lX=[c,u,p,j];}
else {this.__lX=[o,A,m,f];}
;}
;for(var i=0;i<this.__lX.length;i++ ){qx.bom.Event.addNativeListener(this.__lP,this.__lX[i],this.__lS);}
;}
,_stopTouchObserver:function(){for(var i=0;i<this.__lX.length;i++ ){qx.bom.Event.removeNativeListener(this.__lP,this.__lX[i],this.__lS);}
;}
...
;return O;}
,_calcSingleTouchDelta:function(S){if(this.__lR.hasOwnProperty(S.identifier)){var R=this.__lR[S.identifier];var P=Math.floor(S.clientX-R[0]);var Q=Math.floor(S.clientY-R[1]);var T=C;if(Math.abs(P/Q)<1){T=k;}
...
;V.delta=[];if(ba==E){this.__lT=this._getTarget(V);if(V.touches&&V.touches.length>1){this.__lV=this._getScalingDistance(V.touches[0],V.touches[1]);this.__lW=this._getRotationAngle(V.touches[0],V.touches[1]);}
;for(var i=0;i<V.changedTouches.length;i++ ){var Y=V.changedTouches[i];this.__lR[Y.identifier]=[Y.clientX,Y.clientY];}
;}
;if(ba==a){if(typeof V.scale==s&&V.targetTouches.length>1){var W=this._getScalingDistance(V.targetTouches[0],V.targetTouches[1]);V.scale=W/this.__lV;}
;if((typeof V.rotation==s||qx.core.Environment.get(h))&&V.targetTouches.length>1){var X=this._getRotationAngle(V.targetTouches[0],V.targetTouches[1]);V._rotation=X-this.__lW;}
;V.delta=this._calcTouchesDelta(V.targetTouches);}
;this._fireEvent(V,ba,this.__lT);if(qx.core.Environment.get(h)){if(ba==n||ba==t){delete this.__lQ[V.pointerId];}
;}
;if((ba==n||ba==t)&&V.changedTouches[0]){delete this.__lR[V.changedTouches[0].identifier];}
;}
,_detectTouchesByPointer:function(bd,bf){var bc=[];if(bf==E){this.__lQ[bd.pointerId]=bd;}
else if(bf==a){this.__lQ[bd.pointerId]=bd;}
;for(var be in this.__lQ){var bb=this.__lQ[be];bc.push(bb);}
;return bc;}
...
;}
else if(qx.core.Environment.get(h)){var bh=this.__lY(bi);if(bh){bj=bh;}
;}
;return bj;}
,__lY:function(bm){var bk=null;var bl=null;if(bm&&bm.touches&&bm.touches.length!==0){bk=bm.touches[0].clientX;bl=bm.touches[0].clientY;}
...
;var br=br||bq.type;if(bs&&bs.nodeType&&this.__fq){this.__fq.emit(br,bq);}
;}
,dispose:function(){this._stopTouchObserver();this.__lT=this.__lP=this.__lX=this.__lQ=this.__fq=this.__lV=this.__lW=null;}
}});}
)();
...
(function(){var a="cursor:",b="engine.name",c="",d="mshtml",e="nw-resize",f="engine.version",g="nesw-resize",h="browser.documentmode",i=";",j="nwse-resize",k="qx.bom.element.Cursor",l="ne-resize",m="browser.quirksmode",n="cursor";qx.Bootstrap.define(k,{statics:{__eK:{},compile:function(o){return a+(this.__eK[o]||o)+i;}
,get:function(q,p){return qx.bom.element.Style.get(q,n,p,false);}
,set:function(s,r){s.style.cursor=this.__eK[r]||r;}
,reset:function(t){t.style.cursor=c;}
},defer:function(u){if(qx.core.Environment.get(b)==d&&((parseFloat(qx.core.Environment.get(f))<9||qx.core.Environment.get(h)<9)&&!qx.core.Environment.get(m))){u.__eK[g]=l;u.__eK[j]=e;}
;}
});}
)();
...
;if(qx.bom.Document.isStandardMode(qx.dom.Node.getWindow(p))){if(!this.__eM(p)){return i;}
;}
;return b;}
...
,getLinearGradient:function(){qx.bom.client.Css.__eN=false;var cj=Q;var cg=document.createElement(o);var ch=qx.bom.Style.getAppliedStyle(cg,J,cj);if(!ch){cj=bN;var ch=qx.bom.Style.getAppliedStyle(cg,J,cj,false);if(ch){qx.bom.client.Css.__eN=true;}
;}
;if(!ch){return null;}
;var ci=/(.*?)\(/.exec(ch);return ci?ci[1]:null;}
,getFilterGradient:function(){return qx.bom.client.Css.__eO(bD,bA);}
...
,getLegacyWebkitGradient:function(){if(qx.bom.client.Css.__eN===null){qx.bom.client.Css.getLinearGradient();}
;return qx.bom.client.Css.__eN;}
...
,getFilterTextShadow:function(){return qx.bom.client.Css.__eO(L,W);}
,__eO:function(cv,ct){var cu=false;var cw=bR+cv+bG+ct+bJ;var cs=document.createElement(o);document.body.appendChild(cs);cs.style.filter=cw;if(cs.filters&&cs.filters.length>0&&cs.filters.item(cv).enabled==true){cu=true;}
...
(function(){var a="resize",b="os.name",c="qx.event.handler.Orientation",d="landscape",e="android",f="portrait",g="orientationchange";qx.Class.define(c,{extend:qx.core.Object,implement:[qx.event.IEventHandler,qx.core.IDisposable],construct:function(h){qx.core.Object.call(this);this.__dt=h;this.__ct=h.getWindow();this._initObserver();}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{orientationchange:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_WINDOW,IGNORE_CAN_HANDLE:true},members:{__dt:null,__ct:null,__ma:null,_currentOrientation:null,__mb:null,canHandleEvent:function(j,i){}
,registerEvent:function(m,l,k){}
,unregisterEvent:function(p,o,n){}
,_initObserver:function(){this.__mb=qx.lang.Function.listener(this._onNative,this);this.__ma=qx.bom.Event.supportsEvent(this.__ct,g)?g:a;var Event=qx.bom.Event;Event.addNativeListener(this.__ct,this.__ma,this.__mb);}
,_stopObserver:function(){var Event=qx.bom.Event;Event.removeNativeListener(this.__ct,this.__ma,this.__mb);}
...
;}
},destruct:function(){this._stopObserver();this.__dt=this.__ct=null;}
,defer:function(w){qx.event.Registration.addHandler(w);}
});}
)();
(function(){var a="landscape",b="qx.event.type.Orientation",c="portrait";qx.Class.define(b,{extend:qx.event.type.Event,members:{__mc:null,__iq:null,init:function(d,e){qx.event.type.Event.prototype.init.call(this,false,false);this.__mc=d;this.__iq=e;return this;}
,clone:function(f){var g=qx.event.type.Event.prototype.clone.call(this,f);g.__mc=this.__mc;g.__iq=this.__iq;return g;}
,getOrientation:function(){return this.__mc;}
,isLandscape:function(){return this.__iq==a;}
,isPortrait:function(){return this.__iq==c;}
}});}
)();
(function(){var a="touchmove",b="dispose",c="useraction",d="touchend",e="event.touch",f="touchstart",g="qx.event.handler.Touch";qx.Class.define(g,{extend:qx.event.handler.TouchCore,implement:[qx.event.IEventHandler,qx.core.IDisposable],construct:function(h){this.__dt=h;this.__ct=h.getWindow();this.__dW=this.__ct.document;qx.event.handler.TouchCore.apply(this,[this.__dW]);}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{touchstart:1,touchmove:1,touchend:1,touchcancel:1,tap:1,longtap:1,swipe:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE+qx.event.IEventHandler.TARGET_DOCUMENT,IGNORE_CAN_HANDLE:true,MOUSE_TO_TOUCH_MAPPING:{"mousedown":f,"mousemove":a,"mouseup":d}},members:{__dt:null,__ct:null,__dW:null,__md:false,canHandleEvent:function(j,i){}
,registerEvent:function(m,l,k){}
,unregisterEvent:function(p,o,n){}
...
),dispose:function(){this.__kM(b);this.__dt=this.__ct=this.__dW=null;}
,__kM:function(w,v){qx.event.handler.TouchCore.prototype[w].apply(this,v||[]);}
...
;if(!this.__fu){bh.stopPropagation();}
...
;bl[this._processedFlag]=true;var bm=qx.event.handler.PointerCore.TOUCH_TO_POINTER_MAPPING[bl.type];var bo=bl.changedTouches;this._determineActiveTouches(bl.type,bo);if(bl.touches.length<this.__ft.length){for(var i=this.__ft.length-1;i>=0;i-- ){var bq=new qx.event.type.dom.Pointer(o,bl,{identifier:this.__ft[i].identifier,target:bl.target,pointerType:h,pointerId:this.__ft[i].identifier+2});this._fireEvent(bq,o,bl.target);}
;this.__fx=null;this.__ft=[];return;}
;if(bl.type==q&&this.__fx===null){this.__fx=bo[0].identifier;}
...
;if(bp.identifier==this.__fx){bk.isPrimary=true;bk.button=0;bk.buttons=1;qx.event.handler.PointerCore.__fo={"x":bp.clientX,"y":bp.clientY,"time":new Date().getTime()};}
;var br=new qx.event.type.dom.Pointer(bm,bl,bk);this._fireEvent(br,bm,bk.target);if(bl.type==B||bl.type==N){var bs=new qx.event.type.dom.Pointer(W,bl,bk);this._fireEvent(bs,W,bl.target);if(this.__fx==bp.identifier){this.__fx=null;}
;}
;}
;}
...
;if(bt.type==c){this.__fs[bt.which]=1;}
else if(bt.type==M){if(qx.core.Environment.get(b)==C&&qx.core.Environment.get(L)==J){if(this.__fs[bt.which]!=1&&bt.ctrlKey){this.__fs[1]=0;}
;}
;this.__fs[bt.which]=0;}
;var bv=qx.event.handler.PointerCore.MOUSE_TO_POINTER_MAPPING[bt.type];var bu=qx.bom.Event.getTarget(bt);var bw=qx.lang.Array.sum(this.__fs);var bz={pointerType:s,pointerId:1};if(this.__fw!=bw&&bw!==0&&this.__fw!==0){var bx=new qx.event.type.dom.Pointer(H,bt,bz);this._fireEvent(bx,H,bu);}
;this.__fw=bw;if(bt.type==c&&bw>1){return;}
;if(bt.type==M&&bw>0){return;}
;if(bt.type==O){this.__fs[bt.which]=0;return;}
;var by=new qx.event.type.dom.Pointer(bv,bt,bz);this._fireEvent(by,bv,bu);}
,_determineActiveTouches:function(bD,bC){if(bD==q){for(var i=0;i<bC.length;i++ ){this.__ft.push(bC[i]);}
;}
else if(bD==B||bD==N){var bA=[];for(var i=0;i<this.__ft.length;i++ ){var bB=true;for(var j=0;j<bC.length;j++ ){if(this.__ft[i].identifier==bC[j].identifier){bB=false;break;}
;}
;if(bB){bA.push(this.__ft[i]);}
;}
;this.__ft=bA;}
;}
,_isSimulatedMouseEvent:function(x,y){var bF=qx.event.handler.PointerCore.__fo;if(bF){var bG=new Date().getTime()-bF.time;var bE=qx.event.handler.PointerCore.SIM_MOUSE_DISTANCE;var bI=Math.abs(x-qx.event.handler.PointerCore.__fo.x);var bH=Math.abs(y-qx.event.handler.PointerCore.__fo.y);if(bG<qx.event.handler.PointerCore.SIM_MOUSE_DELAY){if(bI<bE||bH<bE){return true;}
;}
;}
;return false;}
,_stopObserver:function(){for(var i=0;i<this.__fr.length;i++ ){qx.bom.Event.removeNativeListener(this.__fp,this.__fr[i],this.__fv);}
;}
...
;}
;if(qx.core.Environment.get(d)){if(!this.__fu){bL.dispatchEvent(bK);}
...
;}
;}
,dispose:function(){this._stopObserver();this.__fp=this.__fq=null;}
}});}
)();
(function(){var a="qx.event.Emitter",b="*";qx.Bootstrap.define(a,{extend:Object,statics:{__fy:[]},members:{__fm:null,__fz:null,on:function(name,c,d){var e=qx.event.Emitter.__fy.length;this.__fA(name).push({listener:c,ctx:d,id:e,name:name});qx.event.Emitter.__fy.push({name:name,listener:c,ctx:d});return e;}
,once:function(name,f,g){var h=qx.event.Emitter.__fy.length;this.__fA(name).push({listener:f,ctx:g,once:true,id:h});qx.event.Emitter.__fy.push({name:name,listener:f,ctx:g});return h;}
,off:function(name,m,k){var l=this.__fA(name);for(var i=l.length-1;i>=0;i-- ){var n=l[i];if(n.listener==m&&n.ctx==k){l.splice(i,1);qx.event.Emitter.__fy[n.id]=null;return n.id;}
;}
;return null;}
...
,emit:function(name,A){var x=this.__fA(name).concat();var y=[];for(var i=0;i<x.length;i++ ){var z=x[i];z.listener.call(z.ctx,A);if(z.once){y.push(z);}
;}
;y.forEach(function(B){var C=this.__fA(name);var D=C.indexOf(B);C.splice(D,1);}
.bind(this));x=this.__fA(b);for(var i=x.length-1;i>=0;i-- ){var z=x[i];z.listener.call(z.ctx,A);}
;}
,getListeners:function(){return this.__fm;}
,getEntryById:function(F){for(var name in this.__fm){var E=this.__fm[name];for(var i=0,j=E.length;i<j;i++ ){if(E[i].id===F){return E[i];}
;}
;}
;}
,__fA:function(name){if(this.__fm==null){this.__fm={};}
...
,dispose:function(){this.__kM(d);this.__dt=this.__ct=this.__dW=null;}
,__kM:function(D,C){qx.event.handler.PointerCore.prototype[D].apply(this,C||[]);}
},defer:function(E){qx.event.Registration.addHandler(E);qx.event.Registration.getManager(document).getHandler(E);}
});}
)();
(function(){var a="offline",b="qx.event.handler.Offline",c="online";qx.Class.define(b,{extend:qx.core.Object,implement:[qx.event.IEventHandler,qx.core.IDisposable],construct:function(d){qx.core.Object.call(this);this.__dt=d;this.__ct=d.getWindow();this._initObserver();}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{online:true,offline:true},TARGET_CHECK:qx.event.IEventHandler.TARGET_WINDOW,IGNORE_CAN_HANDLE:true},members:{__dt:null,__ct:null,__mb:null,canHandleEvent:function(f,e){}
,registerEvent:function(i,h,g){}
,unregisterEvent:function(l,k,j){}
,_initObserver:function(){this.__mb=qx.lang.Function.listener(this._onNative,this);qx.bom.Event.addNativeListener(this.__ct,a,this.__mb);qx.bom.Event.addNativeListener(this.__ct,c,this.__mb);}
,_stopObserver:function(){qx.bom.Event.removeNativeListener(this.__ct,a,this.__mb);qx.bom.Event.removeNativeListener(this.__ct,c,this.__mb);}
...
},destruct:function(){this.__dt=null;this._stopObserver();delete qx.event.handler.Appear.__instances[this.$$hash];}
,defer:function(n){qx.event.Registration.addHandler(n);}
});}
)();
(function(){var a="qx.event.handler.Appear",b="engine.name",c="mshtml",d="disappear",e="appear",f="browser.documentmode";qx.Class.define(a,{extend:qx.core.Object,implement:[qx.event.IEventHandler,qx.core.IDisposable],construct:function(g){qx.core.Object.call(this);this.__dt=g;this.__lM={};qx.event.handler.Appear.__lN[this.$$hash]=this;}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{appear:true,disappear:true},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:true,__lN:{},refresh:function(){var h=this.__lN;for(var i in h){h[i].refresh();}
;}
},members:{__dt:null,__lM:null,canHandleEvent:function(k,j){}
,registerEvent:function(o,p,m){var n=qx.core.ObjectRegistry.toHashCode(o)+p;var l=this.__lM;if(l&&!l[n]){l[n]=o;o.$$displayed=o.offsetWidth>0;}
;}
,unregisterEvent:function(t,u,r){var s=qx.core.ObjectRegistry.toHashCode(t)+u;var q=this.__lM;if(!q){return;}
;if(q[s]){delete q[s];}
;}
,refresh:function(){var A=this.__lM;var x;var y=qx.core.Environment.get(b)==c&&qx.core.Environment.get(f)<9;for(var v in A){x=A[v];var w=x.offsetWidth>0;if(!w&&y){w=x.offsetWidth>0;}
;if((!!x.$$displayed)!==w){x.$$displayed=w;var z=qx.event.Registration.createEvent(w?e:d);this.__dt.dispatchEvent(x,z);}
;}
;}
},destruct:function(){this.__dt=this.__lM=null;delete qx.event.handler.Appear.__lN[this.$$hash];}
,defer:function(B){qx.event.Registration.addHandler(B);}
});}
)();
(function(){var a="dblclick",b="os.name",c="mouseup",d="mousedown",e="useraction",f="webkit",g="contextmenu",h="mousewheel",i="auxclick",j="mouseover",k="mouseout",l="gecko",m="ios",n="click",o="mousemove",p="qx.event.handler.Mouse",q="on",r="engine.name";qx.Class.define(p,{extend:qx.core.Object,implement:[qx.event.IEventHandler,qx.core.IDisposable],construct:function(s){qx.core.Object.call(this);this.__dt=s;this.__ct=s.getWindow();this.__dW=this.__ct.document;this._initButtonObserver();this._initMoveObserver();this._initWheelObserver();}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{mousemove:1,mouseover:1,mouseout:1,mousedown:1,mouseup:1,click:1,auxclick:1,dblclick:1,contextmenu:1,mousewheel:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE+qx.event.IEventHandler.TARGET_DOCUMENT+qx.event.IEventHandler.TARGET_WINDOW,IGNORE_CAN_HANDLE:true},members:{__mg:null,__mh:null,__mi:null,__mj:null,__mk:null,__dt:null,__ct:null,__dW:null,__ml:null,canHandleEvent:function(u,t){}
...
,preventNextClick:function(){this.__ml=true;}
,_initButtonObserver:function(){this.__mg=qx.lang.Function.listener(this._onButtonEvent,this);var Event=qx.bom.Event;Event.addNativeListener(this.__dW,d,this.__mg);Event.addNativeListener(this.__dW,c,this.__mg);Event.addNativeListener(this.__dW,n,this.__mg);Event.addNativeListener(this.__dW,i,this.__mg);Event.addNativeListener(this.__dW,a,this.__mg);Event.addNativeListener(this.__dW,g,this.__mg);}
,_initMoveObserver:function(){this.__mh=qx.lang.Function.listener(this._onMoveEvent,this);var Event=qx.bom.Event;Event.addNativeListener(this.__dW,o,this.__mh);Event.addNativeListener(this.__dW,j,this.__mh);Event.addNativeListener(this.__dW,k,this.__mh);}
,_initWheelObserver:function(){this.__mi=qx.lang.Function.listener(this._onWheelEvent,this);var E=qx.bom.client.Event.getMouseWheel(this.__ct);qx.bom.Event.addNativeListener(E.target,E.type,this.__mi);}
,_stopButtonObserver:function(){var Event=qx.bom.Event;Event.removeNativeListener(this.__dW,d,this.__mg);Event.removeNativeListener(this.__dW,c,this.__mg);Event.removeNativeListener(this.__dW,n,this.__mg);Event.removeNativeListener(this.__dW,a,this.__mg);Event.removeNativeListener(this.__dW,g,this.__mg);}
,_stopMoveObserver:function(){var Event=qx.bom.Event;Event.removeNativeListener(this.__dW,o,this.__mh);Event.removeNativeListener(this.__dW,j,this.__mh);Event.removeNativeListener(this.__dW,k,this.__mh);}
,_stopWheelObserver:function(){var F=qx.bom.client.Event.getMouseWheel(this.__ct);qx.bom.Event.removeNativeListener(F.target,F.type,this.__mi);}
,_onMoveEvent:qx.event.GlobalError.observeMethod(function(G){this.__cM(G);}
),_onButtonEvent:qx.event.GlobalError.observeMethod(function(J){var I=J.type;var K=qx.bom.Event.getTarget(J);if(I==n&&this.__ml){delete this.__ml;return;}
;if(qx.core.Environment.get(r)==l||qx.core.Environment.get(r)==f){if(K&&K.nodeType==3){K=K.parentNode;}
;}
;var H=qx.event.handler.DragDrop&&this.__dt.getHandler(qx.event.handler.DragDrop).isSessionActive();if(H&&I==n){return;}
;if(this.__mn){this.__mn(J,I,K);}
;this.__cM(J,I,K);if(I==i&&J.button==1){this.__cM(J,n,K);}
;if(this.__mm){this.__mm(J,I,K);}
;if(this.__mo&&!H){this.__mo(J,I,K);}
;this.__mj=I;}
),_onWheelEvent:qx.event.GlobalError.observeMethod(function(L){this.__cM(L,h);}
),__mm:qx.core.Environment.select(r,{"opera":function(M,N,O){if(N==c&&M.button==2){this.__cM(M,g,O);}
;}
,"default":null}),__mn:qx.core.Environment.select(r,{"mshtml":function(P,Q,R){if(P.target!==undefined){return;}
;if(Q==c&&this.__mj==n){this.__cM(P,d,R);}
else if(Q==a){this.__cM(P,n,R);}
;}
,"default":null}),__mo:qx.core.Environment.select(r,{"mshtml":null,"default":function(T,S,U){switch(S){case d:this.__mk=U;break;case c:if(U!==this.__mk){var V=qx.dom.Hierarchy.getCommonParent(U,this.__mk);if(V){this.__cM(T,n,V);}
;}
;};}
})},destruct:function(){this._stopButtonObserver();this._stopMoveObserver();this._stopWheelObserver();this.__dt=this.__ct=this.__dW=this.__mk=null;}
,defer:function(W){qx.event.Registration.addHandler(W);}
});}
)();
...
;return this.__lo(h);}
;if(f===b){var x=0;if(g.wheelDelta!==undefined){if(g.wheelDeltaX!==undefined){x=g.wheelDeltaX?this.__lo(-g.wheelDeltaX):0;}
;}
else {if(g.axis&&g.axis==g.HORIZONTAL_AXIS&&(g.detail!==undefined)&&(g.detail>0)){x=this.__lo(g.detail);}
else if(g.deltaX!==undefined){x=this.__lo(g.deltaX);}
;}
;return x;}
;if(f===c){var y=0;if(g.wheelDelta!==undefined){if(g.wheelDeltaY!==undefined){y=g.wheelDeltaY?this.__lo(-g.wheelDeltaY):0;}
else {y=this.__lo(-g.wheelDelta);}
;}
else {if(!(g.axis&&g.axis==g.HORIZONTAL_AXIS)&&(g.detail!==undefined)&&(g.detail>0)){y=this.__lo(g.detail);}
else if(g.deltaY!==undefined){y=this.__lo(g.deltaY);}
;}
;return y;}
;return 0;}
,__lo:function(l){if(qx.util.Wheel.IS_TOUCHPAD){qx.util.Wheel.MINSCROLL=null;qx.util.Wheel.MAXSCROLL=null;qx.util.Wheel.FACTOR=1;return l;}
;var i=Math.abs(l);if(i===0){return 0;}
;if(qx.util.Wheel.MINSCROLL==null||qx.util.Wheel.MINSCROLL>i){qx.util.Wheel.MINSCROLL=i;this.__lp();}
;if(qx.util.Wheel.MAXSCROLL==null||qx.util.Wheel.MAXSCROLL<i){qx.util.Wheel.MAXSCROLL=i;this.__lp();}
...
,__lp:function(){var m=qx.util.Wheel.MAXSCROLL||0;var p=qx.util.Wheel.MINSCROLL||m;if(m<=p){return;}
...
(function(){var a="mshtml",b="engine.name",c="keypress",d="useraction",e="win",f="text",g="keyinput",h="os.name",i="webkit",j="input",k="gecko",l="browser.version",m="off",n="keydown",o="autoComplete",p="keyup",q="qx.event.handler.Keyboard";qx.Class.define(q,{extend:qx.core.Object,implement:[qx.event.IEventHandler,qx.core.IDisposable],construct:function(r){qx.core.Object.call(this);this.__dt=r;this.__ct=r.getWindow();if((qx.core.Environment.get(b)==k)){this.__dW=this.__ct;}
else {this.__dW=this.__ct.document.documentElement;}
;this.__kA={};this._initKeyObserver();}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{keyup:1,keydown:1,keypress:1,keyinput:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:true},members:{__kB:null,__dt:null,__ct:null,__dW:null,__kA:null,__kC:null,__kD:null,__kE:null,canHandleEvent:function(t,s){}
,registerEvent:function(w,v,u){}
,unregisterEvent:function(z,y,x){}
,_fireInputEvent:function(B,A){var C=this.__kF();if(C&&C.offsetWidth!=0){var event=qx.event.Registration.createEvent(g,qx.event.type.KeyInput,[B,C,A]);this.__dt.dispatchEvent(C,event);}
;if(this.__ct){qx.event.Registration.fireEvent(this.__ct,d,qx.event.type.Data,[g]);}
;}
,_fireSequenceEvent:function(E,G,D){var F=this.__kF();var H=E.keyCode;var event=qx.event.Registration.createEvent(G,qx.event.type.KeySequence,[E,F,D]);this.__dt.dispatchEvent(F,event);if(event.getDefaultPrevented()&&G==n){if(qx.core.Environment.get(b)==a||qx.core.Environment.get(b)==i||(qx.core.Environment.get(b)==k&&qx.core.Environment.get(l)>=29)){if(!qx.event.util.Keyboard.isNonPrintableKeyCode(H)&&!this._emulateKeyPress[H]){this._fireSequenceEvent(E,c,D);}
;}
;}
;if(this.__ct){qx.event.Registration.fireEvent(this.__ct,d,qx.event.type.Data,[G]);}
;}
,__kF:function(){var I=this.__dt.getHandler(qx.event.handler.Focus);var J=I.getActive();if(!J||J.offsetWidth==0){J=I.getFocus();}
;if(!J||J.offsetWidth==0){J=this.__dt.getWindow().document.body;}
;return J;}
,_initKeyObserver:function(){this.__kB=qx.lang.Function.listener(this.__kG,this);this.__kE=qx.lang.Function.listener(this.__kI,this);var Event=qx.bom.Event;Event.addNativeListener(this.__dW,p,this.__kB);Event.addNativeListener(this.__dW,n,this.__kB);Event.addNativeListener(this.__dW,c,this.__kE);}
,_stopKeyObserver:function(){var Event=qx.bom.Event;Event.removeNativeListener(this.__dW,p,this.__kB);Event.removeNativeListener(this.__dW,n,this.__kB);Event.removeNativeListener(this.__dW,c,this.__kE);for(var L in (this.__kD||{})){var K=this.__kD[L];Event.removeNativeListener(K.target,c,K.callback);}
;delete (this.__kD);}
,__kG:qx.event.GlobalError.observeMethod(qx.core.Environment.select(b,{"mshtml":function(O){O=window.event||O;var P=O.keyCode;var N=0;var M=O.type;if(!(this.__kA[P]==n&&M==n)){this._idealKeyHandler(P,N,M,O);}
;if(M==n){if(qx.event.util.Keyboard.isNonPrintableKeyCode(P)||this._emulateKeyPress[P]){this._idealKeyHandler(P,N,c,O);}
;}
;this.__kA[P]=M;}
,"gecko":function(R){var T=0;var V=R.keyCode;var U=R.type;var S=qx.event.util.Keyboard;if(qx.core.Environment.get(h)==e){var Q=V?S.keyCodeToIdentifier(V):S.charCodeToIdentifier(T);if(!(this.__kA[Q]==n&&U==n)){this._idealKeyHandler(V,T,U,R);}
;this.__kA[Q]=U;}
else {this._idealKeyHandler(V,T,U,R);}
;this.__kH(R.target,U,V);}
,"webkit":function(Y){var ba=0;var X=0;var W=Y.type;ba=Y.keyCode;this._idealKeyHandler(ba,X,W,Y);if(W==n){if(qx.event.util.Keyboard.isNonPrintableKeyCode(ba)||this._emulateKeyPress[ba]){this._idealKeyHandler(ba,X,c,Y);}
;}
;this.__kA[ba]=W;}
,"opera":function(bb){this.__kC=bb.keyCode;this._idealKeyHandler(bb.keyCode,0,bb.type,bb);}
})),__kH:qx.core.Environment.select(b,{"gecko":function(bd,bf,bg){if(bf===n&&(bg==33||bg==34||bg==38||bg==40)&&bd.type==f&&bd.tagName.toLowerCase()===j&&bd.getAttribute(o)!==m){if(!this.__kD){this.__kD={};}
;var bc=qx.core.ObjectRegistry.toHashCode(bd);if(this.__kD[bc]){return;}
;var self=this;this.__kD[bc]={target:bd,callback:function(bh){qx.bom.Event.stopPropagation(bh);self.__kI(bh);}
};var be=qx.event.GlobalError.observeMethod(this.__kD[bc].callback);qx.bom.Event.addNativeListener(bd,c,be);}
;}
,"default":null}),__kI:qx.event.GlobalError.observeMethod(qx.core.Environment.select(b,{"mshtml":function(bi){bi=window.event||bi;if(this._charCode2KeyCode[bi.keyCode]){this._idealKeyHandler(this._charCode2KeyCode[bi.keyCode],0,bi.type,bi);}
else {this._idealKeyHandler(0,bi.keyCode,bi.type,bi);}
;}
,"gecko":function(bj){var bk=bj.charCode;var bl=bj.type;this._idealKeyHandler(bj.keyCode,bk,bl,bj);}
,"webkit":function(bm){if(this._charCode2KeyCode[bm.keyCode]){this._idealKeyHandler(this._charCode2KeyCode[bm.keyCode],0,bm.type,bm);}
else {this._idealKeyHandler(0,bm.keyCode,bm.type,bm);}
;}
,"opera":function(bn){var bp=bn.keyCode;var bo=bn.type;if(bp!=this.__kC){this._idealKeyHandler(0,this.__kC,bo,bn);}
else {if(qx.event.util.Keyboard.keyCodeToIdentifierMap[bn.keyCode]){this._idealKeyHandler(bn.keyCode,0,bn.type,bn);}
else {this._idealKeyHandler(0,bn.keyCode,bn.type,bn);}
;}
;}
})),_idealKeyHandler:function(bt,br,bu,bs){var bq;if(bt||(!bt&&!br)){bq=qx.event.util.Keyboard.keyCodeToIdentifier(bt);this._fireSequenceEvent(bs,bu,bq);}
else {bq=qx.event.util.Keyboard.charCodeToIdentifier(br);this._fireSequenceEvent(bs,c,bq);this._fireInputEvent(bs,br);}
;}
,_emulateKeyPress:qx.core.Environment.select(b,{"mshtml":{'8':true,'9':true},"webkit":{'8':true,'9':true,'27':true},"default":{}}),_identifierToKeyCode:function(bv){return qx.event.util.Keyboard.identifierToKeyCodeMap[bv]||bv.charCodeAt(0);}
},destruct:function(){this._stopKeyObserver();this.__kC=this.__dt=this.__ct=this.__dW=this.__kA=null;}
,defer:function(bw,bx){qx.event.Registration.addHandler(bw);if((qx.core.Environment.get(b)==a)||qx.core.Environment.get(b)==i){bx._charCode2KeyCode={'13':13,'27':27};}
;}
});}
)();
...
;}
,tryActivate:function(bb){var ba=this.__hI(bb);if(ba){this.setActive(ba);}
;}
,__cM:function(bc,be,bh,bg){var bf=qx.event.Registration;var bd=bf.createEvent(bh,qx.event.type.Focus,[bc,be,bg]);bf.dispatchEvent(bc,bd);}
,_windowFocused:true,__hu:function(){if(this._windowFocused){this._windowFocused=false;this.__cM(this._window,null,c,false);}
;}
,__hv:function(){if(!this._windowFocused){this._windowFocused=true;this.__cM(this._window,null,f,false);}
;}
,_initObserver:qx.core.Environment.select(A,{"gecko":function(){this.__hh=qx.lang.Function.listener(this.__hB,this);this.__hi=qx.lang.Function.listener(this.__hC,this);this.__hj=qx.lang.Function.listener(this.__hA,this);this.__hk=qx.lang.Function.listener(this.__hz,this);this.__hl=qx.lang.Function.listener(this.__hw,this);qx.bom.Event.addNativeListener(this._document,d,this.__hh,true);qx.bom.Event.addNativeListener(this._document,B,this.__hi,true);qx.bom.Event.addNativeListener(this._window,f,this.__hj,true);qx.bom.Event.addNativeListener(this._window,c,this.__hk,true);qx.bom.Event.addNativeListener(this._window,F,this.__hl,true);}
,"mshtml":function(){this.__hh=qx.lang.Function.listener(this.__hB,this);this.__hi=qx.lang.Function.listener(this.__hC,this);this.__hn=qx.lang.Function.listener(this.__hx,this);this.__ho=qx.lang.Function.listener(this.__hy,this);this.__hm=qx.lang.Function.listener(this.__hF,this);qx.bom.Event.addNativeListener(this._document,d,this.__hh);qx.bom.Event.addNativeListener(this._document,B,this.__hi);qx.bom.Event.addNativeListener(this._document,y,this.__hn);qx.bom.Event.addNativeListener(this._document,D,this.__ho);qx.bom.Event.addNativeListener(this._document,a,this.__hm);}
,"webkit":qx.core.Environment.select(l,{"edge":function(bi){this.__hh=qx.lang.Function.listener(this.__hB,this);this.__hi=qx.lang.Function.listener(this.__hC,this);this.__ho=qx.lang.Function.listener(this.__hy,this);this.__hn=qx.lang.Function.listener(this.__hx,this);this.__hm=qx.lang.Function.listener(this.__hF,this);qx.bom.Event.addNativeListener(this._document,d,this.__hh,true);qx.bom.Event.addNativeListener(this._document,B,this.__hi,true);qx.bom.Event.addNativeListener(this._document,a,this.__hm,false);qx.bom.Event.addNativeListener(this._document,y,this.__hn);qx.bom.Event.addNativeListener(this._document,D,this.__ho);}
,"default":function(bj){this.__hh=qx.lang.Function.listener(this.__hB,this);this.__hi=qx.lang.Function.listener(this.__hC,this);this.__ho=qx.lang.Function.listener(this.__hy,this);this.__hj=qx.lang.Function.listener(this.__hA,this);this.__hk=qx.lang.Function.listener(this.__hz,this);this.__hm=qx.lang.Function.listener(this.__hF,this);qx.bom.Event.addNativeListener(this._document,d,this.__hh,true);qx.bom.Event.addNativeListener(this._document,B,this.__hi,true);qx.bom.Event.addNativeListener(this._document,a,this.__hm,false);qx.bom.Event.addNativeListener(this._window,C,this.__ho,true);qx.bom.Event.addNativeListener(this._window,f,this.__hj,true);qx.bom.Event.addNativeListener(this._window,c,this.__hk,true);}
}),"opera":function(){this.__hh=qx.lang.Function.listener(this.__hB,this);this.__hi=qx.lang.Function.listener(this.__hC,this);this.__hn=qx.lang.Function.listener(this.__hx,this);this.__ho=qx.lang.Function.listener(this.__hy,this);qx.bom.Event.addNativeListener(this._document,d,this.__hh,true);qx.bom.Event.addNativeListener(this._document,B,this.__hi,true);qx.bom.Event.addNativeListener(this._window,j,this.__hn,true);qx.bom.Event.addNativeListener(this._window,C,this.__ho,true);}
}),_stopObserver:qx.core.Environment.select(A,{"gecko":function(){qx.bom.Event.removeNativeListener(this._document,d,this.__hh,true);qx.bom.Event.removeNativeListener(this._document,B,this.__hi,true);qx.bom.Event.removeNativeListener(this._window,f,this.__hj,true);qx.bom.Event.removeNativeListener(this._window,c,this.__hk,true);qx.bom.Event.removeNativeListener(this._window,F,this.__hl,true);}
,"mshtml":function(){qx.bom.Event.removeNativeListener(this._document,d,this.__hh);qx.bom.Event.removeNativeListener(this._document,B,this.__hi);qx.bom.Event.removeNativeListener(this._document,y,this.__hn);qx.bom.Event.removeNativeListener(this._document,D,this.__ho);qx.bom.Event.removeNativeListener(this._document,a,this.__hm);}
,"webkit":qx.core.Environment.select(l,{"edge":function(){qx.bom.Event.removeNativeListener(this._document,d,this.__hh);qx.bom.Event.removeNativeListener(this._document,B,this.__hi);qx.bom.Event.removeNativeListener(this._document,y,this.__hn);qx.bom.Event.removeNativeListener(this._document,D,this.__ho);qx.bom.Event.removeNativeListener(this._document,a,this.__hm);}
,"default":function(){qx.bom.Event.removeNativeListener(this._document,d,this.__hh,true);qx.bom.Event.removeNativeListener(this._document,B,this.__hi,true);qx.bom.Event.removeNativeListener(this._document,a,this.__hm,false);qx.bom.Event.removeNativeListener(this._window,C,this.__ho,true);qx.bom.Event.removeNativeListener(this._window,f,this.__hj,true);qx.bom.Event.removeNativeListener(this._window,c,this.__hk,true);}
}),"opera":function(){qx.bom.Event.removeNativeListener(this._document,d,this.__hh,true);qx.bom.Event.removeNativeListener(this._document,B,this.__hi,true);qx.bom.Event.removeNativeListener(this._window,j,this.__hn,true);qx.bom.Event.removeNativeListener(this._window,C,this.__ho,true);}
}),__hw:qx.event.GlobalError.observeMethod(qx.core.Environment.select(A,{"gecko":function(bk){var bl=qx.bom.Event.getTarget(bk);if(!this.__hJ(bl)){qx.bom.Event.preventDefault(bk);}
;}
,"default":null})),__hx:qx.event.GlobalError.observeMethod(qx.core.Environment.select(A,{"mshtml":function(bn){this.__hv();var bo=qx.bom.Event.getTarget(bn);var bm=this.__hH(bo);if(bm){this.setFocus(bm);}
;this.tryActivate(bo);}
,"webkit":qx.core.Environment.select(l,{"edge":function(bq){this.__hv();var br=qx.bom.Event.getTarget(bq);var bp=this.__hH(br);if(bp){this.setFocus(bp);}
;this.tryActivate(br);}
,"default":null}),"opera":function(bs){var bt=qx.bom.Event.getTarget(bs);if(bt==this._document||bt==this._window){this.__hv();if(this.__hp){this.setFocus(this.__hp);delete this.__hp;}
;if(this.__hq){this.setActive(this.__hq);delete this.__hq;}
;}
else {this.setFocus(bt);this.tryActivate(bt);if(!this.__hJ(bt)){bt.selectionStart=0;bt.selectionEnd=0;}
;}
;}
,"default":null})),__hy:qx.event.GlobalError.observeMethod(qx.core.Environment.select(A,{"mshtml":function(bu){var bv=qx.bom.Event.getRelatedTarget(bu);if(bv==null){this.__hu();this.resetFocus();this.resetActive();}
;}
,"webkit":qx.core.Environment.select(l,{"edge":function(bw){var bx=qx.bom.Event.getRelatedTarget(bw);if(bx==null){this.__hu();this.resetFocus();this.resetActive();}
;}
...
;}
}),"opera":function(bA){var bB=qx.bom.Event.getTarget(bA);if(bB==this._document){this.__hu();this.__hp=this.getFocus();this.__hq=this.getActive();this.resetFocus();this.resetActive();}
else {if(bB===this.getFocus()){this.resetFocus();}
;if(bB===this.getActive()){this.resetActive();}
;}
;}
,"default":null})),__hz:qx.event.GlobalError.observeMethod(qx.core.Environment.select(A,{"gecko":function(bC){var bD=qx.bom.Event.getTarget(bC);if(bD===this._window||bD===this._document){this.__hu();this.resetActive();this.resetFocus();}
;}
,"webkit":function(bE){var bF=qx.bom.Event.getTarget(bE);if(bF===this._window||bF===this._document){this.__hu();this.__hp=this.getFocus();this.__hq=this.getActive();this.resetActive();this.resetFocus();}
;}
,"default":null})),__hA:qx.event.GlobalError.observeMethod(qx.core.Environment.select(A,{"gecko":function(bG){var bH=qx.bom.Event.getTarget(bG);if(bH===this._window||bH===this._document){this.__hv();bH=this._body;}
;this.setFocus(bH);this.tryActivate(bH);}
,"webkit":function(bI){var bJ=qx.bom.Event.getTarget(bI);if(bJ===this._window||bJ===this._document){this.__hv();if(this.__hp){this.setFocus(this.__hp);delete this.__hp;}
;if(this.__hq){this.setActive(this.__hq);delete this.__hq;}
;}
else {this.__ht=bI.relatedTarget;this.setFocus(bJ);this.__ht=null;this.tryActivate(bJ);}
;}
,"default":null})),__hB:qx.event.GlobalError.observeMethod(qx.core.Environment.select(A,{"mshtml":function(bL){var bM=qx.bom.Event.getTarget(bL);var bK=this.__hH(bM);if(bK){if(!this.__hJ(bM)){bM.unselectable=w;try{if(document.selection){document.selection.empty();}
;}
catch(bN){}
;try{bK.focus();}
catch(bO){}
;}
;}
else {qx.bom.Event.preventDefault(bL);if(!this.__hJ(bM)){bM.unselectable=w;}
;}
;}
,"webkit":function(bQ){var bR=qx.bom.Event.getTarget(bQ);var bP=this.__hH(bR);if(bP){this.setFocus(bP);}
else {qx.bom.Event.preventDefault(bQ);}
;}
,"gecko":function(bT){var bU=qx.bom.Event.getTarget(bT);var bS=this.__hH(bU);if(bS){this.setFocus(bS);}
else {qx.bom.Event.preventDefault(bT);}
;}
,"opera":function(bX){var bY=qx.bom.Event.getTarget(bX);var bV=this.__hH(bY);if(!this.__hJ(bY)){qx.bom.Event.preventDefault(bX);if(bV){var bW=this.getFocus();if(bW&&bW.selectionEnd){bW.selectionStart=0;bW.selectionEnd=0;bW.blur();}
;if(bV){this.setFocus(bV);}
;}
;}
else if(bV){this.setFocus(bV);}
;}
,"default":null})),__hC:qx.event.GlobalError.observeMethod(qx.core.Environment.select(A,{"mshtml":function(ca){var cb=qx.bom.Event.getTarget(ca);if(cb.unselectable){cb.unselectable=v;}
;this.tryActivate(this.__hE(cb));}
...
;}
,"webkit":function(ce){var cf=qx.bom.Event.getTarget(ce);this.tryActivate(this.__hE(cf));}
,"opera":function(cg){var ch=qx.bom.Event.getTarget(cg);this.tryActivate(this.__hE(ch));}
,"default":null})),__hD:function(ck){var cl=this.getFocus();if(cl&&ck!=cl){if(cl.nodeName.toLowerCase()===p||cl.nodeName.toLowerCase()===m){return cl;}
...
;}
;return ck;}
,__hE:qx.event.GlobalError.observeMethod(qx.core.Environment.select(A,{"mshtml":function(cm){return this.__hD(cm);}
,"webkit":function(cn){return this.__hD(cn);}
,"default":function(co){return co;}
})),__hF:qx.event.GlobalError.observeMethod(qx.core.Environment.select(A,{"mshtml":function(cp){var cq=qx.bom.Event.getTarget(cp);if(!this.__hJ(cq)){qx.bom.Event.preventDefault(cp);}
;}
,"webkit":function(cr){var cs=qx.bom.Event.getTarget(cr);if(!this.__hJ(cs)){qx.bom.Event.preventDefault(cr);}
;}
,"default":null})),__hG:function(ct){var cu=qx.bom.element.Attribute.get(ct,u);if(cu>=1){return true;}
;var cv=qx.event.handler.Focus.FOCUSABLE_ELEMENTS;if(cu>=0&&cv[ct.tagName]){return true;}
;return false;}
,__hH:function(cw){while(cw&&cw.nodeType===1){if(cw.getAttribute(E)==w){return null;}
;if(this.__hG(cw)){return cw;}
;cw=cw.parentNode;}
;return this._body;}
,__hI:function(cx){var cy=cx;while(cx&&cx.nodeType===1){if(cx.getAttribute(G)==w){return null;}
;cx=cx.parentNode;}
;return cy;}
,__hJ:function(cz){while(cz&&cz.nodeType===1){var cA=cz.getAttribute(t);if(cA!=null){return cA===w;}
;cz=cz.parentNode;}
;return true;}
,_applyActive:function(cC,cB){if(cB){this.__cM(cB,cC,k,true);}
;if(cC){this.__cM(cC,cB,x,true);}
;if(this.__hg){window.scrollTo(0,0);}
;}
,_applyFocus:function(cE,cD){if(cD){this.__cM(cD,cE,D,true);}
;if(cE){this.__cM(cE,cD,y,true);}
;if(cD){this.__cM(cD,cE,c,false);}
;if(cE){this.__cM(cE,cD||this.__ht,f,false);}
;}
},destruct:function(){this._stopObserver();this._manager=this._window=this._document=this._root=this._body=this.__hK=this.__ht=null;}
...
;}
;}
,"false":null}),__eX:null,__eY:null,__fa:null,__fb:null,__fc:null,__fd:null,__fe:null,getBounds:function(){return this.__fd||this.__eY||null;}
...
;if(P!=null&&P!==this.__eX){this.__eX=P;qx.ui.core.queue.Layout.add(this);return null;}
;var O=this.__eY;if(!O){O=this.__eY={};}
...
;W=this.__fb=this._computeSizeHint();if(this._hasHeightForWidth()&&this.__eX&&this.getHeight()==null){W.height=this.__eX;}
...
;this.$$parent=this.$$subparent=this.__fe=this.__eY=this.__fd=this.__fb=null;}
});}
)();
...
(function(){var a="qx.ui.core.queue.Visibility",b="visibility";qx.Class.define(a,{statics:{__fg:[],__fh:{},__dj:{},remove:function(c){if(this.__fh[c.$$hash]){delete this.__fh[c.$$hash];qx.lang.Array.remove(this.__fg,c);}
;delete this.__dj[c.$$hash];}
,isVisible:function(d){return this.__dj[d.$$hash]||false;}
,__fi:function(f){var h=this.__dj;var g=f.$$hash;var e;if(f.isExcluded()){e=false;}
else {var parent=f.$$parent;if(parent){e=this.__fi(parent);}
else {e=f.isRootWidget();}
;}
;return h[g]=e;}
,add:function(j){if(this.__fh[j.$$hash]){return;}
;this.__fg.unshift(j);this.__fh[j.$$hash]=j;qx.ui.core.queue.Manager.scheduleFlush(b);}
,flush:function(){var n=this.__fg;var o=this.__dj;for(var i=n.length-1;i>=0;i-- ){var m=n[i].$$hash;if(o[m]!=null){n[i].addChildrenToQueue(n);}
;}
;var k={};for(var i=n.length-1;i>=0;i-- ){var m=n[i].$$hash;k[m]=o[m];o[m]=null;}
;for(var i=n.length-1;i>=0;i-- ){var l=n[i];var m=l.$$hash;n.splice(i,1);if(o[m]==null){this.__fi(l);}
...
;var y=qx.lang.Array.cast(w,v);for(var i=0;i<qxWeb.__fj.length;i++ ){qxWeb.__fj[i].call(y);}
;return y;}
...
;}
,$attachInit:function(G){this.__fj.push(G);}
...
;var C=qx.module.Event.__fk;for(var i=0,l=E.length;i<l;i++ ){var D=E[i];if(qx.lang.Type.isFunction(B)){if(!C[D]){C[D]=[];}
;C[D].push(B);}
;}
;}
,$unregisterEventNormalization:function(I,F){if(!qx.lang.Type.isArray(I)){I=[I];}
;var G=qx.module.Event.__fk;for(var i=0,l=I.length;i<l;i++ ){var H=I[i];if(G[H]){qx.lang.Array.remove(G[H],F);}
;}
;}
,$getEventNormalizationRegistry:function(){return qx.module.Event.__fk;}
,$registerEventHook:function(O,L,K){if(!qx.lang.Type.isArray(O)){O=[O];}
;var M=qx.module.Event.__fl.on;for(var i=0,l=O.length;i<l;i++ ){var N=O[i];if(qx.lang.Type.isFunction(L)){if(!M[N]){M[N]=[];}
;M[N].push(L);}
;}
;if(!K){return;}
;var J=qx.module.Event.__fl.off;for(var i=0,l=O.length;i<l;i++ ){var N=O[i];if(qx.lang.Type.isFunction(K)){if(!J[N]){J[N]=[];}
;J[N].push(K);}
;}
;}
,$unregisterEventHook:function(U,R,Q){if(!qx.lang.Type.isArray(U)){U=[U];}
;var S=qx.module.Event.__fl.on;for(var i=0,l=U.length;i<l;i++ ){var T=U[i];if(S[T]){qx.lang.Array.remove(S[T],R);}
;}
;if(!Q){return;}
;var P=qx.module.Event.__fl.off;for(var i=0,l=U.length;i<l;i++ ){var T=U[i];if(P[T]){qx.lang.Array.remove(P[T],Q);}
;}
;}
,$getEventHookRegistry:function(){return qx.module.Event.__fl;}
},members:{on:function(bd,bb,bc,W){for(var i=0;i<this.length;i++ ){var V=this[i];var Y=bc||qxWeb(V);var X=qx.module.Event.__fl.on;var be=X[t]||[];if(X[bd]){be=be.concat(X[bd]);}
;for(var j=0,m=be.length;j<m;j++ ){be[j](V,bd,bb,bc);}
;var ba=function(bf,event){var bh=qx.module.Event.__fk;var bg=bh[t]||[];if(bh[bd]){bg=bg.concat(bh[bd]);}
...
;V.$$lastlistenerId=V.$$emitter.on(bd,ba,Y);V.$$emitter.getEntryById(V.$$lastlistenerId).useCapture=!!W;if(!V.__fm){V.__fm={};}
;if(!V.__fm[bd]){V.__fm[bd]={};}
;V.__fm[bd][V.$$lastlistenerId]=ba;if(!bc){if(!V.__fn){V.__fn={};}
;V.__fn[V.$$lastlistenerId]=Y;}
;}
;return this;}
,off:function(bt,bo,bq,bj){var br=(bo===null&&bq===null);for(var j=0;j<this.length;j++ ){var bi=this[j];if(!bi.__fm){continue;}
;var bv=[];if(bt!==null){bv.push(bt);}
else {for(var bm in bi.__fm){bv.push(bm);}
;}
;for(var i=0,l=bv.length;i<l;i++ ){for(var bu in bi.__fm[bv[i]]){var bs=bi.__fm[bv[i]][bu];if(br||bs==bo||bs.original==bo){var bl=typeof bi.__fn!==q&&bi.__fn[bu];var bw;if(!bq&&bl){bw=bi.__fn[bu];}
;var bp=bi.$$emitter.off(bv[i],bs,bw||bq);if(br||bs.original==bo){qx.bom.Event.removeNativeListener(bi,bv[i],bs,bj);}
;if(bp!==null){delete bi.__fm[bv[i]][bu];}
;if(bl){delete bi.__fn[bu];}
;}
;}
;var bk=qx.module.Event.__fl.off;var bn=bk[t]||[];if(bk[bt]){bn=bn.concat(bk[bt]);}
...
(function(){var a="borderBottomWidth",b="scroll",c="qx.bom.element.Location",d="gecko",e="paddingLeft",f="borderRightWidth",g="auto",h="static",i="borderTopWidth",j="borderLeftWidth",k="marginBottom",l="marginTop",m="overflowY",n="marginLeft",o="border-box",p="padding",q="paddingBottom",r="paddingTop",s="marginRight",t="browser.quirksmode",u="engine.name",v="position",w="margin",x="paddingRight",y="BODY",z="overflowX",A="border";qx.Bootstrap.define(c,{statics:{__eE:function(C,B){return qx.bom.element.Style.get(C,B,qx.bom.element.Style.COMPUTED_MODE,false);}
,__eF:function(E,D){return parseInt(qx.bom.element.Style.get(E,D,qx.bom.element.Style.COMPUTED_MODE,false),10)||0;}
,__eG:function(G){var H=0,top=0;var F=qx.dom.Node.getWindow(G);H-=qx.bom.Viewport.getScrollLeft(F);top-=qx.bom.Viewport.getScrollTop(F);return {left:H,top:top};}
,__eH:qx.core.Environment.select(u,{"mshtml":function(K){var J=qx.dom.Node.getDocument(K);var I=J.body;var L=0;var top=0;L-=I.clientLeft+J.documentElement.clientLeft;top-=I.clientTop+J.documentElement.clientTop;if(!qx.core.Environment.get(t)){L+=this.__eF(I,j);top+=this.__eF(I,i);}
...
,"gecko":function(R){var Q=qx.dom.Node.getDocument(R).body;var S=Q.offsetLeft;var top=Q.offsetTop;if(qx.bom.element.BoxSizing.get(Q)!==o){S+=this.__eF(Q,j);top+=this.__eF(Q,i);}
;return {left:S,top:top};}
,"default":function(U){var T=qx.dom.Node.getDocument(U).body;var V=T.offsetLeft;var top=T.offsetTop;return {left:V,top:top};}
}),__eI:function(W){var X=W.getBoundingClientRect();return {left:Math.round(X.left),top:Math.round(X.top)};}
,get:function(bf,bg){if(bf.tagName==y){var location=this.__eJ(bf);var bj=location.left;var top=location.top;}
else {var Y=this.__eH(bf);var bd=this.__eI(bf);var scroll=this.__eG(bf);var bj=bd.left+Y.left-scroll.left;var top=bd.top+Y.top-scroll.top;}
...
;var bb=bj+ba;var bc=top+be;if(bg){if(bg==p||bg==b){var bi=qx.bom.element.Style.get(bf,z);if(bi==b||bi==g){bb+=bf.scrollWidth-ba+this.__eF(bf,j)+this.__eF(bf,f);}
;var bh=qx.bom.element.Style.get(bf,m);if(bh==b||bh==g){bc+=bf.scrollHeight-be+this.__eF(bf,i)+this.__eF(bf,a);}
;}
;switch(bg){case p:bj+=this.__eF(bf,e);top+=this.__eF(bf,r);bb-=this.__eF(bf,x);bc-=this.__eF(bf,q);case b:bj-=bf.scrollLeft;top-=bf.scrollTop;bb-=bf.scrollLeft;bc-=bf.scrollTop;case A:bj+=this.__eF(bf,j);top+=this.__eF(bf,i);bb-=this.__eF(bf,f);bc-=this.__eF(bf,a);break;case w:bj-=this.__eF(bf,n);top-=this.__eF(bf,l);bb+=this.__eF(bf,s);bc+=this.__eF(bf,k);break;};}
;return {left:bj,top:top,right:bb,bottom:bc};}
,__eJ:function(bl){var top=bl.offsetTop;var bm=bl.offsetLeft;top+=this.__eF(bl,l);bm+=this.__eF(bl,n);if(qx.core.Environment.get(u)===d){top+=this.__eF(bl,j);bm+=this.__eF(bl,i);}
...
,__fE:{visible:true,hidden:true},getContentWidth:function(t){var q=qx.bom.element.Style;var r=qx.bom.element.Style.get(t,j);var s=parseInt(q.get(t,f)||d,10);var w=parseInt(q.get(t,e)||d,10);if(this.__fE[r]){var v=t.clientWidth;if((qx.core.Environment.get(b)==g)||qx.dom.Node.isBlockNode(t)){v=v-s-w;}
...
;}
;}
,getContentHeight:function(B){var x=qx.bom.element.Style;var A=qx.bom.element.Style.get(B,k);var z=parseInt(x.get(B,i)||d,10);var y=parseInt(x.get(B,h)||d,10);if(this.__fE[A]){return B.clientHeight-z-y;}
...
;}
;return A.className;}
,"default":function(C,E){var D={};var F;var B=C.className;if(B){F=B.split(this.__fF);for(var i=0,l=F.length;i<l;i++ ){D[F[i]]=true;}
...
;return P.className;}
,"default":function(S,Q){var R=new RegExp(v+Q.join(g)+v,s);return S.className=S.className.replace(R,f).replace(this.__fG,f).replace(this.__fF,p);}
...
(function(){var a="qx.bom.client.Stylesheet",b="html.stylesheet.deleterule",c="html.stylesheet.insertrule",d="function",e="html.stylesheet.createstylesheet",f="html.stylesheet.addimport",g="html.stylesheet.removeimport",h="object";qx.Bootstrap.define(a,{statics:{__fH:function(){if(!qx.bom.client.Stylesheet.__fI){qx.bom.client.Stylesheet.__fI=qx.bom.Stylesheet.createElement();}
;return qx.bom.client.Stylesheet.__fI;}
,getCreateStyleSheet:function(){return typeof document.createStyleSheet===h;}
,getInsertRule:function(){return typeof qx.bom.client.Stylesheet.__fH().insertRule===d;}
,getDeleteRule:function(){return typeof qx.bom.client.Stylesheet.__fH().deleteRule===d;}
,getAddImport:function(){return (typeof qx.bom.client.Stylesheet.__fH().addImport===h);}
,getRemoveImport:function(){return (typeof qx.bom.client.Stylesheet.__fH().removeImport===h);}
...
,toParameter:function(O,S){var R,Q=[];for(R in O){if(O.hasOwnProperty(R)){var P=O[R];if(P instanceof Array){for(var i=0;i<P.length;i++ ){this.__fJ(R,P[i],Q,S);}
;}
else {this.__fJ(R,P,Q,S);}
;}
;}
;return Q.join(y);}
,__fJ:function(W,X,V,U){var T=window.encodeURIComponent;if(U){V.push(T(W).replace(/%20/g,b)+E+T(X).replace(/%20/g,b));}
...
,__fK:{},getHelperElement:function(I){if(!I){I=window;}
;var J=I.location.href;if(!qx.dom.Element.__fK[J]){var K=qx.dom.Element.__fK[J]=I.document.createElement(g);if(qx.core.Environment.get(a)==e){K.style.display=c;I.document.body.appendChild(K);}
;}
;return qx.dom.Element.__fK[J];}
...
;var J=this.__fL;name=J.names[name]||name;if(J.bools[name]&&!qx.lang.Type.isBoolean(K)){K=qx.lang.Type.isString(K);}
...
(function(){var a="css.animation",b="translate",c="rotate",d="skew",e="scale",f="qx.bom.element.Animation";qx.Bootstrap.define(f,{statics:{animate:function(h,k,g){var j=qx.bom.element.Animation.__fM(h,k.keyFrames);if(qx.core.Environment.get(a)&&j){return qx.bom.element.AnimationCss.animate(h,k,g);}
else {return qx.bom.element.AnimationJs.animate(h,k,g);}
;}
,animateReverse:function(m,o,l){var n=qx.bom.element.Animation.__fM(m,o.keyFrames);if(qx.core.Environment.get(a)&&n){return qx.bom.element.AnimationCss.animateReverse(m,o,l);}
else {return qx.bom.element.AnimationJs.animateReverse(m,o,l);}
;}
,__fM:function(p,t){var r=[];for(var v in t){var s=t[v];for(var u in s){if(r.indexOf(u)==-1){r.push(u);}
;}
;}
...
;var L=Q.keyFrames;var J=this.__fY(L);var K=this.__fX(P,J);var N=parseInt(P/K,10);this.__fQ(L,G);var O=this.__fS(N,K,J,L,P,Q.timing);var H=new qx.bom.element.AnimationHandle();H.jsAnimation=true;if(I){O.reverse();H.reverse=true;}
;H.desc=Q;H.el=G;H.delta=O;H.stepTime=K;H.steps=N;G.$$animation=H;H.i=0;H.initValues={};H.repeatSteps=this.__fV(N,Q.repeat);var M=Q.delay||0;var self=this;H.delayId=window.setTimeout(function(){H.delayId=null;self.play(H);}
,M);return H;}
,__fQ:function(V,R){var Y={};for(var U in V){for(var name in V[U]){var S=qx.bom.Style.getPropertyName(name);if(S&&S!=name){var X=qx.bom.Style.getCssName(S);V[U][X]=V[U][name];delete V[U][name];name=X;}
;if(Y[name]==undefined){var W=V[U][name];if(typeof W==y){Y[name]=this.__fT(W);}
else {Y[name]=h;}
;}
;}
;}
...
;if(T[name]===h&&this.__fO.indexOf(Y[name])!=-1){T[name]=e+Y[name];}
;}
;}
;}
;}
,__fR:function(bb){bb=qx.lang.Object.clone(bb);var bc;for(var name in bb){if(name in this.__fP){if(!bc){bc={};}
...
;}
;return bb;}
,__fS:function(bw,bh,bo,bi,be,bq){var bp=new Array(bw);var bm=1;bp[0]=this.__fR(bi[0]);var bt=bi[0];var bj=bi[bo[bm]];var bf=Math.floor(bo[bm]/(bh/be*100));var bs=1;for(var i=1;i<bp.length;i++ ){if(i*bh/be*100>bo[bm]){bt=bj;bm++ ;bj=bi[bo[bm]];bf=Math.floor(bo[bm]/(bh/be*100))-bf;bs=1;}
;bp[i]={};var bd;for(var name in bj){var br=bj[name]+h;if(name in this.__fP){if(!bd){bd={};}
;if(qx.Bootstrap.isArray(bt[name])){if(!qx.Bootstrap.isArray(bj[name])){bj[name]=[bj[name]];}
;bd[name]=[];for(var j=0;j<bj[name].length;j++ ){var bu=bj[name][j]+h;var x=bs/bf;bd[name][j]=this.__fU(bu,bt[name],bq,x);}
;}
else {var x=bs/bf;bd[name]=this.__fU(br,bt[name],bq,x);}
;}
...
else if(!isNaN(parseFloat(br))){var x=bs/bf;bp[i][name]=this.__fU(br,bt[name],bq,x);}
...
;}
;bs++ ;}
;bp[bp.length-1]=this.__fR(bi[100]);return bp;}
,__fT:function(by){return by.substring((parseFloat(by)+h).length,by.length);}
,__fU:function(bC,bB,bz,x){var bA=parseFloat(bC)-parseFloat(bB);return (parseFloat(bB)+bA*qx.bom.AnimationFrame.calculateTiming(bz,x))+this.__fT(bC);}
...
;}
;}
;qx.bom.element.AnimationJs.__fW(bD.el,bF);bD.i++ ;if(bD.i%bD.steps==0){bD.emit(w,bD.el);if(bD.desc.alternate){bD.delta.reverse();}
;}
...
;this.__fW(bH,bJ.keyFrames[bL]);}
else {this.__fW(bH,bI);}
;bH.$$animation=null;bK.el=null;bK.ended=true;bK.animationId=null;bK.emit(s,bH);return bK;}
,__fV:function(bN,bM){if(bM==undefined){return bN;}
;if(bM==n){return Number.MAX_VALUE;}
;return bN*bM;}
,__fW:function(bP,bO){for(var bQ in bO){if(bO[bQ]===undefined){continue;}
...
;}
;}
,__fX:function(bT,bR){var bU=100;for(var i=0;i<bR.length-1;i++ ){bU=Math.min(bU,bR[i+1]-bR[i]);}
;var bS=bT*bU/100;while(bS>this.__fN){bS=bS/2;}
;return Math.round(bS);}
,__fY:function(bW){var bV=Object.keys(bW);for(var i=0;i<bV.length;i++ ){bV[i]=parseInt(bV[i],10);}
...
else if(this.isRgbaString(y)){return this.__gb(y);}
else if(this.isRgbString(y)){return this.__ga();}
else if(this.isHex3String(y)){return this.__gc();}
else if(this.isHex6String(y)){return this.__gd();}
;throw new Error(a+y);}
,cssStringToRgb:function(z){if(this.isNamedColor(z)){return this.NAMED[z];}
else if(this.isSystemColor(z)){throw new Error(e+z);}
else if(this.isRgbString(z)){return this.__ga();}
else if(this.isRgbaString(z)){return this.__gb();}
else if(this.isHex3String(z)){return this.__gc();}
else if(this.isHex6String(z)){return this.__gd();}
;throw new Error(a+z);}
...
,__ga:function(){var L=parseInt(RegExp.$1,10);var K=parseInt(RegExp.$2,10);var J=parseInt(RegExp.$3,10);return [L,K,J];}
,__gb:function(){var P=parseInt(RegExp.$1,10);var O=parseInt(RegExp.$2,10);var M=parseInt(RegExp.$3,10);var N=parseFloat(RegExp.$4,10);if(P===0&&O===0&M===0&&N===0){return [-1,-1,-1];}
;return [P,O,M];}
,__gc:function(){var S=parseInt(RegExp.$1,16)*17;var R=parseInt(RegExp.$2,16)*17;var Q=parseInt(RegExp.$3,16)*17;return [S,R,Q];}
,__gd:function(){var V=(parseInt(RegExp.$1,16)*16)+parseInt(RegExp.$2,16);var U=(parseInt(RegExp.$3,16)*16)+parseInt(RegExp.$4,16);var T=(parseInt(RegExp.$5,16)*16)+parseInt(RegExp.$6,16);return [V,U,T];}
,hex3StringToRgb:function(W){if(this.isHex3String(W)){return this.__gc(W);}
;throw new Error(m+W);}
,hex3StringToHex6String:function(X){if(this.isHex3String(X)){return this.rgbToHexString(this.hex3StringToRgb(X));}
;return X;}
,hex6StringToRgb:function(Y){if(this.isHex6String(Y)){return this.__gd(Y);}
;throw new Error(q+Y);}
,hexStringToRgb:function(ba){if(this.isHex3String(ba)){return this.__gc(ba);}
;if(this.isHex6String(ba)){return this.__gd(ba);}
...
(function(){var a="css.animation",b="Element",c="",d="qx.bom.element.AnimationHandle",e="play-state",f="end",g="paused",h="running";qx.Bootstrap.define(d,{extend:qx.event.Emitter,construct:function(){var i=qx.core.Environment.get(a);this.__gf=i&&i[e];this.__gg=true;this.addListenerOnce(f,this.__gi,this);}
,events:{"start":b,"end":b,"iteration":b},members:{__gf:null,__gg:false,__gh:false,isPlaying:function(){return this.__gg;}
,isEnded:function(){return this.__gh;}
,isPaused:function(){return this.el.style[this.__gf]==g;}
,pause:function(){if(this.el){this.el.style[this.__gf]=g;this.el.$$animation.__gg=false;if(this.animationId&&qx.bom.element.AnimationJs){qx.bom.element.AnimationJs.pause(this);}
;}
;}
,play:function(){if(this.el){this.el.style[this.__gf]=h;this.el.$$animation.__gg=true;if(this.i!=undefined&&qx.bom.element.AnimationJs){qx.bom.element.AnimationJs.play(this);}
;}
;}
,stop:function(){if(this.el&&qx.core.Environment.get(a)&&!this.jsAnimation){this.el.style[this.__gf]=c;this.el.style[qx.core.Environment.get(a).name]=c;this.el.$$animation.__gg=false;this.el.$$animation.__gh=true;}
else if(this.jsAnimation){this.stopped=true;qx.bom.element.AnimationJs.stop(this);}
;}
,__gi:function(){this.__gg=false;this.__gh=true;}
}});}
)();
...
,cancelSequence:function(){this.__gj=true;}
...
;if(this.__go!=null){var name=this.__gv(ba,Y);var X=name+K+bd+H+be.timing+K+(be.delay?be.delay+H:y)+be.repeat+K+(be.alternate?n:y);qx.bom.Event.addNativeListener(W,this.__go[k],this.__gp);qx.bom.Event.addNativeListener(W,this.__go[x],this.__gq);qx.bom.Event.addNativeListener(W,this.__go[F],this.__gr);if(qx.core.Environment.get(o)){if(qx.bom.element.Style.get(W,P)==J){qx.log.Logger.warn(W,D);}
;}
;W.style[qx.lang.String.camelCase(this.__go[w])]=X;if(bb&&bb==100&&this.__go[a]){W.style[this.__go[a]]=B;}
;}
...
;if(this.__go==null){window.setTimeout(function(){qx.bom.element.AnimationCss.__gr({target:W});}
,0);}
;return bc;}
,__gp:function(e){if(e.target.$$animation){e.target.$$animation.emit(h,e.target);}
;}
,__gq:function(e){if(e.target!=null&&e.target.$$animation!=null){e.target.$$animation.emit(E,e.target);}
;}
,__gr:function(e){var bf=e.target;var bg=bf.$$animation;if(!bg){return;}
;var bi=bg.desc;if(qx.bom.element.AnimationCss.__go!=null){var bh=qx.lang.String.camelCase(qx.bom.element.AnimationCss.__go[w]);bf.style[bh]=y;qx.bom.Event.removeNativeListener(bf,qx.bom.element.AnimationCss.__go[w],qx.bom.element.AnimationCss.__gr);}
;if(bi.origin!=null){qx.bom.element.Transform.setOrigin(bf,y);}
;qx.bom.element.AnimationCss.__gs(bf,bi.keyFrames[bg.keep]);bf.$$animation=null;bg.el=null;bg.ended=true;bg.emit(i,bf);}
,__gs:function(bj,bk){var bm;for(var bl in bk){if(bl in qx.bom.element.AnimationCss.__fP){if(!bm){bm={};}
;bm[bl]=bk[bl];}
else {bj.style[qx.lang.String.camelCase(bl)]=bk[bl];}
;}
;if(bm){qx.bom.element.Transform.transform(bj,bm);}
;}
,__gt:function(bn){if(!bn.hasOwnProperty(n)){bn.alternate=false;}
...
;}
,__gu:qx.core.Environment.select(o,{"true":function(bp){var bo=[z,r,q,l,u,d,g,n];for(var name in bp){if(!(bo.indexOf(name)!=-1)){qx.Bootstrap.warn(A+name+C);}
;}
...
;}
;}
,"default":null}),__gv:function(frames,bs){var bv=y;for(var bz in frames){bv+=(bs?-(bz-100):bz)+I;var bu=frames[bz];var bx;for(var br in bu){if(br in this.__fP){if(!bx){bx={};}
;bx[br]=bu[br];}
...
;bv+=v;}
;if(this.__gn[bv]){return this.__gn[bv];}
;var name=this.__gm+this.__bG++ ;var bw=this.__go[O]+K+name;qx.bom.Stylesheet.addRule(this.__gl,bw,bv);this.__gn[bv]=name;return name;}
,__gw:function(){this.__bG=0;if(this.__gl){this.__gl.ownerNode.remove();this.__gl=null;this.__gn={};}
;}
},defer:function(bA){if(qx.core.Environment.get(c)===t&&parseInt(qx.core.Environment.get(f))>=8){document.addEventListener(p,function(){if(!document.hidden){bA.__gw();}
;}
,false);}
;}
});}
)();
(function(){var a="Child is already in: ",b="text",c="|bubble|",d="qx.html.Element",f="': ",g="|capture|",h="scroll",j="__gS",k="Invalid capture flag.",m="focus",n="Failed to add event listener for type '",o="Flushing elements...",p="class",q="blur",r="div",s="Flush: ",t="deactivate",u="'",v="css.userselect",w="animationEnd",z="Synced DOM with ",A=" from the target '",B="qx.debug",C="capture",D="visible",E="Root elements could not be inserted into other ones.",F="Has no children!",G="off",H="Invalid callback function",I="releaseCapture",J="Could not move to same index!",K="Invalid context for callback.",L="element",M="",N="Flush invisible element",O="Failed to remove event listener for type '",P="qxSelectable",Q="tabIndex",R="Has no child at this position!",S="Invalid event type.",T="qx.html.Iframe",U="activate",V="Has no parent to remove from.",W=" to the target '",X="mshtml",Y="engine.name",bp=" operations",bq="Flush rendered element",br="none",bl="Has no child: ",bm="css.userselect.none",bn=" ",bo="hidden",bs="on",bt="Switching visibility to: ",bu="id",bv="Could not overwrite existing element!";qx.Class.define(d,{extend:qx.core.Object,implement:[qx.core.IDisposable],construct:function(by,bw,bx){qx.core.Object.call(this);this.__gx=by||r;this.__gy=bw||null;this.__gz=bx||null;}
,statics:{DEBUG:false,_modified:{},_visibility:{},_scroll:{},_actions:[],__gA:{},__gB:null,__gC:null,_scheduleFlush:function(bz){qx.html.Element.__hf.schedule();}
,flush:function(){var bL;if(qx.core.Environment.get(B)){if(this.DEBUG){qx.log.Logger.debug(this,o);}
;}
;var bD=this.__gD();var bB=bD.getFocus();if(bB&&this.__gF(bB)){bD.blur(bB);}
;var bS=bD.getActive();if(bS&&this.__gF(bS)){qx.bom.Element.deactivate(bS);}
;var bG=this.__gE();if(bG&&this.__gF(bG)){qx.bom.Element.releaseCapture(bG);}
;var bM=[];var bN=this._modified;for(var bK in bN){bL=bN[bK];if(bL.__gX()||bL.classname==T){if(bL.__gG&&qx.dom.Hierarchy.isRendered(bL.__gG)){bM.push(bL);}
...
;}
;bL.__gW();}
;var bI=this._visibility;for(var bK in bI){bL=bI[bK];var bO=bL.__gG;if(!bO){delete bI[bK];continue;}
;if(qx.core.Environment.get(B)){if(this.DEBUG){qx.log.Logger.debug(this,bt+bL.__ej);}
;}
;if(!bL.$$disposed){bO.style.display=bL.__ej?M:br;if((qx.core.Environment.get(Y)==X)){if(!(document.documentMode>=8)){bO.style.visibility=bL.__ej?D:bo;}
;}
;}
;delete bI[bK];}
;var scroll=this._scroll;for(var bK in scroll){bL=scroll[bK];var bC=bL.__gG;if(bC&&bC.offsetWidth){var bF=true;if(bL.__gL!=null){bL.__gG.scrollLeft=bL.__gL;delete bL.__gL;}
;if(bL.__gM!=null){bL.__gG.scrollTop=bL.__gM;delete bL.__gM;}
;var bP=bL.__gJ;if(bP!=null){var bJ=bP.element.getDomElement();if(bJ&&bJ.offsetWidth){qx.bom.element.Scroll.intoViewX(bJ,bC,bP.align);delete bL.__gJ;}
else {bF=false;}
;}
;var bQ=bL.__gK;if(bQ!=null){var bJ=bQ.element.getDomElement();if(bJ&&bJ.offsetWidth){qx.bom.element.Scroll.intoViewY(bJ,bC,bQ.align);delete bL.__gK;}
else {bF=false;}
;}
;if(bF){delete scroll[bK];}
;}
;}
;var bE={"releaseCapture":1,"blur":1,"deactivate":1};for(var i=0;i<this._actions.length;i++ ){var bR=this._actions[i];var bO=bR.element.__gG;if(!bO||!bE[bR.type]&&!bR.element.__gX()){continue;}
;var bH=bR.args;bH.unshift(bO);qx.bom.Element[bR.type].apply(qx.bom.Element,bH);}
;this._actions=[];for(var bK in this.__gA){var bA=this.__gA[bK];var bC=bA.element.__gG;if(bC){qx.bom.Selection.set(bC,bA.start,bA.end);delete this.__gA[bK];}
;}
;qx.event.handler.Appear.refresh();}
,__gD:function(){if(!this.__gB){var bT=qx.event.Registration.getManager(window);this.__gB=bT.getHandler(qx.event.handler.Focus);}
;return this.__gB;}
,__gE:function(){if(!this.__gC){var bU=qx.event.Registration.getManager(window);this.__gC=bU.getDispatcher(qx.event.dispatch.MouseCapture);}
;return this.__gC.getCaptureElement();}
,__gF:function(bV){var bW=this.fromDomElement(bV);return bW&&!bW.__gX();}
...
},members:{__gx:null,__gG:null,__gH:null,__dW:false,__gI:true,__ej:true,__gJ:null,__gK:null,__gL:null,__gM:null,__gN:null,__gO:null,__gP:null,__gy:null,__gz:null,__gQ:null,__gR:null,__gS:null,__gT:null,__gU:null,_scheduleChildrenUpdate:function(){if(this.__gT){return;}
;this.__gT=true;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(L);}
,_createDomElement:function(){return qx.dom.Element.create(this.__gx);}
,connectWidget:function(bY){if(qx.core.Environment.get(B)){qx.core.Assert.assertTrue(!this.__gH||this.__gH===bY);}
;this.__gH=bY;if(this.__gG){if(qx.core.Environment.get(B)){qx.core.Assert.assertTrue((!this.__gG.$$widget&&!this.__gG.$$widgetObject)||(this.__gG.$$widgetObject===bY&&this.__gG.$$widget===bY.toHashCode()));}
;this.__gG.$$widget=bY.toHashCode();this.__gG.$$widgetObject=bY;}
;}
,disconnectWidget:function(ca){if(qx.core.Environment.get(B)){qx.core.Assert.assertTrue(this.__gH===ca);}
;delete this.__gH;if(this.__gG){if(qx.core.Environment.get(B)){qx.core.Assert.assertTrue((!this.__gG.$$widget&&!this.__gG.$$widgetObject)||(this.__gG.$$widgetObject===ca&&this.__gG.$$widget===ca.toHashCode()));}
;this.__gG.$$widget=M;delete this.__gG.$$widgetObject;}
;}
,__gV:function(cb){if(qx.core.Environment.get(B)){qx.core.Assert.assertTrue(!this.__gG||this.__gG===cb);qx.core.Assert.assertTrue((cb.$$elementObject===this&&cb.$$element===this.toHashCode())||(!cb.$$elementObject&&!cb.$$element));}
;this.__gG=cb;cb.$$elementObject=this;cb.$$element=this.toHashCode();if(this.__gH){cb.$$widget=this.__gH.toHashCode();cb.$$widgetObject=this.__gH;}
;}
,__gW:function(){if(qx.core.Environment.get(B)){if(this.DEBUG){this.debug(s+this.getAttribute(bu));}
;}
;var length;var cc=this.__gS;if(cc){length=cc.length;var cd;for(var i=0;i<length;i++ ){cd=cc[i];if(cd.__ej&&cd.__gI&&!cd.__gG){cd.__gW();}
;}
;}
;if(!this.__gG){this.__gV(this._createDomElement());this._copyData(false);if(cc&&length>0){this._insertChildren();}
;}
else {this._syncData();if(this.__gT){this._syncChildren();}
;}
;delete this.__gT;}
,_insertChildren:function(){var ce=this.__gS;var length=ce.length;var cg;if(length>2){var cf=document.createDocumentFragment();for(var i=0;i<length;i++ ){cg=ce[i];if(cg.__gG&&cg.__gI){cf.appendChild(cg.__gG);}
;}
;this.__gG.appendChild(cf);}
else {var cf=this.__gG;for(var i=0;i<length;i++ ){cg=ce[i];if(cg.__gG&&cg.__gI){cf.appendChild(cg.__gG);}
;}
;}
;}
,_syncChildren:function(){var ch=this.__gS;var co=ch.length;var ci;var cm;var ck=this.__gG;var cp=ck.childNodes;var cj=0;var cn;if(qx.core.Environment.get(B)){var cl=0;}
;for(var i=cp.length-1;i>=0;i-- ){cn=cp[i];cm=qx.html.Element.fromDomElement(cn);if(!cm||!cm.__gI||cm.__gU!==this){ck.removeChild(cn);if(qx.core.Environment.get(B)){cl++ ;}
;}
;}
;for(var i=0;i<co;i++ ){ci=ch[i];if(ci.__gI){cm=ci.__gG;cn=cp[cj];if(!cm){continue;}
...
;}
;}
,_copyData:function(cr){var ct=this.__gG;var cv=this.__gz;if(cv){var cs=qx.bom.element.Attribute;for(var cu in cv){cs.set(ct,cu,cv[cu]);}
;}
;var cv=this.__gy;if(cv){var cq=qx.bom.element.Style;if(cr){cq.setStyles(ct,cv);}
else {cq.setCss(ct,cq.compile(cv));}
;}
;var cv=this.__gQ;if(cv){for(var cu in cv){this._applyProperty(cu,cv[cu]);}
;}
;var cv=this.__gR;if(cv){qx.event.Registration.getManager(ct).importListeners(ct,cv);delete this.__gR;}
;}
,_syncData:function(){var cA=this.__gG;var cz=qx.bom.element.Attribute;var cx=qx.bom.element.Style;var cy=this.__gO;if(cy){var cD=this.__gz;if(cD){var cB;for(var cC in cy){cB=cD[cC];if(cB!==undefined){cz.set(cA,cC,cB);}
else {cz.reset(cA,cC);}
;}
;}
;this.__gO=null;}
;var cy=this.__gN;if(cy){var cD=this.__gy;if(cD){var cw={};for(var cC in cy){cw[cC]=cD[cC];}
;cx.setStyles(cA,cw);}
;this.__gN=null;}
;var cy=this.__gP;if(cy){var cD=this.__gQ;if(cD){var cB;for(var cC in cy){this._applyProperty(cC,cD[cC]);}
;}
;this.__gP=null;}
;}
,__gX:function(){var cE=this;while(cE){if(cE.__dW){return true;}
;if(!cE.__gI||!cE.__ej){return false;}
;cE=cE.__gU;}
;return false;}
,__gY:function(cF){if(cF.__gU===this){throw new Error(a+cF);}
;if(cF.__dW){throw new Error(E);}
;if(cF.__gU){cF.__gU.remove(cF);}
;cF.__gU=this;if(!this.__gS){this.__gS=[];}
;if(this.__gG){this._scheduleChildrenUpdate();}
;}
,__ha:function(cG){if(cG.__gU!==this){throw new Error(bl+cG);}
;if(this.__gG){this._scheduleChildrenUpdate();}
;delete cG.__gU;}
,__hb:function(cH){if(cH.__gU!==this){throw new Error(bl+cH);}
;if(this.__gG){this._scheduleChildrenUpdate();}
;}
,getChildren:function(){return this.__gS||null;}
,getChild:function(cI){var cJ=this.__gS;return cJ&&cJ[cI]||null;}
,hasChildren:function(){var cK=this.__gS;return cK&&cK[0]!==undefined;}
,indexOf:function(cM){var cL=this.__gS;return cL?cL.indexOf(cM):-1;}
,hasChild:function(cO){var cN=this.__gS;return cN&&cN.indexOf(cO)!==-1;}
,add:function(cP){if(arguments[1]){for(var i=0,l=arguments.length;i<l;i++ ){this.__gY(arguments[i]);}
;this.__gS.push.apply(this.__gS,arguments);}
else {this.__gY(cP);this.__gS.push(cP);}
;return this;}
,addAt:function(cR,cQ){this.__gY(cR);qx.lang.Array.insertAt(this.__gS,cR,cQ);return this;}
,remove:function(cS){var cT=this.__gS;if(!cT){return this;}
;if(arguments[1]){var cU;for(var i=0,l=arguments.length;i<l;i++ ){cU=arguments[i];this.__ha(cU);qx.lang.Array.remove(cT,cU);}
;}
else {this.__ha(cS);qx.lang.Array.remove(cT,cS);}
;return this;}
,removeAt:function(cV){var cW=this.__gS;if(!cW){throw new Error(F);}
;var cX=cW[cV];if(!cX){throw new Error(R);}
;this.__ha(cX);qx.lang.Array.removeAt(this.__gS,cV);return this;}
,removeAll:function(){var cY=this.__gS;if(cY){for(var i=0,l=cY.length;i<l;i++ ){this.__ha(cY[i]);}
;cY.length=0;}
;return this;}
,getParent:function(){return this.__gU||null;}
,insertInto:function(parent,da){parent.__gY(this);if(da==null){parent.__gS.push(this);}
else {qx.lang.Array.insertAt(this.__gS,this,da);}
;return this;}
,insertBefore:function(dc){var parent=dc.__gU;parent.__gY(this);qx.lang.Array.insertBefore(parent.__gS,this,dc);return this;}
,insertAfter:function(dd){var parent=dd.__gU;parent.__gY(this);qx.lang.Array.insertAfter(parent.__gS,this,dd);return this;}
,moveTo:function(de){var parent=this.__gU;parent.__hb(this);var df=parent.__gS.indexOf(this);if(df===de){throw new Error(J);}
else if(df<de){de-- ;}
;qx.lang.Array.removeAt(parent.__gS,df);qx.lang.Array.insertAt(parent.__gS,this,de);return this;}
,moveBefore:function(dg){var parent=this.__gU;return this.moveTo(parent.__gS.indexOf(dg));}
,moveAfter:function(dh){var parent=this.__gU;return this.moveTo(parent.__gS.indexOf(dh)+1);}
,free:function(){var parent=this.__gU;if(!parent){throw new Error(V);}
;if(!parent.__gS){return this;}
;parent.__ha(this);qx.lang.Array.remove(parent.__gS,this);return this;}
,getDomElement:function(){return this.__gG||null;}
,getNodeName:function(){return this.__gx;}
,setNodeName:function(name){this.__gx=name;}
,setRoot:function(di){this.__dW=di;}
,useMarkup:function(dj){if(this.__gG){throw new Error(bv);}
;if(qx.core.Environment.get(Y)==X){var dk=document.createElement(r);}
else {var dk=qx.dom.Element.getHelperElement();}
;dk.innerHTML=dj;this.useElement(dk.firstChild);return this.__gG;}
,useElement:function(dl){if(this.__gG){throw new Error(bv);}
;this.__gV(dl);this._copyData(true);}
,isFocusable:function(){var dn=this.getAttribute(Q);if(dn>=1){return true;}
;var dm=qx.event.handler.Focus.FOCUSABLE_ELEMENTS;if(dn>=0&&dm[this.__gx]){return true;}
;return false;}
...
;}
,isNativelyFocusable:function(){return !!qx.event.handler.Focus.FOCUSABLE_ELEMENTS[this.__gx];}
,include:function(){if(this.__gI){return this;}
;delete this.__gI;if(this.__gU){this.__gU._scheduleChildrenUpdate();}
;return this;}
,exclude:function(){if(!this.__gI){return this;}
;this.__gI=false;if(this.__gU){this.__gU._scheduleChildrenUpdate();}
;return this;}
,isIncluded:function(){return this.__gI===true;}
,fadeIn:function(dr){var ds=qxWeb(this.__gG);if(ds.isPlaying()){ds.stop();}
;if(!this.__gG){this.__gW();ds.push(this.__gG);}
;if(this.__gG){ds.fadeIn(dr).once(w,function(){this.show();qx.html.Element.flush();}
,this);return ds.getAnimationHandles()[0];}
;}
,fadeOut:function(dt){var du=qxWeb(this.__gG);if(du.isPlaying()){du.stop();}
;if(this.__gG){du.fadeOut(dt).once(w,function(){this.hide();qx.html.Element.flush();}
,this);return du.getAnimationHandles()[0];}
;}
,show:function(){if(this.__ej){return this;}
;if(this.__gG){qx.html.Element._visibility[this.$$hash]=this;qx.html.Element._scheduleFlush(L);}
;if(this.__gU){this.__gU._scheduleChildrenUpdate();}
;delete this.__ej;return this;}
,hide:function(){if(!this.__ej){return this;}
;if(this.__gG){qx.html.Element._visibility[this.$$hash]=this;qx.html.Element._scheduleFlush(L);}
;this.__ej=false;return this;}
,isVisible:function(){return this.__ej===true;}
,scrollChildIntoViewX:function(dy,dw,dz){var dv=this.__gG;var dx=dy.getDomElement();if(dz!==false&&dv&&dv.offsetWidth&&dx&&dx.offsetWidth){qx.bom.element.Scroll.intoViewX(dx,dv,dw);}
else {this.__gJ={element:dy,align:dw};qx.html.Element._scroll[this.$$hash]=this;qx.html.Element._scheduleFlush(L);}
;delete this.__gL;}
,scrollChildIntoViewY:function(dD,dB,dE){var dA=this.__gG;var dC=dD.getDomElement();if(dE!==false&&dA&&dA.offsetWidth&&dC&&dC.offsetWidth){qx.bom.element.Scroll.intoViewY(dC,dA,dB);}
else {this.__gK={element:dD,align:dB};qx.html.Element._scroll[this.$$hash]=this;qx.html.Element._scheduleFlush(L);}
;delete this.__gM;}
,scrollToX:function(x,dF){var dG=this.__gG;if(dF!==true&&dG&&dG.offsetWidth){dG.scrollLeft=x;delete this.__gL;}
else {this.__gL=x;qx.html.Element._scroll[this.$$hash]=this;qx.html.Element._scheduleFlush(L);}
;delete this.__gJ;}
,getScrollX:function(){var dH=this.__gG;if(dH){return dH.scrollLeft;}
;return this.__gL||0;}
,scrollToY:function(y,dJ){var dI=this.__gG;if(dJ!==true&&dI&&dI.offsetWidth){dI.scrollTop=y;delete this.__gM;}
else {this.__gM=y;qx.html.Element._scroll[this.$$hash]=this;qx.html.Element._scheduleFlush(L);}
;delete this.__gK;}
,getScrollY:function(){var dK=this.__gG;if(dK){return dK.scrollTop;}
;return this.__gM||0;}
,disableScrolling:function(){this.enableScrolling();this.scrollToX(0);this.scrollToY(0);this.addListener(h,this.__hd,this);}
,enableScrolling:function(){this.removeListener(h,this.__hd,this);}
,__hc:null,__hd:function(e){if(!this.__hc){this.__hc=true;this.__gG.scrollTop=0;this.__gG.scrollLeft=0;delete this.__hc;}
;}
,getTextSelection:function(){var dL=this.__gG;if(dL){return qx.bom.Selection.get(dL);}
;return null;}
,getTextSelectionLength:function(){var dM=this.__gG;if(dM){return qx.bom.Selection.getLength(dM);}
;return null;}
,getTextSelectionStart:function(){var dN=this.__gG;if(dN){return qx.bom.Selection.getStart(dN);}
;return null;}
,getTextSelectionEnd:function(){var dO=this.__gG;if(dO){return qx.bom.Selection.getEnd(dO);}
;return null;}
,setTextSelection:function(dP,dQ){var dR=this.__gG;if(dR){qx.bom.Selection.set(dR,dP,dQ);return;}
;qx.html.Element.__gA[this.toHashCode()]={element:this,start:dP,end:dQ};qx.html.Element._scheduleFlush(L);}
,clearTextSelection:function(){var dS=this.__gG;if(dS){qx.bom.Selection.clear(dS);}
;delete qx.html.Element.__gA[this.toHashCode()];}
,__he:function(dT,dU){var dV=qx.html.Element._actions;dV.push({type:dT,element:this,args:dU||[]});qx.html.Element._scheduleFlush(L);}
,focus:function(){this.__he(m);}
,blur:function(){this.__he(q);}
,activate:function(){this.__he(U);}
,deactivate:function(){this.__he(t);}
,capture:function(dW){this.__he(C,[dW!==false]);}
,releaseCapture:function(){this.__he(I);}
,setStyle:function(dX,dY,ea){if(!this.__gy){this.__gy={};}
;if(this.__gy[dX]==dY){return this;}
;if(dY==null){delete this.__gy[dX];}
else {this.__gy[dX]=dY;}
;if(this.__gG){if(ea){qx.bom.element.Style.set(this.__gG,dX,dY);return this;}
;if(!this.__gN){this.__gN={};}
;this.__gN[dX]=true;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(L);}
;return this;}
,setStyles:function(ec,ee){var ed=qx.bom.element.Style;if(!this.__gy){this.__gy={};}
;if(this.__gG){if(!this.__gN){this.__gN={};}
;for(var eb in ec){var ef=ec[eb];if(this.__gy[eb]==ef){continue;}
;if(ef==null){delete this.__gy[eb];}
else {this.__gy[eb]=ef;}
;if(ee){ed.set(this.__gG,eb,ef);continue;}
;this.__gN[eb]=true;}
;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(L);}
else {for(var eb in ec){var ef=ec[eb];if(this.__gy[eb]==ef){continue;}
;if(ef==null){delete this.__gy[eb];}
else {this.__gy[eb]=ef;}
;}
;}
...
,getStyle:function(ei){return this.__gy?this.__gy[ei]:null;}
,getAllStyles:function(){return this.__gy||null;}
,setAttribute:function(ej,ek,em){if(!this.__gz){this.__gz={};}
;if(this.__gz[ej]==ek){return this;}
;if(ek==null){delete this.__gz[ej];}
else {this.__gz[ej]=ek;}
;if(this.__gG){if(em){qx.bom.element.Attribute.set(this.__gG,ej,ek);return this;}
;if(!this.__gO){this.__gO={};}
;this.__gO[ej]=true;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(L);}
;return this;}
...
,getAttribute:function(es){return this.__gz?this.__gz[es]:null;}
...
,_setProperty:function(ew,ex,ey){if(!this.__gQ){this.__gQ={};}
;if(this.__gQ[ew]==ex){return this;}
;if(ex==null){delete this.__gQ[ew];}
else {this.__gQ[ew]=ex;}
;if(this.__gG){if(ey){this._applyProperty(ew,ex);return this;}
;if(!this.__gP){this.__gP={};}
;this.__gP[ew]=true;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(L);}
;return this;}
,_removeProperty:function(eA,ez){return this._setProperty(eA,null,ez);}
,_getProperty:function(eC){var eB=this.__gQ;if(!eB){return null;}
;var eD=eB[eC];return eD==null?null:eD;}
,addListener:function(eI,eF,self,eE){if(this.$$disposed){return null;}
;if(qx.core.Environment.get(B)){var eG=n+eI+u+W+this+f;this.assertString(eI,eG+S);this.assertFunction(eF,eG+H);if(self!==undefined){this.assertObject(self,K);}
;if(eE!==undefined){this.assertBoolean(eE,k);}
;}
;if(this.__gG){return qx.event.Registration.addListener(this.__gG,eI,eF,self,eE);}
;if(!this.__gR){this.__gR={};}
;if(eE==null){eE=false;}
;var eH=qx.event.Manager.getNextUniqueId();var eJ=eI+(eE?g:c)+eH;this.__gR[eJ]={type:eI,listener:eF,self:self,capture:eE,unique:eH};return eJ;}
...
;}
;if(this.__gG){if(eL.$$wrapped_callback&&eL.$$wrapped_callback[eR+this.$$hash]){var eM=eL.$$wrapped_callback[eR+this.$$hash];delete eL.$$wrapped_callback[eR+this.$$hash];eL=eM;}
;qx.event.Registration.removeListener(this.__gG,eR,eL,self,eK);}
else {var eN=this.__gR;var eQ;if(eK==null){eK=false;}
...
;if(this.__gG){qx.event.Registration.removeListenerById(this.__gG,eS);}
else {delete this.__gR[eS];}
;return this;}
,hasListener:function(eU,eT){if(this.$$disposed){return false;}
;if(this.__gG){return qx.event.Registration.hasListener(this.__gG,eU,eT);}
;var eV=this.__gR;var eX;if(eT==null){eT=false;}
;for(var eW in eV){eX=eV[eW];if(eX.capture===eT&&eX.type===eU){return true;}
;}
;return false;}
,getListeners:function(){if(this.$$disposed){return null;}
;if(this.__gG){return qx.event.Registration.getManager(this.__gG).serializeListeners(this.__gG);}
;var eY=[];for(var fb in this.__gR){var fa=this.__gR[fb];eY.push({type:fa.type,handler:fa.listener,self:fa.self,capture:fa.capture});}
;return eY;}
},defer:function(fc){fc.__hf=new qx.util.DeferredCall(fc.flush,fc);}
,destruct:function(){if(this.$$hash){delete qx.html.Element._modified[this.$$hash];delete qx.html.Element._scroll[this.$$hash];}
;var fd=this.__gG;if(fd){qx.event.Registration.getManager(fd).removeAllListeners(fd);fd.$$element=M;delete fd.$$elementObject;fd.$$widget=M;delete fd.$$widgetObject;}
;if(!qx.core.ObjectRegistry.inShutDown){var parent=this.__gU;if(parent&&!parent.$$disposed){parent.remove(this);}
;}
;this._disposeArray(j);this.__gz=this.__gy=this.__gR=this.__gQ=this.__gO=this.__gN=this.__gP=this.__gG=this.__gU=this.__gJ=this.__gK=null;}
});}
)();
...
,statics:{PRIORITY:qx.event.Registration.PRIORITY_FIRST},members:{__cv:null,__lI:null,__lJ:true,__ct:null,_getParent:function(n){return n.parentNode;}
,canDispatchEvent:function(p,event,o){return !!(this.__lI&&this.__lK[o]);}
,dispatchEvent:function(r,event,q){if(q==g){event.stopPropagation();this.releaseCapture();return;}
;if(this.__lJ||!qx.dom.Hierarchy.contains(this.__lI,r)){r=this.__lI;}
;qx.event.dispatch.AbstractBubbling.prototype.dispatchEvent.call(this,r,event,q);}
,__lK:{"mouseup":1,"mousedown":1,"click":1,"dblclick":1,"mousemove":1,"mouseout":1,"mouseover":1,"pointerdown":1,"pointerup":1,"pointermove":1,"pointerover":1,"pointerout":1,"tap":1,"dbltap":1},activateCapture:function(u,s){var s=s!==false;if(this.__lI===u&&this.__lJ==s){return;}
;if(this.__lI){this.releaseCapture();}
...
;this.__lJ=s;this.__lI=u;this.__cv.fireEvent(u,i,qx.event.type.Event,[true,false]);}
,getCaptureElement:function(){return this.__lI;}
,releaseCapture:function(){var v=this.__lI;if(!v){return;}
;this.__lI=null;this.__cv.fireEvent(v,d,qx.event.type.Event,[true,false]);this.nativeReleaseCapture(v);}
...
;var t=document.createElement(q);var s=t.style;s.height=s.width=m;s.overflow=w;document.body.appendChild(t);var c=z(t);this.__lO=c;document.body.removeChild(t);return this.__lO;}
...
,"default":function(t){if(this.__lL(t)){return t.value.substring(t.selectionStart,t.selectionEnd);}
...
,"default":function(x){if(qx.core.Environment.get(a)==k){var B,C,A;if(this.__lL(x)){var z=x.selectionStart;var y=x.selectionEnd;B=x.value.substring(z,y);C=y-z;}
else {B=qx.bom.Selection.get(x);C=B.length;}
;A=qx.util.StringSplit.split(B,/\r\n/);return C-(A.length-1);}
;if(this.__lL(x)){return x.selectionEnd-x.selectionStart;}
else {return this.get(x).length;}
;}
}),getStart:qx.core.Environment.select(m,{"selection":function(D){if(this.__lL(D)){var I=qx.bom.Range.get();if(!D.contains(I.parentElement())){return -1;}
...
;return  ++L;}
;}
,"default":function(N){if(qx.core.Environment.get(a)===i||qx.core.Environment.get(a)===g){if(this.__lL(N)){return N.selectionStart;}
...
;}
;}
;if(this.__lL(N)){return N.selectionStart;}
else {return qx.bom.Selection.getSelectionObject(qx.dom.Node.getDocument(N)).anchorOffset;}
;}
}),getEnd:qx.core.Environment.select(m,{"selection":function(Q){if(this.__lL(Q)){var V=qx.bom.Range.get();if(!Q.contains(V.parentElement())){return -1;}
...
;return U-( ++Y);}
;}
,"default":function(bb){if(qx.core.Environment.get(a)===i||qx.core.Environment.get(a)===g){if(this.__lL(bb)){return bb.selectionEnd;}
...
;}
;}
;if(this.__lL(bb)){return bb.selectionEnd;}
else {return qx.bom.Selection.getSelectionObject(qx.dom.Node.getDocument(bb)).focusOffset;}
;}
}),__lL:function(be){return qx.dom.Node.isElement(be)&&(be.nodeName.toLowerCase()==h||be.nodeName.toLowerCase()==n);}
...
(function(){var a="swipe",b="os.name",c="pinch",d="event.dispatchevent",e="gesturemove",f="touch",g="longtap",h="os.version",j="event.mousewheel",k="roll",l="dblclick",m="wheel",n="osx",o="rotate",p="trackstart",q="gesturefinish",r="y",s="browser.documentmode",t="dbltap",u="qx.event.handler.GestureCore",v="right",w="mshtml",z="engine.name",A="qx.debug.touchpad.detection",B="gesturecancel",C="gesturebegin",D="track",E="trackend",F="IS_TOUCHPAD : ",G="left",H="tap",I="down",J="x",K="up";qx.Bootstrap.define(u,{extend:Object,implement:[qx.core.IDisposable],statics:{TYPES:[H,a,g,t,D,p,E,o,c,k],GESTURE_EVENTS:[C,q,e,B],TAP_MAX_DISTANCE:{"touch":40,"mouse":5,"pen":20},DOUBLETAP_MAX_DISTANCE:{"touch":10,"mouse":4,"pen":10},SWIPE_DIRECTION:{x:[G,v],y:[K,I]},LONGTAP_TIME:500,DOUBLETAP_TIME:500,ROLL_FACTOR:18,TOUCHPAD_ROLL_FACTOR:1,TOUCHPAD_WHEEL_EVENTS_THRESHOLD:10,TOUCHPAD_WHEEL_EVENTS_PERIOD:100,TOUCHPAD_WHEEL_EVENTS_TIMEOUT:5000},construct:function(L,M){this.__fp=L;this.__fq=M;this.__kN={};this.__kO={};this.__kP={};this.__kQ={};this.__kR=[];this._initObserver();}
,members:{__fp:null,__fq:null,__kN:null,__kS:null,__kT:null,__kU:null,__kV:null,__kO:null,__kW:null,__kP:null,__kX:null,__kQ:null,__kR:null,__kY:0,__la:0,__lb:false,__lc:0,_initObserver:function(){qx.event.handler.GestureCore.GESTURE_EVENTS.forEach(function(O){qxWeb(this.__fp).on(O,this.checkAndFireGesture,this);}
.bind(this));if(qx.core.Environment.get(z)==w&&qx.core.Environment.get(s)<9){qxWeb(this.__fp).on(l,this._onDblClick,this);}
;var N=qx.core.Environment.get(j);qxWeb(N.target).on(N.type,this._fireRoll,this);}
,_stopObserver:function(){qx.event.handler.GestureCore.GESTURE_EVENTS.forEach(function(Q){qxWeb(this.__fp).off(Q,this.checkAndFireGesture,this);}
.bind(this));if(qx.core.Environment.get(z)==w&&qx.core.Environment.get(s)<9){qxWeb(this.__fp).off(l,this._onDblClick,this);}
...
;}
,gestureBegin:function(U,V){if(this.__kN[U.pointerId]){this.__ln(this.__kN[U.pointerId]);delete this.__kN[U.pointerId];}
;if(this._hasIntermediaryHandler(V)){return;}
;this.__kN[U.pointerId]={"startTime":new Date().getTime(),"lastEventTime":new Date().getTime(),"startX":U.clientX,"startY":U.clientY,"clientX":U.clientX,"clientY":U.clientY,"velocityX":0,"velocityY":0,"target":V,"isTap":true,"isPrimary":U.isPrimary,"longTapTimer":window.setTimeout(this.__lm.bind(this,U,V),qx.event.handler.GestureCore.LONGTAP_TIME)};if(U.isPrimary){this.__kU=false;this.__kT=V;this.__lg(p,U,V);}
else {this.__kU=true;if(Object.keys(this.__kN).length===2){this.__kV=this._calcAngle();this.__kX=this._calcDistance();}
;}
;}
,gestureMove:function(X,Y){var ba=this.__kN[X.pointerId];if(ba){var W=ba.clientX;var bb=ba.clientY;ba.clientX=X.clientX;ba.clientY=X.clientY;ba.lastEventTime=new Date().getTime();if(W){ba.velocityX=ba.clientX-W;}
;if(bb){ba.velocityY=ba.clientY-bb;}
;if(Object.keys(this.__kN).length===2){this.__lk(X,ba.target);this.__ll(X,ba.target);}
;if(!this.__kU){this.__lg(D,X,ba.target);this._fireRoll(X,f,ba.target);}
;if(ba.isTap){ba.isTap=this._isBelowTapMaxDistance(X);if(!ba.isTap){this.__ln(ba);}
;}
;}
;}
,_hasIntermediaryHandler:function(bc){while(bc&&bc!==this.__fp){if(bc.$$gestureHandler){return true;}
;bc=bc.parentNode;}
;return false;}
,gestureFinish:function(be,bf){if(!this.__kN[be.pointerId]){return;}
;var bl=this.__kN[be.pointerId];this.__ln(bl);if(this._hasIntermediaryHandler(bf)){return;}
;this.__ld(bl.velocityX,bl.velocityY,be,bl.target);this.__lg(E,be,bl.target);if(bl.isTap){if(bf!==bl.target){delete this.__kN[be.pointerId];return;}
;this._fireEvent(be,H,be.target||bf);var bg=false;if(Object.keys(this.__kO).length>0){var bk=Date.now()-qx.event.handler.GestureCore.DOUBLETAP_TIME;for(var bm in this.__kO){if(bm<bk){delete this.__kO[bm];}
else {var bd=this.__kO[bm];var bi=this.__le(bd.x,bd.y,be.clientX,be.clientY,be.getPointerType());var bj=bd.target===(be.target||bf);var bn=bd.button===be.button;if(bi&&bn&&bj){bg=true;delete this.__kO[bm];this._fireEvent(be,t,be.target||bf);}
;}
;}
;}
;if(!bg){this.__kO[Date.now()]={x:be.clientX,y:be.clientY,target:be.target||bf,button:be.button};}
;}
else if(!this._isBelowTapMaxDistance(be)){var bh=this.__lf(be,bf);if(bh){be.swipe=bh;this._fireEvent(be,a,bl.target||bf);}
;}
;delete this.__kN[be.pointerId];}
,stopMomentum:function(bo){this.__kP[bo]=true;}
,gestureCancel:function(bp){if(this.__kN[bp]){this.__ln(this.__kN[bp]);delete this.__kN[bp];}
;if(this.__kQ[bp]){this.stopMomentum(this.__kQ[bp]);delete this.__kQ[bp];}
;}
,updateGestureTarget:function(bq,br){this.__kN[bq].target=br;}
,__ld:function(bw,bx,bt,bu,bz){var by=bt.timeoutId;if(!bz&&this.__kQ[bt.pointerId]){this.stopMomentum(this.__kQ[bt.pointerId]);}
;if((Math.abs(bx)<1&&Math.abs(bw)<1)||this.__kP[by]||!this.getWindow()){delete this.__kP[by];delete this.__kQ[bt.pointerId];return;}
;if(!bz){bz=1;var bv=2.8;bx=bx/bv;bw=bw/bv;}
;bz+=0.0006;bx=bx/bz;bw=bw/bz;var bs=qx.bom.AnimationFrame.request(qx.lang.Function.bind(function(bA,bB,bC,bD,bE){this.__ld(bA,bB,bC,bD,bE);}
,this,bw,bx,bt,bu,bz));bw=Math.round(bw*100)/100;bx=Math.round(bx*100)/100;bt.delta={x:-bw,y:-bx};bt.momentum=true;bt.timeoutId=bs;this.__kQ[bt.pointerId]=bs;this._fireEvent(bt,k,bt.target||bu);}
,_calcAngle:function(){var bG=null;var bH=null;for(var bF in this.__kN){var bI=this.__kN[bF];if(bG===null){bG=bI;}
else {bH=bI;}
;}
;var x=bG.clientX-bH.clientX;var y=bG.clientY-bH.clientY;return (360+Math.atan2(y,x)*(180/Math.PI))%360;}
,_calcDistance:function(){var bJ=null;var bK=null;for(var bM in this.__kN){var bN=this.__kN[bM];if(bJ===null){bJ=bN;}
else {bK=bN;}
;}
...
,__le:function(bR,bV,bW,bX,bY){var bU=qx.event.handler.GestureCore;var bS=Math.abs(bR-bW)<bU.DOUBLETAP_MAX_DISTANCE[bY];var bT=Math.abs(bV-bX)<bU.DOUBLETAP_MAX_DISTANCE[bY];return bS&&bT;}
,_getDeltaCoordinates:function(cc){var cd=this.__kN[cc.pointerId];if(!cd){return null;}
;var ca=cc.clientX-cd.startX;var cb=cc.clientY-cd.startY;var ce=J;if(Math.abs(ca/cb)<1){ce=r;}
;return {"x":ca,"y":cb,"axis":ce};}
,_fireEvent:function(cg,ci,ch){if(!this.__fp){return;}
...
else if(this.__fq){cf=new qx.event.type.dom.Custom(ci,cg,{target:this.__fp,currentTarget:this.__fp,srcElement:this.__fp,swipe:cg.swipe,scale:cg.scale,angle:cg.angle,delta:cg.delta,pointerType:cg.pointerType,momentum:cg.momentum});this.__fq.emit(ci,cg);}
;}
,_onDblClick:function(cj){var ck=qx.bom.Event.getTarget(cj);this._fireEvent(cj,H,ck);this._fireEvent(cj,t,ck);}
,__lf:function(cn,co){var cu=this.__kN[cn.pointerId];if(!cu){return null;}
...
,__lg:function(cw,cx,cy){cx.delta=this._getDeltaCoordinates(cx);this._fireEvent(cx,cw,cx.target||cy);}
,__lh:function(cz,cA,cB){cz.delta={x:qx.util.Wheel.getDelta(cz,J)*cB,y:qx.util.Wheel.getDelta(cz,r)*cB};cz.delta.axis=Math.abs(cz.delta.x/cz.delta.y)<1?r:J;cz.pointerType=m;this._fireEvent(cz,k,cz.target||cA);}
,__li:function(cE){var cF=qx.event.handler.GestureCore.ROLL_FACTOR;if(qx.util.Wheel.IS_TOUCHPAD){cF=qx.event.handler.GestureCore.TOUCHPAD_ROLL_FACTOR;}
;this.__lc=new Date().getTime();var cC=this.__kR.length;for(var i=0;i<cC;i++ ){var cD=this.__kR[i];this.__lh(cD,cE,cF);}
;this.__kR=[];}
,__lj:function(){if(this.__kR.length>qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_THRESHOLD){qx.util.Wheel.IS_TOUCHPAD=true;}
...
,_fireRoll:function(cG,cK,cH){var cJ;var cI;if(cG.type===qx.core.Environment.get(j).type){if(this._isTouchPadDetectionEnabled()){cJ=new Date().getTime();cI=qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_TIMEOUT;if(this.__lc>0&&cJ-this.__lc>cI){this.__lb=false;this.__kR=[];this.__lc=0;}
;if(!this.__lb){if(this.__kR.length===0){this.__kY=cJ;qx.event.Timer.once(function(){if(!this.__lb){this.__lj();this.__li(cH);}
;}
,this,qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_PERIOD+50);}
;this.__kR.push(cG);this.__la++ ;if(cJ-this.__kY>qx.event.handler.GestureCore.TOUCHPAD_WHEEL_EVENTS_PERIOD){this.__lj();}
;}
;if(this.__lb){if(this.__kR.length===0){this.__kR.push(cG);}
;this.__li(cH);}
;}
else {this.__lh(cG,cH,qx.event.handler.GestureCore.ROLL_FACTOR);}
;}
else {var cL=this.__kN[cG.pointerId];cG.delta={x:-cL.velocityX,y:-cL.velocityY,axis:Math.abs(cL.velocityX/cL.velocityY)<1?r:J};this._fireEvent(cG,k,cG.target||cH);}
;}
,__lk:function(cM,cO){if(!cM.isPrimary){var cN=this._calcAngle();cM.angle=Math.round((cN-this.__kV)%360);this._fireEvent(cM,o,this.__kT);}
;}
,__ll:function(cR,cS){if(!cR.isPrimary){var cP=this._calcDistance();var cQ=cP/this.__kX;cR.scale=(Math.round(cQ*100)/100);this._fireEvent(cR,c,this.__kT);}
;}
,__lm:function(cT,cU){var cV=this.__kN[cT.pointerId];if(cV){this._fireEvent(cT,g,cT.target||cU);cV.longTapTimer=null;cV.isTap=false;}
;}
,__ln:function(cW){if(cW.longTapTimer){window.clearTimeout(cW.longTapTimer);cW.longTapTimer=null;}
;}
,dispose:function(){for(var cX in this.__kN){this.__ln(cX);}
;this._stopObserver();this.__fp=this.__fq=null;}
}});}
)();
...
,n);p.start();return p;}
},properties:{enabled:{init:false,check:f,apply:k},interval:{check:i,init:1000,apply:b}},members:{__eD:null,__eB:null,_applyInterval:function(r,q){if(this.getEnabled()){this.restart();}
;}
...
(function(){var a="dblclick",b="mshtml",c="engine.name",d="dispose",e="useraction",f="gesturemove",g="gesturecancel",h="checkAndFireGesture",i="gesturebegin",j="qx.event.handler.Gesture",k="gesturefinish",l="browser.documentmode";qx.Class.define(j,{extend:qx.event.handler.GestureCore,implement:[qx.event.IEventHandler,qx.core.IDisposable],statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{tap:1,swipe:1,longtap:1,dbltap:1,rotate:1,pinch:1,track:1,trackstart:1,trackend:1,roll:1},GESTURE_EVENTS:[i,k,f,g],TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE+qx.event.IEventHandler.TARGET_DOCUMENT,IGNORE_CAN_HANDLE:true,EVENT_CLASSES:{"tap":qx.event.type.Tap,"longtap":qx.event.type.Tap,"dbltap":qx.event.type.Tap,"swipe":qx.event.type.Swipe,"rotate":qx.event.type.Rotate,"pinch":qx.event.type.Pinch,"track":qx.event.type.Track,"trackstart":qx.event.type.Track,"trackend":qx.event.type.Track,"roll":qx.event.type.Roll}},construct:function(m){this.__dt=m;this.__ct=m.getWindow();this.__dW=this.__ct.document;qx.event.handler.GestureCore.apply(this,[this.__dW]);}
,members:{__dt:null,__ct:null,__dW:null,__fm:null,__lq:null,__lr:null,getWindow:function(){return this.__ct;}
,canHandleEvent:function(o,n){}
,registerEvent:function(r,q,p){}
,unregisterEvent:function(u,t,s){}
,_initObserver:function(){this.__fm=qx.lang.Function.listener(this.checkAndFireGesture,this);qx.event.handler.Gesture.GESTURE_EVENTS.forEach(function(w){qx.event.Registration.addListener(this.__dW,w,this.__fm,this);}
.bind(this));if(qx.core.Environment.get(c)==b&&qx.core.Environment.get(l)<9){this.__lq=qx.lang.Function.listener(this._onDblClick,this);qx.bom.Event.addNativeListener(this.__dW,a,this.__lq);}
;var v=qx.bom.client.Event.getMouseWheel(this.__ct);this.__lr=qx.lang.Function.listener(this._fireRoll,this);qx.bom.Event.addNativeListener(v.target,v.type,this.__lr,true);}
,checkAndFireGesture:function(y,x,z){this.__kM(h,[y.getNativeEvent(),y.getType(),y.getTarget()]);}
,_stopObserver:function(){qx.event.handler.Gesture.GESTURE_EVENTS.forEach(function(B){qx.event.Registration.removeListener(this.__dW,B,this.__fm);}
.bind(this));if(qx.core.Environment.get(c)==b&&qx.core.Environment.get(l)<9){qx.bom.Event.removeNativeListener(this.__dW,a,this.__lq);}
;var A=qx.bom.client.Event.getMouseWheel(this.__ct);qx.bom.Event.removeNativeListener(A.target,A.type,this.__lr);}
...
,dispose:function(){this._stopObserver();this.__kM(d);this.__dt=this.__ct=this.__dW=this.__lq=null;}
,__kM:function(I,H){qx.event.handler.GestureCore.prototype[I].apply(this,H||[]);}
...
;}
;return false;}
,__mw:{quicktime:{plugin:[k],control:f},wmv:{plugin:[v],control:L},divx:{plugin:[E],control:Q},silverlight:{plugin:[c],control:F},pdf:{plugin:[t,g],control:M}},getQuicktimeVersion:function(){var V=qx.bom.client.Plugin.__mw[D];return qx.bom.client.Plugin.__mx(V.control,V.plugin);}
,getWindowsMediaVersion:function(){var W=qx.bom.client.Plugin.__mw[n];return qx.bom.client.Plugin.__mx(W.control,W.plugin,true);}
,getDivXVersion:function(){var X=qx.bom.client.Plugin.__mw[u];return qx.bom.client.Plugin.__mx(X.control,X.plugin);}
,getSilverlightVersion:function(){var Y=qx.bom.client.Plugin.__mw[H];return qx.bom.client.Plugin.__mx(Y.control,Y.plugin);}
,getPdfVersion:function(){var ba=qx.bom.client.Plugin.__mw[m];return qx.bom.client.Plugin.__mx(ba.control,ba.plugin);}
,getQuicktime:function(){var bb=qx.bom.client.Plugin.__mw[D];return qx.bom.client.Plugin.__my(bb.control,bb.plugin);}
,getWindowsMedia:function(){var bc=qx.bom.client.Plugin.__mw[n];return qx.bom.client.Plugin.__my(bc.control,bc.plugin,true);}
,getDivX:function(){var bd=qx.bom.client.Plugin.__mw[u];return qx.bom.client.Plugin.__my(bd.control,bd.plugin);}
,getSilverlight:function(){var be=qx.bom.client.Plugin.__mw[H];return qx.bom.client.Plugin.__my(be.control,be.plugin);}
,getPdf:function(){var bf=qx.bom.client.Plugin.__mw[m];return qx.bom.client.Plugin.__my(bf.control,bf.plugin);}
,__mx:function(bo,bk,bj){var bg=qx.bom.client.Plugin.__my(bo,bk,bj);if(!bg){return w;}
...
;return w;}
;}
,__my:function(bt,br,bq){if(qx.bom.client.Engine.getName()==x&&(qx.bom.client.Browser.getDocumentMode()<11||bq)){if(!this.getActiveX()){return false;}
...
(function(){var a=" due to exceptions in user code. The application has to be reloaded!",b="Error in the 'Appearance' queue:",c="Error in the 'Widget' queue:",d="\n",f="Error in the 'Layout' queue:",g="Error in the 'Visibility' queue:",h="qx.debug",i="useraction",j="qx.debug.ui.queue",k="Error while layout flush: ",l="Error in the 'Dispose' queue:",m="Stack trace: \n",n="event.touch",o="qx.ui.core.queue.Manager",p=" times in a row",q="Fatal Error: Flush terminated ";qx.Class.define(o,{statics:{__id:false,__gj:false,__ic:{},__ie:0,MAX_RETRIES:10,scheduleFlush:function(r){var self=qx.ui.core.queue.Manager;self.__ic[r]=true;if(!self.__id){self.__gj=false;qx.bom.AnimationFrame.request(function(){if(self.__gj){self.__gj=false;return;}
;self.flush();}
,self);self.__id=true;}
;}
,flush:function(){var self=qx.ui.core.queue.Manager;if(self.__if){return;}
;self.__if=true;self.__gj=true;var s=self.__ic;self.__ig(function(){while(s.visibility||s.widget||s.appearance||s.layout||s.element){if(s.widget){delete s.widget;if(qx.core.Environment.get(j)){try{qx.ui.core.queue.Widget.flush();}
...
);self.__ig(function(){if(s.dispose){delete s.dispose;if(qx.core.Environment.get(j)){try{qx.ui.core.queue.Dispose.flush();}
...
,__ig:qx.core.Environment.select(h,{"true":function(t,u){t();u();}
...
;self.__id=false;self.__if=false;self.__ie+=1;if(self.__ie<=self.MAX_RETRIES){self.scheduleFlush();}
else {throw new Error(q+(self.__ie-1)+p+a);}
;throw e;}
finally{w();}
;}
}),__ih:function(e){qx.ui.core.queue.Manager.flush();}
},defer:function(x){qx.html.Element._scheduleFlush=x.scheduleFlush;qx.event.Registration.addListener(window,i,qx.core.Environment.get(n)?x.__ih:x.flush);}
});}
)();
(function(){var a="qx.ui.core.queue.Widget",b="widget",c="$$default";qx.Class.define(a,{statics:{__fg:[],__ic:{},remove:function(e,g){var d=this.__fg;if(!d.includes(e)){return;}
;var f=e.$$hash;if(g==null){qx.lang.Array.remove(d,e);delete this.__ic[f];return;}
;if(this.__ic[f]){delete this.__ic[f][g];if(qx.lang.Object.getLength(this.__ic[f])==0){qx.lang.Array.remove(d,e);}
;}
;}
,add:function(j,l){var h=this.__fg;if(!h.includes(j)){h.unshift(j);}
;if(l==null){l=c;}
;var k=j.$$hash;if(!this.__ic[k]){this.__ic[k]={};}
;this.__ic[k][l]=true;qx.ui.core.queue.Manager.scheduleFlush(b);}
,flush:function(){var m=this.__fg;var n,o;for(var i=m.length-1;i>=0;i-- ){n=m[i];o=this.__ic[n.$$hash];m.splice(i,1);n.syncWidget(o);}
;if(m.length!=0){return;}
;this.__fg=[];this.__ic={};}
}});}
)();
(function(){var a="qx.ui.core.queue.Layout",b="layout";qx.Class.define(a,{statics:{__fg:{},__ii:{},remove:function(c){delete this.__fg[c.$$hash];}
,add:function(d){this.__fg[d.$$hash]=d;qx.ui.core.queue.Manager.scheduleFlush(b);}
,isScheduled:function(e){return !!this.__fg[e.$$hash];}
,flush:function(){var f=this.__ik();for(var i=f.length-1;i>=0;i-- ){var g=f[i];if(g.hasValidLayout()){continue;}
...
;}
;}
,getNestingLevel:function(l){var k=this.__ii;var n=0;var parent=l;while(true){if(k[parent.$$hash]!=null){n+=k[parent.$$hash];break;}
;if(!parent.$$parent){break;}
;parent=parent.$$parent;n+=1;}
;var m=n;while(l&&l!==parent){k[l.$$hash]=m-- ;l=l.$$parent;}
;return n;}
,__ij:function(){var t=qx.ui.core.queue.Visibility;this.__ii={};var s=[];var r=this.__fg;var o,q;for(var p in r){o=r[p];if(t.isVisible(o)){q=this.getNestingLevel(o);if(!s[q]){s[q]={};}
;s[q][p]=o;delete r[p];}
;}
;return s;}
,__ik:function(){var x=[];var z=this.__ij();for(var w=z.length-1;w>=0;w-- ){if(!z[w]){continue;}
...
(function(){var a="dispose",b="qx.ui.core.queue.Dispose";qx.Class.define(b,{statics:{__fg:[],add:function(d){var c=this.__fg;if(c.includes(d)){return;}
;c.unshift(d);qx.ui.core.queue.Manager.scheduleFlush(a);}
,isEmpty:function(){return this.__fg.length==0;}
,flush:function(){var e=this.__fg;for(var i=e.length-1;i>=0;i-- ){var f=e[i];e.splice(i,1);f.dispose();}
;if(e.length!=0){return;}
;this.__fg=[];}
}});}
)();
(function(){var a="appearance",b="qx.ui.core.queue.Appearance";qx.Class.define(b,{statics:{__fg:[],__fh:{},remove:function(c){if(this.__fh[c.$$hash]){qx.lang.Array.remove(this.__fg,c);delete this.__fh[c.$$hash];}
;}
,add:function(d){if(this.__fh[d.$$hash]){return;}
;this.__fg.unshift(d);this.__fh[d.$$hash]=d;qx.ui.core.queue.Manager.scheduleFlush(a);}
,has:function(e){return !!this.__fh[e.$$hash];}
,flush:function(){var h=qx.ui.core.queue.Visibility;var f=this.__fg;var g;for(var i=f.length-1;i>=0;i-- ){g=f[i];f.splice(i,1);delete this.__fh[g.$$hash];if(h.isVisible(g)){g.syncAppearance();}
...
,__ey:null,_fireEvent:function(e){if(e.getTarget()===qx.theme.manager.Color.getInstance()){qx.theme.manager.Decoration.getInstance().refresh();}
;this.fireEvent(i);}
...
(function(){var a="0",b="qx/static",c="http://",d="https://",e="file://",f="qx.util.AliasManager",g="singleton",h=".",i="/",j="static";qx.Class.define(f,{type:g,extend:qx.util.ValueManager,construct:function(){qx.util.ValueManager.call(this);this.__iM={};this.add(j,b);}
,members:{__iM:null,_preprocess:function(n){var m=this._getDynamic();if(m[n]===false){return n;}
else if(m[n]===undefined){if(n.charAt(0)===i||n.charAt(0)===h||n.indexOf(c)===0||n.indexOf(d)===a||n.indexOf(e)===0){m[n]=false;return n;}
;if(this.__iM[n]){return this.__iM[n];}
;var l=n.substring(0,n.indexOf(i));var k=this.__iM[l];if(k!==undefined){m[n]=k+n.substring(l.length);}
;}
;return n;}
,add:function(o,q){this.__iM[o]=q;var p=this._getDynamic();for(var r in p){if(r.substring(0,r.indexOf(i))===o){p[r]=q+r.substring(o.length);}
;}
;}
,remove:function(s){delete this.__iM[s];}
,resolve:function(t){var u=this._getDynamic();if(t!=null){t=this._preprocess(t);}
;return u[t]||t;}
,getAliases:function(){var v={};for(var w in this.__iM){v[w]=this.__iM[w];}
;return v;}
}});}
)();
...
;var l=this.getTheme();if(l!==null&&l.fonts[k]){var m=this.__iO(l.fonts[k]);var n=(new m);if(l.fonts[k].testCharacters){n.setTestCharacters(l.fonts[k].testCharacters);}
...
;var p=this.getTheme();if(p!==null&&r&&p.fonts[r]){var o=this.__iO(p.fonts[r]);var s=(new o);if(p.fonts[r].testCharacters){s.setTestCharacters(p.fonts[r].testCharacters);}
;q[r]=s.set(p.fonts[r]);return true;}
;return false;}
,__iN:function(u,t){if(u[t].include){var v=u[u[t].include];u[t].include=null;delete u[t].include;u[t]=qx.lang.Object.mergeWith(u[t],v,false);this.__iN(u,t);}
;}
,_applyTheme:function(x){var w=this._dynamic;for(var z in w){if(w[z].themed){w[z].dispose();delete w[z];}
;}
;if(x){var y=this._manifestFonts?Object.assign(x.fonts,this._manifestFonts):x.fonts;for(var z in y){if(y[z].include&&y[y[z].include]){this.__iN(y,z);}
;var A=this.__iO(y[z]);var B=(new A);if(y[z].testCharacters){B.setTestCharacters(y[z].testCharacters);}
;w[z]=(new A).set(y[z]);w[z].themed=true;}
;}
;this._setDynamic(w);}
,__iO:function(C){if(C.sources){return qx.bom.webfonts.WebFont;}
;return qx.bom.Font;}
},destruct:function(){this._disposeMap(c);}
});}
)();
(function(){var a="Boolean",b="px",c="_applyItalic",d="_applyBold",e="underline",f="_applyTextShadow",g="Integer",h="string",j="_applyLineHeight",k='"',m="Array",n="line-through",o="_applyFamily",p="overline",q="Color",r="String",s="",t="italic",u="normal",v="qx.bom.Font",w="bold",x="Number",y="_applyDecoration",z=" ",A="_applySize",B=",",C="_applyColor";qx.Class.define(v,{extend:qx.core.Object,construct:function(E,D){qx.core.Object.call(this);this.__iP={fontFamily:s,fontSize:null,fontWeight:null,fontStyle:null,textDecoration:null,lineHeight:null,color:null,textShadow:null};if(E!==undefined){this.setSize(E);}
...
,__iQ:{fontFamily:s,fontSize:s,fontWeight:s,fontStyle:s,textDecoration:s,lineHeight:1.2,color:s,textShadow:s},getDefaultStyles:function(){return this.__iQ;}
...
,init:null,nullable:true},family:{check:m,nullable:true,apply:o},bold:{check:a,nullable:true,apply:d},italic:{check:a,nullable:true,apply:c},decoration:{check:[e,n,p],nullable:true,apply:y},color:{check:q,nullable:true,apply:C},textShadow:{nullable:true,check:r,apply:f}},members:{__iP:null,_applySize:function(O,N){this.__iP.fontSize=O===null?null:O+b;}
,_applyLineHeight:function(Q,P){this.__iP.lineHeight=Q===null?null:Q;}
,_applyFamily:function(R,S){var T=s;for(var i=0,l=R.length;i<l;i++ ){if(R[i].indexOf(z)>0){T+=k+R[i]+k;}
else {T+=R[i];}
;if(i!==l-1){T+=B;}
;}
;this.__iP.fontFamily=T;}
,_applyBold:function(V,U){this.__iP.fontWeight=V==null?null:V?w:u;}
,_applyItalic:function(X,W){this.__iP.fontStyle=X==null?null:X?t:u;}
,_applyDecoration:function(ba,Y){this.__iP.textDecoration=ba==null?null:ba;}
,_applyColor:function(bc,bb){this.__iP.color=null;if(bc){this.__iP.color=qx.theme.manager.Color.getInstance().resolve(bc);}
;}
,_applyTextShadow:function(be,bd){this.__iP.textShadow=be==null?null:be;}
,getStyles:function(){return this.__iP;}
}});}
)();
(function(){var a="Could not parse color: ",b="_applyTheme",c="qx.theme.manager.Color",d="Theme",e="changeTheme",f="string",g="singleton";qx.Class.define(c,{type:g,extend:qx.util.ValueManager,properties:{theme:{check:d,nullable:true,apply:b,event:e}},members:{_applyTheme:function(j){var h={};if(j){var i=j.colors;for(var name in i){h[name]=this.__iR(i,name);}
;}
;this._setDynamic(h);}
,__iR:function(l,name){var k=l[name];if(typeof k===f){if(!qx.util.ColorUtil.isCssString(k)){if(l[k]!=undefined){return this.__iR(l,k);}
;throw new Error(a+k);}
;return k;}
...
(function(){var a="qx.bom.webfonts.WebFont",b="",c="qx.debug",d="changeStatus",e="_applySources",f="qx.event.type.Data",g=" was not applied, perhaps the source file could not be loaded.",h="WebFont ";qx.Class.define(a,{extend:qx.bom.Font,events:{"changeStatus":f},properties:{sources:{nullable:true,apply:e}},members:{__iS:null,_applySources:function(m,o){var j=[];for(var i=0,l=m.length;i<l;i++ ){var k=this._quoteFontFamily(m[i].family);j.push(k);var n=m[i];n.comparisonString=this.getComparisonString();n.version=this.getVersion();qx.bom.webfonts.Manager.getInstance().require(k,n,this._onWebFontChangeStatus,this);}
...
(function(){var a="singleton",b="qx.util.LibraryManager";qx.Class.define(b,{extend:qx.core.Object,type:a,statics:{__iK:qx.$$libraries||{}},members:{has:function(c){return !!this.self(arguments).__iK[c];}
,get:function(d,e){return this.self(arguments).__iK[d][e]?this.self(arguments).__iK[d][e]:null;}
,set:function(f,g,h){this.self(arguments).__iK[f][g]=h;}
}});}
)();
...
,statics:{__bx:qx.$$resources||{},__iL:{}},members:{getIds:function(v){var x=this.self(arguments).__bx;if(!x){return null;}
...
,getImageWidth:function(B){var A=this.self(arguments).__bx[B];return A?A[0]:null;}
,getImageHeight:function(D){var C=this.self(arguments).__bx[D];return C?C[1]:null;}
,getImageFormat:function(F){if(F&&F.startsWith(k)){return l;}
;var E=this.self(arguments).__bx[F];return E?E[2]:null;}
,getCombinedFormat:function(K){var H=e;var J=this.self(arguments).__bx[K];var G=J&&J.length>4&&typeof (J[4])==j&&this.constructor.__bx[J[4]];if(G){var L=J[4];var I=this.constructor.__bx[L];H=I[2];}
;return H;}
,toUri:function(P){if(P==null){return P;}
;var M=this.self(arguments).__bx[P];if(!M){return P;}
;if(typeof M===j){var O=M;}
else {var O=M[3];if(!O){return P;}
;}
;var N=e;if((qx.core.Environment.get(b)==a)&&qx.core.Environment.get(d)){N=this.self(arguments).__iL[O];}
;return N+qx.util.LibraryManager.getInstance().get(O,q)+p+P;}
,toDataUri:function(S){var R=this.constructor.__bx[S];var U=R?this.constructor.__bx[R[4]]:null;var T;if(U){var Q=U[4][S];T=n+Q[h]+o+Q[f]+t+Q[m];}
else {T=this.toUri(S);}
;return T;}
,isFontUri:function(V){return V?V.startsWith(k):false;}
},defer:function(X){if((qx.core.Environment.get(b)==a)){if(qx.core.Environment.get(d)){for(var Y in qx.$$libraries){var W;if(qx.util.LibraryManager.getInstance().get(Y,q)){W=qx.util.LibraryManager.getInstance().get(Y,q);}
else {X.__iL[Y]=e;continue;}
;var bd;var bb=document.getElementsByTagName(i);if(bb.length>0){bd=bb[0].href;}
;if(W.match(/^\/\//)!=null){X.__iL[Y]=window.location.protocol;}
else if(W.match(/^\//)!=null){if(bd){X.__iL[Y]=bd;}
else {X.__iL[Y]=window.location.protocol+c+window.location.host;}
;}
else if(W.match(/^\.\//)!=null){var bc=document.URL;X.__iL[Y]=bc.substring(0,bc.lastIndexOf(p)+1);}
else if(W.match(/^http/)!=null){X.__iL[Y]=e;}
else {if(!bd){var ba=window.location.href.indexOf(g);if(ba==-1){bd=window.location.href;}
else {bd=window.location.href.substring(0,ba);}
;}
;X.__iL[Y]=bd.substring(0,bd.lastIndexOf(p)+1);}
;}
;}
;}
;}
...
(function(){var a="sans-serif",b="changeStatus",c="Integer",d="auto",e="qx.event.type.Data",f="0",g="qx.bom.webfonts.Validator",h="interval",i="Georgia",j="WEei",k="visible",l="Times New Roman",m="__jm",n="Arial",o="normal",p="Helvetica",q="350px",r="_applyFontFamily",s="-1000px",t="hidden",u="serif",v="span",w="absolute",x=",";qx.Class.define(g,{extend:qx.core.Object,construct:function(y,z){qx.core.Object.call(this);if(z){this.setComparisonString(z);}
;if(y){this.setFontFamily(y);this.__jj=this._getRequestedHelpers();}
;}
,statics:{COMPARISON_FONTS:{sans:[n,p,a],serif:[l,i,u]},HELPER_CSS:{position:w,margin:f,padding:f,top:s,left:s,fontSize:q,width:d,height:d,lineHeight:o,fontVariant:o,visibility:t},COMPARISON_STRING:j,__jk:null,__jl:null,removeDefaultHelperElements:function(){var A=qx.bom.webfonts.Validator.__jl;if(A){for(var B in A){document.body.removeChild(A[B]);}
;}
;delete qx.bom.webfonts.Validator.__jl;}
},properties:{fontFamily:{nullable:true,init:null,apply:r},comparisonString:{nullable:true,init:null},timeout:{check:c,init:5000}},events:{"changeStatus":e},members:{__jj:null,__jm:null,__jn:null,validate:function(){this.__jn=new Date().getTime();if(this.__jm){this.__jm.restart();}
else {this.__jm=new qx.event.Timer(100);this.__jm.addListener(h,this.__jo,this);qx.event.Timer.once(function(){this.__jm.start();}
,this,0);}
;}
,_reset:function(){if(this.__jj){for(var D in this.__jj){var C=this.__jj[D];document.body.removeChild(C);}
;this.__jj=null;}
;}
,_isFontValid:function(){if(!qx.bom.webfonts.Validator.__jk){this.__fj();}
;if(!this.__jj){this.__jj=this._getRequestedHelpers();}
;this.__jj.sans.style.visibility=k;this.__jj.sans.style.visibility=t;this.__jj.serif.style.visibility=k;this.__jj.serif.style.visibility=t;var F=qx.bom.element.Dimension.getWidth(this.__jj.sans);var E=qx.bom.element.Dimension.getWidth(this.__jj.serif);var G=qx.bom.webfonts.Validator;if(F!==G.__jk.sans||E!==G.__jk.serif){return true;}
;return false;}
...
;}
,__fj:function(){var P=qx.bom.webfonts.Validator;if(!P.__jl){P.__jl={sans:this._getHelperElement(P.COMPARISON_FONTS.sans),serif:this._getHelperElement(P.COMPARISON_FONTS.serif)};}
;P.__jk={sans:qx.bom.element.Dimension.getWidth(P.__jl.sans),serif:qx.bom.element.Dimension.getWidth(P.__jl.serif)};}
,__jo:function(){if(this._isFontValid()){this.__jm.stop();this._reset();this.fireDataEvent(b,{family:this.getFontFamily(),valid:true});}
else {var Q=new Date().getTime();if(Q-this.__jn>=this.getTimeout()){this.__jm.stop();this._reset();this.fireDataEvent(b,{family:this.getFontFamily(),valid:false});}
;}
;}
},destruct:function(){this._reset();this.__jm.stop();this.__jm.removeListener(h,this.__jo,this);this._disposeObjects(m);}
});}
)();
(function(){var a="mshtml",b="engine.name",c="_applyTheme",d="",e="'.",f="qx-",g="__jq",h="Unable to resolve decorator '",j="singleton",k=";",l="qx.theme.manager.Decoration",m=".",n="Theme",o="object",p="changeTheme",q="string",r="browser.documentmode",s=":";qx.Class.define(l,{type:j,extend:qx.core.Object,implement:[qx.core.IDisposable],statics:{CSS_CLASSNAME_PREFIX:f},construct:function(){qx.core.Object.call(this);this.__gn=[];this.__jp=(qx.core.Environment.get(b)==a&&qx.core.Environment.get(r)<9);}
,properties:{theme:{check:n,nullable:true,apply:c,event:p}},members:{__jq:null,__gn:null,__jp:false,getCssClassName:function(u){var t=qx.theme.manager.Decoration.CSS_CLASSNAME_PREFIX;if(qx.lang.Type.isString(u)){return t+u;}
...
;var F=this.__jp?y:y+(D?s:d);this.__gn.push(F+E);w.addRule(F+E,B);return;}
;A+=E+s+v[E]+k;}
,this);if(A){w.addRule(y,A);this.__gn.push(y);}
;return x;}
,removeAllCssClasses:function(){for(var i=0;i<this.__gn.length;i++ ){var H=this.__gn[i];qx.ui.style.Stylesheet.getInstance().removeRule(H);}
;this.__gn=[];}
,resolve:function(L){if(!L){return null;}
;if(typeof L===o){return L;}
;var M=this.getTheme();if(!M){return null;}
;var J=this.__jq;if(!J){J=this.__jq={};}
...
,isCached:function(T){return !this.__jq?false:qx.lang.Object.contains(this.__jq,T);}
...
;}
;this.removeAllCssClasses();this._disposeMap(g);this.__jq={};}
...
(function(){var a="qx.ui.style.Stylesheet",b="singleton";qx.Class.define(a,{type:b,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__gl=qx.bom.Stylesheet.createElement();this.__gn=[];}
,members:{__gn:null,__gl:null,addRule:function(d,c){if(this.hasRule(d)){return;}
;qx.bom.Stylesheet.addRule(this.__gl,d,c);this.__gn.push(d);}
,hasRule:function(e){return this.__gn.indexOf(e)!=-1;}
,removeRule:function(f){delete this.__gn[this.__gn.indexOf(f)];qx.bom.Stylesheet.removeRule(this.__gl,f);}
}});}
)();
...
;var bh=this.__jv;if(qx.core.Environment.get(u)){bh=this.__jr;}
else if(qx.core.Environment.get(g)&&!qx.core.Environment.get(K)&&qx.core.Environment.get(d)){bh=this.__js;}
else if(qx.core.Environment.get(g)&&!qx.core.Environment.get(K)){bh=this.__ju;}
;var bj=[w,M,Y,j,V,H];(function(bo,bq,bt,br,bk,bn){for(var i=0;i<bo.length;i++ ){var bw=this.__jw(bo[i]);var bp=this.__jw(bq[i]);var bx=bt[i];var bm=br[i];var bl=bk[i];var bu=bn[i];if(!bh(bw,bp,bx,bm,bl,bu,bg,bi)){break;}
;}
...
,__jr:function(bH,bE,bI,bA,bz,bC,by,bD){bI=bI===c?D:bI;if(bA==P){var bG=bz+bI+a+bI;var bF=bC+bI+a+bI;}
else {var bG=m+bI+W+bz+bI;var bF=m+bI+W+bC+bI;}
;var bB=e+bH+b+bE+f;bD.push(o+bG+bf+bF+bf+bB+f);return true;}
,__js:function bJ(bW,bQ,bY,bM,bL,bN,bK,bO){if(!bJ.__jt){bJ.__jt=document.createElement(I);}
...
;}
;bJ.__jt.width=bR;bJ.__jt.height=bP;var bT=bJ.__jt.getContext(U);if(bX){var bV=bT.createLinearGradient(0,0,0,bP);}
...
;bO.push(t+bJ.__jt.toDataURL()+A+ca);return true;}
,__ju:function(ci,ch,ck,cd,cc,ce,cb,cf){var cj=cd==P?1:0;if(!qx.util.ColorUtil.isHex6String(ci)){ci=qx.util.ColorUtil.stringToRgb(ci);ci=qx.util.ColorUtil.rgbToHexString(ci);}
...
;return false;}
,__jv:function(cu,cr,cv,co,cm,cp,cl,cq){var cw=co==P?0:270;var cs=cu+W+cm+cv;var cn=cr+W+cp+cv;var ct=qx.core.Environment.get(K);if(ct===bd){cw=co==P?cw+90:cw-90;}
;cq.push(ct+J+cw+s+cs+bf+cn+f);return true;}
,__jw:function(cx){return qx.core.Environment.get(y)?qx.theme.manager.Color.getInstance().resolve(cx):cx;}
...
;var bc=this.getInnerOpacity();if(bc<1){this.__jy(bd,bc);}
...
;}
,__jy:function(bh,be){if(!qx.core.Environment.get(F)){if(qx.core.Environment.get(m)){qx.log.Logger.warn(A);}
;return;}
...
;}
;}
,__jz:function(){return {top:this.getWidthTop()+this.getInnerWidthTop(),right:this.getWidthRight()+this.getInnerWidthRight(),bottom:this.getWidthBottom()+this.getInnerWidthBottom(),left:this.getWidthLeft()+this.getInnerWidthLeft()};}
}});}
)();
...
;var V=qx.util.AliasManager.getInstance().resolve(this.getBorderImage());var W=this.__jA(V);return {top:this.getSliceTop()||W[0],right:this.getSliceRight()||W[1],bottom:this.getSliceBottom()||W[2],left:this.getSliceLeft()||W[3]};}
,_applyBorderImage:function(){if(qx.core.Environment.get(o)){if(this._isInitialized()){throw new Error(H);}
;}
;}
,__jA:function(be){var bd=this.getBorderImageMode();var bf=0;var bb=0;var bc=0;var bg=0;var bh=/(.*)(\.[a-z]+)$/.exec(be);var X=bh[1];var ba=bh[2];var Y=qx.util.ResourceManager.getInstance();if(bd==K||bd==w){bf=Y.getImageHeight(X+i+ba);bc=Y.getImageHeight(X+q+ba);}
...
(function(){var a="abstract",b="Abstract method called.",c="qx.ui.decoration.Abstract";qx.Class.define(c,{extend:qx.core.Object,implement:[qx.ui.decoration.IDecorator],type:a,members:{__jB:null,_getDefaultInsets:function(){throw new Error(b);}
,_isInitialized:function(){throw new Error(b);}
,_resetInsets:function(){this.__jB=null;}
,getInsets:function(){if(!this.__jB){this.__jB=this._getDefaultInsets();}
;return this.__jB;}
},destruct:function(){this.__jB=null;}
});}
)();
(function(){var a=', ',b="qx.ui.decoration.Decorator",c="_style",d="_getDefaultInsetsFor",e="left",f="top",g="bottom",h="right";qx.Class.define(b,{extend:qx.ui.decoration.Abstract,implement:[qx.ui.decoration.IDecorator],include:[qx.ui.decoration.MBackgroundColor,qx.ui.decoration.MBorderRadius,qx.ui.decoration.MBoxShadow,qx.ui.decoration.MDoubleBorder,qx.ui.decoration.MLinearBackgroundGradient,qx.ui.decoration.MBorderImage,qx.ui.decoration.MTransition],members:{__cW:false,getPadding:function(){var l=this.getInset();var j=this._getDefaultInsetsForBorderImage();var o=l.top-(j.top?j.top:this.getWidthTop());var n=l.right-(j.right?j.right:this.getWidthRight());var k=l.bottom-(j.bottom?j.bottom:this.getWidthBottom());var m=l.left-(j.left?j.left:this.getWidthLeft());return {top:l.top?o:this.getInnerWidthTop(),right:l.right?n:this.getInnerWidthRight(),bottom:l.bottom?k:this.getInnerWidthBottom(),left:l.left?m:this.getInnerWidthLeft()};}
...
,_isInitialized:function(){return this.__cW;}
...
));for(var i=0;i<z.length;i++ ){this.__jC(z[i],y);}
;return z;}
,__jC:function(F,E){var G=F.length;while(F.length<E){F.push(F[F.length%G]);}
;}
}});}
)();
(function(){var a="'.",b="Missing appearance: ",c="_applyTheme",d="qx.debug",e="string",f="Cannot find a matching appearance for '",g="Hint: This may be an issue with nested child controls and a missing alias definition in the appearance theme.",h="qx.theme.manager.Appearance",j=":",k="Theme",l="undefined",m="changeTheme",n="/",o="singleton";qx.Class.define(h,{type:o,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__kj={};this.__kk={};}
,properties:{theme:{check:k,nullable:true,event:m,apply:c}},members:{__kl:{},__kj:null,__kk:null,_applyTheme:function(){this.__kk={};this.__kj={};}
,__km:function(D,z,q,u){var w=z.appearances;var r=w[D];if(!r){var C=n;var s=[];var v=D.split(C);var B=qx.lang.Array.clone(v);var y;while(!r&&v.length>0){s.unshift(v.pop());var t=v.join(C);r=w[t];if(r){y=r.alias||r;if(typeof y===e){var A=y+C+s.join(C);return this.__km(A,z,q,B);}
;}
;}
;for(var i=0;i<s.length-1;i++ ){s.shift();var x=s.join(C);var p=this.__km(x,z,null,B);if(p){return p;}
;}
;if(q!=null){return this.__km(q,z,null,B);}
;if(qx.core.Environment.get(d)){if(typeof u!==l){this.debug(f+u.join(n)+a);if(u.length>1){this.info(g);}
;}
;}
;return null;}
else if(typeof r===e){return this.__km(r,z,q,B);}
else if(r.include&&!r.style){return this.__km(r.include,z,q,B);}
;return D;}
,styleFrom:function(W,O,P,F){if(!P){P=this.getTheme();}
;var M=this.__kk;var E=M[W];if(!E){E=M[W]=this.__km(W,P,F);}
...
;if(J>0){U+=j+J;}
;}
;var K=this.__kj;if(K[U]!==undefined){return K[U];}
;if(!O){O=this.__kl;}
...
;W.include=this.__mz(W.include);W.patch=this.__mz(W.patch);if(qx.core.Environment.get(q)){this.__q(name,W);}
;var V={$$type:Q,name:name,title:W.title,toString:this.genericToString};if(W.extend){V.supertheme=W.extend;}
;V.basename=qx.Bootstrap.createNamespace(name,V);this.__mC(V,W);this.__mA(V,W);this.$$registry[name]=V;for(var i=0,a=W.include,l=a.length;i<l;i++ ){this.include(V,a[i]);}
;for(var i=0,a=W.patch,l=a.length;i<l;i++ ){this.patch(V,a[i]);}
;}
,__mz:function(X){if(!X){return [];}
;if(qx.Bootstrap.isArray(X)){return X;}
else {return [X];}
;}
,__mA:function(Y,ba){var bb=ba.aliases||{};if(ba.extend&&ba.extend.aliases){qx.Bootstrap.objectMergeWith(bb,ba.extend.aliases,false);}
;Y.aliases=bb;}
...
,__mB:function(bd){for(var i=0,bc=this.__mD,l=bc.length;i<l;i++ ){if(bd[bc[i]]){return bc[i];}
;}
;}
,__mC:function(bi,bj){var bf=this.__mB(bj);if(bj.extend&&!bf){bf=bj.extend.type;}
...
,$$registry:{},__mD:[N,u,n,d,j,s,e,R],__p:qx.core.Environment.select(q,{"true":{"title":m,"aliases":U,"type":m,"extend":U,"colors":U,"borders":U,"decorations":U,"fonts":U,"icons":U,"widgets":U,"appearances":U,"meta":U,"include":U,"patch":U},"default":null}),__mE:qx.core.Environment.select(q,{"true":{"color":U,"border":U,"decoration":U,"font":U,"icon":U,"appearance":U,"widget":U},"default":null}),__q:qx.core.Environment.select(q,{"true":function(name,bq){var bp=this.__p;for(var bo in bq){if(bp[bo]===undefined){throw new Error(f+bo+M+name+h);}
...
;}
;if(bq.meta){var bm;for(var bo in bq.meta){bm=bq.meta[bo];if(this.__mE[bo]===undefined){throw new Error(C+bo+S);}
;if(typeof bm!==this.__mE[bo]){throw new Error(I+bo+L);}
...
;}
,"default":function(){}
}),patch:function(bu,bs){this.__mF(bs);var bw=this.__mB(bs);if(bw!==this.__mB(bu)){throw new Error(w+bu.name+v+bs.name+b);}
;var bt=bs[bw];var br=bu.$$clazz.prototype;for(var bv in bt){br[bv]=bt[bv];}
;}
,include:function(bA,by){this.__mF(by);var bC=by.type;if(bC!==bA.type){throw new Error(w+bA.name+v+by.name+b);}
;var bz=by[bC];var bx=bA.$$clazz.prototype;for(var bB in bz){if(bx[bB]!==undefined){continue;}
;bx[bB]=bz[bB];}
;}
,__mF:function(bD){if(typeof bD===H||bD==null){var bF=new Error(g);if(qx.core.Environment.get(q)){var bE=qx.dev.StackTrace.getStackTraceFromError(bF);qx.Bootstrap.error(this,bE);}
;throw bF;}
;}
}});}
)();
...
;}
;return false;}
,__hM:new qx.util.ObjectPool()},members:{__hL:null,__hN:null,__hO:null,__hP:null,_getLayout:function(){return this.__hP;}
,_setLayout:function(fi){if(qx.core.Environment.get(ep)){if(fi){this.assertInstance(fi,qx.ui.layout.Abstract);}
;}
;if(this.__hP){this.__hP.connectToWidget(null);}
;if(fi){fi.connectToWidget(this);}
;this.__hP=fi;qx.ui.core.queue.Layout.add(this);}
,setLayoutParent:function(parent){if(this.$$parent===parent){return;}
...
;if(ft||fq.local||fq.margin){if(this.__hP&&this.hasLayoutChildren()){var fo=this.getInsets();var innerWidth=fm-fo.left-fo.right;var innerHeight=fk-fo.top-fo.bottom;var fs=this.getDecorator();var fn={left:0,right:0,top:0,bottom:0};if(fs){fs=qx.theme.manager.Decoration.getInstance().resolve(fs);fn=fs.getPadding();}
;var fl={top:this.getPaddingTop()+fn.top,right:this.getPaddingRight()+fn.right,bottom:this.getPaddingBottom()+fn.bottom,left:this.getPaddingLeft()+fn.left};this.__hP.renderLayout(innerWidth,innerHeight,fl);}
...
,__hQ:null,clearSeparators:function(){var fv=this.__hQ;if(!fv){return;}
;var fw=qx.ui.core.Widget.__hM;var content=this.getContentElement();var fu;for(var i=0,l=fv.length;i<l;i++ ){fu=fv[i];fw.poolObject(fu);content.remove(fu.getContentElement());}
;fv.length=0;}
,renderSeparator:function(fy,fz){var fx=qx.ui.core.Widget.__hM.getObject(qx.ui.core.Widget);fx.set({decorator:fy});var fB=fx.getContentElement();this.getContentElement().add(fB);var fA=fB.getDomElement();if(fA){fA.style.top=fz.top+dF;fA.style.left=fz.left+dF;fA.style.width=fz.width+dF;fA.style.height=fz.height+dF;}
...
,invalidateLayoutCache:function(){qx.ui.core.LayoutItem.prototype.invalidateLayoutCache.call(this);if(this.__hP){this.__hP.invalidateLayoutCache();}
;}
,_getContentHint:function(){var fN=this.__hP;if(fN){if(this.hasLayoutChildren()){var fO=fN.getSizeHint();if(qx.core.Environment.get(ep)){var fM=eb+this.toString()+dx;this.assertInteger(fO.width,T+fM);this.assertInteger(fO.height,eu+fM);}
;return fO;}
...
,getInsets:function(){var top=this.getPaddingTop();var fY=this.getPaddingRight();var ga=this.getPaddingBottom();var gd=this.getPaddingLeft();if(this.getDecorator()){var gc=qx.theme.manager.Decoration.getInstance().resolve(this.getDecorator());var gb=gc.getInsets();if(qx.core.Environment.get(ep)){this.assertNumber(gb.top,dW+gb.top);this.assertNumber(gb.right,eh+gb.right);this.assertNumber(gb.bottom,ev+gb.bottom);this.assertNumber(gb.left,w+gb.left);}
...
;return false;}
,__hR:function(){var gl=this._createContentElement();gl.connectWidget(this);gl.setStyles({"touch-action":dw,"-ms-touch-action":dw});if(qx.core.Environment.get(ep)){gl.setAttribute(et,this.classname);}
...
,getContentElement:function(){return this.__hL;}
,__hS:null,getLayoutChildren:function(){var gn=this.__hS;if(!gn){return this.__hT;}
...
;return go||gn;}
,scheduleLayoutUpdate:function(){qx.ui.core.queue.Layout.add(this);}
,invalidateLayoutChildren:function(){var gp=this.__hP;if(gp){gp.invalidateChildrenCache();}
;qx.ui.core.queue.Layout.add(this);}
,hasLayoutChildren:function(){var gr=this.__hS;if(!gr){return false;}
;var gq;for(var i=0,l=gr.length;i<l;i++ ){gq=gr[i];if(!gq.hasUserBounds()&&!gq.isExcluded()){return true;}
;}
;return false;}
,getChildrenContainer:function(){return this;}
,__hT:[],_getChildren:function(){return this.__hS||this.__hT;}
,_indexOf:function(gt){var gs=this.__hS;if(!gs){return -1;}
;return gs.indexOf(gt);}
,_hasChildren:function(){var gu=this.__hS;return gu!=null&&(!!gu[0]);}
,addChildrenToQueue:function(gv){var gw=this.__hS;if(!gw){return;}
...
;if(gz.getLayoutParent()==this){qx.lang.Array.remove(this.__hS,gz);}
;if(this.__hS){this.__hS.push(gz);}
else {this.__hS=[gz];}
;this.__hU(gz,gy);}
,_addAt:function(gD,gA,gC){if(!this.__hS){this.__hS=[];}
;if(gD.getLayoutParent()==this){qx.lang.Array.remove(this.__hS,gD);}
;var gB=this.__hS[gA];if(gB===gD){gD.setLayoutProperties(gC);}
;if(gB){qx.lang.Array.insertBefore(this.__hS,gD,gB);}
else {this.__hS.push(gD);}
;this.__hU(gD,gC);}
...
;if(gE.getLayoutParent()==this){qx.lang.Array.remove(this.__hS,gE);}
;qx.lang.Array.insertBefore(this.__hS,gE,gG);this.__hU(gE,gF);}
,_addAfter:function(gJ,gH,gI){if(qx.core.Environment.get(ep)){this.assertInArray(gH,this._getChildren(),eY);}
;if(gJ==gH){return;}
;if(!this.__hS){this.__hS=[];}
;if(gJ.getLayoutParent()==this){qx.lang.Array.remove(this.__hS,gJ);}
;qx.lang.Array.insertAfter(this.__hS,gJ,gH);this.__hU(gJ,gI);}
,_remove:function(gK){if(!this.__hS){throw new Error(dm);}
;qx.lang.Array.remove(this.__hS,gK);this.__hV(gK);}
,_removeAt:function(gL){if(!this.__hS){throw new Error(dm);}
;var gM=this.__hS[gL];qx.lang.Array.removeAt(this.__hS,gL);this.__hV(gM);return gM;}
,_removeAll:function(){if(!this.__hS){return [];}
;var gN=this.__hS.concat();this.__hS.length=0;for(var i=gN.length-1;i>=0;i-- ){this.__hV(gN[i]);}
;qx.ui.core.queue.Layout.add(this);return gN;}
,_afterAddChild:null,_afterRemoveChild:null,__hU:function(gP,gO){if(qx.core.Environment.get(ep)){this.assertInstance(gP,qx.ui.core.LayoutItem,eg+gP);this.assertNotIdentical(gP,this,B+gP);if(gO!=null){this.assertType(gO,j,A+gO);}
;}
...
;}
,__hV:function(gQ){if(qx.core.Environment.get(ep)){this.assertNotUndefined(gQ);}
;if(gQ.getLayoutParent()!==this){throw new Error(X+gQ+ee);}
;gQ.setLayoutParent(null);if(this.__hP){this.__hP.invalidateChildrenCache();}
...
,_applyPadding:function(gW,gV,name){this._updateInsets=true;qx.ui.core.queue.Layout.add(this);this.__hW(name,gW);}
,__hW:function(gX,hb){var content=this.getContentElement();var gY=this.getDecorator();gY=qx.theme.manager.Decoration.getInstance().resolve(gY);if(gY){var ha=qx.Bootstrap.firstLow(gX.replace(eB,eL));hb+=gY.getPadding()[ha]||0;}
...
;}
,_applyToolTipText:function(hg,hf){if(qx.core.Environment.get(y)){if(this.__hO){return;}
;var he=qx.locale.Manager.getInstance();this.__hO=he.addListener(eX,function(){var hh=this.getToolTipText();if(hh&&hh.translate){this.setToolTipText(hh.translate());}
;}
,this);}
;}
...
;}
,__hX:null,$$stateChanges:null,_forwardStates:null,hasState:function(hA){var hz=this.__hX;return !!hz&&!!hz[hA];}
,addState:function(hE){var hD=this.__hX;if(!hD){hD=this.__hX={};}
;if(hD[hE]){return;}
;this.__hX[hE]=true;if(hE===dy){this.syncAppearance();}
else if(!qx.ui.core.queue.Visibility.isVisible(this)){this.$$stateChanges=true;}
else {qx.ui.core.queue.Appearance.add(this);}
;var forward=this._forwardStates;var hC=this.__ib;if(forward&&forward[hE]&&hC){var hB;for(var hF in hC){hB=hC[hF];if(hB instanceof qx.ui.core.Widget){hC[hF].addState(hE);}
;}
;}
;}
,removeState:function(hJ){var hI=this.__hX;if(!hI||!hI[hJ]){return;}
;delete this.__hX[hJ];if(hJ===dy){this.syncAppearance();}
else if(!qx.ui.core.queue.Visibility.isVisible(this)){this.$$stateChanges=true;}
else {qx.ui.core.queue.Appearance.add(this);}
;var forward=this._forwardStates;var hH=this.__ib;if(forward&&forward[hJ]&&hH){for(var hK in hH){var hG=hH[hK];if(hG instanceof qx.ui.core.Widget){hG.removeState(hJ);}
;}
;}
;}
,replaceState:function(hM,hP){var hO=this.__hX;if(!hO){hO=this.__hX={};}
;if(!hO[hP]){hO[hP]=true;}
;if(hO[hM]){delete hO[hM];}
;if(!qx.ui.core.queue.Visibility.isVisible(this)){this.$$stateChanges=true;}
else {qx.ui.core.queue.Appearance.add(this);}
;var forward=this._forwardStates;var hN=this.__ib;if(forward&&forward[hP]&&hN){for(var hQ in hN){var hL=hN[hQ];if(hL instanceof qx.ui.core.Widget){hL.replaceState(hM,hP);}
;}
;}
;}
,__hY:null,__ia:null,syncAppearance:function(){var hV=this.__hX;var hU=this.__hY;var hW=qx.theme.manager.Appearance.getInstance();var hS=qx.core.Property.$$method.setThemed;var ib=qx.core.Property.$$method.resetThemed;if(this.__ia){delete this.__ia;if(hU){var hR=hW.styleFrom(hU,hV,null,this.getAppearance());hU=null;}
;}
;if(!hU){var hT=this;var hX=[];do {hX.push(hT.$$subcontrol||hT.getAppearance());}
while(hT=hT.$$subparent);hU=hX.reverse().join(eI).replace(/#[0-9]+/g,eL);this.__hY=hU;}
;var ia=hW.styleFrom(hU,hV,null,this.getAppearance());if(ia){if(hR){for(var hY in hR){if(ia[hY]===undefined){this[ib[hY]]();}
;}
;}
;if(qx.core.Environment.get(ep)){for(var hY in ia){if(!this[hS[hY]]){throw new Error(this.classname+fb+hY+P+hU);}
;}
;}
...
;}
;this.fireDataEvent(O,this.__hX);}
,_applyAppearance:function(ie,ic){this.updateAppearance();}
,checkAppearanceNeeds:function(){if(!this.__hN){qx.ui.core.queue.Appearance.add(this);this.__hN=true;}
else if(this.$$stateChanges){qx.ui.core.queue.Appearance.add(this);delete this.$$stateChanges;}
;}
,updateAppearance:function(){this.__ia=true;qx.ui.core.queue.Appearance.add(this);var ii=this.__ib;if(ii){var ig;for(var ih in ii){ig=ii[ih];if(ig instanceof qx.ui.core.Widget){ig.updateAppearance();}
;}
;}
;}
...
,hasChildControl:function(iW){if(!this.__ib){return false;}
;return !!this.__ib[iW];}
,__ib:null,_getCreatedChildControls:function(){return this.__ib;}
,getChildControl:function(ja,iY){if(!this.__ib){if(iY){return null;}
;this.__ib={};}
;var iX=this.__ib[ja];if(iX){return iX;}
...
;delete jh.$$subcontrol;delete jh.$$subparent;var ji=this.__hX;var forward=this._forwardStates;if(ji&&forward&&jh instanceof qx.ui.core.Widget){for(var jj in ji){if(forward[jj]){jh.removeState(jj);}
;}
;}
;delete this.__ib[jk];return jh;}
,_createChildControl:function(jp){if(!this.__ib){this.__ib={};}
else if(this.__ib[jp]){throw new Error(L+jp+dN);}
...
;jl.$$subcontrol=jp;jl.$$subparent=this;var jn=this.__hX;var forward=this._forwardStates;if(jn&&forward&&jl instanceof qx.ui.core.Widget){for(var jo in jn){if(forward[jo]){jl.addState(jo);}
;}
;}
;this.fireDataEvent(r,jl);return this.__ib[jp]=jl;}
,_createChildControlImpl:function(js,jr){return null;}
,_disposeChildControls:function(){var jw=this.__ib;if(!jw){return;}
...
;}
;return jG;}
},destruct:function(){if(!qx.core.ObjectRegistry.inShutDown){if(qx.core.Environment.get(y)){if(this.__hO){qx.locale.Manager.getInstance().removeListenerById(this.__hO);}
;}
...
;if(!qx.core.ObjectRegistry.inShutDown){this.clearSeparators();this.__hQ=null;}
else {this._disposeArray(eM);}
;this._disposeArray(v);this.__hX=this.__ib=null;this._disposeObjects(ea,I);}
});}
)();
(function(){var a="Missing renderLayout() implementation!",b="abstract",c="Missing getHeightForWidth() implementation!",d="qx.debug",e="It is not possible to manually set the connected widget.",f="qx.ui.layout.Abstract";qx.Class.define(f,{type:b,extend:qx.core.Object,members:{__fb:null,_invalidChildrenCache:null,__gH:null,invalidateLayoutCache:function(){this.__fb=null;}
...
,"false":null}),_clearSeparators:function(){var m=this.__gH;if(m instanceof qx.ui.core.LayoutItem){m.clearSeparators();}
;}
,_renderSeparator:function(n,o){this.__gH.renderSeparator(n,o);}
,connectToWidget:function(p){if(p&&this.__gH){throw new Error(e);}
;this.__gH=p;this.invalidateChildrenCache();}
,_getWidget:function(){return this.__gH;}
,_applyLayoutChange:function(){if(this.__gH){this.__gH.scheduleLayoutUpdate();}
;}
,_getLayoutChildren:function(){return this.__gH.getLayoutChildren();}
},destruct:function(){this.__gH=this.__fb=null;}
});}
)();
(function(){var a="blur",b="activate",c="focus",d="qx.ui.core.EventHandler";qx.Class.define(d,{extend:qx.core.Object,implement:qx.event.IEventHandler,construct:function(){qx.core.Object.call(this);this.__dt=qx.event.Registration.getManager(window);}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_FIRST,SUPPORTED_TYPES:{mousemove:1,mouseover:1,mouseout:1,mousedown:1,mouseup:1,click:1,auxclick:1,dblclick:1,contextmenu:1,mousewheel:1,keyup:1,keydown:1,keypress:1,keyinput:1,capture:1,losecapture:1,focusin:1,focusout:1,focus:1,blur:1,activate:1,deactivate:1,appear:1,disappear:1,dragstart:1,dragend:1,dragover:1,dragleave:1,drop:1,drag:1,dragchange:1,droprequest:1,touchstart:1,touchend:1,touchmove:1,touchcancel:1,tap:1,longtap:1,swipe:1,dbltap:1,track:1,trackend:1,trackstart:1,pinch:1,rotate:1,roll:1,pointermove:1,pointerover:1,pointerout:1,pointerdown:1,pointerup:1,pointercancel:1},IGNORE_CAN_HANDLE:false},members:{__dt:null,__il:{focusin:1,focusout:1,focus:1,blur:1},__im:{mouseover:1,mouseout:1,appear:1,disappear:1},canHandleEvent:function(f,e){return f instanceof qx.ui.core.Widget;}
...
;if(this.__il[j.getType()]){n=n&&n.getFocusTarget();if(!n){return;}
;}
...
;if(v){if(this.__il[j.getType()]){v=v.getFocusTarget();}
;if(v===n){return;}
;}
;}
;var r=j.getCurrentTarget();var t=qx.ui.core.Widget.getWidgetByElement(r);if(!t||t.isAnonymous()){return;}
;if(this.__il[j.getType()]){t=t.getFocusTarget();}
;var u=j.getType();if(!t||!(t.isEnabled()||this.__im[u])){return;}
;var g=j.getEventPhase()==qx.event.type.Event.CAPTURING_PHASE;var q=this.__dt.getListeners(t,u,g);if(j.getEventPhase()==qx.event.type.Event.AT_TARGET){if(!q){q=[];}
;var h=this.__dt.getListeners(t,u,!g);if(h){q=q.concat(h);}
;}
...
(function(){var a="best-fit",b="placementRight",c="Boolean",d="bottom-right",e="' ",f="widget",g="placementLeft",h="qx.ui.core.MPlacement",i="left-top",j="Integer",k="left-middle",l="right-middle",m="top-center",n="[qx.ui.core.MPlacement.setMoveDirection()], the value was '",o="offsetRight",p="interval",q="keep-align",r="bottom-left",s="pointer",t="direct",u="shorthand",v="Invalid value for the parameter 'direction' ",w="offsetLeft",x="top-left",y="appear",z="offsetBottom",A="top",B="top-right",C="offsetTop",D="but 'top' or 'left' are allowed.",E="right-bottom",F="disappear",G="right-top",H="bottom-center",I="left-bottom",J="left";qx.Mixin.define(h,{statics:{__ej:null,__ek:J,setVisibleElement:function(K){this.__ej=K;}
,getVisibleElement:function(){return this.__ej;}
,setMoveDirection:function(L){if(L===A||L===J){this.__ek=L;}
else {throw new Error(v+n+L+e+D);}
;}
,getMoveDirection:function(){return this.__ek;}
},properties:{position:{check:[x,m,B,r,H,d,i,k,I,G,l,E],init:r,themeable:true},placeMethod:{check:[f,s],init:s,themeable:true},domMove:{check:c,init:false},placementModeX:{check:[t,q,a],init:q,themeable:true},placementModeY:{check:[t,q,a],init:q,themeable:true},offsetLeft:{check:j,init:0,themeable:true},offsetTop:{check:j,init:0,themeable:true},offsetRight:{check:j,init:0,themeable:true},offsetBottom:{check:j,init:0,themeable:true},offset:{group:[C,o,z,w],mode:u,themeable:true}},members:{__el:null,__em:null,__en:null,getLayoutLocation:function(N){var P,O,R,top;O=N.getBounds();if(!O){return null;}
...
;}
,placeToWidget:function(bc,ba){if(ba){this.__eo();this.__el=qx.lang.Function.bind(this.placeToWidget,this,bc,false);qx.event.Idle.getInstance().addListener(p,this.__el);this.__en=function(){this.__eo();}
;this.addListener(F,this.__en,this);}
;var bb=bc.getContentLocation()||this.getLayoutLocation(bc);if(bb!=null){this._place(bb);return true;}
else {return false;}
;}
,__eo:function(){if(this.__el){qx.event.Idle.getInstance().removeListener(p,this.__el);this.__el=null;}
;if(this.__en){this.removeListener(F,this.__en,this);this.__en=null;}
;}
...
,__ep:function(bk){var bl=null;if(this._computePlacementSize){var bl=this._computePlacementSize();}
else if(this.isVisible()){var bl=this.getBounds();}
;if(bl==null){this.addListenerOnce(y,function(){this.__ep(bk);}
,this);}
else {bk.call(this,bl);}
;}
,_place:function(bm){this.__ep(function(bo){var bn=qx.util.placement.Placement.compute(bo,this.getLayoutParent().getBounds(),bm,this._getPlacementOffsets(),this.getPosition(),this.getPlacementModeX(),this.getPlacementModeY());this.removeState(g);this.removeState(b);this.addState(bm.left<bn.left?b:g);this.moveTo(bn.left,bn.top);}
);}
},destruct:function(){this.__eo();}
});}
)();
(function(){var a="-",b="best-fit",c="size",d="edge-start",e="target.bottom",f="offsets",g="size.width",h="bottom",i="offsets.bottom",j="qx.util.placement.Placement",k="Please use '",l="qx.debug",m="keep-align",n="center",o="target.right",p="direct",q="offsets.right",r="middle",s="target",t="align-start",u="Invalid 'mode' argument!'",v='__eq',w="left",x="align-end",y="Class",z="offsets.left",A="top",B="area",C="right",D="edge-end",E="target.top",F="area.height",G="target.left",H="align-center",I="area.width",J="' instead!",K="size.height",L="offsets.top";qx.Class.define(j,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__eq=qx.util.placement.DirectAxis;}
,properties:{axisX:{check:y},axisY:{check:y},edge:{check:[A,C,h,w],init:A},align:{check:[A,C,h,w,n,r],init:C}},statics:{__er:null,compute:function(W,P,M,N,V,Q,R){this.__er=this.__er||new qx.util.placement.Placement();var T=V.split(a);var S=T[0];var O=T[1];if(qx.core.Environment.get(l)){if(O===n||O===r){var U=r;if(S===A||S===h){U=n;}
;qx.core.Assert.assertEquals(U,O,k+S+a+U+J);}
;}
;this.__er.set({axisX:this.__ev(Q),axisY:this.__ev(R),edge:S,align:O});return this.__er.compute(W,P,M,N);}
,__es:null,__et:null,__eu:null,__ev:function(X){switch(X){case p:this.__es=this.__es||qx.util.placement.DirectAxis;return this.__es;case m:this.__et=this.__et||qx.util.placement.KeepAlignAxis;return this.__et;case b:this.__eu=this.__eu||qx.util.placement.BestFitAxis;return this.__eu;default:throw new Error(u);};}
},members:{__eq:null,compute:function(be,bb,Y,ba){if(qx.core.Environment.get(l)){this.assertObject(be,c);this.assertNumber(be.width,g);this.assertNumber(be.height,K);this.assertObject(bb,B);this.assertNumber(bb.width,I);this.assertNumber(bb.height,F);this.assertObject(Y,s);this.assertNumber(Y.top,E);this.assertNumber(Y.right,o);this.assertNumber(Y.bottom,e);this.assertNumber(Y.left,G);this.assertObject(ba,f);this.assertNumber(ba.top,L);this.assertNumber(ba.right,q);this.assertNumber(ba.bottom,i);this.assertNumber(ba.left,z);}
;var bc=this.getAxisX()||this.__eq;var bf=bc.computeStart(be.width,{start:Y.left,end:Y.right},{start:ba.left,end:ba.right},bb.width,this.__ew());var bd=this.getAxisY()||this.__eq;var top=bd.computeStart(be.height,{start:Y.top,end:Y.bottom},{start:ba.top,end:ba.bottom},bb.height,this.__ex());return {left:bf,top:top};}
,__ew:function(){var bh=this.getEdge();var bg=this.getAlign();if(bh==w){return d;}
else if(bh==C){return D;}
else if(bg==w){return t;}
else if(bg==n){return H;}
else if(bg==C){return x;}
;}
,__ex:function(){var bj=this.getEdge();var bi=this.getAlign();if(bj==A){return d;}
...
,events:{"interval":d},properties:{timeoutInterval:{check:a,init:100,apply:c}},members:{__ey:null,_applyTimeoutInterval:function(g){if(this.__ey){this.__ey.setInterval(g);}
;}
,_onInterval:function(){this.fireEvent(b);}
,__ez:function(){if(!this.__ey&&this.hasListener(b)){var h=new qx.event.Timer(this.getTimeoutInterval());h.addListener(b,this._onInterval,this);h.start();this.__ey=h;}
;}
,__eA:function(){if(this.__ey&&!this.hasListener(b)){this.__ey.stop();this.__ey.dispose();this.__ey=null;}
;}
,addListener:function(k,i,self,j){var l=qx.core.Object.prototype.addListener.call(this,k,i,self,j);this.__ez();return l;}
,addListenerOnce:function(o,m,self,n){var p=qx.core.Object.prototype.addListenerOnce.call(this,o,m,self,n);this.__ez();return p;}
,removeListener:function(s,q,self,r){var t=qx.core.Object.prototype.removeListener.call(this,s,q,self,r);this.__eA();return t;}
,removeListenerById:function(u){var v=qx.core.Object.prototype.removeListenerById.call(this,u);this.__eA();return v;}
},destruct:function(){if(this.__ey){this.__ey.stop();}
;this.__ey=null;}
});}
)();
(function(){var a="Image could not be loaded: ",b="Boolean",c="px",d="backgroundRepeat",e="http",f=".png",g="background-image",h="engine.version",j="scale",l="changeSource",m="x",n="div",o=", 0 0",p="class",q="aborted",r="background-size",s="color",t="scaled",u="3",v="qx.ui.basic.Image",w="top",x="0 0",y="fontSize",z="Font source needs either a glyph name or the unicode number in hex",A="verticalAlign",B="qx.debug",C="textAlign",D="loaded",E="backgroundImage",F="center",G="-disabled.$1",H="font",I="qx.event.type.Event",J="image",K="loadingFailed",L="__in",M="css.alphaimageloaderneeded",N="/",O="1.5",P="String",Q="browser.documentmode",R="backgroundPosition",S="border-box",T="",U="changeScale",V="left",W="_applySource",X=",",Y="img",bF="@",bG="data:image/",bH="px, ",bB="middle",bC='.',bD="nonScaled",bE="table-cell",bL=", ",bM="background",bN="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7",bX="mshtml",bI="engine.name",bJ="Virtual image source contains unkown font descriptor",bK="_applyScale",bz="try to load an unmanaged relative image: ",bQ=", no-repeat",bA="position",bR="replacement",bS="2",bV="no-repeat",bO="background-position",bW="hidden",bP="alphaScaled",bT="display",bU="absolute";qx.Class.define(v,{extend:qx.ui.core.Widget,construct:function(bY){this.__in={};qx.ui.core.Widget.call(this);if(bY){this.setSource(bY);}
;}
,properties:{source:{check:P,init:null,nullable:true,event:l,apply:W,themeable:true},scale:{check:b,init:false,event:U,themeable:true,apply:bK},appearance:{refine:true,init:J},allowShrinkX:{refine:true,init:false},allowShrinkY:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false}},events:{loadingFailed:I,loaded:I,aborted:I},statics:{PLACEHOLDER_IMAGE:bN},members:{__io:null,__ip:null,__iq:null,__in:null,__ir:null,__is:null,__it:0,_onChangeTheme:function(){qx.ui.core.Widget.prototype._onChangeTheme.call(this);this._styleSource();}
,getContentElement:function(){return this.__ix();}
,_createContentElement:function(){return this.__ix();}
,_getContentHint:function(){return {width:this.__io||0,height:this.__ip||0};}
,_applyDecorator:function(cc,cb){qx.ui.core.Widget.prototype._applyDecorator.call(this,cc,cb);var cd=this.getSource();cd=qx.util.AliasManager.getInstance().resolve(cd);var ca=this.getContentElement();if(this.__is){ca=ca.getChild(0);}
;this.__iH(ca,cd);}
,_applyTextColor:function(cf){if(this.__iv()===H){var ce=this.getContentElement();if(this.__is){ce=ce.getChild(0);}
;if(cf){ce.setStyle(s,qx.theme.manager.Color.getInstance().resolve(cf));}
else {ce.removeStyle(s);}
;}
;}
,_applyPadding:function(ch,cg,name){qx.ui.core.Widget.prototype._applyPadding.call(this,ch,cg,name);var ci=this.getContentElement();if(this.__is){ci.getChild(0).setStyles({top:this.getPaddingTop()||0,left:this.getPaddingLeft()||0});}
else {ci.setPadding(this.getPaddingLeft()||0,this.getPaddingTop()||0);}
;}
,renderLayout:function(cl,top,cj,cm){qx.ui.core.Widget.prototype.renderLayout.call(this,cl,top,cj,cm);var ck=this.getContentElement();if(this.__is){ck.getChild(0).setStyles({width:cj-(this.getPaddingLeft()||0)-(this.getPaddingRight()||0),height:cm-(this.getPaddingTop()||0)-(this.getPaddingBottom()||0),top:this.getPaddingTop()||0,left:this.getPaddingLeft()||0});}
;}
,_applyEnabled:function(co,cn){qx.ui.core.Widget.prototype._applyEnabled.call(this,co,cn);if(this.getSource()){this._styleSource();}
;}
,_applySource:function(cq,cp){if(cp){if(qx.io.ImageLoader.isLoading(cp)){qx.io.ImageLoader.abort(cp);}
;}
;this._styleSource();}
,_applyScale:function(cr){this._styleSource();}
,__iu:function(cs){this.__iq=cs;}
,__iv:function(){if(this.__iq==null){var cu=this.getSource();if(cu&&qx.lang.String.startsWith(cu,bF)){this.__iq=H;}
;var ct=false;if(cu!=null){ct=cu.endsWith(f);}
;if(this.getScale()&&ct&&qx.core.Environment.get(M)){this.__iq=bP;}
else if(this.getScale()){this.__iq=t;}
else {this.__iq=bD;}
;}
;return this.__iq;}
,__iw:function(cy){var cv;var cz;var cx=qx.html.Image;if(cy==H){cx=qx.html.Label;cv=true;cz=n;}
else if(cy==bP){cv=true;cz=n;}
else if(cy==bD){cv=false;cz=n;}
else {cv=true;cz=Y;}
;var cA=new (cx)(cz);cA.connectWidget(this);cA.setStyles({"overflowX":bW,"overflowY":bW,"boxSizing":S});if(cy==H){cA.setRich(true);}
else {cA.setScale(cv);if(qx.core.Environment.get(M)){var cw=this.__is=new qx.html.Element(n);cA.connectWidget(this);cw.setStyle(bA,bU);cw.add(cA);return cw;}
;}
;return cA;}
,__ix:function(){if(this.$$disposed){return null;}
;var cB=this.__iv();if(this.__in[cB]==null){this.__in[cB]=this.__iw(cB);}
;var cC=this.__in[cB];if(!this.__ir){this.__ir=cC;}
;return cC;}
,_styleSource:function(){var cH=qx.util.AliasManager.getInstance();var cG=qx.util.ResourceManager.getInstance();var cJ=cH.resolve(this.getSource());var cI=this.getContentElement();if(this.__is){cI=cI.getChild(0);}
;if(!cJ){this.__iG(cI);return;}
;this.__iA(cJ);if((qx.core.Environment.get(bI)==bX)&&(parseInt(qx.core.Environment.get(h),10)<9||qx.core.Environment.get(Q)<9)){var cE=this.getScale()?j:bV;cI.tagNameHint=qx.bom.element.Decoration.getTagName(cE,cJ);}
;var cF=this.__iz();if(cG.isFontUri(cJ)){this.__iC(cF,cJ);}
else if(cG.has(cJ)){var cK=this._findHighResolutionSource(cJ);if(cK){var cM=cG.getImageHeight(cJ);var cL=cG.getImageWidth(cJ);this.setWidth(cM);this.setHeight(cL);var cD=cM+bH+cL+c;this.__ir.setStyle(r,cD);this.setSource(cK);cJ=cK;}
;this.__iC(cF,cJ);this.__iy();}
else if(qx.io.ImageLoader.isLoaded(cJ)){this.__iD(cF,cJ);this.__iy();}
else {this.__iE(cF,cJ);}
;}
,__iy:function(){this.__it++ ;qx.bom.AnimationFrame.request(function(cN){if(cN===this.__it){this.fireEvent(D);}
else {this.fireEvent(q);}
;}
.bind(this,this.__it));}
,__iz:function(){var cO=this.__ir;if(this.__is){cO=cO.getChild(0);}
;return cO;}
,__iA:qx.core.Environment.select(bI,{"mshtml":function(cS){var cR=qx.core.Environment.get(M);var cP=cS.endsWith(f);var cQ=cS.startsWith(bF);if(cQ){this.__iu(H);}
else if(cR&&cP){if(this.getScale()&&this.__iv()!=bP){this.__iu(bP);}
else if(!this.getScale()&&this.__iv()!=bD){this.__iu(bD);}
;}
else {if(this.getScale()&&this.__iv()!=t){this.__iu(t);}
else if(!this.getScale()&&this.__iv()!=bD){this.__iu(bD);}
;}
;this.__iB(this.__ix());}
,"default":function(cT){var cU=cT&&qx.lang.String.startsWith(cT,bF);if(cU){this.__iu(H);}
else if(this.getScale()&&this.__iv()!=t){this.__iu(t);}
else if(!this.getScale()&&this.__iv()!=bD){this.__iu(bD);}
;this.__iB(this.__ix());}
}),__iB:function(cY){var cX=this.__ir;if(cX!=cY){if(cX!=null){var dm=c;var cV={};var dc=cX.getAllStyles();if(dc){for(var df in dc){cV[df]=dc[df];}
;}
;var de=this.getBounds();if(de!=null){cV.width=de.width+dm;cV.height=de.height+dm;}
;var dg=this.getInsets();cV.left=parseInt(cX.getStyle(V)||dg.left)+dm;cV.top=parseInt(cX.getStyle(w)||dg.top)+dm;cV.zIndex=10;var dh=this.__is?cY.getChild(0):cY;dh.setStyles(cV,true);dh.setSelectable(this.getSelectable());if(!cX.isVisible()){cY.hide();}
else if(!cY.isVisible()){cY.show();}
;if(!cX.isIncluded()){cY.exclude();}
else if(!cY.isIncluded()){cY.include();}
;var dj=cX.getParent();if(dj){var cW=dj.getChildren().indexOf(cX);dj.removeAt(cW);dj.addAt(cY,cW);}
;var db=dh.getNodeName();if(dh.setSource){dh.setSource(null);}
else {dh.setValue(T);}
;var da=this.__iz();dh.tagNameHint=db;dh.setAttribute(p,da.getAttribute(p));qx.html.Element.flush();var dl=da.getDomElement();var dk=cY.getDomElement();var di=cX.getListeners()||[];di.forEach(function(dn){cY.addListener(dn.type,dn.handler,dn.self,dn.capture);}
);if(dl&&dk){var dd=dl.$$hash;dl.$$hash=dk.$$hash;dk.$$hash=dd;}
;this.__ir=cY;}
;}
;}
,__iC:function(dr,du){var dq=qx.util.ResourceManager.getInstance();var dp=dq.isFontUri(du);if(!this.getEnabled()){var dt=du.replace(/\.([a-z]+)$/,G);if(!dp&&dq.has(dt)){du=dt;this.addState(bR);}
else {this.removeState(bR);}
;}
;if(!dp&&dr.getSource()===du){return;}
;if(dp){var dx;if(this.getScale()&&parseInt(du.split(N)[2],10)){this.setScale(false);}
;if(this.getScale()){var dw=this.getWidth()||this.getHeight()||40;var ds=this.getHeight()||this.getWidth()||40;dx=dw>ds?ds:dw;}
else {var dv=qx.theme.manager.Font.getInstance().resolve(du.match(/@([^/]+)/)[1]);if(qx.core.Environment.get(B)){this.assertObject(dv,bJ);}
;dx=parseInt(du.split(N)[2]||dv.getSize(),10);}
;if(!dx){dx=0;}
;this.__iJ(dx,dx);this.__iH(dr,du);}
else {this.__iH(dr,du);this.__iJ(dq.getImageWidth(du),dq.getImageHeight(du));}
;}
,_applyDimension:function(){qx.ui.core.Widget.prototype._applyDimension.call(this);var dC=this.getSource()&&qx.lang.String.startsWith(this.getSource(),bF);if(dC){var dz=this.getContentElement();if(dz){if(this.getScale()){var dB=this.getWidth()||this.getHeight()||40;var dA=this.getHeight()||this.getWidth()||40;dz.setStyle(y,(dB>dA?dA:dB)+c);}
else {var dy=qx.theme.manager.Font.getInstance().resolve(this.getSource().match(/@([^/]+)/)[1]);dz.setStyle(y,dy.getSize()+c);}
;}
;}
;}
,__iD:function(dD,dH){var dF=qx.io.ImageLoader;this.__iH(dD,dH);var dG=dF.getWidth(dH);var dE=dF.getHeight(dH);this.__iJ(dG,dE);}
,__iE:function(dJ,dL){var dK=qx.io.ImageLoader;if(qx.core.Environment.get(B)){var dI=dL.toLowerCase();if(!dI.startsWith(e)&&!dI.startsWith(bG)){var self=this.self(arguments);if(!self.__iF){self.__iF={};}
;if(!self.__iF[dL]){this.debug(bz+dL);self.__iF[dL]=true;}
;}
;}
;if(!dK.isFailed(dL)){dK.load(dL,this.__iI,this);}
else {this.__iG(dJ);}
;}
,__iG:function(dM){if(dM!=null){if(dM instanceof qx.html.Image){dM.resetSource();}
else {dM.resetValue();}
;}
;}
,__iH:function(dN,dX){var dW=dX&&qx.lang.String.startsWith(dX,bF);if(dW){var dV=dX.split(N);var dU=dX;if(dV.length>2){dU=dV[0]+N+dV[1];}
;var dR=qx.util.ResourceManager.getInstance();var ec=qx.theme.manager.Font.getInstance().resolve(dX.match(/@([^/]+)/)[1]);var dS=qx.lang.Object.clone(ec.getStyles());delete dS.color;dN.setStyles(dS);dN.setStyle(H);dN.setStyle(bT,bE);dN.setStyle(A,bB);dN.setStyle(C,F);if(this.getScale()){dN.setStyle(y,(this.__io>this.__ip?this.__ip:this.__io)+c);}
else {var ef=parseInt(dV[2]||qx.theme.manager.Font.getInstance().resolve(dX.match(/@([^/]+)/)[1]).getSize());dN.setStyle(y,ef+c);}
;var dP=dR.getData(dU);if(dP){dN.setValue(String.fromCharCode(dP[2]));}
else {var eb=parseInt(qx.theme.manager.Font.getInstance().resolve(dX.match(/@([^/]+)\/(.*)$/)[2]),16);if(qx.core.Environment.get(B)){this.assertNumber(eb,z);}
;dN.setValue(String.fromCharCode(eb));}
;return;}
else if(dN.getNodeName()==n){var ee=qx.theme.manager.Decoration.getInstance().resolve(this.getDecorator());if(ee){var dY=(ee.getStartColor()&&ee.getEndColor());var dT=ee.getBackgroundImage();if(dY||dT){var dO=this.getScale()?j:bV;var dQ=qx.bom.element.Decoration.getAttributes(dX,dO);var ed=ee.getStyles(true);var ea={"backgroundImage":dQ.style.backgroundImage,"backgroundPosition":(dQ.style.backgroundPosition||x),"backgroundRepeat":(dQ.style.backgroundRepeat||bV)};if(dT){ea[R]+=X+ed[bO]||x;ea[d]+=bL+ee.getBackgroundRepeat();}
;if(dY){ea[R]+=o;ea[d]+=bQ;}
;ea[E]+=X+(ed[g]||ed[bM]);dN.setStyles(ea);return;}
;}
else {dN.setSource(null);}
;}
;dN.setSource(dX);}
,_findHighResolutionSource:function(eg){var ej=[u,bS,O];var ei=parseFloat(qx.bom.client.Device.getDevicePixelRatio().toFixed(2));if(ei<=1){return false;}
;var i=ej.length;while(i>0&&ei>ej[ --i]){}
;var eh;var k;for(k=i;k>=0;k-- ){eh=this._getHighResolutionSource(eg,ej[k]);if(eh){return eh;}
;}
;for(k=i+1;k<ej.length;k++ ){eh=this._getHighResolutionSource(eg,ej[k]);if(eh){return eh;}
;}
;return null;}
,_getHighResolutionSource:function(ek,em){var eo=ek.lastIndexOf(bC);if(eo>-1){var en=bF+em+m;var ep=ek.slice(0,eo)+en+ek.slice(eo);if(qx.util.ResourceManager.getInstance().has(ep)){return ep;}
;}
;return null;}
,__iI:function(eq,er){if(this.$$disposed===true){return;}
;if(eq!==qx.util.AliasManager.getInstance().resolve(this.getSource())){this.fireEvent(q);return;}
;if(er.failed){this.warn(a+eq);this.fireEvent(K);}
else if(er.aborted){this.fireEvent(q);return;}
else {this.fireEvent(D);}
;this.__iD(this.__iz(),eq);}
,__iJ:function(es,et){if(es!==this.__io||et!==this.__ip){this.__io=es;this.__ip=et;qx.ui.core.queue.Layout.add(this);}
;}
},destruct:function(){for(var eu in this.__in){if(this.__in.hasOwnProperty(eu)){this.__in[eu].disconnectWidget(this);}
;}
;delete this.__ir;if(this.__is){delete this.__is;}
;this._disposeMap(L);}
});}
)();
(function(){var a="mshtml",b='img',c="",d="qx.globalErrorHandling",e="load",f="qx.io.ImageLoader";qx.Bootstrap.define(f,{statics:{__dj:{},__jD:{width:null,height:null},__jE:/\.(png|gif|jpg|jpeg|bmp)\b/i,__jF:/^data:image\/(png|gif|jpg|jpeg|bmp)\b/i,isLoaded:function(g){var h=this.__dj[g];return !!(h&&h.loaded);}
,isFailed:function(j){var k=this.__dj[j];return !!(k&&k.failed);}
,isLoading:function(m){var n=this.__dj[m];return !!(n&&n.loading);}
,getFormat:function(r){var q=this.__dj[r];if(!q||!q.format){var o=this.__jF.exec(r);if(o!=null){var p=(q&&qx.lang.Type.isNumber(q.width)?q.width:this.__jD.width);var s=(q&&qx.lang.Type.isNumber(q.height)?q.height:this.__jD.height);q={loaded:true,format:o[1],width:p,height:s};}
;}
;return q?q.format:null;}
,getSize:function(t){var u=this.__dj[t];return u?{width:u.width,height:u.height}:this.__jD;}
...
;var z=document.createElement(b);var E=qx.lang.Function.listener(this.__jG,this,z,B);z.onload=E;z.onerror=E;z.src=B;D.element=z;}
;}
...
;}
;this.__dj[F]=null;}
,__jG:function(){var J=qx.core.Environment.select(d,{"true":qx.event.GlobalError.observeMethod(this.__jH),"false":this.__jH});J.apply(this,arguments);}
,__jH:function(event,L,K){var P=this.__dj[K];if(qx.bom.client.Engine.getName()==a&&parseFloat(qx.bom.client.Engine.getVersion())===11){document.body.appendChild(L);}
;var M=function(Q){return (Q&&Q.height!==0);}
;if(event.type===e&&M(L)){P.loaded=true;P.width=L.width;P.height=L.height;var N=this.__jE.exec(K);if(N!=null){P.format=N[1];}
;}
else {P.failed=true;}
...
(function(){var a="value",b="qx.html.Label",c="The label mode cannot be modified after initial creation",d='hidden';qx.Class.define(b,{extend:qx.html.Element,members:{__jI:null,_applyProperty:function(name,e){qx.html.Element.prototype._applyProperty.call(this,name,e);if(name==a){var f=this.getDomElement();qx.bom.Label.setValue(f,e);}
;}
,_createDomElement:function(){var h=this.__jI;var g=qx.bom.Label.create(this._content,h);g.style.overflow=d;return g;}
...
;j=!!j;if(this.__jI==j){return this;}
;this.__jI=j;return this;}
...
(function(){var a="text",b="function",c="px",d="http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul",e="crop",f="nowrap",g="end",h="div",i="100%",j="auto",k="0",l="css.textoverflow",m="qx.debug",n="html.xul",o="value",p="visible",q="qx.bom.Label",r="",s="ellipsis",t="normal",u="inherit",v="block",w="label",x="-1000px",y="hidden",z="absolute";qx.Bootstrap.define(q,{statics:{__jJ:{fontFamily:1,fontSize:1,fontWeight:1,fontStyle:1,lineHeight:1},__jK:function(){var A=this.__jM(false);document.body.insertBefore(A,document.body.firstChild);return this._textElement=A;}
,__jL:function(){var B=this.__jM(true);document.body.insertBefore(B,document.body.firstChild);return this._htmlElement=B;}
,__jM:function(E){var C=qx.dom.Element.create(h);var D=C.style;D.width=D.height=j;D.left=D.top=x;D.visibility=y;D.position=z;D.overflow=p;D.display=v;if(E){D.whiteSpace=t;}
else {D.whiteSpace=f;if(!qx.core.Environment.get(l)&&qx.core.Environment.get(n)){var F=document.createElementNS(d,w);var D=F.style;D.padding=k;D.margin=k;D.width=j;for(var G in this.__jJ){D[G]=u;}
;C.appendChild(F);}
;}
;return C;}
,__jN:function(I){var H={};H.overflow=y;if(I){H.whiteSpace=t;}
...
;if(!qx.core.Environment.get(l)&&qx.core.Environment.get(n)){var N=L.document.createElementNS(d,w);var K=N.style;K.cursor=u;K.color=u;K.overflow=y;K.maxWidth=i;K.padding=k;K.margin=k;K.width=j;for(var O in this.__jJ){N.style[O]=u;}
;N.setAttribute(e,g);J.appendChild(N);}
else {qx.bom.element.Style.setStyles(J,this.__jN(M));}
;if(content){this.setValue(J,content);}
;return J;}
,__jO:null,setSanitizer:function(P){if(qx.core.Environment.get(m)){if(P){qx.core.Assert.assertFunction(P);}
;}
;qx.bom.Label.__jO=P;}
,setValue:function(R,Q){Q=Q||r;if(R.useHtml){if(qx.bom.Label.__jO&&typeof (qx.bom.Label.__jO)===b){Q=qx.bom.Label.__jO(Q);}
;R.innerHTML=Q;}
...
;}
,getHtmlSize:function(content,T,U){var V=this._htmlElement||this.__jL();V.style.width=U!=undefined?U+c:j;V.innerHTML=content;return this.__jP(V,T);}
,getTextSize:function(X,W){var Y=this._textElement||this.__jK();if(!qx.core.Environment.get(l)&&qx.core.Environment.get(n)){Y.firstChild.setAttribute(o,X);}
else {qx.bom.element.Attribute.set(Y,a,X);}
;return this.__jP(Y,W);}
,__jP:function(be,ba){var bb=this.__jJ;if(!ba){ba={};}
;for(var bd in bb){be.style[bd]=ba[bd]||r;}
;var bc=qx.bom.element.Dimension.getSize(be);bc.width++ ;return bc;}
}});}
)();
(function(){var a="source",b="engine.name",c="",d="mshtml",e="px",f="px ",g="no-repeat",h="backgroundImage",i="scale",j="webkit",k="div",l="qx.html.Image",m="qx/static/blank.gif",n="backgroundPosition";qx.Class.define(l,{extend:qx.html.Element,members:{__jQ:null,__jR:null,tagNameHint:null,setPadding:function(o,p){this.__jR=o;this.__jQ=p;if(this.getNodeName()==k){this.setStyle(n,o+f+p+e);}
;}
...
;var u=this._getProperty(a);var r=this._getProperty(i);var v=r?i:g;if(u!=null){u=u||null;q.paddingTop=this.__jQ;q.paddingLeft=this.__jR;qx.bom.element.Decoration.update(s,u,v,q);}
;}
;}
...
;}
;}
,getTagName:function(bA,bz){if(bz&&qx.core.Environment.get(u)&&this.__jT[bA]&&bz.endsWith(d)){return k;}
;return this.__jU[bA];}
...
;}
;var bF;if(qx.core.Environment.get(u)&&this.__jT[bC]&&bD===G){var bG=this.__jW(bE);this.__jV(bB,bG.width,bG.height);bF=this.processAlphaFix(bB,bC,bE);}
else {delete bB.clip;if(bC===i){bF=this.__jX(bB,bC,bE);}
else if(bC===t||bC===T){bF=this.__jY(bB,bC,bE);}
else {bF=this.__kc(bB,bC,bE);}
;}
;return bF;}
,__jV:function(bI,bH,bJ){if(bI.width==null&&bH!=null){bI.width=bH+c;}
;if(bI.height==null&&bJ!=null){bI.height=bJ+c;}
;}
,__jW:function(bK){var bL=qx.util.ResourceManager.getInstance().getImageWidth(bK)||qx.io.ImageLoader.getWidth(bK);var bM=qx.util.ResourceManager.getInstance().getImageHeight(bK)||qx.io.ImageLoader.getHeight(bK);return {width:bL,height:bM};}
...
,__jX:function(bT,bU,bS){var bV=qx.util.ResourceManager.getInstance().toUri(bS);var bW=this.__jW(bS);this.__jV(bT,bW.width,bW.height);return {src:bV,style:bT};}
,__jY:function(bX,bY,cb){var ca=qx.util.ResourceManager.getInstance();var ce=ca.getCombinedFormat(cb);var cg=this.__jW(cb);var cc;if(ce){var cf=ca.getData(cb);var cd=cf[4];if(ce==Q){cc=ca.toDataUri(cb);}
else {cc=ca.toUri(cd);}
;if(bY===t){bX=this.__ka(bX,cf,cg.height);}
else {bX=this.__kb(bX,cf,cg.width);}
;return {src:cc,style:bX};}
else {if(qx.core.Environment.get(q)){this.__ke(cb);}
...
;}
,__ka:function(ch,ci,ck){var cj=qx.util.ResourceManager.getInstance().getImageHeight(ci[4]);ch.clip={top:-ci[6],height:ck};ch.height=cj+c;if(ch.top!=null){ch.top=(parseInt(ch.top,10)+ci[6])+c;}
else if(ch.bottom!=null){ch.bottom=(parseInt(ch.bottom,10)+ck-cj-ci[6])+c;}
;return ch;}
,__kb:function(cm,cn,cl){var co=qx.util.ResourceManager.getInstance().getImageWidth(cn[4]);cm.clip={left:-cn[5],width:cl};cm.width=co+c;if(cm.left!=null){cm.left=(parseInt(cm.left,10)+cn[5])+c;}
else if(cm.right!=null){cm.right=(parseInt(cm.right,10)+cl-co-cn[5])+c;}
;return cm;}
,__kc:function(cp,cq,ct){var cs=qx.util.ResourceManager.getInstance();var cr=cs.getCombinedFormat(ct);var cB=this.__jW(ct);if(cr&&cq!==b){var cA=cs.getData(ct);var cy=cA[4];if(cr==Q){var cv=cs.toDataUri(ct);var cu=0;var cw=0;}
...
;return {style:cp};}
else {var top=cp.paddingTop||0;var cC=cp.paddingLeft||0;cp.backgroundPosition=cC+g+top+c;if(qx.core.Environment.get(q)){if(cq!==b){this.__ke(ct);}
;}
;this.__jV(cp,cB.width,cB.height);this.__kd(cp,ct,cq);return {style:cp};}
;}
,__kd:function(cD,cG,cE){var top=null;var cJ=null;if(cD.backgroundPosition){var cF=cD.backgroundPosition.split(U);cJ=parseInt(cF[0],10);if(isNaN(cJ)){cJ=cF[0];}
...
;}
,__ke:function(cK){if(this.DEBUG&&qx.util.ResourceManager.getInstance().has(cK)&&cK.indexOf(a)==-1){if(!this.__jS[cK]){qx.log.Logger.debug(m+cK);this.__jS[cK]=true;}
;}
;}
}});}
)();
(function(){var a="')",b="gecko",c="background-image:url(",d="0",e=");",f="",g="px",h="number",i=")",j="background-repeat:",k="engine.version",l="data:",m=" ",n="qx.bom.element.Background",o=";",p="url(",q="background-position:",r="base64",s="url('",t="engine.name",u="'";qx.Class.define(n,{statics:{__kf:[c,null,e,q,null,o,j,null,o],__kg:{backgroundImage:null,backgroundPosition:null,backgroundRepeat:null},__kh:function(z,top){var v=qx.core.Environment.get(t);var x=qx.core.Environment.get(k);if(v==b&&x<1.9&&z==top&&typeof z==h){top+=0.01;}
;if(z){var y=(typeof z==h)?z+g:z;}
else {y=d;}
;if(top){var w=(typeof top==h)?top+g:top;}
else {w=d;}
;return y+m+w;}
,__ki:function(A){var String=qx.lang.String;var B=A.substr(0,50);return B.startsWith(l)&&String.contains(B,r);}
,compile:function(F,D,H,top){var G=this.__kh(H,top);var E=qx.util.ResourceManager.getInstance().toUri(F);if(this.__ki(E)){E=u+E+u;}
;var C=this.__kf;C[1]=E;C[4]=G;C[7]=D;return C.join(f);}
,getStyles:function(L,J,N,top){if(!L){return this.__kg;}
;var M=this.__kh(N,top);var K=qx.util.ResourceManager.getInstance().toUri(L);var O;if(this.__ki(K)){O=s+K+a;}
else {O=p+K+i;}
...
(function(){var a='indexOf',b='slice',c='concat',d='toLocaleLowerCase',e="qx.type.BaseString",f="",g='trim',h='match',j="qx.debug",k='search',m='replace',n='toLowerCase',o='charCodeAt',p='split',q='substring',r='lastIndexOf',s='substr',t='toLocaleUpperCase',u='toUpperCase',v='charAt';qx.Class.define(e,{extend:Object,construct:function(w){var w=w||f;this.__kn=w;this.length=w.length;}
,members:{$$isString:true,length:0,__kn:null,toString:function(){return this.__kn;}
...
(function(){var a="qx.locale.LocalizedString";qx.Class.define(a,{extend:qx.type.BaseString,construct:function(b,e,c,d){qx.type.BaseString.call(this,b);this.__ko=e;this.__kp=!!d;this.__kq=c;}
,members:{__kp:null,__ko:null,__kq:null,translate:function(){if(this.__kp){return qx.locale.Manager.getInstance().localize(this.__ko,this.__kq);}
;return qx.locale.Manager.getInstance().translate(this.__ko,this.__kq);}
,getMessageId:function(){return this.__ko;}
}});}
)();
(function(){var a="qx.bom.client.Locale",b="-",c="locale",d="",e="android",f="locale.variant",g="C",h="locale.default";qx.Bootstrap.define(a,{statics:{getLocale:function(){var i=qx.bom.client.Locale.__kr();var j=i.indexOf(b);if(j!=-1){i=i.substr(0,j);}
;return i;}
,getVariant:function(){var k=qx.bom.client.Locale.__kr();var m=d;var l=k.indexOf(b);if(l!=-1){m=k.substr(l+1);}
;return m;}
,__kr:function(){var n=(navigator.userLanguage||navigator.language||d);if(qx.bom.client.OperatingSystem.getName()==e){var o=/^(\d+)\.(\d+)(\..+)?/i.exec(qx.bom.client.OperatingSystem.getVersion());if(qx.lang.Type.isArray(o)&&o.length>=3){if(parseInt(o[1])<4||(parseInt(o[1])===4&&parseInt(o[2])<4)){var p=/(\w{2})-(\w{2})/i.exec(navigator.userAgent);if(p){n=p[0];}
;}
;}
;}
...
(function(){var a="locale",b="_applyLocale",c="",d="changeLocale",e="_",f="Locale: ",g="locale.default",h="locale.variant",j="qx.dynlocale",k=" not available.",l="qx.locale.Manager",m="String",n="singleton",o="qx.debug";qx.Class.define(l,{type:n,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__ks=qx.$$translations||{};this.__kt=qx.$$locales||{};this.initLocale();this.__ku=this.getLocale();}
...
;return I;}
)()}},members:{__kv:qx.core.Environment.get(g),__kw:null,__kx:null,__ks:null,__kt:null,__ku:null,getLanguage:function(){return this.__kx;}
,getTerritory:function(){return this.getLocale().split(e)[1]||c;}
,getAvailableLocales:function(L){var M=[];for(var K in this.__kt){if(K!=this.__kv){if(this.__kt[K]===null&&!L){continue;}
;M.push(K);}
;}
;return M;}
,__ky:function(N){var P;if(N==null){return null;}
;var O=N.indexOf(e);if(O==-1){P=N;}
else {P=N.substring(0,O);}
;return P;}
,_applyLocale:function(R,Q){if(qx.core.Environment.get(o)){if(!(R in this.__kt||R==this.__ku)){qx.log.Logger.warn(f+R+k);}
;}
;this.__kw=R;this.__kx=this.__ky(R);}
,addTranslation:function(S,V){var T=this.__ks;if(T[S]){for(var U in V){T[S][U]=V[U];}
;}
else {T[S]=V;}
;}
,addLocale:function(ba,X){var W=this.__kt;if(W[ba]){for(var Y in X){W[ba][Y]=X[Y];}
;}
else {W[ba]=X;}
;}
,translate:function(be,bd,bb){var bc=this.__ks;return this.__kz(bc,be,bd,bb);}
,localize:function(bi,bh,bf){var bg=this.__kt;return this.__kz(bg,bi,bh,bf);}
,__kz:function(bn,bo,bl,bm){if(qx.core.Environment.get(o)){this.assertObject(bn);this.assertString(bo);this.assertArray(bl);}
;var bj;if(!bn){return bo;}
;if(bm){var bk=this.__ky(bm);}
else {bm=this.__kw;bk=this.__kx;}
;if(!bj&&bn[bm]){bj=bn[bm][bo];}
;if(!bj&&bn[bk]){bj=bn[bk][bo];}
;if(!bj&&bn[this.__kv]){bj=bn[this.__kv][bo];}
;if(!bj){bj=bo;}
...
;if(qx.core.Environment.get(j)){bj=new qx.locale.LocalizedString(bj,bo,bl,bn===this.__kt);}
;return bj;}
}});}
)();
(function(){var a="blur",b="qxDraggable",c="touch",d="qx.ui.core.Widget",f="longtap",g="Escape",h="drag",i="keydown",j="Unsupported data type: ",k="roll",l="drop",m="qxDroppable",n="qx.event.handler.DragDrop",o="mouse",p="This method must not be used outside the drop event listener!",q="Control",r="Shift",s="!",t="alias",u="droprequest",v="copy",w="pointerup",x="dragstart",y="move",z="pointerdown",A="dragchange",B="on",C="Alt",D="keyup",E="keypress",F="dragleave",G="none",H="dragend",I="dragover",J="left",K="Please use a droprequest listener to the drag source to fill the manager with data!",L="pointermove",M="display";qx.Class.define(n,{extend:qx.core.Object,implement:[qx.event.IEventHandler,qx.core.IDisposable],construct:function(N){qx.core.Object.call(this);this.__dt=N;this.__dW=N.getWindow().document.documentElement;this.__dt.addListener(this.__dW,f,this._onLongtap,this);this.__dt.addListener(this.__dW,z,this._onPointerdown,this,true);qx.event.Registration.addListener(window,a,this._onWindowBlur,this);this.__lE();}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{dragstart:1,dragend:1,dragover:1,dragleave:1,drop:1,drag:1,dragchange:1,droprequest:1},IGNORE_CAN_HANDLE:true,ALLOWED_BUTTONS:[J],MIN_DRAG_DISTANCE:5},properties:{cursor:{check:d,nullable:true,init:null}},members:{__dt:null,__dW:null,__ls:null,__lt:null,__lu:null,__lv:null,__lw:null,__d:null,__lx:null,__ly:null,__lz:false,__lA:false,__lB:false,__lC:null,__lD:null,canHandleEvent:function(P,O){}
,registerEvent:function(S,R,Q){}
,unregisterEvent:function(V,U,T){}
,addType:function(W){this.__lu[W]=true;}
,addAction:function(X){this.__lv[X]=true;}
,supportsType:function(Y){return !!this.__lu[Y];}
,supportsAction:function(ba){return !!this.__lv[ba];}
,setDropAllowed:function(bb){this.__lA=bb;this.__lF();}
,getData:function(bc){if(!this.__lA||!this.__ls){throw new Error(p);}
;if(!this.__lu[bc]){throw new Error(j+bc+s);}
;if(!this.__d[bc]){this.__lx=bc;this.__cM(u,this.__lt,this.__ls,false);}
;if(!this.__d[bc]){throw new Error(K);}
;return this.__d[bc]||null;}
,getCurrentAction:function(){this.__lF();return this.__ly;}
,getDragTarget:function(){return this.__lC;}
,addData:function(bd,be){this.__d[bd]=be;}
,getCurrentType:function(){return this.__lx;}
,isSessionActive:function(){return this.__lz;}
,__lE:function(){this.__lu={};this.__lv={};this.__lw={};this.__d={};}
,__lF:function(){if(this.__lt==null){return;}
;var bi=this.__lv;var bf=this.__lw;var bg=null;if(this.__lA){if(bf.Shift&&bf.Control&&bi.alias){bg=t;}
...
;}
;var bh=this.__ly;if(bg!=bh){if(this.__ls){this.__ly=bg;this.__lB=this.__cM(A,this.__ls,this.__lt,true);if(!this.__lB){bg=null;}
;}
;if(bg!=bh){this.__ly=bg;this.__cM(A,this.__lt,this.__ls,false);}
;}
;}
,__cM:function(bo,bk,bl,bm,bp){var bn=qx.event.Registration;var bj=bn.createEvent(bo,qx.event.type.Drag,[bm,bp]);if(bk!==bl){bj.setRelatedTarget(bl);}
;return bn.dispatchEvent(bk,bj);}
,__lG:function(bq){while(bq&&bq.nodeType==1){if(bq.getAttribute(b)==B){return bq;}
;bq=bq.parentNode;}
;return null;}
,__lH:function(br){while(br&&br.nodeType==1){if(br.getAttribute(m)==B){return br;}
;br=br.parentNode;}
;return null;}
,clearSession:function(){this.__dt.removeListener(this.__dW,L,this._onPointermove,this);this.__dt.removeListener(this.__dW,w,this._onPointerup,this,true);this.__dt.removeListener(this.__dW,i,this._onKeyDown,this,true);this.__dt.removeListener(this.__dW,D,this._onKeyUp,this,true);this.__dt.removeListener(this.__dW,E,this._onKeyPress,this,true);this.__dt.removeListener(this.__dW,k,this._onRoll,this,true);if(this.__lt){this.__cM(H,this.__lt,this.__ls,false);}
;this.__lA=false;this.__ls=null;if(this.__lC){this.__lC.removeState(h);this.__lC=null;}
;this.__lt=null;this.__lz=false;this.__lD=null;this.__lE();}
,_onLongtap:function(e){if(e.getPointerType()!=c){return;}
;this.__dt.addListener(this.__dW,k,this._onRoll,this,true);this._start(e);}
,_start:function(e){var bt=qx.event.handler.DragDrop.ALLOWED_BUTTONS.indexOf(e.getButton())!==-1;if(!e.isPrimary()||!bt){return false;}
;var bu=this.__lD?this.__lD.target:e.getTarget();var bs=this.__lG(bu);if(bs){this.__lt=bs;var bv=qx.ui.core.Widget.getWidgetByElement(this.__lD.original);while(bv&&bv.isAnonymous()){bv=bv.getLayoutParent();}
;if(bv){this.__lC=bv;bv.addState(h);}
;if(!this.__cM(x,this.__lt,this.__ls,true,e)){return false;}
;this.__dt.addListener(this.__dW,i,this._onKeyDown,this,true);this.__dt.addListener(this.__dW,D,this._onKeyUp,this,true);this.__dt.addListener(this.__dW,E,this._onKeyPress,this,true);this.__lz=true;return true;}
;}
,_onPointerdown:function(e){if(e.isPrimary()){this.__lD={target:e.getTarget(),original:e.getOriginalTarget(),left:e.getDocumentLeft(),top:e.getDocumentTop()};this.__dt.addListener(this.__dW,L,this._onPointermove,this);this.__dt.addListener(this.__dW,w,this._onPointerup,this,true);}
;}
,_onPointermove:function(e){if(!e.isPrimary()){return;}
;if(!this.__lz&&e.getPointerType()==o){var bC=this._getDelta(e);var bw=qx.event.handler.DragDrop.MIN_DRAG_DISTANCE;if(bC&&(Math.abs(bC.x)>bw||Math.abs(bC.y)>bw)){if(!this._start(e)){this.clearSession();return;}
;}
;}
;if(!this.__lz){return;}
;if(!this.__cM(h,this.__lt,this.__ls,true,e)){this.clearSession();}
;var bx=e.getTarget();if(this.__lD.target===bx){bx=e.getNativeEvent().view.document.elementFromPoint(e.getDocumentLeft(),e.getDocumentTop());}
...
;if(bx!==bz){var by=this.__lH(bx);if(by&&by!=this.__ls){if(this.__ls){this.__cM(F,this.__ls,this.__lt,false,e);}
;this.__lA=true;this.__ls=by;this.__lA=this.__cM(I,by,this.__lt,true,e);}
else if(!by&&this.__ls){this.__cM(F,this.__ls,this.__lt,false,e);this.__ls=null;this.__lA=false;qx.event.Timer.once(this.__lF,this,0);}
;}
;var bB=this.__lw;bB.Control=e.isCtrlPressed();bB.Shift=e.isShiftPressed();bB.Alt=e.isAltPressed();this.__lF();}
,_getDelta:function(e){if(!this.__lD){return null;}
;var bE=e.getDocumentLeft()-this.__lD.left;var bF=e.getDocumentTop()-this.__lD.top;return {"x":bE,"y":bF};}
,_onPointerup:function(e){if(!e.isPrimary()){return;}
;if(this.__lA&&this.__lB){this.__cM(l,this.__ls,this.__lt,false,e);}
;if(e.getTarget()==this.__lt){e.stopPropagation();}
;this.clearSession();}
,_onRoll:function(e){e.stop();}
,_onWindowBlur:function(e){this.clearSession();}
,_onKeyDown:function(e){var bG=e.getKeyIdentifier();switch(bG){case C:case q:case r:if(!this.__lw[bG]){this.__lw[bG]=true;this.__lF();}
;};}
,_onKeyUp:function(e){var bH=e.getKeyIdentifier();switch(bH){case C:case q:case r:if(this.__lw[bH]){this.__lw[bH]=false;this.__lF();}
;};}
,_onKeyPress:function(e){var bI=e.getKeyIdentifier();switch(bI){case g:this.clearSession();};}
},destruct:function(){qx.event.Registration.removeListener(window,a,this._onWindowBlur,this);this.__lt=this.__ls=this.__dt=this.__dW=this.__lu=this.__lv=this.__lw=this.__d=null;}
...
(function(){var b="-",c="PageUp",d="Escape",f="Boolean",g="+",h="qx.event.type.Data",j="_applyShortcut",k="PrintScreen",l="NumLock",m="short",n="keydown",o="5",p="8",q="execute",r="Meta",s="0",t="",u="2",v="Control",w="Shift",x="You can only specify one non modifier key!",y="3",z="Unidentified",A="/",B="Delete",C="String",D="changeEnabled",E="*",F="qx.bom.Shortcut",G="6",H="4",I="1",J="Alt",K="Not a valid key name for a shortcut: ",L="PageDown",M="Whitespaces are not allowed within shortcuts",N="_applyEnabled",O="keypress",P="7",Q="a",R="z",S="9";qx.Class.define(F,{extend:qx.core.Object,implement:[qx.core.IDisposable],construct:function(T){qx.core.Object.call(this);this.__rh={};this.__ri=null;if(T!=null){this.setShortcut(T);}
;this.initEnabled();}
,events:{"execute":h},properties:{enabled:{init:true,check:f,event:D,apply:N},shortcut:{check:C,apply:j,nullable:true},autoRepeat:{check:f,init:false}},members:{__rh:t,__ri:t,execute:function(U){this.fireDataEvent(q,U);}
,__rj:function(event){if(this.getEnabled()&&this.__rk(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,__kI:function(event){if(this.getEnabled()&&this.__rk(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,_applyEnabled:function(W,V){if(W){qx.event.Registration.addListener(document.documentElement,n,this.__rj,this);qx.event.Registration.addListener(document.documentElement,O,this.__kI,this);}
else {qx.event.Registration.removeListener(document.documentElement,n,this.__rj,this);qx.event.Registration.removeListener(document.documentElement,O,this.__kI,this);}
;}
,_applyShortcut:function(bb,Y){if(bb){if(bb.search(/[\s]+/)!=-1){var bc=M;this.error(bc);throw new Error(bc);}
;this.__rh={"Control":false,"Shift":false,"Meta":false,"Alt":false};this.__ri=null;var X;var a=[];while(bb.length>0&&X!=-1){X=bb.search(/[-+]+/);a.push((bb.length==1||X==-1)?bb:bb.substring(0,X));bb=bb.substring(X+1);}
;var ba=a.length;for(var i=0;i<ba;i++ ){var bd=this.__rm(a[i]);switch(bd){case v:case w:case r:case J:this.__rh[bd]=true;break;case z:var bc=K+a[i];this.error(bc);throw bc;default:if(this.__ri){var bc=x;this.error(bc);throw bc;}
;this.__ri=bd;};}
;}
;return true;}
,__rk:function(e){var be=this.__ri;if(!be){return false;}
;if((!this.__rh.Shift&&e.isShiftPressed())||(this.__rh.Shift&&!e.isShiftPressed())||(!this.__rh.Control&&e.isCtrlPressed())||(this.__rh.Control&&!e.isCtrlPressed())||(!this.__rh.Meta&&e.isMetaPressed())||(this.__rh.Meta&&!e.isMetaPressed())||(!this.__rh.Alt&&e.isAltPressed())||(this.__rh.Alt&&!e.isAltPressed())){return false;}
;if(be==e.getKeyIdentifier()){return true;}
;return false;}
,__rl:{esc:d,ctrl:v,print:k,del:B,pageup:c,pagedown:L,numlock:l,numpad_0:s,numpad_1:I,numpad_2:u,numpad_3:y,numpad_4:H,numpad_5:o,numpad_6:G,numpad_7:P,numpad_8:p,numpad_9:S,numpad_divide:A,numpad_multiply:E,numpad_minus:b,numpad_plus:g},__rm:function(bg){var bf=qx.event.util.Keyboard;var bh=z;if(bf.isValidKeyIdentifier(bg)){return bg;}
;if(bg.length==1&&bg>=Q&&bg<=R){return bg.toUpperCase();}
;bg=bg.toLowerCase();var bh=this.__rl[bg]||qx.lang.String.firstUp(bg);if(bf.isValidKeyIdentifier(bh)){return bh;}
else {return z;}
;}
,toString:function(){var bj=this.__ri;var bk=[];for(var bi in this.__rh){if(this.__rh[bi]){bk.push(qx.locale.Key.getKeyName(m,bi));}
;}
;if(bj){bk.push(qx.locale.Key.getKeyName(m,bj));}
;return bk.join(g);}
},destruct:function(){this.setEnabled(false);this.__rh=this.__ri=null;}
});}
)();
...
;}
},defer:function(bl){qx.event.Registration.addListener(document.documentElement,H,bl.__kI,bl);qx.event.Registration.addListener(document.documentElement,f,bl.__kI,bl);}
});}
)();
