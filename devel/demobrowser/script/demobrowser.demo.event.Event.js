...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.event.Event.261f41b45311.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,_initLogger:function(p,n,o){this.__x=f+p.join(c)+a;this.__y=o||50;this.__z=n;this._clearLog();}
,_clearLog:function(){this.__z.innerHTML=this.__x+k;this.__A=[];}
,_log:function(q){this.__A.unshift(q);this.__A=this.__A.slice(0,this.__y);var r=[this.__x];for(var i=0;i<this.__A.length;i++ ){r.push(g,this.__A[i].join(e),l);}
;r.push(k);this.__z.innerHTML=r.join(d);this.__z.scrollTop=0;}
,__x:null,__y:null,__z:null,__A:null},destruct:function(){this.__z=this.__A=null;}
});}
)();
...
;event.stop();}
;}
,__ix:function(event){if(this.getEnabled()&&this.__qJ(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,_applyEnabled:function(W,V){if(W){qx.event.Registration.addListener(document.documentElement,n,this.__qI,this);qx.event.Registration.addListener(document.documentElement,O,this.__ix,this);}
else {qx.event.Registration.removeListener(document.documentElement,n,this.__qI,this);qx.event.Registration.removeListener(document.documentElement,O,this.__ix,this);}
;}
