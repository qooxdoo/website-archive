...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.event.Event.892b2f2645d0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,_initLogger:function(p,n,o){this.__AN=f+p.join(c)+a;this.__AO=o||50;this.__AP=n;this._clearLog();}
,_clearLog:function(){this.__AP.innerHTML=this.__AN+k;this.__AQ=[];}
,_log:function(q){this.__AQ.unshift(q);this.__AQ=this.__AQ.slice(0,this.__AO);var r=[this.__AN];for(var i=0;i<this.__AQ.length;i++ ){r.push(g,this.__AQ[i].join(e),l);}
;r.push(k);this.__AP.innerHTML=r.join(d);this.__AP.scrollTop=0;}
,__AN:null,__AO:null,__AP:null,__AQ:null},destruct:function(){this.__AP=this.__AQ=null;}
});}
)();
...
;}
;return ck;}
,__hC:qx.event.GlobalError.observeMethod(qx.core.Environment.select(A,{"mshtml":function(cm){return this.__hB(cm);}
,"webkit":function(cn){return this.__hB(cn);}
,"default":function(co){return co;}
})),__hD:qx.event.GlobalError.observeMethod(qx.core.Environment.select(A,{"mshtml":function(cp){var cq=qx.bom.Event.getTarget(cp);if(!this.__hH(cq)){qx.bom.Event.preventDefault(cp);}
;}
,"webkit":function(cr){var cs=qx.bom.Event.getTarget(cr);if(!this.__hH(cs)){qx.bom.Event.preventDefault(cr);}
;}
,"default":null})),__hE:function(ct){var cu=qx.bom.element.Attribute.get(ct,u);if(cu>=1){return true;}
;var cv=qx.event.handler.Focus.FOCUSABLE_ELEMENTS;if(cu>=0&&cv[ct.tagName]){return true;}
;return false;}
,__hF:function(cw){while(cw&&cw.nodeType===1){if(cw.getAttribute(E)==w){return null;}
;if(this.__hE(cw)){return cw;}
;cw=cw.parentNode;}
;return this._body;}
,__hG:function(cx){var cy=cx;while(cx&&cx.nodeType===1){if(cx.getAttribute(G)==w){return null;}
;cx=cx.parentNode;}
;return cy;}
,__hH:function(cz){while(cz&&cz.nodeType===1){var cA=cz.getAttribute(t);if(cA!=null){return cA===w;}
;cz=cz.parentNode;}
;return true;}
,_applyActive:function(cC,cB){if(cB){this.__cK(cB,cC,k,true);}
;if(cC){this.__cK(cC,cB,x,true);}
;if(this.__he){window.scrollTo(0,0);}
;}
,_applyFocus:function(cE,cD){if(cD){this.__cK(cD,cE,D,true);}
;if(cE){this.__cK(cE,cD,y,true);}
;if(cD){this.__cK(cD,cE,c,false);}
;if(cE){this.__cK(cE,cD||this.__hr,f,false);}
;}
...
,statics:{__bv:qx.$$resources||{},__iJ:{}},members:{getIds:function(v){var x=this.self(arguments).__bv;if(!x){return null;}
;var w=[];for(var u in x){if(x.hasOwnProperty(u)){if(v&&u.indexOf(v)==-1){continue;}
;w.push(u);}
;}
;return w;}
,has:function(y){return !!this.self(arguments).__bv[y];}
,getData:function(z){return this.self(arguments).__bv[z]||null;}
,getImageWidth:function(B){var A=this.self(arguments).__bv[B];return A?A[0]:null;}
,getImageHeight:function(D){var C=this.self(arguments).__bv[D];return C?C[1]:null;}
,getImageFormat:function(F){if(F&&F.startsWith(k)){return l;}
;var E=this.self(arguments).__bv[F];return E?E[2]:null;}
,getCombinedFormat:function(K){var H=e;var J=this.self(arguments).__bv[K];var G=J&&J.length>4&&typeof (J[4])==j&&this.constructor.__bv[J[4]];if(G){var L=J[4];var I=this.constructor.__bv[L];H=I[2];}
;return H;}
,toUri:function(P){if(P==null){return P;}
;var M=this.self(arguments).__bv[P];if(!M){return P;}
;if(typeof M===j){var O=M;}
else {var O=M[3];if(!O){return P;}
;}
;var N=e;if((qx.core.Environment.get(b)==a)&&qx.core.Environment.get(d)){N=this.self(arguments).__iJ[O];}
;return N+qx.util.LibraryManager.getInstance().get(O,q)+p+P;}
,toDataUri:function(S){var R=this.constructor.__bv[S];var U=R?this.constructor.__bv[R[4]]:null;var T;if(U){var Q=U[4][S];T=n+Q[h]+o+Q[f]+t+Q[m];}
else {T=this.toUri(S);}
;return T;}
...
else {X.__iJ[Y]=e;continue;}
;var bd;var bb=qx.bom.Selector.query(i,document);if(bb.length>0){bd=bb[0].href;}
;if(W.match(/^\/\//)!=null){X.__iJ[Y]=window.location.protocol;}
else if(W.match(/^\//)!=null){if(bd){X.__iJ[Y]=bd;}
else {X.__iJ[Y]=window.location.protocol+c+window.location.host;}
;}
else if(W.match(/^\.\//)!=null){var bc=document.URL;X.__iJ[Y]=bc.substring(0,bc.lastIndexOf(p)+1);}
else if(W.match(/^http/)!=null){X.__iJ[Y]=e;}
...
;}
;X.__iJ[Y]=bd.substring(0,bd.lastIndexOf(p)+1);}
;}
;}
;}
;}
...
(function(){var b="-",c="PageUp",d="Escape",f="Boolean",g="+",h="qx.event.type.Data",j="_applyShortcut",k="PrintScreen",l="NumLock",m="short",n="keydown",o="5",p="8",q="execute",r="Meta",s="0",t="",u="2",v="Control",w="Shift",x="You can only specify one non modifier key!",y="3",z="Unidentified",A="/",B="Delete",C="String",D="changeEnabled",E="*",F="qx.bom.Shortcut",G="6",H="4",I="1",J="Alt",K="Not a valid key name for a shortcut: ",L="PageDown",M="Whitespaces are not allowed within shortcuts",N="_applyEnabled",O="keypress",P="7",Q="a",R="z",S="9";qx.Class.define(F,{extend:qx.core.Object,implement:[qx.core.IDisposable],construct:function(T){qx.core.Object.call(this);this.__rf={};this.__rg=null;if(T!=null){this.setShortcut(T);}
;this.initEnabled();}
,events:{"execute":h},properties:{enabled:{init:true,check:f,event:D,apply:N},shortcut:{check:C,apply:j,nullable:true},autoRepeat:{check:f,init:false}},members:{__rf:t,__rg:t,execute:function(U){this.fireDataEvent(q,U);}
,__rh:function(event){if(this.getEnabled()&&this.__ri(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,__kG:function(event){if(this.getEnabled()&&this.__ri(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,_applyEnabled:function(W,V){if(W){qx.event.Registration.addListener(document.documentElement,n,this.__rh,this);qx.event.Registration.addListener(document.documentElement,O,this.__kG,this);}
else {qx.event.Registration.removeListener(document.documentElement,n,this.__rh,this);qx.event.Registration.removeListener(document.documentElement,O,this.__kG,this);}
;}
,_applyShortcut:function(bb,Y){if(bb){if(bb.search(/[\s]+/)!=-1){var bc=M;this.error(bc);throw new Error(bc);}
;this.__rf={"Control":false,"Shift":false,"Meta":false,"Alt":false};this.__rg=null;var X;var a=[];while(bb.length>0&&X!=-1){X=bb.search(/[-+]+/);a.push((bb.length==1||X==-1)?bb:bb.substring(0,X));bb=bb.substring(X+1);}
;var ba=a.length;for(var i=0;i<ba;i++ ){var bd=this.__rk(a[i]);switch(bd){case v:case w:case r:case J:this.__rf[bd]=true;break;case z:var bc=K+a[i];this.error(bc);throw bc;default:if(this.__rg){var bc=x;this.error(bc);throw bc;}
;this.__rg=bd;};}
;}
;return true;}
,__ri:function(e){var be=this.__rg;if(!be){return false;}
;if((!this.__rf.Shift&&e.isShiftPressed())||(this.__rf.Shift&&!e.isShiftPressed())||(!this.__rf.Control&&e.isCtrlPressed())||(this.__rf.Control&&!e.isCtrlPressed())||(!this.__rf.Meta&&e.isMetaPressed())||(this.__rf.Meta&&!e.isMetaPressed())||(!this.__rf.Alt&&e.isAltPressed())||(this.__rf.Alt&&!e.isAltPressed())){return false;}
;if(be==e.getKeyIdentifier()){return true;}
;return false;}
,__rj:{esc:d,ctrl:v,print:k,del:B,pageup:c,pagedown:L,numlock:l,numpad_0:s,numpad_1:I,numpad_2:u,numpad_3:y,numpad_4:H,numpad_5:o,numpad_6:G,numpad_7:P,numpad_8:p,numpad_9:S,numpad_divide:A,numpad_multiply:E,numpad_minus:b,numpad_plus:g},__rk:function(bg){var bf=qx.event.util.Keyboard;var bh=z;if(bf.isValidKeyIdentifier(bg)){return bg;}
;if(bg.length==1&&bg>=Q&&bg<=R){return bg.toUpperCase();}
;bg=bg.toLowerCase();var bh=this.__rj[bg]||qx.lang.String.firstUp(bg);if(bf.isValidKeyIdentifier(bh)){return bh;}
else {return z;}
;}
,toString:function(){var bj=this.__rg;var bk=[];for(var bi in this.__rf){if(this.__rf[bi]){bk.push(qx.locale.Key.getKeyName(m,bi));}
;}
;if(bj){bk.push(qx.locale.Key.getKeyName(m,bj));}
;return bk.join(g);}
},destruct:function(){this.setEnabled(false);this.__rf=this.__rg=null;}
});}
)();
