...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.OwnCodeInModel.60ae0b428449.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="text",b="Retry open native request with method: ",c="activex",d="No XHR support available.",f="If-None-Match",g="xhr",h="If-Modified-Since",i="engine.version",j="onunload",k="GET",l="-1",m="qx.debug.io",n="Undefined",o="Open native request with method: ",p="error",q="loadend",r="Blob",s="HTMLDocument",t="progress",u="load",v="Received native readyState: ",w="abort",x=", url: ",y="String",z="browser.documentmode",A="document",B="",C="readystatechange",D="Microsoft.XMLHTTP",E="Already disposed",F="browser.version",G="opera",H="qx.bom.request.Xhr",I="on",J="timeout",K="gecko",L="If-Match",M="mshtml",N="engine.name",O="Microsoft.XMLDOM",P="file:",Q="Send native request",R="FormData",S="If-Range",T="Content-Type",U="io.xhr",V="Not enough arguments",W="ArrayBuffer",X="undefined",Y=", async: ",bb="Native XHR object doesn't support overrideMimeType.";qx.Bootstrap.define(H,{extend:Object,implement:[qx.core.IDisposable],construct:function(){var bc=qx.Bootstrap.bind(this.__sN,this);if(qx.event&&qx.event.GlobalError&&qx.event.GlobalError.observeMethod){this.__sx=qx.event.GlobalError.observeMethod(bc);}
else {this.__sx=bc;}
;this.__sy=qx.Bootstrap.bind(this.__sL,this);this.__sz=qx.Bootstrap.bind(this.__sM,this);this.__sn=qx.Bootstrap.bind(this.__sR,this);this.__sK();this._emitter=new qx.event.Emitter();if(window.attachEvent){this.__sA=qx.Bootstrap.bind(this.__sU,this);window.attachEvent(j,this.__sA);}
;}
,statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},events:{"readystatechange":H,"error":H,"loadend":H,"timeout":H,"abort":H,"load":H,"progress":H},members:{readyState:0,responseText:B,response:null,responseXML:null,status:0,statusText:B,responseType:B,timeout:0,progress:null,open:function(bh,bd,be,bg,bf){this.__sW();if(typeof bd===X){throw new Error(V);}
else if(typeof bh===X){bh=k;}
;this.__sq=false;this.__sB=false;this.__sC=false;this.__sD=bd;if(typeof be==X){be=true;}
;this.__sE=be;this.status=0;this.statusText=this.responseText=B;this.responseXML=null;this.response=null;if(!this.__sV()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();this.__sK();}
;this.__sG.onreadystatechange=this.__sx;try{if(qx.core.Environment.get(m)){qx.Bootstrap.debug(qx.bom.request.Xhr,o+bh+x+bd+Y+be);}
;this.__sG.open(bh,bd,be,bg,bf);}
catch(bi){if(!qx.util.Request.isCrossDomain(bd)){throw bi;}
;if(!this.__sE){this.__sF=bi;}
;if(this.__sE){if(window.XDomainRequest){this.readyState=4;this.__sG=new XDomainRequest();this.__sG.onerror=qx.Bootstrap.bind(function(){this._emit(C);this._emit(p);this._emit(q);}
,this);if(qx.core.Environment.get(m)){qx.Bootstrap.debug(qx.bom.request.Xhr,b+bh+x+bd+Y+be);}
;this.__sG.open(bh,bd,be,bg,bf);return;}
;window.setTimeout(qx.Bootstrap.bind(function(){if(this.__sI){return;}
;this.readyState=4;this._emit(C);this._emit(p);this._emit(q);}
,this));}
;}
;if(qx.core.Environment.get(N)===M&&qx.core.Environment.get(z)<9&&this.__sG.readyState>0){this.__sG.setRequestHeader(h,l);}
;if(qx.core.Environment.get(N)===K&&parseInt(qx.core.Environment.get(i),10)<2&&!this.__sE){this.readyState=qx.bom.request.Xhr.OPENED;this._emit(C);}
;}
,setRequestHeader:function(bj,bk){this.__sW();if(bj==L||bj==h||bj==f||bj==S){this.__sC=true;}
;this.__sG.setRequestHeader(bj,bk);return this;}
,send:function(bm){this.__sW();if(!this.__sE&&this.__sF){throw this.__sF;}
;if(qx.core.Environment.get(N)===G&&this.timeout===0){this.timeout=10000;}
;if(this.timeout>0){this.__sH=window.setTimeout(this.__sn,this.timeout);}
;bm=typeof bm==X?null:bm;var bl=qx.Bootstrap.getClass(bm);bm=(bm!==null&&this.__sJ.indexOf(bl)===-1)?bm.toString():bm;try{if(qx.core.Environment.get(m)){qx.Bootstrap.debug(qx.bom.request.Xhr,Q);}
;if(this.__sE){this.__sG.responseType=this.responseType;}
;this.__sG.send(bm);}
catch(bo){if(!this.__sE){throw bo;}
;if(this._getProtocol()===P){this.readyState=2;this.__sO();var bn=this;window.setTimeout(function(){if(bn.__sI){return;}
;bn.readyState=3;bn.__sO();bn.readyState=4;bn.__sO();}
);}
;}
;if(qx.core.Environment.get(N)===K&&!this.__sE){this.__sN();}
;this.__sB=true;return this;}
,abort:function(){this.__sW();this.__sq=true;this.__sG.abort();if(this.__sG&&this.readyState!==qx.bom.request.Xhr.DONE){this.readyState=this.__sG.readyState;}
;return this;}
...
,getResponseHeader:function(br){this.__sW();if(qx.core.Environment.get(z)===9&&this.__sG.aborted){return B;}
;return this.__sG.getResponseHeader(br);}
,getAllResponseHeaders:function(){this.__sW();if(qx.core.Environment.get(z)===9&&this.__sG.aborted){return B;}
;return this.__sG.getAllResponseHeaders();}
,overrideMimeType:function(bs){this.__sW();if(this.__sG.overrideMimeType){this.__sG.overrideMimeType(bs);}
else {throw new Error(bb);}
;return this;}
,getRequest:function(){return this.__sG;}
,dispose:function(){if(this.__sI){return false;}
;window.clearTimeout(this.__sH);if(window.detachEvent){window.detachEvent(j,this.__sA);}
;try{this.__sG.onreadystatechange;}
catch(bu){return false;}
;var bt=function(){}
;this.__sG.onreadystatechange=bt;this.__sG.onload=bt;this.__sG.onerror=bt;this.__sG.onprogress=bt;this.abort();this.__sG=null;this.__sI=true;return true;}
,isDisposed:function(){return !!this.__sI;}
...
,_getProtocol:function(){var bw=this.__sD;var bx=/^(\w+:)\/\//;if(bw!==null&&bw.match){var by=bw.match(bx);if(by&&by[1]){return by[1];}
;}
;return window.location.protocol;}
,__sG:null,__sE:null,__sx:null,__sy:null,__sz:null,__sA:null,__sn:null,__sB:null,__sD:null,__sq:null,__cU:null,__sI:null,__sH:null,__sF:null,__sC:null,__sJ:null,__sK:function(){this.__sG=this._createNativeXhr();this.__sG.onreadystatechange=this.__sx;if(qx.Bootstrap.getClass(this.__sG.onabort)!==n){this.__sG.onabort=this.__sy;}
;if(qx.Bootstrap.getClass(this.__sG.onprogress)!==n){this.__sG.onprogress=this.__sz;this.progress={lengthComputable:false,loaded:0,total:0};}
;this.__sI=this.__sB=this.__sq=false;this.__sJ=[W,r,s,y,R];}
,__sL:function(){if(!this.__sq){this.abort();}
;}
,__sM:function(e){this.progress.lengthComputable=e.lengthComputable;this.progress.loaded=e.loaded;this.progress.total=e.total;this._emit(t);}
,__sN:function(){var bz=this.__sG,bA=true;if(qx.core.Environment.get(m)){qx.Bootstrap.debug(qx.bom.request.Xhr,v+bz.readyState);}
;if(this.readyState==bz.readyState){return;}
;this.readyState=bz.readyState;if(this.readyState===qx.bom.request.Xhr.DONE&&this.__sq&&!this.__sB){return;}
;if(!this.__sE&&(bz.readyState==2||bz.readyState==3)){return;}
...
;if(bA){this.__sS();this.__sT();}
;}
;this.__sO();if(this.readyState==qx.bom.request.Xhr.DONE){if(bz){bz.onreadystatechange=function(){}
;}
;}
;}
,__sO:function(){if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__sH);}
;this._emit(C);if(this.readyState===qx.bom.request.Xhr.DONE){this.__sP();}
;}
,__sP:function(){if(this.__cU){this._emit(J);if(qx.core.Environment.get(N)===G){this._emit(p);}
;this.__cU=false;}
else {if(this.__sq){this._emit(w);}
else {if(this.__sQ()){this._emit(p);}
else {this._emit(u);}
;}
;}
;this._emit(q);}
...
;if(qx.core.Environment.get(N)===G){if(bE&&this.__sC&&!this.__sq&&this.status===0){this.status=304;}
;}
;}
,__sT:function(){if(qx.core.Environment.get(N)==M&&(this.getResponseHeader(T)||B).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var bF=new window.ActiveXObject(O);bF.async=false;bF.validateOnParse=false;bF.loadXML(this.responseText);this.responseXML=bF;}
;}
,__sU:function(){try{if(this){this.dispose();}
;}
catch(e){}
;}
,__sV:function(){var name=qx.core.Environment.get(N);var bG=qx.core.Environment.get(F);return !(name==M&&bG<9||name==K&&bG<3.5);}
,__sW:function(){if(this.__sI){throw new Error(E);}
;}
