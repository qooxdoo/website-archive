...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Dynamic.b7a67263d13b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,openNodeAndParents:function(bk){this.__CB(this.getModel(),bk);this.buildLookupTable();}
,closeNode:function(bl){if(qx.lang.Array.contains(this.__Cv,bl)){qx.lang.Array.remove(this.__Cv,bl);this.fireDataEvent(G,bl);this.buildLookupTable();}
;}
...
,isNodeOpen:function(bo){return qx.lang.Array.contains(this.__Cv,bo);}
,_init:function(){this.__ua=new qx.data.Array();this.__Cv=[];this.__Cw=[];this._initLayer();}
...
,this);}
,getLookupTable:function(){return this.__ua;}
,_reverseLookup:function(bp){return bp;}
,_getDataFromRow:function(bq){return this.__ua.getItem(bq);}
,_getSelectables:function(){return this.__ua;}
,getOpenNodes:function(){return this.__Cv;}
,isNode:function(br){return qx.ui.tree.core.Util.isNode(br,this.getChildProperty());}
,getLevel:function(bs){return this.__Cw[bs];}
...
,_applyModel:function(bQ,bP){this.__Cv=[];if(bQ!=null){if(qx.core.Environment.get(w)){if(!qx.Class.hasMixin(bQ.constructor,qx.data.marshal.MEventBubbling)){this.warn(H+h+k);}
;}
;bQ.addListener(J,this._onChangeBubble,this);this.__CA(bQ);}
...
;}
;if(this.__ua.indexOf(bV)!=-1){this.__Cy();}
;}
;}
,_onUpdated:function(event){if(this.__hp==null){this.__hp=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__hp.schedule();}
,_onOpen:function(event){var ca=event.getRow();var bY=this.__ua.getItem(ca);if(this.isNode(bY)){if(this.isNodeOpen(bY)){this.closeNode(bY);}
...
;break;};}
;}
,_beforeApplySelection:function(cg){if(cg.length===0&&this.getSelectionMode()===g){var cf=this.__CD();var ch=this.getLookupTable().indexOf(cf);if(ch>=0){cg.push(ch);}
;}
;}
,_afterApplySelection:function(){var ci=this.getSelection();if(ci.getLength()>0&&this.getSelectionMode()===g){this.__CC(ci.getItem(0));}
else {this.__Cx=[];}
;}
,__Cy:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(Q+f);}
;this._itemWidth=0;var cm=[];this.__Cw=[];var ck=-1;var cl=this.getModel();if(cl!=null){if(!this.isHideRoot()){ck++ ;cm.push(cl);this.__Cw.push(ck);}
;if(this.isNodeOpen(cl)){var cj=this.__Cz(cl,ck);cm=cm.concat(cj);}
;}
;if(!qx.lang.Array.equals(this.__ua.toArray(),cm)){this._provider.removeBindings();this.__ua.removeAll();this.__ua.append(cm);this.__xo();this._updateSelection();}
;}
,__Cz:function(cn,cs){var cr=[];cs++ ;if(!this.isNode(cn)){return cr;}
...
;if(this.isNode(cu)){this.__Cw.push(cs);cr.push(cu);if(this.isNodeOpen(cu)){var co=this.__Cz(cu,cs);cr=cr.concat(co);}
;}
else {if(this.isShowLeafs()){this.__Cw.push(cs);cr.push(cu);}
;}
;}
;cv.dispose();return cr;}
,__CA:function(cw){if(!qx.lang.Array.contains(this.__Cv,cw)){this.__Cv.push(cw);this.fireDataEvent(t,cw);}
;}
,__CB:function(cx,cy){if(cx===cy){this.__CA(cy);return true;}
;if(!this.isNode(cx)){return false;}
;var cB=cx.get(this.getChildProperty());if(cB==null){return false;}
;for(var i=0;i<cB.getLength();i++ ){var cA=cB.getItem(i);var cz=this.__CB(cA,cy);if(cz===true){this.__CA(cA);return true;}
;}
;return false;}
,__xo:function(){this.getPane().getRowConfig().setItemCount(this.__ua.getLength());this.getPane().fullUpdate();}
,getParent:function(cD){var cE=this.__ua.indexOf(cD);if(cE<0){return null;}
;var cF=this.__Cw[cE];while(cE>0){cE-- ;var cC=this.__Cw[cE];if(cC<cF){return this.__ua.getItem(cE);}
;}
;return null;}
,__CC:function(cG){this.__Cx=[];var parent=this.getParent(cG);while(parent!=null){this.__Cx.unshift(parent);parent=this.getParent(parent);}
;}
,__CD:function(){if(this.__Cx==null){return this.getModel();}
;var cH=this.getLookupTable();var parent=this.__Cx.pop();while(parent!=null){if(cH.contains(parent)){return parent;}
;parent=this.__Cx.pop();}
...
;}
;if(!qx.core.ObjectRegistry.inShutDown&&this.__hp!=null){this.__hp.cancel();this.__hp.dispose();}
;var cI=this.getModel();if(cI!=null){cI.removeListener(J,this._onChangeBubble,this);}
;this._layer.removeListener(R,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__ua.dispose();this._layer=this._provider=this.__ua=this.__Cv=this.__hp=null;}
});}
)();
...
(function(){var a="BindingIds",b="model",c="",d="changeDelegate",f="virtual-tree-file",g="label",h="appearance",i="icon",j="bindItem",k=".length",l="]",m="virtual-tree-folder",n="[",o=".",p="String",q="qx.ui.tree.core.MWidgetController";qx.Mixin.define(q,{construct:function(){this.__xB=[];}
,properties:{labelPath:{check:p,nullable:true},iconPath:{check:p,nullable:true},labelOptions:{nullable:true},iconOptions:{nullable:true},childProperty:{check:p,nullable:true},delegate:{event:d,init:null,nullable:true}},members:{__xB:null,bindDefaultProperties:function(u,s){this.bindProperty(c,b,null,u,s);this.bindProperty(this.getLabelPath(),g,this.getLabelOptions(),u,s);var r=this.__xC(s);var t=this._tree.getLookupTable();t=qx.data.SingleValueBinding.resolvePropertyChain(t,r);if(qx.util.OOUtil.hasProperty(t.constructor,this.getChildProperty())){this.bindProperty(this.getChildProperty()+k,h,{converter:function(){return m;}
},u,s);}
else {u.setAppearance(f);}
;if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),i,this.getIconOptions(),u,s);}
;}
,bindProperty:function(y,B,z,x,v){var A=this.__xC(v,y);var w=this._tree.getLookupTable();var C=w.bind(A,x,B,z);this.__uA(x,C);}
,bindPropertyReverse:function(J,F,H,I,D){var G=this.__xC(D,J);var E=this._tree.getLookupTable();var K=I.bind(F,E,G,H);this.__uA(I,K);}
,removeBindings:function(){while(this.__xB.length>0){var L=this.__xB.pop();this._removeBindingsFrom(L);}
;}
...
;}
,_removeBindingsFrom:function(P){var Q=this.__xD(P);while(Q.length>0){var R=Q.pop();try{this._tree.getLookupTable().removeBinding(R);}
catch(e){P.removeBinding(R);}
;}
;if(qx.lang.Array.contains(this.__xB,P)){qx.lang.Array.remove(this.__xB,P);}
;}
,__xC:function(T,U){var S=n+T+l;if(U!=null&&U!=c){S+=o+U;}
;return S;}
,__uA:function(W,X){var V=this.__xD(W);if(!qx.lang.Array.contains(V,X)){V.push(X);}
;if(!qx.lang.Array.contains(this.__xB,W)){this.__xB.push(W);}
;}
,__xD:function(Y){var ba=Y.getUserData(a);if(ba==null){ba=[];Y.setUserData(a,ba);}
;return ba;}
...
;var o=this._renderer.getCellWidget();o.setOpen(p&&this._tree.isNodeOpen(s));o.addListener(a,this.__CE,this);o.setUserData(b,this.getChildProperty());o.setUserData(g,this._tree.isShowLeafs());if(this._tree.getSelection().contains(s)){this._styleSelectabled(o);}
...
,poolCellWidget:function(u){u.removeListener(a,this.__CE,this);this._removeBindingsFrom(u);this._renderer.pool(u);this._onPool(u);}
...
,__CE:function(event){var K=event.getTarget();var L=K.getUserData(k);var J=this._tree.getLookupTable().getItem(L);if(event.getData()){this._tree.openNodeWithoutScrolling(J);}
