...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Dynamic.33ee72ae2ac2.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,openNodeAndParents:function(bk){this.__CC(this.getModel(),bk);this.buildLookupTable();}
,closeNode:function(bl){if(qx.lang.Array.contains(this.__Cw,bl)){qx.lang.Array.remove(this.__Cw,bl);this.fireDataEvent(G,bl);this.buildLookupTable();}
;}
...
,isNodeOpen:function(bo){return qx.lang.Array.contains(this.__Cw,bo);}
,_init:function(){this.__ub=new qx.data.Array();this.__Cw=[];this.__Cx=[];this._initLayer();}
...
,this);}
,getLookupTable:function(){return this.__ub;}
,_reverseLookup:function(bp){return bp;}
,_getDataFromRow:function(bq){return this.__ub.getItem(bq);}
,_getSelectables:function(){return this.__ub;}
,getOpenNodes:function(){return this.__Cw;}
,isNode:function(br){return qx.ui.tree.core.Util.isNode(br,this.getChildProperty());}
,getLevel:function(bs){return this.__Cx[bs];}
...
,_applyModel:function(bQ,bP){this.__Cw=[];if(bQ!=null){if(qx.core.Environment.get(w)){if(!qx.Class.hasMixin(bQ.constructor,qx.data.marshal.MEventBubbling)){this.warn(H+h+k);}
;}
;bQ.addListener(J,this._onChangeBubble,this);this.__CB(bQ);}
...
;}
;if(this.__ub.indexOf(bV)!=-1){this.__Cz();}
;}
;}
,_onUpdated:function(event){if(this.__hq==null){this.__hq=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__hq.schedule();}
,_onOpen:function(event){var ca=event.getRow();var bY=this.__ub.getItem(ca);if(this.isNode(bY)){if(this.isNodeOpen(bY)){this.closeNode(bY);}
...
;break;};}
;}
,_beforeApplySelection:function(cg){if(cg.length===0&&this.getSelectionMode()===g){var cf=this.__CE();var ch=this.getLookupTable().indexOf(cf);if(ch>=0){cg.push(ch);}
;}
;}
,_afterApplySelection:function(){var ci=this.getSelection();if(ci.getLength()>0&&this.getSelectionMode()===g){this.__CD(ci.getItem(0));}
else {this.__Cy=[];}
;}
,__Cz:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(Q+f);}
;this._itemWidth=0;var cm=[];this.__Cx=[];var ck=-1;var cl=this.getModel();if(cl!=null){if(!this.isHideRoot()){ck++ ;cm.push(cl);this.__Cx.push(ck);}
;if(this.isNodeOpen(cl)){var cj=this.__CA(cl,ck);cm=cm.concat(cj);}
;}
;if(!qx.lang.Array.equals(this.__ub.toArray(),cm)){this._provider.removeBindings();this.__ub.removeAll();this.__ub.append(cm);this.__xp();this._updateSelection();}
;}
,__CA:function(cn,cs){var cr=[];cs++ ;if(!this.isNode(cn)){return cr;}
...
;if(this.isNode(cu)){this.__Cx.push(cs);cr.push(cu);if(this.isNodeOpen(cu)){var co=this.__CA(cu,cs);cr=cr.concat(co);}
;}
else {if(this.isShowLeafs()){this.__Cx.push(cs);cr.push(cu);}
;}
;}
;cv.dispose();return cr;}
,__CB:function(cw){if(!qx.lang.Array.contains(this.__Cw,cw)){this.__Cw.push(cw);this.fireDataEvent(t,cw);}
;}
,__CC:function(cx,cy){if(cx===cy){this.__CB(cy);return true;}
;if(!this.isNode(cx)){return false;}
;var cB=cx.get(this.getChildProperty());if(cB==null){return false;}
;for(var i=0;i<cB.getLength();i++ ){var cA=cB.getItem(i);var cz=this.__CC(cA,cy);if(cz===true){this.__CB(cA);return true;}
;}
;return false;}
,__xp:function(){this.getPane().getRowConfig().setItemCount(this.__ub.getLength());this.getPane().fullUpdate();}
,getParent:function(cD){var cE=this.__ub.indexOf(cD);if(cE<0){return null;}
;var cF=this.__Cx[cE];while(cE>0){cE-- ;var cC=this.__Cx[cE];if(cC<cF){return this.__ub.getItem(cE);}
;}
;return null;}
,__CD:function(cG){this.__Cy=[];var parent=this.getParent(cG);while(parent!=null){this.__Cy.unshift(parent);parent=this.getParent(parent);}
;}
,__CE:function(){if(this.__Cy==null){return this.getModel();}
;var cH=this.getLookupTable();var parent=this.__Cy.pop();while(parent!=null){if(cH.contains(parent)){return parent;}
...
;}
;if(!qx.core.ObjectRegistry.inShutDown&&this.__hq!=null){this.__hq.cancel();this.__hq.dispose();}
;var cI=this.getModel();if(cI!=null){cI.removeListener(J,this._onChangeBubble,this);}
;this._layer.removeListener(R,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__ub.dispose();this._layer=this._provider=this.__ub=this.__Cw=this.__hq=null;}
});}
)();
...
;}
,bindProperty:function(y,B,z,x,v){var A=this.__xD(v,y);var w=this._tree.getLookupTable();var C=w.bind(A,x,B,z);this.__uB(x,C);}
,bindPropertyReverse:function(J,F,H,I,D){var G=this.__xD(D,J);var E=this._tree.getLookupTable();var K=I.bind(F,E,G,H);this.__uB(I,K);}
...
;}
,_removeBindingsFrom:function(P){var Q=this.__xE(P);while(Q.length>0){var R=Q.pop();try{this._tree.getLookupTable().removeBinding(R);}
...
;return S;}
,__uB:function(W,X){var V=this.__xE(W);if(!qx.lang.Array.contains(V,X)){V.push(X);}
;if(!qx.lang.Array.contains(this.__xC,W)){this.__xC.push(W);}
;}
,__xE:function(Y){var ba=Y.getUserData(a);if(ba==null){ba=[];Y.setUserData(a,ba);}
;return ba;}
...
;var o=this._renderer.getCellWidget();o.setOpen(p&&this._tree.isNodeOpen(s));o.addListener(a,this.__CF,this);o.setUserData(b,this.getChildProperty());o.setUserData(g,this._tree.isShowLeafs());if(this._tree.getSelection().contains(s)){this._styleSelectabled(o);}
...
,poolCellWidget:function(u){u.removeListener(a,this.__CF,this);this._removeBindingsFrom(u);this._renderer.pool(u);this._onPool(u);}
...
,__CF:function(event){var K=event.getTarget();var L=K.getUserData(k);var J=this._tree.getLookupTable().getItem(L);if(event.getData()){this._tree.openNodeWithoutScrolling(J);}
