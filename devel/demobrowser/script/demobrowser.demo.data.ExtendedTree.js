...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ExtendedTree.6ae998cd031e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="The tree has bound its label to the name of the modeln and its ",b="name",c="Write data to log",d="black",e="demobrowser.demo.data.ExtendedTree",f="textColor to some random colors.",g="gray",h="red",j="<b>A tree configures with bindItem</b><br/>",k="all boring black",l="textColor",m="label",n="green",o="children",p="yellow",q="Item ",r="color",s="blue",t="execute";qx.Class.define(e,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var x=[h,s,n,d,g,p];var z=[];for(var i=0;i<50;i++ ){z[i]=new demobrowser.demo.data.model.Node();z[i].setName(q+i);z[i].setColor(x[i%6]);if(i!=0){z[parseInt(Math.random()*i)].getChildren().push(z[i]);}
;}
;var w=new qx.ui.tree.Tree();this.getRoot().add(w,{left:10,top:80});w.setWidth(250);w.setHeight(300);var A=new qx.data.controller.Tree(z[0],w,o);A.setDelegate({bindItem:function(C,B,D){C.bindProperty(b,m,null,B,D);C.bindProperty(r,l,null,B,D);}
});w.getRoot().setOpen(true);var v=new qx.ui.form.Button(k);v.setWidth(120);this.getRoot().add(v,{left:280,top:80});v.addListener(t,function(){for(var i=0;i<z.length;i++ ){z[i].setColor(d);}
;}
,this);var y=new qx.ui.form.Button(c);y.setWidth(120);this.getRoot().add(y,{left:280,top:115});y.addListener(t,function(){this.info(z[0].toString());}
,this);var u=new qx.ui.basic.Label();u.setRich(true);u.setWidth(300);u.setValue(j+a+f);this.getRoot().add(u,{left:10,top:10});}
}});}
)();
