...
,__pendingDefers:[],addPendingDefer:function(bA,bz){if(qx.$$loader&&qx.$$loader.delayDefer){this.__pendingDefers.push(bA);bA.$$pendingDefer=bz;}
else {bz.call(bA);}
;}
...
;}
;return bU;}
;if(!bG){var bF=this.__pendingDefers;this.__pendingDefers=[];bF.forEach(bB);return;}
;bE(bG);}
...
;}
;return ca;}
,__shadowedKeys:[C,m,q,s,L,T,g],keys:({"ES5":Object.keys,"BROKEN_IE":function(ce){if(ce===null||(typeof ce!==U&&typeof ce!==c)){throw new TypeError(f);}
;var cf=[];var ch=Object.prototype.hasOwnProperty;for(var ci in ce){if(ch.call(ce,ci)){cf.push(ci);}
;}
;var cg=qx.Bootstrap.__shadowedKeys;for(var i=0,a=cg,l=a.length;i<l;i++ ){if(ch.call(ce,a[i])){cf.push(a[i]);}
;}
;return cf;}
...
;var cu=Object.prototype.toString.call(cv);return (qx.Bootstrap.__classToTypeMap[cu]||cu.slice(8,-1));}
...
;var argHandler=ca;setter.push(argHandler);if(themeable){styler.push(argHandler);}
;if(config.mode==cO){var shorthand=dj;setter.push(shorthand);if(themeable){styler.push(shorthand);}
;}
;for(var i=0,a=config.group,l=a.length;i<l;i++ ){{}
;setter.push(bU,this.$$method.set[a[i]],bQ,i,dp);resetter.push(bU,this.$$method.reset[a[i]],t);if(themeable){{}
;styler.push(bU,this.$$method.setThemed[a[i]],bQ,i,dp);unstyler.push(bU,this.$$method.resetThemed[a[i]],t);}
;}
...
;if(config.dereference===undefined&&typeof config.check===bb){config.dereference=this.__shouldBeDereferenced(config.check);}
;var method=this.$$method;var store=this.$$store;store.runtime[name]=bz+name;store.user[name]=bD+name;store.theme[name]=cA+name;store.init[name]=cz+name;store.inherit[name]=dc+name;store.useinit[name]=bo+name;method.get[name]=bk+upname;members[method.get[name]]=function(){return qx.core.Property.executeOptimizedGetter(this,clazz,name,bk);}
;members[method.get[name]].$$install=function(dA){qx.core.Property.installOptimizedGetter(this,clazz,name,bk,arguments);}
;method.set[name]=e+upname;members[method.set[name]]=function(dB){return qx.core.Property.executeOptimizedSetter(this,clazz,name,e,arguments);}
;members[method.set[name]].$$install=function(dC){qx.core.Property.installOptimizedSetter(this,clazz,name,e,arguments);}
;method.reset[name]=ba+upname;members[method.reset[name]]=function(){return qx.core.Property.executeOptimizedSetter(this,clazz,name,ba);}
;members[method.reset[name]].$$install=function(){qx.core.Property.installOptimizedSetter(this,clazz,name,ba);}
;if(config.inheritable||config.apply||config.event||config.deferredInit){method.init[name]=cE+upname;members[method.init[name]]=function(dD){return qx.core.Property.executeOptimizedSetter(this,clazz,name,cE,arguments);}
;{}
;}
;if(config.inheritable){method.refresh[name]=db+upname;members[method.refresh[name]]=function(dE){return qx.core.Property.executeOptimizedSetter(this,clazz,name,db,arguments);}
;{}
;}
;method.setRuntime[name]=bS+upname;members[method.setRuntime[name]]=function(dF){return qx.core.Property.executeOptimizedSetter(this,clazz,name,bS,arguments);}
...
;if(config.themeable){method.setThemed[name]=bA+upname;members[method.setThemed[name]]=function(dG){return qx.core.Property.executeOptimizedSetter(this,clazz,name,bA,arguments);}
...
;{}
;}
;if(config.check===bN){members[da+upname]=new Function(be+method.set[name]+cV+method.get[name]+cg);members[dm+upname]=new Function(be+method.get[name]+bY);{}
;}
;{}
;}
,__shouldBeDereferenced:function(dH){return !!this.__dereference[dH];}
,__errors:{'0':dq,'1':bv,'2':B,'3':bW,'4':cK,'5':L},error:function(dI,dO,dN,dJ,dK){var dL=dI.constructor.classname;var dM=q+dN+T+dL+y+this.$$method[dJ][dN]+di+dK+S;throw new Error(dM+(this.__errors[dO]||cD+dO));}
,__unwrapFunctionFromCode:function(dS,dT,name,dR,dP,dQ){var dU=this.__installFunctionFromCode(dS,dT,name,dR,dP,dQ);if(dQ===undefined){return dU.call(dS);}
else {return dU.call(dS,dQ[0]);}
;}
,__installFunctionFromCode:function(instance,members,name,variant,code,args){var store=this.$$method[variant][name];{members[store]=new Function(cM,code.join(cT));}
;if(qx.core.Environment.get(cG)){members[store]=qx.core.Aspect.wrap(instance.classname+cw+store,members[store],s);}
;qx.Bootstrap.setDisplayName(members[store],instance.classname+bL,store);return instance[store];}
,executeOptimizedGetter:function(dY,dX,name,dW){var dV=this.__compileGetter(dY,dX,name,dW);var ea=dX.prototype;return this.__unwrapFunctionFromCode(dY,ea,name,dW,dV);}
,installOptimizedGetter:function(ee,ed,name,ec){var eb=this.__compileGetter(ee,ed,name,ec);var ef=ed.prototype;this.__installFunctionFromCode(ee,ef,name,ec,eb);}
,__compileGetter:function(ej,ei,name,eh){var ek=ei.$$properties[name];var eg=[];var el=this.$$store;eg.push(I,el.runtime[name],bE);eg.push(J,el.runtime[name],f);if(ek.inheritable){eg.push(K,el.inherit[name],bE);eg.push(J,el.inherit[name],f);eg.push(D);}
;eg.push(I,el.user[name],bE);eg.push(J,el.user[name],f);if(ek.themeable){eg.push(K,el.theme[name],bE);eg.push(J,el.theme[name],f);}
;if(ek.deferredInit&&ek.init===undefined){eg.push(K,el.init[name],bE);eg.push(J,el.init[name],f);}
;eg.push(D);if(ek.init!==undefined){if(ek.inheritable){eg.push(m,el.init[name],f);if(ek.nullable){eg.push(bj);}
;eg.push(k);}
else {eg.push(J,el.init[name],f);}
;}
else if(ek.inheritable||ek.nullable){eg.push(cj);}
else {eg.push(cU,name,cY,ei.classname,dn);}
;return eg;}
,executeOptimizedSetter:function(eq,ep,name,eo,en){var em=this.__compileSetter(eq,ep,name,eo,en);var er=ep.prototype;return this.__unwrapFunctionFromCode(eq,er,name,eo,em,en);}
,installOptimizedSetter:function(ew,ev,name,eu,et){var es=this.__compileSetter(ew,ev,name,eu,et);var ex=ev.prototype;return this.__installFunctionFromCode(ew,ex,name,eu,es,et);}
,__compileSetter:function(eE,eD,name,eC,eB){var eG=eD.$$properties[name];var eF=eD.prototype;var ez=[];var ey=eC===e||eC===bA||eC===bS||(eC===cE&&eG.init===undefined);var eA=eG.apply||eG.event||eG.inheritable;var eH=this.__getStore(eC,name);this.__emitIsEqualFunction(ez,eD,eG,name);this.__emitSetterPreConditions(ez,eG,name,eC,ey);if(ey||eA){this.__emitOldValue(ez,eG,name);}
;if(ey){this.__emitIncomingValueTransformation(ez,eD,eG,name);}
;if(eA){this.__emitOldNewComparison(ez,ey,eH,eC);}
;if(eG.inheritable){ez.push(bm);}
;{}
;if(!eA){this.__emitStoreValue(ez,name,eC,ey);}
else {this.__emitStoreComputedValue(ez,eG,name,eC,ey);}
;if(eG.inheritable){this.__emitStoreInheritedPropertyValue(ez,eG,name,eC);}
else if(eA){this.__emitNormalizeUndefinedValues(ez,eG,name,eC);}
;if(eA){this.__emitCallCallback(ez,eG,name,eC);if(eG.inheritable&&eF._getChildren){this.__emitRefreshChildrenValue(ez,name);}
;}
;if(ey){ez.push(bT);}
;return ez;}
,__getStore:function(eI,name){if(eI===bS||eI===R){var eJ=this.$$store.runtime[name];}
else if(eI===bA||eI===g){eJ=this.$$store.theme[name];}
else if(eI===cE){eJ=this.$$store.init[name];}
else {eJ=this.$$store.user[name];}
;return eJ;}
,__emitIsEqualFunction:function(eK,eN,eL,name){eK.push(bC);if(typeof eL.isEqual===bM){eK.push(Y,eN.classname,cb,name,bO);}
else if(typeof eL.isEqual===bb){var eM=eN.prototype;if(eM[eL.isEqual]!==undefined){eK.push(bu,eL.isEqual,f);}
else {eK.push(X,eL.isEqual,bt);}
;}
else if(typeof eL.isEqual===df){eK.push(ci);}
else {throw new Error(de+name+br+eN.classname+cX);}
;}
,__emitSetterPreConditions:function(eQ,eO,name,eR,eP){{if(!eO.nullable||eO.check||eO.inheritable){eQ.push(dd);}
;if(eR===e){eQ.push(M,name,bi,eR,bw);}
;}
;}
,__emitIncomingValueTransformation:function(eS,eU,eT,name){if(eT.transform){eS.push(bh,eT.transform,n);}
;if(eT.validate){if(typeof eT.validate===bb){eS.push(bu,eT.validate,bq);}
else if(eT.validate instanceof Function){eS.push(eU.classname,cb,name);eS.push(u);}
;}
;}
,__emitOldNewComparison:function(eW,eV,eY,eX){var fa=(eX===ba||eX===g||eX===R);if(eV){eW.push(cd);}
else if(fa){eW.push(I,eY,V);}
;}
,__emitIncomingValueValidation:undefined,__emitStoreValue:function(fc,name,fd,fb){if(fd===bS){fc.push(bu,this.$$store.runtime[name],ch);}
else if(fd===R){fc.push(I,this.$$store.runtime[name],bE);fc.push(Q,this.$$store.runtime[name],f);}
else if(fd===e){fc.push(bu,this.$$store.user[name],ch);}
else if(fd===ba){fc.push(I,this.$$store.user[name],bE);fc.push(Q,this.$$store.user[name],f);}
else if(fd===bA){fc.push(bu,this.$$store.theme[name],ch);}
else if(fd===g){fc.push(I,this.$$store.theme[name],bE);fc.push(Q,this.$$store.theme[name],f);}
else if(fd===cE&&fb){fc.push(bu,this.$$store.init[name],ch);}
;}
,__emitStoreComputedValue:function(fg,fe,name,fh,ff){fg.push(cN);fg.push(I,this.$$store.runtime[name],cB);if(fh===bS){fg.push(cP,this.$$store.runtime[name],ch);}
else if(fh===R){fg.push(Q,this.$$store.runtime[name],f);fg.push(I,this.$$store.user[name],bE);fg.push(cP,this.$$store.user[name],f);fg.push(K,this.$$store.theme[name],bE);fg.push(cP,this.$$store.theme[name],f);fg.push(K,this.$$store.init[name],cB);fg.push(cP,this.$$store.init[name],f);fg.push(bu,this.$$store.useinit[name],E);fg.push(cx);}
else {fg.push(cP,this.$$store.runtime[name],f);if(fh===e){fg.push(bu,this.$$store.user[name],ch);}
else if(fh===ba){fg.push(Q,this.$$store.user[name],f);}
else if(fh===bA){fg.push(bu,this.$$store.theme[name],ch);}
else if(fh===g){fg.push(Q,this.$$store.theme[name],f);}
else if(fh===cE&&ff){fg.push(bu,this.$$store.init[name],ch);}
;}
;fg.push(cx);fg.push(K,this.$$store.user[name],cB);if(fh===e){fg.push(cP,this.$$store.user[name],ch);}
else if(fh===ba){fg.push(Q,this.$$store.user[name],f);fg.push(I,this.$$store.runtime[name],bE);fg.push(cP,this.$$store.runtime[name],f);fg.push(I,this.$$store.theme[name],bE);fg.push(cP,this.$$store.theme[name],f);fg.push(K,this.$$store.init[name],cB);fg.push(cP,this.$$store.init[name],f);fg.push(bu,this.$$store.useinit[name],E);fg.push(cx);}
else {if(fh===bS){fg.push(cP,this.$$store.runtime[name],ch);}
else if(fe.inheritable){fg.push(cP,this.$$store.user[name],f);}
else {fg.push(cP,this.$$store.user[name],f);}
;if(fh===bA){fg.push(bu,this.$$store.theme[name],ch);}
else if(fh===g){fg.push(Q,this.$$store.theme[name],f);}
else if(fh===cE&&ff){fg.push(bu,this.$$store.init[name],ch);}
;}
;fg.push(cx);if(fe.themeable){fg.push(K,this.$$store.theme[name],cB);if(fh===bS){fg.push(cP,this.$$store.runtime[name],ch);}
else if(fh===e){fg.push(cP,this.$$store.user[name],ch);}
else if(fh===bA){fg.push(cP,this.$$store.theme[name],ch);}
else if(fh===g){fg.push(Q,this.$$store.theme[name],f);fg.push(I,this.$$store.init[name],cB);fg.push(cP,this.$$store.init[name],f);fg.push(bu,this.$$store.useinit[name],E);fg.push(cx);}
else if(fh===cE){if(ff){fg.push(bu,this.$$store.init[name],ch);}
;fg.push(cP,this.$$store.theme[name],f);}
else if(fh===db){fg.push(cP,this.$$store.theme[name],f);}
;fg.push(cx);}
;fg.push(K,this.$$store.useinit[name],bF);if(fh===cE){if(ff){fg.push(cP,this.$$store.init[name],ch);}
else {fg.push(cP,this.$$store.init[name],f);}
;}
else if(fh===e||fh===bS||fh===bA||fh===db){fg.push(Q,this.$$store.useinit[name],f);if(fh===bS){fg.push(cP,this.$$store.runtime[name],ch);}
else if(fh===e){fg.push(cP,this.$$store.user[name],ch);}
else if(fh===bA){fg.push(cP,this.$$store.theme[name],ch);}
else if(fh===db){fg.push(cP,this.$$store.init[name],f);}
;}
;fg.push(cx);if(fh===e||fh===bS||fh===bA||fh===cE){fg.push(w);if(fh===bS){fg.push(cP,this.$$store.runtime[name],ch);}
else if(fh===e){fg.push(cP,this.$$store.user[name],ch);}
else if(fh===bA){fg.push(cP,this.$$store.theme[name],ch);}
else if(fh===cE){if(ff){fg.push(cP,this.$$store.init[name],ch);}
else {fg.push(cP,this.$$store.init[name],f);}
;fg.push(bu,this.$$store.useinit[name],E);}
;fg.push(cx);}
;}
,__emitOldValue:function(fi,fj,name){if(fj.inheritable){fi.push(cm,this.$$store.inherit[name],f);}
else {fi.push(r);}
;fi.push(I,this.$$store.runtime[name],cB);fi.push(ce,this.$$store.runtime[name],f);fi.push(cx);if(!fj.inheritable){fi.push(K,this.$$store.user[name],cB);fi.push(ce,this.$$store.user[name],f);fi.push(cx);if(fj.themeable){fi.push(K,this.$$store.theme[name],cB);fi.push(ce,this.$$store.theme[name],f);fi.push(cx);}
;fi.push(K,this.$$store.useinit[name],bF);fi.push(ce,this.$$store.init[name],f);fi.push(cx);}
;}
,__emitStoreInheritedPropertyValue:function(fl,fk,name,fm){fl.push(bH);if(fm===db){fl.push(bx);}
else {fl.push(cp,this.$$store.inherit[name],f);}
;fl.push(cI);fl.push(bu,this.$$store.init[name],ct);fl.push(bu,this.$$store.init[name],b);fl.push(cP,this.$$store.init[name],f);fl.push(bu,this.$$store.useinit[name],E);fl.push(by);fl.push(Q,this.$$store.useinit[name],bB);fl.push(cx);fl.push(dh);fl.push(G);fl.push(bp,this.$$store.inherit[name],f);fl.push(cx);fl.push(P);fl.push(Q,this.$$store.inherit[name],f);fl.push(bg,this.$$store.inherit[name],A);fl.push(cv);if(fk.init!==undefined&&fm!==cE){fl.push(F,this.$$store.init[name],bI);}
else {fl.push(co);}
;fl.push(dk);}
,__emitNormalizeUndefinedValues:function(fp,fo,name,fq){if(fq!==e&&fq!==bS&&fq!==bA){fp.push(cC);}
;fp.push(dh);if(fo.init!==undefined&&fq!==cE){fp.push(F,this.$$store.init[name],bI);}
else {fp.push(co);}
;}
,__emitCallCallback:function(fs,fr,name,ft){if(fr.apply){fs.push(bu,fr.apply,bR,name,N,ft,cl);}
;if(fr.event){fs.push(cf,bV,fr.event,bc,cJ,fr.event,cu,bP);}
;}
,__emitRefreshChildrenValue:function(fu,name){fu.push(bJ);fu.push(O,this.$$method.refresh[name],bX,this.$$method.refresh[name],C);fu.push(cx);}
}});}
)();
(function(){var b=".prototype",c="$$init_",d="qx.event.type.Data",e="constructor",f="Property module disabled.",g="extend",h="module.property",j="singleton",k='undefined',m="module.events",n="qx.aspects",o="@construct",p='extend',q="@destruct",r="Array",s="members",t="static",u='@',v="properties",w="statics",x="",y="Events module not enabled.",z="toString",A="]",B="@",C="qx.Class",D='"extend" parameter is null or undefined',E="[Class ",F="destructor",G="Class",H="destruct",I=".",J="member";qx.Bootstrap.define(C,{statics:{__Property:qx.core.Environment.get(h)?qx.core.Property:null,define:function(name,N){if(!N){N={};}
;if(N.include&&!(qx.Bootstrap.getClass(N.include)===r)){N.include=[N.include];}
;if(N.implement&&!(qx.Bootstrap.getClass(N.implement)===r)){N.implement=[N.implement];}
;var K=false;if(!N.hasOwnProperty(g)&&!N.type){N.type=t;K=true;}
;{}
;var L=this.__createClass(name,N.type,N.extend,N.statics,N.construct,N.destruct,N.include);[B,o,q].forEach(function(O){this.__attachAnno(L,O,null,N[O]);}
,this);if(N.extend){if(N.properties){this.__addProperties(L,N.properties,true);}
;if(N.members){this.__addMembers(L,N.members,true,true,false);}
;if(N.events){this.__addEvents(L,N.events,true);}
;if(N.include){for(var i=0,l=N.include.length;i<l;i++ ){this.__addMixin(L,N.include[i],false);}
;}
;}
else if(N.hasOwnProperty(p)&&false){throw new Error(D);}
;if(N.environment){for(var M in N.environment){qx.core.Environment.add(M,N.environment[M]);}
;}
;if(N.implement){for(var i=0,l=N.implement.length;i<l;i++ ){this.__addInterface(L,N.implement[i]);}
;}
;{}
;if(N.defer){N.defer.self=L;qx.Bootstrap.addPendingDefer(L,function(){N.defer(L,L.prototype,{add:function(name,P){var Q={};Q[name]=P;qx.Class.__addProperties(L,Q,true);}
});}
);}
;return L;}
,undefine:function(name){delete this.$$registry[name];var T=name.split(I);var S=[window];for(var i=0;i<T.length;i++ ){S.push(S[i][T[i]]);}
;for(var i=S.length-1;i>=1;i-- ){var R=S[i];var parent=S[i-1];if(qx.Bootstrap.isFunction(R)||qx.Bootstrap.objectGetLength(R)===0){delete parent[T[i-1]];}
else {break;}
;}
;}
,isDefined:qx.util.OOUtil.classIsDefined,getTotalNumber:function(){return qx.Bootstrap.objectGetLength(this.$$registry);}
,getByName:qx.Bootstrap.getByName,include:function(V,U){{}
;qx.Class.__addMixin(V,U,false);}
,patch:function(X,W){{}
;qx.Class.__addMixin(X,W,true);}
,isSubClassOf:function(ba,Y){if(!ba){return false;}
;if(ba==Y){return true;}
;if(ba.prototype instanceof Y){return true;}
;return false;}
,getPropertyDefinition:qx.util.OOUtil.getPropertyDefinition,getProperties:function(bc){var bb=[];while(bc){if(bc.$$properties){bb.push.apply(bb,Object.keys(bc.$$properties));}
;bc=bc.superclass;}
;return bb;}
,getByProperty:function(bd,name){while(bd){if(bd.$$properties&&bd.$$properties[name]){return bd;}
;bd=bd.superclass;}
;return null;}
,hasProperty:qx.util.OOUtil.hasProperty,getEventType:qx.util.OOUtil.getEventType,supportsEvent:qx.util.OOUtil.supportsEvent,hasOwnMixin:function(bf,be){return bf.$$includes&&bf.$$includes.indexOf(be)!==-1;}
,getByMixin:function(bi,bh){var bg,i,l;while(bi){if(bi.$$includes){bg=bi.$$flatIncludes;for(i=0,l=bg.length;i<l;i++ ){if(bg[i]===bh){return bi;}
;}
;}
;bi=bi.superclass;}
;return null;}
,getMixins:qx.util.OOUtil.getMixins,hasMixin:function(bk,bj){return !!this.getByMixin(bk,bj);}
,hasOwnInterface:function(bm,bl){return bm.$$implements&&bm.$$implements.indexOf(bl)!==-1;}
,getByInterface:qx.util.OOUtil.getByInterface,getInterfaces:function(bo){var bn=[];while(bo){if(bo.$$implements){bn.push.apply(bn,bo.$$flatImplements);}
;bo=bo.superclass;}
;return bn;}
,hasInterface:qx.util.OOUtil.hasInterface,implementsInterface:function(bq,bp){var br=bq.constructor;if(this.hasInterface(br,bp)){return true;}
;if(qx.Interface.objectImplements(bq,bp)){return true;}
;if(qx.Interface.classImplements(br,bp)){return true;}
;return false;}
,getInstance:function(){if(!this.$$instance){this.$$allowconstruct=true;this.$$instance=new this();delete this.$$allowconstruct;}
;return this.$$instance;}
,getSubclasses:function(bs){if(!bs){return null;}
;var bt={};var bu=qx.Class.$$registry;for(var name in bu){if(bu[name].superclass&&bu[name].superclass==bs){bt[name]=bu[name];}
;}
;return bt;}
,genericToString:function(){return E+this.classname+A;}
,$$registry:qx.Bootstrap.$$registry,__allowedKeys:null,__staticAllowedKeys:null,__validateConfig:function(name,bv){}
,__validateAbstractInterfaces:function(bw){}
,__attachAnno:function(bA,by,bz,bx){if(bx!==undefined){if(bA.$$annotations===undefined){bA.$$annotations={};bA.$$annotations[by]={};}
else if(bA.$$annotations[by]===undefined){bA.$$annotations[by]={};}
;if(!qx.lang.Type.isArray(bx)){bx=[bx];}
;if(bz){bA.$$annotations[by][bz]=bx;}
else {bA.$$annotations[by]=bx;}
;}
;}
,__createClass:function(name,bL,bK,bB,bI,bF,bE){var bH=function(){return (typeof this==k);}
;var bG;if(!bK&&qx.core.Environment.get(n)==false){bG=bB||{};qx.Bootstrap.setDisplayNames(bG,name);}
else {bG={};if(bK){if(!bI){bI=this.__createDefaultConstructor();}
;if(this.__needsConstructorWrapper(bK,bE)){bG=this.__wrapConstructor(bI,name,bL);}
else {bG=bI;}
;if(bL===j){bG.getInstance=this.getInstance;}
;qx.Bootstrap.setDisplayName(bI,name,e);}
;if(bB){qx.Bootstrap.setDisplayNames(bB,name);var bJ;for(var i=0,a=Object.keys(bB),l=a.length;i<l;i++ ){bJ=a[i];var bC=bB[bJ];{}
;if(bJ.charAt(0)===u){continue;}
;if(qx.core.Environment.get(n)){if(bC instanceof Function){bC=qx.core.Aspect.wrap(name+I+bJ,bC,t);}
;bG[bJ]=bC;}
else {bG[bJ]=bC;}
;this.__attachAnno(bG,w,bJ,bB[B+bJ]);}
;}
;}
;var bD=name?qx.Bootstrap.createNamespace(name,bG):x;bG.classname=name;if(!bH()){try{bG.name=name;}
catch(bM){}
;}
;bG.basename=bD;bG.$$type=G;if(bL){bG.$$classtype=bL;}
;if(!bG.hasOwnProperty(z)){bG.toString=this.genericToString;}
;if(bK){qx.Bootstrap.extendClass(bG,bI,bK,name,bD);if(bF){if(qx.core.Environment.get(n)){bF=qx.core.Aspect.wrap(name,bF,F);}
;bG.$$destructor=bF;qx.Bootstrap.setDisplayName(bF,name,H);}
;}
;this.$$registry[name]=bG;return bG;}
,__addEvents:function(bN,bO,bQ){{var bP,bP;}
;if(bN.$$events){for(var bP in bO){bN.$$events[bP]=bO[bP];}
;}
else {bN.$$events=bO;}
;}
,__addProperties:function(bS,bV,bT){if(!qx.core.Environment.get(h)){throw new Error(f);}
;var bU;if(bT===undefined){bT=false;}
;var bR=bS.prototype;for(var name in bV){bU=bV[name];{}
;bU.name=name;if(!bU.refine){if(bS.$$properties===undefined){bS.$$properties={};}
;bS.$$properties[name]=bU;}
;if(bU.init!==undefined){bS.prototype[c+name]=bU.init;}
;if(bU.event!==undefined){if(!qx.core.Environment.get(m)){throw new Error(y);}
;var event={};event[bU.event]=d;this.__addEvents(bS,event,bT);}
;if(bU.inheritable){this.__Property.$$inheritable[name]=true;if(!bR.$$refreshInheritables){this.__Property.attachRefreshInheritables(bS);}
;}
;if(!bU.refine){this.__Property.attachMethods(bS,name,bU);}
;this.__attachAnno(bS,v,name,bU[B]);}
;}
,__validateProperty:null,__addMembers:function(ce,bW,bY,cb,cd){var bX=ce.prototype;var cc,ca;qx.Bootstrap.setDisplayNames(bW,ce.classname+b);for(var i=0,a=Object.keys(bW),l=a.length;i<l;i++ ){cc=a[i];ca=bW[cc];{var cf;}
;if(cc.charAt(0)===u){var cf=cc.substring(1);if(bW[cf]===undefined){this.__attachAnno(ce,s,cf,bW[cc]);}
;continue;}
;if(bX[cc]!=undefined&&bX[cc].$$install){bX[cc].$$install();}
;if(cb!==false&&ca instanceof Function&&ca.$$type==null){if(cd==true){ca=this.__mixinMemberWrapper(ca,bX[cc]);}
else {if(bX[cc]){ca.base=bX[cc];}
;ca.self=ce;}
;if(qx.core.Environment.get(n)){ca=qx.core.Aspect.wrap(ce.classname+I+cc,ca,J);}
;}
;bX[cc]=ca;this.__attachAnno(ce,s,cc,bW[B+cc]);}
;}
,__mixinMemberWrapper:function(cg,ch){if(ch){return function(){var cj=cg.base;cg.base=ch;var ci=cg.apply(this,arguments);cg.base=cj;return ci;}
;}
else {return cg;}
;}
,__addInterface:function(cm,ck){{}
;var cl=qx.Interface.flatten([ck]);if(cm.$$implements){cm.$$implements.push(ck);cm.$$flatImplements.push.apply(cm.$$flatImplements,cl);}
else {cm.$$implements=[ck];cm.$$flatImplements=cl;}
;}
,__retrospectWrapConstruct:function(co){var name=co.classname;var cn=this.__wrapConstructor(co,name,co.$$classtype);for(var i=0,a=Object.keys(co),l=a.length;i<l;i++ ){cp=a[i];cn[cp]=co[cp];}
;cn.prototype=co.prototype;var cr=co.prototype;for(var i=0,a=Object.keys(cr),l=a.length;i<l;i++ ){cp=a[i];var cs=cr[cp];if(cs&&cs.self==co){cs.self=cn;}
;}
;for(var cp in this.$$registry){var cq=this.$$registry[cp];if(!cq){continue;}
;if(cq.base==co){cq.base=cn;}
;if(cq.superclass==co){cq.superclass=cn;}
;if(cq.$$original){if(cq.$$original.base==co){cq.$$original.base=cn;}
;if(cq.$$original.superclass==co){cq.$$original.superclass=cn;}
;}
;}
;qx.Bootstrap.createNamespace(name,cn);this.$$registry[name]=cn;return cn;}
,__addMixin:function(cy,cw,cv){{}
;if(this.hasMixin(cy,cw)){return;}
;var ct=cy.$$original;if(cw.$$constructor&&!ct){cy=this.__retrospectWrapConstruct(cy);}
;var cu=qx.Mixin.flatten([cw]);var cx;for(var i=0,l=cu.length;i<l;i++ ){cx=cu[i];if(cx.$$events){this.__addEvents(cy,cx.$$events,cv);}
;if(cx.$$properties){this.__addProperties(cy,cx.$$properties,cv);}
;if(cx.$$members){this.__addMembers(cy,cx.$$members,cv,cv,cv);}
;}
;if(cy.$$includes){cy.$$includes.push(cw);cy.$$flatIncludes.push.apply(cy.$$flatIncludes,cu);}
else {cy.$$includes=[cw];cy.$$flatIncludes=cu;}
;}
,__createDefaultConstructor:function(){function cz(){cz.base.apply(this,arguments);}
;return cz;}
,__needsConstructorWrapper:function(cB,cA){{}
;if(cB&&cB.$$includes){var cC=cB.$$flatIncludes;for(var i=0,l=cC.length;i<l;i++ ){if(cC[i].$$constructor){return true;}
;}
;}
;if(cA){var cD=qx.Mixin.flatten(cA);for(var i=0,l=cD.length;i<l;i++ ){if(cD[i].$$constructor){return true;}
;}
;}
;return false;}
,__wrapConstructor:function(cF,name,cE){var cH=function(){var cK=cH;{}
;var cI=cK.$$original.apply(this,arguments);if(cK.$$includes){var cJ=cK.$$flatIncludes;for(var i=0,l=cJ.length;i<l;i++ ){if(cJ[i].$$constructor){cJ[i].$$constructor.apply(this,arguments);}
;}
;}
;{}
;return cI;}
;if(qx.core.Environment.get(n)){var cG=qx.core.Aspect.wrap(name,cH,e);cH.$$original=cF;cH.constructor=cG;cH=cG;}
;cH.$$original=cF;cF.wrapper=cH;return cH;}
},defer:function(){if(qx.core.Environment.get(n)){for(var cL in qx.Bootstrap.$$registry){var cM=qx.Bootstrap.$$registry[cL];for(var cN in cM){if(cM[cN] instanceof Function){cM[cN]=qx.core.Aspect.wrap(cL+I+cN,cM[cN],t);}
;}
;}
;}
;}
...
(function(){var a=". Error message: ",b="Boolean",c=").",d="set",f="deepBinding",g=") to the object '",h="item",k="Please use only one array at a time: ",l="Integer",m="reset",n=" of object ",p="qx.data.SingleValueBinding",q="Binding property ",r="Failed so set value ",s="change",t='No number or \'last\' value has been given in an array binding: ',u="Binding could not be found!",v="get",w="^",x=" does not work.",y="String",z="Binding from '",A="",B="PositiveNumber",C="]",D="[",E=".",F="PositiveInteger",G="' (",H=" on ",I="Binding does not exist!",J="Number",K=".[",L="Date",M=" not possible: No event available. ",N="last";qx.Class.define(p,{statics:{__bindings:{},__bindingsByTarget:{},bind:function(bb,U,bg,bd,bj){{}
;var bf=this.__setUpTargetBinding(bb,U,bg,bd,bj);var Q=U.split(E);var P=this.__checkForArrayInPropertyChain(Q);var S=[];var bh=[];var R=[];var T=[];var Y=bb;try{for(var i=0;i<Q.length;i++ ){if(P[i]!==A){T.push(s);}
else {var X=this.__getEventNameForProperty(Y,Q[i]);if(!X){if(i==0){throw new qx.core.AssertionError(q+Q[i]+n+Y+M);}
;this.__setInitialValue(undefined,bg,bd,bj,bb);break;}
;T.push(X);}
;S[i]=Y;if(i==Q.length-1){if(P[i]!==A){var ba=P[i]===N?Y.length-1:P[i];var O=Y.getItem(ba);this.__setInitialValue(O,bg,bd,bj,bb);R[i]=this.__bindEventToProperty(Y,T[i],bg,bd,bj,P[i]);}
else {if(Q[i]!=null&&Y[v+qx.lang.String.firstUp(Q[i])]!=null){var O=Y[v+qx.lang.String.firstUp(Q[i])]();this.__setInitialValue(O,bg,bd,bj,bb);}
;R[i]=this.__bindEventToProperty(Y,T[i],bg,bd,bj);}
;}
else {var bi={index:i,propertyNames:Q,sources:S,listenerIds:R,arrayIndexValues:P,targetObject:bg,targetPropertyChain:bd,options:bj,listeners:bh};var be=qx.lang.Function.bind(this.__chainListener,this,bi);bh.push(be);R[i]=Y.addListener(T[i],be);}
...
;}
;if(!Y){this.__setInitialValue(Y,bg,bd,bj,bb);break;}
;}
;}
...
;}
;throw bk;}
;var W={type:f,listenerIds:R,sources:S,targetListenerIds:bf.listenerIds,targets:bf.targets};this.__storeBinding(W,bb,U,bg,bd);return W;}
,__chainListener:function(br){if(br.options&&br.options.onUpdate){br.options.onUpdate(br.sources[br.index],br.targetObject);}
...
;var bp=br.sources[br.index];for(var j=br.index+1;j<br.propertyNames.length;j++ ){if(br.arrayIndexValues[j-1]!==A){bp=bp[v+qx.lang.String.firstUp(br.propertyNames[j-1])](br.arrayIndexValues[j-1]);}
else {bp=bp[v+qx.lang.String.firstUp(br.propertyNames[j-1])]();}
;br.sources[j]=bp;if(!bp){if(br.options&&br.options.converter){var bl=false;if(br.options.ignoreConverter){var bs=br.propertyNames.slice(0,j).join(E);var bq=bs.match(new RegExp(w+br.options.ignoreConverter));bl=bq?bq.length>0:false;}
;if(!bl){this.__setTargetValue(br.targetObject,br.targetPropertyChain,br.options.converter());}
else {this.__resetTargetValue(br.targetObject,br.targetPropertyChain);}
;}
else {this.__resetTargetValue(br.targetObject,br.targetPropertyChain);}
;break;}
;if(j==br.propertyNames.length-1){if(qx.Class.implementsInterface(bp,qx.data.IListData)){var bt=br.arrayIndexValues[j]===N?bp.length-1:br.arrayIndexValues[j];var bm=bp.getItem(bt);this.__setInitialValue(bm,br.targetObject,br.targetPropertyChain,br.options,br.sources[br.index]);br.listenerIds[j]=this.__bindEventToProperty(bp,s,br.targetObject,br.targetPropertyChain,br.options,br.arrayIndexValues[j]);}
else {if(br.propertyNames[j]!=null&&bp[v+qx.lang.String.firstUp(br.propertyNames[j])]!=null){var bm=bp[v+qx.lang.String.firstUp(br.propertyNames[j])]();this.__setInitialValue(bm,br.targetObject,br.targetPropertyChain,br.options,br.sources[br.index]);}
;var bn=this.__getEventNameForProperty(bp,br.propertyNames[j]);if(!bn){this.__resetTargetValue(br.targetObject,br.targetPropertyChain);break;}
;br.listenerIds[j]=this.__bindEventToProperty(bp,bn,br.targetObject,br.targetPropertyChain,br.options);}
;}
else {if(br.listeners[j]==null){var bo=qx.lang.Function.bind(this.__chainListener,this,br);br.listeners.push(bo);}
;if(qx.Class.implementsInterface(bp,qx.data.IListData)){var bn=s;}
else {var bn=this.__getEventNameForProperty(bp,br.propertyNames[j]);}
;if(!bn){this.__resetTargetValue(br.targetObject,br.targetPropertyChain);return;}
;br.listenerIds[j]=bp.addListener(bn,br.listeners[j]);}
;}
;}
,__setUpTargetBinding:function(bw,bE,bI,bA,bC){var bz=bA.split(E);var bx=this.__checkForArrayInPropertyChain(bz);var bH=[];var bG=[];var bB=[];var bF=[];var by=bI;for(var i=0;i<bz.length-1;i++ ){if(bx[i]!==A){bF.push(s);}
else {var bv=this.__getEventNameForProperty(by,bz[i]);if(!bv){break;}
;bF.push(bv);}
;bH[i]=by;var bD=function(){for(var j=i+1;j<bz.length-1;j++ ){var bL=bH[j];bH[j]=null;if(!bL){continue;}
;bL.removeListenerById(bB[j]);}
;var bL=bH[i];for(var j=i+1;j<bz.length-1;j++ ){var bJ=qx.lang.String.firstUp(bz[j-1]);if(bx[j-1]!==A){var bM=bx[j-1]===N?bL.getLength()-1:bx[j-1];bL=bL[v+bJ](bM);}
else {bL=bL[v+bJ]();}
;bH[j]=bL;if(bG[j]==null){bG.push(bD);}
;if(qx.Class.implementsInterface(bL,qx.data.IListData)){var bK=s;}
else {var bK=qx.data.SingleValueBinding.__getEventNameForProperty(bL,bz[j]);if(!bK){break;}
;}
;bB[j]=bL.addListener(bK,bG[j]);}
;qx.data.SingleValueBinding.updateTarget(bw,bE,bI,bA,bC);}
;bG.push(bD);bB[i]=by.addListener(bF[i],bD);var bu=qx.lang.String.firstUp(bz[i]);if(by[v+bu]==null){by=null;}
else if(bx[i]!==A){by=by[v+bu](bx[i]);}
else {by=by[v+bu]();}
;if(!by){break;}
;}
;return {listenerIds:bB,targets:bH};}
,updateTarget:function(bN,bQ,bS,bO,bR){var bP=this.resolvePropertyChain(bN,bQ);bP=qx.data.SingleValueBinding.__convertValue(bP,bS,bO,bR,bN);this.__setTargetValue(bS,bO,bP);}
,resolvePropertyChain:function(o,bT){var bU=this.__getPropertyChainArray(bT);return this.__getTargetFromChain(o,bU,bU.length);}
,__getEventNameForProperty:function(bW,bX){var bV=this.__getEventForProperty(bW,bX);if(bV==null){if(qx.Class.supportsEvent(bW.constructor,bX)){bV=bX;}
else if(qx.Class.supportsEvent(bW.constructor,s+qx.lang.String.firstUp(bX))){bV=s+qx.lang.String.firstUp(bX);}
else {return null;}
;}
;return bV;}
,__resetTargetValue:function(cd,cb){var cc=this.__getPropertyChainArray(cb);var ca=this.__getTargetFromChain(cd,cc);if(ca!=null){var ce=cc[cc.length-1];var bY=this.__getArrayIndex(ce);if(bY){this.__setTargetValue(cd,cb,null);return;}
;if(ca[m+qx.lang.String.firstUp(ce)]!=undefined){ca[m+qx.lang.String.firstUp(ce)]();}
else {ca[d+qx.lang.String.firstUp(ce)](null);}
;}
;}
,__setTargetValue:function(ck,ch,ci){var cj=this.__getPropertyChainArray(ch);var cg=this.__getTargetFromChain(ck,cj);if(cg){var cl=cj[cj.length-1];var cf=this.__getArrayIndex(cl);if(cf){if(cf===N){cf=cg.length-1;}
;cg.setItem(cf,ci);}
else {cg[d+qx.lang.String.firstUp(cl)](ci);}
;}
;}
,__getArrayIndex:function(co){var cm=/^\[(\d+|last)\]$/;var cn=co.match(cm);if(cn){return cn[1];}
;return null;}
,__getPropertyChainArray:function(cp){return cp.replace(/\[/g,K).split(E).filter(function(cq){return cq!==A;}
);}
,__getTargetFromChain:function(cw,cr,cs){cs=cs||cr.length-1;var cu=cw;for(var i=0;cu!==null&&i<cs;i++ ){try{var cv=cr[i];var ct=this.__getArrayIndex(cv);if(ct){if(ct===N){ct=cu.length-1;}
;cu=cu.getItem(ct);}
else {cu=cu[v+qx.lang.String.firstUp(cv)]();}
;}
catch(cx){return null;}
;}
;return cu;}
,__setInitialValue:function(cC,cy,cA,cB,cz){cC=this.__convertValue(cC,cy,cA,cB,cz);if(cC===undefined){this.__resetTargetValue(cy,cA);}
;if(cC!==undefined){try{this.__setTargetValue(cy,cA,cC);if(cB&&cB.onUpdate){cB.onUpdate(cz,cy,cC);}
;}
...
;}
;}
,__checkForArrayInPropertyChain:function(cD){var cE=[];for(var i=0;i<cD.length;i++ ){var name=cD[i];if(name.endsWith(C)){var cF=name.substring(name.indexOf(D)+1,name.indexOf(C));if(name.indexOf(C)!=name.length-1){throw new Error(k+name+x);}
;if(cF!==N){if(cF==A||isNaN(parseInt(cF,10))){throw new Error(t+name+x);}
;}
;if(name.indexOf(D)!=0){cD[i]=name.substring(0,name.indexOf(D));cE[i]=A;cE[i+1]=cF;cD.splice(i+1,0,h);i++ ;}
else {cE[i]=cF;cD.splice(i,1,h);}
;}
else {cE[i]=A;}
;}
;return cE;}
,__bindEventToProperty:function(cG,cJ,cO,cM,cK,cI){{var cH;}
;var cL=function(cR,e){if(cR!==A){if(cR===N){cR=cG.length-1;}
;var cS=cG.getItem(cR);if(cS===undefined){qx.data.SingleValueBinding.__resetTargetValue(cO,cM);}
;var cQ=e.getData().start;var cP=e.getData().end;if(cR<cQ||cR>cP){return;}
;}
else {var cS=e.getData();}
;{}
;cS=qx.data.SingleValueBinding.__convertValue(cS,cO,cM,cK,cG);{}
;try{if(cS!==undefined){qx.data.SingleValueBinding.__setTargetValue(cO,cM,cS);}
else {qx.data.SingleValueBinding.__resetTargetValue(cO,cM);}
...
,__storeBinding:function(da,cU,cX,db,cY){var cV;cV=cU.toHashCode();if(this.__bindings[cV]===undefined){this.__bindings[cV]=[];}
;var cW=[da,cU,cX,db,cY];this.__bindings[cV].push(cW);cV=db.toHashCode();if(this.__bindingsByTarget[cV]===undefined){this.__bindingsByTarget[cV]=[];}
;this.__bindingsByTarget[cV].push(cW);}
,__convertValue:function(df,dl,de,dh,dc){if(dh&&dh.converter){var di;if(dl.getModel){di=dl.getModel();}
;return dh.converter(df,di,dc,dl);}
else {var dg=this.__getPropertyChainArray(de);var dd=this.__getTargetFromChain(dl,dg);var dm=de.substring(de.lastIndexOf(E)+1,de.length);if(dd==null){return df;}
;var dj=qx.Class.getPropertyDefinition(dd.constructor,dm);var dk=dj==null?A:dj.check;return this.__defaultConversion(df,dk);}
;}
,__getEventForProperty:function(dn,dq){var dp=qx.Class.getPropertyDefinition(dn.constructor,dq);if(dp==null){return null;}
;return dp.event;}
,__defaultConversion:function(dt,ds){var dr=qx.lang.Type.getClass(dt);if((dr==J||dr==y)&&(ds==l||ds==F)){dt=parseInt(dt,10);}
;if((dr==b||dr==J||dr==L)&&ds==y){dt=dt+A;}
;if((dr==J||dr==y)&&(ds==J||ds==B)){dt=parseFloat(dt);}
;return dt;}
...
;var dx=this.getAllBindingsForObject(du);if(dx!=undefined){for(var i=0;i<dx.length;i++ ){if(dx[i][0]==dy){var dv=dx[i][3];if(this.__bindingsByTarget[dv.toHashCode()]){qx.lang.Array.remove(this.__bindingsByTarget[dv.toHashCode()],dx[i]);}
;var dw=dx[i][1];if(this.__bindings[dw.toHashCode()]){qx.lang.Array.remove(this.__bindings[dw.toHashCode()],dx[i]);}
;return;}
;}
;}
;throw new Error(u);}
...
;}
;}
,getAllBindingsForObject:function(dH){var dI=dH.toHashCode();if(this.__bindings[dI]===undefined){this.__bindings[dI]=[];}
;var dJ=this.__bindings[dI];var dG=this.__bindingsByTarget[dI]?this.__bindingsByTarget[dI]:[];return qx.lang.Array.unique(dJ.concat(dG));}
,removeAllBindings:function(){for(var dL in this.__bindings){var dK=qx.core.ObjectRegistry.fromHashCode(dL);if(dK==null){delete this.__bindings[dL];continue;}
;this.removeAllBindingsForObject(dK);}
;this.__bindings={};}
,getAllBindings:function(){return this.__bindings;}
,showBindingInLog:function(dN,dP){var dO;for(var i=0;i<this.__bindings[dN.toHashCode()].length;i++ ){if(this.__bindings[dN.toHashCode()][i][0]==dP){dO=this.__bindings[dN.toHashCode()][i];break;}
;}
;if(dO===undefined){var dM=I;}
else {var dM=z+dO[1]+G+dO[2]+g+dO[3]+G+dO[4]+c;}
;qx.log.Logger.debug(dM);}
,showAllBindingsInLog:function(){for(var dR in this.__bindings){var dQ=qx.core.ObjectRegistry.fromHashCode(dR);if(dQ){for(var i=0;i<this.__bindings[dR].length;i++ ){this.showBindingInLog(dQ,this.__bindings[dR][i][0]);}
;}
;}
;}
}});}
...
;}
;return cp;}
,assert:function(cs,cr){cs==true||this.__fail(cr||R,v);}
,fail:function(ct,cu){var cv=cu?R:ch;this.__fail(ct||R,cv);}
,assertTrue:function(cx,cw){(cx===true)||this.__fail(cw||R,m,cx,x);}
,assertFalse:function(cz,cy){(cz===false)||this.__fail(cy||R,X,cz,x);}
,assertEquals:function(cA,cB,cC){cA==cB||this.__fail(cC||R,D,cA,z,cB,P);}
,assertNotEquals:function(cD,cE,cF){cD!=cE||this.__fail(cF||R,D,cD,h,cE,P);}
,assertIdentical:function(cG,cH,cI){cG===cH||this.__fail(cI||R,D,cG,U,cH,P);}
,assertNotIdentical:function(cJ,cK,cL){cJ!==cK||this.__fail(cL||R,D,cJ,bK,cK,P);}
,assertNotUndefined:function(cN,cM){cN!==undefined||this.__fail(cM||R,J);}
,assertUndefined:function(cP,cO){cP===undefined||this.__fail(cO||R,p,cP,I);}
,assertNotNull:function(cR,cQ){cR!==null||this.__fail(cQ||R,bQ);}
,assertNull:function(cT,cS){cT===null||this.__fail(cS||R,t,cT,I);}
,assertJsonEquals:function(cU,cV,cW){this.assertEquals(qx.lang.Json.stringify(cU),qx.lang.Json.stringify(cV),cW);}
,assertMatch:function(da,cY,cX){this.assertString(da);this.assert(qx.lang.Type.isRegExp(cY)||qx.lang.Type.isString(cY),bV);da.search(cY)>=0||this.__fail(cX||R,E,da,bJ,cY.toString(),P);}
,assertArgumentsCount:function(dd,de,df,db){var dc=dd.length;(dc>=de&&dc<=df)||this.__fail(db||R,cg,de,j,df,bS,dc,bN);}
...
;}
;dh===true||this.__fail(dl||R,G,event,O);}
,assertEventNotFired:function(dq,event,dt,du){var dr=false;var ds=function(e){dr=true;}
;var dv=dq.addListener(event,ds,dq);dt.call();dr===false||this.__fail(du||R,G,event,f);dq.removeListenerById(dv);}
,assertException:function(dz,dy,dx,dw){var dy=dy||Error;var dA;try{this.__logError=false;dz();}
catch(dB){dA=dB;}
finally{this.__logError=true;}
;if(dA==null){this.__fail(dw||R,o);}
;dA instanceof dy||this.__fail(dw||R,N,dy,a,dA);if(dx){this.assertMatch(dA.toString(),dx,dw);}
;}
,assertInArray:function(dE,dD,dC){dD.indexOf(dE)!==-1||this.__fail(dC||R,bG,dE,V,dD,x);}
,assertNotInArray:function(dH,dG,dF){dG.indexOf(dH)===-1||this.__fail(dF||R,qx.lang.String.format(L,[dH,dG]));}
...
;}
;}
,assertKeyInMap:function(dN,dM,dL){dM[dN]!==undefined||this.__fail(dL||R,bG,dN,C,dM,x);}
,assertFunction:function(dP,dO){qx.lang.Type.isFunction(dP)||this.__fail(dO||R,cd,dP,I);}
,assertString:function(dR,dQ){qx.lang.Type.isString(dR)||this.__fail(dQ||R,F,dR,I);}
,assertBoolean:function(dT,dS){qx.lang.Type.isBoolean(dT)||this.__fail(dS||R,bP,dT,I);}
,assertNumber:function(dV,dU){(qx.lang.Type.isNumber(dV)&&isFinite(dV))||this.__fail(dU||R,W,dV,I);}
,assertPositiveNumber:function(dX,dW){(qx.lang.Type.isNumber(dX)&&isFinite(dX)&&dX>=0)||this.__fail(dW||R,cc,dX,I);}
,assertInteger:function(ea,dY){(qx.lang.Type.isNumber(ea)&&isFinite(ea)&&ea%1===0)||this.__fail(dY||R,bU,ea,I);}
,assertPositiveInteger:function(ed,eb){var ec=(qx.lang.Type.isNumber(ed)&&isFinite(ed)&&ed%1===0&&ed>=0);ec||this.__fail(eb||R,g,ed,I);}
,assertInRange:function(eg,eh,ef,ee){(eg>=eh&&eg<=ef)||this.__fail(ee||R,qx.lang.String.format(bH,[eg,eh,ef]));}
,assertObject:function(ej,ei){var ek=ej!==null&&(qx.lang.Type.isObject(ej)||typeof ej===bF);ek||this.__fail(ei||R,S,(ej),I);}
,assertArray:function(em,el){qx.lang.Type.isArray(em)||this.__fail(el||R,c,em,I);}
,assertMap:function(eo,en){qx.lang.Type.isObject(eo)||this.__fail(en||R,n,eo,I);}
,assertRegExp:function(eq,ep){qx.lang.Type.isRegExp(eq)||this.__fail(ep||R,r,eq,I);}
,assertType:function(et,es,er){this.assertString(es,u);typeof (et)===es||this.__fail(er||R,bX,es,T,et,I);}
,assertInstance:function(ew,ey,eu){var ev=ey.classname||ey+R;ew instanceof ey||this.__fail(eu||R,ce,ev,T,ew,I);}
,assertInterface:function(eB,eA,ez){qx.Class&&qx.Class.implementsInterface(eB,eA)||this.__fail(ez||R,k,eB,s,eA,P);}
...
catch(eJ){this.__fail(eG||R,H,eH,d,eD.join(B),ca,eE,bT);}
;var eI=eD[0]==eF[0]&&eD[1]==eF[1]&&eD[2]==eF[2];eI||this.__fail(eG||R,H,eD,d,eD.join(B),ca,eE,d,eF.join(B),bE);}
,assertElement:function(eL,eK){!!(eL&&eL.nodeType===1)||this.__fail(eK||R,q,eL,P);}
,assertQxObject:function(eN,eM){this.__isQxInstance(eN,b)||this.__fail(eM||R,bM,eN,I);}
,assertQxWidget:function(eP,eO){this.__isQxInstance(eP,Y)||this.__fail(eO||R,bW,eP,I);}
,__isQxInstance:function(eR,eQ){if(!eR){return false;}
...
;this.__handlers.push(Q);this.__handlers.sort(function(a,b){return a.PRIORITY-b.PRIORITY;}
);}
,getHandlers:function(){return this.__handlers;}
,__dispatchers:[],addDispatcher:function(S,R){{}
;this.__dispatchers.push(S);this.__dispatchers.sort(function(a,b){return a.PRIORITY-b.PRIORITY;}
);}
...
;event.setEventPhase(qx.event.type.Event.AT_TARGET);var g=this._manager.getListeners(e,k,false);if(g){for(var i=0,l=g.length;i<l;i++ ){if(this._manager.isBlacklisted(g[i].unique)){continue;}
;var h=g[i].context||e;{}
