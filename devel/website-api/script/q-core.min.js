...
,exclude:function(L,K){if(L instanceof qx.data.Array){return L.exclude(L,K);}
;{}
...
;}
;return cn;}
,assert:function(cq,cp){cq==true||this.__fail(cp||Q,v);}
,fail:function(cr,cs){var ct=cs?Q:bS;this.__fail(cr||Q,ct);}
,assertTrue:function(cv,cu){(cv===true)||this.__fail(cu||Q,m,cv,x);}
,assertFalse:function(cx,cw){(cx===false)||this.__fail(cw||Q,W,cx,x);}
,assertEquals:function(cy,cz,cA){cy==cz||this.__fail(cA||Q,D,cy,z,cz,O);}
,assertNotEquals:function(cB,cC,cD){cB!=cC||this.__fail(cD||Q,D,cB,h,cC,O);}
,assertIdentical:function(cE,cF,cG){cE===cF||this.__fail(cG||Q,D,cE,T,cF,O);}
,assertNotIdentical:function(cH,cI,cJ){cH!==cI||this.__fail(cJ||Q,D,cH,bH,cI,O);}
,assertNotUndefined:function(cL,cK){cL!==undefined||this.__fail(cK||Q,J);}
,assertUndefined:function(cN,cM){cN===undefined||this.__fail(cM||Q,p,cN,I);}
,assertNotNull:function(cP,cO){cP!==null||this.__fail(cO||Q,bN);}
,assertNull:function(cR,cQ){cR===null||this.__fail(cQ||Q,t,cR,I);}
,assertJsonEquals:function(cS,cT,cU){this.assertEquals(qx.lang.Json.stringify(cS),qx.lang.Json.stringify(cT),cU);}
,assertMatch:function(cX,cW,cV){this.assertString(cX);this.assert(qx.lang.Type.isRegExp(cW)||qx.lang.Type.isString(cW),cf);cX.search(cW)>=0||this.__fail(cV||Q,E,cX,bG,cW.toString(),O);}
,assertArgumentsCount:function(db,dc,dd,cY){var da=db.length;(da>=dc&&da<=dd)||this.__fail(cY||Q,cd,dc,j,dd,bP,da,bK);}
...
;}
;df===true||this.__fail(dj||Q,G,event,N);}
,assertEventNotFired:function(dn,event,dr,ds){var dp=false;var dq=function(e){dp=true;}
;var dt=dn.addListener(event,dq,dn);dr.call();dp===false||this.__fail(ds||Q,G,event,f);dn.removeListenerById(dt);}
,assertException:function(dx,dw,dv,du){var dw=dw||Error;var dy;try{this.__logError=false;dx();}
catch(dz){dy=dz;}
finally{this.__logError=true;}
;if(dy==null){this.__fail(du||Q,o);}
;dy instanceof dw||this.__fail(du||Q,M,dw,a,dy);if(dv){this.assertMatch(dy.toString(),dv,du);}
;}
,assertInArray:function(dC,dB,dA){dB.indexOf(dC)!==-1||this.__fail(dA||Q,bU,dC,U,dB,x);}
...
;}
;}
,assertKeyInMap:function(dI,dH,dG){dH[dI]!==undefined||this.__fail(dG||Q,bU,dI,C,dH,x);}
,assertFunction:function(dK,dJ){qx.lang.Type.isFunction(dK)||this.__fail(dJ||Q,bW,dK,I);}
,assertString:function(dM,dL){qx.lang.Type.isString(dM)||this.__fail(dL||Q,F,dM,I);}
,assertBoolean:function(dO,dN){qx.lang.Type.isBoolean(dO)||this.__fail(dN||Q,bM,dO,I);}
,assertNumber:function(dQ,dP){(qx.lang.Type.isNumber(dQ)&&isFinite(dQ))||this.__fail(dP||Q,V,dQ,I);}
,assertPositiveNumber:function(dS,dR){(qx.lang.Type.isNumber(dS)&&isFinite(dS)&&dS>=0)||this.__fail(dR||Q,bY,dS,I);}
,assertInteger:function(dU,dT){(qx.lang.Type.isNumber(dU)&&isFinite(dU)&&dU%1===0)||this.__fail(dT||Q,cb,dU,I);}
,assertPositiveInteger:function(dX,dV){var dW=(qx.lang.Type.isNumber(dX)&&isFinite(dX)&&dX%1===0&&dX>=0);dW||this.__fail(dV||Q,g,dX,I);}
,assertInRange:function(eb,ec,ea,dY){(eb>=ec&&eb<=ea)||this.__fail(dY||Q,qx.lang.String.format(bL,[eb,ec,ea]));}
,assertObject:function(ee,ed){var ef=ee!==null&&(qx.lang.Type.isObject(ee)||typeof ee===ce);ef||this.__fail(ed||Q,R,(ee),I);}
,assertArray:function(eh,eg){qx.lang.Type.isArray(eh)||this.__fail(eg||Q,c,eh,I);}
,assertMap:function(ej,ei){qx.lang.Type.isObject(ej)||this.__fail(ei||Q,n,ej,I);}
,assertRegExp:function(el,ek){qx.lang.Type.isRegExp(el)||this.__fail(ek||Q,r,el,I);}
,assertType:function(eo,en,em){this.assertString(en,u);typeof (eo)===en||this.__fail(em||Q,bE,en,S,eo,I);}
,assertInstance:function(er,es,ep){var eq=es.classname||es+Q;er instanceof es||this.__fail(ep||Q,ca,eq,S,er,I);}
,assertInterface:function(ev,eu,et){qx.Class&&qx.Class.implementsInterface(ev,eu)||this.__fail(et||Q,k,ev,s,eu,O);}
...
catch(eE){this.__fail(eB||Q,H,eC,d,ey.join(B),bT,ez,bQ);}
;var eD=ey[0]==eA[0]&&ey[1]==eA[1]&&ey[2]==eA[2];eD||this.__fail(eB||Q,H,ey,d,ey.join(B),bT,ez,d,eA.join(B),bO);}
,assertElement:function(eG,eF){!!(eG&&eG.nodeType===1)||this.__fail(eF||Q,q,eG,O);}
,assertQxObject:function(eI,eH){this.__isQxInstance(eI,b)||this.__fail(eH||Q,bJ,eI,I);}
,assertQxWidget:function(eK,eJ){this.__isQxInstance(eK,X)||this.__fail(eJ||Q,bD,eK,I);}
,__isQxInstance:function(eM,eL){if(!eM){return false;}
