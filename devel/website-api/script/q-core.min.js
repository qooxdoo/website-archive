...
;du=du.superclass;}
;return dv;}
,__createRefresher:function(inheritables){var inherit=this.$$store.inherit;var init=this.$$store.init;var refresh=this.$$method.refresh;var code=[o,x];for(var i=0,l=inheritables.length;i<l;i++ ){var name=inheritables[i];code.push(bq,inherit[name],bL,cq,init[name],bL,bW,refresh[name],bv);}
;return new Function(code.join(cS));}
...
;if(config.mode==cN){var shorthand=dj;setter.push(shorthand);if(themeable){styler.push(shorthand);}
;}
;for(var i=0,a=config.group,l=a.length;i<l;i++ ){{}
;setter.push(bW,this.$$method.set[a[i]],bS,i,dp);resetter.push(bW,this.$$method.reset[a[i]],s);if(themeable){{}
;styler.push(bW,this.$$method.setThemed[a[i]],bS,i,dp);unstyler.push(bW,this.$$method.resetThemed[a[i]],s);}
;}
;this.$$method.set[name]=e+upname;members[this.$$method.set[name]]=new Function(setter.join(cS));this.$$method.reset[name]=bc+upname;members[this.$$method.reset[name]]=new Function(resetter.join(cS));if(themeable){this.$$method.setThemed[name]=bD+upname;members[this.$$method.setThemed[name]]=new Function(styler.join(cS));this.$$method.resetThemed[name]=g+upname;members[this.$$method.resetThemed[name]]=new Function(unstyler.join(cS));}
;}
,__attachPropertyMethods:function(clazz,config,name){var upname=qx.Bootstrap.firstUp(name);var members=clazz.prototype;{}
;if(config.dereference===undefined&&typeof config.check===bd){config.dereference=this.__shouldBeDereferenced(config.check);}
;var method=this.$$method;var store=this.$$store;store.runtime[name]=bC+name;store.user[name]=bG+name;store.theme[name]=cA+name;store.init[name]=cz+name;store.inherit[name]=dc+name;store.useinit[name]=br+name;method.get[name]=bm+upname;members[method.get[name]]=function(){return qx.core.Property.executeOptimizedGetter(this,clazz,name,bm);}
;members[method.get[name]].$$install=function(dA){qx.core.Property.installOptimizedGetter(this,clazz,name,bm,arguments);}
...
;members[method.reset[name]].$$install=function(){qx.core.Property.installOptimizedSetter(this,clazz,name,bc);}
...
;{}
;}
;method.setRuntime[name]=bU+upname;members[method.setRuntime[name]]=function(dF){return qx.core.Property.executeOptimizedSetter(this,clazz,name,bU,arguments);}
;method.resetRuntime[name]=T+upname;members[method.resetRuntime[name]]=function(){return qx.core.Property.executeOptimizedSetter(this,clazz,name,T);}
;if(config.themeable){method.setThemed[name]=bD+upname;members[method.setThemed[name]]=function(dG){return qx.core.Property.executeOptimizedSetter(this,clazz,name,bD,arguments);}
...
;{}
;}
;if(config.check===bP){members[da+upname]=new Function(bg+method.set[name]+cU+method.get[name]+ch);members[dm+upname]=new Function(bg+method.get[name]+cb);{}
;}
;{}
;}
,__shouldBeDereferenced:function(dH){return !!this.__dereference[dH];}
,__errors:{'0':dq,'1':by,'2':B,'3':bY,'4':cK,'5':N},error:function(dI,dO,dN,dJ,dK){var dL=dI.constructor.classname;var dM=p+dN+V+dL+y+this.$$method[dJ][dN]+di+dK+U;throw new Error(dM+(this.__errors[dO]||cD+dO));}
...
;}
,__installFunctionFromCode:function(instance,members,name,variant,code,args){var store=this.$$method[variant][name];{members[store]=new Function(cM,code.join(cS));}
;if(qx.core.Environment.get(cG)){members[store]=qx.core.Aspect.wrap(instance.classname+cw+store,members[store],r);}
;qx.Bootstrap.setDisplayName(members[store],instance.classname+bN,store);return instance[store];}
...
,__compileGetter:function(ej,ei,name,eh){var ek=ei.$$properties[name];var eg=[];var el=this.$$store;eg.push(K,el.runtime[name],bH);eg.push(L,el.runtime[name],f);if(ek.inheritable){eg.push(M,el.inherit[name],bH);eg.push(L,el.inherit[name],f);eg.push(D);}
;eg.push(K,el.user[name],bH);eg.push(L,el.user[name],f);if(ek.themeable){eg.push(M,el.theme[name],bH);eg.push(L,el.theme[name],f);}
;if(ek.deferredInit&&ek.init===undefined){eg.push(M,el.init[name],bH);eg.push(L,el.init[name],f);}
;eg.push(D);if(ek.init!==undefined){if(ek.inheritable){eg.push(m,el.init[name],f);if(ek.nullable){eg.push(bl);}
;eg.push(k);}
else {eg.push(L,el.init[name],f);}
;}
else if(ek.inheritable||ek.nullable){eg.push(ck);}
else {eg.push(cT,name,cY,ei.classname,dn);}
;return eg;}
...
;if(ey){ez.push(bV);}
;return ez;}
,__getStore:function(eI,name){if(eI===bU||eI===T){var eJ=this.$$store.runtime[name];}
else if(eI===bD||eI===g){eJ=this.$$store.theme[name];}
else if(eI===cE){eJ=this.$$store.init[name];}
else {eJ=this.$$store.user[name];}
;return eJ;}
,__emitIsEqualFunction:function(eK,eN,eL,name){eK.push(bF);if(typeof eL.isEqual===bO){eK.push(bb,eN.classname,cd,name,bQ);}
else if(typeof eL.isEqual===bd){var eM=eN.prototype;if(eM[eL.isEqual]!==undefined){eK.push(bx,eL.isEqual,f);}
else {eK.push(ba,eL.isEqual,bw);}
;}
else if(typeof eL.isEqual===df){eK.push(cj);}
else {throw new Error(de+name+bu+eN.classname+cX);}
;}
,__emitSetterPreConditions:function(eQ,eO,name,eR,eP){{if(!eO.nullable||eO.check||eO.inheritable){eQ.push(dd);}
;if(eR===e){eQ.push(O,name,bk,eR,bz);}
;}
;}
,__emitIncomingValueTransformation:function(eS,eU,eT,name){if(eT.transform){eS.push(bj,eT.transform,bt);}
;if(eT.validate){if(typeof eT.validate===bd){eS.push(bx,eT.validate,bt);}
else if(eT.validate instanceof Function){eS.push(eU.classname,cd,name);eS.push(t);}
;}
;}
,__emitOldNewComparison:function(eW,eV,eY,eX){var fa=(eX===bc||eX===g||eX===T);if(eV){eW.push(cW,eY,I);}
else if(fa){eW.push(K,eY,X);}
;}
,__emitIncomingValueValidation:undefined,__emitStoreValue:function(fc,name,fd,fb){if(fd===bU){fc.push(bx,this.$$store.runtime[name],ci);}
else if(fd===T){fc.push(K,this.$$store.runtime[name],bH);fc.push(S,this.$$store.runtime[name],f);}
else if(fd===e){fc.push(bx,this.$$store.user[name],ci);}
else if(fd===bc){fc.push(K,this.$$store.user[name],bH);fc.push(S,this.$$store.user[name],f);}
else if(fd===bD){fc.push(bx,this.$$store.theme[name],ci);}
else if(fd===g){fc.push(K,this.$$store.theme[name],bH);fc.push(S,this.$$store.theme[name],f);}
else if(fd===cE&&fb){fc.push(bx,this.$$store.init[name],ci);}
;}
,__emitStoreComputedAndOldValue:function(fg,fe,name,fh,ff){if(fe.inheritable){fg.push(Q,this.$$store.inherit[name],f);}
else {fg.push(cu);}
;fg.push(K,this.$$store.runtime[name],cB);if(fh===bU){fg.push(cO,this.$$store.runtime[name],ci);}
else if(fh===T){fg.push(S,this.$$store.runtime[name],f);fg.push(K,this.$$store.user[name],bH);fg.push(cO,this.$$store.user[name],f);fg.push(M,this.$$store.theme[name],bH);fg.push(cO,this.$$store.theme[name],f);fg.push(M,this.$$store.init[name],cB);fg.push(cO,this.$$store.init[name],f);fg.push(bx,this.$$store.useinit[name],E);fg.push(cx);}
else {fg.push(H,this.$$store.runtime[name],f);if(fh===e){fg.push(bx,this.$$store.user[name],ci);}
else if(fh===bc){fg.push(S,this.$$store.user[name],f);}
else if(fh===bD){fg.push(bx,this.$$store.theme[name],ci);}
else if(fh===g){fg.push(S,this.$$store.theme[name],f);}
else if(fh===cE&&ff){fg.push(bx,this.$$store.init[name],ci);}
;}
;fg.push(cx);fg.push(M,this.$$store.user[name],cB);if(fh===e){if(!fe.inheritable){fg.push(cf,this.$$store.user[name],f);}
;fg.push(cO,this.$$store.user[name],ci);}
else if(fh===bc){if(!fe.inheritable){fg.push(cf,this.$$store.user[name],f);}
;fg.push(S,this.$$store.user[name],f);fg.push(K,this.$$store.runtime[name],bH);fg.push(cO,this.$$store.runtime[name],f);fg.push(K,this.$$store.theme[name],bH);fg.push(cO,this.$$store.theme[name],f);fg.push(M,this.$$store.init[name],cB);fg.push(cO,this.$$store.init[name],f);fg.push(bx,this.$$store.useinit[name],E);fg.push(cx);}
else {if(fh===bU){fg.push(cO,this.$$store.runtime[name],ci);}
else if(fe.inheritable){fg.push(cO,this.$$store.user[name],f);}
else {fg.push(H,this.$$store.user[name],f);}
;if(fh===bD){fg.push(bx,this.$$store.theme[name],ci);}
else if(fh===g){fg.push(S,this.$$store.theme[name],f);}
else if(fh===cE&&ff){fg.push(bx,this.$$store.init[name],ci);}
;}
;fg.push(cx);if(fe.themeable){fg.push(M,this.$$store.theme[name],cB);if(!fe.inheritable){fg.push(cf,this.$$store.theme[name],f);}
;if(fh===bU){fg.push(cO,this.$$store.runtime[name],ci);}
else if(fh===e){fg.push(cO,this.$$store.user[name],ci);}
else if(fh===bD){fg.push(cO,this.$$store.theme[name],ci);}
else if(fh===g){fg.push(S,this.$$store.theme[name],f);fg.push(K,this.$$store.init[name],cB);fg.push(cO,this.$$store.init[name],f);fg.push(bx,this.$$store.useinit[name],E);fg.push(cx);}
else if(fh===cE){if(ff){fg.push(bx,this.$$store.init[name],ci);}
;fg.push(cO,this.$$store.theme[name],f);}
else if(fh===db){fg.push(cO,this.$$store.theme[name],f);}
;fg.push(cx);}
;fg.push(M,this.$$store.useinit[name],bI);if(!fe.inheritable){fg.push(cf,this.$$store.init[name],f);}
;if(fh===cE){if(ff){fg.push(cO,this.$$store.init[name],ci);}
else {fg.push(cO,this.$$store.init[name],f);}
;}
else if(fh===e||fh===bU||fh===bD||fh===db){fg.push(S,this.$$store.useinit[name],f);if(fh===bU){fg.push(cO,this.$$store.runtime[name],ci);}
else if(fh===e){fg.push(cO,this.$$store.user[name],ci);}
else if(fh===bD){fg.push(cO,this.$$store.theme[name],ci);}
else if(fh===db){fg.push(cO,this.$$store.init[name],f);}
;}
;fg.push(cx);if(fh===e||fh===bU||fh===bD||fh===cE){fg.push(w);if(fh===bU){fg.push(cO,this.$$store.runtime[name],ci);}
else if(fh===e){fg.push(cO,this.$$store.user[name],ci);}
else if(fh===bD){fg.push(cO,this.$$store.theme[name],ci);}
else if(fh===cE){if(ff){fg.push(cO,this.$$store.init[name],ci);}
else {fg.push(cO,this.$$store.init[name],f);}
;fg.push(bx,this.$$store.useinit[name],E);}
;fg.push(cx);}
;}
,__emitStoreInheritedPropertyValue:function(fj,fi,name,fk){fj.push(bK);if(fk===db){fj.push(bA);}
else {fj.push(cp,this.$$store.inherit[name],f);}
;fj.push(cI);fj.push(bx,this.$$store.init[name],ct);fj.push(bx,this.$$store.init[name],b);fj.push(cO,this.$$store.init[name],f);fj.push(bx,this.$$store.useinit[name],E);fj.push(bB);fj.push(S,this.$$store.useinit[name],bE);fj.push(cx);fj.push(dh);fj.push(G);fj.push(bs,this.$$store.inherit[name],f);fj.push(cx);fj.push(R);fj.push(S,this.$$store.inherit[name],f);fj.push(bi,this.$$store.inherit[name],A);fj.push(cv);if(fi.init!==undefined&&fk!==cE){fj.push(F,this.$$store.init[name],bL);}
else {fj.push(co);}
;fj.push(dk);}
,__emitNormalizeUndefinedValues:function(fm,fl,name,fo){if(fo!==e&&fo!==bU&&fo!==bD){fm.push(cC);}
;fm.push(dh);if(fl.init!==undefined&&fo!==cE){fm.push(F,this.$$store.init[name],bL);}
else {fm.push(co);}
;}
,__emitCallCallback:function(fq,fp,name,fr){if(fp.apply){fq.push(bx,fp.apply,bT,name,P,fr,cm);}
;if(fp.event){fq.push(cg,bX,fp.event,be,cJ,fp.event,bp,bR);}
;}
,__emitRefreshChildrenValue:function(fs,name){fs.push(q);fs.push(v,this.$$method.refresh[name],ca,this.$$method.refresh[name],C);fs.push(cx);}
}});}
)();
