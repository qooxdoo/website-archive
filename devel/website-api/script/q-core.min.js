...
;var argHandler=cb;setter.push(argHandler);if(themeable){styler.push(argHandler);}
;if(config.mode==cO){var shorthand=dk;setter.push(shorthand);if(themeable){styler.push(shorthand);}
;}
;for(var i=0,a=config.group,l=a.length;i<l;i++ ){{}
;setter.push(bV,this.$$method.set[a[i]],bR,i,dq);resetter.push(bV,this.$$method.reset[a[i]],t);if(themeable){{}
;styler.push(bV,this.$$method.setThemed[a[i]],bR,i,dq);unstyler.push(bV,this.$$method.resetThemed[a[i]],t);}
;}
...
;if(config.dereference===undefined&&typeof config.check===bc){config.dereference=this.__shouldBeDereferenced(config.check);}
;var method=this.$$method;var store=this.$$store;store.runtime[name]=bA+name;store.user[name]=bE+name;store.theme[name]=cA+name;store.init[name]=cz+name;store.inherit[name]=dd+name;store.useinit[name]=bp+name;method.get[name]=bl+upname;members[method.get[name]]=function(){return qx.core.Property.executeOptimizedGetter(this,clazz,name,bl);}
;members[method.get[name]].$$install=function(dB){qx.core.Property.installOptimizedGetter(this,clazz,name,bl,arguments);}
;method.set[name]=e+upname;members[method.set[name]]=function(dC){return qx.core.Property.executeOptimizedSetter(this,clazz,name,e,arguments);}
;members[method.set[name]].$$install=function(dD){qx.core.Property.installOptimizedSetter(this,clazz,name,e,arguments);}
;method.reset[name]=bb+upname;members[method.reset[name]]=function(){return qx.core.Property.executeOptimizedSetter(this,clazz,name,bb);}
;members[method.reset[name]].$$install=function(){qx.core.Property.installOptimizedSetter(this,clazz,name,bb);}
;if(config.inheritable||config.apply||config.event||config.deferredInit){method.init[name]=cE+upname;members[method.init[name]]=function(dE){return qx.core.Property.executeOptimizedSetter(this,clazz,name,cE,arguments);}
;{}
;}
;if(config.inheritable){method.refresh[name]=dc+upname;members[method.refresh[name]]=function(dF){return qx.core.Property.executeOptimizedSetter(this,clazz,name,dc,arguments);}
;{}
;}
;method.setRuntime[name]=bT+upname;members[method.setRuntime[name]]=function(dG){return qx.core.Property.executeOptimizedSetter(this,clazz,name,bT,arguments);}
...
;if(config.themeable){method.setThemed[name]=bB+upname;members[method.setThemed[name]]=function(dH){return qx.core.Property.executeOptimizedSetter(this,clazz,name,bB,arguments);}
...
;{}
;}
;if(config.check===bO){members[db+upname]=new Function(bf+method.set[name]+cV+method.get[name]+cg);members[dn+upname]=new Function(bf+method.get[name]+ca);{}
;}
;{}
;}
,__shouldBeDereferenced:function(dI){return !!this.__dereference[dI];}
,__errors:{'0':dr,'1':bw,'2':B,'3':bX,'4':cK,'5':M},error:function(dJ,dP,dO,dK,dL){var dM=dJ.constructor.classname;var dN=q+dO+U+dM+y+this.$$method[dK][dO]+dj+dL+T;throw new Error(dN+(this.__errors[dP]||cD+dP));}
,__unwrapFunctionFromCode:function(dT,dU,name,dS,dQ,dR){var dV=this.__installFunctionFromCode(dT,dU,name,dS,dQ,dR);if(dR===undefined){return dV.call(dT);}
else {return dV.call(dT,dR[0]);}
;}
...
;qx.Bootstrap.setDisplayName(members[store],instance.classname+bM,store);return instance[store];}
,executeOptimizedGetter:function(ea,dY,name,dX){var dW=this.__compileGetter(ea,dY,name,dX);var eb=dY.prototype;return this.__unwrapFunctionFromCode(ea,eb,name,dX,dW);}
,installOptimizedGetter:function(ef,ee,name,ed){var ec=this.__compileGetter(ef,ee,name,ed);var eg=ee.prototype;this.__installFunctionFromCode(ef,eg,name,ed,ec);}
,__compileGetter:function(ek,ej,name,ei){var el=ej.$$properties[name];var eh=[];var em=this.$$store;eh.push(J,em.runtime[name],bF);eh.push(K,em.runtime[name],f);if(el.inheritable){eh.push(L,em.inherit[name],bF);eh.push(K,em.inherit[name],f);eh.push(D);}
;eh.push(J,em.user[name],bF);eh.push(K,em.user[name],f);if(el.themeable){eh.push(L,em.theme[name],bF);eh.push(K,em.theme[name],f);}
;if(el.deferredInit&&el.init===undefined){eh.push(L,em.init[name],bF);eh.push(K,em.init[name],f);}
;eh.push(D);if(el.init!==undefined){if(el.inheritable){eh.push(m,em.init[name],f);if(el.nullable){eh.push(bk);}
;eh.push(k);}
else {eh.push(K,em.init[name],f);}
;}
else if(el.inheritable||el.nullable){eh.push(cj);}
else {eh.push(cU,name,da,ej.classname,dp);}
;return eh;}
,executeOptimizedSetter:function(er,eq,name,ep,eo){var en=this.__compileSetter(er,eq,name,ep,eo);var es=eq.prototype;return this.__unwrapFunctionFromCode(er,es,name,ep,en,eo);}
,installOptimizedSetter:function(ex,ew,name,ev,eu){var et=this.__compileSetter(ex,ew,name,ev,eu);var ey=ew.prototype;return this.__installFunctionFromCode(ex,ey,name,ev,et,eu);}
,__compileSetter:function(eF,eE,name,eD,eC){var eH=eE.$$properties[name];var eG=eE.prototype;var eA=[];var ez=eD===e||eD===bB||eD===bT||(eD===cE&&eH.init===undefined);var eB=eH.apply||eH.event||eH.inheritable;var eI=this.__getStore(eD,name);this.__emitIsEqualFunction(eA,eE,eH,name);this.__emitSetterPreConditions(eA,eH,name,eD,ez);if(ez||eB){this.__emitOldValue(eA,eH,name);}
;if(ez){this.__emitIncomingValueTransformation(eA,eE,eH,name);}
;if(eB){this.__emitOldNewComparison(eA,ez,eI,eD);}
;if(eH.inheritable){eA.push(bn);}
;{}
;if(!eB){this.__emitStoreValue(eA,name,eD,ez);}
else {this.__emitStoreComputedValue(eA,eH,name,eD,ez);}
;if(eH.inheritable){this.__emitStoreInheritedPropertyValue(eA,eH,name,eD);}
else if(eB){this.__emitNormalizeUndefinedValues(eA,eH,name,eD);}
;if(eB){this.__emitCallCallback(eA,eH,name,eD);if(eH.inheritable&&eG._getChildren){this.__emitRefreshChildrenValue(eA,name);}
;}
;if(ez){eA.push(bU);}
;return eA;}
,__getStore:function(eJ,name){if(eJ===bT||eJ===S){var eK=this.$$store.runtime[name];}
else if(eJ===bB||eJ===g){eK=this.$$store.theme[name];}
else if(eJ===cE){eK=this.$$store.init[name];}
else {eK=this.$$store.user[name];}
;return eK;}
,__emitIsEqualFunction:function(eL,eO,eM,name){eL.push(bD);if(typeof eM.isEqual===bN){eL.push(ba,eO.classname,cc,name,bP);}
else if(typeof eM.isEqual===bc){var eN=eO.prototype;if(eN[eM.isEqual]!==undefined){eL.push(bv,eM.isEqual,f);}
else {eL.push(Y,eM.isEqual,bu);}
;}
else if(typeof eM.isEqual===dg){eL.push(ci);}
else {throw new Error(df+name+bs+eO.classname+cY);}
;}
,__emitSetterPreConditions:function(eR,eP,name,eS,eQ){{if(!eP.nullable||eP.check||eP.inheritable){eR.push(de);}
;if(eS===e){eR.push(N,name,bj,eS,bx);}
;}
;}
,__emitIncomingValueTransformation:function(eT,eV,eU,name){if(eU.transform){eT.push(bi,eU.transform,n);}
;if(eU.validate){if(typeof eU.validate===bc){eT.push(bv,eU.validate,br);}
else if(eU.validate instanceof Function){eT.push(eV.classname,cc,name);eT.push(u);}
;}
;}
,__emitOldNewComparison:function(eX,eW,fa,eY){var fb=(eY===bb||eY===g||eY===S);if(eW){eX.push(cX,fa,H);}
else if(fb){eX.push(J,fa,W);}
;}
,__emitIncomingValueValidation:undefined,__emitStoreValue:function(fd,name,fe,fc){if(fe===bT){fd.push(bv,this.$$store.runtime[name],ch);}
else if(fe===S){fd.push(J,this.$$store.runtime[name],bF);fd.push(R,this.$$store.runtime[name],f);}
else if(fe===e){fd.push(bv,this.$$store.user[name],ch);}
else if(fe===bb){fd.push(J,this.$$store.user[name],bF);fd.push(R,this.$$store.user[name],f);}
else if(fe===bB){fd.push(bv,this.$$store.theme[name],ch);}
else if(fe===g){fd.push(J,this.$$store.theme[name],bF);fd.push(R,this.$$store.theme[name],f);}
else if(fe===cE&&fc){fd.push(bv,this.$$store.init[name],ch);}
;}
,__emitStoreComputedValue:function(fh,ff,name,fi,fg){fh.push(cN);fh.push(J,this.$$store.runtime[name],cB);if(fi===bT){fh.push(cP,this.$$store.runtime[name],ch);}
else if(fi===S){fh.push(R,this.$$store.runtime[name],f);fh.push(J,this.$$store.user[name],bF);fh.push(cP,this.$$store.user[name],f);fh.push(L,this.$$store.theme[name],bF);fh.push(cP,this.$$store.theme[name],f);fh.push(L,this.$$store.init[name],cB);fh.push(cP,this.$$store.init[name],f);fh.push(bv,this.$$store.useinit[name],E);fh.push(cx);}
else {fh.push(cP,this.$$store.runtime[name],f);if(fi===e){fh.push(bv,this.$$store.user[name],ch);}
else if(fi===bb){fh.push(R,this.$$store.user[name],f);}
else if(fi===bB){fh.push(bv,this.$$store.theme[name],ch);}
else if(fi===g){fh.push(R,this.$$store.theme[name],f);}
else if(fi===cE&&fg){fh.push(bv,this.$$store.init[name],ch);}
;}
;fh.push(cx);fh.push(L,this.$$store.user[name],cB);if(fi===e){fh.push(cP,this.$$store.user[name],ch);}
else if(fi===bb){fh.push(R,this.$$store.user[name],f);fh.push(J,this.$$store.runtime[name],bF);fh.push(cP,this.$$store.runtime[name],f);fh.push(J,this.$$store.theme[name],bF);fh.push(cP,this.$$store.theme[name],f);fh.push(L,this.$$store.init[name],cB);fh.push(cP,this.$$store.init[name],f);fh.push(bv,this.$$store.useinit[name],E);fh.push(cx);}
else {if(fi===bT){fh.push(cP,this.$$store.runtime[name],ch);}
else if(ff.inheritable){fh.push(cP,this.$$store.user[name],f);}
else {fh.push(cP,this.$$store.user[name],f);}
;if(fi===bB){fh.push(bv,this.$$store.theme[name],ch);}
else if(fi===g){fh.push(R,this.$$store.theme[name],f);}
else if(fi===cE&&fg){fh.push(bv,this.$$store.init[name],ch);}
;}
;fh.push(cx);if(ff.themeable){fh.push(L,this.$$store.theme[name],cB);if(fi===bT){fh.push(cP,this.$$store.runtime[name],ch);}
else if(fi===e){fh.push(cP,this.$$store.user[name],ch);}
else if(fi===bB){fh.push(cP,this.$$store.theme[name],ch);}
else if(fi===g){fh.push(R,this.$$store.theme[name],f);fh.push(J,this.$$store.init[name],cB);fh.push(cP,this.$$store.init[name],f);fh.push(bv,this.$$store.useinit[name],E);fh.push(cx);}
else if(fi===cE){if(fg){fh.push(bv,this.$$store.init[name],ch);}
;fh.push(cP,this.$$store.theme[name],f);}
else if(fi===dc){fh.push(cP,this.$$store.theme[name],f);}
;fh.push(cx);}
;fh.push(L,this.$$store.useinit[name],bG);if(fi===cE){if(fg){fh.push(cP,this.$$store.init[name],ch);}
else {fh.push(cP,this.$$store.init[name],f);}
;}
else if(fi===e||fi===bT||fi===bB||fi===dc){fh.push(R,this.$$store.useinit[name],f);if(fi===bT){fh.push(cP,this.$$store.runtime[name],ch);}
else if(fi===e){fh.push(cP,this.$$store.user[name],ch);}
else if(fi===bB){fh.push(cP,this.$$store.theme[name],ch);}
else if(fi===dc){fh.push(cP,this.$$store.init[name],f);}
;}
;fh.push(cx);if(fi===e||fi===bT||fi===bB||fi===cE){fh.push(w);if(fi===bT){fh.push(cP,this.$$store.runtime[name],ch);}
else if(fi===e){fh.push(cP,this.$$store.user[name],ch);}
else if(fi===bB){fh.push(cP,this.$$store.theme[name],ch);}
else if(fi===cE){if(fg){fh.push(cP,this.$$store.init[name],ch);}
else {fh.push(cP,this.$$store.init[name],f);}
;fh.push(bv,this.$$store.useinit[name],E);}
;fh.push(cx);}
;}
,__emitOldValue:function(fj,fk,name){if(fk.inheritable){fj.push(cm,this.$$store.inherit[name],f);}
else {fj.push(r);}
;fj.push(J,this.$$store.runtime[name],cB);fj.push(ce,this.$$store.runtime[name],f);fj.push(cx);if(!fk.inheritable){fj.push(L,this.$$store.user[name],cB);fj.push(ce,this.$$store.user[name],f);fj.push(cx);if(fk.themeable){fj.push(L,this.$$store.theme[name],cB);fj.push(ce,this.$$store.theme[name],f);fj.push(cx);}
;fj.push(L,this.$$store.useinit[name],bG);fj.push(ce,this.$$store.init[name],f);fj.push(cx);}
;}
,__emitStoreInheritedPropertyValue:function(fm,fl,name,fo){fm.push(bI);if(fo===dc){fm.push(by);}
else {fm.push(cp,this.$$store.inherit[name],f);}
;fm.push(cI);fm.push(bv,this.$$store.init[name],ct);fm.push(bv,this.$$store.init[name],b);fm.push(cP,this.$$store.init[name],f);fm.push(bv,this.$$store.useinit[name],E);fm.push(bz);fm.push(R,this.$$store.useinit[name],bC);fm.push(cx);fm.push(di);fm.push(G);fm.push(bq,this.$$store.inherit[name],f);fm.push(cx);fm.push(Q);fm.push(R,this.$$store.inherit[name],f);fm.push(bh,this.$$store.inherit[name],A);fm.push(cv);if(fl.init!==undefined&&fo!==cE){fm.push(F,this.$$store.init[name],bJ);}
else {fm.push(co);}
;fm.push(dl);}
,__emitNormalizeUndefinedValues:function(fq,fp,name,fr){if(fr!==e&&fr!==bT&&fr!==bB){fq.push(cC);}
;fq.push(di);if(fp.init!==undefined&&fr!==cE){fq.push(F,this.$$store.init[name],bJ);}
else {fq.push(co);}
;}
,__emitCallCallback:function(ft,fs,name,fu){if(fs.apply){ft.push(bv,fs.apply,bS,name,O,fu,cl);}
;if(fs.event){ft.push(cf,bW,fs.event,bd,cJ,fs.event,cu,bQ);}
;}
,__emitRefreshChildrenValue:function(fv,name){fv.push(bK);fv.push(P,this.$$method.refresh[name],bY,this.$$method.refresh[name],C);fv.push(cx);}
}});}
)();
