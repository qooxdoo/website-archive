...
(function(){var b=".prototype",c="$$init_",d="constructor",e="Property module disabled.",f="extend",g="module.property",h="singleton",j='undefined',k="module.events",m="qx.aspects",n="toString",o='extend',p="Array",q="static",r="qx.event.type.Data",s="",t="Events module not enabled.",u="]",v="Class",w="qx.Class",x='"extend" parameter is null or undefined',y="[Class ",z="destructor",A="destruct",B=".",C="member";qx.Bootstrap.define(w,{statics:{__Property:qx.core.Environment.get(g)?qx.core.Property:null,define:function(name,G){if(!G){G={};}
;if(G.include&&!(qx.Bootstrap.getClass(G.include)===p)){G.include=[G.include];}
;if(G.implement&&!(qx.Bootstrap.getClass(G.implement)===p)){G.implement=[G.implement];}
;var D=false;if(!G.hasOwnProperty(f)&&!G.type){G.type=q;D=true;}
;{}
;var E=this.__createClass(name,G.type,G.extend,G.statics,G.construct,G.destruct,G.include);if(G.extend){if(G.properties){this.__addProperties(E,G.properties,true);}
;if(G.members){this.__addMembers(E,G.members,true,true,false);}
;if(G.events){this.__addEvents(E,G.events,true);}
;if(G.include){for(var i=0,l=G.include.length;i<l;i++ ){this.__addMixin(E,G.include[i],false);}
;}
;}
else if(G.hasOwnProperty(o)&&false){throw new Error(x);}
;if(G.environment){for(var F in G.environment){qx.core.Environment.add(F,G.environment[F]);}
;}
;if(G.implement){for(var i=0,l=G.implement.length;i<l;i++ ){this.__addInterface(E,G.implement[i]);}
;}
;{}
;if(G.defer){G.defer.self=E;qx.Bootstrap.addPendingDefer(E,function(){G.defer(E,E.prototype,{add:function(name,H){var I={};I[name]=H;qx.Class.__addProperties(E,I,true);}
});}
);}
;return E;}
,undefine:function(name){delete this.$$registry[name];var L=name.split(B);var K=[window];for(var i=0;i<L.length;i++ ){K.push(K[i][L[i]]);}
;for(var i=K.length-1;i>=1;i-- ){var J=K[i];var parent=K[i-1];if(qx.Bootstrap.isFunction(J)||qx.Bootstrap.objectGetLength(J)===0){delete parent[L[i-1]];}
else {break;}
;}
;}
,isDefined:qx.util.OOUtil.classIsDefined,getTotalNumber:function(){return qx.Bootstrap.objectGetLength(this.$$registry);}
,getByName:qx.Bootstrap.getByName,include:function(N,M){{}
;qx.Class.__addMixin(N,M,false);}
,patch:function(P,O){{}
;qx.Class.__addMixin(P,O,true);}
,isSubClassOf:function(R,Q){if(!R){return false;}
;if(R==Q){return true;}
;if(R.prototype instanceof Q){return true;}
;return false;}
,getPropertyDefinition:qx.util.OOUtil.getPropertyDefinition,getProperties:function(T){var S=[];while(T){if(T.$$properties){S.push.apply(S,Object.keys(T.$$properties));}
;T=T.superclass;}
;return S;}
,getByProperty:function(U,name){while(U){if(U.$$properties&&U.$$properties[name]){return U;}
;U=U.superclass;}
;return null;}
,hasProperty:qx.util.OOUtil.hasProperty,getEventType:qx.util.OOUtil.getEventType,supportsEvent:qx.util.OOUtil.supportsEvent,hasOwnMixin:function(W,V){return W.$$includes&&W.$$includes.indexOf(V)!==-1;}
,getByMixin:function(ba,Y){var X,i,l;while(ba){if(ba.$$includes){X=ba.$$flatIncludes;for(i=0,l=X.length;i<l;i++ ){if(X[i]===Y){return ba;}
;}
;}
;ba=ba.superclass;}
;return null;}
,getMixins:qx.util.OOUtil.getMixins,hasMixin:function(bc,bb){return !!this.getByMixin(bc,bb);}
,hasOwnInterface:function(be,bd){return be.$$implements&&be.$$implements.indexOf(bd)!==-1;}
,getByInterface:qx.util.OOUtil.getByInterface,getInterfaces:function(bg){var bf=[];while(bg){if(bg.$$implements){bf.push.apply(bf,bg.$$flatImplements);}
;bg=bg.superclass;}
;return bf;}
,hasInterface:qx.util.OOUtil.hasInterface,implementsInterface:function(bi,bh){var bj=bi.constructor;if(this.hasInterface(bj,bh)){return true;}
;if(qx.Interface.objectImplements(bi,bh)){return true;}
;if(qx.Interface.classImplements(bj,bh)){return true;}
;return false;}
,getInstance:function(){if(!this.$$instance){this.$$allowconstruct=true;this.$$instance=new this();delete this.$$allowconstruct;}
;return this.$$instance;}
,getSubclasses:function(bk){if(!bk){return null;}
;var bl={};var bm=qx.Class.$$registry;for(var name in bm){if(bm[name].superclass&&bm[name].superclass==bk){bl[name]=bm[name];}
;}
;return bl;}
,genericToString:function(){return y+this.classname+u;}
,$$registry:qx.Bootstrap.$$registry,__allowedKeys:null,__staticAllowedKeys:null,__validateConfig:function(name,bn){}
,__validateAbstractInterfaces:function(bo){}
,__createClass:function(name,bz,by,bp,bw,bt,bs){var bv=function(){return (typeof this==j);}
;var bu;if(!by&&qx.core.Environment.get(m)==false){bu=bp||{};qx.Bootstrap.setDisplayNames(bu,name);}
else {bu={};if(by){if(!bw){bw=this.__createDefaultConstructor();}
;if(this.__needsConstructorWrapper(by,bs)){bu=this.__wrapConstructor(bw,name,bz);}
else {bu=bw;}
;if(bz===h){bu.getInstance=this.getInstance;}
;qx.Bootstrap.setDisplayName(bw,name,d);}
;if(bp){qx.Bootstrap.setDisplayNames(bp,name);var bx;for(var i=0,a=Object.keys(bp),l=a.length;i<l;i++ ){bx=a[i];var bq=bp[bx];if(qx.core.Environment.get(m)){if(bq instanceof Function){bq=qx.core.Aspect.wrap(name+B+bx,bq,q);}
;bu[bx]=bq;}
else {bu[bx]=bq;}
;}
;}
;}
;var br=name?qx.Bootstrap.createNamespace(name,bu):s;bu.classname=name;if(!bv()){try{bu.name=name;}
catch(bA){}
;}
;bu.basename=br;bu.$$type=v;if(bz){bu.$$classtype=bz;}
;if(!bu.hasOwnProperty(n)){bu.toString=this.genericToString;}
;if(by){qx.Bootstrap.extendClass(bu,bw,by,name,br);if(bt){if(qx.core.Environment.get(m)){bt=qx.core.Aspect.wrap(name,bt,z);}
;bu.$$destructor=bt;qx.Bootstrap.setDisplayName(bt,name,A);}
;}
;this.$$registry[name]=bu;return bu;}
,__addEvents:function(bB,bC,bE){{var bD,bD;}
;if(bB.$$events){for(var bD in bC){bB.$$events[bD]=bC[bD];}
;}
else {bB.$$events=bC;}
;}
,__addProperties:function(bG,bJ,bH){if(!qx.core.Environment.get(g)){throw new Error(e);}
;var bI;if(bH===undefined){bH=false;}
;var bF=bG.prototype;for(var name in bJ){bI=bJ[name];{}
;bI.name=name;if(!bI.refine){if(bG.$$properties===undefined){bG.$$properties={};}
;bG.$$properties[name]=bI;}
;if(bI.init!==undefined){bG.prototype[c+name]=bI.init;}
;if(bI.event!==undefined){if(!qx.core.Environment.get(k)){throw new Error(t);}
;var event={};event[bI.event]=r;this.__addEvents(bG,event,bH);}
;if(bI.inheritable){this.__Property.$$inheritable[name]=true;if(!bF.$$refreshInheritables){this.__Property.attachRefreshInheritables(bG);}
;}
;if(!bI.refine){this.__Property.attachMethods(bG,name,bI);}
;}
;}
,__validateProperty:null,__addMembers:function(bR,bK,bM,bO,bQ){var bL=bR.prototype;var bP,bN;qx.Bootstrap.setDisplayNames(bK,bR.classname+b);for(var i=0,a=Object.keys(bK),l=a.length;i<l;i++ ){bP=a[i];bN=bK[bP];{}
;if(bL[bP]!=undefined&&bL[bP].$$install){bL[bP].$$install();}
;if(bO!==false&&bN instanceof Function&&bN.$$type==null){if(bQ==true){bN=this.__mixinMemberWrapper(bN,bL[bP]);}
else {if(bL[bP]){bN.base=bL[bP];}
;bN.self=bR;}
;if(qx.core.Environment.get(m)){bN=qx.core.Aspect.wrap(bR.classname+B+bP,bN,C);}
;}
;bL[bP]=bN;}
;}
,__mixinMemberWrapper:function(bS,bT){if(bT){return function(){var bV=bS.base;bS.base=bT;var bU=bS.apply(this,arguments);bS.base=bV;return bU;}
;}
else {return bS;}
;}
,__addInterface:function(bY,bW){{}
;var bX=qx.Interface.flatten([bW]);if(bY.$$implements){bY.$$implements.push(bW);bY.$$flatImplements.push.apply(bY.$$flatImplements,bX);}
else {bY.$$implements=[bW];bY.$$flatImplements=bX;}
;}
,__retrospectWrapConstruct:function(cb){var name=cb.classname;var ca=this.__wrapConstructor(cb,name,cb.$$classtype);for(var i=0,a=Object.keys(cb),l=a.length;i<l;i++ ){cc=a[i];ca[cc]=cb[cc];}
;ca.prototype=cb.prototype;var ce=cb.prototype;for(var i=0,a=Object.keys(ce),l=a.length;i<l;i++ ){cc=a[i];var cf=ce[cc];if(cf&&cf.self==cb){cf.self=ca;}
;}
;for(var cc in this.$$registry){var cd=this.$$registry[cc];if(!cd){continue;}
;if(cd.base==cb){cd.base=ca;}
;if(cd.superclass==cb){cd.superclass=ca;}
;if(cd.$$original){if(cd.$$original.base==cb){cd.$$original.base=ca;}
;if(cd.$$original.superclass==cb){cd.$$original.superclass=ca;}
;}
;}
;qx.Bootstrap.createNamespace(name,ca);this.$$registry[name]=ca;return ca;}
,__addMixin:function(cl,cj,ci){{}
;if(this.hasMixin(cl,cj)){return;}
;var cg=cl.$$original;if(cj.$$constructor&&!cg){cl=this.__retrospectWrapConstruct(cl);}
;var ch=qx.Mixin.flatten([cj]);var ck;for(var i=0,l=ch.length;i<l;i++ ){ck=ch[i];if(ck.$$events){this.__addEvents(cl,ck.$$events,ci);}
;if(ck.$$properties){this.__addProperties(cl,ck.$$properties,ci);}
;if(ck.$$members){this.__addMembers(cl,ck.$$members,ci,ci,ci);}
;}
;if(cl.$$includes){cl.$$includes.push(cj);cl.$$flatIncludes.push.apply(cl.$$flatIncludes,ch);}
else {cl.$$includes=[cj];cl.$$flatIncludes=ch;}
;}
,__createDefaultConstructor:function(){function cm(){cm.base.apply(this,arguments);}
;return cm;}
,__needsConstructorWrapper:function(co,cn){{}
;if(co&&co.$$includes){var cp=co.$$flatIncludes;for(var i=0,l=cp.length;i<l;i++ ){if(cp[i].$$constructor){return true;}
;}
;}
;if(cn){var cq=qx.Mixin.flatten(cn);for(var i=0,l=cq.length;i<l;i++ ){if(cq[i].$$constructor){return true;}
;}
;}
;return false;}
,__wrapConstructor:function(cs,name,cr){var cu=function(){var cx=cu;{}
;var cv=cx.$$original.apply(this,arguments);if(cx.$$includes){var cw=cx.$$flatIncludes;for(var i=0,l=cw.length;i<l;i++ ){if(cw[i].$$constructor){cw[i].$$constructor.apply(this,arguments);}
;}
;}
;{}
;return cv;}
;if(qx.core.Environment.get(m)){var ct=qx.core.Aspect.wrap(name,cu,d);cu.$$original=cs;cu.constructor=ct;cu=ct;}
;cu.$$original=cs;cs.wrapper=cu;return cu;}
},defer:function(){if(qx.core.Environment.get(m)){for(var cy in qx.Bootstrap.$$registry){var cz=qx.Bootstrap.$$registry[cy];for(var cA in cz){if(cz[cA] instanceof Function){cz[cA]=qx.core.Aspect.wrap(cy+B+cA,cz[cA],q);}
;}
;}
;}
;}
...
,showAllBindingsInLog:function(){for(var dR in this.__bindings){var dQ=qx.core.ObjectRegistry.fromHashCode(dR);for(var i=0;i<this.__bindings[dR].length;i++ ){this.showBindingInLog(dQ,this.__bindings[dR][i][0]);}
;}
;}
}});}
)();
...
(function(){var c="-",d="",e="qx.core.ObjectRegistry",f="Disposed ",g="$$hash",h="-0",j=" objects",k="Could not dispose object ",m=": ";qx.Bootstrap.define(e,{statics:{inShutDown:false,__registry:{},__nextHash:0,__freeHashes:[],__postId:d,__stackTraces:{},register:function(n){var q=this.__registry;if(!q){return;}
;var p=n.$$hash;if(p==null){var o=this.__freeHashes;if(o.length>0&&true){p=o.pop();}
else {p=(this.__nextHash++ )+this.__postId;}
;n.$$hash=p;{}
;}
;{}
;q[p]=n;}
,unregister:function(r){var s=r.$$hash;if(s==null){return;}
;var t=this.__registry;if(t&&t[s]){delete t[s];this.__freeHashes.push(s);}
;try{delete r.$$hash;}
...
;var w=this.__freeHashes;if(w.length>0){x=w.pop();}
else {x=(this.__nextHash++ )+this.__postId;}
;return v.$$hash=x;}
,clearHashCode:function(y){{}
;var z=y.$$hash;if(z!=null){this.__freeHashes.push(z);try{delete y.$$hash;}
catch(A){if(y.removeAttribute){y.removeAttribute(g);}
;}
;}
;}
,fromHashCode:function(B){return this.__registry[B]||null;}
,shutdown:function(){this.inShutDown=true;var D=this.__registry;var F=[];for(var C in D){F.push(C);}
...
;}
;break;}
;qx.Bootstrap.debug(this,f+l+j);delete this.__registry;}
...
},defer:function(H){if(window&&window.top){var frames=window.top.frames;for(var i=0;i<frames.length;i++ ){if(frames[i]===window){H.__postId=c+(i+1);return;}
;}
;}
;H.__postId=h;}
});}
...
(function(){var a="__dispatchers",b="UNKNOWN_",c="|bubble",d="",e="_",f="c",g="|",h="unload",j="|capture",k="DOM_",m="__handlers",n="WIN_",o="QX_",p="qx.event.Manager",q="capture",r="DOCUMENT_";qx.Class.define(p,{extend:Object,construct:function(s,u){this.__window=s;this.__windowId=qx.core.ObjectRegistry.toHashCode(s);this.__registration=u;if(s.qx!==qx){var self=this;var t=function(){qx.bom.Event.removeNativeListener(s,h,arguments.callee);self.dispose();}
...
},destruct:function(){this.__exc=null;this.__arguments=null;this.__failMessage=null;}
});}
)();
...
(function(){var a="qx.util.ObjectPool",b="Class needs to be defined!",c="Object is already pooled: ",d="Integer";qx.Class.define(a,{extend:qx.core.Object,construct:function(e){qx.core.Object.call(this);this.__pool={};if(e!=null){this.setSize(e);}
;}
...
(function(){var a="qx.util.DeferredCallManager",b="singleton";qx.Class.define(a,{extend:qx.core.Object,type:b,construct:function(){this.__calls={};this.__timeoutWrapper=qx.lang.Function.bind(this.__timeout,this);this.__hasCalls=false;}
