...
else {var Y=this.__computeBody(bd);var bc=this.__computeOffset(bd);var scroll=this.__computeScroll(bd);var bh=bc.left+Y.left-scroll.left;var top=bc.top+Y.top-scroll.top;}
;var ba=bh+bd.offsetWidth;var bb=top+bd.offsetHeight;if(be){if(be==p||be==b){var bg=qx.bom.element.Style.get(bd,z);if(bg==b||bg==g){ba+=bd.scrollWidth-bd.offsetWidth+this.__num(bd,j)+this.__num(bd,f);}
;var bf=qx.bom.element.Style.get(bd,m);if(bf==b||bf==g){bb+=bd.scrollHeight-bd.offsetHeight+this.__num(bd,i)+this.__num(bd,a);}
;}
;switch(be){case p:bh+=this.__num(bd,e);top+=this.__num(bd,r);ba-=this.__num(bd,x);bb-=this.__num(bd,q);case b:bh-=bd.scrollLeft;top-=bd.scrollTop;ba-=bd.scrollLeft;bb-=bd.scrollTop;case A:bh+=this.__num(bd,j);top+=this.__num(bd,i);ba-=this.__num(bd,f);bb-=this.__num(bd,a);break;case w:bh-=this.__num(bd,n);top-=this.__num(bd,l);ba+=this.__num(bd,s);bb+=this.__num(bd,k);break;};}
;return {left:bh,top:top,right:ba,bottom:bb};}
,__getBodyLocation:function(bi){var top=bi.offsetTop;var bj=bi.offsetLeft;top+=this.__num(bi,l);bj+=this.__num(bi,n);if(qx.core.Environment.get(u)===d){top+=this.__num(bi,j);bj+=this.__num(bi,i);}
