...
(function(){var a=". Error message: ",b="Boolean",c=").",d="set",f="deepBinding",g=") to the object '",h="item",k="Please use only one array at a time: ",l="Integer",m="reset",n=" of object ",p="qx.data.SingleValueBinding",q="Binding property ",r="Failed so set value ",s="change",t="Binding could not be found!",u="get",v="^",w=" does not work.",x="String",y="Binding from '",z="",A="PositiveNumber",B="]",C="[",D=".",E="PositiveInteger",F='No number or \'last\' value hast been given in an array binding: ',G="' (",H=" on ",I="Binding does not exist!",J="Number",K=".[",L="Date",M=" not possible: No event available. ",N="last";qx.Class.define(p,{statics:{__bindings:{},__bindingsByTarget:{},bind:function(bb,U,bg,bd,bj){{}
;var bf=this.__setUpTargetBinding(bb,U,bg,bd,bj);var Q=U.split(D);var P=this.__checkForArrayInPropertyChain(Q);var S=[];var bh=[];var R=[];var T=[];var Y=bb;try{for(var i=0;i<Q.length;i++ ){if(P[i]!==z){T.push(s);}
...
;T.push(X);}
;S[i]=Y;if(i==Q.length-1){if(P[i]!==z){var ba=P[i]===N?Y.length-1:P[i];var O=Y.getItem(ba);this.__setInitialValue(O,bg,bd,bj,bb);R[i]=this.__bindEventToProperty(Y,T[i],bg,bd,bj,P[i]);}
else {if(Q[i]!=null&&Y[u+qx.lang.String.firstUp(Q[i])]!=null){var O=Y[u+qx.lang.String.firstUp(Q[i])]();this.__setInitialValue(O,bg,bd,bj,bb);}
...
else {if(br.propertyNames[j]!=null&&bp[u+qx.lang.String.firstUp(br.propertyNames[j])]!=null){var bm=bp[u+qx.lang.String.firstUp(br.propertyNames[j])]();this.__setInitialValue(bm,br.targetObject,br.targetPropertyChain,br.options,br.sources[br.index]);}
...
;}
;}
,__setUpTargetBinding:function(bw,bE,bI,bA,bC){var bz=bA.split(D);var bx=this.__checkForArrayInPropertyChain(bz);var bH=[];var bG=[];var bB=[];var bF=[];var by=bI;for(var i=0;i<bz.length-1;i++ ){if(bx[i]!==z){bF.push(s);}
...
;return null;}
,__getPropertyChainArray:function(cp){return cp.replace(/\[/g,K).split(D).filter(function(cq){return cq!==z;}
);}
...
;}
;}
,__checkForArrayInPropertyChain:function(cD){var cE=[];for(var i=0;i<cD.length;i++ ){var name=cD[i];if(name.endsWith(B)){var cF=name.substring(name.indexOf(C)+1,name.indexOf(B));if(name.indexOf(B)!=name.length-1){throw new Error(k+name+w);}
...
else {var dg=this.__getPropertyChainArray(de);var dd=this.__getTargetFromChain(dl,dg);var dm=de.substring(de.lastIndexOf(D)+1,de.length);if(dd==null){return df;}
;var dj=qx.Class.getPropertyDefinition(dd.constructor,dm);var dk=dj==null?z:dj.check;return this.__defaultConversion(df,dk);}
;}
,__getEventForProperty:function(dn,dq){var dp=qx.Class.getPropertyDefinition(dn.constructor,dq);if(dp==null){return null;}
;return dp.event;}
,__defaultConversion:function(dt,ds){var dr=qx.lang.Type.getClass(dt);if((dr==J||dr==x)&&(ds==l||ds==E)){dt=parseInt(dt,10);}
