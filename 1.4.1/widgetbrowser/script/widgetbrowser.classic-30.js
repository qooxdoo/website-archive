qx.$$packageData['fdb35ce4797f']={"locales":{},"resources":{"qx/static/blank.gif":[1,1,"gif","qx"],"widgetbrowser/blank.html":"widgetbrowser"},"translations":{}};

qx.Part.$$notifyLoad("fdb35ce4797f", function() {
...
this.setSource(i);
this.addListener(b,this.__JI,this);
...
this.setSource(null);
this.setSource(s);
}},__JI:function(e){var u=e.getData();
...
}this._getIframeElement().addListener(d,this.__JD,this);
...
},__JD:function(e){var p=e.getData();

if(p){this.setSource(p);
}this.fireDataEvent("navigate",p);
}}});
})();
(function(){var k="mousedown",j="load",i="help",h="engine.name",g="mouseup",f="losecapture",d="contextmenu",c="none",b="display",a="no",z="Boolean",y="px",x="gecko",w="auto",v="_applyScrollbar",u="DOMNodeInserted",t="_applyNativeHelp",s="yes",r="scrolling",q="/",o="appear",p="block",m="qx.ui.embed.Iframe",n="iframe",l="__JH";
qx.Class.define(m,{extend:qx.ui.embed.AbstractIframe,construct:function(A){if(A!=null){this.__JG=A;
...
this.__JH=this._createBlockerElement();
this.getContainerElement().add(this.__JH);
...
},block:function(){this.__JH.setStyle(b,p);
},release:function(){this.__JH.setStyle(b,c);
...
},properties:{appearance:{refine:true,init:i},scrollbarX:{check:[t,p,d],init:t,themeable:true,apply:l},scrollbarY:{check:[t,p,d],init:t,themeable:true,apply:l},scrollbar:{group:[g,f]}},members:{__JE:null,__JF:null,_getIframeElement:function(){return this.getChildControl(r).getContentElement();
...
e.stop();
},_startIframeObserver:function(){if(this.__JF){this._stopIframeObserver();
}var F=qx.event.Idle.getInstance();
this.__JF=F.addListener("interval",this._onIframeObserverInterval,this);
},_stopIframeObserver:function(){this.__JE=null;

if(!this.__JF){return;
}var G=qx.event.Idle.getInstance();
G.removeListenerById(this.__JF);
},_onIframeObserverInterval:function(){var H=this._getIframeSize();

if(!H){this._disableScollbars();
return;
}
if(this.__JE&&H.width==this.__JE.width&&H.height==this.__JE.height){return;
}this.__JE=H;
...
}},_updateScrollbars:function(){var K=this.__JE;
