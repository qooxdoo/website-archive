...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.Flickr","qx.theme":"qx.theme.Modern"};
...
N.setLabelPath(p);
N.setDelegate({configureItem:function(Q){Q.setShow(q);
}});
var J={converter:function(R,S){return (d+S.getFarm()+i+S.getServer()+j+R+f+S.getSecret()+B);
}};
N.setIconOptions(J);
N.setIconPath(v);
P.bind(a,N,u);
...
var O={converter:function(T){if(T){return (d+T.getFarm()+i+T.getServer()+j+T.getId()+f+T.getSecret()+G);
}return g;
}};
N.bind(C,K,D,O);
...
qx.Class.define(r,{extend:qx.data.store.Json,statics:{RESULT:null},construct:function(U){jsonFlickrApi=function(W){demobrowser.demo.data.store.Flickr.RESULT=W;
};
var V=h+U;
qx.data.store.Json.call(this,V);
},members:{searchForTag:function(X){if(X!=g){this.setUrl(h+X);
}},_createRequest:function(Y){var ba=new qx.io.ScriptLoader();
this.setState(m);
Y+=w;
ba.load(Y,function(bb){this.__Ti();
},this);
},__Ti:function(){this.setState(o);
