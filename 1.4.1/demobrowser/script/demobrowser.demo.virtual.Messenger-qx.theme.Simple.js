...
this.__MH=new qx.ui.virtual.layer.WidgetCell(B);
this.getPane().addLayer(this.__MH);

if(!this.__MF){this.setCellRenderer(qx.ui.virtual.form.ListItemCell.getInstance());
}},_initHtmlLayer:function(){var self=this;
var K={getCellProperties:function(L,M){var N={};

if(self.__MG.isItemSelected(L)){N.selected=true;
}return self._getCellRenderer(L).getCellProperties(self._getCellData(L),N);
}};
this._showChildControl(l);
this.__MH=new qx.ui.virtual.layer.HtmlCell(K);
this.getPane().addLayer(this.__MH);

if(!this.__MF){this.setCellRenderer(new qx.ui.virtual.cell.Cell());
...
},styleSelectable:function(Q,R,S){if(self.__MI){self._styleWidgetSelectable(Q,R,S);
}else{self._styleHtmlSelectable(Q,R,S);
}}};
this.__MG=new qx.ui.virtual.selection.Row(this.getPane(),O);
this.__MG.attachMouseEvents(this.getPane());
this.__MG.attachKeyEvents(this);
},getSelectionManager:function(){return this.__MG;
...
},update:function(){if(this.__MH){this.__MH.updateLayerData();
...
},_applyUseWidgetCells:function(bf,bg){if(this.__MI!==null){throw new Error("The property 'useWidgetCells' cannot be set after the list has "+"been rendered.");
}},_applyCellRenderer:function(bh,bi){this.__MF=bh;

if(this.__MH){this.__MH.fullUpdate();
}},_getCellData:function(bj){return this._delegate.getCellData?this._delegate.getCellData(bj):null;
},_getCellRenderer:function(bk){return this._delegate.getCellRenderer?this._delegate.getCellRenderer(bk):this.__MF;
...
}}},syncWidget:function(){if(this.__MI!==null){return;
}this.__MI=this.getUseWidgetCells();

if(this.__MI){this._initWidgetLayer();
}else{this._initHtmlLayer();
}},_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);
}},destruct:function(){this._delegate=this.__MF=null;
this._disposeObjects(m,a);
}});
})();
