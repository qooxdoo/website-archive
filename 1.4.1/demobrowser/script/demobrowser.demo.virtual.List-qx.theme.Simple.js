...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.virtual.List","qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.virtual.List-qx.theme.Simple.js"]],
  urisBefore : [],
  packageHashes : {"0":"84e8775da7c9"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var bt="value",bs="label",br="icon",bq="one",bp="multi",bo="additive",bn="bold",bm="both",bl="single",bk="changeValue",Y="changeSelection",X="Item No ",W="Julian",V="Show Icon",U="Kevin",T="demobrowser.demo.virtual.List",S="Marvin",R="Marc",Q="Configurable",P="icon/16/places/folder.png",bA="Jason",bB="show",by="Jakob",bz="Lukas",bw="Julius",bx="Jan",bu="Tobias",bv="Mika",bC="Nils",bD="Noah",bd="Tom",bc="Nico",bf="Multi Selection",be="Felix",bh="Leon",bg="Enable drag selection",bj="Show Both",bi="48",bb="changeShowMode",ba="Fabian",a="Joel",b="Dominic",c="Show Label",d="Enable quick selection",f="Finn",g="One as selection mode",h="Jannis",j="Alexander",k="Additive Selection",l="change",bH="Erik",bG="addaptive",bF="Johannes",bE="16",bL="Jonas",bK="icon/",bJ="David",bI="Maximilian",bN="enabled",bM="Tim",A="Justin",B="Single Selection",y="Linus",z="Daniel",E="Paul",F="Philipp",C="Niclas",D="One Selection",w="Nick",x="Jonathan",t="Ben",s="on",v="showMode",u="Vincent",p="/places/folder.png",o="Robin",r="Jannick",q="Luis",n="Lennard",m="Moritz",K="Florian",L="",M="Marlon",N="Luca",G="Simon",H="Colin",I="Max",J="Additive selection",O="Elias";
qx.Class.define(T,{extend:qx.application.Standalone,properties:{showMode:{init:bm,check:[bs,br,bm],event:bb}},members:{__Vu:null,__Vv:null,__Vw:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var bO=this.getRoot();
bO.add(this.createConfigurableList(),{left:20,top:20});
bO.add(this.createOneSelectionList(),{left:330,top:20});
bO.add(this.createAdditiveSelectionList(),{left:520,top:20});
},createConfigurableList:function(){var bR=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
var bT=new qx.ui.basic.Label(Q).set({font:bn});
bR.add(bT);
var cc=[];

for(var i=0;i<2500;i++){cc[i]={label:X+i,icon:(i%4)?bE:bi};
}var cf=qx.data.marshal.Json.createModel(cc);
var bW=this.__Vu=new qx.ui.list.List(cf).set({scrollbarX:s,selectionMode:bp,height:280,width:150,labelPath:bs,iconPath:br,iconOptions:{converter:function(cj){return bK+cj+p;
}}});
bR.add(bW,{top:20});
var bV=this;
var ci={configureItem:function(ck){bV.bind(v,ck,bB);
},bindItem:function(cl,cm,cn){cl.bindDefaultProperties(cm,cn);
cl.bindProperty(bs,bN,{converter:function(co){return parseInt(co.replace(/Item No /g,L))%9!=0;
}},cm,cn);
}};
bW.setDelegate(ci);
bW.getSelection().push(cf.getItem(20));
bW.getSelection().addListener(l,function(e){var cp=bW.getSelection();

for(var i=0;i<cp.getLength();i++){this.debug("Selection: "+cp.getItem(i).getLabel());
}},this);
var ca=bW.getPane().getRowConfig();

for(var i=0;i<2500;i++){if(i%4==0){ca.setItemSize(i,56);
}else{ca.setItemSize(i,25);
}}var cb=new qx.ui.form.RadioButton(B);
var bQ=new qx.ui.form.RadioButton(bf);
var ce=new qx.ui.form.RadioButton(k);
var cg=new qx.ui.form.RadioButton(D);
cb.setUserData(bt,bl);
bQ.setUserData(bt,bp);
ce.setUserData(bt,bo);
cg.setUserData(bt,bq);
bR.add(cb,{left:160,top:20});
bR.add(bQ,{left:160,top:40});
bR.add(ce,{left:160,top:60});
bR.add(cg,{left:160,top:80});
var bX=new qx.ui.form.RadioButton(c);
var bS=new qx.ui.form.RadioButton(V);
var bP=new qx.ui.form.RadioButton(bj);
bX.setUserData(bt,bs);
bS.setUserData(bt,br);
bP.setUserData(bt,bm);
bR.add(bX,{left:160,top:120});
bR.add(bS,{left:160,top:140});
bR.add(bP,{left:160,top:160});
var bY=this.__Vv=new qx.ui.form.CheckBox(bg);
var cd=this.__Vw=new qx.ui.form.CheckBox(d).set({enabled:false});
bR.add(bY,{left:160,top:200});
bR.add(cd,{left:160,top:220});
var ch=new qx.ui.form.RadioGroup(cb,bQ,ce,cg);
ch.setSelection([bQ]);
ch.addListener(Y,this.onSelectionModeChange,this);
var bU=new qx.ui.form.RadioGroup(bX,bS,bP);
bU.setSelection([bP]);
bU.addListener(Y,this.onShowModeChange,this);
bY.addListener(bk,this.onDragCheckChange,this);
cd.addListener(bk,this.onQuickCheckChange,this);
return bR;
},createOneSelectionList:function(){var cq=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
var cr=new qx.ui.basic.Label(g).set({font:bn});
cq.add(cr);
var cu=new qx.data.Array();

for(var i=0;i<250;i++){var ct=new demobrowser.demo.virtual.model.Item(X+i,P);
cu.push(ct);
}var cs=new qx.ui.list.List(cu).set({selectionMode:bq,height:280,width:150,labelPath:bs,iconPath:br});
cq.add(cs,{top:20});
cs.getSelection().push(cu.getItem(16));
return cq;
},createAdditiveSelectionList:function(){var cw=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
var cx=new qx.ui.basic.Label(J).set({font:bn});
cw.add(cx);
var cy=[bh,bz,N,f,bM,be,bL,q,bI,W,I,E,C,bx,t,O,r,F,bD,bd,m,bc,bJ,bC,G,ba,bH,A,j,by,K,w,y,bv,bA,z,n,S,h,bu,b,M,R,bF,x,bw,H,a,U,u,o];
var cB=new qx.data.Array();
var cv=new qx.data.Array();

for(var i=0;i<cy.length;i++){var cA=new demobrowser.demo.virtual.model.Item(cy[i]);
cB.push(cA);

if(i==10||i==12||i==16){cv.push(cA);
}}var cz=new qx.ui.list.List(cB).set({selectionMode:bo,height:200,width:150,labelPath:bs,iconPath:br});
cw.add(cz,{top:20});
cz.setSelection(cv);
return cw;
},onSelectionModeChange:function(e){var cC=e.getData()[0].getUserData(bt);
this.__Vu.setSelectionMode(cC);

if(cC==bl||cC==bq){this.__Vv.setEnabled(false);
this.__Vw.setEnabled(true);
}else if(cC==bp||cC==bG){this.__Vv.setEnabled(true);
this.__Vw.setEnabled(false);
}},onShowModeChange:function(e){var cD=e.getData()[0].getUserData(bt);
this.setShowMode(cD);
},onDragCheckChange:function(e){if(e.getData()){var cE=this.__Vu.getSelectionMode();

if(cE==bl||cE==bq){this.debug("Drag selection is only available for the modes multi or additive");
}}this.__Vu.setDragSelection(e.getData());
},onQuickCheckChange:function(e){if(e.getData()){var cF=this.__Vu.getSelectionMode();

if(cF==bp||cF==bo){this.debug("Quick selection is only available for the modes multi or additive");
}}this.__Vu.setQuickSelection(e.getData());
}}});
})();
