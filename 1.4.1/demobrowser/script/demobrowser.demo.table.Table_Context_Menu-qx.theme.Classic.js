...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Context_Menu","qx.theme":"qx.theme.Classic"};
...
qx.Class.define(c,{extend:qx.application.Standalone,include:demobrowser.demo.table.MUtil,members:{__Ur:null,main:function(){qx.application.Standalone.prototype.main.call(this);
...
},showDialog:function(s){if(!this.__Ur){var v=this.__Ur=new qx.ui.window.Window().set({modal:true,showMinimize:false,showMaximize:false,width:180,contentPadding:[10,10,10,10]});
v.moveTo(315,100);
...
}this.__Ur.getChildren()[1].setValue(s);
this.__Ur.open();
this.__Ur.getChildren()[2].focus();
}},destruct:function(){this._disposeObjects(i,l,b);
}});
})();
(function(){var l="execute",k="changeValue",j="value",h="icon/22/actions/edit-undo.png",g="Change ID sort method",f="Disabled ctx menu",d="icon/22/status/dialog-information.png",c="On",b="Change row with ID 10",a="Default ctx menu #2",D="Remove 5 rows",C="Add 10 rows",B="Context-menu selectable",A="",z="_tableModel",y="Table",x="icon/22/actions/list-remove.png",w="Whether the the first visible row should ",v=", ",u="demobrowser.demo.table.Table_Context_Menu",s="Default ctx menu #1",t="icon/22/actions/list-add.png",q="Selected rows:<br>",r="Keep first row",o="Off",p="be rendered completely when scrolling.",m="Show selection",n="Demonstrate use of alternate sorting algorithm.";
qx.Class.define(u,{extend:demobrowser.demo.table.TableDemo,members:{main:function(){qx.Class.include(qx.ui.table.Table,qx.ui.table.MTableContextMenu);
demobrowser.demo.table.TableDemo.prototype.main.call(this);
this.getRoot().setNativeContextMenu(true);
},getCaption:function(){return y;
},createTable:function(){var G=this.createRandomRows(50);
var F=this._tableModel=new qx.ui.table.model.Simple();
F.setColumns([s,a,f,B]);
F.setData(G);
var H=new qx.ui.table.Table(F);
H.set({width:600,height:400,decorator:null});
var E=H.getTableColumnModel();
E.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
H.setContextMenuHandler(3,this._contextMenuHandlerBoolean);
H.setContextMenuHandler(2,this._contextMenuHandlerNull);
return H;
},_contextMenuHandlerBoolean:function(I,J,K,L,M){var N;

for(var i=0;i<=1;i++){N=new qx.ui.menu.Button(i?c:o);
N.setUserData(j,i?true:false);
N.addListener(l,function(e){L.setValue(I,J,this.getUserData(j));
});
M.add(N);
}return true;
},_contextMenuHandlerNull:function(O,P,Q,R,S){return true;
},createControls:function(){var W=new qx.ui.toolbar.ToolBar();
var U,V,X;
V=new qx.ui.toolbar.Part();
W.add(V);
U=new qx.ui.toolbar.Button(b,h);
U.addListener(l,function(Y){var ba=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,ba[0][i]);
}this.info("Row 10 changed");
},this);
V.add(U);
U=new qx.ui.toolbar.Button(C,t);
U.addListener(l,function(bb){var bc=this.createRandomRows(10);
this._tableModel.addRows(bc);
this.info("10 rows added");
},this);
V.add(U);
U=new qx.ui.toolbar.Button(D,x);
U.addListener(l,function(bd){var be=this._tableModel.getRowCount();
this._tableModel.removeRows(be-5,5);
this.info("5 rows removed");
},this);
V.add(U);
U=new qx.ui.toolbar.Button(m,d);
U.addListener(l,function(bf){var bg=[];
T.getSelectionModel().iterateSelection(function(bh){bg.push(bh+A);
});
this.showDialog(q+bg.join(v));
},this);
V.add(U);
V=new qx.ui.toolbar.Part();
W.add(V);
var T=this._table;
X=new qx.ui.toolbar.CheckBox(r);
X.set({value:T.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(w+p)});
X.addListener(k,function(bi){T.setKeepFirstVisibleRowComplete(this.getValue());
},X);
V.add(X);
X=new qx.ui.toolbar.CheckBox(g);
X.set({value:T.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(n)});
X.addListener(k,function(bj){if(bj.getData()){var bl=function(bm,bn){var bo=bm[arguments.callee.columnIndex];
var bp=bn[arguments.callee.columnIndex];

if(bo%2==1&&bp%2==0){return 1;
}
if(bp%2==1&&bo%2==0){return -1;
}return (bo>bp)?1:((bo==bp)?0:-1);
};
var bk=function(bq,br){var bs=bq[arguments.callee.columnIndex];
var bt=br[arguments.callee.columnIndex];

if(bs%2==1&&bt%2==0){return -1;
}
if(bt%2==1&&bs%2==0){return 1;
}return (bs<bt)?1:((bs==bt)?0:-1);
};
T.getTableModel().setSortMethods(0,{ascending:bl,descending:bk});
}else{T.getTableModel().setSortMethods(0,null);
}},X);
V.add(X);
return W;
...
for(var I=0;I<z;I++){this.__mP[I]={width:E,headerRenderer:F,dataRenderer:C,editorFactory:B};
this.__mL[I]=I;
this.__mM[I]=I;
}this.__mO=null;
this.__mN=true;
...
this.assertNotUndefined(this.__mP[J],h);
}var N=this.__mP[J].width;

if(N!=K){this.__mP[J].width=K;
...
this.assertNotUndefined(this.__mP[O],h);
}return this.__mP[O].width;
...
this.assertNotUndefined(this.__mP[P],h);
}var R=this.__mP[P].headerRenderer;

if(R!==this.__mQ){R.dispose();
}this.__mP[P].headerRenderer=Q;
this.fireDataEvent(q,{col:P});
},getHeaderCellRenderer:function(S){if(qx.core.Environment.get(k)){this.assertInteger(S,j);
this.assertNotUndefined(this.__mP[S],h);
}return this.__mP[S].headerRenderer;
...
this.assertNotUndefined(this.__mP[T],h);
}this.__mP[T].dataRenderer=U;
var V=this.__mP[T].dataRenderer;

if(V!==this.__mR){return V;
}return null;
},getDataCellRenderer:function(W){if(qx.core.Environment.get(k)){this.assertInteger(W,j);
this.assertNotUndefined(this.__mP[W],h);
}return this.__mP[W].dataRenderer;
...
this.assertNotUndefined(this.__mP[X],h);
}var ba=this.__mP[X].headerRenderer;

if(ba!==this.__mS){ba.dispose();
}this.__mP[X].editorFactory=Y;
},getCellEditorFactory:function(bb){if(qx.core.Environment.get(k)){this.assertInteger(bb,j);
this.assertNotUndefined(this.__mP[bb],h);
}return this.__mP[bb].editorFactory;
},_getColToXPosMap:function(){if(this.__mO==null){this.__mO={};

for(var be=0;be<this.__mL.length;be++){var bd=this.__mL[be];
this.__mO[bd]={overX:be};
}
for(var bc=0;bc<this.__mM.length;bc++){var bd=this.__mM[bc];
this.__mO[bd].visX=bc;
}}return this.__mO;
...
for(var x=bo+1;x<this.__mL.length;x++){var bq=this.__mL[x];
var bm=br[bq].visX;

if(bm!=null){bp=bm;
break;
}}if(bp==null){bp=this.__mM.length;
}this.__mM.splice(bp,0,bk);
}else{var bn=this.getVisibleX(bk);
this.__mM.splice(bn,1);
}this.__mO=null;
if(!this.__mN){var bs={col:bk,visible:bl};
...
}this.__mL.splice(bt,1);
this.__mL.splice(bu,0,bx);
this.__mO=null;
...
if(by.length==this.__mL.length){this.__mN=true;
var bB=new Array(by.length);

for(var bz=0;bz<this.__mL.length;bz++){var bA=this.isColumnVisible(bz);
bB[bz]=bA;

if(bA){this.setColumnVisible(bz,false);
}}this.__mL=qx.lang.Array.clone(by);
this.__mO=null;
for(var bz=0;bz<this.__mL.length;bz++){if(bB[bz]){this.setColumnVisible(bz,true);
}}this.__mN=false;
this.fireDataEvent(f);
}else{throw new Error("setColumnsOrder: Invalid number of column positions given, expected "+this.__mL.length+", got "+by.length);
...
this.__Mz=[];
this.__MA=[];
},members:{__Mz:null,__MA:null,setContextMenuHandler:function(m,n,o){this.__Mz[m]=n;
this.__MA[m]=o||this;
},getContextMenuHandler:function(p){return this.__Mz[p];
...
}},members:{__Jn:null,_getDefaultInsets:function(){return this._getDefaultInsetsForBorder();
},_isInitialized:function(){return !!this.__Jn;
},getMarkup:function(j){if(this.__Jn){return this.__Jn;
}var k={};
this._styleBorder(k);
return this.__Jn=this._generateMarkup(k);
