...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.DragDrop","qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.ui.DragDrop-qx.theme.Simple.js"]],
  urisBefore : [],
  packageHashes : {"0":"a8c1e5c4c3ce"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var m="drop",k="value",j="items",h="move",g="multi",f="dragover",d="icon/16/places/folder.png",c="dragstart",b="Item ",a="copy",A="drag",z="TextArea Target",y="solid",x="Reorderable",w="qx.ui.form.ListItem",v="droprequest",u="#33508D",t="Source",s="demobrowser.demo.ui.DragDrop",r="Simple Target",p="Enable drag",q="__Ve",n="\n",o="dragend";
qx.Class.define(s,{extend:qx.application.Standalone,members:{__Ve:null,__Vf:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var M=this.getRoot();
var G=new qx.ui.container.Scroll();
var B=new qx.ui.layout.Basic();
var E=new qx.ui.container.Composite(B).set({allowStretchY:false,allowStretchX:false});
G.add(E);
M.add(G,{edge:0});
var L=new qx.ui.basic.Label(t);
E.add(L,{left:20,top:20});
var J=new qx.ui.form.List;
J.setDraggable(true);
J.setSelectionMode(g);
E.add(J,{left:20,top:40});

for(var i=0;i<20;i++){J.add(new qx.ui.form.ListItem(b+i,d));
}var N=new qx.ui.form.CheckBox(p);
N.setValue(true);
E.add(N,{left:20,top:260});
J.addListener(c,function(e){if(!N.isValue()){e.preventDefault();
}e.addType(k);
e.addType(j);
e.addAction(a);
e.addAction(h);
});
J.addListener(v,function(e){this.debug("Related of droprequest: "+e.getRelatedTarget());
var Q=e.getCurrentAction();
var S=e.getCurrentType();
var P;

switch(S){case j:P=this.getSelection();

if(Q==a){var R=[];

for(var i=0,l=P.length;i<l;i++){R[i]=P[i].clone();
}P=R;
}break;
case k:P=this.getSelection()[0].getLabel();
break;
}if(Q==h){var O=this.getSelection();

for(var i=0,l=O.length;i<l;i++){this.remove(O[i]);
}}e.addData(S,P);
});
var H=new qx.ui.basic.Label(r);
E.add(H,{left:140,top:20});
var F=new qx.ui.form.List;
F.setDroppable(true);
F.setSelectionMode(g);
E.add(F,{left:140,top:40});
F.addListener(m,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
var T=e.getData(j);

for(var i=0,l=T.length;i<l;i++){this.add(T[i]);
}});
F.addListener(f,function(e){if(!e.supportsType(j)){e.preventDefault();
}});
var H=new qx.ui.basic.Label(z);
E.add(H,{left:260,top:20});
var K=new qx.ui.form.TextArea;
K.setDroppable(true);
K.setHeight(100);
E.add(K,{left:260,top:40});
K.addListener(m,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
this.setValue(e.getData(k).replace(/,/g,n));
});
K.addListener(f,function(e){if(!e.supportsType(k)){e.preventDefault();
}});
var I=new qx.ui.basic.Label(x);
E.add(I,{left:500,top:20});
var C=this.__Ve=new qx.ui.form.List;
C.setDraggable(true);
C.setDroppable(true);
C.setSelectionMode(g);
E.add(C,{left:500,top:40});

for(var i=0;i<20;i++){C.add(new qx.ui.form.ListItem(b+i,d));
}var D=new qx.ui.core.Widget;
D.setDecorator(new qx.ui.decoration.Single().set({top:[1,y,u]}));
D.setHeight(0);
D.setOpacity(0.5);
D.setZIndex(100);
D.setLayoutProperties({left:-1000,top:-1000});
D.setDroppable(true);
this.getRoot().add(D);
C.addListener(c,function(e){e.addAction(h);
});
C.addListener(o,function(e){D.setDomPosition(-1000,-1000);
});
C.addListener(A,function(e){var V=e.getOriginalTarget();
if(V instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__Vf=V;
}
if(!qx.ui.core.Widget.contains(this,V)&&V!=D){return;
}var U=V.getContainerLocation();
D.setWidth(V.getBounds().width);
D.setDomPosition(U.left,U.top);
});
C.addListener(f,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
C.addListener(m,function(e){this.__Vg(e.getOriginalTarget());
},this);
D.addListener(m,function(e){this.__Vg(this.__Vf);
},this);
},__Vg:function(W){if(W.classname!=w){return;
}var X=this.__Ve.getSortedSelection();

for(var i=0,l=X.length;i<l;i++){this.__Ve.addBefore(X[i],W);
this.__Ve.addToSelection(X[i]);
