...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.FiniteStateMachine","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.ui.FiniteStateMachine-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"2bca4ddc7d14"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var v="<p>",u="demobrowser/demo/ui/FsmMiceMaze.png",t="FadingOut, and Zombie (about to dispose).",s="which it was instantiated); when it ",r="Each mouse is driven by (and in fact, in this example, is a ",q="The initial mouse is a bit special, in that it looks for ",p="This is a simple example of using a finite state machine, in ",n="behind it, as well as to its left and right.",m="qx.debug",l="Each mouse is quite dumb, having in ",e="arrives at a new cell, it looks left and right and creates a ",k="<u>The finite state diagram is below the maze.</u>",h="its arsenal of knowledge only these characteristics: it knows ",c="subclass of) a finite state machine.  The FSM has five states: ",b="clone of itself to go each available direction; upon hitting a ",g="only how to travel in a single direction (the direction in ",f="demobrowser.demo.ui.FiniteStateMachine",i="wall, it dies.",a="FadingIn, CellArrival (look for cloning opportunities), Moving, ",j="which virtual mice solve a maze. ",d="the possibility to clone a mouse to travel in the direction ";
qx.Class.define(f,{extend:qx.application.Standalone,construct:function(){qx.application.Standalone.call(this);
},members:{main:function(){qx.application.Standalone.prototype.main.call(this);
if((qx.core.Environment.get(m))){qx.log.appender.Native;
qx.log.appender.Console;
}var x=p+j+v+l+h+g+s+e+b+i+v+q+d+n+v+r+c+a+t+v+k;
var scroll=new qx.ui.container.Scroll();
this.getRoot().add(scroll,{edge:0});
var w=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
scroll.add(w);
var o;
o=new qx.ui.basic.Image(u);
o=null;
o=new qx.ui.basic.Label(x);
o.set({width:600,rich:true,wrap:true});
w.add(o,{left:50});
var A=new demobrowser.demo.util.FSMMaze(10,10,50,240);
w.add(A,{left:50,top:240});
o=new qx.ui.basic.Image(u,800);
w.add(o,{left:50,top:800});
var z;
var y=Math.floor(Math.random()*4);

switch(y){case 0:z=demobrowser.demo.util.FSMMaze.Direction.NORTH;
break;
case 1:z=demobrowser.demo.util.FSMMaze.Direction.EAST;
break;
case 2:z=demobrowser.demo.util.FSMMaze.Direction.SOUTH;
break;
case 3:z=demobrowser.demo.util.FSMMaze.Direction.WEST;
break;
}new demobrowser.demo.util.FSMMouse(null,A,z,w);
...
(function(){var g="black",f="solid",e="#b0ffb0",d="&bull;",c="demobrowser.demo.util.FSMMaze",b="#ffb0b0",a="html";
qx.Class.define(c,{extend:qx.ui.container.Composite,statics:{Direction:{WEST:0x8,SOUTH:0x4,EAST:0x2,NORTH:0x1}},construct:function(h,i,x,y,j){qx.ui.container.Composite.call(this);
this.setLayout(new qx.ui.layout.Grid());
this.numRows=(h===undefined?10:h);
this.numCols=(i===undefined?10:i);
this.x=(x===undefined?50:x);
this.y=(y===undefined?50:y);
this.cellSize=(j===undefined?50:j);
...
for(var C=0;C<this.numRows;C++){this.getLayout().setRowHeight(C,this.cellSize);
}
for(var A=0;A<this.numCols;A++){this.getLayout().setColumnWidth(A,this.cellSize);
}this.cells=[];
this.mazeInfo=[];
for(C=0;C<this.numRows;C++){this.cells[C]=[];
this.mazeInfo[C]=[];

for(A=0;A<this.numCols;A++){this.cells[C][A]=new qx.ui.container.Composite();
this.cells[C][A].setLayout(new qx.ui.layout.HBox());
this.add(this.cells[C][A],{row:C,column:A});
this.cells[C][A].setDecorator(new qx.ui.decoration.Single(1,f,g));
this.mazeInfo[C][A]=0xf;
}}var s=[];
var n=1;
var m=this.numRows*this.numCols;
var z;
var k;
var t;
z={row:Math.floor(Math.random()*this.numRows),col:Math.floor(Math.random()*this.numCols)};

while(n<m){k=[];
if(z.col>0&&(this.mazeInfo[z.row][z.col-1]==0xf)){k.push({row:z.row,col:z.col-1,currentCellWall:demobrowser.demo.util.FSMMaze.Direction.WEST,neighborWall:demobrowser.demo.util.FSMMaze.Direction.EAST});
...
}if(k.length>0){var r=Math.floor(Math.random()*k.length);
t=k[r];
this.mazeInfo[z.row][z.col]&=~t.currentCellWall;
this.mazeInfo[t.row][t.col]&=~t.neighborWall;
var p=new qx.ui.decoration.Single(1,f,g);
var w=this.cells[z.row][z.col].getDecorator();
p.set({widthLeft:w.getWidthLeft(),widthBottom:w.getWidthBottom(),widthRight:w.getWidthRight(),widthTop:w.getWidthTop()});

switch(t.currentCellWall){case demobrowser.demo.util.FSMMaze.Direction.WEST:p.setWidthLeft(0);
break;
case demobrowser.demo.util.FSMMaze.Direction.SOUTH:p.setWidthBottom(0);
break;
case demobrowser.demo.util.FSMMaze.Direction.EAST:p.setWidthRight(0);
break;
case demobrowser.demo.util.FSMMaze.Direction.NORTH:p.setWidthTop(0);
break;
}this.cells[z.row][z.col].setDecorator(p);
var v=new qx.ui.decoration.Single(1,f,g);
var l=this.cells[t.row][t.col].getDecorator();
v.set({widthLeft:l.getWidthLeft(),widthBottom:l.getWidthBottom(),widthRight:l.getWidthRight(),widthTop:l.getWidthTop()});

switch(t.neighborWall){case demobrowser.demo.util.FSMMaze.Direction.WEST:v.setWidthLeft(0);
break;
case demobrowser.demo.util.FSMMaze.Direction.SOUTH:v.setWidthBottom(0);
break;
case demobrowser.demo.util.FSMMaze.Direction.EAST:v.setWidthRight(0);
break;
case demobrowser.demo.util.FSMMaze.Direction.NORTH:v.setWidthTop(0);
break;
}this.cells[t.row][t.col].setDecorator(v);
s.push({row:z.row,col:z.col});
z={row:t.row,col:t.col};
n++;
}else{var q=s.pop();
z={row:q.row,col:q.col};
}}this.startCell={row:Math.floor(Math.random()*this.numRows),col:Math.floor(Math.random()*this.numCols)};
var u=this.cells[this.startCell.row][this.startCell.col];
u.setBackgroundColor(e);
do{this.endCell={row:Math.floor(Math.random()*this.numRows),col:Math.floor(Math.random()*this.numCols)};
}while((Math.abs(this.startCell.row-this.endCell.row)<this.numRows/2)||(Math.abs(this.startCell.col-this.endCell.col)<this.numCols/2));
var B=this.cells[this.endCell.row][this.endCell.col];
...
},getWestCell:function(E){var F=demobrowser.demo.util.FSMMaze.Direction.WEST;

if(E.col>0&&((this.mazeInfo[E.row][E.col]&F)==0)){return ({row:E.row,col:E.col-1});
}return null;
},getSouthCell:function(G){var H=demobrowser.demo.util.FSMMaze.Direction.SOUTH;

if(G.row<this.numRows-1&&((this.mazeInfo[G.row][G.col]&H)==0)){return ({row:G.row+1,col:G.col});
}return null;
},getEastCell:function(I){var J=demobrowser.demo.util.FSMMaze.Direction.EAST;

if(I.col<this.numCols-1&&((this.mazeInfo[I.row][I.col]&J)==0)){return ({row:I.row,col:I.col+1});
}return null;
},getNorthCell:function(K){var L=demobrowser.demo.util.FSMMaze.Direction.NORTH;

if(K.row>0&&((this.mazeInfo[K.row][K.col]&L)==0)){return ({row:K.row-1,col:K.col});
}return null;
},markCell:function(M){var N=Math.ceil(this.cellSize/5);
var o=new qx.ui.basic.Label(d,null,a);
o.set({height:N,width:N,paddingTop:(this.cellSize-N)/2,paddingLeft:(this.cellSize-N)/2,rich:true});
this.cells[M.row][M.col].add(o);
}}});
})();
...
(function(){var j="String",h="string",g="Number",f="__OU",d="__OV",c="qx.util.fsm.FiniteStateMachine",b="object";
qx.Class.define(c,{extend:qx.core.Object,construct:function(k){qx.core.Object.call(this);
this.setName(k);
this.__OR={};
this.__OS=null;
this.__OT=[];
...
this.__Pa={};
this.__Pb={};
},statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:j,nullable:true},state:{check:j,nullable:true},previousState:{check:j,nullable:true},nextState:{check:j,nullable:true},maxSavedStates:{check:g,init:2},debugFlags:{check:g,init:7}},members:{__OR:null,__OS:null,__OU:null,__OV:null,__OT:null,__OW:null,__OX:null,__OY:null,__Pa:null,__Pb:null,__Pc:false,addState:function(l){if(!l instanceof qx.util.fsm.State){throw new Error("Invalid state: not an instance of "+"qx.util.fsm.State");
}var m=l.getName();
if(m in this.__OR){throw new Error("State "+m+" already exists");
}if(this.__OS==null){this.__OS=m;
}this.__OR[m]=l;
},replaceState:function(n,o){if(!n instanceof qx.util.fsm.State){throw new Error("Invalid state: not an instance of "+"qx.util.fsm.State");
}var q=n.getName();
var p=this.__OR[q];
this.__OR[q]=n;
if(o){p._bNeedDispose=true;
}return p;
},addObject:function(r,s,u){var w=s.toHashCode();
this.__OX[r]=w;
this.__OY[w]=r;
this.__OW[r]=s;
if(!u){return;
...
if(!this.__Pa[v]){this.__Pa[v]={};
}this.__Pa[v][r]=true;
if(!this.__Pb[r]){this.__Pb[r]=[];
}this.__Pb[r]=this.__Pb[r].concat(u);
}},removeObject:function(x){var z=this.__OX[x];
if(this.__Pb[x]){for(var y in this.__Pb[x]){delete this.__Pa[y];
}delete this.__Pb[x];
}delete this.__OY[z];
delete this.__OX[x];
delete this.__OW[x];
},getObject:function(A){return this.__OW[A];
},getFriendlyName:function(B){var C=B.toHashCode();
return C?this.__OY[C]:null;
},getGroupObjects:function(D){var a=[];

for(var name in this.__Pa[D]){a.push(name);
}return a;
},displayAllObjects:function(){for(var E in this.__OX){var G=this.__OX[E];
var F=this.getObject(E);
this.debug(E+" => "+G);
this.debug("  "+G+" => "+this.__OY[G]);
this.debug("  "+E+" => "+this.getObject(E));
this.debug("  "+this.getObject(E)+" => "+this.getFriendlyName(F));
}},start:function(){var I=this.__OS;

if(I==null){throw new Error("Machine started with no available states");
}this.setState(I);
this.setPreviousState(null);
this.setNextState(null);
var H=(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL);
if(H){this.debug(this.getName()+"#"+I+"#actionsBeforeOnentry");
}this.__OR[I].getAutoActionsBeforeOnentry()(this);
if(H){this.debug(this.getName()+"#"+I+"#entry");
}this.__OR[I].getOnentry()(this,null);
if(H){this.debug(this.getName()+"#"+I+"#actionsAfterOnentry");
}this.__OR[I].getAutoActionsAfterOnentry()(this);
},pushState:function(J){if(this.__OT.length>=this.getMaxSavedStates()){throw new Error("Saved-state stack is full");
}
if(J===true){this.__OT.push(this.getState());
}else if(J){this.__OT.push(J);
}else{this.__OT.push(this.getPreviousState());
}},postponeEvent:function(event){this.__OV.unshift(event);
},enqueueEvent:function(event,K){if(K){this.__OU.push(event);
}else{this.__OU.unshift(event);
}
if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){if(K){this.debug(this.getName()+": Pushed event: "+event.getType());
...
},__Pd:function(){if(this.__Pc){return ;
}this.__Pc=true;
while(this.__OU.length>0){var event=this.__OU.pop();
var S=this.__Pe(event);
if(S){event.dispose();
}}this.__Pc=false;
},__Pe:function(event){var V;
var X;
var U;
var bg;
var be;
...
if(bf){this.debug(this.getName()+": Process event: "+event.getType());
}V=this.getState();
bg=this.__OR[V];
be=bg.transitions;
...
}if(typeof (e)==b){var W=this.getFriendlyName(event.getTarget());

if(!W){if(bb){this.debug(this.getName()+": Could not find friendly name for '"+event.getType()+"' on '"+event.getTarget()+"'");
}return true;
}bc=e[W];
...
}this.__OV.unshift(event);
return false;
default:if(typeof (bc)==h){if(be[bc]){var ba=be[bc];
be={};
be[bc]=ba;
...
if(typeof (X)==h){if(!X in this.__OR){throw new Error("Attempt to transition to nonexistent state "+X);
}this.setNextState(X);
}else{switch(X){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:X=V;
this.setNextState(X);
break;
case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__OT.length==0){throw new Error("Attempt to transition to POP_STATE_STACK "+"while state stack is empty.");
}X=this.__OT.pop();
this.setNextState(X);
break;
...
}bg=this.__OR[this.getNextState()];
...
for(var i=0;i<this.__OV.length;i++){e=this.__OV.pop();
this.__OU.unshift(e);
}
...
this.__OT=this.__OR=null;
}});
})();
(function(){var k="KickInTheAss",j="Die",h="State_Moving",g="State_CellArrival",f="State_FadingOut",e="State_Zombie",d="State_FadingIn",c="Transition_CellArrival_to_Moving_via_KickInTheAss",b="Transition_CellArrival_to_FadingOut_via_Die",a="Transition_FadingOut_to_Zombie_via_Oneshot",w="demobrowser/demo/ui/mouse-west.gif",v="Transition_FadingIn_to_CellArrival_via_Oneshot",u="Transition_FadingOut_to_FadingOut_via_Oneshot",t="Transition_FadingIn_to_FadingIn_via_Oneshot",s="interval",r="demobrowser/demo/ui/mouse-east.gif",q="demobrowser/demo/ui/mouse-north.gif",p="Transition_Moving_to_Moving_via_Oneshot",o="Transition_Moving_to_CellArrival_via_Oneshot",n="oneshot",l="demobrowser.demo.util.FSMMouse",m="demobrowser/demo/ui/mouse-south.gif";
qx.Class.define(l,{extend:qx.util.fsm.FiniteStateMachine,statics:{nextMouse:1,ticksPerSec:20,fadeSeconds:0.5,moveSeconds:0.2},construct:function(x,y,z,A){var B=demobrowser.demo.util.FSMMouse.nextMouse.toString();
qx.util.fsm.FiniteStateMachine.call(this,B);
this.setDebugFlags(0);
++demobrowser.demo.util.FSMMouse.nextMouse;
this.facing=z;
if(x===null){this.currentCell=y.getStartCell();
this.traversed=[];
this.traversed.push(this.currentCell);
this.bLookBehind=true;
this.bLookSideways=true;
}else{this.currentCell=x.currentCell;
this.traversed=x.traversed.concat();
this.bLookBehind=false;
this.bLookSideways=false;
}this.currentLocation=y.getCellTopLeft(this.currentCell);
switch(z){case demobrowser.demo.util.FSMMaze.Direction.NORTH:this.mouseImage=new qx.ui.basic.Image(q);
break;
case demobrowser.demo.util.FSMMaze.Direction.EAST:this.mouseImage=new qx.ui.basic.Image(r);
break;
case demobrowser.demo.util.FSMMaze.Direction.SOUTH:this.mouseImage=new qx.ui.basic.Image(m);
break;
case demobrowser.demo.util.FSMMaze.Direction.WEST:this.mouseImage=new qx.ui.basic.Image(w);
break;
}this.mouseImage.set({opacity:0.0,zIndex:9999});
A.add(this.mouseImage,{top:this.currentLocation.top,left:this.currentLocation.left});
var C=new qx.util.fsm.State(d,{"onentry":function(E,F){E.oneshot(1000/demobrowser.demo.util.FSMMouse.ticksPerSec);
},"events":{"oneshot":qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE}});
this.addState(C);
var D=new qx.util.fsm.Transition(t,{"predicate":function(G,event){return G.opacity<1.0;
},"nextState":d,"ontransition":function(H,event){H.opacity+=1/(demobrowser.demo.util.FSMMouse.fadeSeconds*demobrowser.demo.util.FSMMouse.ticksPerSec);
H.mouseImage.setOpacity(H.opacity);
}});
C.addTransition(D);
var D=new qx.util.fsm.Transition(v,{"nextState":g});
C.addTransition(D);
var C=new qx.util.fsm.State(g,{"onentry":function(I,J){var K=y.getEndCell();

if(I.currentCell.row==K.row&&I.currentCell.col==K.col){for(var i=I.traversed.length-2;i>=0;i--){y.markCell(I.traversed[i]);
}return ;
...
}}I.bLookSideways=true;
if(I.bLookBehind&&y.getEastCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.EAST,A);
I.bLookBehind=false;
}I.destinationCell=y.getWestCell(I.currentCell);

if(I.destinationCell!==null){I.destinationLocation=y.getCellTopLeft(I.destinationCell);
var event=new qx.event.type.Event();
event.setType(k);
I.eventListener(event);
}else{var event=new qx.event.type.Event();
event.setType(j);
I.eventListener(event);
}break;
case demobrowser.demo.util.FSMMaze.Direction.EAST:if(I.bLookSideways){if(y.getSouthCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.SOUTH,A);
}if(y.getNorthCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.NORTH,A);
}}I.bLookSideways=true;
if(I.bLookBehind&&y.getWestCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.WEST,A);
I.bLookBehind=false;
}I.destinationCell=y.getEastCell(I.currentCell);
...
case demobrowser.demo.util.FSMMaze.Direction.SOUTH:if(I.bLookSideways){if(y.getWestCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.WEST,A);
...
if(I.bLookBehind&&y.getNorthCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.NORTH,A);
I.bLookBehind=false;
}I.destinationCell=y.getSouthCell(I.currentCell);
...
case demobrowser.demo.util.FSMMaze.Direction.NORTH:if(I.bLookSideways){if(y.getWestCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.WEST,A);
}if(y.getEastCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.EAST,A);
}}I.bLookSideways=true;
if(I.bLookBehind&&y.getSouthCell(I.currentCell)!==null){new demobrowser.demo.util.FSMMouse(I,y,demobrowser.demo.util.FSMMaze.Direction.SOUTH,A);
I.bLookBehind=false;
}I.destinationCell=y.getNorthCell(I.currentCell);
...
},"events":{"Die":b,"KickInTheAss":c}});
this.addState(C);
var D=new qx.util.fsm.Transition(b,{"nextState":f});
C.addTransition(D);
var D=new qx.util.fsm.Transition(c,{"nextState":h});
C.addTransition(D);
var C=new qx.util.fsm.State(f,{"onentry":function(L,M){L.oneshot(1000/demobrowser.demo.util.FSMMouse.ticksPerSec);
},"events":{"oneshot":qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE}});
this.addState(C);
var D=new qx.util.fsm.Transition(u,{"predicate":function(N,event){return N.opacity>0.0;
},"nextState":f,"ontransition":function(O,event){O.opacity-=1/(demobrowser.demo.util.FSMMouse.fadeSeconds*demobrowser.demo.util.FSMMouse.ticksPerSec);

if(O.opacity<0.0){O.opacity=0.0;
}O.mouseImage.setOpacity(O.opacity);
}});
C.addTransition(D);
var D=new qx.util.fsm.Transition(a,{"nextState":e});
C.addTransition(D);
var C=new qx.util.fsm.State(h,{"onentry":function(P,Q){P.oneshot(1000/demobrowser.demo.util.FSMMouse.ticksPerSec);
},"events":{"oneshot":qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE}});
this.addState(C);
var D=new qx.util.fsm.Transition(p,{"predicate":function(R,event){switch(R.facing){case demobrowser.demo.util.FSMMaze.Direction.WEST:return R.currentLocation.left>R.destinationLocation.left;
case demobrowser.demo.util.FSMMaze.Direction.SOUTH:return R.currentLocation.top<R.destinationLocation.top;
case demobrowser.demo.util.FSMMaze.Direction.EAST:return R.currentLocation.left<R.destinationLocation.left;
case demobrowser.demo.util.FSMMaze.Direction.NORTH:return R.currentLocation.top>R.destinationLocation.top;
}},"nextState":h,"ontransition":function(S,event){var T=Math.ceil(y.cellSize/(demobrowser.demo.util.FSMMouse.moveSeconds*demobrowser.demo.util.FSMMouse.ticksPerSec));
switch(S.facing){case demobrowser.demo.util.FSMMaze.Direction.WEST:S.currentLocation.left=Math.round(S.currentLocation.left-T);
S.mouseImage.setLayoutProperties({left:S.currentLocation.left,top:S.currentLocation.top});
break;
case demobrowser.demo.util.FSMMaze.Direction.SOUTH:S.currentLocation.top=Math.round(S.currentLocation.top+T);
S.mouseImage.setLayoutProperties({left:S.currentLocation.left,top:S.currentLocation.top});
break;
case demobrowser.demo.util.FSMMaze.Direction.EAST:S.currentLocation.left=Math.round(S.currentLocation.left+T);
S.mouseImage.setLayoutProperties({left:S.currentLocation.left,top:S.currentLocation.top});
break;
case demobrowser.demo.util.FSMMaze.Direction.NORTH:S.currentLocation.top=Math.round(S.currentLocation.top-T);
S.mouseImage.setLayoutProperties({left:S.currentLocation.left,top:S.currentLocation.top});
break;
}}});
C.addTransition(D);
var D=new qx.util.fsm.Transition(o,{"nextState":g,"ontransition":function(U,event){U.currentCell=U.destinationCell;
U.traversed.push(U.currentCell);
}});
C.addTransition(D);
var C=new qx.util.fsm.State(e,{"onentry":function(V,W){qx.event.Timer.once(function(){A.remove(this.mouseImage);
...
this.addState(C);
this.start();
},members:{opacity:0.0,oneshot:function(X){var Y=new qx.event.Timer(X);
Y.addListener(s,function(){Y.dispose();
var event=new qx.event.type.Event();
event.setType(n);
this.eventListener(event);
},this);
...
(function(){var p="object",o="string",n='"',m="}",l="{",k="(",h=";",d="__ON",c="function",b="  var groupObjects = ",X="parameters",W="__OP",V=")",U=" fsm.getObject('",T="groups",S="  for (var i = 0; i < groupObjects.length; i++)",R="__OQ",Q="autoActionsAfterOnentry",P="fsm",O="');",w="autoActionsBeforeOnexit",x="autoActionsBeforeOnentry",u="onexit",v="__OL",s="__OJ",t="autoActionsAfterOnexit",q="  fsm.debug(ex);",r="__OO",y="    fsm.getObject(objName).",z=")(fsm)",G="events",E="__OM",I="    var objName = groupObjects[i];",H="').",K="objects",J="catch(ex)",B="  {",N="    fsm.getGroupObjects('",M="  }",L="__OK",A="qx.util.fsm.State",C="onentry",D="try",F=",";
qx.Class.define(A,{extend:qx.core.Object,construct:function(Y,ba){qx.core.Object.call(this);
this.setName(Y);
if(typeof (ba)!=p){throw new Error("State info must be an object");
}for(var bb in ba){switch(bb){case C:this.setOnentry(ba[bb]);
break;
case u:this.setOnexit(ba[bb]);
break;
case x:this.setAutoActionsBeforeOnentry(ba[bb]);
break;
case Q:this.setAutoActionsAfterOnentry(ba[bb]);
break;
case w:this.setAutoActionsBeforeOnexit(ba[bb]);
break;
case t:this.setAutoActionsAfterOnexit(ba[bb]);
break;
case G:this.setEvents(ba[bb]);
break;
default:this.setUserData(bb,ba[bb]);
this.debug("State "+Y+": "+"Adding user-provided field to state: "+bb);
break;
}}if(!this.getEvents()){throw new Error("The events object must be provided in new state info");
}this.transitions={};
},statics:{_commonTransformAutoActions:function(bc,bd){if(typeof (bd)!=p){throw new Error("Invalid "+bc+" value: "+typeof (bd));
}var bi;
var bj=D+l;
var bg;
var be;
for(var f in bd){var bf=bd[f];
if(!bf instanceof Array){throw new Error("Invalid function request type: "+"expected array, found "+typeof (bf));
}for(var i=0;i<bf.length;i++){be=bf[i];
if(typeof (be)!=p){throw new Error("Invalid function request parameter type: "+"expected object, found "+typeof (bf[bg]));
}var bh=be[X];
if(!bh){bh=[];
}else{if(!bh instanceof Array){throw new Error("Invalid function parameters: "+"expected array, found "+typeof (bh));
}}bi=f+k;
for(var j=0;j<bh.length;j++){if(j!=0){bi+=F;
}
if(typeof (bh[j])==c){bi+=k+bh[j]+z;
}else if(typeof (bh[j])==o){bi+=n+bh[j]+n;
}else{bi+=bh[j];
}}bi+=V;
var a=be[K];
if(!a){a=[];
...
for(var j=0;j<a.length;j++){if(typeof (a[j])!=o){throw new Error("Invalid friendly name in 'objects' list: "+a[j]);
}bj+=U+a[j]+H+bi+h;
}var g=be[T];
if(g){if(!g instanceof Array){throw new Error("Invalid 'groups' list: expected array, got "+typeof (g));
}
for(j=0;j<g.length;j++){bj+=b+N+g[j]+O+S+B+I+y+bi+h+M;
}}}}bj+=m+J+l+q+m;
return new Function(P,bj);
}},properties:{name:{transform:s,nullable:true},onentry:{transform:L,nullable:true,init:function(bk,event){}},onexit:{transform:v,nullable:true,init:function(bl,event){}},autoActionsBeforeOnentry:{transform:d,nullable:true,init:function(bm,event){}},autoActionsAfterOnentry:{transform:r,nullable:true,init:function(bn,event){}},autoActionsBeforeOnexit:{transform:W,nullable:true,init:function(bo,event){}},autoActionsAfterOnexit:{transform:R,nullable:true,init:function(bp,event){}},events:{transform:E,nullable:true}},members:{__OJ:function(bq){if(typeof (bq)!="string"||bq.length<1){throw new Error("Invalid state name");
}return bq;
},__OK:function(br){switch(typeof (br)){case "undefined":return function(bs,event){};
case "function":return br;
default:throw new Error("Invalid onentry type: "+typeof (br));
}},__OL:function(bt){switch(typeof (bt)){case "undefined":return function(bu,event){};
case "function":return bt;
default:throw new Error("Invalid onexit type: "+typeof (bt));
return null;
}},__OM:function(bv){if(typeof (bv)!="object"){throw new Error("events must be an object");
}for(var e in bv){var bw=bv[e];

if(typeof (bw)=="number"&&bw!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bw!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error("Invalid numeric value in events object: "+e+": "+bw);
}else if(typeof (bw)=="object"){for(var bx in bw){if(typeof (bw[bx])=="number"&&bw[bx]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bw[bx]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error("Invalid numeric value in events object "+"("+e+"): "+bx+": "+bw[bx]);
}else if(typeof (bw[bx])!="string"&&typeof (bw[bx])!="number"){throw new Error("Invalid value in events object "+"("+e+"): "+bx+": "+bw[bx]);
}}}else if(typeof (bw)!="string"&&typeof (bw)!="number"){throw new Error("Invalid value in events object: "+e+": "+bv[e]);
}}return bv;
},__ON:function(by){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnentry",by);
},__OO:function(bz){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnentry",bz);
},__OP:function(bA){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnexit",bA);
},__OQ:function(bB){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnexit",bB);
},addTransition:function(bC){if(!bC instanceof qx.util.fsm.Transition){throw new Error("Invalid transition: not an instance of "+"qx.util.fsm.Transition");
}this.transitions[bC.getName()]=bC;
}}});
})();
(function(){var o="function",n="undefined",m="__Ph",l="predicate",k="nextState",j="autoActionsBeforeOntransition",i="number",h="__Pf",g="__Pg",f="string",c="autoActionsAfterOntransition",e="qx.util.fsm.Transition",d="ontransition",b="String",a="boolean";
qx.Class.define(e,{extend:qx.core.Object,construct:function(p,q){qx.core.Object.call(this);
this.setName(p);
for(var r in q){switch(r){case l:this.setPredicate(q[r]);
break;
case k:this.setNextState(q[r]);
break;
case j:this.setAutoActionsBeforeOntransition(q[r]);
break;
case c:this.setAutoActionsAfterOntransition(q[r]);
break;
case d:this.setOntransition(q[r]);
break;
default:this.setUserData(r,q[r]);
this.debug("Transition "+p+": "+"Adding user-provided field to transition: "+r);
break;
}}},properties:{name:{check:b,nullable:true},predicate:{init:function(s,event){return true;
},transform:h},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:g},autoActionsBeforeOntransition:{init:function(t,event){}},autoActionsAfterOntransition:{init:function(u,event){}},ontransition:{init:function(v,event){},transform:m}},members:{__Pf:function(w){switch(typeof (w)){case n:return function(x,event){return true;
};
case a:return function(y,event){return w;
};
case o:return w;
default:throw new Error("Invalid transition predicate type: "+typeof (w));
break;
}},__Pg:function(z){switch(typeof (z)){case f:return z;
case i:switch(z){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return z;
default:throw new Error("Invalid transition nextState value: "+z+": "+"nextState must be an explicit state name, "+"or one of the Fsm.StateChange constants");
}break;
default:throw new Error("Invalid transition nextState type: "+typeof (z));
break;
}},__Ph:function(A){switch(typeof (A)){case n:return function(B,event){};
case o:return A;
