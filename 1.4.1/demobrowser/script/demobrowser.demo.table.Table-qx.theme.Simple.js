...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table","qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.table.Table-qx.theme.Simple.js"]],
  urisBefore : [],
  packageHashes : {"0":"609c578bc7ff"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var k="execute",j="changeValue",h="A date",g="Boolean",f="icon/22/actions/edit-undo.png",e="Change ID sort method",d="icon/22/status/dialog-information.png",c="Change row with ID 10",b="A number",a="icon/16/apps/office-calendar.png",A="Remove 5 rows",z="Add 10 rows",y="",x="demobrowser.demo.table.Table",w="_tableModel",v="Table",u="icon/22/actions/list-remove.png",t="Whether the the first visible row should ",s=", ",r="icon/22/actions/list-add.png",p="Selected rows:<br>",q="Keep first row",n="be rendered completely when scrolling.",o="Show selection",l="Demonstrate use of alternate sorting algorithm.",m="ID";
qx.Class.define(x,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return v;
},createTable:function(){var D=this.createRandomRows(50);
var C=this._tableModel=new qx.ui.table.model.Simple();
C.setColumns([m,b,h,g]);
C.setData(D);
C.setColumnEditable(1,true);
C.setColumnEditable(2,true);
C.setColumnSortable(3,false);
var E=new qx.ui.table.Table(C);
E.set({width:600,height:400,decorator:null});
...
B.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(a,h));
return E;
},createControls:function(){var I=new qx.ui.toolbar.ToolBar();
var G,H,J;
H=new qx.ui.toolbar.Part();
I.add(H);
G=new qx.ui.toolbar.Button(c,f);
G.addListener(k,function(K){var L=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,L[0][i]);
}this.info("Row 10 changed");
},this);
H.add(G);
G=new qx.ui.toolbar.Button(z,r);
G.addListener(k,function(M){var N=this.createRandomRows(10);
this._tableModel.addRows(N);
this.info("10 rows added");
},this);
H.add(G);
G=new qx.ui.toolbar.Button(A,u);
G.addListener(k,function(O){var P=this._tableModel.getRowCount();
this._tableModel.removeRows(P-5,5);
this.info("5 rows removed");
},this);
H.add(G);
G=new qx.ui.toolbar.Button(o,d);
G.addListener(k,function(Q){var R=[];
F.getSelectionModel().iterateSelection(function(S){R.push(S+y);
});
this.showDialog(p+R.join(s));
},this);
H.add(G);
H=new qx.ui.toolbar.Part();
I.add(H);
var F=this._table;
J=new qx.ui.toolbar.CheckBox(q);
J.set({value:F.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(t+n)});
J.addListener(j,function(T){F.setKeepFirstVisibleRowComplete(this.getValue());
},J);
H.add(J);
J=new qx.ui.toolbar.CheckBox(e);
J.set({value:F.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(l)});
J.addListener(j,function(U){if(U.getData()){var W=function(X,Y){var ba=X[arguments.callee.columnIndex];
var bb=Y[arguments.callee.columnIndex];

if(ba%2==1&&bb%2==0){return 1;
}
if(bb%2==1&&ba%2==0){return -1;
}return (ba>bb)?1:((ba==bb)?0:-1);
};
var V=function(bc,bd){var be=bc[arguments.callee.columnIndex];
var bf=bd[arguments.callee.columnIndex];

if(be%2==1&&bf%2==0){return -1;
}
if(bf%2==1&&be%2==0){return 1;
}return (be<bf)?1:((be==bf)?0:-1);
};
F.getTableModel().setSortMethods(0,{ascending:W,descending:V});
}else{F.getTableModel().setSortMethods(0,null);
}},J);
H.add(J);
return I;
