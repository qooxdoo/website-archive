...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.ListControllerWithObjects","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.data.ListControllerWithObjects-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"34447d1ac88a"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
var U=[w,x,n,h,q,t,o,b];
var L=[C,f,E,F,D,u];
var R=[];

for(var i=0;i<20;i++){var O=new demobrowser.demo.data.model.Person();
O.setName(U[i%U.length]);
O.setEmote(L[i%L.length]);
O.setOnline(i%3==0);
R.push(O);
}var S=new qx.data.Array(R);
var K=new qx.ui.form.List();
K.setWidth(150);
this.getRoot().add(K,{left:10,top:80});
var T={converter:function(V){return m+V+g;
}};
var P=new qx.data.controller.List(S,K,z);
P.setIconOptions(T);
P.setIconPath(d);
var N={converter:function(W){if(W){return c;
}else{return v;
}}};
var I={bindItem:function(X,Y,ba){X.bindDefaultProperties(Y,ba);
X.bindProperty(B,r,N,Y,ba);
}};
P.setDelegate(I);
var J=new qx.ui.form.Button(G);
J.setWidth(120);
this.getRoot().add(J,{left:180,top:80});
J.addListener(k,function(){for(var i=0;i<S.length;i++){var bb=S.getItem(i);
bb.setEmote(j);
}},this);
var H=new qx.ui.form.Button(l);
H.setWidth(120);
this.getRoot().add(H,{left:180,top:115});
H.addListener(k,function(){for(var i=0;i<S.length;i++){S.getItem(i).toggleOnline();
}},this);
var Q=new qx.ui.form.Button(y);
Q.setWidth(120);
this.getRoot().add(Q,{left:180,top:150});
Q.addListener(k,function(){qx.log.appender.Console.show();
...
M.setWidth(410);
M.setValue(a+p+s+A);
this.getRoot().add(M,{left:20,top:10});
}}});
})();
