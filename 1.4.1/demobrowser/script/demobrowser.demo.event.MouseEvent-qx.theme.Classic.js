...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.event.MouseEvent","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.event.MouseEvent-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"d33b127cbf5c"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
}},_initLogger:function(n,o,p){this.__Tp=e+n.join(h)+k;
this.__Tq=p||50;
this.__Tr=o;
this._clearLog();
},_clearLog:function(){this.__Tr.innerHTML=this.__Tp+l;
this.__Ts=[];
},_log:function(q){this.__Ts.unshift(q);
this.__Ts=this.__Ts.slice(0,this.__Tq);
var r=[this.__Tp];

for(var i=0;i<this.__Ts.length;i++){r.push(d,this.__Ts[i].join(f),a);
}r.push(l);
this.__Tr.innerHTML=r.join(g);
this.__Tr.scrollTop=0;
},__Tp:null,__Tq:null,__Tr:null,__Ts:null},destruct:function(){this.__Tr=this.__Ts=null;
}});
})();
(function(){var l="mouse",k="change",j="mousedown",h="mouseout",g="demobrowser.demo.event.MouseEvent",f="clientX",d="dblclick",c="relatedTarget",b="button",a="capture",C="mousemove",B="pageY",A="screenX",z="logger",y="",x="_",w="mouseover",v="check_",u="click",t="Event",r="Target",s="clientY",p="mouseup",q="losecapture",n="contextmenu",o="screenY",m="pageX";
qx.Class.define(g,{extend:demobrowser.demo.event.EventDemo,members:{main:function(){demobrowser.demo.event.EventDemo.prototype.main.call(this);
this._initLogger([r,t,b,m,B,f,s,A,o,c],document.getElementById(z),50);
var D=document.getElementById(l);
var F=[j,p,u,d,n,C,w,h];

for(var i=0;i<F.length;i++){var G=document.getElementById(v+F[i]);

if(G.checked){qx.bom.Element.addListener(D,F[i],this.logMouseEvent,this);
}qx.bom.Element.addListener(G,k,this.__Tt,this);
}var E=document.getElementById(a);
E.checked=false;
qx.bom.Element.addListener(E,k,function(e){var H=e.getTarget().checked;

if(H){qx.bom.Element.capture(D);
}else{qx.bom.Element.releaseCapture(D);
}},this);
qx.bom.Element.addListener(D,q,function(e){E.checked=false;
},this);
},__Tt:function(e){var K=e.getTarget().id.split(x)[1];
var I=e.getTarget().checked;
var J=document.getElementById(l);

if(I){qx.bom.Element.addListener(J,K,this.logMouseEvent,this);
}else{qx.bom.Element.removeListener(J,K,this.logMouseEvent,this);
}},logMouseEvent:function(L){L.preventDefault();
this._log([L.getTarget().id,L.getType(),L.getButton(),L.getDocumentLeft(),L.getDocumentTop(),L.getScreenLeft(),L.getScreenTop(),L.getViewportLeft(),L.getViewportTop(),L.getRelatedTarget()?L.getRelatedTarget().id:y]);
}}});
})();
