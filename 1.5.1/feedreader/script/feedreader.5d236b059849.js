...
}if(qx.core.Environment.get(g)){qx.locale.Manager.getInstance().addListener(U,this._onChangeLocale,this);
}},events:{"input":c,"changeValue":c},properties:{textAlign:{check:[G,l,y],nullable:true,themeable:true,apply:R},readOnly:{check:d,apply:A,event:t,init:false},selectable:{refine:true,init:true},focusable:{refine:true,init:true},maxLength:{check:O,init:Infinity},liveUpdate:{check:d,init:false},placeholder:{check:D,nullable:true,apply:C},filter:{check:P,nullable:true,init:null}},members:{__E:true,__F:null,__e:null,__f:null,__D:true,__G:null,__H:null,getFocusElement:function(){var bb=this.getContentElement();
...
bm.setAttribute(L,J);
if(qx.core.Environment.get(f)==z||qx.core.Environment.get(f)==p){bm.setStyle(B,i);
}if((qx.core.Environment.get(f)==N)){bm.setStyles({backgroundImage:w+qx.util.ResourceManager.getInstance().toUri(K)+r});
}return bm;
...
}if(this.getTextColor()!=null){delete bs[h];
...
},_applyTextColor:function(bt,bu){if(bt){this.getContentElement().setStyle(h,qx.theme.manager.Color.getInstance().resolve(bt));
...
var bx=true;
this.__E=false;
if(this.__f&&this.__f===by){bx=false;
...
if(qx.core.Environment.get(g)){qx.locale.Manager.getInstance().removeListener(U,this._onChangeLocale,this);
}
