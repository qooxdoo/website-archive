...
},__hg:[r,G,E,m,y,I],getKeys:({"ES5":Object.keys,"BROKEN_IE":function(bw){var bx=[];
var bz=Object.prototype.hasOwnProperty;

for(var bA in bw){if(bz.call(bw,bA)){bx.push(bA);
}}var by=qx.Bootstrap.__hg;
...
},__hh:{"[object String]":d,"[object Array]":f,"[object Object]":h,"[object RegExp]":D,"[object Number]":x,"[object Boolean]":K,"[object Date]":s,"[object Function]":c,"[object Error]":J},bind:function(bI,self,bJ){var bK=Array.prototype.slice.call(arguments,2,arguments.length);
...
return (qx.Bootstrap.__hh[bP]||bP.slice(8,-1));
...
if(qx.bom.client.Engine.__bN()){if(/Opera[\s\/]([0-9]+)\.([0-9])([0-9]*)/.test(v)){if(v.indexOf(a)!=-1){var u=v.match(/Version\/(\d+)\.(\d+)/);
...
if(qx.bom.client.Engine.__bN()){name=b;
}else if(qx.bom.client.Engine.__bO()){name=c;
...
},__bP:function(){return window.controllers&&window.navigator.product===g;
...
qx.Bootstrap.define(b,{statics:{getLocale:function(){var e=qx.bom.client.Locale.__lC();
var f=e.indexOf(d);

if(f!=-1){e=e.substr(0,f);
}return e;
},getVariant:function(){var g=qx.bom.client.Locale.__lC();
var i=c;
var h=g.indexOf(d);

if(h!=-1){i=g.substr(h+1);
}return i;
},__lC:function(){var j=(navigator.userLanguage||navigator.language||c);
...
},__jR:function(p,q,r,s){var w=r.$$members;

if(w){for(var v in w){if(qx.Bootstrap.isFunction(w[v])){var u=this.__jS(q,v);
...
if(x){p[v]=this.__jV(r,p[v],v,w[v]);
...
}}}}}},__jS:function(y,z){var D=z.match(/^(is|toggle|get|set|reset)(.*)$/);
...
},__jT:function(E,F){if(F.$$properties){for(var G in F.$$properties){if(!qx.Bootstrap.getPropertyDefinition(E,G)){throw new Error('The property "'+G+'" is not supported by Class "'+E.classname+'"!');
}}}},__jU:function(H,I){if(I.$$events){for(var J in I.$$events){if(!qx.Bootstrap.supportsEvent(H,J)){throw new Error('The event "'+J+'" is not supported by Class "'+H.classname+'"!');
}}}},assertObject:function(K,L){var N=K.constructor;
this.__jR(K,N,L,false);
this.__jT(N,L);
this.__jU(N,L);
var M=L.$$extends;

if(M){for(var i=0,l=M.length;i<l;i++){this.assertObject(K,M[i]);
}}},assert:function(O,P,Q){this.__jR(O.prototype,O,P,Q);
this.__jT(O,P);
this.__jU(O,P);
var R=P.$$extends;
...
},$$registry:{},__jV:function(){},__ge:null,__gg:function(){}}});
})();
(function(){var d="qx.core.Aspect",c="before",b="*",a="static";
qx.Bootstrap.define(d,{statics:{__hi:[],wrap:function(e,f,g){var m=[];
var h=[];
var l=this.__hi;
var k;
...
j.original=f;
return j;
},addAdvice:function(o,p,q,name){this.__hi.push({fcn:o,pos:p===c?-1:1,type:q,name:name});
}}});
})();
...
},__fQ:{0:b,1:I,2:dc,3:bk,4:cj,5:T},error:function(dR,dS,dT,dU,dV){var dW=dR.constructor.classname;
...
}if(eA===cu){ey.push(U,name,v,eA,X);
}};
...
qx.Class.define(d,{statics:{inShutDown:false,__hi:{},__hj:0,__hk:[],__hl:e,__hm:{},register:function(j){var n=this.__hi;

if(!n){return;
}var m=j.$$hash;

if(m==null){var k=this.__hk;

if(k.length>0&&!qx.core.Environment.get(h)){m=k.pop();
}else{m=(this.__hj++)+this.__hl;
}j.$$hash=m;

if(qx.core.Environment.get(h)&&qx.dev&&qx.dev.Debug&&qx.dev.Debug.disposeProfilingActive){this.__hm[m]=qx.dev.StackTrace.getStackTrace();
}}n[m]=j;
...
}else{u=(this.__hj++)+this.__hl;
}return s.$$hash=u;
},clearHashCode:function(v){var w=v.$$hash;

if(w!=null){this.__hk.push(w);
try{delete v.$$hash;
}catch(x){if(v.removeAttribute){v.removeAttribute(g);
}}}},fromHashCode:function(y){return this.__hi[y]||null;
...
},getNextHash:function(){return this.__hj;
},getPostId:function(){return this.__hl;
},getStackTraces:function(){return this.__hm;
}},defer:function(E){if(window&&window.top){var frames=window.top.frames;

for(var i=0;i<frames.length;i++){if(frames[i]===window){E.__hl=f+(i+1);
return;
}}}E.__hl=c;
}});
})();
...
(function(){var r="|bubble",q="|capture",p="|",o="",n="_",m="unload",k="UNKNOWN_",j="__jB",h="__jC",g="c",c="DOM_",f="WIN_",e="QX_",b="qx.event.Manager",a="capture",d="DOCUMENT_";
qx.Class.define(b,{extend:Object,construct:function(s,t){this.__cp=s;
this.__jK=qx.core.ObjectRegistry.toHashCode(s);
this.__cW=t;
...
this.__jC={};
this.__jM={};
},statics:{__jN:0,getNextUniqueId:function(){return (this.__jN++)+o;
}},members:{__cW:null,__jL:null,__jC:null,__jO:null,__jB:null,__jM:null,__cp:null,__jK:null,getWindow:function(){return this.__cp;
},getWindowId:function(){return this.__jK;
},getHandler:function(u){var v=this.__jB[u.classname];

if(v){return v;
}return this.__jB[u.classname]=new u(this);
},getDispatcher:function(w){var x=this.__jC[w.classname];

if(x){return x;
}return this.__jC[w.classname]=new w(this,this.__cW);
...
},getAllListeners:function(){return this.__jL;
...
if(Q){this.__jP(P,W,R);
}else{this.__jQ(P,W,R);
...
if(!bj){bj=bo[bm]=[];
this.__jP(bg,bl.type,bl.capture);
}bj.push({handler:bl.listener,context:bl.self,unique:bl.unique||(bk.__jN++)+o});
}},addListener:function(bp,bq,br,self,bs){var bw;
var bx=bp.$$hash||qx.core.ObjectRegistry.toHashCode(bp);
var bz=this.__jL[bx];

if(!bz){bz=this.__jL[bx]={};
}var bv=bq+(bs?q:r);
var bu=bz[bv];

if(!bu){bu=bz[bv]=[];
}if(bu.length===0){this.__jP(bp,bq,bs);
}var by=(qx.event.Manager.__jN++)+o;
...
},__jP:function(bP,bQ,bR){var bS=this.findHandler(bP,bQ);
...
if(bY.length==0){this.__jQ(bT,bU,bW);
}return true;
}}return false;
...
if(ci.length==0){this.__jQ(ce,co,cg);
}return true;
}}return false;
...
return true;
},deleteAllListeners:function(cx){delete this.__jL[cx];
},__jQ:function(cy,cz,cA){var cB=this.findHandler(cy,cz);
...
this.__jL=this.__cp=this.__jO=null;
this.__cW=this.__jM=null;
}}});
})();
(function(){var c="qx.event.Registration";
qx.Class.define(c,{statics:{__jA:{},getManager:function(d){if(d==null){d=window;
...
},PRIORITY_FIRST:-32000,PRIORITY_NORMAL:0,PRIORITY_LAST:32000,__jB:[],addHandler:function(Q){this.__jB.push(Q);
this.__jB.sort(function(a,b){return a.PRIORITY-b.PRIORITY;
});
},getHandlers:function(){return this.__jB;
},__jC:[],addDispatcher:function(R,S){this.__jC.push(R);
this.__jC.sort(function(a,b){return a.PRIORITY-b.PRIORITY;
});
},getDispatchers:function(){return this.__jC;
}}});
})();
...
qx.Class.define(s,{statics:{__jD:A,setLevel:function(I){this.__jD=I;
},getLevel:function(){return this.__jD;
},setTreshold:function(J){this.__jF.setMaxMessages(J);
},getTreshold:function(){return this.__jF.getMaxMessages();
},__jE:{},__df:0,register:function(K){if(K.$$id){return;
}var M=this.__df++;
this.__jE[M]=K;
K.$$id=M;
var L=this.__jG;
var N=this.__jF.getAllLogEvents();

for(var i=0,l=N.length;i<l;i++){if(L[N[i].level]>=L[this.__jD]){K.process(N[i]);
}}},unregister:function(O){var P=O.$$id;

if(P==null){return;
}delete this.__jE[P];
delete O.$$id;
},debug:function(Q,R){qx.log.Logger.__jH(A,arguments);
},info:function(S,T){qx.log.Logger.__jH(e,arguments);
},warn:function(U,V){qx.log.Logger.__jH(r,arguments);
},error:function(W,X){qx.log.Logger.__jH(j,arguments);
},trace:function(Y){qx.log.Logger.__jH(e,[Y,qx.dev.StackTrace.getStackTrace().join(u)]);
...
},clear:function(){this.__jF.clearHistory();
},__jF:new qx.log.appender.RingBuffer(50),__jG:{debug:0,info:1,warn:2,error:3},__jH:function(bv,bw){var bB=this.__jG;

if(bB[bv]<bB[this.__jD]){return;
}var by=bw.length<2?null:bw[0];
var bA=by?1:0;
var bx=[];

for(var i=bA,l=bw.length;i<l;i++){bx.push(this.__jJ(bw[i],true));
}var bC=new Date;
var bD={time:bC,offset:bC-qx.Bootstrap.LOADSTART,level:bv,items:bx,win:window};
if(by){if(by.$$hash!==undefined){bD.object=by.$$hash;
}else if(by.$$type){bD.clazz=by;
}}this.__jF.process(bD);
var bE=this.__jE;

for(var bz in bE){bE[bz].process(bD);
}},__jI:function(bF){if(bF===undefined){return y;
...
},__jJ:function(bH,bI){var bP=this.__jI(bH);
var bL=D;
var bK=[];
...
for(var i=0;i<bR.length;i++){bQ.__jH(bR[i][0],bR[i][1]);
...
this.__kz=d;
this.__kA=c;
},members:{__kA:null,__kz:null,__bR:null,toString:function(){return this.__bR;
},getArguments:function(){return this.__kz;
},getSourceException:function(){return this.__kA;
...
qx.Bootstrap.define(a,{statics:{__hX:function(){if(qx.core&&qx.core.Environment){return qx.core.Environment.get(b);
}else{return !!qx.Bootstrap.getEnvironmentSetting(b);
}},setErrorHandler:function(c,d){this.__eB=c||null;
this.__eC=d||window;

if(this.__hX()){if(c&&window.onerror){var e=qx.Bootstrap.bind(this.__ia,this);

if(this.__hY==null){this.__hY=window.onerror;
}var self=this;
window.onerror=function(f,g,h){self.__hY(f,g,h);
e(f,g,h);
};
}
if(c&&!window.onerror){window.onerror=qx.Bootstrap.bind(this.__ia,this);
}if(this.__eB==null){if(this.__hY!=null){window.onerror=this.__hY;
this.__hY=null;
}else{window.onerror=null;
}}}},__ia:function(i,j,k){if(this.__eB){this.handleError(new qx.core.WindowError(i,j,k));
return true;
}},observeMethod:function(l){if(this.__hX()){var self=this;
...
}else{if(this.__ek){qx.Bootstrap.error(bD);
}throw new Error(bD);
...
(function(){var m="element",k="qxSelectable",j="off",h="engine.name",g="on",f="text",d="div",c="",b="mshtml",a="none",G="scroll",F="qx.html.Element",E="|capture|",D="activate",C="blur",B="deactivate",A="capture",z="userSelect",w="-moz-none",v="visible",t="__iO",u="releaseCapture",r="|bubble|",s="tabIndex",p="qx.html.Iframe",q="focus",n="MozUserSelect",o="hidden";
qx.Class.define(F,{extend:qx.core.Object,construct:function(H,I,J){qx.core.Object.call(this);
this.__it=H||d;
this.__iu=I||null;
this.__iv=J||null;
},statics:{DEBUG:false,_modified:{},_visibility:{},_scroll:{},_actions:[],__iw:{},_scheduleFlush:function(K){qx.html.Element.__eR.schedule();
},flush:function(){var V;
var N=this.__ix();
var M=N.getFocus();

if(M&&this.__iB(M)){N.blur(M);
}var bd=N.getActive();

if(bd&&this.__iB(bd)){qx.bom.Element.deactivate(bd);
}var Q=this.__iz();

if(Q&&this.__iB(Q)){qx.bom.Element.releaseCapture(Q);
}var W=[];
var X=this._modified;

for(var U in X){V=X[U];
if(V.__iS()||V.classname==p){if(V.__iC&&qx.dom.Hierarchy.isRendered(V.__iC)){W.push(V);
}else{V.__iR();
}delete X[U];
}}
...
if(!Y){delete S[U];
continue;
}if(!V.$$disposed){Y.style.display=V.__iE?c:a;
if((qx.core.Environment.get(h)==b)){if(!(document.documentMode>=8)){Y.style.visibility=V.__iE?v:o;
}}}delete S[U];
...
if(V.__iH!=null){V.__iC.scrollLeft=V.__iH;
delete V.__iH;
}if(V.__iI!=null){V.__iC.scrollTop=V.__iI;
delete V.__iI;
}var ba=V.__iF;
...
var Y=bc.element.__iC;

if(!Y||!O[bc.type]&&!bc.element.__iS()){continue;
}var R=bc.args;
R.unshift(Y);
qx.bom.Element[bc.type].apply(qx.bom.Element,R);
}this._actions=[];
for(var U in this.__iw){var L=this.__iw[U];
var be=L.element.__iC;

if(be){qx.bom.Selection.set(be,L.start,L.end);
delete this.__iw[U];
}}qx.event.handler.Appear.refresh();
},__ix:function(){if(!this.__iy){var bf=qx.event.Registration.getManager(window);
this.__iy=bf.getHandler(qx.event.handler.Focus);
}return this.__iy;
},__iz:function(){if(!this.__iA){var bg=qx.event.Registration.getManager(window);
this.__iA=bg.getDispatcher(qx.event.dispatch.MouseCapture);
}return this.__iA.getCaptureElement();
},__iB:function(bh){var bi=qx.core.ObjectRegistry.fromHashCode(bh.$$element);
return bi&&!bi.__iS();
}},members:{__it:null,__iC:null,__cq:false,__iD:true,__iE:true,__iF:null,__iG:null,__iH:null,__iI:null,__iJ:null,__iK:null,__iL:null,__iu:null,__iv:null,__iM:null,__iN:null,__iO:null,__iP:null,__iQ:null,_scheduleChildrenUpdate:function(){if(this.__iP){return;
}this.__iP=true;
qx.html.Element._modified[this.$$hash]=this;
qx.html.Element._scheduleFlush(m);
},_createDomElement:function(){return qx.bom.Element.create(this.__it);
},__iR:function(){var length;
var bj=this.__iO;
...
if(bk.__iE&&bk.__iD&&!bk.__iC){bk.__iR();
}}}
if(!this.__iC){this.__iC=this._createDomElement();
this.__iC.$$element=this.$$hash;
this._copyData(false);

if(bj&&length>0){this._insertChildren();
}}else{this._syncData();

if(this.__iP){this._syncChildren();
}}delete this.__iP;
},_insertChildren:function(){var bl=this.__iO;
...
if(bn.__iC&&bn.__iD){bm.appendChild(bn.__iC);
}}this.__iC.appendChild(bm);
}else{var bm=this.__iC;
...
if(!bt||!bt.__iD||bt.__iQ!==this){br.removeChild(bw);
}}for(var i=0;i<bv;i++){bp=bo[i];
if(bp.__iD){bt=bp.__iC;
bw=bu[bq];
...
}}},_copyData:function(by){var bC=this.__iC;
var bB=this.__iv;
...
if(bG){var bL=this.__iM;

if(bL){var bJ;

for(var bK in bG){this._applyProperty(bK,bL[bK]);
}}this.__iL=null;
}},__iS:function(){var bM=this;
while(bM){if(bM.__cq){return true;
}
if(!bM.__iD||!bM.__iE){return false;
}bM=bM.__iQ;
}return false;
},__iT:function(bN){if(bN.__iQ===this){throw new Error("Child is already in: "+bN);
}
if(bN.__cq){throw new Error("Root elements could not be inserted into other ones.");
}if(bN.__iQ){bN.__iQ.remove(bN);
}bN.__iQ=this;
if(!this.__iO){this.__iO=[];
}if(this.__iC){this._scheduleChildrenUpdate();
}},__iU:function(bO){if(bO.__iQ!==this){throw new Error("Has no child: "+bO);
}if(this.__iC){this._scheduleChildrenUpdate();
}delete bO.__iQ;
},__iV:function(bP){if(bP.__iQ!==this){throw new Error("Has no child: "+bP);
}if(this.__iC){this._scheduleChildrenUpdate();
}},getChildren:function(){return this.__iO||null;
},getChild:function(bQ){var bR=this.__iO;
return bR&&bR[bQ]||null;
},hasChildren:function(){var bS=this.__iO;
return bS&&bS[0]!==undefined;
},indexOf:function(bT){var bU=this.__iO;
return bU?bU.indexOf(bT):-1;
},hasChild:function(bV){var bW=this.__iO;
return bW&&bW.indexOf(bV)!==-1;
},add:function(bX){if(arguments[1]){for(var i=0,l=arguments.length;i<l;i++){this.__iT(arguments[i]);
}this.__iO.push.apply(this.__iO,arguments);
}else{this.__iT(bX);
this.__iO.push(bX);
}return this;
},addAt:function(bY,ca){this.__iT(bY);
qx.lang.Array.insertAt(this.__iO,bY,ca);
return this;
},remove:function(cb){var cc=this.__iO;
...
},removeAt:function(ce){var cf=this.__iO;
...
qx.lang.Array.removeAt(this.__iO,ce);
return this;
},removeAll:function(){var ch=this.__iO;

if(ch){for(var i=0,l=ch.length;i<l;i++){this.__iU(ch[i]);
}ch.length=0;
}return this;
},getParent:function(){return this.__iQ||null;
},insertInto:function(parent,ci){parent.__iT(this);

if(ci==null){parent.__iO.push(this);
}else{qx.lang.Array.insertAt(this.__iO,this,ci);
}return this;
},insertBefore:function(cj){var parent=cj.__iQ;
parent.__iT(this);
qx.lang.Array.insertBefore(parent.__iO,this,cj);
return this;
},insertAfter:function(ck){var parent=ck.__iQ;
parent.__iT(this);
qx.lang.Array.insertAfter(parent.__iO,this,ck);
return this;
},moveTo:function(cl){var parent=this.__iQ;
parent.__iV(this);
var cm=parent.__iO.indexOf(this);

if(cm===cl){throw new Error("Could not move to same index!");
}else if(cm<cl){cl--;
}qx.lang.Array.removeAt(parent.__iO,cm);
qx.lang.Array.insertAt(parent.__iO,this,cl);
return this;
},moveBefore:function(cn){var parent=this.__iQ;
return this.moveTo(parent.__iO.indexOf(cn));
},moveAfter:function(co){var parent=this.__iQ;
return this.moveTo(parent.__iO.indexOf(co)+1);
},free:function(){var parent=this.__iQ;

if(!parent){throw new Error("Has no parent to remove from.");
}
if(!parent.__iO){return;
}parent.__iU(this);
qx.lang.Array.remove(parent.__iO,this);
return this;
},getDomElement:function(){return this.__iC||null;
},getNodeName:function(){return this.__it;
},setNodeName:function(name){this.__it=name;
},setRoot:function(cp){this.__cq=cp;
},useMarkup:function(cq){if(this.__iC){throw new Error("Could not overwrite existing element!");
...
return this.__iC;
},useElement:function(cs){if(this.__iC){throw new Error("Could not overwrite existing element!");
}this.__iC=cs;
this.__iC.$$element=this.$$hash;
this._copyData(true);
...
if(cu>=0&&ct[this.__it]){return true;
}return false;
...
}}),isNativelyFocusable:function(){return !!qx.event.handler.Focus.FOCUSABLE_ELEMENTS[this.__it];
},include:function(){if(this.__iD){return;
}delete this.__iD;

if(this.__iQ){this.__iQ._scheduleChildrenUpdate();
}return this;
},exclude:function(){if(!this.__iD){return;
}this.__iD=false;

if(this.__iQ){this.__iQ._scheduleChildrenUpdate();
}return this;
},isIncluded:function(){return this.__iD===true;
},show:function(){if(this.__iE){return;
}
if(this.__iC){qx.html.Element._visibility[this.$$hash]=this;
qx.html.Element._scheduleFlush(m);
}if(this.__iQ){this.__iQ._scheduleChildrenUpdate();
}delete this.__iE;
},hide:function(){if(!this.__iE){return;
}
if(this.__iC){qx.html.Element._visibility[this.$$hash]=this;
qx.html.Element._scheduleFlush(m);
}this.__iE=false;
},isVisible:function(){return this.__iE===true;
},scrollChildIntoViewX:function(cy,cz,cA){var cB=this.__iC;
var cC=cy.getDomElement();

if(cA!==false&&cB&&cB.offsetWidth&&cC&&cC.offsetWidth){qx.bom.element.Scroll.intoViewX(cC,cB,cz);
}else{this.__iF={element:cy,align:cz};
qx.html.Element._scroll[this.$$hash]=this;
qx.html.Element._scheduleFlush(m);
}delete this.__iH;
},scrollChildIntoViewY:function(cD,cE,cF){var cG=this.__iC;
var cH=cD.getDomElement();

if(cF!==false&&cG&&cG.offsetWidth&&cH&&cH.offsetWidth){qx.bom.element.Scroll.intoViewY(cH,cG,cE);
}else{this.__iG={element:cD,align:cE};
qx.html.Element._scroll[this.$$hash]=this;
qx.html.Element._scheduleFlush(m);
}delete this.__iI;
},scrollToX:function(x,cI){var cJ=this.__iC;
...
},getScrollX:function(){var cK=this.__iC;

if(cK){return cK.scrollLeft;
}return this.__iH||0;
},scrollToY:function(y,cL){var cM=this.__iC;
...
},getScrollY:function(){var cN=this.__iC;
...
this.scrollToX(0);
this.scrollToY(0);
this.addListener(G,this.__iX,this);
},enableScrolling:function(){this.removeListener(G,this.__iX,this);
},__iW:null,__iX:function(e){if(!this.__iW){this.__iW=true;
this.__iC.scrollTop=0;
this.__iC.scrollLeft=0;
delete this.__iW;
}},getTextSelection:function(){var cO=this.__iC;

if(cO){return qx.bom.Selection.get(cO);
}return null;
},getTextSelectionLength:function(){var cP=this.__iC;

if(cP){return qx.bom.Selection.getLength(cP);
}return null;
},getTextSelectionStart:function(){var cQ=this.__iC;

if(cQ){return qx.bom.Selection.getStart(cQ);
}return null;
},getTextSelectionEnd:function(){var cR=this.__iC;

if(cR){return qx.bom.Selection.getEnd(cR);
}return null;
},setTextSelection:function(cS,cT){var cU=this.__iC;

if(cU){qx.bom.Selection.set(cU,cS,cT);
return;
}qx.html.Element.__iw[this.toHashCode()]={element:this,start:cS,end:cT};
qx.html.Element._scheduleFlush(m);
},clearTextSelection:function(){var cV=this.__iC;

if(cV){qx.bom.Selection.clear(cV);
}delete qx.html.Element.__iw[this.toHashCode()];
},__iY:function(cW,cX){var cY=qx.html.Element._actions;
cY.push({type:cW,element:this,args:cX||[]});
qx.html.Element._scheduleFlush(m);
},focus:function(){this.__iY(q);
},blur:function(){this.__iY(C);
},activate:function(){this.__iY(D);
},deactivate:function(){this.__iY(B);
},capture:function(da){this.__iY(A,[da!==false]);
},releaseCapture:function(){this.__iY(u);
},setStyle:function(dc,dd,de){if(!this.__iu){this.__iu={};
}
if(this.__iu[dc]==dd){return;
}
if(dd==null){delete this.__iu[dc];
}else{this.__iu[dc]=dd;
}if(this.__iC){if(de){qx.bom.element.Style.set(this.__iC,dc,dd);
return this;
...
if(this.__iC){if(!this.__iJ){this.__iJ={};
}
for(var dj in df){var di=df[dj];

if(this.__iu[dj]==di){continue;
}
if(di==null){delete this.__iu[dj];
}else{this.__iu[dj]=di;
}if(dg){dh.set(this.__iC,dj,di);
continue;
}this.__iJ[dj]=true;
}qx.html.Element._modified[this.$$hash]=this;
qx.html.Element._scheduleFlush(m);
...
},getStyle:function(dm){return this.__iu?this.__iu[dm]:null;
},getAllStyles:function(){return this.__iu||null;
},setAttribute:function(dn,dp,dq){if(!this.__iv){this.__iv={};
}
if(this.__iv[dn]==dp){return;
}
if(dp==null){delete this.__iv[dn];
}else{this.__iv[dn]=dp;
}if(this.__iC){if(dq){qx.bom.element.Attribute.set(this.__iC,dn,dp);
return this;
...
},getAttribute:function(dw){return this.__iv?this.__iv[dw]:null;
},_applyProperty:function(name,dx){},_setProperty:function(dy,dz,dA){if(!this.__iM){this.__iM={};
}
if(this.__iM[dy]==dz){return;
}
if(dz==null){delete this.__iM[dy];
}else{this.__iM[dy]=dz;
}if(this.__iC){if(dA){this._applyProperty(dy,dz);
return this;
...
},_getProperty:function(dD){var dE=this.__iM;
...
if(this.__iC){return qx.event.Registration.addListener(this.__iC,dG,dH,self,dI);
}
if(!this.__iN){this.__iN={};
}
if(dI==null){dI=false;
}var dK=qx.event.Manager.getNextUniqueId();
var dL=dG+(dI?E:r)+dK;
this.__iN[dL]={type:dG,listener:dH,self:self,capture:dI,unique:dK};
return dL;
},removeListener:function(dM,dN,self,dO){var dP;

if(this.$$disposed){return null;
}
if(this.__iC){qx.event.Registration.removeListener(this.__iC,dM,dN,self,dO);
}else{var dR=this.__iN;
var dQ;
...
if(this.__iC){qx.event.Registration.removeListenerById(this.__iC,dT);
}else{delete this.__iN[dT];
}return this;
},hasListener:function(dU,dV){if(this.$$disposed){return false;
}
if(this.__iC){return qx.event.Registration.hasListener(this.__iC,dU,dV);
}var dX=this.__iN;
var dW;
...
}},defer:function(ea){ea.__eR=new qx.util.DeferredCall(ea.flush,ea);
},destruct:function(){var eb=this.__iC;

if(eb){qx.event.Registration.getManager(eb).removeAllListeners(eb);
eb.$$element=c;
}
if(!qx.core.ObjectRegistry.inShutDown){var parent=this.__iQ;

if(parent&&!parent.$$disposed){parent.remove(this);
}}this._disposeArray(t);
this.__iv=this.__iu=this.__iN=this.__iM=this.__iK=this.__iJ=this.__iL=this.__iC=this.__iQ=this.__iF=this.__iG=null;
}});
})();
...
l.target=k.target;
return l;
},__ih:{0:j,2:i,1:h},__ii:{1:j,2:i,4:h},stop:function(){this.stopPropagation();
...
}default:if(this._native.target!==undefined){return this.__ih[this._native.button]||g;
}else{return this.__ii[this._native.button]||g;
...
this._initObserver();
},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{orientationchange:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_WINDOW,IGNORE_CAN_HANDLE:true},members:{__x:null,__cp:null,__kB:null,__kC:null,__kD:null,canHandleEvent:function(g,h){},registerEvent:function(i,j,k){},unregisterEvent:function(l,m,n){},_initObserver:function(){this.__kD=qx.lang.Function.listener(this._onNative,this);
this.__kB=qx.bom.Event.supportsEvent(this.__cp,e)?e:d;
var Event=qx.bom.Event;
Event.addNativeListener(this.__cp,this.__kB,this.__kD);
},_stopObserver:function(){var Event=qx.bom.Event;
Event.removeNativeListener(this.__cp,this.__kB,this.__kD);
},_onNative:qx.event.GlobalError.observeMethod(function(o){var q=qx.bom.Viewport;
var p=q.getOrientation();

if(this.__kC!=p){this.__kC=p;
...
},isMultiTouch:function(){return this.__gF().length>1;
},getScale:function(){return this._native.scale;
},getRotation:function(){return this._native.rotation;
},getDocumentLeft:function(f){return this.__gE(f).pageX;
},getDocumentTop:function(g){return this.__gE(g).pageY;
},getScreenLeft:function(h){return this.__gE(h).screenX;
},getScreenTop:function(j){return this.__gE(j).screenY;
},getViewportLeft:function(k){return this.__gE(k).clientX;
},getViewportTop:function(l){return this.__gE(l).clientY;
},getIdentifier:function(m){return this.__gE(m).identifier;
},__gE:function(n){n=n==null?0:n;
return this.__gF()[n];
},__gF:function(){var o=(this._isTouchEnd()?this.getChangedTargetTouches():this.getTargetTouches());
return o;
...
qx.Class.define(n,{statics:{DEFAULT_SCROLLBAR_WIDTH:14,__do:null,getScrollbarWidth:function(){if(this.__do!==null){return this.__do;
...
(function(){var bY="px",bX="Boolean",bW="qx.event.type.Drag",bV="qx.event.type.Mouse",bU="visible",bT="qx.event.type.Focus",bS="Integer",bR="qx.event.type.Touch",bQ="qx.event.type.Data",bP="excluded",bx="_applyPadding",bw="qx.event.type.Event",bv="on",bu="hidden",bt="engine.name",bs="contextmenu",br="String",bq="tabIndex",bp="focused",bo="changeVisibility",cg="mshtml",ch="hovered",ce="qx.event.type.KeySequence",cf="absolute",cc="backgroundColor",cd="drag",ca="div",cb="disabled",ci="move",cj="dragstart",bI="qx.dynlocale",bH="dragchange",bK="dragend",bJ="resize",bM="Decorator",bL="zIndex",bO="opacity",bN="default",bG="Color",bF="changeToolTipText",c="beforeContextmenuOpen",d="_applyNativeContextMenu",f="_applyBackgroundColor",g="event.pointer",h="_applyFocusable",j="changeShadow",k="qx.event.type.KeyInput",m="createChildControl",n="__jj",o="Font",cn="_applyShadow",cm="_applyEnabled",cl="_applySelectable",ck="Number",cr="_applyKeepActive",cq="_applyVisibility",cp="repeat",co="qxDraggable",ct="syncAppearance",cs="paddingLeft",N="_applyDroppable",O="#",L="qx.event.type.MouseWheel",M="_applyCursor",R="_applyDraggable",S="changeTextColor",P="$$widget",Q="changeContextMenu",J="paddingTop",K="changeSelectable",w="hideFocus",v="none",y="__ja",x="outline",s="_applyAppearance",r="__jn",u="_applyOpacity",t="__jl",q="url(",p=")",X="qx.ui.core.Widget",Y="_applyFont",ba="cursor",bb="qxDroppable",T="__jf",U="changeZIndex",V="changeEnabled",W="changeFont",bc="_applyDecorator",bd="_applyZIndex",G="_applyTextColor",F="qx.ui.menu.Menu",E="__jg",D="_applyToolTipText",C="true",B="widget",A="changeDecorator",z="__jb",I="_applyTabIndex",H="__je",be="changeAppearance",bf="shorthand",bg="/",bh="",bi="_applyContextMenu",bj="paddingBottom",bk="changeNativeContextMenu",bl="undefined",bm="qx.ui.tooltip.ToolTip",bn="qxKeepActive",bB="_applyKeepFocus",bA="paddingRight",bz="changeBackgroundColor",by="changeLocale",bE="qxKeepFocus",bD="opera",bC="qx/static/blank.gif";
...
this.__ja=this._createContainerElement();
this.__jb=this.__jm();
this.__ja.add(this.__jb);
this.initFocusable();
this.initSelectable();
this.initNativeContextMenu();
},events:{appear:bw,disappear:bw,createChildControl:bQ,resize:bQ,move:bQ,syncAppearance:bQ,mousemove:bV,mouseover:bV,mouseout:bV,mousedown:bV,mouseup:bV,click:bV,dblclick:bV,contextmenu:bV,beforeContextmenuOpen:bQ,mousewheel:L,touchstart:bR,touchend:bR,touchmove:bR,touchcancel:bR,tap:bR,swipe:bR,keyup:ce,keydown:ce,keypress:ce,keyinput:k,focus:bT,blur:bT,focusin:bT,focusout:bT,activate:bT,deactivate:bT,capture:bw,losecapture:bw,drop:bW,dragleave:bW,dragover:bW,drag:bW,dragstart:bW,dragend:bW,dragchange:bW,droprequest:bW},properties:{paddingTop:{check:bS,init:0,apply:bx,themeable:true},paddingRight:{check:bS,init:0,apply:bx,themeable:true},paddingBottom:{check:bS,init:0,apply:bx,themeable:true},paddingLeft:{check:bS,init:0,apply:bx,themeable:true},padding:{group:[J,bA,bj,cs],mode:bf,themeable:true},zIndex:{nullable:true,init:null,apply:bd,event:U,check:bS,themeable:true},decorator:{nullable:true,init:null,apply:bc,event:A,check:bM,themeable:true},shadow:{nullable:true,init:null,apply:cn,event:j,check:bM,themeable:true},backgroundColor:{nullable:true,check:bG,apply:f,event:bz,themeable:true},textColor:{nullable:true,check:bG,apply:G,event:S,themeable:true,inheritable:true},font:{nullable:true,apply:Y,check:o,event:W,themeable:true,inheritable:true,dereference:true},opacity:{check:ck,apply:u,themeable:true,nullable:true,init:null},cursor:{check:br,apply:M,themeable:true,inheritable:true,nullable:true,init:null},toolTip:{check:bm,nullable:true},toolTipText:{check:br,nullable:true,event:bF,apply:D},toolTipIcon:{check:br,nullable:true,event:bF},blockToolTip:{check:bX,init:false},visibility:{check:[bU,bu,bP],init:bU,apply:cq,event:bo},enabled:{init:true,check:bX,inheritable:true,apply:cm,event:V},anonymous:{init:false,check:bX},tabIndex:{check:bS,nullable:true,apply:I},focusable:{check:bX,init:false,apply:h},keepFocus:{check:bX,init:false,apply:bB},keepActive:{check:bX,init:false,apply:cr},draggable:{check:bX,init:false,apply:R},droppable:{check:bX,init:false,apply:N},selectable:{check:bX,init:false,event:K,apply:cl},contextMenu:{check:F,apply:bi,nullable:true,event:Q},nativeContextMenu:{check:bX,init:false,themeable:true,event:bk,apply:d},appearance:{check:br,init:B,apply:s,event:be}},statics:{DEBUG:false,getWidgetByElement:function(cu,cv){while(cu){var cw=cu.$$widget;
...
},_setLayout:function(cz){if(this.__jj){this.__jj.connectToWidget(null);
}
...
if(cP.size){var cO=this.__jg;

if(cO){cO.setStyles({width:cF+bY,height:cG+bY});
}}
if(cP.size||this._updateInsets){if(this.__je){this.__je.resize(cF,cG);
}}
if(cP.size){if(this.__jf){var cH=this.__jf.getInsets();
var cL=cF+cH.left+cH.right;
var cJ=cG+cH.top+cH.bottom;
this.__jf.resize(cL,cJ);
}}
if(cM||cP.local||cP.margin){if(this.__jj&&this.hasLayoutChildren()){this.__jj.renderLayout(innerWidth,innerHeight);
...
return cP;
},__jl:null,clearSeparators:function(){var cS=this.__jl;

if(!cS){return;
}var cT=qx.ui.core.Widget.__jc;
var content=this.getContentElement();
var cR;
...
},renderSeparator:function(cU,cV){var cW=qx.ui.core.Widget.__jc.getDecoratorElement(cU);
this.getContentElement().add(cW);
cW.resize(cV.width,cV.height);
cW.setStyles({left:cV.left+bY,top:cV.top+bY});
if(!this.__jl){this.__jl=[cW];
}else{this.__jl.push(cW);
...
if(this.__jj){this.__jj.invalidateLayoutCache();
}},_getContentHint:function(){var dj=this.__jj;
...
if(this.__je){var dv=this.__je.getInsets();
top+=dv.top;
dw+=dv.right;
dy+=dv.bottom;
dx+=dv.left;
...
},__jm:function(){var dE=this._createContentElement();
...
},__jn:null,getLayoutChildren:function(){var dG=this.__jn;

if(!dG){return this.__jo;
}var dH;
...
},invalidateLayoutChildren:function(){var dI=this.__jj;

if(dI){dI.invalidateChildrenCache();
}qx.ui.core.queue.Layout.add(this);
},hasLayoutChildren:function(){var dJ=this.__jn;
...
},_indexOf:function(dL){var dM=this.__jn;

if(!dM){return -1;
}return dM.indexOf(dL);
},_hasChildren:function(){var dN=this.__jn;
return dN!=null&&(!!dN[0]);
},addChildrenToQueue:function(dO){var dP=this.__jn;
...
}},_add:function(dR,dS){if(dR.getLayoutParent()==this){qx.lang.Array.remove(this.__jn,dR);
}
if(this.__jn){this.__jn.push(dR);
}else{this.__jn=[dR];
}this.__jp(dR,dS);
},_addAt:function(dT,dU,dV){if(!this.__jn){this.__jn=[];
}if(dT.getLayoutParent()==this){qx.lang.Array.remove(this.__jn,dT);
}var dW=this.__jn[dU];

if(dW===dT){dT.setLayoutProperties(dV);
}
if(dW){qx.lang.Array.insertBefore(this.__jn,dT,dW);
}else{this.__jn.push(dT);
}this.__jp(dT,dV);
},_addBefore:function(dX,dY,ea){if(dX==dY){return;
}
if(!this.__jn){this.__jn=[];
}if(dX.getLayoutParent()==this){qx.lang.Array.remove(this.__jn,dX);
}qx.lang.Array.insertBefore(this.__jn,dX,dY);
this.__jp(dX,ea);
},_addAfter:function(eb,ec,ed){if(eb==ec){return;
}
if(!this.__jn){this.__jn=[];
}if(eb.getLayoutParent()==this){qx.lang.Array.remove(this.__jn,eb);
}qx.lang.Array.insertAfter(this.__jn,eb,ec);
this.__jp(eb,ed);
},_remove:function(ee){if(!this.__jn){throw new Error("This widget has no children!");
}qx.lang.Array.remove(this.__jn,ee);
this.__jq(ee);
},_removeAt:function(ef){if(!this.__jn){throw new Error("This widget has no children!");
}var eg=this.__jn[ef];
qx.lang.Array.removeAt(this.__jn,ef);
this.__jq(eg);
return eg;
},_removeAll:function(){if(!this.__jn){return [];
}var eh=this.__jn.concat();
this.__jn.length=0;

for(var i=eh.length-1;i>=0;i--){this.__jq(eh[i]);
}qx.ui.core.queue.Layout.add(this);
return eh;
},_afterAddChild:null,_afterRemoveChild:null,__jp:function(ei,ej){var parent=ei.getLayoutParent();
...
}},__jq:function(ek){if(ek.getLayoutParent()!==this){throw new Error("Remove Error: "+ek+" is not a child of this widget!");
}ek.setLayoutParent(null);
if(this.__jj){this.__jj.invalidateChildrenCache();
}qx.ui.core.queue.Layout.add(this);
...
}var ep=this.__jg=new qx.html.Element;
ep.setStyles({position:cf,top:0,left:0,zIndex:7});
var eq=this.getBounds();

if(eq){this.__jg.setStyles({width:eq.width+bY,height:eq.height+bY});
...
},_applyDecorator:function(er,es){var ev=qx.ui.core.Widget.__jc;
var et=this.getContainerElement();
if(!this.__jg&&!qx.core.Environment.get(g)){this._createProtectorElement();
}if(es){et.remove(this.__je);
ev.poolDecorator(this.__je);
}if(er){var eu=this.__je=ev.getDecoratorElement(er);
eu.setStyle(bL,5);
et.add(eu);
}else{delete this.__je;
}this._applyBackgroundColor(this.getBackgroundColor());
if(this.__jk(es,er)){this._updateInsets=true;
qx.ui.core.queue.Layout.add(this);
}else if(er){var ew=this.getBounds();

if(ew){eu.resize(ew.width,ew.height);
this.__jg&&this.__jg.setStyles({width:ew.width+bY,height:ew.height+bY});
}}},_applyShadow:function(ex,ey){var eF=qx.ui.core.Widget.__jd;
var eA=this.getContainerElement();
if(ey){eA.remove(this.__jf);
eF.poolDecorator(this.__jf);
}if(ex){var eC=this.__jf=eF.getDecoratorElement(ex);
eA.add(eC);
var eE=eC.getInsets();
...
}},_applyToolTipText:function(eG,eH){if(qx.core.Environment.get(bI)){if(this.__ji){return;
}var eI=qx.locale.Manager.getInstance();
this.__ji=eI.addListener(by,function(){var eJ=this.getToolTipText();
...
if(this.__je){this.__je.tint(eY);
fb.setStyle(cc,null);
}else{var fa=qx.theme.manager.Color.getInstance().resolve(eY);
fb.setStyle(cc,fa);
}},_applyFont:function(fc,fd){},__jr:null,$$stateChanges:null,_forwardStates:null,hasState:function(fe){var ff=this.__jr;
return !!ff&&!!ff[fe];
},addState:function(fg){var fh=this.__jr;
...
}}}},removeState:function(fl){var fm=this.__jr;
...
}}}},replaceState:function(fq,fr){var fs=this.__jr;
...
}}}},__js:null,__jt:null,syncAppearance:function(){var fA=this.__jr;
var fz=this.__js;
...
}}this.fireDataEvent(ct,this.__jr);
},_applyAppearance:function(fG,fH){this.updateAppearance();
},checkAppearanceNeeds:function(){if(!this.__jh){qx.ui.core.queue.Appearance.add(this);
this.__jh=true;
}else if(this.$$stateChanges){qx.ui.core.queue.Appearance.add(this);
delete this.$$stateChanges;
}},updateAppearance:function(){this.__jt=true;
...
if((qx.core.Environment.get(bt)==cg)){fP.setAttribute(w,C);
}else{fP.setStyle(x,v);
...
},hasChildControl:function(gx){if(!this.__ju){return false;
}return !!this.__ju[gx];
},__ju:null,_getCreatedChildControls:function(){return this.__ju;
},getChildControl:function(gy,gz){if(!this.__ju){if(gz){return null;
}this.__ju={};
}var gA=this.__ju[gy];
...
},_createChildControl:function(gH){if(!this.__ju){this.__ju={};
}else if(this.__ju[gH]){throw new Error("Child control '"+gH+"' already created!");
}var gL=gH.indexOf(O);
...
return this.__ju[gH]=gI;
},_createChildControlImpl:function(gM,gN){return null;
},_disposeChildControls:function(){var gR=this.__ju;
...
}},destruct:function(){if(!qx.core.ObjectRegistry.inShutDown){if(qx.core.Environment.get(bI)){if(this.__ji){qx.locale.Manager.getInstance().removeListenerById(this.__ji);
...
if(this.__je){hg.remove(this.__je);
hh.__jc.poolDecorator(this.__je);
}
if(this.__jf){hg.remove(this.__jf);
hh.__jd.poolDecorator(this.__jf);
}this.clearSeparators();
this.__je=this.__jf=this.__jl=null;
}else{this._disposeArray(t);
this._disposeObjects(H,T);
}this._disposeArray(r);
this.__jr=this.__ju=null;
this._disposeObjects(n,y,z,E);
}});
})();
...
qx.Class.define(a,{statics:{getApplication:qx.core.BaseInit.getApplication,ready:qx.core.BaseInit.ready,__ed:function(e){var f=this.getApplication();

if(f){e.setReturnValue(f.close());
}},__ee:function(){var g=this.getApplication();
...
qx.Class.define(c,{statics:{__hb:qx.core.Environment.select(g,{"mshtml":{"cursor":a,"ew-resize":j,"ns-resize":k,"nesw-resize":h,"nwse-resize":i},"opera":{"col-resize":j,"row-resize":k,"ew-resize":j,"ns-resize":k,"nesw-resize":h,"nwse-resize":i},"default":{}}),compile:function(l){return e+(this.__hb[l]||l)+d;
},get:function(m,n){return qx.bom.element.Style.get(m,b,n,false);
},set:function(o,p){o.style.cursor=this.__hb[p]||p;
...
qx.Class.define(m,{statics:{__il:qx.core.Environment.select(q,{"mshtml":null,"webkit":[o,k,g],"gecko":[a],"opera":[o]}),__im:qx.core.Environment.select(q,{"mshtml":null,"webkit":[n,e,b],"gecko":[h],"opera":[n]}),__in:{tags:{button:true,select:true},types:{search:true,button:true,submit:true,reset:true,checkbox:true,radio:true}},__io:function(r){var s=this.__in;
return s.tags[r.tagName.toLowerCase()]||s.types[r.type];
},compile:qx.core.Environment.select(q,{"mshtml":function(t){},"default":function(u){var w=this.__im;
var v=p;

if(w){for(var i=0,l=w.length;i<l;i++){v+=w[i]+f+u+c;
}}return v;
}}),get:qx.core.Environment.select(q,{"mshtml":function(x){if(qx.bom.Document.isStandardMode(qx.dom.Node.getWindow(x))){if(!this.__io(x)){return d;
}}return j;
},"default":function(y){var A=this.__il;
var z;
...
}}),set:qx.core.Environment.select(q,{"mshtml":function(B,C){},"default":function(D,E){var F=this.__il;
...
this.__gC={fontFamily:k,fontSize:null,fontWeight:null,fontStyle:null,textDecoration:null,lineHeight:null,color:null};
...
I.set(H);
return I;
},__gD:{fontFamily:k,fontSize:k,fontWeight:k,fontStyle:k,textDecoration:k,lineHeight:1.2,color:k},getDefaultStyles:function(){return this.__gD;
}},properties:{size:{check:z,nullable:true,apply:p},lineHeight:{check:r,nullable:true,apply:x},family:{check:w,nullable:true,apply:y},bold:{check:h,nullable:true,apply:a},italic:{check:h,nullable:true,apply:b},decoration:{check:[j,v,u],nullable:true,apply:q},color:{check:t,nullable:true,apply:n}},members:{__gC:null,_applySize:function(J,K){this.__gC.fontSize=J===null?null:J+g;
},_applyLineHeight:function(L,M){this.__gC.lineHeight=L===null?null:L;
...
if(i!==l-1){P+=m;
}}this.__gC.fontFamily=P;
},_applyBold:function(Q,R){this.__gC.fontWeight=Q===null?null:Q?c:d;
},_applyItalic:function(S,T){this.__gC.fontStyle=S===null?null:S?e:d;
},_applyDecoration:function(U,V){this.__gC.textDecoration=U===null?null:U;
},_applyColor:function(W,X){this.__gC.color=W===null?null:W;
},getStyles:function(){return this.__gC;
}}});
})();
...
this.__gG={};
this.__gH={};
},properties:{theme:{check:d,nullable:true,event:c,apply:g}},members:{__gI:{},__gG:null,__gH:null,_applyTheme:function(j,k){this.__gH={};
this.__gG={};
},__gJ:function(l,m,n){var s=m.appearances;
var v=s[l];
...
return this.__gJ(r,m,n);
}}}for(var i=0;i<p.length-1;i++){p.shift();
var q=p.join(w);
var o=this.__gJ(q,m);

if(o){return o;
}}if(n!=null){return this.__gJ(n,m);
}return null;
}else if(typeof v===h){return this.__gJ(v,m,n);
}else if(v.include&&!v.style){return this.__gJ(v.include,m,n);
}return l;
},styleFrom:function(x,y,z,A){if(!z){z=this.getTheme();
}var F=this.__gH;
var B=F[x];

if(!B){B=F[x]=this.__gJ(x,z,A);
...
this.__gL={};
this.__db=[];
this.__gM=this.getPreferredFormats();
},statics:{FONT_FORMATS:["eot","woff","ttf","svg"],VALIDATION_TIMEOUT:5000},members:{__gK:null,__gN:null,__gL:null,__gM:null,__db:null,__gO:null,require:function(U,V,W,X){var Y=[];
...
}if(!(qx.core.Environment.get(g)==h&&qx.bom.client.Browser.getVersion()<9)){this.__gP(U,Y,W,X);
return;
}
if(!this.__gO){this.__gO=new qx.event.Timer(100);
this.__gO.addListener(q,this.__gQ,this);
}
if(!this.__gO.isEnabled()){this.__gO.start();
}this.__db.push([U,Y,W,X]);
},remove:function(bc){var bd=null;

for(var i=0,l=this.__gK.length;i<l;i++){if(this.__gK[i]==bc){bd=i;
this.__gW(bc);
break;
}}
if(bd){qx.lang.Array.removeAt(this.__gK,bd);
}
if(bc in this.__gL){this.__gL[bc].dispose();
delete this.__gL[bc];
...
},removeStyleSheet:function(){this.__gK=[];

if(this.__gN){var bj=this.__gN.ownerNode?this.__gN.ownerNode:this.__gN.owningElement;
qx.dom.Element.removeChild(bj,bj.parentNode);
}this.__gN=null;
},__gP:function(bk,bl,bm,bn){if(!qx.lang.Array.contains(this.__gK,bk)){var bq=this.__gS(bl);
var bp=this.__gT(bk,bq);

if(!bp){throw new Error("Couldn't create @font-face rule for WebFont "+bk+"!");
}
if(!this.__gN){this.__gN=qx.bom.Stylesheet.createElement();
}
try{this.__gV(bp);
}catch(br){}this.__gK.push(bk);
}
if(!this.__gL[bk]){this.__gL[bk]=new qx.bom.webfonts.Validator(bk);
this.__gL[bk].setTimeout(qx.bom.webfonts.Manager.VALIDATION_TIMEOUT);
this.__gL[bk].addListenerOnce(f,this.__gR,this);
}
if(bm){var bo=bn||window;
this.__gL[bk].addListenerOnce(f,bm,bo);
}this.__gL[bk].validate();
},__gQ:function(){if(this.__db.length==0){this.__gO.stop();
return;
}var bs=this.__db.shift();
this.__gP.apply(this,bs);
},__gR:function(bt){var bu=bt.getData();

if(bu.valid===false){qx.event.Timer.once(function(){this.remove(bu.family);
},this,250);
}},__gS:function(bv){var bx=qx.bom.webfonts.Manager.FONT_FORMATS;
var bA={};
...
},__gT:function(bB,bC){var bF=[];
var bD=this.__gM.length>0?this.__gM:qx.bom.webfonts.Manager.FONT_FORMATS;

for(var i=0,l=bD.length;i<l;i++){var bE=bD[i];

if(bC[bE]){bF.push(this.__gU(bE,bC[bE]));
}}var bG=u+bF.join(K)+H;
bG=o+bB+C+bG;
bG=bG+J;
return bG;
},__gU:function(bH,bI){switch(bH){case a:return k+bI+y;
case Q:return k+bI+B;
case S:return k+bI+z;
case e:return k+bI+F;
default:return null;
}},__gV:function(bJ){var bL=t+bJ+r;

if(qx.core.Environment.get(g)==h&&qx.core.Environment.get(R)<9){var bK=this.__gX(this.__gN.cssText);
bK+=bL;
this.__gN.cssText=bK;
}else{this.__gN.insertRule(bL,this.__gN.cssRules.length);
}},__gW:function(bM){var bP=new RegExp(I+bM,P);
...
if(bP.exec(bO)){this.__gN.deleteRule(j);
return;
}}}}},__gX:function(bQ){return bQ.replace(s,A);
}},destruct:function(){delete this.__gK;
this.removeStyleSheet();

for(var bR in this.__gL){this.__gL[bR].dispose();
...
this.__gY=c;
this.__ha=d;
},members:{__gY:null,__ha:null,translate:function(){return qx.locale.Manager.getInstance().translate(this.__gY,this.__ha);
}}});
})();
...
qx.Mixin.define(w,{statics:{__iE:null,setVisibleElement:function(y){this.__iE=y;
},getVisibleElement:function(){return this.__iE;
}},properties:{position:{check:[r,o,d,a,v,l,k,n],init:d,themeable:true},placeMethod:{check:[x,e],init:e,themeable:true},domMove:{check:b,init:false},placementModeX:{check:[c,h,f],init:h,themeable:true},placementModeY:{check:[c,h,f],init:h,themeable:true},offsetLeft:{check:j,init:0,themeable:true},offsetTop:{check:j,init:0,themeable:true},offsetRight:{check:j,init:0,themeable:true},offsetBottom:{check:j,init:0,themeable:true},offset:{group:[m,u,p,s],mode:t,themeable:true}},members:{__kI:null,__kJ:null,__kK:null,getLayoutLocation:function(z){var C,B,D,top;
B=z.getBounds();
D=B.left;
top=B.top;
var E=B;
...
}},placeToWidget:function(L,M){if(M){this.__kL();
this.__kI=qx.lang.Function.bind(this.placeToWidget,this,L,false);
qx.event.Idle.getInstance().addListener(i,this.__kI);
this.__kK=function(){this.__kL();
};
this.addListener(g,this.__kK,this);
}var N=L.getContainerLocation()||this.getLayoutLocation(L);
this.__kN(N);
},__kL:function(){if(this.__kI){qx.event.Idle.getInstance().removeListener(i,this.__kI);
this.__kI=null;
}
if(this.__kK){this.removeListener(g,this.__kK,this);
this.__kK=null;
...
if(R){this.__kI=qx.lang.Function.bind(this.placeToElement,this,Q,false);
qx.event.Idle.getInstance().addListener(i,this.__kI);
this.addListener(g,function(){if(this.__kI){qx.event.Idle.getInstance().removeListener(i,this.__kI);
this.__kI=null;
}},this);
}this.__kN(S);
...
},__kM:function(V){var W=null;

if(this._computePlacementSize){var W=this._computePlacementSize();
}else if(this.isVisible()){var W=this.getBounds();
}
if(W==null){this.addListenerOnce(q,function(){this.__kM(V);
},this);
}else{V.call(this,W);
}},__kN:function(X){this.__kM(function(Y){var ba=qx.util.placement.Placement.compute(Y,this.getLayoutParent().getBounds(),X,this._getPlacementOffsets(),this.getPosition(),this.getPlacementModeX(),this.getPlacementModeY());
this.moveTo(ba.left,ba.top);
});
}},destruct:function(){this.__kL();
}});
})();
(function(){var j="nonScaled",i="scaled",h="alphaScaled",g=".png",f="div",e="replacement",d="qx.event.type.Event",c="engine.name",b="hidden",a="Boolean",z="px",y="engine.version",x="scale",w="changeSource",v="qx.ui.basic.Image",u="loaded",t="-disabled.$1",s="loadingFailed",r="__kU",q="String",o="_applySource",p="img",m="image",n="mshtml",k="_applyScale",l="no-repeat";
qx.Class.define(v,{extend:qx.ui.core.Widget,construct:function(A){this.__kU={};
qx.ui.core.Widget.call(this);

if(A){this.setSource(A);
}},properties:{source:{check:q,init:null,nullable:true,event:w,apply:o,themeable:true},scale:{check:a,init:false,themeable:true,apply:k},appearance:{refine:true,init:m},allowShrinkX:{refine:true,init:false},allowShrinkY:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false}},events:{loadingFailed:d,loaded:d},members:{__kV:null,__kW:null,__eg:null,__kU:null,getContentElement:function(){return this.__lb();
},_createContentElement:function(){return this.__lb();
},_getContentHint:function(){return {width:this.__kV||0,height:this.__kW||0};
...
},__kX:function(F){this.__eg=F;
},__kY:function(){if(this.__eg==null){var H=this.getSource();
var G=false;

if(H!=null){G=qx.lang.String.endsWith(H,g);
}
if(this.getScale()&&G&&qx.bom.element.Decoration.isAlphaImageLoaderEnabled()){this.__eg=h;
}else if(this.getScale()){this.__eg=i;
}else{this.__eg=j;
}}return this.__eg;
},__la:function(I){var J;
var K;
...
return L;
},__lb:function(){var M=this.__kY();

if(this.__kU[M]==null){this.__kU[M]=this.__la(M);
}return this.__kU[M];
...
}if(qx.util.ResourceManager.getInstance().has(N)){this.__le(this.getContentElement(),N);
}else if(qx.io.ImageLoader.isLoaded(N)){this.__lf(this.getContentElement(),N);
}else{this.__lg(this.getContentElement(),N);
}},__lc:qx.core.Environment.select(c,{"mshtml":function(P){var R=qx.bom.element.Decoration.isAlphaImageLoaderEnabled();
var Q=qx.lang.String.endsWith(P,g);

if(R&&Q){if(this.getScale()&&this.__kY()!=h){this.__kX(h);
}else if(!this.getScale()&&this.__kY()!=j){this.__kX(j);
}}else{if(this.getScale()&&this.__kY()!=i){this.__kX(i);
}else if(!this.getScale()&&this.__kY()!=j){this.__kX(j);
}}this.__ld(this.__lb());
},"default":function(S){if(this.getScale()&&this.__kY()!=i){this.__kX(i);
}else if(!this.getScale()&&this.__kY(j)){this.__kX(j);
}this.__ld(this.__lb());
}}),__ld:function(T){var W=this.getContainerElement();
var X=W.getChild(0);
...
W.addAt(T,0);
}},__le:function(bb,bc){var be=qx.util.ResourceManager.getInstance();
...
this.__li(be.getImageWidth(bc),be.getImageHeight(bc));
},__lf:function(bf,bg){var bi=qx.io.ImageLoader;
bf.setSource(bg);
var bh=bi.getWidth(bg);
var bj=bi.getHeight(bg);
this.__li(bh,bj);
},__lg:function(bk,bl){var self;
var bm=qx.io.ImageLoader;
if(!bm.isFailed(bl)){bm.load(bl,this.__lh,this);
}else{if(bk!=null){bk.resetSource();
}}},__lh:function(bn,bo){if(this.$$disposed===true){return;
...
},__li:function(bp,bq){if(bp!==this.__kV||bq!==this.__kW){this.__kV=bp;
this.__kW=bq;
qx.ui.core.queue.Layout.add(this);
...
qx.Class.define(G,{statics:{__hb:{"Array":[f,p,E,q,z,d,a,y,g,h,k,j,n,v,b,D,o],"String":[H,A,w,t,C,r,k,j,x,e,F,I,m,u,B,c,g,h]},__hc:function(J,K){return function(s){return J.prototype[K].apply(s,Array.prototype.slice.call(arguments,1));
};
},__dl:function(){var L=qx.lang.Generics.__hb;
...
if(!N[O]){N[O]=qx.lang.Generics.__hc(N,O);
...
qx.Mixin.define(d,{construct:function(){if(qx.core.Environment.get(l)){qx.locale.Manager.getInstance().addListener(j,this.__hd,this);
...
},__hd:qx.core.Environment.select(l,{"true":function(e){var o=this.getInvalidMessage();
...
}},"false":null})},destruct:function(){if(qx.core.Environment.get(l)){qx.locale.Manager.getInstance().removeListener(j,this.__hd,this);
}}});
})();
...
if(k!==null&&k.fonts[i]){var m=this.__hf(k.fonts[i]);
...
if(p!==null&&n&&p.fonts[n]){var o=this.__hf(p.fonts[n]);
q[n]=(new o).set(p.fonts[n]);
return true;
}return false;
},__he:function(r,s){if(r[s].include){var t=r[r[s].include];
r[s].include=null;
delete r[s].include;
...
for(var y in w){if(w[y].include&&w[w[y].include]){this.__he(w,y);
}var x=this.__hf(w[y]);
v[y]=(new x).set(w[y]);
v[y].themed=true;
}}this._setDynamic(v);
},__hf:function(z){if(z.sources){return qx.bom.webfonts.WebFont;
}return qx.bom.Font;
...
},statics:{__hi:qx.$$resources||{},__ij:{}},members:{has:function(q){return !!this.self(arguments).__hi[q];
},getData:function(r){return this.self(arguments).__hi[r]||null;
},getImageWidth:function(s){var t=this.self(arguments).__hi[s];
return t?t[0]:null;
},getImageHeight:function(u){var v=this.self(arguments).__hi[u];
return v?v[1]:null;
},getImageFormat:function(w){var x=this.self(arguments).__hi[w];
return x?x[2]:null;
},getCombinedFormat:function(y){var B=p;
var A=this.self(arguments).__hi[y];
var z=A&&A.length>4&&typeof (A[4])==k&&this.constructor.__hi[A[4]];

if(z){var D=A[4];
var C=this.constructor.__hi[D];
B=C[2];
}return B;
},toUri:function(E){if(E==null){return E;
}var F=this.self(arguments).__hi[E];
...
if((qx.core.Environment.get(m)==n)&&qx.core.Environment.get(l)){G=this.self(arguments).__ij[H];
}return G+qx.$$libraries[H].resourceUri+o+E;
},toDataUri:function(I){var K=this.constructor.__hi[I];
var L=this.constructor.__hi[K[4]];
var M;
...
continue;
}if(P.match(/^\/\//)!=null){N.__ij[R]=window.location.protocol;
}else if(P.match(/^\//)!=null){N.__ij[R]=window.location.protocol+j+window.location.host;
}else if(P.match(/^\.\//)!=null){var O=document.URL;
N.__ij[R]=O.substring(0,O.lastIndexOf(o)+1);
}else if(P.match(/^http/)!=null){N.__ij[R]=p;
}else{var S=window.location.href.indexOf(i);
var Q;

if(S==-1){Q=window.location.href;
}else{Q=window.location.href.substring(0,S);
}N.__ij[R]=Q.substring(0,Q.lastIndexOf(o)+1);
}}}}}});
})();
...
}},properties:{spacingX:{check:h,init:0,apply:p},spacingY:{check:h,init:0,apply:p}},members:{__hp:null,__hn:null,__ho:null,__hq:null,__hr:null,__hs:null,__ht:null,__hu:null,__hv:null,verifyLayoutProperty:null,__hw:function(){var B=[];
var A=[];
var C=[];
var w=-1;
var v=-1;
...
},_setRowData:function(G,H,I){var J=this.__hn[G];

if(!J){this.__hn[G]={};
this.__hn[G][H]=I;
}else{J[H]=I;
}},_setColumnData:function(K,L,M){var N=this.__ho[K];
...
for(var i=0,l=this.__hr.length;i<l;i++){var cl=this.__hr[i];
var ch=this.__hx(cl);
...
for(var i=0,l=this.__hq.length;i<l;i++){var ct=this.__hq[i];
var cv=this.__hx(ct);
...
for(var cO=0;cO<=cH;cO++){var cG=this.__hp[cQ][cO];
...
if(this.__hr.length>0){this._fixHeightsRowSpan(cP);
}this.__hu=cP;
return cP;
...
for(var dc=0;dc<=cU;dc++){var cR=this.__hp[dc][db];
...
if(this.__hq.length>0){this._fixWidthsColSpan(cV);
}this.__hv=cV;
return cV;
...
},renderLayout:function(ds,dt){if(this._invalidChildrenCache){this.__hw();
...
for(var dC=0;dC<=du;dC++){var dJ=this.__hp[dC][dP];
if(!dJ){top+=dV[dC]+dB;
continue;
...
}},properties:{locale:{check:b,nullable:true,apply:h,event:g}},members:{__hB:f,__hC:null,__hD:null,__hy:null,__hz:null,__hA:null,getLanguage:function(){return this.__hD;
},getTerritory:function(){return this.getLocale().split(l)[1]||k;
},getAvailableLocales:function(){var C=[];

for(var B in this.__hz){if(B!=this.__hB){C.push(B);
}}return C;
},__hE:function(D){var F;
...
},_applyLocale:function(G,H){this.__hC=G;
this.__hD=this.__hE(G);
},addTranslation:function(I,J){var K=this.__hy;

if(K[I]){for(var L in J){K[I][L]=J[L];
}}else{K[I]=J;
}},addLocale:function(M,N){var O=this.__hz;

if(O[M]){for(var P in N){O[M][P]=N[P];
}}else{O[M]=N;
}},translate:function(Q,R,S){var T=this.__hy;
return this.__hF(T,Q,R,S);
},localize:function(U,V,W){var X=this.__hz;
return this.__hF(X,U,V,W);
},__hF:function(Y,ba,bb,bc){var bd;

if(!Y){return ba;
}
if(bc){var bf=this.__hE(bc);
}else{bc=this.__hC;
bf=this.__hD;
}if(!bd&&Y[bc]){bd=Y[bc][ba];
}if(!bd&&Y[bf]){bd=Y[bf][ba];
}if(!bd&&Y[this.__hB]){bd=Y[this.__hB][ba];
}
if(!bd){bd=ba;
}
...
}}},members:{__ks:null,__x:null,__cp:null,__cq:null,__kr:null,__kt:null,__ku:null,__kv:null,canHandleEvent:function(bB,bC){},registerEvent:function(bD,bE,bF){},unregisterEvent:function(bG,bH,bI){},_fireInputEvent:function(bJ,bK){var bL=this.__kw();
...
}},_fireSequenceEvent:function(bM,bN,bO){var bP=this.__kw();
var bQ=bM.keyCode;
...
}},__kw:function(){var bR=this.__x.getHandler(qx.event.handler.Focus);
var bS=bR.getActive();
if(!bS||bS.offsetWidth==0){bS=bR.getFocus();
}if(!bS||bS.offsetWidth==0){bS=this.__x.getWindow().document.body;
}return bS;
},_initKeyObserver:function(){this.__ks=qx.lang.Function.listener(this.__kx,this);
this.__kv=qx.lang.Function.listener(this.__dD,this);
var Event=qx.bom.Event;
Event.addNativeListener(this.__cq,i,this.__ks);
Event.addNativeListener(this.__cq,m,this.__ks);
Event.addNativeListener(this.__cq,k,this.__kv);
},_stopKeyObserver:function(){var Event=qx.bom.Event;
Event.removeNativeListener(this.__cq,i,this.__ks);
Event.removeNativeListener(this.__cq,m,this.__ks);
Event.removeNativeListener(this.__cq,k,this.__kv);

for(var bU in (this.__ku||{})){var bT=this.__ku[bU];
Event.removeNativeListener(bT.target,k,bT.callback);
}delete (this.__ku);
},__kx:qx.event.GlobalError.observeMethod(qx.core.Environment.select(l,{"mshtml":function(bV){bV=window.event||bV;
var bY=bV.keyCode;
var bW=0;
var bX=bV.type;
if(!(this.__kr[bY]==m&&bX==m)){this._idealKeyHandler(bY,bW,bX,bV);
...
if(!(this.__kr[cb]==m&&cd==m)){this._idealKeyHandler(ce,cc,cd,ca);
}this.__kr[cb]=cd;
}else{this._idealKeyHandler(ce,cc,cd,ca);
}this.__ky(ca.target,cd,ce);
...
}},"opera":function(cj){this.__kt=cj.keyCode;
this._idealKeyHandler(cj.keyCode,0,cj.type,cj);
}})),__ky:qx.core.Environment.select(l,{"gecko":function(ck,cl,cm){if(cl===m&&(cm==33||cm==34||cm==38||cm==40)&&ck.type==v&&ck.tagName.toLowerCase()===S&&ck.getAttribute(bj)!==A){if(!this.__ku){this.__ku={};
}var co=qx.core.ObjectRegistry.toHashCode(ck);

if(this.__ku[co]){return;
}var self=this;
this.__ku[co]={target:ck,callback:function(cp){qx.bom.Event.stopPropagation(cp);
self.__dD(cp);
}};
var cn=qx.event.GlobalError.observeMethod(this.__ku[co].callback);
...
var cA=cz.type;
if(cB!=this.__kt){this._idealKeyHandler(0,this.__kt,cA,cz);
...
this.__kt=this.__x=this.__cp=this.__cq=this.__kr=null;
...
this.__x=E;
this.__cq=E.getWindow().document.documentElement;
this.__x.addListener(this.__cq,x,this._onMouseDown,this);
this.__hP();
},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{dragstart:1,dragend:1,dragover:1,dragleave:1,drop:1,drag:1,dragchange:1,droprequest:1},IGNORE_CAN_HANDLE:true},members:{__x:null,__cq:null,__hG:null,__hH:null,__C:null,__hI:null,__hJ:null,__cJ:null,__hK:null,__hL:null,__hM:false,__hN:0,__hO:0,canHandleEvent:function(F,G){},registerEvent:function(H,I,J){},unregisterEvent:function(K,L,M){},addType:function(N){this.__C[N]=true;
},addAction:function(O){this.__hI[O]=true;
},supportsType:function(P){return !!this.__C[P];
},supportsAction:function(Q){return !!this.__hI[Q];
},getData:function(R){if(!this.__hV||!this.__hG){throw new Error("This method must not be used outside the drop event listener!");
}
if(!this.__C[R]){throw new Error("Unsupported data type: "+R+"!");
}
if(!this.__cJ[R]){this.__hK=R;
this.__cA(q,this.__hH,this.__hG,false);
}
if(!this.__cJ[R]){throw new Error("Please use a droprequest listener to the drag source to fill the manager with data!");
}return this.__cJ[R]||null;
},getCurrentAction:function(){return this.__hL;
},addData:function(S,T){this.__cJ[S]=T;
},getCurrentType:function(){return this.__hK;
},isSessionActive:function(){return this.__hM;
},__hP:function(){this.__C={};
this.__hI={};
this.__hJ={};
this.__cJ={};
},__hQ:function(){if(this.__hH==null){return;
}var W=this.__hI;
var U=this.__hJ;
var V=null;

if(this.__hV){if(U.Shift&&U.Control&&W.alias){V=k;
...
if(V!=this.__hL){this.__hL=V;
this.__cA(o,this.__hH,this.__hG,false);
}},__cA:function(X,Y,ba,bb,bc){var be=qx.event.Registration;
var bd=be.createEvent(X,qx.event.type.Drag,[bb,bc]);

if(Y!==ba){bd.setRelatedTarget(ba);
}return be.dispatchEvent(Y,bd);
},__hR:function(bf){while(bf&&bf.nodeType==1){if(bf.getAttribute(w)==y){return bf;
}bf=bf.parentNode;
}return null;
},__hS:function(bg){while(bg&&bg.nodeType==1){if(bg.getAttribute(s)==y){return bg;
}bg=bg.parentNode;
}return null;
},__hT:function(){this.__hH=null;
this.__x.removeListener(this.__cq,c,this._onMouseMove,this,true);
this.__x.removeListener(this.__cq,B,this._onMouseUp,this,true);
qx.event.Registration.removeListener(window,i,this._onWindowBlur,this);
this.__hP();
},__hU:function(){if(this.__hM){this.__x.removeListener(this.__cq,a,this._onMouseOver,this,true);
...
this.__x.removeListener(this.__cq,C,this._onKeyUp,this,true);
this.__x.removeListener(this.__cq,A,this._onKeyPress,this,true);
this.__cA(z,this.__hH,this.__hG,false);
this.__hM=false;
}this.__hV=false;
this.__hG=null;
this.__hT();
},__hV:false,_onWindowBlur:function(e){this.__hU();
},_onKeyDown:function(e){var bh=e.getKeyIdentifier();

switch(bh){case D:case f:case d:if(!this.__hJ[bh]){this.__hJ[bh]=true;
this.__hQ();
}}},_onKeyUp:function(e){var bi=e.getKeyIdentifier();

switch(bi){case D:case f:case d:if(this.__hJ[bi]){this.__hJ[bi]=false;
this.__hQ();
}}},_onKeyPress:function(e){var bj=e.getKeyIdentifier();

switch(bj){case v:this.__hU();
}},_onMouseDown:function(e){if(this.__hM||e.getButton()!==t){return;
}var bk=this.__hR(e.getTarget());

if(bk){this.__hN=e.getDocumentLeft();
this.__hO=e.getDocumentTop();
this.__hH=bk;
this.__x.addListener(this.__cq,c,this._onMouseMove,this,true);
this.__x.addListener(this.__cq,B,this._onMouseUp,this,true);
qx.event.Registration.addListener(window,i,this._onWindowBlur,this);
}},_onMouseUp:function(e){if(this.__hV){this.__cA(r,this.__hG,this.__hH,false,e);
}if(this.__hM){e.stopPropagation();
}this.__hU();
},_onMouseMove:function(e){if(this.__hM){if(!this.__cA(u,this.__hH,this.__hG,true,e)){this.__hU();
}}else{if(Math.abs(e.getDocumentLeft()-this.__hN)>3||Math.abs(e.getDocumentTop()-this.__hO)>3){if(this.__cA(n,this.__hH,this.__hG,true,e)){this.__hM=true;
...
this.__x.addListener(this.__cq,C,this._onKeyUp,this,true);
this.__x.addListener(this.__cq,A,this._onKeyPress,this,true);
var bl=this.__hJ;
bl.Control=e.isCtrlPressed();
bl.Shift=e.isShiftPressed();
bl.Alt=e.isAltPressed();
this.__hQ();
}else{this.__cA(z,this.__hH,this.__hG,false);
this.__hT();
}}}},_onMouseOver:function(e){var bm=e.getTarget();
var bn=this.__hS(bm);

if(bn&&bn!=this.__hG){this.__hV=this.__cA(m,bn,this.__hH,true,e);
this.__hG=bn;
this.__hQ();
}},_onMouseOut:function(e){var bp=this.__hS(e.getTarget());
var bo=this.__hS(e.getRelatedTarget());

if(bp&&bp!==bo&&bp==this.__hG){this.__cA(l,this.__hG,bo,false,e);
this.__hG=null;
this.__hV=false;
qx.event.Timer.once(this.__hQ,this,0);
}}},destruct:function(){this.__hH=this.__hG=this.__x=this.__cq=this.__C=this.__hI=this.__hJ=this.__cJ=null;
},defer:function(bq){qx.event.Registration.addHandler(bq);
}});
})();
(function(){var a="qx.event.type.Data";
qx.Class.define(a,{extend:qx.event.type.Event,members:{__dc:null,__hW:null,init:function(b,c,d){qx.event.type.Event.prototype.init.call(this,false,d);
this.__dc=b;
this.__hW=c;
return this;
...
}},destruct:function(){this.__dc=this.__hW=null;
}});
})();
...
},__ib:{visible:true,hidden:true},getContentWidth:function(r){var s=qx.bom.element.Style;
var t=qx.bom.element.Overflow.getX(r);
var u=parseInt(s.get(r,d)||i,10);
var x=parseInt(s.get(r,e)||i,10);

if(this.__ib[t]){var w=r.clientWidth;
...
if(this.__ib[C]){return y.clientHeight-B-A;
...
qx.Bootstrap.define(a,{statics:{__dc:{},__ic:{width:null,height:null},__id:/\.(png|gif|jpg|jpeg|bmp)\b/i,isLoaded:function(d){var e=this.__dc[d];
return !!(e&&e.loaded);
...
return o?{width:o.width,height:o.height}:this.__ic;
...
var x=qx.lang.Function.listener(this.__ie,this,y,t);
y.onload=x;
y.onerror=x;
y.src=t;
w.element=y;
...
},__ie:qx.event.GlobalError.observeMethod(function(event,D,E){var F=this.__dc[E];
if(event.type===b){F.loaded=true;
F.width=this.__if(D);
F.height=this.__ig(D);
var G=this.__id.exec(E);
...
}}),__if:qx.core.Environment.select(c,{"gecko":function(I){return I.naturalWidth;
},"default":function(J){return J.width;
}}),__ig:qx.core.Environment.select(c,{"gecko":function(K){return K.naturalHeight;
...
(function(){var m="object",l="_applyTheme",k="",j="_",h="__ik",g="qx.ui.decoration.",f="qx.theme.manager.Decoration",e=".",d="Theme",c="changeTheme",a="string",b="singleton";
qx.Class.define(f,{type:b,extend:qx.core.Object,properties:{theme:{check:d,nullable:true,apply:l,event:c}},members:{__ik:null,resolve:function(n){if(!n){return null;
}
...
},isCached:function(A){return qx.lang.Object.contains(this.__ik,A);
...
qx.Class.define(g,{statics:{__ip:[n,null,m,i,null,r,k,null,r],__iq:{backgroundImage:null,backgroundPosition:null,backgroundRepeat:null},__ir:function(v,top){var w=qx.core.Environment.get(d);
...
},__is:function(A){var String=qx.lang.String;
var B=A.substr(0,50);
return String.startsWith(B,c)&&String.contains(B,a);
},compile:function(C,D,E,top){var F=this.__ir(E,top);
var G=qx.util.ResourceManager.getInstance().toUri(C);

if(this.__is(G)){G=q+G+q;
}var H=this.__ip;
H[1]=G;
H[4]=F;
H[7]=D;
...
qx.Class.define(c,{extend:qx.core.Object,implement:[qx.ui.decoration.IDecorator],type:g,properties:{insetLeft:{check:i,nullable:true,apply:h},insetRight:{check:i,nullable:true,apply:h},insetBottom:{check:i,nullable:true,apply:h},insetTop:{check:i,nullable:true,apply:h},insets:{group:[e,f,d,a],mode:b}},members:{__jv:null,_getDefaultInsets:function(){throw new Error("Abstract method called.");
},_isInitialized:function(){throw new Error("Abstract method called.");
},_resetInsets:function(){this.__jv=null;
},getInsets:function(){if(this.__jv){return this.__jv;
}var j=this._getDefaultInsets();
return this.__jv={left:this.getInsetLeft()==null?j.left:this.getInsetLeft(),right:this.getInsetRight()==null?j.right:this.getInsetRight(),bottom:this.getInsetBottom()==null?j.bottom:this.getInsetBottom(),top:this.getInsetTop()==null?j.top:this.getInsetTop()};
},_applyInsets:function(){this.__jv=null;
...
}},properties:{alignY:{check:[m,j,e],init:m,apply:n},alignX:{check:[k,g,a],init:k,apply:n},spacing:{check:c,init:0,apply:n},separator:{check:h,nullable:true,apply:n},reversed:{check:b,init:false,apply:f}},members:{__jw:null,__jx:null,__jy:null,__iO:null,_applyReversed:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__jz:function(){var w=this._getLayoutChildren();
var length=w.length;
var s=false;
var r=this.__jw&&this.__jw.length!=length&&this.__jx&&this.__jw;
var u;
var t=r?this.__jw:new Array(length);
var v=r?this.__jx:new Array(length);
...
this.__iO=w;
delete this._invalidChildrenCache;
},verifyLayoutProperty:null,renderLayout:function(x,y){if(this._invalidChildrenCache){this.__jz();
}var F=this.__iO;
var length=F.length;
...
for(i=0;i<length;i+=1){J=this.__jw[i];
B=J!=null?Math.floor((y-C)*J):F[i].getSizeHint().height;
K.push(B);
Q+=B;
}if(this.__jy&&Q!=y){var H={};
var N,R;

for(i=0;i<length;i+=1){N=this.__jx[i];
...
}},destruct:function(){this.__jw=this.__jx=this.__iO=null;
}});
})();
...
qx.Class.define(w,{statics:{__jW:{},bind:function(G,H,I,J,K){var V=this.__jY(G,H,I,J,K);
var Q=H.split(d);
var M=this.__kf(Q);
var U=[];
var R=[];
...
var T=qx.lang.Function.bind(this.__jX,this,W);
R.push(T);
...
this.__kh(X,G,H,I,J);
return X;
},__jX:function(bc){if(bc.options&&bc.options.onUpdate){bc.options.onUpdate(bc.sources[bc.index],bc.targetObject);
...
if(!bg){this.__kb(bc.targetObject,bc.targetPropertyChain);
break;
...
this.__ke(be,bc.targetObject,bc.targetPropertyChain,bc.options,bc.sources[bc.index]);
bc.listenerIds[j]=this.__kg(bg,g,bc.targetObject,bc.targetPropertyChain,bc.options,bc.arrayIndexValues[j]);
...
this.__ke(be,bc.targetObject,bc.targetPropertyChain,bc.options,bc.sources[bc.index]);
}var bf=this.__ka(bg,bc.propertyNames[j]);
bc.listenerIds[j]=this.__kg(bg,bf,bc.targetObject,bc.targetPropertyChain,bc.options);
}}else{if(bc.listeners[j]==null){var bd=qx.lang.Function.bind(this.__jX,this,bc);
bc.listeners.push(bd);
}if(qx.Class.implementsInterface(bg,qx.data.IListData)){var bf=g;
}else{var bf=this.__ka(bg,bc.propertyNames[j]);
}bc.listenerIds[j]=bg.addListener(bf,bc.listeners[j]);
}}},__jY:function(bi,bj,bk,bl,bm){var bq=bl.split(d);
var bo=this.__kf(bq);
var bv=[];
var bu=[];
var bs=[];
...
}else{try{br.push(this.__ka(bp,bq[i]));
}catch(e){break;
}}bv[i]=bp;
...
}else{try{var bx=qx.data.SingleValueBinding.__ka(by,bq[j]);
}catch(e){break;
...
bF=qx.data.SingleValueBinding.__ki(bF,bC,bD,bE,bA);
this.__kc(bC,bD,bF);
},getValueFromObject:function(o,bG){var bK=this.__kd(o,bG);
var bI;
...
},__ka:function(bN,bO){var bP=this.__kj(bN,bO);
...
}}},__kd:function(cd,ce){var ch=ce.split(d);
var ci=cd;
...
},__ke:function(ck,cl,cm,cn,co){ck=this.__ki(ck,cl,cm,cn,co);
if(ck===undefined){this.__kb(cl,cm);
}if(ck!==undefined){try{this.__kc(cl,cm,ck);
...
},__kg:function(cs,ct,cu,cv,cw,cx){var cy;
var cA=function(cB,e){if(cB!==l){if(cB===h){cB=cs.length-1;
}var cE=cs.getItem(cB);
if(cE===undefined){qx.data.SingleValueBinding.__kb(cu,cv);
...
}cE=qx.data.SingleValueBinding.__ki(cE,cu,cv,cw,cs);
if(qx.core.Environment.get(a)){qx.log.Logger.debug("Data after conversion: "+cE);
}try{if(cE!==undefined){qx.data.SingleValueBinding.__kc(cu,cv,cE);
}else{qx.data.SingleValueBinding.__kb(cu,cv);
...
return cz;
},__kh:function(cF,cG,cH,cI,cJ){if(this.__jW[cG.toHashCode()]===undefined){this.__jW[cG.toHashCode()]=[];
}this.__jW[cG.toHashCode()].push([cF,cG,cH,cI,cJ]);
},__ki:function(cK,cL,cM,cN,cO){if(cN&&cN.converter){var cQ;

if(cL.getModel){cQ=cL.getModel();
}return cN.converter(cK,cQ,cO,cL);
}else{var cS=this.__kd(cL,cM);
...
return this.__kk(cK,cP);
}},__kj:function(cU,cV){var cW=qx.Class.getPropertyDefinition(cU.constructor,cV);

if(cW==null){return null;
}return cW.event;
},__kk:function(cX,cY){var da=qx.lang.Type.getClass(cX);
...
}var dd=this.__jW[db.toHashCode()];
if(dd!=undefined){for(var i=0;i<dd.length;i++){if(dd[i][0]==dc){qx.lang.Array.remove(dd,dd[i]);
return;
}}}throw new Error("Binding could not be found!");
},removeAllBindingsForObject:function(de){var df=this.__jW[de.toHashCode()];

if(df!=undefined){for(var i=df.length-1;i>=0;i--){this.removeBindingFromObject(de,df[i][0]);
}}},getAllBindingsForObject:function(dg){if(this.__jW[dg.toHashCode()]===undefined){this.__jW[dg.toHashCode()]=[];
}return this.__jW[dg.toHashCode()];
},removeAllBindings:function(){for(var di in this.__jW){var dh=qx.core.ObjectRegistry.fromHashCode(di);
if(dh==null){delete this.__jW[di];
continue;
}this.removeAllBindingsForObject(dh);
}this.__jW={};
},getAllBindings:function(){return this.__jW;
},showBindingInLog:function(dj,dk){var dm;
for(var i=0;i<this.__jW[dj.toHashCode()].length;i++){if(this.__jW[dj.toHashCode()][i][0]==dk){dm=this.__jW[dj.toHashCode()][i];
break;
}}
if(dm===undefined){var dl=q;
}else{var dl=v+dm[1]+B+dm[2]+z+dm[3]+B+dm[4]+r;
}qx.log.Logger.debug(dl);
},showAllBindingsInLog:function(){for(var dp in this.__jW){var dn=qx.core.ObjectRegistry.fromHashCode(dp);

for(var i=0;i<this.__jW[dp].length;i++){this.showBindingInLog(dn,this.__jW[dp][i][0]);
}}}}});
})();
...
g.start();
this.__kl=g;
},events:{"interval":c},properties:{timeoutInterval:{check:e,init:100,apply:d}},members:{__kl:null,_applyTimeoutInterval:function(h){this.__kl.setInterval(h);
},_onInterval:function(){this.fireEvent(f);
}},destruct:function(){if(this.__kl){this.__kl.stop();
}this.__kl=null;
}});
})();
...
}},properties:{rich:{check:i,init:false,event:p,apply:n},wrap:{check:i,init:true,apply:a},value:{check:v,apply:y,event:D,nullable:true},buddy:{check:d,apply:w,nullable:true,init:null,dereference:true},textAlign:{check:[m,x,r,s],nullable:true,themeable:true,apply:f,event:b},appearance:{refine:true,init:l},selectable:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false},allowShrinkY:{refine:true,init:false}},members:{__G:null,__km:null,__kn:null,__ko:null,__H:null,_getContentHint:function(){if(this.__km){this.__kp=this.__kq();
delete this.__km;
}return {width:this.__kp.width,height:this.__kp.height};
...
}return this.__kq(G).height;
},_createContentElement:function(){return new qx.html.Label;
},_applyTextAlign:function(H,I){this.getContentElement().setStyle(u,H);
},_applyTextColor:function(J,K){if(J){this.getContentElement().setStyle(k,qx.theme.manager.Color.getInstance().resolve(J));
}else{this.getContentElement().removeStyle(k);
}},__kp:{width:0,height:0},_applyFont:function(L,M){if(M&&this.__G&&this.__H){this.__G.removeListenerById(this.__H);
this.__H=null;
}var N;
...
}if(this.getTextColor()!=null){delete N[k];
}this.getContentElement().setStyles(N);
this.__km=true;
qx.ui.core.queue.Layout.add(this);
},__kq:function(O){var S=qx.bom.Label;
var Q=this.getFont();
...
},_applyBuddy:function(T,U){if(U!=null){U.removeBinding(this.__kn);
this.__kn=null;
this.removeListenerById(this.__ko);
this.__ko=null;
}
if(T!=null){this.__kn=T.bind(h,this,h);
this.__ko=this.addListener(o,function(){if(T.isFocusable()){T.focus.apply(T);
}},this);
...
}},"false":null}),_onWebFontStatusChange:function(ba){if(ba.getData().valid===true){this.__km=true;
qx.ui.core.queue.Layout.add(this);
...
}if(this.__kn!=null){var bd=this.getBuddy();

if(bd!=null&&!bd.isDisposed()){bd.removeBinding(this.__kn);
}}
if(this.__G&&this.__H){this.__G.removeListenerById(this.__H);
}this.__G=this.__kn=null;
}});
})();
...
},flush:function(){var f=this.__kG();
...
}}},getNestingLevel:function(k){var l=this.__kF;
var n=0;
var parent=k;
...
},__kE:function(){var t=qx.ui.core.queue.Visibility;
this.__kF={};
var s=[];
var r=this.__db;
var o,q;
...
qx.Class.define(f,{extend:qx.bom.Font,events:{"changeStatus":a},properties:{sources:{nullable:true,apply:b}},members:{__kH:null,_applySources:function(h,j){var n=[];
...
qx.Class.define(z,{statics:{__kO:function(D,E){return qx.bom.element.Style.get(D,E,qx.bom.element.Style.COMPUTED_MODE,false);
},__kP:function(F,G){return parseInt(qx.bom.element.Style.get(F,G,qx.bom.element.Style.COMPUTED_MODE,false),10)||0;
},__kQ:function(H){var K=0,top=0;
...
},__kR:qx.core.Environment.select(f,{"mshtml":function(L){var N=qx.dom.Node.getDocument(L);
var M=N.body;
var O=0;
var top=0;
...
if(!qx.core.Environment.get(A)){O+=this.__kP(M,i);
top+=this.__kP(M,j);
...
var S=Q.offsetLeft;
var top=Q.offsetTop;
if(parseFloat(qx.core.Environment.get(d))<530.17){S+=this.__kP(Q,i);
top+=this.__kP(Q,j);
}return {left:S,top:top};
},"gecko":function(T){var U=qx.dom.Node.getDocument(T).body;
var V=U.offsetLeft;
var top=U.offsetTop;
if(parseFloat(qx.core.Environment.get(d))<1.9){V+=this.__kP(U,g);
top+=this.__kP(U,h);
}if(qx.bom.element.BoxSizing.get(U)!==c){V+=this.__kP(U,i);
top+=this.__kP(U,j);
...
}}),__kS:qx.core.Environment.select(f,{"mshtml|webkit":function(ba){var bc=qx.dom.Node.getDocument(ba);
...
if(bh.get(bf)!==c){bj-=this.__kP(bf,i);
top-=this.__kP(bf,j);
}
...
if(bh.get(bf)!==c){bj+=this.__kP(bf,i);
top+=this.__kP(bf,j);
}if(bf.parentNode&&this.__kO(bf.parentNode,n)!=v){bj+=this.__kP(bf.parentNode,i);
top+=this.__kP(bf.parentNode,j);
}bf=bf.offsetParent;
...
}}),get:function(bn,bo){if(bn.tagName==u){var location=this.__kT(bn);
var bv=location.left;
var top=location.top;
}else{var bp=this.__kR(bn);
var bu=this.__kS(bn);
var scroll=this.__kQ(bn);
...
if(bs==e||bs==C){bq+=bn.scrollWidth-bn.offsetWidth+this.__kP(bn,i)+this.__kP(bn,a);
}var bt=qx.bom.element.Overflow.getY(bn);

if(bt==e||bt==C){br+=bn.scrollHeight-bn.offsetHeight+this.__kP(bn,j)+this.__kP(bn,b);
}}
switch(bo){case B:bv+=this.__kP(bn,y);
top+=this.__kP(bn,r);
bq-=this.__kP(bn,k);
br-=this.__kP(bn,q);
...
bq-=bn.scrollLeft;
br-=bn.scrollTop;
case s:bv+=this.__kP(bn,i);
top+=this.__kP(bn,j);
bq-=this.__kP(bn,a);
br-=this.__kP(bn,b);
break;
case m:bv-=this.__kP(bn,g);
top-=this.__kP(bn,h);
bq+=this.__kP(bn,o);
br+=this.__kP(bn,w);
break;
}}return {left:bv,top:top,right:bq,bottom:br};
},__kT:qx.core.Environment.select(f,{"default":function(bw){var top=bw.offsetTop+this.__kP(bw,h);
var bx=bw.offsetLeft+this.__kP(bw,g);
return {left:bx,top:top};
},"mshtml":function(by){var top=by.offsetTop;
var bz=by.offsetLeft;

if(!((parseFloat(qx.core.Environment.get(d))<8||qx.core.Environment.get(l)<8)&&!qx.core.Environment.get(A))){top+=this.__kP(by,h);
bz+=this.__kP(by,g);
}return {left:bz,top:top};
},"gecko":function(bA){var top=bA.offsetTop+this.__kP(bA,h)+this.__kP(bA,i);
var bB=bA.offsetLeft+this.__kP(bA,g)+this.__kP(bA,j);
...
qx.Class.define(f,{statics:{__lj:{"onload":true,"onpropertychange":true,"oninput":true,"onchange":true,"name":true,"type":true,"checked":true,"disabled":true},__lk:{},__ll:{},allowCreationWithMarkup:function(s){if(!s){s=window;
}var t=s.location.href;

if(qx.bom.Element.__ll[t]==undefined){try{s.document.createElement(g);
qx.bom.Element.__ll[t]=true;
}catch(e){qx.bom.Element.__ll[t]=false;
}}return qx.bom.Element.__ll[t];
},getHelperElement:function(u){if(!u){u=window;
}var w=u.location.href;

if(!qx.bom.Element.__lk[w]){var v=qx.bom.Element.__lk[w]=u.document.createElement(c);
if(qx.core.Environment.get(r)==b){v.style.display=h;
u.document.body.appendChild(v);
}}return qx.bom.Element.__lk[w];
...
qx.Class.define(r,{statics:{DEBUG:false,__lm:{},__ln:(qx.core.Environment.get(j)==c)&&qx.core.Environment.get(D)<9,__lo:qx.core.Environment.select(j,{"mshtml":{"scale-x":true,"scale-y":true,"scale":true,"no-repeat":true},"default":null}),__lp:{"scale-x":h,"scale-y":h,"scale":h,"repeat":i,"no-repeat":i,"repeat-x":i,"repeat-y":i},update:function(J,K,L,M){var O=this.getTagName(L,K);
...
if(this.__ln){try{J.filters[v].apply();
...
}},getTagName:function(W,X){if((qx.core.Environment.get(j)==c)){if(X&&this.__ln&&this.__lo[W]&&qx.lang.String.endsWith(X,F)){return i;
}}return this.__lp[W];
...
var bc;
if(this.__ln&&this.__lo[ba]&&bd===q){bc=this.__ls(bb,ba,Y);
}else{if(ba===b){bc=this.__lt(bb,ba,Y);
}else if(ba===d||ba===I){bc=this.__lu(bb,ba,Y);
}else{bc=this.__lx(bb,ba,Y);
}}return bc;
},__lq:function(be,bf,bh){if(be.width==null&&bf!=null){be.width=bf+k;
}
if(be.height==null&&bh!=null){be.height=bh+k;
}return be;
},__lr:function(bi){var bj=qx.util.ResourceManager.getInstance().getImageWidth(bi)||qx.io.ImageLoader.getWidth(bi);
var bk=qx.util.ResourceManager.getInstance().getImageHeight(bi)||qx.io.ImageLoader.getHeight(bi);
return {width:bj,height:bk};
},__ls:function(bl,bm,bn){var bq=this.__lr(bn);
bl=this.__lq(bl,bq.width,bq.height);
var bp=bm==f?E:b;
var bo=C+qx.util.ResourceManager.getInstance().toUri(bn)+s+bp+n;
bl.filter=bo;
bl.backgroundImage=bl.backgroundRepeat=g;
return {style:bl};
},__lt:function(br,bs,bt){var bu=qx.util.ResourceManager.getInstance().toUri(bt);
var bv=this.__lr(bt);
br=this.__lq(br,bv.width,bv.height);
return {src:bu,style:br};
},__lu:function(bw,bx,by){var bz=qx.util.ResourceManager.getInstance();
...
if(bx===d){bw=this.__lv(bw,bD,bE.height);
}else{bw=this.__lw(bw,bD,bE.width);
...
return {src:bA,style:bw};
}},__lv:function(bF,bG,bH){var bI=qx.util.ResourceManager.getInstance().getImageHeight(bG[4]);
...
},__lw:function(bJ,bK,bL){var bM=qx.util.ResourceManager.getInstance().getImageWidth(bK[4]);
...
},__lx:function(bN,bO,bP){var bS=qx.util.ResourceManager.getInstance();
...
}else{bN=this.__lq(bN,ca.width,ca.height);
bN=this.__ly(bN,bP,bO);
return {style:bN};
}},__ly:function(cb,cc,cd){var top=null;
var ch=null;
...
},__lz:function(ci){if(this.DEBUG&&qx.util.ResourceManager.getInstance().has(ci)&&ci.indexOf(H)==-1){if(!this.__lm[ci]){qx.log.Logger.debug("Potential clipped image candidate: "+ci);
this.__lm[ci]=true;
}}},isAlphaImageLoaderEnabled:qx.core.Environment.select(j,{"mshtml":function(){return qx.bom.element.Decoration.__ln;
},"default":function(){return false;
}})}});
})();
(function(){var p="Integer",o="_applyContentPadding",n="resetPaddingRight",m="setPaddingBottom",l="resetPaddingTop",k="qx.ui.core.MContentPadding",j="resetPaddingLeft",i="setPaddingTop",h="setPaddingRight",g="resetPaddingBottom",c="contentPaddingLeft",f="setPaddingLeft",e="contentPaddingTop",b="shorthand",a="contentPaddingRight",d="contentPaddingBottom";
qx.Mixin.define(k,{properties:{contentPaddingTop:{check:p,init:0,apply:o,themeable:true},contentPaddingRight:{check:p,init:0,apply:o,themeable:true},contentPaddingBottom:{check:p,init:0,apply:o,themeable:true},contentPaddingLeft:{check:p,init:0,apply:o,themeable:true},contentPadding:{group:[e,a,d,c],mode:b,themeable:true}},members:{__lA:{contentPaddingTop:i,contentPaddingRight:h,contentPaddingBottom:m,contentPaddingLeft:f},__lB:{contentPaddingTop:l,contentPaddingRight:n,contentPaddingBottom:g,contentPaddingLeft:j},_applyContentPadding:function(q,r,name){var s=this._getContentPaddingTarget();

if(q==null){var t=this.__lB[name];
s[t]();
}else{var u=this.__lA[name];
s[u](q);
}}}});
})();
...
if(q){this.setSeparator(q);
}},properties:{alignX:{check:[m,k,c],init:m,apply:n},alignY:{check:[j,g,e],init:j,apply:n},spacing:{check:a,init:0,apply:n},separator:{check:h,nullable:true,apply:n},reversed:{check:d,init:false,apply:f}},members:{__lD:null,__jx:null,__jy:null,__iO:null,_applyReversed:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__jz:function(){var w=this._getLayoutChildren();
var length=w.length;
var t=false;
var r=this.__lD&&this.__lD.length!=length&&this.__jx&&this.__lD;
var u;
...
P.push(K);
F+=K;
}if(this.__jy&&F!=x){var H={};
var L,O;

for(i=0;i<length;i+=1){L=this.__jx[i];
...
}},destruct:function(){this.__lD=this.__jx=this.__iO=null;
}});
})();
...
qx.Bootstrap.define(a,{construct:function(name,f,g){this.__lE=name;
this._readyState=e;
this._packages=f;
this._loader=g;
...
break;
}}},members:{_readyState:null,_loader:null,_packages:null,__lE:null,getReadyState:function(){return this._readyState;
},getName:function(){return this.__lE;
...
qx.Mixin.define(f,{events:{"changeSelection":a},members:{__x:null,getSelection:function(){var g=this.__lF().getSelected();

if(g){return [g];
}else{return [];
}},setSelection:function(h){switch(h.length){case 0:this.resetSelection();
break;
case 1:this.__lF().setSelected(h[0]);
break;
default:throw new Error("Could only select one item, but the selection"+" array contains "+h.length+" items!");
}},resetSelection:function(){this.__lF().resetSelected();
},isSelected:function(i){return this.__lF().isSelected(i);
},isSelectionEmpty:function(){return this.__lF().isSelectionEmpty();
},getSelectables:function(j){return this.__lF().getSelectables(j);
},_onChangeSelected:function(e){var l=e.getData();
var k=e.getOldData();
l==null?l=[]:l=[l];
k==null?k=[]:k=[k];
this.fireDataEvent(d,l,k);
},__lF:function(){if(this.__x==null){var m=this;
...
qx.Bootstrap.define(b,{construct:function(i,j,k){this.__lG=k?e:g;
this.__lH=i;
this.__df=j;
},members:{__lG:null,__lH:null,__df:null,__lI:null,__lJ:null,__bX:null,__lK:null,getId:function(){return this.__df;
},getReadyState:function(){return this.__lG;
},getUrls:function(){return this.__lH;
},saveClosure:function(l){if(this.__lG==h){return;
}this.__lI=l;

if(!this.__lJ){this.execute();
}else{clearTimeout(this.__bX);
this.__lG=a;
this.__lK(this);
}},execute:function(){if(this.__lI){this.__lI();
delete this.__lI;
}
if(qx.$$packageData[this.__df]){this.__lM(qx.$$packageData[this.__df]);
delete qx.$$packageData[this.__df];
}this.__lG=e;
},loadClosure:function(m,self){if(this.__lG!==g){return;
}this.__lJ=true;
this.__lG=f;
this.__lK=qx.Bootstrap.bind(m,self);
this.__lL(this.__lH,function(){},function(){this.__lG=h;
m.call(self,this);
},this);
var n=this;
this.__bX=setTimeout(function(){n.__lG=h;
m.call(self,n);
},qx.Part.TIMEOUT);
},load:function(o,self){if(this.__lG!==g){return;
}this.__lJ=false;
this.__lG=f;
this.__lL(this.__lH,function(){this.__lG=e;
this.execute();
o.call(self,this);
},function(){this.__lG=h;
o.call(self,this);
},this);
},__lL:function(p,q,r,self){if(p.length==0){q.call(self);
return;
}var t=0;
var self=this;
...
if(status!==c){if(self.__lG==f){clearTimeout(self.__bX);
return r.call(self);
...
s(p.concat());
},__lM:qx.$$loader.importPackageData}});
})();
...
},_registerEventChaining:function(l,m,name){if((l instanceof qx.core.Object)&&qx.Class.hasMixin(l.constructor,qx.data.marshal.MEventBubbling)){var n=qx.lang.Function.bind(this.__lN,this,name);
...
}},__lN:function(name,e){var x=e.getData();
var t=x.value;
var r=x.old;
...
},members:{__lS:0,preload:function(h,self){var j=0;
var l=this;
...
}this.__lS=this._packages.length;
...
}}if(this.__lS<=0){this.__lT();
}},__lT:function(){for(var i=0;i<this._packages.length;i++){this._packages[i].execute();
...
qx.Bootstrap.define(d,{construct:function(){this.__md=qx.Bootstrap.bind(this.__mh,this);
this.__me=document.createElement(f);
},statics:{TIMEOUT:15},members:{__mf:null,__mg:null,__eB:null,__eC:null,__md:null,__me:null,load:function(q,r,s){if(this.__mf){throw new Error("Another request is still running!");
}this.__mf=true;
this.__mg=false;
...
this.__eC=s||window;
u.type=b;
u.onerror=u.onload=u.onreadystatechange=this.__md;
var self=this;
...
},abort:function(){if(this.__mf){this.dispose(a);
}},dispose:function(status){if(this.__mg){return;
}this.__mg=true;
var x=this.__me;
...
},__mh:qx.event.GlobalError.observeMethod(function(e){var y=qx.bom.client.Engine.getName();
if(y==n){var z=this.__me.readyState;
...
for(var j in d.packages){var h=new qx.io.part.Package(this.__mc(d.packages[j].uris),j,j==e);
this.__lY[j]=h;
}this.__ma={};
var g=d.parts;
...
for(var i=0;i<m.length;i++){l.push(this.__lY[m[i]]);
...
}},members:{__lU:null,__lY:null,__ma:null,__lX:null,addToPackage:function(s){this.__lY[s.getId()]=s;
},addClosurePackageListener:function(t,u){var v=t.getId();

if(!this.__lX[v]){this.__lX[v]=[];
}this.__lX[v].push(u);
},saveClosure:function(w,x){var z=this.__lY[w];
...
},getParts:function(){return this.__ma;
...
for(var i=0;i<A.length;i++){E.push(this.__ma[A[i]]);
}var D=0;
var C=function(){D+=1;
if(D>=E.length){var F=[];
...
for(var i=0;i<G.length;i++){this.__ma[G[i]].preload(function(){I++;

if(I>=G.length){var J=[];

for(var i=0;i<G.length;i++){J.push(this.__ma[G[i]].getReadyState());
}
if(H){H.call(self,J);
}}},this);
}},__mb:function(){var K=this.__lU.packages;
var L=[];

for(var M in K){L.push(this.__mc(K[M].uris));
}return L;
},__mc:qx.$$loader.decodeUris,__lV:null,addPartListener:function(N,O){var P=N.getName();

if(!this.__lV[P]){this.__lV[P]=[];
}this.__lV[P].push(O);
...
}},__lW:null,addPackageListener:function(T,U){var V=T.getId();

if(!this.__lW[V]){this.__lW[V]=[];
}this.__lW[V].push(U);
...
if(p==undefined){this.__mi=[];
}else if(arguments.length>1){this.__mi=[];

for(var i=0;i<arguments.length;i++){this.__mi.push(arguments[i]);
}}else if(typeof p==c){this.__mi=new Array(p);
}else if(p instanceof Array){this.__mi=qx.lang.Array.clone(p);
}else{this.__mi=[];
this.dispose();
throw new Error("Type of the parameter not supported!");
}for(var i=0;i<this.__mi.length;i++){this._applyEventPropagation(this.__mi[i],null,i);
}this.__mj();
},properties:{autoDisposeItems:{check:e,init:false}},events:{"change":f,"changeLength":f},members:{__mi:null,concat:function(q){if(q){var r=this.__mi.concat(q);
}else{var r=this.__mi.concat();
}return new qx.data.Array(r);
},join:function(s){return this.__mi.join(s);
},pop:function(){var t=this.__mi.pop();
this.__mj();
...
return t;
},push:function(u){for(var i=0;i<arguments.length;i++){this.__mi.push(arguments[i]);
this.__mj();
...
}var v=this.__mi.concat();
this.__mi.reverse();
this.fireDataEvent(o,{start:0,end:this.length-1,type:j,items:null},null);
this.fireDataEvent(n,{value:this.__mi,name:k+(this.__mi.length-1),old:v});
},shift:function(){if(this.length==0){return;
}var w=this.__mi.shift();
this.__mj();
...
return w;
},slice:function(x,y){return new qx.data.Array(this.__mi.slice(x,y));
},splice:function(z,A,B){var J=this.__mi.length;
var F=this.__mi.splice.apply(this.__mi,arguments);
if(this.__mi.length!=J){this.__mj();
}var H=A>0;
var D=arguments.length>2;
var E=null;

if(H||D){if(this.__mi.length>J){var I=m;
}else if(this.__mi.length<J){var I=l;
E=F;
}else{var I=j;
...
}var L=this.__mi.concat();
this.__mi.sort.apply(this.__mi,arguments);
this.fireDataEvent(o,{start:0,end:this.length-1,type:j,items:null},null);
this.fireDataEvent(n,{value:this.__mi,name:k+(this.length-1),old:L});
},unshift:function(M){for(var i=arguments.length-1;i>=0;i--){this.__mi.unshift(arguments[i]);
this.__mj();
this._registerEventChaining(arguments[i],null,0);
this.fireDataEvent(n,{value:[this.__mi[0]],name:g,old:[this.__mi[1]]});
this.fireDataEvent(o,{start:0,end:this.length-1,type:m,items:[arguments[i]]},null);
}return this.length;
},toArray:function(){return this.__mi;
},getItem:function(N){return this.__mi[N];
},setItem:function(O,P){var Q=this.__mi[O];
if(Q===P){return;
}this.__mi[O]=P;
...
},indexOf:function(R){return this.__mi.indexOf(R);
},toString:function(){if(this.__mi!=null){return this.__mi.toString();
}return b;
},contains:function(S){return this.__mi.indexOf(S)!==-1;
...
be.dispose();
return bd;
},removeAll:function(){for(var i=0;i<this.__mi.length;i++){this._registerEventChaining(null,this.__mi[i],i);
}if(this.getLength()==0){return;
}var bg=this.getLength();
var bf=this.__mi.concat();
this.__mi.length=0;
this.__mj();
...
}Array.prototype.push.apply(this.__mi,bh);
for(var i=0;i<bh.length;i++){this._registerEventChaining(bh[i],null,this.__mi.length+i);
}var bi=this.length;
this.__mj();
...
},forEach:function(bp,bq){for(var i=0;i<this.__mi.length;i++){bp.call(bq,this.__mi[i],i,this);
}},__mj:function(){var br=this.length;
this.length=this.__mi.length;
this.fireDataEvent(a,this.length,br);
}},destruct:function(){for(var i=0;i<this.__mi.length;i++){var bs=this.__mi[i];
this._applyEventPropagation(null,bs,i);
if(this.isAutoDisposeItems()&&bs&&bs instanceof qx.core.Object){bs.dispose();
}}this.__mi=null;
}});
})();
(function(){var g="Boolean",f="qx.ui.core.SingleSelectionManager",e="__mk",d="__ml",c="changeSelected",b="__mm",a="qx.event.type.Data";
qx.Class.define(f,{extend:qx.core.Object,construct:function(h){qx.core.Object.call(this);
this.__mk=h;
},events:{"changeSelected":a},properties:{allowEmptySelection:{check:g,init:true,apply:b}},members:{__ml:null,__mk:null,getSelected:function(){return this.__ml;
},setSelected:function(j){if(!this.__mo(j)){throw new Error("Could not select "+j+", because it is not a child element!");
}this.__mn(j);
},resetSelected:function(){this.__mn(null);
},isSelected:function(k){if(!this.__mo(k)){throw new Error("Could not check if "+k+" is selected,"+" because it is not a child element!");
}return this.__ml===k;
},isSelectionEmpty:function(){return this.__ml==null;
},getSelectables:function(l){var m=this.__mk.getItems();
var n=[];

for(var i=0;i<m.length;i++){if(this.__mk.isItemSelectable(m[i])){n.push(m[i]);
}}if(!l){for(var i=n.length-1;i>=0;i--){if(!n[i].getEnabled()){n.splice(i,1);
}}}return n;
},__mm:function(o,p){if(!o){this.__mn(this.__ml);
}},__mn:function(q){var t=this.__ml;
var s=q;
...
},__mo:function(u){var v=this.__mk.getItems();

for(var i=0;i<v.length;i++){if(v[i]===u){return true;
}}return false;
}},destruct:function(){if(this.__mk.toHashCode){this._disposeObjects(e);
}else{this.__mk=null;
...
}this.__mw.addListener(d,this._onMouseDown,this);
this.__mw.addListener(r,this._onMouseUp,this);
this.__mw.addListener(D,this._onMouseMove,this);
this.__mw.addListener(c,this._onMouseOut,this);
this.__mw.addListener(m,this._onMouseUp,this);
},properties:{appearance:{refine:true,init:u},offset:{check:b,init:6,apply:v},orientation:{init:i,check:[i,g],apply:w}},members:{__mp:null,__mq:false,__mr:null,__ms:null,__mt:null,__mu:null,__mv:null,__iO:null,__mw:null,_createChildControlImpl:function(F,G){var H;
...
this._add(H,{type:F});
H.addListener(C,this.__mx,this);
break;
}return H||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,F);
},__mx:function(e){this.__mz(e.getData());
},__my:function(I){this.__mw=new qx.ui.splitpane.Blocker(I);
this.getContentElement().add(this.__mw);
var J=this.getChildControl(j);
var K=J.getWidth();

if(!K){J.addListenerOnce(p,function(){this.__mz();
},this);
}J.addListener(A,function(e){var L=e.getData();

if(L.height==0||L.width==0){this.__mw.hide();
}else{this.__mw.show();
}},this);
},getBlocker:function(){return this.__mw;
},_applyOrientation:function(M,N){var O=this.getChildControl(k);
var R=this.getChildControl(j);
this.__mt=M===i;

if(!this.__mw){this.__my(M);
}this.__mw.setOrientation(M);
...
},__mz:function(U){var V=this.getChildControl(j);
var ba=this.getOffset();
var bb=V.getBounds();
var Y=V.getContainerElement().getDomElement();
if(!Y){return;
}if(this.__mt){var X=null;
...
}this.__mw.setWidth(ba,X);
this.__mw.setLeft(ba,bc);
}}else{var W=null;
...
this.__mp=this.__mt?e.getDocumentLeft()-bi.left+bh.left:e.getDocumentTop()-bi.top+bh.top;
...
if(this.__mq){this.__mA();
var bl=this.getChildControl(k);
var bm=this.__mu;

if(this.__mt){bl.setDomLeft(bm);
this.__mw.setStyle(n,(bm-this.getOffset())+h);
}else{bl.setDomTop(bm);
this.__mw.setStyle(s,(bm-this.getOffset())+h);
}e.stop();
...
this.releaseCapture();
e.stop();
},_finalizeSizes:function(){var br=this.__mu;
var bo=this.__mv;
...
}else{if(this.__mt){bs.setWidth(br);
bp.setWidth(bo);
}else{bs.setHeight(br);
bp.setHeight(bo);
}}},__mA:function(){if(this.__mt){var bx=o,bE=z,by=l,bC=this.__mr;
}else{var bx=q,bE=a,by=B,bC=this.__ms;
...
},__mC:function(){var bR=this.getRequest();
...
qx.Bootstrap.define(a,{construct:function(){this.__mL();
this.__mD=qx.Bootstrap.bind(this._onNativeLoad,this);
this.__mE=qx.Bootstrap.bind(this._onNativeError,this);
this.__mF=qx.Bootstrap.bind(this._onTimeout,this);
this.__mG=document.head||document.getElementsByTagName(f)[0]||document.documentElement;
this.timeout=this.__mP()?0:15000;
},members:{readyState:null,status:null,statusText:null,timeout:null,__mH:null,open:function(l,m){if(this.__mg){return;
}this.__mL();
this.__mI=null;
this.__mJ=m;
...
},setRequestHeader:function(n,o){if(this.__mg){return;
}var p={};

if(this.readyState!==1){throw new Error("Invalid state");
}p[n]=o;
this.__mJ=qx.util.Uri.appendParamsToUrl(this.__mJ,p);
},send:function(){if(this.__mg){return;
}var s=this.__mQ(),q=this.__mG,r=this;

if(this.timeout>0){this.__bX=window.setTimeout(this.__mF,this.timeout);
}
if(qx.core.Environment.get(k)){qx.Bootstrap.debug(qx.bom.request.Script,"Send native request");
}q.insertBefore(s,q.firstChild);
window.setTimeout(function(){r.__mM(2);
r.__mM(3);
});
},abort:function(){if(this.__mg){return;
}this.__mI=true;
this.__mR();
this.onabort();
},onreadystatechange:function(){},onload:function(){},onloadend:function(){},onerror:function(){},onabort:function(){},ontimeout:function(){},getResponseHeader:function(t){if(this.__mg){return;
}return b;
},getAllResponseHeaders:function(){if(this.__mg){return;
}return c;
},setDetermineSuccess:function(u){qx.core.Assert.assertFunction(u);
this.__mH=u;
},dispose:function(){var v=this.__mK;

if(!this.__mg){if(v){v.onload=v.onreadystatechange=null;
this.__mR();
}
if(this.__bX){window.clearTimeout(this.__bX);
}this.__mg=true;
}},_getUrl:function(){return this.__mJ;
},_getScriptElement:function(){return this.__mK;
},_onTimeout:function(){this.__mO();

if(!this.__mP()){this.onerror();
}this.ontimeout();

if(!this.__mP()){this.onloadend();
}},_onNativeLoad:function(){var y=this.__mK,w=this.__mH,x=this;
if(this.__mI){return;
...
if(this.__bX){window.clearTimeout(this.__bX);
}window.setTimeout(function(){x.__mN();
x.__mM(4);
x.onload();
x.onloadend();
});
},_onNativeError:function(){this.__mO();
this.onerror();
this.onloadend();
},__mK:null,__mG:null,__mJ:h,__mD:null,__mE:null,__mF:null,__bX:null,__mI:null,__mg:null,__mL:function(){this.readyState=0;
this.status=0;
this.statusText=h;
},__mM:function(z){this.readyState=z;
this.onreadystatechange();
},__mN:function(){this.__mR();
this.readyState=4;
if(!this.status){this.status=200;
}this.statusText=h+this.status;
},__mO:function(){this.__mR();
this.readyState=4;
this.status=0;
this.statusText=null;
},__mP:function(){var B=qx.core.Environment.get(j)===i&&qx.core.Environment.get(g)<9;
var A=qx.core.Environment.get(j)===d;
return !(B||A);
},__mQ:function(){var C=this.__mK=document.createElement(e);
C.src=this.__mJ;
C.onerror=this.__mE;
C.onload=this.__mD;
if(qx.core.Environment.get(j)===i&&qx.core.Environment.get(g)<9){C.onreadystatechange=this.__mD;
}return C;
},__mR:function(){var D=this.__mK;

if(D&&D.parentNode){this.__mG.removeChild(D);
...
if(qx.ui.decoration.css3.BorderImage.IS_SUPPORTED){this.__mS=new qx.ui.decoration.css3.BorderImage();

if(p){this.__mT(p);
}}else{this.__mS=new qx.ui.decoration.GridDiv(p);
}
if(q!=null){this.__mS.setInsets(q);
}},properties:{baseImage:{check:d,nullable:true,apply:j},insetLeft:{check:o,nullable:true,apply:n},insetRight:{check:o,nullable:true,apply:n},insetBottom:{check:o,nullable:true,apply:n},insetTop:{check:o,nullable:true,apply:n},insets:{group:[k,l,i,e],mode:f}},members:{__mS:null,getMarkup:function(){return this.__mS.getMarkup();
},resize:function(r,s,t){this.__mS.resize(r,s,t);
},tint:function(u,v){},getInsets:function(){return this.__mS.getInsets();
},_applyInsets:function(w,x,name){var y=g+qx.lang.String.firstUp(name);
this.__mS[y](w);
},_applyBaseImage:function(z,A){if(this.__mS instanceof qx.ui.decoration.GridDiv){this.__mS.setBaseImage(z);
}else{this.__mT(z);
}},__mT:function(B){var G,H,J,I;
this.__mS.setBorderImage(B);
...
}},statics:{IS_SUPPORTED:qx.bom.element.Style.isPropertySupported("borderImage")},properties:{borderImage:{check:t,nullable:true,apply:j},sliceTop:{check:h,init:0,apply:j},sliceRight:{check:h,init:0,apply:j},sliceBottom:{check:h,init:0,apply:j},sliceLeft:{check:h,init:0,apply:j},slice:{group:[n,v,a,w],mode:c},repeatX:{check:[i,e,d],init:i,apply:j},repeatY:{check:[i,e,d],init:i,apply:j},repeat:{group:[u,m],mode:c}},members:{__mU:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__mU;
},getMarkup:function(){if(this.__mU){return this.__mU;
...
this.__mU=[x,qx.bom.element.Style.compile({"borderImage":o+B+p+C.join(f)+f+D,position:k,lineHeight:0,fontSize:0,overflow:l,boxSizing:r,borderWidth:C.join(b)+g}),y].join(q);
return this.__mU;
...
}},members:{__mV:null,__mW:function(o){return qx.Bootstrap.getKeys(o).sort().join(f);
...
}if(this.__mV&&this.__mV.getModelClass&&this.__mV.getModelClass(s)!=null){return;
}var z={};
var y={__mX:this.__mX};
...
if(q){z[t].apply=a;
}if(this.__mV&&this.__mV.getValidationRule){var v=this.__mV.getValidationRule(s,t);

if(v){z[t].validate=g+t;
y[g+t]=v;
}}}if(this.__mV&&this.__mV.getModelSuperClass){var x=this.__mV.getModelSuperClass(s)||qx.core.Object;
}else{var x=qx.core.Object;
}var u=[];

if(this.__mV&&this.__mV.getModelMixins){var w=this.__mV.getModelMixins(s);
if(!qx.lang.Type.isArray(w)){if(w!=null){u=[w];
}}}if(q){u.push(qx.data.marshal.MEventBubbling);
}var r={extend:x,include:u,properties:z,members:y,destruct:this.__mY};
qx.Class.define(k+s,r);
},__mY:function(){var A=qx.util.PropertyUtil.getAllProperties(this.constructor);

for(var B in A){this.__mX(this.get(A[B].name));
}},__mX:function(C){if(!(C instanceof qx.core.Object)){return ;
}if(C.isDisposed()){return;
}C.dispose();
},__na:function(D){var E;
if(this.__mV&&this.__mV.getModelClass){E=this.__mV.getModelClass(D);
}
...
(function(){var o="one",n="single",m="selected",k="additive",j="multi",h="os.name",g="osx",f="under",d="PageUp",c="Left",O="lead",N="Down",M="Up",L="Boolean",K="PageDown",J="anchor",I="End",H="Home",G="Right",F="right",v="click",w="above",t="left",u="Escape",r="A",s="Space",p="_applyMode",q="interval",x="changeSelection",y="qx.event.type.Data",A="quick",z="__oG",C="key",B="abstract",E="drag",D="qx.ui.core.selection.Abstract";
qx.Class.define(D,{type:B,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__iw={};
},events:{"changeSelection":y},properties:{mode:{check:[n,j,k,o],init:n,apply:p},drag:{check:L,init:false},quick:{check:L,init:false}},members:{__oE:0,__oF:0,__oG:null,__oH:null,__oI:null,__oJ:null,__oK:null,__oL:null,__oM:null,__oN:null,__oO:null,__oP:null,__oQ:null,__oR:null,__oS:null,__oT:null,__oU:null,__iw:null,__oV:null,__oW:null,_userInteraction:false,__oX:null,getSelectionContext:function(){return this.__oT;
...
},getSelection:function(){return qx.lang.Object.getValues(this.__iw);
},getSortedSelection:function(){var bg=this.getSelectables();
var bf=qx.lang.Object.getValues(this.__iw);
bf.sort(function(a,b){return bg.indexOf(a)-bg.indexOf(b);
});
return bf;
},isItemSelected:function(bh){var bi=this._selectableToHashCode(bh);
return this.__iw[bi]!==undefined;
},isSelectionEmpty:function(){return qx.lang.Object.isEmpty(this.__iw);
...
},_setLeadItem:function(bl){var bm=this.__oU;
...
},_setAnchorItem:function(bn){var bo=this.__oV;
...
if(this.isItemSelected(bM)&&!bL&&!bO&&!this.getDrag()){this.__oW=bM;
this._userInteraction=false;
return;
}else{this.__oW=null;
...
if(this.getDrag()&&bP!==n&&bP!==o&&!bL&&!bO){this.__oK=this._getLocation();
...
if(!bT&&!bQ&&this.__oW!=null){var bR=this._getSelectableFromMouseEvent(event);
...
this._userInteraction=true;
var bV=this.__oO+this.__oH.left;

if(bV>this.__oL){this.__oQ=1;
}else if(bV<this.__oL){this.__oQ=-1;
}else{this.__oQ=0;
}var bU=this.__oP+this.__oH.top;

if(bU>this.__oM){this.__oR=1;
}else if(bU<this.__oM){this.__oR=-1;
}else{this.__oR=0;
...
this.__oG.addListener(q,this._onInterval,this);
}this.__oG.start();
this._autoSelect();
...
}if(this.__oS){this._fireChange(v);
}delete this.__oN;
delete this.__oI;
delete this.__oJ;
...
var bY=Math.max(0,Math.min(this.__oO-this.__oK.left,cg.width))+this.__oH.left;
var bX=Math.max(0,Math.min(this.__oP-this.__oK.top,cg.height))+this.__oH.top;
if(this.__oI===bY&&this.__oJ===bX){return;
}this.__oI=bY;
this.__oJ=bX;
...
}},__oY:function(cG){var cI={};
var cH;

for(var i=0,l=cG.length;i<l;i++){cH=cG[i];
cI[this._selectableToHashCode(cH)]=cH;
}return cI;
},_getSelectedItem:function(){for(var cJ in this.__iw){return this.__iw[cJ];
}return null;
},_setSelectedItem:function(cK){if(this._isSelectable(cK)){var cL=this.__iw;
...
this.__oS=true;
this._fireChange();
},_fireChange:function(dc){if(this.__oS){this.__oT=dc||null;
...
this.__iw=this.__oW=this.__oV=null;
this.__oU=null;
}});
})();
...
(function(){var j="aborted",i="completed",h="failed",g="configured",f="timeout",e="application/json",d="loaded",c="qx.event.type.Data",b="error",a="receiving",y="changeState",x="sending",w="changeModel",v="json",u="GET",t="fail",s="queued",r="success",q="String",p="changePhase",n="_marshaler",o="__ni",l="qx.data.store.Json",m="changeUrl",k="_applyUrl";
...
}},events:{"loaded":c,"error":c},properties:{model:{nullable:true,event:w},state:{check:[g,s,x,a,i,j,f,h],init:g,event:y},url:{check:q,apply:k,event:m,nullable:true}},members:{_marshaler:null,_delegate:null,__ni:null,__qK:null,_applyUrl:function(B,C){if(B!=null){B=qx.util.AliasManager.getInstance().resolve(B);
B=qx.util.ResourceManager.getInstance().toUri(B);
this._createRequest(B);
}},_getRequest:function(){return this.__ni;
},_setRequest:function(D){this.__ni=D;
...
},isDeprecatedTransport:function(){return !!this.__qK;
},_createRequest:function(F){if(this.__ni){this.__ni.dispose();
this.__ni=null;
}
...
G.send();
},__qL:function(I){var J=new qx.io.remote.Request(I,u,e);
this._setRequest(J);
J.addListener(i,this.__qN,this);
...
J.addListener(h,this.__qM,this);
J.addListener(j,this.__qM,this);
J.addListener(f,this.__qM,this);
J.send();
...
},__qM:function(T){this.fireDataEvent(b,T);
...
if(this.__ni){this.__ni.dispose();
this.__ni=null;
}},__qN:function(ba){var bc=ba.getContent();
...
}},destruct:function(){if(this.__ni!=null){this._disposeObjects(o);
...
s.send();
},__nb:function(u){if(this._getRequest()){this._getRequest().dispose();
...
},__nc:function(A){if(A==undefined){this.setState(b);
this.fireEvent(n);
return;
...
}this.__nd=qx.bom.Iframe.create({id:H,name:H,src:J});
qx.bom.element.Style.set(this.__nd,y,z);
...
qx.dom.Element.insertEnd(this.__nd,qx.dom.Node.getBodyElement(document));
qx.event.Registration.addListener(this.__nd,a,this._onload,this);
this.__ne=qx.lang.Function.listener(this._onreadystatechange,this);
qx.bom.Event.addNativeListener(this.__nd,D,this.__ne);
...
},_numericMap:{"uninitialized":1,"loading":2,"loaded":2,"interactive":3,"complete":4}},members:{__dc:null,__nf:0,__Q:null,__nd:null,__ne:null,send:function(){var L=this.getMethod();
var N=this.getUrl();
...
}),_onreadystatechange:qx.event.GlobalError.observeMethod(function(e){this._switchReadyState(qx.io.remote.transport.Iframe._numericMap[this.__nd.readyState]);
}),_switchReadyState:function(S){switch(this.getState()){case C:case s:case p:case u:this.warn("Ignore Ready State Change");
return;
}while(this.__nf<S){this.setState(qx.io.remote.Exchange._nativeMap[++this.__nf]);
}},setRequestHeader:function(T,U){},getResponseHeader:function(V){return null;
},getResponseHeaders:function(){return {};
},getStatusCode:function(){return 200;
},getStatusText:function(){return k;
},getIframeWindow:function(){return qx.bom.Iframe.getWindow(this.__nd);
},getIframeDocument:function(){return qx.bom.Iframe.getDocument(this.__nd);
},getIframeBody:function(){return qx.bom.Iframe.getBody(this.__nd);
...
},destruct:function(){if(this.__nd){qx.event.Registration.removeListener(this.__nd,a,this._onload,this);
qx.bom.Event.removeNativeListener(this.__nd,D,this.__ne);
if((qx.core.Environment.get(j)==x)){this.__nd.src=qx.util.ResourceManager.getInstance().toUri(v);
}qx.dom.Element.remove(this.__nd);
}
if(this.__Q){qx.dom.Element.remove(this.__Q);
}this.__nd=this.__Q=this.__dc=null;
}});
})();
...
this.addListener(t,this.__ng,this);
this.addListener(n,this.__ng,this);
},members:{_stopPropagation:function(e){e.stopPropagation();
},__ng:function(){var A=this.getStyle(k);
...
}},properties:{parseJson:{check:C,init:true}},members:{__nh:false,__nf:0,__ni:null,getRequest:function(){if(this.__ni===null){this.__ni=qx.io.remote.transport.XmlHttp.createRequestObject();
this.__ni.onreadystatechange=qx.lang.Function.bind(this._onreadystatechange,this);
}return this.__ni;
},send:function(){this.__nf=0;
...
if(bk==4){if(!qx.io.remote.Exchange.wasSuccessful(this.getStatusCode(),bk,this.__nh)){if(this.getState()===b){this.setState(g);
}this.failed();
return;
}}if(bk==3&&this.__nf==1){this.setState(qx.io.remote.Exchange._nativeMap[++this.__nf]);
}while(this.__nf<bk){this.setState(qx.io.remote.Exchange._nativeMap[++this.__nf]);
...
if(qx.io.remote.Exchange.wasSuccessful(bF,bG,this.__nh)){try{bH=this.getRequest().responseXML;
...
}r.include=this.__ok(r.include);
r.patch=this.__ok(r.patch);
...
}},__ok:function(t){if(!t){return [];
}
if(qx.Bootstrap.isArray(t)){return t;
}else{return [t];
}},__ol:function(u,v){var w=v.aliases||{};
...
},__om:function(x){for(var i=0,y=this.__oo,l=y.length;i<l;i++){if(x[y[i]]){return y[i];
}}},__on:function(z,A){var D=this.__om(A);
...
z[D]=new F;
},$$registry:{},__oo:[g,d,f,n,b,p,m,e],__ge:null,__op:null,__gg:function(){},patch:function(G,H){this.__oq(H);
var J=this.__om(H);

if(J!==this.__om(G)){throw new Error("The mixins '"+G.name+"' are not compatible '"+H.name+"'!");
}var I=H[J];
var K=G.$$clazz.prototype;

for(var L in I){K[L]=I[L];
}},include:function(M,N){this.__oq(N);
var P=N.type;
...
}},__oq:function(S){if(typeof S===o||S==null){var T;
...
}},members:{__mU:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__mU;
},getMarkup:function(e){if(this.__mU){return this.__mU;
...
qx.Mixin.define(o,{include:[qx.ui.decoration.MSingleBorder,qx.ui.decoration.MBackgroundImage],construct:function(){this._getDefaultInsetsForBorder=this.__qU;
this._resizeBorder=this.__qT;
this._styleBorder=this.__qR;
this._generateMarkup=this.__qS;
},properties:{innerWidthTop:{check:f,init:0},innerWidthRight:{check:f,init:0},innerWidthBottom:{check:f,init:0},innerWidthLeft:{check:f,init:0},innerWidth:{group:[w,z,n,k],mode:a},innerColorTop:{nullable:true,check:g},innerColorRight:{nullable:true,check:g},innerColorBottom:{nullable:true,check:g},innerColorLeft:{nullable:true,check:g},innerColor:{group:[u,v,x,r],mode:a}},members:{__qQ:null,__qR:function(D){var E=qx.theme.manager.Color.getInstance();
D.position=t;
...
}},__qS:function(G){var J=this._generateBackgroundMarkup(G);
...
G[y]=0;
G[p]=0;
return this.__qQ=this._generateBackgroundMarkup(G,J);
},__qT:function(K,L,M){var N=this.getInsets();
L-=N.left+N.right;
M-=N.top+N.bottom;
...
},__qU:function(){return {top:this.getWidthTop()+this.getInnerWidthTop(),right:this.getWidthRight()+this.getInnerWidthRight(),bottom:this.getWidthBottom()+this.getInnerWidthBottom(),left:this.getWidthLeft()+this.getInnerWidthLeft()};
}}});
})();
...
}},properties:{innerColor:{check:d,nullable:true,apply:i},innerOpacity:{check:o,init:1,apply:i},outerColor:{check:d,nullable:true,apply:i}},members:{__mU:null,_getDefaultInsets:function(){return {top:2,right:2,bottom:2,left:2};
},_isInitialized:function(){return !!this.__mU;
},_applyStyle:function(){},getMarkup:function(){if(this.__mU){return this.__mU;
...
},members:{__nj:null,add:function(g){this.__nj.push(g);
this.__nk();
},remove:function(h){if(this.__nj){qx.lang.Array.remove(this.__nj,h);
this.__nk();
...
this._setPhase(p);
this.__nm=qx.lang.Function.bind(this._onReadyStateChange,this);
this.__nn=qx.lang.Function.bind(this._onLoad,this);
this.__no=qx.lang.Function.bind(this._onLoadEnd,this);
this.__np=qx.lang.Function.bind(this._onAbort,this);
this.__mF=qx.lang.Function.bind(this._onTimeout,this);
this.__nq=qx.lang.Function.bind(this._onError,this);
E.onreadystatechange=this.__nm;
E.onload=this.__nn;
E.onloadend=this.__no;
E.onabort=this.__np;
E.ontimeout=this.__mF;
E.onerror=this.__nq;
...
},nullable:true},authentication:{check:A,nullable:true}},members:{__nm:null,__nn:null,__no:null,__np:null,__mF:null,__nq:null,__nr:null,__mI:null,__ns:null,_transport:null,_createTransport:function(){throw new Error("Abstract method call");
...
},getPhase:function(){return this.__ns;
...
},getResponse:function(){return this.__nr;
},_setResponse:function(M){var N=M;

if(this.__nr!==M){this.__nr=M;
this.fireEvent(m,qx.event.type.Data,[this.__nr,N]);
...
this.fireEvent(Q);
},_setPhase:function(R){var S=this.__ns;
this.__ns=R;
...
}},__nt:function(){var V=this.getRequestHeaders();

if(V){qx.lang.Object.getKeys(V).forEach(function(W){this._transport.setRequestHeader(W,V[W]);
},this);
}},__nu:function(){var Y=this.getAuthentication(),X=this._transport;
...
(function(){var i="qx.ui.window.Window",h="changeModal",g="changeVisibility",f="changeActive",d="_applyActiveWindow",c="__x",b="__nX",a="qx.ui.window.MDesktop";
qx.Mixin.define(a,{properties:{activeWindow:{check:i,apply:d,init:null,nullable:true}},members:{__nX:null,__x:null,getWindowManager:function(){if(!this.__x){this.setWindowManager(new qx.ui.window.Window.DEFAULT_MANAGER_CLASS());
}return this.__x;
...
this.getWindowManager().updateStack();
},getWindows:function(){if(!this.__nX){this.__nX=[];
}return this.__nX;
}},destruct:function(){this._disposeArray(b);
this._disposeObjects(c);
}});
})();
(function(){var f="_applyBlockerColor",e="__mw",d="Number",c="qx.ui.core.MBlocker",b="_applyBlockerOpacity",a="Color";
qx.Mixin.define(c,{construct:function(){this.__mw=this._createBlocker();
},properties:{blockerColor:{check:a,init:null,nullable:true,apply:f,themeable:true},blockerOpacity:{check:d,init:1,apply:b,themeable:true}},members:{__mw:null,_createBlocker:function(){return new qx.ui.core.Blocker(this);
},_applyBlockerColor:function(g,h){this.__mw.setColor(g);
},_applyBlockerOpacity:function(i,j){this.__mw.setOpacity(i);
},block:function(){this.__mw.block();
},isBlocked:function(){return this.__mw.isBlocked();
},unblock:function(){this.__mw.unblock();
},forceUnblock:function(){this.__mw.forceUnblock();
},blockContent:function(k){this.__mw.blockContent(k);
},isContentBlocked:function(){return this.__mw.isContentBlocked();
},unblockContent:function(){this.__mw.unblockContent();
},forceUnblockContent:function(){this.__mw.forceUnblockContent();
},getBlocker:function(){return this.__mw;
...
qx.ui.core.queue.Visibility.add(this);
this.initNativeHelp();
this.addListener(p,this.__qP,this);
},properties:{appearance:{refine:true,init:n},enabled:{refine:true,init:true},focusable:{refine:true,init:true},globalCursor:{check:i,nullable:true,themeable:true,apply:d,event:q},nativeContextMenu:{refine:true,init:false},nativeHelp:{check:o,init:false,apply:h}},members:{__qO:null,isRootWidget:function(){return true;
},getLayout:function(){return this._getLayout();
},_applyGlobalCursor:qx.core.Environment.select(t,{"mshtml":function(u,v){},"default":function(w,x){var y=qx.bom.Stylesheet;
var z=this.__qO;

if(!z){this.__qO=z=y.createElement();
}y.removeAllRules(z);
...
},__qP:function(e){if(e.getKeyIdentifier()!==j){return;
}var D=e.getTarget();
...
}},"default":function(){}})},destruct:function(){this.__qO=null;
},defer:function(H,I){qx.ui.core.MChildrenHandling.remap(I);
}});
})();
...
},members:{__cp:null,__nv:null,_createContainerElement:function(){var p=this.__nv;
...
}}},destruct:function(){this.__cp=this.__nv=null;
}});
})();
...
var s=++qx.io.remote.transport.Script.__nw;

if(s>=2000000000){qx.io.remote.transport.Script.__nw=s=1;
}this.__iC=null;
this.__nw=s;
},statics:{__nw:0,_instanceRegistry:{},ScriptTransport_PREFIX:f,ScriptTransport_ID_PARAM:b,ScriptTransport_DATA_PARAM:h,handles:{synchronous:false,asynchronous:true,crossDomain:true,fileUpload:false,programaticFormFields:false,responseTypes:[m,l,o]},isSupported:function(){return true;
...
}})},members:{__nf:0,__iC:null,__nw:null,send:function(){var x=this.getUrl();
x+=(x.indexOf(p)>=0?r:p)+qx.io.remote.transport.Script.ScriptTransport_ID_PARAM+q+this.__nw;
...
}qx.io.remote.transport.Script._instanceRegistry[this.__nw]=this;
this.__iC=document.createElement(c);
this.__iC.charset=e;
this.__iC.src=x;
document.body.appendChild(this.__iC);
},_switchReadyState:function(B){switch(this.getState()){case n:case a:case d:case g:this.warn("Ignore Ready State Change");
return;
}while(this.__nf<B){this.setState(qx.io.remote.Exchange._nativeMap[++this.__nf]);
...
},destruct:function(){if(this.__iC){delete qx.io.remote.transport.Script._instanceRegistry[this.__nw];
document.body.removeChild(this.__iC);
}this.__iC=this._responseContent=null;
}});
})();
...
}if(this.getSelection()==null){this.__rb=new qx.data.Array();
this.setSelection(this.__rb);
}},properties:{selection:{check:d,event:f,apply:b,init:null}},members:{_modifingSelection:0,__rc:null,__rd:null,__rb:null,_applySelection:function(g,h){if(this.__rd!=undefined&&h!=undefined){h.removeListenerById(this.__rd);
}this.__rd=g.addListener(e,this.__re,this);
this._updateSelection();
},__re:function(){this._updateSelection();
},_changeTargetSelection:function(){if(this.getTarget()==null){return;
}if(!this.__rf()&&!this.__rg()){return;
...
},_addChangeTargetListener:function(m,n){if(this.__rc!=undefined&&n!=undefined){n.removeListenerById(this.__rc);
}
if(m!=null){if(this.__rf()||this.__rg()){this.__rc=m.addListener(f,this._changeTargetSelection,this);
...
}}}else if(this.__rg()){var p=this.getSelection().getItem(this.getSelection().length-1);

if(p!==undefined){this.__rh(p);
this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();
}else{this.getTarget().resetSelection();
}}this._endSelectionModification();
},__rf:function(){var t=this.getTarget().constructor;
return qx.Class.implementsInterface(t,qx.ui.core.IMultiSelection);
},__rg:function(){var u=this.getTarget().constructor;
return qx.Class.implementsInterface(u,qx.ui.core.ISingleSelection);
},__rh:function(v){var w=this.__ri(v);
if(w==null){return;
}if(this.__rf()){this.getTarget().addToSelection(w);
}else if(this.__rg()){this.getTarget().setSelection([w]);
}},__ri:function(x){var y=this.getTarget().getSelectables(true);
...
},_applyIconOptions:function(F,G){this.__nJ();
},_applyLabelOptions:function(H,I){this.__nJ();
},_applyIconPath:function(J,K){this.__nJ();
},_applyLabelPath:function(L,M){this.__nJ();
},_applyModel:function(N,O){if(O!=undefined){if(this.__nB!=undefined){O.removeListenerById(this.__nB);
}}if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();
}if(N!=null){this.__nB=N.addListener(k,this.__nE,this);
this.__nL();
this.__nF();
...
for(var i=0;i<length;i++){this.__nI();
...
if(Q!=null){if(this.getModel()!=null){for(var i=0;i<this.__nx.length;i++){this.__nH(this.__nM(i));
}}}},__nE:function(){this.__nD=true;
qx.ui.core.queue.Widget.add(this);
if(this.__nx.length!=this.getModel().getLength()){this.update();
}},syncWidget:function(){if(this.__nC){this._changeTargetSelection();
}
if(this.__nD){this._updateSelection();
}this.__nD=this.__nC=null;
},__nF:function(){if(this.getTarget()==null){return;
}this.__nL();
var U=this.__nx.length;
var T=this.getTarget().getChildren().length;
if(U>T){for(var j=T;j<U;j++){this.__nH(this.__nM(j));
}}else if(U<T){for(var j=T;j>U;j--){this.__nI();
}}this.__nC=true;
qx.ui.core.queue.Widget.add(this);
},__nG:function(){var V=this.getModel();
if(V!=null){V.removeListenerById(this.__nB);
this.__nB=V.addListener(k,this.__nE,this);
...
},__nH:function(Y){var ba=this._createItem();
this._bindListItem(ba,Y);
this.getTarget().add(ba);
},__nI:function(){this._startSelectionModification();
...
if(!qx.lang.Array.contains(this.__ny,bn)){this.__ny.push(bn);
...
if(!qx.lang.Array.contains(this.__nz,bt)){this.__nz.push(bt);
}},_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;
}for(var i=0;i<this.__ny.length;i++){if(this.__nA[this.__ny[i]]!=null){this.__nA[this.__ny[i]]();
}}},_removeBindingsFrom:function(bD){for(var i=0;i<this.__ny.length;i++){var bE=bD.getUserData(this.__ny[i]+g);

if(bE!=null){this.removeBinding(bE);
}}for(var i=0;i<this.__nz.length;i++){var bE=bD.getUserData(this.__nz[i]+h);

if(bE!=null){bD.removeBinding(bE);
}}},__nJ:function(){if(this.getTarget()==null||this.getModel()==null){return;
}var bF=this.getTarget().getChildren();
for(var i=0;i<bF.length;i++){this._removeBindingsFrom(bF[i]);
this._bindListItem(bF[i],this.__nM(i));
}this.__nG();
...
this._endSelectionModification();
this._updateSelection();
},_setFilter:function(bP,bQ){if((bP==null||bP.filter==null)&&(bQ!=null&&bQ.filter!=null)){this.__nK();
...
this.__nL();
if(bS.length>this.__nx.length){for(var j=bS.length;j>this.__nx.length;j--){this.getTarget().removeAt(j-1).destroy();
}}else if(bS.length<this.__nx.length){for(var j=bS.length;j<this.__nx.length;j++){var bT=this._createItem();
this.getTarget().add(bT);
}}var bR=this.getTarget().getChildren();

for(var i=0;i<bR.length;i++){this._bindListItem(bR[i],this.__nM(i));
}this.__nG();
this._endSelectionModification();
this._updateSelection();
},__nK:function(){this.__nL();
this.__nF();
this.__nJ();
this.__nD=true;
qx.ui.core.queue.Widget.add(this);
},__nL:function(){var bW=this.getModel();
...
for(var i=0;i<bW.getLength();i++){if(bV==null||bV(bW.getItem(i))){this.__nx.push(i);
}}},__nM:function(bY){return this.__nx[bY];
}},destruct:function(){this.__nx=this.__nA=this.__ny=null;
this.__nz=null;
qx.ui.core.queue.Widget.remove(this);
}});
})();
...
this.__nT();
},members:{responseJson:null,__df:null,__nN:null,__nO:null,__nP:null,__nQ:null,__mg:null,open:function(j,k){if(this.__mg){return;
}var l={},n,m,o=this;
this.responseJson=null;
this.__nP=false;
n=this.__nN||g;
m=this.__nO||a+this.__df+b;
if(!this.__nO){this.constructor[this.__df]=this;
}else{if(!window[this.__nO]){this.__nQ=true;
window[this.__nO]=function(p){o.callback(p);
};
}else{if(qx.core.Environment.get(i)){qx.log.Logger.debug(qx.bom.request.Jsonp,"Callback "+this.__nO+" already exists");
}}}
...
this.__nS(f,[j,k]);
},callback:function(q){if(this.__mg){return;
}this.__nP=true;
this.responseJson=q;
this.constructor[this.__df]=undefined;
this.__nR();
},setCallbackParam:function(r){this.__nN=r;
},setCallbackName:function(name){this.__nO=name;
},dispose:function(){this.__nR();
this.__nS(e);
},_onNativeLoad:function(){this.status=this.__nP?200:500;
this.__nS(c);
},__nR:function(){if(this.__nQ&&window[this.__nO]){window[this.__nO]=undefined;
this.__nQ=false;
}},__nS:function(s,t){qx.bom.request.Script.prototype[s].apply(this,t||[]);
},__nT:function(){this.__df=(new Date().valueOf())+(d+Math.random()).substring(2,5);
}}});
})();
(function(){var n="pressed",m="abandoned",l="Integer",k="hovered",j="qx.event.type.Event",i="Enter",h="Space",g="press",f="qx.ui.form.RepeatButton",d="release",a="interval",c="execute",b="__kl";
qx.Class.define(f,{extend:qx.ui.form.Button,construct:function(o,p){qx.ui.form.Button.call(this,o,p);
this.__kl=new qx.event.AcceleratingTimer();
this.__kl.addListener(a,this._onInterval,this);
},events:{"execute":j,"press":j,"release":j},properties:{interval:{check:l,init:100},firstInterval:{check:l,init:500},minTimer:{check:l,init:20},timerDecrease:{check:l,init:2}},members:{__nU:null,__kl:null,press:function(){if(this.isEnabled()){if(!this.hasState(n)){this.__nV();
}this.removeState(m);
this.addState(n);
}},release:function(q){if(!this.isEnabled()){return;
}if(this.hasState(n)){if(!this.__nU){this.execute();
}}this.removeState(n);
this.removeState(m);
this.__nW();
...
if(this.hasState(n)&&!this.__nU){this.execute();
}}this.__nW();
e.stopPropagation();
},_onKeyUp:function(e){switch(e.getKeyIdentifier()){case i:case h:if(this.hasState(n)){if(!this.__nU){this.execute();
}this.removeState(n);
this.removeState(m);
e.stopPropagation();
this.__nW();
...
e.stopPropagation();
this.__nV();
}},_onInterval:function(e){this.__nU=true;
this.fireEvent(c);
},__nV:function(){this.fireEvent(g);
this.__nU=false;
this.__kl.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();
this.removeState(m);
this.addState(n);
},__nW:function(){this.fireEvent(d);
this.__kl.stop();
this.removeState(m);
this.removeState(n);
}},destruct:function(){this._disposeObjects(b);
...
(function(){var u="keypress",t="interval",s="keydown",r="mousedown",q="keyup",p="blur",o="Enter",n="__nY",m="__oa",l="Up",d="__nj",k="Escape",h="event.touch",c="qx.ui.menu.Manager",b="Left",g="Down",f="Right",j="singleton",a="Space";
...
this.__nY.addListener(t,this._onOpenInterval,this);
this.__oa=new qx.event.Timer;
this.__oa.addListener(t,this._onCloseInterval,this);
},members:{__ob:null,__oc:null,__nY:null,__oa:null,__nj:null,_getChild:function(x,y,z,A){var B=x.getChildren();
var length=B.length;
var C;
...
},add:function(F){var G=this.__nj;
G.push(F);
F.setZIndex(1e6+G.length);
},remove:function(H){var I=this.__nj;

if(I){qx.lang.Array.remove(I,H);
}},hideAll:function(){var J=this.__nj;

if(J){for(var i=J.length-1;i>=0;i--){J[i].exclude();
}}},getActiveMenu:function(){var K=this.__nj;
return K.length>0?K[K.length-1]:null;
},scheduleOpen:function(L){this.cancelClose(L);
if(L.isVisible()){if(this.__ob){this.cancelOpen(this.__ob);
}}else if(this.__ob!=L){this.__ob=L;
this.__nY.restartWith(L.getOpenInterval());
}},scheduleClose:function(M){this.cancelOpen(M);
if(!M.isVisible()){if(this.__oc){this.cancelClose(this.__oc);
}}else if(this.__oc!=M){this.__oc=M;
this.__oa.restartWith(M.getCloseInterval());
}},cancelOpen:function(N){if(this.__ob==N){this.__nY.stop();
this.__ob=null;
}},cancelClose:function(O){if(this.__oc==O){this.__oa.stop();
this.__oc=null;
}},_onOpenInterval:function(e){this.__nY.stop();
this.__ob.open();
this.__ob=null;
},_onCloseInterval:function(e){this.__oa.stop();
this.__oc.exclude();
this.__oc=null;
...
}if(this.__nj.length>0&&!this._isInMenu(P)){this.hideAll();
}},__od:{"Enter":1,"Space":1},__oe:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var Q=this.getActiveMenu();

if(!Q){return;
}var R=e.getKeyIdentifier();

if(this.__oe[R]||(this.__od[R]&&Q.getSelectedButton())){e.stopPropagation();
...
},init:true}},members:{__of:null,_createTransport:function(){return new qx.bom.request.Xhr();
...
if(typeof z.PARSER[y]===n){return this.__of=z.PARSER[y];
}return this.__of=y;
},_getParser:function(){var A=this.__of,B;
if(A){return A;
...
},__og:function(F){var G=function(){qx.bom.Event.removeNativeListener(F,h,G);
F.$$url=qx.bom.Iframe.queryCurrentUrl(F);
};
qx.bom.Event.addNativeListener(F,h,G);
}}});
})();
(function(){var t="Integer",s="aborted",r="_onaborted",q="_on",p="_applyEnabled",o="Boolean",n="sending",m="interval",l="__kl",k="failed",c="qx.io.remote.RequestQueue",j="timeout",g="completed",b="queued",a="__oh",f="io.maxrequests",d="receiving",h="singleton";
qx.Class.define(c,{type:h,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__db=[];
this.__oh=[];
this.__oi=0;
this.__kl=new qx.event.Timer(500);
this.__kl.addListener(m,this._oninterval,this);
},properties:{enabled:{init:true,check:o,apply:p},maxTotalRequests:{check:t,nullable:true},maxConcurrentRequests:{check:t,init:qx.core.Environment.get(f)},defaultTimeout:{check:t,init:5000}},members:{__db:null,__oh:null,__oi:null,__kl:null,getRequestQueue:function(){return this.__db;
},getActiveQueue:function(){return this.__oh;
},_debug:function(){var u;
},_check:function(){this._debug();
if(this.__oh.length==0&&this.__db.length==0){this.__kl.stop();
}if(!this.getEnabled()){return;
}if(this.__db.length==0||(this.__db[0].isAsynchronous()&&this.__oh.length>=this.getMaxConcurrentRequests())){return;
}if(this.getMaxTotalRequests()!=null&&this.__oi>=this.getMaxTotalRequests()){return;
...
}},_remove:function(x){qx.lang.Array.remove(this.__oh,x);
x.dispose();
this._check();
...
z[r](event);
}}catch(B){}}},_oninterval:function(e){var I=this.__oh;

if(I.length==0){this.__kl.stop();
return;
...
if(this.getEnabled()){this.__kl.start();
...
B+=this.__or(name,z,w);
}}else if(qx.lang.Type.isDate(y)&&x!=null){B+=this.__or(name,x.format(y),w);
}else{B+=this.__or(name,y,w);
}}return B.substring(0,B.length-1);
},__or:function(name,D,E){if(D instanceof qx.core.Object&&E!=null){var F=encodeURIComponent(E(D));
...
},events:{"interval":c},properties:{interval:{check:e,init:100},firstInterval:{check:e,init:500},minimum:{check:e,init:20},decrease:{check:e,init:2}},members:{__kl:null,__ox:null,start:function(){this.__kl.setInterval(this.getFirstInterval());
this.__kl.start();
},stop:function(){this.__kl.stop();
this.__ox=null;
},_onInterval:function(){this.__kl.stop();

if(this.__ox==null){this.__ox=this.getInterval();
}this.__ox=Math.max(this.getMinimum(),this.__ox-this.getDecrease());
this.__kl.setInterval(this.__ox);
this.__kl.start();
this.fireEvent(d);
}},destruct:function(){this._disposeObjects(a);
}});
})();
(function(){var m="separator-vertical",l="Posts",k="bold",j="__oy",i="_applyLoading",h="__oz",g="center",f="feedreader.view.List",e="single",d="middle",a="__fD",c="feedreader/images/loading66.gif",b="Boolean";
...
this.__oy=new qx.ui.form.List();
this.__oy.setDecorator(null);
this.__oy.setSelectionMode(e);
this.__oy.setPadding(0);
this.__oy.setMargin(0);
this.__fD.add(this.__oy);
this.__oz=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,g));
var n=new qx.ui.basic.Image(c);
n.setAlignY(d);
this.__oz.add(n);
this.__fD.add(this.__oz);
},properties:{loading:{check:b,init:false,apply:i}},members:{__fD:null,__oy:null,__oz:null,_applyLoading:function(q,r){if(q){this.__fD.setSelection([this.__oz]);
}else{this.__fD.setSelection([this.__oy]);
}},getList:function(){return this.__oy;
}},destruct:function(){this._disposeObjects(j,a,h);
}});
})();
...
this.addListener(n,function(){this.__hW={"x":0,"y":0};
},this);
this.__hW={};
this.__qY={};
}},properties:{appearance:{refine:true,init:r},width:{refine:true,init:100},height:{refine:true,init:200},scrollbarX:{check:[h,c,E],init:h,themeable:true,apply:b},scrollbarY:{check:[h,c,E],init:h,themeable:true,apply:b},scrollbar:{group:[x,u]}},members:{__hW:null,__qY:null,_createChildControlImpl:function(H,I){var J;
...
if(Y&&X){if(this.__hW[U]==0){var W=0;
}else{var W=-(e[a+V]()-this.__hW[U]);
}this.__hW[U]=e[a+V]();
X.scrollBy(W);
if(this.__qY[U]){clearTimeout(this.__qY[U]);
this.__qY[U]=null;
}this.__qY[U]=setTimeout(qx.lang.Function.bind(function(ba){this.__ra(ba,U);
},this,W),100);
}},__ra:function(bb,bc){this.__qY[bc]=null;
var be=this._isChildControlVisible(d+bc);

if(bb==0||!be){return;
}if(bb>0){bb=Math.max(0,bb-3);
}else{bb=Math.min(0,bb+3);
}this.__qY[bc]=setTimeout(qx.lang.Function.bind(function(bf,bg){this.__ra(bf,bg);
},this,bb,bc),20);
...
},_getContentPaddingTarget:function(){return this.__oB;
...
t.addListener(l,this.__oD(r),this);
t.addListener(c,qx.lang.Function.bind(this.__oC,this,r),this);
},__oC:function(v,e){if(e.getData()==a||e.getData()==g||e.getData()==n){v.setState(m);
}},__oD:function(w){return function(e){var x=e.getData();
if(x==n){w.setState(m);
return;
}w.setArticles(x);
w.setState(l);
...
if(this.__pV){this._remove(K);
}this._add(K);
this.__pV=true;
},addLabel:function(L){var M=this.getChildControl(f);

if(this.__pU){this._remove(M);
}
if(L){this.setLabel(L);
}else{M.setValue(this.getLabel());
}this._add(M);
this.__pU=true;
},_applyIcon:function(N,O){if(!this.__pY()){this.__qa(N);
}else if(!this.isOpen()){this.__qa(N);
}},_applyIconOpened:function(P,Q){if(this.isOpen()){if(this.__pX()&&this.__pY()){this.__qa(P);
}else if(!this.__pX()&&this.__pY()){this.__qa(P);
...
if(U){W=this.__pY()?this.getIconOpened():null;
}else{W=this.getIcon();
}
if(W){this.__qa(W);
}U?this.addState(c):this.removeState(c);
},__pX:function(){return qx.util.PropertyUtil.getUserValue(this,j);
},__pY:function(){return qx.util.PropertyUtil.getUserValue(this,s);
},__qa:function(X){var Y=this.getChildControl(j,true);
...
if(this.__pW){this.__pW.setWidth((this.getLevel()+1)*this.getIndent()-be);
...
(function(){var j="visible",h="excluded",g="qx.ui.tree.core.AbstractTreeItem",f="open",e="abstract",d="addItem",c="removeItem",b="__iO",a="__qW";
...
this.__iO=[];
},properties:{parent:{check:g,nullable:true}},members:{__iO:null,__qW:null,getTree:function(){var n=this;
...
}}},getChildrenContainer:function(){if(!this.__qW){this.__qW=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?j:h});
}return this.__qW;
},hasChildrenContainer:function(){return this.__qW;
...
}},__qX:function(){if(this.getParentChildrenContainer()){this.getParentChildrenContainer()._addAfter(this.getChildrenContainer(),this);
...
}},addAt:function(O,P){if(P==this.__iO.length){this.add(O);
return;
}var T=O.getParent();
...
}qx.lang.Array.insertAt(this.__iO,O,P);
...
if(W){W.remove(U);
}this.addAt(U,this.__iO.indexOf(V));
},addAfter:function(X,Y){var ba=X.getParent();

if(ba){ba.remove(X);
}this.addAt(X,this.__iO.indexOf(Y)+1);
},addAtBegin:function(bb){this.addAt(bb,0);
},remove:function(bc){for(var i=0,l=arguments.length;i<l;i++){var bg=arguments[i];

if(this.__iO.indexOf(bg)==-1){this.warn("Cannot remove treeitem '"+bg+"'. It is not a child of this tree item.");
return;
...
}}qx.lang.Array.remove(this.__iO,bg);
bg.setParent(null);
bd.remove(bg);
}var be=this.getTree();
...
},removeAt:function(bh){var bi=this.__iO[bh];

if(bi){this.remove(bi);
}},removeAll:function(){var bj=this.__iO.concat();

for(var i=this.__iO.length-1;i>=0;i--){this.remove(this.__iO[i]);
}return bj;
}},destruct:function(){this._disposeArray(b);
...
}},properties:{sort:{check:[l,m,n],init:l,apply:j},separatorX:{check:p,nullable:true,apply:s},separatorY:{check:p,nullable:true,apply:s},connectSeparators:{check:e,init:false,apply:s},spacingX:{check:o,init:0,apply:s},spacingY:{check:o,init:0,apply:s}},members:{__iO:null,__pa:null,verifyLayoutProperty:null,_applySort:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__pb:{north:1,south:2,west:3,east:4,center:5},__pc:{1:q,2:d,3:r,4:g},__jz:function(){var x=this._getLayoutChildren();
var F,z;
var length=x.length;
var B=[];
var E=[];
...
delete this._invalidChildrenCache;
},renderLayout:function(H,I){if(this._invalidChildrenCache){this.__jz();
}var bc=qx.ui.layout.Util;
var O=this.__iO;
var bd=this.__pa;
var length=O.length;
...
}},destruct:function(){this.__pa=this.__iO=null;
}});
})();
(function(){var c="Integer",b="_applyLayoutChange",a="qx.ui.menu.Layout";
qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:c,init:0,apply:b},spanColumn:{check:c,init:1,nullable:true,apply:b},iconColumnWidth:{check:c,init:0,themeable:true,apply:b},arrowColumnWidth:{check:c,init:0,themeable:true,apply:b}},members:{__pd:null,_computeSizeHint:function(){var q=this._getLayoutChildren();
var o,g,j;
var e=this.getSpanColumn();
var h=this.__pd=[0,0,0,0];
...
},getColumnSizes:function(){return this.__pd||null;
...
},events:{"execute":R},properties:{enabled:{init:true,check:l,event:o,apply:I},shortcut:{check:n,apply:Q,nullable:true},autoRepeat:{check:l,init:false}},members:{__pe:g,__pf:g,execute:function(U){this.fireDataEvent(L,U);
},__pg:function(event){if(this.getEnabled()&&this.__ph(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},__dD:function(event){if(this.getEnabled()&&this.__ph(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},_applyEnabled:function(V,W){if(V){qx.event.Registration.addListener(document.documentElement,h,this.__pg,this);
qx.event.Registration.addListener(document.documentElement,d,this.__dD,this);
}else{qx.event.Registration.removeListener(document.documentElement,h,this.__pg,this);
...
this.error(bc);
throw new Error(bc);
}this.__pe={"Control":false,"Shift":false,"Meta":false,"Alt":false};
this.__pf=null;
var ba;
var a=[];
...
for(var i=0;i<bb;i++){var bd=this.__pj(a[i]);

switch(bd){case f:case u:case K:case C:this.__pe[bd]=true;
break;
case m:var bc=F+a[i];
this.error(bc);
throw bc;
default:if(this.__pf){var bc=r;
this.error(bc);
throw bc;
}this.__pf=bd;
}}}return true;
},__ph:function(e){var be=this.__pf;

if(!be){return ;
}if((!this.__pe.Shift&&e.isShiftPressed())||(this.__pe.Shift&&!e.isShiftPressed())||(!this.__pe.Control&&e.isCtrlPressed())||(this.__pe.Control&&!e.isCtrlPressed())||(!this.__pe.Meta&&e.isMetaPressed())||(this.__pe.Meta&&!e.isMetaPressed())||(!this.__pe.Alt&&e.isAltPressed())||(this.__pe.Alt&&!e.isAltPressed())){return false;
}
if(be==e.getKeyIdentifier()){return true;
}return false;
},__pi:{esc:S,ctrl:f,print:P,del:q,pageup:b,pagedown:E,numlock:O,numpad_0:J,numpad_1:D,numpad_2:t,numpad_3:s,numpad_4:A,numpad_5:N,numpad_6:B,numpad_7:H,numpad_8:M,numpad_9:z,numpad_divide:p,numpad_multiply:v,numpad_minus:c,numpad_plus:k},__pj:function(bf){var bg=qx.event.handler.Keyboard;
var bh=m;
...
var bh=this.__pi[bf]||qx.lang.String.firstUp(bf);

if(bg.isValidKeyIdentifier(bh)){return bh;
}else{return m;
}},toString:function(){var bk=this.__pf;
var bj=[];

for(var bi in this.__pe){if(this.__pe[bi]){bj.push(qx.locale.Key.getKeyName(j,bi));
}}
if(bk){bj.push(qx.locale.Key.getKeyName(j,bk));
}return bj.join(k);
}},destruct:function(){this.setEnabled(false);
this.__pe=this.__pf=null;
}});
})();
...
(function(){var f="default",e="feedreader.view.Header",d="qooxdoo ",c="1.5.1",b="Feed Reader",a="app-header";
...
var g=new qx.ui.basic.Label(d+c);
g.setFont(f);
this.add(h);
...
(function(){var l="execute",k="settings",j="loading",h="main",g="addfeed",f="selection[0]",d="title",c="north",b="new",a="changeModel",Y="__pr",X="article",W="__px",V="F1",U="selection[0].state",T="partLoaded",S="Control+A",R="loaded",Q="icon/22/places/folder.png",P="__pm",s="feedreader/images/loading22.gif",t="error",q="change",r="user",o="__pv",p="feeds",m="__pt",n="enabled",v="__ps",w="vertical",D="__pu",B="__pn",H="state",F="__pp",L="Control+D",J="icon/22/actions/process-stop.png",y="selection[0].articles",O="__pl",N="__po",M="static",x="icon/22/apps/internet-feed-reader.png",z="model",A="feedreader.Application",C="Control+P",E="Control+R",G="feedreader.model.Feed",I="__pk",K="__pq",u="__pw";
qx.Class.define(A,{extend:qx.application.Standalone,members:{__pk:null,__pl:null,__R:null,__pm:null,__pn:null,__po:null,__pp:null,__pq:null,__pr:null,__ps:null,__pt:null,__pu:null,__pv:null,__pw:null,__px:null,__py:false,__pz:false,main:function(){qx.application.Standalone.prototype.main.call(this);
qx.io.PartLoader.getInstance().addListener(T,function(e){this.debug("part loaded: "+e.getData().getName());
},this);
...
this.__pv=bc.getFeedFolder();
this.__px=bc.getStaticFeedFolder();
this.__pw=bc.getUserFeedFolder();
this._setUpBinding();
this.__pm.getRoot().setOpen(true);
this.__pm.getRoot().getChildren()[0].setOpen(true);
this.__pm.getRoot().getChildren()[1].setOpen(true);
this.__pm.setHideRoot(true);
this.__pk.getSelection().push(this.__px.getFeeds().getItem(0));
this.reload();
},_setUpBinding:function(){var bd={converter:this._state2iconConverter};
this.__pk=new qx.data.controller.Tree(this.__pv,this.__pm,p,d);
this.__pk.setIconOptions(bd);
this.__pk.setIconPath(H);
this.__pl=new qx.data.controller.List(null,this.__pp.getList(),d);
this.__pk.bind(y,this.__pl,z);
this.__pl.bind(f,this.__pt,X);
this.__pl.getSelection().addListener(q,this._listControllerChange,this);
this.__pl.addListener(a,this._treeControllerChange,this);
var be={converter:this._state2loadingConverter};
this.__pk.bind(U,this.__pp,j,be);
be={converter:this._category2enabledConverter};
this.__pk.bind(f,this.__po.getRemoveButton(),n,be);
},_category2enabledConverter:function(bf){return !!bf&&bf.getCategory()==r;
},_state2iconConverter:function(bg){if(bg==b||bg==j){return s;
}else if(bg==R){return x;
}else if(bg==t){return J;
}return Q;
},_state2loadingConverter:function(bh){if(bh==b||bh==j){return true;
}return false;
},_listControllerChange:function(bi){var bj=this.__pk.getSelection().getItem(0);
var bk=this.__pl.getSelection().getItem(0);
if(bk!=undefined){bj.setSelectedArticle(bk);
}},_treeControllerChange:function(bl){var bm=this.__pk.getSelection().getItem(0);
if(bm!=null&&bm.classname==G&&bm.getSelectedArticle()!=null&&bm.getArticles().contains(bm.getSelectedArticle())){qx.event.Timer.once(function(){this.__pl.getSelection().push(bm.getSelectedArticle());
},this,0);
}else{this.__pp.getList().scrollToY(0);
}},_createLayout:function(){var bn=new qx.ui.layout.Dock();
var bo=new qx.ui.container.Composite(bn);
this.getRoot().add(bo,{edge:0});
this.__pn=new feedreader.view.Header();
bo.add(this.__pn,{edge:c});
this.__po=new feedreader.view.ToolBar(this);
bo.add(this.__po,{edge:c});
this.__pq=new qx.ui.splitpane.Pane();
bo.add(this.__pq);
this.__pm=new qx.ui.tree.Tree();
this.__pm.setWidth(250);
this.__pm.setDecorator(h);
this.__pm.setPadding(0);
this.__pq.add(this.__pm,0);
this.__pr=new qx.ui.splitpane.Pane(w);
this.__pr.setDecorator(null);
this.__pq.add(this.__pr,1);
this.__pp=new feedreader.view.List(this.__pv);
this.__pp.setHeight(200);
this.__pp.setDecorator(h);
this.__pr.add(this.__pp,0);
this.__pt=new feedreader.view.Article();
this.__pt.setDecorator(h);
this.__pr.add(this.__pt,1);
},_initializeCommands:function(){var bp={};
bp.reload=new qx.ui.core.Command(E);
bp.reload.addListener(l,this.reload,this);
bp.about=new qx.ui.core.Command(V);
...
bp.addFeed=new qx.ui.core.Command(S);
...
this.__pw.getFeeds().push(bu);
var bv=feedreader.io.FeedLoader.getInstance();
bv.load(bu);
},removeFeed:function(){var bw=this.__pk.getSelection().getItem(0);
if(bw&&bw.getCategory&&bw.getCategory()!==M){var bx=this.__pw.getFeeds();
for(var i=0;i<bx.length;i++){if(bw===bx.getItem(i)){bx.splice(i,1);
return;
}}}},reload:function(){var by=feedreader.io.FeedLoader.getInstance();
by.loadAll(this.__pv);
this.__pt.setArticle(null);
},showPreferences:function(){if(!this.__pz){this.__po.signalLoading(k,true);
}qx.io.PartLoader.require([k],function(){this.__pz=true;
if(!this.__ps){this.__ps=new feedreader.view.PreferenceWindow();
this.getRoot().add(this.__ps);
}this.__ps.center();
this.__ps.open();
this.__po.signalLoading(k,false);
},this);
},showAbout:function(){alert(this.tr("FeedReader (qooxdoo powered)"));
},showAddFeed:function(){if(!this.__py){this.__po.signalLoading(g,true);
}qx.io.PartLoader.require([g],function(){this.__py=true;
if(!this.__pu){this.__pu=new feedreader.view.AddFeedWindow(this);
this.getRoot().add(this.__pu);
}this.__pu.center();
this.__pu.open();
this.__po.signalLoading(g,false);
},this);
}},destruct:function(){this.__R=null;
this._disposeObjects(N,F,m,P,o,K,Y,B,W,u,I,O,v,D);
}});
})();
...
(function(){var k="enabled",j="originalIcon",i="__pC",h=")",g="Remove feed",f="Preferences",d="excluded",c="addFeed",b="removeFeed",a="icon/22/actions/help-about.png",H="Reload the feeds. (%1)",G="Help",F="visible",E="feedreader.view.ToolBar",D="icon/22/actions/media-seek-forward.png",C="toolbar-button",B="Add feed",A="addfeed",z="icon/22/actions/view-refresh.png",y="(",r="reload",s="showItem",p="settings",q="about",n="__pE",o="icon/22/actions/dialog-cancel.png",l="Reload",m="preferences",t="icon/22/actions/dialog-ok.png",u="icon/22/apps/preferences-theme.png",w="hideItem",v="Open preferences window.",x="feedreader/images/loading22.gif";
qx.Class.define(E,{extend:qx.ui.toolbar.ToolBar,construct:function(I){qx.ui.toolbar.ToolBar.call(this);
this.__pA={};
this.__pB=new qx.ui.toolbar.Button(this.tr(B),t);
this.__pB.setCommand(I.getCommand(c));
this.add(this.__pB);
this.__pC=new qx.ui.toolbar.Button(this.tr(g),o);
this.__pC.setCommand(I.getCommand(b));
...
this.add(L);
this.addSeparator();
this.__pD=new qx.ui.toolbar.Button(this.tr(f),u);
this.__pD.setCommand(I.getCommand(m));
this.__pD.setToolTipText(this.tr(v));
this.add(this.__pD);
this.addSpacer();
var M=new qx.ui.toolbar.Button(this.tr(G),a);
var K=I.getCommand(q);
M.setCommand(K);
M.setToolTipText(y+K.toString()+h);
this.add(M);
...
this.__pE=new qx.ui.menu.Menu();
N.setMenu(this.__pE);
this.addListener(w,this._onHideItem,this);
this.addListener(s,this._onShowItem,this);
},members:{__pC:null,__pE:null,__pA:null,__pB:null,__pD:null,getRemoveButton:function(){return this.__pC;
...
Q.setVisibility(d);
},_getMenuItem:function(S){var T=this.__pA[S.toHashCode()];
...
this.__pA[S.toHashCode()]=T;
}return T;
},signalLoading:function(U,V){if(U==A){var W=this.__pB;
}else if(U==p){var W=this.__pD;
}else{return ;
...
(function(){var m="horizontal",k="qx.event.type.Data",j="vertical",h="",g="qx.ui.form.List",f="text",d="Enter",c="one",b="addChildWidget",a="_applySpacing",A="Boolean",z="Integer",y="action",x="keyinput",w="addItem",v="removeChildWidget",u="_applyOrientation",t="single",s="keypress",r="list",p="label",q="pane",n="__oB",o="removeItem";
...
var content=this.__oB;
content.setLayout(F);
content.setAllowGrowX(!E);
content.setAllowGrowY(E);
this._applySpacing(this.getSpacing());
},_applySpacing:function(G,H){this.__oB.getLayout().setSpacing(G);
...
if(!(J===t||J===c)){return;
}if(((new Date).valueOf()-this.__pG)>1000){this.__pF=h;
}this.__pF+=e.getChar();
var K=this.findItemByLabelFuzzy(this.__pF);
if(K){this.setSelection([K]);
}this.__pG=(new Date).valueOf();
...
(function(){var p="Boolean",o="focusout",n="interval",m="mouseover",l="mouseout",k="mousemove",j="__pH",i="widget",h="__pI",g="qx.ui.tooltip.ToolTip",c="__pK",f="_applyCurrent",d="qx.ui.tooltip.Manager",b="tooltip-error",a="singleton";
qx.Class.define(d,{type:a,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
qx.event.Registration.addListener(document.body,m,this.__pR,this,true);
this.__pH=new qx.event.Timer();
this.__pH.addListener(n,this.__pO,this);
this.__pI=new qx.event.Timer();
this.__pI.addListener(n,this.__pP,this);
this.__pJ={left:0,top:0};
},properties:{current:{check:g,nullable:true,apply:f},showInvalidToolTips:{check:p,init:true},showToolTips:{check:p,init:true}},members:{__pJ:null,__pI:null,__pH:null,__pK:null,__pL:null,__pM:function(){if(!this.__pK){this.__pK=new qx.ui.tooltip.ToolTip().set({rich:true});
}return this.__pK;
},__pN:function(){if(!this.__pL){this.__pL=new qx.ui.tooltip.ToolTip().set({appearance:b});
this.__pL.syncAppearance();
}return this.__pL;
...
var s=document.body;
if(q){this.__pH.startWith(q.getShowTimeout());
t.addListener(s,l,this.__pS,this,true);
t.addListener(s,o,this.__pT,this,true);
t.addListener(s,k,this.__pQ,this,true);
}else{t.removeListener(s,l,this.__pS,this,true);
t.removeListener(s,o,this.__pT,this,true);
t.removeListener(s,k,this.__pQ,this,true);
}},__pO:function(e){var u=this.getCurrent();

if(u&&!u.isDisposed()){this.__pI.startWith(u.getHideTimeout());

if(u.getPlaceMethod()==i){u.placeToWidget(u.getOpener());
}else{u.placeToPoint(this.__pJ);
}u.show();
}this.__pH.stop();
},__pP:function(e){var v=this.getCurrent();

if(v&&!v.isDisposed()){v.exclude();
}this.__pI.stop();
this.resetCurrent();
},__pQ:function(e){var w=this.__pJ;
w.left=e.getDocumentLeft();
w.top=e.getDocumentTop();
},__pR:function(e){var z=qx.ui.core.Widget.getWidgetByElement(e.getTarget());
...
if(x){A=this.__pN().set({label:x});
}
if(!A){A=this.__pM().set({label:B,icon:y});
}this.setCurrent(A);
A.setOpener(z);
},__pS:function(e){var C=qx.ui.core.Widget.getWidgetByElement(e.getTarget());
...
}},__pT:function(e){var F=qx.ui.core.Widget.getWidgetByElement(e.getTarget());

if(!F){return;
}var G=this.getCurrent();
if(G&&G==F.getToolTip()){this.setCurrent(null);
}}},destruct:function(){qx.event.Registration.removeListener(document.body,m,this.__pR,this,true);
this._disposeObjects(j,h,c);
this.__pJ=null;
}});
})();
...
(function(){var l="zIndex",k="px",j="keydown",h="deactivate",g="resize",f="keyup",d="keypress",c="backgroundColor",b="_applyOpacity",a="Boolean",x="opacity",w="interval",v="__mw",u="Tab",t="Color",s="__kl",r="qx.ui.root.Page",q="__qf",p="Number",o="qx.ui.core.Blocker",m="qx.ui.root.Application",n="_applyColor";
...
if(this._isPageRoot){y.addListener(g,this.__qh,this);
}
if(qx.Class.isDefined(m)&&y instanceof qx.ui.root.Application){this.setKeepBlockerActive(true);
}this.__qb=[];
this.__qc=[];
this.__qd=[];
},properties:{color:{check:t,init:null,nullable:true,apply:n,themeable:true},opacity:{check:p,init:1,apply:b,themeable:true},keepBlockerActive:{check:a,init:false}},members:{__mw:null,__qe:0,__qf:null,__qd:null,__qb:null,__qc:null,__qg:null,__kl:null,_isPageRoot:false,_widget:null,__qh:function(e){var z=e.getData();
...
this.__qi(c,C);
},_applyOpacity:function(D,E){this.__qi(x,D);
},__qi:function(F,G){var H=[];
this.__mw&&H.push(this.__mw);
this.__qf&&H.push(this.__qf);
...
this.__qb.push(I.getActive());
this.__qc.push(I.getFocus());

if(this._widget.isFocusable()){this._widget.focus();
}},_restoreActiveWidget:function(){var L=this.__qb.length;

if(L>0){var K=this.__qb[L-1];

if(K){qx.bom.Element.activate(K);
}this.__qb.pop();
}var J=this.__qc.length;

if(J>0){var K=this.__qc[J-1];

if(K){qx.bom.Element.focus(this.__qc[J-1]);
}this.__qc.pop();
}},__qj:function(){return new qx.html.Blocker(this.getColor(),this.getOpacity());
},getBlockerElement:function(){if(!this.__mw){this.__mw=this.__qj();
this.__mw.setStyle(l,15);
this._widget.getContainerElement().add(this.__mw);
this.__mw.exclude();
}return this.__mw;
},block:function(){this.__qe++;

if(this.__qe<2){this._backupActiveWidget();
var M=this.getBlockerElement();
M.include();
M.activate();
M.addListener(h,this.__qo,this);
M.addListener(d,this.__qn,this);
M.addListener(j,this.__qn,this);
M.addListener(f,this.__qn,this);
}},isBlocked:function(){return this.__qe>0;
},unblock:function(){if(!this.isBlocked()){return;
}this.__qe--;

if(this.__qe<1){this.__qk();
this.__qe=0;
}},forceUnblock:function(){if(!this.isBlocked()){return;
}this.__qe=0;
this.__qk();
},__qk:function(){this._restoreActiveWidget();
var N=this.getBlockerElement();
N.removeListener(h,this.__qo,this);
N.removeListener(d,this.__qn,this);
N.removeListener(j,this.__qn,this);
N.removeListener(f,this.__qn,this);
N.exclude();
},getContentBlockerElement:function(){if(!this.__qf){this.__qf=this.__qj();
this._widget.getContentElement().add(this.__qf);
this.__qf.exclude();
}return this.__qf;
...
if(this.__qd.length<2){P.include();

if(this._isPageRoot){if(!this.__kl){this.__kl=new qx.event.Timer(300);
this.__kl.addListener(w,this.__qm,this);
}this.__kl.start();
this.__qm();
}}},isContentBlocked:function(){return this.__qd.length>0;
},unblockContent:function(){if(!this.isContentBlocked()){return;
}this.__qd.pop();
var Q=this.__qd[this.__qd.length-1];
var R=this.getContentBlockerElement();
R.setStyle(l,Q);

if(this.__qd.length<1){this.__ql();
this.__qd=[];
...
S.setStyle(l,null);
this.__ql();
},__ql:function(){this.getContentBlockerElement().exclude();

if(this._isPageRoot){this.__kl.stop();
}},__qm:function(){var T=this._widget.getContainerElement().getDomElement();
...
},__qn:function(e){if(e.getKeyIdentifier()==u){e.stop();
}},__qo:function(){if(this.getKeepBlockerActive()){this.getBlockerElement().activate();
}}},destruct:function(){if(this._isPageRoot){this._widget.removeListener(g,this.__qh,this);
}this._disposeObjects(q,v,s);
this.__qg=this.__qb=this.__qc=this._widget=this.__qd=null;
}});
})();
(function(){var k="horizontal",j="px",i="scroll",h="vertical",g="-1px",f="0",d="engine.name",c="hidden",b="__qp",a="mousedown",A="qx.ui.core.scroll.NativeScrollBar",z="PositiveNumber",y="engine.version",x="Integer",w="mousemove",v="_applyMaximum",u="_applyOrientation",t="appear",s="opera",r="PositiveInteger",p="mshtml",q="mouseup",n="Number",o="_applyPosition",l="scrollbar",m="native";
...
if((qx.core.Environment.get(d)==s)&&parseFloat(qx.core.Environment.get(y))<11.5){this.addListener(t,this._onAppear,this);
}this.getContentElement().add(this._getScrollPaneElement());
if(B!=null){this.setOrientation(B);
}else{this.initOrientation();
}},properties:{appearance:{refine:true,init:l},orientation:{check:[k,h],init:k,apply:u},maximum:{check:r,apply:v,init:100},position:{check:n,init:0,apply:o,event:i},singleStep:{check:x,init:20},knobFactor:{check:z,nullable:true}},members:{__mt:null,__qp:null,_getScrollPaneElement:function(){if(!this.__qp){this.__qp=new qx.html.Element();
}return this.__qp;
...
return {width:this.__mt?100:G,maxWidth:this.__mt?null:G,minWidth:this.__mt?null:G,height:this.__mt?G:100,maxHeight:this.__mt?G:null,minHeight:this.__mt?G:null};
...
if(this.__mt){content.scrollToX(K);
}else{content.scrollToY(K);
}},_applyOrientation:function(L,M){var N=this.__mt=L===k;
...
qx.ui.core.queue.Layout.add(this);
},_updateScrollBar:function(){var P=this.__mt;
...
var W=this.__mt?X.getScrollX():X.getScrollY();
this.setPosition(W);
...
},__qs:function(){if(this.isVisible()){if(this.getBlockBackground()){var R=this.getZIndex();
...
if(!bj){this.addListenerOnce(j,this.__qt,this);
return;
...
if(this.__qq){bo.cancelOpen(this.__qq);
this.__qq=null;
...
}if(this.__qq){br.cancelOpen(this.__qq);
...
qx.Bootstrap.define(p,{construct:function(){this.__qu=qx.Bootstrap.bind(this.__qD,this);
this.__mF=qx.Bootstrap.bind(this.__qF,this);
this.__qC();
if(window.attachEvent){this.__qv=qx.Bootstrap.bind(this.__qI,this);
window.attachEvent(c,this.__qv);
}},statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},members:{readyState:0,responseText:i,responseXML:null,status:0,statusText:i,timeout:0,open:function(x,y,z,A,B){if(this.__mg){return;
}this.__mI=false;
this.__qw=false;
this.__qx=false;
...
if(!this.__qJ()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();
this.__qC();
}this.__qA.onreadystatechange=this.__qu;

try{if(qx.core.Environment.get(j)){qx.Bootstrap.debug(qx.bom.request.Xhr,"Open native request with method: "+x+", url: "+y+", async: "+z);
}this.__qA.open(x,y,z,A,B);
}catch(C){if(!qx.util.Request.isCrossDomain(y)){throw C;
}
if(!this.__qy){this.__qz=C;
}
if(this.__qy){if(window.XDomainRequest){this.readyState=4;
this.__qA=new XDomainRequest();
this.__qA.onerror=qx.Bootstrap.bind(function(){this.onreadystatechange();
this.onerror();
this.onloadend();
},this);
...
}}if(qx.core.Environment.get(k)===f&&parseInt(qx.core.Environment.get(d),10)<2&&!this.__qy){this.readyState=qx.bom.request.Xhr.OPENED;
this.onreadystatechange();
}},setRequestHeader:function(D,E){if(this.__mg){return;
}if(D==s||D==t||D==v||D==n){this.__qx=true;
}this.__qA.setRequestHeader(D,E);
},send:function(F){if(this.__mg){return;
}if(!this.__qy&&this.__qz){throw this.__qz;
}if(qx.core.Environment.get(k)===g&&this.timeout===0){this.timeout=10000;
}if(this.timeout>0){this.__qB=window.setTimeout(this.__mF,this.timeout);
}F=typeof F==a?null:F;
try{if(qx.core.Environment.get(j)){qx.Bootstrap.debug(qx.bom.request.Xhr,"Send native request");
}this.__qA.send(F);
}catch(H){if(!this.__qy){throw H;
...
}}if(qx.core.Environment.get(k)===f&&!this.__qy){this.__qD();
}this.__qw=true;
},abort:function(I){if(this.__mg){return;
}this.__mI=true;
this.__qA.abort();

if(this.__qA){this.readyState=this.__qA.readyState;
}
if(I){return;
}},onreadystatechange:function(){},onload:function(){},onloadend:function(){},onerror:function(){},onabort:function(){},ontimeout:function(){},getResponseHeader:function(J){if(this.__mg){return;
}return this.__qA.getResponseHeader(J);
},getAllResponseHeaders:function(){if(this.__mg){return;
}return this.__qA.getAllResponseHeaders();
},getRequest:function(){return this.__qA;
},dispose:function(){if(this.__mg){return false;
}if(window.detachEvent){window.detachEvent(c,this.__qv);
}try{this.__qA.onreadystatechange;
}catch(L){return;
}var K=function(){};
this.__qA.onreadystatechange=K;
this.__qA.onload=K;
this.__qA.onerror=K;
this.abort(true);
this.__qA=null;
...
},__qA:null,__qy:null,__qu:null,__qv:null,__mF:null,__qw:null,__mI:null,__ca:null,__mg:null,__qB:null,__qz:null,__qx:null,__qC:function(){this.__qA=this._createNativeXhr();
this.__qA.onreadystatechange=this.__qu;
this.__mg=this.__qw=this.__mI=false;
},__qD:function(){var N=this.__qA,O=true;
...
if(this.readyState===qx.bom.request.Xhr.DONE&&this.__mI&&!this.__qw){return;
}if(!this.__qy&&(N.readyState==2||N.readyState==3)){return;
}this.status=0;
...
}}},__mM:function(){var Q=this;
if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__qB);
}if(qx.core.Environment.get(k)==h&&qx.core.Environment.get(d)<8){if(this.__qy&&!this.__qw&&this.readyState>=qx.bom.request.Xhr.LOADING){if(this.readyState==qx.bom.request.Xhr.LOADING){return ;
}
...
this.__qE();
},__qE:function(){if(this.readyState===qx.bom.request.Xhr.DONE){if(this.__ca){this.ontimeout();
if(qx.core.Environment.get(k)===g){this.onerror();
}this.__ca=false;
}else{if(this.__mI){this.onabort();
}else{this.statusText?this.onload():this.onerror();
}}this.onloadend();
}},__qF:function(){var R=this.__qA;
...
this.responseXML=null;
this.__mM();
},__qG:function(){var S=this.readyState===qx.bom.request.Xhr.DONE;
if(this._getProtocol()===b&&this.status===0){this.status=200;
}if(this.status===1223){this.status=204;
}if(qx.core.Environment.get(k)===g){if(S&&this.__qx&&!this.__mI&&this.status===0){this.status=304;
}}},__qH:function(){if(qx.core.Environment.get(k)==h&&(this.getResponseHeader(l)||i).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var T=new window.ActiveXObject(o);
T.async=false;
T.validateOnParse=false;
T.loadXML(this.responseText);
this.responseXML=T;
}},__qI:function(){try{if(this){this.dispose();
}}catch(e){}},__qJ:function(){var name=qx.core.Environment.get(k);
var U=qx.core.Environment.get(q);
return !(name==h&&U<9||name==f&&U<3.5);
}},defer:function(){qx.core.Environment.add(j,false);
}});
})();
(function(){var i="Integer",h="hovered",g="hover-button",f="interval",d="mouseover",c="mouseout",b="qx.ui.form.HoverButton",a="__kl";
...
this.__kl=new qx.event.AcceleratingTimer();
this.__kl.addListener(f,this._onInterval,this);
},properties:{appearance:{refine:true,init:g},interval:{check:i,init:80},firstInterval:{check:i,init:200},minTimer:{check:i,init:20},timerDecrease:{check:i,init:2}},members:{__kl:null,_onMouseOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;
}this.__kl.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();
this.addState(h);
},_onMouseOut:function(e){this.__kl.stop();
this.removeState(h);
...
}},__qV:function(t){while(!(t instanceof qx.ui.menu.Menu)){t=t.getLayoutParent();
}return t;
...
}},properties:{model:{check:p,apply:d,event:r,nullable:true,dereference:true},target:{apply:k,event:c,init:null,nullable:true,dereference:true},childPath:{check:t,apply:b,nullable:true},labelPath:{check:t,apply:o,nullable:true},iconPath:{check:t,apply:h,nullable:true},labelOptions:{apply:g,nullable:true},iconOptions:{apply:a,nullable:true},delegate:{apply:q,init:null,nullable:true}},members:{__rj:null,__jW:null,__ny:null,__rk:null,_applyDelegate:function(y,z){this._setConfigureItem(y,z);
this._setCreateItem(y,z);
this._setBindItem(y,z);
},_applyIconOptions:function(A,B){this.__nJ();
},_applyLabelOptions:function(C,D){this.__nJ();
},_applyTarget:function(E,F){if(F!=undefined){this.__rp(F);
}if(this.getModel()!=null){this.__rn();
}this._addChangeTargetListener(E,F);
},_applyModel:function(G,H){this.__rn();
},_applyChildPath:function(I,J){this.__rk=J;
this.__rn();
this.__rk=null;
},_applyIconPath:function(K,L){this.__nJ();
},_applyLabelPath:function(M,N){this.__rn();
},__rl:function(O){var Q=O.getTarget();
var P=this.__rj[Q.toHashCode()].treeNode;
var R=this.__rj[Q.toHashCode()].modelNode;
this.__ro(P,R);
this._updateSelection();
},__rm:function(e){var Y=e.getData();
var U=e.getOldData();
var V=this.__rj[U.toHashCode()];
U.removeListenerById(V.changeListenerId);
delete this.__rj[U.toHashCode()];
...
var S=X.event;
var W=ba.addListener(S,this.__rm,this);
var T=V.treeNode;
this.__rj[Y.toHashCode()]={modelNode:ba,treeNode:T,changeListenerId:V.changeListenerId,changeChildernListenerId:W};
this.__ro(T,ba);
this._updateSelection();
...
},__rn:function(){if(this.getTarget()==null||this.getChildPath()==null){return;
...
this.__rs(this.getModel(),bd);
this.__ro(bd,this.getModel());
}},__ro:function(be,bf){if(bf[s+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;
}var bo=bf[s+qx.lang.String.firstUp(this.getChildPath())]();
if(this.__rj[bo.toHashCode()]==undefined){var bj=bo.addListener(m,this.__rl,this);
var bn=qx.Class.getPropertyDefinition(bf.constructor,this.getChildPath());
var bk=bn.event;
var bl=bf.addListener(bk,this.__rm,this);
this.__rj[bo.toHashCode()]={modelNode:bf,treeNode:be,changeListenerId:bj,changeChildernListenerId:bl};
...
be.addAt(bm,i);
this.__rs(bo.getItem(i),bm);
this.__ro(bm,bo.getItem(i));
}}}for(var i=be.getChildren().length-1;i>=bo.length;i--){var bi=be.getChildren()[i];
this.__rr(bi,be);
}},__rp:function(bq){if(bq==null){bq=this.getTarget();
}if(bq==null){return;
}var bs=bq.getRoot();
...
this.__rj={};
}},__rq:function(bt){var bu=bt.getChildren()||[];
for(var i=bu.length-1;i>=0;i--){if(bu[i].getChildren().length>0){this.__rq(bu[i]);
}this.__rr(bu[i],bt);
}},__rr:function(bv,bw){var by=bv.getModel();
var bA=this.__rk||this.getChildPath();
var bB=s+qx.lang.String.firstUp(bA);
if(by[bB]!=undefined){var bz=by[bB]();
var bx=this.__rj[bz.toHashCode()];
bz.removeListenerById(bx.changeListenerId);
by.removeListenerById(bx.changeChildernListenerId);
this.__rq(bv);
delete this.__rj[bz.toHashCode()];
}this.__rt(by);
bw.remove(bv);
bv.destroy();
},bindProperty:function(bC,bD,bE,bF,bG){var bI=bG.bind(bC,bF,bD,bE);
if(this.__jW[bD]==null){this.__jW[bD]={};
}var bH=this.__jW[bD];
...
}if(!qx.lang.Array.contains(this.__ny,bD)){this.__ny.push(bD);
}},bindPropertyReverse:function(bJ,bK,bL,bM,bN){var bP=bM.bind(bK,bN,bJ,bL);
if(this.__jW[bK]==null){this.__jW[bK]={};
}var bO=this.__jW[bK];
...
}if(!qx.lang.Array.contains(this.__ny,bK)){this.__ny.push(bK);
...
}},__nJ:function(){var bT;

for(var bV in this.__jW){bT=bV;
break;
}for(var bS in this.__jW[bT]){var bW=this.__jW[bT][bS].treeNode;
var bU=qx.core.ObjectRegistry.fromHashCode(bS);
this.__rt(bU);
this.__rs(bU,bW);
}},__rs:function(bX,bY){var ca=this.getDelegate();
if(ca!=null&&ca.bindItem!=null){ca.bindItem(this,bY,bX);
}else{this.bindDefaultProperties(bY,bX);
}},__rt:function(cb){for(var i=0;i<this.__ny.length;i++){var cd=this.__ny[i];
var cc=this.__jW[cd][cb.toHashCode()];
...
cc.reverseId=null;
}delete this.__jW[cd][cb.toHashCode()];
...
this.setModel(null);
this.__jW=this.__rj=this.__ny=null;
}});
})();
...
this.__ry=++qx.io.remote.Request.__ry;
},events:{"created":j,"configured":j,"sending":j,"receiving":j,"completed":P,"aborted":j,"failed":P,"timeout":P},statics:{__ry:0,methodAllowsRequestBody:function(T){return (T==O)||(T==I);
...
},init:true,apply:B},crossDomain:{check:k,init:false},fileUpload:{check:k,init:false},transport:{check:l,nullable:true},useBasicHttpAuth:{check:k,init:false},parseJson:{check:k,init:true}},members:{__ru:null,__rv:null,__rw:null,__rx:null,__ry:null,send:function(){qx.io.remote.RequestQueue.getInstance().add(this);
...
},__rz:qx.event.GlobalError.observeMethod(function(e){var U=e.clone();
U.setTarget(this);
...
},setRequestHeader:function(bf,bg){this.__ru[bf]=bg;
},removeRequestHeader:function(bh){delete this.__ru[bh];
},getRequestHeader:function(bi){return this.__ru[bi]||null;
},getRequestHeaders:function(){return this.__ru;
},setParameter:function(bj,bk,bl){if(bl){this.__rw[bj]=bk;
}else{this.__rv[bj]=bk;
}},removeParameter:function(bm,bn){if(bn){delete this.__rw[bm];
}else{delete this.__rv[bm];
}},getParameter:function(bo,bp){if(bp){return this.__rw[bo]||null;
}else{return this.__rv[bo]||null;
}},getParameters:function(bq){return (bq?this.__rw:this.__rv);
},setFormField:function(br,bs){this.__rx[br]=bs;
},removeFormField:function(bt){delete this.__rx[bt];
},getFormField:function(bu){return this.__rx[bu]||null;
},getFormFields:function(){return this.__rx;
},getSequenceNumber:function(){return this.__ry;
}},destruct:function(){this.setTransport(null);
this.__ru=this.__rv=this.__rw=this.__rx=null;
}});
})();
