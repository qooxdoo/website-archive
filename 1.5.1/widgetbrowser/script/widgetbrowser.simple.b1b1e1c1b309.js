...
(function(){var b="qx.ui.window.Manager",a="__nE";
qx.Class.define(b,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__nE:null,setDesktop:function(c){this.__nE=c;
this.updateStack();
},getDesktop:function(){return this.__nE;
},changeActiveWindow:function(d,e){if(d){this.bringToFront(d);
d.setActive(true);
}
if(e){e.resetActive();
}},_minZIndex:1e5,updateStack:function(){qx.ui.core.queue.Widget.add(this);
},syncWidget:function(){this.__nE.forceUnblockContent();
var f=this.__nE.getWindows();
...
}}this.__nE.setActiveWindow(k);
},bringToFront:function(n){var o=this.__nE.getWindows();
var p=qx.lang.Array.remove(o,n);

if(p){o.push(n);
this.updateStack();
}},sendToBack:function(q){var r=this.__nE.getWindows();
...
(function(){var l="move",k="Boolean",j="mouseup",i="mousedown",h="losecapture",g="__nG",f="qx.ui.core.MMovable",d="__nF",c="mousemove",b="maximized",a="move-frame";
qx.Mixin.define(f,{properties:{movable:{check:k,init:true},useMoveFrame:{check:k,init:false}},members:{__nF:null,__nG:null,__nH:null,__nI:null,__nJ:null,__nK:null,__nL:null,__nM:false,__nN:null,__nO:0,_activateMoveHandle:function(m){if(this.__nF){throw new Error("The move handle could not be redefined!");
}this.__nF=m;
...
m.addListener(h,this.__nS,this);
},__nP:function(){var n=this.__nG;

if(!n){n=this.__nG=new qx.ui.core.Widget();
n.setAppearance(a);
n.exclude();
qx.core.Init.getApplication().getRoot().add(n);
}return n;
},__nQ:function(){var location=this.getContainerLocation();
var p=this.getBounds();
var o=this.__nP();
o.setUserBounds(location.left,location.top,p.width,p.height);
o.show();
o.setZIndex(this.getZIndex()+1);
},__nR:function(e){var r=this.__nH;
var u=Math.max(r.left,Math.min(r.right,e.getDocumentLeft()));
var t=Math.max(r.top,Math.min(r.bottom,e.getDocumentTop()));
var q=this.__nI+u;
var s=this.__nJ+t;
return {viewportLeft:q,viewportTop:s,parentLeft:q-this.__nK,parentTop:s-this.__nL};
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isContentBlocked()){this.__nN=parent.getBlockerColor();
this.__nO=parent.getBlockerOpacity();
parent.setBlockerColor(null);
parent.setBlockerOpacity(1);
parent.blockContent(this.getZIndex()-1);
this.__nM=true;
}}this.__nH={left:w.left,top:w.top,right:w.left+x.width,bottom:w.top+x.height};
var v=this.getContainerLocation();
this.__nK=w.left;
this.__nL=w.top;
this.__nI=v.left-e.getDocumentLeft();
this.__nJ=v.top-e.getDocumentTop();
this.addState(l);
this.__nF.capture();
if(this.getUseMoveFrame()){this.__nQ();
}e.stop();
},_onMoveMouseMove:function(e){if(!this.hasState(l)){return;
}var y=this.__nR(e);

if(this.getUseMoveFrame()){this.__nP().setDomPosition(y.viewportLeft,y.viewportTop);
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__nM){parent.unblockContent();
parent.setBlockerColor(this.__nN);
parent.setBlockerOpacity(this.__nO);
this.__nN=null;
this.__nO=0;
this.__nM=false;
}}this.__nF.releaseCapture();
var z=this.__nR(e);
this.setLayoutProperties({left:z.parentLeft,top:z.parentTop});
if(this.getUseMoveFrame()){this.__nP().exclude();
}e.stopPropagation();
},__nS:function(e){if(!this.hasState(l)){return;
}this.removeState(l);
if(this.getUseMoveFrame()){this.__nP().exclude();
...
this.__nU=bh.left===undefined?0:bh.left;
this.__nT=bh.top===undefined?0:bh.top;
...
this.__nU=bi.left===undefined?0:bi.left;
this.__nT=bi.top===undefined?0:bi.top;
this.removeState(b);
this.hide();
this.fireEvent(u);
