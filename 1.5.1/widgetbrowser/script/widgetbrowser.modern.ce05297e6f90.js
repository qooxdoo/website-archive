...
(function(){var u="keypress",t="interval",s="keydown",r="mousedown",q="keyup",p="blur",o="Enter",n="__nV",m="__nW",l="__mk",d="Up",k="Escape",h="event.touch",c="qx.ui.menu.Manager",b="Left",g="Down",f="Right",j="singleton",a="Space";
...
this.__nV.addListener(t,this._onOpenInterval,this);
this.__nW=new qx.event.Timer;
this.__nW.addListener(t,this._onCloseInterval,this);
},members:{__nX:null,__nY:null,__nV:null,__nW:null,__mk:null,_getChild:function(x,y,z,A){var B=x.getChildren();
var length=B.length;
var C;
...
},add:function(F){var G=this.__mk;
G.push(F);
F.setZIndex(1e6+G.length);
},remove:function(H){var I=this.__mk;

if(I){qx.lang.Array.remove(I,H);
}},hideAll:function(){var J=this.__mk;

if(J){for(var i=J.length-1;i>=0;i--){J[i].exclude();
}}},getActiveMenu:function(){var K=this.__mk;
return K.length>0?K[K.length-1]:null;
},scheduleOpen:function(L){this.cancelClose(L);
if(L.isVisible()){if(this.__nX){this.cancelOpen(this.__nX);
}}else if(this.__nX!=L){this.__nX=L;
this.__nV.restartWith(L.getOpenInterval());
}},scheduleClose:function(M){this.cancelOpen(M);
if(!M.isVisible()){if(this.__nY){this.cancelClose(this.__nY);
}}else if(this.__nY!=M){this.__nY=M;
this.__nW.restartWith(M.getCloseInterval());
}},cancelOpen:function(N){if(this.__nX==N){this.__nV.stop();
this.__nX=null;
}},cancelClose:function(O){if(this.__nY==O){this.__nW.stop();
this.__nY=null;
...
}if(this.__mk.length>0&&!this._isInMenu(P)){this.hideAll();
}},__oa:{"Enter":1,"Space":1},__eu:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var Q=this.getActiveMenu();

if(!Q){return;
}var R=e.getKeyIdentifier();

if(this.__eu[R]||(this.__oa[R]&&Q.getSelectedButton())){e.stopPropagation();
...
qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:c,init:0,apply:b},spanColumn:{check:c,init:1,nullable:true,apply:b},iconColumnWidth:{check:c,init:0,themeable:true,apply:b},arrowColumnWidth:{check:c,init:0,themeable:true,apply:b}},members:{__ob:null,_computeSizeHint:function(){var q=this._getLayoutChildren();
var o,g,j;
var e=this.getSpanColumn();
var h=this.__ob=[0,0,0,0];
...
},getColumnSizes:function(){return this.__ob||null;
}},destruct:function(){this.__ob=null;
}});
})();
(function(){var i="Integer",h="hovered",g="hover-button",f="interval",d="mouseover",c="mouseout",b="__ir",a="qx.ui.form.HoverButton";
...
this.__ir=new qx.event.AcceleratingTimer();
this.__ir.addListener(f,this._onInterval,this);
},properties:{appearance:{refine:true,init:g},interval:{check:i,init:80},firstInterval:{check:i,init:200},minTimer:{check:i,init:20},timerDecrease:{check:i,init:2}},members:{__ir:null,_onMouseOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;
}this.__ir.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();
this.addState(h);
},_onMouseOut:function(e){this.__ir.stop();
this.removeState(h);
...
s.addListener(b,this.__oc,this);
this._add(s);
break;
...
},__oc:function(){var w=this.getChildrenContainer().getChildren();
...
},__of:function(){if(this.isVisible()){if(this.getBlockBackground()){var R=this.getZIndex();
...
},__og:function(){var bj=this._getMenuBounds();

if(!bj){this.addListenerOnce(j,this.__og,this);
return;
...
if(this.__oe){this.__oe.call(this);
delete this.__oe;
...
if(this.__od){bo.cancelOpen(this.__od);
this.__od=null;
...
}if(this.__od){br.cancelOpen(this.__od);
...
}else if(this.__oi.length>0){do{var M=this.__oi[0];
...
var K=false;
if(this.__oi.length==1&&I>0){var D=L-this.getSpacing();
var H=B-I+F+D;
K=A>H;
}if(A>B+F+L||K){this.__ok(M);
B+=F;
if(C&&this.__oi.length==0){C.setVisibility(v);
}}else{return;
}}}while(A>=B&&this.__oi.length>0);
}},__ok:function(N){N.setVisibility(w);
this.__oi.shift();
this.fireDataEvent(p,N);
},__ol:function(O){if(!O){return;
}this.__oi.unshift(O);
O.setVisibility(v);
this.fireDataEvent(j,O);
},_getNextToHide:function(){for(var i=this.__oj.length-1;i>=0;i--){var P=this.__oj[i];
...
}}},setRemovePriority:function(S,T,U){if(!U&&this.__oj[T]!=undefined){throw new Error("Priority already in use!");
}this.__oj[T]=S;
...
}for(var i=0;i<this.__oi.length;i++){this.__oi[i].setVisibility(w);
}this.__oi=[];
...
}},__om:false,_setAllowMenuOpenHover:function(bc){this.__om=bc;
