...
this.__qT=null;
},properties:{filter:{check:h,nullable:true,apply:d}},members:{__uw:null,__qT:null,__ux:null,__uy:null,_rowArr:null,_nodeArr:null,_nodeRowMap:null,_treeColumn:null,_selections:null,setTree:function(o){this.__uw=o;
},getTree:function(){return this.__uw;
},setEditable:function(p){this.__qT=[];

for(var q=0;q<this.getColumnCount();q++){this.__qT[q]=p;
}this.fireEvent(k);
},setColumnEditable:function(r,s){if(s!=this.isColumnEditable(r)){if(this.__qT==null){this.__qT=[];
}this.__qT[r]=s;
this.fireEvent(k);
}},isColumnEditable:function(t){if(t==this._treeColumn){return false;
}return (this.__qT?this.__qT[t]==true:false);
...
},__uz:function(bd,be){var bh=this.getFilter();
var bk=null;
var bj;
...
bk=this._nodeArr[bj];
if(bk==null||(bh&&!bh.call(this,bk))){this.__uy=true;
continue;
}bl.push(bk);
bk.level=be;
bk.bFirstChild=(bf==0);
if(!this.__uy){this.__uB(bk,i==bg-1);
...
}if(bk.bOpened){this.__uz(bj,be+1);
}bf++;
}},__uA:function(bm){var bn=this.__ux;
var br=bn[bm];
var bp=br.length;
...
if(bo){this.__uA(bq.nodeId);
}}},__uB:function(bs,bt){bs.lastChild=[bt];
...
}},__uC:function(){this.__ux=[];
this.__uy=false;
this._rowArr=[];
this._nodeRowMap=[];
this._selections={};
this.__uz(0,1);
if(this.__uy){this.__uA(0);
}this.__ux=null;
...
},addSpacer:function(){if(!this.__uF){this.__uF=new qx.ui.core.Spacer();
}else{this._remove(this.__uF);
}this._add(this.__uF);
},addOpenButton:function(){this._add(this.getChildControl(k));
},_onChangeOpen:function(e){if(this.isOpenable()){this.setOpen(e.getData());
}},addIcon:function(){var K=this.getChildControl(j);

if(this.__uE){this._remove(K);
}this._add(K);
this.__uE=true;
},addLabel:function(L){var M=this.getChildControl(f);

if(this.__uD){this._remove(M);
}
if(L){this.setLabel(L);
}else{M.setValue(this.getLabel());
}this._add(M);
this.__uD=true;
},_applyIcon:function(N,O){if(!this.__uH()){this.__uI(N);
}else if(!this.isOpen()){this.__uI(N);
}},_applyIconOpened:function(P,Q){if(this.isOpen()){if(this.__uG()&&this.__uH()){this.__uI(P);
}else if(!this.__uG()&&this.__uH()){this.__uI(P);
...
if(U){W=this.__uH()?this.getIconOpened():null;
}else{W=this.getIcon();
}
if(W){this.__uI(W);
}U?this.addState(c):this.removeState(c);
},__uG:function(){return qx.util.PropertyUtil.getUserValue(this,j);
},__uH:function(){return qx.util.PropertyUtil.getUserValue(this,s);
},__uI:function(X){var Y=this.getChildControl(j,true);
...
if(this.__uF){this.__uF.setWidth((this.getLevel()+1)*this.getIndent()-be);
...
}},members:{__db:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__db;
},getMarkup:function(e){if(this.__db){return this.__db;
...
(function(){var j="visible",h="excluded",g="qx.ui.tree.core.AbstractTreeItem",f="open",e="abstract",d="__uJ",c="__fv",b="addItem",a="removeItem";
...
this.__fv=[];
},properties:{parent:{check:g,nullable:true}},members:{__fv:null,__uJ:null,getTree:function(){var n=this;
...
}}},getChildrenContainer:function(){if(!this.__uJ){this.__uJ=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?j:h});
}return this.__uJ;
},hasChildrenContainer:function(){return this.__uJ;
...
}},__uK:function(){if(this.getParentChildrenContainer()){this.getParentChildrenContainer()._addAfter(this.getChildrenContainer(),this);
...
}},addAt:function(O,P){if(P==this.__fv.length){this.add(O);
return;
}var T=O.getParent();
...
}qx.lang.Array.insertAt(this.__fv,O,P);
...
if(W){W.remove(U);
}this.addAt(U,this.__fv.indexOf(V));
},addAfter:function(X,Y){var ba=X.getParent();

if(ba){ba.remove(X);
}this.addAt(X,this.__fv.indexOf(Y)+1);
},addAtBegin:function(bb){this.addAt(bb,0);
},remove:function(bc){for(var i=0,l=arguments.length;i<l;i++){var bg=arguments[i];

if(this.__fv.indexOf(bg)==-1){this.warn("Cannot remove treeitem '"+bg+"'. It is not a child of this tree item.");
return;
...
}}qx.lang.Array.remove(this.__fv,bg);
bg.setParent(null);
bd.remove(bg);
}var be=this.getTree();
...
},removeAt:function(bh){var bi=this.__fv[bh];

if(bi){this.remove(bi);
}},removeAll:function(){var bj=this.__fv.concat();

for(var i=this.__fv.length-1;i>=0;i--){this.remove(this.__fv[i]);
}return bj;
...
},getLookupTable:function(){return this.__rA;
},_reverseLookup:function(bb){return bb;
},_getDataFromRow:function(bc){return this.__rA.getItem(bc);
},_getSelectables:function(){return this.__rA;
},getOpenNodes:function(){return this.__uL;
},isNode:function(bd){return qx.Class.hasProperty(bd.constructor,this.getChildProperty());
},getLevel:function(be){return this.__uM[be];
...
},_applyModel:function(bD,bE){this.__uL=[];
...
if(qx.lang.String.startsWith(bI,this.getChildProperty())){this.__uP();
}},_onUpdated:function(event){if(this.__fG==null){this.__fG=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);
},this);
}this.__fG.schedule();
},_onOpen:function(event){var bK=event.getRow();
var bJ=this.__rA.getItem(bK);
...
}}},_beforeApplySelection:function(bP){if(bP.length===0&&this.getSelectionMode()===d){var bQ=this.__uU();
var bR=this.getLookupTable().indexOf(bQ);

if(bR>=0){bP.push(bR);
}}},_afterApplySelection:function(){var bS=this.getSelection();

if(bS.getLength()>0&&this.getSelectionMode()===d){this.__uT(bS.getItem(0));
}else{this.__uO=[];
}},__uP:function(){this.buildLookupTable();
...
if(this.isNodeOpen(bV)){var bT=this.__uQ(bV,bU);
bW=bW.concat(bT);
...
if(this.isNode(cd)){this.__uM.push(bY);
ca.push(cd);

if(this.isNodeOpen(cd)){var cb=this.__uQ(cd,bY);
ca=ca.concat(cb);
}}else{if(this.isShowLeafs()){this.__uM.push(bY);
ca.push(cd);
}}}return ca;
},__uR:function(ce){if(!qx.lang.Array.contains(this.__uL,ce)){this.__uL.push(ce);
this.fireDataEvent(s,ce);
}},__uS:function(cf,cg){if(cf===cg){this.__uR(cg);
return true;
}
...
var ch=this.__uS(cj,cg);

if(ch===true){this.__uR(cj);
return true;
}}return false;
},__rG:function(){this.getPane().getRowConfig().setItemCount(this.__rA.getLength());
this.getPane().fullUpdate();
},getParent:function(ck){var cm=this.__rA.indexOf(ck);
...
if(cl<cn){return this.__rA.getItem(cm);
}}return null;
},__uT:function(co){this.__uO=[];
var parent=this.getParent(co);

while(parent!=null){this.__uO.unshift(parent);
parent=this.getParent(parent);
}},__uU:function(){if(this.__uO==null){return this.getModel();
}var cp=this.getLookupTable();
var parent=this.__uO.pop();

while(parent!=null){if(cp.contains(parent)){return parent;
}parent=this.__uO.pop();
...
this.__rA.dispose();
this._layer=this._provider=this.__rA=this.__uL=null;
}});
})();
...
qx.Mixin.define(d,{construct:function(){this.__ro=[];
},properties:{labelPath:{check:q,nullable:true},iconPath:{check:q,nullable:true},labelOptions:{nullable:true},iconOptions:{nullable:true},childProperty:{check:q,nullable:true},delegate:{event:m,init:null,nullable:true}},members:{__ro:null,bindDefaultProperties:function(r,s){this.bindProperty(o,n,null,r,s);
...
}},bindProperty:function(u,v,w,x,y){var z=this.__rp(y,u);
var A=this._tree.getLookupTable();
var B=A.bind(z,x,v,w);
this.__rq(x,B);
},bindPropertyReverse:function(C,D,E,F,G){var H=this.__rp(G,C);
var I=this._tree.getLookupTable();
var J=F.bind(D,I,H,E);
this.__rq(F,J);
},removeBindings:function(){while(this.__ro.length>0){var K=this.__ro.pop();
...
}},_removeBindingsFrom:function(O){var P=this.__rr(O);
...
if(qx.lang.Array.contains(this.__ro,O)){qx.lang.Array.remove(this.__ro,O);
}},__rp:function(R,S){var T=b+R+g;

if(S!=null&&S!=o){T+=a+S;
}return T;
},__rq:function(U,V){var W=this.__rr(U);

if(!qx.lang.Array.contains(W,V)){W.push(V);
}
if(!qx.lang.Array.contains(this.__ro,U)){this.__ro.push(U);
}},__rr:function(X){var Y=X.getUserData(p);
...
q.setUserData(f,r);
q.addListener(l,this.__uV,this);
...
return q;
},poolCellWidget:function(t){t.removeListener(l,this.__uV,this);
...
},__uV:function(event){var J=event.getTarget();
...
this.__uW=false;
this.__uX=false;
},properties:{behavior:{check:j,init:null,nullable:true,apply:h,event:b}},members:{__uX:null,__uW:null,__pW:null,_applyBehavior:function(p,q){if(q!=null){q.dispose();
q=null;
...
this.__pW._updateScrollerWidths();
...
qx.event.Timer.once(function(){if(this.__pW&&!this.__pW.isDisposed()){this.__pW._updateScrollerWidths();
this.__pW._updateScrollBarVisibility();
}},this,0);
this.__uW=false;
...
this.__va=qx.util.AliasManager.getInstance();
this.__vb=qx.util.ResourceManager.getInstance();
this.__vc=qx.theme.manager.Appearance.getInstance();
this.BLANK=this.__vb.toUri(this.__va.resolve("static/blank.gif"));
},statics:{__vd:{},__uY:true,__ve:function(){qx.theme.manager.Meta.getInstance().initialize();
...
bb.__vd.line=bd.styleFrom(G);
bg(bb.__vd.line.icon);
bb.__vd.contract=bd.styleFrom(L);
bg(bb.__vd.contract.icon);
bb.__vd.expand=bd.styleFrom(C);
bg(bb.__vd.expand.icon);
bb.__vd.onlyContract=bd.styleFrom(H);
bg(bb.__vd.onlyContract.icon);
bb.__vd.onlyExpand=bd.styleFrom(S);
bg(bb.__vd.onlyExpand.icon);
bb.__vd.startContract=bd.styleFrom(W);
bg(bb.__vd.startContract.icon);
bb.__vd.startExpand=bd.styleFrom(N);
bg(bb.__vd.startExpand.icon);
bb.__vd.endContract=bd.styleFrom(v);
bg(bb.__vd.endContract.icon);
bb.__vd.endExpand=bd.styleFrom(B);
bg(bb.__vd.endExpand.icon);
bb.__vd.crossContract=bd.styleFrom(x);
bg(bb.__vd.crossContract.icon);
bb.__vd.crossExpand=bd.styleFrom(E);
bg(bb.__vd.crossExpand.icon);
bb.__vd.end=bd.styleFrom(p);
bg(bb.__vd.end.icon);
bb.__vd.cross=bd.styleFrom(q);
bg(bb.__vd.cross.icon);
}},properties:{useTreeLines:{check:j,init:true},excludeFirstLevelTreeLines:{check:j,init:false},alwaysShowOpenCloseSymbol:{check:j,init:false}},members:{__va:null,__vc:null,__vb:null,_getCellStyle:function(bh){var bi=bh.value;
...
var br=this.__vb.toUri(this.__va.resolve(bq.url));
...
if(!bI){if(bG.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.LEAF){var o=this.__vc.styleFrom(U);
}else{var bJ={opened:bG.bOpened};
var o=this.__vc.styleFrom(z,bJ);
}bI=o.icon;
...
if(bW==0&&cb){bY=false;
}if(bW<bX.level-1){return (bY&&!bX.lastChild[bW]?cc.__vd.line:{icon:this.BLANK});
}var cd=bX.lastChild[bX.lastChild.length-1];
if(bX.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH&&!bX.bHideOpenClose){if(bX.children.length>0||ca){if(!bY){return (bX.bOpened?cc.__vd.contract:cc.__vd.expand);
}if(bW==0&&bX.bFirstChild){if(cd){return (bX.bOpened?cc.__vd.onlyContract:cc.__vd.onlyExpand);
}else{return (bX.bOpened?cc.__vd.startContract:cc.__vd.startExpand);
}}if(cd){return (bX.bOpened?cc.__vd.endContract:cc.__vd.endExpand);
}return (bX.bOpened?cc.__vd.crossContract:cc.__vd.crossExpand);
}}if(bY){if(bX.parentNodeId==0){if(cd&&bX.bFirstChild){return {icon:this.BLANK};
}if(cd){return cc.__vd.end;
}if(bX.bFirstChild){return (bX.bOpened?cc.__vd.startContract:cc.__vd.startExpand);
}}return (cd?cc.__vd.end:cc.__vd.cross);
}return {icon:this.BLANK};
}},destruct:function(){this.__va=this.__vb=this.__vc=null;
}});
})();
...
this.__pW=d;
},members:{__pW:null,getTable:function(){return this.__pW;
...
},members:{__vf:null,renderLayout:function(f,top,g,h){this.__vf=g;
},getComputedWidth:function(){return this.__vf;
...
(function(){var j="__vi",h="Function",g="Boolean",f="minWidth",e="width",d="qx.ui.table.columnmodel.Resize",c="qx.ui.table.columnmodel.resizebehavior.Default",b="__vh",a="maxWidth";
...
this.__vg=[];
this.__vh=new qx.ui.layout.HBox();
this.__vh.connectToWidget(this);
this.__vi=new qx.util.DeferredCall(this._computeColumnsFlexWidth,this);
},properties:{newResizeBehaviorColumnData:{check:h,init:function(k){return new qx.ui.core.ColumnData();
}},initializeWidthsOnEveryAppear:{check:g,init:false},tableColumnModel:{check:d}},members:{__vh:null,__vj:null,__vg:null,__vi:null,__vk:false,setWidth:function(m,n,o){if(m>=this.__vg.length){throw new Error("Column number out of range");
}this.__vg[m].setColumnWidth(n,o);
this.__vi.schedule();
},setMinWidth:function(p,q){if(p>=this.__vg.length){throw new Error("Column number out of range");
}this.__vg[p].setMinWidth(q);
this.__vi.schedule();
},setMaxWidth:function(r,s){if(r>=this.__vg.length){throw new Error("Column number out of range");
}this.__vg[r].setMaxWidth(s);
this.__vi.schedule();
...
}}},onAppear:function(event,w){if(w===true||!this.__vk||this.getInitializeWidthsOnEveryAppear()){this._computeColumnsFlexWidth();
this.__vk=true;
...
},_setNumColumns:function(y){var z=this.__vg;
if(y<=z.length){z.splice(y,z.length);
return;
}for(var i=z.length;i<y;i++){z[i]=this.getNewResizeBehaviorColumnData()();
z[i].columnNumber=i;
}},getLayoutChildren:function(){return this.__vj;
},_computeColumnsFlexWidth:function(){this.__vi.cancel();
...
this.__vl();
this.__vh.renderLayout(E,100);
for(i=0,l=G.length;i<l;i++){var F=G[i].getComputedWidth();
A.setColumnWidth(C[i],F);
}},__vl:function(){this.__vh.invalidateChildrenCache();
var H=this.__vj;
...
}},_getResizeColumnData:function(){return this.__vg;
}},destruct:function(){this.__vg=this.__vj=null;
this._disposeObjects(b,j);
}});
})();
...
(function(){var k="Trash",j="Inbox",h="Sent",g="Data",f="model",e="Edit",d="Inherits from qx.ui.table.Table. Suited for multi-column trees.",c="Windows (C:)",b="Documents (D:)",a="TreeVirtual (Legacy)",C="Desktop",B="widgetbrowser.pages.Tree",A="loaded",z="Virtual implementation of Tree. Shares appearance.",y="Presets",x="Network",w="children",v="Files",u="TreeVirtual",t="name",r="root",s="Workspace",p="Junk #",q="Tree",n="Spam",o="widgetbrowser/tree.json",l="Spam Message #",m="VirtualTree";
...
this.add(this.__kR);
this.initWidgets();
},members:{__kR:null,initWidgets:function(){var H;
var D=this._widgets;
H=new qx.ui.basic.Label(q);
this.__kR.add(H,{row:0,column:0});
var F=this.__vm();
this.__kR.add(F,{row:1,column:0});
D.push(F);
H=new qx.ui.basic.Label(m);
H.setToolTip(new qx.ui.tooltip.ToolTip(z));
this.__kR.add(H,{row:0,column:1});
var G=this.__vn();
this.__kR.add(G,{row:1,column:1});
D.push(G);
H=new qx.ui.basic.Label(a);
H.setToolTip(new qx.ui.tooltip.ToolTip(d));
this.__kR.add(H,{row:0,column:2});
var E=this.__vo();
this.__kR.add(E,{row:1,column:2});
D.push(E);
},__vm:function(){var R=new qx.ui.tree.Tree().set({width:200,height:400});
var W=new qx.ui.tree.TreeFolder(r);
W.setOpen(true);
R.setRoot(W);
var U=new qx.ui.tree.TreeFolder(C);
U.setOpen(true);
W.add(U);
var M=new qx.ui.tree.TreeFolder(v);
var K=new qx.ui.tree.TreeFolder(s);
var L=new qx.ui.tree.TreeFolder(x);
var J=new qx.ui.tree.TreeFolder(k);
U.add(M,K,L,J);
var S=new qx.ui.tree.TreeFile(c);
var I=new qx.ui.tree.TreeFile(b);
K.add(S,I);
var V=new qx.ui.tree.TreeFolder(j);
var N=new qx.ui.tree.TreeFolder(y);
var Q=new qx.ui.tree.TreeFolder(h);
var P=new qx.ui.tree.TreeFolder(k);

for(var i=0;i<30;i++){P.add(new qx.ui.tree.TreeFile(p+i));
}var T=new qx.ui.tree.TreeFolder(g);
var O=new qx.ui.tree.TreeFolder(e);
V.add(N,Q,P,T,O);
W.add(V);
return R;
},__vn:function(){var Y=new qx.ui.tree.VirtualTree(null,t,w).set({width:200,height:400});
var X=o;
var ba=new qx.data.store.Json(X);
ba.bind(f,Y,f);
ba.addListener(A,function(){Y.openNode(Y.getModel().getChildren().getItem(0));
},this);
return Y;
},__vo:function(){var bd=new qx.ui.treevirtual.TreeVirtual(u);
bd.setWidth(200);
var be=bd.getDataModel();
var bb=be.addBranch(null,j,true,false);
var bc=be.addBranch(bb,n,false);

for(var i=1;i<3000;i++){be.addLeaf(bc,l+i);
}be.addBranch(bb,h,true);
be.addBranch(bb,k,true);
be.addBranch(bb,g,true);
be.addBranch(bb,e,true);
be.setData();
return bd;
}}});
})();
...
(function(){var l="dblclick",k="click",j="Boolean",h="excluded",g="visible",f="qx.event.type.Data",d="_applyOpenMode",c="Space",b="Left",a="Enter",z="changeOpenMode",y="_applyRootOpenClose",x="changeSelection",w="qx.ui.tree.Tree",v="qx.ui.tree.core.AbstractTreeItem",u="tree",t="_applyHideRoot",s="changeRoot",r="__ie",q="_applyRoot",o="keypress",p="none",m="pane",n="Right";
