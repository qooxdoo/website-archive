...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.virtual.ComboBox","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.ComboBox-qx.theme.Simple.1343b5ee85bd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="textfield",k="value",j="button",h="dropdown",g="inner",f="list",d="change",c="_applyValue",b="select",a="focusin",z="focusout",y="Enter",x="qx.ui.form.VirtualComboBox",w="changeValue",v="Function",u="qx.debug",t="virtual-combobox",s="String",r="",q="single",o="converter",p="click",n="_applyPlaceholder";
qx.Class.define(x,{extend:qx.ui.form.core.AbstractVirtualBox,implement:[qx.ui.form.IStringForm],construct:function(A){qx.ui.form.core.AbstractVirtualBox.call(this,A);
var B=this._createChildControl(m);
this._createChildControl(j);
var C=this.getChildControl(h);
C.getChildControl(f).setSelectionMode(q);
this.__eH=C.getSelection();
this.__eH.addListener(d,this.__za,this);
this.bind(k,B,k);
B.bind(k,this,k);
this.addListener(a,function(e){B.fireNonBubblingEvent(a,qx.event.type.Focus);
},this);
this.addListener(z,function(e){B.fireNonBubblingEvent(z,qx.event.type.Focus);
},this);
if(qx.core.Environment.get(u)){qx.log.Logger.deprecateMethodOverriding(this,qx.ui.form.VirtualComboBox,c);
}},properties:{appearance:{refine:true,init:t},width:{refine:true,init:120},value:{nullable:true,event:w,apply:c},placeholder:{check:s,nullable:true,apply:n},defaultFormat:{check:v,init:null,nullable:true}},members:{__yW:null,__yX:null,__eH:null,__yY:null,getTextSelection:function(){return this.getChildControl(m).getTextSelection();
},getTextSelectionLength:function(){return this.getChildControl(m).getTextSelectionLength();
},setTextSelection:function(D,E){this.getChildControl(m).setTextSelection(D,E);
...
this.selectAllText();
},tabFocus:function(){var F=this.getChildControl(m);
F.getFocusElement().focus();
F.selectAllText();
},focus:function(){qx.ui.form.core.AbstractVirtualBox.prototype.focus.call(this);
this.getChildControl(m).getFocusElement().focus();
},_createChildControlImpl:function(G,H){var I;

switch(G){case m:I=new qx.ui.form.TextField();
I.setFocusable(false);
I.addState(g);
this._add(I,{flex:1});
break;
case j:I=new qx.ui.form.Button();
I.setFocusable(false);
I.setKeepActive(true);
I.addState(g);
this._add(I);
break;
}return I||qx.ui.form.core.AbstractVirtualBox.prototype._createChildControlImpl.call(this,G,H);
},_beforeOpen:function(){this.__zb();
},_handleKeyboard:function(event){var J=this._getAction(event);

switch(J){case b:this.setValue(this.getChildControl(m).getValue());
break;
default:qx.ui.form.core.AbstractVirtualBox.prototype._handleKeyboard.call(this,event);
break;
}},_getAction:function(event){var K=event.getKeyIdentifier();
var M=this.getChildControl(h).isVisible();
var L=this._isModifierPressed(event);
...
if(O==this.getChildControl(j)){this.toggle();
}else{this.close();
}},__za:function(event){if(this.__yY==true){return;
}var P=this.__eH.getItem(0);
P=this.__bw(P);
this.setValue(P);
},_applyValue:function(Q,R){},_applyPlaceholder:function(S,T){this.getChildControl(m).setPlaceholder(S);
},__zb:function(){var W=this.getValue();
var V=this.getChildControl(h);
var U=V.getSelection();

if(this.__bw(U.getItem(0))!==W){this.__yY=true;
U.removeAll();
this.__yY=false;
if(W==null||W==r){return;
}var bb=this.getModel();
var ba=V.getChildControl(f)._getLookupTable();

for(var i=0,l=ba.length;i<l;i++){var X=bb.getItem(ba[i]);
var Y=this.__bw(X);

if(Y&&Y.indexOf(W)==0){V.setPreselected(X);
break;
}}}},__bw:function(bc){var be=this.getLabelOptions();
var bd=this.getDefaultFormat();
var bg=this.getLabelPath();
var bf=null;

if(bg!=null){bf=qx.data.SingleValueBinding.getValueFromObject(bc,bg);
}else if(qx.lang.Type.isString(bc)){bf=bc;
}var bh=qx.util.Delegate.getMethod(be,o);

if(bh!=null){bf=bh(bf);
}
if(bf!=null&&bd!=null){bf=bd(qx.lang.String.stripTags(bf));
}return bf;
}},destruct:function(){var bi=this.getChildControl(m);
this.removeAllBindings();
bi.removeAllBindings();
