...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.FormController","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.FormController-qx.theme.Simple.542129c3996b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
}},properties:{model:{check:h,apply:a,event:j,nullable:true,dereference:true},target:{check:b,apply:f,event:d,nullable:true,init:null,dereference:true}},members:{__Le:null,__Ld:null,addBindingOptions:function(name,p,q){this.__Ld[name]=[p,q];
if(this.getModel()==null||this.getTarget()==null){return;
}var r=this.getTarget().getItems()[name];
var s=this.__Lh(r)?l:k;
this.__Le.removeTarget(r,s,name);
this.__Le.addTarget(r,s,name,!this._selfUpdate,p,q);
...
}if(this.__Ld[name]&&this.__Ld[name][1]){u=this.__Ld[name][1].converter(u);
}B[y[i]]=u;
}else{if(!B[y[i]]){B[y[i]]={};
}B=B[y[i]];
...
var E=this.__Lh(F)?l:k;
var D=this.__Ld[name];
D=D&&this.__Ld[name][1];
qx.data.SingleValueBinding.updateTarget(F,E,this.getModel(),name,D);
}},_applyTarget:function(G,H){if(H!=null){this.__Lg(H);
}if(this.getModel()==null){return;
}if(G!=null){this.__Lf();
}},_applyModel:function(I,J){if(this.__Le!=null){var K=this.getTarget().getItems();

for(var name in K){var M=K[name];
var L=this.__Lh(M)?l:k;
this.__Le.removeTarget(M,L,name);
}}if(this.__Le!=null){this.__Le.setModel(I);
}if(this.getTarget()==null){return;
}if(I!=null){this.__Lf();
}},__Lf:function(){if(this.__Le==null){this.__Le=new qx.data.controller.Object(this.getModel());
}var N=this.getTarget().getItems();
for(var name in N){var Q=N[name];
var O=this.__Lh(Q)?l:k;
var P=this.__Ld[name];
try{if(P==null){this.__Le.addTarget(Q,O,name,!this._selfUpdate);
}else{this.__Le.addTarget(Q,O,name,!this._selfUpdate,P[0],P[1]);
}}catch(R){if(qx.core.Environment.get(g)){this.warn("Could not bind property "+name+" of "+this.getModel());
}}}},__Lg:function(S){if(this.__Le==null){return;
}var T=S.getItems();
for(var name in T){var V=T[name];
var U=this.__Lh(V)?l:k;
this.__Le.removeTarget(V,U,name);
}},__Lh:function(W){return qx.Class.hasInterface(W.constructor,qx.ui.core.ISingleSelection)&&qx.Class.hasInterface(W.constructor,qx.ui.form.IModelSelection);
}},destruct:function(){if(this.__Le){this.__Le.dispose();
}}});
})();
