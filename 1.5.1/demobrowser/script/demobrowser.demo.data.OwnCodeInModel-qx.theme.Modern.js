...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.OwnCodeInModel","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.OwnCodeInModel-qx.theme.Modern.4474378e34a4.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="demobrowser.demo.data.OwnCodeInModel",b="os.name",a="engine.version";
...
(function(){var r="execute",q="icon/16/mimetypes/",p="name",o="demobrowser/demo/data/list.json",n="model",m="model.items",l="demobrowser.demo.data.OwnCodeInModel",k=".png",j="type",i="Using the delegation to bring two sort function to the root model ",e="Sort By Name",h="items",g="class of this demo. The buttons just calling those sort functions.",d="<b>Adding own code to the model</b><br/>",c="demobrowser.demo.data.ItemsMixin",f="Sort By Type";
qx.Class.define(l,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);
var t=new qx.ui.form.List();
this.getRoot().add(t,{left:10,top:80});
var w=new qx.data.controller.List(null,t);
w.setLabelPath(p);
w.setIconOptions({converter:function(A){return q+A+k;
}});
w.setIconPath(j);
var x={getModelMixins:function(B){if(B==h){return demobrowser.demo.data.ItemsMixin;
}}};
var s=qx.util.ResourceManager.getInstance().toUri(o);
var z=new qx.data.store.Json(s,x);
z.bind(m,w,n);
var y=new qx.ui.form.Button(f);
y.setWidth(120);
this.getRoot().add(y,{left:130,top:80});
y.addListener(r,function(){z.getModel().sortByType();
},this);
var v=new qx.ui.form.Button(e);
v.setWidth(120);
this.getRoot().add(v,{left:130,top:110});
v.addListener(r,function(){z.getModel().sortByName();
},this);
...
qx.Mixin.define(c,{members:{sortByType:function(){var C=this.getItems();
C.sort(function(a,b){if(a.getType()>b.getType()){return 1;
}else if(a.getType()<b.getType()){return -1;
}else{return 0;
}});
},sortByName:function(){var D=this.getItems();
D.sort(function(a,b){return a.getName()>b.getName()?1:-1;
});
}}});
})();
...
(function(){var h="Please use an array as parameter.",g="qx.ui.form.MModelSelection",f="change",d="qx.debug",c="__nj",b="changeSelection",a="qx.event.type.Data";
qx.Mixin.define(g,{construct:function(){this.__nj=new qx.data.Array();
this.__nj.addListener(f,this.__nm,this);
this.addListener(b,this.__nl,this);
},events:{changeModelSelection:a},members:{__nj:null,__nk:false,__nl:function(){if(this.__nk){return;
...
if(!qx.lang.Array.equals(s,r)){this.__nl();
}},getModelSelection:function(){return this.__nj;
},setModelSelection:function(v){if(!v){this.__nj.removeAll();
return;
}
if(qx.core.Environment.get(d)){this.assertArray(v,h);
}v.unshift(this.__nj.getLength());
v.unshift(0);
var w=this.__nj.splice.apply(this.__nj,v);
w.dispose();
}},destruct:function(){this._disposeObjects(c);
}});
})();
...
if(r==undefined){this.__no=[];
}else if(arguments.length>1){this.__no=[];

for(var i=0;i<arguments.length;i++){this.__no.push(arguments[i]);
}}else if(typeof r==a){this.__no=new Array(r);
}else if(r instanceof Array){this.__no=qx.lang.Array.clone(r);
}else{this.__no=[];
this.dispose();
throw new Error("Type of the parameter not supported!");
}for(var i=0;i<this.__no.length;i++){this._applyEventPropagation(this.__no[i],null,i);
}this.__np();
},properties:{autoDisposeItems:{check:g,init:false}},events:{"change":h,"changeLength":h},members:{__no:null,concat:function(s){if(s){var t=this.__no.concat(s);
}else{var t=this.__no.concat();
}return new qx.data.Array(t);
},join:function(u){return this.__no.join(u);
},pop:function(){var v=this.__no.pop();
this.__np();
...
return v;
},push:function(w){for(var i=0;i<arguments.length;i++){this.__no.push(arguments[i]);
this.__np();
...
}var x=this.__no.concat();
this.__no.reverse();
this.fireDataEvent(q,{start:0,end:this.length-1,type:l,items:null},null);
this.fireDataEvent(p,{value:this.__no,name:m+(this.__no.length-1),old:x});
},shift:function(){if(this.length==0){return;
}var y=this.__no.shift();
this.__np();
...
return y;
},slice:function(z,A){return new qx.data.Array(this.__no.slice(z,A));
},splice:function(B,C,D){var L=this.__no.length;
var H=this.__no.splice.apply(this.__no,arguments);
if(this.__no.length!=L){this.__np();
}var J=C>0;
var F=arguments.length>2;
var G=null;

if(J||F){if(this.__no.length>L){var K=o;
}else if(this.__no.length<L){var K=n;
G=H;
}else{var K=l;
...
}var N=this.__no.concat();
this.__no.sort.apply(this.__no,arguments);
this.fireDataEvent(q,{start:0,end:this.length-1,type:l,items:null},null);
this.fireDataEvent(p,{value:this.__no,name:m+(this.length-1),old:N});
},unshift:function(O){for(var i=arguments.length-1;i>=0;i--){this.__no.unshift(arguments[i]);
this.__np();
this._registerEventChaining(arguments[i],null,0);
this.fireDataEvent(p,{value:[this.__no[0]],name:j,old:[this.__no[1]]});
this.fireDataEvent(q,{start:0,end:this.length-1,type:o,items:[arguments[i]]},null);
}return this.length;
},toArray:function(){return this.__no;
},getItem:function(P){return this.__no[P];
},setItem:function(Q,R){var S=this.__no[Q];
if(S===R){return;
}this.__no[Q]=R;
...
},indexOf:function(T){return this.__no.indexOf(T);
},toString:function(){if(this.__no!=null){return this.__no.toString();
}return c;
},contains:function(U){return this.__no.indexOf(U)!==-1;
...
}Array.prototype.push.apply(this.__no,bj);
for(var i=0;i<bj.length;i++){this._registerEventChaining(bj[i],null,this.__no.length+i);
}var bk=this.length;
this.__np();
...
},forEach:function(br,bs){for(var i=0;i<this.__no.length;i++){br.call(bs,this.__no[i],i,this);
}},__np:function(){var bt=this.length;
this.length=this.__no.length;
this.fireDataEvent(d,this.length,bt);
}},destruct:function(){for(var i=0;i<this.__no.length;i++){var bu=this.__no[i];
...
}if(this.getSelection()==null){this.__An=new qx.data.Array();
this.setSelection(this.__An);
}},properties:{selection:{check:d,event:f,apply:b,init:null}},members:{_modifingSelection:0,__Ao:null,__Ap:null,__An:null,_applySelection:function(g,h){if(this.__Ap!=undefined&&h!=undefined){h.removeListenerById(this.__Ap);
}this.__Ap=g.addListener(e,this.__Aq,this);
this._updateSelection();
},__Aq:function(){this._updateSelection();
},_changeTargetSelection:function(){if(this.getTarget()==null){return;
}if(!this.__Ar()&&!this.__As()){return;
...
},_addChangeTargetListener:function(m,n){if(this.__Ao!=undefined&&n!=undefined){n.removeListenerById(this.__Ao);
}
if(m!=null){if(this.__Ar()||this.__As()){this.__Ao=m.addListener(f,this._changeTargetSelection,this);
...
}}}else if(this.__As()){var p=this.getSelection().getItem(this.getSelection().length-1);

if(p!==undefined){this.__sa(p);
this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();
}else{this.getTarget().resetSelection();
}}this._endSelectionModification();
},__Ar:function(){var t=this.getTarget().constructor;
return qx.Class.implementsInterface(t,qx.ui.core.IMultiSelection);
},__As:function(){var u=this.getTarget().constructor;
return qx.Class.implementsInterface(u,qx.ui.core.ISingleSelection);
},__sa:function(v){var w=this.__At(v);
if(w==null){return;
}if(this.__Ar()){this.getTarget().addToSelection(w);
}else if(this.__As()){this.getTarget().setSelection([w]);
}},__At:function(x){var y=this.getTarget().getSelectables(true);
...
}},destruct:function(){if(this.__An){this.__An.dispose();
}}});
})();
...
},_applyIconOptions:function(F,G){this.__Ak();
},_applyLabelOptions:function(H,I){this.__Ak();
},_applyIconPath:function(J,K){this.__Ak();
},_applyLabelPath:function(L,M){this.__Ak();
},_applyModel:function(N,O){if(O!=undefined){if(this.__Ac!=undefined){O.removeListenerById(this.__Ac);
}}if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();
}if(N!=null){this.__Ac=N.addListener(k,this.__Af,this);
this.__yj();
this.__Ag();
...
for(var i=0;i<length;i++){this.__Aj();
...
if(Q!=null){if(this.getModel()!=null){for(var i=0;i<this.__yc.length;i++){this.__Ai(this.__Am(i));
}}}},__Af:function(){this.__Ae=true;
qx.ui.core.queue.Widget.add(this);
if(this.__yc.length!=this.getModel().getLength()){this.update();
}},syncWidget:function(){if(this.__Ad){this._changeTargetSelection();
}
if(this.__Ae){this._updateSelection();
}this.__Ae=this.__Ad=null;
},__Ag:function(){if(this.getTarget()==null){return;
}this.__yj();
var U=this.__yc.length;
var T=this.getTarget().getChildren().length;
if(U>T){for(var j=T;j<U;j++){this.__Ai(this.__Am(j));
}}else if(U<T){for(var j=T;j>U;j--){this.__Aj();
}}this.__Ad=true;
qx.ui.core.queue.Widget.add(this);
},__Ah:function(){var V=this.getModel();
if(V!=null){V.removeListenerById(this.__Ac);
this.__Ac=V.addListener(k,this.__Af,this);
...
},__Ai:function(Y){var ba=this._createItem();
this._bindListItem(ba,Y);
this.getTarget().add(ba);
},__Aj:function(){this._startSelectionModification();
...
if(!qx.lang.Array.contains(this.__zY,bn)){this.__zY.push(bn);
...
if(!qx.lang.Array.contains(this.__Aa,bt)){this.__Aa.push(bt);
}},_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;
}for(var i=0;i<this.__zY.length;i++){if(this.__Ab[this.__zY[i]]!=null){this.__Ab[this.__zY[i]]();
}}},_removeBindingsFrom:function(bD){for(var i=0;i<this.__zY.length;i++){var bE=bD.getUserData(this.__zY[i]+g);

if(bE!=null){this.removeBinding(bE);
}}for(var i=0;i<this.__Aa.length;i++){var bE=bD.getUserData(this.__Aa[i]+h);

if(bE!=null){bD.removeBinding(bE);
}}},__Ak:function(){if(this.getTarget()==null||this.getModel()==null){return;
}var bF=this.getTarget().getChildren();
for(var i=0;i<bF.length;i++){this._removeBindingsFrom(bF[i]);
this._bindListItem(bF[i],this.__Am(i));
}this.__Ah();
...
this._endSelectionModification();
this._updateSelection();
},_setFilter:function(bP,bQ){if((bP==null||bP.filter==null)&&(bQ!=null&&bQ.filter!=null)){this.__Al();
...
this.__yj();
if(bS.length>this.__yc.length){for(var j=bS.length;j>this.__yc.length;j--){this.getTarget().removeAt(j-1).destroy();
}}else if(bS.length<this.__yc.length){for(var j=bS.length;j<this.__yc.length;j++){var bT=this._createItem();
this.getTarget().add(bT);
}}var bR=this.getTarget().getChildren();

for(var i=0;i<bR.length;i++){this._bindListItem(bR[i],this.__Am(i));
}this.__Ah();
this._endSelectionModification();
this._updateSelection();
},__Al:function(){this.__yj();
this.__Ag();
this.__Ak();
this.__Ae=true;
qx.ui.core.queue.Widget.add(this);
},__yj:function(){var bW=this.getModel();
...
for(var i=0;i<bW.getLength();i++){if(bV==null||bV(bW.getItem(i))){this.__yc.push(i);
}}},__Am:function(bY){return this.__yc[bY];
}},destruct:function(){this.__yc=this.__Ab=this.__zY=null;
this.__Aa=null;
qx.ui.core.queue.Widget.remove(this);
}});
})();
...
}},members:{__zy:null,__zz:function(r){return qx.Bootstrap.getKeys(r).sort().join(h);
...
}if(this.__zy&&this.__zy.getModelClass&&this.__zy.getModelClass(v)!=null){return;
}var C={};
var B={__zA:this.__zA};

for(var w in s){w=w.replace(/-/g,m);
if(qx.core.Environment.get(j)){this.assertTrue((/^[$0-9A-Za-z_]*$/).test(w),b+w+a);
}C[w]={};
C[w].nullable=true;
C[w].event=f+qx.lang.String.firstUp(w);
if(t){C[w].apply=c;
}if(this.__zy&&this.__zy.getValidationRule){var y=this.__zy.getValidationRule(v,w);

if(y){C[w].validate=k+w;
B[k+w]=y;
}}}if(this.__zy&&this.__zy.getModelSuperClass){var A=this.__zy.getModelSuperClass(v)||qx.core.Object;
}else{var A=qx.core.Object;
}var x=[];

if(this.__zy&&this.__zy.getModelMixins){var z=this.__zy.getModelMixins(v);
if(!qx.lang.Type.isArray(z)){if(z!=null){x=[z];
}}}if(t){x.push(qx.data.marshal.MEventBubbling);
}var u={extend:A,include:x,properties:C,members:B,destruct:this.__zB};
qx.Class.define(n+v,u);
},__zB:function(){var D=qx.util.PropertyUtil.getAllProperties(this.constructor);

for(var E in D){this.__zA(this.get(D[E].name));
}},__zA:function(F){if(!(F instanceof qx.core.Object)){return ;
}if(F.isDisposed()){return;
}F.dispose();
},__zC:function(G){var H;
if(this.__zy&&this.__zy.getModelClass){H=this.__zy.getModelClass(G);
}
...
}else if(N){var L=this.__zz(J);
var Q=this.__zC(L);
for(var O in J){var M=O.replace(/-/g,m);
...
(function(){var j="qx.event.type.Event",i="qx.debug.io",h="qx.debug",g="fail",f="GET",e="qx.event.type.Data",d="load",c="abort",b="loadEnd",a="Map",D="qx.io.request.AbstractRequest",C="changePhase",B="sent",A="qx.io.request.authentication.IAuthentication",z="error",y="loading",x="success",w="String",v="",u="opened",q="POST",r="statusError",o="readyStateChange",p="abstract",m="unsent",n="changeResponse",k="Number",l="Content-Type",s="timeout",t="undefined";
qx.Class.define(D,{type:p,extend:qx.core.Object,construct:function(E){qx.core.Object.call(this);

if(E!==undefined){this.setUrl(E);
}var F=this._transport=this._createTransport();
this._setPhase(m);
...
F.onreadystatechange=this.__tb;
F.onload=this.__tc;
F.onloadend=this.__td;
F.onabort=this.__te;
F.ontimeout=this.__sG;
F.onerror=this.__tf;
},events:{"readyStateChange":j,"success":j,"load":j,"loadEnd":j,"abort":j,"timeout":j,"error":j,"statusError":j,"fail":j,"changeResponse":e,"changePhase":e},properties:{url:{check:w},requestHeaders:{check:a,nullable:true},timeout:{check:k,nullable:true,init:0},requestData:{check:function(G){return qx.lang.Type.isString(G)||qx.Class.isSubClassOf(G.constructor,qx.core.Object)||qx.lang.Type.isObject(G);
...
},send:function(){var L=this._transport,H,K,I,J;
H=this._getConfiguredUrl();
if(/\#/.test(H)){H=H.replace(/\#.*/,v);
}L.timeout=this.getTimeout()*1000;
K=qx.lang.Type.isFunction(this.getMethod)?this.getMethod():f;
I=qx.lang.Type.isFunction(this.getAsync)?this.getAsync():true;
if(qx.core.Environment.get(i)){this.debug("Open low-level request with method: "+K+", url: "+H+", async: "+I);
}L.open(K,H,I);
this._setPhase(u);
J=this._serializeData(this.getRequestData());
...
},getResponseHeader:function(M){return this._transport.getResponseHeader(M);
},getResponseContentType:function(){return this.getResponseHeader(l);
},isDone:function(){return this.getReadyState()===4;
},getResponse:function(){return this.__tg;
},_setResponse:function(N){var O=N;

if(this.__tg!==N){this.__tg=N;
this.fireEvent(n,qx.event.type.Data,[this.__tg,O]);
}},_onReadyStateChange:function(){var P,Q=this.getReadyState();

if(qx.core.Environment.get(i)){this.debug("Fire readyState: "+Q);
}this.fireEvent(o);
if(this.__sJ){return;
}
if(Q===3){this._setPhase(y);
}
...
}P=this._getParsedResponse();
this._setResponse(P);
this._fireStatefulEvent(x);
}else{if(this.getStatus()!==0){this._fireStatefulEvent(r);
this.fireEvent(g);
}}}},_onLoad:function(){this.fireEvent(d);
},_onLoadEnd:function(){this.fireEvent(b);
},_onAbort:function(){this._fireStatefulEvent(c);
},_onTimeout:function(){this._fireStatefulEvent(s);
this.fireEvent(g);
},_onError:function(){this.fireEvent(z);
this.fireEvent(g);
},_fireStatefulEvent:function(R){if(qx.core.Environment.get(h)){qx.core.Assert.assertString(R);
}this._setPhase(R);
this.fireEvent(R);
},_setPhase:function(S){var T=this.__th;

if(qx.core.Environment.get(h)){qx.core.Assert.assertString(S);
qx.core.Assert.assertMatch(S,/^(unsent)|(opened)|(sent)|(loading)|(load)|(success)|(abort)|(timeout)|(statusError)$/);
}this.__th=S;
this.fireDataEvent(C,S,T);
},_serializeData:function(U){var V=typeof this.getMethod!==t&&this.getMethod()==q;

if(!U){return;
}
if(qx.lang.Type.isString(U)){return U;
}
if(qx.Class.isSubClassOf(U.constructor,qx.core.Object)){return qx.util.Serializer.toUriParameter(U);
}
if(qx.lang.Type.isObject(U)){return qx.lang.Object.toUriParameter(U,V);
}},__ti:function(){var W=this.getRequestHeaders();

if(W){qx.lang.Object.getKeys(W).forEach(function(X){this._transport.setRequestHeader(X,W[X]);
},this);
}},__tj:function(){var ba=this.getAuthentication(),Y=this._transport;

if(ba){ba.getAuthHeaders().forEach(function(bb){if(qx.core.Environment.get(h)){qx.core.Assert.assertString(bb.key);
qx.core.Assert.assertString(bb.value);
}
if(bb.key&&bb.value){if(qx.core.Environment.get(i)){this.debug("Set authentication header '"+bb.key+"' to '"+bb.value+"'");
}Y.setRequestHeader(bb.key,bb.value);
}},this);
}}},environment:{"qx.debug.io":false},destruct:function(){var bd=this._transport,bc=function(){};

if(this._transport){bd.onreadystatechange=bd.onload=bd.onloadend=bd.onabort=bd.ontimeout=bd.onerror=bc;
bd.dispose();
}}});
})();
...
(function(){var k="file",j="strict",h="anchor",g="query",f="source",e="password",d="host",c="protocol",b="qx.debug",a="user",y="directory",x="loose",w="relative",v="queryKey",u="qx.util.Uri",t="",s="path",r="authority",q="&",p="port",l="userInfo",n="?";
qx.Class.define(u,{statics:{parseUri:function(z,A){var B={key:[f,c,r,l,a,e,d,p,w,s,y,k,g,h],q:{name:v,parser:/(?:^|&)([^&=]*)=?([^&]*)/g},parser:{strict:/^(?:([^:\/?#]+):)?(?:\/\/((?:(([^:@]*)(?::([^:@]*))?)?@)?([^:\/?#]*)(?::(\d*))?))?((((?:[^?#\/]*\/)*)([^?#]*))(?:\?([^#]*))?(?:#(.*))?)/,loose:/^(?:(?![^:@]+:[^:@\/]*@)([^:\/?#.]+):)?(?:\/\/)?((?:(([^:@]*)(?::([^:@]*))?)?@)?([^:\/?#]*)(?::(\d*))?)(((\/(?:[^?#](?![^?#\/]*\.[^?#\/.]+(?:[?#]|$)))*\/?)?([^?#\/]*))(?:\?([^#]*))?(?:#(.*))?)/}};
var o=B,m=B.parser[A?j:x].exec(z),C={},i=14;

while(i--){C[o.key[i]]=m[i]||t;
}C[o.q.name]={};
C[o.key[12]].replace(o.q.parser,function(D,E,F){if(E){C[o.q.name][E]=F;
}});
return C;
},appendParamsToUrl:function(G,H){if(H===undefined){return G;
}
if(qx.core.Environment.get(b)){if(!(qx.lang.Type.isString(H)||qx.lang.Type.isObject(H))){throw new Error("params must be either string or object");
}}
if(qx.lang.Type.isObject(H)){H=qx.lang.Object.toUriParameter(H);
}
if(!H){return G;
}return G+=(/\?/).test(G)?q+H:n+H;
}}});
})();
...
(function(){var p="qx.event.type.Event",o="function",n="xml",m="Content-Type",l="GET",k="Boolean",j="",i="Accept",h="qx.debug",g="application/x-www-form-urlencoded",c="Cache-Control",f="qx.debug.io",e="qx.io.request.Xhr",b="json",a="POST",d="String";
qx.Class.define(e,{extend:qx.io.request.AbstractRequest,construct:function(q,r){if(r!==undefined){this.setMethod(r);
}qx.io.request.AbstractRequest.call(this,q);
},events:{"readyStateChange":p,"success":p,"load":p,"statusError":p},statics:{PARSER:{json:qx.lang.Json.parse,xml:qx.xml.Document.fromString}},properties:{method:{init:l},async:{check:k,init:true},accept:{check:d,nullable:true},cache:{check:function(s){return qx.lang.Type.isBoolean(s)||qx.lang.Type.isString(s);
},init:true}},members:{__tk:null,_createTransport:function(){return new qx.bom.request.Xhr();
},_getConfiguredUrl:function(){var t=this.getUrl(),u;

if(this.getMethod()===l&&this.getRequestData()){u=this._serializeData(this.getRequestData());
t=qx.util.Uri.appendParamsToUrl(t,u);
}
if(this.getCache()===false){t=qx.util.Uri.appendParamsToUrl(t,{nocache:new Date().valueOf()});
}return t;
},_setRequestHeaders:function(){var w=this._transport,v=this.getRequestHeaders();
if(qx.lang.Type.isString(this.getCache())){w.setRequestHeader(c,this.getCache());
}if(this.getMethod()===a&&!v||(v&&!v[m])){w.setRequestHeader(m,g);
}if(this.getAccept()){if(qx.core.Environment.get(f)){this.debug("Accepting: '"+this.getAccept()+"'");
}w.setRequestHeader(i,this.getAccept());
}},_getParsedResponse:function(){var y=this._transport.responseText,x=this._getParser();

if(typeof x===o){if(y!==j){return x.call(this,y);
}}return y;
},setParser:function(z){var A=qx.io.request.Xhr;
if(typeof A.PARSER[z]===o){return this.__tk=A.PARSER[z];
}if(qx.core.Environment.get(h)){qx.core.Assert.assertFunction(z);
}return this.__tk=z;
},_getParser:function(){var B=this.__tk,C;
if(B){return B;
}if(!this.isDone()){return;
}C=this.getResponseContentType();

if((/^application\/json/).test(C)){B=qx.io.request.Xhr.PARSER[b];
}
if((/^application\/xml/).test(C)){B=qx.io.request.Xhr.PARSER[n];
}if((/[^\/]+\/[^\+]+\+xml/).test(this.getResponseContentType())){B=qx.io.request.Xhr.PARSER[n];
}return B;
}}});
})();
...
},events:{"created":j,"configured":j,"sending":j,"receiving":j,"completed":R,"aborted":j,"failed":R,"timeout":R},statics:{__rj:0,methodAllowsRequestBody:function(V){return (V==Q)||(V==K);
...
},init:true,apply:H},crossDomain:{check:k,init:false},fileUpload:{check:k,init:false},transport:{check:m,nullable:true},useBasicHttpAuth:{check:k,init:false},parseJson:{check:k,init:true}},members:{__rf:null,__rg:null,__rh:null,__ri:null,__rj:null,send:function(){qx.io.remote.RequestQueue.getInstance().add(this);
...
},__rk:qx.event.GlobalError.observeMethod(function(e){var W=e.clone();
W.setTarget(this);
...
},removeRequestHeader:function(bj){delete this.__rf[bj];
},getRequestHeader:function(bk){return this.__rf[bk]||null;
},getRequestHeaders:function(){return this.__rf;
},setParameter:function(bl,bm,bn){if(bn){this.__rh[bl]=bm;
}else{this.__rg[bl]=bm;
}},removeParameter:function(bo,bp){if(bp){delete this.__rh[bo];
}else{delete this.__rg[bo];
}},getParameter:function(bq,br){if(br){return this.__rh[bq]||null;
}else{return this.__rg[bq]||null;
}},getParameters:function(bs){return (bs?this.__rh:this.__rg);
},setFormField:function(bt,bu){this.__ri[bt]=bu;
},removeFormField:function(bv){delete this.__ri[bv];
},getFormField:function(bw){return this.__ri[bw]||null;
...
(function(){var l="qx.ioRemoteDebug",k="Integer",j="qx.debug",h="aborted",g="_onaborted",f="_on",d="Boolean",c="singleton",b="interval",a="receiving",x="Request-Queue Progress: ",w="queued",v="io.maxrequests",u="_applyEnabled",t="sending",s="completed",r="__rl",q="failed",p="qx.io.remote.RequestQueue",o="__iY",m="timeout",n="/";
qx.Class.define(p,{type:c,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__dz=[];
this.__rl=[];
this.__rm=0;
this.__iY=new qx.event.Timer(500);
this.__iY.addListener(b,this._oninterval,this);
},properties:{enabled:{init:true,check:d,apply:u},maxTotalRequests:{check:k,nullable:true},maxConcurrentRequests:{check:k,init:qx.core.Environment.get(v)},defaultTimeout:{check:k,init:5000}},members:{__dz:null,__rl:null,__rm:null,__iY:null,getRequestQueue:function(){return this.__dz;
},getActiveQueue:function(){return this.__rl;
},_debug:function(){if(qx.core.Environment.get(j)){if(qx.core.Environment.get(l)){var y=this.__rl.length+n+(this.__dz.length+this.__rl.length);
...
}},_remove:function(B){qx.lang.Array.remove(this.__rl,B);
B.dispose();
this._check();
},__rn:0,_onsending:function(e){if(qx.core.Environment.get(j)){if(qx.core.Environment.get(l)){this.__rn++;
e.getTarget()._counted=true;
this.debug("ActiveCount: "+this.__rn);
}}e.getTarget().getRequest()._onsending(e);
},_onreceiving:function(e){e.getTarget().getRequest()._onreceiving(e);
},_oncompleted:function(e){if(qx.core.Environment.get(j)){if(qx.core.Environment.get(l)){if(e.getTarget()._counted){this.__rn--;
this.debug("ActiveCount: "+this.__rn);
...
D[g](event);
}}catch(F){}}},_oninterval:function(e){var M=this.__rl;

if(M.length==0){this.__iY.stop();
return;
...
}this.__iY.setEnabled(N);
},add:function(P){P.setState(w);

if(P.isAsynchronous()){this.__dz.push(P);
}else{this.__dz.unshift(P);
}this._check();
...
if(R){R.abort();
}else if(qx.lang.Array.contains(this.__dz,Q)){qx.lang.Array.remove(this.__dz,Q);
...
},__ro:function(){var bT=this.getRequest();
...
(function(){var s="qx.event.type.Event",r="String",q="qx.debug",p="qx.ioRemoteDebug",o="failed",n="timeout",m="created",l="aborted",k="sending",j="configured",d="receiving",i="completed",g="Object",c="Boolean",b="abstract",f="_applyState",e="GET",h="changeState",a="qx.io.remote.transport.Abstract";
...
},abort:function(){if(qx.core.Environment.get(q)){if(qx.core.Environment.get(p)){this.warn("Aborting...");
}}this.setState(l);
},timeout:function(){if(qx.core.Environment.get(q)){if(qx.core.Environment.get(p)){this.warn("Timeout...");
}}this.setState(n);
},failed:function(){if(qx.core.Environment.get(q)){if(qx.core.Environment.get(p)){this.warn("Failed...");
}}this.setState(o);
...
},_applyState:function(w,x){if(qx.core.Environment.get(q)){if(qx.core.Environment.get(p)){this.debug("State: "+w);
}}
...
}this.__rp=qx.bom.Iframe.create({id:L,name:L,src:N});
qx.bom.element.Style.set(this.__rp,s,A);
this.__rq=qx.bom.Element.create(n,{id:M,name:M,target:L});
qx.bom.element.Style.set(this.__rq,s,A);
qx.dom.Element.insertEnd(this.__rq,qx.dom.Node.getBodyElement(document));
this.__cN=qx.bom.Element.create(J,{id:I,name:I});
qx.dom.Element.insertEnd(this.__cN,this.__rq);
qx.dom.Element.insertEnd(this.__rp,qx.dom.Node.getBodyElement(document));
qx.event.Registration.addListener(this.__rp,H,this._onload,this);
...
},_numericMap:{"uninitialized":1,"loading":2,"loaded":2,"interactive":3,"complete":4}},members:{__cN:null,__rs:0,__rq:null,__rp:null,__rr:null,send:function(){var P=this.getMethod();
var R=this.getUrl();
...
return;
}while(this.__rs<W){this.setState(qx.io.remote.Exchange._nativeMap[++this.__rs]);
...
try{return bd&&bd.length>0?qx.lang.Json.parse(bd):null;
...
},destruct:function(){if(this.__rp){qx.event.Registration.removeListener(this.__rp,H,this._onload,this);
qx.bom.Event.removeNativeListener(this.__rp,E,this.__rr);
if((qx.core.Environment.get(g)==t)){this.__rp.src=qx.util.ResourceManager.getInstance().toUri(y);
...
var v=++qx.io.remote.transport.Script.__ru;

if(v>=2000000000){qx.io.remote.transport.Script.__ru=v=1;
}this.__eN=null;
this.__ru=v;
},statics:{__ru:0,_instanceRegistry:{},ScriptTransport_PREFIX:e,ScriptTransport_ID_PARAM:h,ScriptTransport_DATA_PARAM:g,handles:{synchronous:false,asynchronous:true,crossDomain:true,fileUpload:false,programaticFormFields:false,responseTypes:[o,m,q]},isSupported:function(){return true;
...
}})},members:{__rs:0,__eN:null,__ru:null,send:function(){var A=this.getUrl();
A+=(A.indexOf(l)>=0?t:l)+qx.io.remote.transport.Script.ScriptTransport_ID_PARAM+s+this.__ru;
...
this.__eN=document.createElement(b);
this.__eN.charset=j;
this.__eN.src=A;
...
return;
}while(this.__rs<E){this.setState(qx.io.remote.Exchange._nativeMap[++this.__rs]);
...
}},properties:{parseJson:{check:r,init:true}},members:{__rv:false,__rs:0,__rw:null,getRequest:function(){if(this.__rw===null){this.__rw=qx.io.remote.transport.XmlHttp.createRequestObject();
...
if(bn==4){if(!qx.io.remote.Exchange.wasSuccessful(this.getStatusCode(),bn,this.__rv)){if(this.getState()===N){this.setState(c);
}this.failed();
return;
}}if(bn==3&&this.__rs==1){this.setState(qx.io.remote.Exchange._nativeMap[++this.__rs]);
}while(this.__rs<bn){this.setState(qx.io.remote.Exchange._nativeMap[++this.__rs]);
...
if(qx.io.remote.Exchange.wasSuccessful(bI,bJ,this.__rv)){try{bK=this.getRequest().responseXML;
...
if(this.getParseJson()){bO=qx.lang.Json.parse(bP);
bO=(bO===0?0:(bO||null));
}else{bO=bP;
}return bO;
}else{return null;
