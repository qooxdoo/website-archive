...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.bom.Location","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Location-qx.theme.Modern.1c383f3558b3.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="demobrowser.demo.bom.Location",d="qx.core.BaseInit",c="engine.name",b="os.name",a="engine.version";
qx.Class.define(d,{statics:{getApplication:function(){return this.__cU||null;
...
(function(){var p="px",o="moveable",n="demobrowser.demo.bom.Location",m="borderBottomWidth",k="list",j='#',h="css.boxmodel",g="mode",f="borderTopWidth",e="borderLeftWidth",b="a",d="div",c="borderRightWidth",a="border";
qx.Class.define(n,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);
var q=document.getElementById(k).getElementsByTagName(b);

for(var i=0,l=q.length;i<l;i++){q[i].onclick=demobrowser.demo.bom.Location.jump;
}}},statics:{num:function(r,s){return parseInt(qx.bom.element.Style.get(r,s,qx.bom.element.Style.COMPUTED_MODE,false),10);
},jump:function(event){if(!event){event=window.event;
}event.returnValue=false;

if(event.preventDefault){event.preventDefault();
}function t(y,z){return parseInt(qx.bom.element.Style.get(y,z,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var v=this.href;
var x=v.substr(v.indexOf(j)+1);
var u=document.getElementById(x);
var location=qx.bom.element.Location.get(u,document.getElementById(g).value);
var w=document.getElementById(o);
w.style.left=location.left+p;
w.style.top=location.top+p;

if(qx.core.Environment.get(h)==a){w.style.width=(location.right-location.left)+p;
w.style.height=(location.bottom-location.top)+p;
}else{w.style.width=(location.right-location.left-t(w,e)-t(w,c))+p;
w.style.height=(location.bottom-location.top-t(w,f)-t(w,m))+p;
}return false;
},randomize:function(){var A=document.body.getElementsByTagName(d);

for(var i=0,l=A.length;i<l;i++){if(A[i].id&&A[i].id!=o){A[i].style.margin=Math.round(Math.random()*10)+p;
A[i].style.padding=Math.round(Math.random()*10)+p;
A[i].style.borderWidth=Math.round(Math.random()*10)+p;
}}}}});
})();
