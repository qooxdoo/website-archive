...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Virtual_List-qx.theme.Classic.2ea159364cf8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
this.__Oc=new qx.ui.virtual.layer.WidgetCell(this);
this.getPane().addLayer(this.__Oc);
this.getPane().addListener(z,this._onResize,this);
this.__od=[];
this.__cM=[];
this.__oH=new qx.ui.virtual.selection.Row(this.getPane());
this.__oH.addListener(h,this._onChangeSelection,this);
this.__oH.attachMouseEvents();
this.__oH.attachKeyEvents(this);
this.__oH.attachListEvents(this);
...
this.fireDataEvent(h,this.__Og(e.getData()));
},getCellWidget:function(I,J){var L=this.__od[I];

if(!L){return null;
}var K=this.__cM.pop()||new qx.ui.form.ListItem();
K.set({label:L.getLabel(),icon:L.getIcon()});

if(this.__oH.isItemSelected(I)){K.addState(n);
...
},hasChildren:function(){return this.__od.length>0;
...
qx.ui.core.queue.Widget.add(this);
},add:function(W){this.__od.push(W);
this._addHelper(W);
return this;
},remove:function(X){qx.lang.Array.remove(this.__od,X);
this._removeHelper(X);
return this;
},removeAll:function(){for(var i=0,j=this.__od.length;i<j;i++){this._removeHelper(this.__od[i]);
}this.__od=[];
},indexOf:function(Y){return this.__od.indexOf(Y);
},addAt:function(ba,bb){qx.lang.Array.insertAt(this.__od,ba,bb);
this._addHelper(ba);
},addBefore:function(bc,bd){qx.lang.Array.insertBefore(this.__od,bc,bd);
this._addHelper(bc);
},addAfter:function(be,bf){qx.lang.Array.insertAfter(this.__od,be,bf);
this._addHelper(be);
},removeAt:function(bg){this.__od.splice(bg,1);
this._removeHelper(this.__od[bg]);
},__Od:function(bh){var bi=this.__od.indexOf(bh);

if(bi<0){return null;
}else{return bi;
}},__Oe:function(bj){var bk=this.__od[bj];
return bk;
},__Of:function(bl){var bm=[];

for(var i=0;i<bl.length;i++){bm.push(this.__Od(bl[i]));
}return bm;
},__Og:function(bn){var bo=[];

for(var i=0;i<bn.length;i++){bo.push(this.__od[bn[i]]);
}return bo;
},selectAll:function(){this.__oH.selectAll();
},setSelected:function(bp){this.__oH.selectItem(this.__Od(bp));
},isSelected:function(bq){return this.__oH.isItemSelected(this.__Od(bq));
},addToSelection:function(br){this.__oH.addItem(this.__Od(br));
},removeFromSelection:function(bs){this.__oH.removeItem(this.__Od(bs));
},selectRange:function(bt,bu){this.__oH.selectItemRange(this.__Od(bt),this.__Od(bu));
},resetSelection:function(){this.__oH.clearSelection();
},setSelection:function(bv){this.__oH.replaceSelection(this.__Of(bv));
},getSelectedItem:function(){return this.__od[this.getSelected()];
},getSelected:function(){return this.__od[this.__oH.getSelectedItem()];
},getSelection:function(){return this.__Og(this.__oH.getSelection());
},getSortedSelection:function(){return this.__Og(this.__oH.getSortedSelection());
},isSelectionEmpty:function(){return this.__oH.isSelectionEmpty();
},getSelectionContext:function(){return this.__oH.getSelectionContext();
},getSelectables:function(){return this.__Og(this.__oH.getSelectables());
},_applySelectionMode:function(bw,bx){this.__oH.setMode(bw);
},_applyDragSelection:function(by,bz){this.__oH.setDrag(by);
},_applyQuickSelection:function(bA,bB){this.__oH.setQuick(bA);
}}});
})();
