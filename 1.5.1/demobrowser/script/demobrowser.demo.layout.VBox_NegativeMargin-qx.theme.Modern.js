...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.layout.VBox_NegativeMargin","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.VBox_NegativeMargin-qx.theme.Modern.2d41381c4955.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="demobrowser.demo.layout.VBox_NegativeMargin",b="os.name",a="engine.version";
...
(function(){var k="_applyLayoutChange",j="west",h="Integer",g="auto",f="center",e="y",d="Decorator",c="north",b="top",a="east",x="edge",w="south",v="flex",u="left",t="x",s="Boolean",r="height",q="The property '",p="qx.debug",o="width",m="' is not supported by the Dock layout!",n="qx.ui.layout.Dock",l="_applySort";
qx.Class.define(n,{extend:qx.ui.layout.Abstract,construct:function(y,z,A,B){qx.ui.layout.Abstract.call(this);
...
}},properties:{sort:{check:[g,e,t],init:g,apply:l},separatorX:{check:d,nullable:true,apply:k},separatorY:{check:d,nullable:true,apply:k},connectSeparators:{check:s,init:false,apply:k},spacingX:{check:h,init:0,apply:k},spacingY:{check:h,init:0,apply:k}},members:{__fa:null,__AI:null,verifyLayoutProperty:qx.core.Environment.select(p,{"true":function(C,name,D){this.assertInArray(name,[v,x,r,o],q+name+m);

if(name===x){this.assertInArray(D,[c,w,j,a,f]);
}else if(name===v){this.assertNumber(D);
this.assert(D>=0);
}else{this.assertMatch(D,qx.ui.layout.Util.PERCENT_VALUE);
}},"false":null}),_applySort:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__BO:{north:1,south:2,west:3,east:4,center:5},__BP:{1:"top",2:"bottom",3:"left",4:"right"},__lc:function(){var E=this._getLayoutChildren();
var M,G;
var length=E.length;
var I=[];
var L=[];
var J=[];
var F=this.getSort()===e;
var K=this.getSort()===t;

for(var i=0;i<length;i++){M=E[i];
J=M.getLayoutProperties().edge;

if(J===f){if(G){throw new Error("It is not allowed to have more than one child aligned to 'center'!");
}G=M;
}else if(K||F){if(J===c||J===w){F?I.push(M):L.push(M);
}else if(J===j||J===a){F?L.push(M):I.push(M);
}}else{I.push(M);
}}var N=I.concat(L);
...
for(var i=0;i<length;i++){J=N[i].getLayoutProperties().edge;
H[i]=this.__BO[J]||5;
}this.__AI=H;
delete this._invalidChildrenCache;
},renderLayout:function(O,P){if(this._invalidChildrenCache){this.__lc();
}var bj=qx.ui.layout.Util;
var V=this.__fa;
var bk=this.__AI;
var length=V.length;
var Y,R,X,be,bf,bp,bc,S,bm;
var bt=[];
var bb=[];
var bs=this._getSeparatorWidths();
var bx=this.getSpacingX();
var bw=this.getSpacingY();
var W=-bx;
var bl=-bw;
...
be=R.getLayoutProperties();
X=R.getSizeHint();
bc=X.width;
S=X.height;

if(be.width!=null){bc=Math.floor(O*parseFloat(be.width)/100);

if(bc<X.minWidth){bc=X.minWidth;
}else if(bc>X.maxWidth){bc=X.maxWidth;
}}
if(be.height!=null){S=Math.floor(P*parseFloat(be.height)/100);

if(S<X.minHeight){S=X.minHeight;
}else if(S>X.maxHeight){S=X.maxHeight;
}}bt[i]=bc;
bb[i]=S;
switch(bk[i]){case 1:case 2:bl+=S+R.getMarginTop()+R.getMarginBottom()+bw;

if(bs.y){bl+=bs.y+bw;
}break;
case 3:case 4:W+=bc+R.getMarginLeft()+R.getMarginRight()+bx;

if(bs.x){W+=bs.x+bx;
}break;
default:W+=bc+R.getMarginLeft()+R.getMarginRight()+bx;
bl+=S+R.getMarginTop()+R.getMarginBottom()+bw;
...
switch(bk[i]){case 3:case 4:case 5:bf=R.getLayoutProperties().flex;
if(bf==null&&bk[i]==5){bf=1;
}
if(bf>0){X=R.getSizeHint();
Y[i]={min:X.minWidth,value:bt[i],max:X.maxWidth,flex:bf};
}}}var T=bj.computeFlexOffsets(Y,O,W);

for(var i in T){bm=T[i].offset;
bt[i]+=bm;
W+=bm;
}}if(bl!=P){Y={};
bp=bl<P;
...
switch(bk[i]){case 1:case 2:case 5:bf=R.getLayoutProperties().flex;
if(bf==null&&bk[i]==5){bf=1;
}
if(bf>0){X=R.getSizeHint();
Y[i]={min:X.minHeight,value:bb[i],max:X.maxHeight,flex:bf};
}}}var T=bj.computeFlexOffsets(Y,P,bl);

for(var i in T){bm=T[i].offset;
bb[i]+=bm;
bl+=bm;
}}this._clearSeparators();
var bq=this.getSeparatorX(),br=this.getSeparatorY();
var bu=this.getConnectSeparators();
var bh=0,bo=0;
var bB,top,bc,S,bg,by;
var bi,bv,bA,Q;
var bz,ba,bd,U;
var bn=this.__BP;
...
X=R.getSizeHint();
bz=R.getMarginTop();
ba=R.getMarginBottom();
bd=R.getMarginLeft();
U=R.getMarginRight();
switch(by){case 1:case 2:bc=O-bd-U;
if(bc<X.minWidth){bc=X.minWidth;
}else if(bc>X.maxWidth){bc=X.maxWidth;
}S=bb[i];
top=bh+bj.computeVerticalAlignOffset(bn[by],S,P,bz,ba);
bB=bo+bj.computeHorizontalAlignOffset(R.getAlignX()||u,bc,O,bd,U);
if(bs.y){if(by==1){bv=bh+S+bz+bw+ba;
...
case 3:case 4:S=P-bz-ba;
if(S<X.minHeight){S=X.minHeight;
}else if(S>X.maxHeight){S=X.maxHeight;
}bc=bt[i];
bB=bo+bj.computeHorizontalAlignOffset(bn[by],bc,O,bd,U);
top=bh+bj.computeVerticalAlignOffset(R.getAlignY()||b,S,P,bz,ba);
if(bs.x){if(by==3){bi=bo+bc+bd+bx+U;
...
top=bh+bj.computeVerticalAlignOffset(R.getAlignY()||b,S,P,bz,ba);
}R.renderLayout(bB,top,bc,S);
}},_getSeparatorWidths:function(){var bD=this.getSeparatorX(),bC=this.getSeparatorY();

if(bD||bC){var bI=qx.theme.manager.Decoration.getInstance();
}
if(bD){var bJ=bI.resolve(bD);
var bF=bJ.getInsets();
var bG=bF.left+bF.right;
}
if(bC){var bK=bI.resolve(bC);
var bE=bK.getInsets();
var bH=bE.top+bE.bottom;
...
var bW=0,cg=0;
var bM=0,cb=0;
var cd=this._getSeparatorWidths();
var cf=this.getSpacingX(),ce=this.getSpacingY();
var bQ=-cf,bP=-ce;

if(cd.x){bQ-=cd.x+cf;
}
if(cd.y){bP-=cd.y+ce;
}for(var i=0;i<length;i++){ca=bO[i];
bS=ca.getSizeHint();
bU=ca.getMarginLeft()+ca.getMarginRight();
bT=ca.getMarginTop()+ca.getMarginBottom();
switch(bY[i]){case 1:case 2:bW=Math.max(bW,bS.width+bV+bU);
cg=Math.max(cg,bS.minWidth+ch+bU);
bM+=bS.height+bT;
cb+=bS.minHeight+bT;
bP+=ce;
...
cc=Math.max(cc,bS.minHeight+cb+bT);
bV+=bS.width+bU;
ch+=bS.minWidth+bU;
bQ+=cf;
...
if(cd.y){bP+=cd.y+ce;
}}}var bR=Math.max(ch,cg)+bQ;
var ci=Math.max(bV,bW)+bQ;
var bX=Math.max(cc,cb)+bP;
var bN=Math.max(bL,bM)+bP;
return {minWidth:bR,width:ci,minHeight:bX,height:bN};
...
this.setWidth(null);
this._editorGroups={};
var z=new qx.ui.decoration.Single();
z.setLeft(1,y,i);
this.setDecorator(z);
this.setBackgroundColor(m);
var B=new qx.ui.container.Composite().set({padding:[20,14]});
this.getChildControl(l).add(B);
this._pane=B;
var A=new qx.ui.layout.VBox().set({spacing:5});
B.setLayout(A);
B.add(this._createWidgetIndicator());
B.add(new qx.ui.basic.Label(g).set({font:k,padding:[1,0,5,0]}));
var C=qx.lang.Object.clone(demobrowser.demo.util.PropertyGroup.WIDGET_PROPERTIES);
C.value={type:x,nullable:true};
this._layoutControls=new demobrowser.demo.util.PropertyGroup(C);
B.add(this._layoutControls);
},properties:{widget:{check:r,apply:s}},members:{handleWidgetClick:function(e){var D=e.getTarget();
this.setWidget(D);
},_createWidgetIndicator:function(){var E=new qx.ui.container.Composite(new qx.ui.layout.VBox().set({spacing:5}));
E.add(this._createLabel(n));
this._widgetIndicator=new qx.ui.basic.Label().set({height:30,allowGrowX:true,allowGrowY:true,padding:5});
E._add(this._widgetIndicator);
return E;
},_createLabel:function(F){return new qx.ui.basic.Label(F).set({font:k,allowGrowX:true,padding:[5,0,5,0]});
},_createContainer:function(G,H){var I=new qx.ui.container.Composite(new qx.ui.layout.VBox().set({spacing:5}));
I.add(this._createLabel(G));
I.add(H);
return I;
},updateWidgetLayoutPropertyEditor:function(J){var L=J.getLayoutParent()?J.getLayoutParent()._getLayout():null;
var K=this.getWidgetLayoutPropertyEditor(L);

if(K){K.getChildren()[1].setSelected(J);
}
...
if(!K){this._wlpe=K;
return;
}this._pane.addAfter(K,this._layoutControls);
this._wlpe=K;
...
}var name=a+R.constructor.classname;
var S;

if(this._editorGroups[name]){return this._editorGroups[name];
}
switch(name){case f:S=new demobrowser.demo.util.PropertyGroup(demobrowser.demo.util.PropertyGroup.HBOX_PROPERTIES);
break;
case b:S=new demobrowser.demo.util.PropertyGroup(demobrowser.demo.util.PropertyGroup.DOCK_PROPERTIES);
break;
case o:S=new demobrowser.demo.util.PropertyGroup(demobrowser.demo.util.PropertyGroup.GRID_PROPERTIES);
break;
}
if(!S){return null;
}this._editorGroups[name]=this._createContainer(R.constructor.classname+q,S);
return this._editorGroups[name];
},_applyWidget:function(T,U){this._layoutControls.setSelected(T);
this._widgetIndicator.setBackgroundColor(T.getBackgroundColor());
this._widgetIndicator.setDecorator(T.getDecorator());
this._widgetIndicator.setValue(T.toString());
this.updateLayoutPropertyEditor(T);
this.updateWidgetLayoutPropertyEditor(T);
...
qx.Class.define(g,{extend:qx.ui.core.Widget,construct:function(s){qx.ui.core.Widget.call(this);
this._properties=s;
var u=new qx.ui.layout.Grid(10,5);
u.setColumnAlign(0,a,d);
u.setColumnMinWidth(0,110);
this._setLayout(u);
this.set({allowShrinkX:false,allowShrinkY:false});
var B=0;

for(var x in this._properties){var A=this._properties[x].type;
var w=this._properties[x].nullable;
this._add(new qx.ui.basic.Label(x).set({paddingTop:4}),{row:B,column:0});

if(w){var v=new qx.ui.form.CheckBox(h);
v.addListener(q,this._createOnNullPropertyChange(x),this);
this._add(v,{row:B,column:2});
this._properties[x].nullWidget=v;
}
if(A==n){var z=new qx.ui.form.Spinner().set({minimum:this._properties[x].min||0,maximum:this._properties[x].max!==undefined?this._properties[x].max:1000});
z.addListener(q,this._createOnIntPropertyChange(x),this);
this._add(z,{row:B++,column:1});
}else if(A==k){var z=new qx.ui.form.CheckBox();
z.addListener(q,this._createOnBoolPropertyChange(x),this);
this._add(z,{row:B++,column:1});
}else if(A==p){var y=this._properties[x].values;
var z=new qx.ui.form.RadioGroup();

for(var i=0;i<y.length;i++){var t=new qx.ui.form.RadioButton(y[i]);
t.setUserData(r,y[i]);
z.add(t);
this._add(t,{row:B++,column:1});
}z.addListener(b,this._createOnEnumPropertyChange(x,z),this);
}else if(A==m){var z=new qx.ui.form.TextField();
z.setLiveUpdate(true);
z.addListener(q,this._createOnIntPropertyChange(x),this);
this._add(z,{row:B++,column:1});
...
}var H=o+qx.lang.String.firstUp(name);
var G=this._properties[name].convert;

if(G){F=G(F);
}E[H](F);
},_getProperty:function(I,name){var J=f+qx.lang.String.firstUp(name);
return I[J]();
},_hasProperty:function(K,name){var L=o+qx.lang.String.firstUp(name);
return (typeof (K[L])==j);
},_createOnIntPropertyChange:function(M){return function(e){var N=this.getSelected();
this._setProperty(N,M,e.getTarget().getValue());
};
},_createOnBoolPropertyChange:function(O){return function(e){var P=this.getSelected();
this._setProperty(P,O,e.getTarget().getValue());
};
},_createOnEnumPropertyChange:function(Q,R){return function(e){var S=this.getSelected();
this._setProperty(S,Q,R.getSelection()[0].getUserData(r));
};
},_createOnNullPropertyChange:function(T){return function(e){var V=this.getSelected();
var U=e.getTarget();

if(U.getValue()){this._setProperty(V,T,null);
this._properties[T].formItem.setEnabled(false);
}else{var W=null;

if(this._properties[T].type==p){W=this._properties[T].formItem.getSelection()[0].getUserData(r);
}else{W=this._properties[T].formItem.getValue();
}this._setProperty(V,T,W);
this._properties[T].formItem.setEnabled(true);
}};
},_updateControls:function(X){for(var be in this._properties){var bf=this._properties[be].type;
var bb=this._properties[be].formItem;
var Y=this._properties[be].nullable;

if(!this._hasProperty(X,be)){bb.setEnabled(false);

if(Y){this._properties[be].nullWidget.setEnabled(false);
}return;
}bb.setEnabled(true);
var bd=this._getProperty(X,be);

if(bd!==null){if(bf==n){bb.setValue(parseInt(bd));
}else if(bf==m){bb.setValue(bd.toString());
}else if(bf==k){bb.setValue(!!bd);
}else if(bf==p){var ba=bb.getItems();

for(var i=0,l=ba.length;i<l;i++){var bc=ba[i];

if(bc.getUserData(r)==bd){bb.setSelection([bc]);
break;
}}}}
if(Y){this._properties[be].nullWidget.setValue(bd==null);
this._properties[be].nullWidget.setEnabled(true);
bb.setEnabled(bd!==null);
...
(function(){var j="Boolean",h="qx.ui.core.SingleSelectionManager",g="__nY",f="qx.debug",e="__oa",d="__nX",c="Invalid selectionProvider!",b="changeSelected",a="qx.event.type.Data";
qx.Class.define(h,{extend:qx.core.Object,construct:function(k){qx.core.Object.call(this);

if(qx.core.Environment.get(f)){qx.core.Assert.assertInterface(k,qx.ui.core.ISingleSelectionProvider,c);
}this.__nX=k;
},events:{"changeSelected":a},properties:{allowEmptySelection:{check:j,init:true,apply:e}},members:{__nY:null,__nX:null,getSelected:function(){return this.__nY;
},setSelected:function(l){if(!this.__oc(l)){throw new Error("Could not select "+l+", because it is not a child element!");
}this.__ob(l);
},resetSelected:function(){this.__ob(null);
},isSelected:function(m){if(!this.__oc(m)){throw new Error("Could not check if "+m+" is selected,"+" because it is not a child element!");
}return this.__nY===m;
},isSelectionEmpty:function(){return this.__nY==null;
},getSelectables:function(n){var o=this.__nX.getItems();
var p=[];

for(var i=0;i<o.length;i++){if(this.__nX.isItemSelectable(o[i])){p.push(o[i]);
}}if(!n){for(var i=p.length-1;i>=0;i--){if(!p[i].getEnabled()){p.splice(i,1);
}}}return p;
},__oa:function(q,r){if(!q){this.__ob(this.__nY);
}},__ob:function(s){var v=this.__nY;
var u=s;
...
},__oc:function(w){var x=this.__nX.getItems();
...
}},"edge":{type:"enum",values:["north","east","south","west","center"],nullable:false}},GRID_PROPERTIES:{"row":{type:"int",nullable:false},"column":{type:"int",nullable:false},"rowSpan":{type:"int",nullable:true},"colSpan":{type:"int",nullable:true}}},members:{_setProperty:function(j,name,k){var l=this._properties[name].convert;

if(l){k=l(k);
}var m={};
m[name]=k;
j.setLayoutProperties(m);
},_getProperty:function(n,name){return n.getLayoutProperties()[name]||null;
},_hasProperty:function(o,name){return true;
}}});
})();
(function(){var i="main",h="yellow",g="gray",f="blue",e="center",d="green",c="left",b="right",a="demobrowser.demo.layout.VBox_NegativeMargin";
qx.Class.define(a,{extend:demobrowser.demo.util.LayoutApplication,members:{main:function(){demobrowser.demo.util.LayoutApplication.prototype.main.call(this);
var scroll=new qx.ui.container.Scroll();
this.getRoot().add(scroll,{edge:0});
var j=new qx.ui.container.Composite(new qx.ui.layout.HBox(20)).set({padding:20});
scroll.add(j);
j.add(this.getBox1());
j.add(this.getBox2());
j.add(this.getBox3());
...
j.add(this.getBox6());
j.add(this.getBox7());
},getBox1:function(){var k=new qx.ui.layout.VBox();
var m=new qx.ui.container.Composite(k).set({decorator:i,backgroundColor:h,width:140,allowGrowY:false});
var o=new qx.ui.core.Widget().set({decorator:i,backgroundColor:f,maxWidth:100,alignX:c});
var n=new qx.ui.core.Widget().set({decorator:i,backgroundColor:d,maxWidth:100,alignX:e});
var l=new qx.ui.core.Widget().set({decorator:i,backgroundColor:g,maxWidth:100,alignX:b});
m.add(o);
m.add(n);
m.add(l);
n.setMarginTop(-10);
l.setMarginTop(-10);
return m;
},getBox2:function(){var p=new qx.ui.layout.VBox();
var r=new qx.ui.container.Composite(p).set({decorator:i,backgroundColor:h,width:140,allowGrowY:false});
var t=new qx.ui.core.Widget().set({decorator:i,backgroundColor:f,maxWidth:100,alignX:c});
var s=new qx.ui.core.Widget().set({decorator:i,backgroundColor:d,maxWidth:100,alignX:e});
var q=new qx.ui.core.Widget().set({decorator:i,backgroundColor:g,maxWidth:100,alignX:b});
r.add(t);
r.add(s);
r.add(q);
s.setMarginTop(-10);
s.setMarginBottom(20);
q.setMarginTop(-10);
return r;
},getBox3:function(){var u=new qx.ui.layout.VBox();
var w=new qx.ui.container.Composite(u).set({decorator:i,backgroundColor:h,width:140,height:300,allowGrowY:false});
var y=new qx.ui.core.Widget().set({decorator:i,backgroundColor:f,maxWidth:100,alignX:c});
var x=new qx.ui.core.Widget().set({decorator:i,backgroundColor:d,maxWidth:100,alignX:e});
var v=new qx.ui.core.Widget().set({decorator:i,backgroundColor:g,maxWidth:100,alignX:b});
w.add(y,{flex:1});
w.add(x,{flex:1});
w.add(v,{flex:1});
x.setMarginTop(-10);
v.setMarginTop(-10);
return w;
},getBox4:function(){var z=new qx.ui.layout.VBox();
var B=new qx.ui.container.Composite(z).set({decorator:i,backgroundColor:h,width:140,height:300,allowGrowY:false});
var D=new qx.ui.core.Widget().set({decorator:i,backgroundColor:f,maxWidth:100,alignX:c});
var C=new qx.ui.core.Widget().set({decorator:i,backgroundColor:d,maxWidth:100,alignX:e});
var A=new qx.ui.core.Widget().set({decorator:i,backgroundColor:g,maxWidth:100,alignX:b});
B.add(D,{flex:1});
B.add(C,{flex:2});
B.add(A,{flex:3});
C.setMarginTop(-10);
A.setMarginTop(-10);
return B;
},getBox5:function(){var E=new qx.ui.layout.VBox();
var G=new qx.ui.container.Composite(E).set({decorator:i,backgroundColor:h,width:140,height:300,allowGrowY:false});
var I=new qx.ui.core.Widget().set({decorator:i,backgroundColor:f,maxWidth:100,alignX:c});
var H=new qx.ui.core.Widget().set({decorator:i,backgroundColor:d,maxWidth:100,alignX:e});
var F=new qx.ui.core.Widget().set({decorator:i,backgroundColor:g,maxWidth:100,alignX:b});
G.add(I,{flex:1});
G.add(H,{flex:2});
G.add(F,{flex:3});
H.setMarginTop(-10);
...
F.setHeight(0);
return G;
},getBox6:function(){var J=new qx.ui.layout.VBox();
var L=new qx.ui.container.Composite(J).set({decorator:i,backgroundColor:h,width:140,height:100,allowGrowY:false});
var N=new qx.ui.core.Widget().set({decorator:i,backgroundColor:f,maxWidth:100,alignX:c});
var M=new qx.ui.core.Widget().set({decorator:i,backgroundColor:d,maxWidth:100,alignX:e});
var K=new qx.ui.core.Widget().set({decorator:i,backgroundColor:g,maxWidth:100,alignX:b});
L.add(N,{flex:1});
L.add(M,{flex:1});
L.add(K,{flex:1});
M.setMarginTop(-10);
K.setMarginTop(-10);
return L;
},getBox7:function(){var O=new qx.ui.layout.VBox();
var Q=new qx.ui.container.Composite(O).set({decorator:i,backgroundColor:h,width:140,height:100,allowGrowY:false});
var S=new qx.ui.core.Widget().set({decorator:i,backgroundColor:f,maxWidth:100,alignX:c});
var R=new qx.ui.core.Widget().set({decorator:i,backgroundColor:d,maxWidth:100,alignX:e});
var P=new qx.ui.core.Widget().set({decorator:i,backgroundColor:g,maxWidth:100,alignX:b});
Q.add(S,{flex:1});
Q.add(R,{flex:2});
Q.add(P,{flex:3});
R.setMarginTop(-10);
