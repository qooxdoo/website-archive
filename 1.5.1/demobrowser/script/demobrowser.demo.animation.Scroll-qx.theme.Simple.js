...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.animation.Scroll-qx.theme.Simple.030558328d08.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:d,check:[d,b]},x:{init:0,check:c},y:{init:0,check:c}},members:{__KL:null,__KM:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var e=this._getElement();
this.__KL={x:e.scrollLeft,y:e.scrollTop};
if(this._atEndPosition(this.__KL.x,this.__KL.y)){return;
}
if(this.getMode()==b){this.__KM={left:this.getX()-this.__KL.x,top:this.getY()-this.__KL.y};
}},update:function(f){qx.fx.Base.prototype.update.call(this);
var g=this._getElement();

if(this.getMode()==d){if(this.getX()!=0){g.scrollLeft=this.__KL.x+(this.getX()*f);
}
if(this.getY()!=0){g.scrollTop=this.__KL.y+(this.getY()*f);
}}else{g.scrollLeft=this.__KL.x+(this.__KM.left*f);
g.scrollTop=this.__KL.y+(this.__KM.top*f);
