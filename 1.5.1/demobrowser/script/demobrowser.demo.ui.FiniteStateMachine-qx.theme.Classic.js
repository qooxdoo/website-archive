...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.FiniteStateMachine","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine-qx.theme.Classic.e4175d69ec65.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="os.name",b="engine.version",a="demobrowser.demo.ui.FiniteStateMachine";
...
(function(){var h="String",g="Number",f="string",d="__Kw",c="__Kx",b="qx.util.fsm.FiniteStateMachine";
...
if(!this.__KB[u]){this.__KB[u]={};
}this.__KB[u][q]=true;
if(!this.__KC[q]){this.__KC[q]=[];
}this.__KC[q]=this.__KC[q].concat(s);
}},removeObject:function(w){var y=this.__Kz[w];
if(this.__KC[w]){for(var x in this.__KC[w]){delete this.__KB[x];
}delete this.__KC[w];
}delete this.__KA[y];
delete this.__Kz[w];
delete this.__Ky[w];
},getObject:function(z){return this.__Ky[z];
},getFriendlyName:function(A){var B=A?A.toHashCode():null;
return B?this.__KA[B]:null;
},getGroupObjects:function(C){var a=[];

for(var name in this.__KB[C]){a.push(name);
}return a;
},displayAllObjects:function(){for(var D in this.__Kz){var F=this.__Kz[D];
var E=this.getObject(D);
this.debug(D+" => "+F);
this.debug("  "+F+" => "+this.__KA[F]);
this.debug("  "+D+" => "+this.getObject(D));
this.debug("  "+this.getObject(D)+" => "+this.getFriendlyName(E));
}},start:function(){var H=this.__Ku;
...
}this.__ie[H].getAutoActionsBeforeOnentry()(this);
if(G){this.debug(this.getName()+"#"+H+"#entry");
}this.__ie[H].getOnentry()(this,null);
if(G){this.debug(this.getName()+"#"+H+"#actionsAfterOnentry");
}this.__ie[H].getAutoActionsAfterOnentry()(this);
},pushState:function(I){if(this.__Kv.length>=this.getMaxSavedStates()){throw new Error("Saved-state stack is full");
}
if(I===true){this.__Kv.push(this.getState());
}else if(I){this.__Kv.push(I);
}else{this.__Kv.push(this.getPreviousState());
}},popState:function(){if(this.__Kv.length==0){throw new Error("Saved-state stack is empty");
}return this.__Kv.pop();
},postponeEvent:function(event){this.__Kx.unshift(event);
},enqueueEvent:function(event,J){if(J){this.__Kw.push(event);
}else{this.__Kw.unshift(event);
}
...
},__KE:function(){if(this.__KD){return ;
}this.__KD=true;
while(this.__Kw.length>0){var event=this.__Kw.pop();
var S=this.__KF(event);
if(S){event.dispose();
}}this.__KD=false;
},__KF:function(event){var V;
var X;
var U;
var bg;
var be;
...
if(typeof (X)=="string"){if(!X in this.__ie){throw new Error("Attempt to transition to nonexistent state "+X);
}this.setNextState(X);
}else{switch(X){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:X=V;
this.setNextState(X);
break;
case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Kv.length==0){throw new Error("Attempt to transition to POP_STATE_STACK "+"while state stack is empty.");
}X=this.__Kv.pop();
this.setNextState(X);
break;
...
}bg=this.__ie[this.getNextState()];
...
for(var i=0;i<this.__Kx.length;i++){e=this.__Kx.pop();
this.__Kw.unshift(e);
}
...
this.__Kv=this.__ie=null;
}});
})();
(function(){var p="context",o="string",n="object",m="function",l="number",k="autoActionsAfterOnentry",h='"',d="autoActionsBeforeOnexit",c="autoActionsBeforeOnentry",b="}",bb="{",ba="undefined",Y="autoActionsAfterOnexit",X="(",W=";",V="  var groupObjects = ",U="__Ks",T="__Kr",S="parameters",R=")",w=" fsm.getObject('",x="groups",u="  for (var i = 0; i < groupObjects.length; i++)",v="__Kl",s="fsm",t="');",q="onexit",r="__Kq",y="__Kp",z="  fsm.debug(ex);",G="    fsm.getObject(objName).",E=")(fsm)",K="events",I="    var objName = groupObjects[i];",N="').",M="objects",B="__Ko",Q="__Km",P="catch(ex)",O="  {",A="    fsm.getGroupObjects('",C="__Kn",D="  }",F="qx.util.fsm.State",H="onentry",J="try",L=",";
qx.Class.define(F,{extend:qx.core.Object,construct:function(bc,bd){var bf;
qx.core.Object.call(this);
this.setName(bc);
if(typeof (bd)!=n){throw new Error("State info must be an object");
}bf=bd.context||window;
this.setUserData(p,bf);
for(var be in bd){switch(be){case H:this.setOnentry(this.__Kt(bd[be],bf));
break;
case q:this.setOnexit(this.__Kt(bd[be],bf));
break;
case c:this.setAutoActionsBeforeOnentry(bd[be]);
break;
case k:this.setAutoActionsAfterOnentry(bd[be]);
break;
case d:this.setAutoActionsBeforeOnexit(bd[be]);
break;
case Y:this.setAutoActionsAfterOnexit(bd[be]);
break;
case K:this.setEvents(bd[be]);
break;
case p:break;
default:this.setUserData(be,bd[be]);
this.debug("State "+bc+": "+"Adding user-provided field to state: "+be);
break;
}}if(!this.getEvents()){throw new Error("The events object must be provided in new state info");
}this.transitions={};
},statics:{_commonTransformAutoActions:function(bg,bh,bi){if(typeof (bh)!=n){throw new Error("Invalid "+bg+" value: "+typeof (bh));
}var bn;
var bo=J+bb;
var bl;
var bj;
for(var f in bh){var bk=bh[f];
if(!bk instanceof Array){throw new Error("Invalid function request type: "+"expected array, found "+typeof (bk));
}for(var i=0;i<bk.length;i++){bj=bk[i];
if(typeof (bj)!=n){throw new Error("Invalid function request parameter type: "+"expected object, found "+typeof (bk[bl]));
}var bm=bj[S];
if(!bm){bm=[];
}else{if(!bm instanceof Array){throw new Error("Invalid function parameters: "+"expected array, found "+typeof (bm));
}}bn=f+X;
for(var j=0;j<bm.length;j++){if(j!=0){bn+=L;
}
if(typeof (bm[j])==m){bn+=X+bm[j]+E;
}else if(typeof (bm[j])==o){bn+=h+bm[j]+h;
}else{bn+=bm[j];
}}bn+=R;
var a=bj[M];
if(!a){a=[];
...
for(j=0;j<g.length;j++){bo+=V+A+g[j]+t+u+O+I+G+bn+W+D;
}}}}bo+=b+P+bb+z+b;
return qx.lang.Function.bind(new Function(s,bo),bi);
}},properties:{name:{transform:v,nullable:true},onentry:{transform:Q,nullable:true,init:function(bp,event){}},onexit:{transform:C,nullable:true,init:function(bq,event){}},autoActionsBeforeOnentry:{transform:y,nullable:true,init:function(br,event){}},autoActionsAfterOnentry:{transform:r,nullable:true,init:function(bs,event){}},autoActionsBeforeOnexit:{transform:T,nullable:true,init:function(bt,event){}},autoActionsAfterOnexit:{transform:U,nullable:true,init:function(bu,event){}},events:{transform:B,nullable:true}},members:{__Kl:function(bv){if(typeof (bv)!=o||bv.length<1){throw new Error("Invalid state name");
}return bv;
},__Km:function(bw){switch(typeof (bw)){case ba:return function(bx,event){};
case m:return qx.lang.Function.bind(bw,this.getUserData(p));
default:throw new Error("Invalid onentry type: "+typeof (bw));
}},__Kn:function(by){switch(typeof (by)){case ba:return function(bz,event){};
case m:return qx.lang.Function.bind(by,this.getUserData(p));
default:throw new Error("Invalid onexit type: "+typeof (by));
return null;
}},__Ko:function(bA){if(typeof (bA)!=n){throw new Error("events must be an object");
}for(var e in bA){var bB=bA[e];

if(typeof (bB)==l&&bB!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bB!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error("Invalid numeric value in events object: "+e+": "+bB);
}else if(typeof (bB)==n){for(var bC in bB){if(typeof (bB[bC])==l&&bB[bC]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bB[bC]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error("Invalid numeric value in events object "+"("+e+"): "+bC+": "+bB[bC]);
}else if(typeof (bB[bC])!=o&&typeof (bB[bC])!=l){throw new Error("Invalid value in events object "+"("+e+"): "+bC+": "+bB[bC]);
}}}else if(typeof (bB)!=o&&typeof (bB)!=l){throw new Error("Invalid value in events object: "+e+": "+bA[e]);
}}return bA;
},__Kp:function(bD){return qx.util.fsm.State._commonTransformAutoActions(c,bD,this.getUserData(p));
},__Kq:function(bE){return qx.util.fsm.State._commonTransformAutoActions(k,bE,this.getUserData(p));
},__Kr:function(bF){return qx.util.fsm.State._commonTransformAutoActions(d,bF,this.getUserData(p));
},__Ks:function(bG){return qx.util.fsm.State._commonTransformAutoActions(Y,bG,this.getUserData(p));
},__Kt:function(f,bH){if(typeof (f)==m){f=qx.lang.Function.bind(f,bH);
}return f;
},addTransition:function(bI){if(!bI instanceof qx.util.fsm.Transition){throw new Error("Invalid transition: not an instance of "+"qx.util.fsm.Transition");
}this.transitions[bI.getName()]=bI;
}}});
})();
(function(){var o="context",n="__KI",m="predicate",l="__KH",k="nextState",j="autoActionsBeforeOntransition",i="__KJ",h="function",g="__KG",e="__KK",b="String",d="autoActionsAfterOntransition",c="qx.util.fsm.Transition",a="ontransition";
qx.Class.define(c,{extend:qx.core.Object,construct:function(p,q){var s;
qx.core.Object.call(this);
this.setName(p);
s=q.context||window;
this.setUserData(o,s);
for(var r in q){switch(r){case m:this.setPredicate(this.__Kt(q[r],s));
break;
case k:this.setNextState(q[r]);
break;
case j:this.setAutoActionsBeforeOntransition(this.__Kt(q[r],s));
break;
case d:this.setAutoActionsAfterOntransition(this.__Kt(q[r],s));
break;
case a:this.setOntransition(this.__Kt(q[r],s));
break;
case o:break;
...
},transform:g},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:l},autoActionsBeforeOntransition:{init:function(u,event){},transform:n},autoActionsAfterOntransition:{init:function(v,event){},transform:i},ontransition:{init:function(w,event){},transform:e}},members:{__KG:function(x){switch(typeof (x)){case "undefined":return function(y,event){return true;
};
case "boolean":return function(z,event){return x;
};
case "function":return qx.lang.Function.bind(x,this.getUserData("context"));
default:throw new Error("Invalid transition predicate type: "+typeof (x));
break;
}},__KH:function(A){switch(typeof (A)){case "string":return A;
...
break;
}},__KI:function(B){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOntransition",B,this.getUserData("context"));
},__KJ:function(C){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOntransition",C,this.getUserData("context"));
},__KK:function(D){switch(typeof (D)){case "undefined":return function(E,event){};
case "function":return qx.lang.Function.bind(D,this.getUserData("context"));
default:throw new Error("Invalid ontransition type: "+typeof (D));
break;
}},__Kt:function(f,F){if(typeof (f)==h){f=qx.lang.Function.bind(f,F);
}return f;
}}});
})();
