...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Remote_Model","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Remote_Model-qx.theme.Modern.33fa78459d54.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="demobrowser.demo.table.Table_Remote_Model",b="os.name",a="engine.version";
...
(function(){var f="icon/22/actions/view-refresh.png",e='90%',d="demobrowser.demo.table.Table_Remote_Model",c='10%',b='Reload',a='execute';
qx.Class.define(d,{extend:demobrowser.demo.table.TableDemo,members:{createTable:function(){var g=this._tableModel=new demobrowser.demo.table.RemoteTableModel();
var j={tableColumnModel:function(k){return new qx.ui.table.columnmodel.Resize(k);
}};
var h=new qx.ui.table.Table(g,j);
var i=h.getTableColumnModel().getBehavior();
i.setWidth(0,c);
i.setWidth(1,e);
return h;
},createControls:function(){var n=new qx.ui.toolbar.ToolBar();
var m=new qx.ui.toolbar.Part();
n.add(m);
var l=new qx.ui.toolbar.Button(b,f);
l.addListener(a,function(){this._tableModel.reloadData();
},this);
m.add(l);
return n;
}}});
...
this.__on=null;
this.__om=null;
},properties:{blockSize:{check:c,init:50},maxCachedBlockCount:{check:c,init:15},clearCacheOnRemove:{check:b,init:false},blockConcurrentLoadRowCount:{check:b,init:true}},members:{__JI:null,__JN:null,__JJ:null,__JK:null,__JL:null,__JM:null,__JO:null,__JP:null,__ok:null,__oo:null,__om:null,__on:null,__JQ:false,_getIgnoreCurrentRequest:function(){return this.__JN;
},getRowCount:function(){if(this.__JI==-1){if(!this.__JQ||!this.getBlockConcurrentLoadRowCount()){this.__JQ=true;
this._loadRowCount();
}return (this.__JI==-1)?0:this.__JI;
}else{return this.__JI;
}},_loadRowCount:function(){throw new Error("_loadRowCount is abstract");
},_onRowCountLoaded:function(f){if(this.getBlockConcurrentLoadRowCount()){this.__JQ=false;
}if(f==null||f<0){f=0;
}this.__JI=Number(f);
...
if(this.__JK!=-1){var h=this._cancelCurrentRequest();
...
this.__JQ=true;
this._loadRowCount();
},clearCache:function(){this.__JO={};
this.__JP=0;
},getCacheContent:function(){return {sortColumnIndex:this.__ok,sortAscending:this.__oo,rowCount:this.__JI,lruCounter:this.__JJ,rowBlockCache:this.__JO,rowBlockCount:this.__JP};
},restoreCacheContent:function(j){if(this.__JK!=-1){var k=this._cancelCurrentRequest();

if(k){this.__JK=-1;
this.__JN=false;
}else{this.__JN=true;
}}this.__ok=j.sortColumnIndex;
this.__oo=j.sortAscending;
this.__JI=j.rowCount;
this.__JJ=j.lruCounter;
this.__JO=j.rowBlockCache;
this.__JP=j.rowBlockCount;
var l={firstRow:0,lastRow:this.__JI-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
for(var w=0;w<=o;w++){var q=this.__JO[w];
...
if(r!=null){u[t]=r;
}}}}},prefetchRows:function(x,y){if(this.__JK==-1){var z=this.getBlockSize();
var E=Math.ceil(this.__JI/z);
...
for(var F=D;F<=C;F++){if(this.__JO[F]==null||this.__JO[F].isDirty){if(B==-1){B=F;
}A=F;
}}if(B!=-1){this.__JL=-1;
this.__JM=-1;
this.__JK=B;
...
},_onRowDataLoaded:function(I){if(I!=null&&!this.__JN){var L=this.getBlockSize();
var J=Math.ceil(I.length/L);

if(J==1){this._setRowBlockData(this.__JK,I);
}else{for(var i=0;i<J;i++){var O=i*L;
var N=[];
var K=Math.min(L,I.length-O);

for(var P=0;P<K;P++){N.push(I[O+P]);
}this._setRowBlockData(this.__JK+i,N);
}}var M={firstRow:this.__JK*L,lastRow:(this.__JK+J+1)*L-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(e,M);
}this.__JK=-1;
this.__JN=false;
if(this.__JL!=-1){this.prefetchRows(this.__JL,this.__JM);
}},_setRowBlockData:function(Q,R){if(this.__JO[Q]==null){this.__JP++;

while(this.__JP>this.getMaxCachedBlockCount()){var V;
var U=this.__JJ;

for(var T in this.__JO){var S=this.__JO[T].lru;

if(S<U&&T>1){U=S;
V=T;
}}delete this.__JO[V];
this.__JP--;
}}this.__JO[Q]={lru:++this.__JJ,rowDataArr:R};
...
for(var bf=bc;bf<=bb;bf++){var X=this.__JO[bf];
...
if(bf==bb-1){if(X.rowDataArr.length==0){delete this.__JO[bf];
}}else{var bd=this.__JO[bf+1];

if(bd!=null){X.rowDataArr.push(bd.rowDataArr[0]);
}else{X.isDirty=true;
}}}}
if(this.__JI!=-1){this.__JI--;
...
if(bi.lru!=this.__JJ){bi.lru=++this.__JJ;
}return bj;
...
}}},setEditable:function(bv){this.__om=[];

for(var bw=0;bw<this.getColumnCount();bw++){this.__om[bw]=bv;
}this.fireEvent(d);
},setColumnEditable:function(bx,by){if(by!=this.isColumnEditable(bx)){if(this.__om==null){this.__om=[];
}this.__om[bx]=by;
this.fireEvent(d);
}},isColumnEditable:function(bz){return (this.__om?(this.__om[bz]==true):false);
},setColumnSortable:function(bA,bB){if(bB!=this.isColumnSortable(bA)){if(this.__on==null){this.__on=[];
}this.__on[bA]=bB;
this.fireEvent(d);
}},isColumnSortable:function(bC){return (this.__on?(this.__on[bC]!==false):true);
},sortByColumn:function(bD,bE){if(this.__ok!=bD||this.__oo!=bE){this.__ok=bD;
this.__oo=bE;
this.clearCache();
this.fireEvent(d);
}},getSortColumnIndex:function(){return this.__ok;
},isSortAscending:function(){return this.__oo;
},setSortColumnIndexWithoutSortingData:function(bF){this.__ok=bF;
},setSortAscendingWithoutSortingData:function(bG){this.__oo=bG;
}},destruct:function(){this.__on=this.__om=this.__JO=null;
}});
})();
(function(){var e="text",d="Text",c='demobrowser.demo.table.RemoteTableModel',b="Id",a="id";
qx.Class.define(c,{extend:qx.ui.table.model.Remote,construct:function(){qx.ui.table.model.Remote.call(this);
this.setColumns([b,d],[a,e]);
},members:{__NI:null,__NJ:false,_loadRowCount:function(){this.__NK();

if(this.__NJ){return;
}
if(this.__NI){this.__NL();
}else{this.__NO();
}},_loadRowData:function(f,g){this.__NK();

if(this.__NJ){return;
}
if(this.__NI){this.__NM(f,g);
}else{this.__NP(f,g);
}},__NK:function(){if(this.__NJ||this.__NI!==null){return;
}this.__NJ=true;
this.__NN("method=checkphp",function(h){this.__NJ=false;
this.__NI=(h=="WTF PHP");
this._loadRowCount();
});
},__NL:function(){var j="method=getRowCount";
this.__NN(j,function(k){this._onRowCountLoaded(parseInt(k));
});
},__NM:function(l,m){var n="method=getRowData&start="+l+"&end="+m;
this.__NN(n,function(o){this._onRowDataLoaded(qx.lang.Json.parse(o));
});
},__NN:function(p,q){var r=new qx.io.request.Xhr();
r.setUrl("../../resource/demobrowser/backend/remote_table.php?"+p);
r.addListener("success",function(){q.call(this,r.getResponseText());
},this);
r.send();
},__NO:function(){var self=this;
window.setTimeout(function(){self._onRowCountLoaded(1000000);
},0);
},__NP:function(s,t){var self=this;
window.setTimeout(function(){var u=[];

for(var i=s;i<=t;i++){u.push({id:i,text:'Hello '+i+' Generated on:'+(new Date())});
