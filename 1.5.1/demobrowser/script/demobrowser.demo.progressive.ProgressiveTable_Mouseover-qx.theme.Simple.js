...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.progressive.ProgressiveTable_Mouseover","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.progressive.ProgressiveTable_Mouseover-qx.theme.Simple.6fffe68b12cb.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="os.name",b="engine.version",a="demobrowser.demo.progressive.ProgressiveTable_Mouseover";
...
(function(){var l=".qx-progressive-",k="-col-",j="px;",h="-row",g="}",f="width: ",d="0px ",c="qx-progressive-",b="px",a="  font-size: 11px;",X="content",W="px ",V='',U="end",T="div",S="  position: absolute;",R="qx.tableResizeDebug",Q="  width : 100%;",P="__columnData",O="  border-bottom:1px solid #eeeeee;",s="",t="css.boxmodel",q="  top: 0px;",r="qx.debug",o="  overflow:hidden;",p=" {",m="  cursor:default;",n="  font-family: 'Segoe UI', Corbel, Calibri, Tahoma, 'Lucida Sans Unicode', sans-serif;",u="__IN",v="-row {",C="resize",A="start",G="relative",E="  -o-text-overflow: ellipsis;",K="mshtml",I="  border-right:1px solid #f2f2f2;",x="padding: ",N="  white-space:nowrap;",M=';-moz-user-select:none;',L="  padding : 0px 6px 0px 6px;",w="qx.ui.progressive.renderer.table.Row",y="progressive-table-row-background-odd",z="  text-overflow:ellipsis;",B="  height: 100%;",D="engine.name",F="progressive-table-row-background-even",H="__oB",J="left: ";
qx.Class.define(w,{extend:qx.ui.progressive.renderer.Abstract,construct:function(Y){qx.ui.progressive.renderer.Abstract.call(this);
this.__Jo=Y;
this.__Kf={};
this.__IN=new qx.ui.progressive.renderer.table.cell.Default();
this.__Jn=null;
this.__Kg={};
var ba=qx.theme.manager.Color.getInstance();
this.__Kg.bgcol=[];
this.__Kg.bgcol[0]=ba.resolve(F);
this.__Kg.bgcol[1]=ba.resolve(y);
this.__oB=new qx.ui.layout.HBox();
this.__oB.connectToWidget(this);
},statics:{__op:null,__Kh:6,__Ki:S+q+B+o+z+E+N+I+O+L+m+a+n+((qx.core.Environment.get(D)==K)?V:M)},properties:{defaultRowHeight:{init:16}},members:{__Jn:null,__Az:null,__Kj:null,__Jo:null,__Kf:null,__IN:null,__Kg:null,__oB:null,join:function(bb,name){if(this.__Jn){throw new Error("Renderer is already joined to a Progressive.");
}this.__Jn=bb;
this.__Az=name;
var bd=qx.ui.progressive.renderer.table.Row;

if(!bd.__op){bd.__op={};
}var bc=bb.toHashCode();

if(!bd.__op[bc]){bd.__op[bc]={rowstylesheet:null,cellstylesheet:[]};
var be=l+bc+v+Q+g;
bd.__op[bc].rowstylesheet=qx.bom.Stylesheet.createElement(be);
var bf=this.__Jo.getData();

for(var i=0;i<bf.length;i++){var be=l+bc+k+i+p+bd.__Ki+g;
bd.__op[bc].cellstylesheet[i]=qx.bom.Stylesheet.createElement(be);
}this.__Kj=bc;
var bg=bb.getStructure().getPane();
bg.addListener(C,this._resizeColumns,this);
}},addRenderer:function(bh,bi){var bj=this.__Jo.getData();

if(bh<0||bh>=bj.length){throw new Error("Column "+bh+" out of range (max: "+(bj.length-1)+")");
}this.__Kf[bh]=bi;
},removeRenderer:function(bk){var bl=this.__Jo.getData();

if(bk<0||bk>=bl.length){throw new Error("Column "+bk+" out of range (max: "+(bl.length-1)+")");
}
if(!this.__Kf[bk]){throw new Error("No existing renderer for column "+bk);
}delete this.__Kf[bk];
},render:function(bm,bn){var bx=bn.data;
var bs=[];
var bu;
var bt;
var bq=0;
if(bm.getRendererData()[this.__Az].end===undefined){bm.getRendererData()[this.__Az]={end:0,start:1,rows:0,totalHeight:0};
}var bw=document.createElement(T);
for(var i=0;i<bx.length;i++){var br=c+this.__Kj+k+i;
bt=this.__Kf[i]||this.__IN;
bu={state:bm,rowDiv:bw,stylesheet:br,element:bn,dataIndex:i,cellData:bx[i],height:bq};
bs.push(bt.render(bu));
if(bu.height>bq){bq=bu.height;
}}bq=(bq>0?bq:this.getDefaultRowHeight());
var bv=bm.getRendererData()[this.__Az];
bv.totalHeight+=bq;
bw.style.position=G;
bw.style.height=bq+b;
bw.className=c+this.__Kj+h;
bw.innerHTML=bs.join(s);
switch(bn.location){case U:var bo=bv.end||0;
bw.style.backgroundColor=this.__Kg.bgcol[bo];
bv.end=(bo==0?1:0);
bm.getPane().getContentElement().getDomElement().appendChild(bw);
break;
case A:var bp=bm.getPane().getContentElement().getDomElement();
var by=bp.childNodes;
if(by.length>0){var bo=bv.start;
bw.style.backgroundColor=this.__Kg.bgcol[bo];
bv.start=(bo==0?1:0);
bp.insertBefore(bw,by[0]);
break;
}else{bp.appendChild(bw);
}break;
default:throw new Error("Invalid location: "+bn.location);
}++bv.rows;
},getLayoutChildren:function(){return this.__Jo.getData();
},_resizeColumns:function(e){var bG=this.__Jn.getStructure().getPane();
var bD=bG.getBounds().width-qx.bom.element.Overflow.getScrollbarWidth();
var bE=l+this.__Kj+h;
var bA=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bA.__op[this.__Kj].rowstylesheet,bE);
var bC=f+bD+j;
qx.bom.Stylesheet.addRule(bA.__op[this.__Kj].rowstylesheet,bE,bC);
this.__oB.renderLayout(bD,100);
var bF=this.__Jo.getData();
for(var i=0,bJ=0;i<bF.length;i++,bJ+=bD){var bE=l+this.__Kj+k+i;
var bA=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bA.__op[this.__Kj].cellstylesheet[i],bE);
bD=bF[i].getComputedWidth();

if(qx.core.Environment.get(r)){if(qx.core.Environment.get(R)){this.debug("col "+i+": width="+bD);
}}var bB;

if(qx.core.Environment.get(t)==X){bB=qx.ui.progressive.renderer.table.Row.__Kh*2;
}else{bB=-1;
}var bI=(bD-bB)+j;
var bH=d+qx.ui.progressive.renderer.table.Row.__Kh+W+d+qx.ui.progressive.renderer.table.Row.__Kh+j;
var bz=bJ+j;
var bC=bA.__Ki+f+bI+J+bz+x+bH;
qx.bom.Stylesheet.addRule(bA.__op[this.__Kj].cellstylesheet[i],bE,bC);
}}},destruct:function(){var name;

for(name in this.__Kf){this.__Kf[name]=null;
}var bL=qx.ui.progressive.renderer.table.Row;
var bK=this.__Jn.toHashCode();

if(bL.__op&&bL.__op[bK]){if(bL.__op[bK].rowstylesheet){var bN=l+this.__Kj+h;
var bL=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bL.__op[this.__Kj].rowstylesheet,bN);
}if(bL.__op[bK].cellstylesheet){for(var i=bL.__op[bK].cellstylesheet.length-1;i>=0;i--){var bN=l+this.__Kj+k+i;
var bM=bL.__op[this.__Kj].cellstylesheet[i];
var bL=qx.ui.progressive.renderer.table.Row;
qx.bom.Stylesheet.removeRule(bM,bN);
}}}
if(this.__Jn&&this.__Jn.getRendererData){var bO=this.__Jn.getRendererData();

if(bO&&bO[this.__Az]&&bO[this.__Az].end!==undefined){bO[this.__Az]=null;
}}this.__Kg=this.__Kf=this.__Jn=this.__Jo=null;
this._disposeObjects(H,u,P);
}});
})();
...
qx.Class.define(a,{extend:qx.ui.progressive.renderer.table.cell.Default,statics:{onmouseover:function(K){K.__NE=qx.bom.element.Style.get(K,"color");
K.style.color=q;
...
},onmouseout:function(N){N.style.color=N.__NE;
...
for(var i=0;i<e;i++){this.__LG[i]=new qx.ui.core.ColumnData();
}},members:{__LG:null,getData:function(){return this.__LG;
...
}}},setWidth:function(j,k){if(j>this.__LG.length-1||j<0){throw new Error("Column number out of range");
}this.__LG[j].setColumnWidth(k);
},setMinWidth:function(l,m){if(l>this.__LG.length-1||l<0){throw new Error("Column number out of range");
}this.__LG[l].setMinWidth(m);
},setMaxWidth:function(n,o){if(n>this.__LG.length-1||n<0){throw new Error("Column number out of range");
}this.__LG[n].setMaxWidth(o);
}}});
})();
...
},members:{__Jn:null,join:function(c){this.__Jn=c;
...
this.__oB=new qx.ui.layout.HBox();
this.__oB.connectToWidget(this);
},properties:{appearance:{refine:true,init:c}},members:{__Jo:null,__Jq:null,__Jp:null,__oB:null,join:function(m){qx.ui.progressive.headfoot.Abstract.prototype.join.call(this,m);
},getLayoutChildren:function(){if(this.__Jq){return this.__Jo.getData();
...
this.__Jq=true;
this.__oB.renderLayout(n,100);
this.__Jq=false;
var o=this.__Jo.getData();
for(var i=0;i<o.length;i++){this.__Jp[i].setWidth(o[i].getComputedWidth());
}}},destruct:function(){this.__Jo=this.__Jp=null;
this._disposeObjects(b);
}});
})();
(function(){var m="progressive-progressbar-indicator-undone",l="__KN",k="progress",j="%",i="progressive-progressbar-indicator-done",h="progressive-progressbar-percent-text",g="renderStart",f="progressive-progressbar-background",d="qx.ui.progressive.headfoot.Progress",c="__KO",a="progressive-progressbar-percent-background",b="renderEnd";
qx.Class.define(d,{extend:qx.ui.progressive.headfoot.Abstract,construct:function(n,o){qx.ui.progressive.headfoot.Abstract.call(this);
this.setHeight(16);
this.setPadding(0);
this.__Kg={};
var p=qx.theme.manager.Color.getInstance();
this.__Kg.background=p.resolve(f);
this.__Kg.indicatorDone=p.resolve(i);
this.__Kg.indicatorUndone=p.resolve(m);
this.__Kg.percentBackground=p.resolve(a);
this.__Kg.percentText=p.resolve(h);
this.set({backgroundColor:this.__Kg.background});
this.__KN=new qx.ui.core.Widget();
this.__KN.set({width:0,backgroundColor:this.__Kg.indicatorDone});
this.add(this.__KN);
var q=new qx.ui.core.Widget();
q.set({backgroundColor:this.__Kg.indicatorUndone});
this.add(q,{flex:1});
this.__KO=new qx.ui.basic.Atom("0%");
this.__KO.set({width:100,backgroundColor:this.__Kg.percentBackground,textColor:this.__Kg.percentText});
this.add(this.__KO);
this.exclude();
},members:{__KP:null,__Kg:null,__KN:null,__KO:null,join:function(r){qx.ui.progressive.headfoot.Abstract.prototype.join.call(this,r);
r.addListener(g,function(e){this.__KP=e.getData().initial;
this.show();
},this);
r.addListener(k,function(e){var u=1.0-(e.getData().remaining/this.__KP);
var v=this.getBounds();

if(v){var s=Math.floor((v.width-this.__KO.getBounds().width)*u);
var t=Math.floor(u*100)+j;

if(!isNaN(s)){this.__KN.setMinWidth(s);
this.__KO.setLabel(t);
}}},this);
r.addListener(b,function(e){this.exclude();
},this);
}},destruct:function(){this.__Kg=null;
this._disposeObjects(l,c);
}});
})();
...
if(!e){this.__Ex=new qx.ui.core.Widget();
this.__zg=this.__Ex;
}else{this.__Ex=null;
this.__zg=e;
}this.__zg.getContentElement().setStyle(b,a);
},members:{__Ex:null,__zg:null,applyStructure:function(f){throw new Error("applyStructure() is abstract");
},getPane:function(){return this.__zg;
}},destruct:function(){if(this.__Ex){this.__Ex.dispose();
}this.__Ex=this.__zg=null;
}});
})();
...
if(!b){this.__Ka=new qx.ui.progressive.headfoot.Null();
this.__pG=this.__Ka;
}else{this.__Ka=null;
this.__pG=b;
}if(!c){this.__Kb=new qx.ui.progressive.headfoot.Null();
this.__Kc=this.__Kb;
}else{this.__Kb=null;
this.__Kc=c;
}},members:{__pG:null,__Kc:null,__Ka:null,__Kb:null,applyStructure:function(e){this.__pG.join(e);
this.__Kc.join(e);
e.add(this.__pG);
e.add(this.getPane(),{flex:1});
e.add(this.__Kc);
},getHeader:function(){return this.__pG;
},getFooter:function(){return this.__Kc;
}},destruct:function(){if(this.__Ka){this.__Ka.dispose();
this.__Ka=null;
}
if(this.__Kb){this.__Kb.dispose();
this.__Kb=null;
}this.__pG=this.__Kc=null;
}});
})();
...
this.__JT=false;
this.__JU=0;
},events:{"renderStart":k,"renderEnd":k,"progress":k,"progressDetail":k},properties:{dataModel:{check:c,apply:f},batchSize:{check:g,init:20},flushWidgetQueueAfterBatch:{check:d,init:false},interElementTimeout:{check:g,init:0}},members:{__EQ:null,__JT:null,__JV:null,__JU:null,__JS:null,__JR:null,getStructure:function(){return this.__JR;
},addRenderer:function(name,m){this.__EQ[name]=m;
m.join(this,name);
},removeRenderer:function(name){if(!this.__EQ[name]){throw new Error("No existing renderer named "+name);
}delete this.__EQ[name];
},render:function(){if(this.__JT){return;
}this.__JT=true;
var n=new qx.ui.progressive.State({progressive:this,model:this.getDataModel(),pane:this.__JR.getPane(),batchSize:this.getBatchSize(),rendererData:this.__JX(),userData:{}});
this.__JV=new Date();
if(this.__JS){this.__JU=n.getModel().getElementCount();
this.fireDataEvent(h,{state:n,initial:this.__JU});
this.__JW(n);
}else{qx.event.Timer.once(function(){this.__JU=n.getModel().getElementCount();
this.fireDataEvent(h,{state:n,initial:this.__JU});
this.__JW(n);
this.__JS=true;
},this,10);
}},_applyDataModel:function(o,p){if(p){p.removeListener(j,this.__JY,this);
p.dispose();
}o.addListener(j,this.__JY,this);
},__JW:function(q){var r;
var t;
var s;

for(var i=q.getBatchSize();i>0;i--){r=q.getModel().getNextElement();

if(!r){this.debug("Render time: "+(new Date()-this.__JV)+"ms");
this.__JT=false;
this.fireDataEvent("renderEnd",q);
q.dispose();
return ;
}t=r.element;
s=this.__EQ[t.renderer];
s.render(q,t);
this.fireDataEvent("progressDetail",{initial:this.__JU,remaining:r.remaining,element:t});
}this.fireDataEvent("progress",{initial:this.__JU,remaining:r.remaining});
if(this.getFlushWidgetQueueAfterBatch()){qx.ui.core.queue.Manager.flush();
}qx.event.Timer.once(function(){this.__JW(q);
},this,this.getInterElementTimeout());
},__JX:function(){var u={};

for(var name in this.__EQ){u[name]={};
}return u;
},__JY:function(e){this.__JU=e.getData();
this.render();
}},destruct:function(){for(var name in this.__EQ){this.__EQ[name].dispose();
}this.__JV=this.__EQ=this.__JR=null;
}});
})();
...
this.__kX=[];
},members:{__kX:null,addElements:function(c){this.__kX=this.__kX.concat(c);
this.fireDataEvent(b,this.__kX.length);
},addElement:function(d){this.__kX.push(d);
this.fireDataEvent(b,this.__kX.length);
},getElementCount:function(){return this.__kX.length;
},getNextElement:function(){if(this.__kX.length>0){return ({element:this.__kX.shift(),remaining:this.__kX.length});
}return null;
