...
s.addListener(b,this.__ox,this);
this._add(s);
break;
...
},__ox:function(){var w=this.getChildrenContainer().getChildren();
...
(function(){var u="keypress",t="interval",s="keydown",r="mousedown",q="keyup",p="blur",o="Enter",n="__nv",m="__oc",l="Up",d="Escape",k="event.touch",h="qx.ui.menu.Manager",c="Left",b="__od",g="Down",f="Right",j="singleton",a="Space";
...
}this.__oc=new qx.event.Timer;
this.__oc.addListener(t,this._onOpenInterval,this);
this.__od=new qx.event.Timer;
this.__od.addListener(t,this._onCloseInterval,this);
},members:{__oe:null,__of:null,__oc:null,__od:null,__nv:null,_getChild:function(x,y,z,A){var B=x.getChildren();
var length=B.length;
var C;
...
},add:function(F){var G=this.__nv;
G.push(F);
F.setZIndex(1e6+G.length);
},remove:function(H){var I=this.__nv;

if(I){qx.lang.Array.remove(I,H);
}},hideAll:function(){var J=this.__nv;

if(J){for(var i=J.length-1;i>=0;i--){J[i].exclude();
}}},getActiveMenu:function(){var K=this.__nv;
return K.length>0?K[K.length-1]:null;
},scheduleOpen:function(L){this.cancelClose(L);
if(L.isVisible()){if(this.__oe){this.cancelOpen(this.__oe);
}}else if(this.__oe!=L){this.__oe=L;
this.__oc.restartWith(L.getOpenInterval());
}},scheduleClose:function(M){this.cancelOpen(M);
if(!M.isVisible()){if(this.__of){this.cancelClose(this.__of);
}}else if(this.__of!=M){this.__of=M;
this.__od.restartWith(M.getCloseInterval());
}},cancelOpen:function(N){if(this.__oe==N){this.__oc.stop();
this.__oe=null;
}},cancelClose:function(O){if(this.__of==O){this.__od.stop();
this.__of=null;
}},_onOpenInterval:function(e){this.__oc.stop();
this.__oe.open();
this.__oe=null;
},_onCloseInterval:function(e){this.__od.stop();
this.__of.exclude();
this.__of=null;
...
}if(this.__nv.length>0&&!this._isInMenu(P)){this.hideAll();
}},__og:{"Enter":1,"Space":1},__ld:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var Q=this.getActiveMenu();

if(!Q){return;
}var R=e.getKeyIdentifier();

if(this.__ld[R]||(this.__og[R]&&Q.getSelectedButton())){e.stopPropagation();
...
}else if(this.__os.length>0){do{var M=this.__os[0];
...
var K=false;
if(this.__os.length==1&&I>0){var D=L-this.getSpacing();
var H=B-I+F+D;
K=A>H;
}if(A>B+F+L||K){this.__ou(M);
B+=F;
if(C&&this.__os.length==0){C.setVisibility(v);
}}else{return;
}}}while(A>=B&&this.__os.length>0);
}},__ou:function(N){N.setVisibility(w);
this.__os.shift();
this.fireDataEvent(p,N);
},__ov:function(O){if(!O){return;
}this.__os.unshift(O);
O.setVisibility(v);
this.fireDataEvent(j,O);
},_getNextToHide:function(){for(var i=this.__ot.length-1;i>=0;i--){var P=this.__ot[i];
...
}}},setRemovePriority:function(S,T,U){if(!U&&this.__ot[T]!=undefined){throw new Error("Priority already in use!");
}this.__ot[T]=S;
...
}for(var i=0;i<this.__os.length;i++){this.__os[i].setVisibility(w);
}this.__os=[];
...
}},__ow:false,_setAllowMenuOpenHover:function(bc){this.__ow=bc;
},_isAllowMenuOpenHover:function(){return this.__ow;
...
qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:c,init:0,apply:b},spanColumn:{check:c,init:1,nullable:true,apply:b},iconColumnWidth:{check:c,init:0,themeable:true,apply:b},arrowColumnWidth:{check:c,init:0,themeable:true,apply:b}},members:{__om:null,_computeSizeHint:function(){var q=this._getLayoutChildren();
var o,g,j;
var e=this.getSpanColumn();
var h=this.__om=[0,0,0,0];
...
},getColumnSizes:function(){return this.__om||null;
...
},__ok:function(){if(this.isVisible()){if(this.getBlockBackground()){var R=this.getZIndex();
...
},__ol:function(){var bj=this._getMenuBounds();

if(!bj){this.addListenerOnce(j,this.__ol,this);
return;
...
if(this.__oj){this.__oj.call(this);
delete this.__oj;
...
if(this.__oi){bo.cancelOpen(this.__oi);
this.__oi=null;
...
}if(this.__oi){br.cancelOpen(this.__oi);
...
(function(){var i="Integer",h="hovered",g="hover-button",f="interval",d="mouseover",c="mouseout",b="qx.ui.form.HoverButton",a="__iz";
...
this.__iz=new qx.event.AcceleratingTimer();
this.__iz.addListener(f,this._onInterval,this);
},properties:{appearance:{refine:true,init:g},interval:{check:i,init:80},firstInterval:{check:i,init:200},minTimer:{check:i,init:20},timerDecrease:{check:i,init:2}},members:{__iz:null,_onMouseOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;
}this.__iz.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();
this.addState(h);
},_onMouseOut:function(e){this.__iz.stop();
this.removeState(h);
...
}},__on:function(t){while(!(t instanceof qx.ui.menu.Menu)){t=t.getLayoutParent();
}return t;
...
this.addListener(s,this.__mN,this);
},properties:{appearance:{refine:true,init:n}},members:{__zg:null,_createChildControlImpl:function(B,C){var D;
...
},__mN:function(e){var F=e.getData()[0];
...
this.__zr();
},__zq:function(){var H=this.getChildControl(k).getSelection()[0];
var I=this.getChildControl(j);
var G=H?H.getIcon():a;
G==null?I.resetIcon():I.setIcon(G);
},__zr:function(){var L=this.getChildControl(k).getSelection()[0];
...
if(S==z||S==w){if(this.__zg){this.setSelection([this.__zg]);
this.__zg=null;
}this.toggle();
...
},_onListMouseDown:function(e){if(this.__zg){this.setSelection([this.__zg]);
this.__zg=null;
}},_onListChangeSelection:function(e){var U=e.getData();
var X=e.getOldData();
if(X&&X.length>0){X[0].removeListener(c,this.__zq,this);
X[0].removeListener(d,this.__zr,this);
}
if(U.length>0){var W=this.getChildControl(g);
var V=this.getChildControl(k);
var Y=V.getSelectionContext();

if(W.isVisible()&&(Y==l||Y==v)){this.__zg=U[0];
}else{this.setSelection([U[0]]);
this.__zg=null;
}U[0].addListener(c,this.__zq,this);
U[0].addListener(d,this.__zr,this);
