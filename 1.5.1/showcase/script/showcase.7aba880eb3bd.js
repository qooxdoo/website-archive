...
(function(){var r="Boolean",q="changeInvalidMessage",p="changeValue",o="String",n="_applyAllowEmptySelection",m="_applyInvalidMessage",k="qx.ui.form.RadioGroup",j="_applyValid",h="",g="changeRequired",c="changeValid",f="changeEnabled",d="changeSelection",b="_applyEnabled",a="__mM";
...
}},remove:function(w){var x=this.__mM;
...
}},_applyInvalidMessage:function(z,A){for(var i=0;i<this.__mM.length;i++){this.__mM[i].setInvalidMessage(z);
}},_applyValid:function(B,C){for(var i=0;i<this.__mM.length;i++){this.__mM[i].setValid(B);
}},_applyEnabled:function(D,E){var F=this.__mM;
...
},_isItemSelectable:function(O){return this.__mM.indexOf(O)!=-1;
},__mN:function(e){var Q=e.getData()[0];
...
},events:{"execute":R},properties:{enabled:{init:true,check:l,event:o,apply:I},shortcut:{check:n,apply:Q,nullable:true},autoRepeat:{check:l,init:false}},members:{__yW:g,__yX:g,execute:function(U){this.fireDataEvent(L,U);
},__yY:function(event){if(this.getEnabled()&&this.__za(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},__fE:function(event){if(this.getEnabled()&&this.__za(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},_applyEnabled:function(V,W){if(V){qx.event.Registration.addListener(document.documentElement,h,this.__yY,this);
qx.event.Registration.addListener(document.documentElement,d,this.__fE,this);
}else{qx.event.Registration.removeListener(document.documentElement,h,this.__yY,this);
qx.event.Registration.removeListener(document.documentElement,d,this.__fE,this);
}},_applyShortcut:function(X,Y){if(X){if(X.search(/[\s]+/)!=-1){var bc=y;
this.error(bc);
throw new Error(bc);
}this.__yW={"Control":false,"Shift":false,"Meta":false,"Alt":false};
this.__yX=null;
var ba;
var a=[];
...
for(var i=0;i<bb;i++){var bd=this.__zc(a[i]);

switch(bd){case f:case u:case K:case C:this.__yW[bd]=true;
break;
case m:var bc=F+a[i];
this.error(bc);
throw bc;
default:if(this.__yX){var bc=r;
this.error(bc);
throw bc;
}this.__yX=bd;
}}}return true;
},__za:function(e){var be=this.__yX;

if(!be){return ;
}if((!this.__yW.Shift&&e.isShiftPressed())||(this.__yW.Shift&&!e.isShiftPressed())||(!this.__yW.Control&&e.isCtrlPressed())||(this.__yW.Control&&!e.isCtrlPressed())||(!this.__yW.Meta&&e.isMetaPressed())||(this.__yW.Meta&&!e.isMetaPressed())||(!this.__yW.Alt&&e.isAltPressed())||(this.__yW.Alt&&!e.isAltPressed())){return false;
}
if(be==e.getKeyIdentifier()){return true;
}return false;
},__zb:{esc:S,ctrl:f,print:P,del:q,pageup:b,pagedown:E,numlock:O,numpad_0:J,numpad_1:D,numpad_2:t,numpad_3:s,numpad_4:A,numpad_5:N,numpad_6:B,numpad_7:H,numpad_8:M,numpad_9:z,numpad_divide:p,numpad_multiply:v,numpad_minus:c,numpad_plus:k},__zc:function(bf){var bg=qx.event.handler.Keyboard;
var bh=m;
...
var bh=this.__zb[bf]||qx.lang.String.firstUp(bf);

if(bg.isValidKeyIdentifier(bh)){return bh;
}else{return m;
}},toString:function(){var bk=this.__yX;
var bj=[];

for(var bi in this.__yW){if(this.__yW[bi]){bj.push(qx.locale.Key.getKeyName(j,bi));
}}
if(bk){bj.push(qx.locale.Key.getKeyName(j,bk));
}return bj.join(k);
}},destruct:function(){this.setEnabled(false);
this.__yW=this.__yX=null;
}});
})();
