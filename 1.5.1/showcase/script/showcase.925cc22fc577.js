...
this.__wB(s);
return t;
},__wB:function(u){if(!(g in document.documentElement.style)){return;
}var w=true;
...
this.__wD();
},__wC:function(){this.getView().addListener(d,this._onButtonPress,this);
},_onButtonPress:function(e){this.getModel().readToken(e.getData());
},__wD:function(){var w=this.getModel();
...
this.initState();
},properties:{state:{check:[j,h,g],event:r,init:j,apply:q},errorMessage:{check:D,init:o,event:k},input:{check:D,nullable:true,event:y},maxInputLength:{check:x,init:20},operator:{check:[b,f,C,a],nullable:true,event:m},operant:{check:e,nullable:true,event:t},value:{check:e,nullable:true,event:z},memory:{check:e,nullable:true,event:s}},members:{readToken:function(E){if(E.match(/^[0123456789]$/)){this.__wG(E);
}else if(E.match(/^[\+\-\*\/]$/)){this.__wJ(E);
}else if(E==v){this.__wH();
}else if(E==d){this.__wI();
}else if(E==l){this.__wK();
}else if(E==B){this.__wL();
}else if(E==c){this.__wM(E);
}else if(E==n){this.__wM(E);
}else if(E==w){this.__wO();
}else if(E==p){this.__wN();
}},__wE:function(){return parseFloat(this.getInput());
},__wF:function(F,G,H){switch(H){case b:return F+G;
case f:return F-G;
case C:return F*G;
...
}},__wG:function(K){this.setState(j);
var L=this.getInput();
...
},__wJ:function(Q){var S=this.getState();
...
if(T!==null){this.setValue(this.__wF(T,R,this.getOperator()));
}else{this.setValue(R);
}this.setOperant(R);
this.setOperator(Q);
},__wK:function(){var U=this.getOperator();

if(!U){return;
}var W=this.getValue();

if(this.getState()==h){this.setValue(this.__wF(W,this.getOperant(),U));
return;
}this.setState(h);
var V=this.__wE();
this.setOperant(V);
this.setValue(this.__wF(W,V,U));
},__wL:function(){this.setState(j);
this.setOperator(null);
this.setValue(null);
this.setInput(i);
},__wM:function(X){var Y=this.getState();
var ba;
...
}},__wN:function(){var bc=this.getMemory();

if(bc==null){return;
}this.setState(j);
this.setInput(bc.toString());
},__wO:function(){this.setMemory(null);
}}});
})();
