...
(function(){var l="move",k="Boolean",j="mouseup",i="mousedown",h="losecapture",g="qx.ui.core.MMovable",f="__pv",d="mousemove",c="maximized",b="__pw",a="move-frame";
qx.Mixin.define(g,{properties:{movable:{check:k,init:true},useMoveFrame:{check:k,init:false}},members:{__pv:null,__pw:null,__px:null,__py:null,__pz:null,__pA:null,__pB:null,__pC:false,__pD:null,__pE:0,_activateMoveHandle:function(m){if(this.__pv){throw new Error("The move handle could not be redefined!");
}this.__pv=m;
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isContentBlocked()){this.__pD=parent.getBlockerColor();
this.__pE=parent.getBlockerOpacity();
parent.setBlockerColor(null);
parent.setBlockerOpacity(1);
parent.blockContent(this.getZIndex()-1);
this.__pC=true;
}}this.__px={left:w.left,top:w.top,right:w.left+x.width,bottom:w.top+x.height};
var v=this.getContainerLocation();
this.__pA=w.left;
this.__pB=w.top;
this.__py=v.left-e.getDocumentLeft();
this.__pz=v.top-e.getDocumentTop();
this.addState(l);
this.__pv.capture();
if(this.getUseMoveFrame()){this.__pG();
}e.stop();
},_onMoveMouseMove:function(e){if(!this.hasState(l)){return;
}var y=this.__pH(e);

if(this.getUseMoveFrame()){this.__pF().setDomPosition(y.viewportLeft,y.viewportTop);
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__pC){parent.unblockContent();
parent.setBlockerColor(this.__pD);
parent.setBlockerOpacity(this.__pE);
this.__pD=null;
this.__pE=0;
this.__pC=false;
}}this.__pv.releaseCapture();
var z=this.__pH(e);
this.setLayoutProperties({left:z.parentLeft,top:z.parentTop});
if(this.getUseMoveFrame()){this.__pF().exclude();
}e.stopPropagation();
},__pI:function(e){if(!this.hasState(l)){return;
}this.removeState(l);
if(this.getUseMoveFrame()){this.__pF().exclude();
}}},destruct:function(){this._disposeObjects(b,f);
this.__px=null;
}});
})();
...
(function(){var b="qx.ui.window.Manager",a="__pu";
qx.Class.define(b,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__pu:null,setDesktop:function(c){this.__pu=c;
this.updateStack();
},getDesktop:function(){return this.__pu;
},changeActiveWindow:function(d,e){if(d){this.bringToFront(d);
d.setActive(true);
}
if(e){e.resetActive();
}},_minZIndex:1e5,updateStack:function(){qx.ui.core.queue.Widget.add(this);
},syncWidget:function(){this.__pu.forceUnblockContent();
var f=this.__pu.getWindows();
...
}}this.__pu.setActiveWindow(k);
},bringToFront:function(n){var o=this.__pu.getWindows();
var p=qx.lang.Array.remove(o,n);

if(p){o.push(n);
this.updateStack();
}},sendToBack:function(q){var r=this.__pu.getWindows();
...
A.addListener(x,this.__qa,this);
A.addListener(s,this.__qb,this);
...
this.removeState(w);
if(this.getUseResizeFrame()){this._getResizeFrame().exclude();
}},__qa:function(e){if(this.hasState(j)){var bh=this.__pR(e);
...
}}},__qb:function(e){if(this.getCursor()&&!this.hasState(j)){this.resetCursor();
this.getApplicationRoot().resetGlobalCursor();
...
this.__qd=bh.left===undefined?0:bh.left;
this.__qc=bh.top===undefined?0:bh.top;
...
this.__qd=bi.left===undefined?0:bi.left;
this.__qc=bi.top===undefined?0:bi.top;
this.removeState(b);
this.hide();
this.fireEvent(u);
