...
}},_removeBindingsFrom:function(T){var U=this.__yb(T);
...
if(qx.lang.Array.contains(this.__xX,T)){qx.lang.Array.remove(this.__xX,T);
}},__xY:function(W,X,Y){var ba=j+W+m;

if(Y==k){ba=i+W+m;
}
if(X!=null&&X!=n){ba+=a+X;
}return ba;
},__ya:function(bb,bc){var bd=this.__yb(bb);

if(!qx.lang.Array.contains(bd,bc)){bd.push(bc);
}
if(!qx.lang.Array.contains(this.__xX,bb)){this.__xX.push(bb);
}},__yb:function(be){var bf=be.getUserData(l);
...
return r;
},styleSelectabled:function(t){var u=this.__xJ(t);
this._styleSelectabled(u);
},styleUnselectabled:function(v){var w=this.__xJ(v);
...
}},_styleSelectabled:function(z){this.__xK(z,{selected:1});
},_styleUnselectabled:function(A){this.__xK(A,{});
...
},__xJ:function(D){return this._list._layer.getRenderedCellWidget(D,0);
},__xK:function(E,F){if(E==null){return;
...
this.__yP=new qx.ui.virtual.core.Pane(h,i,j,k);
this.__yP.addListener(f,this._computeScrollbars,this);
this.__yP.addListener(d,this._onScrollPaneX,this);
this.__yP.addListener(g,this._onScrollPaneY,this);

if(qx.core.Environment.get(b)){this._add(this.__yP,{edge:0});
...
},_createChildControlImpl:function(l,m){if(l==c){return this.__yP;
...
}},destruct:function(){this.__yP.dispose();
this.__yP=null;
}});
})();
...
},properties:{selection:{check:j,event:m,apply:f,nullable:false,deferredInit:true},selectionMode:{check:[n,h,a,l],init:n,apply:c},dragSelection:{check:p,init:false,apply:b},quickSelection:{check:p,init:false,apply:d}},members:{_manager:null,__yI:false,__yQ:false,_initSelectionManager:function(){var self=this;
...
},_onChangeSelection:function(e){if(this.__yQ==true){return;
}this.__yI=true;
...
},_onManagerChangeSelection:function(e){if(this.__yI==true){return;
}this.__yQ=true;
this.__yw();
this.__yQ=false;
},__yw:function(){if(this.__yS()){return;
}var I=this._manager.getSelection();
var J=[];

for(var i=0;i<I.length;i++){J.push(this._getDataFromRow(I[i]));
}this.__yR(J);
},__yR:function(K){var L=this.getSelection();

if(K.length>0){var M=[0,L.getLength()];
M=M.concat(K);
L.splice.apply(L,M);
}else{L.removeAll();
}},__yS:function(){var O=this.getSelection();
...
}},_getLookupTable:function(){return this.__xL;
},_lookup:function(L){return this.__xL[L];
},_lookupGroup:function(M){return this.__xM.indexOf(M);
},_reverseLookup:function(N){if(N<0){return -1;
}return this.__xL.indexOf(N);
...
},_onModelChange:function(e){this.__xS();
this._applyDefaultSelection();
},__xR:function(){this.getPane().getRowConfig().setItemCount(this.__xL.length);
this.getPane().fullUpdate();
},__xS:function(){this.__xL=[];
this.__xM=[];
this.__xN={};
...
for(var i=0,l=bj.length;i<l;++i){if(bk==null||bk(bj.getItem(i))){this.__xL.push(i);
}}},_runDelegateSorter:function(bl){if(this.__xL.length==0){return;
}var bm=qx.util.Delegate.getMethod(this.getDelegate(),z);

if(bm!=null){this.__xL.sort(function(a,b){return bm(bl.getItem(a),bl.getItem(b));
});
}},_runDelegateGroup:function(bn){var br=qx.util.Delegate.getMethod(this.getDelegate(),j);

if(br!=null){for(var i=0,l=this.__xL.length;i<l;++i){var bo=this.__xL[i];
var bq=this.getModel().getItem(bo);
var bp=br(bq);
this.__xT(bp,bo);
}this.__xL=this.__xU();
}},__xT:function(bs,bt){if(bs==null){this.__xQ=true;
bs=v;
}var name=this.__xV(bs);

if(this.__xN[name]==null){this.__xN[name]=[];

if(this.isAutoGrouping()){this.getGroups().push(bs);
}}this.__xN[name].push(bt);
...
},__xV:function(bA){var name=null;
...
},__xW:function(){if(this.__xP&&this.__xQ||this.__xP&&this.__xO){throw new Error("GroupingTypeError: You can't mix 'Objects' and 'Strings' as"+" group identifier!");
}}},destruct:function(){this._background.dispose();
this._provider.dispose();
this._layer.dispose();
this._background=this._provider=this._layer=this.__xL=this.__xM=this.__xN=null;
}});
})();
...
this.__hd={};
},properties:{anonymous:{refine:true,init:true}},members:{__hd:null,__cs:null,__yc:null,__yd:null,__ye:null,__om:null,getFirstRow:function(){return this.__yc;
},getFirstColumn:function(){return this.__yd;
},getRowSizes:function(){return this.__ye||[];
},getColumnSizes:function(){return this.__om||[];
},syncWidget:function(){if(!this.getContentElement().getDomElement()){return;
}
if(this.__hd.fullUpdate||this.__hd.updateLayerWindow&&this.__hd.updateLayerData){this._fullUpdate.apply(this,this.__cs);
}else if(this.__hd.updateLayerWindow){this._updateLayerWindow.apply(this,this.__cs);
}else if(this.__hd.updateLayerData&&this.__ye){this._updateLayerData();
}
if(this.__hd.fullUpdate||this.__hd.updateLayerWindow){var c=this.__cs;
this.__yc=c[0];
this.__yd=c[1];
this.__ye=c[2];
this.__om=c[3];
}this.__hd={};
},_updateLayerData:function(){this._fullUpdate(this.__yc,this.__yd,this.__ye,this.__om);
...
},updateLayerData:function(){this.__hd.updateLayerData=true;
qx.ui.core.queue.Widget.add(this);
},fullUpdate:function(l,m,n,o){this.__cs=arguments;
this.__hd.fullUpdate=true;
qx.ui.core.queue.Widget.add(this);
},updateLayerWindow:function(p,q,r,s){this.__cs=arguments;
this.__hd.updateLayerWindow=true;
qx.ui.core.queue.Widget.add(this);
}},destruct:function(){this.__hd=this.__cs=this.__ye=this.__om=null;
}});
})();
(function(){var v="appear",u="qx.ui.virtual.core.CellEvent",t="update",s="change",r="qx.event.type.Data",q="qx.ui.virtual.core.Pane",p="resize",o="__ym",n="click",m="scrollX",d="dblclick",l="contextmenu",h="cellClick",c="__yn",b="qx.event.type.Event",g="scrollY",f="__yg",j="__yf",a="cellDblclick",k="cellContextmenu";
qx.Class.define(q,{extend:qx.ui.core.Widget,construct:function(w,x,y,z){qx.ui.core.Widget.call(this);
this.__yf=new qx.ui.virtual.core.Axis(y,w);
this.__yg=new qx.ui.virtual.core.Axis(z,x);
this.__yh=0;
this.__yi=0;
this.__yj=0;
this.__yk=0;
this.__yl={};
this.__hd={};
this.__ym=new qx.ui.container.Composite();
this.__ym.setUserBounds(0,0,0,0);
this._add(this.__ym);
this.__yn=[];
this.__yf.addListener(s,this.fullUpdate,this);
this.__yg.addListener(s,this.fullUpdate,this);
...
},events:{cellClick:u,cellContextmenu:u,cellDblclick:u,update:b,scrollX:r,scrollY:r},properties:{width:{refine:true,init:400},height:{refine:true,init:300}},members:{__yf:null,__yg:null,__yh:null,__yi:null,__yj:null,__yk:null,__yl:null,__hd:null,__ym:null,__yn:null,__yo:null,__om:null,__ye:null,getRowConfig:function(){return this.__yf;
},getColumnConfig:function(){return this.__yg;
},getChildren:function(){return [this.__ym];
},addLayer:function(A){this.__yn.push(A);
A.setUserBounds(0,0,0,0);
this.__ym.add(A);
},getLayers:function(){return this.__yn;
},getVisibleLayers:function(){var B=[];

for(var i=0;i<this.__yn.length;i++){var C=this.__yn[i];

if(C.isVisible()){B.push(C);
}}return B;
},getScrollMaxX:function(){var D=this.getInnerSize();

if(D){return Math.max(0,this.__yg.getTotalSize()-D.width);
}return 0;
},getScrollMaxY:function(){var E=this.getInnerSize();

if(E){return Math.max(0,this.__yf.getTotalSize()-E.height);
}return 0;
...
if(this.__yh!==F){var H=this.__yh;
this.__yh=F;
this._deferredUpdateScrollPosition();
this.fireDataEvent(g,F,H);
}},getScrollY:function(){return this.__yh;
},setScrollX:function(I){var J=this.getScrollMaxX();

if(I<0){I=0;
}else if(I>J){I=J;
}
if(I!==this.__yi){var K=this.__yi;
this.__yi=I;
this._deferredUpdateScrollPosition();
this.fireDataEvent(m,I,K);
}},getScrollX:function(){return this.__yi;
},getScrollSize:function(){return {width:this.__yg.getTotalSize(),height:this.__yf.getTotalSize()};
...
return;
}var P=this.__yf.getItemPosition(L);
var N=P+this.__yf.getItemSize(L);
...
return;
}var S=this.__yg.getItemPosition(Q);
var R=S+this.__yg.getItemSize(Q);
...
}bb=this.__yf.getItemAtPosition(this.getScrollY()+ba-bd.top);
bc=this.__yg.getItemAtPosition(this.getScrollX()+Y-bd.left);
...
if(!bk){return;
}var bl=this.__yi+bk.width;
var bm=this.__yk-bl;

if(this.__yi-this.__yl.left<Math.min(this.__yi,be)||this.__yl.right-bl<Math.min(bm,bg)){var bn=Math.min(this.__yi,bf);
var bj=Math.min(bm,bh);
this._setLayerWindow(bi,this.__yi-bn,this.__yh,bk.width+bn+bj,bk.height,false);
...
if(!bv){return;
}var bt=this.__yh+bv.height;
var bu=this.__yj-bt;

if(this.__yh-this.__yl.top<Math.min(this.__yh,bo)||this.__yl.bottom-bt<Math.min(bu,bq)){var bx=Math.min(this.__yh,bp);
var bw=Math.min(bu,br);
this._setLayerWindow(bs,this.__yi,this.__yh-bx,bv.width,bv.height+bx+bw,false);
}},_onResize:function(){if(this.getContainerElement().getDomElement()){this.__yo=true;
this._updateScrollPosition();
this.__yo=null;
this.fireEvent(t);
}},_onAppear:function(){this.fullUpdate();
},_onClick:function(e){this.__yp(e,h);
},_onContextmenu:function(e){this.__yp(e,k);
},_onDblclick:function(e){this.__yp(e,a);
},__yp:function(e,by){var bz=this.getCellAtPosition(e.getDocumentLeft(),e.getDocumentTop());

if(!bz){return;
}this.fireNonBubblingEvent(by,qx.ui.virtual.core.CellEvent,[this,e,bz.row,bz.column]);
},syncWidget:function(){if(this.__hd._fullUpdate){this._fullUpdate();
}else if(this.__hd._updateScrollPosition){this._updateScrollPosition();
}this.__hd={};
},_setLayerWindow:function(bA,bB,top,bC,bD,bE){var bJ=this.__yf.getItemAtPosition(top);

if(bJ){var bL=bJ.index;
var bP=this.__yf.getItemSizes(bL,bD+bJ.offset);
...
var bR=0;
var bO=0;
}var bN=this.__yg.getItemAtPosition(bB);

if(bN){var bH=bN.index;
var bG=this.__yg.getItemSizes(bH,bC+bN.offset);
...
var bQ=0;
var bI=0;
}this.__yl={top:bR,bottom:bO,left:bQ,right:bI};
this.__ym.setUserBounds(this.__yl.left-this.__yi,this.__yl.top-this.__yh,bM,bK);
this.__om=bG;
this.__ye=bP;

for(var i=0;i<this.__yn.length;i++){var bF=this.__yn[i];
bF.setUserBounds(0,0,bM,bK);

if(bE){bF.fullUpdate(bL,bH,bP,bG);
}else{bF.updateLayerWindow(bL,bH,bP,bG);
}}},__yq:function(){if(this.__yo){return;
}var bS=this.getScrollSize();

if(this.__yj!==bS.height||this.__yk!==bS.width){this.__yj=bS.height;
this.__yk=bS.width;
this.fireEvent(t);
}},fullUpdate:function(){this.__hd._fullUpdate=1;
qx.ui.core.queue.Widget.add(this);
},isUpdatePending:function(){return !!this.__hd._fullUpdate;
},_fullUpdate:function(){var bT=this.getVisibleLayers();

if(bT.length==0){this.__yq();
return;
}
if(!this.getContainerElement().getDomElement()){return ;
}var bU=this.getBounds();
this._setLayerWindow(bT,this.__yi,this.__yh,bU.width,bU.height,true);
this.__yq();
},_deferredUpdateScrollPosition:function(){this.__hd._updateScrollPosition=1;
qx.ui.core.queue.Widget.add(this);
},_updateScrollPosition:function(){var bV=this.getVisibleLayers();

if(bV.length==0){this.__yq();
return;
}var bX=this.getBounds();

if(!bX){return ;
}var bW={top:this.__yh,bottom:this.__yh+bX.height,left:this.__yi,right:this.__yi+bX.width};

if(this.__yl.top<=bW.top&&this.__yl.bottom>=bW.bottom&&this.__yl.left<=bW.left&&this.__yl.right>=bW.right){this.__ym.setUserBounds(this.__yl.left-bW.left,this.__yl.top-bW.top,this.__yl.right-this.__yl.left,this.__yl.bottom-this.__yl.top);
}else{this._setLayerWindow(bV,this.__yi,this.__yh,bX.width,bX.height,false);
}this.__yq();
}},destruct:function(){this._disposeArray(c);
this._disposeObjects(j,f,o);
this.__yl=this.__hd=this.__om=this.__ye=null;
}});
})();
...
this.__yr=[];
},properties:{anonymous:{refine:true,init:false}},events:{updated:b},members:{__yr:null,getRenderedCellWidget:function(h,j){var k=this.getColumnSizes().length;
...
}},_getSpacer:function(){var q=this.__yr.pop();
...
if(D.getUserData(f)){this.__yr.push(D);
...
if(V.getUserData(f)){this.__yr.push(V);
...
}this._cellProvider=this.__yr=null;
}});
})();
...
this.addListener(d,this.__yu,this);
this.initSelection(new qx.data.Array());
},properties:{autoHide:{refine:true,init:false},keepActive:{refine:true,init:true},selection:{check:e,event:b,apply:c,nullable:false,deferredInit:true}},members:{_target:null,_preselected:null,__ys:false,__yt:false,open:function(){this.placeToWidget(this._target,true);
this.show();
},close:function(){this.hide();
},setPreselected:function(p){this._preselected=p;
this.__yt=true;
var q=this.getChildControl(n).getSelection();
this.__yw(new qx.data.Array([p]),q);
this.__yt=false;
...
},_handleKeyboard:function(event){if(this.isVisible()&&event.getKeyIdentifier()===g){this.__yv();
return;
}var u=event.clone();
u.setTarget(this.getChildControl(n));
u.setBubbles(false);
this.getChildControl(n).dispatchEvent(u);
},_handleMouse:function(event){this.__yv();
},__mN:function(event){if(this.__ys){return;
...
this.__yw(w,v);
this.__ys=false;
},_onListChangeSelection:function(event){if(this.__yt){return;
}var x=this.getChildControl(n).getSelection();

if(this.isVisible()){this.setPreselected(x.getItem(0));
}else{this.__ys=true;
this.__yw(x,this.getSelection());
this.__ys=false;
}},__yu:function(event){if(this.isVisible()){if(this._preselected==null){var y=this.getSelection();
...
},_applySelection:function(A,B){A.addListener(m,this.__mN,this);

if(B!=null){B.removeListener(m,this.__mN,this);
}this.__yw(A,this.getChildControl(n).getSelection(A));
},__yv:function(){if(this._preselected!=null){var C=this.getSelection();
C.splice(0,1,this._preselected);
this._preselected=null;
this.close();
}},__yw:function(D,E){if(D.equals(E)){return;
}
...
}},__yx:function(){this.__yy();
this.__yz();
},__yy:function(){var H=this._target.getBounds().width;
this.setWidth(H);
},__yz:function(){var I=this.__yA();
...
}},__yA:function(){var M=this.getLayoutLocation(this._target);
...
this.__yC={};
},properties:{colorEven:{nullable:true,check:d,apply:b,themeable:true},colorOdd:{nullable:true,check:d,apply:c,themeable:true}},members:{__yD:null,__yE:null,__yB:null,__yC:null,setColor:function(g,h){if(h){this.__yB[g]=qx.theme.manager.Color.getInstance().resolve(h);
}else{delete (this.__yB[g]);
}},clearCustomColors:function(){this.__yB={};
this.updateLayerData();
},getColor:function(i){var j=this.__yB[i];

if(j){return j;
}else{return i%2==0?this.__yD:this.__yE;
}},_applyColorEven:function(k,l){if(k){this.__yD=qx.theme.manager.Color.getInstance().resolve(k);
}else{this.__yD=null;
}this.updateLayerData();
},_applyColorOdd:function(m,n){if(m){this.__yE=qx.theme.manager.Color.getInstance().resolve(m);
}else{this.__yE=null;
}this.updateLayerData();
},setBackground:function(o,p){if(p){this.__yC[o]=qx.theme.manager.Decoration.getInstance().resolve(p);
}else{delete (this.__yC[o]);
}this.updateLayerData();
},getBackground:function(q){return this.__yC[q];
}},destruct:function(){this.__yB=this.__yC=null;
}});
})();
...
this.__cq=[];
},events:{"created":a},members:{__cq:null,_createWidget:function(){throw new Error("abstract method call");
...
},getCellWidget:function(l,m){var n=this.__yF();
this.updateStates(n,m);
this.updateData(n,l);
return n;
},pool:function(o){this.__cq.push(o);
},_cleanupPool:function(){var p=this.__cq.pop();

while(p){p.destroy();
p=this.__cq.pop();
}},__yF:function(){var q=this.__cq.pop();
...
this.__ea=B.getSelection();
this.__ea.addListener(d,this.__yJ,this);
this.bind(k,A,k);
A.bind(k,this,k);
...
},_beforeOpen:function(){this.__yK();
...
}},__yJ:function(event){if(this.__yI==true){return;
}var O=this.__ea.getItem(0);
O=this.__bu(O);
this.setValue(O);
},_applyValue:function(P,Q){},_applyPlaceholder:function(R,S){this.getChildControl(m).setPlaceholder(R);
},__yK:function(){var V=this.getValue();
var U=this.getChildControl(h);
var T=U.getSelection();

if(this.__bu(T.getItem(0))!==V){this.__yI=true;
T.removeAll();
this.__yI=false;
...
break;
}}}},__bu:function(bb){var bd=this.getLabelOptions();
...
this.__ea.removeListener(d,this.__yJ,this);
this.__ea=null;
}});
})();
...
this.customSizes={};
},events:{"change":d},members:{__yL:null,getDefaultItemSize:function(){return this.defaultItemSize;
...
this.__yL=null;
this.fireNonBubblingEvent(e);
},__yM:function(){if(this.__yL){return this.__yL;
...
this.__yL=s;
return s;
},__yN:function(y){var z=this.__yL||this.__yM();
var A=0;
var C=z.length-1;
...
}},__yO:function(K){var L=this.__yL||this.__yM();
var M=0;
var O=L.length-1;
...
}},getTotalSize:function(){var S=this.__yL||this.__yM();
...
}},destruct:function(){this.customSizes=this.__yL=null;
}});
})();
