...
this.__wR(s);
return t;
},__wR:function(u){if(!(g in document.documentElement.style)){return;
}var w=true;
...
this.__wT();
},__wS:function(){this.getView().addListener(d,this._onButtonPress,this);
},_onButtonPress:function(e){this.getModel().readToken(e.getData());
},__wT:function(){var w=this.getModel();
...
this.initState();
},properties:{state:{check:[j,h,g],event:r,init:j,apply:q},errorMessage:{check:D,init:o,event:k},input:{check:D,nullable:true,event:y},maxInputLength:{check:x,init:20},operator:{check:[b,f,C,a],nullable:true,event:m},operant:{check:e,nullable:true,event:t},value:{check:e,nullable:true,event:z},memory:{check:e,nullable:true,event:s}},members:{readToken:function(E){if(E.match(/^[0123456789]$/)){this.__wW(E);
}else if(E.match(/^[\+\-\*\/]$/)){this.__xa(E);
}else if(E==v){this.__wX();
}else if(E==d){this.__wY();
}else if(E==l){this.__xb();
}else if(E==B){this.__xc();
}else if(E==c){this.__xd(E);
}else if(E==n){this.__xd(E);
}else if(E==w){this.__xf();
}else if(E==p){this.__xe();
}},__wU:function(){return parseFloat(this.getInput());
},__wV:function(F,G,H){switch(H){case b:return F+G;
case f:return F-G;
case C:return F*G;
...
}},__wW:function(K){this.setState(j);
var L=this.getInput();
...
},__xa:function(Q){var S=this.getState();
...
if(T!==null){this.setValue(this.__wV(T,R,this.getOperator()));
}else{this.setValue(R);
}this.setOperant(R);
this.setOperator(Q);
},__xb:function(){var U=this.getOperator();

if(!U){return;
}var W=this.getValue();

if(this.getState()==h){this.setValue(this.__wV(W,this.getOperant(),U));
return;
}this.setState(h);
var V=this.__wU();
this.setOperant(V);
this.setValue(this.__wV(W,V,U));
},__xc:function(){this.setState(j);
this.setOperator(null);
this.setValue(null);
this.setInput(i);
},__xd:function(X){var Y=this.getState();
var ba;
...
}},__xe:function(){var bc=this.getMemory();

if(bc==null){return;
}this.setState(j);
this.setInput(bc.toString());
},__xf:function(){this.setMemory(null);
}}});
})();
