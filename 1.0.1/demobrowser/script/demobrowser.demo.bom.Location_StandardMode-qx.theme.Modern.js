...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.bom.Location_StandardMode-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"94e274bc6b0f"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var s="px",r="moveable",q="borderBottomWidth",p="list",o='#',n="mode",m="borderTopWidth",k="borderLeftWidth",j="a",h="div",f="borderRightWidth",g="demobrowser.demo.bom.Location_StandardMode";
qx.Class.define(g,{extend:qx.application.Native,members:{main:function(){arguments.callee.base.call(this);
var v=document.getElementById(p).getElementsByTagName(j);

for(var i=0,l=v.length;i<l;i++){v[i].onclick=demobrowser.demo.bom.Location_StandardMode.jump;
}}},statics:{jump:function(event){if(!event){event=window.event;
}event.returnValue=false;

if(event.preventDefault){event.preventDefault();
}function a(t,u){return parseInt(qx.bom.element.Style.get(t,u,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var c=this.href;
var e=c.substr(c.indexOf(o)+1);
var b=document.getElementById(e);
var location=qx.bom.element.Location.get(b,document.getElementById(n).value);
var d=document.getElementById(r);
d.style.left=location.left+s;
d.style.top=location.top+s;

if(qx.bom.client.Feature.BORDER_BOX){d.style.width=(location.right-location.left)+s;
d.style.height=(location.bottom-location.top)+s;
}else{d.style.width=(location.right-location.left-a(d,k)-a(d,f))+s;
d.style.height=(location.bottom-location.top-a(d,m)-a(d,q))+s;
}return false;
},randomize:function(){var w=document.body.getElementsByTagName(h);

for(var i=0,l=w.length;i<l;i++){if(w[i].id&&w[i].id!=r){w[i].style.margin=Math.round(Math.random()*10)+s;
w[i].style.padding=Math.round(Math.random()*10)+s;
w[i].style.borderWidth=Math.round(Math.random()*10)+s;
}}}}});
})();
