...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.widget.TabView-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"403125f1ad67"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var m="Page #",l="execute",k="icon/32/apps/utilities-terminal.png",j="icon/16/apps/utilities-terminal.png",h="icon/16/apps/utilities-calculator.png",g="demobrowser.demo.widget.TabView",f="bottom",d="Notes",c="Remove first page",b="Layout",C="icon/16/apps/utilities-help.png",B="Add new page",A="changeSelection",z="Calculator...",y="icon/16/apps/utilities-notes.png",x="Bar Position",w="top",v="Calculator",u="right",t="Help",r="Help...",s="Notes...",p="Layout-Settings",q="Remove last page",n="left",o=" with close button.";
qx.Class.define(g,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var O=new qx.ui.container.Scroll();
var N=new qx.ui.container.Composite(new qx.ui.layout.VBox(10));
N.setPadding(20);
N.setAllowStretchX(false);
O.add(N);
this.getRoot().add(O,{edge:0});
N.add(this.getTabView1());
N.add(this.getTabView2());
N.add(this.getTabView3());
},getTabView1:function(){var F=new qx.ui.tabview.TabView();
F.setWidth(500);
var J=new qx.ui.tabview.Page(b,j);
J.setLayout(new qx.ui.layout.VBox());
J.add(new qx.ui.basic.Label(p));
F.add(J);
var H=new qx.ui.tabview.Page(d,y);
H.setLayout(new qx.ui.layout.VBox());
H.add(new qx.ui.basic.Label(s));
F.add(H);
var I=new qx.ui.tabview.Page(v,h);
I.setLayout(new qx.ui.layout.VBox());
I.add(new qx.ui.basic.Label(z));
F.add(I);
var G=new qx.ui.tabview.Page(t,C);
G.setLayout(new qx.ui.layout.VBox());
G.add(new qx.ui.basic.Label(r));
F.add(G);
return F;
},getTabView2:function(){var L=new qx.ui.tabview.TabView();
L.setWidth(500);

for(var i=1;i<=20;i++){var M=new qx.ui.tabview.Page(m+i,j);
M.setLayout(new qx.ui.layout.VBox());
M.setShowCloseButton(true);
M.add(new qx.ui.basic.Label(m+i+o));
L.add(M);
}return L;
...
for(var i=1;i<=3;i++){var bb=new qx.ui.tabview.Page(m+i,k);
bb.setLayout(new qx.ui.layout.VBox(4));
R.add(bb);
bb.add(new qx.ui.basic.Label(m+i));
}var T=R.getChildren()[0];
var Y=new qx.ui.basic.Label(x);
var W=new qx.ui.form.RadioButton(w);
var Q=new qx.ui.form.RadioButton(f);
var V=new qx.ui.form.RadioButton(n);
var X=new qx.ui.form.RadioButton(u);
...
ba.addListener(A,function(e){this.setBarPosition(e.getData()[0].getLabel());
},R);
V.setValue(true);
var S=new qx.ui.form.Button(B);
var U=new qx.ui.form.Button(c);
var P=new qx.ui.form.Button(q);
T.add(new qx.ui.core.Spacer(10,10));
T.add(S);
T.add(U);
T.add(P);
S.addListener(l,function(e){var D=R.getChildren().length+1;
var E=new qx.ui.tabview.Page(m+D,k);
R.add(E);
});
U.addListener(l,function(e){var a=R.getChildren();

if(a.length>0){R.remove(a[0]);
}});
P.addListener(l,function(e){var K=R.getChildren();

if(K.length>0){R.remove(K[K.length-1]);
}});
return R;
}}});
})();
...
(function(){var B="pane",A="lastTab",z="bar",y="page",x="firstTab",w="right",v="bottom",u="button",t="changeSelection",s="close",P="top",O="left",N="qx.event.type.Data",M="barRight",L="beforeChangeSelection",K="tabview",J="vertical",I="_applyBarPosition",H="barLeft",G="horizontal",E="qx.ui.tabview.TabView",F="barTop",C="barBottom",D="__zm";
qx.Class.define(E,{extend:qx.ui.core.Widget,implement:qx.ui.core.ISingleSelection,include:[qx.ui.core.MContentPadding],construct:function(d){arguments.callee.base.call(this);
this.__zl={top:F,right:M,bottom:C,left:H};
this._createChildControl(z);
this._createChildControl(B);
var f=this.__zm=new qx.ui.form.RadioGroup;
f.setWrap(false);
f.addListener(t,this._onChangeSelection,this);
if(d!=null){this.setBarPosition(d);
}else{this.initBarPosition();
}},events:{"changeSelection":N},properties:{appearance:{refine:true,init:K},barPosition:{check:[O,w,P,v],init:P,apply:I}},members:{__zm:null,_createChildControlImpl:function(bn){var bo;

switch(bn){case z:bo=new qx.ui.container.SlideBar();
bo.setZIndex(10);
this._add(bo);
break;
case B:bo=new qx.ui.container.Stack;
bo.setZIndex(5);
this._add(bo,{flex:1});
break;
...
var bd=this.getChildControl(B);
Y.exclude();
bb.add(ba);
bd.add(Y);
this.__zm.add(ba);
Y.addState(this.__zl[this.getBarPosition()]);
Y.addState(A);
var bc=this.getChildren();

if(bc[0]==Y){Y.addState(x);
}else{bc[bc.length-2].removeState(A);
}Y.addListener(s,this._onPageClose,this);
},remove:function(Q){var V=this.getChildControl(B);
var T=this.getChildControl(z);
var S=Q.getButton();
var U=V.getChildren();
if(this.getSelection()[0]==Q){var R=U.indexOf(Q);

if(R==0){if(U[1]){this.setSelection([U[1]]);
}else{this.resetSelection();
}}else{this.setSelection([U[R-1]]);
}}T.remove(S);
V.remove(Q);
this.__zm.remove(S);
Q.removeState(this.__zl[this.getBarPosition()]);
if(Q.hasState(x)){Q.removeState(x);

if(U[0]){U[0].addState(x);
}}
if(Q.hasState(A)){Q.removeState(A);

if(U.length>0){U[U.length-1].addState(A);
}}Q.removeListener(s,this._onPageClose,this);
},getChildren:function(){return this.getChildControl(B).getChildren();
},indexOf:function(a){return this.getChildControl(B).indexOf(a);
},__zl:null,_applyBarPosition:function(g,h){var j=this.getChildControl(z);
var p=g==O||g==w;
var n=g==w||g==v;
...
j.setOrientation(p?J:G);
var q=this.getChildren();
if(h){var k=this.__zl[h];
j.removeState(k);
for(var i=0,l=q.length;i<l;i++){q[i].removeState(k);
}}
if(g){var m=this.__zl[g];
j.addState(m);
for(var i=0,l=q.length;i<l;i++){q[i].addState(m);
}}},getSelection:function(){var be=this.__zm.getSelection();
var bf=[];

for(var i=0;i<be.length;i++){bf.push(be[i].getUserData(y));
}return bf;
},setSelection:function(bk){var bl=[];

for(var i=0;i<bk.length;i++){bl.push(bk[i].getChildControl(u));
}this.__zm.setSelection(bl);
},resetSelection:function(){this.__zm.resetSelection();
},isSelected:function(b){var c=b.getChildControl(u);
return this.__zm.isSelected(c);
},isSelectionEmpty:function(){return this.__zm.isSelectionEmpty();
},getSelectables:function(){var W=this.__zm.getSelectables();
var X=[];

for(var i=0;i<W.length;i++){X.push(W[i].getUserData(y));
}return X;
},_onChangeSelection:function(e){var bj=this.getChildControl(B);
var bg=e.getData()[0];

if(bg){bj.setSelection([bg.getUserData(y)]);
bg.focus();
this.scrollChildIntoView(bg,null,null,false);
}else{bj.resetSelection();
}var bi=bj.getSelection();
var bh=e.getOldData();
this.fireDataEvent(t,bi,bh);
},_onBeforeChangeSelection:function(e){if(!this.fireNonBubblingEvent(L,qx.event.type.Event,[false,true])){e.preventDefault();
}},_onRadioChangeSelection:function(e){var bm=e.getData()[0];

if(bm){this.setSelection([bm.getUserData(y)]);
}else{this.resetSelection();
...
(function(){var u="button",t="",s="close",r="String",q="_applyIcon",p="page",o="qx.event.type.Event",n="_applyShowCloseButton",m="tabview-page",l="qx.ui.tabview.Page",j="_applyLabel",k="Boolean";
qx.Class.define(l,{extend:qx.ui.container.Composite,construct:function(c,d){arguments.callee.base.call(this);
this._createChildControl(u);
if(c!=null){this.setLabel(c);
}
if(d!=null){this.setIcon(d);
}},events:{"close":o},properties:{appearance:{refine:true,init:m},label:{check:r,init:t,apply:j},icon:{check:r,init:t,apply:q},showCloseButton:{check:k,init:false,apply:n}},members:{_forwardStates:{barTop:1,barRight:1,barBottom:1,barLeft:1,firstTab:1,lastTab:1},_applyIcon:function(a,b){this.getChildControl(u).setIcon(a);
},_applyLabel:function(x,y){this.getChildControl(u).setLabel(x);
},_applyEnabled:function(g,h){arguments.callee.base.call(this,g,h);
var i=this.getChildControl(u);
g==null?i.resetEnabled():i.setEnabled(g);
},_createChildControlImpl:function(v){var w;

switch(v){case u:w=new qx.ui.tabview.TabButton;
w.setAllowGrowX(true);
w.setAllowGrowY(true);
w.setUserData(p,this);
w.addListener(s,this._onButtonClose,this);
break;
}return w||arguments.callee.base.call(this,v);
},_applyShowCloseButton:function(e,f){this.getChildControl(u).setShowCloseButton(e);
},_onButtonClose:function(){this.fireEvent(s);
},getButton:function(){return this.getChildControl(u);
}}});
})();
...
v.setRowAlign(0,o,p);
v.setColumnAlign(0,l,p);
this._getLayout().dispose();
this._setLayout(v);
this.initShowCloseButton();
},events:{"close":c},properties:{showCloseButton:{check:j,init:false,apply:g}},members:{_applyIconPosition:function(w,x){var y={icon:this.getChildControl(n),label:this.getChildControl(m),closeButton:this.getShowCloseButton()?this.getChildControl(q):null};
for(var z in y){if(y[z]){this._remove(y[z]);
}}
switch(w){case f:this._add(y.label,{row:3,column:2});
this._add(y.icon,{row:1,column:2});

if(y.closeButton){this._add(y.closeButton,{row:0,column:4});
}break;
case i:this._add(y.label,{row:1,column:2});
this._add(y.icon,{row:3,column:2});

if(y.closeButton){this._add(y.closeButton,{row:0,column:4});
}break;
case o:this._add(y.label,{row:0,column:2});
this._add(y.icon,{row:0,column:0});

if(y.closeButton){this._add(y.closeButton,{row:0,column:4});
}break;
case l:this._add(y.label,{row:0,column:0});
...
switch(t){case m:var u=new qx.ui.basic.Label(this.getLabel());
u.setAnonymous(true);
this._add(u,{row:0,column:2});
this._getLayout().setColumnFlex(2,1);
break;
case n:u=new qx.ui.basic.Image(this.getIcon());
u.setAnonymous(true);
this._add(u,{row:0,column:0});
break;
case q:u=new qx.ui.form.Button();
u.addListener(k,this._onCloseButtonClick,this);
this._add(u,{row:0,column:4});

if(!this.getShowCloseButton()){u.exclude();
}break;
}return u||arguments.callee.base.call(this,t);
},_onCloseButtonClick:function(){this.fireDataEvent(d,this);
},_applyShowCloseButton:function(a,b){if(a){this._showChildControl(q);
}else{this._excludeChildControl(q);
}},_applyCenter:function(r){var s=this._getLayout();

if(r){s.setColumnAlign(2,e,p);
}else{s.setColumnAlign(2,o,p);
}}}});
})();
