...
bm.addListener(j,function(){var a=parseInt(bm.getLabel())+1;
bm.setLabel(a.toString());
});
...
Y.add(ba);
}},createMenuForMenuButton:function(){var by=new qx.ui.menu.Menu;

for(var i=0;i<3;i++){by.add(new qx.ui.menu.RadioButton(y+i));
}var bB=new qx.ui.form.RadioGroup;
bB.add.apply(bB,by.getChildren());
var bA=new qx.ui.menu.Menu();

for(var i=0;i<3;i++){var bC=new qx.ui.menu.Button(o+i);
bA.add(bC);
}var bz=new qx.ui.menu.Button(S,v,null,by);
bA.addSeparator();
bA.add(bz);
return bA;
},createMenuForSplitButton:function(){var b=new qx.ui.menu.Menu;
var e=new qx.ui.menu.Button(L);
var c=new qx.ui.menu.Button(h);
var d=new qx.ui.menu.Button(N);
b.add(e);
b.add(c);
b.add(d);
return b;
...
qx.Class.define(q,{extend:qx.ui.core.Widget,include:[qx.ui.core.MExecutable],implement:[qx.ui.form.IExecutable],construct:function(S,T,U,V){arguments.callee.base.call(this);
this._setLayout(new qx.ui.layout.HBox);
...
}},properties:{appearance:{refine:true,init:E},focusable:{refine:true,init:true},label:{apply:p,nullable:true,check:g},icon:{check:g,apply:t,nullable:true,themeable:true},show:{init:h,check:[h,u,y],themeable:true,inheritable:true,apply:r,event:D},menu:{check:v,nullable:true,apply:z,event:s}},members:{__CS:null,_createChildControlImpl:function(K){var L;

switch(K){case o:L=new qx.ui.form.Button;
L.addListener(A,this._onButtonExecute,this);
L.setFocusable(false);
this._addAt(L,0,{flex:1});
break;
case l:L=new qx.ui.form.MenuButton;
L.setFocusable(false);
this._addAt(L,1);
break;
}return L||arguments.callee.base.call(this,K);
},_forwardStates:{hovered:1,focused:1},_applyLabel:function(a,b){var c=this.getChildControl(o);
a==null?c.resetLabel():c.setLabel(a);
},_applyIcon:function(F,G){var H=this.getChildControl(o);
F==null?H.resetIcon():H.setIcon(F);
},_applyMenu:function(M,N){var O=this.getChildControl(l);

if(M){O.resetEnabled();
O.setMenu(M);
M.setOpener(this);
M.addListener(f,this._onChangeMenuVisibility,this);
}else{O.setEnabled(false);
O.resetMenu();
}
if(N){N.removeListener(f,this._onChangeMenuVisibility,this);
N.resetOpener();
}},_applyShow:function(Q,R){},_onMouseOver:function(e){e.stopPropagation();
this.addState(n);
delete this.__CS;
...
if(I&&I.isVisible()){this.__CS=true;
return;
}this.removeState(n);
},_onKeyDown:function(e){var d=this.getChildControl(o);

switch(e.getKeyIdentifier()){case k:case j:d.removeState(i);
d.addState(m);
}},_onKeyUp:function(e){var P=this.getChildControl(o);

switch(e.getKeyIdentifier()){case k:case j:if(P.hasState(m)){P.removeState(i);
P.removeState(m);
P.execute();
}}},_onButtonExecute:function(e){this.execute();
},_onChangeMenuVisibility:function(e){if(!this.getMenu().isVisible()&&this.__CS){this.removeState(n);
}}}});
})();
