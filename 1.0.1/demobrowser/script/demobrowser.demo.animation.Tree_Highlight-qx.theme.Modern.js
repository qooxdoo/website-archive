...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.animation.Tree_Highlight-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"dc7054fc1089"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var s="execute",r="appear",q="changeSelection",p="Trash",o="mode",n="Windows (C:)",m="Documents (D:)",l="Add before",k="Add after",j="Add at begin",S="Inbox",R="Desktop",Q="Show open button:",P="New tree item name: ",O="auto",N="Hello",M="Presets",L="Data 123213234325345234523453453245",K="demobrowser.demo.animation.Tree_Highlight",J="Network",z="Control",A="Sent",x="Files",y="Remove tree item",v="Add file",w="Workspace",t="Junk #",u="Tree",B="never",C="Add folder",E="Root",D="always",G="Move to parent",F="Reset tree",I="Edit",H="Remove all children";
...
var bs=new qx.ui.groupbox.GroupBox(u).set({contentPadding:[4,4,4,4]});
bs.setLayout(new qx.ui.layout.Canvas());
this._container.add(bs);
this._treeGroup=bs;
var bt=this.getTree();
bs.add(bt,{edge:0});
this._tree=bt;
this._container.add(this.getCommandFrame());
},getTree:function(){var bk=new qx.ui.tree.Tree().set({width:200,height:400});
var bp=new qx.ui.tree.TreeFolder(E);
bp.setOpen(true);
bk.setRoot(bp);
var bn=new qx.ui.tree.TreeFolder(R);
bn.setOpen(true);
bp.add(bn);
var bf=new qx.ui.tree.TreeFolder(x);
var bd=new qx.ui.tree.TreeFolder(w);
var be=new qx.ui.tree.TreeFolder(J);
var bc=new qx.ui.tree.TreeFolder(p);
bn.add(bf,bd,be,bc);
var bl=new qx.ui.tree.TreeFile(n);
var bb=new qx.ui.tree.TreeFile(m);
bd.add(bl,bb);
var bo=new qx.ui.tree.TreeFolder(S);
var bg=new qx.ui.tree.TreeFolder(M);
var bj=new qx.ui.tree.TreeFolder(A);
var bi=new qx.ui.tree.TreeFolder(p);

for(var i=0;i<100;i++){bi.add(new qx.ui.tree.TreeFile(t+i));
}var bm=new qx.ui.tree.TreeFolder(L);
var bh=new qx.ui.tree.TreeFolder(I);
bo.add(bg,bj,bi,bm,bh);
bp.add(bo);
return bk;
},getCommandFrame:function(){var V=new qx.ui.groupbox.GroupBox(z);
V.setLayout(new qx.ui.layout.Grid(3,5));
var X=0;
V.add(new qx.ui.basic.Label(P).set({paddingTop:4}),{row:X,column:0});
this.inputItemName=new qx.ui.form.TextField(N);
V.add(this.inputItemName,{row:X++,column:1});
this.btnAddFolder=new qx.ui.form.Button(C);
this.btnAddFolder.addListener(s,this._addFolder,this);
V.add(this.btnAddFolder,{row:X++,column:0,colSpan:2});
this.btnAddFile=new qx.ui.form.Button(v);
this.btnAddFile.addListener(s,this._addFile,this);
V.add(this.btnAddFile,{row:X++,column:0,colSpan:2});
this.btnAddAfter=new qx.ui.form.Button(k);
this.btnAddAfter.addListener(s,this._addAfter,this);
V.add(this.btnAddAfter,{row:X++,column:0,colSpan:2});
this.btnAddBefore=new qx.ui.form.Button(l);
this.btnAddBefore.addListener(s,this._addBefore,this);
V.add(this.btnAddBefore,{row:X++,column:0,colSpan:2});
this.btnAddBegin=new qx.ui.form.Button(j);
this.btnAddBegin.addListener(s,this._addBegin,this);
V.add(this.btnAddBegin,{row:X++,column:0,colSpan:2});
V.add(new qx.ui.core.Spacer(5,5),{row:X++,column:0});
this.btnRemove=new qx.ui.form.Button(y);
this.btnRemove.addListener(s,this._removeTreeItem,this);
V.add(this.btnRemove,{row:X++,column:0,colSpan:2});
this.btnRemoveAll=new qx.ui.form.Button(H);
this.btnRemoveAll.addListener(s,this._removeAll,this);
V.add(this.btnRemoveAll,{row:X++,column:0,colSpan:2});
V.add(new qx.ui.core.Spacer(5,5),{row:X++,column:0});
this.btnMoveToParent=new qx.ui.form.Button(G);
this.btnMoveToParent.addListener(s,this._moveToParent,this);
V.add(this.btnMoveToParent,{row:X++,column:0,colSpan:2});
V.add(new qx.ui.core.Spacer(5,5),{row:X++,column:0});
V.add(new qx.ui.basic.Label(Q),{row:X,column:0});
var T=[D,B,O];
this.showOpenButtons={};
this.mgrShowRootOpen=new qx.ui.form.RadioGroup();

for(var i=0;i<T.length;i++){var W=T[i];
var U=new qx.ui.form.RadioButton(W).set({value:W==this._tree.getRootOpenClose()});
U.setUserData(o,W);
this.showOpenButtons[W]=U;
this.mgrShowRootOpen.add(U);
V.add(U,{row:X++,column:1});
}this.mgrShowRootOpen.addListener(q,this._onChangeShowOpen,this);
V.add(new qx.ui.core.Spacer(5,5),{row:X++,column:0});
this.btnReset=new qx.ui.form.Button(F);
this.btnReset.addListener(s,this._resetTree,this);
V.add(this.btnReset,{row:X++,column:0,colSpan:2});
this._tree.addListener(q,this._updateControls,this);
this._updateControls();
return V;
},_addFolder:function(){var e=this._tree.getSelection()[0];
var f=new qx.ui.tree.TreeFolder(this.inputItemName.getValue());
f.addListenerOnce(r,function(){var bA=new qx.fx.effect.core.Highlight(f.getContainerElement().getDomElement());
bA.start();
},this);
e.setOpen(true);
e.add(f);
},_addFile:function(){var a=this._tree.getSelection()[0];
var b=new qx.ui.tree.TreeFile(this.inputItemName.getValue());
b.addListenerOnce(r,function(){var bF=new qx.fx.effect.core.Highlight(b.getContainerElement().getDomElement());
bF.start();
},this);
a.setOpen(true);
a.add(b);
},_addBefore:function(){var g=this._tree.getSelection()[0];
var h=new qx.ui.tree.TreeFile(this.inputItemName.getValue());
h.addListenerOnce(r,function(){var Y=new qx.fx.effect.core.Highlight(h.getContainerElement().getDomElement());
Y.start();
},this);
g.getParent().addBefore(h,g);
},_addAfter:function(){var c=this._tree.getSelection()[0];
var d=new qx.ui.tree.TreeFile(this.inputItemName.getValue());
d.addListenerOnce(r,function(){var bv=new qx.fx.effect.core.Highlight(d.getContainerElement().getDomElement());
bv.start();
},this);
c.getParent().addAfter(d,c);
},_addBegin:function(){var by=this._tree.getSelection()[0];
var bz=new qx.ui.tree.TreeFile(this.inputItemName.getValue());
bz.addListenerOnce(r,function(){var br=new qx.fx.effect.core.Highlight(bz.getContainerElement().getDomElement());
br.start();
},this);
by.getParent().addAtBegin(bz);
},_removeTreeItem:function(){var bu=this._tree.getSelection()[0];
var parent=bu.getParent();
parent.remove(bu);
},_removeAll:function(){var ba=this._tree.getSelection()[0];
ba.removeAll();
},_moveToParent:function(){var bq=this._tree.getSelection()[0];
var parent=bq.getParent();
parent.remove(bq);
parent.getParent().add(bq);
this._updateControls();
},_onChangeShowOpen:function(){var bw=this._tree.getSelection()[0];

if(bw!=null){var bx=this.mgrShowRootOpen.getSelection()[0].getUserData(o);
bw.setOpenSymbolMode(bx);
}},_resetTree:function(){this._treeGroup.remove(this._tree);
this._tree=this.getTree();
this._treeGroup.add(this._tree,{edge:0});
this._tree.addListenerOnce(r,function(){this._tree.addListener(q,this._updateControls,this);
},this);
