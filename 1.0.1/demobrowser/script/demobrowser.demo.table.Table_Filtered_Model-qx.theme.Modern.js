...
N.setAdditionalStatusBarText(t);
return N;
...
var f=new qx.ui.toolbar.Button(E,q);
g.add(f);
f.addListener(s,function(e){b.addNumericFilter(z,1,r);
b.applyFilters();
i.setAdditionalStatusBarText(B);
});
var d=new qx.ui.toolbar.Button(y,q);
g.add(d);
d.addListener(s,function(e){b.addNumericFilter(A,1,r);
b.applyFilters();
i.setAdditionalStatusBarText(F);
});
var c=new qx.ui.toolbar.Button(k,q);
g.add(c);
c.addListener(s,function(e){b.addBetweenFilter(m,1000,5000,p);
b.applyFilters();
i.setAdditionalStatusBarText(H);
});
var h=new qx.ui.toolbar.Button(G,n);
g.add(h);
h.addListener(s,function(e){b.resetHiddenRows();
i.setAdditionalStatusBarText(l);
});
return a;
...
(function(){var z='g',y="==",x=">",w="notregex",v="between",u="<",t="regex",s='gi',r="!between",q=">=",n="dataChanged",p="!=",o="<=",m="qx.ui.table.model.Filtered",l='';
qx.Class.define(m,{extend:qx.ui.table.model.Simple,construct:function(){arguments.callee.base.call(this);
this.numericAllowed=new Array(y,p,x,u,q,o);
this.betweenAllowed=new Array(v,r);
this.__AB=false;
this.Filters=new Array();
},members:{__AC:null,__AB:null,_js_in_array:function(f,g){var h=g.toString();

if(h==l){return false;
}var k=new RegExp(f,z);
var j=k.test(g);
return j;
},addBetweenFilter:function(F,G,H,I){if(this._js_in_array(F,this.betweenAllowed)&&I!=null){if(G!=null&&H!=null){var J=new Array(F,G,H,I);
}}
if(J!=null){this.Filters.push(J);
}else{throw new Error("Filter not recognized or value1/value2 is null!");
}},addNumericFilter:function(P,Q,R){var S=null;

if(this._js_in_array(P,this.numericAllowed)&&R!=null){if(Q!=null){S=[P,Q,R];
}}
if(S!=null){this.Filters.push(S);
}else{throw new Error("Filter not recognized: value or target is null!");
}},addRegex:function(K,L,M){var O;

if(M){O=s;
}else{O=z;
}
if(K!=null&&L!=null){var N=new Array(t,K,L,O);
}
...
if(A!=null&&B!=null){var D=new Array(w,A,B,E);
}
...
switch(this.Filters[i][0]){case y:if(V==this.Filters[i][1]){ba=true;
}break;
case p:if(V!=this.Filters[i][1]){ba=true;
}break;
case x:if(V>this.Filters[i][1]){ba=true;
}break;
case u:if(V<this.Filters[i][1]){ba=true;
}break;
case q:if(V>=this.Filters[i][1]){ba=true;
}break;
...
switch(this.Filters[i][0]){case v:if(V>=this.Filters[i][1]&&V<=this.Filters[i][2]){ba=true;
}break;
case r:if(V<this.Filters[i][1]&&V>this.Filters[i][2]){ba=true;
}break;
}}else if(this.Filters[i][0]==t&&ba==false){V=this.getValueById(this.Filters[i][2],bc);
var X=new RegExp(this.Filters[i][1],this.Filters[i][3]);
ba=X.test(V);
}else if(this.Filters[i][0]==w&&ba==false){V=this.getValueById(this.Filters[i][2],bc);
var X=new RegExp(this.Filters[i][1],this.Filters[i][3]);
ba=!X.test(V);
...
this.fireDataEvent(n,bb);
},hideRows:function(a,b,dispatchEvent){var d=this.getData();
dispatchEvent=(dispatchEvent!=null?dispatchEvent:true);

if(!this.__AB){this.__AC=d.slice(0);
this.__AB=true;
}
if(b==null||b<1){b=1;
}
for(var c=a;c<(d.length-b);c++){d[c]=d[c+b];
}this.removeRows(c,b);
if(dispatchEvent){var e={firstRow:0,lastRow:d.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(n,e);
}},resetHiddenRows:function(){if(!this.__AC){return ;
}this.Filters=[];
this.setData(qx.lang.Array.clone(this.__AC));
},setData:function(T,U){this.__AC=qx.lang.Array.clone(T);
this.Filters=[];
arguments.callee.base.call(this,T,U);
}},destruct:function(){this.__AC=this.numericAllowed=this.betweenAllowed=this.Filters=null;
}});
})();
