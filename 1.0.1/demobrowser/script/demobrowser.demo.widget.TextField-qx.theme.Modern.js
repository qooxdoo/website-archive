...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.widget.TextField-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"0865dcc16eed"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var L="execute",K="monospace",J="Web 2.0",I="Toggle enabled",H="Send selection",G="Send end of selection",F="text\narea\nmonospace",E="Send content",D="20px sans-serif",C="Use the following buttons to interact with the ",w="changeValue",B="read only",z="Toggle live update",v="<b>first textfield</b> widget.",u="demobrowser.demo.widget.TextField",y="Send start of selection",x="text\narea",A="text\narea\nnowrap",t="max15";
...
a.setSpacing(10);
var p=new qx.ui.container.Composite(a);
p.setPadding(20);
this.getRoot().add(p,{left:0,top:0});
var c=new qx.ui.form.TextField(t).set({maxLength:15});
c.focus();
c.addListener(w,function(e){this.debug("ChangeValue: "+e.getData());
},this);
p.add(c);
var f=new qx.ui.form.TextField(J).set({font:qx.bom.Font.fromString(D),padding:6});
p.add(f);
var d=new qx.ui.form.TextField(B).set({readOnly:true});
p.add(d);
var g=new qx.ui.form.TextArea(x);
p.add(g);
var o=new qx.ui.form.TextArea(A);
o.setWrap(false);
p.add(o);
var n=new qx.ui.form.TextArea(F);
n.setFont(K);
p.add(n);
var i=new qx.ui.container.Composite(new qx.ui.layout.VBox(8));
i.setPadding(20);
var h=new qx.ui.form.Button(z);
h.addListener(L,function(){c.toggleLiveUpdate();
});
i.add(h);
var l=new qx.ui.form.Button(I);
var m=false;
l.addListener(L,function(){p.setEnabled(m);
m=!m;
});
i.add(l);
i.add(new qx.ui.core.Spacer(null,20));
var q=C+v;
var s=new qx.ui.basic.Label(q);
s.setRich(true);
i.add(s);
var j=new qx.ui.form.Button(E);
j.addListener(L,function(){this.debug("Sending content: "+c.getValue());
});
i.add(j);
var k=new qx.ui.form.Button(H);
k.setFocusable(false);
k.setKeepFocus(true);
k.addListener(L,function(){this.debug("Sending selection: "+c.getTextSelection());
});
i.add(k);
var r=new qx.ui.form.Button(y);
r.setFocusable(false);
r.setKeepFocus(true);
r.addListener(L,function(){this.debug("Sending selection: "+c.getTextSelectionStart());
});
i.add(r);
var b=new qx.ui.form.Button(G);
b.setFocusable(false);
b.setKeepFocus(true);
b.addListener(L,function(){this.debug("Sending selection: "+c.getTextSelectionEnd());
});
i.add(b);
this.getRoot().add(i,{left:200,top:0});
}}});
})();
