...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.widget.Tree_Columns-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"a0fd2ca66d56"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var y="open",x="auto",w="middle",v="icon",u="label",t="changeOpen",s="excluded",r="visible",q="String",p="opened",R="always",Q="qx.ui.tree.AbstractTreeItem",P="addItem",O="Boolean",N="Integer",M="_applyIndent",L="__nV",K="changeOpenSymbolMode",J="_applyOpenSymbolMode",I="__nR",F="resize",G="",D="removeItem",E="abstract",B="never",C="_applyIcon",z="__nS",A="_applyOpen",H="_applyLabel";
qx.Class.define(Q,{extend:qx.ui.core.Widget,type:E,include:[qx.ui.form.MModelProperty],implement:[qx.ui.form.IModel],construct:function(){arguments.callee.base.call(this);
this.__nR=[];
this._setLayout(new qx.ui.layout.HBox());
this._addWidgets();
this.initOpen();
},properties:{open:{check:O,init:false,event:t,apply:A},openSymbolMode:{check:[R,B,x],init:x,event:K,apply:J},indent:{check:N,init:19,apply:M,themeable:true},parent:{check:Q,nullable:true},icon:{check:q,apply:C,nullable:true,themeable:true},label:{check:q,apply:H,init:G}},members:{__nR:null,__nS:null,__nT:null,__nU:null,__nV:null,_addWidgets:function(){throw new Error("Abstract method call.");
},_createChildControlImpl:function(bL){var bM;

switch(bL){case u:bM=new qx.ui.basic.Label().set({alignY:w,value:this.getLabel()});
break;
case v:bM=new qx.ui.basic.Image().set({alignY:w,source:this.getIcon()});
break;
case y:bM=new qx.ui.tree.FolderOpenButton().set({alignY:w});
bM.addListener(t,this._onChangeOpen,this);
bM.addListener(F,this._updateIndent,this);
break;
}return bM||arguments.callee.base.call(this,bL);
},getTree:function(){var bE=this;

while(bE.getParent()){bE=bE.getParent();
}var bD=bE.getLayoutParent()?bE.getLayoutParent().getLayoutParent():0;

if(bD&&bD instanceof qx.ui.core.scroll.ScrollPane){return bD.getLayoutParent();
}return null;
...
}},addIcon:function(){var bh=this.getChildControl(v);

if(this.__nU){this._remove(bh);
}this._add(bh);
this.__nU=true;
},addLabel:function(bp){var bq=this.getChildControl(u);

if(this.__nT){this._remove(bq);
}
...
}}},_applyIcon:function(bm,bn){var bo=this.getChildControl(v,true);

if(bo){bo.setSource(bm);
}},_applyLabel:function(b,c){var d=this.getChildControl(u,true);
...
}by?this.addState(p):this.removeState(p);
},isOpenable:function(){var a=this.getOpenSymbolMode();
return (a===R||a===x&&this.hasChildren());
...
},getLevel:function(){var bd=this.getTree();

if(!bd){return;
}var be=this;
var bf=-1;

while(be){be=be.getParent();
bf+=1;
}if(bd.getHideRoot()){bf-=1;
}
if(!bd.getRootOpenClose()){bf-=1;
}return bf;
},syncWidget:function(){this._updateIndent();
},getChildrenContainer:function(){if(!this.__nS){this.__nS=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?r:s});
}return this.__nS;
...
},recursiveAddToWidgetQueue:function(){var bP=this.getItems(true,true,false);

for(var i=0,l=bP.length;i<l;i++){qx.ui.core.queue.Widget.add(bP[i]);
...
bJ.fireNonBubblingEvent(P,qx.event.type.Data,[bK]);
}}
if(bJ){qx.ui.core.queue.Widget.add(this);
}},addAt:function(h,j){{};

if(j==this.__nR.length){this.add(h);
return;
}var o=h.getParent();
...
}qx.lang.Array.insertAt(this.__nR,h,j);

if(!n){this.__nW();
}
if(this.getTree()){h.recursiveAddToWidgetQueue();
qx.ui.core.queue.Widget.add(this);
}},addBefore:function(bN,bO){{};
this.addAt(bN,this.__nR.indexOf(bO));
},addAfter:function(bb,bc){{};
this.addAt(bb,this.__nR.indexOf(bc)+1);
},addAtBegin:function(U){this.addAt(U,0);
},remove:function(V){for(var i=0,l=arguments.length;i<l;i++){var ba=arguments[i];

if(this.__nR.indexOf(ba)==-1){this.warn("Cannot remove treeitem '"+ba+"'. It is not a child of this tree item.");
return;
...
}}qx.lang.Array.remove(this.__nR,ba);
ba.setParent(null);
W.remove(ba);
}var X=this.getTree();

if(X){X.fireNonBubblingEvent(D,qx.event.type.Data,[ba]);
}qx.ui.core.queue.Widget.add(this);
},removeAt:function(S){var T=this.__nR[S];

if(T){this.remove(T);
}},removeAll:function(){for(var i=this.__nR.length-1;i>=0;i--){this.remove(this.__nR[i]);
}}},destruct:function(){this._disposeArray(I);
this._disposeObjects(L,z);
}});
})();
