...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.widget.Tree-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"535642c7332b"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var B="changeValue",A="execute",z="Trash",y="changeSelection",x="mode",w="multi",v="single",u="Windows (C:)",t="Documents (D:)",s="Inbox",bf="Desktop",be="root",bd="Open mode:",bc="Presets",bb="Hide Root Node",ba="Network",Y="Control",X=" items",W="Sent",V="Files",I="Show Open Items",J="",G="Workspace",H="Enable drag selection",E="Selection: ",F="Junk #",C="demobrowser.demo.widget.Tree",D="Data",K="click",L="Show root open button",O="none",N="Enable multi selection",Q="double click",P="Selection:",S="Show Selected Items",R="Root node:",M="Toggle Height",U="Edit",T="Show Items";
qx.Class.define(C,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var bk=new qx.ui.layout.HBox();
bk.set({spacing:20});
var bj=new qx.ui.container.Composite(bk);
this.getRoot().add(bj);
var bh=this.getTree();
bj.add(bh);
var bi=this.getCommandFrame(bh);
bj.add(bi);
},getTree:function(){var m=new qx.ui.tree.Tree().set({width:200,height:400});
var r=new qx.ui.tree.TreeFolder(be);
r.setOpen(true);
m.setRoot(r);
var p=new qx.ui.tree.TreeFolder(bf);
p.setOpen(true);
r.add(p);
var g=new qx.ui.tree.TreeFolder(V);
var d=new qx.ui.tree.TreeFolder(G);
var f=new qx.ui.tree.TreeFolder(ba);
var c=new qx.ui.tree.TreeFolder(z);
p.add(g,d,f,c);
var n=new qx.ui.tree.TreeFile(u);
var b=new qx.ui.tree.TreeFile(t);
d.add(n,b);
var q=new qx.ui.tree.TreeFolder(s);
var h=new qx.ui.tree.TreeFolder(bc);
var l=new qx.ui.tree.TreeFolder(W);
var k=new qx.ui.tree.TreeFolder(z);

for(var i=0;i<30;i++){k.add(new qx.ui.tree.TreeFile(F+i));
}var o=new qx.ui.tree.TreeFolder(D);
var j=new qx.ui.tree.TreeFolder(U);
q.add(h,l,k,o,j);
r.add(q);
return m;
},getCommandFrame:function(bm){var bw=new qx.ui.groupbox.GroupBox(Y);
var bu=4;
bw.setLayout(new qx.ui.layout.Grid(5,3));
var bD=0;
bw.add(new qx.ui.basic.Label(E).set({paddingTop:4}),{row:bD,column:0});
var bA=new qx.ui.form.TextField().set({readOnly:true});
bw.add(bA,{row:bD++,column:1});
bm.addListener(y,function(e){var bl=e.getData();

if(bl.length>0){if(this.getSelectionMode()===w){bA.setValue(bl.length+X);
}else{bA.setValue(bl[0].getLabel());
}}else{bA.setValue(J);
}});
bw.add(new qx.ui.core.Spacer(bu,bu),{row:bD++,column:0});
bw.add(new qx.ui.basic.Label(bd),{row:bD,column:0});
var bq={"click":K,"dblclick":Q,"none":O};
var bo=new qx.ui.form.RadioGroup();

for(var by in bq){var bp=new qx.ui.form.RadioButton(bq[by]).set({value:by==bm.getOpenMode()});
bp.setUserData(x,by);
bo.add(bp);
bw.add(bp,{row:bD++,column:1});
}bo.addListener(y,function(e){bm.setOpenMode(e.getData()[0].getUserData(x));
});
bw.add(new qx.ui.core.Spacer(bu,bu),{row:bD++,column:0});
bw.add(new qx.ui.basic.Label(P),{row:bD,column:0});
var bs=new qx.ui.form.CheckBox(N);
bw.add(bs,{row:bD++,column:1});
bs.addListener(B,function(e){var a=e.getData();
bm.setSelectionMode(a?w:v);
});
var bt=new qx.ui.form.CheckBox(H);
bw.add(bt,{row:bD++,column:1});
bt.addListener(B,function(e){var bg=e.getData();
bm.setDragSelection(bg);

if(!bs.getValue()){bs.setValue(true);
}});
bw.add(new qx.ui.core.Spacer(bu,bu),{row:bD++,column:0});
bw.add(new qx.ui.basic.Label(R),{row:bD,column:0});
var bB=new qx.ui.form.CheckBox(bb);
bw.add(bB,{row:bD++,column:1});
bB.addListener(B,function(e){bm.setHideRoot(e.getData());
});
var br=new qx.ui.form.CheckBox(L);
bw.add(br,{row:bD++,column:1});
br.addListener(B,function(e){bm.setRootOpenClose(e.getData());
});
bw.add(new qx.ui.core.Spacer(bu,bu),{row:bD++,column:0});
var bz=new qx.ui.form.Button(T);
bw.add(bz,{row:bD++,column:1});
bz.addListener(A,function(e){alert((""+bm.getItems()).replace(",","\n","g"));
});
var bn=new qx.ui.form.Button(I);
bw.add(bn,{row:bD++,column:1});
bn.addListener(A,function(e){alert((""+bm.getItems(true,false)).replace(",","\n","g"));
});
var bx=new qx.ui.form.Button(S);
bw.add(bx,{row:bD++,column:1});
bx.addListener(A,function(e){if(this.getSelectionMode()===v){alert(this.getSelection());
}else{alert(this.getSelection().toString().replace(",","\n","g"));
}},bm);
var bv=new qx.ui.form.Button(M);
bw.add(bv,{row:bD++,column:1});
var bC=true;
bv.addListener(A,function(e){bm.setHeight(bC?600:400);
bC=!bC;
});
return bw;
}}});
