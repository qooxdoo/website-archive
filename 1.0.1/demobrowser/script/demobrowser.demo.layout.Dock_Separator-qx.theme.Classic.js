...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.layout.Dock_Separator","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.layout.Dock_Separator-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"1e0689fdc6e5"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var q="main",p="separator-vertical",o="yellow",n="center",m="red",l="north",k="south",j="separator-horizontal",i="west",h="east",c="green",g="fuchsia",f="orange",b="blue",a="demobrowser.demo.layout.Dock_Separator",e="x",d="y";
...
var s=new qx.ui.container.Composite(new qx.ui.layout.VBox(20)).set({padding:20});
scroll.add(s);
...
t.setSpacingX(5);
t.setSpacingY(5);
var r=new qx.ui.container.Composite(t).set({decorator:q,backgroundColor:o,allowGrowX:false});
var y=new qx.ui.core.Widget().set({decorator:q,backgroundColor:m});
var x=new qx.ui.core.Widget().set({decorator:q,backgroundColor:b});
var w=new qx.ui.core.Widget().set({decorator:q,backgroundColor:f});
var v=new qx.ui.core.Widget().set({decorator:q,backgroundColor:c});
var u=new qx.ui.core.Widget().set({decorator:q,backgroundColor:g});
r.add(y,{edge:l});
r.add(x,{edge:i});
r.add(w,{edge:k});
r.add(v,{edge:h});
...
(function(){var bi="_applyLayoutChange",bh="Decorator",bg="Integer",bf="top",be="left",bd="auto",bc="qx.ui.layout.Dock",bb="_applySort",ba="Boolean",Y="x",X="y";
qx.Class.define(bc,{extend:qx.ui.layout.Abstract,construct:function(bQ,bR,bS,bT){arguments.callee.base.call(this);

if(bQ){this.setSpacingX(bQ);
}
if(bR){this.setSpacingY(bR);
}
if(bS){this.setSeparatorX(bS);
}
if(bT){this.setSeparatorY(bT);
}},properties:{sort:{check:[bd,X,Y],init:bd,apply:bb},separatorX:{check:bh,nullable:true,apply:bi},separatorY:{check:bh,nullable:true,apply:bi},connectSeparators:{check:ba,init:false,apply:bi},spacingX:{check:bg,init:0,apply:bi},spacingY:{check:bg,init:0,apply:bi}},members:{__BN:null,__BO:null,verifyLayoutProperty:null,_applySort:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__BP:{north:1,south:2,west:3,east:4,center:5},__BQ:{1:"top",2:"bottom",3:"left",4:"right"},__BR:function(){var bG=this._getLayoutChildren();
var bO,bI;
var length=bG.length;
var bK=[];
var bN=[];
var bL=[];
var bH=this.getSort()==="y";
var bM=this.getSort()==="x";

for(var i=0;i<length;i++){bO=bG[i];
bL=bO.getLayoutProperties().edge;

if(bL==="center"){if(bI){throw new Error("It is not allowed to have more than one child aligned to 'center'!");
}bI=bO;
}else if(bM||bH){if(bL==="north"||bL==="south"){bH?bK.push(bO):bN.push(bO);
}else if(bL==="west"||bL==="east"){bH?bN.push(bO):bK.push(bO);
}}else{bK.push(bO);
}}var bP=bK.concat(bN);
...
for(var i=0;i<length;i++){bL=bP[i].getLayoutProperties().edge;
bJ[i]=this.__BP[bL]||5;
}this.__BO=bJ;
delete this._invalidChildrenCache;
},renderLayout:function(k,l){if(this._invalidChildrenCache){this.__BR();
...
W=J+E.computeHorizontalAlignOffset(n.getAlignX()||be,x,k,y,q);
...
top=C+E.computeVerticalAlignOffset(n.getAlignY()||bf,o,l,U,v);
...
},_computeSizeHint:function(){if(this._invalidChildrenCache){this.__BR();
}var bm=this.__BN;
var bw=this.__BO;
...
var bC=this.getSpacingX(),bB=this.getSpacingY();
var bo=-bC,bn=-bB;

if(bA.x){bo-=bA.x+bC;
}
if(bA.y){bn-=bA.y+bB;
}for(var i=0;i<length;i++){bx=bm[i];
bq=bx.getSizeHint();
bs=bx.getMarginLeft()+bx.getMarginRight();
br=bx.getMarginTop()+bx.getMarginBottom();
switch(bw[i]){case 1:case 2:bu=Math.max(bu,bq.width+bt+bs);
bD=Math.max(bD,bq.minWidth+bE+bs);
bk+=bq.height+br;
by+=bq.minHeight+br;
bn+=bB;
...
case 3:case 4:bj=Math.max(bj,bq.height+bk+br);
bz=Math.max(bz,bq.minHeight+by+br);
bt+=bq.width+bs;
bE+=bq.minWidth+bs;
bo+=bC;
...
if(bA.y){bn+=bA.y+bB;
}}}var bp=Math.max(bE,bD)+bo;
var bF=Math.max(bt,bu)+bo;
var bv=Math.max(bz,by)+bn;
var bl=Math.max(bj,bk)+bn;
return {minWidth:bp,width:bF,minHeight:bv,height:bl};
...
(function(){var q="bold",p="wlpe_qx.ui.layout.Grid",o="black",n="demobrowser.demo.util.PropertyEditor",m="Widget Properties",l="lpe_qx.ui.layout.HBox",k="wlpe_qx.ui.layout.Dock",j="wlpe_",i="lpe_qx.ui.layout.Dock",h="lpe_",E="solid",D="string",C="wlpe_qx.ui.layout.HBox",B="wlpe_qx.ui.layout.Canvas",A="wlpe_qx.ui.layout.VBox",z=" layout properties",y="_applyWidget",x="qx.ui.core.Widget",w=" properties",v="wlpe_qx.ui.layout.Basic",t="Selected Widget",u="lpe_qx.ui.layout.Grid",r="pane",s="white";
qx.Class.define(n,{extend:qx.ui.core.scroll.AbstractScrollArea,construct:function(){arguments.callee.base.call(this);
this.setWidth(null);
this._editorGroups={};
var L=new qx.ui.decoration.Single();
L.setLeft(1,E,o);
this.setDecorator(L);
this.setBackgroundColor(s);
var N=new qx.ui.container.Composite().set({padding:[20,14]});
this.getChildControl(r).add(N);
this._pane=N;
var M=new qx.ui.layout.VBox().set({spacing:5});
N.setLayout(M);
N.add(this._createWidgetIndicator());
N.add(new qx.ui.basic.Label(m).set({font:q,padding:[1,0,5,0]}));
var O=qx.lang.Object.clone(demobrowser.demo.util.PropertyGroup.WIDGET_PROPERTIES);
O.value={type:D,nullable:true};
this._layoutControls=new demobrowser.demo.util.PropertyGroup(O);
N.add(this._layoutControls);
},properties:{widget:{check:x,apply:y}},members:{handleWidgetClick:function(e){var P=e.getTarget();
this.setWidget(P);
},_createWidgetIndicator:function(){var H=new qx.ui.container.Composite(new qx.ui.layout.VBox().set({spacing:5}));
H.add(this._createLabel(t));
this._widgetIndicator=new qx.ui.basic.Label().set({height:30,allowGrowX:true,allowGrowY:true,padding:5});
H._add(this._widgetIndicator);
return H;
},_createLabel:function(c){return new qx.ui.basic.Label(c).set({font:q,allowGrowX:true,padding:[5,0,5,0]});
},_createContainer:function(I,J){var K=new qx.ui.container.Composite(new qx.ui.layout.VBox().set({spacing:5}));
K.add(this._createLabel(I));
K.add(J);
return K;
},updateWidgetLayoutPropertyEditor:function(Q){var S=Q.getLayoutParent()?Q.getLayoutParent()._getLayout():null;
var R=this.getWidgetLayoutPropertyEditor(S);

if(R){R.getChildren()[1].setSelected(Q);
}
if(R===this._wlpe){return;
}
if(this._wlpe){this._pane.remove(this._wlpe);
}
if(!R){this._wlpe=R;
return;
}this._pane.addAfter(R,this._layoutControls);
this._wlpe=R;
},updateLayoutPropertyEditor:function(d){var g=d.getLayout?d.getLayout():null;
var f=this.getLayoutPropertyEditor(g);

if(f){f.getChildren()[1].setSelected(g);
}
if(f===this._lpe){return;
}
...
switch(name){case C:case A:b=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.BOX_PROPERTIES);
break;
case B:b=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.CANVAS_PROPERTIES);
break;
case v:b=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.BASIC_PROPERTIES);
break;
case k:b=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.DOCK_PROPERTIES);
break;
case p:b=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.GRID_PROPERTIES);
break;
}
if(!b){return null;
}this._editorGroups[name]=this._createContainer(a.constructor.classname+z,b);
...
switch(name){case l:U=new demobrowser.demo.util.PropertyGroup(demobrowser.demo.util.PropertyGroup.HBOX_PROPERTIES);
break;
case i:U=new demobrowser.demo.util.PropertyGroup(demobrowser.demo.util.PropertyGroup.DOCK_PROPERTIES);
break;
case u:U=new demobrowser.demo.util.PropertyGroup(demobrowser.demo.util.PropertyGroup.GRID_PROPERTIES);
break;
}
if(!U){return null;
}this._editorGroups[name]=this._createContainer(T.constructor.classname+w,U);
return this._editorGroups[name];
},_applyWidget:function(F,G){this._layoutControls.setSelected(F);
this._widgetIndicator.setBackgroundColor(F.getBackgroundColor());
this._widgetIndicator.setDecorator(F.getDecorator());
this._widgetIndicator.setValue(F.toString());
this.updateLayoutPropertyEditor(F);
this.updateWidgetLayoutPropertyEditor(F);
...
(function(){var D="value",C="changeValue",B="enum",A="set",z="int",y="string",x="bool",w="function",v="null",u="demobrowser.demo.util.PropertyGroup",r="_applySelected",t="get",s="top",q="changeSelection",p="right";
qx.Class.define(u,{extend:qx.ui.core.Widget,construct:function(F){arguments.callee.base.call(this);
this._properties=F;
var H=new qx.ui.layout.Grid(10,5);
H.setColumnAlign(0,p,s);
H.setColumnMinWidth(0,110);
this._setLayout(H);
this.set({allowShrinkX:false,allowShrinkY:false});
var O=0;

for(var K in this._properties){var N=this._properties[K].type;
var J=this._properties[K].nullable;
this._add(new qx.ui.basic.Label(K).set({paddingTop:4}),{row:O,column:0});

if(J){var I=new qx.ui.form.CheckBox(v);
I.addListener(C,this._createOnNullPropertyChange(K),this);
this._add(I,{row:O,column:2});
this._properties[K].nullWidget=I;
}
if(N==z){var M=new qx.ui.form.Spinner().set({minimum:this._properties[K].min||0,maximum:this._properties[K].max!==undefined?this._properties[K].max:1000});
M.addListener(C,this._createOnIntPropertyChange(K),this);
this._add(M,{row:O++,column:1});
}else if(N==x){var M=new qx.ui.form.CheckBox();
M.addListener(C,this._createOnBoolPropertyChange(K),this);
this._add(M,{row:O++,column:1});
}else if(N==B){var L=this._properties[K].values;
var M=new qx.ui.form.RadioGroup();

for(var i=0;i<L.length;i++){var G=new qx.ui.form.RadioButton(L[i]);
G.setUserData(D,L[i]);
M.add(G);
this._add(G,{row:O++,column:1});
}M.addListener(q,this._createOnEnumPropertyChange(K,M),this);
}else if(N==y){var M=new qx.ui.form.TextField();
M.setLiveUpdate(true);
M.addListener(C,this._createOnIntPropertyChange(K),this);
this._add(M,{row:O++,column:1});
}this._properties[K].formItem=M;
}},statics:{WIDGET_PROPERTIES:{"width":{type:"int",nullable:true},"height":{type:"int",nullable:true},"minWidth":{type:"int",nullable:true},"minHeight":{type:"int",nullable:true},"maxWidth":{type:"int",nullable:true},"maxHeight":{type:"int",nullable:true},"allowGrowX":{type:"bool",nullable:false},"allowGrowY":{type:"bool",nullable:false},"allowShrinkX":{type:"bool",nullable:false},"allowShrinkY":{type:"bool",nullable:false},"marginTop":{type:"int",min:-1000,nullable:false},"marginRight":{type:"int",min:-1000,nullable:false},"marginBottom":{type:"int",min:-1000,nullable:false},"marginLeft":{type:"int",min:-1000,nullable:false},"paddingTop":{type:"int",nullable:false},"paddingRight":{type:"int",nullable:false},"paddingBottom":{type:"int",nullable:false},"paddingLeft":{type:"int",nullable:false},"alignX":{type:"enum",values:["left","center","right"],nullable:true},"alignY":{type:"enum",values:["top","middle","bottom","baseline"],nullable:true}},HBOX_PROPERTIES:{"alignX":{type:"enum",values:["left","center","right"],nullable:false},"alignY":{type:"enum",values:["top","middle","bottom","baseline"],nullable:false},"spacing":{type:"int",nullable:false},"reversed":{type:"bool",nullable:false}},DOCK_PROPERTIES:{"sort":{type:"enum",values:["auto","y","x"],nullable:false},"spacingX":{type:"int",nullable:false},"spacingY":{type:"int",nullable:false}},GRID_PROPERTIES:{"horizontalSpacing":{type:"int",nullable:false},"verticalSpacing":{type:"int",nullable:false}}},properties:{selected:{apply:r,nullable:true}},members:{_applySelected:function(be,bf){this._updateControls(be);
},_setProperty:function(c,name,d){if(!c){return;
}var g=A+qx.lang.String.firstUp(name);
var f=this._properties[name].convert;

if(f){d=f(d);
}c[g](d);
},_getProperty:function(a,name){var b=t+qx.lang.String.firstUp(name);
return a[b]();
},_hasProperty:function(h,name){var j=A+qx.lang.String.firstUp(name);
return (typeof (h[j])==w);
},_createOnIntPropertyChange:function(o){return function(e){var bc=this.getSelected();
this._setProperty(bc,o,e.getTarget().getValue());
};
},_createOnBoolPropertyChange:function(bd){return function(e){var E=this.getSelected();
this._setProperty(E,bd,e.getTarget().getValue());
};
},_createOnEnumPropertyChange:function(m,n){return function(e){var bb=this.getSelected();
this._setProperty(bb,m,n.getSelection()[0].getUserData(D));
};
},_createOnNullPropertyChange:function(k){return function(e){var Q=this.getSelected();
var P=e.getTarget();

if(P.getValue()){this._setProperty(Q,k,null);
this._properties[k].formItem.setEnabled(false);
}else{var R=null;

if(this._properties[k].type==B){R=this._properties[k].formItem.getSelection()[0].getUserData(D);
}else{R=this._properties[k].formItem.getValue();
}this._setProperty(Q,k,R);
this._properties[k].formItem.setEnabled(true);
}};
},_updateControls:function(S){for(var Y in this._properties){var ba=this._properties[Y].type;
var V=this._properties[Y].formItem;
var T=this._properties[Y].nullable;

if(!this._hasProperty(S,Y)){V.setEnabled(false);

if(T){this._properties[Y].nullWidget.setEnabled(false);
}return;
}V.setEnabled(true);
var X=this._getProperty(S,Y);

if(X!==null){if(ba==z){V.setValue(parseInt(X));
}else if(ba==y){V.setValue(X.toString());
}else if(ba==x){V.setValue(!!X);
}else if(ba==B){var U=V.getItems();

for(var i=0,l=U.length;i<l;i++){var W=U[i];

if(W.getUserData(D)==X){V.setSelection([W]);
break;
}}}}
if(T){this._properties[Y].nullWidget.setValue(X==null);
this._properties[Y].nullWidget.setEnabled(true);
V.setEnabled(X!==null);
...
qx.Class.define(b,{extend:qx.ui.form.ToggleButton,include:[qx.ui.form.MForm,qx.ui.form.MModelProperty],implement:[qx.ui.form.IForm,qx.ui.form.IModel],construct:function(a){{};
...
(function(){var L="textfield",K="",J="downbutton",I="upbutton",H="Number",G="inner",F="PageUp",E="Boolean",D="changeValue",C="Down",bj="Up",bi="execute",bh="PageDown",bg="changeLocale",bf="qx.dynlocale",be="on",bd="_applyEditable",bc="_applyWrap",bb="keydown",ba="\-]",S="mousewheel",T="_applyValue",Q="number",R="_applyMinimum",O="qx.util.format.NumberFormat",P="[0-9",M="keyup",N="spinner",U="this._checkValue(value)",V="_applyMaximum",X="changeNumberFormat",W="_applyNumberFormat",Y="qx.ui.form.Spinner";
qx.Class.define(Y,{extend:qx.ui.core.Widget,implement:[qx.ui.form.INumberForm,qx.ui.form.IRange,qx.ui.form.IForm],include:[qx.ui.core.MContentPadding,qx.ui.form.MForm],construct:function(bl,bm,bn){arguments.callee.base.call(this);
var bo=new qx.ui.layout.Grid();
bo.setColumnFlex(0,1);
bo.setRowFlex(0,1);
bo.setRowFlex(1,1);
this._setLayout(bo);
this.addListener(bb,this._onKeyDown,this);
this.addListener(M,this._onKeyUp,this);
this.addListener(S,this._onMouseWheel,this);

if(qx.core.Variant.isSet(bf,be)){qx.locale.Manager.getInstance().addListener(bg,this._onChangeLocale,this);
}this._createChildControl(L);
this._createChildControl(I);
this._createChildControl(J);
if(bl!=null){this.setMinimum(bl);
}
if(bn!=null){this.setMaximum(bn);
}
if(bm!==undefined){this.setValue(bm);
}else{this.initValue();
}},properties:{appearance:{refine:true,init:N},focusable:{refine:true,init:true},singleStep:{check:H,init:1},pageStep:{check:H,init:10},minimum:{check:H,apply:R,init:0},value:{check:U,nullable:true,apply:T,init:0,event:D},maximum:{check:H,apply:V,init:100},wrap:{check:E,init:false,apply:bc},editable:{check:E,init:true,apply:bd},numberFormat:{check:O,apply:W,nullable:true},allowShrinkY:{refine:true,init:false}},members:{__pQ:null,__pR:false,__pS:false,_createChildControlImpl:function(r){var s;

switch(r){case L:s=new qx.ui.form.TextField();
s.setFilter(this._getFilterRegExp());
s.addState(G);
s.setWidth(40);
s.setFocusable(false);
s.addListener(D,this._onTextChange,this);
this._add(s,{column:0,row:0,rowSpan:2});
break;
case I:s=new qx.ui.form.RepeatButton();
s.addState(G);
s.setFocusable(false);
s.addListener(bi,this._countUp,this);
this._add(s,{column:1,row:0});
break;
case J:s=new qx.ui.form.RepeatButton();
s.addState(G);
s.setFocusable(false);
s.addListener(bi,this._countDown,this);
this._add(s,{column:1,row:1});
break;
}return s||arguments.callee.base.call(this,r);
},_getFilterRegExp:function(){var x=qx.locale.Number.getDecimalSeparator(qx.locale.Manager.getInstance().getLocale());
var w=qx.locale.Number.getGroupSeparator(qx.locale.Manager.getInstance().getLocale());
var v=K;
var t=K;

if(this.getNumberFormat()!==null){v=this.getNumberFormat().getPrefix()||K;
t=this.getNumberFormat().getPostfix()||K;
}var u=new RegExp(P+qx.lang.String.escapeRegexpChars(x)+qx.lang.String.escapeRegexpChars(w)+qx.lang.String.escapeRegexpChars(v)+qx.lang.String.escapeRegexpChars(t)+ba);
return u;
},_forwardStates:{focused:true,invalid:true},tabFocus:function(){var d=this.getChildControl(L);
d.getFocusElement().focus();
d.selectAllText();
},_applyMinimum:function(bA,bB){if(this.getMaximum()<bA){this.setMaximum(bA);
}
if(this.getValue()<bA){this.setValue(bA);
}else{this._updateButtons();
}},_applyMaximum:function(bt,bu){if(this.getMinimum()>bt){this.setMin(bt);
}
if(this.getValue()>bt){this.setValue(bt);
}else{this._updateButtons();
...
},_applyValue:function(a,b){var c=this.getChildControl(L);
this._updateButtons();
this.__pQ=a;
if(a!==null){if(this.getNumberFormat()){c.setValue(this.getNumberFormat().format(a));
}else{c.setValue(a+K);
}}else{c.setValue(K);
}},_applyEditable:function(j,k){var l=this.getChildControl(L);

if(l){l.setReadOnly(!j);
}},_applyWrap:function(A,B){this._updateButtons();
},_applyNumberFormat:function(o,p){var q=this.getChildControl(L);
q.setFilter(this._getFilterRegExp());
this.getNumberFormat().addListener(X,this._onChangeNumberFormat,this);
this._applyValue(this.__pQ,undefined);
},_getContentPaddingTarget:function(){return this.getChildControl(L);
},_updateButtons:function(){var bq=this.getChildControl(I);
var bp=this.getChildControl(J);
...
}}}},_onKeyDown:function(e){switch(e.getKeyIdentifier()){case F:this.__pR=true;
case bj:this.getChildControl(I).press();
break;
case bh:this.__pS=true;
case C:this.getChildControl(J).press();
break;
default:return ;
}e.stopPropagation();
e.preventDefault();
...
this.__pR=false;
break;
case bj:this.getChildControl(I).release();
break;
case bh:this.getChildControl(J).release();
this.__pS=false;
break;
case C:this.getChildControl(J).release();
break;
}},_onMouseWheel:function(e){if(e.getWheelDelta()>0){this._countDown();
}else{this._countUp();
}e.stop();
},_onTextChange:function(e){var f=this.getChildControl(L);
var g;
if(this.getNumberFormat()){try{g=this.getNumberFormat().parse(f.getValue());
}catch(bs){}}if(g===undefined){g=parseFloat(f.getValue(),10);
}if(!isNaN(g)){if(g>this.getMaximum()){f.setValue(this.getMaximum()+K);
return;
}else if(g<this.getMinimum()){f.setValue(this.getMinimum()+K);
return;
}this.setValue(g);
}else{this._applyValue(this.__pQ,undefined);
}},_onChangeLocale:function(m){if(this.getNumberFormat()!==null){this.setNumberFormat(this.getNumberFormat());
var n=this.getChildControl(L);
n.setFilter(this._getFilterRegExp());
n.setValue(this.getNumberFormat().format(this.getValue()));
}},_onChangeNumberFormat:function(y){var z=this.getChildControl(L);
z.setFilter(this._getFilterRegExp());
z.setValue(this.getNumberFormat().format(this.getValue()));
},_countUp:function(){if(this.__pR){var bw=this.getValue()+this.getPageStep();
...
},_countDown:function(){if(this.__pS){var by=this.getValue()-this.getPageStep();
}else{var by=this.getValue()-this.getSingleStep();
}if(this.getWrap()){if(by<this.getMinimum()){var bx=this.getMinimum()+by;
by=this.getMaximum()-bx;
}}this.gotoValue(by);
},gotoValue:function(bk){return this.setValue(Math.min(this.getMaximum(),Math.max(this.getMinimum(),bk)));
}},destruct:function(){if(qx.core.Variant.isSet(bf,be)){qx.locale.Manager.getInstance().removeListener(bg,this._onChangeLocale,this);
}}});
})();
...
}},"left":{type:"string",nullable:true,convert:function(i){if(parseInt(i).toString()==i){i=parseInt(i);
}return i||null;
}},"width":{type:"int",nullable:true,convert:function(k){return k==null?null:k+"%";
}},"height":{type:"int",nullable:true,convert:function(h){return h==null?null:h+"%";
}}},DOCK_PROPERTIES:{"width":{type:"int",nullable:true,convert:function(j){return j==null?null:j+"%";
}},"height":{type:"int",nullable:true,convert:function(a){return a==null?null:a+"%";
}},"edge":{type:"enum",values:["north","east","south","west","center"],nullable:false}},GRID_PROPERTIES:{"row":{type:"int",nullable:false},"column":{type:"int",nullable:false},"rowSpan":{type:"int",nullable:true},"colSpan":{type:"int",nullable:true}}},members:{_setProperty:function(l,name,m){var n=this._properties[name].convert;

if(n){m=n(m);
}var o={};
o[name]=m;
l.setLayoutProperties(o);
},_getProperty:function(f,name){return f.getLayoutProperties()[name]||null;
},_hasProperty:function(e,name){return true;
}}});
})();
