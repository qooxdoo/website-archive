...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.data.JsonToList-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"733aa55104d8"},
  boot : "boot",
  bootIsInline : true,
...
t.setLabelPath(o);
t.setIconOptions({converter:function(v){return p+v+k;
}});
t.setIconPath(g);
var r=h;
...
}},properties:{selection:{check:e,event:g,apply:c,init:null}},members:{_modifingSelection:0,__vq:null,__vr:null,_applySelection:function(o,p){if(this.__vr!=undefined&&p!=undefined){p.removeListenerById(this.__vr);
}this.__vr=o.addListener(f,this.__vs,this);
},__vs:function(){this._updateSelection();
},_changeTargetSelection:function(){if(this._inSelectionModification()||this.getTarget()==null){return;
}var w=this.getTarget().getSelection();
var v=this.getSelection();

if(v==null){v=new qx.data.Array();
this.setSelection(v);
}if(w.length>0){v.toArray().splice(0,v.getLength());
}else{v.splice(0,this.getSelection().getLength());
}for(var i=0;i<w.length;i++){var u=w[i].getModel();

if(i+1==w.length){v.push(u);
}else{v.toArray().push(u);
}}this.fireDataEvent(g,this.getSelection());
},_addChangeTargetListener:function(s,t){if(this.__vq!=undefined&&t!=undefined){t.removeListenerById(this.__vq);
}
if(s!=null){if(this.__vt()||this.__vu()){this.__vq=s.addListener(g,this._changeTargetSelection,this);
}}},_updateSelection:function(){this._startSelectionModification();
if(this.__vt()){var j=[];
for(var i=0;i<this.getSelection().length;i++){var h=this.getSelection().getItem(i);
var k=this.__vw(h);
...
for(var i=0;i<j.length;i++){l[i]=j[i].getModel();
}for(var i=this.getSelection().length-1;i>=0;i--){if(!qx.lang.Array.contains(l,this.getSelection().getItem(i))){this.getSelection().splice(i,1);
}}}else if(this.__vu()){this.__vv(this.getSelection().getItem(this.getSelection().length-1));
this.getSelection().splice(0,this.getSelection().getLength()-1);
}this._endSelectionModification();
},__vt:function(){var a=this.getTarget().constructor;
return qx.Class.implementsInterface(a,qx.ui.core.IMultiSelection);
},__vu:function(){var x=this.getTarget().constructor;
return qx.Class.implementsInterface(x,qx.ui.core.ISingleSelection);
},__vv:function(q){var r=this.__vw(q);
if(r==null){return;
}if(this.__vt()){this.getTarget().addToSelection(r);
}else if(this.__vu()){this.getTarget().setSelection([r]);
}},__vw:function(m){var n=this.getTarget().getSelectables();
for(var i=0;i<n.length;i++){if(n[i].getModel()==m){return n[i];
}}return null;
...
qx.Class.define(bT,{extend:qx.core.Object,include:qx.data.controller.MSelection,construct:function(bc,bd,be){arguments.callee.base.call(this);
this.__vx=[];
this.__vy=[];
this.__vz=[];
this.__vA={};
...
}},properties:{model:{check:bN,apply:bV,event:bD,nullable:true},target:{apply:bP,event:bX,nullable:true,init:null},labelPath:{check:bF,apply:bY,nullable:true},iconPath:{check:bF,apply:bS,nullable:true},labelOptions:{apply:bC,nullable:true},iconOptions:{apply:bM,nullable:true},delegate:{apply:bR,event:bQ,init:null,nullable:true}},members:{__vB:null,__vC:null,__vx:null,__vA:null,__vy:null,__vz:null,update:function(){this.__vE();
this.__vI();
this._updateSelection();
},_applyDelegate:function(br,bs){this._setConfigureItem(br,bs);
this._setFilter(br,bs);
this._setCreateItem(br,bs);
this._setBindItem(br,bs);
},_applyIconOptions:function(Q,R){this.__vI();
},_applyLabelOptions:function(bA,bB){this.__vI();
},_applyIconPath:function(ca,cb){this.__vI();
},_applyLabelPath:function(bj,bk){this.__vI();
},_applyModel:function(a,b){if(b!=undefined){if(this.__vB!=undefined){b.removeListenerById(this.__vB);
}
if(this.__vC!=undefined){b.removeListenerById(this.__vC);
}}if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length);
}if(a!=null){this.__vB=a.addListener(bW,this.__vE,this);
this.__vC=a.addListener(bL,this.__vD,this);
this.__vK();
this.__vE();
if(this.getTarget()!=null){var d=this.getModel();
var e=this.getTarget().getChildren();

for(var i=0,l=this.__vx.length;i<l;i++){var f=d.getItem(this.__vL(i));
var c=e[i];
c.setModel(f);
...
if(bv!=null){if(this.getModel()!=null){for(var i=0;i<this.__vx.length;i++){this.__vG(this.__vL(i));
}}}},__vD:function(){for(var i=this.getSelection().length-1;i>=0;i--){if(!this.getModel().contains(this.getSelection().getItem(i))){this.getSelection().splice(i,1);
}}qx.ui.core.queue.Widget.add(this);
if(this.__vx.length!=this.getModel().getLength()){this.update();
}},syncWidget:function(){this._updateSelection();
},__vE:function(){if(this.getTarget()==null){return;
}this.__vK();
var bb=this.__vx.length;
var ba=this.getTarget().getChildren().length;
if(bb>ba){for(var j=ba;j<bb;j++){this.__vG(this.__vL(j));
}}else if(bb<ba){for(var j=ba;j>bb;j--){this.__vH();
}}},__vF:function(){var t=this.getModel();
if(t!=null){t.removeListenerById(this.__vC);
this.__vC=t.addListener(bL,this.__vD,this);
}},_createItem:function(){var M=this.getDelegate();
if(M!=null&&M.createItem!=null){var L=M.createItem();
}else{var L=new qx.ui.form.ListItem();
}if(M!=null&&M.configureItem!=null){M.configureItem(L);
}return L;
},__vG:function(bf){var bg=this._createItem();
bg.setModel(this.getModel().getItem(bf)||null);
this._bindListItem(bg,bf);
this.getTarget().add(bg);
},__vH:function(){this._startSelectionModification();
var J=this.getTarget().getChildren();
var I=J.length-1;
var K=J[I];
this._removeBindingsFrom(K);
this.getTarget().removeAt(I);
K.destroy();
this._endSelectionModification();
},getVisibleModels:function(){var u=[];
var v=this.getTarget();

if(v!=null){var w=v.getChildren();

for(var i=0;i<w.length;i++){u.push(w[i].getModel());
}}return new qx.data.Array(u);
},_bindListItem:function(N,O){var P=this.getDelegate();
if(P!=null&&P.bindItem!=null){P.bindItem(this,N,O);
}else{this.bindDefaultProperties(N,O);
}},bindDefaultProperties:function(X,Y){this.bindProperty(this.getLabelPath(),bO,this.getLabelOptions(),X,Y);
if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),bU,this.getIconOptions(),X,Y);
}},bindProperty:function(g,h,k,m,n){m.setModel(this.getModel().getItem(n));
if(k!=null){var k=qx.lang.Object.clone(k);
this.__vA[h]=k.onUpdate;
delete k.onUpdate;
}else{k={};
this.__vA[h]=null;
...
if(g!=null&&g!=bI){o+=bE+g;
}var p=this.bind(o,m,h,k);
m.setUserData(h+bJ,p);
if(!qx.lang.Array.contains(this.__vy,h)){this.__vy.push(h);
}},bindPropertyReverse:function(B,C,D,E,F){var G=bG+F+bH;

if(B!=null&&B!=bI){G+=bE+B;
}var H=E.bind(C,this,G,D);
E.setUserData(B+bK,H);
if(!qx.lang.Array.contains(this.__vz,B)){this.__vz.push(B);
}},_onBindingSet:function(x,y,z){if(this.getModel()==null||this._inSelectionModification()){return;
}for(var i=0;i<this.__vy.length;i++){if(this.__vA[this.__vy[i]]!=null){this.__vA[this.__vy[i]]();
}}var A=this.getModel().getItem(x);
z.setModel(A==undefined?null:A);
},_removeBindingsFrom:function(bt){for(var i=0;i<this.__vy.length;i++){var bu=bt.getUserData(this.__vy[i]+bJ);

if(bu!=null){this.removeBinding(bu);
}}for(var i=0;i<this.__vz.length;i++){var bu=bt.getUserData(this.__vz[i]+bK);

if(bu!=null){bt.removeBinding(bu);
}}},__vI:function(){if(this.getTarget()==null||this.getModel()==null){return;
}var bi=this.getTarget().getChildren();
for(var i=0;i<bi.length;i++){this._removeBindingsFrom(bi[i]);
this._bindListItem(bi[i],this.__vL(i));
}this.__vF();
},_setConfigureItem:function(q,r){if(q!=null&&q.configureItem!=null&&this.getTarget()!=null){var s=this.getTarget().getChildren();

for(var i=0;i<s.length;i++){q.configureItem(s[i]);
}}},_setBindItem:function(V,W){if(V!=null&&V.bindItem!=null){if(W!=null&&W.bindItem!=null&&V.bindItem==W.bindItem){return;
}this.__vI();
}},_setCreateItem:function(bx,by){if(this.getTarget()==null||this.getModel()==null||bx==null||bx.createItem==null){return;
}this._startSelectionModification();
var bz=this.getTarget().getChildren();

for(var i=0,l=bz.length;i<l;i++){this._removeBindingsFrom(bz[i]);
}this.getTarget().removeAll();
this.update();
this._endSelectionModification();
this._updateSelection();
},_setFilter:function(bl,bm){if((bl==null||bl.filter==null)&&(bm!=null&&bm.filter!=null)){this.__vJ();
}if(this.getTarget()==null||this.getModel()==null||bl==null||bl.filter==null){return;
}this._startSelectionModification();
var bq=this.getTarget().getChildren();

for(var i=0,l=bq.length;i<l;i++){this._removeBindingsFrom(bq[i]);
}var bo=this.__vx;
this.__vK();
if(bo.length>this.__vx.length){for(var j=bo.length;j>this.__vx.length;j--){this.getTarget().removeAt(j-1);
}}else if(bo.length<this.__vx.length){for(var j=bo.length;j<this.__vx.length;j++){var bp=this._createItem();
this.getTarget().add(bp);
}}var bn=this.getTarget().getChildren();

for(var i=0;i<bn.length;i++){this._bindListItem(bn[i],this.__vL(i));
}this.__vF();
this._endSelectionModification();
this._updateSelection();
},__vJ:function(){this.__vK();
this.__vE();
this.__vI();
qx.ui.core.queue.Widget.add(this);
},__vK:function(){var T=this.getModel();
...
for(var i=0;i<T.getLength();i++){if(S==null||S(T.getItem(i))){this.__vx.push(i);
}}},__vL:function(bh){return this.__vx[bh];
}},destruct:function(){this.__vx=this.__vA=this.__vy=null;
this.__vz=null;
}});
})();
