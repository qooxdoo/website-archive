...
(function(){var m="Stopping recurrent 2-second timer",l="Recurrent 2-second timer with limit 3:",k="\n",j="One-shot 1-second timer: ",i=" ",h="demobrowser.demo.test.TimerManager",g="Recurrent 5-second timer: ",f="__zG",e="Start timers",d="execute";
qx.Class.define(h,{extend:qx.application.Standalone,members:{__zG:null,startTimers:function(){var v=qx.util.TimerManager.getInstance();
v.start(function(a,b){this.__zH(g+b);
},5000,this,null,0);
v.start(function(n,o){this.__zH(j+o);
},0,this,null,1000);
v.start(function(p,q){this.__zH(l+q);

if(++p.count==3){this.__zH(m);
v.stop(q);
}},2000,this,{count:0},2000);
},__zH:function(c){c=Date.parse(new Date)+i+c+k;
this.__zG.setValue(this.__zG.getValue()+c);
},main:function(){arguments.callee.base.call(this);
var s=new qx.ui.layout.VBox(5);
var r=new qx.ui.container.Composite(s);
var t=new qx.ui.form.Button(e);
t.set({allowGrowX:false,allowGrowY:false});
t.addListener(d,function(){t.setEnabled(false);
this.startTimers();
},this);
var u=new qx.ui.form.TextArea();
u.set({width:400,height:300});
this.__zG=u;
r.add(u);
r.add(t);
...
qx.Class.define(k,{extend:qx.core.Object,type:j,statics:{__xw:[],__xx:{},__xy:0},members:{start:function(a,b,c,d,e){if(!e){e=b||0;
}var f=(new Date()).getTime()+e;
arguments.callee.self.__xx[++arguments.callee.self.__xy]={callback:a,userData:d||null,expireAt:f,recurTime:b,context:c||this};
this.__xz(f,arguments.callee.self.__xy);
return arguments.callee.self.__xy;
},stop:function(g){var h=arguments.callee.self.__xw;
var length=h.length;

for(var i=0;i<length;i++){if(h[i]==g){h.splice(i,1);
break;
}}delete arguments.callee.self.__xx[g];
if(h.length==0){qx.event.Idle.getInstance().removeListener(l,this.__xA);
}},__xz:function(m,n){var p=arguments.callee.self.__xw;
var o=arguments.callee.self.__xx;
var length=p.length;

for(var i=0;i<length;i++){if(o[p[i]].expireAt>m){p.splice(i,0,n);
break;
}}if(p.length==length){p.push(n);
}if(p.length==1){qx.event.Idle.getInstance().addListener(l,this.__xA,this);
}},__xA:function(){var t=(new Date()).getTime();
var r=arguments.callee.self.__xw;
var s=arguments.callee.self.__xx;
...
this.__xz(q.expireAt,v);
}else{delete s[v];
}}if(r.length==0){qx.event.Idle.getInstance().removeListener(l,this.__xA);
}}}});
})();
