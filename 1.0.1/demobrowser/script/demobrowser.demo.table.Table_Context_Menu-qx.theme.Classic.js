...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.table.Table_Context_Menu-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"bfa78068bc5d"},
  boot : "boot",
  bootIsInline : true,
...
P.setColumns([B,l,p,K]);
P.setData(Q);
var R=new qx.ui.table.Table(P);
R.set({width:600,height:400,decorator:null});
var O=R.getTableColumnModel();
O.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
R.setContextMenuHandler(3,this._contextMenuHandlerBoolean);
R.setContextMenuHandler(2,this._contextMenuHandlerNull);
return R;
},_contextMenuHandlerBoolean:function(bo,bp,bq,br,bs){var bt;

for(var i=0;i<=1;i++){bt=new qx.ui.menu.Button(i?n:x);
bt.setUserData(s,i?true:false);
bt.addListener(u,function(e){br.setValue(bo,bp,this.getUserData(s));
});
bs.add(bt);
}return true;
},_contextMenuHandlerNull:function(c,d,f,g,h){return true;
},createControls:function(){var bh=new qx.ui.toolbar.ToolBar();
var bf,bg,bi;
bg=new qx.ui.toolbar.Part();
bh.add(bg);
bf=new qx.ui.toolbar.Button(m,r);
bf.addListener(u,function(bc){var bd=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,bd[0][i]);
}this.info("Row 10 changed");
},this);
bg.add(bf);
bf=new qx.ui.toolbar.Button(L,C);
bf.addListener(u,function(a){var b=this.createRandomRows(10);
this._tableModel.addRows(b);
this.info("10 rows added");
},this);
bg.add(bf);
bf=new qx.ui.toolbar.Button(M,G);
bf.addListener(u,function(j){var k=this._tableModel.getRowCount();
this._tableModel.removeRows(k-5,5);
this.info("5 rows removed");
},this);
bg.add(bf);
bf=new qx.ui.toolbar.Button(v,o);
bf.addListener(u,function(V){var W=[];
be.getSelectionModel().iterateSelection(function(N){W.push(N+J);
});
this.showDialog(z+W.join(E));
},this);
bg.add(bf);
bg=new qx.ui.toolbar.Part();
bh.add(bg);
var be=this._table;
bi=new qx.ui.toolbar.CheckBox(A);
bi.set({value:be.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(F+y)});
bi.addListener(t,function(bn){be.setKeepFirstVisibleRowComplete(this.getValue());
},bi);
bg.add(bi);
bi=new qx.ui.toolbar.CheckBox(q);
bi.set({value:be.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(w)});
bi.addListener(t,function(S){if(S.getData()){var U=function(bj,bk){var bl=bj[arguments.callee.columnIndex];
var bm=bk[arguments.callee.columnIndex];

if(bl%2==1&&bm%2==0){return 1;
}
if(bm%2==1&&bl%2==0){return -1;
}return (bl>bm)?1:((bl==bm)?0:-1);
};
var T=function(X,Y){var ba=X[arguments.callee.columnIndex];
...
be.getTableModel().setSortMethods(0,{ascending:U,descending:T});
}else{be.getTableModel().setSortMethods(0,null);
}},bi);
bg.add(bi);
return bh;
...
(function(){var d="contextmenu",c="qx.ui.table.MTableContextMenu",b="cellContextmenu";
qx.Mixin.define(c,{construct:function(){this.addListener(b,function(e){if(this._contextMenuTimer){this._contextMenuTimer.dispose();
...
this._contextMenu.addListener(d,function(e){e.preventDefault();
});
...
},getContextMenuHandler:function(a){return this.__Aq[a];
