...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.table.Table_Selection-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"ab937b6f5490"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var p="execute",o="value",n="",m="A date",l="Boolean",k="demobrowser.demo.table.Table_Selection",j="changeValue",i="icon/22/status/dialog-information.png",h="A number",g="icon/16/apps/office-calendar.png",E="Reset Cell Focus",D="changeSelection",C="middle",B="Remove 5 rows",A="Reset Selection",z="_tableModel",y="Table",x="Add 5 rows",w="icon/22/actions/list-remove.png",v=", ",t="icon/22/actions/list-add.png",u="Hide Cell Focus",r="Selected rows:<br>",s="Show selection",q="ID";
qx.Class.define(k,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return y;
},createTable:function(){var H=this.createRandomRows(50);
var G=this._tableModel=new qx.ui.table.model.Simple();
G.setColumns([q,h,m,l]);
G.setData(H);
G.setColumnEditable(1,true);
G.setColumnEditable(2,true);
G.setColumnSortable(3,false);
var I=new qx.ui.table.Table(G);
I.set({width:600,height:400});
I.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var F=I.getTableColumnModel();
F.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
F.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(g,m));
return I;
},createControls:function(){var N=new qx.ui.toolbar.ToolBar();
var P,R;
R=new qx.ui.toolbar.Part();
N.add(R);
var M=new qx.ui.form.SelectBox().set({allowGrowY:false,alignY:C});
...
Q.setUserData(o,O[T]+n);
M.add(Q);
}M.addListener(D,function(e){this._table.getSelectionModel().setSelectionMode(parseInt(e.getData()[0].getUserData(o)));
},this);
R.add(M);
R=new qx.ui.toolbar.Part();
N.add(R);
P=new qx.ui.toolbar.Button(x,t);
P.addListener(p,function(a){var b=this.createRandomRows(10);
this._tableModel.addRows(b);
this.info("5 rows added");
},this);
R.add(P);
P=new qx.ui.toolbar.Button(B,w);
P.addListener(p,function(J){var K=this._tableModel.getRowCount();
this._tableModel.removeRows(K-5,5);
this.info("5 rows removed");
},this);
R.add(P);
R=new qx.ui.toolbar.Part();
N.add(R);
P=new qx.ui.toolbar.Button(A);
P.addListener(p,function(W){this._table.resetSelection();
},this);
R.add(P);
P=new qx.ui.toolbar.Button(E);
P.addListener(p,function(f){this._table.resetCellFocus();
},this);
R.add(P);
var S=new qx.ui.toolbar.CheckBox(u);
S.addListener(j,function(V){this._table.setShowCellFocusIndicator(!S.isValue());
},this);
R.add(S);
P=new qx.ui.toolbar.Button(s,i);
P.addListener(p,function(c){var d=[];
this._table.getSelectionModel().iterateSelection(function(L){d.push(L+n);
});
this.showDialog(r+d.join(v));
},this);
R.add(P);
return N;
