...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.ui.MultiPageForm","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.ui.MultiPageForm-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"9de779cbaf49"},
  boot : "boot",
  bootIsInline : true,
...
qx.Class.define(E,{extend:qx.ui.form.ComboBox,implement:[qx.ui.form.IDateForm],construct:function(){arguments.callee.base.call(this);
...
}},properties:{appearance:{refine:true,init:D},dateFormat:{check:t,apply:J}},statics:{__wM:null,__wN:null,getDefaultDateFormatter:function(){var d=qx.locale.Date.getDateFormat(G).toString();

if(d==this.__wM){return this.__wN;
}
if(this.__wN){this.__wN.dispose();
}this.__wN=new qx.util.format.DateFormat(d,qx.locale.Manager.getInstance().getLocale());
this.__wM=d;
return this.__wN;
}},members:{setValue:function(a){var b=this.getChildControl(y);
b.setValue(this.getDateFormat().format(a));
var c=this.getChildControl(z);
c.setValue(a);
},getValue:function(){var Q=this.getChildControl(y).getValue();
try{return this.getDateFormat().parse(Q);
}catch(T){return null;
}},resetValue:function(){var f=this.getChildControl(y);
f.setValue(v);
var g=this.getChildControl(z);
g.setValue(null);
},_applyDateFormat:function(h,i){try{var k=this.getChildControl(y);
var l=k.getValue();
var j=i.parse(l);
k.setValue(h.format(j));
}catch(W){}},_createChildControlImpl:function(R){var S;

switch(R){case z:S=new qx.ui.control.DateChooser();
S.setFocusable(false);
S.setKeepFocus(true);
S.addListener(L,this._onChangeDate,this);
break;
case x:S=new qx.ui.popup.Popup(new qx.ui.layout.VBox);
S.setAutoHide(false);
S.add(this.getChildControl(z));
S.addListener(F,this._onChangeDate,this);
S.addListener(I,this._onPopupChangeVisibility,this);
break;
}return S||arguments.callee.base.call(this,R);
},_onChangeDate:function(e){var U=this.getChildControl(y);
var V=this.getChildControl(z).getValue();
U.setValue(this.getDateFormat().format(V));
this.close();
},_onKeyPress:function(e){var M=e.getKeyIdentifier();

if(M==w&&e.isAltPressed()){this.toggle();
e.stopPropagation();
return;
}var N=this.getChildControl(x);

if(N.getVisibility()==C){return;
}if(M==u){this.close();
e.stopPropagation();
return;
}if(M===r||M===B||M===w||M===q){e.preventDefault();
}this.getChildControl(z).handleKeyPress(e);
},_onPopupChangeVisibility:function(e){var o=this.getChildControl(x);

if(o.isVisible()){var p=this.getChildControl(z);
var n=this.getValue();
p.setValue(n);
}},_onTextFieldChangeValue:function(e){var O=this.getValue();

if(O!=null){var P=this.getChildControl(z);
P.setValue(O);
}this.fireDataEvent(s,this.getValue());
},isEmpty:function(){var m=this.getChildControl(y).getValue();
return m==null||m==v;
}}});
})();
...
(function(){var s="selected",r="click",q="last-month-button",p="default",o="next-month-button",n="otherMonth",m="month-year-label",l="last-year-button",k="navigation-bar",j="next-year-button",bs="date-pane",br="PageUp",bq="mousedown",bp="today",bo="Integer",bn="PageDown",bm="changeLocale",bl="next-month-button-tooltip",bk="last-month-button-tooltip",bj="qx.dynlocale",B="last-year-button-tooltip",C="next-year-button-tooltip",z="on",A="weekend",v="day",w="lastMonth",t="Next month",u="Escape",F="Left",G="weekday",O="changeValue",M="Space",W="Down",R="qx.ui.control.DateChooser",bf="Date",bc="Enter",I="dblclick",bi="day#",bh="Next year",bg="ww",H="_applyValue",K="Up",L="weekday#",N="datechooser",P="header",S="week",Y="lastYear",be="nextYear",D="changeShownYear",E="week#",J="Last month",V="Right",U="Last year",T="EE",bb="mouseup",ba="keypress",Q="",X="nextMonth",h="week#0",bd="changeShownMonth";
qx.Class.define(R,{extend:qx.ui.core.Widget,include:[qx.ui.core.MExecutable,qx.ui.form.MForm],implement:[qx.ui.form.IExecutable,qx.ui.form.IForm,qx.ui.form.IDateForm],construct:function(by){arguments.callee.base.call(this);
var bA=new qx.ui.layout.VBox();
this._setLayout(bA);
this._createChildControl(k);
this._createChildControl(bs);
this.addListener(ba,this._onKeyPress);
var bz=(by!=null)?by:new Date();
this.showMonth(bz.getMonth(),bz.getFullYear());
if(qx.core.Variant.isSet(bj,z)){qx.locale.Manager.getInstance().addListener(bm,this._updateDatePane,this);
}this.addListener(bq,this._onMouseUpDown,this);
this.addListener(bb,this._onMouseUpDown,this);
},statics:{MONTH_YEAR_FORMAT:qx.locale.Date.getDateTimeFormat("yyyyMMMM","MMMM yyyy")},properties:{appearance:{refine:true,init:N},width:{refine:true,init:200},height:{refine:true,init:150},shownMonth:{check:bo,init:null,nullable:true,event:bd},shownYear:{check:bo,init:null,nullable:true,event:D},value:{check:bf,init:null,nullable:true,event:O,apply:H}},members:{__xc:null,__xd:null,__xe:null,_forwardStates:{invalid:true},_createChildControlImpl:function(ch){var ci;

switch(ch){case k:ci=new qx.ui.container.Composite(new qx.ui.layout.HBox());
ci.add(this.getChildControl(l));
ci.add(this.getChildControl(q));
ci.add(this.getChildControl(m),{flex:1});
ci.add(this.getChildControl(o));
ci.add(this.getChildControl(j));
this._add(ci);
break;
case B:ci=new qx.ui.tooltip.ToolTip(this.tr(U));
break;
case l:ci=new qx.ui.form.Button();
ci.addState(Y);
ci.setFocusable(false);
ci.setToolTip(this.getChildControl(B));
ci.addListener(r,this._onNavButtonClicked,this);
break;
case bk:ci=new qx.ui.tooltip.ToolTip(this.tr(J));
break;
case q:ci=new qx.ui.toolbar.Button();
ci.addState(w);
ci.setFocusable(false);
ci.setToolTip(this.getChildControl(bk));
ci.addListener(r,this._onNavButtonClicked,this);
break;
case bl:ci=new qx.ui.tooltip.ToolTip(this.tr(t));
break;
case o:ci=new qx.ui.toolbar.Button();
ci.addState(X);
ci.setFocusable(false);
ci.setToolTip(this.getChildControl(bl));
ci.addListener(r,this._onNavButtonClicked,this);
break;
case C:ci=new qx.ui.tooltip.ToolTip(this.tr(bh));
break;
case j:ci=new qx.ui.toolbar.Button();
ci.addState(be);
ci.setFocusable(false);
ci.setToolTip(this.getChildControl(C));
ci.addListener(r,this._onNavButtonClicked,this);
break;
case m:ci=new qx.ui.basic.Label();
ci.setAllowGrowX(true);
ci.setAnonymous(true);
break;
case S:ci=new qx.ui.basic.Label();
ci.setAllowGrowX(true);
ci.setAllowGrowY(true);
ci.setSelectable(false);
ci.setAnonymous(true);
ci.setCursor(p);
break;
case G:ci=new qx.ui.basic.Label();
ci.setAllowGrowX(true);
ci.setAllowGrowY(true);
ci.setSelectable(false);
ci.setAnonymous(true);
ci.setCursor(p);
break;
case v:ci=new qx.ui.basic.Label();
ci.setAllowGrowX(true);
ci.setAllowGrowY(true);
ci.setCursor(p);
ci.addListener(bq,this._onDayClicked,this);
ci.addListener(I,this._onDayDblClicked,this);
break;
case bs:var cj=new qx.ui.layout.Grid();
ci=new qx.ui.container.Composite(cj);

for(var i=0;i<8;i++){cj.setColumnFlex(i,1);
}
for(var i=0;i<7;i++){cj.setRowFlex(i,1);
}var ck=this.getChildControl(h);
ck.addState(P);
ci.add(ck,{column:0,row:0});
this.__xc=[];

for(var i=0;i<7;i++){ck=this.getChildControl(L+i);
ci.add(ck,{column:i+1,row:0});
this.__xc.push(ck);
}this.__xd=[];
this.__xe=[];

for(var y=0;y<6;y++){var ck=this.getChildControl(E+(y+1));
ci.add(ck,{column:0,row:y+1});
this.__xe.push(ck);
for(var x=0;x<7;x++){var ck=this.getChildControl(bi+((y*7)+x));
ci.add(ck,{column:x+1,row:y+1});
this.__xd.push(ck);
}}this._add(ci);
break;
...
for(var i=0;i<6*7;i++){var bv=this.__xd[i];

if(bv.hasState(n)){if(bv.hasState(s)){bv.removeState(s);
}}else{var bx=parseInt(bv.getValue());

if(bx==bw){bv.addState(s);
}else if(bv.hasState(s)){bv.removeState(s);
}}}}},_onMouseUpDown:function(e){var a=e.getTarget();

if(a==this.getChildControl(k)||a==this.getChildControl(bs)){e.stopPropagation();
return;
}},_onNavButtonClicked:function(cl){var cn=this.getShownYear();
var cm=this.getShownMonth();

switch(cl.getCurrentTarget()){case this.getChildControl(l):cn--;
break;
case this.getChildControl(q):cm--;

if(cm<0){cm=11;
cn--;
}break;
case this.getChildControl(o):cm++;

if(cm>=12){cm=0;
cn++;
}break;
case this.getChildControl(j):cn++;
break;
...
},_onKeyPress:function(b){var g=null;
var d=null;
var f=null;

if(b.getModifiers()==0){switch(b.getKeyIdentifier()){case F:g=-1;
break;
case V:g=1;
break;
case K:g=-7;
...
case bn:d=1;
break;
case u:if(this.getValue()!=null){this.setValue(null);
return true;
}break;
case bc:case M:if(this.getValue()!=null){this.execute();
}return;
}}else if(b.isShiftPressed()){switch(b.getKeyIdentifier()){case br:f=-1;
break;
case bn:f=1;
break;
}}
if(g!=null||d!=null||f!=null){var c=this.getValue();

if(c!=null){c=new Date(c.getTime());
}
if(c==null){c=new Date();
}else{if(g!=null){c.setDate(c.getDate()+g);
}
if(d!=null){c.setMonth(c.getMonth()+d);
}
if(f!=null){c.setFullYear(c.getFullYear()+f);
}}this.setValue(c);
}},showMonth:function(bD,bE){if((bD!=null&&bD!=this.getShownMonth())||(bE!=null&&bE!=this.getShownYear())){if(bD!=null){this.setShownMonth(bD);
}
if(bE!=null){this.setShownYear(bE);
}this._updateDatePane();
}},handleKeyPress:function(e){this._onKeyPress(e);
},_updateDatePane:function(){var bU=qx.ui.control.DateChooser;
var bR=new Date();
var bK=bR.getFullYear();
var bP=bR.getMonth();
var bN=bR.getDate();
var bV=this.getValue();
var bY=(bV==null)?-1:bV.getFullYear();
var cg=(bV==null)?-1:bV.getMonth();
var bS=(bV==null)?-1:bV.getDate();
var bO=this.getShownMonth();
var cd=this.getShownYear();
var bL=qx.locale.Date.getWeekStart();
var bW=new Date(this.getShownYear(),this.getShownMonth(),1);
var bT=new qx.util.format.DateFormat(bU.MONTH_YEAR_FORMAT);
this.getChildControl(m).setValue(bT.format(bW));
var cf=bW.getDay();
var bQ=1+((7-cf)%7);
var bX=new qx.util.format.DateFormat(T);

for(var i=0;i<7;i++){var ca=(i+bL)%7;
var cc=this.__xc[i];
bW.setDate(bQ+ca);
cc.setValue(bX.format(bW));

if(qx.locale.Date.isWeekend(ca)){cc.addState(A);
}else{cc.removeState(A);
}}bW=new Date(cd,bO,1,12,0,0);
var bG=(7+cf-bL)%7;
bW.setDate(bW.getDate()-bG);
var cb=new qx.util.format.DateFormat(bg);

for(var bF=0;bF<6;bF++){this.__xe[bF].setValue(cb.format(bW));

for(var i=0;i<7;i++){var cc=this.__xd[bF*7+i];
var bJ=bW.getFullYear();
var bI=bW.getMonth();
var bM=bW.getDate();
var bH=(bY==bJ&&cg==bI&&bS==bM);

if(bH){cc.addState(s);
}else{cc.removeState(s);
}
if(bI!=bO){cc.addState(n);
}else{cc.removeState(n);
}var ce=(bJ==bK&&bI==bP&&bM==bN);

if(ce){cc.addState(bp);
}else{cc.removeState(bp);
}cc.setValue(Q+bM);
cc.dateTime=bW.getTime();
bW.setDate(bW.getDate()+1);
}}bT.dispose();
bX.dispose();
cb.dispose();
}},destruct:function(){if(qx.core.Variant.isSet(bj,z)){qx.locale.Manager.getInstance().removeListener(bm,this._updateDatePane,this);
}this.__xc=this.__xd=this.__xe=null;
}});
})();
...
(function(){var b="qx.data.marshal.IMarshaler";
qx.Interface.define(b,{members:{toClass:function(c,d){},toModel:function(a){}}});
})();
(function(){var s="qx.data.model.",r="",q='"',p="change",o="qx.data.marshal.Json",n="set",m="_applyEventPropagation";
qx.Class.define(o,{extend:qx.core.Object,implement:[qx.data.marshal.IMarshaler],construct:function(z){arguments.callee.base.call(this);
this.__vO=z;
},statics:{__vP:null,createModel:function(a,b){if(this.__vP===null){this.__vP=new qx.data.marshal.Json();
}this.__vP.toClass(a,b);
return this.__vP.toModel(a);
}},members:{__vO:null,__vQ:function(t){var u=[];

for(var v in t){u.push(v);
}return u.sort().join(q);
},toClass:function(c,d){if(qx.lang.Type.isNumber(c)||qx.lang.Type.isString(c)||qx.lang.Type.isBoolean(c)||c==null){return;
}if(qx.lang.Type.isArray(c)){for(var i=0;i<c.length;i++){this.toClass(c[i],d);
}return ;
}var f=this.__vQ(c);
if(this.__vO&&this.__vO.getModelClass&&this.__vO.getModelClass(f)!=null){return;
}for(var k in c){this.toClass(c[k],d);
}if(qx.Class.isDefined(s+f)){return;
}var l={};

for(var k in c){k=k.replace(/-/g,r);
l[k]={};
l[k].nullable=true;
l[k].event=p+qx.lang.String.firstUp(k);

if(d){l[k].apply=m;
}}if(this.__vO&&this.__vO.getModelSuperClass){var j=this.__vO.getModelSuperClass(f)||qx.core.Object;
}else{var j=qx.core.Object;
}var g=[];

if(this.__vO&&this.__vO.getModelMixins){var h=this.__vO.getModelMixins(f);
if(!qx.lang.Type.isArray(h)){if(h!=null){g=[h];
}}}if(d){g.push(qx.data.marshal.MEventBubbling);
}var e={extend:j,include:g,properties:l};
qx.Class.define(s+f,e);
},__vR:function(w){var x;
if(this.__vO&&this.__vO.getModelClass){x=this.__vO.getModelClass(w);
}
if(x!=null){return (new x());
}else{var y=qx.Class.getByName(s+w);
return (new y());
}},toModel:function(A){if(qx.lang.Type.isNumber(A)||qx.lang.Type.isString(A)||qx.lang.Type.isBoolean(A)||qx.lang.Type.isDate(A)||A==null){return A;
}else if(qx.lang.Type.isArray(A)){var E=new qx.data.Array();

for(var i=0;i<A.length;i++){E.push(this.toModel(A[i]));
}return E;
}else if(qx.lang.Type.isObject(A)){var B=this.__vQ(A);
var F=this.__vR(B);
for(var D in A){var C=D.replace(/-/g,r);
F[n+qx.lang.String.firstUp(C)](this.toModel(A[D]));
}return F;
...
qx.Class.define(e,{statics:{toUriParameter:function(y,z){var B=q;
var C=qx.util.PropertyUtil.getProperties(y.constructor);

for(var name in C){var A=y[p+qx.lang.String.firstUp(name)]();
if(qx.lang.Type.isArray(A)){for(var i=0;i<A.length;i++){B+=this.__xo(name,A[i],z);
}}else{B+=this.__xo(name,A,z);
}}return B.substring(0,B.length-1);
},__xo:function(name,v,w){if(v instanceof qx.core.Object&&w!=null){var x=encodeURIComponent(w(v));

if(x===undefined){var x=encodeURIComponent(v);
}}else{var x=encodeURIComponent(v);
}return encodeURIComponent(name)+f+x+l;
},toNativeObject:function(K,L){var O;
if(K==null){return null;
}if(qx.Class.hasInterface(K.constructor,qx.data.IListData)){O=[];

for(var i=0;i<K.getLength();i++){O.push(qx.util.Serializer.toNativeObject(K.getItem(i),L));
}return O;
}if(qx.lang.Type.isArray(K)){O=[];

for(var i=0;i<K.length;i++){O.push(qx.util.Serializer.toNativeObject(K[i],L));
}return O;
}if(K instanceof qx.core.Object){if(L!=null){var P=L(K);
if(P!=undefined){return P;
}}O={};
var Q=qx.util.PropertyUtil.getProperties(K.constructor);

for(var name in Q){if(Q[name].group!=undefined){continue;
}var N=K[p+qx.lang.String.firstUp(name)]();
O[name]=qx.util.Serializer.toNativeObject(N,L);
}return O;
}if(qx.lang.Type.isObject(K)){O={};

for(var M in K){O[M]=qx.util.Serializer.toNativeObject(K[M],L);
}return O;
}return K;
},toJson:function(D,E){var H=q;
if(D==null){return d;
}else if(qx.Class.hasInterface(D.constructor,qx.data.IListData)){H+=s;

for(var i=0;i<D.getLength();i++){H+=qx.util.Serializer.toJson(D.getItem(i),E)+r;
}
if(H!=s){H=H.substring(0,H.length-1);
}return H+n;
}else if(qx.lang.Type.isArray(D)){H+=s;

for(var i=0;i<D.length;i++){H+=qx.util.Serializer.toJson(D[i],E)+r;
}
if(H!=s){H=H.substring(0,H.length-1);
}return H+n;
}else if(D instanceof qx.core.Object){if(E!=null){var I=E(D);
if(I!=undefined){return u+I+u;
}}H+=t;
var J=qx.util.PropertyUtil.getProperties(D.constructor);

for(var name in J){if(J[name].group!=undefined){continue;
}var G=D[p+qx.lang.String.firstUp(name)]();
H+=u+name+m+qx.util.Serializer.toJson(G,E)+r;
}
if(H!=t){H=H.substring(0,H.length-1);
}return H+o;
}else if(qx.lang.Type.isObject(D)){H+=t;

for(var F in D){H+=u+F+m+qx.util.Serializer.toJson(D[F],E)+r;
}
if(H!=t){H=H.substring(0,H.length-1);
}return H+o;
}else if(qx.lang.Type.isString(D)){D=D.replace(/([\\])/g,a);
D=D.replace(/(["])/g,g);
D=D.replace(/([\r])/g,h);
D=D.replace(/([\f])/g,j);
D=D.replace(/([\n])/g,c);
...
D=D.replace(/([\b])/g,b);
return u+D+u;
}else if(qx.lang.Type.isDate(D)||qx.lang.Type.isRegExp(D)){return u+D+u;
}return D+q;
}}});
})();
...
(function(){var Y="",X="complete",W="changeValid",V="qx.event.type.Event",U="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",T="qx.ui.form.validation.Manager",S="This field is required",R="String",Q="qx.event.type.Data";
qx.Class.define(T,{extend:qx.core.Object,construct:function(){arguments.callee.base.call(this);
this.__wo=[];
this.__wp={};
},events:{"changeValid":Q,"complete":V},properties:{validator:{check:U,init:null,nullable:true},invalidMessage:{check:R,init:Y}},members:{__wo:null,__wq:null,__wp:null,__wr:null,add:function(N,O){if(!this.__ww(N)){throw new Error("Added widget not supported.");
}if(this.__wx(N)){if(O!=null){throw new Error("Widgets suporting selection can only be validated "+"in the form validator");
}}var P={item:N,validator:O,valid:null};
this.__wo.push(P);
},validate:function(){var L=true;
this.__wr=true;
var I=[];
for(var i=0;i<this.__wo.length;i++){var J=this.__wo[i].item;
var M=this.__wo[i].validator;
I.push(J);
if(M==null){var H=this.__ws(J);
L=L&&H;
this.__wr=H&&this.__wr;
continue;
}var H=this.__wt(this.__wo[i],J.getValue());
L=H&&L;

if(H!=null){this.__wr=H&&this.__wr;
}}var K=this.__wu(I);

if(qx.lang.Type.isBoolean(K)){this.__wr=K&&this.__wr;
}L=K&&L;
this.__wz(L);

if(qx.lang.Object.isEmpty(this.__wp)){this.fireEvent(X);
}return L;
},__ws:function(r){if(r.getRequired()){if(this.__wx(r)){var s=!!r.getSelection()[0];
}else{var s=!!r.getValue();
}r.setValid(s);
r.setInvalidMessage(S);
return s;
}return true;
},__wt:function(g,h){var m=g.item;
var l=g.validator;
if(this.__wv(l)){this.__wp[m.toHashCode()]=null;
l.validate(m,m.getValue(),this);
return null;
}var k=null;
...
g.valid=k;
return k;
},__wu:function(C){var E=this.getValidator();

if(E==null){return true;
}this.setInvalidMessage(Y);

if(this.__wv(E)){this.__wp[this.toHashCode()]=null;
E.validateForm(C,this);
return null;
}
try{var F=E(C,this);
...
if(e.message&&e.message!=qx.type.BaseError.DEFAULTMESSAGE){var D=e.message;
}else{var D=e.getComment();
}this.setInvalidMessage(D);
}else{throw e;
}}return F;
},__wv:function(t){var u=false;

if(!qx.lang.Type.isFunction(t)){u=qx.Class.isSubClassOf(t.constructor,qx.ui.form.validation.AsyncValidator);
}return u;
},__ww:function(z){var A=z.constructor;
return qx.Class.hasInterface(A,qx.ui.form.IForm);
},__wx:function(a){var b=a.constructor;
return qx.Class.hasInterface(b,qx.ui.core.ISingleSelection);
},__wy:function(n){var o=n.constructor;
...
},__wz:function(x){var y=this.__wq;
this.__wq=x;
if(y!=x){this.fireDataEvent(W,x,y);
}},getValid:function(){return this.__wq;
},isValid:function(){return this.getValid();
},getInvalidMessages:function(){var p=[];
for(var i=0;i<this.__wo.length;i++){var q=this.__wo[i].item;

if(!q.getValid()){p.push(q.getInvalidMessage());
}}if(this.getInvalidMessage()!=Y){p.push(this.getInvalidMessage());
}return p;
},reset:function(){for(var i=0;i<this.__wo.length;i++){var B=this.__wo[i];
B.item.setValid(true);
}this.__wq=null;
},setItemValid:function(v,w){this.__wp[v.toHashCode()]=w;
v.setValid(w);
this.__wA();
},setFormValid:function(G){this.__wp[this.toHashCode()]=G;
this.__wA();
},__wA:function(){var d=this.__wr;
for(var f in this.__wp){var c=this.__wp[f];
d=c&&d;
if(c==null){return;
}}this.__wz(d);
this.__wp={};
...
qx.Class.define(d,{extend:qx.core.Object,construct:function(i){arguments.callee.base.call(this);
this.__wB=i;
},members:{__wB:null,__wC:null,__wD:null,__wE:null,validate:function(a,b,c){this.__wE=false;
this.__wC=a;
this.__wD=c;
this.__wB(this,b);
},validateForm:function(e,f){this.__wE=true;
this.__wD=f;
this.__wB(e,this);
},setValid:function(g,h){if(this.__wE){if(h!==undefined){this.__wD.setInvalidMessage(h);
}this.__wD.setFormValid(g);
}else{if(h!==undefined){this.__wC.setInvalidMessage(h);
}this.__wD.setItemValid(this.__wC,g);
}}},destruct:function(){this.__wD=this.__wC=null;
}});
})();
...
},checkNumber:function(a){if((typeof a!==o&&(!(a instanceof Number)))||(!(isFinite(a)))){throw new qx.core.ValidationError(n,a+c);
}},email:function(){return qx.util.Validate.checkEmail;
},checkEmail:function(y){var z=/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;

if(z.test(y)===false){throw new qx.core.ValidationError(t,r+(y||s)+i);
}},string:function(){return qx.util.Validate.checkString;
},checkString:function(G){if(typeof G!==k&&(!(G instanceof String))){throw new qx.core.ValidationError(t,G+d);
}},url:function(){return qx.util.Validate.checkUrl;
},checkUrl:function(v){var w=/([A-Za-z0-9])+:\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;

if(w.test(v)===false){throw new qx.core.ValidationError(t,v+p);
}},color:function(){return qx.util.Validate.checkColor;
},checkColor:function(D){try{qx.util.ColorUtil.stringToRgb(D);
}catch(e){throw new qx.core.ValidationError(t,D+q+e);
}},range:function(E,F){return function(u){if(u<E||u>F){throw new qx.core.ValidationError(t,u+h+E+f+F+g);
}};
},inArray:function(B){return function(x){if(B.indexOf(x)===-1){throw new qx.core.ValidationError(t,x+m+B+s);
}};
},regExp:function(C){return function(A){if(C.test(A)===false){throw new qx.core.ValidationError(t,A+j+C+b);
}};
}}});
})();
