...
(function(){var x="modelSelection[0]",w="value",v="changeModel",u="qx.core.Object",t="_applyTarget",s="qx.data.controller.Form",r="changeTarget",q=".",p="qx.ui.form.Form",o="_applyModel";
qx.Class.define(s,{extend:qx.core.Object,construct:function(e,f){arguments.callee.base.call(this);
this.__xf={};

if(e!=null){this.setModel(e);
}
if(f!=null){this.setTarget(f);
}},properties:{model:{check:u,apply:o,event:v,nullable:true},target:{check:p,apply:t,event:r,nullable:true,init:null}},members:{__xg:null,__xf:null,addBindingOptions:function(name,I,J){this.__xf[name]=[I,J];
if(this.getModel()==null||this.getTarget()==null){return;
}var K=this.getTarget().getItems()[name];
var L=this.__xj(K)?x:w;
this.__xg.removeTarget(K,L,name);
this.__xg.addTarget(K,L,name,true,I,J);
},createModel:function(y){var z=this.getTarget();
if(z==null){throw new Error("No target is set.");
}var A=z.getItems();
var B={};

for(var name in A){var C=name.split(q);
var F=B;

for(var i=0;i<C.length;i++){if(i+1==C.length){var E=A[name].constructor;

if(qx.Class.hasInterface(E,qx.ui.core.ISingleSelection)){F[C[i]]=A[name].getModelSelection();
}else{F[C[i]]=A[name].getValue();
}}else{if(!F[C[i]]){F[C[i]]={};
}F=F[C[i]];
}}}var D=qx.data.marshal.Json.createModel(B,y);
this.setModel(D);
return D;
},_applyTarget:function(h,j){if(j!=null){this.__xi(j);
}if(this.getModel()==null){return;
}if(h!=null){this.__xh();
}},_applyModel:function(G,H){if(this.__xg!=null){this.__xg.setModel(G);
}if(this.getTarget()==null){return;
}if(G!=null){this.__xh();
}},__xh:function(){if(this.__xg==null){this.__xg=new qx.data.controller.Object(this.getModel());
}var k=this.getTarget().getItems();
for(var name in k){var n=k[name];
var l=this.__xj(n)?x:w;
var m=this.__xf[name];

if(m==null){this.__xg.addTarget(n,l,name,true);
}else{this.__xg.addTarget(n,l,name,true,m[0],m[1]);
}}},__xi:function(a){if(this.__xg==null){return;
}var b=a.getItems();
for(var name in b){var d=b[name];
var c=this.__xj(d)?x:w;
this.__xg.removeTarget(d,c,name);
}},__xj:function(g){return qx.Class.hasInterface(g.constructor,qx.ui.core.ISingleSelection)&&qx.Class.hasInterface(g.constructor,qx.ui.form.IModelSelection);
}}});
})();
