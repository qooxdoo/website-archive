...
(function(){var p="qooxdoo",o="/",n=".static.flickr.com/",m="http://api.flickr.com/services/rest/?tags=",l="",k="_",j="http://farm",i="Enter",h="keydown",g="model.photos.photo",L=".jpg",K="loaded",J="execute",I="source",H="selection[0]",G="_s.jpg",F="<span style='font-size: 20px'>Flickr</span>",E="failed",D="loading",C="value",w="demobrowser.demo.data.store.Flickr",x="demobrowser.demo.data.Flickr",u="title",v="icon",s="state",t="completed",q="Search",r="receiving",y="horizontal",z="model",B="&method=flickr.photos.search&api_key=63a8042eead205f7e0040f488c02afd9&format=json",A="id";
qx.Class.define(x,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var V=new demobrowser.demo.data.store.Flickr(p);
var N=new qx.ui.form.TextField(p);
this.getRoot().add(N,{left:30,top:50});
var O=new qx.ui.form.Button(q);
this.getRoot().add(O,{left:140,top:49});
O.addListener(J,function(){V.searchForTag(N.getValue());
},this);
N.addListener(h,function(e){if(e.getKeyIdentifier()==i){V.searchForTag(N.getValue());
}},this);
...
T.setLabelPath(u);
T.setDelegate({configureItem:function(f){f.setShow(v);
}});
var P={converter:function(W,X){return (j+X.getFarm()+n+X.getServer()+o+W+k+X.getSecret()+G);
}};
T.setIconOptions(P);
T.setIconPath(A);
V.bind(g,T,z);
...
var U={converter:function(M){if(M){return (j+M.getFarm()+n+M.getServer()+o+M.getId()+k+M.getSecret()+L);
}return l;
}};
T.bind(H,Q,I,U);
...
qx.Class.define(w,{extend:qx.data.store.Json,statics:{RESULT:null},construct:function(b){jsonFlickrApi=function(d){demobrowser.demo.data.store.Flickr.RESULT=d;
};
var c=m+b;
arguments.callee.base.call(this,c);
},members:{searchForTag:function(bb){if(bb!=l){this.setUrl(m+bb);
}},_createRequest:function(Y){var ba=new qx.io.ScriptLoader();
this.setState(r);
Y+=B;
ba.load(Y,function(a){this.__wh();
},this);
},__wh:function(){this.setState(t);
var bc=demobrowser.demo.data.store.Flickr.RESULT;

if(bc==undefined){this.setState(E);
return;
