...
(function(){var s="qx.data.Array",r="demobrowser.demo.data.model.Node",q="String",p="changeNames",o="changeName",n="__vS",m="",l="Validation Error: ",k="changeChild",j="- ",c="changeColor",h="Marge",f="AFFE",b="is no number (parseFloat says so!).",a="Homer",e="\n",d="changeName2",g="changeChildren";
...
},properties:{children:{check:s,event:g},child:{check:r,event:k,nullable:true},names:{check:s,event:p,init:new qx.data.Array(a,h)},name:{check:q,init:f,event:o},name2:{check:q,event:d},number:{init:10,validate:n},color:{event:c,nullable:true}},members:{__vS:function(v){if(!isNaN(parseFloat(v))){if(/^\d+$/gi.test(v)){return;
}}throw new qx.core.ValidationError(l+v+b);
},toString:function(t){if(t==undefined){t=0;
}var u=m;

for(var i=0;i<t;i++){u+=j;
}u+=this.getName();

for(var i=0;i<this.getChildren().length;i++){u+=e+this.getChildren().getItem(i).toString(t+1);
}return u;
}}});
})();
...
(function(){var bS="get",bR="String",bQ="changeModel",bP="_applyDelegate",bO="qx.core.Object",bN="_applyLabelPath",bM="icon",bL="change",bK="label",bJ="_applyTarget",bE="changeTarget",bI="_applyIconPath",bH="_applyLabelOptions",bD="_applyChildPath",bC="_applyIconOptions",bG="qx.data.controller.Tree",bF="_applyModel";
qx.Class.define(bG,{extend:qx.core.Object,include:qx.data.controller.MSelection,construct:function(bg,bh,bi,bj){arguments.callee.base.call(this);
this.__vT={};
this.__vU=[];
this.__vV={};

if(bi!=null){this.setChildPath(bi);
}
if(bj!=null){this.setLabelPath(bj);
}
if(bg!=null){this.setModel(bg);
}
if(bh!=null){this.setTarget(bh);
}},properties:{model:{check:bO,apply:bF,event:bQ,nullable:true},target:{apply:bJ,event:bE,init:null},childPath:{check:bR,apply:bD,nullable:true},labelPath:{check:bR,apply:bN,nullable:true},iconPath:{check:bR,apply:bI,nullable:true},labelOptions:{apply:bH,nullable:true},iconOptions:{apply:bC,nullable:true},delegate:{apply:bP,init:null,nullable:true}},members:{__vV:null,__vT:null,__vU:null,_applyDelegate:function(ba,bb){this._setConfigureItem(ba,bb);
this._setCreateItem(ba,bb);
this._setBindItem(ba,bb);
},_applyIconOptions:function(f,g){this.__we();
},_applyLabelOptions:function(by,bz){this.__we();
},_applyTarget:function(L,M){if(M!=undefined){var N=M.getRoot();
M.setRoot(null);
N.destroy();
}if(this.getModel()!=null){this.__vY();
}this._addChangeTargetListener(L,M);
},_applyModel:function(bw,bx){this.__vY();
},_applyChildPath:function(J,K){this.__vY();
},_applyIconPath:function(bX,bY){this.__we();
},_applyLabelPath:function(H,I){this.__we();
},__vW:function(bc){var be=bc.getTarget();
var bd=this.__vV[be.toHashCode()].treeNode;
var bf=this.__vV[be.toHashCode()].modelNode;
this.__wa(bd,bf);
this._updateSelection();
},__vX:function(e){var U=e.getData();
var Q=e.getOldData();
var R=this.__vV[Q.toHashCode()];
delete this.__vV[Q.toHashCode()];
R.modelNode.removeListenerById(R.changeChildernListenerId);
var V=R.modelNode;
var T=qx.Class.getPropertyDefinition(R.modelNode.constructor,this.getChildPath());
var O=T.event;
var S=V.addListener(O,this.__vX,this);
var P=R.treeNode;
this.__vV[U.toHashCode()]={modelNode:V,treeNode:P,changeListenerId:R.changeListenerId,changeChildernListenerId:S};
this.__wa(P,V);
this._updateSelection();
...
},__vY:function(){if(this.getTarget()==null||this.getChildPath()==null){return;
...
this.__wf(this.getModel(),r);
this.__wa(r,this.getModel());
}},__wa:function(s,t){if(t[bS+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;
}var C=t[bS+qx.lang.String.firstUp(this.getChildPath())]();
if(this.__vV[C.toHashCode()]==undefined){var x=C.addListener(bL,this.__vW,this);
var B=qx.Class.getPropertyDefinition(t.constructor,this.getChildPath());
var y=B.event;
var z=t.addListener(y,this.__vX,this);
this.__vV[C.toHashCode()]={modelNode:t,treeNode:s,changeListenerId:x,changeChildernListenerId:z};
}for(var i=0;i<C.length;i++){if(s.getChildren()[i]==null||C.getItem(i)!=s.getChildren()[i].getModel()){for(var j=i;j<s.getChildren().length;j++){if(s.getChildren()[j].getModel()===C.getItem(i)){var u=j;
break;
}}if(u!=undefined){var v=s.getChildren()[u];
if(this.getTarget().isSelected(v)){var D=true;
}s.removeAt(u);
s.addAt(v,i);
if(D){this.getTarget().addToSelection(v);
}}else{var A=this._createItem();
A.setModel(C.getItem(i));
s.addAt(A,i);
this.__wf(C.getItem(i),A);
this.__wa(A,C.getItem(i));
}}}for(var i=s.getChildren().length-1;i>=C.length;i--){var w=s.getChildren()[i];
this.__wd(w,s);
}},__wb:function(){if(this.getTarget()==null){return;
}var d=this.getTarget().getRoot();

if(d!=null){this.getTarget().setRoot(null);
this.__wc(d);
this.__wg(d.getModel());
d.destroy();
this.__vV={};
}},__wc:function(bV){var bW=bV.getChildren();
for(var i=bW.length-1;i>=0;i--){if(bW[i].getChildren().length>0){this.__wc(bW[i]);
}this.__wd(bW[i],bV);
}},__wd:function(bk,bl){var bm=bk.getModel();
if(bm[bS+qx.lang.String.firstUp(this.getChildPath())]!=undefined){delete this.__vV[bm[bS+qx.lang.String.firstUp(this.getChildPath())]().toHashCode()];
}this.__wg(bm);
bl.remove(bk);
bk.destroy();
},bindProperty:function(bn,bo,bp,bq,br){var bt=br.bind(bn,bq,bo,bp);
if(this.__vT[bo]==null){this.__vT[bo]={};
}var bs=this.__vT[bo];

if(bs[br.toHashCode()]){bs[br.toHashCode()].reverseId=bt;
}else{bs[br.toHashCode()]={id:bt,reverseId:null,treeNode:bq};
}if(!qx.lang.Array.contains(this.__vU,bo)){this.__vU.push(bo);
}},bindPropertyReverse:function(h,k,l,m,n){var p=m.bind(k,n,h,l);
if(this.__vT[k]==null){this.__vT[k]={};
}var o=this.__vT[k];

if(o[n.toHashCode()]){o[n.toHashCode()].reverseId=p;
}else{o[n.toHashCode()]={id:null,reverseId:p,treeNode:m};
}if(!qx.lang.Array.contains(this.__vU,k)){this.__vU.push(k);
}},bindDefaultProperties:function(ca,cb){this.bindProperty(this.getLabelPath(),bK,this.getLabelOptions(),ca,cb);
if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),bM,this.getIconOptions(),ca,cb);
}},__we:function(){var cd;

for(var cf in this.__vT){cd=cf;
break;
}for(var cc in this.__vT[cd]){var cg=this.__vT[cd][cc].treeNode;
var ce=qx.core.ObjectRegistry.fromHashCode(cc);
this.__wg(ce);
this.__wf(ce,cg);
}},__wf:function(E,F){var G=this.getDelegate();
if(G!=null&&G.bindItem!=null){G.bindItem(this,F,E);
}else{this.bindDefaultProperties(F,E);
}},__wg:function(W){for(var i=0;i<this.__vU.length;i++){var Y=this.__vU[i];
var X=this.__vT[Y][W.toHashCode()];

if(X!=null){if(X.id){W.removeBinding(X.id);
}
if(X.reverseId){X.treeNode.removeBinding(X.reverseId);
}delete this.__vT[Y][W.toHashCode()];
...
this._endSelectionModification();
this._updateSelection();
},_setBindItem:function(bu,bv){if(bu!=null&&bu.bindItem!=null){if(bv!=null&&bv.bindItem!=null&&bu.bindItem==bv.bindItem){return;
}this.__vY();
}}},destruct:function(){this.__vT=this.__vV=this.__vU=null;
}});
})();
