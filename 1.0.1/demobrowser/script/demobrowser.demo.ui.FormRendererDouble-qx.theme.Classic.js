...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.ui.FormRendererDouble-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"480ee4383514"},
  boot : "boot",
  bootIsInline : true,
...
this.__wi=[];
this.__wj=[];
this.__wk=new qx.ui.form.validation.Manager();
this.__wl=new qx.ui.form.Resetter();
},members:{__wi:null,__wk:null,__wm:0,__wj:null,__wl:null,add:function(d,e,f,name){if(this.__wn()){this.__wi.push({title:null,items:[],labels:[],names:[]});
}this.__wi[this.__wm].items.push(d);
this.__wi[this.__wm].labels.push(e);
if(name==null){name=e.replace(/\s+/g,a);
}this.__wi[this.__wm].names.push(name);
this.__wk.add(d,f);
this.__wl.add(d);
},addGroupHeader:function(g){if(!this.__wn()){this.__wm++;
}this.__wi.push({title:g,items:[],labels:[],names:[]});
},addButton:function(k){this.__wj.push(k);
},__wn:function(){return this.__wi.length===0;
},reset:function(){this.__wl.reset();
this.__wk.reset();
},redefineResetter:function(){this.__wl.redefine();
},validate:function(){return this.__wk.validate();
},getValidationManager:function(){return this.__wk;
},getGroups:function(){return this.__wi;
},getButtons:function(){return this.__wj;
},createView:function(h){qx.log.Logger.deprecatedMethodWarning(arguments.callee,b);

if(h==null){h=qx.ui.form.renderer.Single;
}return new h(this);
},getItems:function(){var l={};
for(var i=0;i<this.__wi.length;i++){var m=this.__wi[i];
...
this.__wF=[];
},members:{__wF:null,add:function(a){if(this.__wJ(a)){var b=a.getValue();
}else if(this.__wI(a)){var b=a.getSelection();
}else{throw new Error("Item "+a+" not supported for reseting.");
}this.__wF.push({item:a,init:b});
},reset:function(){for(var i=0;i<this.__wF.length;i++){var l=this.__wF[i];
this.__wG(l.item,l.init);
}},resetItem:function(c){var d;

for(var i=0;i<this.__wF.length;i++){var e=this.__wF[i];

if(e.item===c){d=e.init;
break;
}}if(d===undefined){throw new Error("The given item has not been added.");
}this.__wG(c,d);
},__wG:function(o,p){if(this.__wJ(o)){o.setValue(p);
}else if(this.__wI(o)){o.setSelection(p);
}},redefine:function(){for(var i=0;i<this.__wF.length;i++){var j=this.__wF[i].item;
this.__wF[i].init=this.__wH(j);
}},redefineItem:function(m){var n;

for(var i=0;i<this.__wF.length;i++){if(this.__wF[i].item===m){n=this.__wF[i];
break;
}}if(n===undefined){throw new Error("The given item has not been added.");
}n.init=this.__wH(n.item);
},__wH:function(k){if(this.__wJ(k)){return k.getValue();
}else if(this.__wI(k)){return k.getSelection();
}},__wI:function(q){var r=q.constructor;
return qx.Class.hasInterface(r,qx.ui.core.ISingleSelection);
},__wJ:function(f){var g=f.constructor;
...
qx.Class.define(a,{type:b,extend:qx.ui.core.Widget,implement:qx.ui.form.renderer.IFormRenderer,construct:function(g){arguments.callee.base.call(this);
var k=g.getGroups();

for(var i=0;i<k.length;i++){var j=k[i];
this.addItems(j.items,j.labels,j.title);
}var h=g.getButtons();

for(var i=0;i<h.length;i++){this.addButton(h[i]);
}},members:{addItems:function(c,d,e){throw new Error("Abstract method call");
},addButton:function(f){throw new Error("Abstract method call");
}}});
})();
(function(){var o="",n="right",m=" <span style='color:red'>*</span> ",l="bold",k="qx.ui.form.renderer.Single",j="top",h="left",g=" :";
qx.Class.define(k,{extend:qx.ui.form.renderer.AbstractRenderer,construct:function(c){var d=new qx.ui.layout.Grid();
d.setSpacing(6);
d.setColumnFlex(0,1);
d.setColumnAlign(0,n,j);
this._setLayout(d);
arguments.callee.base.call(this,c);
},members:{_row:0,_buttonRow:null,addItems:function(t,u,v){if(v!=null){this._add(this._createHeader(v),{row:this._row,column:0,colSpan:2});
this._row++;
}for(var i=0;i<t.length;i++){var x=this._createLabel(u[i],t[i]);
this._add(x,{row:this._row,column:0});
var w=t[i];
x.setBuddy(w);
this._add(w,{row:this._row,column:1});
this._row++;
}},addButton:function(a){if(this._buttonRow==null){this._buttonRow=new qx.ui.container.Composite();
this._buttonRow.setMarginTop(5);
var b=new qx.ui.layout.HBox();
b.setAlignX(n);
b.setSpacing(5);
this._buttonRow.setLayout(b);
this._add(this._buttonRow,{row:this._row,column:0,colSpan:2});
this._row++;
}this._buttonRow.add(a);
},getLayout:function(){return this._getLayout();
},_createLabel:function(name,p){var q=o;

if(p.getRequired()){q=m;
}var r=name.length>0||p.getRequired()?g:o;
var s=new qx.ui.basic.Label(name+q+r);
s.setRich(true);
return s;
},_createHeader:function(e){var f=new qx.ui.basic.Label(e);
f.setFont(l);

if(this._row!=0){f.setMarginTop(10);
}f.setAlignX(h);
return f;
}}});
})();
...
(function(){var o="Boolean",n="changeSelection",m="_applyValid",l="",k="changeRequired",j="changeValid",h="__Cy",g="changeInvalidMessage",f="_applyInvalidMessage",d="String",b="qx.ui.form.RadioButtonGroup",c="qx.event.type.Data";
qx.Class.define(b,{extend:qx.ui.core.Widget,include:[qx.ui.core.MLayoutHandling,qx.ui.form.MModelSelection],implement:[qx.ui.form.IForm,qx.ui.core.ISingleSelection,qx.ui.form.IModelSelection],construct:function(v){arguments.callee.base.call(this);
if(v==null){this.setLayout(new qx.ui.layout.VBox(4));
}else{this.setLayout(v);
}this.__Cy=new qx.ui.form.RadioGroup();
this.__Cy.addListener(n,function(e){this.fireDataEvent(n,e.getData(),e.getOldData());
},this);
},properties:{valid:{check:o,init:true,apply:m,event:j},required:{check:o,init:false,event:k},invalidMessage:{check:d,init:l,event:g,apply:f}},events:{"changeSelection":c},members:{__Cy:null,_applyInvalidMessage:function(p,q){var r=this._getChildren();

for(var i=0;i<r.length;i++){r[i].setInvalidMessage(p);
}},_applyValid:function(w,x){var y=this._getChildren();

for(var i=0;i<y.length;i++){y[i].setValid(w);
}},getRadioGroup:function(){return this.__Cy;
},getChildren:function(){return this._getChildren();
},add:function(z,A){this.__Cy.add(z);
this._add(z,A);
},remove:function(u){this.__Cy.remove(u);
this._remove(u);
},removeAll:function(){var s=this.__Cy.getItems();

for(var i=0;i<s.length;i++){this.__Cy.remove(s[i]);
}this._removeAll();
},getSelection:function(){return this.__Cy.getSelection();
},setSelection:function(t){return this.__Cy.setSelection(t);
},resetSelection:function(){return this.__Cy.resetSelection();
},isSelected:function(a){return this.__Cy.isSelected(a);
},isSelectionEmpty:function(){return this.__Cy.isSelectionEmpty();
},getSelectables:function(){return this.__Cy.getSelectables();
...
(function(){var m="top",l="left",k="",j=" <span style='color:red'>*</span> ",h="bold",g="qx.ui.form.renderer.Double",f="right",e=" :";
qx.Class.define(g,{extend:qx.ui.form.renderer.AbstractRenderer,construct:function(p){var q=new qx.ui.layout.Grid();
q.setSpacing(6);
q.setColumnAlign(0,l,m);
q.setColumnAlign(1,l,m);
q.setColumnAlign(2,l,m);
q.setColumnAlign(3,l,m);
this._setLayout(q);
...
},_createLabel:function(name,a){var b=k;

if(a.getRequired()){b=j;
}var c=name.length>0||a.getRequired()?e:k;
var d=new qx.ui.basic.Label(name+b+c);
d.setRich(true);
return d;
},_createHeader:function(r){var s=new qx.ui.basic.Label(r);
s.setFont(h);

if(this._row!=0){s.setMarginTop(10);
}return s;
}}});
})();
