...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.progressive.ProgressiveTable_Conditional-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"99253b074e94"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var v="==",u="",t=">",s="between",r="regex",q="<",p="!between",o=">=",n="!=",m="<=",j='g',l="qx.ui.progressive.renderer.table.cell.Conditional",k=":",h=";";
qx.Class.define(l,{extend:qx.ui.progressive.renderer.table.cell.Abstract,construct:function(E,F,G,H){arguments.callee.base.call(this);
this.__Bx=[v,n,t,q,o,m];
this.__By=[s,p];
this.__Bz=[];
this.__BA=E||"";
this.__BB=F||"";
this.__BC=G||"";
this.__BD=H||"";
},members:{__Bx:null,__By:null,__Bz:null,__BA:null,__BB:null,__BC:null,__BD:null,__BE:function(W,X){if(W.align){X["text-align"]=W.align;
}
...
}},addNumericCondition:function(a,b,c,d,e,f,g){if(!qx.lang.Array.contains(this.__Bx,a)||b==null){throw new Error("Condition not recognized or value is null!");
}this.__Bz.push({condition:a,align:c,color:d,style:e,weight:f,value1:b,target:g});
},addBetweenCondition:function(w,x,y,z,A,B,C,D){if(!qx.lang.Array.contains(this.__By,w)||x==null||y==null){throw new Error("Condition not recognized or value1/value2 is null!");
}this.__Bz.push({condition:w,align:z,color:A,style:B,weight:C,value1:x,value2:y,target:D});
},addRegex:function(Q,R,S,T,U,V){if(!Q){throw new Error("regex cannot be null!");
}this.__Bz.push({condition:r,align:R,color:S,style:T,weight:U,regex:Q,target:V});
},_getCellStyle:function(I){if(this.__Bz.length==0){return I.style||u;
}var i;
var M;
var J;
var L={"text-align":this.__BA,"color":this.__BB,"font-style":this.__BC,"font-weight":this.__BD};

for(i=0;i<this.__Bz.length;i++){var P=this.__Bz[i];
M=false;

if(qx.lang.Array.contains(this.__Bx,P.condition)){if(P.target==null){J=I.cellData;
}else{J=I.element.data[P.target];
}
switch(P.condition){case v:if(J==P.value1){M=true;
}break;
case n:if(J!=P.value1){M=true;
}break;
case t:if(J>P.value1){M=true;
}break;
case q:if(J<P.value1){M=true;
}break;
case o:if(J>=P.value1){M=true;
}break;
case m:if(J<=P.value1){M=true;
}break;
}}else if(qx.lang.Array.contains(this.__By,P.condition)){if(P.target==null){J=I.cellData;
}else{J=I.element.data[P.target];
}
switch(P.condition){case s:if(J>=P.value1&&J<=P.value2){M=true;
}break;
case p:if(J<P.value1&&J>P.value2){M=true;
}break;
}}else if(P.condition==r){if(P.target==null){J=I.cellData;
}else{J=I.element.data[P.target];
}var K=new RegExp(P.value1,j);
M=K.test(J);
}if(M){this.__BE(P,L);
}var N=[];

for(var O in L){if(L[O]){N.push(O,k,L[O],h);
}}}return N.join(u);
}},destruct:function(){this.__Bx=this.__By=this.__Bz=null;
}});
})();
