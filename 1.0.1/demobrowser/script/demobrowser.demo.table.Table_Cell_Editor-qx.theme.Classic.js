...
(function(){var z="checkbox",y='Property',x="options",w="password",v="type",u='email',t='password',s='options',r='type',q='0',bp='Use only numbers!',bo='regExp',bn='[0-9]+',bm='newsletter',bl='admin',bk='editable combobox',bj='secret',bi="demobrowser.demo.table.Table_Cell_Editor",bh='status',bg="validationFunc",G='1',H="regExp",E="required",F='Doubleclick on cell to edit value',C='Newsletter',D='Inactive',A='User Name',B='telephone',K="dataChanged",L='Status',T='Active',R='validationFunc',X='enter website',V="editable",bc='role',ba='username',N='jdoe',bf='Telephone',be='manager',bd='enter valid telephone number',M='Waiting',P='Email',Q='Role',S="Property editor table",U='2',W='user',Y='website',bb='Password',I="email",J='Website',O='enter valid email';
qx.Class.define(bi,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return S;
},main:function(){arguments.callee.base.call(this);
this._container.resetWidth();
this._container.setResizable(false);
},createTable:function(){var bD=[[ba,A,N,{'required':true}],[t,bb,bj,{'type':t,'required':true}],[bc,Q,bk,{'options':[bl,be,W],'editable':true}],[bh,L,q,{'options':[[D,null,q],[T,null,G],[M,null,U]]}],[u,P,O,{'type':u}],[B,bf,bd,{'regExp':bn,'failMsg':bp}],[Y,J,X,{'validationFunc':function(bF){if(bF.search(/^http:/)!=-1){window.open(bF);
}return bF;
}}],[bm,C,true,{'type':z}]];
var bC=function(a){var g=a.table;
var d=g.getTableModel();
var e=d.getRowData(a.row);
var h=e[3];
var f;

for(var c in h){switch(c){case v:switch(h[r]){case z:return new qx.ui.table.cellrenderer.Boolean;
case w:return new qx.ui.table.cellrenderer.Password;
}break;
case x:var f=new qx.ui.table.cellrenderer.Replace;
var b={};
h[s].forEach(function(l){if(l instanceof Array){b[l[0]]=l[2];
}});
f.setReplaceMap(b);
f.addReversedReplaceMap();
return f;
}}return new qx.ui.table.cellrenderer.Default();
};
var bB=new qx.ui.table.cellrenderer.Dynamic(bC);
var bz=function(bG){var bL=bG.table;
var bJ=bL.getTableModel();
var bK=bJ.getRowData(bG.row);
var bN=bK[3];
var bI=new qx.ui.table.celleditor.TextField;
var bM=null;

for(var bH in bN){switch(bH){case x:if(bN.editable){bI=new qx.ui.table.celleditor.ComboBox();
}else{bI=new qx.ui.table.celleditor.SelectBox();
}bI.setListData(bN[s]);
break;
case V:break;
case v:switch(bN[r]){case w:bI=new qx.ui.table.celleditor.PasswordField;
break;
case z:bI=new qx.ui.table.celleditor.CheckBox;
break;
case I:bI.setValidationFunction(function(i,j){var k=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{2}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/;

if(k.test(i)){return i;
}alert("You did not enter a valid email address");
return j;
});
break;
}break;
case H:bI.setValidationFunction(function(bq,br){var bs=new RegExp(bN[bo]);

if(bs.test(bq)){return bq;
}alert(bN['failMsg']);
return br;
});
break;
case bg:bI.setValidationFunction(bN[R]);
break;
case E:bM=function(bt,bu){if(!bt){alert("You need to supply a value here");
return bu;
}return bt;
};
break;
...
var by=new qx.ui.table.celleditor.Dynamic(bz);
var bw=new qx.ui.table.model.Simple();
bw.setColumns([y,y,F]);
var bv={tableColumnModel:function(bE){return new qx.ui.table.columnmodel.Resize(bE);
}};
var bx=new qx.ui.table.Table(bw,bv);
bx.setDecorator(null);
bx.setColumnVisibilityButtonVisible(false);
bx.setKeepFirstVisibleRowComplete(true);
bx.setStatusBarVisible(false);
bx.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var bA=bx.getTableColumnModel();
bA.setColumnVisible(0,false);
bA.getBehavior().setWidth(1,100);
bA.getBehavior().setWidth(2,300);
bw.setColumnEditable(2,true);
bA.setDataCellRenderer(2,bB);
bA.setCellEditorFactory(2,by);
bx.getTableModel().setData(bD);
bx.getTableModel().addListener(K,function(event){if(!(event instanceof qx.event.type.Data)){return;
}var p=event.getData();
var m=this.getTableModel();
var o=m.getValue(0,p.firstRow);
var n=m.getValue(p.firstColumn,p.firstRow);
this.info("User edited property '"+o+"' and entered value '"+n+"'.");
},bx);
return bx;
}}});
})();
