...
(function(){var q='" ',p='src="',o="px'",n="qx.ui.progressive.renderer.table.cell.Icon",m="qx.client",l="string",k="vertical-align:middle;",j="progid:DXImageTransform.Microsoft.AlphaImageLoader(src='",i='<img ',h=">",C=" style='padding-top:2px;'",B="text-align:center;",A="",z=" title='",y="'",x="mshtml",w=" width='",v="abstract",u='" style="filter:',t=" height='",r="qx/static/blank.gif",s="',sizingMethod='scale')";
qx.Class.define(n,{type:v,extend:qx.ui.progressive.renderer.table.cell.Abstract,construct:function(){arguments.callee.base.call(this);
var e=qx.util.AliasManager.getInstance();
var f=qx.util.ResourceManager.getInstance();
var g=e.resolve(r);
this.__Br=f.toUri(g);
},members:{__Br:null,getBlankImage:function(){return this.__Br;
},_identifyImage:function(a){throw new Error("_identifyImage() is abstract");
},_getCellStyle:function(F){var G=arguments.callee.base.call(this,F)+B+k;
return G;
},_getContentHtml:function(b){var c=[];
var d=this.__Bs(b);
c.push(i);
if(qx.core.Variant.isSet(m,x)&&/\.png$/i.test(d.url)){c.push(p,this.__Br,u,j,d.url,s,q);
}else{c.push(p,d.url,q);
}if(d.imageWidth){c.push(w,d.imageWidth,o);
}if(d.imageHeight){c.push(t,d.imageHeight,o);
}c.push(C);
if(d.tooltip){c.push(z,d.tooltip,y);
}if(d.extras){c.push(d.extras);
}c.push(h);
return c.join(A);
},__Bs:function(D){var E=this._identifyImage(D);
if(E==null||typeof E==l){E={url:E,tooltip:null};
}if(E.url==null){E.url=this.__Br;
}return E;
}}});
})();
...
this.__BF=L.toUri(J);
this.__BG=L.toUri(I);
},properties:{allowToggle:{check:E,init:false}},members:{__BF:null,__BG:null,__BH:null,__BI:null,__BJ:null,__BK:null,__BL:null,__BM:null,_identifyImage:function(M){var N={imageWidth:11,imageHeight:11};

switch(M.cellData){case true:N.url=this.__BF;
N.extras=w;
break;
case false:N.url=this.__BG;
N.extras=p;
break;
default:N.url=null;
break;
...
if(qx.core.Variant.isSet(g,c)&&/\.png$/i.test(this.__BF)){N.extras+=i+this.getBlankImage()+j+d+h+a+this.__BF+j+b;
}else{N.extras+=i+this.__BF+j;
}N.extras+=m+v+u+e;

if(qx.core.Variant.isSet(g,c)&&/\.png$/i.test(this.__BG)){N.extras+=i+this.getBlankImage()+j+d+h+a+this.__BG+j+b;
}else{N.extras+=i+this.__BG+j;
}N.extras+=q+f;
N.extras+=s+r+C+t+k+D+f+F;
}return N;
},_getCellStyle:function(G){var H=arguments.callee.base.call(this,G);
return H;
}},destruct:function(){this.__BF=this.__BG=null;
}});
})();
