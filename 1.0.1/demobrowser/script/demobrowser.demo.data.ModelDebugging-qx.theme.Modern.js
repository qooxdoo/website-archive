...
var x=new demobrowser.demo.data.store.Twitter(j);
var v=new qx.ui.embed.Html();
v.setBackgroundColor(g);
v.setDecorator(f);
v.setWidth(500);
v.setHeight(200);
v.setOverflow(p,p);
this.getRoot().add(v,{left:10,top:80});
x.addListener(q,function(){var z=x.getModel();
this.debug(qx.dev.Debug.debugProperties(z));
v.setHtml(qx.dev.Debug.debugProperties(z,10,true));
},this);
...
qx.Class.define(h,{extend:qx.data.store.Json,statics:{saveResult:function(y){this.__xp=y;
}},construct:function(t){var u=d+t+c;
arguments.callee.base.call(this,u);
},members:{_createRequest:function(r){var s=new qx.io.ScriptLoader();
r+=n;
s.load(r,function(b){this.__xq();
},this);
},__xq:function(){var a=demobrowser.demo.data.store.Twitter.__xp;

if(a==undefined){this.setState(k);
return;
...
(function(){var y=": ",x="",w="construct",v="object",u="-",t=":",s=") ***",r="\r\n",q="px;'>",p="): ",T="</span><br>",S="*** EXCEPTION (",R="============================================================",Q="Object",P="<br>",O="null",N="Array",M="members",L="statics",K="get",F="------------------------------------------------------------",G="qx.dev.Debug",D="\n",E="*** TOO MUCH RECURSION: not displaying ***",B="Object, count=",C="  ",z="<span style='padding-left:",A=" ",H="Array, length=",I="undefined",J="index(";
qx.Class.define(G,{statics:{debugObject:function(bk,bl,bm){qx.log.Logger.debug(this,qx.dev.Debug.debugObjectToString(bk,bl,bm,false));
},debugObjectToString:function(d,e,f,g){if(!f){f=10;
}var o=(g?T:D);
var l=function(ba){var bb;

if(!g){bb=x;

for(var i=0;i<ba;i++){bb+=C;
}}else{bb=z+(ba*8)+q;
}return bb;
};
var n=x;
var k=function(U,V,W){if(V>W){n+=(l(V)+E+o);
return;
}if(typeof (U)!=v){n+=l(V)+U+o;
return;
}for(var X in U){if(typeof (U[X])==v){if(U[X] instanceof Array){n+=l(V)+X+y+N+o;
}else if(U[X]===null){n+=l(V)+X+y+O+o;
continue;
}else if(U[X]===undefined){n+=l(V)+X+y+I+o;
continue;
}else{n+=l(V)+X+y+Q+o;
}k(U[X],V+1,W);
}else{n+=l(V)+X+y+U[X]+o;
}}};
if(e){n+=l(0)+e+o;
}
if(d instanceof Array){n+=l(0)+H+d.length+t+o;
}else if(typeof (d)==v){var h=0;
...
switch(b){case w:return a==c?w:null;
case M:return qx.lang.Object.getKeyFromValue(c,a);
case L:return qx.lang.Object.getKeyFromValue(c.prototype,a);
default:if(a==c){return w;
...
}else if(qx.Class.hasInterface(bc.constructor,qx.data.IListData)){for(var i=0;i<bc.length;i++){for(var j=0;j<bf;j++){bi+=u;
}bi+=J+i+p+this.debugProperties(bc.getItem(i),bd-1,be,bf+1)+bg;
}return bi+bg;
}else if(bc.constructor!=null){var bj=bc.constructor.$$properties;

for(var bh in bj){bi+=bg;
for(var j=0;j<bf;j++){bi+=u;
}bi+=A+bh+y+this.debugProperties(bc[K+qx.lang.String.firstUp(bh)](),bd-1,be,bf+1);
}return bi;
}return x;
}}});
})();
