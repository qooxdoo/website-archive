...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.bom.Location-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"35d27c77d421"},
  boot : "boot",
  bootIsInline : true,
...
var b=document.getElementById(k).getElementsByTagName(e);

for(var i=0,l=b.length;i<l;i++){b[i].onclick=demobrowser.demo.bom.Location.jump;
}}},statics:{num:function(v,w){return parseInt(qx.bom.element.Style.get(v,w,qx.bom.element.Style.COMPUTED_MODE,false),10);
},jump:function(event){if(!event){event=window.event;
}event.returnValue=false;

if(event.preventDefault){event.preventDefault();
}function q(x,y){return parseInt(qx.bom.element.Style.get(x,y,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var s=this.href;
...
t.style.left=location.left+p;
t.style.top=location.top+p;

if(qx.bom.client.Feature.BORDER_BOX){t.style.width=(location.right-location.left)+p;
t.style.height=(location.bottom-location.top)+p;
}else{t.style.width=(location.right-location.left-q(t,f)-q(t,d))+p;
t.style.height=(location.bottom-location.top-q(t,g)-q(t,m))+p;
}return false;
},randomize:function(){var a=document.body.getElementsByTagName(c);
