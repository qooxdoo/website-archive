...
var R=new qx.ui.progressive.renderer.table.Widths(1);
R.setWidth(0,r);
var L=new qx.ui.progressive.headfoot.Progress(R);
var P=new qx.ui.progressive.structure.Default(null,L);
var K=new qx.ui.progressive.Progressive(P);
K.setFlushWidgetQueueAfterBatch(true);
var O=function(F){var G={renderer:q,data:F};
return G;
};
var V=new qx.ui.progressive.model.Default();
var S=new qx.ui.progressive.renderer.FunctionCaller();
K.addRenderer(q,S);
var Q=l;
var T=new qx.ui.basic.Image(Q,r,r);
K.add(T);
K.set({height:100,width:272,zIndex:99999,backgroundColor:E,opacity:0.86,batchSize:10});
this.getRoot().add(K,{top:-1000,left:-1000});
var U=[{text:m,background:w,color:s},{text:C,background:k,color:s},{text:A,background:D,color:s},{text:u,background:x,color:s},{text:B,background:j,color:n}];
this.context={document:this.getRoot(),pages:U};
K.addListener(z,function(e){var bg=e.getData().state;
var bc=e.getData().initial;
var bf=this.getRoot().getBounds();
var bd=e.getData().state.getProgressive();
var be=bd.getBounds();
var bh=Math.floor((bf.width-be.width)/2);
var top=Math.floor((bf.height-be.height)/2);
bd.setLayoutProperties({left:bh<0?0:bh,top:top<0?0:top});
bg.getUserData().context=this.context;
bg.getUserData().initialNum=bc;
},this);
K.addListener(v,function(e){qx.event.Timer.once(function(){this.getLayoutParent().remove(this);
this.dispose();
},this,0);
});
V.addElement(O(function(H){var I=H.context;
I.tabview=new qx.ui.tabview.TabView();
I.tabview.set({});
I.document.add(I.tabview,{left:20,right:20,top:20,bottom:20});
I.page=0;
I.currentPage=null;
}));

for(var M=0;M<U.length;M++){V.addElement(O(function(a){var c=a.context;
var b=c.pages[c.page];
c.currentPage=new qx.ui.tabview.Page(b.text);
c.currentPage.setLayout(new qx.ui.layout.VBox(4));
c.tabview.add(c.currentPage);
c.row=0;
c.col=0;
if(c.page==0){c.firstPage=c.currentPage;
}}));
for(var W=0;W<20;W++){V.addElement(O(function(f){var g=f.context;
g.hBox=new qx.ui.container.Composite(new qx.ui.layout.HBox(4));
g.currentPage.add(g.hBox);
g.tabview.setSelection([g.currentPage]);
g.col=0;
}));

for(var N=0;N<10;N++){V.addElement(O(function(Y){var bb=Y.context;
var ba=bb.pages[bb.page];
var o=new qx.ui.basic.Atom(t+bb.row+y+i+bb.col);
o.set({width:90,backgroundColor:ba.background,textColor:ba.color});
bb.hBox.add(o);
}));
V.addElement(O(function(h){++h.context.col;
}));
}V.addElement(O(function(d){++d.context.row;
}));
}V.addElement(O(function(X){++X.context.page;
}));
}V.addElement(O(function(J){J.context.tabview.setSelection([J.context.firstPage]);
}));
K.setDataModel(V);
K.render();
}}});
...
for(var i=0;i<k;i++){this.__AE[i]=new qx.ui.core.ColumnData();
}},members:{__AE:null,getData:function(){return this.__AE;
},set:function(g,h){for(var j in h){switch(j){case c:this.setWidth(g,h[j]);
break;
case d:this.setMinWidth(g,h[j]);
break;
case f:this.setMaxWidth(g,h[j]);
break;
default:throw new Error("Unrecognized key: "+j);
}}},setWidth:function(n,o){if(n>this.__AE.length-1||n<0){throw new Error("Column number out of range");
}this.__AE[n].setColumnWidth(o);
},setMinWidth:function(a,b){if(a>this.__AE.length-1||a<0){throw new Error("Column number out of range");
}this.__AE[a].setMinWidth(b);
},setMaxWidth:function(l,m){if(l>this.__AE.length-1||l<0){throw new Error("Column number out of range");
}this.__AE[l].setMaxWidth(m);
}}});
})();
...
(function(){var c="qx.ui.progressive.headfoot.Abstract",b="abstract";
...
},members:{__AF:null,join:function(a){this.__AF=a;
}},destruct:function(){this.__AF=null;
}});
})();
(function(){var q="progressive-progressbar-indicator-undone",p="progress",o="%",n="progressive-progressbar-indicator-done",m="progressive-progressbar-percent-text",l="renderStart",k="progressive-progressbar-background",j="__AI",i="qx.ui.progressive.headfoot.Progress",h="__AH",f="progressive-progressbar-percent-background",g="renderEnd";
qx.Class.define(i,{extend:qx.ui.progressive.headfoot.Abstract,construct:function(r,s){arguments.callee.base.call(this);
this.setHeight(16);
this.setPadding(0);
this.__AG={};
var t=qx.theme.manager.Color.getInstance();
this.__AG.background=t.resolve(k);
this.__AG.indicatorDone=t.resolve(n);
this.__AG.indicatorUndone=t.resolve(q);
this.__AG.percentBackground=t.resolve(f);
this.__AG.percentText=t.resolve(m);
this.set({backgroundColor:this.__AG.background});
this.__AH=new qx.ui.core.Widget();
this.__AH.set({width:0,backgroundColor:this.__AG.indicatorDone});
this.add(this.__AH);
var u=new qx.ui.core.Widget();
u.set({backgroundColor:this.__AG.indicatorUndone});
this.add(u,{flex:1});
this.__AI=new qx.ui.basic.Atom("0%");
this.__AI.set({width:100,backgroundColor:this.__AG.percentBackground,textColor:this.__AG.percentText});
this.add(this.__AI);
this.exclude();
},members:{__AJ:null,__AG:null,__AH:null,__AI:null,join:function(v){arguments.callee.base.call(this,v);
v.addListener(l,function(e){this.__AJ=e.getData().initial;
this.show();
},this);
v.addListener(p,function(e){var c=1.0-(e.getData().remaining/this.__AJ);
var d=this.getBounds();

if(d){var a=Math.floor((d.width-this.__AI.getBounds().width)*c);
var b=Math.floor(c*100)+o;

if(!isNaN(a)){this.__AH.setMinWidth(a);
this.__AI.setLabel(b);
}}},this);
v.addListener(g,function(e){this.exclude();
},this);
...
(function(){var f="abstract",e="qx.ui.progressive.structure.Abstract",d="overflowY",c="auto";
qx.Class.define(e,{type:f,extend:qx.core.Object,construct:function(b){arguments.callee.base.call(this);
if(!b){this.__AK=new qx.ui.core.Widget();
this.__AL=this.__AK;
}else{this.__AK=null;
this.__AL=b;
}this.__AL.getContentElement().setStyle(d,c);
},members:{__AK:null,__AL:null,applyStructure:function(a){throw new Error("applyStructure() is abstract");
},getPane:function(){return this.__AL;
}},destruct:function(){if(this.__AK){this.__AK.dispose();
}this.__AK=this.__AL=null;
}});
})();
...
if(!c){this.__AM=new qx.ui.progressive.headfoot.Null();
this.__AN=this.__AM;
}else{this.__AM=null;
this.__AN=c;
}if(!d){this.__AO=new qx.ui.progressive.headfoot.Null();
this.__AP=this.__AO;
}else{this.__AO=null;
this.__AP=d;
}},members:{__AN:null,__AP:null,__AM:null,__AO:null,applyStructure:function(b){this.__AN.join(b);
this.__AP.join(b);
b.add(this.__AN);
b.add(this.getPane(),{flex:1});
b.add(this.__AP);
},getHeader:function(){return this.__AN;
},getFooter:function(){return this.__AP;
}},destruct:function(){if(this.__AM){this.__AM.dispose();
this.__AM=null;
}
if(this.__AO){this.__AO.dispose();
this.__AO=null;
}this.__AN=this.__AP=null;
}});
})();
...
(function(){var r="qx.event.type.Data",q="dataAvailable",p="renderStart",o="Integer",n="_applyDataModel",m="Boolean",l="qx.ui.progressive.model.Abstract",k="white",j="qx.ui.progressive.Progressive";
qx.Class.define(j,{extend:qx.ui.container.Composite,construct:function(h){arguments.callee.base.call(this,new qx.ui.layout.VBox());
this.__AQ={};
this.set({backgroundColor:k});
if(!h){h=new qx.ui.progressive.structure.Default();
}this.__AR=h;
h.applyStructure(this);
this.__AS=false;
this.__AT=false;
this.__AU=0;
},events:{"renderStart":r,"renderEnd":r,"progress":r,"progressDetail":r},properties:{dataModel:{check:l,apply:n},batchSize:{check:o,init:20},flushWidgetQueueAfterBatch:{check:m,init:false},interElementTimeout:{check:o,init:0}},members:{__AQ:null,__AT:null,__AV:null,__AU:null,__AS:null,__AR:null,getStructure:function(){return this.__AR;
},addRenderer:function(name,g){this.__AQ[name]=g;
g.join(this,name);
},removeRenderer:function(name){if(!this.__AQ[name]){throw new Error("No existing renderer named "+name);
}delete this.__AQ[name];
},render:function(){if(this.__AT){return;
}this.__AT=true;
var a=new qx.ui.progressive.State({progressive:this,model:this.getDataModel(),pane:this.__AR.getPane(),batchSize:this.getBatchSize(),rendererData:this.__AX(),userData:{}});
this.__AV=new Date();
if(this.__AS){this.__AU=a.getModel().getElementCount();
this.fireDataEvent(p,{state:a,initial:this.__AU});
this.__AW(a);
}else{qx.event.Timer.once(function(){this.__AU=a.getModel().getElementCount();
this.fireDataEvent(p,{state:a,initial:this.__AU});
this.__AW(a);
this.__AS=true;
},this,10);
}},_applyDataModel:function(s,t){if(t){t.removeListener(q,this.__AY,this);
t.dispose();
}s.addListener(q,this.__AY,this);
},__AW:function(b){var c;
var f;
var d;

for(var i=b.getBatchSize();i>0;i--){c=b.getModel().getNextElement();

if(!c){this.debug("Render time: "+(new Date()-this.__AV)+"ms");
this.__AT=false;
this.fireDataEvent("renderEnd",b);
b.dispose();
return ;
}f=c.element;
d=this.__AQ[f.renderer];
d.render(b,f);
this.fireDataEvent("progressDetail",{initial:this.__AU,remaining:c.remaining,element:f});
}this.fireDataEvent("progress",{initial:this.__AU,remaining:c.remaining});
if(this.getFlushWidgetQueueAfterBatch()){qx.ui.core.queue.Manager.flush();
}qx.event.Timer.once(function(){this.__AW(b);
},this,this.getInterElementTimeout());
},__AX:function(){var u={};

for(var name in this.__AQ){u[name]={};
}return u;
},__AY:function(e){this.__AU=e.getData();
this.render();
}},destruct:function(){for(var name in this.__AQ){this.__AQ[name].dispose();
}this.__AV=this.__AQ=this.__AR=null;
}});
})();
...
(function(){var b="qx.ui.progressive.State";
qx.Class.define(b,{extend:qx.core.Object,construct:function(a){arguments.callee.base.call(this);
...
this.__Ba=[];
},members:{__Ba:null,addElements:function(d){this.__Ba=this.__Ba.concat(d);
this.fireDataEvent(c,this.__Ba.length);
},addElement:function(a){this.__Ba.push(a);
this.fireDataEvent(c,this.__Ba.length);
},getElementCount:function(){return this.__Ba.length;
},getNextElement:function(){if(this.__Ba.length>0){return ({element:this.__Ba.shift(),remaining:this.__Ba.length});
}return null;
...
(function(){var c="qx.ui.progressive.renderer.FunctionCaller";
qx.Class.define(c,{extend:qx.ui.progressive.renderer.Abstract,members:{render:function(a,b){b.data(a.getUserData());
}}});
})();
