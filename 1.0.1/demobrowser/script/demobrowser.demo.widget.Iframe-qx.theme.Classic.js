...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.widget.Iframe-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"362c5ac8467c"},
  boot : "boot",
  bootIsInline : true,
...
qx.Class.define(c,{extend:qx.application.Standalone,events:{"surfTo":i},members:{main:function(){arguments.callee.base.call(this);
...
(function(){var A="auto",z="scrollbar-y",w="iframe",v="corner",u="on",t="scrollbar-x",s="scroll",r="hidden",q="_updateScrollbars",p="load",i="off",o="resize",l="interval",h="horizontal",g="mousewheel",k="qx.ui.embed.ThemedIframe",j="scrollbarX",m="scrollbarY",f="scrollarea",n="vertical";
qx.Class.define(k,{extend:qx.ui.embed.AbstractIframe,construct:function(W){arguments.callee.base.call(this,W);
var X=new qx.ui.layout.Grid();
X.setColumnFlex(0,1);
X.setRowFlex(0,1);
this._setLayout(X);
this._showChildControl(w);
},properties:{appearance:{refine:true,init:f},scrollbarX:{check:[A,u,i],init:A,themeable:true,apply:q},scrollbarY:{check:[A,u,i],init:A,themeable:true,apply:q},scrollbar:{group:[j,m]}},members:{__Cr:null,__Cs:null,_getIframeElement:function(){return this.getChildControl(w).getContentElement();
},_createChildControlImpl:function(N){var O;

switch(N){case w:O=new qx.ui.embed.Iframe(this.getSource());
O.addListener(p,this._onIframeLoad,this);
O.addListener(o,this._onIframeResize,this);
this._add(O,{row:0,column:0});
break;
case t:O=new qx.ui.core.scroll.ScrollBar(h);
O.setMinWidth(0);
O.exclude();
O.addListener(s,this._onScrollBarX,this);
this._add(O,{row:1,column:0});
break;
case z:O=new qx.ui.core.scroll.ScrollBar(n);
O.setMinHeight(0);
O.exclude();
O.addListener(s,this._onScrollBarY,this);
this._add(O,{row:0,column:1});
break;
case v:O=new qx.ui.core.Widget();
O.setWidth(0);
O.setHeight(0);
O.exclude();
...
var T=this._getIframeElement().getBody();

if(T){this._startIframeObserver();
...
}},_onMouseWheel:function(e){var b=this._isChildControlVisible(z)&&this.getChildControl(z).isEnabled();

if(!b){return;
}var a=this.getChildControl(z,true);
a.scrollBySteps(e.getWheelDelta());
e.stop();
},_startIframeObserver:function(){if(this.__Cs){this._stopIframeObserver();
}var S=qx.event.Idle.getInstance();
this.__Cs=S.addListener(l,this._onIframeObserverInterval,this);
},_stopIframeObserver:function(){this.__Cr=null;

if(!this.__Cs){return;
}var R=qx.event.Idle.getInstance();
R.removeListenerById(this.__Cs);
},_onIframeObserverInterval:function(){var P=this._getIframeSize();

if(!P){this._disableScollbars();
return;
}
if(this.__Cr&&P.width==this.__Cr.width&&P.height==this.__Cr.height){return;
}this.__Cr=P;
this._preventIframeScrolling();
this._updateScrollbars();
},_preventIframeScrolling:function(){try{var c=this._getIframeElement().getDocument();
c.documentElement.style.overflow=r;
c.body.style.overflow=r;
}catch(e){this._disableScollbars();
}},_updateScrollbars:function(){var G=this.__Cr;
var J=this.getChildControl(w).getBounds();
var H=this.getChildControl(w).getInnerSize();
...
if(K===A&&L===A){var I=G.width>H.width;
var M=G.height>H.height;
if((I||M)&&!(I&&M)){if(I){M=G.height>J.height;
}else if(M){I=G.width>J.width;
}}}else{var I=K===u;
var M=L===u;
if(G.width>(I?J.width:H.width)&&K===A){I=true;
}
if(G.height>(I?J.height:H.height)&&L===A){M=true;
}}this._configureScrollbar(t,I,H.width,G.width);
this._configureScrollbar(z,M,H.height,G.height);
this._updateCornerWidget();
},_getIframeSize:function(){try{var U=this._getIframeElement().getWindow();
var V={width:qx.bom.Document.getWidth(U),height:qx.bom.Document.getHeight(U)};
return V;
}catch(e){return null;
}},_updateCornerWidget:function(){if(this._isChildControlVisible(t)&&this._isChildControlVisible(z)){this._showChildControl(v);
}else{this._excludeChildControl(v);
}},_configureScrollbar:function(B,C,D,E){if(!C){this._excludeChildControl(B);
return;
}var F=this._showChildControl(B);

if(D>=E){F.set({position:0,maximum:E,knobFactor:1,enabled:false});
}else{F.setMaximum(1000000);
F.set({position:Math.min(F.getPosition(),E),maximum:E-D,knobFactor:D/E,enabled:true});
}},_onScrollBarX:function(e){this.scrollToX(e.getData());
},_onScrollBarY:function(e){this.scrollToY(e.getData());
},scrollToX:function(x){try{var Q=this._getIframeElement().getWindow();
Q.scroll(x,qx.bom.Viewport.getScrollTop(Q));
}catch(e){this._disableScollbars();
