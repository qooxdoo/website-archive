...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.test.ChildrenHandling","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.test.ChildrenHandling-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"1a9dd10f14c1"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var u="Remove ",t="_gridWidgets",s="yellow",r="demobrowser.demo.test.ChildrenHandling",q="Horizontal box layout:",p="blue",o="Add ",n="_widgets",m="_hbox",l="green",g="Grid layout:",k="black",j="orange",f="_grid",e="red",h="execute";
qx.Class.define(r,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var z=new qx.ui.layout.VBox();
z.setSpacing(10);
var w=new qx.ui.container.Composite(z).set({padding:10});
w.add(new qx.ui.basic.Label(q));
this._hbox=new qx.ui.container.Composite(new qx.ui.layout.HBox()).set({height:100});
this._widgets=[];
var A=[l,p,s,k,j,e];

for(var i=0;i<A.length;i++){var v=new qx.ui.core.Widget();
v.setBackgroundColor(A[i]);
this._hbox.add(v);
this._widgets.push(v);
}w.add(this._hbox);
w.add(new qx.ui.basic.Label(g));
var y=new qx.ui.layout.Grid();
y.setRowFlex(0,1);
this._grid=new qx.ui.container.Composite(y).set({height:100});
this._gridWidgets=[];

for(var i=0;i<A.length;i++){var v=new qx.ui.core.Widget();
v.setBackgroundColor(A[i]);
this._grid.add(v,{row:0,column:i});
this._gridWidgets.push(v);
}w.add(this._grid);
var x=new qx.ui.container.Composite(new qx.ui.layout.HBox());

for(var i=0;i<this._widgets.length;i++){x.add(this.createRemoveButton(this._widgets[i],i));
}w.add(x);
this.getRoot().add(w,{left:0,top:0});
},createRemoveButton:function(a,b){var d=true;
var c=new qx.ui.form.Button(u+a.getBackgroundColor()).set({width:100});
c.addListener(h,function(){if(d){this._hbox.remove(a);
this._grid.remove(this._gridWidgets[b]);
c.setLabel(o+a.getBackgroundColor());
}else{this._hbox.add(a);
this._grid.add(this._gridWidgets[b],{row:0,column:b});
c.setLabel(u+a.getBackgroundColor());
}d=!d;
},this);
return c;
}},destruct:function(){this._disposeObjects(m,f);
