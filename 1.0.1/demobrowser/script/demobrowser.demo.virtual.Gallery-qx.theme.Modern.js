...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.virtual.Gallery-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"d17b1e6af306"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var N="",M="selected",L="cell",K="layer",J="icon",I="#f3f3f3",H="label",G="Gallery (HTML - divs)",F="manager",E="default",bs="demobrowser.demo.virtual.Gallery",br="demobrowser.demo.virtual.WidgetGallery",bq="text-align: center;",bp="auto",bo="color: white; background-color: #00398D;",bn="user-desktop.png",bm="<img src='",bl="'></img>",bk="group",bj="resize",U="folder.png",V="vertical",S="multi",T="network-server.png",Q="user-trash.png",R="off",O="top",P="icon/32/places/",W="horizontal",X="demobrowser.demo.virtual.AbstractGallery",bb="abstract",ba="position: absolute;",bd="demobrowser.demo.virtual.HtmlGallery",bc="Gallery (widgets)",bf="<br>",be="Icon #",Y="network-workgroup.png",bi="__FD",bh="text-selected",bg="demobrowser.demo.virtual.GalleryCell";
qx.Class.define(bs,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
new demobrowser.demo.virtual.WidgetGallery(bc);
var o=new demobrowser.demo.virtual.HtmlGallery(G);
o.moveTo(400,50);
}}});
qx.Class.define(X,{extend:qx.ui.window.Window,type:bb,construct:function(b){arguments.callee.base.call(this,b);
...
var c=this._createScroller();
c.set({scrollbarX:R,scrollbarY:bp});
c.getPane().addListener(bj,this._onPaneResize,this);
this.add(c);
this.manager=new qx.ui.virtual.selection.CellRectangle(c.getPane(),this).set({mode:S,drag:true});
this.manager.attachMouseEvents();
this.manager.attachKeyEvents(c);
},members:{getItemData:function(j,k){return this.items[j*this.itemPerLine+k];
},_createScroller:function(){},isItemSelectable:function(bN){return !!this.getItemData(bN.row,bN.column);
},styleSelectable:function(bE,bF,bG){},_onPaneResize:function(e){var r=e.getTarget();
var p=e.getData().width;
var q=Math.floor(p/this.itemWidth);

if(q==this.itemsPerLine){return;
}this.itemPerLine=q;
var s=Math.ceil(this.itemCount/q);
r.getColumnConfig().setItemCount(q);
r.getRowConfig().setItemCount(s);
},_generateItems:function(t){var w=[];
var z=[U,Q,T,Y,bn];
var x=qx.util.AliasManager.getInstance();
var v=qx.util.ResourceManager.getInstance();

for(var i=0;i<t;i++){var A=P+z[Math.floor(Math.random()*z.length)];
var u=x.resolve(A);
var y=v.toUri(u);
w[i]={label:be+(i+1),icon:A,resolvedIcon:y};
}return w;
}},destruct:function(){this.items=null;
this._disposeObjects(F);
}});
qx.Class.define(bg,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var l=new qx.ui.basic.Atom().set({iconPosition:O});
l.getChildControl(H).set({padding:[0,4]});
l.getChildControl(J).set({padding:4});
return l;
},updateData:function(m,n){m.set({icon:n.icon,label:n.label});
},updateStates:function(bO,bP){var bR=bO.getChildControl(H);
var bQ=bO.getChildControl(J);

if(bP.selected){bR.setDecorator(M);
bR.setTextColor(bh);
bQ.setDecorator(bk);
}else{bR.resetDecorator();
bR.resetTextColor();
bQ.resetDecorator();
}}}});
qx.Class.define(br,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(a){arguments.callee.base.call(this,a);
this.__FD=new demobrowser.demo.virtual.GalleryCell();
},members:{_createScroller:function(){var d=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.WidgetCell(this);
d.getPane().addLayer(this.layer);
new qx.ui.virtual.behavior.Prefetch(d,0,0,0,0,200,300,600,800).set({interval:500});
return d;
},styleSelectable:function(bH,bI,bJ){if(bI!==M){return;
}var bK=this.layer.getChildren();

for(var i=0;i<bK.length;i++){var bL=bK[i];
var bM=bL.getUserData(L);

if(bH.row!==bM.row||bH.column!==bM.column){continue;
}
if(bJ){this.__FD.updateStates(bL,{selected:1});
}else{this.__FD.updateStates(bL,{});
}}},getCellWidget:function(by,bz){var bD=this.getItemData(by,bz);

if(!bD){return null;
}var bA={row:by,column:bz};
var bB={};

if(this.manager.isItemSelected(bA)){bB.selected=true;
}var bC=this.__FD.getCellWidget(bD,bB);
bC.setUserData(L,bA);
return bC;
},poolCellWidget:function(D){this.__FD.pool(D);
}},destruct:function(){this._disposeObjects(bi,K);
}});
qx.Class.define(bd,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bS){arguments.callee.base.call(this,bS);
var bT=qx.theme.manager.Font.getInstance().resolve(E).getStyles();
this._fontCss=qx.bom.element.Style.compile(bT);
},members:{_createScroller:function(){var C=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.HtmlCell(this);
C.getPane().addLayer(this.layer);
var B=new qx.ui.virtual.layer.GridLines(W,I);
C.getPane().addLayer(B);
var B=new qx.ui.virtual.layer.GridLines(V,I);
C.getPane().addLayer(B);
return C;
},_onPaneResize:function(e){arguments.callee.base.call(this,e);
this.manager.clearSelection();
},styleSelectable:function(f,g,h){this.layer.updateLayerData();
},getCellProperties:function(bt,bu){var bx=this.getItemData(bt,bu);

if(!bx){return N;
}var bw=this.manager.isItemSelected({row:bt,column:bu});
var bv=bw?bo:N;
return {style:[ba,bq,this._fontCss,bv].join(N),content:[bm,bx.resolvedIcon,bl,bf,bx.label].join(N)};
