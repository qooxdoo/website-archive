...
(function(){var q="$1",p="Shockwave Flash",o=".",n="undefined",m="object",l="boolean",k=",",j="$version",h="always",g="ShockwaveFlash.ShockwaveFlash.6",d="qx.bom.client.Flash",f="ShockwaveFlash.ShockwaveFlash.7",e="qx.client",c="ShockwaveFlash.ShockwaveFlash";
qx.Class.define(d,{statics:{AVAILABLE:false,FULLVERSION:"0.0.0",REVISION:"0",VERSION:0.0,EXPRESSINSTALL:false,STRICT_SECURITY_MODEL:false,_cachedSupportsVersion:{},supportsVersion:function(t){if(typeof this._cachedSupportsVersion[t]===l){return this._cachedSupportsVersion[t];
}else{var w=t.split(o);
var v=this.FULLVERSION.split(o);

for(var i=0;i<w.length;i++){var u=parseInt(v[i])-parseInt(w[i]);

if(u>0){return (this._cachedSupportsVersion[t]=true);
}else if(u<0){return (this._cachedSupportsVersion[t]=false);
}}return (this._cachedSupportsVersion[t]=true);
}},__rj:qx.core.Variant.select(e,{"mshtml":function(){if(!window.ActiveXObject){return;
}var C=[0,0,0];
var B=false;

try{var D=new ActiveXObject(f);
}catch(a){try{var D=new ActiveXObject(g);
C=[6,0,21];
D.AllowScriptAccess=h;
}catch(x){if(C[0]==6){B=true;
}}
if(!B){try{D=new ActiveXObject(c);
}catch(b){}}}
if(!B&&typeof D==m){var A=D.GetVariable(j);

if(typeof A!=n){A=A.split(" ")[1].split(k);
C[0]=parseInt(A[0],10);
C[1]=parseInt(A[1],10);
C[2]=parseInt(A[2],10);
}}this.__rk(C);
},"default":function(){if(!navigator.plugins||typeof navigator.plugins[p]!==m){return;
}var y=[0,0,0];
var z=navigator.plugins[p].description;

if(typeof z!=n){z=z.replace(/^.*\s+(\S+\s+\S+$)/,q);
y[0]=parseInt(z.replace(/^(.*)\..*$/,q),10);
y[1]=parseInt(z.replace(/^.*\.(.*)\s.*$/,q),10);
y[2]=/r/.test(z)?parseInt(z.replace(/^.*r(.*)$/,q),10):0;
}this.__rk(y);
}}),__rk:function(r){this.FULLVERSION=r.join(".");
...
qx.Class.define(l,{statics:{__rl:{quicktime:{plugin:g,control:k},wmv:{plugin:o,control:m},divx:{plugin:d,control:e},silverlight:{plugin:p,control:b}},has:function(q,r){if(this.__rl[q]){var s=this.__rl[q];
if(s.installed){if(r==null||(s.version>=parseFloat(r))){return true;
}}}return false;
},__rm:qx.core.Variant.select(j,{"mshtml":function(){var t=window.ActiveXObject;

if(!t){return;
}var v=this.__rl;
var u,w;

for(var x in v){u=v[x];

try{w=new ActiveXObject(u.control);
}catch(a){continue;
}switch(x){case h:u.version=0;
break;
case n:u.version=w.versionInfo;
break;
case c:u.version=0;
break;
case f:try{u.version=w.version===undefined?0:w.version;
}catch(H){try{u.version=w.settings.version===undefined?0:w.settings.version;
}catch(F){u.version=0;
}}break;
}u.installed=true;
}},"default":function(){var C=navigator.plugins;
...
for(var i=0,B=C.length;i<B;i++){y=C[i];
name=y.name;

for(var E in z){D=z[E];

if(!D.installed&&name.indexOf(D.plugin)!==-1){D.installed=true;

if(A.test(y.name)||A.test(y.description)){D.version=parseFloat(RegExp.$1,10);
}else{D.version=0;
}break;
}}}}})},defer:function(G){G.__rm();
}});
})();
