...
(function(){var y="password",x="Send",w="All fields are required. Some by a custom validator, some by a ",v="complete",u="Please enter a password at with least 3 characters.",t="username",s="Server said no!",r="Passwords must be equal.",q="<b>Client side form validation</b><br/>",p="Validating...",m="email address",o="demobrowser.demo.ui.FormValidator",n="Accept",l="execute",k="predefined validator and some by the required flag.";
...
a.setWidth(150);
a.setRequired(true);
this.getRoot().add(a,{left:20,top:70});
var j=new qx.ui.form.TextField();
j.setPlaceholder(m);
j.setWidth(150);
this.getRoot().add(j,{left:20,top:100});
var b=new qx.ui.form.TextField();
b.setPlaceholder(y);
b.setWidth(150);
this.getRoot().add(b,{left:20,top:130});
var c=new qx.ui.form.TextField();
c.setPlaceholder(y);
c.setWidth(150);
this.getRoot().add(c,{left:20,top:160});
var i=new qx.ui.form.CheckBox(n);
i.setRequired(true);
this.getRoot().add(i,{left:20,top:190});
var f=new qx.ui.form.validation.Manager();
var d=function(E,F){var G=E!=null&&E.length>2;

if(!G){F.setInvalidMessage(u);
}return G;
};
var g=new qx.ui.form.validation.AsyncValidator(function(z,A){window.setTimeout(function(){if(A==null||A.length==0){z.setValid(false,s);
}else{z.setValid(true);
}},1000);
});
f.add(a,g);
f.add(j,qx.util.Validate.email());
f.add(b,d);
f.add(c,d);
f.add(i);
f.setValidator(function(B){var D=b.getValue()==c.getValue();

if(!D){var C=r;
b.setInvalidMessage(C);
c.setInvalidMessage(C);
b.setValid(false);
c.setValid(false);
}return D;
});
var e=new qx.ui.form.Button(x);
this.getRoot().add(e,{left:20,top:215});
e.addListener(l,function(){e.setEnabled(false);
e.setLabel(p);
f.validate();
},this);
f.addListener(v,function(){e.setEnabled(true);
e.setLabel(x);
if(f.getValid()){alert("You can send...");
}else{alert(f.getInvalidMessages().join("\n"));
}},this);
