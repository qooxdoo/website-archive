...
(function(){var x="value",w="icon",v="model",u="text",t="qx.client",s="execute",r="source",q="wittemann",p="mshtml",o=".json",V="user.profile_image_url",U="http://twitter.com/statuses/user_timeline/",T="Enter",S="<span style='font-size: 20px'>Twitter</span>",R="<a target='_blank' href='http://twitter.com/home?status=Check%20out%20the%20%23qooxdoo%20twitter%20demo%20at%20http://bit.ly/Mc4Of'>Tweet This</a></span>",Q="demobrowser.demo.data.store.Twitter",P="keydown",O="Friends",N="loaded",M="demobrowser.demo.data.Twitter",E="selection[0]",F="<span style='font-size: 13px'>",C="Location: ",D="Details",A="Name: ",B="?callback=demobrowser.demo.data.store.Twitter.saveResult",y="User",z="http://twitter.com/statuses/friends_timeline.json",G="Postet with: ",H="failed",J="user.name",I="user.location",L="Avatar: ",K="Message: ";
...
i.setDelegate(this);
i.setLabelPath(u);
if(!qx.core.Variant.isSet(t,p)){i.setIconPath(V);
}var m=new demobrowser.demo.data.store.Twitter(q);
m.bind(v,i,v);
var e=new qx.ui.form.Button(O);
this.getRoot().add(e,{left:10,top:55});
e.addListener(s,function(){m.setUrl(z);
},this);
var j=new qx.ui.form.Button(y);
this.getRoot().add(j,{left:90,top:55});
var b=new qx.ui.form.TextField(q);
this.getRoot().add(b,{left:140,top:55});
j.addListener(s,function(){var W=U+b.getValue()+o;

if(m.getUrl()==W){m.reload();
}else{m.setUrl(W);
}},this);
b.addListener(P,function(a){if(a.getKeyIdentifier()==T){j.execute();
}},this);
var f=new qx.ui.groupbox.GroupBox(D);
this.getRoot().add(f,{left:320,top:60});
f.setWidth(300);
f.setHeight(220);
f.setLayout(new qx.ui.layout.Grid(0,5));
f.add(new qx.ui.basic.Label(A),{row:0,column:0});
f.add(new qx.ui.basic.Label(C),{row:1,column:0});
f.add(new qx.ui.basic.Label(K),{row:2,column:0});
f.add(new qx.ui.basic.Label(G),{row:3,column:0});
...
c.addTarget(name,x,J);
c.addTarget(location,x,I);
c.addTarget(k,x,u);
c.addTarget(n,x,r);

if(!qx.core.Variant.isSet(t,p)){f.add(new qx.ui.basic.Label(L),{row:4,column:0});
...
be.getChildControl(w).setWidth(50);
be.getChildControl(w).setHeight(50);
be.getChildControl(w).setScale(true);
}}});
qx.Class.define(Q,{extend:qx.data.store.Json,statics:{saveResult:function(X){this.__xB=X;
}},construct:function(ba){var bb=U+ba+o;
arguments.callee.base.call(this,bb);
},members:{_createRequest:function(bc){var bd=new qx.io.ScriptLoader();
bc+=B;
bd.load(bc,function(bf){this.__xC();
},this);
},__xC:function(){var Y=demobrowser.demo.data.store.Twitter.__xB;

if(Y==undefined){this.setState(H);
return;
