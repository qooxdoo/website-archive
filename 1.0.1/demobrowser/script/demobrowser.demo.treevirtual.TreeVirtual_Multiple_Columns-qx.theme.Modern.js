...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.treevirtual.TreeVirtual_Multiple_Columns-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"e3677ef9d781"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var w="changeValue",v="2007-01-30 22:54:03",u="Trash",t="-rwxr-xr-x",s="Permissions",r="Documents (D:)",q="Windows (C:)",p="Inbox",n="Desktop",m="execute",Y="color: white;",X="font-weight: bold;",W="Disable the tree?",V="1*",U="Network",T="Control",S="Last Accessed",R="changeSelection",Q="Sent",P="Files",D="demobrowser.demo.treevirtual.TreeVirtual_Multiple_Columns",E="background-color: red; ",B="Workspace",C="Open/close click selects row?",z="Remove",A="treeOpenWhileEmpty",x="Tree",y="Spam",F="background-color: cyan;",G="Data",J="Always show open/close symbol?",I="Current Selection: ",L='/',K="Remove open/close if found empty?",N="Exclude first-level tree lines?",M="Spam Message #",H="Use tree lines if theme supports them?",O="Edit";
...
this.getRoot().add(l,{edge:30});
var d=new qx.ui.treevirtual.TreeVirtual([x,s,S]);
d.set({width:400});
d.setAlwaysShowOpenCloseSymbol(true);
var c=d.getTableColumnModel().getBehavior();
c.set(0,{width:V,minWidth:180});
l.add(d);
var k=d.getDataModel();
var g=k.addBranch(null,n,true);
d.nodeSetLabelStyle(g,E+Y+X);
var b;
k.addBranch(g,P,true);
b=k.addBranch(g,B,true);
var j=k.addLeaf(b,q);
k.setColumnData(j,1,t);
k.setColumnData(j,2,v);
j=k.addLeaf(b,r);
k.setColumnData(j,1,t);
k.setColumnData(j,2,v);
k.addBranch(g,U,true);
j=k.addBranch(g,u,true);
d.nodeSetCellStyle(j,F);
var h=k.addBranch(null,p,true);
j=k.addBranch(h,y,false);

for(var i=1;i<3000;i++){k.addLeaf(j,M+i);
}k.addBranch(h,Q,false);
k.addBranch(h,u,false);
k.addBranch(h,G,false);
k.addBranch(h,O,false);
k.setData();
var f=new qx.ui.groupbox.GroupBox(T);
f.setLayout(new qx.ui.layout.Canvas());
l.add(f);
var o=new qx.ui.basic.Atom(I);
f.add(o,{left:0,top:6});
o=new qx.ui.form.TextField();
o.set({readOnly:true});
f.add(o,{left:4,right:0,top:20});
d.addListener(R,function(e){var bb=e.getData();
this.setValue(d.getHierarchy(bb[0].nodeId).join(L));
a.setEnabled(true);
},o);
var a=new qx.ui.form.Button(z);
a.set({enabled:false});
f.add(a,{top:50,left:0});
a.addListener(m,function(e){var ba=d.getSelectedNodes();

for(var i=0;i<ba.length;i++){k.prune(ba[i].nodeId,true);
k.setData();
}});
o=new qx.ui.form.CheckBox(H);
o.set({value:true});
f.add(o,{top:80,left:0});
o.addListener(w,function(e){d.setUseTreeLines(e.getData());
});
o=new qx.ui.form.CheckBox(N);
o.set({value:false});
f.add(o,{top:100,left:0});
o.addListener(w,function(e){d.setExcludeFirstLevelTreeLines(e.getData());
});
o=new qx.ui.form.CheckBox(J);
o.set({value:true});
f.add(o,{top:120,left:0});
o.addListener(w,function(e){d.setAlwaysShowOpenCloseSymbol(e.getData());
});
o=new qx.ui.form.CheckBox(K);
o.set({value:true});
f.add(o,{top:140,left:0});
d.addListener(A,function(e){if(this.getValue()){var bc=e.getData();
d.nodeSetHideOpenClose(bc,true);
}},o);
o=new qx.ui.form.CheckBox(C);
o.set({value:false});
f.add(o,{top:160,left:0});
o.addListener(w,function(e){d.setOpenCloseClickSelectsRow(e.getData());
});
o=new qx.ui.form.CheckBox(W);
o.set({value:false});
f.add(o,{top:180,left:0});
o.addListener(w,function(e){d.setEnabled(!e.getData());
});
}}});
})();
...
qx.Class.define(f,{extend:qx.ui.table.Table,construct:function(N,O){if(!O){O={};
}
if(!O.dataModel){O.dataModel=new qx.ui.treevirtual.SimpleTreeDataModel();
}
if(O.treeColumn===undefined){O.treeColumn=0;
O.dataModel.setTreeColumn(O.treeColumn);
}
if(!O.treeDataCellRenderer){O.treeDataCellRenderer=new qx.ui.treevirtual.SimpleTreeDataCellRenderer();
}
if(!O.defaultDataCellRenderer){O.defaultDataCellRenderer=new qx.ui.treevirtual.DefaultDataCellRenderer();
}
if(!O.dataRowRenderer){O.dataRowRenderer=new qx.ui.treevirtual.SimpleTreeDataRowRenderer();
}
if(!O.selectionManager){O.selectionManager=function(bt){return new qx.ui.treevirtual.SelectionManager(bt);
};
}
if(!O.tableColumnModel){O.tableColumnModel=function(H){return new qx.ui.table.columnmodel.Resize(H);
};
}if(typeof (N)==d){N=[N];
}O.dataModel.setColumns(N);
O.dataModel.setTreeColumn(O.treeColumn);
O.dataModel.setTree(this);
arguments.callee.base.call(this,O.dataModel,O);
this.addListener(g,function(e){this.getDataModel().setData();
},this);
this.setColumnVisibilityButtonVisible(N.length>1);
this.setRowHeight(16);
this.setMetaColumnCounts(N.length>1?[1,-1]:[1]);
this.setOverflow(n);
var Q=O.treeDataCellRenderer;
var P=O.defaultDataCellRenderer;
var T=this.getTableColumnModel();
var S=this.getDataModel().getTreeColumn();

for(var i=0;i<N.length;i++){T.setDataCellRenderer(i,i==S?Q:P);
}this.setDataRowRenderer(O.dataRowRenderer);
this.setFocusCellOnMouseMove(true);
this.setShowCellFocusIndicator(false);
var R=this._getPaneScrollerArr();
for(var i=0;i<R.length;i++){R[i].setSelectBeforeFocus(true);
...
C.setUseTreeLines(b);
if(D.hasListener(r)){var E={firstRow:0,lastRow:D.getRowCount()-1,firstColumn:0,lastColumn:D.getColumnCount()-1};
D.fireDataEvent(r,E);
}},getUseTreeLines:function(){var br=this.getDataModel().getTreeColumn();
var bs=this.getTableColumnModel().getDataCellRenderer(br);
return bs.getUseTreeLines();
...
if(L.hasListener(r)){var M={firstRow:0,lastRow:L.getRowCount()-1,firstColumn:0,lastColumn:L.getColumnCount()-1};
L.fireDataEvent(r,M);
...
},getAlwaysShowOpenCloseSymbol:function(){var z=this.getDataModel().getTreeColumn();
var A=this.getTableColumnModel().getDataCellRenderer(z);
return A.getAlwaysShowOpenCloseSymbol();
},setSelectionMode:function(I){this.getSelectionModel().setSelectionMode(I);
},getSelectionMode:function(){return this.getSelectionModel().getSelectionMode();
},getHierarchy:function(bh){var bl=this;
var bk=[];
var bi;
var bm;

if(typeof (bh)==c){bi=bh;
bm=bi.nodeId;
}else if(typeof (bh)==k){bm=bh;
}else{throw new Error("Expected node object or node id");
}function bj(F){if(!F){return ;
}var G=bl.getDataModel().getData()[F];
bk.unshift(G.label);
bj(G.parentNodeId);
}bj(bm);
return bk;
},getSelectedNodes:function(){return this.getDataModel().getSelectedNodes();
},_onKeyPress:function(U){if(!this.getEnabled()){return;
}var be=U.getKeyIdentifier();
var ba=false;
var bc=U.getModifiers();

if(bc==0){switch(be){case l:var W=this.getDataModel();
var X=this.getFocusedColumn();
var bb=W.getTreeColumn();

if(X==bb){var bd=this.getFocusedRow();
var V=W.getValue(bb,bd);

if(!V.bHideOpenClose){W.setState(V,{bOpened:!V.bOpened});
}ba=true;
}break;
...
break;
}}else if(bc==qx.event.type.Dom.CTRL_MASK){switch(be){case p:var W=this.getDataModel();
var bd=this.getFocusedRow();
var bb=W.getTreeColumn();
var V=W.getValue(bb,bd);
if((V.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH)&&!V.bHideOpenClose&&V.bOpened){W.setState(V,{bOpened:!V.bOpened});
}this.setFocusedCell(bb,bd,true);
ba=true;
break;
case o:var W=this.getDataModel();
var bd=this.getFocusedRow();
var bb=W.getTreeColumn();
var V=W.getValue(bb,bd);
if((V.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH)&&!V.bHideOpenClose&&!V.bOpened){W.setState(V,{bOpened:!V.bOpened});
...
ba=true;
break;
}}else if(bc==qx.event.type.Dom.SHIFT_MASK){switch(be){case p:var W=this.getDataModel();
var bd=this.getFocusedRow();
var bb=W.getTreeColumn();
var V=W.getValue(bb,bd);
if(V.parentNodeId){var Y=W.getRowFromNodeId(V.parentNodeId);
this.setFocusedCell(this._focusedCol,Y,true);
}ba=true;
break;
case o:var W=this.getDataModel();
var bd=this.getFocusedRow();
var bb=W.getTreeColumn();
var V=W.getValue(bb,bd);
if((V.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH)&&!V.bHideOpenClose){if(!V.bOpened){W.setState(V,{bOpened:!V.bOpened});
}if(V.children.length>0){this.moveFocusedCell(0,1);
}}ba=true;
break;
}}if(ba){U.preventDefault();
U.stopPropagation();
}else{arguments.callee.base.call(this,U);
}},_onSelectionChanged:function(t){this.getDataModel()._clearSelections();
if(this.getSelectionMode()!=qx.ui.treevirtual.TreeVirtual.SelectionMode.NONE){var u=this._calculateSelectedNodes();
this.fireDataEvent(h,u);
}arguments.callee.base.call(this,t);
},_calculateSelectedNodes:function(){var bq=this.getDataModel();
var bo=this.getSelectionModel().getSelectedRanges();
var bp=[];
var bn;

for(var i=0;i<bo.length;i++){for(var j=bo[i].minIndex;j<=bo[i].maxIndex;j++){bn=bq.getValue(bq.getTreeColumn(),j);
bq.setState(bn,{bSelected:true});
bp.push(bn);
}}return bp;
...
(function(){var bC="number",bB="dataChanged",bA="object",bz="treeOpenWhileEmpty",by="treeOpenWithContent",bx="bSelected",bw="qx.ui.treevirtual.SimpleTreeDataModel",bv="bOpened",bu="treeClose";
qx.Class.define(bw,{extend:qx.ui.table.model.Abstract,construct:function(){arguments.callee.base.call(this);
this._rowArr=[];
this._nodeArr=[];
this._nodeRowMap=[];
this._treeColumn=0;
this._selections={};
this._nodeArr.push(arguments.callee.self.__FL());
this.__FK=null;
},statics:{__FL:function(){return {label:"<virtual root>",nodeId:0,bOpened:true,children:[]};
},Type:{LEAF:1,BRANCH:2}},members:{__FM:null,__FK:null,_rowArr:null,_nodeArr:null,_nodeRowMap:null,_treeColumn:null,_selections:null,setTree:function(T){this.__FM=T;
},getTree:function(){return this.__FM;
},setEditable:function(bD){this.__FK=[];

for(var bE=0;bE<this.getColumnCount();bE++){this.__FK[bE]=bD;
}this.fireEvent(qx.ui.table.ITableModel.EVENT_TYPE_META_DATA_CHANGED);
},setColumnEditable:function(ba,bb){if(bb!=this.isColumnEditable(ba)){if(this.__FK==null){this.__FK=[];
}this.__FK[ba]=bb;
this.fireEvent(qx.ui.table.ITableModel.EVENT_TYPE_META_DATA_CHANGED);
}},isColumnEditable:function(O){return (this.__FK?this.__FK[O]==true:false);
},isColumnSortable:function(k){return false;
},sortByColumn:function(U,V){throw new Error("Trees can not be sorted by column");
},getSortColumnIndex:function(){return -1;
},setTreeColumn:function(P){this._treeColumn=P;
},getTreeColumn:function(){return this._treeColumn;
},getRowCount:function(){return this._rowArr.length;
},getRowData:function(v){return this._rowArr[v];
},getValue:function(M,N){if(N<0||N>=this._rowArr.length){throw new Error("this._rowArr row "+"("+N+") out of bounds: "+this._rowArr+" (0.."+(this._rowArr.length-1)+")");
}
if(M<0||M>=this._rowArr[N].length){throw new Error("this._rowArr column "+"("+M+") out of bounds: "+this._rowArr[N]+" (0.."+(this._rowArr[N].length-1)+")");
}return this._rowArr[N][M];
},setValue:function(bn,bo,bp){if(bn==this._treeColumn){return ;
}var bq=this.getNodeFromRow(bo);

if(bq.columnData[bn]!=bp){bq.columnData[bn]=bp;
this.setData();
if(this.hasListener(bB)){var br={firstRow:bq.nodeId,lastRow:bq.nodeId,firstColumn:bn,lastColumn:bn};
this.fireDataEvent(bB,br);
}}},_addNode:function(z,A,B,C,D,E,F){var I;
if(z){I=this._nodeArr[z];

if(!I){throw new Error("Request to add a child to a non-existent parent");
}if(I.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.LEAF){throw new Error("Sorry, a LEAF may not have children.");
}}else{I=this._nodeArr[0];
z=0;
}if(D==qx.ui.treevirtual.SimpleTreeDataModel.Type.LEAF){B=false;
C=false;
}var H=this._nodeArr.length;
var G={type:D,nodeId:H,parentNodeId:z,label:A,bSelected:false,bOpened:B,bHideOpenClose:C,icon:E,iconSelected:F,children:[],columnData:[]};
this._nodeArr.push(G);
I.children.push(H);
return H;
},addBranch:function(p,q,r,s,t,u){return this._addNode(p,q,r,s,qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH,t,u);
},addLeaf:function(bj,bk,bl,bm){return this._addNode(bj,bk,false,false,qx.ui.treevirtual.SimpleTreeDataModel.Type.LEAF,bl,bm);
},prune:function(l,m){var n;
var o;

if(typeof (l)==bA){n=l;
o=n.nodeId;
}else if(typeof (l)==bC){o=l;
}else{throw new Error("Expected node object or node id");
}for(var i=this._nodeArr[o].children.length-1;i>=0;i--){this.prune(this._nodeArr[o].children[i],true);
}if(m&&o!=0){n=this._nodeArr[o];
qx.lang.Array.remove(this._nodeArr[n.parentNodeId].children,o);
if(this._selections[o]){delete this._selections[o];
}this._nodeArr[o]=null;
}},move:function(bc,bd){var bi;
var be;
var bh;
var bf;
bd=bd||0;

if(typeof (bc)==bA){bi=bc;
be=bi.nodeId;
}else if(typeof (bc)==bC){be=bc;
bi=this._nodeArr[be];
}else{throw new Error("Expected move node object or node id");
}
if(typeof (bd)==bA){bh=bd;
bf=bh.nodeId;
}else if(typeof (bd)==bC){bf=bd;
bh=this._nodeArr[bf];
}else{throw new Error("Expected parent node object or node id");
}if(bh.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.LEAF){throw new Error("Sorry, a LEAF may not have children.");
}var bg=this._nodeArr[bi.parentNodeId];
qx.lang.Array.remove(bg.children,be);
bh.children.push(be);
this._nodeArr[be].parentNodeId=bf;
},setData:function(J){var L=this;
function K(){var bt=function(b,c){var g=null;
var f;
var d=L._nodeArr[b].children.length;

for(var i=0;i<d;i++){f=L._nodeArr[b].children[i];
g=L._nodeArr[f];
if(g==null){continue;
}g.level=c;
g.bFirstChild=(i==0);
g.lastChild=[i==d-1];
var parent=L._nodeArr[g.parentNodeId];
while(parent.nodeId){var h=parent.lastChild[parent.lastChild.length-1];
g.lastChild.unshift(h);
parent=L._nodeArr[parent.parentNodeId];
}if(!g.columnData){g.columnData=[];
}
if(g.columnData.length<L.getColumnCount()){g.columnData[L.getColumnCount()-1]=null;
}var e=[];
if(g.columnData){for(var j=0;j<g.columnData.length;j++){if(j==L._treeColumn){e.push(g);
}else{e.push(g.columnData[j]);
}}}else{e.push(g);
}if(g.bSelected){e.selected=true;
}L._nodeRowMap[g.nodeId]=L._rowArr.length;
L._rowArr.push(e);
if(g.bOpened){bt(f,c+1);
}}};
L._rowArr=[];
L._nodeRowMap=[];
bt(0,1);
if(L.hasListener(bB)){var bs={firstRow:0,lastRow:L._rowArr.length-1,firstColumn:0,lastColumn:L.getColumnCount()-1};
L.fireDataEvent(bB,bs);
}}
if(J instanceof Array){for(var i=0;i<J.length;i++){if(J[i].selected){this._selections[i]=true;
}}this._nodeArr=J;
}else if(J!==null&&J!==undefined){throw new Error("Expected array of node objects or null/undefined; "+"got "+typeof (J));
}K();
},getData:function(){return this._nodeArr;
},clearData:function(){this._clearSelections();
this.setData([arguments.callee.self.__FL()]);
},setColumnData:function(Q,R,S){this._nodeArr[Q].columnData[R]=S;
},getColumnData:function(W,X){return this._nodeArr[W].columnData[X];
},setState:function(bF,bG){var bH;
var bM;

if(typeof (bF)==bA){bH=bF;
bM=bH.nodeId;
}else if(typeof (bF)==bC){bM=bF;
bH=this._nodeArr[bM];
}else{throw new Error("Expected node object or node id");
}
for(var bK in bG){switch(bK){case bx:var bN=this.getRowFromNodeId(bM);
var bI=this.getTree().getSelectionModel();
var bJ=qx.ui.treevirtual.TreeVirtual;
var bO=(typeof (bN)===bC&&this.getTree().getSelectionMode()!=bJ.SelectionMode.NONE);
if(bG[bK]){this._selections[bM]=true;
if(bO&&!bI.isSelectedIndex(bN)){bI.setSelectionInterval(bN,bN);
}}else{delete this._selections[bM];
if(bO&&bI.isSelectedIndex(bN)){bI.removeSelectionInterval(bN,bN);
}}break;
case bv:if(bG[bK]==bH.bOpened){break;
}var bL=this.__FM;
if(bH.bOpened){bL.fireDataEvent(bu,bH);
}else{if(bH.children.length>0){bL.fireDataEvent(by,bH);
}else{bL.fireDataEvent(bz,bH);
}}if(!bH.bHideOpenClose){bH.bOpened=!bH.bOpened;
bL.getSelectionModel()._clearSelection();
}this.setData();
break;
default:break;
}bH[bK]=bG[bK];
}},getNodeRowMap:function(){return this._nodeRowMap;
},getRowFromNodeId:function(Y){return this._nodeRowMap[Y];
},getNodeFromRow:function(y){return this._nodeArr[this._rowArr[y][this._treeColumn].nodeId];
},_clearSelections:function(){for(var a in this._selections){this._nodeArr[a].bSelected=false;
}this._selections={};
},getSelectedNodes:function(){var w=[];

for(var x in this._selections){w.push(this._nodeArr[x]);
}return w;
}},destruct:function(){this._rowArr=this._nodeArr=this._nodeRowMap=this._selections=this.__FM=null;
}});
})();
...
(function(){var H='px;',G="",F="Boolean",E='',D='px',C='</div>',B='<div style="position:absolute;',A='">',z='left:',y=";",bh='</span>',bg='background-image:url(',bf='">&nbsp;</div>',be="treevirtual-file",bd="qx.ui.treevirtual.SimpleTreeDataCellRenderer",bc="qx.client",bb='height:',ba='style="',Y='top:0;',X='background-repeat:no-repeat;',O='right:',P='<div style="',M='" title="',N=');',K='<span',L=';width:',I=';"',J='>',Q='bottom:',R='top:',T="content-box",S="mshtml",V="treevirtual-folder",U='width:',W=';height:';
qx.Class.define(bd,{extend:qx.ui.table.cellrenderer.Abstract,construct:function(){var bq=qx.ui.treevirtual.SimpleTreeDataCellRenderer;
if(bq.__FN){bq.__FS();
bq.__FN=false;
}arguments.callee.base.call(this);
this.__FO=qx.util.AliasManager.getInstance();
this.__FP=qx.util.ResourceManager.getInstance();
this.__FQ=qx.theme.manager.Appearance.getInstance();
this.BLANK=this.__FP.toUri(this.__FO.resolve("static/blank.gif"));
},statics:{__FR:{},__FN:true,__FS:function(){qx.theme.manager.Meta.getInstance().initialize();
var a=qx.ui.treevirtual.SimpleTreeDataCellRenderer;
...
var g=function(f){d.load(e.toUri(b.resolve(f)));
};
a.__FR.line=c.styleFrom("treevirtual-line");
g(a.__FR.line.icon);
a.__FR.contract=c.styleFrom("treevirtual-contract");
g(a.__FR.contract.icon);
a.__FR.expand=c.styleFrom("treevirtual-expand");
g(a.__FR.expand.icon);
a.__FR.onlyContract=c.styleFrom("treevirtual-only-contract");
g(a.__FR.onlyContract.icon);
a.__FR.onlyExpand=c.styleFrom("treevirtual-only-expand");
g(a.__FR.onlyExpand.icon);
a.__FR.startContract=c.styleFrom("treevirtual-start-contract");
g(a.__FR.startContract.icon);
a.__FR.startExpand=c.styleFrom("treevirtual-start-expand");
g(a.__FR.startExpand.icon);
a.__FR.endContract=c.styleFrom("treevirtual-end-contract");
g(a.__FR.endContract.icon);
a.__FR.endExpand=c.styleFrom("treevirtual-end-expand");
g(a.__FR.endExpand.icon);
a.__FR.crossContract=c.styleFrom("treevirtual-cross-contract");
g(a.__FR.crossContract.icon);
a.__FR.crossExpand=c.styleFrom("treevirtual-cross-expand");
g(a.__FR.crossExpand.icon);
a.__FR.end=c.styleFrom("treevirtual-end");
g(a.__FR.end.icon);
a.__FR.cross=c.styleFrom("treevirtual-cross");
g(a.__FR.cross.icon);
}},properties:{useTreeLines:{check:F,init:true},excludeFirstLevelTreeLines:{check:F,init:false},alwaysShowOpenCloseSymbol:{check:F,init:false}},members:{__FO:null,__FQ:null,__FP:null,_getCellStyle:function(br){var bs=br.value;
var bt=arguments.callee.base.call(this,br)+(bs.cellStyle?bs.cellStyle+y:G);
return bt;
},_getContentHtml:function(bk){var bm=G;
var bn=0;
...
return bm;
},_addImage:function(bG){var bI=[];
var bH=this.__FP.toUri(this.__FO.resolve(bG.url));
if(bG.position){var bJ=bG.position;
bI.push(B);

if(!qx.core.Variant.isSet(bc,S)){bI.push(qx.bom.element.BoxSizing.compile(T));
}
if(bJ.top!==undefined){bI.push(R+bJ.top+H);
}
if(bJ.right!==undefined){bI.push(O+bJ.right+H);
}
if(bJ.bottom!==undefined){bI.push(Q+bJ.bottom+H);
}
if(bJ.left!==undefined){bI.push(z+bJ.left+H);
}
if(bJ.width!==undefined){bI.push(U+bJ.width+H);
}
if(bJ.height!==undefined){bI.push(bb+bJ.height+H);
}bI.push(A);
}bI.push(P);
bI.push(bg+bH+N);
bI.push(X);

if(bG.imageWidth&&bG.imageHeight){bI.push(L+bG.imageWidth+D+W+bG.imageHeight+D);
}var bK=bG.tooltip;

if(bK!=null){bI.push(M+bK);
}bI.push(bf);

if(bG.position){bI.push(C);
}return bI.join(G);
},_addIndentation:function(h,j){var k=h.value;
var l;
var m=G;
var q=this.getUseTreeLines();
var n=this.getExcludeFirstLevelTreeLines();
var p=this.getAlwaysShowOpenCloseSymbol();

for(var i=0;i<k.level;i++){l=this._getIndentSymbol(i,k,q,p,n);
m+=this._addImage({url:l.icon,position:{top:0+(l.paddingTop||0),left:j+(l.paddingLeft||0),width:19,height:16}});
j+=19;
}return ({html:m,pos:j});
},_addIcon:function(bu,bv){var bw=bu.value;
var by=(bw.bSelected?bw.iconSelected:bw.icon);

if(!by){if(bw.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.LEAF){var o=this.__FQ.styleFrom(be);
}else{var bx={opened:bw.bOpened};
var o=this.__FQ.styleFrom(V,bx);
}by=o.icon;
}var bz=this._addImage({url:by,position:{top:0,left:bv,width:19,height:16}});
return ({html:bz,pos:bv+19});
},_addLabel:function(bC,bD){var bE=bC.value;
var bF=B+z+bD+H+Y+(bE.labelStyle?bE.labelStyle+y:G)+A+K+(bC.labelSpanStyle?ba+bC.labelSpanStyle+I:G)+J+bE.label+bh+C;
return bF;
},_addExtraContentBeforeIndentation:function(bL,bM){return {html:E,pos:bM};
},_addExtraContentBeforeIcon:function(bi,bj){return {html:E,pos:bj};
},_addExtraContentBeforeLabel:function(bA,bB){return {html:E,pos:bB};
},_getIndentSymbol:function(r,s,t,u,v){var w=qx.ui.treevirtual.SimpleTreeDataCellRenderer;
if(r==0&&v){t=false;
}if(r<s.level-1){return (t&&!s.lastChild[r]?w.__FR.line:{icon:this.BLANK});
}var x=s.lastChild[s.lastChild.length-1];
if(s.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH&&!s.bHideOpenClose){if(s.children.length>0||u){if(!t){return (s.bOpened?w.__FR.contract:w.__FR.expand);
}if(r==0&&s.bFirstChild){if(x){return (s.bOpened?w.__FR.onlyContract:w.__FR.onlyExpand);
}else{return (s.bOpened?w.__FR.startContract:w.__FR.startExpand);
}}if(x){return (s.bOpened?w.__FR.endContract:w.__FR.endExpand);
}return (s.bOpened?w.__FR.crossContract:w.__FR.crossExpand);
}}if(t){if(s.parentNodeId==0){if(x&&s.bFirstChild){return {icon:this.BLANK};
}if(x){return w.__FR.end;
}if(s.bFirstChild){return (s.bOpened?w.__FR.startContract:w.__FR.startExpand);
}}return (x?w.__FR.end:w.__FR.cross);
...
qx.Class.define(d,{extend:qx.ui.table.selection.Manager,construct:function(a){arguments.callee.base.call(this);
this.__FT=a;
},members:{__FT:null,getTable:function(){return this.__FT;
...
switch(B){case c:return false;
case b:if(!s.bHideOpenClose){C.setState(s,{bOpened:!s.bOpened});
}return p.getOpenCloseClickSelectsRow()?false:true;
default:return true;
}}}var n=l(this.__FT,j,k);
...
this.__Ar=false;
this.__As=false;
},properties:{behavior:{check:l,init:null,nullable:true,apply:j,event:d}},members:{__As:null,__Ar:null,__At:null,_applyBehavior:function(u,v){if(v!=null){v.dispose();
v=null;
}u._setNumColumns(this.getOverallColumnCount());
u.setTableColumnModel(this);
},init:function(a,b){arguments.callee.base.call(this,a);

if(this.__At==null){this.__At=b;
...
},getTable:function(){return this.__At;
...
},_onappear:function(event){if(this.__Ar){return ;
}this.__Ar=true;
{};
this.getBehavior().onAppear(event,event.getType()!==q);
this.__At._updateScrollerWidths();
this.__At._updateScrollBarVisibility();
this.__Ar=false;
this.__As=true;
},_onTableWidthChanged:function(event){if(this.__Ar||!this.__As){return ;
}this.__Ar=true;
{};
this.getBehavior().onTableWidthChanged(event);
this.__Ar=false;
},_onverticalscrollbarchanged:function(event){if(this.__Ar||!this.__As){return ;
}this.__Ar=true;
{};
this.getBehavior().onVerticalScrollBarChanged(event);
qx.event.Timer.once(function(){if(this.__At&&!this.__At.isDisposed()){this.__At._updateScrollerWidths();
this.__At._updateScrollBarVisibility();
}},this,0);
this.__Ar=false;
},_oncolumnwidthchanged:function(event){if(this.__Ar||!this.__As){return ;
}this.__Ar=true;
{};
this.getBehavior().onColumnWidthChanged(event);
this.__Ar=false;
},_onvisibilitychanged:function(event){if(this.__Ar||!this.__As){return ;
}this.__Ar=true;
{};
...
},members:{__Au:null,renderLayout:function(f,top,g,h){this.__Au=g;
},getComputedWidth:function(){return this.__Au;
...
qx.Class.define(g,{type:f,extend:qx.core.Object,members:{_setNumColumns:function(h){throw new Error("_setNumColumns is abstract");
...
(function(){var A="Function",z="__Aw",y="Boolean",x="minWidth",w="width",v="__Ax",u="qx.ui.table.columnmodel.Resize",t="qx.ui.table.columnmodel.resizebehavior.Default",s="maxWidth";
...
this.__Av=[];
this.__Aw=new qx.ui.layout.HBox();
this.__Aw.connectToWidget(this);
this.__Ax=new qx.util.DeferredCall(this._computeColumnsFlexWidth,this);
},properties:{newResizeBehaviorColumnData:{check:A,init:function(r){return new qx.ui.core.ColumnData();
}},initializeWidthsOnEveryAppear:{check:y,init:false},tableColumnModel:{check:u}},members:{__Aw:null,__Ay:null,__Av:null,__Ax:null,__Az:false,setWidth:function(R,S,T){if(R>=this.__Av.length){throw new Error("Column number out of range");
}this.__Av[R].setColumnWidth(S,T);
this.__Ax.schedule();
},setMinWidth:function(W,X){if(W>=this.__Av.length){throw new Error("Column number out of range");
}this.__Av[W].setMinWidth(X);
this.__Ax.schedule();
},setMaxWidth:function(U,V){if(U>=this.__Av.length){throw new Error("Column number out of range");
}this.__Av[U].setMaxWidth(V);
this.__Ax.schedule();
},set:function(O,P){for(var Q in P){switch(Q){case w:this.setWidth(O,P[Q]);
break;
case x:this.setMinWidth(O,P[Q]);
break;
case s:this.setMaxWidth(O,P[Q]);
break;
default:throw new Error("Unknown property: "+Q);
}}},onAppear:function(event,D){if(D===true||!this.__Az||this.getInitializeWidthsOnEveryAppear()){this._computeColumnsFlexWidth();
this.__Az=true;
...
},onVisibilityChanged:function(event){var C=event.getData();
if(C.visible){this._computeColumnsFlexWidth();
return;
}this._extendLastColumn(event);
},_setNumColumns:function(M){var N=this.__Av;
if(M<=N.length){N.splice(M,N.length);
return;
}for(var i=N.length;i<M;i++){N[i]=this.getNewResizeBehaviorColumnData()();
N[i].columnNumber=i;
}},getLayoutChildren:function(){return this.__Ay;
},_computeColumnsFlexWidth:function(){this.__Ax.cancel();
var o=this._getAvailableWidth();

if(o===null){return;
}var j=this.getTableColumnModel();
var m=j.getVisibleColumns();
var n=m.length;
var k=this.__Av;
var i,l;
...
for(i=0;i<n;i++){q.push(k[m[i]]);
}this.__Ay=q;
this.__AA();
this.__Aw.renderLayout(o,100);
for(i=0,l=q.length;i<l;i++){var p=q[i].getComputedWidth();
j.setColumnWidth(m[i],p);
}},__AA:function(){this.__Aw.invalidateChildrenCache();
var B=this.__Ay;

for(var i=0,l=B.length;i<l;i++){B[i].invalidateLayoutCache();
...
}}},_extendLastColumn:function(event){var d=this.getTableColumnModel();
var h=event.getData();
if(h.visible){return;
}var c=d.getVisibleColumns();
if(c.length==0){return;
}var b=this._getAvailableWidth(d);
var a=c.length;
var i;
var f;
var g=0;

for(i=0;i<a;i++){g+=d.getColumnWidth(c[i]);
}if(g<b){f=c[c.length-1];
var e=(b-(g-d.getColumnWidth(f)));
d.setColumnWidth(f,e);
}},_getResizeColumnData:function(){return this.__Av;
}},destruct:function(){this.__Av=this.__Ay=null;
this._disposeObjects(z,v);
}});
})();
(function(){var r="number",q="object",p="qx.ui.treevirtual.MNode";
qx.Mixin.define(p,{members:{nodeGet:function(a){if(typeof (a)==q){return a;
}else if(typeof (a)==r){return this.getTableModel().getData()[a];
}else{throw new Error("Expected node object or node id");
}},nodeToggleOpened:function(G){var H;
var I;

if(typeof (G)==q){H=G;
I=H.nodeId;
}else if(typeof (G)==r){I=G;
H=this.getTableModel().getData()[I];
}else{throw new Error("Expected node object or node id");
}this.getTableModel().setState(I,{bOpened:!H.bOpened});
},nodeSetState:function(v,w){var x;

if(typeof (v)==q){x=v.nodeId;
}else if(typeof (v)==r){x=v;
}else{throw new Error("Expected node object or node id");
}this.getTableModel().setState(x,w);
},nodeSetLabel:function(N,O){this.nodeSetState(N,{label:O});
},nodeGetLabel:function(t){var u=this.nodeGet(t);
return u.label;
},nodeSetSelected:function(e,b){this.nodeSetState(e,{bSelected:b});
},nodeGetSelected:function(f){var g=this.nodeGet(f);
return g.bSelected;
},nodeSetOpened:function(A,b){var B;

if(typeof (A)==q){B=A;
}else if(typeof (A)==r){B=this.getTableModel().getData()[A];
}else{throw new Error("Expected node object or node id");
}if(b!=B.bOpened){this.nodeToggleOpened(B);
}},nodeGetOpened:function(J){var K=this.nodeGet(J);
return K.bOpened;
},nodeSetHideOpenClose:function(s,b){this.nodeSetState(s,{bHideOpenClose:b});
},nodeGetHideOpenClose:function(y){var z=this.nodeGet(y);
return z.bHideOpenClose;
},nodeSetIcon:function(h,i){this.nodeSetState(h,{icon:i});
},nodeGetIcon:function(l){var m=this.nodeGet(l);
return m.icon;
},nodeSetSelectedIcon:function(c,d){this.nodeSetState(c,{iconSelected:d});
},nodeGetSelectedIcon:function(C){var D=this.nodeGet(C);
return D.iconSelected;
},nodeSetCellStyle:function(j,k){this.nodeSetState(j,{cellStyle:k});
