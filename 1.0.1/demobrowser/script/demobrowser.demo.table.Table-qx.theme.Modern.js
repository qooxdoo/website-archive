...
(function(){var t="execute",s="changeValue",r="A date",q="Boolean",p="icon/22/actions/edit-undo.png",o="Change ID sort method",n="icon/22/status/dialog-information.png",m="Change row with ID 10",l="A number",k="icon/16/apps/office-calendar.png",J="Remove 5 rows",I="Add 10 rows",H="",G="demobrowser.demo.table.Table",F="_tableModel",E="Table",D="icon/22/actions/list-remove.png",C="Whether the the first visible row should ",B=", ",A="icon/22/actions/list-add.png",y="Selected rows:<br>",z="Keep first row",w="be rendered completely when scrolling.",x="Show selection",u="Demonstrate use of alternate sorting algorithm.",v="ID";
qx.Class.define(G,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return E;
},createTable:function(){var Q=this.createRandomRows(50);
var P=this._tableModel=new qx.ui.table.model.Simple();
P.setColumns([v,l,r,q]);
P.setData(Q);
...
O.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(k,r));
return R;
},createControls:function(){var bd=new qx.ui.toolbar.ToolBar();
var bb,bc,be;
bc=new qx.ui.toolbar.Part();
bd.add(bc);
bb=new qx.ui.toolbar.Button(m,p);
bb.addListener(t,function(d){var e=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,e[0][i]);
}this.info("Row 10 changed");
},this);
bc.add(bb);
bb=new qx.ui.toolbar.Button(I,A);
bb.addListener(t,function(K){var L=this.createRandomRows(10);
this._tableModel.addRows(L);
this.info("10 rows added");
},this);
bc.add(bb);
bb=new qx.ui.toolbar.Button(J,D);
bb.addListener(t,function(M){var N=this._tableModel.getRowCount();
this._tableModel.removeRows(N-5,5);
this.info("5 rows removed");
},this);
bc.add(bb);
bb=new qx.ui.toolbar.Button(x,n);
bb.addListener(t,function(W){var X=[];
ba.getSelectionModel().iterateSelection(function(Y){X.push(Y+H);
});
this.showDialog(y+X.join(B));
},this);
bc.add(bb);
bc=new qx.ui.toolbar.Part();
bd.add(bc);
var ba=this._table;
be=new qx.ui.toolbar.CheckBox(z);
be.set({value:ba.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(C+w)});
be.addListener(s,function(bf){ba.setKeepFirstVisibleRowComplete(this.getValue());
},be);
bc.add(be);
be=new qx.ui.toolbar.CheckBox(o);
be.set({value:ba.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(u)});
be.addListener(s,function(a){if(a.getData()){var c=function(f,g){var h=f[arguments.callee.columnIndex];
var j=g[arguments.callee.columnIndex];

if(h%2==1&&j%2==0){return 1;
}
if(j%2==1&&h%2==0){return -1;
}return (h>j)?1:((h==j)?0:-1);
};
var b=function(S,T){var U=S[arguments.callee.columnIndex];
var V=T[arguments.callee.columnIndex];

if(U%2==1&&V%2==0){return -1;
}
if(V%2==1&&U%2==0){return 1;
}return (U<V)?1:((U==V)?0:-1);
};
ba.getTableModel().setSortMethods(0,{ascending:c,descending:b});
