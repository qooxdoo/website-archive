...
var L=0;
var H=function(v){var y=[];
var x=new Date().getTime();
var z=400*24*60*60*1000;

for(var A=0;A<v;A++){var w=new Date(x+Math.random()*z-z/2);
y.push({renderer:u,location:i,data:[L++,Math.random()*10000,w]});
}return y;
};
...
qx.Class.define(h,{extend:qx.ui.progressive.renderer.table.cell.Default,statics:{onmouseover:function(O){O.__Bv=qx.bom.element.Style.get(O,g);
O.style.color=s;
O.parentNode.style.height=32;
O.__Bw=O.innerHTML;
var Q=Date.parse(O.attributes.getNamedItem(r).nodeValue);
var P=new Date(Q);
O.innerHTML=P.toUTCString()+f+P.toString();
},onmouseout:function(B){B.style.color=B.__Bv;
B.parentNode.style.height=16;
B.innerHTML=B.__Bw;
}},members:{_getCellExtras:function(a){var b=[];
b.push(arguments.callee.base.call(this));
