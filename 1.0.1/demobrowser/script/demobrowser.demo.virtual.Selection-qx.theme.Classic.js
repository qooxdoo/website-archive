...
(function(){var B="manager",A="multi",z=" / ",y="bold",x="value",w="white",v="selected",u="#EEE",t="horizontal",s="table-scroller-header",W="one",V="single",U="changeValue",T="table-row-background-selected",S="additive",R="default",Q="Additive Selection",P="Cell Line Selection",O="addaptive",N="Options",I="Row Selection",J="changeSelection",G="Column Selection",H="Enable drag selection",E="demobrowser.demo.virtual.Selection",F="Selection Mode",C="Single Selection",D="Cell Rectangle Selection",K="Enable quick selection",L="One Selection",M="Multi Selection";
qx.Class.define(E,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var bs=qx.theme.manager.Font.getInstance().resolve(R).getStyles();
this._fontCss=qx.bom.element.Style.compile(bs);
var bB=new qx.ui.layout.Grid(5,0);
bB.setRowFlex(2,1);
bB.setColumnFlex(0,1);
bB.setColumnFlex(1,1);
bB.setColumnFlex(2,1);
bB.setColumnFlex(3,1);
var bp=[];
var bz=new qx.ui.container.Composite(bB);
this.getRoot().add(bz,{edge:5});
bz.add(new qx.ui.basic.Label(I).set({font:y,decorator:s,padding:3,allowGrowX:true}),{row:1,column:0});
var br=this.createRowSelectionScroller();
bp.push(br.getUserData(B));
bz.add(br,{row:2,column:0});
bz.add(new qx.ui.basic.Label(G).set({font:y,decorator:s,padding:3,allowGrowX:true}),{row:1,column:1});
var br=this.createColumnSelectionScroller();
bp.push(br.getUserData(B));
bz.add(br,{row:2,column:1});
bz.add(new qx.ui.basic.Label(D).set({font:y,decorator:s,padding:3,allowGrowX:true}),{row:1,column:2});
var br=this.createCellRectangleSelectionScroller();
bp.push(br.getUserData(B));
bz.add(br,{row:2,column:2});
bz.add(new qx.ui.basic.Label(P).set({font:y,decorator:s,padding:3,allowGrowX:true}),{row:1,column:3});
var br=this.createCellLineSelectionScroller();
bp.push(br.getUserData(B));
bz.add(br,{row:2,column:3});
var bx=new qx.ui.layout.Grid(20,4);
var bu=new qx.ui.container.Composite(bx).set({padding:15,backgroundColor:w});
bz.add(bu,{row:0,column:0,colSpan:4});
bu.add(new qx.ui.basic.Label(F).set({font:y,padding:3}),{row:0,column:0});
var bt=new qx.ui.form.RadioButton(C);
bt.setUserData(x,V);
bu.add(bt,{row:1,column:0});
var bw=new qx.ui.form.RadioButton(M);
bw.setUserData(x,A);
bw.setValue(true);
bu.add(bw,{row:2,column:0});
var bq=new qx.ui.form.RadioButton(Q);
bq.setUserData(x,S);
bu.add(bq,{row:3,column:0});
var bo=new qx.ui.form.RadioButton(L);
bo.setUserData(x,W);
bu.add(bo,{row:4,column:0});
var bv=new qx.ui.form.RadioGroup(bt,bw,bq,bo);
bv.addListener(J,function(e){var Y=e.getData()[0].getUserData(x);
bp.forEach(function(g){g.setMode(Y);
});

if(Y==V||Y==W){bA.setEnabled(false);
by.setEnabled(true);
}else if(Y==A||Y==O){bA.setEnabled(true);
by.setEnabled(false);
}});
bu.add(new qx.ui.basic.Label(N).set({font:y,padding:3}),{row:0,column:1});
var bA=new qx.ui.form.CheckBox(H);
bu.add(bA,{row:1,column:1});
bA.addListener(U,function(e){if(e.getData()){var bg=bp[0].getMode();

if(bg==V||bg==W){this.debug("Drag selection is only available for the modes multi or additive");
}}bp.forEach(function(bk){bk.setDrag(e.getData());
});
});
var by=new qx.ui.form.CheckBox(K).set({enabled:false});
bu.add(by,{row:2,column:1});
by.addListener(U,function(e){if(e.getData()){var X=bp[0].getMode();

if(X==A||X==S){this.debug("Quick selection is only available for the modes multi or additive");
}}bp.forEach(function(bC){bC.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bN=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var bL=new qx.ui.virtual.layer.Row(w,u);
bN.getPane().addLayer(bL);
bN.getPane().addLayer(new qx.ui.virtual.layer.GridLines(t));
var bM=new qx.ui.virtual.cell.Cell();
var bK=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(p,q){var r={};

if(bJ.isItemSelected(p)){r.selected=true;
}return bM.getCellProperties(p+z+q,r);
}});
bN.getPane().addLayer(bK);
var bJ=new qx.ui.virtual.selection.Row(bN.getPane(),{styleSelectable:function(bD,bE,bF){if(bE!==v){return;
}
if(bF){bL.setDecorator(v);
}else{bL.setDecorator(null);
}bK.updateLayerData();
}});
...
bR.getPane().addLayer(new qx.ui.virtual.layer.Row(w,u));
var bQ=new qx.ui.virtual.layer.Column();
bR.getPane().addLayer(bQ);
bR.getPane().addLayer(new qx.ui.virtual.layer.GridLines(t));
var bS=new qx.ui.virtual.cell.Cell();
var bP=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bl,bm){var bn={};

if(bO.isItemSelected(bm)){bn.selected=true;
}return bS.getCellProperties(bl+z+bm,bn);
}});
bR.getPane().addLayer(bP);
var bO=new qx.ui.virtual.selection.Column(bR.getPane(),{styleSelectable:function(ba,bb,bc){if(bb!==v){return;
}
if(bc){bQ.setDecorator(v);
...
bR.setUserData(B,bO);
return bR;
},createCellRectangleSelectionScroller:function(){var k=new qx.ui.virtual.core.Scroller(1000,100,20,100);
k.getPane().addLayer(new qx.ui.virtual.layer.Row(w,u));
var j=new qx.ui.virtual.cell.Cell();
var l=new qx.ui.virtual.cell.Cell().set({backgroundColor:T});
var i=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bh,bi){var bj={};

if(h.isItemSelected({row:bh,column:bi})){bj.selected=true;
}
if(bj.selected){return l.getCellProperties(bh+z+bi,bj);
}else{j.resetBackgroundColor();
return j.getCellProperties(bh+z+bi,bj);
}}});
k.getPane().addLayer(i);
k.getPane().addLayer(new qx.ui.virtual.layer.GridLines(t));
var h=new qx.ui.virtual.selection.CellRectangle(k.getPane(),{styleSelectable:function(m,n,o){i.updateLayerData();
}});
h.attachMouseEvents(k.getPane());
h.attachKeyEvents(k);
h.set({mode:A});
h.addItem({row:0,column:0});
k.setUserData(B,h);
return k;
},createCellLineSelectionScroller:function(){var d=new qx.ui.virtual.core.Scroller(1000,100,20,100);
d.getPane().addLayer(new qx.ui.virtual.layer.Row(w,u));
var c=new qx.ui.virtual.cell.Cell();
var f=new qx.ui.virtual.cell.Cell().set({backgroundColor:T});
var b=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bG,bH){var bI={};

if(a.isItemSelected({row:bG,column:bH})){bI.selected=true;
}
if(bI.selected){return f.getCellProperties(bG+z+bH,bI);
}else{c.resetBackgroundColor();
return c.getCellProperties(bG+z+bH,bI);
}}});
d.getPane().addLayer(b);
d.getPane().addLayer(new qx.ui.virtual.layer.GridLines(t));
var a=new qx.ui.virtual.selection.CellLines(d.getPane(),{styleSelectable:function(bd,be,bf){b.updateLayerData();
}});
...
(function(){var q="under",p="above",o="left",n="right",m="x",l="qx.ui.virtual.selection.CellRectangle";
qx.Class.define(l,{extend:qx.ui.virtual.selection.Abstract,members:{_getItemCount:function(){return this._pane.getRowConfig().getItemCount()*this._pane.getColumnConfig().getItemCount();
...
},_getFirstSelectable:function(){var N=this._pane.getRowConfig().getItemCount();
var M=this._pane.getColumnConfig().getItemCount();

for(var Q=0;Q<N;Q++){for(var P=0;P<M;P++){var O={row:Q,column:P};

if(this._isSelectable(O)){return O;
}}}return null;
},_getLastSelectable:function(){var U=this._pane.getRowConfig().getItemCount();
var T=this._pane.getColumnConfig().getItemCount();

for(var W=T-1;W>=0;W--){for(var X=U-1;X>=0;X--){var V={row:X,column:W};

if(this._isSelectable(V)){return V;
}}}return null;
},_getRelatedSelectable:function(r,s){var t={row:r.row,column:r.column};

switch(s){case p:for(var x=r.row-1;x>=0;x--){t.row=x;

if(this._isSelectable(t)){return t;
}}break;
case q:var u=this._pane.getRowConfig().getItemCount();

for(var x=r.row+1;x<u;x++){t.row=x;

if(this._isSelectable(t)){return t;
}}break;
case o:for(var w=r.column-1;w>=0;w--){t.column=w;

if(this._isSelectable(t)){return t;
}}break;
case n:var v=this._pane.getColumnConfig().getItemCount();

for(var w=r.column+1;w<v;w++){t.column=w;
...
}},_selectableToHashCode:function(R){return R.column+m+R.row;
...
var d=b.getItemPosition(a.column);
var c=d+b.getItemSize(a.column)-1;
...
(function(){var h="qx.ui.virtual.selection.CellLines";
qx.Class.define(h,{extend:qx.ui.virtual.selection.CellRectangle,members:{_getSelectableRange:function(a,b){var f=[];
var c=this._pane.getColumnConfig().getItemCount();

if(a.row<b.row||a.row==b.row&&a.column<b.column){var e=a.row*c+a.column;
var d=b.row*c+b.column;
}else{var e=b.row*c+b.column;
var d=a.row*c+a.column;
}
for(var i=e;i<=d;i++){var g={row:Math.floor(i/c),column:i%c};

if(this._isSelectable(g)){f.push(g);
}}return f;
}}});
})();
