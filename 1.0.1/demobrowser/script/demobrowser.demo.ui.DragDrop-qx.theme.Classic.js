...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.ui.DragDrop-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"b00291ed7754"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var m="drop",k="value",j="items",h="move",g="multi",f="dragover",d="icon/16/places/folder.png",c="dragstart",b="Item ",a="copy",z="drag",y="TextArea Target",x="solid",w="Reorderable",v="droprequest",u="#33508D",t="Source",s="demobrowser.demo.ui.DragDrop",r="Simple Target",q="Enable drag",o="\n",p="dragend",n="__FF";
qx.Class.define(s,{extend:qx.application.Standalone,members:{__FF:null,__FG:null,main:function(){arguments.callee.base.call(this);
var L=this.getRoot();
...
var U=e.getCurrentAction();
var W=e.getCurrentType();
var T;

switch(W){case j:T=this.getSelection();

if(U==a){var V=[];

for(var i=0,l=T.length;i<l;i++){V[i]=T[i].clone();
}T=V;
}break;
case k:T=this.getSelection()[0].getLabel();
break;
}if(U==h){var S=this.getSelection();

for(var i=0,l=S.length;i<l;i++){this.remove(S[i]);
}}e.addData(W,T);
});
...
for(var i=0,l=R.length;i<l;i++){this.add(R[i]);
}});
...
B.addListener(p,function(e){C.setDomPosition(-1000,-1000);
});
B.addListener(z,function(e){var O=e.getOriginalTarget();
if(O instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__FG=O;
}
if(!qx.ui.core.Widget.contains(this,O)&&O!=C){return;
}var N=O.getContainerLocation();
C.setWidth(O.getBounds().width);
C.setDomPosition(N.left,N.top);
});
B.addListener(f,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
B.addListener(m,function(e){this.__FH(e.getOriginalTarget());
},this);
C.addListener(m,function(e){this.__FH(this.__FG);
},this);
},__FH:function(P){if(P.classname!="qx.ui.form.ListItem"){return;
}var Q=this.__FF.getSortedSelection();

for(var i=0,l=Q.length;i<l;i++){this.__FF.addBefore(Q[i],P);
this.__FF.addToSelection(Q[i]);
