...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.virtual.CellSpan-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"21180c2d054c"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var q="yellow",p="bold",o="table-scroller-header",n="green",m="__Fb",l=" / ",k="Widget Cells",j="demobrowser.demo.virtual.CellSpan",i="HTML Cells",h="x";
qx.Class.define(j,{extend:qx.application.Standalone,members:{__Fb:null,main:function(){arguments.callee.base.call(this);
var y=new qx.ui.layout.Grid(5,0);
y.setRowFlex(1,1);
y.setColumnFlex(0,1);
y.setColumnFlex(1,1);
var x=new qx.ui.container.Composite(y);
this.getRoot().add(x,{edge:0});
x.add(new qx.ui.basic.Label(i).set({font:p,decorator:o,padding:3,allowGrowX:true}),{row:0,column:0});
var z=this.getHtmlCellSpanScroller();
x.add(z,{row:1,column:0});
x.add(new qx.ui.basic.Label(k).set({font:p,decorator:o,padding:3,allowGrowX:true}),{row:0,column:1});
var z=this.getWidgetCellSpanScroller();
x.add(z,{row:1,column:1});
this.__Fb=new qx.ui.virtual.cell.Cell();
},getHtmlCellSpanScroller:function(){var g=new qx.ui.virtual.core.Scroller(1000,100,50,120);
var f=g.getPane();
var e=new qx.ui.virtual.layer.HtmlCellSpan(this,f.getRowConfig(),f.getColumnConfig());
e.setCellSpan(1,1,2,2);
e.setCellSpan(1,5,3,3);
e.setCellSpan(7,4,2,4);
e.setCellSpan(6,0,10,2);
e.setCellSpan(10,9,10,5);
e.setCellSpan(11,3,6,3);
g.getPane().addLayer(e);
return g;
},getCellProperties:function(r,s){var t=(r+s)%2==0?q:n;
this.__Fb.setBackgroundColor(t);
return this.__Fb.getCellProperties(r+l+s);
...
c.set({backgroundColor:(a+b)%2==0?q:n,value:a+h+b,padding:3});
return c;
},poolCellWidget:function(d){this._pool.push(d);
...
qx.Class.define(f,{extend:qx.ui.virtual.layer.HtmlCell,construct:function(M,N,O){arguments.callee.base.call(this,M);
this._spanManager=new qx.ui.virtual.layer.CellSpanManager(N,O);
},members:{setCellSpan:function(H,I,J,K){var L=H+d+I;
this._spanManager.removeCell(L);

if(J>1||K>1){this._spanManager.addCell(L,H,I,J,K);
}qx.ui.core.queue.Widget.add(this);
},__Fc:function(P,Q,R,S,top,T,U){var W=this._cellProvider.getCellProperties(Q,R);
var V=W.insets||[0,0];
...
var G=B[i];
this.__Fc(E,C.firstRow,C.firstColumn,G.left,G.top,G.width,G.height);
}}else{w=[];
...
if(!w[v][D]){this.__Fc(E,v,D,F,top,z,u);
}D++;
F+=z;
}top+=u;
v++;
...
(function(){var y="change",x="lastColumn",w="firstColumn",v="firstRow",u="qx.ui.virtual.layer.CellSpanManager",t="lastRow";
qx.Class.define(u,{extend:qx.core.Object,construct:function(Q,R){arguments.callee.base.call(this);
{};
this._cells={};
this._invalidateSortCache();
this._invalidatePositionCache();
Q.addListener(y,this._onRowConfigChange,this);
R.addListener(y,this._onColumnConfigChange,this);
this._rowConfig=Q;
this._columnConfig=R;
},members:{addCell:function(m,n,o,p,q){this._cells[m]={firstRow:n,lastRow:n+p-1,firstColumn:o,lastColumn:o+q-1,id:m};
this._invalidateSortCache();
},removeCell:function(k){delete (this._cells[k]);
this._invalidateSortCache();
},_invalidateSortCache:function(){this._sorted={};
},_getSortedCells:function(r){if(this._sorted[r]){return this._sorted[r];
}var s=this._sorted[r]=qx.lang.Object.getValues(this._cells);
s.sort(function(a,b){return a[r]<b[r]?-1:1;
});
return s;
},_findCellsInRange:function(D,E,F){var H=this._getSortedCells(D);

if(H.length==0){return {};
}var J=0;
var G=H.length-1;
while(true){var L=J+((G-J)>>1);
var I=H[L];

if(I[D]>=E&&(L==0||H[L-1][D]<E)){break;
}
if(I[D]>=E){G=L-1;
}else{J=L+1;
}
...
while(I&&I[D]>=E&&I[D]<=F){K[I.id]=I;
I=H[L++];
}return K;
},findCellsInWindow:function(S,T,U,V){var ba={};

if(U>0){var X=S+U-1;
qx.lang.Object.merge(ba,this._findCellsInRange(v,S,X),this._findCellsInRange(t,S,X));
}var W={};

if(V>0){var Y=T+V-1;
qx.lang.Object.merge(W,this._findCellsInRange(w,T,Y),this._findCellsInRange(x,T,Y));
}return this.__Fd(W,ba);
},__Fd:function(c,d){var f=[];
...
this._columnPos=[];
},_getRowPosition:function(h){var j=this._rowPos[h];

if(j!==undefined){return j;
}j=this._rowPos[h]=this._rowConfig.getItemPosition(h);
return j;
...
return bc;
},_getSingleCellBounds:function(M,N,O){var P={left:0,top:0,width:0,height:0};
P.height=this._getRowPosition(M.lastRow)+this._rowConfig.getItemSize(M.lastRow)-this._getRowPosition(M.firstRow);
P.top=this._getRowPosition(M.firstRow)-this._getRowPosition(N);
P.width=this._getColumnPosition(M.lastColumn)+this._columnConfig.getItemSize(M.lastColumn)-this._getColumnPosition(M.firstColumn);
P.left=this._getColumnPosition(M.firstColumn)-this._getColumnPosition(O);
return P;
},getCellBounds:function(z,A,B){var C=[];

for(var i=0,l=z.length;i<l;i++){C.push(this._getSingleCellBounds(z[i],A,B));
}return C;
...
}},destruct:function(){this._rowConfig.removeListener(y,this._onRowConfigChange,this);
this._columnConfig.removeListener(y,this._onColumnConfigChange,this);
this._cells=this._sorted=this._rowPos=this._columnPos=this._rowConfig=this._columnConfig=null;
}});
})();
(function(){var r="cell.spanning",q="cell.rowspan",p="cell.column",o="cell.colspan",n="cell.row",m="x",k="qx.ui.virtual.layer.WidgetCellSpan",j="_spanManager",h="_cellLayer";
qx.Class.define(k,{extend:qx.ui.virtual.layer.Abstract,include:[qx.ui.core.MChildrenHandling],construct:function(V,W,X){arguments.callee.base.call(this);
this.setZIndex(2);
this._spanManager=new qx.ui.virtual.layer.CellSpanManager(W,X);
this._cellProvider=V;
this.__Fe=[];
this._cellLayer=new qx.ui.virtual.layer.WidgetCell(this.__Ff());
...
if(!u||u.getUserData(r)){var x=this._getChildren();

for(var i=0,l=x.length;i<l;i++){var w=x[i];

if(w==this._cellLayer){continue;
}var v={row:w.getUserData(n),column:w.getUserData(p),rowSpan:w.getUserData(q),colSpan:w.getUserData(o)};

if(v.row<=s&&s<v.row+v.rowSpan&&v.column<=t&&t<v.column+v.colSpan){return w;
}}return null;
}return u;
},__Fe:null,setCellSpan:function(y,z,A,B){var C=y+m+z;
this._spanManager.removeCell(C);

if(A>1||B>1){this._spanManager.addCell(C,y,z,A,B);
}qx.ui.core.queue.Widget.add(this);
},__Ff:function(){var self=this;
var U=this._cellProvider;
var T=this.__Fe;
var S={getCellWidget:function(a,b){if(!self._spanMap[a][b]){var c=U.getCellWidget(a,b);
}else{var c=T.pop();

if(!c){c=new qx.ui.core.Spacer();
c.setUserData(r,1);
}}return c;
},poolCellWidget:function(Q){if(Q.getUserData(r)){T.push(Q);
}else{U.poolCellWidget(Q);
}}};
return S;
},__Fg:function(D,E,F,G){this._cells=this._spanManager.findCellsInWindow(D,E,F,G);

if(this._cells.length>0){this._bounds=this._spanManager.getCellBounds(this._cells,D,E);
this._spanMap=this._spanManager.computeCellSpanMap(this._cells,D,E,F,G);
}else{this._bounds=[];
this._spanMap=[];

for(var i=0;i<F;i++){this._spanMap[D+i]=[];
}}},__Fh:function(){var L=this.getChildren();

for(var i=L.length-1;i>=0;i--){var K=L[i];

if(K!==this._cellLayer){this._cellProvider.poolCellWidget(K);
this._remove(K);
}}
for(var i=0,l=this._cells.length;i<l;i++){var J=this._cells[i];
var H=this._bounds[i];
var I=this._cellProvider.getCellWidget(J.firstRow,J.firstColumn);

if(I){I.setUserBounds(H.left,H.top,H.width,H.height);
I.setUserData(n,J.firstRow);
I.setUserData(p,J.firstColumn);
I.setUserData(q,J.lastRow-J.firstRow+1);
I.setUserData(o,J.lastColumn-J.firstColumn+1);
this._add(I);
}}},_fullUpdate:function(d,e,f,g){this.__Fg(d,e,f.length,g.length);
this.__Fh();
this._cellLayer.fullUpdate(d,e,f,g);
},_updateLayerWindow:function(M,N,O,P){this.__Fg(M,N,O.length,P.length);
this.__Fh();
this._cellLayer.updateLayerWindow(M,N,O,P);
}},destruct:function(){var R=this._getChildren();

for(var i=0;i<R.length;i++){R[i].dispose();
}this._disposeObjects(j,h);
this._cellProvider=this.__Fe=this._cells=this._bounds=this._spanMap=null;
}});
})();
