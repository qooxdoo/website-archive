...
this.__zj(D,C);
},__zj:function(E,F){var G=new F.clazz();
...
if(J!=null){for(var i=0;i<J.length;i++){if(G instanceof qx.ui.form.Form){this.__zk(G,J[i]);
}else{this.__zj(G,J[i]);
...
}}},__zk:function(L,M){if(M.type===j){L.addGroupHeader(M.label);
return;
