...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.widget.ComboBox-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"39e4b4049faf"},
  boot : "boot",
  bootIsInline : true,
...
},_createDefaultExample:function(){var G=new qx.ui.basic.Label(c);
G.setFont(t);
this.getRoot().add(G,{left:20,top:25});
var E=new qx.ui.form.ComboBox();
for(var i=1;i<31;i++){var F=new qx.ui.form.ListItem(g+i+s+Math.pow(2,i));
E.add(F);
}E.addListener(j,function(e){this.debug("ChangeValue: "+e.getData());
});
this.getRoot().add(E,{left:20,top:40});
},_createIconExample:function(){var A=new qx.ui.basic.Label(k);
A.setFont(t);
this.getRoot().add(A,{left:160,top:25});
var y=new qx.ui.form.ComboBox();
for(var i=1;i<31;i++){var z=new qx.ui.form.ListItem(i+l,f);
y.add(z);
}this.getRoot().add(y,{left:160,top:40});
},_createHtmlExample:function(){var x=new qx.ui.basic.Label(d);
x.setFont(t);
this.getRoot().add(x,{left:300,top:25});
var u=new qx.ui.form.ComboBox().set({width:200});
var v=[r,q,m,n,b,p];
for(var i=0;i<v.length;i++){var w=new qx.ui.form.ListItem(v[i]);

if(i>0){w.setRich(true);
}u.add(w);
}this.getRoot().add(u,{left:300,top:40});
},_createWideExample:function(){var D=new qx.ui.basic.Label(o);
D.setFont(t);
this.getRoot().add(D,{left:20,top:285});
var B=new qx.ui.form.ComboBox();
B.setWidth(300);
for(var i=1;i<101;i++){var C=new qx.ui.form.ListItem(i+a);
B.add(C);
}B.setValue(B.getChildrenContainer().getSelectables()[0].getLabel());
this.getRoot().add(B,{left:20,top:300});
}}});
})();
