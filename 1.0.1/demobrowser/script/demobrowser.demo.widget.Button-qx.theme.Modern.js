...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.widget.Button-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"2c2d4457158d"},
  boot : "boot",
  bootIsInline : true,
...
c.setPadding(20);
this.getRoot().add(c,{left:0,top:0});
this.addNormalButtons(c);
this.addToggleButton(c);
this.addRepeatButton(c);
this.addHoverButton(c);
},addNormalButtons:function(B){var C=new qx.ui.form.Button(o,i);
B.add(C);
var D=new qx.ui.form.Button(l,h);
D.setEnabled(false);
B.add(D);
},addToggleButton:function(t){var u=new qx.ui.form.ToggleButton(d,j);
u.focus();
t.add(u);
u.addListener(f,function(e){this.debug("Checked: "+e.getData());
},this);
},addRepeatButton:function(y){var z=new qx.ui.form.RepeatButton(null,m);
y.add(z);
var A=new qx.ui.basic.Label(s);
A.setDecorator(q);
A.setPadding(2,4);
A.setBackgroundColor(r);
y.add(A);
z.addListener(p,function(){var E=parseInt(A.getValue())+1;
A.setValue(E.toString());
});
},addHoverButton:function(v){var w=new qx.ui.form.HoverButton(g).set({padding:10,backgroundColor:n});
v.add(w);
var x=new qx.ui.basic.Label(s);
x.setDecorator(q);
x.setPadding(2,4);
x.setBackgroundColor(r);
v.add(x);
w.addListener(p,function(){var a=parseInt(x.getValue())+1;
x.setValue(a.toString());
});
}}});
})();
