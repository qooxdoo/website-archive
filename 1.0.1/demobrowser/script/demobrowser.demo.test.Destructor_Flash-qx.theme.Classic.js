...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.test.Destructor_Flash-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"8164185a2468"},
  boot : "boot",
  bootIsInline : true,
...
qx.Class.define(n,{extend:qx.application.Native,members:{__xK:null,__xL:0,main:function(){arguments.callee.base.call(this);
this.__xK=[];
var q=new qx.event.Timer(5000);
q.addListener(k,this.runTest,this);
q.start();
this.runTest();
},runTest:function(){for(var i=0;i<5;i++){var a=qx.bom.Element.create(e);
qx.bom.element.Style.set(a,f,g);
qx.bom.element.Style.set(a,j,m);
document.body.appendChild(a);
var b=d+this.__xL;
this.__xL++;
qx.bom.Flash.create(a,{movie:h,id:b});
this.debug("created: "+b);
this.__xK.push(a);
}var c=qx.event.Timer.once(this.clear,this,2500);
c.start();
},clear:function(){var o=this.__xK;

for(var i=0,l=o.length;i<l;i++){var p=o[i].firstChild.id;
qx.bom.Flash.destroy(o[i]);
this.debug("cleaned: "+p);
...
qx.Class.define(S,{statics:{_flashObjects:{},create:function(m,n,o,p,q){if(!q){q=window;
}{};

if(!n.width){n.width=A;
}
if(!n.height){n.height=A;
}p=p?qx.lang.Object.clone(p):{};

if(!p[y]){p[y]=n.movie;
}n[M]=n.movie;
delete n.movie;
if(o){for(var name in o){if(typeof p.flashvars!=D){p.flashvars+=J+name+x+o[name];
}else{p.flashvars=name+x+o[name];
}}}var r=this.__so(m,n,p,q);
this._flashObjects[n.id]=r;
return r;
},destroy:qx.core.Variant.select(C,{"mshtml":function(ba,bb){ba=this.__sj(ba);

if(ba.readyState==4){this.__sk(ba);
}else{if(!bb){bb=window;
}qx.bom.Event.addNativeListener(bb,Q,function(){qx.bom.Flash.__sk(ba);
});
}},"default":function(U,V){U=this.__sj(U);
U.parentNode.removeChild(U);
delete this._flashObjects[U.id];
}}),__sj:function(X){if(!X){throw new Error("DOM element is null or undefined!");
}
if(X.tagName.toLowerCase()!==B){X=X.firstChild;
}
if(!X||X.tagName.toLowerCase()!==B){throw new Error("DOM element has or is not a flash object!");
}return X;
},__sk:qx.core.Variant.select(C,{"mshtml":qx.event.GlobalError.observeMethod(function(s){for(var i in s){if(typeof s[i]==w){s[i]=null;
}}s.parentNode.removeChild(s);
delete this._flashObjects[s.id];
}),"default":null}),__sl:qx.event.GlobalError.observeMethod(function(){for(var W in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[W]);
}window.__sm=function(){};
window.__sn=function(){};
window.detachEvent(O,qx.bom.Flash.__sl);
}),__so:qx.core.Variant.select(C,{"mshtml":function(a,b,c,d){c.movie=b.data;
delete b.data;
delete b.classid;
var e=L;

for(name in c){e+=R+name+E+c[name]+T;
}if(b.id){a.innerHTML=H+b.id+F+e+z;
delete b.id;
}else{a.innerHTML=u+e+z;
}for(var name in b){a.firstChild.setAttribute(name,b[name]);
}return a.firstChild;
},"default":function(f,g,h,j){delete g.classid;
delete h.movie;
var l=qx.bom.Element.create(B,g,j);
l.setAttribute(P,v);
var k;

for(var name in h){k=qx.bom.Element.create(t,{},j);
k.setAttribute(N,name);
k.setAttribute(I,h[name]);
l.appendChild(k);
}f.appendChild(l);
return l;
}})},defer:function(Y){if(qx.core.Variant.isSet(C,K)){qx.bom.Event.addNativeListener(window,G,Y.__sl);
}}});
})();
