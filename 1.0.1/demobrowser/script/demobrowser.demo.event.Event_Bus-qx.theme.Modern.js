...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.event.Event_Bus","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.event.Event_Bus-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"47438fedbef4"},
  boot : "boot",
  bootIsInline : true,
...
qx.Class.define(j,{extend:qx.application.Native,members:{main:function(){arguments.callee.base.call(this);
var p=document.getElementById(c);

if(p){qx.bom.Element.addListener(p,b,this._clearLog,this);
}},_initLogger:function(m,n,o){this.__xD=e+m.join(h)+k;
this.__xE=o||50;
this.__xF=n;
this._clearLog();
},_clearLog:function(){this.__xF.innerHTML=this.__xD+l;
this.__xG=[];
},_log:function(q){this.__xG.unshift(q);
this.__xG=this.__xG.slice(0,this.__xE);
var r=[this.__xD];

for(var i=0;i<this.__xG.length;i++){r.push(d,this.__xG[i].join(f),a);
}r.push(l);
this.__xF.innerHTML=r.join(g);
this.__xF.scrollTop=0;
},__xD:null,__xE:null,__xF:null,__xG:null},destruct:function(){this.__xF=this.__xG=null;
}});
})();
(function(){var s="",r="X",q="start",p="finished",o="loading",n="_anotherStartCallback",m="_finishedCallback",l="_startCallback",k="_anotherFinishedCallback",j="_loadingCallback",d="logger",i="Callback",h="Dispatch",c="Message",b="Receiving",g="Subscribe",f="demobrowser.demo.event.Event_Bus";
qx.Class.define(f,{extend:demobrowser.demo.event.EventDemo,members:{main:function(){arguments.callee.base.call(this);
this._initLogger([g,h,b,c,i],document.getElementById(d),50);
var a=qx.event.message.Bus;
a.subscribe(q,this._startCallback,this);
this._log([r,s,s,q,l]);
a.subscribe(q,this._anotherStartCallback,this);
this._log([r,s,s,q,n]);
a.subscribe(o,this._loadingCallback,this);
this._log([r,s,s,o,j]);
a.subscribe(p,this._finishedCallback,this);
this._log([r,s,s,p,m]);
a.subscribe(p,this._anotherFinishedCallback,this);
this._log([r,s,s,p,k]);
qx.event.Timer.once(function(e){this._log([s,r,s,q,s]);
a.getInstance().dispatch(q);
},this,1000);
qx.event.Timer.once(function(e){this._log([s,r,s,o,s]);
a.getInstance().dispatch(o);
},this,2500);
qx.event.Timer.once(function(e){this._log([s,r,s,o,s]);
a.getInstance().dispatch(o);
},this,4000);
qx.event.Timer.once(function(e){this._log([s,r,s,p,s]);
a.getInstance().dispatch(p);
},this,5000);
},_startCallback:function(e){this._log([s,s,r,q,l]);
},_anotherStartCallback:function(e){this._log([s,s,r,q,n]);
},_loadingCallback:function(e){this._log([s,s,r,o,j]);
},_finishedCallback:function(e){this._log([s,s,r,p,m]);
},_anotherFinishedCallback:function(e){this._log([s,s,r,p,k]);
}}});
})();
...
(function(){var x="function",w="qx.event.message.Bus",v="*",u="undefined",t="string",s="singleton";
qx.Class.define(w,{type:s,extend:qx.core.Object,statics:{getSubscriptions:function(){return this.getInstance().getSubscriptions();
},subscribe:function(y,z,A){return this.getInstance().subscribe(y,z,A);
},checkSubscription:function(I,J,K){return this.getInstance().checkSubscription(I,J,K);
...
}},construct:function(){this.__xH={};
},members:{__xH:null,getSubscriptions:function(){return this.__xH;
},subscribe:function(M,N,O){if(!M||typeof N!=x){this.error("Invalid parameters! "+[M,N,O]);
return false;
}var P=this.getSubscriptions();

if(this.checkSubscription(M)){if(this.checkSubscription(M,N,O)){this.warn("Object method already subscribed to "+M);
return false;
}P[M].push({subscriber:N,context:O||null});
return true;
}else{P[M]=[{subscriber:N,context:O||null}];
return true;
}},checkSubscription:function(a,b,c){var d=this.getSubscriptions();

if(!d[a]||d[a].length===0){return false;
}
if(b){for(var i=0;i<d[a].length;i++){if(d[a][i].subscriber===b&&d[a][i].context===(c||null)){return true;
}}return false;
}return true;
},unsubscribe:function(e,f,g){var j=this.getSubscriptions();
var h=j[e];
...
if(k.subscriber===f&&k.context===g){h.splice(i,1);

if(h.length===0){j[e]=null;
delete j[e];
}return true;
}}while(i);
}return false;
},dispatch:function(B){if(typeof B==t){var G=typeof arguments[1]!=u?arguments[1]:true;
B=new qx.event.message.Message(B,G);
}var C=this.getSubscriptions();
var D=B.getName();
var E=false;

for(var H in C){var F=H.indexOf(v);

if(F>-1){if(F===0||H.substr(0,F)===D.substr(0,F)){this.__xI(C[H],B);
E=true;
}}else{if(H===D){this.__xI(C[D],B);
E=true;
}}}return E;
},__xI:function(o,p){for(var i=0;i<o.length;i++){var q=o[i].subscriber;
var r=o[i].context;
