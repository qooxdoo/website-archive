...
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:d,check:[d,b]},x:{init:0,check:c},y:{init:0,check:c}},members:{__rd:null,__re:null,start:function(){if(!arguments.callee.base.call(this)){return;
}var i=this._getElement();
this.__rd={x:i.scrollLeft,y:i.scrollTop};
if(this._atEndPosition(this.__rd.x,this.__rd.y)){return;
}
if(this.getMode()==b){this.__re={left:this.getX()-this.__rd.x,top:this.getY()-this.__rd.y};
}},update:function(e){arguments.callee.base.call(this);
var f=this._getElement();

if(this.getMode()==d){if(this.getX()!=0){f.scrollLeft=this.__rd.x+(this.getX()*e);
}
if(this.getY()!=0){f.scrollTop=this.__rd.y+(this.getY()*e);
}}else{f.scrollLeft=this.__rd.x+(this.__re.left*e);
f.scrollTop=this.__rd.y+(this.__re.top*e);
