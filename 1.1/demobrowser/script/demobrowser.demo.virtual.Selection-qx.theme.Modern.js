...
(function(){var y="manager",x="multi",w=" / ",v="bold",u="value",t="white",s="selected",r="#EEE",q="horizontal",p="table-scroller-header",T="one",S="single",R="changeValue",Q="table-row-background-selected",P="additive",O="default",N="Additive Selection",M="Cell Line Selection",L="addaptive",K="Options",F="Row Selection",G="changeSelection",D="Column Selection",E="Enable drag selection",B="demobrowser.demo.virtual.Selection",C="Selection Mode",z="Single Selection",A="Cell Rectangle Selection",H="Enable quick selection",I="One Selection",J="Multi Selection";
...
var bt=qx.theme.manager.Font.getInstance().resolve(O).getStyles();
this._fontCss=qx.bom.element.Style.compile(bt);
var bC=new qx.ui.layout.Grid(5,0);
bC.setRowFlex(2,1);
bC.setColumnFlex(0,1);
bC.setColumnFlex(1,1);
bC.setColumnFlex(2,1);
bC.setColumnFlex(3,1);
var bq=[];
var bA=new qx.ui.container.Composite(bC);
this.getRoot().add(bA,{edge:5});
bA.add(new qx.ui.basic.Label(F).set({font:v,decorator:p,padding:3,allowGrowX:true}),{row:1,column:0});
var bs=this.createRowSelectionScroller();
bq.push(bs.getUserData(y));
bA.add(bs,{row:2,column:0});
bA.add(new qx.ui.basic.Label(D).set({font:v,decorator:p,padding:3,allowGrowX:true}),{row:1,column:1});
var bs=this.createColumnSelectionScroller();
bq.push(bs.getUserData(y));
bA.add(bs,{row:2,column:1});
bA.add(new qx.ui.basic.Label(A).set({font:v,decorator:p,padding:3,allowGrowX:true}),{row:1,column:2});
var bs=this.createCellRectangleSelectionScroller();
bq.push(bs.getUserData(y));
bA.add(bs,{row:2,column:2});
bA.add(new qx.ui.basic.Label(M).set({font:v,decorator:p,padding:3,allowGrowX:true}),{row:1,column:3});
var bs=this.createCellLineSelectionScroller();
bq.push(bs.getUserData(y));
bA.add(bs,{row:2,column:3});
var by=new qx.ui.layout.Grid(20,4);
var bv=new qx.ui.container.Composite(by).set({padding:15,backgroundColor:t});
bA.add(bv,{row:0,column:0,colSpan:4});
bv.add(new qx.ui.basic.Label(C).set({font:v,padding:3}),{row:0,column:0});
var bu=new qx.ui.form.RadioButton(z);
bu.setUserData(u,S);
bv.add(bu,{row:1,column:0});
var bx=new qx.ui.form.RadioButton(J);
bx.setUserData(u,x);
bx.setValue(true);
bv.add(bx,{row:2,column:0});
var br=new qx.ui.form.RadioButton(N);
br.setUserData(u,P);
bv.add(br,{row:3,column:0});
var bp=new qx.ui.form.RadioButton(I);
bp.setUserData(u,T);
bv.add(bp,{row:4,column:0});
var bw=new qx.ui.form.RadioGroup(bu,bx,br,bp);
bw.addListener(G,function(e){var X=e.getData()[0].getUserData(u);
bq.forEach(function(bK){bK.setMode(X);
});

if(X==S||X==T){bB.setEnabled(false);
bz.setEnabled(true);
}else if(X==x||X==L){bB.setEnabled(true);
bz.setEnabled(false);
}});
bv.add(new qx.ui.basic.Label(K).set({font:v,padding:3}),{row:0,column:1});
var bB=new qx.ui.form.CheckBox(E);
bv.add(bB,{row:1,column:1});
bB.addListener(R,function(e){if(e.getData()){var b=bq[0].getMode();

if(b==S||b==T){this.debug("Drag selection is only available for the modes multi or additive");
}}bq.forEach(function(o){o.setDrag(e.getData());
});
});
var bz=new qx.ui.form.CheckBox(H).set({enabled:false});
bv.add(bz,{row:2,column:1});
bz.addListener(R,function(e){if(e.getData()){var a=bq[0].getMode();

if(a==x||a==P){this.debug("Quick selection is only available for the modes multi or additive");
}}bq.forEach(function(bD){bD.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bo=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var bm=new qx.ui.virtual.layer.Row(t,r);
bo.getPane().addLayer(bm);
bo.getPane().addLayer(new qx.ui.virtual.layer.GridLines(q));
var bn=new qx.ui.virtual.cell.Cell();
var bl=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(be,bf){var bg={};

if(bk.isItemSelected(be)){bg.selected=true;
}return bn.getCellProperties(be+w+bf,bg);
}});
bo.getPane().addLayer(bl);
var bk=new qx.ui.virtual.selection.Row(bo.getPane(),{styleSelectable:function(bH,bI,bJ){if(bI!==s){return;
}
if(bJ){bm.setDecorator(s);
}else{bm.setDecorator(null);
}bl.updateLayerData();
}});
bk.attachMouseEvents(bo.getPane());
bk.attachKeyEvents(bo);
bk.set({mode:x});
bk.addItem(0);
bo.setUserData(y,bk);
return bo;
},createColumnSelectionScroller:function(){var bR=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bR.getPane().addLayer(new qx.ui.virtual.layer.Row(t,r));
var bQ=new qx.ui.virtual.layer.Column();
bR.getPane().addLayer(bQ);
bR.getPane().addLayer(new qx.ui.virtual.layer.GridLines(q));
var bS=new qx.ui.virtual.cell.Cell();
var bP=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bh,bi){var bj={};

if(bO.isItemSelected(bi)){bj.selected=true;
}return bS.getCellProperties(bh+w+bi,bj);
}});
bR.getPane().addLayer(bP);
var bO=new qx.ui.virtual.selection.Column(bR.getPane(),{styleSelectable:function(l,m,n){if(m!==s){return;
}
if(n){bQ.setDecorator(s);
}else{bQ.setDecorator(null);
}bP.updateLayerData();
}});
...
bR.setUserData(y,bO);
return bR;
},createCellRectangleSelectionScroller:function(){var bc=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bc.getPane().addLayer(new qx.ui.virtual.layer.Row(t,r));
var bb=new qx.ui.virtual.cell.Cell();
var bd=new qx.ui.virtual.cell.Cell().set({backgroundColor:Q});
var ba=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(U,V){var W={};

if(Y.isItemSelected({row:U,column:V})){W.selected=true;
}
if(W.selected){return bd.getCellProperties(U+w+V,W);
}else{bb.resetBackgroundColor();
return bb.getCellProperties(U+w+V,W);
}}});
bc.getPane().addLayer(ba);
bc.getPane().addLayer(new qx.ui.virtual.layer.GridLines(q));
var Y=new qx.ui.virtual.selection.CellRectangle(bc.getPane(),{styleSelectable:function(bL,bM,bN){ba.updateLayerData();
}});
Y.attachMouseEvents(bc.getPane());
Y.attachKeyEvents(bc);
Y.set({mode:x});
Y.addItem({row:0,column:0});
bc.setUserData(y,Y);
return bc;
},createCellLineSelectionScroller:function(){var g=new qx.ui.virtual.core.Scroller(1000,100,20,100);
g.getPane().addLayer(new qx.ui.virtual.layer.Row(t,r));
var f=new qx.ui.virtual.cell.Cell();
var h=new qx.ui.virtual.cell.Cell().set({backgroundColor:Q});
var d=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bE,bF){var bG={};

if(c.isItemSelected({row:bE,column:bF})){bG.selected=true;
}
if(bG.selected){return h.getCellProperties(bE+w+bF,bG);
}else{f.resetBackgroundColor();
return f.getCellProperties(bE+w+bF,bG);
}}});
g.getPane().addLayer(d);
g.getPane().addLayer(new qx.ui.virtual.layer.GridLines(q));
var c=new qx.ui.virtual.selection.CellLines(g.getPane(),{styleSelectable:function(i,j,k){d.updateLayerData();
}});
c.attachMouseEvents(g.getPane());
c.attachKeyEvents(g);
c.set({mode:x});
c.addItem({row:0,column:0});
g.setUserData(y,c);
return g;
}}});
})();
...
(function(){var H="under",G="above",F="left",E="right",D="x",C="qx.ui.virtual.selection.CellRectangle";
qx.Class.define(C,{extend:qx.ui.virtual.selection.Abstract,members:{_getItemCount:function(){return this._pane.getRowConfig().getItemCount()*this._pane.getColumnConfig().getItemCount();
},_getSelectableFromMouseEvent:function(event){var I=this._pane.getCellAtPosition(event.getDocumentLeft(),event.getDocumentTop());

if(!I){return null;
}return this._isSelectable(I)?I:null;
},getSelectables:function(){var K=[];
var N=this._pane.getRowConfig().getItemCount();
var J=this._pane.getColumnConfig().getItemCount();

for(var O=0;O<N;O++){for(var L=0;L<J;L++){var M={row:O,column:L};

if(this._isSelectable(M)){K.push(M);
}}}return K;
},_getSelectableRange:function(l,m){var p=[];
var q=Math.min(l.row,m.row);
var r=Math.max(l.row,m.row);
var o=Math.min(l.column,m.column);
var t=Math.max(l.column,m.column);

for(var u=q;u<=r;u++){for(var s=o;s<=t;s++){var n={row:u,column:s};

if(this._isSelectable(n)){p.push(n);
}}}return p;
...
},_getRelatedSelectable:function(v,w){var x={row:v.row,column:v.column};

switch(w){case G:for(var B=v.row-1;B>=0;B--){x.row=B;

if(this._isSelectable(x)){return x;
}}break;
case H:var y=this._pane.getRowConfig().getItemCount();

for(var B=v.row+1;B<y;B++){x.row=B;

if(this._isSelectable(x)){return x;
}}break;
case F:for(var A=v.column-1;A>=0;A--){x.column=A;

if(this._isSelectable(x)){return x;
}}break;
case E:var z=this._pane.getColumnConfig().getItemCount();

for(var A=v.column+1;A<z;A++){x.column=A;
...
}},_selectableToHashCode:function(k){return k.column+D+k.row;
},_scrollItemIntoView:function(a){this._pane.scrollCellIntoView(a.column,a.row);
},_getSelectableLocationX:function(R){var S=this._pane.getColumnConfig();
var U=S.getItemPosition(R.column);
var T=U+S.getItemSize(R.column)-1;
return {left:U,right:T};
},_getSelectableLocationY:function(g){var j=this._pane.getRowConfig();
var i=j.getItemPosition(g.row);
var h=i+j.getItemSize(g.row)-1;
return {top:i,bottom:h};
}}});
})();
