...
qx.Class.define(e,{extend:qx.ui.virtual.layer.WidgetCell,construct:function(){qx.ui.virtual.layer.WidgetCell.call(this,this);
this._pool={atom:[],checkbox:[]};
this.__NQ=[];
},members:{_pool:null,__NQ:null,getCellData:function(k,l){if(!this.__NQ[k]){this.__NQ[k]=[];
}
if(!this.__NQ[k][l]){this.__NQ[k][l]={label:this.__NR(),icon:this.__NS()};
}return this.__NQ[k][l];
},getCellWidget:function(r,s){var t;

if(s%2==0){t=this._pool.atom.pop();

if(!t){t=new qx.ui.basic.Atom();
t.addListener(d,function(){var q=this.__NS();
t.setIcon(q);
this.__NQ[r][s].icon=q;
},this);
}t.set(this.getCellData(r,s));
}else{t=this._pool.checkbox.pop();

if(!t){t=new qx.ui.form.CheckBox();
t.addListener(c,function(){this.setLabel(this.getLabel()==g?t.getUserData(h)+i+t.getUserData(f):g);
},t);
}t.set({value:r%2==0,label:r+i+s});
}t.setUserData(h,r);
t.setUserData(f,s);
return t;
},poolCellWidget:function(a){if(a.classname==b){this._pool.atom.push(a);
}else{this._pool.checkbox.push(a);
}},__NR:function(){var name="";

for(var j=0;j<10;j++){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);
}return name;
},__NS:function(){var m="icon/";
var o="places/";
var p=["folder.png","user-trash.png","network-server.png","network-workgroup.png","user-desktop.png"];
var n=Math.floor(Math.random()*4);
return (m+16+"/"+o+p[n]);
}},destruct:function(){this._pool=this.__NQ=null;
}});
})();
