...
qx.Class.define(d,{extend:qx.application.Standalone,members:{__MO:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var R=p;
this.imageData=[{url:R+k,height:4,tooltip:u},{url:R+b,height:4,tooltip:n},o];
this.__MO=0;
...
++this.__MO;
qx.io.ImageLoader.load(Q,function(I,J){this.info("Searching for ["+I+"]");
for(var j=0;j<this.imageData.length;j++){if(this.imageData[j].url==I){this.imageData[j].height=J.height;
--this.__MO;
this.debug("Found ["+this.imageData[j].url+"].  loadCount="+this.__MO+", end="+this.imageData.length);
if(this.__MO==0){this.doLoad();
}return;
}}throw new Error("Image data for "+I+" not found");
},this);
}if(this.__MO==0){this.doLoad();
}},doLoad:function(){var x=this;
var E=function(K,L){var M={renderer:v,location:c,data:[L,x.imageData[K]]};
return M;
};
var D=new qx.ui.progressive.renderer.table.Widths(2);
D.setWidth(0,200);
D.setWidth(1,q);
var z=[e,h];
var A=new qx.ui.progressive.headfoot.TableHeading(D,z);
var y=new qx.ui.progressive.headfoot.Progress(D,z);
var H=new qx.ui.progressive.structure.Default(A,y);
var w=new qx.ui.progressive.Progressive(H);
var F=new qx.ui.basic.Atom(a);
F.setHeight(16);
F.setTextColor(l);
w.add(F);
var C=new qx.ui.progressive.model.Default();
var B=[];
B.push(E(0,g));
B.push(E(1,m));
B.push(E(0,f));
...
C.addElements(B);
w.setDataModel(C);
var G=new qx.ui.progressive.renderer.table.Row(D);
w.addRenderer(v,G);
var r=new qx.ui.progressive.renderer.table.cell.Image();
G.addRenderer(1,r);
w.set({width:400,maxWidth:400});
this.getRoot().add(w,{left:50,top:50,bottom:50});
w.render();
}}});
})();
...
(function(){var e="qx.ui.progressive.renderer.table.cell.Image",d="string",c="";
qx.Class.define(e,{extend:qx.ui.progressive.renderer.table.cell.Icon,construct:function(a,b){qx.ui.progressive.renderer.table.cell.Icon.call(this);

if(a===undefined){this.__GN=a;
}else{this.__GN=16;
}
if(b===undefined){this.__GO=b;
}else{this.__GO=16;
}},members:{__GN:null,__GO:null,_identifyImage:function(f){var j={imageWidth:this.__GN,imageHeight:this.__GO};
var k;
if(typeof (f.cellData)==d){j.url=f.cellData;
}else{j.url=f.cellData.url;
j.tooltip=f.cellData.tooltip;
k=f.cellData.height;
}
if(j.url==c){j.url=this._imageBlank;
