...
(function(){var w="Validation Error",v="%1 is not a number.",u="%1 is not a color! %2",t="",s="%1 is not a string.",r="number",q="Validation Error: ",p="'%1' is not an email address.",o="qx.util.Validate",n="%1 is not an url.",k="%1 is not in %2",m="%1 does not fit %2.",l="string",j="%1 is not in the range from [%2, %3].";
qx.Class.define(o,{statics:{number:function(){return qx.util.Validate.checkNumber;
},checkNumber:function(x){if((typeof x!==r&&(!(x instanceof Number)))||(!(isFinite(x)))){throw new qx.core.ValidationError(q,qx.locale.Manager.tr(v,x));
}},email:function(){return qx.util.Validate.checkEmail;
},checkEmail:function(a){var b=/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;

if(b.test(a)===false){throw new qx.core.ValidationError(w,qx.locale.Manager.tr(p,(a||t)));
}},string:function(){return qx.util.Validate.checkString;
},checkString:function(f){if(typeof f!==l&&(!(f instanceof String))){throw new qx.core.ValidationError(w,qx.locale.Manager.tr(s,f));
}},url:function(){return qx.util.Validate.checkUrl;
},checkUrl:function(c){var d=/([A-Za-z0-9])+:\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;

if(d.test(c)===false){throw new qx.core.ValidationError(w,qx.locale.Manager.tr(n,c));
}},color:function(){return qx.util.Validate.checkColor;
},checkColor:function(h){try{qx.util.ColorUtil.stringToRgb(h);
}catch(e){throw new qx.core.ValidationError(w,qx.locale.Manager.tr(u,h,e));
}},range:function(B,C){return function(y){if(y<B||y>C){throw new qx.core.ValidationError(w,qx.locale.Manager.tr(j,y,B,C));
}};
},inArray:function(z){return function(g){if(z.indexOf(g)===-1){throw new qx.core.ValidationError(w,qx.locale.Manager.tr(k,g,z));
}};
},regExp:function(A){return function(i){if(A.test(i)===false){throw new qx.core.ValidationError(w,qx.locale.Manager.tr(m,i,A));
}};
}}});
})();
(function(){var o="<br>",n="new name: ",m="new gender: ",l="male",k="female",j="new age: ",i="demobrowser.demo.test.Validation",h="<b>Setting the age ",g="property to 27</b><br>",f="- name: Name is validated to a minimum length of two.<br/>",R="<br>old name: ",Q="age:",P="fmale",O="<br><b>Setting the name to 'Hans'</b><br>",N="property to 'female'</b><br>",M="<br><b>Setting the gender ",L="<b>Testing the gender ",K="<br>old gender: ",J="<br><b>Setting the age ",I="- age: Age is validated to be a number.<br/>",v="<b>Initial property values</b><br>",w="",t="<b>Setting the name to ''</b><br>",u="demobrowser.demo.test.Person",r="property to 'true'</b><br>",s="- gender: Gender is validated to be either male or female.",p="property to 'fmale'</b><br>",q="<font color='red'>GENDER NOT SET</font>",x="gender:",y="<br>old age: ",B="Hans",A="The demo creates an internal class containing three properties:<br/>",D="<font color='red'>AGE NOT SET</font>",C="My name",F="<font color='red'>NAME NOT SET</font>",E="__MC",z="white",H="name:",G="<b>Description</b><br/>";
...
var b=new demobrowser.demo.test.Person();
var c=new qx.ui.embed.Html();
c.setBackgroundColor(z);
c.set({padding:10,width:250,height:360});
this.getRoot().add(c,{left:10,top:10});
c.setHtml(v);
c.setHtml(c.getHtml()+H+b.getName()+o);
c.setHtml(c.getHtml()+Q+b.getAge()+o);
c.setHtml(c.getHtml()+x+b.getGender()+o);
c.setHtml(c.getHtml()+O);
b.setName(B);
c.setHtml(c.getHtml()+n+b.getName()+o);
c.setHtml(c.getHtml()+t);
try{b.setName(w);
c.setHtml(c.getHtml()+n+b.getName()+o);
}catch(a){c.setHtml(c.getHtml()+F+R+b.getName()+o);
}c.setHtml(c.getHtml()+J+g);
b.setAge(27);
c.setHtml(c.getHtml()+j+b.getAge()+o);
c.setHtml(c.getHtml()+h+r);
try{b.setAge(true);
c.setHtml(c.getHtml()+j+b.getAge()+o);
}catch(T){c.setHtml(c.getHtml()+D+y+b.getAge()+o);
}c.setHtml(c.getHtml()+M+N);
b.setGender(k);
c.setHtml(c.getHtml()+m+b.getGender()+o);
c.setHtml(c.getHtml()+L+p);
try{b.setAge(P);
c.setHtml(c.getHtml()+m+b.getGender()+o);
}catch(S){c.setHtml(c.getHtml()+q+K+b.getGender()+o);
}var d=new qx.ui.basic.Label();
d.setRich(true);
d.setWidth(250);
d.setValue(G+A+f+I+s);
this.getRoot().add(d,{left:280,top:10});
}}});
qx.Class.define(u,{extend:qx.core.Object,properties:{name:{init:C,validate:E},age:{init:18,validate:qx.util.Validate.number()},gender:{init:l,validate:qx.util.Validate.inArray([l,k])}},members:{__MC:function(e){if(e.length<2){throw new qx.core.ValidationError("Validation Error: Name "+"must be longer than one characters. ("+e+")");
}}}});
})();
