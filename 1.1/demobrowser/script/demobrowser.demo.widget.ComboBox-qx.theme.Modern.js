...
(function(){var G="bold",F=" = ",E="... &gt; (as literal HTML entity)",D="... &gt; (as Richtext)",C="HTML entities: &laquo; &lt; &amp; &gt; &raquo;",B="Wide, long list",A="<u>Underlined Text</u>",z="<b>Bold Text</b>",y="'s Icon",x="With icons",r="Default",w="changeValue",u="2^ ",q="<i>Italic Text</i>",p="'s Item",t="icon/16/places/folder.png",s="With HTML (rich) text",v="demobrowser.demo.widget.ComboBox";
...
},_createDefaultExample:function(){var l=new qx.ui.basic.Label(r);
l.setFont(G);
this.getRoot().add(l,{left:20,top:25});
var j=new qx.ui.form.ComboBox();
for(var i=1;i<31;i++){var k=new qx.ui.form.ListItem(u+i+F+Math.pow(2,i));
j.add(k);
}j.addListener(w,function(e){this.debug("ChangeValue: "+e.getData());
});
this.getRoot().add(j,{left:20,top:40});
},_createIconExample:function(){var o=new qx.ui.basic.Label(x);
o.setFont(G);
this.getRoot().add(o,{left:160,top:25});
var m=new qx.ui.form.ComboBox();
for(var i=1;i<31;i++){var n=new qx.ui.form.ListItem(i+y,t);
m.add(n);
}this.getRoot().add(m,{left:160,top:40});
},_createHtmlExample:function(){var h=new qx.ui.basic.Label(s);
h.setFont(G);
this.getRoot().add(h,{left:300,top:25});
var d=new qx.ui.form.ComboBox().set({width:200});
var f=[E,D,z,A,q,C];
for(var i=0;i<f.length;i++){var g=new qx.ui.form.ListItem(f[i]);

if(i>0){g.setRich(true);
}d.add(g);
}this.getRoot().add(d,{left:300,top:40});
},_createWideExample:function(){var c=new qx.ui.basic.Label(B);
c.setFont(G);
this.getRoot().add(c,{left:20,top:285});
var a=new qx.ui.form.ComboBox();
a.setWidth(300);
for(var i=1;i<101;i++){var b=new qx.ui.form.ListItem(i+p);
a.add(b);
}a.setValue(a.getChildrenContainer().getSelectables()[0].getLabel());
this.getRoot().add(a,{left:20,top:300});
}}});
})();
