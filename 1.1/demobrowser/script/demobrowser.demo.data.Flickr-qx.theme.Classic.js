...
(function(){var o="qooxdoo",n="/",m=".static.flickr.com/",l="http://api.flickr.com/services/rest/?tags=",k="",j="_",i="http://farm",h="Enter",g="keydown",f="model.photos.photo",K=".jpg",J="loaded",I="execute",H="source",G="selection[0]",F="_s.jpg",E="<span style='font-size: 20px'>Flickr</span>",D="failed",C="loading",B="value",v="demobrowser.demo.data.store.Flickr",w="demobrowser.demo.data.Flickr",t="title",u="icon",r="state",s="completed",p="Search",q="receiving",x="horizontal",y="model",A="&method=flickr.photos.search&api_key=63a8042eead205f7e0040f488c02afd9&format=json",z="id";
...
var bc=new demobrowser.demo.data.store.Flickr(o);
var T=new qx.ui.form.TextField(o);
this.getRoot().add(T,{left:30,top:50});
var U=new qx.ui.form.Button(p);
this.getRoot().add(U,{left:140,top:49});
U.addListener(I,function(){bc.searchForTag(T.getValue());
},this);
T.addListener(g,function(e){if(e.getKeyIdentifier()==h){bc.searchForTag(T.getValue());
}},this);
...
X.setHeight(110);
X.setOrientation(x);
this.getRoot().add(X,{left:30,right:30,top:80});
var ba=new qx.data.controller.List(null,X);
ba.setLabelPath(t);
ba.setDelegate({configureItem:function(N){N.setShow(u);
}});
var V={converter:function(L,M){return (i+M.getFarm()+m+M.getServer()+n+L+j+M.getSecret()+F);
}};
ba.setIconOptions(V);
ba.setIconPath(z);
bc.bind(f,ba,y);
var W=new qx.ui.basic.Image();
this.getRoot().add(W,{left:30,top:200});
var bb={converter:function(O){if(O){return (i+O.getFarm()+m+O.getServer()+n+O.getId()+j+O.getSecret()+K);
}return k;
}};
ba.bind(G,W,H,bb);
...
qx.Class.define(v,{extend:qx.data.store.Json,statics:{RESULT:null},construct:function(b){jsonFlickrApi=function(a){demobrowser.demo.data.store.Flickr.RESULT=a;
};
var c=l+b;
qx.data.store.Json.call(this,c);
},members:{searchForTag:function(d){if(d!=k){this.setUrl(l+d);
}},_createRequest:function(P){var Q=new qx.io.ScriptLoader();
this.setState(q);
P+=A;
Q.load(P,function(R){this.__Ml();
},this);
},__Ml:function(){this.setState(s);
var S=demobrowser.demo.data.store.Flickr.RESULT;

if(S==undefined){this.setState(D);
return;
