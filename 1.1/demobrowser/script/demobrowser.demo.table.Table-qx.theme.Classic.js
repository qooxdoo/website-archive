...
(function(){var u="execute",t="changeValue",s="A date",r="Boolean",q="icon/22/actions/edit-undo.png",p="Change ID sort method",o="icon/22/status/dialog-information.png",n="Change row with ID 10",m="A number",l="icon/16/apps/office-calendar.png",K="Remove 5 rows",J="Add 10 rows",I="",H="demobrowser.demo.table.Table",G="_tableModel",F="Table",E="icon/22/actions/list-remove.png",D="Whether the the first visible row should ",C=", ",B="icon/22/actions/list-add.png",z="Selected rows:<br>",A="Keep first row",x="be rendered completely when scrolling.",y="Show selection",v="Demonstrate use of alternate sorting algorithm.",w="ID";
qx.Class.define(H,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return F;
},createTable:function(){var P=this.createRandomRows(50);
var O=this._tableModel=new qx.ui.table.model.Simple();
O.setColumns([w,m,s,r]);
O.setData(P);
O.setColumnEditable(1,true);
O.setColumnEditable(2,true);
O.setColumnSortable(3,false);
...
N.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(l,s));
return Q;
},createControls:function(){var be=new qx.ui.toolbar.ToolBar();
var bc,bd,bf;
bd=new qx.ui.toolbar.Part();
be.add(bd);
bc=new qx.ui.toolbar.Button(n,q);
bc.addListener(u,function(a){var b=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,b[0][i]);
}this.info("Row 10 changed");
},this);
bd.add(bc);
bc=new qx.ui.toolbar.Button(J,B);
bc.addListener(u,function(T){var U=this.createRandomRows(10);
this._tableModel.addRows(U);
this.info("10 rows added");
},this);
bd.add(bc);
bc=new qx.ui.toolbar.Button(K,E);
bc.addListener(u,function(Y){var ba=this._tableModel.getRowCount();
this._tableModel.removeRows(ba-5,5);
this.info("5 rows removed");
},this);
bd.add(bc);
bc=new qx.ui.toolbar.Button(y,o);
bc.addListener(u,function(L){var M=[];
bb.getSelectionModel().iterateSelection(function(S){M.push(S+I);
});
this.showDialog(z+M.join(C));
},this);
bd.add(bc);
bd=new qx.ui.toolbar.Part();
be.add(bd);
var bb=this._table;
bf=new qx.ui.toolbar.CheckBox(A);
bf.set({value:bb.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(D+x)});
bf.addListener(t,function(R){bb.setKeepFirstVisibleRowComplete(this.getValue());
},bf);
bd.add(bf);
bf=new qx.ui.toolbar.CheckBox(p);
bf.set({value:bb.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(v)});
bf.addListener(t,function(V){if(V.getData()){var X=function(g,h){var j=g[arguments.callee.columnIndex];
...
var W=function(c,d){var e=c[arguments.callee.columnIndex];
var f=d[arguments.callee.columnIndex];

if(e%2==1&&f%2==0){return -1;
}
if(f%2==1&&e%2==0){return 1;
}return (e<f)?1:((e==f)?0:-1);
};
bb.getTableModel().setSortMethods(0,{ascending:X,descending:W});
}else{bb.getTableModel().setSortMethods(0,null);
}},bf);
bd.add(bf);
return be;
