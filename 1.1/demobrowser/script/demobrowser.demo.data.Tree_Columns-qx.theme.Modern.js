...
(function(){var s="value",r="size",q="light",p="mode",o="date",n="checkbox",m="Trash",l="",k="checked",j="Windows (C:)",S="Documents (D:)",R="demobrowser.demo.data.Tree_Columns",Q="Inbox",P="Desktop",O="icon/16/places/user-desktop.png",N="Personal",M="source",L="-rw-r--r--",K="Presets",J="Network",z="Sent",A="children",x="Big",y="Files",v="icon/16/status/dialog-information.png",w=" 2010",t="Workspace",u="Spam",B="Data",C="May ",E="Root",D="label",G="Lists",F="kb",I="Item ",H="Edit";
qx.Class.define(R,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);
var V=new qx.ui.container.Scroll();
var U=new qx.ui.container.Composite(new qx.ui.layout.Basic());
U.setAllowGrowX(false);
U.setAllowStretchX(false);
V.add(U);
this.getRoot().add(V,{edge:0});
var W=new qx.ui.tree.Tree().set({width:600,height:500});
U.add(W,{left:20,top:48});
var Y={label:E,children:[{label:P,children:[{label:y},{label:t,children:[{label:j},{label:S}]},{label:J},{label:m}]},{label:Q,children:[{label:K},{label:z},{label:m},{label:B},{label:H},{label:G},{label:N},{label:x,children:[]},{label:u}]}]};

for(var i=0;i<50;i++){Y.children[1].children[7].children[i]={label:I+i};
}this.extendData(Y);
var X=qx.data.marshal.Json.createModel(Y);
var ba=new qx.data.controller.Tree(null,W,A,D);
ba.setDelegate(this);
ba.setModel(X);
},extendData:function(T){T.date=C+Math.round(Math.random()*30+1)+w;
T.size=Math.round(Math.random()*100)+F;
T.light=Math.floor(Math.random()*4)==0;
T.mode=L;
T.checked=Math.random()>=0.5;

if(T.children){for(var i=0;i<T.children.length;i++){this.extendData(T.children[i]);
}}},bindItem:function(a,b,c){a.bindDefaultProperties(b,c);
a.bindProperty(r,s,null,b.getUserData(r),c);
a.bindProperty(k,s,null,b.getUserData(n),c);
a.bindPropertyReverse(k,s,null,b.getUserData(n),c);
a.bindProperty(o,s,null,b.getUserData(o),c);
a.bindProperty(p,s,null,b.getUserData(p),c);
a.bindProperty(q,M,{converter:function(h){return h?v:l;
}},b.getUserData(q),c);
},createItem:function(){var d=new qx.ui.tree.TreeFolder();
var f=new qx.ui.basic.Image();
f.setWidth(16);
d.addWidget(f);
d.setUserData(q,f);
d.addSpacer();
...
d.addIcon();
d.setIcon(O);
var e=new qx.ui.form.CheckBox();
e.setFocusable(false);
d.addWidget(e);
d.setUserData(n,e);
d.addLabel(l);
d.addWidget(new qx.ui.core.Spacer(),{flex:1});
var g=new qx.ui.basic.Label();
g.setWidth(50);
d.addWidget(g);
d.setUserData(r,g);
g=new qx.ui.basic.Label();
g.setWidth(150);
d.addWidget(g);
d.setUserData(o,g);
g=new qx.ui.basic.Label();
