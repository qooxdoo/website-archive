...
R.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(c,j));
return U;
},createControls:function(){var I=new qx.ui.toolbar.ToolBar();
var K,M;
M=new qx.ui.toolbar.Part();
I.add(M);
var H=new qx.ui.form.SelectBox().set({allowGrowY:false,alignY:z});
var P=qx.ui.table.selection.Model;
var J={"Single selection":P.SINGLE_SELECTION,"Single interval":P.SINGLE_INTERVAL_SELECTION,"Multiple intervals":P.MULTIPLE_INTERVAL_SELECTION,"Multiple intervals (toggle)":P.MULTIPLE_INTERVAL_SELECTION_TOGGLE,"No selection":P.NO_SELECTION};

for(var O in J){var L=new qx.ui.form.ListItem(O);
L.setUserData(l,J[O]+k);
H.add(L);
}H.addListener(A,function(e){this._table.getSelectionModel().setSelectionMode(parseInt(e.getData()[0].getUserData(l)));
},this);
M.add(H);
M=new qx.ui.toolbar.Part();
I.add(M);
K=new qx.ui.toolbar.Button(u,q);
K.addListener(m,function(D){var E=this.createRandomRows(10);
this._tableModel.addRows(E);
this.info("5 rows added");
},this);
M.add(K);
K=new qx.ui.toolbar.Button(y,t);
K.addListener(m,function(F){var G=this._tableModel.getRowCount();
...
M=new qx.ui.toolbar.Part();
I.add(M);
K=new qx.ui.toolbar.Button(x);
K.addListener(m,function(W){this._table.resetSelection();
},this);
M.add(K);
K=new qx.ui.toolbar.Button(B);
K.addListener(m,function(Q){this._table.resetCellFocus();
},this);
M.add(K);
var N=new qx.ui.toolbar.CheckBox(r);
N.addListener(g,function(C){this._table.setShowCellFocusIndicator(!N.isValue());
},this);
M.add(N);
K=new qx.ui.toolbar.Button(p,f);
K.addListener(m,function(a){var b=[];
this._table.getSelectionModel().iterateSelection(function(V){b.push(V+k);
});
this.showDialog(o+b.join(s));
},this);
M.add(K);
return I;
