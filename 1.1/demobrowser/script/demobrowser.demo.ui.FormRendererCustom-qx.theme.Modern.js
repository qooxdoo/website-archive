...
(function(){var x="execute",w="Name",v="demobrowser.demo.ui.FormRendererCustom",u="Registration",t="demobrowser.demo.ui.CustomRenderer",s="Bio",r="Send",q="Contact",p="Country",o="Personal Information",l="Password",n="Email",m="Phone",k="right",j="Reset";
...
z.add(D,l);
z.addGroupHeader(q);
z.add(new qx.ui.form.TextField(),n,qx.util.Validate.email());
z.add(new qx.ui.form.TextField(),m);
z.addGroupHeader(o);
z.add(new qx.ui.form.TextField(),p);
z.add(new qx.ui.form.TextArea(),s);
var B=new qx.ui.form.Button(r);
B.addListener(x,function(){if(z.validate()){alert("send...");
}},this);
z.addButton(B);
var A=new qx.ui.form.Button(j);
A.addListener(x,function(){z.reset();
},this);
z.addButton(A);
var C=new demobrowser.demo.ui.CustomRenderer(z);
this.getRoot().add(C,{left:10,top:10});
}}});
qx.Class.define(t,{extend:qx.ui.form.renderer.Single,members:{__NY:0,__Oa:0,addItems:function(a,b,c){var f=0;
if(c!=null){this._add(this._createHeader(c),{row:f,column:this.__NY,colSpan:2});
f++;
}for(var i=0;i<a.length;i++){var e=this._createLabel(b[i],a[i]);
this._add(e,{row:f,column:this.__NY});
var d=a[i];
e.setBuddy(d);

if(d instanceof qx.ui.form.RadioGroup){d=this._createWidgetForRadioGroup(d);
}this._add(d,{row:f,column:this.__NY+1});
f++;
}this.__NY+=2;
this.__Oa<f?this.__Oa=f:null;
},addButton:function(g){if(this._buttonRow==null){this._buttonRow=new qx.ui.container.Composite();
this._buttonRow.setMarginTop(5);
var h=new qx.ui.layout.HBox();
h.setAlignX(k);
h.setSpacing(5);
this._buttonRow.setLayout(h);
this._add(this._buttonRow,{row:this.__Oa,column:0,colSpan:this.__NY});
}this._buttonRow.add(g);
}}});
})();
