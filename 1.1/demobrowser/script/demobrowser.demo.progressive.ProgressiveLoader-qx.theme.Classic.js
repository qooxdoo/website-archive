...
(function(){var s="white",r="100%",q="func",p="demobrowser.demo.progressive.ProgressiveLoader",n="black",m="Red",l="http://resources.qooxdoo.org/images/logo.gif",k="green",j="yellow",i="col ",E="gray",D="blue",C="Green",B="Yellow",A="Blue",z="renderStart",y=", ",x="purple",w="red",v="renderEnd",t="row ",u="Purple";
...
var bb=new qx.ui.progressive.renderer.table.Widths(1);
bb.setWidth(0,r);
var U=new qx.ui.progressive.headfoot.Progress(bb);
var Y=new qx.ui.progressive.structure.Default(null,U);
var T=new qx.ui.progressive.Progressive(Y);
T.setFlushWidgetQueueAfterBatch(true);
var X=function(f){var g={renderer:q,data:f};
return g;
};
var bf=new qx.ui.progressive.model.Default();
var bc=new qx.ui.progressive.renderer.FunctionCaller();
T.addRenderer(q,bc);
var ba=l;
var bd=new qx.ui.basic.Image(ba,r,r);
T.add(bd);
T.set({height:100,width:272,zIndex:99999,backgroundColor:E,opacity:0.86,batchSize:10});
this.getRoot().add(T,{top:-1000,left:-1000});
var be=[{text:m,background:w,color:s},{text:C,background:k,color:s},{text:A,background:D,color:s},{text:u,background:x,color:s},{text:B,background:j,color:n}];
this.context={document:this.getRoot(),pages:be};
T.addListener(z,function(e){var J=e.getData().state;
var F=e.getData().initial;
var I=this.getRoot().getBounds();
var G=e.getData().state.getProgressive();
var H=G.getBounds();
var K=Math.floor((I.width-H.width)/2);
var top=Math.floor((I.height-H.height)/2);
G.setLayoutProperties({left:K<0?0:K,top:top<0?0:top});
J.getUserData().context=this.context;
J.getUserData().initialNum=F;
},this);
T.addListener(v,function(e){qx.event.Timer.once(function(){this.getLayoutParent().remove(this);
this.dispose();
},this,0);
});
bf.addElement(X(function(c){var d=c.context;
d.tabview=new qx.ui.tabview.TabView();
d.tabview.set({});
d.document.add(d.tabview,{left:20,right:20,top:20,bottom:20});
d.page=0;
d.currentPage=null;
}));

for(var V=0;V<be.length;V++){bf.addElement(X(function(Q){var S=Q.context;
var R=S.pages[S.page];
S.currentPage=new qx.ui.tabview.Page(R.text);
S.currentPage.setLayout(new qx.ui.layout.VBox(4));
S.tabview.add(S.currentPage);
S.row=0;
S.col=0;
if(S.page==0){S.firstPage=S.currentPage;
}}));
for(var bg=0;bg<20;bg++){bf.addElement(X(function(a){var b=a.context;
b.hBox=new qx.ui.container.Composite(new qx.ui.layout.HBox(4));
b.currentPage.add(b.hBox);
b.tabview.setSelection([b.currentPage]);
b.col=0;
}));

for(var W=0;W<10;W++){bf.addElement(X(function(N){var P=N.context;
var O=P.pages[P.page];
var o=new qx.ui.basic.Atom(t+P.row+y+i+P.col);
o.set({width:90,backgroundColor:O.background,textColor:O.color});
P.hBox.add(o);
}));
bf.addElement(X(function(M){++M.context.col;
}));
}bf.addElement(X(function(L){++L.context.row;
}));
}bf.addElement(X(function(bh){++bh.context.page;
}));
}bf.addElement(X(function(h){h.context.tabview.setSelection([h.context.firstPage]);
}));
T.setDataModel(bf);
T.render();
}}});
