...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.layout.HBox_Separator-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"3f1d7059743d"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
qx.Class.define(h,{extend:demobrowser.demo.util.LayoutApplication,members:{main:function(){demobrowser.demo.util.LayoutApplication.prototype.main.call(this);
...
qx.Class.define(X,{extend:qx.ui.layout.Abstract,construct:function(bX,bY,ca,cb){qx.ui.layout.Abstract.call(this);
...
}},properties:{sort:{check:[Y,ba,bb],init:Y,apply:W},separatorX:{check:bd,nullable:true,apply:bg},separatorY:{check:bd,nullable:true,apply:bg},connectSeparators:{check:S,init:false,apply:bg},spacingX:{check:bc,init:0,apply:bg},spacingY:{check:bc,init:0,apply:bg}},members:{__Aj:null,__Ak:null,verifyLayoutProperty:null,_applySort:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__Al:{north:1,south:2,west:3,east:4,center:5},__Am:{1:be,2:R,3:bf,4:U},__An:function(){var bh=this._getLayoutChildren();
var bp,bj;
var length=bh.length;
var bl=[];
var bo=[];
var bm=[];
var bi=this.getSort()===ba;
var bn=this.getSort()===bb;

for(var i=0;i<length;i++){bp=bh[i];
bm=bp.getLayoutProperties().edge;

if(bm===P){if(bj){throw new Error("It is not allowed to have more than one child aligned to 'center'!");
}bj=bp;
}else if(bn||bi){if(bm===V||bm===T){bi?bl.push(bp):bo.push(bp);
}else if(bm===Q||bm===O){bi?bo.push(bp):bl.push(bp);
}}else{bl.push(bp);
}}var bq=bl.concat(bo);
...
for(var i=0;i<length;i++){bm=bq[i].getLayoutProperties().edge;
bk[i]=this.__Al[bm]||5;
}this.__Ak=bk;
delete this._invalidChildrenCache;
},renderLayout:function(a,b){if(this._invalidChildrenCache){this.__An();
...
},_computeSizeHint:function(){if(this._invalidChildrenCache){this.__An();
}var bu=this.__Aj;
var bE=this.__Ak;
...
var bK=this.getSpacingX(),bJ=this.getSpacingY();
var bw=-bK,bv=-bJ;

if(bI.x){bw-=bI.x+bK;
}
if(bI.y){bv-=bI.y+bJ;
}for(var i=0;i<length;i++){bF=bu[i];
by=bF.getSizeHint();
bA=bF.getMarginLeft()+bF.getMarginRight();
bz=bF.getMarginTop()+bF.getMarginBottom();
switch(bE[i]){case 1:case 2:bC=Math.max(bC,by.width+bB+bA);
bL=Math.max(bL,by.minWidth+bM+bA);
bs+=by.height+bz;
bG+=by.minHeight+bz;
bv+=bJ;
...
case 3:case 4:br=Math.max(br,by.height+bs+bz);
bH=Math.max(bH,by.minHeight+bG+bz);
bB+=by.width+bA;
bM+=by.minWidth+bA;
bw+=bK;
...
if(bI.y){bv+=bI.y+bJ;
}}}var bx=Math.max(bM,bL)+bw;
var bN=Math.max(bB,bC)+bw;
var bD=Math.max(bH,bG)+bv;
var bt=Math.max(br,bs)+bv;
return {minWidth:bx,width:bN,minHeight:bD,height:bt};
}},destruct:function(){this.__Ak=this.__Aj=null;
}});
})();
...
(function(){var t="bold",s="wlpe_qx.ui.layout.Grid",r="black",q="demobrowser.demo.util.PropertyEditor",p="Widget Properties",o="lpe_qx.ui.layout.HBox",n="wlpe_qx.ui.layout.Dock",m="wlpe_",l="lpe_qx.ui.layout.Dock",k="lpe_",H="solid",G="string",F="wlpe_qx.ui.layout.HBox",E="wlpe_qx.ui.layout.Canvas",D="wlpe_qx.ui.layout.VBox",C=" layout properties",B="_applyWidget",A="qx.ui.core.Widget",z=" properties",y="wlpe_qx.ui.layout.Basic",w="Selected Widget",x="lpe_qx.ui.layout.Grid",u="pane",v="white";
qx.Class.define(q,{extend:qx.ui.core.scroll.AbstractScrollArea,construct:function(){qx.ui.core.scroll.AbstractScrollArea.call(this);
this.setWidth(null);
this._editorGroups={};
var J=new qx.ui.decoration.Single();
J.setLeft(1,H,r);
this.setDecorator(J);
this.setBackgroundColor(v);
var L=new qx.ui.container.Composite().set({padding:[20,14]});
this.getChildControl(u).add(L);
this._pane=L;
var K=new qx.ui.layout.VBox().set({spacing:5});
L.setLayout(K);
L.add(this._createWidgetIndicator());
L.add(new qx.ui.basic.Label(p).set({font:t,padding:[1,0,5,0]}));
var M=qx.lang.Object.clone(demobrowser.demo.util.PropertyGroup.WIDGET_PROPERTIES);
M.value={type:G,nullable:true};
this._layoutControls=new demobrowser.demo.util.PropertyGroup(M);
L.add(this._layoutControls);
},properties:{widget:{check:A,apply:B}},members:{handleWidgetClick:function(e){var I=e.getTarget();
this.setWidget(I);
},_createWidgetIndicator:function(){var a=new qx.ui.container.Composite(new qx.ui.layout.VBox().set({spacing:5}));
a.add(this._createLabel(w));
this._widgetIndicator=new qx.ui.basic.Label().set({height:30,allowGrowX:true,allowGrowY:true,padding:5});
a._add(this._widgetIndicator);
return a;
},_createLabel:function(d){return new qx.ui.basic.Label(d).set({font:t,allowGrowX:true,padding:[5,0,5,0]});
...
S.add(R);
return S;
},updateWidgetLayoutPropertyEditor:function(f){var h=f.getLayoutParent()?f.getLayoutParent()._getLayout():null;
var g=this.getWidgetLayoutPropertyEditor(h);

if(g){g.getChildren()[1].setSelected(f);
}
if(g===this._wlpe){return;
}
if(this._wlpe){this._pane.remove(this._wlpe);
}
if(!g){this._wlpe=g;
return;
}this._pane.addAfter(g,this._layoutControls);
this._wlpe=g;
},updateLayoutPropertyEditor:function(N){var P=N.getLayout?N.getLayout():null;
var O=this.getLayoutPropertyEditor(P);

if(O){O.getChildren()[1].setSelected(P);
}
...
this._lpe=O;
},getWidgetLayoutPropertyEditor:function(b){if(!b){return null;
}var name=m+b.constructor.classname;
var c;

if(this._editorGroups[name]){return this._editorGroups[name];
}
switch(name){case F:case D:c=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.BOX_PROPERTIES);
break;
case E:c=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.CANVAS_PROPERTIES);
break;
case y:c=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.BASIC_PROPERTIES);
break;
case n:c=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.DOCK_PROPERTIES);
break;
case s:c=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.GRID_PROPERTIES);
break;
}
if(!c){return null;
}this._editorGroups[name]=this._createContainer(b.constructor.classname+C,c);
return this._editorGroups[name];
},getLayoutPropertyEditor:function(i){if(!i){return null;
}var name=k+i.constructor.classname;
var j;

if(this._editorGroups[name]){return this._editorGroups[name];
}
switch(name){case o:j=new demobrowser.demo.util.PropertyGroup(demobrowser.demo.util.PropertyGroup.HBOX_PROPERTIES);
break;
case l:j=new demobrowser.demo.util.PropertyGroup(demobrowser.demo.util.PropertyGroup.DOCK_PROPERTIES);
break;
case x:j=new demobrowser.demo.util.PropertyGroup(demobrowser.demo.util.PropertyGroup.GRID_PROPERTIES);
break;
}
if(!j){return null;
}this._editorGroups[name]=this._createContainer(i.constructor.classname+z,j);
return this._editorGroups[name];
...
(function(){var C="value",B="changeValue",A="enum",z="set",y="int",x="string",w="bool",v="function",u="null",t="demobrowser.demo.util.PropertyGroup",q="_applySelected",s="get",r="top",p="changeSelection",o="right";
qx.Class.define(t,{extend:qx.ui.core.Widget,construct:function(E){qx.ui.core.Widget.call(this);
this._properties=E;
var G=new qx.ui.layout.Grid(10,5);
G.setColumnAlign(0,o,r);
G.setColumnMinWidth(0,110);
this._setLayout(G);
this.set({allowShrinkX:false,allowShrinkY:false});
var N=0;

for(var J in this._properties){var M=this._properties[J].type;
var I=this._properties[J].nullable;
this._add(new qx.ui.basic.Label(J).set({paddingTop:4}),{row:N,column:0});

if(I){var H=new qx.ui.form.CheckBox(u);
H.addListener(B,this._createOnNullPropertyChange(J),this);
this._add(H,{row:N,column:2});
this._properties[J].nullWidget=H;
}
if(M==y){var L=new qx.ui.form.Spinner().set({minimum:this._properties[J].min||0,maximum:this._properties[J].max!==undefined?this._properties[J].max:1000});
L.addListener(B,this._createOnIntPropertyChange(J),this);
this._add(L,{row:N++,column:1});
}else if(M==w){var L=new qx.ui.form.CheckBox();
L.addListener(B,this._createOnBoolPropertyChange(J),this);
this._add(L,{row:N++,column:1});
}else if(M==A){var K=this._properties[J].values;
var L=new qx.ui.form.RadioGroup();

for(var i=0;i<K.length;i++){var F=new qx.ui.form.RadioButton(K[i]);
F.setUserData(C,K[i]);
L.add(F);
this._add(F,{row:N++,column:1});
}L.addListener(p,this._createOnEnumPropertyChange(J,L),this);
}else if(M==x){var L=new qx.ui.form.TextField();
L.setLiveUpdate(true);
L.addListener(B,this._createOnIntPropertyChange(J),this);
this._add(L,{row:N++,column:1});
}this._properties[J].formItem=L;
...
}var Y=z+qx.lang.String.firstUp(name);
var X=this._properties[name].convert;

if(X){W=X(W);
}V[Y](W);
},_getProperty:function(O,name){var P=s+qx.lang.String.firstUp(name);
return O[P]();
},_hasProperty:function(bc,name){var bd=z+qx.lang.String.firstUp(name);
return (typeof (bc[bd])==v);
},_createOnIntPropertyChange:function(ba){return function(e){var bb=this.getSelected();
this._setProperty(bb,ba,e.getTarget().getValue());
};
},_createOnBoolPropertyChange:function(Q){return function(e){var R=this.getSelected();
this._setProperty(R,Q,e.getTarget().getValue());
};
},_createOnEnumPropertyChange:function(a,b){return function(e){var n=this.getSelected();
this._setProperty(n,a,b.getSelection()[0].getUserData(C));
};
},_createOnNullPropertyChange:function(D){return function(e){var T=this.getSelected();
var S=e.getTarget();

if(S.getValue()){this._setProperty(T,D,null);
this._properties[D].formItem.setEnabled(false);
}else{var U=null;

if(this._properties[D].type==A){U=this._properties[D].formItem.getSelection()[0].getUserData(C);
}else{U=this._properties[D].formItem.getValue();
}this._setProperty(T,D,U);
this._properties[D].formItem.setEnabled(true);
}};
},_updateControls:function(c){for(var k in this._properties){var m=this._properties[k].type;
var g=this._properties[k].formItem;
var d=this._properties[k].nullable;

if(!this._hasProperty(c,k)){g.setEnabled(false);

if(d){this._properties[k].nullWidget.setEnabled(false);
}return;
}g.setEnabled(true);
var j=this._getProperty(c,k);

if(j!==null){if(m==y){g.setValue(parseInt(j));
}else if(m==x){g.setValue(j.toString());
}else if(m==w){g.setValue(!!j);
}else if(m==A){var f=g.getItems();

for(var i=0,l=f.length;i<l;i++){var h=f[i];

if(h.getUserData(C)==j){g.setSelection([h]);
break;
}}}}
if(d){this._properties[k].nullWidget.setValue(j==null);
this._properties[k].nullWidget.setEnabled(true);
g.setEnabled(j!==null);
...
qx.Class.define(g,{extend:demobrowser.demo.util.PropertyGroup,statics:{BOX_PROPERTIES:{"flex":{type:"int",min:-1000,nullable:true}},BASIC_PROPERTIES:{"top":{type:"int",min:-1000,nullable:true},"left":{type:"int",min:-1000,nullable:true}},CANVAS_PROPERTIES:{"top":{type:"string",nullable:true,convert:function(i){if(parseInt(i).toString()==i){i=parseInt(i);
}return i||null;
}},"right":{type:"string",nullable:true,convert:function(f){if(parseInt(f).toString()==f){f=parseInt(f);
}return f||null;
}},"bottom":{type:"string",nullable:true,convert:function(a){if(parseInt(a).toString()==a){a=parseInt(a);
}return a||null;
}},"left":{type:"string",nullable:true,convert:function(o){if(parseInt(o).toString()==o){o=parseInt(o);
}return o||null;
}},"width":{type:"int",nullable:true,convert:function(j){return j==null?null:j+"%";
...
}},"height":{type:"int",nullable:true,convert:function(k){return k==null?null:k+"%";
}},"edge":{type:"enum",values:["north","east","south","west","center"],nullable:false}},GRID_PROPERTIES:{"row":{type:"int",nullable:false},"column":{type:"int",nullable:false},"rowSpan":{type:"int",nullable:true},"colSpan":{type:"int",nullable:true}}},members:{_setProperty:function(b,name,c){var d=this._properties[name].convert;

if(d){c=d(c);
}var e={};
e[name]=c;
b.setLayoutProperties(e);
},_getProperty:function(m,name){return m.getLayoutProperties()[name]||null;
},_hasProperty:function(l,name){return true;
}}});
})();
