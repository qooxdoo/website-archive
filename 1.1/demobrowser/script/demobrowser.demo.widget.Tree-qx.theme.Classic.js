...
(function(){var n="changeValue",m="execute",l="Trash",k="changeSelection",j="mode",h="multi",g="single",f="Windows (C:)",d="Documents (D:)",c="Inbox",Q="Desktop",P="root",O="Open mode:",N="Presets",M="Hide Root Node",L="Network",K="Control",J=" items",I="Sent",H="Files",u="Show Open Items",v="",s="Workspace",t="Enable drag selection",q="Selection: ",r="Junk #",o="demobrowser.demo.widget.Tree",p="Data",w="click",x="Show root open button",A="none",z="Enable multi selection",C="double click",B="Selection:",E="Show Selected Items",D="Root node:",y="Toggle Height",G="Edit",F="Show Items";
...
var bD=new qx.ui.layout.HBox();
bD.set({spacing:20});
var bC=new qx.ui.container.Composite(bD);
this.getRoot().add(bC);
var bA=this.getTree();
bC.add(bA);
var bB=this.getCommandFrame(bA);
bC.add(bB);
},getTree:function(){var bt=new qx.ui.tree.Tree().set({width:200,height:400});
var by=new qx.ui.tree.TreeFolder(P);
by.setOpen(true);
bt.setRoot(by);
var bw=new qx.ui.tree.TreeFolder(Q);
bw.setOpen(true);
by.add(bw);
var bo=new qx.ui.tree.TreeFolder(H);
var bm=new qx.ui.tree.TreeFolder(s);
var bn=new qx.ui.tree.TreeFolder(L);
var bl=new qx.ui.tree.TreeFolder(l);
bw.add(bo,bm,bn,bl);
var bu=new qx.ui.tree.TreeFile(f);
var bk=new qx.ui.tree.TreeFile(d);
bm.add(bu,bk);
var bx=new qx.ui.tree.TreeFolder(c);
var bp=new qx.ui.tree.TreeFolder(N);
var bs=new qx.ui.tree.TreeFolder(I);
var br=new qx.ui.tree.TreeFolder(l);

for(var i=0;i<30;i++){br.add(new qx.ui.tree.TreeFile(r+i));
}var bv=new qx.ui.tree.TreeFolder(p);
var bq=new qx.ui.tree.TreeFolder(G);
bx.add(bp,bs,br,bv,bq);
by.add(bx);
return bt;
},getCommandFrame:function(R){var bc=new qx.ui.groupbox.GroupBox(K);
var ba=4;
bc.setLayout(new qx.ui.layout.Grid(5,3));
var bj=0;
bc.add(new qx.ui.basic.Label(q).set({paddingTop:4}),{row:bj,column:0});
var bg=new qx.ui.form.TextField().set({readOnly:true});
bc.add(bg,{row:bj++,column:1});
R.addListener(k,function(e){var bz=e.getData();

if(bz.length>0){if(this.getSelectionMode()===h){bg.setValue(bz.length+J);
}else{bg.setValue(bz[0].getLabel());
}}else{bg.setValue(v);
}});
bc.add(new qx.ui.core.Spacer(ba,ba),{row:bj++,column:0});
bc.add(new qx.ui.basic.Label(O),{row:bj,column:0});
var V={"click":w,"dblclick":C,"none":A};
var T=new qx.ui.form.RadioGroup();

for(var be in V){var U=new qx.ui.form.RadioButton(V[be]).set({value:be==R.getOpenMode()});
U.setUserData(j,be);
T.add(U);
bc.add(U,{row:bj++,column:1});
}T.addListener(k,function(e){R.setOpenMode(e.getData()[0].getUserData(j));
});
bc.add(new qx.ui.core.Spacer(ba,ba),{row:bj++,column:0});
bc.add(new qx.ui.basic.Label(B),{row:bj,column:0});
var X=new qx.ui.form.CheckBox(z);
bc.add(X,{row:bj++,column:1});
X.addListener(n,function(e){var a=e.getData();
R.setSelectionMode(a?h:g);
});
var Y=new qx.ui.form.CheckBox(t);
bc.add(Y,{row:bj++,column:1});
Y.addListener(n,function(e){var b=e.getData();
R.setDragSelection(b);

if(!X.getValue()){X.setValue(true);
}});
bc.add(new qx.ui.core.Spacer(ba,ba),{row:bj++,column:0});
bc.add(new qx.ui.basic.Label(D),{row:bj,column:0});
var bh=new qx.ui.form.CheckBox(M);
bc.add(bh,{row:bj++,column:1});
bh.addListener(n,function(e){R.setHideRoot(e.getData());
});
var W=new qx.ui.form.CheckBox(x);
bc.add(W,{row:bj++,column:1});
W.addListener(n,function(e){R.setRootOpenClose(e.getData());
});
bc.add(new qx.ui.core.Spacer(ba,ba),{row:bj++,column:0});
var bf=new qx.ui.form.Button(F);
bc.add(bf,{row:bj++,column:1});
bf.addListener(m,function(e){alert((""+R.getItems()).replace(",","\n","g"));
});
var S=new qx.ui.form.Button(u);
bc.add(S,{row:bj++,column:1});
S.addListener(m,function(e){alert((""+R.getItems(true,false)).replace(",","\n","g"));
});
var bd=new qx.ui.form.Button(E);
bc.add(bd,{row:bj++,column:1});
bd.addListener(m,function(e){if(this.getSelectionMode()===g){alert(this.getSelection());
}else{alert(this.getSelection().toString().replace(",","\n","g"));
}},R);
var bb=new qx.ui.form.Button(y);
bc.add(bb,{row:bj++,column:1});
var bi=true;
bb.addListener(m,function(e){R.setHeight(bi?600:400);
bi=!bi;
});
return bc;
}}});
