...
var M=new qx.ui.container.Composite(new qx.ui.layout.HBox(50));
M.setPadding(20);
M.add(this.createBox1());
M.add(this.createBox2());
M.add(this.createBox3());
M.add(this.createBox4());
N.add(M,{left:20,top:20});
this.getRoot().add(N,{edge:0});
},createBox1:function(){var R=new qx.ui.container.Composite(new qx.ui.layout.VBox(2));
R.add(new qx.ui.basic.Label(l));
var Q=new qx.ui.form.SelectBox();

for(var i=0;i<30;i++){var S=new qx.ui.form.ListItem(A+(i+1));
Q.add(S);
if(i==5){Q.setSelection([S]);
}}Q.addListener(G,function(e){this.debug("changeSelection: "+e.getData()[0]);
});
R.add(Q);
return R;
},createBox2:function(){var U=new qx.ui.container.Composite(new qx.ui.layout.VBox(2));
U.add(new qx.ui.basic.Label(B));
var T=new qx.ui.form.SelectBox();

for(var i=0;i<30;i++){var V=new qx.ui.form.ListItem(L+Math.round(Math.random()*100000000));
T.add(V);
}U.add(T);
return U;
},createBox3:function(){var b=new qx.ui.container.Composite(new qx.ui.layout.VBox(2));
b.add(new qx.ui.basic.Label(u));
var d=[o,j,n,C,s,g,k,r,J,K,w,y,H,z,E,x,F,p,m];
var f=q;
var a=new qx.ui.form.SelectBox();

for(var i=0;i<d.length;i++){var c=new qx.ui.form.ListItem(d[i].substr(0,d[i].indexOf(D)),f+d[i]);
a.add(c);
}b.add(a);
return b;
},createBox4:function(){var P=new qx.ui.container.Composite(new qx.ui.layout.VBox(2));
P.add(new qx.ui.basic.Label(h));
var O=new qx.ui.form.SelectBox();
O.add(new qx.ui.form.ListItem(I));

for(var i=0;i<10;i++){O.add(new qx.ui.form.ListItem(v+(i+1)));
}P.add(O);
return P;
}}});
})();
