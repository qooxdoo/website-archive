...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.data.Gears-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"f2b880d580c4"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var t="value",s='undefined',r="",q="model",p="application/x-googlegears",o="demobrowser.demo.data.store.Gears",n="<span style='font-size: 20px'>google Gears</span>",m="loaded",l="Age: ",k="http://code.google.com/apis/gears/install.html",N="lastname",M="firstname",L="age",K="demobrowser.demo.data.Gears",J="First Name: ",I='qooxdoo',H='Gears.Factory',G='select * from Persons',F="delete from Persons",E="LastName: ",A=' (Firstname varchar(255), Lastname varchar(255), Age int)',B="Person",y='ie_mobile',z="none",w='create table if not exists Persons',x="This demo requires Gears to be installed. Install now?",u='insert into Persons values (?, ?, ?)',v='beta.database',C="changeBubble",D="object";
...
var i=new qx.ui.groupbox.GroupBox(B);
this.getRoot().add(i,{left:10,top:50});
i.setLayout(new qx.ui.layout.Grid(0,3));
i.add(new qx.ui.basic.Label(J),{row:0,column:0});
i.add(new qx.ui.basic.Label(E),{row:1,column:0});
i.add(new qx.ui.basic.Label(l),{row:2,column:0});
var b=new qx.ui.form.TextField();
i.add(b,{row:0,column:1});
var d=new qx.ui.form.TextField();
i.add(d,{row:1,column:1});
var f=new qx.ui.form.Spinner(0,0,1000);
i.add(f,{row:2,column:1});
var g=new qx.data.controller.Object();
g.addTarget(b,t,M,true);
g.addTarget(d,t,N,true);
g.addTarget(f,t,L,true);
var h=new demobrowser.demo.data.store.Gears();
h.bind(q,g,q);
...
qx.Class.define(o,{extend:qx.data.store.Json,construct:function(){qx.data.store.Json.call(this);
this.initGears();
if(!window.google||!google.gears){if(confirm(x)){location.href=k;
return;
}return;
}this.__Mo=this.initDB();
this.__Mp();
},members:{__Mp:function(W){var X=this.getPerson();
this._marshaler.toClass(X,true);
this.setModel(this._marshaler.toModel(X));
this.getModel().addListener(C,this.__Mq,this);
this.fireDataEvent(m,this.getModel());
},__Mq:function(O){var P=this.getModel();
this.update(P.getFirstname(),P.getLastname(),P.getAge());
},update:function(S,T,U){if(!google.gears.factory||!this.__Mo){return;
}this.__Mo.execute(F);
this.__Mo.execute(u,[S,T,U]);
},getPerson:function(){var Q={};
Q.firstname=r;
Q.lastname=r;
Q.age=0;

if(!google.gears.factory||!this.__Mo){return Q;
}var R=this.__Mo.execute(G);

if(R.isValidRow()){Q.firstname=R.field(0);
Q.lastname=R.field(1);
Q.age=parseInt(R.field(2));
}R.close();
return Q;
},initDB:function(){if(window.google&&google.gears){try{var j=google.gears.factory.create(v);

if(j){j.open(I);
j.execute(w+A);
}}catch(V){this.error('Could not create database: '+V.message);
}}return j;
},initGears:function(){if(window.google&&google.gears){return;
}var a=null;
if(typeof GearsFactory!=s){a=new GearsFactory();
}else{try{a=new ActiveXObject(H);
if(a.getBuildInfo().indexOf(y)!=-1){a.privateSetGlobalObject(this);
}}catch(e){if((typeof navigator.mimeTypes!=s)&&navigator.mimeTypes[p]){a=document.createElement(D);
a.style.display=z;
a.width=0;
a.height=0;
a.type=p;
