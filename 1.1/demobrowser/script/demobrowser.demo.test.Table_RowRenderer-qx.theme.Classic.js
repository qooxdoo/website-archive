...
(function(){var F="px",E='" ',D='>',C="",B=";position:relative;height:",A="table",z=' (init)</div>',y='class="',x=" (update)",w="relative",q="div",v='style="',t="px;",p="px; width:",o=": ",s="demobrowser.demo.test.Table_RowRenderer",r=", ",u='<div ';
qx.Class.define(s,{extend:qx.application.Native,include:[demobrowser.demo.table.MUtil],members:{main:function(){qx.application.Native.prototype.main.call(this);
qx.theme.manager.Meta.getInstance().initialize();
this.setUp();
this.testDefaultRenderer();
},setUp:function(){this.tableMock=this.getTableMock();
},testDefaultRenderer:function(g){var g=new qx.ui.table.rowrenderer.Default();
var k=300;
var j=20;
var i={focusedRow:[true,false],selected:[true,false],row:[1,2],table:[this.tableMock]};
var h=this._getNewTableDiv(k);
var self=this;
qx.util.Permutation.permute(i,function(a){var content=[];

for(var e in a){if(e!==A){content.push(e+o+a[e]);
}}content=content.join(r);
var f=self._createRowElement(h,k,j);
var b=g.getRowClass(a);

if(b){f.className=b;
}f.innerHTML=content+x;
g.updateDataRowElement(a,f);
var d=[];
d.push(u);

if(b){d.push(y,b,E);
}var c=g.createRowStyle(a);
c+=B+j+p+k+t;

if(c){d.push(v,c,E);
}d.push(D,content,z);
h.innerHTML+=d.join(C);
self._rowTop+=j;
});
},_rowTop:0,_createRowElement:function(parent,l,m){var n=qx.bom.Element.create(q);
qx.bom.element.Style.setStyles(n,{position:w,width:l+F,height:m+F});
this._rowTop+=20;
