...
},addSimpleStack:function(){var v=new qx.ui.container.Stack();
v.setDecorator(g);
v.setWidth(200);
v.setHeight(120);
this.getRoot().add(v,{left:20,top:20});
...
t.add(y);
t.add(w);
this.getRoot().add(t,{left:20,top:150});
...
v.add(u);
}v.addListener(j,function(e){this.debug("Selected: "+e.getData()[0].getBackgroundColor());
});
...
C.add(B);
}C.addListener(j,function(e){var z=e.getData()[0];
this.debug("Selected: "+z.getBackgroundColor()+" ("+z.getWidth()+"x"+z.getHeight()+")");
});
C.addListener(b,function(e){var a=this.getBounds();
this.debug("Resize to: "+a.width+"x"+a.height);
});
}}});
})();
