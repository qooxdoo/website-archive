...
(function(){var E="value",D="one",C="multi",B="changeValue",A="single",z="additive",y="bold",x="Item No ",w="Julian",v="Moritz",bR="Jannis",bQ="Kevin",bP="Robin",bO="Felix",bN="Vincent",bM="demobrowser.demo.virtual.List",bL="Additive Selection",bK="/places/folder.png",bJ="Nils",bI="Nick",L="Leon",M="Jannick",J="Linus",K="Configurable",H="Marc",I="icon/16/places/folder.png",F="Erik",G="addaptive",T="Colin",U="Lennard",bk="changeSelection",bg="Jason",bs="Tobias",bn="Florian",bE="16",by="Jonas",bb="icon/",bH="Jakob",bG="Enable drag selection",bF="Joel",Y="David",bd="Maximilian",bf="48",bi="Marlon",bl="Lukas",bo="Luca",bu="Julius",bA="Fabian",N="Tim",O="Justin",bc="Single Selection",br="Simon",bq="Luis",bp="Johannes",bw="Nico",bv="Alexander",bm="Jan",bt="Dominic",s="Noah",bz="Enable quick selection",P="Daniel",Q="Paul",bh="Philipp",t="Niclas",u="Max",X="One Selection",R="Additive selection",S="Jonathan",W="Tom",bj="Finn",bC="Ben",bB="on",be="One as selection mode",bD="Elias",ba="Mika",bx="Multi Selection",V="Marvin";
qx.Class.define(bM,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);
var n=new qx.ui.basic.Label(K);
n.setFont(y);
this.getRoot().add(n,{left:20,top:20});
var o=new demobrowser.demo.virtual.list.List();
o.setScrollbarX(bB);
o.set({height:280,width:150,selectionMode:C});
var k;

for(var i=1;i<=2500;i++){k=new demobrowser.demo.virtual.list.ListItem(x+i,bb+((i%4)?bE:bf)+bK);

if(i%4==0){k.setHeight(56);
}o.add(k);
if(i==20){o.setSelected(k);
}}o.addListener(B,function(e){this.debug("Value: "+e.getData());
});
this.getRoot().add(o,{left:20,top:40});
var f=new qx.ui.form.RadioButton(bc);
var b=new qx.ui.form.RadioButton(bx);
var c=new qx.ui.form.RadioButton(bL);
var a=new qx.ui.form.RadioButton(X);
f.setUserData(E,A);
b.setUserData(E,C);
c.setUserData(E,z);
a.setUserData(E,D);
b.setValue(true);
this.getRoot().add(f,{left:180,top:40});
this.getRoot().add(b,{left:180,top:60});
this.getRoot().add(c,{left:180,top:80});
this.getRoot().add(a,{left:180,top:100});
var d=new qx.ui.form.RadioGroup(f,b,c,a);
var j=new qx.ui.form.CheckBox(bG);
var m=new qx.ui.form.CheckBox(bz).set({enabled:false});
this.getRoot().add(j,{left:180,top:220});
this.getRoot().add(m,{left:180,top:240});
j.addListener(B,function(e){if(e.getData()){var r=o.getSelectionMode();

if(r==A||r==D){this.debug("Drag selection is only available for the modes multi or additive");
}}o.setDragSelection(e.getData());
});
m.addListener(B,function(e){if(e.getData()){var bT=o.getSelectionMode();

if(bT==C||bT==z){this.debug("Quick selection is only available for the modes multi or additive");
}}o.setQuickSelection(e.getData());
});
d.addListener(bk,function(e){var bS=e.getData()[0].getUserData(E);
o.setSelectionMode(bS);

if(bS==A||bS==D){j.setEnabled(false);
m.setEnabled(true);
}else if(bS==C||bS==G){j.setEnabled(true);
m.setEnabled(false);
}});
var q=new qx.ui.basic.Label(be);
q.setFont(y);
this.getRoot().add(q,{left:330,top:20});
var l=new demobrowser.demo.virtual.list.List();
l.set({height:280,width:150,selectionMode:D});
var k;

for(var i=1;i<=25;i++){k=new demobrowser.demo.virtual.list.ListItem(x+i,I);
l.add(k);
if(i==16){l.setSelected(k);
}}this.getRoot().add(l,{left:330,top:40});
var n=new qx.ui.basic.Label(R);
n.setFont(y);
this.getRoot().add(n,{left:520,top:20});
var p=new demobrowser.demo.virtual.list.List;
var g;
p.set({width:150,selectionMode:z});
var h=[L,bl,bo,bj,N,bO,by,bq,bd,w,u,Q,t,bm,bC,bD,M,bh,s,W,v,bw,Y,bJ,br,bA,F,O,bv,bH,bn,bI,J,ba,bg,P,U,V,bR,bs,bt,bi,H,bp,S,bu,T,bF,bQ,bN,bP];

for(var i=0;i<h.length;i++){g=new demobrowser.demo.virtual.list.ListItem(h[i]);
p.add(g);

if(i==10||i==12||i==16){p.addToSelection(g);
}}this.getRoot().add(p,{left:520,top:40});
}}});
})();
...
(function(){var b="qx.ui.virtual.core.IWidgetCellProvider";
qx.Interface.define(b,{members:{getCellWidget:function(c,d){},poolCellWidget:function(a){}}});
})();
(function(){var r="selected",q="qx.event.type.Data",p="Boolean",o="changeSelection",n="row",m="single",k="changeHeight",h=",",g="one",f="_applySpacing",G="Integer",F="_applyDragSelection",E="String",D="resize",C="demobrowser.demo.virtual.list.List",B="list",A="multi",z="_applyQuickSelection",y="_applyItemHeight",x="_applySelectionMode",v="additive",w="changeName",t="Number",u="addItem",s="removeItem";
qx.Class.define(C,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider],construct:function(){qx.ui.virtual.core.Scroller.call(this,0,1,this.getItemHeight(),10);
this.__Nw=new qx.ui.virtual.layer.WidgetCell(this);
this.getPane().addLayer(this.__Nw);
this.getPane().addListener(D,this._onResize,this);
this.__Nx=[];
this.__Ny=[];
this.__Nz=new qx.ui.virtual.selection.Row(this.getPane());
this.__Nz.addListener(o,this._onChangeSelection,this);
this.__Nz.attachMouseEvents();
this.__Nz.attachKeyEvents(this);
this.__Nz.attachListEvents(this);
new qx.ui.virtual.behavior.Prefetch(this,0,0,0,0,400,600,400,600).set({interval:500});
},events:{addItem:q,removeItem:q,changeValue:q,changeSelection:q},properties:{appearance:{refine:true,init:B},focusable:{refine:true,init:true},spacing:{check:G,init:0,apply:f,themeable:true},enableInlineFind:{check:p,init:true},name:{check:E,nullable:true,event:w},itemHeight:{init:24,themeable:true,check:t,apply:y},selectionMode:{check:[m,A,v,g],init:m,apply:x},dragSelection:{check:p,init:false,apply:F},quickSelection:{check:p,init:false,apply:z}},members:{__Nx:null,__Ny:null,__Nz:null,__Nw:null,syncWidget:function(){this.update();
},update:function(){var L=this.getPane().getRowConfig();
L.setItemCount(this.__Nx.length);
L.resetItemSizes();

for(var i=0;i<this.__Nx.length;i++){var K=this.__Nx[i].getHeight();

if(K!==null){L.setItemSize(i,K);
}}},updateSelection:function(){var U=this.__Nw.getChildren();

for(var i=0;i<U.length;i++){var V=U[i];
var W=V.getUserData(n);

if(this.__Nz.isItemSelected(W)){V.addState(r);
}else{V.removeState(r);
...
this.fireDataEvent(o,this.__ND(e.getData()));
},getCellWidget:function(a,b){var d=this.__Nx[a];

if(!d){return null;
}var c=this.__Ny.pop()||new qx.ui.form.ListItem();
c.set({label:d.getLabel(),icon:d.getIcon()});

if(this.__Nz.isItemSelected(a)){c.addState(r);
}else{c.removeState(r);
}c.setUserData(n,a);
return c;
},poolCellWidget:function(J){this.__Ny.push(J);
},getValue:function(){var O=this.getSelection();
var P=[];
var Q;

for(var i=0,l=O.length;i<l;i++){Q=O[i].getValue();
if(Q==null){Q=O[i].getLabel();
}P.push(Q);
}return P.join(h);
},setValue:function(bl){var bo=bl.split(h);
var bn=[];
var bm;

for(var i=0,l=bo.length;i<l;i++){bm=this.findItem(bo[i]);

if(bm){bn.push(bm);
}}this.setSelection(bn);
},getChildren:function(){return this.__Nx;
},hasChildren:function(){return this.__Nx.length>0;
},_addHelper:function(bA){this.fireDataEvent(u,bA);
bA.addListener(k,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},_removeHelper:function(bs){this.fireDataEvent(s,bs);
bs.removeListener(k,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},add:function(bh){this.__Nx.push(bh);
this._addHelper(bh);
return this;
},remove:function(bx){qx.lang.Array.remove(this.__Nx,bx);
this._removeHelper(bx);
return this;
},removeAll:function(){for(var i=0,j=this.__Nx.length;i<j;i++){this._removeHelper(this.__Nx[i]);
}this.__Nx=[];
},indexOf:function(H){return this.__Nx.indexOf(H);
},addAt:function(bf,bg){qx.lang.Array.insertAt(this.__Nx,bf,bg);
this._addHelper(bf);
},addBefore:function(by,bz){qx.lang.Array.insertBefore(this.__Nx,by,bz);
this._addHelper(by);
},addAfter:function(ba,bb){qx.lang.Array.insertAfter(this.__Nx,ba,bb);
this._addHelper(ba);
},removeAt:function(bc){this.__Nx.splice(bc,1);
this._removeHelper(this.__Nx[bc]);
},__NA:function(bt){var bu=this.__Nx.indexOf(bt);

if(bu<0){return null;
}else{return bu;
}},__NB:function(bv){var bw=this.__Nx[bv];
return bw;
},__NC:function(bp){var bq=[];

for(var i=0;i<bp.length;i++){bq.push(this.__NA(bp[i]));
}return bq;
},__ND:function(X){var Y=[];

for(var i=0;i<X.length;i++){Y.push(this.__Nx[X[i]]);
}return Y;
},selectAll:function(){this.__Nz.selectAll();
},setSelected:function(bk){this.__Nz.selectItem(this.__NA(bk));
},isSelected:function(br){return this.__Nz.isItemSelected(this.__NA(br));
},addToSelection:function(I){this.__Nz.addItem(this.__NA(I));
},removeFromSelection:function(bB){this.__Nz.removeItem(this.__NA(bB));
},selectRange:function(M,N){this.__Nz.selectItemRange(this.__NA(M),this.__NA(N));
},resetSelection:function(){this.__Nz.clearSelection();
},setSelection:function(R){this.__Nz.replaceSelection(this.__NC(R));
},getSelectedItem:function(){return this.__Nx[this.getSelected()];
},getSelected:function(){return this.__Nx[this.__Nz.getSelectedItem()];
},getSelection:function(){return this.__ND(this.__Nz.getSelection());
},getSortedSelection:function(){return this.__ND(this.__Nz.getSortedSelection());
},isSelectionEmpty:function(){return this.__Nz.isSelectionEmpty();
},getSelectionContext:function(){return this.__Nz.getSelectionContext();
},getSelectables:function(){return this.__ND(this.__Nz.getSelectables());
},_applySelectionMode:function(bi,bj){this.__Nz.setMode(bi);
},_applyDragSelection:function(S,T){this.__Nz.setDrag(S);
},_applyQuickSelection:function(bd,be){this.__Nz.setQuick(bd);
}}});
})();
...
(function(){var j="String",i="Integer",h="changeWidth",g="qx.ui.form.RadioGroup",f="_applyManager",e="changeHeight",d="qx.event.type.Event",c="changeValue",b="demobrowser.demo.virtual.list.ListItem";
qx.Class.define(b,{extend:qx.core.Object,construct:function(k,l,m){qx.core.Object.call(this);
...
}},events:{"action":d},properties:{label:{nullable:true,dispose:true,check:j},icon:{check:j,nullable:true},manager:{check:g,nullable:true,apply:f},value:{check:j,nullable:true,event:c},width:{check:i,nullable:true,event:h,init:null},height:{check:i,nullable:true,event:e,init:null}},members:{_applyManager:function(n,o){if(o){o.remove(this);
}
if(n){n.add(this);
