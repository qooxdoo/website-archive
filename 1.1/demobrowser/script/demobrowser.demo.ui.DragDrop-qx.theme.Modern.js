...
(function(){var B="drop",A="value",z="items",y="move",x="multi",w="dragover",v="icon/16/places/folder.png",u="dragstart",t="Item ",s="copy",O="drag",N="TextArea Target",M="solid",L="Reorderable",K="droprequest",J="#33508D",I="Source",H="__NV",G="demobrowser.demo.ui.DragDrop",F="Simple Target",D="Enable drag",E="\n",C="dragend";
qx.Class.define(G,{extend:qx.application.Standalone,members:{__NV:null,__NW:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var q=this.getRoot();
var j=new qx.ui.container.Scroll();
var c=new qx.ui.layout.Basic();
var g=new qx.ui.container.Composite(c).set({allowStretchY:false,allowStretchX:false});
j.add(g);
q.add(j,{edge:0});
var p=new qx.ui.basic.Label(I);
g.add(p,{left:20,top:20});
var n=new qx.ui.form.List;
n.setDraggable(true);
n.setSelectionMode(x);
g.add(n,{left:20,top:40});

for(var i=0;i<20;i++){n.add(new qx.ui.form.ListItem(t+i,v));
}var r=new qx.ui.form.CheckBox(D);
r.setValue(true);
g.add(r,{left:20,top:260});
n.addListener(u,function(e){if(!r.isValue()){e.preventDefault();
}e.addType(A);
e.addType(z);
e.addAction(s);
e.addAction(y);
});
n.addListener(K,function(e){this.debug("Related of droprequest: "+e.getRelatedTarget());
var R=e.getCurrentAction();
var T=e.getCurrentType();
var Q;

switch(T){case z:Q=this.getSelection();

if(R==s){var S=[];

for(var i=0,l=Q.length;i<l;i++){S[i]=Q[i].clone();
}Q=S;
}break;
case A:Q=this.getSelection()[0].getLabel();
break;
}if(R==y){var P=this.getSelection();

for(var i=0,l=P.length;i<l;i++){this.remove(P[i]);
}}e.addData(T,Q);
});
var k=new qx.ui.basic.Label(F);
g.add(k,{left:140,top:20});
var h=new qx.ui.form.List;
h.setDroppable(true);
h.setSelectionMode(x);
g.add(h,{left:140,top:40});
h.addListener(B,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
var U=e.getData(z);

for(var i=0,l=U.length;i<l;i++){this.add(U[i]);
}});
h.addListener(w,function(e){if(!e.supportsType(z)){e.preventDefault();
}});
var k=new qx.ui.basic.Label(N);
g.add(k,{left:260,top:20});
var o=new qx.ui.form.TextArea;
o.setDroppable(true);
o.setHeight(100);
g.add(o,{left:260,top:40});
o.addListener(B,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
this.setValue(e.getData(A).replace(/,/g,E));
});
o.addListener(w,function(e){if(!e.supportsType(A)){e.preventDefault();
}});
var m=new qx.ui.basic.Label(L);
g.add(m,{left:500,top:20});
var d=this.__NV=new qx.ui.form.List;
d.setDraggable(true);
d.setDroppable(true);
d.setSelectionMode(x);
g.add(d,{left:500,top:40});

for(var i=0;i<20;i++){d.add(new qx.ui.form.ListItem(t+i,v));
}var f=new qx.ui.core.Widget;
f.setDecorator(new qx.ui.decoration.Single().set({top:[1,M,J]}));
f.setHeight(0);
f.setOpacity(0.5);
f.setZIndex(100);
...
f.setDroppable(true);
this.getRoot().add(f);
d.addListener(u,function(e){e.addAction(y);
});
d.addListener(C,function(e){f.setDomPosition(-1000,-1000);
});
d.addListener(O,function(e){var W=e.getOriginalTarget();
if(W instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__NW=W;
}
if(!qx.ui.core.Widget.contains(this,W)&&W!=f){return;
}var V=W.getContainerLocation();
f.setWidth(W.getBounds().width);
f.setDomPosition(V.left,V.top);
});
d.addListener(w,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
d.addListener(B,function(e){this.__NX(e.getOriginalTarget());
},this);
f.addListener(B,function(e){this.__NX(this.__NW);
},this);
},__NX:function(a){if(a.classname!="qx.ui.form.ListItem"){return;
}var b=this.__NV.getSortedSelection();

for(var i=0,l=b.length;i<l;i++){this.__NV.addBefore(b[i],a);
this.__NV.addToSelection(b[i]);
