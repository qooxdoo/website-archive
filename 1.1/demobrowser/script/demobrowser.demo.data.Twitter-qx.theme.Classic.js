...
(function(){var k="value",j="icon",i="model",h="text",g="qx.client",f="execute",e="source",d="wittemann",c="mshtml",b=".json",I="user.profile_image_url",H="http://twitter.com/statuses/user_timeline/",G="Enter",F="<span style='font-size: 20px'>Twitter</span>",E="<a target='_blank' href='http://twitter.com/home?status=Check%20out%20the%20%23qooxdoo%20twitter%20demo%20at%20http://bit.ly/Mc4Of'>Tweet This</a></span>",D="demobrowser.demo.data.store.Twitter",C="keydown",B="Friends",A="loaded",z="demobrowser.demo.data.Twitter",r="selection[0]",s="<span style='font-size: 13px'>",p="Location: ",q="Details",n="Name: ",o="?callback=demobrowser.demo.data.store.Twitter.saveResult",l="User",m="http://twitter.com/statuses/friends_timeline.json",t="Postet with: ",u="failed",w="user.name",v="user.location",y="Avatar: ",x="Message: ";
...
this.getRoot().add(O,{left:10,top:85});
O.setWidth(300);
var Q=new qx.data.controller.List(null,O);
Q.setDelegate(this);
Q.setLabelPath(h);
if(!qx.core.Variant.isSet(g,c)){Q.setIconPath(I);
}var U=new demobrowser.demo.data.store.Twitter(d);
U.bind(i,Q,i);
var M=new qx.ui.form.Button(B);
this.getRoot().add(M,{left:10,top:55});
M.addListener(f,function(){U.setUrl(m);
},this);
var R=new qx.ui.form.Button(l);
this.getRoot().add(R,{left:90,top:55});
var J=new qx.ui.form.TextField(d);
this.getRoot().add(J,{left:140,top:55});
R.addListener(f,function(){var bc=H+J.getValue()+b;

if(U.getUrl()==bc){U.reload();
}else{U.setUrl(bc);
}},this);
J.addListener(C,function(a){if(a.getKeyIdentifier()==G){R.execute();
}},this);
var N=new qx.ui.groupbox.GroupBox(q);
this.getRoot().add(N,{left:320,top:60});
N.setWidth(300);
N.setHeight(220);
N.setLayout(new qx.ui.layout.Grid(0,5));
N.add(new qx.ui.basic.Label(n),{row:0,column:0});
N.add(new qx.ui.basic.Label(p),{row:1,column:0});
N.add(new qx.ui.basic.Label(x),{row:2,column:0});
N.add(new qx.ui.basic.Label(t),{row:3,column:0});
...
S.setRich(true);
S.setWidth(150);
N.add(S,{row:2,column:1});
var V=new qx.ui.basic.Label();
V.setRich(true);
N.add(V,{row:3,column:1});
var K=new qx.data.controller.Object();
K.addTarget(name,k,w);
K.addTarget(location,k,v);
K.addTarget(S,k,h);
K.addTarget(V,k,e);

if(!qx.core.Variant.isSet(g,c)){N.add(new qx.ui.basic.Label(y),{row:4,column:0});
var P=new qx.ui.basic.Image();
N.add(P,{row:4,column:1});
K.addTarget(P,e,I);
}Q.bind(r,K,i);
...
T.setWidth(260);
T.setValue(s+E);
this.getRoot().add(T,{left:10,top:290});
},configureItem:function(W){W.setRich(true);
W.getChildControl(j).setWidth(50);
W.getChildControl(j).setHeight(50);
W.getChildControl(j).setScale(true);
}}});
qx.Class.define(D,{extend:qx.data.store.Json,statics:{saveResult:function(bf){this.__Mt=bf;
}},construct:function(bd){var be=H+bd+b;
qx.data.store.Json.call(this,be);
},members:{_createRequest:function(ba){var bb=new qx.io.ScriptLoader();
ba+=o;
bb.load(ba,function(Y){this.__Mu();
},this);
},__Mu:function(){var X=demobrowser.demo.data.store.Twitter.__Mt;

if(X==undefined){this.setState(u);
return;
