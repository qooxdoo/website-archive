...
(function(){var ck="value",cj="multi",ci="one",ch="bold",cg="changeSelection",cf="additive",ce="single",cd="changeValue",cc="Item No ",cb="Julian",bP="Show Icon",bO="Kevin",bN="Marvin",bM="Marc",bL="Configurable",bK="icon/16/places/folder.png",bJ="computer.png",bI="Jason",bH="Jakob",bG="Lukas",cr="Julius",cs="network-wired.png",cp="Jan",cq="Tobias",cn="Mika",co="Nils",cl="Noah",cm="Tom",ct="Nico",cu="Multi Selection",bT="battery.png",bS="Felix",bV="Leon",bU="input-keyboard.png",bX="camera-web.png",bW="Enable drag selection",ca="icon",bY="Show Both",bR="48",bQ="icon/48/devices/",D="Fabian",E="multimedia-player.png",F="camera-photo.png",G="media-flash.png",H="input-mouse.png",I="Joel",J="Dominic",K="Show Label",L="demobrowser.demo.widget.List",M="Enable quick selection",cy="Finn",cx="One as selection mode",cw="Jannis",cv="audio-card.png",cC="Alexander",cB="Additive Selection",cA="display.png",cz="Erik",cE="addaptive",cD="Johannes",bm="pda.png",bn="16",bk="Jonas",bl="icon/",bq="David",br="Maximilian",bo="Tim",bp="Justin",bi="Single Selection",bj="Linus",U="Daniel",T="Paul",W="Philipp",V="Niclas",Q="One Selection",P="Nick",S="Jonathan",R="Ben",O="on",N="Vincent",bw="/places/folder.png",bx="Robin",by="both",bz="Jannick",bs="Luis",bt="drive-harddisk.png",bu="Lennard",bv="Moritz",bA="Florian",bB="drive-optical.png",bf="media-optical.png",be="Marlon",bd="Luca",bc="network-wireless.png",bb="audio-input-microphone.png",ba="phone.png",Y="Simon",X="Colin",bh="label",bg="printer.png",bC="Max",bD="Additive selection",bE="Horizontal, Icons only",bF="Elias";
...
var n=new qx.ui.basic.Label(bL);
n.setFont(ch);
t.add(n,{left:20,top:20});
var v=new qx.ui.form.List;
v.setScrollbarX(O);
v.set({height:280,width:150,selectionMode:cj});
var w;

for(var i=1;i<=25;i++){w=new qx.ui.form.ListItem(cc+i,bl+((i%4)?bn:bR)+bw);
v.add(w);
!(i%9)&&(w.setEnabled(false));
if(i==20){v.setSelection([w]);
}}v.addListener(cg,function(e){var cG=e.getData();

if(cG[0]){this.debug("Value: "+cG[0].getLabel());
}});
t.add(v,{left:20,top:40});
var j=new qx.ui.form.RadioButton(bi);
var f=new qx.ui.form.RadioButton(cu);
var g=new qx.ui.form.RadioButton(cB);
var d=new qx.ui.form.RadioButton(Q);
j.setUserData(ck,ce);
f.setUserData(ck,cj);
g.setUserData(ck,cf);
d.setUserData(ck,ci);
f.setValue(true);
t.add(j,{left:180,top:40});
t.add(f,{left:180,top:60});
t.add(g,{left:180,top:80});
t.add(d,{left:180,top:100});
var h=new qx.ui.form.RadioGroup(j,f,g,d);
var z=new qx.ui.form.RadioButton(K);
var y=new qx.ui.form.RadioButton(bP);
var B=new qx.ui.form.RadioButton(bY);
z.setUserData(ck,bh);
y.setUserData(ck,ca);
B.setUserData(ck,by);
B.setValue(true);
t.add(z,{left:180,top:140});
t.add(y,{left:180,top:160});
t.add(B,{left:180,top:180});
var u=new qx.ui.form.RadioGroup(z,y,B);
u.addListener(cg,function(e){for(var i=0;i<v.getChildren().length;i++){v.getChildren()[i].setShow(e.getData()[0].getUserData(ck));
}});
var r=new qx.ui.form.CheckBox(bW);
var o=new qx.ui.form.CheckBox(M).set({enabled:false});
t.add(r,{left:180,top:220});
t.add(o,{left:180,top:240});
r.addListener(cd,function(e){if(e.getData()){var C=v.getSelectionMode();

if(C==ce||C==ci){this.debug("Drag selection is only available for the modes multi or additive");
}}v.setDragSelection(e.getData());
});
o.addListener(cd,function(e){if(e.getData()){var a=v.getSelectionMode();

if(a==cj||a==cf){this.debug("Quick selection is only available for the modes multi or additive");
}}v.setQuickSelection(e.getData());
});
h.addListener(cg,function(e){var cF=e.getData()[0].getUserData(ck);
v.setSelectionMode(cF);

if(cF==ce||cF==ci){r.setEnabled(false);
o.setEnabled(true);
}else if(cF==cj||cF==cE){r.setEnabled(true);
o.setEnabled(false);
}});
var A=new qx.ui.basic.Label(cx);
A.setFont(ch);
t.add(A,{left:330,top:20});
var m=new qx.ui.form.List();
m.set({height:280,width:150,selectionMode:ci});
var w;

for(var i=1;i<=25;i++){w=new qx.ui.form.ListItem(cc+i,bK);
m.add(w);
if(i==16){m.setSelection([w]);
}}t.add(m,{left:330,top:40});
var n=new qx.ui.basic.Label(bD);
n.setFont(ch);
t.add(n,{left:520,top:20});
var c=new qx.ui.form.List;
var k;
c.set({width:150,selectionMode:cf});
var q=[bV,bG,bd,cy,bo,bS,bk,bs,br,cb,bC,T,V,cp,R,bF,bz,W,cl,cm,bv,ct,bq,co,Y,D,cz,bp,cC,bH,bA,P,bj,cn,bI,U,bu,bN,cw,cq,J,be,bM,cD,S,cr,X,I,bO,N,bx];

for(var i=0;i<q.length;i++){k=new qx.ui.form.ListItem(q[i]);
c.add(k);

if(i==10||i==12||i==16){c.addToSelection(k);
}}t.add(c,{left:520,top:40});
var n=new qx.ui.basic.Label(bE);
n.setFont(ch);
t.add(n,{left:20,top:350});
var x=new qx.ui.form.List(true);
var l;
x.set({width:550,selectionMode:cj,height:null});
var p=[cv,bb,bT,F,bX,bJ,cA,bt,bB,bU,H,G,bf,E,cs,bc,bm,ba,bg];
var s=bQ;

for(var i=0;i<p.length;i++){l=new qx.ui.form.ListItem(null,s+p[i]);
x.add(l);

if(i==12){x.setSelection([l]);
}}t.add(x,{left:20,top:370});
}}});
})();
