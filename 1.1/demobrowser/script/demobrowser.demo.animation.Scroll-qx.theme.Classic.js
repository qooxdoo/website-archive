...
(function(){var x="execute",w="sinodial",v="spring",u="testDiv",t="Scroll Right",s="Scroll Up",r="Scroll Down",q="Scroll Left",p="Scroll to 40, 100",o="demobrowser.demo.animation.Scroll",n="absolute";
...
var d=new qx.fx.effect.core.Scroll(m).set({y:100,transition:w});
var e=new qx.fx.effect.core.Scroll(m).set({y:-100,transition:v});
var h=new qx.fx.effect.core.Scroll(m).set({x:-100,transition:v});
var k=new qx.fx.effect.core.Scroll(m).set({x:100,transition:w});
var j=new qx.fx.effect.core.Scroll(m).set({x:40,y:100,mode:n});
var a=new qx.ui.form.Button(s);
var g=new qx.ui.form.Button(r);
var b=new qx.ui.form.Button(q);
var c=new qx.ui.form.Button(t);
var l=new qx.ui.form.Button(p);
a.addListener(x,function(){e.start();
});
g.addListener(x,function(){d.start();
});
b.addListener(x,function(){h.start();
});
c.addListener(x,function(){k.start();
});
l.addListener(x,function(){j.start();
});
var i=new qx.ui.container.Composite(new qx.ui.layout.HBox(5));
f.add(i,{left:10,top:10});
i.add(a);
i.add(g);
i.add(new qx.ui.core.Spacer(10));
i.add(b);
i.add(c);
i.add(new qx.ui.core.Spacer(10));
i.add(l);
...
qx.Class.define(c,{extend:qx.fx.Base,properties:{mode:{init:f,check:[f,d]},x:{init:0,check:e},y:{init:0,check:e}},members:{__IP:null,__IQ:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var i=this._getElement();
this.__IP={x:i.scrollLeft,y:i.scrollTop};
if(this._atEndPosition(this.__IP.x,this.__IP.y)){return;
}
if(this.getMode()==d){this.__IQ={left:this.getX()-this.__IP.x,top:this.getY()-this.__IP.y};
}},update:function(a){qx.fx.Base.prototype.update.call(this);
var b=this._getElement();

if(this.getMode()==f){if(this.getX()!=0){b.scrollLeft=this.__IP.x+(this.getX()*a);
}
if(this.getY()!=0){b.scrollTop=this.__IP.y+(this.getY()*a);
}}else{b.scrollLeft=this.__IP.x+(this.__IQ.left*a);
b.scrollTop=this.__IP.y+(this.__IQ.top*a);
