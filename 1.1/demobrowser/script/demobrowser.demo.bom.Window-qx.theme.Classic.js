...
(function(){var s="click",r="button",q="Open Native Window",p="Open Native Modal Window",o="demobrowser.demo.bom.Window",n="Modal Window closed?",m="window1",l="Window closed?",k="Open Faked Native Modal Window",j="window3",h="window2",i="#eee";
qx.Class.define(o,{extend:qx.application.Native,members:{__LT:null,main:function(){qx.application.Native.prototype.main.call(this);
var a=qx.util.ResourceManager.getInstance();
this.__LT=["http://qooxdoo.org","http://sourceforge.net","http://slashdot.org",a.toUri("demobrowser/demo/html/ModalWindow.html")];
var d=qx.bom.Input.create(r);
qx.bom.Input.setValue(d,q);
document.body.appendChild(d);
qx.event.Registration.addListener(d,s,function(e){var u={width:400,height:200,top:200,left:100,scrollbars:false,menubar:true,status:false};
this.window1=qx.bom.Window.open(this.__LT[0],m,u);
},this);
var c=qx.bom.Input.create(r);
qx.bom.Input.setValue(c,l);
document.body.appendChild(c);
qx.event.Registration.addListener(c,s,function(e){alert(qx.bom.Window.isClosed(this.window1));
},this);
var b=qx.bom.Input.create(r);
qx.bom.Input.setValue(b,p);
document.body.appendChild(b);
qx.event.Registration.addListener(b,s,function(e){var t={width:400,height:200,top:200,left:100,scrollbars:false};
this.window2=qx.bom.Window.open(this.__LT[3],h,t,true);
},this);
var g=qx.bom.Input.create(r);
qx.bom.Input.setValue(g,n);
document.body.appendChild(g);
qx.event.Registration.addListener(g,s,function(e){alert(qx.bom.Window.isClosed(this.window2));
},this);
var f=qx.bom.Input.create(r);
qx.bom.Input.setValue(f,k);
document.body.appendChild(f);
qx.event.Registration.addListener(f,s,function(e){var w={width:800,height:600,top:200,left:100,scrollbars:false};
var v=qx.bom.Window.getBlocker();
v.setBlockerColor(i);
v.setBlockerOpacity(0.5);
this.window3=qx.bom.Window.open(this.__LT[0],j,w,true,false);
},this);
...
(function(){var n="qx.bom.Window",m="interval",l="javascript:/",k="qxNativeWindow";
qx.Class.define(n,{statics:{__Fx:null,__Fy:null,__Fz:null,__FA:{"top":"dialogTop",left:"dialogLeft",width:"dialogWidth",height:"dialogHeight",scrollbars:"scroll",resizable:"resizable"},__FB:{"top":1,left:1,width:1,height:1,dependent:1,resizable:1,status:1,location:1,menubar:1,scrollbars:1,toolbar:1},__FC:function(){return window.showModalDialog!=null;
},open:function(v,name,w,x,y){if(v==null){v=l;
}
if(name==null){name=k+new Date().getTime();
}
if(y==null){y=true;
}var z=this.__FD(w,x&&y);

if(x){if(this.__FC()&&y){return window.showModalDialog(v,[window.self],z);
}else{this.getBlocker().block();

if(this.__Fz==null){this.__Fz=new qx.event.Timer(500);
this.__Fz.addListener(m,this.__FE,this);
this.__Fz.start();
}else{this.__Fz.restart();
}this.__Fy=window.open(v,name,z);
return this.__Fy;
}}else{return window.open(v,name,z);
}},__FD:function(o,p){var q;
var t;
var r=[];

if(p&&this.__FC()){for(var s in o){if(qx.bom.Window.__FA[s]){var u="";

if(s!="scrollbars"&&s!="resizable"){u="px";
}t=qx.bom.Window.__FA[s]+":"+o[s]+u;
r.push(t);
}else{qx.log.Logger.warn("Option '"+s+"' is not supported for modal windows.");
}}q=r.join(";");
}else{for(var s in o){if(qx.bom.Window.__FB[s]){if(qx.lang.Type.isBoolean(o[s])){t=s+"="+(o[s]?"yes":"no");
}else{t=s+"="+o[s];
}r.push(t);
}else{qx.log.Logger.warn("Option '"+s+"' is not supported for native windows.");
}}q=r.join(",");
}return q;
},__FE:function(){if(this.isClosed(this.__Fy)){this.getBlocker().unblock();
this.__Fz.stop();
}},getBlocker:function(){if(this.__Fx==null){this.__Fx=new qx.bom.Blocker;
}return this.__Fx;
},close:function(h){if(h){return h.close();
}},isClosed:function(a){var closed=true;

if(a){try{closed=a.closed;
}catch(i){}}return closed;
},moveTo:function(f,top,g){if(!qx.bom.Window.isClosed(f)){try{f.moveTo(g,top);
}catch(j){qx.log.Logger.error("Cross-Domain Scripting problem: Could not move window!",j);
}}},resizeTo:function(c,d,e){if(!qx.bom.Window.isClosed(c)){try{c.resizeTo(d,e);
