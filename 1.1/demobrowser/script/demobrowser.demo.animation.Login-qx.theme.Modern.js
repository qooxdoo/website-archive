...
(function(){var o="top",n="right",m="resize",l="Name",k="__LR",j="Password",h="__LP",g="__LQ",f="appear",d="demobrowser.demo.animation.Login",a="50%",c="execute",b="30%";
qx.Class.define(d,{extend:qx.application.Standalone,members:{__LP:null,__LQ:null,__LR:null,checkInput:function(){this.__LP.start();
},__LS:function(){this.__LP=new qx.fx.effect.combination.Shake(this.__LQ.getContainerElement().getDomElement());
...
this.__LQ=new qx.ui.groupbox.GroupBox().set({contentPadding:[16,16,16,16]});
this.__LQ.setLayout(q);
this.__LQ.addListener(m,function(e){var u=this.__LQ.getBounds();
this.__LQ.set({marginTop:Math.round(-u.height/2),marginLeft:Math.round(-u.width/2)});
},this);
this.getRoot().add(this.__LQ,{left:a,top:b});
var p=[l,j];

for(var i=0;i<p.length;i++){this.__LQ.add(new qx.ui.basic.Label(p[i]).set({allowShrinkX:false,paddingTop:3}),{row:i,column:0});
}var r=new qx.ui.form.TextField();
var s=new qx.ui.form.PasswordField();
this.__LQ.add(r.set({allowShrinkX:false,paddingTop:3}),{row:0,column:1});
this.__LQ.add(s.set({allowShrinkX:false,paddingTop:3}),{row:1,column:1});
var t=this.__LR=new qx.ui.form.Button("Login");
t.setAllowStretchX(false);
this.__LQ.add(t,{row:3,column:1});
t.addListener(c,this.checkInput,this);
this.__LQ.addListener(f,this.__LS,this);
}},destruct:function(){this._disposeObjects(g,k,h);
}});
})();
...
this.__GP=[new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k)];
...
if(this.getDirection()==e){this.__GP[0].set({x:f,y:0,duration:g});
this.__GP[1].set({x:-f*2,y:0,duration:g*2});
this.__GP[2].set({x:f*2,y:0,duration:g*2});
this.__GP[3].set({x:-f*2,y:0,duration:g*2});
this.__GP[4].set({x:f*2,y:0,duration:g*2});
this.__GP[5].set({x:-f,y:0,duration:g*2});
}else if(this.getDirection()==d){this.__GP[0].set({y:f,x:0,duration:g});
this.__GP[1].set({y:-f*2,x:0,duration:g*2});
this.__GP[2].set({y:f*2,x:0,duration:g*2});
this.__GP[3].set({y:-f*2,x:0,duration:g*2});
this.__GP[4].set({y:f*2,x:0,duration:g*2});
this.__GP[5].set({y:-f,x:0,duration:g*2});
}var j=this.__GP;

for(var i=0,h=this.__GP.length;i<h;i++){this.__GP[i].id=i;

if(i<5){this.__GP[i].afterFinishInternal=function(){j[this.id+1].start();
};
}}this.__GP[0].start();
