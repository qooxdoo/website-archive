...
(function(){var t="execute",s="Add 10 rows",r="Remove 10 rows",q="Whether the the first visible row should ",p="demobrowser.demo.table.Table_Resize_Columns",o="A number",n="_tableModel",m="1*",l="icon/22/actions/list-add.png",k="50%",e="A date",j="changeValue",h="Boolean test",d="be rendered completely when scrolling.",c="Table with automatic column sizes",g="Keep first row",f="icon/22/actions/list-remove.png",i="ID";
qx.Class.define(p,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return c;
},createTable:function(){var y=this.createRandomRows(50);
var w=this._tableModel=this._tableModel=new qx.ui.table.model.Simple();
w.setColumns([i,o,e,h]);
w.setData(y);
var v={tableColumnModel:function(A){return new qx.ui.table.columnmodel.Resize(A);
}};
var z=this._table=new qx.ui.table.Table(w,v);
z.set({width:600,height:400});
...
E.add(D);
C=new qx.ui.toolbar.Button(s,l);
C.addListener(t,function(H){var I=this.createRandomRows(10);
this._tableModel.addRows(I);
this.info("10 rows added");
},this);
D.add(C);
C=new qx.ui.toolbar.Button(r,f);
C.addListener(t,function(a){var b=this._tableModel.getRowCount();
this._tableModel.removeRows(b-10,10);
...
E.add(D);
var B=this._table;
F=new qx.ui.toolbar.CheckBox(g);
F.set({value:B.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(q+d)});
F.addListener(j,function(G){B.setKeepFirstVisibleRowComplete(this.getValue());
},F);
D.add(F);
return E;
