...
var V=[x,y,o,j,r,u,p,c];
var M=[D,g,F,G,E,v];
var S=[];

for(var i=0;i<20;i++){var P=new demobrowser.demo.data.model.Person();
P.setName(V[i%V.length]);
P.setEmote(M[i%M.length]);
P.setOnline(i%3==0);
S.push(P);
}var T=new qx.data.Array(S);
var L=new qx.ui.form.List();
L.setWidth(150);
this.getRoot().add(L,{left:10,top:80});
var U={converter:function(ba){return n+ba+h;
}};
var Q=new qx.data.controller.List(T,L,A);
Q.setIconOptions(U);
Q.setIconPath(e);
var O={converter:function(a){if(a){return d;
}else{return w;
}}};
var J={bindItem:function(W,X,Y){W.bindDefaultProperties(X,Y);
W.bindProperty(C,s,O,X,Y);
}};
Q.setDelegate(J);
var K=new qx.ui.form.Button(H);
K.setWidth(120);
this.getRoot().add(K,{left:180,top:80});
K.addListener(l,function(){for(var i=0;i<T.length;i++){var bb=T.getItem(i);
bb.setEmote(k);
}},this);
var I=new qx.ui.form.Button(m);
I.setWidth(120);
this.getRoot().add(I,{left:180,top:115});
I.addListener(l,function(){for(var i=0;i<T.length;i++){T.getItem(i).toggleOnline();
}},this);
var R=new qx.ui.form.Button(z);
R.setWidth(120);
this.getRoot().add(R,{left:180,top:150});
R.addListener(l,function(){qx.log.appender.Console.show();
