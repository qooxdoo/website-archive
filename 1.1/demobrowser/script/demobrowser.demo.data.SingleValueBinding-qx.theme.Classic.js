...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.data.SingleValueBinding","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.data.SingleValueBinding-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"1723c78a2d17"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var k="value",j="<b>Description</b><br/>",i="",h="changeValue",g="execute",f="Change me...",d="demobrowser.demo.data.SingleValueBinding",c="Reverse",b="Binding a number value (slider value) to a labels string (content).",a="child.names[last]",O="child.names[0]",N="First:",M="Bigger than 50!",L="of names.<br/>The reverse button reverses the current selected child ",K=" The default conversion will handle the type conversion.",J="Binding an data event, containing the change of the textfield to",I="Last:",H="Binding the textfield to a property with a validation. The validator",G=" in the object only accepts numbers. On a validation fail, a red ",F=" the label.",r="No number!",s="With validation",p="10",q="containing ['Bart', 'Lisa'] as names.",n=" above or below 50.",o="Bart",l="number",m="Toggle child",t="from a node containing ['Homer', 'Marge'] as names to a node ",u="Binding of node elements containing a node as child and an array ",x="With own conversion",w="names array.<br/>Toggle child will change the child of the root node ",z="with a self written converter to a string only saying it its ",y="Event binding",B="Array binding",A="Lower than 50!",v="Lisa",E="Binding a slider value to a labels text but conversion the number ",D="With default conversion",C=" border will surround the textfield.";
...
this.getRoot().add(new qx.ui.basic.Label(y),{top:10,left:10});
var bd=new qx.ui.form.TextField(f);
bd.setLiveUpdate(true);
this.getRoot().add(bd,{top:30,left:10});
var W=new qx.ui.basic.Label(f);
this.getRoot().add(W,{top:32,left:120});
bd.bind(h,W,k);
this.getRoot().add(new qx.ui.basic.Label(D),{top:60,left:10});
var bc=new qx.ui.form.Slider();
bc.setWidth(100);
this.getRoot().add(bc,{top:80,left:10});
var R=new qx.ui.basic.Label(i);
this.getRoot().add(R,{top:80,left:120});
bc.bind(k,R,k);
this.getRoot().add(new qx.ui.basic.Label(x),{top:110,left:10});
var T=new qx.ui.form.Slider();
T.setWidth(100);
this.getRoot().add(T,{top:130,left:10});
var bi=new qx.ui.basic.Label(i);
this.getRoot().add(bi,{top:130,left:120});
var X={converter:function(bl){if(bl<50){return A;
}else{return M;
}}};
T.bind(k,bi,k,X);
this.getRoot().add(new qx.ui.basic.Label(B),{top:180,left:10});
var bb=new demobrowser.demo.data.model.Node();
var be=new demobrowser.demo.data.model.Node();
var bh=new demobrowser.demo.data.model.Node();
bh.setNames(new qx.data.Array(o,v));
bb.setChild(be);
var bk=new qx.ui.form.Button(c);
this.getRoot().add(bk,{top:200,left:10});
bk.addListener(g,function(){bb.getChild().getNames().reverse();
},this);
var bg=new qx.ui.form.Button(m);
this.getRoot().add(bg,{top:200,left:80});
bg.addListener(g,function(){if(bb.getChild()==be){bb.setChild(bh);
}else{bb.setChild(be);
}},this);
this.getRoot().add(new qx.ui.basic.Label(N),{left:10,top:235});
this.getRoot().add(new qx.ui.basic.Label(I),{left:60,top:235});
var bj=new qx.ui.basic.Label();
this.getRoot().add(bj,{top:250,left:10});
var ba=new qx.ui.basic.Label();
this.getRoot().add(ba,{top:250,left:60});
bb.bind(O,bj,k);
bb.bind(a,ba,k);
this.getRoot().add(new qx.ui.basic.Label(s),{top:310,left:10});
var P=new qx.ui.form.TextField(p);
P.setLiveUpdate(true);
this.getRoot().add(P,{top:330,left:10});
var Y=new qx.ui.basic.Label(i);
this.getRoot().add(Y,{top:332,left:120});
var X={onUpdate:function(){Y.setValue(i);
P.setValid(true);
},onSetFail:function(e){Y.setValue(r);
P.setValid(false);
}};
P.bind(h,bb,l,X);
...
S.setWidth(350);
S.setValue(j+J+F);
this.getRoot().add(S,{left:250,top:10});
var V=new qx.ui.basic.Label();
V.setRich(true);
V.setWidth(350);
V.setValue(j+b+K);
this.getRoot().add(V,{left:250,top:60});
var Q=new qx.ui.basic.Label();
Q.setRich(true);
Q.setWidth(350);
Q.setValue(j+E+z+n);
this.getRoot().add(Q,{left:250,top:110});
var bf=new qx.ui.basic.Label();
bf.setRich(true);
bf.setWidth(350);
bf.setValue(j+u+L+w+t+q);
this.getRoot().add(bf,{left:250,top:180});
var U=new qx.ui.basic.Label();
U.setRich(true);
U.setWidth(350);
U.setValue(j+H+G+C);
this.getRoot().add(U,{left:250,top:310});
}}});
})();
...
(function(){var t="qx.data.Array",s="demobrowser.demo.data.model.Node",r="String",q="changeNames",p="changeName",o="Validation Error: ",n="- ",m="",l="changeChild",k="changeColor",d="Marge",j="AFFE",g="is no number (parseFloat says so!).",c="__Mk",b="Homer",f="\n",e="changeName2",h="changeChildren";
qx.Class.define(s,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.setChildren(new qx.data.Array());
},properties:{children:{check:t,event:h},child:{check:s,event:l,nullable:true},names:{check:t,event:q,init:new qx.data.Array(b,d)},name:{check:r,init:j,event:p},name2:{check:r,event:e},number:{init:10,validate:c},color:{event:k,nullable:true}},members:{__Mk:function(a){if(!isNaN(parseFloat(a))){if(/^\d+$/.test(a)){return;
}}throw new qx.core.ValidationError(o+a+g);
},toString:function(u){if(u==undefined){u=0;
}var v=m;

for(var i=0;i<u;i++){v+=n;
}v+=this.getName();

for(var i=0;i<this.getChildren().length;i++){v+=f+this.getChildren().getItem(i).toString(u+1);
}return v;
}}});
})();
