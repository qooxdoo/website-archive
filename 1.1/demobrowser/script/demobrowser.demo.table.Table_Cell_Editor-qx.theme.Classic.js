...
(function(){var G="checkbox",F='Property',E="options",D="password",C="type",B='email',A='password',z='options',y='type',x='0',bw='Use only numbers!',bv='regExp',bu='[0-9]+',bt='newsletter',bs='admin',br='editable combobox',bq='secret',bp="demobrowser.demo.table.Table_Cell_Editor",bo='status',bn="validationFunc",N='1',O="regExp",L="required",M='Doubleclick on cell to edit value',J='Newsletter',K='Inactive',H='User Name',I='telephone',R="dataChanged",S='Status',bb='Active',Y='validationFunc',bf='enter website',bd="editable",bj='role',bh='username',U='jdoe',bm='Telephone',bl='manager',bk='enter valid telephone number',T='Waiting',W='Email',X='Role',ba="Property editor table",bc='2',be='user',bg='website',bi='Password',P="email",Q='Website',V='enter valid email';
qx.Class.define(bp,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return ba;
...
},createTable:function(){var w=[[bh,H,U,{'required':true}],[A,bi,bq,{'type':A,'required':true}],[bj,X,br,{'options':[bs,bl,be],'editable':true}],[bo,S,x,{'options':[[K,null,x],[bb,null,N],[T,null,bc]]}],[B,W,V,{'type':B}],[I,bm,bk,{'regExp':bu,'failMsg':bw}],[bg,Q,bf,{'validationFunc':function(c){if(c.search(/^http:/)!=-1){window.open(c);
}return c;
}}],[bt,J,true,{'type':G}]];
var v=function(d){var j=d.table;
var g=j.getTableModel();
var h=g.getRowData(d.row);
var k=h[3];
var i;

for(var f in k){switch(f){case C:switch(k[y]){case G:return new qx.ui.table.cellrenderer.Boolean;
case D:return new qx.ui.table.cellrenderer.Password;
}break;
case E:var i=new qx.ui.table.cellrenderer.Replace;
var e={};
k[z].forEach(function(bx){if(bx instanceof Array){e[bx[0]]=bx[2];
}});
i.setReplaceMap(e);
i.addReversedReplaceMap();
return i;
}}return new qx.ui.table.cellrenderer.Default();
};
var u=new qx.ui.table.cellrenderer.Dynamic(v);
var s=function(by){var bD=by.table;
var bB=bD.getTableModel();
var bC=bB.getRowData(by.row);
var bF=bC[3];
var bA=new qx.ui.table.celleditor.TextField;
var bE=null;

for(var bz in bF){switch(bz){case E:if(bF.editable){bA=new qx.ui.table.celleditor.ComboBox();
}else{bA=new qx.ui.table.celleditor.SelectBox();
}bA.setListData(bF[z]);
break;
case bd:break;
case C:switch(bF[y]){case D:bA=new qx.ui.table.celleditor.PasswordField;
break;
case G:bA=new qx.ui.table.celleditor.CheckBox;
break;
case P:bA.setValidationFunction(function(bL,bM){var bN=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{2}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/;
...
case O:bA.setValidationFunction(function(l,m){var n=new RegExp(bF[bv]);

if(n.test(l)){return l;
}alert(bF['failMsg']);
return m;
});
break;
case bn:bA.setValidationFunction(bF[Y]);
break;
case L:bE=function(a,b){if(!a){alert("You need to supply a value here");
return b;
}return a;
};
break;
...
var r=new qx.ui.table.celleditor.Dynamic(s);
var p=new qx.ui.table.model.Simple();
p.setColumns([F,F,M]);
var o={tableColumnModel:function(bK){return new qx.ui.table.columnmodel.Resize(bK);
}};
var q=new qx.ui.table.Table(p,o);
q.setDecorator(null);
q.setColumnVisibilityButtonVisible(false);
q.setKeepFirstVisibleRowComplete(true);
q.setStatusBarVisible(false);
q.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var t=q.getTableColumnModel();
t.setColumnVisible(0,false);
t.getBehavior().setWidth(1,100);
t.getBehavior().setWidth(2,300);
p.setColumnEditable(2,true);
t.setDataCellRenderer(2,u);
t.setCellEditorFactory(2,r);
q.getTableModel().setData(w);
q.getTableModel().addListener(R,function(event){if(!(event instanceof qx.event.type.Data)){return;
...
(function(){var n="",m="row",k="Function",j="qx.ui.table.celleditor.SelectBox",h="number",g="table-editor-selectbox",f="appear",e="Array";
qx.Class.define(j,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,properties:{validationFunction:{check:k,nullable:true,init:null},listData:{check:e,init:null,nullable:true}},members:{createCellEditor:function(o){var r=new qx.ui.form.SelectBox().set({appearance:g});
var s=o.value;
r.originalValue=s;
...
}}var p=r.getChildrenContainer().findItem(n+s);

if(p){r.setSelection([p]);
}else{r.resetSelection();
}r.addListener(f,function(){r.open();
});
return r;
},getCellEditorValue:function(a){var b=a.getSelection();
var d=n;

if(b&&b[0]){d=b[0].getUserData(m)||b[0].getLabel();
}var c=this.getValidationFunction();

if(c){d=c(d,a.originalValue);
}
if(typeof a.originalValue==h){d=parseFloat(d);
}return d;
}}});
})();
...
(function(){var n="keypress",m="Enter",l="qx.ui.table.celleditor.CheckBox",k="focus",j="center",i="keydown",h="middle",g="activate";
qx.Class.define(l,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,members:{createCellEditor:function(c){var f=new qx.ui.container.Composite(new qx.ui.layout.HBox().set({alignX:j,alignY:h})).set({focusable:true});
var d=new qx.ui.form.CheckBox().set({value:c.value});
f.add(d);
f.addListener(k,function(){d.focus();
});
f.addListener(g,function(){d.activate();
});
d.addListener(i,function(e){if(e.getKeyIdentifier()==m){var a=qx.event.Pool.getInstance().getObject(qx.event.type.KeySequence);
var b=f.getContainerElement().getDomElement();
a.init(e.getNativeEvent(),b,e.getKeyIdentifier());
a.setType(n);
qx.event.Registration.dispatchEvent(b,a);
}},this);
return f;
...
qx.Class.define(d,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,construct:function(f){qx.core.Object.call(this);

if(f){this.setCellEditorFactoryFunction(f);
}},properties:{cellEditorFactoryFunction:{check:e,nullable:true,init:null}},members:{__Kb:null,createCellEditor:function(g){var h=this.getCellEditorFactoryFunction();
{};
this.__Kb=h(g);
var i=this.__Kb.createCellEditor(g);
return i;
},getCellEditorValue:function(a){var b=this.getCellEditorFactoryFunction();
{};
var c=this.__Kb.getCellEditorValue(a);
return c;
