...
var J=[j,p,u,d,n,C,w,h];

for(var i=0;i<J.length;i++){var K=document.getElementById(v+J[i]);

if(K.checked){qx.bom.Element.addListener(H,J[i],this.logMouseEvent,this);
}qx.bom.Element.addListener(K,k,this.__Mz,this);
}var I=document.getElementById(a);
I.checked=false;
qx.bom.Element.addListener(I,k,function(e){var D=e.getTarget().checked;

if(D){qx.bom.Element.capture(H);
}else{qx.bom.Element.releaseCapture(H);
}},this);
qx.bom.Element.addListener(H,q,function(e){I.checked=false;
},this);
},__Mz:function(e){var G=e.getTarget().id.split(x)[1];
var E=e.getTarget().checked;
var F=document.getElementById(l);

if(E){qx.bom.Element.addListener(F,G,this.logMouseEvent,this);
}else{qx.bom.Element.removeListener(F,G,this.logMouseEvent,this);
