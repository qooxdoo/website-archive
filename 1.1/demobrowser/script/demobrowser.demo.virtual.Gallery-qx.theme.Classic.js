...
(function(){var I="",H="selected",G="cell",F="layer",E="icon",D="#f3f3f3",C="label",B="Gallery (HTML - divs)",A="manager",z="default",bn="demobrowser.demo.virtual.Gallery",bm="demobrowser.demo.virtual.WidgetGallery",bl="text-align: center;",bk="auto",bj="color: white; background-color: #00398D;",bi="user-desktop.png",bh="<img src='",bg="'></img>",bf="group",be="resize",P="folder.png",Q="__NT",N="vertical",O="multi",L="network-server.png",M="user-trash.png",J="off",K="top",R="icon/32/places/",S="horizontal",V="demobrowser.demo.virtual.AbstractGallery",U="abstract",X="position: absolute;",W="demobrowser.demo.virtual.HtmlGallery",ba="Gallery (widgets)",Y="<br>",T="Icon #",bd="network-workgroup.png",bc="text-selected",bb="demobrowser.demo.virtual.GalleryCell";
qx.Class.define(bn,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);
new demobrowser.demo.virtual.WidgetGallery(ba);
var bG=new demobrowser.demo.virtual.HtmlGallery(B);
bG.moveTo(400,50);
}}});
qx.Class.define(V,{extend:qx.ui.window.Window,type:U,construct:function(bI){qx.ui.window.Window.call(this,bI);
...
bJ.set({scrollbarX:J,scrollbarY:bk});
bJ.getPane().addListener(be,this._onPaneResize,this);
this.add(bJ);
this.manager=new qx.ui.virtual.selection.CellRectangle(bJ.getPane(),this).set({mode:O,drag:true});
this.manager.attachMouseEvents();
this.manager.attachKeyEvents(bJ);
},members:{getItemData:function(bC,bD){return this.items[bC*this.itemPerLine+bD];
},_createScroller:function(){},isItemSelectable:function(bH){return !!this.getItemData(bH.row,bH.column);
},styleSelectable:function(bP,bQ,bR){},_onPaneResize:function(e){var x=e.getTarget();
...
},_generateItems:function(a){var d=[];
var h=[P,M,L,bd,bi];
var f=qx.util.AliasManager.getInstance();
var c=qx.util.ResourceManager.getInstance();

for(var i=0;i<a;i++){var j=R+h[Math.floor(Math.random()*h.length)];
var b=f.resolve(j);
var g=c.toUri(b);
d[i]={label:T+(i+1),icon:j,resolvedIcon:g};
}return d;
}},destruct:function(){this.items=null;
this._disposeObjects(A);
}});
qx.Class.define(bb,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var bx=new qx.ui.basic.Atom().set({iconPosition:K});
bx.getChildControl(C).set({padding:[0,4]});
bx.getChildControl(E).set({padding:4});
return bx;
},updateData:function(bo,bp){bo.set({icon:bp.icon,label:bp.label});
},updateStates:function(by,bz){var bB=by.getChildControl(C);
var bA=by.getChildControl(E);

if(bz.selected){bB.setDecorator(H);
bB.setTextColor(bc);
bA.setDecorator(bf);
}else{bB.resetDecorator();
bB.resetTextColor();
bA.resetDecorator();
}}}});
qx.Class.define(bm,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bF){demobrowser.demo.virtual.AbstractGallery.call(this,bF);
this.__NT=new demobrowser.demo.virtual.GalleryCell();
},members:{_createScroller:function(){var bE=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.WidgetCell(this);
bE.getPane().addLayer(this.layer);
new qx.ui.virtual.behavior.Prefetch(bE,0,0,0,0,200,300,600,800).set({interval:500});
return bE;
},styleSelectable:function(p,q,r){if(q!==H){return;
}var s=this.layer.getChildren();

for(var i=0;i<s.length;i++){var t=s[i];
var u=t.getUserData(G);

if(p.row!==u.row||p.column!==u.column){continue;
}
if(r){this.__NT.updateStates(t,{selected:1});
}else{this.__NT.updateStates(t,{});
}}},getCellWidget:function(br,bs){var bw=this.getItemData(br,bs);

if(!bw){return null;
}var bt={row:br,column:bs};
var bu={};

if(this.manager.isItemSelected(bt)){bu.selected=true;
}var bv=this.__NT.getCellWidget(bw,bu);
bv.setUserData(G,bt);
return bv;
},poolCellWidget:function(bq){this.__NT.pool(bq);
}},destruct:function(){this._disposeObjects(Q,F);
}});
qx.Class.define(W,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bN){demobrowser.demo.virtual.AbstractGallery.call(this,bN);
var bO=qx.theme.manager.Font.getInstance().resolve(z).getStyles();
...
var bS=new qx.ui.virtual.layer.GridLines(S,D);
bT.getPane().addLayer(bS);
var bS=new qx.ui.virtual.layer.GridLines(N,D);
bT.getPane().addLayer(bS);
return bT;
},_onPaneResize:function(e){demobrowser.demo.virtual.AbstractGallery.prototype._onPaneResize.call(this,e);
this.manager.clearSelection();
},styleSelectable:function(bK,bL,bM){this.layer.updateLayerData();
},getCellProperties:function(k,l){var o=this.getItemData(k,l);

if(!o){return I;
}var n=this.manager.isItemSelected({row:k,column:l});
var m=n?bj:I;
return {style:[X,bl,this._fontCss,m].join(I),content:[bh,o.resolvedIcon,bg,Y,o.label].join(I)};
