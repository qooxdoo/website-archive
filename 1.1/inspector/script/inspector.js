...
var settings = {"qx.application":"inspector.Application","qx.theme":"inspector.theme.Theme","qx.version":"1.1"};
...
var libinfo = {"__out__":{"sourceUri":"./script"},"inspector":{"resourceUri":"./resource","sourceUri":"./script","version":"trunk"},"qx":{"resourceUri":"./resource","sourceUri":"./script","version":"1.1"}};
...
(function(){var j="textfield",i="table",h="window",g="toolbar",f="toolbar-button",e="bold",d="inspector.theme.Appearance",c="green";
qx.Theme.define(d,{extend:qx.theme.modern.Appearance,appearances:{"toolbar-button-bold":{alias:f,include:f,style:function(b){return {textColor:c,font:e};
}},"objectsWindow":{alias:h,include:h,style:function(a){return {contentPadding:[0,0,0,0],showMinimize:false,showMaximize:false,width:300,height:200};
}},"objects2-toolbar":{alias:g,include:g,style:function(l){return {paddingLeft:3,paddingRight:3};
}},"objects2-textfield":{alias:j,include:j,style:function(k){return {marginRight:5};
}},"objects2-table":{alias:i,include:i,style:function(m){return {decorator:null};
}}}});
})();
...
(function(){var N="changeValue",M="console",L="_propertyWindow",K="_consoleWindow",J="_widgetsWindow",I="_consoleButton",H="Left",G="execute",F="_propertyButton",E="Height",bL="property",bK="widgets",bJ="url",bI="Open",bH="Top",bG="_widgetsButton",bF="Width",bE="close",bD="Objects",bC="resize",U="_toolbar",V="_reloadButton",S="true",T="Inspect widget",Q="_inspectButton",R="index.html",O="myToolbar",P="_objectsButton",Y="<tt>",ba="open",bi=" Can not access the javascript in the iframe!",bg="Properties",bq="load",bl="_selector",by="changeSelection",bv="_container",bc="middle",bB="_objectsWindow",bA="..",bz="",bb="qooxdoo Inspector",be="Lucida Grande",bf="move",bh="</tt>",bj="inspector.Application",bm="Widgets",bs="_selectedWidgetLabel",bx="inspector/images/icons/edit-find.png",W="objects",X="_iFrame",bd="toolbar-button-bold",bp="Console",bo="inspector/css/propertylisthtml.css",bn="inspector/css/domview.css",bu="inspector/css/consoleview.css",bt="hidden",bk="inspector/css/sourceview.css",br="_urlTextField",D="icon/16/actions/view-refresh.png",bw="minimized";
qx.Class.define(bj,{extend:qx.application.Standalone,construct:function(){qx.application.Standalone.call(this);
qx.bom.Stylesheet.includeFile(bn);
qx.bom.Stylesheet.includeFile(bu);
qx.bom.Stylesheet.includeFile(bk);
qx.bom.Stylesheet.includeFile(bo);
},members:{__Oo:null,_toolbar:null,_objectsButton:null,_widgetsButton:null,_consoleButton:null,_propertyButton:null,_inspectButton:null,_selectedWidgetLabel:null,_urlTextField:null,_reloadButton:null,_objectsWindow:null,_widgetsWindow:null,_consoleWindow:null,_propertyWindow:null,_container:null,_iFrame:null,_loading:null,_selector:null,_loadedWindow:null,__Op:null,__Oq:null,main:function(){qx.application.Standalone.prototype.main.call(this);
{};
this.__Op=new inspector.components.InspectorModel(this);
this.__Oq=new inspector.components.State();
this._container=new qx.ui.container.Composite(new qx.ui.layout.VBox());
this.getRoot().add(this._container,{edge:0});
this.__Ox();
this.__OA(false);
var x=bA;
var w=qx.bom.Cookie.get(bJ);

if(w==undefined||w==bz){w=x;
}
if(window.qxinspector!=undefined&&qxinspector.local){this._urlTextField.setVisibility(bt);
x=R;
}else{x=w;
}this._loading=true;
...
this._iFrame.addListener(bq,this.__Or,this);
this._urlTextField.setValue(x);
},__Or:function(){this.__Oo=0;
this.__Os();
var bQ=this._iFrame.getSource();

try{bQ=this._iFrame.getWindow().location.pathname;
}catch(bR){}
if(window.qxinspector==undefined){this._urlTextField.setValue(bQ);
}qx.bom.Cookie.set(bJ,bQ,7);
},__Os:function(){this._loadedWindow=this._iFrame.getContentElement().getWindow();
this.__OA(false);
if(!this.__Ow()){return;
}this.__OA(true);
...
}this._selector.addListener(by,this._changeSelection,this);
this._selector.setSelection(this._loadedWindow.qx.core.Init.getApplication());
this._loading=false;
this.__Ot();
this.__Op.setObjectRegistry(this._loadedWindow.qx.core.ObjectRegistry);
this.select(this._loadedWindow.qx.core.Init.getApplication().getRoot());
this.__Oq.restoreState();
this.__Ov();
},__Ot:function(){if(this._objectsWindow!=null&&this._objectsWindow.isVisible()){this._objectsWindow.load(this._loadedWindow);
}if(this._widgetsWindow!=null&&this._widgetsWindow.isVisible()){this._widgetsWindow.load(this._loadedWindow);
}},__Ou:function(n,o,name){var r=qx.bom.Cookie.get(name+bI);

if(r==S||r===null){o.setValue(true);
var top=parseInt(qx.bom.Cookie.get(name+bH));
var s=parseInt(qx.bom.Cookie.get(name+H));

if(!isNaN(top)&&!isNaN(s)){this[n].moveTo(s,top);
}var p=parseInt(qx.bom.Cookie.get(name+bF));
var q=parseInt(qx.bom.Cookie.get(name+E));

if(!isNaN(q)){this[n].setHeight(q);
}
if(!isNaN(p)){this[n].setWidth(p);
}}},__Ov:function(){this.__Ou(J,this._widgetsButton,bK);
this.__Ou(K,this._consoleButton,M);
this.__Ou(L,this._propertyButton,bL);
},__Ow:function(){try{this.__Oo++;
if(this.__Oo>20){throw new Error("qooxdoo not found!");
}
try{this._loadedWindow.qx.core.Init.getApplication().getRoot();
return true;
}catch(bM){qx.event.Timer.once(this.__Os,this,500);
throw new Error("qooxdoo isn't ready at the moment!");
}}catch(bN){this._selectedWidgetLabel.setValue(bi);
return false;
}},__Ox:function(){this._toolbar=new qx.ui.toolbar.ToolBar();
this._toolbar.setDecorator(O);
this._toolbar._getLayout().setAlignY(bc);
this._container.add(this._toolbar);
var m=new qx.ui.basic.Label(bb);
m.setPaddingLeft(10);
m.setPaddingRight(5);
var l=new qx.bom.Font(12,[be]);
l.setBold(true);
l.setItalic(true);
m.setFont(l);
this._toolbar.add(m);
this._toolbar.add(new qx.ui.toolbar.Separator());
var k=new inspector.objects2.Window(bD,this.__Op);
this.__Oq.add(k,W);
this._objectsButton=new qx.ui.toolbar.CheckBox(bD);
this._toolbar.add(this._objectsButton);
this._objectsButton.addListener(N,function(e){e.getData()?k.open():k.close();
},this);
k.addListener(ba,function(e){this._objectsButton.setValue(true);
},this);
k.addListener(bE,function(e){this._objectsButton.setValue(false);
},this);
this.__Oy(bG,bm,J,inspector.widgets.WidgetsWindow,bK,function(){this._widgetsWindow.load();
});
this.__Oy(F,bg,L,inspector.property.PropertyWindow,bL,function(){this._propertyWindow.select(this._selector.getSelection());
});
this.__Oy(I,bp,K,inspector.console.ConsoleWindow,M,function(){});
this._toolbar.add(new qx.ui.toolbar.Separator());
this._inspectButton=new qx.ui.toolbar.CheckBox(T,bx);
this._inspectButton.setAppearance(bd);
...
this._reloadButton=new qx.ui.toolbar.Button(null,D);
this._toolbar.add(this._reloadButton);
this._reloadButton.addListener(G,this._reloadIframe,this);
},_reloadIframe:function(e){this._loading=true;
var z=this._iFrame.getSource();

try{z=this._iFrame.getWindow().location.pathname;
}catch(y){}
if(z!=this._urlTextField.getValue()){this._iFrame.setSource(this._urlTextField.getValue());
}else{if(e.getType()==G){if(this._iFrame.getSource!=z){this._iFrame.setSource(z);
}else{this._iFrame.reload();
}}}},__Oy:function(a,b,c,d,name,f){this[a]=new qx.ui.toolbar.CheckBox(b);
this._toolbar.add(this[a]);
var g=false;
this[a].addListener(N,function(e){if(!g){this[c]=new d();
this.getRoot().add(this[c]);
this[c].setInitSizeAndPosition();
this.__Oz(this[c],this[a],name);
}e.getData()?this[c].open():this[c].close();
f.call(this);
g=true;
qx.bom.Cookie.set(name+bI,e.getData(),7);
},this);
},__Oz:function(bO,bP,name){bO.addListener(bE,function(){bP.setValue(false);
},this);
bO.addListener(bf,function(e){qx.bom.Cookie.set(name+H,e.getData().left,7);
qx.bom.Cookie.set(name+bH,e.getData().top,7);
},this);
bO.addListener(bC,function(e){qx.bom.Cookie.set(name+bF,e.getData().width,7);
qx.bom.Cookie.set(name+E,e.getData().height,7);
},this);
},__OA:function(A){this._objectsButton.setEnabled(A);
...
},setWidgetByHash:function(t,u){if(u==M){u=this._consoleWindow;
this._consoleWindow.goToDefaultView();
}var v=this._loadedWindow.qx.core.ObjectRegistry.fromHashCode(t);
this.select(v,u);
},inspectObjectByDomSelecet:function(B,C){if(this._consoleWindow!=null){this._consoleWindow.inspectObjectByDomSelecet(B,C);
}},inspectObjectByInternalId:function(h){if(this._consoleWindow!=null){this._consoleWindow.inspectObjectByInternalId(h);
}},select:function(i,j){if(this._loading||!i){return;
}this._selectedWidgetLabel.setValue(Y+i.toString()+bh);

if(j!=this._selector){if(i!==this._selector.getSelection()){this._selector.setSelection(i);
}}
if(this._objectsWindow!=null&&j!=this._objectsWindow){if(i!=this._objectsWindow.getSelection()){this._objectsWindow.select(i,true);
}}
if(this._widgetsWindow!=null&&j!=this._widgetsWindow){if(i!=this._widgetsWindow.getSelection()){this._widgetsWindow.select(i,true);
}}
if(this._propertyWindow!=null&&j!=this._propertyWindow){if(i!=this._propertyWindow.getSelection()&&this._propertyWindow.getMode()!=bw){this._propertyWindow.select(i,true);
}}this.__Op.setInspected(i);
this._selector.highlightFor(i,1000);
...
this._disposeObjects(bv,U,P,bG,F,I,Q,bs,br,V,X,bl,bB,J,K,L);
}});
})();
...
(function(){var e="inspector.components.IInspectorModel",d="qx.event.type.Event",c="qx.event.type.Data";
qx.Interface.define(e,{events:{"changeObjects":d,"changeInspected":c},members:{getObjectRegistry:function(){return true;
},setObjectRegistry:function(b){return arguments.length==1;
},getApplication:function(){return true;
},setApplication:function(f){return arguments.length==1;
},getObjects:function(){return true;
},getInspected:function(){return true;
},setInspected:function(a){return arguments.length==1;
}}});
})();
...
qx.Class.define(c,{extend:qx.core.Object,implement:[inspector.components.IInspectorModel],construct:function(o){qx.core.Object.call(this);
this.__OB=o;
},events:{"changeObjects":d,"changeInspected":b},members:{__OC:null,__OB:null,__OD:null,getObjectRegistry:function(){return this.__OC;
},setObjectRegistry:function(g){if(this.__OC!==g){this.__OC=g;
this.fireEvent(f);
}},getApplication:function(){return this.__OB;
},setApplication:function(a){if(this.__OB!==a){this.__OB=a;
this.fireEvent(f);
}},getObjects:function(){var k=[];

if(this.__OC===null||this.__OB===null){return k;
}var j=this.__OC.getRegistry();
var h=this.__OB.getExcludes();

for(var i in j){var l=j[i];

if(!qx.lang.Array.contains(h,l)){k.push(l);
}}return k;
},getInspected:function(){return this.__OD;
},setInspected:function(m){if(this.__OD!==m){var n=this.__OD;
this.__OD=m;
this.__OB.select(m);
this.fireDataEvent(e,this.__OD,n);
}}},destruct:function(){this.__OC=this.constructor=this.__OD=null;
}});
})();
(function(){var b="inspector.components.State",a="cookieKey";
qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__OE=[];
},members:{__OE:null,add:function(c,d){if(!qx.lang.Array.contains(this.__OE,c)){c.setUserData(a,d);
this.__OE.push(c);
this.__OF(c);
}},__OF:function(e){var f=e.getUserData("cookieKey");
...
},restoreState:function(){for(var i=0;i<this.__OE.length;i++){this.__OG(this.__OE[i]);
}},__OG:function(g){var h=g.getUserData("cookieKey");
...
qx.Class.define(o,{extend:qx.core.Object,construct:function(N){qx.core.Object.call(this);
this.setJSWindow(N);
},properties:{selection:{event:k,nullable:true}},members:{setJSWindow:function(D){this._iFrameWindow=D;
this._addedWidgets=[];
...
},highlightFor:function(a,b){if(a.classname==n){return;
}this._highlight(a);
...
}}catch(c){}if(w.getContainerElement){var z=w.getContainerElement().getDomElement();
...
},_getCoordinates:function(d){if(d==null){return null;
}var f={};
f.left=qx.bom.element.Location.getLeft(d);
f.right=qx.bom.element.Location.getRight(d);
f.top=qx.bom.element.Location.getTop(d);
f.bottom=qx.bom.element.Location.getBottom(d);
return f;
...
(function(){var g="open",f="objectsWindow",e="qx.event.type.Event",d="__OH",c="__OI",b="inspector.objects2.Window";
qx.Class.define(b,{extend:qx.ui.window.Window,construct:function(name,j){qx.ui.window.Window.call(this,name);
this.setLayout(new qx.ui.layout.Canvas());
this.setAppearance(f);
this.syncAppearance();
this.setInitSizeAndPosition();
this.__OH=new inspector.objects2.Model(j);
this.__OI=new inspector.objects2.Controller(this.__OH);
this.add(this.__OI.getView(),{edge:0});
},events:{"open":e},members:{__OH:null,__OI:null,setSizeAndPosition:function(a){this.moveTo(a.left,a.top);
this.setWidth(a.width);
this.setHeight(a.height);
},setInitSizeAndPosition:function(){var i=parseInt(qx.bom.Viewport.getWidth()-this.getWidth());
...
this.__OJ=f;
f.addListener(e,this.__OK,this);
f.addListener(d,this.__OL,this);
},events:{"changeObjects":c,"changeInspected":a},members:{__OJ:null,getObjects:function(){return this.__OJ.getObjects();
},getObjectFromHashCode:function(h){var i=this.__OJ.getObjectRegistry();
return i.fromHashCode(h);
},getInspected:function(){return this.__OJ.getInspected();
},setInspected:function(g){this.__OJ.setInspected(g);
},__OK:function(event){this.fireEvent("changeObjects");
},__OL:function(event){this.fireDataEvent("changeInspected",event.getData(),event.getOldData());
}},destruct:function(){this.__OJ.removeListener(e,this.__OK,this);
this.__OJ.removeListener(d,this.__OL,this);
this.__OJ=null;
}});
})();
(function(){var f="changeInspected",e="changeObjects",d="inspector.objects2.Controller",c="appear";
qx.Class.define(d,{extend:qx.core.Object,construct:function(i){qx.core.Object.call(this);
{};
this.__OM=i;
this.__OM.addListener(f,this.__OV,this);
this.__OM.addListener(e,this.__OW,this);
this.__ON=new inspector.objects2.View(this);
this.__OO=qx.util.TimerManager.getInstance();
this.__ON.addListenerOnce(c,function(event){this.__ON.selectObject(this.__OM.getInspected());
},this);
this.__OT();
},members:{__ON:null,__OM:null,__OP:null,__OQ:null,__OO:null,__OR:null,__OS:false,getView:function(){return this.__ON;
},reload:function(){this.__OS=true;
this.__OY().reload();
this.__OX().reload();
this.__OS=false;
this.__ON.selectObject(this.__OM.getInspected());
},showByHash:function(){this.__ON.setByHashActive(true);
this.__ON.setByCountActive(false);
this.__ON.setTableModel(this.__OX());
this.__ON.setTableSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
this.__ON.selectObject(this.__OM.getInspected());
},showByCount:function(){this.__ON.setByHashActive(false);
this.__ON.setByCountActive(true);
this.__ON.setTableModel(this.__OY());
this.__ON.setTableSelectionMode(qx.ui.table.selection.Model.NO_SELECTION);
this.__ON.selectObject(null);
},setFilter:function(a){this.__OS=true;
this.__OU();
this.__OR=this.__OO.start(function(){this.__OR=null;
this.__OY().filter(a);
this.__OX().filter(a);
this.__OS=false;
this.__ON.selectObject(this.__OM.getInspected());
},0,this,null,200);
},inspect:function(g){if(g==null){this.__ON.selectObject(this.__OM.getInspected());
return;
}var h=this.__OM.getObjectFromHashCode(g);

if(h!=null&&!this.__OS){this.__OM.setInspected(h);
}},__OT:function(){this.__ON.setFilter("");
this.showByHash();
},__OU:function(){if(this.__OR!=null){this.__OO.stop(this.__OR);
this.__OR=null;
}},__OV:function(event){var b=event.getData();
this.__ON.selectObject(b);
},__OW:function(event){this.reload();
},__OX:function(){if(this.__OP==null){this.__OP=new inspector.objects2.table.HashModel(this.__OM);
}return this.__OP;
},__OY:function(){if(this.__OQ==null){this.__OQ=new inspector.objects2.table.CountModel(this.__OM);
}return this.__OQ;
}},destruct:function(){this.__OU();

if(this.__OP!=null){this.__OP.dispose();
}
if(this.__OQ){this.__OQ.dispose();
}this.__ON.destroy();
this.__ON=this.__OM=this.__OP=this.__OR;
this.__OQ=this.__OO=this.__OS=null;
}});
})();
(function(){var r="execute",q="show",p="count",o="hash",n="Show the objects by there hash value.",m="by Count",l="Reloads the view.",k="changeValue",j="objects2-textfield",h="Filter...",H="Select a item to inspect it or sort the table.",G="20%",F="by Hash",E="objects2-toolbar",D="Enter a case sensitive value to filter the table.",C="Show the objects by there count.",B="__Pb",A="changeSelection",z="middle",y="",w="icon/22/actions/view-refresh.png",x="__Pd",u="inspector.objects2.View",v="objects2-table",s="__Pc",t="80%";
qx.Class.define(u,{extend:qx.ui.container.Composite,construct:function(J){qx.ui.container.Composite.call(this);
{};
this.__Pa=J;
this.__Pf();
},members:{__Pb:null,__Pc:null,__Pd:null,__Pe:null,__Pa:null,setByHashActive:function(P){{};
this.__Pb.setValue(P);
},setByCountActive:function(O){{};
this.__Pc.setValue(O);
},setTableModel:function(I){{};
this.__Pd.setTableModel(I);
this.__Pg();
},setTableSelectionMode:function(g){{};
this.__Pd.getSelectionModel().setSelectionMode(g);
},selectObject:function(a){var c=this.__Pd.getSelectionModel();
c.resetSelection();

if(a!=null&&c.getSelectionMode()!=qx.ui.table.selection.Model.NO_SELECTION){var b=this.__Pd.getTableModel().getData();

for(var i=0;i<b.length;i++){if(b[i][0]==a.toHashCode()){c.setSelectionInterval(i,i);
this.__Pd.scrollCellVisible(0,i);
return;
}}}else{this.__Pd.scrollCellVisible(0,0);
}},setFilter:function(S){this.__Pe.setValue(S);
},__Pf:function(){this.setLayout(new qx.ui.layout.VBox());
var toolbar=new qx.ui.toolbar.ToolBar();
toolbar.setAppearance(E);
toolbar._getLayout().setAlignY(z);
this.add(toolbar);
var Q=new qx.ui.toolbar.Button(null,w);
Q.setToolTipText(l);
Q.addListener(r,this._onReaload,this);
toolbar.add(Q);
toolbar.addSeparator();
this.__Pb=new qx.ui.toolbar.RadioButton(F);
this.__Pb.setUserData(q,o);
this.__Pb.addListener(r,this._onShowByChanged,this);
this.__Pb.setToolTipText(n);
toolbar.add(this.__Pb);
this.__Pc=new qx.ui.toolbar.RadioButton(m);
this.__Pc.setUserData(q,p);
this.__Pc.addListener(r,this._onShowByChanged,this);
this.__Pc.setToolTipText(C);
toolbar.add(this.__Pc);
toolbar.addSpacer();
this.__Pe=new qx.ui.form.TextField().set({appearance:j,liveUpdate:true,placeholder:h,toolTipText:D});
this.__Pe.addListener(k,this._onFilterChanged,this);
toolbar.add(this.__Pe);
var R={tableColumnModel:function(d){return new qx.ui.table.columnmodel.Resize(d);
}};
this.__Pd=new qx.ui.table.Table(null,R).set({appearance:v,columnVisibilityButtonVisible:false,toolTipText:H});
this.__Pd.getSelectionModel().addListener(A,this._onModelChangeSelection,this);
this.__Pd.getDataRowRenderer().setHighlightFocusRow(false);
this.add(this.__Pd,{flex:1});
},_onReaload:function(event){this.__Pa.reload();
},_onShowByChanged:function(event){var e=event.getTarget();
var f=e?e.getUserData(q):y;

switch(f){case o:this.__Pa.showByHash();
break;
case p:this.__Pa.showByCount();
break;
default:this.error("Method: '"+f+"' doesn't exist.");
break;
}},_onFilterChanged:function(event){this.__Pa.setFilter(event.getData());
},_onModelChangeSelection:function(event){var M=this.__Pd.getSelectionModel();

if(M.isSelectionEmpty()){this.__Pa.inspect(null);
return;
}var K=M.getSelectedRanges()[0].minIndex;
var L=this.__Pd.getTableModel().getData()[K];
this.__Pa.inspect(L[0]);
},__Pg:function(){var N=this.__Pd.getTableColumnModel().getBehavior();
N.setWidth(0,G);
N.setMinWidth(0,50);
N.setMaxWidth(0,100);
N.setWidth(1,t);
...
qx.Class.define(c,{extend:qx.core.Object,type:a,statics:{__CJ:[],__CK:{},__CL:0},members:{start:function(q,r,s,t,u){if(!u){u=r||0;
}var v=(new Date()).getTime()+u;
this.self(arguments).__CK[++this.self(arguments).__CL]={callback:q,userData:t||null,expireAt:v,recurTime:r,context:s||this};
this.__CM(v,this.self(arguments).__CL);
return this.self(arguments).__CL;
},stop:function(k){var l=this.self(arguments).__CJ;
var length=l.length;

for(var i=0;i<length;i++){if(l[i]==k){l.splice(i,1);
break;
}}delete this.self(arguments).__CK[k];
if(l.length==0){qx.event.Idle.getInstance().removeListener(b,this.__CN);
}},__CM:function(m,n){var p=this.self(arguments).__CJ;
var o=this.self(arguments).__CK;
var length=p.length;

for(var i=0;i<length;i++){if(o[p[i]].expireAt>m){p.splice(i,0,n);
break;
}}if(p.length==length){p.push(n);
}if(p.length==1){qx.event.Idle.getInstance().addListener("interval",this.__CN,this);
}},__CN:function(){var g=(new Date()).getTime();
var e=this.self(arguments).__CJ;
var f=this.self(arguments).__CK;
...
this.__CM(d.expireAt,j);
}else{delete f[j];
}}if(e.length==0){qx.event.Idle.getInstance().removeListener("interval",this.__CN);
}}}});
})();
(function(){var G='g',F="==",E=">",D="notregex",C="between",B="<",A="regex",z='gi',y="!between",x=">=",u="dataChanged",w="!=",v="<=",t="qx.ui.table.model.Filtered",s='';
qx.Class.define(t,{extend:qx.ui.table.model.Simple,construct:function(){qx.ui.table.model.Simple.call(this);
this.numericAllowed=new Array(F,w,E,B,x,v);
this.betweenAllowed=new Array(C,y);
this.__Es=false;
this.Filters=new Array();
},members:{__Et:null,__Es:null,_js_in_array:function(H,I){var J=I.toString();

if(J==s){return false;
}var L=new RegExp(H,G);
var K=L.test(I);
return K;
},addBetweenFilter:function(n,o,p,q){if(this._js_in_array(n,this.betweenAllowed)&&q!=null){if(o!=null&&p!=null){var r=new Array(n,o,p,q);
}}
if(r!=null){this.Filters.push(r);
}else{throw new Error("Filter not recognized or value1/value2 is null!");
}},addNumericFilter:function(M,N,O){var P=null;

if(this._js_in_array(M,this.numericAllowed)&&O!=null){if(N!=null){P=[M,N,O];
}}
if(P!=null){this.Filters.push(P);
}else{throw new Error("Filter not recognized: value or target is null!");
}},addRegex:function(c,d,e){var g;

if(e){g=z;
}else{g=G;
}
if(c!=null&&d!=null){var f=new Array(A,c,d,g);
}
if(f!=null){this.Filters.push(f);
}else{throw new Error("regex cannot be null!");
}},addNotRegex:function(h,j,k){var m;

if(k){m=z;
}else{m=G;
}
if(h!=null&&j!=null){var l=new Array(D,h,j,m);
}
...
switch(this.Filters[i][0]){case F:if(V==this.Filters[i][1]){ba=true;
}break;
case w:if(V!=this.Filters[i][1]){ba=true;
}break;
case E:if(V>this.Filters[i][1]){ba=true;
}break;
case B:if(V<this.Filters[i][1]){ba=true;
}break;
case x:if(V>=this.Filters[i][1]){ba=true;
}break;
case v:if(V<=this.Filters[i][1]){ba=true;
}break;
}}else if(this._js_in_array(this.Filters[i][0],this.betweenAllowed)&&ba==false){V=this.getValueById(this.Filters[i][3],bc);

switch(this.Filters[i][0]){case C:if(V>=this.Filters[i][1]&&V<=this.Filters[i][2]){ba=true;
}break;
case y:if(V<this.Filters[i][1]&&V>this.Filters[i][2]){ba=true;
}break;
}}else if(this.Filters[i][0]==A&&ba==false){V=this.getValueById(this.Filters[i][2],bc);
var X=new RegExp(this.Filters[i][1],this.Filters[i][3]);
ba=X.test(V);
}else if(this.Filters[i][0]==D&&ba==false){V=this.getValueById(this.Filters[i][2],bc);
var X=new RegExp(this.Filters[i][1],this.Filters[i][3]);
ba=!X.test(V);
...
},hideRows:function(Q,R,dispatchEvent){var T=this.getData();
dispatchEvent=(dispatchEvent!=null?dispatchEvent:true);

if(!this.__Es){this.__Et=T.slice(0);
this.__Es=true;
}
if(R==null||R<1){R=1;
}
for(var S=Q;S<(T.length-R);S++){T[S]=T[S+R];
}this.removeRows(S,R);
if(dispatchEvent){var U={firstRow:0,lastRow:T.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(u,U);
}},resetHiddenRows:function(){if(!this.__Et){return ;
}this.Filters=[];
this.setData(qx.lang.Array.clone(this.__Et));
},setData:function(a,b){this.__Et=qx.lang.Array.clone(a);
this.Filters=[];
qx.ui.table.model.Simple.prototype.setData.call(this,a,b);
}},destruct:function(){this.__Et=this.numericAllowed=this.betweenAllowed=this.Filters=null;
}});
})();
(function(){var f="",e="Classname",d="inspector.objects2.table.AbstractModel";
qx.Class.define(d,{extend:qx.ui.table.model.Filtered,construct:function(a,b){qx.ui.table.model.Filtered.call(this);
this._model=a;
this.setColumns(b);
this.setData(this._getData());
},members:{__Ph:f,_model:null,_getData:function(){throw Error("Abstract Method call!");
},reload:function(){this.setData(this._getData());
this.filter(this.__Ph);
},filter:function(c){this.__Ph=c;
this.resetHiddenRows();

if(c!=f){this.addNotRegex(c,e,true);
this.applyFilters();
...
(function(){var B="instance",A="]",z='id',y=" [",x='instance',v="changeOpen",u="_getChildren",t="getChildren",s="inspector/images/components/image.png",r="qx.ui.form.TextField",bK="qx.ui.layout.DockLayout",bJ="inspector/images/components/radiobutton.png",bI="_structureToggle",bH="qx.ui.pageview.buttonview.Button",bG="inspector/images/components/verticallayout.png",bF="qx.ui.groupbox.RadioGroupBox",bE="inspector/images/components/horizontallayout.png",bD="inspector/images/components/label.png",bC="qx.ui.groupbox.CheckGroupBox",bB="qx.ui.treevirtual.TreeVirtual",I="qx.ui.basic.Image",J="qx.ui.basic.Label",G="qx.ui.pageview.tabview.Button",H="inspector/images/components/toolbar.png",E="qx.ui.toolbar.ToolBar",F="inspector/images/components/window.png",C="_reloadButton",D="_tree",Q="inspector/images/components/button.png",R="inspector/images/components/textarea.png",bf="qx.ui.form.RadioButton",bb="qx.ui.window.Window",bn="changeSelection",bi="inspector/images/components/table.png",bx="qx.ui.tree.Tree",bs="qx.ui.layout.HBox",V="qx.ui.form.ComboBox",bA="inspector/images/components/textfield.png",bz="inspector/images/components/groupbox.png",by="icon/22/actions/view-refresh.png",U="inspector/images/components/checkbox.png",X="qx.ui.form.CheckBox",ba="Display internal widget structure.",bd="qx.ui.table.Table",bg="Reload the window.",bj="qx.ui.layout.VBox",bp="qx.ui.menu.Layout",bu="inspector/images/components/atom.png",K="qx.ui.toolbar.Button",L="qx.ui.menu.Button",W="qx.ui.layout.CanvasLayout",bm="inspector/images/components/spinner.png",bl="qx.ui.layout.FlowLayout",bk="qx.ui.menu.Menu",br="Widgets",bq="qx.ui.form.Button",bh="click",bo="inspector.widgets.WidgetsWindow",o="inspector/images/components/tree.png",bt="qx.ui.layout.GridLayout",M="qx.ui.pageview.radioview.Button",N="qx.ui.form.TextArea",bc="qx.ui.groupbox.Groupbox",p="qx.ui.menubar.Button",q="inspector/images/components/menu.png",T="qx.ui.menubar.MenuBar",O="inspector/images/components/layout.png",P="qx.ui.splitpane.SplitPane",S="inspector/images/components/combobox.png",be="qx.ui.basic.Atom",bw="inspector/images/components/splitpane.png",bv="icon/22/actions/document-properties.png",Y="qx.ui.form.Spinner";
qx.Class.define(bo,{extend:inspector.components.AbstractWindow,construct:function(){inspector.components.AbstractWindow.call(this,br);
this._reloadButton=new qx.ui.toolbar.Button(null,by);
this._reloadButton.setToolTipText(bg);
this._toolbar.add(this._reloadButton);
this._reloadButton.addListener(bh,function(){this.load();
},this);
this._toolbar.addSpacer();
this._structureToggle=new qx.ui.toolbar.CheckBox(null,bv);
this._structureToggle.setToolTipText(ba);
...
this._tree.addListener(bn,function(e){if(e.getData()[0]){var cf=e.getData()[0].getUserData(B);
...
this.setHeight(bV);
},select:function(ce){this._selectWidgetInTheTree(ce);
},getSelection:function(){var a=this._tree.getSelection()[0];
if(a!=null){return a.getUserData(B);
}return null;
},load:function(c){if(c==undefined){this._iFrameWindow=qx.core.Init.getApplication().getIframeWindowObject();
}else{this._iFrameWindow=c;
}var d=this._iFrameWindow.qx.core.Init.getApplication().getRoot();
var f=new qx.ui.tree.TreeFolder(d.classname+y+d.toHashCode()+A);
f.setUserData(B,d);
this._tree.setRoot(f);
this._fillTree(d,f,2);
},_fillTree:function(bL,bM,bN){var bS=bM.getItems(false,true);
var bO=this._structureToggle.isValue()?u:t;
if(bL[bO]==undefined){if(bO===t){bO=u;

if(bL[bO]==undefined){return;
}}else{return;
}}if(bL[bO]().length==0){if(bS.length>1){for(var m=0;m<bS.length;m++){if(bS[m+1]==this._tree.getSelection()[0]){this._tree.resetSelection();
}bM.removeAt(0);
}}}var bT=qx.core.Init.getApplication().getExcludes();
var i=0;
bN--;
for(var k=0;k<bL[bO]().length;k++){var bQ=bL[bO]()[k];
var bP=false;

for(var j=0;j<bT.length;j++){if(bQ==bT[j]){bP=true;
break;
}}if(bP){continue;
}if(bS[i]==null){var bU=new qx.ui.tree.TreeFolder(bQ.classname+y+bQ.toHashCode()+A);
bU.setIcon(this._getIconPath(bQ.classname));
bM.addAt(bU,i);
bU.setUserData(x,bQ);
bU.setUserData(z,bQ.toHashCode());
bU.addListener(v,this._treeOpenHandler,this);
}else{if(bS[i].getLabel()==bQ.classname+y+bQ.toHashCode()+A){var bU=bS[i];
}else{if(bM.getItems()[i]!=null){if(bM.getItems()[i]==this._tree.getSelection()[0]){this._tree.resetSelection();
}}bM.removeAt(i);
var bU=new qx.ui.tree.TreeFolder(bQ.classname+y+bQ.toHashCode()+A);
bM.addAt(bU,i);
bU.setUserData(x,bQ);
bU.setUserData(z,bQ.toHashCode());
bU.addListener(v,this._treeOpenHandler,this);
}}if(bN>0){this._fillTree(bQ,bU,bN);
}if(i+1==bL[bO]().length){var bR=bM.getItems(false,true);
if(bR.length-2!=i){for(var l=i+1;l<bR.length;l++){bM.removeAt(i+1);
}}}i++;
}},_treeOpenHandler:function(e){if(e.getData()){var b=e.getTarget().getUserData(x);
this._fillTree(b,e.getTarget(),2);
}},_getIconPath:function(cd){switch(cd){case I:return s;
case J:return bD;
case be:return bu;
case bx:case bB:return o;
case bk:return q;
case bq:case L:case p:case bH:case M:case G:case K:return Q;
case W:case bK:case bl:case bt:case bp:return O;
case bj:return bG;
case bs:return bE;
case E:case T:return H;
case bb:return F;
case bc:case bC:case bF:return bz;
case Y:return bm;
case V:return S;
case r:return bA;
case N:return R;
...
case bf:return bJ;
default:return null;
}},_selectWidgetInTheTree:function(bX){this._iFrameWindow=qx.core.Init.getApplication().getIframeWindowObject();
if(!(bX instanceof this._iFrameWindow.qx.ui.core.Widget)){return;
}var ca=[];
var w=bX;

if(this._structureToggle.isValue()){while(w.getLayoutParent()!=null){ca.push(w);
w=w.getLayoutParent();
}}else{while(w.getParent!=undefined&&w.getParent()!=null||w.getLayoutParent()!=null){if(w.getParent!=undefined&&w.getParent()!=null){ca.push(w);
w=w.getParent();
}else if(w.getLayoutParent()!=null){ca.push(w);
w=w.getLayoutParent();
}}}for(var i=ca.length-1;i>0;i--){this._openFolder(ca[i]);
}this._tree.getRoot().setOpen(true);
var cc=bX.toHashCode();
var bY=this._tree.getItems(true,true);
var cb=false;
if(this._tree.getRoot().getUserData(B).toHashCode()==cc){this._tree.resetSelection();
this._tree.addToSelection(this._tree.getRoot());
return;
}for(var i=0;i<bY.length;i++){if(bY[i].getUserData(z)==cc){cb=true;
this._tree.resetSelection();
this._tree.addToSelection(bY[i]);
break;
}}if(!cb){this._tree.resetSelection();
}},_openFolder:function(g){var n=g.toHashCode();
var h=this._tree.getItems(true,true);
if(this._tree.getRoot().getUserData(B).toHashCode()==n){this._tree.resetSelection();
this._tree.addToSelection(this._tree.getRoot());
return ;
}for(var i=0;i<h.length;i++){if(h[i].getUserData(z)==n){h[i].setOpen(true);
break;
}}}},destruct:function(){this._iFrameWindow=null;
this._disposeObjects(C,bI,D);
}});
})();
...
(function(){var k="key",j="classname",i="execute",h="changeValue",g="set",f="qx.version",d="Group by inheritance",c="inspector/images/icons/api.png",b="qx.ui.core.Widget",a="_filter",M="_propertyList",L="_setPropertyToDefaultButton",K="inspector/images/icons/highlight.png",J="http://demo.qooxdoo.org/",I="icon/22/actions/view-refresh.png",H="#",G="_highlightCurrentPropertyButton",F="View",E="get",D="_reloadButton",r="/apiviewer/",s="_setNullButton",p="qx.ui.core.Parent",q="Property",n="inspector/images/icons/setnull.png",o="inspector/images/icons/goto.png",l="qx.ui.core.ClientDocument",m="_menu",t="Group by category",u="Show Inherited Porperties",x="_groupButton",w="~",z="_gotoSelectedPropertyButton",y="_inheritedButton",B="current",A="inspector/images/icons/setinit.png",v=" (Loading...)",C="inspector.property.PropertyWindow";
qx.Class.define(C,{extend:inspector.components.AbstractWindow,implement:inspector.property.IPropertyListController,construct:function(){inspector.components.AbstractWindow.call(this,q);
this._filter=new inspector.property.Filter();
this._addToolbarButtons();
this._createMainElement();
},statics:{RELOAD_BUTTON_TOOLTIP_TEXT:"Reload the window.",SHOW_API_BUTTON_TOOLTIP_TEXT:"Show the API of the selected object or property.",SET_NULL_BUTTON_TOOLTIP_TEXT:"Set the currently selected property to null.",SET_DEFAULT_BUTTON_TOOLTIP_TEXT:"Set the currently selected property to its initial value.",HIGHLIGHT_SELECTED_PROPERTY_BUTTON_TOOLTIP_TEXT:"Highlight the currently selected property.",GOTO_SELECTED_PROPERTY_BUTTON_TOOLTIP_TEXT:"Go to the currently selected property.",PROPERTY_CAPTION_TITLE:"Properties"},members:{_propertyList:null,_propertyListFull:null,_propertyListHtmlTable:null,_reloadButton:null,_reloadToolTip:null,_apiButtonToolTip:null,_setNullButton:null,_setNullTooltip:null,_setPropertyToDefaultButton:null,_setPropertyToDefaultTooltip:null,_highlightCurrentPropertyButton:null,_highlightCurrentPropertyTooltip:null,_gotoSelectedPropertyButton:null,_gotoSelectedPropertyTooltip:null,_menu:null,_currentlySelectedProperty:null,_qxObject:null,_showInherited:false,_reloadTimer:null,_filter:null,setInitSizeAndPosition:function(){var bb=qx.bom.Viewport.getWidth()-this.getWidth();
var ba=parseInt((qx.bom.Viewport.getHeight()-30)/3);
this.moveTo(bb,30+2*ba);
this.setHeight(ba);
},select:function(bv){if(this._qxObject==bv){return;
}this._qxObject=bv;
this.setCaption(this.self(arguments).PROPERTY_CAPTION_TITLE+v);
var self=this;
window.setTimeout(function(){self._propertyList.build();
if(self._currentlySelectedProperty!=null){var O=self._currentlySelectedProperty.getUserData(k);
var N=self._currentlySelectedProperty.getUserData(j);
if(self._propertyList.containsProperty(O,N)){self._switchPropertyButtons();
...
},setSelectedProperty:function(Y){this._currentlySelectedProperty=Y;
...
this.add(X,{flex:1});
X.add(this._propertyList);
},_addToolbarButtons:function(){this._createMenu();
var bq=new qx.ui.toolbar.MenuButton(F,null,this._menu);
this._toolbar.add(bq);
this._toolbar.addSeparator();
this._reloadButton=new qx.ui.toolbar.Button(null,I);
this._reloadButton.setToolTipText(this.self(arguments).RELOAD_BUTTON_TOOLTIP_TEXT);
this._reloadButton.addListener(i,function(){this._propertyList.build();
},this);
this._toolbar.add(this._reloadButton);
this._toolbar.addSeparator();
var bp=new qx.ui.toolbar.Button(null,c);
bp.setToolTipText(this.self(arguments).SHOW_API_BUTTON_TOOLTIP_TEXT);
bp.addListener(i,this._onOpenApiWindow,this);
this._toolbar.add(bp);
this._toolbar.addSpacer();
this._setNullButton=new qx.ui.toolbar.Button(null,n);
this._setNullButton.setToolTipText(this.self(arguments).SET_NULL_BUTTON_TOOLTIP_TEXT);
this._setNullButton.addListener(i,this._setNullButtonEventListener,this);
this._setNullButton.setEnabled(false);
this._toolbar.add(this._setNullButton);
this._setPropertyToDefaultButton=new qx.ui.toolbar.Button(null,A);
this._setPropertyToDefaultButton.setToolTipText(this.self(arguments).SET_DEFAULT_BUTTON_TOOLTIP_TEXT);
this._setPropertyToDefaultButton.addListener(i,this._setPropertyToDefaultButtonEventListener,this);
this._setPropertyToDefaultButton.setEnabled(false);
this._toolbar.add(this._setPropertyToDefaultButton);
this._highlightCurrentPropertyButton=new qx.ui.toolbar.Button(null,K);
this._highlightCurrentPropertyButton.setToolTipText(this.self(arguments).HIGHLIGHT_SELECTED_PROPERTY_BUTTON_TOOLTIP_TEXT);
this._highlightCurrentPropertyButton.addListener(i,this._highlightCurrentPropertyButtonEventListener,this);
this._highlightCurrentPropertyButton.setEnabled(false);
this._gotoSelectedPropertyButton=new qx.ui.toolbar.Button(null,o);
this._gotoSelectedPropertyButton.setToolTipText(this.self(arguments).GOTO_SELECTED_PROPERTY_BUTTON_TOOLTIP_TEXT);
this._gotoSelectedPropertyButton.addListener(i,this._gotoSelectedPropertyButtonEventListener,this);
this._gotoSelectedPropertyButton.setEnabled(false);
},_createMenu:function(){this._menu=new qx.ui.menu.Menu();
this._inheritedButton=new qx.ui.menu.CheckBox(u);
this._inheritedButton.addListener(h,this._switchInheritedStatus,this);
this._inheritedButton.setValue(true);
this._menu.add(this._inheritedButton);
this._menu.addSeparator();
var br=new qx.ui.menu.RadioButton(d);
br.addListener(h,function(e){if(this._qxObject!=null){this._propertyList.build();
}this._inheritedButton.setEnabled(true);
},this);
br.setValue(true);
this._menu.add(br);
this._groupButton=new qx.ui.menu.RadioButton(t);
this._groupButton.addListener(h,function(e){if(this._qxObject!=null){this._propertyList.build();
}this._inheritedButton.setEnabled(false);
},this);
this._menu.add(this._groupButton);
new qx.ui.form.RadioGroup(br,this._groupButton);
},_switchPropertyButtons:function(){if(this._currentlySelectedProperty==null){this._setNullButton.setEnabled(false);
this._setPropertyToDefaultButton.setEnabled(false);
this._highlightCurrentPropertyButton.setEnabled(false);
this._gotoSelectedPropertyButton.setEnabled(false);
return ;
}var be=this._currentlySelectedProperty.getUserData(j);
var bf=this._currentlySelectedProperty.getUserData(k);
var bc=qx.core.Init.getApplication().getIframeWindowObject();
var bh=bc.qx.Class.getByName(be).$$properties;
var bg=bh[bf];
if(bg.nullable){this._setNullButton.setEnabled(true);
}else{this._setNullButton.setEnabled(false);
}if(bg.init){this._setPropertyToDefaultButton.setEnabled(true);
}else{this._setPropertyToDefaultButton.setEnabled(false);
}if(bf!=undefined){try{var bi=E+qx.lang.String.firstUp(bf);
if(this._qxObject[bi]==undefined){this._setNullButton.setEnabled(false);
this._setPropertyToDefaultButton.setEnabled(false);
this._highlightCurrentPropertyButton.setEnabled(false);
this._gotoSelectedPropertyButton.setEnabled(false);
return ;
}var bd=this._qxObject[bi].call(this._qxObject);
if((bg.check==b||bg.check==p)&&(this._qxObject.classname!=l)&&(bd!=null)){this._highlightCurrentPropertyButton.setEnabled(true);
this._gotoSelectedPropertyButton.setEnabled(true);
}else{this._highlightCurrentPropertyButton.setEnabled(false);
this._gotoSelectedPropertyButton.setEnabled(false);
}}catch(P){alert("Error during reading the selected Property: "+P);
this._currentlySelectedProperty.setBackgroundColor(null);
this._currentlySelectedProperty=null;
this._highlightCurrentPropertyButton.setEnabled(false);
this._gotoSelectedPropertyButton.setEnabled(false);
}}},_onOpenApiWindow:function(){if(this._qxObject!=null){if(this._currentlySelectedProperty!=null){var V=this._currentlySelectedProperty.getUserData(j);
var W=this._currentlySelectedProperty.getUserData(k);
this._openApiWindow(V,W);
}else{this._openApiWindow(this._qxObject.classname);
}}else{this._openApiWindow();
}},_openApiWindow:function(Q,R){var S=qx.core.Init.getApplication()._loadedWindow.qx.core.Setting.get(f);
var T=J+(S||B)+r;
...
U.focus();
},_setNullButtonEventListener:function(){var bs=this._currentlySelectedProperty.getUserData(j);
var bu=this._currentlySelectedProperty.getUserData(k);
var bt=g+qx.lang.String.firstUp(bu);
try{this._qxObject[bt].call(this._qxObject,null);
var bs=this._currentlySelectedProperty.getUserData(j);
var bu=this._currentlySelectedProperty.getUserData(k);
this._propertyList.update(bu,bs);
this._switchPropertyButtons();
}catch(e){alert(e);
}},_setPropertyToDefaultButtonEventListener:function(){var bk=this._currentlySelectedProperty.getUserData(j);
var bl=this._currentlySelectedProperty.getUserData(k);
var bj=qx.core.Init.getApplication().getIframeWindowObject();
var bo=bj.qx.Class.getByName(bk).$$properties;
var bn=bo[bl];
var bm=g+qx.lang.String.firstUp(bl);
try{this._qxObject[bm].call(this._qxObject,bn.init);
var bk=this._currentlySelectedProperty.getUserData(j);
var bl=this._currentlySelectedProperty.getUserData(k);
...
this._disposeObjects(a,m,y,x,D,s,L,G,z,M);
}});
})();
(function(){var d="inspector.property.IFilter";
qx.Interface.define(d,{members:{sortIn:function(a,b,c){return true;
},getResult:function(){return true;
},empty:function(){return true;
}}});
})();
(function(){var l="Behavior",k="Content",j="Appearance",h="Dimension",g="Spacing",f="Tooltip & Context menu",e="Visibility",d="height",c="content",b="spacing",U="droppable",T="shadow",S="cursor",R="decorator",Q="opacity",P="zIndex",O="color",N="appearance",M="icon",L="inspector.property.Filter",s="value",t="font",q="keepfocus",r="focusable",o="enabled",p="allowShrink",m="keepactive",n="draggable",u="name",v="align",C="width",A="visibility",F="i",E="padding",H="selectable",G="rich",x="allowStretch",K="tabindex",J="anonymous",I="tooltip",w="label",y="margin",z="contextmenu",B="allowGrow",D="checked";
qx.Class.define(L,{extend:qx.core.Object,implement:inspector.property.IFilter,statics:{DEFAULT_CATEGORY_NAME:"Rest"},construct:function(){qx.core.Object.call(this);
this._tests=[];
this._defineTests();
this._createCategories();
},members:{_categories:null,_properties:null,_classnames:null,_tests:null,sortIn:function(W,X,Y){for(var i=0;i<this._tests.length;i++){var ba=new RegExp(this._tests[i][0],F);
...
this.__Pi();
},__Pi:function(){this._properties=[];
this._classnames=[];
for(var i=1;i<=this._categories[this.self(arguments).DEFAULT_CATEGORY_NAME];i++){this._properties[i]={};
this._classnames[i]=[];
}},_defineTests:function(){this._tests.push([y,g]);
this._tests.push([b,g]);
this._tests.push([E,g]);
this._tests.push([v,g]);
this._tests.push([N,j]);
this._tests.push([O,j]);
this._tests.push([R,j]);
this._tests.push([T,j]);
this._tests.push([t,j]);
this._tests.push([Q,j]);
this._tests.push([S,j]);
this._tests.push([C,h]);
this._tests.push([d,h]);
this._tests.push([B,h]);
this._tests.push([p,h]);
this._tests.push([x,h]);
this._tests.push([c,k]);
this._tests.push([G,k]);
this._tests.push([o,k]);
this._tests.push([D,k]);
this._tests.push([s,k]);
this._tests.push([u,k]);
this._tests.push([w,k]);
this._tests.push([M,k]);
this._tests.push([I,f]);
this._tests.push([z,f]);
this._tests.push([A,e]);
this._tests.push([P,e]);
this._tests.push([J,l]);
this._tests.push([K,l]);
this._tests.push([r,l]);
this._tests.push([q,l]);
this._tests.push([m,l]);
this._tests.push([n,l]);
this._tests.push([U,l]);
this._tests.push([H,l]);
}},destruct:function(){this._tests=this._categories=this._properties=this._classnames=null;
}});
})();
(function(){var f="abstract",e="qx.core.Object",d="inspector.property.AbstractPropertyList";
qx.Class.define(d,{extend:qx.ui.container.Composite,type:f,construct:function(x){qx.ui.container.Composite.call(this);
...
},update:function(o,p){throw new Error("Abstract method call (update) in 'PropertyList'!");
},getComponents:function(){throw new Error("Abstract method call (getComponents) in 'PropertyList'!");
},containsProperty:function(b,c){throw new Error("Abstract method call (containsProperty) in 'PropertyList'!");
...
}if(h.qx.Class.getByName(e)==m){break;
...
(function(){var Y=".",X="",W="key",V="classname",U="inspector/images/shell/errorIcon.png",T="click",S="visible",R="inherited",Q="set",P="name",ca="row",bY="changeValue",bX="qx.ui.basic.Label",bW="qx.ui.core.Widget",bV="activate",bU="excluded",bT="inspector/images/close.png",bS="Integer",bR="inspector/images/null.png",bQ="middle",bg="get",bh="getActiveWindow",be="qx.ui.container.Composite",bf="qx.event.type.Focus",bc="Number",bd="NonEmptyString",ba="blur",bb="mousedown",bm="Boolean",bn="icon/16/actions/go-next.png",bw="#FFFFFF",bt="inspector.property.PropertyList",bE="qx.ui.form.TextField",bz="solid",bM="_arrow",bJ="execute",bp="Enter",bP="Double",bO="inspector/images/open.png",bN="#969696",bo="<u>",br="pointer",bs=":",bv="Label",bx="String",bA="hidden",bG="<b>",bL="]</u>",bi="qx.ui.form.CheckBox",bj="</b>",bq=" objects",bD="Float",bC="cursor",bB="qx.ui.form.ComboBox",bI="keypress",bH="inherit",by="Color",bF=" [",O="Font",bK="qx.event.type.KeySequence",bk="white",bl="left",bu="Choose Color";
qx.Class.define(bt,{extend:inspector.property.AbstractPropertyList,construct:function(k){inspector.property.AbstractPropertyList.call(this,k);
this._propertyRows={};
this._comboBoxPopups=[];
this._createColorPopup();
var l=new qx.ui.basic.Image(bn);
l.setPaddingLeft(8);
...
}},update:function(b,c){this._setPropertyValueFull(b,c);
},switchInheritedStatus:function(){var cF=this.getChildren();

for(var i=0;i<cF.length;i++){if(cF[i].getUserData(R)){if(this._controller.getInheritedStatus()){cF[i].setVisibility(S);
}else{cF[i].setVisibility(bU);
}}}},containsProperty:function(cI,cJ){return this._propertyRows[cJ+Y+cI]==null?false:true;
},_reloadPropertyListFull:function(){var p=false;
var s=true;
var z=this._getData(this._controller.getQxObject());
var q=z.names;
var A=z.props;
var r=z.classes;
if(!this._controller.getGroupStatus()){this._removeUnnecessaryClasses(q);
}for(var i=A.length-1;i>0;i--){if(!this._controller.getGroupStatus()){var v=this.getChildren();
if(!p&&v.length>0){var x=v.length-1-2*(A.length-i-1);
if(x>0){var C=v[x].getUserData(P);
if(C!=q[i]){var w=q[i+1];
p=true;
}}else{p=true;
s=false;
}}else{p=true;
...
if(s){this._clearList();
s=false;
}}if(p){if(s){this._removeOld(w);
s=false;
}var o=new qx.ui.basic.Atom(bG+q[i]+bj,bT);
o.setUserData(P,q[i]);
o.setRich(true);
this.addAt(o,0);
var m=new qx.ui.container.Composite(new qx.ui.layout.Grid(6,6));
m.getLayout().setColumnWidth(0,25);
m.setUserData(P,q[i]);
if(!this._controller.getGroupStatus()){if(i==1){m.setUserData(R,false);
o.setUserData(R,false);
}else{m.setUserData(R,true);
o.setUserData(R,true);
}}o.addListener(T,function(e){if(this.isVisible()){this.setVisibility(bU);
e.getTarget().setIcon(bO);
}else{this.setVisibility(S);
e.getTarget().setIcon(bT);
}},m);
this.addAfter(m,o);
var y=0;

for(var u in A[i]){if(A[i][u].group==null){var n=new qx.ui.basic.Label(u+bs);
n.setUserData(V,r[i][u]);
n.setUserData(W,u);
n.setUserData(ca,y);
m.add(n,{row:y,column:1});
var B=null;

try{B=this._getPropertyWidgetFull(A[i][u],u,r[i][u]);
}catch(dc){B=new qx.ui.basic.Label(X);
}B.setUserData(V,r[i][u]);
B.setUserData(W,u);
B.setUserData(ca,y);
m.add(B,{row:y,column:2});
var t=new qx.ui.basic.Image(bR);
t.setUserData(V,r[i][u]);
t.setUserData(W,u);
t.setUserData(ca,y);
m.add(t,{row:y,column:3});
this._propertyRows[r[i][u]+Y+u]={container:m,row:y};
m.getLayout().setRowAlign(y,bl,bQ);
m.getLayout().setRowMinHeight(y,20);
n.addListener(T,this.__Pj,this);
B.addListener(T,this.__Pj,this);
B.addListener(bV,this.__Pj,this);
t.addListener(T,this.__Pj,this);
y++;
}}}}this.switchInheritedStatus();
this._refillPropertyListFull();
},_removeUnnecessaryClasses:function(d){for(;(d.length-1)*2<this.getChildren().length;){var h=this.getChildren()[0];
this.removeAt(0);
h.dispose();
var g=this.getChildren()[0].getChildren();
for(var j=0;j<g.length;j++){if(g[j].classname==bX){var f=g[j].getUserData(V)+Y+g[j].getUserData(W);
delete this._propertyRows[f];
}}this.removeAt(0);
}},_removeOld:function(E){if(E==null){this._clearList();
return;
}while(true){var J=this.getChildren()[0];
var I=J.getUserData(P);
if(I==E){break;
}else{if(J.classname==be){var H=J.getChildren();
for(var F=0;F<H.length;F++){if(H[F].classname==bX){var G=H[F].getUserData(V)+Y+H[F].getUserData(W);
delete this._propertyRows[G];
}}}this.removeAt(0);
}}},_clearList:function(){for(var ce in this._porpertyColumns){delete this._porpertyColumns[ce];
}this.removeAll();
},_getPropertyWidgetFull:function(cf,cg,ch){var cu=bg+qx.lang.String.firstUp(cg);

try{if(cu===bh){throw new Error("Property activeWindow of an instance of qx.ui.root.Abstract is not (yet) ready!");
}else{var cm=this._controller.getQxObject()[cu]();
}}catch(cb){return new qx.ui.basic.Label();
}if(cf.check!==null){if(cf.check==bm){var cj=new qx.ui.form.CheckBox();
var co=function(e){var cB=this._controller.getQxObject()[cu].call(this._controller.getQxObject());

if(e.getData()!=cB){var cA=Q+qx.lang.String.firstUp(cg);
try{this._controller.getQxObject()[cA].call(this._controller.getQxObject(),e.getData());
this._setPropertyValueFull(cg,ch,true);
}catch(cx){alert(cx+" ["+cA+"]");
cj.setValue(!cB);
}}};
cj.addListener(bY,co,this);
return cj;
}else if(cf.check instanceof Array){var ci=new qx.ui.form.ComboBox();
var cp=cf.check;
for(var i=0;i<cp.length;i++){var cn=new qx.ui.form.ListItem(cp[i]);
ci.add(cn);
}ci.addListener(bY,function(e){var N=null;
if(e.getTarget().getValue()!=null){var N=e.getTarget().getValue();
}if(N!=cm){var M=Q+qx.lang.String.firstUp(cg);
try{this._controller.getQxObject()[M].call(this._controller.getQxObject(),N);
cm=this._controller.getQxObject()[cu].call(this._controller.getQxObject());
this._setPropertyValueFull(cg,ch,true);
cm=this._controller.getQxObject()[cu].call(this._controller.getQxObject());
}catch(cw){alert(cw);
}}},this);
return ci;
}else if(cf.check==bS||cf.check==bx||cf.check==bd||cf.check==bv||cf.check==bD||cf.check==bP||cf.check==bc){var cs=new qx.ui.form.TextField();
var cq=function(e){if(e.classname==bK){if(e.getKeyIdentifier()!=bp){return;
}}else if(e.classname==bf){}else{return ;
}var cG=Q+qx.lang.String.firstUp(cg);
try{var cH=cs.getValue();
cm=this._controller.getQxObject()[cu].call(this._controller.getQxObject());
if(e.classname==bf){if(cH==X&&cm==null){return;
}}if(cf.check==bS||cf.check==bc){cH=parseFloat(cH);
}this._controller.getQxObject()[cG].call(this._controller.getQxObject(),cH);
this._setPropertyValueFull(cg,ch,true);
cm=this._controller.getQxObject()[cu].call(this._controller.getQxObject());
}catch(cc){alert(cc);
cs.setValue(cm+X);
}};
cs.addListener(ba,cq,this);
cs.addListener(bI,cq,this);
return cs;
}else if(cf.check==by){var ck=new qx.ui.container.Composite(new qx.ui.layout.HBox(6));
ck.getLayout().setAlignY(bQ);
var cr=new qx.ui.core.Widget();
cr.setDecorator(new qx.ui.decoration.Single(1,bz,bN));
cr.setBackgroundColor(bk);
cr.setHeight(20);
cr.setWidth(20);
cr.setAllowGrowX(false);
cr.setAllowGrowY(false);
ck.add(cr);
this._colorFields[ch+Y+cg]=cr;
var cl=new qx.ui.form.Button(bu);
ck.add(cl);
cl.addListener(bb,function(e){this._colorPopup.setValue(cr.getBackgroundColor());
this._currentColorProperty=ch+Y+cg;
this._colorPopup.placeToMouse(e);
this._colorPopup.show();
},this);
cl.addListener(bJ,this.__Pj,this);
cl.addListener(bV,this.__Pj,this);
return ck;
}else if(cf.ckeck==bW){var cv=new qx.ui.basic.Label();
return cv;
}else{var ct=new qx.ui.basic.Label();
return ct;
}}else{var ct=new qx.ui.basic.Label();
return ct;
}},_refillPropertyListFull:function(){for(var cL in this._propertyRows){try{var cM=cL.substr(cL.lastIndexOf(Y)+1);
var cK=cL.substring(0,cL.lastIndexOf(Y));
this._setPropertyValueFull(cM,cK);
}catch(cy){}}},_setPropertyValueFull:function(cO,cP,cQ){var cT=qx.core.Init.getApplication().getIframeWindowObject();
var cS=this._propertyRows[cP+Y+cO].container.getLayout();
var db=this._propertyRows[cP+Y+cO].row;

if(!cQ&&cS.getCellWidget(db,0)){this._arrow.container.remove(this._arrow.arrow);
this._arrow.container=null;
this._arrow.row=null;
}var da=bg+qx.lang.String.firstUp(cO);

try{if(da===bh){throw new Error("Property activeWindow of an instance of qx.ui.root.Abstract is not (yet) ready!");
}else{var cW=this._controller.getQxObject()[da]();
}}catch(cz){cS.getCellWidget(db,3).setVisibility(S);
cS.getCellWidget(db,3).setSource(U);
var cU=cS.getCellWidget(db,3).getToolTip();

if(!cU){cU=new qx.ui.tooltip.ToolTip(cz+X,U);
}else{cU.setLabel(cz+X);
cU.setIcon(U);
}cS.getCellWidget(db,3).setToolTip(cU);
return;
}if(cW==null){cS.getCellWidget(db,3).setVisibility(S);
cS.getCellWidget(db,3).setSource(bR);
cS.getCellWidget(db,3).resetToolTip();
}else{cS.getCellWidget(db,3).setVisibility(bA);
}
try{var parent=this._controller.getQxObject();

while(cW==bH){parent=parent.getLayoutParent();
cW=parent[da].call(parent);
}}catch(cd){cS.getCellWidget(db,3).setVisibility(S);
cS.getCellWidget(db,3).setSource(U);
var cU=cS.getCellWidget(db,3).getToolTip();

if(!cU){cU=new qx.ui.tooltip.ToolTip(cd+X,U);
}else{cU.setLabel(cd+X);
cU.setIcon(U);
}cS.getCellWidget(db,3).setToolTip(cU);
return;
}if(cS.getCellWidget(db,2).classname==bi){if(cW==null){cS.getCellWidget(db,2).setValue(false);
}else{cS.getCellWidget(db,2).setValue(cW);
}}else if(cS.getCellWidget(db,2).classname==bX){if(cW!=null){var cY=cT.qx.Class.getByName(cP).$$properties;
var cX=cY[cO];
if(cW instanceof Array){cS.getCellWidget(db,2).setValue(cW.length+bq);
}else if((cX.check==bW)&&(this._controller.getQxObject() instanceof qx.application.AbstractGui)){cS.getCellWidget(db,2).setValue(bo+cW.classname+bF+cW.toHashCode()+bL);
cS.getCellWidget(db,2).setStyleProperty(bC,br);
if(cS.getCellWidget(db,2).hasListeners(T)===undefined){cS.getCellWidget(db,2).addListener(T,function(e){if(this._controller.getSelectedProperty()!=null){this._controller.getSelectedProperty().setBackgroundColor(null);
}this._controller.setSelectedProperty(cS.getCellWidget(db,1));
this._controller.gotoSelectedWidget();
},this);
}}else if(cX.check==O){cS.getCellWidget(db,2).setFont(cW);
cS.getCellWidget(db,2).setValue(cW+X);
}else{cS.getCellWidget(db,2).setValue(cW+X);
}}else{cS.getCellWidget(db,2).setValue(X);
}}else if(cS.getCellWidget(db,2).classname==bE){if(cW!=null){cS.getCellWidget(db,2).setValue(cW+X);
}else{cS.getCellWidget(db,2).setValue(X);
}}else if(cS.getCellWidget(db,2).classname==bB){var cR=cS.getCellWidget(db,2);
if(cW==null){cR.resetSelection();
}else{for(var i=0;i<cR.getChildren().length;i++){if(cW==cR.getChildren()[i].getLabel()){if(cW){cR.setValue(cW);
}}}}}else if(cS.getCellWidget(db,2).classname==be){try{var cV=cT.qx.theme.manager.Color.getInstance().resolve(cW);
cS.getCellWidget(db,2).getChildren()[0].setBackgroundColor(cV);
}catch(a){cS.getCellWidget(db,2).getChildren()[0].setBackgroundColor(bw);
}}},_createColorPopup:function(){this._colorPopup=new qx.ui.control.ColorPopup();
var D=qx.core.Init.getApplication();
D.getRoot().add(this._colorPopup);
this._colorFields={};
this._colorPopup.addListener(bY,function(e){if(this._currentColorProperty!=null){var cD=this._currentColorProperty.substr(this._currentColorProperty.lastIndexOf(Y)+1);
var cC=this._currentColorProperty.substring(0,this._currentColorProperty.lastIndexOf(Y));
var cE=Q+qx.lang.String.firstUp(cD);
try{this._controller.getQxObject()[cE].call(this._controller.getQxObject(),e.getData());
this._colorFields[this._currentColorProperty].setBackgroundColor(e.getData());
this._setPropertyValueFull(cD,cC,true);
}catch(cN){alert(cN);
}this._currentColorProperty=null;
}},this);
},__Pj:function(e){var K=e.getTarget();

while(K.getUserData(W)==null){K=K.getLayoutParent();
}var L=K.getUserData(V)+Y+K.getUserData(W);
if(this._arrow.container!=null){this._arrow.container.remove(this._arrow.arrow);
this._arrow.container=null;
this._arrow.row=null;
}if(this._propertyRows[L]!=undefined){this._arrow.container=this._propertyRows[L].container;
this._arrow.row=K.getUserData(ca);
this._arrow.container.add(this._arrow.arrow,{row:this._arrow.row,column:0});
this._controller.setSelectedProperty(K);
...
qx.Class.define(w,{extend:inspector.components.AbstractWindow,construct:function(){inspector.components.AbstractWindow.call(this,m);
...
var I=new qx.ui.form.RadioGroup(this._consoleButton,this._domButton);
I.addListener(A,function(e){this._findField.setValue(n);

if(I.getSelection()[0]==this._consoleButton){this._stack.setSelection([this._consoleView]);
}else if(I.getSelection()[0]==this._domButton){this._stack.setSelection([this._domView]);
}else{this._consoleButton.setValue(true);
}},this);
},members:{setInitSizeAndPosition:function(){var G=qx.bom.Viewport.getWidth()-300;
var H=parseInt((qx.bom.Viewport.getHeight()-30)/3);
this.moveTo(0,2*H+30);
this.setWidth(G);
this.setHeight(H);
},escapeHtml:function(b){function c(a){switch(a){case B:return E;
case D:return j;
case s:return z;
case F:return y;
case d:return v;
}return t;
}return String(b).replace(/[<>&"']/g,c);
},inspectObjectByInternalId:function(L){var o=this._consoleView.getObjectById(L);
...
(function(){var B="",A=")",z="Tab",y="'>",x="qx.core.Init.getApplication().inspectObjectByInternalId(",w="Down",v="Up",u="warning",t="error",s="Control",bB="info",bA="(",bz="<span class='ins_console_link' onclick='",by="display",bx="<div class='ins_console_common'><div class='",bw="Escape",bv="Enter",bu="ins_console_warn",bt="]</span> ",bs="ins_console_return_qxobject",I="changeValue",J="qx.client",G="keydown",H="Courier New",E=", ...",F="' class='ins_console_icon'>",C="\"qx.core.Init.getApplication().setWidgetByHash('",D="<img src='",O="ins_console_return_primitive",P="ins_console_debug",X="&nbsp;",V="<span class='ins_console_dom_link' onclick='",bg="input",bb="Space",bo=">>>&nbsp;",bl="ins_console_text",R="middle",br=" more",bq="<span class='ins_console_link' onclick=",bp="ins_console_error",Q="inspector.console.ConsoleView",T="ins_console_info",U=" </span>",W="</div></div>",Y="]",bc="[",bi="inspector/images/shell/",bn=", ",K="keyup",L="ins_console_return_object",S="ins_console_return_array",bf="</span>",be="mshtml",bd="'>inspect Object</span>",bk="keypress",bj="none",ba="', 'console');\"> ",bh=" [",r=">>>",bm=".",M="Icon.png",N="scroll";
qx.Class.define(Q,{extend:qx.ui.core.Widget,construct:function(console){qx.ui.core.Widget.call(this);
...
this._filter=B;
this._setLayout(new qx.ui.layout.VBox());
this._content=new qx.ui.embed.Html(B);
this._content.setOverflowY(N);
...
var l=new qx.bom.Font(12,[H]);
m.setFont(l);
n.add(m);
this._inputTextField=new qx.ui.form.TextField(B);
...
this._inputTextField.addListener(G,this._keyDownHandler,this);
this._inputTextField.addListener(K,this._keyUpHandler,this);
this._inputTextField.addListener(bk,this._keyPressHandler,this);
this._inputTextField.addListener(I,function(e){if(this._autoCompletePopup.isOnScreen()){this._autoCompletePopup.load(e.getData());
}},this);
...
},members:{clear:function(){this._content.setHtml(B);
},getObjectById:function(cg){return this._objectFolder[cg];
...
for(var i=0;i<bR.length;i++){if(qx.core.Variant.isSet(J,be)){var content=bR[i].innerText;
}else{var content=bR[i].textContent;
}if(qx.dom.Node.isElement(bR[i])){if(bQ.test(content)){qx.bom.element.Style.set(bR[i],by,null);
}else{qx.bom.element.Style.set(bR[i],by,bj);
}}}}catch(e){alert("Unable to filter: "+e);
}},chooseAutoCompleteValue:function(){var k=this._inputTextField.getValue();
var name=this._autoCompletePopup.getCurrentSelection();
if(name){var j=k.substring(k.lastIndexOf(bm)+1);
name=name.substring(j.length,name.length);
this.appendString(name);
}this._autoCompletePopup.hide();
this._inputTextField.focus();
},appendString:function(bE){if(bE!=null){this._inputTextField.setValue(this._inputTextField.getValue()+bE);
if(this._inputTextField.getValue().lastIndexOf(bA)!=-1){var bF=this._inputTextField.getValue().lastIndexOf(bA)+1;
var bG=this._inputTextField.getValue().length-1;
this._inputTextField.setTextSelection(bF,bG);
}}},_process:function(cc){this._printText(this._console.escapeHtml(cc));

try{var cd=inspector.console.Util.evalOnIframe(cc);

if(cd!=null){this._objectFolder[this._objectFolderIndex]={name:cc,object:cd};
this._printReturnValue(cd);
this._objectFolderIndex++;
}}catch(f){this.error(f);
}},_keyDownHandler:function(e){if(e.getKeyIdentifier()==bw){this._autoCompletePopup.hide();
return;
}if(e.getKeyIdentifier()==bv){if(!this._autoCompletePopup.isOnScreen()){this._history.unshift(this._inputTextField.getValue());
this._process(this._inputTextField.getValue());
this._inputTextField.setValue(B);
this._historyCounter=-1;
if(this._history.length>20){this._history.pop();
}}else{this.chooseAutoCompleteValue();
}return;
}if(e.getKeyIdentifier()==v){e.preventDefault();
...
}if(e.getKeyIdentifier()==w){e.preventDefault();
if(!this._autoCompletePopup.isOnScreen()){if(this._historyCounter>0){this._historyCounter--;
this._inputTextField.setValue(this._history[this._historyCounter]);
}}return;
}if(e.getKeyIdentifier()==s){this._ctrl=true;
return;
}if(e.getKeyIdentifier()==bb||e.getKeyIdentifier()==z){if(this._ctrl||e.getKeyIdentifier()==z){e.preventDefault();
if(e.getKeyIdentifier()==z){var self=this;
...
}try{var bJ=qx.bom.element.Location.getLeft(this.getContainerElement().getDomElement());
...
this._autoCompletePopup.open(this._inputTextField.getValue(),bJ,top);
var self=this;
window.setTimeout(function(){self._inputTextField.focus();
},0);
}catch(q){this.info(q);
}}return;
}},_keyUpHandler:function(e){if(e.getKeyIdentifier()==s){this._ctrl=false;
}},_keyPressHandler:function(e){if(this._autoCompletePopup.isOnScreen()){if(e.getKeyIdentifier()==w){this._autoCompletePopup.selectionDown();
}else if(e.getKeyIdentifier()==v){this._autoCompletePopup.selectionUp();
}}},_scrollToLastLine:function(){qx.ui.core.queue.Manager.flush();
var self=this;
window.setTimeout(function(){var bY=self._content.getContentElement();
var bX=qx.bom.element.Dimension.getContentHeight(self._content.getContentElement().getDomElement());
bY.scrollToY(bX);
},0);
},_printReturnValue:function(bK){var bL=qx.core.Init.getApplication().getIframeWindowObject();
if(bL&&bK instanceof bL.qx.core.Object){this._printQxObject(bK);
}else if(bK instanceof bL.Array){var bM=this._printArray(bK);
var bN=this._getLabel(bz+x+this._objectFolderIndex+A+y+bM+bf,S);
this._content.setHtml(this._content.getHtml()+bN);
return;
}else if(bL&&(bK instanceof bL.Object||bK==bL.window||bK==bL.document)){var bN=this._getLabel(bz+x+this._objectFolderIndex+A+y+bK+U,L);
this._content.setHtml(this._content.getHtml()+bN);
return;
}else{var bN=this._getLabel(bK,O);
this._content.setHtml(this._content.getHtml()+bN);
}this._scrollToLastLine();
},_printQxObject:function(bH){var bI=this._getLabel(bq+C+bH.toHashCode()+ba+bH.classname+bh+bH.toHashCode()+bt+V+x+this._objectFolderIndex+A+bd,bs);
this._content.setHtml(this._content.getHtml()+bI);
this._scrollToLastLine();
},_printText:function(ce){var cf=this._getLabel(bo+ce,bl);
this._content.setHtml(this._content.getHtml()+cf);
this._scrollToLastLine();
},_getLabel:function(bS,bT,bU){var bS=bS;
if(bU==bB||bU==t||bU==u){var bW=qx.util.ResourceManager.getInstance().toUri(bi+bU+M);
var bV=D+bW+F;
bS=bV+X+bS;
}bS=bx+bT+y+bS+W;
return bS;
},_printArray:function(a){var b=qx.core.Init.getApplication().getIframeWindowObject();

if(a instanceof b.Array){var c=new qx.util.StringBuilder();
var length=a.length;
var d=B;

if(length>2){d=E+(length-2)+br;
length=2;
}
for(var i=0;i<length;i++){if(!c.isEmpty()){c.add(bn);
}c.add(this._printArray(a[i]));
}return bc+c.get()+d+Y;
}else{return a;
}},error:function(bC){if(!this._console.isVisible()){this._console.open();
}var bD=this._getLabel(bC,bp,t);
this._content.setHtml(this._content.getHtml()+bD);
this._scrollToLastLine();
},warn:function(ca){if(!this._console.isVisible()){this._console.open();
}var cb=this._getLabel(ca,bu,u);
this._content.setHtml(this._content.getHtml()+cb);
this._scrollToLastLine();
},info:function(g){if(!this._console.isVisible()){this._console.open();
}var h=this._getLabel(g,T,bB);
this._content.setHtml(this._content.getHtml()+h);
this._scrollToLastLine();
},debug:function(ch){if(!this._console.isVisible()){this._console.open();
}var ci=this._getLabel(ch,P);
this._content.setHtml(this._content.getHtml()+ci);
this._scrollToLastLine();
},dispose:function(){var bO=qx.core.Init.getApplication().getIframeWindowObject();
bO.qx.log.Logger.unregister(inspector.console.Appender);
}}});
})();
...
qx.Class.define(s,{extend:qx.ui.popup.Popup,construct:function(B){qx.ui.popup.Popup.call(this);
this._controller=B;
this.setLayout(new qx.ui.layout.VBox());
...
var C=new qx.ui.table.cellrenderer.Image(18,18);
this._table.getTableColumnModel().setDataCellRenderer(0,C);
...
var M=H.substring(H.lastIndexOf(i)+1);
if(H==M){H=o+H;
}H=H.substring(0,H.lastIndexOf(i));
var K=null;
...
return ;
}if(K instanceof I.qx.core.Object){this._tableModel.setColumnNamesByIndex([m,K.classname]);
}else{this._tableModel.setColumnNamesByIndex([m,H]);
}var N=new RegExp(z+M);
var O=[];
for(var name in K){try{if(N.test(name)){if(name.substring(0,2)==y){var L=p;
}else if(name.substring(0,1)==v){var L=n;
}else{var L=x;
}if(K[name] instanceof I.Function){var P=name+u;
for(var j=0;j<K[name].length;j++){if(j==K[name].length-1){P+=unescape(k+(61+j));
}else{P+=unescape(k+(61+j)+r);
}}P+=w;
var J=b+L+l;
O.push([J,P]);
}else{var J=f+L+l;
...
if(Q!=-1){return this._tableModel.getData()[Q][1]+m;
}return null;
...
(function(){var s="qx.client",r="",q="');",p="webkit",o="webkit|mshtml|gecko",n="  } catch (ex) {",m="inspector.console.Util",l="  }",k="mshtml",j="  try {",d="return eval('",i="};",g="window.top.inspector.$$inspector = function()",c="{",b="    return ex;",f="opera",e="$2",h="return eval.call(window, '";
qx.Class.define(m,{statics:{evalOnIframe:function(u){var v=qx.core.Init.getApplication().getIframeWindowObject();
var x=null;

try{if(qx.core.Variant.isSet(s,o)){if(qx.core.Variant.isSet(s,k)||qx.core.Variant.isSet(s,p)){u=u.replace(/^(\s*var\s+)(.*)$/,e);
}var w=r;
if(qx.core.Variant.isSet(s,p)&&qx.bom.client.Engine.FULLVERSION>=528){w=d+u+q;
}else{w=h+u+q;
}v.qx.lang.Function.globalEval([g,c,j,w,n,b,l,i].join(r));
x=inspector.$$inspector.call(qx.core.Init.getApplication().getSelectedObject());
}else if(qx.core.Variant.isSet(s,f)){x=(function(a){return v.eval(a);
...
qx.Class.define(d,{statics:{consoleView:null,process:function(C){if(this.consoleView){var D=this.__Pk(C);

if(C.level==o){this.consoleView.info(D);
}else if(C.level==g){this.consoleView.warn(D);
}else if(C.level==a){this.consoleView.error(D);
}else{this.consoleView.debug(D);
}}},__Pk:function(r){var u=qx.core.Init.getApplication().getIframeWindowObject();
...
if(typeof t===b){w.push(this.__Pl(t));
}else if(t.key){w.push(t.key+q+this.__Pl(t.text));
}else{w.push(this.__Pl(t.text));
}}
if(y.type===h){B.add(n,w.join(p),f);
}else{B.add(c,w.join(p),k);
}}else{B.add(this.__Pl(A));
}}return B.get();
},__Pl:function(E){return this.consoleView._console.escapeHtml(E);
}}});
})();
...
(function(){var H="",G="'>",F="inspector/images/spacer.gif",E="</td></tr>",D="</td>",C="</span>",B="'><img class='ins_dom_front_image' src='",A="qx.core.Init.getApplication().inspectObjectByDomSelecet(",z="<tr><td class='",y="</div>",bJ=")",bI="</span></td></tr>",bH="string",bG="<div class='ins_dom_no_prop'>No object selected.</div>",bF="<td><span class='ins_dom_null'>",bE="\")",bD="<td class='ins_dom_object'>",bC=", \"",bB="<tr><td class='ins_dom_key'><img class='ins_dom_front_image' src='",bA=".",O="<div class='ins_dom_func'>",P=" &raquo; <span class='ins_dom_return_path_link' onclick='",M="<div class='ins_dom_return_path_main'>",N="<td class='ins_dom_self_ref'>self reference</td></tr>",K="<table class='ins_dom_table'>",L=" more</span> ]",I="'><a onclick='",J="&quot;</span>",U="<td class='ins_dom_string'>&quot;Error occurs by reading value!&quot;</td></tr>",V="Object",bg="inspector/images/open.png",bd="', 'console');\">select Object</u>",bo="toString",bj=", ",bw="<a onclick='",bt="<td class='ins_dom_string'>&quot;",Y="<td class='ins_dom_basic'>",bz="ins_dom_key_number",by="<td class='ins_dom_func_object'>",bx="<span class='ins_dom_func_object'>function()</span>",X=")'>",bb="</a>",bc=" ]",bf="</a></td>",bh="<span class='ins_dom_basic'>",bk="<div class='ins_dom_no_prop'>There are no properties to show for this object.</div>",bq="&quot;</td></tr>",bv="_htmlEmbed",Q="ins_dom_key",R="</table>",ba="</a> <a style='color: #000000;' onclick=\"qx.core.Init.getApplication().setWidgetByHash('",bn="<span class='ins_dom_object'>",bm=" &raquo; <span class='ins_dom_return_path_selected'>",bl=", ... <span class='ins_dom_array_more'>",bs="[Class ",br="qx_srcview",bi="[ ",bp="Error occurs by reading value!",v="<pre>",bu="appear",S="]</span>",T="scroll",be="<span class='ins_dom_string'>&quot;",w="</pre>",x="inspector.console.DomView",W="<span class='ins_dom_object'>[";
qx.Class.define(x,{extend:qx.ui.core.Widget,statics:{SEARCH_TERM:"Search..."},construct:function(console){qx.ui.core.Widget.call(this);
this._console=console;
this._filter=H;
this._setLayout(new qx.ui.layout.VBox());
this._htmlEmbed=new qx.ui.embed.Html(bG);
this._htmlEmbed.setOverflowY(T);
this._add(this._htmlEmbed,{flex:1});
this.addListenerOnce(bu,function(){this._htmlEmbed.getContentElement().getDomElement().id=br;
},this);
this._breadCrumb=[];
},members:{clear:function(){this._htmlEmbed.setHtml(bG);
},setObject:function(bV,name){this._iFrameWindow=qx.core.Init.getApplication().getIframeWindowObject();
this._breadCrumb=[];
var bX=name.split(bA);
for(var i=0;i<bX.length-1;i++){var bY=H;
...
},setObjectByIndex:function(p,q){this._filter=H;

try{if(q){var s=this._breadCrumb[p].object[q];
for(var i=0;i<this._breadCrumb.length;i++){if(this._breadCrumb[i].object==s){this._htmlEmbed.setHtml(this._getHtmlToObject(s,i,q));
this._htmlEmbed.getContentElement().scrollToY(0);
return ;
}}this._htmlEmbed.setHtml(this._getHtmlToObject(s,(p)+1,q));
this._htmlEmbed.getContentElement().scrollToY(0);
}else{var s=this._breadCrumb[p].object;
var r=this._breadCrumb[p].name;
this._htmlEmbed.setHtml(this._getHtmlToObject(s,p,r));
this._htmlEmbed.getContentElement().scrollToY(0);
this._breadCrumb.splice(p+1,this._breadCrumb.length-p+1);
}}catch(e){alert("Can not select this Object: "+e);
}},filter:function(l){this._filter=l;
if(this._breadCrumb.length>0){var m=this._breadCrumb.length-1;
var n=this._breadCrumb[m].object;
var name=this._breadCrumb[m].name;
this._htmlEmbed.setHtml(this._getHtmlToObject(n,m,name));
}},getFilter:function(){if(this._filter==H){return inspector.console.DomView.SEARCH_TERM;
}else{return this._filter;
}},getCurrentSelectedClassname:function(){if(this._breadCrumb.length>0){var ca=this._breadCrumb[this._breadCrumb.length-1].object;
if(ca.classname!=undefined){if(qx.Class.isDefined(ca.classname)||qx.Interface.isDefined(ca.classname)||qx.Mixin.isDefined(ca.classname)||qx.Theme.isDefined(ca.classname)){return ca.classname;
}}}return null;
},_getHtmlToObject:function(o,bM,name){var bQ=new qx.util.StringBuilder();
if(name==undefined){var name=V;
}this._breadCrumb[bM]={object:o,name:name};
bQ.add(this._getReturnPath(bM));
var bS=true;
var bU=this._sortAndFilterProperties(o);
bQ.add(K);
for(var i=0;i<bU.length;i++){bS=false;
bQ.add(H);
if(!isNaN(bU[i].key)){var bN=bz;
}else{var bN=Q;
}try{bU[i].value instanceof this._iFrameWindow.Object;
}catch(bL){var bP=qx.util.ResourceManager.getInstance().toUri(F);
bQ.add(z+bN+B+bP+G+this._console.escapeHtml(bU[i].key)+D);
bQ.add(bF+bU[i].value+bI);
continue;
}if(!(bU[i].value instanceof this._iFrameWindow.Object)&&bU[i].value!=this._iFrameWindow.window&&bU[i].value!=this._iFrameWindow.document){var bP=qx.util.ResourceManager.getInstance().toUri(F);
bQ.add(z+bN+B+bP+G+this._console.escapeHtml(bU[i].key)+D);
if(bU[i].value==null){bQ.add(bF+bU[i].value+bI);
}else if(typeof bU[i].value==bH){bQ.add(bt+this._console.escapeHtml(bU[i].value)+bq);
}else{bQ.add(Y+bU[i].value+E);
}}else{if(bU[i].value instanceof this._iFrameWindow.Function){var bO=bU[i].value.toString();
if(bO.search(/native code/)!=-1){continue;
}}var bT=null;

if(bT==null){bT=new qx.util.StringBuilder();
}else{bT.clear();
}if(bU[i].value!=o){var bP=qx.util.ResourceManager.getInstance().toUri(bg);
bT.add(z+bN+I+A+bM+bC+bU[i].key+bE+B+bP+G+this._console.escapeHtml(bU[i].key)+bf);
}if(bU[i].value==o){var bP=qx.util.ResourceManager.getInstance().toUri(F);
bQ.add(bB+bP+G+bU[i].key+D);
bQ.add(N);
}else if(bU[i].value instanceof this._iFrameWindow.Function){bQ.add(bT.get());
if(bU[i].value.toString().substr(0,7)==bs){bQ.add(bD+this._getObject(bU[i].value,bM,bU[i].key)+E);
}else{bQ.add(by+this._getObject(bU[i].value,bM,bU[i].key)+E);
}}else{try{var bR=this._getObject(bU[i].value,bM,bU[i].key);
bQ.add(bT.get());
bQ.add(bD+bR+E);
}catch(t){var bP=qx.util.ResourceManager.getInstance().toUri(F);
bQ.add(bB+bP+G+bU[i].key+D);
bQ.add(U);
}}}}bQ.add(R);
if(bS){bQ.add(bk);
}bQ.add(this._getFunctionCode(o));
return bQ.get();
},_getFunctionCode:function(o){if(o instanceof this._iFrameWindow.Function&&!o.hasOwnProperty(bo)){var bK=o.toString();
bK=v+qx.dev.Tokenizer.javaScriptToHtml(bK)+w;
return O+bK+y;
}else{return H;
}},_sortAndFilterProperties:function(o){if(this._filter!=H){try{var cb=new RegExp(this._filter);
}catch(e){alert("Unable to filter: "+e);
}}var cc=[];
for(var cd in o){try{if(cb!=null){if(cb.test(cd)){cc.push({key:cd,value:o[cd]});
}}else{cc.push({key:cd,value:o[cd]});
}}catch(u){cc.push({key:cd,value:bp});
...
k.add(M);
for(var i=0;i<=h;i++){if(i==h){k.add(bm);
}else{k.add(P+A+i+X);
...
g.add(bw+A+d+bC+f+bE+G);
if(c instanceof this._iFrameWindow.Function){if(c.toString().indexOf(bJ)!=-1){g.add(c.toString().substring(0,c.toString().indexOf(bJ)+1));
}else{g.add(c.toString());
}}else if(c instanceof this._iFrameWindow.Array){g.add(bi);
for(var j=0;j<2&&j<c.length;j++){if(c[j] instanceof this._iFrameWindow.Function){g.add(bx);
}else if(typeof c[j]==bH){g.add(be+c[j]+J);
}else if(c[j] instanceof this._iFrameWindow.Array){g.add(W+c[j]+S);
}else if(c[j] instanceof this._iFrameWindow.Object){g.add(bn+c[j]+C);
}else{g.add(bh+c[j]+C);
}if(j!=1&&j!=c.length-1){g.add(bj);
}}if(c.length>2){g.add(bl+(c.length-2)+L);
}else{g.add(bc);
}}else if(c instanceof this._iFrameWindow.qx.core.Object){g.add(c+ba+c.toHashCode()+bd);
}else{g.add(c);
}g.add(bb);
return g.get();
