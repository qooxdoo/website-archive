...
var settings = {"qx.application":"portal.Application","qx.theme":"qx.theme.Modern","qx.version":"1.1"};
...
var libinfo = {"__out__":{"sourceUri":"script"},"portal":{"resourceUri":"resource","sourceUri":"script","version":"trunk"},"qx":{"resourceUri":"resource","sourceUri":"script","version":"1.1"}};
...
(function(){var o="groupBoxes",n="boxData",m="portal.box.Box",h="portal.box.Manager",g="qx.event.type.Event",f="boxes",e="_applyActiveBox",d="loaded",c="singleton";
qx.Class.define(h,{type:c,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__Aq=[];
this.__Ar=[];
this.__As={};
},statics:{dataKeyRoot:n,dataKeyGroupBoxes:o,dataKeyBoxes:f},events:{"loaded":g},properties:{activeBox:{init:null,check:m,apply:e}},members:{__Aq:null,__Ar:null,__As:null,_applyActiveBox:function(a,b){if(b!=null){b.setActive(false);
}a.setActive(true);
},load:function(){this.__At(window[portal.box.Manager.dataKeyRoot]);
qx.event.Registration.fireEvent(this,d);
},__At:function(r){var v,y,w,x,z,r;
...
this.__Aq.push(z);
this.__As[y].push(z);
}}},getBoxes:function(){return this.__Aq;
},getGroupBoxes:function(){return this.__Ar;
},updateGroupBoxMembers:function(A,B,C){var D=qx.dom.Hierarchy.getElementIndex(C.getElement());
qx.lang.Array.remove(this.__As[B],C);
qx.lang.Array.insertAt(this.__As[A],C,D);
},getGroupBoxDataOfBox:function(p){for(var i=0,j=this.__Ar.length,q;i<j;i++){q=this.__As[this.__Ar[i].id];

for(var k=0,l=q.length;k<l;k++){if(q[k].getId()===p){return this.__Ar[i];
}}}return null;
}},destruct:function(){this.__Aq=this.__Ar=this.__As=null;
}});
})();
(function(){var z="Boolean",y="<br/><br/>",x="border",w="portal.box.Box",v="click",u="1px solid #444444",t="height",s="position",r="zIndex",q="_applyActive",k="width",p="box_content",n=" It does contain no UI widget code at all.",j="relative",i="1px solid orange",m="This is a demo application showing the low-level capabilities.",l="div",o="String";
qx.Class.define(w,{extend:qx.core.Object,construct:function(c,d,f,g){qx.core.Object.call(this);
this.__Au=c;
this.__Av=c.id;

if(d!=null){qx.bom.element.Style.set(d,k,c.width);
qx.bom.element.Style.set(d,t,c.height);
this.__Ay(d,f);
}this.setElement(d);
this.setGroupBoxId(g);
this.__Aw=null;
this.__Ax=null;
if(c.resizable){this.setResizable(true);
var h={minWidth:parseInt(c.minWidth),minHeight:parseInt(c.minHeight),maxWidth:parseInt(c.maxWidth),maxHeight:parseInt(c.maxHeight),handles:c.resizeHandles};
this.__Aw=new portal.box.Resizable(this,h);
}if(c.draggable){this.setDraggable(true);
this.__Ax=new portal.box.Draggable(this);
}this.__Az();
},properties:{element:{init:null},groupBoxId:{init:null,check:o},active:{check:z,init:false,apply:q},resizable:{check:z,init:false},draggable:{check:z,init:false}},members:{__Av:null,__Au:null,__Ax:null,__Aw:null,_applyActive:function(A,B){if(A){qx.bom.element.Style.set(this.getElement(),x,i);
}else{qx.bom.element.Style.set(this.getElement(),x,u);
}},getId:function(){return this.getElement()?this.getElement().id:null;
},getData:function(){return this.__Au;
},getDraggableComponent:function(){return this.__Ax;
},getResizableComponent:function(){return this.__Aw;
},__Ay:function(C,D){if(D){for(var E in D){qx.bom.element.Style.set(E,D[E]);
}}qx.bom.element.Style.set(C,s,j);
qx.bom.element.Style.set(C,r,0);
qx.bom.Element.addListener(C,v,this.__AA,this,true);
},__Az:function(){var b=m+n;
b=b+y+b+y+b;
var a=qx.bom.Element.create(l,{"class":p});
a.innerHTML=b;
qx.dom.Element.insertEnd(a,this.getElement());
},__AA:function(e){portal.box.Manager.getInstance().setActiveBox(this);
}},destruct:function(){this.resetElement();
this.__Au=this.__Aw=this.__Ax=null;
}});
})();
...
(function(){var m="portal.box.Resizable",l="mouseover",k="mouseout";
qx.Class.define(m,{extend:qx.core.Object,construct:function(p,q){qx.core.Object.call(this);
this.__AB=p;
this.__AC=this.__AB.getElement();
this.__AD={};
this.__AE=false;
this.__AF={orientation:null,lastCoord:{left:null,top:null}};
this.__AG={minWidth:10,minHeight:10,maxWidth:null,maxHeight:null,autoHide:true,handles:3};

for(var name in this.__AG){if(q[name]){this.__AG[name]=q[name];
}}this.__AH();
},members:{__AF:null,__AB:null,__AC:null,__AD:null,__AG:null,__AE:false,getBox:function(){return this.__AB;
},__AH:function(){switch(this.__AG.handles){case 1:this.__AD.east=null;
break;
case 2:this.__AD.south=null;
break;
case 3:this.__AD.south=null;
this.__AD.southeast=null;
this.__AD.east=null;
break;
}for(var z in this.__AD){var y=this.__AD[z]=qx.bom.Element.create("div");
qx.bom.element.Attribute.set(y,"orientation",z);
qx.dom.Element.insertEnd(y,this.__AC);

if(this.__AC.id){y.id=this.__AC.id+"_"+z;
}this.__AI(z);
this.__AJ(z);
}},__AI:function(o){qx.bom.element.Class.add(this.__AD[o],o+"Handle");
qx.bom.element.Style.set(this.__AD[o],"visibility",this.__AG.autoHide?"hidden":"visible");
},__AJ:function(u){var w=qx.event.Registration;
var v=this.__AD[u];
w.addListener(v,"mousedown",this.__AM,this);
w.addListener(document.body,"mouseup",this.__AN,this);

if(this.__AG.autoHide){w.addListener(this.__AB.getElement(),"mouseover",this.__AK,this);
w.addListener(this.__AB.getElement(),"mouseout",this.__AL,this);
}},__AK:function(e){for(var t in this.__AD){qx.bom.element.Style.set(this.__AD[t],"visibility","visible");
}},__AL:function(e){if(!this.__AE){for(var s in this.__AD){qx.bom.element.Style.set(this.__AD[s],"visibility","hidden");
}}},__AM:function(e){e.stopPropagation();
if(e.isLeftPressed()){qx.event.Registration.addListener(document.body,"mousemove",this.__AO,this,true);
this.__AE=true;
var r=e.getTarget();
this.__AF.orientation=qx.bom.element.Attribute.get(r,"orientation");
this.__AF.lastCoord.left=e.getDocumentLeft();
this.__AF.lastCoord.top=e.getDocumentTop();
portal.box.Util.bringToFront(this.__AC);
}},__AN:function(e){e.stopPropagation();

if(this.__AE){qx.event.Registration.removeListener(document.body,"mousemove",this.__AO,this,true);
portal.box.Util.sendToBack(this.__AC);

if(this.__AG.autoHide){for(var x in this.__AD){qx.bom.element.Style.set(this.__AD[x],"visibility","hidden");
}}this.__AE=false;
qx.bom.Selection.clear(document.body);
}},__AO:function(e){e.stopPropagation();

if(this.__AE){var n={left:null,top:null};

switch(this.__AF.orientation){case "south":n.top=e.getDocumentTop();
break;
case "southeast":n.top=e.getDocumentTop();
n.left=e.getDocumentLeft();
break;
case "east":n.left=e.getDocumentLeft();
break;
}this.__AP(n,this.__AF.lastCoord);
this.__AF.lastCoord.left=e.getDocumentLeft();
this.__AF.lastCoord.top=e.getDocumentTop();
}},__AP:function(a,b){var j=qx.bom.element.Dimension;
var g=portal.box.Manager.getInstance().getGroupBoxDataOfBox(this.__AB.getId());
var i=j.getContentSize(g.element);

if(a.left){var f=a.left-b.left;

if(f!==0){var d=j.getContentWidth(this.__AC)+f;
var h=j.getWidth(this.__AC)+f;

if((this.__AG.minWidth==null||d>this.__AG.minWidth)&&(this.__AG.maxWidth==null||d<=this.__AG.maxWidth)){if(i.width>=h){qx.bom.element.Style.set(this.__AC,"width",d+'px');
}}}}
if(a.top){var f=a.top-b.top;

if(f!==0){var c=j.getContentHeight(this.__AC)+f;

if((this.__AG.minHeight==null||c>this.__AG.minHeight)&&(this.__AG.maxHeight==null||c<=this.__AG.maxHeight)){qx.bom.element.Style.set(this.__AC,"height",c+"px");
}}}}},destruct:function(){if(this.__AG.autoHide){qx.event.Registration.removeListener(this.__AB.getElement(),l,this.__AK,this);
qx.event.Registration.removeListener(this.__AB.getElement(),k,this.__AL,this);
}this.__AB=this.__AG=this.__AC=this.__AD=null;
this.__AF=null;
}});
})();
...
qx.Class.define(b,{statics:{__AS:1000,bringToFront:function(d){var e=qx.bom.element.Style.get(d,c);
e=e==a?0:e;
qx.bom.element.Style.set(d,c,e+this.__AS);
},sendToBack:function(f){var g=qx.bom.element.Style.get(f,c);
qx.bom.element.Style.set(f,c,g-this.__AS);
}}});
})();
...
(function(){var n="mousemove",m="mouseup",l="dragstart",k="mousedown",j="portal.box.Draggable",i="mouseover",h="drag",g="dragend",f="move",d="html",b="cursor",c="qx.client";
qx.Class.define(j,{extend:qx.core.Object,construct:function(q){qx.core.Object.call(this);
this.__AT=q;
this.__AU=q.getElement();
this.__AV=null;
this.__AW=null;
this.__AX();
},members:{__AT:null,__AU:null,__AV:null,__AW:null,getBox:function(){return this.__AT;
},getElement:function(){return this.__AU;
},__AX:function(){this.__AY();
this.__Ba();
},__AY:function(){this.__AV=qx.bom.Element.create("div");
qx.bom.element.Class.add(this.__AV,"dragHandle");
qx.dom.Element.insertBegin(this.__AV,this.__AU);
},__Ba:function(){qx.bom.Element.addListener(this.__AV,k,this.__Bb,this);
qx.bom.Element.addListener(this.__AV,i,function(e){qx.bom.element.Style.set(this,b,f);
},this.__AV);
qx.bom.Element.addListener(this.__AV,l,this.__Be,this);
qx.bom.Element.addListener(this.__AV,g,portal.dragdrop.Manager.getInstance().stopSession,portal.dragdrop.Manager.getInstance());
qx.bom.Element.addListener(this.__AV,h,this.__Bf,this);
},__Bb:function(e){if(e.isLeftPressed()){this.__AW={left:e.getDocumentLeft()-qx.bom.element.Location.getLeft(this.__AU),top:e.getDocumentTop()-qx.bom.element.Location.getTop(this.__AU)};
qx.bom.Element.addListener(document.body,m,this.__Bd,this,true);
this.__Bc();
qx.event.Registration.fireEvent(this.__AV,l,qx.event.type.Event);
}},__Bc:qx.core.Variant.select(c,{"mshtml":function(){var p=this;
var o=qx.lang.Function.bind(this.__Bd,p);
document.getElementsByTagName(d)[0].onmouseleave=o;
},"default":qx.lang.Function.empty}),__Bd:function(e){try{e.stopPropagation();
}catch(a){}
if(portal.dragdrop.Manager.getInstance().isSessionActive()){qx.event.Registration.removeListener(document.body,n,this.__Bf,this,true);
portal.dragdrop.Manager.getInstance().stopSession();
}qx.bom.Element.removeListener(document.body,m,this.__Bd,this,true);
},__Be:function(e){portal.box.Manager.getInstance().setActiveBox(this.__AT);
portal.dragdrop.Manager.getInstance().startSession(this.__AT);
qx.event.Registration.addListener(document.body,n,this.__Bf,this,true);
},__Bf:function(e){e.stopPropagation();
var r=e.getDocumentLeft()-this.__AW.left;
var top=e.getDocumentTop()-this.__AW.top;
qx.event.Timer.once(function(){this.checkGroupBox(r);
this.onDragMove(top,r);
},portal.dragdrop.Manager.getInstance(),0);
}},destruct:function(){this.__AT=this.__AV=this.__AU=this.__AW=null;
}});
})();
(function(){var B="px",A="px;",z="border",y="portal.box.Box",x="1px dashed red",w="top",v="Boolean",u="left",t="paddingTop",s="width:",l="height:",r="finish",o="opacity:0.5;margin:0;position:absolute;",h="left:",g="div",n="absolute",m="spring",p="paddingBottom",f="portal.dragdrop.Manager",q="singleton",k="top:";
qx.Class.define(f,{type:q,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__Bg={top:null,height:null};
this.__Bh={top:0,left:0};
},properties:{activeBox:{check:y,init:null,nullable:true},sessionActive:{check:v,init:false}},members:{__Bg:null,__Bi:null,__Bj:false,__Bh:null,__Bk:null,__Bl:null,startSession:function(a){this.setSessionActive(true);
this.setActiveBox(a);
this.__Bi=a.getGroupBoxId();
this.__Bm();
if(this.__Bk==null){this.__Bk={};
var b=portal.box.Manager.getInstance().getGroupBoxes();

for(var i=0,j=b.length;i<j;i++){this.__Bk[b[i].element.id]={left:qx.bom.element.Location.getLeft(b[i].element),right:qx.bom.element.Location.getLeft(b[i].element)+qx.bom.element.Dimension.getWidth(b[i].element)};
}}},__Bm:function(){if(!this.__Bl){this.__Bl=qx.bom.Element.create(g);
}var U=this.getActiveBox().getElement();
var S=qx.bom.element.Dimension.getContentSize(U);
this.__Bn(U,this.__Bl);
qx.bom.element.Class.add(this.__Bl,qx.bom.element.Class.get(U));
var T=o+k+qx.bom.element.Location.getTop(U)+A+h+qx.bom.element.Location.getLeft(U)+A+s+S.width+A+l+S.height+B;
qx.bom.element.Style.setCss(this.__Bl,T);
qx.bom.element.Style.set(U,z,x);
qx.dom.Node.getBodyElement(U).appendChild(this.__Bl);
portal.box.Util.bringToFront(this.__Bl);
},__Bn:function(C,D){while(C.firstChild){D.appendChild(C.firstChild);
}},stopSession:function(e){this.setSessionActive(false);
var F=this.getActiveBox();
var G=F.getElement();
portal.box.Manager.getInstance().updateGroupBoxMembers(F.getGroupBoxId(),this.__Bi,F);
F.setGroupBoxId(this.__Bi);
var E=new qx.fx.effect.core.Move(this.__Bl);
E.set({x:qx.bom.element.Location.getLeft(G),y:qx.bom.element.Location.getTop(G),mode:n,duration:0.5,transition:m});
E.start();
E.addListener(r,function(){this.__Bn(this.__Bl,G);
qx.bom.element.Style.reset(G,z);
this.__Bl.parentNode.removeChild(this.__Bl);

if(F.isActive()){F._applyActive(true);
}else{portal.box.Manager.getInstance().setActiveBox(F);
}},this);
},onDragMove:function(top,P){if(this.__Bh.left!=P){qx.bom.element.Style.set(this.__Bl,u,P+B);
}
if(this.__Bh.top!=top){qx.bom.element.Style.set(this.__Bl,w,top+B);
}var Q=this.getActiveBox().getElement();

if(this.__Bg.top==null){this.__Bg.top=qx.bom.element.Location.getTop(Q);
}
if(this.__Bg.height==null){this.__Bg.height=qx.bom.element.Dimension.getHeight(Q);
}if(this.__Bj==false&&top>=this.__Bg.top&&top<=(this.__Bg.top+this.__Bg.height)){return;
}var R;
if(this.__Bj){R=qx.dom.Hierarchy.getFirstDescendant(document.getElementById(this.__Bi));
while(R!=null){if(this.__Bo(true,top,Q,R)){this.__Bj=false;
return;
}R=qx.dom.Hierarchy.getNextElementSibling(R);
}}else{R=Q;
}if(top-this.__Bh.top>0){R=qx.dom.Hierarchy.getNextElementSibling(R);

while(R!=null){if(R){if(this.__Bo(true,top,Q,R)){return;
}}R=qx.dom.Hierarchy.getNextElementSibling(R);
}}else{var R=qx.dom.Hierarchy.getPreviousElementSibling(R);

while(R!=null){if(R){if(this.__Bo(false,top,Q,R)){return;
}}R=qx.dom.Hierarchy.getPreviousElementSibling(R);
}}this.__Bh.left=P;
this.__Bh.top=top;
},__Bo:function(H,top,I,J){var K=qx.bom.element.Location.getTop(J);
var M=parseInt(qx.bom.element.Style.get(J,t));
var N=parseInt(qx.bom.element.Style.get(J,p));
var L=qx.bom.element.Dimension.getContentHeight(J)+N+M;
var O;

if(H){if(top>=(K+(L/3))){O=qx.dom.Hierarchy.getNextElementSibling(J);

if(O!=I||O==null){qx.dom.Element.insertAfter(I,J);
this.__Bg.top=qx.bom.element.Location.getTop(I);
}return true;
}}else{if(top<=(K+L*2/3)){O=qx.dom.Hierarchy.getPreviousElementSibling(J);

if(O!=I||O==null){qx.dom.Element.insertBefore(I,J);
this.__Bg.top=qx.bom.element.Location.getTop(I);
}return true;
}}return false;
},checkGroupBox:function(c){if(this.__Bk[this.__Bi].left<=c&&this.__Bk[this.__Bi].right>=c){return;
}
for(var d in this.__Bk){if(d!=this.__Bi){if(this.__Bk[d].left<=c&&this.__Bk[d].right>=c){this.__Bi=d;
this.__Bj=true;
return;
}}}}},destruct:function(){this.__Bg=this.__Bh=this.__Bi=null;
this.__Bp=this.__Bl=this.__Bk=null;
}});
})();
