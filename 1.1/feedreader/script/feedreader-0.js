...
(function(){var p="error",o="http://resources.qooxdoo.org/proxy_1.php?mode=jsonp&proxy=",n="feedreader.io.FeedLoader",m="loaded",l="loading",k="completed",j="text/plain",h="GET",g="timeout",f="failed",d="singleton";
qx.Class.define(n,{extend:qx.core.Object,type:d,members:{loadAll:function(a){var c=a.getFeeds().getItem(0).getFeeds();

for(var i=0;i<c.length;i++){this.load(c.getItem(i));
}var b=a.getFeeds().getItem(1).getFeeds();

for(i=0;i<b.length;i++){this.load(b.getItem(i));
}},load:function(r){r.setState(l);
var t,s;
t=o+encodeURIComponent(r.getUrl());
s=new qx.io.remote.Request(t,h,j);
s.setCrossDomain(true);
s.setTimeout(30000);
s.addListener(k,this.__zB(r),this);
var u=qx.lang.Function.bind(this.__zA,this,r);
s.addListener(g,u,this);
s.addListener(f,u,this);
s.send();
},__zA:function(z,e){z.setState(p);
},__zB:function(y){return function(v){var x=v.getContent();
if(x==null){this.warn("Empty feed content: "+y.getUrl());
y.setState(p);
}else{try{y.getArticles().splice(0,y.getArticles().length);
var w=feedreader.io.FeedParser.parseFeed(x);

for(var i=0;i<w.length;i++){y.getArticles().push(w[i]);
}y.setState(m);
}catch(q){y.setState(p);
this.warn("Could not parse feed: "+y.getUrl());
}}};
}}});
})();
...
qx.Class.define(d,{extend:qx.core.Object,construct:function(o,p,q){qx.core.Object.call(this);
...
(function(){var q="static",p="user",o="execute",n="loading",m="title",l="north",k="main",j="new",h="Surfin' Safari",g="changeModel",by="__zP",bx="selection[0].state",bw="Heise",bv="__zK",bu="__zO",bt="F1",bs="article",br="partLoaded",bq="__zM",bp="Control+A",x="http://blogs.msdn.com/ie/rss.xml",y="http://my.opera.com/desktopteam/xml/rss/blog/",v="settings",w="feedreader/images/loading22.gif",t="error",u="selection[0]",r="change",s="http://blogs.msdn.com/jscript/rss.xml",F="loaded",G="feeds",S="The Mozilla Blog",O="enabled",bb="http://webkit.org/blog/feed/",V="separator-vertical",bl="http://feeds2.feedburner.com/qooxdoo/news/content",bg="http://blog.mozilla.com/feed/",K="vertical",bo="__zD",bn="http://www.heise.de/newsticker/heise-atom.xml",bm="__zH",J="__zC",M="__zJ",N="__zQ",Q="state",T="__zN",W="JScript Team Blog",bd="Feeds",bi="http://feeds2.feedburner.com/ajaxian",z="Control+D",A="__zL",L="icon/22/actions/process-stop.png",ba="selection[0].articles",Y="white",X="IEBlog",bf="Ajaxian",be="User Feeds",U="__zG",bc="icon/22/apps/internet-feed-reader.png",c="model",bh="feedreader.Application",B="http://daringfireball.net/index.xml",C="Control+P",P="Static Feeds",d="Control+R",f="qooxdoo News",I="selection[0].category",D="Opera Desktop Blog",E="feedreader.model.Feed",H="Daring Fireball",R="__zF",bk="addfeed",bj="__zI";
qx.Class.define(bh,{extend:qx.application.Standalone,members:{__zC:null,__zD:null,__zE:null,__zF:null,__zG:null,__zH:null,__zI:null,__zJ:null,__zK:null,__zL:null,__zM:null,__zN:null,__zO:null,__zP:null,__zQ:null,main:function(){qx.application.Standalone.prototype.main.call(this);
{};
qx.io.PartLoader.getInstance().addListener(br,function(e){this.debug("part loaded: "+e.getData().getName());
},this);
this._initializeCommands();
this._createLayout();
this._initializeModel();
this._setUpBinding();
this.__zF.getRoot().setOpen(true);
this.__zF.getRoot().getChildren()[0].setOpen(true);
this.__zF.getRoot().getChildren()[1].setOpen(true);
this.__zF.setHideRoot(true);
this.__zC.getSelection().push(this.__zQ.getFeeds().getItem(0));
},finalize:function(){qx.application.Standalone.prototype.finalize.call(this);
this.reload();
},_initializeModel:function(){this.__zO=new feedreader.model.FeedFolder(bd);
this.__zQ=new feedreader.model.FeedFolder(this.tr(P));
this.__zO.getFeeds().push(this.__zQ);
this.__zQ.getFeeds().push(new feedreader.model.Feed(f,bl,q));
this.__zQ.getFeeds().push(new feedreader.model.Feed(W,s,q));
this.__zQ.getFeeds().push(new feedreader.model.Feed(H,B,q));
this.__zQ.getFeeds().push(new feedreader.model.Feed(h,bb,q));
this.__zQ.getFeeds().push(new feedreader.model.Feed(bf,bi,q));
this.__zP=new feedreader.model.FeedFolder(this.tr(be));
this.__zO.getFeeds().push(this.__zP);
this.__zP.getFeeds().push(new feedreader.model.Feed(bw,bn,p));
this.__zP.getFeeds().push(new feedreader.model.Feed(X,x,p));
this.__zP.getFeeds().push(new feedreader.model.Feed(S,bg,p));
this.__zP.getFeeds().push(new feedreader.model.Feed(D,y,p));
},_setUpBinding:function(){var bH={converter:this._state2iconConverter};
this.__zC=new qx.data.controller.Tree(this.__zO,this.__zF,G,m);
this.__zC.setIconOptions(bH);
this.__zC.setIconPath(Q);
this.__zD=new qx.data.controller.List(null,this.__zI.getList(),m);
this.__zC.bind(ba,this.__zD,c);
this.__zD.bind(u,this.__zM,bs);
this.__zD.getSelection().addListener(r,this._listControllerChange,this);
this.__zD.addListener(g,this._treeControllerChange,this);
var bI={converter:this._state2loadingConverter};
this.__zC.bind(bx,this.__zI,n,bI);
bI={converter:this._category2enabledConverter};
this.__zC.bind(I,this.__zH.getRemoveButton(),O,bI);
},_category2enabledConverter:function(bB){if(bB==p){return true;
}return false;
},_state2iconConverter:function(bC){if(bC==j||bC==n){return w;
}else if(bC==F){return bc;
}else if(bC==t){return L;
}return null;
},_state2loadingConverter:function(a){if(a==j||a==n){return true;
}return false;
},_listControllerChange:function(bL){var bM=this.__zC.getSelection().getItem(0);
var bN=this.__zD.getSelection().getItem(0);
if(bN!=undefined){bM.setSelectedArticle(bN);
}},_treeControllerChange:function(bJ){var bK=this.__zC.getSelection().getItem(0);
if(bK!=null&&bK.classname==E&&bK.getSelectedArticle()!=null&&bK.getArticles().contains(bK.getSelectedArticle())){this.__zD.getSelection().push(bK.getSelectedArticle());
}else{this.__zI.getList().scrollToY(0);
}},_createLayout:function(){var bD=new qx.ui.layout.Dock();
bD.setSeparatorY(V);
var bE=new qx.ui.container.Composite(bD);
this.getRoot().add(bE,{edge:0});
this.__zG=new feedreader.view.Header();
bE.add(this.__zG,{edge:l});
this.__zH=new feedreader.view.ToolBar(this);
bE.add(this.__zH,{edge:l});
this.__zJ=new qx.ui.splitpane.Pane();
bE.add(this.__zJ);
this.__zF=new qx.ui.tree.Tree();
this.__zF.setWidth(250);
this.__zF.setBackgroundColor(Y);
this.__zJ.add(this.__zF,0);
this.__zK=new qx.ui.splitpane.Pane(K);
this.__zK.setDecorator(null);
this.__zJ.add(this.__zK,1);
this.__zI=new feedreader.view.List(this.__zO);
this.__zI.setHeight(200);
this.__zI.setDecorator(k);
this.__zK.add(this.__zI,0);
this.__zM=new feedreader.view.Article();
this.__zM.setDecorator(k);
this.__zK.add(this.__zM,1);
},_initializeCommands:function(){var bA={};
bA.reload=new qx.ui.core.Command(d);
bA.reload.addListener(o,this.reload,this);
bA.about=new qx.ui.core.Command(bt);
bA.about.addListener(o,this.showAbout,this);
bA.preferences=new qx.ui.core.Command(C);
bA.preferences.addListener(o,this.showPreferences,this);
bA.addFeed=new qx.ui.core.Command(bp);
bA.addFeed.addListener(o,this.showAddFeed,this);
bA.removeFeed=new qx.ui.core.Command(z);
bA.removeFeed.addListener(o,this.removeFeed,this);
this.__zE=bA;
},getCommand:function(b){return this.__zE[b];
},addFeed:function(bO,bP,bQ){var bR=new feedreader.model.Feed(bO,bP,bQ);
this.__zP.getFeeds().push(bR);
var bS=feedreader.io.FeedLoader.getInstance();
bS.load(bR);
},removeFeed:function(){var bF=this.__zC.getSelection().getItem(0);
if(bF&&bF.getCategory&&bF.getCategory()!==q){var bG=this.__zP.getFeeds();
for(var i=0;i<bG.length;i++){if(bF===bG.getItem(i)){bG.splice(i,1);
return;
}}}},reload:function(){var bz=feedreader.io.FeedLoader.getInstance();
bz.loadAll(this.__zO);
},showPreferences:function(){qx.io.PartLoader.require([v],function(){if(!this.__zL){this.__zL=new feedreader.view.PreferenceWindow();
this.getRoot().add(this.__zL);
}this.__zL.center();
this.__zL.open();
},this);
},showAbout:function(){alert(this.tr("FeedReader (qooxdoo powered)"));
},showAddFeed:function(){qx.io.PartLoader.require([bk],function(){if(!this.__zN){this.__zN=new feedreader.view.AddFeedWindow(this);
this.getRoot().add(this.__zN);
}this.__zN.center();
this.__zN.open();
},this);
}},destruct:function(){this.__zE=null;
this._disposeObjects(bm,bj,bq,R,bu,M,bv,U,N,by,J,bo,A,T);
}});
})();
...
C.add(D);
this.__zR=new qx.ui.toolbar.Button(this.tr(l),o);
this.__zR.setCommand(v.getCommand(s));
this.__zR.setEnabled(false);
C.add(this.__zR);
C.addSeparator();
...
B.add(A);
},members:{__zR:null,getRemoveButton:function(){return this.__zR;
}},destruct:function(){this._disposeObjects(g);
}});
})();
...
(function(){var k="",j="@attributes",h="en_US",g="feedreader.io.FeedParser",f="yyyy-MM-d'T'HH:mm:ss'Z'",e="EEE, d MMM yyyy HH:mm:ss Z",d="EEE, d MMM yyyy HH:mm:ss z",c="yyyy-MM-d'T'HH:mm:ssZ";
qx.Class.define(g,{statics:{_rssDate1:new qx.util.format.DateFormat(e,h),_rssDate2:new qx.util.format.DateFormat(d,h),_atomDate1:new qx.util.format.DateFormat(f,h),_atomDate2:new qx.util.format.DateFormat(c,h),parseFeed:function(n){var o=[];
...
}catch(b){try{s=this._rssDate2.parse(s);
}catch(v){}}if(!(s instanceof Date)){throw new Error("RSS Date Error: "+s);
s=null;
}var r=new feedreader.model.Article();
r.set({title:t.title||k,author:null,date:s,content:t.description||k,link:t.link||k});
q.push(r);
}return q;
...
}catch(u){try{z=this._atomDate2.parse(z);
}catch(m){}}if(!(z instanceof Date)){throw new Error("Atom Date Error: "+z);
z=null;
}var y=new feedreader.model.Article();
y.set({title:A.title||A.summary||k,author:A.author?A.author.name||null:null,date:z,content:A.content||A.summary||k});

if(qx.lang.Type.isArray(A.link)){y.setLink(A.link[0][j]?A.link[0][j].href||k:k);
}else{y.setLink(A.link[j]?A.link[j].href||k:k);
}x.push(y);
}return x;
}}});
})();
...
(function(){var n="separator-vertical",m="background-medium",l="Posts",k="bold",j="__zS",i="__zU",h="_applyLoading",g="center",f="feedreader.view.List",e="single",b="__zT",d="middle",c="feedreader/images/loading66.gif",a="Boolean";
...
q.setFont(k);
this._add(q);
this.__zS=new qx.ui.container.Stack();
this._add(this.__zS,{flex:1});
this.__zT=new qx.ui.form.List();
this.__zT.setDecorator(null);
this.__zT.setSelectionMode(e);
this.__zS.add(this.__zT);
this.__zU=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,g));
var o=new qx.ui.basic.Image(c);
o.setAlignY(d);
this.__zU.add(o);
this.__zS.add(this.__zU);
},properties:{loading:{check:a,init:false,apply:h}},members:{__zS:null,__zT:null,__zU:null,_applyLoading:function(r,s){if(r){this.__zS.setSelection([this.__zU]);
}else{this.__zS.setSelection([this.__zT]);
}},getList:function(){return this.__zT;
}},destruct:function(){this._disposeObjects(b,j,i);
}});
})();
...
if(!feedreader.view.Article.__Ao){feedreader.view.Article.__Ao=true;
...
w.setLineHeight(1.8);
this.setFont(w);
},properties:{article:{apply:e,nullable:true,check:d}},members:{_applyArticle:function(x,y){var z=this.__Ap();
this.setHtml(z);
qx.html.Element.flush();
var B=this.getContentElement().getDomElement();
var A=B.getElementsByTagName(b);
for(var i=0;i<A.length;i++){A[i].target=u;
}},__Ap:function(){var C=this.getArticle();
if(!C){return q;
}var D=new qx.util.StringBuilder();
D.add(c);
D.add(f,C.getTitle(),j);
D.add(n,C.getDate(),v);
D.add(r,C.getContent(),v);
D.add(o,C.getLink(),k);
D.add(v);
return D.get();
}}});
