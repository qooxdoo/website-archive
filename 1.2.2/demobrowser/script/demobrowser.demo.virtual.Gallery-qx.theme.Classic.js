...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.virtual.Gallery","qx.theme":"qx.theme.Classic"};
...
(function(){var t="",s="selected",r="cell",q="layer",p="icon",o="#f3f3f3",n="label",m="Gallery (HTML - divs)",l="manager",k="default",X="demobrowser.demo.virtual.Gallery",W="demobrowser.demo.virtual.WidgetGallery",V="text-align: center;",U="auto",T="color: white; background-color: #00398D;",S="user-desktop.png",R="<img src='",Q="'></img>",P="group",O="resize",A="folder.png",B="vertical",y="multi",z="network-server.png",w="user-trash.png",x="off",u="top",v="icon/32/places/",C="horizontal",D="demobrowser.demo.virtual.AbstractGallery",G="abstract",F="position: absolute;",I="demobrowser.demo.virtual.HtmlGallery",H="Gallery (widgets)",K="<br>",J="Icon #",E="__Mu",N="network-workgroup.png",M="text-selected",L="demobrowser.demo.virtual.GalleryCell";
...
new demobrowser.demo.virtual.WidgetGallery(H);
var bE=new demobrowser.demo.virtual.HtmlGallery(m);
bE.moveTo(400,50);
}}});
qx.Class.define(D,{extend:qx.ui.window.Window,type:G,construct:function(bC){qx.ui.window.Window.call(this,bC);
...
var bD=this._createScroller();
bD.set({scrollbarX:x,scrollbarY:U});
bD.getPane().addListener(O,this._onPaneResize,this);
this.add(bD);
this.manager=new qx.ui.virtual.selection.CellRectangle(bD.getPane(),this).set({mode:y,drag:true});
this.manager.attachMouseEvents();
this.manager.attachKeyEvents(bD);
},members:{getItemData:function(bo,bp){return this.items[bo*this.itemPerLine+bp];
},_createScroller:function(){},isItemSelectable:function(bF){return !!this.getItemData(bF.row,bF.column);
},styleSelectable:function(br,bs,bt){},_onPaneResize:function(e){var bi=e.getTarget();
var bg=e.getData().width;
var bh=Math.floor(bg/this.itemWidth);

if(bh==this.itemsPerLine){return;
}this.itemPerLine=bh;
var bj=Math.ceil(this.itemCount/bh);
bi.getColumnConfig().setItemCount(bh);
bi.getRowConfig().setItemCount(bj);
},_generateItems:function(bM){var bP=[];
var bS=[A,w,z,N,S];
var bQ=qx.util.AliasManager.getInstance();
var bO=qx.util.ResourceManager.getInstance();

for(var i=0;i<bM;i++){var bT=v+bS[Math.floor(Math.random()*bS.length)];
var bN=bQ.resolve(bT);
var bR=bO.toUri(bN);
bP[i]={label:J+(i+1),icon:bT,resolvedIcon:bR};
}return bP;
}},destruct:function(){this.items=null;
this._disposeObjects(l);
}});
qx.Class.define(L,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var d=new qx.ui.basic.Atom().set({iconPosition:u});
d.getChildControl(n).set({padding:[0,4]});
d.getChildControl(p).set({padding:4});
return d;
},updateData:function(Y,ba){Y.set({icon:ba.icon,label:ba.label});
},updateStates:function(bk,bl){var bn=bk.getChildControl(n);
var bm=bk.getChildControl(p);

if(bl.selected){bn.setDecorator(s);
bn.setTextColor(M);
bm.setDecorator(P);
}else{bn.resetDecorator();
bn.resetTextColor();
bm.resetDecorator();
}}}});
qx.Class.define(W,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bB){demobrowser.demo.virtual.AbstractGallery.call(this,bB);
this.__Mu=new demobrowser.demo.virtual.GalleryCell();
},members:{_createScroller:function(){var bq=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.WidgetCell(this);
bq.getPane().addLayer(this.layer);
new qx.ui.virtual.behavior.Prefetch(bq,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:200,maxAbove:300,minBelow:600,maxBelow:800}).set({interval:500});
return bq;
},styleSelectable:function(bv,bw,bx){if(bw!==s){return;
}var by=this.layer.getChildren();

for(var i=0;i<by.length;i++){var bz=by[i];
var bA=bz.getUserData(r);

if(bv.row!==bA.row||bv.column!==bA.column){continue;
}
if(bx){this.__Mu.updateStates(bz,{selected:1});
}else{this.__Mu.updateStates(bz,{});
}}},getCellWidget:function(bG,bH){var bL=this.getItemData(bG,bH);

if(!bL){return null;
}var bI={row:bG,column:bH};
var bJ={};

if(this.manager.isItemSelected(bI)){bJ.selected=true;
}var bK=this.__Mu.getCellWidget(bL,bJ);
bK.setUserData(r,bI);
return bK;
},poolCellWidget:function(bu){this.__Mu.pool(bu);
}},destruct:function(){this._disposeObjects(E,q);
}});
qx.Class.define(I,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(h){demobrowser.demo.virtual.AbstractGallery.call(this,h);
var j=qx.theme.manager.Font.getInstance().resolve(k).getStyles();
this._fontCss=qx.bom.element.Style.compile(j);
},members:{_createScroller:function(){var g=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.HtmlCell(this);
g.getPane().addLayer(this.layer);
var f=new qx.ui.virtual.layer.GridLines(C,o);
g.getPane().addLayer(f);
var f=new qx.ui.virtual.layer.GridLines(B,o);
g.getPane().addLayer(f);
return g;
},_onPaneResize:function(e){demobrowser.demo.virtual.AbstractGallery.prototype._onPaneResize.call(this,e);
this.manager.clearSelection();
},styleSelectable:function(a,b,c){this.layer.updateLayerData();
},getCellProperties:function(bb,bc){var bf=this.getItemData(bb,bc);

if(!bf){return t;
}var be=this.manager.isItemSelected({row:bb,column:bc});
var bd=be?T:t;
return {style:[F,V,this._fontCss,bd].join(t),content:[R,bf.resolvedIcon,Q,K,bf.label].join(t)};
...
},_getSelectableFromMouseEvent:function(event){var bb=this._pane.getCellAtPosition(event.getDocumentLeft(),event.getDocumentTop());

if(!bb){return null;
}return this._isSelectable(bb)?bb:null;
},getSelectables:function(D){var F=[];
var I=this._pane.getRowConfig().getItemCount();
var E=this._pane.getColumnConfig().getItemCount();

for(var J=0;J<I;J++){for(var G=0;G<E;G++){var H={row:J,column:G};

if(this._isSelectable(H)){F.push(H);
}}}return F;
},_getSelectableRange:function(t,u){var x=[];
var y=Math.min(t.row,u.row);
var z=Math.max(t.row,u.row);
var w=Math.min(t.column,u.column);
var B=Math.max(t.column,u.column);

for(var C=y;C<=z;C++){for(var A=w;A<=B;A++){var v={row:C,column:A};

if(this._isSelectable(v)){x.push(v);
}}}return x;
},_getFirstSelectable:function(){var P=this._pane.getRowConfig().getItemCount();
var O=this._pane.getColumnConfig().getItemCount();

for(var S=0;S<P;S++){for(var R=0;R<O;R++){var Q={row:S,column:R};

if(this._isSelectable(Q)){return Q;
}}}return null;
},_getLastSelectable:function(){var i=this._pane.getRowConfig().getItemCount();
var h=this._pane.getColumnConfig().getItemCount();

for(var k=h-1;k>=0;k--){for(var l=i-1;l>=0;l--){var j={row:l,column:k};

if(this._isSelectable(j)){return j;
}}}return null;
},_getRelatedSelectable:function(m,n){var o={row:m.row,column:m.column};

switch(n){case e:for(var s=m.row-1;s>=0;s--){o.row=s;

if(this._isSelectable(o)){return o;
}}break;
case f:var p=this._pane.getRowConfig().getItemCount();

for(var s=m.row+1;s<p;s++){o.row=s;

if(this._isSelectable(o)){return o;
}}break;
case d:for(var r=m.column-1;r>=0;r--){o.column=r;

if(this._isSelectable(o)){return o;
}}break;
case c:var q=this._pane.getColumnConfig().getItemCount();

for(var r=m.column+1;r<q;r++){o.column=r;
...
}},_selectableToHashCode:function(g){return g.column+b+g.row;
},_scrollItemIntoView:function(ba){this._pane.scrollCellIntoView(ba.column,ba.row);
},_getSelectableLocationX:function(K){var L=this._pane.getColumnConfig();
var N=L.getItemPosition(K.column);
var M=N+L.getItemSize(K.column)-1;
return {left:N,right:M};
},_getSelectableLocationY:function(T){var W=this._pane.getRowConfig();
var V=W.getItemPosition(T.row);
var U=V+W.getItemSize(T.row)-1;
return {top:V,bottom:U};
}}});
})();
...
(function(){var k="scroll",j="scrollbar-x",i="scrollbar-y",h="_applyInterval",g="appear",f="interval",e="qx.ui.virtual.behavior.Prefetch",d="_applyScroller",c="qx.ui.virtual.core.Scroller",b="__Hg",a="Integer";
qx.Class.define(e,{extend:qx.core.Object,construct:function(l,m){{};
...
this.__Hg=new qx.event.Timer(this.getInterval());
this.__Hg.addListener(f,this._onInterval,this);

if(l){this.setScroller(l);
}},properties:{scroller:{check:c,nullable:true,init:null,apply:d},interval:{check:a,init:200,apply:h}},members:{__Hh:null,__Hi:null,__Hg:null,__Hj:null,__Hk:null,setPrefetchX:function(r,s,t,u){this.__Hh=[r,s,t,u];
},setPrefetchY:function(v,w,x,y){this.__Hi=[v,w,x,y];
},_onInterval:function(){var z=this.__Hh;

if(z[1]&&z[3]){this.getScroller().getPane().prefetchX(z[0],z[1],z[2],z[3]);
qx.ui.core.queue.Manager.flush();
}var A=this.__Hi;

if(A[1]&&A[3]){this.getScroller().getPane().prefetchY(A[0],A[1],A[2],A[3]);
qx.ui.core.queue.Manager.flush();
}},_applyScroller:function(n,o){if(o){if(this.__Hj){o.getChildControl(j).removeListenerById(this.__Hj);
}
if(this.__Hk){o.getChildControl(i).removeListenerById(this.__Hk);
}}
if(n){if(!n.getContainerElement().getDomElement()){this.__Hg.stop();
n.addListenerOnce(g,this.__Hg.start,this.__Hg);
}else{this.__Hg.restart();
}this.__Hj=n.getChildControl(j).addListener(k,this.__Hg.restart,this.__Hg);
this.__Hk=n.getChildControl(i).addListener(k,this.__Hg.restart,this.__Hg);
}else{this.__Hg.stop();
}},_applyInterval:function(p,q){this.__Hg.setInterval(p);
}},destruct:function(){this.setScroller(null);
this.__Hh=this.__Hi=null;
this._disposeObjects(b);
}});
})();
(function(){var n="",m="px;",l="' ",k="qx.ui.virtual.layer.HtmlCell",j="width:",i="top:",h=">",g="</div>",f="<div ",e="style='",b="height:",d="class='",c="left:",a="html";
qx.Class.define(k,{extend:qx.ui.virtual.layer.Abstract,construct:function(H){qx.ui.virtual.layer.Abstract.call(this);
this.setZIndex(2);
{};
...
s+=b+p+m;
return s;
},_fullUpdate:function(t,u,v,w){var E=[];
var F=0;
var top=0;
var G=t;
var A=u;

for(var y=0;y<v.length;y++){var F=0;
var A=u;
var B=v[y];

for(var x=0;x<w.length;x++){var D=w[x];
var z=this._cellProvider.getCellProperties(G,A);
var C=z.insets||[0,0];
E.push(f,e,c,F,m,i,top,m,this._getCellSizeStyle(D,B,C[0],C[1]),z.style||n,l,d,z.classes||n,l,z.attributes||n,h,z.content||n,g);
A++;
F+=D;
}top+=B;
G++;
}this.getContentElement().setAttribute(a,E.join(n));
...
qx.Class.define(d,{extend:qx.ui.virtual.layer.Abstract,construct:function(X,Y,ba){qx.ui.virtual.layer.Abstract.call(this);
this.setZIndex(1);

if(Y){this.setDefaultLineColor(Y);
}
if(ba!==undefined){this.setDefaultLineSize(ba);
}this.__Do=[];
this.__Dp=[];
this._isHorizontal=(X||s)==s;
},properties:{defaultLineColor:{init:f,check:j,themeable:true},defaultLineSize:{init:1,check:c,themeable:true}},members:{__Do:null,__Dp:null,isHorizontal:function(){return this._isHorizontal;
},setLineColor:function(z,A){{};
this.__Do[z]=A;

if(this.__Dq(z)){this.updateLayerData();
}},setLineSize:function(a,b){{};
this.__Dp[a]=b;

if(this.__Dq(a)){this.updateLayerData();
}},__Dq:function(S){if(this._isHorizontal){var W=this.getFirstColumn();
var U=W+this.getColumnSizes().length-1;
return S>=W&&S<=U;
}else{var V=this.getFirstRow();
var T=V+this.getRowSizes().length-1;
return S>=V&&S<=T;
}},getLineSize:function(M){return this.__Dp[M]||this.getDefaultLineSize();
},getLineColor:function(w){return this.__Do[w]||this.getDefaultLineColor();
},__Dr:function(H,I,J){var top=0;
var K,L;

for(var y=0;y<J.length-1;y++){K=this.getLineColor(I+y);
L=this.getLineSize(I+y);
top+=J[y];
H.push(q,r,m+L+v,l,o,top-((L>1)?Math.floor(L/2):1),v,t,K,p,u);
}},__Ds:function(B,C,D){var G=0;
var E,F;

for(var x=0;x<D.length-1;x++){E=this.getLineColor(C+x);
F=this.getLineSize(C+x);
G+=D[x];
B.push(q,r,i+F+v,e,k,h,G-((F>1)?Math.floor(F/2):1),v,t,E,p,u);
}},_fullUpdate:function(N,O,P,Q){var R=[];

if(this._isHorizontal){this.__Dr(R,N,P);
}else{this.__Ds(R,O,Q);
}this.getContentElement().setAttribute(g,R.join(n));
