...
(function(){var n="px",m="moveable",k="demobrowser.demo.bom.Location",j="borderBottomWidth",h="list",g='#',f="mode",e="borderTopWidth",d="borderLeftWidth",c="a",a="div",b="borderRightWidth";
qx.Class.define(k,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);
var o=document.getElementById(h).getElementsByTagName(c);

for(var i=0,l=o.length;i<l;i++){o[i].onclick=demobrowser.demo.bom.Location.jump;
}}},statics:{num:function(q,r){return parseInt(qx.bom.element.Style.get(q,r,qx.bom.element.Style.COMPUTED_MODE,false),10);
},jump:function(event){if(!event){event=window.event;
}event.returnValue=false;

if(event.preventDefault){event.preventDefault();
}function s(x,y){return parseInt(qx.bom.element.Style.get(x,y,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var u=this.href;
var w=u.substr(u.indexOf(g)+1);
var t=document.getElementById(w);
var location=qx.bom.element.Location.get(t,document.getElementById(f).value);
var v=document.getElementById(m);
v.style.left=location.left+n;
v.style.top=location.top+n;

if(qx.bom.client.Feature.BORDER_BOX){v.style.width=(location.right-location.left)+n;
v.style.height=(location.bottom-location.top)+n;
}else{v.style.width=(location.right-location.left-s(v,d)-s(v,b))+n;
v.style.height=(location.bottom-location.top-s(v,e)-s(v,j))+n;
}return false;
},randomize:function(){var p=document.body.getElementsByTagName(a);

for(var i=0,l=p.length;i<l;i++){if(p[i].id&&p[i].id!=m){p[i].style.margin=Math.round(Math.random()*10)+n;
p[i].style.padding=Math.round(Math.random()*10)+n;
p[i].style.borderWidth=Math.round(Math.random()*10)+n;
}}}}});
})();
