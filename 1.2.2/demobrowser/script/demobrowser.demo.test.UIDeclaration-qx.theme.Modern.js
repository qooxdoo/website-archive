...
var C={clazz:qx.ui.form.Form,children:[{type:q,label:x},{type:l,label:g,validator:null,name:f,widget:{clazz:qx.ui.form.TextField,required:true,width:200}},{type:l,label:j,widget:{clazz:qx.ui.form.TextField}},{type:l,label:n,widget:{clazz:qx.ui.form.TextField}},{type:l,label:d,widget:{clazz:qx.ui.form.TextField}},{type:l,label:u,widget:{clazz:qx.ui.form.DateField}},{type:k,widget:{clazz:qx.ui.form.Button,label:p}},{type:k,widget:{clazz:qx.ui.form.Button,label:s}}]};
var D={clazz:qx.ui.window.Window,layout:{clazz:qx.ui.layout.VBox,spacing:10},layoutProperties:{left:20,top:20},showStatusbar:true,status:c,caption:y,icon:b,id:t,listeners:[{event:z,handler:function(e){this.debug("Moved to: "+e.getData().left+"x"+e.getData().top);
}},{event:v,handler:function(e){this.debug("Resized to: "+e.getData().width+"x"+e.getData().height);
}}],children:[{clazz:qx.ui.basic.Atom,label:o,icon:r,rich:true},{clazz:qx.ui.tabview.TabView,layoutProperties:{flex:1},children:[{clazz:qx.ui.tabview.Page,label:h,layoutProperties:{flex:1},layout:{clazz:qx.ui.layout.Canvas},children:[C]},{clazz:qx.ui.tabview.Page,label:w,layoutProperties:{flex:1}},{clazz:qx.ui.tabview.Page,label:a,layoutProperties:{flex:1}}]}]};
this.addFromDefinition(D);
this.win.open();
},addFromDefinition:function(A){var B=this.getRoot();
this.__LE(B,A);
},__LE:function(I,J){var K=new J.clazz();
var O=qx.lang.Object.clone(J);
delete O.clazz;
delete O.layout;
delete O.children;
delete O.id;
delete O.listeners;
K.set(O);
if(J.id!=null){this[J.id]=K;
}if(J.layout!=null){var L=new J.layout.clazz();
var O=qx.lang.Object.clone(J.layout);
delete O.clazz;
L.set(O);
K.setLayout(L);
}var N=J.children;

if(N!=null){for(var i=0;i<N.length;i++){if(K instanceof qx.ui.form.Form){this.__LF(K,N[i]);
}else{this.__LE(K,N[i]);
}}}if(K instanceof qx.ui.form.Form){I.add(new qx.ui.form.renderer.Single(K));
}else{I.add(K,J.layoutProperties);
}if(J.listeners){for(var i=0;i<J.listeners.length;i++){var M=J.listeners[i];
K.addListener(M.event,M.handler,M.context);
}}},__LF:function(E,F){if(F.type==="Header"){E.addGroupHeader(F.label);
return;
}var G=new F.widget.clazz();
var H=qx.lang.Object.clone(F.widget);
delete H.clazz;
G.set(H);

if(F.type==="Item"){E.add(G,F.label,F.validator,F.name);
}else if(F.type==="Button"){E.addButton(G);
}}}});
})();
