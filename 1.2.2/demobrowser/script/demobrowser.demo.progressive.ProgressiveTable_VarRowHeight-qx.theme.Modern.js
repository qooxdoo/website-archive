...
(function(){var H="row",G="small icon",F="This, too, is tall.",E="No attributes specified.",D="1*",C="demobrowser/demo/icons/",B="",A="TALL ICON",z="I am tall.",y="red",s="demobrowser.demo.progressive.ProgressiveTable_VarRowHeight",x="format-justify-left.png",v="I am small.",q="end",p="multimedia-player.png",u="This, too, is small.",t="Id",w="Image",o="Note variable row heights in this table.";
qx.Class.define(s,{extend:qx.application.Standalone,members:{__MS:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var M=C;
this.imageData=[{url:M+x,height:4,tooltip:G},{url:M+p,height:4,tooltip:A},B];
this.__MS=0;
...
}I=K.resolve(this.imageData[i].url);
L=J.toUri(I);
this.imageData[i].url=L;
++this.__MS;
qx.io.ImageLoader.load(L,function(N,O){this.info("Searching for ["+N+"]");
for(var j=0;j<this.imageData.length;j++){if(this.imageData[j].url==N){this.imageData[j].height=O.height;
--this.__MS;
this.debug("Found ["+this.imageData[j].url+"].  loadCount="+this.__MS+", end="+this.imageData.length);
if(this.__MS==0){this.doLoad();
}return;
}}throw new Error("Image data for "+N+" not found");
},this);
}if(this.__MS==0){this.doLoad();
}},doLoad:function(){var b=this;
var k=function(P,Q){var R={renderer:H,location:q,data:[Q,b.imageData[P]]};
return R;
};
...
h.setWidth(1,D);
var d=[t,w];
var e=new qx.ui.progressive.headfoot.TableHeading(h,d);
var c=new qx.ui.progressive.headfoot.Progress(h,d);
var n=new qx.ui.progressive.structure.Default(e,c);
var a=new qx.ui.progressive.Progressive(n);
var l=new qx.ui.basic.Atom(o);
l.setHeight(16);
l.setTextColor(y);
a.add(l);
...
g.addElements(f);
a.setDataModel(g);
var m=new qx.ui.progressive.renderer.table.Row(h);
a.addRenderer(H,m);
var r=new qx.ui.progressive.renderer.table.cell.Image();
m.addRenderer(1,r);
a.set({width:400,maxWidth:400});
this.getRoot().add(a,{left:50,top:50,bottom:50});
a.render();
}}});
})();
...
var w=qx.util.AliasManager.getInstance();
var x=qx.util.ResourceManager.getInstance();
var y=w.resolve(k);
this.__Dx=x.toUri(y);
},members:{__Dx:null,getBlankImage:function(){return this.__Dx;
},_identifyImage:function(E){throw new Error("_identifyImage() is abstract");
},_getCellStyle:function(F){var G=qx.ui.progressive.renderer.table.cell.Abstract.prototype._getCellStyle.call(this,F)+u+d;
return G;
},_getContentHtml:function(B){var C=[];
var D=this.__Dy(B);
C.push(b);
if(qx.core.Variant.isSet(f,q)&&/\.png$/i.test(D.url)){C.push(i,this.__Dx,n,c,D.url,l,j);
}else{C.push(i,D.url,j);
}if(D.imageWidth){C.push(p,D.imageWidth,h);
}if(D.imageHeight){C.push(m,D.imageHeight,h);
}C.push(v);
if(D.tooltip){C.push(s,D.tooltip,r);
}if(D.extras){C.push(D.extras);
}C.push(a);
return C.join(t);
},__Dy:function(z){var A=this._identifyImage(z);
if(A==null||typeof A==e){A={url:A,tooltip:null};
}if(A.url==null){A.url=this.__Dx;
}return A;
}}});
})();
...
if(j===undefined){this.__Dz=j;
}else{this.__Dz=16;
}
if(k===undefined){this.__DA=k;
}else{this.__DA=16;
}},members:{__Dz:null,__DA:null,_identifyImage:function(d){var h={imageWidth:this.__Dz,imageHeight:this.__DA};
var i;
