...
this.getRoot().add(T,{left:10,top:85});
T.setWidth(300);
var V=new qx.data.controller.List(null,T);
V.setDelegate(this);
V.setLabelPath(g);
if(!qx.core.Variant.isSet(f,c)){V.setIconPath(a);
}var ba=new demobrowser.demo.data.store.Twitter(d);
ba.bind(h,V,h);
var W=new qx.ui.form.Button(n);
this.getRoot().add(W,{left:10,top:55});
var P=new qx.ui.form.TextField(d);
this.getRoot().add(P,{left:60,top:55});
W.addListener(y,function(){var N=F+P.getValue()+b;

if(ba.getUrl()==N){ba.reload();
}else{ba.setUrl(N);
}},this);
P.addListener(A,function(O){if(O.getKeyIdentifier()==E){W.execute();
}},this);
var S=new qx.ui.groupbox.GroupBox(o);
this.getRoot().add(S,{left:320,top:60});
S.setWidth(300);
S.setHeight(220);
S.setLayout(new qx.ui.layout.Grid(0,5));
S.add(new qx.ui.basic.Label(p),{row:0,column:0});
S.add(new qx.ui.basic.Label(r),{row:1,column:0});
S.add(new qx.ui.basic.Label(u),{row:2,column:0});
S.add(new qx.ui.basic.Label(k),{row:3,column:0});
...
X.setRich(true);
X.setWidth(150);
S.add(X,{row:2,column:1});
var bb=new qx.ui.basic.Label();
bb.setRich(true);
S.add(bb,{row:3,column:1});
var Q=new qx.data.controller.Object();
Q.addTarget(name,j,s);
Q.addTarget(location,j,t);
Q.addTarget(X,j,g);
Q.addTarget(bb,j,e);

if(!qx.core.Variant.isSet(f,c)){S.add(new qx.ui.basic.Label(v),{row:4,column:0});
var U=new qx.ui.basic.Image();
S.add(U,{row:4,column:1});
Q.addTarget(U,e,a);
}V.bind(w,Q,h);
...
Y.setWidth(260);
Y.setValue(q+C);
this.getRoot().add(Y,{left:10,top:290});
},configureItem:function(H){H.setRich(true);
H.getChildControl(i).setWidth(50);
H.getChildControl(i).setHeight(50);
H.getChildControl(i).setScale(true);
}}});
qx.Class.define(B,{extend:qx.data.store.Json,statics:{saveResult:function(K){this.__Nc=K;
...
I+=m;
J.load(I,function(G){this.__Nd();
},this);
},__Nd:function(){var bc=demobrowser.demo.data.store.Twitter.__Nc;
