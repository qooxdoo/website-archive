...
var bE=qx.theme.manager.Font.getInstance().resolve(B).getStyles();
this._fontCss=qx.bom.element.Style.compile(bE);
var bN=new qx.ui.layout.Grid(5,0);
bN.setRowFlex(2,1);
...
var bB=[];
var bL=new qx.ui.container.Composite(bN);
this.getRoot().add(bL,{edge:5});
bL.add(new qx.ui.basic.Label(s).set({font:i,decorator:b,padding:3,allowGrowX:true}),{row:1,column:0});
var bD=this.createRowSelectionScroller();
bB.push(bD.getUserData(l));
bL.add(bD,{row:2,column:0});
bL.add(new qx.ui.basic.Label(q).set({font:i,decorator:b,padding:3,allowGrowX:true}),{row:1,column:1});
var bD=this.createColumnSelectionScroller();
bB.push(bD.getUserData(l));
bL.add(bD,{row:2,column:1});
bL.add(new qx.ui.basic.Label(n).set({font:i,decorator:b,padding:3,allowGrowX:true}),{row:1,column:2});
var bD=this.createCellRectangleSelectionScroller();
bB.push(bD.getUserData(l));
bL.add(bD,{row:2,column:2});
bL.add(new qx.ui.basic.Label(z).set({font:i,decorator:b,padding:3,allowGrowX:true}),{row:1,column:3});
var bD=this.createCellLineSelectionScroller();
bB.push(bD.getUserData(l));
bL.add(bD,{row:2,column:3});
var bJ=new qx.ui.layout.Grid(20,4);
var bG=new qx.ui.container.Composite(bJ).set({padding:15,backgroundColor:g});
bL.add(bG,{row:0,column:0,colSpan:4});
bG.add(new qx.ui.basic.Label(p).set({font:i,padding:3}),{row:0,column:0});
var bF=new qx.ui.form.RadioButton(m);
bF.setUserData(h,F);
bG.add(bF,{row:1,column:0});
var bI=new qx.ui.form.RadioButton(w);
bI.setUserData(h,k);
bI.setValue(true);
bG.add(bI,{row:2,column:0});
var bC=new qx.ui.form.RadioButton(A);
bC.setUserData(h,C);
bG.add(bC,{row:3,column:0});
var bA=new qx.ui.form.RadioButton(v);
bA.setUserData(h,G);
bG.add(bA,{row:4,column:0});
var bH=new qx.ui.form.RadioGroup(bF,bI,bC,bA);
bH.addListener(t,function(e){var bg=e.getData()[0].getUserData(h);
bB.forEach(function(a){a.setMode(bg);
});

if(bg==F||bg==G){bM.setEnabled(false);
bK.setEnabled(true);
}else if(bg==k||bg==y){bM.setEnabled(true);
bK.setEnabled(false);
}});
bG.add(new qx.ui.basic.Label(x).set({font:i,padding:3}),{row:0,column:1});
var bM=new qx.ui.form.CheckBox(r);
bG.add(bM,{row:1,column:1});
bM.addListener(E,function(e){if(e.getData()){var U=bB[0].getMode();

if(U==F||U==G){this.debug("Drag selection is only available for the modes multi or additive");
}}bB.forEach(function(N){N.setDrag(e.getData());
});
});
var bK=new qx.ui.form.CheckBox(u).set({enabled:false});
bG.add(bK,{row:2,column:1});
bK.addListener(E,function(e){if(e.getData()){var bf=bB[0].getMode();

if(bf==k||bf==C){this.debug("Quick selection is only available for the modes multi or additive");
}}bB.forEach(function(T){T.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bu=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var bs=new qx.ui.virtual.layer.Row(g,d);
bu.getPane().addLayer(bs);
bu.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var bt=new qx.ui.virtual.cell.Cell();
var br=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(H,I){var J={};

if(bq.isItemSelected(H)){J.selected=true;
}return bt.getCellProperties(H+j+I,J);
}});
bu.getPane().addLayer(br);
var bq=new qx.ui.virtual.selection.Row(bu.getPane(),{styleSelectable:function(bc,bd,be){if(bd!==f){return;
}
if(be){bs.setBackground(bc,f);
}else{bs.setBackground(bc,null);
}br.updateLayerData();
}});
bq.attachMouseEvents(bu.getPane());
bq.attachKeyEvents(bu);
bq.set({mode:k});
bq.addItem(0);
bu.setUserData(l,bq);
return bu;
},createColumnSelectionScroller:function(){var R=new qx.ui.virtual.core.Scroller(1000,100,20,100);
R.getPane().addLayer(new qx.ui.virtual.layer.Row(g,d));
var Q=new qx.ui.virtual.layer.Column();
R.getPane().addLayer(Q);
R.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var S=new qx.ui.virtual.cell.Cell();
var P=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bn,bo){var bp={};

if(O.isItemSelected(bo)){bp.selected=true;
}return S.getCellProperties(bn+j+bo,bp);
}});
R.getPane().addLayer(P);
var O=new qx.ui.virtual.selection.Column(R.getPane(),{styleSelectable:function(V,W,X){if(W!==f){return;
}
if(X){Q.setBackground(V,f);
}else{Q.setBackground(V,null);
}P.updateLayerData();
}});
O.attachMouseEvents(R.getPane());
O.attachKeyEvents(R);
O.set({mode:k});
O.addItem(0);
R.setUserData(l,O);
return R;
},createCellRectangleSelectionScroller:function(){var by=new qx.ui.virtual.core.Scroller(1000,100,20,100);
by.getPane().addLayer(new qx.ui.virtual.layer.Row(g,d));
var bx=new qx.ui.virtual.cell.Cell();
var bz=new qx.ui.virtual.cell.Cell().set({backgroundColor:D});
var bw=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bk,bl){var bm={};

if(bv.isItemSelected({row:bk,column:bl})){bm.selected=true;
}
if(bm.selected){return bz.getCellProperties(bk+j+bl,bm);
}else{bx.resetBackgroundColor();
return bx.getCellProperties(bk+j+bl,bm);
}}});
by.getPane().addLayer(bw);
by.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var bv=new qx.ui.virtual.selection.CellRectangle(by.getPane(),{styleSelectable:function(K,L,M){bw.updateLayerData();
}});
bv.attachMouseEvents(by.getPane());
bv.attachKeyEvents(by);
bv.set({mode:k});
bv.addItem({row:0,column:0});
by.setUserData(l,bv);
return by;
...
if(bO.isItemSelected({row:bh,column:bi})){bj.selected=true;
}
if(bj.selected){return bS.getCellProperties(bh+j+bi,bj);
}else{bQ.resetBackgroundColor();
return bQ.getCellProperties(bh+j+bi,bj);
}}});
bR.getPane().addLayer(bP);
bR.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var bO=new qx.ui.virtual.selection.CellLines(bR.getPane(),{styleSelectable:function(Y,ba,bb){bP.updateLayerData();
}});
...
},_getRelatedSelectable:function(k,l){if(l==a){var n=k-1;
var m=0;
var o=-1;
}else if(l==c){var n=k+1;
