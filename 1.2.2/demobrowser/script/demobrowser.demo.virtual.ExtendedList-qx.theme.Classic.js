...
var M=[q,y,w,k,x,s,v,g];
var G=[B,j,d,f,C,p];
var K=[];

for(var i=0;i<20;i++){var I=new demobrowser.demo.data.model.Person();
I.setName(M[i%M.length]);
I.setEmote(G[i%G.length]);
I.setOnline(i%3==0);
K.push(I);
}var L=new qx.data.Array(K);
var F=new qx.ui.list.List();
F.setWidth(150);
this.getRoot().add(F,{left:10,top:80});
var E={configureItem:function(N){N.setPadding(3);
...
a.bindPropertyReverse(l,m,null,b,c);
}};
F.setDelegate(E);
F.setModel(L);
...
D.addListener(n,function(){for(var i=0;i<L.length;i++){L.getItem(i).toggleOnline();
}},this);
var J=new qx.ui.form.Button(z);
J.setWidth(120);
this.getRoot().add(J,{left:180,top:115});
J.addListener(n,function(){qx.log.appender.Console.show();
this.info(L.toString());
},this);
...
qx.Class.define(c,{extend:qx.ui.virtual.cell.AbstractWidget,events:{"created":a},members:{_createWidget:function(){var h=new qx.ui.form.CheckBox();
this.fireDataEvent(b,h);
return h;
},updateData:function(d,e){var g=e&&e.label?e.label:null;
var f=e&&e.value?e.value:null;
d.setLabel(g);
d.setValue(f);
}}});
})();
