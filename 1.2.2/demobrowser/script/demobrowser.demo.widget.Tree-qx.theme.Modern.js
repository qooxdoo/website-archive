...
(function(){var m="changeValue",l="execute",k="Trash",j="changeSelection",h="mode",g="multi",f="single",d="Windows (C:)",c="Documents (D:)",b="Inbox",P="Desktop",O="root",N="Open mode:",M="Presets",L="Hide Root Node",K="Network",J="Control",I=" items",H="Sent",G="Files",t="Show Open Items",u="",r="Workspace",s="Enable drag selection",p="Selection: ",q="Junk #",n="demobrowser.demo.widget.Tree",o="Data",v="click",w="Show root open button",z="none",y="Enable multi selection",B="double click",A="Selection:",D="Show Selected Items",C="Root node:",x="Toggle Height",F="Edit",E="Show Items";
...
bB.set({spacing:20});
var bA=new qx.ui.container.Composite(bB);
this.getRoot().add(bA);
var by=this.getTree();
bA.add(by);
var bz=this.getCommandFrame(by);
bA.add(bz);
},getTree:function(){var ba=new qx.ui.tree.Tree().set({width:200,height:400});
var bf=new qx.ui.tree.TreeFolder(O);
bf.setOpen(true);
ba.setRoot(bf);
...
var S=new qx.ui.tree.TreeFolder(r);
var T=new qx.ui.tree.TreeFolder(K);
var R=new qx.ui.tree.TreeFolder(k);
bd.add(U,S,T,R);
var bb=new qx.ui.tree.TreeFile(d);
var Q=new qx.ui.tree.TreeFile(c);
S.add(bb,Q);
var be=new qx.ui.tree.TreeFolder(b);
var V=new qx.ui.tree.TreeFolder(M);
var Y=new qx.ui.tree.TreeFolder(H);
var X=new qx.ui.tree.TreeFolder(k);

for(var i=0;i<30;i++){X.add(new qx.ui.tree.TreeFile(q+i));
}var bc=new qx.ui.tree.TreeFolder(o);
var W=new qx.ui.tree.TreeFolder(F);
be.add(V,Y,X,bc,W);
bf.add(be);
return ba;
},getCommandFrame:function(bg){var bq=new qx.ui.groupbox.GroupBox(J);
var bo=4;
bq.setLayout(new qx.ui.layout.Grid(5,3));
var bx=0;
bq.add(new qx.ui.basic.Label(p).set({paddingTop:4}),{row:bx,column:0});
var bu=new qx.ui.form.TextField().set({readOnly:true});
bq.add(bu,{row:bx++,column:1});
bg.addListener(j,function(e){var a=e.getData();

if(a.length>0){if(this.getSelectionMode()===g){bu.setValue(a.length+I);
}else{bu.setValue(a[0].getLabel());
}}else{bu.setValue(u);
}});
bq.add(new qx.ui.core.Spacer(bo,bo),{row:bx++,column:0});
bq.add(new qx.ui.basic.Label(N),{row:bx,column:0});
var bk={"click":v,"dblclick":B,"none":z};
var bi=new qx.ui.form.RadioGroup();

for(var bs in bk){var bj=new qx.ui.form.RadioButton(bk[bs]).set({value:bs==bg.getOpenMode()});
bj.setUserData(h,bs);
bi.add(bj);
bq.add(bj,{row:bx++,column:1});
}bi.addListener(j,function(e){bg.setOpenMode(e.getData()[0].getUserData(h));
});
bq.add(new qx.ui.core.Spacer(bo,bo),{row:bx++,column:0});
bq.add(new qx.ui.basic.Label(A),{row:bx,column:0});
var bm=new qx.ui.form.CheckBox(y);
bq.add(bm,{row:bx++,column:1});
bm.addListener(m,function(e){var bC=e.getData();
bg.setSelectionMode(bC?g:f);
});
var bn=new qx.ui.form.CheckBox(s);
bq.add(bn,{row:bx++,column:1});
bn.addListener(m,function(e){var bD=e.getData();
bg.setDragSelection(bD);

if(!bm.getValue()){bm.setValue(true);
}});
bq.add(new qx.ui.core.Spacer(bo,bo),{row:bx++,column:0});
bq.add(new qx.ui.basic.Label(C),{row:bx,column:0});
var bv=new qx.ui.form.CheckBox(L);
bq.add(bv,{row:bx++,column:1});
bv.addListener(m,function(e){bg.setHideRoot(e.getData());
});
var bl=new qx.ui.form.CheckBox(w);
bq.add(bl,{row:bx++,column:1});
bl.addListener(m,function(e){bg.setRootOpenClose(e.getData());
});
bq.add(new qx.ui.core.Spacer(bo,bo),{row:bx++,column:0});
var bt=new qx.ui.form.Button(E);
bq.add(bt,{row:bx++,column:1});
bt.addListener(l,function(e){alert((""+bg.getItems()).replace(",","\n","g"));
});
var bh=new qx.ui.form.Button(t);
bq.add(bh,{row:bx++,column:1});
bh.addListener(l,function(e){alert((""+bg.getItems(true,false)).replace(",","\n","g"));
});
var br=new qx.ui.form.Button(D);
bq.add(br,{row:bx++,column:1});
br.addListener(l,function(e){if(this.getSelectionMode()===f){alert(this.getSelection());
}else{alert(this.getSelection().toString().replace(",","\n","g"));
}},bg);
var bp=new qx.ui.form.Button(x);
bq.add(bp,{row:bx++,column:1});
var bw=true;
bp.addListener(l,function(e){bg.setHeight(bw?600:400);
bw=!bw;
});
return bq;
}}});
