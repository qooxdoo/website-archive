...
S.addListener(H,function(e){var bc=e.getData();
this.setValue(S.getHierarchy(bc[0].nodeId).join(B));
P.setEnabled(true);
},o);
var P=new qx.ui.form.Button(p);
P.set({enabled:false});
T.add(P,{top:50,left:0});
P.addListener(a,function(e){var ba=S.getSelectedNodes();

for(var i=0;i<ba.length;i++){X.prune(ba[i].nodeId,true);
X.setData();
}});
...
S.addListener(q,function(e){if(this.getValue()){var bb=e.getData();
S.nodeSetHideOpenClose(bb,true);
}},o);
