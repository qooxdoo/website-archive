...
var I=[k,r,u,f,p,B,w,j];

for(var i=0;i<I.length;i++){var J=document.getElementById(v+I[i]);

if(J.checked){qx.bom.Element.addListener(G,I[i],this.logMouseEvent,this);
}qx.bom.Element.addListener(J,l,this.__LP,this);
}var H=document.getElementById(a);
H.checked=false;
qx.bom.Element.addListener(H,l,function(e){var K=e.getTarget().checked;

if(K){qx.bom.Element.capture(G);
}else{qx.bom.Element.releaseCapture(G);
}},this);
qx.bom.Element.addListener(G,o,function(e){H.checked=false;
},this);
},__LP:function(e){var E=e.getTarget().id.split("_")[1];
var C=e.getTarget().checked;
var D=document.getElementById("mouse");

if(C){qx.bom.Element.addListener(D,E,this.logMouseEvent,this);
}else{qx.bom.Element.removeListener(D,E,this.logMouseEvent,this);
}},logMouseEvent:function(F){F.preventDefault();
this._log([F.getTarget().id,F.getType(),F.getButton(),F.getDocumentLeft(),F.getDocumentTop(),F.getScreenLeft(),F.getScreenTop(),F.getViewportLeft(),F.getViewportTop(),F.getRelatedTarget()?F.getRelatedTarget().id:x]);
}}});
})();
