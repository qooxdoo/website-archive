...
br.addListener(d,function(){var T=parseInt(br.getLabel())+1;
br.setLabel(T.toString());
});
...
},createItems:function(be){for(var i=0;i<this.self(arguments).ITEM_SIZE;i++){var bf=new qx.ui.form.ListItem(y+i);
be.add(bf);
}},createMenuForMenuButton:function(){var Y=new qx.ui.menu.Menu;

for(var i=0;i<3;i++){Y.add(new qx.ui.menu.RadioButton(t+i));
}var bc=new qx.ui.form.RadioGroup;
bc.add.apply(bc,Y.getChildren());
var bb=new qx.ui.menu.Menu();

for(var i=0;i<3;i++){var bd=new qx.ui.menu.Button(j+i);
bb.add(bd);
}var ba=new qx.ui.menu.Button(N,q,null,Y);
bb.addSeparator();
bb.add(ba);
return bb;
},createMenuForSplitButton:function(){var U=new qx.ui.menu.Menu;
var X=new qx.ui.menu.Button(G);
var V=new qx.ui.menu.Button(c);
var W=new qx.ui.menu.Button(I);
U.add(X);
U.add(V);
U.add(W);
return U;
