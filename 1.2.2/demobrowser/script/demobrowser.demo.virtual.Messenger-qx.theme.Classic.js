...
qx.Class.define(D,{extend:qx.application.Standalone,members:{__MC:null,messenger:null,__MD:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var bu=this.getRoot();
var bx=new qx.ui.window.Window(G).set({contentPadding:0,showClose:false,showMinimize:false});
...
toolbar.add(bl);
var bj=new qx.ui.toolbar.Button(j,F).set({show:d});
bj.addListener(l,this.showAddContactWindow,this);
bl.add(bj);
var bk=new qx.ui.toolbar.Button(j,q).set({show:d});
this.messenger.bind(U,bk,o,{converter:function(a){return !!a;
}});
bk.addListener(l,function(){this.messenger.getModel().remove(this.messenger.getSelection().getItem(0));
},this);
bl.add(bk);
return bl;
},createDetailsView:function(){var bq=new qx.data.controller.Object();
this.messenger.bind(U,bq,K);
var bn=new qx.ui.groupbox.GroupBox(x);
var br=new qx.ui.layout.Grid(5,5);
br.setColumnAlign(0,V,k);
bn.setLayout(br);
bn.add(new qx.ui.basic.Label(n),{row:0,column:0});
var bt=new qx.ui.form.TextField();
bq.addTarget(bt,g,u,true);
bn.add(bt,{row:0,column:1});
bn.add(new qx.ui.basic.Label(P),{row:1,column:0});
var bo=new qx.ui.form.TextField();
bq.addTarget(bo,g,m,true);
bn.add(bo,{row:1,column:1});
bn.add(new qx.ui.basic.Label(E),{row:2,column:0});
var bs=new qx.ui.form.SelectBox();
bs.add(new qx.ui.form.ListItem(f).set({model:f}));
bs.add(new qx.ui.form.ListItem(i).set({model:i}));
bs.add(new qx.ui.form.ListItem(c).set({model:c}));
bs.add(new qx.ui.form.ListItem(h).set({model:h}));
this.messenger.bind(b,bs,C,{converter:function(bm){return [bm];
}});
bs.bind(I,this.messenger,b,{converter:function(W){return W[0].getModel();
}});
bn.add(bs,{row:2,column:1});
bn.add(new qx.ui.basic.Label(B).set({alignY:A}),{row:3,column:0});
var bp=new qx.ui.basic.Image().set({alignX:r});
bq.addTarget(bp,s,y);
bn.add(bp,{row:3,column:1});
return bn;
},showAddContactWindow:function(){if(this.__MC==null){var Y=new qx.ui.layout.Grid(5,10);
var bb=new qx.ui.window.Window(L,p);
bb.set({width:165,height:100,showMinimize:false,showClose:false,showMaximize:false});
bb.setLayout(Y);
var be=new qx.ui.basic.Label(N);
var bf=new qx.ui.form.TextField();
var bc=new qx.ui.form.ComboBox(w);
bc.add(new qx.ui.form.ListItem(Q));
bc.add(new qx.ui.form.ListItem(O));
var ba=new qx.ui.basic.Label(t);
var bh=new qx.ui.form.Button(H);
var bg=new qx.ui.form.Button(R);
this.__MD=bf;
bh.setAllowGrowX(false);
bg.setAllowGrowX(false);
bb.addListener(J,bf.focus,bf);
bg.addListener(l,bb.close,bb);
var bd=function(){var bi=new demobrowser.demo.virtual.messenger.BuddyModel();
bi.setName(bf.getValue());
bi.setGroup(bc.getValue());
this.messenger.getModel().push(bi);
this.messenger.getSelection().setItem(0,bi);
bb.close();
};
bh.addListener(l,bd,this);
bb.addListener(v,function(e){var X=e.getKeyIdentifier();

if(X==M){bd.call(this);
}else if(X==S){bb.close();
}},this);
bb.add(be,{row:0,column:0});
bb.add(bf,{row:0,column:1,colSpan:2});
bb.add(ba,{row:1,column:0});
bb.add(bc,{row:1,column:1,colSpan:2});
bb.add(bh,{row:2,column:1});
bb.add(bg,{row:2,column:2});
Y.setColumnAlign(0,z,k);
Y.setColumnAlign(1,V,k);
this.getRoot().add(bb,{left:20,top:20});
this.__MC=bb;
}this.__MD.setValue(j);
this.__MC.center();
this.__MC.open();
...
qx.Class.define(g,{extend:qx.ui.virtual.form.ListController,construct:function(D,E){qx.ui.virtual.form.ListController.call(this,D,E);
this.__ME=new demobrowser.demo.virtual.messenger.BuddyCell();
this.__MF=new demobrowser.demo.virtual.messenger.GroupCell();
this.__MF.addListener(j,this._onChangeOpenGroup,this);
this.__MG=[];
this.__MH={};
this.__MI=[];
this.__MJ=new qx.util.DeferredCall(this._updateGrouping,this);
},members:{__MI:null,__MH:null,__MJ:null,__MG:null,__ME:null,__MF:null,_getRowData:function(l){return this.__MI[l];
},_getModelRow:function(C){return this.__MI.indexOf(C);
},_getRowCount:function(){return this.__MI.length;
},_isGroupHeader:function(q){return this.__MI[q] instanceof demobrowser.demo.virtual.messenger.GroupModel;
},_getGroupModel:function(name,m){var n=this.__MH[name];
...
return n;
},_onChangeOpenGroup:function(e){var c=this.__MI[e.getData().getUserData(d)];
c.toggleOpen();
this.__MJ.schedule();
},getCellRenderer:function(B){if(this._isGroupHeader(B)){return this.__MF;
}else{return this.__ME;
}},isRowSelectable:function(A){return !this._isGroupHeader(A);
},_visualizeGrouping:function(H){var I=this.getTarget();

if(I){for(var i=0;i<this.__MG.length;i++){var J=this.__MG[i].getOldRow();
...
},_updateGrouping:function(){var v=this.getModel();
this.__MI=[];
var w=[];
var s=[];

if(v&&v.length>0){w=qx.lang.Array.clone(v.toArray());
w.sort(function(a,b){var F=a.getGroup();
var G=b.getGroup();

if(F==G){return a.getName()<b.getName()?-1:1;
}return F>G?-1:1;
});
var u=w[0];
var r=this._getGroupModel(u.getGroup(),0);
this.__MI.push(r);
s.push(r);
var x=0;
...
if(t.getGroup()!==r.getName()){r.setItemCount(x);
x=0;
var r=this._getGroupModel(t.getGroup(),this.__MI.length);
this.__MI.push(r);
s.push(r);
}
if(r.isOpen()){this.__MI.push(t);
}else{this.getSelection().remove(t);
}x+=1;
}}r.setItemCount(x);
this._visualizeGrouping(s);
this._syncModelSelectionToView();
this._syncRowCount();
...
this.__MJ.schedule();
},_onChangeBubbleModel:function(e){var o=e.getData();

if(o.name){var p=o.name.toString().split(f)[1];

if(p==k||p==h){this.__MJ.schedule();
...
}},properties:{name:{init:c,event:g,check:a},row:{event:d,check:j,apply:b},open:{check:h,init:true,event:i},itemCount:{check:j,init:0,event:e}},members:{__MK:null,_applyRow:function(k,l){this.__MK=l;
},getOldRow:function(){return this.__MK;
}}});
})();
