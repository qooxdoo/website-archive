...
(function(){var R="changeValue",Q="execute",P="icon/16/apps/office-calendar.png",O="right",N="demobrowser.demo.widget.Window",M="Yes",L="Second Window",K="Page 1",J="bottom",I="Use move frame",bH="Show Statusbar",bG="icon/16/actions/dialog-cancel.png",bF="icon/32/status/dialog-error.png",bE="Show Close",bD="icon/32/apps/office-address-book.png",bC="Third Window",bB="icon/22/apps/utilities-calculator.png",bA="Show Maximize",bz="Allow Close",by="Use resize frame",Y="No",ba="resizable",W="Allow Maximize",X="value",U="The second window",V="Page 3",S="move",T="Resizable ",bd="First Window",be="resize",bm="Moveable",bk="Demo loaded",bq="Allow Minimize",bo="Resizable",bu="Open Modal Dialog 2",bs="Open Modal Dialog 1",bg="First Modal Dialog",bx="top",bw="Movable",bv="Show Minimize",bf="Basics",bi="Modal",bj="Do you want to fly to Berlin?",bl="Welcome to your first own Window.<br/>Have fun!",bn="Application is ready",bp="icon/16/apps/internet-feed-reader.png",br="Second Modal Dialog",bt="icon/16/actions/dialog-ok.png",bb="Page 2",bc="left",bh="icon/16/apps/internet-telephony.png";
...
this.createWindow2();
this.createWindow3();
},createWindow1:function(){var c=new qx.ui.window.Window(bd,P);
c.setLayout(new qx.ui.layout.VBox(10));
c.setShowStatusbar(true);
c.setStatus(bk);
c.open();
this.getRoot().add(c,{left:20,top:20});
c.addListener(S,function(e){this.debug("Moved to: "+e.getData().left+"x"+e.getData().top);
},this);
c.addListener(be,function(e){this.debug("Resized to: "+e.getData().width+"x"+e.getData().height);
},this);
var b=new qx.ui.basic.Atom(bl,bD);
b.setRich(true);
c.add(b);
var a=new qx.ui.tabview.TabView;
c.add(a,{flex:1});
var g=new qx.ui.tabview.Page(K);
a.add(g);
var d=new qx.ui.tabview.Page(bb);
a.add(d);
var f=new qx.ui.tabview.Page(V);
a.add(f);
},createWindow2:function(){var r=new qx.ui.window.Window(L,bp);
r.setLayout(new qx.ui.layout.VBox(10));
r.setStatus(bn);
r.open();
this.getRoot().add(r,{left:350,top:120});
var A=new qx.ui.basic.Atom(U,bB);
r.add(A);
var l=new qx.ui.container.Composite;
l.setLayout(new qx.ui.layout.HBox(10));
r.add(l,{flex:1});
var t=new qx.ui.groupbox.GroupBox(bf);
t.setLayout(new qx.ui.layout.VBox(4));
l.add(t,{flex:1});
var o=new qx.ui.form.CheckBox(bE);
o.setValue(true);
o.addListener(R,function(e){r.setShowClose(e.getData());
});
t.add(o);
var z=new qx.ui.form.CheckBox(bA);
z.setValue(true);
z.addListener(R,function(e){r.setShowMaximize(e.getData());
});
t.add(z);
var s=new qx.ui.form.CheckBox(bv);
s.setValue(true);
s.addListener(R,function(e){r.setShowMinimize(e.getData());
});
t.add(s);
var v=new qx.ui.form.CheckBox(bz);
v.setValue(true);
v.addListener(R,function(e){r.setAllowClose(e.getData());
});
t.add(v);
var B=new qx.ui.form.CheckBox(W);
B.setValue(true);
B.addListener(R,function(e){r.setAllowMaximize(e.getData());
});
t.add(B);
var q=new qx.ui.form.CheckBox(bq);
q.setValue(true);
q.addListener(R,function(e){r.setAllowMinimize(e.getData());
});
t.add(q);
var m=new qx.ui.form.CheckBox(bH);
m.setValue(false);
m.addListener(R,function(e){r.setShowStatusbar(e.getData());
});
t.add(m);
var D=new qx.ui.groupbox.GroupBox(bo);
D.setLayout(new qx.ui.layout.VBox(4));
l.add(D,{flex:1});
var x=new qx.ui.form.CheckBox(by);
x.setValue(true);
x.addListener(R,function(e){r.setUseResizeFrame(e.getData());
});
D.add(x);
var y=[bx,O,J,bc];

for(var i=0;i<y.length;i++){var w=y[i];
var p=new qx.ui.form.CheckBox(T+w).set({value:true});
p.bind(X,r,ba+qx.lang.String.firstUp(w));
D.add(p);
}var n=new qx.ui.groupbox.GroupBox(bm);
n.setLayout(new qx.ui.layout.VBox(4));
l.add(n,{flex:1});
var u=new qx.ui.form.CheckBox(bw);
u.setValue(true);
u.addListener(R,function(e){r.setMovable(e.getData());
});
n.add(u);
var C=new qx.ui.form.CheckBox(I);
C.addListener(R,function(e){r.setUseMoveFrame(e.getData());
});
n.add(C);
},createWindow3:function(){var h=new qx.ui.window.Window(bC,bh);
h.setLayout(new qx.ui.layout.VBox);
h.setMaxWidth(450);
h.setMaxHeight(400);
h.setAllowMaximize(false);
h.open();
this.getRoot().add(h,{left:100,top:250});
var j=this.getModalWindow1();
var k=new qx.ui.form.Button(bs,P);
k.addListener(Q,j.open,j);
h.add(k);
},getModalWindow1:function(){var F=new qx.ui.window.Window(bg);
F.setLayout(new qx.ui.layout.VBox(10));
F.setModal(true);
F.moveTo(150,150);
this.getRoot().add(F);
var E=this.getModalWindow2();
var H=new qx.ui.form.Button(bu,P);
H.addListener(Q,E.open,E);
F.add(H);
var G=new qx.ui.form.CheckBox(bi);
G.setValue(true);
F.add(G);
G.addListener(R,function(e){F.setModal(e.getData());
});
return F;
},getModalWindow2:function(){var bJ=new qx.ui.window.Window(br);
bJ.setLayout(new qx.ui.layout.VBox(10));
bJ.setModal(true);
bJ.setShowClose(false);
bJ.moveTo(300,300);
this.getRoot().add(bJ);
var bI=new qx.ui.basic.Atom(bj,bF);
bJ.add(bI);
var bL=new qx.ui.container.Composite;
bL.setLayout(new qx.ui.layout.HBox(10,O));
bJ.add(bL);
var bM=new qx.ui.form.Button(M,bt);
bM.addListener(Q,function(e){bJ.close();
});
bL.add(bM);
var bK=new qx.ui.form.Button(Y,bG);
bK.addListener(Q,function(e){alert("Sorry, please click 'Yes'!");
});
bL.add(bK);
return bJ;
}}});
