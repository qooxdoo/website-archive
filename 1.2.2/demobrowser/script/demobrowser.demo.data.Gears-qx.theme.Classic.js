...
var R=new qx.ui.groupbox.GroupBox(s);
this.getRoot().add(R,{left:10,top:50});
R.setLayout(new qx.ui.layout.Grid(0,3));
R.add(new qx.ui.basic.Label(A),{row:0,column:0});
R.add(new qx.ui.basic.Label(v),{row:1,column:0});
R.add(new qx.ui.basic.Label(b),{row:2,column:0});
var L=new qx.ui.form.TextField();
R.add(L,{row:0,column:1});
var N=new qx.ui.form.TextField();
R.add(N,{row:1,column:1});
var O=new qx.ui.form.Spinner(0,0,1000);
R.add(O,{row:2,column:1});
var P=new qx.data.controller.Object();
P.addTarget(L,k,D,true);
P.addTarget(N,k,E,true);
P.addTarget(O,k,C,true);
var Q=new demobrowser.demo.data.store.Gears();
Q.bind(h,P,h);
...
}this.__MY=this.initDB();
this.__Na();
},members:{__Na:function(J){var K=this.getPerson();
this._marshaler.toClass(K,true);
this.setModel(this._marshaler.toModel(K));
this.getModel().addListener(t,this.__Nb,this);
this.fireDataEvent(c,this.getModel());
},__Nb:function(T){var U=this.getModel();
this.update(U.getFirstname(),U.getLastname(),U.getAge());
},update:function(G,H,I){if(!google.gears.factory||!this.__MY){return;
}this.__MY.execute(w);
this.__MY.execute(l,[G,H,I]);
},getPerson:function(){var V={};
V.firstname=i;
V.lastname=i;
V.age=0;

if(!google.gears.factory||!this.__MY){return V;
}var W=this.__MY.execute(x);

if(W.isValidRow()){V.firstname=W.field(0);
V.lastname=W.field(1);
V.age=parseInt(W.field(2));
}W.close();
return V;
},initDB:function(){if(window.google&&google.gears){try{var F=google.gears.factory.create(m);

if(F){F.open(z);
F.execute(n+r);
}}catch(S){this.error('Could not create database: '+S.message);
}}return F;
},initGears:function(){if(window.google&&google.gears){return;
}var X=null;
if(typeof GearsFactory!=j){X=new GearsFactory();
}else{try{X=new ActiveXObject(y);
if(X.getBuildInfo().indexOf(p)!=-1){X.privateSetGlobalObject(this);
}}catch(e){if((typeof navigator.mimeTypes!=j)&&navigator.mimeTypes[g]){X=document.createElement(u);
X.style.display=q;
X.width=0;
X.height=0;
X.type=g;
...
if(!google.gears){google.gears={factory:X};
}}}});
})();
