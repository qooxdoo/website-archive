...
},createTable:function(){var K=this._tableModel=new qx.ui.table.model.Filtered();
K.setColumns([o,h,p,j,m]);
var L=[];
var O=new Date().getTime();
var I=400*24*60*60*1000;

for(var P=0;P<20;P++){var M=new Date(O+Math.random()*I-I/2);
var J=g+P+n;
L.push([P,Math.random()*10000,M,(Math.random()>0.5),J]);
}K.setData(L);
K.setColumnEditable(1,true);
K.setColumnEditable(2,true);
var N=this._table=new qx.ui.table.Table(K);
N.setMetaColumnCounts([1,-1]);
N.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
N.getTableColumnModel().setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
N.getTableColumnModel().setDataCellRenderer(4,new qx.ui.table.cellrenderer.Html());
N.setAdditionalStatusBarText(l);
return N;
},createControls:function(){var A=new qx.ui.toolbar.ToolBar();
var F;
F=new qx.ui.toolbar.Part();
A.add(F);
var H=this._table;
var B=this._tableModel;
var E=new qx.ui.toolbar.Button(w,i);
F.add(E);
E.addListener(k,function(e){B.addNumericFilter(r,1,j);
B.applyFilters();
H.setAdditionalStatusBarText(t);
});
var D=new qx.ui.toolbar.Button(q,i);
F.add(D);
D.addListener(k,function(e){B.addNumericFilter(s,1,j);
B.applyFilters();
H.setAdditionalStatusBarText(x);
});
var C=new qx.ui.toolbar.Button(b,i);
F.add(C);
C.addListener(k,function(e){B.addBetweenFilter(d,1000,5000,h);
B.applyFilters();
H.setAdditionalStatusBarText(z);
});
var G=new qx.ui.toolbar.Button(y,f);
F.add(G);
G.addListener(k,function(e){B.resetHiddenRows();
H.setAdditionalStatusBarText(c);
});
return A;
