...
document.getElementById(c).onclick=function(){var q=qx.dev.Tokenizer.tokenizeJavaScript(m.value);
p.innerHTML=qx.bom.String.escape(qx.util.Json.stringify(q,true));
};
var o=function(){var k=qx.dev.Tokenizer.tokenizeJavaScript(m.value);
var l=[];

for(var i=0;i<k.length;i++){l[i]=k[i].value;
}p.innerHTML=h+qx.bom.String.escape(l.join(f))+j;
};
document.getElementById(g).onclick=o;
var n=function(){p.innerHTML=h+qx.dev.Tokenizer.javaScriptToHtml(m.value)+j;
};
