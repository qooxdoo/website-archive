...
var bQ=new qx.ui.basic.Label(u);
bQ.setFont(h);
this.getRoot().add(bQ,{left:20,top:20});
var bR=new demobrowser.demo.virtual.list.List();
bR.setScrollbarX(bl);
bR.set({height:280,width:150,selectionMode:m});
var bN;

for(var i=1;i<=2500;i++){bN=new demobrowser.demo.virtual.list.ListItem(g+i,K+((i%4)?bo:O)+bu);

if(i%4==0){bN.setHeight(56);
}bR.add(bN);
if(i==20){bR.setSelected(bN);
}}bR.addListener(l,function(e){this.debug("Value: "+e.getData());
});
this.getRoot().add(bR,{left:20,top:40});
var bJ=new qx.ui.form.RadioButton(L);
var bG=new qx.ui.form.RadioButton(bh);
var bH=new qx.ui.form.RadioButton(bv);
var bF=new qx.ui.form.RadioButton(H);
bJ.setUserData(o,k);
bG.setUserData(o,m);
bH.setUserData(o,j);
bF.setUserData(o,n);
...
var bM=new qx.ui.form.CheckBox(bq);
var bP=new qx.ui.form.CheckBox(bj).set({enabled:false});
this.getRoot().add(bM,{left:180,top:220});
this.getRoot().add(bP,{left:180,top:240});
bM.addListener(l,function(e){if(e.getData()){var bE=bR.getSelectionMode();

if(bE==k||bE==n){this.debug("Drag selection is only available for the modes multi or additive");
}}bR.setDragSelection(e.getData());
});
bP.addListener(l,function(e){if(e.getData()){var bD=bR.getSelectionMode();

if(bD==m||bD==j){this.debug("Quick selection is only available for the modes multi or additive");
}}bR.setQuickSelection(e.getData());
});
bI.addListener(T,function(e){var bC=e.getData()[0].getUserData(o);
bR.setSelectionMode(bC);

if(bC==k||bC==n){bM.setEnabled(false);
bP.setEnabled(true);
}else if(bC==m||bC==q){bM.setEnabled(true);
bP.setEnabled(false);
}});
var bT=new qx.ui.basic.Label(N);
bT.setFont(h);
this.getRoot().add(bT,{left:330,top:20});
var bO=new demobrowser.demo.virtual.list.List();
bO.set({height:280,width:150,selectionMode:n});
var bN;

for(var i=1;i<=25;i++){bN=new demobrowser.demo.virtual.list.ListItem(g+i,s);
bO.add(bN);
if(i==16){bO.setSelected(bN);
}}this.getRoot().add(bO,{left:330,top:40});
var bQ=new qx.ui.basic.Label(B);
bQ.setFont(h);
this.getRoot().add(bQ,{left:520,top:20});
var bS=new demobrowser.demo.virtual.list.List;
var bK;
bS.set({width:150,selectionMode:j});
var bL=[v,U,X,S,x,by,bi,ba,M,f,c,A,b,V,bm,bn,w,Q,a,G,d,bg,I,bt,bb,bk,bw,y,bf,br,W,bs,t,J,P,z,E,F,bB,bc,bd,R,r,Y,C,be,D,bp,bA,bx,bz];
...
this.__Lw=new qx.ui.virtual.layer.WidgetCell(this);
this.getPane().addLayer(this.__Lw);
this.getPane().addListener(D,this._onResize,this);
this.__Lx=[];
this.__Ly=[];
this.__Lz=new qx.ui.virtual.selection.Row(this.getPane());
this.__Lz.addListener(o,this._onChangeSelection,this);
this.__Lz.attachMouseEvents();
this.__Lz.attachKeyEvents(this);
this.__Lz.attachListEvents(this);
...
this.fireDataEvent(o,this.__LD(e.getData()));
},getCellWidget:function(ba,bb){var bd=this.__Lx[ba];

if(!bd){return null;
}var bc=this.__Ly.pop()||new qx.ui.form.ListItem();
bc.set({label:bd.getLabel(),icon:bd.getIcon()});

if(this.__Lz.isItemSelected(ba)){bc.addState(r);
}else{bc.removeState(r);
}bc.setUserData(n,ba);
return bc;
},poolCellWidget:function(bn){this.__Ly.push(bn);
},getValue:function(){var bf=this.getSelection();
var bg=[];
var bh;

for(var i=0,l=bf.length;i<l;i++){bh=bf[i].getValue();
if(bh==null){bh=bf[i].getLabel();
}bg.push(bh);
}return bg.join(h);
},setValue:function(S){var V=S.split(h);
var U=[];
var T;

for(var i=0,l=V.length;i<l;i++){T=this.findItem(V[i]);

if(T){U.push(T);
}}this.setSelection(U);
},getChildren:function(){return this.__Lx;
},hasChildren:function(){return this.__Lx.length>0;
},_addHelper:function(bi){this.fireDataEvent(u,bi);
bi.addListener(k,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},_removeHelper:function(bu){this.fireDataEvent(s,bu);
bu.removeListener(k,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},add:function(bz){this.__Lx.push(bz);
this._addHelper(bz);
return this;
},remove:function(be){qx.lang.Array.remove(this.__Lx,be);
this._removeHelper(be);
return this;
},removeAll:function(){for(var i=0,j=this.__Lx.length;i<j;i++){this._removeHelper(this.__Lx[i]);
}this.__Lx=[];
},indexOf:function(b){return this.__Lx.indexOf(b);
},addAt:function(c,d){qx.lang.Array.insertAt(this.__Lx,c,d);
this._addHelper(c);
},addBefore:function(bo,bp){qx.lang.Array.insertBefore(this.__Lx,bo,bp);
this._addHelper(bo);
},addAfter:function(O,P){qx.lang.Array.insertAfter(this.__Lx,O,P);
this._addHelper(O);
},removeAt:function(J){this.__Lx.splice(J,1);
this._removeHelper(this.__Lx[J]);
},__LA:function(bv){var bw=this.__Lx.indexOf(bv);

if(bw<0){return null;
}else{return bw;
}},__LB:function(bq){var br=this.__Lx[bq];
return br;
},__LC:function(H){var I=[];

for(var i=0;i<H.length;i++){I.push(this.__LA(H[i]));
}return I;
},__LD:function(bs){var bt=[];

for(var i=0;i<bs.length;i++){bt.push(this.__Lx[bs[i]]);
}return bt;
},selectAll:function(){this.__Lz.selectAll();
},setSelected:function(a){this.__Lz.selectItem(this.__LA(a));
},isSelected:function(bj){return this.__Lz.isItemSelected(this.__LA(bj));
},addToSelection:function(M){this.__Lz.addItem(this.__LA(M));
},removeFromSelection:function(N){this.__Lz.removeItem(this.__LA(N));
},selectRange:function(K,L){this.__Lz.selectItemRange(this.__LA(K),this.__LA(L));
},resetSelection:function(){this.__Lz.clearSelection();
},setSelection:function(Y){this.__Lz.replaceSelection(this.__LC(Y));
},getSelectedItem:function(){return this.__Lx[this.getSelected()];
},getSelected:function(){return this.__Lx[this.__Lz.getSelectedItem()];
},getSelection:function(){return this.__LD(this.__Lz.getSelection());
},getSortedSelection:function(){return this.__LD(this.__Lz.getSortedSelection());
},isSelectionEmpty:function(){return this.__Lz.isSelectionEmpty();
},getSelectionContext:function(){return this.__Lz.getSelectionContext();
},getSelectables:function(){return this.__LD(this.__Lz.getSelectables());
},_applySelectionMode:function(bx,by){this.__Lz.setMode(bx);
},_applyDragSelection:function(bA,bB){this.__Lz.setDrag(bA);
},_applyQuickSelection:function(W,X){this.__Lz.setQuick(W);
}}});
})();
...
qx.Class.define(a,{extend:qx.core.Object,construct:function(l,m,n){qx.core.Object.call(this);
