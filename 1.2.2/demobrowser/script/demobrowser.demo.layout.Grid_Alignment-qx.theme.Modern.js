...
},getNewWidget:function(){var q=new qx.ui.core.Widget().set({decorator:m,backgroundColor:a,allowGrowX:false,allowGrowY:false,height:Math.round((Math.random()*30)+30),width:Math.round((Math.random()*30)+30)});
return q;
},getColumnAlignGrid:function(){var z=new qx.ui.container.Composite().set({decorator:m,backgroundColor:l,allowGrowX:false,allowGrowY:false});
var A=new qx.ui.layout.Grid();
A.setSpacing(3);
A.setColumnAlign(0,h,j);
A.setColumnAlign(1,f,e);
A.setColumnAlign(2,g,k);
z.setLayout(A);

for(var x=0;x<3;x++){for(var y=0;y<6;y++){z.add(this.getNewWidget(),{row:y,column:x});
}}return z;
},getRowAlignGrid:function(){var v=new qx.ui.container.Composite().set({decorator:m,backgroundColor:l,allowGrowX:false,allowGrowY:false});
...
for(var x=0;x<6;x++){for(var y=0;y<3;y++){v.add(this.getNewWidget(),{row:y,column:x});
}}return v;
},getCellAlignGrid:function(){var s=new qx.ui.container.Composite().set({decorator:m,backgroundColor:l,allowGrowX:false,allowGrowY:false});
var u=new qx.ui.layout.Grid();
u.setSpacing(3);
s.setLayout(u);

for(var x=0;x<5;x++){u.setColumnAlign(x,f,e);

for(var y=0;y<5;y++){s.add(this.getNewWidget(),{row:y,column:x});
}}var t=u.getCellWidget(0,0);
t.set({alignX:h,alignY:j});
t.set({backgroundColor:d,width:20,height:20});
var t=u.getCellWidget(4,4);
t.set({alignX:g,alignY:k});
t.set({backgroundColor:d,width:20,height:20});
return s;
},getRowColumnAlignGrid:function(){var n=new qx.ui.container.Composite().set({decorator:m,backgroundColor:l,allowGrowX:false,allowGrowY:false});
var p=new qx.ui.layout.Grid();
p.setSpacing(3);
n.setLayout(p);

for(var x=0;x<5;x++){for(var y=0;y<5;y++){n.add(this.getNewWidget(),{row:y,column:x});
}}p.setColumnAlign(2,h,j);
p.setRowAlign(2,g,k);

for(var i=0;i<5;i++){p.getCellWidget(2,i).setBackgroundColor(c);
p.getCellWidget(i,2).setBackgroundColor(c);
}var o=p.getCellWidget(2,2);
o.set({backgroundColor:d,width:20,height:20});
return n;
}}});
})();
