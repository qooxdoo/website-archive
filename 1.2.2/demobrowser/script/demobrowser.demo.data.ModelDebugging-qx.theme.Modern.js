...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.data.ModelDebugging","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.data.ModelDebugging-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"f9a093f4b2c3"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var n="auto",m="debugModel function in qx.dev.Debug.",l="?callback=demobrowser.demo.data.store.Twitter.saveResult",k="objects with properties. To see the properties, you can use the ",j="demobrowser.demo.data.ModelDebugging",i="wittemann",h="loaded",g="Every model created by the data stores consists only of qooxdoo ",f="demobrowser.demo.data.store.Twitter",e="<b>Debugging models</b><br/>",b="white",d="main",c="http://twitter.com/statuses/user_timeline/",a=".json";
...
var t=new demobrowser.demo.data.store.Twitter(i);
var r=new qx.ui.embed.Html();
r.setBackgroundColor(b);
r.setDecorator(d);
r.setWidth(500);
r.setHeight(200);
r.setOverflow(n,n);
this.getRoot().add(r,{left:10,top:80});
t.addListener(h,function(){var q=t.getModel();
this.debug(qx.dev.Debug.debugProperties(q));
r.setHtml(qx.dev.Debug.debugProperties(q,10,true));
},this);
...
qx.Class.define(f,{extend:qx.data.store.Json,statics:{saveResult:function(w){this.__MT=w;
}},construct:function(u){var v=c+u+a;
qx.data.store.Json.call(this,v);
},members:{_createRequest:function(o){var p=new qx.io.ScriptLoader();
o+=l;
p.load(o,function(x){this.__MU();
},this);
},__MU:function(){var y=demobrowser.demo.data.store.Twitter.__MT;

if(y==undefined){this.setState("failed");
return;
}this._marshaler.toClass(y);
...
qx.Class.define(t,{statics:{debugObject:function(bc,bd,be){qx.log.Logger.debug(this,qx.dev.Debug.debugObjectToString(bc,bd,be,false));
},debugObjectToString:function(bf,bg,bh,bi){if(!bh){bh=10;
}var bo=(bi?I:r);
var bl=function(V){var W;
...
var bn=l;
var bk=function(R,S,T){if(S>T){bn+=(bl(S)+s+bo);
return;
}if(typeof (R)!=h){bn+=bl(S)+R+bo;
return;
}for(var U in R){if(typeof (R[U])==h){try{if(R[U] instanceof Array){bn+=bl(S)+U+m+C+bo;
}else if(R[U]===null){bn+=bl(S)+U+m+D+bo;
continue;
}else if(R[U]===undefined){bn+=bl(S)+U+m+y+bo;
continue;
}else{bn+=bl(S)+U+m+F+bo;
}bk(R[U],S+1,T);
}catch(e){bn+=bl(S)+U+u+bo;
}}else{bn+=bl(S)+U+m+R[U]+bo;
}}};
if(bg){bn+=bl(0)+bg+bo;
}
if(bf instanceof Array){bn+=bl(0)+w+bf.length+f+bo;
}else if(typeof (bf)==h){var bj=0;
...
try{bk(bf,0,bh);
}catch(bb){bn+=bl(0)+H+bb+d+bo;
}bn+=bl(0)+G+bo;
return bn;
},getFunctionName:function(X,Y){var ba=X.self;

if(!ba){return null;
}while(X.wrapper){X=X.wrapper;
}
switch(Y){case k:return X==ba?k:null;
case B:return qx.lang.Object.getKeyFromValue(ba,X);
case A:return qx.lang.Object.getKeyFromValue(ba.prototype,X);
default:if(X==ba){return k;
}return (qx.lang.Object.getKeyFromValue(ba.prototype,X)||qx.lang.Object.getKeyFromValue(ba,X)||null);
}},debugProperties:function(J,K,L,M){if(K==null){K=10;
}if(M==null){M=1;
}var N=l;
L?N=E:N=c;
var P=l;

if(qx.lang.Type.isNumber(J)||qx.lang.Type.isString(J)||qx.lang.Type.isBoolean(J)||J==null||K<=0){return J;
}else if(qx.Class.hasInterface(J.constructor,qx.data.IListData)){for(var i=0;i<J.length;i++){for(var j=0;j<M;j++){P+=g;
}P+=x+i+a+this.debugProperties(J.getItem(i),K-1,L,M+1)+N;
}return P+N;
}else if(J.constructor!=null){var Q=J.constructor.$$properties;

for(var O in Q){P+=N;
for(var j=0;j<M;j++){P+=g;
}P+=o+O+m+this.debugProperties(J[z+qx.lang.String.firstUp(O)](),K-1,L,M+1);
}return P;
}return l;
}}});
})();
