...
(function(){var o="execute",n="changeValue",m="A date",l="Boolean",k="icon/22/actions/edit-undo.png",j="Change ID sort method",h="icon/22/status/dialog-information.png",g="Change row with ID 10",f="A number",e="icon/16/apps/office-calendar.png",E="Remove 5 rows",D="Add 10 rows",C="",B="demobrowser.demo.table.Table",A="_tableModel",z="Table",y="icon/22/actions/list-remove.png",x="Whether the the first visible row should ",w=", ",v="icon/22/actions/list-add.png",t="Selected rows:<br>",u="Keep first row",r="be rendered completely when scrolling.",s="Show selection",p="Demonstrate use of alternate sorting algorithm.",q="ID";
qx.Class.define(B,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return z;
},createTable:function(){var c=this.createRandomRows(50);
var b=this._tableModel=new qx.ui.table.model.Simple();
b.setColumns([q,f,m,l]);
b.setData(c);
b.setColumnEditable(1,true);
b.setColumnEditable(2,true);
b.setColumnSortable(3,false);
var d=new qx.ui.table.Table(b);
d.set({width:600,height:400,decorator:null});
d.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
var a=d.getTableColumnModel();
a.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
a.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(e,m));
return d;
},createControls:function(){var T=new qx.ui.toolbar.ToolBar();
var R,S,U;
S=new qx.ui.toolbar.Part();
T.add(S);
R=new qx.ui.toolbar.Button(g,k);
R.addListener(o,function(be){var bf=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,bf[0][i]);
}this.info("Row 10 changed");
},this);
S.add(R);
R=new qx.ui.toolbar.Button(D,v);
R.addListener(o,function(F){var G=this.createRandomRows(10);
this._tableModel.addRows(G);
this.info("10 rows added");
},this);
S.add(R);
R=new qx.ui.toolbar.Button(E,y);
R.addListener(o,function(ba){var bb=this._tableModel.getRowCount();
this._tableModel.removeRows(bb-5,5);
this.info("5 rows removed");
},this);
S.add(R);
R=new qx.ui.toolbar.Button(s,h);
R.addListener(o,function(bc){var bd=[];
Q.getSelectionModel().iterateSelection(function(H){bd.push(H+C);
});
this.showDialog(t+bd.join(w));
},this);
S.add(R);
S=new qx.ui.toolbar.Part();
T.add(S);
var Q=this._table;
U=new qx.ui.toolbar.CheckBox(u);
U.set({value:Q.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(x+r)});
U.addListener(n,function(V){Q.setKeepFirstVisibleRowComplete(this.getValue());
},U);
S.add(U);
U=new qx.ui.toolbar.CheckBox(j);
U.set({value:Q.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(p)});
U.addListener(n,function(W){if(W.getData()){var Y=function(M,N){var O=M[arguments.callee.columnIndex];
...
var X=function(I,J){var K=I[arguments.callee.columnIndex];
var L=J[arguments.callee.columnIndex];

if(K%2==1&&L%2==0){return -1;
}
if(L%2==1&&K%2==0){return 1;
}return (K<L)?1:((K==L)?0:-1);
};
Q.getTableModel().setSortMethods(0,{ascending:Y,descending:X});
}else{Q.getTableModel().setSortMethods(0,null);
}},U);
S.add(U);
return T;
