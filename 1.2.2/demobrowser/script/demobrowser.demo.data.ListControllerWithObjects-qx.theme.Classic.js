...
var Y={converter:function(L){return m+L+g;
}};
var U=new qx.data.controller.List(X,P,z);
U.setIconOptions(Y);
U.setIconPath(d);
var S={converter:function(bb){if(bb){return c;
}else{return v;
}}};
var N={bindItem:function(I,J,K){I.bindDefaultProperties(J,K);
I.bindProperty(B,r,S,J,K);
}};
U.setDelegate(N);
var O=new qx.ui.form.Button(G);
O.setWidth(120);
this.getRoot().add(O,{left:180,top:80});
O.addListener(k,function(){for(var i=0;i<X.length;i++){var H=X.getItem(i);
H.setEmote(j);
}},this);
var M=new qx.ui.form.Button(l);
M.setWidth(120);
this.getRoot().add(M,{left:180,top:115});
M.addListener(k,function(){for(var i=0;i<X.length;i++){X.getItem(i).toggleOnline();
}},this);
var V=new qx.ui.form.Button(y);
V.setWidth(120);
this.getRoot().add(V,{left:180,top:150});
V.addListener(k,function(){qx.log.appender.Console.show();
