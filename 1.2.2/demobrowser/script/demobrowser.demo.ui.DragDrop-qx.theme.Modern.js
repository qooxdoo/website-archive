...
(function(){var m="drop",k="value",j="items",h="move",g="multi",f="dragover",d="icon/16/places/folder.png",c="dragstart",b="Item ",a="copy",z="drag",y="TextArea Target",x="solid",w="Reorderable",v="droprequest",u="#33508D",t="Source",s="demobrowser.demo.ui.DragDrop",r="Simple Target",q="Enable drag",o="__NK",p="\n",n="dragend";
qx.Class.define(s,{extend:qx.application.Standalone,members:{__NK:null,__NL:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var S=this.getRoot();
...
var G=e.getCurrentType();
var D;

switch(G){case j:D=this.getSelection();

if(E==a){var F=[];

for(var i=0,l=D.length;i<l;i++){F[i]=D[i].clone();
}D=F;
}break;
case k:D=this.getSelection()[0].getLabel();
break;
}if(E==h){var C=this.getSelection();

for(var i=0,l=C.length;i<l;i++){this.remove(C[i]);
}}e.addData(G,D);
});
...
var I=this.__NK=new qx.ui.form.List;
I.setDraggable(true);
I.setDroppable(true);
...
I.addListener(z,function(e){var B=e.getOriginalTarget();
if(B instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__NL=B;
}
if(!qx.ui.core.Widget.contains(this,B)&&B!=J){return;
}var A=B.getContainerLocation();
J.setWidth(B.getBounds().width);
J.setDomPosition(A.left,A.top);
});
I.addListener(f,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
I.addListener(m,function(e){this.__NM(e.getOriginalTarget());
},this);
J.addListener(m,function(e){this.__NM(this.__NL);
},this);
},__NM:function(U){if(U.classname!="qx.ui.form.ListItem"){return;
}var V=this.__NK.getSortedSelection();

for(var i=0,l=V.length;i<l;i++){this.__NK.addBefore(V[i],U);
this.__NK.addToSelection(V[i]);
