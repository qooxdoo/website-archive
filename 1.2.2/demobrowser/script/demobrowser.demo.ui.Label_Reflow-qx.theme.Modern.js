...
o.setRowFlex(0,1);
o.setSpacing(10);
var p=new qx.ui.container.Composite(o).set({backgroundColor:b,decorator:m,padding:10,allowGrowX:false,allowGrowY:false,allowStretchX:false,allowStretchY:false});
q.add(p);
this.getRoot().add(q,{edge:0});
var u=this.getPane();
p.add(u,{row:0,column:0,colSpan:3});
var s=new qx.ui.basic.Label(c).set({backgroundColor:j,decorator:m,padding:[4,8]});
p.add(s,{row:1,column:1});
var r=false;
s.addListener(k,function(){var n=u.getChildren()[0];
n.setValue(r?l:i);
r=!r;
});
var t=new qx.ui.basic.Label(a).set({backgroundColor:j,decorator:m,padding:[4,8]});
p.add(t,{row:1,column:2});
var v=false;
t.addListener(k,function(e){if(v){u.setWidth(600);
}else{u.setWidth(300);
}v=!v;
});
