...
qx.Class.define(l,{extend:qx.ui.form.renderer.Single,members:{__NI:0,__NJ:0,addItems:function(w,x,y){var B=0;
if(y!=null){this._add(this._createHeader(y),{row:B,column:this.__NI,colSpan:2});
B++;
}for(var i=0;i<w.length;i++){var A=this._createLabel(x[i],w[i]);
this._add(A,{row:B,column:this.__NI});
var z=w[i];
A.setBuddy(z);

if(z instanceof qx.ui.form.RadioGroup){z=this._createWidgetForRadioGroup(z);
}this._add(z,{row:B,column:this.__NI+1});
B++;
}this.__NI+=2;
this.__NJ<B?this.__NJ=B:null;
...
this._add(this._buttonRow,{row:this.__NJ,column:0,colSpan:this.__NI});
