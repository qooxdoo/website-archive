...
(function(){var q="password",p="Send",o="All fields are required. Some by a custom validator, some by a ",n="complete",m="Please enter a password at with least 3 characters.",l="username",k="Server said no!",j="Passwords must be equal.",i="<b>Client side form validation</b><br/>",h="Validating...",e="email address",g="demobrowser.demo.ui.FormValidator",f="Accept",d="execute",c="predefined validator and some by the required flag.";
...
u.setWidth(150);
u.setRequired(true);
this.getRoot().add(u,{left:20,top:70});
...
v.setPlaceholder(q);
v.setWidth(150);
this.getRoot().add(v,{left:20,top:130});
var w=new qx.ui.form.TextField();
w.setPlaceholder(q);
w.setWidth(150);
this.getRoot().add(w,{left:20,top:160});
var C=new qx.ui.form.CheckBox(f);
C.setRequired(true);
this.getRoot().add(C,{left:20,top:190});
var z=new qx.ui.form.validation.Manager();
var x=function(E,F){var G=E!=null&&E.length>2;

if(!G){F.setInvalidMessage(m);
}return G;
};
var A=new qx.ui.form.validation.AsyncValidator(function(a,b){window.setTimeout(function(){if(b==null||b.length==0){a.setValid(false,k);
}else{a.setValid(true);
}},1000);
});
z.add(u,A);
z.add(D,qx.util.Validate.email());
z.add(v,x);
z.add(w,x);
z.add(C);
z.setValidator(function(r){var t=v.getValue()==w.getValue();

if(!t){var s=j;
v.setInvalidMessage(s);
w.setInvalidMessage(s);
v.setValid(false);
w.setValid(false);
}return t;
});
var y=new qx.ui.form.Button(p);
this.getRoot().add(y,{left:20,top:215});
y.addListener(d,function(){y.setEnabled(false);
y.setLabel(h);
z.validate();
},this);
z.addListener(n,function(){y.setEnabled(true);
y.setLabel(p);
if(z.getValid()){alert("You can send...");
}else{alert(z.getInvalidMessages().join("\n"));
}},this);
