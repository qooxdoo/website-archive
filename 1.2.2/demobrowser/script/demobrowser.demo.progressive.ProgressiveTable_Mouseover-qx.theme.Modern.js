...
var J=0;
var F=function(s){var v=[];
var u=new Date().getTime();
var w=400*24*60*60*1000;

for(var x=0;x<s;x++){var t=new Date(u+Math.random()*w-w/2);
v.push({renderer:r,location:h,data:[J++,Math.random()*10000,t]});
}return v;
};
var I=new qx.ui.progressive.renderer.table.Widths(3);
I.setWidth(0,100);
I.setWidth(1,l);
I.setWidth(2,300);
var C=[d,n,f];
var E=new qx.ui.progressive.headfoot.TableHeading(I,C);
var B=new qx.ui.progressive.headfoot.Progress(I,C);
var L=new qx.ui.progressive.structure.Default(E,B);
var A=new qx.ui.progressive.Progressive(L);
var K=new qx.ui.basic.Atom(b);
K.setTextColor(m);
K.setHeight(16);
K.setRich(true);
A.add(K);
var H=new qx.ui.progressive.model.Default();
var G=F(500);
H.addElements(G);
A.setDataModel(H);
var D=new qx.ui.progressive.renderer.table.Row(I);
A.addRenderer(r,D);
D.addRenderer(2,new demobrowser.demo.progressive.DateCellRenderer());
this.getRoot().add(A,{edge:50});
A.render();
}}});
qx.Class.define(a,{extend:qx.ui.progressive.renderer.table.cell.Default,statics:{onmouseover:function(N){N.__MQ=qx.bom.element.Style.get(N,"color");
N.style.color=q;
N.parentNode.style.height=32;
N.__MR=N.innerHTML;
var P=Date.parse(N.attributes.getNamedItem(p).nodeValue);
var O=new Date(P);
N.innerHTML=O.toUTCString()+i+O.toString();
},onmouseout:function(M){M.style.color=M.__MQ;
M.parentNode.style.height=16;
M.innerHTML=M.__MR;
}},members:{_getCellExtras:function(y){var z=[];
z.push(qx.ui.progressive.renderer.table.cell.Default.prototype._getCellExtras.call(this));
z.push(k+y.cellData.toUTCString()+j);
z.push(e,g);
return z.join(o);
}}});
})();
