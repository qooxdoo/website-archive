...
qx.Class.define(o,{extend:qx.application.Standalone,members:{__NN:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var D=this.getRoot();
this.__NN=["bottom-left","bottom-right","top-left","top-right","right-top","right-bottom","left-top","left-bottom"];
D.add(this.createTestField(g),{left:20,top:20});
D.add(this.createTestField(j),{right:20,top:20});
D.add(this.createTestField(k),{right:20,bottom:20});
D.add(this.createTestField(f),{left:20,bottom:20});
D.add(this.createTestField(h),{left:20,top:250});
D.add(this.createTestField(c),{right:20,top:250});
D.add(this.createDomClick(),{left:200,top:100});
},createTestField:function(w){var A=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));
var y=new qx.ui.popup.Popup(new qx.ui.layout.Grow);
y.add(new qx.ui.basic.Atom(l));
y.setPadding(20);
var x=new qx.ui.form.SelectBox;
var B=null;

for(var i=0;i<this.__NN.length;i++){var z=new qx.ui.form.ListItem(this.__NN[i]);
x.add(z);

if(this.__NN[i]==w){B=z;
}}A.add(x);
x.addListener(p,function(e){y.setPosition(e.getData()[0].getLabel());
});
x.setSelection([B]);
var C=new qx.ui.form.Button(m);
A.add(C);
C.addListener(q,function(e){y.placeToWidget(C);
y.show();
});
return A;
...
for(var i=0;i<this.__NN.length;i++){r.add(new qx.ui.form.ListItem(this.__NN[i]));
}t.add(r);
