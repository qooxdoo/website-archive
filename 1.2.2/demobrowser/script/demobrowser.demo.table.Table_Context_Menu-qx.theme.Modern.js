...
(function(){var n="execute",m="changeValue",l="value",k="icon/22/actions/edit-undo.png",j="Change ID sort method",h="Disabled ctx menu",g="icon/22/status/dialog-information.png",f="On",d="Change row with ID 10",c="Default ctx menu #2",F="Remove 5 rows",E="Add 10 rows",D="Context-menu selectable",C="",B="_tableModel",A="Table",z="icon/22/actions/list-remove.png",y="Whether the the first visible row should ",x=", ",w="demobrowser.demo.table.Table_Context_Menu",u="Default ctx menu #1",v="icon/22/actions/list-add.png",s="Selected rows:<br>",t="Keep first row",q="Off",r="be rendered completely when scrolling.",o="Show selection",p="Demonstrate use of alternate sorting algorithm.";
qx.Class.define(w,{extend:demobrowser.demo.table.TableDemo,members:{main:function(){qx.Class.include(qx.ui.table.Table,qx.ui.table.MTableContextMenu);
demobrowser.demo.table.TableDemo.prototype.main.call(this);
this.getRoot().setNativeContextMenu(true);
},getCaption:function(){return A;
},createTable:function(){var Y=this.createRandomRows(50);
var X=this._tableModel=new qx.ui.table.model.Simple();
X.setColumns([u,c,h,D]);
X.setData(Y);
var ba=new qx.ui.table.Table(X);
ba.set({width:600,height:400,decorator:null});
var W=ba.getTableColumnModel();
W.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
ba.setContextMenuHandler(3,this._contextMenuHandlerBoolean);
ba.setContextMenuHandler(2,this._contextMenuHandlerNull);
return ba;
},_contextMenuHandlerBoolean:function(N,O,P,Q,R){var S;

for(var i=0;i<=1;i++){S=new qx.ui.menu.Button(i?f:q);
S.setUserData(l,i?true:false);
S.addListener(n,function(e){Q.setValue(N,O,this.getUserData(l));
});
R.add(S);
}return true;
},_contextMenuHandlerNull:function(bb,bc,bd,be,bf){return true;
},createControls:function(){var L=new qx.ui.toolbar.ToolBar();
var J,K,M;
K=new qx.ui.toolbar.Part();
L.add(K);
J=new qx.ui.toolbar.Button(d,k);
J.addListener(n,function(G){var H=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,H[0][i]);
}this.info("Row 10 changed");
},this);
K.add(J);
J=new qx.ui.toolbar.Button(E,v);
J.addListener(n,function(bo){var bp=this.createRandomRows(10);
this._tableModel.addRows(bp);
this.info("10 rows added");
},this);
K.add(J);
J=new qx.ui.toolbar.Button(F,z);
J.addListener(n,function(bh){var bi=this._tableModel.getRowCount();
this._tableModel.removeRows(bi-5,5);
this.info("5 rows removed");
},this);
K.add(J);
J=new qx.ui.toolbar.Button(o,g);
J.addListener(n,function(a){var b=[];
I.getSelectionModel().iterateSelection(function(bg){b.push(bg+C);
});
this.showDialog(s+b.join(x));
},this);
K.add(J);
K=new qx.ui.toolbar.Part();
L.add(K);
var I=this._table;
M=new qx.ui.toolbar.CheckBox(t);
M.set({value:I.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(y+r)});
M.addListener(m,function(bj){I.setKeepFirstVisibleRowComplete(this.getValue());
},M);
K.add(M);
M=new qx.ui.toolbar.CheckBox(j);
M.set({value:I.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(p)});
M.addListener(m,function(T){if(T.getData()){var V=function(bq,br){var bs=bq[arguments.callee.columnIndex];
...
var U=function(bk,bl){var bm=bk[arguments.callee.columnIndex];
var bn=bl[arguments.callee.columnIndex];

if(bm%2==1&&bn%2==0){return -1;
}
if(bn%2==1&&bm%2==0){return 1;
}return (bm<bn)?1:((bm==bn)?0:-1);
};
I.getTableModel().setSortMethods(0,{ascending:V,descending:U});
...
this.__Ea=[];
},members:{__Ea:null,setContextMenuHandler:function(j,k){this.__Ea[j]=k;
},getContextMenuHandler:function(d){return this.__Ea[d];
