...
},createTable:function(){var bM=[[J,k,x,{'required':true}],[d,K,S,{'type':d,'required':true}],[L,A,T,{'options':[U,N,G],'editable':true}],[Q,v,a,{'options':[[n,null,a],[D,null,q],[w,null,E]]}],[e,z,y,{'type':e}],[l,O,M,{'regExp':W,'failMsg':Y}],[I,t,H,{'validationFunc':function(bg){if(bg.search(/^http:/)!=-1){window.open(bg);
}return bg;
}}],[V,m,true,{'type':j}]];
var bL=function(bl){var br=bl.table;
var bo=br.getTableModel();
var bp=bo.getRowData(bl.row);
var bs=bp[3];
var bq;

for(var bn in bs){switch(bn){case f:switch(bs[b]){case j:return new qx.ui.table.cellrenderer.Boolean;
case g:return new qx.ui.table.cellrenderer.Password;
}break;
case h:var bq=new qx.ui.table.cellrenderer.Replace;
var bm={};
bs[c].forEach(function(bB){if(bB instanceof Array){bm[bB[0]]=bB[2];
}});
bq.setReplaceMap(bm);
bq.addReversedReplaceMap();
return bq;
}}return new qx.ui.table.cellrenderer.Default();
};
var bK=new qx.ui.table.cellrenderer.Dynamic(bL);
var bI=function(bt){var by=bt.table;
...
break;
case s:bv.setValidationFunction(function(bd,be){var bf=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{2}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/;
...
case r:bv.setValidationFunction(function(ba,bb){var bc=new RegExp(bA[X]);

if(bc.test(ba)){return ba;
}alert(bA['failMsg']);
return bb;
});
break;
...
break;
case o:bz=function(bC,bD){if(!bC){alert("You need to supply a value here");
return bD;
}return bC;
};
break;
...
bF.setColumns([i,i,p]);
var bE={tableColumnModel:function(bN){return new qx.ui.table.columnmodel.Resize(bN);
}};
...
}var bk=event.getData();
var bh=this.getTableModel();
var bj=bh.getValue(0,bk.firstRow);
var bi=bh.getValue(bk.firstColumn,bk.firstRow);
this.info("User edited property '"+bj+"' and entered value '"+bi+"'.");
},bG);
return bG;
}}});
})();
