...
x.setColumns([c,m,k,n]);
x.setData(z);
x.setColumnEditable(1,true);
var w={tableColumnModel:function(p){return new qx.ui.table.columnmodel.Resize(p);
}};
...
qx.Class.define(b,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,members:{createCellEditor:function(q){var t=new qx.ui.window.Window(d);
t.setLayout(new qx.ui.layout.HBox(4));
t.set({padding:3,modal:true,showClose:false,showMaximize:false,showMinimize:false});
t.moveTo(300,250);
t.addListener(f,function(e){t.__MP.focus();
t.__MP.setTextSelection(0,t.__MP.getValue().length);
});
t.__MP=new qx.ui.form.TextField(q.value+"").set({allowGrowY:true});
t.add(t.__MP);
var r=new qx.ui.form.Button(a);
r.addListener(o,function(e){t.close();
});
t.add(r);
var s=new qx.ui.core.Command(j);
s.addListener(o,function(e){r.execute();
s.dispose();
s=null;
});
return t;
},getCellEditorValue:function(u){return parseFloat(u.__MP.getValue());
}}});
})();
