...
var cs=new qx.ui.basic.Label(bk);
cs.setFont(bF);
cy.add(cs,{left:20,top:20});
var cA=new qx.ui.form.List;
cA.setScrollbarX(n);
cA.set({height:280,width:150,selectionMode:bH});
var cB;

for(var i=1;i<=25;i++){cB=new qx.ui.form.ListItem(bA+i,J+((i%4)?L:bq)+U);
cA.add(cB);
!(i%9)&&(cB.setEnabled(false));
if(i==20){cA.setSelection([cB]);
}}cA.addListener(bE,function(e){var ce=e.getData();

if(ce[0]){this.debug("Value: "+ce[0].getLabel());
}});
cy.add(cA,{left:20,top:40});
var co=new qx.ui.form.RadioButton(G);
var cl=new qx.ui.form.RadioButton(bS);
var cm=new qx.ui.form.RadioButton(ca);
var ck=new qx.ui.form.RadioButton(p);
co.setUserData(bI,bC);
cl.setUserData(bI,bH);
cm.setUserData(bI,bD);
ck.setUserData(bI,bG);
cl.setValue(true);
...
var cE=new qx.ui.form.RadioButton(j);
var cD=new qx.ui.form.RadioButton(bo);
var cG=new qx.ui.form.RadioButton(bx);
cE.setUserData(bI,F);
cD.setUserData(bI,by);
cG.setUserData(bI,W);
cG.setValue(true);
...
cz.addListener(bE,function(e){for(var i=0;i<cA.getChildren().length;i++){cA.getChildren()[i].setShow(e.getData()[0].getUserData(bI));
}});
var cw=new qx.ui.form.CheckBox(bv);
var ct=new qx.ui.form.CheckBox(l).set({enabled:false});
cy.add(cw,{left:180,top:220});
cy.add(ct,{left:180,top:240});
cw.addListener(bB,function(e){if(e.getData()){var cg=cA.getSelectionMode();

if(cg==bC||cg==bG){this.debug("Drag selection is only available for the modes multi or additive");
}}cA.setDragSelection(e.getData());
});
ct.addListener(bB,function(e){if(e.getData()){var cf=cA.getSelectionMode();
...
cn.addListener(bE,function(e){var ch=e.getData()[0].getUserData(bI);
cA.setSelectionMode(ch);

if(ch==bC||ch==bG){cw.setEnabled(false);
ct.setEnabled(true);
}else if(ch==bH||ch==cd){cw.setEnabled(true);
ct.setEnabled(false);
}});
var cF=new qx.ui.basic.Label(bV);
cF.setFont(bF);
cy.add(cF,{left:330,top:20});
var cr=new qx.ui.form.List();
cr.set({height:280,width:150,selectionMode:bG});
var cB;

for(var i=1;i<=25;i++){cB=new qx.ui.form.ListItem(bA+i,bj);
cr.add(cB);
if(i==16){cr.setSelection([cB]);
}}cy.add(cr,{left:330,top:40});
var cs=new qx.ui.basic.Label(bc);
cs.setFont(bF);
cy.add(cs,{left:520,top:20});
var cj=new qx.ui.form.List;
var cp;
cj.set({width:150,selectionMode:bD});
var cv=[bu,bf,B,bW,M,br,I,Q,P,bz,bb,s,u,bN,q,be,X,v,bJ,bK,T,bR,O,bM,x,a,bX,N,cb,bg,Y,o,H,bL,bh,t,S,bm,bU,bO,h,C,bl,cc,r,bP,w,g,bn,m,V];
...
var cs=new qx.ui.basic.Label(bd);
cs.setFont(bF);
cy.add(cs,{left:20,top:350});
var cC=new qx.ui.form.List(true);
var cq;
cC.set({width:550,selectionMode:bH,height:null});
var cu=[bT,z,bs,c,bw,bi,bY,R,ba,bt,f,d,D,b,bQ,A,K,y,E];
var cx=bp;
