...
var R=this._tableModel=new qx.ui.table.model.Simple();
R.setColumns([l,b,h,g]);
R.setData(S);
R.setColumnEditable(1,true);
R.setColumnEditable(2,true);
R.setColumnSortable(3,false);
var T=new qx.ui.table.Table(R);
T.set({width:600,height:400});
T.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var Q=T.getTableColumnModel();
Q.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
Q.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(a,h));
return T;
},createControls:function(){var C=new qx.ui.toolbar.ToolBar();
var E,G;
G=new qx.ui.toolbar.Part();
C.add(G);
var B=new qx.ui.form.SelectBox().set({allowGrowY:false,alignY:x});
var J=qx.ui.table.selection.Model;
var D={"Single selection":J.SINGLE_SELECTION,"Single interval":J.SINGLE_INTERVAL_SELECTION,"Multiple intervals":J.MULTIPLE_INTERVAL_SELECTION,"Multiple intervals (toggle)":J.MULTIPLE_INTERVAL_SELECTION_TOGGLE,"No selection":J.NO_SELECTION};

for(var I in D){var F=new qx.ui.form.ListItem(I);
F.setUserData(j,D[I]+i);
B.add(F);
}B.addListener(y,function(e){this._table.getSelectionModel().setSelectionMode(parseInt(e.getData()[0].getUserData(j)));
},this);
G.add(B);
G=new qx.ui.toolbar.Part();
C.add(G);
E=new qx.ui.toolbar.Button(s,o);
E.addListener(k,function(L){var M=this.createRandomRows(10);
this._tableModel.addRows(M);
this.info("5 rows added");
},this);
G.add(E);
E=new qx.ui.toolbar.Button(w,r);
E.addListener(k,function(U){var V=this._tableModel.getRowCount();
...
G=new qx.ui.toolbar.Part();
C.add(G);
E=new qx.ui.toolbar.Button(v);
E.addListener(k,function(A){this._table.resetSelection();
},this);
G.add(E);
E=new qx.ui.toolbar.Button(z);
E.addListener(k,function(K){this._table.resetCellFocus();
},this);
G.add(E);
var H=new qx.ui.toolbar.CheckBox(p);
H.addListener(d,function(P){this._table.setShowCellFocusIndicator(!H.isValue());
},this);
G.add(H);
E=new qx.ui.toolbar.Button(n,c);
E.addListener(k,function(N){var O=[];
this._table.getSelectionModel().iterateSelection(function(W){O.push(W+i);
});
this.showDialog(m+O.join(q));
},this);
G.add(E);
return C;
