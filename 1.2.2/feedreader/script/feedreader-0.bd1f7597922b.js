...
}},load:function(u){u.setState(d);
var v=j+u.getUrl()+h;
var w=new qx.data.store.Yql(v,{manipulateData:function(q){if(q.query.results){q=q.query.results.item||q.query.results.entry;
for(var i=0;i<q.length;i++){if(!qx.lang.Type.isString(q[i].title)){q[i].title=q[i].title.content;
}}return q;
}return m;
}},qx.bom.client.Feature.SSL);
w.addListener(k,this.__mN(u),this);
w.addListener(c,qx.lang.Function.bind(this.__mM,this,u),this);
},__mM:function(r,e){if(e.getData()==b||e.getData()==g||e.getData()==m){r.setState(l);
}},__mN:function(s){return function(e){var t=e.getData();
if(t==m){s.setState(l);
return;
}s.setArticles(t);
s.setState(k);
...
(function(){var I="static",H="user",G="execute",F="loading",E="selection[0]",D="title",C="north",B="main",A="new",z="Surfin' Safari",bQ="changeModel",bP="__pD",bO="selection[0].state",bN="Heise",bM="__pJ",bL="__pG",bK="F1",bJ="article",bI="partLoaded",bH="Control+A",P="__pE",Q="http://blogs.msdn.com/ie/rss.xml",N="http://my.opera.com/desktopteam/xml/rss/blog/",O="settings",L="feedreader/images/loading22.gif",M="error",J="change",K="http://blogs.msdn.com/jscript/rss.xml",X="__pz",Y="loaded",bl="feeds",bh="__pH",bt="__pB",bo="The Mozilla Blog",bD="enabled",by="http://webkit.org/blog/feed/",bd="__pL",bG="separator-vertical",bF="http://feeds2.feedburner.com/qooxdoo/news/content",bE="http://blog.mozilla.com/feed/",bc="__pI",bf="vertical",bg="http://www.heise.de/newsticker/heise-atom.xml",bj="__pF",bm="state",bp="JScript Team Blog",bv="Feeds",bA="http://feeds2.feedburner.com/ajaxian",R="Control+D",S="icon/22/actions/process-stop.png",be="selection[0].articles",bs="white",br="IEBlog",bq="Ajaxian",bx="User Feeds",bw="icon/22/apps/internet-feed-reader.png",bn="model",bu="feedreader.Application",w="http://daringfireball.net/index.xml",bz="Control+P",T="Static Feeds",U="Control+R",bi="qooxdoo News",x="__pC",y="icon/22/places/folder-open.png",bb="__py",V="Opera Desktop Blog",W="feedreader.model.Feed",ba="Daring Fireball",bk="__pK",bC="addfeed",bB="__pM";
qx.Class.define(bu,{extend:qx.application.Standalone,members:{__py:null,__pz:null,__pA:null,__pB:null,__pC:null,__pD:null,__pE:null,__pF:null,__pG:null,__pH:null,__pI:null,__pJ:null,__pK:null,__pL:null,__pM:null,main:function(){qx.application.Standalone.prototype.main.call(this);
{};
...
this.reload();
},_initializeModel:function(){this.__pK=new feedreader.model.FeedFolder(bv);
this.__pM=new feedreader.model.FeedFolder(this.tr(T));
this.__pK.getFeeds().push(this.__pM);
this.__pM.getFeeds().push(new feedreader.model.Feed(bi,bF,I));
this.__pM.getFeeds().push(new feedreader.model.Feed(bp,K,I));
this.__pM.getFeeds().push(new feedreader.model.Feed(ba,w,I));
this.__pM.getFeeds().push(new feedreader.model.Feed(z,by,I));
this.__pM.getFeeds().push(new feedreader.model.Feed(bq,bA,I));
this.__pL=new feedreader.model.FeedFolder(this.tr(bx));
this.__pK.getFeeds().push(this.__pL);
this.__pL.getFeeds().push(new feedreader.model.Feed(bN,bg,H));
this.__pL.getFeeds().push(new feedreader.model.Feed(br,Q,H));
this.__pL.getFeeds().push(new feedreader.model.Feed(bo,bE,H));
this.__pL.getFeeds().push(new feedreader.model.Feed(V,N,H));
},_setUpBinding:function(){var k={converter:this._state2iconConverter};
this.__py=new qx.data.controller.Tree(this.__pK,this.__pB,bl,D);
this.__py.setIconOptions(k);
this.__py.setIconPath(bm);
this.__pz=new qx.data.controller.List(null,this.__pE.getList(),D);
this.__py.bind(be,this.__pz,bn);
this.__pz.bind(E,this.__pI,bJ);
this.__pz.getSelection().addListener(J,this._listControllerChange,this);
this.__pz.addListener(bQ,this._treeControllerChange,this);
var l={converter:this._state2loadingConverter};
this.__py.bind(bO,this.__pE,F,l);
l={converter:this._category2enabledConverter};
this.__py.bind(E,this.__pD.getRemoveButton(),bD,l);
},_category2enabledConverter:function(b){return !!b&&b.getCategory()==H;
},_state2iconConverter:function(a){if(a==A||a==F){return L;
}else if(a==Y){return bw;
}else if(a==M){return S;
}return y;
},_state2loadingConverter:function(v){if(v==A||v==F){return true;
}return false;
},_listControllerChange:function(r){var s=this.__py.getSelection().getItem(0);
var t=this.__pz.getSelection().getItem(0);
if(t!=undefined){s.setSelectedArticle(t);
}},_treeControllerChange:function(m){var n=this.__py.getSelection().getItem(0);
if(n!=null&&n.classname==W&&n.getSelectedArticle()!=null&&n.getArticles().contains(n.getSelectedArticle())){this.__pz.getSelection().push(n.getSelectedArticle());
...
p.add(this.__pC,{edge:C});
this.__pD=new feedreader.view.ToolBar(this);
p.add(this.__pD,{edge:C});
this.__pF=new qx.ui.splitpane.Pane();
p.add(this.__pF);
this.__pB=new qx.ui.tree.Tree();
this.__pB.setWidth(250);
this.__pB.setBackgroundColor(bs);
this.__pF.add(this.__pB,0);
this.__pG=new qx.ui.splitpane.Pane(bf);
this.__pG.setDecorator(null);
this.__pF.add(this.__pG,1);
this.__pE=new feedreader.view.List(this.__pK);
this.__pE.setHeight(200);
this.__pE.setDecorator(B);
this.__pG.add(this.__pE,0);
this.__pI=new feedreader.view.Article();
this.__pI.setDecorator(B);
this.__pG.add(this.__pI,1);
},_initializeCommands:function(){var q={};
q.reload=new qx.ui.core.Command(U);
q.reload.addListener(G,this.reload,this);
q.about=new qx.ui.core.Command(bK);
q.about.addListener(G,this.showAbout,this);
q.preferences=new qx.ui.core.Command(bz);
q.preferences.addListener(G,this.showPreferences,this);
q.addFeed=new qx.ui.core.Command(bH);
q.addFeed.addListener(G,this.showAddFeed,this);
q.removeFeed=new qx.ui.core.Command(R);
q.removeFeed.addListener(G,this.removeFeed,this);
this.__pA=q;
},getCommand:function(u){return this.__pA[u];
},addFeed:function(d,f,g){var h=new feedreader.model.Feed(d,f,g);
this.__pL.getFeeds().push(h);
var j=feedreader.io.FeedLoader.getInstance();
j.load(h);
},removeFeed:function(){var bT=this.__py.getSelection().getItem(0);
if(bT&&bT.getCategory&&bT.getCategory()!==I){var bU=this.__pL.getFeeds();
for(var i=0;i<bU.length;i++){if(bT===bU.getItem(i)){bU.splice(i,1);
return;
}}}},reload:function(){var c=feedreader.io.FeedLoader.getInstance();
c.loadAll(this.__pK);
this.__pI.setArticle(null);
},showPreferences:function(){qx.io.PartLoader.require([O],function(){if(!this.__pH){this.__pH=new feedreader.view.PreferenceWindow();
this.getRoot().add(this.__pH);
}this.__pH.center();
this.__pH.open();
},this);
},showAbout:function(){alert(this.tr("FeedReader (qooxdoo powered)"));
},showAddFeed:function(){qx.io.PartLoader.require([bC],function(){if(!this.__pJ){this.__pJ=new feedreader.view.AddFeedWindow(this);
this.getRoot().add(this.__pJ);
}this.__pJ.center();
this.__pJ.open();
},this);
}},destruct:function(){this.__pA=null;
this._disposeObjects(bP,P,bc,bt,bk,bj,bL,x,bB,bd,bb,X,bh,bM);
}});
})();
...
B.setLineHeight(1.8);
this.setFont(B);
},properties:{article:{apply:e,nullable:true,check:d}},members:{_applyArticle:function(w,x){var y=this.__ri();
this.setHtml(y);
qx.html.Element.flush();
var A=this.getContentElement().getDomElement();
var z=A.getElementsByTagName(b);
for(var i=0;i<z.length;i++){z[i].target=t;
}},__ri:function(){var D=this.getArticle();
if(!D){return v;
