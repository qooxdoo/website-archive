...
part=splits[i]){if(!parent[part]){parent[part]={};
}parent=parent[part];
}parent[part]=object;
return part;
},
...
open:function(){this._frame.style.display="";
},
close:function(){this._frame.style.display="none";
},
emu:true,
...
}else if(window.console&&!console.firebug){window.console=statics;
}}});
...
switch(vEngine){case "gecko":var vEngineBoxSizingAttr="-moz-box-sizing";
break;
case "webkit":var vEngineBoxSizingAttr="-webkit-box-sizing";
break;
case "mshtml":var vEngineBoxSizingAttr=null;
break;
default:var vEngineBoxSizingAttr="box-sizing";
...
get:function(key){if(this.__settings[key]===undefined){throw new Error('Setting "'+key+'" is not defined.');
}
if(this.__settings[key].defaultValue===undefined){throw new Error('Setting "'+key+'" is not supported by API.');
}
if(this.__settings[key].value!==undefined){return this.__settings[key].value;
}return this.__settings[key].defaultValue;
},
__init:function(){if(window.qxsettings){for(var key in qxsettings){{};
if(!this.__settings[key]){this.__settings[key]={};
...
}if(!config.extend){config.type="static";
}{};
...
construct,
destruct){var clazz;
if(!extend){clazz=statics||{};
}else{if(!construct){construct=this.__createDefaultConstructor();
...
__disposeAll:false,
$$type:"Object",
toHashCode:function(o){if(o._hashCode!=null){return o._hashCode;
...
var disposeStart;
var vObject;
for(var i=qx.core.Object.__db.length-1;i>=0;i--){vObject=qx.core.Object.__db[i];
if(vObject&&vObject.__disposed===false){try{vObject.dispose();
}catch(ex){console.warn("Could not dispose: "+vObject+": "+ex);
}}}var elems,
i,
l,
elem,
...
set:function(data){var setter=qx.core.Property.$$method.set;
...
{statics:{escapeEntities:function(str,
...
"mshtml")){for(var i=0;i<6;i++){members[applyRuntime+propertiesUpper[i]]=new Function(parameter,
style+"pos"+propertiesUpper[i]+"=v");
members[resetRuntime+propertiesUpper[i]]=new Function(style+properties[i]+"=''");
}}else{for(var i=0;i<10;i++){members[applyRuntime+propertiesUpper[i]]=new Function(parameter,
...
{"mshtml":function(value,
old){if(value){return this.removeHtmlProperty("unselectable");
}else{return this.setHtmlProperty("unselectable",
"on");
}},
"gecko":function(value,
...
old){if(value){this.removeStyleProperty("-webkit-user-select");
}else{this.setStyleProperty("-webkit-user-select",
"ignore");
}},
...
old){var pv=value;
var pn="overflow";
switch(pv){case "scrollX":pn="overflowX";
pv="scroll";
break;
...
case "mousewheel":if(qx.core.Variant.isSet("qx.client",
"gecko")){vEventWasProcessed?this._onmousewheel(vOriginalTarget||vDispatchTarget,
vEventObject):qx.event.handler.EventHandler.stopDomEvent(vDomEvent);
}break;
...
{"gecko":function(vTarget,
vEvent){if(vTarget==null){return;
}if(vTarget.getOverflowY()=="hidden"){return this._onmousewheel(vTarget.getParent(),
vEvent);
}var vScrollTop=vTarget.getScrollTop();
var vDelta=20*vEvent.getWheelDelta();
if(vScrollTop==0&&vDelta>0){return this._onmousewheel(vTarget.getParent(),
vEvent);
}var vScrollHeight=vTarget.getScrollHeight();
var vClientHeight=vTarget.getClientHeight();
if(vScrollTop+vClientHeight>=vScrollHeight&&vDelta<0){return this._onmousewheel(vTarget.getParent(),
vEvent);
...
if(value){if(value=="pointer"&&qx.core.Client.getInstance().isMshtml()){var cursor="hand";
}else{var cursor=value;
...
defer:function(){if(qx.core.Setting.get("qx.boxModelCorrection")){qx.html.StyleSheet.createElement("html,body{margin:0;border:0;padding:0;}"+" html{border:0 none;} *{"+qx.core.Client.getInstance().getEngineBoxSizingAttribute()+":border-box;} img{"+qx.core.Client.getInstance().getEngineBoxSizingAttribute()+":content-box;}");
}
...
if(entry.include){var incl=this.styleFromTheme(theme,
entry.include,
states);
var local=entry.style(states);
result={};
for(var key in incl){if(local[key]===undefined){result[key]=incl[key];
...
old){this.__decoration=value||null;
},
...
members:{show:function(source){var data=this.__sources;
data[source]===undefined?data[source]=1:data[source]++;
},
hide:function(source){var data=this.__sources;
data[source]===undefined?data[source]=0:data[source]--;
if(data[source]<0){data[source]=0;
}},
register:function(source){var data=this.__sources;
if(data[source]===undefined){data[source]=0;
}},
getVisibleImages:function(){var data=this.__sources;
var list={};
for(var source in data){if(data[source]>0){list[source]=true;
}}return list;
},
getHiddenImages:function(){var data=this.__sources;
var list={};
for(var source in data){if(data[source]<1){list[source]=true;
}}return list;
}},
destruct:function(){this._disposeFields("__sources");
}});
...
if(qx.core.Variant.isSet("qx.client",
"mshtml")){statics.__originalOnreadystatechange=members._onreadystatechange;
members._onreadystatechange=function(){var self=this;
window.setTimeout(function(){statics.__originalOnreadystatechange.call(self);
},
0);
};
}},
...
Error,
"An object of the name 'testrunner.Same' aready exists and overwriting is not allowed!");
},
