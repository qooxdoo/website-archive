...
(function(){var a=". Error message: ",b="Boolean",c=").",d="set",f="deepBinding",g=") to the object '",h="item",k="Please use only one array at a time: ",l="Integer",m="reset",n=" of object ",p="qx.data.SingleValueBinding",q="Binding property ",r="Failed so set value ",s="change",t="Binding could not be found!",u="get",v="^",w=" does not work.",x="String",y="Binding from '",z="",A="PositiveNumber",B="]",C="[",D=".",E="PositiveInteger",F='No number or \'last\' value hast been given in an array binding: ',G="' (",H=" on ",I="Binding does not exist!",J="Number",K=".[",L="Date",M=" not possible: No event available. ",N="last";qx.Class.define(p,{statics:{__bb:{},bind:function(R,bf,bd,T,bc){{}
;var bg=this.__bd(R,bf,bd,T,bc);var V=bf.split(D);var Q=this.__bl(V);var ba=[];var U=[];var W=[];var bb=[];var S=R;try{for(var i=0;i<V.length;i++ ){if(Q[i]!==z){bb.push(s);}
else {bb.push(this.__be(S,V[i]));}
;ba[i]=S;if(i==V.length-1){if(Q[i]!==z){var bi=Q[i]===N?S.length-1:Q[i];var P=S.getItem(bi);this.__bk(P,bd,T,bc,R);W[i]=this.__bm(S,bb[i],bd,T,bc,Q[i]);}
else {if(V[i]!=null&&S[u+qx.lang.String.firstUp(V[i])]!=null){var P=S[u+qx.lang.String.firstUp(V[i])]();this.__bk(P,bd,T,bc,R);}
;W[i]=this.__bm(S,bb[i],bd,T,bc);}
;}
else {var O={index:i,propertyNames:V,sources:ba,listenerIds:W,arrayIndexValues:Q,targetObject:bd,targetPropertyChain:T,options:bc,listeners:U};var Y=qx.lang.Function.bind(this.__bc,this,O);U.push(Y);W[i]=S.addListener(bb[i],Y);}
...
;}
;if(!S){this.__bk(S,bd,T,bc,R);break;}
;}
;}
...
;}
;throw bj;}
;var bh={type:f,listenerIds:W,sources:ba,targetListenerIds:bg.listenerIds,targets:bg.targets};this.__bn(bh,R,bf,bd,T);return bh;}
,__bc:function(bq){if(bq.options&&bq.options.onUpdate){bq.options.onUpdate(bq.sources[bq.index],bq.targetObject);}
...
;if(!bk){this.__bg(bq.targetObject,bq.targetPropertyChain,bq.options.converter());}
else {this.__bf(bq.targetObject,bq.targetPropertyChain);}
;}
else {this.__bf(bq.targetObject,bq.targetPropertyChain);}
;break;}
;if(j==bq.propertyNames.length-1){if(qx.Class.implementsInterface(bo,qx.data.IListData)){var bs=bq.arrayIndexValues[j]===N?bo.length-1:bq.arrayIndexValues[j];var bl=bo.getItem(bs);this.__bk(bl,bq.targetObject,bq.targetPropertyChain,bq.options,bq.sources[bq.index]);bq.listenerIds[j]=this.__bm(bo,s,bq.targetObject,bq.targetPropertyChain,bq.options,bq.arrayIndexValues[j]);}
else {if(bq.propertyNames[j]!=null&&bo[u+qx.lang.String.firstUp(bq.propertyNames[j])]!=null){var bl=bo[u+qx.lang.String.firstUp(bq.propertyNames[j])]();this.__bk(bl,bq.targetObject,bq.targetPropertyChain,bq.options,bq.sources[bq.index]);}
;var bm=this.__be(bo,bq.propertyNames[j]);bq.listenerIds[j]=this.__bm(bo,bm,bq.targetObject,bq.targetPropertyChain,bq.options);}
;}
else {if(bq.listeners[j]==null){var bn=qx.lang.Function.bind(this.__bc,this,bq);bq.listeners.push(bn);}
;if(qx.Class.implementsInterface(bo,qx.data.IListData)){var bm=s;}
else {var bm=this.__be(bo,bq.propertyNames[j]);}
;bq.listenerIds[j]=bo.addListener(bm,bq.listeners[j]);}
;}
;}
,__bd:function(bu,bC,bG,by,bA){var bx=by.split(D);var bv=this.__bl(bx);var bF=[];var bE=[];var bz=[];var bD=[];var bw=bG;for(var i=0;i<bx.length-1;i++ ){if(bv[i]!==z){bD.push(s);}
else {try{bD.push(this.__be(bw,bx[i]));}
catch(e){break;}
;}
...
else {try{var bI=qx.data.SingleValueBinding.__be(bJ,bx[j]);}
catch(e){break;}
;}
...
,updateTarget:function(bL,bO,bQ,bM,bP){var bN=this.resolvePropertyChain(bL,bO);bN=qx.data.SingleValueBinding.__bo(bN,bQ,bM,bP,bL);this.__bg(bQ,bM,bN);}
,resolvePropertyChain:function(o,bR){var bS=this.__bi(bR);return this.__bj(o,bS,bS.length);}
,__be:function(bU,bV){var bT=this.__bp(bU,bV);if(bT==null){if(qx.Class.supportsEvent(bU.constructor,bV)){bT=bV;}
...
;}
;return bT;}
,__bf:function(cb,bY){var ca=this.__bi(bY);var bX=this.__bj(cb,ca);if(bX!=null){var cc=ca[ca.length-1];var bW=this.__bh(cc);if(bW){this.__bg(cb,bY,null);return;}
;if(bX[m+qx.lang.String.firstUp(cc)]!=undefined){bX[m+qx.lang.String.firstUp(cc)]();}
else {bX[d+qx.lang.String.firstUp(cc)](null);}
;}
;}
,__bg:function(ci,cf,cg){var ch=this.__bi(cf);var ce=this.__bj(ci,ch);if(ce){var cj=ch[ch.length-1];var cd=this.__bh(cj);if(cd){if(cd===N){cd=ce.length-1;}
;ce.setItem(cd,cg);}
else {ce[d+qx.lang.String.firstUp(cj)](cg);}
;}
;}
,__bh:function(cm){var ck=/^\[(\d+|last)\]$/;var cl=cm.match(ck);if(cl){return cl[1];}
;return null;}
,__bi:function(cn){return cn.replace(/\[/g,K).split(D).filter(function(co){return co!==z;}
);}
,__bj:function(cu,cp,cq){cq=cq||cp.length-1;var cs=cu;for(var i=0;i<cq;i++ ){try{var ct=cp[i];var cr=this.__bh(ct);if(cr){if(cr===N){cr=cs.length-1;}
;cs=cs.getItem(cr);}
...
;}
;return cs;}
,__bk:function(cA,cw,cy,cz,cx){cA=this.__bo(cA,cw,cy,cz,cx);if(cA===undefined){this.__bf(cw,cy);}
;if(cA!==undefined){try{this.__bg(cw,cy,cA);if(cz&&cz.onUpdate){cz.onUpdate(cx,cw,cA);}
;}
...
;}
;}
,__bl:function(cB){var cC=[];for(var i=0;i<cB.length;i++ ){var name=cB[i];if(qx.lang.String.endsWith(name,B)){var cD=name.substring(name.indexOf(C)+1,name.indexOf(B));if(name.indexOf(B)!=name.length-1){throw new Error(k+name+w);}
...
;}
;return cC;}
,__bm:function(cE,cH,cM,cK,cI,cG){{var cF;}
;var cJ=function(cP,e){if(cP!==z){if(cP===N){cP=cE.length-1;}
;var cQ=cE.getItem(cP);if(cQ===undefined){qx.data.SingleValueBinding.__bf(cM,cK);}
;var cO=e.getData().start;var cN=e.getData().end;if(cP<cO||cP>cN){return;}
;}
else {var cQ=e.getData();}
;{}
;cQ=qx.data.SingleValueBinding.__bo(cQ,cM,cK,cI,cE);{}
;try{if(cQ!==undefined){qx.data.SingleValueBinding.__bg(cM,cK,cQ);}
else {qx.data.SingleValueBinding.__bf(cM,cK);}
...
,__bn:function(cW,cS,cV,cT,cU){if(this.__bb[cS.toHashCode()]===undefined){this.__bb[cS.toHashCode()]=[];}
;this.__bb[cS.toHashCode()].push([cW,cS,cV,cT,cU]);}
,__bo:function(db,dh,da,dd,cX){if(dd&&dd.converter){var de;if(dh.getModel){de=dh.getModel();}
;return dd.converter(db,de,cX,dh);}
else {var dc=this.__bi(da);var cY=this.__bj(dh,dc);var di=da.substring(da.lastIndexOf(D)+1,da.length);if(cY==null){return db;}
;var df=qx.Class.getPropertyDefinition(cY.constructor,di);var dg=df==null?z:df.check;return this.__bq(db,dg);}
;}
,__bp:function(dj,dl){var dk=qx.Class.getPropertyDefinition(dj.constructor,dl);if(dk==null){return null;}
;return dk.event;}
,__bq:function(dp,dn){var dm=qx.lang.Type.getClass(dp);if((dm==J||dm==x)&&(dn==l||dn==E)){dp=parseInt(dp,10);}
;if((dm==b||dm==J||dm==L)&&dn==x){dp=dp+z;}
;if((dm==J||dm==x)&&(dn==J||dn==A)){dp=parseFloat(dp);}
;return dp;}
,removeBindingFromObject:function(dq,ds){if(ds.type==f){for(var i=0;i<ds.sources.length;i++ ){if(ds.sources[i]){ds.sources[i].removeListenerById(ds.listenerIds[i]);}
;}
;for(var i=0;i<ds.targets.length;i++ ){if(ds.targets[i]){ds.targets[i].removeListenerById(ds.targetListenerIds[i]);}
;}
;}
else {dq.removeListenerById(ds);}
;var dr=this.__bb[dq.toHashCode()];if(dr!=undefined){for(var i=0;i<dr.length;i++ ){if(dr[i][0]==ds){qx.lang.Array.remove(dr,dr[i]);return;}
;}
;}
;throw new Error(t);}
,removeAllBindingsForObject:function(du){{}
;var dt=this.__bb[du.toHashCode()];if(dt!=undefined){for(var i=dt.length-1;i>=0;i-- ){this.removeBindingFromObject(du,dt[i][0]);}
;}
;}
,getAllBindingsForObject:function(dv){if(this.__bb[dv.toHashCode()]===undefined){this.__bb[dv.toHashCode()]=[];}
;return this.__bb[dv.toHashCode()];}
,removeAllBindings:function(){for(var dx in this.__bb){var dw=qx.core.ObjectRegistry.fromHashCode(dx);if(dw==null){delete this.__bb[dx];continue;}
;this.removeAllBindingsForObject(dw);}
;this.__bb={};}
,getAllBindings:function(){return this.__bb;}
,showBindingInLog:function(dz,dB){var dA;for(var i=0;i<this.__bb[dz.toHashCode()].length;i++ ){if(this.__bb[dz.toHashCode()][i][0]==dB){dA=this.__bb[dz.toHashCode()][i];break;}
;}
;if(dA===undefined){var dy=I;}
else {var dy=y+dA[1]+G+dA[2]+g+dA[3]+G+dA[4]+c;}
;qx.log.Logger.debug(dy);}
,showAllBindingsInLog:function(){for(var dD in this.__bb){var dC=qx.core.ObjectRegistry.fromHashCode(dD);for(var i=0;i<this.__bb[dD].length;i++ ){this.showBindingInLog(dC,this.__bb[dD][i][0]);}
;}
;}
}});}
)();
...
(function(){var c="-",d="",e="qx.core.ObjectRegistry",f="Disposed ",g="$$hash",h="-0",j=" objects",k="Could not dispose object ",m=": ";qx.Bootstrap.define(e,{statics:{inShutDown:false,__bJ:{},__bU:0,__bV:[],__bW:d,__bX:{},register:function(n){var q=this.__bJ;if(!q){return;}
;var p=n.$$hash;if(p==null){var o=this.__bV;if(o.length>0&&true){p=o.pop();}
else {p=(this.__bU++ )+this.__bW;}
;n.$$hash=p;{}
;}
;{}
;q[p]=n;}
,unregister:function(r){var s=r.$$hash;if(s==null){return;}
;var t=this.__bJ;if(t&&t[s]){delete t[s];this.__bV.push(s);}
;try{delete r.$$hash;}
...
;var w=this.__bV;if(w.length>0){x=w.pop();}
else {x=(this.__bU++ )+this.__bW;}
;return v.$$hash=x;}
,clearHashCode:function(y){{}
;var z=y.$$hash;if(z!=null){this.__bV.push(z);try{delete y.$$hash;}
catch(A){if(y.removeAttribute){y.removeAttribute(g);}
;}
;}
;}
,fromHashCode:function(B){return this.__bJ[B]||null;}
,shutdown:function(){this.inShutDown=true;var D=this.__bJ;var F=[];for(var C in D){F.push(C);}
...
;}
;break;}
;qx.Bootstrap.debug(this,f+l+j);delete this.__bJ;}
...
},defer:function(H){if(window&&window.top){var frames=window.top.frames;for(var i=0;i<frames.length;i++ ){if(frames[i]===window){H.__bW=c+(i+1);return;}
;}
;}
;H.__bW=h;}
});}
...
(function(){var a="qx.log.Logger",b="[",c="...(+",d="array",e=")",f="info",g="node",h="instance",j="string",k="null",m="error",n="#",o="class",p=": ",q="warn",r="document",s="{...(",t="",u="number",v="stringify",w="]",x="date",y="unknown",z="function",A="text[",B="[...(",C="boolean",D="\n",E=")}",F="debug",G=")]",H="map",I="undefined",J="object";qx.Bootstrap.define(a,{statics:{__ch:F,setLevel:function(K){this.__ch=K;}
,getLevel:function(){return this.__ch;}
,setTreshold:function(L){this.__ck.setMaxMessages(L);}
,getTreshold:function(){return this.__ck.getMaxMessages();}
,__ci:{},__cj:0,register:function(P){if(P.$$id){return;}
;var M=this.__cj++ ;this.__ci[M]=P;P.$$id=M;var N=this.__cl;var O=this.__ck.getAllLogEvents();for(var i=0,l=O.length;i<l;i++ ){if(N[O[i].level]>=N[this.__ch]){P.process(O[i]);}
;}
;}
,unregister:function(Q){var R=Q.$$id;if(R==null){return;}
;delete this.__ci[R];delete Q.$$id;}
,debug:function(T,S){qx.log.Logger.__cm(F,arguments);}
,info:function(V,U){qx.log.Logger.__cm(f,arguments);}
,warn:function(X,W){qx.log.Logger.__cm(q,arguments);}
,error:function(ba,Y){qx.log.Logger.__cm(m,arguments);}
,trace:function(bb){var bc=qx.dev.StackTrace.getStackTrace();qx.log.Logger.__cm(f,[(typeof bb!==I?[bb].concat(bc):bc).join(D)]);}
...
,__ck:new qx.log.appender.RingBuffer(50),__cl:{debug:0,info:1,warn:2,error:3},__cm:function(bz,bB){var bE=this.__cl;if(bE[bz]<bE[this.__ch]){return;}
;var by=bB.length<2?null:bB[0];var bD=by?1:0;var bA=[];for(var i=bD,l=bB.length;i<l;i++ ){bA.push(this.__co(bB[i],true));}
...
;}
;this.__ck.process(bG);var bC=this.__ci;for(var bH in bC){bC[bH].process(bG);}
;}
,__cn:function(bJ){if(bJ===undefined){return I;}
...
;return y;}
,__co:function(bP,bO){var bS=this.__cn(bP);var bM=y;var bL=[];switch(bS){case k:case I:bM=bS;break;case j:case u:case C:case x:bM=bP;break;case g:if(bP.nodeType===9){bM=r;}
...
;bQ=bR[i];bK=this.__co(bP[bQ],false);bK.key=bQ;bM.push(bK);}
;}
else {var bN=0;for(var bQ in bP){bN++ ;}
;bM=s+bN+E;}
;break;};return {type:bS,text:bM,trace:bL};}
},defer:function(bT){var bU=qx.Bootstrap.$$logs;for(var i=0;i<bU.length;i++ ){bT.__cm(bU[i][0],bU[i][1]);}
...
;}
;qx.core.ObjectRegistry.unregister(this);this.__cK=null;if(qx.core.Environment.get(d)){var F=this.constructor;var J;var K=this.__K.$$store;var H=K.user;var I=K.theme;var D=K.inherit;var G=K.useinit;var E=K.init;while(F){J=F.$$properties;if(J){for(var name in J){if(J[name].dereference){this[H[name]]=this[I[name]]=this[D[name]]=this[G[name]]=this[E[name]]=undefined;}
;}
;}
;F=F.superclass;}
;}
...
;}
;}
,__cL:function(L){var K=[];if(qx.ui.mobile&&L instanceof qx.ui.mobile.core.Widget){K=[qx.ui.mobile.container.Composite];}
...
,splice:function(F,H,J){var N=this.__lS.length;var I=this.__lS.splice.apply(this.__lS,arguments);if(this.__lS.length!=N){this.__lT();}
...
;this.__lU(F+(arguments.length-2)-H,this.length);if(L||D){var G=[];for(var i=2;i<arguments.length;i++ ){G[i-2]=arguments[i];}
...
;var O=this.__lS.concat();this.__lS.sort.apply(this.__lS,arguments);if(qx.lang.Array.equals(this.__lS,O)===true){return;}
;this.__lU(0,this.length);this.fireDataEvent(g,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(o,{value:this.__lS,name:k+(this.length-1),old:O,item:this});}
,unshift:function(Q){for(var i=arguments.length-1;i>=0;i-- ){this.__lS.unshift(arguments[i]);this.__lT();this.__lU(0,this.length);this.fireDataEvent(o,{value:[this.__lS[0]],name:m,old:[this.__lS[1]],item:this});this.fireDataEvent(g,{start:0,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,toArray:function(){return this.__lS;}
,getItem:function(R){return this.__lS[R];}
,setItem:function(S,U){var T=this.__lS[S];if(T===U){return;}
;this.__lS[S]=U;this._registerEventChaining(U,T,S);if(this.length!=this.__lS.length){this.__lT();}
...
,indexOf:function(V){return this.__lS.indexOf(V);}
,lastIndexOf:function(W){return this.__lS.lastIndexOf(W);}
,toString:function(){if(this.__lS!=null){return this.__lS.toString();}
;return f;}
,contains:function(X){return this.__lS.indexOf(X)!==-1;}
...
;var bl=this.getLength();var bk=this.__lS.concat();this.__lS.length=0;this.__lT();this.fireDataEvent(o,{value:[],name:k+(bl-1),old:bk,item:this});this.fireDataEvent(g,{start:0,end:bl-1,type:l,removed:bk,added:[]},null);return bk;}
,append:function(bm){if(bm instanceof qx.data.Array){bm=bm.toArray();}
;{}
;Array.prototype.push.apply(this.__lS,bm);for(var i=0;i<bm.length;i++ ){this._registerEventChaining(bm[i],null,this.__lS.length+i);}
;var bn=this.length;this.__lT();var name=bn==(this.length-1)?bn:bn+a+(this.length-1);this.fireDataEvent(o,{value:bm,name:name+f,old:[],item:this});this.fireDataEvent(g,{start:bn,end:this.length-1,type:b,added:bm,removed:[]},null);}
...
,forEach:function(bu,bv){for(var i=0;i<this.__lS.length;i++ ){bu.call(bv,this.__lS[i],i,this);}
;}
,filter:function(bw,self){return new qx.data.Array(this.__lS.filter(bw,self));}
,map:function(bx,self){return new qx.data.Array(this.__lS.map(bx,self));}
,some:function(by,self){return this.__lS.some(by,self);}
,every:function(bz,self){return this.__lS.every(bz,self);}
,reduce:function(bB,bA){return this.__lS.reduce(bB,bA);}
,reduceRight:function(bD,bC){return this.__lS.reduceRight(bD,bC);}
,__lT:function(){var bE=this.length;this.length=this.__lS.length;this.fireDataEvent(p,this.length,bE);}
,__lU:function(bG,bF){for(var i=bG;i<bF;i++ ){this._registerEventChaining(this.__lS[i],this.__lS[i],i);}
;}
},destruct:function(){for(var i=0;i<this.__lS.length;i++ ){var bH=this.__lS[i];this._applyEventPropagation(null,bH,i);if(this.isAutoDisposeItems()&&bH&&bH instanceof qx.core.Object){bH.dispose();}
;}
;this.__lS=null;}
});}
)();
