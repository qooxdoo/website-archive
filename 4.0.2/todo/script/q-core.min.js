...
,_getBoundingClientRect:function(u){var t=u.getBoundingClientRect();if(qx.core.Environment.get(l)===11&&!!document.msFullscreenElement&&window!==window.top&&this.__isChildOfFullScreenElement(u)){var r={};for(var s in t){r[s]=t[s]*100;}
;t=r;}
;return t;}
,__isChildOfFullScreenElement:function(v){if(document.msFullscreenElement===v){return true;}
;return qx.dom.Hierarchy.contains(document.msFullscreenElement,v);}
,__hiddenScrollbars:{visible:true,hidden:true},getContentWidth:function(z){var w=qx.bom.element.Style;var x=qx.bom.element.Style.get(z,k);var y=parseInt(w.get(z,g)||d,10);var C=parseInt(w.get(z,e)||d,10);if(this.__hiddenScrollbars[x]){var B=z.clientWidth;if((qx.core.Environment.get(b)==h)||qx.dom.Node.isBlockNode(z)){B=B-y-C;}
...
else {var A=z.scrollWidth-y;if(qx.core.Environment.get(b)==a&&qx.core.Environment.get(f)>=6){A-=C;}
;return A;}
;}
;}
,getContentHeight:function(I){var D=qx.bom.element.Style;var F=qx.bom.element.Style.get(I,i);var G=parseInt(D.get(I,j)||d,10);var E=parseInt(D.get(I,m)||d,10);if(this.__hiddenScrollbars[F]){return I.clientHeight-G-E;}
else {if(I.clientHeight>=I.scrollHeight){return Math.max(I.clientHeight,I.scrollHeight)-G-E;}
else {var H=I.scrollHeight-G;if(qx.core.Environment.get(b)==a&&qx.core.Environment.get(f)==6){H-=E;}
;return H;}
;}
;}
}});}
...
;var w=function(D,event){var F=qx.module.Event.__normalizations;var E=F[c]||[];if(F[B]){E=E.concat(F[B]);}
...
;}
;return this;}
,off:function(Q,I,N,H){var M=(I===null&&N===null);for(var j=0;j<this.length;j++ ){var G=this[j];if(!G.__listener){continue;}
;var S=[];if(Q!==null){S.push(Q);}
else {for(var K in G.__listener){S.push(K);}
;}
;for(var i=0,l=S.length;i<l;i++ ){for(var L in G.__listener[S[i]]){var P=G.__listener[S[i]][L];if(M||P==I||P.original==I){var J=typeof G.__ctx!==n&&G.__ctx[L];var T;if(!N&&J){T=G.__ctx[L];}
;G.$$emitter.off(S[i],P,T||N);if(M||P.original==I){qx.bom.Event.removeNativeListener(G,S[i],P,H);}
;delete G.__listener[S[i]][L];if(J){delete G.__ctx[L];}
;}
;}
;var O=qx.module.Event.__hooks.off;var R=O[c]||[];if(O[Q]){R=R.concat(O[Q]);}
...
;}
;}
,__isReady:false,ready:function(br){if(document.readyState===q){window.setTimeout(br,1);return;}
;var bs=function(){qx.module.Event.__isReady=true;br();}
...
else {var bu=function(){if(qx.module.Event.__isReady){return;}
...
;var bz=qx.module.Event.__normalizations;for(var i=0,l=bB.length;i<l;i++ ){var bA=bB[i];if(qx.lang.Type.isFunction(by)){if(!bz[bA]){bz[bA]=[];}
;bz[bA].push(by);}
;}
;}
,$unregisterNormalization:function(bF,bC){if(!qx.lang.Type.isArray(bF)){bF=[bF];}
;var bD=qx.module.Event.__normalizations;for(var i=0,l=bF.length;i<l;i++ ){var bE=bF[i];if(bD[bE]){qx.lang.Array.remove(bD[bE],bC);}
;}
;}
...
;var bJ=qx.module.Event.__hooks.on;for(var i=0,l=bL.length;i<l;i++ ){var bK=bL[i];if(qx.lang.Type.isFunction(bI)){if(!bJ[bK]){bJ[bK]=[];}
;bJ[bK].push(bI);}
;}
;if(!bH){return;}
;var bG=qx.module.Event.__hooks.off;for(var i=0,l=bL.length;i<l;i++ ){var bK=bL[i];if(qx.lang.Type.isFunction(bH)){if(!bG[bK]){bG[bK]=[];}
;bG[bK].push(bH);}
;}
;}
,$unregisterEventHook:function(bR,bO,bN){if(!qx.lang.Type.isArray(bR)){bR=[bR];}
;var bP=qx.module.Event.__hooks.on;for(var i=0,l=bR.length;i<l;i++ ){var bQ=bR[i];if(bP[bQ]){qx.lang.Array.remove(bP[bQ],bO);}
;}
;if(!bN){return;}
;var bM=qx.module.Event.__hooks.off;for(var i=0,l=bR.length;i<l;i++ ){var bQ=bR[i];if(bM[bQ]){qx.lang.Array.remove(bM[bQ],bN);}
;}
;}
...
,emit:function(name,z){var w=this.__getStorage(name).concat();var x=[];for(var i=0;i<w.length;i++ ){var y=w[i];y.listener.call(y.ctx,z);if(y.once){x.push(y);}
;}
;x.forEach(function(A){var B=this.__getStorage(name);var C=B.indexOf(A);B.splice(C,1);}
.bind(this));w=this.__getStorage(b);for(var i=w.length-1;i>=0;i-- ){var y=w[i];y.listener.call(y.ctx,z);}
;}
...
,_initObserver:function(U,V){this.__wrappedListener=qx.lang.Function.listener(U,this);this.__eventNames.forEach(function(W){if(V&&qx.dom.Node.isDocument(this.__defaultTarget)){if(!this.__defaultTarget.$$emitter){this.__defaultTarget.$$emitter=new qx.event.Emitter();}
;this.__defaultTarget.$$emitter.on(W,this.__wrappedListener);}
else {qx.bom.Event.addNativeListener(this.__defaultTarget,W,this.__wrappedListener);}
;}
.bind(this));}
...
;if(!this.__nativePointerEvents){ba.stopPropagation();}
...
,_onTouchEvent:function(be){var bf=qx.event.handler.PointerCore.TOUCH_TO_POINTER_MAPPING[be.type];var bg=be.changedTouches;be.stopPropagation();if(be.type==n&&this.__primaryIdentifier===null){this.__primaryIdentifier=bg[0].identifier;}
...
;if(bh.identifier==this.__primaryIdentifier){bd.isPrimary=true;bd.button=0;bd.buttons=1;qx.event.handler.PointerCore.__lastTouch={"x":bh.clientX,"y":bh.clientY,"time":new Date().getTime()};}
;var bi=new qx.event.type.dom.Pointer(bf,be,bd);this._fireEvent(bi,bf,bd.target);if(be.type==w||be.type==H){var bj=new qx.event.type.dom.Pointer(P,be,bd);this._fireEvent(bj,P,be.target);if(this.__primaryIdentifier==bh.identifier){this.__primaryIdentifier=null;}
;}
;}
;}
,_onMouseEvent:function(bk){qx.bom.Event.stopPropagation(bk);if(this._isSimulatedMouseEvent(bk.clientX,bk.clientY)){return;}
;if(bk.type==b){this.__buttonStates[bk.which]=1;}
else if(bk.type==G){this.__buttonStates[bk.which]=0;}
;var bm=qx.event.handler.PointerCore.MOUSE_TO_POINTER_MAPPING[bk.type];var bl=qx.bom.Event.getTarget(bk);var bn=qx.lang.Array.sum(this.__buttonStates);var bq={pointerType:p,pointerId:1};if(this.__lastButtonState!=bn&&bn!==0&&this.__lastButtonState!==0){var bo=new qx.event.type.dom.Pointer(O,bk,bq);this._fireEvent(bo,O,bl);}
;this.__lastButtonState=bn;if(bk.type==b&&bn>1){return;}
;if(bk.type==G&&bn>0){return;}
;if(bk.type==I){this.__buttonStates[bk.which]=0;return;}
;var bp=new qx.event.type.dom.Pointer(bm,bk,bq);this._fireEvent(bp,bm,bl);}
,_isSimulatedMouseEvent:function(x,y){var bs=qx.event.handler.PointerCore.__lastTouch;if(bs){var bt=new Date().getTime()-bs.time;var br=qx.event.handler.PointerCore.SIM_MOUSE_DISTANCE;var bv=Math.abs(x-qx.event.handler.PointerCore.__lastTouch.x);var bu=Math.abs(y-qx.event.handler.PointerCore.__lastTouch.y);if(bt<qx.event.handler.PointerCore.SIM_MOUSE_DELAY){if(bv<br||bu<br){return true;}
;}
;}
;return false;}
...
;if(qx.core.Environment.get(c)){if(!this.__nativePointerEvents){by.dispatchEvent(bx);}
...
;}
else if(!this._isBelowTapMaxDistance(ba)){var bd=this.__getSwipeGesture(ba,bb);if(bd){ba.swipe=bd;this._fireEvent(ba,a,ba.target||bb);}
;}
