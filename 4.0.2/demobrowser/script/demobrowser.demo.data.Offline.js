...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.Offline","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Offline.7f3dde4cbc8b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="demobrowser.demo.data.Offline",b="html.storage.local",c="model",d="Remove item",e="The added data will be keept on reload. Just add or remove strings ",f="and reload to see the result.",g="Add item",h="initial entry",i="Offline storage (local) not supported.",j="qx-offline-demo",k="item ",l="<b>Offline data store</b><br/>",m="execute";qx.Class.define(a,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);if(!qx.core.Environment.get(b)){var p=new qx.ui.basic.Label(i);this.getRoot().add(p,{left:30,top:30});return;}
;var content=new qx.ui.container.Composite();var t=new qx.ui.layout.Grid();t.setSpacing(10);content.setLayout(t);this.getRoot().add(content,{left:10,top:80});var o=new qx.ui.form.List();content.add(o,{row:0,column:0,rowSpan:2});var s=new qx.data.controller.List(null,o);var w=new qx.data.store.Offline(j);if(w.getModel()==null){var v=qx.data.marshal.Json.createModel([h]);w.setModel(v);}
;w.bind(c,s,c);var q=new qx.ui.form.Button(g);var u=new qx.ui.form.Button(d);var r=new qx.ui.form.TextField();r.setValue(k+w.getModel().length);content.add(r,{row:0,column:1,rowSpan:2});content.add(q,{row:0,column:2});content.add(u,{row:1,column:2});q.setAllowGrowY(false);u.setAllowGrowY(false);t.setRowFlex(1,1);q.addListener(m,function(){w.getModel().push(r.getValue());r.setValue(k+w.getModel().length);}
,this);u.addListener(m,function(){w.getModel().remove(s.getSelection().getItem(0));}
,this);var n=new qx.ui.basic.Label();n.setRich(true);n.setWidth(300);n.setValue(l+e+f);this.getRoot().add(n,{left:10,top:10});}
}});}
)();
...
(function(){var a="qx.data.store.Offline",b="session",c="qx.debug",d="changeModel",f="changeBubble",g="_applyModel";qx.Class.define(a,{extend:qx.core.Object,construct:function(j,h,i){qx.core.Object.call(this);try{if(qx.core.Environment.get(c)){this.assertNotUndefined(j);}
;}
catch(e){this.dispose();throw e;}
;if(h==b){this._storage=qx.bom.Storage.getSession();}
else {this._storage=qx.bom.Storage.getLocal();}
;this._marshaler=new qx.data.marshal.Json(i);this._key=j;this._initializeModel();}
,properties:{model:{nullable:true,event:d,apply:g}},members:{_storage:null,__El:null,_applyModel:function(l,k){if(k){k.removeListenerById(this.__El);k.dispose();this.__El=null;}
;if(l){this.__El=l.addListener(f,this.__Em,this);this.__Em();}
else {this._storage.removeItem(this._key);}
;}
,__Em:function(){var m=qx.util.Serializer.toNativeObject(this.getModel());this._storage.setItem(this._key,m);}
,_initializeModel:function(){this._setModel(this._storage.getItem(this._key));}
,_setModel:function(o){this._marshaler.toClass(o,true);if(this.getModel()){this.getModel().dispose();}
;var n=this._marshaler.toModel(o,true);if(n===undefined){n=null;}
;this.setModel(n);}
...
(function(){var a="html.storage.local",b="html.storage.userdata",c="qx.bom.Storage";qx.Bootstrap.define(c,{statics:{__En:null,getLocal:function(){if(qx.core.Environment.get(a)){return qx.bom.storage.Web.getLocal();}
else if(qx.core.Environment.get(b)){return qx.bom.storage.UserData.getLocal();}
;return qx.bom.storage.Memory.getLocal();}
,getSession:function(){if(qx.core.Environment.get(a)){return qx.bom.storage.Web.getSession();}
else if(qx.core.Environment.get(b)){return qx.bom.storage.UserData.getSession();}
;return qx.bom.storage.Memory.getSession();}
}});}
)();
(function(){var a="local",b="session",c="Storage",d="qx.bom.storage.Web",f="Storage full.";qx.Bootstrap.define(d,{statics:{__Eo:null,__Ep:null,getLocal:function(){if(this.__Eo){return this.__Eo;}
;return this.__Eo=new qx.bom.storage.Web(a);}
,getSession:function(){if(this.__Ep){return this.__Ep;}
;return this.__Ep=new qx.bom.storage.Web(b);}
},construct:function(g){this.__pu=g;}
,members:{__pu:null,getStorage:function(){return window[this.__pu+c];}
,getLength:function(){return this.getStorage(this.__pu).length;}
,setItem:function(h,j){j=qx.lang.Json.stringify(j);try{this.getStorage(this.__pu).setItem(h,j);}
catch(e){throw new Error(f);}
;}
,getItem:function(l){var k=this.getStorage(this.__pu).getItem(l);if(qx.lang.Type.isString(k)){k=qx.lang.Json.parse(k);}
else if(k&&k.value&&qx.lang.Type.isString(k.value)){k=qx.lang.Json.parse(k.value);}
;return k;}
,removeItem:function(m){this.getStorage(this.__pu).removeItem(m);}
,clear:function(){var n=this.getStorage(this.__pu);if(!n.clear){for(var i=n.length-1;i>=0;i-- ){n.removeItem(n.key(i));}
;}
else {n.clear();}
;}
,getKey:function(o){return this.getStorage(this.__pu).key(o);}
,forEach:function(p,r){var length=this.getLength();for(var i=0;i<length;i++ ){var q=this.getKey(i);p.call(r,q,this.getItem(q));}
;}
}});}
)();
(function(){var a="session",b="head",c="none",d="qx",e="qx.bom.storage.UserData",f="div",g="local",h="qxtest",j="#default#userdata",k="display";qx.Bootstrap.define(e,{statics:{__Eo:null,__Ep:null,__cj:0,getLocal:function(){if(this.__Eo){return this.__Eo;}
;return this.__Eo=new qx.bom.storage.UserData(g);}
,getSession:function(){if(this.__Ep){return this.__Ep;}
;return this.__Ep=new qx.bom.storage.UserData(a);}
},construct:function(m){this.__Eq=document.createElement(f);this.__Eq.style[k]=c;document.getElementsByTagName(b)[0].appendChild(this.__Eq);this.__Eq.addBehavior(j);this.__Er=m;this.__Eq.load(this.__Er);this.__cV={};this.__Es={};var l=this.__Eq.getAttribute(d+qx.bom.storage.UserData.__cj);while(l!=undefined){l=qx.lang.Json.parse(l);this.__cV[l.key]=l.value;this.__Es[l.key]=d+qx.bom.storage.UserData.__cj;qx.bom.storage.UserData.__cj++ ;l=this.__Eq.getAttribute(d+qx.bom.storage.UserData.__cj);}
;}
,members:{__Eq:null,__Er:h,__cV:null,__Es:null,getStorage:function(){return this.__cV;}
,getLength:function(){return Object.keys(this.__cV).length;}
,setItem:function(n,p){if(this.__Es[n]){var o=this.__Es[n];}
else {var o=d+qx.bom.storage.UserData.__cj;qx.bom.storage.UserData.__cj++ ;}
;var q=qx.lang.Json.stringify({key:n,value:p});this.__Eq.setAttribute(o,q);this.__Eq.save(this.__Er);this.__cV[n]=p;this.__Es[n]=o;}
,getItem:function(r){return this.__cV[r]||null;}
,removeItem:function(v){var u=this.__Es[v];if(u==undefined){return;}
;this.__Eq.removeAttribute(u);qx.bom.storage.UserData.__cj-- ;delete this.__cV[v];delete this.__Es[v];var t=d+qx.bom.storage.UserData.__cj;if(this.__Eq.getAttribute(t)){var w=this.__Eq.getAttribute(d+qx.bom.storage.UserData.__cj);this.__Eq.removeAttribute(t);this.__Eq.setAttribute(u,w);var s=qx.lang.Json.parse(w).key;this.__Es[s]=u;}
;this.__Eq.save(this.__Er);}
,clear:function(){for(var x in this.__Es){this.__Eq.removeAttribute(this.__Es[x]);}
;this.__Eq.save(this.__Er);this.__cV={};this.__Es={};}
,getKey:function(y){return Object.keys(this.__cV)[y];}
,forEach:function(z,B){var length=this.getLength();for(var i=0;i<length;i++ ){var A=this.getKey(i);z.call(B,A,this.getItem(A));}
;}
}});}
)();
(function(){var a="qx.bom.storage.Memory";qx.Bootstrap.define(a,{statics:{__Eo:null,__Ep:null,getLocal:function(){if(this.__Eo){return this.__Eo;}
;return this.__Eo=new qx.bom.storage.Memory();}
,getSession:function(){if(this.__Ep){return this.__Ep;}
;return this.__Ep=new qx.bom.storage.Memory();}
},construct:function(){this.__cV={};}
,members:{__cV:null,getStorage:function(){return this.__cV;}
,getLength:function(){return Object.keys(this.__cV).length;}
,setItem:function(b,c){c=qx.lang.Json.stringify(c);this.__cV[b]=c;}
,getItem:function(e){var d=this.__cV[e];if(qx.lang.Type.isString(d)){d=qx.lang.Json.parse(d);}
;return d;}
,removeItem:function(f){delete this.__cV[f];}
,clear:function(){this.__cV={};}
,getKey:function(h){var g=Object.keys(this.__cV);return g[h];}
,forEach:function(j,l){var length=this.getLength();for(var i=0;i<length;i++ ){var k=this.getKey(i);j.call(l,k,this.getItem(k));}
;}
}});}
)();
