...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.layout.Dock_AutoSize","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Dock_AutoSize.277114b5011f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,properties:{sort:{check:[m,o,A],init:m,apply:r},separatorX:{check:p,nullable:true,apply:h},separatorY:{check:p,nullable:true,apply:h},connectSeparators:{check:a,init:false,apply:h},spacingX:{check:f,init:0,apply:h},spacingY:{check:f,init:0,apply:h}},members:{__fW:null,__HT:null,verifyLayoutProperty:qx.core.Environment.select(j,{"true":function(F,name,G){this.assertInArray(name,[y,w,c,k],g+name+l);if(name===w){this.assertInArray(G,[s,x,d,v,n]);}
...
,__HU:{north:1,south:2,west:3,east:4,center:5},__HV:{'1':t,'2':e,'3':z,'4':u},__nz:function(){var H=this._getLayoutChildren();var P,J;var length=H.length;var L=[];var O=[];var K=[];var I=this.getSort()===o;var N=this.getSort()===A;for(var i=0;i<length;i++ ){P=H[i];K=P.getLayoutProperties().edge;if(K===n){if(J){throw new Error(b);}
;J=P;}
...
;this.__fW=Q;var M=[];for(var i=0;i<length;i++ ){K=Q[i].getLayoutProperties().edge;M[i]=this.__HU[K]||5;}
;this.__HT=M;delete this._invalidChildrenCache;}
,renderLayout:function(bl,bb,bt){if(this._invalidChildrenCache){this.__nz();}
;var bk=qx.ui.layout.Util;var W=this.__fW;var bm=this.__HT;var length=W.length;var ba,bu,Y,bf,bg,br,bd,T,S;var bx=[];var bc=[];var bs=this._getSeparatorWidths();var bB=this.getSpacingX();var bA=this.getSpacingY();var X=-bB;var bn=-bA;if(bs.x){X-=bs.x+bB;}
;if(bs.y){bn-=bs.y+bA;}
...
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__nz();}
;var bS=this.__fW;var cd=this.__HT;var length=bS.length;var bW,ce;var bY,bX;var ca=0,cl=0;var bP=0,cg=0;var cb=0,ck=0;var bQ=0,cf=0;var ch=this._getSeparatorWidths();var cj=this.getSpacingX(),ci=this.getSpacingY();var bU=-cj,bT=-ci;if(ch.x){bU-=ch.x+cj;}
;if(ch.y){bT-=ch.y+ci;}
...
},destruct:function(){this.__HT=this.__fW=null;}
});}
)();
...
(function(){var a="east",b="yellow",c="center",d="red",e="north",f="south",g="west",h="x",i="green",j="blue",k="fuchsia",l="orange",m="main",n="y",o="demobrowser.demo.layout.Dock_AutoSize";qx.Class.define(o,{extend:demobrowser.demo.util.LayoutApplication,members:{main:function(){demobrowser.demo.util.LayoutApplication.prototype.main.call(this);var scroll=new qx.ui.container.Scroll();this.getRoot().add(scroll,{edge:0});var q=new qx.ui.container.Composite(new qx.ui.layout.VBox(20)).set({padding:20});scroll.add(q);var p=new qx.ui.container.Composite(new qx.ui.layout.Dock(3,10)).set({decorator:m,backgroundColor:b,allowGrowX:false});var w=new qx.ui.core.Widget().set({decorator:m,backgroundColor:d});var v=new qx.ui.core.Widget().set({decorator:m,backgroundColor:j});var u=new qx.ui.core.Widget().set({decorator:m,backgroundColor:l});var t=new qx.ui.core.Widget().set({decorator:m,backgroundColor:i});var s=new qx.ui.core.Widget().set({decorator:m,backgroundColor:k});p.add(w,{edge:e});p.add(v,{edge:g});p.add(u,{edge:f});p.add(t,{edge:a});p.add(s,{edge:c});q.add(p);var r=new qx.ui.layout.Dock(3,10);r.setSort(n);var p=(new qx.ui.container.Composite(r)).set({decorator:m,backgroundColor:b,allowGrowX:false});var w=new qx.ui.core.Widget().set({decorator:m,backgroundColor:d});var v=new qx.ui.core.Widget().set({decorator:m,backgroundColor:j});var u=new qx.ui.core.Widget().set({decorator:m,backgroundColor:l});var t=new qx.ui.core.Widget().set({decorator:m,backgroundColor:i});var s=new qx.ui.core.Widget().set({decorator:m,backgroundColor:k});p.add(w,{edge:e});p.add(v,{edge:g});p.add(u,{edge:f});p.add(t,{edge:a});p.add(s,{edge:c});q.add(p);var r=new qx.ui.layout.Dock(3,10);r.setSort(h);var p=(new qx.ui.container.Composite(r)).set({decorator:m,backgroundColor:b,allowGrowX:false});var w=new qx.ui.core.Widget().set({decorator:m,backgroundColor:d});var v=new qx.ui.core.Widget().set({decorator:m,backgroundColor:j});var u=new qx.ui.core.Widget().set({decorator:m,backgroundColor:l});var t=new qx.ui.core.Widget().set({decorator:m,backgroundColor:i});var s=new qx.ui.core.Widget().set({decorator:m,backgroundColor:k});p.add(w,{edge:e});p.add(v,{edge:g});p.add(u,{edge:f});p.add(t,{edge:a});p.add(s,{edge:c});q.add(p);}
}});}
)();
