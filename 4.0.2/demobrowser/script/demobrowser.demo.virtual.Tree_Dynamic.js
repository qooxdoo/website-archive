...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Dynamic.169b983bc545.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,getLevel:function(bo){return this.__CY[bo];}
...
,_applyModel:function(bM,bL){this.__CX=[];if(bM!=null){if(qx.core.Environment.get(w)){if(!qx.Class.hasMixin(bM.constructor,qx.data.marshal.MEventBubbling)){this.warn(H+h+k);}
;}
;bM.addListener(J,this._onChangeBubble,this);this.__Dd(bM);}
...
;}
;if(this.__xd.indexOf(bR)!=-1){this.__Db();}
;}
;}
...
;break;};}
;}
,_beforeApplySelection:function(cc){if(cc.length===0&&this.getSelectionMode()===g){var cb=this.__Dg();var cd=this.getLookupTable().indexOf(cb);if(cd>=0){cc.push(cd);}
;}
;}
,_afterApplySelection:function(){var ce=this.getSelection();if(ce.getLength()>0&&this.getSelectionMode()===g){this.__Df(ce.getItem(0));}
else {this.__Da=[];}
;}
,__Db:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(Q+f);}
;this._itemWidth=0;var ci=[];this.__CY=[];var cg=-1;var ch=this.getModel();if(ch!=null){if(!this.isHideRoot()){cg++ ;ci.push(ch);this.__CY.push(cg);}
;if(this.isNodeOpen(ch)){var cf=this.__Dc(ch,cg);ci=ci.concat(cf);}
;}
...
;}
,__Dc:function(cj,co){var cn=[];co++ ;if(!this.isNode(cj)){return cn;}
...
;if(this.isNode(cq)){this.__CY.push(co);cn.push(cq);if(this.isNodeOpen(cq)){var ck=this.__Dc(cq,co);cn=cn.concat(ck);}
;}
else {if(this.isShowLeafs()){this.__CY.push(co);cn.push(cq);}
;}
;}
;cr.dispose();return cn;}
,__Dd:function(cs){if(!qx.lang.Array.contains(this.__CX,cs)){this.__CX.push(cs);this.fireDataEvent(t,cs);}
;}
,__De:function(ct,cu){if(ct===cu){this.__Dd(cu);return true;}
;if(!this.isNode(ct)){return false;}
;var cx=ct.get(this.getChildProperty());if(cx==null){return false;}
;for(var i=0;i<cx.getLength();i++ ){var cw=cx.getItem(i);var cv=this.__De(cw,cu);if(cv===true){this.__Dd(cw);return true;}
;}
;return false;}
...
;var cB=this.__CY[cA];while(cA>0){cA-- ;var cy=this.__CY[cA];if(cy<cB){return this.__xd.getItem(cA);}
;}
;return null;}
,__Df:function(cC){this.__Da=[];var parent=this.getParent(cC);while(parent!=null){this.__Da.unshift(parent);parent=this.getParent(parent);}
;}
,__Dg:function(){if(this.__Da==null){return this.getModel();}
;var cD=this.getLookupTable();var parent=this.__Da.pop();while(parent!=null){if(cD.contains(parent)){return parent;}
;parent=this.__Da.pop();}
...
;this._layer.removeListener(R,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__xd.dispose();this._layer=this._provider=this.__xd=this.__CX=this.__gi=null;}
});}
)();
...
;var o=this._renderer.getCellWidget();o.setOpen(p&&this._tree.isNodeOpen(s));o.addListener(a,this.__Dh,this);o.setUserData(b,this.getChildProperty());o.setUserData(g,this._tree.isShowLeafs());if(this._tree.getSelection().contains(s)){this._styleSelectabled(o);}
...
,poolCellWidget:function(u){u.removeListener(a,this.__Dh,this);this._removeBindingsFrom(u);this._renderer.pool(u);this._onPool(u);}
...
,__Dh:function(event){var K=event.getTarget();var L=K.getUserData(k);var J=this._tree.getLookupTable().getItem(L);if(event.getData()){this._tree.openNode(J);}
