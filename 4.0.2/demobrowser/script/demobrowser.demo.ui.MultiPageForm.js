...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.MultiPageForm.4964869709fa.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a=" array contains ",b="qx.ui.core.MSingleSelectionHandling",c=" items!",d="changeSelection",f="__fz",g="changeSelected",h="Could only select one item, but the selection",i="qx.event.type.Data";qx.Mixin.define(b,{events:{"changeSelection":i},members:{__fz:null,getSelection:function(){var j=this.__lH().getSelected();if(j){return [j];}
else {return [];}
;}
,setSelection:function(k){switch(k.length){case 0:this.resetSelection();break;case 1:this.__lH().setSelected(k[0]);break;default:throw new Error(h+a+k.length+c);};}
...
,__lH:function(){if(this.__fz==null){var p=this;this.__fz=new qx.ui.core.SingleSelectionManager({getItems:function(){return p._getItems();}
,isItemSelectable:function(q){if(p._isItemSelectable){return p._isItemSelectable(q);}
else {return q.isVisible();}
;}
});this.__fz.addListener(g,this._onChangeSelected,this);}
;this.__fz.setAllowEmptySelection(this._isAllowEmptySelection());return this.__fz;}
...
,__lG:function(e){var m=e.getOldData()[0];var n=e.getData()[0];if(m){if(this.isDynamic()){m.exclude();}
else {m.hide();}
;}
;if(n){n.show();}
;}
...
,_onHtmlInput:function(e){var cF=e.getData();var cE=true;this.__pr=false;if(this.__hS&&this.__hS===cF){cE=false;}
...
;}
;if(cE){this.fireDataEvent(q,cF,this.__hS);this.__hS=cF;if(this.getLiveUpdate()){this.__pt(cF);}
;}
;}
...
;}
,__pt:function(cL){var cK=this.__hR;this.__hR=cL;if(cK!=cL){this.fireNonBubblingEvent(bR,qx.event.type.Data,[cL,cK]);}
;}
,setValue:function(cP){if(cP===null){if(this.__pr){return cP;}
;cP=P;this.__pr=true;}
else {this.__pr=false;if(this.__pq){this._removePlaceholder();}
;}
;if(qx.lang.Type.isString(cP)){var cO=this.getContentElement();if(cO.getValue()!=cP){var cM=cO.getValue();cO.setValue(cP);var cN=this.__pr?null:cP;this.__hR=cM;this.__pt(cN);this.__hS=this.__hR;}
...
,events:{"changeValue":l},properties:{dateFormat:{check:c,apply:s},placeholder:{check:r,nullable:true,apply:E},appearance:{refine:true,init:D},focusable:{refine:true,init:true},width:{refine:true,init:120}},statics:{__wl:null,__wm:null,getDefaultDateFormatter:function(){var K=qx.locale.Date.getDateFormat(y).toString();if(K==this.__wl){return this.__wm;}
;if(this.__wm){this.__wm.dispose();}
;this.__wm=new qx.util.format.DateFormat(K,qx.locale.Manager.getInstance().getLocale());this.__wl=K;return this.__wm;}
},members:{__wn:null,_forwardStates:{focused:true,invalid:true},_setDefaultDateFormat:function(){this.setDateFormat(qx.ui.form.DateField.getDefaultDateFormatter());}
,_addLocaleChangeListener:function(){if(qx.core.Environment.get(x)){this.__wn=qx.locale.Manager.getInstance().addListener(o,function(){this._setDefaultDateFormat();}
,this);}
;}
...
},destruct:function(){if(qx.core.Environment.get(x)){if(this.__wn){qx.locale.Manager.getInstance().removeListenerById(this.__wn);}
;}
;}
});}
)();
(function(){var a='format',b="short",c="HH:mm",d='_',e="stand-alone",f="HHmmsszz",g="cldr_date_format_",h="HHmm",j="narrow",k="HHmmss",l="qx.debug",m="This case should never happen.",n="cldr_time_format_",o="sat",p="abbreviated",q="wide",r="HH:mm:ss zz",s="full",t="cldr_pm",u="cldr_day_",v="_",w="cldr_month_",x="long",y="wed",z="medium",A="fri",B="cldr_am",C="tue",D="qx.locale.Date",E="mon",F="cldr_date_time_format_",G="sun",H="format",I="thu",J="HH:mm:ss",K='stand-alone';qx.Class.define(D,{statics:{__qN:qx.locale.Manager.getInstance(),getAmMarker:function(L){return this.__qN.localize(B,[],L);}
,getPmMarker:function(M){return this.__qN.localize(t,[],M);}
...
;var Q=[G,E,C,y,I,A,o];var R=[];for(var i=0;i<Q.length;i++ ){var N=u+S+v+length+v+Q[i];R.push(P?this.__qO(S,S===a?K:a,N,O):this.__qN.localize(N,[],O));}
;return R;}
...
;var W=[G,E,C,y,I,A,o];var T=u+U+v+length+v+W[Y];return X?this.__qO(U,U===a?K:a,T,V):this.__qN.localize(T,[],V);}
...
;var bd=[];for(var i=0;i<12;i++ ){var ba=w+be+v+length+v+(i+1);bd.push(bc?this.__qO(be,be===a?K:a,ba,bb):this.__qN.localize(ba,[],bb));}
;return bd;}
...
;var bf=w+bg+v+length+v+(bi+1);return bj?this.__qO(bg,bg===a?K:a,bf,bh):this.__qN.localize(bf,[],bh);}
,getDateFormat:function(bm,bk){if(qx.core.Environment.get(l)){qx.core.Assert.assertInArray(bm,[b,z,x,s]);}
;var bl=g+bm;return this.__qN.localize(bl,[],bk);}
,getDateTimeFormat:function(br,bp,bn){var bq=F+br;var bo=this.__qN.localize(bq,[],bn);if(bo==bq){bo=bp;}
;return bo;}
,getTimeFormat:function(bv,bs){if(qx.core.Environment.get(l)){qx.core.Assert.assertInArray(bv,[b,z,x,s]);}
;var bu=n+bv;var bt=this.__qN.localize(bu,[],bs);if(bt!=bu){return bt;}
...
else {bK=this.__qN.getTerritory()||this.__qN.getLanguage();}
;return bK.toUpperCase();}
,__qO:function(bN,bM,bQ,bP){var bO=this.__qN.localize(bQ,[],bP);if(bO==bQ){var bL=bQ.replace(d+bN+d,d+bM+d);return this.__qN.localize(bL,[],bP);}
else {return bO;}
;}
}});}
)();
...
(function(){var a="-",b="QQQ",c='W',d="qqqq",e="h",f="KK",g='L',h=" is not supported",m="Z",n="Date string '",o="GGGGG",p='abbreviated',q="Y+",r=". Wildcard ",s="eeeee",t="(\\d\\d?)",u="LL",v="abbreviated",w="EEEE",x="^",y=":",z='y',A="quoted_literal",B="weekOfYear",C='3rd quarter',D="SSS",E='a',F="day",G="(-*",H="qqq",I="(\\d\\d)",J="locale",K="K",L='q',M="Provided date is invalid",N="pm",O="Error parsing date '",P='Anno Domini',Q='G',R="a",S='2nd quarter',T="yyyy-MM-dd'T'HH:mm:ss",U="GMT",V="HH:mm:ss",W="dd",X="+",Y="qx.util.format.DateFormat",eo='weekDay',ek="am",ep="eee",el="quarter",em="ee",ej="q",en="QQQQ",eu="HHmmss",ev="ww",ew="H",ex='Y+',eq="wide",er='wide',es="(\\d\\d\\d?)",et="(GMT[\\+\\-]\\d\\d:\\d\\d)",eB="_applyLocale",ff="Q",eC="c",eD="(",ey="weekOfMonth",ez="ccc",gg="long",eA="Malformed date format: ",eE="EEEEE",eF='Y',eG="EE",eL="GGG",eM='-',eN="w",eH="mm",eI='h',eJ="S",eK='s',eS='A',eT="yyyy-MM-dd",eU="G",eV='z',eO="GG",eP=")",eQ="kk",eR="ss",fa="([\\+\\-]\\d\\d\\d\\d)",fb="default",gk='BC',fc='H',eW='S',eX='1st quarter',gi="stand-alone",eY="0",fd="MMMM",fe='c',fq="d",fp='Q',fo='Q1',fu="eeee",ft="QQ",fs="LLLLL",fr="HH",fj='k',fi="m",fh="DDD",fg="D",fn='D',fm="LLLL",fl="(\\d\\d*?)",fk='e',fB="': the value for day or month is too large",fA="literal",fz='K',fy="L",fF="hh",fE="ms",fD="(\\d\\d?\\d?)",fC="cccc",fx="e",fw='y+',fv="|",fQ="W",fP="dayOfYear",fO="GGGG",fU='Q2',fT='B',fS="MM",fR='Q4',fJ="y+",fI='narrow',fH="ccccc",fG="E",fN="wildcard",fM='',fL="' does not match the date format: ",fK="short",gb='d',ga="unkown",fY="narrow",fX="\\d?",gf="'",ge="k",gd='Before Christ',gc='Z',fW="qx.debug",fV='m',dR=" ",dQ="yyyy-MM-dd'T'HH:mm:ss'Z'",gl="String",dO='Q3',dP="z",dN="DD",gj="min",dL="",dM='M',dK="sec",gh='isoUtcDateTime',dI='4th quarter',dJ="(\\d?)",dH="SS",eb="MMM",ec="s",dY="M",ea='w',dW="EEE",dX="$",dV="?",dG='AD',dT="\\d",dU="+?",dS="hour",ei="MMMMM",eg='E',eh='month',ee="LLL",ef="qq",ed="format";qx.Class.define(Y,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(gn,gm){qx.core.Object.call(this);if(!gm){this.__jj=qx.locale.Manager.getInstance().getLocale();this.__qt=qx.locale.Manager.getInstance().bind(J,this,J);}
else {this.__jj=gm;this.setLocale(gm);}
;this.__qu=this.__jj;if(gn!=null){this.__qv=gn.toString();if(this.__qv in qx.util.format.DateFormat.ISO_MASKS){if(this.__qv===gh){this.__qw=true;}
;this.__qv=qx.util.format.DateFormat.ISO_MASKS[this.__qv];}
;}
else {this.__qv=qx.locale.Date.getDateFormat(gg,this.__jj)+dR+qx.locale.Date.getDateTimeFormat(eu,V,this.__jj);}
;}
,properties:{locale:{apply:eB,nullable:true,check:gl}},statics:{getDateInstance:function(){var go=qx.util.format.DateFormat;var gp=qx.locale.Date.getDateFormat(fK)+dL;if(go._dateInstance==null||go._dateInstance.__qv!=gp){go._dateInstance=new go(gp);}
;return go._dateInstance;}
,getDateTimeInstance:function(){var gq=qx.util.format.DateFormat;var gr=qx.locale.Date.getDateFormat(gg)+dR+qx.locale.Date.getDateTimeFormat(eu,V);if(gq._dateTimeInstance==null||gq._dateTimeInstance.__qv!=gr){gq._dateTimeInstance=new gq(gr);}
;return gq._dateTimeInstance;}
,ASSUME_YEAR_2000_THRESHOLD:30,ISO_MASKS:{isoDate:eT,isoTime:V,isoDateTime:T,isoUtcDateTime:dQ},AM_MARKER:ek,PM_MARKER:N},members:{__qt:null,__jj:null,__qu:null,__qv:null,__qx:null,__qy:null,__qz:null,__qw:null,__qA:function(gt,gs){var gu=dL+(gt<0?((-1)*gt):gt);while(gu.length<gs){gu=eY+gu;}
;return gt<0?a+gu:gu;}
,__qB:function(gv){var gw=new Date(gv.getTime());var gx=gw.getDate();while(gw.getMonth()!=0){gw.setDate(-1);gx+=gw.getDate()+1;}
;return gx;}
,__qC:function(gy){return new Date(gy.getTime()+(3-((gy.getDay()+6)%7))*86400000);}
,__qD:function(gz){var gB=this.__qC(gz);var gC=gB.getFullYear();var gA=this.__qC(new Date(gC,0,4));return Math.floor(1.5+(gB.getTime()-gA.getTime())/86400000/7);}
,__qE:function(gD){var gF=this.__qC(gD);var gE=this.__qC(new Date(gD.getFullYear(),gD.getMonth(),4));return Math.floor(1.5+(gF.getTime()-gE.getTime())/86400000/7);}
,__qF:function(gG){var gH=this.__qC(gG);return gH.getFullYear();}
,__qG:function(gJ){var gI=new Date(gJ,2,1);gI.setDate(-1);return gI.getDate()+1===29;}
,__qH:function(gO,gP){var gN=0;var gQ=0;if(!gP){gP=1971;}
...
,__qI:function(gU,gS,gR){var gT;switch(gS){case 11:gT=gU-1;if(gU!=this.__qF(new Date(gT,gS,gR))){gT=gU;}
;break;case 0:gT=gU+1;if(gU!=this.__qF(new Date(gT,gS,gR))){gT=gU;}
;break;default:gT=gU;};return gT;}
,_applyLocale:function(gW,gV){this.__jj=gW===null?this.setLocale(this.__qu):gW;}
,format:function(hn){if(hn==null){return null;}
;if(isNaN(hn.getTime())){if(qx.core.Environment.get(fW)){qx.log.Logger.error(M);}
;return null;}
;if(this.__qw){hn=new Date(hn.getUTCFullYear(),hn.getUTCMonth(),hn.getUTCDate(),hn.getUTCHours(),hn.getUTCMinutes(),hn.getUTCSeconds(),hn.getUTCMilliseconds());}
;var ha=this.__jj;var hf=hn.getFullYear();var hb=hn.getMonth();var he=hn.getDate();var hi=hn.getDay();var hl=hn.getHours();var hs=hn.getMinutes();var hj=hn.getSeconds();var hq=hn.getMilliseconds();var hh=hn.getTimezoneOffset();var hm=hh>0?1:-1;var gX=Math.floor(Math.abs(hh)/60);var hp=Math.abs(hh)%60;this.__qJ();var hr=dL;for(var i=0;i<this.__qz.length;i++ ){var hg=this.__qz[i];if(hg.type==fA){hr+=hg.text;}
else {var gY=hg.character;var hk=hg.size;var ht=dV;switch(gY){case z:if(hk==2){ht=this.__qA(hf%100,2);}
else {var hc=Math.abs(hf);ht=hc+dL;if(hk>ht.length){for(var j=ht.length;j<hk;j++ ){ht=eY+ht;}
;}
;if(hf<0){ht=a+ht;}
;}
;break;case eF:ht=this.__qF(hn)+dL;var hc=ht.replace(eM,fM);if(hk>ht.length){for(var j=hc.length;j<hk;j++ ){hc=eY+hc;}
;}
;ht=ht.indexOf(a)!=-1?a+hc:hc;break;case Q:if(hk>=1&&hk<=3){ht=hf>0?dG:gk;}
else if(hk==4){ht=hf>0?P:gd;}
else if(hk==5){ht=hf>0?eS:fT;}
;break;case fp:if(hk==1||hk==2){ht=this.__qA(parseInt(hb/4)+1,hk);}
;if(hk==3){ht=fp+(parseInt(hb/4)+1);}
;break;case L:if(hk==1||hk==2){ht=this.__qA(parseInt(hb/4)+1,hk);}
;if(hk==3){ht=fp+(parseInt(hb/4)+1);}
;break;case fn:ht=this.__qA(this.__qB(hn),hk);break;case gb:ht=this.__qA(he,hk);break;case ea:ht=this.__qA(this.__qD(hn),hk);break;case c:ht=this.__qE(hn);break;case eg:if(hk>=1&&hk<=3){ht=qx.locale.Date.getDayName(v,hi,ha,ed,true);}
else if(hk==4){ht=qx.locale.Date.getDayName(eq,hi,ha,ed,true);}
else if(hk==5){ht=qx.locale.Date.getDayName(fY,hi,ha,ed,true);}
;break;case fk:var hd=qx.locale.Date.getWeekStart(ha);var ho=1+((hi-hd>=0)?(hi-hd):7+(hi-hd));if(hk>=1&&hk<=2){ht=this.__qA(ho,hk);}
...
;break;case dM:if(hk==1||hk==2){ht=this.__qA(hb+1,hk);}
...
;break;case g:if(hk==1||hk==2){ht=this.__qA(hb+1,hk);}
...
;break;case E:ht=(hl<12)?qx.locale.Date.getAmMarker(ha):qx.locale.Date.getPmMarker(ha);break;case fc:ht=this.__qA(hl,hk);break;case fj:ht=this.__qA((hl==0)?24:hl,hk);break;case fz:ht=this.__qA(hl%12,hk);break;case eI:ht=this.__qA(((hl%12)==0)?12:(hl%12),hk);break;case fV:ht=this.__qA(hs,hk);break;case eK:ht=this.__qA(hj,hk);break;case eW:ht=this.__qA(hq,3);if(hk<ht.length){ht=ht.substr(0,hk);}
else {while(hk>ht.length){ht+=eY;}
;}
;break;case eV:if(hk>=1&&hk<=4){ht=U+((hm>0)?a:X)+this.__qA(Math.abs(gX),2)+y+this.__qA(hp,2);}
;break;case gc:if(hk>=1&&hk<=3){ht=((hm>0)?a:X)+this.__qA(Math.abs(gX),2)+this.__qA(hp,2);}
else {ht=U+((hm>0)?a:X)+this.__qA(Math.abs(gX),2)+y+this.__qA(hp,2);}
;break;};hr+=ht;}
;}
;return hr;}
,parse:function(hA){this.__qK();var hD=this.__qx.regex.exec(hA);if(hD==null){throw new Error(n+hA+fL+this.__qv);}
;var hu={era:1,year:1970,quarter:1,month:0,day:1,dayOfYear:1,hour:0,ispm:false,weekDay:4,weekYear:1970,weekOfMonth:1,weekOfYear:1,min:0,sec:0,ms:0};var hy=1;var hv=false;var hw=false;for(var i=0;i<this.__qx.usedRules.length;i++ ){var hx=this.__qx.usedRules[i];var hz=hD[hy];if(hx.field!=null){hu[hx.field]=parseInt(hz,10);}
else {hx.manipulator(hu,hz,hx.pattern);}
;if(hx.pattern==q){var hB=false;for(var k=0;k<this.__qx.usedRules.length;k++ ){if(this.__qx.usedRules[k].pattern==fw){hB=true;break;}
;}
;if(!hB){hv=true;}
;}
;if(hx.pattern.indexOf(fg)!=-1){var hF=false;for(var k=0;k<this.__qx.usedRules.length;k++ ){if(this.__qx.usedRules[k].pattern.indexOf(fq)!=-1){hF=true;break;}
;}
;if(!hF){hw=true;}
;}
;hy+=(hx.groups==null)?1:hx.groups;}
;if(hv){hu.year=this.__qI(hu.weekYear,hu.month,hu.day);}
;if(hw){var hE=this.__qH(hu.dayOfYear,hu.year);hu.month=hE.month;hu.day=hE.day;}
;if(hu.era<0&&(hu.year*hu.era<0)){hu.year=hu.year*hu.era;}
;var hC=new Date(hu.year,hu.month,hu.day,(hu.ispm)?(hu.hour+12):hu.hour,hu.min,hu.sec,hu.ms);if(this.__qw){hC=new Date(hC.getUTCFullYear(),hC.getUTCMonth(),hC.getUTCDate(),hC.getUTCHours(),hC.getUTCMinutes(),hC.getUTCSeconds(),hC.getUTCMilliseconds());}
;if(hu.month!=hC.getMonth()||hu.year!=hC.getFullYear()){throw new Error(O+hA+fB);}
;return hC;}
,__qJ:function(){if(this.__qz!=null){return;}
;this.__qz=[];var hK;var hI=0;var hM=dL;var hG=this.__qv;var hJ=fb;var i=0;while(i<hG.length){var hL=hG.charAt(i);switch(hJ){case A:if(hL==gf){if(i+1>=hG.length){i++ ;break;}
;var hH=hG.charAt(i+1);if(hH==gf){hM+=hL;i++ ;}
else {i++ ;hJ=ga;}
;}
else {hM+=hL;i++ ;}
;break;case fN:if(hL==hK){hI++ ;i++ ;}
else {this.__qz.push({type:fN,character:hK,size:hI});hK=null;hI=0;hJ=fb;}
;break;default:if((hL>=E&&hL<=eV)||(hL>=eS&&hL<=gc)){hK=hL;hJ=fN;}
else if(hL==gf){if(i+1>=hG.length){hM+=hL;i++ ;break;}
;var hH=hG.charAt(i+1);if(hH==gf){hM+=hL;i++ ;}
;i++ ;hJ=A;}
else {hJ=fb;}
;if(hJ!=fb){if(hM.length>0){this.__qz.push({type:fA,text:hM});hM=dL;}
;}
else {hM+=hL;i++ ;}
;break;};}
;if(hK!=null){this.__qz.push({type:fN,character:hK,size:hI});}
else if(hM.length>0){this.__qz.push({type:fA,text:hM});}
;}
,__qK:function(){if(this.__qx!=null){return;}
;var hQ=this.__qv;this.__qM();this.__qJ();var hW=[];var hR=x;for(var hO=0;hO<this.__qz.length;hO++ ){var hX=this.__qz[hO];if(hX.type==fA){hR+=qx.lang.String.escapeRegexpChars(hX.text);}
else {var hP=hX.character;var hS=hX.size;var hU;for(var hY=0;hY<this.__qy.length;hY++ ){var hT=this.__qy[hY];if(this.__qL(hT,hP,hS)){hU=hT;break;}
;}
...
;this.__qx={regex:hN,"usedRules":hW,pattern:hR};}
,__qL:function(ib,ic,id){if(ic===z&&ib.pattern===fw){ib.regex=ib.regexFunc(id);return true;}
else if(ic===eF&&ib.pattern===ex){ib.regex=ib.regexFunc(id);return true;}
else {return ic==ib.pattern.charAt(0)&&id==ib.pattern.length;}
;}
,__qM:function(){var it=qx.util.format.DateFormat;var iw=qx.lang.String;if(this.__qy!=null){return;}
;var iu=this.__qy=[];var ip=qx.locale.Date.getAmMarker(this.__jj).toString()||it.AM_MARKER;var iD=qx.locale.Date.getPmMarker(this.__jj).toString()||it.PM_MARKER;var ig=this.__jj;var il=function(iG,iH){iH=parseInt(iH,10);if(iH>=0){if(iH<it.ASSUME_YEAR_2000_THRESHOLD){iH+=2000;}
...
},destruct:function(){if(this.__qt!=null){qx.locale.Manager.getInstance().removeBinding(this.__qt);}
;this.__qz=this.__qx=this.__qy=null;}
});}
)();
...
else {var bL=(bN==null)?-1:bN.getDate();for(var i=0;i<6*7;i++ ){var bO=this.__wp[i];if(bO.hasState(M)){if(bO.hasState(p)){bO.removeState(p);}
;}
...
;this.__wo=this.__wp=this.__wq=null;}
});}
)();
...
(function(){var a="changeModel",b="qx.core.Object",c="qx.data.controller.Object",d="get",e="[",f="reset",g="_applyModel",h="last";qx.Class.define(c,{extend:qx.core.Object,construct:function(j){qx.core.Object.call(this);this.__bb={};this.__gj=[];if(j!=null){this.setModel(j);}
;}
,properties:{model:{check:b,event:a,apply:g,nullable:true,dereference:true}},members:{__gj:null,__bb:null,_applyModel:function(o,l){for(var i=0;i<this.__gj.length;i++ ){var t=this.__gj[i][0];var q=this.__gj[i][1];var n=this.__gj[i][2];var r=this.__gj[i][3];var p=this.__gj[i][4];var m=this.__gj[i][5];if(l!=undefined&&!l.isDisposed()){this.__ws(t,q,n,l);}
;if(o!=undefined){this.__wr(t,q,n,r,p,m);}
...
;}
;}
,addTarget:function(z,w,u,x,y,v){this.__gj.push([z,w,u,x,y,v]);this.__wr(z,w,u,x,y,v);}
,__wr:function(F,D,B,G,E,A){if(this.getModel()==null){return;}
...
,removeTarget:function(K,J,L){this.__ws(K,J,L,this.getModel());for(var i=0;i<this.__gj.length;i++ ){if(this.__gj[i][0]==K&&this.__gj[i][1]==J&&this.__gj[i][2]==L){this.__gj.splice(i,1);}
;}
;}
,__ws:function(R,O,N,M){if(!(R instanceof qx.core.Object)){return;}
...
(function(){var a="Widgets supporting selection can only be validated ",b="Added widget not supported.",c="",d="in the form validator",f="mouse",g="changeValid",h="qx.event.type.Event",j="complete",k="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",l="qx.ui.form.validation.Manager",m="This field is required",n="String",o="qx.event.type.Data";qx.Class.define(l,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__vq=[];this.__vr={};this.setRequiredFieldMessage(qx.locale.Manager.tr(m));}
,events:{"changeValid":o,"complete":h},properties:{validator:{check:k,init:null,nullable:true},invalidMessage:{check:n,init:c},requiredFieldMessage:{check:n,init:c},context:{nullable:true}},members:{__vq:null,__vs:null,__vr:null,__vt:null,add:function(s,p,r){if(!this.__vy(s)){throw new Error(b);}
;if(this.__vz(s)&&!s.getValue){if(p!=null){throw new Error(a+d);}
;}
;var q={item:s,validator:p,valid:null,context:r};this.__vq.push(q);}
,remove:function(v){var t=this.__vq;for(var i=0,u=t.length;i<u;i++ ){if(v===t[i].item){t.splice(i,1);return v;}
;}
;return null;}
,getItems:function(){var w=[];for(var i=0;i<this.__vq.length;i++ ){w.push(this.__vq[i].item);}
;return w;}
,validate:function(){var B=true;this.__vt=true;var y=[];for(var i=0;i<this.__vq.length;i++ ){var z=this.__vq[i].item;var C=this.__vq[i].validator;y.push(z);if(C==null){var x=this.__vu(z);B=B&&x;this.__vt=x&&this.__vt;continue;}
;var x=this.__vv(this.__vq[i],z.getValue());B=x&&B;if(x!=null){this.__vt=x&&this.__vt;}
;}
;var A=this.__vw(y);if(qx.lang.Type.isBoolean(A)){this.__vt=A&&this.__vt;}
;B=A&&B;this.__vA(B);if(qx.lang.Object.isEmpty(this.__vr)){this.fireEvent(j);}
;return B;}
,__vu:function(E){if(E.getRequired()){if(this.__vz(E)){var D=!!E.getSelection()[0];}
...
;return true;}
,__vv:function(K,L){var M=K.item;var O=K.context;var N=K.validator;if(this.__vx(N)){this.__vr[M.toHashCode()]=null;N.validate(M,M.getValue(),this,O);return null;}
...
,__vw:function(Q){var R=this.getValidator();var S=this.getContext()||this;if(R==null){return true;}
;this.setInvalidMessage(c);if(this.__vx(R)){this.__vr[this.toHashCode()]=null;R.validateForm(Q,this,S);return null;}
...
;}
;return T;}
,__vx:function(V){var U=false;if(!qx.lang.Type.isFunction(V)){U=qx.Class.isSubClassOf(V.constructor,qx.ui.form.validation.AsyncValidator);}
;return U;}
,__vy:function(W){var X=W.constructor;return qx.Class.hasInterface(X,qx.ui.form.IForm);}
,__vz:function(Y){var ba=Y.constructor;return qx.Class.hasInterface(ba,qx.ui.core.ISingleSelection);}
,__vA:function(bb){this._showToolTip(bb);var bc=this.__vs;this.__vs=bb;if(bc!=bb){this.fireDataEvent(g,bb,bc);}
;}
,_showToolTip:function(bg){if(!qx.ui.tooltip||!qx.ui.tooltip.Manager){return;}
;var be=qx.ui.tooltip.Manager.getInstance().getSharedErrorTooltip();if(!bg){var bd;for(var i=0;i<this.__vq.length;i++ ){var bf=this.__vq[i].item;if(!bf.isValid()){bd=bf;if(!(bf.getContentLocation)){return;}
...
;}
,getValid:function(){return this.__vs;}
,isValid:function(){return this.getValid();}
,getInvalidMessages:function(){var bh=[];for(var i=0;i<this.__vq.length;i++ ){var bi=this.__vq[i].item;if(!bi.getValid()){bh.push(bi.getInvalidMessage());}
;}
;if(this.getInvalidMessage()!=c){bh.push(this.getInvalidMessage());}
;return bh;}
,getInvalidFormItems:function(){var bj=[];for(var i=0;i<this.__vq.length;i++ ){var bk=this.__vq[i].item;if(!bk.getValid()){bj.push(bk);}
;}
;return bj;}
,reset:function(){for(var i=0;i<this.__vq.length;i++ ){var bl=this.__vq[i];bl.item.setValid(true);}
;this.__vs=null;}
,setItemValid:function(bn,bm){this.__vr[bn.toHashCode()]=bm;bn.setValid(bm);this.__vB();}
,setFormValid:function(bo){this.__vr[this.toHashCode()]=bo;this.__vB();}
,__vB:function(){var bq=this.__vt;for(var br in this.__vr){var bp=this.__vr[br];bq=bp&&bq;if(bp==null){return;}
;}
;this.__vA(bq);this.__vr={};this.fireEvent(j);}
},destruct:function(){this.__vq=null;}
});}
)();
(function(){var a="qx.ui.form.validation.AsyncValidator";qx.Class.define(a,{extend:qx.core.Object,construct:function(b){qx.core.Object.call(this);this.__vC=b;}
,members:{__vC:null,__vD:null,__fz:null,__vE:null,validate:function(c,e,d,f){this.__vE=false;this.__vD=c;this.__fz=d;this.__vC.call(f||this,this,e);}
,validateForm:function(g,h,i){this.__vE=true;this.__fz=h;this.__vC.call(i,g,this);}
,setValid:function(k,j){if(this.__vE){if(j!==undefined){this.__fz.setInvalidMessage(j);}
;this.__fz.setFormValid(k);}
else {if(j!==undefined){this.__vD.setInvalidMessage(j);}
;this.__fz.setItemValid(this.__vD,k);}
;}
},destruct:function(){this.__fz=this.__vD=null;}
});}
)();
