...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.TreeController.4106c0217fbe.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,addSpacer:function(){if(!this.__ns){this.__ns=new qx.ui.core.Spacer();}
else {this._remove(this.__ns);}
;this._add(this.__ns);}
,addOpenButton:function(){this._add(this.getChildControl(m));}
,_onChangeOpen:function(e){if(this.isOpenable()){this.setOpen(e.getData());}
;}
,addIcon:function(){var L=this.getChildControl(t);if(this.__nr){this._remove(L);}
;this._add(L);this.__nr=true;}
,addLabel:function(M){var N=this.getChildControl(D);if(this.__nq){this._remove(N);}
;if(M){this.setLabel(M);}
else {N.setValue(this.getLabel());}
;this._add(N);this.__nq=true;}
,_applyIcon:function(P,O){if(!this.__nu()){this.__nv(P);}
else if(!this.isOpen()){this.__nv(P);}
;}
,_applyIconOpened:function(R,Q){if(this.isOpen()){if(this.__nt()&&this.__nu()){this.__nv(R);}
else if(!this.__nt()&&this.__nu()){this.__nv(R);}
;}
;}
...
;var X;if(W){X=this.__nu()?this.getIconOpened():null;}
else {X=this.getIcon();}
;if(X){this.__nv(X);}
;W?this.addState(u):this.removeState(u);}
,__nt:function(){return qx.util.PropertyUtil.getUserValue(this,t);}
,__nu:function(){return qx.util.PropertyUtil.getUserValue(this,v);}
,__nv:function(Y){var ba=this.getChildControl(t,true);if(ba){ba.setSource(Y);}
;}
...
;}
;if(this.__ns){this.__ns.setWidth((this.getLevel()+1)*this.getIndent()-bf);}
;}
...
(function(){var a="__nA",b="open",c="abstract",d="__fW",e="qx.debug",f="'. It is not a child of this tree item.",g="addItem",h="visible",j="excluded",k="removeItem",m="Invalid child index: ",n="Cannot remove treeitem '",o="qx.ui.tree.core.AbstractTreeItem";qx.Class.define(o,{extend:qx.ui.tree.core.AbstractItem,type:c,construct:function(p){qx.ui.tree.core.AbstractItem.call(this,p);this.__fW=[];}
,properties:{parent:{check:o,nullable:true}},members:{__fW:null,__nA:null,getTree:function(){var r=this;while(r.getParent()){r=r.getParent();}
...
;}
;}
,getChildrenContainer:function(){if(!this.__nA){this.__nA=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?h:j});}
;return this.__nA;}
...
;}
,getChildren:function(){return this.__fW;}
,hasChildren:function(){return this.__fW?this.__fW.length>0:false;}
...
;}
,__nB:function(){if(this.getParentChildrenContainer()){this.getParentChildrenContainer()._addAfter(this.getChildrenContainer(),this);}
;}
...
;this.__fW.push(Q);if(!N){this.__nB();}
;if(P){Q.recursiveAddToWidgetQueue();P.fireNonBubblingEvent(g,qx.event.type.Data,[Q]);}
;}
;if(P){qx.ui.core.queue.Widget.add(this);}
;}
,addAt:function(X,T){if(qx.core.Environment.get(e)){this.assert(T<=this.__fW.length&&T>=0,m+T);}
;if(T==this.__fW.length){this.add(X);return;}
;var W=X.getParent();if(W){W.remove(X);}
;var U=this.getChildrenContainer();X.setParent(this);var V=this.hasChildren();var S=this.__fW[T];U.addBefore(X,S);if(X.hasChildren()){U.addAfter(X.getChildrenContainer(),X);}
;qx.lang.Array.insertAt(this.__fW,X,T);if(!V){this.__nB();}
;if(this.getTree()){X.recursiveAddToWidgetQueue();qx.ui.core.queue.Widget.add(this);}
;}
,addBefore:function(Y,bb){if(qx.core.Environment.get(e)){this.assert(this.__fW.indexOf(bb)>=0);}
;var ba=Y.getParent();if(ba){ba.remove(Y);}
;this.addAt(Y,this.__fW.indexOf(bb));}
,addAfter:function(bd,bc){if(qx.core.Environment.get(e)){this.assert(this.__fW.indexOf(bc)>=0);}
;var be=bd.getParent();if(be){be.remove(bd);}
;this.addAt(bd,this.__fW.indexOf(bc)+1);}
,addAtBegin:function(bf){this.addAt(bf,0);}
,remove:function(bk){for(var i=0,l=arguments.length;i<l;i++ ){var bj=arguments[i];if(this.__fW.indexOf(bj)==-1){this.warn(n+bj+f);return;}
...
;}
;qx.lang.Array.remove(this.__fW,bj);bj.setParent(null);bg.remove(bj);}
;var bh=this.getTree();if(bh){bh.fireNonBubblingEvent(k,qx.event.type.Data,[bj]);}
;qx.ui.core.queue.Widget.add(this);}
,removeAt:function(bl){var bm=this.__fW[bl];if(bm){this.remove(bm);}
;}
,removeAll:function(){var bn=this.__fW.concat();for(var i=this.__fW.length-1;i>=0;i-- ){this.remove(this.__fW[i]);}
;return bn;}
...
(function(){var a="Enter",b="_applyOpenMode",c="__nD",d="excluded",f="Space",g="Left",h="Boolean",j="changeOpenMode",k="_applyRootOpenClose",l="visible",m="changeSelection",n="qx.ui.tree.Tree",o="qx.event.type.Data",p="qx.ui.tree.core.AbstractTreeItem",q="tree",r="dbltap",s="_applyHideRoot",t="changeRoot",u="_applyRoot",v="keypress",w="none",x="pane",y="tap",z="Right";qx.Class.define(n,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IModelSelection,qx.ui.form.IForm],include:[qx.ui.core.MMultiSelectionHandling,qx.ui.core.MContentPadding,qx.ui.form.MModelSelection,qx.ui.form.MForm],construct:function(){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__nD=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({allowShrinkY:false,allowGrowX:true});this.getChildControl(x).add(this.__nD);this.initOpenMode();this.initRootOpenClose();this.addListener(m,this._onChangeSelection,this);this.addListener(v,this._onKeyPress,this);}
,events:{addItem:o,removeItem:o},properties:{openMode:{check:[y,r,w],init:r,apply:b,event:j,themeable:true},root:{check:p,init:null,nullable:true,event:t,apply:u},hideRoot:{check:h,init:false,apply:s},rootOpenClose:{check:h,init:false,apply:k},appearance:{refine:true,init:q},focusable:{refine:true,init:true}},members:{__nD:null,SELECTION_MANAGER:qx.ui.tree.selection.SelectionManager,getChildrenContainer:function(){return this.__nD;}
...
,_getContentPaddingTarget:function(){return this.__nD;}
...
(function(){var a="changeModel",b="_applyDelegate",c="qx.core.Object",d="_applyLabelPath",f="icon",g="change",h="label",k="' twice.",l="_applyTarget",m="changeTarget",n="_applyIconPath",o="Can not reverse bind the same target property '",p="_applyLabelOptions",q="_applyChildPath",r="_applyIconOptions",s="qx.data.controller.Tree",t="String",u="Can not bind the same target property '",v="_applyModel",w="get";qx.Class.define(s,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(y,z,A,x){qx.core.Object.call(this);this.__bb={};this.__yw=[];this.__IY={};if(A!=null){this.setChildPath(A);}
;if(x!=null){this.setLabelPath(x);}
;if(y!=null){this.setModel(y);}
;if(z!=null){this.setTarget(z);}
;}
,properties:{model:{check:c,apply:v,event:a,nullable:true,dereference:true},target:{apply:l,event:m,init:null,nullable:true,dereference:true},childPath:{check:t,apply:q,nullable:true},labelPath:{check:t,apply:d,nullable:true},iconPath:{check:t,apply:n,nullable:true},labelOptions:{apply:p,nullable:true},iconOptions:{apply:r,nullable:true},delegate:{apply:b,init:null,nullable:true}},members:{__IY:null,__bb:null,__yw:null,__Ja:null,_applyDelegate:function(C,B){this._setConfigureItem(C,B);this._setCreateItem(C,B);this._setBindItem(C,B);}
,_applyIconOptions:function(E,D){this.__yH();}
,_applyLabelOptions:function(G,F){this.__yH();}
,_applyTarget:function(I,H){if(H!=undefined){this.__CR(H);}
;if(this.getModel()!=null){this.__Jd();}
;this._addChangeTargetListener(I,H);}
,_applyModel:function(K,J){this.__Jd();}
,_applyChildPath:function(M,L){this.__Ja=L;this.__Jd();this.__Ja=null;}
,_applyIconPath:function(O,N){this.__yH();}
,_applyLabelPath:function(Q,P){this.__Jd();}
,__Jb:function(R){var T=R.getTarget();var S=this.__IY[T.toHashCode()].treeNode;var U=this.__IY[T.toHashCode()].modelNode;this.__Je(S,U);this._updateSelection();}
,__Jc:function(e){var X=e.getData();var bc=e.getOldData();var V=this.__IY[bc.toHashCode()];bc.removeListenerById(V.changeListenerId);delete this.__IY[bc.toHashCode()];V.modelNode.removeListenerById(V.changeChildernListenerId);var bd=V.modelNode;var bb=qx.Class.getPropertyDefinition(V.modelNode.constructor,this.getChildPath());var Y=bb.event;var ba=bd.addListener(Y,this.__Jc,this);var W=V.treeNode;this.__IY[X.toHashCode()]={modelNode:bd,treeNode:W,changeListenerId:V.changeListenerId,changeChildernListenerId:ba};this.__Je(W,bd);this._updateSelection();}
...
;return be;}
,__Jd:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__CR();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.__xy(this.getModel(),bg);this.__Je(bg,this.getModel());this.getTarget().setRoot(bg);}
;}
,__Je:function(bn,br){if(br[w+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bj=br[w+qx.lang.String.firstUp(this.getChildPath())]();if(this.__IY[bj.toHashCode()]==undefined){var bl=bj.addListener(g,this.__Jb,this);var bp=qx.Class.getPropertyDefinition(br.constructor,this.getChildPath());var bm=bp.event;var bq=br.addListener(bm,this.__Jc,this);this.__IY[bj.toHashCode()]={modelNode:br,treeNode:bn,changeListenerId:bl,changeChildernListenerId:bq};}
...
;}
else {var bo=this._createItem();bo.setModel(bj.getItem(i));bn.addAt(bo,i);this.__xy(bj.getItem(i),bo);this.__Je(bo,bj.getItem(i));}
;}
;}
;for(var i=bn.getChildren().length-1;i>=bj.length;i-- ){var bk=bn.getChildren()[i];this.__Jg(bk,bn);}
;}
,__CR:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__Jf(bv);var bu=bv.getModel();if(bu){this.__mE(bu);}
;bv.destroy();this.__IY={};}
;}
,__Jf:function(bw){var bx=bw.getChildren()||[];for(var i=bx.length-1;i>=0;i-- ){if(bx[i].getChildren().length>0){this.__Jf(bx[i]);}
;this.__Jg(bx[i],bw);}
;}
,__Jg:function(bz,bC){var bD=bz.getModel();var by=this.__Ja||this.getChildPath();var bA=w+qx.lang.String.firstUp(by);if(bD[bA]!=undefined){var bE=bD[bA]();var bB=this.__IY[bE.toHashCode()];bE.removeListenerById(bB.changeListenerId);bD.removeListenerById(bB.changeChildernListenerId);this.__Jf(bz);delete this.__IY[bE.toHashCode()];}
;this.__mE(bD);bC.remove(bz);bz.destroy();}
...
;if(!qx.lang.Array.contains(this.__yw,bJ)){this.__yw.push(bJ);}
;}
...
;if(!qx.lang.Array.contains(this.__yw,bM)){this.__yw.push(bM);}
;}
...
;}
,__yH:function(){var bW;for(var ca in this.__bb){bW=ca;break;}
;for(var bX in this.__bb[bW]){var bV=this.__bb[bW][bX].treeNode;var bY=qx.core.ObjectRegistry.fromHashCode(bX);this.__mE(bY);this.__xy(bY,bV);}
;}
,__xy:function(cd,cb){var cc=this.getDelegate();if(cc!=null&&cc.bindItem!=null){cc.bindItem(this,cb,cd);}
else {this.bindDefaultProperties(cb,cd);}
;}
,__mE:function(cg){for(var i=0;i<this.__yw.length;i++ ){var ce=this.__yw[i];var cf=this.__bb[ce][cg.toHashCode()];if(cf!=null){if(cf.id){cg.removeBinding(cf.id);cf.id=null;}
...
;this._startSelectionModification();this.__CR();this.__Jd();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cn,cm){if(cn!=null&&cn.bindItem!=null){if(cm!=null&&cm.bindItem!=null&&cn.bindItem==cm.bindItem){return;}
;this.__Jd();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__bb=this.__IY=this.__yw=null;}
});}
)();
...
(function(){var a="changeNames",b="changeName",c="__OT",d="",e="Validation Error: ",f="changeChild",g="- ",h="qx.data.Array",j="changeColor",k="Marge",l="AFFE",m="is no number (parseFloat says so!).",n="Homer",o="\n",p="demobrowser.demo.data.model.Node",q="String",r="changeName2",s="changeChildren";qx.Class.define(p,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.setChildren(new qx.data.Array());}
,properties:{children:{check:h,event:s},child:{check:p,event:f,nullable:true},names:{check:h,event:a,init:new qx.data.Array(n,k)},name:{check:q,init:l,event:b},name2:{check:q,event:r},number:{init:10,validate:c},color:{event:j,nullable:true}},members:{__OT:function(t){if(!isNaN(parseFloat(t))){if(/^\d+$/.test(t)){return;}
;}
