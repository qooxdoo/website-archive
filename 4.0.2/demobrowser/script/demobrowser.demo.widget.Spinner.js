...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Spinner.98c269363188.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="Disabled",b="Not Editable",c="demobrowser.demo.widget.Spinner",d="0",f="ChangeValue: ",g="Stepped",h="Wrap",i="-3000",j="100",k="30",l="300",m="3000",n="changeValue",o="-30",p="Simple",q="left",r="middle",s="Number format",t="right";qx.Class.define(c,{extend:qx.application.Standalone,members:{addSimpleSpinner:function(v,w){var u=new qx.ui.form.Spinner();v.add(new qx.ui.basic.Label(p),{column:0,row:w});v.add(new qx.ui.basic.Label(d),{column:1,row:w});v.add(u,{column:2,row:w});v.add(new qx.ui.basic.Label(j),{column:3,row:w});u.addListener(n,function(e){this.debug(f+e.getData());}
);}
,addNonEditableSpinner:function(y,z){var x=new qx.ui.form.Spinner(0,50,100);x.setEditable(false);y.add(new qx.ui.basic.Label(b),{column:0,row:z});y.add(new qx.ui.basic.Label(d),{column:1,row:z});y.add(x,{column:2,row:z});y.add(new qx.ui.basic.Label(j),{column:3,row:z});}
,addWrappingSpinner:function(B,C){var A=new qx.ui.form.Spinner(-30,30,30);A.setWrap(true);B.add(new qx.ui.basic.Label(h),{column:0,row:C});B.add(new qx.ui.basic.Label(o),{column:1,row:C});B.add(A,{column:2,row:C});B.add(new qx.ui.basic.Label(k),{column:3,row:C});}
,addSteppedSpinner:function(E,F){var D=new qx.ui.form.Spinner(-3000,0,3000).set({singleStep:25});E.add(new qx.ui.basic.Label(g),{column:0,row:F});E.add(new qx.ui.basic.Label(i),{column:1,row:F});E.add(D,{column:2,row:F});E.add(new qx.ui.basic.Label(m),{column:3,row:F});}
,addDisabledSpinner:function(H,I){var G=new qx.ui.form.Spinner;G.setEnabled(false);H.add(new qx.ui.basic.Label(a),{column:0,row:I});H.add(new qx.ui.basic.Label(d),{column:1,row:I});H.add(G,{column:2,row:I});H.add(new qx.ui.basic.Label(j),{column:3,row:I++ });}
,addFormattedSpinner:function(K,M){var J=new qx.ui.form.Spinner(0,200,300);J.setSingleStep(0.5);J.setWidth(70);var L=new qx.util.format.NumberFormat();L.setMinimumFractionDigits(2);L.setMaximumFractionDigits(2);J.setNumberFormat(L);K.add(new qx.ui.basic.Label(s),{column:0,row:M});K.add(new qx.ui.basic.Label(d),{column:1,row:M});K.add(J,{column:2,row:M});K.add(new qx.ui.basic.Label(l),{column:3,row:M++ });}
,main:function(){qx.application.Standalone.prototype.main.call(this);var N=new qx.ui.layout.Grid(10,8);N.setColumnAlign(0,q,r);N.setColumnAlign(1,t,r);N.setColumnAlign(2,q,r);N.setColumnAlign(3,q,r);var O=new qx.ui.container.Composite(N);O.setPadding(10);this.getRoot().add(O,{left:0,top:0});this.addSimpleSpinner(O,0);this.addNonEditableSpinner(O,1);this.addWrappingSpinner(O,2);this.addSteppedSpinner(O,3);this.addDisabledSpinner(O,4);this.addFormattedSpinner(O,5);}
}});}
)();
...
(function(){var a="-",b="Boolean",c='{0,1}[0-9]{3}){0,}){0,1}',d="^",e="Number string '",f="' does not match the number format",g="Wrong argument type. String is expected.",h="0",i="NaN",j='(',k="-Infinity",l="String",m="",n='([0-9]{1,3}(?:',o="qx.util.format.NumberFormat",p='([-+]){0,1}',q="g",r="Wrong number of arguments.",s="Number",t="$",u='\\d+){0,1}',v=".",w="changeNumberFormat",x="Infinity";qx.Class.define(o,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(y){qx.core.Object.call(this);if(arguments.length>0){if(arguments.length===1){if(qx.lang.Type.isString(y)){this.__jj=y;}
else {throw new Error(g);}
;}
else {throw new Error(r);}
;}
;}
,properties:{minimumIntegerDigits:{check:s,init:0},maximumIntegerDigits:{check:s,nullable:true},minimumFractionDigits:{check:s,init:0},maximumFractionDigits:{check:s,nullable:true},groupingUsed:{check:b,init:true},prefix:{check:l,init:m,event:w},postfix:{check:l,init:m,event:w}},members:{__jj:null,format:function(E){switch(E){case Infinity:return x;case -Infinity:return k;case NaN:return i;};var C=(E<0);if(C){E=-E;}
;if(this.getMaximumFractionDigits()!=null){var K=Math.pow(10,this.getMaximumFractionDigits());E=Math.round(E*K)/K;}
;var I=String(Math.floor(E)).length;var z=m+E;var G=z.substring(0,I);while(G.length<this.getMinimumIntegerDigits()){G=h+G;}
...
;var F=z.substring(I+1);while(F.length<this.getMinimumFractionDigits()){F+=h;}
...
;if(this.getGroupingUsed()){var B=G;G=m;var J;for(J=B.length;J>3;J-=3){G=m+qx.locale.Number.getGroupSeparator(this.__jj)+B.substring(J-3,J)+G;}
;G=B.substring(0,J)+G;}
;var D=this.getPrefix()?this.getPrefix():m;var A=this.getPostfix()?this.getPostfix():m;var H=D+(C?a:m)+G;if(F.length>0){H+=m+qx.locale.Number.getDecimalSeparator(this.__jj)+F;}
;H+=A;return H;}
,parse:function(T){var P=qx.lang.String.escapeRegexpChars(qx.locale.Number.getGroupSeparator(this.__jj)+m);var N=qx.lang.String.escapeRegexpChars(qx.locale.Number.getDecimalSeparator(this.__jj)+m);var L=new RegExp(d+qx.lang.String.escapeRegexpChars(this.getPrefix())+p+n+P+c+j+N+u+qx.lang.String.escapeRegexpChars(this.getPostfix())+t);var O=L.exec(T);if(O==null){throw new Error(e+T+f);}
;var Q=(O[1]==a);var S=O[2]||h;var R=O[3];S=S.replace(new RegExp(P,q),m);var M=(Q?a:m)+S;if(R!=null&&R.length!=0){R=R.replace(new RegExp(N),m);M+=v+R;}
;return parseFloat(M);}
}});}
)();
