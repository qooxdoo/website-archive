...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.showcase.Translation.9f2a5aef4ff1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="MessageId:",b="<b>Locale and language:</b>",c="new Locale must not be empty string",d="Choose Locale:",f="The quick brown fox",g='Territory: ',h=" already exists",j="en",k="Simple Translate: this.tr('Hello World')",l='Locale: ',m="Orange",n="Add new Locale<br> at runtime",o="en_QX",p="execute",q="A comment: 'The mammal fox that is.'<br>is added in the .po file",r="Translation",s=" has been added for messageId: ",t="Singular/Plural Translate: this.trn('I have an apple', I have %1 apples, ",u="changeLocale",v="MessageId",w="qx.debug",x="The mammal fox that is.",y="Number of apples",z="show Translation",A="Hello World",B="value",C="Locale Name (ex: en_QX)",D='bottom',E="changeSelection",F="Mark the string 'Orange' for later<br>translation so we do not forget it",G='top',H="I have an apple",I="",J="demo",K="Add translation<br>at runtime for: <b>",L="Add new Locale",M="tap 'Show Translation' button to translate<br>the messageId in the current selected locale",N="</b>",O='Territory',P="I have %1 apples",Q="Simple messageId translation",R=" for locale: ",S="Mark the string 'Orange' for later translation so we do not forget it: this.marktr('Orange')",T=", applesNumber)",U=" has been added",V="<b>Dynamic translation. This is added at runtime.</b>",W="<b>Static translation [found in .po files]. Hover the labels below to see more info.</b>",X="Add Translation",Y='Language',bo='middle',bp="Singular/Plural Translate<br>Choose a number from the select box<br>to see plural translation",bq="with .po comment: this.trc('The mammal fox that is.', 'The quick brown fox')",bk='Language: ',bl="<i>MessageId</i>",bm="Number of apples:",bn='right',br='Locale',bs="<i>Translated Text</i>",bt="demobrowser.demo.showcase.Translation";qx.Class.define(bt,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);if((qx.core.Environment.get(w))){qx.log.appender.Native;qx.log.appender.Console;}
;var bJ=new qx.ui.container.Composite(new qx.ui.layout.Grid(8,8));this.getRoot().add(bJ,{top:20,left:20});var bP=new qx.ui.basic.Label(b);bP.setRich(true);bJ.add(bP,{row:0,column:0,colSpan:3});var bR=new qx.ui.basic.Label(d);bR.setMarginTop(5);bR.setAlignY(bo);bJ.add(bR,{row:1,column:0});var cj=qx.locale.Manager.getInstance();var cb=cj.getAvailableLocales().sort();var bH=cj.getLocale();var bG=new qx.ui.form.SelectBox();bG.setAllowGrowY(false);bG.setAlignY(bo);var bQ=null;for(var i=0;i<cb.length;i++ ){var bX=new qx.ui.form.ListItem(cb[i]);bG.add(bX);if((!bQ&&cb[i]==j)||cb[i]==bH){bQ=bX;}
;}
;bG.addListener(E,function(e){var cn=e.getData()[0].getLabel();qx.locale.Manager.getInstance().setLocale(cn);}
);bJ.add(bG,{row:1,column:1});var ck=new qx.ui.basic.Label(br);ck.setMarginTop(5);ck.setWidth(80);var ch=new qx.ui.basic.Label(O);ch.setMarginTop(5);ch.setWidth(80);var ci=new qx.ui.basic.Label(Y);ci.setMarginTop(5);ci.setWidth(80);var bB=new qx.ui.container.Composite(new qx.ui.layout.VBox(3));bB.add(ck);bB.add(ch);bB.add(ci);bJ.add(bB,{row:1,column:2});qx.locale.Manager.getInstance().addListener(u,function(){var co=qx.locale.Manager.getInstance();ck.setValue(l+co.getLocale());ch.setValue(g+co.getTerritory());ci.setValue(bk+co.getLanguage());bu.apply(this);cm.setValue(K+cj.getLocale()+N);}
,this);function bu(){bD.setValue(this.tr(A));bD.setToolTipText(k);var cp=parseInt(cf.getSelection()[0].getLabel());bC.setValue(this.trn(H,P,cp,cp));bC.setToolTipText(t+cp+T);bT.setValue(this.trc(x,f));bT.setToolTipText(bq);bA.setValue(this.marktr(m));bA.setToolTipText(S);var cq=cd.getValue();if(cq&&cq.length>0){bS.setValue(cj.translate(cq,[],cj.getLocale()));}
;}
;var ce=new qx.ui.basic.Label(W);ce.setRich(true);ce.setMarginTop(15);bJ.add(ce,{row:2,column:0,colSpan:4});var bv=new qx.ui.basic.Label(bl);bv.setRich(true);bJ.add(bv,{row:3,column:1});var bN=new qx.ui.basic.Label(bs);bN.setRich(true);bJ.add(bN,{row:3,column:2});var bV=new qx.ui.basic.Label(A);bJ.add(bV,{row:4,column:1});var bD=new qx.ui.basic.Label(this.tr(A));bJ.add(bD,{row:4,column:2});var cg=new qx.ui.basic.Label(Q);bJ.add(cg,{row:4,column:3});var bF=new qx.ui.basic.Label(bm);bF.setAlignX(bn);bF.setAlignY(D);bJ.add(bF,{row:5,column:0});var cf=new qx.ui.form.SelectBox();for(var i=1;i<5;i++ ){var bE=new qx.ui.form.ListItem(i+I);cf.add(bE);}
;cf.setToolTipText(y);cf.setAlignY(G);cf.setAlignX(bn);bJ.add(cf,{row:6,column:0});var bw=new qx.ui.basic.Label(H);bw.setAlignY(bo);bJ.add(bw,{row:5,column:1,rowSpan:2});var bC=new qx.ui.basic.Label(this.trn(H,P,1,1));bC.setAlignY(bo);bJ.add(bC,{row:5,column:2,rowSpan:2});var ca=new qx.ui.basic.Label(bp);ca.setAlignY(bo);ca.setRich(true);bJ.add(ca,{row:5,column:3,rowSpan:2});cf.addListener(E,function(e){bu.apply(this);}
);var bI=new qx.ui.basic.Label(f);bJ.add(bI,{row:7,column:1});var bT=new qx.ui.basic.Label(this.trc(x,f));bT.setWidth(140);bJ.add(bT,{row:7,column:2});var bW=new qx.ui.basic.Label(q);bW.setRich(true);bJ.add(bW,{row:7,column:3});var bL=new qx.ui.basic.Label(m);bJ.add(bL,{row:8,column:1});var bA=new qx.ui.basic.Label(this.marktr(m));bJ.add(bA,{row:8,column:2});var bK=new qx.ui.basic.Label(F);bK.setRich(true);bJ.add(bK,{row:8,column:3});var by=new qx.ui.basic.Label(V);by.setRich(true);by.setMarginTop(15);bJ.add(by,{row:9,column:0,colSpan:3});var cm=new qx.ui.basic.Label(K+cj.getLocale()+N);cm.setRich(true);cm.setWidth(120);bJ.add(cm,{row:10,column:0});var bx=new qx.ui.form.TextField(I);bx.setToolTipText(v);bJ.add(bx,{row:10,column:1});var bU=new qx.ui.form.TextField(I);bU.setToolTipText(r);bJ.add(bU,{row:10,column:2});var bM=new qx.ui.form.Button(X);bM.setAllowGrowY(false);bJ.add(bM,{row:10,column:3});bM.addListener(p,function(e){var cr={};cr[bx.getValue()]=bU.getValue();cj.addTranslation(cj.getLocale(),cr);alert(bU.getValue()+s+bx.getValue()+R+cj.getLocale());}
,this);var bO=new qx.ui.basic.Label(a);bJ.add(bO,{row:11,column:0});var cd=new qx.ui.form.TextField(I);cd.setToolTipText(v);bJ.add(cd,{row:11,column:1});var cc=new qx.ui.form.Button(z);bJ.add(cc,{row:11,column:2});cc.setAllowGrowY(false);cc.addListener(p,function(e){var cs=cd.getValue();bS.setValue(cj.translate(cs,[],cj.getLocale()));}
,this);var bS=new qx.ui.basic.Label(M);bS.setRich(true);bS.setAlignY(bo);bJ.add(bS,{row:11,column:3});var cl=new qx.ui.basic.Label(n);cl.setRich(true);bJ.add(cl,{row:12,column:0});var bz=new qx.ui.form.TextField(I);bz.setPlaceholder(o);bz.setToolTipText(C);bJ.add(bz,{row:12,column:1});var bY=new qx.ui.form.Button(L);bY.setAllowGrowY(false);bJ.add(bY,{row:12,column:2});bY.addListener(p,function(e){var ct=bz.getValue();if(ct!=null&&ct.length<1){alert(c);return;}
;var cu=cj.getAvailableLocales();if(cu.indexOf(ct)!=-1){alert(ct+h);}
else {cj.addTranslation(ct,{"demo":J});alert(ct+U);bG.add(new qx.ui.form.ListItem(ct));}
;}
,this);if(bQ){bG.setSelection([bQ]);}
;bx.bind(B,cd,B);cd.bind(B,bx,B);return;}
}});}
)();
