...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.ListWithFilter.eb01d42a9ee4.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;this.__wX=true;var F=this.getSelection();var H=[];for(var i=0;i<F.getLength();i++ ){var G=F.getItem(i);var I=this._getSelectables();var E=-1;if(I!=null){E=I.indexOf(G);}
...
;}
;this.__xa(M);}
,__xa:function(Q){var P=this.getSelection();if(Q.length>0){var O=[0,P.getLength()];O=O.concat(Q);var R=P.splice.apply(P,O);R.dispose();}
else {P.removeAll();}
;}
,__xb:function(){var T=this.getSelection();var V=this._manager.getSelection();if(T.getLength()!==V.length){return false;}
...
(function(){var c="changeModel",d="resize",f="_applyLabelOptions",g="_applyLabelPath",h="qx.data.Array",j="_applyGroupLabelOptions",m="changeLength",n="Integer",o="_applyModel",p="changeGroups",q="Boolean",r="GroupingTypeError: You can't mix 'Objects' and 'Strings' as",s="sorter",t="row-layer",u="group",v="_applyIconPath",w="_applyDelegate",x="changeDelegate",y=" group identifier!",z="???",A="_applyRowHeight",B="qx.data.IListData",C="filter",D="virtual-list",E="String",F="_applyGroupLabelPath",G="_applyIconOptions",H="qx.ui.list.List";qx.Class.define(H,{extend:qx.ui.virtual.core.Scroller,include:[qx.ui.virtual.selection.MModel],implement:qx.data.controller.ISelection,construct:function(I){qx.ui.virtual.core.Scroller.call(this,0,1,20,100);this._init();this.__xc=new qx.data.Array();this.initGroups(this.__xc);if(I!=null){this.initModel(I);}
;this.initItemHeight();}
,properties:{appearance:{refine:true,init:D},focusable:{refine:true,init:true},width:{refine:true,init:100},height:{refine:true,init:200},model:{check:B,apply:o,event:c,nullable:true,deferredInit:true},itemHeight:{check:n,init:25,apply:A,themeable:true},labelPath:{check:E,apply:g,nullable:true},iconPath:{check:E,apply:v,nullable:true},groupLabelPath:{check:E,apply:F,nullable:true},labelOptions:{apply:f,nullable:true},iconOptions:{apply:G,nullable:true},groupLabelOptions:{apply:j,nullable:true},delegate:{apply:w,event:x,init:null,nullable:true},autoGrouping:{check:q,init:true},groups:{check:h,event:p,nullable:false,deferredInit:true}},members:{_background:null,_provider:null,_layer:null,__xd:null,__xe:null,__xf:null,__xg:false,__xh:false,__xi:false,__xc:null,refresh:function(){this.__xk();}
...
,_init:function(){this._provider=new qx.ui.list.provider.WidgetProvider(this);this.__xd=[];this.__xe=[];this.__xf={};this.__xg=false;this.__xh=false;this.__xi=false;this.getPane().addListener(d,this._onResize,this);this._initBackground();this._initLayer();}
...
;}
,_getLookupTable:function(){return this.__xd;}
,_lookup:function(P){return this.__xd[P];}
,_lookupGroup:function(Q){return this.__xe.indexOf(Q);}
,_reverseLookup:function(R){if(R<0){return -1;}
;return this.__xd.indexOf(R);}
...
,_applyDelegate:function(bl,bk){this._provider.setDelegate(bl);this.__xk();}
,_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);}
,_onModelChange:function(e){this.__xk();this._applyDefaultSelection();}
,__xj:function(){this.getPane().getRowConfig().setItemCount(this.__xd.length);this.getPane().fullUpdate();}
,__xk:function(){this.__xd=[];this.__xe=[];this.__xf={};if(this.isAutoGrouping()){this.getGroups().removeAll();}
;var bm=this.getModel();if(bm!=null){this._runDelegateFilter(bm);this._runDelegateSorter(bm);this._runDelegateGroup(bm);}
;this._updateSelection();this.__xj();}
,_runDelegateFilter:function(bo){var bn=qx.util.Delegate.getMethod(this.getDelegate(),C);for(var i=0,l=bo.length;i<l; ++i){if(bn==null||bn(bo.getItem(i))){this.__xd.push(i);}
;}
;}
,_runDelegateSorter:function(bp){if(this.__xd.length==0){return;}
;var bq=qx.util.Delegate.getMethod(this.getDelegate(),s);if(bq!=null){this.__xd.sort(function(a,b){return bq(bp.getItem(a),bp.getItem(b));}
);}
;}
,_runDelegateGroup:function(bu){var bv=qx.util.Delegate.getMethod(this.getDelegate(),u);if(bv!=null){for(var i=0,l=this.__xd.length;i<l; ++i){var br=this.__xd[i];var bt=this.getModel().getItem(br);var bs=bv(bt);this.__xl(bs,br);}
;this.__xd=this.__xm();}
;}
,__xl:function(bx,bw){if(bx==null){this.__xi=true;bx=z;}
;var name=this.__xn(bx);if(this.__xf[name]==null){this.__xf[name]=[];if(this.isAutoGrouping()){this.getGroups().push(bx);}
;}
;this.__xf[name].push(bw);}
,__xm:function(){this.__xo();var bB=[];var bD=0;var bz=this.getGroups();for(var i=0;i<bz.getLength();i++ ){var by=bz.getItem(i);bB.push(-1);this.__xe.push(bD);bD++ ;var bC=this.__xn(by);var bA=this.__xf[bC];if(bA!=null){for(var k=0;k<bA.length;k++ ){bB.push(bA[k]);bD++ ;}
;}
;}
;return bB;}
,__xn:function(bF){var name=null;if(!qx.lang.Type.isString(bF)){var bE=this.getGroups().indexOf(bF);this.__xh=true;name=u;if(bE==-1){name+=this.getGroups().getLength();}
else {name+=bE;}
;}
else {this.__xg=true;var name=bF;}
;return name;}
,__xo:function(){if(this.__xh&&this.__xi||this.__xh&&this.__xg){throw new Error(r+y);}
;}
...
;this._background.dispose();this._provider.dispose();this._layer.dispose();this._background=this._provider=this._layer=this.__xd=this.__xe=this.__xf=null;if(this.__xc){this.__xc.dispose();}
;}
});}
)();
...
;this.__xr={};this.__xs={};}
,properties:{colorEven:{nullable:true,check:c,apply:b,themeable:true},colorOdd:{nullable:true,check:c,apply:a,themeable:true}},members:{__xt:null,__xu:null,__xr:null,__xs:null,setColor:function(h,g){if(g){this.__xr[h]=qx.theme.manager.Color.getInstance().resolve(g);}
else {delete (this.__xr[h]);}
;}
,clearCustomColors:function(){this.__xr={};this.updateLayerData();}
,getColor:function(i){var j=this.__xr[i];if(j){return j;}
else {return i%2==0?this.__xt:this.__xu;}
;}
,_applyColorEven:function(l,k){if(l){this.__xt=qx.theme.manager.Color.getInstance().resolve(l);}
else {this.__xt=null;}
;this.updateLayerData();}
,_applyColorOdd:function(n,m){if(n){this.__xu=qx.theme.manager.Color.getInstance().resolve(n);}
else {this.__xu=null;}
;this.updateLayerData();}
,setBackground:function(o,p){if(p){this.__xs[o]=qx.theme.manager.Decoration.getInstance().resolve(p);}
else {delete (this.__xs[o]);}
;this.updateLayerData();}
,getBackground:function(q){return this.__xs[q];}
},destruct:function(){this.__xr=this.__xs=null;}
});}
)();
...
;}
,setColor:function(J,I){qx.ui.virtual.layer.AbstractBackground.prototype.setColor.call(this,J,I);if(this.__xv(J)){this.updateLayerData();}
;}
,setBackground:function(K,L){qx.ui.virtual.layer.AbstractBackground.prototype.setBackground.call(this,K,L);if(this.__xv(K)){this.updateLayerData();}
;}
,__xv:function(M){var O=this.getFirstRow();var N=O+this.getRowSizes().length-1;return M>=O&&M<=N;}
}});}
)();
...
(function(){var a="cell.type",b="model[",c="groups[",d="model",f="",g="changeDelegate",h="label",i="qx.ui.list.core.MWidgetController",j="icon",k="]",l="value",m="BindingIds",n=".",o="group",p="String";qx.Mixin.define(i,{construct:function(){this.__xw=[];}
...
;}
,bindProperty:function(u,x,v,t,s){var y=t.getUserData(a);var w=this.__xx(s,u,y);if(v){v.ignoreConverter=d;}
;var z=this._list.bind(w,t,x,v);this.__xy(t,z);}
,bindPropertyReverse:function(D,B,E,F,A){var G=F.getUserData(a);var C=this.__xx(A,D,G);var H=F.bind(B,this._list,C,E);this.__xy(F,H);}
,removeBindings:function(){while(this.__xw.length>0){var I=this.__xw.pop();this._removeBindingsFrom(I);}
;}
...
;}
,_removeBindingsFrom:function(T){var U=this.__xz(T);while(U.length>0){var V=U.pop();try{this._list.removeBinding(V);}
catch(e){T.removeBinding(V);}
;}
;if(qx.lang.Array.contains(this.__xw,T)){qx.lang.Array.remove(this.__xw,T);}
;}
,__xx:function(X,ba,Y){var W=b+X+k;if(Y==o){W=c+X+k;}
;if(ba!=null&&ba!=f){W+=n+ba;}
;return W;}
,__xy:function(bc,bd){var bb=this.__xz(bc);if(!qx.lang.Array.contains(bb,bd)){bb.push(bd);}
;if(!qx.lang.Array.contains(this.__xw,bc)){this.__xw.push(bc);}
;}
,__xz:function(be){var bf=be.getUserData(m);if(bf==null){bf=[];be.setUserData(m,bf);}
;return bf;}
...
,styleSelectabled:function(v){var u=this.__xA(v);this._styleSelectabled(u);}
,styleUnselectabled:function(x){var w=this.__xA(x);this._styleUnselectabled(w);}
...
;}
,_styleSelectabled:function(A){this.__xB(A,{selected:1});}
,_styleUnselectabled:function(B){this.__xB(B,{});}
...
,__xA:function(G){return this._list._layer.getRenderedCellWidget(G,0);}
,__xB:function(I,H){if(I==null){return;}
...
;}
,__xD:function(){var r=this.__cM.shift();if(r==null){r=this._createWidget();this.fireDataEvent(a,r);}
;return r;}
