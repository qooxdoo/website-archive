...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Table.57e193276864.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a=" / ",b="horizontal",c="demobrowser.demo.virtual.Table",d="white",e="#EEE",f="vertical",g="__PN";qx.Class.define(c,{extend:qx.application.Standalone,members:{__PN:null,main:function(){qx.application.Standalone.prototype.main.call(this);var h=new qx.ui.virtual.core.Scroller(10000,10000,20,100);for(var i=0;i<10000;i++ ){h.getPane().getRowConfig().setItemSize(i,20+Math.round(Math.random()*40));h.getPane().getColumnConfig().setItemSize(i,50+Math.round(Math.random()*80));}
;this.getRoot().add(h,{edge:20});h.getPane().addLayer(new qx.ui.virtual.layer.Row(d,e));h.getPane().addLayer(new qx.ui.virtual.layer.GridLines(b));h.getPane().addLayer(new qx.ui.virtual.layer.GridLines(f));h.getPane().addLayer(new qx.ui.virtual.layer.HtmlCell(this));this.__PN=new qx.ui.virtual.cell.Cell();}
,getCellProperties:function(k,j){return this.__PN.getCellProperties(k+a+j);}
...
;this.__CJ=[];this.__CK=[];this._isHorizontal=(w||c)==c;}
,properties:{defaultLineColor:{init:h,check:s,themeable:true},defaultLineSize:{init:1,check:t,themeable:true}},members:{__CJ:null,__CK:null,isHorizontal:function(){return this._isHorizontal;}
...
;this.__CJ[B]=A;if(this.__CL(B)){this.updateLayerData();}
;}
...
;this.__CK[C]=D;if(this.__CL(C)){this.updateLayerData();}
;}
,__CL:function(E){if(this._isHorizontal){var I=this.getFirstColumn();var G=I+this.getColumnSizes().length-1;return E>=I&&E<=G;}
else {var H=this.getFirstRow();var F=H+this.getRowSizes().length-1;return E>=H&&E<=F;}
;}
,getLineSize:function(J){return this.__CK[J]||this.getDefaultLineSize();}
,getLineColor:function(K){return this.__CJ[K]||this.getDefaultLineColor();}
,__CM:function(P,M,O){var top=0;var L,N;for(var y=0;y<O.length-1;y++ ){L=this.getLineColor(M+y);N=this.getLineSize(M+y);top+=O[y];P.push(m,g,f+N+q,j,d,top-((N>1)?Math.floor(N/2):1),q,b,L,n,a);}
;}
,__CN:function(T,Q,S){var V=0;var R,U;for(var x=0;x<S.length-1;x++ ){R=this.getLineColor(Q+x);U=this.getLineSize(Q+x);V+=S[x];T.push(m,g,k+U+q,l,u,p,V-((U>1)?Math.floor(U/2):1),q,b,R,n,a);}
;}
,_fullUpdate:function(ba,bb,X,W){var Y=[];if(this._isHorizontal){this.__CM(Y,ba,X);}
else {this.__CN(Y,bb,W);}
