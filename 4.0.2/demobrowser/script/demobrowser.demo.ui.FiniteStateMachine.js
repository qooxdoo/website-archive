...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.e3919ed45cb4.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b=" => ",c="#autoActionsBeforeOnexit",d=": Could not find friendly name for '",f=": event '",g=" blocked.  Re-queuing.",h="string",j="'",k="Attempt to transition to nonexistent state ",l="Saved-state stack is empty",m="Internal error: invalid nextState",n=" not handled.  Ignoring.",o="State ",p="#autoActionsBeforeOntransition",q=" returned a value other than ",r="#",s=" already exists",u="Attempt to transition to POP_STATE_STACK ",v=":",w="Transition ",x=" does not exist",y="' on '",z="qx.util.fsm.FiniteStateMachine",A="String",B="#autoActionsAfterOntransition",C="",D="#actionsAfterOnentry",E="Saved-state stack is full",F="#autoActionsBeforeOnentry",G="__GS",H=": no transition found.  No state change.",I="#autoActionsAfterOnentry",J="#actionsBeforeOnentry",K=": Pushed event: ",L="Machine started with no available states",M=": Process event: ",N="#entry",O="#ontransition",P=" on ",Q="Invalid state: not an instance of ",R="Number",S="#exit",T="__GT",U="true, false, or null.",V="Explicit transition ",W="  ",X="qx.util.fsm.State",Y="while state stack is empty.",bj=".  Ignoring.",bk="#autoActionsAfterOnexit",bl=": Event '",bg="object",bh=" not handled for target ",bi=": Queued event: ";qx.Class.define(z,{extend:qx.core.Object,construct:function(bm){qx.core.Object.call(this);this.setName(bm);this.__iy={};this.__GQ=null;this.__GR=[];this.__GS=[];this.__GT=[];this.__GU={};this.__GV={};this.__GW={};this.__GX={};this.__GY={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:A,nullable:true},state:{check:A,nullable:true},previousState:{check:A,nullable:true},nextState:{check:A,nullable:true},maxSavedStates:{check:R,init:2},debugFlags:{check:R,init:7}},members:{__iy:null,__GQ:null,__GS:null,__GT:null,__GR:null,__GU:null,__GV:null,__GW:null,__GX:null,__GY:null,__Ha:false,addState:function(bn){if(!bn instanceof qx.util.fsm.State){throw new Error(Q+X);}
;var bo=bn.getName();if(bo in this.__iy){throw new Error(o+bo+s);}
;if(this.__GQ==null){this.__GQ=bo;}
;this.__iy[bo]=bn;}
,replaceState:function(bs,br){if(!bs instanceof qx.util.fsm.State){throw new Error(Q+X);}
;var bq=bs.getName();var bp=this.__iy[bq];this.__iy[bq]=bs;if(br){bp._bNeedDispose=true;}
;return bp;}
,addObject:function(bw,bt,bu){var bx=bt.toHashCode();this.__GV[bw]=bx;this.__GW[bx]=bw;this.__GU[bw]=bt;if(!bu){return;}
;if(typeof (bu)==h){bu=[bu];}
;for(var i=0;i<bu.length;i++ ){var bv=bu[i];if(!this.__GX[bv]){this.__GX[bv]={};}
;this.__GX[bv][bw]=true;if(!this.__GY[bw]){this.__GY[bw]=[];}
;this.__GY[bw].push(bv);}
;}
,removeObject:function(bB){var by;var bz;var bA;var bC;by=this.__GV[bB];if(this.__GY[bB]){for(var i=0;i<this.__GY[bB].length;i++ ){bz=this.__GY[bB][i];delete this.__GX[bz][bB];bC=true;for(bA in this.__GX[bz]){bC=false;break;}
;if(bC){delete this.__GX[bz];}
;}
;delete this.__GY[bB];}
;delete this.__GW[by];delete this.__GV[bB];delete this.__GU[bB];}
,getObject:function(bD){return this.__GU[bD];}
,getFriendlyName:function(bE){var bF=bE?bE.toHashCode():null;return bF?this.__GW[bF]:null;}
,getGroupObjects:function(bG){var a=[];for(var name in this.__GX[bG]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bH in this.__GV){var bJ=this.__GV[bH];var bI=this.getObject(bH);this.debug(bH+b+bJ);this.debug(W+bJ+b+this.__GW[bJ]);this.debug(W+bH+b+this.getObject(bH));this.debug(W+this.getObject(bH)+b+this.getFriendlyName(bI));}
;}
,_getInternalData:function(){return ({"states":this.__iy,"startState":this.__GQ,"eventQueue":this.__GS,"blockedEvents":this.__GT,"savedStates":this.__GR,"friendlyToObject":this.__GU,"friendlyToHash":this.__GV,"hashToFriendly":this.__GW,"groupToFriendly":this.__GX,"friendlyToGroups":this.__GY});}
,start:function(){var bL=this.__GQ;if(bL==null){throw new Error(L);}
...
;this.__iy[bL].getAutoActionsBeforeOnentry()(this);if(bK){this.debug(this.getName()+r+bL+N);}
;this.__iy[bL].getOnentry()(this,null);if(bK){this.debug(this.getName()+r+bL+D);}
;this.__iy[bL].getAutoActionsAfterOnentry()(this);}
,pushState:function(bM){if(this.__GR.length>=this.getMaxSavedStates()){throw new Error(E);}
;if(bM===true){this.__GR.push(this.getState());}
else if(bM){this.__GR.push(bM);}
else {this.__GR.push(this.getPreviousState());}
;}
,popState:function(){if(this.__GR.length==0){throw new Error(l);}
;return this.__GR.pop();}
,postponeEvent:function(event){this.__GT.unshift(event);}
,enqueueEvent:function(event,bN){if(bN){this.__GS.push(event);}
else {this.__GS.unshift(event);}
...
;}
;}
,eventListener:function(event){var e=event.clone();this.enqueueEvent(e,false);this.__Hb();}
...
,this,bV||20);}
,__Hb:function(){if(this.__Ha){return;}
;this.__Ha=true;while(this.__GS.length>0){var event=this.__GS.pop();var bW=this.__Hc(event);if(bW){event.dispose();}
;}
;this.__Ha=false;}
,__Hc:function(event){var ca;var cb;var bY;var cd;var cg;var e;var cf;var bX=this.getDebugFlags();var ck=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var ci=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var ch=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var ce=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var cj=this.getFriendlyName(event.getTarget());if(ck){this.debug(this.getName()+M+event.getType()+(cj?P+cj:C));}
;ca=this.getState();cd=this.__iy[ca];cg=cd.transitions;e=cd.getEvents()[event.getType()];if(!e){if(ck){this.debug(this.getName()+bl+event.getType()+j+n);}
;return true;}
...
;this.__GT.unshift(event);return false;default:if(typeof (cf)==h){if(cg[cf]){var cc=cg[cf];cg={};cg[cf]=cc;}
else {throw new Error(V+cf+x);}
;break;}
;};for(var t in cg){var cc=cg[t];switch(cc.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(w+ca+v+t+q+U);};cb=cc.getNextState();if(typeof (cb)==h){if(!cb in this.__iy){throw new Error(k+cb);}
;this.setNextState(cb);}
else {switch(cb){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cb=ca;this.setNextState(cb);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__GR.length==0){throw new Error(u+Y);}
;cb=this.__GR.pop();this.setNextState(cb);break;default:throw new Error(m);};}
...
;cd.getAutoActionsBeforeOnexit()(this);if(ch){this.debug(this.getName()+r+ca+S);}
...
;cd=this.__iy[this.getNextState()];this.setPreviousState(ca);this.setState(this.getNextState());this.setNextState(null);bY=ca;ca=cb;cb=undefined;if(ch){this.debug(this.getName()+r+ca+F);}
...
;cd.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__GT.length;i++ ){e=this.__GT.pop();this.__GS.unshift(e);}
;if(ci){this.debug(this.getName()+r+bY+b+this.getName()+r+ca);}
;return true;}
;if(ci){this.debug(this.getName()+r+ca+f+event.getType()+j+H);}
;return true;}
},destruct:function(){this._disposeArray(G);this._disposeArray(T);this.__GR=this.__iy=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="parameters",k=")",l=" fsm.getObject('",m="groups",n="Invalid function parameters: ",o="Invalid onexit type: ",p="  for (var i = 0; i < groupObjects.length; i++)",q="context",r="string",s="autoActionsAfterOnentry",t="Invalid numeric value in events object: ",u='"',v="Invalid state name",w=" value: ",x="');",y="State ",z="autoActionsBeforeOnexit",A="autoActionsBeforeOnentry",B="__GH",C="}",D="): ",E="__GL",F="onexit",G="{",H="Invalid function request type: ",I="number",J="__GN",K=": ",L="expected array, found ",M="Invalid friendly name in 'objects' list: ",N="autoActionsAfterOnexit",O="  fsm.debug(ex);",P="(",Q="Invalid onentry type: ",R="    fsm.getObject(objName).",S=")(fsm)",T="events",U="undefined",V="Invalid ",W="__GJ",X="try",Y="    var objName = groupObjects[i];",ba="events must be an object",bb="').",bc="objects",bd="expected object, found ",be="qx.util.fsm.Transition",bf="The events object must be provided in new state info",bg="catch(ex)",bh="  {",bi="    fsm.getGroupObjects('",bj="Invalid value in events object ",bk="Invalid transition: not an instance of ",bl="  }",bm="__GK",bn="__GI",bo="Invalid value in events object: ",bp="Invalid 'objects' list: expected array, got ",bq="Invalid numeric value in events object ",br="__GM",bs="qx.util.fsm.State",bt="onentry",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="__GO",bB="object";qx.Class.define(bs,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(q,bE);for(var bC in bF){switch(bC){case bt:this.setOnentry(this.__GP(bF[bC],bE));break;case F:this.setOnexit(this.__GP(bF[bC],bE));break;case A:this.setAutoActionsBeforeOnentry(bF[bC]);break;case s:this.setAutoActionsAfterOnentry(bF[bC]);break;case z:this.setAutoActionsBeforeOnexit(bF[bC]);break;case N:this.setAutoActionsAfterOnexit(bF[bC]);break;case T:this.setEvents(bF[bC]);break;case q:break;default:this.setUserData(bC,bF[bC]);this.debug(y+bD+K+bz+bC);break;};}
;if(!this.getEvents()){throw new Error(bf);}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(actionType,value,context){if(typeof (value)!=bB){throw new Error(V+actionType+w+typeof (value));}
;var funcFragment;var func=X+G;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(H+L+typeof (functionRequest));}
;for(var i=0;i<functionRequest.length;i++ ){objectAndGroupList=functionRequest[i];if(typeof (objectAndGroupList)!=bB){throw new Error(bv+bd+typeof (functionRequest[param]));}
...
;if(typeof (params[j])==c){funcFragment+=P+params[j]+S;}
else if(typeof (params[j])==r){funcFragment+=u+params[j]+u;}
else {funcFragment+=params[j];}
;}
;funcFragment+=k;var a=objectAndGroupList[bc];if(!a){a=[];}
else if(!a instanceof Array){throw new Error(bp+typeof (a));}
;for(var j=0;j<a.length;j++ ){if(typeof (a[j])!=r){throw new Error(M+a[j]);}
;func+=l+a[j]+bb+funcFragment+bu;}
;var g=objectAndGroupList[m];if(g){if(!g instanceof Array){throw new Error(bw+typeof (g));}
;for(j=0;j<g.length;j++ ){func+=d+bi+g[j]+x+p+bh+Y+R+funcFragment+bu+bl;}
;}
;}
;}
;func+=C+bg+G+O+C;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:B,nullable:true},onentry:{transform:bn,nullable:true,init:function(bG,event){}
},onexit:{transform:W,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:E,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:br,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:J,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:bA,nullable:true,init:function(bL,event){}
},events:{transform:bm,nullable:true}},members:{__GH:function(bM){if(typeof (bM)!=r||bM.length<1){throw new Error(v);}
;return bM;}
,__GI:function(bN){switch(typeof (bN)){case U:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(q));default:throw new Error(Q+typeof (bN));};}
,__GJ:function(bP){switch(typeof (bP)){case U:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(q));default:throw new Error(o+typeof (bP));};}
,__GK:function(bT){if(typeof (bT)!=bB){throw new Error(ba);}
;for(var e in bT){var bR=bT[e];if(typeof (bR)==I&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(t+e+K+bR);}
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==I&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(bq+P+e+D+bS+K+bR[bS]);}
else if(typeof (bR[bS])!=r&&typeof (bR[bS])!=I){throw new Error(bj+P+e+D+bS+K+bR[bS]);}
;}
;}
else if(typeof (bR)!=r&&typeof (bR)!=I){throw new Error(bo+e+K+bT[e]);}
;}
;return bT;}
,__GL:function(bU){return qx.util.fsm.State._commonTransformAutoActions(A,bU,this.getUserData(q));}
,__GM:function(bV){return qx.util.fsm.State._commonTransformAutoActions(s,bV,this.getUserData(q));}
,__GN:function(bW){return qx.util.fsm.State._commonTransformAutoActions(z,bW,this.getUserData(q));}
,__GO:function(bX){return qx.util.fsm.State._commonTransformAutoActions(N,bX,this.getUserData(q));}
,__GP:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bk+be);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="autoActionsAfterOntransition",d="or one of the Fsm.StateChange constants",e="__Hh",g="context",h="string",i="predicate",j="__He",k="nextState must be an explicit state name, ",l="__Hg",m="Transition ",n=": ",o="String",p="number",q="Invalid transition nextState type: ",r="Invalid transition nextState value: ",s="Adding user-provided field to transition: ",t="ontransition",u="Invalid transition predicate type: ",v="boolean",w="nextState",x="__Hf",y="__Hd",z="Invalid ontransition type: ",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(g,E);for(var C in D){switch(C){case i:this.setPredicate(this.__GP(D[C],E));break;case w:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__GP(D[C],E));break;case c:this.setAutoActionsAfterOntransition(this.__GP(D[C],E));break;case t:this.setOntransition(this.__GP(D[C],E));break;case g:break;default:this.setUserData(C,D[C]);this.debug(m+F+n+s+C);break;};}
;}
,properties:{name:{check:o,nullable:true},predicate:{init:function(G,event){return true;}
,transform:y},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:j},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:x},autoActionsAfterOntransition:{init:function(I,event){}
,transform:l},ontransition:{init:function(J,event){}
,transform:e}},members:{__Hd:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
;case v:return function(M,event){return K;}
;case a:return qx.lang.Function.bind(K,this.getUserData(g));default:throw new Error(u+typeof (K));};}
,__He:function(N){switch(typeof (N)){case h:return N;case p:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(r+N+n+k+d);};break;default:throw new Error(q+typeof (N));};}
,__Hf:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(g));}
,__Hg:function(P){return qx.util.fsm.State._commonTransformAutoActions(c,P,this.getUserData(g));}
,__Hh:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(g));default:throw new Error(z+typeof (Q));};}
,__GP:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
