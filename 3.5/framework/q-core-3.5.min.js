...
,createNamespace:function(name,O){var R=name.split(L);var Q=R[0];var parent=this.__root&&this.__root[Q]?this.__root:window;for(var i=0,P=R.length-1;i<P;i++ ,Q=R[i]){if(!parent[Q]){parent=parent[Q]={};}
...
;return bg;}
};qx.Bootstrap.define(A,{statics:{__root:null,LOADSTART:qx.$$start||new Date(),DEBUG:(function(){var bl=true;if(qx.$$environment&&qx.$$environment[r]===false){bl=false;}
;return bl;}
...
;}
;return bs;}
,__shadowedKeys:[x,k,o,q,G,M,g],keys:({"ES5":Object.keys,"BROKEN_IE":function(bv){if(bv===null||(typeof bv!=N&&typeof bv!=c)){throw new TypeError(f);}
;var bw=[];var by=Object.prototype.hasOwnProperty;for(var bz in bv){if(by.call(bv,bz)){bw.push(bz);}
;}
;var bx=qx.Bootstrap.__shadowedKeys;for(var i=0,a=bx,l=a.length;i<l;i++ ){if(by.call(bv,a[i])){bw.push(a[i]);}
;}
;return bw;}
...
,getClass:function(bM){var bL=Object.prototype.toString.call(bM);return (qx.Bootstrap.__classToTypeMap[bL]||bL.slice(8,-1));}
...
,select:function(gu,gt){return this.__pickFromValues(this.get(gu),gt);}
,__pickFromValues:function(gy,gx){var gw=gx[gy];if(gx.hasOwnProperty(gy)){return gw;}
...
else {this._checks[gB]=this.__createCheck(gA);}
;}
;}
...
,__importFromGenerator:function(){if(qx&&qx.$$environment){for(var gE in qx.$$environment){var gF=qx.$$environment[gE];this._checks[gE]=this.__createCheck(gF);}
;}
;}
,__importFromUrl:function(){if(window.document&&window.document.location){var gG=window.document.location.search.slice(1).split(fq);for(var i=0;i<gG.length;i++ ){var gJ=gG[i].split(u);if(gJ.length!=3||gJ[0]!=fh){continue;}
;var gH=gJ[1];var gI=decodeURIComponent(gJ[2]);if(gI==fE){gI=true;}
else if(gI==eW){gI=false;}
else if(/^(\d|\.)+$/.test(gI)){gI=parseFloat(gI);}
;this._checks[gH]=this.__createCheck(gI);}
;}
;}
,__createCheck:function(gK){return qx.Bootstrap.bind(function(gL){return gL;}
,null,gK);}
},defer:function(gM){gM._initDefaultQxValues();gM.__importFromGenerator();if(gM.get(fX)===true){gM.__importFromUrl();}
;}
});}
)();
...
;}
;}
else if(qx.bom.client.Engine.__isWebkit()){if(/AppleWebKit\/([^ ]+)/.test(A)){B=RegExp.$1;var C=RegExp(c).exec(B);if(C){B=B.slice(0,C.index);}
;}
;}
else if(qx.bom.client.Engine.__isGecko()||qx.bom.client.Engine.__isMaple()){if(/rv\:([^\);]+)(\)|;)/.test(A)){B=RegExp.$1;}
;}
else if(qx.bom.client.Engine.__isMshtml()){var z=/Trident\/([^\);]+)(\)|;)/.test(A);if(/MSIE\s+([^\);]+)(\)|;)/.test(A)){B=RegExp.$1;if(B<8&&z){if(RegExp.$1==d){B=i;}
...
;}
;return B;}
,getName:function(){var name;if(qx.bom.client.Engine.__isOpera()){name=p;}
else if(qx.bom.client.Engine.__isWebkit()){name=u;}
else if(qx.bom.client.Engine.__isGecko()||qx.bom.client.Engine.__isMaple()){name=e;}
else if(qx.bom.client.Engine.__isMshtml()){name=s;}
...
,__isGecko:function(){return window.controllers&&window.navigator.product===j&&window.navigator.userAgent.indexOf(k)==-1&&window.navigator.userAgent.indexOf(m)==-1;}
,__isMshtml:function(){return window.navigator.cpuClass&&(/MSIE\s+([^\);]+)(\)|;)/.test(window.navigator.userAgent)||/Trident\/\d+?\.\d+?/.test(window.navigator.userAgent));}
...
(function(){var a="qxWeb",b="data-qx-class";qx.Bootstrap.define(a,{extend:qx.type.BaseArray,statics:{__init:[],$$qx:qx,$init:function(g,e){var f=[];for(var i=0;i<g.length;i++ ){var d=!!(g[i]&&(g[i].nodeType===1||g[i].nodeType===9));if(d){f.push(g[i]);continue;}
...
;var h=qx.lang.Array.cast(f,e);for(var i=0;i<qxWeb.__init.length;i++ ){qxWeb.__init[i].call(h);}
;return h;}
...
;qxWeb[name]=k[name];}
;}
,$attachInit:function(m){this.__init.push(m);}
...
,getHeight:function(C){var D=this[0];if(D){if(qx.dom.Node.isElement(D)){var A;if(C){var B={display:b,position:e,visibility:d};A=qx.module.Css.__swap(D,B,qx.module.Css.getHeight,this);}
...
;}
;return null;}
,getWidth:function(G){var H=this[0];if(H){if(qx.dom.Node.isElement(H)){var E;if(G){var F={display:b,position:e,visibility:d};E=qx.module.Css.__swap(H,F,qx.module.Css.getWidth,this);}
...
;return null;}
,getContentHeight:function(L){var N=this[0];if(qx.dom.Node.isElement(N)){var M;if(L){var K={position:e,visibility:d,display:b};M=qx.module.Css.__swap(N,K,qx.module.Css.getContentHeight,this);}
else {M=qx.bom.element.Dimension.getContentHeight(N);}
;return M;}
;return null;}
,getContentWidth:function(Q){var O=this[0];if(qx.dom.Node.isElement(O)){var R;if(Q){var P={position:e,visibility:d,display:b};R=qx.module.Css.__swap(O,P,qx.module.Css.getContentWidth,this);}
...
else {var bc=qxWeb.getDocument(ba[0]);X=qx.module.Css.__getDisplayDefault(ba[0].tagName,bc);}
;ba.setStyle(g,X);ba[0].$$qPrevDisp=c;}
;}
);return this;}
,__displayDefaults:{},__getDisplayDefault:function(bg,bd){var bf=qx.module.Css.__displayDefaults;if(!bf[bg]){var bh=bd||document;var be=qxWeb(bh.createElement(bg)).appendTo(bd.body);bf[bg]=be.getStyle(g);be.remove();}
;return bf[bg]||a;}
,__swap:function(bk,bi,bl,bo){var bm={};for(var bn in bi){bm[bn]=bk.style[bn];bk.style[bn]=bi[bn];}
...
;return M;}
,__ids:{"Windows NT 6.3":K,"Windows NT 6.2":H,"Windows NT 6.1":m,"Windows NT 6.0":G,"Windows NT 5.2":g,"Windows NT 5.1":W,"Windows NT 5.0":J,"Windows 2000":J,"Windows NT 4.0":bn,"Win 9x 4.90":z,"Windows CE":l,"Windows 98":E,"Win98":E,"Windows 95":t,"Win95":t,"Mac OS X 10_9":q,"Mac OS X 10.9":q,"Mac OS X 10_8":v,"Mac OS X 10.8":v,"Mac OS X 10_7":f,"Mac OS X 10.7":f,"Mac OS X 10_6":P,"Mac OS X 10.6":P,"Mac OS X 10_5":o,"Mac OS X 10.5":o,"Mac OS X 10_4":S,"Mac OS X 10.4":S,"Mac OS X 10_3":d,"Mac OS X 10.3":d,"Mac OS X 10_2":A,"Mac OS X 10.2":A,"Mac OS X 10_1":b,"Mac OS X 10.1":b,"Mac OS X 10_0":R,"Mac OS X 10.0":R},getVersion:function(){var bw=qx.bom.client.OperatingSystem.__getVersionForDesktopOs(navigator.userAgent);if(bw==null){bw=qx.bom.client.OperatingSystem.__getVersionForMobileOs(navigator.userAgent);}
;if(bw!=null){return bw;}
else {return M;}
;}
,__getVersionForDesktopOs:function(bx){var bA=[];for(var bz in qx.bom.client.OperatingSystem.__ids){bA.push(bz);}
;var bB=new RegExp(L+bA.join(r).replace(/\./g,w)+h,bo);var by=bB.exec(bx);if(by&&by[1]){return qx.bom.client.OperatingSystem.__ids[by[1]];}
;return null;}
,__getVersionForMobileOs:function(bF){var bG=bF.indexOf(C)!=-1;var bC=bF.match(/(iPad|iPhone|iPod)/i)?true:false;if(bG){var bE=new RegExp(/ Android (\d+(?:\.\d+)+)/i);var bH=bE.exec(bF);if(bH&&bH[1]){return bH[1];}
;}
...
;}
;return name;}
,getVersion:function(){var Q=navigator.userAgent;var R=new RegExp(A+qx.bom.client.Browser.__agents+l);var O=Q.match(R);if(!O){return y;}
...
;}
,__agents:{"webkit":v,"gecko":n,"mshtml":o,"opera":F}[qx.bom.client.Engine.getName()]},defer:function(S){qx.core.Environment.add(i,S.getName),qx.core.Environment.add(B,S.getVersion),qx.core.Environment.add(H,S.getDocumentMode),qx.core.Environment.add(h,S.getQuirksMode);}
});}
)();
...
,getLinearGradient:function(){qx.bom.client.Css.__WEBKIT_LEGACY_GRADIENT=false;var bW=M;var bT=document.createElement(n);var bU=qx.bom.Style.getAppliedStyle(bT,G,bW);if(!bU){bW=bC;var bU=qx.bom.Style.getAppliedStyle(bT,G,bW,false);if(bU){qx.bom.client.Css.__WEBKIT_LEGACY_GRADIENT=true;}
;}
;if(!bU){return null;}
;var bV=/(.*?)\(/.exec(bU);return bV?bV[1]:null;}
,getFilterGradient:function(){return qx.bom.client.Css.__isFilterSupported(Y,bH);}
...
,getFilterTextShadow:function(){return qx.bom.client.Css.__isFilterSupported(I,S);}
,__isFilterSupported:function(cj,ch){var ci=false;var ck=bI+cj+V+ch+by;var cg=document.createElement(n);document.body.appendChild(cg);cg.style.filter=ck;if(cg.filters&&cg.filters.length>0&&cg.filters.item(cj).enabled==true){ci=true;}
...
(function(){var a="css.float",b="qx.bom.element.Style",c="css.borderimage",d="css.userselect",e="css.boxsizing",f="pixelLeft",g="css.textoverflow",h="Cascaded styles are not supported in this browser!",i="pixelBottom",j="pixelHeight",k="pixelWidth",l="css.appearance",m="pixelRight",n="css.usermodify",o="float",p="",q="pixelTop",r="px";qx.Bootstrap.define(b,{statics:{__styleNames:null,__cssNames:null,__detectVendorProperties:function(){var t={"appearance":qx.core.Environment.get(l),"userSelect":qx.core.Environment.get(d),"textOverflow":qx.core.Environment.get(g),"borderImage":qx.core.Environment.get(c),"float":qx.core.Environment.get(a),"userModify":qx.core.Environment.get(n),"boxSizing":qx.core.Environment.get(e)};this.__cssNames={};for(var s in qx.lang.Object.clone(t)){if(!t[s]){delete t[s];}
else {this.__cssNames[s]=s==o?o:qx.bom.Style.getCssName(t[s]);}
;}
;this.__styleNames=t;}
,__getStyleName:function(name){var u=qx.bom.Style.getPropertyName(name);if(u){this.__styleNames[name]=u;}
;return u;}
,__mshtmlPixel:{width:k,height:j,left:f,right:m,top:q,bottom:i},__special:{clip:qx.bom.element.Clip,cursor:qx.bom.element.Cursor,opacity:qx.bom.element.Opacity,boxSizing:qx.bom.element.BoxSizing},COMPUTED_MODE:1,CASCADED_MODE:2,LOCAL_MODE:3,set:function(x,name,v,w){{}
;name=this.__styleNames[name]||this.__getStyleName(name)||name;if(w!==false&&this.__special[name]){this.__special[name].set(x,v);}
else {x.style[name]=v!==null?v:p;}
;}
,get:function(B,name,D,F){name=this.__styleNames[name]||this.__getStyleName(name)||name;if(F!==false&&this.__special[name]){return this.__special[name].get(B,D);}
...
;var G=this.__mshtmlPixel[name];if(G&&(G in B.style)){var E=B.style[name];B.style[name]=H||0;var A=B.style[G]+r;B.style[name]=E;return A;}
;return H;}
;return B.style[name]||p;};}
},defer:function(I){I.__detectVendorProperties();}
});}
)();
...
;}
;return B;}
,getAncestors:function(E){return this.__getAncestors(null,E);}
,getAncestorsUntil:function(G,F){return this.__getAncestors(G,F);}
,__getAncestors:function(I,J){var H=[];for(var i=0;i<this.length;i++ ){var parent=qx.dom.Element.getParentElement(this[i]);while(parent){var K=[parent];if(I&&qx.bom.Selector.matches(I,K).length>0){break;}
...
;return be;}
,getNextAll:function(bh){var bg=qx.module.Traversing.__hierarchyHelper(this,f,bh);return qxWeb.$init(bg,qxWeb);}
...
;return bn;}
,getPrevAll:function(bq){var bp=qx.module.Traversing.__hierarchyHelper(this,k,bq);return qxWeb.$init(bp,qxWeb);}
...
,getSiblings:function(bx){var bw=qx.module.Traversing.__hierarchyHelper(this,n,bx);return qxWeb.$init(bw,qxWeb);}
...
,equalNodes:function(bY,cb){bY=qx.module.Traversing.__getNodeFromArgument(bY);cb=qx.module.Traversing.__getNodeFromArgument(cb);if(!bY||!cb){return false;}
...
;}
;if(bX){var bQ=qx.module.Traversing.__getAttributes(bY);var ca=qx.module.Traversing.__getAttributes(cb);for(var bS in bQ){if(bQ[bS]!==ca[bS]){return false;}
;}
;}
...
(function(){var a="mshtml",b="engine.name",c="*",d="mouseover",e="mouseout",f="load",g="left",h="qx.module.Event",n="undefined",o="DOMContentLoaded",p="browser.documentmode",q="complete";qx.Bootstrap.define(h,{statics:{__normalizations:{},__hooks:{on:{},off:{}},on:function(B,z,A,s){for(var i=0;i<this.length;i++ ){var r=this[i];var v=A||qxWeb(r);var u=qx.module.Event.__hooks.on;var t=u[c]||[];if(u[B]){t=t.concat(u[B]);}
;for(var j=0,m=t.length;j<m;j++ ){t[j](r,B,z,A);}
;var w=function(event){var E=qx.module.Event.__normalizations;var D=E[c]||[];if(E[B]){D=D.concat(E[B]);}
...
;if(!r.__emitter){r.__emitter=new qx.event.Emitter();}
;var C=r.__emitter.on(B,w,v);if(!r.__listener){r.__listener={};}
;if(!r.__listener[B]){r.__listener[B]={};}
;r.__listener[B][C]=w;if(!A){if(!r.__ctx){r.__ctx={};}
;r.__ctx[C]=v;}
;}
;return this;}
,off:function(P,H,M,G){var L=(H===null&&M===null);for(var j=0;j<this.length;j++ ){var F=this[j];if(!F.__listener){continue;}
;var R=[];if(P!==null){R.push(P);}
else {for(var J in F.__listener){R.push(J);}
;}
;for(var i=0,l=R.length;i<l;i++ ){for(var K in F.__listener[R[i]]){var O=F.__listener[R[i]][K];if(L||O==H||O.original==H){var I=typeof F.__ctx!==n&&F.__ctx[K];var S;if(!M&&I){S=F.__ctx[K];}
;F.__emitter.off(R[i],O,S||M);if(L||O.original==H){qx.bom.Event.removeNativeListener(F,R[i],O,G);}
;delete F.__listener[R[i]][K];if(I){delete F.__ctx[K];}
;}
;}
;var N=qx.module.Event.__hooks.off;var Q=N[c]||[];if(N[P]){Q=Q.concat(N[P]);}
;for(var k=0,m=Q.length;k<m;k++ ){Q[k](F,P,H,M);}
;}
;}
;return this;}
,allOff:function(T){return this.off(T||null,null,null);}
,emit:function(U,V){for(var j=0;j<this.length;j++ ){var W=this[j];if(W.__emitter){W.__emitter.emit(U,V);}
;}
;return this;}
,once:function(Y,X,bb){var self=this;var ba=function(bc){self.off(Y,ba,bb);X.call(this,bc);}
;this.on(Y,ba,bb);return this;}
,hasListener:function(bg,be,bf){if(!this[0]||!this[0].__emitter||!this[0].__emitter.getListeners()[bg]){return false;}
;if(be){var bh=this[0].__emitter.getListeners()[bg];for(var i=0;i<bh.length;i++ ){var bd=false;if(bh[i].listener==be){bd=true;}
...
;}
;return false;}
;return this[0].__emitter.getListeners()[bg].length>0;}
...
;}
;bn.forEach(function(bp){bp.__emitter=null;}
);for(var i=0;i<bm.length;i++ ){var bi=bm[i];if(!bi.__emitter){continue;}
;var bk=bi.__emitter.getListeners();for(var name in bk){for(var j=bk[name].length-1;j>=0;j-- ){var bl=bk[name][j].listener;if(bl.original){bl=bl.original;}
;qxWeb(bn[i]).on(name,bl,bk[name][j].ctx);}
;}
;}
;}
,__isReady:false,ready:function(bq){if(document.readyState===q){window.setTimeout(bq,1);return;}
;var br=function(){qx.module.Event.__isReady=true;bq();}
...
else {var bt=function(){if(qx.module.Event.__isReady){return;}
...
;var by=qx.module.Event.__normalizations;for(var i=0,l=bA.length;i<l;i++ ){var bz=bA[i];if(qx.lang.Type.isFunction(bx)){if(!by[bz]){by[bz]=[];}
;by[bz].push(bx);}
;}
;}
,$unregisterNormalization:function(bE,bB){if(!qx.lang.Type.isArray(bE)){bE=[bE];}
;var bC=qx.module.Event.__normalizations;for(var i=0,l=bE.length;i<l;i++ ){var bD=bE[i];if(bC[bD]){qx.lang.Array.remove(bC[bD],bB);}
;}
;}
...
;var bI=qx.module.Event.__hooks.on;for(var i=0,l=bK.length;i<l;i++ ){var bJ=bK[i];if(qx.lang.Type.isFunction(bH)){if(!bI[bJ]){bI[bJ]=[];}
;bI[bJ].push(bH);}
;}
;if(!bG){return;}
;var bF=qx.module.Event.__hooks.off;for(var i=0,l=bK.length;i<l;i++ ){var bJ=bK[i];if(qx.lang.Type.isFunction(bG)){if(!bF[bJ]){bF[bJ]=[];}
;bF[bJ].push(bG);}
;}
;}
,$unregisterEventHook:function(bQ,bN,bM){if(!qx.lang.Type.isArray(bQ)){bQ=[bQ];}
;var bO=qx.module.Event.__hooks.on;for(var i=0,l=bQ.length;i<l;i++ ){var bP=bQ[i];if(bO[bP]){qx.lang.Array.remove(bO[bP],bN);}
;}
;if(!bM){return;}
;var bL=qx.module.Event.__hooks.off;for(var i=0,l=bQ.length;i<l;i++ ){var bP=bQ[i];if(bL[bP]){qx.lang.Array.remove(bL[bP],bM);}
;}
;}
...
;}
;return null;}
,offById:function(o){var n=qx.event.Emitter.__storage[o];if(n){this.off(n.name,n.listener,n.ctx);}
;return null;}
...
,appendTo:function(parent){parent=qx.module.Manipulating.__getElementArray(parent);for(var i=0,l=parent.length;i<l;i++ ){this._forEachElement(function(r,j){if(i==0){qx.dom.Element.insertEnd(this[j],parent[i]);}
else {qx.dom.Element.insertEnd(this.eq(j).clone(true)[0],parent[i]);}
;}
);}
;return this;}
,insertBefore:function(s){s=qx.module.Manipulating.__getElementArray(s);for(var i=0,l=s.length;i<l;i++ ){this._forEachElement(function(t,u){if(i==0){qx.dom.Element.insertBefore(t,s[i]);}
else {qx.dom.Element.insertBefore(this.eq(u).clone(true)[0],s[i]);}
;}
);}
;return this;}
,insertAfter:function(v){v=qx.module.Manipulating.__getElementArray(v);for(var i=0,l=v.length;i<l;i++ ){for(var j=this.length-1;j>=0;j-- ){if(!this[j]||this[j].nodeType!==1){continue;}
;if(i==0){qx.dom.Element.insertAfter(this[j],v[i]);}
else {qx.dom.Element.insertAfter(this.eq(j).clone(true)[0],v[i]);}
;}
;}
;return this;}
,__getElementArray:function(x){if(!qx.lang.Type.isArray(x)){var w=qxWeb(x);x=w.length>0?w:[x];}
;return x.filter(function(y){return (y&&y.nodeType===1);}
);}
,wrap:function(z){z=qx.module.Manipulating.__getCollectionFromArgument(z);if(z.length==0){return this;}
;this._forEachElement(function(A){var B=z.eq(0).clone(true);qx.dom.Element.insertAfter(B[0],A);var C=qx.module.Manipulating.__getInnermostElement(B[0]);qx.dom.Element.insertEnd(A,C);}
);return this;}
,__getCollectionFromArgument:function(F){var D;if(qx.lang.Type.isArray(F)){D=qxWeb(F);}
else {var E=qx.bom.Html.clean([F]);if(E.length>0&&qx.dom.Node.isElement(E[0])){D=qxWeb(E);}
else {D=qxWeb(F);}
;}
;return D;}
,__getInnermostElement:function(G){if(G.childNodes.length==0){return G;}
;for(var i=0,l=G.childNodes.length;i<l;i++ ){if(G.childNodes[i].nodeType===1){return this.__getInnermostElement(G.childNodes[i]);}
;}
;return G;}
...
;return U.scrollTop;}
,_animationDescription:{scrollLeft:{duration:700,timing:c,keep:100,keyFrames:{'0':{},'100':{scrollLeft:1}}},scrollTop:{duration:700,timing:c,keep:100,keyFrames:{'0':{},'100':{scrollTop:1}}}},__animateScroll:function(W,X,V){var Y=qx.lang.Object.clone(qx.module.Manipulating._animationDescription[W],true);Y.keyFrames[100][W]=X;return this.animate(Y,V);}
...
(function(){var a="tap",b="swipe",c="qx.module.event.Touch",d="longtap";qx.Bootstrap.define(c,{statics:{TYPES:[a,d,b],normalize:function(event,f,e){if(!event){return event;}
;event._type=e;return event;}
},defer:function(g){qxWeb.$registerEventNormalization(g.TYPES,g.normalize);}
});}
)();
...
(function(){var a="swipe",b="touchend",c="touchmove",d="touchcancel",e="longtap",f="tap",g="touchstart",h="qx.module.event.TouchHandler";qx.Bootstrap.define(h,{statics:{TYPES:[f,e,a,g,b,c,d],register:function(i){if(!i.__touchHandler){if(!i.__emitter){i.__emitter=new qx.event.Emitter();}
;i.__touchHandler=new qx.event.handler.TouchCore(i,i.__emitter);}
;}
,unregister:function(l){if(l.__touchHandler){if(!l.__emitter){l.__touchHandler=null;}
else {var k=false;var j=l.__emitter.getListeners();qx.module.event.TouchHandler.TYPES.forEach(function(m){if(m in j&&j[m].length>0){k=true;}
;}
);if(!k){l.__touchHandler=null;}
;}
;}
;}
},defer:function(n){qxWeb.$registerEventHook(n.TYPES,n.register,n.unregister);}
});}
)();
(function(){var a="touchmove",b="os.name",c="MSPointerDown",d="swipe",e="android",f="engine.version",g="pointercancel",h="qx.event.handler.TouchCore",j="event.mspointer",k="MSPointerCancel",l="y",m="pointer-events",n="longtap",o="pointerup",p="touchend",q="pointerdown",r="MSPointerUp",s="right",t="engine.name",u="x",v="touchcancel",w="MSPointerMove",z="webkit",A="none",B="left",C="tap",D="pointermove",E="down",F="undefined",G="up",H="touchstart";qx.Bootstrap.define(h,{extend:Object,statics:{TAP_MAX_DISTANCE:qx.core.Environment.get(b)!=e?10:40,SWIPE_DIRECTION:{x:[B,s],y:[G,E]},SWIPE_MIN_DISTANCE:qx.core.Environment.get(b)!=e?11:41,SWIPE_MIN_VELOCITY:0,LONGTAP_TIME:500},construct:function(I,J){this.__target=I;this.__emitter=J;this._initTouchObserver();this.__pointers=[];}
,members:{__target:null,__emitter:null,__onTouchEventWrapper:null,__originalTarget:null,__startPageX:null,__startPageY:null,__startTime:null,__isSingleTouchGesture:null,__isTapGesture:null,__onMove:null,__beginScalingDistance:null,__beginRotation:null,__longTapTimer:null,__pointers:null,__touchEventNames:null,_initTouchObserver:function(){this.__onTouchEventWrapper=qx.lang.Function.listener(this._onTouchEvent,this);this.__touchEventNames=[H,a,p,v];if(qx.core.Environment.get(j)){var K=parseInt(qx.core.Environment.get(f),10);if(K==10){this.__touchEventNames=[c,w,r,k];}
else {this.__touchEventNames=[q,D,o,g];}
;}
...
;if(Q==H){this.__originalTarget=this._getTarget(T);this.__isTapGesture=true;if(T.touches&&T.touches.length>1){this.__beginScalingDistance=this._getScalingDistance(T.touches[0],T.touches[1]);this.__beginRotation=this._getRotationAngle(T.touches[0],T.touches[1]);}
;}
;if(Q==a){if(typeof T.scale==F&&T.targetTouches.length>1){var U=this._getScalingDistance(T.targetTouches[0],T.targetTouches[1]);T.scale=U/this.__beginScalingDistance;}
;if(typeof T.rotation==F&&T.targetTouches.length>1){var S=this._getRotationAngle(T.targetTouches[0],T.targetTouches[1]);T.rotation=S-this.__beginRotation;}
;if(this.__isTapGesture){this.__isTapGesture=this._isBelowTapMaxDistance(T.changedTouches[0]);}
;}
;this._fireEvent(T,Q,this.__originalTarget);this.__checkAndFireGesture(T,Q);if(qx.core.Environment.get(j)){if(Q==p||Q==v){delete this.__pointers[T.pointerId];}
;}
;}
,_detectTouchesByPointer:function(X,ba){var W=[];if(ba==H){this.__pointers[X.pointerId]=X;}
else if(ba==a){this.__pointers[X.pointerId]=X;}
;for(var Y in this.__pointers){var V=this.__pointers[Y];W.push(V);}
;return W;}
,_mapPointerEvent:function(bb){bb=bb.toLowerCase();if(bb.indexOf(q)!==-1){return H;}
else if(bb.indexOf(o)!==-1){return p;}
else if(bb.indexOf(D)!==-1){if(this.__onMove===true){return a;}
;}
else if(bb.indexOf(g)!==-1){return v;}
;return bb;}
,_isBelowTapMaxDistance:function(bc){var bd={x:bc.screenX-this.__startPageX,y:bc.screenY-this.__startPageY};var be=qx.event.handler.TouchCore;return (Math.abs(bd.x)<=be.TAP_MAX_DISTANCE&&Math.abs(bd.y)<=be.TAP_MAX_DISTANCE);}
...
;}
else if(qx.core.Environment.get(j)){var bf=this.__evaluateTarget(bg);if(bf){bh=bf;}
;}
;return bh;}
,__evaluateTarget:function(bk){var bi=null;var bj=null;if(bk&&bk.touches&&bk.touches.length!==0){bi=bk.touches[0].clientX;bj=bk.touches[0].clientY;}
...
;var bp=bp||bo.type;if(bq&&bq.nodeType&&this.__emitter){this.__emitter.emit(bp,bo);}
;}
,__checkAndFireGesture:function(br,bs,bt){if(!bt){bt=this._getTarget(br);}
;var bs=bs||br.type;if(bs==H){this.__gestureStart(br,bt);}
else if(bs==a){this.__gestureChange(br,bt);}
else if(bs==p){this.__gestureEnd(br,bt);}
;}
,__gestureStart:function(bv,bw){var bu=bv.changedTouches[0];this.__onMove=true;this.__startPageX=bu.screenX;this.__startPageY=bu.screenY;this.__startTime=new Date().getTime();this.__isSingleTouchGesture=bv.targetTouches.length===1;if(this.__isSingleTouchGesture){this.__longTapTimer=window.setTimeout(this.__fireLongTap.bind(this,bv,bw),qx.event.handler.TouchCore.LONGTAP_TIME);}
else {this.__stopLongTapTimer();}
;}
,__gestureChange:function(bx,by){if(this.__isSingleTouchGesture&&bx.changedTouches.length>1){this.__isSingleTouchGesture=false;}
;if(!this._isBelowTapMaxDistance(bx.changedTouches[0])){this.__stopLongTapTimer();}
;}
,__gestureEnd:function(bz,bA){this.__onMove=false;this.__stopLongTapTimer();if(this.__isSingleTouchGesture){var bD=bz.changedTouches[0];var bE={x:bD.screenX-this.__startPageX,y:bD.screenY-this.__startPageY};var bB;if(this.__originalTarget==bA&&this.__isTapGesture){if(qx.event&&qx.event.type&&qx.event.type.Tap){bB=qx.event.type.Tap;}
;this._fireEvent(bz,C,bA,bB);}
else {var bC=this.__getSwipeGesture(bz,bA,bE);if(bC){if(qx.event&&qx.event.type&&qx.event.type.Swipe){bB=qx.event.type.Swipe;}
;bz.swipe=bC;this._fireEvent(bz,d,this.__originalTarget,bB);}
;}
;}
;}
,__getSwipeGesture:function(bH,bI,bN){var bK=qx.event.handler.TouchCore;var bL=new Date().getTime()-this.__startTime;var bO=(Math.abs(bN.x)>=Math.abs(bN.y))?u:l;var bF=bN[bO];var bG=bK.SWIPE_DIRECTION[bO][bF<0?0:1];var bM=(bL!==0)?bF/bL:0;var bJ=null;if(Math.abs(bM)>=bK.SWIPE_MIN_VELOCITY&&Math.abs(bF)>=bK.SWIPE_MIN_DISTANCE){bJ={startTime:this.__startTime,duration:bL,axis:bO,direction:bG,distance:bF,velocity:bM};}
;return bJ;}
,__fireLongTap:function(bP,bQ){this._fireEvent(bP,n,bQ,qx.event.type.Tap);this.__longTapTimer=null;this.__isTapGesture=false;}
,__stopLongTapTimer:function(){if(this.__longTapTimer){window.clearTimeout(this.__longTapTimer);this.__longTapTimer=null;}
;}
,dispose:function(){this._stopTouchObserver();this.__originalTarget=this.__target=this.__touchEventNames=this.__pointers=this.__emitter=this.__beginScalingDistance=this.__beginRotation=null;this.__stopLongTapTimer();}
}});}
)();
