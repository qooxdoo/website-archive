...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.DragDrop.60ecd2c2f1f8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
);P.addListener(o,function(e){this.debug(r+e.getRelatedTarget());var W=e.getCurrentAction();var Y=e.getCurrentType();var V;switch(Y){case B:V=this.getSelection();if(W==p){var X=[];for(var i=0,l=V.length;i<l;i++ ){X[i]=V[i].clone();}
;V=X;}
;break;case j:V=this.getSelection()[0].getLabel();break;};if(W==n){var U=this.getSelection();for(var i=0,l=U.length;i<l;i++ ){this.remove(U[i]);}
;}
;e.addData(Y,V);}
);var L=new qx.ui.basic.Label(v);I.add(L,{left:140,top:20});var J=new qx.ui.form.List;J.setDroppable(true);J.setSelectionMode(t);I.add(J,{left:140,top:40});J.addListener(f,function(e){this.debug(E+e.getRelatedTarget());var ba=e.getData(B);for(var i=0,l=ba.length;i<l;i++ ){this.add(ba[i]);}
;}
);J.addListener(A,function(e){if(!e.supportsType(B)){e.preventDefault();}
;}
);var M=new qx.ui.basic.Label(h);I.add(M,{left:260,top:20});var T=new qx.ui.form.List;T.setDroppable(true);I.add(T,{left:260,top:40});T.addListener(f,function(e){this.debug(E+e.getRelatedTarget());var bb=e.getData(B);for(var i=0,l=bb.length;i<l;i++ ){this.add(bb[i]);}
;}
);T.addListener(A,function(e){if(!e.supportsType(B)){e.preventDefault();}
;if(e.getRelatedTarget().getSelection()[0].getLabel().substr(5)%2==1){e.preventDefault();}
;}
);T.addListener(x,function(e){if(e.getCurrentAction()!=p){e.preventDefault();}
;}
);var L=new qx.ui.basic.Label(c);I.add(L,{left:380,top:20});var Q=new qx.ui.form.TextArea;Q.setDroppable(true);Q.setHeight(100);I.add(Q,{left:380,top:40});Q.addListener(f,function(e){this.debug(E+e.getRelatedTarget());this.setValue(e.getData(j).replace(/,/g,y));}
);Q.addListener(A,function(e){if(!e.supportsType(j)){e.preventDefault();}
;}
);var N=new qx.ui.basic.Label(k);I.add(N,{left:600,top:20});var G=this.__JK=new qx.ui.form.List();G.setDraggable(true);G.setDroppable(true);G.setSelectionMode(t);I.add(G,{left:600,top:40});for(var i=0;i<20;i++ ){G.add(new qx.ui.form.ListItem(C+i,g));}
...
);G.addListener(b,function(e){var bd=e.getOriginalTarget();if(bd instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__RK=bd;}
;if(!qx.ui.core.Widget.contains(this,bd)&&bd!=H){return;}
;var bc=bd.getContentLocation();H.setWidth(bd.getBounds().width);H.setDomPosition(bc.left,bc.top);}
);G.addListener(A,function(e){if(e.getRelatedTarget()){e.preventDefault();}
;}
);G.addListener(f,function(e){this.__RL(e.getOriginalTarget());}
,this);H.addListener(f,function(e){this.__RL(this.__RK);}
,this);}
,__RL:function(bf){if(bf.classname!=m){return;}
;var be=this.__JK.getSortedSelection();for(var i=0,l=be.length;i<l;i++ ){this.__JK.addBefore(be[i],bf);this.__JK.addToSelection(be[i]);}
;}
...
;this._fireChange();}
,_setLeadItem:function(bY){var bX=this.__pd;if(bX!==null){this._styleSelectable(bX,u,false);}
;if(bY!==null){this._styleSelectable(bY,u,true);}
;this.__pd=bY;}
,getLeadItem:function(){return this.__pd!==null?this.__pd:null;}
,_setAnchorItem:function(cb){var ca=this.__pe;if(ca!=null){this._styleSelectable(ca,G,false);}
...
;this._fireChange();}
,handleMouseOver:function(event){if(this.__pg!=null&&this.__pg!=this._getScroll().top){this.__pg=null;return;}
...
;var cy=event.isCtrlPressed()||(qx.core.Environment.get(c)==D&&event.isMetaPressed());var cz=event.isShiftPressed();if(this.isItemSelected(cB)&&!cz&&!cy&&!this.getDrag()){this.__pf=cB;this._userInteraction=false;return;}
else {this.__pf=null;}
...
;break;};var cC=this.getMode();if(this.getDrag()&&cC!==X&&cC!==h&&!cz&&!cy){this.__oS=this._getLocation();this.__oP=this._getScroll();this.__oT=event.getDocumentLeft()+this.__oP.left;this.__oU=event.getDocumentTop()+this.__oP.top;this.__oV=true;this._capture();}
;this._fireChange(bo);this._userInteraction=false;}
,handleMouseUp:function(event){this._userInteraction=true;var cG=event.isCtrlPressed()||(qx.core.Environment.get(c)==D&&event.isMetaPressed());var cD=event.isShiftPressed();if(!cG&&!cD&&this.__pf!=null){var cE=this._getSelectableFromMouseEvent(event);if(cE===null||!this.isItemSelected(cE)){this._userInteraction=false;return;}
...
;this.__oW=event.getDocumentLeft();this.__oX=event.getDocumentTop();this._userInteraction=true;var cH=this.__oW+this.__oP.left;if(cH>this.__oT){this.__oY=1;}
else if(cH<this.__oT){this.__oY=-1;}
else {this.__oY=0;}
;var cI=this.__oX+this.__oP.top;if(cI>this.__oU){this.__pa=1;}
else if(cI<this.__oU){this.__pa=-1;}
else {this.__pa=0;}
;var location=this.__oS;if(this.__oW<location.left){this.__oM=this.__oW-location.left;}
...
;if(!this.__oO){this.__oO=new qx.event.Timer(100);this.__oO.addListener(E,this._onInterval,this);}
;this.__oO.start();this._autoSelect();event.stopPropagation();this._userInteraction=false;}
...
;if(this.__pb){this._fireChange(bo);}
;delete this.__oV;delete this.__oQ;delete this.__oR;this._releaseCapture();if(this.__oO){this.__oO.stop();}
;}
,_onInterval:function(e){this._scrollBy(this.__oM,this.__oN);this.__oP=this._getScroll();this._autoSelect();}
,_autoSelect:function(){var cS=this._getDimension();var cL=Math.max(0,Math.min(this.__oW-this.__oS.left,cS.width))+this.__oP.left;var cK=Math.max(0,Math.min(this.__oX-this.__oS.top,cS.height))+this.__oP.top;if(this.__oQ===cL&&this.__oR===cK){return;}
;this.__oQ=cL;this.__oR=cK;var cP=this._getAnchorItem();var cN=cP;var cQ=this.__oY;var cT,cM;while(cQ!==0){cT=cQ>0?this._getRelatedSelectable(cN,t):this._getRelatedSelectable(cN,T);if(cT!==null){cM=this._getSelectableLocationX(cT);if((cQ>0&&cM.left<=cL)||(cQ<0&&cM.right>=cL)){cN=cT;continue;}
;}
;break;}
;var cR=this.__pa;var cU,cO;while(cR!==0){cU=cR>0?this._getRelatedSelectable(cN,bA):this._getRelatedSelectable(cN,by);if(cU!==null){cO=this._getSelectableLocationY(cU);if((cR>0&&cO.top<=cK)||(cR<0&&cO.bottom>=cK)){cN=cU;continue;}
;}
;break;}
...
;this._fireChange(bB);}
,__ph:{Home:1,Down:1,Right:1,PageDown:1,End:1,Up:1,Left:1,PageUp:1},handleKeyPress:function(event){this._userInteraction=true;var dc,db;var de=event.getKeyIdentifier();var dd=this.getMode();var cX=event.isCtrlPressed()||(qx.core.Environment.get(c)==D&&event.isMetaPressed());var cY=event.isShiftPressed();var da=false;if(de===z&&cX){if(dd!==X&&dd!==h){this._selectAllItems();da=true;}
;}
...
;da=true;}
;}
else if(this.__ph[de]){da=true;if(dd===X||dd==h){dc=this._getSelectedItem();}
...
;}
;break;};this.__pg=this._getScroll().top;this._scrollItemIntoView(db);}
;}
...
;this.__gf={};}
,_selectItemRange:function(dj,dn,dl){var dp=this._getSelectableRange(dj,dn);if(!dl){var dm=this.__gf;var dq=this.__pi(dp);for(var dk in dm){if(!dq[dk]){this._removeFromSelection(dm[dk]);}
;}
;}
...
;}
,__pi:function(dv){var dw={};var du;for(var i=0,l=dv.length;i<l;i++ ){du=dv[i];dw[this._selectableToHashCode(du)]=du;}
;return dw;}
...
;}
;}
,_addToSelection:function(dB){var dC=this._selectableToHashCode(dB);if(this.__gf[dC]==null&&this._isSelectable(dB)){this.__gf[dC]=dB;this._styleSelectable(dB,P,true);this.__pb=true;}
;}
...
;this.__pb=true;}
,_removeFromSelection:function(dF){var dG=this._selectableToHashCode(dF);if(this.__gf[dG]!=null){delete this.__gf[dG];this._styleSelectable(dF,P,false);this.__pb=true;}
;}
...
;if(!dK){return false;}
;this._scrollItemIntoView(dH);this._setLeadItem(dO);this._setAnchorItem(dO);this.__pb=true;this._fireChange();}
,_fireChange:function(dP){if(this.__pb){this.__pc=dP||null;this.fireDataEvent(br,this.getSelection());delete this.__pb;}
;}
...
;return dR;}
;return null;}
},destruct:function(){this._disposeObjects(bq);this.__gf=this.__pf=this.__pe=null;this.__pd=null;}
});}
)();
(function(){var a="vertical",b="under",c="above",d="qx.ui.core.selection.Widget",e="left",f="right";qx.Class.define(d,{extend:qx.ui.core.selection.Abstract,construct:function(g){qx.ui.core.selection.Abstract.call(this);this.__lm=g;}
,members:{__lm:null,_isSelectable:function(h){return this._isItemSelectable(h)&&h.getLayoutParent()===this.__lm;}
,_selectableToHashCode:function(j){return j.$$hash;}
,_styleSelectable:function(k,m,n){n?k.addState(m):k.removeState(m);}
,_capture:function(){this.__lm.capture();}
,_releaseCapture:function(){this.__lm.releaseCapture();}
,_isItemSelectable:function(o){if(this._userInteraction){return o.isVisible()&&o.isEnabled();}
else {return o.isVisible();}
;}
,_getWidget:function(){return this.__lm;}
,_getLocation:function(){var p=this.__lm.getContentElement().getDomElement();return p?qx.bom.element.Location.get(p):null;}
,_getDimension:function(){return this.__lm.getInnerSize();}
...
,_scrollItemIntoView:function(w){this.__lm.scrollChildIntoView(w);}
,getSelectables:function(x){var y=false;if(!x){y=this._userInteraction;this._userInteraction=true;}
;var B=this.__lm.getChildren();var z=[];var A;for(var i=0,l=B.length;i<l;i++ ){A=B[i];if(this._isItemSelectable(A)){z.push(A);}
;}
;this._userInteraction=y;return z;}
,_getSelectableRange:function(C,D){if(C===D){return [C];}
;var H=this.__lm.getChildren();var E=[];var G=false;var F;for(var i=0,l=H.length;i<l;i++ ){F=H[i];if(F===C||F===D){if(G){E.push(F);break;}
else {G=true;}
;}
;if(G&&this._isItemSelectable(F)){E.push(F);}
;}
;return E;}
,_getFirstSelectable:function(){var I=this.__lm.getChildren();for(var i=0,l=I.length;i<l;i++ ){if(this._isItemSelectable(I[i])){return I[i];}
;}
;return null;}
,_getLastSelectable:function(){var J=this.__lm.getChildren();for(var i=J.length-1;i>0;i-- ){if(this._isItemSelectable(J[i])){return J[i];}
;}
;return null;}
,_getRelatedSelectable:function(N,O){var L=this.__lm.getOrientation()===a;var P=this.__lm.getChildren();var K=P.indexOf(N);var M;if((L&&O===c)||(!L&&O===e)){for(var i=K-1;i>=0;i-- ){M=P[i];if(this._isItemSelectable(M)){return M;}
;}
;}
...
(function(){var a="qx.ui.form.List",b="text",c="Boolean",d="one",f="addChildWidget",g="_applySpacing",h="Enter",j="Integer",k="action",m="keyinput",n="__pj",o="qx.event.type.Data",p="addItem",q="vertical",r="removeChildWidget",s="",t="_applyOrientation",u="single",v="keypress",w="list",x="horizontal",y="label",z="pane",A="removeItem";qx.Class.define(a,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MMultiSelectionHandling,qx.ui.form.MForm,qx.ui.form.MModelSelection],construct:function(B){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__pj=this._createListItemContainer();this.__pj.addListener(f,this._onAddChild,this);this.__pj.addListener(r,this._onRemoveChild,this);this.getChildControl(z).add(this.__pj);if(B){this.setOrientation(x);}
else {this.initOrientation();}
;this.addListener(v,this._onKeyPress);this.addListener(m,this._onKeyInput);this.__pk=s;}
...
,_applyOrientation:function(F,C){var D=F===x;var E=D?new qx.ui.layout.HBox():new qx.ui.layout.VBox();var content=this.__pj;content.setLayout(E);content.setAllowGrowX(!D);content.setAllowGrowY(D);this._applySpacing(this.getSpacing());}
,_applySpacing:function(H,G){this.__pj.getLayout().setSpacing(H);}
...
;if(((new Date).valueOf()-this.__pl)>1000){this.__pk=s;}
;this.__pk+=e.getChar();var K=this.findItemByLabelFuzzy(this.__pk);if(K){this.setSelection([K]);}
