...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.FormAndListController.b9d89b901fa0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="Martin",b="male",c="Alien",d="A",e="execute",f="mail",g="Firstname",h="Send By Email",i="dont know!",j="female",k="data",l="",m="Lastname",n="Gender",o="Wittemann",p="sendByEmail",q="demobrowser.demo.data.FormAndListController",r="Show model data in the log",s="M",t="model",u="label",v="F",w="Send By Mail",x="email",y="?";qx.Class.define(q,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var J={firstname:a,lastname:o,gender:[{label:b,data:s},{label:j,data:v},{label:i,data:y},{label:c,data:d}],sendByEmail:f};var I=qx.data.marshal.Json.createModel(J);var A=new qx.ui.form.Form();var B=new qx.ui.form.TextField();A.add(B,g);var G=new qx.ui.form.TextField();A.add(G,m);var D=new qx.ui.form.SelectBox();var z=new qx.data.controller.List(null,D);z.setDelegate({bindItem:function(L,K,M){L.bindProperty(u,u,null,K,M);L.bindProperty(k,t,null,K,M);}
});z.setModel(I.getGender());A.add(D,n);var F=new qx.ui.form.RadioButtonGroup();var E=new qx.ui.form.RadioButton(h);E.setModel(x);var H=new qx.ui.form.RadioButton(w);H.setModel(f);F.add(E);F.add(H);A.add(F,l,null,p);this.getRoot().add(new qx.ui.form.renderer.Single(A),{left:30,top:20});new qx.data.controller.Form(I,A);var C=new qx.ui.form.Button(r);this.getRoot().add(C,{left:240,top:20});C.addListener(e,function(){this.debug(qx.dev.Debug.debugProperties(I));}
,this);}
}});}
)();
...
(function(){var a="Boolean",b="_applyValid",c="",d="changeRequired",f="changeValid",g="changeInvalidMessage",h="__tc",j="changeSelection",k="_applyInvalidMessage",l="String",m="qx.ui.form.RadioButtonGroup",n="qx.event.type.Data";qx.Class.define(m,{extend:qx.ui.core.Widget,include:[qx.ui.core.MLayoutHandling,qx.ui.form.MModelSelection],implement:[qx.ui.form.IForm,qx.ui.core.ISingleSelection,qx.ui.form.IModelSelection],construct:function(o){qx.ui.core.Widget.call(this);if(o==null){this.setLayout(new qx.ui.layout.VBox(4));}
else {this.setLayout(o);}
;this.__tc=new qx.ui.form.RadioGroup();this.__tc.addListener(j,function(e){this.fireDataEvent(j,e.getData(),e.getOldData());}
,this);}
,properties:{valid:{check:a,init:true,apply:b,event:f},required:{check:a,init:false,event:d},invalidMessage:{check:l,init:c,event:g,apply:k},requiredInvalidMessage:{check:l,nullable:true,event:g}},events:{"changeSelection":n},members:{__tc:null,_applyInvalidMessage:function(r,p){var q=this._getChildren();for(var i=0;i<q.length;i++ ){q[i].setInvalidMessage(r);}
;}
,_applyValid:function(u,s){var t=this._getChildren();for(var i=0;i<t.length;i++ ){t[i].setValid(u);}
;}
,getRadioGroup:function(){return this.__tc;}
,getChildren:function(){return this._getChildren();}
,add:function(w,v){this.__tc.add(w);this._add(w,v);}
,remove:function(x){this.__tc.remove(x);this._remove(x);}
,removeAll:function(){var y=this.__tc.getItems();for(var i=0;i<y.length;i++ ){this.__tc.remove(y[i]);}
;return this._removeAll();}
,getSelection:function(){return this.__tc.getSelection();}
,setSelection:function(z){this.__tc.setSelection(z);}
,resetSelection:function(){this.__tc.resetSelection();}
,isSelected:function(A){return this.__tc.isSelected(A);}
,isSelectionEmpty:function(){return this.__tc.isSelectionEmpty();}
,getSelectables:function(B){return this.__tc.getSelectables(B);}
},destruct:function(){this._disposeObjects(h);}
});}
)();
...
(function(){var a="changeModel",b="modelSelection[0]",c="qx.core.Object",d="qx.debug",e="_applyTarget",f="qx.data.controller.Form",g="changeTarget",h="value",j=" of ",k="Could not bind property ",l=".",m="No target is set.",n="qx.ui.form.Form",o="_applyModel";qx.Class.define(f,{extend:qx.core.Object,construct:function(p,r,q){qx.core.Object.call(this);this._selfUpdate=!!q;this.__FH={};if(p!=null){this.setModel(p);}
;if(r!=null){this.setTarget(r);}
;}
,properties:{model:{check:c,apply:o,event:a,nullable:true,dereference:true},target:{check:n,apply:e,event:g,nullable:true,init:null,dereference:true}},members:{__FI:null,__FH:null,addBindingOptions:function(name,u,v){this.__FH[name]=[u,v];if(this.getModel()==null||this.getTarget()==null){return;}
;var s=this.getTarget().getItems()[name];var t=this.__FL(s)?b:h;this.__FI.removeTarget(s,t,name);this.__FI.addTarget(s,t,name,!this._selfUpdate,u,v);}
...
;if(this.__FH[name]&&this.__FH[name][1]){A=this.__FH[name][1].converter(A);}
;D[E[i]]=A;}
else {if(!D[E[i]]){D[E[i]]={};}
;D=D[E[i]];}
;}
...
;var F=this.getTarget().getItems();for(var name in F){var I=F[name];var H=this.__FL(I)?b:h;var G=this.__FH[name];G=G&&this.__FH[name][1];qx.data.SingleValueBinding.updateTarget(I,H,this.getModel(),name,G);}
;}
,_applyTarget:function(K,J){if(J!=null){this.__FK(J);}
;if(this.getModel()==null){return;}
;if(K!=null){this.__FJ();}
;}
,_applyModel:function(N,L){if(this.__FI!=null&&N==null){this.__FI.setModel(null);}
;if(this.__FI!=null){var M=this.getTarget().getItems();for(var name in M){var O=M[name];var P=this.__FL(O)?b:h;this.__FI.removeTarget(O,P,name);}
;}
;if(this.__FI!=null){this.__FI.setModel(N);}
;if(this.getTarget()==null){return;}
;if(N!=null){this.__FJ();}
;}
,__FJ:function(){if(this.__FI==null){this.__FI=new qx.data.controller.Object(this.getModel());}
;var T=this.getTarget().getItems();for(var name in T){var Q=T[name];var R=this.__FL(Q)?b:h;var S=this.__FH[name];try{if(S==null){this.__FI.addTarget(Q,R,name,!this._selfUpdate);}
else {this.__FI.addTarget(Q,R,name,!this._selfUpdate,S[0],S[1]);}
;}
catch(U){if(qx.core.Environment.get(d)){this.warn(k+name+j+this.getModel());}
;}
;}
;this.getTarget().redefineResetter();}
,__FK:function(V){if(this.__FI==null){return;}
;var Y=V.getItems();for(var name in Y){var X=Y[name];var W=this.__FL(X)?b:h;this.__FI.removeTarget(X,W,name);}
;}
,__FL:function(ba){return qx.Class.hasInterface(ba.constructor,qx.ui.core.ISingleSelection)&&qx.Class.hasInterface(ba.constructor,qx.ui.form.IModelSelection);}
...
;return bm;}
;return y;}
,startDisposeProfiling:qx.core.Environment.select(c,{"true":function(){this.disposeProfilingActive=true;this.__QH=qx.core.ObjectRegistry.getNextHash();}
,"default":(function(){}
)}),stopDisposeProfiling:qx.core.Environment.select(c,{"true":function(bv){if(!this.__QH){qx.log.Logger.error(r+this.classname+B);return [];}
;this.disposeProfilingActive=false;var bu=[];while(!qx.ui.core.queue.Dispose.isEmpty()){qx.ui.core.queue.Dispose.flush();}
;var bw=qx.core.ObjectRegistry.getNextHash();var by=qx.core.ObjectRegistry.getPostId();var bz=qx.core.ObjectRegistry.getStackTraces();for(var bx=this.__QH;bx<bw;bx++ ){var bt=qx.core.ObjectRegistry.fromHashCode(bx+by);if(bt&&bt.isDisposed&&!bt.isDisposed()){if(bv&&typeof bv==h&&!bv(bt)){continue;}
