...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Tree_Columns.e86a204b55d8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="__pH",b="open",c="abstract",d="qx.debug",e="'. It is not a child of this tree item.",f="addItem",g="visible",h="excluded",j="removeItem",k="Invalid child index: ",m="Cannot remove treeitem '",n="qx.ui.tree.core.AbstractTreeItem",o="__gx";qx.Class.define(n,{extend:qx.ui.tree.core.AbstractItem,type:c,construct:function(p){qx.ui.tree.core.AbstractItem.call(this,p);this.__gx=[];}
,properties:{parent:{check:n,nullable:true}},members:{__gx:null,__pH:null,getTree:function(){var r=this;while(r.getParent()){r=r.getParent();}
...
;}
;}
,getChildrenContainer:function(){if(!this.__pH){this.__pH=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?g:h});}
;return this.__pH;}
,hasChildrenContainer:function(){return this.__pH;}
,getParentChildrenContainer:function(){if(this.getParent()){return this.getParent().getChildrenContainer();}
else if(this.getLayoutParent()){return this.getLayoutParent();}
else {return null;}
;}
,getChildren:function(){return this.__gx;}
,hasChildren:function(){return this.__gx?this.__gx.length>0:false;}
...
;}
,__GU:function(){if(this.getParentChildrenContainer()){this.getParentChildrenContainer()._addAfter(this.getChildrenContainer(),this);}
;}
...
;this.__gx.push(Q);if(!N){this.__GU();}
;if(P){Q.recursiveAddToWidgetQueue();P.fireNonBubblingEvent(f,qx.event.type.Data,[Q]);}
;}
;if(P){qx.ui.core.queue.Widget.add(this);}
;}
,addAt:function(X,T){if(qx.core.Environment.get(d)){this.assert(T<=this.__gx.length&&T>=0,k+T);}
;if(T==this.__gx.length){this.add(X);return;}
;var W=X.getParent();if(W){W.remove(X);}
;var U=this.getChildrenContainer();X.setParent(this);var V=this.hasChildren();var S=this.__gx[T];U.addBefore(X,S);if(X.hasChildren()){U.addAfter(X.getChildrenContainer(),X);}
;qx.lang.Array.insertAt(this.__gx,X,T);if(!V){this.__GU();}
;if(this.getTree()){X.recursiveAddToWidgetQueue();qx.ui.core.queue.Widget.add(this);}
;}
,addBefore:function(Y,bb){if(qx.core.Environment.get(d)){this.assert(this.__gx.indexOf(bb)>=0);}
;var ba=Y.getParent();if(ba){ba.remove(Y);}
;this.addAt(Y,this.__gx.indexOf(bb));}
,addAfter:function(bd,bc){if(qx.core.Environment.get(d)){this.assert(this.__gx.indexOf(bc)>=0);}
;var be=bd.getParent();if(be){be.remove(bd);}
;this.addAt(bd,this.__gx.indexOf(bc)+1);}
,addAtBegin:function(bf){this.addAt(bf,0);}
,remove:function(bk){for(var i=0,l=arguments.length;i<l;i++ ){var bj=arguments[i];if(this.__gx.indexOf(bj)==-1){this.warn(m+bj+e);return;}
...
;}
;qx.lang.Array.remove(this.__gx,bj);bj.setParent(null);bg.remove(bj);}
;var bh=this.getTree();if(bh){bh.fireNonBubblingEvent(j,qx.event.type.Data,[bj]);}
;qx.ui.core.queue.Widget.add(this);}
,removeAt:function(bl){var bm=this.__gx[bl];if(bm){this.remove(bm);}
;}
,removeAll:function(){var bn=this.__gx.concat();for(var i=this.__gx.length-1;i>=0;i-- ){this.remove(this.__gx[i]);}
;return bn;}
},destruct:function(){this._disposeArray(o);this._disposeObjects(a);}
});}
)();
...
(function(){var a="Boolean",b="_applyOpenMode",c="excluded",d="Space",f="Left",g="Enter",h="dblclick",j="changeOpenMode",k="_applyRootOpenClose",l="__pj",m="visible",n="changeSelection",o="qx.ui.tree.Tree",p="qx.event.type.Data",q="qx.ui.tree.core.AbstractTreeItem",r="tree",s="_applyHideRoot",t="changeRoot",u="_applyRoot",v="click",w="keypress",x="none",y="pane",z="Right";qx.Class.define(o,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IModelSelection,qx.ui.form.IForm],include:[qx.ui.core.MMultiSelectionHandling,qx.ui.core.MContentPadding,qx.ui.form.MModelSelection,qx.ui.form.MForm],construct:function(){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__pj=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({allowShrinkY:false,allowGrowX:true});this.getChildControl(y).add(this.__pj);this.initOpenMode();this.initRootOpenClose();this.addListener(n,this._onChangeSelection,this);this.addListener(w,this._onKeyPress,this);}
,events:{addItem:p,removeItem:p},properties:{openMode:{check:[v,h,x],init:h,apply:b,event:j,themeable:true},root:{check:q,init:null,nullable:true,event:t,apply:u},hideRoot:{check:a,init:false,apply:s},rootOpenClose:{check:a,init:false,apply:k},appearance:{refine:true,init:r},focusable:{refine:true,init:true}},members:{__pj:null,SELECTION_MANAGER:qx.ui.tree.selection.SelectionManager,getChildrenContainer:function(){return this.__pj;}
...
;C.setVisibility(this.getHideRoot()?c:m);C.recursiveAddToWidgetQueue();}
;}
,_applyHideRoot:function(F,D){var E=this.getRoot();if(!E){return;}
;E.setVisibility(F?c:m);E.recursiveAddToWidgetQueue();}
,_applyRootOpenClose:function(I,G){var H=this.getRoot();if(!H){return;}
;H.recursiveAddToWidgetQueue();}
,_getContentPaddingTarget:function(){return this.__pj;}
...
(function(){var a="changeModel",b="_applyDelegate",c="qx.core.Object",d="_applyLabelPath",f="icon",g="change",h="label",k="' twice.",l="_applyTarget",m="changeTarget",n="_applyIconPath",o="Can not reverse bind the same target property '",p="_applyLabelOptions",q="_applyChildPath",r="_applyIconOptions",s="qx.data.controller.Tree",t="String",u="Can not bind the same target property '",v="_applyModel",w="get";qx.Class.define(s,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(y,z,A,x){qx.core.Object.call(this);this.__bd={};this.__Es=[];this.__IE={};if(A!=null){this.setChildPath(A);}
;if(x!=null){this.setLabelPath(x);}
;if(y!=null){this.setModel(y);}
;if(z!=null){this.setTarget(z);}
;}
,properties:{model:{check:c,apply:v,event:a,nullable:true,dereference:true},target:{apply:l,event:m,init:null,nullable:true,dereference:true},childPath:{check:t,apply:q,nullable:true},labelPath:{check:t,apply:d,nullable:true},iconPath:{check:t,apply:n,nullable:true},labelOptions:{apply:p,nullable:true},iconOptions:{apply:r,nullable:true},delegate:{apply:b,init:null,nullable:true}},members:{__IE:null,__bd:null,__Es:null,__IF:null,_applyDelegate:function(C,B){this._setConfigureItem(C,B);this._setCreateItem(C,B);this._setBindItem(C,B);}
,_applyIconOptions:function(E,D){this.__EC();}
,_applyLabelOptions:function(G,F){this.__EC();}
,_applyTarget:function(I,H){if(H!=undefined){this.__yh(H);}
;if(this.getModel()!=null){this.__II();}
;this._addChangeTargetListener(I,H);}
,_applyModel:function(K,J){this.__II();}
,_applyChildPath:function(M,L){this.__IF=L;this.__II();this.__IF=null;}
,_applyIconPath:function(O,N){this.__EC();}
,_applyLabelPath:function(Q,P){this.__II();}
,__IG:function(R){var T=R.getTarget();var S=this.__IE[T.toHashCode()].treeNode;var U=this.__IE[T.toHashCode()].modelNode;this.__IJ(S,U);this._updateSelection();}
,__IH:function(e){var X=e.getData();var bc=e.getOldData();var V=this.__IE[bc.toHashCode()];bc.removeListenerById(V.changeListenerId);delete this.__IE[bc.toHashCode()];V.modelNode.removeListenerById(V.changeChildernListenerId);var bd=V.modelNode;var bb=qx.Class.getPropertyDefinition(V.modelNode.constructor,this.getChildPath());var Y=bb.event;var ba=bd.addListener(Y,this.__IH,this);var W=V.treeNode;this.__IE[X.toHashCode()]={modelNode:bd,treeNode:W,changeListenerId:V.changeListenerId,changeChildernListenerId:ba};this.__IJ(W,bd);this._updateSelection();}
...
;return be;}
,__II:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__yh();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.__wW(this.getModel(),bg);this.__IJ(bg,this.getModel());this.getTarget().setRoot(bg);}
;}
,__IJ:function(bn,br){if(br[w+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bj=br[w+qx.lang.String.firstUp(this.getChildPath())]();if(this.__IE[bj.toHashCode()]==undefined){var bl=bj.addListener(g,this.__IG,this);var bp=qx.Class.getPropertyDefinition(br.constructor,this.getChildPath());var bm=bp.event;var bq=br.addListener(bm,this.__IH,this);this.__IE[bj.toHashCode()]={modelNode:br,treeNode:bn,changeListenerId:bl,changeChildernListenerId:bq};}
...
;}
else {var bo=this._createItem();bo.setModel(bj.getItem(i));bn.addAt(bo,i);this.__wW(bj.getItem(i),bo);this.__IJ(bo,bj.getItem(i));}
;}
;}
;for(var i=bn.getChildren().length-1;i>=bj.length;i-- ){var bk=bn.getChildren()[i];this.__IL(bk,bn);}
;}
,__yh:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__IK(bv);var bu=bv.getModel();if(bu){this.__IM(bu);}
;bv.destroy();this.__IE={};}
;}
,__IK:function(bw){var bx=bw.getChildren()||[];for(var i=bx.length-1;i>=0;i-- ){if(bx[i].getChildren().length>0){this.__IK(bx[i]);}
;this.__IL(bx[i],bw);}
;}
,__IL:function(bz,bC){var bD=bz.getModel();var by=this.__IF||this.getChildPath();var bA=w+qx.lang.String.firstUp(by);if(bD[bA]!=undefined){var bE=bD[bA]();var bB=this.__IE[bE.toHashCode()];bE.removeListenerById(bB.changeListenerId);bD.removeListenerById(bB.changeChildernListenerId);this.__IK(bz);delete this.__IE[bE.toHashCode()];}
;this.__IM(bD);bC.remove(bz);bz.destroy();}
...
;if(!qx.lang.Array.contains(this.__Es,bJ)){this.__Es.push(bJ);}
;}
...
;if(!qx.lang.Array.contains(this.__Es,bM)){this.__Es.push(bM);}
;}
...
;}
,__EC:function(){var bW;for(var ca in this.__bd){bW=ca;break;}
;for(var bX in this.__bd[bW]){var bV=this.__bd[bW][bX].treeNode;var bY=qx.core.ObjectRegistry.fromHashCode(bX);this.__IM(bY);this.__wW(bY,bV);}
;}
,__wW:function(cd,cb){var cc=this.getDelegate();if(cc!=null&&cc.bindItem!=null){cc.bindItem(this,cb,cd);}
else {this.bindDefaultProperties(cb,cd);}
;}
,__IM:function(cg){for(var i=0;i<this.__Es.length;i++ ){var ce=this.__Es[i];var cf=this.__bd[ce][cg.toHashCode()];if(cf!=null){if(cf.id){cg.removeBinding(cf.id);cf.id=null;}
...
;this._startSelectionModification();this.__yh();this.__II();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cn,cm){if(cn!=null&&cn.bindItem!=null){if(cm!=null&&cm.bindItem!=null&&cn.bindItem==cm.bindItem){return;}
;this.__II();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__bd=this.__IE=this.__Es=null;}
});}
)();
