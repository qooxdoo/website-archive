...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.ListControllerWithObjects","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ListControllerWithObjects.7667aab598db.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="smile",b="Jack",c=".png",d="kiss",e="demobrowser.demo.data.ListControllerWithObjects",f="emote",g="green",h="Jim",j="<b>Displaying objects in a list</b><br/>",k="Make all happy",l="sad",m="execute",n="plain",o="surprise",p="embarrassed",q="online",r=" a converter by the icon. The font color shows the online status.",s="name",t="Write data to log",u="Jackob",v="textColor",w="have a name and an emotion, which is displayed with the help of ",x="This list display a set of persons in a list. Every person does ",y="Dan",z="Tim",A="Karl",B="Online <> Offline",C="icon/16/emotes/face-",D="Dustin",E="angel",F="Max",G="red";qx.Class.define(e,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var I=[F,u,z,b,y,D,A,h];var L=[p,d,n,l,o,E];var U=[];for(var i=0;i<20;i++ ){var N=new demobrowser.demo.data.model.Person();N.setName(I[i%I.length]);N.setEmote(L[i%L.length]);N.setOnline(i%3==0);U.push(N);}
;var S=new qx.data.Array(U);var K=new qx.ui.form.List();K.setWidth(150);this.getRoot().add(K,{left:10,top:80});var R={converter:function(V){return C+V+c;}
};var O=new qx.data.controller.List(S,K,s);O.setIconOptions(R);O.setIconPath(f);var T={converter:function(W){if(W){return g;}
else {return G;}
;}
};var Q={bindItem:function(Y,X,ba){Y.bindDefaultProperties(X,ba);Y.bindProperty(q,v,T,X,ba);}
};O.setDelegate(Q);var J=new qx.ui.form.Button(k);J.setWidth(120);this.getRoot().add(J,{left:180,top:80});J.addListener(m,function(){for(var i=0;i<S.length;i++ ){var bb=S.getItem(i);bb.setEmote(a);}
;}
,this);var H=new qx.ui.form.Button(B);H.setWidth(120);this.getRoot().add(H,{left:180,top:115});H.addListener(m,function(){for(var i=0;i<S.length;i++ ){S.getItem(i).toggleOnline();}
;}
,this);var P=new qx.ui.form.Button(t);P.setWidth(120);this.getRoot().add(P,{left:180,top:150});P.addListener(m,function(){qx.log.appender.Console.show();this.info(S.toString());}
,this);var M=new qx.ui.basic.Label();M.setRich(true);M.setWidth(600);M.setValue(j+x+w+r);this.getRoot().add(M,{left:10,top:10});}
}});}
)();
