...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.UIDeclaration.dce80b27a546.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,addFromDefinition:function(F){var G=this.getRoot();this.__EA(G,F);}
,__EA:function(M,H){var I=new H.clazz();var L=qx.lang.Object.clone(H);delete L.clazz;delete L.layout;delete L.children;delete L.id;delete L.listeners;I.set(L);if(H.id!=null){this[H.id]=I;}
;if(H.layout!=null){var J=new H.layout.clazz();var L=qx.lang.Object.clone(H.layout);delete L.clazz;J.set(L);I.setLayout(J);}
;var N=H.children;if(N!=null){for(var i=0;i<N.length;i++ ){if(I instanceof qx.ui.form.Form){this.__RC(I,N[i]);}
else {this.__EA(I,N[i]);}
;}
;}
...
;}
;}
,__RC:function(Q,O){if(O.type===v){Q.addGroupHeader(O.label);return;}
...
(function(){var a="qx.ui.form.Form",b="";qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__Hm=[];this._buttons=[];this._buttonOptions=[];this._validationManager=new qx.ui.form.validation.Manager();this._resetter=this._createResetter();}
,members:{__Hm:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(e,d,f,name,c,g){if(this.__Hn()){this.__Hm.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
;this.__Hm[this._groupCounter].items.push(e);this.__Hm[this._groupCounter].labels.push(d);this.__Hm[this._groupCounter].options.push(g);if(name==null){name=d.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,b);}
;this.__Hm[this._groupCounter].names.push(name);this._validationManager.add(e,f,c);this._resetter.add(e);}
,addGroupHeader:function(k,h){if(!this.__Hn()){this._groupCounter++ ;}
;this.__Hm.push({title:k,items:[],labels:[],names:[],options:[],headerOptions:h});}
,addButton:function(l,m){this._buttons.push(l);this._buttonOptions.push(m||null);}
,__Hn:function(){return this.__Hm.length===0;}
...
,getGroups:function(){return this.__Hm;}
,getButtons:function(){return this._buttons;}
,getButtonOptions:function(){return this._buttonOptions;}
,getItems:function(){var o={};for(var i=0;i<this.__Hm.length;i++ ){var p=this.__Hm[i];for(var j=0;j<p.names.length;j++ ){var name=p.names[j];o[name]=p.items[j];}
;}
;return o;}
,_createResetter:function(){return new qx.ui.form.Resetter();}
},destruct:function(){this.__Hm=this._buttons=this._buttonOptions=null;this._validationManager.dispose();this._resetter.dispose();}
});}
)();
...
,members:{__Dd:null,__De:null,__gb:null,__Df:null,validate:function(c,e,d,f){this.__Df=false;this.__De=c;this.__gb=d;this.__Dd.call(f||this,this,e);}
,validateForm:function(g,h,i){this.__Df=true;this.__gb=h;this.__Dd.call(i,g,this);}
,setValid:function(k,j){if(this.__Df){if(j!==undefined){this.__gb.setInvalidMessage(j);}
;this.__gb.setFormValid(k);}
else {if(j!==undefined){this.__De.setInvalidMessage(j);}
;this.__gb.setItemValid(this.__De,k);}
;}
},destruct:function(){this.__gb=this.__De=null;}
});}
)();
...
(function(){var a="The given item has not been added.",b="qx.ui.form.Resetter",c=" not supported for reseting.",d="Item ";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__rB=[];}
,members:{__rB:null,add:function(e){if(this._supportsValue(e)){var f=e.getValue();}
else if(this.__zR(e)){var f=e.getSelection();}
else if(this.__zS(e)){var f=e.getSelection().concat();}
else {throw new Error(d+e+c);}
;this.__rB.push({item:e,init:f});}
,reset:function(){for(var i=0;i<this.__rB.length;i++ ){var g=this.__rB[i];this.__zP(g.item,g.init);}
;}
,resetItem:function(h){var j;for(var i=0;i<this.__rB.length;i++ ){var k=this.__rB[i];if(k.item===h){j=k.init;break;}
;}
...
;}
,redefine:function(){for(var i=0;i<this.__rB.length;i++ ){var n=this.__rB[i].item;this.__rB[i].init=this.__zQ(n);}
;}
,redefineItem:function(o){var p;for(var i=0;i<this.__rB.length;i++ ){if(this.__rB[i].item===o){p=this.__rB[i];break;}
;}
...
,events:{"changeValue":l},properties:{dateFormat:{check:c,apply:s},placeholder:{check:r,nullable:true,apply:F},appearance:{refine:true,init:E},focusable:{refine:true,init:true},width:{refine:true,init:120}},statics:{__HX:null,__HY:null,getDefaultDateFormatter:function(){var K=qx.locale.Date.getDateFormat(y).toString();if(K==this.__HX){return this.__HY;}
;if(this.__HY){this.__HY.dispose();}
;this.__HY=new qx.util.format.DateFormat(K,qx.locale.Manager.getInstance().getLocale());this.__HX=K;return this.__HY;}
},members:{__Ia:null,_forwardStates:{focused:true,invalid:true},_setDefaultDateFormat:function(){this.setDateFormat(qx.ui.form.DateField.getDefaultDateFormatter());}
,_addLocaleChangeListener:function(){if(qx.core.Environment.get(x)){this.__Ia=qx.locale.Manager.getInstance().addListener(o,function(){this._setDefaultDateFormat();}
,this);}
;}
...
},destruct:function(){if(qx.core.Environment.get(x)){if(this.__Ia){qx.locale.Manager.getInstance().removeListenerById(this.__Ia);}
;}
;}
});}
)();
...
else {var bL=(bN==null)?-1:bN.getDate();for(var i=0;i<6*7;i++ ){var bO=this.__yt[i];if(bO.hasState(N)){if(bO.hasState(q)){bO.removeState(q);}
;}
...
;this.__ys=this.__yt=this.__yu=null;}
});}
)();
