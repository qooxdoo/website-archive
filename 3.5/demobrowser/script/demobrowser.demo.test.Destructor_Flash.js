...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Destructor_Flash.f5b8d6359eab.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="demobrowser.demo.test.Destructor_Flash",b="created: ",c="300px",d="interval",e="height",f="TestFlash.swf",g="400px",h="width",j="cleaned: ",k="div",m="FlashMovie";qx.Class.define(a,{extend:qx.application.Native,members:{__cM:null,__pN:0,main:function(){qx.application.Native.prototype.main.call(this);this.__cM=[];var n=new qx.event.Timer(5000);n.addListener(d,this.runTest,this);n.start();this.runTest();}
,runTest:function(){for(var i=0;i<5;i++ ){var o=qx.dom.Element.create(k);qx.bom.element.Style.set(o,h,g);qx.bom.element.Style.set(o,e,c);document.body.appendChild(o);var p=m+this.__pN;this.__pN++ ;qx.bom.Flash.create(o,{movie:f,id:p});this.debug(b+p);this.__cM.push(o);}
;var q=qx.event.Timer.once(this.clear,this,2500);q.start();}
,clear:function(){var r=this.__cM;for(var i=0,l=r.length;i<l;i++ ){var s=r[i].firstChild.id;qx.bom.Flash.destroy(r[i]);this.debug(j+s);r[i].parentNode.removeChild(r[i]);}
;r.length=0;}
...
;}
;}
;var N=this.__Fr(O,Q,P,L);this._flashObjects[Q.id]=N;return N;}
,destroy:qx.core.Environment.select(C,{"mshtml":function(S,R){S=this.__Fm(S);if(S.readyState==4){this.__Fn(S);}
else {if(!R){R=window;}
;qx.bom.Event.addNativeListener(R,p,function(){qx.bom.Flash.__Fn(S);}
);}
;}
,"default":function(U,T){U=this.__Fm(U);if(U.parentNode){U.parentNode.removeChild(U);}
;delete this._flashObjects[U.id];}
}),__Fm:function(V){if(!V){throw new Error(y);}
;if(V.tagName.toLowerCase()!==K){V=V.firstChild;}
;if(!V||V.tagName.toLowerCase()!==K){throw new Error(z);}
;return V;}
,__Fn:qx.core.Environment.select(C,{"mshtml":qx.event.GlobalError.observeMethod(function(W){for(var i in W){if(typeof W[i]==b){W[i]=null;}
;}
;if(W.parentNode){W.parentNode.removeChild(W);}
;delete this._flashObjects[W.id];}
),"default":null}),__Fo:qx.event.GlobalError.observeMethod(function(){for(var X in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[X]);}
;window.__Fp=function(){}
;window.__Fq=function(){}
;qx.bom.Event.removeNativeListener(window,E,qx.bom.Flash.__Fo);}
),__Fr:qx.core.Environment.select(C,{"mshtml":function(bb,bd,bc,Y){bc.movie=bd.data;delete bd.data;delete bd.classid;var ba=t;for(name in bc){ba+=m+name+I+bc[name]+D;}
...
})},defer:function(bk){if(qx.core.Environment.get(C)==u){qx.bom.Event.addNativeListener(window,E,bk.__Fo);}
;}
});}
)();
