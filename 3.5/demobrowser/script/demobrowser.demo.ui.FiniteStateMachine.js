...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.eaabb5fbd295.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b=" => ",c="#autoActionsBeforeOnexit",d="__GC",f=": Could not find friendly name for '",g=": event '",h=" blocked.  Re-queuing.",j="string",k="'",l="Attempt to transition to nonexistent state ",m="Saved-state stack is empty",n="Internal error: invalid nextState",o=" not handled.  Ignoring.",p="State ",q="#autoActionsBeforeOntransition",r=" returned a value other than ",s="#",u=" already exists",v="Attempt to transition to POP_STATE_STACK ",w=":",x="Transition ",y=" does not exist",z="' on '",A="qx.util.fsm.FiniteStateMachine",B="String",C="#autoActionsAfterOntransition",D="",E="#actionsAfterOnentry",F="Saved-state stack is full",G="#autoActionsBeforeOnentry",H=": no transition found.  No state change.",I="#autoActionsAfterOnentry",J="#actionsBeforeOnentry",K=": Pushed event: ",L="__GB",M="Machine started with no available states",N=": Process event: ",O="#entry",P="#ontransition",Q=" on ",R="Invalid state: not an instance of ",S="Number",T="#exit",U="true, false, or null.",V="Explicit transition ",W="  ",X="qx.util.fsm.State",Y="while state stack is empty.",bj=".  Ignoring.",bk="#autoActionsAfterOnexit",bl=": Event '",bg="object",bh=" not handled for target ",bi=": Queued event: ";qx.Class.define(A,{extend:qx.core.Object,construct:function(bm){qx.core.Object.call(this);this.setName(bm);this.__jN={};this.__Gz=null;this.__GA=[];this.__GB=[];this.__GC=[];this.__GD={};this.__GE={};this.__GF={};this.__GG={};this.__GH={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:B,nullable:true},state:{check:B,nullable:true},previousState:{check:B,nullable:true},nextState:{check:B,nullable:true},maxSavedStates:{check:S,init:2},debugFlags:{check:S,init:7}},members:{__jN:null,__Gz:null,__GB:null,__GC:null,__GA:null,__GD:null,__GE:null,__GF:null,__GG:null,__GH:null,__GI:false,addState:function(bn){if(!bn instanceof qx.util.fsm.State){throw new Error(R+X);}
;var bo=bn.getName();if(bo in this.__jN){throw new Error(p+bo+u);}
;if(this.__Gz==null){this.__Gz=bo;}
;this.__jN[bo]=bn;}
,replaceState:function(bs,br){if(!bs instanceof qx.util.fsm.State){throw new Error(R+X);}
;var bq=bs.getName();var bp=this.__jN[bq];this.__jN[bq]=bs;if(br){bp._bNeedDispose=true;}
;return bp;}
,addObject:function(bw,bt,bu){var bx=bt.toHashCode();this.__GE[bw]=bx;this.__GF[bx]=bw;this.__GD[bw]=bt;if(!bu){return;}
;if(typeof (bu)==j){bu=[bu];}
;for(var i=0;i<bu.length;i++ ){var bv=bu[i];if(!this.__GG[bv]){this.__GG[bv]={};}
;this.__GG[bv][bw]=true;if(!this.__GH[bw]){this.__GH[bw]=[];}
;this.__GH[bw].push(bv);}
;}
,removeObject:function(bB){var by;var bz;var bA;var bC;by=this.__GE[bB];if(this.__GH[bB]){for(var i=0;i<this.__GH[bB].length;i++ ){bz=this.__GH[bB][i];delete this.__GG[bz][bB];bC=true;for(bA in this.__GG[bz]){bC=false;break;}
;if(bC){delete this.__GG[bz];}
;}
;delete this.__GH[bB];}
;delete this.__GF[by];delete this.__GE[bB];delete this.__GD[bB];}
,getObject:function(bD){return this.__GD[bD];}
,getFriendlyName:function(bE){var bF=bE?bE.toHashCode():null;return bF?this.__GF[bF]:null;}
,getGroupObjects:function(bG){var a=[];for(var name in this.__GG[bG]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bH in this.__GE){var bJ=this.__GE[bH];var bI=this.getObject(bH);this.debug(bH+b+bJ);this.debug(W+bJ+b+this.__GF[bJ]);this.debug(W+bH+b+this.getObject(bH));this.debug(W+this.getObject(bH)+b+this.getFriendlyName(bI));}
;}
,_getInternalData:function(){return ({"states":this.__jN,"startState":this.__Gz,"eventQueue":this.__GB,"blockedEvents":this.__GC,"savedStates":this.__GA,"friendlyToObject":this.__GD,"friendlyToHash":this.__GE,"hashToFriendly":this.__GF,"groupToFriendly":this.__GG,"friendlyToGroups":this.__GH});}
,start:function(){var bL=this.__Gz;if(bL==null){throw new Error(M);}
;this.setState(bL);this.setPreviousState(null);this.setNextState(null);var bK=(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL);if(bK){this.debug(this.getName()+s+bL+J);}
;this.__jN[bL].getAutoActionsBeforeOnentry()(this);if(bK){this.debug(this.getName()+s+bL+O);}
;this.__jN[bL].getOnentry()(this,null);if(bK){this.debug(this.getName()+s+bL+E);}
;this.__jN[bL].getAutoActionsAfterOnentry()(this);}
,pushState:function(bM){if(this.__GA.length>=this.getMaxSavedStates()){throw new Error(F);}
;if(bM===true){this.__GA.push(this.getState());}
else if(bM){this.__GA.push(bM);}
else {this.__GA.push(this.getPreviousState());}
;}
,popState:function(){if(this.__GA.length==0){throw new Error(m);}
;return this.__GA.pop();}
,postponeEvent:function(event){this.__GC.unshift(event);}
,enqueueEvent:function(event,bN){if(bN){this.__GB.push(event);}
else {this.__GB.unshift(event);}
;if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){var bO=this.getFriendlyName(event.getTarget());if(bN){this.debug(this.getName()+K+event.getType()+(bO?Q+bO:D));}
else {this.debug(this.getName()+bi+event.getType()+(bO?Q+bO:D));}
;}
;}
,eventListener:function(event){var e=event.clone();this.enqueueEvent(e,false);this.__GJ();}
,fireImmediateEvent:function(bQ,bR,bP){if(bP){var event=qx.event.Registration.createEvent(bQ,qx.event.type.Data,[bP,null,false]);}
else {var event=qx.event.Registration.createEvent(bQ,qx.event.type.Event,[false,false]);}
;event.setTarget(bR);this.eventListener(event);}
,scheduleEvent:function(bT,bU,bS,bV){qx.event.Timer.once(function(){this.fireImmediateEvent(bT,bU,bS);}
,this,bV||20);}
,__GJ:function(){if(this.__GI){return;}
;this.__GI=true;while(this.__GB.length>0){var event=this.__GB.pop();var bW=this.__GK(event);if(bW){event.dispose();}
;}
;this.__GI=false;}
,__GK:function(event){var ca;var cb;var bY;var cd;var cg;var e;var cf;var bX=this.getDebugFlags();var ck=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var ci=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var ch=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var ce=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var cj=this.getFriendlyName(event.getTarget());if(ck){this.debug(this.getName()+N+event.getType()+(cj?Q+cj:D));}
;ca=this.getState();cd=this.__jN[ca];cg=cd.transitions;e=cd.getEvents()[event.getType()];if(!e){if(ck){this.debug(this.getName()+bl+event.getType()+k+o);}
;return true;}
;if(typeof (e)==bg){if(!cj){if(ce){this.debug(this.getName()+f+event.getType()+z+event.getTarget()+k);}
;return true;}
;cf=e[cj];if(!cf){if(ck){this.debug(this.getName()+bl+event.getType()+k+bh+cj+bj);}
;return true;}
;}
else {cf=e;}
;switch(cf){case qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE:break;case qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED:if(ck){this.debug(this.getName()+bl+event.getType()+k+h);}
;this.__GC.unshift(event);return false;default:if(typeof (cf)==j){if(cg[cf]){var cc=cg[cf];cg={};cg[cf]=cc;}
else {throw new Error(V+cf+y);}
;break;}
;};for(var t in cg){var cc=cg[t];switch(cc.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(x+ca+w+t+r+U);};cb=cc.getNextState();if(typeof (cb)==j){if(!cb in this.__jN){throw new Error(l+cb);}
;this.setNextState(cb);}
else {switch(cb){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cb=ca;this.setNextState(cb);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__GA.length==0){throw new Error(v+Y);}
;cb=this.__GA.pop();this.setNextState(cb);break;default:throw new Error(n);};}
;if(ch){this.debug(this.getName()+s+ca+s+t+q);}
;cc.getAutoActionsBeforeOntransition()(this);if(ch){this.debug(this.getName()+s+ca+s+t+P);}
;cc.getOntransition()(this,event);if(ch){this.debug(this.getName()+s+ca+s+t+C);}
;cc.getAutoActionsAfterOntransition()(this);if(ch){this.debug(this.getName()+s+ca+c);}
;cd.getAutoActionsBeforeOnexit()(this);if(ch){this.debug(this.getName()+s+ca+T);}
;cd.getOnexit()(this,event);if(ch){this.debug(this.getName()+s+ca+bk);}
;cd.getAutoActionsAfterOnexit()(this);if(cd._bNeedDispose){cd.dispose();}
;cd=this.__jN[this.getNextState()];this.setPreviousState(ca);this.setState(this.getNextState());this.setNextState(null);bY=ca;ca=cb;cb=undefined;if(ch){this.debug(this.getName()+s+ca+G);}
;cd.getAutoActionsBeforeOnentry()(this);if(ch){this.debug(this.getName()+s+ca+O);}
;cd.getOnentry()(this,event);if(ch){this.debug(this.getName()+s+ca+I);}
;cd.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__GC.length;i++ ){e=this.__GC.pop();this.__GB.unshift(e);}
;if(ci){this.debug(this.getName()+s+bY+b+this.getName()+s+ca);}
;return true;}
;if(ci){this.debug(this.getName()+s+ca+g+event.getType()+k+H);}
;return true;}
},destruct:function(){this._disposeArray(L);this._disposeArray(d);this.__GA=this.__jN=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="parameters",k="__Gt",l=")",m="__Gr",n=" fsm.getObject('",o="__Gv",p="groups",q="Invalid function parameters: ",r="Invalid onexit type: ",s="  for (var i = 0; i < groupObjects.length; i++)",t="context",u="string",v="autoActionsAfterOnentry",w="__Gx",x="Invalid numeric value in events object: ",y='"',z="Invalid state name",A=" value: ",B="');",C="State ",D="autoActionsBeforeOnexit",E="autoActionsBeforeOnentry",F="}",G="__Gu",H="events must be an object",I="onexit",J="{",K="Invalid function request type: ",L="): ",M=": ",N="expected array, found ",O="Invalid friendly name in 'objects' list: ",P="autoActionsAfterOnexit",Q="  fsm.debug(ex);",R="(",S="__Gq",T="Invalid onentry type: ",U="    fsm.getObject(objName).",V=")(fsm)",W="events",X="undefined",Y="Invalid ",ba="__Gs",bb="try",bc="    var objName = groupObjects[i];",bd="__Gw",be="').",bf="objects",bg="expected object, found ",bh="qx.util.fsm.Transition",bi="The events object must be provided in new state info",bj="catch(ex)",bk="  {",bl="    fsm.getGroupObjects('",bm="Invalid value in events object ",bn="Invalid transition: not an instance of ",bo="  }",bp="Invalid value in events object: ",bq="Invalid 'objects' list: expected array, got ",br="Invalid numeric value in events object ",bs="qx.util.fsm.State",bt="onentry",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(bs,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(t,bE);for(var bC in bF){switch(bC){case bt:this.setOnentry(this.__Gy(bF[bC],bE));break;case I:this.setOnexit(this.__Gy(bF[bC],bE));break;case E:this.setAutoActionsBeforeOnentry(bF[bC]);break;case v:this.setAutoActionsAfterOnentry(bF[bC]);break;case D:this.setAutoActionsBeforeOnexit(bF[bC]);break;case P:this.setAutoActionsAfterOnexit(bF[bC]);break;case W:this.setEvents(bF[bC]);break;case t:break;default:this.setUserData(bC,bF[bC]);this.debug(C+bD+M+bz+bC);break;};}
;if(!this.getEvents()){throw new Error(bi);}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(actionType,value,context){if(typeof (value)!=bB){throw new Error(Y+actionType+A+typeof (value));}
;var funcFragment;var func=bb+J;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(K+N+typeof (functionRequest));}
...
else {if(!params instanceof Array){throw new Error(q+N+typeof (params));}
;}
;funcFragment=f+R;for(var j=0;j<params.length;j++ ){if(j!=0){funcFragment+=by;}
;if(typeof (params[j])==c){funcFragment+=R+params[j]+V;}
else if(typeof (params[j])==u){funcFragment+=y+params[j]+y;}
...
;func+=n+a[j]+be+funcFragment+bu;}
;var g=objectAndGroupList[p];if(g){if(!g instanceof Array){throw new Error(bw+typeof (g));}
;for(j=0;j<g.length;j++ ){func+=d+bl+g[j]+B+s+bk+bc+U+funcFragment+bu+bo;}
;}
;}
;}
;func+=F+bj+J+Q+F;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:S,nullable:true},onentry:{transform:m,nullable:true,init:function(bG,event){}
},onexit:{transform:ba,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:G,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:o,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:bd,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:w,nullable:true,init:function(bL,event){}
},events:{transform:k,nullable:true}},members:{__Gq:function(bM){if(typeof (bM)!=u||bM.length<1){throw new Error(z);}
;return bM;}
,__Gr:function(bN){switch(typeof (bN)){case X:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(t));default:throw new Error(T+typeof (bN));};}
,__Gs:function(bP){switch(typeof (bP)){case X:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(t));default:throw new Error(r+typeof (bP));};}
,__Gt:function(bT){if(typeof (bT)!=bB){throw new Error(H);}
;for(var e in bT){var bR=bT[e];if(typeof (bR)==bA&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(x+e+M+bR);}
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==bA&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(br+R+e+L+bS+M+bR[bS]);}
else if(typeof (bR[bS])!=u&&typeof (bR[bS])!=bA){throw new Error(bm+R+e+L+bS+M+bR[bS]);}
;}
;}
else if(typeof (bR)!=u&&typeof (bR)!=bA){throw new Error(bp+e+M+bT[e]);}
;}
;return bT;}
,__Gu:function(bU){return qx.util.fsm.State._commonTransformAutoActions(E,bU,this.getUserData(t));}
,__Gv:function(bV){return qx.util.fsm.State._commonTransformAutoActions(v,bV,this.getUserData(t));}
,__Gw:function(bW){return qx.util.fsm.State._commonTransformAutoActions(D,bW,this.getUserData(t));}
,__Gx:function(bX){return qx.util.fsm.State._commonTransformAutoActions(P,bX,this.getUserData(t));}
,__Gy:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bn+bh);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="or one of the Fsm.StateChange constants",d="autoActionsAfterOntransition",e="context",g="string",h="predicate",i="__GL",j="Transition ",k="__GN",l=": ",m="String",n="nextState must be an explicit state name, ",o="number",p="Invalid transition nextState type: ",q="Invalid transition nextState value: ",r="Adding user-provided field to transition: ",s="ontransition",t="Invalid transition predicate type: ",u="boolean",v="nextState",w="__GP",x="Invalid ontransition type: ",y="__GM",z="qx.util.fsm.Transition",A="undefined",B="__GO";qx.Class.define(z,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(e,E);for(var C in D){switch(C){case h:this.setPredicate(this.__Gy(D[C],E));break;case v:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__Gy(D[C],E));break;case d:this.setAutoActionsAfterOntransition(this.__Gy(D[C],E));break;case s:this.setOntransition(this.__Gy(D[C],E));break;case e:break;default:this.setUserData(C,D[C]);this.debug(j+F+l+r+C);break;};}
;}
,properties:{name:{check:m,nullable:true},predicate:{init:function(G,event){return true;}
,transform:i},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:y},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:k},autoActionsAfterOntransition:{init:function(I,event){}
,transform:B},ontransition:{init:function(J,event){}
,transform:w}},members:{__GL:function(K){switch(typeof (K)){case A:return function(L,event){return true;}
;case u:return function(M,event){return K;}
;case a:return qx.lang.Function.bind(K,this.getUserData(e));default:throw new Error(t+typeof (K));};}
,__GM:function(N){switch(typeof (N)){case g:return N;case o:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(q+N+l+n+c);};break;default:throw new Error(p+typeof (N));};}
,__GN:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(e));}
,__GO:function(P){return qx.util.fsm.State._commonTransformAutoActions(d,P,this.getUserData(e));}
,__GP:function(Q){switch(typeof (Q)){case A:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(e));default:throw new Error(x+typeof (Q));};}
,__Gy:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
