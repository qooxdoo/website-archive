...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.showcase.Dialog","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.showcase.Dialog.0d66396dae37.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="Last Name",b="Notes",c="male",d="First Name",e="default",f="Cancel",g="Sex",h="Reading",j="Smith",k="Swimming",l="City",m="demobrowser.demo.showcase.Dialog",n="Hobbies",o="icon/22/actions/dialog-apply.png",p="top",q="right",r="John",s="female",t="Page",u="icon/22/actions/dialog-cancel.png",v="Country",w="OK",x="USA",y="New York";qx.Class.define(m,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var K=new qx.ui.layout.Grid(9,5);K.setColumnAlign(0,q,p);K.setColumnAlign(2,q,p);K.setColumnWidth(1,160);K.setColumnWidth(2,72);K.setColumnWidth(3,108);var A=new qx.ui.tabview.TabView;var J=new qx.ui.tabview.Page(t);J.setLayout(K);J.setPadding(10);A.add(J);this.getRoot().add(A,{left:40,top:40});var D=[d,a,l,v,b];for(var i=0;i<D.length;i++ ){J.add(new qx.ui.basic.Label(D[i]).set({allowShrinkX:false,paddingTop:3}),{row:i,column:0});}
;var z=[r,j,y,x];for(var i=0;i<z.length;i++ ){J.add(new qx.ui.form.TextField(z[i]),{row:i,column:1});}
;J.add(new qx.ui.form.TextArea().set({height:250}),{row:4,column:1,colSpan:3});J.add(new qx.ui.basic.Label(g).set({allowShrinkX:false,paddingTop:3}),{row:0,column:2});var F=new qx.ui.form.RadioButton(s);var I=new qx.ui.form.RadioButton(c);var E=new qx.ui.form.RadioGroup();E.add(F,I);J.add(F,{row:0,column:3});J.add(I,{row:1,column:3});I.setValue(true);J.add(new qx.ui.basic.Label(n).set({allowShrinkX:false,paddingTop:3}),{row:2,column:2});J.add(new qx.ui.form.CheckBox(h),{row:2,column:3});J.add(new qx.ui.form.CheckBox(k).set({enabled:false}),{row:3,column:3});var H=new qx.ui.layout.HBox().set({spacing:4,alignX:q});var G=new qx.ui.container.Composite(H).set({paddingTop:11});J.add(G,{row:5,column:0,colSpan:4});var B=new qx.ui.form.Button(w,o);B.addState(e);G.add(B);var C=new qx.ui.form.Button(f,u);G.add(C);}
}});}
)();
...
(function(){var a="bottom",b="button",c="qx.event.type.Data",d="lastTab",f="barRight",g="beforeChangeSelection",h="qx.debug",j="horizontal",k="changeSelection",m="bar",n="close-button",o="Index should be less than : ",p="tabview",q="vertical",r="page",s="Incompatible child for TabView: ",t="close",u="_applyBarPosition",v="barLeft",w="top",x="right",y="qx.ui.tabview.TabView",z="barTop",A="barBottom",B="pane",C="__tc",D="left",E="firstTab";qx.Class.define(y,{extend:qx.ui.core.Widget,implement:qx.ui.core.ISingleSelection,include:[qx.ui.core.MContentPadding],construct:function(G){qx.ui.core.Widget.call(this);this.__Dg={top:z,right:f,bottom:A,left:v};this._createChildControl(m);this._createChildControl(B);var F=this.__tc=new qx.ui.form.RadioGroup;F.setWrap(false);F.addListener(k,this._onChangeSelection,this);if(G!=null){this.setBarPosition(G);}
else {this.initBarPosition();}
;}
,events:{"changeSelection":c},properties:{appearance:{refine:true,init:p},barPosition:{check:[D,x,w,a],init:w,apply:u}},members:{__tc:null,_createChildControlImpl:function(J,I){var H;switch(J){case m:H=new qx.ui.container.SlideBar();H.setZIndex(10);this._add(H);break;case B:H=new qx.ui.container.Stack;H.setZIndex(5);this._add(H,{flex:1});break;};return H||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,J);}
...
;}
;var K=O.getButton();var L=this.getChildControl(m);var N=this.getChildControl(B);O.exclude();L.add(K);N.add(O);this.__tc.add(K);O.addState(this.__Dg[this.getBarPosition()]);O.addState(d);var M=this.getChildren();if(M[0]==O){O.addState(E);}
...
;var R=U.getButton();var Q=this.getChildControl(m);var T=this.getChildControl(B);U.exclude();Q.addAt(R,P);T.addAt(U,P);this.__tc.add(R);U.addState(this.__Dg[this.getBarPosition()]);S=this.getChildren();if(P==S.length-1){U.addState(d);}
;if(S[0]==U){U.addState(E);}
else {S[S.length-2].removeState(d);}
;U.addListener(t,this._onPageClose,this);}
,remove:function(Y){var ba=this.getChildControl(B);var W=this.getChildControl(m);var X=Y.getButton();var bb=ba.getChildren();if(this.getSelection()[0]==Y){var V=bb.indexOf(Y);if(V==0){if(bb[1]){this.setSelection([bb[1]]);}
else {this.resetSelection();}
;}
else {this.setSelection([bb[V-1]]);}
;}
;W.remove(X);ba.remove(Y);this.__tc.remove(X);Y.removeState(this.__Dg[this.getBarPosition()]);if(Y.hasState(E)){Y.removeState(E);if(bb[0]){bb[0].addState(E);}
;}
...
,indexOf:function(bc){return this.getChildControl(B).indexOf(bc);}
,__Dg:null,_applyBarPosition:function(bg,bi){var bd=this.getChildControl(m);var bj=this.getChildControl(B);var bl=bg==D||bg==x;var bm=bg==x||bg==a;var bk=bl?qx.ui.layout.HBox:qx.ui.layout.VBox;var bn=this._getLayout();if(bn&&bn instanceof bk){}
else {this._setLayout(bn=new bk);}
;bn.setReversed(bm);bd.setOrientation(bl?q:j);var bh=this.getChildren();if(bi){var bf=this.__Dg[bi];bd.removeState(bf);bj.removeState(bf);for(var i=0,l=bh.length;i<l;i++ ){bh[i].removeState(bf);}
;}
;if(bg){var be=this.__Dg[bg];bd.addState(be);bj.addState(be);for(var i=0,l=bh.length;i<l;i++ ){bh[i].addState(be);}
;}
;}
,getSelection:function(){var bo=this.__tc.getSelection();var bp=[];for(var i=0;i<bo.length;i++ ){bp.push(bo[i].getUserData(r));}
;return bp;}
,setSelection:function(br){var bq=[];for(var i=0;i<br.length;i++ ){bq.push(br[i].getChildControl(b));}
;this.__tc.setSelection(bq);}
,resetSelection:function(){this.__tc.resetSelection();}
,isSelected:function(bs){var bt=bs.getChildControl(b);return this.__tc.isSelected(bt);}
,isSelectionEmpty:function(){return this.__tc.isSelectionEmpty();}
,getSelectables:function(bv){var bu=this.__tc.getSelectables(bv);var bw=[];for(var i=0;i<bu.length;i++ ){bw.push(bu[i].getUserData(r));}
;return bw;}
,_onChangeSelection:function(e){var bB=this.getChildControl(B);var by=e.getData()[0];var bA=e.getOldData()[0];var bz=[];var bx=[];if(by){bz=[by.getUserData(r)];bB.setSelection(bz);by.focus();this.scrollChildIntoView(by,null,null,false);}
...
},destruct:function(){this._disposeObjects(C);this.__Dg=null;}
});}
)();
...
,__nU:function(e){var m=e.getOldData()[0];var n=e.getData()[0];if(m){if(this.isDynamic()){m.exclude();}
else {m.hide();}
;}
;if(n){n.show();}
;}
...
;var cn=this.getContentElement();if(ci&&this.__ny){if(this.__ny){var ck=this.getInsets();this._getPlaceholderElement().setStyles({paddingTop:ck.top+cp,paddingRight:ck.right+cp,paddingBottom:ck.bottom+cp,paddingLeft:ck.left+cp});}
;}
;if(cm||co.margin){if(this.__ny){var ck=this.getInsets();this._getPlaceholderElement().setStyles({"width":(innerWidth-ck.left-ck.right)+cp,"height":(innerHeight-ck.top-ck.bottom)+cp});}
;cn.setStyles({"width":innerWidth+cp,"height":innerHeight+cp});this._renderContentElement(innerHeight,cn);}
;if(co.position){if(this.__ny){this._getPlaceholderElement().setStyles({"left":cq+cp,"top":top+cp});}
;}
;}
...
;return cs;}
,_applyEnabled:function(cv,cu){qx.ui.core.Widget.prototype._applyEnabled.call(this,cv,cu);this.getContentElement().setEnabled(cv);if(this.__ny){if(cv){this._showPlaceholder();}
else {this._removePlaceholder();}
;}
else {var ct=this.getContentElement();ct.setAttribute(A,cv?this.getPlaceholder():P);}
;}
,__nA:{width:16,height:16},_getContentHint:function(){return {width:this.__nA.width*10,height:this.__nA.height||16};}
...
;if(this.__ny){delete cw[r];this._getPlaceholderElement().setStyles(cw);}
;if(cy){this.__nA=qx.bom.Label.getTextSize(m,cw);}
else {delete this.__nA;}
;qx.ui.core.queue.Layout.add(this);}
...
,_getTextSize:function(){return this.__nA;}
,_onHtmlInput:function(e){var cF=e.getData();var cE=true;this.__nz=false;if(this.__iQ&&this.__iQ===cF){cE=false;}
...
;}
;if(cE){this.fireDataEvent(q,cF,this.__iQ);this.__iQ=cF;if(this.getLiveUpdate()){this.__nB(cF);}
;}
;}
,_onWebFontStatusChange:function(cJ){if(cJ.getData().valid===true){var cI=this.__lr.getStyles();this.__nA=qx.bom.Label.getTextSize(m,cI);qx.ui.core.queue.Layout.add(this);}
;}
,__nB:function(cL){var cK=this.__iP;this.__iP=cL;if(cK!=cL){this.fireNonBubblingEvent(bR,qx.event.type.Data,[cL,cK]);}
;}
,setValue:function(cP){if(cP===null){if(this.__nz){return cP;}
;cP=P;this.__nz=true;}
else {this.__nz=false;if(this.__ny){this._removePlaceholder();}
;}
;if(qx.lang.Type.isString(cP)){var cO=this.getContentElement();if(cO.getValue()!=cP){var cM=cO.getValue();cO.setValue(cP);var cN=this.__nz?null:cP;this.__iP=cM;this.__nB(cN);this.__iQ=this.__iP;}
;if(this.__ny){this._showPlaceholder();}
;return cP;}
;throw new Error(l+cP);}
,getValue:function(){var cQ=this.getContentElement().getValue();return this.__nz?null:cQ;}
,resetValue:function(){this.setValue(null);}
,_onChangeContent:function(e){this.__nz=e.getData()===null;this.__nB(e.getData());}
...
,setLayoutParent:function(parent){qx.ui.core.Widget.prototype.setLayoutParent.call(this,parent);if(this.__ny){if(parent){this.getLayoutParent().getContentElement().add(this._getPlaceholderElement());}
...
.bind(this),0);}
,_removePlaceholder:function(){if(this.hasState(v)){if(this.__ny){this._getPlaceholderElement().setStyle(R,bY);}
;this.removeState(v);}
;}
,_syncPlaceholder:function(){if(this.hasState(v)&&this.__ny){this._getPlaceholderElement().setStyle(R,E);}
;}
...
;if(!this.__ny&&qx.ui.form.AbstractField.__dk){qx.bom.Stylesheet.removeSheet(qx.ui.form.AbstractField.__dk);qx.ui.form.AbstractField.__dk=null;qx.ui.form.AbstractField.__nx();}
;}
,_applyPlaceholder:function(cY,cX){if(this.__ny){this._getPlaceholderElement().setValue(cY);if(cY!=null){this.addListener(u,this._removePlaceholder,this);this.addListener(bK,this._showPlaceholder,this);this._showPlaceholder();}
...
;qx.html.Element.call(this,t,q,r);this.__nC=s;}
...
;}
,setEnabled:function(w){this.__nE=w;this.setAttribute(l,w===false);if(qx.core.Environment.get(a)==g){if(!w){this.setStyles({"userModify":m,"userSelect":d});}
else {this.setStyles({"userModify":null,"userSelect":this.__nD?null:d});}
;}
;}
,setSelectable:qx.core.Environment.select(a,{"webkit":function(x){this.__nD=x;qx.html.Element.prototype.setSelectable.call(this,this.__nE&&x);}
...
,setWrap:function(C,D){if(this.__nC===n){this._setProperty(c,C,D);}
else {throw new Error(e);}
;return this;}
,getWrap:function(){if(this.__nC===n){return this._getProperty(c);}
else {throw new Error(e);}
;}
}});}
)();
...
,properties:{wrap:{check:a,init:true,apply:d},appearance:{refine:true,init:i},singleStep:{check:h,init:20},minimalLineHeight:{check:h,apply:g,init:4},autoSize:{check:a,apply:w,init:false}},members:{__nF:null,__nG:null,__nH:null,setValue:function(M){M=qx.ui.form.AbstractField.prototype.setValue.call(this,M);this.__nI();return M;}
...
;}
,__nI:function(){if(this.isAutoSize()){var R=this.__nJ();if(R&&this.getBounds()){this.__nH=this.__nH||this._getAreaHeight();var T=this._getScrolledAreaHeight();if(this.getMaxHeight()){var P=this.getInsets();var Q=-P.top+this.getMaxHeight()-P.bottom;if(T>Q){this.getContentElement().setStyle(H,j);}
else {this.getContentElement().setStyle(H,G);}
;}
;var S=Math.max(T,this.__nH);this._setAreaHeight(S);}
else {this.getContentElement().addListenerOnce(x,function(){this.__nI();}
,this);}
;}
;}
,_getAreaHeight:function(){return this.getInnerSize().height;}
,_setAreaHeight:function(U){if(this._getAreaHeight()!==U){this.__nG=U;qx.ui.core.queue.Layout.add(this);qx.ui.core.queue.Manager.flush();this.__nM();}
;}
,_getScrolledAreaHeight:function(){var W=this.__nJ();var V=W.getDomElement();if(V){if(!V.parentNode){qx.html.Element.flush();return this._getScrolledAreaHeight();}
...
;this.__nL(W);if(qx.core.Environment.get(C)==B&&qx.core.Environment.get(s)==8){if(!V.scrollTop){qx.html.Element.flush();}
;}
;return V.scrollTop;}
;}
,__nJ:function(){this.__nF=this.__nF||this.__nK();return this.__nF;}
,__nK:function(){var ba,Y,bb,X;ba=this.getContentElement();if(!ba.getDomElement()){return null;}
;bb=qx.bom.Element.clone(ba.getDomElement());X=new qx.html.Input(i);X.useElement(bb);Y=X;Y.setStyles({position:K,top:0,left:n,height:0,overflow:G},true);Y.removeAttribute(F);Y.removeAttribute(z);Y.setAttribute(y,f);Y.setValue(ba.getValue()||t);Y.insertBefore(ba);this.__nL(Y);return Y;}
,__nL:function(bc){bc=bc.getDomElement();if(bc){bc.scrollTop=10000;}
;}
...
,_applyAutoSize:function(bh,bg){if(qx.core.Environment.get(J)){this.__nN();}
;if(bh){this.__nI();this.addListener(o,this.__nI,this);if(!this.getBounds()){this.addListenerOnce(x,function(){this.getContentElement().setStyle(H,G);}
);}
else {this.getContentElement().setStyle(H,G);}
;}
else {this.removeListener(o,this.__nI);this.getContentElement().setStyle(H,j);}
;}
,_applyDimension:function(bi){qx.ui.form.AbstractField.prototype._applyDimension.call(this);if(qx.core.Environment.get(J)){this.__nN();}
;if(bi===this.getMaxHeight()){this.__nI();}
;}
,__nM:function(){var content=this.getContentElement();var bk=content.getDomElement();var bj=content.getStyle(u);content.setStyle(u,parseInt(bj,10)+1000+b,true);if(bk){qx.bom.element.Dimension.getWidth(bk);}
;content.setStyle(u,bj,true);}
,__nN:function(){if(this.isAutoSize()&&this.getHeight()){this.warn(m+I+v);}
;}
,_getContentHint:function(){var bl=qx.ui.form.AbstractField.prototype._getContentHint.call(this);bl.height=bl.height*this.getMinimalLineHeight();bl.width=this._getTextSize().width*20;if(this.isAutoSize()){bl.height=this.__nG||bl.height;}
;return bl;}
},destruct:function(){this.setAutoSize(false);if(this.__nF){this.__nF.dispose();}
;}
});}
)();
