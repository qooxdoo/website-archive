...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Service.be9600f13e64.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="url: ",b="qx.debug.io",c="qx.bom.request.Script",d="Invalid state",e="head",f="error",g="loadend",h="qx.debug",i="script",j="load",k="Unknown response headers",l="browser.documentmode",m="abort",n="",o="Received native readyState: loaded",p="readystatechange",q="Response header cannot be determined for ",r="requests made with script transport.",s="opera",t="unknown",u="Open native request with ",v="Response headers cannot be determined for",w="mshtml",x="engine.name",y="Detected error",z="Send native request",A="on",B="timeout",C="Unknown environment key at this phase",D="Received native load";qx.Bootstrap.define(c,{construct:function(){this.__nl();this.__nc=qx.Bootstrap.bind(this._onNativeLoad,this);this.__nd=qx.Bootstrap.bind(this._onNativeError,this);this.__ne=qx.Bootstrap.bind(this._onTimeout,this);this.__nf=document.head||document.getElementsByTagName(e)[0]||document.documentElement;this._emitter=new qx.event.Emitter();this.timeout=this.__nn()?0:15000;}
,events:{"readystatechange":c,"error":c,"loadend":c,"timeout":c,"abort":c,"load":c},members:{readyState:null,status:null,statusText:null,timeout:null,__ng:null,on:function(name,E,F){this._emitter.on(name,E,F);return this;}
,open:function(H,G){if(this.__nj){return;}
;this.__nl();this.__nh=null;this.__ni=G;if(this.__nq(b)){qx.Bootstrap.debug(qx.bom.request.Script,u+a+G);}
;this._readyStateChange(1);}
,setRequestHeader:function(I,J){if(this.__nj){return null;}
;var K={};if(this.readyState!==1){throw new Error(d);}
;K[I]=J;this.__ni=qx.util.Uri.appendParamsToUrl(this.__ni,K);return this;}
,send:function(){if(this.__nj){return null;}
;var M=this.__no(),L=this.__nf,N=this;if(this.timeout>0){this.__fX=window.setTimeout(this.__ne,this.timeout);}
;if(this.__nq(b)){qx.Bootstrap.debug(qx.bom.request.Script,z);}
;L.insertBefore(M,L.firstChild);window.setTimeout(function(){N._readyStateChange(2);N._readyStateChange(3);}
);return this;}
,abort:function(){if(this.__nj){return null;}
;this.__nh=true;this.__np();this._emit(m);return this;}
...
,onabort:function(){}
,ontimeout:function(){}
,getResponseHeader:function(O){if(this.__nj){return null;}
;if(this.__nq(h)){qx.Bootstrap.debug(q+r);}
;return t;}
,getAllResponseHeaders:function(){if(this.__nj){return null;}
;if(this.__nq(h)){qx.Bootstrap.debug(v+r);}
;return k;}
,setDetermineSuccess:function(P){this.__ng=P;}
,dispose:function(){var Q=this.__nk;if(!this.__nj){if(Q){Q.onload=Q.onreadystatechange=null;this.__np();}
;if(this.__fX){window.clearTimeout(this.__fX);}
;this.__nj=true;}
;}
,isDisposed:function(){return !!this.__nj;}
,_getUrl:function(){return this.__ni;}
,_getScriptElement:function(){return this.__nk;}
,_onTimeout:function(){this.__nm();if(!this.__nn()){this._emit(f);}
;this._emit(B);if(!this.__nn()){this._emit(g);}
;}
,_onNativeLoad:function(){var S=this.__nk,R=this.__ng,T=this;if(this.__nh){return;}
;if(this.__nq(x)===w&&this.__nq(l)<9){if(!(/loaded|complete/).test(S.readyState)){return;}
else {if(this.__nq(b)){qx.Bootstrap.debug(qx.bom.request.Script,o);}
;}
;}
;if(this.__nq(b)){qx.Bootstrap.debug(qx.bom.request.Script,D);}
;if(R){if(!this.status){this.status=R()?200:500;}
;}
;if(this.status===500){if(this.__nq(b)){qx.Bootstrap.debug(qx.bom.request.Script,y);}
;}
;if(this.__fX){window.clearTimeout(this.__fX);}
;window.setTimeout(function(){T._success();T._readyStateChange(4);T._emit(j);T._emit(g);}
);}
,_onNativeError:function(){this.__nm();this._emit(f);this._emit(g);}
,__nk:null,__nf:null,__ni:n,__nc:null,__nd:null,__ne:null,__fX:null,__nh:null,__nj:null,__nl:function(){this.readyState=0;this.status=0;this.statusText=n;}
,_readyStateChange:function(U){this.readyState=U;this._emit(p);}
,_success:function(){this.__np();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=n+this.status;}
,__nm:function(){this.__np();this.readyState=4;this.status=0;this.statusText=null;}
,__nn:function(){var W=this.__nq(x)===w&&this.__nq(l)<9;var V=this.__nq(x)===s;return !(W||V);}
,__no:function(){var X=this.__nk=document.createElement(i);X.src=this.__ni;X.onerror=this.__nd;X.onload=this.__nc;if(this.__nq(x)===w&&this.__nq(l)<9){X.onreadystatechange=this.__nc;}
;return X;}
,__np:function(){var Y=this.__nk;if(Y&&Y.parentNode){this.__nf.removeChild(Y);}
;}
,__nq:function(ba){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(ba);}
...
(function(){var a="qx.bom.request.Jsonp",b="callback",c="open",d="dispose",e="",f="qx",g="qx.debug",h="Expecting JavaScript response to call: ",i="_onNativeLoad",j=" already exists",k="Callback ",l="qx.debug.io",m=".callback",n="qx.bom.request.Jsonp.";qx.Bootstrap.define(a,{extend:qx.bom.request.Script,construct:function(){qx.bom.request.Script.apply(this);this.__yC();}
,members:{responseJson:null,__ci:null,__yv:null,__yw:null,__yx:null,__yy:null,__yz:null,__nj:null,__yA:e,open:function(t,p){if(this.__nj){return;}
;var r={},q,s,o=this;this.responseJson=null;this.__yx=false;q=this.__yv||b;s=this.__yw||this.__yA+n+this.__ci+m;if(!this.__yw){this.constructor[this.__ci]=this;}
else {if(!window[this.__yw]){this.__yy=true;window[this.__yw]=function(u){o.callback(u);}
;}
else {if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,k+this.__yw+j);}
;}
;}
;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,h+s);}
;r[q]=s;this.__yz=p=qx.util.Uri.appendParamsToUrl(p,r);this.__hG(c,[t,p]);}
,callback:function(v){if(this.__nj){return;}
;this.__yx=true;if(qx.core.Environment.get(g)){v=qx.lang.Json.stringify(v);v=qx.lang.Json.parse(v);}
;this.responseJson=v;this.constructor[this.__ci]=undefined;this.__yB();}
,setCallbackParam:function(w){this.__yv=w;return this;}
,setCallbackName:function(name){this.__yw=name;return this;}
,setPrefix:function(x){this.__yA=x;}
,getGeneratedUrl:function(){return this.__yz;}
,dispose:function(){this.__yB();this.__hG(d);}
,_onNativeLoad:function(){this.status=this.__yx?200:500;this.__hG(i);}
,__yB:function(){if(this.__yy&&window[this.__yw]){window[this.__yw]=undefined;this.__yy=false;}
;}
,__hG:function(z,y){qx.bom.request.Script.prototype[z].apply(this,y||[]);}
,__yC:function(){this.__ci=f+(new Date().valueOf())+(e+Math.random()).substring(2,5);}
}});}
)();
...
(function(){var a="changeModel",b="qx.core.Object",c="qx.data.controller.Object",d="get",e="[",f="reset",g="_applyModel",h="last";qx.Class.define(c,{extend:qx.core.Object,construct:function(j){qx.core.Object.call(this);this.__bd={};this.__gJ=[];if(j!=null){this.setModel(j);}
;}
,properties:{model:{check:b,event:a,apply:g,nullable:true,dereference:true}},members:{__gJ:null,__bd:null,_applyModel:function(o,l){for(var i=0;i<this.__gJ.length;i++ ){var t=this.__gJ[i][0];var q=this.__gJ[i][1];var n=this.__gJ[i][2];var r=this.__gJ[i][3];var p=this.__gJ[i][4];var m=this.__gJ[i][5];if(l!=undefined&&!l.isDisposed()){this.__FN(t,q,n,l);}
;if(o!=undefined){this.__FM(t,q,n,r,p,m);}
...
;}
;}
,addTarget:function(z,w,u,x,y,v){this.__gJ.push([z,w,u,x,y,v]);this.__FM(z,w,u,x,y,v);}
,__FM:function(F,D,B,G,E,A){if(this.getModel()==null){return;}
...
,removeTarget:function(K,J,L){this.__FN(K,J,L,this.getModel());for(var i=0;i<this.__gJ.length;i++ ){if(this.__gJ[i][0]==K&&this.__gJ[i][1]==J&&this.__gJ[i][2]==L){this.__gJ.splice(i,1);}
;}
;}
,__FN:function(R,O,N,M){if(!(R instanceof qx.core.Object)){return;}
