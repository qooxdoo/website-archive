...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Github.33883f853942.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
},members:{_resource:null,__GV:null,__GW:null,__GX:null,_getResource:function(v){return new qx.bom.rest.Resource(v);}
...
,map:function(G,I,H,J){this.__GY(G,I,H,J);this._resource.map(G,I,H,J);}
,__GY:function(L,N,K,M){this.__GX[L]=[N,K,M];if(L==m){this[L]=undefined;}
;if(typeof this[L]!==q&&this[L]!==null&&this[L].action!==true){throw new Error(o+L+c);}
;this.__Hb(L+p);this.__Hb(L+f);this[L]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,L);return this.invoke.apply(this,arguments);}
,this);this[L].action=true;}
,invoke:function(O,P,Q){var P=(P==null)?{}:P;this.__GX[O].params=P;return this._resource.invoke(O,P,Q);}
...
,poll:function(Y,U,X,ba){if(this.__GW[Y]){this.__GW[Y].dispose();}
;if(typeof X==q){X=this.__GX[Y].params;}
...
;}
;var V=this.__GW[Y]=new qx.event.Timer(U);V.addListener(i,W,this._resource);V.start();return V;}
...
;if(!bj){bi=0;}
;return false;}
;var bf=this.__GV[bh]=this.addListener(bh+p,function bk(){if(be.isDisposed()){return;}
...
,__Ha:function(bn){Object.keys(bn).forEach(function(bp){var bq=bn[bp],bs=bq.method,bo=bq.url,br=bq.check;if(qx.core.Environment.get(s)){qx.core.Assert.assertString(bs,a+bp+b);qx.core.Assert.assertString(bo,n+bp+b);}
;this.map(bp,bs,bo,br);}
,this);}
,__Hb:function(bt){if(!this.constructor.$$events){this.constructor.$$events={};}
;if(!this.constructor.$$events[bt]){this.constructor.$$events[bt]=j;}
;}
},destruct:function(){var bu;if(this.__GW){for(bu in this.__GW){var bw=this.__GW[bu];bw.stop();bw.dispose();}
;}
;if(this.__GV){for(bu in this.__GV){var bv=this.__GV[bu];this.removeListenerById(bv);}
;}
;this._resource.destruct();this._resource=this.__GX=this.__GW=this.__GV=null;}
});}
)();
...
;return S;}
},members:{__Hc:null,__GX:null,__Hd:null,__GW:null,__GV:null,__He:null,__Hf:null,__Hg:null,setRequestFactory:function(V){this.__Hg=V;}
,setRequestHandler:function(W){this.__Hf=W;}
,_getRequestHandler:function(){return (this.__Hf===null)?{onsuccess:{callback:function(Y,X){return function(){var ba={"id":parseInt(Y.toHashCode(),10),"response":Y.getResponse(),"request":Y,"action":X};this.emit(X+G,ba);this.emit(w,ba);}
;}
...
;}
,context:this}}:this.__Hf;}
,getRequestsByAction:function(bg){var bh=(this.__Hc!==null&&bg in this.__Hc);return bh?this.__Hc[bg]:null;}
,configureRequest:function(bi){this.__He=bi;}
,_getRequest:function(){return (this.__Hg===null)?new qx.bom.request.SimpleXhr():this.__Hg();}
,__Hh:function(bj){var bk=this._getRequest();if(!qx.lang.Type.isArray(this.__Hc[bj])){this.__Hc[bj]=[];}
;this.__Hc[bj].push(bk);return bk;}
,map:function(bm,bo,bl,bn){this.__GX[bm]=[bo,bl,bn];this.__Hc[bm]=[];if(bm==u){this[bm]=undefined;}
;if(typeof this[bm]!==N&&this[bm]!==null&&this[bm].action!==true){throw new Error(m+bm+y);}
;this.__Hb(bm+G);this.__Hb(bm+z);this[bm]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,bm);return this.invoke.apply(this,arguments);}
,this);this[bm].action=true;}
,invoke:function(bt,br,bp){var bq=this.__Hh(bt),br=br==null?{}:br,bu=this._getRequestConfig(bt,br);this.__GX[bt].params=br;this.__Hi(br,bu.check);this.__zp(bq,bu,bp);if(this.__He){this.__He.call(this,bq,bt,br,bp);}
;this.__Hj(bq,bu,bp);var bs=this._getRequestHandler();bq.addListenerOnce(w,bs.onsuccess.callback(bq,bt),bs.onsuccess.context);bq.addListenerOnce(s,bs.onfail.callback(bq,bt),bs.onfail.context);bq.addListenerOnce(g,bs.onloadend.callback(bq,bt),bs.onloadend.context);bq.send();return parseInt(bq.toHashCode(),10);}
,setBaseUrl:function(bv){this.__Hd=bv;}
,__Hi:function(bw,bx){if(typeof bx!==N){if(qx.core.Environment.get(r)){qx.core.Assert.assertObject(bx,M);}
...
;}
,__zp:function(bA,bz,bB){bA.setUrl(bz.url);if(!bA.setMethod&&bz.method!==j){throw new Error(D+bA.classname+x);}
;if(bA.setMethod){bA.setMethod(bz.method);}
;if(bB){bA.setRequestData(bB);}
;}
,__Hj:function(bE,bC,bD){if(bD){var bF=bE.getRequestHeader(K);if(bE.getMethod&&qx.util.Request.methodAllowsRequestBody(bE.getMethod())){if((/application\/.*\+?json/).test(bF)){bD=qx.lang.Json.stringify(bD);bE.setRequestData(bD);}
;}
;}
;}
...
;}
else {var bJ=bI;var bH=this.__Hc[bJ];if(this.__Hc[bJ]){bH.forEach(function(bM){bM.abort();}
);}
;}
;}
,refresh:function(bN){this.invoke(bN,this.__GX[bN].params);}
,poll:function(bO,bQ,bR,bP){if(this.__GW[bO]){this.stopPollByAction(bO);}
;if(typeof bR==N){bR=this.__GX[bO].params;}
;if(bP){this.invoke(bO,bR);}
;var bS=(function(bT){return function(){var bU=bT.__Hc[bO][0];if(!bP&&!bU){bT.invoke(bO,bR);return;}
;if(bU.isDone()||bU.isDisposed()){bT.refresh(bO);}
;}
;}
)(this);this._startPoll(bO,bS,bQ);}
,_startPoll:function(bV,bW,bX){this.__GW[bV]={"id":window.setInterval(bW,bX),"interval":bX,"listener":bW};}
,stopPollByAction:function(bY){if(bY in this.__GW){var ca=this.__GW[bY].id;window.clearInterval(ca);}
;}
,restartPollByAction:function(cb){if(cb in this.__GW){var cc=this.__GW[cb];this.stopPollByAction(cb);this._startPoll(cb,cc.listener,cc.interval);}
;}
...
;if(!cj){ci=0;}
;return false;}
;var cf=this.__GV[ch]=this.addListener(ch+G,function ck(){if(ce.isDisposed()){return;}
;if(!cd()){cg=new Date();ce.refresh(ch);}
;}
);this.invoke(ch);return cf;}
,_getRequestConfig:function(cp,co){var cm=this.__GX[cp];var co=qx.lang.Object.clone(co);if(!qx.lang.Type.isArray(cm)){throw new Error(P+cp);}
;var cn=cm[0],cl=this.__Hd!==null?this.__Hd+cm[1]:cm[1],cr=cm[2],cq=qx.bom.rest.Resource.placeholdersFromUrl(cl);co=co||{};cq.forEach(function(cu){var cs=new RegExp(v+cu+c),ct=cl.match(cs)[1];if(typeof co[cu]===N){if(ct){co[cu]=ct;}
else {co[cu]=A;}
;}
...
,__Ha:function(cv){Object.keys(cv).forEach(function(cx){var cy=cv[cx],cA=cy.method,cw=cy.url,cz=cy.check;if(qx.core.Environment.get(r)){qx.core.Assert.assertString(cA,I+cx+o);qx.core.Assert.assertString(cw,l+cx+o);}
;this.map(cx,cA,cw,cz);}
,this);}
,__Hb:function(cB){if(!this.constructor.$$events){this.constructor.$$events={};}
...
;}
;}
,destruct:function(){var cG;for(cG in this.__Hc){if(this.__Hc[cG]){this.__Hc[cG].forEach(function(cI){cI.dispose();}
);}
;}
;if(this.__GW){for(cG in this.__GW){this.stopPollByAction(cG);}
;}
;if(this.__GV){for(cG in this.__GV){var cH=this.__GV[cG];this.removeListenerById(cH);}
;}
;this.__Hc=this.__GX=this.__GW=null;}
}});}
)();
...
;this.useCaching(true);this.setMethod((v!==undefined)?v:m);this._transport=this._registerTransportListener(this._createTransport());qx.core.ObjectRegistry.register(this);this.__rC={};this.__sk=this._createResponseParser();}
,members:{setRequestHeader:function(w,x){this.__rC[w]=x;return this;}
,getRequestHeader:function(y){return this.__rC[y];}
,setUrl:function(z){if(qx.lang.Type.isString(z)){this.__ni=z;}
;return this;}
,getUrl:function(){return this.__ni;}
,setMethod:function(A){if(qx.util.Request.isMethod(A)){this.__Hk=A;}
;return this;}
,getMethod:function(){return this.__Hk;}
,setRequestData:function(B){if(qx.lang.Type.isString(B)||qx.lang.Type.isObject(B)){this.__Hl=B;}
;return this;}
,getRequestData:function(){return this.__Hl;}
,getResponse:function(){if(this.__rI!==null){return this.__rI;}
...
,setParser:function(C){return this.__sk.setParser(C);}
,setTimeout:function(D){if(qx.lang.Type.isNumber(D)){this.__ga=D;}
;return this;}
,getTimeout:function(){return this.__ga;}
...
,send:function(){var K=this.getTimeout(),H=(this.getRequestData()!==null),I=this.__rC.hasOwnProperty(j),F=qx.util.Request.methodAllowsRequestBody(this.getMethod()),L=this.getRequestHeader(k),G=this._serializeData(this.getRequestData(),L);if(this.getMethod()===m&&H){this.setUrl(qx.util.Uri.appendParamsToUrl(this.getUrl(),G));}
...
;this._transport.open(this.getMethod(),this.getUrl(),true);for(var J in this.__rC){this._transport.setRequestHeader(J,this.__rC[J]);}
...
,dispose:function(){if(this._transport.dispose()){this.__sk=null;this.__nj=true;return true;}
;return false;}
...
,_setResponse:function(N){this.__rI=N;}
...
;return null;}
,__rC:null,__Hl:null,__Hk:d,__ni:d,__rI:null,__sk:null,__d:null,__ga:null,__nj:null,addListenerOnce:function(name,S,T){this._transport._emitter.once(name,S,T);return this;}
...
;}
,__rM:function(){if(qx.core.Environment.get(n)){qx.Bootstrap.debug(f+this._transport.status);}
...
;this._setResponse(this.__sk.parse(V,U));this._transport._emit(o);}
else {try{this._setResponse(this.__sk.parse(V,U));}
catch(e){}
...
(function(){var a="changeModel",b="model.user.avatar_url",c="application/json",d="github.view.Gist",e="model.description",f="west",g="GET",h="/gists/{id}",i="Description",j="https://api.github.com",k="demobrowser.demo.data.Github",l="source",m="/gists",n="change",o="Content",p="center",q="value",r="get",s="#fff",t="Authorization",u="html",v="",w="model.user.login",x="model.files",y="north",z="Gists",A="Accept",B="token ",C="bold",D="model",E="github.rest.Resource",F="description",G="undefined",H="</pre>",I="<pre>";qx.Class.define(k,{extend:qx.application.Standalone,members:{__OR:null,__OS:null,__OT:null,__OU:null,__JK:null,__OV:null,main:function(){if(typeof GITHUB==G){return;}
;qx.application.Standalone.prototype.main.call(this);this._setUpResources();this._setUpStores();this._createGui();this._setUpBinding();this.__OR.get();this.__JK.addListener(a,function(K){var J=K.getData();this.__JK.getSelection().push(J.getItem(0));}
,this);this.__JK.getSelection().addListener(n,function(L){var M=this.__JK.getSelection().getItem(0).getId();this.__OS.get({id:M});}
,this);}
,_setUpResources:function(){this.__OR=new github.rest.Resource({"get":{method:g,url:m}});this.__OS=new github.rest.Resource({"get":{method:g,url:h}});}
,_setUpStores:function(){this.__OT=new qx.data.store.Rest(this.__OR,r);this.__OU=new qx.data.store.Rest(this.__OS,r);}
,_createGui:function(){var N=new qx.ui.container.Composite(new qx.ui.layout.Dock());N.setPadding(10);var O=new qx.ui.basic.Label(z);O.setFont(C);O.setPaddingBottom(10);N.add(O,{edge:y});this.__JK=new qx.ui.list.List();this.__JK.setWidth(200);this.__OV=new github.view.Gist();N.add(this.__JK,{edge:f});N.add(this.__OV,{edge:p});this.getRoot().add(N,{edge:0});}
,_setUpBinding:function(){var Q=this.__JK,P=this.__OT,R=this.__OU;Q.setLabelPath(F);Q.setLabelOptions({converter:function(W,U,T,V){if(W===null||!W.length){return U.getId();}
;return W;}
});P.bind(D,Q,D);var S=this.__OV;R.bind(e,S.getDescription(),q);R.bind(w,S.getUsername(),q);R.bind(b,S.getGravatar(),l);R.bind(x,S.getContent(),u,{converter:function(Y){var X=qx.Class.getProperties(Y.constructor);var content=Y.get(X[0]).getContent();content=qx.bom.String.escape(content);return I+content+H;}
});}
...
});qx.Class.define(d,{extend:qx.ui.container.Composite,construct:function(){qx.ui.container.Composite.call(this);var bc=new qx.ui.layout.Grid(5,5);bc.setColumnFlex(0,1);bc.setRowFlex(2,1);this.setLayout(bc);this.setPadding(0,10);var bd;var bf=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));bd=new qx.ui.basic.Label(i);bd.setFont(C);this.__mB=new qx.ui.basic.Label();bf.add(bd);bf.add(this.__mB);this.add(bf,{row:0,column:0});var be=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));this.__OW=new qx.ui.basic.Image();this.__OW.set({width:40,height:40,scale:true,backgroundColor:s});this.__OX=new qx.ui.basic.Label();be.add(this.__OW);be.add(this.__OX);this.add(be,{row:0,column:1});bd=new qx.ui.basic.Label(o);bd.setFont(C);var scroll=new qx.ui.container.Scroll();this.__pj=new qx.ui.embed.Html();this.__pj.setMinHeight(1000);scroll.add(this.__pj);this.add(bd,{row:1,column:0});this.add(scroll,{row:2,column:0,colSpan:2});}
,members:{__mB:v,__OW:v,__OX:v,__pj:v,getDescription:function(){return this.__mB;}
,getGravatar:function(){return this.__OW;}
,getUsername:function(){return this.__OX;}
,getContent:function(){return this.__pj;}
}});}
)();
...
;this._delegate=j;this._marshaler=new qx.data.marshal.Json(j);if(j&&qx.lang.Type.isFunction(j.configureRequest)){this.__zp();}
;this.__zo=qx.lang.Function.bind(this.__zr,this);this.__zq();}
,properties:{resource:{check:c},actionName:{check:f},model:{nullable:true,event:a}},members:{_marshaler:null,_delegate:null,__zo:null,__zp:function(){var k=this.getResource(),l=this._delegate;k.configureRequest(l.configureRequest);}
,__zq:function(){var n=this.getResource(),m=this.getActionName();if(n&&m){n.addListener(this.getActionName()+d,this.__zo);}
;}
,__zr:function(e){var r=e.getData(),q=this._marshaler,s,o=this.getModel(),p=this._delegate;if(r){if(p&&p.manipulateData){r=p.manipulateData(r);}
...
(function(){var a="scrollY",b="The method 'getItemRight' is not implemented!",c="update",d="scrollX",f="The method 'getItemLeft' is not implemented!",g="The method 'getItemBottom' is not implemented!",h="The method 'getItemTop' is not implemented!",i="pane",j="os.scrollBarOverlayed",k="qx.ui.virtual.core.Scroller";qx.Class.define(k,{extend:qx.ui.core.scroll.AbstractScrollArea,construct:function(o,l,n,m){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__xo=new qx.ui.virtual.core.Pane(o,l,n,m);this.__xo.addListener(c,this._computeScrollbars,this);this.__xo.addListener(d,this._onScrollPaneX,this);this.__xo.addListener(a,this._onScrollPaneY,this);if(qx.core.Environment.get(j)){this._add(this.__xo,{edge:0});}
else {this._add(this.__xo,{row:0,column:0});}
;}
,properties:{width:{refine:true,init:null},height:{refine:true,init:null}},members:{__xo:null,getPane:function(){return this.__xo;}
,_createChildControlImpl:function(q,p){if(q==i){return this.__xo;}
...
,_onScrollBarX:function(e){this.__xo.setScrollX(e.getData());}
,_onScrollBarY:function(e){this.__xo.setScrollY(e.getData());}
},destruct:function(){this.__xo.dispose();this.__xo=null;}
});}
)();
(function(){var a="scrollY",b="mousedown",c="qx.event.type.Data",d="dblclick",f="change",g="qx.debug",h="__xp",j="qx.event.type.Event",k="cellContextmenu",l="resize",m="scrollX",n="appear",o="qx.ui.virtual.core.CellEvent",p="cellDblclick",q="qx.ui.virtual.core.Pane",r="click",s="update",t="cellClick",u="contextmenu",v="__xu",w="__xq",x="__xv";qx.Class.define(q,{extend:qx.ui.core.Widget,construct:function(B,y,A,z){qx.ui.core.Widget.call(this);this.__xp=new qx.ui.virtual.core.Axis(A,B);this.__xq=new qx.ui.virtual.core.Axis(z,y);this.__pw=0;this.__pv=0;this.__xr=0;this.__xs=0;this.__xt={};this.__jV={};this.__xu=new qx.ui.container.Composite();this.__xu.setUserBounds(0,0,0,0);this._add(this.__xu);this.__xv=[];this.__xp.addListener(f,this.fullUpdate,this);this.__xq.addListener(f,this.fullUpdate,this);this.addListener(l,this._onResize,this);this.addListenerOnce(n,this._onAppear,this);this.addListener(b,this._onMouseDown,this);this.addListener(r,this._onClick,this);this.addListener(d,this._onDblclick,this);this.addListener(u,this._onContextmenu,this);}
,events:{cellClick:o,cellContextmenu:o,cellDblclick:o,update:j,scrollX:c,scrollY:c},properties:{width:{refine:true,init:400},height:{refine:true,init:300}},members:{__xp:null,__xq:null,__pw:null,__pv:null,__xr:null,__xs:null,__xt:null,__jV:null,__xu:null,__xv:null,__xw:null,__sP:null,__tU:null,__xx:null,getRowConfig:function(){return this.__xp;}
,getColumnConfig:function(){return this.__xq;}
,getChildren:function(){return [this.__xu];}
,addLayer:function(C){if(qx.core.Environment.get(g)){this.assertInterface(C,qx.ui.virtual.core.ILayer);}
;this.__xv.push(C);C.setUserBounds(0,0,0,0);this.__xu.add(C);}
,getLayers:function(){return this.__xv;}
,getVisibleLayers:function(){var D=[];for(var i=0;i<this.__xv.length;i++ ){var E=this.__xv[i];if(E.isVisible()){D.push(E);}
;}
;return D;}
,getScrollMaxX:function(){var F=this.getInnerSize();if(F){return Math.max(0,this.__xq.getTotalSize()-F.width);}
;return 0;}
,getScrollMaxY:function(){var G=this.getInnerSize();if(G){return Math.max(0,this.__xp.getTotalSize()-G.height);}
;return 0;}
,setScrollY:function(J){var H=this.getScrollMaxY();if(J<0){J=0;}
else if(J>H){J=H;}
;if(this.__pw!==J){var I=this.__pw;this.__pw=J;this._deferredUpdateScrollPosition();this.fireDataEvent(a,J,I);}
;}
,getScrollY:function(){return this.__pw;}
,setScrollX:function(M){var K=this.getScrollMaxX();if(M<0){M=0;}
else if(M>K){M=K;}
;if(M!==this.__pv){var L=this.__pv;this.__pv=M;this._deferredUpdateScrollPosition();this.fireDataEvent(m,M,L);}
;}
,getScrollX:function(){return this.__pv;}
,getScrollSize:function(){return {width:this.__xq.getTotalSize(),height:this.__xp.getTotalSize()};}
,scrollRowIntoView:function(P){var Q=this.getBounds();if(!Q){this.addListenerOnce(n,function(){qx.event.Timer.once(function(){this.scrollRowIntoView(P);}
,this,0);}
,this);return;}
;var R=this.__xp.getItemPosition(P);var O=R+this.__xp.getItemSize(P);var N=this.getScrollY();if(R<N){this.setScrollY(R);}
...
,this,0);}
,this);return;}
;var U=this.__xq.getItemPosition(S);var T=U+this.__xq.getItemSize(S);var W=this.getScrollX();if(U<W){this.setScrollX(U);}
else if(T>W+V.width){this.setScrollX(T-V.width);}
;}
,scrollCellIntoView:function(X,ba){var Y=this.getBounds();if(!Y){this.addListenerOnce(n,function(){qx.event.Timer.once(function(){this.scrollCellIntoView(X,ba);}
,this,0);}
,this);return;}
...
;bf=this.__xp.getItemAtPosition(this.getScrollY()+bc-bd.top);be=this.__xq.getItemAtPosition(this.getScrollX()+bb-bd.left);if(!bf||!be){return null;}
...
;var bn=this.__pv+bl.width;var bo=this.__xs-bn;if(this.__pv-this.__xt.left<Math.min(this.__pv,bj)||this.__xt.right-bn<Math.min(bo,bk)){var bp=Math.min(this.__pv,bm);var bh=Math.min(bo,bi);this._setLayerWindow(bg,this.__pv-bp,this.__pw,bl.width+bp+bh,bl.height,false);}
;}
,prefetchY:function(bz,bv,br,bu){var bq=this.getVisibleLayers();if(bq.length==0){return;}
;var bw=this.getBounds();if(!bw){return;}
;var bs=this.__pw+bw.height;var bt=this.__xr-bs;if(this.__pw-this.__xt.top<Math.min(this.__pw,bz)||this.__xt.bottom-bs<Math.min(bt,br)){var by=Math.min(this.__pw,bv);var bx=Math.min(bt,bu);this._setLayerWindow(bq,this.__pv,this.__pw-by,bw.width,bw.height+by+bx,false);}
;}
,_onResize:function(){if(this.getContentElement().getDomElement()){this.__xw=true;this._updateScrollPosition();this.__xw=null;this.fireEvent(s);}
;}
,_onAppear:function(){this.fullUpdate();}
,_onMouseDown:function(e){this.__xx=this.getCellAtPosition(e.getDocumentLeft(),e.getDocumentTop());}
,_onClick:function(e){this.__xy(e,t);}
,_onContextmenu:function(e){this.__xy(e,k);}
,_onDblclick:function(e){this.__xy(e,p);}
,__xy:function(e,bC){var bB=this.getCellAtPosition(e.getDocumentLeft(),e.getDocumentTop());if(!bB){return;}
;var bA=this.__xx;if(bA==null||bA.row!==bB.row||bA.column!==bB.column){return;}
;this.fireNonBubblingEvent(bC,qx.ui.virtual.core.CellEvent,[this,e,bB.row,bB.column]);}
,syncWidget:function(bD){if(this.__jV._fullUpdate){this._fullUpdate();}
else if(this.__jV._updateScrollPosition){this._updateScrollPosition();}
;this.__jV={};}
,_setLayerWindow:function(bE,bT,top,bP,bL,bV){var bK=this.__xp.getItemAtPosition(top);if(bK){var bN=bK.index;var bR=this.__xp.getItemSizes(bN,bL+bK.offset);var bM=qx.lang.Array.sum(bR);var bU=top-bK.offset;var bQ=top-bK.offset+bM;}
else {var bN=0;var bR=[];var bM=0;var bU=0;var bQ=0;}
;var bF=this.__xq.getItemAtPosition(bT);if(bF){var bJ=bF.index;var bH=this.__xq.getItemSizes(bJ,bP+bF.offset);var bO=qx.lang.Array.sum(bH);var bS=bT-bF.offset;var bI=bT-bF.offset+bO;}
else {var bJ=0;var bH=[];var bO=0;var bS=0;var bI=0;}
;this.__xt={top:bU,bottom:bQ,left:bS,right:bI};this.__xu.setUserBounds((this.getPaddingLeft()||0)+(this.__xt.left-this.__pv),(this.getPaddingTop()||0)+(this.__xt.top-this.__pw),bO,bM);this.__sP=bH;this.__tU=bR;for(var i=0;i<this.__xv.length;i++ ){var bG=this.__xv[i];bG.setUserBounds(0,0,bO,bM);if(bV){bG.fullUpdate(bN,bJ,bR,bH);}
else {bG.updateLayerWindow(bN,bJ,bR,bH);}
;}
;}
,__xz:function(){if(this.__xw){return;}
;var bW=this.getScrollSize();if(this.__xr!==bW.height||this.__xs!==bW.width){this.__xr=bW.height;this.__xs=bW.width;this.fireEvent(s);}
;}
,fullUpdate:function(){this.__jV._fullUpdate=1;qx.ui.core.queue.Widget.add(this);}
,isUpdatePending:function(){return !!this.__jV._fullUpdate;}
,_fullUpdate:function(){var bX=this.getVisibleLayers();if(bX.length==0){this.__xz();return;}
;var bY=this.getBounds();if(!bY){return;}
;this._setLayerWindow(bX,this.__pv,this.__pw,bY.width,bY.height,true);this.__xz();}
,_deferredUpdateScrollPosition:function(){this.__jV._updateScrollPosition=1;qx.ui.core.queue.Widget.add(this);}
,_updateScrollPosition:function(){var ca=this.getVisibleLayers();if(ca.length==0){this.__xz();return;}
;var cc=this.getBounds();if(!cc){return;}
;var cb={top:this.__pw,bottom:this.__pw+cc.height,left:this.__pv,right:this.__pv+cc.width};if(this.__xt.top<=cb.top&&this.__xt.bottom>=cb.bottom&&this.__xt.left<=cb.left&&this.__xt.right>=cb.right){this.__xu.setUserBounds((this.getPaddingLeft()||0)+(this.__xt.left-cb.left),(this.getPaddingTop()||0)+(this.__xt.top-cb.top),this.__xt.right-this.__xt.left,this.__xt.bottom-this.__xt.top);}
else {this._setLayerWindow(ca,this.__pv,this.__pw,cc.width,cc.height,false);}
;this.__xz();}
},destruct:function(){this._disposeArray(x);this._disposeObjects(h,w,v);this.__xt=this.__jV=this.__sP=this.__tU=null;}
});}
)();
...
(function(){var a="one",b="Boolean",c="qx.ui.virtual.selection.MModel",d="qx.data.Array",f="multi",g="selected",h="_applySelection",j="change",k="_applyDragSelection",l="single",m="_applyQuickSelection",n="changeSelection",o="_applySelectionMode",p="additive",q="qx.event.type.Data";qx.Mixin.define(c,{construct:function(){this._initSelectionManager();this.__xe=new qx.data.Array();this.initSelection(this.__xe);}
,properties:{selection:{check:d,event:n,apply:h,nullable:false,deferredInit:true},selectionMode:{check:[l,f,p,a],init:l,apply:o},dragSelection:{check:b,init:false,apply:k},quickSelection:{check:b,init:false,apply:m}},events:{"changeSelection":q},members:{_manager:null,__xE:false,__xF:false,__xe:null,_initSelectionManager:function(){var self=this;var r={isItemSelectable:function(s){return self._provider.isSelectable(s);}
...
;this.__xE=true;var F=this.getSelection();var H=[];for(var i=0;i<F.getLength();i++ ){var G=F.getItem(i);var I=this._getSelectables();var E=-1;if(I!=null){E=I.indexOf(G);}
...
;this.__xF=true;this.__xj();this.__xF=false;}
,__xj:function(){if(this.__xH()){return;}
;var L=this._manager.getSelection();var M=[];for(var i=0;i<L.length;i++ ){var N=this._getDataFromRow(L[i]);if(N!=null){M.push(N);}
;}
;this.__xG(M);}
,__xG:function(Q){var P=this.getSelection();if(Q.length>0){var O=[0,P.getLength()];O=O.concat(Q);var R=P.splice.apply(P,O);R.dispose();}
else {P.removeAll();}
;}
,__xH:function(){var T=this.getSelection();var V=this._manager.getSelection();if(T.getLength()!==V.length){return false;}
...
;}
},destruct:function(){this._manager.dispose();this._manager=null;if(this.__xe){this.__xe.dispose();}
;}
});}
)();
...
(function(){var c="changeModel",d="resize",f="_applyLabelOptions",g="_applyLabelPath",h="qx.data.Array",j="_applyGroupLabelOptions",m="changeLength",n="Integer",o="_applyModel",p="changeGroups",q="Boolean",r="GroupingTypeError: You can't mix 'Objects' and 'Strings' as",s="sorter",t="row-layer",u="group",v="_applyIconPath",w="_applyDelegate",x="changeDelegate",y=" group identifier!",z="???",A="_applyRowHeight",B="qx.data.IListData",C="filter",D="virtual-list",E="String",F="_applyGroupLabelPath",G="_applyIconOptions",H="qx.ui.list.List";qx.Class.define(H,{extend:qx.ui.virtual.core.Scroller,include:[qx.ui.virtual.selection.MModel],implement:qx.data.controller.ISelection,construct:function(I){qx.ui.virtual.core.Scroller.call(this,0,1,20,100);this._init();this.__xI=new qx.data.Array();this.initGroups(this.__xI);if(I!=null){this.initModel(I);}
;this.initItemHeight();}
,properties:{appearance:{refine:true,init:D},focusable:{refine:true,init:true},width:{refine:true,init:100},height:{refine:true,init:200},model:{check:B,apply:o,event:c,nullable:true,deferredInit:true},itemHeight:{check:n,init:25,apply:A,themeable:true},labelPath:{check:E,apply:g,nullable:true},iconPath:{check:E,apply:v,nullable:true},groupLabelPath:{check:E,apply:F,nullable:true},labelOptions:{apply:f,nullable:true},iconOptions:{apply:G,nullable:true},groupLabelOptions:{apply:j,nullable:true},delegate:{apply:w,event:x,init:null,nullable:true},autoGrouping:{check:q,init:true},groups:{check:h,event:p,nullable:false,deferredInit:true}},members:{_background:null,_provider:null,_layer:null,__xJ:null,__xK:null,__xL:null,__xM:false,__xN:false,__xO:false,__xI:null,refresh:function(){this.__xQ();}
...
,_init:function(){this._provider=new qx.ui.list.provider.WidgetProvider(this);this.__xJ=[];this.__xK=[];this.__xL={};this.__xM=false;this.__xN=false;this.__xO=false;this.getPane().addListener(d,this._onResize,this);this._initBackground();this._initLayer();}
...
;}
,_getLookupTable:function(){return this.__xJ;}
,_lookup:function(P){return this.__xJ[P];}
,_lookupGroup:function(Q){return this.__xK.indexOf(Q);}
,_reverseLookup:function(R){if(R<0){return -1;}
;return this.__xJ.indexOf(R);}
...
,_applyDelegate:function(bl,bk){this._provider.setDelegate(bl);this.__xQ();}
...
,__xP:function(){this.getPane().getRowConfig().setItemCount(this.__xJ.length);this.getPane().fullUpdate();}
,__xQ:function(){this.__xJ=[];this.__xK=[];this.__xL={};if(this.isAutoGrouping()){this.getGroups().removeAll();}
;var bm=this.getModel();if(bm!=null){this._runDelegateFilter(bm);this._runDelegateSorter(bm);this._runDelegateGroup(bm);}
;this._updateSelection();this.__xP();}
,_runDelegateFilter:function(bo){var bn=qx.util.Delegate.getMethod(this.getDelegate(),C);for(var i=0,l=bo.length;i<l; ++i){if(bn==null||bn(bo.getItem(i))){this.__xJ.push(i);}
;}
;}
,_runDelegateSorter:function(bp){if(this.__xJ.length==0){return;}
;var bq=qx.util.Delegate.getMethod(this.getDelegate(),s);if(bq!=null){this.__xJ.sort(function(a,b){return bq(bp.getItem(a),bp.getItem(b));}
);}
;}
,_runDelegateGroup:function(bu){var bv=qx.util.Delegate.getMethod(this.getDelegate(),u);if(bv!=null){for(var i=0,l=this.__xJ.length;i<l; ++i){var br=this.__xJ[i];var bt=this.getModel().getItem(br);var bs=bv(bt);this.__xR(bs,br);}
;this.__xJ=this.__xS();}
;}
,__xR:function(bx,bw){if(bx==null){this.__xO=true;bx=z;}
;var name=this.__xT(bx);if(this.__xL[name]==null){this.__xL[name]=[];if(this.isAutoGrouping()){this.getGroups().push(bx);}
;}
;this.__xL[name].push(bw);}
,__xS:function(){this.__xU();var bB=[];var bD=0;var bz=this.getGroups();for(var i=0;i<bz.getLength();i++ ){var by=bz.getItem(i);bB.push(-1);this.__xK.push(bD);bD++ ;var bC=this.__xT(by);var bA=this.__xL[bC];if(bA!=null){for(var k=0;k<bA.length;k++ ){bB.push(bA[k]);bD++ ;}
;}
;}
;return bB;}
,__xT:function(bF){var name=null;if(!qx.lang.Type.isString(bF)){var bE=this.getGroups().indexOf(bF);this.__xN=true;name=u;if(bE==-1){name+=this.getGroups().getLength();}
else {name+=bE;}
;}
else {this.__xM=true;var name=bF;}
;return name;}
,__xU:function(){if(this.__xN&&this.__xO||this.__xN&&this.__xM){throw new Error(r+y);}
;}
...
;this._background.dispose();this._provider.dispose();this._layer.dispose();this._background=this._provider=this._layer=this.__xJ=this.__xK=this.__xL=null;if(this.__xI){this.__xI.dispose();}
;}
});}
)();
(function(){var a="qx.ui.virtual.layer.Abstract",b="abstract",c="Abstract method '_fullUpdate' called!";qx.Class.define(a,{extend:qx.ui.core.Widget,type:b,implement:[qx.ui.virtual.core.ILayer],construct:function(){qx.ui.core.Widget.call(this);this.__jV={};}
,properties:{anonymous:{refine:true,init:true}},members:{__jV:null,__bH:null,__tS:null,__tT:null,__tU:null,__sP:null,getFirstRow:function(){return this.__tS;}
,getFirstColumn:function(){return this.__tT;}
,getRowSizes:function(){return this.__tU||[];}
,getColumnSizes:function(){return this.__sP||[];}
,syncWidget:function(e){if(!this.getContentElement().getDomElement()){return;}
;if(this.__jV.fullUpdate||this.__jV.updateLayerWindow&&this.__jV.updateLayerData){this._fullUpdate.apply(this,this.__bH);}
else if(this.__jV.updateLayerWindow){this._updateLayerWindow.apply(this,this.__bH);}
else if(this.__jV.updateLayerData&&this.__tU){this._updateLayerData();}
;if(this.__jV.fullUpdate||this.__jV.updateLayerWindow){var d=this.__bH;this.__tS=d[0];this.__tT=d[1];this.__tU=d[2];this.__sP=d[3];}
;this.__jV={};}
,_updateLayerData:function(){this._fullUpdate(this.__tS,this.__tT,this.__tU,this.__sP);}
,_fullUpdate:function(g,i,h,f){throw new Error(c);}
,_updateLayerWindow:function(k,m,l,j){this._fullUpdate(k,m,l,j);}
,updateLayerData:function(){this.__jV.updateLayerData=true;qx.ui.core.queue.Widget.add(this);}
,fullUpdate:function(p,q,o,n){this.__bH=arguments;this.__jV.fullUpdate=true;qx.ui.core.queue.Widget.add(this);}
,updateLayerWindow:function(t,u,s,r){this.__bH=arguments;this.__jV.updateLayerWindow=true;qx.ui.core.queue.Widget.add(this);}
},destruct:function(){this.__jV=this.__bH=this.__tU=this.__sP=null;}
});}
)();
...
;}
,setColor:function(J,I){qx.ui.virtual.layer.AbstractBackground.prototype.setColor.call(this,J,I);if(this.__ya(J)){this.updateLayerData();}
;}
,setBackground:function(K,L){qx.ui.virtual.layer.AbstractBackground.prototype.setBackground.call(this,K,L);if(this.__ya(K)){this.updateLayerData();}
;}
,__ya:function(M){var O=this.getFirstRow();var N=O+this.getRowSizes().length-1;return M>=O&&M<=N;}
}});}
)();
...
(function(){var a="cell.type",b="model[",c="groups[",d="model",f="",g="changeDelegate",h="label",i="qx.ui.list.core.MWidgetController",j="icon",k="]",l="value",m="BindingIds",n=".",o="group",p="String";qx.Mixin.define(i,{construct:function(){this.__wU=[];}
,properties:{labelPath:{check:p,nullable:true},iconPath:{check:p,nullable:true},groupLabelPath:{check:p,nullable:true},labelOptions:{nullable:true},iconOptions:{nullable:true},groupLabelOptions:{nullable:true},delegate:{event:g,init:null,nullable:true}},members:{__wU:null,bindDefaultProperties:function(q,r){if(q.getUserData(a)!=o){this.bindProperty(f,d,null,q,r);this.bindProperty(this.getLabelPath(),h,this.getLabelOptions(),q,r);if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),j,this.getIconOptions(),q,r);}
;}
else {this.bindProperty(this.getGroupLabelPath(),l,this.getGroupLabelOptions(),q,r);}
;}
,bindProperty:function(u,x,v,t,s){var y=t.getUserData(a);var w=this.__wV(s,u,y);if(v){v.ignoreConverter=d;}
;var z=this._list.bind(w,t,x,v);this.__wW(t,z);}
,bindPropertyReverse:function(D,B,E,F,A){var G=F.getUserData(a);var C=this.__wV(A,D,G);var H=F.bind(B,this._list,C,E);this.__wW(F,H);}
,removeBindings:function(){while(this.__wU.length>0){var I=this.__wU.pop();this._removeBindingsFrom(I);}
;}
...
;}
,_removeBindingsFrom:function(T){var U=this.__wX(T);while(U.length>0){var V=U.pop();try{this._list.removeBinding(V);}
catch(e){T.removeBinding(V);}
;}
;if(qx.lang.Array.contains(this.__wU,T)){qx.lang.Array.remove(this.__wU,T);}
;}
,__wV:function(X,ba,Y){var W=b+X+k;if(Y==o){W=c+X+k;}
;if(ba!=null&&ba!=f){W+=n+ba;}
;return W;}
,__wW:function(bc,bd){var bb=this.__wX(bc);if(!qx.lang.Array.contains(bb,bd)){bb.push(bd);}
;if(!qx.lang.Array.contains(this.__wU,bc)){this.__wU.push(bc);}
;}
,__wX:function(be){var bf=be.getUserData(m);if(bf==null){bf=[];be.setUserData(m,bf);}
;return bf;}
...
,styleSelectabled:function(v){var u=this.__wY(v);this._styleSelectabled(u);}
,styleUnselectabled:function(x){var w=this.__wY(x);this._styleUnselectabled(w);}
...
;}
,_styleSelectabled:function(A){this.__xa(A,{selected:1});}
,_styleUnselectabled:function(B){this.__xa(B,{});}
...
,__wY:function(G){return this._list._layer.getRenderedCellWidget(G,0);}
,__xa:function(I,H){if(I==null){return;}
...
;this._cellProvider=h;this.__xb=[];}
,properties:{anonymous:{refine:true,init:false}},events:{updated:c},members:{__xb:null,getRenderedCellWidget:function(q,n){var j=this.getColumnSizes().length;var p=this.getRowSizes().length;var o=this.getFirstRow();var m=this.getFirstColumn();if(q<o||q>=o+p||n<m||n>=m+j){return null;}
;var l=(n-m)+(q-o)*j;var k=this._getChildren()[l];if(k.getUserData(g)){return null;}
else {return k;}
;}
,_getSpacer:function(){var r=this.__xb.pop();if(!r){r=new qx.ui.core.Spacer();r.setUserData(g,1);}
;return r;}
...
;}
;}
,_fullUpdate:function(A,v,D,z){var B=this._cellProvider;var G=this._getChildren();for(var i=0;i<G.length;i++ ){var E=G[i];if(E.getUserData(g)){this.__xb.push(E);}
...
;}
;}
;var J=this._cellProvider;var N=this._getChildren();for(var i=0;i<N.length;i++ ){if(!W[i]){var T=N[i];if(T.getUserData(g)){this.__xb.push(T);}
...
;this._cellProvider=this.__xb=null;}
});}
)();
...
;}
;j.setUserData(c,h);}
,getCellWidget:function(o,m){var n=this.__xc();this.updateStates(n,m);this.updateData(n,o);return n;}
,pool:function(p){this.__cL.push(p);}
,_cleanupPool:function(){var q=this.__cL.pop();while(q){q.destroy();q=this.__cL.pop();}
;}
,__xc:function(){var r=this.__cL.pop();if(r==null){r=this._createWidget();this.fireDataEvent(a,r);}
;return r;}
