...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.VSplit.ce67c9bb1156.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="slider",b="mousedown",c="splitter",d="px",f="mouseout",g="Integer",h="height",i="mousemove",j="move",k="maxHeight",l="resize",m="vertical",n="width",o="_applyOrientation",p="_applyOffset",q="splitpane",r="qx.ui.splitpane.Pane",s="top",t="minHeight",u="knob",v="mouseup",w="horizontal",z="minWidth",A="appear",B="losecapture",C="left",D="maxWidth";qx.Class.define(r,{extend:qx.ui.core.Widget,construct:function(E){qx.ui.core.Widget.call(this);this.__gx=[];if(E){this.setOrientation(E);}
else {this.initOrientation();}
;this.__lH.addListener(b,this._onMouseDown,this);this.__lH.addListener(v,this._onMouseUp,this);this.__lH.addListener(i,this._onMouseMove,this);this.__lH.addListener(f,this._onMouseOut,this);this.__lH.addListener(B,this._onMouseUp,this);}
,properties:{appearance:{refine:true,init:q},offset:{check:g,init:6,apply:p},orientation:{init:w,check:[w,m],apply:o}},members:{__mJ:null,__mK:false,__mL:null,__mM:null,__mN:null,__mO:null,__mP:null,__gx:null,__lH:null,_createChildControlImpl:function(H,G){var F;switch(H){case a:F=new qx.ui.splitpane.Slider(this);F.exclude();this._add(F,{type:H});break;case c:F=new qx.ui.splitpane.Splitter(this);this._add(F,{type:H});F.addListener(j,this.__mQ,this);break;};return F||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,H);}
,__mQ:function(e){this.__mS(e.getData());}
,__mR:function(K){this.__lH=new qx.ui.splitpane.Blocker(K);this.getContentElement().add(this.__lH);var I=this.getChildControl(c);var J=I.getWidth();if(!J){I.addListenerOnce(A,function(){this.__mS();}
,this);}
;I.addListener(l,function(e){var L=e.getData();if(L.height==0||L.width==0){this.__lH.hide();}
else {this.__lH.show();}
;}
,this);}
,getBlocker:function(){return this.__lH;}
,_applyOrientation:function(Q,N){var R=this.getChildControl(a);var M=this.getChildControl(c);this.__mN=Q===w;if(!this.__lH){this.__mR(Q);}
;this.__lH.setOrientation(Q);var P=this._getLayout();if(P){P.dispose();}
;var O=Q===m?new qx.ui.splitpane.VLayout:new qx.ui.splitpane.HLayout;this._setLayout(O);M.removeState(N);M.addState(Q);M.getChildControl(u).removeState(N);M.getChildControl(u).addState(Q);R.removeState(N);R.addState(Q);qx.ui.core.queue.Manager.flush();this.__mS();}
,_applyOffset:function(T,S){this.__mS();}
,__mS:function(V){var U=this.getChildControl(c);var ba=this.getOffset();var bb=U.getBounds();var Y=U.getContentElement().getDomElement();if(!Y){return;}
;if(this.__mN){var X=null;if(V){X=V.width;}
...
;this.__gx.push(be);}
,remove:function(bf){this._remove(bf);qx.lang.Array.remove(this.__gx,bf);}
,getChildren:function(){return this.__gx;}
,_onMouseDown:function(e){if(!e.isLeftPressed()){return;}
;var bg=this.getChildControl(c);var bj=bg.getContentLocation();var bh=this.getContentLocation();this.__mJ=this.__mN?e.getDocumentLeft()-bj.left+bh.left:e.getDocumentTop()-bj.top+bh.top;var bi=this.getChildControl(a);var bk=bg.getBounds();bi.setUserBounds(bk.left,bk.top,bk.width,bk.height);bi.setZIndex(bg.getZIndex()+1);bi.show();this.__mK=true;this.__lH.capture();e.stop();}
,_onMouseMove:function(e){this._setLastMousePosition(e.getDocumentLeft(),e.getDocumentTop());if(this.__mK){this.__mT();var bl=this.getChildControl(a);var bm=this.__mO;if(this.__mN){bl.setDomLeft(bm);this.__lH.setStyle(C,(bm-this.getOffset())+d);}
else {bl.setDomTop(bm);this.__lH.setStyle(s,(bm-this.getOffset())+d);}
;e.stop();}
;}
...
;this._finalizeSizes();var bn=this.getChildControl(a);bn.exclude();this.__mK=false;this.releaseCapture();e.stop();}
,_finalizeSizes:function(){var br=this.__mO;var bo=this.__mP;if(br==null){return;}
...
else {if(this.__mN){bs.setWidth(br);bq.setWidth(bo);}
else {bs.setHeight(br);bq.setHeight(bo);}
;}
;}
,__mT:function(){if(this.__mN){var bx=z,bE=n,by=D,bC=this.__mL;}
else {var bx=t,bE=h,by=k,bC=this.__mM;}
;var bD=this._getChildren();var bv=bD[2].getSizeHint();var bA=bD[3].getSizeHint();var bB=bD[2].getBounds()[bE]+bD[3].getBounds()[bE];var bz=bC-this.__mJ;var bw=bB-bz;if(bz<bv[bx]){bw-=bv[bx]-bz;bz=bv[bx];}
else if(bw<bA[bx]){bz-=bA[bx]-bw;bw=bA[bx];}
;if(bz>bv[by]){bw+=bz-bv[by];bz=bv[by];}
else if(bw>bA[by]){bz+=bw-bA[by];bw=bA[by];}
;this.__mO=bz;this.__mP=bw;}
,_isActiveDragSession:function(){return this.__mK;}
,_setLastMousePosition:function(x,y){this.__mL=x;this.__mM=y;}
