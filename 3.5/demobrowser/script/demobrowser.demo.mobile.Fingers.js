...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.mobile.Fingers.92aa6a3c115c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;this.__QO=[];this.__QP=[];J.addListener(s,this._onTouchStart,this);J.addListener(a,this._onTouchMove,this);}
,_onTouchStart:function(e){var L=e.getAllTouches();for(var i=0;i<L.length;i++ ){var K=L[i];this.__QO[i]=parseInt(K.target.style.left)-K.pageX;this.__QP[i]=parseInt(K.target.style.top)-K.pageY;}
;}
,_onTouchMove:function(e){var N=e.getAllTouches();for(var i=0;i<N.length;i++ ){var M=N[i];if(N.target==document.body){continue;}
;qx.bom.element.Style.setStyles(M.target,{"left":(M.pageX+this.__QO[i])+c,"top":(M.pageY+this.__QP[i])+c});}
;e.preventDefault();}
}});}
)();
