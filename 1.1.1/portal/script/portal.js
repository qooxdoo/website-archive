...
var settings = {"qx.application":"portal.Application","qx.theme":"qx.theme.Modern","qx.version":"1.1.1"};
...
var libinfo = {"__out__":{"sourceUri":"script"},"portal":{"resourceUri":"resource","sourceUri":"script","version":"trunk"},"qx":{"resourceUri":"resource","sourceUri":"script","version":"1.1.1"}};
...
this.__qE=[];
this.__qF={};
},statics:{dataKeyRoot:h,dataKeyGroupBoxes:m,dataKeyBoxes:d},events:{"loaded":e},properties:{activeBox:{init:null,check:g,apply:c}},members:{__qD:null,__qE:null,__qF:null,_applyActiveBox:function(n,o){if(o!=null){o.setActive(false);
}n.setActive(true);
},load:function(){this.__qG(window[portal.box.Manager.dataKeyRoot]);
qx.event.Registration.fireEvent(this,b);
},__qG:function(t){var x,A,y,z,B,t;
...
this.__qD.push(B);
this.__qF[A].push(B);
}}},getBoxes:function(){return this.__qD;
},getGroupBoxes:function(){return this.__qE;
},updateGroupBoxMembers:function(p,q,r){var s=qx.dom.Hierarchy.getElementIndex(r.getElement());
qx.lang.Array.remove(this.__qF[q],r);
qx.lang.Array.insertAt(this.__qF[p],r,s);
},getGroupBoxDataOfBox:function(C){for(var i=0,j=this.__qE.length,D;i<j;i++){D=this.__qF[this.__qE[i].id];

for(var k=0,l=D.length;k<l;k++){if(D[k].getId()===C){return this.__qE[i];
}}}return null;
...
this.__qJ=new portal.box.Resizable(this,x);
}if(t.draggable){this.setDraggable(true);
this.__qK=new portal.box.Draggable(this);
}this.__qM();
},properties:{element:{init:null},groupBoxId:{init:null,check:h},active:{check:s,init:false,apply:j},resizable:{check:s,init:false},draggable:{check:s,init:false}},members:{__qI:null,__qH:null,__qK:null,__qJ:null,_applyActive:function(y,z){if(y){qx.bom.element.Style.set(this.getElement(),q,a);
}else{qx.bom.element.Style.set(this.getElement(),q,n);
}},getId:function(){return this.getElement()?this.getElement().id:null;
},getData:function(){return this.__qH;
},getDraggableComponent:function(){return this.__qK;
},getResizableComponent:function(){return this.__qJ;
},__qL:function(A,B){if(B){for(var C in B){qx.bom.element.Style.set(C,B[C]);
}}qx.bom.element.Style.set(A,l,b);
qx.bom.element.Style.set(A,k,0);
qx.bom.Element.addListener(A,o,this.__qN,this,true);
},__qM:function(){var E=f+g;
E=E+r+E+r+E;
var D=qx.bom.Element.create(d,{"class":i});
D.innerHTML=E;
qx.dom.Element.insertEnd(D,this.getElement());
},__qN:function(e){portal.box.Manager.getInstance().setActiveBox(this);
}},destruct:function(){this.resetElement();
this.__qH=this.__qJ=this.__qK=null;
}});
})();
...
this.__qO=v;
this.__qP=this.__qO.getElement();
this.__qQ={};
this.__qR=false;
this.__qS={orientation:null,lastCoord:{left:null,top:null}};
this.__qT={minWidth:10,minHeight:10,maxWidth:null,maxHeight:null,autoHide:true,handles:3};

for(var name in this.__qT){if(w[name]){this.__qT[name]=w[name];
}}this.__qU();
},members:{__qS:null,__qO:null,__qP:null,__qQ:null,__qT:null,__qR:false,getBox:function(){return this.__qO;
},__qU:function(){switch(this.__qT.handles){case 1:this.__qQ.east=null;
break;
case 2:this.__qQ.south=null;
break;
case 3:this.__qQ.south=null;
this.__qQ.southeast=null;
this.__qQ.east=null;
break;
}for(var Q in this.__qQ){var P=this.__qQ[Q]=qx.bom.Element.create(f);
qx.bom.element.Attribute.set(P,o,Q);
qx.dom.Element.insertEnd(P,this.__qP);

if(this.__qP.id){P.id=this.__qP.id+h+Q;
}this.__qV(Q);
this.__qW(Q);
}},__qV:function(y){qx.bom.element.Class.add(this.__qQ[y],y+j);
qx.bom.element.Style.set(this.__qQ[y],u,this.__qT.autoHide?t:q);
},__qW:function(z){var B=qx.event.Registration;
var A=this.__qQ[z];
B.addListener(A,i,this.__ra,this);
B.addListener(document.body,m,this.__rb,this);

if(this.__qT.autoHide){B.addListener(this.__qO.getElement(),s,this.__qX,this);
B.addListener(this.__qO.getElement(),r,this.__qY,this);
}},__qX:function(e){for(var C in this.__qQ){qx.bom.element.Style.set(this.__qQ[C],u,q);
}},__qY:function(e){if(!this.__qR){for(var D in this.__qQ){qx.bom.element.Style.set(this.__qQ[D],u,t);
}}},__ra:function(e){e.stopPropagation();
if(e.isLeftPressed()){qx.event.Registration.addListener(document.body,p,this.__rc,this,true);
this.__qR=true;
var O=e.getTarget();
this.__qS.orientation=qx.bom.element.Attribute.get(O,o);
this.__qS.lastCoord.left=e.getDocumentLeft();
this.__qS.lastCoord.top=e.getDocumentTop();
portal.box.Util.bringToFront(this.__qP);
}},__rb:function(e){e.stopPropagation();

if(this.__qR){qx.event.Registration.removeListener(document.body,p,this.__rc,this,true);
portal.box.Util.sendToBack(this.__qP);

if(this.__qT.autoHide){for(var E in this.__qQ){qx.bom.element.Style.set(this.__qQ[E],u,t);
}}this.__qR=false;
qx.bom.Selection.clear(document.body);
}},__rc:function(e){e.stopPropagation();

if(this.__qR){var x={left:null,top:null};

switch(this.__qS.orientation){case n:x.top=e.getDocumentTop();
break;
case a:x.top=e.getDocumentTop();
x.left=e.getDocumentLeft();
break;
case g:x.left=e.getDocumentLeft();
break;
}this.__rd(x,this.__qS.lastCoord);
this.__qS.lastCoord.left=e.getDocumentLeft();
this.__qS.lastCoord.top=e.getDocumentTop();
}},__rd:function(F,G){var N=qx.bom.element.Dimension;
var K=portal.box.Manager.getInstance().getGroupBoxDataOfBox(this.__qO.getId());
var M=N.getContentSize(K.element);

if(F.left){var J=F.left-G.left;

if(J!==0){var I=N.getContentWidth(this.__qP)+J;
var L=N.getWidth(this.__qP)+J;

if((this.__qT.minWidth==null||I>this.__qT.minWidth)&&(this.__qT.maxWidth==null||I<=this.__qT.maxWidth)){if(M.width>=L){qx.bom.element.Style.set(this.__qP,k,I+d);
}}}}
if(F.top){var J=F.top-G.top;

if(J!==0){var H=N.getContentHeight(this.__qP)+J;

if((this.__qT.minHeight==null||H>this.__qT.minHeight)&&(this.__qT.maxHeight==null||H<=this.__qT.maxHeight)){qx.bom.element.Style.set(this.__qP,c,H+b);
}}}}},destruct:function(){if(this.__qT.autoHide){qx.event.Registration.removeListener(this.__qO.getElement(),s,this.__qX,this);
qx.event.Registration.removeListener(this.__qO.getElement(),r,this.__qY,this);
}this.__qO=this.__qT=this.__qP=this.__qQ=null;
this.__qS=null;
}});
})();
...
qx.Class.define(b,{statics:{__rg:1000,bringToFront:function(f){var g=qx.bom.element.Style.get(f,c);
g=g==a?0:g;
qx.bom.element.Style.set(f,c,g+this.__rg);
},sendToBack:function(d){var e=qx.bom.element.Style.get(d,c);
qx.bom.element.Style.set(d,c,e-this.__rg);
}}});
})();
...
this.__rh=p;
this.__ri=p.getElement();
this.__rj=null;
this.__rk=null;
this.__rl();
},members:{__rh:null,__ri:null,__rj:null,__rk:null,getBox:function(){return this.__rh;
},getElement:function(){return this.__ri;
},__rl:function(){this.__rm();
this.__rn();
},__rm:function(){this.__rj=qx.bom.Element.create(g);
qx.bom.element.Class.add(this.__rj,b);
qx.dom.Element.insertBegin(this.__rj,this.__ri);
},__rn:function(){qx.bom.Element.addListener(this.__rj,l,this.__ro,this);
qx.bom.Element.addListener(this.__rj,j,function(e){qx.bom.element.Style.set(this,c,f);
},this.__rj);
qx.bom.Element.addListener(this.__rj,m,this.__rr,this);
qx.bom.Element.addListener(this.__rj,h,portal.dragdrop.Manager.getInstance().stopSession,portal.dragdrop.Manager.getInstance());
qx.bom.Element.addListener(this.__rj,i,this.__rs,this);
},__ro:function(e){if(e.isLeftPressed()){this.__rk={left:e.getDocumentLeft()-qx.bom.element.Location.getLeft(this.__ri),top:e.getDocumentTop()-qx.bom.element.Location.getTop(this.__ri)};
qx.bom.Element.addListener(document.body,n,this.__rq,this,true);
this.__rp();
qx.event.Registration.fireEvent(this.__rj,m,qx.event.type.Event);
}},__rp:qx.core.Variant.select(a,{"mshtml":function(){var s=this;
var r=qx.lang.Function.bind(this.__rq,s);
document.getElementsByTagName(d)[0].onmouseleave=r;
},"default":qx.lang.Function.empty}),__rq:function(e){try{e.stopPropagation();
}catch(q){}
if(portal.dragdrop.Manager.getInstance().isSessionActive()){qx.event.Registration.removeListener(document.body,o,this.__rs,this,true);
portal.dragdrop.Manager.getInstance().stopSession();
}qx.bom.Element.removeListener(document.body,n,this.__rq,this,true);
},__rr:function(e){portal.box.Manager.getInstance().setActiveBox(this.__rh);
portal.dragdrop.Manager.getInstance().startSession(this.__rh);
qx.event.Registration.addListener(document.body,o,this.__rs,this,true);
},__rs:function(e){e.stopPropagation();
var t=e.getDocumentLeft()-this.__rk.left;
var top=e.getDocumentTop()-this.__rk.top;
qx.event.Timer.once(function(){this.checkGroupBox(t);
this.onDragMove(top,t);
},portal.dragdrop.Manager.getInstance(),0);
}},destruct:function(){this.__rh=this.__rj=this.__ri=this.__rk=null;
}});
})();
...
this.__rt={top:null,height:null};
this.__ru={top:0,left:0};
},properties:{activeBox:{check:u,init:null,nullable:true},sessionActive:{check:r,init:false}},members:{__rt:null,__rv:null,__rw:false,__ru:null,__rx:null,__ry:null,startSession:function(T){this.setSessionActive(true);
this.setActiveBox(T);
this.__rv=T.getGroupBoxId();
this.__rz();
if(this.__rx==null){this.__rx={};
var U=portal.box.Manager.getInstance().getGroupBoxes();

for(var i=0,j=U.length;i<j;i++){this.__rx[U[i].element.id]={left:qx.bom.element.Location.getLeft(U[i].element),right:qx.bom.element.Location.getLeft(U[i].element)+qx.bom.element.Dimension.getWidth(U[i].element)};
}}},__rz:function(){if(!this.__ry){this.__ry=qx.bom.Element.create(b);
}var F=this.getActiveBox().getElement();
var D=qx.bom.element.Dimension.getContentSize(F);
this.__rA(F,this.__ry);
qx.bom.element.Class.add(this.__ry,qx.bom.element.Class.get(F));
var E=k+d+qx.bom.element.Location.getTop(F)+w+c+qx.bom.element.Location.getLeft(F)+w+o+D.width+w+f+D.height+x;
qx.bom.element.Style.setCss(this.__ry,E);
qx.bom.element.Style.set(F,v,t);
qx.dom.Node.getBodyElement(F).appendChild(this.__ry);
portal.box.Util.bringToFront(this.__ry);
},__rA:function(y,z){while(y.firstChild){z.appendChild(y.firstChild);
}},stopSession:function(e){this.setSessionActive(false);
var H=this.getActiveBox();
var I=H.getElement();
portal.box.Manager.getInstance().updateGroupBoxMembers(H.getGroupBoxId(),this.__rv,H);
H.setGroupBoxId(this.__rv);
var G=new qx.fx.effect.core.Move(this.__ry);
G.set({x:qx.bom.element.Location.getLeft(I),y:qx.bom.element.Location.getTop(I),mode:h,duration:0.5,transition:g});
G.start();
G.addListener(n,function(){this.__rA(this.__ry,I);
qx.bom.element.Style.reset(I,v);
this.__ry.parentNode.removeChild(this.__ry);

if(H.isActive()){H._applyActive(true);
}else{portal.box.Manager.getInstance().setActiveBox(H);
}},this);
},onDragMove:function(top,A){if(this.__ru.left!=A){qx.bom.element.Style.set(this.__ry,q,A+x);
}
if(this.__ru.top!=top){qx.bom.element.Style.set(this.__ry,s,top+x);
}var B=this.getActiveBox().getElement();

if(this.__rt.top==null){this.__rt.top=qx.bom.element.Location.getTop(B);
}
if(this.__rt.height==null){this.__rt.height=qx.bom.element.Dimension.getHeight(B);
}if(this.__rw==false&&top>=this.__rt.top&&top<=(this.__rt.top+this.__rt.height)){return;
}var C;
if(this.__rw){C=qx.dom.Hierarchy.getFirstDescendant(document.getElementById(this.__rv));
while(C!=null){if(this.__rB(true,top,B,C)){this.__rw=false;
return;
}C=qx.dom.Hierarchy.getNextElementSibling(C);
}}else{C=B;
}if(top-this.__ru.top>0){C=qx.dom.Hierarchy.getNextElementSibling(C);

while(C!=null){if(C){if(this.__rB(true,top,B,C)){return;
}}C=qx.dom.Hierarchy.getNextElementSibling(C);
}}else{var C=qx.dom.Hierarchy.getPreviousElementSibling(C);

while(C!=null){if(C){if(this.__rB(false,top,B,C)){return;
}}C=qx.dom.Hierarchy.getPreviousElementSibling(C);
}}this.__ru.left=A;
this.__ru.top=top;
},__rB:function(L,top,M,N){var O=qx.bom.element.Location.getTop(N);
...
this.__rt.top=qx.bom.element.Location.getTop(M);
}return true;
...
},checkGroupBox:function(J){if(this.__rx[this.__rv].left<=J&&this.__rx[this.__rv].right>=J){return;
}
for(var K in this.__rx){if(K!=this.__rv){if(this.__rx[K].left<=J&&this.__rx[K].right>=J){this.__rv=K;
this.__rw=true;
return;
}}}}},destruct:function(){this.__rt=this.__ru=this.__rv=null;
this.__rC=this.__ry=this.__rx=null;
}});
})();
