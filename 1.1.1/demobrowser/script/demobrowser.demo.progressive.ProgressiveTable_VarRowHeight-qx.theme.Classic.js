...
qx.Class.define(d,{extend:qx.application.Standalone,members:{__My:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var C=p;
this.imageData=[{url:C+k,height:4,tooltip:u},{url:C+b,height:4,tooltip:n},o];
this.__My=0;
...
}y=A.resolve(this.imageData[i].url);
B=z.toUri(y);
this.imageData[i].url=B;
++this.__My;
qx.io.ImageLoader.load(B,function(w,x){this.info("Searching for ["+w+"]");
for(var j=0;j<this.imageData.length;j++){if(this.imageData[j].url==w){this.imageData[j].height=x.height;
--this.__My;
this.debug("Found ["+this.imageData[j].url+"].  loadCount="+this.__My+", end="+this.imageData.length);
if(this.__My==0){this.doLoad();
}return;
...
if(j===undefined){this.__CT=j;
}else{this.__CT=16;
}
if(k===undefined){this.__CU=k;
}else{this.__CU=16;
}},members:{__CT:null,__CU:null,_identifyImage:function(d){var h={imageWidth:this.__CT,imageHeight:this.__CU};
var i;
