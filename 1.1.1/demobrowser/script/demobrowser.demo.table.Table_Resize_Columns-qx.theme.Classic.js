...
var A=this._tableModel=this._tableModel=new qx.ui.table.model.Simple();
A.setColumns([g,m,c,f]);
A.setData(C);
var z={tableColumnModel:function(G){return new qx.ui.table.columnmodel.Resize(G);
}};
var D=this._table=new qx.ui.table.Table(A,z);
D.set({width:600,height:400});
D.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
var B=D.getTableColumnModel();
var y=B.getBehavior();
y.set(0,{width:k,minWidth:40,maxWidth:80});
y.setWidth(1,i);
y.setMinWidth(1,100);
y.setMaxWidth(1,320);
var B=D.getTableColumnModel();
...
w.add(v);
u=new qx.ui.toolbar.Button(q,j);
u.addListener(r,function(H){var I=this.createRandomRows(10);
this._tableModel.addRows(I);
this.info("10 rows added");
},this);
v.add(u);
u=new qx.ui.toolbar.Button(p,d);
u.addListener(r,function(E){var F=this._tableModel.getRowCount();
this._tableModel.removeRows(F-10,10);
...
w.add(v);
var t=this._table;
x=new qx.ui.toolbar.CheckBox(e);
x.set({value:t.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(o+b)});
x.addListener(h,function(s){t.setKeepFirstVisibleRowComplete(this.getValue());
},x);
v.add(x);
return w;
