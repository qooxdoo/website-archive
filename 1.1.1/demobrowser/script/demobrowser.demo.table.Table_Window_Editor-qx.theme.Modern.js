...
},createTable:function(){var z=this.createRandomRows(50);
var x=new qx.ui.table.model.Simple();
x.setColumns([b,m,k,n]);
x.setData(z);
x.setColumnEditable(1,true);
var w={tableColumnModel:function(B){return new qx.ui.table.columnmodel.Resize(B);
}};
var A=new qx.ui.table.Table(x,w);
A.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
var y=A.getTableColumnModel();
var v=y.getBehavior();
v.set(0,{width:i,minWidth:40,maxWidth:80});
v.setWidth(1,h);
v.setMinWidth(1,100);
v.setMaxWidth(1,320);
v.set(3,{width:100});
y.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
A.setShowCellFocusIndicator(false);
y.setCellEditorFactory(1,new demobrowser.demo.table.ModalCellEditorFactory());
return A;
}}});
qx.Class.define(f,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,members:{createCellEditor:function(q){var t=new qx.ui.window.Window(d);
t.setLayout(new qx.ui.layout.HBox(4));
t.set({padding:3,modal:true,showClose:false,showMaximize:false,showMinimize:false});
t.moveTo(300,250);
t.addListener(c,function(e){t.__Mv.focus();
t.__Mv.setTextSelection(0,t.__Mv.getValue().length);
});
t.__Mv=new qx.ui.form.TextField(q.value+j).set({allowGrowY:true});
t.add(t.__Mv);
var r=new qx.ui.form.Button(a);
r.addListener(p,function(e){t.close();
});
t.add(r);
var s=new qx.ui.core.Command(o);
s.addListener(p,function(e){r.execute();
s.dispose();
s=null;
});
return t;
},getCellEditorValue:function(u){return parseFloat(u.__Mv.getValue());
}}});
})();
