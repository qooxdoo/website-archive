...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.table.Table_Selection-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"3cc430f04e08"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
var S=this._tableModel=new qx.ui.table.model.Simple();
S.setColumns([l,b,h,g]);
S.setData(T);
S.setColumnEditable(1,true);
S.setColumnEditable(2,true);
S.setColumnSortable(3,false);
var U=new qx.ui.table.Table(S);
U.set({width:600,height:400});
U.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var R=U.getTableColumnModel();
R.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
R.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(a,h));
return U;
...
D.addListener(k,function(O){var P=this.createRandomRows(10);
this._tableModel.addRows(P);
this.info("5 rows added");
},this);
F.add(D);
D=new qx.ui.toolbar.Button(w,r);
D.addListener(k,function(L){var M=this._tableModel.getRowCount();
this._tableModel.removeRows(M-5,5);
...
D.addListener(k,function(N){this._table.resetSelection();
},this);
F.add(D);
D=new qx.ui.toolbar.Button(z);
D.addListener(k,function(Q){this._table.resetCellFocus();
},this);
F.add(D);
var G=new qx.ui.toolbar.CheckBox(p);
G.addListener(d,function(K){this._table.setShowCellFocusIndicator(!G.isValue());
},this);
F.add(G);
D=new qx.ui.toolbar.Button(n,c);
D.addListener(k,function(V){var W=[];
this._table.getSelectionModel().iterateSelection(function(J){W.push(J+i);
});
