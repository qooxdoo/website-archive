...
(function(){var s="ul",r="li",q="select",p="input",o="h2",n="ul,ol",m="color",l="field-title",k="center",j="title",bt="field-mail",bs=":contains('List Item 1')",br="fontWeight",bq="p",bp="bold",bo="h1,h2",bn="#detail1,#detail2,#detail3",bm="text",bl="backgroundColor",bk="<li>inserted via after()</li>",z=":first-child,:last-child",A="underline",x="green",y="<option>Mrs</option>",v="<option>Prof</option>",w="<option>Mr</option>",t="for",u="<li>inserted via append()</li>",F="<h3>Dynamically Inserted H3</h3>",G="opacity",P=" Title ",M="li:first-child",X="#detail2",S="li a",bg='<div style="border:2px solid red"><div style="border:2px solid blue"></div></div>',bd="h1",I="#eee",bj="0.5",bi="[type=submit]",bh="header",H="name",K="textAlign",L='<div style="background:#FFFFBB"></div>',O="<script type='text/javascript'>qx.log.Logger.debug('Globally executed script');</script>",Q="<li>inserted via before()</li>",T="click",ba="form",bf="label[for=field-mail]",B="<option>Dr</option>",C="submit",J="demobrowser.demo.bom.Selector",W="ul li:contains('List Item 3')",V="<b></b>",U="p:eq(1)",bc="label",bb="red",R="Click to follow the link",Y="input[type=submit]",i="a",be="<li>inserted via prepend()</li>",D="orange",E="li:last-child",N="id";
qx.Class.define(J,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);
var f=qx.log.Logger;
var b=qx.bom.Collection;
b.query(bq).setStyle(m,bb);
b.query(U).setStyle(br,bp).setStyle(G,bj);
b.query(bd).setStyles({"textAlign":k,"textDecoration":A,"color":x});
f.debug("href of first link: "+b.query("a").getAttribute("href"));
b.query(i).setAttribute(j,R);
b.query(bo).addClass(bh);
b.query(bq).addListener(T,function(){alert(this.innerHTML);
});
(new qx.bom.Collection).add(o).setStyle(m,D).add(r).setStyle(bl,I).end().setStyle(K,k);
f.debug("Found "+qx.bom.Collection.query("ul,ol").children().length+" children of ul/ol elements using children().");
f.debug("Found "+qx.bom.Collection.query("ul,ol").children(":first-child").length+" first children of ul/ol elements using children(selector)");
f.debug("Number of ul/ol elements which are closest to a li element: "+qx.bom.Collection.query("li").closest("ul,ol").length);
f.debug("Number of child nodes of the body: "+new qx.bom.Collection(document.body).contents().length);
var a=qx.bom.Collection.query(s).find(r);
f.debug("Found "+a.length+" li elements in uls using find()");
var a=new qx.bom.Collection(document.body).find(bo);
f.debug("Found "+a.length+" headers using find()");
var a=qx.bom.Collection.query(s).next()[0].tagName;
f.debug("Tag following the UL element: "+a);
var a=qx.bom.Collection.query(s).nextAll().length;
f.debug("Number of elements following the UL: "+a);
var a=qx.bom.Collection.query(r).parent().length;
f.debug("Number of parent elements of all li elems: "+a);
var a=qx.bom.Collection.query(Y).parents().length;
f.debug("Number of all parent elements of the submit button: "+a);
var a=qx.bom.Collection.query(s).prev()[0].tagName;
f.debug("Tag before the UL element: "+a);
var a=qx.bom.Collection.query(s).prevAll().length;
f.debug("Number of elements preceding the UL: "+a);
var a=qx.bom.Collection.query(W).siblings().length;
f.debug("List Item 3 has "+a+" siblings");
var a=qx.bom.Collection.query(q).getOffsetParent()[0].tagName;
f.debug("Offset parent of select box: "+a);
var a=qx.bom.Collection.query(q).getOffset();
f.debug("Offset of select box: ");
f.debug(a);
b.query(S).eq(1).setStyle(br,bp);
var a=qx.bom.Collection.query(p).filter(bi);
f.debug("Found "+a.length+" submit buttons using filter(selector)");
var a=qx.bom.Collection.query(p).filter(function(bu,bv,bw){return bu.type===C;
});
f.debug("Found "+a.length+" submit button using filter(function)");
var a=qx.bom.Collection.query(o).hasClass(ba);
f.debug("Any H2 with form has a class 'form': "+a);
var a=qx.bom.Collection.query(s).find(r).is(bs);
f.debug("Found li element with text 'List Item 1': "+a);
var a=qx.bom.Collection.query(s).find(r).not(bs);
f.debug("Number of found li elements without text 'List Item 1': "+a.length);
var a=qx.bom.Collection.query(s).find(r).not(z);
f.debug("Number of found li elements in the middle: "+a.length);
qx.bom.Collection.query(n).append(u);
qx.bom.Collection.query(n).prepend(be);
qx.bom.Collection.query(s).append(O);
qx.bom.Collection.query(M).after(bk);
qx.bom.Collection.query(E).before(Q);
var h=document.createElement(q);
var g=document.createElement(bc);
var c=new qx.bom.Collection(h).setAttribute(N,l).setAttribute(H,j);
new qx.bom.Collection(g).setAttribute(t,l).setAttribute(bm,P);
qx.bom.Collection.query(q).after(g,h);
c.append(w,y,B,v);
$(F).insertAfter(o);
qx.bom.Collection.id(bt).addListener(p,function(){f.debug("Typing in e-mail field...");
});
var d=qx.bom.Collection.id(bt);
var e=d.add(bf).reverse();
d.after(e.clone(true));
qx.bom.Collection.query(bn).wrapAll(bg);
qx.bom.Collection.query(bn).wrapInner(V);
qx.bom.Collection.query(X).wrap(L);
}}});
})();
