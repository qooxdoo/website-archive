...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.event.Event_Bus-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"efd7f02c00f2"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var o="</table>",n="</th></tr>",m="demobrowser.demo.event.EventDemo",l="</th><th>",k="",j="</td><td>",h="<table><tr><th>",g="<tr><td>",f="btnClear",e="click",d="</td></tr>";
...
if(p){qx.bom.Element.addListener(p,e,this._clearLog,this);
}},_initLogger:function(a,b,c){this.__Lm=h+a.join(l)+n;
this.__Ln=c||50;
this.__Lo=b;
this._clearLog();
},_clearLog:function(){this.__Lo.innerHTML=this.__Lm+o;
this.__Lp=[];
},_log:function(q){this.__Lp.unshift(q);
this.__Lp=this.__Lp.slice(0,this.__Ln);
var r=[this.__Lm];

for(var i=0;i<this.__Lp.length;i++){r.push(g,this.__Lp[i].join(j),d);
}r.push(o);
this.__Lo.innerHTML=r.join(k);
this.__Lo.scrollTop=0;
},__Lm:null,__Ln:null,__Lo:null,__Lp:null},destruct:function(){this.__Lo=this.__Lp=null;
}});
})();
...
},checkSubscription:function(m,n,o){return this.getInstance().checkSubscription(m,n,o);
...
}},construct:function(){this.__tQ={};
},members:{__tQ:null,getSubscriptions:function(){return this.__tQ;
},subscribe:function(g,h,j){if(!g||typeof h!=f){this.error("Invalid parameters! "+[g,h,j]);
return false;
}var k=this.getSubscriptions();

if(this.checkSubscription(g)){if(this.checkSubscription(g,h,j)){this.warn("Object method already subscribed to "+g);
return false;
}k[g].push({subscriber:h,context:j||null});
return true;
}else{k[g]=[{subscriber:h,context:j||null}];
return true;
...
},unsubscribe:function(D,E,F){var H=this.getSubscriptions();
var G=H[D];
...
if(I.subscriber===E&&I.context===F){G.splice(i,1);

if(G.length===0){H[D]=null;
delete H[D];
}return true;
}}while(i);
}return false;
},dispatch:function(w){if(typeof w==b){var B=typeof arguments[1]!=c?arguments[1]:true;
w=new qx.event.message.Message(w,B);
}var x=this.getSubscriptions();
var y=w.getName();
var z=false;

for(var C in x){var A=C.indexOf(d);

if(A>-1){if(A===0||C.substr(0,A)===y.substr(0,A)){this.__tR(x[C],w);
z=true;
}}else{if(C===y){this.__tR(x[y],w);
z=true;
}}}return z;
},__tR:function(J,K){for(var i=0;i<J.length;i++){var L=J[i].subscriber;
var M=J[i].context;
if(M&&M.isDisposed){if(M.isDisposed()){J.splice(i,1);
i--;
}else{L.call(M,K);
}}else{L.call(M,K);
}}}}});
