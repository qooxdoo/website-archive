...
}}cA.addListener(bE,function(e){var cf=e.getData();
...
cw.addListener(bB,function(e){if(e.getData()){var ce=cA.getSelectionMode();

if(ce==bC||ce==bG){this.debug("Drag selection is only available for the modes multi or additive");
}}cA.setDragSelection(e.getData());
});
ct.addListener(bB,function(e){if(e.getData()){var ch=cA.getSelectionMode();

if(ch==bH||ch==bD){this.debug("Quick selection is only available for the modes multi or additive");
}}cA.setQuickSelection(e.getData());
});
cn.addListener(bE,function(e){var cg=e.getData()[0].getUserData(bI);
cA.setSelectionMode(cg);

if(cg==bC||cg==bG){cw.setEnabled(false);
ct.setEnabled(true);
}else if(cg==bH||cg==cd){cw.setEnabled(true);
ct.setEnabled(false);
}});
