...
var N=qx.theme.manager.Font.getInstance().resolve(A).getStyles();
this._fontCss=qx.bom.element.Style.compile(N);
var W=new qx.ui.layout.Grid(5,0);
W.setRowFlex(2,1);
W.setColumnFlex(0,1);
W.setColumnFlex(1,1);
W.setColumnFlex(2,1);
W.setColumnFlex(3,1);
var K=[];
var U=new qx.ui.container.Composite(W);
this.getRoot().add(U,{edge:5});
U.add(new qx.ui.basic.Label(r).set({font:h,decorator:a,padding:3,allowGrowX:true}),{row:1,column:0});
var M=this.createRowSelectionScroller();
K.push(M.getUserData(k));
U.add(M,{row:2,column:0});
U.add(new qx.ui.basic.Label(p).set({font:h,decorator:a,padding:3,allowGrowX:true}),{row:1,column:1});
var M=this.createColumnSelectionScroller();
K.push(M.getUserData(k));
U.add(M,{row:2,column:1});
U.add(new qx.ui.basic.Label(m).set({font:h,decorator:a,padding:3,allowGrowX:true}),{row:1,column:2});
var M=this.createCellRectangleSelectionScroller();
K.push(M.getUserData(k));
U.add(M,{row:2,column:2});
U.add(new qx.ui.basic.Label(y).set({font:h,decorator:a,padding:3,allowGrowX:true}),{row:1,column:3});
var M=this.createCellLineSelectionScroller();
K.push(M.getUserData(k));
U.add(M,{row:2,column:3});
var S=new qx.ui.layout.Grid(20,4);
var P=new qx.ui.container.Composite(S).set({padding:15,backgroundColor:f});
U.add(P,{row:0,column:0,colSpan:4});
P.add(new qx.ui.basic.Label(o).set({font:h,padding:3}),{row:0,column:0});
var O=new qx.ui.form.RadioButton(l);
O.setUserData(g,E);
P.add(O,{row:1,column:0});
var R=new qx.ui.form.RadioButton(v);
R.setUserData(g,j);
R.setValue(true);
P.add(R,{row:2,column:0});
var L=new qx.ui.form.RadioButton(z);
L.setUserData(g,B);
P.add(L,{row:3,column:0});
var J=new qx.ui.form.RadioButton(u);
J.setUserData(g,F);
P.add(J,{row:4,column:0});
var Q=new qx.ui.form.RadioGroup(O,R,L,J);
Q.addListener(s,function(e){var bk=e.getData()[0].getUserData(g);
K.forEach(function(bb){bb.setMode(bk);
});

if(bk==E||bk==F){V.setEnabled(false);
T.setEnabled(true);
}else if(bk==j||bk==x){V.setEnabled(true);
T.setEnabled(false);
}});
P.add(new qx.ui.basic.Label(w).set({font:h,padding:3}),{row:0,column:1});
var V=new qx.ui.form.CheckBox(q);
P.add(V,{row:1,column:1});
V.addListener(D,function(e){if(e.getData()){var bI=K[0].getMode();

if(bI==E||bI==F){this.debug("Drag selection is only available for the modes multi or additive");
}}K.forEach(function(bH){bH.setDrag(e.getData());
});
});
var T=new qx.ui.form.CheckBox(t).set({enabled:false});
P.add(T,{row:2,column:1});
T.addListener(D,function(e){if(e.getData()){var bS=K[0].getMode();

if(bS==j||bS==B){this.debug("Quick selection is only available for the modes multi or additive");
}}K.forEach(function(bR){bR.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bg=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var be=new qx.ui.virtual.layer.Row(f,c);
bg.getPane().addLayer(be);
bg.getPane().addLayer(new qx.ui.virtual.layer.GridLines(b));
var bf=new qx.ui.virtual.cell.Cell();
var bd=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bO,bP){var bQ={};

if(bc.isItemSelected(bO)){bQ.selected=true;
}return bf.getCellProperties(bO+i+bP,bQ);
}});
bg.getPane().addLayer(bd);
var bc=new qx.ui.virtual.selection.Row(bg.getPane(),{styleSelectable:function(bh,bi,bj){if(bi!==d){return;
}
if(bj){be.setBackground(bh,d);
}else{be.setBackground(bh,null);
}bd.updateLayerData();
}});
...
bM.getPane().addLayer(new qx.ui.virtual.layer.Row(f,c));
var bL=new qx.ui.virtual.layer.Column();
bM.getPane().addLayer(bL);
bM.getPane().addLayer(new qx.ui.virtual.layer.GridLines(b));
var bN=new qx.ui.virtual.cell.Cell();
var bK=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bo,bp){var bq={};

if(bJ.isItemSelected(bp)){bq.selected=true;
}return bN.getCellProperties(bo+i+bp,bq);
}});
bM.getPane().addLayer(bK);
var bJ=new qx.ui.virtual.selection.Column(bM.getPane(),{styleSelectable:function(bl,bm,bn){if(bm!==d){return;
}
if(bn){bL.setBackground(bl,d);
}else{bL.setBackground(bl,null);
}bK.updateLayerData();
}});
...
bC.getPane().addLayer(new qx.ui.virtual.layer.Row(f,c));
var bB=new qx.ui.virtual.cell.Cell();
var bD=new qx.ui.virtual.cell.Cell().set({backgroundColor:C});
var bA=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(X,Y){var ba={};

if(bz.isItemSelected({row:X,column:Y})){ba.selected=true;
}
if(ba.selected){return bD.getCellProperties(X+i+Y,ba);
}else{bB.resetBackgroundColor();
return bB.getCellProperties(X+i+Y,ba);
}}});
bC.getPane().addLayer(bA);
bC.getPane().addLayer(new qx.ui.virtual.layer.GridLines(b));
var bz=new qx.ui.virtual.selection.CellRectangle(bC.getPane(),{styleSelectable:function(G,H,I){bA.updateLayerData();
}});
bz.attachMouseEvents(bC.getPane());
bz.attachKeyEvents(bC);
bz.set({mode:j});
bz.addItem({row:0,column:0});
bC.setUserData(k,bz);
return bC;
},createCellLineSelectionScroller:function(){var bx=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bx.getPane().addLayer(new qx.ui.virtual.layer.Row(f,c));
var bw=new qx.ui.virtual.cell.Cell();
var by=new qx.ui.virtual.cell.Cell().set({backgroundColor:C});
var bv=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bE,bF){var bG={};

if(bu.isItemSelected({row:bE,column:bF})){bG.selected=true;
}
if(bG.selected){return by.getCellProperties(bE+i+bF,bG);
}else{bw.resetBackgroundColor();
return bw.getCellProperties(bE+i+bF,bG);
}}});
bx.getPane().addLayer(bv);
bx.getPane().addLayer(new qx.ui.virtual.layer.GridLines(b));
var bu=new qx.ui.virtual.selection.CellLines(bx.getPane(),{styleSelectable:function(br,bs,bt){bv.updateLayerData();
}});
bu.attachMouseEvents(bx.getPane());
bu.attachKeyEvents(bx);
bu.set({mode:j});
bu.addItem({row:0,column:0});
bx.setUserData(k,bu);
return bx;
}}});
})();
...
},_getRelatedSelectable:function(j,k){if(k==b){var m=j-1;
var l=0;
var n=-1;
}else if(k==d){var m=j+1;
...
var h=f.getItemPosition(e);
var g=h+f.getItemSize(e)-1;
...
var Q=this._pane.getRowConfig().getItemCount();
var M=this._pane.getColumnConfig().getItemCount();

for(var R=0;R<Q;R++){for(var O=0;O<M;O++){var P={row:R,column:O};

if(this._isSelectable(P)){N.push(P);
}}}return N;
...
switch(k){case f:for(var p=j.row-1;p>=0;p--){l.row=p;

if(this._isSelectable(l)){return l;
}}break;
case g:var m=this._pane.getRowConfig().getItemCount();

for(var p=j.row+1;p<m;p++){l.row=p;

if(this._isSelectable(l)){return l;
}}break;
case e:for(var o=j.column-1;o>=0;o--){l.column=o;

if(this._isSelectable(l)){return l;
}}break;
case d:var n=this._pane.getColumnConfig().getItemCount();
...
},_getPage:function(h,i){if(i){return this._getFirstSelectable();
}else{return this._getLastSelectable();
}},_selectableToHashCode:function(L){return L.column+c+L.row;
},_scrollItemIntoView:function(F){this._pane.scrollCellIntoView(F.column,F.row);
},_getSelectableLocationX:function(W){var X=this._pane.getColumnConfig();
var ba=X.getItemPosition(W.column);
var Y=ba+X.getItemSize(W.column)-1;
return {left:ba,right:Y};
