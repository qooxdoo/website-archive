...
var w=document.getElementById(k).getElementsByTagName(e);

for(var i=0,l=w.length;i<l;i++){w[i].onclick=demobrowser.demo.bom.Location.jump;
}}},statics:{num:function(x,y){return parseInt(qx.bom.element.Style.get(x,y,qx.bom.element.Style.COMPUTED_MODE,false),10);
},jump:function(event){if(!event){event=window.event;
}event.returnValue=false;

if(event.preventDefault){event.preventDefault();
}function q(a,b){return parseInt(qx.bom.element.Style.get(a,b,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var s=this.href;
...
},randomize:function(){var v=document.body.getElementsByTagName(c);

for(var i=0,l=v.length;i<l;i++){if(v[i].id&&v[i].id!=o){v[i].style.margin=Math.round(Math.random()*10)+p;
v[i].style.padding=Math.round(Math.random()*10)+p;
v[i].style.borderWidth=Math.round(Math.random()*10)+p;
}}}}});
})();
