...
(function(){var Q="",P="selected",O="cell",N="layer",M="icon",L="#f3f3f3",K="label",J="Gallery (HTML - divs)",I="__LU",H="manager",bv="default",bu="demobrowser.demo.virtual.Gallery",bt="demobrowser.demo.virtual.WidgetGallery",bs="text-align: center;",br="auto",bq="color: white; background-color: #00398D;",bp="user-desktop.png",bo="<img src='",bn="'></img>",bm="group",X="resize",Y="folder.png",V="vertical",W="multi",T="network-server.png",U="user-trash.png",R="off",S="top",ba="icon/32/places/",bb="horizontal",be="demobrowser.demo.virtual.AbstractGallery",bd="abstract",bg="position: absolute;",bf="demobrowser.demo.virtual.HtmlGallery",bi="Gallery (widgets)",bh="<br>",bc="Icon #",bl="network-workgroup.png",bk="text-selected",bj="demobrowser.demo.virtual.GalleryCell";
qx.Class.define(bu,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);
new demobrowser.demo.virtual.WidgetGallery(bi);
var n=new demobrowser.demo.virtual.HtmlGallery(J);
n.moveTo(400,50);
}}});
qx.Class.define(be,{extend:qx.ui.window.Window,type:bd,construct:function(bS){qx.ui.window.Window.call(this,bS);
...
var bT=this._createScroller();
bT.set({scrollbarX:R,scrollbarY:br});
bT.getPane().addListener(X,this._onPaneResize,this);
this.add(bT);
this.manager=new qx.ui.virtual.selection.CellRectangle(bT.getPane(),this).set({mode:W,drag:true});
this.manager.attachMouseEvents();
this.manager.attachKeyEvents(bT);
},members:{getItemData:function(by,bz){return this.items[by*this.itemPerLine+bz];
},_createScroller:function(){},isItemSelectable:function(bx){return !!this.getItemData(bx.row,bx.column);
},styleSelectable:function(g,h,j){},_onPaneResize:function(e){var bQ=e.getTarget();
var bO=e.getData().width;
var bP=Math.floor(bO/this.itemWidth);

if(bP==this.itemsPerLine){return;
}this.itemPerLine=bP;
var bR=Math.ceil(this.itemCount/bP);
bQ.getColumnConfig().setItemCount(bP);
bQ.getRowConfig().setItemCount(bR);
},_generateItems:function(bG){var bJ=[];
var bM=[Y,U,T,bl,bp];
var bK=qx.util.AliasManager.getInstance();
var bI=qx.util.ResourceManager.getInstance();

for(var i=0;i<bG;i++){var bN=ba+bM[Math.floor(Math.random()*bM.length)];
var bH=bK.resolve(bN);
var bL=bI.toUri(bH);
bJ[i]={label:bc+(i+1),icon:bN,resolvedIcon:bL};
}return bJ;
}},destruct:function(){this.items=null;
this._disposeObjects(H);
}});
qx.Class.define(bj,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var u=new qx.ui.basic.Atom().set({iconPosition:S});
u.getChildControl(K).set({padding:[0,4]});
u.getChildControl(M).set({padding:4});
return u;
},updateData:function(v,w){v.set({icon:w.icon,label:w.label});
},updateStates:function(C,D){var F=C.getChildControl(K);
var E=C.getChildControl(M);

if(D.selected){F.setDecorator(P);
F.setTextColor(bk);
E.setDecorator(bm);
}else{F.resetDecorator();
F.resetTextColor();
E.resetDecorator();
}}}});
qx.Class.define(bt,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(G){demobrowser.demo.virtual.AbstractGallery.call(this,G);
this.__LU=new demobrowser.demo.virtual.GalleryCell();
},members:{_createScroller:function(){var bw=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.WidgetCell(this);
bw.getPane().addLayer(this.layer);
new qx.ui.virtual.behavior.Prefetch(bw,0,0,0,0,200,300,600,800).set({interval:500});
return bw;
},styleSelectable:function(bA,bB,bC){if(bB!==P){return;
}var bD=this.layer.getChildren();

for(var i=0;i<bD.length;i++){var bE=bD[i];
var bF=bE.getUserData(O);

if(bA.row!==bF.row||bA.column!==bF.column){continue;
}
if(bC){this.__LU.updateStates(bE,{selected:1});
}else{this.__LU.updateStates(bE,{});
}}},getCellWidget:function(o,p){var t=this.getItemData(o,p);

if(!t){return null;
}var q={row:o,column:p};
var r={};

if(this.manager.isItemSelected(q)){r.selected=true;
}var s=this.__LU.getCellWidget(t,r);
s.setUserData(O,q);
return s;
},poolCellWidget:function(m){this.__LU.pool(m);
}},destruct:function(){this._disposeObjects(I,N);
}});
qx.Class.define(bf,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(x){demobrowser.demo.virtual.AbstractGallery.call(this,x);
var y=qx.theme.manager.Font.getInstance().resolve(bv).getStyles();
this._fontCss=qx.bom.element.Style.compile(y);
},members:{_createScroller:function(){var l=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.HtmlCell(this);
l.getPane().addLayer(this.layer);
var k=new qx.ui.virtual.layer.GridLines(bb,L);
l.getPane().addLayer(k);
var k=new qx.ui.virtual.layer.GridLines(V,L);
l.getPane().addLayer(k);
return l;
},_onPaneResize:function(e){demobrowser.demo.virtual.AbstractGallery.prototype._onPaneResize.call(this,e);
this.manager.clearSelection();
},styleSelectable:function(z,A,B){this.layer.updateLayerData();
},getCellProperties:function(a,b){var f=this.getItemData(a,b);

if(!f){return Q;
}var d=this.manager.isItemSelected({row:a,column:b});
var c=d?bq:Q;
return {style:[bg,bs,this._fontCss,c].join(Q),content:[bo,f.resolvedIcon,bn,bh,f.label].join(Q)};
