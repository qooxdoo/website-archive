...
(function(){var q="execute",p="changeValue",o="value",n="icon/22/actions/edit-undo.png",m="Change ID sort method",l="Disabled ctx menu",k="icon/22/status/dialog-information.png",j="On",h="Change row with ID 10",g="Default ctx menu #2",I="Remove 5 rows",H="Add 10 rows",G="Context-menu selectable",F="",E="_tableModel",D="Table",C="icon/22/actions/list-remove.png",B="Whether the the first visible row should ",A=", ",z="demobrowser.demo.table.Table_Context_Menu",x="Default ctx menu #1",y="icon/22/actions/list-add.png",v="Selected rows:<br>",w="Keep first row",t="Off",u="be rendered completely when scrolling.",r="Show selection",s="Demonstrate use of alternate sorting algorithm.";
qx.Class.define(z,{extend:demobrowser.demo.table.TableDemo,members:{main:function(){qx.Class.include(qx.ui.table.Table,qx.ui.table.MTableContextMenu);
demobrowser.demo.table.TableDemo.prototype.main.call(this);
this.getRoot().setNativeContextMenu(true);
},getCaption:function(){return D;
},createTable:function(){var ba=this.createRandomRows(50);
var Y=this._tableModel=new qx.ui.table.model.Simple();
Y.setColumns([x,g,l,G]);
Y.setData(ba);
...
bb.setContextMenuHandler(3,this._contextMenuHandlerBoolean);
bb.setContextMenuHandler(2,this._contextMenuHandlerNull);
return bb;
},_contextMenuHandlerBoolean:function(bc,bd,be,bf,bg){var bh;

for(var i=0;i<=1;i++){bh=new qx.ui.menu.Button(i?j:t);
bh.setUserData(o,i?true:false);
bh.addListener(q,function(e){bf.setValue(bc,bd,this.getUserData(o));
});
bg.add(bh);
}return true;
},_contextMenuHandlerNull:function(a,b,c,d,f){return true;
},createControls:function(){var M=new qx.ui.toolbar.ToolBar();
var K,L,N;
L=new qx.ui.toolbar.Part();
M.add(L);
K=new qx.ui.toolbar.Button(h,n);
K.addListener(q,function(bl){var bm=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,bm[0][i]);
}this.info("Row 10 changed");
},this);
L.add(K);
K=new qx.ui.toolbar.Button(H,y);
K.addListener(q,function(bi){var bj=this.createRandomRows(10);
this._tableModel.addRows(bj);
this.info("10 rows added");
},this);
L.add(K);
K=new qx.ui.toolbar.Button(I,C);
K.addListener(q,function(V){var W=this._tableModel.getRowCount();
this._tableModel.removeRows(W-5,5);
this.info("5 rows removed");
},this);
L.add(K);
K=new qx.ui.toolbar.Button(r,k);
K.addListener(q,function(bn){var bo=[];
J.getSelectionModel().iterateSelection(function(bp){bo.push(bp+F);
});
this.showDialog(v+bo.join(A));
},this);
L.add(K);
L=new qx.ui.toolbar.Part();
M.add(L);
var J=this._table;
N=new qx.ui.toolbar.CheckBox(w);
N.set({value:J.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(B+u)});
N.addListener(p,function(bk){J.setKeepFirstVisibleRowComplete(this.getValue());
},N);
L.add(N);
N=new qx.ui.toolbar.CheckBox(m);
N.set({value:J.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(s)});
N.addListener(p,function(O){if(O.getData()){var Q=function(bq,br){var bs=bq[arguments.callee.columnIndex];
...
var P=function(R,S){var T=R[arguments.callee.columnIndex];
var U=S[arguments.callee.columnIndex];

if(T%2==1&&U%2==0){return -1;
}
if(U%2==1&&T%2==0){return 1;
}return (T<U)?1:((T==U)?0:-1);
};
J.getTableModel().setSortMethods(0,{ascending:Q,descending:P});
...
this.__Du=[];
},members:{__Du:null,setContextMenuHandler:function(d,f){this.__Du[d]=f;
},getContextMenuHandler:function(g){return this.__Du[g];
