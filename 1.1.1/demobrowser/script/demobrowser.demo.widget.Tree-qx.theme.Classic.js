...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.widget.Tree","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.widget.Tree-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"da083e9594dd"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var l="changeValue",k="execute",j="Trash",h="changeSelection",g="mode",f="multi",d="single",c="Windows (C:)",b="Documents (D:)",a="Inbox",O="Desktop",N="root",M="Open mode:",L="Presets",K="Hide Root Node",J="Network",I="Control",H=" items",G="Sent",F="Files",s="Show Open Items",t="",q="Workspace",r="Enable drag selection",o="Selection: ",p="Junk #",m="demobrowser.demo.widget.Tree",n="Data",u="click",v="Show root open button",y="none",x="Enable multi selection",A="double click",z="Selection:",C="Show Selected Items",B="Root node:",w="Toggle Height",E="Edit",D="Show Items";
...
var bn=new qx.ui.layout.HBox();
bn.set({spacing:20});
var bm=new qx.ui.container.Composite(bn);
this.getRoot().add(bm);
var bk=this.getTree();
bm.add(bk);
var bl=this.getCommandFrame(bk);
bm.add(bl);
},getTree:function(){var by=new qx.ui.tree.Tree().set({width:200,height:400});
var bD=new qx.ui.tree.TreeFolder(N);
bD.setOpen(true);
by.setRoot(bD);
var bB=new qx.ui.tree.TreeFolder(O);
bB.setOpen(true);
bD.add(bB);
var bt=new qx.ui.tree.TreeFolder(F);
var br=new qx.ui.tree.TreeFolder(q);
var bs=new qx.ui.tree.TreeFolder(J);
var bq=new qx.ui.tree.TreeFolder(j);
bB.add(bt,br,bs,bq);
var bz=new qx.ui.tree.TreeFile(c);
var bp=new qx.ui.tree.TreeFile(b);
br.add(bz,bp);
var bC=new qx.ui.tree.TreeFolder(a);
var bu=new qx.ui.tree.TreeFolder(L);
var bx=new qx.ui.tree.TreeFolder(G);
var bw=new qx.ui.tree.TreeFolder(j);

for(var i=0;i<30;i++){bw.add(new qx.ui.tree.TreeFile(p+i));
}var bA=new qx.ui.tree.TreeFolder(n);
var bv=new qx.ui.tree.TreeFolder(E);
bC.add(bu,bx,bw,bA,bv);
bD.add(bC);
return by;
},getCommandFrame:function(Q){var bb=new qx.ui.groupbox.GroupBox(I);
var Y=4;
bb.setLayout(new qx.ui.layout.Grid(5,3));
var bi=0;
bb.add(new qx.ui.basic.Label(o).set({paddingTop:4}),{row:bi,column:0});
var bf=new qx.ui.form.TextField().set({readOnly:true});
bb.add(bf,{row:bi++,column:1});
Q.addListener(h,function(e){var P=e.getData();

if(P.length>0){if(this.getSelectionMode()===f){bf.setValue(P.length+H);
}else{bf.setValue(P[0].getLabel());
}}else{bf.setValue(t);
}});
bb.add(new qx.ui.core.Spacer(Y,Y),{row:bi++,column:0});
bb.add(new qx.ui.basic.Label(M),{row:bi,column:0});
var U={"click":u,"dblclick":A,"none":y};
var S=new qx.ui.form.RadioGroup();

for(var bd in U){var T=new qx.ui.form.RadioButton(U[bd]).set({value:bd==Q.getOpenMode()});
T.setUserData(g,bd);
S.add(T);
bb.add(T,{row:bi++,column:1});
}S.addListener(h,function(e){Q.setOpenMode(e.getData()[0].getUserData(g));
});
bb.add(new qx.ui.core.Spacer(Y,Y),{row:bi++,column:0});
bb.add(new qx.ui.basic.Label(z),{row:bi,column:0});
var W=new qx.ui.form.CheckBox(x);
bb.add(W,{row:bi++,column:1});
W.addListener(l,function(e){var bo=e.getData();
Q.setSelectionMode(bo?f:d);
});
var X=new qx.ui.form.CheckBox(r);
bb.add(X,{row:bi++,column:1});
X.addListener(l,function(e){var bj=e.getData();
Q.setDragSelection(bj);

if(!W.getValue()){W.setValue(true);
}});
bb.add(new qx.ui.core.Spacer(Y,Y),{row:bi++,column:0});
bb.add(new qx.ui.basic.Label(B),{row:bi,column:0});
var bg=new qx.ui.form.CheckBox(K);
bb.add(bg,{row:bi++,column:1});
bg.addListener(l,function(e){Q.setHideRoot(e.getData());
});
var V=new qx.ui.form.CheckBox(v);
bb.add(V,{row:bi++,column:1});
V.addListener(l,function(e){Q.setRootOpenClose(e.getData());
});
bb.add(new qx.ui.core.Spacer(Y,Y),{row:bi++,column:0});
var be=new qx.ui.form.Button(D);
bb.add(be,{row:bi++,column:1});
be.addListener(k,function(e){alert((""+Q.getItems()).replace(",","\n","g"));
});
var R=new qx.ui.form.Button(s);
bb.add(R,{row:bi++,column:1});
R.addListener(k,function(e){alert((""+Q.getItems(true,false)).replace(",","\n","g"));
});
var bc=new qx.ui.form.Button(C);
bb.add(bc,{row:bi++,column:1});
bc.addListener(k,function(e){if(this.getSelectionMode()===d){alert(this.getSelection());
}else{alert(this.getSelection().toString().replace(",","\n","g"));
}},Q);
var ba=new qx.ui.form.Button(w);
bb.add(ba,{row:bi++,column:1});
var bh=true;
ba.addListener(k,function(e){Q.setHeight(bh?600:400);
bh=!bh;
});
return bb;
}}});
