...
var u=document.getElementById(j).getElementsByTagName(d);
...
}function p(v,w){return parseInt(qx.bom.element.Style.get(v,w,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var r=this.href;
...
},randomize:function(){var o=document.body.getElementsByTagName(c);

for(var i=0,l=o.length;i<l;i++){if(o[i].id&&o[i].id!=m){o[i].style.margin=Math.round(Math.random()*10)+n;
o[i].style.padding=Math.round(Math.random()*10)+n;
o[i].style.borderWidth=Math.round(Math.random()*10)+n;
}}}}});
})();
