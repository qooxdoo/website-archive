...
},addSimpleStack:function(){var B=new qx.ui.container.Stack();
B.setDecorator(f);
B.setWidth(200);
B.setHeight(120);
this.getRoot().add(B,{left:20,top:20});
var z=new qx.ui.container.Composite((new qx.ui.layout.HBox).set({spacing:4}));
var E=new qx.ui.form.Button(l,c);
var C=new qx.ui.form.Button(m,j);
z.add(E);
z.add(C);
this.getRoot().add(z,{left:20,top:150});
E.addListener(r,B.previous,B);
C.addListener(r,B.next,B);
var D=[n,o,b,g,p,q,k];
var A;

for(var i=0;i<D.length;i++){A=new qx.ui.core.Widget;
A.setBackgroundColor(D[i]);
B.add(A);
}B.addListener(h,function(e){this.debug("Selected: "+e.getData()[0].getBackgroundColor());
});
...
var y=new qx.ui.form.Button(l,c);
var w=new qx.ui.form.Button(m,j);
t.add(y);
t.add(w);
this.getRoot().add(t,{left:250,top:150});
y.addListener(r,v.previous,v);
w.addListener(r,v.next,v);
var x=[n,o,b,g,p,q,k];
var u;
...
v.add(u);
}v.addListener(h,function(e){var s=e.getData()[0];
this.debug("Selected: "+s.getBackgroundColor()+" ("+s.getWidth()+"x"+s.getHeight()+")");
});
v.addListener(a,function(e){var F=this.getBounds();
