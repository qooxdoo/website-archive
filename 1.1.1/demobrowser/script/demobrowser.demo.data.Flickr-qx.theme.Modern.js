...
var T=new demobrowser.demo.data.store.Flickr(k);
var L=new qx.ui.form.TextField(k);
this.getRoot().add(L,{left:30,top:50});
var M=new qx.ui.form.Button(q);
this.getRoot().add(M,{left:140,top:49});
M.addListener(C,function(){T.searchForTag(L.getValue());
},this);
L.addListener(b,function(e){if(e.getKeyIdentifier()==c){T.searchForTag(L.getValue());
}},this);
...
P.setHeight(110);
P.setOrientation(o);
this.getRoot().add(P,{left:30,right:30,top:80});
var R=new qx.data.controller.List(null,P);
R.setLabelPath(r);
R.setDelegate({configureItem:function(V){V.setShow(s);
}});
var N={converter:function(X,Y){return (d+Y.getFarm()+i+Y.getServer()+j+X+f+Y.getSecret()+z);
}};
R.setIconOptions(N);
R.setIconPath(t);
T.bind(a,R,l);
var O=new qx.ui.basic.Image();
this.getRoot().add(O,{left:30,top:200});
var S={converter:function(U){if(U){return (d+U.getFarm()+i+U.getServer()+j+U.getId()+f+U.getSecret()+D);
}return g;
}};
R.bind(A,O,B,S);
...
qx.Class.define(v,{extend:qx.data.store.Json,statics:{RESULT:null},construct:function(G){jsonFlickrApi=function(E){demobrowser.demo.data.store.Flickr.RESULT=E;
};
var H=h+G;
qx.data.store.Json.call(this,H);
},members:{searchForTag:function(W){if(W!=g){this.setUrl(h+W);
}},_createRequest:function(I){var J=new qx.io.ScriptLoader();
this.setState(n);
I+=m;
J.load(I,function(F){this.__MJ();
},this);
},__MJ:function(){this.setState("completed");
