...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.layout.Grid_Alignment","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.layout.Grid_Alignment-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"57ebd135157e"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var o="main",n="yellow",m="bottom",l="top",k="left",j="right",h="center",g="middle",f="orange",e="gray",c="green",d="demobrowser.demo.layout.Grid_Alignment";
...
var r=new qx.ui.container.Composite(new qx.ui.layout.Grid(20)).set({padding:20});
scroll.add(r);
r.add(this.getCellAlignGrid(),{row:0,column:0});
r.add(this.getRowColumnAlignGrid(),{row:0,column:1});
r.add(this.getColumnAlignGrid(),{row:0,column:2});
r.add(this.getRowAlignGrid(),{row:1,column:0});
},getNewWidget:function(){var v=new qx.ui.core.Widget().set({decorator:o,backgroundColor:c,allowGrowX:false,allowGrowY:false,height:Math.round((Math.random()*30)+30),width:Math.round((Math.random()*30)+30)});
return v;
},getColumnAlignGrid:function(){var a=new qx.ui.container.Composite().set({decorator:o,backgroundColor:n,allowGrowX:false,allowGrowY:false});
var b=new qx.ui.layout.Grid();
b.setSpacing(3);
b.setColumnAlign(0,k,l);
b.setColumnAlign(1,h,g);
b.setColumnAlign(2,j,m);
a.setLayout(b);

for(var x=0;x<3;x++){for(var y=0;y<6;y++){a.add(this.getNewWidget(),{row:y,column:x});
}}return a;
},getRowAlignGrid:function(){var p=new qx.ui.container.Composite().set({decorator:o,backgroundColor:n,allowGrowX:false,allowGrowY:false});
...
for(var x=0;x<6;x++){for(var y=0;y<3;y++){p.add(this.getNewWidget(),{row:y,column:x});
}}return p;
},getCellAlignGrid:function(){var w=new qx.ui.container.Composite().set({decorator:o,backgroundColor:n,allowGrowX:false,allowGrowY:false});
var A=new qx.ui.layout.Grid();
A.setSpacing(3);
w.setLayout(A);

for(var x=0;x<5;x++){A.setColumnAlign(x,h,g);

for(var y=0;y<5;y++){w.add(this.getNewWidget(),{row:y,column:x});
}}var z=A.getCellWidget(0,0);
z.set({alignX:k,alignY:l});
z.set({backgroundColor:f,width:20,height:20});
var z=A.getCellWidget(4,4);
z.set({alignX:j,alignY:m});
z.set({backgroundColor:f,width:20,height:20});
return w;
},getRowColumnAlignGrid:function(){var s=new qx.ui.container.Composite().set({decorator:o,backgroundColor:n,allowGrowX:false,allowGrowY:false});
var u=new qx.ui.layout.Grid();
u.setSpacing(3);
s.setLayout(u);

for(var x=0;x<5;x++){for(var y=0;y<5;y++){s.add(this.getNewWidget(),{row:y,column:x});
}}u.setColumnAlign(2,k,l);
u.setRowAlign(2,j,m);

for(var i=0;i<5;i++){u.getCellWidget(2,i).setBackgroundColor(e);
u.getCellWidget(i,2).setBackgroundColor(e);
}var t=u.getCellWidget(2,2);
t.set({backgroundColor:f,width:20,height:20});
return s;
}}});
})();
