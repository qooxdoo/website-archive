...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.virtual.List","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.virtual.List-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"2b2162fc44f2"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var o="value",n="one",m="multi",l="changeValue",k="single",j="additive",h="bold",g="Item No ",f="Julian",d="Moritz",bB="Jannis",bA="Kevin",bz="Robin",by="Felix",bx="Vincent",bw="demobrowser.demo.virtual.List",bv="Additive Selection",bu="/places/folder.png",bt="Nils",bs="Nick",v="Leon",w="Jannick",t="Linus",u="Configurable",r="Marc",s="icon/16/places/folder.png",p="Erik",q="addaptive",D="Colin",E="Lennard",T="changeSelection",P="Jason",bc="Tobias",W="Florian",bo="16",bi="Jonas",K="icon/",br="Jakob",bq="Enable drag selection",bp="Joel",I="David",M="Maximilian",O="48",R="Marlon",U="Lukas",X="Luca",be="Julius",bk="Fabian",x="Tim",y="Justin",L="Single Selection",bb="Simon",ba="Luis",Y="Johannes",bg="Nico",bf="Alexander",V="Jan",bd="Dominic",a="Noah",bj="Enable quick selection",z="Daniel",A="Paul",Q="Philipp",b="Niclas",c="Max",H="One Selection",B="Additive selection",C="Jonathan",G="Tom",S="Finn",bm="Ben",bl="on",N="One as selection mode",bn="Elias",J="Mika",bh="Multi Selection",F="Marvin";
qx.Class.define(bw,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);
...
bJ.addListener(l,function(e){if(e.getData()){var bS=bO.getSelectionMode();

if(bS==k||bS==n){this.debug("Drag selection is only available for the modes multi or additive");
}}bO.setDragSelection(e.getData());
});
bM.addListener(l,function(e){if(e.getData()){var bR=bO.getSelectionMode();

if(bR==m||bR==j){this.debug("Quick selection is only available for the modes multi or additive");
...
var bI=[v,U,X,S,x,by,bi,ba,M,f,c,A,b,V,bm,bn,w,Q,a,G,d,bg,I,bt,bb,bk,p,y,bf,br,W,bs,t,J,P,z,E,F,bB,bc,bd,R,r,Y,C,be,D,bp,bA,bx,bz];
...
(function(){var t="selected",s="qx.event.type.Data",r="Boolean",q="changeSelection",p="row",o="single",n="changeHeight",m=",",k="one",h="_applySpacing",I="Integer",H="_applyDragSelection",G="String",F="resize",E="demobrowser.demo.virtual.list.List",D="list",C="multi",B="_applyQuickSelection",A="_applyItemHeight",z="_applySelectionMode",x="additive",y="changeName",v="Number",w="addItem",u="removeItem";
qx.Class.define(E,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider],construct:function(){qx.ui.virtual.core.Scroller.call(this,0,1,this.getItemHeight(),10);
this.__Mm=new qx.ui.virtual.layer.WidgetCell(this);
this.getPane().addLayer(this.__Mm);
this.getPane().addListener(F,this._onResize,this);
this.__Mn=[];
this.__Mo=[];
this.__Mp=new qx.ui.virtual.selection.Row(this.getPane());
this.__Mp.addListener(q,this._onChangeSelection,this);
this.__Mp.attachMouseEvents();
this.__Mp.attachKeyEvents(this);
this.__Mp.attachListEvents(this);
new qx.ui.virtual.behavior.Prefetch(this,0,0,0,0,400,600,400,600).set({interval:500});
},events:{addItem:s,removeItem:s,changeValue:s,changeSelection:s},properties:{appearance:{refine:true,init:D},focusable:{refine:true,init:true},spacing:{check:I,init:0,apply:h,themeable:true},enableInlineFind:{check:r,init:true},name:{check:G,nullable:true,event:y},itemHeight:{init:24,themeable:true,check:v,apply:A},selectionMode:{check:[o,C,x,k],init:o,apply:z},dragSelection:{check:r,init:false,apply:H},quickSelection:{check:r,init:false,apply:B}},members:{__Mn:null,__Mo:null,__Mp:null,__Mm:null,syncWidget:function(){this.update();
},update:function(){var Q=this.getPane().getRowConfig();
Q.setItemCount(this.__Mn.length);
Q.resetItemSizes();

for(var i=0;i<this.__Mn.length;i++){var P=this.__Mn[i].getHeight();

if(P!==null){Q.setItemSize(i,P);
}}},updateSelection:function(){var K=this.__Mm.getChildren();

for(var i=0;i<K.length;i++){var L=K[i];
var M=L.getUserData(p);

if(this.__Mp.isItemSelected(M)){L.addState(t);
}else{L.removeState(t);
...
this.fireDataEvent(q,this.__Mt(e.getData()));
},getCellWidget:function(T,U){var W=this.__Mn[T];

if(!W){return null;
}var V=this.__Mo.pop()||new qx.ui.form.ListItem();
V.set({label:W.getLabel(),icon:W.getIcon()});

if(this.__Mp.isItemSelected(T)){V.addState(t);
}else{V.removeState(t);
}V.setUserData(p,T);
return V;
},poolCellWidget:function(bq){this.__Mo.push(bq);
},getValue:function(){var bh=this.getSelection();
var bi=[];
var bj;

for(var i=0,l=bh.length;i<l;i++){bj=bh[i].getValue();
if(bj==null){bj=bh[i].getLabel();
}bi.push(bj);
}return bi.join(m);
},setValue:function(bw){var bz=bw.split(m);
var by=[];
var bx;

for(var i=0,l=bz.length;i<l;i++){bx=this.findItem(bz[i]);

if(bx){by.push(bx);
}}this.setSelection(by);
},getChildren:function(){return this.__Mn;
},hasChildren:function(){return this.__Mn.length>0;
},_addHelper:function(bd){this.fireDataEvent(w,bd);
bd.addListener(n,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},_removeHelper:function(J){this.fireDataEvent(u,J);
J.removeListener(n,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},add:function(X){this.__Mn.push(X);
this._addHelper(X);
return this;
},remove:function(bt){qx.lang.Array.remove(this.__Mn,bt);
this._removeHelper(bt);
return this;
},removeAll:function(){for(var i=0,j=this.__Mn.length;i<j;i++){this._removeHelper(this.__Mn[i]);
}this.__Mn=[];
},indexOf:function(bg){return this.__Mn.indexOf(bg);
},addAt:function(bn,bo){qx.lang.Array.insertAt(this.__Mn,bn,bo);
this._addHelper(bn);
},addBefore:function(br,bs){qx.lang.Array.insertBefore(this.__Mn,br,bs);
this._addHelper(br);
},addAfter:function(f,g){qx.lang.Array.insertAfter(this.__Mn,f,g);
this._addHelper(f);
},removeAt:function(bB){this.__Mn.splice(bB,1);
this._removeHelper(this.__Mn[bB]);
},__Mq:function(be){var bf=this.__Mn.indexOf(be);

if(bf<0){return null;
}else{return bf;
}},__Mr:function(c){var d=this.__Mn[c];
return d;
},__Ms:function(R){var S=[];

for(var i=0;i<R.length;i++){S.push(this.__Mq(R[i]));
}return S;
},__Mt:function(N){var O=[];

for(var i=0;i<N.length;i++){O.push(this.__Mn[N[i]]);
}return O;
},selectAll:function(){this.__Mp.selectAll();
},setSelected:function(ba){this.__Mp.selectItem(this.__Mq(ba));
},isSelected:function(bp){return this.__Mp.isItemSelected(this.__Mq(bp));
},addToSelection:function(bA){this.__Mp.addItem(this.__Mq(bA));
},removeFromSelection:function(Y){this.__Mp.removeItem(this.__Mq(Y));
},selectRange:function(bu,bv){this.__Mp.selectItemRange(this.__Mq(bu),this.__Mq(bv));
},resetSelection:function(){this.__Mp.clearSelection();
},setSelection:function(bk){this.__Mp.replaceSelection(this.__Ms(bk));
},getSelectedItem:function(){return this.__Mn[this.getSelected()];
},getSelected:function(){return this.__Mn[this.__Mp.getSelectedItem()];
},getSelection:function(){return this.__Mt(this.__Mp.getSelection());
},getSortedSelection:function(){return this.__Mt(this.__Mp.getSortedSelection());
...
},getSelectables:function(){return this.__Mt(this.__Mp.getSelectables());
},_applySelectionMode:function(bl,bm){this.__Mp.setMode(bl);
},_applyDragSelection:function(a,b){this.__Mp.setDrag(a);
},_applyQuickSelection:function(bb,bc){this.__Mp.setQuick(bb);
}}});
})();
