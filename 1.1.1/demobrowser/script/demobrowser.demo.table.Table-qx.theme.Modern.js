...
var H,I,K;
I=new qx.ui.toolbar.Part();
J.add(I);
H=new qx.ui.toolbar.Button(g,k);
H.addListener(o,function(L){var M=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,M[0][i]);
}this.info("Row 10 changed");
},this);
I.add(H);
H=new qx.ui.toolbar.Button(D,v);
H.addListener(o,function(ba){var bb=this.createRandomRows(10);
this._tableModel.addRows(bb);
this.info("10 rows added");
},this);
I.add(H);
H=new qx.ui.toolbar.Button(E,y);
H.addListener(o,function(R){var S=this._tableModel.getRowCount();
this._tableModel.removeRows(S-5,5);
this.info("5 rows removed");
},this);
I.add(H);
H=new qx.ui.toolbar.Button(s,h);
H.addListener(o,function(T){var U=[];
G.getSelectionModel().iterateSelection(function(N){U.push(N+C);
});
this.showDialog(t+U.join(w));
},this);
I.add(H);
I=new qx.ui.toolbar.Part();
J.add(I);
var G=this._table;
K=new qx.ui.toolbar.CheckBox(u);
K.set({value:G.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(x+r)});
K.addListener(n,function(F){G.setKeepFirstVisibleRowComplete(this.getValue());
},K);
I.add(K);
K=new qx.ui.toolbar.CheckBox(j);
K.set({value:G.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(p)});
K.addListener(n,function(O){if(O.getData()){var Q=function(bc,bd){var be=bc[arguments.callee.columnIndex];
...
var P=function(V,W){var X=V[arguments.callee.columnIndex];
...
G.getTableModel().setSortMethods(0,{ascending:Q,descending:P});
}else{G.getTableModel().setSortMethods(0,null);
}},K);
I.add(K);
return J;
