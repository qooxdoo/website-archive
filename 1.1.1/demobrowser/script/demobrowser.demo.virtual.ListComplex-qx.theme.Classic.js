...
(function(){var q=" / ",p="row",o="foobar!",n="column",m="icon/",l="folder.png",k="places/",i="demobrowser.demo.virtual.DemoLayer",h="network-server.png",g="mouseover",c="user-trash.png",f="changeValue",e="network-workgroup.png",b="qx.ui.basic.Atom",a="/",d="user-desktop.png";
...
this.__LX=[];
},members:{_pool:null,__LX:null,getCellData:function(r,s){if(!this.__LX[r]){this.__LX[r]=[];
}
if(!this.__LX[r][s]){this.__LX[r][s]={label:this.__LY(),icon:this.__Ma()};
}return this.__LX[r][s];
},getCellWidget:function(t,u){var v;

if(u%2==0){v=this._pool.atom.pop();

if(!v){v=new qx.ui.basic.Atom();
v.addListener(g,function(){var A=this.__Ma();
v.setIcon(A);
this.__LX[t][u].icon=A;
},this);
}v.set(this.getCellData(t,u));
}else{v=this._pool.checkbox.pop();

if(!v){v=new qx.ui.form.CheckBox();
v.addListener(f,function(){this.setLabel(this.getLabel()==o?v.getUserData(p)+q+v.getUserData(n):o);
},v);
}v.set({value:t%2==0,label:t+q+u});
}v.setUserData(p,t);
v.setUserData(n,u);
return v;
},poolCellWidget:function(B){if(B.classname==b){this._pool.atom.push(B);
}else{this._pool.checkbox.push(B);
}},__LY:function(){var name="";

for(var j=0;j<10;j++){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);
}return name;
},__Ma:function(){var w=m;
var y=k;
var z=[l,c,h,e,d];
var x=Math.floor(Math.random()*4);
return (w+16+a+y+z[x]);
}},destruct:function(){this._pool=this.__LX=null;
}});
})();
