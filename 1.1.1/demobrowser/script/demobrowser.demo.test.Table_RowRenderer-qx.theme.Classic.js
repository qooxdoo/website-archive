...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.test.Table_RowRenderer","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.test.Table_RowRenderer-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"5d3bd7557809"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
},testDefaultRenderer:function(B){var B=new qx.ui.table.rowrenderer.Default();
var F=300;
var E=20;
var D={focusedRow:[true,false],selected:[true,false],row:[1,2],table:[this.tableMock]};
var C=this._getNewTableDiv(F);
var self=this;
qx.util.Permutation.permute(D,function(v){var content=[];

for(var z in v){if(z!==m){content.push(z+a+v[z]);
}}content=content.join(d);
var A=self._createRowElement(C,F,E);
var w=B.getRowClass(v);

if(w){A.className=w;
}A.innerHTML=content+j;
B.updateDataRowElement(v,A);
var y=[];
y.push(g);

if(w){y.push(k,w,q);
}var x=B.createRowStyle(v);
x+=n+E+b+F+f;

if(x){y.push(h,x,q);
}y.push(p,content,l);
C.innerHTML+=y.join(o);
self._rowTop+=E;
});
},_rowTop:0,_createRowElement:function(parent,s,t){var u=qx.bom.Element.create(c);
qx.bom.element.Style.setStyles(u,{position:i,width:s+r,height:t+r});
this._rowTop+=20;
