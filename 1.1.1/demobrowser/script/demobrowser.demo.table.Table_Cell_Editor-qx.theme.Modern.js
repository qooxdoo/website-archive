...
qx.Class.define(j,{extend:qx.application.Standalone,include:demobrowser.demo.table.MUtil,members:{__Mu:null,main:function(){qx.application.Standalone.prototype.main.call(this);
...
(function(){var k="checkbox",j='Property',i="options",h="password",g="type",f='email',e='password',d='options',c='type',b='0',ba='Use only numbers!',Y='regExp',X='[0-9]+',W='newsletter',V='admin',U='editable combobox',T='secret',S="demobrowser.demo.table.Table_Cell_Editor",R='status',Q="validationFunc",r='1',s="regExp",p="required",q='Doubleclick on cell to edit value',n='Newsletter',o='Inactive',l='User Name',m='telephone',v="dataChanged",w='Status',E='Active',C='validationFunc',I='enter website',G="editable",M='role',K='username',y='jdoe',P='Telephone',O='manager',N='enter valid telephone number',x='Waiting',A='Email',B='Role',D="Property editor table",F='2',H='user',J='website',L='Password',t="email",u='Website',z='enter valid email';
qx.Class.define(S,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return D;
...
},createTable:function(){var bN=[[K,l,y,{'required':true}],[e,L,T,{'type':e,'required':true}],[M,B,U,{'options':[V,O,H],'editable':true}],[R,w,b,{'options':[[o,null,b],[E,null,r],[x,null,F]]}],[f,A,z,{'type':f}],[m,P,N,{'regExp':X,'failMsg':ba}],[J,u,I,{'validationFunc':function(bp){if(bp.search(/^http:/)!=-1){window.open(bp);
}return bp;
}}],[W,n,true,{'type':k}]];
var bM=function(bx){var bD=bx.table;
var bA=bD.getTableModel();
var bB=bA.getRowData(bx.row);
var bE=bB[3];
var bC;

for(var bz in bE){switch(bz){case g:switch(bE[c]){case k:return new qx.ui.table.cellrenderer.Boolean;
case h:return new qx.ui.table.cellrenderer.Password;
}break;
case i:var bC=new qx.ui.table.cellrenderer.Replace;
var by={};
bE[d].forEach(function(a){if(a instanceof Array){by[a[0]]=a[2];
}});
bC.setReplaceMap(by);
bC.addReversedReplaceMap();
return bC;
}}return new qx.ui.table.cellrenderer.Default();
};
var bL=new qx.ui.table.cellrenderer.Dynamic(bM);
var bJ=function(bb){var bg=bb.table;
var be=bg.getTableModel();
var bf=be.getRowData(bb.row);
var bi=bf[3];
var bd=new qx.ui.table.celleditor.TextField;
var bh=null;

for(var bc in bi){switch(bc){case i:if(bi.editable){bd=new qx.ui.table.celleditor.ComboBox();
}else{bd=new qx.ui.table.celleditor.SelectBox();
}bd.setListData(bi[d]);
break;
case G:break;
case g:switch(bi[c]){case h:bd=new qx.ui.table.celleditor.PasswordField;
break;
case k:bd=new qx.ui.table.celleditor.CheckBox;
break;
case t:bd.setValidationFunction(function(bj,bk){var bl=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{2}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/;

if(bl.test(bj)){return bj;
}alert("You did not enter a valid email address");
return bk;
});
break;
}break;
case s:bd.setValidationFunction(function(bm,bn){var bo=new RegExp(bi[Y]);

if(bo.test(bm)){return bm;
}alert(bi['failMsg']);
return bn;
});
break;
case Q:bd.setValidationFunction(bi[C]);
break;
case p:bh=function(bu,bv){if(!bu){alert("You need to supply a value here");
return bv;
}return bu;
};
break;
...
var bI=new qx.ui.table.celleditor.Dynamic(bJ);
var bG=new qx.ui.table.model.Simple();
bG.setColumns([j,j,q]);
var bF={tableColumnModel:function(bw){return new qx.ui.table.columnmodel.Resize(bw);
}};
var bH=new qx.ui.table.Table(bG,bF);
bH.setDecorator(null);
bH.setColumnVisibilityButtonVisible(false);
bH.setKeepFirstVisibleRowComplete(true);
bH.setStatusBarVisible(false);
bH.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var bK=bH.getTableColumnModel();
bK.setColumnVisible(0,false);
bK.getBehavior().setWidth(1,100);
bK.getBehavior().setWidth(2,300);
bG.setColumnEditable(2,true);
bK.setDataCellRenderer(2,bL);
bK.setCellEditorFactory(2,bI);
bH.getTableModel().setData(bN);
bH.getTableModel().addListener(v,function(event){if(!(event instanceof qx.event.type.Data)){return;
}var bt=event.getData();
var bq=this.getTableModel();
var bs=bq.getValue(0,bt.firstRow);
var br=bq.getValue(bt.firstColumn,bt.firstRow);
this.info("User edited property '"+bs+"' and entered value '"+br+"'.");
},bH);
return bH;
}}});
})();
...
this.initIconTrue();
this.initIconFalse();
},properties:{iconTrue:{check:i,init:g,apply:h},iconFalse:{check:i,init:d,apply:c}},members:{__Hw:null,__Hx:false,__Hv:null,_applyIconTrue:function(l){this.__Hw=this.__Hv.resolve(l);
},_applyIconFalse:function(j){this.__Hx=this.__Hv.resolve(j);
},_insetY:5,_getCellStyle:function(k){return qx.ui.table.cellrenderer.AbstractImage.prototype._getCellStyle.call(this,k)+e;
},_identifyImage:function(a){var b={imageWidth:11,imageHeight:11};

switch(a.value){case true:b.url=this.__Hw;
break;
case false:b.url=this.__Hx;
break;
default:b.url=null;
break;
}return b;
