...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.ui.Placement-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"d5bbba8d4354"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
qx.Class.define(k,{extend:qx.application.Standalone,members:{__Nt:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var A=this.getRoot();
this.__Nt=[n,p,q,l,o,b,d,m];
A.add(this.createTestField(n),{left:20,top:20});
A.add(this.createTestField(p),{right:20,top:20});
A.add(this.createTestField(q),{right:20,bottom:20});
A.add(this.createTestField(l),{left:20,bottom:20});
A.add(this.createTestField(o),{left:20,top:250});
A.add(this.createTestField(m),{right:20,top:250});
A.add(this.createDomClick(),{left:200,top:100});
},createTestField:function(t){var x=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));
var v=new qx.ui.popup.Popup(new qx.ui.layout.Grow);
v.add(new qx.ui.basic.Atom(g));
v.setPadding(20);
var u=new qx.ui.form.SelectBox;
var y=null;

for(var i=0;i<this.__Nt.length;i++){var w=new qx.ui.form.ListItem(this.__Nt[i]);
u.add(w);

if(this.__Nt[i]==t){y=w;
}}x.add(u);
u.addListener(r,function(e){v.setPosition(e.getData()[0].getLabel());
});
u.setSelection([y]);
var z=new qx.ui.form.Button(h);
x.add(z);
z.addListener(s,function(e){v.placeToWidget(z);
v.show();
});
return x;
...
for(var i=0;i<this.__Nt.length;i++){B.add(new qx.ui.form.ListItem(this.__Nt[i]));
}D.add(B);
