...
},getNewWidget:function(){var s=new qx.ui.core.Widget().set({width:50,height:50,backgroundColor:j});
s.addListener(h,function(e){if(this._active==s){return;
}var q=[];
var o=0.3;

if(this._active){this._active.set({backgroundColor:j,width:50,height:50});
}s.set({backgroundColor:b});
var r=s.getBounds();
q.push(new demobrowser.demo.layout.Grid_Animated_Property(s,f).set({from:r.width,to:200,duration:o,transition:i}));
q.push(new demobrowser.demo.layout.Grid_Animated_Property(s,d).set({from:r.height,to:200,duration:o,transition:i}));
var p=new qx.fx.effect.core.Parallel(q[0],q[1]);
p.start();
this._active=s;
},this);
return s;
