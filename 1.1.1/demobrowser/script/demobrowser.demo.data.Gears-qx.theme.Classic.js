...
var Q=new qx.ui.groupbox.GroupBox(r);
this.getRoot().add(Q,{left:10,top:50});
Q.setLayout(new qx.ui.layout.Grid(0,3));
Q.add(new qx.ui.basic.Label(z),{row:0,column:0});
Q.add(new qx.ui.basic.Label(u),{row:1,column:0});
Q.add(new qx.ui.basic.Label(c),{row:2,column:0});
var K=new qx.ui.form.TextField();
Q.add(K,{row:0,column:1});
var M=new qx.ui.form.TextField();
Q.add(M,{row:1,column:1});
var N=new qx.ui.form.Spinner(0,0,1000);
Q.add(N,{row:2,column:1});
var O=new qx.data.controller.Object();
O.addTarget(K,k,C,true);
O.addTarget(M,k,a,true);
O.addTarget(N,k,B,true);
var P=new demobrowser.demo.data.store.Gears();
P.bind(h,O,h);
...
}this.__ME=this.initDB();
this.__MF();
},members:{__MF:function(U){var V=this.getPerson();
this._marshaler.toClass(V,true);
this.setModel(this._marshaler.toModel(V));
this.getModel().addListener("changeBubble",this.__MG,this);
this.fireDataEvent("loaded",this.getModel());
},__MG:function(F){var G=this.getModel();
this.update(G.getFirstname(),G.getLastname(),G.getAge());
},update:function(R,S,T){if(!google.gears.factory||!this.__ME){return;
}this.__ME.execute(v);
this.__ME.execute(o,[R,S,T]);
},getPerson:function(){var D={};
D.firstname=i;
D.lastname=i;
D.age=0;

if(!google.gears.factory||!this.__ME){return D;
}var E=this.__ME.execute(w);

if(E.isValidRow()){D.firstname=E.field(0);
D.lastname=E.field(1);
D.age=parseInt(E.field(2));
}E.close();
return D;
},initDB:function(){if(window.google&&google.gears){try{var H=google.gears.factory.create(l);

if(H){H.open(y);
H.execute(q+t);
}}catch(I){this.error('Could not create database: '+I.message);
}}return H;
},initGears:function(){if(window.google&&google.gears){return;
}var J=null;
if(typeof GearsFactory!=j){J=new GearsFactory();
}else{try{J=new ActiveXObject(x);
if(J.getBuildInfo().indexOf(s)!=-1){J.privateSetGlobalObject(this);
}}catch(e){if((typeof navigator.mimeTypes!=j)&&navigator.mimeTypes[g]){J=document.createElement(m);
J.style.display=p;
J.width=0;
J.height=0;
J.type=g;
...
if(!google.gears){google.gears={factory:J};
}}}});
})();
