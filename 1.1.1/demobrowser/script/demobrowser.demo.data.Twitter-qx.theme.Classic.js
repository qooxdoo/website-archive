...
this.getRoot().add(N,{left:10,top:85});
N.setWidth(300);
var P=new qx.data.controller.List(null,N);
P.setDelegate(this);
P.setLabelPath(h);
if(!qx.core.Variant.isSet(g,c)){P.setIconPath(G);
}var T=new demobrowser.demo.data.store.Twitter(d);
T.bind(i,P,i);
var L=new qx.ui.form.Button(z);
this.getRoot().add(L,{left:10,top:55});
L.addListener(f,function(){T.setUrl(l);
},this);
var Q=new qx.ui.form.Button(o);
this.getRoot().add(Q,{left:90,top:55});
var I=new qx.ui.form.TextField(d);
this.getRoot().add(I,{left:140,top:55});
Q.addListener(f,function(){var bc=F+I.getValue()+b;

if(T.getUrl()==bc){T.reload();
}else{T.setUrl(bc);
}},this);
I.addListener(A,function(a){if(a.getKeyIdentifier()==E){Q.execute();
}},this);
var M=new qx.ui.groupbox.GroupBox(p);
this.getRoot().add(M,{left:320,top:60});
M.setWidth(300);
M.setHeight(220);
M.setLayout(new qx.ui.layout.Grid(0,5));
M.add(new qx.ui.basic.Label(q),{row:0,column:0});
M.add(new qx.ui.basic.Label(s),{row:1,column:0});
M.add(new qx.ui.basic.Label(v),{row:2,column:0});
M.add(new qx.ui.basic.Label(m),{row:3,column:0});
var name=new qx.ui.basic.Label();
M.add(name,{row:0,column:1});
var location=new qx.ui.basic.Label();
M.add(location,{row:1,column:1});
var R=new qx.ui.basic.Label();
R.setRich(true);
R.setWidth(150);
M.add(R,{row:2,column:1});
var U=new qx.ui.basic.Label();
U.setRich(true);
M.add(U,{row:3,column:1});
var J=new qx.data.controller.Object();
J.addTarget(name,k,t);
J.addTarget(location,k,u);
J.addTarget(R,k,h);
J.addTarget(U,k,e);

if(!qx.core.Variant.isSet(g,c)){M.add(new qx.ui.basic.Label(w),{row:4,column:0});
var O=new qx.ui.basic.Image();
M.add(O,{row:4,column:1});
J.addTarget(O,e,G);
}P.bind(x,J,i);
...
S.setWidth(260);
S.setValue(r+C);
this.getRoot().add(S,{left:10,top:290});
},configureItem:function(bd){bd.setRich(true);
bd.getChildControl(j).setWidth(50);
bd.getChildControl(j).setHeight(50);
bd.getChildControl(j).setScale(true);
}}});
qx.Class.define(B,{extend:qx.data.store.Json,statics:{saveResult:function(X){this.__MH=X;
}},construct:function(ba){var bb=F+ba+b;
qx.data.store.Json.call(this,bb);
},members:{_createRequest:function(V){var W=new qx.io.ScriptLoader();
V+=n;
W.load(V,function(Y){this.__MI();
},this);
},__MI:function(){var H=demobrowser.demo.data.store.Twitter.__MH;

if(H==undefined){this.setState("failed");
return;
}this._marshaler.toClass(H);
