...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.ui.DragDrop","qx.theme":"qx.theme.Modern"};
...
(function(){var m="drop",k="value",j="items",h="move",g="multi",f="dragover",d="icon/16/places/folder.png",c="dragstart",b="Item ",a="copy",A="drag",z="TextArea Target",y="solid",x="Reorderable",w="qx.ui.form.ListItem",v="droprequest",u="#33508D",t="Source",s="demobrowser.demo.ui.DragDrop",r="Simple Target",p="Enable drag",q="\n",n="dragend",o="__Nq";
qx.Class.define(s,{extend:qx.application.Standalone,members:{__Nq:null,__Nr:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var S=this.getRoot();
var M=new qx.ui.container.Scroll();
var H=new qx.ui.layout.Basic();
var K=new qx.ui.container.Composite(H).set({allowStretchY:false,allowStretchX:false});
M.add(K);
S.add(M,{edge:0});
var R=new qx.ui.basic.Label(t);
K.add(R,{left:20,top:20});
var P=new qx.ui.form.List;
P.setDraggable(true);
P.setSelectionMode(g);
K.add(P,{left:20,top:40});

for(var i=0;i<20;i++){P.add(new qx.ui.form.ListItem(b+i,d));
}var T=new qx.ui.form.CheckBox(p);
T.setValue(true);
K.add(T,{left:20,top:260});
P.addListener(c,function(e){if(!T.isValue()){e.preventDefault();
}e.addType(k);
e.addType(j);
e.addAction(a);
e.addAction(h);
});
P.addListener(v,function(e){this.debug("Related of droprequest: "+e.getRelatedTarget());
var D=e.getCurrentAction();
var F=e.getCurrentType();
var C;

switch(F){case j:C=this.getSelection();

if(D==a){var E=[];

for(var i=0,l=C.length;i<l;i++){E[i]=C[i].clone();
}C=E;
}break;
case k:C=this.getSelection()[0].getLabel();
break;
}if(D==h){var B=this.getSelection();

for(var i=0,l=B.length;i<l;i++){this.remove(B[i]);
}}e.addData(F,C);
});
var N=new qx.ui.basic.Label(r);
K.add(N,{left:140,top:20});
var L=new qx.ui.form.List;
L.setDroppable(true);
L.setSelectionMode(g);
K.add(L,{left:140,top:40});
L.addListener(m,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
var G=e.getData(j);

for(var i=0,l=G.length;i<l;i++){this.add(G[i]);
}});
L.addListener(f,function(e){if(!e.supportsType(j)){e.preventDefault();
}});
var N=new qx.ui.basic.Label(z);
K.add(N,{left:260,top:20});
var Q=new qx.ui.form.TextArea;
Q.setDroppable(true);
Q.setHeight(100);
K.add(Q,{left:260,top:40});
Q.addListener(m,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
this.setValue(e.getData(k).replace(/,/g,q));
});
Q.addListener(f,function(e){if(!e.supportsType(k)){e.preventDefault();
}});
var O=new qx.ui.basic.Label(x);
K.add(O,{left:500,top:20});
var I=this.__Nq=new qx.ui.form.List;
I.setDraggable(true);
I.setDroppable(true);
I.setSelectionMode(g);
K.add(I,{left:500,top:40});

for(var i=0;i<20;i++){I.add(new qx.ui.form.ListItem(b+i,d));
}var J=new qx.ui.core.Widget;
J.setDecorator(new qx.ui.decoration.Single().set({top:[1,y,u]}));
J.setHeight(0);
J.setOpacity(0.5);
J.setZIndex(100);
J.setLayoutProperties({left:-1000,top:-1000});
J.setDroppable(true);
this.getRoot().add(J);
I.addListener(c,function(e){e.addAction(h);
});
I.addListener(n,function(e){J.setDomPosition(-1000,-1000);
});
I.addListener(A,function(e){var X=e.getOriginalTarget();
if(X instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__Nr=X;
}
if(!qx.ui.core.Widget.contains(this,X)&&X!=J){return;
}var W=X.getContainerLocation();
J.setWidth(X.getBounds().width);
J.setDomPosition(W.left,W.top);
});
I.addListener(f,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
I.addListener(m,function(e){this.__Ns(e.getOriginalTarget());
},this);
J.addListener(m,function(e){this.__Ns(this.__Nr);
},this);
},__Ns:function(U){if(U.classname!=w){return;
}var V=this.__Nq.getSortedSelection();

for(var i=0,l=V.length;i<l;i++){this.__Nq.addBefore(V[i],U);
this.__Nq.addToSelection(V[i]);
