...
var A=new qx.ui.treevirtual.TreeVirtual(l);
A.setColumnWidth(0,400);
A.setAlwaysShowOpenCloseSymbol(true);
this.getRoot().add(A,{edge:30});
var F=A.getDataModel();
var C=F.addBranch(null,b,true);
var E;
F.addBranch(C,a,true);
E=F.addBranch(C,n,true);
F.addLeaf(E,v);
F.addLeaf(E,t);
F.addBranch(C,f,true);
F.addBranch(C,w,true);
var D=F.addBranch(null,c,true);
E=F.addBranch(D,d,false);

for(var i=1;i<3000;i++){F.addLeaf(E,m+i);
}F.addBranch(D,g,false);
F.addBranch(D,w,false);
F.addBranch(D,u,false);
F.addBranch(D,o,false);
F.setData();
var B=1;
A.addListener(r,function(e){alert('treeOpenWithContent ');
var x=e.getData();
F.addLeaf(x.nodeId,B.toString());
B++;
});
A.addListener(k,function(e){alert('treeClose ');
});
A.addListener(q,function(e){alert('treeOpenWhileEmpty');
var G=e.getData();
A.nodeSetHideOpenClose(G,true);
});
A.addListener(h,function(e){var y=j;
var z=e.getData();

for(i=0;i<z.length;i++){y+=s+z[i].label;
}alert('changeSelection: '+y);
});
}}});
})();
