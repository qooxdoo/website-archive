...
var Y={label:w,children:[{label:H,children:[{label:q},{label:l,children:[{label:a},{label:K}]},{label:B},{label:d}]},{label:I,children:[{label:C},{label:r},{label:d},{label:t},{label:z},{label:y},{label:F},{label:p,children:[]},{label:m}]}]};

for(var i=0;i<50;i++){Y.children[1].children[7].children[i]={label:A+i};
}this.extendData(Y);
var X=qx.data.marshal.Json.createModel(Y);
var ba=new qx.data.controller.Tree(null,W,s,v);
ba.setDelegate(this);
ba.setModel(X);
},extendData:function(Q){Q.date=u+Math.round(Math.random()*30+1)+o;
Q.size=Math.round(Math.random()*100)+x;
Q.light=Math.floor(Math.random()*4)==0;
Q.mode=D;
Q.checked=Math.random()>=0.5;

if(Q.children){for(var i=0;i<Q.children.length;i++){this.extendData(Q.children[i]);
}}},bindItem:function(R,S,T){R.bindDefaultProperties(S,T);
R.bindProperty(j,k,null,S.getUserData(j),T);
R.bindProperty(b,k,null,S.getUserData(e),T);
R.bindPropertyReverse(b,k,null,S.getUserData(e),T);
R.bindProperty(f,k,null,S.getUserData(f),T);
R.bindProperty(g,k,null,S.getUserData(g),T);
R.bindProperty(h,E,{converter:function(L){return L?n:c;
}},S.getUserData(h),T);
},createItem:function(){var M=new qx.ui.tree.TreeFolder();
...
M.setUserData(e,N);
M.addLabel(c);
M.addWidget(new qx.ui.core.Spacer(),{flex:1});
var P=new qx.ui.basic.Label();
P.setWidth(50);
M.addWidget(P);
M.setUserData(j,P);
P=new qx.ui.basic.Label();
P.setWidth(150);
M.addWidget(P);
M.setUserData(f,P);
P=new qx.ui.basic.Label();
