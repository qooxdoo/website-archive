...
var G=new qx.ui.progressive.renderer.table.Widths(1);
G.setWidth(0,j);
var A=new qx.ui.progressive.headfoot.Progress(G);
var E=new qx.ui.progressive.structure.Default(null,A);
var z=new qx.ui.progressive.Progressive(E);
z.setFlushWidgetQueueAfterBatch(true);
var D=function(T){var U={renderer:i,data:T};
return U;
};
var K=new qx.ui.progressive.model.Default();
var H=new qx.ui.progressive.renderer.FunctionCaller();
z.addRenderer(i,H);
var F=d;
var I=new qx.ui.basic.Image(F,j,j);
z.add(I);
z.set({height:100,width:272,zIndex:99999,backgroundColor:x,opacity:0.86,batchSize:10});
this.getRoot().add(z,{top:-1000,left:-1000});
var J=[{text:f,background:p,color:k},{text:v,background:c,color:k},{text:t,background:w,color:k},{text:m,background:q,color:k},{text:u,background:b,color:g}];
this.context={document:this.getRoot(),pages:J};
z.addListener(s,function(e){var bd=e.getData().state;
...
z.addListener(n,function(e){qx.event.Timer.once(function(){this.getLayoutParent().remove(this);
this.dispose();
},this,0);
});
K.addElement(D(function(O){var P=O.context;
...
for(var B=0;B<J.length;B++){K.addElement(D(function(V){var X=V.context;
...
for(var L=0;L<20;L++){K.addElement(D(function(bg){var bh=bg.context;
bh.hBox=new qx.ui.container.Composite(new qx.ui.layout.HBox(4));
bh.currentPage.add(bh.hBox);
bh.tabview.setSelection([bh.currentPage]);
bh.col=0;
}));

for(var C=0;C<10;C++){K.addElement(D(function(Q){var S=Q.context;
var R=S.pages[S.page];
var o=new qx.ui.basic.Atom(l+S.row+r+a+S.col);
o.set({width:90,backgroundColor:R.background,textColor:R.color});
S.hBox.add(o);
}));
K.addElement(D(function(M){++M.context.col;
}));
}K.addElement(D(function(bf){++bf.context.row;
}));
}K.addElement(D(function(N){++N.context.page;
}));
}K.addElement(D(function(y){y.context.tabview.setSelection([y.context.firstPage]);
}));
z.setDataModel(K);
z.render();
}}});
