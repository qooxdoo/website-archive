...
var settings = {"qx.application":"showcase.Application","qx.theme":"showcase.theme.Theme","qx.version":"1.1.1"};
...
var libinfo = {"__out__":{"sourceUri":"script"},"qx":{"resourceUri":"resource","sourceUri":"script","version":"1.1.1"},"showcase":{"resourceUri":"resource","sourceUri":"script","version":"trunk"}};
...
qx.$$loader = {
  ...
  urisBefore : [],
  packageHashes : {"0":"294c109e8a00","1":"e5447082f288","2":"e173f003f5ba","3":"449f73434488","4":"93f1f25ab5a1","5":"0ad8ec32d601","6":"08cb962ea207","7":"0667ab14a658","8":"a961380211d0","9":"17442660a12b","10":"cdd37ee3f8f0","11":"c6e71cd8178a","12":"fe5445a80c93"},
  boot : "boot",
...
this.set({name:e,part:c,icon:d,contentClass:a,description:showcase.page.DescriptionBuilder.build(e,this.__IS,null,this.__IT,this.__IU,this.__IV,this.__IW)});
},members:{__IS:"qooxdoo provides a powerful theming system built on a custom JSON-like "+"syntax. Unlike CSS this syntax doesn't have any cross "+"browser issues and allows styling of any widget property. It is "+"possible to create entirely different themes without touching the "+"application code."+"<p> The two calculators on this page share exactly the same application "+"and UI code and differ only in their theme.",__IT:{"States":"Widgets can have states like \"selected\" or "+"\"hovered\", which can be used by the theme to style the widgets.","Decorators":"Any HTML code can be used to style the background of a "+"widget independent of its content. Qooxdoo comes with a rich set of "+"pre-defined decorators."},__IU:{"ui_theming":"Theming","ui_appearance":"Appearance","ui_custom_themes":"Custom themes","ui_decorators":"Decorators"},__IV:{"#ui~Decoration.html":"This demo shows all available decorators."},__IW:{"#qx.theme":"This namespace contains the theme managers and the two standard themes."}}});
})();
...
}},"display/operation":{style:function(v){return {marginLeft:50};
...
(function(){var w="",v='#',u="String",t="request",s="mshtml",r="changeTitle",q="abstract",p="_applyState",o="qx.client",n="changeState",k="qx.bom.History",m="_applyTitle",l="qx.event.type.Data";
qx.Class.define(k,{extend:qx.core.Object,type:q,construct:function(){qx.core.Object.call(this);
this._baseUrl=window.location.href.split(v)[0]+v;
this.__tB={};
this._setInitialState();
},events:{"request":l},statics:{SUPPORTS_HASH_CHANGE_EVENT:(qx.bom.client.Engine.MSHTML&&document.documentMode>=8)||(!qx.bom.client.Engine.MSHTML&&document.documentMode&&"onhashchange" in window),getInstance:function(){if(!this.$$instance){if(this.SUPPORTS_HASH_CHANGE_EVENT){this.$$instance=new qx.bom.NativeHistory();
}else if(qx.core.Variant.isSet(o,s)){this.$$instance=new qx.bom.IframeHistory();
}else{this.$$instance=new qx.bom.NativeHistory();
}}return this.$$instance;
}},properties:{title:{check:u,event:r,nullable:true,apply:m},state:{check:u,event:n,nullable:true,apply:p}},members:{__tB:null,_applyState:function(h,i){this._writeState(h);
...
},_applyTitle:function(a){if(a!=null){document.title=a||w;
}},addToHistory:function(f,g){if(!qx.lang.Type.isString(f)){f=f+w;
}
...
(function(){var m="HTML Editor",l="showcase/htmleditor/icon.png",k="Overview HTML Editing",j="Low-Level editor",i="MenuBar",h="htmleditor",g="HTML Area",f="You can insert HTML tables, images, hyperlinks, ...",e="Editor widget",d="showcase.page.htmleditor.Content",a="showcase.page.htmleditor.Page",c="Menu",b="Toolbar";
qx.Class.define(a,{extend:showcase.Page,construct:function(){showcase.Page.call(this);
this.set({name:m,part:h,icon:l,contentClass:d,description:showcase.page.DescriptionBuilder.build(m,this.__IX,this.__IY,this.__Ja,this.__Jb,this.__Jc,this.__Jd)});
},members:{__IX:"The Html Editor, embedded here in a window with menu bar and toolbar, provides basic"+" cross-browser HTML editing capabilities and is available both as a low-level component"+" and as a qooxdoo widget. It offers events, allowing easy implementation of a toolbar "+"supplement. The UI controls of the toolbar can be used to interact "+"with the HTML editing component.",__IY:{"Text Formatting":"Format some text with underline, bold, italic, ...","Alignment":"Align the text on the right side.","Lists":"Insert a numbered or bullet point list.","Redo/Undo":"Try reverting your changes by using the undo button."},__Ja:{"Inserting":f},__Jb:{"ui_html_editing":k},__Jc:{"#bom~HtmlArea.html":j,"#widget~HtmlArea.html":e},__Jd:{"#qx.bom.htmlarea":g,"#qx.ui.toolbar":b,"#qx.ui.menubar":i,"#qx.ui.menu":c}}});
})();
...
this.set({name:j,part:w,icon:u,contentClass:a,description:showcase.page.DescriptionBuilder.build(j,this.__Je,this.__Jf,this.__Jg,this.__Jh,this.__Ji,this.__Jj)});
},members:{__Je:v+e,__Jf:{"Selection":d,"Text":q,"Buttons":n,"MenuButton":i},__Jg:{"Widgets":m,"Keyboard Navigation":k},__Jh:{"ui_form_handling":o},__Ji:{"#ui~FormRenderer.html":r,"#ui~FormRendererCustom.html":z,"#ui~FormRendererDouble.html":c,"#ui~FormRendererPlaceholder.html":l,"#ui~FormValidator.html":x,"#ui~MultiPageForm.html":b,"#showcase~Form.html":g,"#data~FormController.html":t,"#data~Form.html":f},__Jj:{"#qx.ui.form":h,"#qx.ui.form.renderer":p,"#qx.data.controller.Form":y}}});
})();
...
qx.Theme.define(d,{appearances:{"modern-calculator":e,"modern-calculator-button":b,"modern-display":{style:function(a){return {decorator:c,height:40,padding:3,marginBottom:10};
}},"modern-display/label":{style:function(i){return {font:f,marginLeft:5};
...
qx.Theme.define(t,{extend:qx.theme.modern.Appearance,include:[showcase.page.theme.calc.theme.appearance.Black,showcase.page.theme.calc.theme.appearance.Modern],appearances:{"root":{style:function(a){return {backgroundColor:j,textColor:r,font:o};
}},"page-preview":{alias:i,include:i,style:function(L){return {iconPosition:q,padding:[-10,-6,8,-6],gap:-20,decorator:null,cursor:y};
}},"page-preview/label":{include:m,style:function(M){return {textColor:M.selected?A:c,padding:[6,15],height:35,decorator:M.selected?v:null,font:qx.bom.Font.fromConfig({size:20,family:[B,s,l,E]}),zIndex:50};
}},"preview-list":{style:function(I){return {backgroundColor:z,decorator:new qx.ui.decoration.Single().set({bottom:[1,k,h],backgroundImage:b,backgroundRepeat:D}),shadow:e,zIndex:111,padding:5};
}},"preview-list/scrollbar-x/slider":g,"preview-list/scrollbar-x":g,"preview-list/scrollbar-x/button":{style:function(G){return {width:0,height:0};
}},"preview-list/scrollbar-x/button-begin":f,"preview-list/scrollbar-x/button-end":f,"preview-list/scrollbar-x/slider/knob":{style:function(K){return {decorator:new qx.ui.decoration.HBox(p),opacity:qx.bom.client.Engine.MSHTML?1:(K.hovered?1:0.6),height:12};
}},"separator":{style:function(N){return {backgroundColor:h,decorator:new qx.ui.decoration.Single().set({top:[1,k,j],bottom:[1,k,j]}),height:7};
...
qx.Class.define(B,{extend:showcase.Page,construct:function(){showcase.Page.call(this);
this.set({name:j,part:C,icon:n,contentClass:t,description:showcase.page.DescriptionBuilder.build(j,this.__Jk,this.__Jl,this.__Jm,this.__Jn,this.__Jo,this.__Jp)});
},members:{__Jk:d+e+o+w+q+k,__Jl:{"Sorting":b,"Reordering":x,"Column Resizing":m,"Hide Columns":p,"Table Resize":u},__Jm:{"Cell Renderer":c,"Header Renderer":y},__Jn:{"remote_table_model":z},__Jo:{"#table~Table.html":r,"#table~Table_Cell_Editor.html":A,"#table~Table_Conditional.html":v,"#table~Table_Context_Menu.html":s,"#table~Table_Events.html":l,"#table~Table_Filtered_Model.html":f,"#table~Table_Huge.html":g,"#table~Table_Meta_Columns.html":a,"#table~Table_Resize_Columns.html":h,"#table~Table_Selection.html":D,"#table~Table_Window_Editor.html":E},__Jp:{"#qx.ui.table.Table":i}}});
})();
...
this.set({name:g,part:c,icon:j,contentClass:f,description:showcase.page.DescriptionBuilder.build(p,this.__Jq,this.__Jr,this.__Js,this.__Jt,this.__Ju,this.__Jv)});
},members:{__Jq:d+b+l+e,__Jr:{"Drag":a,"Reorder":i,"Move":m},__Js:null,__Jt:{"ui_dragdrop":p},__Ju:{"#ui~DragDrop.html":k},__Jv:{"#qx.ui.core.Widget~drag":o,"#qx.ui.core.DragDropCursor":h}}});
})();
(function(){var s="Data Binding",r="data",q="showcase/databinding/icon.png",p="Clicking on a tweet will invoke a second binding which displays the ",o="The twitter demo illustrates the use of data binding. ",n="The data is loaded from twitter in real time via JSONP.",m="Clicking a tweet in the list shows the details.",l="showcase.page.databinding.Content",k="Data Binding Concepts",j="Enter your twitter username in the text field and press \"Show\" to see your recent tweets.",d="showcase.page.databinding.Page",i="The friends button displays the list of followers (Requires credentials).",g="selected tweet in the detail view right beside the list.",c="binding. The demo fetches the data and binds the result to the list. ",b="Fundamental Layer",f="Controller",e="A binding connects the model to the list view.",h="Twitter offers a REST / JSONP API, making it a perfect match for data ",a="Data Stores";
qx.Class.define(d,{extend:showcase.Page,construct:function(){showcase.Page.call(this);
this.set({name:s,part:r,icon:q,contentClass:l,description:showcase.page.DescriptionBuilder.build(s,this.__Jw,this.__Jx,this.__Jy,this.__Jz,this.__JA,this.__JB)});
},members:{__Jw:o+h+c+p+g,__Jx:{"Detail View":m,"Friends":i,"Change Tweet":j},__Jy:{"Loading Data":n,"Binding":e},__Jz:{"data_binding":k,"data_binding/single_value_binding":b,"data_binding/controller":f,"data_binding/stores":a},__JA:{"#data~SingleValueBinding.html":"Single Value Binding Demo","#data~ListControllerWith3Widgets.html":"List Binding Demo","#data~TreeController.html":"Tree Binding Demo","#data~FormController.html":"Form Binding Demo","#data~Flickr.html":"Flickr Foto Search"},__JB:{"#qx.data":"Main Data Binding API Documentation","#qx.data.store.Jsonp":"JSONP Data Store","#qx.data.controller.List":"List Controller","#qx.data.controller.Object":"Object Controller"}}});
})();
...
this.set({name:c,part:a,icon:d,contentClass:b,description:showcase.page.DescriptionBuilder.build(f,this.__JC,this.__JD,this.__JE,this.__JF,this.__JG,this.__JH)});
},members:{__JC:'Internationalization (or \"I18N\" for short) is all about making'+" a system support different natural languages and"+" locales in its user interface. qooxdoo has full translation support "+"and comprises locale information of virtually every country in the world.",__JD:{"Change the language":"Hit one of the flag buttons on the top to change "+"the language. All labels on the page will be translated, including "+"labels in standard qooxdoo widgets like the calendar.","Change the country":"The first select box on the left lets you change the country code. "+"The country code defines things like date or number formats. Try "+"changing the country code from <em>United States</em> to "+"<em>Great Britain</em>. You will see that e.g. the start of the week "+"changes from Sunday to Monday.","Open the command menu":"The command menu button in the lower left opens a popup menu. Even the "+"keyboard shortcuts are localized."},__JE:{"Standards based translation":"Translation data is extracted into standard <em>.po</em> "+"files, as defined by GNU <em>gettext</em> tools. Many "+"open source tools can process those translation files.","CLDR":"Localisation data like date and time formats are taken from the "+"<a href='http://cldr.unicode.org/'>Unicode Common Locale Data Repository</a> (CLDR). This "+"guarantees that qooxdoo uses the standardized data for even the smallest "+"countries.","Live locale switching":"All widgets are designed in a way that allows for locale switching in the running application."},__JF:{"internationalization":"Internationalization"},__JG:{"#showcase~Localization.html":"Localization demo"},__JH:{"#qx.locale":"qx.locale.*"}}});
})();
...
qx.Class.define(d,{extend:showcase.Page,construct:function(){showcase.Page.call(this);
this.set({name:e,part:b,icon:a,contentClass:c,description:showcase.page.DescriptionBuilder.build(e,this.__JI,this.__JJ,this.__JK,this.__JL,this.__JM,this.__JN)});
},members:{__JI:"The two tree views display some hierarchical data "+"including folders, icons and labels. The tree in the right window has "+"some additional infomation attached to each item.",__JJ:{"Expand":"Try expanding some folders by using the arrow icon or double clicking.","Resize":"Resize the window to make the tree scrollbars appear.","Selection":"Select multiple items, e.g. by holding the Shift key."},__JK:{"Configurable":"Each tree item has a configurable label and icon.","Scrolling":"Tree displays scrollbars if necessary.","Multi Columns":"Tree folders can display additional information in separate columns."},__JL:{"widget/tree":"The Tree Widget"},__JM:{"#widget~Tree.html":"Configurable Tree","#widget~Tree_Columns.html":"Multi Column Tree","#data~TreeController.html":"Tree using Data Binding","#data~JsonToTree.html":"Tree filled with Data from a JSON file.","#data~ExtendedTree.html":"Tree using Data Binding with configuration"},__JN:{"#qx.ui.tree":"The tree namespace"}}});
})();
...
qx.Class.define(n,{statics:{_demoPrefix:r+qx.core.Setting.get(q)+a,_apiPrefix:r+qx.core.Setting.get(q)+e,_manualPrefix:h,build:function(z,A,B,C,D,E,F){var G=[];
G.push(m,p,z,c,i,A,f);

if(B){G.push(g,this.__JP(B));
}
if(C){G.push(d,this.__JP(C));
}
if(D){G.push(k,this.__JO(this._manualPrefix,D));
}
if(E){G.push(j,this.__JO(this._demoPrefix,E));
}
if(F){G.push(b,this.__JO(this._apiPrefix,F));
}G.push(o);
return G.join(l);
},__JO:function(s,t){var u=["<ul>"];

for(var v in t){u.push("<li><a href='",s,v,"' target='_blank'>",t[v],"</a></li>");
}u.push("</ul>");
return u.join("");
},__JP:function(w){var x=["<ul>"];

for(var y in w){x.push("<li><strong>",y,"</strong>: ",w[y],"</li>");
}x.push("</ul>");
return x.join("");
}}});
})();
...
(function(){var k="complete",j="_applySelectedPage",h="Boolean",g="showcase/images/welcome.png",f="showcase.Application",e="__JV",d="__JS",c="__JQ",b="stack",a="showcase/images/loading66.gif",C="selection[0]",B="selection[0].readyState",A="value",z="selection[0].description",y="showcase",x="name",w="icon",v="selectedPage",u="showcase.Page",t="state",r="_applyShowLoadIndicator",s="en_US",p="page-preview",q="50%",n="__JU",o="__JT",l="selection[0].part",m="showLoadIndicator";
qx.Class.define(f,{extend:qx.application.Inline,properties:{selectedPage:{check:u,apply:j,nullable:true},showLoadIndicator:{check:h,init:false,apply:r}},members:{__JQ:null,__JR:null,__JS:null,__JT:null,__JU:null,__JV:null,main:function(){qx.application.Inline.prototype.main.call(this);
{};
...
this.__JQ=new qx.ui.container.Stack();
this.__JQ.set({appearance:b,maxWidth:600,allowGrowX:false});
J.add(this.__JQ,{row:T,column:0});
var N=new qx.ui.basic.Image(g).set({allowGrowX:true,allowGrowY:true,allowShrinkX:true,padding:[5,0,0,180]});
this.__JQ.add(N);
this.__JS=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
var Q=new qx.ui.basic.Image(a).set({marginLeft:-33});
this.__JS.add(Q,{left:q,top:200});
this.__JQ.add(this.__JS);
this.__JT=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
this.__JQ.add(this.__JT);
this.__JV=new showcase.ui.Description();
J.add(this.__JV,{row:T++,column:1});
this.__JV.exclude();
...
var S=new qx.data.controller.List(R,K,x);
S.setIconPath(w);
S.bind(C,this,v);
S.bind(z,this.__JV,A);
S.bind(B,this,m,{converter:function(E){return E!==k;
}});
S.setDelegate({configureItem:function(F){F.set({appearance:p});
}});
...
}}},_applyShowLoadIndicator:function(I){if(I){this.__JQ.setSelection([this.__JS]);
}else{this.__JQ.setSelection([this.__JT]);
}},_applySelectedPage:function(U,V){if(V){this._hidePage(V);
}this._showPage(U);
},_hidePage:function(W){if(this.getSelectedPage()!==W){if(W.getReadyState()==k){W.getContent().getView().hide();
this.__JW();
}}},_showPage:function(D){this.__JV.show();
D.load(function(G){if(this.getSelectedPage()==G){var H=G.getContent().getView();
this.__JT.add(H,{edge:0});
H.show();
this.__JX(H);
}},this);
},__JW:function(){if(this.__JU){this.__JU.cancel();
this.__JU.dispose();
this.__JU=null;
}},__JX:function(Y){if(qx.core.Variant.isSet("qx.client","mshtml")){return;
}Y.getContentElement().setStyle("display","none",true);
this.__JW();
qx.event.Timer.once(function(){var X=Y.getContentElement().getDomElement();
this.__JU=new qx.fx.effect.core.Fade(X);
this.__JU.set({from:0,to:1});
this.__JU.addListenerOnce("update",function(){Y.getContentElement().setStyle("display","block");
},this);
this.__JU.start();
},this,0);
}},destruct:function(){this._disposeObjects(c,d,o,e,n);
}});
})();
...
(function(){var p="dragover",o="items",n="drag",m="dragend",k="drop",j="droprequest",h="dragstart",g="groupbox/legend",f="printer",d="icon",bn="Battery",bm="Scanner",bl=".png",bk="Soundblaster",bj="Cart",bi="Cell Phone",bh="BluRay Drive",bg="computer",bf="WiFi",be="camera-photo",w="Printer",x="DVD",u="network-wired",v="Keyboard",s="HDD",t="center",q="name",r="Computer",A="pda",B="move",J="showcase.page.dragdrop.Content",H="middle",R="PDA",M="Camera",ba="selected",W="iPod",D="icon/64/actions/object-flip-horizontal.png",bd="network-wireless",bc="Mouse",bb="drive-optical",C="camera-web",F="media-flash",G="Display",I="Mic.",K="input-mouse",N="SD Card",T="Network Cable",Y="icon/64/devices/",y="battery",z="drive-harddisk",E="scanner",Q="audio-input-microphone",P="media-optical",O="Shop",V="phone",U="Webcam",L="input-keyboard",S="multimedia-player",c="audio-card",X="display";
qx.Class.define(J,{extend:showcase.AbstractContent,construct:function(bo){showcase.AbstractContent.call(this,bo);
this.setView(this._createView());
},members:{__Ig:null,__Ih:null,_createView:function(){var bw=new qx.ui.layout.Grid();
bw.setRowFlex(1,1);
bw.setColumnFlex(0,1);
bw.setColumnFlex(2,1);
bw.setColumnAlign(1,t,H);
var bx=new qx.ui.container.Composite(bw);
bx.setPadding(20);
bx.add(new qx.ui.basic.Label(O).set({appearance:g,paddingBottom:5}),{row:0,column:0});
bx.add(new qx.ui.basic.Label(bj).set({appearance:g,paddingBottom:5}),{row:0,column:2});
var bq=new qx.ui.form.List();
bq.set({draggable:true,droppable:true});
bx.add(bq,{row:1,column:0});
bq.addListener(h,this.__Ik,this);
bq.addListener(j,this.__Ij,this);
bq.addListener(k,this.__Ii,this);
bq.addListener(n,this.__Im,this);
bq.addListener(m,this.__Il,this);
var bu=new qx.ui.basic.Image(D);
bu.setPadding(10);
bx.add(bu,{row:1,column:1});
var bv=new qx.ui.form.List();
bv.set({draggable:true,droppable:true});
bx.add(bv,{row:1,column:2});
bv.addListener(h,this.__Ik,this);
bv.addListener(j,this.__Ij,this);
bv.addListener(k,this.__Ii,this);
bv.addListener(n,this.__Im,this);
bv.addListener(m,this.__Il,this);
var br=qx.data.marshal.Json.createModel([{"name":bm,"icon":E},{"name":bk,"icon":c},{"name":I,"icon":Q},{"name":bn,"icon":y},{"name":M,"icon":be},{"name":U,"icon":C},{"name":r,"icon":bg},{"name":G,"icon":X},{"name":s,"icon":z},{"name":bh,"icon":bb},{"name":v,"icon":L},{"name":bc,"icon":K},{"name":N,"icon":F},{"name":x,"icon":P},{"name":W,"icon":S},{"name":T,"icon":u},{"name":bf,"icon":bd},{"name":R,"icon":A},{"name":bi,"icon":V},{"name":w,"icon":f}]);
var bs=new qx.data.controller.List(null,bq);
bs.setLabelPath(q);
bs.setIconPath(d);
bs.setIconOptions({converter:function(bp){return Y+bp+bl;
}});
bs.setModel(br);
var bt=new qx.ui.form.ListItem();
this.__Ig=bt;
bt.setOpacity(0.5);
bt.setZIndex(500);
bt.addState(ba);
bt.setLayoutProperties({left:-1000,top:-1000});
qx.core.Init.getApplication().getRoot().add(bt);
return bx;
},__Ii:function(e){var bA=e.getData(o);
...
e.addData(o,a);
},__Ik:function(e){e.addType(o);
e.addAction(B);
var by=e.getTarget().getSelection()[0];
this.__Ig.set({label:by.getLabel(),icon:by.getIcon(),width:by.getBounds().width});
},__Il:function(e){this.__Ih&&this.__Ih.removeState(p);
this.__Ig.setDomPosition(-1000,-1000);
},__Im:function(e){var bz=e.getOriginalTarget();

if(bz instanceof qx.ui.form.ListItem){if(bz!=this.__Ih){this.__Ih&&this.__Ih.removeState(p);
bz.addState(p);
this.__Ih=bz;
}}else{this.__Ih&&this.__Ih.removeState(p);
