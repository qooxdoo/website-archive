...
qx.Bootstrap.define(b,{construct:function(u,v,w){this.__lp=w?f:h;
this.__lq=u;
this.__lr=v;
},members:{__lp:null,__lq:null,__lr:null,__ls:null,__lt:null,__lu:null,__lv:null,getId:function(){return this.__lr;
},getReadyState:function(){return this.__lp;
},getUrls:function(){return this.__lq;
},saveClosure:function(l){if(this.__lp==i){return;
}this.__ls=l;

if(!this.__lt){this.execute();
}else{clearTimeout(this.__lu);
this.__lp=a;
this.__lv(this);
}},execute:function(){if(this.__ls){this.__ls();
delete this.__ls;
}
if(qx.$$packageData[this.__lr]){this.__lx(qx.$$packageData[this.__lr]);
delete qx.$$packageData[this.__lr];
}this.__lp=f;
},loadClosure:function(j,self){if(this.__lp!==h){return;
}this.__lt=true;
this.__lp=g;
this.__lv=qx.Bootstrap.bind(j,self);
this.__lw(this.__lq,function(){},function(){this.__lp=i;
j.call(self,this);
},this);
var k=this;
this.__lu=setTimeout(function(){k.__lp=i;
j.call(self,k);
},qx.Part.TIMEOUT);
},load:function(m,self){if(this.__lp!==h){return;
}this.__lt=false;
this.__lp=g;
this.__lw(this.__lq,function(){this.__lp=f;
this.execute();
m.call(self,this);
},function(){this.__lp=i;
m.call(self,this);
},this);
},__lw:function(p,q,r,self){if(p.length==0){q.call(self);
return;
}var t=0;
var self=this;
var s=function(n){if(t>=p.length){q.call(self);
return;
}var o=new qx.io.ScriptLoader();
o.load(n.shift(),function(status){t+=1;
o.dispose();

if(status!==c){if(self.__lp==g){clearTimeout(self.__lu);
return r.call(self);
}}
if(qx.core.Variant.isSet(d,e)){setTimeout(function(){s.call(self,n,q,self);
},0);
}else{s.call(self,n,q,self);
}},self);
};
s(p.concat());
},__lx:qx.$$loader.importPackageData}});
})();
...
qx.Class.define(f,{extend:qx.core.Object,type:a,members:{loadAll:function(n){var p=n.getFeeds().getItem(0).getFeeds();
...
}},load:function(t){t.setState(d);
var u=j+t.getUrl()+h;
var v=new qx.data.store.Yql(u,{manipulateData:function(r){if(r.query.results){return r.query.results.item||r.query.results.entry;
}return m;
}},qx.bom.client.Feature.SSL);
v.addListener(k,this.__lI(t),this);
v.addListener(c,qx.lang.Function.bind(this.__lH,this,t),this);
},__lH:function(q,e){if(e.getData()==b||e.getData()==g||e.getData()==m){q.setState(l);
}},__lI:function(w){return function(e){var s=e.getData();
if(s==m){w.setState(l);
return;
}w.setArticles(s);
w.setState(k);
...
qx.Bootstrap.define(a,{construct:function(name,l,m){this.__lJ=name;
this._readyState=e;
this._packages=l;
this._loader=m;

for(var i=0;i<l.length;i++){if(l[i].getReadyState()!==e){this._readyState=b;
break;
}}},members:{_readyState:null,_loader:null,_packages:null,__lJ:null,getReadyState:function(){return this._readyState;
},getName:function(){return this.__lJ;
},getPackages:function(){return this._packages;
},preload:function(n,self){if(n){window.setTimeout(function(){n.call(self,this);
},0);
}},load:function(o,self){if(this._checkCompleteLoading(o,self)){return;
}this._readyState=d;

if(o){this._appendPartListener(o,self,this);
}var q=this;
var p=function(){q.load();
};

for(var i=0;i<this._packages.length;i++){var r=this._packages[i];

switch(r.getReadyState()){case b:this._loader.addPackageListener(r,p);
r.load(this._loader.notifyPackageResult,this._loader);
return;
case d:this._loader.addPackageListener(r,p);
return;
case e:break;
case c:this._markAsCompleted(c);
return;
default:throw new Error("Invalid case! "+r.getReadyState());
}}this._markAsCompleted(e);
},_appendPartListener:function(j,self,k){this._loader.addPartListener(this,function(){j.call(self,k._readyState);
});
},_markAsCompleted:function(h){this._readyState=h;
this._loader.notifyPartResult(this);
},_checkCompleteLoading:function(f,self){var g=this._readyState;

if(g==e||g==c){if(f){setTimeout(function(){f.call(self,g);
},0);
}return true;
...
},properties:{callbackParam:{check:a,nullable:false}},members:{__mg:null,_createRequest:function(j){if(this.__mg){this.__mg.dispose();
}this.__mg=new qx.io.ScriptLoader();
var l=this._delegate;

if(l&&qx.lang.Type.isFunction(l.configureRequest)){this._delegate.configureRequest(this.__mg);
}var k=j.indexOf(i)==-1?i:h;
j+=k+this.getCallbackParam()+c;
var m=parseInt(this.toHashCode());
qx.data.store.Jsonp[m]=this;
j+=f+m+e;
this.__mg.load(j,function(o){delete this[m];
},this);
},callback:function(n){if(this.isDisposed()){return;
}this.__mh(n);
},__mh:function(p){if(p==undefined){this.setState(d);
return;
}var q=this._delegate;

if(q&&qx.lang.Type.isFunction(q.manipulateData)){p=this._delegate.manipulateData(p);
}this._marshaler.toClass(p);
this.setModel(this._marshaler.toModel(p));
this.fireDataEvent(g,this.getModel());
}},destruct:function(){if(this.__mg){this.__mg.dispose();
}this.__mg=null;
}});
})();
...
(function(){var r="static",q="user",p="execute",o="loading",n="title",m="north",l="main",k="new",j="Surfin' Safari",h="changeModel",bz="__ov",by="__oy",bx="selection[0].state",bw="Heise",bv="F1",bu="article",bt="partLoaded",bs="Control+A",br="http://blogs.msdn.com/ie/rss.xml",bq="http://my.opera.com/desktopteam/xml/rss/blog/",y="settings",z="feedreader/images/loading22.gif",w="error",x="selection[0]",u="change",v="http://blogs.msdn.com/jscript/rss.xml",s="__oB",t="__oA",G="loaded",H="__oE",T="feeds",P="The Mozilla Blog",bc="enabled",W="__or",bm="http://webkit.org/blog/feed/",bh="separator-vertical",L="http://feeds2.feedburner.com/qooxdoo/news/content",bp="http://blog.mozilla.com/feed/",bo="vertical",bn="__ox",K="http://www.heise.de/newsticker/heise-atom.xml",N="__oz",O="state",R="JScript Team Blog",U="Feeds",X="http://feeds2.feedburner.com/ajaxian",be="Control+D",bj="icon/22/actions/process-stop.png",A="selection[0].articles",B="white",M="IEBlog",bb="Ajaxian",ba="__ou",Y="User Feeds",bg="icon/22/apps/internet-feed-reader.png",bf="model",V="feedreader.Application",bd="http://daringfireball.net/index.xml",d="Control+P",bi="Static Feeds",C="__ow",D="Control+R",Q="qooxdoo News",f="selection[0].category",g="__oD",J="Opera Desktop Blog",E="feedreader.model.Feed",F="Daring Fireball",I="__os",S="__oF",bl="__oC",bk="addfeed";
qx.Class.define(V,{extend:qx.application.Standalone,members:{__or:null,__os:null,__ot:null,__ou:null,__ov:null,__ow:null,__ox:null,__oy:null,__oz:null,__oA:null,__oB:null,__oC:null,__oD:null,__oE:null,__oF:null,main:function(){qx.application.Standalone.prototype.main.call(this);
{};
qx.io.PartLoader.getInstance().addListener(bt,function(e){this.debug("part loaded: "+e.getData().getName());
},this);
this._initializeCommands();
this._createLayout();
this._initializeModel();
this._setUpBinding();
this.__ou.getRoot().setOpen(true);
this.__ou.getRoot().getChildren()[0].setOpen(true);
this.__ou.getRoot().getChildren()[1].setOpen(true);
this.__ou.setHideRoot(true);
this.__or.getSelection().push(this.__oF.getFeeds().getItem(0));
},finalize:function(){qx.application.Standalone.prototype.finalize.call(this);
this.reload();
},_initializeModel:function(){this.__oD=new feedreader.model.FeedFolder(U);
this.__oF=new feedreader.model.FeedFolder(this.tr(bi));
this.__oD.getFeeds().push(this.__oF);
this.__oF.getFeeds().push(new feedreader.model.Feed(Q,L,r));
this.__oF.getFeeds().push(new feedreader.model.Feed(R,v,r));
this.__oF.getFeeds().push(new feedreader.model.Feed(F,bd,r));
this.__oF.getFeeds().push(new feedreader.model.Feed(j,bm,r));
this.__oF.getFeeds().push(new feedreader.model.Feed(bb,X,r));
this.__oE=new feedreader.model.FeedFolder(this.tr(Y));
this.__oD.getFeeds().push(this.__oE);
this.__oE.getFeeds().push(new feedreader.model.Feed(bw,K,q));
this.__oE.getFeeds().push(new feedreader.model.Feed(M,br,q));
this.__oE.getFeeds().push(new feedreader.model.Feed(P,bp,q));
this.__oE.getFeeds().push(new feedreader.model.Feed(J,bq,q));
},_setUpBinding:function(){var bI={converter:this._state2iconConverter};
this.__or=new qx.data.controller.Tree(this.__oD,this.__ou,T,n);
this.__or.setIconOptions(bI);
this.__or.setIconPath(O);
this.__os=new qx.data.controller.List(null,this.__ox.getList(),n);
this.__or.bind(A,this.__os,bf);
this.__os.bind(x,this.__oB,bu);
this.__os.getSelection().addListener(u,this._listControllerChange,this);
this.__os.addListener(h,this._treeControllerChange,this);
var bJ={converter:this._state2loadingConverter};
this.__or.bind(bx,this.__ox,o,bJ);
bJ={converter:this._category2enabledConverter};
this.__or.bind(f,this.__ow.getRemoveButton(),bc,bJ);
},_category2enabledConverter:function(bK){if(bK==q){return true;
}return false;
},_state2iconConverter:function(bH){if(bH==k||bH==o){return z;
}else if(bH==G){return bg;
}else if(bH==w){return bj;
}return null;
},_state2loadingConverter:function(bO){if(bO==k||bO==o){return true;
}return false;
},_listControllerChange:function(a){var b=this.__or.getSelection().getItem(0);
var c=this.__os.getSelection().getItem(0);
if(c!=undefined){b.setSelectedArticle(c);
}},_treeControllerChange:function(bM){var bN=this.__or.getSelection().getItem(0);
if(bN!=null&&bN.classname==E&&bN.getSelectedArticle()!=null&&bN.getArticles().contains(bN.getSelectedArticle())){this.__os.getSelection().push(bN.getSelectedArticle());
}else{this.__ox.getList().scrollToY(0);
}},_createLayout:function(){var bR=new qx.ui.layout.Dock();
bR.setSeparatorY(bh);
var bS=new qx.ui.container.Composite(bR);
this.getRoot().add(bS,{edge:0});
this.__ov=new feedreader.view.Header();
bS.add(this.__ov,{edge:m});
this.__ow=new feedreader.view.ToolBar(this);
bS.add(this.__ow,{edge:m});
this.__oy=new qx.ui.splitpane.Pane();
bS.add(this.__oy);
this.__ou=new qx.ui.tree.Tree();
this.__ou.setWidth(250);
this.__ou.setBackgroundColor(B);
this.__oy.add(this.__ou,0);
this.__oz=new qx.ui.splitpane.Pane(bo);
this.__oz.setDecorator(null);
this.__oy.add(this.__oz,1);
this.__ox=new feedreader.view.List(this.__oD);
this.__ox.setHeight(200);
this.__ox.setDecorator(l);
this.__oz.add(this.__ox,0);
this.__oB=new feedreader.view.Article();
this.__oB.setDecorator(l);
this.__oz.add(this.__oB,1);
},_initializeCommands:function(){var bB={};
bB.reload=new qx.ui.core.Command(D);
bB.reload.addListener(p,this.reload,this);
bB.about=new qx.ui.core.Command(bv);
bB.about.addListener(p,this.showAbout,this);
bB.preferences=new qx.ui.core.Command(d);
bB.preferences.addListener(p,this.showPreferences,this);
bB.addFeed=new qx.ui.core.Command(bs);
bB.addFeed.addListener(p,this.showAddFeed,this);
bB.removeFeed=new qx.ui.core.Command(be);
bB.removeFeed.addListener(p,this.removeFeed,this);
this.__ot=bB;
},getCommand:function(bL){return this.__ot[bL];
},addFeed:function(bC,bD,bE){var bF=new feedreader.model.Feed(bC,bD,bE);
this.__oE.getFeeds().push(bF);
var bG=feedreader.io.FeedLoader.getInstance();
bG.load(bF);
},removeFeed:function(){var bP=this.__or.getSelection().getItem(0);
if(bP&&bP.getCategory&&bP.getCategory()!==r){var bQ=this.__oE.getFeeds();
for(var i=0;i<bQ.length;i++){if(bP===bQ.getItem(i)){bQ.splice(i,1);
return;
}}}},reload:function(){var bA=feedreader.io.FeedLoader.getInstance();
bA.loadAll(this.__oD);
},showPreferences:function(){qx.io.PartLoader.require([y],function(){if(!this.__oA){this.__oA=new feedreader.view.PreferenceWindow();
this.getRoot().add(this.__oA);
}this.__oA.center();
this.__oA.open();
},this);
},showAbout:function(){alert(this.tr("FeedReader (qooxdoo powered)"));
},showAddFeed:function(){qx.io.PartLoader.require([bk],function(){if(!this.__oC){this.__oC=new feedreader.view.AddFeedWindow(this);
this.getRoot().add(this.__oC);
}this.__oC.center();
this.__oC.open();
},this);
}},destruct:function(){this.__ot=null;
this._disposeObjects(C,bn,s,ba,g,by,N,bz,S,H,W,I,t,bl);
}});
})();
...
C.add(D);
this.__pc=new qx.ui.toolbar.Button(this.tr(e),n);
this.__pc.setCommand(v.getCommand(r));
this.__pc.setEnabled(false);
C.add(this.__pc);
C.addSeparator();
...
B.add(A);
},members:{__pc:null,getRemoveButton:function(){return this.__pc;
...
(function(){var n="separator-vertical",m="__pH",l="__pF",k="background-medium",j="Posts",i="bold",h="_applyLoading",g="__pG",f="center",e="feedreader.view.List",b="single",d="middle",c="feedreader/images/loading66.gif",a="Boolean";
qx.Class.define(e,{extend:qx.ui.core.Widget,construct:function(){qx.ui.core.Widget.call(this);
var r=new qx.ui.layout.VBox();
r.setSeparator(n);
this._setLayout(r);
var s=new qx.ui.basic.Label(this.tr(j));
s.setBackgroundColor(k);
s.setPadding(5);
s.setAllowGrowX(true);
s.setFont(i);
this._add(s);
this.__pF=new qx.ui.container.Stack();
this._add(this.__pF,{flex:1});
this.__pG=new qx.ui.form.List();
this.__pG.setDecorator(null);
this.__pG.setSelectionMode(b);
this.__pF.add(this.__pG);
this.__pH=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,f));
var q=new qx.ui.basic.Image(c);
q.setAlignY(d);
this.__pH.add(q);
this.__pF.add(this.__pH);
},properties:{loading:{check:a,init:false,apply:h}},members:{__pF:null,__pG:null,__pH:null,_applyLoading:function(o,p){if(o){this.__pF.setSelection([this.__pH]);
}else{this.__pF.setSelection([this.__pG]);
}},getList:function(){return this.__pG;
}},destruct:function(){this._disposeObjects(g,l,m);
}});
})();
...
qx.Bootstrap.define(b,{construct:function(P){var X=P.parts[P.boot][0];
this.__pJ=P;
this.__pK={};
this.__pL={};
this.__pM={};
...
for(var i=0;i<Y.length;i++){var Q=P.packageHashes[i];
var U=new qx.io.part.Package(Y[i],Q,i==X);
this.__pN.push(U);
}this.__pO={};
var T=P.parts;
var R=P.closureParts||{};

for(var name in T){var S=T[name];
var W=[];

for(var i=0;i<S.length;i++){W.push(this.__pN[S[i]]);
}if(R[name]){var V=new qx.io.part.ClosurePart(name,W,this);
}else{var V=new qx.io.part.Part(name,W,this);
}this.__pO[name]=V;
}},statics:{TIMEOUT:7500,getInstance:function(){if(!this.$$instance){this.$$instance=new this(qx.$$loader);
}return this.$$instance;
},require:function(I,J,self){this.getInstance().require(I,J,self);
},preload:function(L){this.getInstance().preload(L);
},$$notifyLoad:function(C,D){this.getInstance().saveClosure(C,D);
}},members:{__pJ:null,__pN:null,__pO:null,__pM:null,addToPackage:function(E){this.__pN.push(E);
},addClosurePackageListener:function(F,G){var H=F.getId();

if(!this.__pM[H]){this.__pM[H]=[];
}this.__pM[H].push(G);
},saveClosure:function(y,z){var B;

for(var i=0;i<this.__pN.length;i++){if(this.__pN[i].getId()==y){B=this.__pN[i];
break;
}}if(!B){throw new Error("Package not available: "+y);
}B.saveClosure(z);
var A=this.__pM[y];

if(!A){return;
}
for(var i=0;i<A.length;i++){A[i](a,y);
}this.__pM[y]=[];
},getParts:function(){return this.__pO;
},require:function(j,k,self){var k=k||function(){};
var self=self||window;

if(qx.Bootstrap.isString(j)){j=[j];
}var n=[];

for(var i=0;i<j.length;i++){n.push(this.__pO[j[i]]);
}var m=0;
var l=function(){m+=1;
if(m>=n.length){var u=[];

for(var i=0;i<n.length;i++){u.push(n[i].getReadyState());
}k.call(self,u);
}};
...
for(var i=0;i<v.length;i++){this.__pO[v[i]].preload(function(){x++;

if(x>=v.length){var K=[];

for(var i=0;i<v.length;i++){K.push(this.__pO[v[i]].getReadyState());
}
if(w){w.call(self,K);
}}},this);
}},__pP:function(){var d=this.__pJ.uris;
var e=[];

for(var i=0;i<d.length;i++){e.push(this.__pQ(d[i]));
}return e;
},__pQ:qx.$$loader.decodeUris,__pK:null,addPartListener:function(M,N){var O=M.getName();

if(!this.__pK[O]){this.__pK[O]=[];
}this.__pK[O].push(N);
},onpart:null,notifyPartResult:function(o){var q=o.getName();
var p=this.__pK[q];

if(p){for(var i=0;i<p.length;i++){p[i](o.getReadyState());
}this.__pK[q]=[];
}
if(typeof this.onpart==c){this.onpart(o);
}},__pL:null,addPackageListener:function(r,s){var t=r.getId();

if(!this.__pL[t]){this.__pL[t]=[];
}this.__pL[t].push(s);
},notifyPackageResult:function(f){var h=f.getId();
var g=this.__pL[h];

if(!g){return;
}
for(var i=0;i<g.length;i++){g[i](f.getReadyState());
}this.__pL[h]=[];
}}});
})();
...
var self=this;
g.onpart=function(j){if(j.readyState==b){self.fireDataEvent(c,j);
}else{self.fireDataEvent(e,j.name);
}};
},events:{"partLoaded":f,"partLoadingError":f},statics:{require:function(k,l,self){this.getInstance().require(k,l,self);
}},members:{require:function(h,i,self){return this._loader.require(h,i,self);
...
},members:{__pV:0,preload:function(h,self){var j=0;
var l=this;

for(var i=0;i<this._packages.length;i++){var k=this._packages[i];

if(k.getReadyState()==d){k.loadClosure(function(p){j++;
l._loader.notifyPackageResult(p);
...
}this.__pV=this._packages.length;

for(var i=0;i<this._packages.length;i++){var o=this._packages[i];
var n=o.getReadyState();
if(n==d){o.loadClosure(this._loader.notifyPackageResult,this._loader);
}if(n==d||n==c){this._loader.addPackageListener(o,qx.Bootstrap.bind(this._onPackageLoad,this,o));
}else if(n==e){this._markAsCompleted(e);
return;
}else{this.__pV--;
}}if(this.__pV<=0){this.__pW();
}},__pW:function(){for(var i=0;i<this._packages.length;i++){this._packages[i].execute();
...
if(!feedreader.view.Article.__qp){feedreader.view.Article.__qp=true;
...
w.setLineHeight(1.8);
this.setFont(w);
},properties:{article:{apply:e,nullable:true,check:d}},members:{_applyArticle:function(x,y){var z=this.__qq();
this.setHtml(z);
qx.html.Element.flush();
var B=this.getContentElement().getDomElement();
var A=B.getElementsByTagName(b);
for(var i=0;i<A.length;i++){A[i].target=t;
}},__qq:function(){var D=this.getArticle();
if(!D){return v;
