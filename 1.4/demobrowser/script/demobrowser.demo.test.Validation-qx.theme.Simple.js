...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Validation","qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.test.Validation-qx.theme.Simple.js"]],
  urisBefore : [],
  packageHashes : {"0":"90e50ddeb9c9"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var j="<br>",i="new name: ",h="new gender: ",g="male",f="female",e="new age: ",d="demobrowser.demo.test.Validation",c="<b>Setting the age ",b="property to 27</b><br>",a="- name: Name is validated to a minimum length of two.<br/>",M="<br>old name: ",L="age:",K="fmale",J="<br><b>Setting the name to 'Hans'</b><br>",I="property to 'female'</b><br>",H="<br><b>Setting the gender ",G="<b>Testing the gender ",F="<br>old gender: ",E="<br><b>Setting the age ",D="- age: Age is validated to be a number.<br/>",q="<b>Initial property values</b><br>",r="",o="<b>Setting the name to ''</b><br>",p="demobrowser.demo.test.Person",m="property to 'true'</b><br>",n="- gender: Gender is validated to be either male or female.",k="property to 'fmale'</b><br>",l="<font color='red'>GENDER NOT SET</font>",s="gender:",t="<br>old age: ",w="Hans",v="The demo creates an internal class containing three properties:<br/>",y="<font color='red'>AGE NOT SET</font>",x="My name",A="<font color='red'>NAME NOT SET</font>",z="white",u="name:",C="__US",B="<b>Description</b><br/>";
...
var N=new demobrowser.demo.test.Person();
var O=new qx.ui.embed.Html();
O.setBackgroundColor(z);
O.set({padding:10,width:250,height:360});
this.getRoot().add(O,{left:10,top:10});
O.setHtml(q);
O.setHtml(O.getHtml()+u+N.getName()+j);
O.setHtml(O.getHtml()+L+N.getAge()+j);
O.setHtml(O.getHtml()+s+N.getGender()+j);
O.setHtml(O.getHtml()+J);
N.setName(w);
O.setHtml(O.getHtml()+i+N.getName()+j);
O.setHtml(O.getHtml()+o);
try{N.setName(r);
O.setHtml(O.getHtml()+i+N.getName()+j);
}catch(Q){O.setHtml(O.getHtml()+A+M+N.getName()+j);
}O.setHtml(O.getHtml()+E+b);
N.setAge(27);
O.setHtml(O.getHtml()+e+N.getAge()+j);
O.setHtml(O.getHtml()+c+m);
try{N.setAge(true);
O.setHtml(O.getHtml()+e+N.getAge()+j);
}catch(R){O.setHtml(O.getHtml()+y+t+N.getAge()+j);
}O.setHtml(O.getHtml()+H+I);
N.setGender(f);
O.setHtml(O.getHtml()+h+N.getGender()+j);
O.setHtml(O.getHtml()+G+k);
try{N.setAge(K);
O.setHtml(O.getHtml()+h+N.getGender()+j);
}catch(S){O.setHtml(O.getHtml()+l+F+N.getGender()+j);
}var P=new qx.ui.basic.Label();
P.setRich(true);
P.setWidth(250);
P.setValue(B+v+a+D+n);
this.getRoot().add(P,{left:280,top:10});
}}});
qx.Class.define(p,{extend:qx.core.Object,properties:{name:{init:x,validate:C},age:{init:18,validate:qx.util.Validate.number()},gender:{init:g,validate:qx.util.Validate.inArray([g,f])}},members:{__US:function(T){if(T.length<2){throw new qx.core.ValidationError("Validation Error: Name "+"must be longer than one characters. ("+T+")");
}}}});
})();
