...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Selection","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.table.Table_Selection-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"0e82a07b1289"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var k="execute",j="value",i="",h="A date",g="Boolean",f="demobrowser.demo.table.Table_Selection",d="changeValue",c="icon/22/status/dialog-information.png",b="A number",a="icon/16/apps/office-calendar.png",z="Reset Cell Focus",y="changeSelection",x="middle",w="Remove 5 rows",v="Reset Selection",u="_tableModel",t="Table",s="Add 5 rows",r="icon/22/actions/list-remove.png",q=", ",o="icon/22/actions/list-add.png",p="Hide Cell Focus",m="Selected rows:<br>",n="Show selection",l="ID";
qx.Class.define(f,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return t;
},createTable:function(){var C=this.createRandomRows(50);
var B=this._tableModel=new qx.ui.table.model.Simple();
B.setColumns([l,b,h,g]);
B.setData(C);
...
A.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(a,h));
return D;
},createControls:function(){var F=new qx.ui.toolbar.ToolBar();
var H,J;
J=new qx.ui.toolbar.Part();
F.add(J);
var E=new qx.ui.form.SelectBox().set({allowGrowY:false,alignY:x});
var M=qx.ui.table.selection.Model;
var G={"Single selection":M.SINGLE_SELECTION,"Single interval":M.SINGLE_INTERVAL_SELECTION,"Multiple intervals":M.MULTIPLE_INTERVAL_SELECTION,"Multiple intervals (toggle)":M.MULTIPLE_INTERVAL_SELECTION_TOGGLE,"No selection":M.NO_SELECTION};

for(var L in G){var I=new qx.ui.form.ListItem(L);
I.setUserData(j,G[L]+i);
E.add(I);
}E.addListener(y,function(e){this._table.getSelectionModel().setSelectionMode(parseInt(e.getData()[0].getUserData(j)));
},this);
J.add(E);
J=new qx.ui.toolbar.Part();
F.add(J);
H=new qx.ui.toolbar.Button(s,o);
H.addListener(k,function(N){var O=this.createRandomRows(10);
this._tableModel.addRows(O);
this.info("5 rows added");
},this);
J.add(H);
H=new qx.ui.toolbar.Button(w,r);
H.addListener(k,function(P){var Q=this._tableModel.getRowCount();
this._tableModel.removeRows(Q-5,5);
this.info("5 rows removed");
},this);
J.add(H);
J=new qx.ui.toolbar.Part();
F.add(J);
H=new qx.ui.toolbar.Button(v);
H.addListener(k,function(R){this._table.resetSelection();
},this);
J.add(H);
H=new qx.ui.toolbar.Button(z);
H.addListener(k,function(S){this._table.resetCellFocus();
},this);
J.add(H);
var K=new qx.ui.toolbar.CheckBox(p);
K.addListener(d,function(T){this._table.setShowCellFocusIndicator(!K.isValue());
},this);
J.add(K);
H=new qx.ui.toolbar.Button(n,c);
H.addListener(k,function(U){var V=[];
this._table.getSelectionModel().iterateSelection(function(W){V.push(W+i);
});
this.showDialog(m+V.join(q));
},this);
J.add(H);
return F;
