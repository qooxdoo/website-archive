...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.root.Inline_Dynamic_Resize","qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.root.Inline_Dynamic_Resize-qx.theme.Simple.js"]],
  urisBefore : [],
  packageHashes : {"0":"e96ada52b7a9"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var u="engine.name",t="relative",s="resize",r="mshtml",q="height",p="",o="px",n="position",m="qx.debug",l="engine.version",c="qx.ui.root.Inline",k="$$widget",g="hidden",b="div",a="left",f="opera",d="appear",h="Please use a DOM element to create an inline root.";
qx.Class.define(c,{extend:qx.ui.root.Abstract,include:[qx.ui.core.MLayoutHandling],construct:function(v,w,x){if(qx.core.Environment.get(m)){this.assertElement(v,h);
}this.__vA=v;
v.style.overflow=g;
v.style.textAlign=a;
this.__vB=w||false;
...
qx.ui.core.queue.Layout.add(this);
qx.ui.core.FocusHandler.getInstance().connectTo(this);
if((qx.core.Environment.get(u)==f)){this.setSelectable(true);
}if((qx.core.Environment.get(u)==r)){this.setKeepFocus(true);
}},members:{__vB:false,__vC:false,__vA:null,__vD:function(){if(this.__vB||this.__vC){var y=qx.bom.element.Dimension.getSize(this.__vA);

if(this.__vB&&y.width<1){throw new Error("The root element "+this.__vA+" of "+this+" needs a width when its width size should be used!");
}
if(this.__vC){if(y.height<1){throw new Error("The root element "+this.__vA+" of "+this+" needs a height when its height size should be used!");
}if(y.height>=1&&qx.bom.element.Style.get(this.__vA,q,3)==p){qx.bom.element.Style.set(this.__vA,q,y.height+o);
}}qx.event.Registration.addListener(this.__vA,s,this._onResize,this);
}},_createContainerElement:function(){var z=this.__vA;

if(this.__vB||this.__vC){var D=document.createElement(b);
z.appendChild(D);
if((qx.core.Environment.get(u)==r)&&qx.core.Environment.get(l)==6){var C=qx.dom.Node.getBodyElement(z);
var B;
var F;
var E=false;
var A=qx.dom.Hierarchy.getAncestors(z);

for(var i=0,j=A.length;i<j;i++){B=A[i];

if(B!=C){F=qx.bom.element.Style.get(B,n);

if(F==t){E=true;
break;
}}else{break;
}}
if(E){z.style.position=t;
}}}else{D=z;
}var G=new qx.html.Root(D);
D.style.position=t;
G.setAttribute(k,this.toHashCode());
qx.event.Timer.once(function(e){this.fireEvent(d);
},this,0);
return G;
},_onResize:function(e){var H=e.getData();

if((H.oldWidth!==H.width)&&this.__vB||(H.oldHeight!==H.height)&&this.__vC){qx.ui.core.queue.Layout.add(this);
}},_computeSizeHint:function(){var M=this.__vB;
var J=this.__vC;

if(!M||!J){var I=qx.ui.root.Abstract.prototype._computeSizeHint.call(this);
}else{I={};
}var N=qx.bom.element.Dimension;

if(M){var L=N.getContentWidth(this.__vA);
I.width=L;
I.minWidth=L;
I.maxWidth=L;
}
if(J){var K=N.getContentHeight(this.__vA);
I.height=K;
I.minHeight=K;
I.maxHeight=K;
}return I;
}},defer:function(O,P){qx.ui.core.MLayoutHandling.remap(P);
},destruct:function(){qx.event.Registration.removeListener(this.__vA,s,this._onResize,this);
this.__vA=null;
}});
})();
