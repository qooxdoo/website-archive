...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Decorator_Box","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.test.Decorator_Box-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"2277da631c91"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var j="-l",i="horizontal",h="qx.ui.decoration.AbstractBox",g="set",f="qx.debug",e="_applyBaseImage",d="-b",c="-t",b="String",a="-r";
qx.Class.define(h,{extend:qx.ui.decoration.Abstract,implement:[qx.ui.decoration.IDecorator],construct:function(k,l,m){qx.ui.decoration.Abstract.call(this);
this._setOrientation(m);

if(qx.ui.decoration.css3.BorderImage.IS_SUPPORTED){this.__wc=new qx.ui.decoration.css3.BorderImage();

if(k){this.__wd(k,m);
}if(l!=null){this.__wc.setInsets(l);
}}else{this.__wc=new qx.ui.decoration.BoxDiv(k,l,m);
}},properties:{baseImage:{check:b,nullable:true,apply:e}},members:{__wc:null,_isHorizontal:null,_setOrientation:function(n){this._isHorizontal=n==i;
},getMarkup:function(){return this.__wc.getMarkup();
},resize:function(o,p,q){this.__wc.resize(o,p,q);
},tint:function(r,s){},getInsets:function(){return this.__wc.getInsets();
},_applyInsets:function(t,u,name){var v=g+qx.lang.String.firstUp(name);
this.__wc[v](t);
},_applyBaseImage:function(w,x){if(this.__wc instanceof qx.ui.decoration.BoxDiv){this.__wc.setBaseImage(w);
}else{this.__wd(w);
}},__wd:function(y){this.__wc.setBorderImage(y);
var E=qx.util.AliasManager.getInstance().resolve(y);
var F=/(.*)(\.[a-z]+)$/.exec(E);
var C=F[1];
var D=F[2];
var B=qx.util.ResourceManager.getInstance();
if(qx.core.Environment.get(f)){if(!B.has(E)){this.warn("The border image '"+E+"' is not available!");
}}
if(this._isHorizontal){var H=B.getImageWidth(C+j+D);
var z=B.getImageWidth(C+a+D);
this.__wc.setSlice([0,z,0,H]);
}else{var A=B.getImageHeight(C+d+D);
var G=B.getImageHeight(C+c+D);
this.__wc.setSlice([G,0,A,0]);
...
(function(){var j="px",i="no-repeat",h="engine.version",g="0",f="-1px",e="-l",d='</div>',c="-t",b="browser.quirksmode",a="-r",w='<div style="position:absolute;top:0;left:0;overflow:hidden;font-size:0;line-height:0;">',v="qx.debug",u="_applyBaseImage",t="-b",s="repeat-y",r="String",q="",p="qx.ui.decoration.BoxDiv",o="-c",n="mshtml",l="engine.name",m="horizontal",k="repeat-x";
qx.Class.define(p,{extend:qx.ui.decoration.Abstract,construct:function(x,y,z){qx.ui.decoration.Abstract.call(this);
this._setOrientation(z);
if(x!=null){this.setBaseImage(x);
}
if(y!=null){this.setInsets(y);
...
},_setOrientation:function(A){this._isHorizontal=A==m;
...
if(this._isHorizontal){E.push(B.create(C.l,i,{top:0,left:0}));
E.push(B.create(C.c,k,{top:0,left:D.left+j}));
E.push(B.create(C.r,i,{top:0,right:0}));
}else{E.push(B.create(C.t,i,{top:0,left:0}));
E.push(B.create(C.c,s,{top:D.top+j,left:D.left+j}));
E.push(B.create(C.b,i,{bottom:0,left:0}));
}E.push(d);
return this.__vl=E.join(q);
},resize:function(F,G,H){F.style.width=G+j;
F.style.height=H+j;
var I=this.__vn;

if(this._isHorizontal){var innerWidth=G-I.left-I.right;
innerWidth=innerWidth<0?0:innerWidth;
F.childNodes[1].style.width=innerWidth+j;
}else{var innerHeight=H-I.top-I.bottom;
innerHeight=innerHeight<0?0:innerHeight;
F.childNodes[1].style.height=innerHeight+j;
}
if((qx.core.Environment.get(l)==n)){if(parseFloat(qx.core.Environment.get(h))<7||(qx.core.Environment.get(b)&&parseFloat(qx.core.Environment.get(h))<8)){if(this._isHorizontal){F.childNodes[2].style.marginRight=(G%2==1)?f:g;
}else{F.childNodes[2].style.marginBottom=(H%2==1)?f:g;
}}}},tint:function(J,K){},_applyBaseImage:function(L,M){if(qx.core.Environment.get(v)){if(this.__vl){throw new Error("This decorator is already in-use. Modification is not possible anymore!");
}}var N=qx.util.ResourceManager.getInstance();

if(L){var P=qx.util.AliasManager.getInstance();
var R=P.resolve(L);
var S=/(.*)(\.[a-z]+)$/.exec(R);
var Q=S[1];
var O=S[2];
var T=this.__vm={t:Q+c+O,b:Q+t+O,c:Q+o+O,l:Q+e+O,r:Q+a+O};
this.__vn={top:N.getImageHeight(T.t),bottom:N.getImageHeight(T.b),left:N.getImageWidth(T.l),right:N.getImageWidth(T.r)};
