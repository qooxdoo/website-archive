...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.UIDeclaration","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.test.UIDeclaration-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"6df2d3b4a48e"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var l="Item",k="Button",j="Last Name",h="Page 1",g="First Name",f="firstname",d="Email",c="Demo loaded",b="icon/16/apps/office-calendar.png",a="Page 3",z="move",y="First Window",x="Simple Form",w="Page 2",v="resize",u="Date",t="win",s="Cancel",r="icon/32/apps/office-address-book.png",q="Header",o="Welcome to your first own Window.<br/>Have fun!",p="Save",m="demobrowser.demo.test.UIDeclaration",n="Company";
...
var A={clazz:qx.ui.form.Form,children:[{type:q,label:x},{type:l,label:g,validator:null,name:f,widget:{clazz:qx.ui.form.TextField,required:true,width:200}},{type:l,label:j,widget:{clazz:qx.ui.form.TextField}},{type:l,label:n,widget:{clazz:qx.ui.form.TextField}},{type:l,label:d,widget:{clazz:qx.ui.form.TextField}},{type:l,label:u,widget:{clazz:qx.ui.form.DateField}},{type:k,widget:{clazz:qx.ui.form.Button,label:p}},{type:k,widget:{clazz:qx.ui.form.Button,label:s}}]};
var B={clazz:qx.ui.window.Window,layout:{clazz:qx.ui.layout.VBox,spacing:10},layoutProperties:{left:20,top:20},showStatusbar:true,status:c,caption:y,icon:b,id:t,listeners:[{event:z,handler:function(e){this.debug("Moved to: "+e.getData().left+"x"+e.getData().top);
}},{event:v,handler:function(e){this.debug("Resized to: "+e.getData().width+"x"+e.getData().height);
}}],children:[{clazz:qx.ui.basic.Atom,label:o,icon:r,rich:true},{clazz:qx.ui.tabview.TabView,layoutProperties:{flex:1},children:[{clazz:qx.ui.tabview.Page,label:h,layoutProperties:{flex:1},layout:{clazz:qx.ui.layout.Canvas},children:[A]},{clazz:qx.ui.tabview.Page,label:w,layoutProperties:{flex:1}},{clazz:qx.ui.tabview.Page,label:a,layoutProperties:{flex:1}}]}]};
this.addFromDefinition(B);
this.win.open();
},addFromDefinition:function(C){var D=this.getRoot();
this.__UQ(D,C);
},__UQ:function(E,F){var G=new F.clazz();
var K=qx.lang.Object.clone(F);
delete K.clazz;
delete K.layout;
delete K.children;
delete K.id;
delete K.listeners;
G.set(K);
if(F.id!=null){this[F.id]=G;
}if(F.layout!=null){var H=new F.layout.clazz();
var K=qx.lang.Object.clone(F.layout);
delete K.clazz;
H.set(K);
G.setLayout(H);
}var J=F.children;

if(J!=null){for(var i=0;i<J.length;i++){if(G instanceof qx.ui.form.Form){this.__UR(G,J[i]);
}else{this.__UQ(G,J[i]);
}}}if(G instanceof qx.ui.form.Form){E.add(new qx.ui.form.renderer.Single(G));
}else{E.add(G,F.layoutProperties);
}if(F.listeners){for(var i=0;i<F.listeners.length;i++){var I=F.listeners[i];
G.addListener(I.event,I.handler,I.context);
}}},__UR:function(L,M){if(M.type==="Header"){L.addGroupHeader(M.label);
return;
}var N=new M.widget.clazz();
var O=qx.lang.Object.clone(M.widget);
delete O.clazz;
N.set(O);

if(M.type==="Item"){L.add(N,M.label,M.validator,M.name);
}else if(M.type==="Button"){L.addButton(N);
}}}});
})();
...
},members:{__DN:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(c,d,e,name,f,g){if(this.__DO()){this.__DN.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});
}this.__DN[this._groupCounter].items.push(c);
this.__DN[this._groupCounter].labels.push(d);
this.__DN[this._groupCounter].options.push(g);
if(name==null){name=d.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,a);
}this.__DN[this._groupCounter].names.push(name);
this._validationManager.add(c,e,f);
this._resetter.add(c);
},addGroupHeader:function(h,k){if(!this.__DO()){this._groupCounter++;
}this.__DN.push({title:h,items:[],labels:[],names:[],options:[],headerOptions:k});
},addButton:function(l,m){this._buttons.push(l);
this._buttonOptions.push(m||null);
},__DO:function(){return this.__DN.length===0;
...
},events:{"changeValid":a,"complete":f},properties:{validator:{check:d,init:null,nullable:true},invalidMessage:{check:h,init:j},requiredFieldMessage:{check:h,init:j},context:{nullable:true}},members:{__Ds:null,__Du:null,__Dt:null,__Dv:null,add:function(k,l,m){if(!this.__DA(k)){throw new Error("Added widget not supported.");
}if(this.__DB(k)){if(l!=null){throw new Error("Widgets supporting selection can only be validated "+"in the form validator");
}}var n={item:k,validator:l,valid:null,context:m};
this.__Ds.push(n);
},remove:function(o){var p=this.__Ds;

for(var i=0,q=p.length;i<q;i++){if(o===p[i].item){p.splice(i,1);
return o;
}}return null;
},getItems:function(){var r=[];

for(var i=0;i<this.__Ds.length;i++){r.push(this.__Ds[i].item);
}return r;
},validate:function(){var w=true;
this.__Dv=true;
var t=[];
for(var i=0;i<this.__Ds.length;i++){var u=this.__Ds[i].item;
var x=this.__Ds[i].validator;
t.push(u);
if(x==null){var s=this.__Dw(u);
w=w&&s;
this.__Dv=s&&this.__Dv;
continue;
}var s=this.__Dx(this.__Ds[i],u.getValue());
w=s&&w;

if(s!=null){this.__Dv=s&&this.__Dv;
}}var v=this.__Dy(t);

if(qx.lang.Type.isBoolean(v)){this.__Dv=v&&this.__Dv;
}w=v&&w;
this.__DC(w);

if(qx.lang.Object.isEmpty(this.__Dt)){this.fireEvent(g);
}return w;
},__Dw:function(y){if(y.getRequired()){if(this.__DB(y)){var z=!!y.getSelection()[0];
}else{var z=!!y.getValue();
}y.setValid(z);
var B=y.getRequiredInvalidMessage();
var A=B?B:this.getRequiredFieldMessage();
y.setInvalidMessage(A);
return z;
}return true;
},__Dx:function(C,D){var I=C.item;
var H=C.context;
var G=C.validator;
if(this.__Dz(G)){this.__Dt[I.toHashCode()]=null;
G.validate(I,I.getValue(),this,H);
return null;
}var F=null;

try{var F=G.call(H||this,D,I);

if(F===undefined){F=true;
}}catch(e){if(e instanceof qx.core.ValidationError){F=false;

if(e.message&&e.message!=qx.type.BaseError.DEFAULTMESSAGE){var E=e.message;
}else{var E=e.getComment();
}I.setInvalidMessage(E);
}else{throw e;
}}I.setValid(F);
C.valid=F;
return F;
},__Dy:function(J){var L=this.getValidator();
var M=this.getContext()||this;

if(L==null){return true;
}this.setInvalidMessage(j);

if(this.__Dz(L)){this.__Dt[this.toHashCode()]=null;
L.validateForm(J,this,M);
return null;
}
try{var N=L.call(M,J,this);

if(N===undefined){N=true;
}}catch(e){if(e instanceof qx.core.ValidationError){N=false;

if(e.message&&e.message!=qx.type.BaseError.DEFAULTMESSAGE){var K=e.message;
}else{var K=e.getComment();
}this.setInvalidMessage(K);
}else{throw e;
}}return N;
},__Dz:function(O){var P=false;

if(!qx.lang.Type.isFunction(O)){P=qx.Class.isSubClassOf(O.constructor,qx.ui.form.validation.AsyncValidator);
}return P;
},__DA:function(Q){var R=Q.constructor;
return qx.Class.hasInterface(R,qx.ui.form.IForm);
},__DB:function(S){var T=S.constructor;
return qx.Class.hasInterface(T,qx.ui.core.ISingleSelection);
},__DC:function(U){var V=this.__Du;
this.__Du=U;
if(V!=U){this.fireDataEvent("changeValid",U,V);
}},getValid:function(){return this.__Du;
},isValid:function(){return this.getValid();
},getInvalidMessages:function(){var W=[];
for(var i=0;i<this.__Ds.length;i++){var X=this.__Ds[i].item;

if(!X.getValid()){W.push(X.getInvalidMessage());
}}if(this.getInvalidMessage()!=j){W.push(this.getInvalidMessage());
}return W;
},reset:function(){for(var i=0;i<this.__Ds.length;i++){var Y=this.__Ds[i];
Y.item.setValid(true);
}this.__Du=null;
},setItemValid:function(ba,bb){this.__Dt[ba.toHashCode()]=bb;
ba.setValid(bb);
this.__DD();
},setFormValid:function(bc){this.__Dt[this.toHashCode()]=bc;
this.__DD();
},__DD:function(){var be=this.__Dv;
for(var bf in this.__Dt){var bd=this.__Dt[bf];
be=bd&&be;
if(bd==null){return;
}}this.__DC(be);
this.__Dt={};
...
this.__DE=b;
},members:{__DE:null,__DF:null,__DG:null,__DH:null,validate:function(c,d,e,f){this.__DH=false;
this.__DF=c;
this.__DG=e;
this.__DE.call(f||this,this,d);
},validateForm:function(g,h,i){this.__DH=true;
this.__DG=h;
this.__DE.call(i,g,this);
},setValid:function(j,k){if(this.__DH){if(k!==undefined){this.__DG.setInvalidMessage(k);
}this.__DG.setFormValid(j);
}else{if(k!==undefined){this.__DF.setInvalidMessage(k);
}this.__DG.setItemValid(this.__DF,j);
}}},destruct:function(){this.__DG=this.__DF=null;
}});
})();
...
this.__Do=[];
},members:{__Do:null,add:function(b){if(this._supportsValue(b)){var c=b.getValue();
}else if(this.__Dr(b)){var c=b.getSelection();
}else{throw new Error("Item "+b+" not supported for reseting.");
}this.__Do.push({item:b,init:c});
},reset:function(){for(var i=0;i<this.__Do.length;i++){var d=this.__Do[i];
this.__Dp(d.item,d.init);
}},resetItem:function(e){var f;

for(var i=0;i<this.__Do.length;i++){var g=this.__Do[i];

if(g.item===e){f=g.init;
break;
}}if(f===undefined){throw new Error("The given item has not been added.");
}this.__Dp(e,f);
},__Dp:function(h,j){if(this._supportsValue(h)){h.setValue(j);
}else if(this.__Dr(h)){h.setSelection(j);
}},redefine:function(){for(var i=0;i<this.__Do.length;i++){var k=this.__Do[i].item;
this.__Do[i].init=this.__Dq(k);
}},redefineItem:function(l){var m;

for(var i=0;i<this.__Do.length;i++){if(this.__Do[i].item===l){m=this.__Do[i];
break;
}}if(m===undefined){throw new Error("The given item has not been added.");
}m.init=this.__Dq(m.item);
},__Dq:function(n){if(this._supportsValue(n)){return n.getValue();
}else if(this.__Dr(n)){return n.getSelection();
}},__Dr:function(o){var p=o.constructor;
...
(function(){var m="pane",k="lastTab",j="bar",h="page",g="firstTab",f="right",d="bottom",c="button",b="changeSelection",a="close",C="top",B="left",A="qx.event.type.Data",z="barRight",y="beforeChangeSelection",x="qx.debug",w="close-button",v="tabview",u="vertical",t="_applyBarPosition",r="barLeft",s="__sq",p="horizontal",q="qx.ui.tabview.TabView",n="barTop",o="barBottom";
qx.Class.define(q,{extend:qx.ui.core.Widget,implement:qx.ui.core.ISingleSelection,include:[qx.ui.core.MContentPadding],construct:function(D){qx.ui.core.Widget.call(this);
this.__sp={top:n,right:z,bottom:o,left:r};
this._createChildControl(j);
this._createChildControl(m);
var E=this.__sq=new qx.ui.form.RadioGroup;
E.setWrap(false);
E.addListener(b,this._onChangeSelection,this);
if(D!=null){this.setBarPosition(D);
}else{this.initBarPosition();
}},events:{"changeSelection":A},properties:{appearance:{refine:true,init:v},barPosition:{check:[B,f,C,d],init:C,apply:t}},members:{__sq:null,_createChildControlImpl:function(F,G){var H;

switch(F){case j:H=new qx.ui.container.SlideBar();
H.setZIndex(10);
this._add(H);
break;
...
M.add(I);
this.__sq.add(J);
I.addState(this.__sp[this.getBarPosition()]);
I.addState(k);
...
},remove:function(N){var S=this.getChildControl(m);
var Q=this.getChildControl(j);
var P=N.getButton();
var R=S.getChildren();
if(this.getSelection()[0]==N){var O=R.indexOf(N);

if(O==0){if(R[1]){this.setSelection([R[1]]);
}else{this.resetSelection();
}}else{this.setSelection([R[O-1]]);
}}Q.remove(P);
S.remove(N);
this.__sq.remove(P);
N.removeState(this.__sp[this.getBarPosition()]);
if(N.hasState(g)){N.removeState(g);

if(R[0]){R[0].addState(g);
}}
if(N.hasState(k)){N.removeState(k);

if(R.length>0){R[R.length-1].addState(k);
}}N.removeListener(a,this._onPageClose,this);
},getChildren:function(){return this.getChildControl(m).getChildren();
},indexOf:function(T){return this.getChildControl(m).indexOf(T);
},__sp:null,_applyBarPosition:function(U,V){var W=this.getChildControl(j);
var bc=U==B||U==f;
var ba=U==f||U==d;
...
W.setOrientation(bc?u:p);
var bd=this.getChildren();
if(V){var X=this.__sp[V];
W.removeState(X);
for(var i=0,l=bd.length;i<l;i++){bd[i].removeState(X);
}}
if(U){var Y=this.__sp[U];
W.addState(Y);
for(var i=0,l=bd.length;i<l;i++){bd[i].addState(Y);
}}},getSelection:function(){var bf=this.__sq.getSelection();
var bg=[];

for(var i=0;i<bf.length;i++){bg.push(bf[i].getUserData(h));
}return bg;
},setSelection:function(bh){var bi=[];

for(var i=0;i<bh.length;i++){bi.push(bh[i].getChildControl(c));
}this.__sq.setSelection(bi);
},resetSelection:function(){this.__sq.resetSelection();
},isSelected:function(bj){var bk=bj.getChildControl(c);
return this.__sq.isSelected(bk);
},isSelectionEmpty:function(){return this.__sq.isSelectionEmpty();
},getSelectables:function(bl){var bm=this.__sq.getSelectables(bl);
var bn=[];

for(var i=0;i<bm.length;i++){bn.push(bm[i].getUserData(h));
}return bn;
},_onChangeSelection:function(e){var bs=this.getChildControl(m);
var bp=e.getData()[0];
var br=e.getOldData()[0];
var bo=[];
var bq=[];

if(bp){bo=[bp.getUserData(h)];
bs.setSelection(bo);
bp.focus();
this.scrollChildIntoView(bp,null,null,false);
}else{bs.resetSelection();
}
if(br){bq=[br.getUserData(h)];
}this.fireDataEvent(b,bo,bq);
},_onBeforeChangeSelection:function(e){if(!this.fireNonBubblingEvent(y,qx.event.type.Event,[false,true])){e.preventDefault();
}},_onRadioChangeSelection:function(e){var bt=e.getData()[0];

if(bt){this.setSelection([bt.getUserData(h)]);
}else{this.resetSelection();
}},_onPageClose:function(e){var bv=e.getTarget();
var bu=bv.getButton().getChildControl(w);
bu.reset();
this.remove(bv);
}},destruct:function(){this._disposeObjects(s);
this.__sp=null;
