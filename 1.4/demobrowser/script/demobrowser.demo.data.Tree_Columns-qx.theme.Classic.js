...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.Tree_Columns","qx.theme":"qx.theme.Classic"};
...
Q.setDelegate(this);
Q.setModel(O);
},extendData:function(R){R.date=u+Math.round(Math.random()*30+1)+o;
R.size=Math.round(Math.random()*100)+x;
R.light=Math.floor(Math.random()*4)==0;
R.mode=D;
R.checked=Math.random()>=0.5;

if(R.children){for(var i=0;i<R.children.length;i++){this.extendData(R.children[i]);
}}},bindItem:function(S,T,U){S.bindDefaultProperties(T,U);
S.bindProperty(j,k,null,T.getUserData(j),U);
S.bindProperty(b,k,null,T.getUserData(e),U);
S.bindPropertyReverse(b,k,null,T.getUserData(e),U);
S.bindProperty(f,k,null,T.getUserData(f),U);
S.bindProperty(g,k,null,T.getUserData(g),U);
S.bindProperty(h,E,{converter:function(V){return V?n:c;
}},T.getUserData(h),U);
},createItem:function(){var W=new qx.ui.tree.TreeFolder();
var Y=new qx.ui.basic.Image();
Y.setWidth(16);
W.addWidget(Y);
W.setUserData(h,Y);
W.addSpacer();
...
W.addIcon();
W.setIcon(G);
var X=new qx.ui.form.CheckBox();
X.setFocusable(false);
W.addWidget(X);
W.setUserData(e,X);
W.addLabel(c);
W.addWidget(new qx.ui.core.Spacer(),{flex:1});
var ba=new qx.ui.basic.Label();
ba.setWidth(50);
W.addWidget(ba);
W.setUserData(j,ba);
ba=new qx.ui.basic.Label();
ba.setWidth(150);
W.addWidget(ba);
W.setUserData(f,ba);
ba=new qx.ui.basic.Label();
