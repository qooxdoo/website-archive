...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.showcase.Localization","qx.theme":"qx.theme.Modern"};
...
(function(){var n="short",m="long",l="Cut",k="medium",j="Copy",h="full",g="Paste",f="icon/16/actions/edit-find.png",d="Locale information",c="Ctrl+V",bo="icon/16/actions/edit-cut.png",bn="Del",bm="Date medium:",bl="Open Color Popup",bk="en",bj="changeValue",bi="Time format long:",bh="mousedown",bg="Format of %1:",bf="Locale:",u="F3",v="Undo",s="Date short:",t="Date full:",q="Date format short:",r="changeLocale",o="#23F3C1",p="qx.debug",A="Time long:",B="Ctrl+Z",K="Choose a locale:",H="wide",S="Ctrl+X",N="changeSelection",bb="middle",X="Delete",D="Select All",be="Date long:",bd="icon/16/actions/edit-copy.png",bc="A date:",C="Date format medium:",F="Date format full:",G="Week start:",J="Ctrl+F",L="icon/16/actions/edit-paste.png",O="icon/16/actions/edit-delete.png",U="Date format long:",ba="right",w="main",x="Search",E="Locale Information",R="Redo",Q="Ctrl+C",P="demobrowser.demo.showcase.Localization",W="Time short:",V="Ctrl+A",M="Search Again",T="Localized ComboBox:",a="Time format short:",Y="Territory code:",y="Ctrl+Y",z="Colors",I="Command Menu (keyboard shortcuts)",b="Form Elements";
...
if((qx.core.Environment.get(p))){qx.log.appender.Native;
qx.log.appender.Console;
}var bK=new qx.ui.layout.Grid(10,10);
bK.setColumnFlex(0,1);
bK.setColumnFlex(1,1);
bK.setRowFlex(3,1);
bK.setColumnAlign(0,ba,bb);
var bA=new qx.ui.window.Window(this.tr(b)).set({showClose:false,showMinimize:false});
bA.setLayout(bK);
bA.moveTo(50,50);
bA.open();
...
var bw=bT.getLocale();
var bS=new qx.ui.basic.Label(this.tr(K));
...
}}bv.addListener(N,function(e){var bW=e.getData()[0].getLabel();
qx.locale.Manager.getInstance().setLocale(bW);
});

if(bG){bv.setSelection([bG]);
}bA.add(bv,{row:0,column:1});
bA.add(new qx.ui.basic.Label(this.tr(T)),{row:1,column:0});
...
var bU=new qx.ui.basic.Label(this.tr(bc));
...
var bq=new qx.ui.core.Command(B);
var bB=new qx.ui.core.Command(y);
var bp=new qx.ui.core.Command(S);
var bR=new qx.ui.core.Command(Q);
var bV=new qx.ui.core.Command(c);
var bN=new qx.ui.core.Command(bn);
var bM=new qx.ui.core.Command(V);
var bx=new qx.ui.core.Command(J);
var br=new qx.ui.core.Command(u);
var bC=new qx.ui.menu.Menu;
bC.add(new qx.ui.menu.Button(this.tr(v),null,bq));
bC.add(new qx.ui.menu.Button(this.tr(R),null,bB));
bC.add(new qx.ui.menu.Separator());
bC.add(new qx.ui.menu.Button(this.tr(l),bo,bp));
bC.add(new qx.ui.menu.Button(this.tr(j),bd,bR));
bC.add(new qx.ui.menu.Button(this.tr(g),L,bV));
bC.add(new qx.ui.menu.Button(this.tr(X),O,bN));
bC.add(new qx.ui.menu.Button(this.tr(D),null,bM));
bC.add(new qx.ui.menu.Separator());
bC.add(new qx.ui.menu.Button(this.tr(x),null,bx));
bC.add(new qx.ui.menu.Button(this.tr(M),null,br));
var bt=new qx.ui.form.MenuButton(this.tr(I),null,bC);
...
var bE=new qx.ui.core.Widget().set({decorator:w});
bA.add(bE,{row:5,column:1});
bQ.addListener(bj,function(e){bE.setBackgroundColor(e.getData());
});
var bJ=new qx.ui.window.Window(z).set({width:400,showClose:false,showMinimize:false});
bJ.setLayout(new qx.ui.layout.Canvas());
bJ.moveTo(150,300);
bJ.open();
var bF=new qx.ui.control.ColorSelector;
bJ.add(bF);
var bs=new qx.ui.window.Window(E,f).set({showClose:false,showMinimize:false});
...
bP[(i++*2)+1]=this.tr(G);
bP[(i++*2)+1]=qx.locale.Date.getDayName(H,qx.locale.Date.getWeekStart());
bP[(i++*2)+1]=this.tr(bg,10000.12);
bP[(i++*2)+1]=(new qx.util.format.NumberFormat()).format(10000.12);
bI.setValue(bP.join(""));
};
...
qx.Class.define(t,{statics:{__nq:qx.locale.Manager.getInstance(),getAmMarker:function(H){return this.__nq.localize(m,[],H);
},getPmMarker:function(I){return this.__nq.localize(l,[],I);
...
}else{bC=this.__nq.getTerritory()||this.__nq.getLanguage();
...
if(!di){this.__mX=qx.locale.Manager.getInstance().getLocale();
}else{this.__mX=di;
}
if(dh!=null){this.__mY=dh.toString();
}else{this.__mY=qx.locale.Date.getDateFormat(h,this.__mX)+da+qx.locale.Date.getDateTimeFormat(bX,cg,this.__mX);
...
if(dk._dateInstance==null||dk._dateInstance.__mY!=dj){dk._dateTimeInstance=new dk();
...
if(dm._dateInstance==null||dm._dateInstance.__mY!=dl){dm._dateInstance=new dm(dl);
}return dm._dateInstance;
},ASSUME_YEAR_2000_THRESHOLD:30,LOGGING_DATE_TIME__format:cm,AM_MARKER:"am",PM_MARKER:"pm"},members:{__mX:null,__mY:null,__na:null,__nb:null,__nc:null,__nd:function(dn,dp){var dq=cw+(dn<0?((-1)*dn):dn);

while(dq.length<dp){dq=bx+dq;
}return dn<0?cv+dq:dq;
},__ne:function(dr){var ds=new Date(dr.getTime());
var dt=ds.getDate();

while(ds.getMonth()!=0){ds.setDate(-1);
dt+=ds.getDate()+1;
}return dt;
},__nf:function(du){return new Date(du.getTime()+(3-((du.getDay()+6)%7))*86400000);
},__ng:function(dv){var dx=this.__nf(dv);
var dy=dx.getFullYear();
var dw=this.__nf(new Date(dy,0,4));
return Math.floor(1.5+(dx.getTime()-dw.getTime())/86400000/7);
},__nh:function(dz){var dB=this.__nf(dz);
var dA=this.__nf(new Date(dz.getFullYear(),dz.getMonth(),4));
return Math.floor(1.5+(dB.getTime()-dA.getTime())/86400000/7);
},__ni:function(dC){var dD=this.__nf(dC);
return dD.getFullYear();
},__nj:function(dE){var dF=new Date(dE,2,1);
dF.setDate(-1);
return dF.getDate()+1===29;
},__nk:function(dG,dH){var dL=0;
var dM=0;
if(!dH){dH=1971;
}var dI=0;
...
},__nl:function(dN,dO,dP){var dQ;

switch(dO){case 11:dQ=dN-1;

if(dN!=this.__ni(new Date(dQ,dO,dP))){dQ=dN;
}break;
case 0:dQ=dN+1;
...
for(var i=0;i<this.__nc.length;i++){var eb=this.__nc[i];
...
switch(dT){case ca:if(ef==2){en=this.__nd(ea%100,2);
...
case cH:if(ef==1||ef==2){en=this.__nd(parseInt(dV/4)+1,ef);
}
if(ef==3){en=cH+(parseInt(dV/4)+1);
}break;
case A:if(ef==1||ef==2){en=this.__nd(parseInt(dV/4)+1,ef);
}
if(ef==3){en=cH+(parseInt(dV/4)+1);
}break;
case bM:en=this.__nd(this.__ne(dR),ef);
break;
case co:en=this.__nd(dY,ef);
break;
case cR:en=this.__nd(this.__ng(dR),ef);
break;
case dc:en=this.__nh(dR);
break;
...
if(ef>=1&&ef<=2){en=this.__nd(ei,ef);
...
case cU:if(ef==1||ef==2){en=this.__nd(dV+1,ef);
...
case S:if(ef==1||ef==2){en=this.__nd(dV+1,ef);
}else if(ef==3){en=qx.locale.Date.getMonthName(cu,dV,dU,cx);
}else if(ef==4){en=qx.locale.Date.getMonthName(ct,dV,dU,cx);
}else if(ef==5){en=qx.locale.Date.getMonthName(bC,dV,dU,cs);
}break;
...
break;
case bo:en=this.__nd(eg,ef);
break;
case bJ:en=this.__nd((eg==0)?24:eg,ef);
break;
case bU:en=this.__nd(eg%12,ef);
break;
case F:en=this.__nd(((eg%12)==0)?12:(eg%12),ef);
break;
case ci:en=this.__nd(em,ef);
break;
case N:en=this.__nd(ee,ef);
break;
case bp:en=ek+cw;
...
case m:if(ef>=1&&ef<=4){en=ch+((eh>0)?cv:cG)+this.__nd(Math.abs(dS),2)+cb+this.__nd(ej,2);
}break;
case t:if(ef>=1&&ef<=3){en=((eh>0)?cv:cG)+this.__nd(Math.abs(dS),2)+this.__nd(ej,2);
}else{en=ch+((eh>0)?cv:cG)+this.__nd(Math.abs(dS),2)+cb+this.__nd(ej,2);
}break;
}el+=en;
}}return el;
},parse:function(eo){this.__nn();
var ex=this.__na.regex.exec(eo);

if(ex==null){throw new Error("Date string '"+eo+"' does not match the date format: "+this.__mY);
...
for(var i=0;i<this.__na.usedRules.length;i++){var et=this.__na.usedRules[i];
var eu=ex[er];
...
for(var k=0;k<this.__na.usedRules.length;k++){if(this.__na.usedRules[k].pattern==bY){ev=true;
break;
}}
if(!ev){eq=true;
}}
...
for(var k=0;k<this.__na.usedRules.length;k++){if(this.__na.usedRules[k].pattern.indexOf(o)!=-1){ez=true;
break;
}}
if(!ez){es=true;
}}er+=(et.groups==null)?1:et.groups;
}
if(eq){ep.year=this.__nl(ep.weekYear,ep.month,ep.day);
}
if(es){var ey=this.__nk(ep.dayOfYear,ep.year);
ep.month=ey.month;
ep.day=ey.day;
}
...
},__nm:function(){if(this.__nc!=null){return;
}this.__nc=[];
var eE;
var eC=0;
var eG=cw;
...
case bv:if(eF==eE){eC++;
i++;
}else{this.__nc.push({type:bv,character:eE,size:eC});
eE=null;
eC=0;
eD=by;
}break;
...
if(eD!=by){if(eG.length>0){this.__nc.push({type:bw,text:eG});
eG=cw;
}}else{eG+=eF;
i++;
}break;
}}if(eE!=null){this.__nc.push({type:bv,character:eE,size:eC});
}else if(eG.length>0){this.__nc.push({type:bw,text:eG});
}},__nn:function(){if(this.__na!=null){return ;
}var eK=this.__mY;
this.__np();
this.__nm();
var eQ=[];
...
for(var eI=0;eI<this.__nc.length;eI++){var eR=this.__nc[eI];
...
for(var eS=0;eS<this.__nb.length;eS++){var eO=this.__nb[eS];

if(this.__no(eO,eJ,eN)){eL=eO;
break;
...
}this.__na={regex:eH,"usedRules":eQ,pattern:eM};
},__no:function(eU,eV,eW){if(eV===ca&&eU.pattern===bY){eU.regex=eU.regexFunc(eW);
return true;
}else if(eV===l&&eU.pattern===bg){eU.regex=eU.regexFunc(eW);
return true;
}else{return eV==eU.pattern.charAt(0)&&eW==eU.pattern.length;
}},__np:function(){var fo=qx.util.format.DateFormat;
var fr=qx.lang.String;

if(this.__nb!=null){return ;
}var fp=this.__nb=[];
var fj=qx.locale.Date.getAmMarker(this.__mX).toString()||fo.AM_MARKER;
var fB=qx.locale.Date.getPmMarker(this.__mX).toString()||fo.PM_MARKER;
var fa=this.__mX;
...
}},destruct:function(){this.__nc=this.__na=this.__nb=null;
}});
})();
