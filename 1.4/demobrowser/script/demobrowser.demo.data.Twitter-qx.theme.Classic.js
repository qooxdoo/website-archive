...
var envinfo = {"qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.Twitter","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.data.Twitter-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"76bad4233c70"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var j="value",i="icon",h="model",g="text",f="source",e="wittemann",d="mshtml",c="engine.name",b=".json",a="user.profile_image_url",F="http://twitter.com/statuses/user_timeline/",E="Enter",D="<span style='font-size: 20px'>Twitter</span>",C="<a target='_blank' href='http://twitter.com/home?status=Check%20out%20the%20%23qooxdoo%20twitter%20demo%20at%20http://bit.ly/Mc4Of'>Tweet This</a></span>",B="demobrowser.demo.data.store.Twitter",A="keydown",z="loaded",y="execute",x="demobrowser.demo.data.Twitter",w="selection[0]",q="<span style='font-size: 13px'>",r="Location: ",o="Details",p="Name: ",m="?callback=demobrowser.demo.data.store.Twitter.saveResult",n="User",k="Postet with: ",l="failed",s="user.name",t="user.location",v="Avatar: ",u="Message: ";
...
this.getRoot().add(K,{left:10,top:85});
K.setWidth(300);
var M=new qx.data.controller.List(null,K);
M.setDelegate(this);
M.setLabelPath(g);
if(qx.core.Environment.get(c)!=d){M.setIconPath(a);
}var Q=new demobrowser.demo.data.store.Twitter(e);
Q.bind(h,M,h);
var N=new qx.ui.form.Button(n);
this.getRoot().add(N,{left:10,top:55});
var G=new qx.ui.form.TextField(e);
this.getRoot().add(G,{left:60,top:55});
N.addListener(y,function(){var S=F+G.getValue()+b;

if(Q.getUrl()==S){Q.reload();
}else{Q.setUrl(S);
}},this);
G.addListener(A,function(T){if(T.getKeyIdentifier()==E){N.execute();
}},this);
var J=new qx.ui.groupbox.GroupBox(o);
this.getRoot().add(J,{left:320,top:60});
J.setWidth(300);
J.setHeight(220);
J.setLayout(new qx.ui.layout.Grid(0,5));
J.add(new qx.ui.basic.Label(p),{row:0,column:0});
J.add(new qx.ui.basic.Label(r),{row:1,column:0});
J.add(new qx.ui.basic.Label(u),{row:2,column:0});
J.add(new qx.ui.basic.Label(k),{row:3,column:0});
var name=new qx.ui.basic.Label();
J.add(name,{row:0,column:1});
var location=new qx.ui.basic.Label();
J.add(location,{row:1,column:1});
var O=new qx.ui.basic.Label();
O.setRich(true);
O.setWidth(150);
J.add(O,{row:2,column:1});
var R=new qx.ui.basic.Label();
R.setRich(true);
J.add(R,{row:3,column:1});
var H=new qx.data.controller.Object();
H.addTarget(name,j,s);
H.addTarget(location,j,t);
H.addTarget(O,j,g);
H.addTarget(R,j,f);

if(qx.core.Environment.get(c)!=d){J.add(new qx.ui.basic.Label(v),{row:4,column:0});
var L=new qx.ui.basic.Image();
J.add(L,{row:4,column:1});
H.addTarget(L,f,a);
}M.bind(w,H,h);
var I=new qx.ui.basic.Label();
I.setRich(true);
I.setWidth(260);
I.setValue(D);
this.getRoot().add(I,{left:10,top:10});
var P=new qx.ui.basic.Label();
P.setRich(true);
P.setWidth(260);
P.setValue(q+C);
this.getRoot().add(P,{left:10,top:290});
},configureItem:function(U){U.setRich(true);
U.getChildControl(i).setWidth(50);
U.getChildControl(i).setHeight(50);
U.getChildControl(i).setScale(true);
}}});
qx.Class.define(B,{extend:qx.data.store.Json,statics:{saveResult:function(V){this.__Tn=V;
}},construct:function(W){var X=F+W+b;
qx.data.store.Json.call(this,X);
},members:{_createRequest:function(Y){var ba=new qx.io.ScriptLoader();
Y+=m;
ba.load(Y,function(bb){this.__To();
},this);
},__To:function(){var bc=demobrowser.demo.data.store.Twitter.__Tn;

if(bc==undefined){this.setState(l);
return;
}this._marshaler.toClass(bc);
this.setModel(this._marshaler.toModel(bc));
this.fireDataEvent(z,this.getModel());
}}});
})();
