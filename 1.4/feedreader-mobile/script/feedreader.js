...
var settings = {"qx.application":"feedreader.MobileApplication","qx.revision":"26649","qx.theme":"qx.theme.Modern","qx.version":"1.4"};
...
var envinfo = {"qx.application":"feedreader.MobileApplication","qx.debug":false,"qx.mobile.emulatetouch":true,"qx.nativeScrollBars":true,"qx.revision":"26649","qx.theme":"qx.theme.Modern","qx.version":"1.4"};
...
if(qx.core.Environment.get(a)==false){this.__DP.setShowScrollbarY(false);
...
qx.Class.define(f,{extend:qx.application.Mobile,members:{__DQ:null,main:function(){qx.application.Mobile.prototype.main.call(this);
var j=new feedreader.model.Model();
var k=feedreader.io.FeedLoader.getInstance();
this.__DQ=j.getFeedFolder();
k.loadAll(this.__DQ);
this.buildUpGui();
...
this.__fn=t;
this.__fo=t.getWindow();
this.__fp=this.__fo.document;
this._initTouchObserver();
this._initMouseObserver();
},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{touchstart:1,touchmove:1,touchend:1,touchcancel:1,tap:1,swipe:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE+qx.event.IEventHandler.TARGET_DOCUMENT,IGNORE_CAN_HANDLE:true,MOUSE_TO_TOUCH_MAPPING:{"mousedown":"touchstart","mousemove":"touchmove","mouseup":"touchend"},SWIPE_DIRECTION:{x:["left","right"],y:["up","down"]},TAP_MAX_DISTANCE:qx.core.Environment.get("os.name")!="android"?10:40,SWIPE_MIN_DISTANCE:qx.core.Environment.get("os.name")!="android"?11:41,SWIPE_MIN_VELOCITY:0},members:{__fq:null,__fr:null,__fn:null,__fo:null,__fp:null,__fs:null,__ft:null,__fu:null,__fv:null,__fw:false,__fx:null,canHandleEvent:function(u,v){},registerEvent:function(w,x,y){},unregisterEvent:function(z,A,B){},__fy:function(C){var D=qx.bom.Event.getTarget(C);
if((qx.core.Environment.get(n)==a)){if(D&&D.nodeType==3){D=D.parentNode;
}}return D;
},__fz:function(E,F,G,H){if(!G){G=this.__fy(E);
}var F=F||E.type;

if(G&&G.nodeType){qx.event.Registration.fireEvent(G,F,H||qx.event.type.Touch,[E,G,null,true,true]);
}qx.event.Registration.fireEvent(this.__fo,i,qx.event.type.Data,[F]);
},__fA:function(I,J,K){if(!K){K=this.__fy(I);
}var J=J||I.type;

if(J==r){this.__fB(I,K);
}else if(J==q){this.__fC(I,K);
}else if(J==s){this.__fD(I,K);
}},__fB:function(L,M){var N=L.changedTouches[0];
this.__fs=N.screenX;
this.__ft=N.screenY;
this.__fu=new Date().getTime();
this.__fv=L.changedTouches.length===1;
},__fC:function(O,P){if(this.__fv&&O.changedTouches.length>1){this.__fv=false;
}},__fD:function(Q,R){if(this.__fv){var S=Q.changedTouches[0];
var U={x:S.screenX-this.__fs,y:S.screenY-this.__ft};
var V=qx.event.handler.Touch;

if(this.__fx==R&&Math.abs(U.x)<=V.TAP_MAX_DISTANCE&&Math.abs(U.y)<=V.TAP_MAX_DISTANCE){this.__fz(Q,f,R,qx.event.type.Tap);
}else{var T=this.__fE(Q,R,U);

if(T){Q.swipe=T;
this.__fz(Q,g,R,qx.event.type.Swipe);
}}}},__fE:function(W,X,Y){var bd=qx.event.handler.Touch;
var be=new Date().getTime()-this.__fu;
...
if(Math.abs(bf)>=bd.SWIPE_MIN_VELOCITY&&Math.abs(ba)>=bd.SWIPE_MIN_DISTANCE){bc={startTime:this.__fu,duration:be,axis:bg,direction:bb,distance:ba,velocity:bf};
}return bc;
},__fF:function(bh){var bi=bh.type;
var bk=qx.event.handler.Touch.MOUSE_TO_TOUCH_MAPPING;

if(bk[bi]){bi=bk[bi];
if(bi==r&&this.__fG(bh)){this.__fw=true;
}else if(bi==s){this.__fw=false;
}var bl=this.__fH(bh);
var bj=(bi==s?[]:[bl]);
bh.touches=bj;
bh.targetTouches=bj;
...
},__fG:function(bm){if((qx.core.Environment.get(n)==j)){var bn=1;
}else{var bn=0;
}return bm.button==bn;
},__fH:function(bo){var bp=this.__fy(bo);
...
},_initMouseObserver:function(){if(!qx.core.Environment.get(p)){this.__fr=qx.lang.Function.listener(this._onMouseEvent,this);
var Event=qx.bom.Event;
...
}),_onMouseEvent:qx.event.GlobalError.observeMethod(function(br){if(!qx.core.Environment.get(p)){if(br.type==o&&!this.__fw){return;
}var bs=this.__fF(br);
this._commonTouchEventHandler(br,bs);
}}),_commonTouchEventHandler:function(bt,bu){var bu=bu||bt.type;

if(bu==r){this.__fx=this.__fy(bt);
}this.__fz(bt,bu);
this.__fA(bt,bu);
...
(function(){var l="os.name",k="android",j="phonegap",h="backKeyDown",g="menuKeyDown",f="qx.event.type.Data",e="qx.mobile.nativescroll",d="add",c="input",b="qx.ui.mobile.page.manager.Simple",a="remove";
qx.Class.define(b,{extend:qx.core.Object,construct:function(m){qx.core.Object.call(this);
this.__Eo={};
this._setRoot(m||qx.core.Init.getApplication().getRoot());
this.__Et();
},events:{add:f,remove:f},members:{__Eo:null,__Ep:null,__Eq:null,__Er:null,__Es:null,__Et:function(){if(qx.core.Environment.get(j)&&qx.core.Environment.get(l)==k){this.__Er=qx.lang.Function.bind(this._onBackButton,this);
this.__Es=qx.lang.Function.bind(this._onMenuButton,this);
BackButton.override();
qx.bom.Event.addNativeListener(document,h,this.__Er);
qx.bom.Event.addNativeListener(document,g,this.__Es);
}},__Eu:function(){if(qx.core.Environment.get(j)&&qx.core.Environment.get(l)==k){qx.bom.Event.removeNativeListener(document,h,this.__Er);
qx.bom.Event.removeNativeListener(document,g,this.__Es);
}},_onBackButton:function(){if(qx.core.Environment.get(j)&&qx.core.Environment.get(l)==k){var n=true;

if(this.__Ep){n=this.__Ep.back();
}
if(n){BackButton.exitApp();
}}},_onMenuButton:function(){if(qx.core.Environment.get(j)&&qx.core.Environment.get(l)==k){if(this.__Ep){this.__Ep.menu();
}}},add:function(o){this.__Eo[o.getId()]=o;
this.fireDataEvent(d,o);
},remove:function(p){var q=this.getPage(p);

if(this.__Ep==q){throw new Error("Current page with the Id "+q.getId()+" can not be removed");
}delete this.__Eo[q.getId()];
this.fireDataEvent(a,q);
},show:function(r){var s=this.__Ep;
...
},_setRoot:function(t){this.__Eq=t;
},getCurrentPage:function(){return this.__Ep;
},_setCurrentPage:function(u){this.__Ep=u;
},getPage:function(v){return this.__Eo[v];
},__Ev:function(){var w=document.getElementsByTagName(c);
...
},events:{"initialize":q,"start":q,"stop":q,"pause":q,"resume":q,"back":q,"menu":q},properties:{defaultCssClass:{refine:true,init:j}},statics:{__EF:null,__EG:null,getManager:function(){if(!this.__EF){this.__EF=new qx.ui.mobile.page.Page.__EG();
}return qx.ui.mobile.page.Page.__EF;
},setManagerClass:function(s){qx.ui.mobile.page.Page.__EG=s;
}},members:{__EH:false,_resize:function(){if(qx.core.Environment.get(l)){this._setStyle(b,window.innerHeight+o);
...
this.setTitle(b);
},properties:{feeds:{event:f,init:null,apply:a},selectedFeed:{event:g,init:null,nullable:true}},members:{__EO:null,__EP:null,_initialize:function(){qx.ui.mobile.page.NavigationPage.prototype._initialize.call(this);
this.__EO=new qx.ui.mobile.list.List();
this.__EO.setDelegate({configureItem:function(h,i){h.setTitle(i.getTitle());
h.setShowArrow(true);
}});
this.__EO.addListener(c,function(e){var j=this.__EP.getItem(e.getData());
this.setSelectedFeed(j);
},this);
this.getContent().add(this.__EO);
},_applyFeeds:function(k,l){this.__EP=k.getFeeds().getItem(0).getFeeds();
this.__EO.setModel(this.__EP);
}}});
})();
...
},properties:{feed:{event:d,init:null,nullable:true,apply:g},selectedArticle:{event:a,init:null,nullable:true}},members:{__Fk:null,__Fl:null,_initialize:function(){qx.ui.mobile.page.NavigationPage.prototype._initialize.call(this);
this.__Fk=new qx.ui.mobile.list.List();
this.__Fk.setDelegate({configureItem:function(h,i){h.setTitle(i.getTitle());
h.setShowArrow(true);
}});
this.__Fk.addListener(c,function(e){var j=this.__Fl.getItem(e.getData());
this.setSelectedArticle(j);
},this);
this.getContent().add(this.__Fk);
},_applyFeed:function(k,l){if(k!=null){this.__Fl=k.getArticles();
this.__Fk.setModel(this.__Fl);
...
},properties:{article:{event:a,init:null,nullable:true,apply:c}},members:{__Fm:null,_initialize:function(){qx.ui.mobile.page.NavigationPage.prototype._initialize.call(this);
this.__Fm=new qx.ui.mobile.embed.Html();
this.getContent().add(this.__Fm);
