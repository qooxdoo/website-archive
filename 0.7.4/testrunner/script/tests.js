...
if(qxsettings["qx.logAppender"]==undefined)qxsettings["qx.logAppender"]="qx.log.appender.RingBuffer";
...
isSet:function(key){return this.__settings[key]!==undefined;
},
...
}else if(typeof vBrowserVendor==="string"&&vBrowserVendor==="KDE"&&/KHTML\/([0-9-\.]*)/.test(vBrowserUserAgent)){vEngine="webkit";
vBrowser="konqueror";
vEngineVersion="420";
...
vPlatform="unix";
}else if(vBrowserPlatform.indexOf("iPhone")!=-1||vBrowserPlatform.indexOf("iPod")!=-1){vPlatformIphone=true;
vPlatform="iphone";
...
if(properties){for(var name in this.$$inheritable){if(properties[name]&&widget[refresh[name]]){{};
...
if(config.inheritable||config.apply||config.event||config.deferredInit){method.init[name]=prefix+"init"+postfix;
...
if(!this[name].dispose){throw new Error(this.classname+"."+name+" is not a qooxdoo object. Use _disposeFields instead of _disposeObjects.");
}this[name].dispose();
this[name]=null;
}},
...
if(!className||className==""){className="DEBUG";
}else{className=className.toUpperCase();
}
if(!window.confirm(className+": "+message.join("")+"\n\nClick OK to continue debugging.")){this._unloadCancelled=true;
...
members:{_applyMaxMessages:function(value,
old){this._history=[];
this._nextIndexToStoreTo=0;
},
appendLogEvent:function(evt){evt.time=new Date().getTime()-qx.core.Bootstrap.LOADSTART;
...
retrieveLogEvents:function(count){if(count>this._history.length){count=this._history.length;
...
if(startIndex<=indexOfYoungestElementInHistory){result=this._history.slice(startIndex,
indexOfYoungestElementInHistory+1);
}else{result=this._history.slice(startIndex,
this._history.length).concat(this._history.slice(0,
indexOfYoungestElementInHistory+1));
}return result;
},
formatLogEvents:function(count){if(this._appenderToFormatStrings==null){this._appenderToFormatStrings=new qx.log.appender.Abstract();
}var events=this.retrieveLogEvents(count);
var string="";
for(var idx=0;idx<events.length;idx++){string+=this._appenderToFormatStrings.formatLogEvent(events[idx])+"\n";
}return string;
}},
destruct:function(){this._disposeFields("_history");
}});
...
removeAppender:function(appender){if(this._appenderArr!=null){qx.lang.Array.remove(this._appenderArr,
appender);
}},
...
object:obj,
value:value};
if(!this._connectedObjects[objectHash]){this._connectedObjects[objectHash]=[];
...
members:{_addStatic:function(){this.add("static",
qx.core.Setting.isSet("qx.staticUri")?qx.core.Setting.get("qx.staticUri"):qx.core.Setting.get("qx.resourceUri")+"/static");
},
...
backgroundColor:states.selected?"selected":"undefined",
textColor:states.selected?"text-selected":"undefined",
border:states.lead&&!states.selected?new qx.ui.core.Border.fromConfig({top:[1,
"solid",
"effect"],
bottom:[1,
"solid",
...
{members:{hasConnectionTo:function(valueManager){if(!this._valueManager){this._valueManager={};
}this._valueManager[valueManager.toHashCode()]=valueManager;
}},
destruct:function(){for(var key in this._valueManager){var valueManager=this._valueManager[key];
...
for(var i=0,
l=queue.length;i<l;i++){var widget=queue[i];
if(widget._isInGlobalStateQueue){widget._renderAppearance();
...
}if(qx.lang.Object.isEmpty(vLazyQueues)){qx.ui.core.Widget._lazyGlobalDisplayQueues={};
}for(var i=0,
...
removeFromGlobalDisposeQueue:function(vWidget){if(vWidget._isInGlobalDisposeQueue){qx.lang.Array.remove(qx.ui.core.Widget._globalDisposeQueue,
vWidget);
...
if(value){vMgr.setCaptureWidget(this);
}},
...
"none");
this.setStyleProperty("KhtmlUserSelect",
"none");
}},
"default":function(value,
...
try{vDomEvent.keyCode=0;
}catch(ex){}vDomEvent.returnValue=false;
},
...
"webkit":function(domEvent){var charCode=0;
var type=domEvent.type;
if(qx.core.Client.getInstance().getVersion()<525.13){var keyCode=0;
...
type,
domEvent);
}else{var keyCode=domEvent.keyCode;
if(!(this._lastUpDownType[keyCode]=="keydown"&&type=="keydown")){this._idealKeyHandler(keyCode,
charCode,
type,
domEvent);
...
"webkit":function(domEvent){if(qx.core.Client.getInstance().getVersion()<525.13){var keyCode=0;
var charCode=0;
var type=domEvent.type;
if(qx.core.Client.getInstance().getVersion()<420){if(!this._lastCharCodeForType){this._lastCharCodeForType={};
}var isSafariSpecialKey=this._lastCharCodeForType[type]>63000;
...
type,
domEvent);
}else{if(this._charCode2KeyCode[domEvent.keyCode]){this._idealKeyHandler(this._charCode2KeyCode[domEvent.keyCode],
0,
domEvent.type,
domEvent);
}else{this._idealKeyHandler(0,
...
if(qx.core.Variant.isSet("qx.client",
"mshtml")){members._charCode2KeyCode={13:13,
27:27};
}else if(qx.core.Variant.isSet("qx.client",
"gecko")){members._keyCodeFix={12:members._identifierToKeyCode("NumLock")};
}else if(qx.core.Variant.isSet("qx.client",
...
destruct:function(){this._disposeFields("_blocked",
"_blockData");
}});
...
if(child.isDisposed()||!(child instanceof qx.ui.core.Parent)&&!(child instanceof qx.ui.basic.Terminator)){continue;
}
...
var left=body.offsetLeft;
var top=body.offsetTop;
if(qx.bom.element.BoxSizing.get(body)!=="border-box"){left+=this.__num(body,
"borderLeftWidth");
top+=this.__num(body,
"borderTopWidth");
...
if(box.get(elem)!=="border-box"){left-=this.__num(elem,
"borderLeftWidth");
top-=this.__num(elem,
"borderTopWidth");
}
...
if(overX=="scroll"||overX=="auto"){right+=elem.scrollWidth-elem.offsetWidth+this.__num(elem,
"borderLeftWidth")+this.__num(elem,
"borderRightWidth");
}var overY=qx.bom.element.Overflow.getY(elem);
if(overY=="scroll"||overY=="auto"){bottom+=elem.scrollHeight-elem.offsetHeight+this.__num(elem,
"borderTopWidth")+this.__num(elem,
"borderBottomWidth");
}}
switch(mode){case "padding":left+=this.__num(elem,
"paddingLeft");
top+=this.__num(elem,
"paddingTop");
...
"borderBottomWidth");
break;
case "margin":left-=this.__num(elem,
"marginLeft");
top-=this.__num(elem,
"marginTop");
right+=this.__num(elem,
...
{statics:{__styleProperties:qx.core.Variant.select("qx.client",
{"mshtml":null,
"webkit":["boxSizing",
"KhtmlBoxSizing",
"WebkitBoxSizing"],
...
{"mshtml":function(value){qx.log.Logger.warn(this,
...
var css="";
if(props){for(var i=0,
l=props.length;i<l;i++){css+=props[i]+":"+value+";";
}}return css;
}}),
get:qx.core.Variant.select("qx.client",
...
var value;
if(props){for(var i=0,
l=props.length;i<l;i++){value=qx.bom.element.Style.get(element,
props[i],
null,
false);
...
{"mshtml":function(element,
value){qx.log.Logger.warn(this,
"This client do not support the dynamic modification of the box-sizing property.");
},
"default":function(element,
value){var props=this.__styleProperties;
if(props){for(var i=0,
l=props.length;i<l;i++){element.style[props[i]]=value;
}}}}),
...
GECKO:false,
MSHTML:false,
__init:function(){var engine="unknown";
var version="0.0.0";
var agent=navigator.userAgent;
if(window.opera){engine="opera";
this.OPERA=true;
if(/Opera[\s\/]([0-9\.]*)/.test(agent)){version=RegExp.$1.substring(0,
3)+"."+RegExp.$1.substring(3);
}else{throw new Error("Could not detect Opera version: "+agent+"!");
}}else if(navigator.userAgent.indexOf("AppleWebKit/")!=-1){engine="webkit";
this.WEBKIT=true;
if(/AppleWebKit\/([^ ]+)/.test(agent)){version=RegExp.$1;
var invalidCharacter=RegExp("[^\\.0-9]").exec(version);
if(invalidCharacter){version=version.slice(0,
invalidCharacter.index);
}}else{throw new Error("Could not detect Webkit version: "+agent+"!");
}}else if(window.controllers&&navigator.product==="Gecko"){engine="gecko";
this.GECKO=true;
if(/rv\:([^\);]+)(\)|;)/.test(agent)){version=RegExp.$1;
}else{throw new Error("Could not detect Gecko version: "+agent+"!");
}}else if(navigator.cpuClass&&/MSIE\s+([^\);]+)(\)|;)/.test(agent)){engine="mshtml";
version=RegExp.$1;
this.MSHTML=true;
}else{throw new Error("Unsupported client: "+agent+"!");
}this.NAME=engine;
...
compileX:function(value){return this._compile("overflow-x",
value);
},
compileY:function(value){return this._compile("overflow-y",
value);
},
...
_oninterval:function(){if(this.getEnabled()&&this.hasEventListeners("interval")){this.dispatchEvent(this.__event,
false);
}}},
...
destruct:function(){if(this._element){this._element.src="";
...
members:{add:function(testClassOrNamespace){try{if(typeof (testClassOrNamespace)=="string"){var evalTestClassOrNamespace=eval(testClassOrNamespace);
...
return;
}}catch(ex){alert("An error occured while adding test classes/namespaces\nPlease try a different test file.");
}},
...
}finally{if(typeof (cls.tearDown)=="function"){cls.tearDown();
}}});
}
...
"mshtml|opera")){if((qx.core.Client.getInstance().getMajor()>=7)&&(objectName=="window")&&(key=="external")){continue;
...
if(ex.classname=="testrunner.AssertionError"){this.__createError("failure",
ex,
test);
...
{statics:{define:function(name,
config){qx.Class.createNamespace(name,
config);
qx.locale.Manager.getInstance().addTranslationFromClass(name,
config);
}}});
...
testPatchOverwritten:function(){qx.Class.define("testrunner.Patch1",
{extend:qx.core.Object,
members:{sayJuhu:function(){return "Juhu";
},
foo:function(){return "foo";
}}});
qx.Class.define("testrunner.Patch2",
{extend:qx.core.Object,
members:{sayJuhu:function(){return "Huhu";
},
foo:function(){return "bar";
}}});
qx.Mixin.define("testrunner.MPatch",
{members:{sayJuhu:function(){return arguments.callee.base.call(this)+" Kinners";
},
foo:function(dontRecurs){var s="";
if(!dontRecurs){var b=new testrunner.Patch2();
s+="++"+b.foo(true)+"__";
}s+=arguments.callee.base.call(this);
return s;
}}});
this.assertExceptionDebugOn(function(){qx.Class.include(testrunner.Patch1,
testrunner.MPatch);
},
Error,
new RegExp('Overwriting member ".*" of Class ".*" is not allowed!'));
qx.Class.patch(testrunner.Patch1,
testrunner.MPatch);
qx.Class.patch(testrunner.Patch2,
testrunner.MPatch);
var o=new testrunner.Patch1();
this.assertEquals("Juhu Kinners",
o.sayJuhu());
var o=new testrunner.Patch2();
this.assertEquals("Huhu Kinners",
o.sayJuhu());
var o=new testrunner.Patch1();
this.assertEquals("++bar__foo",
o.foo());
}}});
...
if(qualifiedName&&vObject){var xmlStr=new qx.util.StringBuilder();
...
init:5000}},
members:{_debug:function(){var vText;
},
...
try{returnValue=vTransport.send();
}catch(exc){returnValue=exc;
}if(returnValue!==true){var response=new qx.io.remote.Response('failed');
response.setContent(returnValue);
vTransport.dispatchEvent(response);
}else{vTransport._start=(new Date()).valueOf();
...
if(vTimeout==0||vTransport._start==null){continue;
}
...
case 12007:return "Internet name not resolved";
...
_applyState:function(value,
old){{};
switch(value){case "sending":case "receiving":this.createDispatchEvent(value);
break;
...
"webkit")){vRequest.setRequestHeader('Referer',
...
}else{this.__format=qx.locale.Date.getDateFormat("long",
this.__locale)+" "+qx.locale.Date.getDateTimeFormat("HHmmss",
"HH:mm:ss",
this.__locale);
}},
...
"HH:mm:ss");
if(DateFormat._dateInstance==null||DateFormat.__format!=format){DateFormat._dateTimeInstance=new DateFormat();
...
if(DateFormat._dateInstance==null||DateFormat.__format!=format){DateFormat._dateInstance=new DateFormat(format);
...
this.__parseRules.push({pattern:"yyyy",
regex:"(\\d\\d(\\d\\d)?)",
groups:2,
manipulator:yearManipulator});
this.__parseRules.push({pattern:"yy",
regex:"(\\d\\d)",
manipulator:yearManipulator});
this.__parseRules.push({pattern:"M",
regex:"(\\d\\d?)",
manipulator:monthManipulator});
this.__parseRules.push({pattern:"MM",
regex:"(\\d\\d?)",
manipulator:monthManipulator});
this.__parseRules.push({pattern:"MMM",
regex:"("+shortMonthNames.join("|")+")",
manipulator:shortMonthNamesManipulator});
this.__parseRules.push({pattern:"MMMM",
regex:"("+fullMonthNames.join("|")+")",
manipulator:fullMonthNamesManipulator});
this.__parseRules.push({pattern:"dd",
regex:"(\\d\\d?)",
field:"day"});
this.__parseRules.push({pattern:"d",
regex:"(\\d\\d?)",
field:"day"});
this.__parseRules.push({pattern:"EE",
regex:"("+narrowDayNames.join("|")+")",
manipulator:narrowDayNamesManipulator});
this.__parseRules.push({pattern:"EEE",
regex:"("+abbrDayNames.join("|")+")",
manipulator:abbrDayNamesManipulator});
this.__parseRules.push({pattern:"EEEE",
regex:"("+fullDayNames.join("|")+")",
manipulator:fullDayNamesManipulator});
this.__parseRules.push({pattern:"a",
regex:"("+DateFormat.AM_MARKER+"|"+DateFormat.PM_MARKER+")",
manipulator:ampmManipulator});
this.__parseRules.push({pattern:"HH",
regex:"(\\d\\d?)",
field:"hour"});
this.__parseRules.push({pattern:"H",
regex:"(\\d\\d?)",
field:"hour"});
this.__parseRules.push({pattern:"kk",
regex:"(\\d\\d?)",
manipulator:noZeroHourManipulator});
this.__parseRules.push({pattern:"k",
regex:"(\\d\\d?)",
manipulator:noZeroHourManipulator});
this.__parseRules.push({pattern:"KK",
regex:"(\\d\\d?)",
...
regex:"(\\d\\d?)",
field:"min"});
this.__parseRules.push({pattern:"ss",
regex:"(\\d\\d?)",
field:"sec"});
this.__parseRules.push({pattern:"s",
regex:"(\\d\\d?)",
field:"sec"});
this.__parseRules.push({pattern:"SSS",
regex:"(\\d\\d?\\d?)",
field:"ms"});
this.__parseRules.push({pattern:"SS",
regex:"(\\d\\d?\\d?)",
...
regex:"(\\d\\d?\\d?)",
field:"ms"});
this.__parseRules.push({pattern:"Z",
regex:"((\\+|\\-)\\d\\d:?\\d\\d)",
...
destruct:function(){this._disposeFields("__format",
"__locale",
"__formatTree",
"__parseFeed",
"__parseRules");
...
initialMessage,
maxLevel,
appender){var logger=new qx.log.Logger("Debug",
