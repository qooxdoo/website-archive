...
;}
;return bu;}
,__shadowedKeys:[z,m,q,s,I,O,g],keys:({"ES5":Object.keys,"BROKEN_IE":function(bx){if(bx===null||(typeof bx!=P&&typeof bx!=c)){throw new TypeError(f);}
;var by=[];var bA=Object.prototype.hasOwnProperty;for(var bB in bx){if(bA.call(bx,bB)){by.push(bB);}
;}
;var bz=qx.Bootstrap.__shadowedKeys;for(var i=0,a=bz,l=a.length;i<l;i++ ){if(bA.call(bx,a[i])){by.push(a[i]);}
;}
;return by;}
...
;var bN=Object.prototype.toString.call(bO);return (qx.Bootstrap.__classToTypeMap[bN]||bN.slice(8,-1));}
...
,select:function(H,G){return this.__pickFromValues(this.get(H),G);}
,__pickFromValues:function(L,K){var J=K[L];if(K.hasOwnProperty(L)){return J;}
;for(var M in K){if(M.indexOf(g)!=-1){var I=M.split(g);for(var i=0;i<I.length;i++ ){if(I[i]==L){return K[M];}
;}
;}
;}
;if(K[b]!==undefined){return K[b];}
;if(qx.Bootstrap.DEBUG){throw new Error(q+L+m+(typeof L)+c+n+qx.Bootstrap.keys(K)+k);}
;}
,add:function(O,N){if(this._checks[O]==undefined){if(N instanceof Function){if(!this._checksMap[O]&&N.displayName){this._checksMap[O]=N.displayName.substr(0,N.displayName.length-2);}
;this._checks[O]=N;}
else {this._checks[O]=this.__createCheck(N);}
;}
;}
,addAsync:function(Q,P){if(this._checks[Q]==undefined){this._asyncChecks[Q]=P;}
;}
,_initDefaultQxValues:function(){var R=function(T){return function(){return T;}
;}
;for(var S in this._defaults){this.add(S,R(this._defaults[S]));}
;}
,__importFromGenerator:function(){if(qx&&qx.$$environment){for(var U in qx.$$environment){var V=qx.$$environment[U];this._checks[U]=this.__createCheck(V);}
;}
;}
,__importFromUrl:function(){if(window.document&&window.document.location){var W=window.document.location.search.slice(1).split(d);for(var i=0;i<W.length;i++ ){var ba=W[i].split(l);if(ba.length!=3||ba[0]!=s){continue;}
;var X=ba[1];var Y=decodeURIComponent(ba[2]);if(Y==f){Y=true;}
else if(Y==j){Y=false;}
else if(/^(\d|\.)+$/.test(Y)){Y=parseFloat(Y);}
;this._checks[X]=this.__createCheck(Y);}
;}
;}
,__createCheck:function(bb){return qx.Bootstrap.bind(function(bc){return bc;}
,null,bb);}
},defer:function(bd){bd._initDefaultQxValues();bd.__importFromGenerator();if(bd.get(p)===true){bd.__importFromUrl();}
;}
});}
)();
(function(){var a="[object Opera]",b="function",c="[^\\.0-9]",d="4.0",e="gecko",f="1.9.0.0",g="Version/",h="9.0",i="8.0",j="engine.version",k="Gecko",l="Maple",m="AppleWebKit/",n="Trident",o="Unsupported client: ",p="",q="opera",r="Windows Phone",s="! Assumed gecko version 1.9.0.0 (Firefox 3.0).",t="mshtml",u="engine.name",v="webkit",w="5.0",x=".",y="qx.bom.client.Engine";qx.Bootstrap.define(y,{statics:{getVersion:function(){var B=window.navigator.userAgent;var C=p;if(qx.bom.client.Engine.__isMshtml()){var A=/Trident\/([^\);]+)(\)|;)/.test(B);if(/MSIE\s+([^\);]+)(\)|;)/.test(B)){C=RegExp.$1;if(C<8&&A){if(RegExp.$1==d){C=i;}
...
;}
;}
else if(qx.bom.client.Engine.__isOpera()){if(/Opera[\s\/]([0-9]+)\.([0-9])([0-9]*)/.test(B)){if(B.indexOf(g)!=-1){var E=B.match(/Version\/(\d+)\.(\d+)/);C=E[1]+x+E[2].charAt(0)+x+E[2].substring(1,E[2].length);}
else {C=RegExp.$1+x+RegExp.$2;if(RegExp.$3!=p){C+=x+RegExp.$3;}
;}
;}
;}
else if(qx.bom.client.Engine.__isWebkit()){if(/AppleWebKit\/([^ ]+)/.test(B)){C=RegExp.$1;var D=RegExp(c).exec(C);if(D){C=C.slice(0,D.index);}
;}
;}
else if(qx.bom.client.Engine.__isGecko()||qx.bom.client.Engine.__isMaple()){if(/rv\:([^\);]+)(\)|;)/.test(B)){C=RegExp.$1;}
;}
else {var z=window.qxFail;if(z&&typeof z===b){C=z().FULLVERSION;}
else {C=f;qx.Bootstrap.warn(o+B+s);}
;}
;return C;}
,getName:function(){var name;if(qx.bom.client.Engine.__isMshtml()){name=t;}
else if(qx.bom.client.Engine.__isOpera()){name=q;}
else if(qx.bom.client.Engine.__isWebkit()){name=v;}
...
,__isWebkit:function(){return window.navigator.userAgent.indexOf(m)!=-1;}
,__isMaple:function(){return window.navigator.userAgent.indexOf(l)!=-1;}
,__isGecko:function(){return window.navigator.mozApps&&window.navigator.product===k&&window.navigator.userAgent.indexOf(l)==-1&&window.navigator.userAgent.indexOf(n)==-1;}
,__isMshtml:function(){if(window.navigator.cpuClass&&(/MSIE\s+([^\);]+)(\)|;)/.test(window.navigator.userAgent)||/Trident\/\d+?\.\d+?/.test(window.navigator.userAgent))){return true;}
;if(qx.bom.client.Engine.__isWindowsPhone()){return true;}
;return false;}
,__isWindowsPhone:function(){return window.navigator.userAgent.indexOf(r)>-1;}
...
(function(){var a="name",b="qxWeb",c="toString",d="$",e="number",f="_",g="data-qx-class",h="basename",j="classname";qx.Bootstrap.define(b,{extend:qx.type.BaseArray,statics:{__init:[],$$qx:qx,$init:function(p,n){if(p.length&&p.length==1&&p[0]&&p[0].$widget instanceof qxWeb){return p[0].$widget;}
...
;var r=qx.lang.Array.cast(o,n);for(var i=0;i<qxWeb.__init.length;i++ ){qxWeb.__init[i].call(r);}
;return r;}
...
;}
,$attachInit:function(z){this.__init.push(z);}
...
(function(){var a="readOnly",b="data-",c="accessKey",d="qx.bom.element.Attribute",e="rowSpan",f="vAlign",g="className",h="textContent",i="htmlFor",j="longDesc",k="cellSpacing",l="frameBorder",m="",n="useMap",o="innerText",p="innerHTML",q="tabIndex",r="dateTime",s="maxLength",t="html.element.textcontent",u="mshtml",v="engine.name",w="cellPadding",x="browser.documentmode",y="colSpan",z="undefined";qx.Bootstrap.define(d,{statics:{__hints:{names:{"class":g,"for":i,html:p,text:qx.core.Environment.get(t)?h:o,colspan:y,rowspan:e,valign:f,datetime:r,accesskey:c,tabindex:q,maxlength:s,readonly:a,longdesc:j,cellpadding:w,cellspacing:k,frameborder:l,usemap:n},runtime:{"html":1,"text":1},bools:{compact:1,nowrap:1,ismap:1,declare:1,noshade:1,checked:1,disabled:1,readOnly:1,multiple:1,selected:1,noresize:1,defer:1,allowTransparency:1},property:{$$html:1,$$widget:1,checked:1,readOnly:1,multiple:1,selected:1,value:1,maxLength:1,className:1,innerHTML:1,innerText:1,textContent:1,htmlFor:1,tabIndex:1},qxProperties:{$$widget:1,$$html:1},propertyDefault:{disabled:false,checked:false,readOnly:false,multiple:false,selected:false,value:m,className:m,innerHTML:m,innerText:m,textContent:m,htmlFor:m,tabIndex:0,maxLength:qx.core.Environment.select(v,{"mshtml":2147483647,"webkit":524288,"default":-1})},removeableProperties:{disabled:1,multiple:1,maxLength:1}},get:function(C,name){var A=this.__hints;var B;name=A.names[name]||name;if(A.property[name]){B=C[name];if(typeof A.propertyDefault[name]!==z&&B==A.propertyDefault[name]){if(typeof A.bools[name]===z){return null;}
else {return B;}
;}
;}
else {B=C.getAttribute(name);if(A.bools[name]&&!(qx.core.Environment.get(v)==u&&parseInt(qx.core.Environment.get(x),10)<=8)){return qx.Bootstrap.isString(B);}
;}
;if(A.bools[name]){return !!B;}
;return B;}
,set:function(F,name,E){if(typeof E===z){return;}
;var D=this.__hints;name=D.names[name]||name;if(D.bools[name]&&!qx.lang.Type.isBoolean(E)){E=qx.lang.Type.isString(E);}
;if(D.property[name]&&(!(F[name]===undefined)||D.qxProperties[name])){if(E==null){if(D.removeableProperties[name]){F.removeAttribute(name);return;}
else if(typeof D.propertyDefault[name]!==z){E=D.propertyDefault[name];}
;}
;F[name]=E;}
else {if((D.bools[name]||E===null)&&name.indexOf(b)!==0){if(E===true){F.setAttribute(name,name);}
else if(E===false||E===null){F.removeAttribute(name);}
;}
else {F.setAttribute(name,E);}
;}
;}
}});}
)();
...
;return O;}
,__ids:{"Windows NT 10.0":H,"Windows NT 6.3":M,"Windows NT 6.2":J,"Windows NT 6.1":n,"Windows NT 6.0":I,"Windows NT 5.2":h,"Windows NT 5.1":Y,"Windows NT 5.0":L,"Windows 2000":L,"Windows NT 4.0":by,"Win 9x 4.90":A,"Windows CE":m,"Windows 98":F,"Win98":F,"Windows 95":u,"Win95":u,"Mac OS X 10_10":bo,"Mac OS X 10.10":bo,"Mac OS X 10_9":r,"Mac OS X 10.9":r,"Mac OS X 10_8":w,"Mac OS X 10.8":w,"Mac OS X 10_7":g,"Mac OS X 10.7":g,"Mac OS X 10_6":R,"Mac OS X 10.6":R,"Mac OS X 10_5":p,"Mac OS X 10.5":p,"Mac OS X 10_4":U,"Mac OS X 10.4":U,"Mac OS X 10_3":d,"Mac OS X 10.3":d,"Mac OS X 10_2":B,"Mac OS X 10.2":B,"Mac OS X 10_1":b,"Mac OS X 10.1":b,"Mac OS X 10_0":T,"Mac OS X 10.0":T},getVersion:function(){var bC=qx.bom.client.OperatingSystem.__getVersionForDesktopOs(navigator.userAgent);if(bC==null){bC=qx.bom.client.OperatingSystem.__getVersionForMobileOs(navigator.userAgent);}
;if(bC!=null){return bC;}
else {return O;}
;}
,__getVersionForDesktopOs:function(bD){var bG=[];for(var bF in qx.bom.client.OperatingSystem.__ids){bG.push(bF);}
;var bH=new RegExp(N+bG.join(s).replace(/\./g,x)+i,bt);var bE=bH.exec(bD);if(bE&&bE[1]){return qx.bom.client.OperatingSystem.__ids[bE[1]];}
;return null;}
,__getVersionForMobileOs:function(bN){var bI=bN.indexOf(e)!=-1;var bO=bN.indexOf(D)!=-1;var bJ=bN.match(/(iPad|iPhone|iPod)/i)?true:false;if(bI){var bQ=new RegExp(/Windows Phone (\d+(?:\.\d+)+)/i);var bL=bQ.exec(bN);if(bL&&bL[1]){return bL[1];}
;}
...
;}
;return name;}
,getVersion:function(){var T=navigator.userAgent;var U=new RegExp(B+qx.bom.client.Browser.__agents+n);var R=T.match(U);if(!R){return z;}
...
;}
,__agents:{"webkit":i,"gecko":p,"mshtml":q,"opera":H}[qx.bom.client.Engine.getName()]},defer:function(V){qx.core.Environment.add(k,V.getName);qx.core.Environment.add(C,V.getVersion);qx.core.Environment.add(K,V.getDocumentMode);qx.core.Environment.add(j,V.getQuirksMode);}
});}
)();
...
,__convertMap:{opt:[1,b,d],leg:[1,a,x],table:[1,w,H],tr:[2,y,q],td:[3,h,e],col:[2,o,s],def:qx.core.Environment.select(C,{"mshtml":[1,z,c],"default":null})},fixEmptyTags:function(N){return N.replace(/(<(\w+)[^>]*?)\/>/g,this.__fixNonDirectlyClosableHelper);}
,__convertHtmlString:function(S,T){var V=T.createElement(g);S=qx.bom.Html.fixEmptyTags(S);var P=S.replace(/^\s+/,v).substring(0,5).toLowerCase();var U,O=this.__convertMap;if(!P.indexOf(t)){U=O.opt;}
...
;}
;var Y,bb=[];for(var i=0,l=X.length;i<l;i++ ){Y=X[i];if(typeof Y===k){Y=this.__convertHtmlString(Y,bc);}
...
;return t;}
;return s;}
,equals:function(w,x){return qx.lang.Object.__equals(w,x,[],[]);}
,__equals:function(E,A,y,z){if(E===A){return E!==0||1/E==1/A;}
...
;y.push(E);z.push(A);var H=0,F=true;if(D==j){H=E.length;F=H==A.length;if(F){while(H-- ){if(!(F=qx.lang.Object.__equals(E[H],A[H],y,z))){break;}
;}
;}
;}
else {for(var G in E){if(Object.prototype.hasOwnProperty.call(E,G)){H++ ;if(!(F=Object.prototype.hasOwnProperty.call(A,G)&&qx.lang.Object.__equals(E[G],A[G],y,z))){break;}
;}
;}
...
,__helperElement:{},getHelperElement:function(I){if(!I){I=window;}
;var J=I.location.href;if(!qx.dom.Element.__helperElement[J]){var K=qx.dom.Element.__helperElement[J]=I.document.createElement(g);if(qx.core.Environment.get(a)==e){K.style.display=c;I.document.body.appendChild(K);}
;}
;return qx.dom.Element.__helperElement[J];}
...
;}
;return null;}
,_getWidth:function(t){var u=this[0];if(u){if(qx.dom.Node.isElement(u)){var r;if(t){var s={display:c,position:m,visibility:h};r=qx.module.Css.__swap(u,s,j,this);}
...
;}
;return null;}
,_getContentHeight:function(w){var y=this[0];if(qx.dom.Node.isElement(y)){var x;if(w){var v={position:m,visibility:h,display:c};x=qx.module.Css.__swap(y,v,g,this);}
else {x=qx.bom.element.Dimension.getContentHeight(y);}
;return x;}
;return null;}
,_getContentWidth:function(B){var z=this[0];if(qx.dom.Node.isElement(z)){var C;if(B){var A={position:m,visibility:h,display:c};C=qx.module.Css.__swap(z,A,f,this);}
else {C=qx.bom.element.Dimension.getContentWidth(z);}
;return C;}
;return null;}
,__displayDefaults:{},__getDisplayDefault:function(G,D){var F=qx.module.Css.__displayDefaults;if(!F[G]){var H=D||document;var E=qxWeb(H.createElement(G)).appendTo(D.body);F[G]=E.getStyle(a);E.remove();}
;return F[G]||b;}
,__swap:function(L,I,J,O){var M={};for(var N in I){M[N]=L.style[N];L.style[N]=I[N];}
...
else {var ba=qxWeb.getDocument(X[0]);V=qx.module.Css.__getDisplayDefault(X[0].tagName,ba);}
...
(function(){var b="qx.dom.Node",c="";qx.Bootstrap.define(b,{statics:{ELEMENT:1,TEXT:3,DOCUMENT:9,DOCUMENT_FRAGMENT:11,getDocument:function(d){return d.nodeType===this.DOCUMENT?d:d.ownerDocument||d.document;}
...
,isDocumentFragment:function(j){return !!(j&&j.nodeType===this.DOCUMENT_FRAGMENT);}
...
;switch(p.nodeType){case 1:var i,a=[],q=p.childNodes,length=q.length;for(i=0;i<length;i++ ){a[i]=this.getText(q[i]);}
;return a.join(c);case 2:case 3:case 4:return p.nodeValue;};return null;}
,isBlockNode:function(r){if(!qx.dom.Node.isElement(r)){return false;}
;r=qx.dom.Node.getName(r);return /^(body|form|textarea|fieldset|ul|ol|dl|dt|dd|li|div|hr|p|h[1-6]|quote|pre|table|thead|tbody|tfoot|tr|td|th|iframe|address|blockquote)$/.test(r);}
}});}
)();
...
,_getBoundingClientRect:function(t){var s=t.getBoundingClientRect();if(qx.core.Environment.get(k)===11&&!!document.msFullscreenElement&&window!==window.top&&this.__isChildOfFullScreenElement(t)){var q={};for(var r in s){q[r]=s[r]*100;}
;s=q;}
;return s;}
,__isChildOfFullScreenElement:function(u){if(document.msFullscreenElement===u){return true;}
;return qx.dom.Hierarchy.contains(document.msFullscreenElement,u);}
,__hiddenScrollbars:{visible:true,hidden:true},getContentWidth:function(y){var v=qx.bom.element.Style;var w=qx.bom.element.Style.get(y,j);var x=parseInt(v.get(y,f)||d,10);var B=parseInt(v.get(y,e)||d,10);if(this.__hiddenScrollbars[w]){var A=y.clientWidth;if((qx.core.Environment.get(b)==g)||qx.dom.Node.isBlockNode(y)){A=A-x-B;}
;if(qx.core.Environment.get(b)==a){if(A===0&&y.offsetHeight===0){return y.offsetWidth;}
;}
;return A;}
else {if(y.clientWidth>=y.scrollWidth){return Math.max(y.clientWidth,y.scrollWidth)-x-B;}
else {var z=y.scrollWidth-x;if(qx.core.Environment.get(b)==a){z-=B;}
;return z;}
;}
;}
,getContentHeight:function(G){var C=qx.bom.element.Style;var F=qx.bom.element.Style.get(G,h);var E=parseInt(C.get(G,i)||d,10);var D=parseInt(C.get(G,l)||d,10);if(this.__hiddenScrollbars[F]){return G.clientHeight-E-D;}
else {if(G.clientHeight>=G.scrollHeight){return Math.max(G.clientHeight,G.scrollHeight)-E-D;}
else {return G.scrollHeight-E;}
;}
;}
}});}
)();
...
(function(){var a="cursor:",b="engine.name",c="",d="mshtml",e="nw-resize",f="engine.version",g="nesw-resize",h="browser.documentmode",i=";",j="nwse-resize",k="qx.bom.element.Cursor",l="ne-resize",m="browser.quirksmode",n="cursor";qx.Bootstrap.define(k,{statics:{__map:{},compile:function(o){return a+(this.__map[o]||o)+i;}
,get:function(q,p){return qx.bom.element.Style.get(q,n,p,false);}
,set:function(s,r){s.style.cursor=this.__map[r]||r;}
,reset:function(t){t.style.cursor=c;}
},defer:function(u){if(qx.core.Environment.get(b)==d&&((parseFloat(qx.core.Environment.get(f))<9||qx.core.Environment.get(h)<9)&&!qx.core.Environment.get(m))){u.__map[g]=l;u.__map[j]=e;}
;}
});}
)();
...
(function(){var a="-",b="qx.bom.Style",c="",d='-',e="Webkit",f="ms",g=":",h=";",j="Moz",k="O",m="string",n="Khtml";qx.Bootstrap.define(b,{statics:{VENDOR_PREFIXES:[e,j,k,f,n],__cssName:{},__supports:null,getPropertyName:function(q){var o=document.documentElement.style;if(o[q]!==undefined){return q;}
...
;}
;return null;}
,getCssName:function(r){var s=this.__cssName[r];if(!s){s=r.replace(/[A-Z]/g,function(t){return (d+t.charAt(0).toLowerCase());}
);if((/^ms/.test(s))){s=a+s;}
;this.__cssName[r]=s;}
;return s;}
,getAppliedStyle:function(A,x,z,v){var C=qx.bom.Style.getCssName(x);var w=qx.dom.Node.getWindow(A);var u=(v!==false)?[null].concat(this.VENDOR_PREFIXES):[null];for(var i=0,l=u.length;i<l;i++ ){var y=false;var B=u[i]?a+u[i].toLowerCase()+a+z:z;if(qx.bom.Style.__supports){y=qx.bom.Style.__supports.call(w,C,B);}
else {A.style.cssText+=C+g+B+h;y=(typeof A.style[x]==m&&A.style[x]!==c);}
;if(y){return B;}
;}
;return null;}
},defer:function(D){if(window.CSS&&window.CSS.supports){qx.bom.Style.__supports=window.CSS.supports.bind(window.CSS);}
else if(window.supportsCSS){qx.bom.Style.__supports=window.supportsCSS.bind(window);}
;}
});}
)();
...
,getLinearGradient:function(){qx.bom.client.Css.__WEBKIT_LEGACY_GRADIENT=false;var cj=Q;var cg=document.createElement(o);var ch=qx.bom.Style.getAppliedStyle(cg,J,cj);if(!ch){cj=bN;var ch=qx.bom.Style.getAppliedStyle(cg,J,cj,false);if(ch){qx.bom.client.Css.__WEBKIT_LEGACY_GRADIENT=true;}
;}
;if(!ch){return null;}
;var ci=/(.*?)\(/.exec(ch);return ci?ci[1]:null;}
,getFilterGradient:function(){return qx.bom.client.Css.__isFilterSupported(bD,bA);}
...
,getFilterTextShadow:function(){return qx.bom.client.Css.__isFilterSupported(L,W);}
,__isFilterSupported:function(cv,ct){var cu=false;var cw=bR+cv+bG+ct+bJ;var cs=document.createElement(o);document.body.appendChild(cs);cs.style.filter=cw;if(cs.filters&&cs.filters.length>0&&cs.filters.item(cv).enabled==true){cu=true;}
...
else {if(t===o){this.__cssNames[g]=t;}
else {this.__cssNames[t]=qx.bom.Style.getCssName(u[t]);}
;}
;}
;this.__styleNames=u;}
,__getStyleName:function(name){var v=qx.bom.Style.getPropertyName(name);if(v){this.__styleNames[name]=v;}
;return v;}
,__mshtmlPixel:{width:m,height:k,left:f,right:l,top:r,bottom:j},__special:{clip:qx.bom.element.Clip,cursor:qx.bom.element.Cursor,opacity:qx.bom.element.Opacity,boxSizing:qx.bom.element.BoxSizing},COMPUTED_MODE:1,CASCADED_MODE:2,LOCAL_MODE:3,set:function(y,name,w,x){{}
...
;}
,get:function(C,name,E,G){name=this.__styleNames[name]||this.__getStyleName(name)||name;if(G!==false&&this.__special[name]){return this.__special[name].get(C,E);}
;switch(E){case this.LOCAL_MODE:return C.style[name]||q;case this.CASCADED_MODE:if(C.currentStyle){return C.currentStyle[name]||q;}
;throw new Error(i);default:var A=qx.dom.Node.getDocument(C);var D=A.defaultView?A.defaultView.getComputedStyle:undefined;if(D!==undefined){var z=D(C,null);if(z&&z[name]){return z[name];}
;}
else {if(!C.currentStyle){return C.style[name]||q;}
;var I=C.currentStyle[name]||C.style[name]||q;if(/^-?[\.\d]+(px)?$/i.test(I)){return I;}
;var H=this.__mshtmlPixel[name];if(H&&(H in C.style)){var F=C.style[name];C.style[name]=I||0;var B=C.style[H]+s;C.style[name]=F;return B;}
;return I;}
;return C.style[name]||q;};}
},defer:function(J){J.__detectVendorProperties();}
});}
)();
...
,__getOrientationNormalizer:function(m){var o=this.getWidth(m)>this.getHeight(m)?90:0;var n=m.orientation;if(n==null||Math.abs(n%180)==o){return {"-270":90,"-180":180,"-90":-90,"0":0,"90":90,"180":180,"270":-90};}
else {return {"-270":180,"-180":-90,"-90":0,"0":90,"90":180,"180":-90,"270":0};}
;}
,__orientationNormalizer:null,getOrientation:function(p){var p=p||window.top;var q=p.orientation;if(q==null){q=this.getWidth(p)>this.getHeight(p)?90:0;}
else {if(this.__orientationNormalizer==null){this.__orientationNormalizer=this.__getOrientationNormalizer(p);}
;q=this.__orientationNormalizer[q];}
;return q;}
...
,__computeScroll:function(E){var F=0,top=0;var D=qx.dom.Node.getWindow(E);F-=qx.bom.Viewport.getScrollLeft(D);top-=qx.bom.Viewport.getScrollTop(D);return {left:F,top:top};}
,__computeBody:qx.core.Environment.select(u,{"mshtml":function(I){var H=qx.dom.Node.getDocument(I);var G=H.body;var J=0;var top=0;J-=G.clientLeft+H.documentElement.clientLeft;top-=G.clientTop+H.documentElement.clientTop;if(!qx.core.Environment.get(t)){J+=this.__num(G,j);top+=this.__num(G,i);}
;return {left:J,top:top};}
,"webkit":function(M){var L=qx.dom.Node.getDocument(M);var K=L.body;var N=K.offsetLeft;var top=K.offsetTop;return {left:N,top:top};}
,"gecko":function(P){var O=qx.dom.Node.getDocument(P).body;var Q=O.offsetLeft;var top=O.offsetTop;if(qx.bom.element.BoxSizing.get(O)!==o){Q+=this.__num(O,j);top+=this.__num(O,i);}
;return {left:Q,top:top};}
,"default":function(S){var R=qx.dom.Node.getDocument(S).body;var T=R.offsetLeft;var top=R.offsetTop;return {left:T,top:top};}
}),__computeOffset:function(U){var V=U.getBoundingClientRect();return {left:Math.round(V.left),top:Math.round(V.top)};}
,get:function(bb,bc){if(bb.tagName==y){var location=this.__getBodyLocation(bb);var bf=location.left;var top=location.top;}
else {var W=this.__computeBody(bb);var ba=this.__computeOffset(bb);var scroll=this.__computeScroll(bb);var bf=ba.left+W.left-scroll.left;var top=ba.top+W.top-scroll.top;}
;var X=bf+bb.offsetWidth;var Y=top+bb.offsetHeight;if(bc){if(bc==p||bc==b){var be=qx.bom.element.Style.get(bb,z);if(be==b||be==g){X+=bb.scrollWidth-bb.offsetWidth+this.__num(bb,j)+this.__num(bb,f);}
;var bd=qx.bom.element.Style.get(bb,m);if(bd==b||bd==g){Y+=bb.scrollHeight-bb.offsetHeight+this.__num(bb,i)+this.__num(bb,a);}
;}
;switch(bc){case p:bf+=this.__num(bb,e);top+=this.__num(bb,r);X-=this.__num(bb,x);Y-=this.__num(bb,q);case b:bf-=bb.scrollLeft;top-=bb.scrollTop;X-=bb.scrollLeft;Y-=bb.scrollTop;case A:bf+=this.__num(bb,j);top+=this.__num(bb,i);X-=this.__num(bb,f);Y-=this.__num(bb,a);break;case w:bf-=this.__num(bb,n);top-=this.__num(bb,l);X+=this.__num(bb,s);Y+=this.__num(bb,k);break;};}
;return {left:bf,top:top,right:X,bottom:Y};}
,__getBodyLocation:function(bg){var top=bg.offsetTop;var bh=bg.offsetLeft;top+=this.__num(bg,l);bh+=this.__num(bg,n);if(qx.core.Environment.get(u)===d){top+=this.__num(bg,j);bh+=this.__num(bg,i);}
;return {left:bh,top:top};}
,getRelative:function(bl,bk,bj,bi){var bn=this.get(bl,bj);var bm=this.get(bk,bi);return {left:bn.left-bm.left,top:bn.top-bm.top,right:bn.right-bm.right,bottom:bn.bottom-bm.bottom};}
,getPosition:function(bo){return this.getRelative(bo,this.getOffsetParent(bo));}
,getOffsetParent:function(br){var bq=br.offsetParent||document.body;var bp=qx.bom.element.Style;while(bq&&(!/^body|html$/i.test(bq.tagName)&&bp.get(bq,v)===h)){bq=bq.offsetParent;}
;return bq;}
}});}
)();
(function(){var a='',b="g",c="(^|\\s)",d='function',e="(\\s|$)",f="",g="\\b|\\b",h="qx.bom.element.Class",j='SVGAnimatedString',k="html.classlist",m="default",n=" ",o='object',p="$2",q="native",r="\\b",s='undefined';qx.Bootstrap.define(h,{statics:{__splitter:/\s+/g,__trim:/^\s+|\s+$/g,add:{"native":function(t,name){if(name.length>0){t.classList.add(name);}
;return name;}
...
(function(){var a="mshtml",b="engine.name",c="complete",d="Array",f="pointerout",g="pointerover",h="load",n="left",o="qx.module.Event",p="undefined",q="DOMContentLoaded",r="browser.documentmode",s="*";qx.Bootstrap.define(o,{statics:{__normalizations:{},__hooks:{on:{},off:{}},__isReady:false,ready:function(t){if(document.readyState===c){window.setTimeout(t,1);return;}
;var u=function(){qx.module.Event.__isReady=true;t();}
...
else {var w=function(){if(qx.module.Event.__isReady){return;}
...
;var B=qx.module.Event.__normalizations;for(var i=0,l=D.length;i<l;i++ ){var C=D[i];if(qx.lang.Type.isFunction(A)){if(!B[C]){B[C]=[];}
;B[C].push(A);}
;}
;}
,$unregisterEventNormalization:function(H,E){if(!qx.lang.Type.isArray(H)){H=[H];}
;var F=qx.module.Event.__normalizations;for(var i=0,l=H.length;i<l;i++ ){var G=H[i];if(F[G]){qx.lang.Array.remove(F[G],E);}
;}
;}
,$getEventNormalizationRegistry:function(){return qx.module.Event.__normalizations;}
,$registerEventHook:function(N,K,J){if(!qx.lang.Type.isArray(N)){N=[N];}
;var L=qx.module.Event.__hooks.on;for(var i=0,l=N.length;i<l;i++ ){var M=N[i];if(qx.lang.Type.isFunction(K)){if(!L[M]){L[M]=[];}
;L[M].push(K);}
;}
;if(!J){return;}
;var I=qx.module.Event.__hooks.off;for(var i=0,l=N.length;i<l;i++ ){var M=N[i];if(qx.lang.Type.isFunction(J)){if(!I[M]){I[M]=[];}
;I[M].push(J);}
;}
;}
,$unregisterEventHook:function(T,Q,P){if(!qx.lang.Type.isArray(T)){T=[T];}
;var R=qx.module.Event.__hooks.on;for(var i=0,l=T.length;i<l;i++ ){var S=T[i];if(R[S]){qx.lang.Array.remove(R[S],Q);}
;}
;if(!P){return;}
;var O=qx.module.Event.__hooks.off;for(var i=0,l=T.length;i<l;i++ ){var S=T[i];if(O[S]){qx.lang.Array.remove(O[S],P);}
;}
;}
,$getEventHookRegistry:function(){return qx.module.Event.__hooks;}
},members:{on:function(bc,ba,bb,V){for(var i=0;i<this.length;i++ ){var U=this[i];var X=bb||qxWeb(U);var W=qx.module.Event.__hooks.on;var bd=W[s]||[];if(W[bc]){bd=bd.concat(W[bc]);}
;for(var j=0,m=bd.length;j<m;j++ ){bd[j](U,bc,ba,bb);}
;var Y=function(be,event){var bg=qx.module.Event.__normalizations;var bf=bg[s]||[];if(bg[bc]){bf=bf.concat(bg[bc]);}
...
;U.$$lastlistenerId=U.$$emitter.on(bc,Y,X);U.$$emitter.getEntryById(U.$$lastlistenerId).useCapture=!!V;if(!U.__listener){U.__listener={};}
;if(!U.__listener[bc]){U.__listener[bc]={};}
;U.__listener[bc][U.$$lastlistenerId]=Y;if(!bb){if(!U.__ctx){U.__ctx={};}
;U.__ctx[U.$$lastlistenerId]=X;}
;}
;return this;}
,off:function(br,bj,bo,bi){var bn=(bj===null&&bo===null);for(var j=0;j<this.length;j++ ){var bh=this[j];if(!bh.__listener){continue;}
;var bt=[];if(br!==null){bt.push(br);}
else {for(var bl in bh.__listener){bt.push(bl);}
;}
;for(var i=0,l=bt.length;i<l;i++ ){for(var bm in bh.__listener[bt[i]]){var bq=bh.__listener[bt[i]][bm];if(bn||bq==bj||bq.original==bj){var bk=typeof bh.__ctx!==p&&bh.__ctx[bm];var bu;if(!bo&&bk){bu=bh.__ctx[bm];}
;bh.$$emitter.off(bt[i],bq,bu||bo);if(bn||bq.original==bj){qx.bom.Event.removeNativeListener(bh,bt[i],bq,bi);}
;delete bh.__listener[bt[i]][bm];if(bk){delete bh.__ctx[bm];}
;}
;}
;var bp=qx.module.Event.__hooks.off;var bs=bp[s]||[];if(bp[br]){bs=bs.concat(bp[br]);}
...
(function(){var a="qx.event.Emitter",b="*";qx.Bootstrap.define(a,{extend:Object,statics:{__storage:[]},members:{__listener:null,__any:null,on:function(name,c,d){var e=qx.event.Emitter.__storage.length;this.__getStorage(name).push({listener:c,ctx:d,id:e,name:name});qx.event.Emitter.__storage.push({name:name,listener:c,ctx:d});return e;}
...
,off:function(name,m,k){var l=this.__getStorage(name);for(var i=l.length-1;i>=0;i-- ){var n=l[i];if(n.listener==m&&n.ctx==k){l.splice(i,1);qx.event.Emitter.__storage[n.id]=null;return n.id;}
;}
;return null;}
,offById:function(p){var o=qx.event.Emitter.__storage[p];if(o){this.off(o.name,o.listener,o.ctx);}
;return null;}
...
,emit:function(name,A){var x=this.__getStorage(name).concat();var y=[];for(var i=0;i<x.length;i++ ){var z=x[i];z.listener.call(z.ctx,A);if(z.once){y.push(z);}
;}
;y.forEach(function(B){var C=this.__getStorage(name);var D=C.indexOf(B);C.splice(D,1);}
.bind(this));x=this.__getStorage(b);for(var i=x.length-1;i>=0;i-- ){var z=x[i];z.listener.call(z.ctx,A);}
;}
,getListeners:function(){return this.__listener;}
,getEntryById:function(F){for(var name in this.__listener){var E=this.__listener[name];for(var i=0,j=E.length;i<j;i++ ){if(E[i].id===F){return E[i];}
;}
;}
;}
...
;if(!this.__nativePointerEvents){bh.stopPropagation();}
...
;if(bp.identifier==this.__primaryIdentifier){bk.isPrimary=true;bk.button=0;bk.buttons=1;qx.event.handler.PointerCore.__lastTouch={"x":bp.clientX,"y":bp.clientY,"time":new Date().getTime()};}
;var br=new qx.event.type.dom.Pointer(bm,bl,bk);this._fireEvent(br,bm,bk.target);if(bl.type==B||bl.type==N){var bs=new qx.event.type.dom.Pointer(W,bl,bk);this._fireEvent(bs,W,bl.target);if(this.__primaryIdentifier==bp.identifier){this.__primaryIdentifier=null;}
;}
;}
;}
...
;if(bt.type==c){this.__buttonStates[bt.which]=1;}
else if(bt.type==M){if(qx.core.Environment.get(b)==C&&qx.core.Environment.get(L)==J){if(this.__buttonStates[bt.which]!=1&&bt.ctrlKey){this.__buttonStates[1]=0;}
;}
;this.__buttonStates[bt.which]=0;}
;var bv=qx.event.handler.PointerCore.MOUSE_TO_POINTER_MAPPING[bt.type];var bu=qx.bom.Event.getTarget(bt);var bw=qx.lang.Array.sum(this.__buttonStates);var bz={pointerType:s,pointerId:1};if(this.__lastButtonState!=bw&&bw!==0&&this.__lastButtonState!==0){var bx=new qx.event.type.dom.Pointer(H,bt,bz);this._fireEvent(bx,H,bu);}
;this.__lastButtonState=bw;if(bt.type==c&&bw>1){return;}
;if(bt.type==M&&bw>0){return;}
;if(bt.type==O){this.__buttonStates[bt.which]=0;return;}
;var by=new qx.event.type.dom.Pointer(bv,bt,bz);this._fireEvent(by,bv,bu);}
,_determineActiveTouches:function(bD,bC){if(bD==q){for(var i=0;i<bC.length;i++ ){this.__activeTouches.push(bC[i]);}
;}
else if(bD==B||bD==N){var bA=[];for(var i=0;i<this.__activeTouches.length;i++ ){var bB=true;for(var j=0;j<bC.length;j++ ){if(this.__activeTouches[i].identifier==bC[j].identifier){bB=false;break;}
;}
;if(bB){bA.push(this.__activeTouches[i]);}
;}
;this.__activeTouches=bA;}
;}
,_isSimulatedMouseEvent:function(x,y){var bF=qx.event.handler.PointerCore.__lastTouch;if(bF){var bG=new Date().getTime()-bF.time;var bE=qx.event.handler.PointerCore.SIM_MOUSE_DISTANCE;var bI=Math.abs(x-qx.event.handler.PointerCore.__lastTouch.x);var bH=Math.abs(y-qx.event.handler.PointerCore.__lastTouch.y);if(bG<qx.event.handler.PointerCore.SIM_MOUSE_DELAY){if(bI<bE||bH<bE){return true;}
;}
;}
;return false;}
,_stopObserver:function(){for(var i=0;i<this.__eventNames.length;i++ ){qx.bom.Event.removeNativeListener(this.__defaultTarget,this.__eventNames[i],this.__wrappedListener);}
;}
...
;if(qx.core.Environment.get(d)){if(!this.__nativePointerEvents){bL.dispatchEvent(bK);}
...
;}
;}
,dispose:function(){this._stopObserver();this.__defaultTarget=this.__emitter=null;}
}});}
)();
...
(function(){var a="scrollLeft",b="qx.module.Manipulating",c="ease-in",d="scrollTop";qx.Bootstrap.define(b,{statics:{_animationDescription:{scrollLeft:{duration:700,timing:c,keep:100,keyFrames:{'0':{},'100':{scrollLeft:1}}},scrollTop:{duration:700,timing:c,keep:100,keyFrames:{'0':{},'100':{scrollTop:1}}}},__animateScroll:function(f,g,e){var h=qx.lang.Object.clone(qx.module.Manipulating._animationDescription[f],true);h.keyFrames[100][f]=g;return this.animate(h,e);}
,__getCollectionFromArgument:function(o){var k;if(qx.lang.Type.isArray(o)){k=qxWeb(o);}
else {var n=qx.bom.Html.clean([o]);if(n.length>0&&qx.dom.Node.isElement(n[0])){k=qxWeb(n);}
else {k=qxWeb(o);}
;}
;return k;}
,__getInnermostElement:function(p){if(p.childNodes.length==0){return p;}
;for(var i=0,l=p.childNodes.length;i<l;i++ ){if(p.childNodes[i].nodeType===1){return this.__getInnermostElement(p.childNodes[i]);}
;}
;return p;}
,__getElementArray:function(r){if(!qx.lang.Type.isArray(r)){var q=qxWeb(r);r=q.length>0?q:[r];}
...
,appendTo:function(parent){parent=qx.module.Manipulating.__getElementArray(parent);for(var i=0,l=parent.length;i<l;i++ ){this._forEachElement(function(C,j){if(i==0){qx.dom.Element.insertEnd(this[j],parent[i]);}
else {qx.dom.Element.insertEnd(this.eq(j).clone(true)[0],parent[i]);}
;}
);}
;return this;}
,insertBefore:function(D){D=qx.module.Manipulating.__getElementArray(D);for(var i=0,l=D.length;i<l;i++ ){this._forEachElement(function(E,F){if(i==0){qx.dom.Element.insertBefore(E,D[i]);}
else {qx.dom.Element.insertBefore(this.eq(F).clone(true)[0],D[i]);}
;}
);}
;return this;}
,insertAfter:function(G){G=qx.module.Manipulating.__getElementArray(G);for(var i=0,l=G.length;i<l;i++ ){for(var j=this.length-1;j>=0;j-- ){if(!this[j]||this[j].nodeType!==1){continue;}
;if(i==0){qx.dom.Element.insertAfter(this[j],G[i]);}
else {qx.dom.Element.insertAfter(this.eq(j).clone(true)[0],G[i]);}
;}
;}
;return this;}
,wrap:function(H){H=qx.module.Manipulating.__getCollectionFromArgument(H);if(H.length==0){return this;}
;this._forEachElement(function(I){var J=H.eq(0).clone(true);qx.dom.Element.insertAfter(J[0],I);var K=qx.module.Manipulating.__getInnermostElement(J[0]);qx.dom.Element.insertEnd(I,K);}
);return this;}
...
;return Y.scrollTop;}
,setScrollLeft:function(bb,bc){var Node=qx.dom.Node;if(bc&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__animateScroll.bind(this,a,bb,bc)();}
...
;}
;return this;}
,setScrollTop:function(be,bf){var Node=qx.dom.Node;if(bf&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__animateScroll.bind(this,d,be,bf)();}
...
,__getElementFromArgument:function(v){if(v instanceof qxWeb){return v[0];}
else if(qx.Bootstrap.isString(v)){return qxWeb(v)[0];}
;return v;}
,__getNodeFromArgument:function(w){if(typeof w==q){w=qxWeb(w);}
;if(w instanceof Array||w instanceof qxWeb){w=w[0];}
;return qxWeb.isNode(w)?w:null;}
,__getAttributes:function(x){var y={};for(var z in x.attributes){if(z==p){continue;}
;var name=x.attributes[z].name;var A=x.attributes[z].value;y[name]=A;}
;return y;}
,__hierarchyHelper:function(D,G,E){var B=[];var C=qx.dom.Hierarchy;for(var i=0,l=D.length;i<l;i++ ){B.push.apply(B,C[G](D[i]));}
;var F=qx.lang.Array.unique(B);if(E){F=qx.bom.Selector.matches(E,F);}
;return F;}
,isElement:function(H){return qx.dom.Node.isElement(qx.module.Traversing.__getElementFromArgument(H));}
,isNode:function(I){return qx.dom.Node.isNode(qx.module.Traversing.__getElementFromArgument(I));}
,isNodeName:function(K,J){return qx.dom.Node.isNodeName(qx.module.Traversing.__getElementFromArgument(K),J);}
...
,getWindow:function(N){return qx.dom.Node.getWindow(qx.module.Traversing.__getElementFromArgument(N));}
...
,getDocument:function(Q){return qx.dom.Node.getDocument(qx.module.Traversing.__getElementFromArgument(Q));}
,getNodeName:function(R){return qx.dom.Node.getName(qx.module.Traversing.__getElementFromArgument(R));}
,getNodeText:function(S){return qx.dom.Node.getText(qx.module.Traversing.__getElementFromArgument(S));}
,isBlockNode:function(T){return qx.dom.Node.isBlockNode(qx.module.Traversing.__getElementFromArgument(T));}
,equalNodes:function(bd,bf){bd=qx.module.Traversing.__getNodeFromArgument(bd);bf=qx.module.Traversing.__getNodeFromArgument(bf);if(!bd||!bf){return false;}
...
;}
;if(bc){var U=qx.module.Traversing.__getAttributes(bd);var be=qx.module.Traversing.__getAttributes(bf);for(var W in U){if(U[W]!==be[W]){return false;}
;}
;}
...
;}
;return bq;}
,getAncestors:function(bt){return this.__getAncestors(null,bt);}
,getAncestorsUntil:function(bv,bu){return this.__getAncestors(bv,bu);}
...
;return bO;}
,getNextAll:function(bR){var bQ=qx.module.Traversing.__hierarchyHelper(this,f,bR);return qxWeb.$init(bQ,qxWeb);}
...
;return bX;}
,getPrevAll:function(cb){var ca=qx.module.Traversing.__hierarchyHelper(this,k,cb);return qxWeb.$init(ca,qxWeb);}
...
,getSiblings:function(ci){var ch=qx.module.Traversing.__hierarchyHelper(this,n,ci);return qxWeb.$init(ch,qxWeb);}
...
},defer:function(cp){qxWeb.$attachAll(this);qxWeb.$attach({"__getAncestors":cp.__getAncestors});}
});}
)();
(function(){var a="swipe",b="function",c="getDistance",d="getStartTime",e="getDirection",f="getAxis",g="getDuration",h="getVelocity",j="qx.module.event.Swipe";qx.Bootstrap.define(j,{statics:{TYPES:[a],BIND_METHODS:[d,g,f,e,h,c],getStartTime:function(){return this._original.swipe.startTime;}
,getDuration:function(){return this._original.swipe.duration;}
,getAxis:function(){return this._original.swipe.axis;}
,getDirection:function(){return this._original.swipe.direction;}
,getVelocity:function(){return this._original.swipe.velocity;}
,getDistance:function(){return this._original.swipe.distance;}
,normalize:function(event,m){if(!event){return event;}
;var k=qx.module.event.Swipe.BIND_METHODS;for(var i=0,l=k.length;i<l;i++ ){if(typeof event[k[i]]!=b){event[k[i]]=qx.module.event.Swipe[k[i]].bind(event);}
;}
;return event;}
},defer:function(n){qxWeb.$registerEventNormalization(qx.module.event.Swipe.TYPES,n.normalize);}
});}
)();
(function(){var a="getScale",b="qx.module.event.Pinch",c="pinch",d="function";qx.Bootstrap.define(b,{statics:{TYPES:[c],BIND_METHODS:[a],getScale:function(){return this._original.scale;}
,normalize:function(event,f){if(!event){return event;}
;var e=qx.module.event.Pinch.BIND_METHODS;for(var i=0,l=e.length;i<l;i++ ){if(typeof event[e[i]]!=d){event[e[i]]=qx.module.event.Pinch[e[i]].bind(event);}
;}
;return event;}
},defer:function(g){qxWeb.$registerEventNormalization(qx.module.event.Pinch.TYPES,g.normalize);}
});}
)();
(function(){var a="tap",b="swipe",c="qx.module.event.Touch",d="longtap",e="dbltap";qx.Bootstrap.define(c,{statics:{TYPES:[a,d,b,e],normalize:function(event,g,f){if(!event){return event;}
;event._type=f;return event;}
},defer:function(h){qxWeb.$registerEventNormalization(h.TYPES,h.normalize);}
});}
)();
...
(function(){var a="swipe",b="pinch",c="event.dispatchevent",d="gesturemove",e="touch",f="longtap",g="event.mousewheel",h="roll",i="dblclick",j="wheel",k="rotate",l="trackstart",m="gesturefinish",n="y",o="browser.documentmode",p="dbltap",q="qx.event.handler.GestureCore",r="right",s="mshtml",t="engine.name",u="gesturecancel",v="gesturebegin",w="track",z="trackend",A="left",B="tap",C="down",D="x",E="up";qx.Bootstrap.define(q,{extend:Object,statics:{TYPES:[B,a,f,p,w,l,z,k,b,h],GESTURE_EVENTS:[v,m,d,u],TAP_MAX_DISTANCE:{"touch":40,"mouse":5,"pen":20},DOUBLETAP_MAX_DISTANCE:{"touch":10,"mouse":4,"pen":10},SWIPE_DIRECTION:{x:[A,r],y:[E,C]},LONGTAP_TIME:500,DOUBLETAP_TIME:500,ROLL_FACTOR:18},construct:function(F,G){this.__defaultTarget=F;this.__emitter=G;this.__gesture={};this.__lastTap={};this.__stopMomentum={};this._initObserver();}
,members:{__defaultTarget:null,__emitter:null,__gesture:null,__eventName:null,__primaryTarget:null,__isMultiPointerGesture:null,__initialAngle:null,__lastTap:null,__rollImpulseId:null,__stopMomentum:null,__initialDistance:null,_initObserver:function(){qx.event.handler.GestureCore.GESTURE_EVENTS.forEach(function(I){qxWeb(this.__defaultTarget).on(I,this.checkAndFireGesture,this);}
.bind(this));if(qx.core.Environment.get(t)==s&&qx.core.Environment.get(o)<9){qxWeb(this.__defaultTarget).on(i,this._onDblClick,this);}
;var H=qx.core.Environment.get(g);qxWeb(H.target).on(H.type,this._fireRoll,this);}
,_stopObserver:function(){qx.event.handler.GestureCore.GESTURE_EVENTS.forEach(function(K){qxWeb(this.__defaultTarget).off(K,this.checkAndFireGesture,this);}
.bind(this));if(qx.core.Environment.get(t)==s&&qx.core.Environment.get(o)<9){qxWeb(this.__defaultTarget).off(i,this._onDblClick,this);}
...
;}
,gestureBegin:function(O,P){if(this.__gesture[O.pointerId]){this.__stopLongTapTimer(this.__gesture[O.pointerId]);delete this.__gesture[O.pointerId];}
;if(this._hasIntermediaryHandler(P)){return;}
;this.__gesture[O.pointerId]={"startTime":new Date().getTime(),"lastEventTime":new Date().getTime(),"startX":O.clientX,"startY":O.clientY,"clientX":O.clientX,"clientY":O.clientY,"velocityX":0,"velocityY":0,"target":P,"isTap":true,"isPrimary":O.isPrimary,"longTapTimer":window.setTimeout(this.__fireLongTap.bind(this,O,P),qx.event.handler.GestureCore.LONGTAP_TIME)};if(O.isPrimary){this.__isMultiPointerGesture=false;this.__primaryTarget=P;this.__fireTrack(l,O,P);}
else {this.__isMultiPointerGesture=true;if(Object.keys(this.__gesture).length===2){this.__initialAngle=this._calcAngle();this.__initialDistance=this._calcDistance();}
;}
;}
,gestureMove:function(R,S){var T=this.__gesture[R.pointerId];if(T){var Q=T.clientX;var U=T.clientY;T.clientX=R.clientX;T.clientY=R.clientY;T.lastEventTime=new Date().getTime();if(Q){T.velocityX=T.clientX-Q;}
;if(U){T.velocityY=T.clientY-U;}
;if(Object.keys(this.__gesture).length===2){this.__fireRotate(R,T.target);this.__firePinch(R,T.target);}
;if(!this.__isMultiPointerGesture){this.__fireTrack(w,R,T.target);this._fireRoll(R,e,T.target);}
;if(T.isTap){T.isTap=this._isBelowTapMaxDistance(R);if(!T.isTap){this.__stopLongTapTimer(T);}
;}
;}
;}
,_hasIntermediaryHandler:function(V){while(V&&V!==this.__defaultTarget){if(V.$$gestureHandler){return true;}
;V=V.parentNode;}
;return false;}
,gestureFinish:function(X,Y){if(!this.__gesture[X.pointerId]){return;}
;var bf=this.__gesture[X.pointerId];this.__stopLongTapTimer(bf);if(this._hasIntermediaryHandler(Y)){return;}
;this.__handleRollImpulse(bf.velocityX,bf.velocityY,X,bf.target);this.__fireTrack(z,X,bf.target);if(bf.isTap){if(Y!==bf.target){delete this.__gesture[X.pointerId];return;}
;this._fireEvent(X,B,X.target||Y);var ba=false;if(Object.keys(this.__lastTap).length>0){var be=Date.now()-qx.event.handler.GestureCore.DOUBLETAP_TIME;for(var bg in this.__lastTap){if(bg<be){delete this.__lastTap[bg];}
else {var W=this.__lastTap[bg];var bc=this.__isBelowDoubleTapDistance(W.x,W.y,X.clientX,X.clientY,X.getPointerType());var bd=W.target===(X.target||Y);var bh=W.button===X.button;if(bc&&bh&&bd){ba=true;delete this.__lastTap[bg];this._fireEvent(X,p,X.target||Y);}
;}
;}
;}
;if(!ba){this.__lastTap[Date.now()]={x:X.clientX,y:X.clientY,target:X.target||Y,button:X.button};}
;}
else if(!this._isBelowTapMaxDistance(X)){var bb=this.__getSwipeGesture(X,Y);if(bb){X.swipe=bb;this._fireEvent(X,a,bf.target||Y);}
;}
;delete this.__gesture[X.pointerId];}
,stopMomentum:function(bi){this.__stopMomentum[bi]=true;}
,gestureCancel:function(bj){if(this.__gesture[bj]){this.__stopLongTapTimer(this.__gesture[bj]);delete this.__gesture[bj];}
;}
,updateGestureTarget:function(bk,bl){this.__gesture[bk].target=bl;}
,__handleRollImpulse:function(bq,br,bn,bo,bt){var bs=bn.timeoutId;if((Math.abs(br)<1&&Math.abs(bq)<1)||this.__stopMomentum[bs]){delete this.__stopMomentum[bs];return;}
;if(!bt){bt=1;var bp=2.8;br=br/bp;bq=bq/bp;}
;bt+=0.0006;br=br/bt;bq=bq/bt;var bm=qx.bom.AnimationFrame.request(qx.lang.Function.bind(function(bu,bv,bw,bx,by){this.__handleRollImpulse(bu,bv,bw,bx,by);}
...
,_calcAngle:function(){var bA=null;var bB=null;for(var bz in this.__gesture){var bC=this.__gesture[bz];if(bA===null){bA=bC;}
else {bB=bC;}
;}
;var x=bA.clientX-bB.clientX;var y=bA.clientY-bB.clientY;return (360+Math.atan2(y,x)*(180/Math.PI))%360;}
,_calcDistance:function(){var bD=null;var bE=null;for(var bG in this.__gesture){var bH=this.__gesture[bG];if(bD===null){bD=bH;}
else {bE=bH;}
;}
...
,__isBelowDoubleTapDistance:function(bL,bP,bQ,bR,bS){var bO=qx.event.handler.GestureCore;var bM=Math.abs(bL-bQ)<bO.DOUBLETAP_MAX_DISTANCE[bS];var bN=Math.abs(bP-bR)<bO.DOUBLETAP_MAX_DISTANCE[bS];return bM&&bN;}
,_getDeltaCoordinates:function(bV){var bW=this.__gesture[bV.pointerId];if(!bW){return null;}
;var bT=bV.clientX-bW.startX;var bU=bV.clientY-bW.startY;var bX=D;if(Math.abs(bT/bU)<1){bX=n;}
;return {"x":bT,"y":bU,"axis":bX};}
,_fireEvent:function(ca,cc,cb){if(!this.__defaultTarget){return;}
...
else if(this.__emitter){bY=new qx.event.type.dom.Custom(cc,ca,{target:this.__defaultTarget,currentTarget:this.__defaultTarget,srcElement:this.__defaultTarget,swipe:ca.swipe,scale:ca.scale,angle:ca.angle,delta:ca.delta,pointerType:ca.pointerType,momentum:ca.momentum});this.__emitter.emit(cc,ca);}
;}
,_onDblClick:function(cd){var ce=qx.bom.Event.getTarget(cd);this._fireEvent(cd,B,ce);this._fireEvent(cd,p,ce);}
,__getSwipeGesture:function(ch,ci){var co=this.__gesture[ch.pointerId];if(!co){return null;}
...
,__fireTrack:function(cq,cr,cs){cr.delta=this._getDeltaCoordinates(cr);this._fireEvent(cr,cq,cr.target||cs);}
...
else {var cw=this.__gesture[cu.pointerId];cu.delta={x:-cw.velocityX,y:-cw.velocityY,axis:Math.abs(cw.velocityX/cw.velocityY)<1?n:D};}
;this._fireEvent(cu,h,cu.target||cv);}
,__fireRotate:function(cx,cz){if(!cx.isPrimary){var cy=this._calcAngle();cx.angle=Math.round((cy-this.__initialAngle)%360);this._fireEvent(cx,k,this.__primaryTarget);}
;}
,__firePinch:function(cC,cD){if(!cC.isPrimary){var cA=this._calcDistance();var cB=cA/this.__initialDistance;cC.scale=(Math.round(cB*100)/100);this._fireEvent(cC,b,this.__primaryTarget);}
;}
,__fireLongTap:function(cE,cF){var cG=this.__gesture[cE.pointerId];if(cG){this._fireEvent(cE,f,cE.target||cF);cG.longTapTimer=null;cG.isTap=false;}
;}
,__stopLongTapTimer:function(cH){if(cH.longTapTimer){window.clearTimeout(cH.longTapTimer);cH.longTapTimer=null;}
;}
...
,dispose:function(){for(var cK in this.__gesture){this.__stopLongTapTimer(cK);}
;this._stopObserver();this.__defaultTarget=this.__emitter=null;}
}});}
)();
(function(){var b="ease-in-out",c="Number",d="css.animation.requestframe",e="qx.bom.AnimationFrame",f="frame",g="end",h="linear",j="ease-in",k="ease-out";qx.Bootstrap.define(e,{extend:qx.event.Emitter,events:{"end":undefined,"frame":c},members:{__canceled:false,startSequence:function(l){this.__canceled=false;var m=+(new Date());var n=function(p){if(this.__canceled){this.id=null;return;}
...
,cancelSequence:function(){this.__canceled=true;}
...
;return y;}
,request:function(r,t){var s=qx.core.Environment.get(d);var u=function(v){if(v<1e10){v=this.__start+v;}
;v=v||+(new Date());r.call(t,v);}
;if(s){return window[s](u);}
else {return window.setTimeout(function(){u();}
,qx.bom.AnimationFrame.TIMEOUT);}
;}
},defer:function(w){w.__start=window.performance&&performance.timing&&performance.timing.navigationStart;if(!w.__start){w.__start=Date.now();}
;}
});}
)();
...
;return this.__normalize(f);}
;if(d===a){var x=0;if(e.wheelDelta!==undefined){if(e.wheelDeltaX!==undefined){x=e.wheelDeltaX?this.__normalize(-e.wheelDeltaX):0;}
;}
else {if(e.axis&&e.axis==e.HORIZONTAL_AXIS&&(e.detail!==undefined)&&(e.detail>0)){x=this.__normalize(e.detail);}
else if(e.deltaX!==undefined){x=this.__normalize(e.deltaX);}
;}
;return x;}
;if(d===b){var y=0;if(e.wheelDelta!==undefined){if(e.wheelDeltaY!==undefined){y=e.wheelDeltaY?this.__normalize(-e.wheelDeltaY):0;}
else {y=this.__normalize(-e.wheelDelta);}
;}
else {if(!(e.axis&&e.axis==e.HORIZONTAL_AXIS)&&(e.detail!==undefined)&&(e.detail>0)){y=this.__normalize(e.detail);}
else if(e.deltaY!==undefined){y=this.__normalize(e.deltaY);}
;}
;return y;}
;return 0;}
,__normalize:function(j){var g=Math.abs(j);if(g===0){return 0;}
;if(qx.util.Wheel.MINSCROLL==null||qx.util.Wheel.MINSCROLL>g){qx.util.Wheel.MINSCROLL=g;this.__recalculateMultiplicator();}
;if(qx.util.Wheel.MAXSCROLL==null||qx.util.Wheel.MAXSCROLL<g){qx.util.Wheel.MAXSCROLL=g;this.__recalculateMultiplicator();}
...
,__recalculateMultiplicator:function(){var k=qx.util.Wheel.MAXSCROLL||0;var n=qx.util.Wheel.MINSCROLL||k;if(k<=n){return;}
;var l=k-n;var m=(k/l)*Math.log(l);if(m==0){m=1;}
;qx.util.Wheel.FACTOR=6/m;}
}});}
)();
(function(){var a="qx.module.event.Rotate",b="getAngle",c="function",d="rotate";qx.Bootstrap.define(a,{statics:{TYPES:[d],BIND_METHODS:[b],getAngle:function(){return this._original.angle;}
,normalize:function(event,f){if(!event){return event;}
;var e=qx.module.event.Rotate.BIND_METHODS;for(var i=0,l=e.length;i<l;i++ ){if(typeof event[e[i]]!=c){event[e[i]]=qx.module.event.Rotate[e[i]].bind(event);}
;}
;return event;}
},defer:function(g){qxWeb.$registerEventNormalization(qx.module.event.Rotate.TYPES,g.normalize);}
});}
)();
(function(){var a="track",b="qx.module.event.Track",c="function",d="getDelta";qx.Bootstrap.define(b,{statics:{TYPES:[a],BIND_METHODS:[d],getDelta:function(){return this._original.delta;}
,normalize:function(event,f){if(!event){return event;}
;var e=qx.module.event.Track.BIND_METHODS;for(var i=0,l=e.length;i<l;i++ ){if(typeof event[e[i]]!=c){event[e[i]]=qx.module.event.Track[e[i]].bind(event);}
;}
;return event;}
},defer:function(g){qxWeb.$registerEventNormalization(qx.module.event.Track.TYPES,g.normalize);}
});}
)();
...
(function(){var a="qx.module.event.Pointer",b="pointerup",c="pointerover",d="pointerdown",e="pointermove",f="pointercancel",g="pointerout";qx.Bootstrap.define(a,{statics:{TYPES:[d,b,e,f,c,g],getPointerType:function(){return false;}
,getViewportLeft:function(){return false;}
,getViewportTop:function(){return false;}
,getDocumentLeft:function(){return false;}
,getDocumentTop:function(){return false;}
,getScreenLeft:function(){return false;}
,getScreenTop:function(){return false;}
,normalize:function(event,h){if(!event){return event;}
;qx.event.type.dom.Pointer.normalize(event);return event;}
},defer:function(i){qxWeb.$registerEventNormalization(qx.module.event.Pointer.TYPES,i.normalize);}
});}
)();
...
;if(!d.__orientationHandler){if(!d.$$emitter){d.$$emitter=new qx.event.Emitter();}
;d.__orientationHandler=new qx.event.handler.OrientationCore(d,d.$$emitter);}
;}
,unregister:function(g){if(g.__orientationHandler){if(!g.$$emitter){g.__orientationHandler=null;}
else {var f=false;var e=g.$$emitter.getListeners();qx.module.event.OrientationHandler.TYPES.forEach(function(h){if(h in e&&e[h].length>0){f=true;}
;}
...
(function(){var a="qx.module.event.TouchHandler",b="touchmove",c="touchcancel",d="touchstart",e="touchend";qx.Bootstrap.define(a,{statics:{TYPES:[d,e,b,c],register:function(f){if(!f.__touchHandler){if(!f.$$emitter){f.$$emitter=new qx.event.Emitter();}
;f.__touchHandler=new qx.event.handler.TouchCore(f,f.$$emitter);}
;}
,unregister:function(i){if(i.__touchHandler){if(!i.$$emitter){i.__touchHandler=null;}
else {var h=false;var g=i.$$emitter.getListeners();qx.module.event.TouchHandler.TYPES.forEach(function(j){if(j in g&&g[j].length>0){h=true;}
;}
);if(!h){i.__touchHandler=null;}
;}
;}
;}
},defer:function(k){qxWeb.$registerEventHook(k.TYPES,k.register,k.unregister);}
});}
)();
(function(){var a="touchmove",b="os.name",c="MSPointerDown",d="android",e="engine.version",f="pointercancel",g="qx.event.handler.TouchCore",h="event.mspointer",j="MSPointerCancel",k="y",l="pointer-events",m="pointerup",n="touchend",o="pointerdown",p="MSPointerUp",q="right",r="engine.name",s="undefined",t="touchcancel",u="MSPointerMove",v="webkit",w="none",z="left",A="pointermove",B="down",C="x",D="up",E="touchstart";qx.Bootstrap.define(g,{extend:Object,statics:{TAP_MAX_DISTANCE:qx.core.Environment.get(b)!=d?10:40,SWIPE_DIRECTION:{x:[z,q],y:[D,B]},SWIPE_MIN_DISTANCE:qx.core.Environment.get(b)!=d?11:41,SWIPE_MIN_VELOCITY:0,LONGTAP_TIME:500},construct:function(F,G){this.__target=F;this.__emitter=G;this._initTouchObserver();this.__pointers=[];this.__touchStartPosition={};}
,members:{__target:null,__emitter:null,__onTouchEventWrapper:null,__originalTarget:null,__touchStartPosition:null,__startTime:null,__beginScalingDistance:null,__beginRotation:null,__pointers:null,__touchEventNames:null,_initTouchObserver:function(){this.__onTouchEventWrapper=qx.lang.Function.listener(this._onTouchEvent,this);this.__touchEventNames=[E,a,n,t];if(qx.core.Environment.get(h)){var H=parseInt(qx.core.Environment.get(e),10);if(H==10){this.__touchEventNames=[c,u,p,j];}
else {this.__touchEventNames=[o,A,m,f];}
;}
;for(var i=0;i<this.__touchEventNames.length;i++ ){qx.bom.Event.addNativeListener(this.__target,this.__touchEventNames[i],this.__onTouchEventWrapper);}
;}
,_stopTouchObserver:function(){for(var i=0;i<this.__touchEventNames.length;i++ ){qx.bom.Event.removeNativeListener(this.__target,this.__touchEventNames[i],this.__onTouchEventWrapper);}
;}
...
;return O;}
,_calcSingleTouchDelta:function(S){if(this.__touchStartPosition.hasOwnProperty(S.identifier)){var R=this.__touchStartPosition[S.identifier];var P=Math.floor(S.clientX-R[0]);var Q=Math.floor(S.clientY-R[1]);var T=C;if(Math.abs(P/Q)<1){T=k;}
...
;V.delta=[];if(ba==E){this.__originalTarget=this._getTarget(V);if(V.touches&&V.touches.length>1){this.__beginScalingDistance=this._getScalingDistance(V.touches[0],V.touches[1]);this.__beginRotation=this._getRotationAngle(V.touches[0],V.touches[1]);}
;for(var i=0;i<V.changedTouches.length;i++ ){var Y=V.changedTouches[i];this.__touchStartPosition[Y.identifier]=[Y.clientX,Y.clientY];}
;}
;if(ba==a){if(typeof V.scale==s&&V.targetTouches.length>1){var W=this._getScalingDistance(V.targetTouches[0],V.targetTouches[1]);V.scale=W/this.__beginScalingDistance;}
;if((typeof V.rotation==s||qx.core.Environment.get(h))&&V.targetTouches.length>1){var X=this._getRotationAngle(V.targetTouches[0],V.targetTouches[1]);V._rotation=X-this.__beginRotation;}
;V.delta=this._calcTouchesDelta(V.targetTouches);}
;this._fireEvent(V,ba,this.__originalTarget);if(qx.core.Environment.get(h)){if(ba==n||ba==t){delete this.__pointers[V.pointerId];}
;}
;if(ba==n||ba==t&&V.changedTouches[0]){delete this.__touchStartPosition[V.changedTouches[0].identifier];}
;}
,_detectTouchesByPointer:function(bd,bf){var bc=[];if(bf==E){this.__pointers[bd.pointerId]=bd;}
else if(bf==a){this.__pointers[bd.pointerId]=bd;}
;for(var be in this.__pointers){var bb=this.__pointers[be];bc.push(bb);}
;return bc;}
...
;}
else if(qx.core.Environment.get(h)){var bh=this.__evaluateTarget(bi);if(bh){bj=bh;}
;}
;return bj;}
,__evaluateTarget:function(bm){var bk=null;var bl=null;if(bm&&bm.touches&&bm.touches.length!==0){bk=bm.touches[0].clientX;bl=bm.touches[0].clientY;}
...
;var br=br||bq.type;if(bs&&bs.nodeType&&this.__emitter){this.__emitter.emit(br,bq);}
;}
,dispose:function(){this._stopTouchObserver();this.__originalTarget=this.__target=this.__touchEventNames=this.__pointers=this.__emitter=this.__beginScalingDistance=this.__beginRotation=null;}
}});}
)();
...
;var w=v.getTarget();var x=qxWeb(w).getValue();if(!w.__oldInputValue||w.__oldInputValue!==x){w.__oldInputValue=x;v.type=v._type=h;w.$$emitter.emit(h,v);}
;}
