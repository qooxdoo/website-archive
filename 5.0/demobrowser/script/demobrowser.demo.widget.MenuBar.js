...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.MenuBar.fe0a59a618f7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,createCommands:function(){this._newCommand=new qx.ui.command.Command(v);this._newCommand.addListener(O,this.debugCommand);this._openCommand=new qx.ui.command.Command(I);this._openCommand.addListener(O,this.debugCommand);this._saveCommand=new qx.ui.command.Command(cQ);this._saveCommand.addListener(O,this.debugCommand);this._undoCommand=new qx.ui.command.Command(dg);this._undoCommand.addListener(O,this.debugCommand);this._redoCommand=new qx.ui.command.Command(cG);this._redoCommand.addListener(O,this.debugCommand);this._cutCommand=new qx.ui.command.Command(cW);this._cutCommand.addListener(O,this.debugCommand);this._copyCommand=new qx.ui.command.Command(cV);this._copyCommand.addListener(O,this.debugCommand);this._pasteCommand=new qx.ui.command.Command(cn);this._pasteCommand.addListener(O,this.debugCommand);this._pasteCommand.setEnabled(false);}
...
(function(){var b="-",c="PageUp",d="Escape",f="Boolean",g="+",h="qx.event.type.Data",j="_applyShortcut",k="PrintScreen",l="NumLock",m="short",n="keydown",o="5",p="8",q="execute",r="Meta",s="0",t="",u="2",v="Control",w="Shift",x="You can only specify one non modifier key!",y="3",z="Unidentified",A="/",B="Delete",C="String",D="changeEnabled",E="*",F="qx.bom.Shortcut",G="6",H="4",I="1",J="Alt",K="Not a valid key name for a shortcut: ",L="PageDown",M="Whitespaces are not allowed within shortcuts",N="_applyEnabled",O="keypress",P="7",Q="a",R="z",S="9";qx.Class.define(F,{extend:qx.core.Object,construct:function(T){qx.core.Object.call(this);this.__tQ={};this.__tR=null;if(T!=null){this.setShortcut(T);}
;this.initEnabled();}
,events:{"execute":h},properties:{enabled:{init:true,check:f,event:D,apply:N},shortcut:{check:C,apply:j,nullable:true},autoRepeat:{check:f,init:false}},members:{__tQ:t,__tR:t,execute:function(U){this.fireDataEvent(q,U);}
,__tS:function(event){if(this.getEnabled()&&this.__tT(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,__hJ:function(event){if(this.getEnabled()&&this.__tT(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,_applyEnabled:function(W,V){if(W){qx.event.Registration.addListener(document.documentElement,n,this.__tS,this);qx.event.Registration.addListener(document.documentElement,O,this.__hJ,this);}
else {qx.event.Registration.removeListener(document.documentElement,n,this.__tS,this);qx.event.Registration.removeListener(document.documentElement,O,this.__hJ,this);}
;}
,_applyShortcut:function(bb,Y){if(bb){if(bb.search(/[\s]+/)!=-1){var bc=M;this.error(bc);throw new Error(bc);}
;this.__tQ={"Control":false,"Shift":false,"Meta":false,"Alt":false};this.__tR=null;var X;var a=[];while(bb.length>0&&X!=-1){X=bb.search(/[-+]+/);a.push((bb.length==1||X==-1)?bb:bb.substring(0,X));bb=bb.substring(X+1);}
;var ba=a.length;for(var i=0;i<ba;i++ ){var bd=this.__tV(a[i]);switch(bd){case v:case w:case r:case J:this.__tQ[bd]=true;break;case z:var bc=K+a[i];this.error(bc);throw bc;default:if(this.__tR){var bc=x;this.error(bc);throw bc;}
;this.__tR=bd;};}
;}
;return true;}
,__tT:function(e){var be=this.__tR;if(!be){return false;}
;if((!this.__tQ.Shift&&e.isShiftPressed())||(this.__tQ.Shift&&!e.isShiftPressed())||(!this.__tQ.Control&&e.isCtrlPressed())||(this.__tQ.Control&&!e.isCtrlPressed())||(!this.__tQ.Meta&&e.isMetaPressed())||(this.__tQ.Meta&&!e.isMetaPressed())||(!this.__tQ.Alt&&e.isAltPressed())||(this.__tQ.Alt&&!e.isAltPressed())){return false;}
;if(be==e.getKeyIdentifier()){return true;}
;return false;}
,__tU:{esc:d,ctrl:v,print:k,del:B,pageup:c,pagedown:L,numlock:l,numpad_0:s,numpad_1:I,numpad_2:u,numpad_3:y,numpad_4:H,numpad_5:o,numpad_6:G,numpad_7:P,numpad_8:p,numpad_9:S,numpad_divide:A,numpad_multiply:E,numpad_minus:b,numpad_plus:g},__tV:function(bg){var bf=qx.event.util.Keyboard;var bh=z;if(bf.isValidKeyIdentifier(bg)){return bg;}
;if(bg.length==1&&bg>=Q&&bg<=R){return bg.toUpperCase();}
;bg=bg.toLowerCase();var bh=this.__tU[bg]||qx.lang.String.firstUp(bg);if(bf.isValidKeyIdentifier(bh)){return bh;}
else {return z;}
;}
,toString:function(){var bj=this.__tR;var bk=[];for(var bi in this.__tQ){if(this.__tQ[bi]){bk.push(qx.locale.Key.getKeyName(m,bi));}
;}
;if(bj){bk.push(qx.locale.Key.getKeyName(m,bj));}
;return bk.join(g);}
},destruct:function(){this.setEnabled(false);this.__tQ=this.__tR=null;}
});}
)();
...
;var P=K.getMarginLeft()+K.getMarginRight();P=Math.max(P,this.getSpacing());var J=K.getSizeHint().width+P;this.__qo(K);G-=J;if(E&&E.getVisibility()!=o){E.setVisibility(o);G+=L;var H=E.getMarginLeft()+E.getMarginRight();G+=Math.max(H,this.getSpacing());}
;}
while(G>F);}
else if(this.__ql.length>0){do {var Q=this.__ql[0];if(Q){var P=Q.getMarginLeft()+Q.getMarginRight();P=Math.max(P,this.getSpacing());if(Q.getContentElement().getDomElement()==null){Q.syncAppearance();Q.invalidateLayoutCache();}
;var I=Q.getSizeHint().width;var O=false;if(this.__ql.length==1&&L>0){var N=P-this.getSpacing();var M=G-L+I+N;O=F>M;}
;if(F>G+I+P||O){this.__qn(Q);G+=I;if(E&&this.__ql.length==0){E.setVisibility(y);}
;}
else {return;}
;}
;}
while(F>=G&&this.__ql.length>0);}
;}
,__qn:function(R){R.setVisibility(o);this.__ql.shift();this.fireDataEvent(m,R);}
,__qo:function(S){if(!S){return;}
;this.__ql.unshift(S);S.setVisibility(y);this.fireDataEvent(A,S);}
,_getNextToHide:function(){for(var i=this.__qm.length-1;i>=0;i-- ){var T=this.__qm[i];if(T&&T.getVisibility&&T.getVisibility()==o){return T;}
;}
...
;}
;}
,setRemovePriority:function(X,W,Y){if(!Y&&this.__qm[W]!=undefined){throw new Error(v);}
;this.__qm[W]=X;}
...
;for(var i=0;i<this.__ql.length;i++ ){this.__ql[i].setVisibility(o);}
;this.__ql=[];}
;}
...
;bf.setVisibility(y);}
;}
,__qp:false,_setAllowMenuOpenHover:function(bg){this.__qp=bg;}
,_isAllowMenuOpenHover:function(){return this.__qp;}
...
(function(){var a="blur",b="Enter",c="Escape",d="Space",f="__qa",g="Left",h="Down",j="Up",k="singleton",l="qx.debug",m="interval",n="roll",o="__oE",p="__pY",q="event.touch",r="Object is no menu: ",s="pointerdown",t="qx.ui.menu.Manager",u="keydown",v="keyup",w="keypress",x="contextmenu",y="Right";qx.Class.define(t,{type:k,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__oE=[];var z=document.body;var A=qx.event.Registration;A.addListener(window.document.documentElement,s,this._onPointerDown,this,true);A.addListener(z,n,this._onRoll,this,true);A.addListener(z,u,this._onKeyUpDown,this,true);A.addListener(z,v,this._onKeyUpDown,this,true);A.addListener(z,w,this._onKeyPress,this,true);if(!qx.core.Environment.get(q)){qx.bom.Element.addListener(window,a,this.hideAll,this);}
;this.__pY=new qx.event.Timer();this.__pY.addListener(m,this._onOpenInterval,this);this.__qa=new qx.event.Timer();this.__qa.addListener(m,this._onCloseInterval,this);}
,members:{__qb:null,__qc:null,__pY:null,__qa:null,__oE:null,_getChild:function(B,D,C,G){var F=B.getChildren();var length=F.length;var E;for(var i=D;i<length&&i>=0;i+=C){E=F[i];if(E.isEnabled()&&!E.isAnonymous()&&E.isVisible()){return E;}
;}
...
;H=H.getLayoutParent();}
;return false;}
,_isMenuOpener:function(J){var I=this.__oE;for(var i=0;i<I.length;i++ ){if(I[i].getOpener()===J){return true;}
;}
;return false;}
,_getMenuButton:function(K){while(K){if(K instanceof qx.ui.menu.AbstractButton){return K;}
;K=K.getLayoutParent();}
;return null;}
,add:function(L){if(qx.core.Environment.get(l)){if(!(L instanceof qx.ui.menu.Menu)){throw new Error(r+L);}
;}
;var M=this.__oE;M.push(L);L.setZIndex(1e6+M.length);}
,remove:function(N){if(qx.core.Environment.get(l)){if(!(N instanceof qx.ui.menu.Menu)){throw new Error(r+N);}
;}
;var O=this.__oE;if(O){qx.lang.Array.remove(O,N);}
;}
,hideAll:function(){var P=this.__oE;if(P){for(var i=P.length-1;i>=0;i-- ){P[i].exclude();}
;}
;}
,getActiveMenu:function(){var Q=this.__oE;return Q.length>0?Q[Q.length-1]:null;}
,scheduleOpen:function(R){this.cancelClose(R);if(R.isVisible()){if(this.__qb){this.cancelOpen(this.__qb);}
;}
else if(this.__qb!=R){this.__qb=R;this.__pY.restartWith(R.getOpenInterval());}
;}
,scheduleClose:function(S){this.cancelOpen(S);if(!S.isVisible()){if(this.__qc){this.cancelClose(this.__qc);}
;}
else if(this.__qc!=S){this.__qc=S;this.__qa.restartWith(S.getCloseInterval());}
;}
,cancelOpen:function(T){if(this.__qb==T){this.__pY.stop();this.__qb=null;}
;}
,cancelClose:function(U){if(this.__qc==U){this.__qa.stop();this.__qc=null;}
;}
,_onOpenInterval:function(e){this.__pY.stop();this.__qb.open();this.__qb=null;}
,_onCloseInterval:function(e){this.__qa.stop();this.__qc.exclude();this.__qc=null;}
,preventContextMenuOnce:function(){qx.event.Registration.addListener(document.body,x,this.__qd,this,true);}
,__qd:function(e){var V=e.getTarget();V=qx.ui.core.Widget.getWidgetByElement(V,true);if(this._isInMenu(V)){e.stopPropagation();e.preventDefault();}
;qx.event.Registration.removeListener(document.body,x,this.__qd,this,true);}
...
;if(this.__oE.length>0&&!this._isInMenu(W)){this.hideAll();}
;}
,__qe:{"Enter":1,"Space":1},__nz:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var X=this.getActiveMenu();if(!X){return;}
;var Y=e.getKeyIdentifier();if(this.__nz[Y]||(this.__qe[Y]&&X.getSelectedButton())){e.stopPropagation();}
;}
,_onKeyPress:function(e){var bc=this.getActiveMenu();if(!bc){return;}
;var ba=e.getKeyIdentifier();var be=this.__nz[ba];var bb=this.__qe[ba];if(be){switch(ba){case j:this._onKeyPressUp(bc);break;case h:this._onKeyPressDown(bc);break;case g:this._onKeyPressLeft(bc);break;case y:this._onKeyPressRight(bc);break;case c:this.hideAll();break;};e.stopPropagation();e.preventDefault();}
...
;}
,_onRoll:function(e){var bK=e.getTarget();bK=qx.ui.core.Widget.getWidgetByElement(bK,true);if(this.__oE.length>0&&!this._isInMenu(bK)&&!this._isMenuOpener(bK)&&!e.getMomentum()){this.hideAll();}
;}
},destruct:function(){var bM=qx.event.Registration;var bL=document.body;bM.removeListener(window.document.documentElement,s,this._onPointerDown,this,true);bM.removeListener(bL,u,this._onKeyUpDown,this,true);bM.removeListener(bL,v,this._onKeyUpDown,this,true);bM.removeListener(bL,w,this._onKeyPress,this,true);this._disposeObjects(p,f);this._disposeArray(o);}
});}
)();
...
;this.__qh();}
,__qh:function(){if(this.isVisible()){if(this.getBlockBackground()){var V=this.getZIndex();this._blocker.blockContent(V-1);}
;}
...
,__qi:function(){var bn=this._getMenuBounds();if(!bn){this.addListenerOnce(n,this.__qi,this);return;}
...
;this.__qg=bp;qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(bq){this.getChildControl(h);if(this.__qg){this.__qg.call(this);delete this.__qg;}
;}
...
;this.__qi();}
;}
,_onPointerOver:function(e){var bt=qx.ui.menu.Manager.getInstance();bt.cancelClose(this);var bu=e.getTarget();if(bu.isEnabled()&&bu instanceof qx.ui.menu.AbstractButton){this.setSelectedButton(bu);var bs=bu.getMenu&&bu.getMenu();if(bs){bs.setOpener(bu);bt.scheduleOpen(bs);this.__qf=bs;}
else {var bv=this.getOpenedButton();if(bv){bt.scheduleClose(bv.getMenu());}
;if(this.__qf){bt.cancelOpen(this.__qf);this.__qf=null;}
;}
;}
...
;if(this.__qf){bw.cancelOpen(this.__qf);}
;}
;}
...
(function(){var a="qx.ui.menu.Layout",b="Integer",c="_applyLayoutChange";qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:b,init:0,apply:c},spanColumn:{check:b,init:1,nullable:true,apply:c},iconColumnWidth:{check:b,init:0,themeable:true,apply:c},arrowColumnWidth:{check:b,init:0,themeable:true,apply:c}},members:{__qj:null,_computeSizeHint:function(){var q=this._getLayoutChildren();var o,g,k;var e=this.getSpanColumn();var j=this.__qj=[0,0,0,0];var n=this.getColumnSpacing();var m=0;var f=0;for(var i=0,l=q.length;i<l;i++ ){o=q[i];if(o.isAnonymous()){continue;}
...
,getColumnSizes:function(){return this.__qj||null;}
...
;var x=this.__qk(y[0]);var w=x.getColumnSizes();var s=x.getSpacingX();var z=qx.lang.Array.sum(w)+s*(w.length-1);if(z<o){w[1]+=o-z;}
...
;}
;}
,__qk:function(B){while(!(B instanceof qx.ui.menu.Menu)){B=B.getLayoutParent();}
;return B;}
...
,__qq:function(){var y=this.getChildrenContainer().getChildren();y=y.filter(function(z){return z.getVisibility()==k;}
