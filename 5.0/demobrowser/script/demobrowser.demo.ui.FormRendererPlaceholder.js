...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormRendererPlaceholder.5880b1dfc9da.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="change",b="qx.ui.form.Form",c="qx.event.type.Event",d="";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__vs=[];this._buttons=[];this._buttonOptions=[];this._validationManager=this._createValidationManager();this._resetter=this._createResetter();}
,events:{"change":c},members:{__vs:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(g,f,h,name,e,k){if(this.__vt()){this.__vs.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
;this.__vs[this._groupCounter].items.push(g);this.__vs[this._groupCounter].labels.push(f);this.__vs[this._groupCounter].options.push(k);if(name==null){name=f.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,d);}
;this.__vs[this._groupCounter].names.push(name);this._validationManager.add(g,h,e);this._resetter.add(g);this.fireEvent(a);}
,addGroupHeader:function(m,l){if(!this.__vt()){this._groupCounter++ ;}
;this.__vs.push({title:m,items:[],labels:[],names:[],options:[],headerOptions:l});this.fireEvent(a);}
...
,remove:function(p){for(var i=0;i<this.__vs.length;i++ ){var r=this.__vs[i];for(var j=0;j<r.items.length;j++ ){var q=r.items[j];if(q===p){r.items.splice(j,1);r.labels.splice(j,1);r.names.splice(j,1);r.options.splice(j,1);this._validationManager.remove(p);this._resetter.remove(p);this.fireEvent(a);return true;}
;}
;}
;return false;}
,removeGroupHeader:function(u){for(var i=0;i<this.__vs.length;i++ ){var t=this.__vs[i];if(t.title===u){var s;if(i==0){if(this.__vs.length==1){t.title=null;t.headerOptions={};this.fireEvent(a);return true;}
else {s=this.__vs[i+1];}
;}
else {s=this.__vs[i-1];}
;s.items=s.items.concat(t.items);s.labels=s.labels.concat(t.labels);s.names=s.names.concat(t.names);s.options=s.options.concat(t.options);this.__vs.splice(i,1);this._groupCounter-- ;this.fireEvent(a);return true;}
;}
;return false;}
...
;}
;return false;}
,getItems:function(){var x={};for(var i=0;i<this.__vs.length;i++ ){var y=this.__vs[i];for(var j=0;j<y.names.length;j++ ){var name=y.names[j];x[name]=y.items[j];}
;}
;return x;}
...
(function(){var a="Widgets supporting selection can only be validated ",b="Added widget not supported.",c="",d="in the form validator",f="mouse",g="changeValid",h="qx.event.type.Event",j="complete",k="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",l="qx.ui.form.validation.Manager",m="This field is required",n="String",o="qx.event.type.Data";qx.Class.define(l,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__vu=[];this.__vv={};this.setRequiredFieldMessage(qx.locale.Manager.tr(m));}
,events:{"changeValid":o,"complete":h},properties:{validator:{check:k,init:null,nullable:true},invalidMessage:{check:n,init:c},requiredFieldMessage:{check:n,init:c},context:{nullable:true}},members:{__vu:null,__vw:null,__vv:null,__vx:null,add:function(s,p,r){if(!this.__vC(s)){throw new Error(b);}
;if(this.__vD(s)&&!s.getValue){if(p!=null){throw new Error(a+d);}
;}
;var q={item:s,validator:p,valid:null,context:r};this.__vu.push(q);}
,remove:function(v){var t=this.__vu;for(var i=0,u=t.length;i<u;i++ ){if(v===t[i].item){t.splice(i,1);return v;}
;}
;return null;}
,getItems:function(){var w=[];for(var i=0;i<this.__vu.length;i++ ){w.push(this.__vu[i].item);}
;return w;}
,validate:function(){var B=true;this.__vx=true;var y=[];for(var i=0;i<this.__vu.length;i++ ){var z=this.__vu[i].item;var C=this.__vu[i].validator;y.push(z);if(C==null){var x=this.__vy(z);B=B&&x;this.__vx=x&&this.__vx;continue;}
;var x=this.__vz(this.__vu[i],z.getValue());B=x&&B;if(x!=null){this.__vx=x&&this.__vx;}
;}
;var A=this.__vA(y);if(qx.lang.Type.isBoolean(A)){this.__vx=A&&this.__vx;}
;B=A&&B;this._setValid(B);if(qx.lang.Object.isEmpty(this.__vv)){this.fireEvent(j);}
;return B;}
,__vy:function(E){if(E.getRequired()){if(this.__vD(E)){var D=!!E.getSelection()[0];}
...
;return true;}
,__vz:function(K,L){var M=K.item;var O=K.context;var N=K.validator;if(this.__vB(N)){this.__vv[M.toHashCode()]=null;N.validate(M,M.getValue(),this,O);return null;}
...
,__vA:function(Q){var R=this.getValidator();var S=this.getContext()||this;if(R==null){return true;}
;this.setInvalidMessage(c);if(this.__vB(R)){this.__vv[this.toHashCode()]=null;R.validateForm(Q,this,S);return null;}
...
;}
;return T;}
,__vB:function(V){var U=false;if(!qx.lang.Type.isFunction(V)){U=qx.Class.isSubClassOf(V.constructor,qx.ui.form.validation.AsyncValidator);}
;return U;}
,__vC:function(W){var X=W.constructor;return qx.Class.hasInterface(X,qx.ui.form.IForm);}
,__vD:function(Y){var ba=Y.constructor;return qx.Class.hasInterface(ba,qx.ui.core.ISingleSelection);}
,_setValid:function(bb){this._showToolTip(bb);var bc=this.__vw;this.__vw=bb;if(bc!=bb){this.fireDataEvent(g,bb,bc);}
;}
,_showToolTip:function(bg){if(!qx.ui.tooltip||!qx.ui.tooltip.Manager){return;}
;var be=qx.ui.tooltip.Manager.getInstance().getSharedErrorTooltip();if(!bg){var bd;for(var i=0;i<this.__vu.length;i++ ){var bf=this.__vu[i].item;if(!bf.isValid()){bd=bf;if(!(bf.getContentLocation)){return;}
...
;}
,getValid:function(){return this.__vw;}
,isValid:function(){return this.getValid();}
,getInvalidMessages:function(){var bh=[];for(var i=0;i<this.__vu.length;i++ ){var bi=this.__vu[i].item;if(!bi.getValid()){bh.push(bi.getInvalidMessage());}
;}
;if(this.getInvalidMessage()!=c){bh.push(this.getInvalidMessage());}
;return bh;}
,getInvalidFormItems:function(){var bj=[];for(var i=0;i<this.__vu.length;i++ ){var bk=this.__vu[i].item;if(!bk.getValid()){bj.push(bk);}
;}
;return bj;}
,reset:function(){for(var i=0;i<this.__vu.length;i++ ){var bl=this.__vu[i];bl.item.setValid(true);}
;this.__vw=null;this._showToolTip(true);}
,setItemValid:function(bn,bm){this.__vv[bn.toHashCode()]=bm;bn.setValid(bm);this.__vE();}
,setFormValid:function(bo){this.__vv[this.toHashCode()]=bo;this.__vE();}
,__vE:function(){var bq=this.__vx;for(var br in this.__vv){var bp=this.__vv[br];bq=bp&&bq;if(bp==null){return;}
;}
;this._setValid(bq);this.__vv={};this.fireEvent(j);}
},destruct:function(){this.__vu=null;}
});}
)();
(function(){var a="qx.ui.form.validation.AsyncValidator";qx.Class.define(a,{extend:qx.core.Object,construct:function(b){qx.core.Object.call(this);this.__vF=b;}
,members:{__vF:null,__vG:null,__gz:null,__vH:null,validate:function(c,e,d,f){this.__vH=false;this.__vG=c;this.__gz=d;this.__vF.call(f||this,this,e);}
,validateForm:function(g,h,i){this.__vH=true;this.__gz=h;this.__vF.call(i,g,this);}
,setValid:function(k,j){if(this.__vH){if(j!==undefined){this.__gz.setInvalidMessage(j);}
;this.__gz.setFormValid(k);}
else {if(j!==undefined){this.__vG.setInvalidMessage(j);}
;this.__gz.setItemValid(this.__vG,k);}
;}
},destruct:function(){this.__gz=this.__vG=null;}
});}
)();
...
(function(){var a="The given item has not been added.",b="qx.ui.form.Resetter",c=" not supported for reseting.",d="Item ";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__mh=[];}
,members:{__mh:null,add:function(e){if(this._supportsValue(e)){var f=e.getValue();}
else if(this.__vD(e)){var f=e.getSelection();}
else if(this.__vK(e)){var f=e.getSelection().concat();}
else {throw new Error(d+e+c);}
;this.__mh.push({item:e,init:f});}
,remove:function(g){for(var i=0;i<this.__mh.length;i++ ){var h=this.__mh[i];if(h.item===g){this.__mh.splice(i,1);return true;}
;}
;return false;}
,reset:function(){for(var i=0;i<this.__mh.length;i++ ){var j=this.__mh[i];this.__vI(j.item,j.init);}
;}
,resetItem:function(k){var l;for(var i=0;i<this.__mh.length;i++ ){var m=this.__mh[i];if(m.item===k){l=m.init;break;}
;}
;if(l===undefined){throw new Error(a);}
;this.__vI(k,l);}
,__vI:function(n,o){if(this._supportsValue(n)){n.setValue(o);}
else if(this.__vD(n)||this.__vK(n)){n.setSelection(o);}
;}
,redefine:function(){for(var i=0;i<this.__mh.length;i++ ){var p=this.__mh[i].item;this.__mh[i].init=this.__vJ(p);}
;}
,redefineItem:function(q){var r;for(var i=0;i<this.__mh.length;i++ ){if(this.__mh[i].item===q){r=this.__mh[i];break;}
;}
;if(r===undefined){throw new Error(a);}
;r.init=this.__vJ(r.item);}
,__vJ:function(s){if(this._supportsValue(s)){return s.getValue();}
else if(this.__vD(s)||this.__vK(s)){return s.getSelection();}
;}
,__vD:function(t){var u=t.constructor;return qx.Class.hasInterface(u,qx.ui.core.ISingleSelection);}
...
,properties:{wrap:{check:b,init:true,apply:f},appearance:{refine:true,init:j},singleStep:{check:i,init:20},minimalLineHeight:{check:i,apply:h,init:4},autoSize:{check:b,apply:w,init:false}},members:{__vL:null,__vM:null,__vN:null,setValue:function(M){M=qx.ui.form.AbstractField.prototype.setValue.call(this,M);this.__vO();return M;}
...
;}
,__vO:function(){if(this.isAutoSize()){var R=this.__vP();if(R&&this.getBounds()){this.__vN=this.__vN||this._getAreaHeight();var T=this._getScrolledAreaHeight();if(this.getMaxHeight()){var P=this.getInsets();var Q=-P.top+this.getMaxHeight()-P.bottom;if(T>Q){this.getContentElement().setStyle(I,k);}
else {this.getContentElement().setStyle(I,H);}
;}
;var S=Math.max(T,this.__vN);this._setAreaHeight(S);}
else {this.getContentElement().addListenerOnce(x,function(){this.__vO();}
,this);}
;}
;}
,_getAreaHeight:function(){return this.getInnerSize().height;}
,_setAreaHeight:function(U){if(this._getAreaHeight()!==U){this.__vM=U;qx.ui.core.queue.Layout.add(this);qx.ui.core.queue.Manager.flush();this.__vS();}
;}
,_getScrolledAreaHeight:function(){var W=this.__vP();var V=W.getDomElement();if(V){if(!V.parentNode){qx.html.Element.flush();return this._getScrolledAreaHeight();}
...
;this.__vR(W);if(qx.core.Environment.get(C)==B&&qx.core.Environment.get(s)==8){if(!V.scrollTop){qx.html.Element.flush();}
;}
;return V.scrollTop;}
;}
,__vP:function(){this.__vL=this.__vL||this.__vQ();return this.__vL;}
,__vQ:function(){var ba,Y,bb,X;ba=this.getContentElement();if(!ba.getDomElement()){return null;}
;bb=qx.bom.Element.clone(ba.getDomElement());X=new qx.html.Input(j);X.useElement(bb);Y=X;Y.setStyles({position:K,top:0,left:o,height:0,overflow:H},true);Y.removeAttribute(G);Y.removeAttribute(z);Y.setAttribute(y,g);Y.setValue(ba.getValue()||t);Y.insertBefore(ba);this.__vR(Y);return Y;}
,__vR:function(bc){bc=bc.getDomElement();if(bc){bc.scrollTop=10000;}
;}
...
,_applyAutoSize:function(bh,bg){if(qx.core.Environment.get(m)){this.__vT();}
;if(bh){this.__vO();this.addListener(a,this.__vO,this);if(!this.getBounds()){this.addListenerOnce(x,function(){this.getContentElement().setStyle(I,H);}
);}
else {this.getContentElement().setStyle(I,H);}
;}
else {this.removeListener(a,this.__vO);this.getContentElement().setStyle(I,k);}
;}
,_applyDimension:function(bi){qx.ui.form.AbstractField.prototype._applyDimension.call(this);if(qx.core.Environment.get(m)){this.__vT();}
;if(bi===this.getMaxHeight()){this.__vO();}
;}
,__vS:function(){var content=this.getContentElement();var bk=content.getDomElement();var bj=content.getStyle(u);content.setStyle(u,parseInt(bj,10)+1000+c,true);if(bk){qx.bom.element.Dimension.getWidth(bk);}
;content.setStyle(u,bj,true);}
,__vT:function(){if(this.isAutoSize()&&this.getHeight()){this.warn(n+J+v);}
;}
,_getContentHint:function(){var bl=qx.ui.form.AbstractField.prototype._getContentHint.call(this);bl.height=bl.height*this.getMinimalLineHeight();bl.width=this._getTextSize().width*20;if(this.isAutoSize()){bl.height=this.__vM||bl.height;}
;return bl;}
},destruct:function(){this.setAutoSize(false);if(this.__vL){this.__vL.dispose();}
;}
});}
)();
