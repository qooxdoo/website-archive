...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.DragDrop.25322990661f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;W=Y;}
;break;case j:W=U[0].getLabel();break;};if(X==o){for(var i=0,l=U.length;i<l;i++ ){this.remove(U[i]);}
;}
;e.addData(ba,W);}
...
;}
);var N=new qx.ui.basic.Label(m);I.add(N,{left:600,top:20});var G=this.__Kk=new qx.ui.form.List();G.setDraggable(true);G.setDroppable(true);G.setSelectionMode(u);I.add(G,{left:600,top:40});for(var i=0;i<20;i++ ){G.add(new qx.ui.form.ListItem(D+i,g));}
...
);G.addListener(b,function(e){var bf=e.getOriginalTarget();if(bf instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__Pv=bf;}
...
;}
);G.addListener(f,function(e){this.__Pw(e.getOriginalTarget(),e.getDragTarget());}
,this);H.addListener(f,function(e){this.__Pw(this.__Pv);}
,this);}
,__Pw:function(bi,bg){if(bi.classname!=n){return;}
;var bh=this.__Kk.getSortedSelection();if(bh.length===0){bh=[bg];}
else if(bh.indexOf(bg)==-1){bh=[bg];}
;for(var i=0,l=bh.length;i<l;i++ ){this.__Kk.addBefore(bh[i],bi);}
;}
