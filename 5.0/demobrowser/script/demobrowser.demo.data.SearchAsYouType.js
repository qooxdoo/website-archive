...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType.394aace13bf8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="_applySearchString",b="demobrowser.demo.data.filter.SearchAsYouTypeFilter",c="String",d="";qx.Class.define(b,{extend:qx.core.Object,construct:function(e){qx.core.Object.call(this);this.__tM=e;this.filter=qx.lang.Function.bind(function(f){return f.search(this.getSearchString())!=-1;}
,this);this.__uA=null;}
,properties:{searchString:{check:c,apply:a,init:d}},members:{__uA:null,__tM:null,_applySearchString:function(h,g){var i=qx.util.TimerManager.getInstance();if(this.__uA!=null){i.stop(this.__uA);this.__uA=null;}
;this.__uA=i.start(function(){this.__tM.update();this.__uA=null;}
,0,this,null,200);}
,filter:null},destruct:function(){this.__tM=null;}
});}
)();
(function(){var a="qx.util.TimerManager",b="interval",c="singleton";qx.Class.define(a,{extend:qx.core.Object,type:c,statics:{__EW:[],__EX:{},__uA:0},members:{__EY:false,start:function(g,f,h,d,j){if(!j){j=f||0;}
;var e=(new Date()).getTime()+j;this.self(arguments).__EX[ ++this.self(arguments).__uA]={callback:g,userData:d||null,expireAt:e,recurTime:f,context:h||this};this.__Fa(e,this.self(arguments).__uA);return this.self(arguments).__uA;}
,stop:function(k){var l=this.self(arguments).__EW;var length=l.length;for(var i=0;i<length;i++ ){if(l[i]==k){l.splice(i,1);break;}
;}
;delete this.self(arguments).__EX[k];if(l.length==0&&this.__EY){qx.event.Idle.getInstance().removeListener(b,this.__Fb,this);this.__EY=false;}
;}
,__Fa:function(m,p){var o=this.self(arguments).__EW;var n=this.self(arguments).__EX;var length=o.length;for(var i=0;i<length;i++ ){if(n[o[i]].expireAt>m){o.splice(i,0,p);break;}
;}
;if(o.length==length){o.push(p);}
;if(!this.__EY){qx.event.Idle.getInstance().addListener(b,this.__Fb,this);this.__EY=true;}
;}
,__Fb:function(){var r=(new Date()).getTime();var t=this.self(arguments).__EW;var s=this.self(arguments).__EX;while(t.length>0&&s[t[0]].expireAt<=r){var v=t.shift();var q=s[v];q.callback.call(q.context,q.userData,v);if(q.recurTime&&s[v]){var u=(new Date()).getTime();q.expireAt=u+q.recurTime;this.__Fa(q.expireAt,v);}
else {delete s[v];}
;}
;if(t.length==0&&this.__EY){qx.event.Idle.getInstance().removeListener(b,this.__Fb,this);this.__EY=false;}
;}
}});}
)();
