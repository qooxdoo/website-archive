...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.6d61bf15e88b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="TERMINATED",c="__Hm",d=" => ",f="#autoActionsBeforeOnexit",g="__Hn",h=": Could not find friendly name for '",j=": event '",k=" blocked.  Re-queuing.",l="string",m="qx.event.type.Data",n="'",o="Attempt to transition to nonexistent state ",p="Saved-state stack is empty",q="Internal error: invalid nextState",r=" not handled.  Ignoring.",s="State ",u="#autoActionsBeforeOntransition",v=" returned a value other than ",w="#",x=" already exists",y="Attempt to transition to POP_STATE_STACK ",z=":",A="Transition ",B=" does not exist",C="' on '",D="qx.util.fsm.FiniteStateMachine",E="String",F="terminated",G="#autoActionsAfterOntransition",H="",I="#actionsAfterOnentry",J="Saved-state stack is full",K="#autoActionsBeforeOnentry",L=": no transition found.  No state change.",M="#autoActionsAfterOnentry",N="#actionsBeforeOnentry",O="', because the finite state machine is not running.",P=": Pushed event: ",Q=": Cannot listen to event '",R="Machine started with no available states",S=": Process event: ",T="#entry",U="#ontransition",V=" on ",W="Invalid state: not an instance of ",X="Number",Y="#exit",bp="true, false, or null.",bq="Explicit transition ",br="  ",bl="qx.util.fsm.State",bm="while state stack is empty.",bn=".  Ignoring.",bo="#autoActionsAfterOnexit",bs=": Event '",bt="object",bu=" not handled for target ",bv=": Queued event: ";qx.Class.define(D,{extend:qx.core.Object,construct:function(bw){qx.core.Object.call(this);this.setName(bw);this.__jz={};this.__Hk=null;this.__Hl=[];this.__Hm=[];this.__Hn=[];this.__Ho={};this.__Hp={};this.__Hq={};this.__Hr={};this.__Hs={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},events:{"terminated":m},properties:{name:{check:E,nullable:true},state:{check:E,nullable:true},previousState:{check:E,nullable:true},nextState:{check:E,nullable:true},maxSavedStates:{check:X,init:2},debugFlags:{check:X,init:7}},members:{__jz:null,__Hk:null,__Hm:null,__Hn:null,__Hl:null,__Ho:null,__Hp:null,__Hq:null,__Hr:null,__Hs:null,__Ht:false,__Hu:true,isTerminated:function(){return this.__Hu;}
,addState:function(bx){if(!bx instanceof qx.util.fsm.State){throw new Error(W+bl);}
;var by=bx.getName();if(by in this.__jz){throw new Error(s+by+x);}
;if(this.__Hk==null){this.__Hk=by;}
;this.__jz[by]=bx;}
...
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__Hp[bG]=bH;this.__Hq[bH]=bG;this.__Ho[bG]=bD;if(!bE){return;}
;if(typeof (bE)==l){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__Hr[bF]){this.__Hr[bF]={};}
;this.__Hr[bF][bG]=true;if(!this.__Hs[bG]){this.__Hs[bG]=[];}
;this.__Hs[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__Hp[bL];if(this.__Hs[bL]){for(var i=0;i<this.__Hs[bL].length;i++ ){bJ=this.__Hs[bL][i];delete this.__Hr[bJ][bL];bM=true;for(bK in this.__Hr[bJ]){bM=false;break;}
;if(bM){delete this.__Hr[bJ];}
;}
;delete this.__Hs[bL];}
;delete this.__Hq[bI];delete this.__Hp[bL];delete this.__Ho[bL];}
,getObject:function(bN){return this.__Ho[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__Hq[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__Hr[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__Hp){var bT=this.__Hp[bR];var bS=this.getObject(bR);this.debug(bR+d+bT);this.debug(br+bT+d+this.__Hq[bT]);this.debug(br+bR+d+this.getObject(bR));this.debug(br+this.getObject(bR)+d+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__jz,"startState":this.__Hk,"eventQueue":this.__Hm,"blockedEvents":this.__Hn,"savedStates":this.__Hl,"friendlyToObject":this.__Ho,"friendlyToHash":this.__Hp,"hashToFriendly":this.__Hq,"groupToFriendly":this.__Hr,"friendlyToGroups":this.__Hs});}
,start:function(){this.__Hu=false;var bV=this.__Hk;if(bV==null){throw new Error(R);}
;this.setState(bV);this.setPreviousState(null);this.setNextState(null);var bU=(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL);if(bU){this.debug(this.getName()+w+bV+N);}
;this.__jz[bV].getAutoActionsBeforeOnentry()(this);if(bU){this.debug(this.getName()+w+bV+T);}
;this.__jz[bV].getOnentry()(this,null);if(bU){this.debug(this.getName()+w+bV+I);}
;this.__jz[bV].getAutoActionsAfterOnentry()(this);}
,pushState:function(bW){if(this.__Hl.length>=this.getMaxSavedStates()){throw new Error(J);}
;if(bW===true){this.__Hl.push(this.getState());}
else if(bW){this.__Hl.push(bW);}
else {this.__Hl.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Hl.length==0){throw new Error(p);}
;return this.__Hl.pop();}
,postponeEvent:function(event){this.__Hn.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__Hm.push(event);}
else {this.__Hm.unshift(event);}
...
;}
;}
,eventListener:function(event){if(this.__Hu){this.debug(this.getName()+Q+event.getType()+O);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__Hv();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__Hu){this.debug(this.getName()+Q+cb+O);return;}
...
,this,cg||20);}
,__Hv:function(){if(this.__Ht){return;}
;this.__Ht=true;while(this.__Hm.length>0){var event=this.__Hm.pop();var ch=this.__Hw(event);if(ch){event.dispose();}
;}
;this.__Ht=false;}
,__Hw:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+S+event.getType()+(ct?V+ct:H));}
;ck=this.getState();cn=this.__jz[ck];cq=cn.transitions;e=cn.getEvents()[event.getType()];if(!e){if(cu){this.debug(this.getName()+bs+event.getType()+n+r);}
;return true;}
;if(typeof (e)==bt){if(!ct){if(co){this.debug(this.getName()+h+event.getType()+C+event.getTarget()+n);}
;return true;}
;cp=e[ct];if(!cp){if(cu){this.debug(this.getName()+bs+event.getType()+n+bu+ct+bn);}
;return true;}
;}
else {cp=e;}
;switch(cp){case qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE:break;case qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED:if(cu){this.debug(this.getName()+bs+event.getType()+n+k);}
;this.__Hn.unshift(event);return false;default:if(typeof (cp)==l){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bq+cp+B);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(A+ck+z+t+v+bp);};cl=cm.getNextState();if(typeof (cl)==l){if(!cl in this.__jz){throw new Error(o+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Hl.length==0){throw new Error(y+bm);}
;cl=this.__Hl.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__Hu=true;this.setNextState(null);break;default:throw new Error(q);};}
;if(cr){this.debug(this.getName()+w+ck+w+t+u);}
;cm.getAutoActionsBeforeOntransition()(this);if(cr){this.debug(this.getName()+w+ck+w+t+U);}
;cm.getOntransition()(this,event);if(cr){this.debug(this.getName()+w+ck+w+t+G);}
;cm.getAutoActionsAfterOntransition()(this);if(cr){this.debug(this.getName()+w+ck+f);}
;cn.getAutoActionsBeforeOnexit()(this);if(cr){this.debug(this.getName()+w+ck+Y);}
;cn.getOnexit()(this,event);if(cr){this.debug(this.getName()+w+ck+bo);}
;cn.getAutoActionsAfterOnexit()(this);if(cn._bNeedDispose){cn.dispose();}
;if(this.__Hu){if(cr){this.debug(this.getName()+w+b);}
;this.fireDataEvent(F,ck);return true;}
;cn=this.__jz[this.getNextState()];this.setPreviousState(ck);this.setState(this.getNextState());this.setNextState(null);cj=ck;ck=cl;cl=undefined;if(cr){this.debug(this.getName()+w+ck+K);}
;cn.getAutoActionsBeforeOnentry()(this);if(cr){this.debug(this.getName()+w+ck+T);}
;cn.getOnentry()(this,event);if(cr){this.debug(this.getName()+w+ck+M);}
;cn.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__Hn.length;i++ ){e=this.__Hn.pop();this.__Hm.unshift(e);}
;if(cs){this.debug(this.getName()+w+cj+d+this.getName()+w+ck);}
;return true;}
;if(cs){this.debug(this.getName()+w+ck+j+event.getType()+n+L);}
;return true;}
},destruct:function(){this._disposeArray(c);this._disposeArray(g);this.__Hl=this.__jz=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="parameters",k=")",l=" fsm.getObject('",m="groups",n="Invalid function parameters: ",o="Invalid onexit type: ",p="  for (var i = 0; i < groupObjects.length; i++)",q="__Hh",r="context",s="string",t="autoActionsAfterOnentry",u="Invalid numeric value in events object: ",v='"',w="Invalid state name",x=" value: ",y="');",z="State ",A="autoActionsBeforeOnexit",B="autoActionsBeforeOnentry",C="__He",D="}",E="): ",F="events must be an object",G="onexit",H="{",I="Invalid function request type: ",J="number",K="__Hc",L=": ",M="expected array, found ",N="Invalid friendly name in 'objects' list: ",O="autoActionsAfterOnexit",P="  fsm.debug(ex);",Q="(",R="Invalid onentry type: ",S="    fsm.getObject(objName).",T=")(fsm)",U="events",V="undefined",W="Invalid ",X="__Hg",Y="try",ba="    var objName = groupObjects[i];",bb="Invalid value in events object: ",bc="').",bd="objects",be="expected object, found ",bf="__Hi",bg="qx.util.fsm.Transition",bh="The events object must be provided in new state info",bi="catch(ex)",bj="  {",bk="    fsm.getGroupObjects('",bl="Invalid value in events object ",bm="Invalid transition: not an instance of ",bn="  }",bo="__Hf",bp="__Hd",bq="Invalid 'objects' list: expected array, got ",br="Invalid numeric value in events object ",bs="qx.util.fsm.State",bt="onentry",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="__Hb",bB="object";qx.Class.define(bs,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(r,bE);for(var bC in bF){switch(bC){case bt:this.setOnentry(this.__Hj(bF[bC],bE));break;case G:this.setOnexit(this.__Hj(bF[bC],bE));break;case B:this.setAutoActionsBeforeOnentry(bF[bC]);break;case t:this.setAutoActionsAfterOnentry(bF[bC]);break;case A:this.setAutoActionsBeforeOnexit(bF[bC]);break;case O:this.setAutoActionsAfterOnexit(bF[bC]);break;case U:this.setEvents(bF[bC]);break;case r:break;default:this.setUserData(bC,bF[bC]);this.debug(z+bD+L+bz+bC);break;};}
;if(!this.getEvents()){throw new Error(bh);}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(actionType,value,context){if(typeof (value)!=bB){throw new Error(W+actionType+x+typeof (value));}
;var funcFragment;var func=Y+H;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(I+M+typeof (functionRequest));}
;for(var i=0;i<functionRequest.length;i++ ){objectAndGroupList=functionRequest[i];if(typeof (objectAndGroupList)!=bB){throw new Error(bv+be+typeof (functionRequest[param]));}
;var params=objectAndGroupList[h];if(!params){params=[];}
else {if(!params instanceof Array){throw new Error(n+M+typeof (params));}
;}
;funcFragment=f+Q;for(var j=0;j<params.length;j++ ){if(j!=0){funcFragment+=by;}
;if(typeof (params[j])==c){funcFragment+=Q+params[j]+T;}
else if(typeof (params[j])==s){funcFragment+=v+params[j]+v;}
else {funcFragment+=params[j];}
;}
;funcFragment+=k;var a=objectAndGroupList[bd];if(!a){a=[];}
else if(!a instanceof Array){throw new Error(bq+typeof (a));}
;for(var j=0;j<a.length;j++ ){if(typeof (a[j])!=s){throw new Error(N+a[j]);}
;func+=l+a[j]+bc+funcFragment+bu;}
;var g=objectAndGroupList[m];if(g){if(!g instanceof Array){throw new Error(bw+typeof (g));}
;for(j=0;j<g.length;j++ ){func+=d+bk+g[j]+y+p+bj+ba+S+funcFragment+bu+bn;}
;}
;}
;}
;func+=D+bi+H+P+D;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:bA,nullable:true},onentry:{transform:K,nullable:true,init:function(bG,event){}
},onexit:{transform:bp,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:bo,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:X,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:q,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:bf,nullable:true,init:function(bL,event){}
},events:{transform:C,nullable:true}},members:{__Hb:function(bM){if(typeof (bM)!=s||bM.length<1){throw new Error(w);}
;return bM;}
,__Hc:function(bN){switch(typeof (bN)){case V:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(r));default:throw new Error(R+typeof (bN));};}
,__Hd:function(bP){switch(typeof (bP)){case V:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(r));default:throw new Error(o+typeof (bP));};}
,__He:function(bT){if(typeof (bT)!=bB){throw new Error(F);}
;for(var e in bT){var bR=bT[e];if(typeof (bR)==J&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(u+e+L+bR);}
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==J&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(br+Q+e+E+bS+L+bR[bS]);}
else if(typeof (bR[bS])!=s&&typeof (bR[bS])!=J){throw new Error(bl+Q+e+E+bS+L+bR[bS]);}
;}
;}
else if(typeof (bR)!=s&&typeof (bR)!=J){throw new Error(bb+e+L+bT[e]);}
;}
;return bT;}
,__Hf:function(bU){return qx.util.fsm.State._commonTransformAutoActions(B,bU,this.getUserData(r));}
,__Hg:function(bV){return qx.util.fsm.State._commonTransformAutoActions(t,bV,this.getUserData(r));}
,__Hh:function(bW){return qx.util.fsm.State._commonTransformAutoActions(A,bW,this.getUserData(r));}
,__Hi:function(bX){return qx.util.fsm.State._commonTransformAutoActions(O,bX,this.getUserData(r));}
,__Hj:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bm+bg);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="or one of the Fsm.StateChange constants",d="autoActionsAfterOntransition",e="context",g="string",h="__HB",i="predicate",j="__Hx",k="nextState must be an explicit state name, ",l="__Hz",m="Transition ",n=": ",o="String",p="number",q="Invalid transition nextState type: ",r="Invalid transition nextState value: ",s="Adding user-provided field to transition: ",t="__HA",u="ontransition",v="Invalid transition predicate type: ",w="boolean",x="nextState",y="__Hy",z="Invalid ontransition type: ",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(e,E);for(var C in D){switch(C){case i:this.setPredicate(this.__Hj(D[C],E));break;case x:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__Hj(D[C],E));break;case d:this.setAutoActionsAfterOntransition(this.__Hj(D[C],E));break;case u:this.setOntransition(this.__Hj(D[C],E));break;case e:break;default:this.setUserData(C,D[C]);this.debug(m+F+n+s+C);break;};}
;}
,properties:{name:{check:o,nullable:true},predicate:{init:function(G,event){return true;}
,transform:j},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:y},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:l},autoActionsAfterOntransition:{init:function(I,event){}
,transform:t},ontransition:{init:function(J,event){}
,transform:h}},members:{__Hx:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
;case w:return function(M,event){return K;}
;case a:return qx.lang.Function.bind(K,this.getUserData(e));default:throw new Error(v+typeof (K));};}
,__Hy:function(N){switch(typeof (N)){case g:return N;case p:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(r+N+n+k+c);};break;default:throw new Error(q+typeof (N));};}
,__Hz:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(e));}
,__HA:function(P){return qx.util.fsm.State._commonTransformAutoActions(d,P,this.getUserData(e));}
,__HB:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(e));default:throw new Error(z+typeof (Q));};}
,__Hj:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
