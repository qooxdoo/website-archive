...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Selection.8a906e880f32.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="one",b="manager",c="default",d="Additive Selection",f="changeValue",g="selected",h=" / ",i="Cell Line Selection",j="addaptive",k="value",l="Options",m="Row Selection",n="changeSelection",o="Column Selection",p="table-row-background-selected",q="Enable drag selection",r="multi",s="demobrowser.demo.virtual.Selection",t="Selection Mode",u="single",v="#EEE",w="Single Selection",x="additive",y="horizontal",z="bold",A="Cell Rectangle Selection",B="Enable quick selection",C="table-scroller-header",D="One Selection",E="Quick selection is only available for the modes multi or additive",F="white",G="Drag selection is only available for the modes multi or additive",H="Multi Selection";qx.Class.define(s,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var N=qx.theme.manager.Font.getInstance().resolve(c).getStyles();this._fontCss=qx.bom.element.Style.compile(N);var V=new qx.ui.layout.Grid(5,0);V.setRowFlex(2,1);V.setColumnFlex(0,1);V.setColumnFlex(1,1);V.setColumnFlex(2,1);V.setColumnFlex(3,1);var R=[];var J=new qx.ui.container.Composite(V);this.getRoot().add(J,{edge:5});J.add(new qx.ui.basic.Label(m).set({font:z,decorator:C,padding:3,allowGrowX:true}),{row:1,column:0});var L=this.createRowSelectionScroller();R.push(L.getUserData(b));J.add(L,{row:2,column:0});J.add(new qx.ui.basic.Label(o).set({font:z,decorator:C,padding:3,allowGrowX:true}),{row:1,column:1});var L=this.createColumnSelectionScroller();R.push(L.getUserData(b));J.add(L,{row:2,column:1});J.add(new qx.ui.basic.Label(A).set({font:z,decorator:C,padding:3,allowGrowX:true}),{row:1,column:2});var L=this.createCellRectangleSelectionScroller();R.push(L.getUserData(b));J.add(L,{row:2,column:2});J.add(new qx.ui.basic.Label(i).set({font:z,decorator:C,padding:3,allowGrowX:true}),{row:1,column:3});var L=this.createCellLineSelectionScroller();R.push(L.getUserData(b));J.add(L,{row:2,column:3});var Q=new qx.ui.layout.Grid(20,4);var O=new qx.ui.container.Composite(Q).set({padding:15,backgroundColor:F});J.add(O,{row:0,column:0,colSpan:4});O.add(new qx.ui.basic.Label(t).set({font:z,padding:3}),{row:0,column:0});var M=new qx.ui.form.RadioButton(w);M.setUserData(k,u);O.add(M,{row:1,column:0});var T=new qx.ui.form.RadioButton(H);T.setUserData(k,r);T.setValue(true);O.add(T,{row:2,column:0});var K=new qx.ui.form.RadioButton(d);K.setUserData(k,x);O.add(K,{row:3,column:0});var I=new qx.ui.form.RadioButton(D);I.setUserData(k,a);O.add(I,{row:4,column:0});var P=new qx.ui.form.RadioGroup(M,T,K,I);P.addListener(n,function(e){var W=e.getData()[0].getUserData(k);R.forEach(function(X){X.setMode(W);}
);if(W==u||W==a){U.setEnabled(false);S.setEnabled(true);}
else if(W==r||W==j){U.setEnabled(true);S.setEnabled(false);}
;}
);O.add(new qx.ui.basic.Label(l).set({font:z,padding:3}),{row:0,column:1});var U=new qx.ui.form.CheckBox(q);O.add(U,{row:1,column:1});U.addListener(f,function(e){if(e.getData()){var Y=R[0].getMode();if(Y==u||Y==a){this.debug(G);}
;}
;R.forEach(function(ba){ba.setDrag(e.getData());}
);}
);var S=new qx.ui.form.CheckBox(B).set({enabled:false});O.add(S,{row:2,column:1});S.addListener(f,function(e){if(e.getData()){var bb=R[0].getMode();if(bb==r||bb==x){this.debug(E);}
;}
;R.forEach(function(bc){bc.setQuick(e.getData());}
);}
);}
,createRowSelectionScroller:function(){var bg=new qx.ui.virtual.core.Scroller(1000,100,20,100);var be=new qx.ui.virtual.layer.Row(F,v);bg.getPane().addLayer(be);bg.getPane().addLayer(new qx.ui.virtual.layer.GridLines(y));var bh=new qx.ui.virtual.cell.Cell();var bd=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bk,bi){var bj={};if(bf.isItemSelected(bk)){bj.selected=true;}
;return bh.getCellProperties(bk+h+bi,bj);}
});bg.getPane().addLayer(bd);var bf=new qx.ui.virtual.selection.Row(bg.getPane(),{styleSelectable:function(bl,bm,bn){if(bm!==g){return;}
;if(bn){be.setBackground(bl,g);}
else {be.setBackground(bl,null);}
;bd.updateLayerData();}
});bf.attachPointerEvents(bg.getPane());bf.attachKeyEvents(bg);bf.set({mode:r});bf.addItem(0);bg.setUserData(b,bf);return bg;}
,createColumnSelectionScroller:function(){var br=new qx.ui.virtual.core.Scroller(1000,100,20,100);br.getPane().addLayer(new qx.ui.virtual.layer.Row(F,v));var bo=new qx.ui.virtual.layer.Column();br.getPane().addLayer(bo);br.getPane().addLayer(new qx.ui.virtual.layer.GridLines(y));var bs=new qx.ui.virtual.cell.Cell();var bp=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bv,bt){var bu={};if(bq.isItemSelected(bt)){bu.selected=true;}
;return bs.getCellProperties(bv+h+bt,bu);}
});br.getPane().addLayer(bp);var bq=new qx.ui.virtual.selection.Column(br.getPane(),{styleSelectable:function(bw,bx,by){if(bx!==g){return;}
;if(by){bo.setBackground(bw,g);}
else {bo.setBackground(bw,null);}
;bp.updateLayerData();}
});bq.attachPointerEvents(br.getPane());bq.attachKeyEvents(br);bq.set({mode:r});bq.addItem(0);br.setUserData(b,bq);return br;}
,createCellRectangleSelectionScroller:function(){var bC=new qx.ui.virtual.core.Scroller(1000,100,20,100);bC.getPane().addLayer(new qx.ui.virtual.layer.Row(F,v));var bD=new qx.ui.virtual.cell.Cell();var bA=new qx.ui.virtual.cell.Cell().set({backgroundColor:p});var bz=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bG,bE){var bF={};if(bB.isItemSelected({row:bG,column:bE})){bF.selected=true;}
;if(bF.selected){return bA.getCellProperties(bG+h+bE,bF);}
else {bD.resetBackgroundColor();return bD.getCellProperties(bG+h+bE,bF);}
;}
});bC.getPane().addLayer(bz);bC.getPane().addLayer(new qx.ui.virtual.layer.GridLines(y));var bB=new qx.ui.virtual.selection.CellRectangle(bC.getPane(),{styleSelectable:function(bH,bI,bJ){bz.updateLayerData();}
});bB.attachPointerEvents(bC.getPane());bB.attachKeyEvents(bC);bB.set({mode:r});bB.addItem({row:0,column:0});bC.setUserData(b,bB);return bC;}
,createCellLineSelectionScroller:function(){var bN=new qx.ui.virtual.core.Scroller(1000,100,20,100);bN.getPane().addLayer(new qx.ui.virtual.layer.Row(F,v));var bO=new qx.ui.virtual.cell.Cell();var bL=new qx.ui.virtual.cell.Cell().set({backgroundColor:p});var bK=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bR,bP){var bQ={};if(bM.isItemSelected({row:bR,column:bP})){bQ.selected=true;}
;if(bQ.selected){return bL.getCellProperties(bR+h+bP,bQ);}
else {bO.resetBackgroundColor();return bO.getCellProperties(bR+h+bP,bQ);}
;}
});bN.getPane().addLayer(bK);bN.getPane().addLayer(new qx.ui.virtual.layer.GridLines(y));var bM=new qx.ui.virtual.selection.CellLines(bN.getPane(),{styleSelectable:function(bS,bT,bU){bK.updateLayerData();}
});bM.attachPointerEvents(bN.getPane());bM.attachKeyEvents(bN);bM.set({mode:r});bM.addItem({row:0,column:0});bN.setUserData(b,bM);return bN;}
}});}
)();
...
,getCssClasses:function(br,bp){var bq=this.__dP.getCssClass(this.__BY)||t;return v+bq;}
,__Cc:function(bs){if(!bs){bs={};}
;var bu=this.getAppearance();var bt=bu+a+Object.keys(bs).sort().join(H);if(this.__BY==bt){return;}
;this.__BY=bt;var bv=this.__jz[this.__BY];if(!bv){this.__Cd();this.__Ce(bs);this.__Cf(bs);this.__Cg();this.__jz[this.__BY]=1;}
;this.__Ch();}
...
;}
;this.__Ca=false;}
,__Cf:function(){var bD=qx.lang.Object.getValues(this.__BT).join(I);this.__dP.computeClassForStyles(this.__BY,bD);}
...
},destruct:function(){this.__dP=this.__BS=this.__BT=this.__BU=this.__BV=this.__jz=this.__BW=this.__BX=null;}
});}
)();
...
(function(){var a="<div style='",b="width:",c="background-color:",d="block",e="",f="position: absolute;",g="top: 0;",h="height:",i="column-layer",j="'>",k="qx.ui.virtual.layer.Column",l=";",m="px;",n="</div>",o="none",p="left:";qx.Class.define(k,{extend:qx.ui.virtual.layer.AbstractBackground,properties:{appearance:{refine:true,init:i}},members:{_fullUpdate:function(y,u,t,w){var B=[];var z=qx.lang.Array.sum(t);var E=0;var v=u;var s=0;for(var x=0;x<w.length;x++ ){var A=this.getColor(v);var C=A?c+A+l:e;var D=this.getBackground(v);var q=D?qx.bom.element.Style.compile(D.getStyles()):e;B.push(a,f,g,p,E,m,b,w[x],m,h,z,m,C,q,j,n);s++ ;E+=w[x];v+=1;}
;var r=this.getContentElement().getDomElement();r.style.display=o;r.innerHTML=B.join(e);r.style.display=d;this._height=z;}
,updateLayerWindow:function(H,I,G,F){if(I!==this.getFirstColumn()||F.length!==this.getColumnSizes().length||this._height<qx.lang.Array.sum(G)){this._fullUpdate(H,I,G,F);}
;}
,setColor:function(K,J){qx.ui.virtual.layer.AbstractBackground.prototype.setColor.call(this,K,J);var M=this.getFirstColumn();var L=M+this.getColumnSizes().length-1;if(K>=M&&K<=L){this.updateLayerData();}
;}
}});}
)();
(function(){var a="right",b="qx.ui.virtual.selection.Column",c="left";qx.Class.define(b,{extend:qx.ui.virtual.selection.Row,members:{_getItemCount:function(){return this._pane.getColumnConfig().getItemCount();}
,_getSelectableFromPointerEvent:function(event){var d=this._pane.getCellAtPosition(event.getDocumentLeft(),event.getDocumentTop());if(!d){return null;}
;return this._isSelectable(d.column)?d.column:null;}
,_getRelatedSelectable:function(f,h){if(h==c){var g=f-1;var e=0;var j=-1;}
else if(h==a){var g=f+1;var e=this._getItemCount()-1;var j=1;}
else {return null;}
;for(var i=g;i!==e+j;i+=j){if(this._isSelectable(i)){return i;}
;}
;return null;}
,_scrollItemIntoView:function(k){if(this._autoScrollIntoView){this._pane.scrollColumnIntoView(k);}
;}
,_getSelectableLocationX:function(m){var l=this._pane.getColumnConfig();var n=l.getItemPosition(m);var o=n+l.getItemSize(m)-1;return {left:n,right:o};}
...
(function(){var a="qx.ui.virtual.selection.CellLines";qx.Class.define(a,{extend:qx.ui.virtual.selection.CellRectangle,members:{_getSelectableRange:function(e,g){var h=[];var b=this._pane.getColumnConfig().getItemCount();if(e.row<g.row||e.row==g.row&&e.column<g.column){var d=e.row*b+e.column;var c=g.row*b+g.column;}
else {var d=g.row*b+g.column;var c=e.row*b+e.column;}
;for(var i=d;i<=c;i++ ){var f={row:Math.floor(i/b),column:i%b};if(this._isSelectable(f)){h.push(f);}
;}
;return h;}
}});}
)();
