...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ListControllerWith3Widgets.0eff02cd2b7d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,properties:{appearance:{refine:true,init:t}},members:{__wo:null,_createChildControlImpl:function(B,A){var z;switch(B){case s:z=new qx.ui.core.Spacer();this._add(z,{flex:1});break;case y:z=new qx.ui.basic.Atom(v);z.setCenter(false);z.setAnonymous(true);this._add(z,{flex:1});break;case n:z=new qx.ui.basic.Image();z.setAnonymous(true);this._add(z);break;};return z||qx.ui.form.AbstractSelectBox.prototype._createChildControlImpl.call(this,B);}
...
,__lR:function(e){var D=e.getData()[0];var C=this.getChildControl(l);if(C.getSelection()[0]!=D){if(D){C.setSelection([D]);}
else {C.resetSelection();}
;}
;this.__xO();this.__xP();}
,__xO:function(){var F=this.getChildControl(l).getSelection()[0];var E=this.getChildControl(y);var G=F?F.getIcon():q;G==null?E.resetIcon():E.setIcon(G);}
,__xP:function(){var J=this.getChildControl(l).getSelection()[0];var H=this.getChildControl(y);var I=J?J.getLabel():q;var K=this.getFormat();if(K!=null){I=K.call(this,J);}
...
,_onKeyPress:function(e){var L=e.getKeyIdentifier();if(L==b||L==i){if(this.__wo){this.setSelection([this.__wo]);this.__wo=null;}
;this.toggle();}
...
,_onListPointerDown:function(e){if(this.__wo){this.setSelection([this.__wo]);this.__wo=null;}
;}
,_onListChangeSelection:function(e){var Q=e.getData();var O=e.getOldData();if(O&&O.length>0){O[0].removeListener(h,this.__xO,this);O[0].removeListener(g,this.__xP,this);}
;if(Q.length>0){var N=this.getChildControl(u);var P=this.getChildControl(l);var R=P.getSelectionContext();if(N.isVisible()&&(R==p||R==k)){this.__wo=Q[0];}
else {this.setSelection([Q[0]]);this.__wo=null;}
;Q[0].addListener(h,this.__xO,this);Q[0].addListener(g,this.__xP,this);}
...
,this);}
,properties:{appearance:{refine:true,init:t},placeholder:{check:n,nullable:true,apply:w}},events:{"changeValue":j},members:{__wo:null,__wp:null,_applyPlaceholder:function(A,z){this.getChildControl(p).setPlaceholder(A);}
...
,_setPreselectedItem:function(){if(this.__wo){var J=this.__wo.getLabel();if(this.getFormat()!=null){J=this.getFormat().call(this,this.__wo);}
;if(J&&J.translate){J=J.translate();}
;this.setValue(J);this.__wo=null;}
;}
,_onListChangeSelection:function(e){var K=e.getData();if(K.length>0){var M=this.getChildControl(o);var L=M.getSelectionContext();if(L==m||L==f){this.__wo=K[0];}
else {var N=K[0].getLabel();if(this.getFormat()!=null){N=this.getFormat().call(this,K[0]);}
;if(N&&N.translate){N=N.translate();}
;this.setValue(N);this.__wo=null;}
;}
;}
