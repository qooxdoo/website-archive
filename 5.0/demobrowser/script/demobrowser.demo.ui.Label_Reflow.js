...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.Label_Reflow","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.Label_Reflow.1fe714e19e27.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="qooxdoo is a comprehensive and innovative Ajax application framework. Leveraging object-oriented JavaScript allows developers to build impressive cross-browser applications. No HTML, CSS nor DOM knowledge is needed.",b="<b>qooxdoo is cool.</b>",c="qooxdoo (pronounced [’ku:ksdu:]) is a comprehensive and innovative Ajax application framework. Leveraging object-oriented JavaScript allows developers to build impressive cross-browser applications. No HTML, CSS nor DOM knowledge is needed. qooxdoo includes a platform-independent development tool chain, a state-of-the-art GUI toolkit and an advanced client-server communication layer. It is Open Source under an LGPL/EPL dual license.",d="#CCC",f="#FFF",g="It includes a platform-independent development tool chain, a state-of-the-art GUI toolkit and an advanced client-server communication layer. It is Open Source under an LGPL/EPL dual license.",h="demobrowser.demo.ui.Label_Reflow",i="tap",j="Toggle first paragraph",k="Resize Pane",l="#EEE",m="main";qx.Class.define(h,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var p=new qx.ui.container.Scroll();var n=new qx.ui.layout.Grid();n.setColumnFlex(0,1);n.setRowFlex(0,1);n.setSpacing(10);var t=new qx.ui.container.Composite(n).set({backgroundColor:l,decorator:m,padding:10,allowGrowX:false,allowGrowY:false,allowStretchX:false,allowStretchY:false});p.add(t);this.getRoot().add(p,{edge:0});var o=this.getPane();t.add(o,{row:0,column:0,colSpan:3});var r=new qx.ui.basic.Label(j).set({backgroundColor:d,decorator:m,padding:[4,8]});t.add(r,{row:1,column:1});var q=false;r.addListener(i,function(){var v=o.getChildren()[0];v.setValue(q?a:b);q=!q;}
);var s=new qx.ui.basic.Label(k).set({backgroundColor:d,decorator:m,padding:[4,8]});t.add(s,{row:1,column:2});var u=false;s.addListener(i,function(e){if(u){o.setWidth(600);}
else {o.setWidth(300);}
;u=!u;}
);}
,getPane:function(){var z=new qx.ui.container.Composite(new qx.ui.layout.VBox(14)).set({backgroundColor:f,width:600,decorator:m,padding:10});var w=new qx.ui.basic.Label().set({rich:true,value:a});z.add(w);var x=new qx.ui.basic.Label().set({rich:true,value:g});z.add(x);var y=new qx.ui.basic.Label().set({rich:true,value:c});z.add(y);return z;}
}});}
)();
