...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.mobile.PingPong.5e219825adfd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;this.__Pc=this.__Pg(L);U.add(this.__Pc);var Q=this.__Pg(A);U.add(Q);this.__OS=this.__Ph(L);this.__Pc.add(this.__OS);this.__Pc.addListener(M,qx.lang.Function.bind(this.__AG,this),this);this.__OR=this.__Ph(A);Q.add(this.__OR);Q.addListener(M,qx.lang.Function.bind(this.__AG,this),this);this.__OT=this.__Pi();U.add(this.__OT);this.__Pa=this.__Pj();this.__Pc.add(this.__Pa);this.__Pb=this.__Pj();Q.add(this.__Pb);this.__OY=[0,0];this.__Pd();}
,__Pd:function(){this.__OT.setStyles({"left":k,"top":k});this.__OU=new qx.event.Timer(40);this.__OU.addListener(n,function(){var x=(parseInt(this.__OT.getStyle(L))+this.__OV*this.__OW);var y=(parseInt(this.__OT.getStyle(z))+this.__OV*this.__OX);this.__OT.setStyle(L,x+c);this.__OT.setStyle(z,y+c);this.__Pf();}
,this);this.__OU.start();}
,__Pe:function(V){this.__OU.stop();this.__OU.dispose();this.__OV=5;this.__OY[V]=this.__OY[V]+1;this.__Pa.setAttribute(w,this.__OY[0]);this.__Pb.setAttribute(w,this.__OY[1]);this.__Pd();}
,__Pf:function(){var X=this.getBoundsFor(this.__OT);if(X.top<=0){this.__OX=1;}
;if(X.bottom>=window.innerHeight-5){this.__OX=-1;}
;if(X.left<=0||X.right>=window.innerWidth){this.__Pe(X.left<=0?1:0);}
;var Y=this.getBoundsFor(this.__OS);if(X.left<=Y.right&&X.bottom>=Y.top&&X.top<=Y.bottom){this.__OW=1;this.__OV=Math.min(this.__OV+1,20);}
;var W=this.getBoundsFor(this.__OR);if(X.right>=W.left&&X.bottom>=W.top&&X.top<=W.bottom){this.__OW=-1;this.__OV=Math.min(this.__OV+1,20);}
;}
,__AG:function(e){var ba;if(qx.dom.Hierarchy.contains(this.__Pc.getDomElement(),e.getTarget())){ba=this.__OS;}
else {ba=this.__OR;}
...
,__Pg:function(bd){var bc={"width":C,"height":h,"position":N,"backgroundColor":a};if(bd==L){bc[J]=u;}
else {bc[L]=C;bc[d]=u;}
;return new qx.html.Element(g,bc);}
,__Ph:function(bf){var be={"width":l,"height":t,"top":t,"position":N,"backgroundColor":K};be[bf]=F;return new qx.html.Element(g,be);}
,__Pi:function(){var bg={"width":F,"height":F,"position":N,"backgroundColor":K,"userSelect":I};return new qx.html.Element(g,bg);}
,__Pj:function(){var bh={"width":h,"height":h,"textAlign":r,"fontSize":H,"color":b,"fontFamily":B};var bi=new qx.html.Element(g,bh);bi.setAttribute(w,j);return bi;}
}});}
)();
