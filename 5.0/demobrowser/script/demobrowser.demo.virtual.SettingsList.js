...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.SettingsList.942be0eedd77.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="name",b="group.checked",c="#ddd",d="Fruits",f="label",g="Apple",h="Carrot",j="Vegetables",k="demobrowser.demo.virtual.SettingsList",l="value",m="changeValue",n="Orange",o="Potato",p="checked";qx.Class.define(k,{extend:qx.application.Standalone,members:{__cP:null,main:function(){qx.application.Standalone.prototype.main.call(this);var q=[];var s=[g,n,o,h];for(var i=0;i<s.length;i++ ){q[i]={name:s[i],checked:i<2,group:null};}
;this.__cP=qx.data.marshal.Json.createModel(q,true);var t=[];for(var i=0;i<2;i++ ){t[i]={name:null,checked:false};}
;t=qx.data.marshal.Json.createModel(t,true);t.getItem(0).setName(d);t.getItem(1).setName(j);this.__cP.toArray().forEach(function(u,v){v=v<2?0:1;u.setGroup(t.getItem(v));}
);var r=new qx.ui.list.List();r.setWidth(150);this.getRoot().add(r,{left:20,top:20});r.setDelegate(this);r.setModel(this.__cP);}
...
;var y=e.getData();this.__cP.forEach(function(z){if(x.getLabel()==z.getGroup().getName()){z.setChecked(y);}
;}
);}
,this);}
,bindItem:function(B,A,D){B.bindProperty(a,f,null,A,D);B.bindProperty(p,l,null,A,D);B.bindPropertyReverse(p,l,null,A,D);var C=this.__cP;B.bindPropertyReverse(b,l,{converter:function(H,J){var I=J.getItem(D);var F=I.getGroup();var G=C.toArray().filter(function(L){return L.getGroup()==F;}
...
(function(){var c="changeModel",d="resize",f="_applyLabelOptions",g="_applyLabelPath",h="qx.data.Array",j="_applyGroupLabelOptions",m="changeLength",n="row-layer",o="_applyModel",p="changeGroups",q="Boolean",r="GroupingTypeError: You can't mix 'Objects' and 'Strings' as",s="Integer",t="sorter",u="_applyGroupRowHeight",v="group",w="_applyIconPath",x="_applyDelegate",y="changeDelegate",z=" group identifier!",A="???",B="_applyRowHeight",C="qx.data.IListData",D="filter",E="virtual-list",F="String",G="_applyGroupLabelPath",H="_applyIconOptions",I="qx.ui.list.List";qx.Class.define(I,{extend:qx.ui.virtual.core.Scroller,include:[qx.ui.virtual.selection.MModel],implement:qx.data.controller.ISelection,construct:function(J){qx.ui.virtual.core.Scroller.call(this,0,1,20,100);this._init();this.__xh=new qx.data.Array();this.initGroups(this.__xh);if(J!=null){this.initModel(J);}
;this.initItemHeight();}
...
,_lookup:function(Q){return this.__xi[Q];}
,_lookupGroup:function(R){return this.__xj.indexOf(R);}
,_reverseLookup:function(S){if(S<0){return -1;}
;return this.__xi.indexOf(S);}
...
,_applyGroupRowHeight:function(ba,Y){this.__xp();}
...
,_applyDelegate:function(bo,bn){this._provider.setDelegate(bo);this.__xq();}
,_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);}
,_onModelChange:function(e){this.__xq();this._applyDefaultSelection();}
,__xo:function(){this.getPane().getRowConfig().setItemCount(this.__xi.length);this.getPane().fullUpdate();}
,__xp:function(){var bp=this.getPane().getRowConfig();var bq=this.getGroupItemHeight();bp.resetItemSizes();if(bq){for(var i=0,l=this.__xi.length;i<l; ++i){if(this.__xi[i]==-1){bp.setItemSize(i,bq);}
;}
;}
;}
,__xq:function(){this.__xi=[];this.__xj=[];this.__xk={};if(this.isAutoGrouping()){this.getGroups().removeAll();}
;var br=this.getModel();if(br!=null){this._runDelegateFilter(br);this._runDelegateSorter(br);this._runDelegateGroup(br);}
;this._updateSelection();this.__xp();this.__xo();}
,_runDelegateFilter:function(bt){var bs=qx.util.Delegate.getMethod(this.getDelegate(),D);for(var i=0,l=bt.length;i<l; ++i){if(bs==null||bs(bt.getItem(i))){this.__xi.push(i);}
;}
;}
,_runDelegateSorter:function(bu){if(this.__xi.length==0){return;}
;var bv=qx.util.Delegate.getMethod(this.getDelegate(),t);if(bv!=null){this.__xi.sort(function(a,b){return bv(bu.getItem(a),bu.getItem(b));}
);}
;}
,_runDelegateGroup:function(bz){var bA=qx.util.Delegate.getMethod(this.getDelegate(),v);if(bA!=null){for(var i=0,l=this.__xi.length;i<l; ++i){var bw=this.__xi[i];var by=this.getModel().getItem(bw);var bx=bA(by);this.__xr(bx,bw);}
;this.__xi=this.__xs();}
;}
,__xr:function(bC,bB){if(bC==null){this.__xn=true;bC=A;}
;var name=this.__xt(bC);if(this.__xk[name]==null){this.__xk[name]=[];if(this.isAutoGrouping()){this.getGroups().push(bC);}
;}
;this.__xk[name].push(bB);}
,__xs:function(){this.__xu();var bG=[];var bI=0;var bE=this.getGroups();for(var i=0;i<bE.getLength();i++ ){var bD=bE.getItem(i);bG.push(-1);this.__xj.push(bI);bI++ ;var bH=this.__xt(bD);var bF=this.__xk[bH];if(bF!=null){for(var k=0;k<bF.length;k++ ){bG.push(bF[k]);bI++ ;}
;}
;}
;return bG;}
,__xt:function(bK){var name=null;if(!qx.lang.Type.isString(bK)){var bJ=this.getGroups().indexOf(bK);this.__xm=true;name=v;if(bJ==-1){name+=this.getGroups().getLength();}
else {name+=bJ;}
;}
else {this.__xl=true;var name=bK;}
;return name;}
,__xu:function(){if(this.__xm&&this.__xn||this.__xm&&this.__xl){throw new Error(r+z);}
;}
...
(function(){var a="cell.type",b="model[",c="groups[",d="model",f="",g="changeDelegate",h="label",i="qx.ui.list.core.MWidgetController",j="icon",k="]",l="value",m="BindingIds",n=".",o="group",p="String";qx.Mixin.define(i,{construct:function(){this.__xC=[];}
...
;}
,bindProperty:function(u,x,v,t,s){var y=t.getUserData(a);var w=this.__xD(s,u,y);if(v){v.ignoreConverter=d;}
;var z=this._list.bind(w,t,x,v);this.__xE(t,z);}
,bindPropertyReverse:function(D,B,E,F,A){var G=F.getUserData(a);var C=this.__xD(A,D,G);var H=F.bind(B,this._list,C,E);this.__xE(F,H);}
,removeBindings:function(){while(this.__xC.length>0){var I=this.__xC.pop();this._removeBindingsFrom(I);}
;}
...
;}
,_removeBindingsFrom:function(T){var U=this.__xF(T);while(U.length>0){var V=U.pop();try{this._list.removeBinding(V);}
catch(e){T.removeBinding(V);}
;}
;if(qx.lang.Array.contains(this.__xC,T)){qx.lang.Array.remove(this.__xC,T);}
;}
,__xD:function(X,ba,Y){var W=b+X+k;if(Y==o){W=c+X+k;}
;if(ba!=null&&ba!=f){W+=n+ba;}
;return W;}
,__xE:function(bc,bd){var bb=this.__xF(bc);if(!qx.lang.Array.contains(bb,bd)){bb.push(bd);}
;if(!qx.lang.Array.contains(this.__xC,bc)){this.__xC.push(bc);}
;}
,__xF:function(be){var bf=be.getUserData(m);if(bf==null){bf=[];be.setUserData(m,bf);}
;return bf;}
...
,styleSelectabled:function(v){var u=this.__xG(v);this._styleSelectabled(u);}
,styleUnselectabled:function(x){var w=this.__xG(x);this._styleUnselectabled(w);}
...
;}
,_styleSelectabled:function(A){this.__xH(A,{selected:1});}
,_styleUnselectabled:function(B){this.__xH(B,{});}
...
,__xG:function(G){return this._list._layer.getRenderedCellWidget(G,0);}
,__xH:function(I,H){if(I==null){return;}
