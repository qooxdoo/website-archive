...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.ComboBox.da67ccf4e2a7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
},destruct:function(){if(this.__wy){this.__wy.dispose();}
;}
});}
)();
...
(function(){var a="changeModel",b="one",c="Enter",d="list",e="changeDelegate",f="change",g="qx.data.Array",h="changeVisibility",j="qx.ui.form.core.VirtualDropDownList",k="_applySelection",l="changeSelection",m="tap",n="Invalid parameter 'target'!",o="qx.event.type.Data";qx.Class.define(j,{extend:qx.ui.popup.Popup,construct:function(p){qx.core.Assert.assertNotNull(p,n);qx.core.Assert.assertNotUndefined(p,n);qx.core.Assert.assertInterface(p,qx.ui.form.core.AbstractVirtualBox,n);qx.ui.popup.Popup.call(this,new qx.ui.layout.VBox());this._target=p;this._createChildControl(d);this.addListener(h,this.__wC,this);this.__wz=new qx.data.Array();this.initSelection(this.__wz);}
,properties:{autoHide:{refine:true,init:false},keepActive:{refine:true,init:true},selection:{check:g,event:l,apply:k,nullable:false,deferredInit:true}},events:{"changeSelection":o},members:{_target:null,_preselected:null,__wA:false,__wB:false,__wz:null,open:function(){this.placeToWidget(this._target,true);this.show();}
,close:function(){this.hide();}
,setPreselected:function(q){this._preselected=q;this.__wB=true;var s=this.getChildControl(d).getSelection();var r=new qx.data.Array([q]);this.__wE(r,s);r.dispose();this.__wB=false;}
...
,_handleKeyboard:function(event){if(this.isVisible()&&event.getKeyIdentifier()===c){this.__wD();return;}
;var w=event.clone();w.setTarget(this.getChildControl(d));w.setBubbles(false);this.getChildControl(d).dispatchEvent(w);}
,_handlePointer:function(event){this.__wD();}
,__lR:function(event){if(this.__wA){return;}
;var x=this.getSelection();var y=this.getChildControl(d).getSelection();this.__wB=true;this.__wE(x,y);this.__wB=false;this.__wA=true;this.__wE(y,x);this.__wA=false;}
,_onListChangeSelection:function(event){if(this.__wB){return;}
;var z=this.getChildControl(d).getSelection();if(this.isVisible()){this.setPreselected(z.getItem(0));}
else {this.__wA=true;this.__wE(z,this.getSelection());this.__wA=false;}
;}
,__wC:function(event){if(this.isVisible()){if(this._preselected==null){var A=this.getSelection();var B=this.getChildControl(d).getSelection();this.__wE(A,B);}
;this.__wF();}
...
,_applySelection:function(D,C){D.addListener(f,this.__lR,this);if(C!=null){C.removeListener(f,this.__lR,this);}
;this.__wE(D,this.getChildControl(d).getSelection(D));}
,__wD:function(){if(this._preselected!=null){var E=this.getSelection();E.splice(0,1,this._preselected);this._preselected=null;this.close();}
;}
,__wE:function(F,H){if(F.equals(H)){return;}
...
;}
,__wF:function(){this.__wG();this.__wH();}
,__wG:function(){var I=this._target.getBounds().width;this.setWidth(I);}
,__wH:function(){var J=this.__wI();var L=this._target.getMaxListHeight();var K=this.getChildControl(d);var M=K.getPane().getRowConfig().getTotalSize();if(L==null||M<L){L=M;}
;if(L>J){K.setMaxHeight(J);}
else if(L<J){K.setMaxHeight(L);}
;}
,__wI:function(){var N=this.getLayoutLocation(this._target);var Q=qx.bom.Viewport.getHeight();var P=N.top;var O=Q-N.bottom;return P>O?P:O;}
},destruct:function(){if(this.__wz){this.__wz.dispose();}
;}
});}
)();
...
,_beforeOpen:function(){this.__xN();}
...
;this.close();}
,__xM:function(event){if(this.__xd==true){return;}
;var N=this.__gD.getItem(0);if(N){N=this.__bp(N);this.setValue(N);}
;}
,_applyPlaceholder:function(P,O){this.getChildControl(r).setPlaceholder(P);}
,__xN:function(){var S=this.getValue();var R=this.getChildControl(s);var Q=R.getSelection();if(this.__bp(Q.getItem(0))!==S){this.__xd=true;Q.removeAll();this.__xd=false;if(S==null||S==q){return;}
;var W=this.getModel();var V=R.getChildControl(j)._getLookupTable();for(var i=0,l=V.length;i<l;i++ ){var T=W.getItem(V[i]);var U=this.__bp(T);if(U&&U.indexOf(S)==0){R.setPreselected(T);break;}
;}
;}
;}
,__bp:function(ba){var Y=this.getLabelOptions();var bc=this.getDefaultFormat();var bd=this.getLabelPath();var bb=null;if(bd!=null){bb=qx.data.SingleValueBinding.resolvePropertyChain(ba,bd);}
...
;return bb;}
},destruct:function(){var be=this.getChildControl(r);this.removeAllBindings();be.removeAllBindings();this.__gD.removeListener(k,this.__xM,this);this.__gD=null;}
});}
)();
