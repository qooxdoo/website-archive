...
(function(){var a='"',b="Class '",c="",d="Unsupported type!",e="change",f="qx.data.marshal.Json",g="Array",h="_validate",j="' could not be found.",k="]",l="[",m="set",n="_applyEventPropagation",o="qx.data.model.";qx.Class.define(f,{extend:qx.core.Object,implement:[qx.data.marshal.IMarshaler],construct:function(p){qx.core.Object.call(this);this.__tz=p;}
...
},members:{__tz:null,__tA:function(s){return Object.keys(s).sort().join(a);}
,toClass:function(t,u){this.__tB(t,u,null,0);}
,__tB:function(G,B,v,D){if(!qx.lang.Type.isObject(G)||!!G.$$isString||G instanceof qx.core.Object){if(G instanceof Array||qx.Bootstrap.getClass(G)==g){for(var i=0;i<G.length;i++ ){this.__tB(G[i],B,v+l+i+k,D+1);}
;}
;return;}
;var x=this.__tA(G);if(this.__tF(x,v,D)){return;}
;for(var F in G){this.__tB(G[F],B,F,D+1);}
;if(qx.Class.isDefined(o+x)){return;}
;if(this.__tz&&this.__tz.getModelClass&&this.__tz.getModelClass(x,G,v,D)!=null){return;}
;var H={};var y={__tC:this.__tC};for(var F in G){if(this.__tz&&this.__tz.getPropertyMapping){F=this.__tz.getPropertyMapping(F,x);}
;F=F.replace(/-|\.|\s+/g,c);{}
;H[F]={};H[F].nullable=true;H[F].event=e+qx.lang.String.firstUp(F);if(B){H[F].apply=n;}
;if(this.__tz&&this.__tz.getValidationRule){var A=this.__tz.getValidationRule(x,F);if(A){H[F].validate=h+F;y[h+F]=A;}
;}
;}
;if(this.__tz&&this.__tz.getModelSuperClass){var E=this.__tz.getModelSuperClass(x,v,D)||qx.core.Object;}
else {var E=qx.core.Object;}
;var z=[];if(this.__tz&&this.__tz.getModelMixins){var C=this.__tz.getModelMixins(x,v,D);if(!qx.lang.Type.isArray(C)){if(C!=null){z=[C];}
;}
else {z=C;}
;}
;if(B){z.push(qx.data.marshal.MEventBubbling);}
;var w={extend:E,include:z,properties:H,members:y,destruct:this.__tD};qx.Class.define(o+x,w);}
,__tD:function(){var I=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var J in I){this.__tC(this.get(I[J].name));}
;}
,__tC:function(K){if(!(K instanceof qx.core.Object)){return;}
;if(K.isDisposed()){return;}
;K.dispose();}
,__tE:function(L,P,Q,O){var R;if(this.__tz&&this.__tz.getModelClass){R=this.__tz.getModelClass(L,P,Q,O);}
;if(R!=null){return (new R());}
else {var N=o+L;var M=qx.Class.getByName(N);if(!M){throw new Error(b+N+j);}
;return (new M());}
;}
,__tF:function(S,V,T){var U=this.__tz;return U&&U.ignore&&U.ignore(S,V,T);}
,toModel:function(W){return this.__tG(W,null,0);}
,__tG:function(bj,Y,bf){var bd=qx.lang.Type.isObject(bj);var X=bj instanceof Array||qx.Bootstrap.getClass(bj)==g;if((!bd&&!X)||!!bj.$$isString||bj instanceof qx.core.Object){return bj;}
else if(this.__tF(this.__tA(bj),Y,bf)){return bj;}
else if(X){var bh=qx.data.Array;if(this.__tz&&this.__tz.getArrayClass){var be=this.__tz.getArrayClass(Y,bf);bh=be||bh;}
;var bk=new bh();bk.setAutoDisposeItems(true);for(var i=0;i<bj.length;i++ ){bk.push(this.__tG(bj[i],Y+l+i+k,bf+1));}
;return bk;}
else if(bd){var ba=this.__tA(bj);var bi=this.__tE(ba,bj,Y,bf);for(var bg in bj){var bb=bg;if(this.__tz&&this.__tz.getPropertyMapping){bb=this.__tz.getPropertyMapping(bg,ba);}
;var bl=bb.replace(/-|\.|\s+/g,c);{}
;bb=bl;var bc=m+qx.lang.String.firstUp(bb);if(bi[bc]){bi[bc](this.__tG(bj[bg],bg,bf+1));}
;}
;return bi;}
;throw new Error(d);}
