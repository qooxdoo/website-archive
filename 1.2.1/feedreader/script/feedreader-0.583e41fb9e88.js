...
qx.Class.define(j,{extend:qx.core.Object,type:d,members:{loadAll:function(u){var w=u.getFeeds().getItem(0).getFeeds();

for(var i=0;i<w.length;i++){this.load(w.getItem(i));
}var v=u.getFeeds().getItem(1).getFeeds();

for(i=0;i<v.length;i++){this.load(v.getItem(i));
}},load:function(a){a.setState(h);
var b=m+a.getUrl()+l;
var c=new qx.data.store.Yql(b,{manipulateData:function(t){if(t.query.results){t=t.query.results.item||t.query.results.entry;
for(var i=0;i<t.length;i++){if(!qx.lang.Type.isString(t[i].title)){t[i].title=t[i].title.content;
}}return t;
}return p;
}},qx.bom.client.Feature.SSL);
c.addListener(n,this.__Au(a),this);
c.addListener(g,qx.lang.Function.bind(this.__At,this,a),this);
},__At:function(r,e){if(e.getData()==f||e.getData()==k||e.getData()==p){r.setState(o);
}},__Au:function(q){return function(e){var s=e.getData();
if(s==p){q.setState(o);
return;
}q.setArticles(s);
q.setState(n);
...
(function(){var o="static",n="user",m="execute",l="loading",k="selection[0]",j="title",h="north",g="main",f="new",d="Surfin' Safari",br="changeModel",bq="__Av",bp="selection[0].state",bo="Heise",bn="__AC",bm="__Az",bl="__AG",bk="F1",bj="article",bi="partLoaded",v="Control+A",w="http://blogs.msdn.com/ie/rss.xml",t="http://my.opera.com/desktopteam/xml/rss/blog/",u="settings",r="feedreader/images/loading22.gif",s="error",p="change",q="http://blogs.msdn.com/jscript/rss.xml",C="__Ay",D="loaded",N="feeds",K="__AJ",V="The Mozilla Blog",Q="enabled",be="__AH",bb="http://webkit.org/blog/feed/",G="separator-vertical",bh="http://feeds2.feedburner.com/qooxdoo/news/content",bg="http://blog.mozilla.com/feed/",bf="__Aw",F="http://www.heise.de/newsticker/heise-atom.xml",I="__AB",J="state",M="JScript Team Blog",O="http://feeds2.feedburner.com/ajaxian",R="Control+D",X="__AD",bd="__AF",x="icon/22/actions/process-stop.png",y="selection[0].articles",H="white",U="IEBlog",T="Ajaxian",S="__AA",ba="icon/22/apps/internet-feed-reader.png",Y="model",P="feedreader.Application",W="http://daringfireball.net/index.xml",a="Control+P",bc="__AE",z="Control+R",A="qooxdoo News",L="Opera Desktop Blog",b="feedreader.model.Feed",c="Daring Fireball",E="__AI",B="addfeed";
qx.Class.define(P,{extend:qx.application.Standalone,members:{__Av:null,__Aw:null,__Ax:null,__Ay:null,__Az:null,__AA:null,__AB:null,__AC:null,__AD:null,__AE:null,__AF:null,__AG:null,__AH:null,__AI:null,__AJ:null,main:function(){qx.application.Standalone.prototype.main.call(this);
{};
qx.io.PartLoader.getInstance().addListener(bi,function(e){this.debug("part loaded: "+e.getData().getName());
},this);
var bz=qx.locale.Manager.getInstance().getLanguage();
var by=qx.Part.getInstance().getParts();
if(by[bz]){qx.io.PartLoader.require([bz],function(){qx.locale.Manager.getInstance().setLocale(bz);
this.buildUpGui();
},this);
}else{this.warn("Cannot load locale part for current language "+bz+", falling back to English.");
this.buildUpGui();
}},buildUpGui:function(){this._initializeCommands();
this._createLayout();
this._initializeModel();
this._setUpBinding();
this.__Ay.getRoot().setOpen(true);
this.__Ay.getRoot().getChildren()[0].setOpen(true);
this.__Ay.getRoot().getChildren()[1].setOpen(true);
this.__Ay.setHideRoot(true);
this.__Av.getSelection().push(this.__AJ.getFeeds().getItem(0));
this.reload();
},_initializeModel:function(){this.__AH=new feedreader.model.FeedFolder("Feeds");
this.__AJ=new feedreader.model.FeedFolder(this.tr("Static Feeds"));
this.__AH.getFeeds().push(this.__AJ);
this.__AJ.getFeeds().push(new feedreader.model.Feed(A,bh,o));
this.__AJ.getFeeds().push(new feedreader.model.Feed(M,q,o));
this.__AJ.getFeeds().push(new feedreader.model.Feed(c,W,o));
this.__AJ.getFeeds().push(new feedreader.model.Feed(d,bb,o));
this.__AJ.getFeeds().push(new feedreader.model.Feed(T,O,o));
this.__AI=new feedreader.model.FeedFolder(this.tr("User Feeds"));
this.__AH.getFeeds().push(this.__AI);
this.__AI.getFeeds().push(new feedreader.model.Feed(bo,F,n));
this.__AI.getFeeds().push(new feedreader.model.Feed(U,w,n));
this.__AI.getFeeds().push(new feedreader.model.Feed(V,bg,n));
this.__AI.getFeeds().push(new feedreader.model.Feed(L,t,n));
},_setUpBinding:function(){var bJ={converter:this._state2iconConverter};
this.__Av=new qx.data.controller.Tree(this.__AH,this.__Ay,N,j);
this.__Av.setIconOptions(bJ);
this.__Av.setIconPath(J);
this.__Aw=new qx.data.controller.List(null,this.__AB.getList(),j);
this.__Av.bind(y,this.__Aw,Y);
this.__Aw.bind(k,this.__AF,bj);
this.__Aw.getSelection().addListener(p,this._listControllerChange,this);
this.__Aw.addListener(br,this._treeControllerChange,this);
var bK={converter:this._state2loadingConverter};
this.__Av.bind(bp,this.__AB,l,bK);
bK={converter:this._category2enabledConverter};
this.__Av.bind(k,this.__AA.getRemoveButton(),Q,bK);
},_category2enabledConverter:function(bF){return !!bF&&bF.getCategory()==n;
},_state2iconConverter:function(bI){if(bI==f||bI==l){return r;
}else if(bI==D){return ba;
}else if(bI==s){return x;
}return null;
},_state2loadingConverter:function(bG){if(bG==f||bG==l){return true;
}return false;
},_listControllerChange:function(bC){var bD=this.__Av.getSelection().getItem(0);
var bE=this.__Aw.getSelection().getItem(0);
if(bE!=undefined){bD.setSelectedArticle(bE);
}},_treeControllerChange:function(bL){var bM=this.__Av.getSelection().getItem(0);
if(bM!=null&&bM.classname==b&&bM.getSelectedArticle()!=null&&bM.getArticles().contains(bM.getSelectedArticle())){this.__Aw.getSelection().push(bM.getSelectedArticle());
}else{this.__AB.getList().scrollToY(0);
}},_createLayout:function(){var bO=new qx.ui.layout.Dock();
bO.setSeparatorY(G);
var bP=new qx.ui.container.Composite(bO);
this.getRoot().add(bP,{edge:0});
this.__Az=new feedreader.view.Header();
bP.add(this.__Az,{edge:h});
this.__AA=new feedreader.view.ToolBar(this);
bP.add(this.__AA,{edge:h});
this.__AC=new qx.ui.splitpane.Pane();
bP.add(this.__AC);
this.__Ay=new qx.ui.tree.Tree();
this.__Ay.setWidth(250);
this.__Ay.setBackgroundColor(H);
this.__AC.add(this.__Ay,0);
this.__AD=new qx.ui.splitpane.Pane("vertical");
this.__AD.setDecorator(null);
this.__AC.add(this.__AD,1);
this.__AB=new feedreader.view.List(this.__AH);
this.__AB.setHeight(200);
this.__AB.setDecorator(g);
this.__AD.add(this.__AB,0);
this.__AF=new feedreader.view.Article();
this.__AF.setDecorator(g);
this.__AD.add(this.__AF,1);
},_initializeCommands:function(){var bH={};
bH.reload=new qx.ui.core.Command(z);
bH.reload.addListener(m,this.reload,this);
bH.about=new qx.ui.core.Command(bk);
bH.about.addListener(m,this.showAbout,this);
bH.preferences=new qx.ui.core.Command(a);
bH.preferences.addListener(m,this.showPreferences,this);
bH.addFeed=new qx.ui.core.Command(v);
bH.addFeed.addListener(m,this.showAddFeed,this);
bH.removeFeed=new qx.ui.core.Command(R);
bH.removeFeed.addListener(m,this.removeFeed,this);
this.__Ax=bH;
},getCommand:function(bN){return this.__Ax[bN];
},addFeed:function(bt,bu,bv){var bw=new feedreader.model.Feed(bt,bu,bv);
this.__AI.getFeeds().push(bw);
var bx=feedreader.io.FeedLoader.getInstance();
bx.load(bw);
},removeFeed:function(){var bA=this.__Av.getSelection().getItem(0);
if(bA&&bA.getCategory&&bA.getCategory()!==o){var bB=this.__AI.getFeeds();
for(var i=0;i<bB.length;i++){if(bA===bB.getItem(i)){bB.splice(i,1);
return;
}}}},reload:function(){var bs=feedreader.io.FeedLoader.getInstance();
bs.loadAll(this.__AH);
},showPreferences:function(){qx.io.PartLoader.require([u],function(){if(!this.__AE){this.__AE=new feedreader.view.PreferenceWindow();
this.getRoot().add(this.__AE);
}this.__AE.center();
this.__AE.open();
},this);
},showAbout:function(){alert(this.tr("FeedReader (qooxdoo powered)"));
},showAddFeed:function(){qx.io.PartLoader.require([B],function(){if(!this.__AG){this.__AG=new feedreader.view.AddFeedWindow(this);
this.getRoot().add(this.__AG);
}this.__AG.center();
this.__AG.open();
},this);
}},destruct:function(){this.__Ax=null;
this._disposeObjects(S,I,bd,C,be,bn,X,bm,K,E,bq,bf,bc,bl);
}});
})();
...
(function(){var s="reload",r="Add feed",q="removeFeed",p="Open preferences window.",o="icon/22/actions/view-refresh.png",n="Reload the feeds. (%1)",m="(",l=")",k="__AK",j="Reload",d="Preferences",i="icon/22/actions/dialog-ok.png",g="preferences",c="icon/22/apps/preferences-theme.png",b="icon/22/actions/help-about.png",f="feedreader.view.ToolBar",e="about",h="Help",a="addFeed";
qx.Class.define(f,{extend:qx.ui.toolbar.ToolBar,construct:function(t){qx.ui.toolbar.ToolBar.call(this);
var A=new qx.ui.toolbar.Part;
this.add(A);
var B=new qx.ui.toolbar.Button(this.tr(r),i);
B.setCommand(t.getCommand(a));
A.add(B);
this.__AK=new qx.ui.toolbar.Button(this.tr("Remove feed"),"icon/22/actions/dialog-cancel.png");
this.__AK.setCommand(t.getCommand(q));
this.__AK.setEnabled(false);
A.add(this.__AK);
A.addSeparator();
var w=new qx.ui.toolbar.Button(this.tr(j),o);
var x=t.getCommand(s);
w.setCommand(x);
w.setToolTipText(this.tr(n,x.toString()));
A.add(w);
A.addSeparator();
var v=new qx.ui.toolbar.Button(this.tr(d),c);
v.setCommand(t.getCommand(g));
v.setToolTipText(this.tr(p));
A.add(v);
this.addSpacer();
var z=new qx.ui.toolbar.Part;
this.add(z);
var y=new qx.ui.toolbar.Button(this.tr(h),b);
var u=t.getCommand(e);
y.setCommand(u);
y.setToolTipText(m+u.toString()+l);
z.add(y);
},members:{__AK:null,getRemoveButton:function(){return this.__AK;
...
(function(){var p="separator-vertical",o="background-medium",n="Posts",m="bold",l="__AM",k="_applyLoading",j="__AN",i="feedreader.view.List",h="single",g="__AL",d="middle",f="feedreader/images/loading66.gif",e="Boolean";
...
c.setBackgroundColor(o);
c.setPadding(5);
c.setAllowGrowX(true);
c.setFont(m);
this._add(c);
this.__AL=new qx.ui.container.Stack();
this._add(this.__AL,{flex:1});
this.__AM=new qx.ui.form.List();
this.__AM.setDecorator(null);
this.__AM.setSelectionMode(h);
this.__AL.add(this.__AM);
this.__AN=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,"center"));
var a=new qx.ui.basic.Image(f);
a.setAlignY(d);
this.__AN.add(a);
this.__AL.add(this.__AN);
},properties:{loading:{check:e,init:false,apply:k}},members:{__AL:null,__AM:null,__AN:null,_applyLoading:function(q,r){if(q){this.__AL.setSelection([this.__AN]);
}else{this.__AL.setSelection([this.__AM]);
}},getList:function(){return this.__AM;
}},destruct:function(){this._disposeObjects(l,g,j);
}});
})();
...
if(!feedreader.view.Article.__Bc){feedreader.view.Article.__Bc=true;
...
B.setLineHeight(1.8);
this.setFont(B);
},properties:{article:{apply:e,nullable:true,check:d}},members:{_applyArticle:function(w,x){var y=this.__Bd();
this.setHtml(y);
qx.html.Element.flush();
var A=this.getContentElement().getDomElement();
var z=A.getElementsByTagName(b);
for(var i=0;i<z.length;i++){z[i].target=t;
}},__Bd:function(){var D=this.getArticle();
if(!D){return v;
