...
var self=this;
qx.util.Permutation.permute(D,function(s){var content=[];

for(var w in s){if(w!==m){content.push(w+a+s[w]);
}}content=content.join(d);
var x=self._createRowElement(C,F,E);
var t=B.getRowClass(s);

if(t){x.className=t;
}x.innerHTML=content+j;
B.updateDataRowElement(s,x);
var v=[];
v.push(g);

if(t){v.push(k,t,q);
}var u=B.createRowStyle(s);
u+=n+E+b+F+f;

if(u){v.push(h,u,q);
}v.push(p,content,l);
C.innerHTML+=v.join(o);
self._rowTop+=E;
});
},_rowTop:0,_createRowElement:function(parent,y,z){var A=qx.bom.Element.create(c);
qx.bom.element.Style.setStyles(A,{position:i,width:y+r,height:z+r});
this._rowTop+=20;
