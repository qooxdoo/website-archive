...
var L=new qx.ui.progressive.renderer.table.Widths(1);
L.setWidth(0,q);
var F=new qx.ui.progressive.headfoot.Progress(L);
var J=new qx.ui.progressive.structure.Default(null,F);
var E=new qx.ui.progressive.Progressive(J);
E.setFlushWidgetQueueAfterBatch(true);
var I=function(be){var bf={renderer:p,data:be};
return bf;
};
var P=new qx.ui.progressive.model.Default();
var M=new qx.ui.progressive.renderer.FunctionCaller();
E.addRenderer(p,M);
var K=k;
var N=new qx.ui.basic.Image(K,q,q);
E.add(N);
E.set({height:100,width:272,zIndex:99999,backgroundColor:D,opacity:0.86,batchSize:10});
this.getRoot().add(E,{top:-1000,left:-1000});
var O=[{text:l,background:v,color:r},{text:B,background:j,color:r},{text:z,background:C,color:r},{text:t,background:w,color:r},{text:A,background:i,color:m}];
this.context={document:this.getRoot(),pages:O};
E.addListener(y,function(e){var f=e.getData().state;
...
P.addElement(I(function(W){var X=W.context;
X.tabview=new qx.ui.tabview.TabView();
X.tabview.set({});
X.document.add(X.tabview,{left:20,right:20,top:20,bottom:20});
X.page=0;
X.currentPage=null;
}));

for(var G=0;G<O.length;G++){P.addElement(I(function(Y){var bb=Y.context;
var ba=bb.pages[bb.page];
bb.currentPage=new qx.ui.tabview.Page(ba.text);
bb.currentPage.setLayout(new qx.ui.layout.VBox(4));
bb.tabview.add(bb.currentPage);
bb.row=0;
bb.col=0;
if(bb.page==0){bb.firstPage=bb.currentPage;
}}));
for(var Q=0;Q<20;Q++){P.addElement(I(function(R){var S=R.context;
...
for(var H=0;H<10;H++){P.addElement(I(function(T){var V=T.context;
var U=V.pages[V.page];
var o=new qx.ui.basic.Atom(s+V.row+x+h+V.col);
o.set({width:90,backgroundColor:U.background,textColor:U.color});
V.hBox.add(o);
}));
P.addElement(I(function(bd){++bd.context.col;
}));
}P.addElement(I(function(bh){++bh.context.row;
}));
}P.addElement(I(function(bg){++bg.context.page;
}));
}P.addElement(I(function(bc){bc.context.tabview.setSelection([bc.context.firstPage]);
}));
E.setDataModel(P);
E.render();
}}});
