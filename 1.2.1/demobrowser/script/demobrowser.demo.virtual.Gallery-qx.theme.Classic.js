...
(function(){var ba="",Y="selected",X="cell",W="layer",V="icon",U="#f3f3f3",T="label",S="Gallery (HTML - divs)",R="manager",Q="default",bE="demobrowser.demo.virtual.Gallery",bD="demobrowser.demo.virtual.WidgetGallery",bC="text-align: center;",bB="auto",bA="color: white; background-color: #00398D;",bz="user-desktop.png",by="<img src='",bx="'></img>",bw="group",bv="resize",bh="folder.png",bi="vertical",bf="multi",bg="network-server.png",bd="user-trash.png",be="<br>",bb="off",bc="top",bj="icon/32/places/",bk="horizontal",bn="demobrowser.demo.virtual.AbstractGallery",bm="abstract",bp="position: absolute;",bo="demobrowser.demo.virtual.HtmlGallery",br="Gallery (widgets)",bq="__Nj",bl="Icon #",bu="network-workgroup.png",bt="text-selected",bs="demobrowser.demo.virtual.GalleryCell";
qx.Class.define(bE,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);
new demobrowser.demo.virtual.WidgetGallery(br);
var d=new demobrowser.demo.virtual.HtmlGallery(S);
d.moveTo(400,50);
}}});
qx.Class.define(bn,{extend:qx.ui.window.Window,type:bm,construct:function(u){qx.ui.window.Window.call(this,u);
...
v.set({scrollbarX:bb,scrollbarY:bB});
v.getPane().addListener(bv,this._onPaneResize,this);
this.add(v);
this.manager=new qx.ui.virtual.selection.CellRectangle(v.getPane(),this).set({mode:bf,drag:true});
...
},_createScroller:function(){},isItemSelectable:function(w){return !!this.getItemData(w.row,w.column);
},styleSelectable:function(z,A,B){},_onPaneResize:function(e){var p=e.getTarget();
var n=e.getData().width;
var o=Math.floor(n/this.itemWidth);

if(o==this.itemsPerLine){return;
}this.itemPerLine=o;
var q=Math.ceil(this.itemCount/o);
p.getColumnConfig().setItemCount(o);
p.getRowConfig().setItemCount(q);
},_generateItems:function(H){var K=[];
var N=[bh,bd,bg,bu,bz];
var L=qx.util.AliasManager.getInstance();
var J=qx.util.ResourceManager.getInstance();

for(var i=0;i<H;i++){var O=bj+N[Math.floor(Math.random()*N.length)];
var I=L.resolve(O);
var M=J.toUri(I);
K[i]={label:bl+(i+1),icon:O,resolvedIcon:M};
}return K;
}},destruct:function(){this.items=null;
this._disposeObjects(R);
}});
qx.Class.define(bs,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var P=new qx.ui.basic.Atom().set({iconPosition:bc});
P.getChildControl(T).set({padding:[0,4]});
P.getChildControl(V).set({padding:4});
return P;
},updateData:function(bS,bT){bS.set({icon:bT.icon,label:bT.label});
},updateStates:function(C,D){var F=C.getChildControl(T);
var E=C.getChildControl(V);

if(D.selected){F.setDecorator(Y);
F.setTextColor(bt);
E.setDecorator(bw);
...
qx.Class.define(bD,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(G){demobrowser.demo.virtual.AbstractGallery.call(this,G);
this.__Nj=new demobrowser.demo.virtual.GalleryCell();
},members:{_createScroller:function(){var t=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.WidgetCell(this);
t.getPane().addLayer(this.layer);
new qx.ui.virtual.behavior.Prefetch(t,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:200,maxAbove:300,minBelow:600,maxBelow:800}).set({interval:500});
return t;
},styleSelectable:function(g,h,j){if(h!==Y){return;
}var k=this.layer.getChildren();

for(var i=0;i<k.length;i++){var l=k[i];
var m=l.getUserData(X);

if(g.row!==m.row||g.column!==m.column){continue;
}
if(j){this.__Nj.updateStates(l,{selected:1});
}else{this.__Nj.updateStates(l,{});
}}},getCellWidget:function(bM,bN){var bR=this.getItemData(bM,bN);

if(!bR){return null;
}var bO={row:bM,column:bN};
var bP={};

if(this.manager.isItemSelected(bO)){bP.selected=true;
}var bQ=this.__Nj.getCellWidget(bR,bP);
bQ.setUserData(X,bO);
return bQ;
},poolCellWidget:function(f){this.__Nj.pool(f);
}},destruct:function(){this._disposeObjects(bq,W);
}});
qx.Class.define(bo,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bK){demobrowser.demo.virtual.AbstractGallery.call(this,bK);
var bL=qx.theme.manager.Font.getInstance().resolve(Q).getStyles();
this._fontCss=qx.bom.element.Style.compile(bL);
...
var x=new qx.ui.virtual.layer.GridLines(bk,U);
y.getPane().addLayer(x);
var x=new qx.ui.virtual.layer.GridLines(bi,U);
y.getPane().addLayer(x);
return y;
...
},getCellProperties:function(bF,bG){var bJ=this.getItemData(bF,bG);

if(!bJ){return ba;
}var bI=this.manager.isItemSelected({row:bF,column:bG});
var bH=bI?bA:ba;
return {style:[bp,bC,this._fontCss,bH].join(ba),content:[by,bJ.resolvedIcon,bx,be,bJ.label].join(ba)};
