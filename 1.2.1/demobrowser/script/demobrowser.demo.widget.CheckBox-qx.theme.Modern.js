...
r.add(u);
var q=new qx.ui.form.CheckBox(d);
var x=new qx.ui.form.CheckBox(f);
var v=new qx.ui.form.CheckBox(c);
var w=new qx.ui.form.CheckBox(j);
this._checkBoxes=[q,x,v,w];
r.add(q);
r.add(x);
r.add(v);
r.add(w);
var t=new qx.ui.form.Button(g);
t.addListener(a,this._onExecute,this);
t.setAllowGrowX(false);
r.add(t);
},_onExecute:function(e){var o=this._checkBoxes;
var n=0;
var p=m;

for(var i=0;i<o.length;i++){if(o[i].getValue()){n++;
p+=(o[i].getLabel()+l);
}}
if(n>0){p=p.substring(0,p.length-2);
alert("You need these things for the beach: "+p);
