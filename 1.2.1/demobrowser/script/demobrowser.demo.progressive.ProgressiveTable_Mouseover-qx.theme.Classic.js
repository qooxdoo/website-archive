...
var J=0;
var F=function(u){var x=[];
var w=new Date().getTime();
var y=400*24*60*60*1000;

for(var z=0;z<u;z++){var v=new Date(w+Math.random()*y-y/2);
x.push({renderer:r,location:h,data:[J++,Math.random()*10000,v]});
}return x;
};
...
qx.Class.define(a,{extend:qx.ui.progressive.renderer.table.cell.Default,statics:{onmouseover:function(M){M.__NF=qx.bom.element.Style.get(M,"color");
M.style.color=q;
M.parentNode.style.height=32;
M.__NG=M.innerHTML;
var O=Date.parse(M.attributes.getNamedItem(p).nodeValue);
var N=new Date(O);
M.innerHTML=N.toUTCString()+i+N.toString();
},onmouseout:function(P){P.style.color=P.__NF;
P.parentNode.style.height=16;
P.innerHTML=P.__NG;
}},members:{_getCellExtras:function(s){var t=[];
t.push(qx.ui.progressive.renderer.table.cell.Default.prototype._getCellExtras.call(this));
t.push(k+s.cellData.toUTCString()+j);
t.push(e,g);
return t.join(o);
}}});
})();
