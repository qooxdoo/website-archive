...
(function(){var m="drop",k="value",j="items",h="move",g="multi",f="dragover",d="icon/16/places/folder.png",c="dragstart",b="Item ",a="copy",z="__Oz",y="drag",x="TextArea Target",w="solid",v="Reorderable",u="droprequest",t="#33508D",s="Source",r="demobrowser.demo.ui.DragDrop",q="Simple Target",o="Enable drag",p="\n",n="dragend";
qx.Class.define(r,{extend:qx.application.Standalone,members:{__Oz:null,__OA:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var V=this.getRoot();
...
P.add(N);
V.add(P,{edge:0});
var U=new qx.ui.basic.Label(s);
N.add(U,{left:20,top:20});
var S=new qx.ui.form.List;
S.setDraggable(true);
S.setSelectionMode(g);
N.add(S,{left:20,top:40});

for(var i=0;i<20;i++){S.add(new qx.ui.form.ListItem(b+i,d));
}var W=new qx.ui.form.CheckBox(o);
W.setValue(true);
N.add(W,{left:20,top:260});
S.addListener(c,function(e){if(!W.isValue()){e.preventDefault();
}e.addType(k);
e.addType(j);
e.addAction(a);
e.addAction(h);
});
S.addListener(u,function(e){this.debug("Related of droprequest: "+e.getRelatedTarget());
var G=e.getCurrentAction();
var I=e.getCurrentType();
var F;

switch(I){case j:F=this.getSelection();

if(G==a){var H=[];

for(var i=0,l=F.length;i<l;i++){H[i]=F[i].clone();
}F=H;
}break;
case k:F=this.getSelection()[0].getLabel();
break;
}if(G==h){var E=this.getSelection();

for(var i=0,l=E.length;i<l;i++){this.remove(E[i]);
}}e.addData(I,F);
});
var Q=new qx.ui.basic.Label(q);
N.add(Q,{left:140,top:20});
var O=new qx.ui.form.List;
O.setDroppable(true);
O.setSelectionMode(g);
N.add(O,{left:140,top:40});
O.addListener(m,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
var J=e.getData(j);

for(var i=0,l=J.length;i<l;i++){this.add(J[i]);
}});
O.addListener(f,function(e){if(!e.supportsType(j)){e.preventDefault();
}});
var Q=new qx.ui.basic.Label(x);
N.add(Q,{left:260,top:20});
var T=new qx.ui.form.TextArea;
T.setDroppable(true);
T.setHeight(100);
N.add(T,{left:260,top:40});
T.addListener(m,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
this.setValue(e.getData(k).replace(/,/g,p));
});
T.addListener(f,function(e){if(!e.supportsType(k)){e.preventDefault();
}});
var R=new qx.ui.basic.Label(v);
N.add(R,{left:500,top:20});
var L=this.__Oz=new qx.ui.form.List;
L.setDraggable(true);
L.setDroppable(true);
L.setSelectionMode(g);
N.add(L,{left:500,top:40});

for(var i=0;i<20;i++){L.add(new qx.ui.form.ListItem(b+i,d));
}var M=new qx.ui.core.Widget;
M.setDecorator(new qx.ui.decoration.Single().set({top:[1,w,t]}));
M.setHeight(0);
M.setOpacity(0.5);
M.setZIndex(100);
...
M.setDroppable(true);
this.getRoot().add(M);
L.addListener(c,function(e){e.addAction(h);
});
L.addListener(n,function(e){M.setDomPosition(-1000,-1000);
});
L.addListener(y,function(e){var B=e.getOriginalTarget();
if(B instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__OA=B;
}
...
L.addListener(f,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
L.addListener(m,function(e){this.__OB(e.getOriginalTarget());
},this);
M.addListener(m,function(e){this.__OB(this.__OA);
},this);
},__OB:function(C){if(C.classname!="qx.ui.form.ListItem"){return;
}var D=this.__Oz.getSortedSelection();

for(var i=0,l=D.length;i<l;i++){this.__Oz.addBefore(D[i],C);
this.__Oz.addToSelection(D[i]);
