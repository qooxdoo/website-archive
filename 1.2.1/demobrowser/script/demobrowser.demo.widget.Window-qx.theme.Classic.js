...
this.createWindow2();
this.createWindow3();
},createWindow1:function(){var x=new qx.ui.window.Window(bf,R);
x.setLayout(new qx.ui.layout.VBox(10));
x.setShowStatusbar(true);
x.setStatus(bm);
x.open();
this.getRoot().add(x,{left:20,top:20});
x.addListener(U,function(e){this.debug("Moved to: "+e.getData().left+"x"+e.getData().top);
},this);
x.addListener(bg,function(e){this.debug("Resized to: "+e.getData().width+"x"+e.getData().height);
},this);
var w=new qx.ui.basic.Atom(bn,bF);
w.setRich(true);
x.add(w);
var v=new qx.ui.tabview.TabView;
x.add(v,{flex:1});
var A=new qx.ui.tabview.Page(M);
v.add(A);
var y=new qx.ui.tabview.Page(bd);
v.add(y);
var z=new qx.ui.tabview.Page(X);
v.add(z);
...
bK.add(bM);
},getModalWindow1:function(){var H=new qx.ui.window.Window(bi);
H.setLayout(new qx.ui.layout.VBox(10));
H.setModal(true);
H.moveTo(150,150);
this.getRoot().add(H);
var G=this.getModalWindow2();
var J=new qx.ui.form.Button(bw,R);
J.addListener(S,G.open,G);
H.add(J);
var I=new qx.ui.form.CheckBox(bk);
I.setValue(true);
H.add(I);
I.addListener(T,function(e){H.setModal(e.getData());
});
return H;
},getModalWindow2:function(){var C=new qx.ui.window.Window(bt);
C.setLayout(new qx.ui.layout.VBox(10));
C.setModal(true);
C.setShowClose(false);
C.moveTo(300,300);
this.getRoot().add(C);
var B=new qx.ui.basic.Atom(bl,bH);
C.add(B);
var E=new qx.ui.container.Composite;
E.setLayout(new qx.ui.layout.HBox(10,Q));
C.add(E);
var F=new qx.ui.form.Button(O,bv);
F.addListener(S,function(e){C.close();
});
E.add(F);
var D=new qx.ui.form.Button(bb,bI);
D.addListener(S,function(e){alert("Sorry, please click 'Yes'!");
});
E.add(D);
return C;
}}});
