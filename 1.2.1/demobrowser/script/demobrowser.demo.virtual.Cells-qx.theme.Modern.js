...
(function(){var O="",N="' ",M="<b>HTML</b>",L="px;",K="Juhu",J="testImageCell",I="qx.theme",H="</div>",G="<div ",F="testNumberCell",bj="class='",bi="width:",bh="testHtmlCell",bg="testDateCell",bf=">",be="height:",bd="testStringCell",bc="testBooleanCellCustomImage",bb="demobrowser.demo.virtual.Cells",ba="testBooleanCell",V="foobar!",W="qx.theme.Modern",T="icon/32/apps/utilities-notes.png",U="medium",R="main",S="icon/16/emotes/face-sad.png",P="icon/16/apps/internet-feed-reader.png",Q="icon/16/emotes/face-smile.png",X="This is a feed reader!",Y="style='";
qx.Class.define(bb,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);
var a=new qx.ui.layout.HBox(5);
this.topContainer=new qx.ui.container.Composite(a);
this.runTest(bd);
this.runTest(F);
this.runTest(bg);
this.runTest(bh);
if(qx.core.Setting.get(I)==W){this.runTest(J);
this.runTest(ba);
this.runTest(bc);
}this.getRoot().add(this.topContainer,{edge:5});
},runTest:function(q){this.setUp();
this[q]();
...
},testStringCell:function(){var g={value:[K,M,O,null],states:[{},{selected:1}]};
var f=new qx.ui.virtual.cell.String;
qx.util.Permutation.permute(g,function(z){var A=this.__Nl(f,z.value,z.states);
...
qx.util.Permutation.permute(bv,function(bk){if(bk.format){bw.setNumberFormat(bk.format);
}else{bw.resetNumberFormat();
}var bl=this.__Nl(bw,bk.value,bk.states);
this.container.add(bl);
},this);
},testDateCell:function(){var D={value:[new Date(),null],format:[null,new qx.util.format.DateFormat(qx.locale.Date.getDateFormat(U))],states:[{},{selected:1}]};
var E=new qx.ui.virtual.cell.Date();
qx.util.Permutation.permute(D,function(m){if(m.format){E.setDateFormat(m.format);
}else{E.resetDateFormat();
}var n=this.__Nl(E,m.value,m.states);
this.container.add(n);
},this);
},testHtmlCell:function(){var t={value:[K,M,O,null],states:[{},{selected:1}]};
var u=new qx.ui.virtual.cell.Html;
qx.util.Permutation.permute(t,function(b){var c=this.__Nl(u,b.value,b.states);
this.container.add(c);
},this);
},testImageCell:function(){var B={value:[{url:P,tooltip:X},{url:T,tooltip:V},null],states:[{},{selected:1}]};
var C=new qx.ui.virtual.cell.Image;
qx.util.Permutation.permute(B,function(o){var p=this.__Nl(C,o.value,o.states);
this.container.add(p);
},this);
},testBooleanCell:function(){var v={value:[true,false,null],states:[{},{selected:1}]};
var w=new qx.ui.virtual.cell.Boolean;
qx.util.Permutation.permute(v,function(x){var y=this.__Nl(w,x.value,x.states);
this.container.add(y);
},this);
},testBooleanCellCustomImage:function(){var r={value:[true,false,null],states:[{},{selected:1}]};
var s=new qx.ui.virtual.cell.Boolean;
s.setIconFalse(S);
s.setIconTrue(Q);
qx.util.Permutation.permute(r,function(d){var e=this.__Nl(s,d.value,d.states);
this.container.add(e);
},this);
},__Nl:function(bm,bn,bo){var bs=100;
var bq=30;
var bu=new qx.ui.embed.Html().set({width:bs,height:bq,decorator:bo.selected?null:R});
var bp=bm.getCellProperties(bn,bo);
var br=bp.insets;
var bt=[G,Y,this._getCellSizeStyle(bs,bq,br[0],br[1]),bp.style,N,bj,bp.classes,N,bp.attributes,bf,bp.content,H].join(O);
bu.setHtml(bt);
return bu;
},_getCellSizeStyle:function(h,i,j,k){var l=O;
...
(function(){var x="",w="px",v='" ',u="no-repeat",t='"',s='" style="',r='title="',q='<div style="',p='></div>',o="qx.ui.virtual.cell.AbstractImage",i="img",n="inline-block",l='/>',h="static",g="top",k='<img src="',j="abstract",m="string",f="-moz-inline-box";
qx.Class.define(o,{extend:qx.ui.virtual.cell.Cell,type:j,construct:function(){qx.ui.virtual.cell.Cell.call(this);
this._aliasManager=qx.util.AliasManager.getInstance();
},members:{__Fl:16,__Fm:16,_aliasManager:null,__Fn:function(a){var d=qx.util.ResourceManager.getInstance();
var c=qx.io.ImageLoader;
var b,e;
if(d.has(a)){b=d.getImageWidth(a),e=d.getImageHeight(a);
}else if(c.isLoaded(a)){b=c.getWidth(a);
e=c.getHeight(a);
}else{b=this.__Fl;
e=this.__Fm;
}return {width:b,height:e};
},__Fo:function(z){if(typeof (z)==m){z={url:z};
}var A=this._aliasManager.resolve(z.url||null);
var B;

if(z.width&&z.height){B={width:z.width,height:z.height};
}else{B=this.__Fn(A);
}return {width:B.width,height:B.height,url:A,tooltip:z.tooltip};
},_identifyImage:function(y){throw new Error("_identifyImage is abstract");
},getContent:function(C,D){if(C===null){return x;
}var content=x;
var F=this.__Fo(this._identifyImage(C));
var J=qx.bom.client.Engine.GECKO&&qx.bom.client.Engine.VERSION<1.9;
var G=F.tooltip?r+F.tooltip+t:x;
var E={width:F.width+w,height:F.height+w,display:J?f:n,verticalAlign:g,position:h};
var H=qx.bom.element.Decoration.getTagName(u,F.url);
var I=qx.bom.element.Decoration.getAttributes(F.url,u,E);
var K=qx.bom.element.Style.compile(I.style);

if(H===i){content=k+I.src+s+K+v;
content+=G+l;
}else{content=q+K+v;
content+=G+p;
}return content;
...
this.__Ha=qx.util.AliasManager.getInstance();
},properties:{appearance:{refine:true,init:a},iconTrue:{check:e,themeable:true,apply:b},iconFalse:{check:e,themeable:true,apply:d}},members:{__Hb:null,__Hc:null,__Ha:null,_applyIconTrue:function(f){this.__Hb=this.__Ha.resolve(f);
},_applyIconFalse:function(g){this.__Hc=this.__Ha.resolve(g);
},_identifyImage:function(h){return h==true?this.__Hb:this.__Hc;
}}});
})();
