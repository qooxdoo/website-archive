...
var u=new qx.ui.container.Composite(new qx.ui.layout.Grid(20)).set({padding:20});
scroll.add(u);
u.add(this.getCellAlignGrid(),{row:0,column:0});
u.add(this.getRowColumnAlignGrid(),{row:0,column:1});
u.add(this.getColumnAlignGrid(),{row:0,column:2});
u.add(this.getRowAlignGrid(),{row:1,column:0});
},getNewWidget:function(){var v=new qx.ui.core.Widget().set({decorator:m,backgroundColor:a,allowGrowX:false,allowGrowY:false,height:Math.round((Math.random()*30)+30),width:Math.round((Math.random()*30)+30)});
return v;
},getColumnAlignGrid:function(){var n=new qx.ui.container.Composite().set({decorator:m,backgroundColor:l,allowGrowX:false,allowGrowY:false});
var o=new qx.ui.layout.Grid();
o.setSpacing(3);
o.setColumnAlign(0,h,j);
o.setColumnAlign(1,f,e);
o.setColumnAlign(2,g,k);
n.setLayout(o);

for(var x=0;x<3;x++){for(var y=0;y<6;y++){n.add(this.getNewWidget(),{row:y,column:x});
}}return n;
},getRowAlignGrid:function(){var p=new qx.ui.container.Composite().set({decorator:m,backgroundColor:l,allowGrowX:false,allowGrowY:false});
...
},getCellAlignGrid:function(){var r=new qx.ui.container.Composite().set({decorator:m,backgroundColor:l,allowGrowX:false,allowGrowY:false});
var t=new qx.ui.layout.Grid();
t.setSpacing(3);
r.setLayout(t);

for(var x=0;x<5;x++){t.setColumnAlign(x,f,e);

for(var y=0;y<5;y++){r.add(this.getNewWidget(),{row:y,column:x});
}}var s=t.getCellWidget(0,0);
s.set({alignX:h,alignY:j});
s.set({backgroundColor:d,width:20,height:20});
var s=t.getCellWidget(4,4);
s.set({alignX:g,alignY:k});
s.set({backgroundColor:d,width:20,height:20});
return r;
