...
qx.Class.define(v,{extend:qx.application.Standalone,properties:{showMode:{init:m,check:[p,n,m],event:bf}},members:{__NA:null,__NB:null,__NC:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var ch=this.getRoot();
ch.add(this.createConfigurableList(),{left:20,top:20});
ch.add(this.createOneSelectionList(),{left:330,top:20});
ch.add(this.createAdditiveSelectionList(),{left:520,top:20});
},createConfigurableList:function(){var bO=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
var bQ=new qx.ui.basic.Label(G).set({font:h});
bO.add(bQ);
var bY=[];

for(var i=0;i<2500;i++){bY[i]={label:bH+i,icon:(i%4)?O:A};
}var cc=qx.data.marshal.Json.createModel(bY);
var bT=this.__NA=new qx.ui.list.List(cc).set({scrollbarX:"on",selectionMode:"multi",height:280,width:150,labelPath:"label",iconPath:"icon",iconOptions:{converter:function(bJ){return "icon/"+bJ+"/places/folder.png";
}}});
var bS=this;
var cf={configureItem:function(cg){bS.bind(bz,cg,T);
},bindItem:function(cx,cy,cz){cx.bindDefaultProperties(cy,cz);
cx.bindProperty(p,bv,{converter:function(cw){return parseInt(cw.replace(/Item No /g,bk))%9!=0;
}},cy,cz);
}};
bT.setDelegate(cf);
bO.add(bT,{top:20});
bT.getSelection().push(cc.getItem(20));
bT.getSelection().addListener(bu,function(e){var bL=bT.getSelection();

for(var i=0;i<bL.getLength();i++){this.debug("Selection: "+bL.getItem(i).getLabel());
}},this);
var bW=bT.getPane().getRowConfig();

for(var i=0;i<2500;i++){if(i%4==0){bW.setItemSize(i,56);
}else{bW.setItemSize(i,25);
}}var bX=new qx.ui.form.RadioButton(bj);
var bN=new qx.ui.form.RadioButton(Q);
var cb=new qx.ui.form.RadioButton(w);
var cd=new qx.ui.form.RadioButton(br);
bX.setUserData(q,k);
bN.setUserData(q,l);
cb.setUserData(q,j);
cd.setUserData(q,o);
bO.add(bX,{left:160,top:20});
bO.add(bN,{left:160,top:40});
bO.add(cb,{left:160,top:60});
bO.add(cd,{left:160,top:80});
var bU=new qx.ui.form.RadioButton(f);
var bP=new qx.ui.form.RadioButton(bD);
var bM=new qx.ui.form.RadioButton(H);
bU.setUserData(q,p);
bP.setUserData(q,n);
bM.setUserData(q,m);
bO.add(bU,{left:160,top:120});
bO.add(bP,{left:160,top:140});
bO.add(bM,{left:160,top:160});
var bV=this.__NB=new qx.ui.form.CheckBox("Enable drag selection");
var ca=this.__NC=new qx.ui.form.CheckBox("Enable quick selection").set({enabled:false});
bO.add(bV,{left:160,top:200});
bO.add(ca,{left:160,top:220});
var ce=new qx.ui.form.RadioGroup(bX,bN,cb,cd);
ce.setSelection([bN]);
ce.addListener(bI,this.onSelectionModeChange,this);
var bR=new qx.ui.form.RadioGroup(bU,bP,bM);
bR.setSelection([bM]);
bR.addListener(bI,this.onShowModeChange,this);
bV.addListener(g,this.onDragCheckChange,this);
ca.addListener(g,this.onQuickCheckChange,this);
return bO;
},createOneSelectionList:function(){var ci=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
var cj=new qx.ui.basic.Label(U).set({font:h});
ci.add(cj);
var cm=new qx.data.Array();

for(var i=0;i<250;i++){var cl=new demobrowser.demo.virtual.model.Item(bH+i,bi);
cm.push(cl);
}var ck=new qx.ui.list.List(cm).set({selectionMode:o,height:280,width:150,labelPath:p,iconPath:n});
ci.add(ck,{top:20});
ck.getSelection().push(cm.getItem(16));
return ci;
},createAdditiveSelectionList:function(){var co=new qx.ui.container.Composite(new qx.ui.layout.Canvas());
var cp=new qx.ui.basic.Label(S).set({font:h});
co.add(cp);
var cq=[r,bh,bg,z,bl,x,X,bp,bb,bF,bs,E,Y,W,J,C,s,I,K,M,bE,d,bq,t,a,bm,bo,bc,V,be,R,u,F,bx,bd,D,bw,c,bC,L,b,P,bG,B,bt,ba,by,bn,bB,y,bA];
var ct=new qx.data.Array();
var cn=new qx.data.Array();

for(var i=0;i<cq.length;i++){var cs=new demobrowser.demo.virtual.model.Item(cq[i]);
ct.push(cs);

if(i==10||i==12||i==16){cn.push(cs);
}}var cr=new qx.ui.list.List(ct).set({selectionMode:j,height:200,width:150,labelPath:p,iconPath:n});
co.add(cr,{top:20});
cr.setSelection(cn);
return co;
},onSelectionModeChange:function(e){var cv=e.getData()[0].getUserData(q);
this.__NA.setSelectionMode(cv);

if(cv==k||cv==o){this.__NB.setEnabled(false);
this.__NC.setEnabled(true);
}else if(cv==l||cv==N){this.__NB.setEnabled(true);
this.__NC.setEnabled(false);
}},onShowModeChange:function(e){var cu=e.getData()[0].getUserData(q);
this.setShowMode(cu);
},onDragCheckChange:function(e){if(e.getData()){var bK=this.__NA.getSelectionMode();

if(bK==k||bK==o){this.debug("Drag selection is only available for the modes multi or additive");
}}this.__NA.setDragSelection(e.getData());
},onQuickCheckChange:function(e){if(e.getData()){var cA=this.__NA.getSelectionMode();

if(cA==l||cA==j){this.debug("Quick selection is only available for the modes multi or additive");
}}this.__NA.setQuickSelection(e.getData());
}}});
})();
