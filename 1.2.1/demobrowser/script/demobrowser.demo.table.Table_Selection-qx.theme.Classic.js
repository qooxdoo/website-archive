...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.table.Table_Selection","qx.theme":"qx.theme.Classic"};
...
(function(){var m="execute",l="value",k="",j="A date",i="Boolean",h="demobrowser.demo.table.Table_Selection",g="changeValue",f="icon/22/status/dialog-information.png",d="A number",c="icon/16/apps/office-calendar.png",B="Reset Cell Focus",A="changeSelection",z="middle",y="Remove 5 rows",x="Reset Selection",w="_tableModel",v="Table",u="Add 5 rows",t="icon/22/actions/list-remove.png",s=", ",q="icon/22/actions/list-add.png",r="Hide Cell Focus",o="Selected rows:<br>",p="Show selection",n="ID";
qx.Class.define(h,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return v;
},createTable:function(){var Q=this.createRandomRows(50);
var P=this._tableModel=new qx.ui.table.model.Simple();
P.setColumns([n,d,j,i]);
P.setData(Q);
P.setColumnEditable(1,true);
P.setColumnEditable(2,true);
P.setColumnSortable(3,false);
var R=new qx.ui.table.Table(P);
R.set({width:600,height:400});
R.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var O=R.getTableColumnModel();
O.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
O.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(c,j));
return R;
},createControls:function(){var D=new qx.ui.toolbar.ToolBar();
var F,H;
H=new qx.ui.toolbar.Part();
D.add(H);
var C=new qx.ui.form.SelectBox().set({allowGrowY:false,alignY:z});
var K=qx.ui.table.selection.Model;
var E={"Single selection":K.SINGLE_SELECTION,"Single interval":K.SINGLE_INTERVAL_SELECTION,"Multiple intervals":K.MULTIPLE_INTERVAL_SELECTION,"Multiple intervals (toggle)":K.MULTIPLE_INTERVAL_SELECTION_TOGGLE,"No selection":K.NO_SELECTION};

for(var J in E){var G=new qx.ui.form.ListItem(J);
G.setUserData(l,E[J]+k);
C.add(G);
}C.addListener(A,function(e){this._table.getSelectionModel().setSelectionMode(parseInt(e.getData()[0].getUserData(l)));
},this);
H.add(C);
H=new qx.ui.toolbar.Part();
D.add(H);
F=new qx.ui.toolbar.Button(u,q);
F.addListener(m,function(U){var V=this.createRandomRows(10);
this._tableModel.addRows(V);
this.info("5 rows added");
},this);
H.add(F);
F=new qx.ui.toolbar.Button(y,t);
F.addListener(m,function(a){var b=this._tableModel.getRowCount();
this._tableModel.removeRows(b-5,5);
this.info("5 rows removed");
},this);
H.add(F);
H=new qx.ui.toolbar.Part();
D.add(H);
F=new qx.ui.toolbar.Button(x);
F.addListener(m,function(L){this._table.resetSelection();
},this);
H.add(F);
F=new qx.ui.toolbar.Button(B);
F.addListener(m,function(W){this._table.resetCellFocus();
},this);
H.add(F);
var I=new qx.ui.toolbar.CheckBox(r);
I.addListener(g,function(M){this._table.setShowCellFocusIndicator(!I.isValue());
},this);
H.add(I);
F=new qx.ui.toolbar.Button(p,f);
F.addListener(m,function(S){var T=[];
this._table.getSelectionModel().iterateSelection(function(N){T.push(N+k);
});
this.showDialog(o+T.join(s));
},this);
H.add(F);
return D;
