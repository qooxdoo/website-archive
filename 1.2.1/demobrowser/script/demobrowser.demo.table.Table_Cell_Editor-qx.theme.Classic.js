...
},createTable:function(){var bH=[[J,k,x,{'required':true}],[d,K,S,{'type':d,'required':true}],[L,A,T,{'options':[U,N,G],'editable':true}],[Q,v,a,{'options':[[n,null,a],[D,null,q],[w,null,E]]}],[e,z,y,{'type':e}],[l,O,M,{'regExp':W,'failMsg':Y}],[I,t,H,{'validationFunc':function(bI){if(bI.search(/^http:/)!=-1){window.open(bI);
}return bI;
}}],[V,m,true,{'type':j}]];
var bG=function(bb){var bh=bb.table;
var be=bh.getTableModel();
var bf=be.getRowData(bb.row);
var bi=bf[3];
var bg;

for(var bd in bi){switch(bd){case f:switch(bi[b]){case j:return new qx.ui.table.cellrenderer.Boolean;
case g:return new qx.ui.table.cellrenderer.Password;
}break;
case h:var bg=new qx.ui.table.cellrenderer.Replace;
var bc={};
bi[c].forEach(function(br){if(br instanceof Array){bc[br[0]]=br[2];
}});
bg.setReplaceMap(bc);
bg.addReversedReplaceMap();
return bg;
}}return new qx.ui.table.cellrenderer.Default();
};
var bF=new qx.ui.table.cellrenderer.Dynamic(bG);
var bD=function(bj){var bo=bj.table;
var bm=bo.getTableModel();
var bn=bm.getRowData(bj.row);
var bq=bn[3];
var bl=new qx.ui.table.celleditor.TextField;
var bp=null;

for(var bk in bq){switch(bk){case h:if(bq.editable){bl=new qx.ui.table.celleditor.ComboBox();
}else{bl=new qx.ui.table.celleditor.SelectBox();
}bl.setListData(bq[c]);
break;
case F:break;
case f:switch(bq[b]){case g:bl=new qx.ui.table.celleditor.PasswordField;
break;
case j:bl=new qx.ui.table.celleditor.CheckBox;
break;
case s:bl.setValidationFunction(function(bL,bM){var bN=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{2}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/;

if(bN.test(bL)){return bL;
}alert("You did not enter a valid email address");
return bM;
});
break;
}break;
case r:bl.setValidationFunction(function(bs,bt){var bu=new RegExp(bq[X]);
...
break;
case P:bl.setValidationFunction(bq[B]);
break;
case o:bp=function(bJ,bK){if(!bJ){alert("You need to supply a value here");
return bK;
}return bJ;
};
break;
...
var bC=new qx.ui.table.celleditor.Dynamic(bD);
var bA=new qx.ui.table.model.Simple();
bA.setColumns([i,i,p]);
var bz={tableColumnModel:function(ba){return new qx.ui.table.columnmodel.Resize(ba);
}};
var bB=new qx.ui.table.Table(bA,bz);
bB.setDecorator(null);
bB.setColumnVisibilityButtonVisible(false);
bB.setKeepFirstVisibleRowComplete(true);
bB.setStatusBarVisible(false);
bB.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var bE=bB.getTableColumnModel();
bE.setColumnVisible(0,false);
bE.getBehavior().setWidth(1,100);
bE.getBehavior().setWidth(2,300);
bA.setColumnEditable(2,true);
bE.setDataCellRenderer(2,bF);
bE.setCellEditorFactory(2,bC);
bB.getTableModel().setData(bH);
bB.getTableModel().addListener(u,function(event){if(!(event instanceof qx.event.type.Data)){return;
}var by=event.getData();
var bv=this.getTableModel();
var bx=bv.getValue(0,by.firstRow);
var bw=bv.getValue(by.firstColumn,by.firstRow);
this.info("User edited property '"+bx+"' and entered value '"+bw+"'.");
},bB);
return bB;
}}});
})();
...
}},properties:{cellEditorFactoryFunction:{check:b,nullable:true,init:null}},members:{__Kg:null,createCellEditor:function(g){var h=this.getCellEditorFactoryFunction();
{};
this.__Kg=h(g);
var i=this.__Kg.createCellEditor(g);
return i;
},getCellEditorValue:function(d){var e=this.getCellEditorFactoryFunction();
{};
var f=this.__Kg.getCellEditorValue(d);
return f;
