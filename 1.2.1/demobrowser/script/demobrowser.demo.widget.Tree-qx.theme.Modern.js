...
bm.set({spacing:20});
var bl=new qx.ui.container.Composite(bm);
this.getRoot().add(bl);
var bj=this.getTree();
bl.add(bj);
var bk=this.getCommandFrame(bj);
bl.add(bk);
},getTree:function(){var bw=new qx.ui.tree.Tree().set({width:200,height:400});
var bB=new qx.ui.tree.TreeFolder(N);
bB.setOpen(true);
bw.setRoot(bB);
var bz=new qx.ui.tree.TreeFolder(O);
bz.setOpen(true);
bB.add(bz);
var br=new qx.ui.tree.TreeFolder(F);
var bp=new qx.ui.tree.TreeFolder(q);
var bq=new qx.ui.tree.TreeFolder(J);
var bo=new qx.ui.tree.TreeFolder(j);
bz.add(br,bp,bq,bo);
var bx=new qx.ui.tree.TreeFile(c);
var bn=new qx.ui.tree.TreeFile(b);
bp.add(bx,bn);
var bA=new qx.ui.tree.TreeFolder(a);
var bs=new qx.ui.tree.TreeFolder(L);
var bv=new qx.ui.tree.TreeFolder(G);
var bu=new qx.ui.tree.TreeFolder(j);

for(var i=0;i<30;i++){bu.add(new qx.ui.tree.TreeFile(p+i));
}var by=new qx.ui.tree.TreeFolder(n);
var bt=new qx.ui.tree.TreeFolder(E);
bA.add(bs,bv,bu,by,bt);
bB.add(bA);
return bw;
},getCommandFrame:function(P){var ba=new qx.ui.groupbox.GroupBox(I);
var X=4;
ba.setLayout(new qx.ui.layout.Grid(5,3));
var bh=0;
ba.add(new qx.ui.basic.Label(o).set({paddingTop:4}),{row:bh,column:0});
var be=new qx.ui.form.TextField().set({readOnly:true});
ba.add(be,{row:bh++,column:1});
P.addListener(h,function(e){var bD=e.getData();

if(bD.length>0){if(this.getSelectionMode()===f){be.setValue(bD.length+H);
}else{be.setValue(bD[0].getLabel());
}}else{be.setValue(t);
}});
ba.add(new qx.ui.core.Spacer(X,X),{row:bh++,column:0});
ba.add(new qx.ui.basic.Label(M),{row:bh,column:0});
var T={"click":u,"dblclick":A,"none":y};
var R=new qx.ui.form.RadioGroup();

for(var bc in T){var S=new qx.ui.form.RadioButton(T[bc]).set({value:bc==P.getOpenMode()});
S.setUserData(g,bc);
R.add(S);
ba.add(S,{row:bh++,column:1});
}R.addListener(h,function(e){P.setOpenMode(e.getData()[0].getUserData(g));
});
ba.add(new qx.ui.core.Spacer(X,X),{row:bh++,column:0});
ba.add(new qx.ui.basic.Label(z),{row:bh,column:0});
var V=new qx.ui.form.CheckBox(x);
ba.add(V,{row:bh++,column:1});
V.addListener(l,function(e){var bC=e.getData();
P.setSelectionMode(bC?f:d);
});
var W=new qx.ui.form.CheckBox(r);
ba.add(W,{row:bh++,column:1});
W.addListener(l,function(e){var bi=e.getData();
P.setDragSelection(bi);

if(!V.getValue()){V.setValue(true);
}});
ba.add(new qx.ui.core.Spacer(X,X),{row:bh++,column:0});
ba.add(new qx.ui.basic.Label(B),{row:bh,column:0});
var bf=new qx.ui.form.CheckBox(K);
ba.add(bf,{row:bh++,column:1});
bf.addListener(l,function(e){P.setHideRoot(e.getData());
});
var U=new qx.ui.form.CheckBox(v);
ba.add(U,{row:bh++,column:1});
U.addListener(l,function(e){P.setRootOpenClose(e.getData());
});
ba.add(new qx.ui.core.Spacer(X,X),{row:bh++,column:0});
var bd=new qx.ui.form.Button(D);
ba.add(bd,{row:bh++,column:1});
bd.addListener(k,function(e){alert((""+P.getItems()).replace(",","\n","g"));
});
var Q=new qx.ui.form.Button(s);
ba.add(Q,{row:bh++,column:1});
Q.addListener(k,function(e){alert((""+P.getItems(true,false)).replace(",","\n","g"));
});
var bb=new qx.ui.form.Button(C);
ba.add(bb,{row:bh++,column:1});
bb.addListener(k,function(e){if(this.getSelectionMode()===d){alert(this.getSelection());
}else{alert(this.getSelection().toString().replace(",","\n","g"));
}},P);
var Y=new qx.ui.form.Button(w);
ba.add(Y,{row:bh++,column:1});
var bg=true;
Y.addListener(k,function(e){P.setHeight(bg?600:400);
bg=!bg;
});
return ba;
}}});
