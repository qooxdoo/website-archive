...
(function(){var l="value",k="icon",j="model",i="text",h="qx.client",g="execute",f="source",e="wittemann",d="mshtml",c=".json",H="user.profile_image_url",G="http://twitter.com/statuses/user_timeline/",F="Enter",E="<span style='font-size: 20px'>Twitter</span>",D="<a target='_blank' href='http://twitter.com/home?status=Check%20out%20the%20%23qooxdoo%20twitter%20demo%20at%20http://bit.ly/Mc4Of'>Tweet This</a></span>",C="demobrowser.demo.data.store.Twitter",B="keydown",A="Friends",z="demobrowser.demo.data.Twitter",y="selection[0]",s="<span style='font-size: 13px'>",t="Location: ",q="Details",r="Name: ",o="?callback=demobrowser.demo.data.store.Twitter.saveResult",p="User",m="http://twitter.com/statuses/friends_timeline.json",n="Postet with: ",u="user.name",v="user.location",x="Avatar: ",w="Message: ";
...
this.getRoot().add(U,{left:10,top:85});
U.setWidth(300);
var W=new qx.data.controller.List(null,U);
W.setDelegate(this);
W.setLabelPath(i);
if(!qx.core.Variant.isSet(h,d)){W.setIconPath(H);
}var bb=new demobrowser.demo.data.store.Twitter(e);
bb.bind(j,W,j);
var S=new qx.ui.form.Button(A);
this.getRoot().add(S,{left:10,top:55});
S.addListener(g,function(){bb.setUrl(m);
},this);
var X=new qx.ui.form.Button(p);
this.getRoot().add(X,{left:90,top:55});
var P=new qx.ui.form.TextField(e);
this.getRoot().add(P,{left:140,top:55});
X.addListener(g,function(){var J=G+P.getValue()+c;

if(bb.getUrl()==J){bb.reload();
}else{bb.setUrl(J);
}},this);
P.addListener(B,function(I){if(I.getKeyIdentifier()==F){X.execute();
}},this);
var T=new qx.ui.groupbox.GroupBox(q);
this.getRoot().add(T,{left:320,top:60});
T.setWidth(300);
T.setHeight(220);
T.setLayout(new qx.ui.layout.Grid(0,5));
T.add(new qx.ui.basic.Label(r),{row:0,column:0});
T.add(new qx.ui.basic.Label(t),{row:1,column:0});
T.add(new qx.ui.basic.Label(w),{row:2,column:0});
T.add(new qx.ui.basic.Label(n),{row:3,column:0});
...
Y.setRich(true);
Y.setWidth(150);
T.add(Y,{row:2,column:1});
var bc=new qx.ui.basic.Label();
bc.setRich(true);
T.add(bc,{row:3,column:1});
var Q=new qx.data.controller.Object();
Q.addTarget(name,l,u);
Q.addTarget(location,l,v);
Q.addTarget(Y,l,i);
Q.addTarget(bc,l,f);

if(!qx.core.Variant.isSet(h,d)){T.add(new qx.ui.basic.Label(x),{row:4,column:0});
var V=new qx.ui.basic.Image();
T.add(V,{row:4,column:1});
Q.addTarget(V,f,H);
}W.bind(y,Q,j);
...
ba.setWidth(260);
ba.setValue(s+D);
this.getRoot().add(ba,{left:10,top:290});
},configureItem:function(L){L.setRich(true);
L.getChildControl(k).setWidth(50);
L.getChildControl(k).setHeight(50);
L.getChildControl(k).setScale(true);
}}});
qx.Class.define(C,{extend:qx.data.store.Json,statics:{saveResult:function(K){this.__NQ=K;
}},construct:function(N){var O=G+N+c;
qx.data.store.Json.call(this,O);
},members:{_createRequest:function(a){var b=new qx.io.ScriptLoader();
a+=o;
b.load(a,function(M){this.__NR();
},this);
},__NR:function(){var bd=demobrowser.demo.data.store.Twitter.__NQ;

if(bd==undefined){this.setState("failed");
return;
}this._marshaler.toClass(bd);
this.setModel(this._marshaler.toModel(bd));
