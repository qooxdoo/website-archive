...
T.setColumns([s,a,f,B]);
T.setData(U);
var V=new qx.ui.table.Table(T);
V.set({width:600,height:400,decorator:null});
var S=V.getTableColumnModel();
S.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
V.setContextMenuHandler(3,this._contextMenuHandlerBoolean);
V.setContextMenuHandler(2,this._contextMenuHandlerNull);
return V;
},_contextMenuHandlerBoolean:function(W,X,Y,ba,bb){var bc;

for(var i=0;i<=1;i++){bc=new qx.ui.menu.Button(i?c:o);
bc.setUserData(j,i?true:false);
bc.addListener(l,function(e){ba.setValue(W,X,this.getUserData(j));
});
bb.add(bc);
}return true;
},_contextMenuHandlerNull:function(bf,bg,bh,bi,bj){return true;
},createControls:function(){var M=new qx.ui.toolbar.ToolBar();
var K,L,N;
L=new qx.ui.toolbar.Part();
M.add(L);
K=new qx.ui.toolbar.Button(b,h);
K.addListener(l,function(bd){var be=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,be[0][i]);
}this.info("Row 10 changed");
},this);
L.add(K);
K=new qx.ui.toolbar.Button(C,t);
K.addListener(l,function(bo){var bp=this.createRandomRows(10);
this._tableModel.addRows(bp);
this.info("10 rows added");
},this);
L.add(K);
K=new qx.ui.toolbar.Button(D,x);
K.addListener(l,function(bk){var bl=this._tableModel.getRowCount();
this._tableModel.removeRows(bl-5,5);
this.info("5 rows removed");
},this);
L.add(K);
K=new qx.ui.toolbar.Button(m,d);
K.addListener(l,function(bm){var bn=[];
J.getSelectionModel().iterateSelection(function(bt){bn.push(bt+A);
});
this.showDialog(q+bn.join(v));
},this);
L.add(K);
L=new qx.ui.toolbar.Part();
M.add(L);
var J=this._table;
N=new qx.ui.toolbar.CheckBox(r);
N.set({value:J.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(w+p)});
N.addListener(k,function(E){J.setKeepFirstVisibleRowComplete(this.getValue());
},N);
L.add(N);
N=new qx.ui.toolbar.CheckBox(g);
N.set({value:J.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(n)});
N.addListener(k,function(bq){if(bq.getData()){var bs=function(F,G){var H=F[arguments.callee.columnIndex];
var I=G[arguments.callee.columnIndex];

if(H%2==1&&I%2==0){return 1;
}
if(I%2==1&&H%2==0){return -1;
}return (H>I)?1:((H==I)?0:-1);
};
var br=function(O,P){var Q=O[arguments.callee.columnIndex];
var R=P[arguments.callee.columnIndex];

if(Q%2==1&&R%2==0){return -1;
}
if(R%2==1&&Q%2==0){return 1;
}return (Q<R)?1:((Q==R)?0:-1);
};
J.getTableModel().setSortMethods(0,{ascending:bs,descending:br});
...
this.__Hl=[];
},members:{__Hl:null,setContextMenuHandler:function(d,f){this.__Hl[d]=f;
},getContextMenuHandler:function(g){return this.__Hl[g];
