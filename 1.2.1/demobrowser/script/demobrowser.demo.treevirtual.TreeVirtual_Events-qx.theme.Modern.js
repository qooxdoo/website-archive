...
var B=new qx.ui.treevirtual.TreeVirtual(l);
B.setColumnWidth(0,400);
B.setAlwaysShowOpenCloseSymbol(true);
this.getRoot().add(B,{edge:30});
var G=B.getDataModel();
var D=G.addBranch(null,b,true);
var F;
G.addBranch(D,a,true);
F=G.addBranch(D,n,true);
G.addLeaf(F,v);
G.addLeaf(F,t);
G.addBranch(D,f,true);
G.addBranch(D,w,true);
var E=G.addBranch(null,c,true);
F=G.addBranch(E,d,false);

for(var i=1;i<3000;i++){G.addLeaf(F,m+i);
}G.addBranch(E,g,false);
G.addBranch(E,w,false);
G.addBranch(E,u,false);
G.addBranch(E,o,false);
G.setData();
var C=1;
B.addListener(r,function(e){alert('treeOpenWithContent ');
var y=e.getData();
G.addLeaf(y.nodeId,C.toString());
C++;
});
B.addListener(k,function(e){alert('treeClose ');
});
B.addListener(q,function(e){alert('treeOpenWhileEmpty');
var x=e.getData();
B.nodeSetHideOpenClose(x,true);
});
B.addListener(h,function(e){var z=j;
var A=e.getData();

for(i=0;i<A.length;i++){z+=s+A[i].label;
}alert('changeSelection: '+z);
});
}}});
})();
