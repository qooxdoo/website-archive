...
qx.Class.define(o,{extend:qx.application.Standalone,members:{__OC:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var w=this.getRoot();
this.__OC=["bottom-left","bottom-right","top-left","top-right","right-top","right-bottom","left-top","left-bottom"];
w.add(this.createTestField(g),{left:20,top:20});
w.add(this.createTestField(j),{right:20,top:20});
w.add(this.createTestField(k),{right:20,bottom:20});
w.add(this.createTestField(f),{left:20,bottom:20});
w.add(this.createTestField(h),{left:20,top:250});
w.add(this.createTestField(c),{right:20,top:250});
w.add(this.createDomClick(),{left:200,top:100});
},createTestField:function(x){var B=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));
var z=new qx.ui.popup.Popup(new qx.ui.layout.Grow);
z.add(new qx.ui.basic.Atom(l));
z.setPadding(20);
var y=new qx.ui.form.SelectBox;
var C=null;

for(var i=0;i<this.__OC.length;i++){var A=new qx.ui.form.ListItem(this.__OC[i]);
y.add(A);

if(this.__OC[i]==x){C=A;
}}B.add(y);
y.addListener(p,function(e){z.setPosition(e.getData()[0].getLabel());
});
y.setSelection([C]);
var D=new qx.ui.form.Button(m);
B.add(D);
D.addListener(q,function(e){z.placeToWidget(D);
z.show();
});
return B;
},createDomClick:function(){var t=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));
var v=new qx.ui.basic.Label(a);
t.add(v);
var s=new qx.ui.popup.Popup(new qx.ui.layout.Grow);
s.add(new qx.ui.basic.Atom(n));
s.setPadding(20);
var r=new qx.ui.form.SelectBox;

for(var i=0;i<this.__OC.length;i++){r.add(new qx.ui.form.ListItem(this.__OC[i]));
}t.add(r);
r.addListener(p,function(e){s.setPosition(e.getData()[0].getLabel());
});
var u=new qx.ui.form.Button(d);
t.add(u);
u.addListener(q,function(e){s.placeToElement(document.getElementById(b));
s.show();
});
return t;
