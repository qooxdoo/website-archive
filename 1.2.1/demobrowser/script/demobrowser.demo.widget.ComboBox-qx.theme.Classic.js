...
(function(){var w="bold",v=" = ",u="... &gt; (as literal HTML entity)",t="... &gt; (as Richtext)",s="HTML entities: &laquo; &lt; &amp; &gt; &raquo;",r="Wide, long list",q="<u>Underlined Text</u>",p="<b>Bold Text</b>",o="'s Icon",n="With icons",g="Default",m="changeValue",k="2^ ",f="<i>Italic Text</i>",d="'s Item",j="icon/16/places/folder.png",h="With HTML (rich) text",l="demobrowser.demo.widget.ComboBox";
...
},_createDefaultExample:function(){var G=new qx.ui.basic.Label(g);
G.setFont(w);
this.getRoot().add(G,{left:20,top:25});
var E=new qx.ui.form.ComboBox();
for(var i=1;i<31;i++){var F=new qx.ui.form.ListItem(k+i+v+Math.pow(2,i));
E.add(F);
}E.addListener(m,function(e){this.debug("ChangeValue: "+e.getData());
});
this.getRoot().add(E,{left:20,top:40});
},_createIconExample:function(){var D=new qx.ui.basic.Label(n);
D.setFont(w);
this.getRoot().add(D,{left:160,top:25});
var B=new qx.ui.form.ComboBox();
for(var i=1;i<31;i++){var C=new qx.ui.form.ListItem(i+o,j);
B.add(C);
}this.getRoot().add(B,{left:160,top:40});
},_createHtmlExample:function(){var A=new qx.ui.basic.Label(h);
A.setFont(w);
this.getRoot().add(A,{left:300,top:25});
var x=new qx.ui.form.ComboBox().set({width:200});
var y=[u,t,p,q,f,s];
for(var i=0;i<y.length;i++){var z=new qx.ui.form.ListItem(y[i]);

if(i>0){z.setRich(true);
}x.add(z);
}this.getRoot().add(x,{left:300,top:40});
},_createWideExample:function(){var c=new qx.ui.basic.Label(r);
c.setFont(w);
this.getRoot().add(c,{left:20,top:285});
var a=new qx.ui.form.ComboBox();
a.setWidth(300);
for(var i=1;i<101;i++){var b=new qx.ui.form.ListItem(i+d);
a.add(b);
