...
qx.Class.define(D,{extend:qx.application.Standalone,members:{__Nr:null,messenger:null,__Ns:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var W=this.getRoot();
var ba=new qx.ui.window.Window(G).set({contentPadding:0,showClose:false,showMinimize:false});
...
var X=demobrowser.demo.virtual.messenger.BuddyModel.createBuddies(200);
this.messenger.setModel(X);
ba.add(this.createToolbar());
ba.add(this.messenger,{flex:1});
W.add(this.createDetailsView(),{left:20,top:20});
},createToolbar:function(){var toolbar=new qx.ui.toolbar.ToolBar();
var bg=new qx.ui.toolbar.Part();
toolbar.add(bg);
var be=new qx.ui.toolbar.Button(j,F).set({show:d});
be.addListener(l,this.showAddContactWindow,this);
bg.add(be);
var bf=new qx.ui.toolbar.Button(j,q).set({show:d});
this.messenger.bind(U,bf,o,{converter:function(a){return !!a;
}});
bf.addListener(l,function(){this.messenger.getModel().remove(this.messenger.getSelection().getItem(0));
},this);
bg.add(bf);
return bg;
},createDetailsView:function(){var bu=new qx.data.controller.Object();
this.messenger.bind(U,bu,K);
var br=new qx.ui.groupbox.GroupBox(x);
var bv=new qx.ui.layout.Grid(5,5);
bv.setColumnAlign(0,V,k);
br.setLayout(bv);
br.add(new qx.ui.basic.Label(n),{row:0,column:0});
var bx=new qx.ui.form.TextField();
bu.addTarget(bx,g,u,true);
br.add(bx,{row:0,column:1});
br.add(new qx.ui.basic.Label(P),{row:1,column:0});
var bs=new qx.ui.form.TextField();
bu.addTarget(bs,g,m,true);
br.add(bs,{row:1,column:1});
br.add(new qx.ui.basic.Label(E),{row:2,column:0});
var bw=new qx.ui.form.SelectBox();
bw.add(new qx.ui.form.ListItem(f).set({model:f}));
bw.add(new qx.ui.form.ListItem(i).set({model:i}));
bw.add(new qx.ui.form.ListItem(c).set({model:c}));
bw.add(new qx.ui.form.ListItem(h).set({model:h}));
this.messenger.bind(b,bw,C,{converter:function(bd){return [bd];
}});
bw.bind(I,this.messenger,b,{converter:function(bh){return bh[0].getModel();
}});
br.add(bw,{row:2,column:1});
br.add(new qx.ui.basic.Label(B).set({alignY:A}),{row:3,column:0});
var bt=new qx.ui.basic.Image().set({alignX:r});
bu.addTarget(bt,s,y);
br.add(bt,{row:3,column:1});
return br;
},showAddContactWindow:function(){if(this.__Nr==null){var bi=new qx.ui.layout.Grid(5,10);
var bk=new qx.ui.window.Window(L,p);
bk.set({width:165,height:100,showMinimize:false,showClose:false,showMaximize:false});
bk.setLayout(bi);
var bn=new qx.ui.basic.Label(N);
var bo=new qx.ui.form.TextField();
var bl=new qx.ui.form.ComboBox(w);
bl.add(new qx.ui.form.ListItem(Q));
bl.add(new qx.ui.form.ListItem(O));
var bj=new qx.ui.basic.Label(t);
var bq=new qx.ui.form.Button(H);
var bp=new qx.ui.form.Button(R);
this.__Ns=bo;
bq.setAllowGrowX(false);
bp.setAllowGrowX(false);
bk.addListener(J,bo.focus,bo);
bp.addListener(l,bk.close,bk);
var bm=function(){var bc=new demobrowser.demo.virtual.messenger.BuddyModel();
bc.setName(bo.getValue());
bc.setGroup(bl.getValue());
this.messenger.getModel().push(bc);
this.messenger.getSelection().setItem(0,bc);
bk.close();
};
bq.addListener(l,bm,this);
bk.addListener(v,function(e){var bb=e.getKeyIdentifier();

if(bb==M){bm.call(this);
}else if(bb==S){bk.close();
}},this);
...
bi.setColumnAlign(0,z,k);
bi.setColumnAlign(1,V,k);
this.getRoot().add(bk,{left:20,top:20});
this.__Nr=bk;
}this.__Ns.setValue(j);
this.__Nr.center();
this.__Nr.open();
...
qx.Class.define(g,{extend:qx.ui.virtual.form.ListController,construct:function(y,z){qx.ui.virtual.form.ListController.call(this,y,z);
this.__Nt=new demobrowser.demo.virtual.messenger.BuddyCell();
this.__Nu=new demobrowser.demo.virtual.messenger.GroupCell();
this.__Nu.addListener(j,this._onChangeOpenGroup,this);
this.__Nv=[];
this.__Nw={};
this.__Nx=[];
this.__Ny=new qx.util.DeferredCall(this._updateGrouping,this);
},members:{__Nx:null,__Nw:null,__Ny:null,__Nv:null,__Nt:null,__Nu:null,_getRowData:function(n){return this.__Nx[n];
},_getModelRow:function(o){return this.__Nx.indexOf(o);
},_getRowCount:function(){return this.__Nx.length;
},_isGroupHeader:function(p){return this.__Nx[p] instanceof demobrowser.demo.virtual.messenger.GroupModel;
},_getGroupModel:function(name,t){var u=this.__Nw[name];

if(!u){var u=new demobrowser.demo.virtual.messenger.GroupModel();
u.setName(name);
this.__Nw[name]=u;
}u.setRow(t);
return u;
},_onChangeOpenGroup:function(e){var c=this.__Nx[e.getData().getUserData(d)];
c.toggleOpen();
this.__Ny.schedule();
},getCellRenderer:function(v){if(this._isGroupHeader(v)){return this.__Nu;
}else{return this.__Nt;
}},isRowSelectable:function(J){return !this._isGroupHeader(J);
},_visualizeGrouping:function(q){var r=this.getTarget();

if(r){for(var i=0;i<this.__Nv.length;i++){var s=this.__Nv[i].getOldRow();

if(s!==null){r.unstyleGroup(s);
}}
for(var i=0;i<q.length;i++){var s=q[i].getRow();
r.styleGroup(s);
}}this.__Nv=q;
...
H.sort(function(a,b){var w=a.getGroup();
var x=b.getGroup();

if(w==x){return a.getName()<b.getName()?-1:1;
}return w>x?-1:1;
});
var F=H[0];
...
I=0;
var C=this._getGroupModel(E.getGroup(),this.__Nx.length);
this.__Nx.push(C);
D.push(C);
}
if(C.isOpen()){this.__Nx.push(E);
...
this._syncModelSelectionToView();
this._syncRowCount();
},_applyModel:function(l,m){qx.ui.virtual.form.ListController.prototype._applyModel.call(this,l,m);
this.__Ny.schedule();
},_onChangeModel:function(e){qx.ui.virtual.form.ListController.prototype._onChangeModel.call(this,e);
this.__Ny.schedule();
},_onChangeBubbleModel:function(e){var A=e.getData();

if(A.name){var B=A.name.toString().split(f)[1];

if(B==k||B==h){this.__Ny.schedule();
...
}},properties:{name:{init:c,event:g,check:a},row:{event:d,check:j,apply:b},open:{check:h,init:true,event:i},itemCount:{check:j,init:0,event:e}},members:{__Nz:null,_applyRow:function(k,l){this.__Nz=l;
},getOldRow:function(){return this.__Nz;
}}});
})();
