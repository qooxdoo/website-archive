...
(function(){var n="ul",m="li",l="select",k="input",j="h2",i="ul,ol",h="color",g="field-title",f="center",e="title",bo="field-mail",bn=":contains('List Item 1')",bm="fontWeight",bl="p",bk="bold",bj="h1,h2",bi="#detail1,#detail2,#detail3",bh="text",bg="backgroundColor",bf="<li>inserted via after()</li>",u=":first-child,:last-child",v="underline",s="green",t="<option>Mrs</option>",q="<option>Prof</option>",r="<option>Mr</option>",o="for",p="<li>inserted via append()</li>",A="<h3>Dynamically Inserted H3</h3>",B="opacity",K=" Title ",H="li:first-child",S="#detail2",N="li a",bb='<div style="border:2px solid red"><div style="border:2px solid blue"></div></div>',X="h1",D="#eee",be="0.5",bd="[type=submit]",bc="header",C="name",F="textAlign",G='<div style="background:#FFFFBB"></div>',J="<script type='text/javascript'>qx.log.Logger.debug('Globally executed script');</script>",L="<li>inserted via before()</li>",O="click",U="form",ba="label[for=field-mail]",w="<option>Dr</option>",x="submit",E="demobrowser.demo.bom.Selector",R="ul li:contains('List Item 3')",Q="<b></b>",P="p:eq(1)",W="label",V="red",M="Click to follow the link",T="input[type=submit]",d="a",Y="<li>inserted via prepend()</li>",y="orange",z="li:last-child",I="id";
...
bq.query(bl).setStyle(h,V);
bq.query(P).setStyle(bm,bk).setStyle(B,be);
bq.query(X).setStyles({"textAlign":f,"textDecoration":v,"color":s});
bu.debug("href of first link: "+bq.query("a").getAttribute("href"));
bq.query(d).setAttribute(e,M);
bq.query(bj).addClass(bc);
bq.query(bl).addListener(O,function(){alert(this.innerHTML);
});
(new qx.bom.Collection).add(j).setStyle(h,y).add(m).setStyle(bg,D).end().setStyle(F,f);
...
var bp=qx.bom.Collection.query(n).find(m);
bu.debug("Found "+bp.length+" li elements in uls using find()");
var bp=new qx.bom.Collection(document.body).find(bj);
bu.debug("Found "+bp.length+" headers using find()");
var bp=qx.bom.Collection.query(n).next()[0].tagName;
bu.debug("Tag following the UL element: "+bp);
var bp=qx.bom.Collection.query(n).nextAll().length;
bu.debug("Number of elements following the UL: "+bp);
var bp=qx.bom.Collection.query(m).parent().length;
bu.debug("Number of parent elements of all li elems: "+bp);
var bp=qx.bom.Collection.query(T).parents().length;
bu.debug("Number of all parent elements of the submit button: "+bp);
var bp=qx.bom.Collection.query(n).prev()[0].tagName;
bu.debug("Tag before the UL element: "+bp);
var bp=qx.bom.Collection.query(n).prevAll().length;
bu.debug("Number of elements preceding the UL: "+bp);
var bp=qx.bom.Collection.query(R).siblings().length;
bu.debug("List Item 3 has "+bp+" siblings");
var bp=qx.bom.Collection.query(l).getOffsetParent()[0].tagName;
bu.debug("Offset parent of select box: "+bp);
var bp=qx.bom.Collection.query(l).getOffset();
bu.debug("Offset of select box: ");
bu.debug(bp);
bq.query(N).eq(1).setStyle(bm,bk);
var bp=qx.bom.Collection.query(k).filter(bd);
bu.debug("Found "+bp.length+" submit buttons using filter(selector)");
var bp=qx.bom.Collection.query(k).filter(function(a,b,c){return a.type===x;
});
bu.debug("Found "+bp.length+" submit button using filter(function)");
var bp=qx.bom.Collection.query(j).hasClass(U);
bu.debug("Any H2 with form has a class 'form': "+bp);
var bp=qx.bom.Collection.query(n).find(m).is(bn);
bu.debug("Found li element with text 'List Item 1': "+bp);
var bp=qx.bom.Collection.query(n).find(m).not(bn);
bu.debug("Number of found li elements without text 'List Item 1': "+bp.length);
var bp=qx.bom.Collection.query(n).find(m).not(u);
bu.debug("Number of found li elements in the middle: "+bp.length);
qx.bom.Collection.query(i).append(p);
qx.bom.Collection.query(i).prepend(Y);
qx.bom.Collection.query(n).append(J);
qx.bom.Collection.query(H).after(bf);
qx.bom.Collection.query(z).before(L);
var bw=document.createElement(l);
var bv=document.createElement(W);
var br=new qx.bom.Collection(bw).setAttribute(I,g).setAttribute(C,e);
new qx.bom.Collection(bv).setAttribute(o,g).setAttribute(bh,K);
qx.bom.Collection.query(l).after(bv,bw);
br.append(r,t,w,q);
$(A).insertAfter(j);
qx.bom.Collection.id(bo).addListener(k,function(){bu.debug("Typing in e-mail field...");
});
var bs=qx.bom.Collection.id(bo);
var bt=bs.add(ba).reverse();
bs.after(bt.clone(true));
qx.bom.Collection.query(bi).wrapAll(bb);
qx.bom.Collection.query(bi).wrapInner(Q);
qx.bom.Collection.query(S).wrap(G);
}}});
})();
