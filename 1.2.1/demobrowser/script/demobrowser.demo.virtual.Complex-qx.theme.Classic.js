...
(function(){var v=" / ",u="row",t="foobar!",s="column",r="icon/",q="",p="folder.png",o="places/",n="demobrowser.demo.virtual.DemoLayer",m="network-server.png",g="mouseover",l="user-trash.png",k="changeValue",f="network-workgroup.png",e="qx.ui.basic.Atom",i="/",h="user-desktop.png";
qx.Class.define(n,{extend:qx.ui.virtual.layer.WidgetCell,construct:function(){qx.ui.virtual.layer.WidgetCell.call(this,this);
this._pool={atom:[],checkbox:[]};
this.__Nn=[];
},members:{_pool:null,__Nn:null,getCellData:function(A,B){if(!this.__Nn[A]){this.__Nn[A]=[];
}
if(!this.__Nn[A][B]){this.__Nn[A][B]={label:this.__No(),icon:this.__Np()};
}return this.__Nn[A][B];
},getCellWidget:function(x,y){var z;

if(y%2==0){z=this._pool.atom.pop();

if(!z){z=new qx.ui.basic.Atom();
z.addListener(g,function(){var C=this.__Np();
z.setIcon(C);
this.__Nn[x][y].icon=C;
},this);
}z.set(this.getCellData(x,y));
}else{z=this._pool.checkbox.pop();

if(!z){z=new qx.ui.form.CheckBox();
z.addListener(k,function(){this.setLabel(this.getLabel()==t?z.getUserData(u)+v+z.getUserData(s):t);
},z);
}z.set({value:x%2==0,label:x+v+y});
}z.setUserData(u,x);
z.setUserData(s,y);
return z;
},poolCellWidget:function(w){if(w.classname==e){this._pool.atom.push(w);
}else{this._pool.checkbox.push(w);
}},__No:function(){var name=q;

for(var j=0;j<10;j++){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);
}return name;
},__Np:function(){var a=r;
var c=o;
var d=[p,l,m,f,h];
var b=Math.floor(Math.random()*4);
return (a+16+i+c+d[b]);
}},destruct:function(){this._pool=this.__Nn=null;
}});
})();
