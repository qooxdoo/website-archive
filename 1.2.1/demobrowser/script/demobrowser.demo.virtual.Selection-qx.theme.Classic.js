...
var P=qx.theme.manager.Font.getInstance().resolve(B).getStyles();
this._fontCss=qx.bom.element.Style.compile(P);
...
this.getRoot().add(W,{edge:5});
W.add(new qx.ui.basic.Label(s).set({font:i,decorator:b,padding:3,allowGrowX:true}),{row:1,column:0});
var O=this.createRowSelectionScroller();
M.push(O.getUserData(l));
W.add(O,{row:2,column:0});
W.add(new qx.ui.basic.Label(q).set({font:i,decorator:b,padding:3,allowGrowX:true}),{row:1,column:1});
var O=this.createColumnSelectionScroller();
M.push(O.getUserData(l));
W.add(O,{row:2,column:1});
W.add(new qx.ui.basic.Label(n).set({font:i,decorator:b,padding:3,allowGrowX:true}),{row:1,column:2});
var O=this.createCellRectangleSelectionScroller();
M.push(O.getUserData(l));
W.add(O,{row:2,column:2});
W.add(new qx.ui.basic.Label(z).set({font:i,decorator:b,padding:3,allowGrowX:true}),{row:1,column:3});
var O=this.createCellLineSelectionScroller();
M.push(O.getUserData(l));
W.add(O,{row:2,column:3});
var U=new qx.ui.layout.Grid(20,4);
var R=new qx.ui.container.Composite(U).set({padding:15,backgroundColor:g});
W.add(R,{row:0,column:0,colSpan:4});
R.add(new qx.ui.basic.Label(p).set({font:i,padding:3}),{row:0,column:0});
var Q=new qx.ui.form.RadioButton(m);
Q.setUserData(h,F);
R.add(Q,{row:1,column:0});
var T=new qx.ui.form.RadioButton(w);
T.setUserData(h,k);
T.setValue(true);
R.add(T,{row:2,column:0});
var N=new qx.ui.form.RadioButton(A);
N.setUserData(h,C);
R.add(N,{row:3,column:0});
var L=new qx.ui.form.RadioButton(v);
L.setUserData(h,G);
R.add(L,{row:4,column:0});
var S=new qx.ui.form.RadioGroup(Q,T,N,L);
S.addListener(t,function(e){var bh=e.getData()[0].getUserData(h);
M.forEach(function(bl){bl.setMode(bh);
});

if(bh==F||bh==G){X.setEnabled(false);
V.setEnabled(true);
}else if(bh==k||bh==y){X.setEnabled(true);
V.setEnabled(false);
}});
R.add(new qx.ui.basic.Label(x).set({font:i,padding:3}),{row:0,column:1});
var X=new qx.ui.form.CheckBox(r);
R.add(X,{row:1,column:1});
X.addListener(E,function(e){if(e.getData()){var a=M[0].getMode();

if(a==F||a==G){this.debug("Drag selection is only available for the modes multi or additive");
}}M.forEach(function(bx){bx.setDrag(e.getData());
});
});
var V=new qx.ui.form.CheckBox(u).set({enabled:false});
R.add(V,{row:2,column:1});
V.addListener(E,function(e){if(e.getData()){var H=M[0].getMode();

if(H==k||H==C){this.debug("Quick selection is only available for the modes multi or additive");
}}M.forEach(function(bd){bd.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bF=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var bD=new qx.ui.virtual.layer.Row(g,d);
bF.getPane().addLayer(bD);
bF.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
...
}return bE.getCellProperties(bG+j+bH,bI);
}});
bF.getPane().addLayer(bC);
var bB=new qx.ui.virtual.selection.Row(bF.getPane(),{styleSelectable:function(by,bz,bA){if(bz!==f){return;
}
if(bA){bD.setBackground(by,f);
}else{bD.setBackground(by,null);
}bC.updateLayerData();
}});
...
bF.setUserData(l,bB);
return bF;
},createColumnSelectionScroller:function(){var bM=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bM.getPane().addLayer(new qx.ui.virtual.layer.Row(g,d));
var bL=new qx.ui.virtual.layer.Column();
bM.getPane().addLayer(bL);
bM.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var bN=new qx.ui.virtual.cell.Cell();
var bK=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bu,bv){var bw={};

if(bJ.isItemSelected(bv)){bw.selected=true;
}return bN.getCellProperties(bu+j+bv,bw);
}});
bM.getPane().addLayer(bK);
var bJ=new qx.ui.virtual.selection.Column(bM.getPane(),{styleSelectable:function(br,bs,bt){if(bs!==f){return;
}
if(bt){bL.setBackground(br,f);
}else{bL.setBackground(br,null);
}bK.updateLayerData();
}});
bJ.attachMouseEvents(bM.getPane());
bJ.attachKeyEvents(bM);
bJ.set({mode:k});
bJ.addItem(0);
bM.setUserData(l,bJ);
return bM;
},createCellRectangleSelectionScroller:function(){var bp=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bp.getPane().addLayer(new qx.ui.virtual.layer.Row(g,d));
var bo=new qx.ui.virtual.cell.Cell();
var bq=new qx.ui.virtual.cell.Cell().set({backgroundColor:D});
var bn=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(be,bf){var bg={};

if(bm.isItemSelected({row:be,column:bf})){bg.selected=true;
}
if(bg.selected){return bq.getCellProperties(be+j+bf,bg);
}else{bo.resetBackgroundColor();
return bo.getCellProperties(be+j+bf,bg);
}}});
bp.getPane().addLayer(bn);
bp.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var bm=new qx.ui.virtual.selection.CellRectangle(bp.getPane(),{styleSelectable:function(ba,bb,bc){bn.updateLayerData();
}});
bm.attachMouseEvents(bp.getPane());
bm.attachKeyEvents(bp);
bm.set({mode:k});
bm.addItem({row:0,column:0});
bp.setUserData(l,bm);
return bp;
},createCellLineSelectionScroller:function(){var bR=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bR.getPane().addLayer(new qx.ui.virtual.layer.Row(g,d));
var bQ=new qx.ui.virtual.cell.Cell();
var bS=new qx.ui.virtual.cell.Cell().set({backgroundColor:D});
var bP=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(I,J){var K={};

if(bO.isItemSelected({row:I,column:J})){K.selected=true;
}
if(K.selected){return bS.getCellProperties(I+j+J,K);
}else{bQ.resetBackgroundColor();
return bQ.getCellProperties(I+j+J,K);
}}});
bR.getPane().addLayer(bP);
bR.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var bO=new qx.ui.virtual.selection.CellLines(bR.getPane(),{styleSelectable:function(bi,bj,bk){bP.updateLayerData();
}});
...
if(!n){return null;
}return this._isSelectable(n.column)?n.column:null;
},_getRelatedSelectable:function(h,j){if(j==e){var l=h-1;
var k=0;
var m=-1;
}else if(j==g){var l=h+1;
var k=this._getItemCount()-1;
var m=1;
}else{return null;
}
for(var i=l;i!==k+m;i+=m){if(this._isSelectable(i)){return i;
}}return null;
...
(function(){var k="under",j="above",i="left",h="right",g="x",f="qx.ui.virtual.selection.CellRectangle";
qx.Class.define(f,{extend:qx.ui.virtual.selection.Abstract,members:{_getItemCount:function(){return this._pane.getRowConfig().getItemCount()*this._pane.getColumnConfig().getItemCount();
...
},getSelectables:function(v){var x=[];
var A=this._pane.getRowConfig().getItemCount();
var w=this._pane.getColumnConfig().getItemCount();

for(var B=0;B<A;B++){for(var y=0;y<w;y++){var z={row:B,column:y};

if(this._isSelectable(z)){x.push(z);
}}}return x;
...
},_getFirstSelectable:function(){var K=this._pane.getRowConfig().getItemCount();
var J=this._pane.getColumnConfig().getItemCount();

for(var N=0;N<K;N++){for(var M=0;M<J;M++){var L={row:N,column:M};

if(this._isSelectable(L)){return L;
}}}return null;
},_getLastSelectable:function(){var b=this._pane.getRowConfig().getItemCount();
var a=this._pane.getColumnConfig().getItemCount();

for(var d=a-1;d>=0;d--){for(var e=b-1;e>=0;e--){var c={row:e,column:d};

if(this._isSelectable(c)){return c;
}}}return null;
},_getRelatedSelectable:function(C,D){var E={row:C.row,column:C.column};

switch(D){case j:for(var I=C.row-1;I>=0;I--){E.row=I;

if(this._isSelectable(E)){return E;
}}break;
case k:var F=this._pane.getRowConfig().getItemCount();

for(var I=C.row+1;I<F;I++){E.row=I;

if(this._isSelectable(E)){return E;
}}break;
case i:for(var H=C.column-1;H>=0;H--){E.column=H;

if(this._isSelectable(E)){return E;
}}break;
case h:var G=this._pane.getColumnConfig().getItemCount();

for(var H=C.column+1;H<G;H++){E.column=H;
...
}},_selectableToHashCode:function(bb){return bb.column+g+bb.row;
},_scrollItemIntoView:function(O){this._pane.scrollCellIntoView(O.column,O.row);
...
var W=X.getItemPosition(U.row);
var V=W+X.getItemSize(U.row)-1;
