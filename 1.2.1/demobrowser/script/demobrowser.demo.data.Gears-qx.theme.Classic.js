...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.data.Gears-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"0428e45bfe55"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var l="value",k='undefined',j="",i="model",h="application/x-googlegears",g="demobrowser.demo.data.store.Gears",f="<span style='font-size: 20px'>google Gears</span>",d="Age: ",c="http://code.google.com/apis/gears/install.html",b="lastname",D="firstname",C="age",B="demobrowser.demo.data.Gears",A="First Name: ",z='qooxdoo',y='Gears.Factory',x='select * from Persons',w="delete from Persons",v="LastName: ",u=' (Firstname varchar(255), Lastname varchar(255), Age int)',s="Person",t='ie_mobile',q="none",r='create table if not exists Persons',o="This demo requires Gears to be installed. Install now?",p='insert into Persons values (?, ?, ?)',m='beta.database',n="object";
...
this.getRoot().add(V,{left:10,top:50});
V.setLayout(new qx.ui.layout.Grid(0,3));
V.add(new qx.ui.basic.Label(A),{row:0,column:0});
V.add(new qx.ui.basic.Label(v),{row:1,column:0});
V.add(new qx.ui.basic.Label(d),{row:2,column:0});
var P=new qx.ui.form.TextField();
V.add(P,{row:0,column:1});
var R=new qx.ui.form.TextField();
V.add(R,{row:1,column:1});
var S=new qx.ui.form.Spinner(0,0,1000);
V.add(S,{row:2,column:1});
var T=new qx.data.controller.Object();
T.addTarget(P,l,D,true);
T.addTarget(R,l,b,true);
T.addTarget(S,l,C,true);
var U=new demobrowser.demo.data.store.Gears();
U.bind(i,T,i);
...
qx.Class.define(g,{extend:qx.data.store.Json,construct:function(){qx.data.store.Json.call(this);
this.initGears();
if(!window.google||!google.gears){if(confirm(o)){location.href=c;
return;
}return;
}this.__NN=this.initDB();
this.__NO();
},members:{__NO:function(K){var L=this.getPerson();
this._marshaler.toClass(L,true);
this.setModel(this._marshaler.toModel(L));
this.getModel().addListener("changeBubble",this.__NP,this);
this.fireDataEvent("loaded",this.getModel());
},__NP:function(E){var F=this.getModel();
this.update(F.getFirstname(),F.getLastname(),F.getAge());
},update:function(M,N,O){if(!google.gears.factory||!this.__NN){return;
}this.__NN.execute(w);
this.__NN.execute(p,[M,N,O]);
},getPerson:function(){var I={};
I.firstname=j;
I.lastname=j;
I.age=0;

if(!google.gears.factory||!this.__NN){return I;
}var J=this.__NN.execute(x);
...
return I;
},initDB:function(){if(window.google&&google.gears){try{var a=google.gears.factory.create(m);
...
if(typeof GearsFactory!=k){G=new GearsFactory();
}else{try{G=new ActiveXObject(y);
if(G.getBuildInfo().indexOf(t)!=-1){G.privateSetGlobalObject(this);
}}catch(e){if((typeof navigator.mimeTypes!=k)&&navigator.mimeTypes[h]){G=document.createElement(n);
G.style.display=q;
G.width=0;
G.height=0;
G.type=h;
...
if(!google.gears){google.gears={factory:G};
}}}});
})();
