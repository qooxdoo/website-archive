...
try{U.setName(p);
V.setHtml(V.getHtml()+i+U.getName()+j);
}catch(T){V.setHtml(V.getHtml()+F+P+U.getName()+j);
...
try{U.setAge(true);
V.setHtml(V.getHtml()+e+U.getAge()+j);
}catch(Q){V.setHtml(V.getHtml()+B+x+U.getAge()+j);
...
try{U.setAge(M);
V.setHtml(V.getHtml()+h+U.getGender()+j);
}catch(S){V.setHtml(V.getHtml()+t+G+U.getGender()+j);
...
qx.Class.define(n,{extend:qx.core.Object,properties:{name:{init:v,validate:E},age:{init:18,validate:qx.util.Validate.number()},gender:{init:g,validate:qx.util.Validate.inArray([g,f])}},members:{__Mh:function(R){if(R.length<2){throw new qx.core.ValidationError(C+I+R+O);
}}}});
})();
