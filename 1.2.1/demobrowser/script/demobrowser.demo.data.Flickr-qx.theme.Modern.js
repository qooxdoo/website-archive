...
var X=new demobrowser.demo.data.store.Flickr(k);
var P=new qx.ui.form.TextField(k);
this.getRoot().add(P,{left:30,top:50});
var Q=new qx.ui.form.Button(q);
this.getRoot().add(Q,{left:140,top:49});
Q.addListener(C,function(){X.searchForTag(P.getValue());
},this);
P.addListener(b,function(e){if(e.getKeyIdentifier()==c){X.searchForTag(P.getValue());
}},this);
...
V.setLabelPath(r);
V.setDelegate({configureItem:function(G){G.setShow(s);
}});
var R={converter:function(E,F){return (d+F.getFarm()+i+F.getServer()+j+E+f+F.getSecret()+z);
}};
V.setIconOptions(R);
V.setIconPath(t);
X.bind(a,V,l);
...
var W={converter:function(O){if(O){return (d+O.getFarm()+i+O.getServer()+j+O.getId()+f+O.getSecret()+D);
}return g;
}};
V.bind(A,S,B,W);
...
qx.Class.define(v,{extend:qx.data.store.Json,statics:{RESULT:null},construct:function(L){jsonFlickrApi=function(H){demobrowser.demo.data.store.Flickr.RESULT=H;
};
var M=h+L;
qx.data.store.Json.call(this,M);
},members:{searchForTag:function(Y){if(Y!=g){this.setUrl(h+Y);
}},_createRequest:function(I){var J=new qx.io.ScriptLoader();
this.setState(n);
I+=m;
J.load(I,function(N){this.__NS();
},this);
},__NS:function(){this.setState("completed");
var K=demobrowser.demo.data.store.Flickr.RESULT;

if(K==undefined){this.setState("failed");
return;
