...
(function(){var m="execute",l="changeValue",k="A date",j="Boolean",h="icon/22/actions/edit-undo.png",g="Change ID sort method",f="icon/22/status/dialog-information.png",e="Change row with ID 10",d="A number",c="icon/16/apps/office-calendar.png",C="Remove 5 rows",B="Add 10 rows",A="",z="demobrowser.demo.table.Table",y="_tableModel",x="Table",w="icon/22/actions/list-remove.png",v="Whether the the first visible row should ",u=", ",t="icon/22/actions/list-add.png",r="Selected rows:<br>",s="Keep first row",p="be rendered completely when scrolling.",q="Show selection",n="Demonstrate use of alternate sorting algorithm.",o="ID";
qx.Class.define(z,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return x;
},createTable:function(){var T=this.createRandomRows(50);
var S=this._tableModel=new qx.ui.table.model.Simple();
S.setColumns([o,d,k,j]);
S.setData(T);
S.setColumnEditable(1,true);
S.setColumnEditable(2,true);
S.setColumnSortable(3,false);
var U=new qx.ui.table.Table(S);
U.set({width:600,height:400,decorator:null});
...
R.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(c,k));
return U;
},createControls:function(){var ba=new qx.ui.toolbar.ToolBar();
var X,Y,bb;
Y=new qx.ui.toolbar.Part();
ba.add(Y);
X=new qx.ui.toolbar.Button(e,h);
X.addListener(m,function(D){var E=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,E[0][i]);
}this.info("Row 10 changed");
},this);
Y.add(X);
X=new qx.ui.toolbar.Button(B,t);
X.addListener(m,function(a){var b=this.createRandomRows(10);
this._tableModel.addRows(b);
this.info("10 rows added");
},this);
Y.add(X);
X=new qx.ui.toolbar.Button(C,w);
X.addListener(m,function(F){var G=this._tableModel.getRowCount();
this._tableModel.removeRows(G-5,5);
this.info("5 rows removed");
},this);
Y.add(X);
X=new qx.ui.toolbar.Button(q,f);
X.addListener(m,function(I){var J=[];
W.getSelectionModel().iterateSelection(function(V){J.push(V+A);
});
this.showDialog(r+J.join(u));
},this);
Y.add(X);
Y=new qx.ui.toolbar.Part();
ba.add(Y);
var W=this._table;
bb=new qx.ui.toolbar.CheckBox(s);
bb.set({value:W.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(v+p)});
bb.addListener(l,function(H){W.setKeepFirstVisibleRowComplete(this.getValue());
},bb);
Y.add(bb);
bb=new qx.ui.toolbar.CheckBox(g);
bb.set({value:W.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(n)});
bb.addListener(l,function(O){if(O.getData()){var Q=function(bc,bd){var be=bc[arguments.callee.columnIndex];
...
var P=function(K,L){var M=K[arguments.callee.columnIndex];
...
W.getTableModel().setSortMethods(0,{ascending:Q,descending:P});
