...
Q.setDelegate(this);
Q.setModel(O);
},extendData:function(S){S.date=u+Math.round(Math.random()*30+1)+o;
S.size=Math.round(Math.random()*100)+x;
S.light=Math.floor(Math.random()*4)==0;
S.mode=D;
S.checked=Math.random()>=0.5;

if(S.children){for(var i=0;i<S.children.length;i++){this.extendData(S.children[i]);
}}},bindItem:function(T,U,V){T.bindDefaultProperties(U,V);
T.bindProperty(j,k,null,U.getUserData(j),V);
T.bindProperty(b,k,null,U.getUserData(e),V);
T.bindPropertyReverse(b,k,null,U.getUserData(e),V);
T.bindProperty(f,k,null,U.getUserData(f),V);
T.bindProperty(g,k,null,U.getUserData(g),V);
T.bindProperty(h,E,{converter:function(R){return R?n:c;
}},U.getUserData(h),V);
