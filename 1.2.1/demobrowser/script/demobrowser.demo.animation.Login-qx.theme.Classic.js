...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.animation.Login","qx.theme":"qx.theme.Classic"};
...
(function(){var p="top",o="right",n="resize",m="Name",l="Password",k="Login",j="__Me",h="__Md",g="appear",f="__Mc",b="demobrowser.demo.animation.Login",d="50%",c="execute",a="30%";
qx.Class.define(b,{extend:qx.application.Standalone,members:{__Mc:null,__Md:null,__Me:null,checkInput:function(){this.__Mc.start();
},__Mf:function(){this.__Mc=new qx.fx.effect.combination.Shake(this.__Md.getContainerElement().getDomElement());
...
this.__Md=new qx.ui.groupbox.GroupBox().set({contentPadding:[16,16,16,16]});
this.__Md.setLayout(r);
this.__Md.addListener(n,function(e){var v=this.__Md.getBounds();
this.__Md.set({marginTop:Math.round(-v.height/2),marginLeft:Math.round(-v.width/2)});
},this);
this.getRoot().add(this.__Md,{left:d,top:a});
var q=[m,l];

for(var i=0;i<q.length;i++){this.__Md.add(new qx.ui.basic.Label(q[i]).set({allowShrinkX:false,paddingTop:3}),{row:i,column:0});
}var s=new qx.ui.form.TextField();
var t=new qx.ui.form.PasswordField();
this.__Md.add(s.set({allowShrinkX:false,paddingTop:3}),{row:0,column:1});
this.__Md.add(t.set({allowShrinkX:false,paddingTop:3}),{row:1,column:1});
var u=this.__Me=new qx.ui.form.Button(k);
u.setAllowStretchX(false);
this.__Md.add(u,{row:3,column:1});
u.addListener(c,this.checkInput,this);
this.__Md.addListener(g,this.__Mf,this);
}},destruct:function(){this._disposeObjects(h,j,f);
}});
})();
...
(function(){var e="horizontal",d="vertical",c="qx.fx.effect.combination.Shake",b="__GP",a="Number";
qx.Class.define(c,{extend:qx.fx.Base,construct:function(f){qx.fx.Base.call(this,f);
this.__GP=[new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f)];
},properties:{direction:{init:e,check:[e,d]},duration:{init:0.5,refine:true},distance:{init:20,check:a}},members:{__GP:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var g=parseFloat(this.getDistance());
var h=parseFloat(this.getDuration())/10.0;

if(this.getDirection()==e){this.__GP[0].set({x:g,y:0,duration:h});
this.__GP[1].set({x:-g*2,y:0,duration:h*2});
this.__GP[2].set({x:g*2,y:0,duration:h*2});
this.__GP[3].set({x:-g*2,y:0,duration:h*2});
this.__GP[4].set({x:g*2,y:0,duration:h*2});
this.__GP[5].set({x:-g,y:0,duration:h*2});
}else if(this.getDirection()==d){this.__GP[0].set({y:g,x:0,duration:h});
this.__GP[1].set({y:-g*2,x:0,duration:h*2});
this.__GP[2].set({y:g*2,x:0,duration:h*2});
this.__GP[3].set({y:-g*2,x:0,duration:h*2});
this.__GP[4].set({y:g*2,x:0,duration:h*2});
this.__GP[5].set({y:-g,x:0,duration:h*2});
}var k=this.__GP;

for(var i=0,j=this.__GP.length;i<j;i++){this.__GP[i].id=i;

if(i<5){this.__GP[i].afterFinishInternal=function(){k[this.id+1].start();
};
}}this.__GP[0].start();
