...
var s=new demobrowser.demo.data.store.Twitter(i);
var q=new qx.ui.embed.Html();
q.setBackgroundColor(b);
q.setDecorator(d);
q.setWidth(500);
q.setHeight(200);
q.setOverflow(n,n);
this.getRoot().add(q,{left:10,top:80});
s.addListener(h,function(){var o=s.getModel();
this.debug(qx.dev.Debug.debugProperties(o));
q.setHtml(qx.dev.Debug.debugProperties(o,10,true));
},this);
...
qx.Class.define(f,{extend:qx.data.store.Json,statics:{saveResult:function(y){this.__NI=y;
...
w+=l;
x.load(w,function(p){this.__NJ();
},this);
},__NJ:function(){var t=demobrowser.demo.data.store.Twitter.__NI;

if(t==undefined){this.setState("failed");
return;
...
qx.Class.define(t,{statics:{debugObject:function(R,S,T){qx.log.Logger.debug(this,qx.dev.Debug.debugObjectToString(R,S,T,false));
},debugObjectToString:function(X,Y,ba,bb){if(!ba){ba=10;
}var bh=(bb?I:r);
var be=function(bn){var bo;

if(!bb){bo=l;

for(var i=0;i<bn;i++){bo+=q;
}}else{bo=n+(bn*8)+b;
}return bo;
};
var bg=l;
...
try{bd(X,0,ba);
}catch(bm){bg+=be(0)+H+bm+d+bh;
}bg+=be(0)+G+bh;
return bg;
},getFunctionName:function(U,V){var W=U.self;

if(!W){return null;
}while(U.wrapper){U=U.wrapper;
}
switch(V){case k:return U==W?k:null;
case B:return qx.lang.Object.getKeyFromValue(W,U);
case A:return qx.lang.Object.getKeyFromValue(W.prototype,U);
default:if(U==W){return k;
}return (qx.lang.Object.getKeyFromValue(W.prototype,U)||qx.lang.Object.getKeyFromValue(W,U)||null);
