...
bJ.addListener(l,function(e){if(e.getData()){var bT=bO.getSelectionMode();

if(bT==k||bT==n){this.debug("Drag selection is only available for the modes multi or additive");
...
bF.addListener(T,function(e){var bS=e.getData()[0].getUserData(o);
bO.setSelectionMode(bS);

if(bS==k||bS==n){bJ.setEnabled(false);
bM.setEnabled(true);
}else if(bS==m||bS==q){bJ.setEnabled(true);
bM.setEnabled(false);
}});
...
(function(){var v="selected",u="qx.event.type.Data",t="Boolean",s="changeSelection",r="row",q="single",p="changeHeight",o=",",n="one",m="_applySpacing",K="Integer",J="_applyDragSelection",I="String",H="resize",G="demobrowser.demo.virtual.list.List",F="list",E="multi",D="_applyQuickSelection",C="_applyItemHeight",B="_applySelectionMode",z="additive",A="changeName",x="Number",y="addItem",w="removeItem";
qx.Class.define(G,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider],construct:function(){qx.ui.virtual.core.Scroller.call(this,0,1,this.getItemHeight(),10);
...
this.__Mo.addListener(s,this._onChangeSelection,this);
...
},events:{addItem:u,removeItem:u,changeValue:u,changeSelection:u},properties:{appearance:{refine:true,init:F},focusable:{refine:true,init:true},spacing:{check:K,init:0,apply:m,themeable:true},enableInlineFind:{check:t,init:true},name:{check:I,nullable:true,event:A},itemHeight:{init:24,themeable:true,check:x,apply:C},selectionMode:{check:[q,E,z,n],init:q,apply:B},dragSelection:{check:t,init:false,apply:J},quickSelection:{check:t,init:false,apply:D}},members:{__Mm:null,__Mn:null,__Mo:null,__Ml:null,syncWidget:function(){this.update();
},update:function(){var bt=this.getPane().getRowConfig();
bt.setItemCount(this.__Mm.length);
bt.resetItemSizes();

for(var i=0;i<this.__Mm.length;i++){var bs=this.__Mm[i].getHeight();

if(bs!==null){bt.setItemSize(i,bs);
}}},updateSelection:function(){var O=this.__Ml.getChildren();

for(var i=0;i<O.length;i++){var P=O[i];
var Q=P.getUserData(r);

if(this.__Mo.isItemSelected(Q)){P.addState(v);
}else{P.removeState(v);
...
this.fireDataEvent(s,this.__Ms(e.getData()));
},getCellWidget:function(bk,bl){var bn=this.__Mm[bk];

if(!bn){return null;
}var bm=this.__Mn.pop()||new qx.ui.form.ListItem();
bm.set({label:bn.getLabel(),icon:bn.getIcon()});

if(this.__Mo.isItemSelected(bk)){bm.addState(v);
}else{bm.removeState(v);
}bm.setUserData(r,bk);
return bm;
},poolCellWidget:function(k){this.__Mn.push(k);
},getValue:function(){var L=this.getSelection();
var M=[];
var N;

for(var i=0,l=L.length;i<l;i++){N=L[i].getValue();
if(N==null){N=L[i].getLabel();
}M.push(N);
}return M.join(o);
},setValue:function(bc){var bf=bc.split(o);
var be=[];
var bd;

for(var i=0,l=bf.length;i<l;i++){bd=this.findItem(bf[i]);

if(bd){be.push(bd);
}}this.setSelection(be);
},getChildren:function(){return this.__Mm;
},hasChildren:function(){return this.__Mm.length>0;
},_addHelper:function(bB){this.fireDataEvent(y,bB);
bB.addListener(p,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},_removeHelper:function(bi){this.fireDataEvent(w,bi);
bi.removeListener(p,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},add:function(by){this.__Mm.push(by);
this._addHelper(by);
return this;
},remove:function(U){qx.lang.Array.remove(this.__Mm,U);
this._removeHelper(U);
return this;
},removeAll:function(){for(var i=0,j=this.__Mm.length;i<j;i++){this._removeHelper(this.__Mm[i]);
}this.__Mm=[];
},indexOf:function(bv){return this.__Mm.indexOf(bv);
},addAt:function(bq,br){qx.lang.Array.insertAt(this.__Mm,bq,br);
this._addHelper(bq);
},addBefore:function(bo,bp){qx.lang.Array.insertBefore(this.__Mm,bo,bp);
this._addHelper(bo);
},addAfter:function(d,f){qx.lang.Array.insertAfter(this.__Mm,d,f);
this._addHelper(d);
},removeAt:function(a){this.__Mm.splice(a,1);
this._removeHelper(this.__Mm[a]);
},__Mp:function(g){var h=this.__Mm.indexOf(g);

if(h<0){return null;
}else{return h;
}},__Mq:function(R){var S=this.__Mm[R];
return S;
},__Mr:function(bg){var bh=[];

for(var i=0;i<bg.length;i++){bh.push(this.__Mp(bg[i]));
}return bh;
},__Ms:function(W){var X=[];

for(var i=0;i<W.length;i++){X.push(this.__Mm[W[i]]);
}return X;
},selectAll:function(){this.__Mo.selectAll();
},setSelected:function(T){this.__Mo.selectItem(this.__Mp(T));
},isSelected:function(Y){return this.__Mo.isItemSelected(this.__Mp(Y));
},addToSelection:function(bj){this.__Mo.addItem(this.__Mp(bj));
},removeFromSelection:function(V){this.__Mo.removeItem(this.__Mp(V));
},selectRange:function(bw,bx){this.__Mo.selectItemRange(this.__Mp(bw),this.__Mp(bx));
},resetSelection:function(){this.__Mo.clearSelection();
},setSelection:function(bu){this.__Mo.replaceSelection(this.__Mr(bu));
...
},_applySelectionMode:function(ba,bb){this.__Mo.setMode(ba);
},_applyDragSelection:function(bz,bA){this.__Mo.setDrag(bz);
},_applyQuickSelection:function(b,c){this.__Mo.setQuick(b);
}}});
})();
...
(function(){var o="scroll",n="scrollbar-x",m="scrollbar-y",l="__Kb",k="_applyInterval",j="appear",i="interval",h="qx.ui.virtual.behavior.Prefetch",g="_applyScroller",f="qx.ui.virtual.core.Scroller",e="Integer";
qx.Class.define(h,{extend:qx.core.Object,construct:function(r,s){{};
qx.core.Object.call(this);
this.setPrefetchX(s.minLeft,s.maxLeft,s.minRight,s.maxRight);
this.setPrefetchY(s.minAbove,s.maxAbove,s.minBelow,s.maxBelow);
this.__Kb=new qx.event.Timer(this.getInterval());
this.__Kb.addListener(i,this._onInterval,this);

if(r){this.setScroller(r);
}},properties:{scroller:{check:f,nullable:true,init:null,apply:g},interval:{check:e,init:200,apply:k}},members:{__Kc:null,__Kd:null,__Kb:null,__Ke:null,__Kf:null,setPrefetchX:function(a,b,c,d){this.__Kc=[a,b,c,d];
},setPrefetchY:function(v,w,x,y){this.__Kd=[v,w,x,y];
},_onInterval:function(){var z=this.__Kc;
...
qx.ui.core.queue.Manager.flush();
}},_applyScroller:function(t,u){if(u){if(this.__Ke){u.getChildControl(n).removeListenerById(this.__Ke);
}
if(this.__Kf){u.getChildControl(m).removeListenerById(this.__Kf);
}}
if(t){if(!t.getContainerElement().getDomElement()){this.__Kb.stop();
t.addListenerOnce(j,this.__Kb.start,this.__Kb);
}else{this.__Kb.restart();
}this.__Ke=t.getChildControl(n).addListener(o,this.__Kb.restart,this.__Kb);
this.__Kf=t.getChildControl(m).addListener(o,this.__Kb.restart,this.__Kb);
}else{this.__Kb.stop();
}},_applyInterval:function(p,q){this.__Kb.setInterval(p);
}},destruct:function(){this.setScroller(null);
this.__Kc=this.__Kd=null;
