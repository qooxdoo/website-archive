...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.animation.Transition_Position","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.animation.Transition_Position-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"27a963cf56d2"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var t="Name",s="Wobble will bounce the element forwards and backwards.",r="EaseInQuad will accelerate exponentially.",q="finish",p="Description",o="changeSelection",n="Linear is the default transition for many effects.",m="demobrowser.demo.animation.Transition_Position",l="execute",k="appear",d="Duration",j="Spring will overshoot the target and then move back.",h="Show it!",c="EaseOutQuad will slow down exponentially.",b="test",g="Sinodial transition will accelerate sinusoidal.",f="Reverse behaves like linear, but in the opposite direction.",i='<span style="color:white;">Welcome to <br><b style="color:#F3FFB3;">qooxdoo</b> animations!</span>',a="absolute";
...
H.setCssClass(b);
x.add(H);
var E={linear:n,easeInQuad:r,easeOutQuad:c,sinodial:g,reverse:f,wobble:s,spring:j};
var v=new qx.ui.form.SelectBox();
var G=new qx.ui.form.Button(h);
var z=new qx.ui.basic.Label(t);
var A=new qx.ui.basic.Label(p);
var F=new qx.ui.basic.Label(d);
var A=new qx.ui.basic.Label(E.linear);
var B=new qx.ui.form.Spinner;

for(var y in E){v.add(new qx.ui.form.ListItem(y));
}v.addListener(o,function(e){A.setValue(E[A.setValue(e.getData()[0].getLabel())]);
});
B.set({maximum:10.0,minimum:0.1,value:1.0});
var w;
var D=false;
H.addListenerOnce(k,function(){w=new qx.fx.effect.core.Move(H.getContainerElement().getDomElement());
w.set({x:600,y:300,mode:a});
w.addListener(q,function(){w.set({x:D?90:600,y:D?90:300});
D=!D;
});
},this);
var C=new qx.util.format.NumberFormat();
C.setMaximumFractionDigits(2);
B.setNumberFormat(C);
G.addListener(l,function(){var u=v.getSelection()[0].getLabel();
w.set({transition:u,duration:B.getValue()});
w.start();
});
x.add(z,{left:25,top:50});
x.add(A,{left:25,top:75});
x.add(F,{left:25,top:25});
x.add(v,{left:90,top:50});
x.add(A,{left:90,top:75});
x.add(B,{left:90,top:25});
x.add(G,{left:23,top:100});
}}});
})();
