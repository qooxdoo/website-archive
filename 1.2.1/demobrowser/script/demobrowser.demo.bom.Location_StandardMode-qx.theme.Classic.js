...
var t=document.getElementById(j).getElementsByTagName(d);

for(var i=0,l=t.length;i<l;i++){t[i].onclick=demobrowser.demo.bom.Location_StandardMode.jump;
}}},statics:{jump:function(event){if(!event){event=window.event;
}event.returnValue=false;

if(event.preventDefault){event.preventDefault();
}function o(u,v){return parseInt(qx.bom.element.Style.get(u,v,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var q=this.href;
var s=q.substr(q.indexOf(h)+1);
var p=document.getElementById(s);
var location=qx.bom.element.Location.get(p,document.getElementById(g).value);
var r=document.getElementById(m);
r.style.left=location.left+n;
r.style.top=location.top+n;

if(qx.bom.client.Feature.BORDER_BOX){r.style.width=(location.right-location.left)+n;
r.style.height=(location.bottom-location.top)+n;
}else{r.style.width=(location.right-location.left-o(r,e)-o(r,a))+n;
r.style.height=(location.bottom-location.top-o(r,f)-o(r,k))+n;
}return false;
