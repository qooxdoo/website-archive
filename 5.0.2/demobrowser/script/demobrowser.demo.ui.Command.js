...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.Command.288b7cbfcb8e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="Saving...",b="demobrowser.demo.ui.Command",c="Save",d="  Button in the content at the bottom\n",e="label",f="icon",g="Editor",h="Save the current file.",i="  Button in the menu\n",j="icon/16/actions/document-save.png",k="  Button in the toolbar\n",l="CTRL+S",m="and have the same execute listener.",n="File",o="You can see three buttons in this window:\n",p="execute",q="Each of the buttons is configured using the same command ";qx.Class.define(b,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var v=new qx.ui.command.Command(l);v.setLabel(c);v.setIcon(j);v.setToolTipText(h);v.addListener(p,function(){alert(a);}
,this);var w=new qx.ui.window.Window(g);w.setLayout(new qx.ui.layout.VBox(0));w.setWidth(400);w.setHeight(300);w.setContentPadding(0);w.moveTo(30,10);w.open();var menubar=new qx.ui.menubar.MenuBar();w.add(menubar);var t=new qx.ui.menubar.Button(n);menubar.add(t);var u=new qx.ui.menu.Menu();t.setMenu(u);var r=new qx.ui.menu.Button();r.setCommand(v);u.add(r);var toolbar=new qx.ui.toolbar.ToolBar();w.add(toolbar);var y=new qx.ui.toolbar.Button();y.setCommand(v);y.setShow(f);toolbar.add(y);var content=new qx.ui.container.Composite();content.setLayout(new qx.ui.layout.VBox());w.add(content,{flex:1});var s=new qx.ui.form.TextArea();content.add(s,{flex:1});var x=new qx.ui.form.Button();x.setCommand(v);x.setShow(e);content.add(x);var z=o+i+k+d+q+m;s.setValue(z);}
}});}
)();
...
(function(){var b="-",c="PageUp",d="Escape",f="Boolean",g="+",h="qx.event.type.Data",j="_applyShortcut",k="PrintScreen",l="NumLock",m="short",n="keydown",o="5",p="8",q="execute",r="Meta",s="0",t="",u="2",v="Control",w="Shift",x="You can only specify one non modifier key!",y="3",z="Unidentified",A="/",B="Delete",C="String",D="changeEnabled",E="*",F="qx.bom.Shortcut",G="6",H="4",I="1",J="Alt",K="Not a valid key name for a shortcut: ",L="PageDown",M="Whitespaces are not allowed within shortcuts",N="_applyEnabled",O="keypress",P="7",Q="a",R="z",S="9";qx.Class.define(F,{extend:qx.core.Object,construct:function(T){qx.core.Object.call(this);this.__pX={};this.__pY=null;if(T!=null){this.setShortcut(T);}
;this.initEnabled();}
,events:{"execute":h},properties:{enabled:{init:true,check:f,event:D,apply:N},shortcut:{check:C,apply:j,nullable:true},autoRepeat:{check:f,init:false}},members:{__pX:t,__pY:t,execute:function(U){this.fireDataEvent(q,U);}
,__qa:function(event){if(this.getEnabled()&&this.__qb(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,__hP:function(event){if(this.getEnabled()&&this.__qb(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,_applyEnabled:function(W,V){if(W){qx.event.Registration.addListener(document.documentElement,n,this.__qa,this);qx.event.Registration.addListener(document.documentElement,O,this.__hP,this);}
else {qx.event.Registration.removeListener(document.documentElement,n,this.__qa,this);qx.event.Registration.removeListener(document.documentElement,O,this.__hP,this);}
;}
,_applyShortcut:function(bb,Y){if(bb){if(bb.search(/[\s]+/)!=-1){var bc=M;this.error(bc);throw new Error(bc);}
;this.__pX={"Control":false,"Shift":false,"Meta":false,"Alt":false};this.__pY=null;var X;var a=[];while(bb.length>0&&X!=-1){X=bb.search(/[-+]+/);a.push((bb.length==1||X==-1)?bb:bb.substring(0,X));bb=bb.substring(X+1);}
;var ba=a.length;for(var i=0;i<ba;i++ ){var bd=this.__qd(a[i]);switch(bd){case v:case w:case r:case J:this.__pX[bd]=true;break;case z:var bc=K+a[i];this.error(bc);throw bc;default:if(this.__pY){var bc=x;this.error(bc);throw bc;}
;this.__pY=bd;};}
;}
;return true;}
,__qb:function(e){var be=this.__pY;if(!be){return false;}
;if((!this.__pX.Shift&&e.isShiftPressed())||(this.__pX.Shift&&!e.isShiftPressed())||(!this.__pX.Control&&e.isCtrlPressed())||(this.__pX.Control&&!e.isCtrlPressed())||(!this.__pX.Meta&&e.isMetaPressed())||(this.__pX.Meta&&!e.isMetaPressed())||(!this.__pX.Alt&&e.isAltPressed())||(this.__pX.Alt&&!e.isAltPressed())){return false;}
...
;}
,toString:function(){var bj=this.__pY;var bk=[];for(var bi in this.__pX){if(this.__pX[bi]){bk.push(qx.locale.Key.getKeyName(m,bi));}
;}
;if(bj){bk.push(qx.locale.Key.getKeyName(m,bj));}
;return bk.join(g);}
},destruct:function(){this.setEnabled(false);this.__pX=this.__pY=null;}
});}
)();
...
;var P=K.getMarginLeft()+K.getMarginRight();P=Math.max(P,this.getSpacing());var J=K.getSizeHint().width+P;this.__nF(K);G-=J;if(E&&E.getVisibility()!=o){E.setVisibility(o);G+=L;var H=E.getMarginLeft()+E.getMarginRight();G+=Math.max(H,this.getSpacing());}
;}
while(G>F);}
else if(this.__nC.length>0){do {var Q=this.__nC[0];if(Q){var P=Q.getMarginLeft()+Q.getMarginRight();P=Math.max(P,this.getSpacing());if(Q.getContentElement().getDomElement()==null){Q.syncAppearance();Q.invalidateLayoutCache();}
;var I=Q.getSizeHint().width;var O=false;if(this.__nC.length==1&&L>0){var N=P-this.getSpacing();var M=G-L+I+N;O=F>M;}
;if(F>G+I+P||O){this.__nE(Q);G+=I;if(E&&this.__nC.length==0){E.setVisibility(y);}
;}
else {return;}
;}
;}
while(F>=G&&this.__nC.length>0);}
;}
,__nE:function(R){R.setVisibility(o);this.__nC.shift();this.fireDataEvent(m,R);}
,__nF:function(S){if(!S){return;}
;this.__nC.unshift(S);S.setVisibility(y);this.fireDataEvent(A,S);}
,_getNextToHide:function(){for(var i=this.__nD.length-1;i>=0;i-- ){var T=this.__nD[i];if(T&&T.getVisibility&&T.getVisibility()==o){return T;}
;}
...
;}
;}
,setRemovePriority:function(X,W,Y){if(!Y&&this.__nD[W]!=undefined){throw new Error(v);}
;this.__nD[W]=X;}
...
;for(var i=0;i<this.__nC.length;i++ ){this.__nC[i].setVisibility(o);}
;this.__nC=[];}
;}
...
;bf.setVisibility(y);}
;}
,__nG:false,_setAllowMenuOpenHover:function(bg){this.__nG=bg;}
,_isAllowMenuOpenHover:function(){return this.__nG;}
...
,__nI:function(){var y=this.getChildrenContainer().getChildren();y=y.filter(function(z){return z.getVisibility()==k;}
...
(function(){var a="blur",b="Enter",c="Escape",d="Space",f="keydown",g="Left",h="Down",j="Up",k="singleton",l="qx.debug",m="interval",n="__nO",o="event.touch",p="__lr",q="Object is no menu: ",r="pointerdown",s="qx.ui.menu.Manager",t="keyup",u="keypress",v="roll",w="contextmenu",x="__nN",y="Right";qx.Class.define(s,{type:k,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__lr=[];var z=document.body;var A=qx.event.Registration;A.addListener(window.document.documentElement,r,this._onPointerDown,this,true);A.addListener(z,v,this._onRoll,this,true);A.addListener(z,f,this._onKeyUpDown,this,true);A.addListener(z,t,this._onKeyUpDown,this,true);A.addListener(z,u,this._onKeyPress,this,true);if(!qx.core.Environment.get(o)){qx.bom.Element.addListener(window,a,this.hideAll,this);}
;this.__nN=new qx.event.Timer();this.__nN.addListener(m,this._onOpenInterval,this);this.__nO=new qx.event.Timer();this.__nO.addListener(m,this._onCloseInterval,this);}
,members:{__nP:null,__nQ:null,__nN:null,__nO:null,__lr:null,_getChild:function(B,D,C,G){var F=B.getChildren();var length=F.length;var E;for(var i=D;i<length&&i>=0;i+=C){E=F[i];if(E.isEnabled()&&!E.isAnonymous()&&E.isVisible()){return E;}
;}
...
;H=H.getLayoutParent();}
;return false;}
,_isMenuOpener:function(J){var I=this.__lr;for(var i=0;i<I.length;i++ ){if(I[i].getOpener()===J){return true;}
;}
;return false;}
,_getMenuButton:function(K){while(K){if(K instanceof qx.ui.menu.AbstractButton){return K;}
;K=K.getLayoutParent();}
;return null;}
,add:function(L){if(qx.core.Environment.get(l)){if(!(L instanceof qx.ui.menu.Menu)){throw new Error(q+L);}
;}
;var M=this.__lr;M.push(L);L.setZIndex(1e6+M.length);}
,remove:function(N){if(qx.core.Environment.get(l)){if(!(N instanceof qx.ui.menu.Menu)){throw new Error(q+N);}
;}
;var O=this.__lr;if(O){qx.lang.Array.remove(O,N);}
;}
,hideAll:function(){var P=this.__lr;if(P){for(var i=P.length-1;i>=0;i-- ){P[i].exclude();}
;}
;}
,getActiveMenu:function(){var Q=this.__lr;return Q.length>0?Q[Q.length-1]:null;}
,scheduleOpen:function(R){this.cancelClose(R);if(R.isVisible()){if(this.__nP){this.cancelOpen(this.__nP);}
;}
else if(this.__nP!=R){this.__nP=R;this.__nN.restartWith(R.getOpenInterval());}
;}
,scheduleClose:function(S){this.cancelOpen(S);if(!S.isVisible()){if(this.__nQ){this.cancelClose(this.__nQ);}
;}
else if(this.__nQ!=S){this.__nQ=S;this.__nO.restartWith(S.getCloseInterval());}
;}
,cancelOpen:function(T){if(this.__nP==T){this.__nN.stop();this.__nP=null;}
;}
,cancelClose:function(U){if(this.__nQ==U){this.__nO.stop();this.__nQ=null;}
;}
,_onOpenInterval:function(e){this.__nN.stop();this.__nP.open();this.__nP=null;}
,_onCloseInterval:function(e){this.__nO.stop();this.__nQ.exclude();this.__nQ=null;}
,preventContextMenuOnce:function(){qx.event.Registration.addListener(document.body,w,this.__nR,this,true);}
,__nR:function(e){var V=e.getTarget();V=qx.ui.core.Widget.getWidgetByElement(V,true);if(this._isInMenu(V)){e.stopPropagation();e.preventDefault();}
;qx.event.Registration.removeListener(document.body,w,this.__nR,this,true);}
...
;if(this.__lr.length>0&&!this._isInMenu(W)){this.hideAll();}
;}
,__nS:{"Enter":1,"Space":1},__nT:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var X=this.getActiveMenu();if(!X){return;}
;var Y=e.getKeyIdentifier();if(this.__nT[Y]||(this.__nS[Y]&&X.getSelectedButton())){e.stopPropagation();}
;}
,_onKeyPress:function(e){var bc=this.getActiveMenu();if(!bc){return;}
;var ba=e.getKeyIdentifier();var be=this.__nT[ba];var bb=this.__nS[ba];if(be){switch(ba){case j:this._onKeyPressUp(bc);break;case h:this._onKeyPressDown(bc);break;case g:this._onKeyPressLeft(bc);break;case y:this._onKeyPressRight(bc);break;case c:this.hideAll();break;};e.stopPropagation();e.preventDefault();}
...
;}
;}
,_onKeyPressEnter:function(bE,bF,e){if(bF.hasListener(u)){var bG=e.clone();bG.setBubbles(false);bG.setTarget(bF);bF.dispatchEvent(bG);}
;this.hideAll();}
,_onKeyPressSpace:function(bH,bI,e){if(bI.hasListener(u)){var bJ=e.clone();bJ.setBubbles(false);bJ.setTarget(bI);bI.dispatchEvent(bJ);}
;}
,_onRoll:function(e){var bK=e.getTarget();bK=qx.ui.core.Widget.getWidgetByElement(bK,true);if(this.__lr.length>0&&!this._isInMenu(bK)&&!this._isMenuOpener(bK)&&!e.getMomentum()){this.hideAll();}
;}
},destruct:function(){var bM=qx.event.Registration;var bL=document.body;bM.removeListener(window.document.documentElement,r,this._onPointerDown,this,true);bM.removeListener(bL,f,this._onKeyUpDown,this,true);bM.removeListener(bL,t,this._onKeyUpDown,this,true);bM.removeListener(bL,u,this._onKeyPress,this,true);this._disposeObjects(x,n);this._disposeArray(p);}
});}
)();
...
;this.__nW();}
,__nW:function(){if(this.isVisible()){if(this.getBlockBackground()){var V=this.getZIndex();this._blocker.blockContent(V-1);}
;}
...
,__nX:function(){var bn=this._getMenuBounds();if(!bn){this.addListenerOnce(n,this.__nX,this);return;}
...
;this.__nV=bp;qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(bq){this.getChildControl(h);if(this.__nV){this.__nV.call(this);delete this.__nV;}
;}
...
;this.__nX();}
;}
,_onPointerOver:function(e){var bt=qx.ui.menu.Manager.getInstance();bt.cancelClose(this);var bu=e.getTarget();if(bu.isEnabled()&&bu instanceof qx.ui.menu.AbstractButton){this.setSelectedButton(bu);var bs=bu.getMenu&&bu.getMenu();if(bs){bs.setOpener(bu);bt.scheduleOpen(bs);this.__nU=bs;}
else {var bv=this.getOpenedButton();if(bv){bt.scheduleClose(bv.getMenu());}
;if(this.__nU){bt.cancelOpen(this.__nU);this.__nU=null;}
;}
;}
...
;if(this.__nU){bw.cancelOpen(this.__nU);}
;}
;}
...
(function(){var a="hovered",b="hover-button",c="pointerover",d="interval",f="__dE",g="qx.ui.form.HoverButton",h="Integer",i="pointerout";qx.Class.define(g,{extend:qx.ui.basic.Atom,include:[qx.ui.core.MExecutable],implement:[qx.ui.form.IExecutable],construct:function(j,k){qx.ui.basic.Atom.call(this,j,k);this.addListener(c,this._onPointerOver,this);this.addListener(i,this._onPointerOut,this);this.__dE=new qx.event.AcceleratingTimer();this.__dE.addListener(d,this._onInterval,this);}
,properties:{appearance:{refine:true,init:b},interval:{check:h,init:80},firstInterval:{check:h,init:200},minTimer:{check:h,init:20},timerDecrease:{check:h,init:2}},members:{__dE:null,_onPointerOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;}
;this.__dE.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();this.addState(a);}
,_onPointerOut:function(e){this.__dE.stop();this.removeState(a);if(!this.isEnabled()||e.getTarget()!==this){return;}
;}
...
(function(){var a="qx.ui.menu.Layout",b="Integer",c="_applyLayoutChange";qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:b,init:0,apply:c},spanColumn:{check:b,init:1,nullable:true,apply:c},iconColumnWidth:{check:b,init:0,themeable:true,apply:c},arrowColumnWidth:{check:b,init:0,themeable:true,apply:c}},members:{__oe:null,_computeSizeHint:function(){var q=this._getLayoutChildren();var o,g,k;var e=this.getSpanColumn();var j=this.__oe=[0,0,0,0];var n=this.getColumnSpacing();var m=0;var f=0;for(var i=0,l=q.length;i<l;i++ ){o=q[i];if(o.isAnonymous()){continue;}
...
,getColumnSizes:function(){return this.__oe||null;}
...
;var x=this.__of(y[0]);var w=x.getColumnSizes();var s=x.getSpacingX();var z=qx.lang.Array.sum(w)+s*(w.length-1);if(z<o){w[1]+=o-z;}
...
;}
;}
,__of:function(B){while(!(B instanceof qx.ui.menu.Menu)){B=B.getLayoutParent();}
;return B;}
