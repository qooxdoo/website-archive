...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.JsonToList.42915862dee0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;throw new Error(n);}
,getSelection:function(){return qx.lang.Object.getValues(this.__fK);}
,getSortedSelection:function(){var bU=this.getSelectables();var bT=qx.lang.Object.getValues(this.__fK);bT.sort(function(a,b){return bU.indexOf(a)-bU.indexOf(b);}
);return bT;}
,isItemSelected:function(bV){var bW=this._selectableToHashCode(bV);return this.__fK[bW]!==undefined;}
,isSelectionEmpty:function(){return qx.lang.Object.isEmpty(this.__fK);}
...
;this._fireChange();}
,_setLeadItem:function(cb){var ca=this.__qP;if(ca!==null){this._styleSelectable(ca,u,false);}
;if(cb!==null){this._styleSelectable(cb,u,true);}
;this.__qP=cb;}
,getLeadItem:function(){return this.__qP;}
,_setAnchorItem:function(cd){var cc=this.__qQ;if(cc!=null){this._styleSelectable(cc,I,false);}
;if(cd!=null){this._styleSelectable(cd,I,true);}
;this.__qQ=cd;}
,_getAnchorItem:function(){return this.__qQ!==null?this.__qQ:null;}
...
;this._fireChange();}
,handlePointerOver:function(event){if(this.__qS!=null&&this.__qS!=this._getScroll().top){this.__qS=null;return;}
...
;var cD=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cA=event.isShiftPressed();if(this.isItemSelected(cB)&&!cA&&!cD&&!this.getDrag()){this.__qR=cB;this._userInteraction=false;return;}
else {this.__qR=null;}
;this._scrollItemIntoView(cB);var cC=this.getMode();if(this.getDrag()&&cC!==bt&&cC!==h&&!cA&&!cD&&event.getPointerType()==F){this._setAnchorItem(cB);this._setLeadItem(cB);this.__qF=this._getLocation();this.__qC=this._getScroll();this.__qG=event.getDocumentLeft()+this.__qC.left;this.__qH=event.getDocumentTop()+this.__qC.top;this.__qI=true;this._capture();}
;this._fireChange(bD);this._userInteraction=false;}
,handleTap:function(event){this._userInteraction=true;var cH=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cE=event.isShiftPressed();if(!cH&&!cE&&this.__qR!=null){this._userInteraction=false;var cF=this._getSelectableFromPointerEvent(event);if(cF===null||!this.isItemSelected(cF)){return;}
;}
...
,handlePointerMove:function(event){if(!this.__qI){return;}
;this.__qJ=event.getDocumentLeft();this.__qK=event.getDocumentTop();this._userInteraction=true;var cI=this.__qJ+this.__qC.left;if(cI>this.__qG){this.__qL=1;}
else if(cI<this.__qG){this.__qL=-1;}
else {this.__qL=0;}
;var cJ=this.__qK+this.__qC.top;if(cJ>this.__qH){this.__qM=1;}
else if(cJ<this.__qH){this.__qM=-1;}
else {this.__qM=0;}
;var location=this.__qF;if(this.__qJ<location.left){this.__qz=this.__qJ-location.left;}
else if(this.__qJ>location.right){this.__qz=this.__qJ-location.right;}
else {this.__qz=0;}
;if(this.__qK<location.top){this.__qA=this.__qK-location.top;}
else if(this.__qK>location.bottom){this.__qA=this.__qK-location.bottom;}
else {this.__qA=0;}
;if(!this.__qB){this.__qB=new qx.event.Timer(100);this.__qB.addListener(G,this._onInterval,this);}
;this.__qB.start();this._autoSelect();event.stopPropagation();this._userInteraction=false;}
,handleAddItem:function(e){var cK=e.getData();if(this.getMode()===h&&this.isSelectionEmpty()){this.addItem(cK);}
;}
,handleRemoveItem:function(e){this.removeItem(e.getData());}
,_cleanup:function(){if(!this.getDrag()&&this.__qI){return;}
;if(this.__qN){this._fireChange(bD);}
;delete this.__qI;delete this.__qD;delete this.__qE;this._releaseCapture();if(this.__qB){this.__qB.stop();}
;}
,_onInterval:function(e){this._scrollBy(this.__qz,this.__qA);this.__qC=this._getScroll();this._autoSelect();}
,_autoSelect:function(){var cT=this._getDimension();var cM=Math.max(0,Math.min(this.__qJ-this.__qF.left,cT.width))+this.__qC.left;var cL=Math.max(0,Math.min(this.__qK-this.__qF.top,cT.height))+this.__qC.top;if(this.__qD===cM&&this.__qE===cL){return;}
;this.__qD=cM;this.__qE=cL;var cQ=this._getAnchorItem();var cO=cQ;var cR=this.__qL;var cU,cN;while(cR!==0){cU=cR>0?this._getRelatedSelectable(cO,t):this._getRelatedSelectable(cO,V);if(cU!==null){cN=this._getSelectableLocationX(cU);if((cR>0&&cN.left<=cM)||(cR<0&&cN.right>=cM)){cO=cU;continue;}
;}
;break;}
;var cS=this.__qM;var cV,cP;while(cS!==0){cV=cS>0?this._getRelatedSelectable(cO,z):this._getRelatedSelectable(cO,bB);if(cV!==null){cP=this._getSelectableLocationY(cV);if((cS>0&&cP.top<=cL)||(cS<0&&cP.bottom>=cL)){cO=cV;continue;}
;}
;break;}
...
;this._fireChange(bw);}
,__nT:{Home:1,Down:1,Right:1,PageDown:1,End:1,Up:1,Left:1,PageUp:1},handleKeyPress:function(event){this._userInteraction=true;var dd,dc;var df=event.getKeyIdentifier();var de=this.getMode();var cY=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var da=event.isShiftPressed();var db=false;if(df===A&&cY){if(de!==bt&&de!==h){this._selectAllItems();db=true;}
;}
...
;db=true;}
;}
else if(this.__nT[df]){db=true;if(de===bt||de==h){dd=this._getSelectedItem();}
...
;}
;break;};this.__qS=this._getScroll().top;this._scrollItemIntoView(dc);}
;}
...
;}
,_clearSelection:function(){var di=this.__fK;for(var dj in di){this._removeFromSelection(di[dj]);}
;this.__fK={};}
,_selectItemRange:function(dk,dp,dm){var dq=this._getSelectableRange(dk,dp);if(!dm){var dn=this.__fK;var dr=this.__qT(dq);for(var dl in dn){if(!dr[dl]){this._removeFromSelection(dn[dl]);}
;}
;}
...
;}
,__qT:function(dw){var dx={};var dv;for(var i=0,l=dw.length;i<l;i++ ){dv=dw[i];dx[this._selectableToHashCode(dv)]=dv;}
;return dx;}
,_getSelectedItem:function(){for(var dy in this.__fK){return this.__fK[dy];}
;return null;}
,_setSelectedItem:function(dA){if(this._isSelectable(dA)){var dz=this.__fK;var dB=this._selectableToHashCode(dA);if(!dz[dB]||(dz.length>=2)){this._clearSelection();this._addToSelection(dA);}
;}
;}
,_addToSelection:function(dC){var dD=this._selectableToHashCode(dC);if(this.__fK[dD]==null&&this._isSelectable(dC)){this.__fK[dD]=dC;this._styleSelectable(dC,R,true);this.__qN=true;}
;}
,_toggleInSelection:function(dE){var dF=this._selectableToHashCode(dE);if(this.__fK[dF]==null){this.__fK[dF]=dE;this._styleSelectable(dE,R,true);}
else {delete this.__fK[dF];this._styleSelectable(dE,R,false);}
;this.__qN=true;}
,_removeFromSelection:function(dG){var dH=this._selectableToHashCode(dG);if(this.__fK[dH]!=null){delete this.__fK[dH];this._styleSelectable(dG,R,false);this.__qN=true;}
;}
...
;}
;var dP=dK[0];var dI=dO;var dM=this.__fK;for(var dN in dM){if(dJ[dN]){delete dJ[dN];}
...
;if(!dL){return false;}
;this._scrollItemIntoView(dI);this._setLeadItem(dP);this._setAnchorItem(dP);this.__qN=true;this._fireChange();}
,_fireChange:function(dQ){if(this.__qN){this.__qO=dQ||null;this.fireDataEvent(bs,this.getSelection());delete this.__qN;}
;}
...
;return dS;}
;return null;}
},destruct:function(){this._disposeObjects(bA);this.__fK=this.__qR=this.__qQ=null;this.__qP=null;}
});}
)();
(function(){var a="vertical",b="under",c="above",d="qx.ui.core.selection.Widget",e="left",f="right";qx.Class.define(d,{extend:qx.ui.core.selection.Abstract,construct:function(g){qx.ui.core.selection.Abstract.call(this);this.__jS=g;}
,members:{__jS:null,_isSelectable:function(h){return this._isItemSelectable(h)&&h.getLayoutParent()===this.__jS;}
,_selectableToHashCode:function(j){return j.$$hash;}
,_styleSelectable:function(k,m,n){n?k.addState(m):k.removeState(m);}
,_capture:function(){this.__jS.capture();}
,_releaseCapture:function(){this.__jS.releaseCapture();}
,_isItemSelectable:function(o){if(this._userInteraction){return o.isVisible()&&o.isEnabled();}
else {return o.isVisible();}
;}
,_getWidget:function(){return this.__jS;}
,_getLocation:function(){var p=this.__jS.getContentElement().getDomElement();return p?qx.bom.element.Location.get(p):null;}
,_getDimension:function(){return this.__jS.getInnerSize();}
...
,_scrollItemIntoView:function(w){this.__jS.scrollChildIntoView(w);}
,getSelectables:function(x){var y=false;if(!x){y=this._userInteraction;this._userInteraction=true;}
;var B=this.__jS.getChildren();var z=[];var A;for(var i=0,l=B.length;i<l;i++ ){A=B[i];if(this._isItemSelectable(A)){z.push(A);}
;}
;this._userInteraction=y;return z;}
,_getSelectableRange:function(C,D){if(C===D){return [C];}
;var H=this.__jS.getChildren();var E=[];var G=false;var F;for(var i=0,l=H.length;i<l;i++ ){F=H[i];if(F===C||F===D){if(G){E.push(F);break;}
else {G=true;}
;}
;if(G&&this._isItemSelectable(F)){E.push(F);}
;}
;return E;}
,_getFirstSelectable:function(){var I=this.__jS.getChildren();for(var i=0,l=I.length;i<l;i++ ){if(this._isItemSelectable(I[i])){return I[i];}
;}
;return null;}
,_getLastSelectable:function(){var J=this.__jS.getChildren();for(var i=J.length-1;i>0;i-- ){if(this._isItemSelectable(J[i])){return J[i];}
;}
;return null;}
,_getRelatedSelectable:function(N,O){var L=this.__jS.getOrientation()===a;var P=this.__jS.getChildren();var K=P.indexOf(N);var M;if((L&&O===c)||(!L&&O===e)){for(var i=K-1;i>=0;i-- ){M=P[i];if(this._isItemSelectable(M)){return M;}
;}
;}
...
(function(){var a="indexOf",b="addAfter",c="add",d="addBefore",e="_",f="addAt",g="hasChildren",h="removeAt",i="removeAll",j="getChildren",k="remove",l="qx.ui.core.MRemoteChildrenHandling";qx.Mixin.define(l,{members:{__nH:function(q,m,o,n){var p=this.getChildrenContainer();if(p===this){q=e+q;}
;return (p[q])(m,o,n);}
,getChildren:function(){return this.__nH(j);}
,hasChildren:function(){return this.__nH(g);}
,add:function(s,r){return this.__nH(c,s,r);}
,remove:function(t){return this.__nH(k,t);}
,removeAll:function(){return this.__nH(i);}
,indexOf:function(u){return this.__nH(a,u);}
,addAt:function(x,v,w){this.__nH(f,x,v,w);}
,addBefore:function(y,A,z){this.__nH(d,y,A,z);}
,addAfter:function(D,B,C){this.__nH(b,D,B,C);}
,removeAt:function(E){return this.__nH(h,E);}
}});}
)();
...
,events:{"changeSelection":d},properties:{selectionMode:{check:[r,p,w,b],init:r,apply:v},dragSelection:{check:a,init:false,apply:g},quickSelection:{check:a,init:false,apply:q}},members:{__et:null,selectAll:function(){this.__et.selectAll();}
...
,_getLeadItem:function(){var O=this.__et.getMode();if(O===r||O===b){return this.__et.getSelectedItem();}
...
(function(){var a="qx.ui.form.List",b="text",c="Boolean",d="one",f="addChildWidget",g="_applySpacing",h="Enter",j="Integer",k="action",m="keyinput",n="qx.event.type.Data",o="addItem",p="vertical",q="removeChildWidget",r="",s="_applyOrientation",t="single",u="keypress",v="list",w="horizontal",x="label",y="pane",z="removeItem",A="__rC";qx.Class.define(a,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MMultiSelectionHandling,qx.ui.form.MForm,qx.ui.form.MModelSelection],construct:function(B){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__rC=this._createListItemContainer();this.__rC.addListener(f,this._onAddChild,this);this.__rC.addListener(q,this._onRemoveChild,this);this.getChildControl(y).add(this.__rC);if(B){this.setOrientation(w);}
else {this.initOrientation();}
;this.addListener(u,this._onKeyPress);this.addListener(m,this._onKeyInput);this.__ts=r;}
,events:{addItem:n,removeItem:n},properties:{appearance:{refine:true,init:v},focusable:{refine:true,init:true},orientation:{check:[w,p],init:p,apply:s},spacing:{check:j,init:0,apply:g,themeable:true},enableInlineFind:{check:c,init:true}},members:{__ts:null,__tt:null,__rC:null,SELECTION_MANAGER:qx.ui.core.selection.ScrollArea,getChildrenContainer:function(){return this.__rC;}
...
,_applyOrientation:function(F,C){var D=F===w;var E=D?new qx.ui.layout.HBox():new qx.ui.layout.VBox();var content=this.__rC;content.setLayout(E);content.setAllowGrowX(!D);content.setAllowGrowY(D);this._applySpacing(this.getSpacing());}
,_applySpacing:function(H,G){this.__rC.getLayout().setSpacing(H);}
...
;if(((new Date).valueOf()-this.__tt)>1000){this.__ts=r;}
;this.__ts+=e.getChar();var K=this.findItemByLabelFuzzy(this.__ts);if(K){this.setSelection([K]);}
;this.__tt=(new Date).valueOf();}
...
;if(this.getSelection()==null){this.__tA=new qx.data.Array();this.setSelection(this.__tA);}
;}
,properties:{selection:{check:b,event:f,apply:d,init:null}},events:{"changeSelection":h},members:{_modifingSelection:0,__tB:null,__tC:null,__tA:null,_applySelection:function(k,j){if(this.__tC!=undefined&&j!=undefined){j.removeListenerById(this.__tC);}
;this.__tC=k.addListener(a,this.__tD,this);this._updateSelection();}
,__tD:function(){this._updateSelection();}
,_changeTargetSelection:function(){if(this.getTarget()==null){return;}
;if(!this.__tE()&&!this.__tF()){return;}
;if(this._inSelectionModification()){return;}
;var m=this.getTarget().getSelection();var l=this.getSelection();if(l==null){l=new qx.data.Array();this.__tA=l;this.setSelection(l);}
...
,_addChangeTargetListener:function(p,o){if(this.__tB!=undefined&&o!=undefined){o.removeListenerById(this.__tB);}
;if(p!=null){if(this.__tE()||this.__tF()){this.__tB=p.addListener(f,this._changeTargetSelection,this);}
;}
;}
,_updateSelection:function(){if(!this.getTarget()){return;}
;this._startSelectionModification();if(this.__tE()){var s=[];for(var i=0;i<this.getSelection().length;i++ ){var t=this.getSelection().getItem(i);var u=this.__tH(t);if(u!=null){s.push(u);}
;}
...
;}
;}
else if(this.__tF()){var r=this.getSelection().getItem(this.getSelection().length-1);if(r!==undefined){this.__tG(r);this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();}
else {this.getTarget().resetSelection();}
;}
;this._endSelectionModification();}
,__tE:function(){var v=this.getTarget().constructor;return qx.Class.implementsInterface(v,qx.ui.core.IMultiSelection);}
,__tF:function(){var w=this.getTarget().constructor;return qx.Class.implementsInterface(w,qx.ui.core.ISingleSelection);}
,__tG:function(x){var y=this.__tH(x);if(y==null){return;}
;if(this.__tE()){this.getTarget().addToSelection(y);}
else if(this.__tF()){this.getTarget().setSelection([y]);}
;}
,__tH:function(z){var A=this.getTarget().getSelectables(true);for(var i=0;i<A.length;i++ ){if(A[i].getModel()==z){return A[i];}
;}
;return null;}
...
},destruct:function(){if(this.__tA){this.__tA.dispose();}
;}
});}
)();
...
(function(){var a="changeModel",b="_applyLabelOptions",c="_applyLabelPath",d="changeTarget",e="_applyModel",f="change",g="icon",h="ReverseBindingId",k="qx.data.controller.List",m="_applyIconPath",n="BindingId",o="_applyDelegate",p="",q="changeDelegate",r="_applyTarget",s="]",t="qx.data.IListData",u="model[",v="model",w="label",x="String",y="_applyIconOptions",z=".";qx.Class.define(k,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(A,B,C){qx.core.Object.call(this);this.__tI=[];this.__tJ=[];this.__tK=[];this.__tL={};if(C!=null){this.setLabelPath(C);}
;if(A!=null){this.setModel(A);}
;if(B!=null){this.setTarget(B);}
;}
,properties:{model:{check:t,apply:e,event:a,nullable:true,dereference:true},target:{apply:r,event:d,nullable:true,init:null,dereference:true},labelPath:{check:x,apply:c,nullable:true},iconPath:{check:x,apply:m,nullable:true},labelOptions:{apply:b,nullable:true},iconOptions:{apply:y,nullable:true},delegate:{apply:o,event:q,init:null,nullable:true}},members:{__tM:null,__tI:null,__tL:null,__tJ:null,__tK:null,__tN:null,__tO:null,update:function(){this.__tQ();this.__tU();this._updateSelection();}
,_applyDelegate:function(E,D){this._setConfigureItem(E,D);this._setFilter(E,D);this._setCreateItem(E,D);this._setBindItem(E,D);}
,_applyIconOptions:function(G,F){this.__tU();}
,_applyLabelOptions:function(I,H){this.__tU();}
,_applyIconPath:function(K,J){this.__tU();}
,_applyLabelPath:function(M,L){this.__tU();}
,_applyModel:function(P,N){if(N!=undefined){if(this.__tM!=undefined){N.removeListenerById(this.__tM);}
;}
;if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();}
;if(P!=null){this.__tM=P.addListener(f,this.__tP,this);this.__tW();this.__tQ();if(N==null){this._changeTargetSelection();}
else {this.__tN=true;qx.ui.core.queue.Widget.add(this);}
;}
else {var O=this.getTarget();if(O!=null){var length=O.getChildren().length;for(var i=0;i<length;i++ ){this.__tT();}
;}
;}
;}
...
;if(S!=null){if(this.getModel()!=null){for(var i=0;i<this.__tI.length;i++ ){this.__tS(this.__tX(i));}
;}
;}
;}
,__tP:function(){this.__tO=true;qx.ui.core.queue.Widget.add(this);if(this.__tI.length!=this.getModel().getLength()){this.update();}
;}
,syncWidget:function(){if(this.__tN){this._changeTargetSelection();}
;if(this.__tO){this._updateSelection();}
;this.__tO=this.__tN=null;}
,__tQ:function(){if(this.getTarget()==null){return;}
;this.__tW();var U=this.__tI.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++ ){this.__tS(this.__tX(j));}
;}
else if(U<T){for(var j=T;j>U;j-- ){this.__tT();}
;}
;this.__tN=true;qx.ui.core.queue.Widget.add(this);}
,__tR:function(){var V=this.getModel();if(V!=null){V.removeListenerById(this.__tM);this.__tM=V.addListener(f,this.__tP,this);}
;}
...
;return W;}
,__tS:function(Y){var ba=this._createItem();this._bindListItem(ba,Y);this.getTarget().add(ba);}
,__tT:function(){this._startSelectionModification();var bc=this.getTarget().getChildren();var bb=bc.length-1;var bd=bc[bb];this._removeBindingsFrom(bd);this.getTarget().removeAt(bb);bd.destroy();this._endSelectionModification();}
...
;}
,bindProperty:function(bo,br,bp,bn,bm){if(bp!=null){var bp=qx.lang.Object.clone(bp);this.__tL[br]=bp.onUpdate;delete bp.onUpdate;}
else {bp={};this.__tL[br]=null;}
;bp.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,bm);bp.ignoreConverter=v;var bq=u+bm+s;if(bo!=null&&bo!=p){bq+=z+bo;}
;var bs=this.bind(bq,bn,br,bp);bn.setUserData(br+n,bs);if(!qx.lang.Array.contains(this.__tJ,br)){this.__tJ.push(br);}
;}
,bindPropertyReverse:function(by,bv,bw,bx,bt){var bu=u+bt+s;if(by!=null&&by!=p){bu+=z+by;}
;var bz=bx.bind(bv,this,bu,bw);bx.setUserData(by+h,bz);if(!qx.lang.Array.contains(this.__tK,by)){this.__tK.push(by);}
;}
,_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;}
;for(var i=0;i<this.__tJ.length;i++ ){if(this.__tL[this.__tJ[i]]!=null){this.__tL[this.__tJ[i]]();}
;}
;}
,_removeBindingsFrom:function(bD){for(var i=0;i<this.__tJ.length;i++ ){var bE=bD.getUserData(this.__tJ[i]+n);if(bE!=null){this.removeBinding(bE);}
;}
;for(var i=0;i<this.__tK.length;i++ ){var bE=bD.getUserData(this.__tK[i]+h);if(bE!=null){bD.removeBinding(bE);}
;}
;}
,__tU:function(){if(this.getTarget()==null||this.getModel()==null){return;}
;var bF=this.getTarget().getChildren();for(var i=0;i<bF.length;i++ ){this._removeBindingsFrom(bF[i]);this._bindListItem(bF[i],this.__tX(i));}
;this.__tR();}
...
,_setFilter:function(bR,bP){if((bR==null||bR.filter==null)&&(bP!=null&&bP.filter!=null)){this.__tV();}
...
;var bQ=this.__tI;this.__tW();if(bQ.length>this.__tI.length){for(var j=bQ.length;j>this.__tI.length;j-- ){this.getTarget().removeAt(j-1).destroy();}
;}
else if(bQ.length<this.__tI.length){for(var j=bQ.length;j<this.__tI.length;j++ ){var bT=this._createItem();this.getTarget().add(bT);}
;}
;var bS=this.getTarget().getChildren();for(var i=0;i<bS.length;i++ ){this._bindListItem(bS[i],this.__tX(i));}
;this.__tR();this._endSelectionModification();this._updateSelection();}
,__tV:function(){this.__tW();this.__tQ();this.__tU();this.__tO=true;qx.ui.core.queue.Widget.add(this);}
,__tW:function(){var bW=this.getModel();if(bW==null){return;}
;var bX=this.getDelegate();if(bX!=null){var bV=bX.filter;}
;this.__tI=[];for(var i=0;i<bW.getLength();i++ ){if(bV==null||bV(bW.getItem(i))){this.__tI.push(i);}
;}
;}
,__tX:function(bY){return this.__tI[bY];}
},destruct:function(){this.__tI=this.__tL=this.__tJ=null;this.__tK=null;qx.ui.core.queue.Widget.remove(this);}
});}
)();
...
(function(){var a="changeModel",b="application/json",c="json",d="aborted",e="loaded",f="qx.event.type.Data",g="error",h="fail",i="receiving",j="queued",k="changeState",l="success",m="String",n="changePhase",o="sending",p="_marshaler",q="completed",r="failed",s="qx.data.store.Json",t="configured",u="changeUrl",v="__uk",w="timeout",x="_applyUrl";qx.Class.define(s,{extend:qx.core.Object,construct:function(y,z){qx.core.Object.call(this);this._marshaler=new qx.data.marshal.Json(z);this._delegate=z;if(y!=null){this.setUrl(y);}
;}
,events:{"loaded":f,"error":f},properties:{model:{nullable:true,event:a},state:{check:[t,j,o,i,q,d,w,r],init:t,event:k},url:{check:m,apply:x,event:u,nullable:true}},members:{_marshaler:null,_delegate:null,__uk:null,_applyUrl:function(B,A){if(B!=null){B=qx.util.AliasManager.getInstance().resolve(B);B=qx.util.ResourceManager.getInstance().toUri(B);this._createRequest(B);}
;}
,_getRequest:function(){return this.__uk;}
,_setRequest:function(C){this.__uk=C;}
,_createRequest:function(D){if(this.__uk){this.__uk.dispose();this.__uk=null;}
...
,_onChangePhase:function(I){var G=I.getData(),J={},H;J={"opened":t,"sent":o,"loading":i,"success":q,"abort":d,"timeout":w,"statusError":r};H=J[G];if(H){this.setState(H);}
;}
...
;this.fireDataEvent(e,this.getModel());if(this.__uk){this.__uk.dispose();this.__uk=null;}
;}
,reload:function(){var R=this.getUrl();if(R!=null){this._createRequest(R);}
;}
},destruct:function(){if(this.__uk!=null){this._disposeObjects(v);}
...
;this.__rD={};var S=this._transport=this._createTransport();this._setPhase(H);this.__rE=qx.lang.Function.bind(this._onReadyStateChange,this);this.__rF=qx.lang.Function.bind(this._onLoad,this);this.__rG=qx.lang.Function.bind(this._onLoadEnd,this);this.__rH=qx.lang.Function.bind(this._onAbort,this);this.__rI=qx.lang.Function.bind(this._onTimeout,this);this.__rJ=qx.lang.Function.bind(this._onError,this);S.onreadystatechange=this.__rE;S.onload=this.__rF;S.onloadend=this.__rG;S.onabort=this.__rH;S.ontimeout=this.__rI;S.onerror=this.__rJ;}
...
,nullable:true},authentication:{check:k,nullable:true}},members:{__rE:null,__rF:null,__rG:null,__rH:null,__rI:null,__rJ:null,__rK:null,__rL:null,__rM:null,__rD:null,__rN:null,_transport:null,_createTransport:function(){throw new Error(F);}
...
;this.__rL=true;this.__rM=t;this._transport.abort();}
...
;}
,_getAllRequestHeaders:function(){var bd={};qx.lang.Object.mergeWith(bd,this._getConfiguredRequestHeaders());qx.lang.Object.mergeWith(bd,this.__rO());qx.lang.Object.mergeWith(bd,this.__rN);qx.lang.Object.mergeWith(bd,this.__rD);return bd;}
,__rO:function(){var bf=this.getAuthentication(),be={};if(bf){bf.getAuthHeaders().forEach(function(bg){be[bg.key]=bg.value;}
);return be;}
;}
,setRequestHeader:function(bh,bi){this.__rD[bh]=bi;}
,getRequestHeader:function(bj){return this.__rD[bj];}
,removeRequestHeader:function(bk){if(this.__rD[bk]){delete this.__rD[bk];}
;}
,getTransport:function(){return this._transport;}
,getReadyState:function(){return this._transport.readyState;}
,getPhase:function(){return this.__rM;}
...
,getResponse:function(){return this.__rK;}
,_setResponse:function(bo){var bn=bo;if(this.__rK!==bo){this.__rK=bo;this.fireEvent(I,qx.event.type.Data,[this.__rK,bn]);}
;}
,_onReadyStateChange:function(){var bp=this.getReadyState();if(qx.core.Environment.get(f)){this.debug(C+bp);}
;this.fireEvent(E);if(this.__rL){return;}
;if(bp===3){this._setPhase(q);}
;if(this.isDone()){this.__rP();}
;}
,__rP:function(){if(qx.core.Environment.get(f)){this.debug(B+this.getStatus());}
...
,_setPhase:function(br){var bs=this.__rM;if(qx.core.Environment.get(n)){qx.core.Assert.assertString(br);qx.core.Assert.assertMatch(br,/^(unsent)|(opened)|(sent)|(loading)|(load)|(success)|(abort)|(timeout)|(statusError)$/);}
;this.__rM=br;this.fireDataEvent(c,br,bs);}
...
;var A=z[h+qx.lang.String.firstUp(name)]();if(qx.lang.Type.isArray(A)){var D=qx.data&&qx.data.IListData&&qx.Class.hasInterface(A&&A.constructor,qx.data.IListData);for(var i=0;i<A.length;i++ ){var F=D?A.getItem(i):A[i];E+=this.__rQ(name,F,C);}
;}
else if(qx.lang.Type.isDate(A)&&y!=null){E+=this.__rQ(name,y.format(A),C);}
else {E+=this.__rQ(name,A,C);}
;}
;return E.substring(0,E.length-1);}
,__rQ:function(name,I,G){if(I&&I.$$type==o){I=I.classname;}
...
;}
,statics:{PARSER:{json:qx.lang.Json.parse,xml:qx.xml.Document.fromString}},members:{__rR:null,parse:function(h,g){var f=this._getParser(g);if(typeof f===a){if(h!==c){return f.call(this,h);}
;}
;return h;}
,setParser:function(i){if(typeof qx.util.ResponseParser.PARSER[i]===a){return this.__rR=qx.util.ResponseParser.PARSER[i];}
;if(qx.core.Environment.get(d)){qx.core.Assert.assertFunction(i);}
;return this.__rR=i;}
,_getParser:function(k){var j=this.__rR,m=c,l=c;if(j){return j;}
...
(function(){var a="Retry open native request with method: ",b="activex",c="No XHR support available.",d="If-None-Match",f="xhr",g="If-Modified-Since",h="engine.version",i="onunload",j="GET",k="-1",l="qx.debug.io",m="HTMLDocument",n="Open native request with method: ",o="error",p="loadend",q="Blob",r="load",s="Received native readyState: ",t="abort",u=", url: ",v="String",w="browser.documentmode",x="",y="readystatechange",z="Microsoft.XMLHTTP",A="Already disposed",B="browser.version",C="opera",D="qx.bom.request.Xhr",E="on",F="timeout",G="gecko",H="If-Match",I="mshtml",J="engine.name",K="Microsoft.XMLDOM",L="file:",M="Send native request",N="FormData",O="If-Range",P="Content-Type",Q="io.xhr",R="Not enough arguments",S="ArrayBuffer",T="undefined",U=", async: ",V="Native XHR object doesn't support overrideMimeType.";qx.Bootstrap.define(D,{extend:Object,construct:function(){var W=qx.Bootstrap.bind(this.__sh,this);if(qx.event&&qx.event.GlobalError&&qx.event.GlobalError.observeMethod){this.__rS=qx.event.GlobalError.observeMethod(W);}
else {this.__rS=W;}
;this.__rT=qx.Bootstrap.bind(this.__sg,this);this.__rI=qx.Bootstrap.bind(this.__sl,this);this.__sf();this._emitter=new qx.event.Emitter();if(window.attachEvent){this.__rU=qx.Bootstrap.bind(this.__so,this);window.attachEvent(i,this.__rU);}
;}
,statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},events:{"readystatechange":D,"error":D,"loadend":D,"timeout":D,"abort":D,"load":D},members:{readyState:0,responseText:x,responseXML:null,status:0,statusText:x,timeout:0,open:function(bc,X,Y,bb,ba){this.__sq();if(typeof X===T){throw new Error(R);}
else if(typeof bc===T){bc=j;}
;this.__rL=false;this.__rV=false;this.__rW=false;this.__rX=X;if(typeof Y==T){Y=true;}
;this.__rY=Y;if(!this.__sp()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();this.__sf();}
;this.__sb.onreadystatechange=this.__rS;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,n+bc+u+X+U+Y);}
;this.__sb.open(bc,X,Y,bb,ba);}
catch(bd){if(!qx.util.Request.isCrossDomain(X)){throw bd;}
;if(!this.__rY){this.__sa=bd;}
;if(this.__rY){if(window.XDomainRequest){this.readyState=4;this.__sb=new XDomainRequest();this.__sb.onerror=qx.Bootstrap.bind(function(){this._emit(y);this._emit(o);this._emit(p);}
,this);if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,a+bc+u+X+U+Y);}
;this.__sb.open(bc,X,Y,bb,ba);return;}
;window.setTimeout(qx.Bootstrap.bind(function(){if(this.__sd){return;}
;this.readyState=4;this._emit(y);this._emit(o);this._emit(p);}
,this));}
;}
;if(qx.core.Environment.get(J)===I&&qx.core.Environment.get(w)<9&&this.__sb.readyState>0){this.__sb.setRequestHeader(g,k);}
;if(qx.core.Environment.get(J)===G&&parseInt(qx.core.Environment.get(h),10)<2&&!this.__rY){this.readyState=qx.bom.request.Xhr.OPENED;this._emit(y);}
;}
,setRequestHeader:function(be,bf){this.__sq();if(be==H||be==g||be==d||be==O){this.__rW=true;}
;this.__sb.setRequestHeader(be,bf);return this;}
,send:function(bh){this.__sq();if(!this.__rY&&this.__sa){throw this.__sa;}
;if(qx.core.Environment.get(J)===C&&this.timeout===0){this.timeout=10000;}
;if(this.timeout>0){this.__sc=window.setTimeout(this.__rI,this.timeout);}
;bh=typeof bh==T?null:bh;var bg=qx.Bootstrap.getClass(bh);bh=(bh!==null&&this.__se.indexOf(bg)===-1)?bh.toString():bh;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,M);}
;this.__sb.send(bh);}
catch(bj){if(!this.__rY){throw bj;}
;if(this._getProtocol()===L){this.readyState=2;this.__si();var bi=this;window.setTimeout(function(){if(bi.__sd){return;}
;bi.readyState=3;bi.__si();bi.readyState=4;bi.__si();}
);}
;}
;if(qx.core.Environment.get(J)===G&&!this.__rY){this.__sh();}
;this.__rV=true;return this;}
,abort:function(){this.__sq();this.__rL=true;this.__sb.abort();if(this.__sb){this.readyState=this.__sb.readyState;}
;return this;}
...
,getResponseHeader:function(bm){this.__sq();if(qx.core.Environment.get(w)===9&&this.__sb.aborted){return x;}
;return this.__sb.getResponseHeader(bm);}
,getAllResponseHeaders:function(){this.__sq();if(qx.core.Environment.get(w)===9&&this.__sb.aborted){return x;}
;return this.__sb.getAllResponseHeaders();}
,overrideMimeType:function(bn){this.__sq();if(this.__sb.overrideMimeType){this.__sb.overrideMimeType(bn);}
else {throw new Error(V);}
;return this;}
,getRequest:function(){return this.__sb;}
,dispose:function(){if(this.__sd){return false;}
;window.clearTimeout(this.__sc);if(window.detachEvent){window.detachEvent(i,this.__rU);}
;try{this.__sb.onreadystatechange;}
catch(bp){return false;}
;var bo=function(){}
;this.__sb.onreadystatechange=bo;this.__sb.onload=bo;this.__sb.onerror=bo;this.abort();this.__sb=null;this.__sd=true;return true;}
,isDisposed:function(){return !!this.__sd;}
...
,_getProtocol:function(){var br=this.__rX;var bs=/^(\w+:)\/\//;if(br!==null&&br.match){var bt=br.match(bs);if(bt&&bt[1]){return bt[1];}
;}
;return window.location.protocol;}
,__sb:null,__rY:null,__rS:null,__rT:null,__rU:null,__rI:null,__rV:null,__rX:null,__rL:null,__es:null,__sd:null,__sc:null,__sa:null,__rW:null,__se:null,__sf:function(){this.__sb=this._createNativeXhr();this.__sb.onreadystatechange=this.__rS;if(this.__sb.onabort){this.__sb.onabort=this.__rT;}
;this.__sd=this.__rV=this.__rL=false;this.__se=[S,q,m,v,N];}
,__sg:function(){if(!this.__rL){this.abort();}
;}
,__sh:function(){var bu=this.__sb,bv=true;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,s+bu.readyState);}
;if(this.readyState==bu.readyState){return;}
;this.readyState=bu.readyState;if(this.readyState===qx.bom.request.Xhr.DONE&&this.__rL&&!this.__rV){return;}
;if(!this.__rY&&(bu.readyState==2||bu.readyState==3)){return;}
...
;if(bv){this.__sm();this.__sn();}
;}
;this.__si();if(this.readyState==qx.bom.request.Xhr.DONE){if(bu){bu.onreadystatechange=function(){}
;}
;}
;}
,__si:function(){if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__sc);}
;this._emit(y);if(this.readyState===qx.bom.request.Xhr.DONE){this.__sj();}
;}
,__sj:function(){if(this.__es){this._emit(F);if(qx.core.Environment.get(J)===C){this._emit(o);}
;this.__es=false;}
else {if(this.__rL){this._emit(t);}
else {if(this.__sk()){this._emit(o);}
else {this._emit(r);}
;}
;}
;this._emit(p);}
,__sk:function(){var bx;if(this._getProtocol()===L){bx=!this.responseText;}
else {bx=!this.statusText&&this.status!==204;}
;return bx;}
,__sl:function(){var by=this.__sb;this.readyState=qx.bom.request.Xhr.DONE;this.__es=true;by.aborted=true;by.abort();this.responseText=x;this.responseXML=null;this.__si();}
,__sm:function(){var bz=this.readyState===qx.bom.request.Xhr.DONE;if(this._getProtocol()===L&&this.status===0&&bz){if(!this.__sk()){this.status=200;}
;}
;if(this.status===1223){this.status=204;}
;if(qx.core.Environment.get(J)===C){if(bz&&this.__rW&&!this.__rL&&this.status===0){this.status=304;}
;}
;}
,__sn:function(){if(qx.core.Environment.get(J)==I&&(this.getResponseHeader(P)||x).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var bA=new window.ActiveXObject(K);bA.async=false;bA.validateOnParse=false;bA.loadXML(this.responseText);this.responseXML=bA;}
;}
,__so:function(){try{if(this){this.dispose();}
;}
catch(e){}
;}
,__sp:function(){var name=qx.core.Environment.get(J);var bB=qx.core.Environment.get(B);return !(name==I&&bB<9||name==G&&bB<3.5);}
,__sq:function(){if(this.__sd){throw new Error(A);}
;}
...
},members:{__ul:null,__um:function(z){return Object.keys(z).sort().join(a);}
,toClass:function(A,B){this.__un(A,B,null,0);}
,__un:function(N,I,C,K){if(!qx.lang.Type.isObject(N)||!!N.$$isString||N instanceof qx.core.Object){if(N instanceof Array||qx.Bootstrap.getClass(N)==h){for(var i=0;i<N.length;i++ ){this.__un(N[i],I,C+o+i+m,K+1);}
;}
;return;}
;var E=this.__um(N);if(this.__ur(E,C,K)){return;}
;for(var M in N){this.__un(N[M],I,M,K+1);}
;if(qx.Class.isDefined(s+E)){return;}
;if(this.__ul&&this.__ul.getModelClass&&this.__ul.getModelClass(E,N,C,K)!=null){return;}
;var O={};var F={__uo:this.__uo};for(var M in N){if(this.__ul&&this.__ul.getPropertyMapping){M=this.__ul.getPropertyMapping(M,E);}
...
;if(this.__ul&&this.__ul.getValidationRule){var H=this.__ul.getValidationRule(E,M);if(H){O[M].validate=k+M;F[k+M]=H;}
;}
;}
;if(this.__ul&&this.__ul.getModelSuperClass){var L=this.__ul.getModelSuperClass(E,C,K)||qx.core.Object;}
else {var L=qx.core.Object;}
;var G=[];if(this.__ul&&this.__ul.getModelMixins){var J=this.__ul.getModelMixins(E,C,K);if(!qx.lang.Type.isArray(J)){if(J!=null){G=[J];}
;}
else {G=J;}
;}
;if(I){G.push(qx.data.marshal.MEventBubbling);}
;var D={extend:L,include:G,properties:O,members:F,destruct:this.__up};qx.Class.define(s+E,D);}
,__up:function(){var P=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var Q in P){this.__uo(this.get(P[Q].name));}
;}
,__uo:function(R){if(!(R instanceof qx.core.Object)){return;}
;if(R.isDisposed()){return;}
;R.dispose();}
,__uq:function(S,W,X,V){var Y;if(this.__ul&&this.__ul.getModelClass){Y=this.__ul.getModelClass(S,W,X,V);}
;if(Y!=null){return (new Y());}
else {var U=s+S;var T=qx.Class.getByName(U);if(!T){throw new Error(b+U+l);}
;return (new T());}
;}
,__ur:function(ba,bd,bb){var bc=this.__ul;return bc&&bc.ignore&&bc.ignore(ba,bd,bb);}
,toModel:function(be){return this.__us(be,null,0);}
,__us:function(bq,bg,bm){var bk=qx.lang.Type.isObject(bq);var bf=bq instanceof Array||qx.Bootstrap.getClass(bq)==h;if((!bk&&!bf)||!!bq.$$isString||bq instanceof qx.core.Object){return bq;}
else if(this.__ur(this.__um(bq),bg,bm)){return bq;}
else if(bf){var bo=qx.data.Array;if(this.__ul&&this.__ul.getArrayClass){var bl=this.__ul.getArrayClass(bg,bm);bo=bl||bo;}
;var br=new bo();br.setAutoDisposeItems(true);for(var i=0;i<bq.length;i++ ){br.push(this.__us(bq[i],bg+o+i+m,bm+1));}
;return br;}
else if(bk){var bh=this.__um(bq);var bp=this.__uq(bh,bq,bg,bm);for(var bn in bq){var bi=bn;if(this.__ul&&this.__ul.getPropertyMapping){bi=this.__ul.getPropertyMapping(bn,bh);}
;var bs=bi.replace(/-|\.|\s+/g,d);if((qx.core.Environment.get(t))&&qx.core.Environment.get(e)){if(bs!=bi){this.warn(c+bn+n+bi+v);}
;}
;bi=bs;var bj=p+qx.lang.String.firstUp(bi);if(bp[bj]){bp[bj](this.__us(bq[bn],bn,bm+1));}
;}
;return bp;}
;throw new Error(j);}
