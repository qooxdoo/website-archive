...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.NamesList.789665579d89.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="persons.json</a> and bind the items to the list widget. In the ",b="converter for the label, the names will be set to both, the first ",c="model.persons",d="Loading...",e="' target='_blank'>",f="demobrowser/demo/data/persons.json",g="model",h="no model...",i="state",j="and last name.",k="value",l="demobrowser.demo.data.NamesList",m="firstname",n="<b>List bound to data in a json file</b><br/>",o=", ",p="Loading the json file <a href='";qx.Class.define(l,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var s=new qx.ui.form.List();this.getRoot().add(s,{left:10,top:80});var t=new qx.data.controller.List(null,s);t.setLabelPath(m);t.setLabelOptions({converter:function(w,v){return v?v.getLastname()+o+w:h;}
});var r=qx.util.ResourceManager.getInstance().toUri(f);var u=new qx.data.store.Json(r);var status=new qx.ui.basic.Label(d);this.getRoot().add(status,{left:120,top:80});u.bind(c,t,g);u.bind(i,status,k);var q=new qx.ui.basic.Label();q.setRich(true);q.setWidth(470);q.setValue(n+p+r+e+a+b+j);this.getRoot().add(q,{left:10,top:10});}
}});}
)();
