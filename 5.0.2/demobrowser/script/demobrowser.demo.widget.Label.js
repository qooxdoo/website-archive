...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Label.171fed7f7adb.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="qx.ui.window.Manager",b="__og";qx.Class.define(a,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__og:null,setDesktop:function(c){this.__og=c;this.updateStack();}
...
,syncWidget:function(){this.__og.forceUnblock();var h=this.__og.getWindows();var g=this._minZIndex;var m=g+h.length*2;var j=g+h.length*4;var k=null;for(var i=0,l=h.length;i<l;i++ ){var f=h[i];if(!f.isVisible()){continue;}
;k=k||f;if(f.isModal()){f.setZIndex(j);this.__og.blockContent(j-1);j+=2;k=f;}
...
,bringToFront:function(o){var n=this.__og.getWindows();var p=qx.lang.Array.remove(n,o);if(p){n.push(o);this.updateStack();}
;}
,sendToBack:function(r){var q=this.__og.getWindows();var s=qx.lang.Array.remove(q,r);if(s){q.unshift(r);this.updateStack();}
;}
},destruct:function(){this._disposeObjects(b);}
});}
)();
(function(){var a="Boolean",b="w-resize",c="touch",d="sw-resize",f="n-resize",g="resizableRight",h="ne-resize",i="se-resize",j="Integer",k="e-resize",l="resizableLeft",m="move",n="shorthand",o="maximized",p="resize",q="nw-resize",r="pointerup",s="pointerdown",t="qx.ui.core.MResizable",u="losecapture",v="resize-frame",w="resizableBottom",x="s-resize",y="pointermove",z="resizableTop",A="pointerout";qx.Mixin.define(t,{construct:function(){var content=this.getContentElement();content.addListener(s,this.__ot,this,true);content.addListener(r,this.__ou,this);content.addListener(y,this.__ow,this);content.addListener(A,this.__ox,this);content.addListener(u,this.__ov,this);var B=content.getDomElement();if(B==null){B=window;}
;this.__oh=qx.event.Registration.getManager(B).getHandler(qx.event.handler.DragDrop);}
,properties:{resizableTop:{check:a,init:true},resizableRight:{check:a,init:true},resizableBottom:{check:a,init:true},resizableLeft:{check:a,init:true},resizable:{group:[z,g,w,l],mode:n},resizeSensitivity:{check:j,init:5},useResizeFrame:{check:a,init:true}},members:{__oh:null,__oi:null,__oj:null,__ok:null,__ol:null,__om:null,__on:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__oi;if(!C){C=this.__oi=new qx.ui.core.Widget();C.setAppearance(v);C.exclude();qx.core.Init.getApplication().getRoot().add(C);}
;return C;}
,__oo:function(){var location=this.getContentLocation();var D=this._getResizeFrame();D.setUserBounds(location.left,location.top,location.right-location.left,location.bottom-location.top);D.show();D.setZIndex(this.getZIndex()+1);}
,__op:function(e){var F=this.__oj;var E=this.getSizeHint();var I=this.__on;var H=this.__om;var J=H.width;var G=H.height;var L=H.left;var top=H.top;var K;if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){K=Math.max(I.top,Math.min(I.bottom,e.getDocumentTop()))-this.__ol;if(F&this.RESIZE_TOP){G-=K;}
else {G+=K;}
;if(G<E.minHeight){G=E.minHeight;}
else if(G>E.maxHeight){G=E.maxHeight;}
;if(F&this.RESIZE_TOP){top+=H.height-G;}
;}
;if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){K=Math.max(I.left,Math.min(I.right,e.getDocumentLeft()))-this.__ok;if(F&this.RESIZE_LEFT){J-=K;}
else {J+=K;}
...
,__oq:{'1':f,'2':x,'4':b,'8':k,'5':q,'6':d,'9':h,'10':i},__or:function(e){var location=this.getContentLocation();var O=this.getResizeSensitivity();var N=e.getDocumentLeft();var P=e.getDocumentTop();var M=this.__os(location,N,P,O);if(M>0){M=M|this.__os(location,N,P,O*2);}
;this.__oj=M;}
,__os:function(location,T,R,S){var Q=0;if(this.getResizableTop()&&Math.abs(location.top-R)<S&&T>location.left-S&&T<location.right+S){Q+=this.RESIZE_TOP;}
...
;return Q;}
,__ot:function(e){if(!this.__oj||!this.getEnabled()||e.getPointerType()==c){return;}
;this.addState(p);this.__ok=e.getDocumentLeft();this.__ol=e.getDocumentTop();var location=this.getContentLocation();var V=this.getBounds();this.__om={top:location.top,left:location.left,width:location.right-location.left,height:location.bottom-location.top,bounds:qx.lang.Object.clone(V)};var parent=this.getLayoutParent();var W=parent.getContentLocation();var U=parent.getBounds();this.__on={left:W.left,top:W.top,right:W.left+U.width,bottom:W.top+U.height};if(this.getUseResizeFrame()){this.__oo();}
;this.capture();e.stop();}
,__ou:function(e){if(!this.hasState(p)||!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.getUseResizeFrame()){this._getResizeFrame().exclude();}
;var X=this.__op(e);this.setWidth(X.width);this.setHeight(X.height);if(this.getResizableLeft()||this.getResizableTop()){this.setLayoutProperties({left:X.parentLeft,top:X.parentTop});}
;this.__oj=0;this.removeState(p);this.resetCursor();this.getApplicationRoot().resetGlobalCursor();this.releaseCapture();e.stopPropagation();}
,__ov:function(e){if(!this.__oj){return;}
...
;}
,__ow:function(e){if(!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.hasState(p)){var ba=this.__op(e);if(this.getUseResizeFrame()){var Y=this._getResizeFrame();Y.setUserBounds(ba.viewportLeft,ba.viewportTop,ba.width,ba.height);}
...
;}
;e.stopPropagation();}
else if(!this.hasState(o)&&!this.__oh.isSessionActive()){this.__or(e);var bd=this.__oj;var bc=this.getApplicationRoot();if(bd){var bb=this.__oq[bd];this.setCursor(bb);bc.setGlobalCursor(bb);}
else if(this.getCursor()){this.resetCursor();bc.resetGlobalCursor();}
;}
;}
,__ox:function(e){if(e.getPointerType()==c){return;}
;if(this.getCursor()&&!this.hasState(p)){this.resetCursor();this.getApplicationRoot().resetGlobalCursor();}
;}
},destruct:function(){if(this.__oi!=null&&!qx.core.ObjectRegistry.inShutDown){this.__oi.destroy();this.__oi=null;}
;this.__oh=null;}
});}
)();
...
(function(){var a="pointerup",b="__oy",c="Boolean",d="losecapture",f="move",g="qx.ui.core.MMovable",h="__oz",i="The move handle could not be redefined!",j="pointerdown",k="maximized",l="pointermove",m="roll",n="move-frame";qx.Mixin.define(g,{properties:{movable:{check:c,init:true},useMoveFrame:{check:c,init:false}},members:{__oy:null,__oz:null,__oA:null,__oB:null,__oC:null,__oD:null,__oE:null,__oF:false,__oG:null,__oH:0,_activateMoveHandle:function(o){if(this.__oy){throw new Error(i);}
;this.__oy=o;o.addListener(j,this._onMovePointerDown,this);o.addListener(a,this._onMovePointerUp,this);o.addListener(l,this._onMovePointerMove,this);o.addListener(d,this.__oL,this);}
,__oI:function(){var p=this.__oz;if(!p){p=this.__oz=new qx.ui.core.Widget();p.setAppearance(n);p.exclude();qx.core.Init.getApplication().getRoot().add(p);}
;return p;}
,__oJ:function(){var location=this.getContentLocation();var r=this.getBounds();var q=this.__oI();q.setUserBounds(location.left,location.top,r.width,r.height);q.show();q.setZIndex(this.getZIndex()+1);}
,__oK:function(e){var t=this.__oA;var s=Math.max(t.left,Math.min(t.right,e.getDocumentLeft()));var v=Math.max(t.top,Math.min(t.bottom,e.getDocumentTop()));var u=this.__oB+s;var w=this.__oC+v;return {viewportLeft:parseInt(u,10),viewportTop:parseInt(w,10),parentLeft:parseInt(u-this.__oD,10),parentTop:parseInt(w-this.__oE,10)};}
,_onMoveRoll:function(e){e.stop();}
,_onMovePointerDown:function(e){if(!this.getMovable()||this.hasState(k)){return;}
;this.addListener(m,this._onMoveRoll,this);var parent=this.getLayoutParent();var x=parent.getContentLocation();var z=parent.getBounds();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isBlocked()){this.__oG=parent.getBlockerColor();this.__oH=parent.getBlockerOpacity();parent.setBlockerColor(null);parent.setBlockerOpacity(1);parent.blockContent(this.getZIndex()-1);this.__oF=true;}
;}
;this.__oA={left:x.left,top:x.top,right:x.left+z.width,bottom:x.top+z.height};var y=this.getContentLocation();this.__oD=x.left;this.__oE=x.top;this.__oB=y.left-e.getDocumentLeft();this.__oC=y.top-e.getDocumentTop();this.addState(f);this.__oy.capture();if(this.getUseMoveFrame()){this.__oJ();}
;e.stop();}
,_onMovePointerMove:function(e){if(!this.hasState(f)){return;}
;var B=this.__oK(e);if(this.getUseMoveFrame()){this.__oI().setDomPosition(B.viewportLeft,B.viewportTop);}
else {var A=this.getLayoutParent().getInsets();this.setDomPosition(B.parentLeft-(A.left||0),B.parentTop-(A.top||0));}
;e.stopPropagation();}
,_onMovePointerUp:function(e){if(this.hasListener(m)){this.removeListener(m,this._onMoveRoll,this);}
;if(!this.hasState(f)){return;}
;this.removeState(f);var parent=this.getLayoutParent();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__oF){parent.unblock();parent.setBlockerColor(this.__oG);parent.setBlockerOpacity(this.__oH);this.__oG=null;this.__oH=0;this.__oF=false;}
;}
;this.__oy.releaseCapture();var D=this.__oK(e);var C=this.getLayoutParent().getInsets();this.setLayoutProperties({left:D.parentLeft-(C.left||0),top:D.parentTop-(C.top||0)});if(this.getUseMoveFrame()){this.__oI().exclude();}
;e.stopPropagation();}
,__oL:function(e){if(!this.hasState(f)){return;}
;this.removeState(f);if(this.getUseMoveFrame()){this.__oI().exclude();}
;}
},destruct:function(){this._disposeObjects(h,b);this.__oA=null;}
});}
)();
...
;var br=this.getLayoutProperties();this.__oQ=br.left===undefined?0:br.left;this.__oP=br.top===undefined?0:br.top;this.setLayoutProperties({left:null,top:null,edge:0});this.addState(F);this._updateCaptionBar();this.fireEvent(Q);}
;}
;}
,minimize:function(){if(!this.isVisible()){return;}
;if(this.fireNonBubblingEvent(b,qx.event.type.Event,[false,true])){var bs=this.getLayoutProperties();this.__oQ=bs.left===undefined?0:bs.left;this.__oP=bs.top===undefined?0:bs.top;this.removeState(F);this.hide();this.fireEvent(v);}
;}
...
;var bt=this.__oQ;var top=this.__oP;this.setLayoutProperties({edge:null,left:bt,top:top});this.removeState(F);this._updateCaptionBar();this.fireEvent(S);}
;}
