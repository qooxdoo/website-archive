...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.f097858850e3.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b=" => ",c="#autoActionsBeforeOnexit",d=": Could not find friendly name for '",f=": event '",g="__Ha",h=" blocked.  Re-queuing.",j="string",k="'",l="Attempt to transition to nonexistent state ",m="Saved-state stack is empty",n="Internal error: invalid nextState",o=" not handled.  Ignoring.",p="State ",q="#autoActionsBeforeOntransition",r=" returned a value other than ",s="#",u=" already exists",v="Attempt to transition to POP_STATE_STACK ",w=":",x="Transition ",y=" does not exist",z="' on '",A="qx.util.fsm.FiniteStateMachine",B="String",C="#autoActionsAfterOntransition",D="",E="__GY",F="#actionsAfterOnentry",G="Saved-state stack is full",H="#autoActionsBeforeOnentry",I=": no transition found.  No state change.",J="#autoActionsAfterOnentry",K="#actionsBeforeOnentry",L=": Pushed event: ",M="Machine started with no available states",N=": Process event: ",O="#entry",P="#ontransition",Q=" on ",R="Invalid state: not an instance of ",S="Number",T="#exit",U="true, false, or null.",V="Explicit transition ",W="  ",X="qx.util.fsm.State",Y="while state stack is empty.",bj=".  Ignoring.",bk="#autoActionsAfterOnexit",bl=": Event '",bg="object",bh=" not handled for target ",bi=": Queued event: ";qx.Class.define(A,{extend:qx.core.Object,construct:function(bm){qx.core.Object.call(this);this.setName(bm);this.__iy={};this.__GW=null;this.__GX=[];this.__GY=[];this.__Ha=[];this.__Hb={};this.__Hc={};this.__Hd={};this.__He={};this.__Hf={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:B,nullable:true},state:{check:B,nullable:true},previousState:{check:B,nullable:true},nextState:{check:B,nullable:true},maxSavedStates:{check:S,init:2},debugFlags:{check:S,init:7}},members:{__iy:null,__GW:null,__GY:null,__Ha:null,__GX:null,__Hb:null,__Hc:null,__Hd:null,__He:null,__Hf:null,__Hg:false,addState:function(bn){if(!bn instanceof qx.util.fsm.State){throw new Error(R+X);}
;var bo=bn.getName();if(bo in this.__iy){throw new Error(p+bo+u);}
;if(this.__GW==null){this.__GW=bo;}
;this.__iy[bo]=bn;}
...
;return bp;}
,addObject:function(bw,bt,bu){var bx=bt.toHashCode();this.__Hc[bw]=bx;this.__Hd[bx]=bw;this.__Hb[bw]=bt;if(!bu){return;}
;if(typeof (bu)==j){bu=[bu];}
;for(var i=0;i<bu.length;i++ ){var bv=bu[i];if(!this.__He[bv]){this.__He[bv]={};}
;this.__He[bv][bw]=true;if(!this.__Hf[bw]){this.__Hf[bw]=[];}
;this.__Hf[bw].push(bv);}
;}
,removeObject:function(bB){var by;var bz;var bA;var bC;by=this.__Hc[bB];if(this.__Hf[bB]){for(var i=0;i<this.__Hf[bB].length;i++ ){bz=this.__Hf[bB][i];delete this.__He[bz][bB];bC=true;for(bA in this.__He[bz]){bC=false;break;}
;if(bC){delete this.__He[bz];}
;}
;delete this.__Hf[bB];}
;delete this.__Hd[by];delete this.__Hc[bB];delete this.__Hb[bB];}
,getObject:function(bD){return this.__Hb[bD];}
,getFriendlyName:function(bE){var bF=bE?bE.toHashCode():null;return bF?this.__Hd[bF]:null;}
,getGroupObjects:function(bG){var a=[];for(var name in this.__He[bG]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bH in this.__Hc){var bJ=this.__Hc[bH];var bI=this.getObject(bH);this.debug(bH+b+bJ);this.debug(W+bJ+b+this.__Hd[bJ]);this.debug(W+bH+b+this.getObject(bH));this.debug(W+this.getObject(bH)+b+this.getFriendlyName(bI));}
;}
,_getInternalData:function(){return ({"states":this.__iy,"startState":this.__GW,"eventQueue":this.__GY,"blockedEvents":this.__Ha,"savedStates":this.__GX,"friendlyToObject":this.__Hb,"friendlyToHash":this.__Hc,"hashToFriendly":this.__Hd,"groupToFriendly":this.__He,"friendlyToGroups":this.__Hf});}
,start:function(){var bL=this.__GW;if(bL==null){throw new Error(M);}
;this.setState(bL);this.setPreviousState(null);this.setNextState(null);var bK=(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL);if(bK){this.debug(this.getName()+s+bL+K);}
;this.__iy[bL].getAutoActionsBeforeOnentry()(this);if(bK){this.debug(this.getName()+s+bL+O);}
;this.__iy[bL].getOnentry()(this,null);if(bK){this.debug(this.getName()+s+bL+F);}
;this.__iy[bL].getAutoActionsAfterOnentry()(this);}
,pushState:function(bM){if(this.__GX.length>=this.getMaxSavedStates()){throw new Error(G);}
;if(bM===true){this.__GX.push(this.getState());}
else if(bM){this.__GX.push(bM);}
else {this.__GX.push(this.getPreviousState());}
;}
,popState:function(){if(this.__GX.length==0){throw new Error(m);}
;return this.__GX.pop();}
,postponeEvent:function(event){this.__Ha.unshift(event);}
,enqueueEvent:function(event,bN){if(bN){this.__GY.push(event);}
else {this.__GY.unshift(event);}
...
;}
;}
,eventListener:function(event){var e=event.clone();this.enqueueEvent(e,false);this.__Hh();}
...
,this,bV||20);}
,__Hh:function(){if(this.__Hg){return;}
;this.__Hg=true;while(this.__GY.length>0){var event=this.__GY.pop();var bW=this.__Hi(event);if(bW){event.dispose();}
;}
;this.__Hg=false;}
,__Hi:function(event){var ca;var cb;var bY;var cd;var cg;var e;var cf;var bX=this.getDebugFlags();var ck=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var ci=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var ch=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var ce=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var cj=this.getFriendlyName(event.getTarget());if(ck){this.debug(this.getName()+N+event.getType()+(cj?Q+cj:D));}
;ca=this.getState();cd=this.__iy[ca];cg=cd.transitions;e=cd.getEvents()[event.getType()];if(!e){if(ck){this.debug(this.getName()+bl+event.getType()+k+o);}
;return true;}
...
;this.__Ha.unshift(event);return false;default:if(typeof (cf)==j){if(cg[cf]){var cc=cg[cf];cg={};cg[cf]=cc;}
else {throw new Error(V+cf+y);}
;break;}
;};for(var t in cg){var cc=cg[t];switch(cc.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(x+ca+w+t+r+U);};cb=cc.getNextState();if(typeof (cb)==j){if(!cb in this.__iy){throw new Error(l+cb);}
;this.setNextState(cb);}
else {switch(cb){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cb=ca;this.setNextState(cb);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__GX.length==0){throw new Error(v+Y);}
;cb=this.__GX.pop();this.setNextState(cb);break;default:throw new Error(n);};}
...
;for(j=0;j<g.length;j++ ){func+=d+bj+g[j]+A+q+bi+ba+T+funcFragment+bu+bm;}
;}
;}
;}
...
},onexit:{transform:bn,nullable:true,init:function(bH,event){}
...
},events:{transform:x,nullable:true}},members:{__GN:function(bM){if(typeof (bM)!=s||bM.length<1){throw new Error(y);}
;return bM;}
,__GO:function(bN){switch(typeof (bN)){case W:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(r));default:throw new Error(S+typeof (bN));};}
,__GP:function(bP){switch(typeof (bP)){case W:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(r));default:throw new Error(p+typeof (bP));};}
,__GQ:function(bT){if(typeof (bT)!=bB){throw new Error(G);}
;for(var e in bT){var bR=bT[e];if(typeof (bR)==K&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(u+e+M+bR);}
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==K&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(br+R+e+F+bS+M+bR[bS]);}
else if(typeof (bR[bS])!=s&&typeof (bR[bS])!=K){throw new Error(bk+R+e+F+bS+M+bR[bS]);}
;}
;}
else if(typeof (bR)!=s&&typeof (bR)!=K){throw new Error(bb+e+M+bT[e]);}
;}
;return bT;}
,__GR:function(bU){return qx.util.fsm.State._commonTransformAutoActions(D,bU,this.getUserData(r));}
,__GS:function(bV){return qx.util.fsm.State._commonTransformAutoActions(t,bV,this.getUserData(r));}
,__GT:function(bW){return qx.util.fsm.State._commonTransformAutoActions(C,bW,this.getUserData(r));}
,__GU:function(bX){return qx.util.fsm.State._commonTransformAutoActions(P,bX,this.getUserData(r));}
,__GV:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bl+bf);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="__Hn",d="__Hl",e="autoActionsAfterOntransition",g="context",h="string",i="__Hk",j="predicate",k="nextState must be an explicit state name, ",l="Transition ",m=": ",n="String",o="number",p="Invalid transition nextState type: ",q="Invalid transition nextState value: ",r="Adding user-provided field to transition: ",s="__Hm",t="ontransition",u="Invalid transition predicate type: ",v="boolean",w="nextState",x="__Hj",y="Invalid ontransition type: ",z="or one of the Fsm.StateChange constants",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(g,E);for(var C in D){switch(C){case j:this.setPredicate(this.__GV(D[C],E));break;case w:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__GV(D[C],E));break;case e:this.setAutoActionsAfterOntransition(this.__GV(D[C],E));break;case t:this.setOntransition(this.__GV(D[C],E));break;case g:break;default:this.setUserData(C,D[C]);this.debug(l+F+m+r+C);break;};}
;}
,properties:{name:{check:n,nullable:true},predicate:{init:function(G,event){return true;}
,transform:x},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:i},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:d},autoActionsAfterOntransition:{init:function(I,event){}
,transform:s},ontransition:{init:function(J,event){}
,transform:c}},members:{__Hj:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
...
,__Hk:function(N){switch(typeof (N)){case h:return N;case o:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(q+N+m+k+z);};break;default:throw new Error(p+typeof (N));};}
,__Hl:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(g));}
,__Hm:function(P){return qx.util.fsm.State._commonTransformAutoActions(e,P,this.getUserData(g));}
,__Hn:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(g));default:throw new Error(y+typeof (Q));};}
,__GV:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
