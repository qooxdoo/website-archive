...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.SearchAsYouType","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType.227097ee6957.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="searchString",b="Bart",c="<b>Search As You Type List</b><br/>",d="Marge",e="Maggy",f="Skinner",g="Moe",h="Smithers",j="Type a name and the list will show only the matching results (case-sensitive).",k="Lisa",l=" ",m="Homer",n="Nelson",o="Flanders",p="changeValue",q="demobrowser.demo.data.SearchAsYouType";qx.Class.define(q,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var w=[b,k,m,d,g,h,n,o,e,f];var z=[];for(var i=0;i<60;i++ ){z.push(w[i%10]+l+i);}
;var y=new qx.data.Array(z);var r=new qx.ui.container.Composite(new qx.ui.layout.VBox(8));this.getRoot().add(r,{top:10,left:10});var x=new qx.ui.form.TextField();x.setLiveUpdate(true);r.add(x);var u=new qx.ui.form.List();r.add(u);var v=new qx.data.controller.List(y,u);var t=new demobrowser.demo.data.filter.SearchAsYouTypeFilter(v);v.setDelegate(t);x.bind(p,t,a);var s=new qx.ui.basic.Label();s.setRich(true);s.setWidth(200);s.setValue(c+j);r.addAt(s,0);}
}});}
)();
...
(function(){var a="_applySearchString",b="demobrowser.demo.data.filter.SearchAsYouTypeFilter",c="String",d="";qx.Class.define(b,{extend:qx.core.Object,construct:function(e){qx.core.Object.call(this);this.__tJ=e;this.filter=qx.lang.Function.bind(function(f){return f.search(this.getSearchString())!=-1;}
,this);this.__ux=null;}
,properties:{searchString:{check:c,apply:a,init:d}},members:{__ux:null,__tJ:null,_applySearchString:function(h,g){var i=qx.util.TimerManager.getInstance();if(this.__ux!=null){i.stop(this.__ux);this.__ux=null;}
;this.__ux=i.start(function(){this.__tJ.update();this.__ux=null;}
,0,this,null,200);}
,filter:null},destruct:function(){this.__tJ=null;}
});}
)();
(function(){var a="qx.util.TimerManager",b="interval",c="singleton";qx.Class.define(a,{extend:qx.core.Object,type:c,statics:{__FM:[],__FN:{},__ux:0},members:{__FO:false,start:function(g,f,h,d,j){if(!j){j=f||0;}
;var e=(new Date()).getTime()+j;this.self(arguments).__FN[ ++this.self(arguments).__ux]={callback:g,userData:d||null,expireAt:e,recurTime:f,context:h||this};this.__FP(e,this.self(arguments).__ux);return this.self(arguments).__ux;}
,stop:function(k){var l=this.self(arguments).__FM;var length=l.length;for(var i=0;i<length;i++ ){if(l[i]==k){l.splice(i,1);break;}
;}
;delete this.self(arguments).__FN[k];if(l.length==0&&this.__FO){qx.event.Idle.getInstance().removeListener(b,this.__FQ,this);this.__FO=false;}
;}
,__FP:function(m,p){var o=this.self(arguments).__FM;var n=this.self(arguments).__FN;var length=o.length;for(var i=0;i<length;i++ ){if(n[o[i]].expireAt>m){o.splice(i,0,p);break;}
;}
;if(o.length==length){o.push(p);}
;if(!this.__FO){qx.event.Idle.getInstance().addListener(b,this.__FQ,this);this.__FO=true;}
;}
,__FQ:function(){var r=(new Date()).getTime();var t=this.self(arguments).__FM;var s=this.self(arguments).__FN;while(t.length>0&&s[t[0]].expireAt<=r){var v=t.shift();var q=s[v];q.callback.call(q.context,q.userData,v);if(q.recurTime&&s[v]){var u=(new Date()).getTime();q.expireAt=u+q.recurTime;this.__FP(q.expireAt,v);}
else {delete s[v];}
;}
;if(t.length==0&&this.__FO){qx.event.Idle.getInstance().removeListener(b,this.__FQ,this);this.__FO=false;}
;}
}});}
)();
