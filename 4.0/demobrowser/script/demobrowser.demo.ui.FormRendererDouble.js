...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormRendererDouble.0d645c6ae518.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="change",b="qx.ui.form.Form",c="qx.event.type.Event",d="";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__vo=[];this._buttons=[];this._buttonOptions=[];this._validationManager=this._createValidationManager();this._resetter=this._createResetter();}
,events:{"change":c},members:{__vo:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(g,f,h,name,e,k){if(this.__vp()){this.__vo.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
;this.__vo[this._groupCounter].items.push(g);this.__vo[this._groupCounter].labels.push(f);this.__vo[this._groupCounter].options.push(k);if(name==null){name=f.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,d);}
;this.__vo[this._groupCounter].names.push(name);this._validationManager.add(g,h,e);this._resetter.add(g);this.fireEvent(a);}
,addGroupHeader:function(m,l){if(!this.__vp()){this._groupCounter++ ;}
;this.__vo.push({title:m,items:[],labels:[],names:[],options:[],headerOptions:l});this.fireEvent(a);}
,addButton:function(n,o){this._buttons.push(n);this._buttonOptions.push(o||null);this.fireEvent(a);}
,__vp:function(){return this.__vo.length===0;}
,remove:function(p){for(var i=0;i<this.__vo.length;i++ ){var r=this.__vo[i];for(var j=0;j<r.items.length;j++ ){var q=r.items[j];if(q===p){r.items.splice(j,1);r.labels.splice(j,1);r.names.splice(j,1);r.options.splice(j,1);this._validationManager.remove(p);this._resetter.remove(p);this.fireEvent(a);return true;}
;}
;}
;return false;}
,removeGroupHeader:function(u){for(var i=0;i<this.__vo.length;i++ ){var t=this.__vo[i];if(t.title===u){var s;if(i==0){if(this.__vo.length==1){t.title=null;t.headerOptions={};this.fireEvent(a);return true;}
else {s=this.__vo[i+1];}
;}
else {s=this.__vo[i-1];}
;s.items=s.items.concat(t.items);s.labels=s.labels.concat(t.labels);s.names=s.names.concat(t.names);s.options=s.options.concat(t.options);this.__vo.splice(i,1);this._groupCounter-- ;this.fireEvent(a);return true;}
;}
;return false;}
...
,redefineResetterItem:function(x){this._resetter.redefineItem(x);}
,validate:function(){return this._validationManager.validate();}
,getValidationManager:function(){return this._validationManager;}
,getGroups:function(){return this.__vo;}
,getButtons:function(){return this._buttons;}
,getButtonOptions:function(){return this._buttonOptions;}
,getItems:function(){var y={};for(var i=0;i<this.__vo.length;i++ ){var z=this.__vo[i];for(var j=0;j<z.names.length;j++ ){var name=z.names[j];y[name]=z.items[j];}
;}
;return y;}
...
},destruct:function(){this.__vo=this._buttons=this._buttonOptions=null;this._validationManager.dispose();this._resetter.dispose();}
});}
)();
...
,members:{__vC:null,__vD:null,__fy:null,__vE:null,validate:function(c,e,d,f){this.__vE=false;this.__vD=c;this.__fy=d;this.__vC.call(f||this,this,e);}
,validateForm:function(g,h,i){this.__vE=true;this.__fy=h;this.__vC.call(i,g,this);}
,setValid:function(k,j){if(this.__vE){if(j!==undefined){this.__fy.setInvalidMessage(j);}
;this.__fy.setFormValid(k);}
else {if(j!==undefined){this.__vD.setInvalidMessage(j);}
;this.__fy.setItemValid(this.__vD,k);}
;}
},destruct:function(){this.__fy=this.__vD=null;}
});}
)();
...
(function(){var a="The given item has not been added.",b="qx.ui.form.Resetter",c=" not supported for reseting.",d="Item ";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__lV=[];}
,members:{__lV:null,add:function(e){if(this._supportsValue(e)){var f=e.getValue();}
else if(this.__vz(e)){var f=e.getSelection();}
else if(this.__vH(e)){var f=e.getSelection().concat();}
else {throw new Error(d+e+c);}
;this.__lV.push({item:e,init:f});}
,remove:function(g){for(var i=0;i<this.__lV.length;i++ ){var h=this.__lV[i];if(h.item===g){this.__lV.splice(i,1);return true;}
;}
;return false;}
,reset:function(){for(var i=0;i<this.__lV.length;i++ ){var j=this.__lV[i];this.__vF(j.item,j.init);}
;}
,resetItem:function(k){var l;for(var i=0;i<this.__lV.length;i++ ){var m=this.__lV[i];if(m.item===k){l=m.init;break;}
;}
;if(l===undefined){throw new Error(a);}
;this.__vF(k,l);}
,__vF:function(n,o){if(this._supportsValue(n)){n.setValue(o);}
else if(this.__vz(n)||this.__vH(n)){n.setSelection(o);}
;}
,redefine:function(){for(var i=0;i<this.__lV.length;i++ ){var p=this.__lV[i].item;this.__lV[i].init=this.__vG(p);}
;}
,redefineItem:function(q){var r;for(var i=0;i<this.__lV.length;i++ ){if(this.__lV[i].item===q){r=this.__lV[i];break;}
;}
;if(r===undefined){throw new Error(a);}
;r.init=this.__vG(r.item);}
,__vG:function(s){if(this._supportsValue(s)){return s.getValue();}
else if(this.__vz(s)||this.__vH(s)){return s.getSelection();}
;}
,__vz:function(t){var u=t.constructor;return qx.Class.hasInterface(u,qx.ui.core.ISingleSelection);}
,__vH:function(v){var w=v.constructor;return qx.Class.hasInterface(w,qx.data.controller.ISelection);}
...
,properties:{wrap:{check:b,init:true,apply:f},appearance:{refine:true,init:j},singleStep:{check:i,init:20},minimalLineHeight:{check:i,apply:h,init:4},autoSize:{check:b,apply:w,init:false}},members:{__vI:null,__vJ:null,__vK:null,setValue:function(M){M=qx.ui.form.AbstractField.prototype.setValue.call(this,M);this.__vL();return M;}
...
;}
,__vL:function(){if(this.isAutoSize()){var R=this.__vM();if(R&&this.getBounds()){this.__vK=this.__vK||this._getAreaHeight();var T=this._getScrolledAreaHeight();if(this.getMaxHeight()){var P=this.getInsets();var Q=-P.top+this.getMaxHeight()-P.bottom;if(T>Q){this.getContentElement().setStyle(I,k);}
else {this.getContentElement().setStyle(I,H);}
;}
;var S=Math.max(T,this.__vK);this._setAreaHeight(S);}
else {this.getContentElement().addListenerOnce(x,function(){this.__vL();}
,this);}
;}
;}
,_getAreaHeight:function(){return this.getInnerSize().height;}
,_setAreaHeight:function(U){if(this._getAreaHeight()!==U){this.__vJ=U;qx.ui.core.queue.Layout.add(this);qx.ui.core.queue.Manager.flush();this.__vP();}
;}
,_getScrolledAreaHeight:function(){var W=this.__vM();var V=W.getDomElement();if(V){if(!V.parentNode){qx.html.Element.flush();return this._getScrolledAreaHeight();}
...
;this.__vO(W);if(qx.core.Environment.get(C)==B&&qx.core.Environment.get(s)==8){if(!V.scrollTop){qx.html.Element.flush();}
;}
;return V.scrollTop;}
;}
,__vM:function(){this.__vI=this.__vI||this.__vN();return this.__vI;}
,__vN:function(){var ba,Y,bb,X;ba=this.getContentElement();if(!ba.getDomElement()){return null;}
;bb=qx.bom.Element.clone(ba.getDomElement());X=new qx.html.Input(j);X.useElement(bb);Y=X;Y.setStyles({position:K,top:0,left:o,height:0,overflow:H},true);Y.removeAttribute(G);Y.removeAttribute(z);Y.setAttribute(y,g);Y.setValue(ba.getValue()||t);Y.insertBefore(ba);this.__vO(Y);return Y;}
,__vO:function(bc){bc=bc.getDomElement();if(bc){bc.scrollTop=10000;}
;}
...
,_applyAutoSize:function(bh,bg){if(qx.core.Environment.get(m)){this.__vQ();}
;if(bh){this.__vL();this.addListener(a,this.__vL,this);if(!this.getBounds()){this.addListenerOnce(x,function(){this.getContentElement().setStyle(I,H);}
);}
else {this.getContentElement().setStyle(I,H);}
;}
else {this.removeListener(a,this.__vL);this.getContentElement().setStyle(I,k);}
;}
,_applyDimension:function(bi){qx.ui.form.AbstractField.prototype._applyDimension.call(this);if(qx.core.Environment.get(m)){this.__vQ();}
;if(bi===this.getMaxHeight()){this.__vL();}
;}
,__vP:function(){var content=this.getContentElement();var bk=content.getDomElement();var bj=content.getStyle(u);content.setStyle(u,parseInt(bj,10)+1000+c,true);if(bk){qx.bom.element.Dimension.getWidth(bk);}
;content.setStyle(u,bj,true);}
,__vQ:function(){if(this.isAutoSize()&&this.getHeight()){this.warn(n+J+v);}
;}
,_getContentHint:function(){var bl=qx.ui.form.AbstractField.prototype._getContentHint.call(this);bl.height=bl.height*this.getMinimalLineHeight();bl.width=this._getTextSize().width*20;if(this.isAutoSize()){bl.height=this.__vJ||bl.height;}
;return bl;}
},destruct:function(){this.setAutoSize(false);if(this.__vI){this.__vI.dispose();}
;}
});}
)();
...
,_connectVisibility:function(r,t){var s=r.bind(h,t,h);this._visibilityBindingIds.push({id:s,item:r});}
,_onChangeLocale:qx.core.Environment.select(j,{"true":function(e){for(var i=0;i<this._names.length;i++ ){var v=this._names[i];if(v.name&&v.name.translate){v.name=v.name.translate();}
;var u=this._createLabelText(v.name,v.item);v.label.setValue(u);}
;}
,"false":null}),_createLabelText:function(name,w){var x=d;if(w.getRequired()){x=a;}
;var y=name.length>0||w.getRequired()?l:d;return name+x+y;}
,addItems:function(z,A,B){throw new Error(b);}
,addButton:function(C){throw new Error(b);}
...
;for(var i=0;i<this._visibilityBindingIds.length;i++ ){var D=this._visibilityBindingIds[i];D.item.removeBinding(D.id);}
;}
});}
)();
...
,members:{_row:0,_buttonRow:null,_onFormChange:function(){this._buttonRow.destroy();this._buttonRow=null;this._row=0;qx.ui.form.renderer.AbstractRenderer.prototype._onFormChange.call(this);}
,addItems:function(l,o,k){if(k!=null){this._add(this._createHeader(k),{row:this._row,column:0,colSpan:4});this._row++ ;}
