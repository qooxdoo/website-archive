...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.MenuBar.25b1367864bc.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="-",c="PageUp",d="Escape",f="Boolean",g="+",h="qx.event.type.Data",j="_applyShortcut",k="PrintScreen",l="NumLock",m="short",n="keydown",o="5",p="8",q="execute",r="Meta",s="0",t="",u="2",v="Control",w="Shift",x="You can only specify one non modifier key!",y="3",z="Unidentified",A="/",B="Delete",C="String",D="changeEnabled",E="*",F="qx.bom.Shortcut",G="6",H="4",I="1",J="Alt",K="Not a valid key name for a shortcut: ",L="PageDown",M="Whitespaces are not allowed within shortcuts",N="_applyEnabled",O="keypress",P="7",Q="a",R="z",S="9";qx.Class.define(F,{extend:qx.core.Object,construct:function(T){qx.core.Object.call(this);this.__tN={};this.__tO=null;if(T!=null){this.setShortcut(T);}
;this.initEnabled();}
,events:{"execute":h},properties:{enabled:{init:true,check:f,event:D,apply:N},shortcut:{check:C,apply:j,nullable:true},autoRepeat:{check:f,init:false}},members:{__tN:t,__tO:t,execute:function(U){this.fireDataEvent(q,U);}
,__tP:function(event){if(this.getEnabled()&&this.__tQ(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,__gJ:function(event){if(this.getEnabled()&&this.__tQ(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());}
;event.stop();}
;}
,_applyEnabled:function(W,V){if(W){qx.event.Registration.addListener(document.documentElement,n,this.__tP,this);qx.event.Registration.addListener(document.documentElement,O,this.__gJ,this);}
else {qx.event.Registration.removeListener(document.documentElement,n,this.__tP,this);qx.event.Registration.removeListener(document.documentElement,O,this.__gJ,this);}
;}
,_applyShortcut:function(bb,Y){if(bb){if(bb.search(/[\s]+/)!=-1){var bc=M;this.error(bc);throw new Error(bc);}
;this.__tN={"Control":false,"Shift":false,"Meta":false,"Alt":false};this.__tO=null;var X;var a=[];while(bb.length>0&&X!=-1){X=bb.search(/[-+]+/);a.push((bb.length==1||X==-1)?bb:bb.substring(0,X));bb=bb.substring(X+1);}
;var ba=a.length;for(var i=0;i<ba;i++ ){var bd=this.__tS(a[i]);switch(bd){case v:case w:case r:case J:this.__tN[bd]=true;break;case z:var bc=K+a[i];this.error(bc);throw bc;default:if(this.__tO){var bc=x;this.error(bc);throw bc;}
;this.__tO=bd;};}
;}
;return true;}
,__tQ:function(e){var be=this.__tO;if(!be){return false;}
;if((!this.__tN.Shift&&e.isShiftPressed())||(this.__tN.Shift&&!e.isShiftPressed())||(!this.__tN.Control&&e.isCtrlPressed())||(this.__tN.Control&&!e.isCtrlPressed())||(!this.__tN.Meta&&e.isMetaPressed())||(this.__tN.Meta&&!e.isMetaPressed())||(!this.__tN.Alt&&e.isAltPressed())||(this.__tN.Alt&&!e.isAltPressed())){return false;}
;if(be==e.getKeyIdentifier()){return true;}
;return false;}
,__tR:{esc:d,ctrl:v,print:k,del:B,pageup:c,pagedown:L,numlock:l,numpad_0:s,numpad_1:I,numpad_2:u,numpad_3:y,numpad_4:H,numpad_5:o,numpad_6:G,numpad_7:P,numpad_8:p,numpad_9:S,numpad_divide:A,numpad_multiply:E,numpad_minus:b,numpad_plus:g},__tS:function(bg){var bf=qx.event.util.Keyboard;var bh=z;if(bf.isValidKeyIdentifier(bg)){return bg;}
;if(bg.length==1&&bg>=Q&&bg<=R){return bg.toUpperCase();}
;bg=bg.toLowerCase();var bh=this.__tR[bg]||qx.lang.String.firstUp(bg);if(bf.isValidKeyIdentifier(bh)){return bh;}
else {return z;}
;}
,toString:function(){var bj=this.__tO;var bk=[];for(var bi in this.__tN){if(this.__tN[bi]){bk.push(qx.locale.Key.getKeyName(m,bi));}
;}
;if(bj){bk.push(qx.locale.Key.getKeyName(m,bj));}
;return bk.join(g);}
},destruct:function(){this.setEnabled(false);this.__tN=this.__tO=null;}
});}
)();
...
;var P=K.getMarginLeft()+K.getMarginRight();P=Math.max(P,this.getSpacing());var J=K.getSizeHint().width+P;this.__qj(K);G-=J;if(E&&E.getVisibility()!=o){E.setVisibility(o);G+=L;var H=E.getMarginLeft()+E.getMarginRight();G+=Math.max(H,this.getSpacing());}
;}
while(G>F);}
else if(this.__qg.length>0){do {var Q=this.__qg[0];if(Q){var P=Q.getMarginLeft()+Q.getMarginRight();P=Math.max(P,this.getSpacing());if(Q.getContentElement().getDomElement()==null){Q.syncAppearance();Q.invalidateLayoutCache();}
;var I=Q.getSizeHint().width;var O=false;if(this.__qg.length==1&&L>0){var N=P-this.getSpacing();var M=G-L+I+N;O=F>M;}
;if(F>G+I+P||O){this.__qi(Q);G+=I;if(E&&this.__qg.length==0){E.setVisibility(y);}
;}
else {return;}
;}
;}
while(F>=G&&this.__qg.length>0);}
;}
,__qi:function(R){R.setVisibility(o);this.__qg.shift();this.fireDataEvent(m,R);}
,__qj:function(S){if(!S){return;}
;this.__qg.unshift(S);S.setVisibility(y);this.fireDataEvent(A,S);}
,_getNextToHide:function(){for(var i=this.__qh.length-1;i>=0;i-- ){var T=this.__qh[i];if(T&&T.getVisibility&&T.getVisibility()==o){return T;}
;}
...
;}
;}
,setRemovePriority:function(X,W,Y){if(!Y&&this.__qh[W]!=undefined){throw new Error(v);}
;this.__qh[W]=X;}
...
;for(var i=0;i<this.__qg.length;i++ ){this.__qg[i].setVisibility(o);}
;this.__qg=[];}
;}
...
;bf.setVisibility(y);}
;}
,__qk:false,_setAllowMenuOpenHover:function(bg){this.__qk=bg;}
,_isAllowMenuOpenHover:function(){return this.__qk;}
...
(function(){var a="blur",b="Enter",c="__pV",d="Escape",f="Space",g="keydown",h="Left",j="Down",k="Up",l="singleton",m="qx.debug",n="interval",o="__oA",p="roll",q="event.touch",r="Object is no menu: ",s="pointerdown",t="qx.ui.menu.Manager",u="__pU",v="keyup",w="keypress",x="Right";qx.Class.define(t,{type:l,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__oA=[];var y=document.body;var z=qx.event.Registration;z.addListener(window.document.documentElement,s,this._onPointerDown,this,true);z.addListener(y,p,this._onRoll,this,true);z.addListener(y,g,this._onKeyUpDown,this,true);z.addListener(y,v,this._onKeyUpDown,this,true);z.addListener(y,w,this._onKeyPress,this,true);if(!qx.core.Environment.get(q)){qx.bom.Element.addListener(window,a,this.hideAll,this);}
;this.__pU=new qx.event.Timer();this.__pU.addListener(n,this._onOpenInterval,this);this.__pV=new qx.event.Timer();this.__pV.addListener(n,this._onCloseInterval,this);}
,members:{__pW:null,__pX:null,__pU:null,__pV:null,__oA:null,_getChild:function(A,C,B,F){var E=A.getChildren();var length=E.length;var D;for(var i=C;i<length&&i>=0;i+=B){D=E[i];if(D.isEnabled()&&!D.isAnonymous()&&D.isVisible()){return D;}
;}
...
;G=G.getLayoutParent();}
;return false;}
,_isMenuOpener:function(I){var H=this.__oA;for(var i=0;i<H.length;i++ ){if(H[i].getOpener()===I){return true;}
;}
;return false;}
,_getMenuButton:function(J){while(J){if(J instanceof qx.ui.menu.AbstractButton){return J;}
;J=J.getLayoutParent();}
;return null;}
,add:function(K){if(qx.core.Environment.get(m)){if(!(K instanceof qx.ui.menu.Menu)){throw new Error(r+K);}
;}
;var L=this.__oA;L.push(K);K.setZIndex(1e6+L.length);}
,remove:function(M){if(qx.core.Environment.get(m)){if(!(M instanceof qx.ui.menu.Menu)){throw new Error(r+M);}
;}
;var N=this.__oA;if(N){qx.lang.Array.remove(N,M);}
;}
,hideAll:function(){var O=this.__oA;if(O){for(var i=O.length-1;i>=0;i-- ){O[i].exclude();}
;}
;}
,getActiveMenu:function(){var P=this.__oA;return P.length>0?P[P.length-1]:null;}
,scheduleOpen:function(Q){this.cancelClose(Q);if(Q.isVisible()){if(this.__pW){this.cancelOpen(this.__pW);}
;}
else if(this.__pW!=Q){this.__pW=Q;this.__pU.restartWith(Q.getOpenInterval());}
;}
,scheduleClose:function(R){this.cancelOpen(R);if(!R.isVisible()){if(this.__pX){this.cancelClose(this.__pX);}
;}
else if(this.__pX!=R){this.__pX=R;this.__pV.restartWith(R.getCloseInterval());}
;}
,cancelOpen:function(S){if(this.__pW==S){this.__pU.stop();this.__pW=null;}
;}
,cancelClose:function(T){if(this.__pX==T){this.__pV.stop();this.__pX=null;}
;}
,_onOpenInterval:function(e){this.__pU.stop();this.__pW.open();this.__pW=null;}
,_onCloseInterval:function(e){this.__pV.stop();this.__pX.exclude();this.__pX=null;}
...
;if(this.__oA.length>0&&!this._isInMenu(U)){this.hideAll();}
;}
,__pY:{"Enter":1,"Space":1},__na:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var V=this.getActiveMenu();if(!V){return;}
;var W=e.getKeyIdentifier();if(this.__na[W]||(this.__pY[W]&&V.getSelectedButton())){e.stopPropagation();}
;}
,_onKeyPress:function(e){var ba=this.getActiveMenu();if(!ba){return;}
;var X=e.getKeyIdentifier();var bc=this.__na[X];var Y=this.__pY[X];if(bc){switch(X){case k:this._onKeyPressUp(ba);break;case j:this._onKeyPressDown(ba);break;case h:this._onKeyPressLeft(ba);break;case x:this._onKeyPressRight(ba);break;case d:this.hideAll();break;};e.stopPropagation();e.preventDefault();}
else if(Y){var bb=ba.getSelectedButton();if(bb){switch(X){case b:this._onKeyPressEnter(ba,bb,e);break;case f:this._onKeyPressSpace(ba,bb,e);break;};e.stopPropagation();e.preventDefault();}
;}
;}
...
;}
;}
,_onKeyPressEnter:function(bC,bD,e){if(bD.hasListener(w)){var bE=e.clone();bE.setBubbles(false);bE.setTarget(bD);bD.dispatchEvent(bE);}
;this.hideAll();}
,_onKeyPressSpace:function(bF,bG,e){if(bG.hasListener(w)){var bH=e.clone();bH.setBubbles(false);bH.setTarget(bG);bG.dispatchEvent(bH);}
;}
,_onRoll:function(e){var bI=e.getTarget();bI=qx.ui.core.Widget.getWidgetByElement(bI,true);if(this.__oA.length>0&&!this._isInMenu(bI)&&!this._isMenuOpener(bI)&&!e.getMomentum()){this.hideAll();}
;}
},destruct:function(){var bK=qx.event.Registration;var bJ=document.body;bK.removeListener(window.document.documentElement,s,this._onPointerDown,this,true);bK.removeListener(bJ,g,this._onKeyUpDown,this,true);bK.removeListener(bJ,v,this._onKeyUpDown,this,true);bK.removeListener(bJ,w,this._onKeyPress,this,true);this._disposeObjects(u,c);this._disposeArray(o);}
});}
)();
...
,openAtPointer:function(e){this.placeToPointer(e);this.__qd();this.show();this._placementTarget={left:e.getDocumentLeft(),top:e.getDocumentTop()};}
,openAtPoint:function(I){this.placeToPoint(I);this.__qd();this.show();this._placementTarget=I;}
...
;this.__qc();}
,__qc:function(){if(this.isVisible()){if(this.getBlockBackground()){var X=this.getZIndex();this._blocker.blockContent(X-1);}
;}
...
,__qd:function(){var bp=this._getMenuBounds();if(!bp){this.addListenerOnce(o,this.__qd,this);return;}
...
;this.__qb=br;qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(bs){this.getChildControl(h);if(this.__qb){this.__qb.call(this);delete this.__qb;}
;}
...
;this.__qd();}
;}
,_onPointerOver:function(e){var bv=qx.ui.menu.Manager.getInstance();bv.cancelClose(this);var bw=e.getTarget();if(bw.isEnabled()&&bw instanceof qx.ui.menu.AbstractButton){this.setSelectedButton(bw);var bu=bw.getMenu&&bw.getMenu();if(bu){bu.setOpener(bw);bv.scheduleOpen(bu);this.__qa=bu;}
else {var bx=this.getOpenedButton();if(bx){bv.scheduleClose(bx.getMenu());}
;if(this.__qa){bv.cancelOpen(this.__qa);this.__qa=null;}
;}
;}
...
;if(this.__qa){by.cancelOpen(this.__qa);}
;}
;}
...
(function(){var a="qx.ui.menu.Layout",b="Integer",c="_applyLayoutChange";qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:b,init:0,apply:c},spanColumn:{check:b,init:1,nullable:true,apply:c},iconColumnWidth:{check:b,init:0,themeable:true,apply:c},arrowColumnWidth:{check:b,init:0,themeable:true,apply:c}},members:{__qe:null,_computeSizeHint:function(){var q=this._getLayoutChildren();var o,g,k;var e=this.getSpanColumn();var j=this.__qe=[0,0,0,0];var n=this.getColumnSpacing();var m=0;var f=0;for(var i=0,l=q.length;i<l;i++ ){o=q[i];if(o.isAnonymous()){continue;}
...
,getColumnSizes:function(){return this.__qe||null;}
...
;var x=this.__qf(y[0]);var w=x.getColumnSizes();var s=x.getSpacingX();var z=qx.lang.Array.sum(w)+s*(w.length-1);if(z<o){w[1]+=o-z;}
...
;}
;}
,__qf:function(B){while(!(B instanceof qx.ui.menu.Menu)){B=B.getLayoutParent();}
;return B;}
...
(function(){var a="__ju",b="hovered",c="hover-button",d="pointerover",f="interval",g="qx.ui.form.HoverButton",h="Integer",i="pointerout";qx.Class.define(g,{extend:qx.ui.basic.Atom,include:[qx.ui.core.MExecutable],implement:[qx.ui.form.IExecutable],construct:function(j,k){qx.ui.basic.Atom.call(this,j,k);this.addListener(d,this._onPointerOver,this);this.addListener(i,this._onPointerOut,this);this.__ju=new qx.event.AcceleratingTimer();this.__ju.addListener(f,this._onInterval,this);}
,properties:{appearance:{refine:true,init:c},interval:{check:h,init:80},firstInterval:{check:h,init:200},minTimer:{check:h,init:20},timerDecrease:{check:h,init:2}},members:{__ju:null,_onPointerOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;}
...
,_onPointerOut:function(e){this.__ju.stop();this.removeState(b);if(!this.isEnabled()||e.getTarget()!==this){return;}
;}
...
,__ql:function(){var y=this.getChildrenContainer().getChildren();y=y.filter(function(z){return z.getVisibility()==k;}
