...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.bom.Flash","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Flash.6622b67d6dd1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="demobrowser/demo/flash/fo_tester.swf",b="this is passed in via FlashVars",c="div",d="20px",e="position",f="demobrowser.demo.bom.Flash",g="left",h="opaque",i="movie",j="top",k="bottom",l="right",m="noscale",n="#FF6600",o="absolute";qx.Class.define(f,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var s=qx.dom.Element.create(c);qx.bom.element.Style.set(s,e,o);qx.bom.element.Style.set(s,j,d);qx.bom.element.Style.set(s,k,d);qx.bom.element.Style.set(s,l,d);qx.bom.element.Style.set(s,g,d);document.body.appendChild(s);var p=qx.util.ResourceManager.getInstance().toUri(a);var r={flashVarText:b};var q={bgcolor:n,scale:m,wmode:h};qx.bom.Flash.create(s,{movie:p,id:i},r,q);}
}});}
)();
...
;}
;}
;var N=this.__Gb(O,Q,P,L);this._flashObjects[Q.id]=N;return N;}
,destroy:qx.core.Environment.select(C,{"mshtml":function(S,R){S=this.__FV(S);if(S.readyState==4){this.__FW(S);}
else {if(!R){R=window;}
;qx.bom.Event.addNativeListener(R,p,function(){qx.bom.Flash.__FW(S);}
);}
;}
,"default":function(U,T){U=this.__FV(U);if(U.parentNode){U.parentNode.removeChild(U);}
;delete this._flashObjects[U.id];}
}),__FV:function(V){if(!V){throw new Error(y);}
;if(V.tagName.toLowerCase()!==K){V=V.firstChild;}
;if(!V||V.tagName.toLowerCase()!==K){throw new Error(z);}
;return V;}
,__FW:qx.core.Environment.select(C,{"mshtml":qx.event.GlobalError.observeMethod(function(W){for(var i in W){if(typeof W[i]==b){W[i]=null;}
;}
;if(W.parentNode){W.parentNode.removeChild(W);}
;delete this._flashObjects[W.id];}
),"default":null}),__FX:qx.event.GlobalError.observeMethod(function(){for(var X in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[X]);}
;window.__FY=function(){}
;window.__Ga=function(){}
;qx.bom.Event.removeNativeListener(window,E,qx.bom.Flash.__FX);}
),__Gb:qx.core.Environment.select(C,{"mshtml":function(bb,bd,bc,Y){bc.movie=bd.data;delete bd.data;delete bd.classid;var ba=t;for(name in bc){ba+=m+name+I+bc[name]+D;}
...
})},defer:function(bk){if(qx.core.Environment.get(C)==u){qx.bom.Event.addNativeListener(window,E,bk.__FX);}
;}
});}
)();
