...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Gallery.646028df1c53.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="Gallery (HTML - divs)",b="manager",c="background",d="demobrowser.demo.virtual.Gallery",f="demobrowser.demo.virtual.WidgetGallery",g="selected",h="text-align: center;",j="background-selected",k="auto",l="color: white; background-color: #00398D;",m="user-desktop.png",n="cell",o="<img src='",p="'></img>",q="layer",r="resize",s="folder.png",t="vertical",u="",v="icon",w="multi",x="network-server.png",y="user-trash.png",z="off",A="white-box",B="top",C="icon/32/places/",D="horizontal",E="demobrowser.demo.virtual.AbstractGallery",F="default",G="abstract",H="#f3f3f3",I="position: absolute;",J="label",K="demobrowser.demo.virtual.HtmlGallery",L="Gallery (widgets)",M="<br>",N="Icon #",O="network-workgroup.png",P="__PQ",Q="text-selected",R="demobrowser.demo.virtual.GalleryCell";qx.Class.define(d,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);new demobrowser.demo.virtual.WidgetGallery(L);var S=new demobrowser.demo.virtual.HtmlGallery(a);S.moveTo(400,50);}
...
,_generateItems:function(bg){var bj=[];var bm=[s,y,x,O,m];var bk=qx.util.AliasManager.getInstance();var bi=qx.util.ResourceManager.getInstance();for(var i=0;i<bg;i++ ){var bn=C+bm[Math.floor(Math.random()*bm.length)];var bh=bk.resolve(bn);var bl=bi.toUri(bh);bj[i]={label:N+(i+1),icon:bn,resolvedIcon:bl};}
;return bj;}
...
;}
}});qx.Class.define(f,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bv){demobrowser.demo.virtual.AbstractGallery.call(this,bv);this.__PQ=new demobrowser.demo.virtual.GalleryCell();}
,members:{__PQ:null,_createScroller:function(){var bw=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.WidgetCell(this);bw.getPane().addLayer(this.layer);new qx.ui.virtual.behavior.Prefetch(bw,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:200,maxAbove:300,minBelow:600,maxBelow:800}).set({interval:500});return bw;}
...
;if(by){this.__PQ.updateStates(bx,{selected:1});}
else {this.__PQ.updateStates(bx,{});}
;}
;}
...
;var bE=this.__PQ.getCellWidget(bF,bD);bE.setUserData(n,bH);return bE;}
,poolCellWidget:function(bJ){this.__PQ.pool(bJ);}
},destruct:function(){this._disposeObjects(P,q);}
});qx.Class.define(K,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bK){demobrowser.demo.virtual.AbstractGallery.call(this,bK);var bL=qx.theme.manager.Font.getInstance().resolve(F).getStyles();this._fontCss=qx.bom.element.Style.compile(bL);}
,members:{_createScroller:function(){var bN=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.HtmlCell(this);bN.getPane().addLayer(this.layer);var bM=new qx.ui.virtual.layer.GridLines(D,H);bN.getPane().addLayer(bM);var bM=new qx.ui.virtual.layer.GridLines(t,H);bN.getPane().addLayer(bM);return bN;}
...
;var bS=this.manager.isItemSelected({row:bV,column:bR});var bU=bS?l:u;return {style:[I,h,this._fontCss,bU].join(u),content:[o,bT.resolvedIcon,p,M,bT.label].join(u)};}
...
(function(){var a="__ju",b="_applyInterval",c="appear",d="qx.debug",e="interval",f="qx.ui.virtual.behavior.Prefetch",g="scroll",h="_applyScroller",i="qx.ui.virtual.core.Scroller",j="scrollbar-x",k="Integer",l="scrollbar-y";qx.Class.define(f,{extend:qx.core.Object,construct:function(m,n){if(qx.core.Environment.get(d)){this.assertObject(n);this.assertPositiveInteger(n.minLeft);this.assertPositiveInteger(n.maxLeft);this.assertPositiveInteger(n.minRight);this.assertPositiveInteger(n.maxRight);this.assertPositiveInteger(n.minAbove);this.assertPositiveInteger(n.maxAbove);this.assertPositiveInteger(n.minBelow);this.assertPositiveInteger(n.maxBelow);}
;qx.core.Object.call(this);this.setPrefetchX(n.minLeft,n.maxLeft,n.minRight,n.maxRight);this.setPrefetchY(n.minAbove,n.maxAbove,n.minBelow,n.maxBelow);this.__ju=new qx.event.Timer(this.getInterval());this.__ju.addListener(e,this._onInterval,this);if(m){this.setScroller(m);}
;}
,properties:{scroller:{check:i,nullable:true,init:null,apply:h},interval:{check:k,init:200,apply:b}},members:{__wf:null,__wg:null,__ju:null,__wh:null,__wi:null,setPrefetchX:function(o,r,p,q){this.__wf=[o,r,p,q];}
...
;}
;if(z){if(!z.getContentElement().getDomElement()){this.__ju.stop();z.addListenerOnce(c,this.__ju.start,this.__ju);}
else {this.__ju.restart();}
;this.__wh=z.getChildControl(j).addListener(g,this.__ju.restart,this.__ju);this.__wi=z.getChildControl(l).addListener(g,this.__ju.restart,this.__ju);}
else {this.__ju.stop();}
;}
,_applyInterval:function(B,A){this.__ju.setInterval(B);}
},destruct:function(){this.setScroller(null);this.__wf=this.__wg=null;this._disposeObjects(a);}
});}
)();
...
;this.__CI=[];this.__CJ=[];this._isHorizontal=(w||c)==c;}
,properties:{defaultLineColor:{init:h,check:s,themeable:true},defaultLineSize:{init:1,check:t,themeable:true}},members:{__CI:null,__CJ:null,isHorizontal:function(){return this._isHorizontal;}
...
;this.__CI[B]=A;if(this.__CK(B)){this.updateLayerData();}
;}
...
;this.__CJ[C]=D;if(this.__CK(C)){this.updateLayerData();}
;}
,__CK:function(E){if(this._isHorizontal){var I=this.getFirstColumn();var G=I+this.getColumnSizes().length-1;return E>=I&&E<=G;}
else {var H=this.getFirstRow();var F=H+this.getRowSizes().length-1;return E>=H&&E<=F;}
;}
,getLineSize:function(J){return this.__CJ[J]||this.getDefaultLineSize();}
,getLineColor:function(K){return this.__CI[K]||this.getDefaultLineColor();}
,__CL:function(P,M,O){var top=0;var L,N;for(var y=0;y<O.length-1;y++ ){L=this.getLineColor(M+y);N=this.getLineSize(M+y);top+=O[y];P.push(m,g,f+N+q,j,d,top-((N>1)?Math.floor(N/2):1),q,b,L,n,a);}
;}
,__CM:function(T,Q,S){var V=0;var R,U;for(var x=0;x<S.length-1;x++ ){R=this.getLineColor(Q+x);U=this.getLineSize(Q+x);V+=S[x];T.push(m,g,k+U+q,l,u,p,V-((U>1)?Math.floor(U/2):1),q,b,R,n,a);}
;}
,_fullUpdate:function(ba,bb,X,W){var Y=[];if(this._isHorizontal){this.__CL(Y,ba,X);}
else {this.__CM(Y,bb,W);}
