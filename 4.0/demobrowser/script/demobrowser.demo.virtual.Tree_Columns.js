...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Columns.3547b8ab6b60.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,_getDataFromRow:function(bm){return this.__xd.getItem(bm);}
,_getSelectables:function(){return this.__xd;}
,getOpenNodes:function(){return this.__CW;}
,isNode:function(bn){return qx.ui.tree.core.Util.isNode(bn,this.getChildProperty());}
,getLevel:function(bo){return this.__CX[bo];}
...
,_applyModel:function(bM,bL){this.__CW=[];if(bM!=null){if(qx.core.Environment.get(w)){if(!qx.Class.hasMixin(bM.constructor,qx.data.marshal.MEventBubbling)){this.warn(H+h+k);}
;}
;bM.addListener(J,this._onChangeBubble,this);this.__Dc(bM);}
...
;}
;if(this.__xd.indexOf(bR)!=-1){this.__Da();}
;}
;}
,_onUpdated:function(event){if(this.__gh==null){this.__gh=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__gh.schedule();}
,_onOpen:function(event){var bV=event.getRow();var bU=this.__xd.getItem(bV);if(this.isNode(bU)){if(this.isNodeOpen(bU)){this.closeNode(bU);}
...
;break;};}
;}
,_beforeApplySelection:function(cc){if(cc.length===0&&this.getSelectionMode()===g){var cb=this.__Df();var cd=this.getLookupTable().indexOf(cb);if(cd>=0){cc.push(cd);}
;}
;}
,_afterApplySelection:function(){var ce=this.getSelection();if(ce.getLength()>0&&this.getSelectionMode()===g){this.__De(ce.getItem(0));}
else {this.__CY=[];}
;}
,__Da:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(Q+f);}
;this._itemWidth=0;var ci=[];this.__CX=[];var cg=-1;var ch=this.getModel();if(ch!=null){if(!this.isHideRoot()){cg++ ;ci.push(ch);this.__CX.push(cg);}
;if(this.isNodeOpen(ch)){var cf=this.__Db(ch,cg);ci=ci.concat(cf);}
;}
;if(!qx.lang.Array.equals(this.__xd.toArray(),ci)){this._provider.removeBindings();this.__xd.removeAll();this.__xd.append(ci);this.__xj();this._updateSelection();}
;}
,__Db:function(cj,co){var cn=[];co++ ;if(!this.isNode(cj)){return cn;}
...
;if(this.isNode(cq)){this.__CX.push(co);cn.push(cq);if(this.isNodeOpen(cq)){var ck=this.__Db(cq,co);cn=cn.concat(ck);}
;}
else {if(this.isShowLeafs()){this.__CX.push(co);cn.push(cq);}
;}
;}
;cr.dispose();return cn;}
,__Dc:function(cs){if(!qx.lang.Array.contains(this.__CW,cs)){this.__CW.push(cs);this.fireDataEvent(t,cs);}
;}
,__Dd:function(ct,cu){if(ct===cu){this.__Dc(cu);return true;}
;if(!this.isNode(ct)){return false;}
;var cx=ct.get(this.getChildProperty());if(cx==null){return false;}
;for(var i=0;i<cx.getLength();i++ ){var cw=cx.getItem(i);var cv=this.__Dd(cw,cu);if(cv===true){this.__Dc(cw);return true;}
;}
;return false;}
,__xj:function(){this.getPane().getRowConfig().setItemCount(this.__xd.getLength());this.getPane().fullUpdate();}
,getParent:function(cz){var cA=this.__xd.indexOf(cz);if(cA<0){return null;}
;var cB=this.__CX[cA];while(cA>0){cA-- ;var cy=this.__CX[cA];if(cy<cB){return this.__xd.getItem(cA);}
;}
;return null;}
,__De:function(cC){this.__CY=[];var parent=this.getParent(cC);while(parent!=null){this.__CY.unshift(parent);parent=this.getParent(parent);}
;}
,__Df:function(){if(this.__CY==null){return this.getModel();}
;var cD=this.getLookupTable();var parent=this.__CY.pop();while(parent!=null){if(cD.contains(parent)){return parent;}
;parent=this.__CY.pop();}
...
;}
;if(!qx.core.ObjectRegistry.inShutDown&&this.__gh!=null){this.__gh.cancel();this.__gh.dispose();}
;var cE=this.getModel();if(cE!=null){cE.removeListener(J,this._onChangeBubble,this);}
;this._layer.removeListener(R,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__xd.dispose();this._layer=this._provider=this.__xd=this.__CW=this.__gh=null;}
});}
)();
...
;var o=this._renderer.getCellWidget();o.setOpen(p&&this._tree.isNodeOpen(s));o.addListener(a,this.__Dg,this);o.setUserData(b,this.getChildProperty());o.setUserData(g,this._tree.isShowLeafs());if(this._tree.getSelection().contains(s)){this._styleSelectabled(o);}
...
,poolCellWidget:function(u){u.removeListener(a,this.__Dg,this);this._removeBindingsFrom(u);this._renderer.pool(u);this._onPool(u);}
...
,__Dg:function(event){var K=event.getTarget();var L=K.getUserData(k);var J=this._tree.getLookupTable().getItem(L);if(event.getData()){this._tree.openNode(J);}
...
(function(){var a="source",b="changeLeadIcon",c="changeSize",d="changeMode",e="changeDate",f="checked",g="changeChecked",h="icon/16/places/user-desktop.png",i="value",j="size",k="leadIcon",l="date",m="mode",n="demobrowser.demo.virtual.tree.TreeItem",o="String",p="Boolean";qx.Class.define(n,{extend:qx.ui.tree.VirtualTreeItem,properties:{leadIcon:{check:o,event:b,nullable:true},checked:{check:p,event:g,nullable:true},size:{check:o,event:c,nullable:true},date:{check:o,event:e,nullable:true},mode:{check:o,event:d,nullable:true}},members:{__PW:null,__PX:null,__PY:null,__Qa:null,__hA:null,_addWidgets:function(){var q=this.__PW=new qx.ui.basic.Image();this.bind(k,q,a);q.setWidth(16);this.addWidget(q);this.addSpacer();this.addOpenButton();this.addIcon();this.setIcon(h);var r=this.__PX=new qx.ui.form.CheckBox();this.bind(f,r,i);r.bind(i,this,f);r.setFocusable(false);r.setTriState(true);r.setMarginRight(4);this.addWidget(r);this.addLabel();this.addWidget(new qx.ui.core.Spacer(),{flex:1});var s=this.__PY=new qx.ui.basic.Label();this.bind(j,s,i);s.setWidth(50);this.addWidget(s);s=this.__Qa=new qx.ui.basic.Label();this.bind(l,s,i);s.setWidth(150);this.addWidget(s);s=this.__hA=new qx.ui.basic.Label();this.bind(m,s,i);s.setWidth(80);this.addWidget(s);}
}});}
)();
