...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.ComboBox.0ded128bd94c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
},destruct:function(){if(this.__wt){this.__wt.dispose();}
;}
});}
)();
...
(function(){var a="changeModel",b="one",c="Enter",d="list",e="changeDelegate",f="change",g="qx.data.Array",h="changeVisibility",j="qx.ui.form.core.VirtualDropDownList",k="_applySelection",l="changeSelection",m="tap",n="Invalid parameter 'target'!",o="qx.event.type.Data";qx.Class.define(j,{extend:qx.ui.popup.Popup,construct:function(p){qx.core.Assert.assertNotNull(p,n);qx.core.Assert.assertNotUndefined(p,n);qx.core.Assert.assertInterface(p,qx.ui.form.core.AbstractVirtualBox,n);qx.ui.popup.Popup.call(this,new qx.ui.layout.VBox());this._target=p;this._createChildControl(d);this.addListener(h,this.__wx,this);this.__wu=new qx.data.Array();this.initSelection(this.__wu);}
,properties:{autoHide:{refine:true,init:false},keepActive:{refine:true,init:true},selection:{check:g,event:l,apply:k,nullable:false,deferredInit:true}},events:{"changeSelection":o},members:{_target:null,_preselected:null,__wv:false,__ww:false,__wu:null,open:function(){this.placeToWidget(this._target,true);this.show();}
,close:function(){this.hide();}
,setPreselected:function(q){this._preselected=q;this.__ww=true;var s=this.getChildControl(d).getSelection();var r=new qx.data.Array([q]);this.__wz(r,s);r.dispose();this.__ww=false;}
,_createChildControlImpl:function(v,u){var t;switch(v){case d:t=new qx.ui.list.List().set({focusable:false,keepFocus:true,height:null,width:null,maxHeight:this._target.getMaxListHeight(),selectionMode:b,quickSelection:true});t.getSelection().addListener(f,this._onListChangeSelection,this);t.addListener(m,this._handlePointer,this);t.addListener(a,this._onChangeModel,this);t.addListener(e,this._onChangeDelegate,this);this.add(t,{flex:1});break;};return t||qx.ui.popup.Popup.prototype._createChildControlImpl.call(this,v,u);}
,_handleKeyboard:function(event){if(this.isVisible()&&event.getKeyIdentifier()===c){this.__wy();return;}
;var w=event.clone();w.setTarget(this.getChildControl(d));w.setBubbles(false);this.getChildControl(d).dispatchEvent(w);}
,_handlePointer:function(event){this.__wy();}
,__lG:function(event){if(this.__wv){return;}
;var x=this.getSelection();var y=this.getChildControl(d).getSelection();this.__ww=true;this.__wz(x,y);this.__ww=false;this.__wv=true;this.__wz(y,x);this.__wv=false;}
,_onListChangeSelection:function(event){if(this.__ww){return;}
;var z=this.getChildControl(d).getSelection();if(this.isVisible()){this.setPreselected(z.getItem(0));}
else {this.__wv=true;this.__wz(z,this.getSelection());this.__wv=false;}
;}
,__wx:function(event){if(this.isVisible()){if(this._preselected==null){var A=this.getSelection();var B=this.getChildControl(d).getSelection();this.__wz(A,B);}
;this.__wA();}
...
,_applySelection:function(D,C){D.addListener(f,this.__lG,this);if(C!=null){C.removeListener(f,this.__lG,this);}
;this.__wz(D,this.getChildControl(d).getSelection(D));}
,__wy:function(){if(this._preselected!=null){var E=this.getSelection();E.splice(0,1,this._preselected);this._preselected=null;this.close();}
;}
,__wz:function(F,H){if(F.equals(H)){return;}
...
;H.length=G.length;H.fireDataEvent(f,{});}
;}
,__wA:function(){this.__wB();this.__wC();}
,__wB:function(){var I=this._target.getBounds().width;this.setWidth(I);}
,__wC:function(){var J=this.__wD();var L=this._target.getMaxListHeight();var K=this.getChildControl(d);var M=K.getPane().getRowConfig().getTotalSize();if(L==null||M<L){L=M;}
;if(L>J){K.setMaxHeight(J);}
else if(L<J){K.setMaxHeight(L);}
;}
,__wD:function(){var N=this.getLayoutLocation(this._target);var Q=qx.bom.Viewport.getHeight();var P=N.top;var O=Q-N.bottom;return P>O?P:O;}
},destruct:function(){if(this.__wu){this.__wu.dispose();}
;}
});}
)();
...
;}
,__xD:function(){var r=this.__cK.shift();if(r==null){r=this._createWidget();this.fireDataEvent(a,r);}
;return r;}
...
,_createChildControlImpl:function(H,G){var F;switch(H){case r:F=new qx.ui.form.TextField();F.setFocusable(false);F.addState(h);this._add(F,{flex:1});break;case d:F=new qx.ui.form.Button();F.setFocusable(false);F.setKeepActive(true);F.addState(h);F.addListener(f,this.toggle,this);this._add(F);break;};return F||qx.ui.form.core.AbstractVirtualBox.prototype._createChildControlImpl.call(this,H,G);}
,_beforeOpen:function(){this.__xH();}
...
;}
,_handlePointer:function(event){qx.ui.form.core.AbstractVirtualBox.prototype._handlePointer.call(this,event);var M=event.getType();if(M!==y){return;}
;this.close();}
,__xG:function(event){if(this.__wX==true){return;}
;var N=this.__fC.getItem(0);if(N){N=this.__bm(N);this.setValue(N);}
;}
,_applyPlaceholder:function(P,O){this.getChildControl(r).setPlaceholder(P);}
,__xH:function(){var S=this.getValue();var R=this.getChildControl(s);var Q=R.getSelection();if(this.__bm(Q.getItem(0))!==S){this.__wX=true;Q.removeAll();this.__wX=false;if(S==null||S==q){return;}
;var W=this.getModel();var V=R.getChildControl(j)._getLookupTable();for(var i=0,l=V.length;i<l;i++ ){var T=W.getItem(V[i]);var U=this.__bm(T);if(U&&U.indexOf(S)==0){R.setPreselected(T);break;}
;}
;}
;}
,__bm:function(ba){var Y=this.getLabelOptions();var bc=this.getDefaultFormat();var bd=this.getLabelPath();var bb=null;if(bd!=null){bb=qx.data.SingleValueBinding.resolvePropertyChain(ba,bd);}
...
;return bb;}
},destruct:function(){var be=this.getChildControl(r);this.removeAllBindings();be.removeAllBindings();this.__fC.removeListener(k,this.__xG,this);this.__fC=null;}
});}
)();
