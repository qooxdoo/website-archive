...
;}
;if(this.__cache[gx]!=undefined){return this.__cache[gx];}
;var gz=this._checks[gx];if(gz){var gv=gz();this.__cache[gx]=gv;return gv;}
;var gu=this._getClassNameFromEnvKey(gx);if(gu[0]!=undefined){var gy=gu[0];var gw=gu[1];var gv=gy[gw]();this.__cache[gx]=gv;return gv;}
...
,select:function(gI,gH){return this.__pickFromValues(this.get(gI),gH);}
,__pickFromValues:function(gM,gL){var gK=gL[gM];if(gL.hasOwnProperty(gM)){return gK;}
;for(var gN in gL){if(gN.indexOf(fQ)!=-1){var gJ=gN.split(fQ);for(var i=0;i<gJ.length;i++ ){if(gJ[i]==gM){return gL[gN];}
;}
;}
;}
;if(gL[fl]!==undefined){return gL[fl];}
;if(qx.Bootstrap.DEBUG){throw new Error(eE+gM+fi+(typeof gM)+n+fx+qx.Bootstrap.keys(gL)+g);}
;}
,add:function(gP,gO){if(this._checks[gP]==undefined){if(gO instanceof Function){this._checks[gP]=gO;}
else {this._checks[gP]=this.__createCheck(gO);}
;}
;}
,addAsync:function(gR,gQ){if(this._checks[gR]==undefined){this._asyncChecks[gR]=gQ;}
;}
...
,__importFromGenerator:function(){if(qx&&qx.$$environment){for(var gS in qx.$$environment){var gT=qx.$$environment[gS];this._checks[gS]=this.__createCheck(gT);}
;}
;}
,__importFromUrl:function(){if(window.document&&window.document.location){var gU=window.document.location.search.slice(1).split(fW);for(var i=0;i<gU.length;i++ ){var gX=gU[i].split(w);if(gX.length!=3||gX[0]!=fz){continue;}
;var gV=gX[1];var gW=decodeURIComponent(gX[2]);if(gW==gf){gW=true;}
else if(gW==fw){gW=false;}
else if(/^(\d|\.)+$/.test(gW)){gW=parseFloat(gW);}
;this._checks[gV]=this.__createCheck(gW);}
;}
;}
,__createCheck:function(gY){return qx.Bootstrap.bind(function(ha){return ha;}
,null,gY);}
},defer:function(hb){hb._initDefaultQxValues();hb.__importFromGenerator();if(hb.get(gd)===true){hb.__importFromUrl();}
;}
});}
)();
...
,__isGecko:function(){return (window.controllers||window.navigator.mozApps)&&window.navigator.product===j&&window.navigator.userAgent.indexOf(k)==-1&&window.navigator.userAgent.indexOf(m)==-1;}
...
else if((p.nodeType===1||p.nodeType===9||p.nodeType===11)||(p.history&&p.location&&p.document)){p=[p];}
...
,map:function(y,z){return qxWeb.$init(Array.prototype.map.apply(this,arguments),qxWeb);}
...
,_forEachElement:function(E,D){for(var i=0,l=this.length;i<l;i++ ){if(this[i]&&(this[i].nodeType===1||this[i].nodeType===11)){E.apply(D||this,[this[i],i,this]);}
;}
;return this;}
...
(function(){var a="HTMLEvents",b="engine.name",c="",d="mshtml",f="qx.bom.Event",g="pointerover",h="function",j="mouseover",k="transitionend",m="gecko",n="css.transition",o="on",p="undefined",q="return;",r="browser.documentmode",s="end-event";qx.Bootstrap.define(f,{statics:{addNativeListener:function(w,v,t,u){if(w.addEventListener){w.addEventListener(v,t,!!u);}
else if(w.attachEvent){w.attachEvent(o+v,t);}
else if(typeof w[o+v]!=p){w[o+v]=t;}
else {{}
;}
;}
,removeNativeListener:function(A,z,x,y){if(A.removeEventListener){A.removeEventListener(z,x,!!y);}
else if(A.detachEvent){try{A.detachEvent(o+z,x);}
catch(e){if(e.number!==-2146828218){throw e;}
;}
;}
else if(typeof A[o+z]!=p){A[o+z]=null;}
else {{}
;}
;}
...
else if(e.fromElement!==undefined&&(e.type===j||e.type===g)){return e.fromElement;}
...
;}
,fire:function(F,D){if(document.createEvent){var E=document.createEvent(a);E.initEvent(D,true,true);return !F.dispatchEvent(E);}
else {var E=document.createEventObject();return F.fireEvent(o+D,E);}
;}
,supportsEvent:function(K,J){if(J.toLowerCase().indexOf(k)!=-1&&qx.core.Environment.get(b)===d&&qx.core.Environment.get(r)>9){return true;}
;if(K!=window&&J.toLowerCase().indexOf(k)!=-1){var I=qx.core.Environment.get(n);return (I&&I[s]==J);}
;var G=o+J.toLowerCase();var H=(G in K);if(!H){H=typeof K[G]==h;if(!H&&K.setAttribute){K.setAttribute(G,q);H=typeof K[G]==h;K.removeAttribute(G);}
;}
;return H;}
...
;var C=r.$$emitter.on(B,w,v);if(!r.__listener){r.__listener={};}
...
;if(!o.__pointerListeners){o.__pointerListeners=0;}
...
,unregister:function(p){if(p.$$pointerHandler){p.__pointerListeners-- ;if(p.__pointerListeners===0){p.$$pointerHandler.dispose();p.$$pointerHandler=null;}
;}
;}
...
,_onTouchEvent:function(bd){var be=qx.event.handler.PointerCore.TOUCH_TO_POINTER_MAPPING[bd.type];var bf=bd.changedTouches;bd.stopPropagation();if(bd.type==m&&this.__primaryIdentifier===null){this.__primaryIdentifier=bf[0].identifier;}
...
;var bh=new qx.event.type.dom.Pointer(be,bd,bc);if(bg.identifier==this.__primaryIdentifier){bh.isPrimary=true;bh.button=0;bh.buttons=1;qx.event.handler.PointerCore.__lastTouch={"x":bg.clientX,"y":bg.clientY,"time":new Date().getTime()};}
;this._fireEvent(bh,be,bc.target);if(bd.type==v||bd.type==G){var bi=new qx.event.type.dom.Pointer(O,bd,bc);this._fireEvent(bi,O,bd.target);if(this.__primaryIdentifier==bg.identifier){this.__primaryIdentifier=null;}
;}
;}
;}
,_onMouseEvent:function(bj){qx.bom.Event.stopPropagation(bj);if(this._isSimulatedMouseEvent(bj.clientX,bj.clientY)){return;}
;if(bj.type==b){this.__buttonStates[bj.which]=1;}
else if(bj.type==F){this.__buttonStates[bj.which]=0;}
;var bl=qx.event.handler.PointerCore.MOUSE_TO_POINTER_MAPPING[bj.type];var bk=qx.bom.Event.getTarget(bj);var bm=qx.lang.Array.sum(this.__buttonStates);var bp={pointerType:o,pointerId:1};if(this.__lastButtonState!=bm&&bm!==0&&this.__lastButtonState!==0){var bn=new qx.event.type.dom.Pointer(N,bj,bp);this._fireEvent(bn,N,bk);}
;this.__lastButtonState=bm;if(bj.type==b&&bm>1){return;}
;if(bj.type==F&&bm>0){return;}
;if(bj.type==H){this.__buttonStates[bj.which]=0;return;}
;var bo=new qx.event.type.dom.Pointer(bl,bj,bp);this._fireEvent(bo,bl,bk);}
,_isSimulatedMouseEvent:function(x,y){var br=qx.event.handler.PointerCore.__lastTouch;if(br){var bs=new Date().getTime()-br.time;var bq=qx.event.handler.PointerCore.SIM_MOUSE_DISTANCE;var bu=Math.abs(x-qx.event.handler.PointerCore.__lastTouch.x);var bt=Math.abs(y-qx.event.handler.PointerCore.__lastTouch.y);if(bs<qx.event.handler.PointerCore.SIM_MOUSE_DELAY){if(bu<bq||bt<bq){return true;}
;}
;}
;return false;}
...
;if(qx.core.Environment.get(c)){if(!this.__nativePointerEvents){bx.dispatchEvent(bw);}
...
(function(){var a="swipe",b="pinch",c="event.dispatchevent",d="longtap",e="rotate",f="track",g="trackstart",h="trackend",i="tap",j="qx.module.event.GestureHandler",k="roll",l="dbltap";qx.Bootstrap.define(j,{statics:{TYPES:[i,d,a,l,f,g,h,k,e,b],register:function(n,m){if(!n.__gestureListeners){n.__gestureListeners=0;}
...
;n.__gestureListeners++ ;}
,unregister:function(o){if(o.$$gestureHandler){o.__gestureListeners-- ;if(o.__gestureListeners===0){o.$$gestureHandler.dispose();o.$$gestureHandler=null;}
;}
;}
...
;X=X.parentNode;}
;return false;}
,gestureFinish:function(Y,ba){if(!this.__gesture[Y.pointerId]){return;}
;var be=this.__gesture[Y.pointerId];this.__stopLongTapTimer(be);if(this._hasIntermediaryHandler(ba)){return;}
;this.__handleRollImpulse(be.velocityX,be.velocityY,Y,be.target);this.__fireTrack(z,Y,be.target);if(ba!==be.target){delete this.__gesture[Y.pointerId];return;}
;if(be.isTap){this._fireEvent(Y,B,Y.target||ba);if(Object.keys(this.__lastTap).length>0){var bc=Date.now()-qx.event.handler.GestureCore.DOUBLETAP_TIME;for(var bd in this.__lastTap){if(bd<bc){delete this.__lastTap[bd];}
else {if(this.__isBelowDoubleTapDistance(this.__lastTap[bd].x,this.__lastTap[bd].y,Y.clientX,Y.clientY,Y.getPointerType())){this._fireEvent(Y,p,Y.target||ba);}
;}
;}
;}
;this.__lastTap[Date.now()]={x:Y.clientX,y:Y.clientY};}
else if(!this._isBelowTapMaxDistance(Y)){var bb=this.__getSwipeGesture(Y,ba);if(bb){Y.swipe=bb;this._fireEvent(Y,a,Y.target||ba);}
;}
;delete this.__gesture[Y.pointerId];}
,stopMomentum:function(bf){this.__stopMomentum[bf]=true;}
,gestureCancel:function(bg){if(this.__gesture[bg]){this.__stopLongTapTimer(this.__gesture[bg]);delete this.__gesture[bg];}
;}
,__handleRollImpulse:function(bl,bm,bi,bj,bo){var bn=bi.timeoutId;if((Math.abs(bm)<1&&Math.abs(bl)<1)||this.__stopMomentum[bn]){delete this.__stopMomentum[bn];return;}
;if(!bo){bo=1;var bk=2.8;bm=bm/bk;bl=bl/bk;}
;bo+=0.0006;bm=bm/bo;bl=bl/bo;var bh=qx.bom.AnimationFrame.request(qx.lang.Function.bind(function(bp,bq,br,bs,bt){this.__handleRollImpulse(bp,bq,br,bs,bt);}
...
,_calcAngle:function(){var bv=null;var bw=null;for(var bu in this.__gesture){var bx=this.__gesture[bu];if(bv===null){bv=bx;}
else {bw=bx;}
;}
;var x=bv.clientX-bw.clientX;var y=bv.clientY-bw.clientY;return (360+Math.atan2(y,x)*(180/Math.PI))%360;}
,_calcDistance:function(){var by=null;var bz=null;for(var bB in this.__gesture){var bC=this.__gesture[bB];if(by===null){by=bC;}
else {bz=bC;}
;}
...
,__isBelowDoubleTapDistance:function(bG,bK,bL,bM,bN){var bJ=qx.event.handler.GestureCore;var bH=Math.abs(bG-bL)<bJ.TAP_MAX_DISTANCE[bN];var bI=Math.abs(bK-bM)<bJ.TAP_MAX_DISTANCE[bN];return bH&&bI;}
,_getDeltaCoordinates:function(bQ){var bR=this.__gesture[bQ.pointerId];if(!bR){return null;}
;var bO=bQ.clientX-bR.startX;var bP=bQ.clientY-bR.startY;var bS=E;if(Math.abs(bO/bP)<1){bS=n;}
;return {"x":bO,"y":bP,"axis":bS};}
,_fireEvent:function(bU,bW,bV){if(!this.__defaultTarget){return;}
...
else if(this.__emitter){bT=new qx.event.type.dom.Custom(bW,bU,{target:this.__defaultTarget,currentTarget:this.__defaultTarget,srcElement:this.__defaultTarget,swipe:bU.swipe,scale:bU.scale,angle:bU.angle,delta:bU.delta,pointerType:bU.pointerType,momentum:bU.momentum});this.__emitter.emit(bW,bU);}
;}
,_onDblClick:function(bX){var bY=qx.bom.Event.getTarget(bX);this._fireEvent(bX,B,bY);this._fireEvent(bX,p,bY);}
,__getSwipeGesture:function(cc,cd){var cj=this.__gesture[cc.pointerId];if(!cj){return null;}
...
,__fireTrack:function(cl,cm,cn){cm.delta=this._getDeltaCoordinates(cm);this._fireEvent(cm,cl,cm.target||cn);}
...
else {var cr=this.__gesture[cp.pointerId];cp.delta={x:-cr.velocityX,y:-cr.velocityY,axis:Math.abs(cr.velocityX/cr.velocityY)<1?n:E};}
;this._fireEvent(cp,h,cp.target||cq);}
,__fireRotate:function(cs,cu){if(!cs.isPrimary){var ct=this._calcAngle();cs.angle=Math.round((ct-this.__initialAngle)%360);this._fireEvent(cs,k,this.__primaryTarget);}
;}
,__firePinch:function(cx,cy){if(!cx.isPrimary){var cv=this._calcDistance();var cw=cv/this.__initialDistance;cx.scale=(Math.round(cw*100)/100);this._fireEvent(cx,b,this.__primaryTarget);}
;}
,__fireLongTap:function(cz,cA){var cB=this.__gesture[cz.pointerId];if(cB){this._fireEvent(cz,f,cz.target||cA);cB.longTapTimer=null;cB.isTap=false;}
;}
,__stopLongTapTimer:function(cC){if(cC.longTapTimer){window.clearTimeout(cC.longTapTimer);cC.longTapTimer=null;}
;}
...
,dispose:function(){for(var cF in this.__gesture){this.__stopLongTapTimer(cF);}
...
;}
else {if(e.axis&&e.axis==e.HORIZONTAL_AXIS){if((e.detail!==undefined)&&(e.detail>0)){x=this.__normalize(e.detail);}
...
;}
else {if(!(e.axis&&e.axis==e.HORIZONTAL_AXIS)){if((e.detail!==undefined)&&(e.detail>0)){y=this.__normalize(e.detail);}
