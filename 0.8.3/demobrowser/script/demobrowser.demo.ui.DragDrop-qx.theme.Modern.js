...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.ui.DragDrop-qx.theme.Modern-0.js"]],
  boot : "boot",
...
(function(){var r="drop",q="value",p="items",o="move",n="multi",m="dragover",k="icon/16/places/folder.png",j="dragstart",h="Item ",g="copy",E="drag",D="TextArea Target",C="solid",B="Reorderable",A="droprequest",z="#33508D",y="Source",x="demobrowser.demo.ui.DragDrop",w="Simple Target",v="Enable drag",t="\n",u="dragend",s="__Bh";
qx.Class.define(x,{extend:qx.application.Standalone,members:{__Bh:null,__Bi:null,main:function(){arguments.callee.base.call(this);
var Q=this.getRoot();
var K=new qx.ui.container.Scroll();
var F=new qx.ui.layout.Basic();
var I=new qx.ui.container.Composite(F).set({allowStretchY:false,allowStretchX:false});
K.add(I);
Q.add(K,{edge:0});
var P=new qx.ui.basic.Label(y);
I.add(P,{left:20,top:20});
var N=new qx.ui.form.List;
N.setDraggable(true);
N.setSelectionMode(n);
I.add(N,{left:20,top:40});

for(var i=0;i<20;i++){N.add(new qx.ui.form.ListItem(h+i,k));
}var R=new qx.ui.form.CheckBox(v);
R.setValue(true);
I.add(R,{left:20,top:260});
N.addListener(j,function(e){if(!R.isValue()){e.preventDefault();
}e.addType(q);
e.addType(p);
e.addAction(g);
e.addAction(o);
});
N.addListener(A,function(e){this.debug("Related of droprequest: "+e.getRelatedTarget());
...
var L=new qx.ui.basic.Label(w);
I.add(L,{left:140,top:20});
var J=new qx.ui.form.List;
J.setDroppable(true);
J.setSelectionMode(n);
I.add(J,{left:140,top:40});
J.addListener(r,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
var U=e.getData(p);

for(var i=0,l=U.length;i<l;i++){this.add(U[i]);
}});
J.addListener(m,function(e){if(!e.supportsType(p)){e.preventDefault();
}});
var L=new qx.ui.basic.Label(D);
I.add(L,{left:260,top:20});
var O=new qx.ui.form.TextArea;
O.setDroppable(true);
O.setHeight(100);
I.add(O,{left:260,top:40});
O.addListener(r,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
this.setValue(e.getData(q).replace(/,/g,t));
});
O.addListener(m,function(e){if(!e.supportsType(q)){e.preventDefault();
}});
var M=new qx.ui.basic.Label(B);
I.add(M,{left:500,top:20});
var G=this.__Bh=new qx.ui.form.List;
G.setDraggable(true);
G.setDroppable(true);
G.setSelectionMode(n);
I.add(G,{left:500,top:40});

for(var i=0;i<20;i++){G.add(new qx.ui.form.ListItem(h+i,k));
}var H=new qx.ui.core.Widget;
H.setDecorator(new qx.ui.decoration.Single().set({top:[1,C,z]}));
H.setHeight(0);
H.setOpacity(0.5);
H.setZIndex(100);
H.setLayoutProperties({left:-1000,top:-1000});
H.setDroppable(true);
this.getRoot().add(H);
G.addListener(j,function(e){e.addAction(o);
});
G.addListener(u,function(e){H.setDomPosition(-1000,-1000);
});
G.addListener(E,function(e){var W=e.getOriginalTarget();
if(W instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__Bi=W;
}
if(!qx.ui.core.Widget.contains(this,W)&&W!=H){return;
}var V=W.getContainerLocation();
H.setWidth(W.getBounds().width);
H.setDomPosition(V.left,V.top);
});
G.addListener(m,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
G.addListener(r,function(e){this.__Bj(e.getOriginalTarget());
},this);
H.addListener(r,function(e){this.__Bj(this.__Bi);
},this);
},__Bj:function(S){var T=this.__Bh.getSortedSelection();

for(var i=0,l=T.length;i<l;i++){this.__Bh.addBefore(T[i],S);
this.__Bh.addToSelection(T[i]);
