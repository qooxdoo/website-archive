...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.test.TimerManager", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.test.TimerManager-qx.theme.Modern-0.js"]],
  boot : "boot",
...
(function(){var r="Stopping recurrent 2-second timer",q="Recurrent 2-second timer with limit 3:",p="\n",o="__kd",n="One-shot 1-second timer: ",m=" ",l="demobrowser.demo.test.TimerManager",k="Recurrent 5-second timer: ",j="Start timers",i="execute";
qx.Class.define(l,{extend:qx.application.Standalone,members:{startTimers:function(){var t=qx.util.TimerManager.getInstance();
t.start(function(a,b){this.__kc(k+b);
},5000,this,null,0);
t.start(function(u,v){this.__kc(n+v);
},0,this,null,1000);
t.start(function(c,d){this.__kc(q+d);

if(++c.count==3){this.__kc(r);
t.stop(d);
}},2000,this,{count:0},2000);
},__kc:function(s){s=Date.parse(new Date)+m+s+p;
this.__kd.setValue(this.__kd.getValue()+s);
},main:function(){arguments.callee.base.call(this);
var f=new qx.ui.layout.VBox(5);
var e=new qx.ui.container.Composite(f);
var g=new qx.ui.form.Button(j);
g.set({allowGrowX:false,allowGrowY:false});
g.addListener(i,function(){g.setEnabled(false);
this.startTimers();
},this);
var h=new qx.ui.form.TextArea();
h.set({width:400,height:300});
this.__kd=h;
e.add(h);
e.add(g);
...
qx.Class.define(d,{extend:qx.core.Object,type:c,statics:{__ke:[],__kf:{},__kg:0},members:{start:function(f,g,h,j,k){if(!k){k=g||0;
}var l=(new Date()).getTime()+k;
arguments.callee.self.__kf[++arguments.callee.self.__kg]={callback:f,userData:j||null,expireAt:l,recurTime:g,context:h||this};
this.__kh(l,arguments.callee.self.__kg);
return arguments.callee.self.__kg;
},stop:function(a){var b=arguments.callee.self.__ke;
var length=b.length;

for(var i=0;i<length;i++){if(b[i]==a){b.splice(i,1);
break;
}}delete arguments.callee.self.__kf[a];
if(b.length==0){qx.event.Idle.getInstance().removeListener(e,this.__ki);
}},__kh:function(m,n){var p=arguments.callee.self.__ke;
var o=arguments.callee.self.__kf;
var length=p.length;

for(var i=0;i<length;i++){if(o[p[i]].expireAt>m){p.splice(i,0,n);
break;
}}if(p.length==length){p.push(n);
}if(p.length==1){qx.event.Idle.getInstance().addListener(e,this.__ki,this);
}},__ki:function(){var t=(new Date()).getTime();
var r=arguments.callee.self.__ke;
var s=arguments.callee.self.__kf;
...
this.__kh(q.expireAt,v);
}else{delete s[v];
}}if(r.length==0){qx.event.Idle.getInstance().removeListener(e,this.__ki);
}}}});
})();
