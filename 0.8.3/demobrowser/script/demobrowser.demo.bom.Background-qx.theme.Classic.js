...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.bom.Background", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.bom.Background-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var bf="",be="string",bd="div",bc="previousSibling",bb="*",ba="href",Y="e",X="[object Array]",W="Syntax error, unrecognized expression: ",V="HTML",bM="text",bL="file",bK="+",bJ="'/>",bI="CLASS",bH="radio",bG="script",bF="className",bE="BUTTON",bD="TAG",bm="password",bn="htmlFor",bk="reset",bl="<a href='#'></a>",bi="<a name='",bj="#",bg="qx.bom.Selector",bh="button",bo="parentNode",bp="NAME",bv="number",bu="submit",bx="image",bw="<div class='test e'></div><div class='test'></div>",bz="~",by="checkbox",br="sourceIndex",bC="\\",bB="hidden",bA=".TEST",bq="<p class='TEST'></p>",bs="undefined",bt="ID";
qx.Bootstrap.define(bg,{statics:{query:null,matches:null}});
(function(){var dB=/((?:\((?:\([^()]+\)|[^()]+)+\)|\[(?:\[[^[\]]*\]|['"][^'"]*['"]|[^[\]'"]+)+\]|\\.|[^ >+~,(\[\\]+)+|[>+~])(\s*,\s*)?/g,dI=0,dF=Object.prototype.toString,dD=false;
var dx=function(dR,dS,dT,dU){dT=dT||[];
var ee=dS=dS||document;

if(dS.nodeType!==1&&dS.nodeType!==9){return [];
}
if(!dR||typeof dR!==be){return dT;
}var eb=[],m,dX,dV,eg,ef,ea,dY=true,dW=dC(dS);
dB.lastIndex=0;

while((m=dB.exec(dR))!==null){eb.push(m[1]);

if(m[2]){ea=RegExp.rightContext;
break;
}}
if(eb.length>1&&dE.exec(dR)){if(eb.length===2&&dz.relative[eb[0]]){dX=dy(eb[0]+eb[1],dS);
}else{dX=dz.relative[eb[0]]?[dS]:dx(eb.shift(),dS);

while(eb.length){dR=eb.shift();

if(dz.relative[dR])dR+=eb.shift();
dX=dy(dR,dX);
}}}else{if(!dU&&eb.length>1&&dS.nodeType===9&&!dW&&dz.match.ID.test(eb[0])&&!dz.match.ID.test(eb[eb.length-1])){var ec=dx.find(eb.shift(),dS,dW);
dS=ec.expr?dx.filter(ec.expr,ec.set)[0]:ec.set[0];
}
if(dS){var ec=dU?
{expr:eb.pop(),set:dw(dU)}:dx.find(eb.pop(),eb.length===1&&(eb[0]===bz||eb[0]===bK)&&dS.parentNode?dS.parentNode:dS,dW);
dX=ec.expr?dx.filter(ec.expr,ec.set):ec.set;

if(eb.length>0){dV=dw(dX);
}else{dY=false;
}
while(eb.length){var eh=eb.pop(),ed=eh;

if(!dz.relative[eh]){eh=bf;
}else{ed=eb.pop();
}
if(ed==null){ed=dS;
}dz.relative[eh](dV,ed,dW);
}}else{dV=eb=[];
}}
if(!dV){dV=dX;
}
...
if(dF.call(dV)===X){if(!dY){dT.push.apply(dT,dV);
}else if(dS&&dS.nodeType===1){for(var i=0;dV[i]!=null;i++){if(dV[i]&&(dV[i]===true||dV[i].nodeType===1&&dA(dS,dV[i]))){dT.push(dX[i]);
}}}else{for(var i=0;dV[i]!=null;i++){if(dV[i]&&dV[i].nodeType===1){dT.push(dX[i]);
}}}}else{dw(dV,dT);
}
if(ea){dx(ea,ee,dT,dU);
dx.uniqueSort(dT);
}return dT;
};
dx.uniqueSort=function(dj){if(dG){dD=false;
dj.sort(dG);

if(dD){for(var i=1;i<dj.length;i++){if(dj[i]===dj[i-1]){dj.splice(i--,1);
}}}}};
dx.matches=function(cW,cX){return dx(cW,null,null,cX);
};
dx.find=function(ex,ey,ez){var eA,eC;

if(!ex){return [];
}
for(var i=0,l=dz.order.length;i<l;i++){var eB=dz.order[i],eC;

if((eC=dz.match[eB].exec(ex))){var eD=RegExp.leftContext;

if(eD.substr(eD.length-1)!==bC){eC[1]=(eC[1]||bf).replace(/\\/g,bf);
eA=dz.find[eB](eC,ey,ez);

if(eA!=null){ex=ex.replace(dz.match[eB],bf);
break;
}}}}
if(!eA){eA=ey.getElementsByTagName(bb);
}return {set:eA,expr:ex};
};
dx.filter=function(D,E,F,G){var H=D,M=[],I=E,R,Q,J=E&&E[0]&&dC(E[0]);

while(D&&E.length){for(var P in dz.filter){if((R=dz.match[P].exec(D))!=null){var K=dz.filter[P],O,L;
Q=false;

if(I==M){M=[];
}
if(dz.preFilter[P]){R=dz.preFilter[P](R,I,F,M,G,J);

if(!R){Q=O=true;
}else if(R===true){continue;
}}
if(R){for(var i=0;(L=I[i])!=null;i++){if(L){O=K(L,R,i,I);
var N=G^!!O;

if(F&&O!=null){if(N){Q=true;
}else{I[i]=false;
}}else if(N){M.push(L);
Q=true;
}}}}
if(O!==undefined){if(!F){I=M;
}D=D.replace(dz.match[P],bf);

if(!Q){return [];
}break;
}}}if(D==H){if(Q==null){throw W+D;
}else{break;
}}H=D;
}return I;
};
var dz=dx.selectors={order:[bt,bp,bD],match:{ID:/#((?:[\w\u00c0-\uFFFF_-]|\\.)+)/,CLASS:/\.((?:[\w\u00c0-\uFFFF_-]|\\.)+)/,NAME:/\[name=['"]*((?:[\w\u00c0-\uFFFF_-]|\\.)+)['"]*\]/,ATTR:/\[\s*((?:[\w\u00c0-\uFFFF_-]|\\.)+)\s*(?:(\S?=)\s*(['"]*)(.*?)\3|)\s*\]/,TAG:/^((?:[\w\u00c0-\uFFFF\*_-]|\\.)+)/,CHILD:/:(only|nth|last|first)-child(?:\((even|odd|[\dn+-]*)\))?/,POS:/:(nth|eq|gt|lt|first|last|even|odd)(?:\((\d*)\))?(?=[^-]|$)/,PSEUDO:/:((?:[\w\u00c0-\uFFFF_-]|\\.)+)(?:\((['"]*)((?:\([^\)]+\)|[^\2\(\)]*)+)\2\))?/},attrMap:{"class":bF,"for":bn},attrHandle:{href:function(cl){return cl.getAttribute(ba);
}},relative:{"+":function(cs,ct,cu){var cv=typeof ct===be,cx=cv&&!/\W/.test(ct),cy=cv&&!cx;

if(cx&&!cu){ct=ct.toUpperCase();
}
for(var i=0,l=cs.length,cw;i<l;i++){if((cw=cs[i])){while((cw=cw.previousSibling)&&cw.nodeType!==1){}cs[i]=cy||cw&&cw.nodeName===ct?cw||false:cw===ct;
}}
if(cy){dx.filter(ct,cs,true);
}},">":function(ej,ek,el){var em=typeof ek===be;

if(em&&!/\W/.test(ek)){ek=el?ek:ek.toUpperCase();

for(var i=0,l=ej.length;i<l;i++){var en=ej[i];

if(en){var parent=en.parentNode;
ej[i]=parent.nodeName===ek?parent:false;
}}}else{for(var i=0,l=ej.length;i<l;i++){var en=ej[i];

if(en){ej[i]=em?en.parentNode:en.parentNode===ek;
}}
if(em){dx.filter(ek,ej,true);
}}},"":function(er,es,et){var eu=dI++,ev=dJ;

if(!/\W/.test(es)){var ew=es=et?es:es.toUpperCase();
ev=dL;
}ev(bo,es,eu,er,ew,et);
},"~":function(cf,cg,ch){var ci=dI++,cj=dJ;

if(typeof cg===be&&!/\W/.test(cg)){var ck=cg=ch?cg:cg.toUpperCase();
cj=dL;
}cj(bc,cg,ci,cf,ck,ch);
}},find:{ID:function(dN,dO,dP){if(typeof dO.getElementById!=="undefined"&&!dP){var m=dO.getElementById(dN[1]);
return m?[m]:[];
}},NAME:function(bY,ca,cb){if(typeof ca.getElementsByName!=="undefined"){var cd=[],cc=ca.getElementsByName(bY[1]);

for(var i=0,l=cc.length;i<l;i++){if(cc[i].getAttribute("name")===bY[1]){cd.push(cc[i]);
}}return cd.length===0?null:cd;
}},TAG:function(db,dc){return dc.getElementsByTagName(db[1]);
}},preFilter:{CLASS:function(eX,eY,fa,fb,fc,fd){eX=" "+eX[1].replace(/\\/g,"")+" ";

if(fd){return eX;
}
for(var i=0,fe;(fe=eY[i])!=null;i++){if(fe){if(fc^(fe.className&&(" "+fe.className+" ").indexOf(eX)>=0)){if(!fa)fb.push(fe);
}else if(fa){eY[i]=false;
}}}return false;
},ID:function(dM){return dM[1].replace(/\\/g,"");
},TAG:function(eo,ep){for(var i=0;ep[i]===false;i++){}return ep[i]&&dC(ep[i])?eo[1]:eo[1].toUpperCase();
},CHILD:function(cU){if(cU[1]=="nth"){var cV=/(-?)(\d*)n((?:\+|-)?\d*)/.exec(cU[2]=="even"&&"2n"||cU[2]=="odd"&&"2n+1"||!/\D/.test(cU[2])&&"0n+"+cU[2]||cU[2]);
cU[2]=(cV[1]+(cV[2]||1))-0;
cU[3]=cV[3]-0;
}cU[0]=dI++;
return cU;
},ATTR:function(dq,dr,ds,dt,du,dv){var name=dq[1].replace(/\\/g,"");

if(!dv&&dz.attrMap[name]){dq[1]=dz.attrMap[name];
}
if(dq[2]==="~="){dq[4]=" "+dq[4]+" ";
}return dq;
},PSEUDO:function(cJ,cK,cL,cM,cN){if(cJ[1]==="not"){if(dB.exec(cJ[3]).length>1||/^\w/.test(cJ[3])){cJ[3]=dx(cJ[3],null,null,cK);
}else{var cO=dx.filter(cJ[3],cK,cL,true^cN);

if(!cL){cM.push.apply(cM,cO);
}return false;
}}else if(dz.match.POS.test(cJ[0])||dz.match.CHILD.test(cJ[0])){return true;
}return cJ;
},POS:function(bN){bN.unshift(true);
return bN;
}},filters:{enabled:function(S){return S.disabled===false&&S.type!==bB;
},disabled:function(c){return c.disabled===true;
},checked:function(C){return C.checked===true;
},selected:function(bT){bT.parentNode.selectedIndex;
return bT.selected===true;
},parent:function(bU){return !!bU.firstChild;
},empty:function(dQ){return !dQ.firstChild;
},has:function(ff,i,fg){return !!dx(fg[3],ff).length;
},header:function(cr){return /h\d/i.test(cr.nodeName);
},text:function(cm){return bM===cm.type;
},radio:function(eq){return bH===eq.type;
},checkbox:function(cn){return by===cn.type;
},file:function(dn){return bL===dn.type;
},password:function(u){return bm===u.type;
},submit:function(ei){return bu===ei.type;
},image:function(fu){return bx===fu.type;
},reset:function(eM){return bk===eM.type;
},button:function(dp){return bh===dp.type||dp.nodeName.toUpperCase()===bE;
},input:function(cz){return /input|select|textarea|button/i.test(cz.nodeName);
}},setFilters:{first:function(fJ,i){return i===0;
},last:function(dk,i,dl,dm){return i===dm.length-1;
},even:function(cS,i){return i%2===0;
},odd:function(eH,i){return i%2===1;
},lt:function(T,i,U){return i<U[3]-0;
},gt:function(fv,i,fw){return i>fw[3]-0;
},nth:function(v,i,w){return w[3]-0==i;
},eq:function(eK,i,eL){return eL[3]-0==i;
}},filter:{PSEUDO:function(x,y,i,z){var name=y[1],A=dz.filters[name];

if(A){return A(x,i,y,z);
}else if(name==="contains"){return (x.textContent||x.innerText||"").indexOf(y[3])>=0;
}else if(name==="not"){var B=y[3];

for(i=0,l=B.length;i<l;i++){if(B[i]===x){return false;
}}return true;
}},CHILD:function(eO,eP){var eV=eP[1],eQ=eO;

switch(eV){case 'only':case 'first':while((eQ=eQ.previousSibling)){if(eQ.nodeType===1)return false;
}
if(eV=='first')return true;
eQ=eO;
case 'last':while((eQ=eQ.nextSibling)){if(eQ.nodeType===1)return false;
}return true;
case 'nth':var eW=eP[2],eS=eP[3];

if(eW==1&&eS==0){return true;
}var eU=eP[0],parent=eO.parentNode;

if(parent&&(parent.sizcache!==eU||!eO.nodeIndex)){var eR=0;

for(eQ=parent.firstChild;eQ;eQ=eQ.nextSibling){if(eQ.nodeType===1){eQ.nodeIndex=++eR;
}}parent.sizcache=eU;
}var eT=eO.nodeIndex-eS;

if(eW==0){return eT==0;
}else{return (eT%eW==0&&eT/eW>=0);
}}},ID:function(eI,eJ){return eI.nodeType===1&&eI.getAttribute("id")===eJ;
},TAG:function(dh,di){return (di==="*"&&dh.nodeType===1)||dh.nodeName===di;
},CLASS:function(fK,fL){return (" "+(fK.className||fK.getAttribute("class"))+" ").indexOf(fL)>-1;
},ATTR:function(n,o){var name=o[1],s=dz.attrHandle[name]?dz.attrHandle[name](n):n[name]!=null?n[name]:n.getAttribute(name),r=s+"",q=o[2],p=o[4];
return s==null?q==="!=":q==="="?r===p:q==="*="?r.indexOf(p)>=0:q==="~="?(" "+r+" ").indexOf(p)>=0:!p?r&&s!==false:q==="!="?r!=p:q==="^="?r.indexOf(p)===0:q==="$="?r.substr(r.length-p.length)===p:q==="|="?r===p||r.substr(0,p.length+1)===p+"-":false;
},POS:function(fk,fl,i,fm){var name=fl[2],fn=dz.setFilters[name];

if(fn){return fn(fk,i,fl,fm);
}}}};
var dE=dz.match.POS;

for(var dK in dz.match){dz.match[dK]=new RegExp(dz.match[dK].source+/(?![^\[]*\])(?![^\(]*\))/.source);
}var dw=function(cY,da){cY=Array.prototype.slice.call(cY,0);

if(da){da.push.apply(da,cY);
return da;
}return cY;
};
try{Array.prototype.slice.call(document.documentElement.childNodes,0);
}catch(e){dw=function(fh,fi){var fj=fi||[];

if(dF.call(fh)===X){Array.prototype.push.apply(fj,fh);
}else{if(typeof fh.length===bv){for(var i=0,l=fh.length;i<l;i++){fj.push(fh[i]);
}}else{for(var i=0;fh[i];i++){fj.push(fh[i]);
}}}return fj;
};
}var dG;

if(document.documentElement.compareDocumentPosition){dG=function(a,b){var cT=a.compareDocumentPosition(b)&4?-1:a===b?0:1;

if(cT===0){dD=true;
}return cT;
};
}else if(br in document.documentElement){dG=function(a,b){var bO=a.sourceIndex-b.sourceIndex;

if(bO===0){dD=true;
}return bO;
};
}else if(document.createRange){dG=function(a,b){var fs=a.ownerDocument.createRange(),ft=b.ownerDocument.createRange();
fs.selectNode(a);
fs.collapse(true);
ft.selectNode(b);
ft.collapse(true);
...
}(function(){var bX=document.createElement(bd),bW=bG+(new Date).getTime();
bX.innerHTML=bi+bW+bJ;
var bV=document.documentElement;
bV.insertBefore(bX,bV.firstChild);
if(!!document.getElementById(bW)){dz.find.ID=function(cP,cQ,cR){if(typeof cQ.getElementById!=="undefined"&&!cR){var m=cQ.getElementById(cP[1]);
return m?m.id===cP[1]||typeof m.getAttributeNode!=="undefined"&&m.getAttributeNode("id").nodeValue===cP[1]?[m]:undefined:[];
}};
dz.filter.ID=function(fo,fp){var fq=typeof fo.getAttributeNode!=="undefined"&&fo.getAttributeNode("id");
return fo.nodeType===1&&fq&&fq.nodeValue===fp;
};
}bV.removeChild(bX);
bV=bX=null;
})();
(function(){var eN=document.createElement(bd);
eN.appendChild(document.createComment(bf));
if(eN.getElementsByTagName(bb).length>0){dz.find.TAG=function(dd,de){var dg=de.getElementsByTagName(dd[1]);
if(dd[1]==="*"){var df=[];

for(var i=0;dg[i];i++){if(dg[i].nodeType===1){df.push(dg[i]);
}}dg=df;
}return dg;
};
}eN.innerHTML=bl;

if(eN.firstChild&&typeof eN.firstChild.getAttribute!==bs&&eN.firstChild.getAttribute(ba)!==bj){dz.attrHandle.href=function(t){return t.getAttribute(ba,2);
};
}eN=null;
})();

if(document.querySelectorAll)(function(){var fH=dx,fG=document.createElement(bd);
fG.innerHTML=bq;
if(fG.querySelectorAll&&fG.querySelectorAll(bA).length===0){return;
}dx=function(bP,bQ,bR,bS){bQ=bQ||document;
if(!bS&&bQ.nodeType===9&&!dC(bQ)){try{return dw(bQ.querySelectorAll(bP),bR);
}catch(e){}}return fH(bP,bQ,bR,bS);
};

for(var fI in fH){dx[fI]=fH[fI];
}fG=null;
})();

if(document.getElementsByClassName&&document.documentElement.getElementsByClassName)(function(){var ce=document.createElement(bd);
ce.innerHTML=bw;
if(ce.getElementsByClassName(Y).length===0)return;
ce.lastChild.className=Y;

if(ce.getElementsByClassName(Y).length===1)return;
dz.order.splice(1,0,bI);
dz.find.CLASS=function(eE,eF,eG){if(typeof eF.getElementsByClassName!=="undefined"&&!eG){return eF.getElementsByClassName(eE[1]);
}};
ce=null;
})();
function dL(cA,cB,cC,cD,cE,cF){var cG=cA==bc&&!cF;

for(var i=0,l=cD.length;i<l;i++){var cI=cD[i];

if(cI){if(cG&&cI.nodeType===1){cI.sizcache=cC;
cI.sizset=i;
}cI=cI[cA];
var cH=false;

while(cI){if(cI.sizcache===cC){cH=cD[cI.sizset];
break;
}
if(cI.nodeType===1&&!cF){cI.sizcache=cC;
cI.sizset=i;
}
if(cI.nodeName===cB){cH=cI;
break;
}cI=cI[cA];
}cD[i]=cH;
}}}function dJ(fx,fy,fz,fA,fB,fC){var fD=fx==bc&&!fC;

for(var i=0,l=fA.length;i<l;i++){var fF=fA[i];

if(fF){if(fD&&fF.nodeType===1){fF.sizcache=fz;
fF.sizset=i;
}fF=fF[fx];
var fE=false;

while(fF){if(fF.sizcache===fz){fE=fA[fF.sizset];
break;
}
if(fF.nodeType===1){if(!fC){fF.sizcache=fz;
fF.sizset=i;
}
if(typeof fy!==be){if(fF===fy){fE=true;
break;
}}else if(dx.filter(fy,[fF]).length>0){fE=fF;
break;
}}fF=fF[fx];
}fA[i]=fE;
}}}var dA=document.compareDocumentPosition?
function(a,b){return a.compareDocumentPosition(b)&16;
}:
function(a,b){return a!==b&&(a.contains?a.contains(b):true);
};
var dC=function(cq){return cq.nodeType===9&&cq.documentElement.nodeName!==V||!!cq.ownerDocument&&cq.ownerDocument.documentElement.nodeName!==V;
};
var dy=function(d,f){var h=[],g=bf,k,j=f.nodeType?[f]:f;
while((k=dz.match.PSEUDO.exec(d))){g+=k[0];
d=d.replace(dz.match.PSEUDO,bf);
}d=dz.relative[d]?d+bb:d;

for(var i=0,l=j.length;i<l;i++){dx(d,j[i],h);
}return dx.filter(g,h);
};
var dH=qx.bom.Selector;
dH.query=function(fM,fN){return dx(fM,fN);
};
dH.matches=function(co,cp){return dx(co,null,null,cp);
};
})();
})();
...
},has:function(p,name){var q=new RegExp(h+name+i);
return q.test(p.className);
},remove:function(a,name){var b=new RegExp(h+name+i);
a.className=a.className.replace(b,d);
return name;
},replace:function(l,m,n){this.remove(l,m);
return this.add(l,n);
},toggle:function(j,name,k){if(k==null){k=!this.has(j,name);
...
(function(){var cY="get",cX="set",cW="reset",cV=":not(",cU="getValue",cT="append",cS=")",cR="getPreviousSiblings",cQ="getOffsetParent",cP="qx.bom.Collection",dy="setValue",dx="prepend",dw="string",dv="getAncestors",du="#",dt="remove",ds=">*",dr="add",dq="*",dp="",dg="addListener",dh="has",de="toggle",df="getSiblings",dc="replace",dd="after",da="replaceWith",db="setCss",di="setStyles",dj="before",dl="getNextSiblings",dk="getPosition",dn="getCss",dm="removeListener";
(function(){var br=function(d,e){return function(Y,ba,bb,bc,bd,be){var length=this.length;

if(length>0){var bf=d[e];

for(var i=0;i<length;i++){if(this[i].nodeType===1){bf.call(d,this[i],Y,ba,bb,bc,bd,be);
}}}return this;
};
};
var bq=function(bj,bk){return function(n,o,p,q,r,s){if(this.length>0){var t=this[0].nodeType===1?bj[bk](this[0],n,o,p,q,r,s):null;

if(t&&t.nodeType){return this.__xf([t]);
}else{return t;
}}return null;
};
};
qx.Class.define(cP,{extend:qx.type.BaseArray,statics:{query:function(J,K){var L=qx.bom.Selector.query(J,K);
return qx.lang.Array.cast(L,qx.bom.Collection);
},id:function(cv){var cw=document.getElementById(cv);
if(cw&&cw.id!=cv){return qx.bom.Collection.query(du+cv);
}return new qx.bom.Collection(cw);
},html:function(bt,bu){var bv=qx.bom.Html.clean([bt],bu);
return qx.lang.Array.cast(bv,qx.bom.Collection);
},__xb:/^[^<]*(<(.|\s)+>)[^>]*$|^#([\w-]+)$/,create:function(cL,cM){var cO=qx.bom.Collection;
if(cL.nodeType){return new cO(cL);
}else if(typeof cL===dw){var cN=cO.__xb.exec(cL);

if(cN){return cN[1]?cO.html(cN[1],cM):cO.id(cN[3].substring(1));
}else{return cO.query(cL,cM);
}}else{return qx.lang.Array.cast(cL,qx.bom.Collection);
}}},members:{__xc:null,setAttribute:br(qx.bom.element.Attribute,cX),resetAttribute:br(qx.bom.element.Attribute,cW),getAttribute:bq(qx.bom.element.Attribute,cY),addClass:br(qx.bom.element.Class,dr),getClass:bq(qx.bom.element.Class,cY),hasClass:bq(qx.bom.element.Class,dh),removeClass:br(qx.bom.element.Class,dt),replaceClass:br(qx.bom.element.Class,dc),toggleClass:br(qx.bom.element.Class,de),setValue:br(qx.bom.Input,dy),getValue:bq(qx.bom.Input,cU),setStyle:br(qx.bom.element.Style,cX),setStyles:br(qx.bom.element.Style,di),resetStyle:br(qx.bom.element.Style,cW),getStyle:bq(qx.bom.element.Style,cY),setCss:br(qx.bom.element.Style,db),getCss:br(qx.bom.element.Style,dn),getOffset:bq(qx.bom.element.Location,cY),getPosition:bq(qx.bom.element.Location,dk),getOffsetParent:bq(qx.bom.element.Location,cQ),setScrollLeft:function(z){var Node=qx.dom.Node;

for(var i=0,l=this.length,A;i<l;i++){A=this[i];

if(Node.isElement(A)){A.scrollLeft=z;
}else if(Node.isWindow(A)){A.scrollTo(z,this.getScrollTop(A));
}else if(Node.isDocument(A)){Node.getWindow(A).scrollTo(z,this.getScrollTop(A));
}}return this;
...
},getScrollLeft:function(){var f=this[0];

if(!f){return null;
}var Node=qx.dom.Node;

if(Node.isWindow(f)||Node.isDocument(f)){return qx.bom.Viewport.getScrollLeft();
}return f.scrollLeft;
...
},getWidth:function(){var V=this[0];
var Node=qx.dom.Node;

if(V){if(Node.isElement(V)){return qx.bom.element.Dimension.getWidth(V);
}else if(Node.isDocument(V)){return qx.bom.Document.getWidth(Node.getWindow(V));
}else if(Node.isWindow(V)){return qx.bom.Viewport.getWidth(V);
}}return null;
},getContentWidth:function(){var b=this[0];

if(qx.dom.Node.isElement(b)){return qx.bom.element.Dimension.getContentWidth(b);
}return null;
},getHeight:function(){var cr=this[0];
var Node=qx.dom.Node;

if(cr){if(Node.isElement(cr)){return qx.bom.element.Dimension.getHeight(cr);
}else if(Node.isDocument(cr)){return qx.bom.Document.getHeight(Node.getWindow(cr));
}else if(Node.isWindow(cr)){return qx.bom.Viewport.getHeight(cr);
}}return null;
},getContentHeight:function(){var g=this[0];

if(qx.dom.Node.isElement(g)){return qx.bom.element.Dimension.getContentHeight(g);
}return null;
},addListener:br(qx.bom.Element,dg),removeListener:br(qx.bom.Element,dm),eq:function(bH){return this.slice(bH,+bH+1);
},filter:function(M,N){var O;

if(qx.lang.Type.isFunction(M)){O=qx.type.BaseArray.prototype.filter.call(this,M,N);
}else{O=qx.bom.Selector.matches(M,this);
}return this.__xf(O);
},is:function(bA){return !!bA&&qx.bom.Selector.matches(bA,this).length>0;
},__xd:/^.[^:#\[\.,]*$/,not:function(v){if(this.__xd.test(v)){var w=qx.bom.Selector.matches(cV+v+cS,this);
return this.__xf(w);
}var w=qx.bom.Selector.matches(v,this);
return this.filter(function(bB){return w.indexOf(bB)===-1;
});
},add:function(cn,co){var cp=qx.bom.Selector.query(cn,co);
var cq=qx.lang.Array.unique(this.concat(cp));
return this.__xf(cq);
},children:function(cs){var ct=[];

for(var i=0,l=this.length;i<l;i++){ct.push.apply(ct,qx.dom.Hierarchy.getChildElements(this[i]));
}
if(cs){ct=qx.bom.Selector.matches(cs,ct);
}return this.__xf(ct);
},closest:function(bw){var bx=new qx.bom.Collection(1);
var bz=qx.bom.Selector;
var by=this.map(function(bE){while(bE&&bE.ownerDocument){bx[0]=bE;

if(bz.matches(bw,bx).length>0){return bE;
}bE=bE.parentNode;
}});
return this.__xf(qx.lang.Array.unique(by));
},contents:function(){var cz=[];
var cy=qx.lang.Array;

for(var i=0,l=this.length;i<l;i++){cz.push.apply(cz,cy.fromCollection(this[i].childNodes));
}return this.__xf(cz);
},find:function(h){var m=qx.bom.Selector;
if(this.length===1){return this.__xf(m.query(h,this[0]));
}else{var k=[];

for(var i=0,l=this.length;i<l;i++){k.push.apply(k,m.query(h,this[i]));
}return this.__xf(qx.lang.Array.unique(k));
}},next:function(cd){var ce=qx.dom.Hierarchy;
var cf=this.map(ce.getNextElementSibling,ce);
if(cd){cf=qx.bom.Selector.matches(cd,cf);
}return this.__xf(cf);
},nextAll:function(bi){return this.__xe(dl,bi);
},prev:function(cB){var cC=qx.dom.Hierarchy;
var cD=this.map(cC.getPreviousElementSibling,cC);
if(cB){cD=qx.bom.Selector.matches(cB,cD);
}return this.__xf(cD);
},prevAll:function(cg){return this.__xe(cR,cg);
},parent:function(D){var Element=qx.dom.Element;
var E=qx.lang.Array.unique(this.map(Element.getParentElement,Element));
if(D){E=qx.bom.Selector.matches(D,E);
}return this.__xf(E);
},parents:function(U){return this.__xe(dv,U);
},siblings:function(bs){return this.__xe(df,bs);
},__xe:function(P,Q){var S=[];
var R=qx.dom.Hierarchy;

for(var i=0,l=this.length;i<l;i++){S.push.apply(S,R[P](this[i]));
}var T=qx.lang.Array.unique(S);
if(Q){T=qx.bom.Selector.matches(Q,T);
}return this.__xf(T);
},__xf:function(cb){var cc=new qx.bom.Collection;
cc.__xc=this;
cb=Array.prototype.slice.call(cb,0);
cc.push.apply(cc,cb);
return cc;
},andSelf:function(){return this.add(this.__xc);
},end:function(){return this.__xc||new qx.bom.Collection();
},__xg:function(bI,bJ){var bO=this[0];
var bN=bO.ownerDocument||bO;
var bM=bN.createDocumentFragment();
var bQ=qx.bom.Html.clean(bI,bN,bM);
var bS=bM.firstChild;
if(bS){var bK=this.length-1;

for(var i=0,l=bK;i<l;i++){bJ.call(this,this[i],bM.cloneNode(true));
}bJ.call(this,this[bK],bM);
}if(bQ){var bL;
var bR=qx.io2.ScriptLoader;
var bP=qx.lang.Function;

for(var i=0,l=bQ.length;i<l;i++){bL=bQ[i];
if(bL.src){bR.get().load(bL.src);
}else{bP.globalEval(bL.text||bL.textContent||bL.innerHTML||dp);
}if(bL.parentNode){bL.parentNode.removeChild(bL);
}}}return this;
},__xh:function(ch,ci){var ck=qx.bom.Selector;
var cj=qx.lang.Array;
var cm=[];

for(var i=0,l=ch.length;i<l;i++){{};
cm.push.apply(cm,ck.query(ch[i]));
}cm=cj.cast(cj.unique(cm),qx.bom.Collection);
for(var i=0,cl=this.length;i<cl;i++){cm[ci](this[i]);
}return this;
},append:function(c){return this.__xg(arguments,this.__xi);
},prepend:function(bF){return this.__xg(arguments,this.__xj);
},__xi:function(x,y){x.appendChild(y);
},__xj:function(W,X){W.insertBefore(X,W.firstChild);
},appendTo:function(cx){return this.__xh(arguments,cT);
},prependTo:function(bU){return this.__xh(arguments,dx);
},before:function(bC){return this.__xg(arguments,this.__xk);
},after:function(bp){return this.__xg(arguments,this.__xl);
},__xk:function(bg,bh){bg.parentNode.insertBefore(bh,bg);
},__xl:function(bl,bm){bl.parentNode.insertBefore(bm,bl.nextSibling);
},insertBefore:function(a){return this.__xh(arguments,dj);
},insertAfter:function(H){return this.__xh(arguments,dd);
},wrapAll:function(content){var bo=this[0];

if(bo){var bn=qx.bom.Collection.create(content,bo.ownerDocument).clone();
if(bo.parentNode){bo.parentNode.insertBefore(bn[0],bo);
}bn.map(this.__xm).append(this);
}return this;
},__xm:function(bD){while(bD.firstChild){bD=bD.firstChild;
}return bD;
},wrapInner:function(content){var cA=new qx.bom.Collection(1);

for(var i=0,l=this.length;i<l;i++){cA[0]=this[i];
cA.contents().wrapAll(content);
}return this;
...
},replaceAll:function(I){return this.__xh(arguments,da);
...
},empty:function(){var cu=qx.bom.Collection;

for(var i=0,l=this.length;i<l;i++){cu.query(ds,this[i]).destroy();
while(this.firstChild){this.removeChild(this.firstChild);
}}return this;
},clone:function(bG){var Element=qx.bom.Element;
return bG?
this.map(function(u){return Element.clone(u,true);
}):this.map(Element.clone,Element);
}},defer:function(B,C){if(window.$==null){window.$=B.create;
}}});
})();
})();
...
qx.Class.define(x,{statics:{__xn:function(be,bf,bg){return bg.match(/^(abbr|br|col|img|input|link|meta|param|hr|area|embed)$/i)?be:bf+J+bg+R;
},__xo:{opt:[1,r,p],leg:[1,s,E],table:[1,t,I],tr:[2,B,O],td:[3,S,o],col:[2,Q,M],def:qx.core.Variant.select(w,{"mshtml":[1,C,q],"default":null})},__xp:function(U,V){var bc=V.createElement(T);
U=U.replace(/(<(\w+)[^>]*?)\/>/g,this.__xn);
...
},hasChildElements:function(j){j=j.firstChild;

while(j){if(j.nodeType===1){return true;
}j=j.nextSibling;
}return false;
},getParentElement:function(c){return c.parentNode;
...
return true;
},removeChild:function(k,parent){if(k.parentNode!==parent){return false;
