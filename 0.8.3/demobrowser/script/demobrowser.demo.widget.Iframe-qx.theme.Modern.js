...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.widget.Iframe", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.widget.Iframe-qx.theme.Modern-0.js"]],
  boot : "boot",
...
(function(){var m="url",l="http://www.gmx.net",k="web.de",j="qx.event.type.DataEvent",i="http://www.web.de",h="../welcome.html",g="GMX",f="load",c="demobrowser.demo.widget.Iframe",b="changeSelection",a="local";
qx.Class.define(c,{extend:qx.application.Standalone,events:{"surfTo":j},members:{main:function(){arguments.callee.base.call(this);
var q=new qx.ui.embed.ThemedIframe();
var d=this.getRoot();
q.addListener(f,function(e){this.debug("Loaded: "+this.getSource());
});
q.setSource(l);
d.add(q,{top:100,right:20,bottom:20,left:20});
var n=new qx.ui.form.RadioButton(g);
n.setUserData(m,l);
...
o.setUserData(m,h);
n.setValue(true);
var r=new qx.ui.form.RadioGroup(n,p,o);
r.addListener(b,function(e){this.setSource(e.getData()[0].getUserData(m));
},q);
d.add(n,{left:20,top:48});
d.add(p,{left:120,top:48});
d.add(o,{left:220,top:48});
}}});
})();
...
(function(){var r="auto",q="scrollbar-y",p="corner",o="iframe",n="scrollbar-x",m="on",l="_updateScrollbars",k="load",j="off",i="scroll",F="__we",E="mousewheel",D="interval",C="scrollbarX",B="resize",A="vertical",z="__wd",w="horizontal",v="scrollbarY",u="hidden",s="scrollarea",t="qx.ui.embed.ThemedIframe";
qx.Class.define(t,{extend:qx.ui.embed.AbstractIframe,construct:function(T){arguments.callee.base.call(this,T);
var U=new qx.ui.layout.Grid();
U.setColumnFlex(0,1);
U.setRowFlex(0,1);
this._setLayout(U);
this._showChildControl(o);
},properties:{appearance:{refine:true,init:s},scrollbarX:{check:[r,m,j],init:r,themeable:true,apply:l},scrollbarY:{check:[r,m,j],init:r,themeable:true,apply:l},scrollbar:{group:[C,v]}},members:{__wd:null,__we:null,_getIframeElement:function(){return this.getChildControl(o).getContentElement();
},_createChildControlImpl:function(X){var Y;

switch(X){case o:Y=new qx.ui.embed.Iframe(this.getSource());
Y.addListener(k,this._onIframeLoad,this);
Y.addListener(B,this._onIframeResize,this);
this._add(Y,{row:0,column:0});
break;
case n:Y=new qx.ui.core.ScrollBar(w);
Y.setMinWidth(0);
Y.exclude();
Y.addListener(i,this._onScrollBarX,this);
this._add(Y,{row:1,column:0});
break;
case q:Y=new qx.ui.core.ScrollBar(A);
Y.setMinHeight(0);
Y.exclude();
Y.addListener(i,this._onScrollBarY,this);
this._add(Y,{row:0,column:1});
break;
case p:Y=new qx.ui.core.Widget();
Y.setWidth(0);
Y.setHeight(0);
Y.exclude();
...
var bb=this._getIframeElement().getBody();

if(bb){this._startIframeObserver();
this._addWheelListener();
}this.fireEvent(k);
},_onIframeResize:function(){this._updateScrollbars();
},_disableScollbars:function(){this._excludeChildControl(n);
this._excludeChildControl(q);
this._excludeChildControl(p);
this._stopIframeObserver();
},_addWheelListener:function(){try{var ba=this._getIframeElement().getBody();
qx.bom.Element.addListener(ba,E,this._onMouseWheel,this);
}catch(e){this._disableScollbars();
}},_onMouseWheel:function(e){var J=this._isChildControlVisible(q);

if(!J){return;
}var I=this.getChildControl(q,true);
I.scrollBySteps(e.getWheelDelta());
e.stop();
},_startIframeObserver:function(){if(this.__we){this._stopIframeObserver();
}var L=qx.event.Idle.getInstance();
this.__we=L.addListener(D,this._onIframeObserverInterval,this);
...
if(this.__wd&&g.width==this.__wd.width&&g.height==this.__wd.height){return;
}this.__wd=g;
this._preventIframeScrolling();
this._updateScrollbars();
},_preventIframeScrolling:function(){try{var G=this._getIframeElement().getBody();
G.style.overflow=u;
}catch(e){this._disableScollbars();
}},_updateScrollbars:function(){var M=this.__wd;
var P=this.getChildControl(o).getBounds();
...
if(Q===r&&R===r){var O=M.width>N.width;
var S=M.height>N.height;
if((O||S)&&!(O&&S)){if(O){S=M.height>P.height;
}else if(S){O=M.width>P.width;
}}}else{var O=Q===m;
var S=R===m;
if(M.width>(O?P.width:N.width)&&Q===r){O=true;
}
if(M.height>(O?P.height:N.height)&&R===r){S=true;
}}this._configureScrollbar(n,O,N.width,M.width);
this._configureScrollbar(q,S,N.height,M.height);
this._updateCornerWidget();
},_getIframeSize:function(){try{var V=this._getIframeElement().getDocument();
var W={width:qx.bom.element.Dimension.getWidth(V.body),height:qx.bom.element.Dimension.getHeight(V.body)};
return W;
}catch(e){return null;
}},_updateCornerWidget:function(){if(this._isChildControlVisible(n)&&this._isChildControlVisible(q)){this._showChildControl(p);
...
},scrollToX:function(x){try{var H=this._getIframeElement().getWindow();
H.scroll(x,qx.bom.Viewport.getScrollTop(H));
}catch(e){this._disableScollbars();
}},scrollToY:function(y){try{var K=this._getIframeElement().getWindow();
K.scroll(qx.bom.Viewport.getScrollLeft(K),y);
}catch(e){this._disableScollbars();
}}},destruct:function(){this._stopIframeObserver();
this._disposeFields(F,z);
}});
})();
