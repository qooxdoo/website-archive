...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.table.Table_Cell_Editor", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.table.Table_Cell_Editor-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var y="checkbox",x='Property',w="options",v="password",u="type",t='email',s='password',r='options',q='type',p='0',bo='Use only numbers!',bn='regExp',bm='[0-9]+',bl='newsletter',bk='admin',bj='editable combobox',bi='secret',bh="demobrowser.demo.table.Table_Cell_Editor",bg='status',bf="validationFunc",F='1',G="regExp",D="required",E='Doubleclick on cell to edit value',B='Newsletter',C='Inactive',z='User Name',A='telephone',J="dataChanged",K='Status',S='Active',Q='validationFunc',W='enter website',U="editable",bb='role',Y='username',M='jdoe',be='Telephone',bd='manager',bc='enter valid telephone number',L='Waiting',O='Email',P='Role',R="Property editor table",T='2',V='user',X='website',ba='Password',H="email",I='Website',N='enter valid email';
qx.Class.define(bh,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return R;
},main:function(){arguments.callee.base.call(this);
this._container.resetWidth();
this._container.setResizable(false);
},createTable:function(){var bM=[[Y,z,M,{'required':true}],[s,ba,bi,{'type':s,'required':true}],[bb,P,bj,{'options':[bk,bd,V],'editable':true}],[bg,K,p,{'options':[[C,null,p],[S,null,F],[L,null,T]]}],[t,O,N,{'type':t}],[A,be,bc,{'regExp':bm,'failMsg':bo}],[X,I,W,{'validationFunc':function(bN){if(bN.search(/^http:/)!=-1){window.open(bN);
}return bN;
}}],[bl,B,true,{'type':y}]];
var bL=function(e){var k=e.table;
var h=k.getTableModel();
var i=h.getRowData(e.row);
var l=i[3];
var j;

for(var g in l){switch(g){case u:switch(l[q]){case y:return new qx.ui.table.cellrenderer.Boolean;
case v:return new qx.ui.table.cellrenderer.Password;
}break;
case w:var j=new qx.ui.table.cellrenderer.Replace;
var f={};
l[r].forEach(function(m){if(m instanceof Array){f[m[0]]=m[2];
}});
j.setReplaceMap(f);
j.addReversedReplaceMap();
return j;
}}return new qx.ui.table.cellrenderer.Default();
};
var bK=new qx.ui.table.cellrenderer.Dynamic(bL);
var bI=function(bw){var bB=bw.table;
var bz=bB.getTableModel();
var bA=bz.getRowData(bw.row);
var bD=bA[3];
var by=new qx.ui.table.celleditor.TextField;
var bC=null;

for(var bx in bD){switch(bx){case w:if(bD.editable){by=new qx.ui.table.celleditor.ComboBox();
}else{by=new qx.ui.table.celleditor.SelectBox();
}by.setListData(bD[r]);
break;
case U:break;
case u:switch(bD[q]){case v:by=new qx.ui.table.celleditor.PasswordField;
break;
case y:by=new qx.ui.table.celleditor.CheckBox;
break;
case H:by.setValidationFunction(function(bs,bt){var bu=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{2}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/;

if(bu.test(bs)){return bs;
}alert("You did not enter a valid email address");
return bt;
});
break;
}break;
case G:by.setValidationFunction(function(bp,bq){var br=new RegExp(bD[bn]);

if(br.test(bp)){return bp;
}alert(bD['failMsg']);
return bq;
});
break;
case bf:by.setValidationFunction(bD[Q]);
break;
case D:bC=function(n,o){if(!n){alert("You need to supply a value here");
return o;
}return n;
};
break;
...
var bH=new qx.ui.table.celleditor.Dynamic(bI);
var bF=new qx.ui.table.model.Simple();
bF.setColumns([x,x,E]);
var bE={tableColumnModel:function(bv){return new qx.ui.table.columnmodel.Resize(bv);
}};
var bG=new qx.ui.table.Table(bF,bE);
bG.setDecorator(null);
bG.setColumnVisibilityButtonVisible(false);
bG.setKeepFirstVisibleRowComplete(true);
bG.setStatusBarVisible(false);
bG.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var bJ=bG.getTableColumnModel();
bJ.setColumnVisible(0,false);
bJ.getBehavior().setWidth(1,100);
bJ.getBehavior().setWidth(2,300);
bF.setColumnEditable(2,true);
bJ.setDataCellRenderer(2,bK);
bJ.setCellEditorFactory(2,bH);
bG.getTableModel().setData(bM);
bG.getTableModel().addListener(J,function(event){if(!(event instanceof qx.event.type.Data)){return;
}var d=event.getData();
var a=this.getTableModel();
var c=a.getValue(0,d.firstRow);
var b=a.getValue(d.firstColumn,d.firstRow);
this.info("User edited property '"+c+"' and entered value '"+b+"'.");
},bG);
return bG;
}}});
})();
...
},properties:{validationFunction:{check:f,nullable:true,init:null},listData:{check:c,init:null,nullable:true}},members:{__oE:null,createCellEditor:function(m){var o=new qx.ui.form.ComboBox().set({appearance:b});
var p=m.value;
...
if(!this.__oE&&j){k=j(k,h.originalValue);
this.__oE=true;
}
...
(function(){var w="",v="row",u="Function",t="qx.ui.table.celleditor.SelectBox",s="number",r="table-editor-selectbox",q="appear",p="Array";
qx.Class.define(t,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,construct:function(){arguments.callee.base.call(this);
},properties:{validationFunction:{check:u,nullable:true,init:null},listData:{check:p,init:null,nullable:true}},members:{__oG:null,createCellEditor:function(a){var d=new qx.ui.form.SelectBox().set({appearance:r});
var e=a.value;
d.originalValue=e;
var h=a.table.getTableColumnModel().getDataCellRenderer(a.col);
var f=h._getContentHtml(a);

if(e!=f){e=f;
}if(e===null){e=w;
}var c=this.getListData();

if(c){var g;

for(var i=0,l=c.length;i<l;i++){var j=c[i];

if(j instanceof Array){g=new qx.ui.form.ListItem(j[0],j[1]);
g.setUserData(v,j[2]);
}else{g=new qx.ui.form.ListItem(j,null);
g.setUserData(v,j);
}d.add(g);
}}var b=d.getChildrenContainer().findItem(w+e);

if(b){d.setSelection([b]);
}else{d.resetSelection();
}d.addListener(q,function(){d.open();
});
return d;
},getCellEditorValue:function(k){var m=k.getSelection();
var o=w;

if(m&&m[0]){o=m[0].getUserData(v)||m[0].getLabel();
}var n=this.getValidationFunction();

if(!this.__oG&&n){o=n(o,k.originalValue);
this.__oG=true;
}
if(typeof k.originalValue==s){o=parseFloat(o);
}return o;
}}});
})();
...
(function(){var f="",e="Function",d="qx.ui.table.celleditor.PasswordField",c="number",b="table-editor-textfield",a="appear";
qx.Class.define(d,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,construct:function(){arguments.callee.base.call(this);
},properties:{validationFunction:{check:e,nullable:true,init:null}},members:{__oJ:null,createCellEditor:function(j){var k=new qx.ui.form.PasswordField();
k.setAppearance(b);
...
if(!this.__oJ&&h){i=h(i,g.originalValue);
this.__oJ=true;
}
...
},members:{createCellEditor:function(k){var m=new qx.ui.container.Composite(new qx.ui.layout.HBox().set({alignX:d,alignY:b})).set({focusable:true});
...
(function(){var g="Function",f="__oL",e="qx.ui.table.celleditor.Dynamic";
qx.Class.define(e,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,construct:function(a){arguments.callee.base.call(this);

if(a){this.setCellEditorFactoryFunction(a);
}},properties:{cellEditorFactoryFunction:{check:g,nullable:true,init:null}},members:{__oL:null,createCellEditor:function(b){var c=this.getCellEditorFactoryFunction();
{};
this.__oL=c(b);
var d=this.__oL.createCellEditor(b);
return d;
},getCellEditorValue:function(h){var i=this.getCellEditorFactoryFunction();
{};
var j=this.__oL.getCellEditorValue(h);
return j;
