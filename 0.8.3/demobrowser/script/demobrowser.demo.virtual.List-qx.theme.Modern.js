...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.virtual.List", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.virtual.List-qx.theme.Modern-0.js"]],
  boot : "boot",
...
(function(){var q="value",p="one",o="multi",n="changeValue",m="single",l="additive",k="bold",j="Item No ",h="Julian",g="Moritz",bD="Jannis",bC="Kevin",bB="Robin",bA="Felix",bz="Vincent",by="demobrowser.demo.virtual.List",bx="Additive Selection",bw="/places/folder.png",bv="Nils",bu="Nick",x="Leon",y="Jannick",v="Linus",w="Configurable",t="Marc",u="icon/16/places/folder.png",r="Erik",s="addaptive",F="Colin",G="Lennard",V="changeSelection",R="Jason",be="Tobias",Y="Florian",bq="16",bk="Jonas",M="icon/",bt="Jakob",bs="Enable drag selection",br="Joel",K="David",O="Maximilian",Q="48",T="Marlon",W="Lukas",ba="Luca",bg="Julius",bm="Fabian",z="Tim",A="Justin",N="Single Selection",bd="Simon",bc="Luis",bb="Johannes",bi="Nico",bh="Alexander",X="Jan",bf="Dominic",c="Noah",bl="Enable quick selection",B="Daniel",C="Paul",S="Philipp",d="Niclas",f="Max",J="One Selection",D="Additive selection",E="Jonathan",I="Tom",U="Finn",bo="Ben",bn="on",P="One as selection mode",bp="Elias",L="Mika",bj="Multi Selection",H="Marvin";
qx.Class.define(by,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var bQ=new qx.ui.basic.Label(w);
bQ.setFont(k);
this.getRoot().add(bQ,{left:20,top:20});
var bR=new demobrowser.demo.virtual.list.List();
bR.setScrollbarX(bn);
bR.set({height:280,width:150,selectionMode:o});
var bN;

for(var i=1;i<=2500;i++){bN=new demobrowser.demo.virtual.list.ListItem(j+i,M+((i%4)?bq:Q)+bw);

if(i%4==0){bN.setHeight(56);
}bR.add(bN);
if(i==20){bR.setSelected(bN);
}}bR.addListener(n,function(e){this.debug("Value: "+e.getData());
});
this.getRoot().add(bR,{left:20,top:40});
var bJ=new qx.ui.form.RadioButton(N);
var bG=new qx.ui.form.RadioButton(bj);
var bH=new qx.ui.form.RadioButton(bx);
var bF=new qx.ui.form.RadioButton(J);
bJ.setUserData(q,m);
bG.setUserData(q,o);
bH.setUserData(q,l);
bF.setUserData(q,p);
bG.setValue(true);
this.getRoot().add(bJ,{left:180,top:40});
this.getRoot().add(bG,{left:180,top:60});
this.getRoot().add(bH,{left:180,top:80});
this.getRoot().add(bF,{left:180,top:100});
var bI=new qx.ui.form.RadioGroup(bJ,bG,bH,bF);
var bM=new qx.ui.form.CheckBox(bs);
var bP=new qx.ui.form.CheckBox(bl).set({enabled:false});
this.getRoot().add(bM,{left:180,top:220});
this.getRoot().add(bP,{left:180,top:240});
bM.addListener(n,function(e){if(e.getData()){var bE=bR.getSelectionMode();

if(bE==m||bE==p){this.debug("Drag selection is only available for the modes multi or additive");
}}bR.setDragSelection(e.getData());
});
bP.addListener(n,function(e){if(e.getData()){var b=bR.getSelectionMode();

if(b==o||b==l){this.debug("Quick selection is only available for the modes multi or additive");
}}bR.setQuickSelection(e.getData());
});
bI.addListener(V,function(e){var a=e.getData()[0].getUserData(q);
bR.setSelectionMode(a);

if(a==m||a==p){bM.setEnabled(false);
bP.setEnabled(true);
}else if(a==o||a==s){bM.setEnabled(true);
bP.setEnabled(false);
}});
var bT=new qx.ui.basic.Label(P);
bT.setFont(k);
this.getRoot().add(bT,{left:330,top:20});
var bO=new demobrowser.demo.virtual.list.List();
bO.set({height:280,width:150,selectionMode:p});
var bN;

for(var i=1;i<=25;i++){bN=new demobrowser.demo.virtual.list.ListItem(j+i,u);
bO.add(bN);
if(i==16){bO.setSelected(bN);
}}this.getRoot().add(bO,{left:330,top:40});
var bQ=new qx.ui.basic.Label(D);
bQ.setFont(k);
this.getRoot().add(bQ,{left:520,top:20});
var bS=new demobrowser.demo.virtual.list.List;
var bK;
bS.set({width:150,selectionMode:l});
var bL=[x,W,ba,U,z,bA,bk,bc,O,h,f,C,d,X,bo,bp,y,S,c,I,g,bi,K,bv,bd,bm,r,A,bh,bt,Y,bu,v,L,R,B,G,H,bD,be,bf,T,t,bb,E,bg,F,br,bC,bz,bB];

for(var i=0;i<bL.length;i++){bK=new demobrowser.demo.virtual.list.ListItem(bL[i]);
bS.add(bK);

if(i==10||i==12||i==16){bS.addToSelection(bK);
...
(function(){var F="selected",E="qx.event.type.Data",D="Boolean",C="changeSelection",B="row",A="single",z="changeHeight",y=",",x="one",w="_applySpacing",U="Integer",T="_applyDragSelection",S="String",R="resize",Q="demobrowser.demo.virtual.list.List",P="list",O="multi",N="_applyQuickSelection",M="_applyItemHeight",L="_applySelectionMode",J="additive",K="changeName",H="Number",I="addItem",G="removeItem";
qx.Class.define(Q,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider,qx.ui.form.IFormElement],construct:function(){arguments.callee.base.call(this,0,1,this.getItemHeight(),10);
this.__tx=new qx.ui.virtual.layer.WidgetCell(this);
this.getPane().addLayer(this.__tx);
this.getPane().addListener(R,this._onResize,this);
this.__ty=[];
this.__tz=[];
this.__tA=new qx.ui.virtual.selection.Row(this.getPane());
this.__tA.addListener(C,this._onChangeSelection,this);
this.__tA.attachMouseEvents();
this.__tA.attachKeyEvents(this);
this.__tA.attachListEvents(this);
var n=new qx.ui.virtual.behavior.Prefetch(this,0,0,0,0,400,600,400,600).set({interval:500});
},events:{addItem:E,removeItem:E,changeValue:E,changeSelection:E},properties:{appearance:{refine:true,init:P},focusable:{refine:true,init:true},spacing:{check:U,init:0,apply:w,themeable:true},enableInlineFind:{check:D,init:true},name:{check:S,nullable:true,event:K},itemHeight:{init:24,themeable:true,check:H,apply:M},selectionMode:{check:[A,O,J,x],init:A,apply:L},dragSelection:{check:D,init:false,apply:T},quickSelection:{check:D,init:false,apply:N}},members:{__ty:null,__tz:null,__tA:null,syncWidget:function(){this.update();
},update:function(){var r=this.getPane().getRowConfig();
r.setItemCount(this.__ty.length);
r.resetItemSizes();

for(var i=0;i<this.__ty.length;i++){var q=this.__ty[i].getHeight();

if(q!==null){r.setItemSize(i,q);
}}},updateSelection:function(){var bk=this.__tx.getChildren();

for(var i=0;i<bk.length;i++){var bl=bk[i];
var bm=bl.getUserData(B);

if(this.__tA.isItemSelected(bm)){bl.addState(F);
}else{bl.removeState(F);
...
this.fireDataEvent(C,this.__tE(e.getData()));
},getCellWidget:function(bn,bo){var bq=this.__ty[bn];

if(!bq){return null;
}var bp=this.__tz.pop()||new qx.ui.form.ListItem();
bp.set({label:bq.getLabel(),icon:bq.getIcon()});

if(this.__tA.isItemSelected(bn)){bp.addState(F);
}else{bp.removeState(F);
}bp.setUserData(B,bn);
return bp;
},poolCellWidget:function(bA){this.__tz.push(bA);
...
},setValue:function(g){var m=g.split(y);
var k=[];
var h;

for(var i=0,l=m.length;i<l;i++){h=this.findItem(m[i]);

if(h){k.push(h);
}}this.setSelection(k);
},getChildren:function(){return this.__ty;
},hasChildren:function(){return this.__ty.length>0;
},_addHelper:function(v){this.fireDataEvent(I,v);
v.addListener(z,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},_removeHelper:function(bz){this.fireDataEvent(G,bz);
bz.removeListener(z,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},add:function(V){this.__ty.push(V);
this._addHelper(V);
return this;
},remove:function(bf){qx.lang.Array.remove(this.__ty,bf);
this._removeHelper(bf);
return this;
},removeAll:function(){for(var i=0,j=this.__ty.lenth;i<j;i++){this._removeHelper(child);
}this.__ty=[];
},indexOf:function(bC){return this.__ty.indexOf(bC);
},addAt:function(bd,be){qx.lang.Array.insertAt(this.__ty,bd,be);
this._addHelper(bd);
},addBefore:function(bt,bu){qx.lang.Array.insertBefore(this.__ty,bt,bu);
this._addHelper(bt);
},addAfter:function(bg,bh){qx.lang.Array.insertAfter(this.__ty,bg,bh);
this._addHelper(bg);
},removeAt:function(W){this.__ty.splice(W,1);
this._removeHelper(child);
},__tB:function(o){var p=this.__ty.indexOf(o);

if(p<0){return null;
}else{return p;
}},__tC:function(bx){var by=this.__ty[bx];
return by;
},__tD:function(t){var u=[];

for(var i=0;i<t.length;i++){u.push(this.__tB(t[i]));
}return u;
},__tE:function(bb){var bc=[];

for(var i=0;i<bb.length;i++){bc.push(this.__ty[bb[i]]);
}return bc;
},selectAll:function(){this.__tA.selectAll();
},setSelected:function(bB){this.__tA.selectItem(this.__tB(bB));
},isSelected:function(X){return this.__tA.isItemSelected(this.__tB(X));
},addToSelection:function(ba){this.__tA.addItem(this.__tB(ba));
},removeFromSelection:function(s){this.__tA.removeItem(this.__tB(s));
},selectRange:function(br,bs){this.__tA.selectItemRange(this.__tB(br),this.__tB(bs));
},resetSelection:function(){this.__tA.clearSelection();
},setSelection:function(Y){this.__tA.replaceSelection(this.__tD(Y));
},getSelectedItem:function(){return this.__ty[this.getSelected()];
},getSelected:function(){return this.__ty[this.__tA.getSelectedItem()];
},getSelection:function(){return this.__tE(this.__tA.getSelection());
},getSortedSelection:function(){return this.__tE(this.__tA.getSortedSelection());
},isSelectionEmpty:function(){return this.__tA.isSelectionEmpty();
},getSelectionContext:function(){return this.__tA.getSelectionContext();
},getSelectables:function(){return this.__tE(this.__tA.getSelectables());
},_applySelectionMode:function(d,f){this.__tA.setMode(d);
},_applyDragSelection:function(bi,bj){this.__tA.setDrag(bi);
},_applyQuickSelection:function(bv,bw){this.__tA.setQuick(bv);
}}});
})();
...
(function(){var q="scroll",p="scrollbar-x",o="scrollbar-y",n="__tt",m="_applyInterval",l="__ts",k="appear",j="interval",i="qx.ui.virtual.behavior.Prefetch",h="_applyScroller",e="qx.ui.virtual.core.Scroller",g="__tr",f="Integer";
qx.Class.define(i,{extend:qx.core.Object,construct:function(t,u,v,w,x,y,z,A,B){arguments.callee.base.call(this);
this.setPrefetchX(u,v,w,x);
this.setPrefetchY(y,z,A,B);
this.__tr=new qx.event.Timer(this.getInterval());
this.__tr.addListener(j,this._onInterval,this);

if(t){this.setScroller(t);
}},properties:{scroller:{check:e,nullable:true,init:null,apply:h},interval:{check:f,init:200,apply:m}},members:{__ts:null,__tt:null,__tr:null,__tu:null,__tv:null,setPrefetchX:function(G,H,I,J){this.__ts=[G,H,I,J];
},setPrefetchY:function(a,b,c,d){this.__tt=[a,b,c,d];
},_onInterval:function(){var r=this.__ts;
...
qx.ui.core.queue.Manager.flush();
}},_applyScroller:function(E,F){if(F){if(this.__tu){F.getChildControl(p).removeListenerById(this.__tu);
}
if(this.__tv){F.getChildControl(o).removeListenerById(this.__tv);
}}
if(E){if(!E.getContainerElement().getDomElement()){this.__tr.stop();
E.addListenerOnce(k,this.__tr.start,this.__tr);
}else{this.__tr.restart();
}this.__tu=E.getChildControl(p).addListener(q,this.__tr.restart,this.__tr);
this.__tv=E.getChildControl(o).addListener(q,this.__tr.restart,this.__tr);
}else{this.__tr.stop();
}},_applyInterval:function(C,D){this.__tr.setInterval(C);
}},destruct:function(){this.setScroller(null);
this._disposeFields(l,n);
