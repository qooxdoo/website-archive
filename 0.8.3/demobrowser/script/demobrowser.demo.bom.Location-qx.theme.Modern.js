...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.bom.Location", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.bom.Location-qx.theme.Modern-0.js"]],
  boot : "boot",
...
(function(){var s="px",r="moveable",q="demobrowser.demo.bom.Location",p="borderBottomWidth",o="list",n='#',m="mode",k="borderTopWidth",j="borderLeftWidth",h="a",f="div",g="borderRightWidth";
qx.Class.define(q,{extend:qx.application.Native,members:{main:function(){arguments.callee.base.call(this);
var e=document.getElementById(o).getElementsByTagName(h);

for(var i=0,l=e.length;i<l;i++){e[i].onclick=demobrowser.demo.bom.Location.jump;
}}},statics:{num:function(c,d){return parseInt(qx.bom.element.Style.get(c,d,qx.bom.element.Style.COMPUTED_MODE,false),10);
},jump:function(event){if(!event){event=window.event;
}event.returnValue=false;

if(event.preventDefault){event.preventDefault();
}function u(a,b){return parseInt(qx.bom.element.Style.get(a,b,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var w=this.href;
var y=w.substr(w.indexOf(n)+1);
var v=document.getElementById(y);
var location=qx.bom.element.Location.get(v,document.getElementById(m).value);
var x=document.getElementById(r);
x.style.left=location.left+s;
x.style.top=location.top+s;

if(qx.bom.client.Feature.BORDER_BOX){x.style.width=(location.right-location.left)+s;
x.style.height=(location.bottom-location.top)+s;
}else{x.style.width=(location.right-location.left-u(x,j)-u(x,g))+s;
x.style.height=(location.bottom-location.top-u(x,k)-u(x,p))+s;
}return false;
},randomize:function(){var t=document.body.getElementsByTagName(f);

for(var i=0,l=t.length;i<l;i++){if(t[i].id&&t[i].id!=r){t[i].style.margin=Math.round(Math.random()*10)+s;
t[i].style.padding=Math.round(Math.random()*10)+s;
t[i].style.borderWidth=Math.round(Math.random()*10)+s;
}}}}});
})();
