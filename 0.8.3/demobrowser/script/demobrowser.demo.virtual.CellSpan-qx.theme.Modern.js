...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.virtual.CellSpan", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.virtual.CellSpan-qx.theme.Modern-0.js"]],
  boot : "boot",
...
(function(){var n="yellow",m="bold",l="table-scroller-header",k="green",j=" / ",i="Widget Cells",h="demobrowser.demo.virtual.CellSpan",g="HTML Cells",f="x",e="_pool",d="__tF";
...
a.add(new qx.ui.basic.Label(i).set({font:m,decorator:l,padding:3,allowGrowX:true}),{row:0,column:1});
var c=this.getWidgetCellSpanScroller();
a.add(c,{row:1,column:1});
this.__tF=new qx.ui.virtual.cell.Cell();
},getHtmlCellSpanScroller:function(){var t=new qx.ui.virtual.core.Scroller(1000,100,50,120);
var s=t.getPane();
var r=new qx.ui.virtual.layer.HtmlCellSpan(this,s.getRowConfig(),s.getColumnConfig());
r.setCellSpan(1,1,2,2);
r.setCellSpan(1,5,3,3);
r.setCellSpan(7,4,2,4);
r.setCellSpan(6,0,10,2);
r.setCellSpan(10,9,10,5);
r.setCellSpan(11,3,6,3);
t.getPane().addLayer(r);
return t;
},getCellProperties:function(y,z){var A=(y+z)%2==0?n:k;
this.__tF.setBackgroundColor(A);
return this.__tF.getCellProperties(y+j+z);
},getWidgetCellSpanScroller:function(){var q=new qx.ui.virtual.core.Scroller(1000,100,50,120);
var p=q.getPane();
var o=new qx.ui.virtual.layer.WidgetCellSpan(this,p.getRowConfig(),p.getColumnConfig());
o.setCellSpan(1,1,2,2);
o.setCellSpan(1,5,3,3);
o.setCellSpan(7,4,2,4);
o.setCellSpan(6,0,10,2);
o.setCellSpan(10,9,10,5);
o.setCellSpan(11,3,6,3);
q.getPane().addLayer(o);
this._pool=[];
return q;
},getCellWidget:function(u,v){var w=this._pool.pop()||new qx.ui.basic.Label().set({allowGrowX:true});
w.set({backgroundColor:(u+v)%2==0?n:k,value:u+f+v,padding:3});
return w;
},poolCellWidget:function(x){this._pool.push(x);
...
(function(){var z="",w="' ",v="px;",u="</div>",t="top:",s=">",r="_spanManager",q="<div ",p="qx.ui.virtual.layer.HtmlCellSpan",o="style='",k="class='",n="x",m="left:",j="html";
qx.Class.define(p,{extend:qx.ui.virtual.layer.HtmlCell,construct:function(P,Q,R){arguments.callee.base.call(this,P);
this._spanManager=new qx.ui.virtual.layer.CellSpanManager(Q,R);
},members:{setCellSpan:function(S,T,U,V){var W=S+n+T;
this._spanManager.removeCell(W);

if(U>1||V>1){this._spanManager.addCell(W,S,T,U,V);
}qx.ui.core.queue.Widget.add(this);
},__tG:function(a,b,c,d,top,e,f){var h=this._cellProvider.getCellProperties(b,c);
var g=h.insets||[0,0];
a.push(q,o,m,d,v,t,top,v,this._getCellSizeStyle(e,f,g[0],g[1]),h.style||z,w,k,h.classes||z,w,h.attributes||z,s,h.content||z,u);
...
var O=J[i];
this.__tG(M,K.firstRow,K.firstColumn,O.left,O.top,O.width,O.height);
}}else{G=[];
...
if(!G[F][L]){this.__tG(M,F,L,N,top,H,E);
}L++;
N+=H;
}top+=E;
F++;
}this.getContentElement().setAttribute(j,M.join(z));
}},destruct:function(){this._disposeObjects(r);
}});
})();
(function(){var G="change",F="_cells",E="lastColumn",D="_sorted",C="_columnPos",B="_rowPos",A="_rowConfig",z="firstColumn",y="firstRow",x="qx.ui.virtual.layer.CellSpanManager",v="lastRow",w="_columnConfig";
qx.Class.define(x,{extend:qx.core.Object,construct:function(bn,bo){arguments.callee.base.call(this);
{};
this._cells={};
this._invalidateSortCache();
this._invalidatePositionCache();
bn.addListener(G,this._onRowConfigChange,this);
bo.addListener(G,this._onColumnConfigChange,this);
this._rowConfig=bn;
this._columnConfig=bo;
...
this._invalidateSortCache();
},removeCell:function(P){delete (this._cells[P]);
this._invalidateSortCache();
},_invalidateSortCache:function(){this._sorted={};
},_getSortedCells:function(bt){if(this._sorted[bt]){return this._sorted[bt];
}var bu=this._sorted[bt]=qx.lang.Object.getValues(this._cells);
bu.sort(function(a,b){return a[bt]<b[bt]?-1:1;
});
return bu;
},_findCellsInRange:function(Y,ba,bb){var bd=this._getSortedCells(Y);

if(bd.length==0){return {};
}var bf=0;
var bc=bd.length-1;
while(true){var bh=bf+((bc-bf)>>1);
var be=bd[bh];

if(be[Y]>=ba&&(bh==0||bd[bh-1][Y]<ba)){break;
}
if(be[Y]>=ba){bc=bh-1;
}else{bf=bh+1;
}
...
while(be&&be[Y]>=ba&&be[Y]<=bb){bg[be.id]=be;
be=bd[bh++];
}return bg;
},findCellsInWindow:function(H,I,J,K){var O={};

if(J>0){var M=H+J-1;
qx.lang.Object.merge(O,this._findCellsInRange(y,H,M),this._findCellsInRange(v,H,M));
}var L={};

if(K>0){var N=I+K-1;
qx.lang.Object.merge(L,this._findCellsInRange(z,I,N),this._findCellsInRange(E,I,N));
}return this.__tH(L,O);
},__tH:function(U,V){var W=[];
...
return by;
},_getColumnPosition:function(bv){var bw=this._columnPos[bv];

if(bw!==undefined){return bw;
}bw=this._columnPos[bv]=this._columnConfig.getItemPosition(bv);
return bw;
},_getSingleCellBounds:function(Q,R,S){var T={left:0,top:0,width:0,height:0};
T.height=this._getRowPosition(Q.lastRow)+this._rowConfig.getItemSize(Q.lastRow)-this._getRowPosition(Q.firstRow);
T.top=this._getRowPosition(Q.firstRow)-this._getRowPosition(R);
T.width=this._getColumnPosition(Q.lastColumn)+this._columnConfig.getItemSize(Q.lastColumn)-this._getColumnPosition(Q.firstColumn);
T.left=this._getColumnPosition(Q.firstColumn)-this._getColumnPosition(S);
return T;
},getCellBounds:function(bp,bq,br){var bs=[];

for(var i=0,l=bp.length;i<l;i++){bs.push(this._getSingleCellBounds(bp[i],bq,br));
}return bs;
},computeCellSpanMap:function(c,d,f,g,h){var j=[];

if(g<=0){return j;
}var t=d+g-1;

for(var i=d;i<=t;i++){j[i]=[];
}
if(h<=0){return j;
}var m=f+h-1;

for(var i=0,l=c.length;i<l;i++){var p=c[i];
var n=Math.max(d,p.firstRow);
var o=Math.min(t,p.lastRow);
var u;

for(var q=n;q<=o;q++){u=j[q];
var k=Math.max(f,p.firstColumn);
var s=Math.min(m,p.lastColumn);

for(var r=k;r<=s;r++){u[r]=1;
}}}return j;
}},destruct:function(){this._rowConfig.removeListener(G,this._onRowConfigChange,this);
this._columnConfig.removeListener(G,this._onColumnConfigChange,this);
this._disposeFields(F,D,B,C,A,w);
}});
})();
(function(){var H="cell.spanning",G="cell.rowspan",F="cell.column",E="cell.colspan",D="cell.row",C="x",B="qx.ui.virtual.layer.WidgetCellSpan",A="_spanManager",z="_cellProvider",y="_bounds",v="_cellLayer",x="__tI",w="_spanMap",u="_cells";
qx.Class.define(B,{extend:qx.ui.virtual.layer.Abstract,include:[qx.ui.core.MChildrenHandling],construct:function(r,s,t){arguments.callee.base.call(this);
this.setZIndex(2);
this._spanManager=new qx.ui.virtual.layer.CellSpanManager(s,t);
this._cellProvider=r;
this.__tI=[];
this._cellLayer=new qx.ui.virtual.layer.WidgetCell(this.__tJ());
this._cellLayer.setZIndex(0);
this._setLayout(new qx.ui.layout.Grow());
this._add(this._cellLayer);
},properties:{anonymous:{refine:true,init:false}},members:{getRenderedCellWidget:function(e,f){var g=this._cellLayer.getRenderedCellWidget(e,f);

if(!g||g.getUserData(H)){var k=this._getChildren();

for(var i=0,l=k.length;i<l;i++){var j=k[i];

if(j==this._cellLayer){continue;
}var h={row:j.getUserData(D),column:j.getUserData(F),rowSpan:j.getUserData(G),colSpan:j.getUserData(E)};

if(h.row<=e&&e<h.row+h.rowSpan&&h.column<=f&&f<h.column+h.colSpan){return j;
}}return null;
}return g;
},__tI:null,setCellSpan:function(L,M,N,O){var P=L+C+M;
this._spanManager.removeCell(P);

if(N>1||O>1){this._spanManager.addCell(P,L,M,N,O);
}qx.ui.core.queue.Widget.add(this);
},__tJ:function(){var self=this;
var q=this._cellProvider;
var p=this.__tI;
var o={getCellWidget:function(I,J){if(!self._spanMap[I][J]){var K=q.getCellWidget(I,J);
}else{var K=p.pop();

if(!K){K=new qx.ui.core.Spacer();
K.setUserData(H,1);
}}return K;
},poolCellWidget:function(n){if(n.getUserData(H)){p.push(n);
}else{q.poolCellWidget(n);
}}};
return o;
},__tK:function(ba,bb,bc,bd){this._cells=this._spanManager.findCellsInWindow(ba,bb,bc,bd);

if(this._cells.length>0){this._bounds=this._spanManager.getCellBounds(this._cells,ba,bb);
this._spanMap=this._spanManager.computeCellSpanMap(this._cells,ba,bb,bc,bd);
}else{this._bounds=[];
this._spanMap=[];

for(var i=0;i<bc;i++){this._spanMap[ba+i]=[];
}}},__tL:function(){var Y=this.getChildren();
...
V.setUserData(D,W.firstRow);
V.setUserData(F,W.firstColumn);
V.setUserData(G,W.lastRow-W.firstRow+1);
V.setUserData(E,W.lastColumn-W.firstColumn+1);
this._add(V);
}}},_fullUpdate:function(a,b,c,d){this.__tK(a,b,c.length,d.length);
this.__tL();
this._cellLayer.fullUpdate(a,b,c,d);
},_updateLayerWindow:function(Q,R,S,T){this.__tK(Q,R,S.length,T.length);
this.__tL();
this._cellLayer.updateLayerWindow(Q,R,S,T);
}},destruct:function(){var m=this._getChildren();

for(var i=0;i<m.length;i++){m[i].dispose();
}this._disposeObjects(A,v);
this._disposeFields(z,x,u,y,w);
}});
})();
