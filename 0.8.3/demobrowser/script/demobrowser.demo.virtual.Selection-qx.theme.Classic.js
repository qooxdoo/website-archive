...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.virtual.Selection", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.virtual.Selection-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var q="manager",p="multi",o=" / ",n="bold",m="value",l="white",k="selected",j="#EEE",i="horizontal",h="table-scroller-header",L="one",K="single",J="changeValue",I="table-row-background-selected",H="additive",G="default",F="Additive Selection",E="Cell Line Selection",D="addaptive",C="Options",x="Row Selection",y="changeSelection",v="Column Selection",w="Enable drag selection",t="demobrowser.demo.virtual.Selection",u="Selection Mode",r="Single Selection",s="Cell Rectangle Selection",z="Enable quick selection",A="One Selection",B="Multi Selection";
qx.Class.define(t,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var T=qx.theme.manager.Font.getInstance().resolve(G).getStyles();
this._fontCss=qx.bom.element.Style.compile(T);
var bd=new qx.ui.layout.Grid(5,0);
bd.setRowFlex(2,1);
bd.setColumnFlex(0,1);
bd.setColumnFlex(1,1);
bd.setColumnFlex(2,1);
bd.setColumnFlex(3,1);
var Q=[];
var bb=new qx.ui.container.Composite(bd);
this.getRoot().add(bb,{edge:5});
bb.add(new qx.ui.basic.Label(x).set({font:n,decorator:h,padding:3,allowGrowX:true}),{row:1,column:0});
var S=this.createRowSelectionScroller();
Q.push(S.getUserData(q));
bb.add(S,{row:2,column:0});
bb.add(new qx.ui.basic.Label(v).set({font:n,decorator:h,padding:3,allowGrowX:true}),{row:1,column:1});
var S=this.createColumnSelectionScroller();
Q.push(S.getUserData(q));
bb.add(S,{row:2,column:1});
bb.add(new qx.ui.basic.Label(s).set({font:n,decorator:h,padding:3,allowGrowX:true}),{row:1,column:2});
var S=this.createCellRectangleSelectionScroller();
Q.push(S.getUserData(q));
bb.add(S,{row:2,column:2});
bb.add(new qx.ui.basic.Label(E).set({font:n,decorator:h,padding:3,allowGrowX:true}),{row:1,column:3});
var S=this.createCellLineSelectionScroller();
Q.push(S.getUserData(q));
bb.add(S,{row:2,column:3});
var Y=new qx.ui.layout.Grid(20,4);
var V=new qx.ui.container.Composite(Y).set({padding:15,backgroundColor:l});
bb.add(V,{row:0,column:0,colSpan:4});
V.add(new qx.ui.basic.Label(u).set({font:n,padding:3}),{row:0,column:0});
var U=new qx.ui.form.RadioButton(r);
U.setUserData(m,K);
V.add(U,{row:1,column:0});
var X=new qx.ui.form.RadioButton(B);
X.setUserData(m,p);
X.setValue(true);
V.add(X,{row:2,column:0});
var R=new qx.ui.form.RadioButton(F);
R.setUserData(m,H);
V.add(R,{row:3,column:0});
var P=new qx.ui.form.RadioButton(A);
P.setUserData(m,L);
V.add(P,{row:4,column:0});
var W=new qx.ui.form.RadioGroup(U,X,R,P);
W.addListener(y,function(e){var bA=e.getData()[0].getUserData(m);
Q.forEach(function(a){a.setMode(bA);
});

if(bA==K||bA==L){bc.setEnabled(false);
ba.setEnabled(true);
}else if(bA==p||bA==D){bc.setEnabled(true);
ba.setEnabled(false);
}});
V.add(new qx.ui.basic.Label(C).set({font:n,padding:3}),{row:0,column:1});
var bc=new qx.ui.form.CheckBox(w);
V.add(bc,{row:1,column:1});
bc.addListener(J,function(e){if(e.getData()){var g=Q[0].getMode();

if(g==K||g==L){this.debug("Drag selection is only available for the modes multi or additive");
}}Q.forEach(function(b){b.setDrag(e.getData());
});
});
var ba=new qx.ui.form.CheckBox(z).set({enabled:false});
V.add(ba,{row:2,column:1});
ba.addListener(J,function(e){if(e.getData()){var bq=Q[0].getMode();

if(bq==p||bq==H){this.debug("Quick selection is only available for the modes multi or additive");
}}Q.forEach(function(be){be.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bN=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var bL=new qx.ui.virtual.layer.Row(l,j);
bN.getPane().addLayer(bL);
bN.getPane().addLayer(new qx.ui.virtual.layer.GridLines(i));
var bM=new qx.ui.virtual.cell.Cell();
var bK=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(c,d){var f={};

if(bJ.isItemSelected(c)){f.selected=true;
}return bM.getCellProperties(c+o+d,f);
}});
bN.getPane().addLayer(bK);
var bJ=new qx.ui.virtual.selection.Row(bN.getPane(),{styleSelectable:function(br,bs,bt){if(bs!==k){return;
}
if(bt){bL.setDecorator(br,k);
}else{bL.setDecorator(br,null);
}bK.updateLayerData();
}});
bJ.attachMouseEvents(bN.getPane());
bJ.attachKeyEvents(bN);
bJ.set({mode:p});
bJ.addItem(0);
bN.setUserData(q,bJ);
return bN;
},createColumnSelectionScroller:function(){var bi=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bi.getPane().addLayer(new qx.ui.virtual.layer.Row(l,j));
var bh=new qx.ui.virtual.layer.Column();
bi.getPane().addLayer(bh);
bi.getPane().addLayer(new qx.ui.virtual.layer.GridLines(i));
var bj=new qx.ui.virtual.cell.Cell();
var bg=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bn,bo){var bp={};

if(bf.isItemSelected(bo)){bp.selected=true;
}return bj.getCellProperties(bn+o+bo,bp);
}});
bi.getPane().addLayer(bg);
var bf=new qx.ui.virtual.selection.Column(bi.getPane(),{styleSelectable:function(M,N,O){if(N!==k){return;
}
if(O){bh.setDecorator(M,k);
}else{bh.setDecorator(M,null);
}bg.updateLayerData();
}});
bf.attachMouseEvents(bi.getPane());
bf.attachKeyEvents(bi);
bf.set({mode:p});
bf.addItem(0);
bi.setUserData(q,bf);
return bi;
},createCellRectangleSelectionScroller:function(){var bE=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bE.getPane().addLayer(new qx.ui.virtual.layer.Row(l,j));
var bD=new qx.ui.virtual.cell.Cell();
var bF=new qx.ui.virtual.cell.Cell().set({backgroundColor:I});
var bC=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bu,bv){var bw={};

if(bB.isItemSelected({row:bu,column:bv})){bw.selected=true;
}
if(bw.selected){return bF.getCellProperties(bu+o+bv,bw);
}else{bD.resetBackgroundColor();
return bD.getCellProperties(bu+o+bv,bw);
}}});
bE.getPane().addLayer(bC);
bE.getPane().addLayer(new qx.ui.virtual.layer.GridLines(i));
var bB=new qx.ui.virtual.selection.CellRectangle(bE.getPane(),{styleSelectable:function(bx,by,bz){bC.updateLayerData();
}});
bB.attachMouseEvents(bE.getPane());
bB.attachKeyEvents(bE);
bB.set({mode:p});
bB.addItem({row:0,column:0});
bE.setUserData(q,bB);
return bE;
},createCellLineSelectionScroller:function(){var bR=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bR.getPane().addLayer(new qx.ui.virtual.layer.Row(l,j));
var bQ=new qx.ui.virtual.cell.Cell();
var bS=new qx.ui.virtual.cell.Cell().set({backgroundColor:I});
var bP=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bG,bH){var bI={};

if(bO.isItemSelected({row:bG,column:bH})){bI.selected=true;
}
if(bI.selected){return bS.getCellProperties(bG+o+bH,bI);
}else{bQ.resetBackgroundColor();
return bQ.getCellProperties(bG+o+bH,bI);
}}});
bR.getPane().addLayer(bP);
bR.getPane().addLayer(new qx.ui.virtual.layer.GridLines(i));
var bO=new qx.ui.virtual.selection.CellLines(bR.getPane(),{styleSelectable:function(bk,bl,bm){bP.updateLayerData();
}});
bO.attachMouseEvents(bR.getPane());
bO.attachKeyEvents(bR);
bO.set({mode:p});
bO.addItem({row:0,column:0});
bR.setUserData(q,bO);
return bR;
}}});
})();
...
(function(){var y="Please use the value property instead.",x="changeChecked",w="checked",v="Please use the changeValue event instead.",u="keypress",t="Boolean",s="Right",r="_applyValue",q="changeValue",p="qx.ui.form.RadioButton",j="radiobutton",o="Left",m="qx.ui.form.RadioGroup",i="Down",h="_applyGroup",l="Up",k="execute",n="qx.event.type.Data";
qx.Class.define(p,{extend:qx.ui.form.Button,include:[qx.ui.form.MForm,qx.ui.form.MModelProperty],implement:[qx.ui.form.IRadioItem,qx.ui.form.IForm,qx.ui.form.IBooleanForm,qx.ui.form.IModel],construct:function(z){{};
arguments.callee.base.call(this,z);
this.addListener(k,this._onExecute);
this.addListener(u,this._onKeyPress);
},properties:{group:{check:m,nullable:true,apply:h},value:{check:t,nullable:true,event:q,apply:r,init:false},appearance:{refine:true,init:j},allowGrowX:{refine:true,init:false}},events:{"changeChecked":n},members:{_applyValue:function(B,C){B?this.addState(w):this.removeState(w);

if(B&&this.getFocusable()){this.focus();
}this.fireDataEvent(x,B,C);
},_applyGroup:function(f,g){if(g){g.remove(this);
}
if(f){f.add(this);
...
break;
case s:case i:A.selectNext();
break;
}},setChecked:function(d){qx.log.Logger.deprecatedMethodWarning(arguments.callee,y);
this.setValue(d);
},isChecked:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,y);
return this.getValue();
},toggleChecked:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,y);
this.setValue(!this.getValue());
},getChecked:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,y);
return this.getValue();
},resetChecked:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,y);
this.resetValue();
},addListener:function(a,b,self,c){if(a==x){qx.log.Logger.deprecatedEventWarning(arguments.callee,x,v);
...
(function(){var h="__nv",g="qx.ui.core.MSingleSelectionHandling",f="changeSelection",d="changeSelected",c="qx.event.type.Data";
qx.Mixin.define(g,{events:{"changeSelection":c},members:{__nv:null,getSelection:function(){var b=this.__nw().getSelected();

if(b){return [b];
}else{return [];
}},setSelection:function(k){switch(k.length){case 0:this.resetSelection();
break;
case 1:this.__nw().setSelected(k[0]);
break;
...
},isSelected:function(m){return this.__nw().isSelected(m);
},isSelectionEmpty:function(){return this.__nw().isSelectionEmpty();
},getSelectables:function(){return this.__nw().getSelectables();
},_onChangeSelected:function(e){var j=e.getData();
var i=e.getOldData();
j==null?j=[]:j=[j];
i==null?i=[]:i=[i];
this.fireDataEvent(f,j,i);
},__nw:function(){if(this.__nv==null){var l=this;
this.__nv=new qx.ui.core.SingleSelectionManager({getItems:function(){return l._getItems();
},isItemSelectable:function(a){if(l._isItemSelectable){return l._isItemSelectable(a);
}else{return a.isEnabled()&&a.isVisible();
}}});
this.__nv.addListener(d,this._onChangeSelected,this);
}this.__nv.setAllowEmptySelection(this._isAllowEmptySelection());
return this.__nv;
...
(function(){var E="Boolean",D="changeValue",C="changeSelected",B="qx.event.type.Data",A="Use 'setSelection' instead!",z="Use 'changeSelection' instead!",y="changeValid",x="changeRequired",w="changeEnabled",v="Please use getModelSelection instead.",R="Please use setModelSelection instead.",Q="qx.ui.form.RadioGroup",P="_applyValid",O="__nx",N="changeInvalidMessage",M="changeSelection",L="_applyEnabled",K="String",J="_applyAllowEmptySelection",I="",G="Use 'resetSelection' instead!",H="Use 'getSelection' instead!",F="_applyInvalidMessage";
qx.Class.define(Q,{extend:qx.core.Object,implement:[qx.ui.form.IFormElement,qx.ui.core.ISingleSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MSingleSelectionHandling,qx.ui.form.MFormElement,qx.ui.form.MModelSelection],construct:function(q){arguments.callee.base.call(this);
this.__nx=[];
this.addListener(M,this.__nz,this);

if(q!=null){this.add.apply(this,arguments);
}},properties:{enabled:{check:E,apply:L,event:w},wrap:{check:E,init:true},allowEmptySelection:{check:E,init:false,apply:J},valid:{check:E,init:true,apply:P,event:y},required:{check:E,init:false,event:x},invalidMessage:{check:K,init:I,event:N,apply:F}},events:{"changeValue":B,"changeSelected":B},members:{__nx:null,getItems:function(){return this.__nx;
},select:function(bm){qx.log.Logger.deprecatedMethodWarning(arguments.callee,A);
this.setSelection([bm]);
},setValue:function(r){qx.log.Logger.deprecatedMethodWarning(arguments.callee,R);
var s=this.__nx;
var t;

for(var i=0,l=s.length;i<l;i++){t=s[i];

if(t.getValue()==r){this.setSelection([t]);
break;
}}},getValue:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,v);
var u=this.getSelection()[0];
return u?u.getValue():null;
},add:function(j){var k=this.__nx;
var m;

for(var i=0,l=arguments.length;i<l;i++){m=arguments[i];

if(qx.lang.Array.contains(k,m)){continue;
}m.addListener(D,this._onItemChangeChecked,this);
k.push(m);
m.setGroup(this);
if(m.getValue()){this.setSelection([m]);
}}if(!this.isAllowEmptySelection()&&k.length>0&&!this.getSelection()[0]){this.setSelection([k[0]]);
}},remove:function(V){var W=this.__nx;

if(qx.lang.Array.contains(W,V)){qx.lang.Array.remove(W,V);
if(V.getGroup()===this){V.resetGroup();
}V.removeListener(D,this._onItemChangeChecked,this);
if(V.getValue()){this.resetSelection();
}}},getChildren:function(){return this.__nx;
},_onItemChangeChecked:function(e){var bl=e.getTarget();

if(bl.getValue()){this.setSelection([bl]);
}else if(this.getSelection()[0]==bl){this.resetSelection();
}},setSelected:function(h){qx.log.Logger.deprecatedMethodWarning(arguments.callee,A);
this.setSelection([h]);
},getSelected:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,H);
var d=this.getSelection()[0];

if(d){return d;
}else{return null;
}},resetSelected:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,G);
this.resetSelection();
},_applyInvalidMessage:function(f,g){for(var i=0;i<this.__nx.length;i++){this.__nx[i].setInvalidMessage(f);
}},_applyValid:function(bj,bk){for(var i=0;i<this.__nx.length;i++){this.__nx[i].setValid(bj);
}},_applyEnabled:function(n,o){var p=this.__nx;

if(n==null){for(var i=0,l=p.length;i<l;i++){p[i].resetEnabled();
}}else{for(var i=0,l=p.length;i<l;i++){p[i].setEnabled(n);
}}},_applyAllowEmptySelection:function(bh,bi){if(!bh&&this.isSelectionEmpty()){this.resetSelection();
}},__ny:function(bf){var bg=null;

if(bf){bg=bf.getValue();

if(bg==null){bg=bf.getLabel();
}}return bg;
...
while(i<length&&!ba[Y].getEnabled()){Y=(Y+1)%length;
i++;
}this.setSelection([ba[Y]]);
...
},__nz:function(e){var bd=e.getData()[0];
var bb=e.getOldData()[0];

if(bb){bb.setValue(false);
}
if(bd){bd.setValue(true);
}var bc=this.__ny(bb);
var be=this.__ny(bd);
this.fireDataEvent(D,be,bc);
if(this.hasListener(C)){this.fireDataEvent(C,bd,bb);
}},addListener:function(S,T,self,U){if(S===C){qx.log.Logger.deprecatedEventWarning(arguments.callee,C,z);
}
if(S===D){qx.log.Logger.deprecatedEventWarning(arguments.callee,D,z);
}return arguments.callee.base.call(this,S,T,self,U);
}},destruct:function(){this._disposeArray(O);
}});
})();
(function(){var m="__nA",l="Boolean",k="qx.ui.core.SingleSelectionManager",j="__nB",h="changeSelected",g="__nC",f="qx.event.type.Data";
qx.Class.define(k,{extend:qx.core.Object,construct:function(n){arguments.callee.base.call(this);
{};
this.__nA=n;
},events:{"changeSelected":f},properties:{allowEmptySelection:{check:l,init:true,apply:g}},members:{__nB:null,__nA:null,getSelected:function(){return this.__nB;
},setSelected:function(e){if(!this.__nE(e)){throw new Error("Could not select "+e+", because it is not a child element!");
}this.__nD(e);
},resetSelected:function(){this.__nD(null);
},isSelected:function(q){if(!this.__nE(q)){throw new Error("Could not check if "+q+" is selected,"+" because it is not a child element!");
}return this.__nB===q;
},isSelectionEmpty:function(){return this.__nB==null;
},getSelectables:function(){var o=this.__nA.getItems();
var p=[];

for(var i=0;i<o.length;i++){if(this.__nA.isItemSelectable(o[i])){p.push(o[i]);
}}return p;
},__nC:function(c,d){if(!c){this.__nD(this.__nB);
}},__nD:function(r){var u=this.__nB;
var t=r;
...
this.fireDataEvent(h,t,u);
},__nE:function(a){var b=this.__nA.getItems();

for(var i=0;i<b.length;i++){if(b[i]===a){return true;
}}return false;
}},destruct:function(){if(this.__nA.toHashCode){this._disposeObjects(m);
...
qx.Class.define(s,{extend:qx.ui.basic.Atom,include:[qx.ui.form.MFormElement,qx.ui.core.MExecutable],implement:[qx.ui.form.IFormElement,qx.ui.form.IBooleanForm,qx.ui.form.IExecutable],construct:function(D,E){arguments.callee.base.call(this,D,E);
...
},events:{"changeChecked":u},properties:{appearance:{refine:true,init:v},focusable:{refine:true,init:true},value:{check:r,nullable:true,event:x,apply:t,init:false}},members:{_applyValue:function(a,b){if(qx.lang.Type.isString(a)){qx.log.Logger.deprecatedMethodWarning(arguments.callee,o);
return;
}a?this.addState(f):this.removeState(f);
this.fireDataEvent(j,a,b);
...
this.removeState(m);
e.stopPropagation();
}},setChecked:function(C){qx.log.Logger.deprecatedMethodWarning(arguments.callee,k);
this.setValue(C);
...
this.resetValue();
},addListener:function(z,A,self,B){if(z==j){qx.log.Logger.deprecatedEventWarning(arguments.callee,j,c);
...
(function(){var u="px;",t="<div style='",s="</div>",r="position: absolute;",q="top: 0;",p="'>",o="left:",n="qx.ui.virtual.layer.Column",m="block",k="",g="height:",j="column-layer",h="background-color:",f="width:",e="none";
qx.Class.define(n,{extend:qx.ui.virtual.layer.AbstractBackground,properties:{appearance:{refine:true,init:j}},members:{_fullUpdate:function(A,B,C,D){var K=[];
...
K.push(t,r,q,o,N,u,p,M.getMarkup(),s);
G++;
}else{var J=this.getColor(H);

if(J){K.push(t,r,q,o,N,u,f,D[x],u,g,I,u,h,J,p,s);
G++;
}}N+=D[x];
H+=1;
}var E=this.getContentElement().getDomElement();
E.style.display=e;
E.innerHTML=K.join(k);
...
}},setColor:function(a,b){arguments.callee.base.call(this,a,b);
var d=this.getFirstColumn();
var c=d+this.getColumnSizes().length-1;

if(a>=d&&a<=c){this.updateLayerData();
}}}});
})();
(function(){var d="right",c="qx.ui.virtual.selection.Column",b="left";
qx.Class.define(c,{extend:qx.ui.virtual.selection.Row,members:{_getItemCount:function(){return this._pane.getColumnConfig().getItemCount();
},_getSelectableFromMouseEvent:function(event){var a=this._pane.getCellAtPosition(event.getDocumentLeft(),event.getDocumentTop());

if(!a){return null;
}return this._isSelectable(a.column)?a.column:null;
},_getRelatedSelectable:function(e,f){if(f==b){var h=e-1;
var g=0;
var j=-1;
}else if(f==d){var h=e+1;
var g=this._getItemCount()-1;
var j=1;
}else{return null;
}
...
},_getSelectableLocationX:function(m){var n=this._pane.getColumnConfig();
var p=n.getItemPosition(m);
var o=p+n.getItemSize(m)-1;
return {left:p,right:o};
},_getSelectableLocationY:function(l){return {top:0,bottom:this._pane.getRowConfig().getTotalSize()-1};
}}});
})();
(function(){var s="under",r="above",q="left",p="right",o="x",n="qx.ui.virtual.selection.CellRectangle";
qx.Class.define(n,{extend:qx.ui.virtual.selection.Abstract,members:{_getItemCount:function(){return this._pane.getRowConfig().getItemCount()*this._pane.getColumnConfig().getItemCount();
},_getSelectableFromMouseEvent:function(event){var m=this._pane.getCellAtPosition(event.getDocumentLeft(),event.getDocumentTop());

if(!m){return null;
}return this._isSelectable(m)?m:null;
},getSelectables:function(){var B=[];
var E=this._pane.getRowConfig().getItemCount();
var A=this._pane.getColumnConfig().getItemCount();

for(var F=0;F<E;F++){for(var C=0;C<A;C++){var D={row:F,column:C};

if(this._isSelectable(D)){B.push(D);
}}}return B;
},_getSelectableRange:function(Q,R){var U=[];
var V=Math.min(Q.row,R.row);
var W=Math.max(Q.row,R.row);
var T=Math.min(Q.column,R.column);
var Y=Math.max(Q.column,R.column);

for(var ba=V;ba<=W;ba++){for(var X=T;X<=Y;X++){var S={row:ba,column:X};

if(this._isSelectable(S)){U.push(S);
}}}return U;
},_getFirstSelectable:function(){var i=this._pane.getRowConfig().getItemCount();
var h=this._pane.getColumnConfig().getItemCount();

for(var l=0;l<i;l++){for(var k=0;k<h;k++){var j={row:l,column:k};

if(this._isSelectable(j)){return j;
}}}return null;
},_getLastSelectable:function(){var M=this._pane.getRowConfig().getItemCount();
var L=this._pane.getColumnConfig().getItemCount();

for(var O=L-1;O>=0;O--){for(var P=M-1;P>=0;P--){var N={row:P,column:O};

if(this._isSelectable(N)){return N;
}}}return null;
},_getRelatedSelectable:function(a,b){var c={row:a.row,column:a.column};

switch(b){case r:for(var g=a.row-1;g>=0;g--){c.row=g;

if(this._isSelectable(c)){return c;
}}break;
case s:var d=this._pane.getRowConfig().getItemCount();

for(var g=a.row+1;g<d;g++){c.row=g;

if(this._isSelectable(c)){return c;
}}break;
case q:for(var f=a.column-1;f>=0;f--){c.column=f;

if(this._isSelectable(c)){return c;
}}break;
case p:var e=this._pane.getColumnConfig().getItemCount();

for(var f=a.column+1;f<e;f++){c.column=f;

if(this._isSelectable(c)){return c;
}}break;
}return null;
},_getPage:function(t,u){if(u){return this._getFirstSelectable();
}else{return this._getLastSelectable();
}},_selectableToHashCode:function(G){return G.column+o+G.row;
},_scrollItemIntoView:function(v){this._pane.scrollCellIntoView(v.column,v.row);
},_getSelectableLocationX:function(w){var x=this._pane.getColumnConfig();
var z=x.getItemPosition(w.column);
var y=z+x.getItemSize(w.column)-1;
return {left:z,right:y};
},_getSelectableLocationY:function(H){var K=this._pane.getRowConfig();
var J=K.getItemPosition(H.row);
var I=J+K.getItemSize(H.row)-1;
return {top:J,bottom:I};
}}});
})();
