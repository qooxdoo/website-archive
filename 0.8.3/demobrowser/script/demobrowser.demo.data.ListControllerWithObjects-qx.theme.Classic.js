...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.data.ListControllerWithObjects-qx.theme.Classic-0.js"]],
  boot : "boot",
...
var Y=[y,z,o,f,n,w,l,a];
var P=[D,d,F,G,E,v];
var V=[];

for(var i=0;i<20;i++){var S=new demobrowser.demo.data.model.Person();
S.setName(Y[i%Y.length]);
S.setEmote(P[i%P.length]);
S.setOnline(i%3==0);
V.push(S);
}var W=new qx.data.Array(V);
var O=new qx.ui.form.List();
O.setWidth(150);
this.getRoot().add(O,{left:10,top:80});
var X={converter:function(K){return u+K+e;
}};
var T=new qx.data.controller.List(W,O,h,j,null,X);
var R={converter:function(J){if(J){return b;
}else{return x;
}}};
var M={bindItem:function(bb,bc,bd){bb.bindProperty(h,t,null,bc,bd);
bb.bindProperty(j,r,X,bc,bd);
bb.bindProperty(C,s,R,bc,bd);
}};
T.setDelegate(M);
var N=new qx.ui.form.Button(H);
N.setWidth(120);
this.getRoot().add(N,{left:180,top:80});
N.addListener(k,function(){for(var i=0;i<W.length;i++){var ba=W.getItem(i);
ba.setEmote(g);
}},this);
var L=new qx.ui.form.Button(m);
L.setWidth(120);
this.getRoot().add(L,{left:180,top:115});
L.addListener(k,function(){for(var i=0;i<W.length;i++){W.getItem(i).toggleOnline();
}},this);
var U=new qx.ui.form.Button(A);
U.setWidth(120);
this.getRoot().add(U,{left:180,top:150});
U.addListener(k,function(){qx.log.appender.Console.show();
this.info(W.toString());
},this);
