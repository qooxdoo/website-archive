...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.ui.FormValidator", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.ui.FormValidator-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var r="password",q="Send",p="All fields are required. Some by a custom validator, some by a ",o="complete",n="Please enter a password at with least 3 characters.",m="username",l="Server said no!",k="Passwords must be equal.",j="<b>Client side form validation</b><br/>",i="Validating...",f="email address",h="demobrowser.demo.ui.FormValidator",g="Accept",e="execute",d="predefined validator and some by the required flag.";
...
s.setWidth(150);
s.setRequired(true);
this.getRoot().add(s,{left:20,top:70});
...
t.setPlaceholder(r);
t.setWidth(150);
this.getRoot().add(t,{left:20,top:130});
var u=new qx.ui.form.TextField();
u.setPlaceholder(r);
u.setWidth(150);
this.getRoot().add(u,{left:20,top:160});
var A=new qx.ui.form.CheckBox(g);
A.setRequired(true);
this.getRoot().add(A,{left:20,top:190});
var x=new qx.ui.form.validation.Manager();
var v=function(a,b){var c=a!=null&&a.length>2;

if(!c){b.setInvalidMessage(n);
}return c;
};
var y=new qx.ui.form.validation.AsyncValidator(function(C,D){window.setTimeout(function(){if(D==null||D.length==0){C.setValid(false,l);
}else{C.setValid(true);
}},1000);
});
x.add(s,y);
x.add(B,qx.util.Validate.email());
x.add(t,v);
x.add(u,v);
x.add(A);
x.setValidator(function(E){var G=t.getValue()==u.getValue();

if(!G){var F=k;
t.setInvalidMessage(F);
u.setInvalidMessage(F);
t.setValid(false);
u.setValid(false);
}return G;
});
var w=new qx.ui.form.Button(q);
this.getRoot().add(w,{left:20,top:215});
w.addListener(e,function(){w.setEnabled(false);
w.setLabel(i);
x.validate();
},this);
x.addListener(o,function(){w.setEnabled(true);
w.setLabel(q);
if(x.getValid()){alert("You can send...");
}else{alert(x.getInvalidMessages().join("\n"));
}},this);
var z=new qx.ui.basic.Label();
z.setRich(true);
z.setWidth(320);
z.setValue(j+p+d);
this.getRoot().add(z,{left:20,top:10});
}}});
})();
...
(function(){var m="",l="complete",k="changeValid",j="qx.event.type.Event",h="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",g="qx.ui.form.validation.Manager",f="This field is required",d="String",c="qx.event.type.Data";
qx.Class.define(g,{extend:qx.core.Object,construct:function(){arguments.callee.base.call(this);
this.__mr=[];
this.__ms={};
},events:{"changeValid":c,"complete":j},properties:{validator:{check:h,init:null,nullable:true},invalidMessage:{check:d,init:m}},members:{__mr:null,__mt:null,__ms:null,__mu:null,add:function(C,D){if(!this.__mz(C)){throw new Error("Added widget not supported.");
}if(this.__mA(C)){if(D!=null){throw new Error("Widgets suporting selection can only be validated "+"in the form validator");
}}var E={item:C,validator:D,valid:null};
this.__mr.push(E);
},validate:function(){var u=true;
this.__mu=true;
var r=[];
for(var i=0;i<this.__mr.length;i++){var s=this.__mr[i].item;
var v=this.__mr[i].validator;
r.push(s);
if(v==null){var q=this.__mv(s);
u=u&&q;
this.__mu=q&&this.__mu;
continue;
}var q=this.__mw(this.__mr[i],s.getValue());
u=q&&u;

if(q!=null){this.__mu=q&&this.__mu;
}}var t=this.__mx(r);

if(qx.lang.Type.isBoolean(t)){this.__mu=t&&this.__mu;
}u=t&&u;
this.__mC(u);

if(qx.lang.Object.isEmpty(this.__ms)){this.fireEvent(l);
}return u;
},__mv:function(L){if(L.getRequired()){var M=!!L.getValue();
L.setValid(M);
L.setInvalidMessage(f);
return M;
}return true;
},__mw:function(R,S){var W=R.item;
var V=R.validator;
if(this.__my(V)){this.__ms[W.toHashCode()]=null;
...
R.valid=U;
return U;
},__mx:function(y){var A=this.getValidator();

if(A==null){return true;
}this.setInvalidMessage(m);

if(this.__my(A)){this.__ms[this.toHashCode()]=null;
A.validateForm(y,this);
return null;
}
try{var B=A(y,this);
...
if(e.message&&e.message!=qx.type.BaseError.DEFAULTMESSAGE){var z=e.message;
}else{var z=e.getComment();
}this.setInvalidMessage(z);
}else{throw e;
}}return B;
},__my:function(w){var x=false;

if(!qx.lang.Type.isFunction(w)){x=qx.Class.isSubClassOf(w.constructor,qx.ui.form.validation.AsyncValidator);
}return x;
},__mz:function(P){var Q=P.constructor;
return qx.Class.hasInterface(Q,qx.ui.form.IForm);
},__mA:function(F){var G=F.constructor;
return qx.Class.hasInterface(G,qx.ui.core.ISingleSelection);
},__mB:function(o){var p=o.constructor;
...
},__mC:function(X){var Y=this.__mt;
this.__mt=X;
if(Y!=X){this.fireDataEvent(k,X,Y);
}},getValid:function(){return this.__mt;
},isValid:function(){return this.getValid();
},getInvalidMessages:function(){var a=[];
for(var i=0;i<this.__mr.length;i++){var b=this.__mr[i].item;

if(!b.getValid()){a.push(b.getInvalidMessage());
}}if(this.getInvalidMessage()!=m){a.push(this.getInvalidMessage());
}return a;
},reset:function(){for(var i=0;i<this.__mr.length;i++){var n=this.__mr[i];
n.item.setValid(true);
}this.__mt=null;
},setItemValid:function(N,O){this.__ms[N.toHashCode()]=O;
N.setValid(O);
this.__mD();
},setFormValid:function(K){this.__ms[this.toHashCode()]=K;
this.__mD();
},__mD:function(){var I=this.__mu;
for(var J in this.__ms){var H=this.__ms[J];
I=H&&I;
if(H==null){return;
}}this.__mC(I);
this.__ms={};
...
(function(){var c="__mG",b="__mF",a="qx.ui.form.validation.AsyncValidator";
qx.Class.define(a,{extend:qx.core.Object,construct:function(k){arguments.callee.base.call(this);
this.__mE=k;
},members:{__mE:null,__mF:null,__mG:null,__mH:null,validate:function(f,g,h){this.__mH=false;
this.__mF=f;
this.__mG=h;
this.__mE(this,g);
},validateForm:function(i,j){this.__mH=true;
this.__mG=j;
this.__mE(i,this);
},setValid:function(d,e){if(this.__mH){if(e!==undefined){this.__mG.setInvalidMessage(e);
}this.__mG.setFormValid(d);
}else{if(e!==undefined){this.__mF.setInvalidMessage(e);
}this.__mG.setItemValid(this.__mF,d);
...
(function(){var s="Validation Error",r="",q="'",p=" is not a color! ",o=" is not an url.",n="number",m="Validation Error: ",l=" is not in ",k="qx.util.Validate",j="string",c=" is not a string.",i=" does not fit ",g=" is not in the range from [",b=" is not a number.",a=".",f="].",d=", ",h="' is not an email address.";
qx.Class.define(k,{statics:{number:function(){return qx.util.Validate.checkNumber;
},checkNumber:function(t){if((typeof t!==n&&(!(t instanceof Number)))||(!(isFinite(t)))){throw new qx.core.ValidationError(m,t+b);
}},email:function(){return qx.util.Validate.checkEmail;
},checkEmail:function(z){var A=/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;

if(A.test(z)===false){throw new qx.core.ValidationError(s,q+(z||r)+h);
}},string:function(){return qx.util.Validate.checkString;
},checkString:function(w){if(typeof w!==j&&(!(w instanceof String))){throw new qx.core.ValidationError(s,w+c);
}},url:function(){return qx.util.Validate.checkUrl;
},checkUrl:function(D){var E=/([A-Za-z0-9])+:\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;

if(E.test(D)===false){throw new qx.core.ValidationError(s,D+o);
}},color:function(){return qx.util.Validate.checkColor;
},checkColor:function(F){try{qx.util.ColorUtil.stringToRgb(F);
}catch(e){throw new qx.core.ValidationError(s,F+p+e);
}},range:function(u,v){return function(C){if(C<u||C>v){throw new qx.core.ValidationError(s,C+g+u+d+v+f);
}};
},inArray:function(B){return function(y){if(B.indexOf(y)===-1){throw new qx.core.ValidationError(s,y+l+B+r);
}};
},regExp:function(x){return function(G){if(x.test(G)===false){throw new qx.core.ValidationError(s,G+i+x+a);
}};
}}});
})();
