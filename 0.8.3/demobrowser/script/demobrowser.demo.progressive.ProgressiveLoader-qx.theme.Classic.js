...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.progressive.ProgressiveLoader", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.progressive.ProgressiveLoader-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var t="white",s="100%",r="func",q="demobrowser.demo.progressive.ProgressiveLoader",p="black",n="Red",m="http://resources.qooxdoo.org/images/logo.gif",l="green",k="yellow",j="col ",F="gray",E="blue",D="Green",C="Yellow",B="Blue",A="renderStart",z=", ",y="purple",x="red",w="renderEnd",u="row ",v="Purple";
...
R.setWidth(0,s);
var K=new qx.ui.progressive.headfoot.Progress(R);
var P=new qx.ui.progressive.structure.Default(null,K);
var J=new qx.ui.progressive.Progressive(P);
J.setFlushWidgetQueueAfterBatch(true);
var O=function(h){var i={renderer:r,data:h};
return i;
};
...
J.addRenderer(r,S);
var Q=m;
var T=new qx.ui.basic.Image(Q,s,s);
J.add(T);
J.set({height:100,width:272,zIndex:99999,backgroundColor:F,opacity:0.86,batchSize:10});
this.getRoot().add(J,{top:-1000,left:-1000});
var U=[{text:n,background:x,color:t},{text:D,background:l,color:t},{text:B,background:E,color:t},{text:v,background:y,color:t},{text:C,background:k,color:p}];
this.context={document:this.getRoot(),pages:U};
J.addListener(A,function(e){var f=e.getData().state;
var a=e.getData().initial;
var d=this.getRoot().getBounds();
var b=e.getData().state.getProgressive();
var c=b.getBounds();
var g=Math.floor((d.width-c.width)/2);
var top=Math.floor((d.height-c.height)/2);
b.setLayoutProperties({left:g<0?0:g,top:top<0?0:top});
f.getUserData().context=this.context;
f.getUserData().initialNum=a;
},this);
J.addListener(w,function(e){qx.event.Timer.once(function(){this.getLayoutParent().remove(this);
this.dispose();
},this,0);
});
...
for(var M=0;M<U.length;M++){V.addElement(O(function(bc){var be=bc.context;
var bd=be.pages[be.page];
be.currentPage=new qx.ui.tabview.Page(bd.text);
be.currentPage.setLayout(new qx.ui.layout.VBox(4));
be.tabview.add(be.currentPage);
be.row=0;
be.col=0;
if(be.page==0){be.firstPage=be.currentPage;
}}));
for(var W=0;W<20;W++){V.addElement(O(function(Y){var ba=Y.context;
...
for(var L=0;L<10;L++){V.addElement(O(function(bf){var bh=bf.context;
var bg=bh.pages[bh.page];
var o=new qx.ui.basic.Atom(u+bh.row+z+j+bh.col);
o.set({width:90,backgroundColor:bg.background,textColor:bg.color});
bh.hBox.add(o);
}));
V.addElement(O(function(bb){++bb.context.col;
}));
}V.addElement(O(function(bi){++bi.context.row;
}));
}V.addElement(O(function(G){++G.context.page;
}));
}V.addElement(O(function(X){X.context.tabview.setSelection([X.context.firstPage]);
}));
J.setDataModel(V);
J.render();
}}});
