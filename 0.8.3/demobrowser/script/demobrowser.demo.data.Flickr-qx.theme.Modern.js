...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.data.Flickr", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.data.Flickr-qx.theme.Modern-0.js"]],
  boot : "boot",
...
(function(){var o="qooxdoo",n="/",m=".static.flickr.com/",l="http://api.flickr.com/services/rest/?tags=",k="",j="_",i="http://farm",h="Enter",g="keydown",f="model.photos.photo",H=".jpg",G="execute",F="source",E="selection[0]",D="_s.jpg",C="<span style='font-size: 20px'>Flickr</span>",B="loading",A="value",z="demobrowser.demo.data.store.Flickr",y="demobrowser.demo.data.Flickr",v="title",w="icon",t="state",u="Search",r="receiving",s="horizontal",p="model",q="&method=flickr.photos.search&api_key=63a8042eead205f7e0040f488c02afd9&format=json",x="id";
qx.Class.define(y,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var X=new demobrowser.demo.data.store.Flickr(o);
var P=new qx.ui.form.TextField(o);
this.getRoot().add(P,{left:30,top:50});
var Q=new qx.ui.form.Button(u);
this.getRoot().add(Q,{left:140,top:49});
Q.addListener(G,function(){X.searchForTag(P.getValue());
},this);
P.addListener(g,function(e){if(e.getKeyIdentifier()==h){X.searchForTag(P.getValue());
}},this);
...
T.setHeight(110);
T.setOrientation(s);
this.getRoot().add(T,{left:30,right:30,top:80});
var V=new qx.data.controller.List(null,T);
V.setLabelPath(v);
V.setDelegate({configureItem:function(K){K.setShow(w);
}});
var R={converter:function(a,b){return (i+b.getFarm()+m+b.getServer()+n+a+j+b.getSecret()+D);
}};
V.setIconOptions(R);
V.setIconPath(x);
X.bind(f,V,p);
var S=new qx.ui.basic.Image();
this.getRoot().add(S,{left:30,top:200});
var W={converter:function(c){if(c){return (i+c.getFarm()+m+c.getServer()+n+c.getId()+j+c.getSecret()+H);
}return k;
}};
V.bind(E,S,F,W);
...
qx.Class.define(z,{extend:qx.data.store.Json,statics:{RESULT:null},construct:function(M){jsonFlickrApi=function(L){demobrowser.demo.data.store.Flickr.RESULT=L;
};
var N=l+M;
arguments.callee.base.call(this,N);
},members:{searchForTag:function(Y){if(Y!=k){this.setUrl(l+Y);
}},_createRequest:function(I){var J=new qx.io2.ScriptLoader();
this.setState(r);
I+=q;
J.load(I,function(d){this.__AQ();
},this);
},__AQ:function(){this.setState("completed");
var O=demobrowser.demo.data.store.Flickr.RESULT;

if(O==undefined){this.setState("failed");
return;
