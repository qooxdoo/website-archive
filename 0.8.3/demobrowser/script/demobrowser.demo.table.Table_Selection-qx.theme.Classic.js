...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.table.Table_Selection", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.table.Table_Selection-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var s="execute",r="",q="value",p="A date",o="Boolean",n="demobrowser.demo.table.Table_Selection",m="changeValue",l="icon/22/status/dialog-information.png",k="A number",j="icon/16/apps/office-calendar.png",H="Reset Cell Focus",G="changeSelection",F="middle",E="Remove 5 rows",D="Clear Selection",C="_tableModel",B="Table",A="Add 5 rows",z="icon/22/actions/list-remove.png",y=", ",w="icon/22/actions/list-add.png",x="Hide Cell Focus",u="Selected rows:<br>",v="Show selection",t="ID";
qx.Class.define(n,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return B;
},createTable:function(){var c=this.createRandomRows(50);
var b=this._tableModel=new qx.ui.table.model.Simple();
b.setColumns([t,k,p,o]);
b.setData(c);
b.setColumnEditable(1,true);
b.setColumnEditable(2,true);
b.setColumnSortable(3,false);
var d=new qx.ui.table.Table(b);
d.set({width:600,height:400});
d.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var a=d.getTableColumnModel();
a.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
a.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(j,p));
return d;
},createControls:function(){var L=new qx.ui.toolbar.ToolBar();
var N,P;
P=new qx.ui.toolbar.Part();
L.add(P);
var K=new qx.ui.form.SelectBox().set({allowGrowY:false,alignY:F});
var S=qx.ui.table.selection.Model;
var M={"Single selection":S.SINGLE_SELECTION,"Single interval":S.SINGLE_INTERVAL_SELECTION,"Multiple intervals":S.MULTIPLE_INTERVAL_SELECTION,"Multiple intervals (toggle)":S.MULTIPLE_INTERVAL_SELECTION_TOGGLE,"No selection":S.NO_SELECTION};

for(var R in M){var O=new qx.ui.form.ListItem(R);
O.setUserData(q,M[R]+r);
K.add(O);
}K.addListener(G,function(e){this._table.getSelectionModel().setSelectionMode(parseInt(e.getData()[0].getUserData(q)));
},this);
P.add(K);
P=new qx.ui.toolbar.Part();
L.add(P);
N=new qx.ui.toolbar.Button(A,w);
N.addListener(s,function(f){var g=this.createRandomRows(10);
this._tableModel.addRows(g);
this.info("5 rows added");
},this);
P.add(N);
N=new qx.ui.toolbar.Button(E,z);
N.addListener(s,function(h){var i=this._tableModel.getRowCount();
this._tableModel.removeRows(i-5,5);
this.info("5 rows removed");
},this);
P.add(N);
P=new qx.ui.toolbar.Part();
L.add(P);
N=new qx.ui.toolbar.Button(D);
N.addListener(s,function(T){this._table.clearSelection();
},this);
P.add(N);
N=new qx.ui.toolbar.Button(H);
N.addListener(s,function(I){this._table.resetCellFocus();
},this);
P.add(N);
var Q=new qx.ui.toolbar.CheckBox(x);
Q.addListener(m,function(J){this._table.setShowCellFocusIndicator(!Q.isValue());
},this);
P.add(Q);
N=new qx.ui.toolbar.Button(v,l);
N.addListener(s,function(V){var W=[];
this._table.getSelectionModel().iterateSelection(function(U){W.push(U+r);
});
this.showDialog(u+W.join(y));
},this);
P.add(N);
return L;
...
C.addListener(z,this._onMousewheel,this,true);
this.addListener(y,this._onResize,this);
},properties:{focusable:{refine:true,init:true},width:{refine:true,init:120},maxListHeight:{check:t,apply:r,nullable:true,init:200}},events:{"changeValue":q},members:{_createChildControlImpl:function(d){var f;

switch(d){case A:f=new qx.ui.form.List().set({focusable:false,keepFocus:true,height:null,width:null,maxHeight:this.getMaxListHeight(),selectionMode:n,quickSelection:true});
f.addListener(s,this._onListChangeSelection,this);
f.addListener(i,this._onListMouseDown,this);
break;
case B:f=new qx.ui.popup.Popup(new qx.ui.layout.VBox);
f.setAutoHide(false);
f.setKeepActive(true);
f.addListener(k,this.close,this);
f.add(this.getChildControl(A));
f.addListener(j,this._onPopupChangeVisibility,this);
break;
}return f||arguments.callee.base.call(this,d);
},_applyMaxListHeight:function(g,h){this.getChildControl(A).setMaxHeight(g);
},getChildrenContainer:function(){return this.getChildControl(A);
},open:function(){var G=this.getChildControl(B);
G.placeToWidget(this,true);
G.show();
},close:function(){this.getChildControl(B).hide();
},toggle:function(){var H=this.getChildControl(B).isVisible();
...
if(qx.ui.core.Widget.contains(E,F)){return;
}this.close();
e.preventDefault();
...
if(c){c.removeListener(z,this._onMousewheel,this,true);
}}});
})();
...
(function(){var s="list",r="changeSelected",q="pressed",p="abandoned",o="popup",n="atom",m="hovered",l="changeValue",k="arrow",j="",Q="spacer",P="Enter",O="one",N="__oH",M="Please use getModelSelection instead.",L="Space",K="key",J="qx.event.type.Data",I="Please use setModelSelection instead.",H="mousewheel",z="keyinput",A="Use 'setSelection' instead!",x="changeSelection",y="Use 'changeSelection' instead!",v="quick",w="qx.ui.form.SelectBox",t="mouseover",u="selectbox",B="Please use resetSelection instead.",C="mouseout",E="click",D="Use 'resetSelection' instead!",G="Use 'getSelection' instead!",F=" ";
qx.Class.define(w,{extend:qx.ui.form.AbstractSelectBox,implement:[qx.ui.form.IFormElement,qx.ui.core.ISingleSelection,qx.ui.form.IModelSelection],include:[qx.ui.core.MSingleSelectionHandling,qx.ui.form.MModelSelection],construct:function(){arguments.callee.base.call(this);
this._createChildControl(n);
this._createChildControl(Q);
this._createChildControl(k);
this.addListener(t,this._onMouseOver,this);
this.addListener(C,this._onMouseOut,this);
this.addListener(E,this._onClick,this);
this.addListener(H,this._onMouseWheel,this);
this.addListener(z,this._onKeyInput,this);
this.addListener(x,this.__oI,this);
},events:{"changeSelected":J},properties:{appearance:{refine:true,init:u}},members:{__oH:null,_createChildControlImpl:function(be){var bf;

switch(be){case Q:bf=new qx.ui.core.Spacer();
this._add(bf,{flex:1});
break;
case n:bf=new qx.ui.basic.Atom(F);
bf.setCenter(false);
bf.setAnonymous(true);
this._add(bf,{flex:1});
break;
case k:bf=new qx.ui.basic.Image();
bf.setAnonymous(true);
this._add(bf);
break;
}return bf||arguments.callee.base.call(this,be);
},_forwardStates:{focused:true},setSelected:function(R){qx.log.Logger.deprecatedMethodWarning(arguments.callee,A);
this.setSelection([R]);
...
}},resetSelected:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,D);
this.resetSelection();
},setValue:function(a){qx.log.Logger.deprecatedMethodWarning(arguments.callee,I);
this.getChildControl(s).setValue(a);
},getValue:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,M);
var bh=this.getSelection()[0];
return bh?bh.getFormValue():null;
},resetValue:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,B);
var T=this.getChildControl(s);
var U=T.getChildren();

if(U[0]){T.setValue(U[0].getFormValue()||null);
}},_getItems:function(){return this.getChildrenContainer().getChildren();
},_isAllowEmptySelection:function(){return !this.getChildrenContainer().getSelectionMode()===O;
},__oI:function(e){var Y=e.getData()[0];
var W=this.getChildControl(s);

if(W.getSelection()[0]!=Y){W.setSelection([Y]);
}var bb=this.getChildControl(n);
var X=Y?Y.getLabel():j;
if(X&&X.translate){X=X.translate();
}X==null?bb.resetLabel():bb.setLabel(X);
var bd=Y?Y.getIcon():j;
bd==null?bb.resetIcon():bb.setIcon(bd);
if(this.hasListener(l)){this.fireDataEvent(l,W.getValue());
}if(this.hasListener(r)){var bc=e.getData()[0];
var ba=e.getOldData()[0];
this.fireDataEvent(r,bc,ba);
}},_onMouseOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;
}
...
}var bl=e.getWheelDelta()>0?1:-1;
var bn=this.getSelectables();
var bm=this.getSelection()[0];

if(!bm){bm=bn[0];
}var bk=bn.indexOf(bm)+bl;
var bo=bn.length-1;
if(bk<0){bk=0;
}else if(bk>=bo){bk=bo;
}this.setSelection([bn[bk]]);
e.stopPropagation();
e.preventDefault();
},_onKeyPress:function(e){var S=e.getKeyIdentifier();

if(S==P||S==L){if(this.__oH){this.setSelection([this.__oH]);
this.__oH=null;
}this.toggle();
}else{arguments.callee.base.call(this,e);
}},_onKeyInput:function(e){var V=e.clone();
V.setTarget(this._list);
V.setBubbles(false);
this.getChildControl(s).dispatchEvent(V);
},_onListMouseDown:function(e){if(this.__oH){this.setSelection([this.__oH]);
this.__oH=null;
}},_onListChangeSelection:function(e){var b=e.getData();

if(b.length>0){var c=this.getChildControl(o);
var d=this.getChildControl(s);
var f=d.getSelectionContext();

if(c.isVisible()&&(f==v||f==K)){this.__oH=b[0];
}else{this.setSelection([b[0]]);
this.__oH=null;
}}else{this.resetSelection();
}},_onPopupChangeVisibility:function(e){var bi=this.getChildControl(o);

if(!bi.isVisible()){var bj=this.getChildControl(s);
if(bj.hasChildren()){bj.setSelection(this.getSelection());
}}},addListener:function(g,h,self,i){if(g===r){qx.log.Logger.deprecatedEventWarning(arguments.callee,r,y);
...
(function(){var q="Use 'setSelection' instead!",p="Boolean",o="changeSelection",n="single",m="Use 'getSelection' instead!",l="mousedown",k="one",j="qx.event.type.Data",h="_applyDragSelection",g="__mO",D="mousemove",C="addItem",B="multi",A="_applyQuickSelection",z="mouseover",y="keypress",x="_applySelectionMode",w="additive",v="mouseup",u="Use 'resetSelection' instead!",s="losecapture",t="removeItem",r="qx.ui.core.MMultiSelectionHandling";
qx.Mixin.define(r,{construct:function(){var M=this.SELECTION_MANAGER;
var L=this.__mO=new M(this);
this.addListener(l,L.handleMouseDown,L);
this.addListener(v,L.handleMouseUp,L);
this.addListener(z,L.handleMouseOver,L);
this.addListener(D,L.handleMouseMove,L);
this.addListener(s,L.handleLoseCapture,L);
this.addListener(y,L.handleKeyPress,L);
this.addListener(C,L.handleAddItem,L);
this.addListener(t,L.handleRemoveItem,L);
L.addListener(o,this._onSelectionChange,this);
},events:{"changeSelection":j},properties:{selectionMode:{check:[n,B,w,k],init:n,apply:x},dragSelection:{check:p,init:false,apply:h},quickSelection:{check:p,init:false,apply:A}},members:{__mO:null,selectAll:function(){this.__mO.selectAll();
},select:function(R){qx.log.Logger.deprecatedMethodWarning(arguments.callee,q);
this.setSelection([R]);
...
}return this.__mO.isItemSelected(H);
...
},removeFromSelection:function(P){if(!qx.ui.core.Widget.contains(this,P)){throw new Error("Could not remove "+P+" from selection, because it is not a child element!");
}this.__mO.removeItem(P);
},selectRange:function(c,d){this.__mO.selectItemRange(c,d);
...
},replaceSelection:function(G){qx.log.Logger.deprecatedMethodWarning(arguments.callee,q);
this.setSelection(G);
...
}else{this.__mO.replaceSelection(K);
}},getSelectedItem:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,m);
var a=this.getSelection();

if(a.length>0){return a[0];
}else{return null;
}},getSelected:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,m);
...
},getSelectables:function(){return this.__mO.getSelectables();
},invertSelection:function(){this.__mO.invertSelection();
},_applySelectionMode:function(I,J){this.__mO.setMode(I);
},_applyDragSelection:function(E,F){this.__mO.setDrag(E);
},_applyQuickSelection:function(N,O){this.__mO.setQuick(N);
},_onSelectionChange:function(e){this.fireDataEvent(o,e.getData());
...
(function(){var j="vertical",h="under",g="above",f="qx.ui.core.selection.Widget",e="__nm",d="left",c="right";
qx.Class.define(f,{extend:qx.ui.core.selection.Abstract,construct:function(v){arguments.callee.base.call(this);
this.__nm=v;
},members:{__nm:null,_isSelectable:function(m){return m.isEnabled()&&m.isVisible()&&m.getLayoutParent()===this.__nm;
},_selectableToHashCode:function(u){return u.$$hash;
},_styleSelectable:function(n,o,p){p?n.addState(o):n.removeState(o);
},_capture:function(){this.__nm.capture();
},_releaseCapture:function(){this.__nm.releaseCapture();
},_getWidget:function(){return this.__nm;
},_getLocation:function(){var P=this.__nm.getContentElement().getDomElement();
return P?qx.bom.element.Location.get(P):null;
},_getDimension:function(){return this.__nm.getInnerSize();
},_getSelectableLocationX:function(C){var D=C.getBounds();

if(D){return {left:D.left,right:D.left+D.width};
}},_getSelectableLocationY:function(M){var N=M.getBounds();

if(N){return {top:N.top,bottom:N.top+N.height};
}},_getScroll:function(){return {left:0,top:0};
},_scrollBy:function(a,b){},_scrollItemIntoView:function(k){this.__nm.scrollChildIntoView(k);
},getSelectables:function(){var s=this.__nm.getChildren();
var t=[];
var r;

for(var i=0,l=s.length;i<l;i++){r=s[i];

if(r.isEnabled()&&r.isVisible()){t.push(r);
}}return t;
},_getSelectableRange:function(E,F){if(E===F){return [E];
}var J=this.__nm.getChildren();
var G=[];
var I=false;
var H;
...
if(I&&H.isEnabled()&&H.isVisible()){G.push(H);
}}return G;
},_getFirstSelectable:function(){var q=this.__nm.getChildren();

for(var i=0,l=q.length;i<l;i++){if(q[i].isEnabled()&&q[i].isVisible()){return q[i];
}}return null;
},_getLastSelectable:function(){var O=this.__nm.getChildren();

for(var i=O.length-1;i>0;i--){if(O[i].isEnabled()&&O[i].isVisible()){return O[i];
}}return null;
},_getRelatedSelectable:function(w,x){var A=this.__nm.getOrientation()===j;
var z=this.__nm.getChildren();
var y=z.indexOf(w);
var B;

if((A&&x===g)||(!A&&x===d)){for(var i=y-1;i>=0;i--){B=z[i];

if(B.isEnabled()&&B.isVisible()){return B;
}}}else if((A&&x===h)||(!A&&x===c)){for(var i=y+1;i<z.length;i++){B=z[i];
...
(function(){var w="changeValue",v="qx.event.type.Data",u="horizontal",t="vertical",s="",r=",",q="qx.ui.form.List",p="Boolean",o="one",n="action",P="addChildWidget",O="_applySpacing",N="Please use setModelSelection instead.",M="list",L="Please use the changeSelection event instead.",K="keyinput",J="Integer",I="changeSelection",H="addItem",G="removeChildWidget",D="_applyOrientation",E="__nn",B="multi",C="single",z="keypress",A="Please use getModelSelection instead.",x="Enter",y="pane",F="removeItem";
qx.Class.define(q,{extend:qx.ui.core.AbstractScrollArea,implement:[qx.ui.form.IFormElement,qx.ui.core.IMultiSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MMultiSelectionHandling,qx.ui.form.MFormElement,qx.ui.form.MForm,qx.ui.form.MModelSelection],construct:function(W){arguments.callee.base.call(this);
this.__nn=new qx.ui.container.Composite();
this.__nn.addListener(P,this._onAddChild,this);
this.__nn.addListener(G,this._onRemoveChild,this);
this.getChildControl(y).add(this.__nn);
if(W){this.setOrientation(u);
}else{this.initOrientation();
}this.addListener(z,this._onKeyPress);
this.addListener(K,this._onKeyInput);
this.addListener(I,this._onChangeSelection);
this.__no=s;
},events:{addItem:v,removeItem:v,changeValue:v},properties:{appearance:{refine:true,init:M},focusable:{refine:true,init:true},orientation:{check:[u,t],init:t,apply:D},spacing:{check:J,init:0,apply:O,themeable:true},enableInlineFind:{check:p,init:true}},members:{__no:null,__np:null,__nn:null,SELECTION_MANAGER:qx.ui.core.selection.ScrollArea,getChildrenContainer:function(){return this.__nn;
},_onAddChild:function(e){this.fireDataEvent(H,e.getData());
},_onRemoveChild:function(e){this.fireDataEvent(F,e.getData());
},getValue:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee,A);
var X=this.getSelection();
var Y=[];
var ba;

for(var i=0,l=X.length;i<l;i++){ba=X[i].getValue();
if(ba==null){ba=X[i].getLabel();
}Y.push(ba);
}return Y.join(r);
},setValue:function(d){qx.log.Logger.deprecatedMethodWarning(arguments.callee,N);
var h=[d];

if(this.getSelectionMode()===B){h=d.split(r);
}var g=[];
var f;

for(var i=0,l=h.length;i<l;i++){f=this.findItem(h[i]);

if(f){g.push(f);
}}this.setSelection(g);
},handleKeyPress:function(e){if(!this._onKeyPress(e)){this._getManager().handleKeyPress(e);
}},_applyOrientation:function(Q,R){var S=Q===u;
var T=S?new qx.ui.layout.HBox():new qx.ui.layout.VBox();
var content=this.__nn;
content.setLayout(T);
content.setAllowGrowX(!S);
content.setAllowGrowY(S);
this._applySpacing(this.getSpacing());
},_applySpacing:function(be,bf){this.__nn.getLayout().setSpacing(be);
},_onKeyPress:function(e){if(e.getKeyIdentifier()==x&&!e.isAltPressed()){var bg=this.getSelection();

for(var i=0;i<bg.length;i++){bg[i].fireEvent(n);
}return true;
}return false;
},_onChangeSelection:function(){if(this.hasListener(w)){this.fireDataEvent(w,this.getValue());
}},_onKeyInput:function(e){if(!this.getEnableInlineFind()){return;
}var U=this.getSelectionMode();

if(!(U===C||U===o)){return;
}if(((new Date).valueOf()-this.__np)>1000){this.__no=s;
}this.__no+=e.getChar();
var V=this.findItemByLabelFuzzy(this.__no);
if(V){this.setSelection([V]);
}this.__np=(new Date).valueOf();
...
},findItem:function(bb){bb=bb.toLowerCase();
var bc=this.getChildren();
var bd;
for(var i=0,l=bc.length;i<l;i++){bd=bc[i];

if((bd.getLabel()!=null)&&(bd.getLabel().toLowerCase()==bb)){return bd;
}}return null;
},addListener:function(j,k,self,m){if(j==w){qx.log.Logger.deprecatedEventWarning(arguments.callee,w,L);
}return arguments.callee.base.call(this,j,k,self,m);
