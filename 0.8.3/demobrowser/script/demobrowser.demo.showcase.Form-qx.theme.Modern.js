...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.showcase.Form", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.showcase.Form-qx.theme.Modern-0.js"]],
  boot : "boot",
...
m.addListener(B,function(){var bv=parseInt(m.getLabel())+1;
m.setLabel(bv.toString());
});
...
},createItems:function(bB){for(var i=0;i<arguments.callee.self.ITEM_SIZE;i++){var bC=new qx.ui.form.ListItem(V+i);
bB.add(bC);
}},createMenuForMenuButton:function(){var bw=new qx.ui.menu.Menu;

for(var i=0;i<3;i++){bw.add(new qx.ui.menu.RadioButton(Q+i));
}var bz=new qx.ui.form.RadioGroup;
bz.add.apply(bz,bw.getChildren());
var by=new qx.ui.menu.Menu();

for(var i=0;i<3;i++){var bA=new qx.ui.menu.Button(G+i);
by.add(bA);
}var bx=new qx.ui.menu.Button(bl,N,null,bw);
by.addSeparator();
by.add(bx);
return by;
},createMenuForSplitButton:function(){var br=new qx.ui.menu.Menu;
var bu=new qx.ui.menu.Button(be);
var bs=new qx.ui.menu.Button(A);
var bt=new qx.ui.menu.Button(bg);
br.add(bu);
br.add(bs);
br.add(bt);
return br;
...
(function(){var v="button",u="hovered",t="pressed",s="changeValue",r="arrow",q="Enter",p="Space",o="abandoned",n="both",m="String",O="changeVisibility",N="splitbutton",M="changeShow",L="mouseout",K="keydown",J="execute",I="qx.event.type.Data",H="_applyMenu",G="The value property will be removed.",F="icon",C="mouseover",D="keyup",A="qx.ui.menu.Menu",B="_applyIcon",y="label",z="_applyShow",w="changeMenu",x="_applyLabel",E="qx.ui.form.SplitButton";
qx.Class.define(E,{extend:qx.ui.core.Widget,include:[qx.ui.core.MExecutable,qx.ui.form.MFormElement],implement:[qx.ui.form.IFormElement,qx.ui.form.IExecutable],construct:function(g,h,i,j){arguments.callee.base.call(this);
this._setLayout(new qx.ui.layout.HBox);
this._createChildControl(r);
this.addListener(C,this._onMouseOver,this,true);
this.addListener(L,this._onMouseOut,this,true);
...
if(j!=null){this.setCommand(j);
}},events:{"changeValue":I},properties:{appearance:{refine:true,init:N},focusable:{refine:true,init:true},label:{apply:x,nullable:true,dispose:true,check:m},icon:{check:m,apply:B,nullable:true,themeable:true},show:{init:n,check:[n,y,F],themeable:true,inheritable:true,apply:z,event:M},menu:{check:A,nullable:true,apply:H,event:w}},members:{__wb:null,_createChildControlImpl:function(V){var W;

switch(V){case v:W=new qx.ui.form.Button;
W.addListener(J,this._onButtonExecute,this);
W.setFocusable(false);
this._addAt(W,0,{flex:1});
break;
case r:W=new qx.ui.form.MenuButton;
W.setFocusable(false);
this._addAt(W,1);
break;
}return W||arguments.callee.base.call(this,V);
},_forwardStates:{hovered:1,focused:1},_applyLabel:function(P,Q){var R=this.getChildControl(v);
P==null?R.resetLabel():R.setLabel(P);
},_applyIcon:function(S,T){var U=this.getChildControl(v);
S==null?U.resetIcon():U.setIcon(S);
},_applyMenu:function(c,d){var f=this.getChildControl(r);

if(c){f.resetEnabled();
f.setMenu(c);
c.setOpener(this);
c.addListener(O,this._onChangeMenuVisibility,this);
}else{f.setEnabled(false);
f.resetMenu();
}
if(d){d.removeListener(O,this._onChangeMenuVisibility,this);
d.resetOpener();
}},_applyShow:function(bb,bc){},_onMouseOver:function(e){e.stopPropagation();
this.addState(u);
delete this.__wb;
},_onMouseOut:function(e){e.stopPropagation();
if(!this.hasState(u)){return;
}var b=e.getRelatedTarget();

if(qx.ui.core.Widget.contains(this,b)){return;
}var a=this.getMenu();

if(a&&a.isVisible()){this.__wb=true;
return;
}this.removeState(u);
},_onKeyDown:function(e){var be=this.getChildControl(v);

switch(e.getKeyIdentifier()){case q:case p:be.removeState(o);
be.addState(t);
}},_onKeyUp:function(e){var bd=this.getChildControl(v);

switch(e.getKeyIdentifier()){case q:case p:if(bd.hasState(t)){bd.removeState(o);
bd.removeState(t);
bd.execute();
}}},_onButtonExecute:function(e){this.execute();
},_onChangeMenuVisibility:function(e){if(!this.getMenu().isVisible()&&this.__wb){this.removeState(u);
}},__wc:null,setValue:function(k){qx.log.Logger.deprecatedMethodWarning(arguments.callee);
var l=this.__wc;
this.__wc=k;
this.fireDataEvent(s,k,l);
},getValue:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee);
return this.__wc;
},resetValue:function(){qx.log.Logger.deprecatedMethodWarning(arguments.callee);
this.__wc=null;
},addListener:function(X,Y,self,ba){if(X==s){qx.log.Logger.deprecatedEventWarning(arguments.callee,s,G);
