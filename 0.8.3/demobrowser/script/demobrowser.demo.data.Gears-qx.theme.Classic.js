...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.data.Gears", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.data.Gears-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var s="value",r='undefined',q="",p="model",o="application/x-googlegears",n="demobrowser.demo.data.store.Gears",m="<span style='font-size: 20px'>google Gears</span>",l="Age: ",k="http://code.google.com/apis/gears/install.html",j="lastname",K="firstname",J="age",I="demobrowser.demo.data.Gears",H="First Name: ",G='qooxdoo',F='Gears.Factory',E='select * from Persons',D="delete from Persons",C="LastName: ",B=' (Firstname varchar(255), Lastname varchar(255), Age int)',z="Person",A='ie_mobile',x="none",y='create table if not exists Persons',v="This demo requires Gears to be installed. Install now?",w='insert into Persons values (?, ?, ?)',t='beta.database',u="object";
qx.Class.define(I,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var T=new qx.ui.groupbox.GroupBox(z);
this.getRoot().add(T,{left:10,top:50});
T.setLayout(new qx.ui.layout.Grid(0,3));
T.add(new qx.ui.basic.Label(H),{row:0,column:0});
T.add(new qx.ui.basic.Label(C),{row:1,column:0});
T.add(new qx.ui.basic.Label(l),{row:2,column:0});
...
R.addTarget(N,s,K,true);
R.addTarget(P,s,j,true);
R.addTarget(Q,s,J,true);
...
qx.Class.define(n,{extend:qx.data.store.Json,construct:function(){arguments.callee.base.call(this);
this.initGears();
if(!window.google||!google.gears){if(confirm(v)){location.href=k;
return;
}return;
}this.__AL=this.initDB();
this.__AM();
},members:{__AM:function(h){var i=this.getPerson();
this._marshaler.toClass(i,true);
this.setModel(this._marshaler.toModel(i));
this.getModel().addListener("changeBubble",this.__AN,this);
this.fireDataEvent("loaded",this.getModel());
},__AN:function(U){var V=this.getModel();
this.update(V.getFirstname(),V.getLastname(),V.getAge());
},update:function(c,d,f){if(!google.gears.factory||!this.__AL){return;
}this.__AL.execute(D);
this.__AL.execute(w,[c,d,f]);
},getPerson:function(){var a={};
a.firstname=q;
a.lastname=q;
a.age=0;

if(!google.gears.factory||!this.__AL){return a;
}var b=this.__AL.execute(E);

if(b.isValidRow()){a.firstname=b.field(0);
a.lastname=b.field(1);
a.age=parseInt(b.field(2));
}b.close();
return a;
},initDB:function(){if(window.google&&google.gears){try{var g=google.gears.factory.create(t);

if(g){g.open(G);
g.execute(y+B);
}}catch(M){this.error('Could not create database: '+M.message);
}}return g;
},initGears:function(){if(window.google&&google.gears){return;
}var L=null;
if(typeof GearsFactory!=r){L=new GearsFactory();
}else{try{L=new ActiveXObject(F);
if(L.getBuildInfo().indexOf(A)!=-1){L.privateSetGlobalObject(this);
}}catch(e){if((typeof navigator.mimeTypes!=r)&&navigator.mimeTypes[o]){L=document.createElement(u);
L.style.display=x;
L.width=0;
L.height=0;
L.type=o;
