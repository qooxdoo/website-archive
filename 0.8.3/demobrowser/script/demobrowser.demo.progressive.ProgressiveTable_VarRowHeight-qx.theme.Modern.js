...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.progressive.ProgressiveTable_VarRowHeight-qx.theme.Modern-0.js"]],
  boot : "boot",
...
var S=s;
this.imageData=[{url:S+m,height:4,tooltip:w},{url:S+d,height:4,tooltip:p},q];
this.__zd=0;
var Q=qx.util.AliasManager.getInstance();
var P=qx.util.ResourceManager.getInstance();
var O;
var R;

for(var i=0;i<this.imageData.length;i++){if(!this.imageData[i]){continue;
}O=Q.resolve(this.imageData[i].url);
R=P.toUri(O);
this.imageData[i].url=R;
++this.__zd;
qx.io2.ImageLoader.load(R,function(a,b){this.info("Searching for ["+a+"]");
for(var j=0;j<this.imageData.length;j++){if(this.imageData[j].url==a){this.imageData[j].height=b.height;
--this.__zd;
this.debug("Found ["+this.imageData[j].url+"].  loadCount="+this.__zd+", end="+this.imageData.length);
if(this.__zd==0){this.doLoad();
}return;
}}throw new Error("Image data for "+a+" not found");
},this);
}if(this.__zd==0){this.doLoad();
}},doLoad:function(){var I=0;
var z=this;
var H=function(L,M){var N={renderer:x,location:e,data:[M,z.imageData[L]]};
return N;
};
...
G.setWidth(1,t);
var B=[g,l];
var D=new qx.ui.progressive.headfoot.TableHeading(G,B);
var A=new qx.ui.progressive.headfoot.Progress(G,B);
var K=new qx.ui.progressive.structure.Default(D,A);
var y=new qx.ui.progressive.Progressive(K);
var J=new qx.ui.basic.Atom(c);
J.setHeight(16);
J.setTextColor(n);
y.add(J);
...
F.addElements(E);
y.setDataModel(F);
var C=new qx.ui.progressive.renderer.table.Row(G);
y.addRenderer(x,C);
...
var E=this.__yT(C);
D.push(d);
if(qx.core.Variant.isSet(h,t)&&/\.png$/i.test(E.url)){D.push(k,this.__yS,p,e,E.url,m,l);
}else{D.push(k,E.url,l);
}if(E.imageWidth){D.push(s,E.imageWidth,j);
...
return D.join(w);
},__yT:function(a){var b=this._identifyImage(a);
if(b==null||typeof b==g){b={url:b,tooltip:null};
}if(b.url==null){b.url=this.__yS;
}return b;
}},destruct:function(){this._disposeFields(q);
}});
})();
(function(){var g="__zf",f="__ze",e="qx.ui.progressive.renderer.table.cell.Image",d="string",c="";
qx.Class.define(e,{extend:qx.ui.progressive.renderer.table.cell.Icon,construct:function(a,b){arguments.callee.base.call(this);

if(a===undefined){this.__ze=a;
}else{this.__ze=16;
}
if(b===undefined){this.__zf=b;
}else{this.__zf=16;
}},members:{__ze:null,__zf:null,_identifyImage:function(h){var l={imageWidth:this.__ze,imageHeight:this.__zf};
var m;
if(typeof (h.cellData)==d){l.url=h.cellData;
}else{l.url=h.cellData.url;
l.tooltip=h.cellData.tooltip;
m=h.cellData.height;
}
if(l.url==c){l.url=this._imageBlank;
