...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.data.TreeController", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.data.TreeController-qx.theme.Modern-0.js"]],
  boot : "boot",
...
(function(){var C="execute",B="name",A="name2",z="Thing ",y="Item ",x="Change the label binding",w="Write data to log",v="Remove an item from root",u="'Thing i'.",t="A list containing the selected items of the tree.",n="multi",s="<b>Tree binding</b><br/>",q="Add an item to root",m="<b>Selected Items</b><br/>",l="This tree is bound to a set of randomly generated nodes. Every node",p="children",o="demobrowser.demo.data.TreeController",r=" does have a name and a name2 property, containing 'Item i' and ";
qx.Class.define(o,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var g=[];

for(var i=0;i<50;i++){g[i]=new demobrowser.demo.data.model.Node();
g[i].setName(y+i);
g[i].setName2(z+i);
if(i!=0){g[parseInt(Math.random()*i)].getChildren().push(g[i]);
}}var a=new qx.ui.tree.Tree();
a.setSelectionMode(n);
this.getRoot().add(a,{left:10,top:100});
a.setWidth(250);
a.setHeight(300);
var j=new qx.data.controller.Tree(g[0],a,p,B);
a.getRoot().setOpen(true);
var b=new qx.ui.form.List();
b.setHeight(300);
this.getRoot().add(b,{left:270,top:100});
new qx.data.controller.List(j.getSelection(),b,A);
var h=new qx.ui.form.Button(q);
h.setWidth(180);
this.getRoot().add(h,{left:390,top:100});
h.addListener(C,function(){var D=new demobrowser.demo.data.model.Node();
D.setName(y+a.getItems(true).length);
D.setName2(z+a.getItems(true).length);
g[0].getChildren().push(D);
},this);
var c=new qx.ui.form.Button(v);
c.setWidth(180);
this.getRoot().add(c,{left:390,top:135});
c.addListener(C,function(){g[0].getChildren().pop();
},this);
var d=new qx.ui.form.Button(x);
d.setWidth(180);
this.getRoot().add(d,{left:390,top:170});
d.addListener(C,function(){if(j.getLabelPath()==B){j.setLabelPath(A);
}else{j.setLabelPath(B);
}},this);
var e=new qx.ui.form.Button(w);
e.setWidth(180);
this.getRoot().add(e,{left:390,top:205});
e.addListener(C,function(){this.info(g[0].toString());
},this);
...
k.setWidth(100);
k.setValue(m+t);
this.getRoot().add(k,{left:270,top:10});
}}});
})();
...
(function(){var s="qx.data.Array",r="demobrowser.demo.data.model.Node",q="String",p="changeNames",o="changeName",n="Validation Error: ",m="- ",l="",k="changeChild",j="__zJ",c="changeColor",h="Marge",f="AFFE",b="is no number (parseFloat says so!).",a="Homer",e="\n",d="changeName2",g="changeChildren";
...
},properties:{children:{check:s,event:g},child:{check:r,event:k,nullable:true},names:{check:s,event:p,init:new qx.data.Array(a,h)},name:{check:q,init:f,event:o},name2:{check:q,event:d},number:{init:10,validate:j},color:{event:c,nullable:true}},members:{__zJ:function(t){if(!isNaN(parseFloat(t))){return;
...
(function(){var J="get",I="String",H="changeModel",G="_applyDelegate",F="qx.core.Object",E="_applyLabelPath",D="icon",C="change",B="label",A="_applyTarget",t="changeTarget",z="_applyIconPath",w="__Ap",s="_applyLabelOptions",r="_applyChildPath",v="__Ao",u="_applyIconOptions",x="qx.data.controller.Tree",q="__An",y="_applyModel";
qx.Class.define(x,{extend:qx.core.Object,include:qx.data.controller.MSelection,construct:function(N,O,P,Q){arguments.callee.base.call(this);
this.__An={};
this.__Ao=[];
this.__Ap={};

if(P!=null){this.setChildPath(P);
}
if(Q!=null){this.setLabelPath(Q);
}
if(N!=null){this.setModel(N);
}
if(O!=null){this.setTarget(O);
}},properties:{model:{check:F,apply:y,event:H,nullable:true},target:{apply:A,event:t,init:null},childPath:{check:I,apply:r,nullable:true},labelPath:{check:I,apply:E,nullable:true},iconPath:{check:I,apply:z,nullable:true},labelOptions:{apply:s,nullable:true},iconOptions:{apply:u,nullable:true},delegate:{apply:G,init:null,nullable:true}},members:{__Ap:null,__An:null,__Ao:null,_applyDelegate:function(m,n){this._setConfigureItem(m,n);
this._setCreateItem(m,n);
this._setBindItem(m,n);
},_applyIconOptions:function(U,V){this.__Ax();
},_applyLabelOptions:function(bo,bp){this.__Ax();
},_applyTarget:function(K,L){if(L!=undefined){var M=L.getRoot();
L.setRoot(null);
M.destroy();
}if(this.getModel()!=null){this.__As();
}this._addChangeTargetListener(K,L);
},_applyModel:function(k,l){this.__As();
},_applyChildPath:function(bJ,bK){this.__As();
},_applyIconPath:function(bR,bS){this.__Ax();
},_applyLabelPath:function(bL,bM){this.__Ax();
},__Aq:function(bN){var bP=bN.getTarget();
var bO=this.__Ap[bP.toHashCode()].treeNode;
var bQ=this.__Ap[bP.toHashCode()].modelNode;
this.__At(bO,bQ);
this._updateSelection();
},__Ar:function(e){var bu=e.getData();
var bt=e.getOldData();
var bv=this.__Ap[bt.toHashCode()];
delete this.__Ap[bt.toHashCode()];
bv.modelNode.removeListenerById(bv.changeChildernListenerId);
var by=bv.modelNode;
var bx=qx.util.PropertyUtil.getProperties(bv.modelNode.constructor);
var br=bx[this.getChildPath()].event;
var bw=by.addListener(br,this.__Ar,this);
var bs=bv.treeNode;
this.__Ap[bu.toHashCode()]={modelNode:by,treeNode:bs,changeListenerId:bv.changeListenerId,changeChildernListenerId:bw};
this.__At(bs,by);
this._updateSelection();
},_createItem:function(){var f=this.getDelegate();
if(f!=null&&f.createItem!=null){var d=f.createItem();
}else{var d=new qx.ui.tree.TreeFolder();
}if(f!=null&&f.configureItem!=null){f.configureItem(d);
}return d;
},__As:function(){if(this.getTarget()==null||this.getChildPath()==null){return;
...
}var bm=this.getTarget().getRoot();

if(bm!=null){this.getTarget().resetRoot();
bm.destroy();
}if(this.getModel()!=null){var bn=this._createItem();
bn.setModel(this.getModel());
this.getTarget().setRoot(bn);
this.__Ay(this.getModel(),bn);
this.__At(bn,this.getModel());
}},__At:function(W,X){if(X[J+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;
}var bc=X[J+qx.lang.String.firstUp(this.getChildPath())]();
if(this.__Ap[bc.toHashCode()]==undefined){var bg=bc.addListener(C,this.__Aq,this);
var bh=qx.util.PropertyUtil.getProperties(X.constructor);
var bd=bh[this.getChildPath()].event;
var be=X.addListener(bd,this.__Ar,this);
this.__Ap[bc.toHashCode()]={modelNode:X,treeNode:W,changeListenerId:bg,changeChildernListenerId:be};
}for(var i=0;i<bc.length;i++){if(W.getChildren()[i]==null||bc.getItem(i)!=W.getChildren()[i].getModel()){for(var j=i;j<W.getChildren().length;j++){if(W.getChildren()[j].getModel()===bc.getItem(i)){var Y=j;
break;
...
W.addAt(bf,i);
this.__Ay(bc.getItem(i),bf);
this.__At(bf,bc.getItem(i));
}}}for(var i=W.getChildren().length-1;i>=bc.length;i--){var bb=W.getChildren()[i];
this.__Aw(bb,W);
}},__Au:function(){if(this.getTarget()==null){return;
}var bq=this.getTarget().getRoot();

if(bq!=null){this.getTarget().setRoot(null);
this.__Av(bq);
this.__Az(bq.getModel());
bq.destroy();
this.__Ap={};
}},__Av:function(g){var h=g.getChildren();
for(var i=h.length-1;i>=0;i--){if(h[i].getChildren().length>0){this.__Av(h[i]);
}this.__Aw(h[i],g);
}},__Aw:function(R,S){var T=R.getModel();
if(T[J+qx.lang.String.firstUp(this.getChildPath())]!=undefined){delete this.__Ap[T[J+qx.lang.String.firstUp(this.getChildPath())]().toHashCode()];
}this.__Az(T);
S.remove(R);
R.destroy();
},bindProperty:function(bT,bU,bV,bW,bX){var bY=bX.bind(bT,bW,bU,bV);
if(this.__An[bU]==null){this.__An[bU]={};
}this.__An[bU][bX.toHashCode()]={id:bY,treeNode:bW};
if(!qx.lang.Array.contains(this.__Ao,bU)){this.__Ao.push(bU);
}},__Ax:function(){var bD;

for(var bF in this.__An){bD=bF;
break;
}for(var bC in this.__An[bD]){var bG=this.__An[bD][bC].treeNode;
var bE=qx.core.ObjectRegistry.fromHashCode(bC);
this.__Az(bE);
this.__Ay(bE,bG);
}},__Ay:function(bz,bA){var bB=this.getDelegate();
if(bB!=null&&bB.bindItem!=null){bB.bindItem(this,bA,bz);
}else{this.bindProperty(this.getLabelPath(),B,this.getLabelOptions(),bA,bz);
if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),D,this.getIconOptions(),bA,bz);
}}},__Az:function(bj){for(var i=0;i<this.__Ao.length;i++){var bl=this.__Ao[i];
var bk=this.__An[bl][bj.toHashCode()];

if(bk!=null){bj.removeBinding(bk.id);
delete this.__An[bl][bj.toHashCode()];
}}},_setConfigureItem:function(a,b){if(a!=null&&a.configureItem!=null&&this.getTarget()!=null){var c=this.getTarget().getRoot().getItems(true,true,false);

for(var i=0;i<c.length;i++){a.configureItem(c[i]);
}}},_setCreateItem:function(o,p){if(this.getTarget()==null||this.getModel()==null){return;
}if(o==null||o.createItem==null){return;
}if(p&&p.createItem&&o&&o.createItem&&p.createItem==o.createTtem){return;
...
}}},destruct:function(){this._disposeFields(q,w);
