...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.bom.Selector-qx.theme.Modern-0.js"]],
  boot : "boot",
...
br.query(bl).setStyle(h,V);
br.query(P).setStyle(bm,bk).setStyle(B,be);
br.query(X).setStyles({"textAlign":f,"textDecoration":v,"color":s});
bv.debug("href of first link: "+br.query("a").getAttribute("href"));
br.query(d).setAttribute(e,M);
br.query(bj).addClass(bc);
br.query(bl).addListener(O,function(){alert(this.innerHTML);
});
...
bv.debug(bp);
br.query(N).eq(1).setStyle(bm,bk);
...
var bx=document.createElement(l);
var bw=document.createElement(W);
var bs=new qx.bom.Collection(bx).setAttribute(I,g).setAttribute(C,e);
var bq=new qx.bom.Collection(bw).setAttribute(o,g).setAttribute(bh,K);
qx.bom.Collection.query(l).after(bw,bx);
bs.append(r,t,w,q);
$(A).insertAfter(j);
qx.bom.Collection.id(bo).addListener(k,function(){bv.debug("Typing in e-mail field...");
});
var bt=qx.bom.Collection.id(bo);
var bu=bt.add(ba).reverse();
