...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.table.Table_Context_Menu", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.table.Table_Context_Menu-qx.theme.Modern-0.js"]],
  boot : "boot",
...
(function(){var y="execute",x="changeValue",w="value",v="icon/22/actions/edit-undo.png",u="Change ID sort method",t="Disabled ctx menu",s="icon/22/status/dialog-information.png",r="On",q="Change row with ID 10",p="Default ctx menu #2",Q="Remove 5 rows",P="Add 10 rows",O="Context-menu selectable",N="",M="_tableModel",L="Table",K="icon/22/actions/list-remove.png",J="Whether the the first visible row should ",I=", ",H="demobrowser.demo.table.Table_Context_Menu",F="Default ctx menu #1",G="icon/22/actions/list-add.png",D="Selected rows:<br>",E="Keep first row",B="Off",C="be rendered completely when scrolling.",z="Show selection",A="Demonstrate use of alternate sorting algorithm.";
qx.Class.define(H,{extend:demobrowser.demo.table.TableDemo,members:{main:function(){qx.Class.include(qx.ui.table.Table,qx.ui.table.MTableContextMenu);
arguments.callee.base.call(this);
this.getRoot().setNativeContextMenu(true);
},getCaption:function(){return L;
},createTable:function(){var bf=this.createRandomRows(50);
var be=this._tableModel=new qx.ui.table.model.Simple();
be.setColumns([F,p,t,O]);
be.setData(bf);
var bg=new qx.ui.table.Table(be);
bg.set({width:600,height:400,decorator:null});
var bd=bg.getTableColumnModel();
bd.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
bg.setContextMenuHandler(3,this._contextMenuHandlerBoolean);
bg.setContextMenuHandler(2,this._contextMenuHandlerNull);
return bg;
},_contextMenuHandlerBoolean:function(W,X,Y,ba,bb){var bc;

for(var i=0;i<=1;i++){bc=new qx.ui.menu.Button(i?r:B);
bc.setUserData(w,i?true:false);
bc.addListener(y,function(e){ba.setValue(W,X,this.getUserData(w));
});
bb.add(bc);
}return true;
},_contextMenuHandlerNull:function(bj,bk,bl,bm,bn){return true;
},createControls:function(){var bs=new qx.ui.toolbar.ToolBar();
var bq,br,bt;
br=new qx.ui.toolbar.Part();
bs.add(br);
bq=new qx.ui.toolbar.Button(q,v);
bq.addListener(y,function(n){var o=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,o[0][i]);
}this.info("Row 10 changed");
},this);
br.add(bq);
bq=new qx.ui.toolbar.Button(P,G);
bq.addListener(y,function(l){var m=this.createRandomRows(10);
this._tableModel.addRows(m);
this.info("10 rows added");
},this);
br.add(bq);
bq=new qx.ui.toolbar.Button(Q,K);
bq.addListener(y,function(bh){var bi=this._tableModel.getRowCount();
this._tableModel.removeRows(bi-5,5);
this.info("5 rows removed");
},this);
br.add(bq);
bq=new qx.ui.toolbar.Button(z,s);
bq.addListener(y,function(U){var V=[];
bp.getSelectionModel().iterateSelection(function(bo){V.push(bo+N);
});
this.showDialog(D+V.join(I));
},this);
br.add(bq);
br=new qx.ui.toolbar.Part();
bs.add(br);
var bp=this._table;
bt=new qx.ui.toolbar.CheckBox(E);
bt.set({value:bp.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(J+C)});
bt.addListener(x,function(f){bp.setKeepFirstVisibleRowComplete(this.getValue());
},bt);
br.add(bt);
bt=new qx.ui.toolbar.CheckBox(u);
bt.set({value:bp.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(A)});
bt.addListener(x,function(R){if(R.getData()){var T=function(g,h){var j=g[arguments.callee.columnIndex];
var k=h[arguments.callee.columnIndex];
...
var S=function(a,b){var c=a[arguments.callee.columnIndex];
var d=b[arguments.callee.columnIndex];

if(c%2==1&&d%2==0){return -1;
}
if(d%2==1&&c%2==0){return 1;
}return (c<d)?1:((c==d)?0:-1);
};
bp.getTableModel().setSortMethods(0,{ascending:T,descending:S});
...
(function(){var k="contextmenu",j="qx.ui.table.MTableContextMenu",i="cellContextmenu",h="__pR";
qx.Mixin.define(j,{construct:function(){this.addListener(i,function(e){if(this._contextMenuTimer){this._contextMenuTimer.dispose();
...
this.__pR=[];
},members:{__pR:null,setContextMenuHandler:function(a,b){this.__pR[a]=b;
},getContextMenuHandler:function(l){return this.__pR[l];
}},destruct:function(){this._disposeFields(h);
}});
})();
