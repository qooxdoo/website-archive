...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.event.Event_Bus", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.event.Event_Bus-qx.theme.Modern-0.js"]],
  boot : "boot",
...
},unsubscribe:function(p,q,r){return this.getInstance().unsubscribe(p,q,r);
},dispatch:function(a){return this.getInstance().dispatch.apply(this,arguments);
}},construct:function(){this.__qA={};
},members:{__qA:null,getSubscriptions:function(){return this.__qA;
},subscribe:function(v,w,x){if(!v||typeof w!=g){this.error("Invalid parameters! "+[v,w,x]);
return false;
}var y=this.getSubscriptions();

if(this.checkSubscription(v)){if(this.checkSubscription(v,w,x)){this.warn("Object method already subscribed to "+v);
return false;
}y[v].push({subscriber:w,context:x||null});
return true;
}else{y[v]=[{subscriber:w,context:x||null}];
return true;
}},checkSubscription:function(l,m,n){var o=this.getSubscriptions();

if(!o[l]||o[l].length==0){return false;
}
if(m){for(var i=0;i<o[l].length;i++){if(o[l][i].subscriber==m&&o[l][i].context==(n||null)){return true;
}}return false;
}return true;
},unsubscribe:function(H,I,J){var K=this.getSubscriptions();

if(!K[H]){return false;
}
if(I){for(var i=0;i<K[H].length;i++){if(K[H][i].subscriber==I&&K[H][i].context==(J||null)){K[H].splice(i,1);
return true;
}}}K[H]=null;
return true;
},dispatch:function(z){if(typeof z==c){var E=typeof arguments[1]!=d?arguments[1]:true;
z=new qx.event.message.Message(z,E);
}var B=this.getSubscriptions();
var C=z.getName();

for(var G in B){var D=G.indexOf(e);

if(D>-1){if(D==1||G.substr(0,D)==C.substr(0,D)){for(var i=0;i<B[G].length;i++){var F=B[G][i].subscriber;
var A=B[G][i].context;
F.call(A,z);
}}}else{if(G==C){for(var i=0;i<B[C].length;i++){var F=B[C][i].subscriber;
var A=B[C][i].context;
F.call(A,z);
}return true;
}}}}}});
