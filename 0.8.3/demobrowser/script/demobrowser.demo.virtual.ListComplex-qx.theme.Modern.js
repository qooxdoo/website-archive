...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.virtual.ListComplex", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.virtual.ListComplex-qx.theme.Modern-0.js"]],
  boot : "boot",
...
var f=new qx.ui.virtual.core.Scroller(100,15,32,120);
f.getPane().setWidth(450);
for(var i=2;i<10;i++){f.getPane().getRowConfig().setItemSize(i,50+Math.round(Math.random()*40));
f.getPane().getColumnConfig().setItemSize(i,50+Math.round(Math.random()*80));
}this.getRoot().add(f,{left:20,top:10});
f.getPane().addLayer(new qx.ui.virtual.layer.Row(a,b));
f.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
f.getPane().addLayer(new demobrowser.demo.virtual.DemoLayer);
var e=new qx.ui.virtual.behavior.Prefetch(f,500,600,1000,1200,0,0,0,0);
}}});
})();
...
(function(){var v=" / ",u="row",t="foobar!",s="column",r="__to",q="icon/",p="",o="folder.png",n="places/",m="demobrowser.demo.virtual.DemoLayer",f="network-server.png",l="mouseover",i="user-trash.png",e="changeValue",d="network-workgroup.png",h="qx.ui.basic.Atom",g="/",k="_pool",c="user-desktop.png";
qx.Class.define(m,{extend:qx.ui.virtual.layer.WidgetCell,construct:function(){arguments.callee.base.call(this,this);
this._pool={atom:[],checkbox:[]};
this.__to=[];
},members:{_pool:null,_rowData:null,getCellData:function(a,b){if(!this.__to[a]){this.__to[a]=[];
}
if(!this.__to[a][b]){this.__to[a][b]={label:this.__tp(),icon:this.__tq()};
}return this.__to[a][b];
},getCellWidget:function(x,y){var z;

if(y%2==0){z=this._pool.atom.pop();

if(!z){z=new qx.ui.basic.Atom();
z.addListener(l,function(){var w=this.__tq();
z.setIcon(w);
this.__to[x][y].icon=w;
},this);
}z.set(this.getCellData(x,y));
}else{z=this._pool.checkbox.pop();

if(!z){z=new qx.ui.form.CheckBox();
z.addListener(e,function(){this.setLabel(this.getLabel()==t?z.getUserData(u)+v+z.getUserData(s):t);
},z);
}z.set({value:x%2==0,label:x+v+y});
}z.setUserData(u,x);
z.setUserData(s,y);
return z;
},poolCellWidget:function(A){if(A.classname==h){this._pool.atom.push(A);
}else{this._pool.checkbox.push(A);
}},__tp:function(){var name=p;

for(var j=0;j<10;j++){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);
}return name;
},__tq:function(){var E=q;
var B=n;
var F=[o,i,f,d,c];
...
return (E+16+g+B+F[D]);
}},destruct:function(){this._disposeFields(k,r);
}});
})();
