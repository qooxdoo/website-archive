...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.ui.FormRenderer", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.ui.FormRenderer-qx.theme.Classic-0.js"]],
  boot : "boot",
...
q.addButton(r);
this.getRoot().add(q.createView(),{left:10,top:10});
}}});
})();
...
(function(){var l="qx.ui.form.Form",k="";
qx.Class.define(l,{extend:qx.core.Object,construct:function(){arguments.callee.base.call(this);
this.__mk=[];
this.__ml=[];
this.__mm=new qx.ui.form.validation.Manager();
this.__mn=new qx.ui.form.Resetter();
},members:{__mk:null,__mm:null,__mo:0,__ml:null,__mn:null,add:function(f,g,h,name){if(this.__mp()){this.__mk.push({title:null,items:[],labels:[],names:[]});
}this.__mk[this.__mo].items.push(f);
this.__mk[this.__mo].labels.push(g);
if(name==null){name=g.replace(/\s+/g,k);
}this.__mk[this.__mo].names.push(name);
this.__mm.add(f,h);
this.__mn.add(f);
},addGroupHeader:function(p){if(!this.__mp()){this.__mo++;
}this.__mk.push({title:p,items:[],labels:[],names:[]});
},addButton:function(o){this.__ml.push(o);
},__mp:function(){return this.__mk.length===0;
},reset:function(){this.__mn.reset();
this.__mm.reset();
},validate:function(){return this.__mm.validate();
},getValidationManager:function(){return this.__mm;
},createView:function(c){if(c==null){c=qx.ui.form.renderer.Single;
}else{this.__mq(c);
}var e=new c();
for(var i=0;i<this.__mk.length;i++){var d=this.__mk[i];
e.addItems(d.items,d.labels,d.title);
}for(var i=0;i<this.__ml.length;i++){e.addButton(this.__ml[i]);
}return e;
},__mq:function(a){var b;
{};
},getItems:function(){var m={};
for(var i=0;i<this.__mk.length;i++){var n=this.__mk[i];
for(var j=0;j<n.names.length;j++){var name=n.names[j];
m[name]=n.items[j];
}}return m;
}}});
})();
...
this.__mI=[];
},members:{__mI:null,add:function(e){if(this.__mK(e)){var f=e.getValue();
}else if(this.__mJ(e)){var f=e.getSelection();
}else{throw new Error("Item "+e+" not supported for reseting.");
}this.__mI.push({item:e,init:f});
},reset:function(){for(var i=0;i<this.__mI.length;i++){var d=this.__mI[i];
if(this.__mK(d.item)){d.item.setValue(d.init);
}else if(this.__mJ(d.item)){d.item.setSelection(d.init);
}}},__mJ:function(a){var b=a.constructor;
return qx.Class.hasInterface(b,qx.ui.core.ISingleSelection);
},__mK:function(g){var h=g.constructor;
...
(function(){var k="right",j=" <span style='color:red'>*</span> ",h="bold",g="",f="qx.ui.form.renderer.Single",e="top",d="left",c=" :";
qx.Class.define(f,{extend:qx.ui.core.Widget,implement:qx.ui.form.renderer.IFormRenderer,construct:function(){arguments.callee.base.call(this);
var o=new qx.ui.layout.Grid();
o.setSpacing(6);
o.setColumnAlign(0,k,e);
this._setLayout(o);
...
},_createLabel:function(name,l){var m=g;

if(l.getRequired()){m=j;
}var n=new qx.ui.basic.Label(name+m+c);
n.setRich(true);
return n;
