...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.table.Table_Filtered_Model-qx.theme.Modern-0.js"]],
  boot : "boot",
...
(function(){var s="execute",r="A boolean",q="icon/22/actions/list-remove.png",p="A number",o="<b>HTML-Test <i>",n="icon/22/actions/edit-undo.png",m="between",l=", additional Status. All Values are shown.",k="Hide 1k-5k",j="Table with data filtering",H=", additional Status. 1k - 5k Values are hidden.",G="Show All",F=", additional Status. False Values are hidden.",E="Hide True",D="_tableModel",C="demobrowser.demo.table.Table_Filtered_Model",B=", additional Status. True Values are hidden.",A="!=",z="==",y="Hide False",w="ID",x="A date",u="HTML",v="</i></b>",t=", additional Status.";
qx.Class.define(C,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return j;
},createTable:function(){var c=this._tableModel=new qx.ui.table.model.Filtered();
c.setColumns([w,p,x,r,u]);
var d=[];
var h=new Date().getTime();
var a=400*24*60*60*1000;

for(var i=0;i<20;i++){var f=new Date(h+Math.random()*a-a/2);
var b=o+i+v;
d.push([i,Math.random()*10000,f,(Math.random()>0.5),b]);
}c.setData(d);
c.setColumnEditable(1,true);
c.setColumnEditable(2,true);
var g=this._table=new qx.ui.table.Table(c);
g.setMetaColumnCounts([1,-1]);
g.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
g.getTableColumnModel().setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
g.getTableColumnModel().setDataCellRenderer(4,new qx.ui.table.cellrenderer.Html());
g.setAdditionalStatusBarText(t);
return g;
},createControls:function(){var J=new qx.ui.toolbar.ToolBar();
var L,P,I;
P=new qx.ui.toolbar.Part();
J.add(P);
var R=this._table;
var N=this._tableModel;
var O=new qx.ui.toolbar.Button(E,q);
P.add(O);
O.addListener(s,function(e){N.addNumericFilter(z,1,r);
N.applyFilters();
R.setAdditionalStatusBarText(B);
});
var K=new qx.ui.toolbar.Button(y,q);
P.add(K);
K.addListener(s,function(e){N.addNumericFilter(A,1,r);
N.applyFilters();
R.setAdditionalStatusBarText(F);
});
var M=new qx.ui.toolbar.Button(k,q);
P.add(M);
M.addListener(s,function(e){N.addBetweenFilter(m,1000,5000,p);
N.applyFilters();
R.setAdditionalStatusBarText(H);
});
var Q=new qx.ui.toolbar.Button(G,n);
P.add(Q);
Q.addListener(s,function(e){N.resetHiddenRows();
R.setAdditionalStatusBarText(l);
});
return J;
...
(function(){var P='g',O="==",N=">",M="notregex",L="between",K="<",J="regex",I='gi',H="!between",G=">=",A="dataChanged",F="!=",D="<=",z="qx.ui.table.model.Filtered",y="Filters",C='',B="__qu",E="betweenAllowed",x="numericAllowed";
qx.Class.define(z,{extend:qx.ui.table.model.Simple,construct:function(){arguments.callee.base.call(this);
this.numericAllowed=new Array(O,F,N,K,G,D);
this.betweenAllowed=new Array(L,H);
this.__qt=false;
this.Filters=new Array();
},members:{__qu:null,__qt:null,_js_in_array:function(Q,R){var S=R.toString();

if(S==C){return false;
}var U=new RegExp(Q,P);
var T=U.test(R);
return T;
},addBetweenFilter:function(a,b,c,d){if(this._js_in_array(a,this.betweenAllowed)&&d!=null){if(b!=null&&c!=null){var e=new Array(a,b,c,d);
}}
if(e!=null){this.Filters.push(e);
}else{throw new Error("Filter not recognized or value1/value2 is null!");
}},addNumericFilter:function(bd,be,bf){var bg=null;

if(this._js_in_array(bd,this.numericAllowed)&&bf!=null){if(be!=null){bg=[bd,be,bf];
}}
if(bg!=null){this.Filters.push(bg);
}else{throw new Error("Filter not recognized: value or target is null!");
}},addRegex:function(f,g,h){var k;

if(h){k=I;
}else{k=P;
}
if(f!=null&&g!=null){var j=new Array(J,f,g,k);
}
...
if(X!=null&&Y!=null){var bb=new Array(M,X,Y,bc);
}
...
case G:if(q>=this.Filters[i][1]){u=true;
}break;
...
case H:if(q<this.Filters[i][1]&&q>this.Filters[i][2]){u=true;
}break;
...
}},resetHiddenRows:function(){if(!this.__qu){return ;
}this.Filters=[];
this.setData(qx.lang.Array.clone(this.__qu));
},setData:function(V,W){this.__qu=qx.lang.Array.clone(V);
this.Filters=[];
arguments.callee.base.call(this,V,W);
}},destruct:function(){this._disposeFields(B,x,E,y);
}});
})();
