...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.data.FormController", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.data.FormController-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var l="changeModel",k="qx.core.Object",j="_applyTarget",h="qx.data.controller.Form",g="changeTarget",f="qx.ui.form.Form",e="_applyModel";
...
}},properties:{model:{check:k,apply:e,event:l,nullable:true},target:{check:f,apply:j,event:g,nullable:true,init:null}},members:{__AC:null,__AA:null,__AB:null,createModel:function(A){var E=this.getTarget();
if(E==null){throw new Error("No target is set.");
}var B=E.getItems();
var D={};

for(var name in B){D[name]=null;
}var C=qx.data.marshal.Json.createModel(D,A);
this.setModel(C);
return C;
},_applyTarget:function(w,x){if(x!=null){this.__AE(x);
}if(this.getModel()==null){return;
}if(w!=null){this.__AD();
}},_applyModel:function(r,s){if(this.__AC!=null){this.__AC.setModel(r);
}if(this.getTarget()==null){return;
}if(r!=null){this.__AD();
}},__AD:function(){if(this.__AC==null){this.__AC=new qx.data.controller.Object(this.getModel());
}var t=this.getTarget().getItems();
for(var name in t){var v=t[name];
if(this.__AF(v)){var u=this.getModel();
this.__AA.push(u.bind(name,v,"modelSelection",{converter:function(y){return [y];
}}));
this.__AB[v.toHashCode()]=v.bind("selection",u,name,{converter:function(a){var b=a[0];

if(b!=null){return b.getModel();
}return null;
}});
}else{this.__AC.addTarget(v,"value",name,true);
}}},__AE:function(m){if(this.__AC==null){return;
}var n=m.getItems();
var p=this.getModel();
for(var i=0;i<this.__AA.length;i++){p.removeBinding(this.__AA[i]);
}this.__AA=[];
for(var name in n){var o=n[name];

if(this.__AF(o)){var q=this.__AB[o.toHashCode()];
o.removeBinding(q);
}else{this.__AC.removeTarget(o,"value",name);
}}this.__AB={};
},__AF:function(z){return qx.Class.hasInterface(z.constructor,qx.ui.core.ISingleSelection)&&qx.Class.hasInterface(z.constructor,qx.ui.form.IModelSelection);
}}});
})();
...
(function(){var h="changeModel",g="qx.core.Object",f="qx.data.controller.Object",e="__tW",d="__tV",c="reset",b="_applyModel";
qx.Class.define(f,{extend:qx.core.Object,construct:function(a){arguments.callee.base.call(this);
this.__tV={};
this.__tW=[];

if(a!=null){this.setModel(a);
}},properties:{model:{check:g,event:h,apply:b,nullable:true}},members:{__tW:null,__tV:null,_applyModel:function(B,C){for(var i=0;i<this.__tW.length;i++){var I=this.__tW[i][0];
var F=this.__tW[i][1];
var E=this.__tW[i][2];
var G=this.__tW[i][3];
var H=this.__tW[i][4];
var D=this.__tW[i][5];
if(C!=undefined){this.__tY(I,F,E,C);
}if(B!=undefined){this.__tX(I,F,E,G,H,D);
}else{I[c+qx.lang.String.firstUp(F)]();
}}},addTarget:function(J,K,L,M,N,O){this.__tW.push([J,K,L,M,N,O]);
this.__tX(J,K,L,M,N,O);
},__tX:function(j,k,l,m,n,o){if(this.getModel()==null){return;
...
if(this.__tV[r]==undefined){this.__tV[r]=[];
}this.__tV[r].push([p,q,k,l,n,o]);
},removeTarget:function(y,z,A){this.__tY(y,z,A,this.getModel());
for(var i=0;i<this.__tW.length;i++){if(this.__tW[i][0]==y&&this.__tW[i][1]==z&&this.__tW[i][2]==A){this.__tW.splice(i,1);
}}},__tY:function(s,t,u,v){if(!(s instanceof qx.core.Object)){return ;
}var w=this.__tV[s.toHashCode()];
if(w==undefined||w.length==0){return;
}for(var i=0;i<w.length;i++){if(w[i][2]==t&&w[i][3]==u){var x=w[i][0];
v.removeBinding(x);
if(w[i][1]!=null){s.removeBinding(w[i][1]);
}w.splice(i,1);
return;
}}}},destruct:function(){this._disposeFields(d,e);
}});
})();
(function(){var s='"',r="[",q="",p=",",o="{",n="]",m="get",l="null",k='\\"',j='\\n',c='\\b',h="qx.util.Serializer",f="}",b='\\\\',a='\\r',e='\\t',d='\\f',g='":';
qx.Class.define(h,{statics:{toUriParameter:function(C,D){var F=q;
var G=qx.util.PropertyUtil.getProperties(C.constructor);

for(var name in G){var E=C[m+qx.lang.String.firstUp(name)]();
if(qx.lang.Type.isArray(E)){for(var i=0;i<E.length;i++){F+=this.__AG(name,E[i],D);
}}else{F+=this.__AG(name,E,D);
}}return F.substring(0,F.length-1);
},__AG:function(name,t,u){if(t instanceof qx.core.Object&&u!=null){var v=encodeURIComponent(u(t));

if(v===undefined){var v=encodeURIComponent(t);
}}else{var v=encodeURIComponent(t);
}return encodeURIComponent(name)+"="+v+"&";
},toJson:function(w,x){var z=q;
if(w==null){return l;
}else if(qx.Class.hasInterface(w.constructor,qx.data.IListData)){z+=r;

for(var i=0;i<w.getLength();i++){z+=qx.util.Serializer.toJson(w.getItem(i),x)+p;
}
if(z!=r){z=z.substring(0,z.length-1);
}return z+n;
}else if(qx.lang.Type.isArray(w)){z+=r;

for(var i=0;i<w.length;i++){z+=qx.util.Serializer.toJson(w[i],x)+p;
}
if(z!=r){z=z.substring(0,z.length-1);
}return z+n;
}else if(w instanceof qx.core.Object){if(x!=null){var A=x(w);
if(A!=undefined){return s+A+s;
}}z+=o;
var B=qx.util.PropertyUtil.getProperties(w.constructor);

for(var name in B){if(B[name].group!=undefined){continue;
}var y=w[m+qx.lang.String.firstUp(name)]();
z+=s+name+g+qx.util.Serializer.toJson(y,x)+p;
}
if(z!=o){z=z.substring(0,z.length-1);
}return z+f;
}else if(qx.lang.Type.isString(w)){w=w.replace(/([\\])/g,b);
w=w.replace(/(["])/g,k);
w=w.replace(/([\r])/g,a);
w=w.replace(/([\f])/g,d);
w=w.replace(/([\n])/g,j);
...
w=w.replace(/([\b])/g,c);
return s+w+s;
}else if(qx.lang.Type.isDate(w)||qx.lang.Type.isRegExp(w)){return s+w+s;
}return w+q;
}}});
})();
