...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.bom.Client", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.bom.Client-qx.theme.Classic-0.js"]],
  boot : "boot",
...
t.add(c);
var q=[qx.bom.client.Engine,qx.bom.client.System,qx.bom.client.Platform,qx.bom.client.Feature,qx.bom.client.Locale,qx.bom.client.Flash];
...
(function(){function q(j){if(qx.core.Variant.isSet(d,h)){p.prototype={length:0,$$isArray:true};
var m=g.split(c);

for(var length=m.length;length;){p.prototype[m[--length]]=Array.prototype[m[length]];
}}var n=Array.prototype.slice;
p.prototype.concat=function(){var b=this.slice(0);

for(var i=0,length=arguments.length;i<length;i++){var a;

if(arguments[i] instanceof p){a=n.call(arguments[i],0);
}else if(arguments[i] instanceof Array){a=arguments[i];
}else{a=[arguments[i]];
}b.push.apply(b,a);
}return b;
};
p.prototype.toString=function(){return n.call(this,0).toString();
};
p.prototype.toLocaleString=function(){return n.call(this,0).toLocaleString();
};
...
var k=qx.lang.Core.arrayFilter;
var l=qx.lang.Core.arrayMap;
p.prototype.filter=function(){var s=new this.constructor;
s.push.apply(s,k.apply(this,arguments));
return s;
};
p.prototype.map=function(){var u=new this.constructor;
...
p.prototype.splice=function(){var r=new this.constructor;
...
return p;
}function p(length){if(arguments.length===1&&typeof length===f){this.length=-1<length&&length===length>>.5?length:this.push(length);
...
(function(){var p="$1",o="Shockwave Flash",n=".",m="undefined",l="object",k="boolean",j=",",h="$version",g="always",f="ShockwaveFlash.ShockwaveFlash.6",c="qx.bom.client.Flash",e="ShockwaveFlash.ShockwaveFlash.7",d="qx.client",b="ShockwaveFlash.ShockwaveFlash";
qx.Bootstrap.define(c,{statics:{AVAILABLE:false,FULLVERSION:"0.0.0",REVISION:"0",VERSION:0.0,EXPRESSINSTALL:false,STRICT_SECURITY_MODEL:false,_cachedSupportsVersion:{},supportsVersion:function(q){if(typeof this._cachedSupportsVersion[q]===k){return this._cachedSupportsVersion[q];
}else{var t=q.split(n);
var s=this.FULLVERSION.split(n);

for(var i=0;i<t.length;i++){var r=parseInt(s[i])-parseInt(t[i]);

if(r>0){return (this._cachedSupportsVersion[q]=true);
}else if(r<0){return (this._cachedSupportsVersion[q]=false);
}}return (this._cachedSupportsVersion[q]=true);
}},__xz:qx.core.Variant.select(d,{"mshtml":function(){if(!window.ActiveXObject){return;
}var w=[0,0,0];
var v=false;

try{var x=new ActiveXObject(e);
}catch(D){try{var x=new ActiveXObject(f);
w=[6,0,21];
x.AllowScriptAccess=g;
}catch(E){if(w[0]==6){v=true;
}}
if(!v){try{x=new ActiveXObject(b);
}catch(a){}}}
if(!v&&typeof x==l){var u=x.GetVariable(h);

if(typeof u!=m){u=u.split(" ")[1].split(j);
w[0]=parseInt(u[0],10);
w[1]=parseInt(u[1],10);
w[2]=parseInt(u[2],10);
}}this.__xA(w);
},"default":function(){if(!navigator.plugins||typeof navigator.plugins[o]!==l){return;
}var y=[0,0,0];
var z=navigator.plugins[o].description;

if(typeof z!=m){z=z.replace(/^.*\s+(\S+\s+\S+$)/,p);
y[0]=parseInt(z.replace(/^(.*)\..*$/,p),10);
y[1]=parseInt(z.replace(/^.*\.(.*)\s.*$/,p),10);
y[2]=/r/.test(z)?parseInt(z.replace(/^.*r(.*)$/,p),10):0;
}this.__xA(y);
}}),__xA:function(A){this.FULLVERSION=A.join(".");
...
qx.Bootstrap.define(p,{statics:{__xB:{quicktime:{plugin:l,control:o},wmv:{plugin:s,control:q},divx:{plugin:h,control:j},silverlight:{plugin:t,control:f}},has:function(w,x){if(this.__xB[w]){var y=this.__xB[w];
if(y.installed){if(x==null||(y.version>=parseFloat(x))){return true;
}}}return false;
},__xC:qx.core.Variant.select(n,{"mshtml":function(){var a=window.ActiveXObject;
...
}switch(e){case m:b.version=0;
break;
case r:b.version=d.versionInfo;
break;
case g:b.version=0;
break;
case k:try{b.version=d.version===undefined?0:d.version;
}catch(v){try{b.version=d.settings.version===undefined?0:d.settings.version;
}catch(H){b.version=0;
}}break;
}b.installed=true;
