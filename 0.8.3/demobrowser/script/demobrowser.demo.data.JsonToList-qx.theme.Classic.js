...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.data.JsonToList", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.data.JsonToList-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var q="icon/16/mimetypes/",p="name",o="model.items",n="Loading...",m=" will be created by a converter which converts the type to an icon url.",l=".png",k="model",j="state",i="json/list.json",h="type",d="list.json</a> and bind the items to the list widget. The icons ",g="' target='_blank'>",f="demobrowser.demo.data.JsonToList",c="<b>List bound to data in a json file</b><br/>",b="Loading the json file <a href='",e="value";
...
u.setLabelPath(p);
u.setIconOptions({converter:function(a){return q+a+l;
}});
u.setIconPath(h);
var s=i;
var v=new qx.data.store.Json(s);
var status=new qx.ui.basic.Label(n);
...
(function(){var g="changeSelection",f="change",e="qx.data.Array",d="qx.data.controller.MSelection",c="_applySelection",b="target";
qx.Mixin.define(d,{construct:function(){if(!qx.Class.hasProperty(this.constructor,b)){throw new Error("Target property is needed.");
}this.setSelection(new qx.data.Array());
},properties:{selection:{check:e,event:g,apply:c,init:null}},members:{_modifingSelection:0,__zq:null,__zr:null,_applySelection:function(h,j){if(this.__zr!=undefined&&j!=undefined){j.removeListenerById(this.__zr);
}this.__zr=h.addListener(f,this.__zs,this);
},__zs:function(){this._updateSelection();
},__zt:function(){if(this._inSelectionModification()||this.getTarget()==null){return;
}
if(this.__zu()){var s=this.getTarget().getSelection();
}else if(this.__zv()){var s=this.getTarget().getSelection();
}for(var i=0;i<s.length;i++){var r=s[i].getModel();

if(!this.getSelection().contains(r)){this.getSelection().splice(this.getSelection().length,0,r);
}}var t=[];
...
},_addChangeTargetListener:function(k,l){if(this.__zq!=undefined&&l!=undefined){l.removeListenerById(this.__zq);
}if(this.__zu()||this.__zv()){this.__zq=k.addListener(g,this.__zt,this);
}},_updateSelection:function(){this._startSelectionModification();
if(this.__zu()){this.getTarget().resetSelection();
for(var i=0;i<this.getSelection().length;i++){this.__zw(this.getSelection().getItem(i));
}var m=this.getTarget().getSelection();
var n=[];

for(var i=0;i<m.length;i++){n[i]=m[i].getModel();
...
}}}else if(this.__zv()){this.__zw(this.getSelection().getItem(this.getSelection().length-1));
}this._endSelectionModification();
},__zu:function(){var a=this.getTarget().constructor;
return qx.Class.implementsInterface(a,qx.ui.core.IMultiSelection);
},__zv:function(){var o=this.getTarget().constructor;
return qx.Class.implementsInterface(o,qx.ui.core.ISingleSelection);
},__zw:function(p){var q=this.getTarget().getSelectables();
for(var i=0;i<q.length;i++){if(q[i].getModel()==p){if(this.__zu()){this.getTarget().addToSelection(q[i]);
}else if(this.__zv()){this.getTarget().setSelection([q[i]]);
}return;
...
(function(){var bx="BindingId",bw="String",bv="changeModel",bu="_applyLabelOptions",bt="_applyLabelPath",bs="changeTarget",br="changeLength",bq="_applyModel",bp="__zy",bo="change",bL="icon",bK="qx.data.controller.List",bJ="_applyIconPath",bI="_applyDelegate",bH="changeDelegate",bG="_applyTarget",bF="]",bE="qx.data.IListData",bD="model[",bC="label",bA="_applyIconOptions",bB=".",by="__zx",bz="__zz";
qx.Class.define(bK,{extend:qx.core.Object,include:qx.data.controller.MSelection,construct:function(a,b,c){arguments.callee.base.call(this);
this.__zx=[];
this.__zy=[];
this.__zz={};
...
}},properties:{model:{check:bE,apply:bq,event:bv,nullable:true},target:{apply:bG,event:bs,nullable:true,init:null},labelPath:{check:bw,apply:bt,nullable:true},iconPath:{check:bw,apply:bJ,nullable:true},labelOptions:{apply:bu,nullable:true},iconOptions:{apply:bA,nullable:true},delegate:{apply:bI,event:bH,init:null,nullable:true}},members:{__zA:null,__zB:null,__zx:null,__zz:null,__zy:null,update:function(){this.__zH();
this.__zD();
this.__zG();
...
},_applyIconOptions:function(C,D){this.__zG();
},_applyLabelOptions:function(V,W){this.__zG();
},_applyIconPath:function(bg,bh){this.__zG();
},_applyLabelPath:function(d,e){this.__zG();
},_applyModel:function(t,u){if(u!=undefined){if(this.__zA!=undefined){u.removeListenerById(this.__zA);
}
if(this.__zB!=undefined){u.removeListenerById(this.__zB);
}}if(t!=null){this.__zA=t.addListener(br,this.__zD,this);
this.__zB=t.addListener(bo,this.__zC,this);
this.__zH();
this.__zD();
if(this.getTarget()!=null){var w=this.getModel();
var x=this.getTarget().getChildren();

for(var i=0,l=this.__zx.length;i<l;i++){var y=w.getItem(this.__zI(i));
var v=x[i];
v.setModel(y);
}}}if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length);
}},_applyTarget:function(X,Y){if(Y!=undefined){Y.removeAll();
this.removeAllBindings();
}
if(X!=null){if(this.getModel()!=null){for(var i=0;i<this.__zx.length;i++){this.__zE(this.__zI(i));
}}this._addChangeTargetListener(X,Y);
}},__zC:function(){for(var i=this.getSelection().length-1;i>=0;i--){if(!this.getModel().contains(this.getSelection().getItem(i))){this.getSelection().splice(i,1);
}}qx.ui.core.queue.Widget.add(this);
},syncWidget:function(){this._updateSelection();
},__zD:function(){if(this.getTarget()==null){return;
}this.__zH();
var F=this.__zx.length;
var E=this.getTarget().getChildren().length;
if(F>E){for(var j=E;j<F;j++){this.__zE(this.__zI(j));
}}else if(F<E){for(var j=E;j>F;j--){this.__zF();
...
},__zE:function(k){var m=this._createItem();
m.setModel(this.getModel().getItem(this.__zI(k))||null);
this._bindListItem(m,k);
this.getTarget().add(m);
},__zF:function(){this._startSelectionModification();
...
}else{this.bindProperty(this.getLabelPath(),bC,this.getLabelOptions(),S,T);
if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),bL,this.getIconOptions(),S,T);
}}},bindProperty:function(G,H,I,J,K){J.setModel(this.getModel().getItem(K));
if(I!=null){var I=qx.lang.Object.clone(I);
this.__zz[H]=I.onUpdate;
delete I.onUpdate;
}else{I={};
this.__zz[H]=null;
}I.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,K);
var L=bD+K+bF;

if(G!=null){L+=bB+G;
}var M=this.bind(L,J,H,I);
J.setUserData(H+bx,M);
if(!qx.lang.Array.contains(this.__zy,H)){this.__zy.push(H);
}},_onBindingSet:function(bi,bj,bk){if(this.getModel()==null||this._inSelectionModification()){return;
}for(var i=0;i<this.__zy.length;i++){if(this.__zz[this.__zy[i]]!=null){this.__zz[this.__zy[i]]();
}}var bl=this.getModel().getItem(this.__zI(bi));
bk.setModel(bl||null);
if(this.getSelection()!=null){this._updateSelection();
}},_removeBindingsFrom:function(be){for(var i=0;i<this.__zy.length;i++){var bf=be.getUserData(this.__zy[i]+bx);

if(bf!=null){this.removeBinding(bf);
}}},__zG:function(){if(this.getTarget()==null){return;
}var f=this.getTarget().getChildren();
for(var i=0;i<f.length;i++){this._removeBindingsFrom(f[i]);
this._bindListItem(f[i],this.__zI(i));
...
}}},_setBindItem:function(bm,bn){if(bm!=null&&bm.bindItem!=null){if(bn!=null&&bn.bindItem!=null&&bm.bindItem==bn.bindItem){return;
}this.__zG();
}},_setCreateItem:function(z,A){if(this.getTarget()==null||this.getModel()==null||z==null||z.createItem==null){return;
}this._startSelectionModification();
var B=this.getTarget().getChildren();

for(var i=0,l=B.length;i<l;i++){this._removeBindingsFrom(B[i]);
}this.getTarget().removeAll();
this.update();
this._endSelectionModification();
this._updateSelection();
},_setFilter:function(n,o){if(this.getTarget()==null||this.getModel()==null||n==null||n.filter==null){return;
}this._startSelectionModification();
...
this.__zH();
if(q.length>this.__zx.length){for(var j=q.length;j>this.__zx.length;j--){this.getTarget().removeAt(j-1);
}}else if(q.length<this.__zx.length){for(var j=q.length;j<this.__zx.length;j++){var r=this._createItem();
this.getTarget().add(r);
}}var p=this.getTarget().getChildren();

for(var i=0;i<p.length;i++){this._bindListItem(p[i],this.__zI(i));
}this._endSelectionModification();
this._updateSelection();
},__zH:function(){var Q=this.getModel();
...
for(var i=0;i<Q.getLength();i++){if(P==null||P(Q.getItem(i))){this.__zx.push(i);
}}},__zI:function(ba){return this.__zx[ba];
}},destruct:function(){this._disposeFields(by,bz,bp);
}});
})();
...
(function(){var v="configured",u="completed",t="changeState",s="changeModel",r="qx.data.store.Json",q="GET",p="_marshaler",o="sending",n="application/json",m="changeUrl",f="failed",l="loaded",i="timeout",e="__zK",d="queued",h="String",g="aborted",j="_applyUrl",c="receiving",k="qx.event.type.Data";
qx.Class.define(r,{extend:qx.core.Object,construct:function(z,A){arguments.callee.base.call(this);
this._marshaler=new qx.data.marshal.Json(A);

if(z!=null){this.setUrl(z);
}},events:{"loaded":k},properties:{model:{nullable:true,event:s},state:{check:[v,d,o,c,u,g,i,f],init:v,event:t},url:{check:h,apply:j,event:m}},members:{__zK:null,_applyUrl:function(x,y){if(x!=null){this._createRequest(x);
}},_createRequest:function(b){this.__zK=new qx.io.remote.Request(b,q,n);
this.__zK.addListener(u,this.__zL,this);
this.__zK.addListener(t,function(w){this.setState(w.getData());
},this);
this.__zK.send();
},__zL:function(B){var C=B.getContent();
...
}}},destruct:function(){this._disposeObjects(p,e);
}});
})();
(function(){var c="qx.data.marshal.IMarshaler";
qx.Interface.define(c,{members:{toClass:function(a,b){},toModel:function(d){}}});
})();
(function(){var j="qx.data.model.",h='"',g="change",f="qx.data.marshal.Json",e="__zM",d="Please use toClass instead.",c="Please use toModel instead.",b="set",a="_applyEventPropagation";
qx.Class.define(f,{extend:qx.core.Object,implement:[qx.data.marshal.IMarshaler],construct:function(H){arguments.callee.base.call(this);
this.__zM=H;
},statics:{__zN:null,createModel:function(C,D){if(this.__zN===null){this.__zN=new qx.data.marshal.Json();
}this.__zN.toClass(C,D);
return this.__zN.toModel(C);
}},members:{__zM:null,jsonToClass:function(I,J){qx.log.Logger.deprecatedMethodWarning(arguments.callee,d);
this.toClass(I,J);
},jsonToModel:function(k){qx.log.Logger.deprecatedMethodWarning(arguments.callee,c);
this.toModel(k);
},__zO:function(E){var F=[];

for(var G in E){F.push(G);
}return F.sort().join(h);
},toClass:function(q,r){if(qx.lang.Type.isNumber(q)||qx.lang.Type.isString(q)||qx.lang.Type.isBoolean(q)||q==null){return;
}if(qx.lang.Type.isArray(q)){for(var i=0;i<q.length;i++){this.toClass(q[i],r);
}return ;
}var t=this.__zO(q);
if(this.__zM&&this.__zM.getModelClass&&this.__zM.getModelClass(t)!=null){return;
}for(var x in q){this.toClass(q[x],r);
}if(qx.Class.isDefined(j+t)){return;
}var y={};

for(var x in q){y[x]={};
y[x].nullable=true;
y[x].event=g+qx.lang.String.firstUp(x);

if(r){y[x].apply=a;
}}if(this.__zM&&this.__zM.getModelSuperClass){var w=this.__zM.getModelSuperClass(t)||qx.core.Object;
}else{var w=qx.core.Object;
}var u=[];

if(this.__zM&&this.__zM.getModelMixins){var v=this.__zM.getModelMixins(t);
if(!qx.lang.Type.isArray(v)){if(v!=null){u=[v];
}}}if(r){u.push(qx.data.marshal.MEventBubbling);
}var s={extend:w,include:u,properties:y};
qx.Class.define(j+t,s);
},__zP:function(z){var A;
if(this.__zM&&this.__zM.getModelClass){A=this.__zM.getModelClass(z);
}
if(A!=null){return (new A());
}else{var B=qx.Class.getByName(j+z);
return (new B());
...
}else if(qx.lang.Type.isArray(l)){var m=new qx.data.Array();

for(var i=0;i<l.length;i++){m.push(this.toModel(l[i]));
}return m;
}else if(qx.lang.Type.isObject(l)){var n=this.__zO(l);
var o=this.__zP(n);
for(var p in l){o[b+qx.lang.String.firstUp(p)](this.toModel(l[p]));
}return o;
}throw new Error("Unsupported type!");
}},destruct:function(){this._disposeFields(e);
}});
})();
(function(){var J="Boolean",I="qx.event.type.Event",H="queued",G="String",F="sending",E="qx.io.remote.Response",D="receiving",C="aborted",B="failed",A="nocache",bs="completed",br="POST",bq="configured",bp="timeout",bo="GET",bn="Pragma",bm="no-url-params-on-post",bl="no-cache",bk="Cache-Control",bj="Content-Type",Q="text/plain",R="application/xml",O="__zR",P="application/json",M="text/html",N="application/x-www-form-urlencoded",K="qx.io.remote.Exchange",L="__zT",S="Integer",T="X-Qooxdoo-Response-Type",bb="HEAD",Y="qx.io.remote.Request",bd="__zS",bc="_applyResponseType",bf="_applyState",be="text/javascript",V="changeState",bi="PUT",bh="_applyProhibitCaching",bg="",U="__zQ",W="_applyMethod",X="DELETE",ba="boolean";
qx.Class.define(Y,{extend:qx.core.Object,construct:function(bt,bu,bv){arguments.callee.base.call(this);
this.__zQ={};
this.__zR={};
this.__zS={};
this.__zT={};
...
this.__zU=++qx.io.remote.Request.__zU;
},events:{"created":I,"configured":I,"sending":I,"receiving":I,"completed":E,"aborted":E,"failed":E,"timeout":E},statics:{__zU:0},properties:{url:{check:G,init:bg},method:{check:[bo,br,bi,bb,X],apply:W,init:bo},asynchronous:{check:J,init:true},data:{check:G,nullable:true},username:{check:G,nullable:true},password:{check:G,nullable:true},state:{check:[bq,H,F,D,bs,C,bp,B],init:bq,apply:bf,event:V},responseType:{check:[Q,be,P,R,M],init:Q,apply:bc},timeout:{check:S,nullable:true},prohibitCaching:{check:function(v){return typeof v==ba||v===bm;
},init:true,apply:bh},crossDomain:{check:J,init:false},fileUpload:{check:J,init:false},transport:{check:K,nullable:true},useBasicHttpAuth:{check:J,init:false}},members:{__zQ:null,__zR:null,__zS:null,__zT:null,__zU:null,send:function(){qx.io.remote.RequestQueue.getInstance().add(this);
},abort:function(){qx.io.remote.RequestQueue.getInstance().abort(this);
},reset:function(){switch(this.getState()){case F:case D:this.error("Aborting already sent request!");
case H:this.abort();
break;
}},isConfigured:function(){return this.getState()===bq;
},isQueued:function(){return this.getState()===H;
},isSending:function(){return this.getState()===F;
},isReceiving:function(){return this.getState()===D;
},isCompleted:function(){return this.getState()===bs;
...
},__zV:function(e){var f=e.clone();
f.setTarget(this);
this.dispatchEvent(f);
},_onqueued:function(e){this.setState(H);
this.__zV(e);
},_onsending:function(e){this.setState(F);
this.__zV(e);
},_onreceiving:function(e){this.setState(D);
this.__zV(e);
},_oncompleted:function(e){this.setState(bs);
this.__zV(e);
this.dispose();
...
},_applyProhibitCaching:function(a,b){if(!a){this.removeParameter(A);
this.removeRequestHeader(bn);
this.removeRequestHeader(bk);
return;
}if(a!==bm||this.getMethod()!=br){this.setParameter(A,new Date().valueOf());
}else{this.removeParameter(A);
}this.setRequestHeader(bn,bl);
this.setRequestHeader(bk,bl);
},_applyMethod:function(q,r){if(q===br){this.setRequestHeader(bj,N);
...
},setRequestHeader:function(w,x){this.__zQ[w]=x;
},removeRequestHeader:function(z){delete this.__zQ[z];
},getRequestHeader:function(i){return this.__zQ[i]||null;
},getRequestHeaders:function(){return this.__zQ;
},setParameter:function(n,o,p){if(p){this.__zS[n]=o;
}else{this.__zR[n]=o;
}},removeParameter:function(l,m){if(m){delete this.__zS[l];
}else{delete this.__zR[l];
}},getParameter:function(t,u){if(u){return this.__zS[t]||null;
}else{return this.__zR[t]||null;
}},getParameters:function(k){return (k?this.__zS:this.__zR);
},setFormField:function(bw,bx){this.__zT[bw]=bx;
},removeFormField:function(y){delete this.__zT[y];
},getFormField:function(j){return this.__zT[j]||null;
},getFormFields:function(){return this.__zT;
},getSequenceNumber:function(){return this.__zU;
}},destruct:function(){this.setTransport(null);
this._disposeFields(U,O,bd,L);
}});
})();
...
(function(){var F="Integer",E="_on",D="_applyEnabled",C="__zW",B="Boolean",A="__Aa",z="sending",y="interval",x="__zX",w="failed",r="qx.io.remote.RequestQueue",v="timeout",u="completed",q="queued",p="aborted",t="receiving",s="singleton";
qx.Class.define(r,{type:s,extend:qx.core.Object,construct:function(){arguments.callee.base.call(this);
this.__zW=[];
this.__zX=[];
this.__zY=0;
this.__Aa=new qx.event.Timer(500);
this.__Aa.addListener(y,this._oninterval,this);
},properties:{enabled:{init:true,check:B,apply:D},maxTotalRequests:{check:F,nullable:true},maxConcurrentRequests:{check:F,init:qx.bom.client.Transport.getMaxConcurrentRequestCount()},defaultTimeout:{check:F,init:5000}},members:{__zW:null,__zX:null,__zY:null,__Aa:null,getRequestQueue:function(){return this.__zW;
},getActiveQueue:function(){return this.__zX;
},_debug:function(){var n;
{};
},_check:function(){this._debug();
if(this.__zX.length==0&&this.__zW.length==0){this.__Aa.stop();
}if(!this.getEnabled()){return;
}if(this.__zW.length==0||(this.__zW[0].isAsynchronous()&&this.__zX.length>=this.getMaxConcurrentRequests())){return;
}if(this.getMaxTotalRequests()!=null&&this.__zY>=this.getMaxTotalRequests()){return;
}var G=this.__zW.shift();
var H=new qx.io.remote.Exchange(G);
this.__zY++;
this.__zX.push(H);
this._debug();
H.addListener(z,this._onsending,this);
H.addListener(t,this._onreceiving,this);
H.addListener(u,this._oncompleted,this);
H.addListener(p,this._oncompleted,this);
H.addListener(v,this._oncompleted,this);
H.addListener(w,this._oncompleted,this);
H._start=(new Date).valueOf();
H.send();
if(this.__zW.length>0){this._check();
}},_remove:function(o){qx.lang.Array.remove(this.__zX,o);
o.dispose();
this._check();
},__Ab:0,_onsending:function(e){{};
...
},_oninterval:function(e){var k=this.__zX;

if(k.length==0){this.__Aa.stop();
return;
...
for(var i=k.length-1;i>=0;i--){h=k[i];
f=h.getRequest();
...
if(c>g){this.warn("Timeout: transport "+h.toHashCode());
this.warn(c+"ms > "+g+"ms");
h.timeout();
}}}},_applyEnabled:function(l,m){if(l){this._check();
}this.__Aa.setEnabled(l);
},add:function(K){K.setState(q);

if(K.isAsynchronous()){this.__zW.push(K);
}else{this.__zW.unshift(K);
}this._check();

if(this.getEnabled()){this.__Aa.start();
}},abort:function(a){var b=a.getTransport();

if(b){b.abort();
}else if(qx.lang.Array.contains(this.__zW,a)){qx.lang.Array.remove(this.__zW,a);
}}},destruct:function(){this._disposeArray(x);
this._disposeObjects(A);
this._disposeFields(C);
}});
})();
(function(){var y="failed",x="sending",w="completed",v="receiving",u="aborted",t="timeout",s="qx.io.remote.Response",r="Connection dropped",q="configured",p="qx.event.type.Event",bu="Proxy authentication required",bt="qx.io.remote.transport.Abstract",bs="MSHTML-specific HTTP status code",br="Not available",bq="Precondition failed",bp="Server error",bo="Moved temporarily",bn="qx.io.remote.Exchange",bm="Bad gateway",bl="Gone",F="See other",G="Partial content",D="Server timeout",E="qx.io.remote.transport.Script",B="HTTP version not supported",C="Unauthorized",z="Multiple choices",A="Payment required",J="Not implemented",K="Request-URL too large",S="Length required",Q="_applyState",bb="changeState",V="Not modified",bh="qx.io.remote.Request",bf="Connection closed by server",M="Moved permanently",bk="_applyImplementation",bj="Method not allowed",bi="Forbidden",L="Use proxy",O="Ok",P="Not found",R="Not acceptable",T="Request time-out",W="Bad request",bc="Conflict",bg="No content",H="qx.io.remote.transport.XmlHttp",I="qx.io.remote.transport.Iframe",N="Request entity too large",ba="Unknown status code",Y="Unsupported media type",X="Gateway time-out",be="created",bd="Out of resources",U="undefined";
qx.Class.define(bn,{extend:qx.core.Object,construct:function(d){arguments.callee.base.call(this);
this.setRequest(d);
d.setTransport(this);
},events:{"sending":p,"receiving":p,"completed":s,"aborted":s,"failed":s,"timeout":s},statics:{typesOrder:[H,I,E],typesReady:false,typesAvailable:{},typesSupported:{},registerType:function(bz,bA){qx.io.remote.Exchange.typesAvailable[bA]=bz;
...
}},canHandle:function(bJ,bK,bL){if(!qx.lang.Array.contains(bJ.handles.responseTypes,bL)){return false;
}
for(var bM in bK){if(!bJ.handles[bM]){return false;
}}return true;
},_nativeMap:{0:be,1:q,2:x,3:v,4:w},wasSuccessful:function(bv,bw,bx){if(bx){switch(bv){case null:case 0:return true;
case -1:return bw<4;
default:return typeof bv===U;
...
return false;
}}},statusCodeToString:function(bI){switch(bI){case -1:return br;
case 200:return O;
case 304:return V;
case 206:return G;
case 204:return bg;
...
default:return ba;
}}},properties:{request:{check:bh,nullable:true},implementation:{check:bt,nullable:true,apply:bk},state:{check:[q,x,v,w,u,t,y],init:q,event:bb,apply:Q}},members:{send:function(){var j=this.getRequest();

if(!j){return this.error("Please attach a request object first");
}qx.io.remote.Exchange.initTypes();
var g=qx.io.remote.Exchange.typesOrder;
var f=qx.io.remote.Exchange.typesSupported;
var m=j.getResponseType();
var n={};

if(j.getAsynchronous()){n.asynchronous=true;
}else{n.synchronous=true;
}
if(j.getCrossDomain()){n.crossDomain=true;
}
if(j.getFileUpload()){n.fileUpload=true;
}for(var k in j.getFormFields()){n.programaticFormFields=true;
break;
}var o,h;

for(var i=0,l=g.length;i<l;i++){o=f[g[i]];
...
h.setUseBasicHttpAuth(j.getUseBasicHttpAuth());
h.send();
return true;
}catch(bN){this.error("Request handler throws error");
this.error(bN);
return;
}}}this.error("There is no transport implementation available to handle this request: "+j);
...
},__Ac:function(){var c=this.getRequest();
...
},_applyImplementation:function(bO,bP){if(bP){bP.removeListener(x,this._onsending,this);
bP.removeListener(v,this._onreceiving,this);
bP.removeListener(w,this._oncompleted,this);
bP.removeListener(u,this._onabort,this);
bP.removeListener(t,this._ontimeout,this);
bP.removeListener(y,this._onfailed,this);
}
if(bO){var bQ=this.getRequest();
bO.setUrl(bQ.getUrl());
bO.setMethod(bQ.getMethod());
bO.setAsynchronous(bQ.getAsynchronous());
bO.setUsername(bQ.getUsername());
bO.setPassword(bQ.getPassword());
bO.setParameters(bQ.getParameters());
bO.setFormFields(bQ.getFormFields());
bO.setRequestHeaders(bQ.getRequestHeaders());
bO.setData(bQ.getData());
bO.setResponseType(bQ.getResponseType());
bO.addListener(x,this._onsending,this);
bO.addListener(v,this._onreceiving,this);
bO.addListener(w,this._oncompleted,this);
bO.addListener(u,this._onabort,this);
bO.addListener(t,this._ontimeout,this);
bO.addListener(y,this._onfailed,this);
}},_applyState:function(bD,bE){{};

switch(bD){case x:this.fireEvent(x);
break;
case v:this.fireEvent(v);
break;
case w:case u:case t:case y:var bG=this.getImplementation();

if(!bG){break;
}this.__Ac();

if(this.hasListener(bD)){var bH=qx.event.Registration.createEvent(bD,qx.io.remote.Response);

if(bD==w){var bF=bG.getResponseContent();
bH.setContent(bF);
if(bF===null){{};
bD=y;
}}else if(bD==y){bH.setContent(bG.getResponseContent());
}bH.setStatusCode(bG.getStatusCode());
bH.setResponseHeaders(bG.getResponseHeaders());
this.dispatchEvent(bH);
...
(function(){var p="failed",o="completed",n="=",m="aborted",k="",j="sending",h="&",g="configured",f="timeout",d="application/xml",M="qx.io.remote.transport.XmlHttp",L="application/json",K="text/html",J="qx.client",I="receiving",H="text/plain",G="text/javascript",F="?",E="created",D='Referer',x='Basic ',y="\n</pre>",v="string",w='Authorization',t="<pre>Could not execute json: \n",u="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",q=':',r="parseerror",z="file:",A="webkit",C="__Af",B="object";
qx.Class.define(M,{extend:qx.io.remote.transport.Abstract,statics:{handles:{synchronous:true,asynchronous:true,crossDomain:false,fileUpload:false,programaticFormFields:false,responseTypes:[H,G,L,d,K]},requestObjects:[],requestObjectCount:0,createRequestObject:qx.core.Variant.select(J,{"default":function(){return new XMLHttpRequest;
},"mshtml":function(){if(window.ActiveXObject&&qx.xml.Document.XMLHTTP){return new ActiveXObject(qx.xml.Document.XMLHTTP);
}
if(window.XMLHttpRequest){return new XMLHttpRequest;
}}}),isSupported:function(){return !!this.createRequestObject();
}},members:{__Ad:false,__Ae:0,__Af:null,getRequest:function(){if(this.__Af===null){this.__Af=qx.io.remote.transport.XmlHttp.createRequestObject();
this.__Af.onreadystatechange=qx.lang.Function.bind(this._onreadystatechange,this);
}return this.__Af;
},send:function(){this.__Ae=0;
var bL=this.getRequest();
var bH=this.getMethod();
var bO=this.getAsynchronous();
var bN=this.getUrl();
var bJ=(window.location.protocol===z&&!(/^http(s){0,1}\:/.test(bN)));
this.__Ad=bJ;
var bR=this.getParameters(false);
var bP=[];

for(var bI in bR){var bM=bR[bI];

if(bM instanceof Array){for(var i=0;i<bM.length;i++){bP.push(encodeURIComponent(bI)+n+encodeURIComponent(bM[i]));
}}else{bP.push(encodeURIComponent(bI)+n+encodeURIComponent(bM));
}}
if(bP.length>0){bN+=(bN.indexOf(F)>=0?h:F)+bP.join(h);
}if(this.getData()===null){var bR=this.getParameters(true);
var bP=[];
...
if(bP.length>0){this.setData(bP.join(h));
}}var bQ=function(bb){var bg=u;
var bk=k;
var be,bd,bc;
var bh,bi,bj,bf;
var i=0;
...
try{if(this.getUsername()){if(this.getUseBasicHttpAuth()){bL.open(bH,bN,bO);
bL.setRequestHeader(w,x+bQ(this.getUsername()+q+this.getPassword()));
}else{bL.open(bH,bN,bO,this.getUsername(),this.getPassword());
}}else{bL.open(bH,bN,bO);
}}catch(a){this.error("Failed with exception: "+a);
this.failed();
return;
}if(!qx.core.Variant.isSet(J,A)){bL.setRequestHeader(D,window.location.href);
}var bK=this.getRequestHeaders();

for(var bI in bK){bL.setRequestHeader(bI,bK[bI]);
}try{{};
bL.send(this.getData());
}catch(bz){if(bJ){this.failedLocally();
}else{this.error("Failed to send data: "+bz,"send");
this.failed();
}return;
}if(!bO){this._onreadystatechange();
}},failedLocally:function(){if(this.getState()===p){return;
}this.warn("Could not load from file: "+this.getUrl());
this.failed();
},_onreadystatechange:qx.event.GlobalError.observeMethod(function(e){switch(this.getState()){case o:case m:case p:case f:{};
return;
}var bG=this.getReadyState();

if(bG==4){if(!qx.io.remote.Exchange.wasSuccessful(this.getStatusCode(),bG,this.__Ad)){if(this.getState()===g){this.setState(j);
}return this.failed();
}}while(this.__Ae<bG){this.setState(qx.io.remote.Exchange._nativeMap[++this.__Ae]);
}}),getReadyState:function(){var b=null;

try{b=this.getRequest().readyState;
}catch(ba){}return b;
...
},getStatusText:function(){var N=k;

try{N=this.getRequest().statusText;
}catch(c){}return N;
},getResponseText:function(){var bx=null;

try{bx=this.getRequest().responseText;
}catch(by){bx=null;
}return bx;
...
if(qx.io.remote.Exchange.wasSuccessful(bu,bv,this.__Ad)){try{bw=this.getRequest().responseXML;
}catch(bs){}}if(typeof bw==B&&bw!=null){if(!bw.documentElement){var s=String(this.getRequest().responseText).replace(/<\?xml[^\?]*\?>/,k);
bw.loadXML(s);
}if(!bw.documentElement){throw new Error("Missing Document Element!");
}
if(bw.documentElement.tagName==r){throw new Error("XML-File is not well-formed!");
}}else{throw new Error("Response was not a valid xml document ["+this.getRequest().responseText+"]");
}return bw;
},getFetchedLength:function(){var S=this.getResponseText();
return typeof S==v?S.length:0;
...
try{if(bC&&bC.length>0){var bB=qx.util.Json.parseQx(bC);
return (bB===0?0:(bB||null));
}else{return null;
}}catch(bp){this.error("Could not execute json: ["+bC+"]",bp);
return t+bC+y;
}case G:{};
...
}}catch(bl){this.error("Could not execute javascript: ["+bC+"]",bl);
return null;
}case d:bC=this.getResponseXml();
{};
...
switch(Q){case E:this.fireEvent(E);
break;
...
this.fireEvent(f);
break;
}}},defer:function(O,P){qx.io.remote.Exchange.registerType(qx.io.remote.transport.XmlHttp,M);
...
(function(){var w="=",v="&",u="application/xml",t="application/json",s="text/html",r="qx.client",q="textarea",p="none",o="text/plain",n="text/javascript",S="",R="completed",Q="?",P="qx.io.remote.transport.Iframe",O="gecko",N="frame_",M="aborted",L="_data_",K="pre",J="__Ag",D="javascript:void(0)",E="sending",B="form",C="__Ah",z="failed",A='<iframe name="',x="mshtml",y="form_",F='"></iframe>',G="iframe",I="timeout",H="qx/static/blank.gif";
qx.Class.define(P,{extend:qx.io.remote.transport.Abstract,construct:function(){arguments.callee.base.call(this);
var T=(new Date).valueOf();
var U=N+T;
var V=y+T;
if(qx.core.Variant.isSet(r,x)){this.__Ag=document.createElement(A+U+F);
}else{this.__Ag=document.createElement(G);
}this.__Ag.src=D;
this.__Ag.id=this.__Ag.name=U;
this.__Ag.onload=qx.lang.Function.bind(this._onload,this);
this.__Ag.style.display=p;
document.body.appendChild(this.__Ag);
this.__Ah=document.createElement(B);
this.__Ah.target=U;
this.__Ah.id=this.__Ah.name=V;
this.__Ah.style.display=p;
document.body.appendChild(this.__Ah);
this.__Ai=document.createElement(q);
this.__Ai.id=this.__Ai.name=L;
this.__Ah.appendChild(this.__Ai);
this.__Ag.onreadystatechange=qx.lang.Function.bind(this._onreadystatechange,this);
...
},_numericMap:{"uninitialized":1,"loading":2,"loaded":2,"interactive":3,"complete":4}},members:{__Ai:null,__Aj:0,__Ah:null,__Ag:null,send:function(){var f=this.getMethod();
var h=this.getUrl();
...
if(j instanceof Array){for(var i=0;i<j.length;i++){l.push(encodeURIComponent(g)+w+encodeURIComponent(j[i]));
}}else{l.push(encodeURIComponent(g)+w+encodeURIComponent(j));
}}
if(l.length>0){h+=(h.indexOf(Q)>=0?v:Q)+l.join(v);
...
if(l.length>0){this.setData(l.join(v));
}}var d=this.getFormFields();

for(var g in d){var k=document.createElement(q);
k.name=g;
k.appendChild(document.createTextNode(d[g]));
this.__Ah.appendChild(k);
}this.__Ah.action=h;
this.__Ah.method=f;
this.__Ai.appendChild(document.createTextNode(this.getData()));
this.__Ah.submit();
this.setState(E);
},_onload:qx.event.GlobalError.observeMethod(function(e){if(this.__Ah.src){return;
}this._switchReadyState(qx.io.remote.transport.Iframe._numericMap.complete);
}),_onreadystatechange:qx.event.GlobalError.observeMethod(function(e){this._switchReadyState(qx.io.remote.transport.Iframe._numericMap[this.__Ag.readyState]);
}),_switchReadyState:function(bf){switch(this.getState()){case R:case M:case z:case I:this.warn("Ignore Ready State Change");
return;
}while(this.__Aj<bf){this.setState(qx.io.remote.Exchange._nativeMap[++this.__Aj]);
}},setRequestHeader:function(b,c){},getResponseHeader:function(Y){return null;
},getResponseHeaders:function(){return {};
},getStatusCode:function(){return 200;
},getStatusText:function(){return S;
},getIframeWindow:function(){return qx.bom.Iframe.getWindow(this.__Ag);
},getIframeDocument:function(){return qx.bom.Iframe.getDocument(this.__Ag);
},getIframeBody:function(){return qx.bom.Iframe.getBody(this.__Ag);
},getIframeTextContent:function(){var a=this.getIframeBody();

if(!a){return null;
}
if(!a.firstChild){return S;
}if(a.firstChild.tagName&&a.firstChild.tagName.toLowerCase()==K){return a.firstChild.innerHTML;
}else{return a.innerHTML;
}},getIframeHtmlContent:function(){var W=this.getIframeBody();
return W?W.innerHTML:null;
...
return ba;
break;
case t:ba=this.getIframeHtmlContent();
{};

try{return ba&&ba.length>0?qx.util.Json.parseQx(ba):null;
}catch(X){return this.error("Could not execute json: ("+ba+")",X);
}case n:ba=this.getIframeHtmlContent();
{};

try{return ba&&ba.length>0?window.eval(ba):null;
}catch(be){return this.error("Could not execute javascript: ("+ba+")",be);
}case u:ba=this.getIframeDocument();
{};
return ba;
default:this.warn("No valid responseType specified ("+this.getResponseType()+")!");
return null;
}}},defer:function(bb,bc,bd){qx.io.remote.Exchange.registerType(qx.io.remote.transport.Iframe,P);
},destruct:function(){if(this.__Ag){this.__Ag.onload=null;
this.__Ag.onreadystatechange=null;
if(qx.core.Variant.isSet(r,O)){this.__Ag.src=qx.util.ResourceManager.getInstance().toUri(H);
}document.body.removeChild(this.__Ag);
}
if(this.__Ah){document.body.removeChild(this.__Ah);
}this._disposeFields(J,C);
}});
})();
...
(function(){var B="&",A="=",z="?",y="application/json",x="completed",w="text/plain",v="text/javascript",u="qx.io.remote.transport.Script",t="",s="_ScriptTransport_data",m="_responseContent",r="script",p="timeout",l="_ScriptTransport_",k="__Al",o="_ScriptTransport_id",n="aborted",q="utf-8",j="failed";
qx.Class.define(u,{extend:qx.io.remote.transport.Abstract,construct:function(){arguments.callee.base.call(this);
var b=++qx.io.remote.transport.Script.__Ak;

if(b>=2000000000){qx.io.remote.transport.Script.__Ak=b=1;
}this.__Al=null;
this.__Ak=b;
},statics:{__Ak:0,_instanceRegistry:{},ScriptTransport_PREFIX:l,ScriptTransport_ID_PARAM:o,ScriptTransport_DATA_PARAM:s,handles:{synchronous:false,asynchronous:true,crossDomain:true,fileUpload:false,programaticFormFields:false,responseTypes:[w,v,y]},isSupported:function(){return true;
...
}})},members:{__Am:0,__Al:null,__Ak:null,send:function(){var e=this.getUrl();
e+=(e.indexOf(z)>=0?B:z)+qx.io.remote.transport.Script.ScriptTransport_ID_PARAM+A+this.__Ak;
...
}qx.io.remote.transport.Script._instanceRegistry[this.__Ak]=this;
this.__Al=document.createElement(r);
this.__Al.charset=q;
this.__Al.src=e;
{};
document.body.appendChild(this.__Al);
},_switchReadyState:function(C){switch(this.getState()){case x:case n:case j:case p:this.warn("Ignore Ready State Change");
return;
}while(this.__Am<C){this.setState(qx.io.remote.Exchange._nativeMap[++this.__Am]);
...
default:this.warn("No valid responseType specified ("+this.getResponseType()+")!");
return null;
}}},defer:function(I,J,K){qx.io.remote.Exchange.registerType(qx.io.remote.transport.Script,u);
qx.io.remote.ScriptTransport=I;
},destruct:function(){if(this.__Al){delete qx.io.remote.transport.Script._instanceRegistry[this.__Ak];
document.body.removeChild(this.__Al);
}this._disposeFields(k,m);
}});
})();
