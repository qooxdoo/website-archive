...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.widget.List-qx.theme.Modern-0.js"]],
  boot : "boot",
...
var cq=new qx.ui.basic.Label(bm);
cq.setFont(bH);
cw.add(cq,{left:20,top:20});
var cy=new qx.ui.form.List;
cy.setScrollbarX(p);
cy.set({height:280,width:150,selectionMode:bJ});
var cz;

for(var i=1;i<=25;i++){cz=new qx.ui.form.ListItem(bC+i,L+((i%4)?N:bs)+W);
cy.add(cz);
!(i%9)&&(cz.setEnabled(false));
if(i==20){cy.setSelection([cz]);
}}cy.addListener(bG,function(e){var b=e.getData();

if(b[0]){this.debug("Value: "+b[0].getLabel());
}});
cw.add(cy,{left:20,top:40});
var cm=new qx.ui.form.RadioButton(I);
var cj=new qx.ui.form.RadioButton(bU);
var ck=new qx.ui.form.RadioButton(cc);
var ci=new qx.ui.form.RadioButton(r);
cm.setUserData(bK,bE);
cj.setUserData(bK,bJ);
ck.setUserData(bK,bF);
ci.setUserData(bK,bI);
cj.setValue(true);
...
var cC=new qx.ui.form.RadioButton(l);
var cB=new qx.ui.form.RadioButton(bq);
var cE=new qx.ui.form.RadioButton(bz);
cC.setUserData(bK,H);
cB.setUserData(bK,bA);
cE.setUserData(bK,Y);
cE.setValue(true);
...
cx.addListener(bG,function(e){for(var i=0;i<cy.getChildren().length;i++){cy.getChildren()[i].setShow(e.getData()[0].getUserData(bK));
}});
var cu=new qx.ui.form.CheckBox(bx);
var cr=new qx.ui.form.CheckBox(n).set({enabled:false});
cw.add(cu,{left:180,top:220});
cw.add(cr,{left:180,top:240});
cu.addListener(bD,function(e){if(e.getData()){var cF=cy.getSelectionMode();

if(cF==bE||cF==bI){this.debug("Drag selection is only available for the modes multi or additive");
}}cy.setDragSelection(e.getData());
});
cr.addListener(bD,function(e){if(e.getData()){var a=cy.getSelectionMode();

if(a==bJ||a==bF){this.debug("Quick selection is only available for the modes multi or additive");
}}cy.setQuickSelection(e.getData());
});
cl.addListener(bG,function(e){var cG=e.getData()[0].getUserData(bK);
cy.setSelectionMode(cG);

if(cG==bE||cG==bI){cu.setEnabled(false);
cr.setEnabled(true);
}else if(cG==bJ||cG==cf){cu.setEnabled(true);
cr.setEnabled(false);
}});
var cD=new qx.ui.basic.Label(bX);
cD.setFont(bH);
cw.add(cD,{left:330,top:20});
var cp=new qx.ui.form.List();
cp.set({height:280,width:150,selectionMode:bI});
var cz;

for(var i=1;i<=25;i++){cz=new qx.ui.form.ListItem(bC+i,bl);
cp.add(cz);
...
var cn;
ch.set({width:150,selectionMode:bF});
var ct=[bw,bh,D,bY,O,bt,K,S,R,bB,bd,u,w,bP,s,bg,ba,x,bL,bM,V,bT,Q,bO,z,c,ca,P,cd,bi,bb,q,J,bN,bj,v,U,bo,bW,bQ,k,E,bn,ce,t,bR,y,j,bp,o,X];
...
var cq=new qx.ui.basic.Label(bf);
cq.setFont(bH);
cw.add(cq,{left:20,top:350});
var cA=new qx.ui.form.List(true);
var co;
cA.set({width:550,selectionMode:bJ,height:null});
var cs=[bV,B,bu,f,by,bk,cb,T,bc,bv,h,g,F,d,bS,C,M,A,G];
var cv=br;
