...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.bom.Flash", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.bom.Flash-qx.theme.Modern-0.js"]],
  boot : "boot",
...
(function(){var p="qx.client",o="object",n="100%",m="movie",l="=",k="function",j="application/x-shockwave-flash",h='<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">',g="Please only use the following arguments for this method: qx.bom.Flash.create(element, attributes, variables, params, win)",f='" />',G="string",F="qx.bom.Flash",E='<param name="',D="load",C="type",B="onbeforeunload",A="name",z="data",y='</object>',x="",v="mshtml",w="value",t="&",u="beforeunload",r="param",s='" value="',q="undefined";
qx.Class.define(F,{statics:{_flashObjects:{},create:function(H,I,J,K,L){if(typeof I===G){qx.log.Logger.deprecatedMethodWarning(arguments.callee,g);
I={id:arguments[2],movie:arguments[1]};
J=arguments[3];
K=arguments[4];
L=arguments[5];
}
...
if(!I.width){I.width=n;
}
if(!I.height){I.height=n;
}K=K?qx.lang.Object.clone(K):{};

if(!K[m]){K[m]=I.movie;
}I[z]=I.movie;
delete I.movie;
if(J){for(var name in J){if(typeof K.flashvars!=q){K.flashvars+=t+name+l+J[name];
}else{K.flashvars=name+l+J[name];
}}}var M=this.__eS(H,I,K,L);
this._flashObjects[I.id]=M;
return M;
},destroy:qx.core.Variant.select(p,{"mshtml":function(c,d){c=this.__eN(c);

if(c.readyState==4){this.__eO(c);
}else{if(!d){d=window;
}qx.bom.Event.addNativeListener(d,D,function(){qx.bom.Flash.__eO(c);
});
}},"default":function(T,U){T=this.__eN(T);
T.parentNode.removeChild(T);
delete this._flashObjects[T.id];
}}),__eN:function(a){if(!a){throw new Error("DOM element is null or undefined!");
}
if(a.tagName.toLowerCase()!==o){a=a.firstChild;
}
if(!a||a.tagName.toLowerCase()!==o){throw new Error("DOM element has or is not a flash object!");
}return a;
},__eO:qx.core.Variant.select(p,{"mshtml":qx.event.GlobalError.observeMethod(function(e){for(var i in e){if(typeof e[i]==k){e[i]=null;
}}e.parentNode.removeChild(e);
delete this._flashObjects[e.id];
}),"default":null}),__eP:qx.event.GlobalError.observeMethod(function(){for(var b in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[b]);
}window.__eQ=function(){};
window.__eR=function(){};
window.detachEvent(B,qx.bom.Flash.__eP);
}),__eS:qx.core.Variant.select(p,{"mshtml":function(V,W,X,Y){X.movie=W.data;
delete W.data;
delete W.classid;
var ba=x;

for(name in X){ba+=E+name+s+X[name]+f;
}V.innerHTML=h+ba+y;
for(var name in W){V.firstChild.setAttribute(name,W[name]);
}return V.firstChild;
},"default":function(N,O,P,Q){delete O.classid;
delete P.movie;
var S=qx.bom.Element.create(o,O,Q);
S.setAttribute(C,j);
var R;

for(var name in P){R=qx.bom.Element.create(r,{},Q);
R.setAttribute(A,name);
R.setAttribute(w,P[name]);
S.appendChild(R);
}N.appendChild(S);
return S;
}})},defer:function(bb){if(qx.core.Variant.isSet(p,v)){qx.bom.Event.addNativeListener(window,u,bb.__eP);
}}});
})();
