...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.data.Twitter-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var z="value",y="icon",x="model",w="text",v="qx.client",u="execute",t="source",s="wittemann",r="mshtml",q=".json",V="user.profile_image_url",U="http://twitter.com/statuses/user_timeline/",T="Enter",S="<span style='font-size: 20px'>Twitter</span>",R="<a target='_blank' href='http://twitter.com/home?status=Check%20out%20the%20%23qooxdoo%20twitter%20demo%20at%20http://bit.ly/Mc4Of'>Tweet This</a></span>",Q="demobrowser.demo.data.store.Twitter",P="keydown",O="Friends",N="demobrowser.demo.data.Twitter",M="selection[0]",G="<span style='font-size: 13px'>",H="Location: ",E="Details",F="Name: ",C="?callback=demobrowser.demo.data.store.Twitter.saveResult",D="User",A="http://twitter.com/statuses/friends_timeline.json",B="Postet with: ",I="user.name",J="user.location",L="Avatar: ",K="Message: ";
...
this.getRoot().add(f,{left:10,top:85});
f.setWidth(300);
var h=new qx.data.controller.List(null,f);
h.setDelegate(this);
h.setLabelPath(w);
if(!qx.core.Variant.isSet(v,r)){h.setIconPath(V);
}var l=new demobrowser.demo.data.store.Twitter(s);
l.bind(x,h,x);
var d=new qx.ui.form.Button(O);
this.getRoot().add(d,{left:10,top:55});
d.addListener(u,function(){l.setUrl(A);
},this);
var i=new qx.ui.form.Button(D);
this.getRoot().add(i,{left:90,top:55});
var a=new qx.ui.form.TextField(s);
this.getRoot().add(a,{left:140,top:55});
i.addListener(u,function(){var o=U+a.getValue()+q;

if(l.getUrl()==o){l.reload();
}else{l.setUrl(o);
}},this);
a.addListener(P,function(Y){if(Y.getKeyIdentifier()==T){i.execute();
}},this);
var e=new qx.ui.groupbox.GroupBox(E);
this.getRoot().add(e,{left:320,top:60});
e.setWidth(300);
e.setHeight(220);
e.setLayout(new qx.ui.layout.Grid(0,5));
e.add(new qx.ui.basic.Label(F),{row:0,column:0});
e.add(new qx.ui.basic.Label(H),{row:1,column:0});
e.add(new qx.ui.basic.Label(K),{row:2,column:0});
e.add(new qx.ui.basic.Label(B),{row:3,column:0});
...
j.setRich(true);
j.setWidth(150);
e.add(j,{row:2,column:1});
var m=new qx.ui.basic.Label();
m.setRich(true);
e.add(m,{row:3,column:1});
var b=new qx.data.controller.Object();
b.addTarget(name,z,I);
b.addTarget(location,z,J);
b.addTarget(j,z,w);
b.addTarget(m,z,t);

if(!qx.core.Variant.isSet(v,r)){e.add(new qx.ui.basic.Label(L),{row:4,column:0});
var g=new qx.ui.basic.Image();
e.add(g,{row:4,column:1});
b.addTarget(g,t,V);
}h.bind(M,b,x);
...
k.setWidth(260);
k.setValue(G+R);
this.getRoot().add(k,{left:10,top:290});
},configureItem:function(bc){bc.setRich(true);
bc.getChildControl(y).setWidth(50);
bc.getChildControl(y).setHeight(50);
bc.getChildControl(y).setScale(true);
}}});
qx.Class.define(Q,{extend:qx.data.store.Json,statics:{saveResult:function(bd){this.__AO=bd;
}},construct:function(ba){var bb=U+ba+q;
arguments.callee.base.call(this,bb);
},members:{_createRequest:function(W){var X=new qx.io2.ScriptLoader();
W+=C;
X.load(W,function(p){this.__AP();
},this);
},__AP:function(){var n=demobrowser.demo.data.store.Twitter.__AO;

if(n==undefined){this.setState("failed");
return;
