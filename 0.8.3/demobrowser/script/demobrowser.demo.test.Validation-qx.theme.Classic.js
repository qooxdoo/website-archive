...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.test.Validation", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.test.Validation-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var k="<br>",j="male",i="new gender: ",h="new age: ",g="new name: ",f="female",e="demobrowser.demo.test.Validation",d="<b>Setting the age ",c="property to 27</b><br>",b="<br>old name: ",Q=")",P="age:",O="qx.Person",N="<b>Testing the gender ",M="fmale",L="<br><b>Setting the name to 'Hans'</b><br>",K="property to 'female'</b><br>",J="<br><b>Setting the gender ",I="must be longer than one characters. (",H="<font color='red'>GENDER NOT SET</font>",r="<br>old gender: ",s="- name: Name is validated to a minimum length of two.<br/>",p="<br><b>Setting the age ",q="- age: Age is validated to be a number.<br/>",n="<b>Initial property values</b><br>",o="",l="<b>Setting the name to ''</b><br>",m="property to 'true'</b><br>",t="- gender: Gender is validated to be either male or female.",u="property to 'fmale'</b><br>",z="gender:",y="<br>old age: ",B="Hans",A="The demo creates an internal class containing three properties:<br/>",D="Validation Error: Name ",C="<font color='red'>AGE NOT SET</font>",w="<font color='red'>NAME NOT SET</font>",G="My name",F="__oD",E="white",v="name:",x="<b>Description</b><br/>";
qx.Class.define(e,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
qx.Class.define(O,{extend:qx.core.Object,properties:{name:{init:G,validate:F},age:{init:18,validate:qx.util.Validate.number()},gender:{init:j,validate:qx.util.Validate.inArray([j,f])}},members:{__oD:function(a){if(a.length<2){throw new qx.core.ValidationError(D+I+a+Q);
}}}});
var S=new qx.Person();
...
T.setHtml(n);
T.setHtml(T.getHtml()+v+S.getName()+k);
T.setHtml(T.getHtml()+P+S.getAge()+k);
T.setHtml(T.getHtml()+z+S.getGender()+k);
T.setHtml(T.getHtml()+L);
S.setName(B);
T.setHtml(T.getHtml()+g+S.getName()+k);
T.setHtml(T.getHtml()+l);
try{S.setName(o);
T.setHtml(T.getHtml()+g+S.getName()+k);
}catch(R){T.setHtml(T.getHtml()+w+b+S.getName()+k);
}T.setHtml(T.getHtml()+p+c);
S.setAge(27);
T.setHtml(T.getHtml()+h+S.getAge()+k);
T.setHtml(T.getHtml()+d+m);
try{S.setAge(true);
T.setHtml(T.getHtml()+h+S.getAge()+k);
}catch(V){T.setHtml(T.getHtml()+C+y+S.getAge()+k);
}T.setHtml(T.getHtml()+J+K);
S.setGender(f);
T.setHtml(T.getHtml()+i+S.getGender()+k);
T.setHtml(T.getHtml()+N+u);
try{S.setAge(M);
T.setHtml(T.getHtml()+i+S.getGender()+k);
}catch(W){T.setHtml(T.getHtml()+H+r+S.getGender()+k);
}var U=new qx.ui.basic.Label();
U.setRich(true);
U.setWidth(250);
U.setValue(x+A+s+q+t);
