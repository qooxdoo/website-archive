...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.treevirtual.TreeVirtual_Multiple_Columns", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Modern", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.treevirtual.TreeVirtual_Multiple_Columns-qx.theme.Modern-0.js"]],
  boot : "boot",
...
U.addListener(J,function(e){var bc=e.getData();
this.setValue(U.getHierarchy(bc[0].nodeId).join(D));
R.setEnabled(true);
},o);
...
U.addListener(s,function(e){if(this.getValue()){var a=e.getData();
U.nodeSetHideOpenClose(a,true);
}},o);
...
(function(){var F="dataChanged",E="qx.event.type.Data",D="Left",C="Right",B="hidden",A="Boolean",z="Enter",y="number",x="This method no longer does anything and will be removed in 0.9",w="changeSelection",t="dataEdited",v="object",u="qx.ui.treevirtual.TreeVirtual",r="string",q="treevirtual";
qx.Class.define(u,{extend:qx.ui.table.Table,construct:function(I,J){if(!J){J={};
}
if(!J.dataModel){J.dataModel=new qx.ui.treevirtual.SimpleTreeDataModel();
}
if(J.treeColumn===undefined){J.treeColumn=0;
J.dataModel.setTreeColumn(J.treeColumn);
}
if(!J.treeDataCellRenderer){J.treeDataCellRenderer=new qx.ui.treevirtual.SimpleTreeDataCellRenderer();
}
if(!J.defaultDataCellRenderer){J.defaultDataCellRenderer=new qx.ui.treevirtual.DefaultDataCellRenderer();
}
if(!J.dataRowRenderer){J.dataRowRenderer=new qx.ui.treevirtual.SimpleTreeDataRowRenderer();
}
if(!J.selectionManager){J.selectionManager=function(bm){return new qx.ui.treevirtual.SelectionManager(bm);
};
}
if(!J.tableColumnModel){J.tableColumnModel=function(bv){return new qx.ui.table.columnmodel.Resize(bv);
};
}if(typeof (I)==r){I=[I];
}J.dataModel.setColumns(I);
J.dataModel.setTreeColumn(J.treeColumn);
J.dataModel.setTree(this);
arguments.callee.base.call(this,J.dataModel,J);
this.addListener(t,function(e){this.getDataModel().setData();
},this);
this.setColumnVisibilityButtonVisible(I.length>1);
this.setRowHeight(16);
this.setMetaColumnCounts(I.length>1?[1,-1]:[1]);
this.setOverflow(B);
var L=J.treeDataCellRenderer;
var K=J.defaultDataCellRenderer;
var O=this.getTableColumnModel();
var N=this.getDataModel().getTreeColumn();

for(var i=0;i<I.length;i++){O.setDataCellRenderer(i,i==N?L:K);
}this.setDataRowRenderer(J.dataRowRenderer);
this.setFocusCellOnMouseMove(true);
this.setShowCellFocusIndicator(false);
var M=this._getPaneScrollerArr();
for(var i=0;i<M.length;i++){M[i].setSelectBeforeFocus(true);
}},events:{"treeOpenWithContent":E,"treeOpenWhileEmpty":E,"treeClose":E,"changeSelection":E},statics:{SelectionMode:{NONE:qx.ui.table.selection.Model.NO_SELECTION,SINGLE:qx.ui.table.selection.Model.SINGLE_SELECTION,SINGLE_INTERVAL:qx.ui.table.selection.Model.SINGLE_INTERVAL_SELECTION,MULTIPLE_INTERVAL:qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION,MULTIPLE_INTERVAL_TOGGLE:qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION_TOGGLE}},properties:{openCloseClickSelectsRow:{check:A,init:false},appearance:{refine:true,init:q}},members:{getDataModel:function(){return this.getTableModel();
},setUseTreeLines:function(b){var be=this.getDataModel();
var bc=be.getTreeColumn();
var bd=this.getTableColumnModel().getDataCellRenderer(bc);
bd.setUseTreeLines(b);
if(be.hasListener(F)){var bf={firstRow:0,lastRow:be.getRowCount()-1,firstColumn:0,lastColumn:be.getColumnCount()-1};
be.fireDataEvent(F,bf);
...
if(bp.hasListener(F)){var bq={firstRow:0,lastRow:bp.getRowCount()-1,firstColumn:0,lastColumn:bp.getColumnCount()-1};
bp.fireDataEvent(F,bq);
}},setExcludeFirstLevelTreeLines:function(b){var bt=this.getDataModel();
var br=bt.getTreeColumn();
var bs=this.getTableColumnModel().getDataCellRenderer(br);
bs.setExcludeFirstLevelTreeLines(b);
if(bt.hasListener(F)){var bu={firstRow:0,lastRow:bt.getRowCount()-1,firstColumn:0,lastColumn:bt.getColumnCount()-1};
bt.fireDataEvent(F,bu);
}},getExcludeFirstLevelTreeLines:function(){var S=this.getDataModel().getTreeColumn();
var T=this.getTableColumnModel().getDataCellRenderer(S);
return T.getExcludeFirstLevelTreeLines();
},getAlwaysShowOpenCloseSymbol:function(){var Q=this.getDataModel().getTreeColumn();
var R=this.getTableColumnModel().getDataCellRenderer(Q);
return R.getAlwaysShowOpenCloseSymbol();
},setSelectionMode:function(P){this.getSelectionModel().setSelectionMode(P);
},getSelectionMode:function(){return this.getSelectionModel().getSelectionMode();
},setCellFocusAttributes:function(U){qx.log.Logger.deprecatedMethodWarning(arguments.callee,x);
},getHierarchy:function(V){var ba=this;
var Y=[];
var W;
var bb;
...
if(typeof (V)==v){W=V;
bb=W.nodeId;
}else if(typeof (V)==y){bb=V;
}else{throw new Error("Expected node object or node id");
}function X(bk){if(!bk){return ;
}var bl=ba.getDataModel().getData()[bk];
Y.unshift(bl.label);
X(bl.parentNodeId);
}X(bb);
return Y;
...
}var p=d.getKeyIdentifier();
var l=false;
var n=d.getModifiers();

if(n==0){switch(p){case z:var g=this.getDataModel();
var h=this.getFocusedColumn();
var m=g.getTreeColumn();

if(h==m){var o=this.getFocusedRow();
var f=g.getValue(m,o);

if(!f.bHideOpenClose){g.setState(f,{bOpened:!f.bOpened});
}l=true;
}break;
case D:this.moveFocusedCell(-1,0);
break;
case C:this.moveFocusedCell(1,0);
break;
}}else if(n==qx.event.type.Dom.CTRL_MASK){switch(p){case D:var g=this.getDataModel();
var o=this.getFocusedRow();
var m=g.getTreeColumn();
var f=g.getValue(m,o);
if((f.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH)&&!f.bHideOpenClose&&f.bOpened){g.setState(f,{bOpened:!f.bOpened});
}this.setFocusedCell(m,o,true);
l=true;
break;
case C:var g=this.getDataModel();
var o=this.getFocusedRow();
var m=g.getTreeColumn();
var f=g.getValue(m,o);
if((f.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH)&&!f.bHideOpenClose&&!f.bOpened){g.setState(f,{bOpened:!f.bOpened});
...
l=true;
break;
}}else if(n==qx.event.type.Dom.SHIFT_MASK){switch(p){case D:var g=this.getDataModel();
var o=this.getFocusedRow();
var m=g.getTreeColumn();
var f=g.getValue(m,o);
if(f.parentNodeId){var k=g.getRowFromNodeId(f.parentNodeId);
...
var m=g.getTreeColumn();
var f=g.getValue(m,o);
if((f.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH)&&!f.bHideOpenClose){if(!f.bOpened){g.setState(f,{bOpened:!f.bOpened});
}if(f.children.length>0){this.moveFocusedCell(0,1);
}}l=true;
break;
}}if(l){d.preventDefault();
d.stopPropagation();
}else{arguments.callee.base.call(this,d);
...
},_calculateSelectedNodes:function(){var bj=this.getDataModel();
var bh=this.getSelectionModel().getSelectedRanges();
var bi=[];
var bg;

for(var i=0;i<bh.length;i++){for(var j=bh[i].minIndex;j<=bh[i].maxIndex;j++){bg=bj.getValue(bj.getTreeColumn(),j);
bj.setState(bg,{bSelected:true});
bi.push(bg);
}}return bi;
},setOverflow:function(s){if(s!=B){throw new Error("Tree overflow must be hidden.  "+"The internal elements of it will scroll.");
}}}});
})();
...
(function(){var bi="number",bh="dataChanged",bg="object",bf="treeOpenWhileEmpty",be="__vK",bd="treeOpenWithContent",bc="_treeColumn",bb="_selections",ba="bSelected",Y="_nodeRowMap",U="qx.ui.treevirtual.SimpleTreeDataModel",X="bOpened",W="_rowArr",T="_nodeArr",S="treeClose",V="<virtual root>";
qx.Class.define(U,{extend:qx.ui.table.model.Abstract,construct:function(){arguments.callee.base.call(this);
this._rowArr=[];
this._nodeArr=[];
this._nodeRowMap=[];
this._treeColumn=0;
this._selections={};
this._nodeArr.push(arguments.callee.self.__vJ());
this.__vI=null;
},statics:{__vJ:function(){return {label:V,nodeId:0,bOpened:true,children:[]};
},Type:{LEAF:1,BRANCH:2}},members:{__vK:null,__vI:null,_rowArr:null,_nodeArr:null,_nodeRowMap:null,_treeColumn:null,_selections:null,setTree:function(bq){this.__vK=bq;
},getTree:function(){return this.__vK;
},setEditable:function(bU){this.__vI=[];

for(var bV=0;bV<this.getColumnCount();bV++){this.__vI[bV]=bU;
}this.fireEvent(qx.ui.table.ITableModel.EVENT_TYPE_META_DATA_CHANGED);
},setColumnEditable:function(bo,bp){if(bp!=this.isColumnEditable(bo)){if(this.__vI==null){this.__vI=[];
}this.__vI[bo]=bp;
this.fireEvent(qx.ui.table.ITableModel.EVENT_TYPE_META_DATA_CHANGED);
}},isColumnEditable:function(bs){return (this.__vI?this.__vI[bs]==true:false);
},isColumnSortable:function(bn){return false;
},sortByColumn:function(r,s){throw new Error("Trees can not be sorted by column");
},getSortColumnIndex:function(){return -1;
},setTreeColumn:function(N){this._treeColumn=N;
...
},getValue:function(bQ,bR){if(bR<0||bR>=this._rowArr.length){throw new Error("this._rowArr row "+"("+bR+") out of bounds: "+this._rowArr+" (0.."+(this._rowArr.length-1)+")");
}
if(bQ<0||bQ>=this._rowArr[bR].length){throw new Error("this._rowArr column "+"("+bQ+") out of bounds: "+this._rowArr[bR]+" (0.."+(this._rowArr[bR].length-1)+")");
}return this._rowArr[bR][bQ];
},setValue:function(m,n,o){if(m==this._treeColumn){return ;
}var p=this.getNodeFromRow(n);

if(p.columnData[m]!=o){p.columnData[m]=o;
this.setData();
if(this.hasListener(bh)){var q={firstRow:p.nodeId,lastRow:p.nodeId,firstColumn:m,lastColumn:m};
this.fireDataEvent(bh,q);
}}},_addNode:function(D,E,F,G,H,I,J){var M;
if(D){M=this._nodeArr[D];
...
}}else{M=this._nodeArr[0];
D=0;
}if(H==qx.ui.treevirtual.SimpleTreeDataModel.Type.LEAF){F=false;
G=false;
}var L=this._nodeArr.length;
var K={type:H,nodeId:L,parentNodeId:D,label:E,bSelected:false,bOpened:F,bHideOpenClose:G,icon:I,iconSelected:J,children:[],columnData:[]};
this._nodeArr.push(K);
M.children.push(L);
return L;
},addBranch:function(w,x,y,z,A,B){return this._addNode(w,x,y,z,qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH,A,B);
...
},prune:function(O,P){var Q;
var R;

if(typeof (O)==bg){Q=O;
R=Q.nodeId;
}else if(typeof (O)==bi){R=O;
}else{throw new Error("Expected node object or node id");
}for(var i=this._nodeArr[R].children.length-1;i>=0;i--){this.prune(this._nodeArr[R].children[i],true);
}if(P&&R!=0){Q=this._nodeArr[R];
qx.lang.Array.remove(this._nodeArr[Q.parentNodeId].children,R);
if(this._selections[R]){delete this._selections[R];
}this._nodeArr[R]=null;
}},move:function(a,b){var g;
var c;
var f;
var d;
b=b||0;
...
if(typeof (a)==bg){g=a;
c=g.nodeId;
}else if(typeof (a)==bi){c=a;
g=this._nodeArr[c];
}else{throw new Error("Expected move node object or node id");
}
if(typeof (b)==bg){f=b;
d=f.nodeId;
}else if(typeof (b)==bi){d=b;
f=this._nodeArr[d];
}else{throw new Error("Expected parent node object or node id");
}if(f.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.LEAF){throw new Error("Sorry, a LEAF may not have children.");
}var e=this._nodeArr[g.parentNodeId];
qx.lang.Array.remove(e.children,c);
f.children.push(c);
this._nodeArr[c].parentNodeId=d;
},setData:function(h){var l=this;
function k(){var bT=function(bJ,bK){var bO=null;
var bN;
var bL=l._nodeArr[bJ].children.length;

for(var i=0;i<bL;i++){bN=l._nodeArr[bJ].children[i];
bO=l._nodeArr[bN];
if(bO==null){continue;
}bO.level=bK;
bO.bFirstChild=(i==0);
bO.lastChild=[i==bL-1];
var parent=l._nodeArr[bO.parentNodeId];
while(parent.nodeId){var bP=parent.lastChild[parent.lastChild.length-1];
bO.lastChild.unshift(bP);
parent=l._nodeArr[parent.parentNodeId];
}if(!bO.columnData){bO.columnData=[];
}
if(bO.columnData.length<l.getColumnCount()){bO.columnData[l.getColumnCount()-1]=null;
}var bM=[];
if(bO.columnData){for(var j=0;j<bO.columnData.length;j++){if(j==l._treeColumn){bM.push(bO);
}else{bM.push(bO.columnData[j]);
}}}else{bM.push(bO);
}if(bO.bSelected){bM.selected=true;
}l._nodeRowMap[bO.nodeId]=l._rowArr.length;
l._rowArr.push(bM);
if(bO.bOpened){bT(bN,bK+1);
}}};
l._rowArr=[];
l._nodeRowMap=[];
bT(0,1);
if(l.hasListener(bh)){var bS={firstRow:0,lastRow:l._rowArr.length-1,firstColumn:0,lastColumn:l.getColumnCount()-1};
l.fireDataEvent(bh,bS);
}}
...
this.setData([arguments.callee.self.__vJ()]);
},setColumnData:function(bt,bu,bv){this._nodeArr[bt].columnData[bu]=bv;
},getColumnData:function(bx,by){return this._nodeArr[bx].columnData[by];
},setState:function(bz,bA){var bB;
var bG;

if(typeof (bz)==bg){bB=bz;
bG=bB.nodeId;
}else if(typeof (bz)==bi){bG=bz;
bB=this._nodeArr[bG];
}else{throw new Error("Expected node object or node id");
}
for(var bE in bA){switch(bE){case ba:var bH=this.getRowFromNodeId(bG);
var bC=this.getTree().getSelectionModel();
var bD=qx.ui.treevirtual.TreeVirtual;
var bI=(typeof (bH)===bi&&this.getTree().getSelectionMode()!=bD.SelectionMode.NONE);
if(bA[bE]){this._selections[bG]=true;
if(bI&&!bC.isSelectedIndex(bH)){bC.setSelectionInterval(bH,bH);
}}else{delete this._selections[bG];
if(bI&&bC.isSelectedIndex(bH)){bC.removeSelectionInterval(bH,bH);
}}break;
case X:if(bA[bE]==bB.bOpened){break;
}var bF=this.__vK;
if(bB.bOpened){bF.fireDataEvent(S,bB);
}else{if(bB.children.length>0){bF.fireDataEvent(bd,bB);
}else{bF.fireDataEvent(bf,bB);
}}if(!bB.bHideOpenClose){bB.bOpened=!bB.bOpened;
bF.getSelectionModel()._clearSelection();
}this.setData();
break;
default:break;
}bB[bE]=bA[bE];
}},getNodeRowMap:function(){return this._nodeRowMap;
},getRowFromNodeId:function(bw){return this._nodeRowMap[bw];
},getNodeFromRow:function(br){return this._nodeArr[this._rowArr[br][this._treeColumn].nodeId];
},_clearSelections:function(){for(var C in this._selections){this._nodeArr[C].bSelected=false;
}this._selections={};
},getSelectedNodes:function(){var t=[];

for(var u in this._selections){t.push(this._nodeArr[u]);
}return t;
}},destruct:function(){this._disposeFields(W,T,Y,bc,bb,be);
}});
})();
...
(function(){var q='px;',p="",n="Boolean",m='',l='px',k='</div>',j='<div style="position:absolute;',h=";",g='left:',e='">',bh='</span>',bg='background-image:url(',bf="treevirtual-start-contract",be='">&nbsp;</div>',bd="treevirtual-file",bc="qx.ui.treevirtual.SimpleTreeDataCellRenderer",bb="qx.client",ba="treevirtual-only-expand",Y='style="',X='top:0;',x='background-repeat:no-repeat;',y='>',v='right:',w='<div style="',t='" title="',u="treevirtual-end",r="treevirtual-cross",s=');',B='<span',C="treevirtual-end-contract",K="__vO",I=';width:',P=';"',M="__vM",T="treevirtual-end-expand",R="BLANK",E="treevirtual-only-contract",W="treevirtual-contract",V='top:',U="content-box",D='bottom:',G="treevirtual-start-expand",H="mshtml",J="treevirtual-cross-contract",L="treevirtual-folder",N='width:',Q="treevirtual-expand",S="treevirtual-cross-expand",z="__vN",A="treevirtual-line",F=';height:',O='height:';
qx.Class.define(bc,{extend:qx.ui.table.cellrenderer.Abstract,construct:function(){var bT=qx.ui.treevirtual.SimpleTreeDataCellRenderer;
if(bT.__vL){bT.__vQ();
bT.__vL=false;
}arguments.callee.base.call(this);
this.__vM=qx.util.AliasManager.getInstance();
this.__vN=qx.util.ResourceManager.getInstance();
this.__vO=qx.theme.manager.Appearance.getInstance();
this.BLANK=this.__vN.toUri(this.__vM.resolve("static/blank.gif"));
},statics:{__vP:{},__vL:true,__vQ:function(){qx.theme.manager.Meta.getInstance().initialize();
var bN=qx.ui.treevirtual.SimpleTreeDataCellRenderer;
var bQ=qx.io2.ImageLoader;
var bO=qx.util.AliasManager.getInstance();
var bR=qx.util.ResourceManager.getInstance();
var bP=qx.theme.manager.Appearance.getInstance();
var bS=function(f){bQ.load(bR.toUri(bO.resolve(f)));
};
bN.__vP.line=bP.styleFrom(A);
bS(bN.__vP.line.icon);
bN.__vP.contract=bP.styleFrom(W);
bS(bN.__vP.contract.icon);
bN.__vP.expand=bP.styleFrom(Q);
bS(bN.__vP.expand.icon);
bN.__vP.onlyContract=bP.styleFrom(E);
bS(bN.__vP.onlyContract.icon);
bN.__vP.onlyExpand=bP.styleFrom(ba);
bS(bN.__vP.onlyExpand.icon);
bN.__vP.startContract=bP.styleFrom(bf);
bS(bN.__vP.startContract.icon);
bN.__vP.startExpand=bP.styleFrom(G);
bS(bN.__vP.startExpand.icon);
bN.__vP.endContract=bP.styleFrom(C);
bS(bN.__vP.endContract.icon);
bN.__vP.endExpand=bP.styleFrom(T);
bS(bN.__vP.endExpand.icon);
bN.__vP.crossContract=bP.styleFrom(J);
bS(bN.__vP.crossContract.icon);
bN.__vP.crossExpand=bP.styleFrom(S);
bS(bN.__vP.crossExpand.icon);
bN.__vP.end=bP.styleFrom(u);
bS(bN.__vP.end.icon);
bN.__vP.cross=bP.styleFrom(r);
bS(bN.__vP.cross.icon);
}},properties:{useTreeLines:{check:n,init:true},excludeFirstLevelTreeLines:{check:n,init:false},alwaysShowOpenCloseSymbol:{check:n,init:false}},members:{__vM:null,__vO:null,__vN:null,_getCellStyle:function(bi){var bj=bi.value;
var bk=arguments.callee.base.call(this,bi)+(bj.cellStyle?bj.cellStyle+h:p);
return bk;
},_getContentHtml:function(bl){var bn=p;
var bo=0;
...
var bs=this.__vN.toUri(this.__vM.resolve(br.url));
if(br.position){var bu=br.position;
bt.push(j);

if(!qx.core.Variant.isSet(bb,H)){bt.push(qx.bom.element.BoxSizing.compile(U));
}
if(bu.top!==undefined){bt.push(V+bu.top+q);
}
if(bu.right!==undefined){bt.push(v+bu.right+q);
}
if(bu.bottom!==undefined){bt.push(D+bu.bottom+q);
}
if(bu.left!==undefined){bt.push(g+bu.left+q);
}
if(bu.width!==undefined){bt.push(N+bu.width+q);
}
if(bu.height!==undefined){bt.push(O+bu.height+q);
}bt.push(e);
}bt.push(w);
bt.push(bg+bs+s);
bt.push(x);

if(br.imageWidth&&br.imageHeight){bt.push(I+br.imageWidth+l+F+br.imageHeight+l);
}var bv=br.tooltip;

if(bv!=null){bt.push(t+bv);
}bt.push(be);

if(br.position){bt.push(k);
}return bt.join(p);
},_addIndentation:function(bW,bX){var bY=bW.value;
var ca;
var cb=p;
var ce=this.getUseTreeLines();
var cc=this.getExcludeFirstLevelTreeLines();
var cd=this.getAlwaysShowOpenCloseSymbol();

for(var i=0;i<bY.level;i++){ca=this._getIndentSymbol(i,bY,ce,cd,cc);
cb+=this._addImage({url:ca.icon,position:{top:0+(ca.paddingTop||0),left:bX+(ca.paddingLeft||0),width:19,height:16}});
bX+=19;
}return ({html:cb,pos:bX});
},_addIcon:function(bH,bI){var bJ=bH.value;
var bL=(bJ.bSelected?bJ.iconSelected:bJ.icon);

if(!bL){if(bJ.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.LEAF){var o=this.__vO.styleFrom(bd);
}else{var bK={opened:bJ.bOpened};
var o=this.__vO.styleFrom(L,bK);
}bL=o.icon;
}var bM=this._addImage({url:bL,position:{top:0,left:bI,width:19,height:16}});
return ({html:bM,pos:bI+19});
},_addLabel:function(bD,bE){var bF=bD.value;
var bG=j+g+bE+q+X+(bF.labelStyle?bF.labelStyle+h:p)+e+B+(bD.labelSpanStyle?Y+bD.labelSpanStyle+P:p)+y+bF.label+bh+k;
return bG;
},_addExtraContentBeforeIndentation:function(bU,bV){return {html:m,pos:bV};
},_addExtraContentBeforeIcon:function(a,b){return {html:m,pos:b};
},_addExtraContentBeforeLabel:function(c,d){return {html:m,pos:d};
},_getIndentSymbol:function(bw,bx,by,bz,bA){var bB=qx.ui.treevirtual.SimpleTreeDataCellRenderer;
if(bw==0&&bA){by=false;
}if(bw<bx.level-1){return (by&&!bx.lastChild[bw]?bB.__vP.line:{icon:this.BLANK});
}var bC=bx.lastChild[bx.lastChild.length-1];
if(bx.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH&&!bx.bHideOpenClose){if(bx.children.length>0||bz){if(!by){return (bx.bOpened?bB.__vP.contract:bB.__vP.expand);
}if(bw==0&&bx.bFirstChild){if(bC){return (bx.bOpened?bB.__vP.onlyContract:bB.__vP.onlyExpand);
}else{return (bx.bOpened?bB.__vP.startContract:bB.__vP.startExpand);
}}if(bC){return (bx.bOpened?bB.__vP.endContract:bB.__vP.endExpand);
}return (bx.bOpened?bB.__vP.crossContract:bB.__vP.crossExpand);
}}if(by){if(bx.parentNodeId==0){if(bC&&bx.bFirstChild){return {icon:this.BLANK};
}if(bC){return bB.__vP.end;
}if(bx.bFirstChild){return (bx.bOpened?bB.__vP.startContract:bB.__vP.startExpand);
}}return (bC?bB.__vP.end:bB.__vP.cross);
}return {icon:this.BLANK};
}},destruct:function(){this._disposeFields(M,z,K,R);
}});
})();
(function(){var c="qx.ui.treevirtual.DefaultDataCellRenderer";
qx.Class.define(c,{extend:qx.ui.table.cellrenderer.Default,construct:function(){arguments.callee.base.call(this);
},members:{_getCellStyle:function(a){var b=arguments.callee.base.call(this,a)+qx.ui.treevirtual.SimpleTreeDataCellRenderer.MAIN_DIV_STYLE;
return b;
}}});
})();
(function(){var i="qx.ui.treevirtual.SimpleTreeDataRowRenderer";
qx.Class.define(i,{extend:qx.ui.table.rowrenderer.Default,construct:function(){arguments.callee.base.call(this);
},members:{updateDataRowElement:function(a,b){var e=a.table;
var g=a.rowData;
var f=e.getTableModel();
var d=f.getTreeColumn();
var c=g[d];
a.selected=c.bSelected;

if(c.bSelected){var h=a.row;
e.getSelectionModel()._addSelectionInterval(h,h);
}arguments.callee.base.call(this,a,b);
}}});
})();
(function(){var j="__vR",h="qx.ui.treevirtual.SelectionManager",g="Space",f="Enter";
qx.Class.define(h,{extend:qx.ui.table.selection.Manager,construct:function(a){arguments.callee.base.call(this);
this.__vR=a;
},members:{__vR:null,getTable:function(){return this.__vR;
},handleMoveKeyDown:function(b,c){var e=this.getSelectionModel();

switch(c.getModifiers()){case 0:break;
case qx.event.type.Dom.SHIFT_MASK:var d=e.getAnchorSelectionIndex();

if(d==-1){e.setSelectionInterval(b,b);
}else{e.setSelectionInterval(d,b);
}break;
}},_handleSelectEvent:function(D,E){var I=this;
function F(o,p,q){var B=o.getDataModel();
...
if(t!=v){o.startEditing();
return false;
}var r=B.getValue(v,o.getFocusedRow());
...
for(i=0;i<z[v].visX;i++){C+=w.getColumnWidth(z[i].visX);
...
}else{return I._handleExtendedClick(o,q,r,C);
}}else{var A=q.getKeyIdentifier();

switch(A){case g:return false;
case f:if(!r.bHideOpenClose){B.setState(r,{bOpened:!r.bOpened});
}return o.getOpenCloseClickSelectsRow()?false:true;
default:return true;
}}}var H=F(this.__vR,D,E);
...
(function(){var p="appear",n="columnVisibilityMenuCreateEnd",m="tableWidthChanged",l="__qg",k="verticalScrollBarChanged",j="qx.ui.table.columnmodel.resizebehavior.Abstract",i="qx.ui.table.columnmodel.Resize",h="_applyBehavior",g="separator",f="visibilityChanged",c="Reset column widths",e="changeBehavior",d="user-button",b="widthChanged",a="execute";
...
this.__qe=false;
this.__qf=false;
},properties:{behavior:{check:j,init:null,nullable:true,apply:h,event:e}},members:{__qf:null,__qe:null,__qg:null,_applyBehavior:function(v,w){if(w!=null){w.dispose();
w=null;
...
this.__qg.addListener(n,this._addResetColumnWidthButton,this);
this.getBehavior()._setNumColumns(t);
},getTable:function(){return this.__qg;
...
},_onappear:function(event){if(this.__qe){return ;
}this.__qe=true;
{};
this.getBehavior().onAppear(event,event.getType()!==p);
this.__qg._updateScrollerWidths();
this.__qg._updateScrollBarVisibility();
this.__qe=false;
this.__qf=true;
},_onTableWidthChanged:function(event){if(this.__qe||!this.__qf){return ;
}this.__qe=true;
{};
this.getBehavior().onTableWidthChanged(event);
this.__qe=false;
},_onverticalscrollbarchanged:function(event){if(this.__qe||!this.__qf){return ;
}this.__qe=true;
{};
this.getBehavior().onVerticalScrollBarChanged(event);
qx.event.Timer.once(function(){if(this.__qg&&!this.__qg.isDisposed()){this.__qg._updateScrollerWidths();
this.__qg._updateScrollBarVisibility();
}},this,0);
this.__qe=false;
},_oncolumnwidthchanged:function(event){if(this.__qe||!this.__qf){return ;
}this.__qe=true;
{};
this.getBehavior().onColumnWidthChanged(event);
this.__qe=false;
},_onvisibilitychanged:function(event){if(this.__qe||!this.__qf){return ;
}this.__qe=true;
{};
...
},members:{__qh:null,renderLayout:function(j,top,k,l){this.__qh=k;
},getComputedWidth:function(){return this.__qh;
...
qx.Class.define(d,{type:c,extend:qx.core.Object,members:{_setNumColumns:function(b){throw new Error("_setNumColumns is abstract");
...
(function(){var B="__qk",A="Function",z="__qj",y="Boolean",x="minWidth",w="width",v="qx.ui.table.columnmodel.Resize",u="qx.ui.table.columnmodel.resizebehavior.Default",t="__qi",s="maxWidth",r="__ql";
qx.Class.define(u,{extend:qx.ui.table.columnmodel.resizebehavior.Abstract,construct:function(){arguments.callee.base.call(this);
this.__qi=[];
this.__qj=new qx.ui.layout.HBox();
this.__qj.connectToWidget(this);
this.__qk=new qx.util.DeferredCall(this._computeColumnsFlexWidth,this);
},properties:{newResizeBehaviorColumnData:{check:A,init:function(a){return new qx.ui.core.ColumnData();
}},initializeWidthsOnEveryAppear:{check:y,init:false},tableColumnModel:{check:v}},members:{__qj:null,__ql:null,__qi:null,__qk:null,__qm:false,setWidth:function(e,f,g){if(e>=this.__qi.length){throw new Error("Column number out of range");
}this.__qi[e].setColumnWidth(f,g);
this.__qk.schedule();
},setMinWidth:function(K,L){if(K>=this.__qi.length){throw new Error("Column number out of range");
}this.__qi[K].setMinWidth(L);
this.__qk.schedule();
},setMaxWidth:function(M,N){if(M>=this.__qi.length){throw new Error("Column number out of range");
}this.__qi[M].setMaxWidth(N);
this.__qk.schedule();
...
}}},onAppear:function(event,C){if(C===true||!this.__qm||this.getInitializeWidthsOnEveryAppear()){this._computeColumnsFlexWidth();
this.__qm=true;
...
},onVisibilityChanged:function(event){var d=event.getData();
if(d.visible){this._computeColumnsFlexWidth();
return;
}this._extendLastColumn(event);
},_setNumColumns:function(b){var c=this.__qi;
if(b<=c.length){c.splice(b,c.length);
return;
}for(var i=c.length;i<b;i++){c[i]=this.getNewResizeBehaviorColumnData()();
c[i].columnNumber=i;
}},getLayoutChildren:function(){return this.__ql;
},_computeColumnsFlexWidth:function(){this.__qk.cancel();
var H=this._getAvailableWidth();

if(H===null){return;
}var D=this.getTableColumnModel();
var F=D.getVisibleColumns();
var G=F.length;
var E=this.__qi;
var i,l;
...
for(i=0;i<G;i++){J.push(E[F[i]]);
}this.__ql=J;
this.__qn();
this.__qj.renderLayout(H,100);
for(i=0,l=J.length;i<l;i++){var I=J[i].getComputedWidth();
D.setColumnWidth(F[i],I);
}},__qn:function(){this.__qj.invalidateChildrenCache();
var R=this.__ql;
...
}}},_extendLastColumn:function(event){var m=this.getTableColumnModel();
var q=event.getData();
if(q.visible){return;
}var k=m.getVisibleColumns();
if(k.length==0){return;
}var j=this._getAvailableWidth(m);
var h=k.length;
var i;
var o;
var p=0;

for(i=0;i<h;i++){p+=m.getColumnWidth(k[i]);
}if(p<j){o=k[k.length-1];
var n=(j-(p-m.getColumnWidth(o)));
m.setColumnWidth(o,n);
}},_getResizeColumnData:function(){return this.__qi;
}},destruct:function(){this._disposeFields(t,r);
this._disposeObjects(z,B);
}});
})();
(function(){var v="number",u="object",t="qx.ui.treevirtual.MNode";
qx.Mixin.define(t,{members:{nodeGet:function(y){if(typeof (y)==u){return y;
}else if(typeof (y)==v){return this.getTableModel().getData()[y];
}else{throw new Error("Expected node object or node id");
}},nodeToggleOpened:function(m){var n;
var o;

if(typeof (m)==u){n=m;
o=n.nodeId;
}else if(typeof (m)==v){o=m;
n=this.getTableModel().getData()[o];
}else{throw new Error("Expected node object or node id");
}this.getTableModel().setState(o,{bOpened:!n.bOpened});
},nodeSetState:function(M,N){var O;

if(typeof (M)==u){O=M.nodeId;
}else if(typeof (M)==v){O=M;
}else{throw new Error("Expected node object or node id");
}this.getTableModel().setState(O,N);
},nodeSetLabel:function(I,J){this.nodeSetState(I,{label:J});
...
return F.bSelected;
},nodeSetOpened:function(d,b){var e;

if(typeof (d)==u){e=d;
}else if(typeof (d)==v){e=this.getTableModel().getData()[d];
}else{throw new Error("Expected node object or node id");
}if(b!=e.bOpened){this.nodeToggleOpened(e);
}},nodeGetOpened:function(j){var k=this.nodeGet(j);
return k.bOpened;
},nodeSetHideOpenClose:function(B,b){this.nodeSetState(B,{bHideOpenClose:b});
},nodeGetHideOpenClose:function(r){var s=this.nodeGet(r);
return s.bHideOpenClose;
},nodeSetIcon:function(p,q){this.nodeSetState(p,{icon:q});
},nodeGetIcon:function(z){var A=this.nodeGet(z);
return A.icon;
},nodeSetSelectedIcon:function(C,D){this.nodeSetState(C,{iconSelected:D});
},nodeGetSelectedIcon:function(w){var x=this.nodeGet(w);
return x.iconSelected;
},nodeSetCellStyle:function(f,g){this.nodeSetState(f,{cellStyle:g});
},nodeGetCellStyle:function(G){var H=this.nodeGet(G);
return H.cellStyle;
},nodeSetLabelStyle:function(h,i){this.nodeSetState(h,{labelStyle:i});
