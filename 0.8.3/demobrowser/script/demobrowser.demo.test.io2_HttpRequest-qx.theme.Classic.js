...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.test.io2_HttpRequest", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.test.io2_HttpRequest-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var k="qx.event.type.Event",j="Boolean",i="String",h="Last-Modified",g="basic",f="GET",e="load",d="text/plain",c="abort",b="http",O="POST",N="error",M="timeout",L="application/xml",K="X-Requested-With",J="application/json",I="text/html",H="If-Modified-Since",G='Basic ',F="Integer",r="qx.event.type.Data",s="HEAD",p="XMLHttpRequest",q='Authorization',n="change",o="Accept",l="OPTIONS",m="text/javascript",t="",u="PUT",x=':',w="__oc",z="__oa",y="application/x-www-form-urlencoded",B="DELETE",A="no-cache",v="Cache-Control",E="Content-Type",D="qx.io2.HttpRequest",C="Thu, 01 Jan 1970 00:00:00 GMT";
qx.Class.define(D,{extend:qx.core.Object,construct:function(V){arguments.callee.base.call(this);
this.__oa={};
if(V!=null){this.setUrl(V);
}},statics:{__ob:{}},events:{"change":r,"timeout":k,"load":k,"error":k,"abort":k},properties:{refresh:{check:j,init:false},data:{nullable:true},method:{check:[f,O,u,s,B,l],init:f},async:{check:j,init:true},mime:{check:[d,m,J,L,I],init:d},url:{check:i,init:t},username:{check:i,nullable:true},password:{check:i,nullable:true},auth:{check:[b,g],init:b},timeout:{check:F,nullable:true},cache:{check:j,init:false}},members:{__oc:null,__oa:null,setRequestHeader:function(T,U){this.__oa[T]=U;
},removeRequestHeader:function(a){delete this.__oa[a];
},getRequestHeader:function(bc){var bd=this.__oa[bc];

if(bd===undefined){bd=null;
}return bd;
},getResponseText:function(){var P=this.__oc;

if(P){return P.responseText;
}},getResponseXml:function(){var be=this.__oc;

if(be){return be.responseXML;
}},getResponseHeader:function(Q){var R=this.__oc;

if(R){return R.getResponseHeader(Q);
}},getAllResponseHeaders:function(){var Y=this.__oc;

if(Y){return Y.getAllResponseHeaders();
}},isNotModified:function(){var ba=this.__oc;

if(!ba){return false;
}var bb=ba.getResponseHeader(h);
return ba.status===304||qx.io2.HttpRequest.__ob[this.getUrl()]===bb;
},isSuccessful:function(){var bf=this.__oc;
return !bf||bf.isSuccessful();
},getStatusCode:function(){var X=this.__oc;

if(X){return X.status;
}},getStatusText:function(){var S=this.__oc;

if(S){return S.statusText;
}},getReadyState:function(){var bg=this.__oc;

if(bg){return bg.readyState;
}},send:function(){if(this.__oc){if(this.getReadyState()!==4){throw new Error("Request is still pending at ready state: "+this.getReadyState());
}this.__oc.dispose();
}var bj=this.__oc=new qx.bom.Request;
bj.onreadystatechange=qx.lang.Function.bind(this.__od,this);
bj.ontimeout=qx.lang.Function.bind(this.__oe,this);
bj.onload=qx.lang.Function.bind(this.__of,this);
bj.onerror=qx.lang.Function.bind(this.__og,this);
bj.onabort=qx.lang.Function.bind(this.__oh,this);
var bh=this.getUsername();
var bl=this.getPassword();

if(this.getAuth()==g){bj.setRequestHeader(q,G+qx.util.Base64.encode(bh+x+bl));
bh=bl=null;
}var bi=this.getUrl();
bj.open(this.getMethod(),bi,this.getAsync(),bh,bl);
bj.timeout=this.getTimeout();
if(!this.getCache()){bj.setRequestHeader(v,A);
}if(this.getRefresh()){bj.setRequestHeader(H,qx.io2.HttpRequest.__ob[bi]||C);
}bj.setRequestHeader(K,p);
if(this.getMethod()===O){bj.setRequestHeader(E,y);
}bj.setRequestHeader(o,this.getMime());
var bk=this.__oa;

for(var name in bk){bj.setRequestHeader(name,bk[name]);
}bj.send(this.getData());
},abort:function(){if(this.__oc){this.__oc.abort();
}},__od:qx.event.GlobalError.observeMethod(function(){this.fireDataEvent(n,this.getReadyState());
if(this.getRefresh()&&this.getReadyState()===4&&this.isSuccessful()){var W=this.getResponseHeader(h);

if(W){qx.io2.HttpRequest.__ob[this.getUrl()]=W;
}}}),__oe:qx.event.GlobalError.observeMethod(function(){if(this.hasListener(M)){this.fireEvent(M);
}}),__of:qx.event.GlobalError.observeMethod(function(){if(this.hasListener(e)){this.fireEvent(e);
}}),__og:qx.event.GlobalError.observeMethod(function(){if(this.hasListener(N)){this.fireEvent(N);
}}),__oh:qx.event.GlobalError.observeMethod(function(){if(this.hasListener(c)){this.fireEvent(c);
...
qx.Bootstrap.define(y,{construct:function(){this.__oi={};
this.__oj=this.__ot();
},statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},members:{__ok:null,__ol:null,__oj:null,__om:null,__on:null,__oo:null,__op:null,__oi:null,readyState:0,responseText:A,responseXML:null,status:0,statusText:A,timeout:0,onreadystatechange:function(){},ontimeout:function(){},onload:function(){},onerror:function(){},onabort:function(){},open:function(d,e,f,g,h){this.__ok=f;
this.__ol=qx.lang.Function.bind(this.__oq,this);
this.__oj.onreadystatechange=this.__ol;
this.__om=qx.lang.Function.bind(this.__or,this);
if(!qx.core.Variant.isSet(E,D)){this.__oj.ontimeout=this.__om;
}if(this.timeout!=null&&this.timeout>0){this.__oj.timeout=this.timeout;
}this.__oj.open(d,e,f,g,h);
if(qx.core.Variant.isSet(E,C)){if(!f){this.readyState=qx.bom.Request.OPENED;
this.__ou();
}}},send:function(p){var q=this.__oi;
if(p&&p.nodeType){p=window.XMLSerializer?new XMLSerializer().serializeToString(p):p.xml;

if(!q[B]){q[B]=z;
}}for(var r in q){this.__oj.setRequestHeader(r,q[r]);
}if(this.timeout!=null&&this.timeout>0){this.__on=window.setTimeout(this.__om,this.timeout);
}this.__oj.send(p);
if(qx.core.Variant.isSet(E,C)){if(!this.__ok){this.readyState=qx.bom.Request.OPENED;
this.__ow(this);
...
},abort:function(){this.__os();
this.onabort();
},__oq:qx.event.GlobalError.observeMethod(function(){if(qx.core.Variant.isSet(E,C)){if(!this.__ok){return;
}}this.readyState=this.__oj.readyState;
this.__ow();
if(this.__oo){this.readyState=qx.bom.Request.UNSENT;
return ;
}this.__ou();
if(this.readyState==qx.bom.Request.DONE){this.__ox();
}}),__or:qx.event.GlobalError.observeMethod(function(){this.__os();
this.ontimeout();
}),getAllResponseHeaders:function(){try{return this.__oj.getAllResponseHeaders();
}catch(t){return null;
}},getResponseHeader:function(v){try{return this.__oj.getResponseHeader(v);
}catch(l){return null;
}},setRequestHeader:function(m,n){if(n==null){delete this.__oi[m];
}else{this.__oi[m]=n;
}},removeRequestHeader:function(a,b){delete this.__oi[a];
},getRequestHeader:function(i){return this.__oi[i]||null;
},__os:function(){this.__ow();
if(this.readyState>qx.bom.Request.UNSENT){this.__oo=true;
}this.__oj.abort();
this.__ox();
},__ot:qx.core.Variant.select(E,{"default":function(){return new XMLHttpRequest;
},"mshtml":function(){if(window.ActiveXObject&&qx.xml.Document.XMLHTTP){return new ActiveXObject(qx.xml.Document.XMLHTTP);
}
if(window.XMLHttpRequest){return new XMLHttpRequest;
}}}),__ou:function(){if(this.__op===this.readyState){return;
}this.onreadystatechange();
this.__op=this.readyState;
if(this.readyState===4){if(this.isSuccessful()){this.onload();
}else{this.onerror();
}}},__ov:function(){var u=this.__oj.responseXML;

if(qx.core.Variant.isSet(E,D)){if(u&&!u.documentElement&&this.__oj.getResponseHeader(B).match(/[^\/]+\/[^\+]+\+xml/)){u=new ActiveXObject(qx.xml.Document.DOMDOC);
u.loadXML(this.__oj.responseText);
}if(u&&u.parseError!=0){return null;
}}else if(u&&u.documentElement&&u.documentElement.tagName==w){return null;
}return u;
},__ow:function(){var c=this.__oj;

try{this.responseText=c.responseText;
}catch(j){}
try{this.responseXML=this.__ov();
}catch(k){}
try{this.status=c.status;
}catch(o){}
try{this.statusText=c.statusText;
}catch(s){}if(qx.core.Variant.isSet(E,D)){if(this.status===1223){this.status=204;
}}if(!this.status&&location.protocol===x){this.status=204;
}},__ox:function(){if(this.__on){window.clearTimeout(this.__on);
}if(this.__oj){this.__oj.onreadystatechange=qx.lang.Function.empty;
...
qx.Class.define(b,{statics:{__oy:a,encode:function(d){var j=this.__oy;
var n=c;
var g,f,e;
var k,l,m,h;
var i=0;
