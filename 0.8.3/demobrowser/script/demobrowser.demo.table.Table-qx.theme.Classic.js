...
var settings = {"qx.legacy.theme": "qx.legacy.theme.ClassicRoyale", "qx.application": "demobrowser.demo.table.Table", "qx.allowUrlVariants": "true", "qx.theme": "qx.theme.Classic", "qx.allowUrlSettings": "true"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.table.Table-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var w="execute",v="changeValue",u="A date",t="Boolean",s="icon/22/actions/edit-undo.png",r="Change ID sort method",q="icon/22/status/dialog-information.png",p="Change row with ID 10",o="A number",n="icon/16/apps/office-calendar.png",M="Remove 5 rows",L="Add 10 rows",K="",J="demobrowser.demo.table.Table",I="_tableModel",H="Table",G="icon/22/actions/list-remove.png",F="Whether the the first visible row should ",E=", ",D="icon/22/actions/list-add.png",B="Selected rows:<br>",C="Keep first row",z="be rendered completely when scrolling.",A="Show selection",x="Demonstrate use of alternate sorting algorithm.",y="ID";
qx.Class.define(J,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return H;
},createTable:function(){var ba=this.createRandomRows(50);
var Y=this._tableModel=new qx.ui.table.model.Simple();
Y.setColumns([y,o,u,t]);
Y.setData(ba);
...
X.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(n,u));
return bb;
},createControls:function(){var U=new qx.ui.toolbar.ToolBar();
var S,T,V;
T=new qx.ui.toolbar.Part();
U.add(T);
S=new qx.ui.toolbar.Button(p,s);
S.addListener(w,function(b){var c=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,c[0][i]);
}this.info("Row 10 changed");
},this);
T.add(S);
S=new qx.ui.toolbar.Button(L,D);
S.addListener(w,function(f){var g=this.createRandomRows(10);
this._tableModel.addRows(g);
this.info("10 rows added");
},this);
T.add(S);
S=new qx.ui.toolbar.Button(M,G);
S.addListener(w,function(d){var e=this._tableModel.getRowCount();
this._tableModel.removeRows(e-5,5);
this.info("5 rows removed");
},this);
T.add(S);
S=new qx.ui.toolbar.Button(A,q);
S.addListener(w,function(l){var m=[];
R.getSelectionModel().iterateSelection(function(a){m.push(a+K);
});
this.showDialog(B+m.join(E));
},this);
T.add(S);
T=new qx.ui.toolbar.Part();
U.add(T);
var R=this._table;
V=new qx.ui.toolbar.CheckBox(C);
V.set({value:R.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(F+z)});
V.addListener(v,function(W){R.setKeepFirstVisibleRowComplete(this.getValue());
},V);
T.add(V);
V=new qx.ui.toolbar.CheckBox(r);
V.set({value:R.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(x)});
V.addListener(v,function(h){if(h.getData()){var k=function(bc,bd){var be=bc[arguments.callee.columnIndex];
...
var j=function(N,O){var P=N[arguments.callee.columnIndex];
var Q=O[arguments.callee.columnIndex];
...
R.getTableModel().setSortMethods(0,{ascending:k,descending:j});
}else{R.getTableModel().setSortMethods(0,null);
}},V);
T.add(V);
return U;
