...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["qx:demobrowser.demo.data.ModelDebugging-qx.theme.Classic-0.js"]],
  boot : "boot",
...
(function(){var o="auto",n="debugModel function in qx.dev.Debug.",m="?callback=demobrowser.demo.data.store.Twitter.saveResult",l="objects with properties. To see the properties, you can use the ",k="demobrowser.demo.data.ModelDebugging",j="wittemann",i="loaded",h="Every model created by the data stores consists only of qooxdoo ",g="demobrowser.demo.data.store.Twitter",f="<b>Debugging models</b><br/>",c="white",e="main",d="http://twitter.com/statuses/user_timeline/",b=".json";
qx.Class.define(k,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var r=new demobrowser.demo.data.store.Twitter(j);
...
r.addListener(i,function(){var a=r.getModel();
this.debug(qx.dev.Debug.debugProperties(a));
p.setHtml(qx.dev.Debug.debugProperties(a,10,true));
},this);
...
qx.Class.define(g,{extend:qx.data.store.Json,statics:{saveResult:function(y){this.__AH=y;
}},construct:function(w){var x=d+w+b;
arguments.callee.base.call(this,x);
},members:{_createRequest:function(s){var t=new qx.io2.ScriptLoader();
s+=m;
t.load(s,function(v){this.__AI();
},this);
},__AI:function(){var u=demobrowser.demo.data.store.Twitter.__AH;

if(u==undefined){this.setState("failed");
return;
}this._marshaler.toClass(u);
this.setModel(this._marshaler.toModel(u));
...
qx.Class.define(i,{extend:qx.ui.core.Widget,include:[qx.ui.core.MNativeOverflow],construct:function(C){arguments.callee.base.call(this);
...
},_applyHtml:function(z,A){var B=this.getContentElement();
B.setAttribute(g,z||r);
B.setStyle(n,m);
B.setStyle(h,t);
},_applyCssClass:function(u,v){this.getContentElement().setAttribute(j,u);
},_applySelectable:function(w){arguments.callee.base.call(this,w);
if(qx.core.Variant.isSet(d,l)){this.getContainerElement().setStyle(q,w?s:t);
this.getContentElement().setStyle(q,w?s:t);
...
},_applyTextColor:function(x,y){if(x){this.getContentElement().setStyle(p,qx.theme.manager.Color.getInstance().resolve(x));
}else{this.getContentElement().removeStyle(p);
}}}});
})();
(function(){var l=": ",k="",h="construct",g="object",f="-",e=":",d=") ***",c="\r\n",b="px;'>",a="): ",G="</span><br>",F="*** EXCEPTION (",E="============================================================",D="Object",C="<br>",B="null",A="Array",z="members",y="statics",x="get",s="------------------------------------------------------------",t="qx.dev.Debug",q="\n",r="*** TOO MUCH RECURSION: not displaying ***",o="Object, count=",p="  ",m="<span style='padding-left:",n=" ",u="Array, length=",v="undefined",w="index(";
qx.Class.define(t,{statics:{debugObject:function(bk,bl,bm){qx.log.Logger.debug(this,qx.dev.Debug.debugObjectToString(bk,bl,bm,false));
},debugObjectToString:function(R,S,T,U){if(!T){T=10;
}var bb=(U?G:q);
var X=function(L){var M;

if(!U){M=k;

for(var i=0;i<L;i++){M+=p;
}}else{M=m+(L*8)+b;
}return M;
};
var ba=k;
var W=function(H,I,J){if(I>J){ba+=(X(I)+r+bb);
return;
}if(typeof (H)!=g){ba+=X(I)+H+bb;
return;
}for(var K in H){if(typeof (H[K])==g){if(H[K] instanceof Array){ba+=X(I)+K+l+A+bb;
}else if(H[K]===null){ba+=X(I)+K+l+B+bb;
continue;
}else if(H[K]===undefined){ba+=X(I)+K+l+v+bb;
continue;
}else{ba+=X(I)+K+l+D+bb;
}W(H[K],I+1,J);
}else{ba+=X(I)+K+l+H[K]+bb;
}}};
if(S){ba+=X(0)+S+bb;
}
if(R instanceof Array){ba+=X(0)+u+R.length+e+bb;
}else if(typeof (R)==g){var V=0;
...
try{W(R,0,T);
}catch(Q){ba+=X(0)+F+Q+d+bb;
}ba+=X(0)+E+bb;
return ba;
},getFunctionName:function(N,O){var P=N.self;

if(!P){return null;
}while(N.wrapper){N=N.wrapper;
}
switch(O){case h:return N==P?h:null;
case z:return qx.lang.Object.getKeyFromValue(P,N);
case y:return qx.lang.Object.getKeyFromValue(P.prototype,N);
default:if(N==P){return h;
}return (qx.lang.Object.getKeyFromValue(P.prototype,N)||qx.lang.Object.getKeyFromValue(P,N)||null);
...
}else if(qx.Class.hasInterface(bc.constructor,qx.data.IListData)){for(var i=0;i<bc.length;i++){for(var j=0;j<bf;j++){bi+=f;
}bi+=w+i+a+this.debugProperties(bc.getItem(i),bd-1,be,bf+1)+bg;
}return bi+bg;
}else if(bc.constructor!=null){var bj=bc.constructor.$$properties;

for(var bh in bj){bi+=bg;
for(var j=0;j<bf;j++){bi+=f;
}bi+=n+bh+l+this.debugProperties(bc[x+qx.lang.String.firstUp(bh)](),bd-1,be,bf+1);
}return bi;
}return k;
}}});
})();
(function(){var s="success",r="head",q="complete",p="loaded",o="readystatechange",n="__xt",m="error",l="load",k="script",j="fail",d="__xq",i="qx.io2.ScriptLoader",h="qx.client",c="text/javascript",b="__xu",g="__xr",f="abort";
qx.Class.define(i,{extend:qx.core.Object,construct:function(){arguments.callee.base.call(this);
this.__xq=qx.lang.Function.bind(this.__xw,this);
this.__xr=document.createElement(k);
},members:{__xs:null,__xt:null,__xu:null,__xq:null,__xr:null,load:function(t,u,v){if(this.__xs){throw new Error("Another request is still running!");
}this.__xs=true;
...
this.__xu=v||window;
x.type=c;
x.onerror=x.onload=x.onreadystatechange=this.__xq;
x.src=t;
w.appendChild(x);
},abort:function(){if(this.__xs){this.__xv(f);
}},__xv:function(status){var y=this.__xr;
y.onerror=y.onload=y.onreadystatechange=null;
document.getElementsByTagName(r)[0].removeChild(y);
delete this.__xs;
this.__xt.call(this.__xu,status);
},__xw:qx.event.GlobalError.observeMethod(qx.core.Variant.select(h,{"mshtml":function(e){var a=this.__xr.readyState;

if(a==p){this.__xv(s);
}else if(a==q){this.__xv(s);
}else{return;
}},"default":function(e){if(qx.lang.Type.isString(e)||e.type===m){this.__xv(j);
}else if(e.type===l){this.__xv(s);
}else if(e.type===o&&(e.target.readyState===q||e.target.readyState===p)){this.__xv(s);
}else{return;
}}}))},destruct:function(){this._disposeFields(g,d,n,b);
}});
})();
