...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.showcase.Maps","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.showcase.Maps-qx.theme.Simple.3c70a472079b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var p="mshtml",o="engine.name",n="Google Maps",m="<b>",l="center_changed",k="Karlsruhe",j="Yahoo Maps",i="</b>",h="Could not create Google map!<br/>",g="Could not create Yahoo map!<br/>",c="demobrowser.demo.showcase.Maps",f='zIndex',e="IE 10 is not yet supported by Yahoo maps.",b="browser.documentmode",a="appear",d="main";qx.Class.define(c,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var q=this._createYahooMap();var r=this._createGoogleMap();this.getRoot().add(this._createMapContainer(j,q),{left:20,top:20});this.getRoot().add(this._createMapContainer(n,r),{left:490,top:20});}
,_createMapContainer:function(s,t){var v=new qx.ui.container.Composite();v.setLayout(new qx.ui.layout.VBox().set({spacing:10}));v.setWidth(450);v.setHeight(400);var u=new qx.ui.basic.Label(m+s+i).set({rich:true});v.add(u);v.add(t);return v;}
,_createYahooMap:function(){var w=new qx.ui.core.Widget().set({width:450,height:400,decorator:d});w.addListenerOnce(a,function(){try{var y=new YMap(w.getContentElement().getDomElement());y.addTypeControl();y.setMapType(YAHOO_MAP_REG);y.drawZoomAndCenter(k,5);}
catch(z){var x;if(qx.core.Environment.get(o)==p&&qx.core.Environment.get(b)>9){x=e;}
else {x=g+z.toString();}
;this.getContentElement().getDomElement().innerHTML+=x;}
;}
);return w;}
,_createGoogleMap:function(){var A=new qx.ui.core.Widget().set({width:450,height:400});A.setDecorator(d);A.addListenerOnce(a,function(){try{var C=new google.maps.Map(A.getContentElement().getDomElement(),{zoom:13,mapTypeId:google.maps.MapTypeId.ROADMAP});google.maps.event.addListenerOnce(C,l,function(){window.setTimeout(function(){var D=A.getContentElement().getStyle(f);A.getContentElement().getDomElement().style.zIndex=D;}
,500);}
);C.setCenter(new google.maps.LatLng(49.011899,8.403311));}
catch(E){var B=h+E.toString();this.getContentElement().getDomElement().innerHTML+=B;}
;}
);return A;}
}});}
)();
