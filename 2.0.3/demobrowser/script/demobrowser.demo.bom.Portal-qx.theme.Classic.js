...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.bom.Portal","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Portal-qx.theme.Classic.fe304208a060.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="groupBoxes",h="demobrowser.demo.bom.portal.box.Manager",g="boxData",f="qx.event.type.Event",e="boxes",d="demobrowser.demo.bom.portal.box.Box",c="_applyActiveBox",b="loaded",a="singleton";qx.Class.define(h,{type:a,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__Ol=[];this.__Om=[];this.__On={};}
,statics:{dataKeyRoot:g,dataKeyGroupBoxes:m,dataKeyBoxes:e},events:{"loaded":f},properties:{activeBox:{init:null,check:d,apply:c}},members:{__Ol:null,__Om:null,__On:null,_applyActiveBox:function(n,o){if(o!=null){o.setActive(false);}
;n.setActive(true);}
...
,__bf:function(p){var t,w,u,v,x,p;var q=p[demobrowser.demo.bom.portal.box.Manager.dataKeyGroupBoxes];var s=p[demobrowser.demo.bom.portal.box.Manager.dataKeyBoxes];for(var w in q){t=q[w];u=t.boxes;w=t.id;v={id:w,element:document.getElementById(w)};this.__Om.push(v);this.__On[w]=[];for(var k=0,l=u.length;k<l;k++){p=s[u[k]];var r=document.getElementById(p.id);var x=new demobrowser.demo.bom.portal.box.Box(p,r,null,w);this.__Ol.push(x);this.__On[w].push(x);}
;}
;}
,getBoxes:function(){return this.__Ol;}
,getGroupBoxes:function(){return this.__Om;}
,updateGroupBoxMembers:function(y,z,A){var B=qx.dom.Hierarchy.getElementIndex(A.getElement());qx.lang.Array.remove(this.__On[z],A);qx.lang.Array.insertAt(this.__On[y],A,B);}
,getGroupBoxDataOfBox:function(C){for(var i=0,j=this.__Om.length,D;i<j;i++){D=this.__On[this.__Om[i].id];for(var k=0,l=D.length;k<l;k++){if(D[k].getId()===C){return this.__Om[i];}
;}
;}
;return null;}
},destruct:function(){this.__Ol=this.__Om=this.__On=null;}
});}
)();
(function(){var s="click",r="1px solid #444444",q="height",p="position",o="zIndex",n="_applyActive",m="width",l="box_content",k=" It does contain no UI widget code at all.",j="relative",c="demobrowser.demo.bom.portal.box.Box",i="1px solid orange",g="This is a demo application showing the low-level capabilities.",b="div",a="String",f="<br/><br/>",d="border",h="Boolean";qx.Class.define(c,{extend:qx.core.Object,construct:function(t,u,v,w){qx.core.Object.call(this);this.__Oo=t;this.__bV=t.id;if(u!=null){qx.bom.element.Style.set(u,m,t.width);qx.bom.element.Style.set(u,q,t.height);this.__Or(u,v);}
;this.setElement(u);this.setGroupBoxId(w);this.__Op=null;this.__Oq=null;if(t.resizable){this.setResizable(true);var x={minWidth:parseInt(t.minWidth),minHeight:parseInt(t.minHeight),maxWidth:parseInt(t.maxWidth),maxHeight:parseInt(t.maxHeight),handles:t.resizeHandles};this.__Op=new demobrowser.demo.bom.portal.box.Resizable(this,x);}
;if(t.draggable){this.setDraggable(true);this.__Oq=new demobrowser.demo.bom.portal.box.Draggable(this);}
;this.__Os();}
,properties:{element:{init:null},groupBoxId:{init:null,check:a},active:{check:h,init:false,apply:n},resizable:{check:h,init:false},draggable:{check:h,init:false}},members:{__bV:null,__Oo:null,__Oq:null,__Op:null,_applyActive:function(y,z){if(y){qx.bom.element.Style.set(this.getElement(),d,i);}
else {qx.bom.element.Style.set(this.getElement(),d,r);}
;}
,getId:function(){return this.getElement()?this.getElement().id:null;}
,getData:function(){return this.__Oo;}
,getDraggableComponent:function(){return this.__Oq;}
,getResizableComponent:function(){return this.__Op;}
,__Or:function(A,B){if(B){for(var C in B){qx.bom.element.Style.set(C,B[C]);}
;}
;qx.bom.element.Style.set(A,p,j);qx.bom.element.Style.set(A,o,0);qx.bom.Element.addListener(A,s,this.__Ot,this,true);}
,__Os:function(){var E=g+k;E=E+f+E+f+E;var D=qx.dom.Element.create(b,{"class":l});D.innerHTML=E;qx.dom.Element.insertEnd(D,this.getElement());}
,__Ot:function(e){demobrowser.demo.bom.portal.box.Manager.getInstance().setActiveBox(this);}
},destruct:function(){this.resetElement();this.__Oo=this.__Op=this.__Oq=null;}
});}
)();
...
(function(){var k="visibility",j="_",i="mouseover",h="mouseout",g="visible",f="hidden",d="div",c="orientation",b="demobrowser.demo.bom.portal.box.Resizable",a="Handle";qx.Class.define(b,{extend:qx.core.Object,construct:function(l,m){qx.core.Object.call(this);this.__Ou=l;this.__ko=this.__Ou.getElement();this.__Ov={};this.__Ow=false;this.__Ox={orientation:null,lastCoord:{left:null,top:null}};this.__Oy={minWidth:10,minHeight:10,maxWidth:null,maxHeight:null,autoHide:true,handles:3};for(var name in this.__Oy){if(m[name]){this.__Oy[name]=m[name];}
;}
;this.__Or();}
,members:{__Ox:null,__Ou:null,__ko:null,__Ov:null,__Oy:null,__Ow:false,getBox:function(){return this.__Ou;}
,__Or:function(){switch(this.__Oy.handles){case 1:this.__Ov.east=null;break;case 2:this.__Ov.south=null;break;case 3:this.__Ov.south=null;this.__Ov.southeast=null;this.__Ov.east=null;break;};for(var o in this.__Ov){var n=this.__Ov[o]=qx.dom.Element.create(d);qx.bom.element.Attribute.set(n,c,o);qx.dom.Element.insertEnd(n,this.__ko);if(this.__ko.id){n.id=this.__ko.id+j+o;}
;this.__Oz(o);this.__OA(o);}
;}
,__Oz:function(p){qx.bom.element.Class.add(this.__Ov[p],p+a);qx.bom.element.Style.set(this.__Ov[p],k,this.__Oy.autoHide?f:g);}
,__OA:function(q){var s=qx.event.Registration;var r=this.__Ov[q];s.addListener(r,"mousedown",this.__OD,this);s.addListener(document,"mouseup",this.__OE,this);if(this.__Oy.autoHide){s.addListener(this.__Ou.getElement(),"mouseover",this.__OB,this);s.addListener(this.__Ou.getElement(),"mouseout",this.__OC,this);}
;}
,__OB:function(e){for(var t in this.__Ov){qx.bom.element.Style.set(this.__Ov[t],"visibility","visible");}
;}
,__OC:function(e){if(!this.__Ow){for(var u in this.__Ov){qx.bom.element.Style.set(this.__Ov[u],"visibility","hidden");}
;}
;}
,__OD:function(e){e.stopPropagation();if(e.isLeftPressed()){qx.event.Registration.addListener(document,"mousemove",this.__OF,this,true);this.__Ow=true;var v=e.getTarget();this.__Ox.orientation=qx.bom.element.Attribute.get(v,"orientation");this.__Ox.lastCoord.left=e.getDocumentLeft();this.__Ox.lastCoord.top=e.getDocumentTop();demobrowser.demo.bom.portal.box.Util.bringToFront(this.__ko);}
;}
,__OE:function(e){e.stopPropagation();if(this.__Ow){qx.event.Registration.removeListener(document,"mousemove",this.__OF,this,true);demobrowser.demo.bom.portal.box.Util.sendToBack(this.__ko);if(this.__Oy.autoHide){for(var w in this.__Ov){qx.bom.element.Style.set(this.__Ov[w],"visibility","hidden");}
;}
;this.__Ow=false;qx.bom.Selection.clear(document.body);}
;}
,__OF:function(e){e.stopPropagation();if(this.__Ow){var x={left:null,top:null};switch(this.__Ox.orientation){case "south":x.top=e.getDocumentTop();break;case "southeast":x.top=e.getDocumentTop();x.left=e.getDocumentLeft();break;case "east":x.left=e.getDocumentLeft();break;};this.__OG(x,this.__Ox.lastCoord);this.__Ox.lastCoord.left=e.getDocumentLeft();this.__Ox.lastCoord.top=e.getDocumentTop();}
;}
,__OG:function(y,z){var G=qx.bom.element.Dimension;var D=demobrowser.demo.bom.portal.box.Manager.getInstance().getGroupBoxDataOfBox(this.__Ou.getId());var F=G.getContentSize(D.element);if(y.left){var C=y.left-z.left;if(C!==0){var B=G.getContentWidth(this.__ko)+C;var E=G.getWidth(this.__ko)+C;if((this.__Oy.minWidth==null||B>this.__Oy.minWidth)&&(this.__Oy.maxWidth==null||B<=this.__Oy.maxWidth)){if(F.width>=E){qx.bom.element.Style.set(this.__ko,"width",B+'px');this.__Ou.getData().width=B;}
;}
;}
;}
;if(y.top){var C=y.top-z.top;if(C!==0){var A=G.getContentHeight(this.__ko)+C;if((this.__Oy.minHeight==null||A>this.__Oy.minHeight)&&(this.__Oy.maxHeight==null||A<=this.__Oy.maxHeight)){qx.bom.element.Style.set(this.__ko,"height",A+"px");}
;}
;}
;}
},destruct:function(){if(this.__Oy.autoHide){qx.event.Registration.removeListener(this.__Ou.getElement(),i,this.__OB,this);qx.event.Registration.removeListener(this.__Ou.getElement(),h,this.__OC,this);}
;this.__Ou=this.__Oy=this.__ko=this.__Ov=null;this.__Ox=null;}
});}
)();
...
(function(){var c="auto",b="demobrowser.demo.bom.portal.box.Util",a="zIndex";qx.Class.define(b,{statics:{__OH:1000,bringToFront:function(d){var e=qx.bom.element.Style.get(d,a);e=e==c?0:e;qx.bom.element.Style.set(d,a,e+this.__OH);}
,sendToBack:function(f){var g=qx.bom.element.Style.get(f,a);qx.bom.element.Style.set(f,a,g-this.__OH);}
}});}
)();
...
(function(){var d="mousemove",c="demobrowser.demo.bom.portal.box.Draggable",b="mouseup",a="dragstart";qx.Class.define(c,{extend:qx.core.Object,construct:function(f){qx.core.Object.call(this);this.__Ou=f;this.__ko=f.getElement();this.__OI=null;this.__OJ=null;this.__Or();}
,members:{__Ou:null,__ko:null,__OI:null,__OJ:null,getBox:function(){return this.__Ou;}
,getElement:function(){return this.__ko;}
,__Or:function(){this.__OK();this.__OA();}
,__OK:function(){this.__OI=qx.dom.Element.create("div");qx.bom.element.Class.add(this.__OI,"dragHandle");qx.dom.Element.insertBegin(this.__OI,this.__ko);}
,__OA:function(){qx.bom.Element.addListener(this.__OI,"mousedown",this.__nP,this);qx.bom.Element.addListener(this.__OI,"mouseover",function(e){qx.bom.element.Style.set(this,"cursor","move");}
,this.__OI);qx.bom.Element.addListener(this.__OI,"dragstart",this.__Lt,this);qx.bom.Element.addListener(this.__OI,"dragend",demobrowser.demo.bom.portal.dragdrop.Manager.getInstance().stopSession,demobrowser.demo.bom.portal.dragdrop.Manager.getInstance());qx.bom.Element.addListener(this.__OI,"drag",this.__ON,this);}
,__nP:function(e){if(e.isLeftPressed()){this.__OJ={left:e.getDocumentLeft()-qx.bom.element.Location.getLeft(this.__ko),top:e.getDocumentTop()-qx.bom.element.Location.getTop(this.__ko)};qx.bom.Element.addListener(document.body,b,this.__OM,this,true);this.__OL();qx.event.Registration.fireEvent(this.__OI,a,qx.event.type.Event);}
;}
,__OL:qx.core.Environment.select("engine.name",{"mshtml":function(){var h=this;var g=qx.lang.Function.bind(this.__OM,h);document.getElementsByTagName("html")[0].onmouseleave=g;}
,"default":qx.lang.Function.empty}),__OM:function(e){try{e.stopPropagation();}
catch(i){}
;if(demobrowser.demo.bom.portal.dragdrop.Manager.getInstance().isSessionActive()){qx.event.Registration.removeListener(document.body,"mousemove",this.__ON,this,true);demobrowser.demo.bom.portal.dragdrop.Manager.getInstance().stopSession();}
;qx.bom.Element.removeListener(document.body,"mouseup",this.__OM,this,true);}
,__Lt:function(e){if(demobrowser.demo.bom.portal.dragdrop.Manager.getInstance().isSessionActive()){return;}
;demobrowser.demo.bom.portal.box.Manager.getInstance().setActiveBox(this.__Ou);demobrowser.demo.bom.portal.dragdrop.Manager.getInstance().startSession(this.__Ou);qx.event.Registration.addListener(document.body,d,this.__ON,this,true);}
,__ON:function(e){e.stopPropagation();var j=e.getDocumentLeft()-this.__OJ.left;var top=e.getDocumentTop()-this.__OJ.top;qx.event.Timer.once(function(){this.checkGroupBox(j,parseInt(this.getActiveBox().getData().width));this.onDragMove(top,j);}
,demobrowser.demo.bom.portal.dragdrop.Manager.getInstance(),0);}
},destruct:function(){this.__Ou=this.__OI=this.__ko=this.__OJ=null;}
});}
)();
(function(){var m="Boolean",l="left",k="demobrowser.demo.bom.portal.dragdrop.Manager",h="end",g="border",f="top",d="demobrowser.demo.bom.portal.box.Box",c="ease-out",b="singleton",a="px";qx.Class.define(k,{type:b,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__OO={top:null,height:null};this.__OP={top:0,left:0};}
,properties:{activeBox:{check:d,init:null,nullable:true},sessionActive:{check:m,init:false}},members:{__OO:null,__OQ:null,__OR:false,__OP:null,__OS:null,__OT:null,startSession:function(n){this.setSessionActive(true);this.setActiveBox(n);this.__OQ=n.getGroupBoxId();this.__OU();if(this.__OS==null){this.__OS={};var o=demobrowser.demo.bom.portal.box.Manager.getInstance().getGroupBoxes();for(var i=0,j=o.length;i<j;i++){this.__OS[o[i].element.id]={left:qx.bom.element.Location.getLeft(o[i].element),right:qx.bom.element.Location.getLeft(o[i].element)+qx.bom.element.Dimension.getWidth(o[i].element)};}
;}
;}
,__OU:function(){if(!this.__OT){this.__OT=qx.dom.Element.create("div");}
;var r=this.getActiveBox().getElement();var p=qx.bom.element.Dimension.getContentSize(r);this.__OV(r,this.__OT);qx.bom.element.Class.add(this.__OT,qx.bom.element.Class.get(r));var q="opacity:0.5;margin:0;position:absolute;"+"top:"+qx.bom.element.Location.getTop(r)+"px;"+"left:"+qx.bom.element.Location.getLeft(r)+"px;"+"width:"+p.width+"px;"+"height:"+p.height+"px";qx.bom.element.Style.setCss(this.__OT,q);qx.bom.element.Style.set(r,"border","1px dashed red");qx.dom.Node.getBodyElement(r).appendChild(this.__OT);demobrowser.demo.bom.portal.box.Util.bringToFront(this.__OT);}
,__OV:function(s,t){while(s.firstChild){t.appendChild(s.firstChild);}
;}
,stopSession:function(e){this.setSessionActive(false);var y=this.getActiveBox();var v=y.getElement();demobrowser.demo.bom.portal.box.Manager.getInstance().updateGroupBoxMembers(y.getGroupBoxId(),this.__OQ,y);y.setGroupBoxId(this.__OQ);var x={left:qx.bom.element.Location.getLeft(this.__OT)+a,top:qx.bom.element.Location.getTop(this.__OT)+a};var w={left:qx.bom.element.Location.getLeft(v)+a,top:qx.bom.element.Location.getTop(v)+a};var z={timing:c,duration:300,keep:100,keyFrames:{'0':x,'100':w}};var u=qx.bom.element.Animation.animate(this.__OT,z);u.addListener(h,function(){this.__OV(this.__OT,v);qx.bom.element.Style.reset(v,g);this.__OT.parentNode.removeChild(this.__OT);if(y.isActive()){y._applyActive(true);}
else {demobrowser.demo.bom.portal.box.Manager.getInstance().setActiveBox(y);}
;}
,this);}
,onDragMove:function(top,A){if(this.__OP.left!=A){qx.bom.element.Style.set(this.__OT,l,A+a);}
;if(this.__OP.top!=top){qx.bom.element.Style.set(this.__OT,f,top+a);}
;var B=this.getActiveBox().getElement();if(this.__OO.top==null){this.__OO.top=qx.bom.element.Location.getTop(B);}
;if(this.__OO.height==null){this.__OO.height=qx.bom.element.Dimension.getHeight(B);}
;if(this.__OR==false&&top>=this.__OO.top&&top<=(this.__OO.top+this.__OO.height)){return;}
;var C;if(this.__OR){C=qx.dom.Hierarchy.getFirstDescendant(document.getElementById(this.__OQ));while(C!=null){if(this.__OW(true,top,B,C)){this.__OR=false;return;}
;C=qx.dom.Hierarchy.getNextElementSibling(C);}
;}
else {C=B;}
;if(top-this.__OP.top>0){C=qx.dom.Hierarchy.getNextElementSibling(C);while(C!=null){if(C){if(this.__OW(true,top,B,C)){return;}
;}
;C=qx.dom.Hierarchy.getNextElementSibling(C);}
;}
else {var C=qx.dom.Hierarchy.getPreviousElementSibling(C);while(C!=null){if(C){if(this.__OW(false,top,B,C)){return;}
;}
;C=qx.dom.Hierarchy.getPreviousElementSibling(C);}
;}
;this.__OP.left=A;this.__OP.top=top;}
,__OW:function(D,top,E,F){var G=qx.bom.element.Location.getTop(F);var I=parseInt(qx.bom.element.Style.get(F,"paddingTop"));var J=parseInt(qx.bom.element.Style.get(F,"paddingBottom"));var H=qx.bom.element.Dimension.getContentHeight(F)+J+I;var K;if(D){if(top>=(G+(H/3))){K=qx.dom.Hierarchy.getNextElementSibling(F);if(K!=E||K==null){qx.dom.Element.insertAfter(E,F);this.__OO.top=qx.bom.element.Location.getTop(E);}
;return true;}
;}
else {if(top<=(G+H*2/3)){K=qx.dom.Hierarchy.getPreviousElementSibling(F);if(K!=E||K==null){qx.dom.Element.insertBefore(E,F);this.__OO.top=qx.bom.element.Location.getTop(E);}
;return true;}
;}
;return false;}
,checkGroupBox:function(L,M){if(this.__OS[this.__OQ].left<=L&&this.__OS[this.__OQ].right>=L){return;}
;for(var N in this.__OS){if(N!=this.__OQ){if(this.__OS[N].left<=L+M&&this.__OS[N].right>=L){this.__OQ=N;this.__OR=true;return;}
;}
;}
;}
},destruct:function(){this.__OO=this.__OP=this.__OQ=null;this.__OX=this.__OT=this.__OS=null;}
});}
)();
...
(function(){var k="No 'keyFrames' given > 0",j="end",i="Anni",h="keyFrames",g="The key 'reverse' is deprecated: Please use the method ",f="Keyframe position needs to be between 0 and 100",d="duration",c=":",b="} ",a="Unknown key '",G="forwards",F="' in the animation description.",E="end-event",D="css.animation",C="ms ",B="% {",A="'animateReverse' instead.",z="linear",y=";",x="qx.bom.element.AnimationCss",r="keyframes",s="fill-mode",p="repeat",q="timing",n="keep",o="origin",l="reverse",m="alternate",t="qx.debug",u="name",w="",v=" ";qx.Bootstrap.define(x,{statics:{__tP:null,__tQ:i,__bV:0,__tR:{},__tS:{"scale":true,"rotate":true,"skew":true,"translate":true},__tT:qx.core.Environment.get(D),animateReverse:function(H,I,J){return this._animate(H,I,J,true);}
,animate:function(K,L,M){return this._animate(K,L,M,false);}
,_animate:function(N,O,P,Q){this.__tW(O);if(O.hasOwnProperty(l)){Q=O.reverse;if(qx.core.Environment.get(t)){qx.log.Logger.warn(g+A);qx.log.Logger.trace();}
;}
;if(qx.core.Environment.get(t)){this.__tX(O);}
;var T=O.keep;if(T!=null&&(Q||(O.alternate&&O.repeat%2==0))){T=100-T;}
;if(!this.__tP){this.__tP=qx.bom.Stylesheet.createElement();}
;var S=O.keyFrames;if(P==undefined){P=O.duration;}
;if(this.__tT!=null){var name=this.__tY(S,Q);var R=name+v+P+C+O.repeat+v+O.timing+v+(O.alternate?m:w);var U=this.__tT[E];qx.bom.Event.addNativeListener(N,U,this.__tU);N.style[qx.lang.String.camelCase(this.__tT[u])]=R;if(T&&T==100&&this.__tT[s]){N.style[this.__tT[s]]=G;}
;}
;var V=new qx.bom.element.AnimationHandle();V.desc=O;V.el=N;V.keep=T;N.$$animation=V;if(O.origin!=null){qx.bom.element.Transform.setOrigin(N,O.origin);}
;if(this.__tT==null){window.setTimeout(function(){qx.bom.element.AnimationCss.__tU({target:N});}
,0);}
;return V;}
,__tU:function(e){var W=e.target;var X=W.$$animation;if(!X){return;}
;var ba=X.desc;if(qx.bom.element.AnimationCss.__tT!=null){var Y=qx.lang.String.camelCase(qx.bom.element.AnimationCss.__tT[u]);W.style[Y]=w;qx.bom.Event.removeNativeListener(W,qx.bom.element.AnimationCss.__tT[u],qx.bom.element.AnimationCss.__tU);}
;if(ba.origin!=null){qx.bom.element.Transform.setOrigin(W,w);}
;qx.bom.element.AnimationCss.__tV(W,ba.keyFrames[X.keep]);W.$$animation=null;X.el=null;X.ended=true;X.emit(j,W);}
,__tV:function(bb,bc){var be;for(var bd in bc){if(bd in qx.bom.element.AnimationCss.__tS){if(!be){be={};}
;be[bd]=bc[bd];}
else {bb.style[qx.lang.String.camelCase(bd)]=bc[bd];}
;}
;if(be){qx.bom.element.Transform.transform(bb,be);}
;}
,__tW:function(bf){if(!bf.hasOwnProperty(m)){bf.alternate=false;}
;if(!bf.hasOwnProperty(n)){bf.keep=null;}
;if(!bf.hasOwnProperty(p)){bf.repeat=1;}
;if(!bf.hasOwnProperty(q)){bf.timing=z;}
;if(!bf.hasOwnProperty(o)){bf.origin=null;}
;}
,__tX:qx.core.Environment.select(t,{"true":function(bg){var bh=[o,d,n,h,p,q,m,l];for(var name in bg){if(!(bh.indexOf(name)!=-1)){qx.Bootstrap.warn(a+name+F);}
;}
;if(bg.keyFrames==null){qx.Bootstrap.warn(k);}
else {for(var bi in bg.keyFrames){if(bi<0||bi>100){qx.Bootstrap.warn(f);}
;}
;}
;}
,"default":null}),__tY:function(frames,bj){var bm=w;for(var bp in frames){bm+=(bj?-(bp-100):bp)+B;var bl=frames[bp];var bo;for(var bk in bl){if(bk in this.__tS){if(!bo){bo={};}
;bo[bk]=bl[bk];}
else {bm+=bk+c+bl[bk]+y;}
;}
;if(bo){bm+=qx.bom.element.Transform.getCss(bo);}
;bm+=b;}
;if(this.__tR[bm]){return this.__tR[bm];}
;var name=this.__tQ+this.__bV++;var bn=this.__tT[r]+v+name;qx.bom.Stylesheet.addRule(this.__tP,bn,bm);this.__tR[bm]=name;return name;}
}});}
)();
...
(function(){var g="Element",f="qx.bom.element.AnimationHandle",e="play-state",d="paused",c="running",b="",a="css.animation";qx.Bootstrap.define(f,{extend:qx.event.Emitter,construct:function(){var h=qx.core.Environment.get(a);this.__ua=h&&h[e];this.__ub=true;}
,events:{"end":g},members:{__ua:null,__ub:false,__uc:false,isPlaying:function(){return this.__ub;}
,isEnded:function(){return this.__uc;}
,pause:function(){if(this.el){this.el.style[this.__ua]=d;this.el.$$animation.__ub=false;if(this.animationId&&qx.bom.element.AnimationJs){qx.bom.element.AnimationJs.pause(this);}
;}
;}
,play:function(){if(this.el){this.el.style[this.__ua]=c;this.el.$$animation.__ub=true;if(this.i!=undefined&&qx.bom.element.AnimationJs){qx.bom.element.AnimationJs.play(this);}
;}
;}
,stop:function(){if(this.el&&qx.core.Environment.get(a)){this.el.style[this.__ua]=b;this.el.style[qx.core.Environment.get(a).name]=b;this.el.$$animation.__ub=false;this.el.$$animation.__uc=true;}
...
,getCss:function(H){var J=this.__uf(H);if(this.__ue!=null){var I=this.__ue[l];return qx.lang.String.hyphenate(I)+n+J+o;}
;return j;}
,setOrigin:function(K,L){if(this.__ue!=null){K.style[this.__ue[e]]=L;}
;}
,getOrigin:function(M){if(this.__ue!=null){return M.style[this.__ue[e]];}
;return j;}
,setStyle:function(N,O){if(this.__ue!=null){N.style[this.__ue[h]]=O;}
;}
,getStyle:function(P){if(this.__ue!=null){return P.style[this.__ue[h]];}
;return j;}
,setPerspective:function(Q,R){if(this.__ue!=null){Q.style[this.__ue[d]]=R+u;}
;}
,getPerspective:function(S){if(this.__ue!=null){return S.style[this.__ue[d]];}
;return j;}
,setPerspectiveOrigin:function(T,U){if(this.__ue!=null){T.style[this.__ue[a]]=U;}
;}
,getPerspectiveOrigin:function(V){if(this.__ue!=null){var X=V.style[this.__ue[a]];if(X!=j){return X;}
else {var Y=V.style[this.__ue[a]+b];var W=V.style[this.__ue[a]+f];if(Y!=j){return Y+s+W;}
;}
;}
;return j;}
,setBackfaceVisibility:function(ba,bb){if(this.__ue!=null){ba.style[this.__ue[m]]=bb?k:q;}
;}
,getBackfaceVisibility:function(bc){if(this.__ue!=null){return bc.style[this.__ue[m]]==k;}
;return true;}
,__uf:function(bd){var bg=j;for(var bf in bd){var be=bd[bf];if(qx.Bootstrap.isArray(be)){for(var i=0;i<be.length;i++){if(be[i]==undefined){continue;}
;bg+=bf+this.__ud[i]+g;bg+=be[i];bg+=c;}
;}
...
;var R=L.keyFrames;var P=this.__uo(R);var Q=this.__un(M,P);var S=parseInt(M/Q,10);this.__ui(R,K);var T=this.__uj(S,Q,P,R,M,L.timing);var O=new qx.bom.element.AnimationHandle();if(N){T.reverse();O.reverse=true;}
;O.desc=L;O.el=K;O.delta=T;O.stepTime=Q;O.steps=S;K.$$animation=O;O.i=0;O.initValues={};O.repeatSteps=this.__ul(S,L.repeat);return this.play(O);}
,__ui:function(U,V){var W={};for(var ba in U){for(var name in U[ba]){if(W[name]==undefined){var X=U[ba][name];if(typeof X==g){W[name]=X.substring((parseInt(X,10)+p).length,X.length);}
else {W[name]=p;}
;}
;}
;}
...
;if(Y[name]==p&&this.__uh.indexOf(W[name])!=-1){Y[name]=e+W[name];}
;}
;}
;}
;}
,__uj:function(bb,bc,bd,be,bf,bg){var bo=new Array(bb);var bq=1;bo[0]=be[0];var bi=be[0];var bk=be[bd[bq]];for(var i=1;i<bo.length;i++){if(i*bc/bf*100>bd[bq]){bi=bk;bq++;bk=be[bd[bq]];}
;bo[i]={};for(var name in bk){var bp=bk[name]+p;if(bp.charAt(0)==C){var bj=qx.util.ColorUtil.cssStringToRgb(bi[name]);var bn=qx.util.ColorUtil.cssStringToRgb(bp);var bh=[];for(var j=0;j<bj.length;j++){var bl=bj[j]-bn[j];bh[j]=parseInt(bj[j]-bl*this.__uk(bg,i/bb),10);}
;bo[i][name]=qx.util.ColorUtil.rgbToHexString(bh);}
else if(!isNaN(parseInt(bp,10))){var bm=bp.substring((parseInt(bp,10)+p).length,bp.length);var bl=parseFloat(bp)-parseFloat(bi[name]);bo[i][name]=(parseFloat(bi[name])+bl*this.__uk(bg,i/bb))+bm;}
...
;}
;}
;qx.bom.element.AnimationJs.__um(br.el,bt);br.i++;if(br.i%br.steps==0){if(br.desc.alternate){br.delta.reverse();}
;}
...
;this.__um(bw,bz.keyFrames[by]);}
else {this.__um(bw,bx);}
;bw.$$animation=null;bv.el=null;bv.ended=true;bv.animationId=null;bv.emit(h,bw);}
,__uk:function(bA,x){if(bA==B){var a=[3.1223e-7,0.0757,1.2646,-0.167,-0.4387,0.2654];}
...
;return y;}
,__ul:function(bB,bC){if(bC==undefined){return bB;}
;if(bC==w){return Number.MAX_VALUE;}
;return bB*bC;}
,__um:function(bD,bE){for(var bF in bE){if(bE[bF]===undefined){continue;}
...
;}
;}
,__un:function(bG,bH){var bJ=100;for(var i=0;i<bH.length-1;i++){bJ=Math.min(bJ,bH[i+1]-bH[i]);}
;var bI=bG*bJ/100;while(bI>this.__ug){bI=bI/2;}
;return Math.round(bI);}
,__uo:function(bK){var bL=qx.Bootstrap.getKeys(bK);for(var i=0;i<bL.length;i++){bL[i]=parseInt(bL[i],10);}
