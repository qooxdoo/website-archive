...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.virtual.List","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.List-qx.theme.Classic.c2d2337a465a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var bw="Julian",bv="Show Icon",bu="Kevin",bt="demobrowser.demo.virtual.List",bs="Marvin",br="Marc",bq="Configurable",bp="icon/16/places/folder.png",bo="Jason",bn="show",bc="Jakob",bb="Lukas",ba="Julius",Y="Selection: ",X="Jan",W="Tobias",V="Mika",U="Nils",T="Noah",S="Quick selection is only available for the modes multi or additive",bD="Tom",bE="Nico",bB="Multi Selection",bC="Felix",bz="Leon",bA="Enable drag selection",bx="Show Both",by="48",bF="changeShowMode",bG="Fabian",bg="Drag selection is only available for the modes multi or additive",bf="Joel",bi="Dominic",bh="Show Label",bk="Enable quick selection",bj="Finn",bm="One as selection mode",bl="Jannis",be="Alexander",bd="Additive Selection",a="change",b="Erik",c="addaptive",d="Johannes",f="16",g="Jonas",h="icon/",j="David",k="Maximilian",l="enabled",bK="Tim",bJ="Justin",bI="Single Selection",bH="Linus",bO="Daniel",bN="Paul",bM="Philipp",bL="Niclas",bQ="One Selection",bP="Nick",C="Jonathan",D="Ben",A="on",B="showMode",G="Vincent",H="/places/folder.png",E="Robin",F="Jannick",y="Luis",z="Lennard",t="Moritz",s="Florian",v="",u="Marlon",p="Luca",o="Simon",r="Colin",q="Max",n="Additive selection",m="Elias",M="changeValue",N="changeSelection",O="Item No ",P="additive",I="bold",J="both",K="single",L="one",Q="multi",R="icon",x="label",w="value";qx.Class.define(bt,{extend:qx.application.Standalone,properties:{showMode:{init:J,check:[x,R,J],event:bF}},members:{__QA:null,__QB:null,__QC:null,main:function(){qx.application.Standalone.prototype.main.call(this);var bR=this.getRoot();bR.add(this.createConfigurableList(),{left:20,top:20});bR.add(this.createOneSelectionList(),{left:330,top:20});bR.add(this.createAdditiveSelectionList(),{left:520,top:20});}
,createConfigurableList:function(){var bU=new qx.ui.container.Composite(new qx.ui.layout.Canvas());var bW=new qx.ui.basic.Label(bq).set({font:I});bU.add(bW);var cf=[];for(var i=0;i<2500;i++){cf[i]={label:O+i,icon:(i%4)?f:by};}
;var ci=qx.data.marshal.Json.createModel(cf);var ca=this.__QA=new qx.ui.list.List(ci).set({scrollbarX:A,selectionMode:Q,height:280,width:150,labelPath:x,iconPath:R,iconOptions:{converter:function(cm){return h+cm+H;}
}});bU.add(ca,{top:20});var bY=this;var cl={configureItem:function(cn){bY.bind(B,cn,bn);}
,bindItem:function(co,cp,cq){co.bindDefaultProperties(cp,cq);co.bindProperty(x,l,{converter:function(cr){return parseInt(cr.replace(/Item No /g,v))%9!=0;}
},cp,cq);}
};ca.setDelegate(cl);ca.getSelection().push(ci.getItem(20));ca.getSelection().addListener(a,function(e){var cs=ca.getSelection();for(var i=0;i<cs.getLength();i++){this.debug(Y+cs.getItem(i).getLabel());}
;}
,this);var cd=ca.getPane().getRowConfig();for(var i=0;i<2500;i++){if(i%4==0){cd.setItemSize(i,56);}
else {cd.setItemSize(i,25);}
;}
;var ce=new qx.ui.form.RadioButton(bI);var bT=new qx.ui.form.RadioButton(bB);var ch=new qx.ui.form.RadioButton(bd);var cj=new qx.ui.form.RadioButton(bQ);ce.setUserData(w,K);bT.setUserData(w,Q);ch.setUserData(w,P);cj.setUserData(w,L);bU.add(ce,{left:160,top:20});bU.add(bT,{left:160,top:40});bU.add(ch,{left:160,top:60});bU.add(cj,{left:160,top:80});var cb=new qx.ui.form.RadioButton(bh);var bV=new qx.ui.form.RadioButton(bv);var bS=new qx.ui.form.RadioButton(bx);cb.setUserData(w,x);bV.setUserData(w,R);bS.setUserData(w,J);bU.add(cb,{left:160,top:120});bU.add(bV,{left:160,top:140});bU.add(bS,{left:160,top:160});var cc=this.__QB=new qx.ui.form.CheckBox(bA);var cg=this.__QC=new qx.ui.form.CheckBox(bk).set({enabled:false});bU.add(cc,{left:160,top:200});bU.add(cg,{left:160,top:220});var ck=new qx.ui.form.RadioGroup(ce,bT,ch,cj);ck.setSelection([bT]);ck.addListener(N,this.onSelectionModeChange,this);var bX=new qx.ui.form.RadioGroup(cb,bV,bS);bX.setSelection([bS]);bX.addListener(N,this.onShowModeChange,this);cc.addListener(M,this.onDragCheckChange,this);cg.addListener(M,this.onQuickCheckChange,this);return bU;}
,createOneSelectionList:function(){var ct=new qx.ui.container.Composite(new qx.ui.layout.Canvas());var cu=new qx.ui.basic.Label(bm).set({font:I});ct.add(cu);var cx=new qx.data.Array();for(var i=0;i<250;i++){var cw=new demobrowser.demo.virtual.model.Item(O+i,bp);cx.push(cw);}
;var cv=new qx.ui.list.List(cx).set({selectionMode:L,height:280,width:150,labelPath:x,iconPath:R});ct.add(cv,{top:20});cv.getSelection().push(cx.getItem(16));return ct;}
,createAdditiveSelectionList:function(){var cz=new qx.ui.container.Composite(new qx.ui.layout.Canvas());var cA=new qx.ui.basic.Label(n).set({font:I});cz.add(cA);var cB=[bz,bb,p,bj,bK,bC,g,y,k,bw,q,bN,bL,X,D,m,F,bM,T,bD,t,bE,j,U,o,bG,b,bJ,be,bc,s,bP,bH,V,bo,bO,z,bs,bl,W,bi,u,br,d,C,ba,r,bf,bu,G,E];var cE=new qx.data.Array();var cy=new qx.data.Array();for(var i=0;i<cB.length;i++){var cD=new demobrowser.demo.virtual.model.Item(cB[i]);cE.push(cD);if(i==10||i==12||i==16){cy.push(cD);}
;}
;var cC=new qx.ui.list.List(cE).set({selectionMode:P,height:200,width:150,labelPath:x,iconPath:R});cz.add(cC,{top:20});cC.setSelection(cy);return cz;}
,onSelectionModeChange:function(e){var cF=e.getData()[0].getUserData(w);this.__QA.setSelectionMode(cF);if(cF==K||cF==L){this.__QB.setEnabled(false);this.__QC.setEnabled(true);}
else if(cF==Q||cF==c){this.__QB.setEnabled(true);this.__QC.setEnabled(false);}
;}
,onShowModeChange:function(e){var cG=e.getData()[0].getUserData(w);this.setShowMode(cG);}
,onDragCheckChange:function(e){if(e.getData()){var cH=this.__QA.getSelectionMode();if(cH==K||cH==L){this.debug(bg);}
;}
;this.__QA.setDragSelection(e.getData());}
,onQuickCheckChange:function(e){if(e.getData()){var cI=this.__QA.getSelectionMode();if(cI==Q||cI==P){this.debug(S);}
;}
;this.__QA.setQuickSelection(e.getData());}
}});}
)();
...
(function(){var d="demobrowser.demo.virtual.model.Item",c="changeIcon",b="changeLabel",a="String";qx.Class.define(d,{extend:qx.core.Object,construct:function(e,f){qx.core.Object.call(this);if(e!=null){this.setLabel(e);}
;if(f!=null){this.setIcon(f);}
;}
,properties:{label:{check:a,event:b,nullable:true},icon:{check:a,event:c,nullable:true}}});}
)();
