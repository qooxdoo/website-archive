...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.bom.Flash","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Flash-qx.theme.Classic.e4254af56e82.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="Invalid parameter 'element'.",j="application/x-shockwave-flash",h='<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">',g="param",f="Invalid parameter 'attributes'.",e='" />',d="Invalid attribute 'movie'.",c="qx.bom.Flash",b='<param name="',a="qx.debug",F="type",E="load",D="name",C="data",B="Invalid attribute 'id'.",A="",z="mshtml",y="value",x="&",w="beforeunload",r="The parent DOM element isn't in DOM! The External Interface doesn't work in IE!",s='<object id="',p='" value="',q='" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">',n="undefined",o="object",l='</object>',m="100%",t="movie",u="=",v="engine.name";qx.Class.define(c,{statics:{_flashObjects:{},create:function(G,H,I,J,K){if(!K){K=window;}
;if(qx.core.Environment.get(a)){qx.core.Assert.assertElement(G,k);qx.core.Assert.assertMap(H,f);qx.core.Assert.assertString(H.movie,d);qx.core.Assert.assertString(H.id,B);if(!qx.dom.Element.isInDom(G,K)){qx.log.Logger.warn(this,r);}
;}
;if(!H.width){H.width=m;}
;if(!H.height){H.height=m;}
;J=J?qx.lang.Object.clone(J):{};if(!J[t]){J[t]=H.movie;}
;H[C]=H.movie;delete H.movie;if(I){for(var name in I){if(typeof J.flashvars!=n){J.flashvars+=x+name+u+I[name];}
else {J.flashvars=name+u+I[name];}
;}
;}
;var L=this.__Ia(G,H,J,K);this._flashObjects[H.id]=L;return L;}
,destroy:qx.core.Environment.select(v,{"mshtml":function(M,N){M=this.__HU(M);if(M.readyState==4){this.__HV(M);}
else {if(!N){N=window;}
;qx.bom.Event.addNativeListener(N,E,function(){qx.bom.Flash.__HV(M);}
);}
;}
,"default":function(O,P){O=this.__HU(O);if(O.parentNode){O.parentNode.removeChild(O);}
;delete this._flashObjects[O.id];}
}),__HU:function(Q){if(!Q){throw new Error("DOM element is null or undefined!");}
;if(Q.tagName.toLowerCase()!=="object"){Q=Q.firstChild;}
;if(!Q||Q.tagName.toLowerCase()!=="object"){throw new Error("DOM element has or is not a flash object!");}
;return Q;}
,__HV:qx.core.Environment.select("engine.name",{"mshtml":qx.event.GlobalError.observeMethod(function(R){for(var i in R){if(typeof R[i]=="function"){R[i]=null;}
;}
;if(R.parentNode){R.parentNode.removeChild(R);}
;delete this._flashObjects[R.id];}
),"default":null}),__HW:qx.event.GlobalError.observeMethod(function(){for(var S in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[S]);}
;window.__HX=function(){}
;window.__HY=function(){}
;qx.bom.Event.removeNativeListener(window,"beforeunload",qx.bom.Flash.__HW);}
),__Ia:qx.core.Environment.select(v,{"mshtml":function(T,U,V,W){V.movie=U.data;delete U.data;delete U.classid;var X=A;for(name in V){X+=b+name+p+V[name]+e;}
;if(U.id){T.innerHTML=s+U.id+q+X+l;delete U.id;}
else {T.innerHTML=h+X+l;}
;for(var name in U){T.firstChild.setAttribute(name,U[name]);}
;return T.firstChild;}
,"default":function(Y,ba,bb,bc){delete ba.classid;delete bb.movie;var be=qx.dom.Element.create(o,ba,bc);be.setAttribute(F,j);var bd;for(var name in bb){bd=qx.dom.Element.create(g,{},bc);bd.setAttribute(D,name);bd.setAttribute(y,bb[name]);be.appendChild(bd);}
;Y.appendChild(be);return be;}
})},defer:function(bf){if((qx.core.Environment.get(v)==z)){qx.bom.Event.addNativeListener(window,w,bf.__HW);}
;}
});}
)();
