...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.layout.Flow","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Flow-qx.theme.Indigo.5a60fee76e4d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var q="7th a wide, short button",p="1. First Button",o="Flow Layout with line breaks",n="Number 6",m="demobrowser.demo.layout.Flow",l="Flow Layout",k="center",j="20px Margins around the great big 5th button!",h="top",g="bottom",c="3rd really, really, really long Button",f="middle",e="Number 4",b="item #",a="2. Second longer Button...",d="icon/48/devices/computer.png";qx.Class.define(m,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);this._createGalleryWindow(20,20);this._createLineBreakWindow(160,50);}
,_createGalleryWindow:function(r,top){var s=new qx.ui.window.Window(l).set({contentPadding:0,width:200});s.setLayout(new qx.ui.layout.Grow());var u=new qx.ui.container.Scroll();s.add(u);var t=new qx.ui.container.Composite(new qx.ui.layout.Flow()).set({allowShrinkY:false});u.add(t);for(var i=0;i<30;i++){t.add(new qx.ui.basic.Atom(b+(i+1),d).set({iconPosition:h,width:60,padding:5}));}
;s.moveTo(r,top);s.open();}
,_createLineBreakWindow:function(v,top){var E=new qx.ui.layout.Flow();E.setAlignX(k);var w=new qx.ui.window.Window(o).set({contentPadding:0,width:500,height:200,layout:E});var F=d;var z=new qx.ui.form.Button(p,F);w.add(z);var y=new qx.ui.form.Button(a,F);w.add(y,{lineBreak:true});var x=new qx.ui.form.Button(c,F);x.setHeight(100);w.add(x);var D=new qx.ui.form.Button(e,F);D.setAlignY(g);w.add(D);var C=new qx.ui.form.Button(j);C.setHeight(100);C.setMargin(20);w.add(C,{lineBreak:true});var B=new qx.ui.form.Button(n,F);B.setAlignY(f);w.add(B);var A=new qx.ui.form.Button(q,F);A.setMaxHeight(20);w.add(A);w.moveTo(v,top);w.open();}
}});}
)();
...
(function(){var o="lineBreak",n="' is not supported by the flow layout!",m="middle",l="qx.debug",k="bottom",j="qx.ui.layout.Flow",h="center",g="right",f="The property '",e="Boolean",b="top",d="left",c="Integer",a="_applyLayoutChange";qx.Class.define(j,{extend:qx.ui.layout.Abstract,construct:function(p,q,r){qx.ui.layout.Abstract.call(this);if(p){this.setSpacingX(p);}
;if(q){this.setSpacingY(q);}
;if(r){this.setAlignX(r);}
;}
,properties:{alignX:{check:[d,h,g],init:d,apply:a},alignY:{check:[b,m,k],init:b,apply:a},spacingX:{check:c,init:0,apply:a},spacingY:{check:c,init:0,apply:a},reversed:{check:e,init:false,apply:a}},members:{verifyLayoutProperty:qx.core.Environment.select(l,{"true":function(s,name,t){this.assertEquals(o,name,f+name+n);}
,"false":null}),connectToWidget:function(u){qx.ui.layout.Abstract.prototype.connectToWidget.call(this,u);if(u){u.setAllowShrinkY(false);}
;}
,renderLayout:function(v,w){var A=this._getLayoutChildren();if(this.getReversed()){A=A.concat().reverse();}
;var x=new qx.ui.layout.LineSizeIterator(A,this.getSpacingX());var y=0;while(x.hasMoreLines()){var z=x.computeNextLine(v);this.__IF(z,y,v);y+=z.height+this.getSpacingY();}
;}
,__IF:function(B,C,D){var F=qx.ui.layout.Util;var I=0;if(this.getAlignX()!="left"){I=D-B.width;if(this.getAlignX()=="center"){I=Math.round(I/2);}
;}
;for(var i=0;i<B.children.length;i++){var G=B.children[i];var J=G.getSizeHint();var H=G.getMarginTop();var E=G.getMarginBottom();var top=F.computeVerticalAlignOffset(G.getAlignY()||this.getAlignY(),H+J.height+E,B.height,H,E);G.renderLayout(I+B.gapsBefore[i],C+top,J.width,J.height);I+=B.gapsBefore[i]+J.width;}
;}
,_computeSizeHint:function(){return this.__IG(Infinity);}
,hasHeightForWidth:function(){return true;}
,getHeightForWidth:function(K){return this.__IG(K).height;}
,__IG:function(L){var M=new qx.ui.layout.LineSizeIterator(this._getLayoutChildren(),this.getSpacingX());var Q=0;var N=0;var O=0;while(M.hasMoreLines()){var P=M.computeNextLine(L);O+=1;N=Math.max(N,P.width);Q+=P.height;}
;return {width:N,height:Q+this.getSpacingY()*(O-1)};}
}});}
)();
(function(){var b="No more lines to compute",a="qx.ui.layout.LineSizeIterator";qx.Class.define(a,{extend:Object,construct:function(c,d){this.__kA=c;this.__GP=d;this.__GQ=c.length>0;this.__GR=0;}
,members:{__kA:null,__GP:null,__GQ:null,__GR:null,computeNextLine:function(e){var e=e||Infinity;if(!this.__GQ){throw new Error(b);}
;var p=this.__kA;var n=0;var g=0;var o=[];var k=[];for(var i=this.__GR;i<p.length;i++){var m=p[i];var q=m.getSizeHint();var j=this.__GS(i);var h=q.width+j;var f=i==this.__GR;if(!f&&g+h>e){this.__GR=i;break;}
;var l=q.height+m.getMarginTop()+m.getMarginBottom();o.push(m);k.push(j);g+=h;n=Math.max(n,l);if(m.getLayoutProperties().lineBreak){this.__GR=i+1;break;}
;}
;if(i>=p.length){this.__GQ=false;}
;return {height:n,width:g,children:o,gapsBefore:k};}
,__GS:function(r){var s=r==this.__GR;if(s){return this.__kA[r].getMarginLeft();}
else {return Math.max(this.__kA[r-1].getMarginRight(),this.__kA[r].getMarginLeft(),this.__GP);}
;}
,hasMoreLines:function(){return this.__GQ;}
}});}
)();
