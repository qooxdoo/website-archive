...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree-qx.theme.Simple.e3017f5164ab.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="Show Leafs",k="Show top level open/close icons",j="loaded",h="drop",g="File #",f="demobrowser.demo.virtual.Tree",d="Enable drag selection",c="Open mode:",b="change",a="Hide Root Node",P="Update Model",O="Control",N=" items",M="icon/48/places/folder.png",L="New Item ",K="Enable multi selection",J="copy",I="name",H="",G="changeSelection",s="Drag me:",t="single",q="Selection: ",r="children",o="Tree:",p="click",m="json/tree.json",n="none",u="double click",v="Selection:",y="Root node:",x="Toggle Height",A="item",z="execute",C="move",B="dragstart",w="mode",F="multi",E="model",D="changeValue";qx.Class.define(f,{extend:qx.application.Standalone,members:{i:0,__wy:null,main:function(){qx.application.Standalone.prototype.main.call(this);var T=new qx.ui.container.Composite(new qx.ui.layout.HBox(20));this.getRoot().add(T,{top:20,left:20});var R=new qx.ui.tree.VirtualTree(null,I,r).set({width:200,height:400});T.add(R);R.setDroppable(true);R.setDraggable(true);R.addListener(B,function(e){e.addType(A);e.addAction(C);}
);R.addListener(h,function(e){var W=e.getOriginalTarget();var X=W&&W.getModel?W.getModel():U.getModel();if(e.getCurrentAction()==C){var V=R.getSelection().getItem(0);this.removeItem(U.getModel(),V);}
else {var V=qx.data.marshal.Json.createModel({"name":L+this.i++,"children":[]});}
;if(X.getChildren){X.getChildren().push(V);}
;}
,this);var Q=m;var U=new qx.data.store.Json(Q);this.__wy=U;U.bind(E,R,E);U.addListener(j,function(){R.openNode(R.getModel().getChildren().getItem(0));}
,this);var S=this.getCommandFrame(R);T.add(S);}
,removeItem:function(Y,ba){if(!Y.getChildren||Y==ba){return;}
;var bb=Y.getChildren();for(var i=0;i<bb.length;i++){if(bb.getItem(i)==ba){bb.remove(ba);return;}
;this.removeItem(bb.getItem(i),ba);}
;}
,getCommandFrame:function(bc){var bl=new qx.ui.groupbox.GroupBox(O);var bi=4;bl.setLayout(new qx.ui.layout.Grid(5,3));var bt=0;bl.add(new qx.ui.basic.Label(q).set({paddingTop:4}),{row:bt,column:0});var bp=new qx.ui.form.TextField().set({readOnly:true});bl.add(bp,{row:bt++,column:1});bc.getSelection().addListener(b,function(e){var bu=bc.getSelection();if(bu.getLength()>0){if(bc.getSelectionMode()===F){bp.setValue(bu.getLength()+N);}
else {bp.setValue(bu.getItem(0).getName());}
;}
else {bp.setValue(H);}
;}
);bl.add(new qx.ui.core.Spacer(bi,bi),{row:bt++,column:0});bl.add(new qx.ui.basic.Label(c),{row:bt,column:0});var bf={"click":p,"dblclick":u,"none":n};var bd=new qx.ui.form.RadioGroup();for(var bm in bf){var be=new qx.ui.form.RadioButton(bf[bm]).set({value:bm==bc.getOpenMode()});be.setUserData(w,bm);bd.add(be);bl.add(be,{row:bt++,column:1});}
;bd.addListener(G,function(e){bc.setOpenMode(e.getData()[0].getUserData(w));}
);bl.add(new qx.ui.core.Spacer(bi,bi),{row:bt++,column:0});bl.add(new qx.ui.basic.Label(v),{row:bt,column:0});var bg=new qx.ui.form.CheckBox(K);bl.add(bg,{row:bt++,column:1});bg.addListener(D,function(e){var bv=e.getData();bc.setSelectionMode(bv?F:t);}
);var bh=new qx.ui.form.CheckBox(d);bl.add(bh,{row:bt++,column:1});bh.addListener(D,function(e){var bw=e.getData();bc.setDragSelection(bw);if(!bg.getValue()){bg.setValue(true);}
;}
);bl.add(new qx.ui.core.Spacer(bi,bi),{row:bt++,column:0});bl.add(new qx.ui.basic.Label(y),{row:bt,column:0});var br=new qx.ui.form.CheckBox(a);bl.add(br,{row:bt++,column:1});br.addListener(D,function(e){bc.setHideRoot(e.getData());}
);var bk=new qx.ui.form.CheckBox(k);bl.add(bk,{row:bt++,column:1});bk.addListener(D,function(e){bc.setShowTopLevelOpenCloseIcons(e.getData());}
);bl.add(new qx.ui.core.Spacer(bi,bi),{row:bt++,column:0});bl.add(new qx.ui.basic.Label(o),{row:bt,column:0});var bn=new qx.ui.form.CheckBox(l);bn.setValue(bc.isShowLeafs());bl.add(bn,{row:bt++,column:1});bn.addListener(D,function(e){bc.setShowLeafs(e.getData());}
);bl.add(new qx.ui.core.Spacer(bi,bi),{row:bt++,column:0});var bq=new qx.ui.form.Button(P);bq.addListener(z,function(e){var bA=bc.getModel().getChildren().getItem(0);var bx=bA.getChildren().getItem(2);var by=[];for(var i=0;i<10;i++){by.push({"name":g+(bx.getChildren().getLength()+i)});}
;var bz=qx.data.marshal.Json.createModel(by,true);bx.getChildren().append(bz);}
,this);bl.add(bq,{row:bt++,column:1});var bj=new qx.ui.form.Button(x);bl.add(bj,{row:bt++,column:1});var bo=new qx.ui.basic.Image(M);bo.setDraggable(true);bo.addListener(B,function(e){e.addType(A);e.addAction(J);}
);bl.add(new qx.ui.basic.Label(s),{row:bt,column:0});bl.add(bo,{row:bt++,column:1});var bs=true;bj.addListener(z,function(e){bc.setHeight(bs?600:400);bs=!bs;}
);return bl;}
}});}
)();
...
,getLookupTable:function(){return this.__od;}
,_reverseLookup:function(bi){return bi;}
,_getDataFromRow:function(bj){return this.__od.getItem(bj);}
,_getSelectables:function(){return this.__od;}
,getOpenNodes:function(){return this.__CX;}
,isNode:function(bk){return qx.Class.hasProperty(bk.constructor,this.getChildProperty());}
,getLevel:function(bl){return this.__CY[bl];}
...
,_applyModel:function(bK,bL){this.__CX=[];if(bK!=null){if(qx.core.Environment.get(O)){if(!qx.Class.hasMixin(bK.constructor,qx.data.marshal.MEventBubbling)){this.warn(r+h+f);}
;}
;bK.addListener(w,this._onChangeBubble,this);this.__De(bK);}
...
;if(qx.lang.String.startsWith(bP,this.getChildProperty())){this.__Dc();}
;}
,_onUpdated:function(event){if(this.__hw==null){this.__hw=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__hw.schedule();}
,_onOpen:function(event){var bR=event.getRow();var bQ=this.__od.getItem(bR);if(this.isNode(bQ)){if(this.isNodeOpen(bQ)){this.closeNode(bQ);}
...
;break;};}
;}
,_beforeApplySelection:function(bW){if(bW.length===0&&this.getSelectionMode()===G){var bX=this.__Dh();var bY=this.getLookupTable().indexOf(bX);if(bY>=0){bW.push(bY);}
;}
;}
,_afterApplySelection:function(){var ca=this.getSelection();if(ca.getLength()>0&&this.getSelectionMode()===G){this.__Dg(ca.getItem(0));}
else {this.__Db=[];}
;}
,__Dc:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(C+k);}
;this.__Da=0;var ce=[];this.__CY=[];var cc=-1;var cd=this.getModel();if(cd!=null){if(!this.isHideRoot()){cc++;ce.push(cd);this.__CY.push(cc);}
;if(this.isNodeOpen(cd)){var cb=this.__Dd(cd,cc);ce=ce.concat(cb);}
;}
;this._provider.removeBindings();this.__od.removeAll();this.__od.append(ce);this.__xN();this._updateSelection();}
,__Dd:function(cf,cg){var ch=[];cg++;if(!this.isNode(cf)){return ch;}
;var cj=cf.get(this.getChildProperty());if(cj==null){return ch;}
;for(var i=0;i<cj.getLength();i++){var ck=cj.getItem(i);if(this.isNode(ck)){this.__CY.push(cg);ch.push(ck);if(this.isNodeOpen(ck)){var ci=this.__Dd(ck,cg);ch=ch.concat(ci);}
;}
else {if(this.isShowLeafs()){this.__CY.push(cg);ch.push(ck);}
;}
;}
;return ch;}
,__De:function(cl){if(!qx.lang.Array.contains(this.__CX,cl)){this.__CX.push(cl);this.fireDataEvent(Q,cl);}
;}
,__Df:function(cm,cn){if(cm===cn){this.__De(cn);return true;}
;if(!this.isNode(cm)){return false;}
;var cp=cm.get(this.getChildProperty());if(cp==null){return false;}
;for(var i=0;i<cp.getLength();i++){var cq=cp.getItem(i);var co=this.__Df(cq,cn);if(co===true){this.__De(cq);return true;}
;}
;return false;}
,__xN:function(){this.getPane().getRowConfig().setItemCount(this.__od.getLength());this.getPane().fullUpdate();}
,getParent:function(cr){var ct=this.__od.indexOf(cr);if(ct<0){return null;}
;var cu=this.__CY[ct];while(ct>0){ct--;var cs=this.__CY[ct];if(cs<cu){return this.__od.getItem(ct);}
;}
;return null;}
,__Dg:function(cv){this.__Db=[];var parent=this.getParent(cv);while(parent!=null){this.__Db.unshift(parent);parent=this.getParent(parent);}
;}
,__Dh:function(){if(this.__Db==null){return this.getModel();}
;var cw=this.getLookupTable();var parent=this.__Db.pop();while(parent!=null){if(cw.contains(parent)){return parent;}
;parent=this.__Db.pop();}
...
;}
;if(!qx.core.ObjectRegistry.inShutDown&&this.__hw!=null){this.__hw.cancel();this.__hw.dispose();}
;var cx=this.getModel();if(cx!=null){cx.removeListener(w,this._onChangeBubble,this);}
;this._layer.removeListener(x,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__od.dispose();this._layer=this._provider=this.__od=this.__CX=this.__hw=null;}
});}
)();
...
(function(){var q="model",p="changeDelegate",o="virtual-tree-file",n="label",m="appearance",l="icon",k="bindItem",j=".length",i="]",h="virtual-tree-folder",c="[",g=".",f="qx.ui.tree.core.MWidgetController",b="BindingIds",a="",d="String";qx.Mixin.define(f,{construct:function(){this.__ya=[];}
...
;}
,bindProperty:function(u,v,w,x,y){var z=this.__yb(y,u);var A=this._tree.getLookupTable();var B=A.bind(z,x,v,w);this.__rQ(x,B);}
,bindPropertyReverse:function(C,D,E,F,G){var H=this.__yb(G,C);var I=this._tree.getLookupTable();var J=F.bind(D,I,H,E);this.__rQ(F,J);}
,removeBindings:function(){while(this.__ya.length>0){var K=this.__ya.pop();this._removeBindingsFrom(K);}
;}
...
;}
,_removeBindingsFrom:function(O){var P=this.__yc(O);while(P.length>0){var Q=P.pop();try{this._tree.getLookupTable().removeBinding(Q);}
catch(e){O.removeBinding(Q);}
;}
;if(qx.lang.Array.contains(this.__ya,O)){qx.lang.Array.remove(this.__ya,O);}
;}
,__yb:function(R,S){var T=c+R+i;if(S!=null&&S!=a){T+=g+S;}
;return T;}
,__rQ:function(U,V){var W=this.__yc(U);if(!qx.lang.Array.contains(W,V)){W.push(V);}
;if(!qx.lang.Array.contains(this.__ya,U)){this.__ya.push(U);}
;}
,__yc:function(X){var Y=X.getUserData(b);if(Y==null){Y=[];X.setUserData(b,Y);}
;return Y;}
...
;var q=this._renderer.getCellWidget();q.setOpen(r&&this._tree.isNodeOpen(p));q.setUserData(g,r);q.addListener(b,this.__Di,this);if(this._tree.getSelection().contains(p)){this._styleSelectabled(q);}
...
,poolCellWidget:function(t){t.removeListener(b,this.__Di,this);this._removeBindingsFrom(t);this._renderer.pool(t);this._onPool(t);}
...
,__Di:function(event){var J=event.getTarget();var K=J.getUserData(c);var I=this._tree.getLookupTable().getItem(K);if(event.getData()){this._tree.openNode(I);}
