...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Window_Editor","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Window_Editor-qx.theme.Classic.6355dbaf4664.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var v="columnVisibilityMenuCreateEnd",u="tableWidthChanged",t="ontablewidthchanged",s="verticalScrollBarChanged",r="oncolumnwidthchanged",q="onvisibilitychanged",p="qx.ui.table.columnmodel.resizebehavior.Abstract",n="qx.ui.table.columnmodel.Resize",m="_applyBehavior",l="separator",e="visibilityChanged",k="Reset column widths",h="changeBehavior",c="user-button",b="onverticalscrollbarchanged",g="onappear",f="widthChanged",i="execute",a="appear",j="qx.tableResizeDebug",d="qx.debug";qx.Class.define(n,{extend:qx.ui.table.columnmodel.Basic,include:qx.locale.MTranslation,construct:function(){qx.ui.table.columnmodel.Basic.call(this);this.__AY=false;this.__Ba=false;}
,properties:{behavior:{check:p,init:null,nullable:true,apply:m,event:h}},members:{__Ba:null,__AY:null,__Af:null,_applyBehavior:function(w,x){if(x!=null){x.dispose();x=null;}
;w._setNumColumns(this.getOverallColumnCount());w.setTableColumnModel(this);}
,init:function(y,z){qx.ui.table.columnmodel.Basic.prototype.init.call(this,y,z);if(this.__Af==null){this.__Af=z;z.addListener(a,this._onappear,this);z.addListener(u,this._onTableWidthChanged,this);z.addListener(s,this._onverticalscrollbarchanged,this);z.addListener(v,this._addResetColumnWidthButton,this);this.addListener(f,this._oncolumnwidthchanged,this);this.addListener(e,this._onvisibilitychanged,this);}
...
,_onappear:function(event){if(this.__AY){return;}
;this.__AY=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(g);}
;}
;this.getBehavior().onAppear(event,event.getType()!==a);this.__Af._updateScrollerWidths();this.__Af._updateScrollBarVisibility();this.__AY=false;this.__Ba=true;}
,_onTableWidthChanged:function(event){if(this.__AY||!this.__Ba){return;}
;this.__AY=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(t);}
;}
;this.getBehavior().onTableWidthChanged(event);this.__AY=false;}
,_onverticalscrollbarchanged:function(event){if(this.__AY||!this.__Ba){return;}
;this.__AY=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(b);}
;}
;this.getBehavior().onVerticalScrollBarChanged(event);qx.event.Timer.once(function(){if(this.__Af&&!this.__Af.isDisposed()){this.__Af._updateScrollerWidths();this.__Af._updateScrollBarVisibility();}
;}
,this,0);this.__AY=false;}
,_oncolumnwidthchanged:function(event){if(this.__AY||!this.__Ba){return;}
;this.__AY=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(r);}
;}
;this.getBehavior().onColumnWidthChanged(event);this.__AY=false;}
,_onvisibilitychanged:function(event){if(this.__AY||!this.__Ba){return;}
;this.__AY=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(q);}
;}
;this.getBehavior().onVisibilityChanged(event);this.__AY=false;}
...
(function(){var m="Function",k="Boolean",j="minWidth",h="width",g="qx.ui.table.columnmodel.Resize",f="qx.ui.table.columnmodel.resizebehavior.Default",e="__td",d="Unknown property: ",c="maxWidth",b="__Bd",a="Column number out of range";qx.Class.define(f,{extend:qx.ui.table.columnmodel.resizebehavior.Abstract,construct:function(){qx.ui.table.columnmodel.resizebehavior.Abstract.call(this);this.__Bc=[];this.__td=new qx.ui.layout.HBox();this.__td.connectToWidget(this);this.__Bd=new qx.util.DeferredCall(this._computeColumnsFlexWidth,this);}
,properties:{newResizeBehaviorColumnData:{check:m,init:function(n){return new qx.ui.core.ColumnData();}
},initializeWidthsOnEveryAppear:{check:k,init:false},tableColumnModel:{check:g}},members:{__td:null,__Be:null,__Bc:null,__Bd:null,__Bf:false,setWidth:function(o,p,q){if(o>=this.__Bc.length){throw new Error(a);}
;this.__Bc[o].setColumnWidth(p,q);this.__Bd.schedule();}
,setMinWidth:function(r,s){if(r>=this.__Bc.length){throw new Error(a);}
;this.__Bc[r].setMinWidth(s);this.__Bd.schedule();}
,setMaxWidth:function(t,u){if(t>=this.__Bc.length){throw new Error(a);}
;this.__Bc[t].setMaxWidth(u);this.__Bd.schedule();}
,set:function(v,w){for(var x in w){switch(x){case h:this.setWidth(v,w[x]);break;case j:this.setMinWidth(v,w[x]);break;case c:this.setMaxWidth(v,w[x]);break;default:throw new Error(d+x);};}
;}
,onAppear:function(event,y){if(y===true||!this.__Bf||this.getInitializeWidthsOnEveryAppear()){this._computeColumnsFlexWidth();this.__Bf=true;}
;}
...
,_setNumColumns:function(A){var B=this.__Bc;if(A<=B.length){B.splice(A,B.length);return;}
;for(var i=B.length;i<A;i++){B[i]=this.getNewResizeBehaviorColumnData()();B[i].columnNumber=i;}
;}
,getLayoutChildren:function(){return this.__Be;}
,_computeColumnsFlexWidth:function(){this.__Bd.cancel();var G=this._getAvailableWidth();if(G===null){return;}
;var C=this.getTableColumnModel();var E=C.getVisibleColumns();var F=E.length;var D=this.__Bc;var i,l;if(F===0){return;}
;var I=[];for(i=0;i<F;i++){I.push(D[E[i]]);}
;this.__Be=I;this.__Bg();this.__td.renderLayout(G,100);for(i=0,l=I.length;i<l;i++){var H=I[i].getComputedWidth();C.setColumnWidth(E[i],H);}
;}
,__Bg:function(){this.__td.invalidateChildrenCache();var J=this.__Be;for(var i=0,l=J.length;i<l;i++){J[i].invalidateLayoutCache();}
;}
...
;}
,_getResizeColumnData:function(){return this.__Bc;}
},destruct:function(){this.__Bc=this.__Be=null;this._disposeObjects(e,b);}
});}
)();
