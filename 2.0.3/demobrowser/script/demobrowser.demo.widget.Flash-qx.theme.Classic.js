...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Flash-qx.theme.Classic.fee1e3c4021a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,createFlashDemo1:function(){var N=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));var L={flashVarText:q};var M=new qx.ui.embed.Flash(p).set({scale:t,variables:L});M.getContentElement().setParam(a,u);N.add(M,{flex:1});var O=new qx.ui.form.Button(m);O.addListener(v,function(){this.__ce.open();}
,this);N.add(O);return N;}
,__HT:null,__QH:null,__QI:null,__QJ:null,createFlashDemo2:function(){this.self(arguments).setCallBackInstance(this);var R=new qx.ui.container.Composite(new qx.ui.layout.HBox(8));var Q=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));R.add(Q,{flex:1});Q.add(new qx.ui.basic.Label(y).set({rich:true}));Q.add(new qx.ui.basic.Label(g));this.__QH=new qx.ui.form.TextArea(A);this.__QH.setEnabled(false);Q.add(this.__QH,{flex:1});Q.add(new qx.ui.basic.Label(l));this.__QI=new qx.ui.form.TextArea(h);Q.add(this.__QI,{flex:1});var S=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,i));this.__QJ=new qx.ui.form.Button(C);this.__QJ.setEnabled(false);this.__QJ.addListener(v,function(e){this.__HT.getFlashElement().sendMessage(this.__QI.getValue());}
,this);S.add(this.__QJ);Q.add(S);var P=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));R.add(P,{flex:1});P.add(new qx.ui.basic.Label(z).set({rich:true}));this.__HT=new qx.ui.embed.Flash(E);this.__HT.setVariables({init:w});P.add(this.__HT,{flex:1});return R;}
,sendMessage:function(T){this.__QH.setValue(T);}
,initFlash:function(){if(this.__HT.getFlashElement().setup){this.__HT.getFlashElement().setup(x);this.__QJ.setEnabled(true);}
else {alert(r+D+k);}
;}
},statics:{__QK:null,setCallBackInstance:function(U){demobrowser.demo.widget.Flash.__QK=U;}
,sendMessage:function(V){demobrowser.demo.widget.Flash.__QK.sendMessage(V);}
,flashReady:function(){demobrowser.demo.widget.Flash.__QK.initFlash();}
},destruct:function(){this._disposeObjects(d,o,f,s,b);}
});}
)();
...
,_checkLoading:function(){var bi=this.getSource();if(bi!=t&&bi!=null&&bi!=J){if(!this.isLoaded()){if(!this.__HO){this.__HO=new Date().getTime();}
;var bj=new Date().getTime()-this.__HO;if(this.getLoadTimeout()>bj){var bk=qx.util.TimerManager.getInstance();bk.start(this._checkLoading,0,this,null,10);this.fireEvent(S);}
else {if(qx.core.Environment.get(N)){this.debug(r+bj);}
;this.fireEvent(F);this.__HO=null;}
;}
else {this.fireEvent(ba);this.__HO=null;}
;}
;}
...
,_applyQuality:function(bu,bv){this.__HP(j,bu);}
,_applyScale:function(bw,bx){this.__HP(f,bw);}
,_applyWmode:function(by,bz){this.__HP(a,by);}
,_applyPlay:function(bA,bB){this.__HP(l,bA);}
,_applyLoop:function(bC,bD){this.__HP(W,bC);}
,_applyMenu:function(bE,bF){this.__HP(U,bE);}
,_applyAllowScriptAccess:function(bG,bH){this.__HP(V,bG);}
,_applyLiveConnect:function(bI,bJ){this.__HP(q,bI);}
,__HP:function(bK,bL){this.getContentElement().setParam(bK,bL);qx.ui.core.queue.Layout.add(this);}
}});}
)();
(function(){var m="The id cannot be modified after initial creation",l="The variables cannot be modified after initial creation",k="movie",j="The attributes cannot be modified after initial creation",i="The source cannot be modified after initial creation",h="The params cannot be modified after initial creation",g="id",f="qx.html.Flash",e="Invalid attribute 'key'.",d="div",a="Invalid attribute 'value' expected String, Boolean or null.",c="Invalid attribute 'value'.",b="qx.debug";qx.Class.define(f,{extend:qx.html.Element,construct:function(n,o){qx.html.Element.call(this,d,n,o);this.__HQ={};this.__HR={};this.__HS={};}
,members:{__HQ:null,__HS:null,__HR:null,__HT:null,_createDomElement:function(){return qx.dom.Element.create(d);}
,createFlash:function(){this.__HT=qx.bom.Flash.create(this.getDomElement(),this.getAttributes(),this.__HR,this.__HQ);}
,setSource:function(p){if(qx.core.Environment.get(b)){qx.core.Assert.assertString(p,c);}
;if(this.__HT){throw new Error(i);}
;this.setAttribute(k,p);}
,setId:function(q){if(qx.core.Environment.get(b)){qx.core.Assert.assertString(q,c);}
;if(this.__HT){throw new Error(m);}
;this.setAttribute(g,q);}
,getVariables:function(){return this.__HR;}
,setVariables:function(r){if(qx.core.Environment.get(b)){qx.core.Assert.assertMap(r,c);}
;if(this.__HT){throw new Error(l);}
;this.__HR=r;}
,getAttributes:function(){return this.__HS;}
...
;}
;}
;if(this.__HT){throw new Error(j);}
;if(t===null||t===undefined){delete this.__HS[s];}
else {this.__HS[s]=t;}
;}
,getParams:function(){return this.__HQ;}
...
;}
;}
;if(this.__HT){throw new Error(h);}
;if(v===null||v===undefined){delete this.__HQ[u];}
else {this.__HQ[u]=v;}
;}
,getFlashElement:function(){return this.__HT;}
},destruct:function(){if(this.__HT){qx.bom.Flash.destroy(this.__HT);}
;this.__HQ=this.__HR=this.__HS=null;}
});}
)();
