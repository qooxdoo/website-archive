...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.virtual.GroupedList","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.GroupedList-qx.theme.Classic.5dc3e24b2463.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="persons.json</a> and binds the created model to all list widgets. ",k="demobrowser.demo.virtual.GroupedList",j="Friends",i="json/persons.json",h="by first name and groups it by the 'group' model property. The ",g="third list is configured  to use a customized group item for rendering.",f="Grouped by last name:",e="<b>Grouped List</b><br/>",d="Raw List:",c="The second list sorts the items by last name and groups the items ",F="Colleagues",E="icon",D="#005E00",C="Loads the json file <a href='json/persons.json' target='_blank'>",B="The first list shows only the row data and uses a converter for the ",A=", ",z="Grouped by group:",y="icon/16/emotes/face-laugh.png",x="by the first letter of the last name. The third list sorts the items ",w="label",s="icon/16/categories/system.png",t="icon/16/categories/office.png",q="white",r="label that concatenates the first and last name. ",o=" ",p="firstname",m="model.persons",n="bold",u="model",v="no model...";qx.Class.define(k,{extend:qx.application.Standalone,members:{__ry:null,__Qy:null,__Qz:null,main:function(){qx.application.Standalone.prototype.main.call(this);var J=this.createFirstExample();var L=this.createSecondExample();var K=this.createThirdExample();this.loadData();var G=new qx.ui.container.Composite(new qx.ui.layout.VBox(10));this.getRoot().add(G,{edge:20});var H=new qx.ui.basic.Label();H.setRich(true);H.setWidth(470);H.setSelectable(true);H.setValue(e+C+l+B+r+c+x+h+g);G.add(H);var I=new qx.ui.container.Composite(new qx.ui.layout.HBox(20));G.add(I,{flex:1});I.add(J);I.add(L);I.add(K);}
,loadData:function(){var M=i;var N=new qx.data.store.Json(M);N.bind(m,this.__ry,u);N.bind(m,this.__Qy,u);N.bind(m,this.__Qz,u);}
,createFirstExample:function(){var P=new qx.ui.container.Composite(new qx.ui.layout.Canvas());var Q=new qx.ui.basic.Label(d).set({font:n});P.add(Q);var O=this.__ry=new qx.ui.list.List().set({height:280,width:150,labelPath:p,labelOptions:{converter:function(R,S){return S?R+o+S.getLastname():v;}
}});P.add(O,{top:20});return P;}
,createSecondExample:function(){var U=new qx.ui.container.Composite(new qx.ui.layout.Canvas());var W=new qx.ui.basic.Label(f).set({font:n});U.add(W);var T=this.__Qy=new qx.ui.list.List().set({height:280,width:150,labelPath:p,labelOptions:{converter:function(X,Y){return Y?Y.getLastname()+A+X:v;}
...
};T.setDelegate(V);T.setSelection(this.__ry.getSelection());return U;}
,createThirdExample:function(){var bc=new qx.ui.container.Composite(new qx.ui.layout.Canvas());var be=new qx.ui.basic.Label(z).set({font:n});bc.add(be);var bb=this.__Qz=new qx.ui.list.List().set({height:280,width:150,labelPath:p,labelOptions:{converter:function(bf,bg){return bg?bf+o+bg.getLastname():v;}
...
,configureGroupItem:function(bi){bi.setBackgroundColor(D);bi.setTextColor(q);}
,bindGroupItem:function(bj,bk,bl){bj.bindProperty(null,w,null,bk,bl);bj.bindProperty(null,E,{converter:function(bm){switch(bm){case j:return y;case F:return t;default:return s;};}
},bk,bl);}
};bb.setDelegate(bd);bb.setSelection(this.__ry.getSelection());return bc;}
},destruct:function(){this.__ry.dispose();this.__Qy.dispose();this.__Qz.dispose();this.__ry=this.__Qy=this.__Qz=null;}
});}
)();
