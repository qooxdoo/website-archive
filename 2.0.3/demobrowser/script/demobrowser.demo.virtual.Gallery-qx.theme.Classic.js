...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Gallery-qx.theme.Classic.2099aec83469.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="Gallery (HTML - divs)",k="manager",j="background",h="demobrowser.demo.virtual.Gallery",g="demobrowser.demo.virtual.WidgetGallery",f="selected",d="text-align: center;",c="background-selected",b="auto",a="color: white; background-color: #00398D;",R="user-desktop.png",Q="<img src='",P="'></img>",O="resize",N="folder.png",M="vertical",L="multi",K="network-server.png",J="user-trash.png",I="off",s="white-box",t="top",q="__Qx",r="icon/32/places/",o="horizontal",p="demobrowser.demo.virtual.AbstractGallery",m="default",n="abstract",u="position: absolute;",v="demobrowser.demo.virtual.HtmlGallery",A="Gallery (widgets)",z="<br>",C="Icon #",B="network-workgroup.png",E="text-selected",D="demobrowser.demo.virtual.GalleryCell",x="cell",H="layer",G="icon",F="#f3f3f3",w="label",y="";qx.Class.define(h,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);new demobrowser.demo.virtual.WidgetGallery(A);var S=new demobrowser.demo.virtual.HtmlGallery(l);S.moveTo(400,50);}
}});qx.Class.define(p,{extend:qx.ui.window.Window,type:n,construct:function(T){qx.ui.window.Window.call(this,T);this.set({contentPadding:0,showClose:false,showMinimize:false,width:320,height:400});this.setLayout(new qx.ui.layout.Grow());this.moveTo(30,50);this.open();this.itemHeight=65;this.itemWidth=70;this.itemCount=431;this.itemPerLine=1;this.items=this._generateItems(this.itemCount);var U=this._createScroller();U.set({scrollbarX:I,scrollbarY:b});U.getPane().addListener(O,this._onPaneResize,this);this.add(U);this.manager=new qx.ui.virtual.selection.CellRectangle(U.getPane(),this).set({mode:L,drag:true});this.manager.attachMouseEvents();this.manager.attachKeyEvents(U);}
...
,_generateItems:function(bg){var bj=[];var bm=[N,J,K,B,R];var bk=qx.util.AliasManager.getInstance();var bi=qx.util.ResourceManager.getInstance();for(var i=0;i<bg;i++){var bn=r+bm[Math.floor(Math.random()*bm.length)];var bh=bk.resolve(bn);var bl=bi.toUri(bh);bj[i]={label:C+(i+1),icon:bn,resolvedIcon:bl};}
;return bj;}
},destruct:function(){this.items=null;this._disposeObjects(k);}
});qx.Class.define(D,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var bo=new qx.ui.basic.Atom().set({iconPosition:t});bo.getChildControl(w).set({padding:[0,4]});bo.getChildControl(G).set({padding:4});return bo;}
,updateData:function(bp,bq){bp.set({icon:bq.icon,label:bq.label});}
,updateStates:function(br,bs){var bu=br.getChildControl(w);var bt=br.getChildControl(G);if(bs.selected){bu.setBackgroundColor(c);bu.setTextColor(E);bt.setDecorator(s);bt.setBackgroundColor(j);}
else {bu.resetBackgroundColor();bu.resetTextColor();bt.resetDecorator();bt.resetBackgroundColor();}
;}
}});qx.Class.define(g,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bv){demobrowser.demo.virtual.AbstractGallery.call(this,bv);this.__Qx=new demobrowser.demo.virtual.GalleryCell();}
...
;if(bz){this.__Qx.updateStates(bB,{selected:1});}
else {this.__Qx.updateStates(bB,{});}
;}
;}
...
;var bH=this.__Qx.getCellWidget(bI,bG);bH.setUserData(x,bF);return bH;}
,poolCellWidget:function(bJ){this.__Qx.pool(bJ);}
...
,members:{_createScroller:function(){var bN=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.HtmlCell(this);bN.getPane().addLayer(this.layer);var bM=new qx.ui.virtual.layer.GridLines(o,F);bN.getPane().addLayer(bM);var bM=new qx.ui.virtual.layer.GridLines(M,F);bN.getPane().addLayer(bM);return bN;}
...
;var bU=this.manager.isItemSelected({row:bR,column:bS});var bT=bU?a:y;return {style:[u,d,this._fontCss,bT].join(y),content:[Q,bV.resolvedIcon,P,z,bV.label].join(y)};}
