...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Github-qx.theme.Indigo.e10b428cf936.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var v=") already exists",u="",t=" ms. Throttling.",s="loadEnd",r="qx.io.rest.Resource",q="interval",p=" times subsequently, each within ",o="fail",n="Received successful response more than ",m="get",f="{",l="=?(\\w+)?}",i="Method with name of action (",c="No route for action ",b="error",h="Error",g="success",j="qx.debug",a="Success",k="qx.event.type.Rest",d="undefined";qx.Class.define(r,{extend:qx.core.Object,construct:function(w){qx.core.Object.call(this);this.__JK={};this.__GY={};this.__JL={};this.__JM={};try{if(typeof w!==d){if(qx.core.Environment.get(j)){qx.core.Assert.assertMap(w);}
;this.__JS(w);}
;}
catch(e){this.dispose();throw e;}
;}
,events:{"success":k,"actionSuccess":k,"error":k,"actionError":k},statics:{POLL_THROTTLE_LIMIT:100,POLL_THROTTLE_COUNT:30,REQUIRED:true,placeholdersFromUrl:function(x){var z=/\{(\w+)(=\w+)?\}/g,A,y=[];while((A=z.exec(x))){y.push(A[1]);}
;return y;}
},members:{__JK:null,__GY:null,__JN:null,__JL:null,__JM:null,__JO:null,configureRequest:function(B){this.__JO=B;}
,_getRequest:function(){return new qx.io.request.Xhr();}
,__JP:function(C){var D=this._getRequest();if(!qx.lang.Type.isArray(this.__JK[C])){this.__JK[C]=[];}
;this.__JK[C].push(D);return D;}
,map:function(E,F,G,H){this.__GY[E]=[F,G,H];this.__JK[E]=[];if(E==m){this[E]=undefined;}
;if(typeof this[E]!==d&&this[E].action!==true){if(this[E]!==qx.lang.Function.empty){throw new Error(i+E+v);}
;}
;this.__JT(E+a);this.__JT(E+h);this[E]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,E);return this.invoke.apply(this,arguments);}
,this);this[E].action=true;}
,invoke:function(I,J,K){var M=this.__JP(I),J=J==null?{}:J,P=this._getRequestConfig(I,J);this.__GY[I].params=J;this.__JQ(J,P.check);this.__Du(M,P,K);if(this.__JO){this.__JO.call(this,M,I,J,K);}
;this.__JR(M,P,K);M.addListenerOnce(g,function O(){var Q=[M.getResponse(),null,false,M,I,M.getPhase()];this.fireEvent(I+a,qx.event.type.Rest,Q);this.fireEvent(g,qx.event.type.Rest,Q);}
,this);M.addListenerOnce(o,function N(){var R=[M.getResponse(),null,false,M,I,M.getPhase()];this.fireEvent(I+h,qx.event.type.Rest,R);this.fireEvent(b,qx.event.type.Rest,R);}
,this);M.addListenerOnce(s,function L(){M.dispose();}
,this);M.send();return parseInt(M.toHashCode(),10);}
,setBaseUrl:function(S){this.__JN=S;}
,__JQ:function(T,U){if(typeof U!=="undefined"){if(qx.core.Environment.get("qx.debug")){qx.core.Assert.assertObject(U,"Check must be object with params as keys");}
;qx.lang.Object.getKeys(U).forEach(function(V){if(qx.core.Environment.get("qx.debug")){if(U[V]!==true){if(qx.core.Environment.get("qx.debug")){qx.core.Assert.assertRegExp(U[V]);}
;}
;}
;if(U[V]===qx.io.rest.Resource.REQUIRED&&typeof T[V]==="undefined"){throw new Error("Missing parameter '"+V+"'");}
;if(!(U[V]&&typeof U[V].test=="function")){return;}
;if(!U[V].test(T[V])){throw new Error("Parameter '"+V+"' is invalid");}
;}
);}
;}
,__Du:function(W,X,Y){W.set({method:X.method,url:X.url});if(Y){W.setRequestData(Y);}
;}
,__JR:function(ba,bb,bc){if(bc){var bd=ba.getRequestHeader("Content-Type");if(qx.util.Request.methodAllowsRequestBody(ba.getMethod())){if((/application\/.*\+?json/).test(bd)){bc=qx.lang.Json.stringify(bc);ba.setRequestData(bc);}
;}
;}
;}
,abort:function(be){if(qx.lang.Type.isNumber(be)){var bi=be;var bg=qx.core.ObjectRegistry.getPostId();var bh=qx.core.ObjectRegistry.fromHashCode(bi+bg);if(bh){bh.abort();}
;}
else {var bf=be;var bj=this.__JK[bf];if(this.__JK[bf]){bj.forEach(function(bk){bk.abort();}
);}
;}
;}
,refresh:function(bl){this.invoke(bl,this.__GY[bl].params);}
,poll:function(bm,bn,bo){if(this.__JL[bm]){this.__JL[bm].dispose();}
;if(typeof bo==d){bo=this.__GY[bm].params;}
;this.invoke(bm,bo);var bp=this.__JL[bm]=new qx.event.Timer(bn);bp.addListener(q,function bq(){var br=this.__JK[bm][0];if(br.isDone()||br.isDisposed()){this.refresh(bm);}
;}
,this);bp.start();return bp;}
,longPoll:function(bs){var bu=this,bw=true,bx,by=0;function bt(){var bA=bx&&((new Date())-bx)<bu._getThrottleLimit();if(bA){by+=1;if(by>bu._getThrottleCount()){if(qx.core.Environment.get(j)){bu.debug(n+bu._getThrottleCount()+p+bu._getThrottleLimit()+t);}
;return true;}
;}
;if(!bA){by=0;}
;return false;}
;var bv=this.__JM[bs]=this.addListener(bs+a,function bz(){if(bu.isDisposed()){return;}
;if(!bt()){bx=new Date();bu.refresh(bs);}
;}
,bw);this.invoke(bs);return bv;}
,_getRequestConfig:function(bB,bC){var bE=this.__GY[bB];var bC=qx.lang.Object.clone(bC);if(!qx.lang.Type.isArray(bE)){throw new Error(c+bB);}
;var bH=bE[0],bD=this.__JN!==null?this.__JN+bE[1]:bE[1],bF=bE[2],bG=qx.io.rest.Resource.placeholdersFromUrl(bD);bC=bC||{};bG.forEach(function(bI){var bJ=new RegExp(f+bI+l),bK=bD.match(bJ)[1];if(typeof bC[bI]===d){if(bK){bC[bI]=bK;}
else {bC[bI]=u;}
;}
;bD=bD.replace(bJ,bC[bI]);}
);return {method:bH,url:bD,check:bF};}
...
,__JS:function(bL){qx.lang.Object.getKeys(bL).forEach(function(bM){var bO=bL[bM],bP=bO.method,bN=bO.url,bQ=bO.check;if(qx.core.Environment.get("qx.debug")){qx.core.Assert.assertString(bP,"Method must be string for route '"+bM+"'");qx.core.Assert.assertString(bN,"URL must be string for route '"+bM+"'");}
;this.map(bM,bP,bN,bQ);}
,this);}
,__JT:function(bR){if(!this.constructor.$$events){this.constructor.$$events={};}
;if(!this.constructor.$$events[bR]){this.constructor.$$events[bR]="qx.event.type.Rest";}
;}
},destruct:function(){var bS;for(bS in this.__JK){if(this.__JK[bS]){this.__JK[bS].forEach(function(bV){bV.dispose();}
);}
;}
;if(this.__JL){for(bS in this.__JL){var bU=this.__JL[bS];bU.stop();bU.dispose();}
;}
;if(this.__JM){for(bS in this.__JM){var bT=this.__JM[bS];this.removeListenerById(bT);}
;}
;this.__JK=this.__GY=this.__JL=null;}
});}
)();
...
(function(){var j="changeModel",i="model.user.avatar_url",h="application/json",g="github.view.Gist",f="model.description",e="west",d="/gists/{id}",c="Description",b="https://api.github.com",a="demobrowser.demo.data.Github",H="source",G="/gists",F="change",E="Content",D="center",C="#fff",B="Authorization",A="html",z="model.user.login",y="model.files",q="north",r="Gists",o="Accept",p="token ",m="github.rest.Resource",n="description",k="undefined",l="</pre>",s="<pre>",t="GET",v="value",u="get",x="model",w="bold";qx.Class.define(a,{extend:qx.application.Standalone,members:{__Pc:null,__Pd:null,__Pe:null,__Pf:null,__ry:null,__Pg:null,main:function(){if(typeof GITHUB==k){return;}
;qx.application.Standalone.prototype.main.call(this);this._setUpResources();this._setUpStores();this._createGui();this._setUpBinding();this.__Pc.get();this.__ry.addListener(j,function(I){var J=I.getData();this.__ry.getSelection().push(J.getItem(0));}
,this);this.__ry.getSelection().addListener(F,function(K){var L=this.__ry.getSelection().getItem(0).getId();this.__Pd.get({id:L});}
,this);}
,_setUpResources:function(){this.__Pc=new github.rest.Resource({"get":{method:t,url:G}});this.__Pd=new github.rest.Resource({"get":{method:t,url:d}});}
,_setUpStores:function(){this.__Pe=new qx.data.store.Rest(this.__Pc,u);this.__Pf=new qx.data.store.Rest(this.__Pd,u);}
,_createGui:function(){var M=new qx.ui.container.Composite(new qx.ui.layout.Dock());M.setPadding(10);var N=new qx.ui.basic.Label(r);N.setFont(w);N.setPaddingBottom(10);M.add(N,{edge:q});this.__ry=new qx.ui.list.List();this.__ry.setWidth(200);this.__Pg=new github.view.Gist();M.add(this.__ry,{edge:e});M.add(this.__Pg,{edge:D});this.getRoot().add(M,{edge:0});}
,_setUpBinding:function(){var P=this.__ry,O=this.__Pe,Q=this.__Pf;P.setLabelPath(n);P.setLabelOptions({converter:function(S,T,U,V){if(S===null||!S.length){return T.getId();}
;return S;}
});O.bind(x,P,x);var R=this.__Pg;Q.bind(f,R.getDescription(),v);Q.bind(z,R.getUsername(),v);Q.bind(i,R.getGravatar(),H);Q.bind(y,R.getContent(),A,{converter:function(W){var X=qx.Class.getProperties(W.constructor);var content=W.get(X[0]).getContent();content=qx.bom.String.escape(content);return s+content+l;}
});}
...
});qx.Class.define(g,{extend:qx.ui.container.Composite,construct:function(){qx.ui.container.Composite.call(this);var bb=new qx.ui.layout.Grid(5,5);bb.setColumnFlex(0,1);bb.setRowFlex(2,1);this.setLayout(bb);this.setPadding(0,10);var be;var bc=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));be=new qx.ui.basic.Label(c);be.setFont(w);this.__Ms=new qx.ui.basic.Label();bc.add(be);bc.add(this.__Ms);this.add(bc,{row:0,column:0});var bd=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));this.__Ph=new qx.ui.basic.Image();this.__Ph.set({width:40,height:40,scale:true,backgroundColor:C});this.__Pi=new qx.ui.basic.Label();bd.add(this.__Ph);bd.add(this.__Pi);this.add(bd,{row:0,column:1});be=new qx.ui.basic.Label(E);be.setFont(w);var scroll=new qx.ui.container.Scroll();this.__oX=new qx.ui.embed.Html();this.__oX.setMinHeight(1000);scroll.add(this.__oX);this.add(be,{row:1,column:0});this.add(scroll,{row:2,column:0,colSpan:2});}
,members:{getDescription:function(){return this.__Ms;}
,getGravatar:function(){return this.__Ph;}
,getUsername:function(){return this.__Pi;}
,getContent:function(){return this.__oX;}
}});}
)();
...
;this._delegate=j;this._marshaler=new qx.data.marshal.Json(j);if(j&&qx.lang.Type.isFunction(j.configureRequest)){this.__Du();}
;this.__Dt=qx.lang.Function.bind(this.__Dw,this);this.__Dv();}
,properties:{resource:{check:d},actionName:{check:b},model:{nullable:true,event:g}},members:{_marshaler:null,_delegate:null,__Dt:null,__Du:function(){var k=this.getResource(),l=this._delegate;k.configureRequest(l.configureRequest);}
,__Dv:function(){var n=this.getResource(),m=this.getActionName();if(n&&m){n.addListener(this.getActionName()+c,this.__Dt);}
;}
,__Dw:function(e){var p=e.getData(),o=this._marshaler,q,r=this.getModel(),s=this._delegate;if(p){if(s&&s.manipulateData){p=s.manipulateData(p);}
