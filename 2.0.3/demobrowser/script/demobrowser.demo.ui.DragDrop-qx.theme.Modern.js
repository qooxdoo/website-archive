...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.DragDrop","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.DragDrop-qx.theme.Modern.8c13c8d83ba7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="drag",k="TextArea Target",j="solid",h="Reorderable",g="qx.ui.form.ListItem",f="droprequest",d="#33508D",c="Source",b="Related of droprequest: ",a="demobrowser.demo.ui.DragDrop",C="Simple Target",B="Enable drag",A="\n",z="dragend",y="__ry",x="icon/16/places/folder.png",w="dragstart",v="Item ",u="copy",t="Related of drop: ",r="move",s="multi",p="dragover",q="drop",n="value",o="items";qx.Class.define(a,{extend:qx.application.Standalone,members:{__ry:null,__Qp:null,main:function(){qx.application.Standalone.prototype.main.call(this);var O=this.getRoot();var I=new qx.ui.container.Scroll();var D=new qx.ui.layout.Basic();var G=new qx.ui.container.Composite(D).set({allowStretchY:false,allowStretchX:false});I.add(G);O.add(I,{edge:0});var N=new qx.ui.basic.Label(c);G.add(N,{left:20,top:20});var L=new qx.ui.form.List;L.setDraggable(true);L.setSelectionMode(s);G.add(L,{left:20,top:40});for(var i=0;i<20;i++){L.add(new qx.ui.form.ListItem(v+i,x));}
;var P=new qx.ui.form.CheckBox(B);P.setValue(true);G.add(P,{left:20,top:260});L.addListener(w,function(e){if(!P.isValue()){e.preventDefault();}
;e.addType(n);e.addType(o);e.addAction(u);e.addAction(r);}
);L.addListener(f,function(e){this.debug(b+e.getRelatedTarget());var S=e.getCurrentAction();var U=e.getCurrentType();var R;switch(U){case o:R=this.getSelection();if(S==u){var T=[];for(var i=0,l=R.length;i<l;i++){T[i]=R[i].clone();}
;R=T;}
;break;case n:R=this.getSelection()[0].getLabel();break;};if(S==r){var Q=this.getSelection();for(var i=0,l=Q.length;i<l;i++){this.remove(Q[i]);}
;}
;e.addData(U,R);}
);var J=new qx.ui.basic.Label(C);G.add(J,{left:140,top:20});var H=new qx.ui.form.List;H.setDroppable(true);H.setSelectionMode(s);G.add(H,{left:140,top:40});H.addListener(q,function(e){this.debug(t+e.getRelatedTarget());var V=e.getData(o);for(var i=0,l=V.length;i<l;i++){this.add(V[i]);}
;}
);H.addListener(p,function(e){if(!e.supportsType(o)){e.preventDefault();}
;}
);var J=new qx.ui.basic.Label(k);G.add(J,{left:260,top:20});var M=new qx.ui.form.TextArea;M.setDroppable(true);M.setHeight(100);G.add(M,{left:260,top:40});M.addListener(q,function(e){this.debug(t+e.getRelatedTarget());this.setValue(e.getData(n).replace(/,/g,A));}
);M.addListener(p,function(e){if(!e.supportsType(n)){e.preventDefault();}
;}
);var K=new qx.ui.basic.Label(h);G.add(K,{left:500,top:20});var E=this.__ry=new qx.ui.form.List;E.setDraggable(true);E.setDroppable(true);E.setSelectionMode(s);G.add(E,{left:500,top:40});for(var i=0;i<20;i++){E.add(new qx.ui.form.ListItem(v+i,x));}
;var F=new qx.ui.core.Widget;F.setDecorator(new qx.ui.decoration.Single().set({top:[1,j,d]}));F.setHeight(0);F.setOpacity(0.5);F.setZIndex(100);F.setLayoutProperties({left:-1000,top:-1000});F.setDroppable(true);this.getRoot().add(F);E.addListener(w,function(e){e.addAction(r);}
);E.addListener(z,function(e){F.setDomPosition(-1000,-1000);}
);E.addListener(m,function(e){var X=e.getOriginalTarget();if(X instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__Qp=X;}
;if(!qx.ui.core.Widget.contains(this,X)&&X!=F){return;}
;var W=X.getContainerLocation();F.setWidth(X.getBounds().width);F.setDomPosition(W.left,W.top);}
);E.addListener(p,function(e){if(e.getRelatedTarget()){e.preventDefault();}
;}
);E.addListener(q,function(e){this.__Qq(e.getOriginalTarget());}
,this);F.addListener(q,function(e){this.__Qq(this.__Qp);}
,this);}
,__Qq:function(Y){if(Y.classname!=g){return;}
;var ba=this.__ry.getSortedSelection();for(var i=0,l=ba.length;i<l;i++){this.__ry.addBefore(ba[i],Y);this.__ry.addToSelection(ba[i]);}
;}
