...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Virtual_List-qx.theme.Simple.00b40e279a50.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var n="one",m="_applySpacing",k="Integer",h="_applyDragSelection",g="String",f="resize",d="demobrowser.demo.virtual.list.List",c="list",b="multi",a="_applyQuickSelection",C="_applyItemHeight",B="_applySelectionMode",A="additive",z="changeName",y="Number",x="addItem",w="removeItem",v="changeSelection",u="row",t="single",r="changeHeight",s=",",p="Boolean",q="selected",o="qx.event.type.Data";qx.Class.define(d,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider],construct:function(){qx.ui.virtual.core.Scroller.call(this,0,1,this.getItemHeight(),10);this.__Qj=new qx.ui.virtual.layer.WidgetCell(this);this.getPane().addLayer(this.__Qj);this.getPane().addListener(f,this._onResize,this);this.__dO=[];this.__cQ=[];this.__za=new qx.ui.virtual.selection.Row(this.getPane());this.__za.addListener(v,this._onChangeSelection,this);this.__za.attachMouseEvents();this.__za.attachKeyEvents(this);this.__za.attachListEvents(this);new qx.ui.virtual.behavior.Prefetch(this,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:400,maxAbove:600,minBelow:400,maxBelow:600}).set({interval:500});}
,events:{addItem:o,removeItem:o,changeValue:o,changeSelection:o},properties:{appearance:{refine:true,init:c},focusable:{refine:true,init:true},spacing:{check:k,init:0,apply:m,themeable:true},enableInlineFind:{check:p,init:true},name:{check:g,nullable:true,event:z},itemHeight:{init:24,themeable:true,check:y,apply:C},selectionMode:{check:[t,b,A,n],init:t,apply:B},dragSelection:{check:p,init:false,apply:h},quickSelection:{check:p,init:false,apply:a}},members:{__dO:null,__cQ:null,__za:null,__Qj:null,syncWidget:function(){this.update();}
,update:function(){var E=this.getPane().getRowConfig();E.setItemCount(this.__dO.length);E.resetItemSizes();for(var i=0;i<this.__dO.length;i++){var D=this.__dO[i].getHeight();if(D!==null){E.setItemSize(i,D);}
;}
;}
,updateSelection:function(){var F=this.__Qj.getChildren();for(var i=0;i<F.length;i++){var G=F[i];var H=G.getUserData(u);if(this.__za.isItemSelected(H)){G.addState(q);}
...
,_onChangeSelection:function(e){this.updateSelection();this.fireDataEvent(v,this.__Qn(e.getData()));}
,getCellWidget:function(I,J){var L=this.__dO[I];if(!L){return null;}
;var K=this.__cQ.pop()||new qx.ui.form.ListItem();K.set({label:L.getLabel(),icon:L.getIcon()});if(this.__za.isItemSelected(I)){K.addState(q);}
...
,hasChildren:function(){return this.__dO.length>0;}
...
,add:function(W){this.__dO.push(W);this._addHelper(W);return this;}
,remove:function(X){qx.lang.Array.remove(this.__dO,X);this._removeHelper(X);return this;}
,removeAll:function(){for(var i=0,j=this.__dO.length;i<j;i++){this._removeHelper(this.__dO[i]);}
;this.__dO=[];}
,indexOf:function(Y){return this.__dO.indexOf(Y);}
,addAt:function(ba,bb){qx.lang.Array.insertAt(this.__dO,ba,bb);this._addHelper(ba);}
,addBefore:function(bc,bd){qx.lang.Array.insertBefore(this.__dO,bc,bd);this._addHelper(bc);}
,addAfter:function(be,bf){qx.lang.Array.insertAfter(this.__dO,be,bf);this._addHelper(be);}
,removeAt:function(bg){this.__dO.splice(bg,1);this._removeHelper(this.__dO[bg]);}
,__Qk:function(bh){var bi=this.__dO.indexOf(bh);if(bi<0){return null;}
else {return bi;}
;}
,__Ql:function(bj){var bk=this.__dO[bj];return bk;}
,__Qm:function(bl){var bm=[];for(var i=0;i<bl.length;i++){bm.push(this.__Qk(bl[i]));}
;return bm;}
,__Qn:function(bn){var bo=[];for(var i=0;i<bn.length;i++){bo.push(this.__dO[bn[i]]);}
;return bo;}
,selectAll:function(){this.__za.selectAll();}
,setSelected:function(bp){this.__za.selectItem(this.__Qk(bp));}
,isSelected:function(bq){return this.__za.isItemSelected(this.__Qk(bq));}
,addToSelection:function(br){this.__za.addItem(this.__Qk(br));}
,removeFromSelection:function(bs){this.__za.removeItem(this.__Qk(bs));}
,selectRange:function(bt,bu){this.__za.selectItemRange(this.__Qk(bt),this.__Qk(bu));}
,resetSelection:function(){this.__za.clearSelection();}
,setSelection:function(bv){this.__za.replaceSelection(this.__Qm(bv));}
,getSelectedItem:function(){return this.__dO[this.getSelected()];}
,getSelected:function(){return this.__dO[this.__za.getSelectedItem()];}
,getSelection:function(){return this.__Qn(this.__za.getSelection());}
,getSortedSelection:function(){return this.__Qn(this.__za.getSortedSelection());}
,isSelectionEmpty:function(){return this.__za.isSelectionEmpty();}
,getSelectionContext:function(){return this.__za.getSelectionContext();}
,getSelectables:function(){return this.__Qn(this.__za.getSelectables());}
,_applySelectionMode:function(bw,bx){this.__za.setMode(bw);}
,_applyDragSelection:function(by,bz){this.__za.setDrag(by);}
,_applyQuickSelection:function(bA,bB){this.__za.setQuick(bA);}
}});}
)();
