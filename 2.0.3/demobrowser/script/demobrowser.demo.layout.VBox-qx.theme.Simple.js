...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.layout.VBox","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.VBox-qx.theme.Simple.484d65f52d48.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="Boolean",j="It is not allowed to have more than one child aligned to 'center'!",h="height",g="bottom",f="The property '",e="qx.debug",d="width",c="' is not supported by the Dock layout!",b="qx.ui.layout.Dock",a="_applySort",A="right",z="west",y="Integer",x="auto",w="center",v="y",u="Decorator",t="north",s="east",r="edge",p="south",q="flex",n="x",o="top",l="left",m="_applyLayoutChange";qx.Class.define(b,{extend:qx.ui.layout.Abstract,construct:function(B,C,D,E){qx.ui.layout.Abstract.call(this);if(B){this.setSpacingX(B);}
;if(C){this.setSpacingY(C);}
;if(D){this.setSeparatorX(D);}
;if(E){this.setSeparatorY(E);}
;}
,properties:{sort:{check:[x,v,n],init:x,apply:a},separatorX:{check:u,nullable:true,apply:m},separatorY:{check:u,nullable:true,apply:m},connectSeparators:{check:k,init:false,apply:m},spacingX:{check:y,init:0,apply:m},spacingY:{check:y,init:0,apply:m}},members:{__kA:null,__pw:null,verifyLayoutProperty:qx.core.Environment.select(e,{"true":function(F,name,G){this.assertInArray(name,[q,r,h,d],f+name+c);if(name===r){this.assertInArray(G,[t,p,z,s,w]);}
else if(name===q){this.assertNumber(G);this.assert(G>=0);}
...
,__px:{north:1,south:2,west:3,east:4,center:5},__py:{'1':o,'2':g,'3':l,'4':A},__ll:function(){var H=this._getLayoutChildren();var P,J;var length=H.length;var L=[];var O=[];var M=[];var I=this.getSort()===v;var N=this.getSort()===n;for(var i=0;i<length;i++){P=H[i];M=P.getLayoutProperties().edge;if(M===w){if(J){throw new Error(j);}
;J=P;}
else if(N||I){if(M===t||M===p){I?L.push(P):O.push(P);}
else if(M===z||M===s){I?O.push(P):L.push(P);}
;}
else {L.push(P);}
;}
;var Q=L.concat(O);if(J){Q.push(J);}
;this.__kA=Q;var K=[];for(var i=0;i<length;i++){M=Q[i].getLayoutProperties().edge;K[i]=this.__px[M]||5;}
;this.__pw=K;delete this._invalidChildrenCache;}
,renderLayout:function(R,S){if(this._invalidChildrenCache){this.__ll();}
;var bm=qx.ui.layout.Util;var Y=this.__kA;var bn=this.__pw;var length=Y.length;var bc,U,bb,bh,bi,bs,bf,V,bp;var bw=[];var be=[];var bv=this._getSeparatorWidths();var bA=this.getSpacingX();var bz=this.getSpacingY();var ba=-bA;var bo=-bz;if(bv.x){ba-=bv.x+bA;}
;if(bv.y){bo-=bv.y+bz;}
;for(var i=0;i<length;i++){U=Y[i];bh=U.getLayoutProperties();bb=U.getSizeHint();bf=bb.width;V=bb.height;if(bh.width!=null){bf=Math.floor(R*parseFloat(bh.width)/100);if(bf<bb.minWidth){bf=bb.minWidth;}
else if(bf>bb.maxWidth){bf=bb.maxWidth;}
;}
;if(bh.height!=null){V=Math.floor(S*parseFloat(bh.height)/100);if(V<bb.minHeight){V=bb.minHeight;}
else if(V>bb.maxHeight){V=bb.maxHeight;}
;}
;bw[i]=bf;be[i]=V;switch(bn[i]){case 1:case 2:bo+=V+U.getMarginTop()+U.getMarginBottom()+bz;if(bv.y){bo+=bv.y+bz;}
;break;case 3:case 4:ba+=bf+U.getMarginLeft()+U.getMarginRight()+bA;if(bv.x){ba+=bv.x+bA;}
;break;default:ba+=bf+U.getMarginLeft()+U.getMarginRight()+bA;bo+=V+U.getMarginTop()+U.getMarginBottom()+bz;if(bv.x){ba+=bv.x+bA;}
;if(bv.y){bo+=bv.y+bz;}
;};}
;if(ba!=R){bc={};bs=ba<R;for(var i=0;i<length;i++){U=Y[i];switch(bn[i]){case 3:case 4:case 5:bi=U.getLayoutProperties().flex;if(bi==null&&bn[i]==5){bi=1;}
;if(bi>0){bb=U.getSizeHint();bc[i]={min:bb.minWidth,value:bw[i],max:bb.maxWidth,flex:bi};}
;};}
;var W=bm.computeFlexOffsets(bc,R,ba);for(var i in W){bp=W[i].offset;bw[i]+=bp;ba+=bp;}
;}
;if(bo!=S){bc={};bs=bo<S;for(var i=0;i<length;i++){U=Y[i];switch(bn[i]){case 1:case 2:case 5:bi=U.getLayoutProperties().flex;if(bi==null&&bn[i]==5){bi=1;}
;if(bi>0){bb=U.getSizeHint();bc[i]={min:bb.minHeight,value:be[i],max:bb.maxHeight,flex:bi};}
;};}
;var W=bm.computeFlexOffsets(bc,S,bo);for(var i in W){bp=W[i].offset;be[i]+=bp;bo+=bp;}
;}
;this._clearSeparators();var bt=this.getSeparatorX(),bu=this.getSeparatorY();var bx=this.getConnectSeparators();var bk=0,br=0;var bE,top,bf,V,bj,bB;var bl,by,bD,T;var bC,bd,bg,X;var bq=this.__py;for(var i=0;i<length;i++){U=Y[i];bB=bn[i];bb=U.getSizeHint();bC=U.getMarginTop();bd=U.getMarginBottom();bg=U.getMarginLeft();X=U.getMarginRight();switch(bB){case 1:case 2:bf=R-bg-X;if(bf<bb.minWidth){bf=bb.minWidth;}
else if(bf>bb.maxWidth){bf=bb.maxWidth;}
;V=be[i];top=bk+bm.computeVerticalAlignOffset(bq[bB],V,S,bC,bd);bE=br+bm.computeHorizontalAlignOffset(U.getAlignX()||l,bf,R,bg,X);if(bv.y){if(bB==1){by=bk+V+bC+bz+bd;}
else {by=bk+S-V-bC-bz-bd-bv.y;}
;bl=bE;bD=R;if(bx&&bl>0){bl-=bA+bg;bD+=(bA)*2;}
else {bl-=bg;}
;this._renderSeparator(bu,{left:bl,top:by,width:bD,height:bv.y});}
;bj=V+bC+bd+bz;if(bv.y){bj+=bv.y+bz;}
;S-=bj;if(bB==1){bk+=bj;}
;break;case 3:case 4:V=S-bC-bd;if(V<bb.minHeight){V=bb.minHeight;}
else if(V>bb.maxHeight){V=bb.maxHeight;}
;bf=bw[i];bE=br+bm.computeHorizontalAlignOffset(bq[bB],bf,R,bg,X);top=bk+bm.computeVerticalAlignOffset(U.getAlignY()||o,V,S,bC,bd);if(bv.x){if(bB==3){bl=br+bf+bg+bA+X;}
else {bl=br+R-bf-bg-bA-X-bv.x;}
;by=top;T=S;if(bx&&by>0){by-=bz+bC;T+=(bz)*2;}
else {by-=bC;}
;this._renderSeparator(bt,{left:bl,top:by,width:bv.x,height:T});}
;bj=bf+bg+X+bA;if(bv.x){bj+=bv.x+bA;}
;R-=bj;if(bB==3){br+=bj;}
;break;default:bf=R-bg-X;V=S-bC-bd;if(bf<bb.minWidth){bf=bb.minWidth;}
else if(bf>bb.maxWidth){bf=bb.maxWidth;}
;if(V<bb.minHeight){V=bb.minHeight;}
else if(V>bb.maxHeight){V=bb.maxHeight;}
;bE=br+bm.computeHorizontalAlignOffset(U.getAlignX()||l,bf,R,bg,X);top=bk+bm.computeVerticalAlignOffset(U.getAlignY()||o,V,S,bC,bd);};U.renderLayout(bE,top,bf,V);}
;}
,_getSeparatorWidths:function(){var bG=this.getSeparatorX(),bF=this.getSeparatorY();if(bG||bF){var bL=qx.theme.manager.Decoration.getInstance();}
;if(bG){var bM=bL.resolve(bG);var bI=bM.getInsets();var bJ=bI.left+bI.right;}
;if(bF){var bN=bL.resolve(bF);var bH=bN.getInsets();var bK=bH.top+bH.bottom;}
;return {x:bJ||0,y:bK||0};}
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__ll();}
;var bR=this.__kA;var cc=this.__pw;var length=bR.length;var bV,cd;var bX,bW;var bY=0,ck=0;var bO=0,cf=0;var ca=0,cj=0;var bP=0,ce=0;var cg=this._getSeparatorWidths();var ci=this.getSpacingX(),ch=this.getSpacingY();var bT=-ci,bS=-ch;if(cg.x){bT-=cg.x+ci;}
;if(cg.y){bS-=cg.y+ch;}
;for(var i=0;i<length;i++){cd=bR[i];bV=cd.getSizeHint();bX=cd.getMarginLeft()+cd.getMarginRight();bW=cd.getMarginTop()+cd.getMarginBottom();switch(cc[i]){case 1:case 2:ca=Math.max(ca,bV.width+bY+bX);cj=Math.max(cj,bV.minWidth+ck+bX);bP+=bV.height+bW;ce+=bV.minHeight+bW;bS+=ch;if(cg.y){bS+=cg.y+ch;}
;break;case 3:case 4:bO=Math.max(bO,bV.height+bP+bW);cf=Math.max(cf,bV.minHeight+ce+bW);bY+=bV.width+bX;ck+=bV.minWidth+bX;bT+=ci;if(cg.x){bT+=cg.x+ci;}
;break;default:bY+=bV.width+bX;ck+=bV.minWidth+bX;bP+=bV.height+bW;ce+=bV.minHeight+bW;bT+=ci;if(cg.x){bT+=cg.x+ci;}
;bS+=ch;if(cg.y){bS+=cg.y+ch;}
;};}
;var bU=Math.max(ck,cj)+bT;var cl=Math.max(bY,ca)+bT;var cb=Math.max(cf,ce)+bS;var bQ=Math.max(bO,bP)+bS;return {minWidth:bU,width:cl,minHeight:cb,height:bQ};}
},destruct:function(){this.__pw=this.__kA=null;}
});}
)();
...
(function(){var i="demobrowser.demo.layout.VBox",h="center",g="left",f="right",e="middle",d="bottom",c="yellow",b="green",a="main";qx.Class.define(i,{extend:demobrowser.demo.util.LayoutApplication,members:{main:function(){demobrowser.demo.util.LayoutApplication.prototype.main.call(this);var scroll=new qx.ui.container.Scroll();this.getRoot().add(scroll,{edge:0});var j=new qx.ui.container.Composite(new qx.ui.layout.HBox(20)).set({padding:20});scroll.add(j);j.add(this.getBox1());j.add(this.getBox2());j.add(this.getBox3());j.add(this.getBox4());j.add(this.getBox5());j.add(this.getBox6());j.add(this.getBox7());}
,getBox1:function(){var k=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({decorator:a,backgroundColor:c,allowGrowY:false});k.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}));k.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}));k.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}));return k;}
,getBox2:function(){var l=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({decorator:a,backgroundColor:c,minWidth:60,allowGrowY:false});l.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,maxWidth:40,alignX:g}));l.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,maxWidth:40,alignX:h}));l.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,maxWidth:40,alignX:f}));return l;}
,getBox3:function(){var m=new qx.ui.layout.VBox();var n=new qx.ui.container.Composite(m).set({decorator:a,backgroundColor:c,height:300,allowGrowY:false});m.setSpacing(5);m.setAlignY(d);n.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}));n.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}));n.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}));return n;}
,getBox4:function(){var o=new qx.ui.layout.VBox();var p=new qx.ui.container.Composite(o).set({decorator:a,backgroundColor:c,height:300,allowGrowY:false});o.setSpacing(5);o.setAlignY(e);p.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}));p.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}));p.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}));return p;}
,getBox5:function(){var r=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({decorator:a,backgroundColor:c,allowGrowY:false});var t=new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,marginBottom:10});var s=new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,marginBottom:10,marginTop:20});var q=new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,marginBottom:10});r.add(t);r.add(s);r.add(q);return r;}
,getBox6:function(){var u=new qx.ui.layout.VBox();var w=new qx.ui.container.Composite(u).set({decorator:a,backgroundColor:c,height:300,allowGrowY:false});u.setSpacing(5);u.setAlignY(d);var y=new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,marginBottom:10});var x=new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,marginBottom:10,marginTop:20});var v=new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,marginBottom:10});w.add(y);w.add(x);w.add(v);return w;}
,getBox7:function(){var z=new qx.ui.layout.VBox();var B=new qx.ui.container.Composite(z).set({decorator:a,backgroundColor:c,height:300,allowGrowY:false});z.setSpacing(5);z.setAlignY(e);var D=new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,marginBottom:10});var C=new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,marginBottom:10,marginTop:20});var A=new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,marginBottom:10});B.add(D);B.add(C);B.add(A);return B;}
}});}
)();
