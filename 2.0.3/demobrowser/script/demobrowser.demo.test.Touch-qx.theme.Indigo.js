...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Touch-qx.theme.Indigo.3bdc6ea7c47a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="touchmove",k="Two",j="Target Touches: ",h="Changed Target Touches: ",g="<b>Cancel Touch</b>",f="Is Multi-Touch: ",d="Screen: Left = ",c="30px",b="qx.debug",a="Identifier: ",ba="All Touches: ",Y="event.touch",X=":",W="Touch canceled",V="Document: Left = ",U="touchend",T="Viewport: Left = ",S='Lucida Grande',R="qx.mobile.emulatetouch",Q="touchstart",s="__infoArea",t="demobrowser.demo.test.Touch",q="9px",r="engine.name",o="click",p="Type: ",m="20px",n="red",u="</b>",v="webkit",E="<br>",C="touchcancel",I="<b>This demo is supposed to be run in a WebKit-based browser on a touch-enabled device.</b>",G="Are all touch targets div ",M="One",K="blue",z="<b>Touch me:<br>",P="==============",O="green",N="55px",w=" / Top = ",A="12px",B="Arial",D="90px",F="innerHTML",H="absolute",J="div",L="px";qx.Class.define(t,{extend:qx.application.Native,members:{__Qb:null,__Qc:null,main:function(){qx.application.Native.prototype.main.call(this);if((qx.core.Environment.get(b))){qx.log.appender.Native;qx.log.appender.Console;}
;var bc=new qx.html.Element(J);bc.useElement(document.body);bc.setRoot(true);if(qx.core.Environment.get(r)!=v||(!qx.core.Environment.get(Y)&&qx.core.Environment.get(R)==false)){var bb={"color":O,"position":H,"font-family":S,"font-size":A,"left":c,"top":m};var bd=new qx.html.Element(J,bb);bc.add(bd);bd.setAttribute(F,I);return;}
;bc.add(this.__Qd(10,10,O,M));bc.add(this.__Qd(110,10,K,k));this.__Qb=this.__Qe(10,110);bc.add(this.__Qb);this.__Qc=this.__Qe(110,110);bc.add(this.__Qc);bc.add(this.__Qf(210,10));}
,__Qd:function(x,y,be,bf){var bg={"position":H,"left":x+L,"top":y+L,"width":D,"height":D,"backgroundColor":be,"fontFamily":B,"fontSize":A};var bh=new qx.html.Element(J,bg,{"id":bf});bh.setAttribute(F,z+bf+u);bh.addListener(Q,this.__Qg,this);bh.addListener(l,this.__Qg,this);bh.addListener(U,this.__Qg,this);bh.addListener(C,this.__Qg,this);return bh;}
,__Qe:function(x,y){var bi={"position":H,"left":x+L,"top":y+L,"width":D,"fontFamily":B,"fontSize":q};return new qx.html.Element(J,bi);}
,__Qf:function(x,y){var bj={"position":H,"left":x+L,"top":y+L,"width":N,"height":N,"backgroundColor":n,"fontFamily":B,"fontSize":A};var bk=new qx.html.Element(J,bj);bk.setAttribute(F,g);bk.addListener(o,function(){window.setTimeout(function(){alert(W);}
,0);}
,this);return bk;}
,__Qg:function(e){var bl=[];bl.push(p+e.getType());bl.push(P);bl.push(f+e.isMultiTouch());bl.push(ba+e.getAllTouches().length);bl.push(j+e.getTargetTouches().length);bl.push(h+e.getChangedTargetTouches().length);bl.push(P);bl.push(V+e.getDocumentLeft()+w+e.getDocumentTop());bl.push(d+e.getScreenLeft()+w+e.getScreenTop());bl.push(T+e.getViewportLeft()+w+e.getViewportTop());bl.push(a+e.getIdentifier());var bn=e.getTarget();var bm=false;var bp=e.getTargetTouches().length>0?e.getTargetTouches():e.getChangedTargetTouches();for(var i=0;i<bp.length;i++){bm=bp[0].target==bn;if(!bm){break;}
;}
;bl.push(G+bn.id+X+bm);var bo=this[s+bn.id];if(bo){bo.setAttribute(F,bl.join(E));}
;}
}});}
)();
