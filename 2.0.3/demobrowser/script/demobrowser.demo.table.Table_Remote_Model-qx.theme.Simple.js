...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Remote_Model-qx.theme.Simple.3ff76761909c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="icon/22/actions/view-refresh.png",j='90%',i="interval",h="demobrowser.demo.table.Table_Remote_Model",g='10%',f="value",e='Reload',d='execute',c="enabled",b='Poll',a="execute";qx.Class.define(h,{extend:demobrowser.demo.table.TableDemo,construct:function(){demobrowser.demo.table.TableDemo.call(this);this.__lm=new qx.event.Timer(1000);}
,members:{__lm:null,createTable:function(){var l=this._tableModel=new demobrowser.demo.table.RemoteTableModel();var o={tableColumnModel:function(p){return new qx.ui.table.columnmodel.Resize(p);}
};this.__lm.addListener(i,l.reloadData,l);var m=new qx.ui.table.Table(l,o);var n=m.getTableColumnModel().getBehavior();n.setWidth(0,g);n.setWidth(1,j);return m;}
...
,this);s.add(r);var q=new qx.ui.toolbar.CheckBox(b);q.addListener(a,function(){if(q.getValue()){this.__lm.start();}
else {this.__lm.stop();}
;}
...
(function(){var f="qx.ui.table.model.Remote",e="Boolean",d="Integer",c="_loadRowCount is abstract",b="metaDataChanged",a="dataChanged";qx.Class.define(f,{extend:qx.ui.table.model.Abstract,construct:function(){qx.ui.table.model.Abstract.call(this);this.__AN=-1;this.__AQ=true;this.__In=-1;this.__Io=0;this.__Ip=-1;this.__Iq=-1;this.__Ir=-1;this.__Is=false;this.__It={};this.__Iu=0;this.__AP=null;this.__yN=null;}
,properties:{blockSize:{check:d,init:50},maxCachedBlockCount:{check:d,init:15},clearCacheOnRemove:{check:e,init:false},blockConcurrentLoadRowCount:{check:e,init:true}},members:{__In:null,__Is:null,__Io:null,__Ip:null,__Iq:null,__Ir:null,__It:null,__Iu:null,__AN:null,__AQ:null,__yN:null,__AP:null,__Iv:false,_clearCache:false,_getIgnoreCurrentRequest:function(){return this.__Is;}
,getRowCount:function(){if(this.__In==-1){if(!this.__Iv||!this.getBlockConcurrentLoadRowCount()){this.__Iv=true;this._loadRowCount();}
;return (this.__In==-1)?0:this.__In;}
else {return this.__In;}
;}
,_loadRowCount:function(){throw new Error(c);}
,_onRowCountLoaded:function(g){if(this.getBlockConcurrentLoadRowCount()){this.__Iv=false;}
;if(g==null||g<0){g=0;}
;this.__In=Number(g);var h={firstRow:0,lastRow:g-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,h);}
,reloadData:function(){if(this.__Ip!=-1){var j=this._cancelCurrentRequest();if(j){this.__Ip=-1;this.__Is=false;}
else {this.__Is=true;}
;}
;this._clearCache=true;this.__Iq=-1;this.__Ir=-1;this.__Iv=true;this._loadRowCount();}
,clearCache:function(){this.__It={};this.__Iu=0;}
,getCacheContent:function(){return {sortColumnIndex:this.__AN,sortAscending:this.__AQ,rowCount:this.__In,lruCounter:this.__Io,rowBlockCache:this.__It,rowBlockCount:this.__Iu};}
,restoreCacheContent:function(k){if(this.__Ip!=-1){var m=this._cancelCurrentRequest();if(m){this.__Ip=-1;this.__Is=false;}
else {this.__Is=true;}
;}
;this.__AN=k.sortColumnIndex;this.__AQ=k.sortAscending;this.__In=k.rowCount;this.__Io=k.lruCounter;this.__It=k.rowBlockCache;this.__Iu=k.rowBlockCount;var l={firstRow:0,lastRow:this.__In-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,l);}
,_cancelCurrentRequest:function(){return false;}
,iterateCachedRows:function(n,o){var q=this.getBlockSize();var p=Math.ceil(this.getRowCount()/q);for(var x=0;x<=p;x++){var r=this.__It[x];if(r!=null){var w=x*q;var v=r.rowDataArr;for(var u=0;u<v.length;u++){var t=v[u];var s=n.call(o,w+u,t);if(s!=null){v[u]=s;}
;}
;}
;}
;}
,prefetchRows:function(y,z){if(this.__Ip==-1||this._clearCache){var A=this.getBlockSize();var F=Math.ceil(this.__In/A);var E=parseInt(y/A,10)-1;if(E<0){E=0;}
;var D=parseInt(z/A,10)+1;if(D>=F){D=F-1;}
;var C=-1;var B=-1;for(var G=E;G<=D;G++){if(this._clearCache||this.__It[G]==null||this.__It[G].isDirty){if(C==-1){C=G;}
;B=G;}
;}
;if(C!=-1){this.__Iq=-1;this.__Ir=-1;this.__Ip=C;this._loadRowData(C*A,(B+1)*A-1);}
;}
else {this.__Iq=y;this.__Ir=z;}
;}
...
;if(J!=null&&!this.__Is){var M=this.getBlockSize();var K=Math.ceil(J.length/M);if(K==1){this._setRowBlockData(this.__Ip,J);}
else {for(var i=0;i<K;i++){var P=i*M;var O=[];var L=Math.min(M,J.length-P);for(var Q=0;Q<L;Q++){O.push(J[P+Q]);}
;this._setRowBlockData(this.__Ip+i,O);}
;}
;var N={firstRow:this.__Ip*M,lastRow:(this.__Ip+K+1)*M-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,N);}
;this.__Ip=-1;this.__Is=false;if(this.__Iq!=-1){this.prefetchRows(this.__Iq,this.__Ir);}
;}
,_setRowBlockData:function(R,S){if(this.__It[R]==null){this.__Iu++;while(this.__Iu>this.getMaxCachedBlockCount()){var W;var V=this.__Io;for(var U in this.__It){var T=this.__It[U].lru;if(T<V&&U>1){V=T;W=U;}
;}
;delete this.__It[W];this.__Iu--;}
;}
;this.__It[R]={lru:++this.__Io,rowDataArr:S};}
...
else {var bb=this.getBlockSize();var bc=Math.ceil(this.getRowCount()/bb);var bd=parseInt(X/bb,10);for(var bg=bd;bg<=bc;bg++){var Y=this.__It[bg];if(Y!=null){var ba=0;if(bg==bd){ba=X-bg*bb;}
;Y.rowDataArr.splice(ba,1);if(bg==bc-1){if(Y.rowDataArr.length==0){delete this.__It[bg];}
;}
else {var be=this.__It[bg+1];if(be!=null){Y.rowDataArr.push(be.rowDataArr[0]);}
else {Y.isDirty=true;}
;}
;}
;}
;if(this.__In!=-1){this.__In--;}
...
;}
;}
,getRowData:function(bh){var bi=this.getBlockSize();var bl=parseInt(bh/bi,10);var bj=this.__It[bl];if(bj==null){return null;}
else {var bk=bj.rowDataArr[bh-(bl*bi)];if(bj.lru!=this.__Io){bj.lru=++this.__Io;}
;return bk;}
;}
...
;}
;}
,setEditable:function(bw){this.__yN=[];for(var bx=0;bx<this.getColumnCount();bx++){this.__yN[bx]=bw;}
;this.fireEvent(b);}
,setColumnEditable:function(by,bz){if(bz!=this.isColumnEditable(by)){if(this.__yN==null){this.__yN=[];}
;this.__yN[by]=bz;this.fireEvent(b);}
;}
,isColumnEditable:function(bA){return (this.__yN?(this.__yN[bA]==true):false);}
,setColumnSortable:function(bB,bC){if(bC!=this.isColumnSortable(bB)){if(this.__AP==null){this.__AP=[];}
;this.__AP[bB]=bC;this.fireEvent(b);}
;}
,isColumnSortable:function(bD){return (this.__AP?(this.__AP[bD]!==false):true);}
,sortByColumn:function(bE,bF){if(this.__AN!=bE||this.__AQ!=bF){this.__AN=bE;this.__AQ=bF;this.clearCache();this.fireEvent(b);}
;}
,getSortColumnIndex:function(){return this.__AN;}
,isSortAscending:function(){return this.__AQ;}
,setSortColumnIndexWithoutSortingData:function(bG){this.__AN=bG;}
,setSortAscendingWithoutSortingData:function(bH){this.__AQ=bH;}
},destruct:function(){this.__AP=this.__yN=this.__It=null;}
});}
)();
...
,members:{__PQ:null,__PR:false,_loadRowCount:function(){this.__PS();if(this.__PR){return;}
;if(this.__PQ){this.__PT();}
else {this.__PW();}
;}
,_loadRowData:function(f,g){this.__PS();if(this.__PR){return;}
;if(this.__PQ){this.__PU(f,g);}
else {this.__PX(f,g);}
;}
,__PS:function(){if(this.__PR||this.__PQ!==null){return;}
;this.__PR=true;this.__PV("method=checkphp",function(h){this.__PR=false;this.__PQ=(h=="WTF PHP");this._loadRowCount();}
);}
,__PT:function(){var j="method=getRowCount";this.__PV(j,function(k){this._onRowCountLoaded(parseInt(k));}
);}
,__PU:function(l,m){var n="method=getRowData&start="+l+"&end="+m;this.__PV(n,function(o){this._onRowDataLoaded(qx.lang.Json.parse(o));}
);}
,__PV:function(p,q){var r=new qx.io.request.Xhr();r.setUrl("../../resource/demobrowser/backend/remote_table.php?"+p);r.addListener("success",function(){q.call(this,r.getResponseText());}
,this);r.send();}
,__PW:function(){var self=this;window.setTimeout(function(){self._onRowCountLoaded(1000000);}
,0);}
,__PX:function(s,t){var self=this;window.setTimeout(function(){var u=[];for(var i=s;i<=t;i++){u.push({id:i,text:'Hello '+i+' Generated on:'+(new Date())});}
