...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.ExtendedList","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ExtendedList-qx.theme.Modern.47421b0a83eb.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="Jack",j="kiss",h="demobrowser.demo.data.ExtendedList",g="Jim",f="sad",e="<b>Displaying Checkboxes in a list</b><br/>",d="plain",c="surprise",b="embarrassed",a="name",z="Write data to log",y="Jakob",x="Dan",w="Tim",v="Karl",u="Online <> Offline",t="This list display a set of persons in a list as checkbox. Every ",s="label",r="Dustin",q="angel",o="Max",p="person does have a name as label and an online status as checkbox.",m="execute",n="value",l="online";qx.Class.define(h,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var K=[o,y,w,k,x,r,v,g];var D=[b,j,d,f,c,q];var I=[];for(var i=0;i<20;i++){var F=new demobrowser.demo.data.model.Person();F.setName(K[i%K.length]);F.setEmote(D[i%D.length]);F.setOnline(i%3==0);I.push(F);}
;var J=new qx.data.Array(I);var C=new qx.ui.form.List();C.setWidth(150);this.getRoot().add(C,{left:10,top:80});var G=new qx.data.controller.List(null,C);var B={configureItem:function(L){L.setPadding(3);}
,createItem:function(){return new qx.ui.form.CheckBox();}
,bindItem:function(M,N,O){M.bindProperty(a,s,null,N,O);M.bindProperty(l,n,null,N,O);M.bindPropertyReverse(l,n,null,N,O);}
};G.setDelegate(B);G.setModel(J);var A=new qx.ui.form.Button(u);A.setWidth(120);this.getRoot().add(A,{left:180,top:80});A.addListener(m,function(){for(var i=0;i<J.length;i++){J.getItem(i).toggleOnline();}
;}
,this);var H=new qx.ui.form.Button(z);H.setWidth(120);this.getRoot().add(H,{left:180,top:115});H.addListener(m,function(){qx.log.appender.Console.show();this.info(J.toString());}
,this);var E=new qx.ui.basic.Label();E.setRich(true);E.setWidth(410);E.setValue(e+t+p);this.getRoot().add(E,{left:10,top:10});}
}});}
)();
(function(){var i="demobrowser.demo.data.model.Person",h="Boolean",g="changeName",f="offline",e=" is ",d="online",c="changeOnline",b="changeEmote",a="String";qx.Class.define(i,{extend:qx.core.Object,properties:{name:{check:a,event:g,nullable:true},emote:{check:a,event:b},online:{check:h,event:c,init:true}},members:{toString:function(){return this.getName()+e+(this.getOnline()?d:f);}
}});}
)();
