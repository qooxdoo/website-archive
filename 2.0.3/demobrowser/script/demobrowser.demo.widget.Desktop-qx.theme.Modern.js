...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.Desktop","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Desktop-qx.theme.Modern.586b7848911d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var f="demobrowser.demo.widget.Desktop",e="icon/32/actions/go-home.png",d="Window #",c="Desktop",b="main",a="background-pane";qx.Class.define(f,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var h=new qx.ui.tabview.TabView();this.getRoot().add(h,{edge:0});var m=new qx.ui.tabview.Page(c,e);m.setLayout(new qx.ui.layout.Grow());h.add(m);var j=new qx.ui.window.Manager();var k=new qx.ui.window.Desktop(j);k.set({decorator:b,backgroundColor:a});m.add(k);var g=[[300,200,30,50],[250,250,150,70],[400,300,300,60]];for(var i=0;i<g.length;i++){var n=g[i];var l=new qx.ui.window.Window(d+(i+1)).set({width:n[0],height:n[1],showClose:false,showMinimize:false});l.moveTo(n[2],n[3]);k.add(l);l.open();}
;}
}});}
)();
...
(function(){var m="qx.event.type.Data",k="barRight",j="beforeChangeSelection",h="horizontal",g="__Bi",f="close-button",d="Index should be less than : ",c="tabview",b="vertical",a="_applyBarPosition",E="barLeft",D="qx.ui.tabview.TabView",C="barTop",B="barBottom",A="bottom",z="button",y="qx.debug",x="changeSelection",w="Incompatible child for TabView: ",v="top",t="left",u="close",r="right",s="page",p="firstTab",q="bar",n="lastTab",o="pane";qx.Class.define(D,{extend:qx.ui.core.Widget,implement:qx.ui.core.ISingleSelection,include:[qx.ui.core.MContentPadding],construct:function(F){qx.ui.core.Widget.call(this);this.__GM={top:C,right:k,bottom:B,left:E};this._createChildControl(q);this._createChildControl(o);var G=this.__Bi=new qx.ui.form.RadioGroup;G.setWrap(false);G.addListener(x,this._onChangeSelection,this);if(F!=null){this.setBarPosition(F);}
else {this.initBarPosition();}
;}
,events:{"changeSelection":m},properties:{appearance:{refine:true,init:c},barPosition:{check:[t,r,v,A],init:v,apply:a}},members:{__Bi:null,_createChildControlImpl:function(H,I){var J;switch(H){case q:J=new qx.ui.container.SlideBar();J.setZIndex(10);this._add(J);break;case o:J=new qx.ui.container.Stack;J.setZIndex(5);this._add(J,{flex:1});break;};return J||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,H);}
...
;}
;var L=K.getButton();var M=this.getChildControl(q);var O=this.getChildControl(o);K.exclude();M.add(L);O.add(K);this.__Bi.add(L);K.addState(this.__GM[this.getBarPosition()]);K.addState(n);var N=this.getChildren();if(N[0]==K){K.addState(p);}
...
;}
;var T=this.getChildren();if(!(Q==null)&&Q>T.length){throw new Error(d+T.length);}
;if(Q==null){Q=T.length;}
;var R=P.getButton();var S=this.getChildControl(q);var U=this.getChildControl(o);P.exclude();S.addAt(R,Q);U.addAt(P,Q);this.__Bi.add(R);P.addState(this.__GM[this.getBarPosition()]);T=this.getChildren();if(Q==T.length-1){P.addState(n);}
...
;}
;Y.remove(X);bb.remove(V);this.__Bi.remove(X);V.removeState(this.__GM[this.getBarPosition()]);if(V.hasState(p)){V.removeState(p);if(ba[0]){ba[0].addState(p);}
;}
...
,__GM:null,_applyBarPosition:function(bd,be){var bf=this.getChildControl(q);var bk=bd==t||bd==r;var bi=bd==r||bd==A;var bj=bk?qx.ui.layout.HBox:qx.ui.layout.VBox;var bm=this._getLayout();if(bm&&bm instanceof bj){}
else {this._setLayout(bm=new bj);}
;bm.setReversed(bi);bf.setOrientation(bk?b:h);var bl=this.getChildren();if(be){var bg=this.__GM[be];bf.removeState(bg);for(var i=0,l=bl.length;i<l;i++){bl[i].removeState(bg);}
;}
;if(bd){var bh=this.__GM[bd];bf.addState(bh);for(var i=0,l=bl.length;i<l;i++){bl[i].addState(bh);}
;}
;}
,getSelection:function(){var bn=this.__Bi.getSelection();var bo=[];for(var i=0;i<bn.length;i++){bo.push(bn[i].getUserData(s));}
;return bo;}
,setSelection:function(bp){var bq=[];for(var i=0;i<bp.length;i++){bq.push(bp[i].getChildControl(z));}
;this.__Bi.setSelection(bq);}
,resetSelection:function(){this.__Bi.resetSelection();}
,isSelected:function(br){var bs=br.getChildControl(z);return this.__Bi.isSelected(bs);}
,isSelectionEmpty:function(){return this.__Bi.isSelectionEmpty();}
,getSelectables:function(bt){var bu=this.__Bi.getSelectables(bt);var bv=[];for(var i=0;i<bu.length;i++){bv.push(bu[i].getUserData(s));}
;return bv;}
...
;}
,_onPageClose:function(e){var bD=e.getTarget();var bC=bD.getButton().getChildControl(f);bC.reset();this.remove(bD);}
},destruct:function(){this._disposeObjects(g);this.__GM=null;}
});}
)();
