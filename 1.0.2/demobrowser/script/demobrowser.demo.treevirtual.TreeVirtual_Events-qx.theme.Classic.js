...
var y=new qx.ui.treevirtual.TreeVirtual(m);
y.setColumnWidth(0,400);
y.setAlwaysShowOpenCloseSymbol(true);
this.getRoot().add(y,{edge:30});
var D=y.getDataModel();
var A=D.addBranch(null,c,true);
var C;
D.addBranch(A,b,true);
C=D.addBranch(A,o,true);
D.addLeaf(C,w);
D.addLeaf(C,u);
D.addBranch(A,g,true);
D.addBranch(A,x,true);
var B=D.addBranch(null,d,true);
C=D.addBranch(B,f,false);

for(var i=1;i<3000;i++){D.addLeaf(C,n+i);
}D.addBranch(B,h,false);
D.addBranch(B,x,false);
D.addBranch(B,v,false);
D.addBranch(B,p,false);
D.setData();
var z=1;
y.addListener(s,function(e){alert('treeOpenWithContent ');
var E=e.getData();
D.addLeaf(E.nodeId,z.toString());
z++;
});
y.addListener(l,function(e){alert('treeClose ');
});
y.addListener(r,function(e){alert('treeOpenWhileEmpty');
var a=e.getData();
y.nodeSetHideOpenClose(a,true);
});
y.addListener(j,function(e){var F=k;
var G=e.getData();
