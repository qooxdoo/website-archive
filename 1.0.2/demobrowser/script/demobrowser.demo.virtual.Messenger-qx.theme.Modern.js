...
qx.Class.define(C,{extend:qx.application.Standalone,members:{__Mw:null,messenger:null,__Mx:null,main:function(){arguments.callee.base.call(this);
var br=this.getRoot();
var bu=new qx.ui.window.Window(F).set({contentPadding:0,showClose:false,showMinimize:false});
var bt=new qx.ui.layout.VBox();
bt.setSeparator(S);
bu.setLayout(bt);
bu.moveTo(250,20);
bu.open();
this.messenger=new demobrowser.demo.virtual.messenger.Roster();
var bs=demobrowser.demo.virtual.messenger.BuddyModel.createBuddies(200);
this.messenger.setModel(bs);
bu.add(this.createToolbar());
bu.add(this.messenger,{flex:1});
br.add(this.createDetailsView(),{left:20,top:20});
},createToolbar:function(){var toolbar=new qx.ui.toolbar.ToolBar();
var bq=new qx.ui.toolbar.Part();
toolbar.add(bq);
var bo=new qx.ui.toolbar.Button(i,E).set({show:c});
bo.addListener(k,this.showAddContactWindow,this);
bq.add(bo);
var bp=new qx.ui.toolbar.Button(i,p).set({show:c});
this.messenger.bind(T,bp,n,{converter:function(bw){return !!bw;
}});
bp.addListener(k,function(){this.messenger.getModel().remove(this.messenger.getSelection().getItem(0));
},this);
bq.add(bp);
return bq;
},createDetailsView:function(){var bb=new qx.data.controller.Object();
this.messenger.bind(T,bb,J);
var X=new qx.ui.groupbox.GroupBox(w);
var bc=new qx.ui.layout.Grid(5,5);
bc.setColumnAlign(0,U,j);
X.setLayout(bc);
X.add(new qx.ui.basic.Label(m),{row:0,column:0});
var be=new qx.ui.form.TextField();
bb.addTarget(be,f,t,true);
X.add(be,{row:0,column:1});
X.add(new qx.ui.basic.Label(O),{row:1,column:0});
var Y=new qx.ui.form.TextField();
bb.addTarget(Y,f,l,true);
X.add(Y,{row:1,column:1});
X.add(new qx.ui.basic.Label(D),{row:2,column:0});
var bd=new qx.ui.form.SelectBox();
bd.add(new qx.ui.form.ListItem(d).set({model:d}));
bd.add(new qx.ui.form.ListItem(h).set({model:h}));
bd.add(new qx.ui.form.ListItem(b).set({model:b}));
bd.add(new qx.ui.form.ListItem(g).set({model:g}));
this.messenger.bind(a,bd,B,{converter:function(W){return [W];
}});
bd.bind(H,this.messenger,a,{converter:function(V){return V[0].getModel();
}});
X.add(bd,{row:2,column:1});
X.add(new qx.ui.basic.Label(A).set({alignY:z}),{row:3,column:0});
var ba=new qx.ui.basic.Image().set({alignX:q});
bb.addTarget(ba,r,x);
X.add(ba,{row:3,column:1});
return X;
},showAddContactWindow:function(){if(this.__Mw==null){var bf=new qx.ui.layout.Grid(5,10);
var bh=new qx.ui.window.Window(K,o);
bh.set({width:165,height:100,showMinimize:false,showClose:false,showMaximize:false});
bh.setLayout(bf);
var bk=new qx.ui.basic.Label(M);
var bl=new qx.ui.form.TextField();
var bi=new qx.ui.form.ComboBox(v);
bi.add(new qx.ui.form.ListItem(P));
bi.add(new qx.ui.form.ListItem(N));
var bg=new qx.ui.basic.Label(s);
var bn=new qx.ui.form.Button(G);
var bm=new qx.ui.form.Button(Q);
this.__Mx=bl;
bn.setAllowGrowX(false);
bm.setAllowGrowX(false);
bh.addListener(I,bl.focus,bl);
bm.addListener(k,bh.close,bh);
var bj=function(){var bv=new demobrowser.demo.virtual.messenger.BuddyModel();
bv.setName(bl.getValue());
bv.setGroup(bi.getValue());
this.messenger.getModel().push(bv);
this.messenger.getSelection().setItem(0,bv);
bh.close();
};
bn.addListener(k,bj,this);
bh.addListener(u,function(e){var bx=e.getKeyIdentifier();

if(bx==L){bj.call(this);
}else if(bx==R){bh.close();
}},this);
bh.add(bk,{row:0,column:0});
bh.add(bl,{row:0,column:1,colSpan:2});
bh.add(bg,{row:1,column:0});
bh.add(bi,{row:1,column:1,colSpan:2});
bh.add(bn,{row:2,column:1});
bh.add(bm,{row:2,column:2});
bf.setColumnAlign(0,y,j);
bf.setColumnAlign(1,U,j);
this.getRoot().add(bh,{left:20,top:20});
this.__Mw=bh;
}this.__Mx.setValue(i);
this.__Mw.center();
this.__Mw.open();
...
qx.Class.define(i,{extend:qx.ui.virtual.form.List,construct:function(){arguments.callee.base.call(this);
this.set({scrollbarX:e,scrollbarY:a,width:200,height:300,rowHeight:28,useWidgetCells:true,decorator:null});
var m=new demobrowser.demo.virtual.messenger.Controller(null,this);
this.bind(k,m,k);
this.setSelection(m.getSelection());
...
},properties:{model:{event:j,check:h,nullable:true},selection:{event:c}},members:{styleGroup:function(n){var o=b;
this.getPane().getRowConfig().setItemSize(n,15);
this.rowLayer.setColor(n,o);
},unstyleGroup:function(l){this.getPane().getRowConfig().setItemSize(l,null);
this.rowLayer.setColor(l,null);
}}});
})();
...
qx.Class.define(f,{extend:qx.ui.virtual.form.ListController,construct:function(o,p){arguments.callee.base.call(this,o,p);
this.__My=new demobrowser.demo.virtual.messenger.BuddyCell();
this.__Mz=new demobrowser.demo.virtual.messenger.GroupCell();
this.__Mz.addListener(h,this._onChangeOpenGroup,this);
this.__MA=[];
this.__MB={};
this.__MC=[];
this.__MD=new qx.util.DeferredCall(this._updateGrouping,this);
},members:{__MC:null,__MB:null,__MD:null,__MA:null,__My:null,__Mz:null,_getRowData:function(J){return this.__MC[J];
},_getModelRow:function(I){return this.__MC.indexOf(I);
},_getRowCount:function(){return this.__MC.length;
},_isGroupHeader:function(t){return this.__MC[t] instanceof demobrowser.demo.virtual.messenger.GroupModel;
},_getGroupModel:function(name,m){var n=this.__MB[name];

if(!n){var n=new demobrowser.demo.virtual.messenger.GroupModel();
n.setName(name);
this.__MB[name]=n;
}n.setRow(m);
return n;
},_onChangeOpenGroup:function(e){var D=this.__MC[e.getData().getUserData(c)];
D.toggleOpen();
this.__MD.schedule();
},getCellRenderer:function(E){if(this._isGroupHeader(E)){return this.__Mz;
}else{return this.__My;
}},isRowSelectable:function(s){return !this._isGroupHeader(s);
},_visualizeGrouping:function(F){var G=this.getTarget();

if(G){for(var i=0;i<this.__MA.length;i++){var H=this.__MA[i].getOldRow();

if(H!==null){G.unstyleGroup(H);
}}
for(var i=0;i<F.length;i++){var H=F[i].getRow();
G.styleGroup(H);
}}this.__MA=F;
},_updateGrouping:function(){var A=this.getModel();
this.__MC=[];
var B=[];
var x=[];

if(A&&A.length>0){B=qx.lang.Array.clone(A.toArray());
B.sort(function(a,b){var u=a.getGroup();
var v=b.getGroup();

if(u==v){return a.getName()<b.getName()?-1:1;
}return u>v?-1:1;
});
var z=B[0];
var w=this._getGroupModel(z.getGroup(),0);
this.__MC.push(w);
x.push(w);
var C=0;
...
if(y.getGroup()!==w.getName()){w.setItemCount(C);
C=0;
var w=this._getGroupModel(y.getGroup(),this.__MC.length);
this.__MC.push(w);
x.push(w);
}
if(w.isOpen()){this.__MC.push(y);
}else{this.getSelection().remove(y);
}C+=1;
}}w.setItemCount(C);
this._visualizeGrouping(x);
this._syncModelSelectionToView();
this._syncRowCount();
},_applyModel:function(k,l){arguments.callee.base.call(this,k,l);
this.__MD.schedule();
},_onChangeModel:function(e){arguments.callee.base.call(this,e);
this.__MD.schedule();
},_onChangeBubbleModel:function(e){var q=e.getData();

if(q.name){var r=q.name.toString().split(d)[1];

if(r==j||r==g){this.__MD.schedule();
}}arguments.callee.base.call(this);
}}});
})();
...
}},properties:{name:{init:c,event:g,check:a},row:{event:d,check:j,apply:b},open:{check:h,init:true,event:i},itemCount:{check:j,init:0,event:e}},members:{__ME:null,_applyRow:function(k,l){this.__ME=l;
},getOldRow:function(){return this.__ME;
}}});
})();
