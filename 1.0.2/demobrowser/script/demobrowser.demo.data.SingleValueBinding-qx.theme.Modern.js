...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.data.SingleValueBinding-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"d801c0818dd7"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var l="value",k="<b>Description</b><br/>",j="",i="changeValue",h="execute",g="Change me...",f="demobrowser.demo.data.SingleValueBinding",d="Reverse",c="Binding a number value (slider value) to a labels string (content).",b="child.names[last]",P="child.names[0]",O="First:",N="Bigger than 50!",M="of names.<br/>The reverse button reverses the current selected child ",L=" The default conversion will handle the type conversion.",K="Binding an data event, containing the change of the textfield to",J="Last:",I="Binding the textfield to a property with a validation. The validator",H=" in the object only accepts numbers. On a validation fail, a red ",G=" the label.",s="No number!",t="With validation",q="10",r="containing ['Bart', 'Lisa'] as names.",o=" above or below 50.",p="Bart",m="number",n="Toggle child",u="from a node containing ['Homer', 'Marge'] as names to a node ",v="Binding of node elements containing a node as child and an array ",y="With own conversion",x="names array.<br/>Toggle child will change the child of the root node ",A="with a self written converter to a string only saying it its ",z="Event binding",C="Array binding",B="Lower than 50!",w="Lisa",F="Binding a slider value to a labels text but conversion the number ",E="With default conversion",D=" border will surround the textfield.";
qx.Class.define(f,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
this.getRoot().add(new qx.ui.basic.Label(z),{top:10,left:10});
var be=new qx.ui.form.TextField(g);
be.setLiveUpdate(true);
this.getRoot().add(be,{top:30,left:10});
var X=new qx.ui.basic.Label(g);
this.getRoot().add(X,{top:32,left:120});
be.bind(i,X,l);
this.getRoot().add(new qx.ui.basic.Label(E),{top:60,left:10});
var bd=new qx.ui.form.Slider();
bd.setWidth(100);
this.getRoot().add(bd,{top:80,left:10});
var S=new qx.ui.basic.Label(j);
this.getRoot().add(S,{top:80,left:120});
bd.bind(l,S,l);
this.getRoot().add(new qx.ui.basic.Label(y),{top:110,left:10});
var U=new qx.ui.form.Slider();
U.setWidth(100);
this.getRoot().add(U,{top:130,left:10});
var bj=new qx.ui.basic.Label(j);
this.getRoot().add(bj,{top:130,left:120});
var Y={converter:function(a){if(a<50){return B;
}else{return N;
}}};
U.bind(l,bj,l,Y);
this.getRoot().add(new qx.ui.basic.Label(C),{top:180,left:10});
...
bi.setNames(new qx.data.Array(p,w));
bc.setChild(bf);
var bl=new qx.ui.form.Button(d);
this.getRoot().add(bl,{top:200,left:10});
bl.addListener(h,function(){bc.getChild().getNames().reverse();
},this);
var bh=new qx.ui.form.Button(n);
this.getRoot().add(bh,{top:200,left:80});
bh.addListener(h,function(){if(bc.getChild()==bf){bc.setChild(bi);
}else{bc.setChild(bf);
}},this);
this.getRoot().add(new qx.ui.basic.Label(O),{left:10,top:235});
this.getRoot().add(new qx.ui.basic.Label(J),{left:60,top:235});
...
bc.bind(P,bk,l);
bc.bind(b,bb,l);
this.getRoot().add(new qx.ui.basic.Label(t),{top:310,left:10});
var Q=new qx.ui.form.TextField(q);
Q.setLiveUpdate(true);
this.getRoot().add(Q,{top:330,left:10});
var ba=new qx.ui.basic.Label(j);
this.getRoot().add(ba,{top:332,left:120});
var Y={onUpdate:function(){ba.setValue(j);
Q.setValid(true);
},onSetFail:function(e){ba.setValue(s);
Q.setValid(false);
}};
Q.bind(i,bc,m,Y);
...
(function(){var s="qx.data.Array",r="demobrowser.demo.data.model.Node",q="String",p="changeNames",o="changeName",n="Validation Error: ",m="- ",l="",k="changeChild",j="changeColor",c="Marge",h="AFFE",f="is no number (parseFloat says so!).",b="\n",a="Homer",e="__Lg",d="changeName2",g="changeChildren";
...
},properties:{children:{check:s,event:g},child:{check:r,event:k,nullable:true},names:{check:s,event:p,init:new qx.data.Array(a,c)},name:{check:q,init:h,event:o},name2:{check:q,event:d},number:{init:10,validate:e},color:{event:j,nullable:true}},members:{__Lg:function(v){if(!isNaN(parseFloat(v))){if(/^\d+$/.test(v)){return;
}}throw new qx.core.ValidationError(n+v+f);
},toString:function(t){if(t==undefined){t=0;
}var u=l;

for(var i=0;i<t;i++){u+=m;
}u+=this.getName();

for(var i=0;i<this.getChildren().length;i++){u+=b+this.getChildren().getItem(i).toString(t+1);
}return u;
}}});
})();
