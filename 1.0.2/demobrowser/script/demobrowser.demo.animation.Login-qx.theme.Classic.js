...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.animation.Login-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"fc17deab9b22"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var p="top",o="right",n="resize",m="Name",l="__KG",k="Password",j="__KI",h="appear",g="__KH",f="demobrowser.demo.animation.Login",b="50%",d="execute",c="30%";
qx.Class.define(f,{extend:qx.application.Standalone,members:{__KG:null,__KH:null,__KI:null,checkInput:function(){this.__KG.start();
},__KJ:function(){this.__KG=new qx.fx.effect.combination.Shake(this.__KH.getContainerElement().getDomElement());
},main:function(){arguments.callee.base.call(this);
var r=new qx.ui.layout.Grid(9,5);
r.setColumnAlign(0,o,p);
r.setColumnAlign(2,o,p);
this.__KH=new qx.ui.groupbox.GroupBox().set({contentPadding:[16,16,16,16]});
this.__KH.setLayout(r);
this.__KH.addListener(n,function(e){var a=this.__KH.getBounds();
this.__KH.set({marginTop:Math.round(-a.height/2),marginLeft:Math.round(-a.width/2)});
},this);
this.getRoot().add(this.__KH,{left:b,top:c});
var q=[m,k];

for(var i=0;i<q.length;i++){this.__KH.add(new qx.ui.basic.Label(q[i]).set({allowShrinkX:false,paddingTop:3}),{row:i,column:0});
}var s=new qx.ui.form.TextField();
var t=new qx.ui.form.PasswordField();
this.__KH.add(s.set({allowShrinkX:false,paddingTop:3}),{row:0,column:1});
this.__KH.add(t.set({allowShrinkX:false,paddingTop:3}),{row:1,column:1});
var u=this.__KI=new qx.ui.form.Button("Login");
u.setAllowStretchX(false);
this.__KH.add(u,{row:3,column:1});
u.addListener(d,this.checkInput,this);
this.__KH.addListener(h,this.__KJ,this);
}},destruct:function(){this._disposeObjects(g,j,l);
}});
})();
...
(function(){var e="horizontal",d="vertical",c="__Fs",b="qx.fx.effect.combination.Shake",a="Number";
qx.Class.define(b,{extend:qx.fx.Base,construct:function(k){arguments.callee.base.call(this,k);
this.__Fs=[new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k)];
},properties:{direction:{init:e,check:[e,d]},duration:{init:0.5,refine:true},distance:{init:20,check:a}},members:{__Fs:null,start:function(){if(!arguments.callee.base.call(this)){return;
}var f=parseFloat(this.getDistance());
var g=parseFloat(this.getDuration())/10.0;

if(this.getDirection()==e){this.__Fs[0].set({x:f,y:0,duration:g});
this.__Fs[1].set({x:-f*2,y:0,duration:g*2});
this.__Fs[2].set({x:f*2,y:0,duration:g*2});
this.__Fs[3].set({x:-f*2,y:0,duration:g*2});
this.__Fs[4].set({x:f*2,y:0,duration:g*2});
this.__Fs[5].set({x:-f,y:0,duration:g*2});
}else if(this.getDirection()==d){this.__Fs[0].set({y:f,x:0,duration:g});
this.__Fs[1].set({y:-f*2,x:0,duration:g*2});
this.__Fs[2].set({y:f*2,x:0,duration:g*2});
this.__Fs[3].set({y:-f*2,x:0,duration:g*2});
this.__Fs[4].set({y:f*2,x:0,duration:g*2});
this.__Fs[5].set({y:-f,x:0,duration:g*2});
}var j=this.__Fs;

for(var i=0,h=this.__Fs.length;i<h;i++){this.__Fs[i].id=i;

if(i<5){this.__Fs[i].afterFinishInternal=function(){j[this.id+1].start();
};
}}this.__Fs[0].start();
