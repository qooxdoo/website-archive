...
}qx.bom.Element.addListener(G,k,this.__Lv,this);
}var E=document.getElementById(a);
E.checked=false;
qx.bom.Element.addListener(E,k,function(e){var I=e.getTarget().checked;

if(I){qx.bom.Element.capture(D);
}else{qx.bom.Element.releaseCapture(D);
}},this);
qx.bom.Element.addListener(D,q,function(e){E.checked=false;
},this);
},__Lv:function(e){var L=e.getTarget().id.split(x)[1];
...
}},logMouseEvent:function(H){H.preventDefault();
this._log([H.getTarget().id,H.getType(),H.getButton(),H.getDocumentLeft(),H.getDocumentTop(),H.getScreenLeft(),H.getScreenTop(),H.getViewportLeft(),H.getViewportTop(),H.getRelatedTarget()?H.getRelatedTarget().id:y]);
}}});
})();
