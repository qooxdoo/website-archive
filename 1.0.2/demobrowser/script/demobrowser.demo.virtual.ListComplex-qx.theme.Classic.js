...
qx.Class.define(i,{extend:qx.ui.virtual.layer.WidgetCell,construct:function(){arguments.callee.base.call(this,this);
this._pool={atom:[],checkbox:[]};
this.__Mt=[];
},members:{_pool:null,__Mt:null,getCellData:function(w,x){if(!this.__Mt[w]){this.__Mt[w]=[];
}
if(!this.__Mt[w][x]){this.__Mt[w][x]={label:this.__Mu(),icon:this.__Mv()};
}return this.__Mt[w][x];
},getCellWidget:function(y,z){var A;

if(z%2==0){A=this._pool.atom.pop();

if(!A){A=new qx.ui.basic.Atom();
A.addListener(c,function(){var B=this.__Mv();
A.setIcon(B);
this.__Mt[y][z].icon=B;
},this);
}A.set(this.getCellData(y,z));
}else{A=this._pool.checkbox.pop();

if(!A){A=new qx.ui.form.CheckBox();
A.addListener(f,function(){this.setLabel(this.getLabel()==p?A.getUserData(q)+r+A.getUserData(o):p);
},A);
}A.set({value:y%2==0,label:y+r+z});
}A.setUserData(q,y);
A.setUserData(o,z);
return A;
},poolCellWidget:function(C){if(C.classname==a){this._pool.atom.push(C);
}else{this._pool.checkbox.push(C);
}},__Mu:function(){var name=m;

for(var j=0;j<10;j++){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);
}return name;
},__Mv:function(){var s=n;
var u=k;
var v=[l,g,h,b,d];
var t=Math.floor(Math.random()*4);
return (s+16+e+u+v[t]);
}},destruct:function(){this._pool=this.__Mt=null;
}});
})();
