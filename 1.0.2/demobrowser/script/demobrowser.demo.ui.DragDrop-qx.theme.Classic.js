...
(function(){var o="drop",n="value",m="items",k="move",j="multi",h="dragover",g="icon/16/places/folder.png",f="dragstart",d="Item ",c="copy",C="drag",B="TextArea Target",A="__Mb",z="solid",y="Reorderable",x="qx.ui.form.ListItem",w="droprequest",v="#33508D",u="Source",t="demobrowser.demo.ui.DragDrop",r="Simple Target",s="Enable drag",p="\n",q="dragend";
qx.Class.define(t,{extend:qx.application.Standalone,members:{__Mb:null,__Mc:null,main:function(){arguments.callee.base.call(this);
var O=this.getRoot();
var I=new qx.ui.container.Scroll();
var D=new qx.ui.layout.Basic();
var G=new qx.ui.container.Composite(D).set({allowStretchY:false,allowStretchX:false});
I.add(G);
O.add(I,{edge:0});
var N=new qx.ui.basic.Label(u);
G.add(N,{left:20,top:20});
var L=new qx.ui.form.List;
L.setDraggable(true);
L.setSelectionMode(j);
G.add(L,{left:20,top:40});

for(var i=0;i<20;i++){L.add(new qx.ui.form.ListItem(d+i,g));
}var P=new qx.ui.form.CheckBox(s);
P.setValue(true);
G.add(P,{left:20,top:260});
L.addListener(f,function(e){if(!P.isValue()){e.preventDefault();
}e.addType(n);
e.addType(m);
e.addAction(c);
e.addAction(k);
});
L.addListener(w,function(e){this.debug("Related of droprequest: "+e.getRelatedTarget());
var S=e.getCurrentAction();
var U=e.getCurrentType();
var R;

switch(U){case m:R=this.getSelection();

if(S==c){var T=[];

for(var i=0,l=R.length;i<l;i++){T[i]=R[i].clone();
}R=T;
}break;
case n:R=this.getSelection()[0].getLabel();
break;
}if(S==k){var Q=this.getSelection();

for(var i=0,l=Q.length;i<l;i++){this.remove(Q[i]);
}}e.addData(U,R);
});
var J=new qx.ui.basic.Label(r);
G.add(J,{left:140,top:20});
var H=new qx.ui.form.List;
H.setDroppable(true);
H.setSelectionMode(j);
G.add(H,{left:140,top:40});
H.addListener(o,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
var X=e.getData(m);

for(var i=0,l=X.length;i<l;i++){this.add(X[i]);
}});
H.addListener(h,function(e){if(!e.supportsType(m)){e.preventDefault();
}});
var J=new qx.ui.basic.Label(B);
G.add(J,{left:260,top:20});
var M=new qx.ui.form.TextArea;
M.setDroppable(true);
M.setHeight(100);
G.add(M,{left:260,top:40});
M.addListener(o,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
this.setValue(e.getData(n).replace(/,/g,p));
});
M.addListener(h,function(e){if(!e.supportsType(n)){e.preventDefault();
}});
var K=new qx.ui.basic.Label(y);
G.add(K,{left:500,top:20});
var E=this.__Mb=new qx.ui.form.List;
E.setDraggable(true);
E.setDroppable(true);
E.setSelectionMode(j);
G.add(E,{left:500,top:40});

for(var i=0;i<20;i++){E.add(new qx.ui.form.ListItem(d+i,g));
}var F=new qx.ui.core.Widget;
F.setDecorator(new qx.ui.decoration.Single().set({top:[1,z,v]}));
F.setHeight(0);
F.setOpacity(0.5);
F.setZIndex(100);
...
F.setDroppable(true);
this.getRoot().add(F);
E.addListener(f,function(e){e.addAction(k);
});
E.addListener(q,function(e){F.setDomPosition(-1000,-1000);
});
E.addListener(C,function(e){var b=e.getOriginalTarget();
if(b instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__Mc=b;
}
if(!qx.ui.core.Widget.contains(this,b)&&b!=F){return;
}var a=b.getContainerLocation();
F.setWidth(b.getBounds().width);
F.setDomPosition(a.left,a.top);
});
E.addListener(h,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
E.addListener(o,function(e){this.__Md(e.getOriginalTarget());
},this);
F.addListener(o,function(e){this.__Md(this.__Mc);
},this);
},__Md:function(V){if(V.classname!=x){return;
}var W=this.__Mb.getSortedSelection();

for(var i=0,l=W.length;i<l;i++){this.__Mb.addBefore(W[i],V);
this.__Mb.addToSelection(W[i]);
