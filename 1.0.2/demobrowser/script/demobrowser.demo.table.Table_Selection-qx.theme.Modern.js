...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.table.Table_Selection","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.table.Table_Selection-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"3ec9a0d92f57"},
  boot : "boot",
  bootIsInline : true,
...
K.setColumns([l,b,h,g]);
K.setData(L);
K.setColumnEditable(1,true);
K.setColumnEditable(2,true);
K.setColumnSortable(3,false);
var M=new qx.ui.table.Table(K);
M.set({width:600,height:400});
M.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var J=M.getTableColumnModel();
J.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
J.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(a,h));
return M;
},createControls:function(){var B=new qx.ui.toolbar.ToolBar();
var D,F;
F=new qx.ui.toolbar.Part();
B.add(F);
var A=new qx.ui.form.SelectBox().set({allowGrowY:false,alignY:x});
var I=qx.ui.table.selection.Model;
var C={"Single selection":I.SINGLE_SELECTION,"Single interval":I.SINGLE_INTERVAL_SELECTION,"Multiple intervals":I.MULTIPLE_INTERVAL_SELECTION,"Multiple intervals (toggle)":I.MULTIPLE_INTERVAL_SELECTION_TOGGLE,"No selection":I.NO_SELECTION};

for(var H in C){var E=new qx.ui.form.ListItem(H);
E.setUserData(j,C[H]+i);
A.add(E);
}A.addListener(y,function(e){this._table.getSelectionModel().setSelectionMode(parseInt(e.getData()[0].getUserData(j)));
},this);
F.add(A);
F=new qx.ui.toolbar.Part();
B.add(F);
D=new qx.ui.toolbar.Button(s,o);
D.addListener(k,function(Q){var R=this.createRandomRows(10);
this._tableModel.addRows(R);
this.info("5 rows added");
},this);
F.add(D);
D=new qx.ui.toolbar.Button(w,r);
D.addListener(k,function(N){var O=this._tableModel.getRowCount();
...
F=new qx.ui.toolbar.Part();
B.add(F);
D=new qx.ui.toolbar.Button(v);
D.addListener(k,function(U){this._table.resetSelection();
},this);
F.add(D);
D=new qx.ui.toolbar.Button(z);
D.addListener(k,function(T){this._table.resetCellFocus();
},this);
F.add(D);
var G=new qx.ui.toolbar.CheckBox(p);
G.addListener(d,function(S){this._table.setShowCellFocusIndicator(!G.isValue());
},this);
F.add(G);
D=new qx.ui.toolbar.Button(n,c);
D.addListener(k,function(V){var W=[];
this._table.getSelectionModel().iterateSelection(function(P){W.push(P+i);
});
this.showDialog(m+W.join(q));
},this);
F.add(D);
return B;
