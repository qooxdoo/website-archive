...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.widget.Button-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"5c9339f79f16"},
  boot : "boot",
  bootIsInline : true,
...
this.addNormalButtons(B);
this.addToggleButton(B);
this.addRepeatButton(B);
this.addHoverButton(B);
},addNormalButtons:function(a){var b=new qx.ui.form.Button(o,i);
a.add(b);
var c=new qx.ui.form.Button(l,h);
c.setEnabled(false);
a.add(c);
},addToggleButton:function(C){var D=new qx.ui.form.ToggleButton(d,j);
D.focus();
C.add(D);
D.addListener(f,function(e){this.debug("Checked: "+e.getData());
},this);
},addRepeatButton:function(t){var u=new qx.ui.form.RepeatButton(null,m);
t.add(u);
var v=new qx.ui.basic.Label(s);
v.setDecorator(q);
v.setPadding(2,4);
v.setBackgroundColor(r);
t.add(v);
u.addListener(p,function(){var w=parseInt(v.getValue())+1;
v.setValue(w.toString());
});
},addHoverButton:function(x){var y=new qx.ui.form.HoverButton(g).set({padding:10,backgroundColor:n});
x.add(y);
var z=new qx.ui.basic.Label(s);
z.setDecorator(q);
z.setPadding(2,4);
z.setBackgroundColor(r);
x.add(z);
y.addListener(p,function(){var E=parseInt(z.getValue())+1;
z.setValue(E.toString());
});
}}});
})();
