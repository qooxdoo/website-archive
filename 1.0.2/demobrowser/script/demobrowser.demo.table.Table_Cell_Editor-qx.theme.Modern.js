...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.table.Table_Cell_Editor-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"ddc3ba2ef291"},
  boot : "boot",
  bootIsInline : true,
...
},createTable:function(){var bs=[[K,l,y,{'required':true}],[e,L,T,{'type':e,'required':true}],[M,B,U,{'options':[V,O,H],'editable':true}],[R,w,b,{'options':[[o,null,b],[E,null,r],[x,null,F]]}],[f,A,z,{'type':f}],[m,P,N,{'regExp':X,'failMsg':ba}],[J,u,I,{'validationFunc':function(bN){if(bN.search(/^http:/)!=-1){window.open(bN);
}return bN;
}}],[W,n,true,{'type':k}]];
var br=function(bt){var bz=bt.table;
var bw=bz.getTableModel();
var bx=bw.getRowData(bt.row);
var bA=bx[3];
var by;

for(var bv in bA){switch(bv){case g:switch(bA[c]){case k:return new qx.ui.table.cellrenderer.Boolean;
case h:return new qx.ui.table.cellrenderer.Password;
}break;
case i:var by=new qx.ui.table.cellrenderer.Replace;
var bu={};
bA[d].forEach(function(bb){if(bb instanceof Array){bu[bb[0]]=bb[2];
}});
by.setReplaceMap(bu);
...
var bo=function(bc){var bh=bc.table;
var bf=bh.getTableModel();
var bg=bf.getRowData(bc.row);
var bj=bg[3];
var be=new qx.ui.table.celleditor.TextField;
var bi=null;

for(var bd in bj){switch(bd){case i:if(bj.editable){be=new qx.ui.table.celleditor.ComboBox();
}else{be=new qx.ui.table.celleditor.SelectBox();
}be.setListData(bj[d]);
break;
case G:break;
case g:switch(bj[c]){case h:be=new qx.ui.table.celleditor.PasswordField;
break;
case k:be=new qx.ui.table.celleditor.CheckBox;
break;
case t:be.setValidationFunction(function(bB,bC){var bD=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{2}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/;
...
case s:be.setValidationFunction(function(bK,bL){var bM=new RegExp(bj[Y]);

if(bM.test(bK)){return bK;
}alert(bj['failMsg']);
return bL;
});
break;
case Q:be.setValidationFunction(bj[C]);
break;
case p:bi=function(bE,bF){if(!bE){alert("You need to supply a value here");
return bF;
}return bE;
};
break;
...
bl.setColumns([j,j,q]);
var bk={tableColumnModel:function(a){return new qx.ui.table.columnmodel.Resize(a);
}};
...
bm.getTableModel().addListener(v,function(event){if(!(event instanceof qx.event.type.Data)){return;
}var bJ=event.getData();
var bG=this.getTableModel();
var bI=bG.getValue(0,bJ.firstRow);
var bH=bG.getValue(bJ.firstColumn,bJ.firstRow);
this.info("User edited property '"+bI+"' and entered value '"+bH+"'.");
},bm);
return bm;
}}});
})();
