...
V.addListener(H,function(e){var R=e.getData();
this.setValue(V.getHierarchy(R[0].nodeId).join(B));
S.setEnabled(true);
},o);
var S=new qx.ui.form.Button(p);
S.set({enabled:false});
W.add(S,{top:50,left:0});
S.addListener(a,function(e){var P=V.getSelectedNodes();

for(var i=0;i<P.length;i++){bb.prune(P[i].nodeId,true);
bb.setData();
}});
...
V.addListener(q,function(e){if(this.getValue()){var Q=e.getData();
