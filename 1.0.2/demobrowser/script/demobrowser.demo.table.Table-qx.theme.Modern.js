...
N.setColumns([r,g,n,m]);
N.setData(O);
N.setColumnEditable(1,true);
N.setColumnEditable(2,true);
N.setColumnSortable(3,false);
var P=new qx.ui.table.Table(N);
P.set({width:600,height:400,decorator:null});
P.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
var M=P.getTableColumnModel();
M.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
M.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(f,n));
return P;
...
b.addListener(p,function(K){var L=this.createRandomRows(1);
...
b.addListener(p,function(G){var H=this._tableModel.getRowCount();
this._tableModel.removeRows(H-5,5);
this.info("5 rows removed");
},this);
c.add(b);
b=new qx.ui.toolbar.Button(t,j);
b.addListener(p,function(Q){var R=[];
a.getSelectionModel().iterateSelection(function(I){R.push(I+D);
});
this.showDialog(u+R.join(x));
},this);
c.add(b);
c=new qx.ui.toolbar.Part();
d.add(c);
...
e.addListener(o,function(J){a.setKeepFirstVisibleRowComplete(this.getValue());
},e);
c.add(e);
e=new qx.ui.toolbar.CheckBox(k);
e.set({value:a.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(q)});
e.addListener(o,function(S){if(S.getData()){var U=function(ba,bb){var bc=ba[arguments.callee.columnIndex];
...
var T=function(V,W){var X=V[arguments.callee.columnIndex];
...
a.getTableModel().setSortMethods(0,{ascending:U,descending:T});
