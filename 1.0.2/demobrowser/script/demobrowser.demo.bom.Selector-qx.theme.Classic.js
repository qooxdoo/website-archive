...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.bom.Selector-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"8ec3d69b84a5"},
  boot : "boot",
  bootIsInline : true,
...
var bu=qx.log.Logger;
var bq=qx.bom.Collection;
bq.query(bi).setStyle(e,S);
bq.query(M).setStyle(bj,bh).setStyle(y,bb);
bq.query(U).setStyles({"textAlign":c,"textDecoration":s,"color":p});
bu.debug("href of first link: "+bq.query("a").getAttribute("href"));
bq.query(a).setAttribute(b,J);
bq.query(bg).addClass(Y);
bq.query(bi).addListener(L,function(){alert(this.innerHTML);
});
(new qx.bom.Collection).add(g).setStyle(e,v).add(j).setStyle(bd,A).end().setStyle(C,c);
bu.debug("Found "+qx.bom.Collection.query("ul,ol").children().length+" children of ul/ol elements using children().");
bu.debug("Found "+qx.bom.Collection.query("ul,ol").children(":first-child").length+" first children of ul/ol elements using children(selector)");
bu.debug("Number of ul/ol elements which are closest to a li element: "+qx.bom.Collection.query("li").closest("ul,ol").length);
bu.debug("Number of child nodes of the body: "+new qx.bom.Collection(document.body).contents().length);
var bp=qx.bom.Collection.query(k).find(j);
bu.debug("Found "+bp.length+" li elements in uls using find()");
var bp=new qx.bom.Collection(document.body).find(bg);
bu.debug("Found "+bp.length+" headers using find()");
var bp=qx.bom.Collection.query(k).next()[0].tagName;
bu.debug("Tag following the UL element: "+bp);
var bp=qx.bom.Collection.query(k).nextAll().length;
bu.debug("Number of elements following the UL: "+bp);
var bp=qx.bom.Collection.query(j).parent().length;
bu.debug("Number of parent elements of all li elems: "+bp);
var bp=qx.bom.Collection.query(Q).parents().length;
bu.debug("Number of all parent elements of the submit button: "+bp);
var bp=qx.bom.Collection.query(k).prev()[0].tagName;
bu.debug("Tag before the UL element: "+bp);
var bp=qx.bom.Collection.query(k).prevAll().length;
bu.debug("Number of elements preceding the UL: "+bp);
var bp=qx.bom.Collection.query(O).siblings().length;
bu.debug("List Item 3 has "+bp+" siblings");
var bp=qx.bom.Collection.query(i).getOffsetParent()[0].tagName;
bu.debug("Offset parent of select box: "+bp);
var bp=qx.bom.Collection.query(i).getOffset();
bu.debug("Offset of select box: ");
bu.debug(bp);
bq.query(K).eq(1).setStyle(bj,bh);
var bp=qx.bom.Collection.query(h).filter(ba);
bu.debug("Found "+bp.length+" submit buttons using filter(selector)");
var bp=qx.bom.Collection.query(h).filter(function(bm,bn,bo){return bm.type===u;
});
bu.debug("Found "+bp.length+" submit button using filter(function)");
var bp=qx.bom.Collection.query(g).hasClass(R);
bu.debug("Any H2 with form has a class 'form': "+bp);
var bp=qx.bom.Collection.query(k).find(j).is(bk);
bu.debug("Found li element with text 'List Item 1': "+bp);
var bp=qx.bom.Collection.query(k).find(j).not(bk);
bu.debug("Number of found li elements without text 'List Item 1': "+bp.length);
var bp=qx.bom.Collection.query(k).find(j).not(r);
bu.debug("Number of found li elements in the middle: "+bp.length);
...
var bw=document.createElement(i);
var bv=document.createElement(T);
var br=new qx.bom.Collection(bw).setAttribute(F,d).setAttribute(z,b);
new qx.bom.Collection(bv).setAttribute(l,d).setAttribute(be,H);
qx.bom.Collection.query(i).after(bv,bw);
br.append(o,q,t,n);
$(x).insertAfter(g);
qx.bom.Collection.id(bl).addListener(h,function(){bu.debug("Typing in e-mail field...");
});
var bs=qx.bom.Collection.id(bl);
var bt=bs.add(W).reverse();
bs.after(bt.clone(true));
