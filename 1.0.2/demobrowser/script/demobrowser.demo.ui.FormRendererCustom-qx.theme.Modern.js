...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.ui.FormRendererCustom","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.ui.FormRendererCustom-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"2c008b32cb26"},
  boot : "boot",
  bootIsInline : true,
...
z.add(D,c);
z.addGroupHeader(h);
z.add(new qx.ui.form.TextField(),e,qx.util.Validate.email());
z.add(new qx.ui.form.TextField(),d);
z.addGroupHeader(f);
z.add(new qx.ui.form.TextField(),g);
z.add(new qx.ui.form.TextArea(),k);
var B=new qx.ui.form.Button(j);
B.addListener(p,function(){if(z.validate()){alert("send...");
}},this);
z.addButton(B);
var A=new qx.ui.form.Button(a);
A.addListener(p,function(){z.reset();
},this);
z.addButton(A);
var C=new demobrowser.demo.ui.CustomRenderer(z);
this.getRoot().add(C,{left:10,top:10});
}}});
qx.Class.define(l,{extend:qx.ui.form.renderer.Single,members:{__Me:0,__Mf:0,addItems:function(q,r,s){var v=0;
if(s!=null){this._add(this._createHeader(s),{row:v,column:this.__Me,colSpan:2});
v++;
}for(var i=0;i<q.length;i++){var u=this._createLabel(r[i],q[i]);
this._add(u,{row:v,column:this.__Me});
var t=q[i];
u.setBuddy(t);

if(t instanceof qx.ui.form.RadioGroup){t=this._createWidgetForRadioGroup(t);
}this._add(t,{row:v,column:this.__Me+1});
v++;
}this.__Me+=2;
this.__Mf<v?this.__Mf=v:null;
...
this._add(this._buttonRow,{row:this.__Mf,column:0,colSpan:this.__Me});
