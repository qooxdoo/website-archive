...
var X=new qx.ui.progressive.renderer.table.Widths(1);
X.setWidth(0,l);
var R=new qx.ui.progressive.headfoot.Progress(X);
var V=new qx.ui.progressive.structure.Default(null,R);
var Q=new qx.ui.progressive.Progressive(V);
Q.setFlushWidgetQueueAfterBatch(true);
var U=function(D){var E={renderer:k,data:D};
return E;
};
var bc=new qx.ui.progressive.model.Default();
var Y=new qx.ui.progressive.renderer.FunctionCaller();
Q.addRenderer(k,Y);
var W=g;
var ba=new qx.ui.basic.Image(W,l,l);
Q.add(ba);
Q.set({height:100,width:272,zIndex:99999,backgroundColor:z,opacity:0.86,batchSize:10});
this.getRoot().add(Q,{top:-1000,left:-1000});
var bb=[{text:h,background:r,color:m},{text:x,background:f,color:m},{text:v,background:y,color:m},{text:p,background:s,color:m},{text:w,background:d,color:i}];
this.context={document:this.getRoot(),pages:bb};
Q.addListener(u,function(e){var L=e.getData().state;
var H=e.getData().initial;
var K=this.getRoot().getBounds();
var I=e.getData().state.getProgressive();
var J=I.getBounds();
var M=Math.floor((K.width-J.width)/2);
var top=Math.floor((K.height-J.height)/2);
I.setLayoutProperties({left:M<0?0:M,top:top<0?0:top});
L.getUserData().context=this.context;
L.getUserData().initialNum=H;
},this);
Q.addListener(q,function(e){qx.event.Timer.once(function(){this.getLayoutParent().remove(this);
this.dispose();
},this,0);
});
bc.addElement(U(function(O){var P=O.context;
...
for(var S=0;S<bb.length;S++){bc.addElement(U(function(bf){var bh=bf.context;
...
for(var bd=0;bd<20;bd++){bc.addElement(U(function(a){var b=a.context;
...
for(var T=0;T<10;T++){bc.addElement(U(function(A){var C=A.context;
...
bc.addElement(U(function(N){++N.context.col;
}));
}bc.addElement(U(function(G){++G.context.row;
}));
}bc.addElement(U(function(be){++be.context.page;
}));
}bc.addElement(U(function(F){F.context.tabview.setSelection([F.context.firstPage]);
}));
Q.setDataModel(bc);
Q.render();
}}});
