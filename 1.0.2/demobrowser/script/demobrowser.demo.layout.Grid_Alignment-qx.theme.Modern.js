...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.layout.Grid_Alignment-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"dd786d22c3b4"},
  boot : "boot",
  bootIsInline : true,
...
var A=new qx.ui.container.Composite(new qx.ui.layout.Grid(20)).set({padding:20});
scroll.add(A);
A.add(this.getCellAlignGrid(),{row:0,column:0});
A.add(this.getRowColumnAlignGrid(),{row:0,column:1});
A.add(this.getColumnAlignGrid(),{row:0,column:2});
A.add(this.getRowAlignGrid(),{row:1,column:0});
},getNewWidget:function(){var p=new qx.ui.core.Widget().set({decorator:m,backgroundColor:a,allowGrowX:false,allowGrowY:false,height:Math.round((Math.random()*30)+30),width:Math.round((Math.random()*30)+30)});
return p;
...
},getRowAlignGrid:function(){var q=new qx.ui.container.Composite().set({decorator:m,backgroundColor:l,allowGrowX:false,allowGrowY:false});
...
for(var x=0;x<6;x++){for(var y=0;y<3;y++){q.add(this.getNewWidget(),{row:y,column:x});
}}return q;
...
t.set({backgroundColor:d,width:20,height:20});
return s;
},getRowColumnAlignGrid:function(){var v=new qx.ui.container.Composite().set({decorator:m,backgroundColor:l,allowGrowX:false,allowGrowY:false});
var z=new qx.ui.layout.Grid();
z.setSpacing(3);
v.setLayout(z);

for(var x=0;x<5;x++){for(var y=0;y<5;y++){v.add(this.getNewWidget(),{row:y,column:x});
}}z.setColumnAlign(2,h,j);
z.setRowAlign(2,g,k);

for(var i=0;i<5;i++){z.getCellWidget(2,i).setBackgroundColor(c);
z.getCellWidget(i,2).setBackgroundColor(c);
}var w=z.getCellWidget(2,2);
w.set({backgroundColor:d,width:20,height:20});
return v;
}}});
})();
