...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.bom.Location-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"b550a11fb0a2"},
  boot : "boot",
  bootIsInline : true,
...
var w=document.getElementById(h).getElementsByTagName(c);

for(var i=0,l=w.length;i<l;i++){w[i].onclick=demobrowser.demo.bom.Location.jump;
}}},statics:{num:function(t,u){return parseInt(qx.bom.element.Style.get(t,u,qx.bom.element.Style.COMPUTED_MODE,false),10);
},jump:function(event){if(!event){event=window.event;
}event.returnValue=false;

if(event.preventDefault){event.preventDefault();
}function o(x,y){return parseInt(qx.bom.element.Style.get(x,y,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var q=this.href;
...
},randomize:function(){var v=document.body.getElementsByTagName(a);

for(var i=0,l=v.length;i<l;i++){if(v[i].id&&v[i].id!=m){v[i].style.margin=Math.round(Math.random()*10)+n;
v[i].style.padding=Math.round(Math.random()*10)+n;
v[i].style.borderWidth=Math.round(Math.random()*10)+n;
}}}}});
})();
