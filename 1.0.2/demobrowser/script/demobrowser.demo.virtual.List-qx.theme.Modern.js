...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.virtual.List-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"34d326c79768"},
  boot : "boot",
  bootIsInline : true,
...
qx.Class.define(bw,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var bP=new qx.ui.basic.Label(u);
bP.setFont(h);
this.getRoot().add(bP,{left:20,top:20});
var bQ=new demobrowser.demo.virtual.list.List();
bQ.setScrollbarX(bl);
bQ.set({height:280,width:150,selectionMode:m});
var bM;

for(var i=1;i<=2500;i++){bM=new demobrowser.demo.virtual.list.ListItem(g+i,K+((i%4)?bo:O)+bu);

if(i%4==0){bM.setHeight(56);
}bQ.add(bM);
if(i==20){bQ.setSelected(bM);
}}bQ.addListener(l,function(e){this.debug("Value: "+e.getData());
});
this.getRoot().add(bQ,{left:20,top:40});
var bI=new qx.ui.form.RadioButton(L);
var bF=new qx.ui.form.RadioButton(bh);
var bG=new qx.ui.form.RadioButton(bv);
var bE=new qx.ui.form.RadioButton(H);
bI.setUserData(o,k);
bF.setUserData(o,m);
bG.setUserData(o,j);
bE.setUserData(o,n);
bF.setValue(true);
this.getRoot().add(bI,{left:180,top:40});
this.getRoot().add(bF,{left:180,top:60});
this.getRoot().add(bG,{left:180,top:80});
this.getRoot().add(bE,{left:180,top:100});
var bH=new qx.ui.form.RadioGroup(bI,bF,bG,bE);
var bL=new qx.ui.form.CheckBox(bq);
var bO=new qx.ui.form.CheckBox(bj).set({enabled:false});
this.getRoot().add(bL,{left:180,top:220});
this.getRoot().add(bO,{left:180,top:240});
bL.addListener(l,function(e){if(e.getData()){var bD=bQ.getSelectionMode();

if(bD==k||bD==n){this.debug("Drag selection is only available for the modes multi or additive");
}}bQ.setDragSelection(e.getData());
});
bO.addListener(l,function(e){if(e.getData()){var bC=bQ.getSelectionMode();

if(bC==m||bC==j){this.debug("Quick selection is only available for the modes multi or additive");
}}bQ.setQuickSelection(e.getData());
});
bH.addListener(T,function(e){var bT=e.getData()[0].getUserData(o);
bQ.setSelectionMode(bT);

if(bT==k||bT==n){bL.setEnabled(false);
bO.setEnabled(true);
}else if(bT==m||bT==q){bL.setEnabled(true);
bO.setEnabled(false);
}});
var bS=new qx.ui.basic.Label(N);
bS.setFont(h);
this.getRoot().add(bS,{left:330,top:20});
var bN=new demobrowser.demo.virtual.list.List();
bN.set({height:280,width:150,selectionMode:n});
var bM;

for(var i=1;i<=25;i++){bM=new demobrowser.demo.virtual.list.ListItem(g+i,s);
bN.add(bM);
if(i==16){bN.setSelected(bM);
}}this.getRoot().add(bN,{left:330,top:40});
var bP=new qx.ui.basic.Label(B);
bP.setFont(h);
this.getRoot().add(bP,{left:520,top:20});
var bR=new demobrowser.demo.virtual.list.List;
var bJ;
bR.set({width:150,selectionMode:j});
var bK=[v,U,X,S,x,by,bi,ba,M,f,c,A,b,V,bm,bn,w,Q,a,G,d,bg,I,bt,bb,bk,p,y,bf,br,W,bs,t,J,P,z,E,F,bB,bc,bd,R,r,Y,C,be,D,bp,bA,bx,bz];

for(var i=0;i<bK.length;i++){bJ=new demobrowser.demo.virtual.list.ListItem(bK[i]);
bR.add(bJ);

if(i==10||i==12||i==16){bR.addToSelection(bJ);
}}this.getRoot().add(bR,{left:520,top:40});
}}});
})();
...
qx.Class.define(bn,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider],construct:function(){arguments.callee.base.call(this,0,1,this.getItemHeight(),10);
this.__Ml=new qx.ui.virtual.layer.WidgetCell(this);
this.getPane().addLayer(this.__Ml);
this.getPane().addListener(bo,this._onResize,this);
this.__Mm=[];
this.__Mn=[];
this.__Mo=new qx.ui.virtual.selection.Row(this.getPane());
this.__Mo.addListener(Y,this._onChangeSelection,this);
this.__Mo.attachMouseEvents();
this.__Mo.attachKeyEvents(this);
this.__Mo.attachListEvents(this);
new qx.ui.virtual.behavior.Prefetch(this,0,0,0,0,400,600,400,600).set({interval:500});
},events:{addItem:bb,removeItem:bb,changeValue:bb,changeSelection:bb},properties:{appearance:{refine:true,init:bm},focusable:{refine:true,init:true},spacing:{check:br,init:0,apply:S,themeable:true},enableInlineFind:{check:ba,init:true},name:{check:bp,nullable:true,event:bh},itemHeight:{init:24,themeable:true,check:be,apply:bj},selectionMode:{check:[W,bl,bg,T],init:W,apply:bi},dragSelection:{check:ba,init:false,apply:bq},quickSelection:{check:ba,init:false,apply:bk}},members:{__Mm:null,__Mn:null,__Mo:null,__Ml:null,syncWidget:function(){this.update();
},update:function(){var k=this.getPane().getRowConfig();
k.setItemCount(this.__Mm.length);
k.resetItemSizes();

for(var i=0;i<this.__Mm.length;i++){var h=this.__Mm[i].getHeight();

if(h!==null){k.setItemSize(i,h);
}}},updateSelection:function(){var d=this.__Ml.getChildren();

for(var i=0;i<d.length;i++){var f=d[i];
var g=f.getUserData(X);

if(this.__Mo.isItemSelected(g)){f.addState(bc);
}else{f.removeState(bc);
...
this.fireDataEvent(Y,this.__Ms(e.getData()));
},getCellWidget:function(K,L){var N=this.__Mm[K];

if(!N){return null;
}var M=this.__Mn.pop()||new qx.ui.form.ListItem();
M.set({label:N.getLabel(),icon:N.getIcon()});

if(this.__Mo.isItemSelected(K)){M.addState(bc);
}else{M.removeState(bc);
}M.setUserData(X,K);
return M;
},poolCellWidget:function(q){this.__Mn.push(q);
},getValue:function(){var E=this.getSelection();
var F=[];
var G;

for(var i=0,l=E.length;i<l;i++){G=E[i].getValue();
if(G==null){G=E[i].getLabel();
}F.push(G);
}return F.join(U);
},setValue:function(bv){var by=bv.split(U);
var bx=[];
var bw;

for(var i=0,l=by.length;i<l;i++){bw=this.findItem(by[i]);

if(bw){bx.push(bw);
}}this.setSelection(bx);
},getChildren:function(){return this.__Mm;
},hasChildren:function(){return this.__Mm.length>0;
},_addHelper:function(bu){this.fireDataEvent(bf,bu);
bu.addListener(V,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},_removeHelper:function(J){this.fireDataEvent(bd,J);
J.removeListener(V,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},add:function(P){this.__Mm.push(P);
this._addHelper(P);
return this;
},remove:function(Q){qx.lang.Array.remove(this.__Mm,Q);
this._removeHelper(Q);
return this;
},removeAll:function(){for(var i=0,j=this.__Mm.length;i<j;i++){this._removeHelper(this.__Mm[i]);
}this.__Mm=[];
},indexOf:function(A){return this.__Mm.indexOf(A);
},addAt:function(C,D){qx.lang.Array.insertAt(this.__Mm,C,D);
this._addHelper(C);
},addBefore:function(o,p){qx.lang.Array.insertBefore(this.__Mm,o,p);
this._addHelper(o);
},addAfter:function(a,b){qx.lang.Array.insertAfter(this.__Mm,a,b);
this._addHelper(a);
},removeAt:function(r){this.__Mm.splice(r,1);
this._removeHelper(this.__Mm[r]);
},__Mp:function(bs){var bt=this.__Mm.indexOf(bs);

if(bt<0){return null;
}else{return bt;
}},__Mq:function(u){var v=this.__Mm[u];
return v;
},__Mr:function(y){var z=[];

for(var i=0;i<y.length;i++){z.push(this.__Mp(y[i]));
}return z;
},__Ms:function(bA){var bB=[];

for(var i=0;i<bA.length;i++){bB.push(this.__Mm[bA[i]]);
}return bB;
},selectAll:function(){this.__Mo.selectAll();
},setSelected:function(B){this.__Mo.selectItem(this.__Mp(B));
},isSelected:function(O){return this.__Mo.isItemSelected(this.__Mp(O));
},addToSelection:function(c){this.__Mo.addItem(this.__Mp(c));
},removeFromSelection:function(R){this.__Mo.removeItem(this.__Mp(R));
},selectRange:function(w,x){this.__Mo.selectItemRange(this.__Mp(w),this.__Mp(x));
},resetSelection:function(){this.__Mo.clearSelection();
},setSelection:function(bz){this.__Mo.replaceSelection(this.__Mr(bz));
},getSelectedItem:function(){return this.__Mm[this.getSelected()];
},getSelected:function(){return this.__Mm[this.__Mo.getSelectedItem()];
},getSelection:function(){return this.__Ms(this.__Mo.getSelection());
},getSortedSelection:function(){return this.__Ms(this.__Mo.getSortedSelection());
...
},getSelectables:function(){return this.__Ms(this.__Mo.getSelectables());
},_applySelectionMode:function(m,n){this.__Mo.setMode(m);
},_applyDragSelection:function(s,t){this.__Mo.setDrag(s);
},_applyQuickSelection:function(H,I){this.__Mo.setQuick(H);
}}});
})();
...
(function(){var n="_applyInterval",m="appear",l="interval",k="qx.ui.virtual.behavior.Prefetch",j="__IF",i="_applyScroller",h="qx.ui.virtual.core.Scroller",g="scrollbar-x",f="Integer",e="scrollbar-y";
qx.Class.define(k,{extend:qx.core.Object,construct:function(o,p,q,r,s,t,u,v,w){arguments.callee.base.call(this);
this.setPrefetchX(p,q,r,s);
this.setPrefetchY(t,u,v,w);
this.__IF=new qx.event.Timer(this.getInterval());
this.__IF.addListener(l,this._onInterval,this);

if(o){this.setScroller(o);
}},properties:{scroller:{check:h,nullable:true,init:null,apply:i},interval:{check:f,init:200,apply:n}},members:{__IG:null,__IH:null,__IF:null,__II:null,__IJ:null,setPrefetchX:function(a,b,c,d){this.__IG=[a,b,c,d];
},setPrefetchY:function(z,A,B,C){this.__IH=[z,A,B,C];
},_onInterval:function(){var F=this.__IG;

if(F[1]&&F[3]){this.getScroller().getPane().prefetchX(F[0],F[1],F[2],F[3]);
qx.ui.core.queue.Manager.flush();
}var G=this.__IH;

if(G[1]&&G[3]){this.getScroller().getPane().prefetchY(G[0],G[1],G[2],G[3]);
qx.ui.core.queue.Manager.flush();
}},_applyScroller:function(D,E){if(E){if(this.__II){E.getChildControl(g).removeListenerById(this.__II);
}
if(this.__IJ){E.getChildControl(e).removeListenerById(this.__IJ);
}}
if(D){if(!D.getContainerElement().getDomElement()){this.__IF.stop();
D.addListenerOnce(m,this.__IF.start,this.__IF);
}else{this.__IF.restart();
}this.__II=D.getChildControl("scrollbar-x").addListener("scroll",this.__IF.restart,this.__IF);
this.__IJ=D.getChildControl("scrollbar-y").addListener("scroll",this.__IF.restart,this.__IF);
}else{this.__IF.stop();
}},_applyInterval:function(x,y){this.__IF.setInterval(x);
}},destruct:function(){this.setScroller(null);
this.__IG=this.__IH=null;
...
qx.Class.define(a,{extend:qx.core.Object,construct:function(l,m,n){arguments.callee.base.call(this);
