...
(function(){var s="",r="selected",q="cell",p="layer",o="icon",n="#f3f3f3",m="label",l="Gallery (HTML - divs)",k="manager",j="default",W="demobrowser.demo.virtual.Gallery",V="demobrowser.demo.virtual.WidgetGallery",U="text-align: center;",T="auto",S="color: white; background-color: #00398D;",R="user-desktop.png",Q="__Mk",P="<img src='",O="'></img>",N="group",z="resize",A="folder.png",x="vertical",y="multi",v="network-server.png",w="user-trash.png",t="off",u="top",B="icon/32/places/",C="horizontal",F="demobrowser.demo.virtual.AbstractGallery",E="abstract",H="position: absolute;",G="demobrowser.demo.virtual.HtmlGallery",J="Gallery (widgets)",I="<br>",D="Icon #",M="network-workgroup.png",L="text-selected",K="demobrowser.demo.virtual.GalleryCell";
...
var bJ=new demobrowser.demo.virtual.HtmlGallery(l);
bJ.moveTo(400,50);
}}});
qx.Class.define(F,{extend:qx.ui.window.Window,type:E,construct:function(b){arguments.callee.base.call(this,b);
...
c.set({scrollbarX:t,scrollbarY:T});
c.getPane().addListener(z,this._onPaneResize,this);
this.add(c);
this.manager=new qx.ui.virtual.selection.CellRectangle(c.getPane(),this).set({mode:y,drag:true});
this.manager.attachMouseEvents();
this.manager.attachKeyEvents(c);
},members:{getItemData:function(bK,bL){return this.items[bK*this.itemPerLine+bL];
},_createScroller:function(){},isItemSelectable:function(bs){return !!this.getItemData(bs.row,bs.column);
},styleSelectable:function(bz,bA,bB){},_onPaneResize:function(e){var g=e.getTarget();
var d=e.getData().width;
var f=Math.floor(d/this.itemWidth);

if(f==this.itemsPerLine){return;
}this.itemPerLine=f;
var h=Math.ceil(this.itemCount/f);
g.getColumnConfig().setItemCount(f);
g.getRowConfig().setItemCount(h);
},_generateItems:function(bi){var bl=[];
var bo=[A,w,v,M,R];
var bm=qx.util.AliasManager.getInstance();
var bk=qx.util.ResourceManager.getInstance();

for(var i=0;i<bi;i++){var bp=B+bo[Math.floor(Math.random()*bo.length)];
var bj=bm.resolve(bp);
var bn=bk.toUri(bj);
bl[i]={label:D+(i+1),icon:bp,resolvedIcon:bn};
}return bl;
}},destruct:function(){this.items=null;
this._disposeObjects(k);
}});
qx.Class.define(K,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var bC=new qx.ui.basic.Atom().set({iconPosition:u});
bC.getChildControl(m).set({padding:[0,4]});
bC.getChildControl(o).set({padding:4});
return bC;
},updateData:function(bM,bN){bM.set({icon:bN.icon,label:bN.label});
},updateStates:function(bv,bw){var by=bv.getChildControl(m);
var bx=bv.getChildControl(o);

if(bw.selected){by.setDecorator(r);
by.setTextColor(L);
bx.setDecorator(N);
}else{by.resetDecorator();
by.resetTextColor();
bx.resetDecorator();
}}}});
qx.Class.define(V,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bb){arguments.callee.base.call(this,bb);
this.__Mk=new demobrowser.demo.virtual.GalleryCell();
},members:{_createScroller:function(){var a=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.WidgetCell(this);
a.getPane().addLayer(this.layer);
new qx.ui.virtual.behavior.Prefetch(a,0,0,0,0,200,300,600,800).set({interval:500});
return a;
},styleSelectable:function(bc,bd,be){if(bd!==r){return;
}var bf=this.layer.getChildren();

for(var i=0;i<bf.length;i++){var bg=bf[i];
var bh=bg.getUserData(q);

if(bc.row!==bh.row||bc.column!==bh.column){continue;
}
if(be){this.__Mk.updateStates(bg,{selected:1});
}else{this.__Mk.updateStates(bg,{});
}}},getCellWidget:function(bD,bE){var bI=this.getItemData(bD,bE);

if(!bI){return null;
}var bF={row:bD,column:bE};
var bG={};

if(this.manager.isItemSelected(bF)){bG.selected=true;
}var bH=this.__Mk.getCellWidget(bI,bG);
bH.setUserData(q,bF);
return bH;
},poolCellWidget:function(bT){this.__Mk.pool(bT);
}},destruct:function(){this._disposeObjects(Q,p);
}});
qx.Class.define(G,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bq){arguments.callee.base.call(this,bq);
var br=qx.theme.manager.Font.getInstance().resolve(j).getStyles();
...
var bt=new qx.ui.virtual.layer.GridLines(C,n);
bu.getPane().addLayer(bt);
var bt=new qx.ui.virtual.layer.GridLines(x,n);
bu.getPane().addLayer(bt);
return bu;
...
},getCellProperties:function(bO,bP){var bS=this.getItemData(bO,bP);

if(!bS){return s;
}var bR=this.manager.isItemSelected({row:bO,column:bP});
var bQ=bR?S:s;
return {style:[H,U,this._fontCss,bQ].join(s),content:[P,bS.resolvedIcon,O,I,bS.label].join(s)};
