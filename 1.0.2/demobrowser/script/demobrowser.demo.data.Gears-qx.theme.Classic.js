...
(function(){var k="value",j='undefined',i="",h="model",g="application/x-googlegears",f="demobrowser.demo.data.store.Gears",d="<span style='font-size: 20px'>google Gears</span>",c="loaded",b="Age: ",a="http://code.google.com/apis/gears/install.html",E="lastname",D="firstname",C="age",B="demobrowser.demo.data.Gears",A="First Name: ",z='qooxdoo',y='Gears.Factory',x='select * from Persons',w="delete from Persons",v="LastName: ",r=' (Firstname varchar(255), Lastname varchar(255), Age int)',s="Person",p='ie_mobile',q="none",n='create table if not exists Persons',o="This demo requires Gears to be installed. Install now?",l='insert into Persons values (?, ?, ?)',m='beta.database',t="changeBubble",u="object";
qx.Class.define(B,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var S=new qx.ui.groupbox.GroupBox(s);
this.getRoot().add(S,{left:10,top:50});
S.setLayout(new qx.ui.layout.Grid(0,3));
S.add(new qx.ui.basic.Label(A),{row:0,column:0});
S.add(new qx.ui.basic.Label(v),{row:1,column:0});
S.add(new qx.ui.basic.Label(b),{row:2,column:0});
var M=new qx.ui.form.TextField();
S.add(M,{row:0,column:1});
var O=new qx.ui.form.TextField();
S.add(O,{row:1,column:1});
var P=new qx.ui.form.Spinner(0,0,1000);
S.add(P,{row:2,column:1});
var Q=new qx.data.controller.Object();
Q.addTarget(M,k,D,true);
Q.addTarget(O,k,E,true);
Q.addTarget(P,k,C,true);
var R=new demobrowser.demo.data.store.Gears();
R.bind(h,Q,h);
...
qx.Class.define(f,{extend:qx.data.store.Json,construct:function(){arguments.callee.base.call(this);
this.initGears();
if(!window.google||!google.gears){if(confirm(o)){location.href=a;
return;
}return;
}this.__Li=this.initDB();
this.__Lj();
},members:{__Lj:function(F){var G=this.getPerson();
this._marshaler.toClass(G,true);
this.setModel(this._marshaler.toModel(G));
this.getModel().addListener(t,this.__Lk,this);
this.fireDataEvent(c,this.getModel());
},__Lk:function(T){var U=this.getModel();
this.update(U.getFirstname(),U.getLastname(),U.getAge());
},update:function(V,W,X){if(!google.gears.factory||!this.__Li){return;
}this.__Li.execute(w);
this.__Li.execute(l,[V,W,X]);
},getPerson:function(){var I={};
I.firstname=i;
I.lastname=i;
I.age=0;

if(!google.gears.factory||!this.__Li){return I;
}var J=this.__Li.execute(x);
...
return I;
},initDB:function(){if(window.google&&google.gears){try{var K=google.gears.factory.create(m);

if(K){K.open(z);
K.execute(n+r);
}}catch(H){this.error('Could not create database: '+H.message);
}}return K;
},initGears:function(){if(window.google&&google.gears){return;
}var L=null;
if(typeof GearsFactory!=j){L=new GearsFactory();
}else{try{L=new ActiveXObject(y);
if(L.getBuildInfo().indexOf(p)!=-1){L.privateSetGlobalObject(this);
}}catch(e){if((typeof navigator.mimeTypes!=j)&&navigator.mimeTypes[g]){L=document.createElement(u);
L.style.display=q;
L.width=0;
L.height=0;
L.type=g;
...
if(!google.gears){google.gears={factory:L};
}}}});
})();
