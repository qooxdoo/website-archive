...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.animation.Transition_Position-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"ee39fac0f90a"},
  boot : "boot",
  bootIsInline : true,
...
var w=this.getRoot();
var G=new qx.ui.embed.Html();
G.setHtml(i);
G.setCssClass(b);
w.add(G);
var D={linear:n,easeInQuad:r,easeOutQuad:c,sinodial:g,reverse:f,wobble:s,spring:j};
var u=new qx.ui.form.SelectBox();
var F=new qx.ui.form.Button(h);
var y=new qx.ui.basic.Label(t);
var z=new qx.ui.basic.Label(p);
var E=new qx.ui.basic.Label(d);
var z=new qx.ui.basic.Label(D.linear);
var A=new qx.ui.form.Spinner;

for(var x in D){u.add(new qx.ui.form.ListItem(x));
}u.addListener(o,function(e){z.setValue(D[z.setValue(e.getData()[0].getLabel())]);
});
A.set({maximum:10.0,minimum:0.1,value:1.0});
var v;
var C=false;
G.addListenerOnce(k,function(){v=new qx.fx.effect.core.Move(G.getContainerElement().getDomElement());
v.set({x:600,y:300,mode:a});
v.addListener(q,function(){v.set({x:C?90:600,y:C?90:300});
C=!C;
});
},this);
var B=new qx.util.format.NumberFormat();
B.setMaximumFractionDigits(2);
A.setNumberFormat(B);
F.addListener(l,function(){var H=u.getSelection()[0].getLabel();
v.set({transition:H,duration:A.getValue()});
v.start();
});
w.add(y,{left:25,top:50});
w.add(z,{left:25,top:75});
w.add(E,{left:25,top:25});
w.add(u,{left:90,top:50});
w.add(z,{left:90,top:75});
w.add(A,{left:90,top:25});
w.add(F,{left:23,top:100});
}}});
})();
...
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:g,check:[g,e]},x:{init:0,check:f},y:{init:0,check:f}},members:{__Az:null,__AA:null,__AB:null,__AC:null,__AD:null,setup:function(){var m=this._getElement();
arguments.callee.base.call(this);

if(m.parentNode){this.__AB=qx.bom.element.Location.getLeft(m)-qx.bom.element.Location.getLeft(m.parentNode);
this.__AC=qx.bom.element.Location.getTop(m)-qx.bom.element.Location.getTop(m.parentNode);
}else{this.__AB=qx.bom.element.Location.getLeft(m);
this.__AC=qx.bom.element.Location.getTop(m);
}this.__AD=qx.bom.element.Style.get(m,"position");
qx.bom.element.Style.set(m,h,e);

if(this.getMode()==b){this.__Az=this.getX()-this.__AB;
this.__AA=this.getY()-this.__AC;
}else{this.__Az=this.getX();
this.__AA=this.getY();
}},update:function(j){var l=this._getElement();
arguments.callee.base.call(this);
var k=Math.round(this.__Az*j+this.__AB);
var top=Math.round(this.__AA*j+this.__AC);
qx.bom.element.Style.set(l,c,k+i);
qx.bom.element.Style.set(l,d,top+i);
},afterFinishInternal:function(){qx.bom.element.Style.set(this._getElement(),h,this.__AD);
}}});
})();
