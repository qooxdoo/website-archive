...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.test.Validation-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"5efea09a4acd"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var j="<br>",i="new name: ",h="new gender: ",g="male",f="female",e="new age: ",d="demobrowser.demo.test.Validation",c="<b>Setting the age ",b="property to 27</b><br>",a="- name: Name is validated to a minimum length of two.<br/>",P="<br>old name: ",O=")",N="age:",M="fmale",L="<br><b>Setting the name to 'Hans'</b><br>",K="property to 'female'</b><br>",J="<br><b>Setting the gender ",I="must be longer than one characters. (",H="<b>Testing the gender ",G="<br>old gender: ",q="<br><b>Setting the age ",r="- age: Age is validated to be a number.<br/>",o="<b>Initial property values</b><br>",p="",m="<b>Setting the name to ''</b><br>",n="demobrowser.demo.test.Person",k="property to 'true'</b><br>",l="- gender: Gender is validated to be either male or female.",s="property to 'fmale'</b><br>",t="__Ma",y="<font color='red'>GENDER NOT SET</font>",x="gender:",A="<br>old age: ",z="Hans",C="The demo creates an internal class containing three properties:<br/>",B="Validation Error: Name ",v="<font color='red'>AGE NOT SET</font>",F="My name",E="<font color='red'>NAME NOT SET</font>",D="white",u="name:",w="<b>Description</b><br/>";
qx.Class.define(d,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var S=new demobrowser.demo.test.Person();
var T=new qx.ui.embed.Html();
T.setBackgroundColor(D);
T.set({padding:10,width:250,height:360});
this.getRoot().add(T,{left:10,top:10});
T.setHtml(o);
T.setHtml(T.getHtml()+u+S.getName()+j);
T.setHtml(T.getHtml()+N+S.getAge()+j);
T.setHtml(T.getHtml()+x+S.getGender()+j);
T.setHtml(T.getHtml()+L);
S.setName(z);
T.setHtml(T.getHtml()+i+S.getName()+j);
T.setHtml(T.getHtml()+m);
try{S.setName(p);
T.setHtml(T.getHtml()+i+S.getName()+j);
}catch(V){T.setHtml(T.getHtml()+E+P+S.getName()+j);
}T.setHtml(T.getHtml()+q+b);
S.setAge(27);
T.setHtml(T.getHtml()+e+S.getAge()+j);
T.setHtml(T.getHtml()+c+k);
try{S.setAge(true);
T.setHtml(T.getHtml()+e+S.getAge()+j);
}catch(Q){T.setHtml(T.getHtml()+v+A+S.getAge()+j);
}T.setHtml(T.getHtml()+J+K);
S.setGender(f);
T.setHtml(T.getHtml()+h+S.getGender()+j);
T.setHtml(T.getHtml()+H+s);
try{S.setAge(M);
T.setHtml(T.getHtml()+h+S.getGender()+j);
}catch(R){T.setHtml(T.getHtml()+y+G+S.getGender()+j);
}var U=new qx.ui.basic.Label();
U.setRich(true);
U.setWidth(250);
U.setValue(w+C+a+r+l);
this.getRoot().add(U,{left:280,top:10});
}}});
qx.Class.define(n,{extend:qx.core.Object,properties:{name:{init:F,validate:t},age:{init:18,validate:qx.util.Validate.number()},gender:{init:g,validate:qx.util.Validate.inArray([g,f])}},members:{__Ma:function(W){if(W.length<2){throw new qx.core.ValidationError(B+I+W+O);
}}}});
})();
