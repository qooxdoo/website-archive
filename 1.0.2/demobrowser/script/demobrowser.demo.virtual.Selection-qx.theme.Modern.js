...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.virtual.Selection","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.virtual.Selection-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"5d5df0396635"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var n="manager",m="multi",l=" / ",k="bold",j="value",i="white",h="selected",g="#EEE",f="horizontal",d="table-scroller-header",I="one",H="single",G="changeValue",F="table-row-background-selected",E="additive",D="default",C="Additive Selection",B="Cell Line Selection",A="addaptive",z="Options",u="Row Selection",v="changeSelection",s="Column Selection",t="Enable drag selection",q="demobrowser.demo.virtual.Selection",r="Selection Mode",o="Single Selection",p="Cell Rectangle Selection",w="Enable quick selection",x="One Selection",y="Multi Selection";
qx.Class.define(q,{extend:qx.application.Standalone,members:{main:function(){arguments.callee.base.call(this);
var bF=qx.theme.manager.Font.getInstance().resolve(D).getStyles();
this._fontCss=qx.bom.element.Style.compile(bF);
var bO=new qx.ui.layout.Grid(5,0);
bO.setRowFlex(2,1);
bO.setColumnFlex(0,1);
bO.setColumnFlex(1,1);
bO.setColumnFlex(2,1);
bO.setColumnFlex(3,1);
var bC=[];
var bM=new qx.ui.container.Composite(bO);
this.getRoot().add(bM,{edge:5});
bM.add(new qx.ui.basic.Label(u).set({font:k,decorator:d,padding:3,allowGrowX:true}),{row:1,column:0});
var bE=this.createRowSelectionScroller();
bC.push(bE.getUserData(n));
bM.add(bE,{row:2,column:0});
bM.add(new qx.ui.basic.Label(s).set({font:k,decorator:d,padding:3,allowGrowX:true}),{row:1,column:1});
var bE=this.createColumnSelectionScroller();
bC.push(bE.getUserData(n));
bM.add(bE,{row:2,column:1});
bM.add(new qx.ui.basic.Label(p).set({font:k,decorator:d,padding:3,allowGrowX:true}),{row:1,column:2});
var bE=this.createCellRectangleSelectionScroller();
bC.push(bE.getUserData(n));
bM.add(bE,{row:2,column:2});
bM.add(new qx.ui.basic.Label(B).set({font:k,decorator:d,padding:3,allowGrowX:true}),{row:1,column:3});
var bE=this.createCellLineSelectionScroller();
bC.push(bE.getUserData(n));
bM.add(bE,{row:2,column:3});
var bK=new qx.ui.layout.Grid(20,4);
var bH=new qx.ui.container.Composite(bK).set({padding:15,backgroundColor:i});
bM.add(bH,{row:0,column:0,colSpan:4});
bH.add(new qx.ui.basic.Label(r).set({font:k,padding:3}),{row:0,column:0});
var bG=new qx.ui.form.RadioButton(o);
bG.setUserData(j,H);
bH.add(bG,{row:1,column:0});
var bJ=new qx.ui.form.RadioButton(y);
bJ.setUserData(j,m);
bJ.setValue(true);
bH.add(bJ,{row:2,column:0});
var bD=new qx.ui.form.RadioButton(C);
bD.setUserData(j,E);
bH.add(bD,{row:3,column:0});
var bB=new qx.ui.form.RadioButton(x);
bB.setUserData(j,I);
bH.add(bB,{row:4,column:0});
var bI=new qx.ui.form.RadioGroup(bG,bJ,bD,bB);
bI.addListener(v,function(e){var bP=e.getData()[0].getUserData(j);
bC.forEach(function(U){U.setMode(bP);
});

if(bP==H||bP==I){bN.setEnabled(false);
bL.setEnabled(true);
}else if(bP==m||bP==A){bN.setEnabled(true);
bL.setEnabled(false);
}});
bH.add(new qx.ui.basic.Label(z).set({font:k,padding:3}),{row:0,column:1});
var bN=new qx.ui.form.CheckBox(t);
bH.add(bN,{row:1,column:1});
bN.addListener(G,function(e){if(e.getData()){var T=bC[0].getMode();

if(T==H||T==I){this.debug("Drag selection is only available for the modes multi or additive");
}}bC.forEach(function(bk){bk.setDrag(e.getData());
});
});
var bL=new qx.ui.form.CheckBox(w).set({enabled:false});
bH.add(bL,{row:2,column:1});
bL.addListener(G,function(e){if(e.getData()){var J=bC[0].getMode();

if(J==m||J==E){this.debug("Quick selection is only available for the modes multi or additive");
}}bC.forEach(function(P){P.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bg=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var be=new qx.ui.virtual.layer.Row(i,g);
bg.getPane().addLayer(be);
bg.getPane().addLayer(new qx.ui.virtual.layer.GridLines(f));
var bf=new qx.ui.virtual.cell.Cell();
var bd=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(a,b){var c={};

if(bc.isItemSelected(a)){c.selected=true;
}return bf.getCellProperties(a+l+b,c);
}});
bg.getPane().addLayer(bd);
var bc=new qx.ui.virtual.selection.Row(bg.getPane(),{styleSelectable:function(V,W,X){if(W!==h){return;
}
if(X){be.setDecorator(h);
}else{be.setDecorator(null);
}bd.updateLayerData();
}});
bc.attachMouseEvents(bg.getPane());
bc.attachKeyEvents(bg);
bc.set({mode:m});
bc.addItem(0);
bg.setUserData(n,bc);
return bg;
},createColumnSelectionScroller:function(){var bw=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bw.getPane().addLayer(new qx.ui.virtual.layer.Row(i,g));
var bv=new qx.ui.virtual.layer.Column();
bw.getPane().addLayer(bv);
bw.getPane().addLayer(new qx.ui.virtual.layer.GridLines(f));
var bx=new qx.ui.virtual.cell.Cell();
var bu=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(Q,R){var S={};

if(bt.isItemSelected(R)){S.selected=true;
}return bx.getCellProperties(Q+l+R,S);
}});
bw.getPane().addLayer(bu);
var bt=new qx.ui.virtual.selection.Column(bw.getPane(),{styleSelectable:function(bl,bm,bn){if(bm!==h){return;
}
if(bn){bv.setDecorator(h);
}else{bv.setDecorator(null);
}bu.updateLayerData();
}});
bt.attachMouseEvents(bw.getPane());
bt.attachKeyEvents(bw);
bt.set({mode:m});
bt.addItem(0);
bw.setUserData(n,bt);
return bw;
},createCellRectangleSelectionScroller:function(){var N=new qx.ui.virtual.core.Scroller(1000,100,20,100);
N.getPane().addLayer(new qx.ui.virtual.layer.Row(i,g));
var M=new qx.ui.virtual.cell.Cell();
var O=new qx.ui.virtual.cell.Cell().set({backgroundColor:F});
var L=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bh,bi){var bj={};

if(K.isItemSelected({row:bh,column:bi})){bj.selected=true;
}
if(bj.selected){return O.getCellProperties(bh+l+bi,bj);
}else{M.resetBackgroundColor();
return M.getCellProperties(bh+l+bi,bj);
}}});
N.getPane().addLayer(L);
N.getPane().addLayer(new qx.ui.virtual.layer.GridLines(f));
var K=new qx.ui.virtual.selection.CellRectangle(N.getPane(),{styleSelectable:function(Y,ba,bb){L.updateLayerData();
}});
K.attachMouseEvents(N.getPane());
K.attachKeyEvents(N);
K.set({mode:m});
K.addItem({row:0,column:0});
N.setUserData(n,K);
return N;
},createCellLineSelectionScroller:function(){var br=new qx.ui.virtual.core.Scroller(1000,100,20,100);
br.getPane().addLayer(new qx.ui.virtual.layer.Row(i,g));
var bq=new qx.ui.virtual.cell.Cell();
var bs=new qx.ui.virtual.cell.Cell().set({backgroundColor:F});
var bp=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bQ,bR){var bS={};

if(bo.isItemSelected({row:bQ,column:bR})){bS.selected=true;
}
if(bS.selected){return bs.getCellProperties(bQ+l+bR,bS);
}else{bq.resetBackgroundColor();
return bq.getCellProperties(bQ+l+bR,bS);
}}});
br.getPane().addLayer(bp);
br.getPane().addLayer(new qx.ui.virtual.layer.GridLines(f));
var bo=new qx.ui.virtual.selection.CellLines(br.getPane(),{styleSelectable:function(by,bz,bA){bp.updateLayerData();
}});
...
},_getRelatedSelectable:function(l,m){if(m==a){var o=l-1;
var n=0;
var p=-1;
}else if(m==c){var o=l+1;
var n=this._getItemCount()-1;
var p=1;
}else{return null;
}
for(var i=o;i!==n+p;i+=p){if(this._isSelectable(i)){return i;
}}return null;
},_scrollItemIntoView:function(e){this._pane.scrollColumnIntoView(e);
},_getSelectableLocationX:function(f){var g=this._pane.getColumnConfig();
var j=g.getItemPosition(f);
var h=j+g.getItemSize(f)-1;
return {left:j,right:h};
},_getSelectableLocationY:function(k){return {top:0,bottom:this._pane.getRowConfig().getTotalSize()-1};
}}});
})();
...
},_getSelectableFromMouseEvent:function(event){var U=this._pane.getCellAtPosition(event.getDocumentLeft(),event.getDocumentTop());

if(!U){return null;
}return this._isSelectable(U)?U:null;
},getSelectables:function(){var P=[];
var S=this._pane.getRowConfig().getItemCount();
var O=this._pane.getColumnConfig().getItemCount();

for(var T=0;T<S;T++){for(var Q=0;Q<O;Q++){var R={row:T,column:Q};

if(this._isSelectable(R)){P.push(R);
}}}return P;
...
},_getFirstSelectable:function(){var W=this._pane.getRowConfig().getItemCount();
var V=this._pane.getColumnConfig().getItemCount();

for(var ba=0;ba<W;ba++){for(var Y=0;Y<V;Y++){var X={row:ba,column:Y};
...
},_getRelatedSelectable:function(E,F){var G={row:E.row,column:E.column};

switch(F){case e:for(var K=E.row-1;K>=0;K--){G.row=K;

if(this._isSelectable(G)){return G;
}}break;
case f:var H=this._pane.getRowConfig().getItemCount();

for(var K=E.row+1;K<H;K++){G.row=K;

if(this._isSelectable(G)){return G;
}}break;
case d:for(var J=E.column-1;J>=0;J--){G.column=J;

if(this._isSelectable(G)){return G;
}}break;
case c:var I=this._pane.getColumnConfig().getItemCount();

for(var J=E.column+1;J<I;J++){G.column=J;
...
},_scrollItemIntoView:function(L){this._pane.scrollCellIntoView(L.column,L.row);
},_getSelectableLocationX:function(m){var n=this._pane.getColumnConfig();
var p=n.getItemPosition(m.column);
var o=p+n.getItemSize(m.column)-1;
return {left:p,right:o};
},_getSelectableLocationY:function(A){var D=this._pane.getRowConfig();
var C=D.getItemPosition(A.row);
var B=C+D.getItemSize(A.row)-1;
