...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.table.Table_Context_Menu-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"14ab21102f7e"},
  boot : "boot",
  bootIsInline : true,
...
(function(){var r="execute",q="changeValue",p="value",o="icon/22/actions/edit-undo.png",n="Change ID sort method",m="Disabled ctx menu",l="icon/22/status/dialog-information.png",k="On",j="Change row with ID 10",h="Default ctx menu #2",J="Remove 5 rows",I="Add 10 rows",H="Context-menu selectable",G="",F="_tableModel",E="Table",D="icon/22/actions/list-remove.png",C="Whether the the first visible row should ",B=", ",A="demobrowser.demo.table.Table_Context_Menu",y="Default ctx menu #1",z="icon/22/actions/list-add.png",w="Selected rows:<br>",x="Keep first row",u="Off",v="be rendered completely when scrolling.",s="Show selection",t="Demonstrate use of alternate sorting algorithm.";
qx.Class.define(A,{extend:demobrowser.demo.table.TableDemo,members:{main:function(){qx.Class.include(qx.ui.table.Table,qx.ui.table.MTableContextMenu);
arguments.callee.base.call(this);
this.getRoot().setNativeContextMenu(true);
},getCaption:function(){return E;
},createTable:function(){var bn=this.createRandomRows(50);
var bm=this._tableModel=new qx.ui.table.model.Simple();
bm.setColumns([y,h,m,H]);
bm.setData(bn);
var bo=new qx.ui.table.Table(bm);
bo.set({width:600,height:400,decorator:null});
var bl=bo.getTableColumnModel();
bl.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
bo.setContextMenuHandler(3,this._contextMenuHandlerBoolean);
bo.setContextMenuHandler(2,this._contextMenuHandlerNull);
return bo;
},_contextMenuHandlerBoolean:function(a,b,c,d,f){var g;

for(var i=0;i<=1;i++){g=new qx.ui.menu.Button(i?k:u);
g.setUserData(p,i?true:false);
g.addListener(r,function(e){d.setValue(a,b,this.getUserData(p));
});
f.add(g);
}return true;
},_contextMenuHandlerNull:function(N,O,P,Q,R){return true;
},createControls:function(){var bd=new qx.ui.toolbar.ToolBar();
var bb,bc,be;
bc=new qx.ui.toolbar.Part();
bd.add(bc);
bb=new qx.ui.toolbar.Button(j,o);
bb.addListener(r,function(S){var T=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,T[0][i]);
}this.info("Row 10 changed");
},this);
bc.add(bb);
bb=new qx.ui.toolbar.Button(I,z);
bb.addListener(r,function(bs){var bt=this.createRandomRows(10);
this._tableModel.addRows(bt);
this.info("10 rows added");
},this);
bc.add(bb);
bb=new qx.ui.toolbar.Button(J,D);
bb.addListener(r,function(bf){var bg=this._tableModel.getRowCount();
this._tableModel.removeRows(bg-5,5);
this.info("5 rows removed");
},this);
bc.add(bb);
bb=new qx.ui.toolbar.Button(s,l);
bb.addListener(r,function(bq){var br=[];
ba.getSelectionModel().iterateSelection(function(bp){br.push(bp+G);
});
this.showDialog(w+br.join(B));
},this);
bc.add(bb);
bc=new qx.ui.toolbar.Part();
bd.add(bc);
var ba=this._table;
be=new qx.ui.toolbar.CheckBox(x);
be.set({value:ba.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(C+v)});
be.addListener(q,function(Y){ba.setKeepFirstVisibleRowComplete(this.getValue());
},be);
bc.add(be);
be=new qx.ui.toolbar.CheckBox(n);
be.set({value:ba.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(t)});
be.addListener(q,function(K){if(K.getData()){var M=function(bh,bi){var bj=bh[arguments.callee.columnIndex];
var bk=bi[arguments.callee.columnIndex];

if(bj%2==1&&bk%2==0){return 1;
}
if(bk%2==1&&bj%2==0){return -1;
}return (bj>bk)?1:((bj==bk)?0:-1);
};
var L=function(U,V){var W=U[arguments.callee.columnIndex];
...
ba.getTableModel().setSortMethods(0,{ascending:M,descending:L});
}else{ba.getTableModel().setSortMethods(0,null);
}},be);
bc.add(be);
return bd;
...
this.__FT=[];
},members:{__FT:null,setContextMenuHandler:function(j,k){this.__FT[j]=k;
},getContextMenuHandler:function(i){return this.__FT[i];
