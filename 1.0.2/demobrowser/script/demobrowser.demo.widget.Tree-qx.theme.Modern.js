...
bA.setOpen(true);
bC.add(bA);
var bs=new qx.ui.tree.TreeFolder(G);
var bq=new qx.ui.tree.TreeFolder(r);
var br=new qx.ui.tree.TreeFolder(K);
var bp=new qx.ui.tree.TreeFolder(k);
bA.add(bs,bq,br,bp);
var by=new qx.ui.tree.TreeFile(d);
var bo=new qx.ui.tree.TreeFile(c);
bq.add(by,bo);
var bB=new qx.ui.tree.TreeFolder(b);
var bt=new qx.ui.tree.TreeFolder(M);
var bw=new qx.ui.tree.TreeFolder(H);
var bv=new qx.ui.tree.TreeFolder(k);

for(var i=0;i<30;i++){bv.add(new qx.ui.tree.TreeFile(q+i));
}var bz=new qx.ui.tree.TreeFolder(o);
var bu=new qx.ui.tree.TreeFolder(F);
bB.add(bt,bw,bv,bz,bu);
bC.add(bB);
return bx;
},getCommandFrame:function(Q){var bb=new qx.ui.groupbox.GroupBox(J);
var Y=4;
bb.setLayout(new qx.ui.layout.Grid(5,3));
var bi=0;
bb.add(new qx.ui.basic.Label(p).set({paddingTop:4}),{row:bi,column:0});
var bf=new qx.ui.form.TextField().set({readOnly:true});
bb.add(bf,{row:bi++,column:1});
Q.addListener(j,function(e){var a=e.getData();

if(a.length>0){if(this.getSelectionMode()===g){bf.setValue(a.length+I);
}else{bf.setValue(a[0].getLabel());
}}else{bf.setValue(u);
}});
bb.add(new qx.ui.core.Spacer(Y,Y),{row:bi++,column:0});
bb.add(new qx.ui.basic.Label(N),{row:bi,column:0});
var U={"click":v,"dblclick":B,"none":z};
...
}S.addListener(j,function(e){Q.setOpenMode(e.getData()[0].getUserData(h));
});
bb.add(new qx.ui.core.Spacer(Y,Y),{row:bi++,column:0});
bb.add(new qx.ui.basic.Label(A),{row:bi,column:0});
var W=new qx.ui.form.CheckBox(y);
bb.add(W,{row:bi++,column:1});
W.addListener(m,function(e){var bD=e.getData();
Q.setSelectionMode(bD?g:f);
});
var X=new qx.ui.form.CheckBox(s);
bb.add(X,{row:bi++,column:1});
X.addListener(m,function(e){var bn=e.getData();
Q.setDragSelection(bn);

if(!W.getValue()){W.setValue(true);
}});
bb.add(new qx.ui.core.Spacer(Y,Y),{row:bi++,column:0});
bb.add(new qx.ui.basic.Label(C),{row:bi,column:0});
var bg=new qx.ui.form.CheckBox(L);
bb.add(bg,{row:bi++,column:1});
bg.addListener(m,function(e){Q.setHideRoot(e.getData());
});
var V=new qx.ui.form.CheckBox(w);
bb.add(V,{row:bi++,column:1});
V.addListener(m,function(e){Q.setRootOpenClose(e.getData());
});
bb.add(new qx.ui.core.Spacer(Y,Y),{row:bi++,column:0});
var be=new qx.ui.form.Button(E);
bb.add(be,{row:bi++,column:1});
be.addListener(l,function(e){alert((""+Q.getItems()).replace(",","\n","g"));
});
var R=new qx.ui.form.Button(t);
bb.add(R,{row:bi++,column:1});
R.addListener(l,function(e){alert((""+Q.getItems(true,false)).replace(",","\n","g"));
});
var bc=new qx.ui.form.Button(D);
bb.add(bc,{row:bi++,column:1});
bc.addListener(l,function(e){if(this.getSelectionMode()===f){alert(this.getSelection());
}else{alert(this.getSelection().toString().replace(",","\n","g"));
}},Q);
var ba=new qx.ui.form.Button(x);
bb.add(ba,{row:bi++,column:1});
var bh=true;
ba.addListener(l,function(e){Q.setHeight(bh?600:400);
bh=!bh;
});
return bb;
}}});
