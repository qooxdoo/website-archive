...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.layout.VBox_Reversed","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["demobrowser:demobrowser.demo.layout.VBox_Reversed-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"7d6222123df3"},
  boot : "boot",
  bootIsInline : true,
...
qx.Class.define(k,{extend:demobrowser.demo.util.LayoutApplication,members:{main:function(){arguments.callee.base.call(this);
...
qx.Class.define(h,{extend:qx.ui.core.scroll.AbstractScrollArea,construct:function(){arguments.callee.base.call(this);
this.setWidth(null);
this._editorGroups={};
...
},properties:{widget:{check:r,apply:s}},members:{handleWidgetClick:function(e){var S=e.getTarget();
this.setWidget(S);
},_createWidgetIndicator:function(){var z=new qx.ui.container.Composite(new qx.ui.layout.VBox().set({spacing:5}));
z.add(this._createLabel(n));
this._widgetIndicator=new qx.ui.basic.Label().set({height:30,allowGrowX:true,allowGrowY:true,padding:5});
z._add(this._widgetIndicator);
return z;
},_createLabel:function(L){return new qx.ui.basic.Label(L).set({font:k,allowGrowX:true,padding:[5,0,5,0]});
...
I.add(H);
return I;
},updateWidgetLayoutPropertyEditor:function(M){var O=M.getLayoutParent()?M.getLayoutParent()._getLayout():null;
var N=this.getWidgetLayoutPropertyEditor(O);

if(N){N.getChildren()[1].setSelected(M);
}
if(N===this._wlpe){return;
}
if(this._wlpe){this._pane.remove(this._wlpe);
}
if(!N){this._wlpe=N;
return;
}this._pane.addAfter(N,this._layoutControls);
this._wlpe=N;
},updateLayoutPropertyEditor:function(P){var R=P.getLayout?P.getLayout():null;
var Q=this.getLayoutPropertyEditor(R);

if(Q){Q.getChildren()[1].setSelected(R);
}
if(Q===this._lpe){return;
}
...
}var name=c+A.constructor.classname;
var B;

if(this._editorGroups[name]){return this._editorGroups[name];
}
switch(name){case w:case u:B=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.BOX_PROPERTIES);
break;
case v:B=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.CANVAS_PROPERTIES);
break;
case p:B=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.BASIC_PROPERTIES);
break;
case d:B=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.DOCK_PROPERTIES);
break;
case j:B=new demobrowser.demo.util.LayoutPropertyGroup(demobrowser.demo.util.LayoutPropertyGroup.GRID_PROPERTIES);
break;
}
if(!B){return null;
}this._editorGroups[name]=this._createContainer(A.constructor.classname+t,B);
return this._editorGroups[name];
},getLayoutPropertyEditor:function(J){if(!J){return null;
}var name=a+J.constructor.classname;
var K;

if(this._editorGroups[name]){return this._editorGroups[name];
}
switch(name){case f:K=new demobrowser.demo.util.PropertyGroup(demobrowser.demo.util.PropertyGroup.HBOX_PROPERTIES);
break;
case b:K=new demobrowser.demo.util.PropertyGroup(demobrowser.demo.util.PropertyGroup.DOCK_PROPERTIES);
break;
case o:K=new demobrowser.demo.util.PropertyGroup(demobrowser.demo.util.PropertyGroup.GRID_PROPERTIES);
break;
}
if(!K){return null;
}this._editorGroups[name]=this._createContainer(J.constructor.classname+q,K);
return this._editorGroups[name];
...
qx.Class.define(g,{extend:qx.ui.core.Widget,construct:function(Q){arguments.callee.base.call(this);
this._properties=Q;
var S=new qx.ui.layout.Grid(10,5);
S.setColumnAlign(0,a,d);
S.setColumnMinWidth(0,110);
this._setLayout(S);
this.set({allowShrinkX:false,allowShrinkY:false});
var ba=0;

for(var V in this._properties){var Y=this._properties[V].type;
var U=this._properties[V].nullable;
this._add(new qx.ui.basic.Label(V).set({paddingTop:4}),{row:ba,column:0});

if(U){var T=new qx.ui.form.CheckBox(h);
T.addListener(q,this._createOnNullPropertyChange(V),this);
this._add(T,{row:ba,column:2});
this._properties[V].nullWidget=T;
}
if(Y==n){var X=new qx.ui.form.Spinner().set({minimum:this._properties[V].min||0,maximum:this._properties[V].max!==undefined?this._properties[V].max:1000});
X.addListener(q,this._createOnIntPropertyChange(V),this);
this._add(X,{row:ba++,column:1});
}else if(Y==k){var X=new qx.ui.form.CheckBox();
X.addListener(q,this._createOnBoolPropertyChange(V),this);
this._add(X,{row:ba++,column:1});
}else if(Y==p){var W=this._properties[V].values;
var X=new qx.ui.form.RadioGroup();

for(var i=0;i<W.length;i++){var R=new qx.ui.form.RadioButton(W[i]);
R.setUserData(r,W[i]);
X.add(R);
this._add(R,{row:ba++,column:1});
}X.addListener(b,this._createOnEnumPropertyChange(V,X),this);
}else if(Y==m){var X=new qx.ui.form.TextField();
X.setLiveUpdate(true);
X.addListener(q,this._createOnIntPropertyChange(V),this);
this._add(X,{row:ba++,column:1});
}this._properties[V].formItem=X;
}},statics:{WIDGET_PROPERTIES:{"width":{type:"int",nullable:true},"height":{type:"int",nullable:true},"minWidth":{type:"int",nullable:true},"minHeight":{type:"int",nullable:true},"maxWidth":{type:"int",nullable:true},"maxHeight":{type:"int",nullable:true},"allowGrowX":{type:"bool",nullable:false},"allowGrowY":{type:"bool",nullable:false},"allowShrinkX":{type:"bool",nullable:false},"allowShrinkY":{type:"bool",nullable:false},"marginTop":{type:"int",min:-1000,nullable:false},"marginRight":{type:"int",min:-1000,nullable:false},"marginBottom":{type:"int",min:-1000,nullable:false},"marginLeft":{type:"int",min:-1000,nullable:false},"paddingTop":{type:"int",nullable:false},"paddingRight":{type:"int",nullable:false},"paddingBottom":{type:"int",nullable:false},"paddingLeft":{type:"int",nullable:false},"alignX":{type:"enum",values:["left","center","right"],nullable:true},"alignY":{type:"enum",values:["top","middle","bottom","baseline"],nullable:true}},HBOX_PROPERTIES:{"alignX":{type:"enum",values:["left","center","right"],nullable:false},"alignY":{type:"enum",values:["top","middle","bottom","baseline"],nullable:false},"spacing":{type:"int",nullable:false},"reversed":{type:"bool",nullable:false}},DOCK_PROPERTIES:{"sort":{type:"enum",values:["auto","y","x"],nullable:false},"spacingX":{type:"int",nullable:false},"spacingY":{type:"int",nullable:false}},GRID_PROPERTIES:{"horizontalSpacing":{type:"int",nullable:false},"verticalSpacing":{type:"int",nullable:false}}},properties:{selected:{apply:c,nullable:true}},members:{_applySelected:function(K,L){this._updateControls(K);
},_setProperty:function(G,name,H){if(!G){return;
}var J=o+qx.lang.String.firstUp(name);
var I=this._properties[name].convert;

if(I){H=I(H);
}G[J](H);
},_getProperty:function(N,name){var O=f+qx.lang.String.firstUp(name);
return N[O]();
},_hasProperty:function(D,name){var E=o+qx.lang.String.firstUp(name);
return (typeof (D[E])==j);
},_createOnIntPropertyChange:function(bc){return function(e){var P=this.getSelected();
this._setProperty(P,bc,e.getTarget().getValue());
};
},_createOnBoolPropertyChange:function(M){return function(e){var u=this.getSelected();
this._setProperty(u,M,e.getTarget().getValue());
};
},_createOnEnumPropertyChange:function(s,t){return function(e){var F=this.getSelected();
this._setProperty(F,s,t.getSelection()[0].getUserData(r));
};
},_createOnNullPropertyChange:function(bb){return function(e){var be=this.getSelected();
var bd=e.getTarget();

if(bd.getValue()){this._setProperty(be,bb,null);
this._properties[bb].formItem.setEnabled(false);
}else{var bf=null;

if(this._properties[bb].type==p){bf=this._properties[bb].formItem.getSelection()[0].getUserData(r);
}else{bf=this._properties[bb].formItem.getValue();
}this._setProperty(be,bb,bf);
this._properties[bb].formItem.setEnabled(true);
}};
...
}},"right":{type:"string",nullable:true,convert:function(m){if(parseInt(m).toString()==m){m=parseInt(m);
}return m||null;
}},"bottom":{type:"string",nullable:true,convert:function(h){if(parseInt(h).toString()==h){h=parseInt(h);
}return h||null;
...
}}},DOCK_PROPERTIES:{"width":{type:"int",nullable:true,convert:function(c){return c==null?null:c+"%";
}},"height":{type:"int",nullable:true,convert:function(g){return g==null?null:g+"%";
}},"edge":{type:"enum",values:["north","east","south","west","center"],nullable:false}},GRID_PROPERTIES:{"row":{type:"int",nullable:false},"column":{type:"int",nullable:false},"rowSpan":{type:"int",nullable:true},"colSpan":{type:"int",nullable:true}}},members:{_setProperty:function(i,name,j){var k=this._properties[name].convert;

if(k){j=k(j);
}var l={};
l[name]=j;
i.setLayoutProperties(l);
},_getProperty:function(d,name){return d.getLayoutProperties()[name]||null;
},_hasProperty:function(f,name){return true;
}}});
})();
