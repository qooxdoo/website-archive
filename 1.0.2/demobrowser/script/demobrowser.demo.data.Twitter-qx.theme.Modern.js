...
(function(){var j="value",i="icon",h="model",g="text",f="qx.client",e="execute",d="source",c="wittemann",b="mshtml",a=".json",H="user.profile_image_url",G="http://twitter.com/statuses/user_timeline/",F="Enter",E="<span style='font-size: 20px'>Twitter</span>",D="<a target='_blank' href='http://twitter.com/home?status=Check%20out%20the%20%23qooxdoo%20twitter%20demo%20at%20http://bit.ly/Mc4Of'>Tweet This</a></span>",C="demobrowser.demo.data.store.Twitter",B="keydown",A="Friends",z="loaded",y="demobrowser.demo.data.Twitter",q="selection[0]",r="<span style='font-size: 13px'>",o="Location: ",p="Details",m="Name: ",n="?callback=demobrowser.demo.data.store.Twitter.saveResult",k="User",l="http://twitter.com/statuses/friends_timeline.json",s="Postet with: ",t="failed",v="user.name",u="user.location",x="Avatar: ",w="Message: ";
...
this.getRoot().add(P,{left:10,top:85});
P.setWidth(300);
var R=new qx.data.controller.List(null,P);
R.setDelegate(this);
R.setLabelPath(g);
if(!qx.core.Variant.isSet(f,b)){R.setIconPath(H);
}var V=new demobrowser.demo.data.store.Twitter(c);
V.bind(h,R,h);
var N=new qx.ui.form.Button(A);
this.getRoot().add(N,{left:10,top:55});
N.addListener(e,function(){V.setUrl(l);
},this);
var S=new qx.ui.form.Button(k);
this.getRoot().add(S,{left:90,top:55});
var K=new qx.ui.form.TextField(c);
this.getRoot().add(K,{left:140,top:55});
S.addListener(e,function(){var X=G+K.getValue()+a;

if(V.getUrl()==X){V.reload();
}else{V.setUrl(X);
}},this);
K.addListener(B,function(Y){if(Y.getKeyIdentifier()==F){S.execute();
}},this);
var O=new qx.ui.groupbox.GroupBox(p);
this.getRoot().add(O,{left:320,top:60});
O.setWidth(300);
O.setHeight(220);
O.setLayout(new qx.ui.layout.Grid(0,5));
O.add(new qx.ui.basic.Label(m),{row:0,column:0});
O.add(new qx.ui.basic.Label(o),{row:1,column:0});
O.add(new qx.ui.basic.Label(w),{row:2,column:0});
O.add(new qx.ui.basic.Label(s),{row:3,column:0});
var name=new qx.ui.basic.Label();
O.add(name,{row:0,column:1});
var location=new qx.ui.basic.Label();
O.add(location,{row:1,column:1});
var T=new qx.ui.basic.Label();
T.setRich(true);
T.setWidth(150);
O.add(T,{row:2,column:1});
var W=new qx.ui.basic.Label();
W.setRich(true);
O.add(W,{row:3,column:1});
var L=new qx.data.controller.Object();
L.addTarget(name,j,v);
L.addTarget(location,j,u);
L.addTarget(T,j,g);
L.addTarget(W,j,d);

if(!qx.core.Variant.isSet(f,b)){O.add(new qx.ui.basic.Label(x),{row:4,column:0});
var Q=new qx.ui.basic.Image();
O.add(Q,{row:4,column:1});
L.addTarget(Q,d,H);
}R.bind(q,L,h);
...
U.setWidth(260);
U.setValue(r+D);
this.getRoot().add(U,{left:10,top:290});
},configureItem:function(bb){bb.setRich(true);
bb.getChildControl(i).setWidth(50);
bb.getChildControl(i).setHeight(50);
bb.getChildControl(i).setScale(true);
}}});
qx.Class.define(C,{extend:qx.data.store.Json,statics:{saveResult:function(ba){this.__Lp=ba;
}},construct:function(bc){var bd=G+bc+a;
arguments.callee.base.call(this,bd);
},members:{_createRequest:function(I){var J=new qx.io.ScriptLoader();
I+=n;
J.load(I,function(bf){this.__Lq();
},this);
},__Lq:function(){var be=demobrowser.demo.data.store.Twitter.__Lp;

if(be==undefined){this.setState(t);
return;
}this._marshaler.toClass(be);
this.setModel(this._marshaler.toModel(be));
