...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.bom.Carousel","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Carousel-qx.theme.Modern.f24be099d0d1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var s="carousel",r="F",q="G",p="entry",o="D",n="E",m="#00FF00",l="#FF0000",k="B",j="C",c="A",h="demobrowser.demo.bom.Carousel",f="carousel-entry",b="zIndex",a="0px",e="left",d="background-color",g="px";qx.Class.define(h,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var v=[c,k,j,o,n,r,q];var x=30;var D=40;var t=l;var u=m;var A=30;var z=document.getElementById(s);var w=[];for(var i=0;i<v.length;i++){w[i]=qx.bom.Template.get(f,{data:v[i]});qx.bom.element.Class.add(w[i],p);qx.dom.Element.insertEnd(w[i],z);}
;for(var i=0;i<w.length;i++){qx.bom.element.Style.set(w[i],e,(i*x-A)+g);}
;var C=Math.floor(v.length/2)+1;var B=w[C-1];var y=(D-x)/2;qx.bom.element.Style.setStyles(B,{left:(parseInt(qx.bom.element.Style.get(B,e))-y)+g,backgroundColor:u,width:D+g,height:D+g,top:a,zIndex:10});window.setInterval(function(){for(var i=0;i<w.length;i++){var E=w[i];var frames={'0':{},'100':{}};var F=parseInt(qx.bom.element.Style.get(E,e))-x;if(F<-A){F=(w.length-1)*x-A;frames[0].opacity=0;frames[99]={opacity:0};frames[100].opacity=1;}
;if(i==C){frames[0][d]=t;frames[100][d]=u;frames[100].width=D+g;frames[100].height=D+g;frames[100].top=a;frames[100].left=(F-y)+g;qx.bom.element.Style.set(E,b,10);}
else if(i==(C+w.length-1)%w.length){frames[0][d]=u;frames[100][d]=t;frames[100].width=x+g;frames[100].height=x+g;frames[100].top=y+g;frames[100].left=(F+y)+g;qx.bom.element.Style.set(E,b,1);}
else {frames[100].left=F+g;}
;qx.bom.element.Animation.animate(E,{duration:500,keep:100,keyFrames:frames});}
;C=(C+1)%w.length;}
,1000);}
}});}
)();
(function(){var r="view,partials,stack,lookup,escapeHTML,renderSection,render",q="/",p="^",o='\n  return function () {',m="&gt;",l=';',k='\nbuffer += lookup("',h='\nif (partial) {',g='&#39;',f="div",bA='\nvar name = "',bz="\nbuffer += renderSection(name,stack,callback);",by="\nbuffer += renderSection(name,stack,callback,true);",bx='\n}',bw="&lt;",bv='",stack,"");',bu='"];',bt="Section name may not be empty",bs="qx.debug",br='\nline = ',z="#",A='Section "',x="}",y="\r",v="!",w=":",t="qx.bom.Template",u="&amp;",H='\n    var buffer = "";',I='Section named "',U='\n  };',Q='\n  buffer += render(partial,stack[stack.length - 1],partials);',bd="0.5.0-dev",X='Tag "',bn="\nvar line = 1;",bi='"',M="}}",bq=" >> ",bp="\xA0",bo="    ",L='\nbuffer += escapeHTML(lookup("',O='\n    return buffer;',P='\n})();',S='\nvar callback = (function () {',V='" was never opened',Y="\ntry {",bf="\\n",bk='\nvar partial = partials["',B="mustache.js",C="&",N="{{",bc='var buffer = "";',bb="<template>",ba="[object Array]",bh="\n} catch (e) { throw {error: e, line: line}; }",bg="{",W='",stack,""));',be='&quot;',a="Please use the method 'render' instead.",bj="\nreturn buffer;",D=">",E="\\",R='" was not closed properly',b="=",d=".",K="undefined",F="object",G="function",J="\n",T='\nbuffer += "',bm='";',bl="";qx.Bootstrap.define(t,{statics:{version:null,render:null,toHtml:function(bB,bC,bD){if(qx.core.Environment.get(bs)){qx.log.Logger.deprecatedMethodWarning(arguments.callee,a);}
;return this.render(bB,bC,bD);}
,get:function(bE,bF,bG){var bJ=document.getElementById(bE);var bI=bJ.innerHTML;bI=this.render(bI,bF,bG);if(bI.search(/<|>/)===-1){return bI;}
;var bH=qx.dom.Element.create(f);bH.innerHTML=bI;return bH.children[0];}
}});(function(){var bK=(typeof module!==K&&module.exports)||{};(function(bL){bL.name=B;bL.version=bd;bL.tags=[N,M];bL.parse=bP;bL.compile=cg;bL.render=bO;bL.clearCache=bR;bL.to_html=function(cj,ck,cl,cm){var cn=bO(cj,ck,cl);if(typeof cm===G){cm(cn);}
else {return cn;}
;}
;var bU=Object.prototype.toString;var ce=Array.isArray;var ca=Array.prototype.forEach;var bX=String.prototype.trim;var bY;if(ce){bY=ce;}
else {bY=function(co){return bU.call(co)===ba;}
;}
;var cc;if(ca){cc=function(cp,cq,cr){return ca.call(cp,cq,cr);}
;}
else {cc=function(cs,ct,cu){for(var i=0,cv=cs.length;i<cv;++i){ct.call(cu,cs[i],i,cs);}
;}
;}
;var ci=/^\s*$/;function bW(cw){return ci.test(cw);}
;var bM;if(bX){bM=function(cx){return cx==null?bl:bX.call(cx);}
;}
else {var cf,cb;if(bW(bp)){cf=/^\s+/;cb=/\s+$/;}
else {cf=/^[\s\xA0]+/;cb=/[\s\xA0]+$/;}
;bM=function(cy){return cy==null?bl:String(cy).replace(cf,bl).replace(cb,bl);}
;}
;var bS={"&":u,"<":bw,">":m,'"':be,"'":g};function cd(cz){return String(cz).replace(/&(?!\w+;)|[<>"']/g,function(s){return bS[s]||s;}
);}
;function ch(e,cA,cB,cC){cC=cC||bb;var cE=cA.split(J),cG=Math.max(cB-3,0),cD=Math.min(cE.length,cB+3),cH=cE.slice(cG,cD);var c;for(var i=0,cF=cH.length;i<cF;++i){c=i+cG+1;cH[i]=(c===cB?bq:bo)+cH[i];}
;e.template=cA;e.line=cB;e.file=cC;e.message=[cC+w+cB,cH.join(J),bl,e.message].join(J);return e;}
;function bT(name,cI,cJ){if(name===d){return cI[cI.length-1];}
;var cN=name.split(d);var cL=cN.length-1;var cK=cN[cL];var cM,cO,i=cI.length,j,cP;while(i){cP=cI.slice(0);cO=cI[--i];j=0;while(j<cL){cO=cO[cN[j++]];if(cO==null){break;}
;cP.push(cO);}
;if(cO&&typeof cO===F&&cK in cO){cM=cO[cK];break;}
;}
;if(typeof cM===G){cM=cM.call(cP[cP.length-1]);}
;if(cM==null){return cJ;}
;return cM;}
;function bN(name,cQ,cR,cS){var cT=bl;var cW=bT(name,cQ);if(cS){if(cW==null||cW===false||(bY(cW)&&cW.length===0)){cT+=cR();}
;}
else if(bY(cW)){cc(cW,function(cX){cQ.push(cX);cT+=cR();cQ.pop();}
);}
else if(typeof cW===F){cQ.push(cW);cT+=cR();cQ.pop();}
else if(typeof cW===G){var cU=cQ[cQ.length-1];var cV=function(cY){return bO(cY,cU);}
;cT+=cW.call(cU,cR(),cV)||bl;}
else if(cW){cT+=cR();}
;;;;return cT;}
;function bP(da,db){db=db||{};var dq=db.tags||bL.tags,dn=dq[0],de=dq[dq.length-1];var dc=[bc,bn,Y,T];var dA=[],dj=false,dx=false;var dw=function(){if(dj&&!dx&&!db.space){while(dA.length){dc.splice(dA.pop(),1);}
;}
else {dA=[];}
;dj=false;dx=false;}
;var dB=[],dg,df,di;var dl=function(dC){dq=bM(dC).split(/\s+/);df=dq[0];di=dq[dq.length-1];}
;var dd=function(dD){dc.push(bm,dg,bk+bM(dD)+bu,h,Q,bx,T);}
;var ds=function(dE,dF){var name=bM(dE);if(name===bl){throw ch(new Error(bt),da,dv,db.file);}
;dB.push({name:name,inverted:dF});dc.push(bm,dg,bA+name+bm,S,o,H,T);}
;var dk=function(dG){ds(dG,true);}
;var dr=function(dH){var name=bM(dH);var dI=dB.length!=0&&dB[dB.length-1].name;if(!dI||name!=dI){throw ch(new Error(I+name+V),da,dv,db.file);}
;var dJ=dB.pop();dc.push(bm,O,U,P);if(dJ.inverted){dc.push(by);}
else {dc.push(bz);}
;dc.push(T);}
;var dh=function(dK){dc.push(bm,dg,k+bM(dK)+bv,T);}
;var dy=function(dL){dc.push(bm,dg,L+bM(dL)+W,T);}
;var dv=1,c,dz;for(var i=0,dt=da.length;i<dt;++i){if(da.slice(i,i+dn.length)===dn){i+=dn.length;c=da.substr(i,1);dg=br+dv+l;df=dn;di=de;dj=true;switch(c){case v:i++;dz=null;break;case b:i++;de=b+de;dz=dl;break;case D:i++;dz=dd;break;case z:i++;dz=ds;break;case p:i++;dz=dk;break;case q:i++;dz=dr;break;case bg:de=x+de;case C:i++;dx=true;dz=dh;break;default:dx=true;dz=dy;};var du=da.indexOf(de,i);if(du===-1){throw ch(new Error(X+dn+R),da,dv,db.file);}
;var dm=da.substring(i,du);if(dz){dz(dm);}
;var n=0;while(~(n=dm.indexOf(J,n))){dv++;n++;}
;i=du+de.length-1;dn=df;de=di;}
else {c=da.substr(i,1);switch(c){case bi:case E:dx=true;dc.push(E+c);break;case y:break;case J:dA.push(dc.length);dc.push(bf);dw();dv++;break;default:if(bW(c)){dA.push(dc.length);}
else {dx=true;}
;dc.push(c);};}
;}
;if(dB.length!=0){throw ch(new Error(A+dB[dB.length-1].name+R),da,dv,db.file);}
;dw();dc.push(bm,bj,bh);var dp=dc.join(bl).replace(/buffer \+= "";\n/g,bl);if(db.debug){if(typeof console!=K&&console.log){console.log(dp);}
else if(typeof print===G){print(dp);}
;}
;return dp;}
;function bV(dM,dN){var dP=r;var dO=bP(dM,dN);var dQ=new Function(dP,dO);return function(dR,dS){dS=dS||{};var dT=[dR];try{return dQ(dR,dS,dT,bT,cd,bN,bO);}
catch(e){throw ch(e.error,dM,e.line,dN.file);}
;}
;}
;var bQ={};function bR(){bQ={};}
;function cg(dU,dV){dV=dV||{};if(dV.cache!==false){if(!bQ[dU]){bQ[dU]=bV(dU,dV);}
;return bQ[dU];}
;return bV(dU,dV);}
;function bO(dW,dX,dY){return cg(dW)(dX,dY);}
;}
)(bK);qx.bom.Template.version=bK.version;qx.bom.Template.render=bK.render;}
)();}
)();
...
(function(){var k="repeat",j="timing",i="end",h="Anni",g="The key 'reverse' is deprecated: Please use the method ",f="keep",d=":",c="} ",b="origin",a="forwards",A="end-event",z="reverse",y="css.animation",x="ms ",w="% {",v="'animateReverse' instead.",u="linear",t=";",s="qx.bom.element.AnimationCss",r="keyframes",p="fill-mode",q="alternate",n="qx.debug",o="name",l="",m=" ";qx.Bootstrap.define(s,{statics:{__sv:null,__sw:h,__bV:0,__sx:{},__sy:{"scale":true,"rotate":true,"skew":true,"translate":true},__sz:qx.core.Environment.get(y),animateReverse:function(B,C,D){return this._animate(B,C,D,true);}
,animate:function(E,F,G){return this._animate(E,F,G,false);}
,_animate:function(H,I,J,K){this.__sC(I);if(I.hasOwnProperty(z)){K=I.reverse;if(qx.core.Environment.get(n)){qx.log.Logger.warn(g+v);qx.log.Logger.trace();}
;}
;if(qx.core.Environment.get(n)){this.__BQ(I);}
;var N=I.keep;if(N!=null&&(K||(I.alternate&&I.repeat%2==0))){N=100-N;}
;if(!this.__sv){this.__sv=qx.bom.Stylesheet.createElement();}
;var M=I.keyFrames;if(J==undefined){J=I.duration;}
;if(this.__sz!=null){var name=this.__sD(M,K);var L=name+m+J+x+I.repeat+m+I.timing+m+(I.alternate?q:l);var O=this.__sz[A];qx.bom.Event.addNativeListener(H,O,this.__sA);H.style[qx.lang.String.camelCase(this.__sz[o])]=L;if(N&&N==100&&this.__sz[p]){H.style[this.__sz[p]]=a;}
;}
;var P=new qx.bom.element.AnimationHandle();P.desc=I;P.el=H;P.keep=N;H.$$animation=P;if(I.origin!=null){qx.bom.element.Transform.setOrigin(H,I.origin);}
;if(this.__sz==null){window.setTimeout(function(){qx.bom.element.AnimationCss.__sA({target:H});}
,0);}
;return P;}
,__sA:function(e){var Q=e.target;var R=Q.$$animation;if(!R){return;}
;var T=R.desc;if(qx.bom.element.AnimationCss.__sz!=null){var S=qx.lang.String.camelCase(qx.bom.element.AnimationCss.__sz[o]);Q.style[S]=l;qx.bom.Event.removeNativeListener(Q,qx.bom.element.AnimationCss.__sz[o],qx.bom.element.AnimationCss.__sA);}
;if(T.origin!=null){qx.bom.element.Transform.setOrigin(Q,l);}
;qx.bom.element.AnimationCss.__sB(Q,T.keyFrames[R.keep]);Q.$$animation=null;R.el=null;R.ended=true;R.emit(i,Q);}
,__sB:function(U,V){var X;for(var W in V){if(W in qx.bom.element.AnimationCss.__sy){if(!X){X={};}
;X[W]=V[W];}
else {U.style[qx.lang.String.camelCase(W)]=V[W];}
;}
;if(X){qx.bom.element.Transform.transform(U,X);}
;}
,__sC:function(Y){if(!Y.hasOwnProperty(q)){Y.alternate=false;}
;if(!Y.hasOwnProperty(f)){Y.keep=null;}
;if(!Y.hasOwnProperty(k)){Y.repeat=1;}
;if(!Y.hasOwnProperty(j)){Y.timing=u;}
;if(!Y.hasOwnProperty(b)){Y.origin=null;}
;}
,__BQ:qx.core.Environment.select("qx.debug",{"true":function(ba){var bb=["origin","duration","keep","keyFrames","repeat","timing","alternate","reverse"];for(var name in ba){if(!(bb.indexOf(name)!=-1)){qx.Bootstrap.warn("Unknown key '"+name+"' in the animation description.");}
;}
;if(ba.keyFrames==null){qx.Bootstrap.warn("No 'keyFrames' given > 0");}
else {for(var bc in ba.keyFrames){if(bc<0||bc>100){qx.Bootstrap.warn("Keyframe position needs to be between 0 and 100");}
;}
;}
;}
,"default":null}),__sD:function(frames,bd){var bg=l;for(var bj in frames){bg+=(bd?-(bj-100):bj)+w;var bf=frames[bj];var bi;for(var be in bf){if(be in this.__sy){if(!bi){bi={};}
;bi[be]=bf[be];}
else {bg+=be+d+bf[be]+t;}
;}
;if(bi){bg+=qx.bom.element.Transform.getCss(bi);}
;bg+=c;}
;if(this.__sx[bg]){return this.__sx[bg];}
;var name=this.__sw+this.__bV++;var bh=this.__sz[r]+m+name;qx.bom.Stylesheet.addRule(this.__sv,bh,bg);this.__sx[bg]=name;return name;}
}});}
)();
...
(function(){var g="Element",f="qx.bom.element.AnimationHandle",e="play-state",d="paused",c="running",b="",a="css.animation";qx.Bootstrap.define(f,{extend:qx.event.Emitter,construct:function(){var h=qx.core.Environment.get(a);this.__sE=h&&h[e];this.__sF=true;}
,events:{"end":g},members:{__sE:null,__sF:false,__sG:false,isPlaying:function(){return this.__sF;}
,isEnded:function(){return this.__sG;}
,pause:function(){if(this.el){this.el.style[this.__sE]=d;this.el.$$animation.__sF=false;if(this.animationId&&qx.bom.element.AnimationJs){qx.bom.element.AnimationJs.pause(this);}
;}
;}
,play:function(){if(this.el){this.el.style[this.__sE]=c;this.el.$$animation.__sF=true;if(this.i!=undefined&&qx.bom.element.AnimationJs){qx.bom.element.AnimationJs.play(this);}
;}
;}
,stop:function(){if(this.el&&qx.core.Environment.get(a)){this.el.style[this.__sE]=b;this.el.style[qx.core.Environment.get(a).name]=b;this.el.$$animation.__sF=false;this.el.$$animation.__sG=true;}
...
,getCss:function(G){var I=this.__st(G);if(this.__ss!=null){var H=this.__ss[l];return qx.lang.String.hyphenate(H)+n+I+o;}
;return a;}
,setOrigin:function(J,K){if(this.__ss!=null){J.style[this.__ss[f]]=K;}
;}
,getOrigin:function(L){if(this.__ss!=null){return L.style[this.__ss[f]];}
;return a;}
,setStyle:function(M,N){if(this.__ss!=null){M.style[this.__ss[e]]=N;}
;}
,getStyle:function(O){if(this.__ss!=null){return O.style[this.__ss[e]];}
;return a;}
,setPerspective:function(P,Q){if(this.__ss!=null){P.style[this.__ss[k]]=Q;}
;}
,getPerspective:function(R){if(this.__ss!=null){return R.style[this.__ss[k]];}
;return a;}
,setPerspectiveOrigin:function(S,T){if(this.__ss!=null){S.style[this.__ss[h]]=T;}
;}
,getPerspectiveOrigin:function(U){if(this.__ss!=null){var W=U.style[this.__ss[h]];if(W!=a){return W;}
else {var X=U.style[this.__ss[h]+c];var V=U.style[this.__ss[h]+b];if(X!=a){return X+s+V;}
;}
;}
;return a;}
,setBackfaceVisibility:function(Y,ba){if(this.__ss!=null){Y.style[this.__ss[m]]=ba?d:q;}
;}
,getBackfaceVisibility:function(bb){if(this.__ss!=null){return bb.style[this.__ss[m]]==d;}
;return true;}
,__st:function(bc){var bf=a;for(var be in bc){var bd=bc[be];if(qx.Bootstrap.isArray(bd)){for(var i=0;i<bd.length;i++){if(bd[i]==undefined){continue;}
;bf+=be+this.__sr[i]+j;bf+=bd[i];bf+=g;}
;}
...
(function(){var n="mm",m="pt",l="pc",k="px",h="end",g="string",f="The key 'reverse' is deprecated: Please use the method ",e="0",d="%",c="qx.debug",D="qx.bom.element.AnimationJs",C="#",B="ease-in",A="ex",z="cm",w="infinite",v="em",u="reverse",t="ease-in-out",s="in",q="'animateReverse' instead.",r="linear",o="ease-out",p="";qx.Bootstrap.define(D,{statics:{__sH:30,__sI:[d,s,z,n,v,A,m,l,k],animate:function(E,F,G){return this._animate(E,F,G,false);}
,animateReverse:function(H,I,J){return this._animate(H,I,J,true);}
,_animate:function(K,L,M,N){if(K.$$animation){return;}
;if(L.hasOwnProperty(u)){N=L.reverse;if(qx.core.Environment.get(c)){qx.log.Logger.warn(f+q);qx.log.Logger.trace();}
;}
;if(M==undefined){M=L.duration;}
;var R=L.keyFrames;var P=this.__sP(R);var Q=this.__sO(M,P);var S=parseInt(M/Q,10);this.__sJ(R,K);var T=this.__sK(S,Q,P,R,M,L.timing);var O=new qx.bom.element.AnimationHandle();if(N){T.reverse();O.reverse=true;}
;O.desc=L;O.el=K;O.delta=T;O.stepTime=Q;O.steps=S;K.$$animation=O;O.i=0;O.initValues={};O.repeatSteps=this.__sM(S,L.repeat);return this.play(O);}
,__sJ:function(U,V){var W={};for(var ba in U){for(var name in U[ba]){if(W[name]==undefined){var X=U[ba][name];if(typeof X==g){W[name]=X.substring((parseInt(X,10)+p).length,X.length);}
else {W[name]=p;}
;}
;}
;}
;for(var ba in U){var Y=U[ba];for(var name in W){if(Y[name]==undefined){if(window.getComputedStyle){Y[name]=getComputedStyle(V,null)[name];}
else {Y[name]=V.style[name];}
;if(Y[name]==p&&this.__sI.indexOf(W[name])!=-1){Y[name]=e+W[name];}
;}
;}
;}
;}
,__sK:function(bb,bc,bd,be,bf,bg){var bo=new Array(bb);var bq=1;bo[0]=be[0];var bi=be[0];var bk=be[bd[bq]];for(var i=1;i<bo.length;i++){if(i*bc/bf*100>bd[bq]){bi=bk;bq++;bk=be[bd[bq]];}
;bo[i]={};for(var name in bk){var bp=bk[name]+p;if(bp.charAt(0)==C){var bj=qx.util.ColorUtil.cssStringToRgb(bi[name]);var bn=qx.util.ColorUtil.cssStringToRgb(bp);var bh=[];for(var j=0;j<bj.length;j++){var bl=bj[j]-bn[j];bh[j]=parseInt(bj[j]-bl*this.__sL(bg,i/bb),10);}
;bo[i][name]=qx.util.ColorUtil.rgbToHexString(bh);}
else if(!isNaN(parseInt(bp,10))){var bm=bp.substring((parseInt(bp,10)+p).length,bp.length);var bl=parseFloat(bp)-parseFloat(bi[name]);bo[i][name]=(parseFloat(bi[name])+bl*this.__sL(bg,i/bb))+bm;}
else {bo[i][name]=bi[name]+p;}
;}
;}
;bo[bo.length-1]=be[100];return bo;}
,play:function(br){var bs=window.setInterval(function(){br.repeatSteps--;var bt=br.delta[br.i%br.steps];if(br.i==0){for(var name in bt){if(br.initValues[name]==undefined){if(qx.bom.element.Style){br.initValues[name]=qx.bom.element.Style.get(br.el,qx.lang.String.camelCase(name));}
else {br.initValues[name]=br.el.style[qx.lang.String.camelCase(name)];}
;}
;}
;}
;qx.bom.element.AnimationJs.__sN(br.el,bt);br.i++;if(br.i%br.steps==0){if(br.desc.alternate){br.delta.reverse();}
;}
;if(br.repeatSteps<0){qx.bom.element.AnimationJs.stop(br);}
;}
,br.stepTime);br.animationId=bs;return br;}
,pause:function(bu){window.clearInterval(bu.animationId);bu.animationId=null;}
,stop:function(bv){var bz=bv.desc;var bw=bv.el;var bx=bv.initValues;if(bv.animationId){window.clearInterval(bv.animationId);}
;if(bw==undefined){return;}
;var by=bz.keep;if(by!=undefined){if(bv.reverse||(bz.alternate&&bz.repeat&&bz.repeat%2==0)){by=100-by;}
;this.__sN(bw,bz.keyFrames[by]);}
else {this.__sN(bw,bx);}
;bw.$$animation=null;bv.el=null;bv.ended=true;bv.animationId=null;bv.emit(h,bw);}
,__sL:function(bA,x){if(bA==B){var a=[3.1223e-7,0.0757,1.2646,-0.167,-0.4387,0.2654];}
else if(bA==o){var a=[-7.0198e-8,1.652,-0.551,-0.0458,0.1255,-0.1807];}
else if(bA==r){return x;}
else if(bA==t){var a=[2.482e-7,-0.2289,3.3466,-1.0857,-1.7354,0.7034];}
else {var a=[-0.0021,0.2472,9.8054,-21.6869,17.7611,-5.1226];}
;;;var y=0;for(var i=0;i<a.length;i++){y+=a[i]*Math.pow(x,i);}
;return y;}
,__sM:function(bB,bC){if(bC==undefined){return bB;}
;if(bC==w){return Number.MAX_VALUE;}
;return bB*bC;}
,__sN:function(bD,bE){for(var bF in bE){if(bE[bF]===undefined){continue;}
;var name=qx.lang.String.camelCase(bF);if(qx.bom.element.Style){qx.bom.element.Style.set(bD,name,bE[bF]);}
else {bD.style[name]=bE[bF];}
;}
;}
,__sO:function(bG,bH){var bJ=100;for(var i=0;i<bH.length-1;i++){bJ=Math.min(bJ,bH[i+1]-bH[i]);}
;var bI=bG*bJ/100;while(bI>this.__sH){bI=bI/2;}
;return Math.round(bI);}
,__sP:function(bK){var bL=qx.Bootstrap.getKeys(bK);for(var i=0;i<bL.length;i++){bL[i]=parseInt(bL[i],10);}
;bL.sort(function(a,b){return a-b;}
);return bL;}
}});}
)();
