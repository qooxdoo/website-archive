...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.progressive.ProgressiveTable_VarRowHeight","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.progressive.ProgressiveTable_VarRowHeight-qx.theme.Simple.50037f9679c0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="demobrowser.demo.progressive.ProgressiveTable_VarRowHeight",k="format-justify-left.png",h="end",g="Found [",f="Id",e="No attributes specified.",d="TALL ICON",c="1*",b=", end=",a="This, too, is small.",C="This, too, is tall.",B="",A="Searching for [",z="]",y=" not found",x="small icon",w="demobrowser/demo/icons/",v="I am small.",u="red",t="Image data for ",q="I am tall.",s="].  loadCount=",o="multimedia-player.png",p="Image",m="Note variable row heights in this table.",n="row";qx.Class.define(l,{extend:qx.application.Standalone,members:{__QJ:null,main:function(){qx.application.Standalone.prototype.main.call(this);var H=w;this.imageData=[{url:H+k,height:4,tooltip:x},{url:H+o,height:4,tooltip:d},B];this.__QJ=0;var F=qx.util.AliasManager.getInstance();var E=qx.util.ResourceManager.getInstance();var D;var G;for(var i=0;i<this.imageData.length;i++){if(!this.imageData[i]){continue;}
;D=F.resolve(this.imageData[i].url);G=E.toUri(D);this.imageData[i].url=G;++this.__QJ;qx.io.ImageLoader.load(G,function(I,J){this.info(A+I+z);for(var j=0;j<this.imageData.length;j++){if(this.imageData[j].url==I){this.imageData[j].height=J.height;--this.__QJ;this.debug(g+this.imageData[j].url+s+this.__QJ+b+this.imageData.length);if(this.__QJ==0){this.doLoad();}
;return;}
;}
;throw new Error(t+I+y);}
,this);}
;if(this.__QJ==0){this.doLoad();}
;}
...
(function(){var f="Unrecognized key: ",e="qx.ui.progressive.renderer.table.Widths",d="minWidth",c="width",b="maxWidth",a="Column number out of range";qx.Class.define(e,{extend:qx.core.Object,construct:function(g){qx.core.Object.call(this);this.__MA=[];for(var i=0;i<g;i++){this.__MA[i]=new qx.ui.core.ColumnData();}
;}
,members:{__MA:null,getData:function(){return this.__MA;}
...
;}
,setWidth:function(l,m){if(l>this.__MA.length-1||l<0){throw new Error(a);}
;this.__MA[l].setColumnWidth(m);}
,setMinWidth:function(n,o){if(n>this.__MA.length-1||n<0){throw new Error(a);}
;this.__MA[n].setMinWidth(o);}
,setMaxWidth:function(p,q){if(p>this.__MA.length-1||p<0){throw new Error(a);}
;this.__MA[p].setMaxWidth(q);}
}});}
)();
...
,members:{__nM:null,renderLayout:function(f,top,g,h){this.__nM=g;}
,getComputedWidth:function(){return this.__nM;}
...
,members:{__Ju:null,join:function(c){this.__Ju=c;}
...
(function(){var f="resize",d="progressive-table-header",c="_layout",b="qx.ui.progressive.headfoot.TableHeading",a="progressive-table-header-cell";qx.Class.define(b,{extend:qx.ui.progressive.headfoot.Abstract,construct:function(g,h){qx.ui.progressive.headfoot.Abstract.call(this);this.__Jr=g;var k=g.getData();this.__FE=[];for(var i=0;i<k.length;i++){var l=new qx.ui.basic.Atom(h[i]);l.setAppearance(a);this.add(l);this.__FE[i]=l;}
;var j=new qx.ui.core.Widget();j.set({height:16,appearance:a,minWidth:0,width:0});this.add(j,{flex:1});this.addListener(f,this._resizeColumns,this);this.__nO=new qx.ui.layout.HBox();this.__nO.connectToWidget(this);}
,properties:{appearance:{refine:true,init:d}},members:{__Jr:null,__Ku:null,__FE:null,__nO:null,join:function(m){qx.ui.progressive.headfoot.Abstract.prototype.join.call(this,m);}
,getLayoutChildren:function(){if(this.__Ku){return this.__Jr.getData();}
else {return qx.ui.progressive.headfoot.Abstract.prototype.getLayoutChildren.call(this);}
;}
,_resizeColumns:function(e){var n=this.getBounds().width-qx.bom.element.Overflow.getScrollbarWidth();this.__Ku=true;this.__nO.renderLayout(n,100);this.__Ku=false;var o=this.__Jr.getData();for(var i=0;i<o.length;i++){this.__FE[i].setWidth(o[i].getComputedWidth());}
;}
},destruct:function(){this.__Jr=this.__FE=null;this._disposeObjects(c);}
});}
)();
(function(){var n="progressive-progressbar-indicator-undone",m="progress",l="%",k="progressive-progressbar-indicator-done",j="progressive-progressbar-percent-text",i="renderStart",h="progressive-progressbar-background",g="0%",f="__Mh",d="qx.ui.progressive.headfoot.Progress",a="__Mg",c="progressive-progressbar-percent-background",b="renderEnd";qx.Class.define(d,{extend:qx.ui.progressive.headfoot.Abstract,construct:function(o,p){qx.ui.progressive.headfoot.Abstract.call(this);this.setHeight(16);this.setPadding(0);this.__Jv={};var q=qx.theme.manager.Color.getInstance();this.__Jv.background=q.resolve(h);this.__Jv.indicatorDone=q.resolve(k);this.__Jv.indicatorUndone=q.resolve(n);this.__Jv.percentBackground=q.resolve(c);this.__Jv.percentText=q.resolve(j);this.set({backgroundColor:this.__Jv.background});this.__Mg=new qx.ui.core.Widget();this.__Mg.set({width:0,backgroundColor:this.__Jv.indicatorDone});this.add(this.__Mg);var r=new qx.ui.core.Widget();r.set({backgroundColor:this.__Jv.indicatorUndone});this.add(r,{flex:1});this.__Mh=new qx.ui.basic.Atom(g);this.__Mh.set({width:100,backgroundColor:this.__Jv.percentBackground,textColor:this.__Jv.percentText});this.add(this.__Mh);this.exclude();}
,members:{__Mi:null,__Jv:null,__Mg:null,__Mh:null,join:function(s){qx.ui.progressive.headfoot.Abstract.prototype.join.call(this,s);s.addListener(i,function(e){this.__Mi=e.getData().initial;this.show();}
,this);s.addListener(m,function(e){var v=1.0-(e.getData().remaining/this.__Mi);var w=this.getBounds();if(w){var t=Math.floor((w.width-this.__Mh.getBounds().width)*v);var u=Math.floor(v*100)+l;if(!isNaN(t)){this.__Mg.setMinWidth(t);this.__Mh.setLabel(u);}
;}
;}
,this);s.addListener(b,function(e){this.exclude();}
,this);}
},destruct:function(){this.__Jv=null;this._disposeObjects(a,f);}
});}
)();
(function(){var e="applyStructure() is abstract",d="abstract",c="qx.ui.progressive.structure.Abstract",b="overflowY",a="auto";qx.Class.define(c,{type:d,extend:qx.core.Object,construct:function(f){qx.core.Object.call(this);if(!f){this.__EW=new qx.ui.core.Widget();this.__AG=this.__EW;}
else {this.__EW=null;this.__AG=f;}
;this.__AG.getContentElement().setStyle(b,a);}
,members:{__EW:null,__AG:null,applyStructure:function(g){throw new Error(e);}
,getPane:function(){return this.__AG;}
},destruct:function(){if(this.__EW){this.__EW.dispose();}
;this.__EW=this.__AG=null;}
});}
)();
(function(){var a="qx.ui.progressive.structure.Default";qx.Class.define(a,{extend:qx.ui.progressive.structure.Abstract,construct:function(b,c,d){qx.ui.progressive.structure.Abstract.call(this,d);if(!b){this.__Kk=new qx.ui.progressive.headfoot.Null();this.__oV=this.__Kk;}
else {this.__Kk=null;this.__oV=b;}
;if(!c){this.__Kl=new qx.ui.progressive.headfoot.Null();this.__Km=this.__Kl;}
else {this.__Kl=null;this.__Km=c;}
;}
,members:{__oV:null,__Km:null,__Kk:null,__Kl:null,applyStructure:function(e){this.__oV.join(e);this.__Km.join(e);e.add(this.__oV);e.add(this.getPane(),{flex:1});e.add(this.__Km);}
,getHeader:function(){return this.__oV;}
,getFooter:function(){return this.__Km;}
},destruct:function(){if(this.__Kk){this.__Kk.dispose();this.__Kk=null;}
;if(this.__Kl){this.__Kl.dispose();this.__Kl=null;}
;this.__oV=this.__Km=null;}
});}
)();
...
(function(){var q="_applyDataModel",p="Boolean",o="Render time: ",n="qx.ui.progressive.model.Abstract",m="ms",l="progress",k="white",j="No existing renderer named ",h="qx.ui.progressive.Progressive",g="progressDetail",c="renderEnd",f="dataAvailable",d="renderStart",b="Integer",a="qx.event.type.Data";qx.Class.define(h,{extend:qx.ui.container.Composite,construct:function(r){qx.ui.container.Composite.call(this,new qx.ui.layout.VBox());this.__FF={};this.set({backgroundColor:k});if(!r){r=new qx.ui.progressive.structure.Default();}
;this.__Lu=r;r.applyStructure(this);this.__Lv=false;this.__Lw=false;this.__Lx=0;}
,events:{"renderStart":a,"renderEnd":a,"progress":a,"progressDetail":a},properties:{dataModel:{check:n,apply:q},batchSize:{check:b,init:20},flushWidgetQueueAfterBatch:{check:p,init:false},interElementTimeout:{check:b,init:0}},members:{__FF:null,__Lw:null,__Ly:null,__Lx:null,__Lv:null,__Lu:null,getStructure:function(){return this.__Lu;}
,addRenderer:function(name,s){this.__FF[name]=s;s.join(this,name);}
,removeRenderer:function(name){if(!this.__FF[name]){throw new Error(j+name);}
;delete this.__FF[name];}
,render:function(){if(this.__Lw){return;}
;this.__Lw=true;var t=new qx.ui.progressive.State({progressive:this,model:this.getDataModel(),pane:this.__Lu.getPane(),batchSize:this.getBatchSize(),rendererData:this.__LA(),userData:{}});this.__Ly=new Date();if(this.__Lv){this.__Lx=t.getModel().getElementCount();this.fireDataEvent(d,{state:t,initial:this.__Lx});this.__Lz(t);}
else {qx.event.Timer.once(function(){this.__Lx=t.getModel().getElementCount();this.fireDataEvent(d,{state:t,initial:this.__Lx});this.__Lz(t);this.__Lv=true;}
,this,10);}
;}
,_applyDataModel:function(u,v){if(v){v.removeListener(f,this.__LB,this);v.dispose();}
;u.addListener(f,this.__LB,this);}
,__Lz:function(w){var x;var z;var y;for(var i=w.getBatchSize();i>0;i--){x=w.getModel().getNextElement();if(!x){this.debug(o+(new Date()-this.__Ly)+m);this.__Lw=false;this.fireDataEvent(c,w);w.dispose();return;}
;z=x.element;y=this.__FF[z.renderer];y.render(w,z);this.fireDataEvent(g,{initial:this.__Lx,remaining:x.remaining,element:z});}
;this.fireDataEvent(l,{initial:this.__Lx,remaining:x.remaining});if(this.getFlushWidgetQueueAfterBatch()){qx.ui.core.queue.Manager.flush();}
;qx.event.Timer.once(function(){this.__Lz(w);}
,this,this.getInterElementTimeout());}
,__LA:function(){var A={};for(var name in this.__FF){A[name]={};}
;return A;}
,__LB:function(e){this.__Lx=e.getData();this.render();}
},destruct:function(){for(var name in this.__FF){this.__FF[name].dispose();}
;this.__Ly=this.__FF=this.__Lu=null;}
});}
)();
...
(function(){var l="content",k="px",j="  font-size: 11px;",h="px ",g=';',f='',d="padding: ",c="end",b="div",a="  position: absolute;",bh="qx.tableResizeDebug",bg="  width : 100%;",bf="__columnData",be="Renderer is already joined to a Progressive.",bd="css.boxmodel",bc="  top: 0px;",bb="qx.debug",ba="  overflow:hidden;",Y=" {",X="  border-right:1px solid #f2f2f2;",s="__Jt",t="Invalid location: ",q="  cursor:default;",r="  font-family: 'Segoe UI', Corbel, Calibri, Tahoma, 'Lucida Sans Unicode', sans-serif;",o="-row {",p="resize",m="start",n="relative",w=": width=",x=':ellipsis;',F=':',D="  white-space:nowrap;",N="  padding : 0px 6px 0px 6px;",I="qx.ui.progressive.renderer.table.Row",T="progressive-table-row-background-odd",R="__nO",z="  height: 100%;",W="progressive-table-row-background-even",V="col ",U="  border-bottom:1px solid #eeeeee;",y="left: ",B="No existing renderer for column ",C="css.userselect.none",E=")",G="css.userselect",J="css.textoverflow",O="}",S="",u="width: ",v=" out of range (max: ",A="0px ",M="qx-progressive-",L="Column ",K="-row",Q="-col-",P="px;",H=".qx-progressive-";qx.Class.define(I,{extend:qx.ui.progressive.renderer.Abstract,construct:function(bi){qx.ui.progressive.renderer.Abstract.call(this);this.__Jr=bi;this.__Js={};this.__Jt=new qx.ui.progressive.renderer.table.cell.Default();this.__Ju=null;this.__Jv={};var bj=qx.theme.manager.Color.getInstance();this.__Jv.bgcol=[];this.__Jv.bgcol[0]=bj.resolve(W);this.__Jv.bgcol[1]=bj.resolve(T);this.__nO=new qx.ui.layout.HBox();this.__nO.connectToWidget(this);}
,statics:{__nd:null,__Jw:6,__Jx:a+bc+z+ba+(qx.core.Environment.get(J)?qx.lang.String.hyphenate(qx.core.Environment.get(J))+x:S)+D+X+U+N+q+j+r+(qx.core.Environment.get(G)?qx.lang.String.hyphenate(qx.core.Environment.get(G))+F+qx.core.Environment.get(C)+g:f)},properties:{defaultRowHeight:{init:16}},members:{__Ju:null,__BH:null,__Jy:null,__Jr:null,__Js:null,__Jt:null,__Jv:null,__nO:null,join:function(bk,name){if(this.__Ju){throw new Error(be);}
;this.__Ju=bk;this.__BH=name;var bm=qx.ui.progressive.renderer.table.Row;if(!bm.__nd){bm.__nd={};}
;var bl=bk.toHashCode();if(!bm.__nd[bl]){bm.__nd[bl]={rowstylesheet:null,cellstylesheet:[]};var bn=H+bl+o+bg+O;bm.__nd[bl].rowstylesheet=qx.bom.Stylesheet.createElement(bn);var bo=this.__Jr.getData();for(var i=0;i<bo.length;i++){var bn=H+bl+Q+i+Y+bm.__Jx+O;bm.__nd[bl].cellstylesheet[i]=qx.bom.Stylesheet.createElement(bn);}
;this.__Jy=bl;var bp=bk.getStructure().getPane();bp.addListener(p,this._resizeColumns,this);}
;}
,addRenderer:function(bq,br){var bs=this.__Jr.getData();if(bq<0||bq>=bs.length){throw new Error(L+bq+v+(bs.length-1)+E);}
;this.__Js[bq]=br;}
,removeRenderer:function(bt){var bu=this.__Jr.getData();if(bt<0||bt>=bu.length){throw new Error(L+bt+v+(bu.length-1)+E);}
;if(!this.__Js[bt]){throw new Error(B+bt);}
;delete this.__Js[bt];}
,render:function(bv,bw){var bG=bw.data;var bB=[];var bD;var bC;var bz=0;if(bv.getRendererData()[this.__BH].end===undefined){bv.getRendererData()[this.__BH]={end:0,start:1,rows:0,totalHeight:0};}
;var bF=document.createElement(b);for(var i=0;i<bG.length;i++){var bA=M+this.__Jy+Q+i;bC=this.__Js[i]||this.__Jt;bD={state:bv,rowDiv:bF,stylesheet:bA,element:bw,dataIndex:i,cellData:bG[i],height:bz};bB.push(bC.render(bD));if(bD.height>bz){bz=bD.height;}
;}
;bz=(bz>0?bz:this.getDefaultRowHeight());var bE=bv.getRendererData()[this.__BH];bE.totalHeight+=bz;bF.style.position=n;bF.style.height=bz+k;bF.className=M+this.__Jy+K;bF.innerHTML=bB.join(S);switch(bw.location){case c:var bx=bE.end||0;bF.style.backgroundColor=this.__Jv.bgcol[bx];bE.end=(bx==0?1:0);bv.getPane().getContentElement().getDomElement().appendChild(bF);break;case m:var by=bv.getPane().getContentElement().getDomElement();var bH=by.childNodes;if(bH.length>0){var bx=bE.start;bF.style.backgroundColor=this.__Jv.bgcol[bx];bE.start=(bx==0?1:0);by.insertBefore(bF,bH[0]);break;}
else {by.appendChild(bF);}
;break;default:throw new Error(t+bw.location);};++bE.rows;}
,getLayoutChildren:function(){return this.__Jr.getData();}
,_resizeColumns:function(e){var bP=this.__Ju.getStructure().getPane();var bM=bP.getBounds().width-qx.bom.element.Overflow.getScrollbarWidth();var bN=H+this.__Jy+K;var bJ=qx.ui.progressive.renderer.table.Row;qx.bom.Stylesheet.removeRule(bJ.__nd[this.__Jy].rowstylesheet,bN);var bL=u+bM+P;qx.bom.Stylesheet.addRule(bJ.__nd[this.__Jy].rowstylesheet,bN,bL);this.__nO.renderLayout(bM,100);var bO=this.__Jr.getData();for(var i=0,bS=0;i<bO.length;i++,bS+=bM){var bN=H+this.__Jy+Q+i;var bJ=qx.ui.progressive.renderer.table.Row;qx.bom.Stylesheet.removeRule(bJ.__nd[this.__Jy].cellstylesheet[i],bN);bM=bO[i].getComputedWidth();if(qx.core.Environment.get(bb)){if(qx.core.Environment.get(bh)){this.debug(V+i+w+bM);}
;}
;var bK;if(qx.core.Environment.get(bd)==l){bK=qx.ui.progressive.renderer.table.Row.__Jw*2;}
else {bK=-1;}
;var bR=(bM-bK)+P;var bQ=A+qx.ui.progressive.renderer.table.Row.__Jw+h+A+qx.ui.progressive.renderer.table.Row.__Jw+P;var bI=bS+P;var bL=bJ.__Jx+u+bR+y+bI+d+bQ;qx.bom.Stylesheet.addRule(bJ.__nd[this.__Jy].cellstylesheet[i],bN,bL);}
;}
},destruct:function(){var name;for(name in this.__Js){this.__Js[name]=null;}
;var bU=qx.ui.progressive.renderer.table.Row;var bT=this.__Ju.toHashCode();if(bU.__nd&&bU.__nd[bT]){if(bU.__nd[bT].rowstylesheet){var bW=H+this.__Jy+K;var bU=qx.ui.progressive.renderer.table.Row;qx.bom.Stylesheet.removeRule(bU.__nd[this.__Jy].rowstylesheet,bW);}
;if(bU.__nd[bT].cellstylesheet){for(var i=bU.__nd[bT].cellstylesheet.length-1;i>=0;i--){var bW=H+this.__Jy+Q+i;var bV=bU.__nd[this.__Jy].cellstylesheet[i];var bU=qx.ui.progressive.renderer.table.Row;qx.bom.Stylesheet.removeRule(bV,bW);}
;}
;}
;if(this.__Ju&&this.__Ju.getRendererData){var bX=this.__Ju.getRendererData();if(bX&&bX[this.__BH]&&bX[this.__BH].end!==undefined){bX[this.__BH]=null;}
;}
;this.__Jv=this.__Js=this.__Ju=this.__Jr=null;this._disposeObjects(R,s,bf);}
});}
)();
...
,_getContentHtml:function(A){var B=[];var C=this.__IQ(A);B.push(o);if(qx.bom.element.Decoration.isAlphaImageLoaderEnabled()&&/\.png$/i.test(C.url)){B.push(i,this.__IP,m,b,C.url,e,a);}
...
,__IQ:function(D){var E=this._identifyImage(D);if(E==null||typeof E=="string"){E={url:E,tooltip:null};}
;if(E.url==null){E.url=this.__IP;}
;return E;}
}});}
)();
(function(){var c="qx.ui.progressive.renderer.table.cell.Image",b="string",a="";qx.Class.define(c,{extend:qx.ui.progressive.renderer.table.cell.Icon,construct:function(d,e){qx.ui.progressive.renderer.table.cell.Icon.call(this);if(d===undefined){this.__qr=d;}
else {this.__qr=16;}
;if(e===undefined){this.__qs=e;}
else {this.__qs=16;}
;}
,members:{__qr:null,__qs:null,_identifyImage:function(f){var j={imageWidth:this.__qr,imageHeight:this.__qs};var k;if(typeof (f.cellData)==b){j.url=f.cellData;}
