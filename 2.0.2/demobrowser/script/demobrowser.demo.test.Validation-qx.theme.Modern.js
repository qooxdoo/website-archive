...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Validation-qx.theme.Modern.e3f6f2e535ec.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j="demobrowser.demo.test.Validation",i="<b>Setting the age ",h="property to 27</b><br>",g="- name: Name is validated to a minimum length of two.<br/>",f="<br>old name: ",e=")",d="age:",c="__Rd",b="fmale",a="<br><b>Setting the name to 'Hans'</b><br>",P="property to 'female'</b><br>",O="<br><b>Setting the gender ",N="must be longer than one characters. (",M="<b>Testing the gender ",L="<br>old gender: ",K="<br><b>Setting the age ",J="- age: Age is validated to be a number.<br/>",I="<b>Initial property values</b><br>",H="",G="<b>Setting the name to ''</b><br>",q="demobrowser.demo.test.Person",r="property to 'true'</b><br>",o="- gender: Gender is validated to be either male or female.",p="property to 'fmale'</b><br>",m="<font color='red'>GENDER NOT SET</font>",n="gender:",k="<br>old age: ",l="Hans",s="The demo creates an internal class containing three properties:<br/>",t="Validation Error: Name ",y="<font color='red'>AGE NOT SET</font>",x="My name",A="<font color='red'>NAME NOT SET</font>",z="white",C="name:",B="<b>Description</b><br/>",v="new name: ",F="new gender: ",E="male",D="female",u="new age: ",w="<br>";qx.Class.define(j,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var Q=new demobrowser.demo.test.Person();var R=new qx.ui.embed.Html();R.setBackgroundColor(z);R.set({padding:10,width:250,height:360});this.getRoot().add(R,{left:10,top:10});R.setHtml(I);R.setHtml(R.getHtml()+C+Q.getName()+w);R.setHtml(R.getHtml()+d+Q.getAge()+w);R.setHtml(R.getHtml()+n+Q.getGender()+w);R.setHtml(R.getHtml()+a);Q.setName(l);R.setHtml(R.getHtml()+v+Q.getName()+w);R.setHtml(R.getHtml()+G);try{Q.setName(H);R.setHtml(R.getHtml()+v+Q.getName()+w);}
catch(T){R.setHtml(R.getHtml()+A+f+Q.getName()+w);}
;R.setHtml(R.getHtml()+K+h);Q.setAge(27);R.setHtml(R.getHtml()+u+Q.getAge()+w);R.setHtml(R.getHtml()+i+r);try{Q.setAge(true);R.setHtml(R.getHtml()+u+Q.getAge()+w);}
catch(U){R.setHtml(R.getHtml()+y+k+Q.getAge()+w);}
;R.setHtml(R.getHtml()+O+P);Q.setGender(D);R.setHtml(R.getHtml()+F+Q.getGender()+w);R.setHtml(R.getHtml()+M+p);try{Q.setAge(b);R.setHtml(R.getHtml()+F+Q.getGender()+w);}
catch(V){R.setHtml(R.getHtml()+m+L+Q.getGender()+w);}
;var S=new qx.ui.basic.Label();S.setRich(true);S.setWidth(250);S.setValue(B+s+g+J+o);this.getRoot().add(S,{left:280,top:10});}
}});qx.Class.define(q,{extend:qx.core.Object,properties:{name:{init:x,validate:c},age:{init:18,validate:qx.util.Validate.number()},gender:{init:E,validate:qx.util.Validate.inArray([E,D])}},members:{__Rd:function(W){if(W.length<2){throw new qx.core.ValidationError(t+N+W+e);}
;}
}});}
)();
