...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Touch-qx.theme.Classic.f04dd9aad6c6.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="Two",k="Target Touches: ",j="Changed Target Touches: ",h="<b>Cancel Touch</b>",g="Is Multi-Touch: ",f="Screen: Left = ",d="30px",c="qx.debug",b="Identifier: ",a="event.touch",R=":",Q="Touch canceled",P="Viewport: Left = ",O='Lucida Grande',N="qx.mobile.emulatetouch",M="__infoArea",L="demobrowser.demo.test.Touch",K="Arial",J="engine.name",I="click",s="Type: ",t="20px",q="All Touches: ",r="red",o="webkit",p="<br>",m="Document: Left = ",n="<b>This demo is supposed to be run in a WebKit-based browser on a touch-enabled device.</b>",u="Are all touch targets div ",v="One",A="blue",z="px",C="==============",B="green",E="55px",D="12px",w="absolute",H="div",G=" / Top = ",F="innerHTML";qx.Class.define(L,{extend:qx.application.Native,members:{__QV:null,__QW:null,main:function(){qx.application.Native.prototype.main.call(this);if((qx.core.Environment.get(c))){qx.log.appender.Native;qx.log.appender.Console;}
;var T=new qx.html.Element(H);T.useElement(document.body);T.setRoot(true);if(qx.core.Environment.get(J)!=o||(!qx.core.Environment.get(a)&&qx.core.Environment.get(N)==false)){var S={"color":B,"position":w,"font-family":O,"font-size":D,"left":d,"top":t};var U=new qx.html.Element(H,S);T.add(U);U.setAttribute(F,n);return;}
;T.add(this.__QX(10,10,B,v));T.add(this.__QX(110,10,A,l));this.__QV=this.__QY(10,110);T.add(this.__QV);this.__QW=this.__QY(110,110);T.add(this.__QW);T.add(this.__Ra(210,10));}
,__QX:function(x,y,V,W){var X={"position":"absolute","left":x+"px","top":y+"px","width":"90px","height":"90px","backgroundColor":V,"fontFamily":"Arial","fontSize":"12px"};var Y=new qx.html.Element("div",X,{"id":W});Y.setAttribute("innerHTML","<b>Touch me:<br>"+W+"</b>");Y.addListener("touchstart",this.__Rb,this);Y.addListener("touchmove",this.__Rb,this);Y.addListener("touchend",this.__Rb,this);Y.addListener("touchcancel",this.__Rb,this);return Y;}
,__QY:function(x,y){var ba={"position":"absolute","left":x+"px","top":y+"px","width":"90px","fontFamily":"Arial","fontSize":"9px"};return new qx.html.Element("div",ba);}
,__Ra:function(x,y){var bb={"position":w,"left":x+z,"top":y+z,"width":E,"height":E,"backgroundColor":r,"fontFamily":K,"fontSize":D};var bc=new qx.html.Element(H,bb);bc.setAttribute(F,h);bc.addListener(I,function(){window.setTimeout(function(){alert(Q);}
,0);}
,this);return bc;}
,__Rb:function(e){var bd=[];bd.push(s+e.getType());bd.push(C);bd.push(g+e.isMultiTouch());bd.push(q+e.getAllTouches().length);bd.push(k+e.getTargetTouches().length);bd.push(j+e.getChangedTargetTouches().length);bd.push(C);bd.push(m+e.getDocumentLeft()+G+e.getDocumentTop());bd.push(f+e.getScreenLeft()+G+e.getScreenTop());bd.push(P+e.getViewportLeft()+G+e.getViewportTop());bd.push(b+e.getIdentifier());var bf=e.getTarget();var be=false;var bh=e.getTargetTouches().length>0?e.getTargetTouches():e.getChangedTargetTouches();for(var i=0;i<bh.length;i++){be=bh[0].target==bf;if(!be){break;}
;}
;bd.push(u+bf.id+R+be);var bg=this[M+bf.id];if(bg){bg.setAttribute(F,bd.join(p));}
;}
}});}
)();
