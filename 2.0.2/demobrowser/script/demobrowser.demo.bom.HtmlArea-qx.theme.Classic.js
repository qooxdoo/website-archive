...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.bom.HtmlArea","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.HtmlArea-qx.theme.Classic.2aa2fbe6350f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,this.__yL);qx.dom.Element.insertBegin(cI,cH);return cH;}
,__yT:function(){var cP=qx.dom.Element.create(bE);qx.bom.element.Style.setStyles(cP,{"marginRight":bx,"float":by});var cR;var cQ=qx.dom.Element.create(bj,{title:m});if(qx.core.Environment.get(bA)==bD){qx.bom.element.Attribute.set(cQ,bC,bF);}
...
;}
,this.__yL);qx.dom.Element.insertBegin(cQ,cP);return cP;}
,__yV:function(){var cW=[{bold:{text:cn,image:s,action:this.__yL.setBold},italic:{text:bu,image:U,action:this.__yL.setItalic},underline:{text:B,image:bt,action:this.__yL.setUnderline},strikethrough:{text:C,image:ct,action:this.__yL.setStrikeThrough},removeFormat:{text:M,image:h,action:this.__yL.removeFormat}},{alignLeft:{text:cc,image:t,action:this.__yL.setJustifyLeft},alignCenter:{text:X,image:cd,action:this.__yL.setJustifyCenter},alignRight:{text:bv,image:p,action:this.__yL.setJustifyRight},alignJustify:{text:cl,image:bb,action:this.__yL.setJustifyFull}},{fontFamily:{custom:this.__yS},fontSize:{custom:this.__yT},fontColor:{text:b,image:c,action:this.__yM},textBackgroundColor:{text:bN,image:bR,action:this.__yN}},{indent:{text:bT,image:bn,action:this.__yL.insertIndent},outdent:{text:D,image:n,action:this.__yL.insertOutdent}},{insertImage:{text:o,image:K,action:this.__yO},insertTable:{text:ca,image:f,action:this.__yP},insertLink:{text:bQ,image:H,action:this.__yQ},insertHTML:{text:O,image:bK,action:this.__yR},insertHR:{text:bJ,image:bM,action:this.__yL.insertHorizontalRuler}},{ol:{text:I,image:bo,action:this.__yL.insertOrderedList},ul:{text:z,image:bL,action:this.__yL.insertUnorderedList}},{undo:{text:T,image:g,action:this.__yL.undo},redo:{text:co,image:Y,action:this.__yL.redo}}];var cY,dd,cV,db;for(var i=0,j=cW.length;i<j;i++){dd={};dd[cr]=i==j-1?bG:bx;dd[cg]=by;cY=qx.dom.Element.create(bE);qx.bom.element.Style.setStyles(cY,dd);for(var da in cW[i]){var dc=cW[i][da];if(dc.custom){cV=dc.custom.call(this);}
...
;qx.event.Registration.addListener(cV,cu,dc.action,this.__yL);}
;qx.dom.Element.insertEnd(cV,cY);}
;qx.dom.Element.insertEnd(cY,this.__Pd);}
;qx.dom.Element.insertEnd(qx.dom.Element.create(cs,{style:cb}),this.__Pd);}
}});}
)();
...
;return this;}
;}
;var O=function(ba,bb){return function(bc,bd,be,bf,bg,bh){if(this.length>0){var bi=this[0].nodeType===1?ba[bb](this[0],bc,bd,be,bf,bg,bh):null;if(bi&&bi.nodeType){return this.__vB([bi]);}
else {return bi;}
;}
;return null;}
;}
...
,__vA:/^.[^:#\[\.,]*$/,not:function(bL){if(this.__vA.test(bL)){var bM=qx.bom.Selector.matches(m+bL+g,this);return this.__vB(bM);}
;var bM=qx.bom.Selector.matches(bL,this);return this.filter(function(bN){return bM.indexOf(bN)===-1;}
);}
,add:function(bO,bP){var bQ=qx.bom.Selector.query(bO,bP);var bR=qx.lang.Array.unique(this.concat(bQ));return this.__vB(bR);}
...
;bY=bY.parentNode;}
;}
);return this.__vB(qx.lang.Array.unique(bW));}
...
;return this.__vB(cb);}
,find:function(cc){var ce=qx.bom.Selector;if(this.length===1){return this.__vB(ce.query(cc,this[0]));}
else {var cd=[];for(var i=0,l=this.length;i<l;i++){cd.push.apply(cd,ce.query(cc,this[i]));}
;return this.__vB(qx.lang.Array.unique(cd));}
;}
...
;return this.__vB(ch);}
,nextAll:function(ci){return this.__sR(v,ci);}
...
;return this.__vB(cl);}
,prevAll:function(cm){return this.__sR(f,cm);}
...
;return this.__vB(co);}
,parents:function(cp){return this.__sR(a,cp);}
,siblings:function(cq){return this.__sR(u,cq);}
,__sR:function(cr,cs){var cu=[];var ct=qx.dom.Hierarchy;for(var i=0,l=this.length;i<l;i++){cu.push.apply(cu,ct[cr](this[i]));}
;var cv=qx.lang.Array.unique(cu);if(cs){cv=qx.bom.Selector.matches(cs,cv);}
;return this.__vB(cv);}
,__vB:function(cw){var cx=new qx.bom.Collection;cx.__vz=this;cw=Array.prototype.slice.call(cw,0);cx.push.apply(cx,cw);return cx;}
,andSelf:function(){return this.add(this.__vz);}
,end:function(){return this.__vz||new qx.bom.Collection();}
,__vC:function(cy,cz){var cE=this[0];var cD=cE.ownerDocument||cE;var cC=cD.createDocumentFragment();var cG=qx.bom.Html.clean(cy,cD,cC);var cI=cC.firstChild;if(cI){var cA=this.length-1;for(var i=0,l=cA;i<l;i++){cz.call(this,this[i],cC.cloneNode(true));}
...
;}
;return this;}
,__vD:function(cJ,cK){var cM=qx.bom.Selector;var cL=qx.lang.Array;var cO=[];for(var i=0,l=cJ.length;i<l;i++){if(qx.core.Environment.get(N)){if(typeof cJ[i]!==C){throw new Error(M+cJ[i]);}
;}
;cO.push.apply(cO,cM.query(cJ[i]));}
;cO=cL.cast(cL.unique(cO),qx.bom.Collection);for(var i=0,cN=this.length;i<cN;i++){cO[cK](this[i]);}
;return this;}
,append:function(cP){return this.__vC(arguments,this.__vE);}
,prepend:function(cQ){return this.__vC(arguments,this.__vF);}
,__vE:function(cR,cS){cR.appendChild(cS);}
,__vF:function(cT,cU){cT.insertBefore(cU,cT.firstChild);}
,appendTo:function(cV){return this.__vD(arguments,h);}
,prependTo:function(cW){return this.__vD(arguments,b);}
,before:function(cX){return this.__vC(arguments,this.__vG);}
,after:function(cY){return this.__vC(arguments,this.__vH);}
,__vG:function(da,db){da.parentNode.insertBefore(db,da);}
,__vH:function(dc,dd){dc.parentNode.insertBefore(dd,dc.nextSibling);}
,insertBefore:function(de){return this.__vD(arguments,o);}
,insertAfter:function(df){return this.__vD(arguments,s);}
...
;dg.map(this.__vI).append(this);}
;return this;}
,__vI:function(di){while(di.firstChild){di=di.firstChild;}
;return di;}
...
,replaceAll:function(dl){return this.__vD(arguments,p);}
...
;}
;var ba,bc=[];for(var i=0,l=W.length;i<l;i++){ba=W[i];if(typeof ba===w){ba=this.__tQ(ba,X);}
...
(function(){var j="url: ",i="Invalid state",h="head",g="script",f="load",e="Unknown response headers",d="abort",c="Received native readyState: loaded",b="readystatechange",a="Response header cannot be determined for ",D="opera",C="unknown",B="Open native request with ",A="Response headers cannot be determined for",z="Detected error",y="Send native request",x="on",w="timeout",v="Unknown environment key at this phase",u="Received native load",q="error",r="qx.debug",o="requests made with script transport.",p="loadend",m="",n="mshtml",k="browser.documentmode",l="engine.name",s="qx.debug.io",t="qx.bom.request.Script";qx.Bootstrap.define(t,{construct:function(){this.__ty();this.__tt=qx.Bootstrap.bind(this._onNativeLoad,this);this.__tu=qx.Bootstrap.bind(this._onNativeError,this);this.__sV=qx.Bootstrap.bind(this._onTimeout,this);this.__tv=document.head||document.getElementsByTagName(h)[0]||document.documentElement;this._emitter=new qx.event.Emitter();this.timeout=this.__tA()?0:15000;}
,events:{"readystatechange":t,"error":t,"loadend":t,"timeout":t,"abort":t,"load":t},members:{readyState:null,status:null,statusText:null,timeout:null,__tw:null,on:function(name,E,F){this._emitter.on(name,E,F);return this;}
,open:function(G,H){if(this.__tg){return;}
;this.__ty();this.__sX=null;this.__tb=H;if(this.__tD(s)){qx.Bootstrap.debug(qx.bom.request.Script,B+j+H);}
;this._readyStateChange(1);}
,setRequestHeader:function(I,J){if(this.__tg){return;}
;var K={};if(this.readyState!==1){throw new Error(i);}
;K[I]=J;this.__tb=qx.util.Uri.appendParamsToUrl(this.__tb,K);return this;}
,send:function(){if(this.__tg){return;}
;var N=this.__tB(),L=this.__tv,M=this;if(this.timeout>0){this.__dw=window.setTimeout(this.__sV,this.timeout);}
;if(this.__tD(s)){qx.Bootstrap.debug(qx.bom.request.Script,y);}
;L.insertBefore(N,L.firstChild);window.setTimeout(function(){M._readyStateChange(2);M._readyStateChange(3);}
);return this;}
,abort:function(){if(this.__tg){return;}
;this.__sX=true;this.__tC();this._emit(d);return this;}
...
,onabort:function(){}
,ontimeout:function(){}
,getResponseHeader:function(O){if(this.__tg){return;}
;if(this.__tD(r)){qx.Bootstrap.debug(a+o);}
;return C;}
,getAllResponseHeaders:function(){if(this.__tg){return;}
;if(this.__tD(r)){qx.Bootstrap.debug(A+o);}
;return e;}
,setDetermineSuccess:function(P){this.__tw=P;}
,dispose:function(){var Q=this.__tx;if(!this.__tg){if(Q){Q.onload=Q.onreadystatechange=null;this.__tC();}
;if(this.__dw){window.clearTimeout(this.__dw);}
;this.__tg=true;}
;}
,_getUrl:function(){return this.__tb;}
,_getScriptElement:function(){return this.__tx;}
,_onTimeout:function(){this.__tz();if(!this.__tA()){this._emit(q);}
;this._emit(w);if(!this.__tA()){this._emit(p);}
;}
,_onNativeLoad:function(){var T=this.__tx,R=this.__tw,S=this;if(this.__sX){return;}
;if(this.__tD(l)===n&&this.__tD(k)<9){if(!(/loaded|complete/).test(T.readyState)){return;}
else {if(this.__tD(s)){qx.Bootstrap.debug(qx.bom.request.Script,c);}
;}
;}
;if(this.__tD(s)){qx.Bootstrap.debug(qx.bom.request.Script,u);}
;if(R){if(!this.status){this.status=R()?200:500;}
;}
;if(this.status===500){if(this.__tD(s)){qx.Bootstrap.debug(qx.bom.request.Script,z);}
;}
;if(this.__dw){window.clearTimeout(this.__dw);}
;window.setTimeout(function(){S._success();S._readyStateChange(4);S._emit(f);S._emit(p);}
);}
,_onNativeError:function(){this.__tz();this._emit(q);this._emit(p);}
,__tx:null,__tv:null,__tb:m,__tt:null,__tu:null,__sV:null,__dw:null,__sX:null,__tg:null,__ty:function(){this.readyState=0;this.status=0;this.statusText=m;}
,_readyStateChange:function(U){this.readyState=U;this._emit(b);}
,_success:function(){this.__tC();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=m+this.status;}
,__tz:function(){this.__tC();this.readyState=4;this.status=0;this.statusText=null;}
,__tA:function(){var W=this.__tD(l)===n&&this.__tD(k)<9;var V=this.__tD(l)===D;return !(W||V);}
,__tB:function(){var X=this.__tx=document.createElement(g);X.src=this.__tb;X.onerror=this.__tu;X.onload=this.__tt;if(this.__tD(l)===n&&this.__tD(k)<9){X.onreadystatechange=this.__tt;}
;return X;}
,__tC:function(){var Y=this.__tx;if(Y&&Y.parentNode){this.__tv.removeChild(Y);}
;}
,__tD:function(ba){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(ba);}
...
(function(){var b="qx.event.Emitter",a="*";qx.Bootstrap.define(b,{extend:Object,statics:{__so:[]},members:{__sm:null,__sp:null,on:function(name,c,d){this.__sq(name).push({listener:c,ctx:d});qx.event.Emitter.__so.push({name:name,listener:c,ctx:d});return qx.event.Emitter.__so.length-1;}
,once:function(name,e,f){this.__sq(name).push({listener:e,ctx:f,once:true});qx.event.Emitter.__so.push({name:name,listener:e,ctx:f});return qx.event.Emitter.__so.length-1;}
,off:function(name,g,h){var k=this.__sq(name);for(var i=k.length-1;i>=0;i--){var j=k[i];if(j.listener==g&&j.ctx==h){k.splice(i,1);return i;}
;}
;return null;}
,offById:function(l){var m=qx.event.Emitter.__so[l];this.off(m.name,m.listener,m.ctx);}
...
,emit:function(name,u){var w=this.__sq(name);for(var i=w.length-1;i>=0;i--){var v=w[i];v.listener.call(v.ctx,u);if(v.once){w.splice(i,1);}
;}
;w=this.__sq(a);for(var i=w.length-1;i>=0;i--){var v=w[i];v.listener.call(v.ctx,u);}
;}
,getListeners:function(){return this.__sm;}
,__sq:function(name){if(this.__sm==null){this.__sm={};}
;if(this.__sm[name]==null){this.__sm[name]=[];}
;return this.__sm[name];}
}});}
)();
