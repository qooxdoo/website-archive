...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.List-qx.theme.Indigo.1482ace907dd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var ci=qx.data.marshal.Json.createModel(cf);var ca=this.__Rv=new qx.ui.list.List(ci).set({scrollbarX:A,selectionMode:Q,height:280,width:150,labelPath:x,iconPath:R,iconOptions:{converter:function(cm){return h+cm+H;}
...
,onSelectionModeChange:function(e){var cF=e.getData()[0].getUserData(w);this.__Rv.setSelectionMode(cF);if(cF==K||cF==L){this.__Rw.setEnabled(false);this.__Rx.setEnabled(true);}
else if(cF==Q||cF==c){this.__Rw.setEnabled(true);this.__Rx.setEnabled(false);}
;}
,onShowModeChange:function(e){var cG=e.getData()[0].getUserData(w);this.setShowMode(cG);}
,onDragCheckChange:function(e){if(e.getData()){var cH=this.__Rv.getSelectionMode();if(cH==K||cH==L){this.debug(bg);}
;}
;this.__Rv.setDragSelection(e.getData());}
,onQuickCheckChange:function(e){if(e.getData()){var cI=this.__Rv.getSelectionMode();if(cI==Q||cI==P){this.debug(S);}
;}
;this.__Rv.setQuickSelection(e.getData());}
}});}
)();
