...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.ListWithFilter-qx.theme.Indigo.99d555c71e56.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var q="Show all numbers.",p="Write data to log",o="Add an item",n="Remove an item",m="demobrowser.demo.virtual.ListWithFilter",l="Odd / Even filter",k="Remove filter",j="<b>Filtered List</b><br/>",h="Reverse order",g="Show only even numbers.",c="value",f="Show only odd numbers.",e="init",b="delegate",a="List showing numbered items, bound to a data array.",d="execute";qx.Class.define(m,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var A=[];for(var i=0;i<8;i++){A.push(i);}
...
;var z=new qx.ui.basic.Label(e);this.getRoot().add(z,{left:10,top:290});var D={converter:function(H){if(H==w){return g;}
else if(H==u){return f;}
;return q;}
};v.bind(b,z,c,D);var x=new qx.ui.form.Button(o);x.setWidth(120);this.getRoot().add(x,{left:130,top:80});x.addListener(d,function(){C.push(C.length);}
,this);var B=new qx.ui.form.Button(n);B.setWidth(120);this.getRoot().add(B,{left:130,top:115});B.addListener(d,function(){C.pop();}
,this);var y=new qx.ui.form.Button(p);y.setWidth(120);this.getRoot().add(y,{left:130,top:150});y.addListener(d,function(){qx.log.appender.Console.show();this.info(C.toString());}
,this);var s=new qx.ui.form.Button(l);s.setWidth(120);this.getRoot().add(s,{left:130,top:185});s.addListener(d,function(){v.getDelegate()==u?v.setDelegate(w):v.setDelegate(u);}
,this);var r=new qx.ui.form.Button(k);r.setWidth(120);this.getRoot().add(r,{left:130,top:220});r.addListener(d,function(){v.setDelegate(null);}
,this);var E=new qx.ui.form.Button(h);E.setWidth(120);this.getRoot().add(E,{left:130,top:255});E.addListener(d,function(){C.reverse();}
,this);var t=new qx.ui.basic.Label();t.setRich(true);t.setWidth(200);t.setValue(j+a);this.getRoot().add(t,{left:10,top:10});}
}});}
)();
...
,_init:function(){this._provider=new qx.ui.list.provider.WidgetProvider(this);this.__zA=[];this.__zB=[];this.__zC={};this.__zD=false;this.__zE=false;this.__zF=false;this.getPane().addListener(o,this._onResize,this);this._initBackground();this._initLayer();}
...
;}
,_getLookupTable:function(){return this.__zA;}
,_lookup:function(O){return this.__zA[O];}
,_lookupGroup:function(P){return this.__zB.indexOf(P);}
,_reverseLookup:function(Q){if(Q<0){return -1;}
;return this.__zA.indexOf(Q);}
...
,_applyDelegate:function(bj,bk){this._provider.setDelegate(bj);this.__zH();}
,_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);}
,_onModelChange:function(e){this.__zH();this._applyDefaultSelection();}
,__zG:function(){this.getPane().getRowConfig().setItemCount(this.__zA.length);this.getPane().fullUpdate();}
,__zH:function(){this.__zA=[];this.__zB=[];this.__zC={};if(this.isAutoGrouping()){this.getGroups().removeAll();}
;var bl=this.getModel();if(bl!=null){this._runDelegateFilter(bl);this._runDelegateSorter(bl);this._runDelegateGroup(bl);}
;this._updateSelection();this.__zG();}
,_runDelegateFilter:function(bm){var bn=qx.util.Delegate.getMethod(this.getDelegate(),h);for(var i=0,l=bm.length;i<l;++i){if(bn==null||bn(bm.getItem(i))){this.__zA.push(i);}
;}
;}
,_runDelegateSorter:function(bo){if(this.__zA.length==0){return;}
;var bp=qx.util.Delegate.getMethod(this.getDelegate(),F);if(bp!=null){this.__zA.sort(function(a,b){return bp(bo.getItem(a),bo.getItem(b));}
);}
;}
,_runDelegateGroup:function(bq){var bu=qx.util.Delegate.getMethod(this.getDelegate(),s);if(bu!=null){for(var i=0,l=this.__zA.length;i<l;++i){var br=this.__zA[i];var bt=this.getModel().getItem(br);var bs=bu(bt);this.__zI(bs,br);}
;this.__zA=this.__zJ();}
;}
,__zI:function(bv,bw){if(bv==null){this.__zF=true;bv=A;}
;var name=this.__zK(bv);if(this.__zC[name]==null){this.__zC[name]=[];if(this.isAutoGrouping()){this.getGroups().push(bv);}
;}
;this.__zC[name].push(bw);}
,__zJ:function(){this.__zL();var bA=[];var bC=0;var by=this.getGroups();for(var i=0;i<by.getLength();i++){var bx=by.getItem(i);bA.push(-1);this.__zB.push(bC);bC++;var bB=this.__zK(bx);var bz=this.__zC[bB];if(bz!=null){for(var k=0;k<bz.length;k++){bA.push(bz[k]);bC++;}
;}
;}
;return bA;}
,__zK:function(bD){var name=null;if(!qx.lang.Type.isString(bD)){var bE=this.getGroups().indexOf(bD);this.__zE=true;name=s;if(bE==-1){name+=this.getGroups().getLength();}
else {name+=bE;}
;}
else {this.__zD=true;var name=bD;}
;return name;}
,__zL:function(){if(this.__zE&&this.__zF||this.__zE&&this.__zD){throw new Error(G+B);}
;}
},destruct:function(){this._background.dispose();this._provider.dispose();this._layer.dispose();this._background=this._provider=this._layer=this.__zA=this.__zB=this.__zC=null;if(this.__zz){this.__zz.dispose();}
;}
});}
)();
...
;this.__As={};this.__At={};}
,properties:{colorEven:{nullable:true,check:a,apply:c,themeable:true},colorOdd:{nullable:true,check:a,apply:d,themeable:true}},members:{__Au:null,__Av:null,__As:null,__At:null,setColor:function(g,h){if(h){this.__As[g]=qx.theme.manager.Color.getInstance().resolve(h);}
else {delete (this.__As[g]);}
;}
,clearCustomColors:function(){this.__As={};this.updateLayerData();}
,getColor:function(i){var j=this.__As[i];if(j){return j;}
else {return i%2==0?this.__Au:this.__Av;}
;}
,_applyColorEven:function(k,l){if(k){this.__Au=qx.theme.manager.Color.getInstance().resolve(k);}
else {this.__Au=null;}
;this.updateLayerData();}
,_applyColorOdd:function(m,n){if(m){this.__Av=qx.theme.manager.Color.getInstance().resolve(m);}
else {this.__Av=null;}
;this.updateLayerData();}
,setBackground:function(o,p){if(p){this.__At[o]=qx.theme.manager.Decoration.getInstance().resolve(p);}
else {delete (this.__At[o]);}
;this.updateLayerData();}
,getBackground:function(q){return this.__At[q];}
...
;}
,setColor:function(J,K){qx.ui.virtual.layer.AbstractBackground.prototype.setColor.call(this,J,K);if(this.__AK(J)){this.updateLayerData();}
;}
,setBackground:function(L,M){qx.ui.virtual.layer.AbstractBackground.prototype.setBackground.call(this,L,M);if(this.__AK(L)){this.updateLayerData();}
;}
,__AK:function(N){var P=this.getFirstRow();var O=P+this.getRowSizes().length-1;return N>=P&&N<=O;}
}});}
)();
...
(function(){var p="model[",o="groups[",n="model",m="changeDelegate",l="label",k="qx.ui.list.core.MWidgetController",j="icon",i="value",h=".",g="",c="]",f="BindingIds",d="group",b="cell.type",a="String";qx.Mixin.define(k,{construct:function(){this.__zM=[];}
...
;}
,bindProperty:function(s,t,u,v,w){var y=v.getUserData(b);var x=this.__zN(w,s,y);var z=this._list.bind(x,v,t,u);this.__zO(v,z);}
,bindPropertyReverse:function(A,B,C,D,E){var G=D.getUserData(b);var F=this.__zN(E,A,G);var H=D.bind(B,this._list,F,C);this.__zO(D,H);}
,removeBindings:function(){while(this.__zM.length>0){var I=this.__zM.pop();this._removeBindingsFrom(I);}
;}
...
;}
,_removeBindingsFrom:function(T){var U=this.__zP(T);while(U.length>0){var V=U.pop();try{this._list.removeBinding(V);}
catch(e){T.removeBinding(V);}
;}
;if(qx.lang.Array.contains(this.__zM,T)){qx.lang.Array.remove(this.__zM,T);}
;}
,__zN:function(W,X,Y){var ba=p+W+c;if(Y==d){ba=o+W+c;}
;if(X!=null&&X!=g){ba+=h+X;}
;return ba;}
,__zO:function(bb,bc){var bd=this.__zP(bb);if(!qx.lang.Array.contains(bd,bc)){bd.push(bc);}
;if(!qx.lang.Array.contains(this.__zM,bb)){this.__zM.push(bb);}
;}
,__zP:function(be){var bf=be.getUserData(f);if(bf==null){bf=[];be.setUserData(f,bf);}
;return bf;}
...
,styleSelectabled:function(u){var v=this.__zx(u);this._styleSelectabled(v);}
,styleUnselectabled:function(w){var x=this.__zx(w);this._styleUnselectabled(x);}
...
;}
,_styleSelectabled:function(A){this.__zy(A,{selected:1});}
,_styleUnselectabled:function(B){this.__zy(B,{});}
...
,__zx:function(G){return this._list._layer.getRenderedCellWidget(G,0);}
,__zy:function(H,I){if(H==null){return;}
