...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.bom.Storage","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Storage-qx.theme.Indigo.083b2ec4682a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var r="<em>no content</em>",q="Memory Storage",p="html.storage.userdata",o="list",n="save",m="value",l="Web Storage",k="demobrowser.demo.bom.Storage",j="<br>",i="userData Storage",c="clear",h="html.storage.local",f=":",b="using",a="remove",e="key",d="",g="click";qx.Class.define(k,{extend:qx.application.Native,members:{__so:null,main:function(){qx.application.Native.prototype.main.call(this);this.setUsing();this.__so=qx.bom.Storage.getLocal();var t=document.getElementById(n);var s=document.getElementById(c);var u=document.getElementById(a);var self=this;qx.bom.Event.addNativeListener(t,g,function(){var w=qx.bom.Input.getValue(document.getElementById(e));var v=qx.bom.Input.getValue(document.getElementById(m));self.__so.setItem(w,v);self.updateList();}
);qx.bom.Event.addNativeListener(s,g,function(){self.__so.clear();self.updateList();}
);qx.bom.Event.addNativeListener(u,g,function(){self.__so.removeItem(self.__so.getKey(0));self.updateList();}
);this.updateList();}
,updateList:function(){var x=document.getElementById(o);x.innerHTML=d;this.__so.forEach(function(y,z){x.innerHTML+=y+f+z+j;}
,x);if(x.innerHTML==d){x.innerHTML=r;}
;}
,setUsing:function(){var A=document.getElementById(b);if(qx.core.Environment.get(h)){A.innerHTML=l;}
else if(qx.core.Environment.get(p)){A.innerHTML=i;}
else {A.innerHTML=q;}
;}
}});}
)();
(function(){var f="local",d="session",c="Storage",b="qx.bom.storage.Web",a="Storage full.";qx.Bootstrap.define(b,{statics:{__CA:null,__CB:null,getLocal:function(){if(this.__CA){return this.__CA;}
;return this.__CA=new qx.bom.storage.Web(f);}
,getSession:function(){if(this.__CB){return this.__CB;}
;return this.__CB=new qx.bom.storage.Web(d);}
},construct:function(g){this.__mH=g;}
,members:{__mH:null,getStorage:function(){return window[this.__mH+c];}
,getLength:function(){return this.getStorage(this.__mH).length;}
,setItem:function(h,j){j=qx.lang.Json.stringify(j);try{this.getStorage(this.__mH).setItem(h,j);}
catch(e){throw new Error(a);}
;}
,getItem:function(k){var l=this.getStorage(this.__mH).getItem(k);if(qx.lang.Type.isString(l)){l=qx.lang.Json.parse(l);}
else if(l&&l.value&&qx.lang.Type.isString(l.value)){l=qx.lang.Json.parse(l.value);}
;return l;}
,removeItem:function(m){this.getStorage(this.__mH).removeItem(m);}
,clear:function(){var n=this.getStorage(this.__mH);if(!n.clear){for(var i=n.length-1;i>=0;i--){n.removeItem(n.key(i));}
;}
else {n.clear();}
;}
,getKey:function(o){return this.getStorage(this.__mH).key(o);}
,forEach:function(p,q){var length=this.getLength();for(var i=0;i<length;i++){var r=this.getKey(i);p.call(q,r,this.getItem(r));}
;}
}});}
)();
...
(function(){var j="session",h="head",g="none",f="qx.bom.storage.UserData",e="div",d="local",c="#default#userdata",b="display",a="qx";qx.Bootstrap.define(f,{statics:{__CA:null,__CB:null,__bV:0,getLocal:function(){if(this.__CA){return this.__CA;}
;return this.__CA=new qx.bom.storage.UserData(d);}
,getSession:function(){if(this.__CB){return this.__CB;}
;return this.__CB=new qx.bom.storage.UserData(j);}
},construct:function(k){this.__CC=document.createElement(e);this.__CC.style[b]=g;document.getElementsByTagName(h)[0].appendChild(this.__CC);this.__CC.addBehavior(c);this.__CD=k;this.__CC.load(this.__CD);this.__so={};this.__CE={};var l=this.__CC.getAttribute(a+qx.bom.storage.UserData.__bV);while(l!=undefined){l=qx.lang.Json.parse(l);this.__so[l.key]=l.value;this.__CE[l.key]=a+qx.bom.storage.UserData.__bV;qx.bom.storage.UserData.__bV++;l=this.__CC.getAttribute(a+qx.bom.storage.UserData.__bV);}
;}
,members:{__CC:null,__CD:"qxtest",__so:null,__CE:null,getStorage:function(){return this.__so;}
,getLength:function(){return qx.Bootstrap.getKeys(this.__so).length;}
,setItem:function(m,n){if(this.__CE[m]){var o=this.__CE[m];}
else {var o=a+qx.bom.storage.UserData.__bV;qx.bom.storage.UserData.__bV++;}
;var p=qx.lang.Json.stringify({key:m,value:n});this.__CC.setAttribute(o,p);this.__CC.save(this.__CD);this.__so[m]=n;this.__CE[m]=o;}
,getItem:function(q){return this.__so[q]||null;}
,removeItem:function(r){var u=this.__CE[r];if(u==undefined){return;}
;this.__CC.removeAttribute(u);qx.bom.storage.UserData.__bV--;delete this.__so[r];delete this.__CE[r];var t=a+qx.bom.storage.UserData.__bV;if(this.__CC.getAttribute(t)){var v=this.__CC.getAttribute(a+qx.bom.storage.UserData.__bV);this.__CC.removeAttribute(t);this.__CC.setAttribute(u,v);var s=qx.lang.Json.parse(v).key;this.__CE[s]=u;}
;this.__CC.save(this.__CD);}
,clear:function(){for(var w in this.__CE){this.__CC.removeAttribute(this.__CE[w]);}
;this.__CC.save(this.__CD);this.__so={};this.__CE={};}
,getKey:function(x){return qx.Bootstrap.getKeys(this.__so)[x];}
,forEach:function(y,z){var length=this.getLength();for(var i=0;i<length;i++){var A=this.getKey(i);y.call(z,A,this.getItem(A));}
;}
}});}
)();
(function(){var a="qx.bom.storage.Memory";qx.Bootstrap.define(a,{statics:{__CA:null,__CB:null,getLocal:function(){if(this.__CA){return this.__CA;}
;return this.__CA=new qx.bom.storage.Memory();}
,getSession:function(){if(this.__CB){return this.__CB;}
;return this.__CB=new qx.bom.storage.Memory();}
},construct:function(){this.__so={};}
,members:{__so:null,getStorage:function(){return this.__so;}
,getLength:function(){return qx.Bootstrap.getKeys(this.__so).length;}
,setItem:function(b,c){c=qx.lang.Json.stringify(c);this.__so[b]=c;}
,getItem:function(d){var e=this.__so[d];if(qx.lang.Type.isString(e)){e=qx.lang.Json.parse(e);}
;return e;}
,removeItem:function(f){delete this.__so[f];}
,clear:function(){this.__so={};}
,getKey:function(g){var h=qx.Bootstrap.getKeys(this.__so);return h[g];}
,forEach:function(j,k){var length=this.getLength();for(var i=0;i<length;i++){var l=this.getKey(i);j.call(k,l,this.getItem(l));}
;}
}});}
)();
(function(){var c="html.storage.local",b="html.storage.userdata",a="qx.bom.Storage";qx.Bootstrap.define(a,{statics:{__BF:null,getLocal:function(){return this.__BF.getLocal();}
,getSession:function(){return this.__BF.getSession();}
},defer:function(d){if(qx.core.Environment.get(c)){d.__BF=qx.bom.storage.Web;}
else if(qx.core.Environment.get(b)){d.__BF=qx.bom.storage.UserData;}
else {d.__BF=qx.bom.storage.Memory;}
;}
});}
)();
