...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.ListControllerWithObjects","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ListControllerWithObjects-qx.theme.Classic.48fda05febcd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="smile",j="Jack",h=".png",g="kiss",f="demobrowser.demo.data.ListControllerWithObjects",e="emote",d="green",c="Jim",b="<b>Displaying objects in a list</b><br/>",a="Make all happy",G="sad",F="plain",E="surprise",D="embarrassed",C="online",B=" a converter by the icon. The font color shows the online status.",A="name",z="Write data to log",y="Jackob",x="textColor",r="have a name and an emotion, which is displayed with the help of ",s="This list display a set of persons in a list. Every person does ",p="Dan",q="Tim",n="Karl",o="Online <> Offline",l="icon/16/emotes/face-",m="Dustin",t="angel",u="Max",w="red",v="execute";qx.Class.define(f,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var U=[u,y,q,j,p,m,n,c];var L=[D,g,F,G,E,t];var R=[];for(var i=0;i<20;i++){var O=new demobrowser.demo.data.model.Person();O.setName(U[i%U.length]);O.setEmote(L[i%L.length]);O.setOnline(i%3==0);R.push(O);}
;var S=new qx.data.Array(R);var K=new qx.ui.form.List();K.setWidth(150);this.getRoot().add(K,{left:10,top:80});var T={converter:function(V){return l+V+h;}
};var P=new qx.data.controller.List(S,K,A);P.setIconOptions(T);P.setIconPath(e);var N={converter:function(W){if(W){return d;}
else {return w;}
;}
};var I={bindItem:function(X,Y,ba){X.bindDefaultProperties(Y,ba);X.bindProperty(C,x,N,Y,ba);}
};P.setDelegate(I);var J=new qx.ui.form.Button(a);J.setWidth(120);this.getRoot().add(J,{left:180,top:80});J.addListener(v,function(){for(var i=0;i<S.length;i++){var bb=S.getItem(i);bb.setEmote(k);}
;}
,this);var H=new qx.ui.form.Button(o);H.setWidth(120);this.getRoot().add(H,{left:180,top:115});H.addListener(v,function(){for(var i=0;i<S.length;i++){S.getItem(i).toggleOnline();}
;}
,this);var Q=new qx.ui.form.Button(z);Q.setWidth(120);this.getRoot().add(Q,{left:180,top:150});Q.addListener(v,function(){qx.log.appender.Console.show();this.info(S.toString());}
,this);var M=new qx.ui.basic.Label();M.setRich(true);M.setWidth(600);M.setValue(b+s+r+B);this.getRoot().add(M,{left:10,top:10});}
}});}
)();
