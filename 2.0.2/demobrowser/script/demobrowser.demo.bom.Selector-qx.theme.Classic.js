...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.bom.Selector","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Selector-qx.theme.Classic.825ba6ef3125.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var bs="backgroundColor",br=" children of ul/ol elements using children().",bq="Found li element with text 'List Item 1': ",bp="label[for=field-mail]",bo="#detail2",bn="opacity",bm="li:first-child",bl='<div style="border:2px solid red"><div style="border:2px solid blue"></div></div>',bk=" first children of ul/ol elements using children(selector)",bj="List Item 3 has ",X="form",W="<b></b>",V="p:eq(1)",U=" li elements in uls using find()",T="Click to follow the link",S="0.5",R="<li>inserted via prepend()</li>",Q="orange",P="id",O="text",bz="Offset of select box: ",bA="Number of parent elements of all li elems: ",bx=" submit button using filter(function)",by=" headers using find()",bv="for",bw="<h3>Dynamically Inserted H3</h3>",bt=" Title ",bu=" siblings",bB="<script type='text/javascript'>qx.log.Logger.debug('Globally executed script');</script>",bC="Number of all parent elements of the submit button: ",bc="[type=submit]",bb="header",be='<div style="background:#FFFFBB"></div>',bd="Number of elements preceding the UL: ",bg="<option>Mr</option>",bf="demobrowser.demo.bom.Selector",bi="red",bh=":first-child",ba="<li>inserted via after()</li>",Y="underline",a="<option>Mrs</option>",b="Tag following the UL element: ",c="li a",d="<option>Prof</option>",e="#eee",f="li:last-child",g="name",h="textAlign",i="h1",j="<option>Dr</option>",bG="submit",bF="ul li:contains('List Item 3')",bE="Number of child nodes of the body: ",bD="click",bK="Offset parent of select box: ",bJ=" submit buttons using filter(selector)",bI=":first-child,:last-child",bH="green",bM="Number of elements following the UL: ",bL="<li>inserted via append()</li>",y="Number of found li elements in the middle: ",z="Number of ul/ol elements which are closest to a li element: ",w="Number of found li elements without text 'List Item 1': ",x="href",C="input[type=submit]",D="href of first link: ",A="label",B="Tag before the UL element: ",u="Any H2 with form has a class 'form': ",v="<li>inserted via before()</li>",r="Typing in e-mail field...",q="p",t="bold",s="#detail1,#detail2,#detail3",n="color",m=":contains('List Item 1')",p="a",o="title",l="h1,h2",k="center",I="fontWeight",J="field-title",K="field-mail",L="h2",E="input",F="select",G="ul,ol",H="Found ",M="li",N="ul";qx.Class.define(bf,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var bS=qx.log.Logger;var bO=qx.bom.Collection;bO.query(q).setStyle(n,bi);bO.query(V).setStyle(I,t).setStyle(bn,S);bO.query(i).setStyles({"textAlign":k,"textDecoration":Y,"color":bH});bS.debug(D+bO.query(p).getAttribute(x));bO.query(p).setAttribute(o,T);bO.query(l).addClass(bb);bO.query(q).addListener(bD,function(){alert(this.innerHTML);}
);(new qx.bom.Collection).add(L).setStyle(n,Q).add(M).setStyle(bs,e).end().setStyle(h,k);bS.debug(H+qx.bom.Collection.query(G).children().length+br);bS.debug(H+qx.bom.Collection.query(G).children(bh).length+bk);bS.debug(z+qx.bom.Collection.query(M).closest(G).length);bS.debug(bE+new qx.bom.Collection(document.body).contents().length);var bN=qx.bom.Collection.query(N).find(M);bS.debug(H+bN.length+U);var bN=new qx.bom.Collection(document.body).find(l);bS.debug(H+bN.length+by);var bN=qx.bom.Collection.query(N).next()[0].tagName;bS.debug(b+bN);var bN=qx.bom.Collection.query(N).nextAll().length;bS.debug(bM+bN);var bN=qx.bom.Collection.query(M).parent().length;bS.debug(bA+bN);var bN=qx.bom.Collection.query(C).parents().length;bS.debug(bC+bN);var bN=qx.bom.Collection.query(N).prev()[0].tagName;bS.debug(B+bN);var bN=qx.bom.Collection.query(N).prevAll().length;bS.debug(bd+bN);var bN=qx.bom.Collection.query(bF).siblings().length;bS.debug(bj+bN+bu);var bN=qx.bom.Collection.query(F).getOffsetParent()[0].tagName;bS.debug(bK+bN);var bN=qx.bom.Collection.query(F).getOffset();bS.debug(bz);bS.debug(bN);bO.query(c).eq(1).setStyle(I,t);var bN=qx.bom.Collection.query(E).filter(bc);bS.debug(H+bN.length+bJ);var bN=qx.bom.Collection.query(E).filter(function(bV,bW,bX){return bV.type===bG;}
);bS.debug(H+bN.length+bx);var bN=qx.bom.Collection.query(L).hasClass(X);bS.debug(u+bN);var bN=qx.bom.Collection.query(N).find(M).is(m);bS.debug(bq+bN);var bN=qx.bom.Collection.query(N).find(M).not(m);bS.debug(w+bN.length);var bN=qx.bom.Collection.query(N).find(M).not(bI);bS.debug(y+bN.length);qx.bom.Collection.query(G).append(bL);qx.bom.Collection.query(G).prepend(R);qx.bom.Collection.query(N).append(bB);qx.bom.Collection.query(bm).after(ba);qx.bom.Collection.query(f).before(v);var bU=document.createElement(F);var bT=document.createElement(A);var bP=new qx.bom.Collection(bU).setAttribute(P,J).setAttribute(g,o);new qx.bom.Collection(bT).setAttribute(bv,J).setAttribute(O,bt);qx.bom.Collection.query(F).after(bT,bU);bP.append(bg,a,j,d);$(bw).insertAfter(L);qx.bom.Collection.id(K).addListener(E,function(){bS.debug(r);}
);var bQ=qx.bom.Collection.id(K);var bR=bQ.add(bp).reverse();bQ.after(bR.clone(true));qx.bom.Collection.query(s).wrapAll(bl);qx.bom.Collection.query(s).wrapInner(W);qx.bom.Collection.query(bo).wrap(be);}
}});}
)();
