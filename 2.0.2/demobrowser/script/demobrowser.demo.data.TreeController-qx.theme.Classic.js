...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.TreeController","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.TreeController-qx.theme.Classic.a09e16f5c71e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var q="changeNames",p="changeName",o="- ",n="",m="changeChild",l="changeColor",k="Marge",j="AFFE",h="Homer",g="\n",c="__PT",f="changeName2",e="changeChildren",b="qx.data.Array",a="demobrowser.demo.data.model.Node",d="String";qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.setChildren(new qx.data.Array());}
,properties:{children:{check:b,event:e},child:{check:a,event:m,nullable:true},names:{check:b,event:q,init:new qx.data.Array(h,k)},name:{check:d,init:j,event:p},name2:{check:d,event:f},number:{init:10,validate:c},color:{event:l,nullable:true}},members:{__PT:function(r){if(!isNaN(parseFloat(r))){if(/^\d+$/.test(r)){return;}
;}
...
(function(){var w="changeModel",v="_applyDelegate",u="qx.core.Object",t="_applyLabelPath",s="icon",r="change",q="label",p="_applyTarget",o="changeTarget",n="_applyIconPath",d="Can not reverse bind the same target property '",m="_applyLabelOptions",h="_applyChildPath",c="_applyIconOptions",b="qx.data.controller.Tree",g="Can not bind the same target property '",f="_applyModel",k="' twice.",a="String",l="get";qx.Class.define(b,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(x,y,z,A){qx.core.Object.call(this);this.__bg={};this.__Bf=[];this.__Dj={};if(z!=null){this.setChildPath(z);}
;if(A!=null){this.setLabelPath(A);}
;if(x!=null){this.setModel(x);}
;if(y!=null){this.setTarget(y);}
;}
,properties:{model:{check:u,apply:f,event:w,nullable:true,dereference:true},target:{apply:p,event:o,init:null,nullable:true,dereference:true},childPath:{check:a,apply:h,nullable:true},labelPath:{check:a,apply:t,nullable:true},iconPath:{check:a,apply:n,nullable:true},labelOptions:{apply:m,nullable:true},iconOptions:{apply:c,nullable:true},delegate:{apply:v,init:null,nullable:true}},members:{__Dj:null,__bg:null,__Bf:null,__Dk:null,_applyDelegate:function(B,C){this._setConfigureItem(B,C);this._setCreateItem(B,C);this._setBindItem(B,C);}
,_applyIconOptions:function(D,E){this.__Bq();}
,_applyLabelOptions:function(F,G){this.__Bq();}
,_applyTarget:function(H,I){if(I!=undefined){this.__CG(I);}
;if(this.getModel()!=null){this.__Dn();}
;this._addChangeTargetListener(H,I);}
,_applyModel:function(J,K){this.__Dn();}
,_applyChildPath:function(L,M){this.__Dk=M;this.__Dn();this.__Dk=null;}
,_applyIconPath:function(N,O){this.__Bq();}
,_applyLabelPath:function(P,Q){this.__Dn();}
,__Dl:function(R){var T=R.getTarget();var S=this.__Dj[T.toHashCode()].treeNode;var U=this.__Dj[T.toHashCode()].modelNode;this.__Do(S,U);this._updateSelection();}
,__Dm:function(e){var bc=e.getData();var X=e.getOldData();var Y=this.__Dj[X.toHashCode()];X.removeListenerById(Y.changeListenerId);delete this.__Dj[X.toHashCode()];Y.modelNode.removeListenerById(Y.changeChildernListenerId);var bd=Y.modelNode;var bb=qx.Class.getPropertyDefinition(Y.modelNode.constructor,this.getChildPath());var V=bb.event;var ba=bd.addListener(V,this.__Dm,this);var W=Y.treeNode;this.__Dj[bc.toHashCode()]={modelNode:bd,treeNode:W,changeListenerId:Y.changeListenerId,changeChildernListenerId:ba};this.__Do(W,bd);this._updateSelection();}
...
;return be;}
,__Dn:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__CG();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.getTarget().setRoot(bg);this.__zO(this.getModel(),bg);this.__Do(bg,this.getModel());}
;}
,__Do:function(bh,bi){if(bi[l+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var br=bi[l+qx.lang.String.firstUp(this.getChildPath())]();if(this.__Dj[br.toHashCode()]==undefined){var bm=br.addListener(r,this.__Dl,this);var bq=qx.Class.getPropertyDefinition(bi.constructor,this.getChildPath());var bn=bq.event;var bo=bi.addListener(bn,this.__Dm,this);this.__Dj[br.toHashCode()]={modelNode:bi,treeNode:bh,changeListenerId:bm,changeChildernListenerId:bo};}
...
;}
else {var bp=this._createItem();bp.setModel(br.getItem(i));bh.addAt(bp,i);this.__zO(br.getItem(i),bp);this.__Do(bp,br.getItem(i));}
;}
;}
;for(var i=bh.getChildren().length-1;i>=br.length;i--){var bl=bh.getChildren()[i];this.__Dq(bl,bh);}
;}
,__CG:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__Dp(bv);var bu=bv.getModel();if(bu){this.__rc(bu);}
;bv.destroy();this.__Dj={};}
;}
,__Dp:function(bw){var bx=bw.getChildren()||[];for(var i=bx.length-1;i>=0;i--){if(bx[i].getChildren().length>0){this.__Dp(bx[i]);}
;this.__Dq(bx[i],bw);}
;}
,__Dq:function(by,bz){var bB=by.getModel();var bD=this.__Dk||this.getChildPath();var bE=l+qx.lang.String.firstUp(bD);if(bB[bE]!=undefined){var bC=bB[bE]();var bA=this.__Dj[bC.toHashCode()];bC.removeListenerById(bA.changeListenerId);bB.removeListenerById(bA.changeChildernListenerId);this.__Dp(by);delete this.__Dj[bC.toHashCode()];}
;this.__rc(bB);bz.remove(by);by.destroy();}
...
;if(!qx.lang.Array.contains(this.__Bf,bG)){this.__Bf.push(bG);}
;}
...
;if(!qx.lang.Array.contains(this.__Bf,bN)){this.__Bf.push(bN);}
;}
...
;}
,__Bq:function(){var bW;for(var bY in this.__bg){bW=bY;break;}
;for(var bV in this.__bg[bW]){var ca=this.__bg[bW][bV].treeNode;var bX=qx.core.ObjectRegistry.fromHashCode(bV);this.__rc(bX);this.__zO(bX,ca);}
;}
,__zO:function(cb,cc){var cd=this.getDelegate();if(cd!=null&&cd.bindItem!=null){cd.bindItem(this,cc,cb);}
else {this.bindDefaultProperties(cc,cb);}
;}
,__rc:function(ce){for(var i=0;i<this.__Bf.length;i++){var cg=this.__Bf[i];var cf=this.__bg[cg][ce.toHashCode()];if(cf!=null){if(cf.id){ce.removeBinding(cf.id);cf.id=null;}
...
;this._startSelectionModification();this.__CG();this.__Dn();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cm,cn){if(cm!=null&&cm.bindItem!=null){if(cn!=null&&cn.bindItem!=null&&cm.bindItem==cn.bindItem){return;}
;this.__Dn();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__bg=this.__Dj=this.__Bf=null;}
});}
)();
