...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.ChildrenHandling","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.ChildrenHandling-qx.theme.Indigo.9accf457d7e1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var q="_gridWidgets",p="yellow",o="demobrowser.demo.test.ChildrenHandling",n="Horizontal box layout:",m="blue",l="Add ",k="_widgets",j="_hbox",h="green",g="Grid layout:",c="black",f="orange",e="_grid",b="red",a="execute",d="Remove ";qx.Class.define(o,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var v=new qx.ui.layout.VBox();v.setSpacing(10);var s=new qx.ui.container.Composite(v).set({padding:10});s.add(new qx.ui.basic.Label(n));this._hbox=new qx.ui.container.Composite(new qx.ui.layout.HBox()).set({height:100});this._widgets=[];var w=[h,m,p,c,f,b];for(var i=0;i<w.length;i++){var r=new qx.ui.core.Widget();r.setBackgroundColor(w[i]);this._hbox.add(r);this._widgets.push(r);}
;s.add(this._hbox);s.add(new qx.ui.basic.Label(g));var u=new qx.ui.layout.Grid();u.setRowFlex(0,1);this._grid=new qx.ui.container.Composite(u).set({height:100});this._gridWidgets=[];for(var i=0;i<w.length;i++){var r=new qx.ui.core.Widget();r.setBackgroundColor(w[i]);this._grid.add(r,{row:0,column:i});this._gridWidgets.push(r);}
;s.add(this._grid);var t=new qx.ui.container.Composite(new qx.ui.layout.HBox());for(var i=0;i<this._widgets.length;i++){t.add(this.createRemoveButton(this._widgets[i],i));}
;s.add(t);this.getRoot().add(s,{left:0,top:0});}
,createRemoveButton:function(x,y){var A=true;var z=new qx.ui.form.Button(d+x.getBackgroundColor()).set({width:100});z.addListener(a,function(){if(A){this._hbox.remove(x);this._grid.remove(this._gridWidgets[y]);z.setLabel(l+x.getBackgroundColor());}
else {this._hbox.add(x);this._grid.add(this._gridWidgets[y],{row:0,column:y});z.setLabel(d+x.getBackgroundColor());}
;A=!A;}
,this);return z;}
},destruct:function(){this._disposeObjects(j,e);this._disposeArray(k);this._disposeArray(q);}
});}
)();
