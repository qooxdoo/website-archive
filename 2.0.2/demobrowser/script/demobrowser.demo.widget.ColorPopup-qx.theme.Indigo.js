...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.ColorPopup-qx.theme.Indigo.a2610609600c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var bp=this._tables[this.__MR].values;var bq=this.__MO[this.__MR];if(!bp){return;}
;var br=this.getValue();if(!br){return;}
;var bo=bp.indexOf(br);if(bo!=-1){qx.lang.Array.removeAt(bp,bo);}
else if(bp.length==this.__MS){bp.shift();}
...
;this.__MQ.setRed(bz);this.__MQ.setGreen(by);this.__MQ.setBlue(bx);this.__MP.open();}
,_onColorSelectorOk:function(){var bA=this.__MQ;this.setValue(qx.util.ColorUtil.rgbToRgbString([bA.getRed(),bA.getGreen(),bA.getBlue()]));this.__MP.close();}
,_onColorSelectorCancel:function(){this.__MP.close();}
...
;}
,_tables:{core:{label:w,values:[b,c,f,o,Q,s,y,I,T,a,k,h]},recent:{label:H,values:[]}}},destruct:function(){this._disposeObjects(t,m);this._tables=this.__MO=null;}
});}
)();
...
,setValue:function(bK){var bL;if(bK==null){this.__mC=true;bL=[255,255,255];}
else {bL=qx.util.ColorUtil.stringToRgb(bK);this.__mC=false;}
;this.__IF=true;this.setRed(bL[0]);this.setGreen(bL[1]);this.__IF=false;this.setBlue(bL[2]);}
,getValue:function(){return this.__mC?null:qx.util.ColorUtil.rgbToHexString([this.getRed(),this.getGreen(),this.getBlue()]);}
,resetValue:function(){this.__mC=true;this.__IF=true;this.setRed(255);this.setGreen(255);this.__IF=false;this.setBlue(255);}
,__mF:function(){if(!this.__IF){this.__mC=false;this.fireDataEvent(D,this.getValue());}
;}
,_applyRed:function(bM,bN){if(this.__Iz===null){this.__Iz=N;}
;if(this.__Iz!==bk){this.getChildControl(be).setValue(bM);}
;if(this.__Iz!==G){this._setHexFromRgb();}
;switch(this.__Iz){case bk:case G:case N:this._setHueFromRgb();};this._setPreviewFromRgb();this.__mF();if(this.__Iz===N){this.__Iz=null;}
;}
,_applyGreen:function(bO,bP){if(this.__Iz===null){this.__Iz=z;}
;if(this.__Iz!==bk){this.getChildControl(bh).setValue(bO);}
;if(this.__Iz!==G){this._setHexFromRgb();}
;switch(this.__Iz){case bk:case G:case z:this._setHueFromRgb();};this._setPreviewFromRgb();this.__mF();if(this.__Iz===z){this.__Iz=null;}
;}
,_applyBlue:function(bQ,bR){if(this.__Iz===null){this.__Iz=Q;}
;if(this.__Iz!==bk){this.getChildControl(A).setValue(bQ);}
;if(this.__Iz!==G){this._setHexFromRgb();}
;switch(this.__Iz){case bk:case G:case Q:this._setHueFromRgb();};this._setPreviewFromRgb();this.__mF();if(this.__Iz===Q){this.__Iz=null;}
;}
,_applyHue:function(bS,bT){if(this.__Iz===null){this.__Iz=bd;}
;if(this.__Iz!==R){this.getChildControl(b).setValue(bS);}
;if(this.__Iz!==H){if(this.getChildControl(bj).getBounds()){this.getChildControl(bj).setDomLeft(Math.round(bS/1.40625)+this.getChildControl(a).getPaddingLeft());}
else {this.getChildControl(bj).setLayoutProperties({left:Math.round(bS/1.40625)});}
;}
;switch(this.__Iz){case R:case H:case bd:this._setRgbFromHue();};this._setBrightnessGradiant();if(this.__Iz===bd){this.__Iz=null;}
;}
,_applySaturation:function(bU,bV){if(this.__Iz===null){this.__Iz=W;}
;if(this.__Iz!==R){this.getChildControl(Y).setValue(bU);}
;if(this.__Iz!==H){this._setBrightnessGradiant();if(this.getChildControl(bj).getBounds()){this.getChildControl(bj).setDomTop(256-Math.round(bU*2.56)+this.getChildControl(a).getPaddingTop());}
else {this.getChildControl(bj).setLayoutProperties({top:256-Math.round(bU*2.56)});}
;}
;switch(this.__Iz){case R:case H:case W:this._setRgbFromHue();};if(this.__Iz===W){this.__Iz=null;}
;}
,_applyBrightness:function(bW,bX){if(this.__Iz===null){this.__Iz=ba;}
;if(this.__Iz!==R){this.getChildControl(bf).setValue(bW);}
;if(this.__Iz!==B){var bY=256-Math.round(bW*2.56);if(this.getChildControl(M).getBounds()){this.getChildControl(M).setDomTop(bY+this.getChildControl(K).getPaddingTop());}
else {this.getChildControl(M).setLayoutProperties({top:bY});}
;}
;switch(this.__Iz){case R:case B:case ba:this._setRgbFromHue();};if(this.__Iz===ba){this.__Iz=null;}
;}
,_onBrightnessHandleMouseDown:function(e){this.getChildControl(M).capture();this.__IB=M;var cc=this.getChildControl(C).getContainerLocation();var cb=this.getChildControl(M).getContainerLocation();var ca=this.getChildControl(C).getBounds();this.__IC=cc.top+(e.getDocumentTop()-cb.top)-ca.top;e.stopPropagation();}
,_onBrightnessHandleMouseUp:function(e){this.getChildControl(M).releaseCapture();this.__IB=null;}
,_onBrightnessHandleMouseMove:function(e){if(this.__IB===M){this._setBrightnessOnFieldEvent(e);e.stopPropagation();}
;}
,_onBrightnessFieldMouseDown:function(e){var location=this.getChildControl(C).getContainerLocation();var cd=this.getChildControl(M).getBounds();this.__IC=location.top+(cd.height/2);this._setBrightnessOnFieldEvent(e);this.getChildControl(M).capture();this.__IB=M;}
...
,_setBrightnessOnFieldEvent:function(e){var ce=qx.lang.Number.limit(e.getDocumentTop()-this.__IC,0,256);this.__Iz=B;if(this.getChildControl(M).getBounds()){this.getChildControl(M).setDomTop(ce);}
else {this.getChildControl(M).setLayoutProperties({top:ce});}
;this.setBrightness(100-Math.round(ce/2.56));this.__Iz=null;}
,_onHueSaturationHandleMouseUp:function(e){if(this.__IB){e.stopPropagation();this.getChildControl(bj).releaseCapture();this.__IB=null;}
;}
,_onHueSaturationHandleMouseMove:function(e){if(this.__IB===bj){this._setHueSaturationOnFieldEvent(e);e.stopPropagation();}
;}
,_onHueSaturationFieldMouseDown:function(e){var location=this.getChildControl(X).getContainerLocation();var cf=this.getChildControl(bj).getBounds();var cg=this.getChildControl(X).getBounds();this.__ID=location.top+(cf.height/2)-cg.top;this.__IE=location.left+(cf.width/2)-cg.left;this._setHueSaturationOnFieldEvent(e);this.getChildControl(bj).capture();this.__IB=bj;}
...
,_setHueSaturationOnFieldEvent:function(e){var ci=qx.lang.Number.limit(e.getDocumentTop()-this.__ID,0,256);var ch=qx.lang.Number.limit(e.getDocumentLeft()-this.__IE,0,256);this.getChildControl(bj).setDomPosition(ch,ci);this.__Iz=H;this.setSaturation(100-Math.round(ci/2.56));this.setHue(Math.round(ch*1.40625));this.__Iz=null;}
,_setRedFromSpinner:function(){if(this.__Iz!==null){return;}
;this.__Iz=bk;this.setRed(this.getChildControl(be).getValue());this.__Iz=null;}
,_setGreenFromSpinner:function(){if(this.__Iz!==null){return;}
;this.__Iz=bk;this.setGreen(this.getChildControl(bh).getValue());this.__Iz=null;}
,_setBlueFromSpinner:function(){if(this.__Iz!==null){return;}
;this.__Iz=bk;this.setBlue(this.getChildControl(A).getValue());this.__Iz=null;}
,_setHueFromSpinner:function(){if(this.__Iz!==null){return;}
;this.__Iz=R;this.setHue(this.getChildControl(b).getValue());this.__Iz=null;}
,_setSaturationFromSpinner:function(){if(this.__Iz!==null){return;}
;this.__Iz=R;this.setSaturation(this.getChildControl(Y).getValue());this.__Iz=null;}
,_setBrightnessFromSpinner:function(){if(this.__Iz!==null){return;}
;this.__Iz=R;this.setBrightness(this.getChildControl(bf).getValue());this.__Iz=null;}
,_onHexFieldChange:function(e){if(this.__Iz!==null){return;}
;try{var ck=this.getChildControl(P);var cj=qx.util.ColorUtil.hexStringToRgb(bl+ck.getValue());}
catch(cl){return;}
;this.__Iz=G;this.setRed(cj[0]);this.setGreen(cj[1]);this.setBlue(cj[2]);this.__Iz=null;}
...
,_setHueFromRgb:function(){switch(this.__Iz){case R:case H:case B:break;default:var cp=qx.util.ColorUtil.rgbToHsb([this.getRed(),this.getGreen(),this.getBlue()]);this.setHue(cp[0]);this.setSaturation(cp[1]);this.setBrightness(cp[2]);};}
,_setRgbFromHue:function(){switch(this.__Iz){case bk:case G:break;default:var cq=qx.util.ColorUtil.hsbToRgb([this.getHue(),this.getSaturation(),this.getBrightness()]);this.setRed(cq[0]);this.setGreen(cq[1]);this.setBlue(cq[2]);};}
