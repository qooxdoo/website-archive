...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.VBox_Flex-qx.theme.Modern.d492d6f29d97.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,__CN:{north:1,south:2,west:3,east:4,center:5},__CO:{'1':"top",'2':"bottom",'3':"left",'4':"right"},__kQ:function(){var F=this._getLayoutChildren();var N,H;var length=F.length;var J=[];var M=[];var K=[];var G=this.getSort()===v;var L=this.getSort()===l;for(var i=0;i<length;i++){N=F[i];K=N.getLayoutProperties().edge;if(K===w){if(H){throw new Error(j);}
;H=N;}
...
;this.__dW=O;var I=[];for(var i=0;i<length;i++){K=O[i].getLayoutProperties().edge;I[i]=this.__CN[K]||5;}
;this.__BS=I;delete this._invalidChildrenCache;}
,renderLayout:function(P,Q){if(this._invalidChildrenCache){this.__kQ();}
;var bk=qx.ui.layout.Util;var W=this.__dW;var bl=this.__BS;var length=W.length;var ba,S,Y,bf,bg,bq,bd,T,bn;var bu=[];var bc=[];var bt=this._getSeparatorWidths();var by=this.getSpacingX();var bx=this.getSpacingY();var X=-by;var bm=-bx;if(bt.x){X-=bt.x+by;}
;if(bt.y){bm-=bt.y+bx;}
...
;var bP=this.__dW;var ca=this.__BS;var length=bP.length;var bT,cb;var bV,bU;var bW=0,ci=0;var bM=0,cd=0;var bX=0,ch=0;var bN=0,cc=0;var ce=this._getSeparatorWidths();var cg=this.getSpacingX(),cf=this.getSpacingY();var bR=-cg,bQ=-cf;if(ce.x){bR-=ce.x+cg;}
;if(ce.y){bQ-=ce.y+cf;}
...
},destruct:function(){this.__BS=this.__dW=null;}
});}
)();
...
,_applyValue:function(bf,bg){var bh=this.getChildControl(z);this._updateButtons();this.__vV=bf;if(bf!==null){if(this.getNumberFormat()){bh.setValue(this.getNumberFormat().format(bf));}
...
,_applyNumberFormat:function(bn,bo){var bp=this.getChildControl(z);bp.setFilter(this._getFilterRegExp());this.getNumberFormat().addListener(E,this._onChangeNumberFormat,this);this._applyValue(this.__vV,undefined);}
...
;}
;}
,_onKeyDown:function(e){switch(e.getKeyIdentifier()){case r:this.__vW=true;case n:this.getChildControl(y).press();break;case l:this.__vX=true;case q:this.getChildControl(v).press();break;default:return;};e.stopPropagation();e.preventDefault();}
,_onKeyUp:function(e){switch(e.getKeyIdentifier()){case r:this.getChildControl(y).release();this.__vW=false;break;case n:this.getChildControl(y).release();break;case l:this.getChildControl(v).release();this.__vX=false;break;case q:this.getChildControl(v).release();break;};}
...
,_countUp:function(){if(this.__vW){var bC=this.getValue()+this.getPageStep();}
...
;}
;this.gotoValue(bC);}
,_countDown:function(){if(this.__vX){var bE=this.getValue()-this.getPageStep();}
...
(function(){var d="demobrowser.demo.layout.VBox_Flex",c="yellow",b="green",a="main";qx.Class.define(d,{extend:demobrowser.demo.util.LayoutApplication,members:{main:function(){demobrowser.demo.util.LayoutApplication.prototype.main.call(this);var scroll=new qx.ui.container.Scroll();this.getRoot().add(scroll,{edge:0});var e=new qx.ui.container.Composite(new qx.ui.layout.HBox(20)).set({padding:20});scroll.add(e);e.add(this.getBox1());e.add(this.getBox2());e.add(this.getBox3());e.add(this.getBox4());e.add(this.getBox5());e.add(this.getBox6());}
,getBox1:function(){var f=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({decorator:a,backgroundColor:c,height:300,allowGrowY:false});f.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}),{flex:1});f.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}),{flex:2});f.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}),{flex:3});return f;}
,getBox2:function(){var g=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({decorator:a,backgroundColor:c,height:300,allowGrowY:false});g.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,maxHeight:30}),{flex:1});g.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b}),{flex:2});g.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,maxHeight:100}),{flex:3});return g;}
,getBox3:function(){var h=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({decorator:a,backgroundColor:c,height:300,allowGrowY:false});for(var i=0;i<25;i++){h.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,height:5}),{flex:1});}
;return h;}
,getBox4:function(){var j=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({decorator:a,backgroundColor:c,height:300,minHeight:300,allowGrowY:false});j.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,maxHeight:60}),{flex:1});j.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,maxHeight:60}),{flex:2});j.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,maxHeight:60}),{flex:3});return j;}
,getBox5:function(){var k=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({height:150,minHeight:0,decorator:a,backgroundColor:c,allowGrowY:false});k.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,minHeight:60}),{flex:1});k.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,minHeight:60}),{flex:2});k.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,minHeight:60}),{flex:3});return k;}
,getBox6:function(){var l=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({height:150,decorator:a,backgroundColor:c,allowGrowY:false});l.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,minHeight:60}),{flex:1});l.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,minHeight:60}),{flex:2});l.add(new qx.ui.core.Widget().set({decorator:a,backgroundColor:b,minHeight:60}),{flex:3});return l;}
}});}
)();
