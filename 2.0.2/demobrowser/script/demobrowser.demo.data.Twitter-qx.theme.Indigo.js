...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.Twitter","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Twitter-qx.theme.Indigo.cc1488bd436f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="qxc/application/twitterdemo/twitter_logo_outline.png",j="Enter",h="one",g="http",f="keydown",e="http://twitter.com/statuses/user_timeline/1and1.json",d="<a href='",c="execute",b="1and1",a="error",M="selection[0]",L="username",K="Location: ",J="Details",I="user.name",H="Name: ",G="qxc.application.twitterdemo.Demo",F="target='_blank'",E="... service unavailable!",D="</a>",r="Posted with: ",s="' target='_blank'>",p="invalid",q="Show",n="user.location",o=".json",l="http://twitter.com/statuses/user_timeline/",m="Avatar: ",t="Message: ",u="text",x="source",w="mshtml",z="engine.name",y="user.profile_image_url",B="icon",A="model",v="value",C=" ";qx.Class.define(G,{extend:qx.ui.container.Composite,construct:function(){qx.ui.container.Composite.call(this);this._createView();}
,members:{_createView:function(){this.setLayout(new qx.ui.layout.Canvas());var X=new qx.ui.basic.Image(k);this.add(X,{left:10,top:15});var R=new qx.ui.form.List();this.add(R,{left:10,top:135,bottom:5});R.set({selectionMode:h,width:300,maxHeight:400});var U=new qx.data.controller.List(null,R);U.setDelegate(this);U.setLabelPath(u);if(qx.core.Environment.get(z)!=w){U.setIconPath(y);}
;var P=e;var ba=new qx.data.store.Jsonp(P);ba.bind(A,U,A);var S=new qx.ui.container.Composite(new qx.ui.layout.HBox(5));S.set({width:300});S.add(new qx.ui.core.Spacer(),{flex:1});var N=new qx.ui.form.TextField(b);N.setPlaceholder(L);S.add(N);var V=new qx.ui.form.Button(q);S.add(V);V.addListener(c,function(){var bc=l+N.getValue()+o;if(ba.getUrl()==bc){ba.reload();}
else {ba.setUrl(bc);}
;}
,this);N.addListener(f,function(bd){if(bd.getKeyIdentifier()==j){V.execute();}
;}
,this);this.add(S,{left:10,top:105});var W=new qx.ui.basic.Label(E);W.setTextColor(p);W.hide();this.add(W,{left:290,top:75});ba.addListener(a,function(){W.show();S.setEnabled(false);R.setEnabled(false);Q.setEnabled(false);}
,this);var Q=new qx.ui.groupbox.GroupBox(J);this.add(Q,{left:320,top:116,bottom:5});Q.setWidth(270);Q.setHeight(220);Q.setAllowGrowY(false);Q.setLayout(new qx.ui.layout.Grid(5,5));Q.add(new qx.ui.basic.Label(H),{row:0,column:0});Q.add(new qx.ui.basic.Label(K),{row:1,column:0});Q.add(new qx.ui.basic.Label(t),{row:2,column:0});Q.add(new qx.ui.basic.Label(r),{row:3,column:0});var name=new qx.ui.basic.Label();Q.add(name,{row:0,column:1});var location=new qx.ui.basic.Label();Q.add(location,{row:1,column:1});var Y=new qx.ui.basic.Label();Y.setRich(true);Y.setWidth(150);Y.setSelectable(true);Q.add(Y,{row:2,column:1});var bb=new qx.ui.basic.Label();bb.setRich(true);Q.add(bb,{row:3,column:1});var O=new qx.data.controller.Object();O.addTarget(name,v,I);O.addTarget(location,v,n);O.addTarget(Y,v,u,false,{converter:function(be){var bf=be.split(C);for(var i=bf.length-1;i>=0;i--){if(bf[i].indexOf(g)==0){bf[i]=d+bf[i]+s+bf[i]+D;}
;}
;return bf.join(C);}
});O.addTarget(bb,v,x,false,{converter:function(bg){bg=bg.split(C);bg.splice(1,0,F);return bg.join(C);}
});if(qx.core.Environment.get(z)!=w){Q.add(new qx.ui.basic.Label(m),{row:4,column:0});var T=new qx.ui.basic.Image();Q.add(T,{row:4,column:1});O.addTarget(T,x,y);}
;U.bind(M,O,A);}
,configureItem:function(bh){bh.setRich(true);bh.getChildControl(B).setWidth(48);bh.getChildControl(B).setHeight(48);bh.getChildControl(B).setScale(true);}
}});}
)();
