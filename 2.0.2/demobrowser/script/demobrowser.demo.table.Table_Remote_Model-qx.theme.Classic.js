...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Remote_Model","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Remote_Model-qx.theme.Classic.b00ca7d1640d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var f="qx.ui.table.model.Remote",e="Boolean",d="Integer",c="_loadRowCount is abstract",b="metaDataChanged",a="dataChanged";qx.Class.define(f,{extend:qx.ui.table.model.Abstract,construct:function(){qx.ui.table.model.Abstract.call(this);this.__mX=-1;this.__nc=true;this.__KU=-1;this.__KV=0;this.__KW=-1;this.__KX=-1;this.__KY=-1;this.__La=false;this.__Lb={};this.__Lc=0;this.__nb=null;this.__na=null;}
,properties:{blockSize:{check:d,init:50},maxCachedBlockCount:{check:d,init:15},clearCacheOnRemove:{check:e,init:false},blockConcurrentLoadRowCount:{check:e,init:true}},members:{__KU:null,__La:null,__KV:null,__KW:null,__KX:null,__KY:null,__Lb:null,__Lc:null,__mX:null,__nc:null,__na:null,__nb:null,__Ld:false,_getIgnoreCurrentRequest:function(){return this.__La;}
,getRowCount:function(){if(this.__KU==-1){if(!this.__Ld||!this.getBlockConcurrentLoadRowCount()){this.__Ld=true;this._loadRowCount();}
;return (this.__KU==-1)?0:this.__KU;}
else {return this.__KU;}
;}
,_loadRowCount:function(){throw new Error(c);}
,_onRowCountLoaded:function(g){if(this.getBlockConcurrentLoadRowCount()){this.__Ld=false;}
;if(g==null||g<0){g=0;}
;this.__KU=Number(g);var h={firstRow:0,lastRow:g-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,h);}
,reloadData:function(){this.clearCache();if(this.__KW!=-1){var j=this._cancelCurrentRequest();if(j){this.__KW=-1;this.__La=false;}
else {this.__La=true;}
;}
;this.__KX=-1;this.__KY=-1;this.__Ld=true;this._loadRowCount();}
,clearCache:function(){this.__Lb={};this.__Lc=0;}
,getCacheContent:function(){return {sortColumnIndex:this.__mX,sortAscending:this.__nc,rowCount:this.__KU,lruCounter:this.__KV,rowBlockCache:this.__Lb,rowBlockCount:this.__Lc};}
,restoreCacheContent:function(k){if(this.__KW!=-1){var m=this._cancelCurrentRequest();if(m){this.__KW=-1;this.__La=false;}
else {this.__La=true;}
;}
;this.__mX=k.sortColumnIndex;this.__nc=k.sortAscending;this.__KU=k.rowCount;this.__KV=k.lruCounter;this.__Lb=k.rowBlockCache;this.__Lc=k.rowBlockCount;var l={firstRow:0,lastRow:this.__KU-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,l);}
,_cancelCurrentRequest:function(){return false;}
,iterateCachedRows:function(n,o){var q=this.getBlockSize();var p=Math.ceil(this.getRowCount()/q);for(var x=0;x<=p;x++){var r=this.__Lb[x];if(r!=null){var w=x*q;var v=r.rowDataArr;for(var u=0;u<v.length;u++){var t=v[u];var s=n.call(o,w+u,t);if(s!=null){v[u]=s;}
;}
;}
;}
;}
,prefetchRows:function(y,z){if(this.__KW==-1){var A=this.getBlockSize();var F=Math.ceil(this.__KU/A);var E=parseInt(y/A,10)-1;if(E<0){E=0;}
;var D=parseInt(z/A,10)+1;if(D>=F){D=F-1;}
;var C=-1;var B=-1;for(var G=E;G<=D;G++){if(this.__Lb[G]==null||this.__Lb[G].isDirty){if(C==-1){C=G;}
;B=G;}
;}
;if(C!=-1){this.__KX=-1;this.__KY=-1;this.__KW=C;this._loadRowData(C*A,(B+1)*A-1);}
;}
else {this.__KX=y;this.__KY=z;}
;}
,_loadRowData:function(H,I){throw new Error(c);}
,_onRowDataLoaded:function(J){if(J!=null&&!this.__La){var M=this.getBlockSize();var K=Math.ceil(J.length/M);if(K==1){this._setRowBlockData(this.__KW,J);}
else {for(var i=0;i<K;i++){var P=i*M;var O=[];var L=Math.min(M,J.length-P);for(var Q=0;Q<L;Q++){O.push(J[P+Q]);}
;this._setRowBlockData(this.__KW+i,O);}
;}
;var N={firstRow:this.__KW*M,lastRow:(this.__KW+K+1)*M-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(a,N);}
;this.__KW=-1;this.__La=false;if(this.__KX!=-1){this.prefetchRows(this.__KX,this.__KY);}
;}
,_setRowBlockData:function(R,S){if(this.__Lb[R]==null){this.__Lc++;while(this.__Lc>this.getMaxCachedBlockCount()){var W;var V=this.__KV;for(var U in this.__Lb){var T=this.__Lb[U].lru;if(T<V&&U>1){V=T;W=U;}
;}
;delete this.__Lb[W];this.__Lc--;}
;}
;this.__Lb[R]={lru:++this.__KV,rowDataArr:S};}
...
else {var bb=this.getBlockSize();var bc=Math.ceil(this.getRowCount()/bb);var bd=parseInt(X/bb,10);for(var bg=bd;bg<=bc;bg++){var Y=this.__Lb[bg];if(Y!=null){var ba=0;if(bg==bd){ba=X-bg*bb;}
;Y.rowDataArr.splice(ba,1);if(bg==bc-1){if(Y.rowDataArr.length==0){delete this.__Lb[bg];}
;}
else {var be=this.__Lb[bg+1];if(be!=null){Y.rowDataArr.push(be.rowDataArr[0]);}
else {Y.isDirty=true;}
;}
;}
;}
;if(this.__KU!=-1){this.__KU--;}
...
;}
;}
,getRowData:function(bh){var bi=this.getBlockSize();var bl=parseInt(bh/bi,10);var bj=this.__Lb[bl];if(bj==null){return null;}
else {var bk=bj.rowDataArr[bh-(bl*bi)];if(bj.lru!=this.__KV){bj.lru=++this.__KV;}
;return bk;}
;}
...
;}
;}
,setEditable:function(bw){this.__na=[];for(var bx=0;bx<this.getColumnCount();bx++){this.__na[bx]=bw;}
;this.fireEvent(b);}
,setColumnEditable:function(by,bz){if(bz!=this.isColumnEditable(by)){if(this.__na==null){this.__na=[];}
;this.__na[by]=bz;this.fireEvent(b);}
;}
,isColumnEditable:function(bA){return (this.__na?(this.__na[bA]==true):false);}
,setColumnSortable:function(bB,bC){if(bC!=this.isColumnSortable(bB)){if(this.__nb==null){this.__nb=[];}
;this.__nb[bB]=bC;this.fireEvent(b);}
;}
,isColumnSortable:function(bD){return (this.__nb?(this.__nb[bD]!==false):true);}
,sortByColumn:function(bE,bF){if(this.__mX!=bE||this.__nc!=bF){this.__mX=bE;this.__nc=bF;this.clearCache();this.fireEvent(b);}
;}
,getSortColumnIndex:function(){return this.__mX;}
,isSortAscending:function(){return this.__nc;}
,setSortColumnIndexWithoutSortingData:function(bG){this.__mX=bG;}
,setSortAscendingWithoutSortingData:function(bH){this.__nc=bH;}
},destruct:function(){this.__nb=this.__na=this.__Lb=null;}
});}
)();
...
,members:{__QL:null,__QM:false,_loadRowCount:function(){this.__QN();if(this.__QM){return;}
;if(this.__QL){this.__QO();}
else {this.__QR();}
;}
,_loadRowData:function(f,g){this.__QN();if(this.__QM){return;}
;if(this.__QL){this.__QP(f,g);}
else {this.__QS(f,g);}
;}
,__QN:function(){if(this.__QM||this.__QL!==null){return;}
;this.__QM=true;this.__QQ("method=checkphp",function(h){this.__QM=false;this.__QL=(h=="WTF PHP");this._loadRowCount();}
);}
,__QO:function(){var j="method=getRowCount";this.__QQ(j,function(k){this._onRowCountLoaded(parseInt(k));}
);}
,__QP:function(l,m){var n="method=getRowData&start="+l+"&end="+m;this.__QQ(n,function(o){this._onRowDataLoaded(qx.lang.Json.parse(o));}
);}
,__QQ:function(p,q){var r=new qx.io.request.Xhr();r.setUrl("../../resource/demobrowser/backend/remote_table.php?"+p);r.addListener("success",function(){q.call(this,r.getResponseText());}
,this);r.send();}
,__QR:function(){var self=this;window.setTimeout(function(){self._onRowCountLoaded(1000000);}
,0);}
,__QS:function(s,t){var self=this;window.setTimeout(function(){var u=[];for(var i=s;i<=t;i++){u.push({id:i,text:'Hello '+i+' Generated on:'+(new Date())});}
