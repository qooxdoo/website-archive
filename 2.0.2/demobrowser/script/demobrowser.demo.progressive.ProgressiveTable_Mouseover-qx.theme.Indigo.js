...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.progressive.ProgressiveTable_Mouseover-qx.theme.Indigo.7cc96453ebf8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var s="",r="rgb(255, 0, 0)",q="celldata",p="onmouseover='demobrowser.demo.progressive.DateCellRenderer.onmouseover(this);' ",o="Number",n="red",m="1*",l="celldata='",k="' ",j="<br>",d="demobrowser.demo.progressive.ProgressiveTable_Mouseover",i="end",g="Date",c="Move your mouse over the Date field",b="demobrowser.demo.progressive.DateCellRenderer",f="color",e="Id",h="onmouseout='demobrowser.demo.progressive.DateCellRenderer.onmouseout(this);' ",a="row";qx.Class.define(d,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var C=0;var y=function(F){var I=[];var H=new Date().getTime();var J=400*24*60*60*1000;for(var K=0;K<F;K++){var G=new Date(H+Math.random()*J-J/2);I.push({renderer:a,location:i,data:[C++,Math.random()*10000,G]});}
;return I;}
;var B=new qx.ui.progressive.renderer.table.Widths(3);B.setWidth(0,100);B.setWidth(1,m);B.setWidth(2,300);var v=[e,o,g];var x=new qx.ui.progressive.headfoot.TableHeading(B,v);var u=new qx.ui.progressive.headfoot.Progress(B,v);var E=new qx.ui.progressive.structure.Default(x,u);var t=new qx.ui.progressive.Progressive(E);var D=new qx.ui.basic.Atom(c);D.setTextColor(n);D.setHeight(16);D.setRich(true);t.add(D);var A=new qx.ui.progressive.model.Default();var z=y(500);A.addElements(z);t.setDataModel(A);var w=new qx.ui.progressive.renderer.table.Row(B);t.addRenderer(a,w);w.addRenderer(2,new demobrowser.demo.progressive.DateCellRenderer());this.getRoot().add(t,{edge:50});t.render();}
}});qx.Class.define(b,{extend:qx.ui.progressive.renderer.table.cell.Default,statics:{onmouseover:function(L){L.__QH=qx.bom.element.Style.get(L,f);L.style.color=r;L.parentNode.style.height=32;L.__QI=L.innerHTML;var N=Date.parse(L.attributes.getNamedItem(q).nodeValue);var M=new Date(N);L.innerHTML=M.toUTCString()+j+M.toString();}
,onmouseout:function(O){O.style.color=O.__QH;O.parentNode.style.height=16;O.innerHTML=O.__QI;}
},members:{_getCellExtras:function(P){var Q=[];Q.push(qx.ui.progressive.renderer.table.cell.Default.prototype._getCellExtras.call(this));Q.push(l+P.cellData.toUTCString()+k);Q.push(p,h);return Q.join(s);}
}});}
)();
