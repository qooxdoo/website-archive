...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Table_RowRenderer","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Table_RowRenderer-qx.theme.Modern.8bfb6c26401b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var r='>',q="",p=";position:relative;height:",o="table",n=' (init)</div>',m='class="',l=" (update)",k="relative",j="div",i='style="',c="px;",h="px; width:",f=": ",b="demobrowser.demo.test.Table_RowRenderer",a=", ",e='<div ',d="px",g='" ';qx.Class.define(b,{extend:qx.application.Native,include:[demobrowser.demo.table.MUtil],members:{main:function(){qx.application.Native.prototype.main.call(this);qx.theme.manager.Meta.getInstance().initialize();this.setUp();this.testDefaultRenderer();}
,setUp:function(){this.tableMock=this.getTableMock();}
,testDefaultRenderer:function(s){var s=new qx.ui.table.rowrenderer.Default();var w=300;var v=20;var u={focusedRow:[true,false],selected:[true,false],row:[1,2],table:[this.tableMock]};var t=this._getNewTableDiv(w);var self=this;qx.util.Permutation.permute(u,function(x){var content=[];for(var B in x){if(B!==o){content.push(B+f+x[B]);}
;}
;content=content.join(a);var C=self._createRowElement(t,w,v);var y=s.getRowClass(x);if(y){C.className=y;}
;C.innerHTML=content+l;s.updateDataRowElement(x,C);var A=[];A.push(e);if(y){A.push(m,y,g);}
;var z=s.createRowStyle(x);z+=p+v+h+w+c;if(z){A.push(i,z,g);}
;A.push(r,content,n);t.innerHTML+=A.join(q);self._rowTop+=v;}
);}
,_rowTop:0,_createRowElement:function(parent,D,E){var F=qx.dom.Element.create(j);qx.bom.element.Style.setStyles(F,{position:k,width:D+d,height:E+d});this._rowTop+=20;parent.appendChild(F);return F;}
