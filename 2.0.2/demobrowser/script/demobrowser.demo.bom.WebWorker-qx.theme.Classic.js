...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.bom.WebWorker","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.WebWorker-qx.theme.Classic.7cd78df8773d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="demobrowser/demo/webworker/webworker.js",l="Your browser doesn't support Web Workers",k="30",j="message",i="html.webworker",h="Calculating...",g="demobrowser.demo.bom.WebWorker",f="Your browser supports Web Workers",d="execute",c="Calculate Fibonacci Numbers in a Web Worker",a="",b="Calculate";qx.Class.define(g,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var r=this.getRoot();var q=qx.util.ResourceManager.getInstance().toUri(m);var s=new qx.bom.WebWorker(q);var u=new qx.ui.basic.Label(c);var n=new qx.ui.basic.Label(a);var p=new qx.ui.form.Button(b).set({width:100});var o=new qx.ui.form.TextField(k);var t=new qx.ui.form.List().set({width:200});if(qx.core.Environment.get(i)){n.setValue(f);}
else {n.setValue(l);}
;r.add(n,{left:20,top:10});r.add(u,{left:20,top:40});r.add(o,{left:20,top:60});r.add(p,{left:120,top:58});r.add(t,{left:20,top:90});s.addListener(j,function(e){var v=e.getData();t.addAt(new qx.ui.form.ListItem(a+v.result),0);if(v.done){p.setEnabled(true);o.setEnabled(true);p.setLabel(b);}
;}
);p.addListener(d,function(e){t.removeAll();s.postMessage(parseInt(o.getValue()||0,10));p.setEnabled(false);o.setEnabled(false);p.setLabel(h);}
,this);}
}});}
)();
(function(){var k="GET",j="onmessage",i='message',h="html.webworker",g="var onmessage = null, postMessage = ",f="qx.bom.WebWorker",d=";",c="qx.event.type.Data",b="message",a="error";qx.Class.define(f,{extend:qx.core.Object,construct:function(l){qx.core.Object.call(this);this.__Ke=qx.core.Environment.get(h);this.__Ke?this.__Kg(l):this.__Kh(l);}
,events:{"message":c,"error":c},members:{_worker:null,_handleErrorBound:null,_handleMessageBound:null,__Ke:true,__Kf:null,__Kg:function(m){this._worker=new window.Worker(m);this._handleMessageBound=qx.lang.Function.bind(this._handleMessage,this);this._handleErrorBound=qx.lang.Function.bind(this._handleError,this);qx.bom.Event.addNativeListener(this._worker,b,this._handleMessageBound);qx.bom.Event.addNativeListener(this._worker,a,this._handleErrorBound);}
,__Kh:function(n){var p=this;var o=new qx.bom.request.Xhr();o.onload=function(){p.__Kf=(function(){var q=function(e){p.fireDataEvent(i,e);}
;eval(g+q+d+o.responseText);return {onmessage:eval(j),postMessage:q};}
)();}
;o.open(k,n,false);o.send();}
,postMessage:function(r){var s=this;if(this.__Ke){this._worker.postMessage(r);}
else {setTimeout(function(){try{s.__Kf.onmessage&&s.__Kf.onmessage({data:r});}
catch(t){s.fireDataEvent(a,t);}
;}
,0);}
;}
,_handleMessage:function(e){this.fireDataEvent(b,e.data);}
,_handleError:function(e){this.fireDataEvent(a,e.message);}
},destruct:function(){if(this.__Ke){qx.bom.Event.removeNativeListener(this._worker,b,this._handleMessageBound);qx.bom.Event.removeNativeListener(this._worker,a,this._handleErrorBound);if(this._worker){this._worker.terminate();this._worker=null;}
;}
else {if(this.__Kf){this.__Kf=null;}
;}
;}
});}
)();
