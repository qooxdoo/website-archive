...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.progressive.ProgressiveTable_Boolean-qx.theme.Indigo.9b53f631d25c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j="\"",i="Boolean",h="  this.attributes.setNamedItem(namedItem); ",g="} catch (e) { ",f="var node = this.attributes.getNamedItem('celldata'); ",e="qx.ui.progressive.renderer.table.cell.Boolean",d="decoration/table/boolean-true.png",c="var value = node.nodeValue; ",b="decoration/table/boolean-false.png",a="celldata='1' ",D="  node.nodeValue='0'; ",C="  this.attributes.setNamedItem(node); ",B="if (value == '0') ",A="celldata='0' ",z="  node.nodeValue='1'; ",y="onclick=\"",x="  namedItem.value = node.nodeValue; ",w="var src; ",v="try { ",u="  var namedItem = document.createAttribute('celldata'); ",q="} ",r="else ",o="  var filters = this.filters.item(loader); ",p="}",m="{",n="  var loader = 'DXImageTransform.Microsoft.AlphaImageLoader'; ",k="  filters.sizingMethod = 'scale'; ",l="  filters.src='",s="  this.src='",t="'; ";qx.Class.define(e,{extend:qx.ui.progressive.renderer.table.cell.Icon,construct:function(){qx.ui.progressive.renderer.table.cell.Icon.call(this);var G=qx.util.AliasManager.getInstance();var H=qx.util.ResourceManager.getInstance();var F=G.resolve(d);var E=G.resolve(b);this.__zf=H.toUri(F);this.__zg=H.toUri(E);}
,properties:{allowToggle:{check:i,init:false}},members:{__zf:null,__zg:null,__Kn:null,__Kp:null,__yY:null,__za:null,__zb:null,__zc:null,_identifyImage:function(I){var J={imageWidth:11,imageHeight:11};switch(I.cellData){case true:J.url=this.__zf;J.extras=a;break;case false:J.url=this.__zg;J.extras=A;break;default:J.url=null;break;};if(this.getAllowToggle()){J.extras+=y+f+c+w+B+m;if(qx.bom.element.Decoration.isAlphaImageLoaderEnabled()&&/\.png$/i.test(this.__zf)){J.extras+=s+this.getBlankImage()+t+n+o+l+this.__zf+t+k;}
else {J.extras+=s+this.__zf+t;}
;J.extras+=z+q+r+m;if(qx.bom.element.Decoration.isAlphaImageLoaderEnabled()&&/\.png$/i.test(this.__zg)){J.extras+=s+this.getBlankImage()+t+n+o+l+this.__zg+t+k;}
else {J.extras+=s+this.__zg+t;}
;J.extras+=D+p;J.extras+=v+C+g+u+x+h+p+j;}
;return J;}
,_getCellStyle:function(K){var L=qx.ui.progressive.renderer.table.cell.Icon.prototype._getCellStyle.call(this,K);return L;}
},destruct:function(){this.__zf=this.__zg=null;}
});}
)();
