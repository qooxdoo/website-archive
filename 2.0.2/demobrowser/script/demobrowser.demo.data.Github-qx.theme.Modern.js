...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Github-qx.theme.Modern.c0b9755b58a0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;return H;}
},members:{__Mm:null,__JX:null,__Mp:null,__Mn:null,__Mo:null,__Mq:null,configureRequest:function(K){this.__Mq=K;}
,_getRequest:function(){return new qx.io.request.Xhr();}
,__Mr:function(L){var M=this._getRequest();if(!qx.lang.Type.isArray(this.__Mm[L])){this.__Mm[L]=[];}
;this.__Mm[L].push(M);return M;}
,map:function(N,O,P,Q){this.__JX[N]=[O,P,Q];this.__Mm[N]=[];if(N==z){this[N]=undefined;}
;if(typeof this[N]!==u&&this[N].action!==true){if(this[N]!==qx.lang.Function.empty){throw new Error(C+N+x);}
;}
;this.__Mv(N+l);this.__Mv(N+n);this[N]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,N);return this.invoke.apply(this,arguments);}
,this);this[N].action=true;}
,invoke:function(R,S,T){var V=this.__Mr(R),S=S==null?{}:S,Y=this._getRequestConfig(R,S);this.__JX[R].params=S;this.__Ms(S,Y.check);this.__Jo(V,Y,T);if(this.__Mq){this.__Mq.call(this,V,R,S,T);}
;this.__Mt(V,Y,T);V.addListenerOnce(q,function X(){var ba=[V.getResponse(),null,false,V,R,V.getPhase()];this.fireEvent(R+l,qx.event.type.Rest,ba);this.fireEvent(q,qx.event.type.Rest,ba);}
...
,setBaseUrl:function(bc){this.__Mp=bc;}
,__Ms:function(bd,be){if(typeof be!==u){if(qx.core.Environment.get(t)){qx.core.Assert.assertObject(be,p);}
...
;}
,__Jo:function(bg,bh,bi){bg.set({method:bh.method,url:bh.url});if(bi){bg.setRequestData(bi);}
;}
,__Mt:function(bj,bk,bl){if(bl){var bm=bj.getRequestHeader(s);if(qx.util.Request.methodAllowsRequestBody(bj.getMethod())){if((/application\/.*\+?json/).test(bm)){bl=qx.lang.Json.stringify(bl);bj.setRequestData(bl);}
;}
;}
;}
...
;}
else {var bo=bn;var bs=this.__Mm[bo];if(this.__Mm[bo]){bs.forEach(function(bt){bt.abort();}
);}
;}
;}
,refresh:function(bu){this.invoke(bu,this.__JX[bu].params);}
,poll:function(bv,bw,bx){if(this.__Mn[bv]){this.__Mn[bv].dispose();}
;if(typeof bx==u){bx=this.__JX[bv].params;}
;this.invoke(bv,bx);var by=this.__Mn[bv]=new qx.event.Timer(bw);by.addListener(A,function bz(){var bA=this.__Mm[bv][0];if(bA.isDone()||bA.isDisposed()){this.refresh(bv);}
;}
...
;if(!bJ){bH=0;}
;return false;}
;var bE=this.__Mo[bB]=this.addListener(bB+l,function bI(){if(bD.isDisposed()){return;}
;if(!bC()){bG=new Date();bD.refresh(bB);}
;}
,bF);this.invoke(bB);return bE;}
,_getRequestConfig:function(bK,bL){var bN=this.__JX[bK];var bL=qx.lang.Object.clone(bL);if(!qx.lang.Type.isArray(bN)){throw new Error(E+bK);}
;var bQ=bN[0],bM=this.__Mp!==null?this.__Mp+bN[1]:bN[1],bO=bN[2],bP=qx.io.rest.Resource.placeholdersFromUrl(bM);bL=bL||{};bP.forEach(function(bR){var bS=new RegExp(y+bR+i),bT=bM.match(bS)[1];if(typeof bL[bR]===u){if(bT){bL[bR]=bT;}
else {bL[bR]=w;}
;}
...
,__Mu:function(bU){qx.lang.Object.getKeys(bU).forEach(function(bV){var bX=bU[bV],bY=bX.method,bW=bX.url,ca=bX.check;if(qx.core.Environment.get(t)){qx.core.Assert.assertString(bY,r+bV+o);qx.core.Assert.assertString(bW,D+bV+o);}
;this.map(bV,bY,bW,ca);}
,this);}
,__Mv:function(cb){if(!this.constructor.$$events){this.constructor.$$events={};}
;if(!this.constructor.$$events[cb]){this.constructor.$$events[cb]=m;}
;}
},destruct:function(){var cc;for(cc in this.__Mm){if(this.__Mm[cc]){this.__Mm[cc].forEach(function(cf){cf.dispose();}
);}
;}
;if(this.__Mn){for(cc in this.__Mn){var ce=this.__Mn[cc];ce.stop();ce.dispose();}
;}
;if(this.__Mo){for(cc in this.__Mo){var cd=this.__Mo[cc];this.removeListenerById(cd);}
;}
;this.__Mm=this.__JX=this.__Mn=null;}
});}
)();
...
(function(){var j="changeModel",i="model.user.avatar_url",h="application/json",g="github.view.Gist",f="model.description",e="west",d="/gists/{id}",c="Description",b="https://api.github.com",a="demobrowser.demo.data.Github",H="source",G="/gists",F="change",E="Content",D="center",C="#fff",B="Authorization",A="html",z="model.user.login",y="model.files",q="north",r="Gists",o="Accept",p="token ",m="github.rest.Resource",n="description",k="undefined",l="</pre>",s="<pre>",t="GET",v="value",u="get",x="model",w="bold";qx.Class.define(a,{extend:qx.application.Standalone,members:{__PW:null,__PX:null,__PY:null,__Qa:null,__Di:null,__Qb:null,main:function(){if(typeof GITHUB==k){return;}
;qx.application.Standalone.prototype.main.call(this);this._setUpResources();this._setUpStores();this._createGui();this._setUpBinding();this.__PW.get();this.__Di.addListener(j,function(I){var J=I.getData();this.__Di.getSelection().push(J.getItem(0));}
,this);this.__Di.getSelection().addListener(F,function(K){var L=this.__Di.getSelection().getItem(0).getId();this.__PX.get({id:L});}
,this);}
,_setUpResources:function(){this.__PW=new github.rest.Resource({"get":{method:t,url:G}});this.__PX=new github.rest.Resource({"get":{method:t,url:d}});}
,_setUpStores:function(){this.__PY=new qx.data.store.Rest(this.__PW,u);this.__Qa=new qx.data.store.Rest(this.__PX,u);}
,_createGui:function(){var M=new qx.ui.container.Composite(new qx.ui.layout.Dock());M.setPadding(10);var N=new qx.ui.basic.Label(r);N.setFont(w);N.setPaddingBottom(10);M.add(N,{edge:q});this.__Di=new qx.ui.list.List();this.__Di.setWidth(200);this.__Qb=new github.view.Gist();M.add(this.__Di,{edge:e});M.add(this.__Qb,{edge:D});this.getRoot().add(M,{edge:0});}
,_setUpBinding:function(){var P=this.__Di,O=this.__PY,Q=this.__Qa;P.setLabelPath(n);P.setLabelOptions({converter:function(S,T,U,V){if(S===null||!S.length){return T.getId();}
;return S;}
});O.bind(x,P,x);var R=this.__Qb;Q.bind(f,R.getDescription(),v);Q.bind(z,R.getUsername(),v);Q.bind(i,R.getGravatar(),H);Q.bind(y,R.getContent(),A,{converter:function(W){var X=qx.Class.getProperties(W.constructor);var content=W.get(X[0]).getContent();content=qx.bom.String.escape(content);return s+content+l;}
});}
...
});qx.Class.define(g,{extend:qx.ui.container.Composite,construct:function(){qx.ui.container.Composite.call(this);var bb=new qx.ui.layout.Grid(5,5);bb.setColumnFlex(0,1);bb.setRowFlex(2,1);this.setLayout(bb);this.setPadding(0,10);var be;var bc=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));be=new qx.ui.basic.Label(c);be.setFont(w);this.__Co=new qx.ui.basic.Label();bc.add(be);bc.add(this.__Co);this.add(bc,{row:0,column:0});var bd=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));this.__Qc=new qx.ui.basic.Image();this.__Qc.set({width:40,height:40,scale:true,backgroundColor:C});this.__Qd=new qx.ui.basic.Label();bd.add(this.__Qc);bd.add(this.__Qd);this.add(bd,{row:0,column:1});be=new qx.ui.basic.Label(E);be.setFont(w);var scroll=new qx.ui.container.Scroll();this.__mr=new qx.ui.embed.Html();this.__mr.setMinHeight(1000);scroll.add(this.__mr);this.add(be,{row:1,column:0});this.add(scroll,{row:2,column:0,colSpan:2});}
,members:{getDescription:function(){return this.__Co;}
,getGravatar:function(){return this.__Qc;}
,getUsername:function(){return this.__Qd;}
,getContent:function(){return this.__mr;}
}});}
)();
...
;this._delegate=j;this._marshaler=new qx.data.marshal.Json(j);if(j&&qx.lang.Type.isFunction(j.configureRequest)){this.__Jo();}
;this.__Jn=qx.lang.Function.bind(this.__Jp,this);this.__xD();}
,properties:{resource:{check:d},actionName:{check:b},model:{nullable:true,event:g}},members:{_marshaler:null,_delegate:null,__Jn:null,__Jo:function(){var k=this.getResource(),l=this._delegate;k.configureRequest(l.configureRequest);}
,__xD:function(){var n=this.getResource(),m=this.getActionName();if(n&&m){n.addListener(this.getActionName()+c,this.__Jn);}
;}
,__Jp:function(e){var p=e.getData(),o=this._marshaler,q,r=this.getModel(),s=this._delegate;if(p){if(s&&s.manipulateData){p=s.manipulateData(p);}
