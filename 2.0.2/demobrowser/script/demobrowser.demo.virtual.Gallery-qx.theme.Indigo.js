...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.virtual.Gallery","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Gallery-qx.theme.Indigo.c2b9de14841c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="Gallery (HTML - divs)",k="manager",j="background",h="demobrowser.demo.virtual.Gallery",g="demobrowser.demo.virtual.WidgetGallery",f="selected",d="text-align: center;",c="background-selected",b="auto",a="color: white; background-color: #00398D;",R="user-desktop.png",Q="__Rs",P="<img src='",O="'></img>",N="resize",M="folder.png",L="vertical",K="multi",J="network-server.png",I="user-trash.png",s="off",t="white-box",q="top",r="icon/32/places/",o="horizontal",p="demobrowser.demo.virtual.AbstractGallery",m="default",n="abstract",u="position: absolute;",v="demobrowser.demo.virtual.HtmlGallery",A="Gallery (widgets)",z="<br>",C="Icon #",B="network-workgroup.png",E="text-selected",D="demobrowser.demo.virtual.GalleryCell",x="cell",H="layer",G="icon",F="#f3f3f3",w="label",y="";qx.Class.define(h,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);new demobrowser.demo.virtual.WidgetGallery(A);var S=new demobrowser.demo.virtual.HtmlGallery(l);S.moveTo(400,50);}
}});qx.Class.define(p,{extend:qx.ui.window.Window,type:n,construct:function(T){qx.ui.window.Window.call(this,T);this.set({contentPadding:0,showClose:false,showMinimize:false,width:320,height:400});this.setLayout(new qx.ui.layout.Grow());this.moveTo(30,50);this.open();this.itemHeight=65;this.itemWidth=70;this.itemCount=431;this.itemPerLine=1;this.items=this._generateItems(this.itemCount);var U=this._createScroller();U.set({scrollbarX:s,scrollbarY:b});U.getPane().addListener(N,this._onPaneResize,this);this.add(U);this.manager=new qx.ui.virtual.selection.CellRectangle(U.getPane(),this).set({mode:K,drag:true});this.manager.attachMouseEvents();this.manager.attachKeyEvents(U);}
,members:{getItemData:function(V,W){return this.items[V*this.itemPerLine+W];}
,_createScroller:function(){}
,isItemSelectable:function(X){return !!this.getItemData(X.row,X.column);}
,styleSelectable:function(Y,ba,bb){}
,_onPaneResize:function(e){var be=e.getTarget();var bc=e.getData().width;var bd=Math.max(1,Math.floor(bc/this.itemWidth));if(bd==this.itemsPerLine){return;}
;this.itemPerLine=bd;var bf=Math.ceil(this.itemCount/bd);be.getColumnConfig().setItemCount(bd);be.getRowConfig().setItemCount(bf);}
,_generateItems:function(bg){var bj=[];var bm=[M,I,J,B,R];var bk=qx.util.AliasManager.getInstance();var bi=qx.util.ResourceManager.getInstance();for(var i=0;i<bg;i++){var bn=r+bm[Math.floor(Math.random()*bm.length)];var bh=bk.resolve(bn);var bl=bi.toUri(bh);bj[i]={label:C+(i+1),icon:bn,resolvedIcon:bl};}
;return bj;}
},destruct:function(){this.items=null;this._disposeObjects(k);}
});qx.Class.define(D,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var bo=new qx.ui.basic.Atom().set({iconPosition:q});bo.getChildControl(w).set({padding:[0,4]});bo.getChildControl(G).set({padding:4});return bo;}
,updateData:function(bp,bq){bp.set({icon:bq.icon,label:bq.label});}
,updateStates:function(br,bs){var bu=br.getChildControl(w);var bt=br.getChildControl(G);if(bs.selected){bu.setBackgroundColor(c);bu.setTextColor(E);bt.setDecorator(t);bt.setBackgroundColor(j);}
else {bu.resetBackgroundColor();bu.resetTextColor();bt.resetDecorator();bt.resetBackgroundColor();}
;}
}});qx.Class.define(g,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bv){demobrowser.demo.virtual.AbstractGallery.call(this,bv);this.__Rs=new demobrowser.demo.virtual.GalleryCell();}
,members:{_createScroller:function(){var bw=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.WidgetCell(this);bw.getPane().addLayer(this.layer);new qx.ui.virtual.behavior.Prefetch(bw,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:200,maxAbove:300,minBelow:600,maxBelow:800}).set({interval:500});return bw;}
,styleSelectable:function(bx,by,bz){if(by!==f){return;}
;var bA=this.layer.getChildren();for(var i=0;i<bA.length;i++){var bB=bA[i];var bC=bB.getUserData(x);if(bx.row!==bC.row||bx.column!==bC.column){continue;}
;if(bz){this.__Rs.updateStates(bB,{selected:1});}
else {this.__Rs.updateStates(bB,{});}
;}
;}
,getCellWidget:function(bD,bE){var bI=this.getItemData(bD,bE);if(!bI){return null;}
;var bF={row:bD,column:bE};var bG={};if(this.manager.isItemSelected(bF)){bG.selected=true;}
;var bH=this.__Rs.getCellWidget(bI,bG);bH.setUserData(x,bF);return bH;}
,poolCellWidget:function(bJ){this.__Rs.pool(bJ);}
},destruct:function(){this._disposeObjects(Q,H);}
});qx.Class.define(v,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bK){demobrowser.demo.virtual.AbstractGallery.call(this,bK);var bL=qx.theme.manager.Font.getInstance().resolve(m).getStyles();this._fontCss=qx.bom.element.Style.compile(bL);}
,members:{_createScroller:function(){var bN=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.HtmlCell(this);bN.getPane().addLayer(this.layer);var bM=new qx.ui.virtual.layer.GridLines(o,F);bN.getPane().addLayer(bM);var bM=new qx.ui.virtual.layer.GridLines(L,F);bN.getPane().addLayer(bM);return bN;}
,_onPaneResize:function(e){demobrowser.demo.virtual.AbstractGallery.prototype._onPaneResize.call(this,e);this.manager.clearSelection();}
,styleSelectable:function(bO,bP,bQ){this.layer.updateLayerData();}
,getCellProperties:function(bR,bS){var bV=this.getItemData(bR,bS);if(!bV){return y;}
;var bU=this.manager.isItemSelected({row:bR,column:bS});var bT=bU?a:y;return {style:[u,d,this._fontCss,bT].join(y),content:[P,bV.resolvedIcon,O,z,bV.label].join(y)};}
...
;}
,properties:{scroller:{check:e,nullable:true,init:null,apply:f},interval:{check:d,init:200,apply:l}},members:{__uS:null,__uT:null,__iG:null,__uU:null,__uV:null,setPrefetchX:function(o,p,q,r){this.__uS=[o,p,q,r];}
,setPrefetchY:function(s,t,u,v){this.__uT=[s,t,u,v];}
,_onInterval:function(){var w=this.__uS;if(w[1]&&w[3]){this.getScroller().getPane().prefetchX(w[0],w[1],w[2],w[3]);qx.ui.core.queue.Manager.flush();}
;var x=this.__uT;if(x[1]&&x[3]){this.getScroller().getPane().prefetchY(x[0],x[1],x[2],x[3]);qx.ui.core.queue.Manager.flush();}
;}
,_applyScroller:function(y,z){if(z){if(this.__uU){z.getChildControl(a).removeListenerById(this.__uU);}
;if(this.__uV){z.getChildControl(b).removeListenerById(this.__uV);}
;}
...
;this.__uU=y.getChildControl(a).addListener(c,this.__iG.restart,this.__iG);this.__uV=y.getChildControl(b).addListener(c,this.__iG.restart,this.__iG);}
else {this.__iG.stop();}
;}
,_applyInterval:function(A,B){this.__iG.setInterval(A);}
},destruct:function(){this.setScroller(null);this.__uS=this.__uT=null;this._disposeObjects(h);}
});}
)();
...
(function(){var h="",g="gray",f="qx.ui.virtual.layer.GridLines",e="Color",d="PositiveInteger",c="html",b="horizontal",a="qx.debug";qx.Class.define(f,{extend:qx.ui.virtual.layer.Abstract,construct:function(i,j,k){qx.ui.virtual.layer.Abstract.call(this);this.setZIndex(1);if(j){this.setDefaultLineColor(j);}
;if(k!==undefined){this.setDefaultLineSize(k);}
;this.__IK=[];this.__IL=[];this._isHorizontal=(i||b)==b;}
,properties:{defaultLineColor:{init:g,check:e,themeable:true},defaultLineSize:{init:1,check:d,themeable:true}},members:{__IK:null,__IL:null,isHorizontal:function(){return this._isHorizontal;}
,setLineColor:function(l,m){if(qx.core.Environment.get(a)){qx.core.Assert.assertPositiveNumber(l);qx.core.Assert.assertString(m);}
;this.__IK[l]=m;if(this.__IM(l)){this.updateLayerData();}
;}
,setLineSize:function(n,o){if(qx.core.Environment.get(a)){qx.core.Assert.assertPositiveInteger(n);qx.core.Assert.assertPositiveInteger(o);}
;this.__IL[n]=o;if(this.__IM(n)){this.updateLayerData();}
;}
,__IM:function(p){if(this._isHorizontal){var t=this.getFirstColumn();var r=t+this.getColumnSizes().length-1;return p>=t&&p<=r;}
else {var s=this.getFirstRow();var q=s+this.getRowSizes().length-1;return p>=s&&p<=q;}
;}
,getLineSize:function(u){return this.__IL[u]||this.getDefaultLineSize();}
,getLineColor:function(v){return this.__IK[v]||this.getDefaultLineColor();}
,__IN:function(w,z,A){var top=0;var B,C;for(var y=0;y<A.length-1;y++){B=this.getLineColor(z+y);C=this.getLineSize(z+y);top+=A[y];w.push("<div style='","position: absolute;","height: "+C+"px;","width: 100%;","top:",top-((C>1)?Math.floor(C/2):1),"px;","background-color:",B,"'>","</div>");}
;}
,__IO:function(D,E,F){var I=0;var G,H;for(var x=0;x<F.length-1;x++){G=this.getLineColor(E+x);H=this.getLineSize(E+x);I+=F[x];D.push("<div style='","position: absolute;","width: "+H+"px;","height: 100%;","top: 0px;","left:",I-((H>1)?Math.floor(H/2):1),"px;","background-color:",G,"'>","</div>");}
;}
,_fullUpdate:function(J,K,L,M){var N=[];if(this._isHorizontal){this.__IN(N,J,L);}
else {this.__IO(N,K,M);}
;this.getContentElement().setAttribute(c,N.join(h));}
,_updateLayerWindow:function(O,P,Q,R){var S=O!==this.getFirstRow()||Q.length!==this.getRowSizes().length;var T=P!==this.getFirstColumn()||R.length!==this.getColumnSizes().length;if((this._isHorizontal&&S)||(!this._isHorizontal&&T)){this._fullUpdate(O,P,Q,R);}
;}
},destruct:function(){this.__IK=this.__IL=null;}
});}
)();
