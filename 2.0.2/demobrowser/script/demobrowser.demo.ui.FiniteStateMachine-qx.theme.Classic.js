...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine-qx.theme.Classic.5f990fa5642d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="#actionsBeforeOnentry",l="string",k="Saved-state stack is empty",j="State ",h=" already exists",g="qx.util.fsm.FiniteStateMachine",f="__LP",d="#actionsAfterOnentry",c="Saved-state stack is full",b="__LO",A=": Pushed event: ",z="Machine started with no available states",y="#entry",x=": Queued event: ",w="",v=" on ",u="Invalid state: not an instance of ",s="Number",r="qx.util.fsm.State",q="#",o="  ",p=" => ",n="String";qx.Class.define(g,{extend:qx.core.Object,construct:function(B){qx.core.Object.call(this);this.setName(B);this.__hy={};this.__LM=null;this.__LN=[];this.__LO=[];this.__LP=[];this.__LQ={};this.__LR={};this.__LS={};this.__LT={};this.__LU={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:n,nullable:true},state:{check:n,nullable:true},previousState:{check:n,nullable:true},nextState:{check:n,nullable:true},maxSavedStates:{check:s,init:2},debugFlags:{check:s,init:7}},members:{__hy:null,__LM:null,__LO:null,__LP:null,__LN:null,__LQ:null,__LR:null,__LS:null,__LT:null,__LU:null,__LV:false,addState:function(C){if(!C instanceof qx.util.fsm.State){throw new Error(u+r);}
;var D=C.getName();if(D in this.__hy){throw new Error(j+D+h);}
;if(this.__LM==null){this.__LM=D;}
;this.__hy[D]=C;}
...
;return G;}
,addObject:function(I,J,K){var M=J.toHashCode();this.__LR[I]=M;this.__LS[M]=I;this.__LQ[I]=J;if(!K){return;}
;if(typeof (K)==l){K=[K];}
;for(var i=0;i<K.length;i++){var L=K[i];if(!this.__LT[L]){this.__LT[L]={};}
;this.__LT[L][I]=true;if(!this.__LU[I]){this.__LU[I]=[];}
;this.__LU[I].push(L);}
;}
,removeObject:function(N){var P;var O;var Q;var R;P=this.__LR[N];if(this.__LU[N]){for(var i=0;i<this.__LU[N].length;i++){O=this.__LU[N][i];delete this.__LT[O][N];R=true;for(Q in this.__LT[O]){R=false;break;}
;if(R){delete this.__LT[O];}
;}
;delete this.__LU[N];}
;delete this.__LS[P];delete this.__LR[N];delete this.__LQ[N];}
,getObject:function(S){return this.__LQ[S];}
,getFriendlyName:function(T){var U=T?T.toHashCode():null;return U?this.__LS[U]:null;}
,getGroupObjects:function(V){var a=[];for(var name in this.__LT[V]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var W in this.__LR){var Y=this.__LR[W];var X=this.getObject(W);this.debug(W+p+Y);this.debug(o+Y+p+this.__LS[Y]);this.debug(o+W+p+this.getObject(W));this.debug(o+this.getObject(W)+p+this.getFriendlyName(X));}
;}
,_getInternalData:function(){return ({"states":this.__hy,"startState":this.__LM,"eventQueue":this.__LO,"blockedEvents":this.__LP,"savedStates":this.__LN,"friendlyToObject":this.__LQ,"friendlyToHash":this.__LR,"hashToFriendly":this.__LS,"groupToFriendly":this.__LT,"friendlyToGroups":this.__LU});}
,start:function(){var bb=this.__LM;if(bb==null){throw new Error(z);}
;this.setState(bb);this.setPreviousState(null);this.setNextState(null);var ba=(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL);if(ba){this.debug(this.getName()+q+bb+m);}
;this.__hy[bb].getAutoActionsBeforeOnentry()(this);if(ba){this.debug(this.getName()+q+bb+y);}
;this.__hy[bb].getOnentry()(this,null);if(ba){this.debug(this.getName()+q+bb+d);}
;this.__hy[bb].getAutoActionsAfterOnentry()(this);}
,pushState:function(bc){if(this.__LN.length>=this.getMaxSavedStates()){throw new Error(c);}
;if(bc===true){this.__LN.push(this.getState());}
else if(bc){this.__LN.push(bc);}
else {this.__LN.push(this.getPreviousState());}
;}
,popState:function(){if(this.__LN.length==0){throw new Error(k);}
;return this.__LN.pop();}
,postponeEvent:function(event){this.__LP.unshift(event);}
,enqueueEvent:function(event,bd){if(bd){this.__LO.push(event);}
else {this.__LO.unshift(event);}
...
;}
;}
,eventListener:function(event){var e=event.clone();this.enqueueEvent(e,false);this.__LW();}
...
,this,bl||20);}
,__LW:function(){if(this.__LV){return;}
;this.__LV=true;while(this.__LO.length>0){var event=this.__LO.pop();var bm=this.__LX(event);if(bm){event.dispose();}
;}
;this.__LV=false;}
,__LX:function(event){var bp;var br;var bo;var bz;var bx;var e;var bv;var bn=this.getDebugFlags();var by=bn&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var bs=bn&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var bw=bn&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var bu=bn&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var bq=this.getFriendlyName(event.getTarget());if(by){this.debug(this.getName()+": Process event: "+event.getType()+(bq?" on "+bq:""));}
...
;this.__LP.unshift(event);return false;default:if(typeof (bv)=="string"){if(bx[bv]){var bt=bx[bv];bx={};bx[bv]=bt;}
...
else {switch(br){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:br=bp;this.setNextState(br);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__LN.length==0){throw new Error("Attempt to transition to POP_STATE_STACK "+"while state stack is empty.");}
;br=this.__LN.pop();this.setNextState(br);break;default:throw new Error("Internal error: invalid nextState");break;};}
...
;bz.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__LP.length;i++){e=this.__LP.pop();this.__LO.unshift(e);}
...
;return true;}
},destruct:function(){this._disposeArray(b);this._disposeArray(f);this.__LN=this.__hy=null;}
});}
)();
(function(){var q="State info must be an object",p="function",o="  var groupObjects = ",n="__LF",m="__LG",l="parameters",k="__LJ",h=")",d=" fsm.getObject('",c="groups",bq="Invalid function parameters: ",bp="  for (var i = 0; i < groupObjects.length; i++)",bo="autoActionsAfterOnentry",bn=" value: ",bm="');",bl="State ",bk="autoActionsBeforeOnexit",bj="autoActionsBeforeOnentry",bi="__LH",bh="onexit",x="Invalid function request type: ",y="__LD",v=": ",w="Invalid friendly name in 'objects' list: ",t="autoActionsAfterOnexit",u="  fsm.debug(ex);",r="__LE",s="    fsm.getObject(objName).",D=")(fsm)",E="events",M="Invalid ",K="__LI",U="__LK",P="try",bd="    var objName = groupObjects[i];",ba="').",G="objects",bg="expected object, found ",bf="qx.util.fsm.Transition",be="The events object must be provided in new state info",F="catch(ex)",I="  {",J="    fsm.getGroupObjects('",L="Invalid transition: not an instance of ",N="  }",Q="Invalid 'objects' list: expected array, got ",W="qx.util.fsm.State",bc="onentry",z="Invalid function request parameter type: ",A="Invalid 'groups' list: expected array, got ",H="fsm",T=",",S="Adding user-provided field to state: ",R="context",Y="string",X='"',O="}",V="{",b="expected array, found ",bb="(",B=";",C="object";qx.Class.define(W,{extend:qx.core.Object,construct:function(br,bs){var bu;qx.core.Object.call(this);this.setName(br);if(typeof (bs)!=C){throw new Error(q);}
;bu=bs.context||window;this.setUserData(R,bu);for(var bt in bs){switch(bt){case bc:this.setOnentry(this.__LL(bs[bt],bu));break;case bh:this.setOnexit(this.__LL(bs[bt],bu));break;case bj:this.setAutoActionsBeforeOnentry(bs[bt]);break;case bo:this.setAutoActionsAfterOnentry(bs[bt]);break;case bk:this.setAutoActionsBeforeOnexit(bs[bt]);break;case t:this.setAutoActionsAfterOnexit(bs[bt]);break;case E:this.setEvents(bs[bt]);break;case R:break;default:this.setUserData(bt,bs[bt]);this.debug(bl+br+v+S+bt);break;};}
;if(!this.getEvents()){throw new Error(be);}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(bv,bw,bx){if(typeof (bw)!=C){throw new Error(M+bv+bn+typeof (bw));}
;var bC;var bD=P+V;var bA;var by;for(var f in bw){var bz=bw[f];if(!bz instanceof Array){throw new Error(x+b+typeof (bz));}
;for(var i=0;i<bz.length;i++){by=bz[i];if(typeof (by)!=C){throw new Error(z+bg+typeof (bz[bA]));}
;var bB=by[l];if(!bB){bB=[];}
else {if(!bB instanceof Array){throw new Error(bq+b+typeof (bB));}
;}
...
;}
;bC+=h;var a=by[G];if(!a){a=[];}
else if(!a instanceof Array){throw new Error(Q+typeof (a));}
;for(var j=0;j<a.length;j++){if(typeof (a[j])!=Y){throw new Error(w+a[j]);}
;bD+=d+a[j]+ba+bC+B;}
;var g=by[c];if(g){if(!g instanceof Array){throw new Error(A+typeof (g));}
;for(j=0;j<g.length;j++){bD+=o+J+g[j]+bm+bp+I+bd+s+bC+B+N;}
;}
;}
;}
;bD+=O+F+V+u+O;return qx.lang.Function.bind(new Function(H,bD),bx);}
},properties:{name:{transform:y,nullable:true},onentry:{transform:r,nullable:true,init:function(bE,event){}
},onexit:{transform:n,nullable:true,init:function(bF,event){}
},autoActionsBeforeOnentry:{transform:bi,nullable:true,init:function(bG,event){}
},autoActionsAfterOnentry:{transform:K,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnexit:{transform:k,nullable:true,init:function(bI,event){}
},autoActionsAfterOnexit:{transform:U,nullable:true,init:function(bJ,event){}
},events:{transform:m,nullable:true}},members:{__LD:function(bK){if(typeof (bK)!="string"||bK.length<1){throw new Error("Invalid state name");}
;return bK;}
,__LE:function(bL){switch(typeof (bL)){case "undefined":return function(bM,event){}
...
,__LF:function(bN){switch(typeof (bN)){case "undefined":return function(bO,event){}
...
,__LG:function(bP){if(typeof (bP)!="object"){throw new Error("events must be an object");}
...
;;}
;return bP;}
,__LH:function(bS){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnentry",bS,this.getUserData("context"));}
,__LI:function(bT){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnentry",bT,this.getUserData("context"));}
,__LJ:function(bU){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnexit",bU,this.getUserData("context"));}
,__LK:function(bV){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnexit",bV,this.getUserData("context"));}
,__LL:function(f,bW){if(typeof (f)=="function"){f=qx.lang.Function.bind(f,bW);}
;return f;}
,addTransition:function(bX){if(!bX instanceof qx.util.fsm.Transition){throw new Error(L+bf);}
;this.transitions[bX.getName()]=bX;}
}});}
)();
(function(){var k="function",j="or one of the Fsm.StateChange constants",i="__Mb",h="string",g="__LY",e="predicate",d="nextState must be an explicit state name, ",c="Transition ",b="String",a="number",z="Invalid transition nextState type: ",y="Invalid transition nextState value: ",x="__Ma",w="__Mc",v="ontransition",u="__Md",t="nextState",s="Adding user-provided field to transition: ",r="Invalid ontransition type: ",q="qx.util.fsm.Transition",o="undefined",p="autoActionsBeforeOntransition",m="autoActionsAfterOntransition",n=": ",l="context";qx.Class.define(q,{extend:qx.core.Object,construct:function(A,B){var D;qx.core.Object.call(this);this.setName(A);D=B.context||window;this.setUserData(l,D);for(var C in B){switch(C){case e:this.setPredicate(this.__LL(B[C],D));break;case t:this.setNextState(B[C]);break;case p:this.setAutoActionsBeforeOntransition(this.__LL(B[C],D));break;case m:this.setAutoActionsAfterOntransition(this.__LL(B[C],D));break;case v:this.setOntransition(this.__LL(B[C],D));break;case l:break;default:this.setUserData(C,B[C]);this.debug(c+A+n+s+C);break;};}
;}
,properties:{name:{check:b,nullable:true},predicate:{init:function(E,event){return true;}
,transform:g},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:x},autoActionsBeforeOntransition:{init:function(F,event){}
,transform:i},autoActionsAfterOntransition:{init:function(G,event){}
,transform:w},ontransition:{init:function(H,event){}
,transform:u}},members:{__LY:function(I){switch(typeof (I)){case "undefined":return function(J,event){return true;}
;case "boolean":return function(K,event){return I;}
;case "function":return qx.lang.Function.bind(I,this.getUserData("context"));default:throw new Error("Invalid transition predicate type: "+typeof (I));break;};}
,__Ma:function(L){switch(typeof (L)){case h:return L;case a:switch(L){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return L;default:throw new Error(y+L+n+d+j);};break;default:throw new Error(z+typeof (L));break;};}
,__Mb:function(M){return qx.util.fsm.State._commonTransformAutoActions(p,M,this.getUserData(l));}
,__Mc:function(N){return qx.util.fsm.State._commonTransformAutoActions(m,N,this.getUserData(l));}
,__Md:function(O){switch(typeof (O)){case o:return function(P,event){}
;case k:return qx.lang.Function.bind(O,this.getUserData(l));default:throw new Error(r+typeof (O));break;};}
,__LL:function(f,Q){if(typeof (f)=="function"){f=qx.lang.Function.bind(f,Q);}
;return f;}
}});}
)();
