...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.HtmlArea","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.HtmlArea-qx.theme.Simple.099cc7d5571b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,__yR:function(e){var co=window.prompt(bl,x);this.insertHtml(co);}
,__yS:function(){var cq=new qx.ui.form.SelectBox;cq.set({toolTipText:m,focusable:false,keepFocus:true,width:120,height:16,margin:[4,0]});cq.add(new qx.ui.form.ListItem(x));var cr=[X,O,r,U,f,J,M,bz,g,ba,R];var cp;for(var i=0,j=cr.length;i<j;i++){cp=new qx.ui.form.ListItem(cr[i]);cp.set({focusable:false,keepFocus:true,font:qx.bom.Font.fromString(bU+cr[i])});cq.add(cp);}
;cq.addListener(C,function(e){var cs=e.getData()[0].getLabel();if(cs!=x){this.setFontFamily(cs);cq.setSelection([cq.getChildren()[0]]);}
;}
,this.__yL);return cq;}
,__yT:function(){var cu=new qx.ui.form.SelectBox;cu.set({toolTipText:bx,focusable:false,keepFocus:true,width:50,height:16,margin:[4,0]});cu.add(new qx.ui.form.ListItem(x));var ct;for(var i=1;i<=7;i++){ct=new qx.ui.form.ListItem(i+x);ct.set({focusable:false,keepFocus:true});cu.add(ct);}
;cu.addListener(C,function(e){var cv=e.getData()[0].getLabel();if(cv!=x){this.setFontSize(cv);cu.setSelection([cu.getChildren()[0]]);}
;}
,this.__yL);return cu;}
,__yU:function(){return [{bold:{text:l,image:D,action:this.__yL.setBold},italic:{text:bh,image:T,action:this.__yL.setItalic},underline:{text:V,image:bg,action:this.__yL.setUnderline},strikethrough:{text:Q,image:bO,action:this.__yL.setStrikeThrough},removeFormat:{text:bT,image:d,action:this.__yL.removeFormat}},{alignLeft:{text:w,image:L,action:this.__yL.setJustifyLeft},alignCenter:{text:o,image:bH,action:this.__yL.setJustifyCenter},alignRight:{text:bs,image:K,action:this.__yL.setJustifyRight},alignJustify:{text:bE,image:N,action:this.__yL.setJustifyFull}},{fontFamily:{custom:this.__yS},fontSize:{custom:this.__yT},fontColor:{text:br,image:a,action:this.__yM},textBackgroundColor:{text:bk,image:bF,action:this.__yN}},{indent:{text:bv,image:bB,action:this.__yL.insertIndent},outdent:{text:P,image:p,action:this.__yL.insertOutdent}},{insertImage:{text:u,image:bR,action:this.__yO},insertTable:{text:bq,image:b,action:this.__yP},insertLink:{text:bm,image:F,action:this.__yQ},insertHTML:{text:I,image:bC,action:this.__yR},insertHR:{text:bD,image:bK,action:this.__yL.insertHorizontalRuler}},{ol:{text:G,image:bb,action:this.__yL.insertOrderedList},ul:{text:bt,image:bL,action:this.__yL.insertUnorderedList}},{undo:{text:h,image:c,action:this.__yL.undo},redo:{text:k,image:n,action:this.__yL.redo}}];}
,__yV:function(){var toolbar=new qx.ui.toolbar.ToolBar;toolbar.setDecorator(bN);var cw;var cx=this.__yU();for(var i=0,j=cx.length;i<j;i++){var cy=new qx.ui.toolbar.Part;toolbar.add(cy);for(var cz in cx[i]){var cA=cx[i][cz];if(cA.custom){cw=cA.custom.call(this);}
else {cw=new qx.ui.toolbar.Button(null,cA.image);cw.set({focusable:false,keepFocus:true,center:true,toolTipText:cA.text?cA.text:x});cw.addListener(y,cA.action,this.__yL);}
;cy.add(cw);}
;}
;return toolbar;}
}});}
...
},properties:{contentType:{check:t,init:k,apply:T},messengerMode:{check:P,init:false,apply:h},insertParagraphOnLinebreak:{check:P,init:true,apply:ba},insertLinebreakOnCtrlEnter:{check:P,init:true,apply:X},postProcess:{check:bc,nullable:true,init:null,apply:F},useUndoRedo:{check:P,init:true,apply:j},appearance:{refine:true,init:o},defaultFontFamily:{check:t,init:W,apply:H},defaultFontSize:{check:bf,init:4,apply:E},focusable:{refine:true,init:true}},members:{__wb:null,__vY:null,__wc:null,__wa:null,__wd:null,_initBlockerElement:function(){if(!this.__wc){this.__wc=this._createBlockerElement();}
;}
,_applyContentType:function(bp,bq){if(this.__wb!=null){this.__wb.setContentType(bp);}
else {this.__vY[r]=bp;}
;}
,_applyMessengerMode:function(br,bs){if(this.__wb!=null){this.__wb.setMessengerMode(br);}
else {this.__vY[w]=br;}
;}
,_applyInsertParagraphOnLinebreak:function(bt,bu){if(this.__wb!=null){this.__wb.setInsertParagraphOnLinebreak(bt);}
else {this.__vY[d]=bt;}
;}
,_applyInsertLinebreakOnCtrlEnter:function(bv,bw){if(this.__wb!=null){this.__wb.setInsertLinebreakOnCtrlEnter(bv);}
else {this.__vY[B]=bv;}
;}
,_applyPostProcess:function(bx,by){if(this.__wb!=null){this.__wb.setPostProcess(bx);}
else {this.__vY[Y]=bx;}
;}
,_applyUseUndoRedo:function(bz,bA){if(this.__wb!=null){this.__wb.setUseUndoRedo(bz);}
else {this.__vY[M]=bz;}
;}
,_applyDefaultFontFamily:function(bB,bC){if(this.__wb!=null){this.__wb.setDefaultFontFamily(bB);}
else {this.__vY[U]=bB;}
;}
,_applyDefaultFontSize:function(bD,bE){if(this.__wb!=null){this.__wb.setDefaultFontSize(bD);}
else {this.__vY[q]=bD;}
;}
,_applyNativeContextMenu:function(bF,bG){if(this.__wb!=null){this.__wb.setNativeContextMenu(bF);}
else {this.__vY[be]=bF;}
;}
...
;return bH;}
,__we:function(){this.addListenerOnce(L,this.__wf);}
,__wf:function(){var bI=this.getContentElement().getDomElement();this.__wb=new qx.bom.htmlarea.HtmlArea(bI,this.__wa.content,this.__wa.styleInfo,this.__wa.source);this.__wg();this.__wh();if((qx.core.Environment.get(z)==c)){this.__wk();}
;this.addListener(L,this.forceEditable);}
,__wg:function(){for(var bJ in this.__vY){this.__wb[p+bJ](this.__vY[bJ]);}
;}
,__wh:function(){this.__wb.addListener(l,this.__wi,this);this.__wb.addListener(A,this.__wi,this);this.__wb.addListener(bd,this.__wi,this);this.__wb.addListener(i,this.__wi,this);this.__wb.addListener(m,this.__wj,this);this.__wb.addListener(n,this.__wj,this);this.__wb.addListener(x,this.__wj,this);this.__wb.addListener(a,this.__wj,this);this.__wb.addListener(bg,this.__wj,this);}
,__wi:function(e){var bK=e.clone();this.fireEvent(bK.getType());}
,__wj:function(e){var bL=e.clone();this.fireDataEvent(bL.getType(),e.getData());}
,__wk:function(){this.__wd=qx.lang.Function.bind(this.__wl,this);var bM=this.getContainerElement().getDomElement();qx.bom.Event.addNativeListener(bM,S,this.__wd);}
,__wl:qx.event.GlobalError.observeMethod(function(e){if(this.__wb&&!this.__wb.isDisposed()){this.__wb.invalidateEditor();}
;}
),renderLayout:function(bN,top,bO,bP){qx.ui.core.Widget.prototype.renderLayout.call(this,bN,top,bO,bP);var bR=s;var bQ=this.getInsets();if(!this.__wc){this._initBlockerElement();}
;this.__wc.setStyles({left:bQ.left+bR,top:bQ.top+bR,width:(bO-bQ.left-bQ.right)+bR,height:(bP-bQ.top-bQ.bottom)+bR});}
,getIframeObject:function(){return this.__wb!=null?this.__wb.getIframeObject():null;}
,getCommandManager:function(){return this.__wb!=null?this.__wb.getCommandManager():null;}
,setValue:function(bS){if(this.__wb!=null){this.__wb.setValue(bS);}
else {this.__wa.content=bS;}
;}
,getValue:function(){if(this.__wb!=null){return this.__wb.getValue();}
else {return this.__wa.content;}
;}
,getComputedValue:function(bT){return this.__wb!=null?this.__wb.getHtml(bT):null;}
,getCompleteHtml:function(){return this.__wb!=null?this.__wb.getCompleteHtml():null;}
,getContentDocument:function(){return this.__wb!=null?this.__wb.getContentDocument():null;}
,getContentBody:function(){return this.__wb!=null?this.__wb.getContentBody():null;}
,getContentWindow:function(){return this.__wb!=null?this.__wb.getContentWindow():null;}
,getWords:function(bU){return this.__wb!=null?this.__wb.getWords(bU):null;}
,getWordsWithElement:function(){return this.__wb!=null?this.__wb.getWordsWithElement():null;}
,getTextNodes:function(){return this.__wb!=null?this.__wb.getTextNodes():null;}
,isReady:function(){return this.__wb!=null?this.__wb.isReady():false;}
,forceEditable:function(){if(this.__wb!=null){this.__wb.forceEditable();}
;}
,isLoaded:function(){return this.__wb!=null?this.__wb.isLoaded():false;}
,setEditable:function(bV){if(this.__wb!=null){this.__wb.setEditable(bV);}
;}
,getEditable:function(){return this.__wb!=null?this.__wb.getEditable():false;}
,isEditable:function(){return this.__wb!=null?this.__wb.isEditable():false;}
,insertHtml:function(bW){return this.__wb!=null?this.__wb.insertHtml(bW):false;}
,removeFormat:function(){return this.__wb!=null?this.__wb.removeFormat():false;}
,setBold:function(){return this.__wb!=null?this.__wb.setBold():false;}
,setItalic:function(){return this.__wb!=null?this.__wb.setItalic():false;}
,setUnderline:function(){return this.__wb!=null?this.__wb.setUnderline():false;}
,setStrikeThrough:function(){return this.__wb!=null?this.__wb.setStrikeThrough():false;}
,setFontSize:function(bX){return this.__wb!=null?this.__wb.setFontSize(bX):false;}
,setFontFamily:function(bY){return this.__wb!=null?this.__wb.setFontFamily(bY):false;}
,setTextColor:function(ca){return this.__wb!=null?this.__wb.setTextColor(ca):false;}
,setTextBackgroundColor:function(cb){return this.__wb!=null?this.__wb.setTextBackgroundColor(cb):false;}
,setJustifyLeft:function(){return this.__wb!=null?this.__wb.setJustifyLeft():false;}
,setJustifyCenter:function(){return this.__wb!=null?this.__wb.setJustifyCenter():false;}
,setJustifyRight:function(){return this.__wb!=null?this.__wb.setJustifyRight():false;}
,setJustifyFull:function(){return this.__wb!=null?this.__wb.setJustifyFull():false;}
,insertIndent:function(){return this.__wb!=null?this.__wb.insertIndent():false;}
,insertOutdent:function(){return this.__wb!=null?this.__wb.insertOutdent():false;}
,insertOrderedList:function(){return this.__wb!=null?this.__wb.insertOrderedList():false;}
,insertUnorderedList:function(){return this.__wb!=null?this.__wb.insertUnorderedList():false;}
,insertHorizontalRuler:function(){return this.__wb!=null?this.__wb.insertHorizontalRuler():false;}
,insertImage:function(cc){return this.__wb!=null?this.__wb.insertImage(cc):false;}
,insertHyperLink:function(cd){return this.__wb!=null?this.__wb.insertHyperLink(cd):false;}
,removeBackgroundColor:function(){return this.__wb!=null?this.__wb.removeBackgroundColor():false;}
,setBackgroundColor:function(ce){return this.__wb!=null?this.__wb.setBackgroundColor(ce):false;}
,removeBackgroundImage:function(){return this.__wb!=null?this.__wb.removeBackgroundImage():false;}
,setBackgroundImage:function(cf,cg,ch){return this.__wb!=null?this.__wb.setBackgroundImage(cf,cg,ch):false;}
,selectAll:function(){return this.__wb!=null?this.__wb.selectAll():false;}
,undo:function(){return this.__wb!=null?this.__wb.undo():false;}
,redo:function(){return this.__wb!=null?this.__wb.redo():false;}
,resetHtml:function(){if(this.__wb!=null){this.__wb.resetHtml();}
;}
,getHtml:function(ci){return this.__wb!=null?this.__wb.getHtml(ci):null;}
,containsOnlyPlaceholder:function(){return this.__wb!=null?this.__wb.containsOnlyPlaceHolder():false;}
,getContextInformation:function(){return this.__wb!=null?this.__wb.getContextInformation():null;}
,getSelection:function(){return this.__wb!=null?this.__wb.getSelection():null;}
,getSelectedText:function(){return this.__wb!=null?this.__wb.getSelectedText():null;}
,getSelectedHtml:function(){return this.__wb!=null?this.__wb.getSelectedHtml():null;}
,clearSelection:function(){if(this.__wb!=null){this.__wb.clearSelection();}
;}
,getRange:function(){return this.__wb.getRange();}
,saveRange:function(){this.__wb.saveRange();}
,getSavedRange:function(){return this.__wb.getSavedRange();}
,resetSavedRange:function(){this.__wb.resetSavedRange();}
,getFocusNode:function(){return this.__wb!=null?this.__wb.getFocusNode():null;}
,block:function(){if(!this.__wc){this._initBlockerElement();}
;if(!this.getContainerElement().hasChild(this.__wc)){this.getContainerElement().add(this.__wc);}
;this.__wc.setStyle(Q,V);}
,release:function(){if(this.__wc){this.__wc.setStyle(Q,u);}
;}
,focus:function(){qx.ui.core.Widget.prototype.focus.call(this);this.__wm();}
,tabFocus:function(){qx.ui.core.Widget.prototype.tabFocus.call(this);this.__wm();}
,__wm:function(){if(this.__wb!=null){qx.event.Timer.once(function(){this.__wb.focusContent();}
,this,0);}
;}
},destruct:function(){this.__vY=this.__wa=null;qx.event.Registration.removeListener(document.body,O,this.block,this,true);qx.event.Registration.removeListener(document.body,K,this.release,this,true);qx.event.Registration.removeListener(document.body,J,this.release,this,true);var cj=this.getContainerElement().getDomElement();if(cj){qx.bom.Event.removeNativeListener(cj,S,this.__wd);}
;this._disposeObjects(v,C);}
});}
)();
(function(){var dV="qx.blankpage",dU='</span>',dT="backgroundColor",dS="li",dR='undo',dQ="<br class='webkit-block-placeholder' />",dP="Function",dO="fontSize",dN="textcolor",dM="cant open document on source '",cu="home",ct="<",cs="focused",cr="fontFamily",cq="load",cp='document not available, try again...',co='<font ',cn='id="__elementToFocus__"',cl="setUnderline",ck="backgroundPosition",ed="<!--",ee=' size="',eb='/www.w3.org/1999/xhtml" xml:lang="en" lang="en">',ec="'>",dY='underline',ea='bold',dW="&copy;",dX="ul",ef="cursorContext",eg="readyAfterInvalid",dr="styleWithCSS",dq="span",dt=" />",ds="setBold",dv="<P>&nbsp;</P>",du="</",dx="textbackgroundcolor",dw="&gt;",dp='="',dn="can't extract style from elem. ",o="paddingLeft",p='size',q="inserthorizontalrule",r='<p>',t='</font>',u="backgroundRepeat",v="insertimage",w="a",x="&amp;",y="removeformat",ev="marginLeft",eu="ready",et="<p>&nbsp;</p>",es='DOCTYPE html PUBLIC "-/',eA="browser.version",ez=" html, body {overflow-x: visible; } ",ey='<body>',ew="insertorderedlist",eC='body { font-size:100.01%; font-family:Verdana, Geneva, Arial, Helvetica, sans-serif; background-color:transparent; overflow:visible; background-image:none; margin:0px; padding:5px; } ',eB="hideFocus",br="none",bs='/DTD XHTML 1.0 Transitional/',bp='<html xmlns="http:/',bq="outline",bv="setItalic",bw="font-style",bt="marginBottom",bu="xhtml",bn="focusOut",bo="justifyleft",S="handleMouse ",R='</p>',U="paddingTop",T="Please use the method 'qx.dom.Node.isBlockNode' instead.",O='<title></title><meta http-equiv="Content-type" content="text/html; charset=UTF-8" />',N="Control",Q='<span style="font-family:',P='/W3C/',M="placeholder",L='cant load HtmlArea. Document is not available. ',bC="backgroundImage",bD="marginTop",bE="> ",bF=" { ",by="font-weight",bz="LI",bA='italic',bB='<!',bG="\ufeff",bH="&quot;",bg="marginRight",bf="STYLE",be="justifyfull",bd="inserthyperlink",bc="<br />",bb='html { margin:0px; padding:0px; } ',ba="<br>",Y='/EN" "http:/',bk="justifycenter",bj="<br /><div id='placeholder'></div>",bI="useCSS",bJ="object",bK="strikethrough",bL="text/html",bM="true",bN="1.9",bO='',bP=' style="',bQ="indent",bR="messengerContent",cC="Integer",cB="insertunorderedlist",cA="Text",cz="&lt;",cG='<br _moz_editor_bogus_node="TRUE" _moz_dirty=""/>',cF='<br/><div class="placeholder"></div>',cE="selectall",cD="transparent",cK=" SCRIPT STYLE DIV SPAN TR TD TBODY TABLE EM STRONG FONT A P B I U STRIKE H1 H2 H3 H4 H5 H6 ",cJ="justifyright",di='/www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">',dj="ol",dg="//text()[string-length(normalize-space(.))>0]",dh="outdent",de="about:blank",df="<body style='",dc="qx.bom.htmlarea.HtmlArea",dd="paddingBottom",dk="Verdana",dl="$",dB="-->",dA=" }",dD="paddingRight",dC='html { width:100%; height:100%; margin:0px; padding:0px; overflow-y:auto; overflow-x:auto; } ',dF="Off",dE='p { margin:0px; padding:0px; }',dH="__wn",dG='body { font-size: 100.01%; font-family:Verdana, Geneva, Arial, Helvetica, sans-serif; width:100%; height:100%; background-color:transparent; overflow:auto; background-image:none; margin:0px; padding:5px; } ',dz="pageup",dy="backgroundcolor",eo="Failed to enable rich edit functionality",ep="end",eq="div",er='On',ej="redo",ek="i",em="backspace",en="bold",eh="mouseup",ei="y",n="style",m="backgroundimage",l="100%",k='</head>',h='<head>',g="k",f=":",d='redo',c='Off',b='">',B="down",C="font-family",z="g",A="keypress",F="fontsize",G="undo",D="text-decoration",E="pagedown",I="justify",J="center",cO="p",cI='</style>',cV="u",cR=" | ",cx="focus",cv="control",W="click",cy='<style type="text/css">',bi="up",bh="blur",cc="font-size",cd="None",ce="delete",cf="_moz_dirty",cg=">",ch='</body></html>',ci="line-through",cj="String",bY="z",ca=";",cw="fontfamily",cU="keydown",cT="keyup",cS="focusout",da="engine.version",cY="inserthtml",cX="italic",cW="body",cQ="underline",cP="br",H="old_id",bm="'",bl="b",cH="loadingError",bx="contextmenu",cN="right",cM="enter",cL='"',V="left",db="text-align",K="qx.bom.htmlarea.HtmlArea.debug",X="qx.event.type.Data",bS="opera",bT="id",bU="qx.event.type.Event",bV=" ",bW="Boolean",bX="__elementToFocus__",dm="qx.debug",cb="webkit",dJ="gecko",dI="mshtml",dL="",dK="engine.name";qx.Class.define(dc,{extend:qx.core.Object,construct:function(eD,eE,eF,eG){qx.core.Object.call(this);var eH=eG||qx.util.ResourceManager.getInstance().toUri(qx.core.Environment.get(dV));this.__xt(eD);this.__xu();this._createAndAddIframe(eH);this._addIframeLoadListener();this.__xb=qx.bom.htmlarea.HtmlArea.__xd(eF);if(qx.lang.Type.isString(eE)){this.__xc=eE;}
;this.__wn=this.__xH();}
,events:{"load":bU,"loadingError":X,"messengerContent":X,"cursorContext":X,"ready":bU,"readyAfterInvalid":bU,"focused":bU,"focusOut":bU,"contextmenu":X,"undoRedoState":X},statics:{simpleLinebreak:ba,EMPTY_DIV:"<div></div>",GetWordsRegExp:/([^\u0000-\u0040\u005b-\u005f\u007b-\u007f]|['])+/g,CleanupWordsRegExp:/[\u0000-\u0040]/gi,hotkeyInfo:{bold:{method:ds},italic:{method:bv},underline:{method:cl},undo:{method:G},redo:{method:ej}},__xd:function(eI){if(eI==null||eI==dL){return dL;}
else if(typeof eI==bJ){var eJ=dL;for(var i in eI){eJ+=i+bF+eI[i]+dA;}
;return eJ;}
else {return eI;}
;}
,__xe:function(eK){var eL={};var a=eK.split(ca);var i;for(i=0;i<a.length;i++){var eM=a[i],eN=eM.indexOf(f);if(eN===-1){continue;}
...
;}
;return eL;}
,__xf:function(eP,eQ,eR,eS){var eX=[];switch(eP.nodeType){case 1:case 11:var i;var eV=eP.tagName.toLowerCase();var fa={};var eT={};var closed=(!(eP.hasChildNodes()||qx.bom.htmlarea.HtmlArea.__xg(eP)));if(eQ){if((qx.core.Environment.get(dK)==dI)){if(eV==eq&&eP.className&&eP.className==M){for(i=eP.firstChild;i;i=i.nextSibling){eX.push(qx.bom.htmlarea.HtmlArea.__xf(i,true,eR,eS));}
;return eX.join(dL);}
;}
...
;if(name.charAt(0)===dl){continue;}
;fa[name]=fb;}
;if(fa.style!==undefined){eT=qx.bom.htmlarea.HtmlArea.__xe(fa.style);delete fa.style;}
...
;if(!qx.lang.Object.isEmpty(eT)){eX.push(bP);for(var name in eT){var fb=eT[name];eX.push(name,f,fb.toString().replace(new RegExp(cL,z),bm),ca);}
;eX.push(cL);}
;eX.push(closed?dt:cg);}
;}
;for(i=eP.firstChild;i;i=i.nextSibling){eX.push(qx.bom.htmlarea.HtmlArea.__xf(i,true,eR,eS));}
;if(eQ&&!closed&&eV){eX.push(du,eV,cg);}
;break;case 3:eX.push(eR?eP.data:qx.bom.htmlarea.HtmlArea.__xh(eP.data));break;case 8:eX.push(ed,eP.data,dB);break;};return eX.join(dL);}
,closingTags:cK,__xg:function(fc){return (qx.bom.htmlarea.HtmlArea.closingTags.indexOf(bV+fc.tagName+bV)!=-1);}
,__xh:function(s){s=s.replace(/&/ig,x);s=s.replace(/</ig,cz);s=s.replace(/>/ig,dw);s=s.replace(/\x22/ig,bH);s=s.replace(/\xA9/ig,dW);return s;}
...
},properties:{contentType:{check:cj,init:bu},messengerMode:{check:bW,init:false},insertParagraphOnLinebreak:{check:bW,init:true},insertLinebreakOnCtrlEnter:{check:bW,init:true},postProcess:{check:dP,nullable:true,init:null},useUndoRedo:{check:bW,init:true},nativeContextMenu:{check:bW,init:false},defaultFontFamily:{check:cj,init:dk},defaultFontSize:{check:cC,init:4}},members:{__kq:null,__he:false,__xi:false,__xj:false,__xk:false,__xl:false,__wn:null,__xm:null,__xn:null,__xo:null,__rN:null,__xb:null,__xp:null,__xq:null,__xr:false,__xs:false,__xt:function(fj){if(qx.dom.Node.isElement(fj)&&qx.dom.Node.isNodeName(fj,eq)){this.__kq=fj;}
;}
,_createAndAddIframe:function(fk){this.__rN=qx.bom.Iframe.create();qx.bom.Iframe.setSource(this.__rN,fk);if((qx.core.Environment.get(dK)==dI)){qx.bom.element.Attribute.set(this.__rN,eB,bM);}
else {qx.bom.element.Style.set(this.__rN,bq,br);}
;qx.bom.element.Style.setStyles(this.__rN,{width:l,height:l});qx.dom.Element.insertBegin(this.__rN,this.__kq);}
,_getIframeDocument:function(){return qx.bom.Iframe.getDocument(this.__rN);}
,_getIframeWindow:function(){return qx.bom.Iframe.getWindow(this.__rN);}
,_addIframeLoadListener:function(){qx.event.Registration.addListener(this.__rN,cq,this._loaded,this);}
,__xu:function(){this.__xp={"xhtml":{doctype:bB+es+P+bs+Y+di,html:bp+eb,meta:O,style:qx.core.Environment.select(dK,{"mshtml":bb+dG,"default":dC+eC}),contentStyle:dE,body:ey,footer:ch}};}
,__xc:dL,getIframeObject:function(){return this.__rN;}
,getCommandManager:function(){return this.__wn;}
,setValue:function(fl){if(qx.lang.Type.isString(fl)){this.__xc=fl;var fm=this._getIframeDocument();if(fm&&fm.body){fm.body.innerHTML=this.__xv(fl);}
;}
;}
,__xv:function(fn){var fq=dL;if((qx.core.Environment.get(dK)==dJ)){if(qx.core.Environment.get(eA)<=2){fq+=cG;}
;}
;var fr=fn.length==0?bG:dL;var fp=qx.core.Environment.get(dK)==dJ||qx.core.Environment.get(dK)==cb?cn:bO;var fo=r+Q+this.getDefaultFontFamily()+b+co+fp+ee+this.getDefaultFontSize()+b+fq+fn+fr+t+dU+R;return fo;}
,getValue:function(){return this.__xc;}
,getComputedValue:function(fs){return this.getHtml(fs);}
,getCompleteHtml:function(){var fu=this.__xp[this.getContentType()];var ft=fu.html+h+fu.meta+cy+fu.contentStyle+cI+k;ft+=df+this.__xw()+ec;ft+=this.getHtml()+ch;return ft;}
,__xw:function(){var fx=[dT,bC,u,ck,cr,dO,bD,bt,ev,bg,U,dd,o,dD];var fw=qx.bom.element.Style;var fv=this.getContentBody();var fA={};var fz,fB;var fy=qx.core.Environment.get(dK)==dI?2:1;for(var i=0,j=fx.length;i<j;i++){fz=fx[i];fB=fw.get(fv,fz,fy);if(fB!==undefined&&fB!=dL){fA[fz]=fB;}
;}
...
;}
;return fR;}
,__xx:0,__xy:function(){var fS=this._getIframeDocument();if(!fS){this.__xx++;if(this.__xx>5){this.error(L+fS);this.fireDataEvent(cH);}
else {if(qx.core.Environment.get(dm)){this.error(cp);}
;qx.event.Timer.once(function(){this.__xy();}
,this,0);}
;}
else {this.__xx=0;this._onDocumentIsReady();}
;}
,_loaded:function(e){if(this.__xj){return;}
;if(this.__xi){this.__xz();return;}
;if((qx.core.Environment.get(dK)==dI)){this.__xy();}
...
;this.__xj=true;if((qx.core.Environment.get(dK)==dI)){this.setEditable(true);}
;this.__xC();if(!(qx.core.Environment.get(dK)==bS)){this.__xD();}
;if(!(qx.core.Environment.get(dK)==dI)){this.setEditable(true);}
;this.__he=true;this.__wn=fT;fT.setContentDocument(this._getIframeDocument());this.__xI();if((qx.core.Environment.get(dK)==bS)){this.__xD();}
;this.fireEvent(eu);}
,forceEditable:qx.core.Environment.select(dK,{"gecko":function(){var fU=this._getIframeDocument();if(fU){if(parseFloat(qx.core.Environment.get(da))>=bN){fU.designMode=dF;fU.body.contentEditable=false;fU.body.contentEditable=true;}
else {fU.body.contentEditable=true;this.__xJ(true);}
;}
;}
,"default":qx.lang.Function.empty}),invalidateEditor:qx.core.Environment.select(dK,{"gecko":function(){this.__xj=false;this.__he=false;this.__xi=true;}
,"default":function(){}
}),__xz:qx.core.Environment.select(dK,{"gecko":function(){this.__xC();this.__xD();this.__wn.setContentDocument(this._getIframeDocument());this.setEditable(true);this.forceEditable();this.__xj=true;this.__he=true;this.__xi=false;this.fireEvent(eg);}
,"default":function(){}
}),__xA:function(fV){var fX=dL;if(!fV){return fX;}
...
;return fX;}
,__xB:function(ga){var gb=qx.core.Environment.get(dK)==dJ?ez:dL;var gc=this.__xp[this.getContentType()];var gd=h+gc.meta+cy+gb+gc.style+gc.contentStyle+this.__xb+cI+k;var content=gc.body+ga;return gc.html+gd+content+gc.footer;}
,__xC:function(){var gf=this.__xv(this.getValue());if(qx.lang.Type.isString(gf)){var ge=this._getIframeDocument();try{ge.open(bL,true);ge.write(this.__xB(gf));ge.close();}
catch(e){this.error(dM+qx.bom.Iframe.queryCurrentUrl(this.__rN)+bm,e);this.fireDataEvent(cH,e);}
;}
;}
,__xD:function(){this.__xE();this.__xG();this.__xF();}
,__xE:function(){var gh=qx.event.Registration;var gg=this._getIframeDocument();gh.addListener(gg.body,A,this._handleKeyPress,this);gh.addListener(gg.body,cT,this._handleKeyUp,this);gh.addListener(gg.body,cU,this._handleKeyDown,this);}
,__xF:function(){var gk=qx.event.Registration;var gi=this._getIframeDocument();var gj=qx.core.Environment.get(dK)==cb?this._getIframeWindow():gi.body;gk.addListener(gj,cx,this._handleFocusEvent,this);gk.addListener(gj,bh,this._handleBlurEvent,this);gk.addListener(gi,cS,this._handleFocusOutEvent,this);}
,__xG:function(){var gn=qx.event.Registration;var gm=this._getIframeDocument();var gl=qx.core.Environment.get(dK)==dI?W:eh;gn.addListener(gm.body,gl,this._handleMouseUpOnBody,this);gn.addListener(gm.documentElement,gl,this._handleMouseUpOnDocument,this);gn.addListener(gm.documentElement,bx,this._handleContextMenuEvent,this);}
,__xH:function(){if(this.__xm==null){this.__xm={execute:function(go,gp){this.stackedCommands=true;this.commandStack.push({command:go,value:gp});}
,commandStack:[],stackedCommands:false};}
;this.__xm.stackedCommands=false;return this.__xm;}
,__xI:function(){var gq=this.__xm;if(gq!=null&&gq.stackedCommands){var gr=gq.commandStack;if(gr!=null){for(var i=0,j=gr.length;i<j;i++){this.__wn.execute(gr[i].command,gr[i].value);}
;}
;}
;}
,__xJ:function(gs){var gt=this._getIframeDocument();if(this.__xj&&gt){try{if((qx.core.Environment.get(dK)==dJ)){gt.designMode=(gs!==false)?c:er;}
;gt.designMode=(gs!==false)?er:c;}
catch(e){}
;}
;}
,setEditable:function(gu){if(this.__xj){this.__xJ(true);if((qx.core.Environment.get(dK)==dJ)){try{var gv=this._getIframeDocument();gv.execCommand(dr,false,true);}
catch(gw){try{var gv=this._getIframeDocument();gv.execCommand(bI,false,false);}
...
;this.__xk=gu;}
;}
,getEditable:function(){return this.__xk;}
,isEditable:function(){return this.__xk;}
,__xK:false,_handleKeyUp:function(e){var gy=e.getKeyIdentifier().toLowerCase();this.__xn=e;if((qx.core.Environment.get(dm))&&qx.core.Environment.get(K)){this.debug(e.getType()+cR+gy);}
;if(qx.core.Environment.get(dK)==dI||qx.core.Environment.get(dK)==cb){if(this.__xK){switch(gy){case cM:if(this.getInsertLinebreakOnCtrlEnter()){if((qx.core.Environment.get(dK)==cb)){this.__xL();e.preventDefault();e.stopPropagation();}
else {var gz=this.__xV(this.getSelection());if(gz){gz.collapse(true);gz.pasteHTML(cF);}
;}
;this.__xQ();}
;break;case cv:this.__xK=false;return;break;};}
;}
else if((qx.core.Environment.get(dK)==dJ)){switch(gy){case V:case cN:case bi:case B:case dz:case E:case ce:case ep:case em:this.__xl=(this.getFocusNode()==this.getContentBody().firstChild);break;};}
;}
,__xL:function(){var gA=this.getSelection();var gB=dL;if(gA&&(gA.focusNode.textContent==dL||gA.focusNode.parentElement.tagName==bz)){gB=dQ;}
;this.__wn.execute(cY,gB+qx.bom.htmlarea.HtmlArea.simpleLinebreak);}
...
;if(this.__xK&&(gC==bY||gC==ei||gC==bl||gC==cV||gC==ek||gC==g)){e.preventDefault();e.stopPropagation();}
;if(gC==cv){this.__xK=true;}
;}
,"default":function(e){}
}),_handleKeyPress:function(e){var gH=this.getContentDocument();var gD=e.getKeyIdentifier().toLowerCase();var gF=e.isCtrlPressed();var gG=e.isShiftPressed();this.__xn=e;if((qx.core.Environment.get(dm))&&qx.core.Environment.get(K)){this.debug(e.getType()+cR+gD);}
;if(this.__xr){var gN=!((qx.core.Environment.get(dK)==dI)&&gD==cM)||!((qx.core.Environment.get(dK)==dJ)&&gD==cM);if(gN){this.__xQ();this.__xr=false;}
;}
;switch(gD){case cM:if(!gG&&!gF&&this.getMessengerMode()){e.preventDefault();e.stopPropagation();this.fireDataEvent(bR,this.getComputedValue());this.resetHtml();}
;if(gF){if(!this.getInsertLinebreakOnCtrlEnter()){return;}
;e.preventDefault();e.stopPropagation();if((qx.core.Environment.get(dK)==dJ)){if(this.__xT()){this.insertHtml(bc);this.__xQ();return;}
;this.insertHtml(bj);}
else if((qx.core.Environment.get(dK)==bS)){var gM=this.getSelection();var gI=this.__xV(gM);if(gM&&gI){var gJ=gH.createElement(cP);gI.collapse(true);gI.insertNode(gJ);gI.collapse(true);gI.selectNode(gJ);gM.addRange(gI);gI.collapse(true);}
;}
;this.__xQ();}
;if((qx.core.Environment.get(dK)==dI)){if(!this.getInsertParagraphOnLinebreak()){if(this.__wn.execute(cY,qx.bom.htmlarea.HtmlArea.simpleLinebreak)){this.__xQ();e.preventDefault();e.stopPropagation();}
;}
;}
else if((qx.core.Environment.get(dK)==dJ)){if(this.getInsertParagraphOnLinebreak()&&!gG&&!gF){var gM=this.getSelection();if(gM){var gE=gM.focusNode;if(this.__xT()){this.__xQ();return;}
;if(this.__xU()){this.__xQ();return;}
;while(!qx.dom.Node.isNodeName(gE,cW)){if(qx.dom.Node.isNodeName(gE,dS)){this.__xQ();return;}
;gE=gE.parentNode;}
;}
;this.__wn.insertParagraphOnLinebreak();e.preventDefault();e.stopPropagation();this.__xQ();this.__xr=true;}
;}
else if((qx.core.Environment.get(dK)==cb)){if(this.getInsertParagraphOnLinebreak()&&gG){this.__xL();e.preventDefault();e.stopPropagation();this.__xQ();}
;}
;;break;case bi:if(qx.core.Environment.get(dK)==dJ&&qx.core.Environment.get(da)<1.9&&gG){var gM=this.getSelection();if(gM&&gM.focusNode==gH.body.firstChild){if(this.__xl){if(gM.focusOffset!=0){gM.extend(gM.focusNode,0);}
;}
;}
;}
;this.__xQ();break;case cu:if(qx.core.Environment.get(dK)==dJ&&qx.core.Environment.get(da)<1.9){if(gF){var gM=this.getSelection();if(gG){if(gM&&(gM.focusOffset!=0)||(gM.focusNode!=gH.body.firstChild)){gM.extend(gH.body.firstChild,0);}
;}
else {var gK=null;var gL;if(gH){gK=gH.evaluate(dg,gH.body,null,XPathResult.ANY_TYPE,null);}
;if(gK&&gM){while(gL=gK.iterateNext()){if(gL&&gL.parentNode&&gL.parentNode.tagName!=bf){try{gM.extend(gL,0);if(!this.isSelectionCollapsed()){gM.collapseToStart();}
;}
catch(e){}
;break;}
;}
...
;}
;}
;this.__xQ();break;case V:case cN:case B:case dz:case E:case ce:case ep:case em:this.__xQ();break;case bl:if(gF){this.__xM(ea,true);}
;break;case ek:case g:if(gF){this.__xM(bA,true);}
;break;case cV:if(gF){this.__xM(dY,true);}
;break;case bY:if(gF&&!gG){this.__xM(dR,true);}
else if(gF&&gG){this.__xM(d,true);}
;break;case ei:if(gF){this.__xM(d,true);}
;break;case w:if(gF){this.selectAll();}
;break;};this.__xn=null;}
,__xM:function(gO,gP){var gR=null;var gQ=qx.bom.htmlarea.HtmlArea.hotkeyInfo;if(gQ[gO]){gR=gQ[gO].method;}
;if(gR!=null&&this[gR]){this[gR]();if(gP){this.__xn.preventDefault();this.__xn.stopPropagation();}
;if(this.isSelectionCollapsed()){this.__xr=true;}
;this.__xQ();}
;}
,_handleFocusEvent:function(e){this.__xo=null;if(qx.core.Environment.get(dK)==dJ||qx.core.Environment.get(dK)==cb){var gS=this.getContentDocument().getElementById(bX);if(gS){qx.bom.element.Attribute.reset(gS,bT);}
;}
;this.fireEvent(cs);}
,_handleBlurEvent:function(e){this.__xc=this.getComputedValue();}
,_handleFocusOutEvent:function(e){this.__xK=false;if(this.__xo==null){this.__xo=this.getSelectedHtml();}
;this.fireEvent(bn);}
,_handleMouseUpOnBody:function(e){if((qx.core.Environment.get(dm))&&qx.core.Environment.get(K)){this.debug(S+e.getType());}
;this.__xs=true;this.__xQ();}
,_handleMouseUpOnDocument:qx.core.Environment.select(dK,{"mshtml":qx.lang.Function.empty,"default":function(e){if(!this.__xs){qx.bom.Element.activate(this.getContentBody());}
;this.__xs=false;}
...
,this,0);}
;}
,isLoaded:function(){return this.__xj;}
,insertHtml:function(gY){return this.__wn.execute(cY,gY);}
,removeFormat:function(){var ha=this.__wn.execute(y);this.__wn.execute(F,this.getDefaultFontSize());this.__wn.execute(cw,this.getDefaultFontFamily());return ha;}
,setBold:function(){return this.__wn.execute(en);}
,setItalic:function(){return this.__wn.execute(cX);}
,setUnderline:function(){return this.__wn.execute(cQ);}
,setStrikeThrough:function(){return this.__wn.execute(bK);}
,setFontSize:function(hb){return this.__wn.execute(F,hb);}
,setFontFamily:function(hc){return this.__wn.execute(cw,hc);}
,setTextColor:function(hd){return this.__wn.execute(dN,hd);}
,setTextBackgroundColor:function(he){return this.__wn.execute(dx,he);}
,setJustifyLeft:function(){return this.__wn.execute(bo);}
,setJustifyCenter:function(){return this.__wn.execute(bk);}
,setJustifyRight:function(){return this.__wn.execute(cJ);}
,setJustifyFull:function(){return this.__wn.execute(be);}
,insertIndent:function(){return this.__wn.execute(bQ);}
,insertOutdent:function(){return this.__wn.execute(dh);}
,insertOrderedList:function(){return this.__wn.execute(ew);}
,insertUnorderedList:function(){return this.__wn.execute(cB);}
,insertHorizontalRuler:function(){return this.__wn.execute(q);}
,insertImage:function(hf){return this.__wn.execute(v,hf);}
,insertHyperLink:function(hg){return this.__wn.execute(bd,hg);}
,removeBackgroundColor:function(){this.__wn.execute(dy,cD);}
,setBackgroundColor:function(hh){this.__wn.execute(dy,hh);}
,removeBackgroundImage:function(){this.__wn.execute(m);}
,setBackgroundImage:function(hi,hj,hk){return this.__wn.execute(m,[hi,hj,hk]);}
,selectAll:function(){return this.__wn.execute(cE);}
,undo:function(){if(this.getUseUndoRedo()){return this.__wn.execute(G);}
else {return true;}
;}
,redo:function(){if(this.getUseUndoRedo()){return this.__wn.execute(ej);}
else {return true;}
;}
...
;return qx.bom.htmlarea.HtmlArea.__xf(hp.body,false,ho,this.getPostProcess());}
...
;this.__xN();}
,"opera":function(){qx.bom.Element.focus(this.getContentWindow());qx.bom.Element.focus(this.getContentBody());this.__xN();}
,"default":function(){qx.bom.Element.focus(this.getContentBody());this.__xN();}
}),__xN:function(){if(!this.__xO()){this.__xP();}
;}
,__xO:qx.core.Environment.select(dK,{"gecko":function(){var hx=this.getContentBody().childNodes;if(hx.length==0){return false;}
...
;}
}),__xP:qx.core.Environment.select(dK,{"gecko|webkit":function(){this.getContentDocument().body.innerHTML=this.__xv(dL);var hA=this.getContentDocument().getElementById(bX);qx.bom.element.Attribute.reset(hA,bT);this._selectElement(hA);}
...
,__xQ:function(){qx.event.Timer.once(function(e){var hC=this.__xR();this.fireDataEvent(ef,hC);}
,this,200);}
,__xR:function(){if(this._processingExamineCursorContext||this.getEditable()==false){return;}
;this._processingExamineCursorContext=true;if(!this.__xO()){this.__xP();}
...
else {hN=hT.getPropertyValue(by)==en||qx.dom.Node.isNodeName(hG,bl);}
;}
;var hE=hG;if(hE!=null&&hE.parentNode!=null&&!qx.dom.Node.isDocument(hE.parentNode)){while(hE!=null&&!qx.dom.Node.isNodeName(hE,cW)){var hL=qx.dom.Node.getName(hE);if(hL==dj){hS=true;break;}
...
}),getSelectedHtml:function(){if(this.__xo!=null){return this.__xo;}
;var hW=this.getRange();if(!hW){return dL;}
else {return this.__xS(hW);}
;}
,__xS:qx.core.Environment.select(dK,{"mshtml":function(hX){if(!hX){return dL;}
...
;}
}),__xT:qx.core.Environment.select(dK,{"gecko":function(){var ig=this.getSelection();var ih=this.getFocusNode();return ig&&this.isSelectionCollapsed()&&ih!=null&&qx.dom.Node.isText(ih)&&ig.anchorOffset<ih.length;}
,"default":function(){return false;}
}),__xU:qx.core.Environment.select(dK,{"gecko":function(){return qx.dom.Node.isElement(this.getFocusNode());}
,"default":function(){return false;}
}),getRange:function(){return this.__xV(this.getSelection());}
,__xV:qx.core.Environment.select(dK,{"mshtml":function(ii){var ij=this._getIframeDocument();if(ii){try{return ii.createRange();}
...
;}
}),saveRange:qx.core.Environment.select(dK,{"mshtml":function(){this.__xq=this.getRange();}
,"default":function(){}
}),getSavedRange:qx.core.Environment.select(dK,{"mshtml":function(){return this.__xq;}
,"default":function(){}
}),resetSavedRange:qx.core.Environment.select(dK,{"mshtml":function(){this.__xq=null;}
,"default":function(){}
}),getFocusNode:qx.core.Environment.select(dK,{"mshtml":function(){var ip=this.getSelection();var iq;switch(ip.type){case cA:case cd:iq=this.__xV(ip);if(iq){iq.collapse(false);return iq.parentElement();}
else {return null;}
;break;case N:iq=this.__xV(ip);if(iq){try{iq.collapse(false);}
catch(ir){}
;return iq.item(0);}
else {return null;}
...
})},environment:{"qx.bom.htmlarea.HtmlArea.debug":false},destruct:function(){try{var iu=this._getIframeDocument();var iw=qx.event.Registration;iw.removeListener(iu.body,A,this._handleKeyPress,this);iw.removeListener(iu.body,cT,this._handleKeyUp,this);iw.removeListener(iu.body,cU,this._handleKeyDown,this);var iv=qx.core.Environment.get(dK)==cb?this._getIframeWindow():iu.body;iw.removeListener(iv,cx,this._handleFocusEvent);iw.removeListener(iv,bh,this._handleBlurEvent);iw.removeListener(iu,cS,this._handleFocusOutEvent);var it=qx.core.Environment.get(dK)==dI?W:eh;iw.removeListener(iu.body,it,this._handleMouseUpOnBody,this);iw.removeListener(iu.documentElement,it,this._handleMouseUpOnDocument,this);iw.removeListener(iu.documentElement,bx,this._handleContextMenuEvent,this);if((qx.core.Environment.get(dK)==dI)){qx.bom.Iframe.setSource(this.__rN,de);}
;}
catch(ix){}
;if(this.__wn instanceof qx.core.Object){this._disposeObjects(dH);}
else {this.__wn=null;}
;this.__xp=this.__rN=this.__kq=this.__xm=null;}
});}
)();
...
;return h;}
,__qI:function(G){var H=function(){qx.bom.Event.removeNativeListener(G,a,H);G.$$url=qx.bom.Iframe.queryCurrentUrl(G);}
;qx.bom.Event.addNativeListener(G,a,H);}
}});}
)();
(function(){var cL="backgroundColor",cK="_moz",cJ="top",cI="__yJ",cH="formatBlock",cG="insertParagraph",cF=" succeded",cE="li",cD='<span style="',cC="underline",bM="__yC",bL="FONT",bK="' is not allowed for parameter 'position'. Possible values are '",bJ="no-repeat",bI=' <a href="',bH="__yr",bG="inserthyperlink failed!",bF="|top|bottom|center|left|right|right top|left top|left bottom|right bottom|",bE="__placeholder__",bD="JustifyCenter",cS="Cut",cT="outdent",cQ='</a> ',cR="InsertImage",cO="__yI",cP="__resetHtml",cM="FontName",cN="bold",cU="blockquote",cV="__yh",cl="If an array is provided for parameter 'position' it has to define two elements!",ck="__yq",cn="The method '",cm="qx.bom.htmlarea.manager.Command",cp="__yH",co="'><br class='webkit-block-placeholder' />",cr="a",cq='"',cj="__yo",ci="__yA",p="type",q="CreateLink",r="':'",s="italic",t="' you calling to execute the command '",u="qx_link",v="none",w='"></span>',x="Italic",y="fontStyle",dk="style",dj=" failed",di="InsertUnorderedList",dh="OL",dp='text-align:',dn="Bold",dm="ForeColor",dl="url(",dr='" ',dq="repeat repeat-x repeat-y no-repeat",bd='</p>',be="<hr />",bb="UL",bc="__yF",bh="editor not ready! '",bi='<font style="',bf="Command ",bg="IMG",Y='<p id="',ba="StrikeThrough",L="LI",K="__getHtml",N='>',M='font-family:',H='" size="',G="Indent",J="HiliteColor",I="__yE",F="br",E="fontWeight",bn="Copy",bo="</span>",bp="Paste",bq="RemoveFormat",bj="JustifyRight",bk="' is not allowed for parameter 'repeat'. Possible values are '",bl='',bm="__yB",br='style="',bs="' is not available!",V="_moz_dirty",U="color",T="JustifyLeft",S=" is currently not supported!",R="Underline",Q="SelectAll",P="<p style='",O="line-through",X=")",W="href",bt="__yK",bu='<span id="',bv="<span></span>",bw="__yz",bx="JustifyFull",by="__yD",bz="Outdent",bA="indent",bB="selectall",bC="InsertOrderedList",bQ="/>",bP="<img ",bO="</font>",bN="backgroundPosition",bU='|',bT="font-size",bS="string",bR="background-color",bW="backgroundRepeat",bV="font",ce=":",cf="='",cc=" with value ",cd=" ",ca=";",cb="body",bX="px",bY="backgroundImage",cg="__ym",ch="Control",cv="__paragraph__",cu=':',cx="The value '",cw="qx.bom.htmlarea.HtmlArea.debug",cz="size",cy="' ",cB="webkit",cA="execCommand ",ct="textDecoration",cs="BODY",dd="undefined",de="transparent",df="id",dg="InsertHtml",cY='">',da="font-family",db="child",dc="gecko",cW="text-align",cX='text-decoration',o="BackColor",n="FontSize",m="'",h=';',g="p",f="span",d="__yl",c='color',b="text-decoration",a="inserthtml",B="qx.debug",C="mshtml",z="legacy-font-size",A="",D="engine.name";qx.Class.define(cm,{extend:qx.core.Object,construct:function(ds){qx.core.Object.call(this);this.__wo=ds;this.__kp=null;this._commands=null;this.__wQ();this.__xW={"Bold":true,"Italic":true,"Underline":true,"StrikeThrough":true};this.__xX=[10,12,16,18,24,32,48];this.__xY=0;}
,statics:{__ya:bF,__yb:dq},members:{__kp:null,__wo:null,__yc:false,__xW:null,__xX:null,__xY:null,setContentDocument:function(dt){this.__kp=dt;}
,getCommandObject:function(du){if(this._commands[du]){return this._commands[du];}
else {return null;}
;}
,__wQ:function(){this._commands={bold:{useBuiltin:false,identifier:dn,method:cp},italic:{useBuiltin:false,identifier:x,method:cO},underline:{useBuiltin:false,identifier:R,method:cI},strikethrough:{useBuiltin:false,identifier:ba,method:bt},fontfamily:{useBuiltin:true,identifier:cM,method:null},fontsize:{useBuiltin:false,identifier:n,method:bw},textcolor:{useBuiltin:true,identifier:dm,method:null},textbackgroundcolor:{useBuiltin:false,identifier:null,method:ci},backgroundcolor:{useBuiltin:false,identifier:null,method:bm},backgroundimage:{useBuiltin:false,identifier:null,method:bM},justifyleft:{useBuiltin:false,identifier:T,method:d},justifyright:{useBuiltin:false,identifier:bj,method:d},justifycenter:{useBuiltin:false,identifier:bD,method:d},justifyfull:{useBuiltin:false,identifier:bx,method:d},indent:{useBuiltin:true,identifier:G,method:null},outdent:{useBuiltin:true,identifier:bz,method:null},copy:{useBuiltin:true,identifier:bn,method:null},cut:{useBuiltin:true,identifier:cS,method:null},paste:{useBuiltin:true,identifier:bp,method:null},insertorderedlist:{useBuiltin:false,identifier:bC,method:cg},insertunorderedlist:{useBuiltin:false,identifier:di,method:cg},inserthorizontalrule:{useBuiltin:false,identifier:dg,method:bH},insertimage:{useBuiltin:false,identifier:cR,method:cj},inserthyperlink:{useBuiltin:false,identifier:q,method:ck},selectall:{useBuiltin:false,identifier:Q,method:by},selectedtext:{useBuiltin:false,identifier:null,method:I},selectedhtml:{useBuiltin:false,identifier:null,method:bc},inserthtml:{useBuiltin:false,identifier:dg,method:cV},resethtml:{useBuiltin:false,identifier:null,method:cP},gethtml:{useBuiltin:false,identifier:null,method:K},removeformat:{useBuiltin:true,identifier:bq,method:null}};}
,execute:function(dv,dw){if(!this.__wo.isReady()){this.error(bh+dv+r+dw+m);return false;}
;dv=dv.toLowerCase();dw=dw!=null?dw:null;if(this._commands[dv]){var dx;var dy=this._commands[dv];if(!((qx.core.Environment.get(D)==cB)&&(dv==bA||dv==cT))){if(this.__yd()){this.__ye();}
;}
;if(dy.useBuiltin){dx=this.__yf(dy.identifier,false,dw);}
else {if(dy.method!=null&&this[dy.method]){dx=this[dy.method].call(this,dw,dy);}
else {this.error(cn+dy.method+t+dv+bs);}
;}
;this.__wo.resetSavedRange();return dx;}
else {this.error(bf+dv+S);}
;}
,__yd:function(){var Node=qx.dom.Node;var dz=this.__wo.getFocusNode();var dC=false;var dA=false;if(dz){if(Node.isText(dz)){var dB=qx.dom.Hierarchy.getAncestors(dz);for(var i=0,j=dB.length;i<j;i++){if(Node.isNodeName(dB[i],g)||qx.bom.htmlarea.HtmlArea.isHeadlineNode(dB[i])){dC=true;break;}
;}
;}
else if(Node.isNodeName(dz,cb)){dA=true;}
;}
;return dA||!dC;}
,__ye:function(){this.__yf(cH,false,g);}
,__yf:function(dD,dE,dF){try{var dI=this.__kp;var dG=false;var dH=this.__wo.getRange();this.__kp.body.focus();if((qx.core.Environment.get(D)==C)){if(dD!=bB){dH.select();if(((dH.text)&&(dH.text.length>0))||((dH.length==1)&&(dH.item(0))&&(dH.item(0).tagName==bg))){dI=dH;}
else {dI=this.__kp;}
;}
;if(((qx.core.Environment.get(D)==C))&&(this.__xW[dD])){if(dH.text==A){dG=true;}
;}
;}
;var dJ=dI.execCommand(dD,dE,dF);if(dG&&dH.text!=A){dH.collapse();}
;if((qx.core.Environment.get(B))&&qx.core.Environment.get(cw)){this.debug(cA+dD+cc+dF+cF);}
;this.__yc=false;}
catch(dK){if((qx.core.Environment.get(B))&&qx.core.Environment.get(cw)){this.debug(cA+dD+cc+dF+dj);}
;return false;}
;return dJ;}
,__yg:qx.core.Environment.select(D,{"mshtml":function(){var dM=this.__wo;var dL=dM.getSavedRange()!=null?dM.getSavedRange():dM.getRange();return dL;}
,"default":function(){return this.__wo.getRange();}
}),__yh:qx.core.Environment.select(D,{"mshtml":function(dN,dO){var dR;if(dN==qx.bom.htmlarea.HtmlArea.simpleLinebreak){return this.__yk();}
else {this.__kp.body.focus();var dQ=this.__wo.getSelection();var dP=this.__yg();if(dP&&dQ&&dQ.type!=ch){try{dP.pasteHTML(dN);dP.collapse(false);dP.select();dR=true;}
catch(e){}
;}
else {dR=false;}
;this.__wo.resetSavedRange();return dR;}
;}
,"default":function(dS,dT){this.__kp.body.focus();return this.__kp.execCommand(dT.identifier,false,dS);}
}),insertParagraphOnLinebreak:qx.core.Environment.select(D,{"gecko":function(){var ec=this.__yu();var eg=this.__yi(ec);var dX=this.__ys(ec);var ed=bE+Date.parse(new Date());var ee=cv+Date.parse(new Date());var dV=bl;var ea=bu+ed+w;dV+=ea;dV+=Y+ee+dr+eg+N;dV+=dX+bd;this.__wo.getCommandManager().addUndoStep(a,cG,this.getCommandObject(a));this.execute(a,dV);this.__yj();qx.bom.element.Attribute.reset(this.__kp.getElementById(ed),df);var eb=this.__kp.getElementById(ee);if(eb.previousSibling.innerHTML==ea){var dU=this.__yt();var dY=this.__kp.createElement(F);var dW=this.__kp.createAttribute(V);dW.nodeValue=A;dY.setAttributeNode(dW);var ef=this.__kp.createAttribute(p);ef.nodeValue=cK;dY.setAttributeNode(ef);eb.previousSibling.appendChild(dU);eb.previousSibling.appendChild(dY);}
...
;}
;this.__wo.insertHtml(P+ei+co);}
,"default":qx.lang.Function.empty}),__yi:qx.core.Environment.select(D,{"gecko":function(el){var eq=br;var ep=el.child;if(ep[cW]){eq+=dp+ep[cW]+h;delete el.child[cW];}
;if(ep[da]){eq+=M+ep[da]+h;delete el.child[da];}
;var eo={"padding-top":true,"padding-bottom":true,"padding-left":true,"padding-right":true};var em={"margin-top":true,"margin-bottom":true,"margin-left":true,"margin-right":true};for(var en in ep){if(eo[en]||em[en]){eq+=en+cu+ep[en]+h;delete el.child[en];}
;}
;eq+=cq;return eq;}
,"default":function(){return A;}
}),__yj:qx.core.Environment.select(D,{"gecko":function(){var ez=this.__wo.getSelection();if(!ez||!ez.focusNode){return;}
;var er=ez.focusNode;var ew=ez.focusNode;while(!qx.dom.Node.isNodeName(ew,g)){ew=ew.parentNode;}
;var ev=ew.previousSibling.id;var eA=ew.nextSibling?ew.nextSibling.id:null;if(qx.lang.String.startsWith(ev,cv)&&ev==eA){var ey=ew.nextSibling;var et=this.__wo.getRange();et.selectNode(ey);ez.addRange(et);var es=qx.bom.htmlarea.HtmlArea.EMPTY_DIV;this.__wo.getCommandManager().addUndoStep(a,es,this.getCommandObject(a));this.execute(a,es);var eu=this.__wo.getRange();if(er){while(er&&er.firstChild&&qx.dom.Node.isElement(er.firstChild)){er=er.firstChild;}
;}
;eu.selectNode(er);ez.addRange(eu);eu.collapse(true);}
;}
,"default":qx.lang.Function.empty}),__yk:qx.core.Environment.select(D,{"mshtml":function(){var eB=this.__wo.getRange();if(eB&&!qx.dom.Node.isNodeName(eB.parentElement(),cE)){eB.pasteHTML(qx.bom.htmlarea.HtmlArea.simpleLinebreak);eB.collapse(false);eB.select();return true;}
;return false;}
,"default":function(){return false;}
}),__yl:function(eC,eD){var eE=(qx.core.Environment.get(D)==C)?this.__wo.getRange():this.__kp;return eE.execCommand(eD.identifier,false,eC);}
,__ym:function(eF,eG){if((qx.core.Environment.get(D)==C)){var eH=this.__wo.getFocusNode();this.__yn(eH);}
;this.__kp.body.focus();var eI=this.__kp.execCommand(eG.identifier,false,eF);if((qx.core.Environment.get(D)==cB)){var eH=this.__wo.getFocusNode();this.__yn(eH.parentNode);}
;return eI;}
,__yn:function(eJ){var eK=[];var parent=eJ.parentNode;while(qx.dom.Node.isNodeName(parent,cU)){eK.push(parent);parent=parent.parentNode;}
;if(eK.length>0){parent.appendChild(eJ);parent.removeChild(eK[eK.length-1]);}
;}
,__yo:qx.core.Environment.select(D,{"gecko":function(eL,eM){if(eL.src){this.__kp.execCommand(eM.identifier,false,eL.src);delete eL.src;var fa=this.__wo.getSelection();if(fa){var eY=fa.anchorNode;var eT=fa.anchorOffset;var eQ=eY.childNodes[eT-1];var eW;for(var eS in eL){eW=this.__kp.createAttribute(eS);eW.nodeValue=eL[eS];eQ.setAttributeNode(eW);}
...
;var eP=this.__kp.createDocumentFragment();var fb;if(eV&&eO!=null){var eR=this.__yp(eO);eP.appendChild(eR.root);fb=eR.inline;}
...
;var eN=this.__wo.getRange();eN.selectNodeContents(fb);}
;return true;}
else {return false;}
;}
,"mshtml":function(fc,fd){var fh=false;var fi=bP;for(var fe in fc){fi+=fe+cf+fc[fe]+cy;}
;fi+=bQ;var ff=this.__wo.getSelection();var fg=this.__yg();if(ff&&ff.type!=ch){try{fg.select();fg.pasteHTML(fi);fh=true;}
catch(e){}
;}
;this.__wo.resetSavedRange();return fh;}
,"default":function(fj,fk){if(fj.src){var fm=bP;for(var fl in fj){fm+=fl+cf+fj[fl]+cy;}
;fm+=bQ;return this.__kp.execCommand(dg,false,fm);}
else {return false;}
;}
}),__yp:function(fn){while(fn.firstChild&&fn.firstChild.nodeType==1){fn=fn.firstChild;}
;var fr=this.__yu(fn);var fv,fp,ft;var fo=A;var parent=null;var fu=null;var fs=fr.child;while(fs){fu=this.__kp.createElement(fs[z]?bV:f);fp=this.__kp.createAttribute(dk);fu.setAttributeNode(fp);for(var fq in fs){if(fq!=db&&fq!=z){fo+=fq+ce+fs[fq]+ca;}
else if(fq==z){ft=this.__kp.createAttribute(cz);ft.nodeValue=fs[fq];fu.setAttributeNode(ft);}
;}
;fp.nodeValue=fo;if(parent!=null){parent.appendChild(fu);}
else {fv=fu;}
;parent=fu;fs=fs.child;fo=A;}
;return {root:fv,inline:fu};}
,__yq:qx.core.Environment.select(D,{"gecko|opera":function(fw,fx){var fD=this.__wo.getSelection();var fA=this.__wo.getRange();if(fD.isCollapsed){var fz=u+(++this.__xY);var fE=this.__kp.createElement(cr);var fB=this.__kp.createAttribute(W);var fy=this.__kp.createAttribute(df);var fC=this.__kp.createTextNode(fw);fy.nodeValue=fz;fE.setAttributeNode(fy);fB.nodeValue=fw;fE.setAttributeNode(fB);fE.appendChild(fC);fA.insertNode(fE);fA.selectNode(fE);fD.collapseToEnd();return true;}
else {return this.__kp.execCommand(fx.identifier,false,fw);}
;}
,"mshtml":function(fF,fG){try{var fI;var fH=this.__yg();var fJ=this.__wo;var fH=fJ.getSavedRange()!=null?fJ.getSavedRange():fJ.getRange();if(fH!=null&&fH.text!=A){fI=fH.execCommand(fG.identifier,false,fF);}
else {fI=this.__yh(bI+fF+cY+fF+cQ,fG);}
;this.__wo.resetSavedRange();return fI;}
...
}),__yr:function(fM,fN){var fO=be;if((qx.core.Environment.get(D)==dc)){fO+=this.__ys();}
;return this.__yh(fO,fN);}
,__ys:function(fP){var fT=A;var fS=cD;var fR=[];var fW=typeof fP!==dd?fP:this.__yu();var fU=fW.child;var fV=false;if(qx.lang.Object.isEmpty(fU)){return A;}
;while(fU){fV=fU[z]!=null;fT+=fV?bi:fS;for(var fQ in fU){fT+=(fQ!=db&&fQ!=z)?fQ+cu+fU[fQ]+h:A;}
;fT+=fV?H+fU[z]+cY:cY;fR.unshift(fV?bO:bo);fU=fU.child;}
;if(fR.length==1&&fR[0]==bO){fT+=bv;}
;for(var i=0,j=fR.length;i<j;i++){fT+=fR[i];}
;return fT;}
,__yt:function(){var fY=this.__kp.createDocumentFragment();var ge=this.__yu();var parent=fY;var gc=ge.child;var ga;var gd=false;while(gc){gd=gc[z]!=null;ga=this.__kp.createElement(gd?bV:f);parent.appendChild(ga);for(var fX in gc){if(fX!=db&&fX!=z){qx.bom.element.Style.set(ga,fX,gc[fX]);}
;}
;if(gd){var gb=this.__kp.createAttribute(cz);gb.nodeValue=gc[z];ga.setAttributeNode(gb);}
;parent=ga;gc=gc.child;}
;return fY;}
,__yu:function(gf){var gh={};var gi=null;var gj=this.getCurrentStyles(gf);gi=gh.child={};for(var gg in gj){if(gg!=b){gi[gg]=gj[gg];}
;}
...
;return gh;}
,getCurrentStyles:function(gl){if(gl==null){var gn=this.__wo.getSelection();if(!gn||gn.focusNode==null){return {};}
;gl=(gn.focusNode.nodeType==3)?gn.focusNode.parentNode:gn.focusNode;}
;var go=qx.dom.Hierarchy.getAncestors(gl);var gq=qx.lang.Array.insertBefore(go,gl,go[0]);var gm=this.__yv(gq);var gp=this.__yw(gm,gq);return gp;}
,__yv:function(gr){var gu={};var gs,gt;for(var i=0,j=gr.length;i<j;i++){gt=gr[i];for(var k=0,l=gt.style.length;k<l;k++){gs=gt.style[k];if(gs.length>0&&typeof gu[gs]===dd){gu[gs]=gt.style.getPropertyValue(gs);}
;}
;if(gt.tagName.toUpperCase()==bL&&gt.size&&gu[z]===undefined){gu[z]=gt.size;}
;}
;if(gu[z]&&gu[bT]){delete gu[bT];}
;return gu;}
,__yw:function(gv,gw){var gB=gw[0];var gz=this.__wo.getContentWindow().getComputedStyle(gB,null);var gx;var gA={};for(var gy in gv){if(gy!=z){gx=gz.getPropertyValue(gy);}
else {gx=gv[gy];}
;if(gy==bR&&gx==de){gA[gy]=this.__yy(gw);}
else if(gy==b){gA[gy]=this.__yx(gw);}
else {gA[gy]=gx;}
;}
;return gA;}
,__yx:function(gC){var gF,gH,gD;var gG=[];var gE=this.__wo.getContentWindow();for(var i=0,j=gC.length;i<j;i++){gD=gE.getComputedStyle(gC[i],null);gF=gD.getPropertyValue(b);gH=gD.getPropertyValue(U);if(gF!=v){gG.push({'text-decoration':gF,'color':gH});}
;}
;return gG;}
,__yy:function(gI){var gK,gJ,gL;for(var i=0;i<gI.length;i++){gK=gI[i];gJ=this.__wo.getContentWindow().getComputedStyle(gK,null);gL=gJ.getPropertyValue(bR);if(gL!=de){return gL;}
;}
;}
,__yz:function(gM,gN){var ha=this.__wo.getSelection();var gU=((qx.core.Environment.get(D)==C))?this.__wo.getRange():gU=ha.getRangeAt(0);var gV=[];var gY;var gT=[dh,bb];var gO,i,j,gW;gW=((qx.core.Environment.get(D)==C))?gU.parentElement():gU.commonAncestorContainer;if(gW.tagName==cs){for(var i=0;i<gT.length;i++){gO=gW.getElementsByTagName(gT[i]);for(var j=0;j<gO.length;j++){if(gO[j]){gV.push(gO[j]);}
;}
;}
;}
else if(qx.lang.Array.contains(gT,gW.tagName)){gV.push(gW);}
;if(gV.length>0){for(var i=0;i<gV.length;i++){var gQ=gV[i];gY=((qx.core.Environment.get(D)==C))?((gQ==gW)||(gW.tagName==cs)):ha.containsNode(gQ,false);for(j=0;j<gQ.childNodes.length;j++){var gS=gQ.childNodes[j];if(gY||(ha.focusNode.nodeValue&&qx.dom.Hierarchy.contains(gS,ha.focusNode)&&(ha.focusOffset==ha.focusNode.nodeValue.length))||(qx.dom.Hierarchy.contains(gS,ha.anchorNode)&&(ha.anchorOffset==0))||(ha.containsNode(gS,false))){gS.style.fontSize=(this.__xX[gM]||gM)+bX;}
;}
;}
;}
else {var gR=((qx.core.Environment.get(D)==C))?gW:ha.focusNode;var gX=qx.dom.Hierarchy.getAncestors(gR);for(i=0;i<gX.length;i++){if(gX[i].tagName==L){if((((qx.core.Environment.get(D)==dc))&&((ha.anchorOffset==0)&&(ha.focusNode.nodeValue&&(ha.focusOffset==ha.focusNode.nodeValue.length))&&qx.dom.Hierarchy.contains(gX[i].firstChild,ha.anchorNode)&&qx.dom.Hierarchy.contains(gX[i].lastChild,ha.focusNode)))||(((qx.core.Environment.get(D)==C))&&(gU.htmlText==gX[i].innerHTML))){gX[i].style.fontSize=(this.__xX[gM]||gM)+bX;}
;break;}
;}
;}
;if((qx.core.Environment.get(D)==C)){this.__kp.body.focus();this.__wo.getRange().select();return this.__kp.execCommand(n,false,gM);}
...
,__yA:qx.core.Environment.select(D,{"mshtml":function(hb,hc){this.__kp.body.focus();return this.__kp.execCommand(o,false,hb);}
,"gecko|opera":function(hd,he){this.__kp.body.focus();return this.__kp.execCommand(J,false,hd);}
,"webkit":function(hf,hg){var hh=this.__wo.getSelection();var hi=this.__wo.getRange();if(!hh||!hh.isCollapsed){this.__kp.body.focus();this.__kp.execCommand(o,false,hf);if(hh){hh.collapseToEnd();}
;return true;}
...
;}
}),__yB:function(hm,hn){hm=hm!=null&&typeof hm==bS?hm:de;qx.bom.element.Style.set(this.__kp.body,cL,hm);return true;}
,__yC:function(ho,hp){var hq,hs,hr;var ht=qx.bom.htmlarea.manager.Command;if(ho==null){hq=null;}
...
else {if(hq.search(/^url.*\(/)==-1){hq=dl+hq+X;}
;}
;if(hs!=null&&!qx.lang.String.contains(ht.__yb,hs)){if(qx.core.Environment.get(B)){this.error(cx+hs+bk+ht.__yb+m);}
;return false;}
else {hs=bJ;}
;if(hr!=null){if(qx.lang.Type.isString(hr)&&!qx.lang.String.contains(ht.__ya,bU+hr+bU)){if(qx.core.Environment.get(B)){this.error(cx+hr+bK+ht.__ya+m);}
;return false;}
...
,__yD:qx.core.Environment.select(D,{"mshtml":function(hu,hv){var hw=this.__kp.body.createTextRange();hw.select();return true;}
,"default":function(hx,hy){return this.__yf(hy.identifier,false,hx);}
}),__yE:function(){return this.__wo.getSelectedText();}
,__yF:function(){return this.__wo.getSelectedHtml();}
,__yG:function(hz,hA){var hE=this.__wo.getFocusNode();if(hE.textContent==A){var hC=qx.dom.Hierarchy.getAncestors(hE);hC.unshift(hE);var Node=qx.dom.Node;var hB=qx.bom.element.Style;var hD=hC.shift();while(hC.length>0){if(Node.getName(hD)==g||Node.getName(hD)==cb){break;}
;if(hB.get(hD,hz,hB.LOCAL_MODE)==hA){hB.reset(hD,hz);return true;}
;hD=hC.shift();}
;}
;return false;}
,__yH:qx.core.Environment.select(D,{"gecko":function(hF,hG){if(this.__yG(E,cN)){return true;}
else {return this.__yf(hG.identifier,false,hF);}
;}
,"default":function(hH,hI){return this.__yf(hI.identifier,false,hH);}
}),__yI:qx.core.Environment.select(D,{"gecko":function(hJ,hK){if(this.__yG(y,s)){return true;}
else {return this.__yf(hK.identifier,false,hJ);}
;}
,"default":function(hL,hM){return this.__yf(hM.identifier,false,hL);}
}),__yJ:qx.core.Environment.select(D,{"gecko":function(hN,hO){if(this.__yG(ct,cC)){return true;}
else {return this.__yf(hO.identifier,false,hN);}
;}
,"default":function(hP,hQ){return this.__yf(hQ.identifier,false,hP);}
}),__yK:qx.core.Environment.select(D,{"gecko":function(hR,hS){if(this.__yG(ct,O)){return true;}
else {return this.__yf(hS.identifier,false,hR);}
;}
,"default":function(hT,hU){return this.__yf(hU.identifier,false,hT);}
})},destruct:function(){this.__kp=this.__wo=this._commands=null;this.__xW=this.__xX=null;}
});}
)();
(function(){var n="undo",m="pageup",l="ADD TO UNDO STACK",k="control",h="shift",g="end",f="right",d="qx_link",c="b",b="insertunorderedlist",bo="Undo",bn="pagedown",bm="k",bl="home",bk="Redo",bj="a",bi="insertorderedlist",bh="y",bg="justifyright",bf="__wn",u="redo",v="insertParagraph",s="qx.bom.htmlarea.manager.UndoRedo",t="left",q="i",r="u",o="enter",p="execCommand failed! Details: ",A="ADD TO REDO STACK",B="insertimage",K="z",H="undoRedoState",S="down",N="up",bb="body",X="backgroundColor",D="actionType ",be="mousedown",bd=" is not managed! Please provide a handler method!",bc="table",C="backgroundImage",F="backgroundRepeat",G="p",J="backgroundPosition",L="td",O="img",U="inserthyperlink",ba="mouseup",w="keypress",x="qx.bom.htmlarea.HtmlArea.debug",E="backgroundimage",R="qx.debug",Q="backgroundcolor",P="webkit",W=" ",V="Content",M="Internal",T="inserthtml",a="gecko",Y="Custom",y="Command",z="mshtml",I="engine.name";qx.Class.define(s,{extend:qx.core.Object,construct:function(bp,bq){qx.core.Object.call(this);this.__wn=bp;this.__wo=bq;this.__wp=[];this.__wq=[];this._commands=null;this.__kp=null;this.__wr={};this.__ws={command:true,content:true,custom:true};this.__wQ();this.__wt=qx.lang.Function.bind(this._handleKeyPress,this);this.__wu=qx.lang.Function.bind(this._handleMouseUp,this);if((qx.core.Environment.get(I)==z)){this.__wv=qx.lang.Function.bind(this._handleMouseDown,this);}
;}
,members:{__ww:false,__wx:false,__wy:false,__ws:null,__wr:null,__wn:null,__kp:null,__wp:null,__wq:null,__wo:null,__wt:null,__wu:null,__wv:null,__wz:null,setContentDocument:function(br){this.__kp=br;this.__wn.setContentDocument(br);qx.event.Registration.addListener(br.body,w,this.__wt,this);qx.event.Registration.addListener(br.body,ba,this.__wu,this);if((qx.core.Environment.get(I)==z)){qx.event.Registration.addListener(br.body,be,this.__wv,this,true);}
;}
,insertParagraphOnLinebreak:function(){return this.__wn.insertParagraphOnLinebreak();}
,execute:function(bs,bt){var bu;bs=bs.toLowerCase();if(this._commands[bs]){if(this._commands[bs].passthrough){bu=this.__wn.execute(bs,bt);}
else {bu=this[bs].call(this);}
;}
else {if((qx.core.Environment.get(I)==z)||(qx.core.Environment.get(I)==P)){this.__wR(bs,bt,this.__wn.getCommandObject(bs));bu=this.__wn.execute(bs,bt);if(!bu){this.__wp.pop();}
;}
else {bu=this.__wn.execute(bs,bt);if(bu){this.__wR(bs,bt,this.__wn.getCommandObject(bs));}
;}
;if(bs==n&&this.__wp.length==0){this.__wx=false;this.__xa();}
else if(bs==u&&this.__wq.length==0){this.__ww=false;this.__xa();}
;}
;this.__wy=false;return bu;}
,addUndoStep:function(bv,bw,bx){this.__wR(bv,bw,bx);}
,registerHandler:function(by,bz,bA,bB){this.__wr[by]={undo:bz,redo:bA,context:bB};}
,isUndoPossible:function(){return this.__wx;}
,undo:function(){var bE;if(this.__wy){this.__wT();}
;if(this.__wp.length>0){var bD=this.__wp.pop();if(this.__ws[bD.actionType.toLowerCase()]){switch(bD.actionType){case y:bE=this.__wB(bD);break;case V:bE=this.__wF(bD);break;case M:bE=this.__wE(bD);break;case Y:bE=this.__wA(bD);break;};}
else if(this.__wr[bD.actionType]){var bC=this.__wr[bD.actionType];bE=bC.undo.call(bC.context?bC.context:this,bD);this.__wV(bD);}
else {this.error(D+bD.actionType+bd);}
;this.__ww=true;this.__xa();return bE;}
;}
,__wA:qx.core.Environment.select(I,{"mshtml|webkit":function(bF){var bG=this.__kp.body.innerHTML;var bI=bF.content;this.__kp.body.innerHTML=bI;var bH=bF;bH.content=bG;this.__wV(bH);return true;}
,"default":function(bJ){var bN=bJ;var bM=qx.bom.element.Style;switch(bJ.command){case Q:bN.parameter=[bM.get(this.__kp.body,X)];break;case E:bN.parameter=[bM.get(this.__kp.body,C),bM.get(this.__kp.body,F),bM.get(this.__kp.body,J)];break;};this.__wV(bN);if(bJ.command==U){if((qx.core.Environment.get(I)==a)){var bL=d+this.__wn.__hyperLinkId;var bK=this.__kp.getElementById(bL);if(bK){bK.parentNode.removeChild(bK);return true;}
else {return false;}
;}
;}
else {return this.__wn.execute(bJ.command,bJ.value);}
;}
}),__wB:qx.core.Environment.select(I,{"mshtml|webkit":function(bO){}
,"default":function(bP){this.__wV(bP);if((qx.core.Environment.get(I)==a)){if(bP.command==T&&bP.value==qx.bom.htmlarea.HtmlArea.EMPTY_DIV&&this.__wC(T,v)){this.__wD();}
;}
;return this.__wG();}
}),__wC:function(bQ,bR){if(this.__wp.length>0){var bS=this.__wp[this.__wp.length-1];return (bS.command==bQ&&bS.value==bR);}
;return false;}
,__wD:function(){this.__wG();if(this.__wp.length>0){var bT=this.__wp.pop();this.__wV(bT);}
;}
,__wE:function(bU){this.__wV(bU);return this.__wG();}
,__wF:qx.core.Environment.select(I,{"gecko":function(bV){this.__wV(bV);try{return this.__wG();}
catch(bW){if(qx.core.Environment.get(R)){this.error(p+bW);}
;}
;}
,"mshtml|webkit":function(bX){}
,"default":function(bY){this.__wV(bY);return this.__wG();}
}),__wG:function(){try{return this.__kp.execCommand(bo,false,null);}
catch(e){return false;}
;}
,isRedoPossible:function(){return this.__ww;}
,redo:function(){if(this.__ww){var cb;if(this.__wq.length>0){var cc=this.__wq.pop();if(this.__ws[cc.actionType.toLowerCase()]){switch(cc.actionType){case y:cb=this.__wI(cc);break;case V:cb=this.__wO(cc);break;case M:cb=this.__wN(cc);break;case Y:cb=this.__wH(cc);break;};}
else if(this.__wr[cc.actionType]){var ca=this.__wr[cc.actionType];cb=ca.redo.call(ca.context?ca.context:this,cc);this.__wU(cc);}
else {this.error(D+cc.actionType+bd);}
;this.__wx=true;this.__xa();}
;return cb;}
;}
,__wH:qx.core.Environment.select(I,{"mshtml|webkit":function(cd){var ce=this.__kp.body.innerHTML;var cg=cd.content;this.__kp.body.innerHTML=cg;var cf=cd;cf.content=ce;this.__wU(cf);return true;}
,"default":function(ch){this.__wU(ch);return this.__wP();}
}),__wI:qx.core.Environment.select(I,{"mshtml|webkit":function(ci){}
,"default":function(cj){this.__wU(cj);var ck=this.__wP();if((qx.core.Environment.get(I)==a)){if(this.__wJ(T,qx.bom.htmlarea.HtmlArea.EMPTY_DIV)){var cl=this.__wK();this.__wL();if(cl!=null){this.__wM(cl);}
;}
;}
;return ck;}
}),__wJ:function(cm,cn){if(this.__wq.length>0){var co=this.__wq[this.__wq.length-1];return (co.command==cm&&co.value==cn);}
;return false;}
,__wK:function(){if(this.__wo==null){return null;}
;var cp=this.__wo.getSelection();var cq=cp?cp.focusNode:null;if(cq==null){return null;}
...
;}
,__wL:function(){var cs=this.__wq.pop();this.__wU(cs);this.__wP();}
,__wM:qx.core.Environment.select(I,{"gecko":function(ct){if(ct==this.__wo.getContentBody().lastChild){return;}
;var cw=ct.firstChild;while(cw.firstChild){cw=cw.firstChild;}
;var cu=this.__wo.getSelection();var cv=this.__wo.getRange();if(cu&&cv){cv.selectNode(cw);cu.addRange(cv);cv.collapse(true);}
;}
,"default":qx.lang.Function.empty}),__wN:function(cx){this.__wU(cx);return this.__wP();}
,__wO:qx.core.Environment.select(I,{"mshtml|webkit":function(cy){}
,"default":function(cz){this.__wU(cz);return this.__wP();}
}),__wP:function(){try{return this.__kp.execCommand(bk,false,null);}
catch(e){return false;}
;}
,__wQ:function(){this._commands={undo:{passthrough:false},redo:{passthrough:false}};this.__wn.getCommandObject(Q).customUndo=true;this.__wn.getCommandObject(E).customUndo=true;if((qx.core.Environment.get(I)==a)){}
;}
,__wR:qx.core.Environment.select(I,{"mshtml|webkit":function(cA,cB,cC){var cD=this.getUndoRedoObject();cD.commandObject=cC;cD.command=cA;cD.value=cB;cD.actionType=Y;cD.content=this.__kp.body.innerHTML;this.__wS(cD);}
,"default":function(cE,cF,cG){if(this.__wo==null){return;}
;var cI=this.getUndoRedoObject();cI.commandObject=cG;cI.command=cE;cI.value=cF;cI.actionType=Y;var cH=this.__wo.getSelection();if(cG.customUndo){var cJ=[];switch(cE){case Q:cJ.push(qx.bom.element.Style.get(this.__kp.body,X));break;case E:cJ.push(qx.bom.element.Style.get(this.__kp.body,C),qx.bom.element.Style.get(this.__kp.body,F),qx.bom.element.Style.get(this.__kp.body,J));break;case U:if(cH&&!cH.isCollapsed){cI.actionType=y;}
;break;};cI.parameter=cJ;}
else {if((qx.core.Environment.get(I)==a)){if(cH&&cH.isCollapsed){switch(cE){case bi:case b:case bg:case T:case B:cI.actionType=y;break;default:return;};}
else {cI.actionType=y;}
;}
else {cI.actionType=y;}
;}
;this.__wS(cI);}
}),__wS:function(cK){if(this.__wy){this.__wT();}
;this.__wU(cK);this.__ww=false;this.__wq=[];this.__xa();}
,__wT:function(){var cL=this.__wp[this.__wp.length-1];if(cL==null||cL.actionType!=V){var cM=this.getUndoRedoObject();cM.actionType=V;if((qx.core.Environment.get(I)==z)||(qx.core.Environment.get(I)==P)){cM.content=this.__wz;cM.actionType=Y;this.__wz=null;}
;this.__wU(cM);this.__wy=false;}
;}
...
,__wU:function(cN){if((qx.core.Environment.get(R))&&qx.core.Environment.get(x)){this.debug(l);this.debug(cN.actionType+W+cN.command+W+cN.value);}
;this.__wp.push(cN);}
,__wV:function(cO){if((qx.core.Environment.get(R))&&qx.core.Environment.get(x)){this.debug(A);this.debug(cO.actionType+W+cO.command+W+cO.value);}
;this.__wq.push(cO);}
,_handleKeyPress:function(e){var cP=e.getKeyIdentifier().toLowerCase();var cQ=e.isCtrlPressed();switch(cP){case k:case h:case t:case f:case N:case S:case m:case bn:case bl:case g:case o:break;case bj:case c:case q:case r:case bm:case bh:case K:if(!cQ){this.__wW();}
;break;default:this.__ww=false;this.__wq=[];this.__wW();};}
,__wW:function(){if(!this.__wy){this.__wy=true;this.__wx=true;if((qx.core.Environment.get(I)==z)||(qx.core.Environment.get(I)==P)){this.__wz=this.__kp.body.innerHTML;}
;this.__xa();}
;}
,__wX:null,_handleMouseDown:qx.core.Environment.select(I,{"mshtml":function(e){var cR=e.getOriginalTarget();if(qx.dom.Node.isElement(cR)&&(qx.dom.Node.isNodeName(cR,O)||qx.dom.Node.isNodeName(cR,bc))){this.__wX={node:cR,content:cR.outerHTML};}
else {this.__wX=null;}
;}
,"default":function(e){return true;}
}),_handleMouseUp:qx.core.Environment.select(I,{"gecko":function(e){if(this.__wo==null){return;}
;var cT=this.__wo.getSelection();if(!cT){this.__wX=null;return;}
;var cS=cT.anchorNode;var cU=cS.childNodes[cT.anchorOffset];if(qx.dom.Node.isNodeName(cU,O)){if(this.__wX==null){this.__wX=cU.cloneNode(true);}
else {if(this.__wX.style.width!=cU.style.width||this.__wX.style.height!=cU.style.height){this.__wY();this.__wX=cU.cloneNode(true);}
;}
;}
...
;if(this.__wX==null){this.__wX=cV.cloneNode(true);}
else {qx.event.Timer.once(function(){if(cV.style.width!=this.__wX.style.width||cV.style.height!=this.__wX.style.height||cV.innerHTML!=this.__wX.innerHTML){this.__wY();this.__wX=cV.cloneNode(true);}
;}
,this,0);}
;}
else {this.__wX=null;}
;}
,"default":function(e){var cW=qx.bom.Event.getTarget(e);if(this.__wX!=null){if(cW.nodeType==1){if(cW==this.__wX.node){if(cW.outerHTML!=this.__wX.content){this.__wX.content=cW.outerHTML;this.__wY();}
;}
else {for(var i=0,j=cW.childNodes.length;i<j;i++){if(cW.childNodes[i]==this.__wX.node){if(cW.childNodes[i].outerHTML!=this.__wX.content){this.__wX.content=cW.childNodes[i].outerHTML;this.__wY();}
;}
;}
;}
;}
...
;}
;}
}),__wY:qx.core.Environment.select(I,{"mshtml|webkit":function(){this.__wR(M,null,null);}
,"default":function(){var cX=this.getUndoRedoObject();cX.actionType=M;this.__wU(cX);}
}),__xa:function(){qx.event.Timer.once(function(e){if(this.__wo!=null){var cY={undo:this.isUndoPossible()?0:-1,redo:this.isRedoPossible()?0:-1};this.__wo.fireDataEvent(H,cY);}
;}
,this,200);}
},destruct:function(){try{qx.event.Registration.removeListener(this.__kp.body,w,this.__wt);qx.event.Registration.removeListener(this.__kp,ba,this.__wu);if((qx.core.Environment.get(I)==z)){qx.event.Registration.removeListener(this.__kp,be,this.__wv);}
;}
catch(e){}
;this._disposeObjects(bf);this.__wo=this.__wp=this.__wq=this._commands=this.__kp=null;this.__ws=this.__wr=null;}
});}
)();
...
(function(){var i=" array contains ",h="qx.ui.core.MSingleSelectionHandling",g="__dz",f=" items!",d="changeSelection",c="changeSelected",b="Could only select one item, but the selection",a="qx.event.type.Data";qx.Mixin.define(h,{events:{"changeSelection":a},members:{__dz:null,getSelection:function(){var j=this.__mK().getSelected();if(j){return [j];}
else {return [];}
;}
,setSelection:function(k){switch(k.length){case 0:this.resetSelection();break;case 1:this.__mK().setSelected(k[0]);break;default:throw new Error(b+i+k.length+f);};}
,resetSelection:function(){this.__mK().resetSelected();}
,isSelected:function(l){return this.__mK().isSelected(l);}
,isSelectionEmpty:function(){return this.__mK().isSelectionEmpty();}
,getSelectables:function(m){return this.__mK().getSelectables(m);}
,_onChangeSelected:function(e){var o=e.getData();var n=e.getOldData();o==null?o=[]:o=[o];n==null?n=[]:n=[n];this.fireDataEvent(d,o,n);}
,__mK:function(){if(this.__dz==null){var p=this;this.__dz=new qx.ui.core.SingleSelectionManager({getItems:function(){return p._getItems();}
...
(function(){var o=", because it is not a child element!",n=" because it is not a child element!",m="Boolean",l="qx.ui.core.SingleSelectionManager",k="qx.debug",j="Invalid selectionProvider!",h="Could not check if ",g=" is selected,",f="__mN",e="__mL",b="changeSelected",d="Could not select ",c="__mM",a="qx.event.type.Data";qx.Class.define(l,{extend:qx.core.Object,construct:function(p){qx.core.Object.call(this);if(qx.core.Environment.get(k)){qx.core.Assert.assertInterface(p,qx.ui.core.ISingleSelectionProvider,j);}
;this.__mL=p;}
,events:{"changeSelected":a},properties:{allowEmptySelection:{check:m,init:true,apply:f}},members:{__mM:null,__mL:null,getSelected:function(){return this.__mM;}
,setSelected:function(q){if(!this.__mP(q)){throw new Error(d+q+o);}
;this.__mO(q);}
,resetSelected:function(){this.__mO(null);}
,isSelected:function(r){if(!this.__mP(r)){throw new Error(h+r+g+n);}
;return this.__mM===r;}
,isSelectionEmpty:function(){return this.__mM==null;}
,getSelectables:function(s){var t=this.__mL.getItems();var u=[];for(var i=0;i<t.length;i++){if(this.__mL.isItemSelectable(t[i])){u.push(t[i]);}
;}
;if(!s){for(var i=u.length-1;i>=0;i--){if(!u[i].getEnabled()){u.splice(i,1);}
;}
;}
;return u;}
,__mN:function(v,w){if(!v){this.__mO(this.__mM);}
;}
,__mO:function(x){var A=this.__mM;var z=x;if(z!=null&&A===z){return;}
;if(!this.isAllowEmptySelection()&&z==null){var y=this.getSelectables(true)[0];if(y){z=y;}
;}
;this.__mM=z;this.fireDataEvent(b,z,A);}
,__mP:function(B){var C=this.__mL.getItems();for(var i=0;i<C.length;i++){if(C[i]===B){return true;}
;}
;return false;}
},destruct:function(){if(this.__mL.toHashCode){this._disposeObjects(e);}
else {this.__mL=null;}
...
;if((m instanceof qx.core.Object)&&qx.Class.hasMixin(m.constructor,qx.data.marshal.MEventBubbling)){var o=qx.lang.Function.bind(this.__lV,this,name);var q=m.addListener(f,o,this);var p=m.getUserData(b+this.$$hash);if(p==null){p=[];m.setUserData(b+this.$$hash,p);}
;p.push(q);}
;}
...
;var z=this.__lW.concat();this.__lW.reverse();this.__lY(0,this.length);this.fireDataEvent(d,{start:0,end:this.length-1,type:h,items:null},null);this.fireDataEvent(g,{value:this.__lW,name:f+(this.__lW.length-1),old:z,item:this});}
,shift:function(){if(this.length==0){return;}
;var A=this.__lW.shift();this.__lX();this._registerEventChaining(null,A,this.length-1);this.__lY(0,this.length);this.fireDataEvent(g,{value:[],name:j,old:[A],item:this});this.fireDataEvent(d,{start:0,end:this.length-1,type:b,items:[A]},null);return A;}
...
;this.__lY(D+arguments.length-2,this.length);var K=[];for(var i=2;i<arguments.length;i++){K[i-2]=arguments[i];}
...
;this.__lY(0,this.length);this.fireDataEvent(d,{start:0,end:this.length-1,type:h,items:null},null);this.fireDataEvent(g,{value:this.__lW,name:f+(this.length-1),old:P,item:this});}
,unshift:function(Q){for(var i=arguments.length-1;i>=0;i--){this.__lW.unshift(arguments[i]);this.__lX();this.__lY(0,this.length);this.fireDataEvent(g,{value:[this.__lW[0]],name:j,old:[this.__lW[1]],item:this});this.fireDataEvent(d,{start:0,end:this.length-1,type:a,items:[arguments[i]]},null);}
;return this.length;}
...
,__lY:function(bw,bx){for(var i=bw;i<bx;i++){this._registerEventChaining(this.__lW[i],this.__lW[i],i);}
;}
},destruct:function(){for(var i=0;i<this.__lW.length;i++){var by=this.__lW[i];this._applyEventPropagation(null,by,i);if(this.isAutoDisposeItems()&&by&&by instanceof qx.core.Object){by.dispose();}
;}
;this.__lW=null;}
});}
)();
...
;this.__cS[U]=e[x+V]();X.scrollBy(W);if(this.__mc[U]){clearTimeout(this.__mc[U]);this.__mc[U]=null;}
;this.__mc[U]=setTimeout(qx.lang.Function.bind(function(ba){this.__md(ba,U);}
,this,W),100);}
;}
,__md:function(bb,bc){this.__mc[bc]=null;var be=this._isChildControlVisible(q+bc);if(bb==0||!be){return;}
;if(bb>0){bb=Math.max(0,bb-3);}
else {bb=Math.min(0,bb+3);}
;this.__mc[bc]=setTimeout(qx.lang.Function.bind(function(bf,bg){this.__md(bf,bg);}
...
,_applyOrientation:function(C,D){var E=C===o;var F=E?new qx.ui.layout.HBox():new qx.ui.layout.VBox();var content=this.__mr;content.setLayout(F);content.setAllowGrowX(!E);content.setAllowGrowY(E);this._applySpacing(this.getSpacing());}
,_applySpacing:function(G,H){this.__mr.getLayout().setSpacing(G);}
...
;if(((new Date).valueOf()-this.__Be)>1000){this.__Bd=n;}
;this.__Bd+=e.getChar();var K=this.findItemByLabelFuzzy(this.__Bd);if(K){this.setSelection([K]);}
;this.__Be=(new Date).valueOf();}
...
,__mR:function(e){var E=e.getData()[0];var D=this.getChildControl(l);if(D.getSelection()[0]!=E){if(E){D.setSelection([E]);}
else {D.resetSelection();}
;}
;this.__AL();this.__AM();}
,__AL:function(){var G=this.getChildControl("list").getSelection()[0];var H=this.getChildControl("atom");var F=G?G.getIcon():"";F==null?H.resetIcon():H.setIcon(F);}
,__AM:function(){var K=this.getChildControl("list").getSelection()[0];var L=this.getChildControl("atom");var J=K?K.getLabel():"";var I=this.getFormat();if(I!=null){J=I.call(this,K);}
...
,_onKeyPress:function(e){var R=e.getKeyIdentifier();if(R==k||R==h){if(this.__zk){this.setSelection([this.__zk]);this.__zk=null;}
;this.toggle();}
...
,_onListMouseDown:function(e){if(this.__zk){this.setSelection([this.__zk]);this.__zk=null;}
;}
,_onListChangeSelection:function(e){var T=e.getData();var W=e.getOldData();if(W&&W.length>0){W[0].removeListener(s,this.__AL,this);W[0].removeListener(t,this.__AM,this);}
;if(T.length>0){var V=this.getChildControl(n);var U=this.getChildControl(l);var X=U.getSelectionContext();if(V.isVisible()&&(X==w||X==g)){this.__zk=T[0];}
else {this.setSelection([T[0]]);this.__zk=null;}
;T[0].addListener(s,this.__AL,this);T[0].addListener(t,this.__AM,this);}
...
(function(){var j="),to(",i="from(",h="background-image",g="background",f="<div style=\"position: absolute; width: 100%; height: 100%; filter:progid:DXImageTransform.Microsoft.Gradient",e="StartColorStr='#FF",d="', ",c="-webkit-gradient(linear,",b="startColorPosition",a="qx.debug",P="deg, ",O="css.gradient.legacywebkit",N="EndColorStr='#FF",M="startColor",L="qx.theme",K="(GradientType=",J="qx.ui.decoration.MLinearBackgroundGradient",I="(",H="endColorPosition",G="';)\"></div>",q="endColor",r=", ",o="overflow",p="hidden",m="This decorator is already in-use. Modification is not possible anymore!",n="linear-gradient",k=" 0",l="px",s="0",t="shorthand",y="Color",x="vertical",A="",z="css.gradient.filter",C="Number",B="%",v=")",F="css.gradient.linear",E=",",D=" ",u="horizontal",w="_applyLinearBackgroundGradient";qx.Mixin.define(J,{properties:{startColor:{check:y,nullable:true,apply:w},endColor:{check:y,nullable:true,apply:w},orientation:{check:[u,x],init:x,apply:w},startColorPosition:{check:C,init:0,apply:w},endColorPosition:{check:C,init:100,apply:w},colorPositionUnit:{check:[l,B],init:B,apply:w},gradientStart:{group:[M,b],mode:t},gradientEnd:{group:[q,H],mode:t}},members:{_styleLinearBackgroundGradient:function(Q){var W=this.__si();var bb=W.start;var U=W.end;var bc=this.getColorPositionUnit();if(qx.core.Environment.get(O)){bc=bc===l?A:bc;if(this.getOrientation()==u){var ba=this.getStartColorPosition()+bc+k+bc;var X=this.getEndColorPosition()+bc+k+bc;}
else {var ba=s+bc+D+this.getStartColorPosition()+bc;var X=s+bc+D+this.getEndColorPosition()+bc;}
;var S=i+bb+j+U+v;var T=c+ba+E+X+E+S+v;Q[g]=T;}
else if(qx.core.Environment.get(z)&&!qx.core.Environment.get(F)){Q[o]=p;}
else {var bd=this.getOrientation()==u?0:270;var V=bb+D+this.getStartColorPosition()+bc;var R=U+D+this.getEndColorPosition()+bc;var Y=qx.core.Environment.get(F);if(Y===n){bd=this.getOrientation()==u?bd+90:bd-90;}
;Q[h]=Y+I+bd+P+V+E+R+v;}
;}
,__si:function(){if(qx.core.Environment.get(L)){var be=qx.theme.manager.Color.getInstance();var bg=be.resolve(this.getStartColor());var bf=be.resolve(this.getEndColor());}
else {var bg=this.getStartColor();var bf=this.getEndColor();}
;return {start:bg,end:bf};}
,_getContent:function(){if(qx.core.Environment.get(z)&&!qx.core.Environment.get(F)){var bi=this.__si();var bk=this.getOrientation()==u?1:0;var bj=qx.util.ColorUtil.hex3StringToHex6String(bi.start);var bh=qx.util.ColorUtil.hex3StringToHex6String(bi.end);bj=bj.substring(1,bj.length);bh=bh.substring(1,bh.length);return f+K+bk+r+e+bj+d+N+bh+G;}
;return A;}
,_resizeLinearBackgroundGradient:function(bl,bm,bn){var bo=this.getInsets();bm-=bo.left+bo.right;bn-=bo.top+bo.bottom;return {left:bo.left,top:bo.top,width:bm,height:bn};}
,_applyLinearBackgroundGradient:function(){if(qx.core.Environment.get(a)){if(this._isInitialized()){throw new Error(m);}
;}
;}
}});}
)();
