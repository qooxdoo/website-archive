...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Virtual_List","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Virtual_List-qx.theme.Classic.616b6284b47d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var n="one",m="_applySpacing",k="Integer",h="_applyDragSelection",g="String",f="resize",d="demobrowser.demo.virtual.list.List",c="list",b="multi",a="_applyQuickSelection",C="_applyItemHeight",B="_applySelectionMode",A="additive",z="changeName",y="Number",x="addItem",w="removeItem",v="changeSelection",u="row",t="single",r="changeHeight",s=",",p="Boolean",q="selected",o="qx.event.type.Data";qx.Class.define(d,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider],construct:function(){qx.ui.virtual.core.Scroller.call(this,0,1,this.getItemHeight(),10);this.__Re=new qx.ui.virtual.layer.WidgetCell(this);this.getPane().addLayer(this.__Re);this.getPane().addListener(f,this._onResize,this);this.__mQ=[];this.__cQ=[];this.__nU=new qx.ui.virtual.selection.Row(this.getPane());this.__nU.addListener(v,this._onChangeSelection,this);this.__nU.attachMouseEvents();this.__nU.attachKeyEvents(this);this.__nU.attachListEvents(this);new qx.ui.virtual.behavior.Prefetch(this,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:400,maxAbove:600,minBelow:400,maxBelow:600}).set({interval:500});}
,events:{addItem:o,removeItem:o,changeValue:o,changeSelection:o},properties:{appearance:{refine:true,init:c},focusable:{refine:true,init:true},spacing:{check:k,init:0,apply:m,themeable:true},enableInlineFind:{check:p,init:true},name:{check:g,nullable:true,event:z},itemHeight:{init:24,themeable:true,check:y,apply:C},selectionMode:{check:[t,b,A,n],init:t,apply:B},dragSelection:{check:p,init:false,apply:h},quickSelection:{check:p,init:false,apply:a}},members:{__mQ:null,__cQ:null,__nU:null,__Re:null,syncWidget:function(){this.update();}
,update:function(){var E=this.getPane().getRowConfig();E.setItemCount(this.__mQ.length);E.resetItemSizes();for(var i=0;i<this.__mQ.length;i++){var D=this.__mQ[i].getHeight();if(D!==null){E.setItemSize(i,D);}
;}
;}
,updateSelection:function(){var F=this.__Re.getChildren();for(var i=0;i<F.length;i++){var G=F[i];var H=G.getUserData(u);if(this.__nU.isItemSelected(H)){G.addState(q);}
...
,_onChangeSelection:function(e){this.updateSelection();this.fireDataEvent(v,this.__Ri(e.getData()));}
,getCellWidget:function(I,J){var L=this.__mQ[I];if(!L){return null;}
;var K=this.__cQ.pop()||new qx.ui.form.ListItem();K.set({label:L.getLabel(),icon:L.getIcon()});if(this.__nU.isItemSelected(I)){K.addState(q);}
...
,hasChildren:function(){return this.__mQ.length>0;}
...
,add:function(W){this.__mQ.push(W);this._addHelper(W);return this;}
,remove:function(X){qx.lang.Array.remove(this.__mQ,X);this._removeHelper(X);return this;}
,removeAll:function(){for(var i=0,j=this.__mQ.length;i<j;i++){this._removeHelper(this.__mQ[i]);}
;this.__mQ=[];}
,indexOf:function(Y){return this.__mQ.indexOf(Y);}
,addAt:function(ba,bb){qx.lang.Array.insertAt(this.__mQ,ba,bb);this._addHelper(ba);}
,addBefore:function(bc,bd){qx.lang.Array.insertBefore(this.__mQ,bc,bd);this._addHelper(bc);}
,addAfter:function(be,bf){qx.lang.Array.insertAfter(this.__mQ,be,bf);this._addHelper(be);}
,removeAt:function(bg){this.__mQ.splice(bg,1);this._removeHelper(this.__mQ[bg]);}
,__Rf:function(bh){var bi=this.__mQ.indexOf(bh);if(bi<0){return null;}
else {return bi;}
;}
,__Rg:function(bj){var bk=this.__mQ[bj];return bk;}
,__Rh:function(bl){var bm=[];for(var i=0;i<bl.length;i++){bm.push(this.__Rf(bl[i]));}
;return bm;}
,__Ri:function(bn){var bo=[];for(var i=0;i<bn.length;i++){bo.push(this.__mQ[bn[i]]);}
;return bo;}
,selectAll:function(){this.__nU.selectAll();}
,setSelected:function(bp){this.__nU.selectItem(this.__Rf(bp));}
,isSelected:function(bq){return this.__nU.isItemSelected(this.__Rf(bq));}
,addToSelection:function(br){this.__nU.addItem(this.__Rf(br));}
,removeFromSelection:function(bs){this.__nU.removeItem(this.__Rf(bs));}
,selectRange:function(bt,bu){this.__nU.selectItemRange(this.__Rf(bt),this.__Rf(bu));}
,resetSelection:function(){this.__nU.clearSelection();}
,setSelection:function(bv){this.__nU.replaceSelection(this.__Rh(bv));}
,getSelectedItem:function(){return this.__mQ[this.getSelected()];}
,getSelected:function(){return this.__mQ[this.__nU.getSelectedItem()];}
,getSelection:function(){return this.__Ri(this.__nU.getSelection());}
,getSortedSelection:function(){return this.__Ri(this.__nU.getSortedSelection());}
,isSelectionEmpty:function(){return this.__nU.isSelectionEmpty();}
,getSelectionContext:function(){return this.__nU.getSelectionContext();}
,getSelectables:function(){return this.__Ri(this.__nU.getSelectables());}
,_applySelectionMode:function(bw,bx){this.__nU.setMode(bw);}
,_applyDragSelection:function(by,bz){this.__nU.setDrag(by);}
,_applyQuickSelection:function(bA,bB){this.__nU.setQuick(bA);}
}});}
)();
