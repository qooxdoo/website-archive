...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Resize_Columns","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Resize_Columns-qx.theme.Simple.82aaa37aa9e0.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var v="columnVisibilityMenuCreateEnd",u="tableWidthChanged",t="ontablewidthchanged",s="verticalScrollBarChanged",r="oncolumnwidthchanged",q="onvisibilitychanged",p="qx.ui.table.columnmodel.resizebehavior.Abstract",n="qx.ui.table.columnmodel.Resize",m="_applyBehavior",l="separator",e="visibilityChanged",k="Reset column widths",h="changeBehavior",c="user-button",b="onverticalscrollbarchanged",g="onappear",f="widthChanged",i="execute",a="appear",j="qx.tableResizeDebug",d="qx.debug";qx.Class.define(n,{extend:qx.ui.table.columnmodel.Basic,include:qx.locale.MTranslation,construct:function(){qx.ui.table.columnmodel.Basic.call(this);this.__nK=false;this.__nL=false;}
,properties:{behavior:{check:p,init:null,nullable:true,apply:m,event:h}},members:{__nL:null,__nK:null,__mw:null,_applyBehavior:function(w,x){if(x!=null){x.dispose();x=null;}
;w._setNumColumns(this.getOverallColumnCount());w.setTableColumnModel(this);}
,init:function(y,z){qx.ui.table.columnmodel.Basic.prototype.init.call(this,y,z);if(this.__mw==null){this.__mw=z;z.addListener(a,this._onappear,this);z.addListener(u,this._onTableWidthChanged,this);z.addListener(s,this._onverticalscrollbarchanged,this);z.addListener(v,this._addResetColumnWidthButton,this);this.addListener(f,this._oncolumnwidthchanged,this);this.addListener(e,this._onvisibilitychanged,this);}
...
,_onappear:function(event){if(this.__nK){return;}
;this.__nK=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(g);}
;}
;this.getBehavior().onAppear(event,event.getType()!==a);this.__mw._updateScrollerWidths();this.__mw._updateScrollBarVisibility();this.__nK=false;this.__nL=true;}
,_onTableWidthChanged:function(event){if(this.__nK||!this.__nL){return;}
;this.__nK=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(t);}
;}
;this.getBehavior().onTableWidthChanged(event);this.__nK=false;}
,_onverticalscrollbarchanged:function(event){if(this.__nK||!this.__nL){return;}
;this.__nK=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(b);}
;}
;this.getBehavior().onVerticalScrollBarChanged(event);qx.event.Timer.once(function(){if(this.__mw&&!this.__mw.isDisposed()){this.__mw._updateScrollerWidths();this.__mw._updateScrollBarVisibility();}
;}
,this,0);this.__nK=false;}
,_oncolumnwidthchanged:function(event){if(this.__nK||!this.__nL){return;}
;this.__nK=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(r);}
;}
;this.getBehavior().onColumnWidthChanged(event);this.__nK=false;}
,_onvisibilitychanged:function(event){if(this.__nK||!this.__nL){return;}
;this.__nK=true;if(qx.core.Environment.get(d)){if(qx.core.Environment.get(j)){this.debug(q);}
;}
;this.getBehavior().onVisibilityChanged(event);this.__nK=false;}
...
(function(){var m="Function",k="Boolean",j="minWidth",h="width",g="qx.ui.table.columnmodel.Resize",f="qx.ui.table.columnmodel.resizebehavior.Default",e="__nO",d="Unknown property: ",c="__nP",b="maxWidth",a="Column number out of range";qx.Class.define(f,{extend:qx.ui.table.columnmodel.resizebehavior.Abstract,construct:function(){qx.ui.table.columnmodel.resizebehavior.Abstract.call(this);this.__nN=[];this.__nO=new qx.ui.layout.HBox();this.__nO.connectToWidget(this);this.__nP=new qx.util.DeferredCall(this._computeColumnsFlexWidth,this);}
,properties:{newResizeBehaviorColumnData:{check:m,init:function(n){return new qx.ui.core.ColumnData();}
},initializeWidthsOnEveryAppear:{check:k,init:false},tableColumnModel:{check:g}},members:{__nO:null,__nQ:null,__nN:null,__nP:null,__nR:false,setWidth:function(o,p,q){if(o>=this.__nN.length){throw new Error(a);}
;this.__nN[o].setColumnWidth(p,q);this.__nP.schedule();}
,setMinWidth:function(r,s){if(r>=this.__nN.length){throw new Error(a);}
;this.__nN[r].setMinWidth(s);this.__nP.schedule();}
,setMaxWidth:function(t,u){if(t>=this.__nN.length){throw new Error(a);}
;this.__nN[t].setMaxWidth(u);this.__nP.schedule();}
,set:function(v,w){for(var x in w){switch(x){case h:this.setWidth(v,w[x]);break;case j:this.setMinWidth(v,w[x]);break;case b:this.setMaxWidth(v,w[x]);break;default:throw new Error(d+x);};}
;}
,onAppear:function(event,y){if(y===true||!this.__nR||this.getInitializeWidthsOnEveryAppear()){this._computeColumnsFlexWidth();this.__nR=true;}
;}
...
,_setNumColumns:function(A){var B=this.__nN;if(A<=B.length){B.splice(A,B.length);return;}
;for(var i=B.length;i<A;i++){B[i]=this.getNewResizeBehaviorColumnData()();B[i].columnNumber=i;}
;}
,getLayoutChildren:function(){return this.__nQ;}
,_computeColumnsFlexWidth:function(){this.__nP.cancel();var G=this._getAvailableWidth();if(G===null){return;}
;var C=this.getTableColumnModel();var E=C.getVisibleColumns();var F=E.length;var D=this.__nN;var i,l;if(F===0){return;}
;var I=[];for(i=0;i<F;i++){I.push(D[E[i]]);}
;this.__nQ=I;this.__nS();this.__nO.renderLayout(G,100);for(i=0,l=I.length;i<l;i++){var H=I[i].getComputedWidth();C.setColumnWidth(E[i],H);}
;}
,__nS:function(){this.__nO.invalidateChildrenCache();var J=this.__nQ;for(var i=0,l=J.length;i<l;i++){J[i].invalidateLayoutCache();}
;}
...
;}
,_getResizeColumnData:function(){return this.__nN;}
},destruct:function(){this.__nN=this.__nQ=null;this._disposeObjects(e,c);}
});}
)();
