...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.TextField","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.TextField-qx.theme.Classic.854c8416ad22.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="text\narea\nmonospace",j="Use the following buttons to interact with the ",i="changeValue",h="Send content",g="20px sans-serif",f="ChangeValue: ",d="Send end of selection",c="demobrowser.demo.widget.TextField",b="Toggle live update",a="Web 2.0",w="Toggle enabled",v="Sending content: ",u="<b>first textfield</b> widget.",t="text\narea\nnowrap",s="max15",r="Send selection",q="monospace",p="read only",o="Send start of selection",n="text\narea",l="Sending selection: ",m="execute";qx.Class.define(c,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var L=new qx.ui.container.Composite(new qx.ui.layout.HBox());this.getRoot().add(L);var x=new qx.ui.layout.VBox();x.setSpacing(10);var M=new qx.ui.container.Composite(x);M.setPadding(20);L.add(M);var z=new qx.ui.form.TextField(s).set({maxLength:15});z.focus();z.addListener(i,function(e){this.debug(f+e.getData());}
,this);M.add(z);var B=new qx.ui.form.TextField(a).set({font:qx.bom.Font.fromString(g),padding:6});M.add(B);var A=new qx.ui.form.TextField(p).set({readOnly:true});M.add(A);var D=new qx.ui.form.TextArea(n);M.add(D);var H=new qx.ui.form.TextArea(t);H.setWrap(false);M.add(H);var K=new qx.ui.form.TextArea(k);K.setFont(q);M.add(K);var F=new qx.ui.container.Composite(new qx.ui.layout.VBox(8));F.setPadding(20);var E=new qx.ui.form.Button(b);E.addListener(m,function(){z.toggleLiveUpdate();}
);F.add(E);var I=new qx.ui.form.Button(w);var J=false;I.addListener(m,function(){M.setEnabled(J);J=!J;}
);F.add(I);F.add(new qx.ui.core.Spacer(null,20));var N=j+u;var P=new qx.ui.basic.Label(N);P.setRich(true);F.add(P);var G=new qx.ui.form.Button(h);G.addListener(m,function(){this.debug(v+z.getValue());}
);F.add(G);var C=new qx.ui.form.Button(r);C.setFocusable(false);C.setKeepFocus(true);C.addListener(m,function(){this.debug(l+z.getTextSelection());}
);F.add(C);var O=new qx.ui.form.Button(o);O.setFocusable(false);O.setKeepFocus(true);O.addListener(m,function(){this.debug(l+z.getTextSelectionStart());}
);F.add(O);var y=new qx.ui.form.Button(d);y.setFocusable(false);y.setKeepFocus(true);y.addListener(m,function(){this.debug(l+z.getTextSelectionEnd());}
);F.add(y);L.add(F);}
}});}
)();
