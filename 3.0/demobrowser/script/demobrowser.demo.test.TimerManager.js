...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.TimerManager.bdc0742c7915.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="Stopping recurrent 2-second timer",b="Recurrent 2-second timer with limit 3:",c="\n",d="__on",e="One-shot 1-second timer: ",f=" ",g="demobrowser.demo.test.TimerManager",h="Recurrent 5-second timer: ",i="Start timers",j="execute";qx.Class.define(g,{extend:qx.application.Standalone,members:{__on:null,startTimers:function(){var k=qx.util.TimerManager.getInstance();k.start(function(l,m){this.__Rh(h+m);}
,5000,this,null,0);k.start(function(n,o){this.__Rh(e+o);}
,0,this,null,1000);k.start(function(p,q){this.__Rh(b+q);if( ++p.count==3){this.__Rh(a);k.stop(q);}
;}
,2000,this,{count:0},2000);}
,__Rh:function(r){r=Date.parse(new Date)+f+r+c;this.__on.setValue(this.__on.getValue()+r);}
...
,this);var v=new qx.ui.form.TextArea();v.set({width:400,height:300});this.__on=v;t.add(v);t.add(u);this.getRoot().add(t,{left:20,top:10});}
},destruct:function(){this._disposeObjects(d);}
});}
)();
(function(){var a="qx.util.TimerManager",b="interval",c="singleton";qx.Class.define(a,{extend:qx.core.Object,type:c,statics:{__Fl:[],__Fm:{},__rP:0},members:{__Fn:false,start:function(g,f,h,d,j){if(!j){j=f||0;}
;var e=(new Date()).getTime()+j;this.self(arguments).__Fm[ ++this.self(arguments).__rP]={callback:g,userData:d||null,expireAt:e,recurTime:f,context:h||this};this.__Fo(e,this.self(arguments).__rP);return this.self(arguments).__rP;}
,stop:function(k){var l=this.self(arguments).__Fl;var length=l.length;for(var i=0;i<length;i++ ){if(l[i]==k){l.splice(i,1);break;}
;}
;delete this.self(arguments).__Fm[k];if(l.length==0&&this.__Fn){qx.event.Idle.getInstance().removeListener(b,this.__Fp,this);this.__Fn=false;}
;}
,__Fo:function(m,p){var o=this.self(arguments).__Fl;var n=this.self(arguments).__Fm;var length=o.length;for(var i=0;i<length;i++ ){if(n[o[i]].expireAt>m){o.splice(i,0,p);break;}
;}
;if(o.length==length){o.push(p);}
;if(!this.__Fn){qx.event.Idle.getInstance().addListener(b,this.__Fp,this);this.__Fn=true;}
;}
,__Fp:function(){var r=(new Date()).getTime();var t=this.self(arguments).__Fl;var s=this.self(arguments).__Fm;while(t.length>0&&s[t[0]].expireAt<=r){var v=t.shift();var q=s[v];q.callback.call(q.context,q.userData,v);if(q.recurTime&&s[v]){var u=(new Date()).getTime();q.expireAt=u+q.recurTime;this.__Fo(q.expireAt,v);}
else {delete s[v];}
;}
;if(t.length==0&&this.__Fn){qx.event.Idle.getInstance().removeListener(b,this.__Fp,this);this.__Fn=false;}
;}
}});}
)();
