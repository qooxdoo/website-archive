...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Columns.0b8ec42e0a01.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,getLookupTable:function(){return this.__xG;}
,_reverseLookup:function(bk){return bk;}
,_getDataFromRow:function(bl){return this.__xG.getItem(bl);}
,_getSelectables:function(){return this.__xG;}
,getOpenNodes:function(){return this.__yE;}
,isNode:function(bm){return qx.Class.hasProperty(bm.constructor,this.getChildProperty());}
,getLevel:function(bn){return this.__yF[bn];}
...
,_applyModel:function(bN,bM){this.__yE=[];if(bN!=null){if(qx.core.Environment.get(v)){if(!qx.Class.hasMixin(bN.constructor,qx.data.marshal.MEventBubbling)){this.warn(E+g+j);}
;}
;bN.addListener(U,this._onChangeBubble,this);this.__yK(bN);}
...
;}
;if(this.__xG.indexOf(bS)!=-1){this.__yI();}
;}
;}
,_onUpdated:function(event){if(this.__hh==null){this.__hh=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__hh.schedule();}
,_onOpen:function(event){var bW=event.getRow();var bV=this.__xG.getItem(bW);if(this.isNode(bV)){if(this.isNodeOpen(bV)){this.closeNode(bV);}
...
;break;};}
;}
,_beforeApplySelection:function(cd){if(cd.length===0&&this.getSelectionMode()===f){var cc=this.__yN();var ce=this.getLookupTable().indexOf(cc);if(ce>=0){cd.push(ce);}
;}
;}
,_afterApplySelection:function(){var cf=this.getSelection();if(cf.getLength()>0&&this.getSelectionMode()===f){this.__yM(cf.getItem(0));}
else {this.__yH=[];}
;}
,__yI:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(P+b);}
;this.__yG=0;var cj=[];this.__yF=[];var ch=-1;var ci=this.getModel();if(ci!=null){if(!this.isHideRoot()){ch++ ;cj.push(ci);this.__yF.push(ch);}
;if(this.isNodeOpen(ci)){var cg=this.__yJ(ci,ch);cj=cj.concat(cg);}
;}
;this._provider.removeBindings();this.__xG.removeAll();this.__xG.append(cj);this.__xM();this._updateSelection();}
,__yJ:function(ck,cp){var co=[];cp++ ;if(!this.isNode(ck)){return co;}
...
;if(this.isNode(cr)){this.__yF.push(cp);co.push(cr);if(this.isNodeOpen(cr)){var cl=this.__yJ(cr,cp);co=co.concat(cl);}
;}
else {if(this.isShowLeafs()){this.__yF.push(cp);co.push(cr);}
;}
;}
;cs.dispose();return co;}
,__yK:function(ct){if(!qx.lang.Array.contains(this.__yE,ct)){this.__yE.push(ct);this.fireDataEvent(t,ct);}
;}
,__yL:function(cu,cv){if(cu===cv){this.__yK(cv);return true;}
;if(!this.isNode(cu)){return false;}
;var cy=cu.get(this.getChildProperty());if(cy==null){return false;}
;for(var i=0;i<cy.getLength();i++ ){var cx=cy.getItem(i);var cw=this.__yL(cx,cv);if(cw===true){this.__yK(cx);return true;}
;}
;return false;}
,__xM:function(){this.getPane().getRowConfig().setItemCount(this.__xG.getLength());this.getPane().fullUpdate();}
,getParent:function(cA){var cB=this.__xG.indexOf(cA);if(cB<0){return null;}
;var cC=this.__yF[cB];while(cB>0){cB-- ;var cz=this.__yF[cB];if(cz<cC){return this.__xG.getItem(cB);}
;}
;return null;}
,__yM:function(cD){this.__yH=[];var parent=this.getParent(cD);while(parent!=null){this.__yH.unshift(parent);parent=this.getParent(parent);}
;}
,__yN:function(){if(this.__yH==null){return this.getModel();}
;var cE=this.getLookupTable();var parent=this.__yH.pop();while(parent!=null){if(cE.contains(parent)){return parent;}
;parent=this.__yH.pop();}
...
;this._layer.removeListener(Q,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__xG.dispose();this._layer=this._provider=this.__xG=this.__yE=this.__hh=null;}
});}
)();
...
(function(){var a="BindingIds",b="model",c="",d="changeDelegate",f="virtual-tree-file",g="label",h="appearance",i="icon",j="bindItem",k=".length",l="]",m="virtual-tree-folder",n="[",o=".",p="String",q="qx.ui.tree.core.MWidgetController";qx.Mixin.define(q,{construct:function(){this.__wR=[];}
...
;}
,bindProperty:function(x,A,y,w,u){var z=this.__wS(u,x);var v=this._tree.getLookupTable();var B=v.bind(z,w,A,y);this.__wT(w,B);}
,bindPropertyReverse:function(I,E,G,H,C){var F=this.__wS(C,I);var D=this._tree.getLookupTable();var J=H.bind(E,D,F,G);this.__wT(H,J);}
,removeBindings:function(){while(this.__wR.length>0){var K=this.__wR.pop();this._removeBindingsFrom(K);}
;}
...
;}
,_removeBindingsFrom:function(O){var P=this.__wU(O);while(P.length>0){var Q=P.pop();try{this._tree.getLookupTable().removeBinding(Q);}
catch(e){O.removeBinding(Q);}
;}
;if(qx.lang.Array.contains(this.__wR,O)){qx.lang.Array.remove(this.__wR,O);}
;}
,__wS:function(S,T){var R=n+S+l;if(T!=null&&T!=c){R+=o+T;}
;return R;}
,__wT:function(V,W){var U=this.__wU(V);if(!qx.lang.Array.contains(U,W)){U.push(W);}
;if(!qx.lang.Array.contains(this.__wR,V)){this.__wR.push(V);}
;}
,__wU:function(X){var Y=X.getUserData(a);if(Y==null){Y=[];X.setUserData(a,Y);}
;return Y;}
...
;var n=this._renderer.getCellWidget();n.setOpen(o&&this._tree.isNodeOpen(r));n.setUserData(g,o);n.addListener(a,this.__yO,this);if(this._tree.getSelection().contains(r)){this._styleSelectabled(n);}
...
,poolCellWidget:function(t){t.removeListener(a,this.__yO,this);this._removeBindingsFrom(t);this._renderer.pool(t);this._onPool(t);}
...
,__yO:function(event){var J=event.getTarget();var K=J.getUserData(k);var I=this._tree.getLookupTable().getItem(K);if(event.getData()){this._tree.openNode(I);}
