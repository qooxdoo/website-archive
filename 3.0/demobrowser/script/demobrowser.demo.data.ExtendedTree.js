...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ExtendedTree.4223b143b94d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="changeModel",b="_applyDelegate",c="qx.core.Object",d="_applyLabelPath",f="icon",g="change",h="label",k="' twice.",l="_applyTarget",m="changeTarget",n="_applyIconPath",o="Can not reverse bind the same target property '",p="_applyLabelOptions",q="_applyChildPath",r="_applyIconOptions",s="qx.data.controller.Tree",t="String",u="Can not bind the same target property '",v="_applyModel",w="get";qx.Class.define(s,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(y,z,A,x){qx.core.Object.call(this);this.__bd={};this.__Em=[];this.__Iw={};if(A!=null){this.setChildPath(A);}
;if(x!=null){this.setLabelPath(x);}
;if(y!=null){this.setModel(y);}
;if(z!=null){this.setTarget(z);}
;}
,properties:{model:{check:c,apply:v,event:a,nullable:true,dereference:true},target:{apply:l,event:m,init:null,nullable:true,dereference:true},childPath:{check:t,apply:q,nullable:true},labelPath:{check:t,apply:d,nullable:true},iconPath:{check:t,apply:n,nullable:true},labelOptions:{apply:p,nullable:true},iconOptions:{apply:r,nullable:true},delegate:{apply:b,init:null,nullable:true}},members:{__Iw:null,__bd:null,__Em:null,__Ix:null,_applyDelegate:function(C,B){this._setConfigureItem(C,B);this._setCreateItem(C,B);this._setBindItem(C,B);}
,_applyIconOptions:function(E,D){this.__Ew();}
,_applyLabelOptions:function(G,F){this.__Ew();}
,_applyTarget:function(I,H){if(H!=undefined){this.__ye(H);}
;if(this.getModel()!=null){this.__IA();}
;this._addChangeTargetListener(I,H);}
,_applyModel:function(K,J){this.__IA();}
,_applyChildPath:function(M,L){this.__Ix=L;this.__IA();this.__Ix=null;}
,_applyIconPath:function(O,N){this.__Ew();}
,_applyLabelPath:function(Q,P){this.__IA();}
,__Iy:function(R){var T=R.getTarget();var S=this.__Iw[T.toHashCode()].treeNode;var U=this.__Iw[T.toHashCode()].modelNode;this.__IB(S,U);this._updateSelection();}
,__Iz:function(e){var X=e.getData();var bc=e.getOldData();var V=this.__Iw[bc.toHashCode()];bc.removeListenerById(V.changeListenerId);delete this.__Iw[bc.toHashCode()];V.modelNode.removeListenerById(V.changeChildernListenerId);var bd=V.modelNode;var bb=qx.Class.getPropertyDefinition(V.modelNode.constructor,this.getChildPath());var Y=bb.event;var ba=bd.addListener(Y,this.__Iz,this);var W=V.treeNode;this.__Iw[X.toHashCode()]={modelNode:bd,treeNode:W,changeListenerId:V.changeListenerId,changeChildernListenerId:ba};this.__IB(W,bd);this._updateSelection();}
...
;return be;}
,__IA:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__ye();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.__wT(this.getModel(),bg);this.__IB(bg,this.getModel());this.getTarget().setRoot(bg);}
;}
,__IB:function(bn,br){if(br[w+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bj=br[w+qx.lang.String.firstUp(this.getChildPath())]();if(this.__Iw[bj.toHashCode()]==undefined){var bl=bj.addListener(g,this.__Iy,this);var bp=qx.Class.getPropertyDefinition(br.constructor,this.getChildPath());var bm=bp.event;var bq=br.addListener(bm,this.__Iz,this);this.__Iw[bj.toHashCode()]={modelNode:br,treeNode:bn,changeListenerId:bl,changeChildernListenerId:bq};}
...
;}
else {var bo=this._createItem();bo.setModel(bj.getItem(i));bn.addAt(bo,i);this.__wT(bj.getItem(i),bo);this.__IB(bo,bj.getItem(i));}
;}
;}
;for(var i=bn.getChildren().length-1;i>=bj.length;i-- ){var bk=bn.getChildren()[i];this.__ID(bk,bn);}
;}
,__ye:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__IC(bv);var bu=bv.getModel();if(bu){this.__IE(bu);}
;bv.destroy();this.__Iw={};}
;}
,__IC:function(bw){var bx=bw.getChildren()||[];for(var i=bx.length-1;i>=0;i-- ){if(bx[i].getChildren().length>0){this.__IC(bx[i]);}
;this.__ID(bx[i],bw);}
;}
,__ID:function(bz,bC){var bD=bz.getModel();var by=this.__Ix||this.getChildPath();var bA=w+qx.lang.String.firstUp(by);if(bD[bA]!=undefined){var bE=bD[bA]();var bB=this.__Iw[bE.toHashCode()];bE.removeListenerById(bB.changeListenerId);bD.removeListenerById(bB.changeChildernListenerId);this.__IC(bz);delete this.__Iw[bE.toHashCode()];}
;this.__IE(bD);bC.remove(bz);bz.destroy();}
...
;if(!qx.lang.Array.contains(this.__Em,bJ)){this.__Em.push(bJ);}
;}
...
;if(!qx.lang.Array.contains(this.__Em,bM)){this.__Em.push(bM);}
;}
...
;}
,__Ew:function(){var bW;for(var ca in this.__bd){bW=ca;break;}
;for(var bX in this.__bd[bW]){var bV=this.__bd[bW][bX].treeNode;var bY=qx.core.ObjectRegistry.fromHashCode(bX);this.__IE(bY);this.__wT(bY,bV);}
;}
,__wT:function(cd,cb){var cc=this.getDelegate();if(cc!=null&&cc.bindItem!=null){cc.bindItem(this,cb,cd);}
else {this.bindDefaultProperties(cb,cd);}
;}
,__IE:function(cg){for(var i=0;i<this.__Em.length;i++ ){var ce=this.__Em[i];var cf=this.__bd[ce][cg.toHashCode()];if(cf!=null){if(cf.id){cg.removeBinding(cf.id);cf.id=null;}
...
;this._startSelectionModification();this.__ye();this.__IA();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cn,cm){if(cn!=null&&cn.bindItem!=null){if(cm!=null&&cm.bindItem!=null&&cn.bindItem==cm.bindItem){return;}
;this.__IA();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__bd=this.__Iw=this.__Em=null;}
});}
)();
...
(function(){var a="changeNames",b="changeName",c="Validation Error: ",d="- ",e="",f="changeChild",g="qx.data.Array",h="changeColor",j="Marge",k="AFFE",l="is no number (parseFloat says so!).",m="Homer",n="\n",o="demobrowser.demo.data.model.Node",p="__Qr",q="String",r="changeName2",s="changeChildren";qx.Class.define(o,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.setChildren(new qx.data.Array());}
,properties:{children:{check:g,event:s},child:{check:o,event:f,nullable:true},names:{check:g,event:a,init:new qx.data.Array(m,j)},name:{check:q,init:k,event:b},name2:{check:q,event:r},number:{init:10,validate:p},color:{event:h,nullable:true}},members:{__Qr:function(t){if(!isNaN(parseFloat(t))){if(/^\d+$/.test(t)){return;}
;}
