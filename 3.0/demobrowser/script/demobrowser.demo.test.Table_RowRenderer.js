...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Table_RowRenderer.bf212a9f60b9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a='>',b="",c="px",d=";position:relative;height:",e='" ',f="table",g=' (init)</div>',h='class="',i=" (update)",j="relative",k="div",l='style="',m="px;",n="px; width:",o=": ",p="demobrowser.demo.test.Table_RowRenderer",q=", ",r='<div ';qx.Class.define(p,{extend:qx.application.Native,include:[demobrowser.demo.table.MUtil],members:{main:function(){qx.application.Native.prototype.main.call(this);qx.theme.manager.Meta.getInstance().initialize();this.setUp();this.testDefaultRenderer();}
,setUp:function(){this.tableMock=this.getTableMock();}
,testDefaultRenderer:function(w){var w=new qx.ui.table.rowrenderer.Default();var v=300;var u=20;var t={focusedRow:[true,false],selected:[true,false],row:[1,2],table:[this.tableMock]};var s=this._getNewTableDiv(v);var self=this;qx.util.Permutation.permute(t,function(A){var content=[];for(var B in A){if(B!==f){content.push(B+o+A[B]);}
;}
;content=content.join(q);var C=self._createRowElement(s,v,u);var x=w.getRowClass(A);if(x){C.className=x;}
;C.innerHTML=content+i;w.updateDataRowElement(A,C);var z=[];z.push(r);if(x){z.push(h,x,e);}
;var y=w.createRowStyle(A);y+=d+u+n+v+m;if(y){z.push(l,y,e);}
;z.push(a,content,g);s.innerHTML+=z.join(b);self._rowTop+=u;}
);}
,_rowTop:0,_createRowElement:function(parent,D,F){var E=qx.dom.Element.create(k);qx.bom.element.Style.setStyles(E,{position:j,width:D+c,height:F+c});this._rowTop+=20;parent.appendChild(E);return E;}
