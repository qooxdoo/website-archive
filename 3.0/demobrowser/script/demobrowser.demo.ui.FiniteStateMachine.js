...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.38093452bca8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b=" => ",c="#autoActionsBeforeOnexit",d=": Could not find friendly name for '",f=": event '",g=" blocked.  Re-queuing.",h="string",j="'",k="Attempt to transition to nonexistent state ",l="Saved-state stack is empty",m="Internal error: invalid nextState",n=" not handled.  Ignoring.",o="State ",p="#autoActionsBeforeOntransition",q=" returned a value other than ",r="#",s=" already exists",u="Attempt to transition to POP_STATE_STACK ",v=":",w="Transition ",x=" does not exist",y="' on '",z="qx.util.fsm.FiniteStateMachine",A="String",B="#autoActionsAfterOntransition",C="",D="__Gu",E="#actionsAfterOnentry",F="Saved-state stack is full",G="#autoActionsBeforeOnentry",H=": no transition found.  No state change.",I="#autoActionsAfterOnentry",J="#actionsBeforeOnentry",K=": Pushed event: ",L="Machine started with no available states",M=": Process event: ",N="#entry",O="#ontransition",P=" on ",Q="__Gv",R="Invalid state: not an instance of ",S="Number",T="#exit",U="true, false, or null.",V="Explicit transition ",W="  ",X="qx.util.fsm.State",Y="while state stack is empty.",bj=".  Ignoring.",bk="#autoActionsAfterOnexit",bl=": Event '",bg="object",bh=" not handled for target ",bi=": Queued event: ";qx.Class.define(z,{extend:qx.core.Object,construct:function(bm){qx.core.Object.call(this);this.setName(bm);this.__kk={};this.__Gs=null;this.__Gt=[];this.__Gu=[];this.__Gv=[];this.__Gw={};this.__Gx={};this.__Gy={};this.__Gz={};this.__GA={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:A,nullable:true},state:{check:A,nullable:true},previousState:{check:A,nullable:true},nextState:{check:A,nullable:true},maxSavedStates:{check:S,init:2},debugFlags:{check:S,init:7}},members:{__kk:null,__Gs:null,__Gu:null,__Gv:null,__Gt:null,__Gw:null,__Gx:null,__Gy:null,__Gz:null,__GA:null,__GB:false,addState:function(bn){if(!bn instanceof qx.util.fsm.State){throw new Error(R+X);}
;var bo=bn.getName();if(bo in this.__kk){throw new Error(o+bo+s);}
;if(this.__Gs==null){this.__Gs=bo;}
;this.__kk[bo]=bn;}
,replaceState:function(bs,br){if(!bs instanceof qx.util.fsm.State){throw new Error(R+X);}
;var bq=bs.getName();var bp=this.__kk[bq];this.__kk[bq]=bs;if(br){bp._bNeedDispose=true;}
;return bp;}
,addObject:function(bw,bt,bu){var bx=bt.toHashCode();this.__Gx[bw]=bx;this.__Gy[bx]=bw;this.__Gw[bw]=bt;if(!bu){return;}
;if(typeof (bu)==h){bu=[bu];}
;for(var i=0;i<bu.length;i++ ){var bv=bu[i];if(!this.__Gz[bv]){this.__Gz[bv]={};}
;this.__Gz[bv][bw]=true;if(!this.__GA[bw]){this.__GA[bw]=[];}
;this.__GA[bw].push(bv);}
;}
,removeObject:function(bB){var by;var bz;var bA;var bC;by=this.__Gx[bB];if(this.__GA[bB]){for(var i=0;i<this.__GA[bB].length;i++ ){bz=this.__GA[bB][i];delete this.__Gz[bz][bB];bC=true;for(bA in this.__Gz[bz]){bC=false;break;}
;if(bC){delete this.__Gz[bz];}
;}
;delete this.__GA[bB];}
;delete this.__Gy[by];delete this.__Gx[bB];delete this.__Gw[bB];}
,getObject:function(bD){return this.__Gw[bD];}
,getFriendlyName:function(bE){var bF=bE?bE.toHashCode():null;return bF?this.__Gy[bF]:null;}
,getGroupObjects:function(bG){var a=[];for(var name in this.__Gz[bG]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bH in this.__Gx){var bJ=this.__Gx[bH];var bI=this.getObject(bH);this.debug(bH+b+bJ);this.debug(W+bJ+b+this.__Gy[bJ]);this.debug(W+bH+b+this.getObject(bH));this.debug(W+this.getObject(bH)+b+this.getFriendlyName(bI));}
;}
,_getInternalData:function(){return ({"states":this.__kk,"startState":this.__Gs,"eventQueue":this.__Gu,"blockedEvents":this.__Gv,"savedStates":this.__Gt,"friendlyToObject":this.__Gw,"friendlyToHash":this.__Gx,"hashToFriendly":this.__Gy,"groupToFriendly":this.__Gz,"friendlyToGroups":this.__GA});}
,start:function(){var bL=this.__Gs;if(bL==null){throw new Error(L);}
;this.setState(bL);this.setPreviousState(null);this.setNextState(null);var bK=(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL);if(bK){this.debug(this.getName()+r+bL+J);}
;this.__kk[bL].getAutoActionsBeforeOnentry()(this);if(bK){this.debug(this.getName()+r+bL+N);}
;this.__kk[bL].getOnentry()(this,null);if(bK){this.debug(this.getName()+r+bL+E);}
;this.__kk[bL].getAutoActionsAfterOnentry()(this);}
,pushState:function(bM){if(this.__Gt.length>=this.getMaxSavedStates()){throw new Error(F);}
;if(bM===true){this.__Gt.push(this.getState());}
else if(bM){this.__Gt.push(bM);}
else {this.__Gt.push(this.getPreviousState());}
;}
,popState:function(){if(this.__Gt.length==0){throw new Error(l);}
;return this.__Gt.pop();}
,postponeEvent:function(event){this.__Gv.unshift(event);}
,enqueueEvent:function(event,bN){if(bN){this.__Gu.push(event);}
else {this.__Gu.unshift(event);}
;if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){var bO=this.getFriendlyName(event.getTarget());if(bN){this.debug(this.getName()+K+event.getType()+(bO?P+bO:C));}
else {this.debug(this.getName()+bi+event.getType()+(bO?P+bO:C));}
;}
;}
,eventListener:function(event){var e=event.clone();this.enqueueEvent(e,false);this.__GC();}
...
,this,bV||20);}
,__GC:function(){if(this.__GB){return;}
;this.__GB=true;while(this.__Gu.length>0){var event=this.__Gu.pop();var bW=this.__GD(event);if(bW){event.dispose();}
;}
;this.__GB=false;}
,__GD:function(event){var ca;var cb;var bY;var cd;var cg;var e;var cf;var bX=this.getDebugFlags();var ck=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var ci=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var ch=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var ce=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var cj=this.getFriendlyName(event.getTarget());if(ck){this.debug(this.getName()+M+event.getType()+(cj?P+cj:C));}
;ca=this.getState();cd=this.__kk[ca];cg=cd.transitions;e=cd.getEvents()[event.getType()];if(!e){if(ck){this.debug(this.getName()+bl+event.getType()+j+n);}
;return true;}
;if(typeof (e)==bg){if(!cj){if(ce){this.debug(this.getName()+d+event.getType()+y+event.getTarget()+j);}
;return true;}
;cf=e[cj];if(!cf){if(ck){this.debug(this.getName()+bl+event.getType()+j+bh+cj+bj);}
;return true;}
;}
else {cf=e;}
;switch(cf){case qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE:break;case qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED:if(ck){this.debug(this.getName()+bl+event.getType()+j+g);}
;this.__Gv.unshift(event);return false;default:if(typeof (cf)==h){if(cg[cf]){var cc=cg[cf];cg={};cg[cf]=cc;}
else {throw new Error(V+cf+x);}
;break;}
;};for(var t in cg){var cc=cg[t];switch(cc.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(w+ca+v+t+q+U);};cb=cc.getNextState();if(typeof (cb)==h){if(!cb in this.__kk){throw new Error(k+cb);}
;this.setNextState(cb);}
else {switch(cb){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cb=ca;this.setNextState(cb);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__Gt.length==0){throw new Error(u+Y);}
;cb=this.__Gt.pop();this.setNextState(cb);break;default:throw new Error(m);break;};}
;if(ch){this.debug(this.getName()+r+ca+r+t+p);}
;cc.getAutoActionsBeforeOntransition()(this);if(ch){this.debug(this.getName()+r+ca+r+t+O);}
;cc.getOntransition()(this,event);if(ch){this.debug(this.getName()+r+ca+r+t+B);}
;cc.getAutoActionsAfterOntransition()(this);if(ch){this.debug(this.getName()+r+ca+c);}
;cd.getAutoActionsBeforeOnexit()(this);if(ch){this.debug(this.getName()+r+ca+T);}
;cd.getOnexit()(this,event);if(ch){this.debug(this.getName()+r+ca+bk);}
;cd.getAutoActionsAfterOnexit()(this);if(cd._bNeedDispose){cd.dispose();}
;cd=this.__kk[this.getNextState()];this.setPreviousState(ca);this.setState(this.getNextState());this.setNextState(null);bY=ca;ca=cb;cb=undefined;if(ch){this.debug(this.getName()+r+ca+G);}
;cd.getAutoActionsBeforeOnentry()(this);if(ch){this.debug(this.getName()+r+ca+N);}
;cd.getOnentry()(this,event);if(ch){this.debug(this.getName()+r+ca+I);}
;cd.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__Gv.length;i++ ){e=this.__Gv.pop();this.__Gu.unshift(e);}
;if(ci){this.debug(this.getName()+r+bY+b+this.getName()+r+ca);}
;return true;}
;if(ci){this.debug(this.getName()+r+ca+f+event.getType()+j+H);}
;return true;}
},destruct:function(){this._disposeArray(D);this._disposeArray(Q);this.__Gt=this.__kk=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="__Gp",k="parameters",l="__Gk",m=")",n="__Gn",o=" fsm.getObject('",p="__Go",q="groups",r="Invalid function parameters: ",s="Invalid onexit type: ",t="  for (var i = 0; i < groupObjects.length; i++)",u="context",v="string",w="autoActionsAfterOnentry",x="Invalid numeric value in events object: ",y='"',z="Invalid state name",A=" value: ",B="');",C="State ",D="autoActionsBeforeOnexit",E="autoActionsBeforeOnentry",F="}",G="): ",H="events must be an object",I="onexit",J="{",K="Invalid function request type: ",L=": ",M="expected array, found ",N="Invalid friendly name in 'objects' list: ",O="autoActionsAfterOnexit",P="  fsm.debug(ex);",Q="(",R="__Gq",S="Invalid onentry type: ",T="    fsm.getObject(objName).",U=")(fsm)",V="events",W="undefined",X="Invalid ",Y="__Gj",ba="try",bb="__Gl",bc="    var objName = groupObjects[i];",bd="Invalid value in events object: ",be="').",bf="objects",bg="expected object, found ",bh="qx.util.fsm.Transition",bi="The events object must be provided in new state info",bj="catch(ex)",bk="  {",bl="__Gm",bm="    fsm.getGroupObjects('",bn="Invalid value in events object ",bo="Invalid transition: not an instance of ",bp="  }",bq="Invalid 'objects' list: expected array, got ",br="Invalid numeric value in events object ",bs="qx.util.fsm.State",bt="onentry",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(bs,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(u,bE);for(var bC in bF){switch(bC){case bt:this.setOnentry(this.__Gr(bF[bC],bE));break;case I:this.setOnexit(this.__Gr(bF[bC],bE));break;case E:this.setAutoActionsBeforeOnentry(bF[bC]);break;case w:this.setAutoActionsAfterOnentry(bF[bC]);break;case D:this.setAutoActionsBeforeOnexit(bF[bC]);break;case O:this.setAutoActionsAfterOnexit(bF[bC]);break;case V:this.setEvents(bF[bC]);break;case u:break;default:this.setUserData(bC,bF[bC]);this.debug(C+bD+L+bz+bC);break;};}
...
;var funcFragment;var func=ba+J;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(K+M+typeof (functionRequest));}
...
;var params=objectAndGroupList[k];if(!params){params=[];}
else {if(!params instanceof Array){throw new Error(r+M+typeof (params));}
;}
;funcFragment=f+Q;for(var j=0;j<params.length;j++ ){if(j!=0){funcFragment+=by;}
;if(typeof (params[j])==c){funcFragment+=Q+params[j]+U;}
else if(typeof (params[j])==v){funcFragment+=y+params[j]+y;}
else {funcFragment+=params[j];}
;}
;funcFragment+=m;var a=objectAndGroupList[bf];if(!a){a=[];}
else if(!a instanceof Array){throw new Error(bq+typeof (a));}
;for(var j=0;j<a.length;j++ ){if(typeof (a[j])!=v){throw new Error(N+a[j]);}
;func+=o+a[j]+be+funcFragment+bu;}
;var g=objectAndGroupList[q];if(g){if(!g instanceof Array){throw new Error(bw+typeof (g));}
;for(j=0;j<g.length;j++ ){func+=d+bm+g[j]+B+t+bk+bc+T+funcFragment+bu+bp;}
;}
;}
;}
;func+=F+bj+J+P+F;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:Y,nullable:true},onentry:{transform:l,nullable:true,init:function(bG,event){}
},onexit:{transform:bb,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:n,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:p,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:h,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:R,nullable:true,init:function(bL,event){}
},events:{transform:bl,nullable:true}},members:{__Gj:function(bM){if(typeof (bM)!=v||bM.length<1){throw new Error(z);}
;return bM;}
,__Gk:function(bN){switch(typeof (bN)){case W:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(u));default:throw new Error(S+typeof (bN));};}
,__Gl:function(bP){switch(typeof (bP)){case W:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(u));default:throw new Error(s+typeof (bP));return null;};}
,__Gm:function(bT){if(typeof (bT)!=bB){throw new Error(H);}
;for(var e in bT){var bR=bT[e];if(typeof (bR)==bA&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(x+e+L+bR);}
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==bA&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(br+Q+e+G+bS+L+bR[bS]);}
else if(typeof (bR[bS])!=v&&typeof (bR[bS])!=bA){throw new Error(bn+Q+e+G+bS+L+bR[bS]);}
;}
;}
else if(typeof (bR)!=v&&typeof (bR)!=bA){throw new Error(bd+e+L+bT[e]);}
;}
;return bT;}
,__Gn:function(bU){return qx.util.fsm.State._commonTransformAutoActions(E,bU,this.getUserData(u));}
,__Go:function(bV){return qx.util.fsm.State._commonTransformAutoActions(w,bV,this.getUserData(u));}
,__Gp:function(bW){return qx.util.fsm.State._commonTransformAutoActions(D,bW,this.getUserData(u));}
,__Gq:function(bX){return qx.util.fsm.State._commonTransformAutoActions(O,bX,this.getUserData(u));}
,__Gr:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bo+bh);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="autoActionsAfterOntransition",d="or one of the Fsm.StateChange constants",e="__GE",g="context",h="string",i="__GF",j="predicate",k="__GH",l="nextState must be an explicit state name, ",m="Transition ",n=": ",o="String",p="number",q="Invalid transition nextState type: ",r="Invalid transition nextState value: ",s="Adding user-provided field to transition: ",t="ontransition",u="Invalid transition predicate type: ",v="boolean",w="nextState",x="__GG",y="__GI",z="Invalid ontransition type: ",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(g,E);for(var C in D){switch(C){case j:this.setPredicate(this.__Gr(D[C],E));break;case w:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__Gr(D[C],E));break;case c:this.setAutoActionsAfterOntransition(this.__Gr(D[C],E));break;case t:this.setOntransition(this.__Gr(D[C],E));break;case g:break;default:this.setUserData(C,D[C]);this.debug(m+F+n+s+C);break;};}
;}
,properties:{name:{check:o,nullable:true},predicate:{init:function(G,event){return true;}
,transform:e},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:i},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:x},autoActionsAfterOntransition:{init:function(I,event){}
,transform:k},ontransition:{init:function(J,event){}
,transform:y}},members:{__GE:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
;case v:return function(M,event){return K;}
;case a:return qx.lang.Function.bind(K,this.getUserData(g));default:throw new Error(u+typeof (K));break;};}
,__GF:function(N){switch(typeof (N)){case h:return N;case p:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(r+N+n+l+d);};break;default:throw new Error(q+typeof (N));break;};}
,__GG:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(g));}
,__GH:function(P){return qx.util.fsm.State._commonTransformAutoActions(c,P,this.getUserData(g));}
,__GI:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(g));default:throw new Error(z+typeof (Q));break;};}
,__Gr:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
