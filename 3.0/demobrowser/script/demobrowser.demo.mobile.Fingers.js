...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.mobile.Fingers.8677b5483155.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="touchmove",b="black",c="px",d="green",f="div",g="100%",h="yellow",j="30px",k="qx.debug",l="0px",m="12px",n="100px",o="demobrowser.demo.mobile.Fingers",p='Lucida Grande',q="50px",r="innerHTML",s="touchstart",t="<b>This demo is supposed to be run in a WebKit-based browser.</b>",u="engine.name",v="20px",w="red",x="webkit",y="white",z="<b>Use your fingers to move the dots</b>",A="blue",B="absolute";qx.Class.define(o,{extend:qx.application.Native,members:{__QA:null,__QB:null,main:function(){qx.application.Native.prototype.main.call(this);if(qx.core.Environment.get(k)){qx.log.appender.Native;qx.log.appender.Console;}
;var E={"width":g,"height":g,"backgroundColor":b,"margin":l};var J=new qx.html.Element(f,E);J.useElement(document.body);J.setRoot(true);if(qx.core.Environment.get(u)!=x){var H={"color":d,"position":B,"font-family":p,"font-size":m,"left":j,"top":v};var F=new qx.html.Element(f,H);J.add(F);F.setAttribute(r,t);return;}
;var D={"color":y,"position":B,"left":j,"top":v};var F=new qx.html.Element(f,D);J.add(F);F.setAttribute(r,z);var G=[A,w,d,y,h];for(var i=0;i<G.length;i++ ){var C={"backgroundColor":G[i],"width":n,"height":n,"position":B,"-moz-border-radius":q,"-webkit-border-radius":q,"border-radius":q,"top":((i+5)*30)+c,"left":((i+1)*150)+c};var I=new qx.html.Element(f,C);J.add(I);}
;this.__QA=[];this.__QB=[];J.addListener(s,this._onTouchStart,this);J.addListener(a,this._onTouchMove,this);}
,_onTouchStart:function(e){var L=e.getAllTouches();for(var i=0;i<L.length;i++ ){var K=L[i];this.__QA[i]=parseInt(K.target.style.left)-K.pageX;this.__QB[i]=parseInt(K.target.style.top)-K.pageY;}
;}
,_onTouchMove:function(e){var N=e.getAllTouches();for(var i=0;i<N.length;i++ ){var M=N[i];if(N.target==document.body){continue;}
;qx.bom.element.Style.setStyles(M.target,{"left":(M.pageX+this.__QA[i])+c,"top":(M.pageY+this.__QB[i])+c});}
;e.stop();}
}});}
)();
