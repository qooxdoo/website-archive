...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Github.18d590f3d44f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
},members:{_resource:null,__GO:null,__GP:null,__GQ:null,_getResource:function(v){return new qx.bom.rest.Resource(v);}
...
,map:function(G,I,H,J){this.__GR(G,I,H,J);this._resource.map(G,I,H,J);}
,__GR:function(L,N,K,M){this.__GQ[L]=[N,K,M];if(L==m){this[L]=undefined;}
;if(typeof this[L]!==q&&this[L]!==null&&this[L].action!==true){throw new Error(o+L+c);}
;this.__GT(L+p);this.__GT(L+f);this[L]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,L);return this.invoke.apply(this,arguments);}
,this);this[L].action=true;}
,invoke:function(O,P,Q){var P=(P==null)?{}:P;this.__GQ[O].params=P;return this._resource.invoke(O,P,Q);}
...
,poll:function(Y,U,X,ba){if(this.__GP[Y]){this.__GP[Y].dispose();}
;if(typeof X==q){X=this.__GQ[Y].params;}
...
;}
;var V=this.__GP[Y]=new qx.event.Timer(U);V.addListener(i,W,this._resource);V.start();return V;}
...
;if(!bj){bi=0;}
;return false;}
;var bf=this.__GO[bh]=this.addListener(bh+p,function bk(){if(be.isDisposed()){return;}
...
,__GS:function(bn){Object.keys(bn).forEach(function(bp){var bq=bn[bp],bs=bq.method,bo=bq.url,br=bq.check;if(qx.core.Environment.get(s)){qx.core.Assert.assertString(bs,a+bp+b);qx.core.Assert.assertString(bo,n+bp+b);}
;this.map(bp,bs,bo,br);}
,this);}
,__GT:function(bt){if(!this.constructor.$$events){this.constructor.$$events={};}
;if(!this.constructor.$$events[bt]){this.constructor.$$events[bt]=j;}
;}
},destruct:function(){var bu;if(this.__GP){for(bu in this.__GP){var bw=this.__GP[bu];bw.stop();bw.dispose();}
;}
;if(this.__GO){for(bu in this.__GO){var bv=this.__GO[bu];this.removeListenerById(bv);}
;}
;this._resource.destruct();this._resource=this.__GQ=this.__GP=this.__GO=null;}
});}
)();
...
;return S;}
},members:{__GU:null,__GQ:null,__GV:null,__GP:null,__GO:null,__GW:null,__GX:null,__GY:null,setRequestFactory:function(V){this.__GY=V;}
,setRequestHandler:function(W){this.__GX=W;}
,_getRequestHandler:function(){return (this.__GX===null)?{onsuccess:{callback:function(Y,X){return function(){var ba={"id":parseInt(Y.toHashCode(),10),"response":Y.getResponse(),"request":Y,"action":X};this.emit(X+G,ba);this.emit(w,ba);}
;}
...
;}
,context:this}}:this.__GX;}
,getRequestsByAction:function(bg){var bh=(this.__GU!==null&&bg in this.__GU);return bh?this.__GU[bg]:null;}
,configureRequest:function(bi){this.__GW=bi;}
,_getRequest:function(){return (this.__GY===null)?new qx.bom.request.SimpleXhr():this.__GY();}
,__Ha:function(bj){var bk=this._getRequest();if(!qx.lang.Type.isArray(this.__GU[bj])){this.__GU[bj]=[];}
;this.__GU[bj].push(bk);return bk;}
,map:function(bm,bo,bl,bn){this.__GQ[bm]=[bo,bl,bn];this.__GU[bm]=[];if(bm==u){this[bm]=undefined;}
;if(typeof this[bm]!==N&&this[bm]!==null&&this[bm].action!==true){throw new Error(m+bm+y);}
;this.__GT(bm+G);this.__GT(bm+z);this[bm]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,bm);return this.invoke.apply(this,arguments);}
,this);this[bm].action=true;}
,invoke:function(bt,br,bp){var bq=this.__Ha(bt),br=br==null?{}:br,bu=this._getRequestConfig(bt,br);this.__GQ[bt].params=br;this.__Hb(br,bu.check);this.__zn(bq,bu,bp);if(this.__GW){this.__GW.call(this,bq,bt,br,bp);}
;this.__Hc(bq,bu,bp);var bs=this._getRequestHandler();bq.addListenerOnce(w,bs.onsuccess.callback(bq,bt),bs.onsuccess.context);bq.addListenerOnce(s,bs.onfail.callback(bq,bt),bs.onfail.context);bq.addListenerOnce(g,bs.onloadend.callback(bq,bt),bs.onloadend.context);bq.send();return parseInt(bq.toHashCode(),10);}
,setBaseUrl:function(bv){this.__GV=bv;}
,__Hb:function(bw,bx){if(typeof bx!==N){if(qx.core.Environment.get(r)){qx.core.Assert.assertObject(bx,M);}
...
;}
,__zn:function(bA,bz,bB){bA.setUrl(bz.url);if(!bA.setMethod&&bz.method!==j){throw new Error(D+bA.classname+x);}
;if(bA.setMethod){bA.setMethod(bz.method);}
;if(bB){bA.setRequestData(bB);}
;}
,__Hc:function(bE,bC,bD){if(bD){var bF=bE.getRequestHeader(K);if(bE.getMethod&&qx.util.Request.methodAllowsRequestBody(bE.getMethod())){if((/application\/.*\+?json/).test(bF)){bD=qx.lang.Json.stringify(bD);bE.setRequestData(bD);}
;}
;}
;}
...
;}
else {var bJ=bI;var bH=this.__GU[bJ];if(this.__GU[bJ]){bH.forEach(function(bM){bM.abort();}
);}
;}
;}
,refresh:function(bN){this.invoke(bN,this.__GQ[bN].params);}
,poll:function(bO,bQ,bR,bP){if(this.__GP[bO]){this.stopPollByAction(bO);}
;if(typeof bR==N){bR=this.__GQ[bO].params;}
;if(bP){this.invoke(bO,bR);}
;var bS=(function(bT){return function(){var bU=bT.__GU[bO][0];if(!bP&&!bU){bT.invoke(bO,bR);return;}
;if(bU.isDone()||bU.isDisposed()){bT.refresh(bO);}
;}
;}
)(this);this._startPoll(bO,bS,bQ);}
,_startPoll:function(bV,bW,bX){this.__GP[bV]={"id":window.setInterval(bW,bX),"interval":bX,"listener":bW};}
,stopPollByAction:function(bY){if(bY in this.__GP){var ca=this.__GP[bY].id;window.clearInterval(ca);}
;}
,restartPollByAction:function(cb){if(cb in this.__GP){var cc=this.__GP[cb];this.stopPollByAction(cb);this._startPoll(cb,cc.listener,cc.interval);}
;}
...
;if(!cj){ci=0;}
;return false;}
;var cf=this.__GO[ch]=this.addListener(ch+G,function ck(){if(ce.isDisposed()){return;}
;if(!cd()){cg=new Date();ce.refresh(ch);}
;}
);this.invoke(ch);return cf;}
,_getRequestConfig:function(cp,co){var cm=this.__GQ[cp];var co=qx.lang.Object.clone(co);if(!qx.lang.Type.isArray(cm)){throw new Error(P+cp);}
;var cn=cm[0],cl=this.__GV!==null?this.__GV+cm[1]:cm[1],cr=cm[2],cq=qx.bom.rest.Resource.placeholdersFromUrl(cl);co=co||{};cq.forEach(function(cu){var cs=new RegExp(v+cu+c),ct=cl.match(cs)[1];if(typeof co[cu]===N){if(ct){co[cu]=ct;}
else {co[cu]=A;}
;}
...
,__GS:function(cv){Object.keys(cv).forEach(function(cx){var cy=cv[cx],cA=cy.method,cw=cy.url,cz=cy.check;if(qx.core.Environment.get(r)){qx.core.Assert.assertString(cA,I+cx+o);qx.core.Assert.assertString(cw,l+cx+o);}
;this.map(cx,cA,cw,cz);}
,this);}
,__GT:function(cB){if(!this.constructor.$$events){this.constructor.$$events={};}
...
;}
;}
,destruct:function(){var cG;for(cG in this.__GU){if(this.__GU[cG]){this.__GU[cG].forEach(function(cI){cI.dispose();}
);}
;}
;if(this.__GP){for(cG in this.__GP){this.stopPollByAction(cG);}
;}
;if(this.__GO){for(cG in this.__GO){var cH=this.__GO[cG];this.removeListenerById(cH);}
;}
;this.__GU=this.__GQ=this.__GP=null;}
}});}
)();
...
;this.useCaching(true);this.setMethod((v!==undefined)?v:m);this._transport=this._registerTransportListener(this._createTransport());qx.core.ObjectRegistry.register(this);this.__rv={};this.__sd=this._createResponseParser();}
,members:{setRequestHeader:function(w,x){this.__rv[w]=x;return this;}
,getRequestHeader:function(y){return this.__rv[y];}
,setUrl:function(z){if(qx.lang.Type.isString(z)){this.__oe=z;}
;return this;}
,getUrl:function(){return this.__oe;}
,setMethod:function(A){if(qx.util.Request.isMethod(A)){this.__Hd=A;}
;return this;}
,getMethod:function(){return this.__Hd;}
,setRequestData:function(B){if(qx.lang.Type.isString(B)||qx.lang.Type.isObject(B)){this.__He=B;}
;return this;}
,getRequestData:function(){return this.__He;}
,getResponse:function(){if(this.__rB!==null){return this.__rB;}
...
,setParser:function(C){return this.__sd.setParser(C);}
,setTimeout:function(D){if(qx.lang.Type.isNumber(D)){this.__gy=D;}
;return this;}
,getTimeout:function(){return this.__gy;}
...
,send:function(){var K=this.getTimeout(),H=(this.getRequestData()!==null),I=this.__rv.hasOwnProperty(j),F=qx.util.Request.methodAllowsRequestBody(this.getMethod()),L=this.getRequestHeader(k),G=this._serializeData(this.getRequestData(),L);if(this.getMethod()===m&&H){this.setUrl(qx.util.Uri.appendParamsToUrl(this.getUrl(),G));}
...
;this._transport.open(this.getMethod(),this.getUrl(),true);for(var J in this.__rv){this._transport.setRequestHeader(J,this.__rv[J]);}
...
,dispose:function(){if(this._transport.dispose()){this.__sd=null;this.__of=true;return true;}
;return false;}
...
,_setResponse:function(N){this.__rB=N;}
,_serializeData:function(S,R){var O=this.getMethod()===q,Q=(/application\/.*\+?json/).test(R),P=d;if(!S){return null;}
;if(qx.lang.Type.isString(S)){return S;}
;if(qx.lang.Type.isObject(S)){P=(Q)?qx.lang.Json.stringify(S):qx.util.Uri.toParameter(S,O);return P;}
;}
,__rv:null,__He:null,__Hd:d,__oe:d,__rB:null,__sd:null,__d:null,__gy:null,__of:null,addListenerOnce:function(name,T,U){this._transport._emitter.once(name,T,U);return this;}
...
;}
,__rF:function(){if(qx.core.Environment.get(n)){qx.Bootstrap.debug(f+this._transport.status);}
;var W=this._transport.responseText;var V=this._transport.getResponseHeader(k);if(qx.util.Request.isSuccessful(this._transport.status)){if(qx.core.Environment.get(n)){qx.Bootstrap.debug(t+this.getResponseContentType()+a);}
;this._setResponse(this.__sd.parse(W,V));this._transport._emit(o);}
else {try{this._setResponse(this.__sd.parse(W,V));}
catch(e){}
...
(function(){var a="changeModel",b="model.user.avatar_url",c="application/json",d="github.view.Gist",e="model.description",f="west",g="GET",h="/gists/{id}",i="Description",j="https://api.github.com",k="demobrowser.demo.data.Github",l="source",m="/gists",n="change",o="Content",p="center",q="value",r="get",s="#fff",t="Authorization",u="html",v="",w="model.user.login",x="model.files",y="north",z="Gists",A="Accept",B="token ",C="bold",D="model",E="github.rest.Resource",F="description",G="undefined",H="</pre>",I="<pre>";qx.Class.define(k,{extend:qx.application.Standalone,members:{__OD:null,__OE:null,__OF:null,__OG:null,__JB:null,__OH:null,main:function(){if(typeof GITHUB==G){return;}
;qx.application.Standalone.prototype.main.call(this);this._setUpResources();this._setUpStores();this._createGui();this._setUpBinding();this.__OD.get();this.__JB.addListener(a,function(K){var J=K.getData();this.__JB.getSelection().push(J.getItem(0));}
,this);this.__JB.getSelection().addListener(n,function(L){var M=this.__JB.getSelection().getItem(0).getId();this.__OE.get({id:M});}
,this);}
,_setUpResources:function(){this.__OD=new github.rest.Resource({"get":{method:g,url:m}});this.__OE=new github.rest.Resource({"get":{method:g,url:h}});}
,_setUpStores:function(){this.__OF=new qx.data.store.Rest(this.__OD,r);this.__OG=new qx.data.store.Rest(this.__OE,r);}
,_createGui:function(){var N=new qx.ui.container.Composite(new qx.ui.layout.Dock());N.setPadding(10);var O=new qx.ui.basic.Label(z);O.setFont(C);O.setPaddingBottom(10);N.add(O,{edge:y});this.__JB=new qx.ui.list.List();this.__JB.setWidth(200);this.__OH=new github.view.Gist();N.add(this.__JB,{edge:f});N.add(this.__OH,{edge:p});this.getRoot().add(N,{edge:0});}
,_setUpBinding:function(){var Q=this.__JB,P=this.__OF,R=this.__OG;Q.setLabelPath(F);Q.setLabelOptions({converter:function(W,U,T,V){if(W===null||!W.length){return U.getId();}
;return W;}
});P.bind(D,Q,D);var S=this.__OH;R.bind(e,S.getDescription(),q);R.bind(w,S.getUsername(),q);R.bind(b,S.getGravatar(),l);R.bind(x,S.getContent(),u,{converter:function(Y){var X=qx.Class.getProperties(Y.constructor);var content=Y.get(X[0]).getContent();content=qx.bom.String.escape(content);return I+content+H;}
});}
...
});qx.Class.define(d,{extend:qx.ui.container.Composite,construct:function(){qx.ui.container.Composite.call(this);var bc=new qx.ui.layout.Grid(5,5);bc.setColumnFlex(0,1);bc.setRowFlex(2,1);this.setLayout(bc);this.setPadding(0,10);var bd;var bf=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));bd=new qx.ui.basic.Label(i);bd.setFont(C);this.__nC=new qx.ui.basic.Label();bf.add(bd);bf.add(this.__nC);this.add(bf,{row:0,column:0});var be=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));this.__OI=new qx.ui.basic.Image();this.__OI.set({width:40,height:40,scale:true,backgroundColor:s});this.__OJ=new qx.ui.basic.Label();be.add(this.__OI);be.add(this.__OJ);this.add(be,{row:0,column:1});bd=new qx.ui.basic.Label(o);bd.setFont(C);var scroll=new qx.ui.container.Scroll();this.__pK=new qx.ui.embed.Html();this.__pK.setMinHeight(1000);scroll.add(this.__pK);this.add(bd,{row:1,column:0});this.add(scroll,{row:2,column:0,colSpan:2});}
,members:{__nC:v,__OI:v,__OJ:v,__pK:v,getDescription:function(){return this.__nC;}
,getGravatar:function(){return this.__OI;}
,getUsername:function(){return this.__OJ;}
,getContent:function(){return this.__pK;}
}});}
)();
...
;this._delegate=j;this._marshaler=new qx.data.marshal.Json(j);if(j&&qx.lang.Type.isFunction(j.configureRequest)){this.__zn();}
;this.__zm=qx.lang.Function.bind(this.__zp,this);this.__zo();}
,properties:{resource:{check:c},actionName:{check:f},model:{nullable:true,event:a}},members:{_marshaler:null,_delegate:null,__zm:null,__zn:function(){var k=this.getResource(),l=this._delegate;k.configureRequest(l.configureRequest);}
,__zo:function(){var n=this.getResource(),m=this.getActionName();if(n&&m){n.addListener(this.getActionName()+d,this.__zm);}
;}
,__zp:function(e){var r=e.getData(),q=this._marshaler,s,o=this.getModel(),p=this._delegate;if(r){if(p&&p.manipulateData){r=p.manipulateData(r);}
