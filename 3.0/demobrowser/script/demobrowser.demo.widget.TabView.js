...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.TabView.5fd26bf335b4.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="bottom",b="button",c="qx.event.type.Data",d="lastTab",f="barRight",g="beforeChangeSelection",h="qx.debug",j="__sU",k="horizontal",m="changeSelection",n="bar",o="close-button",p="Index should be less than : ",q="tabview",r="vertical",s="page",t="Incompatible child for TabView: ",u="close",v="_applyBarPosition",w="barLeft",x="top",y="right",z="qx.ui.tabview.TabView",A="barTop",B="barBottom",C="pane",D="left",E="firstTab";qx.Class.define(z,{extend:qx.ui.core.Widget,implement:qx.ui.core.ISingleSelection,include:[qx.ui.core.MContentPadding],construct:function(G){qx.ui.core.Widget.call(this);this.__De={top:A,right:f,bottom:B,left:w};this._createChildControl(n);this._createChildControl(C);var F=this.__sU=new qx.ui.form.RadioGroup;F.setWrap(false);F.addListener(m,this._onChangeSelection,this);if(G!=null){this.setBarPosition(G);}
else {this.initBarPosition();}
;}
,events:{"changeSelection":c},properties:{appearance:{refine:true,init:q},barPosition:{check:[D,y,x,a],init:x,apply:v}},members:{__sU:null,_createChildControlImpl:function(J,I){var H;switch(J){case n:H=new qx.ui.container.SlideBar();H.setZIndex(10);this._add(H);break;case C:H=new qx.ui.container.Stack;H.setZIndex(5);this._add(H,{flex:1});break;};return H||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,J);}
...
;}
;var K=O.getButton();var L=this.getChildControl(n);var N=this.getChildControl(C);O.exclude();L.add(K);N.add(O);this.__sU.add(K);O.addState(this.__De[this.getBarPosition()]);O.addState(d);var M=this.getChildren();if(M[0]==O){O.addState(E);}
...
;var R=U.getButton();var Q=this.getChildControl(n);var T=this.getChildControl(C);U.exclude();Q.addAt(R,P);T.addAt(U,P);this.__sU.add(R);U.addState(this.__De[this.getBarPosition()]);S=this.getChildren();if(P==S.length-1){U.addState(d);}
...
;}
;W.remove(X);ba.remove(Y);this.__sU.remove(X);Y.removeState(this.__De[this.getBarPosition()]);if(Y.hasState(E)){Y.removeState(E);if(bb[0]){bb[0].addState(E);}
;}
...
,__De:null,_applyBarPosition:function(bg,bi){var bd=this.getChildControl(n);var bj=this.getChildControl(C);var bl=bg==D||bg==y;var bm=bg==y||bg==a;var bk=bl?qx.ui.layout.HBox:qx.ui.layout.VBox;var bn=this._getLayout();if(bn&&bn instanceof bk){}
else {this._setLayout(bn=new bk);}
;bn.setReversed(bm);bd.setOrientation(bl?r:k);var bh=this.getChildren();if(bi){var bf=this.__De[bi];bd.removeState(bf);bj.removeState(bf);for(var i=0,l=bh.length;i<l;i++ ){bh[i].removeState(bf);}
;}
;if(bg){var be=this.__De[bg];bd.addState(be);bj.addState(be);for(var i=0,l=bh.length;i<l;i++ ){bh[i].addState(be);}
;}
;}
,getSelection:function(){var bo=this.__sU.getSelection();var bp=[];for(var i=0;i<bo.length;i++ ){bp.push(bo[i].getUserData(s));}
;return bp;}
,setSelection:function(br){var bq=[];for(var i=0;i<br.length;i++ ){bq.push(br[i].getChildControl(b));}
;this.__sU.setSelection(bq);}
,resetSelection:function(){this.__sU.resetSelection();}
,isSelected:function(bs){var bt=bs.getChildControl(b);return this.__sU.isSelected(bt);}
,isSelectionEmpty:function(){return this.__sU.isSelectionEmpty();}
,getSelectables:function(bv){var bu=this.__sU.getSelectables(bv);var bw=[];for(var i=0;i<bu.length;i++ ){bw.push(bu[i].getUserData(s));}
;return bw;}
...
},destruct:function(){this._disposeObjects(j);this.__De=null;}
});}
)();
...
,__oQ:function(e){var m=e.getOldData()[0];var n=e.getData()[0];if(m){if(this.isDynamic()){m.exclude();}
else {m.hide();}
;}
;if(n){n.show();}
;}
...
(function(){var a="_applyIcon",b="",c="page",d="close",e="qx.event.type.Event",f="_applyShowCloseButton",g="tabview-page",h="qx.ui.tabview.Page",i="_applyLabel",j="button",k="String",l="Boolean";qx.Class.define(h,{extend:qx.ui.container.Composite,construct:function(m,n){qx.ui.container.Composite.call(this);this._createChildControl(j);if(m!=null){this.setLabel(m);}
;if(n!=null){this.setIcon(n);}
;}
,events:{"close":e},properties:{appearance:{refine:true,init:g},label:{check:k,init:b,apply:i},icon:{check:k,init:b,apply:a,nullable:true},showCloseButton:{check:l,init:false,apply:f}},members:{_forwardStates:{barTop:1,barRight:1,barBottom:1,barLeft:1,firstTab:1,lastTab:1},_applyIcon:function(p,o){this.getChildControl(j).setIcon(p);}
,_applyLabel:function(r,q){this.getChildControl(j).setLabel(r);}
,_applyEnabled:function(u,t){qx.ui.container.Composite.prototype._applyEnabled.call(this,u,t);var s=this.getChildControl(j);u==null?s.resetEnabled():s.setEnabled(u);}
,_createChildControlImpl:function(x,w){var v;switch(x){case j:v=new qx.ui.tabview.TabButton;v.setAllowGrowX(true);v.setAllowGrowY(true);v.setUserData(c,this);v.addListener(d,this._onButtonClose,this);break;};return v||qx.ui.container.Composite.prototype._createChildControlImpl.call(this,x);}
,_applyShowCloseButton:function(z,y){this.getChildControl(j).setShowCloseButton(z);}
,_onButtonClose:function(){this.fireEvent(d);}
,getButton:function(){return this.getChildControl(j);}
}});}
)();
