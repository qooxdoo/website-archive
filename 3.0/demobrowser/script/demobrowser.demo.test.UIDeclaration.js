...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.UIDeclaration.b69da5339341.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="Last Name",b="Page 1",c="First Name",d="firstname",f="Email",g="Demo loaded",h="icon/16/apps/office-calendar.png",j="Item",k="Page 3",l="move",m="First Window",n="Resized to: ",o="Page 2",p="resize",q="Date",r="win",s="Moved to: ",t="Cancel",u="icon/32/apps/office-address-book.png",v="Header",w="Welcome to your first own Window.<br/>Have fun!",x="Save",y="Button",z="Simple Form",A="demobrowser.demo.test.UIDeclaration",B="Company",C="x";qx.Class.define(A,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var D={clazz:qx.ui.form.Form,children:[{type:v,label:z},{type:j,label:c,validator:null,name:d,widget:{clazz:qx.ui.form.TextField,required:true,width:200}},{type:j,label:a,widget:{clazz:qx.ui.form.TextField}},{type:j,label:B,widget:{clazz:qx.ui.form.TextField}},{type:j,label:f,widget:{clazz:qx.ui.form.TextField}},{type:j,label:q,widget:{clazz:qx.ui.form.DateField}},{type:y,widget:{clazz:qx.ui.form.Button,label:x}},{type:y,widget:{clazz:qx.ui.form.Button,label:t}}]};var E={clazz:qx.ui.window.Window,layout:{clazz:qx.ui.layout.VBox,spacing:10},layoutProperties:{left:20,top:20},showStatusbar:true,status:g,caption:m,icon:h,id:r,listeners:[{event:l,handler:function(e){this.debug(s+e.getData().left+C+e.getData().top);}
},{event:p,handler:function(e){this.debug(n+e.getData().width+C+e.getData().height);}
}],children:[{clazz:qx.ui.basic.Atom,label:w,icon:u,rich:true},{clazz:qx.ui.tabview.TabView,layoutProperties:{flex:1},children:[{clazz:qx.ui.tabview.Page,label:b,layoutProperties:{flex:1},layout:{clazz:qx.ui.layout.Canvas},children:[D]},{clazz:qx.ui.tabview.Page,label:o,layoutProperties:{flex:1}},{clazz:qx.ui.tabview.Page,label:k,layoutProperties:{flex:1}}]}]};this.addFromDefinition(E);this.win.open();}
,addFromDefinition:function(F){var G=this.getRoot();this.__Eu(G,F);}
,__Eu:function(M,H){var I=new H.clazz();var L=qx.lang.Object.clone(H);delete L.clazz;delete L.layout;delete L.children;delete L.id;delete L.listeners;I.set(L);if(H.id!=null){this[H.id]=I;}
;if(H.layout!=null){var J=new H.layout.clazz();var L=qx.lang.Object.clone(H.layout);delete L.clazz;J.set(L);I.setLayout(J);}
;var N=H.children;if(N!=null){for(var i=0;i<N.length;i++ ){if(I instanceof qx.ui.form.Form){this.__Ro(I,N[i]);}
else {this.__Eu(I,N[i]);}
;}
;}
;if(I instanceof qx.ui.form.Form){M.add(new qx.ui.form.renderer.Single(I));}
else {M.add(I,H.layoutProperties);}
;if(H.listeners){for(var i=0;i<H.listeners.length;i++ ){var K=H.listeners[i];I.addListener(K.event,K.handler,K.context);}
;}
;}
,__Ro:function(Q,O){if(O.type===v){Q.addGroupHeader(O.label);return;}
;var P=new O.widget.clazz();var R=qx.lang.Object.clone(O.widget);delete R.clazz;P.set(R);if(O.type===j){Q.add(P,O.label,O.validator,O.name);}
else if(O.type===y){Q.addButton(P);}
;}
}});}
)();
...
,events:{"changeValue":l},properties:{dateFormat:{check:c,apply:s},placeholder:{check:r,nullable:true,apply:F},appearance:{refine:true,init:E},focusable:{refine:true,init:true},width:{refine:true,init:120}},statics:{__HT:null,__HU:null,getDefaultDateFormatter:function(){var K=qx.locale.Date.getDateFormat(y).toString();if(K==this.__HT){return this.__HU;}
;if(this.__HU){this.__HU.dispose();}
;this.__HU=new qx.util.format.DateFormat(K,qx.locale.Manager.getInstance().getLocale());this.__HT=K;return this.__HU;}
},members:{__HV:null,_forwardStates:{focused:true,invalid:true},_setDefaultDateFormat:function(){this.setDateFormat(qx.ui.form.DateField.getDefaultDateFormatter());}
,_addLocaleChangeListener:function(){if(qx.core.Environment.get(x)){this.__HV=qx.locale.Manager.getInstance().addListener(o,function(){this._setDefaultDateFormat();}
,this);}
;}
...
,_onClick:function(e){var bi=e.getTarget();if(bi==this.getChildControl(i)){this.toggle();}
else {this.close();}
;}
,_onBlur:function(e){this.close();}
,_onKeyPress:function(e){var bj=e.getKeyIdentifier();if(bj==h&&e.isAltPressed()){this.toggle();e.stopPropagation();return;}
;var bk=this.getChildControl(A);if(bk.getVisibility()==G){return;}
;if(bj==d){this.close();e.stopPropagation();return;}
;if(bj===g||bj===H||bj===h||bj===j){e.preventDefault();}
;this.getChildControl(n).handleKeyPress(e);}
,_onPopupChangeVisibility:function(e){e.getData()==p?this.addState(a):this.removeState(a);var bm=this.getChildControl(A);if(bm.isVisible()){var bn=this.getChildControl(n);var bl=this.getValue();bn.setValue(bl);}
;}
,_onTextFieldChangeValue:function(e){var bo=this.getValue();if(bo!=null){var bp=this.getChildControl(n);bp.setValue(bo);}
;this.fireDataEvent(f,this.getValue());}
,isEmpty:function(){var bq=this.getChildControl(v).getValue();return bq==null||bq==t;}
},destruct:function(){if(qx.core.Environment.get(x)){if(this.__HV){qx.locale.Manager.getInstance().removeListenerById(this.__HV);}
;}
;}
});}
)();
...
;var bI=this.getChildControl(bv);bI.addState(K);bG.add(bI,{column:0,row:0});this.__yp=[];for(var i=0;i<7;i++ ){bI=this.getChildControl(H+i);bG.add(bI,{column:i+1,row:0});this.__yp.push(bI);}
;this.__yq=[];this.__yr=[];for(var y=0;y<6;y++ ){var bI=this.getChildControl(S+(y+1));bG.add(bI,{column:0,row:y+1});this.__yr.push(bI);for(var x=0;x<7;x++ ){var bI=this.getChildControl(A+((y*7)+x));bG.add(bI,{column:x+1,row:y+1});this.__yq.push(bI);}
;}
...
else {var bL=(bN==null)?-1:bN.getDate();for(var i=0;i<6*7;i++ ){var bO=this.__yq[i];if(bO.hasState(N)){if(bO.hasState(q)){bO.removeState(q);}
;}
...
;this.__yp=this.__yq=this.__yr=null;}
});}
)();
