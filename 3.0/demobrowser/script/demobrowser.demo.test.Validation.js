...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Validation","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Validation.7d9bd005ea25.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="%1 is not a number.",b="%1 is not a color! %2",c="",d="%1 is not a string.",f="number",g="'%1' is not an email address.",h="qx.util.Validate",i="%1 is not an url.",j="%1 is not in %2",k="%1 does not fit %2.",l="string",m="%1 is not in the range from [%2, %3].",n="Validation Error";qx.Class.define(h,{statics:{number:function(o){return function(p){qx.util.Validate.checkNumber(p,null,o);}
;}
,checkNumber:function(r,s,q){q=q||qx.locale.Manager.tr(a,r);if((typeof r!==f&&(!(r instanceof Number)))||(!(isFinite(r)))){throw new qx.core.ValidationError(n,q);}
;}
,email:function(t){return function(u){qx.util.Validate.checkEmail(u,null,t);}
;}
,checkEmail:function(x,y,v){v=v||qx.locale.Manager.tr(g,(x||c));var w=/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;if(w.test(x)===false){throw new qx.core.ValidationError(n,v);}
;}
,string:function(z){return function(A){qx.util.Validate.checkString(A,null,z);}
;}
,checkString:function(C,D,B){B=B||qx.locale.Manager.tr(d,C);if(typeof C!==l&&(!(C instanceof String))){throw new qx.core.ValidationError(n,B);}
;}
,url:function(E){return function(F){qx.util.Validate.checkUrl(F,null,E);}
;}
,checkUrl:function(I,J,G){G=G||qx.locale.Manager.tr(i,I);var H=/([A-Za-z0-9])+:\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;if(!H.test(I)){throw new qx.core.ValidationError(n,G);}
;}
,color:function(K){return function(L){qx.util.Validate.checkColor(L,null,K);}
;}
,checkColor:function(O,P,N){try{qx.util.ColorUtil.stringToRgb(O);}
catch(e){var M=N||qx.locale.Manager.tr(b,O,e);throw new qx.core.ValidationError(n,M);}
;}
,range:function(S,Q,R){return function(U){var T=R||qx.locale.Manager.tr(m,U,S,Q);if(U<S||U>Q){throw new qx.core.ValidationError(n,T);}
;}
;}
,inArray:function(V,W){return function(Y){var X=W||qx.locale.Manager.tr(j,Y,V);if(V.indexOf(Y)===-1){throw new qx.core.ValidationError(n,X);}
;}
;}
,regExp:function(bb,ba){return function(bd){var bc=ba||qx.locale.Manager.tr(k,bd,bb);if(!bb.test(bd)){throw new qx.core.ValidationError(n,bc);}
;}
;}
}});}
)();
...
(function(){var a="demobrowser.demo.test.Validation",b="<b>Setting the age ",c="property to 27</b><br>",d="- name: Name is validated to a minimum length of two.<br/>",e="<br>old name: ",f=")",g="age:",h="new name: ",i="new gender: ",j="fmale",k="<br><b>Setting the name to 'Hans'</b><br>",l="property to 'female'</b><br>",m="<br><b>Setting the gender ",n="must be longer than one characters. (",o="<b>Testing the gender ",p="<br>old gender: ",q="male",r="<br><b>Setting the age ",s="- age: Age is validated to be a number.<br/>",t="<b>Initial property values</b><br>",u="female",v="",w="<b>Setting the name to ''</b><br>",x="demobrowser.demo.test.Person",y="property to 'true'</b><br>",z="- gender: Gender is validated to be either male or female.",A="property to 'fmale'</b><br>",B="<font color='red'>GENDER NOT SET</font>",C="gender:",D="<br>old age: ",E="Hans",F="The demo creates an internal class containing three properties:<br/>",G="new age: ",H="Validation Error: Name ",I="<font color='red'>AGE NOT SET</font>",J="My name",K="__Rp",L="<font color='red'>NAME NOT SET</font>",M="<br>",N="white",O="name:",P="<b>Description</b><br/>";qx.Class.define(a,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var Q=new demobrowser.demo.test.Person();var R=new qx.ui.embed.Html();R.setBackgroundColor(N);R.set({padding:10,width:250,height:360});this.getRoot().add(R,{left:10,top:10});R.setHtml(t);R.setHtml(R.getHtml()+O+Q.getName()+M);R.setHtml(R.getHtml()+g+Q.getAge()+M);R.setHtml(R.getHtml()+C+Q.getGender()+M);R.setHtml(R.getHtml()+k);Q.setName(E);R.setHtml(R.getHtml()+h+Q.getName()+M);R.setHtml(R.getHtml()+w);try{Q.setName(v);R.setHtml(R.getHtml()+h+Q.getName()+M);}
catch(T){R.setHtml(R.getHtml()+L+e+Q.getName()+M);}
;R.setHtml(R.getHtml()+r+c);Q.setAge(27);R.setHtml(R.getHtml()+G+Q.getAge()+M);R.setHtml(R.getHtml()+b+y);try{Q.setAge(true);R.setHtml(R.getHtml()+G+Q.getAge()+M);}
catch(U){R.setHtml(R.getHtml()+I+D+Q.getAge()+M);}
;R.setHtml(R.getHtml()+m+l);Q.setGender(u);R.setHtml(R.getHtml()+i+Q.getGender()+M);R.setHtml(R.getHtml()+o+A);try{Q.setAge(j);R.setHtml(R.getHtml()+i+Q.getGender()+M);}
catch(V){R.setHtml(R.getHtml()+B+p+Q.getGender()+M);}
;var S=new qx.ui.basic.Label();S.setRich(true);S.setWidth(250);S.setValue(P+F+d+s+z);this.getRoot().add(S,{left:280,top:10});}
}});qx.Class.define(x,{extend:qx.core.Object,properties:{name:{init:J,validate:K},age:{init:18,validate:qx.util.Validate.number()},gender:{init:q,validate:qx.util.Validate.inArray([q,u])}},members:{__Rp:function(W){if(W.length<2){throw new qx.core.ValidationError(H+n+W+f);}
;}
}});}
)();
(function(){var a="_applyOverflowX",b="_applyOverflowY",c="auto",d="overflowX",e="visible",f="hidden",g="qx.ui.core.MNativeOverflow",h="scroll",i="overflowY";qx.Mixin.define(g,{properties:{overflowX:{check:[f,e,h,c],nullable:true,apply:a},overflowY:{check:[f,e,h,c],nullable:true,apply:b},overflow:{group:[d,i]}},members:{_applyOverflowX:function(j){this.getContentElement().setStyle(d,j);}
,_applyOverflowY:function(k){this.getContentElement().setStyle(i,k);}
}});}
)();
(function(){var a="text",b="engine.name",c="",d="none",e="webkit",f="changeHtml",g="_applyCssClass",h="qx.ui.embed.Html",i="_applyHtml",j="userSelect",k="color",l="String",m="html";qx.Class.define(h,{extend:qx.ui.core.Widget,include:[qx.ui.core.MNativeOverflow],construct:function(n){qx.ui.core.Widget.call(this);if(n!=null){this.setHtml(n);}
;}
,properties:{html:{check:l,apply:i,event:f,nullable:true},cssClass:{check:l,init:c,apply:g},selectable:{refine:true,init:true},focusable:{refine:true,init:true}},members:{getFocusElement:function(){return this.getContentElement();}
,_applyHtml:function(q,o){var p=this.getContentElement();p.setAttribute(m,q||c);}
,_applyCssClass:function(s,r){this.getContentElement().removeClass(r);this.getContentElement().addClass(s);}
,_applySelectable:function(t){qx.ui.core.Widget.prototype._applySelectable.call(this,t);if((qx.core.Environment.get(b)==e)){this.getContentElement().setStyle(j,t?a:d);}
;}
,_applyFont:function(w,v){var u=w?qx.theme.manager.Font.getInstance().resolve(w).getStyles():qx.bom.Font.getDefaultStyles();if(this.getTextColor()!=null){delete u[k];}
;this.getContentElement().setStyles(u);}
,_applyTextColor:function(y,x){if(y){this.getContentElement().setStyle(k,qx.theme.manager.Color.getInstance().resolve(y));}
else {this.getContentElement().removeStyle(k);}
;}
}});}
)();
