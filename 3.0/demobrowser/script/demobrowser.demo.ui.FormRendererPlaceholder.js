...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormRendererPlaceholder.a6b82dcb312f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="qx.ui.form.Form",b="";qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__Hf=[];this._buttons=[];this._buttonOptions=[];this._validationManager=new qx.ui.form.validation.Manager();this._resetter=this._createResetter();}
,members:{__Hf:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(e,d,f,name,c,g){if(this.__Hg()){this.__Hf.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
;this.__Hf[this._groupCounter].items.push(e);this.__Hf[this._groupCounter].labels.push(d);this.__Hf[this._groupCounter].options.push(g);if(name==null){name=d.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,b);}
;this.__Hf[this._groupCounter].names.push(name);this._validationManager.add(e,f,c);this._resetter.add(e);}
,addGroupHeader:function(k,h){if(!this.__Hg()){this._groupCounter++ ;}
;this.__Hf.push({title:k,items:[],labels:[],names:[],options:[],headerOptions:h});}
,addButton:function(l,m){this._buttons.push(l);this._buttonOptions.push(m||null);}
,__Hg:function(){return this.__Hf.length===0;}
...
,getGroups:function(){return this.__Hf;}
,getButtons:function(){return this._buttons;}
,getButtonOptions:function(){return this._buttonOptions;}
,getItems:function(){var o={};for(var i=0;i<this.__Hf.length;i++ ){var p=this.__Hf[i];for(var j=0;j<p.names.length;j++ ){var name=p.names[j];o[name]=p.items[j];}
;}
;return o;}
,_createResetter:function(){return new qx.ui.form.Resetter();}
},destruct:function(){this.__Hf=this._buttons=this._buttonOptions=null;this._validationManager.dispose();this._resetter.dispose();}
});}
)();
(function(){var a="Widgets supporting selection can only be validated ",b="Added widget not supported.",c="",d="in the form validator",f="changeValid",g="qx.event.type.Event",h="complete",j="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",k="qx.ui.form.validation.Manager",l="This field is required",m="String",n="qx.event.type.Data";qx.Class.define(k,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__CP=[];this.__CQ={};this.setRequiredFieldMessage(qx.locale.Manager.tr(l));}
,events:{"changeValid":n,"complete":g},properties:{validator:{check:j,init:null,nullable:true},invalidMessage:{check:m,init:c},requiredFieldMessage:{check:m,init:c},context:{nullable:true}},members:{__CP:null,__CR:null,__CQ:null,__CS:null,add:function(r,o,q){if(!this.__CX(r)){throw new Error(b);}
;if(this.__zP(r)&&!r.getValue){if(o!=null){throw new Error(a+d);}
;}
;var p={item:r,validator:o,valid:null,context:q};this.__CP.push(p);}
,remove:function(u){var s=this.__CP;for(var i=0,t=s.length;i<t;i++ ){if(u===s[i].item){s.splice(i,1);return u;}
;}
;return null;}
,getItems:function(){var v=[];for(var i=0;i<this.__CP.length;i++ ){v.push(this.__CP[i].item);}
;return v;}
,validate:function(){var A=true;this.__CS=true;var x=[];for(var i=0;i<this.__CP.length;i++ ){var y=this.__CP[i].item;var B=this.__CP[i].validator;x.push(y);if(B==null){var w=this.__CT(y);A=A&&w;this.__CS=w&&this.__CS;continue;}
;var w=this.__CU(this.__CP[i],y.getValue());A=w&&A;if(w!=null){this.__CS=w&&this.__CS;}
;}
;var z=this.__CV(x);if(qx.lang.Type.isBoolean(z)){this.__CS=z&&this.__CS;}
;A=z&&A;this.__CY(A);if(qx.lang.Object.isEmpty(this.__CQ)){this.fireEvent(h);}
;return A;}
,__CT:function(D){if(D.getRequired()){if(this.__zP(D)){var C=!!D.getSelection()[0];}
...
;return true;}
,__CU:function(J,K){var L=J.item;var N=J.context;var M=J.validator;if(this.__CW(M)){this.__CQ[L.toHashCode()]=null;M.validate(L,L.getValue(),this,N);return null;}
...
,__CV:function(P){var Q=this.getValidator();var R=this.getContext()||this;if(Q==null){return true;}
;this.setInvalidMessage(c);if(this.__CW(Q)){this.__CQ[this.toHashCode()]=null;Q.validateForm(P,this,R);return null;}
...
;}
;return S;}
,__CW:function(U){var T=false;if(!qx.lang.Type.isFunction(U)){T=qx.Class.isSubClassOf(U.constructor,qx.ui.form.validation.AsyncValidator);}
;return T;}
,__CX:function(V){var W=V.constructor;return qx.Class.hasInterface(W,qx.ui.form.IForm);}
,__zP:function(X){var Y=X.constructor;return qx.Class.hasInterface(Y,qx.ui.core.ISingleSelection);}
,__CY:function(ba){var bb=this.__CR;this.__CR=ba;if(bb!=ba){this.fireDataEvent(f,ba,bb);}
;}
,getValid:function(){return this.__CR;}
,isValid:function(){return this.getValid();}
,getInvalidMessages:function(){var bc=[];for(var i=0;i<this.__CP.length;i++ ){var bd=this.__CP[i].item;if(!bd.getValid()){bc.push(bd.getInvalidMessage());}
;}
;if(this.getInvalidMessage()!=c){bc.push(this.getInvalidMessage());}
;return bc;}
,getInvalidFormItems:function(){var be=[];for(var i=0;i<this.__CP.length;i++ ){var bf=this.__CP[i].item;if(!bf.getValid()){be.push(bf);}
;}
;return be;}
,reset:function(){for(var i=0;i<this.__CP.length;i++ ){var bg=this.__CP[i];bg.item.setValid(true);}
;this.__CR=null;}
,setItemValid:function(bi,bh){this.__CQ[bi.toHashCode()]=bh;bi.setValid(bh);this.__Da();}
,setFormValid:function(bj){this.__CQ[this.toHashCode()]=bj;this.__Da();}
,__Da:function(){var bl=this.__CS;for(var bm in this.__CQ){var bk=this.__CQ[bm];bl=bk&&bl;if(bk==null){return;}
;}
;this.__CY(bl);this.__CQ={};this.fireEvent(h);}
},destruct:function(){this.__CP=null;}
});}
)();
(function(){var a="qx.ui.form.validation.AsyncValidator";qx.Class.define(a,{extend:qx.core.Object,construct:function(b){qx.core.Object.call(this);this.__Db=b;}
,members:{__Db:null,__Dc:null,__gz:null,__Dd:null,validate:function(c,e,d,f){this.__Dd=false;this.__Dc=c;this.__gz=d;this.__Db.call(f||this,this,e);}
,validateForm:function(g,h,i){this.__Dd=true;this.__gz=h;this.__Db.call(i,g,this);}
,setValid:function(k,j){if(this.__Dd){if(j!==undefined){this.__gz.setInvalidMessage(j);}
;this.__gz.setFormValid(k);}
else {if(j!==undefined){this.__Dc.setInvalidMessage(j);}
;this.__gz.setItemValid(this.__Dc,k);}
;}
},destruct:function(){this.__gz=this.__Dc=null;}
});}
)();
...
(function(){var a="The given item has not been added.",b="qx.ui.form.Resetter",c=" not supported for reseting.",d="Item ";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__ru=[];}
,members:{__ru:null,add:function(e){if(this._supportsValue(e)){var f=e.getValue();}
else if(this.__zP(e)){var f=e.getSelection();}
else if(this.__zQ(e)){var f=e.getSelection().concat();}
else {throw new Error(d+e+c);}
;this.__ru.push({item:e,init:f});}
,reset:function(){for(var i=0;i<this.__ru.length;i++ ){var g=this.__ru[i];this.__zN(g.item,g.init);}
;}
,resetItem:function(h){var j;for(var i=0;i<this.__ru.length;i++ ){var k=this.__ru[i];if(k.item===h){j=k.init;break;}
;}
;if(j===undefined){throw new Error(a);}
;this.__zN(h,j);}
,__zN:function(l,m){if(this._supportsValue(l)){l.setValue(m);}
else if(this.__zP(l)||this.__zQ(l)){l.setSelection(m);}
;}
,redefine:function(){for(var i=0;i<this.__ru.length;i++ ){var n=this.__ru[i].item;this.__ru[i].init=this.__zO(n);}
;}
,redefineItem:function(o){var p;for(var i=0;i<this.__ru.length;i++ ){if(this.__ru[i].item===o){p=this.__ru[i];break;}
;}
;if(p===undefined){throw new Error(a);}
;p.init=this.__zO(p.item);}
,__zO:function(q){if(this._supportsValue(q)){return q.getValue();}
else if(this.__zP(q)||this.__zQ(q)){return q.getSelection();}
;}
,__zP:function(r){var s=r.constructor;return qx.Class.hasInterface(s,qx.ui.core.ISingleSelection);}
,__zQ:function(t){var u=t.constructor;return qx.Class.hasInterface(u,qx.data.controller.ISelection);}
...
,members:{_row:0,_buttonRow:null,addItems:function(m,p,l){if(l!=null){this._add(this._createHeader(l),{row:this._row,column:0,colSpan:2});this._row++ ;}
