...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Pane_GridLines.0846f595aab8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;this.__wM=[];this.__wN=[];this._isHorizontal=(w||c)==c;}
,properties:{defaultLineColor:{init:h,check:s,themeable:true},defaultLineSize:{init:1,check:t,themeable:true}},members:{__wM:null,__wN:null,isHorizontal:function(){return this._isHorizontal;}
...
;this.__wM[B]=A;if(this.__wO(B)){this.updateLayerData();}
;}
...
;this.__wN[C]=D;if(this.__wO(C)){this.updateLayerData();}
;}
,__wO:function(E){if(this._isHorizontal){var I=this.getFirstColumn();var G=I+this.getColumnSizes().length-1;return E>=I&&E<=G;}
else {var H=this.getFirstRow();var F=H+this.getRowSizes().length-1;return E>=H&&E<=F;}
;}
,getLineSize:function(J){return this.__wN[J]||this.getDefaultLineSize();}
,getLineColor:function(K){return this.__wM[K]||this.getDefaultLineColor();}
,__wP:function(P,M,O){var top=0;var L,N;for(var y=0;y<O.length-1;y++ ){L=this.getLineColor(M+y);N=this.getLineSize(M+y);top+=O[y];P.push(m,g,f+N+q,j,d,top-((N>1)?Math.floor(N/2):1),q,b,L,n,a);}
;}
,__wQ:function(T,Q,S){var V=0;var R,U;for(var x=0;x<S.length-1;x++ ){R=this.getLineColor(Q+x);U=this.getLineSize(Q+x);V+=S[x];T.push(m,g,k+U+q,l,u,p,V-((U>1)?Math.floor(U/2):1),q,b,R,n,a);}
;}
,_fullUpdate:function(ba,bb,X,W){var Y=[];if(this._isHorizontal){this.__wP(Y,ba,X);}
else {this.__wQ(Y,bb,W);}
...
;}
},destruct:function(){this.__wM=this.__wN=null;}
});}
)();
