...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Table_CellRenderer","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Table_CellRenderer.84aae1552274.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,properties:{selectionModel:{check:a}},members:{__uo:null,handleMouseDown:function(c,d){if(d.isLeftPressed()){var e=this.getSelectionModel();if(!e.isSelectedIndex(c)){this._handleSelectEvent(c,d);this.__uo=true;}
else {this.__uo=false;}
;}
else if(d.isRightPressed()&&d.getModifiers()==0){var e=this.getSelectionModel();if(!e.isSelectedIndex(c)){e.setSelectionInterval(c,c);}
;}
;}
,handleMouseUp:function(f,g){if(g.isLeftPressed()&&!this.__uo){this._handleSelectEvent(f,g);}
;}
...
(function(){var a="..",b="changeSelection",c="Try to turn off batch mode althoug it was not turned on.",d="_applySelectionMode",f="]",g="qx.event.type.Event",h="Ranges:",k="qx.ui.table.selection.Model",l=" [";qx.Class.define(k,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__uA=[];this.__uB=-1;this.__uC=-1;this.hasBatchModeRefCount=0;this.__uD=false;}
,events:{"changeSelection":g},statics:{NO_SELECTION:1,SINGLE_SELECTION:2,SINGLE_INTERVAL_SELECTION:3,MULTIPLE_INTERVAL_SELECTION:4,MULTIPLE_INTERVAL_SELECTION_TOGGLE:5},properties:{selectionMode:{init:2,check:[1,2,3,4,5],apply:d}},members:{__uD:null,__uB:null,__uC:null,__uA:null,_applySelectionMode:function(m){this.resetSelection();}
,setBatchMode:function(n){if(n){this.hasBatchModeRefCount+=1;}
else {if(this.hasBatchModeRefCount==0){throw new Error(c);}
;this.hasBatchModeRefCount-=1;if(this.__uD){this.__uD=false;this._fireChangeSelection();}
;}
;return this.hasBatchMode();}
,hasBatchMode:function(){return this.hasBatchModeRefCount>0;}
,getAnchorSelectionIndex:function(){return this.__uB;}
,_setAnchorSelectionIndex:function(o){this.__uB=o;}
,getLeadSelectionIndex:function(){return this.__uC;}
,_setLeadSelectionIndex:function(p){this.__uC=p;}
,_getSelectedRangeArr:function(){return this.__uA;}
,resetSelection:function(){if(!this.isSelectionEmpty()){this._resetSelection();this._fireChangeSelection();}
;}
,isSelectionEmpty:function(){return this.__uA.length==0;}
,getSelectedCount:function(){var r=0;for(var i=0;i<this.__uA.length;i++ ){var q=this.__uA[i];r+=q.maxIndex-q.minIndex+1;}
;return r;}
,isSelectedIndex:function(s){for(var i=0;i<this.__uA.length;i++ ){var t=this.__uA[i];if(s>=t.minIndex&&s<=t.maxIndex){return true;}
;}
;return false;}
,getSelectedRanges:function(){var u=[];for(var i=0;i<this.__uA.length;i++ ){u.push({minIndex:this.__uA[i].minIndex,maxIndex:this.__uA[i].maxIndex});}
;return u;}
,iterateSelection:function(w,v){for(var i=0;i<this.__uA.length;i++ ){for(var j=this.__uA[i].minIndex;j<=this.__uA[i].maxIndex;j++ ){w.call(v,j);}
;}
;}
...
,removeSelectionInterval:function(G,E){this.__uB=G;this.__uC=E;var D=Math.min(G,E);var H=Math.max(G,E);for(var i=0;i<this.__uA.length;i++ ){var J=this.__uA[i];if(J.minIndex>H){break;}
else if(J.maxIndex>=D){var K=(J.minIndex>=D)&&(J.minIndex<=H);var F=(J.maxIndex>=D)&&(J.maxIndex<=H);if(K&&F){this.__uA.splice(i,1);i-- ;}
else if(K){J.minIndex=H+1;}
else if(F){J.maxIndex=D-1;}
else {var I={minIndex:H+1,maxIndex:J.maxIndex};this.__uA.splice(i+1,0,I);J.maxIndex=D-1;break;}
;}
;}
;this._fireChangeSelection();}
,_resetSelection:function(){this.__uA=[];this.__uB=-1;this.__uC=-1;}
,_addSelectionInterval:function(O,M){this.__uB=O;this.__uC=M;var L=Math.min(O,M);var P=Math.max(O,M);var N=0;for(;N<this.__uA.length;N++ ){var Q=this.__uA[N];if(Q.minIndex>L){break;}
;}
;this.__uA.splice(N,0,{minIndex:L,maxIndex:P});var R=this.__uA[0];for(var i=1;i<this.__uA.length;i++ ){var Q=this.__uA[i];if(R.maxIndex+1>=Q.minIndex){R.maxIndex=Math.max(R.maxIndex,Q.maxIndex);this.__uA.splice(i,1);i-- ;}
else {R=Q;}
;}
;}
,_dumpRanges:function(){var S=h;for(var i=0;i<this.__uA.length;i++ ){var T=this.__uA[i];S+=l+T.minIndex+a+T.maxIndex+f;}
;this.debug(S);}
,_fireChangeSelection:function(){if(this.hasBatchMode()){this.__uD=true;}
...
;}
,properties:{highlightFocusRow:{check:b,init:true}},members:{_colors:null,__uP:null,__uQ:null,initThemeValues:function(){this.__uQ=r;this.__uP={};this._colors={};this._renderFont(qx.theme.manager.Font.getInstance().resolve(d));var y=qx.theme.manager.Color.getInstance();this._colors.bgcolFocusedSelected=y.resolve(u);this._colors.bgcolFocused=y.resolve(i);this._colors.bgcolSelected=y.resolve(p);this._colors.bgcolEven=y.resolve(a);this._colors.bgcolOdd=y.resolve(l);this._colors.colSelected=y.resolve(t);this._colors.colNormal=y.resolve(h);this._colors.horLine=y.resolve(n);}
,_insetY:1,_renderFont:function(z){if(z){this.__uP=z.getStyles();this.__uQ=qx.bom.element.Style.compile(this.__uP);this.__uQ=this.__uQ.replace(/"/g,g);}
else {this.__uQ=r;this.__uP=qx.bom.Font.getDefaultStyles();}
;}
,updateDataRowElement:function(D,A){var B=this.__uP;var C=A.style;qx.bom.element.Style.setStyles(A,B);if(D.focusedRow&&this.getHighlightFocusRow()){C.backgroundColor=D.selected?this._colors.bgcolFocusedSelected:this._colors.bgcolFocused;}
...
;return e+E+v;}
,createRowStyle:function(G){var F=[];F.push(x);F.push(this.__uQ);F.push(q);if(G.focusedRow&&this.getHighlightFocusRow()){F.push(G.selected?this._colors.bgcolFocusedSelected:this._colors.bgcolFocused);}
...
},destruct:function(){this._colors=this.__uP=this.__uQ=null;if(qx.core.Environment.get(o)){qx.theme.manager.Color.getInstance().removeListener(f,this.initThemeValues,this);qx.theme.manager.Font.getInstance().removeListener(f,this.initThemeValues,this);}
;}
});}
)();
...
(function(){var a="first",b="hovered",c="__uS",d="last",e="qx.ui.table.pane.Header";qx.Class.define(e,{extend:qx.ui.core.Widget,construct:function(f){qx.ui.core.Widget.call(this);this._setLayout(new qx.ui.layout.HBox());this.__mY=new qx.ui.core.Blocker(this);this.__uS=f;}
,members:{__uS:null,__vb:null,__vc:null,__mY:null,getPaneScroller:function(){return this.__uS;}
,getTable:function(){return this.__uS.getTable();}
...
;}
,setMouseOverColumn:function(l){if(l!=this.__vc){if(this.__vc!=null){var k=this.getHeaderWidgetAtColumn(this.__vc);if(k!=null){k.removeState(b);}
;}
...
,showColumnMoveFeedback:function(y,x){var r=this.getContentLocation();if(this.__vb==null){var w=this.getTable();var o=this.getPaneScroller().getTablePaneModel().getX(y);var q=this._getChildren()[o];var s=w.getTableModel();var u=w.getTableColumnModel();var v={xPos:o,col:y,name:s.getColumnName(y),table:w};var t=u.getHeaderCellRenderer(y);var p=t.createHeaderCell(v);var z=q.getBounds();p.setWidth(z.width);p.setHeight(z.height);p.setZIndex(1000000);p.setOpacity(0.8);p.setLayoutProperties({top:r.top});this.getApplicationRoot().add(p);this.__vb=p;}
;this.__vb.setLayoutProperties({left:r.left+x});this.__vb.show();}
,hideColumnMoveFeedback:function(){if(this.__vb!=null){this.__vb.destroy();this.__vb=null;}
;}
,isShowingColumnMoveFeedback:function(){return this.__vb!=null;}
...
;}
},destruct:function(){this.__mY.dispose();this._disposeObjects(c);}
});}
)();
...
(function(){var a='<div>',b="</div>",c="overflow: hidden;",d="qx.event.type.Data",e='</div>',f="paneReloadsData",g="div",h='style="',j="_applyMaxCacheLines",k="qx.ui.table.pane.Pane",l="width: 100%;",m="qx.event.type.Event",n="_applyVisibleRowCount",o='>',p="",q='" ',r="line-height: ",s="paneUpdated",t="appear",u='class="',v="width:100%;",w="px;",z='<div ',A="'>",B="Number",C="_applyFirstVisibleRow",D="<div style='",E=";position:relative;";qx.Class.define(k,{extend:qx.ui.core.Widget,construct:function(F){qx.ui.core.Widget.call(this);this.__uS=F;this.__uT=0;this.__uK=0;this.__uU=[];}
,events:{"paneReloadsData":d,"paneUpdated":m},properties:{firstVisibleRow:{check:B,init:0,apply:C},visibleRowCount:{check:B,init:0,apply:n},maxCacheLines:{check:B,init:1000,apply:j},allowShrinkX:{refine:true,init:false}},members:{__uK:null,__uT:null,__uS:null,__uV:null,__uH:null,__uG:null,__uU:null,__uW:0,_applyFirstVisibleRow:function(H,G){this.updateContent(false,H-G);}
,_applyVisibleRowCount:function(J,I){this.updateContent(true);}
,_getContentHint:function(){return {width:this.getPaneScroller().getTablePaneModel().getTotalWidth(),height:400};}
,getPaneScroller:function(){return this.__uS;}
,getTable:function(){return this.__uS.getTable();}
,setFocusedCell:function(M,L,K){if(M!=this.__uG||L!=this.__uH){var N=this.__uH;this.__uG=M;this.__uH=L;if(L!=N&&!K){if(N!==null){this.updateContent(false,null,N,true);}
...
,onTableModelDataChanged:function(T,V,R,Q){this.__uX();var S=this.getFirstVisibleRow();var U=this.getVisibleRowCount();if(V==-1||V>=S&&T<S+U){this.updateContent();}
;}
,onTableModelMetaDataChanged:function(){this.updateContent(true);}
,_applyMaxCacheLines:function(X,W){if(this.__uW>=X&&X!==-1){this.__uX();}
;}
,__uX:function(){this.__uU=[];this.__uW=0;}
,__uY:function(bb,Y,ba){if(!Y&&!ba&&this.__uU[bb]){return this.__uU[bb];}
else {return null;}
;}
,__va:function(bg,bf,bd,be){var bc=this.getMaxCacheLines();if(!bd&&!be&&!this.__uU[bg]&&bc>0){this._applyMaxCacheLines(bc);this.__uU[bg]=bf;this.__uW+=1;}
;}
,updateContent:function(bh,bk,bj,bi){if(bh){this.__uX();}
...
;bM.push(e);var bP=bM.join(p);this.__va(bC,bP,bF,bJ);bX.push(bP);}
...
;}
;if(!this.__uV){this.__uV=document.createElement(g);}
;var cj=a;cj+=this._getRowsHtml(ch+cd,Math.abs(cm));cj+=e;this.__uV.innerHTML=cj;var ci=this.__uV.firstChild.childNodes;if(cm>0){for(var i=ci.length-1;i>=0;i-- ){var cg=ci[0];ck.appendChild(cg);}
;}
else {for(var i=ci.length-1;i>=0;i-- ){var cg=ci[ci.length-1];ck.insertBefore(cg,ck.firstChild);}
;}
;if(this.__uH!==null){this._updateRowStyles(this.__uH-cm);this._updateRowStyles(this.__uH);}
;this.fireEvent(s);}
...
;var cA=cu.join(p);cs.innerHTML=cA;this.setWidth(cr);this.__uT=cw;this.__uK=cq;this.fireEvent(s);}
},destruct:function(){this.__uV=this.__uS=this.__uU=null;}
});}
)();
...
;qx.ui.table.cellrenderer.Abstract.__tK.stylesheet=qx.bom.Stylesheet.createElement(N);}
...
;if(this.getGroupingUsed()){var z=E;E=l;var H;for(H=z.length;H>3;H-=3){E=l+qx.locale.Number.getGroupSeparator(this.__kE)+z.substring(H-3,H)+E;}
;E=z.substring(0,H)+E;}
;var B=this.getPrefix()?this.getPrefix():l;var y=this.getPostfix()?this.getPostfix():l;var F=B+(A?a:l)+E;if(D.length>0){F+=l+qx.locale.Number.getDecimalSeparator(this.__kE)+D;}
;F+=y;return F;}
,parse:function(R){var N=qx.lang.String.escapeRegexpChars(qx.locale.Number.getGroupSeparator(this.__kE)+l);var L=qx.lang.String.escapeRegexpChars(qx.locale.Number.getDecimalSeparator(this.__kE)+l);var J=new RegExp(c+qx.lang.String.escapeRegexpChars(this.getPrefix())+o+m+N+k+h+L+s+qx.lang.String.escapeRegexpChars(this.getPostfix())+q);var M=J.exec(R);if(M==null){throw new Error(d+R+e);}
...
(function(){var a="-",b="QQQ",c='W',d="qqqq",e="h",f="KK",g='L',h=" is not supported",m="Z",n="Date string '",o="GGGGG",p='abbreviated',q="Y+",r=". Wildcard ",s="eeeee",t="(\\d\\d?)",u="LL",v="abbreviated",w="EEEE",x="^",y=":",z='y',A="quoted_literal",B="weekOfYear",C='3rd quarter',D="SSS",E='a',F="day",G="(-*",H="qqq",I="locale",J="K",K='q',L="Provided date is invalid",M="pm",N="Error parsing date '",O='Anno Domini',P='G',Q="a",R='2nd quarter',S="yyyy-MM-dd'T'HH:mm:ss",T="GMT",U="HH:mm:ss",V="dd",W="+",X="qx.util.format.DateFormat",Y='weekDay',en="am",ei="eee",eo="quarter",ek="ee",el="q",eh="QQQQ",em="HHmmss",es="ww",et="H",eu='Y+',ev="wide",ep='wide',eq="(\\d\\d\\d?)",ej="(GMT[\\+\\-]\\d\\d:\\d\\d)",er="_applyLocale",ez="Q",fd="c",eA="(",eB="weekOfMonth",ew="ccc",ex="long",ge="Malformed date format: ",ey="EEEEE",eC='Y',eD="EE",eE="GGG",eJ='-',eK="w",eL="mm",eF='h',eG="S",eH='s',eI='A',eQ="yyyy-MM-dd",eR="G",eS='z',eT="GG",eM=")",eN="kk",eO="ss",eP="([\\+\\-]\\d\\d\\d\\d)",eX="default",eY='BC',gi='H',fa='S',eU='1st quarter',eV="stand-alone",gg="0",eW="MMMM",fb='c',fc="d",fo='Q',fn='Q1',fm="eeee",fs="QQ",fr="LLLLL",fq="HH",fp='k',fh="m",fg="DDD",ff="D",fe='D',fl="LLLL",fk="(\\d\\d*?)",fj='e',fi="': the value for day or month is too large",fz="literal",fy='K',fx="L",fw="hh",fD="ms",fC="(\\d\\d?\\d?)",fB="cccc",fA="e",fv='y+',fu="|",ft="W",fO="dayOfYear",fN="GGGG",fM='Q2',fS='B',fR="MM",fQ='Q4',fP="y+",fH='narrow',fG="ccccc",fF="E",fE="wildcard",fL='',fK="' does not match the date format: ",fJ="short",fI='d',fY="unkown",fX="narrow",fW="\\d?",fV="'",gd="k",gc='Before Christ',gb='Z',ga="qx.debug",fU='m',fT=" ",dQ="yyyy-MM-dd'T'HH:mm:ss'Z'",dP="String",gj='Q3',dN="z",dO="DD",dM="min",gh="",dK='M',dL="sec",dJ='isoUtcDateTime',gf='4th quarter',dH="(\\d?)",dI="SS",dG="MMM",ea="s",eb="M",dX='w',dY="EEE",dV="$",dW="?",dU='AD',dF="\\d",dS="+?",dT="hour",dR="MMMMM",eg='E',ee='month',ef="LLL",ec="qq",ed="format";qx.Class.define(X,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(gl,gk){qx.core.Object.call(this);if(!gk){this.__kE=qx.locale.Manager.getInstance().getLocale();this.__to=qx.locale.Manager.getInstance().bind(I,this,I);}
else {this.__kE=gk;this.setLocale(gk);}
;this.__tp=this.__kE;if(gl!=null){this.__tq=gl.toString();if(this.__tq in qx.util.format.DateFormat.ISO_MASKS){if(this.__tq===dJ){this.__tr=true;}
;this.__tq=qx.util.format.DateFormat.ISO_MASKS[this.__tq];}
;}
else {this.__tq=qx.locale.Date.getDateFormat(ex,this.__kE)+fT+qx.locale.Date.getDateTimeFormat(em,U,this.__kE);}
;}
,properties:{locale:{apply:er,nullable:true,check:dP}},statics:{getDateInstance:function(){var gm=qx.util.format.DateFormat;var gn=qx.locale.Date.getDateFormat(fJ)+gh;if(gm._dateInstance==null||gm._dateInstance.__tq!=gn){gm._dateInstance=new gm(gn);}
;return gm._dateInstance;}
,getDateTimeInstance:function(){var go=qx.util.format.DateFormat;var gp=qx.locale.Date.getDateFormat(ex)+fT+qx.locale.Date.getDateTimeFormat(em,U);if(go._dateTimeInstance==null||go._dateTimeInstance.__tq!=gp){go._dateTimeInstance=new go(gp);}
;return go._dateTimeInstance;}
,ASSUME_YEAR_2000_THRESHOLD:30,ISO_MASKS:{isoDate:eQ,isoTime:U,isoDateTime:S,isoUtcDateTime:dQ},AM_MARKER:en,PM_MARKER:M},members:{__to:null,__kE:null,__tp:null,__tq:null,__ts:null,__tt:null,__tu:null,__tr:null,__tv:function(gr,gq){var gs=gh+(gr<0?((-1)*gr):gr);while(gs.length<gq){gs=gg+gs;}
;return gr<0?a+gs:gs;}
,__tw:function(gt){var gu=new Date(gt.getTime());var gv=gu.getDate();while(gu.getMonth()!=0){gu.setDate(-1);gv+=gu.getDate()+1;}
;return gv;}
,__tx:function(gw){return new Date(gw.getTime()+(3-((gw.getDay()+6)%7))*86400000);}
,__ty:function(gx){var gz=this.__tx(gx);var gA=gz.getFullYear();var gy=this.__tx(new Date(gA,0,4));return Math.floor(1.5+(gz.getTime()-gy.getTime())/86400000/7);}
,__tz:function(gB){var gD=this.__tx(gB);var gC=this.__tx(new Date(gB.getFullYear(),gB.getMonth(),4));return Math.floor(1.5+(gD.getTime()-gC.getTime())/86400000/7);}
,__tA:function(gE){var gF=this.__tx(gE);return gF.getFullYear();}
,__tB:function(gH){var gG=new Date(gH,2,1);gG.setDate(-1);return gG.getDate()+1===29;}
,__tC:function(gM,gN){var gL=0;var gO=0;if(!gN){gN=1971;}
...
,__tD:function(gS,gQ,gP){var gR;switch(gQ){case 11:gR=gS-1;if(gS!=this.__tA(new Date(gR,gQ,gP))){gR=gS;}
;break;case 0:gR=gS+1;if(gS!=this.__tA(new Date(gR,gQ,gP))){gR=gS;}
;break;default:gR=gS;};return gR;}
,_applyLocale:function(gU,gT){this.__kE=gU===null?this.setLocale(this.__tp):gU;}
,format:function(hl){if(hl==null){return null;}
;if(isNaN(hl.getTime())){if(qx.core.Environment.get(ga)){qx.log.Logger.error(L);}
;return null;}
;if(this.__tr){hl=new Date(hl.getUTCFullYear(),hl.getUTCMonth(),hl.getUTCDate(),hl.getUTCHours(),hl.getUTCMinutes(),hl.getUTCSeconds(),hl.getUTCMilliseconds());}
;var gX=this.__kE;var hd=hl.getFullYear();var gY=hl.getMonth();var hc=hl.getDate();var hg=hl.getDay();var hj=hl.getHours();var hq=hl.getMinutes();var hh=hl.getSeconds();var ho=hl.getMilliseconds();var hf=hl.getTimezoneOffset();var hk=hf>0?1:-1;var gV=Math.floor(Math.abs(hf)/60);var hn=Math.abs(hf)%60;this.__tE();var hp=gh;for(var i=0;i<this.__tu.length;i++ ){var he=this.__tu[i];if(he.type==fz){hp+=he.text;}
else {var gW=he.character;var hi=he.size;var hr=dW;switch(gW){case z:if(hi==2){hr=this.__tv(hd%100,2);}
else {var ha=Math.abs(hd);hr=ha+gh;if(hi>hr.length){for(var j=hr.length;j<hi;j++ ){hr=gg+hr;}
;}
;if(hd<0){hr=a+hr;}
;}
;break;case eC:hr=this.__tA(hl)+gh;var ha=hr.replace(eJ,fL);if(hi>hr.length){for(var j=ha.length;j<hi;j++ ){ha=gg+ha;}
;}
;hr=hr.indexOf(a)!=-1?a+ha:ha;break;case P:if(hi>=1&&hi<=3){hr=hd>0?dU:eY;}
else if(hi==4){hr=hd>0?O:gc;}
else if(hi==5){hr=hd>0?eI:fS;}
;break;case fo:if(hi==1||hi==2){hr=this.__tv(parseInt(gY/4)+1,hi);}
;if(hi==3){hr=fo+(parseInt(gY/4)+1);}
;break;case K:if(hi==1||hi==2){hr=this.__tv(parseInt(gY/4)+1,hi);}
;if(hi==3){hr=fo+(parseInt(gY/4)+1);}
;break;case fe:hr=this.__tv(this.__tw(hl),hi);break;case fI:hr=this.__tv(hc,hi);break;case dX:hr=this.__tv(this.__ty(hl),hi);break;case c:hr=this.__tz(hl);break;case eg:if(hi>=1&&hi<=3){hr=qx.locale.Date.getDayName(v,hg,gX,ed,true);}
else if(hi==4){hr=qx.locale.Date.getDayName(ev,hg,gX,ed,true);}
else if(hi==5){hr=qx.locale.Date.getDayName(fX,hg,gX,ed,true);}
;break;case fj:var hb=qx.locale.Date.getWeekStart(gX);var hm=1+((hg-hb>=0)?(hg-hb):7+(hg-hb));if(hi>=1&&hi<=2){hr=this.__tv(hm,hi);}
...
;break;case dK:if(hi==1||hi==2){hr=this.__tv(gY+1,hi);}
...
;break;case g:if(hi==1||hi==2){hr=this.__tv(gY+1,hi);}
...
;break;case E:hr=(hj<12)?qx.locale.Date.getAmMarker(gX):qx.locale.Date.getPmMarker(gX);break;case gi:hr=this.__tv(hj,hi);break;case fp:hr=this.__tv((hj==0)?24:hj,hi);break;case fy:hr=this.__tv(hj%12,hi);break;case eF:hr=this.__tv(((hj%12)==0)?12:(hj%12),hi);break;case fU:hr=this.__tv(hq,hi);break;case eH:hr=this.__tv(hh,hi);break;case fa:hr=this.__tv(ho,3);if(hi<hr.length){hr=hr.substr(0,hi);}
else {while(hi>hr.length){hr+=gg;}
;}
;break;case eS:if(hi>=1&&hi<=4){hr=T+((hk>0)?a:W)+this.__tv(Math.abs(gV),2)+y+this.__tv(hn,2);}
;break;case gb:if(hi>=1&&hi<=3){hr=((hk>0)?a:W)+this.__tv(Math.abs(gV),2)+this.__tv(hn,2);}
else {hr=T+((hk>0)?a:W)+this.__tv(Math.abs(gV),2)+y+this.__tv(hn,2);}
;break;};hp+=hr;}
;}
;return hp;}
,parse:function(hy){this.__tF();var hB=this.__ts.regex.exec(hy);if(hB==null){throw new Error(n+hy+fK+this.__tq);}
;var hs={era:1,year:1970,quarter:1,month:0,day:1,dayOfYear:1,hour:0,ispm:false,weekDay:4,weekYear:1970,weekOfMonth:1,weekOfYear:1,min:0,sec:0,ms:0};var hw=1;var ht=false;var hu=false;for(var i=0;i<this.__ts.usedRules.length;i++ ){var hv=this.__ts.usedRules[i];var hx=hB[hw];if(hv.field!=null){hs[hv.field]=parseInt(hx,10);}
else {hv.manipulator(hs,hx,hv.pattern);}
;if(hv.pattern==q){var hz=false;for(var k=0;k<this.__ts.usedRules.length;k++ ){if(this.__ts.usedRules[k].pattern==fv){hz=true;break;}
;}
;if(!hz){ht=true;}
;}
;if(hv.pattern.indexOf(ff)!=-1){var hD=false;for(var k=0;k<this.__ts.usedRules.length;k++ ){if(this.__ts.usedRules[k].pattern.indexOf(fc)!=-1){hD=true;break;}
;}
;if(!hD){hu=true;}
;}
;hw+=(hv.groups==null)?1:hv.groups;}
;if(ht){hs.year=this.__tD(hs.weekYear,hs.month,hs.day);}
;if(hu){var hC=this.__tC(hs.dayOfYear,hs.year);hs.month=hC.month;hs.day=hC.day;}
;if(hs.era<0&&(hs.year*hs.era<0)){hs.year=hs.year*hs.era;}
;var hA=new Date(hs.year,hs.month,hs.day,(hs.ispm)?(hs.hour+12):hs.hour,hs.min,hs.sec,hs.ms);if(this.__tr){hA=new Date(hA.getUTCFullYear(),hA.getUTCMonth(),hA.getUTCDate(),hA.getUTCHours(),hA.getUTCMinutes(),hA.getUTCSeconds(),hA.getUTCMilliseconds());}
;if(hs.month!=hA.getMonth()||hs.year!=hA.getFullYear()){throw new Error(N+hy+fi);}
;return hA;}
,__tE:function(){if(this.__tu!=null){return;}
;this.__tu=[];var hI;var hG=0;var hK=gh;var hE=this.__tq;var hH=eX;var i=0;while(i<hE.length){var hJ=hE.charAt(i);switch(hH){case A:if(hJ==fV){if(i+1>=hE.length){i++ ;break;}
;var hF=hE.charAt(i+1);if(hF==fV){hK+=hJ;i++ ;}
else {i++ ;hH=fY;}
;}
else {hK+=hJ;i++ ;}
;break;case fE:if(hJ==hI){hG++ ;i++ ;}
else {this.__tu.push({type:fE,character:hI,size:hG});hI=null;hG=0;hH=eX;}
;break;default:if((hJ>=E&&hJ<=eS)||(hJ>=eI&&hJ<=gb)){hI=hJ;hH=fE;}
else if(hJ==fV){if(i+1>=hE.length){hK+=hJ;i++ ;break;}
;var hF=hE.charAt(i+1);if(hF==fV){hK+=hJ;i++ ;}
;i++ ;hH=A;}
else {hH=eX;}
;if(hH!=eX){if(hK.length>0){this.__tu.push({type:fz,text:hK});hK=gh;}
;}
else {hK+=hJ;i++ ;}
;break;};}
;if(hI!=null){this.__tu.push({type:fE,character:hI,size:hG});}
else if(hK.length>0){this.__tu.push({type:fz,text:hK});}
;}
,__tF:function(){if(this.__ts!=null){return;}
;var hO=this.__tq;this.__tH();this.__tE();var hU=[];var hP=x;for(var hM=0;hM<this.__tu.length;hM++ ){var hV=this.__tu[hM];if(hV.type==fz){hP+=qx.lang.String.escapeRegexpChars(hV.text);}
else {var hN=hV.character;var hQ=hV.size;var hS;for(var hW=0;hW<this.__tt.length;hW++ ){var hR=this.__tt[hW];if(this.__tG(hR,hN,hQ)){hS=hR;break;}
;}
...
;this.__ts={regex:hL,"usedRules":hU,pattern:hP};}
,__tG:function(hY,ia,ib){if(ia===z&&hY.pattern===fv){hY.regex=hY.regexFunc(ib);return true;}
else if(ia===eC&&hY.pattern===eu){hY.regex=hY.regexFunc(ib);return true;}
else {return ia==hY.pattern.charAt(0)&&ib==hY.pattern.length;}
;}
,__tH:function(){var ir=qx.util.format.DateFormat;var iu=qx.lang.String;if(this.__tt!=null){return;}
;var is=this.__tt=[];var im=qx.locale.Date.getAmMarker(this.__kE).toString()||ir.AM_MARKER;var iB=qx.locale.Date.getPmMarker(this.__kE).toString()||ir.PM_MARKER;var id=this.__kE;var ij=function(iE,iF){iF=parseInt(iF,10);if(iF>0){if(iF<ir.ASSUME_YEAR_2000_THRESHOLD){iF+=2000;}
...
},destruct:function(){if(this.__to!=null){qx.locale.Manager.getInstance().removeBinding(this.__to);}
;this.__tu=this.__ts=this.__tt=null;}
});}
)();
(function(){var a='format',b="short",c="HH:mm",d='_',e="stand-alone",f="HHmmsszz",g="cldr_date_format_",h="HHmm",j="narrow",k="HHmmss",l="qx.debug",m="This case should never happen.",n="cldr_time_format_",o="sat",p="abbreviated",q="wide",r="HH:mm:ss zz",s="full",t="cldr_pm",u="cldr_day_",v="_",w="cldr_month_",x="long",y="wed",z="medium",A="fri",B="cldr_am",C="tue",D="qx.locale.Date",E="mon",F="cldr_date_time_format_",G="sun",H="format",I="thu",J="HH:mm:ss",K='stand-alone';qx.Class.define(D,{statics:{__tI:qx.locale.Manager.getInstance(),getAmMarker:function(L){return this.__tI.localize(B,[],L);}
,getPmMarker:function(M){return this.__tI.localize(t,[],M);}
...
;var Q=[G,E,C,y,I,A,o];var R=[];for(var i=0;i<Q.length;i++ ){var N=u+S+v+length+v+Q[i];R.push(P?this.__tJ(S,S===a?K:a,N,O):this.__tI.localize(N,[],O));}
;return R;}
...
;var W=[G,E,C,y,I,A,o];var T=u+U+v+length+v+W[Y];return X?this.__tJ(U,U===a?K:a,T,V):this.__tI.localize(T,[],V);}
...
;var bd=[];for(var i=0;i<12;i++ ){var ba=w+be+v+length+v+(i+1);bd.push(bc?this.__tJ(be,be===a?K:a,ba,bb):this.__tI.localize(ba,[],bb));}
;return bd;}
...
;var bf=w+bg+v+length+v+(bi+1);return bj?this.__tJ(bg,bg===a?K:a,bf,bh):this.__tI.localize(bf,[],bh);}
,getDateFormat:function(bm,bk){if(qx.core.Environment.get(l)){qx.core.Assert.assertInArray(bm,[b,z,x,s]);}
;var bl=g+bm;return this.__tI.localize(bl,[],bk);}
,getDateTimeFormat:function(br,bp,bn){var bq=F+br;var bo=this.__tI.localize(bq,[],bn);if(bo==bq){bo=bp;}
;return bo;}
,getTimeFormat:function(bv,bs){if(qx.core.Environment.get(l)){qx.core.Assert.assertInArray(bv,[b,z,x,s]);}
;var bu=n+bv;var bt=this.__tI.localize(bu,[],bs);if(bt!=bu){return bt;}
...
else {bK=this.__tI.getTerritory()||this.__tI.getLanguage();}
;return bK.toUpperCase();}
,__tJ:function(bN,bM,bQ,bP){var bO=this.__tI.localize(bQ,[],bP);if(bO==bQ){var bL=bQ.replace(d+bN+d,d+bM+d);return this.__tI.localize(bL,[],bP);}
else {return bO;}
;}
}});}
)();
...
(function(){var a="</div>",b="qx/icon/Oxygen",c="waiting",d="icon/16/actions/go-next.png",e="qx.theme",f="http://resources.qooxdoo.org/images/logo.gif",g="$ ",h="string",j="border: 1px dotted green",k="inactive",l="Hello",m="decoration",n="multi<br>line",o="between",p="<",q="<div style='position:absolute;height:20px;width:",r="px;top:",s="icon/16/actions/go-down.png",t="World",u="icon/16/actions/go-previous.png",v="",w="icon",x="number",y="px'>",z="Classic",A="<i>italic</i>",B="0$",C="qx/icon/Tango",D="<b>bold</b>",E="icon/16/actions/go-up.png",F="qx/decoration/",G="boolean",H="demobrowser.demo.test.Table_CellRenderer",I="bold",J="tooltip",K="Juhu",L="red",M="icon/16/emotes/face-sad.png",N="icon/16/emotes/face-smile.png",O="left",P=">=",Q="active",R="object";qx.Class.define(H,{extend:qx.application.Native,include:[demobrowser.demo.table.MUtil],members:{main:function(){qx.application.Native.prototype.main.call(this);this.setUp();this.testDefaultRenderer(new qx.ui.table.cellrenderer.Default());this.testDefaultRenderer(new qx.ui.table.cellrenderer.Default().set({useAutoAlign:false}));this.testBooleanRenderer();this.testDateRenderer();this.testNumberRenderer();this.testImageRenderer();this.testStringRenderer();this.testPasswordRenderer();this.testHtmlRenderer();this.testConditionalRenderer();this.testDynamicRenderer();this.testReplaceRenderer();}
,setUp:function(){this.tableMock=this.getTableMock();var T=qx.core.Environment.get(e);var S=/\.([a-z]+)$/i.exec(T)[1];qx.util.AliasManager.getInstance().add(m,F+S);if(S==z){qx.util.AliasManager.getInstance().add(w,b);}
else {qx.util.AliasManager.getInstance().add(w,C);}
;}
,__HB:function(X,top,W,Y,U){var V=[q,W,r,top,y];Y.createDataCellHtml(U,V);V.push(a);X.innerHTML+=V.join(v);}
,testDefaultRenderer:function(bc){var bd=100;var ba={value:[null,K,32,new Date()],styleLeft:[0],styleWidth:[bd],styleHeight:[20],style:[v]};var bb=this._getNewTableDiv(bd);var top=0;qx.util.Permutation.permute(ba,function(be){this.__HB(bb,top,bd,bc,be);top+=20;}
,this);}
,testBooleanRenderer:function(){var bi=70;var bh=30;var bj=new qx.ui.table.cellrenderer.Boolean();var bf={value:[null,true,false],styleLeft:[0],styleWidth:[bi],styleHeight:[bh],style:[v]};var bg=this._getNewTableDiv(bi);var top=0;qx.util.Permutation.permute(bf,function(bk){this.__HB(bg,top,bi,bj,bk);top+=bh;}
,this);bj.setIconFalse(M);bj.setIconTrue(N);qx.util.Permutation.permute(bf,function(bl){this.__HB(bg,top,bi,bj,bl);top+=bh;}
,this);}
,testDateRenderer:function(){var bn=180;var bq=new qx.ui.table.cellrenderer.Date();var bm={value:new Date(),styleLeft:[0],styleWidth:[bn],styleHeight:[20],style:[v]};var bp=qx.util.format.DateFormat;var br=[bp.getDateTimeInstance(),bp.getDateInstance()];var bs=this._getNewTableDiv(bn);var top=0;for(var i=0;i<br.length;i++ ){var bo=br[i];bq.setDateFormat(bo);this.__HB(bs,top,bn,bq,bm);top+=20;}
;}
,testNumberRenderer:function(){var bx=new qx.ui.table.cellrenderer.Number();var bw=100;var bt={value:Math.PI,styleLeft:[0],styleWidth:[bw],styleHeight:[20],style:[v]};var bv=qx.util.format.NumberFormat;var by=[new bv().set({maximumFractionDigits:0}),new bv().set({minimumFractionDigits:3,maximumFractionDigits:5,prefix:g})];var bz=this._getNewTableDiv(bw);var top=0;for(var i=0;i<by.length;i++ ){var bu=by[i];bx.setNumberFormat(bu);this.__HB(bz,top,bw,bx,bt);top+=20;}
;}
,testImageRenderer:function(){var bC=new qx.ui.table.cellrenderer.Image();var bD=50;var bA={value:[null,s,E,u,d,f],styleLeft:[0],styleWidth:[bD],styleHeight:[20],style:[v],tooltip:[v,J]};var bB=this._getNewTableDiv(bD);var top=0;qx.util.Permutation.permute(bA,function(bE){this.__HB(bB,top,bD,bC,bE);top+=20;}
,this);}
,testStringRenderer:function(){var bJ=new qx.ui.table.cellrenderer.String();var bI=100;var bH=20;var bF={value:[null,l,t],styleLeft:[0],styleWidth:[bI],styleHeight:[bH],style:[v]};var bG=this._getNewTableDiv(bI);var top=0;qx.util.Permutation.permute(bF,function(bK){this.__HB(bG,top,bI,bJ,bK);top+=20;}
,this);}
,testPasswordRenderer:function(){var bP=new qx.ui.table.cellrenderer.Password();var bO=100;var bN=20;var bL={value:[null,l,t],styleLeft:[0],styleWidth:[bO],styleHeight:[bN],style:[v]};var bM=this._getNewTableDiv(bO);var top=0;qx.util.Permutation.permute(bL,function(bQ){this.__HB(bM,top,bO,bP,bQ);top+=20;}
,this);}
,testHtmlRenderer:function(){var bV=new qx.ui.table.cellrenderer.Html();var bU=100;var bT=50;var bR={value:[null,D,A,n],styleLeft:[0],styleWidth:[bU],styleHeight:[bT],style:[v]};var bS=this._getNewTableDiv(bU);var top=0;qx.util.Permutation.permute(bR,function(bW){this.__HB(bS,top,bU,bV,bW);top+=20;}
,this);}
,testConditionalRenderer:function(){var cc=new qx.ui.table.cellrenderer.Conditional();cc.addNumericCondition(p,0,null,L,null,null);cc.addNumericCondition(P,100,O,null,null,null);cc.addBetweenCondition(o,0,2,null,null,null,I,null);cc.addRegex(B,null,null,I,j);var cb=100;var ca=20;var bX={value:[null,1,2,100,200,-100,0],styleLeft:[0],styleWidth:[cb],table:[this.tableMock],styleHeight:[ca],style:[v]};var bY=this._getNewTableDiv(cb);var top=0;qx.util.Permutation.permute(bX,function(cd){this.__HB(bY,top,cb,cc,cd);top+=20;}
,this);}
,testDynamicRenderer:function(){var ci=new qx.ui.table.cellrenderer.Dynamic();ci.setCellRendererFactoryFunction(function(cj){var cl=cj.value;var ck=qx.ui.table.cellrenderer;switch(typeof cl){case h:if(cl.match(/^icon\//)){return new ck.Image();}
else {return new ck.String();}
;case G:return new ck.Boolean();case x:return new ck.Number();case R:if(cl instanceof Date){return new ck.Date();}
;return new ck.Default();};}
);var ch=100;var cg=20;var ce={value:[null,K,new Date(),30,s,false],styleLeft:[0],styleWidth:[ch],styleHeight:[cg],table:[this.tableMock],style:[v]};var cf=this._getNewTableDiv(ch);var top=0;qx.util.Permutation.permute(ce,function(cm){this.__HB(cf,top,ch,ci,cm);top+=20;}
,this);}
,testReplaceRenderer:function(){var cs=new qx.ui.table.cellrenderer.Replace();cs.setReplaceMap({'1':Q,'2':k,'3':c});cs.addReversedReplaceMap();var cq=100;var cp=20;var cn={value:[1,2,3],styleLeft:[0],styleWidth:[cq],table:[this.tableMock],styleHeight:[cp],style:[v]};var co=this._getNewTableDiv(cq);var top=0;qx.util.Permutation.permute(cn,function(ct){this.__HB(co,top,cq,cs,ct);top+=20;}
,this);}
},destruct:function(){this.tableMock=null;}
});}
)();
(function(){var a="qx.util.Permutation";qx.Class.define(a,{statics:{permute:function(j,d,c){var e=Object.keys(j);var b={};var h=[];for(var i=0;i<e.length;i++ ){h[i]=0;var g=e[i];b[g]=j[g][0];}
;var f=function(k,l){if(k>=e.length){return;}
;var n=e[k];var m=j[n];for(var i=0;i<m.length;i++ ){if(l!==i){h[k]=i;b[n]=m[i];d.call(c||window,b);}
;f(k+1,h[k+1]);}
;}
;f(0,-1);}
}});}
)();
(function(){var a="css.inlineblock",b="repeat",c="px",d="scale",e="_identifyImage is abstract",f="string",g="static",h="'",i="qx.ui.table.cellrenderer.AbstractImage",j="}",k="  text-align:center;",l="scale-x",m="repeat-y",n=".qooxdoo-table-cell-icon {",o="",p="<div></div>",q="top",r="abstract",s=" qooxdoo-table-cell-icon",t="no-repeat",u="repeat-x",v="  padding-top:1px;",w="title='",x="scale-y";qx.Class.define(i,{extend:qx.ui.table.cellrenderer.Abstract,type:r,construct:function(){qx.ui.table.cellrenderer.Abstract.call(this);var y=this.self(arguments);if(!y.stylesheet){y.stylesheet=qx.bom.Stylesheet.createElement(n+k+v+j);}
;}
,properties:{repeat:{check:function(A){var z=[d,l,x,b,u,m,t];return qx.lang.Array.contains(z,A);}
,init:t}},members:{__zr:16,__zs:16,__zt:null,_insetY:2,_identifyImage:function(B){throw new Error(e);}
,_getImageInfos:function(C){var D=this._identifyImage(C);if(D==null||typeof D==f){D={url:D,tooltip:null};}
;if(!D.imageWidth||!D.imageHeight){var E=this.__zu(D.url);D.imageWidth=E.width;D.imageHeight=E.height;}
;D.width=D.imageWidth;D.height=D.imageHeight;return D;}
,__zu:function(H){var I=qx.util.ResourceManager.getInstance();var G=qx.io.ImageLoader;var F,J;if(I.has(H)){F=I.getImageWidth(H);J=I.getImageHeight(H);}
else if(G.isLoaded(H)){F=G.getWidth(H);J=G.getHeight(H);}
else {F=this.__zr;J=this.__zs;}
;return {width:F,height:J};}
,createDataCellHtml:function(K,L){this.__zt=this._getImageInfos(K);return qx.ui.table.cellrenderer.Abstract.prototype.createDataCellHtml.call(this,K,L);}
,_getCellClass:function(M){return qx.ui.table.cellrenderer.Abstract.prototype._getCellClass.call(this)+s;}
,_getContentHtml:function(N){var content=p;if(this.__zt.url){content=qx.bom.element.Decoration.create(this.__zt.url,this.getRepeat(),{width:this.__zt.width+c,height:this.__zt.height+c,display:qx.core.Environment.get(a),verticalAlign:q,position:g});}
;return content;}
,_getCellAttributes:function(O){var P=this.__zt.tooltip;if(P){return w+P+h;}
else {return o;}
;}
},destruct:function(){this.__zt=null;}
});}
)();
(function(){var a="qx.dyntheme",b="_applyIconTrue",c="decoration/table/boolean-true.png",d="qx.ui.table.cellrenderer.Boolean",e=";padding-top:4px;",f="decoration/table/boolean-false.png",g="_applyIconFalse",h="changeTheme",i="String";qx.Class.define(d,{extend:qx.ui.table.cellrenderer.AbstractImage,construct:function(){qx.ui.table.cellrenderer.AbstractImage.call(this);this.__ES=qx.util.AliasManager.getInstance();this.initIconTrue();this.initIconFalse();if(qx.core.Environment.get(a)){qx.theme.manager.Appearance.getInstance().addListener(h,this._onChangeTheme,this);}
;}
,properties:{iconTrue:{check:i,init:c,apply:b},iconFalse:{check:i,init:f,apply:g}},members:{__Hm:null,__Hn:false,__ES:null,_onChangeTheme:qx.core.Environment.select(a,{"true":function(){this._applyIconTrue(this.getIconTrue());this._applyIconFalse(this.getIconFalse());}
,"false":null}),_applyIconTrue:function(j){this.__Hm=this.__ES.resolve(j);}
,_applyIconFalse:function(k){this.__Hn=this.__ES.resolve(k);}
,_insetY:5,_getCellStyle:function(l){return qx.ui.table.cellrenderer.AbstractImage.prototype._getCellStyle.call(this,l)+e;}
,_identifyImage:function(m){var n={imageWidth:11,imageHeight:11};switch(m.value){case true:n.url=this.__Hm;break;case false:n.url=this.__Hn;break;default:n.url=null;break;};return n;}
},destruct:function(){this.__ES=null;if(qx.core.Environment.get(a)){qx.theme.manager.Appearance.getInstance().removeListener(h,this._onChangeTheme,this);}
;}
});}
)();
(function(){var a="regex cannot be null!",b="font-weight",c="==",d="Condition not recognized or value1/value2 is null!",e="Condition not recognized or value is null!",f="",g=">",h="text-align",j="between",k="<",l=";",m='g',n="regex",o=":",p="qx.ui.table.cellrenderer.Conditional",q="!between",r=">=",s="color",t="font-style",u="!=",v="<=";qx.Class.define(p,{extend:qx.ui.table.cellrenderer.Default,construct:function(y,w,x,z){qx.ui.table.cellrenderer.Default.call(this);this.numericAllowed=[c,u,g,k,r,v];this.betweenAllowed=[j,q];this.conditions=[];this.__wa=y||f;this.__wb=w||f;this.__wc=x||f;this.__wd=z||f;}
,members:{__wa:null,__wb:null,__wc:null,__wd:null,__we:function(B,A){if(B[1]!=null){A[h]=B[1];}
;if(B[2]!=null){A[s]=B[2];}
;if(B[3]!=null){A[t]=B[3];}
;if(B[4]!=null){A[b]=B[4];}
;}
,addNumericCondition:function(J,I,G,F,C,E,D){var H=null;if(qx.lang.Array.contains(this.numericAllowed,J)){if(I!=null){H=[J,G,F,C,E,I,D];}
;}
;if(H!=null){this.conditions.push(H);}
else {throw new Error(e);}
;}
,addBetweenCondition:function(R,Q,S,O,N,K,M,L){if(qx.lang.Array.contains(this.betweenAllowed,R)){if(Q!=null&&S!=null){var P=[R,O,N,K,M,Q,S,L];}
;}
;if(P!=null){this.conditions.push(P);}
else {throw new Error(d);}
;}
,addRegex:function(T,Y,X,U,W,V){if(T!=null){var ba=[n,Y,X,U,W,T,V];}
;if(ba!=null){this.conditions.push(ba);}
else {throw new Error(a);}
;}
,_getCellStyle:function(bi){if(!this.conditions.length){return bi.style||f;}
;var bf=bi.table.getTableModel();var i;var be;var bb;var bd={"text-align":this.__wa,"color":this.__wb,"font-style":this.__wc,"font-weight":this.__wd};for(i in this.conditions){be=false;if(qx.lang.Array.contains(this.numericAllowed,this.conditions[i][0])){if(this.conditions[i][6]==null){bb=bi.value;}
...
;break;case g:if(bb>this.conditions[i][5]){be=true;}
;break;case k:if(bb<this.conditions[i][5]){be=true;}
;break;case r:if(bb>=this.conditions[i][5]){be=true;}
;break;case v:if(bb<=this.conditions[i][5]){be=true;}
;break;};}
else if(qx.lang.Array.contains(this.betweenAllowed,this.conditions[i][0])){if(this.conditions[i][7]==null){bb=bi.value;}
else {bb=bf.getValueById(this.conditions[i][7],bi.row);}
;switch(this.conditions[i][0]){case j:if(bb>=this.conditions[i][5]&&bb<=this.conditions[i][6]){be=true;}
;break;case q:if(bb<this.conditions[i][5]||bb>this.conditions[i][6]){be=true;}
;break;};}
...
;if(be==true){this.__we(this.conditions[i],bd);}
;}
...
(function(){var a="qx.util.format.DateFormat",b="qooxdoo-table-cell",c="qx.ui.table.cellrenderer.Date",d="";qx.Class.define(c,{extend:qx.ui.table.cellrenderer.Conditional,properties:{dateFormat:{check:a,init:null,nullable:true}},members:{_getContentHtml:function(e){var f=this.getDateFormat();if(f){if(e.value){return qx.bom.String.escape(f.format(e.value));}
else {return d;}
;}
...
(function(){var a="qooxdoo-table-cell qooxdoo-table-cell-right",b="0",c="qx.util.format.NumberFormat",d="qx.ui.table.cellrenderer.Number",e="";qx.Class.define(d,{extend:qx.ui.table.cellrenderer.Conditional,properties:{numberFormat:{check:c,init:null,nullable:true}},members:{_getContentHtml:function(f){var g=this.getNumberFormat();if(g){if(f.value||f.value==0){return g.format(f.value);}
else {return e;}
;}
else {return f.value==0?b:(f.value||e);}
;}
,_getCellClass:function(h){return a;}
}});}
)();
(function(){var a="qx.ui.table.cellrenderer.Image",b="";qx.Class.define(a,{extend:qx.ui.table.cellrenderer.AbstractImage,construct:function(c,d){qx.ui.table.cellrenderer.AbstractImage.call(this);if(c){this.__yC=c;}
;if(d){this.__yD=d;}
;this.__vS=qx.util.AliasManager.getInstance();}
,members:{__vS:null,__yD:16,__yC:16,_identifyImage:function(e){var f={imageWidth:this.__yC,imageHeight:this.__yD};if(e.value==b){f.url=null;}
else {f.url=this.__vS.resolve(e.value);}
;f.tooltip=e.tooltip;return f;}
},destruct:function(){this.__vS=null;}
});}
)();
(function(){var a="qx.ui.table.cellrenderer.String",b="qooxdoo-table-cell",c="";qx.Class.define(a,{extend:qx.ui.table.cellrenderer.Conditional,members:{_getContentHtml:function(d){return qx.bom.String.escape(d.value||c);}
,_getCellClass:function(e){return b;}
}});}
)();
(function(){var a="",b="qx.ui.table.cellrenderer.Password",c="*";qx.Class.define(b,{extend:qx.ui.table.cellrenderer.Default,members:{_getContentHtml:function(d){var e=d.value;if(e===null){e=a;}
;d.value=e.replace(/./g,c);return qx.bom.String.escape(this._formatValue(d));}
}});}
)();
(function(){var a="qooxdoo-table-cell",b="qx.ui.table.cellrenderer.Html",c="";qx.Class.define(b,{extend:qx.ui.table.cellrenderer.Conditional,members:{_getContentHtml:function(d){return (d.value||c);}
,_getCellClass:function(e){return a;}
}});}
)();
(function(){var a="Function",b="No function provided! Aborting.",c="qx.ui.table.cellrenderer.Dynamic";qx.Class.define(c,{extend:qx.ui.table.cellrenderer.Default,construct:function(d){qx.ui.table.cellrenderer.Default.call(this);if(d){this.setCellRendererFactoryFunction(d);}
;}
,properties:{cellRendererFactoryFunction:{check:a,nullable:true,init:null}},members:{createDataCellHtml:function(e,h){var g=this.getCellRendererFactoryFunction();if(!g){throw new Error(b);}
;var f=g(e);return f.createDataCellHtml(e,h);}
}});}
)();
(function(){var a="Function",b="undefined",c="qx.ui.table.cellrenderer.Replace",d="Object";qx.Class.define(c,{extend:qx.ui.table.cellrenderer.Default,properties:{replaceMap:{check:d,nullable:true,init:null},replaceFunction:{check:a,nullable:true,init:null}},members:{_getContentHtml:function(g){var h=g.value;var e=this.getReplaceMap();var f=this.getReplaceFunction();var i;if(e){i=e[h];if(typeof i!=b){g.value=i;return qx.bom.String.escape(this._formatValue(g));}
;}
;if(f){g.value=f(h);}
;return qx.bom.String.escape(this._formatValue(g));}
,addReversedReplaceMap:function(){var j=this.getReplaceMap();for(var k in j){var l=j[k];j[l]=k;}
;return true;}
}});}
)();
