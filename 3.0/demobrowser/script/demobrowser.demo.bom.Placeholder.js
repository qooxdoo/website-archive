...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.bom.Placeholder","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Placeholder.770e8b21a4b2.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="getPreviousSiblings",b="getSiblings",c="qx.module.Traversing",d="getNextSiblings";qx.Bootstrap.define(c,{statics:{add:function(e){if(e instanceof qxWeb){e=e[0];}
;if(qx.module.Traversing.isElement(e)||qx.module.Traversing.isDocument(e)||qx.module.Traversing.isWindow(e)){this.push(e);}
;return this;}
...
;n=n.concat(m);}
;return qxWeb.$init(n);}
,getAncestors:function(p){return this.__nt(null,p);}
,getAncestorsUntil:function(r,q){return this.__nt(r,q);}
,__nt:function(t,u){var s=[];for(var i=0;i<this.length;i++ ){var parent=qx.dom.Element.getParentElement(this[i]);while(parent){var v=[parent];if(t&&qx.bom.Selector.matches(t,v).length>0){break;}
...
;return qxWeb.$init(C);}
,getContents:function(){var E=[];this._forEachElement(function(F){E=E.concat(qx.lang.Array.fromCollection(F.childNodes));}
);return qxWeb.$init(E);}
,is:function(G){if(qx.lang.Type.isFunction(G)){return this.filter(G).length>0;}
;return !!G&&qx.bom.Selector.matches(G,this).length>0;}
,eq:function(H){return this.slice(H,+H+1);}
,getFirst:function(){return this.slice(0,1);}
,getLast:function(){return this.slice(this.length-1);}
,has:function(J){var I=[];this._forEachElement(function(K,L){var M=qx.bom.Selector.matches(J,this.eq(L).getContents());if(M.length>0){I.push(K);}
;}
);return qxWeb.$init(I);}
,getNext:function(O){var N=this.map(qx.dom.Hierarchy.getNextElementSibling,qx.dom.Hierarchy);if(O){N=qx.bom.Selector.matches(O,N);}
;return N;}
,getNextAll:function(Q){var P=qx.module.Traversing.__nu(this,d,Q);return qxWeb.$init(P);}
,getNextUntil:function(S){var R=[];this.forEach(function(V,T){var U=qx.dom.Hierarchy.getNextSiblings(V);for(var i=0,l=U.length;i<l;i++ ){if(qx.bom.Selector.matches(S,[U[i]]).length>0){break;}
;R.push(U[i]);}
;}
);return qxWeb.$init(R);}
,getPrev:function(X){var W=this.map(qx.dom.Hierarchy.getPreviousElementSibling,qx.dom.Hierarchy);if(X){W=qx.bom.Selector.matches(X,W);}
;return W;}
,getPrevAll:function(ba){var Y=qx.module.Traversing.__nu(this,a,ba);return qxWeb.$init(Y);}
,getPrevUntil:function(bc){var bb=[];this.forEach(function(bf,bd){var be=qx.dom.Hierarchy.getPreviousSiblings(bf);for(var i=0,l=be.length;i<l;i++ ){if(qx.bom.Selector.matches(bc,[be[i]]).length>0){break;}
;bb.push(be[i]);}
;}
);return qxWeb.$init(bb);}
,getSiblings:function(bh){var bg=qx.module.Traversing.__nu(this,b,bh);return qxWeb.$init(bg);}
,not:function(bj){if(qx.lang.Type.isFunction(bj)){return this.filter(function(bk,bm,bl){return !bj(bk,bm,bl);}
);}
;var bi=qx.bom.Selector.matches(bj,this);return this.filter(function(bn){return bi.indexOf(bn)===-1;}
);}
...
,isElement:function(bo){return qx.dom.Node.isElement(bo);}
,isNode:function(bp){return qx.dom.Node.isNode(bp);}
,isNodeName:function(bq,br){return qx.dom.Node.isNodeName(bq,br);}
,isDocument:function(bs){return qx.dom.Node.isDocument(bs);}
,getWindow:function(bt){return qx.dom.Node.getWindow(bt);}
,isTextNode:function(bu){return qx.dom.Node.isText(bu);}
,isWindow:function(bv){return qx.dom.Node.isWindow(bv);}
,getDocument:function(bw){return qx.dom.Node.getDocument(bw);}
,getNodeName:function(bx){return qx.dom.Node.getName(bx);}
,getNodeText:function(by){return qx.dom.Node.getText(by);}
,isBlockNode:function(bz){return qx.dom.Node.isBlockNode(bz);}
,__nu:function(bC,bF,bD){var bA=[];var bB=qx.dom.Hierarchy;for(var i=0,l=bC.length;i<l;i++ ){bA.push.apply(bA,bB[bF](bC[i]));}
;var bE=qx.lang.Array.unique(bA);if(bD){bE=qx.bom.Selector.matches(bD,bE);}
;return bE;}
},defer:function(bG){qxWeb.$attach({"add":bG.add,"getChildren":bG.getChildren,"forEach":bG.forEach,"getParents":bG.getParents,"getAncestors":bG.getAncestors,"getAncestorsUntil":bG.getAncestorsUntil,"__nt":bG.__nt,"getClosest":bG.getClosest,"find":bG.find,"getContents":bG.getContents,"is":bG.is,"eq":bG.eq,"getFirst":bG.getFirst,"getLast":bG.getLast,"has":bG.has,"getNext":bG.getNext,"getNextAll":bG.getNextAll,"getNextUntil":bG.getNextUntil,"getPrev":bG.getPrev,"getPrevAll":bG.getPrevAll,"getPrevUntil":bG.getPrevUntil,"getSiblings":bG.getSiblings,"not":bG.not,"getOffsetParent":bG.getOffsetParent,"isRendered":bG.isRendered});qxWeb.$attachStatic({"isElement":bG.isElement,"isNode":bG.isNode,"isNodeName":bG.isNodeName,"isDocument":bG.isDocument,"getDocument":bG.getDocument,"getWindow":bG.getWindow,"isWindow":bG.isWindow,"isBlockNode":bG.isBlockNode,"getNodeName":bG.getNodeName,"getNodeText":bG.getNodeText,"isTextNode":bG.isTextNode});}
});}
)();
...
(function(){var a="scrollLeft",b="qx.module.Manipulating",c="ease-in",d="scrollTop";qx.Bootstrap.define(b,{statics:{create:function(e){return qxWeb.$init(qx.bom.Html.clean([e]));}
,clone:function(g){var f=[];for(var i=0;i<this.length;i++ ){if(this[i]&&this[i].nodeType===1){f[i]=this[i].cloneNode(true);}
;}
;if(g===true&&this.copyEventsTo){this.copyEventsTo(f);}
;return qxWeb(f);}
,append:function(k){var h=qx.bom.Html.clean([k]);var n=qxWeb.$init(h);this._forEachElement(function(o,p){for(var j=0,m=n.length;j<m;j++ ){if(p==0){qx.dom.Element.insertEnd(n[j],o);}
else {qx.dom.Element.insertEnd(n.eq(j).clone(true)[0],o);}
;}
;}
);return this;}
,appendTo:function(parent){parent=qx.module.Manipulating.__nv(parent);for(var i=0,l=parent.length;i<l;i++ ){this._forEachElement(function(q,j){if(i==0){qx.dom.Element.insertEnd(this[j],parent[i]);}
else {qx.dom.Element.insertEnd(this.eq(j).clone(true)[0],parent[i]);}
;}
);}
;return this;}
,insertBefore:function(r){r=qx.module.Manipulating.__nv(r);for(var i=0,l=r.length;i<l;i++ ){this._forEachElement(function(s,t){if(i==0){qx.dom.Element.insertBefore(s,r[i]);}
else {qx.dom.Element.insertBefore(this.eq(t).clone(true)[0],r[i]);}
;}
);}
;return this;}
,insertAfter:function(u){u=qx.module.Manipulating.__nv(u);for(var i=0,l=u.length;i<l;i++ ){for(var j=this.length-1;j>=0;j-- ){if(!this[j]||this[j].nodeType!==1){continue;}
;if(i==0){qx.dom.Element.insertAfter(this[j],u[i]);}
else {qx.dom.Element.insertAfter(this.eq(j).clone(true)[0],u[i]);}
;}
;}
;return this;}
,__nv:function(w){if(!qx.lang.Type.isArray(w)){var v=qxWeb(w);w=v.length>0?v:[w];}
;return w.filter(function(x){return (x&&x.nodeType===1);}
);}
,wrap:function(y){y=qx.module.Manipulating.__nw(y);if(y.length==0){return this;}
;this._forEachElement(function(z){var A=y.eq(0).clone(true);qx.dom.Element.insertAfter(A[0],z);var B=qx.module.Manipulating.__nx(A[0]);qx.dom.Element.insertEnd(z,B);}
);return this;}
,__nw:function(E){var C;if(qx.lang.Type.isArray(E)){C=qxWeb(E);}
else {var D=qx.bom.Html.clean([E]);if(D.length>0&&qx.dom.Node.isElement(D[0])){C=qxWeb(D);}
else {C=qxWeb(E);}
;}
;return C;}
,__nx:function(F){if(F.childNodes.length==0){return F;}
;for(var i=0,l=F.childNodes.length;i<l;i++ ){if(F.childNodes[i].nodeType===1){return this.__nx(F.childNodes[i]);}
;}
;return F;}
,remove:function(){this._forEachElement(function(G){qx.dom.Element.remove(G);}
);return this;}
,empty:function(){this._forEachElement(function(H){while(H.firstChild){H.removeChild(H.firstChild);}
;}
);return this;}
,before:function(content){if(!qx.lang.Type.isArray(content)){content=[content];}
;var I=document.createDocumentFragment();qx.bom.Html.clean(content,document,I);this._forEachElement(function(L,J){var K=qx.lang.Array.cast(I.childNodes,Array);for(var i=0,l=K.length;i<l;i++ ){var M;if(J<this.length-1){M=K[i].cloneNode(true);}
else {M=K[i];}
;L.parentNode.insertBefore(M,L);}
;}
,this);return this;}
,after:function(content){if(!qx.lang.Type.isArray(content)){content=[content];}
;var N=document.createDocumentFragment();qx.bom.Html.clean(content,document,N);this._forEachElement(function(R,O){var Q=qx.lang.Array.cast(N.childNodes,Array);for(var i=Q.length-1;i>=0;i-- ){var P;if(O<this.length-1){P=Q[i].cloneNode(true);}
else {P=Q[i];}
;R.parentNode.insertBefore(P,R.nextSibling);}
;}
,this);return this;}
,getScrollLeft:function(){var S=this[0];if(!S){return null;}
;var Node=qx.dom.Node;if(Node.isWindow(S)||Node.isDocument(S)){return qx.bom.Viewport.getScrollLeft();}
;return S.scrollLeft;}
,getScrollTop:function(){var T=this[0];if(!T){return null;}
;var Node=qx.dom.Node;if(Node.isWindow(T)||Node.isDocument(T)){return qx.bom.Viewport.getScrollTop();}
;return T.scrollTop;}
,_animationDescription:{scrollLeft:{duration:700,timing:c,keep:100,keyFrames:{'0':{},'100':{scrollLeft:1}}},scrollTop:{duration:700,timing:c,keep:100,keyFrames:{'0':{},'100':{scrollTop:1}}}},__ny:function(V,W,U){var X=qx.lang.Object.clone(qx.module.Manipulating._animationDescription[V],true);X.keyFrames[100][V]=W;return this.animate(X,U);}
,setScrollLeft:function(ba,bb){var Node=qx.dom.Node;if(bb&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__ny.bind(this,a,ba,bb)();}
;for(var i=0,l=this.length,Y;i<l;i++ ){Y=this[i];if(Node.isElement(Y)){if(!(bb&&qx.bom.element&&qx.bom.element.AnimationJs)){Y.scrollLeft=ba;}
;}
else if(Node.isWindow(Y)){Y.scrollTo(ba,this.getScrollTop(Y));}
else if(Node.isDocument(Y)){Node.getWindow(Y).scrollTo(ba,this.getScrollTop(Y));}
;}
;return this;}
,setScrollTop:function(bd,be){var Node=qx.dom.Node;if(be&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__ny.bind(this,d,bd,be)();}
;for(var i=0,l=this.length,bc;i<l;i++ ){bc=this[i];if(Node.isElement(bc)){if(!(be&&qx.bom.element&&qx.bom.element.AnimationJs)){bc.scrollTop=bd;}
;}
else if(Node.isWindow(bc)){bc.scrollTo(this.getScrollLeft(bc),bd);}
else if(Node.isDocument(bc)){Node.getWindow(bc).scrollTo(this.getScrollLeft(bc),bd);}
;}
;return this;}
,focus:function(){try{this[0].focus();}
catch(bf){}
;return this;}
,blur:function(){this.forEach(function(bg,bh){try{bg.blur();}
catch(bi){}
;}
);return this;}
},defer:function(bj){qxWeb.$attachStatic({"create":bj.create});qxWeb.$attach({"append":bj.append,"appendTo":bj.appendTo,"remove":bj.remove,"empty":bj.empty,"before":bj.before,"insertBefore":bj.insertBefore,"after":bj.after,"insertAfter":bj.insertAfter,"wrap":bj.wrap,"clone":bj.clone,"getScrollLeft":bj.getScrollLeft,"setScrollLeft":bj.setScrollLeft,"getScrollTop":bj.getScrollTop,"setScrollTop":bj.setScrollTop,"focus":bj.focus,"blur":bj.blur});}
});}
)();
...
,__nr:{opt:[1,b,d],leg:[1,a,x],table:[1,w,H],tr:[2,y,q],td:[3,h,e],col:[2,o,s],def:qx.core.Environment.select(C,{"mshtml":[1,z,c],"default":null})},fixEmptyTags:function(N){return N.replace(/(<(\w+)[^>]*?)\/>/g,this.__nq);}
,__ns:function(S,T){var V=T.createElement(g);S=qx.bom.Html.fixEmptyTags(S);var P=S.replace(/^\s+/,v).substring(0,5).toLowerCase();var U,O=this.__nr;if(!P.indexOf(t)){U=O.opt;}
...
,clean:function(X,be,bb){be=be||document;if(typeof be.createElement===I){be=be.ownerDocument||be[0]&&be[0].ownerDocument||document;}
;if(!bb&&X.length===1&&typeof X[0]===k){var bg=/^<(\w+)\s*\/?>$/.exec(X[0]);if(bg){return [be.createElement(bg[1])];}
;}
;var ba,bc=[];for(var i=0,l=X.length;i<l;i++ ){ba=X[i];if(typeof ba===k){ba=this.__ns(ba,be);}
;if(ba.nodeType){bc.push(ba);}
else if(ba instanceof qx.type.BaseArray||(typeof qxWeb!==I&&ba instanceof qxWeb)){bc.push.apply(bc,Array.prototype.slice.call(ba,0));}
else if(ba.toElement){bc.push(ba.toElement());}
else {bc.push.apply(bc,ba);}
;}
;if(bb){var bf=[],bd;for(var i=0;bc[i];i++ ){bd=bc[i];if(bd.nodeType==1&&bd.tagName.toLowerCase()===n&&(!bd.type||bd.type.toLowerCase()===u)){if(bd.parentNode){bd.parentNode.removeChild(bc[i]);}
;bf.push(bd);}
else {if(bd.nodeType===1){var Y=qx.lang.Array.fromCollection(bd.getElementsByTagName(n));bc.splice.apply(bc,[i+1,0].concat(Y));}
;bb.appendChild(bd);}
;}
;return bf;}
;return bc;}
...
;var J=L.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);if(!J){J=qx.module.Placeholder.__Ip(L);}
;var K=L.isRendered();var M=J.isRendered();if(K&&!M){L.before(J);}
else if(!K&&M){J.remove();return this;}
;qx.module.Placeholder.__Io(L);}
;}
;return this;}
,__Io:function(T){var U=T.getAttribute(o);var Q=T.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);var S=T.getStyle(B);var P=parseInt(T.getStyle(p))+2*parseInt(T.getStyle(m));var R=parseInt(T.getStyle(z))+2*parseInt(T.getStyle(F));Q.setHtml(U).setStyles({display:T.getValue()==t?g:A,zIndex:S==j?1:S+1,textAlign:T.getStyle(n),width:(T.getWidth()-P-4)+d,height:(T.getHeight()-R-4)+d,left:T.getOffset().left+d,top:T.getOffset().top+d,fontFamily:T.getStyle(l),fontStyle:T.getStyle(H),fontVariant:T.getStyle(f),fontWeight:T.getStyle(q),fontSize:T.getStyle(c),paddingTop:(parseInt(T.getStyle(z))+2)+d,paddingRight:(parseInt(T.getStyle(m))+2)+d,paddingBottom:(parseInt(T.getStyle(F))+2)+d,paddingLeft:(parseInt(T.getStyle(p))+2)+d});}
,__Ip:function(V){var W=qxWeb.create(b).setStyles({position:G,color:r,overflow:E,pointerEvents:A});V.setProperty(qx.module.Placeholder.PLACEHOLDER_NAME,W);V.on(w,function(X){var Y=X.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);Y.setStyle(x,X.getValue()==t?g:A);}
...
(function(){var a="monospace",b="cursive",c="input[type=button]",d="bold",e="px",f="sans-serif",g="css.placeholder",h="<input type='text' placeholder='",i="italic",j="#support",k="demobrowser.demo.bom.Placeholder",l="serif",m="disabled",n="'/><br>",o="click",p="normal";qx.Class.define(k,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);q(j).setHtml(q.env.get(g));q(c).setAttribute(m,null).on(o,this.addTextField);q.placeholder.update();}
,addTextField:function(){var s=Date.now();var r=q.create(h+s+n).appendTo(document.body);r.setStyles({width:((s%100)+50)+e,padding:(s%20)+e,fontSize:((s%30)+10)+e,fontWeight:s%2?d:p,fontStyle:s%2?i:p,fontFamily:[a,l,f,b][s%4]}).updatePlaceholder();}
}});}
)();
