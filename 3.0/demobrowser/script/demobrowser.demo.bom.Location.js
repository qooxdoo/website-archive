...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Location.e4f9eabe64ee.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="demobrowser.demo.bom.Location",b="borderBottomWidth",c="list",d='#',e="css.boxmodel",f="px",g="mode",h="borderTopWidth",j="borderLeftWidth",k="a",m="div",n="moveable",o="borderRightWidth",p="border";qx.Class.define(a,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var q=document.getElementById(c).getElementsByTagName(k);for(var i=0,l=q.length;i<l;i++ ){q[i].onclick=demobrowser.demo.bom.Location.jump;}
;}
},statics:{num:function(s,r){return parseInt(qx.bom.element.Style.get(s,r,qx.bom.element.Style.COMPUTED_MODE,false),10);}
,jump:function(event){if(!event){event=window.event;}
;event.returnValue=false;if(event.preventDefault){event.preventDefault();}
;function t(z,y){return parseInt(qx.bom.element.Style.get(z,y,qx.bom.element.Style.COMPUTED_MODE,false),10);}
;var v=this.href;var x=v.substr(v.indexOf(d)+1);var u=document.getElementById(x);var location=qx.bom.element.Location.get(u,document.getElementById(g).value);var w=document.getElementById(n);w.style.left=location.left+f;w.style.top=location.top+f;if(qx.core.Environment.get(e)==p){w.style.width=(location.right-location.left)+f;w.style.height=(location.bottom-location.top)+f;}
else {w.style.width=(location.right-location.left-t(w,j)-t(w,o))+f;w.style.height=(location.bottom-location.top-t(w,h)-t(w,b))+f;}
;return false;}
,randomize:function(){var A=document.body.getElementsByTagName(m);for(var i=0,l=A.length;i<l;i++ ){if(A[i].id&&A[i].id!=n){A[i].style.margin=Math.round(Math.random()*10)+f;A[i].style.padding=Math.round(Math.random()*10)+f;A[i].style.borderWidth=Math.round(Math.random()*10)+f;}
;}
;}
}});}
)();
