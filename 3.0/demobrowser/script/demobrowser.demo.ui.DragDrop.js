...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.DragDrop.0a25ee4cc4de.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
);P.addListener(o,function(e){this.debug(r+e.getRelatedTarget());var W=e.getCurrentAction();var Y=e.getCurrentType();var V;switch(Y){case C:V=this.getSelection();if(W==p){var X=[];for(var i=0,l=V.length;i<l;i++ ){X[i]=V[i].clone();}
;V=X;}
...
;}
;e.addData(Y,V);}
);var L=new qx.ui.basic.Label(v);I.add(L,{left:140,top:20});var J=new qx.ui.form.List;J.setDroppable(true);J.setSelectionMode(t);I.add(J,{left:140,top:40});J.addListener(f,function(e){this.debug(E+e.getRelatedTarget());var ba=e.getData(C);for(var i=0,l=ba.length;i<l;i++ ){this.add(ba[i]);}
;}
);J.addListener(B,function(e){if(!e.supportsType(C)){e.preventDefault();}
;}
);var M=new qx.ui.basic.Label(h);I.add(M,{left:260,top:20});var T=new qx.ui.form.List;T.setDroppable(true);I.add(T,{left:260,top:40});T.addListener(f,function(e){this.debug(E+e.getRelatedTarget());var bb=e.getData(C);for(var i=0,l=bb.length;i<l;i++ ){this.add(bb[i]);}
;}
...
;}
);var L=new qx.ui.basic.Label(c);I.add(L,{left:380,top:20});var Q=new qx.ui.form.TextArea;Q.setDroppable(true);Q.setHeight(100);I.add(Q,{left:380,top:40});Q.addListener(f,function(e){this.debug(E+e.getRelatedTarget());this.setValue(e.getData(j).replace(/,/g,z));}
);Q.addListener(B,function(e){if(!e.supportsType(j)){e.preventDefault();}
;}
);var N=new qx.ui.basic.Label(k);I.add(N,{left:600,top:20});var G=this.__JB=new qx.ui.form.List();G.setDraggable(true);G.setDroppable(true);G.setSelectionMode(t);I.add(G,{left:600,top:40});for(var i=0;i<20;i++ ){G.add(new qx.ui.form.ListItem(D+i,g));}
...
);G.addListener(b,function(e){var bd=e.getOriginalTarget();if(bd instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__Rw=bd;}
...
;}
);G.addListener(f,function(e){this.__Rx(e.getOriginalTarget());}
,this);H.addListener(f,function(e){this.__Rx(this.__Rw);}
,this);}
,__Rx:function(bf){if(bf.classname!=m){return;}
;var be=this.__JB.getSortedSelection();for(var i=0,l=be.length;i<l;i++ ){this.__JB.addBefore(be[i],bf);this.__JB.addToSelection(be[i]);}
;}
