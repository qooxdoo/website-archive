...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.progressive.ProgressiveLoader.615544d99e8d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="demobrowser.demo.progressive.ProgressiveLoader",b="black",c="Red",d="http://resources.qooxdoo.org/images/logo.gif",f="green",g="100%",h="yellow",i="col ",j="gray",k="blue",l="Green",m="Yellow",n="Blue",p="renderStart",q=", ",r="purple",s="red",t="func",u="renderEnd",v="white",w="row ",x="Purple";qx.Class.define(a,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var F=new qx.ui.progressive.renderer.table.Widths(1);F.setWidth(0,g);var z=new qx.ui.progressive.headfoot.Progress(F);var A=new qx.ui.progressive.structure.Default(null,z);var y=new qx.ui.progressive.Progressive(A);y.setFlushWidgetQueueAfterBatch(true);var D=function(L){var M={renderer:t,data:L};return M;}
;var J=new qx.ui.progressive.model.Default();var G=new qx.ui.progressive.renderer.FunctionCaller();y.addRenderer(t,G);var E=d;var H=new qx.ui.basic.Image(E,g,g);y.add(H);y.set({height:100,width:272,zIndex:99999,backgroundColor:j,opacity:0.86,batchSize:10});this.getRoot().add(y,{top:-1000,left:-1000});var B=[{text:c,background:s,color:v},{text:l,background:f,color:v},{text:n,background:k,color:v},{text:x,background:r,color:v},{text:m,background:h,color:b}];this.context={document:this.getRoot(),pages:B};y.addListener(p,function(e){var R=e.getData().state;var N=e.getData().initial;var Q=this.getRoot().getBounds();var O=e.getData().state.getProgressive();var P=O.getBounds();var S=Math.floor((Q.width-P.width)/2);var top=Math.floor((Q.height-P.height)/2);O.setLayoutProperties({left:S<0?0:S,top:top<0?0:top});R.getUserData().context=this.context;R.getUserData().initialNum=N;}
,this);y.addListener(u,function(e){qx.event.Timer.once(function(){this.getLayoutParent().remove(this);this.dispose();}
,this,0);}
);J.addElement(D(function(T){var U=T.context;U.tabview=new qx.ui.tabview.TabView();U.tabview.set({});U.document.add(U.tabview,{left:20,right:20,top:20,bottom:20});U.page=0;U.currentPage=null;}
));for(var C=0;C<B.length;C++ ){J.addElement(D(function(V){var X=V.context;var W=X.pages[X.page];X.currentPage=new qx.ui.tabview.Page(W.text);X.currentPage.setLayout(new qx.ui.layout.VBox(4));X.tabview.add(X.currentPage);X.row=0;X.col=0;if(X.page==0){X.firstPage=X.currentPage;}
;}
));for(var K=0;K<20;K++ ){J.addElement(D(function(Y){var ba=Y.context;ba.hBox=new qx.ui.container.Composite(new qx.ui.layout.HBox(4));ba.currentPage.add(ba.hBox);ba.tabview.setSelection([ba.currentPage]);ba.col=0;}
));for(var I=0;I<10;I++ ){J.addElement(D(function(bb){var bd=bb.context;var bc=bd.pages[bd.page];var o=new qx.ui.basic.Atom(w+bd.row+q+i+bd.col);o.set({width:90,backgroundColor:bc.background,textColor:bc.color});bd.hBox.add(o);}
));J.addElement(D(function(be){ ++be.context.col;}
));}
;J.addElement(D(function(bf){ ++bf.context.row;}
));}
;J.addElement(D(function(bg){ ++bg.context.page;}
));}
;J.addElement(D(function(bh){bh.context.tabview.setSelection([bh.context.firstPage]);}
));y.setDataModel(J);y.render();}
}});}
)();
(function(){var a="Column number out of range",b="Unrecognized key: ",c="qx.ui.progressive.renderer.table.Widths",d="minWidth",e="width",f="maxWidth";qx.Class.define(c,{extend:qx.core.Object,construct:function(g){qx.core.Object.call(this);this.__Ht=[];for(var i=0;i<g;i++ ){this.__Ht[i]=new qx.ui.core.ColumnData();}
;}
,members:{__Ht:null,getData:function(){return this.__Ht;}
...
;}
,setWidth:function(l,m){if(l>this.__Ht.length-1||l<0){throw new Error(a);}
;this.__Ht[l].setColumnWidth(m);}
,setMinWidth:function(n,o){if(n>this.__Ht.length-1||n<0){throw new Error(a);}
;this.__Ht[n].setMinWidth(o);}
,setMaxWidth:function(p,q){if(p>this.__Ht.length-1||p<0){throw new Error(a);}
;this.__Ht[p].setMaxWidth(q);}
}});}
)();
...
,members:{__ui:null,renderLayout:function(g,top,f,h){this.__ui=f;}
,getComputedWidth:function(){return this.__ui;}
...
,members:{__ED:null,join:function(c){this.__ED=c;}
},destruct:function(){this.__ED=null;}
});}
)();
(function(){var a="progressive-progressbar-indicator-undone",b="progress",c="%",d="progressive-progressbar-indicator-done",f="__GK",g="progressive-progressbar-percent-text",h="renderStart",i="progressive-progressbar-background",j="0%",k="__GJ",l="qx.ui.progressive.headfoot.Progress",m="progressive-progressbar-percent-background",n="renderEnd";qx.Class.define(l,{extend:qx.ui.progressive.headfoot.Abstract,construct:function(o,q){qx.ui.progressive.headfoot.Abstract.call(this);this.setHeight(16);this.setPadding(0);this.__FJ={};this.__FN();this.set({backgroundColor:this.__FJ.background});this.__GJ=new qx.ui.core.Widget();this.__GJ.set({width:0,backgroundColor:this.__FJ.indicatorDone});this.add(this.__GJ);var p=new qx.ui.core.Widget();p.set({backgroundColor:this.__FJ.indicatorUndone});this.add(p,{flex:1});this.__GK=new qx.ui.basic.Atom(j);this.__GK.set({width:100,backgroundColor:this.__FJ.percentBackground,textColor:this.__FJ.percentText});this.add(this.__GK);this.exclude();}
,members:{__GL:null,__FJ:null,__GJ:null,__GK:null,_onChangeTheme:function(){qx.ui.progressive.headfoot.Abstract.prototype._onChangeTheme.call(this);this.__FN();}
,__FN:function(){var r=qx.theme.manager.Color.getInstance();this.__FJ.background=r.resolve(i);this.__FJ.indicatorDone=r.resolve(d);this.__FJ.indicatorUndone=r.resolve(a);this.__FJ.percentBackground=r.resolve(m);this.__FJ.percentText=r.resolve(g);}
,join:function(s){qx.ui.progressive.headfoot.Abstract.prototype.join.call(this,s);s.addListener(h,function(e){this.__GL=e.getData().initial;this.show();}
,this);s.addListener(b,function(e){var u=1.0-(e.getData().remaining/this.__GL);var w=this.getBounds();if(w){var v=Math.floor((w.width-this.__GK.getBounds().width)*u);var t=Math.floor(u*100)+c;if(!isNaN(v)){this.__GJ.setMinWidth(v);this.__GK.setLabel(t);}
;}
;}
,this);s.addListener(n,function(e){this.exclude();}
,this);}
},destruct:function(){this.__FJ=null;this._disposeObjects(k,f);}
});}
)();
(function(){var a="applyStructure() is abstract",b="abstract",c="qx.ui.progressive.structure.Abstract",d="overflowY",e="auto";qx.Class.define(c,{type:b,extend:qx.core.Object,construct:function(f){qx.core.Object.call(this);if(!f){this.__Ez=new qx.ui.core.Widget();this.__xl=this.__Ez;}
else {this.__Ez=null;this.__xl=f;}
;this.__xl.getContentElement().setStyle(d,e);}
,members:{__Ez:null,__xl:null,applyStructure:function(g){throw new Error(a);}
,getPane:function(){return this.__xl;}
},destruct:function(){if(this.__Ez){this.__Ez.dispose();}
;this.__Ez=this.__xl=null;}
});}
)();
(function(){var a="qx.ui.progressive.structure.Default";qx.Class.define(a,{extend:qx.ui.progressive.structure.Abstract,construct:function(b,d,c){qx.ui.progressive.structure.Abstract.call(this,c);if(!b){this.__EA=new qx.ui.progressive.headfoot.Null();this.__nz=this.__EA;}
else {this.__EA=null;this.__nz=b;}
;if(!d){this.__EB=new qx.ui.progressive.headfoot.Null();this.__EC=this.__EB;}
else {this.__EB=null;this.__EC=d;}
;}
,members:{__nz:null,__EC:null,__EA:null,__EB:null,applyStructure:function(e){this.__nz.join(e);this.__EC.join(e);e.add(this.__nz);e.add(this.getPane(),{flex:1});e.add(this.__EC);}
,getHeader:function(){return this.__nz;}
,getFooter:function(){return this.__EC;}
},destruct:function(){if(this.__EA){this.__EA.dispose();this.__EA=null;}
;if(this.__EB){this.__EB.dispose();this.__EB=null;}
;this.__nz=this.__EC=null;}
});}
)();
...
;this.__FP=r;r.applyStructure(this);this.__FQ=false;this.__FR=false;this.__FS=0;}
,events:{"renderStart":q,"renderEnd":q,"progress":q,"progressDetail":q},properties:{dataModel:{check:f,apply:b},batchSize:{check:o,init:20},flushWidgetQueueAfterBatch:{check:c,init:false},interElementTimeout:{check:o,init:0}},members:{__FO:null,__FR:null,__FT:null,__FS:null,__FQ:null,__FP:null,getStructure:function(){return this.__FP;}
,addRenderer:function(name,s){this.__FO[name]=s;s.join(this,name);}
,removeRenderer:function(name){if(!this.__FO[name]){throw new Error(l+name);}
;delete this.__FO[name];}
,render:function(){if(this.__FR){return;}
;this.__FR=true;var t=new qx.ui.progressive.State({progressive:this,model:this.getDataModel(),pane:this.__FP.getPane(),batchSize:this.getBatchSize(),rendererData:this.__FV(),userData:{}});this.__FT=new Date();if(this.__FQ){this.__FS=t.getModel().getElementCount();this.fireDataEvent(g,{state:t,initial:this.__FS});this.__FU(t);}
else {qx.event.Timer.once(function(){this.__FS=t.getModel().getElementCount();this.fireDataEvent(g,{state:t,initial:this.__FS});this.__FU(t);this.__FQ=true;}
,this,10);}
;}
,_applyDataModel:function(v,u){if(u){u.removeListener(a,this.__FW,this);u.dispose();}
;v.addListener(a,this.__FW,this);}
,__FU:function(x){var w;var z;var y;for(var i=x.getBatchSize();i>0;i-- ){w=x.getModel().getNextElement();if(!w){this.debug(d+(new Date()-this.__FT)+h);this.__FR=false;this.fireDataEvent(p,x);x.dispose();return;}
;z=w.element;y=this.__FO[z.renderer];y.render(x,z);this.fireDataEvent(n,{initial:this.__FS,remaining:w.remaining,element:z});}
;this.fireDataEvent(j,{initial:this.__FS,remaining:w.remaining});if(this.getFlushWidgetQueueAfterBatch()){qx.ui.core.queue.Manager.flush();}
;qx.event.Timer.once(function(){this.__FU(x);}
,this,this.getInterElementTimeout());}
,__FV:function(){var A={};for(var name in this.__FO){A[name]={};}
;return A;}
,__FW:function(e){this.__FS=e.getData();this.render();}
...
(function(){var a="dataAvailable",b="qx.ui.progressive.model.Default";qx.Class.define(b,{extend:qx.ui.progressive.model.Abstract,construct:function(){qx.ui.progressive.model.Abstract.call(this);this.__pe=[];}
,members:{__pe:null,addElements:function(c){this.__pe=this.__pe.concat(c);this.fireDataEvent(a,this.__pe.length);}
,addElement:function(d){this.__pe.push(d);this.fireDataEvent(a,this.__pe.length);}
,getElementCount:function(){return this.__pe.length;}
,getNextElement:function(){if(this.__pe.length>0){return ({element:this.__pe.shift(),remaining:this.__pe.length});}
;return null;}
