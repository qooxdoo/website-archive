...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.Button","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Button.3e23a19a5834.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="Off",b="Button A",c="0",d="#AAA",f="icon/22/actions/list-add.png",g="Button B",h="demobrowser.demo.widget.Button",i="icon/22/apps/internet-web-browser.png",j="Hover",k="icon/22/apps/media-video-player.png",l="icon/22/apps/internet-mail.png",m="changeValue",n="white",o="On",p="main",q="Checked: ",r="execute",s="Toggle Button";qx.Class.define(h,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var t=new qx.ui.layout.HBox();t.setSpacing(10);var u=new qx.ui.container.Composite(t);u.setPadding(20);this.getRoot().add(u,{left:0,top:0});this.addNormalButtons(u);this.addToggleButton(u);this.addToggleButtonGroup(u);this.addRepeatButton(u);this.addHoverButton(u);}
,addNormalButtons:function(v){var w=new qx.ui.form.Button(b,k);v.add(w);var x=new qx.ui.form.Button(g,l);x.setEnabled(false);v.add(x);}
,addToggleButton:function(z){var y=new qx.ui.form.ToggleButton(s,i);y.focus();z.add(y);y.addListener(m,function(e){this.debug(q+e.getData());}
,this);}
,addToggleButtonGroup:function(C){var B=new qx.ui.form.ToggleButton(o);var D=new qx.ui.form.ToggleButton(a);var A=new qx.ui.form.RadioGroup();A.add(B,D);C.add(B);C.add(D);}
,addRepeatButton:function(F){var E=new qx.ui.form.RepeatButton(null,f);F.add(E);var G=new qx.ui.basic.Label(c);G.setDecorator(p);G.setPadding(2,4);G.setBackgroundColor(n);F.add(G);E.addListener(r,function(){var H=parseInt(G.getValue())+1;G.setValue(H.toString());}
);}
,addHoverButton:function(J){var I=new qx.ui.form.HoverButton(j).set({padding:10,backgroundColor:d});J.add(I);var K=new qx.ui.basic.Label(c);K.setDecorator(p);K.setPadding(2,4);K.setBackgroundColor(n);J.add(K);I.addListener(r,function(){var L=parseInt(K.getValue())+1;K.setValue(L.toString());}
);}
}});}
)();
...
(function(){var a="hovered",b="hover-button",c="interval",d="mouseover",f="mouseout",g="__jO",h="qx.ui.form.HoverButton",i="Integer";qx.Class.define(h,{extend:qx.ui.basic.Atom,include:[qx.ui.core.MExecutable],implement:[qx.ui.form.IExecutable],construct:function(j,k){qx.ui.basic.Atom.call(this,j,k);this.addListener(d,this._onMouseOver,this);this.addListener(f,this._onMouseOut,this);this.__jO=new qx.event.AcceleratingTimer();this.__jO.addListener(c,this._onInterval,this);}
,properties:{appearance:{refine:true,init:b},interval:{check:i,init:80},firstInterval:{check:i,init:200},minTimer:{check:i,init:20},timerDecrease:{check:i,init:2}},members:{__jO:null,_onMouseOver:function(e){if(!this.isEnabled()||e.getTarget()!==this){return;}
;this.__jO.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();this.addState(a);}
,_onMouseOut:function(e){this.__jO.stop();this.removeState(a);if(!this.isEnabled()||e.getTarget()!==this){return;}
;}
