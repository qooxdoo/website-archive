...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.VBox_Flex.ed62e899da85.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,properties:{sort:{check:[m,o,A],init:m,apply:r},separatorX:{check:p,nullable:true,apply:h},separatorY:{check:p,nullable:true,apply:h},connectSeparators:{check:a,init:false,apply:h},spacingX:{check:f,init:0,apply:h},spacingY:{check:f,init:0,apply:h}},members:{__gV:null,__Hp:null,verifyLayoutProperty:qx.core.Environment.select(j,{"true":function(F,name,G){this.assertInArray(name,[y,w,c,k],g+name+l);if(name===w){this.assertInArray(G,[s,x,d,v,n]);}
...
,__Hq:{north:1,south:2,west:3,east:4,center:5},__Hr:{'1':t,'2':e,'3':z,'4':u},__lq:function(){var H=this._getLayoutChildren();var P,J;var length=H.length;var L=[];var O=[];var K=[];var I=this.getSort()===o;var N=this.getSort()===A;for(var i=0;i<length;i++ ){P=H[i];K=P.getLayoutProperties().edge;if(K===n){if(J){throw new Error(b);}
;J=P;}
...
;this.__gV=Q;var M=[];for(var i=0;i<length;i++ ){K=Q[i].getLayoutProperties().edge;M[i]=this.__Hq[K]||5;}
;this.__Hp=M;delete this._invalidChildrenCache;}
,renderLayout:function(bl,bb,bt){if(this._invalidChildrenCache){this.__lq();}
;var bk=qx.ui.layout.Util;var W=this.__gV;var bm=this.__Hp;var length=W.length;var ba,bu,Y,bf,bg,br,bd,T,S;var bx=[];var bc=[];var bs=this._getSeparatorWidths();var bB=this.getSpacingX();var bA=this.getSpacingY();var X=-bB;var bn=-bA;if(bs.x){X-=bs.x+bB;}
;if(bs.y){bn-=bs.y+bA;}
...
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__lq();}
;var bS=this.__gV;var cd=this.__Hp;var length=bS.length;var bW,ce;var bY,bX;var ca=0,cl=0;var bP=0,cg=0;var cb=0,ck=0;var bQ=0,cf=0;var ch=this._getSeparatorWidths();var cj=this.getSpacingX(),ci=this.getSpacingY();var bU=-cj,bT=-ci;if(ch.x){bU-=ch.x+cj;}
;if(ch.y){bT-=ch.y+ci;}
...
},destruct:function(){this.__Hp=this.__gV=null;}
});}
)();
...
,_applyValue:function(bh,bg){var bf=this.getChildControl(u);this._updateButtons();this.__wm=bh;if(bh!==null){if(this.getNumberFormat()){bf.setValue(this.getNumberFormat().format(bh));}
...
,_applyNumberFormat:function(bp,bo){var bn=this.getChildControl(u);bn.setFilter(this._getFilterRegExp());this.getNumberFormat().addListener(F,this._onChangeNumberFormat,this);this._applyValue(this.__wm,undefined);}
...
;}
;}
,_onKeyDown:function(e){switch(e.getKeyIdentifier()){case a:this.__wn=true;case j:this.getChildControl(z).press();break;case m:this.__wo=true;case h:this.getChildControl(r).press();break;default:return;};e.stopPropagation();e.preventDefault();}
,_onKeyUp:function(e){switch(e.getKeyIdentifier()){case a:this.getChildControl(z).release();this.__wn=false;break;case j:this.getChildControl(z).release();break;case m:this.getChildControl(r).release();this.__wo=false;break;case h:this.getChildControl(r).release();break;};}
...
;this.setValue(bv);}
else {this._applyValue(this.__wm,undefined);}
;}
...
,_countUp:function(){if(this.__wn){var bC=this.getValue()+this.getPageStep();}
...
;}
;this.gotoValue(bC);}
,_countDown:function(){if(this.__wo){var bE=this.getValue()-this.getPageStep();}
...
;var cn=this.getContentElement();if(ci&&this.__ou){if(this.__ou){var ck=this.getInsets();this._getPlaceholderElement().setStyles({paddingTop:ck.top+cp,paddingRight:ck.right+cp,paddingBottom:ck.bottom+cp,paddingLeft:ck.left+cp});}
;}
;if(cm||co.margin){if(this.__ou){var ck=this.getInsets();this._getPlaceholderElement().setStyles({"width":(innerWidth-ck.left-ck.right)+cp,"height":(innerHeight-ck.top-ck.bottom)+cp});}
;cn.setStyles({"width":innerWidth+cp,"height":innerHeight+cp});this._renderContentElement(innerHeight,cn);}
;if(co.position){if(this.__ou){this._getPlaceholderElement().setStyles({"left":cq+cp,"top":top+cp});}
;}
;}
...
;return cs;}
,_applyEnabled:function(cv,cu){qx.ui.core.Widget.prototype._applyEnabled.call(this,cv,cu);this.getContentElement().setEnabled(cv);if(this.__ou){if(cv){this._showPlaceholder();}
else {this._removePlaceholder();}
;}
else {var ct=this.getContentElement();ct.setAttribute(A,cv?this.getPlaceholder():P);}
;}
,__ow:{width:16,height:16},_getContentHint:function(){return {width:this.__ow.width*10,height:this.__ow.height||16};}
,_applyFont:function(cy,cx){if(cx&&this.__lr&&this.__lv){this.__lr.removeListenerById(this.__lv);this.__lv=null;}
;var cw;if(cy){this.__lr=qx.theme.manager.Font.getInstance().resolve(cy);if(this.__lr instanceof qx.bom.webfonts.WebFont){this.__lv=this.__lr.addListener(z,this._onWebFontStatusChange,this);}
;cw=this.__lr.getStyles();}
else {cw=qx.bom.Font.getDefaultStyles();}
;if(this.getTextColor()!=null){delete cw[r];}
;if(qx.core.Environment.get(bO)==bN&&qx.core.Environment.get(K)<11){qx.ui.core.queue.Manager.flush();this.getContentElement().setStyles(cw,true);}
else {this.getContentElement().setStyles(cw);}
;if(this.__ou){delete cw[r];this._getPlaceholderElement().setStyles(cw);}
;if(cy){this.__ow=qx.bom.Label.getTextSize(m,cw);}
else {delete this.__ow;}
;qx.ui.core.queue.Layout.add(this);}
...
,_getTextSize:function(){return this.__ow;}
,_onHtmlInput:function(e){var cF=e.getData();var cE=true;this.__ov=false;if(this.__jm&&this.__jm===cF){cE=false;}
...
;}
;if(cE){this.fireDataEvent(q,cF,this.__jm);this.__jm=cF;if(this.getLiveUpdate()){this.__ox(cF);}
;}
;}
,_onWebFontStatusChange:function(cJ){if(cJ.getData().valid===true){var cI=this.__lr.getStyles();this.__ow=qx.bom.Label.getTextSize(m,cI);qx.ui.core.queue.Layout.add(this);}
;}
,__ox:function(cL){var cK=this.__jl;this.__jl=cL;if(cK!=cL){this.fireNonBubblingEvent(bR,qx.event.type.Data,[cL,cK]);}
;}
,setValue:function(cP){if(cP===null){if(this.__ov){return cP;}
;cP=P;this.__ov=true;}
else {this.__ov=false;if(this.__ou){this._removePlaceholder();}
;}
;if(qx.lang.Type.isString(cP)){var cO=this.getContentElement();if(cO.getValue()!=cP){var cM=cO.getValue();cO.setValue(cP);var cN=this.__ov?null:cP;this.__jl=cM;this.__ox(cN);this.__jm=this.__jl;}
;if(this.__ou){this._showPlaceholder();}
;return cP;}
;throw new Error(l+cP);}
,getValue:function(){var cQ=this.getContentElement().getValue();return this.__ov?null:cQ;}
,resetValue:function(){this.setValue(null);}
,_onChangeContent:function(e){this.__ov=e.getData()===null;this.__ox(e.getData());}
...
,setLayoutParent:function(parent){qx.ui.core.Widget.prototype.setLayoutParent.call(this,parent);if(this.__ou){if(parent){this.getLayoutParent().getContentElement().add(this._getPlaceholderElement());}
...
.bind(this),0);}
,_removePlaceholder:function(){if(this.hasState(v)){if(this.__ou){this._getPlaceholderElement().setStyle(R,bY);}
;this.removeState(v);}
;}
,_syncPlaceholder:function(){if(this.hasState(v)&&this.__ou){this._getPlaceholderElement().setStyle(R,E);}
;}
...
;if(!this.__ou&&qx.ui.form.AbstractField.__dq){qx.bom.Stylesheet.removeSheet(qx.ui.form.AbstractField.__dq);qx.ui.form.AbstractField.__dq=null;qx.ui.form.AbstractField.__ot();}
;}
,_applyPlaceholder:function(cY,cX){if(this.__ou){this._getPlaceholderElement().setValue(cY);if(cY!=null){this.addListener(u,this._removePlaceholder,this);this.addListener(bK,this._showPlaceholder,this);this._showPlaceholder();}
...
;this._placeholder=this.__lr=null;if(qx.core.Environment.get(bH)){qx.locale.Manager.getInstance().removeListener(bT,this._onChangeLocale,this);}
;if(this.__lr&&this.__lv){this.__lr.removeListenerById(this.__lv);}
;this.getContentElement().removeListener(q,this._onHtmlInput,this);}
});}
)();
(function(){var a="engine.name",b="",c="wrap",d="none",e="Text wrapping is only support by textareas!",f="overflow",g="input",h="qx.html.Input",i="value",j="select",k="disabled",l="read-only",m="textarea",n="overflowX",o="overflowY";qx.Class.define(h,{extend:qx.html.Element,construct:function(r,p,q){if(r===j||r===m){var s=r;}
else {s=g;}
;qx.html.Element.call(this,s,p,q);this.__oy=r;}
,members:{__oy:null,__oz:null,__oA:null,_createDomElement:function(){return qx.bom.Input.create(this.__oy);}
,_applyProperty:function(name,t){qx.html.Element.prototype._applyProperty.call(this,name,t);var u=this.getDomElement();if(name===i){qx.bom.Input.setValue(u,t);}
else if(name===c){qx.bom.Input.setWrap(u,t);this.setStyle(f,u.style.overflow,true);this.setStyle(n,u.style.overflowX,true);this.setStyle(o,u.style.overflowY,true);}
;}
,setEnabled:qx.core.Environment.select(a,{"webkit":function(v){this.__oA=v;if(!v){this.setStyles({"userModify":l,"userSelect":d});}
else {this.setStyles({"userModify":null,"userSelect":this.__oz?null:d});}
;}
,"default":function(w){this.setAttribute(k,w===false);}
}),setSelectable:qx.core.Environment.select(a,{"webkit":function(x){this.__oz=x;qx.html.Element.prototype.setSelectable.call(this,this.__oA&&x);}
...
(function(){var a="demobrowser.demo.layout.VBox_Flex",b="green",c="main",d="yellow";qx.Class.define(a,{extend:demobrowser.demo.util.LayoutApplication,members:{main:function(){demobrowser.demo.util.LayoutApplication.prototype.main.call(this);var scroll=new qx.ui.container.Scroll();this.getRoot().add(scroll,{edge:0});var e=new qx.ui.container.Composite(new qx.ui.layout.HBox(20)).set({padding:20});scroll.add(e);e.add(this.getBox1());e.add(this.getBox2());e.add(this.getBox3());e.add(this.getBox4());e.add(this.getBox5());e.add(this.getBox6());}
,getBox1:function(){var f=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({decorator:c,backgroundColor:d,height:300,allowGrowY:false});f.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b}),{flex:1});f.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b}),{flex:2});f.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b}),{flex:3});return f;}
,getBox2:function(){var g=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({decorator:c,backgroundColor:d,height:300,allowGrowY:false});g.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b,maxHeight:30}),{flex:1});g.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b}),{flex:2});g.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b,maxHeight:100}),{flex:3});return g;}
,getBox3:function(){var h=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({decorator:c,backgroundColor:d,height:300,allowGrowY:false});for(var i=0;i<25;i++ ){h.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b,height:5}),{flex:1});}
;return h;}
,getBox4:function(){var j=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({decorator:c,backgroundColor:d,height:300,minHeight:300,allowGrowY:false});j.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b,maxHeight:60}),{flex:1});j.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b,maxHeight:60}),{flex:2});j.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b,maxHeight:60}),{flex:3});return j;}
,getBox5:function(){var k=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({height:150,minHeight:0,decorator:c,backgroundColor:d,allowGrowY:false});k.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b,minHeight:60}),{flex:1});k.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b,minHeight:60}),{flex:2});k.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b,minHeight:60}),{flex:3});return k;}
,getBox6:function(){var l=new qx.ui.container.Composite(new qx.ui.layout.VBox(5)).set({height:150,decorator:c,backgroundColor:d,allowGrowY:false});l.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b,minHeight:60}),{flex:1});l.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b,minHeight:60}),{flex:2});l.add(new qx.ui.core.Widget().set({decorator:c,backgroundColor:b,minHeight:60}),{flex:3});return l;}
}});}
)();
