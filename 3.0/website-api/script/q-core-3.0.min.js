...
;}
;return name;}
,getVersion:function(){var P=navigator.userAgent;var Q=new RegExp(z+qx.bom.client.Browser.__agents+k);var N=P.match(Q);if(!N){return x;}
...
;}
,__agents:{"webkit":u,"gecko":m,"mshtml":n,"opera":E}[qx.bom.client.Engine.getName()]},defer:function(R){qx.core.Environment.add(h,R.getName),qx.core.Environment.add(A,R.getVersion),qx.core.Environment.add(G,R.getDocumentMode),qx.core.Environment.add(g,R.getQuirksMode);}
});}
)();
...
(function(){var a="touchmove",b="os.name",c="MSPointerDown",d="swipe",e="android",f="qx.event.handler.TouchCore",g="event.mspointer",h="MSPointerCancel",j="y",k="pointer-events",l="longtap",m="touchend",n="MSPointerUp",o="right",p="engine.name",q="x",r="touchcancel",s="MSPointerMove",t="webkit",u="none",v="left",w="tap",z="down",A="undefined",B="up",C="touchstart";qx.Bootstrap.define(f,{extend:Object,statics:{TAP_MAX_DISTANCE:qx.core.Environment.get(b)!=e?10:40,SWIPE_DIRECTION:{x:[v,o],y:[B,z]},SWIPE_MIN_DISTANCE:qx.core.Environment.get(b)!=e?11:41,SWIPE_MIN_VELOCITY:0,LONGTAP_TIME:500},construct:function(D,E){this.__target=D;this.__emitter=E;this._initTouchObserver();}
,members:{__target:null,__emitter:null,__onTouchEventWrapper:null,__originalTarget:null,__startPageX:null,__startPageY:null,__startTime:null,__isSingleTouchGesture:null,__isTapGesture:null,__onMove:null,__beginScalingDistance:null,__beginRotation:null,__longTapTimer:null,_initTouchObserver:function(){this.__onTouchEventWrapper=qx.lang.Function.listener(this._onTouchEvent,this);var Event=qx.bom.Event;Event.addNativeListener(this.__target,C,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,a,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,m,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,r,this.__onTouchEventWrapper);if(qx.core.Environment.get(g)){Event.addNativeListener(this.__target,c,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,s,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,n,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,h,this.__onTouchEventWrapper);}
;}
,_stopTouchObserver:function(){var Event=qx.bom.Event;Event.removeNativeListener(this.__target,C,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,a,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,m,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,r,this.__onTouchEventWrapper);if(qx.core.Environment.get(g)){Event.removeNativeListener(this.__target,c,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,s,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,n,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,h,this.__onTouchEventWrapper);}
;}
...
else if(K==s){if(this.__onMove==true){K=a;}
;}
else if(K==h){K=r;}
;}
;if(K==C){this.__originalTarget=this._getTarget(M);this.__isTapGesture=true;if(M.touches&&M.touches.length>1){this.__beginScalingDistance=this._getScalingDistance(M.touches[0],M.touches[1]);this.__beginRotation=this._getRotationAngle(M.touches[0],M.touches[1]);}
;}
;if(K==a){if(typeof M.scale==A&&M.targetTouches.length>1){var N=this._getScalingDistance(M.targetTouches[0],M.targetTouches[1]);M.scale=N/this.__beginScalingDistance;}
;if(typeof M.rotation==A&&M.targetTouches.length>1){var L=this._getRotationAngle(M.targetTouches[0],M.targetTouches[1]);M.rotation=L-this.__beginRotation;}
;if(this.__isTapGesture){this.__isTapGesture=this._isBelowTapMaxDistance(M.changedTouches[0]);}
;}
;this._fireEvent(M,K);this.__checkAndFireGesture(M,K);}
,_isBelowTapMaxDistance:function(O){var P={x:O.screenX-this.__startPageX,y:O.screenY-this.__startPageY};var Q=qx.event.handler.TouchCore;return (Math.abs(P.x)<=Q.TAP_MAX_DISTANCE&&Math.abs(P.y)<=Q.TAP_MAX_DISTANCE);}
,_getTarget:function(S){var T=qx.bom.Event.getTarget(S);if(qx.core.Environment.get(p)==t){if(T&&T.nodeType==3){T=T.parentNode;}
;}
else if(qx.core.Environment.get(g)){var R=this.__evaluateTarget(S);if(R){T=R;}
;}
;return T;}
,__evaluateTarget:function(W){if(W&&W.touches){var U=W.touches[0].clientX;var V=W.touches[0].clientY;}
...
;var bc=bc||bb.type;if(bd&&bd.nodeType&&this.__emitter){this.__emitter.emit(bc,bb);}
;}
,__checkAndFireGesture:function(be,bf,bg){if(!bg){bg=this._getTarget(be);}
;var bf=bf||be.type;if(bf==C){this.__gestureStart(be,bg);}
else if(bf==a){this.__gestureChange(be,bg);}
else if(bf==m){this.__gestureEnd(be,bg);}
;}
,__gestureStart:function(bi,bj){var bh=bi.changedTouches[0];this.__onMove=true;this.__startPageX=bh.screenX;this.__startPageY=bh.screenY;this.__startTime=new Date().getTime();this.__isSingleTouchGesture=bi.targetTouches.length===1;if(this.__isSingleTouchGesture){this.__longTapTimer=window.setTimeout(this.__fireLongTap.bind(this,bi,bj),qx.event.handler.TouchCore.LONGTAP_TIME);}
else {this.__stopLongTapTimer();}
;}
,__gestureChange:function(bk,bl){if(this.__isSingleTouchGesture&&bk.changedTouches.length>1){this.__isSingleTouchGesture=false;}
;if(!this._isBelowTapMaxDistance(bk.changedTouches[0])){this.__stopLongTapTimer();}
;}
,__gestureEnd:function(bm,bn){this.__onMove=false;this.__stopLongTapTimer();if(this.__isSingleTouchGesture){var bq=bm.changedTouches[0];var br={x:bq.screenX-this.__startPageX,y:bq.screenY-this.__startPageY};var bo;if(this.__originalTarget==bn&&this.__isTapGesture){if(qx.event&&qx.event.type&&qx.event.type.Tap){bo=qx.event.type.Tap;}
;this._fireEvent(bm,w,bn,bo);}
else {var bp=this.__getSwipeGesture(bm,bn,br);if(bp){if(qx.event&&qx.event.type&&qx.event.type.Swipe){bo=qx.event.type.Swipe;}
;bm.swipe=bp;this._fireEvent(bm,d,bn,bo);}
;}
;}
;}
,__getSwipeGesture:function(bu,bv,bA){var bx=qx.event.handler.TouchCore;var by=new Date().getTime()-this.__startTime;var bB=(Math.abs(bA.x)>=Math.abs(bA.y))?q:j;var bs=bA[bB];var bt=bx.SWIPE_DIRECTION[bB][bs<0?0:1];var bz=(by!==0)?bs/by:0;var bw=null;if(Math.abs(bz)>=bx.SWIPE_MIN_VELOCITY&&Math.abs(bs)>=bx.SWIPE_MIN_DISTANCE){bw={startTime:this.__startTime,duration:by,axis:bB,direction:bt,distance:bs,velocity:bz};}
;return bw;}
,__fireLongTap:function(bC,bD){this._fireEvent(bC,l,bD,qx.event.type.Tap);this.__longTapTimer=null;this.__isTapGesture=false;}
