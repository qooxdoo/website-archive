...
;var bd;var Y=null;qx.Bootstrap.setDisplayNames(bg.statics,name);if(bg.members||bg.extend){qx.Bootstrap.setDisplayNames(bg.members,name+b);bd=bg.construct||new Function;if(bg.extend){this.extendClass(bd,bd,bg.extend,name,bb);}
;var X=bg.statics||{};for(var i=0,ba=qx.Bootstrap.keys(X),l=ba.length;i<l;i++){var W=ba[i];bd[W]=X[W];}
;Y=bd.prototype;var be=bg.members||{};for(var i=0,ba=qx.Bootstrap.keys(be),l=ba.length;i<l;i++){var W=ba[i];Y[W]=be[W];}
;}
else {bd=bg.statics||{};if(qx.Bootstrap.$$registry&&qx.Bootstrap.$$registry[name]){var bf=qx.Bootstrap.$$registry[name];if(this.keys(bd).length!==0){if(bg.defer){bg.defer(bd,Y);}
;for(var bc in bd){bf[bc]=bd[bc];}
;return;}
;}
;}
;bd.$$type=D;if(!bd.hasOwnProperty(q)){bd.toString=this.genericToString;}
;var bb=name?this.createNamespace(name,bd):z;bd.name=bd.classname=name;bd.basename=bb;if(bg.defer){bg.defer(bd,Y);}
;qx.Bootstrap.$$registry[name]=bd;return bd;}
};qx.Bootstrap.define(A,{statics:{__root:null,LOADSTART:qx.$$start||new Date(),DEBUG:(function(){var bh=true;if(qx.$$environment&&qx.$$environment[r]===false){bh=false;}
;return bh;}
...
;}
,createNamespace:qx.Bootstrap.createNamespace,setRoot:function(bl){this.__root=bl;}
...
,$$registry:{},objectGetLength:function(bm){return qx.Bootstrap.keys(bm).length;}
,objectMergeWith:function(bo,bn,bq){if(bq===undefined){bq=true;}
;for(var bp in bn){if(bq||bo[bp]===undefined){bo[bp]=bn[bp];}
;}
;return bo;}
,__shadowedKeys:[x,k,o,q,G,L,g],keys:({"ES5":Object.keys,"BROKEN_IE":function(br){if(br===null||(typeof br!=M&&typeof br!=c)){throw new TypeError(f);}
;var bs=[];var bu=Object.prototype.hasOwnProperty;for(var bv in br){if(bu.call(br,bv)){bs.push(bv);}
;}
;var bt=qx.Bootstrap.__shadowedKeys;for(var i=0,a=bt,l=a.length;i<l;i++){if(bu.call(br,a[i])){bs.push(a[i]);}
;}
;return bs;}
,"default":function(bw){if(bw===null||(typeof bw!=M&&typeof bw!=c)){throw new TypeError(f);}
;var bx=[];var by=Object.prototype.hasOwnProperty;for(var bz in bw){if(by.call(bw,bz)){bx.push(bz);}
;}
;return bx;}
})[typeof (Object.keys)==c?J:(function(){for(var bA in {toString:1}){return bA;}
;}
)()!==q?w:j],__classToTypeMap:{"[object String]":u,"[object Array]":E,"[object Object]":n,"[object RegExp]":t,"[object Number]":H,"[object Boolean]":d,"[object Date]":y,"[object Function]":B,"[object Error]":e},bind:function(bC,self,bD){var bB=Array.prototype.slice.call(arguments,2,arguments.length);return function(){var bE=Array.prototype.slice.call(arguments,0,arguments.length);return bC.apply(self,bB.concat(bE));}
;}
...
;}
,_initDefaultQxValues:function(){this.add(fq,function(){return true;}
);this.add(fl,function(){return false;}
);this.add(fd,function(){return false;}
);this.add(A,function(){return 0;}
);this.add(fz,function(){return true;}
);this.add(fc,function(){return true;}
);this.add(fj,function(){return false;}
...
;}
;}
,__importFromUrl:function(){if(window.document&&window.document.location){var gm=window.document.location.search.slice(1).split(eY);for(var i=0;i<gm.length;i++){var gp=gm[i].split(u);if(gp.length!=3||gp[0]!=ew){continue;}
;var gn=gp[1];var go=decodeURIComponent(gp[2]);if(go==fq){go=true;}
else if(go==fJ){go=false;}
...
,null,gq);}
},defer:function(gs){gs._initDefaultQxValues();gs.__importFromGenerator();if(gs.get(fl)===true){gs.__importFromUrl();}
;}
});}
)();
...
,slice:function(o,p){if(p!==undefined){return qxWeb.$init(Array.prototype.slice.call(this,o,p));}
;return qxWeb.$init(Array.prototype.slice.call(this,o));}
...
,getPlaceholder:function(){var i=document.createElement(o);return B in i;}
,getAppearance:function(){return qx.bom.Style.getPropertyName(A);}
...
;var bG=document.createElement(m);if(bH===r){bG.style[bH]=x;if(bG.style.borderImageSource.indexOf(M)>=0&&bG.style.borderImageSlice.indexOf(bx)>=0&&bG.style.borderImageRepeat.indexOf(e)>=0){return true;}
;}
else {bG.style[bH]=S;if(bG.style[bH].indexOf(M)>=0){return false;}
;}
;return null;}
,getUserSelect:function(){return qx.bom.Style.getPropertyName(s);}
,getUserSelectNone:function(){var bJ=qx.bom.client.Css.getUserSelect();if(bJ){var bI=document.createElement(W);bI.style[bJ]=C;return bI.style[bJ]===C?C:br;}
;return null;}
,getUserModify:function(){return qx.bom.Style.getPropertyName(bz);}
,getFloat:function(){var bK=document.documentElement.style;return bK.cssFloat!==undefined?bq:bK.styleFloat!==undefined?t:null;}
,getLinearGradient:function(){qx.bom.client.Css.__WEBKIT_LEGACY_GRADIENT=false;var bO=J;var bL=document.createElement(m);var bM=qx.bom.Style.getAppliedStyle(bL,D,bO);if(!bM){bO=X;var bM=qx.bom.Style.getAppliedStyle(bL,D,bO,false);if(bM){qx.bom.client.Css.__WEBKIT_LEGACY_GRADIENT=true;}
;}
;if(!bM){return null;}
;var bN=/(.*?)\(/.exec(bM);return bN?bN[1]:null;}
,getFilterGradient:function(){return qx.bom.client.Css.__isFilterSupported(U,bs);}
,getRadialGradient:function(){var bS=P;var bP=document.createElement(m);var bQ=qx.bom.Style.getAppliedStyle(bP,D,bS);if(!bQ){return null;}
...
,getFilterTextShadow:function(){return qx.bom.client.Css.__isFilterSupported(F,O);}
...
(function(){var a="event.pointer",b="foo",c="onhashchange",d="event.help",e="event.mspointer",f="event.touch",g="msPointerEnabled",h="event.hashchange",i="onhelp",j="pointerEvents",k="documentMode",l="qx.bom.client.Event",m="ontouchstart",n="mshtml",o="auto";qx.Bootstrap.define(l,{statics:{getTouch:function(){return (m in window);}
,getPointer:function(){var p=document.documentElement;if(j in p.style){var r=p.style.pointerEvents;p.style.pointerEvents=o;p.style.pointerEvents=b;var q=p.style.pointerEvents==o;p.style.pointerEvents=r;return q;}
;return false;}
,getMsPointer:function(){if(g in window.navigator){return window.navigator.msPointerEnabled;}
;return false;}
,getHelp:function(){return (i in document);}
,getHashChange:function(){var s=qx.bom.client.Engine.getName();var t=c in window;return (s!==n&&t)||(s===n&&k in document&&document.documentMode>=8&&t);}
},defer:function(u){qx.core.Environment.add(f,u.getTouch);qx.core.Environment.add(a,u.getPointer);qx.core.Environment.add(e,u.getMsPointer);qx.core.Environment.add(d,u.getHelp);qx.core.Environment.add(h,u.getHashChange);}
});}
)();
...
;}
,supportsEvent:function(A,z){var x=i+z.toLowerCase();var y=(x in A);if(!y){y=typeof A[x]==f;if(!y&&A.setAttribute){A.setAttribute(x,d);y=typeof A[x]==f;A.removeAttribute(x);}
;}
;return y;}
}});}
)();
...
,clone:function(g){var f=[];for(var i=0;i<this.length;i++){f[i]=this[i].cloneNode(true);}
;if(g===true&&this.copyEventsTo){this.copyEventsTo(f);}
;return qxWeb(f);}
,append:function(k){var h=qx.bom.Html.clean([k]);var n=qxWeb.$init(h);for(var i=0,l=this.length;i<l;i++){for(var j=0,m=n.length;j<m;j++){if(i==0){qx.dom.Element.insertEnd(n[j],this[i]);}
...
;}
;return this;}
,insertBefore:function(o){o=qx.module.Manipulating.__getElementArray(o);for(var i=0,l=o.length;i<l;i++){for(var j=0,m=this.length;j<m;j++){if(i==0){qx.dom.Element.insertBefore(this[j],o[i]);}
else {qx.dom.Element.insertBefore(this.eq(j).clone(true)[0],o[i]);}
;}
;}
;return this;}
,insertAfter:function(p){p=qx.module.Manipulating.__getElementArray(p);for(var i=0,l=p.length;i<l;i++){for(var j=this.length-1;j>=0;j--){if(i==0){qx.dom.Element.insertAfter(this[j],p[i]);}
...
;return r;}
,wrap:function(u){var u=qx.module.Manipulating.__getCollectionFromArgument(u);if(u.length==0||!qx.dom.Node.isElement(u[0])){return this;}
;for(var i=0,l=this.length;i<l;i++){var s=u.eq(0).clone(true);qx.dom.Element.insertAfter(s[0],this[i]);var t=qx.module.Manipulating.__getInnermostElement(s[0]);qx.dom.Element.insertEnd(this[i],t);}
;return this;}
,__getCollectionFromArgument:function(x){var v;if(qx.lang.Type.isArray(x)){v=qxWeb(x);}
else {var w=qx.bom.Html.clean([x]);if(w.length>0&&qx.dom.Node.isElement(w[0])){v=qxWeb(w);}
else {v=qxWeb(x);}
;}
;return v;}
,__getInnermostElement:function(y){if(y.childNodes.length==0){return y;}
;for(var i=0,l=y.childNodes.length;i<l;i++){if(y.childNodes[i].nodeType===1){return this.__getInnermostElement(y.childNodes[i]);}
;}
;return y;}
...
;var z=document.createDocumentFragment();qx.bom.Html.clean(content,document,z);this.forEach(function(C,A){var B=qx.lang.Array.cast(z.childNodes,Array);for(var i=0,l=B.length;i<l;i++){var D;if(A<this.length-1){D=B[i].cloneNode(true);}
else {D=B[i];}
;C.parentNode.insertBefore(D,C);}
;}
,this);return this;}
,after:function(content){if(!qx.lang.Type.isArray(content)){content=[content];}
;var E=document.createDocumentFragment();qx.bom.Html.clean(content,document,E);this.forEach(function(I,F){var H=qx.lang.Array.cast(E.childNodes,Array);for(var i=H.length-1;i>=0;i--){var G;if(F<this.length-1){G=H[i].cloneNode(true);}
else {G=H[i];}
...
,setScrollLeft:function(Q,R){var Node=qx.dom.Node;if(R&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__animateScroll.bind(this,a,Q,R)();}
;for(var i=0,l=this.length,P;i<l;i++){P=this[i];if(Node.isElement(P)){if(!(R&&qx.bom.element&&qx.bom.element.AnimationJs)){P.scrollLeft=Q;}
;}
else if(Node.isWindow(P)){P.scrollTo(Q,this.getScrollTop(P));}
else if(Node.isDocument(P)){Node.getWindow(P).scrollTo(Q,this.getScrollTop(P));}
;}
;return this;}
,setScrollTop:function(T,U){var Node=qx.dom.Node;if(U&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__animateScroll.bind(this,d,T,U)();}
;for(var i=0,l=this.length,S;i<l;i++){S=this[i];if(Node.isElement(S)){if(!(U&&qx.bom.element&&qx.bom.element.AnimationJs)){S.scrollTop=T;}
;}
else if(Node.isWindow(S)){S.scrollTo(this.getScrollLeft(S),T);}
else if(Node.isDocument(S)){Node.getWindow(S).scrollTo(this.getScrollLeft(S),T);}
;}
;return this;}
...
;}
;if(J==B){this.__originalTarget=this._getTarget(L);this.__isTapGesture=true;if(L.touches&&L.touches.length>1){this.__beginScalingDistance=this._getScalingDistance(L.touches[0],L.touches[1]);this.__beginRotation=this._getRotationAngle(L.touches[0],L.touches[1]);}
;}
...
;if(this.__isTapGesture){this.__isTapGesture=this._isBelowTapMaxDistance(L.changedTouches[0]);}
;}
;this._fireEvent(L,J);this.__checkAndFireGesture(L,J);}
,_isBelowTapMaxDistance:function(N){var O={x:N.screenX-this.__startPageX,y:N.screenY-this.__startPageY};var P=qx.event.handler.TouchCore;return (Math.abs(O.x)<=P.TAP_MAX_DISTANCE&&Math.abs(O.y)<=P.TAP_MAX_DISTANCE);}
,_getTarget:function(R){var S=qx.bom.Event.getTarget(R);if(qx.core.Environment.get(o)==s){if(S&&S.nodeType==3){S=S.parentNode;}
;}
else if(qx.core.Environment.get(g)){var Q=this.__evaluateTarget(R);if(Q){S=Q;}
;}
;return S;}
,__evaluateTarget:function(V){if(V&&V.touches){var T=V.touches[0].clientX;var U=V.touches[0].clientY;}
;var X=document.msElementsFromPoint(T,U);if(X){for(var i=0;i<X.length;i++){var Y=X[i];var W=qx.bom.element.Style.get(Y,k,3);if(W!=t){return Y;}
;}
;}
;return null;}
,_fireEvent:function(ba,bb,bc){if(!bc){bc=this._getTarget(ba);}
;var bb=bb||ba.type;if(bc&&bc.nodeType&&this.__emitter){this.__emitter.emit(bb,ba);}
;}
,__checkAndFireGesture:function(bd,be,bf){if(!bf){bf=this._getTarget(bd);}
;var be=be||bd.type;if(be==B){this.__gestureStart(bd,bf);}
else if(be==a){this.__gestureChange(bd,bf);}
else if(be==l){this.__gestureEnd(bd,bf);}
;}
,__gestureStart:function(bh,bi){var bg=bh.changedTouches[0];this.__onMove=true;this.__startPageX=bg.screenX;this.__startPageY=bg.screenY;this.__startTime=new Date().getTime();this.__isSingleTouchGesture=bh.changedTouches.length===1;}
,__gestureChange:function(bj,bk){if(this.__isSingleTouchGesture&&bj.changedTouches.length>1){this.__isSingleTouchGesture=false;}
;}
,__gestureEnd:function(bl,bm){this.__onMove=false;if(this.__isSingleTouchGesture){var bp=bl.changedTouches[0];var bq={x:bp.screenX-this.__startPageX,y:bp.screenY-this.__startPageY};var bn;if(this.__originalTarget==bm&&this.__isTapGesture){if(qx.event&&qx.event.type&&qx.event.type.Tap){bn=qx.event.type.Tap;}
;this._fireEvent(bl,v,bm,bn);}
else {var bo=this.__getSwipeGesture(bl,bm,bq);if(bo){if(qx.event&&qx.event.type&&qx.event.type.Swipe){bn=qx.event.type.Swipe;}
;bl.swipe=bo;this._fireEvent(bl,d,bm,bn);}
;}
;}
;}
,__getSwipeGesture:function(bt,bu,bz){var bw=qx.event.handler.TouchCore;var bx=new Date().getTime()-this.__startTime;var bA=(Math.abs(bz.x)>=Math.abs(bz.y))?p:j;var br=bz[bA];var bs=bw.SWIPE_DIRECTION[bA][br<0?0:1];var by=(bx!==0)?br/bx:0;var bv=null;if(Math.abs(by)>=bw.SWIPE_MIN_VELOCITY&&Math.abs(br)>=bw.SWIPE_MIN_DISTANCE){bv={startTime:this.__startTime,duration:bx,axis:bA,direction:bs,distance:br,velocity:by};}
;return bv;}
