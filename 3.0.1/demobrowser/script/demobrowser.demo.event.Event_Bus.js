...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.event.Event_Bus.0a22f0b0a7b8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,_initLogger:function(p,n,o){this.__Qy=f+p.join(c)+a;this.__Qz=o||50;this.__QA=n;this._clearLog();}
,_clearLog:function(){this.__QA.innerHTML=this.__Qy+j;this.__QB=[];}
,_log:function(q){this.__QB.unshift(q);this.__QB=this.__QB.slice(0,this.__Qz);var r=[this.__Qy];for(var i=0;i<this.__QB.length;i++ ){r.push(g,this.__QB[i].join(e),l);}
;r.push(j);this.__QA.innerHTML=r.join(d);this.__QA.scrollTop=0;}
,__Qy:null,__Qz:null,__QA:null,__QB:null},destruct:function(){this.__QA=this.__QB=null;}
});}
)();
...
},construct:function(){this.__sz={};}
,members:{__sz:null,getSubscriptions:function(){return this.__sz;}
...
;}
;return false;}
,dispatch:function(L){var G=this.getSubscriptions();var H=L.getName();var I=false;for(var K in G){var J=K.indexOf(c);if(J>-1){if(J===0||K.substr(0,J)===H.substr(0,J)){this.__sA(G[K],L);I=true;}
;}
else {if(K===H){this.__sA(G[H],L);I=true;}
;}
;}
;return I;}
,dispatchByName:function(name,N){var M=new qx.event.message.Message(name,N);return this.dispatch(M);}
,__sA:function(P,O){for(var i=0;i<P.length;i++ ){var R=P[i].subscriber;var Q=P[i].context;if(Q&&Q.isDisposed){if(Q.isDisposed()){P.splice(i,1);i-- ;}
else {R.call(Q,O);}
;}
else {R.call(Q,O);}
;}
