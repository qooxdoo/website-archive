...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Dynamic.18c3effecf28.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="changeModel",b="or 'labelPath' is 'null'!",c="_applyLabelOptions",d="Boolean",f="one",g="marshal.MEventBubbling'. Therefore the tree can not update ",h="_applyOpenMode",j="the view automatically on model changes.",k="qx.core.Object",l="Space",m="Left",n="filter",o="Integer",p="_applyModel",q="_applyShowTopLevelOpenCloseIcons",r="_applyLabelPath",s="dblclick",t="open",u="changeOpenMode",v="qx.debug",w="changeDelegate",x="virtual-tree",y="_applyChildProperty",z="qx.event.type.Data",A="_applyIconPath",B="_applyDelegate",C="_applyHideRoot",D="close",E="The model item doesn't support the Mixin 'qx.data.",F="_applyRowHeight",G="sorter",H="Right",I="cellDblclick",J="Enter",K="qx.ui.tree.VirtualTree",L="click",M="keypress",N="cellClick",O="none",P="Could not build tree, because 'childProperty' and/",Q="updated",R="_applyShowLeafs",S="String",T="_applyIconOptions",U="changeBubble",V=".";qx.Class.define(K,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.tree.core.IVirtualTree,qx.data.controller.ISelection],include:[qx.ui.virtual.selection.MModel,qx.ui.core.MContentPadding],construct:function(W,Y,X){qx.ui.virtual.core.Scroller.call(this,0,1,20,100);this._init();if(Y!=null){this.setLabelPath(Y);}
;if(X!=null){this.setChildProperty(X);}
;if(W!=null){this.initModel(W);}
;this.initItemHeight();this.initOpenMode();this.addListener(M,this._onKeyPress,this);}
,events:{open:z,close:z},properties:{appearance:{refine:true,init:x},focusable:{refine:true,init:true},width:{refine:true,init:100},height:{refine:true,init:200},itemHeight:{check:o,init:25,apply:F,themeable:true},openMode:{check:[L,s,O],init:s,apply:h,event:u,themeable:true},hideRoot:{check:d,init:false,apply:C},showTopLevelOpenCloseIcons:{check:d,init:false,apply:q},showLeafs:{check:d,init:true,apply:R},childProperty:{check:S,apply:y,nullable:true},labelPath:{check:S,apply:r,nullable:true},iconPath:{check:S,apply:A,nullable:true},labelOptions:{apply:c,nullable:true},iconOptions:{apply:T,nullable:true},model:{check:k,apply:p,event:a,nullable:true,deferredInit:true},delegate:{event:w,apply:B,init:null,nullable:true}},members:{_provider:null,_layer:null,__xI:null,__yG:null,__yH:null,__gL:null,__yI:0,__yJ:null,syncWidget:function(bb){var bd=this._layer.getFirstRow();var bc=this._layer.getRowSizes().length;for(var bf=bd;bf<bd+bc;bf++ ){var ba=this._layer.getRenderedCellWidget(bf,0);if(ba!=null){this.__yI=Math.max(this.__yI,ba.getSizeHint().width);}
;}
;var be=this.getPane().getInnerSize().width;this.getPane().getColumnConfig().setItemSize(0,Math.max(this.__yI,be));}
,openNode:function(bg){this.__yM(bg);this.buildLookupTable();}
,refresh:function(){this.buildLookupTable();}
,openNodeAndParents:function(bh){this.__yN(this.getModel(),bh);this.buildLookupTable();}
,closeNode:function(bi){if(qx.lang.Array.contains(this.__yG,bi)){qx.lang.Array.remove(this.__yG,bi);this.fireDataEvent(D,bi);this.buildLookupTable();}
;}
,isNodeOpen:function(bj){return qx.lang.Array.contains(this.__yG,bj);}
,_init:function(){this.__xI=new qx.data.Array();this.__yG=[];this.__yH=[];this._initLayer();}
,_initLayer:function(){this._provider=new qx.ui.tree.provider.WidgetProvider(this);this._layer=this._provider.createLayer();this._layer.addListener(Q,this._onUpdated,this);this.getPane().addLayer(this._layer);}
,getLookupTable:function(){return this.__xI;}
,_reverseLookup:function(bk){return bk;}
,_getDataFromRow:function(bl){return this.__xI.getItem(bl);}
,_getSelectables:function(){return this.__xI;}
,getOpenNodes:function(){return this.__yG;}
,isNode:function(bm){return qx.Class.hasProperty(bm.constructor,this.getChildProperty());}
,getLevel:function(bn){return this.__yH[bn];}
,hasChildren:function(bo){var bp=bo.get(this.getChildProperty());if(bp==null){return false;}
;if(this.isShowLeafs()){return bp.length>0;}
else {for(var i=0;i<bp.getLength();i++ ){var bq=bp.getItem(i);if(this.isNode(bq)){return true;}
;}
;}
;return false;}
,_getContentPaddingTarget:function(){return this.getPane();}
,_applyRowHeight:function(bs,br){this.getPane().getRowConfig().setDefaultItemSize(bs);}
,_applyOpenMode:function(bu,bt){var bv=this.getPane();if(bu===s){bv.addListener(I,this._onOpen,this);}
else if(bu===L){bv.addListener(N,this._onOpen,this);}
;if(bt===s){bv.removeListener(I,this._onOpen,this);}
else if(bt===L){bv.removeListener(N,this._onOpen,this);}
;}
,_applyHideRoot:function(bx,bw){this.buildLookupTable();}
,_applyShowTopLevelOpenCloseIcons:function(bz,by){this.buildLookupTable();}
,_applyShowLeafs:function(bB,bA){this.buildLookupTable();}
,_applyChildProperty:function(bD,bC){this._provider.setChildProperty(bD);}
,_applyLabelPath:function(bF,bE){this._provider.setLabelPath(bF);}
,_applyIconPath:function(bH,bG){this._provider.setIconPath(bH);}
,_applyLabelOptions:function(bJ,bI){this._provider.setLabelOptions(bJ);}
,_applyIconOptions:function(bL,bK){this._provider.setIconOptions(bL);}
,_applyModel:function(bN,bM){this.__yG=[];if(bN!=null){if(qx.core.Environment.get(v)){if(!qx.Class.hasMixin(bN.constructor,qx.data.marshal.MEventBubbling)){this.warn(E+g+j);}
;}
;bN.addListener(U,this._onChangeBubble,this);this.__yM(bN);}
;if(bM!=null){bM.removeListener(U,this._onChangeBubble,this);}
;this.__yK();}
,_applyDelegate:function(bP,bO){this._provider.setDelegate(bP);this.buildLookupTable();}
,_onChangeBubble:function(event){var bU=event.getData();var bR=bU.name;var bQ=bR.lastIndexOf(V);if(bQ!=-1){bR=bR.substr(bQ+1,bR.length);}
;if(qx.lang.String.startsWith(bR,this.getChildProperty())){var bS=bU.item;if(qx.Class.isSubClassOf(bS.constructor,qx.data.Array)){if(bQ===-1){bS=this.getModel();}
else {var bT=bU.name.substr(0,bQ);bS=qx.data.SingleValueBinding.resolvePropertyChain(this.getModel(),bT);}
;}
;if(this.__xI.indexOf(bS)!=-1){this.__yK();}
;}
;}
,_onUpdated:function(event){if(this.__gL==null){this.__gL=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__gL.schedule();}
,_onOpen:function(event){var bW=event.getRow();var bV=this.__xI.getItem(bW);if(this.isNode(bV)){if(this.isNodeOpen(bV)){this.closeNode(bV);}
else {this.openNode(bV);}
;}
;}
,_onKeyPress:function(e){var bX=this.getSelection();if(bX.getLength()>0){var ca=bX.getItem(0);var bY=this.isNode(ca);switch(e.getKeyIdentifier()){case m:if(bY&&this.isNodeOpen(ca)){this.closeNode(ca);}
else {var parent=this.getParent(ca);if(parent!=null){bX.splice(0,1,parent);}
;}
;break;case H:if(bY&&!this.isNodeOpen(ca)){this.openNode(ca);}
else {if(bY){var cb=ca.get(this.getChildProperty());if(cb!=null&&cb.getLength()>0){bX.splice(0,1,cb.getItem(0));}
;}
;}
;break;case J:case l:if(!bY){return;}
;if(this.isNodeOpen(ca)){this.closeNode(ca);}
else {this.openNode(ca);}
;break;};}
;}
,_beforeApplySelection:function(cd){if(cd.length===0&&this.getSelectionMode()===f){var cc=this.__yP();var ce=this.getLookupTable().indexOf(cc);if(ce>=0){cd.push(ce);}
;}
;}
,_afterApplySelection:function(){var cf=this.getSelection();if(cf.getLength()>0&&this.getSelectionMode()===f){this.__yO(cf.getItem(0));}
else {this.__yJ=[];}
;}
,__yK:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(P+b);}
;this.__yI=0;var cj=[];this.__yH=[];var ch=-1;var ci=this.getModel();if(ci!=null){if(!this.isHideRoot()){ch++ ;cj.push(ci);this.__yH.push(ch);}
;if(this.isNodeOpen(ci)){var cg=this.__yL(ci,ch);cj=cj.concat(cg);}
;}
;this._provider.removeBindings();this.__xI.removeAll();this.__xI.append(cj);this.__xO();this._updateSelection();}
,__yL:function(ck,cp){var co=[];cp++ ;if(!this.isNode(ck)){return co;}
;var cs=ck.get(this.getChildProperty());if(cs==null){return co;}
;cs=cs.copy();var cq=this.getDelegate();var cn=qx.util.Delegate.getMethod(cq,n);var cm=qx.util.Delegate.getMethod(cq,G);if(cm!=null){cs.sort(cm);}
;for(var i=0;i<cs.getLength();i++ ){var cr=cs.getItem(i);if(cn&&!cn(cr)){continue;}
;if(this.isNode(cr)){this.__yH.push(cp);co.push(cr);if(this.isNodeOpen(cr)){var cl=this.__yL(cr,cp);co=co.concat(cl);}
;}
else {if(this.isShowLeafs()){this.__yH.push(cp);co.push(cr);}
;}
;}
;cs.dispose();return co;}
,__yM:function(ct){if(!qx.lang.Array.contains(this.__yG,ct)){this.__yG.push(ct);this.fireDataEvent(t,ct);}
;}
,__yN:function(cu,cv){if(cu===cv){this.__yM(cv);return true;}
;if(!this.isNode(cu)){return false;}
;var cy=cu.get(this.getChildProperty());if(cy==null){return false;}
;for(var i=0;i<cy.getLength();i++ ){var cx=cy.getItem(i);var cw=this.__yN(cx,cv);if(cw===true){this.__yM(cx);return true;}
;}
;return false;}
,__xO:function(){this.getPane().getRowConfig().setItemCount(this.__xI.getLength());this.getPane().fullUpdate();}
,getParent:function(cA){var cB=this.__xI.indexOf(cA);if(cB<0){return null;}
;var cC=this.__yH[cB];while(cB>0){cB-- ;var cz=this.__yH[cB];if(cz<cC){return this.__xI.getItem(cB);}
;}
;return null;}
,__yO:function(cD){this.__yJ=[];var parent=this.getParent(cD);while(parent!=null){this.__yJ.unshift(parent);parent=this.getParent(parent);}
;}
,__yP:function(){if(this.__yJ==null){return this.getModel();}
;var cE=this.getLookupTable();var parent=this.__yJ.pop();while(parent!=null){if(cE.contains(parent)){return parent;}
;parent=this.__yJ.pop();}
;return this.getModel();}
},destruct:function(){var cG=this.getPane();if(cG!=null){if(cG.hasListener(I)){cG.removeListener(I,this._onOpen,this);}
;if(cG.hasListener(N)){cG.removeListener(N,this._onOpen,this);}
;}
;if(!qx.core.ObjectRegistry.inShutDown&&this.__gL!=null){this.__gL.cancel();this.__gL.dispose();}
;var cF=this.getModel();if(cF!=null){cF.removeListener(U,this._onChangeBubble,this);}
;this._layer.removeListener(Q,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__xI.dispose();this._layer=this._provider=this.__xI=this.__yG=this.__gL=null;}
});}
)();
...
(function(){var a="BindingIds",b="model",c="",d="changeDelegate",f="virtual-tree-file",g="label",h="appearance",i="icon",j="bindItem",k=".length",l="]",m="virtual-tree-folder",n="[",o=".",p="String",q="qx.ui.tree.core.MWidgetController";qx.Mixin.define(q,{construct:function(){this.__wT=[];}
...
;}
,bindProperty:function(x,A,y,w,u){var z=this.__wU(u,x);var v=this._tree.getLookupTable();var B=v.bind(z,w,A,y);this.__wV(w,B);}
,bindPropertyReverse:function(I,E,G,H,C){var F=this.__wU(C,I);var D=this._tree.getLookupTable();var J=H.bind(E,D,F,G);this.__wV(H,J);}
,removeBindings:function(){while(this.__wT.length>0){var K=this.__wT.pop();this._removeBindingsFrom(K);}
;}
...
;}
,_removeBindingsFrom:function(O){var P=this.__wW(O);while(P.length>0){var Q=P.pop();try{this._tree.getLookupTable().removeBinding(Q);}
catch(e){O.removeBinding(Q);}
;}
;if(qx.lang.Array.contains(this.__wT,O)){qx.lang.Array.remove(this.__wT,O);}
;}
,__wU:function(S,T){var R=n+S+l;if(T!=null&&T!=c){R+=o+T;}
;return R;}
,__wV:function(V,W){var U=this.__wW(V);if(!qx.lang.Array.contains(U,W)){U.push(W);}
;if(!qx.lang.Array.contains(this.__wT,V)){this.__wT.push(V);}
;}
,__wW:function(X){var Y=X.getUserData(a);if(Y==null){Y=[];X.setUserData(a,Y);}
;return Y;}
...
;var n=this._renderer.getCellWidget();n.setOpen(o&&this._tree.isNodeOpen(r));n.setUserData(g,o);n.addListener(a,this.__yQ,this);if(this._tree.getSelection().contains(r)){this._styleSelectabled(n);}
...
,poolCellWidget:function(t){t.removeListener(a,this.__yQ,this);this._removeBindingsFrom(t);this._renderer.pool(t);this._onPool(t);}
...
,__yQ:function(event){var J=event.getTarget();var K=J.getUserData(k);var I=this._tree.getLookupTable().getItem(K);if(event.getData()){this._tree.openNode(I);}
