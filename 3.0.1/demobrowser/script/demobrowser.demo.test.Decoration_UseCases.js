...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Decoration_UseCases.aa36c8eaf82f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;n=n.concat(m);}
;return qxWeb.$init(n);}
,getAncestors:function(p){return this.__nv(null,p);}
,getAncestorsUntil:function(r,q){return this.__nv(r,q);}
,__nv:function(t,u){var s=[];for(var i=0;i<this.length;i++ ){var parent=qx.dom.Element.getParentElement(this[i]);while(parent){var v=[parent];if(t&&qx.bom.Selector.matches(t,v).length>0){break;}
...
,getNext:function(O){var N=this.map(qx.dom.Hierarchy.getNextElementSibling,qx.dom.Hierarchy);if(O){N=qxWeb.$init(qx.bom.Selector.matches(O,N));}
;return N;}
,getNextAll:function(Q){var P=qx.module.Traversing.__nw(this,d,Q);return qxWeb.$init(P);}
...
,getPrev:function(X){var W=this.map(qx.dom.Hierarchy.getPreviousElementSibling,qx.dom.Hierarchy);if(X){W=qxWeb.$init(qx.bom.Selector.matches(X,W));}
;return W;}
,getPrevAll:function(ba){var Y=qx.module.Traversing.__nw(this,a,ba);return qxWeb.$init(Y);}
...
,getSiblings:function(bh){var bg=qx.module.Traversing.__nw(this,b,bh);return qxWeb.$init(bg);}
...
;}
;var ba,bc=[];for(var i=0,l=X.length;i<l;i++ ){ba=X[i];if(typeof ba===k){ba=this.__nu(ba,be);}
...
,appendTo:function(parent){parent=qx.module.Manipulating.__nx(parent);for(var i=0,l=parent.length;i<l;i++ ){this._forEachElement(function(q,j){if(i==0){qx.dom.Element.insertEnd(this[j],parent[i]);}
else {qx.dom.Element.insertEnd(this.eq(j).clone(true)[0],parent[i]);}
;}
);}
;return this;}
,insertBefore:function(r){r=qx.module.Manipulating.__nx(r);for(var i=0,l=r.length;i<l;i++ ){this._forEachElement(function(s,t){if(i==0){qx.dom.Element.insertBefore(s,r[i]);}
else {qx.dom.Element.insertBefore(this.eq(t).clone(true)[0],r[i]);}
;}
);}
;return this;}
,insertAfter:function(u){u=qx.module.Manipulating.__nx(u);for(var i=0,l=u.length;i<l;i++ ){for(var j=this.length-1;j>=0;j-- ){if(!this[j]||this[j].nodeType!==1){continue;}
;if(i==0){qx.dom.Element.insertAfter(this[j],u[i]);}
else {qx.dom.Element.insertAfter(this.eq(j).clone(true)[0],u[i]);}
;}
;}
;return this;}
,__nx:function(w){if(!qx.lang.Type.isArray(w)){var v=qxWeb(w);w=v.length>0?v:[w];}
;return w.filter(function(x){return (x&&x.nodeType===1);}
);}
,wrap:function(y){y=qx.module.Manipulating.__ny(y);if(y.length==0){return this;}
;this._forEachElement(function(z){var A=y.eq(0).clone(true);qx.dom.Element.insertAfter(A[0],z);var B=qx.module.Manipulating.__nz(A[0]);qx.dom.Element.insertEnd(z,B);}
);return this;}
,__ny:function(E){var C;if(qx.lang.Type.isArray(E)){C=qxWeb(E);}
else {var D=qx.bom.Html.clean([E]);if(D.length>0&&qx.dom.Node.isElement(D[0])){C=qxWeb(D);}
else {C=qxWeb(E);}
;}
;return C;}
,__nz:function(F){if(F.childNodes.length==0){return F;}
;for(var i=0,l=F.childNodes.length;i<l;i++ ){if(F.childNodes[i].nodeType===1){return this.__nz(F.childNodes[i]);}
;}
;return F;}
...
;return T.scrollTop;}
,_animationDescription:{scrollLeft:{duration:700,timing:c,keep:100,keyFrames:{'0':{},'100':{scrollLeft:1}}},scrollTop:{duration:700,timing:c,keep:100,keyFrames:{'0':{},'100':{scrollTop:1}}}},__nA:function(V,W,U){var X=qx.lang.Object.clone(qx.module.Manipulating._animationDescription[V],true);X.keyFrames[100][V]=W;return this.animate(X,U);}
,setScrollLeft:function(ba,bb){var Node=qx.dom.Node;if(bb&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__nA.bind(this,a,ba,bb)();}
...
;}
;return this;}
,setScrollTop:function(bd,be){var Node=qx.dom.Node;if(be&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__nA.bind(this,d,bd,be)();}
...
(function(){var a="backgroundColor",b="demobrowser/demo/test/logo.gif",c="qx/decoration/Modern/cursors/alias.gif",d="scale",e="<div style='position:absolute;left:600px;top:1100px;font-size:36px;font-weight:bold;color:red'>Scaled Clipped Alpha PNG</div>",f="demobrowser/demo/test/demobrowser_thumb.png",g="41px",h="500px",i="136px",j="scale-y",k="880px",l="qx/decoration/Modern/app-header.png",m="200px",n="scale-x",o="qx/icon/Tango/22/places/folder-open.png",p="450px",r="repeat-y",s="650px",t="<div style='position:absolute;left:600px;top:20px;font-size:36px;font-weight:bold;color:red'>Single PNG</div>",u="demobrowser/demo/test/alphaPNG.png",v="230px",w="400px",x="qx/decoration/Modern/form/checkbox-focused.png",y="50px",z="<div style='position:absolute;left:600px;top:230px;font-size:36px;font-weight:bold;color:red'>Clipped PNG</div>",A="1700px",B="120px",C="1100px",D="250px",E="350px",F="20px",G="15px",H="no-repeat",I="150px",J="1400px",K="<div style='position:absolute;left:600px;top:1700px;font-size:36px;font-weight:bold;color:red'>Clipped GIF</div>",L="<div style='position:absolute;left:600px;top:1400px;font-size:36px;font-weight:bold;color:red'>Single GIF</div>",M="repeat-x",N="body",O="<div style='position:absolute;left:600px;top:650px;font-size:36px;font-weight:bold;color:red'>Repeated Single Alpha PNG</div>",P="19px",Q="420px",R="orange",S="<div style='position:absolute;left:600px;top:880px;font-size:36px;font-weight:bold;color:red'>Repeated Clipped Alpha PNG</div>",T="demobrowser.demo.test.Decoration_UseCases";qx.Class.define(T,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var bi=q(N);var ba=l;var bl=f;var bh=u;var bj=o;var bk=x;var Y=b;var bm=c;var bu=qx.bom.element.Decoration;var bt=bu.create(ba,H,{top:F});bi.append(bt);var bq=bu.create(ba,M,{width:E,height:F,top:F,left:B});bi.append(bq);var br=bu.create(ba,r,{width:F,height:m,top:F,left:h});bi.append(br);bi.append(q.create(t));var bs=bu.create(bl,H,{top:v});bi.append(bs);var bc=bu.create(bl,r,{height:w,top:v,left:m});bi.append(bc);bi.append(q.create(z));var be=bu.create(bh,H,{top:s});bi.append(be);var V=bu.create(bh,M,{width:E,top:s,left:y});bi.append(V);var U=bu.create(bh,r,{height:m,top:s,left:Q});bi.append(U);bi.append(q.create(O));var bs=bu.create(bj,H,{top:k});bi.append(bs);var bb=bu.create(bj,M,{width:E,top:k,left:y});bi.append(bb);var bc=bu.create(bk,r,{height:m,top:k,left:Q});bi.append(bc);bi.append(q.create(S));var bd=bu.create(bk,d,{width:I,height:I,top:C});bi.append(bd);var bf=bu.create(bk,n,{width:m,top:C,left:m});bi.append(bf);var bg=bu.create(bk,j,{height:m,top:C,left:p});bi.append(bg);bi.append(q.create(e));var bo=bu.create(Y,H,{width:i,height:g,top:J});bi.append(bo);var X=bu.create(Y,M,{width:D,top:J,left:I});bi.append(X);var W=bu.create(Y,r,{height:m,top:J,left:p});bi.append(W);bi.append(q.create(L));var bn=bu.create(bm,H,{width:P,height:G,top:A});bi.append(bn);var bp=bu.create(bm,r,{width:P,height:m,top:A,left:y});bi.append(bp);bi.append(q.create(K));bi.setStyle(a,R);}
}});}
)();
