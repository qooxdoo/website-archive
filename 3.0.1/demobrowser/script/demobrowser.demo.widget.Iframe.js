...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Iframe.dcb2a2b84577.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="resize",b="scroll",c="interval",d="horizontal",f="hidden",g="mousewheel",h="qx.ui.embed.ThemedIframe",i="_updateScrollbars",j="iframe",k="scrollbarX",l="scrollbarY",m="scrollbar-y",n="load",o="off",p="corner",q="on",r="scrollarea",s="scrollbar-x",t="auto",u="vertical";qx.Class.define(h,{extend:qx.ui.embed.AbstractIframe,include:qx.ui.core.scroll.MWheelHandling,construct:function(v){qx.ui.embed.AbstractIframe.call(this,v);var w=new qx.ui.layout.Grid();w.setColumnFlex(0,1);w.setRowFlex(0,1);this._setLayout(w);this._showChildControl(j);}
,properties:{appearance:{refine:true,init:r},scrollbarX:{check:[t,q,o],init:t,themeable:true,apply:i},scrollbarY:{check:[t,q,o],init:t,themeable:true,apply:i},scrollbar:{group:[k,l]}},members:{__wt:null,__wu:null,_getIframeElement:function(){return this.getChildControl(j).getContentElement();}
,_createChildControlImpl:function(B,A){var z;switch(B){case j:z=new qx.ui.embed.Iframe(this.getSource());z.addListener(n,this._onIframeLoad,this);z.addListener(a,this._onIframeResize,this);this._add(z,{row:0,column:0});break;case s:z=new qx.ui.core.scroll.ScrollBar(d);z.setMinWidth(0);z.exclude();z.addListener(b,this._onScrollBarX,this);this._add(z,{row:1,column:0});break;case m:z=new qx.ui.core.scroll.ScrollBar(u);z.setMinHeight(0);z.exclude();z.addListener(b,this._onScrollBarY,this);this._add(z,{row:0,column:1});break;case p:z=new qx.ui.core.Widget();z.setWidth(0);z.setHeight(0);z.exclude();this._add(z,{row:1,column:1});break;};return z||qx.ui.embed.AbstractIframe.prototype._createChildControlImpl.call(this,B);}
...
,_addWheelListener:function(){try{var D=this._getIframeElement().getBody();qx.bom.Element.addListener(D,g,this._onMouseWheel,this);}
catch(e){this._disableScollbars();}
;}
,_startIframeObserver:function(){if(this.__wu){this._stopIframeObserver();}
;var E=qx.event.Idle.getInstance();this.__wu=E.addListener(c,this._onIframeObserverInterval,this);}
,_stopIframeObserver:function(){this.__wt=null;if(!this.__wu){return;}
;var F=qx.event.Idle.getInstance();F.removeListenerById(this.__wu);}
,_onIframeObserverInterval:function(){var G=this._getIframeSize();if(!G){this._disableScollbars();return;}
;if(this.__wt&&G.width==this.__wt.width&&G.height==this.__wt.height){return;}
;this.__wt=G;this._preventIframeScrolling();this._updateScrollbars();}
,_preventIframeScrolling:function(){try{var H=this._getIframeElement().getWindow();var I=this._getIframeElement().getDocument();if(qx.bom.Document.isStandardMode(H)){I.documentElement.style.overflow=f;}
else {I.body.style.overflow=f;}
;}
catch(e){this._disableScollbars();}
;}
,_updateScrollbars:function(){var J=this.__wt;var M=this.getChildControl(j).getBounds();var K=this.getChildControl(j).getInnerSize();if(!J||!K||!K){return;}
;var L=false;var P=false;var N=this.getScrollbarX();var O=this.getScrollbarY();if(N===t&&O===t){var L=J.width>K.width;var P=J.height>K.height;if((L||P)&&!(L&&P)){if(L){P=J.height>M.height;}
else if(P){L=J.width>M.width;}
;}
;}
else {var L=N===q;var P=O===q;if(J.width>(L?M.width:K.width)&&N===t){L=true;}
;if(J.height>(L?M.height:K.height)&&O===t){P=true;}
;}
;this._configureScrollbar(s,L,K.width,J.width);this._configureScrollbar(m,P,K.height,J.height);this._updateCornerWidget();}
...
;}
,_configureScrollbar:function(S,T,W,V){if(!T){this._excludeChildControl(S);return;}
;var U=this._showChildControl(S);if(W>=V){U.set({position:0,maximum:V,knobFactor:1,enabled:false});}
else {U.setMaximum(1000000);U.set({position:Math.min(U.getPosition(),V),maximum:V-W,knobFactor:W/V,enabled:true});}
;}
...
;}
},destruct:function(){this._stopIframeObserver();this.__wt=null;}
});}
)();
