...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Touch.3e81ec9a3e28.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="touchmove",b="Two",c="Target Touches: ",d="px",f="==============",g="Changed Target Touches: ",h="<b>Cancel Touch</b>",j="Is Multi-Touch: ",k="green",l="div",m="Screen: Left = ",n="55px",o="30px",p="qx.debug",q="Identifier: ",r=" / Top = ",s="12px",t="All Touches: ",u="event.touch",v=":",w="Touch canceled",z="Document: Left = ",A="touchend",B="Viewport: Left = ",C='Lucida Grande',D="innerHTML",E="qx.mobile.emulatetouch",F="touchstart",G="__infoArea",H="demobrowser.demo.test.Touch",I="9px",J="Arial",K="engine.name",L="click",M="Type: ",N="20px",O="red",P="</b>",Q="webkit",R="<br>",S="touchcancel",T="90px",U="<b>This demo is supposed to be run in a WebKit-based browser on a touch-enabled device.</b>",V="Are all touch targets div ",W="One",X="absolute",Y="blue",bb="<b>Touch me:<br>";qx.Class.define(H,{extend:qx.application.Native,members:{__Rm:null,__Rn:null,main:function(){qx.application.Native.prototype.main.call(this);if((qx.core.Environment.get(p))){qx.log.appender.Native;qx.log.appender.Console;}
;var bd=new qx.html.Element(l);bd.useElement(document.body);bd.setRoot(true);if(qx.core.Environment.get(K)!=Q||(!qx.core.Environment.get(u)&&qx.core.Environment.get(E)==false)){var bc={"color":k,"position":X,"font-family":C,"font-size":s,"left":o,"top":N};var be=new qx.html.Element(l,bc);bd.add(be);be.setAttribute(D,U);return;}
;bd.add(this.__Ro(10,10,k,W));bd.add(this.__Ro(110,10,Y,b));this.__Rm=this.__Rp(10,110);bd.add(this.__Rm);this.__Rn=this.__Rp(110,110);bd.add(this.__Rn);bd.add(this.__Rq(210,10));}
,__Ro:function(x,y,bh,bi){var bf={"position":X,"left":x+d,"top":y+d,"width":T,"height":T,"backgroundColor":bh,"fontFamily":J,"fontSize":s};var bg=new qx.html.Element(l,bf,{"id":bi});bg.setAttribute(D,bb+bi+P);bg.addListener(F,this.__Rr,this);bg.addListener(a,this.__Rr,this);bg.addListener(A,this.__Rr,this);bg.addListener(S,this.__Rr,this);return bg;}
,__Rp:function(x,y){var bj={"position":X,"left":x+d,"top":y+d,"width":T,"fontFamily":J,"fontSize":I};return new qx.html.Element(l,bj);}
,__Rq:function(x,y){var bk={"position":X,"left":x+d,"top":y+d,"width":n,"height":n,"backgroundColor":O,"fontFamily":J,"fontSize":s};var bl=new qx.html.Element(l,bk);bl.setAttribute(D,h);bl.addListener(L,function(){window.setTimeout(function(){alert(w);}
,0);}
,this);return bl;}
,__Rr:function(e){var bm=[];bm.push(M+e.getType());bm.push(f);bm.push(j+e.isMultiTouch());bm.push(t+e.getAllTouches().length);bm.push(c+e.getTargetTouches().length);bm.push(g+e.getChangedTargetTouches().length);bm.push(f);bm.push(z+e.getDocumentLeft()+r+e.getDocumentTop());bm.push(m+e.getScreenLeft()+r+e.getScreenTop());bm.push(B+e.getViewportLeft()+r+e.getViewportTop());bm.push(q+e.getIdentifier());var bo=e.getTarget();var bn=false;var bq=e.getTargetTouches().length>0?e.getTargetTouches():e.getChangedTargetTouches();for(var i=0;i<bq.length;i++ ){bn=bq[0].target==bo;if(!bn){break;}
;}
;bm.push(V+bo.id+v+bn);var bp=this[G+bo.id];if(bp){bp.setAttribute(D,bm.join(R));}
;}
}});}
)();
