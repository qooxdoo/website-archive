...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.Window","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Window.9c86a4a1918b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="demobrowser.demo.widget.Window",b="Yes",c="Second Window",d="Page 1",f="bottom",g="Use move frame",h="Show Statusbar",j="icon/16/actions/dialog-cancel.png",k="icon/32/status/dialog-error.png",l="Show Close",m="changeValue",n="right",o="Third Window",p="icon/22/apps/utilities-calculator.png",q="Show Maximize",r="Allow Close",s="Use resize frame",t="No",u="resizable",v="icon/16/apps/office-calendar.png",w="Allow Maximize",x="value",y="The second window",z="execute",A="Page 3",B="move",C="Resizable ",D="First Window",E="Resized to: ",F="resize",G="Moveable",H="Demo loaded",I="Allow Minimize",J="Moved to: ",K="Open Modal Dialog 2",L="Do you want to fly to Berlin?",M="Open Modal Dialog 1",N="Resizable",O="top",P="Movable",Q="icon/32/apps/office-address-book.png",R="Sorry, please click 'Yes'!",S="Basics",T="Modal",U="icon/16/apps/internet-telephony.png",V="Welcome to your first own Window.<br/>Have fun!",W="Application is ready",X="icon/16/apps/internet-feed-reader.png",Y="Second Modal Dialog",bj="icon/16/actions/dialog-ok.png",bk="Page 2",bl="left",bg="x",bh="Show Minimize",bi="First Modal Dialog";qx.Class.define(a,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);this.createWindow1();this.createWindow2();this.createWindow3();}
,createWindow1:function(){var bo=new qx.ui.window.Window(D,v);bo.setLayout(new qx.ui.layout.VBox(10));bo.setShowStatusbar(true);bo.setStatus(H);bo.open();this.getRoot().add(bo,{left:20,top:20});bo.addListener(B,function(e){this.debug(J+e.getData().left+bg+e.getData().top);}
,this);bo.addListener(F,function(e){this.debug(E+e.getData().width+bg+e.getData().height);}
,this);var bn=new qx.ui.basic.Atom(V,Q);bn.setRich(true);bo.add(bn);var bm=new qx.ui.tabview.TabView;bo.add(bm,{flex:1});var br=new qx.ui.tabview.Page(d);bm.add(br);var bp=new qx.ui.tabview.Page(bk);bm.add(bp);var bq=new qx.ui.tabview.Page(A);bm.add(bq);}
,createWindow2:function(){var by=new qx.ui.window.Window(c,X);by.setLayout(new qx.ui.layout.VBox(10));by.setStatus(W);by.open();this.getRoot().add(by,{left:350,top:120});var bF=new qx.ui.basic.Atom(y,p);by.add(bF);var bs=new qx.ui.container.Composite;bs.setLayout(new qx.ui.layout.HBox(10));by.add(bs,{flex:1});var bA=new qx.ui.groupbox.GroupBox(S);bA.setLayout(new qx.ui.layout.VBox(4));bs.add(bA,{flex:1});var bv=new qx.ui.form.CheckBox(l);bv.setValue(true);bv.addListener(m,function(e){by.setShowClose(e.getData());}
);bA.add(bv);var bH=new qx.ui.form.CheckBox(q);bH.setValue(true);bH.addListener(m,function(e){by.setShowMaximize(e.getData());}
);bA.add(bH);var bI=new qx.ui.form.CheckBox(bh);bI.setValue(true);bI.addListener(m,function(e){by.setShowMinimize(e.getData());}
);bA.add(bI);var bC=new qx.ui.form.CheckBox(r);bC.setValue(true);bC.addListener(m,function(e){by.setAllowClose(e.getData());}
);bA.add(bC);var bJ=new qx.ui.form.CheckBox(w);bJ.setValue(true);bJ.addListener(m,function(e){by.setAllowMaximize(e.getData());}
);bA.add(bJ);var bx=new qx.ui.form.CheckBox(I);bx.setValue(true);bx.addListener(m,function(e){by.setAllowMinimize(e.getData());}
);bA.add(bx);var bt=new qx.ui.form.CheckBox(h);bt.setValue(false);bt.addListener(m,function(e){by.setShowStatusbar(e.getData());}
);bA.add(bt);var bK=new qx.ui.groupbox.GroupBox(N);bK.setLayout(new qx.ui.layout.VBox(4));bs.add(bK,{flex:1});var bE=new qx.ui.form.CheckBox(s);bE.setValue(true);bE.addListener(m,function(e){by.setUseResizeFrame(e.getData());}
);bK.add(bE);var bG=[O,n,f,bl];for(var i=0;i<bG.length;i++ ){var bD=bG[i];var bw=new qx.ui.form.CheckBox(C+bD).set({value:true});bw.bind(x,by,u+qx.lang.String.firstUp(bD));bK.add(bw);}
;var bz=new qx.ui.groupbox.GroupBox(G);bz.setLayout(new qx.ui.layout.VBox(4));bs.add(bz,{flex:1});var bB=new qx.ui.form.CheckBox(P);bB.setValue(true);bB.addListener(m,function(e){by.setMovable(e.getData());}
);bz.add(bB);var bu=new qx.ui.form.CheckBox(g);bu.addListener(m,function(e){by.setUseMoveFrame(e.getData());}
);bz.add(bu);}
,createWindow3:function(){var bL=new qx.ui.window.Window(o,U);bL.setLayout(new qx.ui.layout.VBox);bL.setMinWidth(200);bL.setMaxWidth(450);bL.setMaxHeight(400);bL.setAllowMaximize(false);bL.open();this.getRoot().add(bL,{left:100,top:250});var bM=this.getModalWindow1();var bN=new qx.ui.form.Button(M,v);bN.addListener(z,bM.open,bM);bL.add(bN);}
,getModalWindow1:function(){var bP=new qx.ui.window.Window(bi);bP.setLayout(new qx.ui.layout.VBox(10));bP.setModal(true);bP.moveTo(150,150);this.getRoot().add(bP);var bO=this.getModalWindow2();var bR=new qx.ui.form.Button(K,v);bR.addListener(z,bO.open,bO);bP.add(bR);var bQ=new qx.ui.form.CheckBox(T);bQ.setValue(true);bP.add(bQ);bQ.addListener(m,function(e){bP.setModal(e.getData());}
);return bP;}
,getModalWindow2:function(){var bT=new qx.ui.window.Window(Y);bT.setLayout(new qx.ui.layout.VBox(10));bT.setModal(true);bT.setShowClose(false);bT.moveTo(300,300);this.getRoot().add(bT);var bU=new qx.ui.basic.Atom(L,k);bT.add(bU);var bS=new qx.ui.container.Composite;bS.setLayout(new qx.ui.layout.HBox(10,n));bT.add(bS);var bW=new qx.ui.form.Button(b,bj);bW.addListener(z,function(e){bT.close();}
);bS.add(bW);var bV=new qx.ui.form.Button(t,j);bV.addListener(z,function(e){alert(R);}
);bS.add(bV);return bT;}
}});}
)();
