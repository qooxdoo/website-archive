...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.HtmlArea.08bcf939774f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
,this.__Nm);qx.dom.Element.insertBegin(ei,eh);return eh;}
,__Nu:function(){var ep=qx.dom.Element.create(c);qx.bom.element.Style.setStyles(ep,{"marginRight":cA,"float":di});var er;var eq=qx.dom.Element.create(cN,{title:cQ});if(qx.core.Environment.get(cU)==cD){qx.bom.element.Attribute.set(eq,cx,dT);}
...
;}
,this.__Nm);qx.dom.Element.insertBegin(eq,ep);return ep;}
,__Nw:function(){var ew=[{bold:{text:cO,image:dq,action:this.__Nm.setBold},italic:{text:u,image:dz,action:this.__Nm.setItalic},underline:{text:dI,image:w,action:this.__Nm.setUnderline},strikethrough:{text:dJ,image:cS,action:this.__Nm.setStrikeThrough},removeFormat:{text:cX,image:cH,action:this.__Nm.removeFormat}},{alignLeft:{text:K,image:dp,action:this.__Nm.setJustifyLeft},alignCenter:{text:dw,image:L,action:this.__Nm.setJustifyCenter},alignRight:{text:t,image:dl,action:this.__Nm.setJustifyRight},alignJustify:{text:O,image:dx,action:this.__Nm.setJustifyFull}},{fontFamily:{custom:this.__Nt},fontSize:{custom:this.__Nu},fontColor:{text:cJ,image:cL,action:this.__Nn},textBackgroundColor:{text:V,image:s,action:this.__No}},{indent:{text:m,image:C,action:this.__Nm.insertIndent},outdent:{text:dK,image:dH,action:this.__Nm.insertOutdent}},{insertImage:{text:dD,image:dg,action:this.__Np},insertTable:{text:b,image:cF,action:this.__Nq},insertLink:{text:X,image:de,action:this.__Nr},insertHTML:{text:dR,image:Q,action:this.__Ns},insertHR:{text:R,image:S,action:this.__Nm.insertHorizontalRuler}},{ol:{text:dm,image:B,action:this.__Nm.insertOrderedList},ul:{text:dO,image:T,action:this.__Nm.insertUnorderedList}},{undo:{text:dA,image:cG,action:this.__Nm.undo},redo:{text:dQ,image:dv,action:this.__Nm.redo}}];var ey,eC,ev,ez;for(var i=0,j=ew.length;i<j;i++ ){eC={};eC[cY]=i==j-1?dC:cA;eC[G]=di;ey=qx.dom.Element.create(c);qx.bom.element.Style.setStyles(ey,eC);for(var eA in ew[i]){var eB=ew[i][eA];if(eB.custom){ev=eB.custom.call(this);}
...
;if(name!=gc){if(qx.core.Environment.get(eK)==gM){if(name==fI&&iC.getAttribute(eG)){iB=iC.getAttribute(eG);}
...
;if(name==eG){continue;}
;if(!iB){continue;}
;if(name==fI&&iB==ic){continue;}
;if(name.charAt(0)===fN){continue;}
;iA[name]=iB;}
;if(iA.style!==undefined){ir=qx.bom.htmlarea.HtmlArea.__Ap(iA.style);delete iA.style;}
;if(iy){var iw={domElement:iC,tag:it,attributes:iA,styles:ir};iy(iw);iw.domElement=null;it=iw.tag;}
;if(it){iu.push(z,it);for(var name in iA){var iB=iA[name];iu.push(hh,name,gg,iB.toString().replace(new RegExp(hT,gN),fe),hT);}
;if(!qx.lang.Object.isEmpty(ir)){iu.push(fi);for(var name in ir){var iB=ir[name];iu.push(name,gA,iB.toString().replace(new RegExp(hT,gN),fe),fE);}
;iu.push(hT);}
;iu.push(closed?gb:fa);}
;}
;for(i=iC.firstChild;i;i=i.nextSibling){iu.push(qx.bom.htmlarea.HtmlArea.__Aq(i,true,iz,iy));}
;if(is&&!closed&&it){iu.push(gp,it,fa);}
;break;case 3:iu.push(iz?iC.data:qx.bom.htmlarea.HtmlArea.__As(iC.data));break;case 8:iu.push(J,iC.data,fM);break;};return iu.join(fD);}
,closingTags:fn,__Ar:function(iD){return (qx.bom.htmlarea.HtmlArea.closingTags.indexOf(hh+iD.tagName+hh)!=-1);}
,__As:function(s){s=s.replace(/&/ig,gw);s=s.replace(/</ig,fc);s=s.replace(/>/ig,gf);s=s.replace(/\x22/ig,hJ);s=s.replace(/\xA9/ig,R);return s;}
...
},properties:{contentType:{check:hV,init:ha},messengerMode:{check:gY,init:false},insertParagraphOnLinebreak:{check:gY,init:true},insertLinebreakOnCtrlEnter:{check:gY,init:true},postProcess:{check:t,nullable:true,init:null},useUndoRedo:{check:gY,init:true},nativeContextMenu:{check:gY,init:false},defaultFontFamily:{check:hV,init:fQ},defaultFontSize:{check:hW,init:4}},members:{__lo:null,__cX:false,__At:false,__Au:false,__Av:false,__Aw:false,__An:null,__Ax:null,__Ay:null,__Az:null,__rd:null,__Am:null,__AA:null,__AB:null,__AC:false,__AD:false,__AE:function(iH){if(qx.dom.Node.isElement(iH)&&qx.dom.Node.isNodeName(iH,p)){this.__lo=iH;}
;}
,_createAndAddIframe:function(iI){this.__rd=qx.bom.Iframe.create();qx.bom.Iframe.setSource(this.__rd,iI);if((qx.core.Environment.get(eK)==gM)){qx.bom.element.Attribute.set(this.__rd,gR,fm);}
else {qx.bom.element.Style.set(this.__rd,hg,he);}
;qx.bom.element.Style.setStyles(this.__rd,{width:gq,height:gq});qx.dom.Element.insertBegin(this.__rd,this.__lo);}
,_getIframeDocument:function(){return qx.bom.Iframe.getDocument(this.__rd);}
,_getIframeWindow:function(){return qx.bom.Iframe.getWindow(this.__rd);}
,_addIframeLoadListener:function(){qx.event.Registration.addListener(this.__rd,C,this._loaded,this);}
,__AF:function(){this.__AA={"xhtml":{doctype:hM+gC+hI+hc+eX+hU,html:hi+M,meta:hj,style:qx.core.Environment.select(eK,{"mshtml":eH+fW,"default":fL+gK}),contentStyle:fX,body:gP,footer:fs}};}
,__zX:fD,getIframeObject:function(){return this.__rd;}
,getCommandManager:function(){return this.__An;}
,setValue:function(iK){if(qx.lang.Type.isString(iK)){this.__zX=iK;var iJ=this._getIframeDocument();if(iJ&&iJ.body){iJ.body.innerHTML=this.__AG(iK);}
;}
;}
,__AG:function(iO){var iN=fD;if((qx.core.Environment.get(eK)==hs)){if(qx.core.Environment.get(gF)<=2){iN+=fu;}
;}
;var iP=iO.length==0?hR:fD;var iM=qx.core.Environment.get(eK)==hs||qx.core.Environment.get(eK)==fO?F:fl;var iL=gm+hC+this.getDefaultFontFamily()+gE+E+iM+K+this.getDefaultFontSize()+gE+iN+iO+iP+hk+c+ht;return iL;}
,getValue:function(){return this.__zX;}
,getComputedValue:function(iQ){return this.getHtml(iQ);}
,getCompleteHtml:function(){var iS=this.__AA[this.getContentType()];var iR=iS.html+gn+iS.meta+hX+iS.contentStyle+hS+gl;iR+=fC+this.__AH()+N;iR+=this.getHtml()+fs;return iR;}
,__AH:function(){var iW=[d,hw,gt,H,B,v,hu,gU,gG,eO,hr,fw,gh,fK];var iU=qx.bom.element.Style;var iT=this.getContentBody();var iY={};var iV,ja;var iX=qx.core.Environment.get(eK)==gM?2:1;for(var i=0,j=iW.length;i<j;i++ ){iV=iW[i];ja=iU.get(iT,iV,iX);if(ja!==undefined&&ja!=fD){iY[iV]=ja;}
;}
...
;if(!jb){return [];}
;var jd=jb.cloneNode(true);var jf=jd.innerHTML;jf=jf.replace(/>/gi,hB);jf=jf.replace(/\n/gi,hh);jf=jf.replace(/<!--.*-->/gi,fD);jd.innerHTML=jf;var jc=qx.core.Environment.get(eK)==gM||qx.core.Environment.get(eK)==P?jd.innerText:jd.textContent;var je=jc.match(qx.bom.htmlarea.HtmlArea.GetWordsRegExp);return !je?[]:je;}
...
;}
;return jp;}
,__AI:0,__AJ:function(){var jr=this._getIframeDocument();if(!jr){this.__AI++ ;if(this.__AI>5){this.error(hF+jr);this.fireDataEvent(fR);}
else {if(qx.core.Environment.get(fb)){this.error(D);}
;qx.event.Timer.once(function(){this.__AJ();}
,this,0);}
;}
else {this.__AI=0;this._onDocumentIsReady();}
;}
,_loaded:function(e){if(this.__Au){return;}
;if(this.__At){this.__AK();return;}
;if((qx.core.Environment.get(eK)==gM)){this.__AJ();}
...
;this.__Au=true;if((qx.core.Environment.get(eK)==gM)){this.setEditable(true);}
;this.__AN();if(!(qx.core.Environment.get(eK)==P)){this.__zq();}
;if(!(qx.core.Environment.get(eK)==gM)){this.setEditable(true);}
;this.__cX=true;this.__An=js;js.setContentDocument(this._getIframeDocument());this.__AS();if((qx.core.Environment.get(eK)==P)){this.__zq();}
;this.fireEvent(gH);}
...
else {jt.body.contentEditable=true;this.__AT(true);}
;}
;}
,"default":(function(){}
)}),invalidateEditor:qx.core.Environment.select(eK,{"gecko":function(){this.__Au=false;this.__cX=false;this.__At=true;}
,"default":function(){}
}),__AK:qx.core.Environment.select(eK,{"gecko":function(){this.__AN();this.__zq();this.__An.setContentDocument(this._getIframeDocument());this.setEditable(true);this.forceEditable();this.__Au=true;this.__cX=true;this.__At=false;this.fireEvent(gd);}
,"default":function(){}
}),__AL:function(jw){var jv=fD;if(!jw){return jv;}
;try{var ju=jw.getAttribute(gc);if(!ju){return jv;}
;if((qx.core.Environment.get(eK)==gM)){jv=ju.cssText;}
else {jv=ju;}
;}
catch(jx){this.error(fY);}
;return jv;}
,__AM:function(jB){var jA=qx.core.Environment.get(eK)==hs?gO:fD;var jz=this.__AA[this.getContentType()];var jy=gn+jz.meta+hX+jA+jz.style+jz.contentStyle+this.__Am+hS+gl;var content=jz.body+jB;return jz.html+jy+content+jz.footer;}
,__AN:function(){var jD=this.__AG(this.getValue());if(qx.lang.Type.isString(jD)){var jC=this._getIframeDocument();try{jC.open(eP,true);jC.write(this.__AM(jD));jC.close();}
catch(e){this.error(x+qx.bom.Iframe.queryCurrentUrl(this.__rd)+fe,e);this.fireDataEvent(fR,e);}
;}
;}
,__zq:function(){this.__AO();this.__AQ();this.__AP();}
,__AO:function(){var jF=qx.event.Registration;var jE=this._getIframeDocument();jF.addListener(jE.body,gS,this._handleKeyPress,this);jF.addListener(jE.body,fx,this._handleKeyUp,this);jF.addListener(jE.body,fy,this._handleKeyDown,this);}
,__AP:function(){var jI=qx.event.Registration;var jG=this._getIframeDocument();var jH=qx.core.Environment.get(eK)==fO?this._getIframeWindow():jG.body;jI.addListener(jH,hK,this._handleFocusEvent,this);jI.addListener(jH,eD,this._handleBlurEvent,this);jI.addListener(jG,fS,this._handleFocusOutEvent,this);}
,__AQ:function(){var jL=qx.event.Registration;var jK=this._getIframeDocument();var jJ=qx.core.Environment.get(eK)==gM?ia:W;jL.addListener(jK.body,jJ,this._handleMouseUpOnBody,this);jL.addListener(jK.documentElement,jJ,this._handleMouseUpOnDocument,this);jL.addListener(jK.documentElement,fT,this._handleContextMenuEvent,this);}
,__AR:function(){if(this.__Ax==null){this.__Ax={execute:function(jM,jN){this.stackedCommands=true;this.commandStack.push({command:jM,value:jN});}
,commandStack:[],stackedCommands:false};}
;this.__Ax.stackedCommands=false;return this.__Ax;}
,__AS:function(){var jO=this.__Ax;if(jO!=null&&jO.stackedCommands){var jP=jO.commandStack;if(jP!=null){for(var i=0,j=jP.length;i<j;i++ ){this.__An.execute(jP[i].command,jP[i].value);}
;}
;}
;}
,__AT:function(jQ){var jR=this._getIframeDocument();if(this.__Au&&jR){try{if((qx.core.Environment.get(eK)==hs)){jR.designMode=(jQ!==false)?gx:u;}
;jR.designMode=(jQ!==false)?u:gx;}
catch(e){}
;}
;}
,setEditable:function(jT){if(this.__Au){this.__AT(true);if((qx.core.Environment.get(eK)==hs)){try{var jS=this._getIframeDocument();jS.execCommand(ge,false,true);}
...
;this.__Av=jT;}
;}
,getEditable:function(){return this.__Av;}
,isEditable:function(){return this.__Av;}
,__AU:false,_handleKeyUp:function(e){var jW=e.getKeyIdentifier().toLowerCase();this.__Ay=e;if((qx.core.Environment.get(fb))&&qx.core.Environment.get(eF)){this.debug(e.getType()+hP+jW);}
;if(qx.core.Environment.get(eK)==gM||qx.core.Environment.get(eK)==fO){if(this.__AU){switch(jW){case X:if(this.getInsertLinebreakOnCtrlEnter()){if((qx.core.Environment.get(eK)==fO)){this.__AV();e.preventDefault();e.stopPropagation();}
else {var jX=this.__Bg(this.getSelection());if(jX){jX.collapse(true);jX.pasteHTML(ft);}
;}
;this.__Bb();}
;break;case eM:this.__AU=false;return;break;};}
;}
else if((qx.core.Environment.get(eK)==hs)){switch(jW){case hd:case q:case eW:case gQ:case g:case hD:case hY:case n:case T:this.__Aw=(this.getFocusNode()==this.getContentBody().firstChild);break;};}
;}
,__AV:function(){var jY=this.getSelection();var ka=fD;if(jY&&(jY.focusNode.textContent==fD||jY.focusNode.parentElement.tagName==hy)){ka=o;}
;this.__An.execute(fU,ka+qx.bom.htmlarea.HtmlArea.simpleLinebreak);}
...
;if(this.__AU&&(kb==fo||kb==Y||kb==fv||kb==hQ||kb==L||kb==gs)){e.preventDefault();e.stopPropagation();}
;if(kb==eM){this.__AU=true;}
;}
,"default":function(e){}
}),_handleKeyPress:function(e){var kg=this.getContentDocument();var kc=e.getKeyIdentifier().toLowerCase();var ke=e.isCtrlPressed();var kf=e.isShiftPressed();this.__Ay=e;if((qx.core.Environment.get(fb))&&qx.core.Environment.get(eF)){this.debug(e.getType()+hP+kc);}
;if(this.__AC){var km=!((qx.core.Environment.get(eK)==gM)&&kc==X)||!((qx.core.Environment.get(eK)==hs)&&kc==X);if(km){this.__Bb();this.__AC=false;}
;}
;switch(kc){case X:if(!kf&&!ke&&this.getMessengerMode()){e.preventDefault();e.stopPropagation();this.fireDataEvent(fh,this.getComputedValue());this.resetHtml();}
;if(ke){if(!this.getInsertLinebreakOnCtrlEnter()){return;}
;e.preventDefault();e.stopPropagation();if((qx.core.Environment.get(eK)==hs)){if(this.__Be()){this.insertHtml(eJ);this.__Bb();return;}
;this.insertHtml(eV);}
else if((qx.core.Environment.get(eK)==P)){var kj=this.getSelection();var kh=this.__Bg(kj);if(kj&&kh){var ki=kg.createElement(hl);kh.collapse(true);kh.insertNode(ki);kh.collapse(true);kh.selectNode(ki);kj.addRange(kh);kh.collapse(true);}
;}
;this.__Bb();}
;if((qx.core.Environment.get(eK)==gM)){if(!this.getInsertParagraphOnLinebreak()){if(this.__An.execute(fU,qx.bom.htmlarea.HtmlArea.simpleLinebreak)){this.__Bb();e.preventDefault();e.stopPropagation();}
;}
;}
else if((qx.core.Environment.get(eK)==hs)){if(this.getInsertParagraphOnLinebreak()&&!kf&&!ke){var kj=this.getSelection();if(kj){var kl=kj.focusNode;if(this.__Be()){this.__Bb();return;}
;if(this.__Bf()){this.__Bb();return;}
;while(!qx.dom.Node.isNodeName(kl,gV)){if(qx.dom.Node.isNodeName(kl,f)){this.__Bb();return;}
;kl=kl.parentNode;}
;}
;this.__An.insertParagraphOnLinebreak();e.preventDefault();e.stopPropagation();this.__Bb();this.__AC=true;}
;}
else if((qx.core.Environment.get(eK)==fO)){if(this.getInsertParagraphOnLinebreak()&&kf){this.__AV();e.preventDefault();e.stopPropagation();this.__Bb();}
;}
;break;case eW:if(qx.core.Environment.get(eK)==hs&&qx.core.Environment.get(k)<1.9&&kf){var kj=this.getSelection();if(kj&&kj.focusNode==kg.body.firstChild){if(this.__Aw){if(kj.focusOffset!=0){kj.extend(kj.focusNode,0);}
;}
;}
;}
;this.__Bb();break;case y:if(qx.core.Environment.get(eK)==hs&&qx.core.Environment.get(k)<1.9){if(ke){var kj=this.getSelection();if(kf){if(kj&&(kj.focusOffset!=0)||(kj.focusNode!=kg.body.firstChild)){kj.extend(kg.body.firstChild,0);}
;}
...
;}
;}
;this.__Bb();break;case hd:case q:case gQ:case g:case hD:case hY:case n:case T:this.__Bb();break;case fv:if(ke){this.__AW(Q,true);}
;break;case L:case gs:if(ke){this.__AW(hN,true);}
;break;case hQ:if(ke){this.__AW(O,true);}
;break;case fo:if(ke&&!kf){this.__AW(h,true);}
else if(ke&&kf){this.__AW(gB,true);}
;break;case Y:if(ke){this.__AW(gB,true);}
;break;case gv:if(ke){this.selectAll();}
;break;};this.__Ay=null;}
,__AW:function(ko,kr){var kq=null;var kp=qx.bom.htmlarea.HtmlArea.hotkeyInfo;if(kp[ko]){kq=kp[ko].method;}
;if(kq!=null&&this[kq]){this[kq]();if(kr){this.__Ay.preventDefault();this.__Ay.stopPropagation();}
;if(this.isSelectionCollapsed()){this.__AC=true;}
;this.__Bb();}
;}
,_handleFocusEvent:function(e){this.__Az=null;if(qx.core.Environment.get(eK)==hs||qx.core.Environment.get(eK)==fO){var ks=this.getContentDocument().getElementById(ic);if(ks){qx.bom.element.Attribute.reset(ks,fI);}
;}
;this.fireEvent(A);}
,_handleBlurEvent:function(e){this.__zX=this.getComputedValue();}
,_handleFocusOutEvent:function(e){this.__AU=false;if(this.__Az==null){this.__Az=this.getSelectedHtml();}
;this.fireEvent(hp);}
,_handleMouseUpOnBody:function(e){if((qx.core.Environment.get(fb))&&qx.core.Environment.get(eF)){this.debug(hm+e.getType());}
;this.__AD=true;this.__Bb();}
,_handleMouseUpOnDocument:qx.core.Environment.select(eK,{"mshtml":(function(){}
),"default":function(e){if(!this.__AD){qx.bom.Element.activate(this.getContentBody());}
;this.__AD=false;}
}),_handleContextMenuEvent:function(e){if(!this.getNativeContextMenu()){var ku=e.getViewportLeft();var kt=e.getViewportTop();var kv=qx.bom.element.Location.getLeft(this.__lo)+ku;var kw=qx.bom.element.Location.getTop(this.__lo)+kt;var kx={x:kv,y:kw,relX:ku,relY:kt,target:e.getTarget()};e.preventDefault();e.stopPropagation();qx.event.Timer.once(function(){this.fireDataEvent(fT,kx);}
,this,0);}
;}
,isLoaded:function(){return this.__Au;}
,insertHtml:function(ky){return this.__An.execute(fU,ky);}
,removeFormat:function(){var kz=this.__An.execute(gy);this.__An.execute(hf,this.getDefaultFontSize());this.__An.execute(fz,this.getDefaultFontFamily());return kz;}
,setBold:function(){return this.__An.execute(V);}
,setItalic:function(){return this.__An.execute(gD);}
,setUnderline:function(){return this.__An.execute(hq);}
,setStrikeThrough:function(){return this.__An.execute(eS);}
,setFontSize:function(kA){return this.__An.execute(hf,kA);}
,setFontFamily:function(kB){return this.__An.execute(fz,kB);}
,setTextColor:function(kC){return this.__An.execute(w,kC);}
,setTextBackgroundColor:function(kD){return this.__An.execute(gk,kD);}
,setJustifyLeft:function(){return this.__An.execute(hn);}
,setJustifyCenter:function(){return this.__An.execute(eY);}
,setJustifyRight:function(){return this.__An.execute(fH);}
,setJustifyFull:function(){return this.__An.execute(eL);}
,insertIndent:function(){return this.__An.execute(fj);}
,insertOutdent:function(){return this.__An.execute(fF);}
,insertOrderedList:function(){return this.__An.execute(ib);}
,insertUnorderedList:function(){return this.__An.execute(ff);}
,insertHorizontalRuler:function(){return this.__An.execute(gr);}
,insertImage:function(kE){return this.__An.execute(gu,kE);}
,insertHyperLink:function(kF){return this.__An.execute(eI,kF);}
,removeBackgroundColor:function(){return this.__An.execute(l,fr);}
,setBackgroundColor:function(kG){return this.__An.execute(l,kG);}
,removeBackgroundImage:function(){return this.__An.execute(go);}
,setBackgroundImage:function(kH,kJ,kI){return this.__An.execute(go,[kH,kJ,kI]);}
,selectAll:function(){return this.__An.execute(fq);}
,undo:function(){if(this.getUseUndoRedo()){return this.__An.execute(hb);}
else {return true;}
;}
,redo:function(){if(this.getUseUndoRedo()){return this.__An.execute(I);}
else {return true;}
;}
,resetHtml:function(){var kK=this._getIframeDocument();while(kK.body.firstChild){kK.body.removeChild(kK.body.firstChild);}
;if(qx.core.Environment.get(eK)==hs){kK.body.innerHTML=gI;}
...
;return qx.bom.htmlarea.HtmlArea.__Aq(kO.body,false,kN,this.getPostProcess());}
,containsOnlyPlaceholder:qx.core.Environment.select(eK,{"mshtml":function(){var kP=this._getIframeDocument();return (kP.body.innerHTML==gj);}
...
;}
,"webkit":function(){qx.bom.Element.focus(this.getContentWindow());qx.bom.Element.focus(this.getContentBody());var kV=this.getContentDocument().getElementById(ic);if(kV){qx.bom.element.Attribute.reset(kV,fI);}
;this.__AX();}
,"opera":function(){qx.bom.Element.focus(this.getContentWindow());qx.bom.Element.focus(this.getContentBody());this.__AX();}
,"default":function(){qx.bom.Element.focus(this.getContentBody());this.__AX();}
}),__AX:function(){if(!this.__AY()){this.__Ba();}
;}
,__AY:qx.core.Environment.select(eK,{"gecko":function(){var kW=this.getContentBody().childNodes;if(kW.length==0){return false;}
else if(kW.length==1){return !(kW[0]&&qx.dom.Node.isNodeName(kW[0],hl)&&qx.bom.element.Attribute.get(kW[0],fd)!=null);}
else {return true;}
;}
...
;}
}),__Ba:qx.core.Environment.select(eK,{"gecko|webkit":function(){this.getContentDocument().body.innerHTML=this.__AG(fD);var la=this.getContentDocument().getElementById(ic);qx.bom.element.Attribute.reset(la,fI);this._selectElement(la);}
,"default":function(){var lb=qx.dom.Hierarchy.getFirstDescendant(this.getContentBody());if(qx.dom.Node.isNodeName(lb,hO)){qx.bom.element.Style.set(lb,gL,this.getDefaultFontFamily());qx.bom.element.Style.set(lb,eQ,this.getDefaultFontSize());}
;}
}),getContextInformation:function(){return this.__Bc();}
,__Bb:function(){qx.event.Timer.once(function(e){var lc=this.__Bc();this.fireDataEvent(U,lc);}
,this,200);}
,__Bc:function(){if(this._processingExamineCursorContext||this.getEditable()==false){return null;}
;this._processingExamineCursorContext=true;if(!this.__AY()){this.__Ba();}
;var lg=this.getFocusNode();if(lg==null){return null;}
;if(qx.dom.Node.isText(lg)){lg=lg.parentNode;}
;var lk=this._getIframeDocument();var lt=(qx.core.Environment.get(eK)==gM)?lg.currentStyle:lk.defaultView.getComputedStyle(lg,null);var lm=false;var ld=false;var li=false;var lh=false;var ll=false;var ls=false;var lj=false;var lu=false;var lf=false;var lp=false;var lq=null;var lr=null;var ln=null;if(lt!=null){if((qx.core.Environment.get(eK)==gM)){ld=lt.fontStyle==gD;li=lt.textDecoration.indexOf(hq)!==-1;lh=lt.textDecoration.indexOf(fp)!==-1;lq=lt.fontSize;ln=lt.fontFamily;lj=lt.textAlign==hd;lu=lt.textAlign==hv;lf=lt.textAlign==q;lp=lt.textAlign==hx;}
else {ld=lt.getPropertyValue(gW)==gD;li=lt.getPropertyValue(ho).indexOf(hq)!==-1;lh=lt.getPropertyValue(ho).indexOf(fp)!==-1;lq=lt.getPropertyValue(eQ);ln=lt.getPropertyValue(gL);lj=lt.getPropertyValue(hH)==hd;lu=lt.getPropertyValue(hH)==hv;lf=lt.getPropertyValue(hH)==q;lp=lt.getPropertyValue(hH)==hx;}
;if(qx.core.Environment.get(eK)==gM||qx.core.Environment.get(eK)==P){lm=lt.fontWeight==700;}
else {lm=lt.getPropertyValue(hz)==V||qx.dom.Node.isNodeName(lg,fv);}
;}
;var le=lg;if(le!=null&&le.parentNode!=null&&!qx.dom.Node.isDocument(le.parentNode)){while(le!=null&&!qx.dom.Node.isNodeName(le,gV)){var lv=qx.dom.Node.getName(le);if(lv==fJ){ls=true;break;}
else if(lv==S){ll=true;break;}
;if(lr==null||lr==fD){lr=qx.bom.element.Attribute.get(le,gT);}
;le=le.parentNode;}
;}
...
}),getSelectedHtml:function(){if(this.__Az!=null){return this.__Az;}
;var lw=this.getRange();if(!lw){return fD;}
else {return this.__Bd(lw);}
;}
,__Bd:qx.core.Environment.select(eK,{"mshtml":function(lx){if(!lx){return fD;}
...
;}
}),__Be:qx.core.Environment.select(eK,{"gecko":function(){var lE=this.getSelection();var lF=this.getFocusNode();return lE&&this.isSelectionCollapsed()&&lF!=null&&qx.dom.Node.isText(lF)&&lE.anchorOffset<lF.length;}
,"default":function(){return false;}
}),__Bf:qx.core.Environment.select(eK,{"gecko":function(){return qx.dom.Node.isElement(this.getFocusNode());}
,"default":function(){return false;}
}),getRange:function(){return this.__Bg(this.getSelection());}
,__Bg:qx.core.Environment.select(eK,{"mshtml":function(lH){var lG=this._getIframeDocument();if(lH){try{return lH.createRange();}
...
;}
}),saveRange:qx.core.Environment.select(eK,{"mshtml":function(){this.__AB=this.getRange();}
,"default":function(){}
}),getSavedRange:qx.core.Environment.select(eK,{"mshtml":function(){return this.__AB;}
,"default":function(){}
}),resetSavedRange:qx.core.Environment.select(eK,{"mshtml":function(){this.__AB=null;}
,"default":function(){}
}),getFocusNode:qx.core.Environment.select(eK,{"mshtml":function(){var lM=this.getSelection();var lN;switch(lM.type){case fg:case eR:lN=this.__Bg(lM);if(lN){lN.collapse(false);return lN.parentElement();}
else {return null;}
;break;case hE:lN=this.__Bg(lM);if(lN){try{lN.collapse(false);}
catch(lO){}
;return lN.item(0);}
else {return null;}
...
})},environment:{"qx.bom.htmlarea.HtmlArea.debug":false},destruct:function(){try{var lR=this._getIframeDocument();var lT=qx.event.Registration;lT.removeListener(lR.body,gS,this._handleKeyPress,this);lT.removeListener(lR.body,fx,this._handleKeyUp,this);lT.removeListener(lR.body,fy,this._handleKeyDown,this);var lS=qx.core.Environment.get(eK)==fO?this._getIframeWindow():lR.body;lT.removeListener(lS,hK,this._handleFocusEvent);lT.removeListener(lS,eD,this._handleBlurEvent);lT.removeListener(lR,fS,this._handleFocusOutEvent);var lQ=qx.core.Environment.get(eK)==gM?ia:W;lT.removeListener(lR.body,lQ,this._handleMouseUpOnBody,this);lT.removeListener(lR.documentElement,lQ,this._handleMouseUpOnDocument,this);lT.removeListener(lR.documentElement,fT,this._handleContextMenuEvent,this);if((qx.core.Environment.get(eK)==gM)){qx.bom.Iframe.setSource(this.__rd,gJ);}
;}
catch(lU){}
;if(this.__An instanceof qx.core.Object){this._disposeObjects(hL);}
else {this.__An=null;}
;this.__AA=this.__rd=this.__lo=this.__Ax=null;}
});}
)();
...
(function(){var a="__Bt",b="backgroundColor",c="__BW",d="top",f="/>",g="formatBlock",h=';',m="insertParagraph",n=" succeded",o="_moz",p='<span style="',q="__BO",r="underline",s="<img ",t="' is not allowed for parameter 'position'. Possible values are '",u="no-repeat",v="</font>",w=' <a href="',x="FONT",y="inserthyperlink failed!",z="backgroundPosition",A="|top|bottom|center|left|right|right top|left top|left bottom|right bottom|",B="__placeholder__",C="JustifyCenter",D="transparent",E="Cut",F="outdent",G="__BT",H='</a> ',I="InsertImage",J="font-size",K="__resetHtml",L="FontName",M="p",N="bold",O="blockquote",P="li",Q="If an array is provided for parameter 'position' it has to define two elements!",R="inserthtml",S="span",T='|',U="id",V="style",W='color',X="__BU",Y="The method '",dM="qx.bom.htmlarea.manager.Command",dH="'><br class='webkit-block-placeholder' />",dN="string",dJ="a",dK='"',dG="<span></span>",dL="__BL",dR="background-color",dS="backgroundRepeat",dT="type",dU="font",dO="CreateLink",dP=":",dI="InsertHtml",dQ="__BD",dY="':'",eD="='",ea=" with value ",eb="italic",dV='">',dW="' you calling to execute the command '",fE="font-family",dX="mshtml",ec="qx_link",ed="none",ee='"></span>',ej="Italic",ek="fontStyle",el=" ",ef=";",eg=" failed",eh="InsertUnorderedList",ei="__BR",eq="body",er="OL",es='text-align:',et="Bold",em="ForeColor",en="child",eo="px",ep="url(",ey='" ',ez="repeat repeat-x repeat-y no-repeat",fI='</p>',eA="gecko",eu="<hr />",ev="legacy-font-size",fG="backgroundImage",ew="editor not ready! '",eB='<font style="',eC="text-align",eO="Command ",eN="Control",eM="IMG",eS="__paragraph__",eR='<p id="',eQ="StrikeThrough",eP='text-decoration',eH="LI",eG="__getHtml",eF='>',eE="__BM",eL='font-family:',eK='" size="',eJ="Indent",eI=':',fa="HiliteColor",eY="br",eX="fontWeight",eW="Copy",fe="</span>",fd="engine.name",fc="Paste",fb="The value '",eV="BackColor",eU="qx.bom.htmlarea.HtmlArea.debug",eT="__BC",fp="__BP",fo="RemoveFormat",fn="text-decoration",ft="JustifyRight",fs="__Bx",fr="size",fq="__BQ",fi="FontSize",fh="' is not allowed for parameter 'repeat'. Possible values are '",fg="' ",ff='',fm="indent",fl='style="',fk="execCommand ",fj="'",fz="' is not available!",fy="_moz_dirty",fx="color",fw="qx.debug",fD="JustifyLeft",fC="textDecoration",fB=" is currently not supported!",fA="Underline",fv="SelectAll",fu="<p style='",dC="line-through",dB="BODY",fJ=")",dz="__BV",dA="",dy="href",fH='<span id="',dw="__By",dx="__BN",dv="UL",fF="JustifyFull",dt="Outdent",du="webkit",ds="selectall",dE="__BA",dF="InsertOrderedList",dD="undefined";qx.Class.define(dM,{extend:qx.core.Object,construct:function(fK){qx.core.Object.call(this);this.__Bh=fK;this.__nr=null;this._commands=null;this.__Bo();this.__Bi={"Bold":true,"Italic":true,"Underline":true,"StrikeThrough":true};this.__Bj=[10,12,16,18,24,32,48];this.__Bk=0;}
,statics:{__Bl:A,__Bm:ez},members:{__nr:null,__Bh:null,__Bn:false,__Bi:null,__Bj:null,__Bk:null,setContentDocument:function(fL){this.__nr=fL;}
,getCommandObject:function(fM){if(this._commands[fM]){return this._commands[fM];}
else {return null;}
;}
,__Bo:function(){this._commands={bold:{useBuiltin:false,identifier:et,method:G},italic:{useBuiltin:false,identifier:ej,method:X},underline:{useBuiltin:false,identifier:fA,method:dz},strikethrough:{useBuiltin:false,identifier:eQ,method:c},fontfamily:{useBuiltin:true,identifier:L,method:null},fontsize:{useBuiltin:false,identifier:fi,method:dL},textcolor:{useBuiltin:true,identifier:em,method:null},textbackgroundcolor:{useBuiltin:false,identifier:null,method:eE},backgroundcolor:{useBuiltin:false,identifier:null,method:dx},backgroundimage:{useBuiltin:false,identifier:null,method:q},justifyleft:{useBuiltin:false,identifier:fD,method:fs},justifyright:{useBuiltin:false,identifier:ft,method:fs},justifycenter:{useBuiltin:false,identifier:C,method:fs},justifyfull:{useBuiltin:false,identifier:fF,method:fs},indent:{useBuiltin:true,identifier:eJ,method:null},outdent:{useBuiltin:true,identifier:dt,method:null},copy:{useBuiltin:true,identifier:eW,method:null},cut:{useBuiltin:true,identifier:E,method:null},paste:{useBuiltin:true,identifier:fc,method:null},insertorderedlist:{useBuiltin:false,identifier:dF,method:dw},insertunorderedlist:{useBuiltin:false,identifier:eh,method:dw},inserthorizontalrule:{useBuiltin:false,identifier:dI,method:dQ},insertimage:{useBuiltin:false,identifier:I,method:dE},inserthyperlink:{useBuiltin:false,identifier:dO,method:eT},selectall:{useBuiltin:false,identifier:fv,method:fp},selectedtext:{useBuiltin:false,identifier:null,method:fq},selectedhtml:{useBuiltin:false,identifier:null,method:ei},inserthtml:{useBuiltin:false,identifier:dI,method:a},resethtml:{useBuiltin:false,identifier:null,method:K},gethtml:{useBuiltin:false,identifier:null,method:eG},removeformat:{useBuiltin:true,identifier:fo,method:null}};}
,execute:function(fN,fP){if(!this.__Bh.isReady()){this.error(ew+fN+dY+fP+fj);return false;}
;fN=fN.toLowerCase();fP=fP!=null?fP:null;if(this._commands[fN]){var fO;var fQ=this._commands[fN];if(!((qx.core.Environment.get(fd)==du)&&(fN==fm||fN==F))){if(this.__Bp()){this.__Bq();}
;}
;if(fQ.useBuiltin){fO=this.__Br(fQ.identifier,false,fP);}
else {if(fQ.method!=null&&this[fQ.method]){fO=this[fQ.method].call(this,fP,fQ);}
else {this.error(Y+fQ.method+dW+fN+fz);}
;}
;this.__Bh.resetSavedRange();return fO;}
else {this.error(eO+fN+fB);}
;}
,__Bp:function(){var Node=qx.dom.Node;var fR=this.__Bh.getFocusNode();var fU=false;var fS=false;if(fR){if(Node.isText(fR)){var fT=qx.dom.Hierarchy.getAncestors(fR);for(var i=0,j=fT.length;i<j;i++ ){if(Node.isNodeName(fT[i],M)||qx.bom.htmlarea.HtmlArea.isHeadlineNode(fT[i])){fU=true;break;}
;}
;}
else if(Node.isNodeName(fR,eq)){fS=true;}
;}
;return fS||!fU;}
,__Bq:function(){this.__Br(g,false,M);}
,__Br:function(gc,ga,fX){try{var fW=this.__nr;var fV=false;var fY=this.__Bh.getRange();this.__nr.body.focus();if((qx.core.Environment.get(fd)==dX)){if(gc!=ds){fY.select();if(((fY.text)&&(fY.text.length>0))||((fY.length==1)&&(fY.item(0))&&(fY.item(0).tagName==eM))){fW=fY;}
else {fW=this.__nr;}
;}
;if(((qx.core.Environment.get(fd)==dX))&&(this.__Bi[gc])){if(fY.text==dA){fV=true;}
;}
;}
;var gb=fW.execCommand(gc,ga,fX);if(fV&&fY.text!=dA){fY.collapse();}
;if((qx.core.Environment.get(fw))&&qx.core.Environment.get(eU)){this.debug(fk+gc+ea+fX+n);}
;this.__Bn=false;}
catch(gd){if((qx.core.Environment.get(fw))&&qx.core.Environment.get(eU)){this.debug(fk+gc+ea+fX+eg);}
;return false;}
;return gb;}
,__Bs:qx.core.Environment.select(fd,{"mshtml":function(){var gf=this.__Bh;var ge=gf.getSavedRange()!=null?gf.getSavedRange():gf.getRange();return ge;}
,"default":function(){return this.__Bh.getRange();}
}),__Bt:qx.core.Environment.select(fd,{"mshtml":function(gj,gk){var gi;if(gj==qx.bom.htmlarea.HtmlArea.simpleLinebreak){return this.__Bw();}
else {this.__nr.body.focus();var gh=this.__Bh.getSelection();var gg=this.__Bs();if(gg&&gh&&gh.type!=eN){try{gg.pasteHTML(gj);gg.collapse(false);gg.select();gi=true;}
catch(e){}
;}
else {gi=false;}
;this.__Bh.resetSavedRange();return gi;}
;}
,"default":function(gl,gm){this.__nr.body.focus();return this.__nr.execCommand(gm.identifier,false,gl);}
}),insertParagraphOnLinebreak:qx.core.Environment.select(fd,{"gecko":function(){var gu=this.__BG();var gy=this.__Bu(gu);var gq=this.__BE(gu);var gv=B+Date.parse(new Date());var gw=eS+Date.parse(new Date());var go=ff;var gs=fH+gv+ee;go+=gs;go+=eR+gw+ey+gy+eF;go+=gq+fI;this.__Bh.getCommandManager().addUndoStep(R,m,this.getCommandObject(R));this.execute(R,go);this.__Bv();qx.bom.element.Attribute.reset(this.__nr.getElementById(gv),U);var gt=this.__nr.getElementById(gw);if(gt.previousSibling.innerHTML==gs){var gn=this.__BF();var gr=this.__nr.createElement(eY);var gp=this.__nr.createAttribute(fy);gp.nodeValue=dA;gr.setAttributeNode(gp);var gx=this.__nr.createAttribute(dT);gx.nodeValue=o;gr.setAttributeNode(gx);gt.previousSibling.appendChild(gn);gt.previousSibling.appendChild(gr);}
;qx.bom.element.Attribute.reset(gt,U);return true;}
...
;}
;this.__Bh.insertHtml(fu+gA+dH);}
,"default":(function(){}
)}),__Bu:qx.core.Environment.select(fd,{"gecko":function(gG){var gI=fl;var gD=gG.child;if(gD[eC]){gI+=es+gD[eC]+h;delete gG.child[eC];}
;if(gD[fE]){gI+=eL+gD[fE]+h;delete gG.child[fE];}
;var gH={"padding-top":true,"padding-bottom":true,"padding-left":true,"padding-right":true};var gF={"margin-top":true,"margin-bottom":true,"margin-left":true,"margin-right":true};for(var gE in gD){if(gH[gE]||gF[gE]){gI+=gE+eI+gD[gE]+h;delete gG.child[gE];}
;}
;gI+=dK;return gI;}
,"default":function(){return dA;}
}),__Bv:qx.core.Environment.select(fd,{"gecko":function(){var gQ=this.__Bh.getSelection();if(!gQ||!gQ.focusNode){return;}
;var gJ=gQ.focusNode;var gO=gQ.focusNode;while(!qx.dom.Node.isNodeName(gO,M)){gO=gO.parentNode;}
;var gN=gO.previousSibling.id;var gR=gO.nextSibling?gO.nextSibling.id:null;if(qx.lang.String.startsWith(gN,eS)&&gN==gR){var gP=gO.nextSibling;var gL=this.__Bh.getRange();gL.selectNode(gP);gQ.addRange(gL);var gK=qx.bom.htmlarea.HtmlArea.EMPTY_DIV;this.__Bh.getCommandManager().addUndoStep(R,gK,this.getCommandObject(R));this.execute(R,gK);var gM=this.__Bh.getRange();if(gJ){while(gJ&&gJ.firstChild&&qx.dom.Node.isElement(gJ.firstChild)){gJ=gJ.firstChild;}
;}
;gM.selectNode(gJ);gQ.addRange(gM);gM.collapse(true);}
;}
,"default":(function(){}
)}),__Bw:qx.core.Environment.select(fd,{"mshtml":function(){var gS=this.__Bh.getRange();if(gS&&!qx.dom.Node.isNodeName(gS.parentElement(),P)){gS.pasteHTML(qx.bom.htmlarea.HtmlArea.simpleLinebreak);gS.collapse(false);gS.select();return true;}
;return false;}
,"default":function(){return false;}
}),__Bx:function(gU,gV){var gT=(qx.core.Environment.get(fd)==dX)?this.__Bh.getRange():this.__nr;return gT.execCommand(gV.identifier,false,gU);}
,__By:function(gY,ha){if((qx.core.Environment.get(fd)==dX)){var gX=this.__Bh.getFocusNode();this.__Bz(gX);}
;this.__nr.body.focus();var gW=this.__nr.execCommand(ha.identifier,false,gY);if((qx.core.Environment.get(fd)==du)){var gX=this.__Bh.getFocusNode();this.__Bz(gX.parentNode);}
;return gW;}
,__Bz:function(hb){var hc=[];var parent=hb.parentNode;while(qx.dom.Node.isNodeName(parent,O)){hc.push(parent);parent=parent.parentNode;}
;if(hc.length>0){parent.appendChild(hb);parent.removeChild(hc[hc.length-1]);}
;}
,__BA:qx.core.Environment.select(fd,{"gecko":function(hs,hm){if(hs.src){this.__nr.execCommand(hm.identifier,false,hs.src);delete hs.src;var hf=this.__Bh.getSelection();if(hf){var he=hf.anchorNode;var hn=hf.anchorOffset;var hg=he.childNodes[hn-1];var hp;for(var hi in hs){hp=this.__nr.createAttribute(hi);hp.nodeValue=hs[hi];hg.setAttributeNode(hp);}
...
;var hq=this.__nr.createDocumentFragment();var hr;if(hl&&hd!=null){var hh=this.__BB(hd);hq.appendChild(hh.root);hr=hh.inline;}
else {hr=this.__nr.createElement(S);hq.appendChild(hr);}
;var ho=this.__nr.createTextNode(dA);hr.appendChild(ho);var hk=hg.parentNode;if(hg==hk.lastChild){hk.appendChild(hq);}
else {hk.insertBefore(hq,hg.nextSibling);}
;var hj=this.__Bh.getRange();hj.selectNodeContents(hr);}
;return true;}
else {return false;}
;}
,"mshtml":function(hx,hu){var hw=false;var ht=s;for(var hv in hx){ht+=hv+eD+hx[hv]+fg;}
;ht+=f;var hy=this.__Bh.getSelection();var hz=this.__Bs();if(hy&&hy.type!=eN){try{hz.select();hz.pasteHTML(ht);hw=true;}
catch(e){}
;}
;this.__Bh.resetSavedRange();return hw;}
,"default":function(hB,hD){if(hB.src){var hC=s;for(var hA in hB){hC+=hA+eD+hB[hA]+fg;}
;hC+=f;return this.__nr.execCommand(dI,false,hC);}
else {return false;}
;}
}),__BB:function(hF){while(hF.firstChild&&hF.firstChild.nodeType==1){hF=hF.firstChild;}
;var hI=this.__BG(hF);var hM,hG,hK;var hE=dA;var parent=null;var hL=null;var hJ=hI.child;while(hJ){hL=this.__nr.createElement(hJ[ev]?dU:S);hG=this.__nr.createAttribute(V);hL.setAttributeNode(hG);for(var hH in hJ){if(hH!=en&&hH!=ev){hE+=hH+dP+hJ[hH]+ef;}
else if(hH==ev){hK=this.__nr.createAttribute(fr);hK.nodeValue=hJ[hH];hL.setAttributeNode(hK);}
;}
;hG.nodeValue=hE;if(parent!=null){parent.appendChild(hL);}
else {hM=hL;}
;parent=hL;hJ=hJ.child;hE=dA;}
;return {root:hM,inline:hL};}
,__BC:qx.core.Environment.select(fd,{"gecko|opera":function(hN,hR){var hU=this.__Bh.getSelection();var hP=this.__Bh.getRange();if(hU.isCollapsed){var hO=ec+( ++this.__Bk);var hV=this.__nr.createElement(dJ);var hQ=this.__nr.createAttribute(dy);var hS=this.__nr.createAttribute(U);var hT=this.__nr.createTextNode(hN);hS.nodeValue=hO;hV.setAttributeNode(hS);hQ.nodeValue=hN;hV.setAttributeNode(hQ);hV.appendChild(hT);hP.insertNode(hV);hP.selectNode(hV);hU.collapseToEnd();return true;}
else {return this.__nr.execCommand(hR.identifier,false,hN);}
;}
,"mshtml":function(hW,ib){try{var ia;var hX=this.__Bs();var hY=this.__Bh;var hX=hY.getSavedRange()!=null?hY.getSavedRange():hY.getRange();if(hX!=null&&hX.text!=dA){ia=hX.execCommand(ib.identifier,false,hW);}
else {ia=this.__Bt(w+hW+dV+hW+H,ib);}
;this.__Bh.resetSavedRange();return ia;}
catch(e){if(qx.core.Environment.get(fw)){this.error(y);}
;return false;}
;}
,"default":function(ic,id){return this.__nr.execCommand(id.identifier,false,ic);}
}),__BD:function(ig,ih){var ie=eu;if((qx.core.Environment.get(fd)==eA)){ie+=this.__BE();}
;return this.__Bt(ie,ih);}
,__BE:function(ij){var im=dA;var il=p;var ik=[];var iq=typeof ij!==dD?ij:this.__BG();var io=iq.child;var ip=false;if(qx.lang.Object.isEmpty(io)){return dA;}
;while(io){ip=io[ev]!=null;im+=ip?eB:il;for(var ii in io){im+=(ii!=en&&ii!=ev)?ii+eI+io[ii]+h:dA;}
;im+=ip?eK+io[ev]+dV:dV;ik.unshift(ip?v:fe);io=io.child;}
;if(ik.length==1&&ik[0]==v){im+=dG;}
;for(var i=0,j=ik.length;i<j;i++ ){im+=ik[i];}
;return im;}
,__BF:function(){var is=this.__nr.createDocumentFragment();var ix=this.__BG();var parent=is;var iv=ix.child;var it;var iw=false;while(iv){iw=iv[ev]!=null;it=this.__nr.createElement(iw?dU:S);parent.appendChild(it);for(var ir in iv){if(ir!=en&&ir!=ev){qx.bom.element.Style.set(it,ir,iv[ir]);}
;}
;if(iw){var iu=this.__nr.createAttribute(fr);iu.nodeValue=iv[ev];it.setAttributeNode(iu);}
;parent=it;iv=iv.child;}
;return is;}
,__BG:function(iz){var iA={};var iB=null;var iC=this.getCurrentStyles(iz);iB=iA.child={};for(var iy in iC){if(iy!=fn){iB[iy]=iC[iy];}
;}
;if(iC[fn]){var iD=iC[fn];for(var i=0,j=iD.length;i<j;i++ ){if(iB==null){iB=iA.child={};}
else {iB=iB.child={};}
;iB[W]=iD[i][W];iB[eP]=iD[i][eP];}
;}
;if(iC[W]&&iC[eP]){iB=iB.child={};iB[W]=iC[W];}
;return iA;}
,getCurrentStyles:function(iG){if(iG==null){var iJ=this.__Bh.getSelection();if(!iJ||iJ.focusNode==null){return {};}
;iG=(iJ.focusNode.nodeType==3)?iJ.focusNode.parentNode:iJ.focusNode;}
;var iH=qx.dom.Hierarchy.getAncestors(iG);var iE=qx.lang.Array.insertBefore(iH,iG,iH[0]);var iI=this.__BH(iE);var iF=this.__BI(iI,iE);return iF;}
,__BH:function(iN){var iM={};var iK,iL;for(var i=0,j=iN.length;i<j;i++ ){iL=iN[i];for(var k=0,l=iL.style.length;k<l;k++ ){iK=iL.style[k];if(iK.length>0&&typeof iM[iK]===dD){iM[iK]=iL.style.getPropertyValue(iK);}
;}
;if(iL.tagName.toUpperCase()==x&&iL.size&&iM[ev]===undefined){iM[ev]=iL.size;}
;}
;if(iM[ev]&&iM[J]){delete iM[J];}
;return iM;}
,__BI:function(iT,iP){var iR=iP[0];var iS=this.__Bh.getContentWindow().getComputedStyle(iR,null);var iU;var iQ={};for(var iO in iT){if(iO!=ev){iU=iS.getPropertyValue(iO);}
else {iU=iT[iO];}
;if(iO==dR&&iU==D){iQ[iO]=this.__BK(iP);}
else if(iO==fn){iQ[iO]=this.__BJ(iP);}
else {iQ[iO]=iU;}
;}
;return iQ;}
,__BJ:function(iX){var iY,jb,iV;var ja=[];var iW=this.__Bh.getContentWindow();for(var i=0,j=iX.length;i<j;i++ ){iV=iW.getComputedStyle(iX[i],null);iY=iV.getPropertyValue(fn);jb=iV.getPropertyValue(fx);if(iY!=ed){ja.push({'text-decoration':iY,'color':jb});}
;}
;return ja;}
,__BK:function(jd){var je,jc,jf;for(var i=0;i<jd.length;i++ ){je=jd[i];jc=this.__Bh.getContentWindow().getComputedStyle(je,null);jf=jc.getPropertyValue(dR);if(jf!=D){return jf;}
;}
;}
,__BL:function(jm,jn){var jh=this.__Bh.getSelection();var jl=((qx.core.Environment.get(fd)==dX))?this.__Bh.getRange():jl=jh.getRangeAt(0);var jp=[];var jt;var jk=[er,dv];var jg,i,j,jr;jr=((qx.core.Environment.get(fd)==dX))?jl.parentElement():jl.commonAncestorContainer;if(jr.tagName==dB){for(var i=0;i<jk.length;i++ ){jg=jr.getElementsByTagName(jk[i]);for(var j=0;j<jg.length;j++ ){if(jg[j]){jp.push(jg[j]);}
;}
;}
;}
else if(qx.lang.Array.contains(jk,jr.tagName)){jp.push(jr);}
;if(jp.length>0){for(var i=0;i<jp.length;i++ ){var js=jp[i];jt=((qx.core.Environment.get(fd)==dX))?((js==jr)||(jr.tagName==dB)):jh.containsNode(js,false);for(j=0;j<js.childNodes.length;j++ ){var jj=js.childNodes[j];if(jt||(jh.focusNode.nodeValue&&qx.dom.Hierarchy.contains(jj,jh.focusNode)&&(jh.focusOffset==jh.focusNode.nodeValue.length))||(qx.dom.Hierarchy.contains(jj,jh.anchorNode)&&(jh.anchorOffset==0))||(jh.containsNode(jj,false))){jj.style.fontSize=(this.__Bj[jm]||jm)+eo;}
;}
;}
;}
else {var ji=((qx.core.Environment.get(fd)==dX))?jr:jh.focusNode;var jo=qx.dom.Hierarchy.getAncestors(ji);for(i=0;i<jo.length;i++ ){if(jo[i].tagName==eH){if((((qx.core.Environment.get(fd)==eA))&&((jh.anchorOffset==0)&&(jh.focusNode.nodeValue&&(jh.focusOffset==jh.focusNode.nodeValue.length))&&qx.dom.Hierarchy.contains(jo[i].firstChild,jh.anchorNode)&&qx.dom.Hierarchy.contains(jo[i].lastChild,jh.focusNode)))||(((qx.core.Environment.get(fd)==dX))&&(jl.htmlText==jo[i].innerHTML))){jo[i].style.fontSize=(this.__Bj[jm]||jm)+eo;}
;break;}
;}
;}
;if((qx.core.Environment.get(fd)==dX)){this.__nr.body.focus();this.__Bh.getRange().select();return this.__nr.execCommand(fi,false,jm);}
else if((qx.core.Environment.get(fd)==eA)){var parent=jl.commonAncestorContainer;if(parent.nodeType===1){var jq=parent.getElementsByTagName(S);for(i=0;i<jq.length;i++ ){if(jq[i].style.fontSize){jq[i].style.fontSize=null;}
;}
;}
;}
;return this.__nr.execCommand(fi,false,jm);}
,__BM:qx.core.Environment.select(fd,{"mshtml":function(ju,jv){this.__nr.body.focus();return this.__nr.execCommand(eV,false,ju);}
,"gecko|opera":function(jw,jx){this.__nr.body.focus();return this.__nr.execCommand(fa,false,jw);}
,"webkit":function(jA,jB){var jC=this.__Bh.getSelection();var jz=this.__Bh.getRange();if(!jC||!jC.isCollapsed){this.__nr.body.focus();this.__nr.execCommand(eV,false,jA);if(jC){jC.collapseToEnd();}
;return true;}
...
;}
;jz.setStart(jC.anchorNode,jC.anchorNode.nodeValue.charAt(jE)==el?jE+1:jE);jz.setEnd(jC.anchorNode,jy);jC.addRange(jz);this.__nr.body.focus();this.__nr.execCommand(eV,false,jA);jC.collapseToEnd();return true;}
;}
}),__BN:function(jF,jG){jF=jF!=null&&typeof jF==dN?jF:D;qx.bom.element.Style.set(this.__nr.body,b,jF);return true;}
,__BO:function(jJ,jK){var jI,jH,jM;var jL=qx.bom.htmlarea.manager.Command;if(jJ==null){jI=null;}
else {jI=jJ[0];jH=jJ[1];jM=jJ[2];}
;if(jI==null||typeof jI!=dN){qx.bom.element.Style.set(this.__nr.body,fG,dA);qx.bom.element.Style.set(this.__nr.body,dS,dA);qx.bom.element.Style.set(this.__nr.body,z,dA);return true;}
else {if(jI.search(/^url.*\(/)==-1){jI=ep+jI+fJ;}
;}
;if(jH!=null&&!qx.lang.String.contains(jL.__Bm,jH)){if(qx.core.Environment.get(fw)){this.error(fb+jH+fh+jL.__Bm+fj);}
;return false;}
else {jH=u;}
;if(jM!=null){if(qx.lang.Type.isString(jM)&&!qx.lang.String.contains(jL.__Bl,T+jM+T)){if(qx.core.Environment.get(fw)){this.error(fb+jM+t+jL.__Bl+fj);}
;return false;}
else {if(qx.lang.Type.isArray(jM)&&jM.length==2){jM=jM[0]+el+jM[1];}
else {if(qx.core.Environment.get(fw)){this.error(Q);}
;return false;}
;}
;}
else {jM=d;}
;qx.bom.element.Style.set(this.__nr.body,fG,jI);qx.bom.element.Style.set(this.__nr.body,dS,jH);qx.bom.element.Style.set(this.__nr.body,z,jM);return true;}
,__BP:qx.core.Environment.select(fd,{"mshtml":function(jO,jP){var jN=this.__nr.body.createTextRange();jN.select();return true;}
,"default":function(jQ,jR){return this.__Br(jR.identifier,false,jQ);}
}),__BQ:function(){return this.__Bh.getSelectedText();}
,__BR:function(){return this.__Bh.getSelectedHtml();}
,__BS:function(jV,jW){var jU=this.__Bh.getFocusNode();if(jU.textContent==dA){var jT=qx.dom.Hierarchy.getAncestors(jU);jT.unshift(jU);var Node=qx.dom.Node;var jS=qx.bom.element.Style;var jX=jT.shift();while(jT.length>0){if(Node.getName(jX)==M||Node.getName(jX)==eq){break;}
;if(jS.get(jX,jV,jS.LOCAL_MODE)==jW){jS.reset(jX,jV);return true;}
;jX=jT.shift();}
;}
;return false;}
,__BT:qx.core.Environment.select(fd,{"gecko":function(jY,ka){if(this.__BS(eX,N)){return true;}
else {return this.__Br(ka.identifier,false,jY);}
;}
,"default":function(kb,kc){return this.__Br(kc.identifier,false,kb);}
}),__BU:qx.core.Environment.select(fd,{"gecko":function(kd,ke){if(this.__BS(ek,eb)){return true;}
else {return this.__Br(ke.identifier,false,kd);}
;}
,"default":function(kf,kg){return this.__Br(kg.identifier,false,kf);}
}),__BV:qx.core.Environment.select(fd,{"gecko":function(kh,ki){if(this.__BS(fC,r)){return true;}
else {return this.__Br(ki.identifier,false,kh);}
;}
,"default":function(kj,kk){return this.__Br(kk.identifier,false,kj);}
}),__BW:qx.core.Environment.select(fd,{"gecko":function(kl,km){if(this.__BS(fC,dC)){return true;}
else {return this.__Br(km.identifier,false,kl);}
;}
,"default":function(kn,ko){return this.__Br(ko.identifier,false,kn);}
})},destruct:function(){this.__nr=this.__Bh=this._commands=null;this.__Bi=this.__Bj=null;}
});}
)();
(function(){var a="undo",b="backgroundColor",c="actionType ",d="mousedown",f="pageup",g="ADD TO UNDO STACK",h="backgroundimage",k="control",l="shift",m="end",n="right",o="qx_link",p="b",q="insertunorderedlist",r="Command",s=" ",t="Undo",u="pagedown",v="k",w="home",x="qx.debug",y="Content",z="Redo",A=" is not managed! Please provide a handler method!",B="table",C="backgroundImage",D="backgroundRepeat",E="p",F="a",G="insertorderedlist",H="y",I="backgroundPosition",J="justifyright",K="redo",L="insertParagraph",M="qx.bom.htmlarea.manager.UndoRedo",N="left",O="i",P="backgroundcolor",Q="__An",R="td",S="img",T="Internal",U="u",V="inserthyperlink",W="gecko",X="enter",Y="mshtml",bu="engine.name",bv="mouseup",bw="keypress",bq="execCommand failed! Details: ",br="ADD TO REDO STACK",bs="qx.bom.htmlarea.HtmlArea.debug",bt="webkit",bA="insertimage",bB="inserthtml",bC="z",bD="Custom",bx="undoRedoState",by="down",bz="up",bp="body";qx.Class.define(M,{extend:qx.core.Object,construct:function(bF,bE){qx.core.Object.call(this);this.__An=bF;this.__Bh=bE;this.__BX=[];this.__BY=[];this._commands=null;this.__nr=null;this.__Ca={};this.__Cb={command:true,content:true,custom:true};this.__Bo();this.__Cc=qx.lang.Function.bind(this._handleKeyPress,this);this.__Cd=qx.lang.Function.bind(this._handleMouseUp,this);if((qx.core.Environment.get(bu)==Y)){this.__Ce=qx.lang.Function.bind(this._handleMouseDown,this);}
;}
,members:{__Cf:false,__Cg:false,__Ch:false,__Cb:null,__Ca:null,__An:null,__nr:null,__BX:null,__BY:null,__Bh:null,__Cc:null,__Cd:null,__Ce:null,__Ci:null,setContentDocument:function(bG){this.__nr=bG;this.__An.setContentDocument(bG);qx.event.Registration.addListener(bG.body,bw,this.__Cc,this);qx.event.Registration.addListener(bG.body,bv,this.__Cd,this);if((qx.core.Environment.get(bu)==Y)){qx.event.Registration.addListener(bG.body,d,this.__Ce,this,true);}
;}
,insertParagraphOnLinebreak:function(){return this.__An.insertParagraphOnLinebreak();}
,execute:function(bH,bJ){var bI;bH=bH.toLowerCase();if(this._commands[bH]){if(this._commands[bH].passthrough){bI=this.__An.execute(bH,bJ);}
else {bI=this[bH].call(this);}
;}
else {if((qx.core.Environment.get(bu)==Y)||(qx.core.Environment.get(bu)==bt)){this.__Cz(bH,bJ,this.__An.getCommandObject(bH));bI=this.__An.execute(bH,bJ);if(!bI){this.__BX.pop();}
;}
else {bI=this.__An.execute(bH,bJ);if(bI){this.__Cz(bH,bJ,this.__An.getCommandObject(bH));}
;}
;if(bH==a&&this.__BX.length==0){this.__Cg=false;this.__CH();}
else if(bH==K&&this.__BY.length==0){this.__Cf=false;this.__CH();}
;}
;this.__Ch=false;return bI;}
,addUndoStep:function(bK,bL,bM){this.__Cz(bK,bL,bM);}
,registerHandler:function(bO,bN,bQ,bP){this.__Ca[bO]={undo:bN,redo:bQ,context:bP};}
,isUndoPossible:function(){return this.__Cg;}
,undo:function(){var bT;if(this.__Ch){this.__CB();}
;if(this.__BX.length>0){var bS=this.__BX.pop();if(this.__Cb[bS.actionType.toLowerCase()]){switch(bS.actionType){case r:bT=this.__Ck(bS);break;case y:bT=this.__Co(bS);break;case T:bT=this.__Cn(bS);break;case bD:bT=this.__Cj(bS);break;};}
else if(this.__Ca[bS.actionType]){var bR=this.__Ca[bS.actionType];bT=bR.undo.call(bR.context?bR.context:this,bS);this.__CD(bS);}
else {this.error(c+bS.actionType+A);}
;this.__Cf=true;this.__CH();return bT;}
;}
,__Cj:qx.core.Environment.select(bu,{"mshtml|webkit":function(bW){var bU=this.__nr.body.innerHTML;var bX=bW.content;this.__nr.body.innerHTML=bX;var bV=bW;bV.content=bU;this.__CD(bV);return true;}
,"default":function(cc){var cb=cc;var ca=qx.bom.element.Style;switch(cc.command){case P:cb.parameter=[ca.get(this.__nr.body,b)];break;case h:cb.parameter=[ca.get(this.__nr.body,C),ca.get(this.__nr.body,D),ca.get(this.__nr.body,I)];break;};this.__CD(cb);if(cc.command==V){if((qx.core.Environment.get(bu)==W)){var cd=o+this.__An.__hyperLinkId;var bY=this.__nr.getElementById(cd);if(bY){bY.parentNode.removeChild(bY);return true;}
else {return false;}
;}
;}
else {return this.__An.execute(cc.command,cc.value);}
;}
}),__Ck:qx.core.Environment.select(bu,{"mshtml|webkit":function(ce){}
,"default":function(cf){this.__CD(cf);if((qx.core.Environment.get(bu)==W)){if(cf.command==bB&&cf.value==qx.bom.htmlarea.HtmlArea.EMPTY_DIV&&this.__Cl(bB,L)){this.__Cm();}
;}
;return this.__Cp();}
}),__Cl:function(cg,ch){if(this.__BX.length>0){var ci=this.__BX[this.__BX.length-1];return (ci.command==cg&&ci.value==ch);}
;return false;}
,__Cm:function(){this.__Cp();if(this.__BX.length>0){var cj=this.__BX.pop();this.__CD(cj);}
;}
,__Cn:function(ck){this.__CD(ck);return this.__Cp();}
,__Co:qx.core.Environment.select(bu,{"gecko":function(cl){this.__CD(cl);try{return this.__Cp();}
catch(cm){if(qx.core.Environment.get(x)){this.error(bq+cm);}
;}
;}
,"mshtml|webkit":function(cn){}
,"default":function(co){this.__CD(co);return this.__Cp();}
}),__Cp:function(){try{return this.__nr.execCommand(t,false,null);}
catch(e){return false;}
;}
,isRedoPossible:function(){return this.__Cf;}
,redo:function(){if(this.__Cf){var cq;if(this.__BY.length>0){var cr=this.__BY.pop();if(this.__Cb[cr.actionType.toLowerCase()]){switch(cr.actionType){case r:cq=this.__Cr(cr);break;case y:cq=this.__Cx(cr);break;case T:cq=this.__Cw(cr);break;case bD:cq=this.__Cq(cr);break;};}
else if(this.__Ca[cr.actionType]){var cp=this.__Ca[cr.actionType];cq=cp.redo.call(cp.context?cp.context:this,cr);this.__CC(cr);}
else {this.error(c+cr.actionType+A);}
;this.__Cg=true;this.__CH();}
;return cq;}
;}
,__Cq:qx.core.Environment.select(bu,{"mshtml|webkit":function(cu){var cs=this.__nr.body.innerHTML;var cv=cu.content;this.__nr.body.innerHTML=cv;var ct=cu;ct.content=cs;this.__CC(ct);return true;}
,"default":function(cw){this.__CC(cw);return this.__Cy();}
}),__Cr:qx.core.Environment.select(bu,{"mshtml|webkit":function(cx){}
,"default":function(cy){this.__CC(cy);var cz=this.__Cy();if((qx.core.Environment.get(bu)==W)){if(this.__Cs(bB,qx.bom.htmlarea.HtmlArea.EMPTY_DIV)){var cA=this.__Ct();this.__Cu();if(cA!=null){this.__Cv(cA);}
;}
;}
;return cz;}
}),__Cs:function(cC,cD){if(this.__BY.length>0){var cB=this.__BY[this.__BY.length-1];return (cB.command==cC&&cB.value==cD);}
;return false;}
,__Ct:function(){if(this.__Bh==null){return null;}
;var cE=this.__Bh.getSelection();var cF=cE?cE.focusNode:null;if(cF==null){return null;}
...
;}
,__Cu:function(){var cH=this.__BY.pop();this.__CC(cH);this.__Cy();}
,__Cv:qx.core.Environment.select(bu,{"gecko":function(cK){if(cK==this.__Bh.getContentBody().lastChild){return;}
;var cL=cK.firstChild;while(cL.firstChild){cL=cL.firstChild;}
;var cI=this.__Bh.getSelection();var cJ=this.__Bh.getRange();if(cI&&cJ){cJ.selectNode(cL);cI.addRange(cJ);cJ.collapse(true);}
;}
,"default":(function(){}
)}),__Cw:function(cM){this.__CC(cM);return this.__Cy();}
,__Cx:qx.core.Environment.select(bu,{"mshtml|webkit":function(cN){}
,"default":function(cO){this.__CC(cO);return this.__Cy();}
}),__Cy:function(){try{return this.__nr.execCommand(z,false,null);}
catch(e){return false;}
;}
,__Bo:function(){this._commands={undo:{passthrough:false},redo:{passthrough:false}};this.__An.getCommandObject(P).customUndo=true;this.__An.getCommandObject(h).customUndo=true;if((qx.core.Environment.get(bu)==W)){}
;}
,__Cz:qx.core.Environment.select(bu,{"mshtml|webkit":function(cP,cR,cS){var cQ=this.getUndoRedoObject();cQ.commandObject=cS;cQ.command=cP;cQ.value=cR;cQ.actionType=bD;cQ.content=this.__nr.body.innerHTML;this.__CA(cQ);}
,"default":function(cX,cV,cW){if(this.__Bh==null){return;}
;var cT=this.getUndoRedoObject();cT.commandObject=cW;cT.command=cX;cT.value=cV;cT.actionType=bD;var cY=this.__Bh.getSelection();if(cW.customUndo){var cU=[];switch(cX){case P:cU.push(qx.bom.element.Style.get(this.__nr.body,b));break;case h:cU.push(qx.bom.element.Style.get(this.__nr.body,C),qx.bom.element.Style.get(this.__nr.body,D),qx.bom.element.Style.get(this.__nr.body,I));break;case V:if(cY&&!cY.isCollapsed){cT.actionType=r;}
...
;}
;this.__CA(cT);}
}),__CA:function(da){if(this.__Ch){this.__CB();}
;this.__CC(da);this.__Cf=false;this.__BY=[];this.__CH();}
,__CB:function(){var db=this.__BX[this.__BX.length-1];if(db==null||db.actionType!=y){var dc=this.getUndoRedoObject();dc.actionType=y;if((qx.core.Environment.get(bu)==Y)||(qx.core.Environment.get(bu)==bt)){dc.content=this.__Ci;dc.actionType=bD;this.__Ci=null;}
;this.__CC(dc);this.__Ch=false;}
;}
...
,__CC:function(dd){if((qx.core.Environment.get(x))&&qx.core.Environment.get(bs)){this.debug(g);this.debug(dd.actionType+s+dd.command+s+dd.value);}
;this.__BX.push(dd);}
,__CD:function(de){if((qx.core.Environment.get(x))&&qx.core.Environment.get(bs)){this.debug(br);this.debug(de.actionType+s+de.command+s+de.value);}
;this.__BY.push(de);}
,_handleKeyPress:function(e){var df=e.getKeyIdentifier().toLowerCase();var dg=e.isCtrlPressed();switch(df){case k:case l:case N:case n:case bz:case by:case f:case u:case w:case m:case X:break;case F:case p:case O:case U:case v:case H:case bC:if(!dg){this.__CE();}
;break;default:this.__Cf=false;this.__BY=[];this.__CE();};}
,__CE:function(){if(!this.__Ch){this.__Ch=true;this.__Cg=true;if((qx.core.Environment.get(bu)==Y)||(qx.core.Environment.get(bu)==bt)){this.__Ci=this.__nr.body.innerHTML;}
;this.__CH();}
;}
,__CF:null,_handleMouseDown:qx.core.Environment.select(bu,{"mshtml":function(e){var dh=e.getOriginalTarget();if(qx.dom.Node.isElement(dh)&&(qx.dom.Node.isNodeName(dh,S)||qx.dom.Node.isNodeName(dh,B))){this.__CF={node:dh,content:dh.outerHTML};}
else {this.__CF=null;}
;}
,"default":function(e){return true;}
}),_handleMouseUp:qx.core.Environment.select(bu,{"gecko":function(e){if(this.__Bh==null){return;}
;var dj=this.__Bh.getSelection();if(!dj){this.__CF=null;return;}
;var dl=dj.anchorNode;var dk=dl.childNodes[dj.anchorOffset];if(qx.dom.Node.isNodeName(dk,S)){if(this.__CF==null){this.__CF=dk.cloneNode(true);}
else {if(this.__CF.style.width!=dk.style.width||this.__CF.style.height!=dk.style.height){this.__CG();this.__CF=dk.cloneNode(true);}
;}
;}
...
;if(this.__CF==null){this.__CF=di.cloneNode(true);}
else {qx.event.Timer.once(function(){if(di.style.width!=this.__CF.style.width||di.style.height!=this.__CF.style.height||di.innerHTML!=this.__CF.innerHTML){this.__CG();this.__CF=di.cloneNode(true);}
;}
,this,0);}
;}
else {this.__CF=null;}
;}
,"default":function(e){var dm=qx.bom.Event.getTarget(e);if(this.__CF!=null){if(dm.nodeType==1){if(dm==this.__CF.node){if(dm.outerHTML!=this.__CF.content){this.__CF.content=dm.outerHTML;this.__CG();}
;}
else {for(var i=0,j=dm.childNodes.length;i<j;i++ ){if(dm.childNodes[i]==this.__CF.node){if(dm.childNodes[i].outerHTML!=this.__CF.content){this.__CF.content=dm.childNodes[i].outerHTML;this.__CG();}
;}
;}
;}
;}
...
;}
;}
}),__CG:qx.core.Environment.select(bu,{"mshtml|webkit":function(){this.__Cz(T,null,null);}
,"default":function(){var dn=this.getUndoRedoObject();dn.actionType=T;this.__CC(dn);}
}),__CH:function(){qx.event.Timer.once(function(e){if(this.__Bh!=null){var dp={undo:this.isUndoPossible()?0:-1,redo:this.isRedoPossible()?0:-1};this.__Bh.fireDataEvent(bx,dp);}
;}
,this,200);}
},destruct:function(){try{qx.event.Registration.removeListener(this.__nr.body,bw,this.__Cc);qx.event.Registration.removeListener(this.__nr,bv,this.__Cd);if((qx.core.Environment.get(bu)==Y)){qx.event.Registration.removeListener(this.__nr,d,this.__Ce);}
;}
catch(e){}
;this._disposeObjects(Q);this.__Bh=this.__BX=this.__BY=this._commands=this.__nr=null;this.__Cb=this.__Ca=null;}
});}
)();
