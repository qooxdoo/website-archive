...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.DragDrop.94b5e23b0dc9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;e.addType(j);e.addType(C);e.addAction(q);e.addAction(o);}
...
;V=X;}
;break;case j:V=this.getSelection()[0].getLabel();break;};if(W==o){var U=this.getSelection();for(var i=0,l=U.length;i<l;i++ ){this.remove(U[i]);}
;}
;e.addData(Y,V);}
);var L=new qx.ui.basic.Label(w);I.add(L,{left:140,top:20});var J=new qx.ui.form.List;J.setDroppable(true);J.setSelectionMode(u);I.add(J,{left:140,top:40});J.addListener(f,function(e){this.debug(E+e.getRelatedTarget());var ba=e.getData(C);for(var i=0,l=ba.length;i<l;i++ ){this.add(ba[i]);}
;}
...
;}
);var N=new qx.ui.basic.Label(m);I.add(N,{left:600,top:20});var G=this.__JE=new qx.ui.form.List();G.setDraggable(true);G.setDroppable(true);G.setSelectionMode(u);I.add(G,{left:600,top:40});for(var i=0;i<20;i++ ){G.add(new qx.ui.form.ListItem(D+i,g));}
...
);G.addListener(b,function(e){var bd=e.getOriginalTarget();if(bd instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__RA=bd;}
...
;}
);G.addListener(f,function(e){this.__RB(e.getOriginalTarget());}
,this);H.addListener(f,function(e){this.__RB(this.__RA);}
,this);}
,__RB:function(bf){if(bf.classname!=n){return;}
;var be=this.__JE.getSortedSelection();for(var i=0,l=be.length;i<l;i++ ){this.__JE.addBefore(be[i],bf);this.__JE.addToSelection(be[i]);}
;}
