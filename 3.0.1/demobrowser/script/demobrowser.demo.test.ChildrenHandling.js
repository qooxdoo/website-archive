...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.ChildrenHandling.81be9914df0b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="_gridWidgets",b="yellow",c="demobrowser.demo.test.ChildrenHandling",d="Horizontal box layout:",e="blue",f="Add ",g="Remove ",h="_widgets",j="_hbox",k="green",l="Grid layout:",m="black",n="orange",o="_grid",p="red",q="execute";qx.Class.define(c,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var v=new qx.ui.layout.VBox();v.setSpacing(10);var s=new qx.ui.container.Composite(v).set({padding:10});s.add(new qx.ui.basic.Label(d));this._hbox=new qx.ui.container.Composite(new qx.ui.layout.HBox()).set({height:100});this._widgets=[];var w=[k,e,b,m,n,p];for(var i=0;i<w.length;i++ ){var r=new qx.ui.core.Widget();r.setBackgroundColor(w[i]);this._hbox.add(r);this._widgets.push(r);}
;s.add(this._hbox);s.add(new qx.ui.basic.Label(l));var u=new qx.ui.layout.Grid();u.setRowFlex(0,1);this._grid=new qx.ui.container.Composite(u).set({height:100});this._gridWidgets=[];for(var i=0;i<w.length;i++ ){var r=new qx.ui.core.Widget();r.setBackgroundColor(w[i]);this._grid.add(r,{row:0,column:i});this._gridWidgets.push(r);}
...
,createRemoveButton:function(x,y){var A=true;var z=new qx.ui.form.Button(g+x.getBackgroundColor()).set({width:100});z.addListener(q,function(){if(A){this._hbox.remove(x);this._grid.remove(this._gridWidgets[y]);z.setLabel(f+x.getBackgroundColor());}
else {this._hbox.add(x);this._grid.add(this._gridWidgets[y],{row:0,column:y});z.setLabel(g+x.getBackgroundColor());}
;A=!A;}
,this);return z;}
},destruct:function(){this._disposeObjects(j,o);this._disposeArray(h);this._disposeArray(a);}
});}
)();
