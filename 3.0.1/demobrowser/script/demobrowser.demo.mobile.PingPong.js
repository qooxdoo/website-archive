...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.mobile.PingPong.8ae384af84b7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="touchmove",b="black",c="#333",d="px",f="border-left",g="green",h="div",i="100%",j="demobrowser.demo.mobile.PingPong",k="0",l="200px",m="30px",n="qx.debug",o="interval",p="0px",q="12px",r="15em",s="center",t="100px",u="1px solid white",v='Lucida Grande',w="innerHTML",z="top",A="right",B="Arial",C="50%",D="<b>This demo is supposed to be run in a WebKit-based browser.</b>",E="engine.name",F="20px",G="webkit",H="none",I="border-right",J="white",K="left",L="absolute";qx.Class.define(j,{extend:qx.application.Native,members:{__QG:null,__QH:null,__QI:null,__QJ:null,__QK:5,__QL:1,__QM:1,__QN:null,__QO:null,__QP:null,main:function(){qx.application.Native.prototype.main.call(this);if(qx.core.Environment.get(n)){qx.log.appender.Native;qx.log.appender.Console;}
...
;var M=this.__QT(K);R.add(M);var N=this.__QT(A);R.add(N);this.__QH=this.__QU(K);M.add(this.__QH);M.addListener(a,qx.lang.Function.bind(this.__hR,this,this.__QH),this);this.__QG=this.__QU(A);N.add(this.__QG);N.addListener(a,qx.lang.Function.bind(this.__hR,this,this.__QG),this);this.__QI=this.__QV();R.add(this.__QI);this.__QO=this.__QW();M.add(this.__QO);this.__QP=this.__QW();N.add(this.__QP);this.__QN=[0,0];this.__QQ();}
,__QQ:function(){this.__QI.setStyles({"left":l,"top":l});this.__QJ=new qx.event.Timer(40);this.__QJ.addListener(o,function(){var x=(parseInt(this.__QI.getStyle(K))+this.__QK*this.__QL);var y=(parseInt(this.__QI.getStyle(z))+this.__QK*this.__QM);this.__QI.setStyle(K,x+d);this.__QI.setStyle(z,y+d);this.__QS();}
,this);this.__QJ.start();}
,__QR:function(S){this.__QJ.stop();this.__QJ.dispose();this.__QK=5;this.__QN[S]=this.__QN[S]+1;this.__QO.setAttribute(w,this.__QN[0]);this.__QP.setAttribute(w,this.__QN[1]);this.__QQ();}
,__QS:function(){var U=this.getBoundsFor(this.__QI);if(U.top<=0){this.__QM=1;}
;if(U.bottom>=window.innerHeight-5){this.__QM=-1;}
;if(U.left<=0||U.right>=window.innerWidth){this.__QR(U.left<=0?1:0);}
;var V=this.getBoundsFor(this.__QH);if(U.left<=V.right&&U.bottom>=V.top&&U.top<=V.bottom){this.__QL=1;this.__QK=Math.min(this.__QK+1,20);}
;var T=this.getBoundsFor(this.__QG);if(U.right>=T.left&&U.bottom>=T.top&&U.top<=T.bottom){this.__QL=-1;this.__QK=Math.min(this.__QK+1,20);}
;}
,__hR:function(W,e){W.setStyles({"top":(e.getDocumentTop()-50)+d});}
,getBoundsFor:function(X){return qx.bom.element.Location.get(X.getDomElement());}
,__QT:function(ba){var Y={"width":C,"height":i,"position":L,"backgroundColor":b};if(ba==K){Y[I]=u;}
else {Y[K]=C;Y[f]=u;}
;return new qx.html.Element(h,Y);}
,__QU:function(bc){var bb={"width":m,"height":t,"top":t,"position":L,"backgroundColor":J};bb[bc]=F;return new qx.html.Element(h,bb);}
,__QV:function(){var bd={"width":F,"height":F,"position":L,"backgroundColor":J,"userSelect":H};return new qx.html.Element(h,bd);}
,__QW:function(){var be={"width":i,"height":i,"textAlign":s,"fontSize":r,"color":c,"fontFamily":B};var bf=new qx.html.Element(h,be);bf.setAttribute(w,k);return bf;}
}});}
)();
