...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Github.9ff0bb88a84a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
},members:{_resource:null,__GJ:null,__GK:null,__GL:null,_getResource:function(v){return new qx.bom.rest.Resource(v);}
...
,map:function(G,I,H,J){this.__GM(G,I,H,J);this._resource.map(G,I,H,J);}
,__GM:function(L,N,K,M){this.__GL[L]=[N,K,M];if(L==m){this[L]=undefined;}
;if(typeof this[L]!==q&&this[L]!==null&&this[L].action!==true){throw new Error(o+L+c);}
;this.__GO(L+p);this.__GO(L+f);this[L]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,L);return this.invoke.apply(this,arguments);}
,this);this[L].action=true;}
,invoke:function(O,P,Q){var P=(P==null)?{}:P;this.__GL[O].params=P;return this._resource.invoke(O,P,Q);}
...
,poll:function(Y,U,X,ba){if(this.__GK[Y]){this.__GK[Y].dispose();}
;if(typeof X==q){X=this.__GL[Y].params;}
...
;}
;var V=this.__GK[Y]=new qx.event.Timer(U);V.addListener(i,W,this._resource);V.start();return V;}
...
;if(!bj){bi=0;}
;return false;}
;var bf=this.__GJ[bh]=this.addListener(bh+p,function bk(){if(be.isDisposed()){return;}
...
,__GN:function(bn){Object.keys(bn).forEach(function(bp){var bq=bn[bp],bs=bq.method,bo=bq.url,br=bq.check;if(qx.core.Environment.get(s)){qx.core.Assert.assertString(bs,a+bp+b);qx.core.Assert.assertString(bo,n+bp+b);}
;this.map(bp,bs,bo,br);}
,this);}
,__GO:function(bt){if(!this.constructor.$$events){this.constructor.$$events={};}
;if(!this.constructor.$$events[bt]){this.constructor.$$events[bt]=j;}
;}
},destruct:function(){var bu;if(this.__GK){for(bu in this.__GK){var bw=this.__GK[bu];bw.stop();bw.dispose();}
;}
;if(this.__GJ){for(bu in this.__GJ){var bv=this.__GJ[bu];this.removeListenerById(bv);}
;}
;this._resource.destruct();this._resource=this.__GL=this.__GK=this.__GJ=null;}
});}
)();
...
;return S;}
},members:{__GP:null,__GL:null,__GQ:null,__GK:null,__GJ:null,__GR:null,__GS:null,__GT:null,setRequestFactory:function(V){this.__GT=V;}
,setRequestHandler:function(W){this.__GS=W;}
,_getRequestHandler:function(){return (this.__GS===null)?{onsuccess:{callback:function(Y,X){return function(){var ba={"id":parseInt(Y.toHashCode(),10),"response":Y.getResponse(),"request":Y,"action":X};this.emit(X+G,ba);this.emit(w,ba);}
;}
...
;}
,context:this}}:this.__GS;}
,getRequestsByAction:function(bg){var bh=(this.__GP!==null&&bg in this.__GP);return bh?this.__GP[bg]:null;}
,configureRequest:function(bi){this.__GR=bi;}
,_getRequest:function(){return (this.__GT===null)?new qx.bom.request.SimpleXhr():this.__GT();}
,__GU:function(bj){var bk=this._getRequest();if(!qx.lang.Type.isArray(this.__GP[bj])){this.__GP[bj]=[];}
;this.__GP[bj].push(bk);return bk;}
,map:function(bm,bo,bl,bn){this.__GL[bm]=[bo,bl,bn];this.__GP[bm]=[];if(bm==u){this[bm]=undefined;}
;if(typeof this[bm]!==N&&this[bm]!==null&&this[bm].action!==true){throw new Error(m+bm+y);}
;this.__GO(bm+G);this.__GO(bm+z);this[bm]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,bm);return this.invoke.apply(this,arguments);}
,this);this[bm].action=true;}
,invoke:function(bt,br,bp){var bq=this.__GU(bt),br=br==null?{}:br,bu=this._getRequestConfig(bt,br);this.__GL[bt].params=br;this.__GV(br,bu.check);this.__zp(bq,bu,bp);if(this.__GR){this.__GR.call(this,bq,bt,br,bp);}
;this.__GW(bq,bu,bp);var bs=this._getRequestHandler();bq.addListenerOnce(w,bs.onsuccess.callback(bq,bt),bs.onsuccess.context);bq.addListenerOnce(s,bs.onfail.callback(bq,bt),bs.onfail.context);bq.addListenerOnce(g,bs.onloadend.callback(bq,bt),bs.onloadend.context);bq.send();return parseInt(bq.toHashCode(),10);}
,setBaseUrl:function(bv){this.__GQ=bv;}
,__GV:function(bw,bx){if(typeof bx!==N){if(qx.core.Environment.get(r)){qx.core.Assert.assertObject(bx,M);}
...
;}
,__GW:function(bE,bC,bD){if(bD){var bF=bE.getRequestHeader(K);if(bE.getMethod&&qx.util.Request.methodAllowsRequestBody(bE.getMethod())){if((/application\/.*\+?json/).test(bF)){bD=qx.lang.Json.stringify(bD);bE.setRequestData(bD);}
;}
;}
;}
...
;}
else {var bJ=bI;var bH=this.__GP[bJ];if(this.__GP[bJ]){bH.forEach(function(bM){bM.abort();}
);}
;}
;}
,refresh:function(bN){this.invoke(bN,this.__GL[bN].params);}
,poll:function(bO,bQ,bR,bP){if(this.__GK[bO]){this.stopPollByAction(bO);}
;if(typeof bR==N){bR=this.__GL[bO].params;}
;if(bP){this.invoke(bO,bR);}
;var bS=(function(bT){return function(){var bU=bT.__GP[bO][0];if(!bP&&!bU){bT.invoke(bO,bR);return;}
;if(bU.isDone()||bU.isDisposed()){bT.refresh(bO);}
;}
;}
)(this);this._startPoll(bO,bS,bQ);}
,_startPoll:function(bV,bW,bX){this.__GK[bV]={"id":window.setInterval(bW,bX),"interval":bX,"listener":bW};}
,stopPollByAction:function(bY){if(bY in this.__GK){var ca=this.__GK[bY].id;window.clearInterval(ca);}
;}
,restartPollByAction:function(cb){if(cb in this.__GK){var cc=this.__GK[cb];this.stopPollByAction(cb);this._startPoll(cb,cc.listener,cc.interval);}
;}
...
;if(!cj){ci=0;}
;return false;}
;var cf=this.__GJ[ch]=this.addListener(ch+G,function ck(){if(ce.isDisposed()){return;}
;if(!cd()){cg=new Date();ce.refresh(ch);}
;}
);this.invoke(ch);return cf;}
,_getRequestConfig:function(cp,co){var cm=this.__GL[cp];var co=qx.lang.Object.clone(co);if(!qx.lang.Type.isArray(cm)){throw new Error(P+cp);}
;var cn=cm[0],cl=this.__GQ!==null?this.__GQ+cm[1]:cm[1],cr=cm[2],cq=qx.bom.rest.Resource.placeholdersFromUrl(cl);co=co||{};cq.forEach(function(cu){var cs=new RegExp(v+cu+c),ct=cl.match(cs)[1];if(typeof co[cu]===N){if(ct){co[cu]=ct;}
else {co[cu]=A;}
;}
...
,__GN:function(cv){Object.keys(cv).forEach(function(cx){var cy=cv[cx],cA=cy.method,cw=cy.url,cz=cy.check;if(qx.core.Environment.get(r)){qx.core.Assert.assertString(cA,I+cx+o);qx.core.Assert.assertString(cw,l+cx+o);}
;this.map(cx,cA,cw,cz);}
,this);}
,__GO:function(cB){if(!this.constructor.$$events){this.constructor.$$events={};}
...
;}
;}
,destruct:function(){var cG;for(cG in this.__GP){if(this.__GP[cG]){this.__GP[cG].forEach(function(cI){cI.dispose();}
);}
;}
;if(this.__GK){for(cG in this.__GK){this.stopPollByAction(cG);}
;}
;if(this.__GJ){for(cG in this.__GJ){var cH=this.__GJ[cG];this.removeListenerById(cH);}
;}
;this.__GP=this.__GL=this.__GK=null;}
}});}
)();
...
;this.useCaching(true);this.setMethod((v!==undefined)?v:m);this._transport=this._registerTransportListener(this._createTransport());qx.core.ObjectRegistry.register(this);this.__ry={};this.__sg=this._createResponseParser();}
,members:{setRequestHeader:function(w,x){this.__ry[w]=x;return this;}
,getRequestHeader:function(y){return this.__ry[y];}
,setUrl:function(z){if(qx.lang.Type.isString(z)){this.__og=z;}
;return this;}
,getUrl:function(){return this.__og;}
,setMethod:function(A){if(qx.util.Request.isMethod(A)){this.__GX=A;}
;return this;}
,getMethod:function(){return this.__GX;}
,setRequestData:function(B){if(qx.lang.Type.isString(B)||qx.lang.Type.isObject(B)){this.__GY=B;}
;return this;}
,getRequestData:function(){return this.__GY;}
,getResponse:function(){if(this.__rE!==null){return this.__rE;}
...
,setParser:function(C){return this.__sg.setParser(C);}
,setTimeout:function(D){if(qx.lang.Type.isNumber(D)){this.__gd=D;}
;return this;}
,getTimeout:function(){return this.__gd;}
...
,send:function(){var K=this.getTimeout(),H=(this.getRequestData()!==null),I=this.__ry.hasOwnProperty(j),F=qx.util.Request.methodAllowsRequestBody(this.getMethod()),L=this.getRequestHeader(k),G=this._serializeData(this.getRequestData(),L);if(this.getMethod()===m&&H){this.setUrl(qx.util.Uri.appendParamsToUrl(this.getUrl(),G));}
...
;this._transport.open(this.getMethod(),this.getUrl(),true);for(var J in this.__ry){this._transport.setRequestHeader(J,this.__ry[J]);}
...
,dispose:function(){if(this._transport.dispose()){this.__sg=null;this.__oh=true;return true;}
;return false;}
...
,_setResponse:function(N){this.__rE=N;}
...
;return null;}
,__ry:null,__GY:null,__GX:d,__og:d,__rE:null,__sg:null,__d:null,__gd:null,__oh:null,addListenerOnce:function(name,S,T){this._transport._emitter.once(name,S,T);return this;}
...
;}
,__rI:function(){if(qx.core.Environment.get(n)){qx.Bootstrap.debug(f+this._transport.status);}
...
;this._setResponse(this.__sg.parse(V,U));this._transport._emit(o);}
else {try{this._setResponse(this.__sg.parse(V,U));}
catch(e){}
...
(function(){var a="changeModel",b="model.user.avatar_url",c="application/json",d="github.view.Gist",e="model.description",f="west",g="GET",h="/gists/{id}",i="Description",j="https://api.github.com",k="demobrowser.demo.data.Github",l="source",m="/gists",n="change",o="Content",p="center",q="value",r="get",s="#fff",t="Authorization",u="html",v="",w="model.user.login",x="model.files",y="north",z="Gists",A="Accept",B="token ",C="bold",D="model",E="github.rest.Resource",F="description",G="undefined",H="</pre>",I="<pre>";qx.Class.define(k,{extend:qx.application.Standalone,members:{__OH:null,__OI:null,__OJ:null,__OK:null,__JE:null,__OL:null,main:function(){if(typeof GITHUB==G){return;}
;qx.application.Standalone.prototype.main.call(this);this._setUpResources();this._setUpStores();this._createGui();this._setUpBinding();this.__OH.get();this.__JE.addListener(a,function(K){var J=K.getData();this.__JE.getSelection().push(J.getItem(0));}
,this);this.__JE.getSelection().addListener(n,function(L){var M=this.__JE.getSelection().getItem(0).getId();this.__OI.get({id:M});}
,this);}
,_setUpResources:function(){this.__OH=new github.rest.Resource({"get":{method:g,url:m}});this.__OI=new github.rest.Resource({"get":{method:g,url:h}});}
,_setUpStores:function(){this.__OJ=new qx.data.store.Rest(this.__OH,r);this.__OK=new qx.data.store.Rest(this.__OI,r);}
,_createGui:function(){var N=new qx.ui.container.Composite(new qx.ui.layout.Dock());N.setPadding(10);var O=new qx.ui.basic.Label(z);O.setFont(C);O.setPaddingBottom(10);N.add(O,{edge:y});this.__JE=new qx.ui.list.List();this.__JE.setWidth(200);this.__OL=new github.view.Gist();N.add(this.__JE,{edge:f});N.add(this.__OL,{edge:p});this.getRoot().add(N,{edge:0});}
,_setUpBinding:function(){var Q=this.__JE,P=this.__OJ,R=this.__OK;Q.setLabelPath(F);Q.setLabelOptions({converter:function(W,U,T,V){if(W===null||!W.length){return U.getId();}
;return W;}
});P.bind(D,Q,D);var S=this.__OL;R.bind(e,S.getDescription(),q);R.bind(w,S.getUsername(),q);R.bind(b,S.getGravatar(),l);R.bind(x,S.getContent(),u,{converter:function(Y){var X=qx.Class.getProperties(Y.constructor);var content=Y.get(X[0]).getContent();content=qx.bom.String.escape(content);return I+content+H;}
});}
...
});qx.Class.define(d,{extend:qx.ui.container.Composite,construct:function(){qx.ui.container.Composite.call(this);var bc=new qx.ui.layout.Grid(5,5);bc.setColumnFlex(0,1);bc.setRowFlex(2,1);this.setLayout(bc);this.setPadding(0,10);var bd;var bf=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));bd=new qx.ui.basic.Label(i);bd.setFont(C);this.__nE=new qx.ui.basic.Label();bf.add(bd);bf.add(this.__nE);this.add(bf,{row:0,column:0});var be=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));this.__OM=new qx.ui.basic.Image();this.__OM.set({width:40,height:40,scale:true,backgroundColor:s});this.__ON=new qx.ui.basic.Label();be.add(this.__OM);be.add(this.__ON);this.add(be,{row:0,column:1});bd=new qx.ui.basic.Label(o);bd.setFont(C);var scroll=new qx.ui.container.Scroll();this.__pM=new qx.ui.embed.Html();this.__pM.setMinHeight(1000);scroll.add(this.__pM);this.add(bd,{row:1,column:0});this.add(scroll,{row:2,column:0,colSpan:2});}
,members:{__nE:v,__OM:v,__ON:v,__pM:v,getDescription:function(){return this.__nE;}
,getGravatar:function(){return this.__OM;}
,getUsername:function(){return this.__ON;}
,getContent:function(){return this.__pM;}
}});}
)();
