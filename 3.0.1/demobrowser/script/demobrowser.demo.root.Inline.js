...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.root.Inline","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.root.Inline.d6099ddecce9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="black",b="red",c="demobrowser.demo.root.Inline",d="isle",e="green",f="white",g="yellow",h="main",i="blue";qx.Class.define(c,{extend:qx.application.Inline,members:{main:function(){qx.application.Inline.prototype.main.call(this);var n=new qx.ui.root.Inline(document.getElementById(d)).set({decorator:h,padding:10,textColor:a,backgroundColor:f});var m=new qx.ui.core.Widget().set({backgroundColor:b,decorator:h,padding:10});var l=new qx.ui.core.Widget().set({backgroundColor:i,decorator:h});var k=new qx.ui.core.Widget().set({backgroundColor:e,decorator:h});var j=new qx.ui.core.Widget().set({backgroundColor:g,decorator:h});n.add(m);n.add(l);n.add(k);n.add(j);n.setLayout(new qx.ui.layout.HBox());}
}});}
)();
(function(){var a="resize",b="mshtml",c="engine.name",d="relative",f="",g=" needs a height when its height size should be used!",h="px",i="qx.debug",j="height",k=" needs a width when its width size should be used!",l="qx.ui.root.Inline",m="$$widget",n="appear",o="hidden",p="div",q="left",r="The root element ",s=" of ",t="Please use a DOM element to create an inline root.";qx.Class.define(l,{extend:qx.ui.root.Abstract,include:[qx.ui.core.MLayoutHandling],construct:function(v,w,u){if(qx.core.Environment.get(i)){this.assertElement(v,t);}
;this.__pc=v;v.style.overflow=o;v.style.textAlign=q;this.__pd=w||false;this.__pe=u||false;this.__pf();qx.ui.root.Abstract.call(this);this._setLayout(new qx.ui.layout.Basic());qx.ui.core.queue.Layout.add(this);qx.ui.core.FocusHandler.getInstance().connectTo(this);if((qx.core.Environment.get(c)==b)){this.setKeepFocus(true);}
;var window=qx.dom.Node.getWindow(v);qx.event.Registration.addListener(window,a,this._onWindowResize,this);}
,members:{__pd:false,__pe:false,__pc:null,__pf:function(){if(this.__pd||this.__pe){var x=qx.bom.element.Dimension.getSize(this.__pc);if(this.__pd&&x.width<1){throw new Error(r+this.__pc+s+this+k);}
;if(this.__pe){if(x.height<1){throw new Error(r+this.__pc+s+this+g);}
;if(x.height>=1&&qx.bom.element.Style.get(this.__pc,j,3)==f){qx.bom.element.Style.set(this.__pc,j,x.height+h);}
;}
;qx.event.Registration.addListener(this.__pc,a,this._onResize,this);}
;}
,_createContentElement:function(){var y=this.__pc;if(this.__pd||this.__pe){var A=document.createElement(p);y.appendChild(A);}
else {A=y;}
;var z=new qx.html.Root(A);A.style.position=d;z.setAttribute(m,this.toHashCode());qx.event.Timer.once(function(e){this.fireEvent(n);}
,this,0);return z;}
,_onResize:function(e){var B=e.getData();if((B.oldWidth!==B.width)&&this.__pd||(B.oldHeight!==B.height)&&this.__pe){qx.ui.core.queue.Layout.add(this);}
;}
...
;}
,_computeSizeHint:function(){var G=this.__pd;var D=this.__pe;if(!G||!D){var C=qx.ui.root.Abstract.prototype._computeSizeHint.call(this);}
else {C={};}
;var H=qx.bom.element.Dimension;if(G){var F=H.getContentWidth(this.__pc);C.width=F;C.minWidth=F;C.maxWidth=F;}
;if(D){var E=H.getContentHeight(this.__pc);C.height=E;C.minHeight=E;C.maxHeight=E;}
;return C;}
},defer:function(I,J){qx.ui.core.MLayoutHandling.remap(J);}
,destruct:function(){qx.event.Registration.removeListener(this.__pc,a,this._onResize,this);this.__pc=null;}
});}
)();
(function(){var a="resize",b="__ju",c="interval",d="body",f="qx.event.handler.ElementResize";qx.Class.define(f,{extend:qx.core.Object,implement:qx.event.IEventHandler,construct:function(g){qx.core.Object.call(this);this.__ge=g;this.__pg={};this.__ju=new qx.event.Timer(200);this.__ju.addListener(c,this._onInterval,this);}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{resize:true},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__pg:null,__ge:null,__ju:null,canHandleEvent:function(i,h){return i.tagName.toLowerCase()!==d;}
,registerEvent:function(m,n,j){var l=qx.core.ObjectRegistry.toHashCode(m);var k=this.__pg;if(!k[l]){k[l]={element:m,width:qx.bom.element.Dimension.getWidth(m),height:qx.bom.element.Dimension.getHeight(m)};this.__ju.start();}
;}
,unregisterEvent:function(r,s,o){var q=qx.core.ObjectRegistry.toHashCode(r);var p=this.__pg;if(p[q]){delete p[q];if(qx.lang.Object.isEmpty(p)){this.__ju.stop();}
;}
;}
,_onInterval:function(e){var u=this.__pg;for(var x in u){var y=u[x];var t=y.element;var w=qx.bom.element.Dimension.getWidth(t);var v=qx.bom.element.Dimension.getHeight(t);if(y.height!==v||y.width!==w){qx.event.Registration.fireNonBubblingEvent(t,a,qx.event.type.Data,[{width:w,oldWidth:y.width,height:v,oldHeight:y.height}]);y.width=w;y.height=v;}
;}
;}
},destruct:function(){this.__ge=this.__pg=null;this._disposeObjects(b);}
