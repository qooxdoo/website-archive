...
,createNamespace:function(name,N){var Q=name.split(K);var P=Q[0];var parent=this.__root&&this.__root[P]?this.__root:window;for(var i=0,O=Q.length-1;i<O;i++ ,P=Q[i]){if(!parent[P]){parent=parent[P]={};}
...
};qx.Bootstrap.define(A,{statics:{__root:null,LOADSTART:qx.$$start||new Date(),DEBUG:(function(){var bk=true;if(qx.$$environment&&qx.$$environment[r]===false){bk=false;}
;return bk;}
...
;}
,createNamespace:qx.Bootstrap.createNamespace,setRoot:function(bo){this.__root=bo;}
...
;}
;return br;}
,__shadowedKeys:[x,k,o,q,G,L,g],keys:({"ES5":Object.keys,"BROKEN_IE":function(bu){if(bu===null||(typeof bu!=M&&typeof bu!=c)){throw new TypeError(f);}
;var bv=[];var bx=Object.prototype.hasOwnProperty;for(var by in bu){if(bx.call(bu,by)){bv.push(by);}
;}
;var bw=qx.Bootstrap.__shadowedKeys;for(var i=0,a=bw,l=a.length;i<l;i++ ){if(bx.call(bu,a[i])){bv.push(a[i]);}
;}
;return bv;}
...
,getClass:function(bL){var bK=Object.prototype.toString.call(bL);return (qx.Bootstrap.__classToTypeMap[bK]||bK.slice(8,-1));}
...
;for(var gz in gx){if(gz.indexOf(fI)!=-1){var gv=gz.split(fI);for(var i=0;i<gv.length;i++ ){if(gv[i]==gy){return gx[gz];}
;}
;}
;}
;if(gx[gc]!==undefined){return gx[gc];}
;if(qx.Bootstrap.DEBUG){throw new Error(ed+gy+ga+(typeof gy)+m+eR+qx.Bootstrap.keys(gx)+g);}
;}
...
;}
;}
,__importFromUrl:function(){if(window.document&&window.document.location){var gG=window.document.location.search.slice(1).split(fw);for(var i=0;i<gG.length;i++ ){var gJ=gG[i].split(u);if(gJ.length!=3||gJ[0]!=fi){continue;}
;var gH=gJ[1];var gI=decodeURIComponent(gJ[2]);if(gI==fG){gI=true;}
else if(gI==eV){gI=false;}
...
,null,gK);}
},defer:function(gM){gM._initDefaultQxValues();gM.__importFromGenerator();if(gM.get(fQ)===true){gM.__importFromUrl();}
;}
});}
)();
...
(function(){var a="qxWeb";qx.Bootstrap.define(a,{extend:qx.type.BaseArray,statics:{__init:[],$$qx:qx,$init:function(e){var d=[];for(var i=0;i<e.length;i++ ){var c=!!(e[i]&&(e[i].nodeType===1||e[i].nodeType===9));if(c){d.push(e[i]);continue;}
...
else {var L=qxWeb.getDocument(K[0]);O=qx.module.Css.__getDisplayDefault(K[0].tagName,L);}
;K.setStyle(d,O);K[0].$$qPrevDisp=a;}
;}
);return this;}
,__displayDefaults:{},__getDisplayDefault:function(T,Q){var S=qx.module.Css.__displayDefaults;if(!S[T]){var U=Q||document;var R=qxWeb(U.createElement(T)).appendTo(Q.body);S[T]=R.getStyle(d);R.remove();}
;return S[T]||c;}
...
;return N;}
,getNextAll:function(Q){var P=qx.module.Traversing.__hierarchyHelper(this,d,Q);return qxWeb.$init(P);}
...
;return W;}
,getPrevAll:function(ba){var Y=qx.module.Traversing.__hierarchyHelper(this,a,ba);return qxWeb.$init(Y);}
...
,getSiblings:function(bh){var bg=qx.module.Traversing.__hierarchyHelper(this,b,bh);return qxWeb.$init(bg);}
...
;}
;var ba,bc=[];for(var i=0,l=X.length;i<l;i++ ){ba=X[i];if(typeof ba===k){ba=this.__convertHtmlString(ba,be);}
...
;}
;bf.forEach(function(bh){bh.__emitter=null;}
...
;}
;}
,__isReady:false,ready:function(bi){if(document.readyState===o){window.setTimeout(bi,1);return;}
;var bj=function(){qx.module.Event.__isReady=true;bi();}
...
else {var bl=function(){if(qx.module.Event.__isReady){return;}
...
;var bo=qx.module.Event.__normalizations;for(var i=0,l=bq.length;i<l;i++ ){var bp=bq[i];if(qx.lang.Type.isFunction(bn)){if(!bo[bp]){bo[bp]=[];}
;bo[bp].push(bn);}
;}
;}
,$unregisterNormalization:function(bu,br){if(!qx.lang.Type.isArray(bu)){bu=[bu];}
;var bs=qx.module.Event.__normalizations;for(var i=0,l=bu.length;i<l;i++ ){var bt=bu[i];if(bs[bt]){qx.lang.Array.remove(bs[bt],br);}
;}
;}
...
;var by=qx.module.Event.__hooks.on;for(var i=0,l=bA.length;i<l;i++ ){var bz=bA[i];if(qx.lang.Type.isFunction(bx)){if(!by[bz]){by[bz]=[];}
;by[bz].push(bx);}
;}
;if(!bw){return;}
;var bv=qx.module.Event.__hooks.off;for(var i=0,l=bA.length;i<l;i++ ){var bz=bA[i];if(qx.lang.Type.isFunction(bw)){if(!bv[bz]){bv[bz]=[];}
;bv[bz].push(bw);}
;}
;}
,$unregisterEventHook:function(bG,bD,bC){if(!qx.lang.Type.isArray(bG)){bG=[bG];}
;var bE=qx.module.Event.__hooks.on;for(var i=0,l=bG.length;i<l;i++ ){var bF=bG[i];if(bE[bF]){qx.lang.Array.remove(bE[bF],bD);}
;}
;if(!bC){return;}
;var bB=qx.module.Event.__hooks.off;for(var i=0,l=bG.length;i<l;i++ ){var bF=bG[i];if(bB[bF]){qx.lang.Array.remove(bB[bF],bC);}
;}
;}
...
,appendTo:function(parent){parent=qx.module.Manipulating.__getElementArray(parent);for(var i=0,l=parent.length;i<l;i++ ){this._forEachElement(function(q,j){if(i==0){qx.dom.Element.insertEnd(this[j],parent[i]);}
else {qx.dom.Element.insertEnd(this.eq(j).clone(true)[0],parent[i]);}
;}
);}
;return this;}
,insertBefore:function(r){r=qx.module.Manipulating.__getElementArray(r);for(var i=0,l=r.length;i<l;i++ ){this._forEachElement(function(s,t){if(i==0){qx.dom.Element.insertBefore(s,r[i]);}
else {qx.dom.Element.insertBefore(this.eq(t).clone(true)[0],r[i]);}
;}
);}
;return this;}
,insertAfter:function(u){u=qx.module.Manipulating.__getElementArray(u);for(var i=0,l=u.length;i<l;i++ ){for(var j=this.length-1;j>=0;j-- ){if(!this[j]||this[j].nodeType!==1){continue;}
;if(i==0){qx.dom.Element.insertAfter(this[j],u[i]);}
else {qx.dom.Element.insertAfter(this.eq(j).clone(true)[0],u[i]);}
;}
;}
;return this;}
,__getElementArray:function(w){if(!qx.lang.Type.isArray(w)){var v=qxWeb(w);w=v.length>0?v:[w];}
;return w.filter(function(x){return (x&&x.nodeType===1);}
);}
,wrap:function(y){y=qx.module.Manipulating.__getCollectionFromArgument(y);if(y.length==0){return this;}
;this._forEachElement(function(z){var A=y.eq(0).clone(true);qx.dom.Element.insertAfter(A[0],z);var B=qx.module.Manipulating.__getInnermostElement(A[0]);qx.dom.Element.insertEnd(z,B);}
);return this;}
,__getCollectionFromArgument:function(E){var C;if(qx.lang.Type.isArray(E)){C=qxWeb(E);}
else {var D=qx.bom.Html.clean([E]);if(D.length>0&&qx.dom.Node.isElement(D[0])){C=qxWeb(D);}
else {C=qxWeb(E);}
;}
;return C;}
,__getInnermostElement:function(F){if(F.childNodes.length==0){return F;}
;for(var i=0,l=F.childNodes.length;i<l;i++ ){if(F.childNodes[i].nodeType===1){return this.__getInnermostElement(F.childNodes[i]);}
;}
;return F;}
...
;return T.scrollTop;}
,_animationDescription:{scrollLeft:{duration:700,timing:c,keep:100,keyFrames:{'0':{},'100':{scrollLeft:1}}},scrollTop:{duration:700,timing:c,keep:100,keyFrames:{'0':{},'100':{scrollTop:1}}}},__animateScroll:function(V,W,U){var X=qx.lang.Object.clone(qx.module.Manipulating._animationDescription[V],true);X.keyFrames[100][V]=W;return this.animate(X,U);}
,setScrollLeft:function(ba,bb){var Node=qx.dom.Node;if(bb&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__animateScroll.bind(this,a,ba,bb)();}
...
;}
;return this;}
,setScrollTop:function(bd,be){var Node=qx.dom.Node;if(be&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__animateScroll.bind(this,d,bd,be)();}
...
(function(){var a="touchmove",b="os.name",c="MSPointerDown",d="swipe",e="android",f="pointercancel",g="qx.event.handler.TouchCore",h="event.mspointer",j="MSPointerCancel",k="y",l="pointer-events",m="longtap",n="pointerup",o="touchend",p="pointerdown",q="MSPointerUp",r="right",s="engine.name",t="x",u="touchcancel",v="MSPointerMove",w="webkit",z="none",A="left",B="tap",C="pointermove",D="down",E="undefined",F="up",G="touchstart";qx.Bootstrap.define(g,{extend:Object,statics:{TAP_MAX_DISTANCE:qx.core.Environment.get(b)!=e?10:40,SWIPE_DIRECTION:{x:[A,r],y:[F,D]},SWIPE_MIN_DISTANCE:qx.core.Environment.get(b)!=e?11:41,SWIPE_MIN_VELOCITY:0,LONGTAP_TIME:500},construct:function(H,I){this.__target=H;this.__emitter=I;this._initTouchObserver();}
,members:{__target:null,__emitter:null,__onTouchEventWrapper:null,__originalTarget:null,__startPageX:null,__startPageY:null,__startTime:null,__isSingleTouchGesture:null,__isTapGesture:null,__onMove:null,__beginScalingDistance:null,__beginRotation:null,__longTapTimer:null,_initTouchObserver:function(){this.__onTouchEventWrapper=qx.lang.Function.listener(this._onTouchEvent,this);var Event=qx.bom.Event;Event.addNativeListener(this.__target,G,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,a,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,o,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,u,this.__onTouchEventWrapper);if(qx.core.Environment.get(h)){Event.addNativeListener(this.__target,c,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,v,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,q,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,j,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,p,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,C,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,n,this.__onTouchEventWrapper);Event.addNativeListener(this.__target,f,this.__onTouchEventWrapper);}
;}
,_stopTouchObserver:function(){var Event=qx.bom.Event;Event.removeNativeListener(this.__target,G,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,a,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,o,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,u,this.__onTouchEventWrapper);if(qx.core.Environment.get(h)){Event.removeNativeListener(this.__target,c,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,v,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,q,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,j,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,p,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,C,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,n,this.__onTouchEventWrapper);Event.removeNativeListener(this.__target,f,this.__onTouchEventWrapper);}
;}
...
;if(O==G){this.__originalTarget=this._getTarget(Q);this.__isTapGesture=true;if(Q.touches&&Q.touches.length>1){this.__beginScalingDistance=this._getScalingDistance(Q.touches[0],Q.touches[1]);this.__beginRotation=this._getRotationAngle(Q.touches[0],Q.touches[1]);}
;}
;if(O==a){if(typeof Q.scale==E&&Q.targetTouches.length>1){var R=this._getScalingDistance(Q.targetTouches[0],Q.targetTouches[1]);Q.scale=R/this.__beginScalingDistance;}
;if(typeof Q.rotation==E&&Q.targetTouches.length>1){var P=this._getRotationAngle(Q.targetTouches[0],Q.targetTouches[1]);Q.rotation=P-this.__beginRotation;}
;if(this.__isTapGesture){this.__isTapGesture=this._isBelowTapMaxDistance(Q.changedTouches[0]);}
;}
;this._fireEvent(Q,O);this.__checkAndFireGesture(Q,O);}
,_mapPointerEvent:function(S){S=S.toLowerCase();if(S.indexOf(p)!==-1){return G;}
else if(S.indexOf(n)!==-1){return o;}
else if(S.indexOf(C)!==-1){if(this.__onMove===true){return a;}
;}
else if(S.indexOf(f)!==-1){return u;}
;return S;}
,_isBelowTapMaxDistance:function(T){var U={x:T.screenX-this.__startPageX,y:T.screenY-this.__startPageY};var V=qx.event.handler.TouchCore;return (Math.abs(U.x)<=V.TAP_MAX_DISTANCE&&Math.abs(U.y)<=V.TAP_MAX_DISTANCE);}
,_getTarget:function(X){var Y=qx.bom.Event.getTarget(X);if(qx.core.Environment.get(s)==w){if(Y&&Y.nodeType==3){Y=Y.parentNode;}
;}
else if(qx.core.Environment.get(h)){var W=this.__evaluateTarget(X);if(W){Y=W;}
;}
;return Y;}
,__evaluateTarget:function(bc){if(bc&&bc.touches){var ba=bc.touches[0].clientX;var bb=bc.touches[0].clientY;}
...
;var bh=bh||bg.type;if(bi&&bi.nodeType&&this.__emitter){this.__emitter.emit(bh,bg);}
;}
,__checkAndFireGesture:function(bj,bk,bl){if(!bl){bl=this._getTarget(bj);}
;var bk=bk||bj.type;if(bk==G){this.__gestureStart(bj,bl);}
else if(bk==a){this.__gestureChange(bj,bl);}
else if(bk==o){this.__gestureEnd(bj,bl);}
;}
,__gestureStart:function(bn,bo){var bm=bn.changedTouches[0];this.__onMove=true;this.__startPageX=bm.screenX;this.__startPageY=bm.screenY;this.__startTime=new Date().getTime();this.__isSingleTouchGesture=bn.targetTouches.length===1;if(this.__isSingleTouchGesture){this.__longTapTimer=window.setTimeout(this.__fireLongTap.bind(this,bn,bo),qx.event.handler.TouchCore.LONGTAP_TIME);}
else {this.__stopLongTapTimer();}
;}
,__gestureChange:function(bp,bq){if(this.__isSingleTouchGesture&&bp.changedTouches.length>1){this.__isSingleTouchGesture=false;}
;if(!this._isBelowTapMaxDistance(bp.changedTouches[0])){this.__stopLongTapTimer();}
;}
,__gestureEnd:function(br,bs){this.__onMove=false;this.__stopLongTapTimer();if(this.__isSingleTouchGesture){var bv=br.changedTouches[0];var bw={x:bv.screenX-this.__startPageX,y:bv.screenY-this.__startPageY};var bt;if(this.__originalTarget==bs&&this.__isTapGesture){if(qx.event&&qx.event.type&&qx.event.type.Tap){bt=qx.event.type.Tap;}
;this._fireEvent(br,B,bs,bt);}
else {var bu=this.__getSwipeGesture(br,bs,bw);if(bu){if(qx.event&&qx.event.type&&qx.event.type.Swipe){bt=qx.event.type.Swipe;}
;br.swipe=bu;this._fireEvent(br,d,bs,bt);}
;}
;}
;}
,__getSwipeGesture:function(bz,bA,bF){var bC=qx.event.handler.TouchCore;var bD=new Date().getTime()-this.__startTime;var bG=(Math.abs(bF.x)>=Math.abs(bF.y))?t:k;var bx=bF[bG];var by=bC.SWIPE_DIRECTION[bG][bx<0?0:1];var bE=(bD!==0)?bx/bD:0;var bB=null;if(Math.abs(bE)>=bC.SWIPE_MIN_VELOCITY&&Math.abs(bx)>=bC.SWIPE_MIN_DISTANCE){bB={startTime:this.__startTime,duration:bD,axis:bG,direction:by,distance:bx,velocity:bE};}
;return bB;}
,__fireLongTap:function(bH,bI){this._fireEvent(bH,m,bI,qx.event.type.Tap);this.__longTapTimer=null;this.__isTapGesture=false;}
,__stopLongTapTimer:function(){if(this.__longTapTimer){window.clearTimeout(this.__longTapTimer);this.__longTapTimer=null;}
;}
,dispose:function(){this._stopTouchObserver();this.__originalTarget=this.__target=this.__emitter=this.__beginScalingDistance=this.__beginRotation=null;this.__stopLongTapTimer();}
}});}
)();
