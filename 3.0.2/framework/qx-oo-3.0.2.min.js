...
;if(config.dereference===undefined&&typeof config.check===Y){config.dereference=this.__x(config.check);}
...
;{}
;}
,__x:function(dr){return !!this.__r[dr];}
,__y:{'0':db,'1':br,'2':A,'3':bP,'4':cA,'5':L},error:function(ds,dy,dx,dt,du){var dv=ds.constructor.classname;var dw=q+dx+T+dv+x+this.$$method[dt][dx]+cT+du+S;throw new Error(dw+(this.__y[dy]||cu+dy));}
,__z:function(instance,members,name,variant,code,args){var store=this.$$method[variant][name];{members[store]=new Function(cC,code.join(cI));}
;{}
...
;return this.__z(dC,dD,name,dA,dz);}
,executeOptimizedSetter:function(dM,dL,name,dK,dJ){var dO=dL.$$properties[name];var dN=dL.prototype;var dH=[];var dG=dK===e||dK===bv||dK===bK||(dK===cv&&dO.init===undefined);var dI=dO.apply||dO.event||dO.inheritable;var dP=this.__A(dK,name);this.__B(dH,dO,name,dK,dG);if(dG){this.__C(dH,dL,dO,name);}
;if(dI){this.__D(dH,dG,dP,dK);}
;if(dO.inheritable){dH.push(bk);}
;{}
;if(!dI){this.__F(dH,name,dK,dG);}
else {this.__G(dH,dO,name,dK,dG);}
;if(dO.inheritable){this.__H(dH,dO,name,dK);}
else if(dI){this.__I(dH,dO,name,dK);}
;if(dI){this.__J(dH,dO,name,dK);if(dO.inheritable&&dN._getChildren){this.__K(dH,name);}
;}
;if(dG){dH.push(bL);}
;return this.__z(dM,dN,name,dK,dH,dJ);}
,__A:function(dQ,name){if(dQ===bK||dQ===R){var dR=this.$$store.runtime[name];}
else if(dQ===bv||dQ===g){dR=this.$$store.theme[name];}
else if(dQ===cv){dR=this.$$store.init[name];}
else {dR=this.$$store.user[name];}
;return dR;}
,__B:function(dU,dS,name,dV,dT){{if(!dS.nullable||dS.check||dS.inheritable){dU.push(cR);}
;if(dV===e){dU.push(M,name,bg,dV,bs);}
;}
;}
,__C:function(dW,dY,dX,name){if(dX.transform){dW.push(bf,dX.transform,bx);}
...
;}
;}
,__D:function(eb,ea,ed,ec){var ee=(ec===X||ec===g||ec===R);if(ea){eb.push(I,ed,j);}
else if(ee){eb.push(I,ed,V);}
;}
,__E:undefined,__F:function(eg,name,eh,ef){if(eh===bK){eg.push(bq,this.$$store.runtime[name],bY);}
...
;}
,__G:function(ek,ei,name,el,ej){if(ei.inheritable){ek.push(O,this.$$store.inherit[name],f);}
else {ek.push(cl);}
...
;ek.push(co);}
;}
,__H:function(en,em,name,eo){en.push(bC);if(eo===cP){en.push(bt);}
...
;{}
;var A=this.__O(name,C.type,C.extend,C.statics,C.construct,C.destruct,C.include);if(C.extend){if(C.properties){this.__Q(A,C.properties,true);}
;if(C.members){this.__S(A,C.members,true,true,false);}
;if(C.events){this.__P(A,C.events,true);}
;if(C.include){for(var i=0,l=C.include.length;i<l;i++ ){this.__W(A,C.include[i],false);}
;}
;}
...
;}
;if(C.implement){for(var i=0,l=C.implement.length;i<l;i++ ){this.__U(A,C.implement[i]);}
;}
;{}
;if(C.defer){C.defer.self=A;C.defer(A,A.prototype,{add:function(name,D){var E={};E[name]=D;qx.Class.__Q(A,E,true);}
});}
;return A;}
...
;qx.Class.__W(J,I,false);}
,patch:function(L,K){{}
;qx.Class.__W(L,K,true);}
...
,$$registry:qx.Bootstrap.$$registry,__i:null,__M:null,__j:function(name,bg){}
,__N:function(bh){}
,__O:function(name,br,bq,bi,bo,bm,bl){var bn;if(!bq&&true){bn=bi||{};qx.Bootstrap.setDisplayNames(bn,name);}
else {bn={};if(bq){if(!bo){bo=this.__X();}
;if(this.__ba(bq,bl)){bn=this.__bb(bo,name,br);}
else {bn=bo;}
...
,__P:function(bs,bt,bv){{var bu,bu;}
;if(bs.$$events){for(var bu in bt){bs.$$events[bu]=bt[bu];}
;}
else {bs.$$events=bt;}
;}
,__Q:function(bx,bA,by){if(!qx.core.Environment.get(g)){throw new Error(e);}
...
;var event={};event[bz.event]=j;this.__P(bx,event,by);}
;if(bz.inheritable){this.__L.$$inheritable[name]=true;if(!bw.$$refreshInheritables){this.__L.attachRefreshInheritables(bx);}
;}
;if(!bz.refine){this.__L.attachMethods(bx,name,bz);}
;}
;}
,__R:null,__S:function(bI,bB,bD,bF,bH){var bC=bI.prototype;var bG,bE;qx.Bootstrap.setDisplayNames(bB,bI.classname+b);for(var i=0,a=Object.keys(bB),l=a.length;i<l;i++ ){bG=a[i];bE=bB[bG];{}
;if(bF!==false&&bE instanceof Function&&bE.$$type==null){if(bH==true){bE=this.__T(bE,bC[bG]);}
...
;bC[bG]=bE;}
;}
,__T:function(bJ,bK){if(bK){return function(){var bM=bJ.base;bJ.base=bK;var bL=bJ.apply(this,arguments);bJ.base=bM;return bL;}
;}
else {return bJ;}
;}
,__U:function(bP,bN){{}
...
;}
,__V:function(bR){var name=bR.classname;var bQ=this.__bb(bR,name,bR.$$classtype);for(var i=0,a=Object.keys(bR),l=a.length;i<l;i++ ){bS=a[i];bQ[bS]=bR[bS];}
...
,__W:function(cc,ca,bY){{}
;if(this.hasMixin(cc,ca)){return;}
;var bW=cc.$$original;if(ca.$$constructor&&!bW){cc=this.__V(cc);}
;var bX=qx.Mixin.flatten([ca]);var cb;for(var i=0,l=bX.length;i<l;i++ ){cb=bX[i];if(cb.$$events){this.__P(cc,cb.$$events,bY);}
;if(cb.$$properties){this.__Q(cc,cb.$$properties,bY);}
;if(cb.$$members){this.__S(cc,cb.$$members,bY,bY,bY);}
;}
;if(cc.$$includes){cc.$$includes.push(ca);cc.$$flatIncludes.push.apply(cc.$$flatIncludes,bX);}
else {cc.$$includes=[ca];cc.$$flatIncludes=bX;}
;}
,__X:function(){function cd(){cd.base.apply(this,arguments);}
;return cd;}
...
;}
;return false;}
,__bb:function(cj,name,ci){var cl=function(){var co=cl;{}
...
(function(){var a=". Error message: ",b="Boolean",c="set",d="deepBinding",f=") to the object '",g="item",h="Please use only one array at a time: ",k="Integer",l="reset",m=" of object ",n="qx.data.SingleValueBinding",p="Binding property ",q="Failed so set value ",r="change",s="Binding could not be found!",t="get",u="^",v=" does not work.",w="String",x="Binding from '",y="",z="PositiveNumber",A="]",B="[",C=".",D="PositiveInteger",E='No number or \'last\' value hast been given in an array binding: ',F="' (",G=" on ",H="Binding does not exist!",I="Number",J=").",K="Date",L=" not possible: No event available. ",M="last";qx.Class.define(n,{statics:{__bc:{},bind:function(Q,be,bc,S,bb){var bf=this.__be(Q,be,bc,S,bb);var U=be.split(C);var P=this.__bk(U);var Y=[];var T=[];var V=[];var ba=[];var R=Q;try{for(var i=0;i<U.length;i++ ){if(P[i]!==y){ba.push(r);}
else {ba.push(this.__bf(R,U[i]));}
;Y[i]=R;if(i==U.length-1){if(P[i]!==y){var bh=P[i]===M?R.length-1:P[i];var O=R.getItem(bh);this.__bj(O,bc,S,bb,Q);V[i]=this.__bl(R,ba[i],bc,S,bb,P[i]);}
else {if(U[i]!=null&&R[t+qx.lang.String.firstUp(U[i])]!=null){var O=R[t+qx.lang.String.firstUp(U[i])]();this.__bj(O,bc,S,bb,Q);}
;V[i]=this.__bl(R,ba[i],bc,S,bb);}
;}
else {var N={index:i,propertyNames:U,sources:Y,listenerIds:V,arrayIndexValues:P,targetObject:bc,targetPropertyChain:S,options:bb,listeners:T};var X=qx.lang.Function.bind(this.__bd,this,N);T.push(X);V[i]=R.addListener(ba[i],X);}
...
;}
;throw bi;}
;var bg={type:d,listenerIds:V,sources:Y,targetListenerIds:bf.listenerIds,targets:bf.targets};this.__bm(bg,Q,be,bc,S);return bg;}
,__bd:function(bp){if(bp.options&&bp.options.onUpdate){bp.options.onUpdate(bp.sources[bp.index],bp.targetObject);}
...
;this.__bh(bp.targetObject,bp.targetPropertyChain,br);}
else {this.__bg(bp.targetObject,bp.targetPropertyChain);}
;break;}
;if(j==bp.propertyNames.length-1){if(qx.Class.implementsInterface(bn,qx.data.IListData)){var bs=bp.arrayIndexValues[j]===M?bn.length-1:bp.arrayIndexValues[j];var bk=bn.getItem(bs);this.__bj(bk,bp.targetObject,bp.targetPropertyChain,bp.options,bp.sources[bp.index]);bp.listenerIds[j]=this.__bl(bn,r,bp.targetObject,bp.targetPropertyChain,bp.options,bp.arrayIndexValues[j]);}
else {if(bp.propertyNames[j]!=null&&bn[t+qx.lang.String.firstUp(bp.propertyNames[j])]!=null){var bk=bn[t+qx.lang.String.firstUp(bp.propertyNames[j])]();this.__bj(bk,bp.targetObject,bp.targetPropertyChain,bp.options,bp.sources[bp.index]);}
;var bl=this.__bf(bn,bp.propertyNames[j]);bp.listenerIds[j]=this.__bl(bn,bl,bp.targetObject,bp.targetPropertyChain,bp.options);}
;}
else {if(bp.listeners[j]==null){var bm=qx.lang.Function.bind(this.__bd,this,bp);bp.listeners.push(bm);}
;if(qx.Class.implementsInterface(bn,qx.data.IListData)){var bl=r;}
else {var bl=this.__bf(bn,bp.propertyNames[j]);}
;bp.listenerIds[j]=bn.addListener(bl,bp.listeners[j]);}
;}
;}
,__be:function(bu,bC,bG,by,bA){var bx=by.split(C);var bv=this.__bk(bx);var bF=[];var bE=[];var bz=[];var bD=[];var bw=bG;for(var i=0;i<bx.length-1;i++ ){if(bv[i]!==y){bD.push(r);}
...
;}
;}
,__bi:function(ct,cq){var cs=cq.split(C);var cp=ct;for(var i=0;i<cs.length-1;i++ ){try{var cr=cs[i];if(cr.indexOf(A)==cr.length-1){var co=cr.substring(cr.indexOf(B)+1,cr.length-1);cr=cr.substring(0,cr.indexOf(B));}
...
;}
;return cp;}
,__bj:function(cz,cv,cx,cy,cw){cz=this.__bn(cz,cv,cx,cy,cw);if(cz===undefined){this.__bg(cv,cx);}
;if(cz!==undefined){try{this.__bh(cv,cx,cz);if(cy&&cy.onUpdate){cy.onUpdate(cw,cv,cz);}
;}
...
;}
;}
,__bk:function(cA){var cB=[];for(var i=0;i<cA.length;i++ ){var name=cA[i];if(qx.lang.String.endsWith(name,A)){var cC=name.substring(name.indexOf(B)+1,name.indexOf(A));if(name.indexOf(A)!=name.length-1){throw new Error(h+name+v);}
...
;}
;return cB;}
,__bl:function(cD,cG,cL,cJ,cH,cF){{var cE;}
;var cI=function(cO,e){if(cO!==y){if(cO===M){cO=cD.length-1;}
;var cP=cD.getItem(cO);if(cP===undefined){qx.data.SingleValueBinding.__bg(cL,cJ);}
;var cN=e.getData().start;var cM=e.getData().end;if(cO<cN||cO>cM){return;}
;}
else {var cP=e.getData();}
;{}
;cP=qx.data.SingleValueBinding.__bn(cP,cL,cJ,cH,cD);{}
;try{if(cP!==undefined){qx.data.SingleValueBinding.__bh(cL,cJ,cP);}
else {qx.data.SingleValueBinding.__bg(cL,cJ);}
...
,__bm:function(cV,cR,cU,cS,cT){if(this.__bc[cR.toHashCode()]===undefined){this.__bc[cR.toHashCode()]=[];}
;this.__bc[cR.toHashCode()].push([cV,cR,cU,cS,cT]);}
,__bn:function(da,df,cY,db,cW){if(db&&db.converter){var dc;if(df.getModel){dc=df.getModel();}
;return db.converter(da,dc,cW,df);}
else {var cX=this.__bi(df,cY);var dg=cY.substring(cY.lastIndexOf(C)+1,cY.length);if(cX==null){return da;}
;var dd=qx.Class.getPropertyDefinition(cX.constructor,dg);var de=dd==null?y:dd.check;return this.__bp(da,de);}
;}
,__bo:function(dh,dj){var di=qx.Class.getPropertyDefinition(dh.constructor,dj);if(di==null){return null;}
;return di.event;}
,__bp:function(dm,dl){var dk=qx.lang.Type.getClass(dm);if((dk==I||dk==w)&&(dl==k||dl==D)){dm=parseInt(dm,10);}
...
;var dp=this.__bc[dn.toHashCode()];if(dp!=undefined){for(var i=0;i<dp.length;i++ ){if(dp[i][0]==dq){qx.lang.Array.remove(dp,dp[i]);return;}
;}
;}
;throw new Error(s);}
,removeAllBindingsForObject:function(ds){{}
;var dr=this.__bc[ds.toHashCode()];if(dr!=undefined){for(var i=dr.length-1;i>=0;i-- ){this.removeBindingFromObject(ds,dr[i][0]);}
;}
;}
,getAllBindingsForObject:function(dt){if(this.__bc[dt.toHashCode()]===undefined){this.__bc[dt.toHashCode()]=[];}
;return this.__bc[dt.toHashCode()];}
,removeAllBindings:function(){for(var dv in this.__bc){var du=qx.core.ObjectRegistry.fromHashCode(dv);if(du==null){delete this.__bc[dv];continue;}
;this.removeAllBindingsForObject(du);}
;this.__bc={};}
,getAllBindings:function(){return this.__bc;}
,showBindingInLog:function(dx,dz){var dy;for(var i=0;i<this.__bc[dx.toHashCode()].length;i++ ){if(this.__bc[dx.toHashCode()][i][0]==dz){dy=this.__bc[dx.toHashCode()][i];break;}
;}
;if(dy===undefined){var dw=H;}
else {var dw=x+dy[1]+F+dy[2]+f+dy[3]+F+dy[4]+J;}
;qx.log.Logger.debug(dw);}
,showAllBindingsInLog:function(){for(var dB in this.__bc){var dA=qx.core.ObjectRegistry.fromHashCode(dB);for(var i=0;i<this.__bc[dB].length;i++ ){this.showBindingInLog(dA,this.__bc[dB][i][0]);}
;}
;}
}});}
)();
...
(function(){var c="-",d="",e="qx.core.ObjectRegistry",f="Disposed ",g="$$hash",h="-0",j=" objects",k="Could not dispose object ",m=": ";qx.Bootstrap.define(e,{statics:{inShutDown:false,__bI:{},__bT:0,__bU:[],__bV:d,__bW:{},register:function(n){var q=this.__bI;if(!q){return;}
...
;var t=this.__bI;if(t&&t[s]){delete t[s];this.__bU.push(s);}
;try{delete r.$$hash;}
...
;}
;}
,fromHashCode:function(B){return this.__bI[B]||null;}
,shutdown:function(){this.inShutDown=true;var D=this.__bI;var F=[];for(var C in D){F.push(C);}
...
;}
;break;}
;qx.Bootstrap.debug(this,f+l+j);delete this.__bI;}
...
;}
;qx.core.ObjectRegistry.unregister(this);this.__cK=null;if(qx.core.Environment.get(d)){var F=this.constructor;var J;var K=this.__L.$$store;var H=K.user;var I=K.theme;var D=K.inherit;var G=K.useinit;var E=K.init;while(F){J=F.$$properties;if(J){for(var name in J){if(J[name].dereference){this[H[name]]=this[I[name]]=this[D[name]]=this[G[name]]=this[E[name]]=undefined;}
;}
;}
;F=F.superclass;}
;}
...
(function(){var a='"',b="",c="Unsupported type!",d="change",e="qx.data.marshal.Json",f="Array",g="_validate",h="set",j="_applyEventPropagation",k="qx.data.model.";qx.Class.define(e,{extend:qx.core.Object,implement:[qx.data.marshal.IMarshaler],construct:function(l){qx.core.Object.call(this);this.__tr=l;}
...
},members:{__tr:null,__ts:function(o){return Object.keys(o).sort().join(a);}
,toClass:function(p,q){this.__tt(p,q,null,0);}
,__tt:function(C,x,r,z){if(!qx.lang.Type.isObject(C)||!!C.$$isString||C instanceof qx.core.Object){if(C instanceof Array||qx.Bootstrap.getClass(C)==f){for(var i=0;i<C.length;i++ ){this.__tt(C[i],x,null,z+1);}
;}
;return;}
;var t=this.__ts(C);if(this.__tx(t,r,z)){return;}
;for(var B in C){this.__tt(C[B],x,B,z+1);}
;if(qx.Class.isDefined(k+t)){return;}
;if(this.__tr&&this.__tr.getModelClass&&this.__tr.getModelClass(t,C)!=null){return;}
;var D={};var u={__tu:this.__tu};for(var B in C){if(this.__tr&&this.__tr.getPropertyMapping){B=this.__tr.getPropertyMapping(B,t);}
;B=B.replace(/-|\.|\s+/g,b);{}
;D[B]={};D[B].nullable=true;D[B].event=d+qx.lang.String.firstUp(B);if(x){D[B].apply=j;}
;if(this.__tr&&this.__tr.getValidationRule){var w=this.__tr.getValidationRule(t,B);if(w){D[B].validate=g+B;u[g+B]=w;}
;}
;}
;if(this.__tr&&this.__tr.getModelSuperClass){var A=this.__tr.getModelSuperClass(t)||qx.core.Object;}
else {var A=qx.core.Object;}
;var v=[];if(this.__tr&&this.__tr.getModelMixins){var y=this.__tr.getModelMixins(t);if(!qx.lang.Type.isArray(y)){if(y!=null){v=[y];}
;}
else {v=y;}
;}
;if(x){v.push(qx.data.marshal.MEventBubbling);}
;var s={extend:A,include:v,properties:D,members:u,destruct:this.__tv};qx.Class.define(k+t,s);}
,__tv:function(){var E=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var F in E){this.__tu(this.get(E[F].name));}
;}
,__tu:function(G){if(!(G instanceof qx.core.Object)){return;}
;if(G.isDisposed()){return;}
;G.dispose();}
,__tw:function(I,H){var J;if(this.__tr&&this.__tr.getModelClass){J=this.__tr.getModelClass(I,H);}
;if(J!=null){return (new J());}
else {var K=qx.Class.getByName(k+I);return (new K());}
;}
,__tx:function(L,O,M){var N=this.__tr;return N&&N.ignore&&N.ignore(L,O,M);}
,toModel:function(P){return this.__ty(P,null,0);}
,__ty:function(ba,R,W){var V=qx.lang.Type.isObject(ba);var Q=ba instanceof Array||qx.Bootstrap.getClass(ba)==f;if((!V&&!Q)||!!ba.$$isString||ba instanceof qx.core.Object){return ba;}
else if(this.__tx(this.__ts(ba),R,W)){return ba;}
else if(Q){var bb=new qx.data.Array();bb.setAutoDisposeItems(true);for(var i=0;i<ba.length;i++ ){bb.push(this.__ty(ba[i],null,W+1));}
;return bb;}
else if(V){var S=this.__ts(ba);var Y=this.__tw(S,ba);for(var X in ba){var T=X;if(this.__tr&&this.__tr.getPropertyMapping){T=this.__tr.getPropertyMapping(X,S);}
;var bc=T.replace(/-|\.|\s+/g,b);{}
;T=bc;var U=h+qx.lang.String.firstUp(T);if(Y[U]){Y[U](this.__ty(ba[X],X,W+1));}
;}
;return Y;}
;throw new Error(c);}
...
,pop:function(){var v=this.__mS.pop();this.__mT();this._registerEventChaining(null,v,this.length-1);this.fireDataEvent(o,{value:[],name:this.length+f,old:[v],item:this});this.fireDataEvent(g,{start:this.length-1,end:this.length-1,type:l,items:[v],removed:[v],added:[]},null);return v;}
,push:function(w){for(var i=0;i<arguments.length;i++ ){this.__mS.push(arguments[i]);this.__mT();this._registerEventChaining(arguments[i],null,this.length-1);this.fireDataEvent(o,{value:[arguments[i]],name:(this.length-1)+f,old:[],item:this});this.fireDataEvent(g,{start:this.length-1,end:this.length-1,type:b,items:[arguments[i]],added:[arguments[i]],removed:[]},null);}
;return this.length;}
,reverse:function(){if(this.length==0){return;}
;var x=this.__mS.concat();this.__mS.reverse();this.__mU(0,this.length);this.fireDataEvent(g,{start:0,end:this.length-1,type:c,items:null,added:[],removed:[]},null);this.fireDataEvent(o,{value:this.__mS,name:k+(this.__mS.length-1),old:x,item:this});}
,shift:function(){if(this.length==0){return;}
;var y=this.__mS.shift();this.__mT();this._registerEventChaining(null,y,this.length-1);this.__mU(0,this.length);this.fireDataEvent(o,{value:[],name:m,old:[y],item:this});this.fireDataEvent(g,{start:0,end:this.length-1,type:l,items:[y],removed:[y],added:[]},null);return y;}
,slice:function(A,z){return new qx.data.Array(this.__mS.slice(A,z));}
,splice:function(G,I,K){var N=this.__mS.length;var J=this.__mS.splice.apply(this.__mS,arguments);if(this.__mS.length!=N){this.__mT();}
...
;this.__mU(G+(arguments.length-2)-I,this.length);var H=[];for(var i=2;i<arguments.length;i++ ){H[i-2]=arguments[i];}
...
;var O=this.__mS.concat();this.__mS.sort.apply(this.__mS,arguments);if(qx.lang.Array.equals(this.__mS,O)===true){return;}
;this.__mU(0,this.length);this.fireDataEvent(g,{start:0,end:this.length-1,type:c,items:null,added:[],removed:[]},null);this.fireDataEvent(o,{value:this.__mS,name:k+(this.length-1),old:O,item:this});}
,unshift:function(Q){for(var i=arguments.length-1;i>=0;i-- ){this.__mS.unshift(arguments[i]);this.__mT();this.__mU(0,this.length);this.fireDataEvent(o,{value:[this.__mS[0]],name:m,old:[this.__mS[1]],item:this});this.fireDataEvent(g,{start:0,end:this.length-1,type:b,items:[arguments[i]],added:[arguments[i]],removed:[]},null);}
;return this.length;}
,toArray:function(){return this.__mS;}
,getItem:function(R){return this.__mS[R];}
,setItem:function(S,U){var T=this.__mS[S];if(T===U){return;}
;this.__mS[S]=U;this._registerEventChaining(U,T,S);if(this.length!=this.__mS.length){this.__mT();}
...
,indexOf:function(V){return this.__mS.indexOf(V);}
,lastIndexOf:function(W){return this.__mS.lastIndexOf(W);}
,toString:function(){if(this.__mS!=null){return this.__mS.toString();}
;return f;}
,contains:function(X){return this.__mS.indexOf(X)!==-1;}
...
;var bl=this.getLength();var bk=this.__mS.concat();this.__mS.length=0;this.__mT();this.fireDataEvent(o,{value:[],name:k+(bl-1),old:bk,item:this});this.fireDataEvent(g,{start:0,end:bl-1,type:l,items:bk,removed:bk,added:[]},null);return bk;}
,append:function(bm){if(bm instanceof qx.data.Array){bm=bm.toArray();}
;{}
;Array.prototype.push.apply(this.__mS,bm);for(var i=0;i<bm.length;i++ ){this._registerEventChaining(bm[i],null,this.__mS.length+i);}
;var bn=this.length;this.__mT();var name=bn==(this.length-1)?bn:bn+a+(this.length-1);this.fireDataEvent(o,{value:bm,name:name+f,old:[],item:this});this.fireDataEvent(g,{start:bn,end:this.length-1,type:b,items:bm,added:bm,removed:[]},null);}
...
,forEach:function(bu,bv){for(var i=0;i<this.__mS.length;i++ ){bu.call(bv,this.__mS[i],i,this);}
;}
,filter:function(bw,self){return new qx.data.Array(this.__mS.filter(bw,self));}
,map:function(bx,self){return new qx.data.Array(this.__mS.map(bx,self));}
,some:function(by,self){return this.__mS.some(by,self);}
,every:function(bz,self){return this.__mS.every(bz,self);}
,reduce:function(bB,bA){return this.__mS.reduce(bB,bA);}
,reduceRight:function(bD,bC){return this.__mS.reduceRight(bD,bC);}
,__mT:function(){var bE=this.length;this.length=this.__mS.length;this.fireDataEvent(p,this.length,bE);}
,__mU:function(bG,bF){for(var i=bG;i<bF;i++ ){this._registerEventChaining(this.__mS[i],this.__mS[i],i);}
;}
},destruct:function(){for(var i=0;i<this.__mS.length;i++ ){var bH=this.__mS[i];this._applyEventPropagation(null,bH,i);if(this.isAutoDisposeItems()&&bH&&bH instanceof qx.core.Object){bH.dispose();}
;}
;this.__mS=null;}
});}
)();
