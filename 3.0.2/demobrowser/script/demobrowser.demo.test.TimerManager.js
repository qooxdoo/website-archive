...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.TimerManager.0a071dbe7b67.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="Stopping recurrent 2-second timer",b="Recurrent 2-second timer with limit 3:",c="__CX",d="\n",e="One-shot 1-second timer: ",f=" ",g="demobrowser.demo.test.TimerManager",h="Recurrent 5-second timer: ",i="Start timers",j="execute";qx.Class.define(g,{extend:qx.application.Standalone,members:{__CX:null,startTimers:function(){var k=qx.util.TimerManager.getInstance();k.start(function(l,m){this.__Sn(h+m);}
,5000,this,null,0);k.start(function(n,o){this.__Sn(e+o);}
,0,this,null,1000);k.start(function(p,q){this.__Sn(b+q);if( ++p.count==3){this.__Sn(a);k.stop(q);}
;}
,2000,this,{count:0},2000);}
,__Sn:function(r){r=Date.parse(new Date)+f+r+d;this.__CX.setValue(this.__CX.getValue()+r);}
...
,this);var v=new qx.ui.form.TextArea();v.set({width:400,height:300});this.__CX=v;t.add(v);t.add(u);this.getRoot().add(t,{left:20,top:10});}
},destruct:function(){this._disposeObjects(c);}
});}
)();
(function(){var a="qx.util.TimerManager",b="interval",c="singleton";qx.Class.define(a,{extend:qx.core.Object,type:c,statics:{__HT:[],__HU:{},__ui:0},members:{__HV:false,start:function(g,f,h,d,j){if(!j){j=f||0;}
;var e=(new Date()).getTime()+j;this.self(arguments).__HU[ ++this.self(arguments).__ui]={callback:g,userData:d||null,expireAt:e,recurTime:f,context:h||this};this.__HW(e,this.self(arguments).__ui);return this.self(arguments).__ui;}
,stop:function(k){var l=this.self(arguments).__HT;var length=l.length;for(var i=0;i<length;i++ ){if(l[i]==k){l.splice(i,1);break;}
;}
;delete this.self(arguments).__HU[k];if(l.length==0&&this.__HV){qx.event.Idle.getInstance().removeListener(b,this.__HX,this);this.__HV=false;}
;}
,__HW:function(m,p){var o=this.self(arguments).__HT;var n=this.self(arguments).__HU;var length=o.length;for(var i=0;i<length;i++ ){if(n[o[i]].expireAt>m){o.splice(i,0,p);break;}
;}
;if(o.length==length){o.push(p);}
;if(!this.__HV){qx.event.Idle.getInstance().addListener(b,this.__HX,this);this.__HV=true;}
;}
,__HX:function(){var r=(new Date()).getTime();var t=this.self(arguments).__HT;var s=this.self(arguments).__HU;while(t.length>0&&s[t[0]].expireAt<=r){var v=t.shift();var q=s[v];q.callback.call(q.context,q.userData,v);if(q.recurTime&&s[v]){var u=(new Date()).getTime();q.expireAt=u+q.recurTime;this.__HW(q.expireAt,v);}
else {delete s[v];}
;}
;if(t.length==0&&this.__HV){qx.event.Idle.getInstance().removeListener(b,this.__HX,this);this.__HV=false;}
;}
}});}
)();
