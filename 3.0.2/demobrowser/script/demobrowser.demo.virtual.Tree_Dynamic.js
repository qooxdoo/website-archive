...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Dynamic.d2739df8ef8d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="open",b="",c="Root",d="icon",e="label",f="demobrowser.demo.virtual.Tree_Dynamic",g="demobrowser/demo/icons/loading22.gif",h="default",j="Loading",k="loading",l="children",m="Item ",n="icon/22/mimetypes/office-document.png",o="loaded",p="icon/22/places/folder.png";qx.Class.define(f,{extend:qx.application.Standalone,members:{count:0,main:function(){qx.application.Standalone.prototype.main.call(this);var q={label:c,children:[],icon:h,loaded:true};q=qx.data.marshal.Json.createModel(q,true);this.createRandomData(q);var r=new qx.ui.tree.VirtualTree(q,e,l);this.getRoot().add(r,{edge:20});r.setIconPath(d);r.setIconOptions({converter:function(v,u){if(v==h){if(u.getChildren!=null){return p;}
else {return n;}
;}
else {return g;}
;}
});var t=this;var s={bindItem:function(x,w,y){x.bindDefaultProperties(w,y);x.bindProperty(b,a,{converter:function(z,A,D,B){var C=B.isOpen();if(C&&!z.getLoaded()){z.setLoaded(true);qx.event.Timer.once(function(){z.getChildren().removeAll();this.createRandomData(z);}
,t,1000);}
;return C;}
},w,y);}
};r.setDelegate(s);}
,createRandomData:function(parent){var F=parseInt(Math.random()*50);for(var i=0;i<F;i++ ){var E={label:m+this.count++ ,icon:h,loaded:true};if(Math.random()>0.3){E[o]=false;E[l]=[{label:j,icon:k}];}
;parent.getChildren().push(qx.data.marshal.Json.createModel(E,true));}
;}
}});}
)();
...
,_getDataFromRow:function(bm){return this.__zr.getItem(bm);}
,_getSelectables:function(){return this.__zr;}
,getOpenNodes:function(){return this.__Fx;}
,isNode:function(bn){return qx.Class.hasProperty(bn.constructor,this.getChildProperty());}
,getLevel:function(bo){return this.__Fy[bo];}
,hasChildren:function(bp){var bq=bp.get(this.getChildProperty());if(bq==null){return false;}
;if(this.isShowLeafs()){return bq.length>0;}
else {for(var i=0;i<bq.getLength();i++ ){var br=bq.getItem(i);if(this.isNode(br)){return true;}
;}
;}
;return false;}
,_getContentPaddingTarget:function(){return this.getPane();}
,_applyRowHeight:function(bt,bs){this.getPane().getRowConfig().setDefaultItemSize(bt);}
,_applyOpenMode:function(bv,bu){var bw=this.getPane();if(bv===t){bw.addListener(J,this._onOpen,this);}
else if(bv===M){bw.addListener(O,this._onOpen,this);}
;if(bu===t){bw.removeListener(J,this._onOpen,this);}
else if(bu===M){bw.removeListener(O,this._onOpen,this);}
;}
,_applyHideRoot:function(by,bx){this.buildLookupTable();}
,_applyShowTopLevelOpenCloseIcons:function(bA,bz){this.buildLookupTable();}
,_applyShowLeafs:function(bC,bB){this.buildLookupTable();}
,_applyChildProperty:function(bE,bD){this._provider.setChildProperty(bE);}
,_applyLabelPath:function(bG,bF){this._provider.setLabelPath(bG);}
,_applyIconPath:function(bI,bH){this._provider.setIconPath(bI);}
,_applyLabelOptions:function(bK,bJ){this._provider.setLabelOptions(bK);}
,_applyIconOptions:function(bM,bL){this._provider.setIconOptions(bM);}
,_applyModel:function(bO,bN){this.__Fx=[];if(bO!=null){if(qx.core.Environment.get(w)){if(!qx.Class.hasMixin(bO.constructor,qx.data.marshal.MEventBubbling)){this.warn(F+h+k);}
;}
;bO.addListener(V,this._onChangeBubble,this);this.__FD(bO);}
;if(bN!=null){bN.removeListener(V,this._onChangeBubble,this);}
;this.__FB();}
,_applyDelegate:function(bQ,bP){this._provider.setDelegate(bQ);this.buildLookupTable();}
,_onChangeBubble:function(event){var bV=event.getData();var bS=bV.name;var bR=bS.lastIndexOf(W);if(bR!=-1){bS=bS.substr(bR+1,bS.length);}
;if(qx.lang.String.startsWith(bS,this.getChildProperty())){var bT=bV.item;if(qx.Class.isSubClassOf(bT.constructor,qx.data.Array)){if(bR===-1){bT=this.getModel();}
else {var bU=bV.name.substr(0,bR);bT=qx.data.SingleValueBinding.resolvePropertyChain(this.getModel(),bU);}
;}
;if(this.__zr.indexOf(bT)!=-1){this.__FB();}
;}
;}
,_onUpdated:function(event){if(this.__fB==null){this.__fB=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__fB.schedule();}
,_onOpen:function(event){var bX=event.getRow();var bW=this.__zr.getItem(bX);if(this.isNode(bW)){if(this.isNodeOpen(bW)){this.closeNode(bW);}
else {this.openNode(bW);}
;}
;}
,_onKeyPress:function(e){var bY=this.getSelection();if(bY.getLength()>0){var cb=bY.getItem(0);var ca=this.isNode(cb);switch(e.getKeyIdentifier()){case n:if(ca&&this.isNodeOpen(cb)){this.closeNode(cb);}
else {var parent=this.getParent(cb);if(parent!=null){bY.splice(0,1,parent);}
;}
;break;case I:if(ca&&!this.isNodeOpen(cb)){this.openNode(cb);}
else {if(ca){var cc=cb.get(this.getChildProperty());if(cc!=null&&cc.getLength()>0){bY.splice(0,1,cc.getItem(0));}
;}
;}
;break;case K:case m:if(!ca){return;}
;if(this.isNodeOpen(cb)){this.closeNode(cb);}
else {this.openNode(cb);}
;break;};}
;}
,_beforeApplySelection:function(ce){if(ce.length===0&&this.getSelectionMode()===g){var cd=this.__FG();var cf=this.getLookupTable().indexOf(cd);if(cf>=0){ce.push(cf);}
;}
;}
,_afterApplySelection:function(){var cg=this.getSelection();if(cg.getLength()>0&&this.getSelectionMode()===g){this.__FF(cg.getItem(0));}
else {this.__FA=[];}
;}
,__FB:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(Q+f);}
;this.__Fz=0;var ck=[];this.__Fy=[];var ci=-1;var cj=this.getModel();if(cj!=null){if(!this.isHideRoot()){ci++ ;ck.push(cj);this.__Fy.push(ci);}
;if(this.isNodeOpen(cj)){var ch=this.__FC(cj,ci);ck=ck.concat(ch);}
;}
;this._provider.removeBindings();this.__zr.removeAll();this.__zr.append(ck);this.__zx();this._updateSelection();}
,__FC:function(cl,cq){var cp=[];cq++ ;if(!this.isNode(cl)){return cp;}
;var ct=cl.get(this.getChildProperty());if(ct==null){return cp;}
;ct=ct.copy();var cr=this.getDelegate();var co=qx.util.Delegate.getMethod(cr,o);var cn=qx.util.Delegate.getMethod(cr,H);if(cn!=null){ct.sort(cn);}
;for(var i=0;i<ct.getLength();i++ ){var cs=ct.getItem(i);if(co&&!co(cs)){continue;}
;if(this.isNode(cs)){this.__Fy.push(cq);cp.push(cs);if(this.isNodeOpen(cs)){var cm=this.__FC(cs,cq);cp=cp.concat(cm);}
;}
else {if(this.isShowLeafs()){this.__Fy.push(cq);cp.push(cs);}
;}
;}
;ct.dispose();return cp;}
,__FD:function(cu){if(!qx.lang.Array.contains(this.__Fx,cu)){this.__Fx.push(cu);this.fireDataEvent(u,cu);}
;}
,__FE:function(cv,cw){if(cv===cw){this.__FD(cw);return true;}
;if(!this.isNode(cv)){return false;}
;var cz=cv.get(this.getChildProperty());if(cz==null){return false;}
;for(var i=0;i<cz.getLength();i++ ){var cy=cz.getItem(i);var cx=this.__FE(cy,cw);if(cx===true){this.__FD(cy);return true;}
;}
;return false;}
,__zx:function(){this.getPane().getRowConfig().setItemCount(this.__zr.getLength());this.getPane().fullUpdate();}
,getParent:function(cB){var cC=this.__zr.indexOf(cB);if(cC<0){return null;}
;var cD=this.__Fy[cC];while(cC>0){cC-- ;var cA=this.__Fy[cC];if(cA<cD){return this.__zr.getItem(cC);}
;}
;return null;}
,__FF:function(cE){this.__FA=[];var parent=this.getParent(cE);while(parent!=null){this.__FA.unshift(parent);parent=this.getParent(parent);}
;}
,__FG:function(){if(this.__FA==null){return this.getModel();}
;var cF=this.getLookupTable();var parent=this.__FA.pop();while(parent!=null){if(cF.contains(parent)){return parent;}
;parent=this.__FA.pop();}
;return this.getModel();}
},destruct:function(){var cH=this.getPane();if(cH!=null){if(cH.hasListener(J)){cH.removeListener(J,this._onOpen,this);}
;if(cH.hasListener(O)){cH.removeListener(O,this._onOpen,this);}
;}
;if(!qx.core.ObjectRegistry.inShutDown&&this.__fB!=null){this.__fB.cancel();this.__fB.dispose();}
;var cG=this.getModel();if(cG!=null){cG.removeListener(V,this._onChangeBubble,this);}
;this._layer.removeListener(R,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__zr.dispose();this._layer=this._provider=this.__zr=this.__Fx=this.__fB=null;}
});}
)();
...
(function(){var a="BindingIds",b="model",c="",d="changeDelegate",f="virtual-tree-file",g="label",h="appearance",i="icon",j="bindItem",k=".length",l="]",m="virtual-tree-folder",n="[",o=".",p="String",q="qx.ui.tree.core.MWidgetController";qx.Mixin.define(q,{construct:function(){this.__zD=[];}
...
;}
,bindProperty:function(x,A,y,w,u){var z=this.__zE(u,x);var v=this._tree.getLookupTable();var B=v.bind(z,w,A,y);this.__zF(w,B);}
,bindPropertyReverse:function(I,E,G,H,C){var F=this.__zE(C,I);var D=this._tree.getLookupTable();var J=H.bind(E,D,F,G);this.__zF(H,J);}
,removeBindings:function(){while(this.__zD.length>0){var K=this.__zD.pop();this._removeBindingsFrom(K);}
;}
...
;}
,_removeBindingsFrom:function(O){var P=this.__zG(O);while(P.length>0){var Q=P.pop();try{this._tree.getLookupTable().removeBinding(Q);}
catch(e){O.removeBinding(Q);}
;}
;if(qx.lang.Array.contains(this.__zD,O)){qx.lang.Array.remove(this.__zD,O);}
;}
,__zE:function(S,T){var R=n+S+l;if(T!=null&&T!=c){R+=o+T;}
;return R;}
,__zF:function(V,W){var U=this.__zG(V);if(!qx.lang.Array.contains(U,W)){U.push(W);}
;if(!qx.lang.Array.contains(this.__zD,V)){this.__zD.push(V);}
;}
,__zG:function(X){var Y=X.getUserData(a);if(Y==null){Y=[];X.setUserData(a,Y);}
;return Y;}
...
(function(){var a="changeOpen",b="never",c="changeDelegate",d="cell.level",e="createItem",f="auto",g="cell.children",h="configureItem",i="created",j="onPool",k="cell.row",l="qx.ui.tree.provider.WidgetProvider";qx.Class.define(l,{extend:qx.core.Object,implement:[qx.ui.virtual.core.IWidgetCellProvider,qx.ui.tree.provider.IVirtualTreeProvider],include:[qx.ui.tree.core.MWidgetController],construct:function(m){qx.core.Object.call(this);this._tree=m;this.addListener(c,this._onChangeDelegate,this);this._onChangeDelegate();}
,members:{_tree:null,_renderer:null,getCellWidget:function(s,q){var r=this._tree.getLookupTable().getItem(s);var o=false;if(this._tree.isNode(r)){o=this._tree.hasChildren(r);}
;var n=this._renderer.getCellWidget();n.setOpen(o&&this._tree.isNodeOpen(r));n.setUserData(g,o);n.addListener(a,this.__FH,this);if(this._tree.getSelection().contains(r)){this._styleSelectabled(n);}
else {this._styleUnselectabled(n);}
;var p=this._tree.getLevel(s);if(!this._tree.isShowTopLevelOpenCloseIcons()){p-=1;}
;n.setUserData(d,p);if(!this._tree.isShowTopLevelOpenCloseIcons()&&p==-1){n.setOpenSymbolMode(b);}
else {n.setOpenSymbolMode(f);}
;this._bindItem(n,s);qx.ui.core.queue.Widget.add(n);return n;}
,poolCellWidget:function(t){t.removeListener(a,this.__FH,this);this._removeBindingsFrom(t);this._renderer.pool(t);this._onPool(t);}
,createLayer:function(){return new qx.ui.virtual.layer.WidgetCell(this);}
,createRenderer:function(){var u=qx.util.Delegate.getMethod(this.getDelegate(),e);if(u==null){u=function(){return new qx.ui.tree.VirtualTreeItem();}
;}
;var v=new qx.ui.virtual.cell.WidgetCell();v.setDelegate({createWidget:u});return v;}
,styleSelectabled:function(x){var w=this._tree._layer.getRenderedCellWidget(x,0);this._styleSelectabled(w);}
,styleUnselectabled:function(z){var y=this._tree._layer.getRenderedCellWidget(z,0);this._styleUnselectabled(y);}
,isSelectable:function(B){var A=this._tree._layer.getRenderedCellWidget(B,0);if(A!=null){return A.isEnabled();}
else {return true;}
;}
...
,_onPool:function(E){var F=qx.util.Delegate.getMethod(this.getDelegate(),j);if(F!=null){F(E);}
;}
,_onItemCreated:function(event){var G=qx.util.Delegate.getMethod(this.getDelegate(),h);if(G!=null){var H=event.getData();G(H);}
;}
...
,__FH:function(event){var J=event.getTarget();var K=J.getUserData(k);var I=this._tree.getLookupTable().getItem(K);if(event.getData()){this._tree.openNode(I);}
...
(function(){var a="cell.level",b="qx.ui.tree.VirtualTreeItem",c="open",d="virtual-tree-folder",e="cell.children";qx.Class.define(b,{extend:qx.ui.tree.core.AbstractItem,properties:{appearance:{refine:true,init:d}},members:{_forwardStates:{selected:true},_addWidgets:function(){this.addSpacer();this.addOpenButton();this.addIcon();this.addLabel();}
,_shouldShowOpenSymbol:function(){var open=this.getChildControl(c,true);if(open==null){return false;}
;return this.isOpenable();}
,getLevel:function(){return this.getUserData(a);}
,hasChildren:function(){return !!this.getUserData(e);}
}});}
)();
