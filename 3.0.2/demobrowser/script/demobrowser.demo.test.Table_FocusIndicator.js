...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Table_FocusIndicator","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Table_FocusIndicator.7cc77560897e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="keypress",b="#FFE",c="demobrowser.demo.test.Table_FocusIndicator",d="Left",f="Right",g="Down",h="main",i="Up";qx.Class.define(c,{extend:qx.application.Standalone,include:[demobrowser.demo.table.MUtil,qx.core.MAssert],members:{main:function(){qx.application.Standalone.prototype.main.call(this);var j=new qx.ui.container.Composite(new qx.ui.layout.Canvas()).set({decorator:h,backgroundColor:b,width:600,height:400});this.getRoot().add(j,{left:10,top:10});var m=new qx.ui.table.pane.FocusIndicator(this.getScrollerMock()).set({decorator:h});j.add(m);var k=6;var l=2;m.moveToCell(l,k);j.addListener(a,function(e){switch(e.getKeyIdentifier()){case d:l-=1;break;case f:l+=1;break;case i:k-=1;break;case g:k+=1;break;default:return;};l=Math.min(Math.max(0,l),5);k=Math.min(Math.max(0,k),19);m.moveToCell(l,k);}
);}
}});}
)();
(function(){var a="Escape",b="keypress",c="Enter",d="excluded",f="Integer",g="qx.ui.table.pane.FocusIndicator";qx.Class.define(g,{extend:qx.ui.container.Composite,construct:function(h){qx.ui.container.Composite.call(this);this.__qo=h;this.setKeepActive(true);this.addListener(b,this._onKeyPress,this);}
,properties:{visibility:{refine:true,init:d},row:{check:f,nullable:true},column:{check:f,nullable:true}},members:{__qo:null,_onKeyPress:function(e){var i=e.getKeyIdentifier();if(i!==a&&i!==c){e.stopPropagation();}
;}
,moveToCell:function(p,q){if(!this.__qo.getShowCellFocusIndicator()&&!this.__qo.getTable().getTableModel().isColumnEditable(p)){this.exclude();return;}
else {this.show();}
;if(p==null){this.hide();this.setRow(null);this.setColumn(null);}
else {var j=this.__qo.getTablePaneModel().getX(p);if(j==-1){this.hide();this.setRow(null);this.setColumn(null);}
else {var o=this.__qo.getTable();var m=o.getTableColumnModel();var n=this.__qo.getTablePaneModel();var l=this.__qo.getTablePane().getFirstVisibleRow();var k=o.getRowHeight();this.setUserBounds(n.getColumnLeft(p)-2,(q-l)*k-2,m.getColumnWidth(p)+3,k+3);this.show();this.setRow(q);this.setColumn(p);}
;}
;}
