...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Flickr.4a94adc3a2e9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="url: ",b="qx.debug.io",c="qx.bom.request.Script",d="Invalid state",e="head",f="error",g="loadend",h="qx.debug",i="script",j="load",k="Unknown response headers",l="browser.documentmode",m="abort",n="",o="Received native readyState: loaded",p="readystatechange",q="Response header cannot be determined for ",r="requests made with script transport.",s="opera",t="unknown",u="Open native request with ",v="Response headers cannot be determined for",w="mshtml",x="engine.name",y="Detected error",z="Send native request",A="on",B="timeout",C="Unknown environment key at this phase",D="Received native load";qx.Bootstrap.define(c,{construct:function(){this.__Bc();this.__AW=qx.Bootstrap.bind(this._onNativeLoad,this);this.__AX=qx.Bootstrap.bind(this._onNativeError,this);this.__tO=qx.Bootstrap.bind(this._onTimeout,this);this.__AY=document.head||document.getElementsByTagName(e)[0]||document.documentElement;this._emitter=new qx.event.Emitter();this.timeout=this.__Be()?0:15000;}
,events:{"readystatechange":c,"error":c,"loadend":c,"timeout":c,"abort":c,"load":c},members:{readyState:null,status:null,statusText:null,timeout:null,__Ba:null,on:function(name,E,F){this._emitter.on(name,E,F);return this;}
,open:function(H,G){if(this.__uj){return;}
;this.__Bc();this.__tR=null;this.__ue=G;if(this.__Bh(b)){qx.Bootstrap.debug(qx.bom.request.Script,u+a+G);}
;this._readyStateChange(1);}
,setRequestHeader:function(I,J){if(this.__uj){return null;}
;var K={};if(this.readyState!==1){throw new Error(d);}
;K[I]=J;this.__ue=qx.util.Uri.appendParamsToUrl(this.__ue,K);return this;}
,send:function(){if(this.__uj){return null;}
;var M=this.__Bf(),L=this.__AY,N=this;if(this.timeout>0){this.__eQ=window.setTimeout(this.__tO,this.timeout);}
;if(this.__Bh(b)){qx.Bootstrap.debug(qx.bom.request.Script,z);}
;L.insertBefore(M,L.firstChild);window.setTimeout(function(){N._readyStateChange(2);N._readyStateChange(3);}
);return this;}
,abort:function(){if(this.__uj){return null;}
;this.__tR=true;this.__Bg();this._emit(m);return this;}
...
,onabort:function(){}
,ontimeout:function(){}
,getResponseHeader:function(O){if(this.__uj){return null;}
;if(this.__Bh(h)){qx.Bootstrap.debug(q+r);}
;return t;}
,getAllResponseHeaders:function(){if(this.__uj){return null;}
;if(this.__Bh(h)){qx.Bootstrap.debug(v+r);}
;return k;}
,setDetermineSuccess:function(P){this.__Ba=P;}
,dispose:function(){var Q=this.__Bb;if(!this.__uj){if(Q){Q.onload=Q.onreadystatechange=null;this.__Bg();}
;if(this.__eQ){window.clearTimeout(this.__eQ);}
;this.__uj=true;}
;}
,isDisposed:function(){return !!this.__uj;}
,_getUrl:function(){return this.__ue;}
,_getScriptElement:function(){return this.__Bb;}
,_onTimeout:function(){this.__Bd();if(!this.__Be()){this._emit(f);}
;this._emit(B);if(!this.__Be()){this._emit(g);}
;}
,_onNativeLoad:function(){var S=this.__Bb,R=this.__Ba,T=this;if(this.__tR){return;}
;if(this.__Bh(x)===w&&this.__Bh(l)<9){if(!(/loaded|complete/).test(S.readyState)){return;}
else {if(this.__Bh(b)){qx.Bootstrap.debug(qx.bom.request.Script,o);}
;}
;}
;if(this.__Bh(b)){qx.Bootstrap.debug(qx.bom.request.Script,D);}
;if(R){if(!this.status){this.status=R()?200:500;}
;}
;if(this.status===500){if(this.__Bh(b)){qx.Bootstrap.debug(qx.bom.request.Script,y);}
;}
;if(this.__eQ){window.clearTimeout(this.__eQ);}
;window.setTimeout(function(){T._success();T._readyStateChange(4);T._emit(j);T._emit(g);}
);}
,_onNativeError:function(){this.__Bd();this._emit(f);this._emit(g);}
,__Bb:null,__AY:null,__ue:n,__AW:null,__AX:null,__tO:null,__eQ:null,__tR:null,__uj:null,__Bc:function(){this.readyState=0;this.status=0;this.statusText=n;}
,_readyStateChange:function(U){this.readyState=U;this._emit(p);}
,_success:function(){this.__Bg();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=n+this.status;}
,__Bd:function(){this.__Bg();this.readyState=4;this.status=0;this.statusText=null;}
,__Be:function(){var W=this.__Bh(x)===w&&this.__Bh(l)<9;var V=this.__Bh(x)===s;return !(W||V);}
,__Bf:function(){var X=this.__Bb=document.createElement(i);X.src=this.__ue;X.onerror=this.__AX;X.onload=this.__AW;if(this.__Bh(x)===w&&this.__Bh(l)<9){X.onreadystatechange=this.__AW;}
;return X;}
,__Bg:function(){var Y=this.__Bb;if(Y&&Y.parentNode){this.__AY.removeChild(Y);}
;}
,__Bh:function(ba){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(ba);}
...
(function(){var a="qx.bom.request.Jsonp",b="callback",c="open",d="dispose",e="",f="qx",g="qx.debug",h="Expecting JavaScript response to call: ",i="_onNativeLoad",j=" already exists",k="Callback ",l="qx.debug.io",m=".callback",n="qx.bom.request.Jsonp.";qx.Bootstrap.define(a,{extend:qx.bom.request.Script,construct:function(){qx.bom.request.Script.apply(this);this.__uD();}
,members:{responseJson:null,__co:null,__uw:null,__ux:null,__uy:null,__uz:null,__uA:null,__uj:null,__uB:e,open:function(t,p){if(this.__uj){return;}
;var r={},q,s,o=this;this.responseJson=null;this.__uy=false;q=this.__uw||b;s=this.__ux||this.__uB+n+this.__co+m;if(!this.__ux){this.constructor[this.__co]=this;}
else {if(!window[this.__ux]){this.__uz=true;window[this.__ux]=function(u){o.callback(u);}
;}
...
;r[q]=s;this.__uA=p=qx.util.Uri.appendParamsToUrl(p,r);this.__gx(c,[t,p]);}
,callback:function(v){if(this.__uj){return;}
;this.__uy=true;if(qx.core.Environment.get(g)){v=qx.lang.Json.stringify(v);v=qx.lang.Json.parse(v);}
;this.responseJson=v;this.constructor[this.__co]=undefined;this.__uC();}
,setCallbackParam:function(w){this.__uw=w;return this;}
,setCallbackName:function(name){this.__ux=name;return this;}
,setPrefix:function(x){this.__uB=x;}
,getGeneratedUrl:function(){return this.__uA;}
,dispose:function(){this.__uC();this.__gx(d);}
,_onNativeLoad:function(){this.status=this.__uy?200:500;this.__gx(i);}
,__uC:function(){if(this.__uz&&window[this.__ux]){window[this.__ux]=undefined;this.__uz=false;}
;}
,__gx:function(z,y){qx.bom.request.Script.prototype[z].apply(this,y||[]);}
,__uD:function(){this.__co=f+(new Date().valueOf())+(e+Math.random()).substring(2,5);}
}});}
)();
...
}});qx.Class.define(n,{extend:qx.data.store.Jsonp,construct:function(R){this.setCallbackName(z);qx.data.store.Jsonp.call(this,this.__Ry(R));}
,members:{searchForTag:function(S){if(S!=t){this.setUrl(this.__Ry(S));}
;}
,__Ry:function(U){if(!U){return;}
