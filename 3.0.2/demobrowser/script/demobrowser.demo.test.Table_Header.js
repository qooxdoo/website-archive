...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Table_Header","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Table_Header.8c1302f5f819.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="qx.ui.table.selection.Model",b="qx.ui.table.selection.Manager";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);}
,properties:{selectionModel:{check:a}},members:{__oK:null,handleMouseDown:function(c,d){if(d.isLeftPressed()){var e=this.getSelectionModel();if(!e.isSelectedIndex(c)){this._handleSelectEvent(c,d);this.__oK=true;}
else {this.__oK=false;}
;}
else if(d.isRightPressed()&&d.getModifiers()==0){var e=this.getSelectionModel();if(!e.isSelectedIndex(c)){e.setSelectionInterval(c,c);}
;}
;}
,handleMouseUp:function(f,g){if(g.isLeftPressed()&&!this.__oK){this._handleSelectEvent(f,g);}
;}
...
;break;};}
,_handleSelectEvent:function(p,r){var t=this.getSelectionModel();var q=t.getLeadSelectionIndex();var s=t.getAnchorSelectionIndex();if(r.isShiftPressed()){if(p!=q||t.isSelectionEmpty()){if(s==-1){s=p;}
;if(r.isCtrlOrCommandPressed()){t.addSelectionInterval(s,p);}
else {t.setSelectionInterval(s,p);}
;}
;}
else if(r.isCtrlOrCommandPressed()){if(t.isSelectedIndex(p)){t.removeSelectionInterval(p,p);}
...
(function(){var a="demobrowser.demo.table.MUtil",b="500px",c="px",d="Column #",e="Cell ",f="#FFE",g="div",h="x",j="absolute";qx.Mixin.define(a,{construct:function(){this._tableLeft=10;}
...
,getNewTablePaneHeader:function(r){return function(t){var s=new qx.ui.table.pane.Header(t);return s;}
;}
...
,getRowData:function(L){var K=[];for(var i=0;i<4;i++ ){K.push(e+i+h+L);}
;return K;}
,getValue:function(M,N){return e+M+h+N;}
};}
...
,_getNewTableDiv:function(R){var S=qx.dom.Element.create(g);qx.bom.element.Style.setStyles(S,{position:j,left:this._tableLeft+c,width:(R||150)+c,top:20+c,height:b,backgroundColor:f});this._tableLeft+=(R||150)+20;document.body.appendChild(S);return S;}
}});}
)();
(function(){var a="..",b="changeSelection",c="Try to turn off batch mode althoug it was not turned on.",d="_applySelectionMode",f="]",g="qx.event.type.Event",h="Ranges:",k="qx.ui.table.selection.Model",l=" [";qx.Class.define(k,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__oL=[];this.__oM=-1;this.__oN=-1;this.hasBatchModeRefCount=0;this.__oO=false;}
,events:{"changeSelection":g},statics:{NO_SELECTION:1,SINGLE_SELECTION:2,SINGLE_INTERVAL_SELECTION:3,MULTIPLE_INTERVAL_SELECTION:4,MULTIPLE_INTERVAL_SELECTION_TOGGLE:5},properties:{selectionMode:{init:2,check:[1,2,3,4,5],apply:d}},members:{__oO:null,__oM:null,__oN:null,__oL:null,_applySelectionMode:function(m){this.resetSelection();}
,setBatchMode:function(n){if(n){this.hasBatchModeRefCount+=1;}
else {if(this.hasBatchModeRefCount==0){throw new Error(c);}
;this.hasBatchModeRefCount-=1;if(this.__oO){this.__oO=false;this._fireChangeSelection();}
;}
;return this.hasBatchMode();}
,hasBatchMode:function(){return this.hasBatchModeRefCount>0;}
,getAnchorSelectionIndex:function(){return this.__oM;}
,_setAnchorSelectionIndex:function(o){this.__oM=o;}
,getLeadSelectionIndex:function(){return this.__oN;}
,_setLeadSelectionIndex:function(p){this.__oN=p;}
,_getSelectedRangeArr:function(){return this.__oL;}
,resetSelection:function(){if(!this.isSelectionEmpty()){this._resetSelection();this._fireChangeSelection();}
;}
,isSelectionEmpty:function(){return this.__oL.length==0;}
,getSelectedCount:function(){var r=0;for(var i=0;i<this.__oL.length;i++ ){var q=this.__oL[i];r+=q.maxIndex-q.minIndex+1;}
;return r;}
,isSelectedIndex:function(s){for(var i=0;i<this.__oL.length;i++ ){var t=this.__oL[i];if(s>=t.minIndex&&s<=t.maxIndex){return true;}
;}
;return false;}
,getSelectedRanges:function(){var u=[];for(var i=0;i<this.__oL.length;i++ ){u.push({minIndex:this.__oL[i].minIndex,maxIndex:this.__oL[i].maxIndex});}
;return u;}
,iterateSelection:function(w,v){for(var i=0;i<this.__oL.length;i++ ){for(var j=this.__oL[i].minIndex;j<=this.__oL[i].maxIndex;j++ ){w.call(v,j);}
;}
;}
,setSelectionInterval:function(z,y){var x=this.self(arguments);switch(this.getSelectionMode()){case x.NO_SELECTION:return;case x.SINGLE_SELECTION:if(this.isSelectedIndex(y)){return;}
;z=y;break;case x.MULTIPLE_INTERVAL_SELECTION_TOGGLE:this.setBatchMode(true);try{for(var i=z;i<=y;i++ ){if(!this.isSelectedIndex(i)){this._addSelectionInterval(i,i);}
...
,removeSelectionInterval:function(G,E){this.__oM=G;this.__oN=E;var D=Math.min(G,E);var H=Math.max(G,E);for(var i=0;i<this.__oL.length;i++ ){var J=this.__oL[i];if(J.minIndex>H){break;}
else if(J.maxIndex>=D){var K=(J.minIndex>=D)&&(J.minIndex<=H);var F=(J.maxIndex>=D)&&(J.maxIndex<=H);if(K&&F){this.__oL.splice(i,1);i-- ;}
else if(K){J.minIndex=H+1;}
else if(F){J.maxIndex=D-1;}
else {var I={minIndex:H+1,maxIndex:J.maxIndex};this.__oL.splice(i+1,0,I);J.maxIndex=D-1;break;}
;}
;}
;this._fireChangeSelection();}
,_resetSelection:function(){this.__oL=[];this.__oM=-1;this.__oN=-1;}
,_addSelectionInterval:function(O,M){this.__oM=O;this.__oN=M;var L=Math.min(O,M);var P=Math.max(O,M);var N=0;for(;N<this.__oL.length;N++ ){var Q=this.__oL[N];if(Q.minIndex>L){break;}
;}
;this.__oL.splice(N,0,{minIndex:L,maxIndex:P});var R=this.__oL[0];for(var i=1;i<this.__oL.length;i++ ){var Q=this.__oL[i];if(R.maxIndex+1>=Q.minIndex){R.maxIndex=Math.max(R.maxIndex,Q.maxIndex);this.__oL.splice(i,1);i-- ;}
else {R=Q;}
;}
;}
,_dumpRanges:function(){var S=h;for(var i=0;i<this.__oL.length;i++ ){var T=this.__oL[i];S+=l+T.minIndex+a+T.maxIndex+f;}
;this.debug(S);}
,_fireChangeSelection:function(){if(this.hasBatchMode()){this.__oO=true;}
else {this.fireEvent(b);}
;}
},destruct:function(){this.__oL=null;}
});}
)();
(function(){var a="qx.ui.table.IRowRenderer";qx.Interface.define(a,{members:{updateDataRowElement:function(c,b){}
,getRowHeightStyle:function(d){}
,createRowStyle:function(e){}
,getRowClass:function(f){}
}});}
)();
(function(){var a="table-row-background-even",b="Boolean",c="content",d="default",e="height:",f="changeTheme",g="'",h="table-row",i="table-row-background-focused",j="css.boxmodel",k=';color:',l="table-row-background-odd",m="1px solid ",n="table-row-line",o="qx.dyntheme",p="table-row-background-selected",q="background-color:",r="",s=';border-bottom: 1px solid ',t="table-row-selected",u="table-row-background-focused-selected",v="px;",w="qx.ui.table.rowrenderer.Default",x=";";qx.Class.define(w,{extend:qx.core.Object,implement:qx.ui.table.IRowRenderer,construct:function(){qx.core.Object.call(this);this.initThemeValues();if(qx.core.Environment.get(o)){qx.theme.manager.Color.getInstance().addListener(f,this.initThemeValues,this);qx.theme.manager.Font.getInstance().addListener(f,this.initThemeValues,this);}
;}
,properties:{highlightFocusRow:{check:b,init:true}},members:{_colors:null,__ok:null,__ol:null,initThemeValues:function(){this.__ol=r;this.__ok={};this._colors={};this._renderFont(qx.theme.manager.Font.getInstance().resolve(d));var y=qx.theme.manager.Color.getInstance();this._colors.bgcolFocusedSelected=y.resolve(u);this._colors.bgcolFocused=y.resolve(i);this._colors.bgcolSelected=y.resolve(p);this._colors.bgcolEven=y.resolve(a);this._colors.bgcolOdd=y.resolve(l);this._colors.colSelected=y.resolve(t);this._colors.colNormal=y.resolve(h);this._colors.horLine=y.resolve(n);}
,_insetY:1,_renderFont:function(z){if(z){this.__ok=z.getStyles();this.__ol=qx.bom.element.Style.compile(this.__ok);this.__ol=this.__ol.replace(/"/g,g);}
else {this.__ol=r;this.__ok=qx.bom.Font.getDefaultStyles();}
;}
,updateDataRowElement:function(D,A){var B=this.__ok;var C=A.style;qx.bom.element.Style.setStyles(A,B);if(D.focusedRow&&this.getHighlightFocusRow()){C.backgroundColor=D.selected?this._colors.bgcolFocusedSelected:this._colors.bgcolFocused;}
else {if(D.selected){C.backgroundColor=this._colors.bgcolSelected;}
else {C.backgroundColor=(D.row%2==0)?this._colors.bgcolEven:this._colors.bgcolOdd;}
;}
;C.color=D.selected?this._colors.colSelected:this._colors.colNormal;C.borderBottom=m+this._colors.horLine;}
,getRowHeightStyle:function(E){if(qx.core.Environment.get(j)==c){E-=this._insetY;}
;return e+E+v;}
,createRowStyle:function(G){var F=[];F.push(x);F.push(this.__ol);F.push(q);if(G.focusedRow&&this.getHighlightFocusRow()){F.push(G.selected?this._colors.bgcolFocusedSelected:this._colors.bgcolFocused);}
else {if(G.selected){F.push(this._colors.bgcolSelected);}
else {F.push((G.row%2==0)?this._colors.bgcolEven:this._colors.bgcolOdd);}
;}
;F.push(k);F.push(G.selected?this._colors.colSelected:this._colors.colNormal);F.push(s,this._colors.horLine);return F.join(r);}
,getRowClass:function(H){return r;}
,getRowAttributes:function(I){return r;}
},destruct:function(){this._colors=this.__ok=this.__ol=null;if(qx.core.Environment.get(o)){qx.theme.manager.Color.getInstance().removeListener(f,this.initThemeValues,this);qx.theme.manager.Font.getInstance().removeListener(f,this.initThemeValues,this);}
;}
});}
)();
...
(function(){var a="first",b="hovered",c="__pw",d="last",e="qx.ui.table.pane.Header";qx.Class.define(e,{extend:qx.ui.core.Widget,construct:function(f){qx.ui.core.Widget.call(this);this._setLayout(new qx.ui.layout.HBox());this.__la=new qx.ui.core.Blocker(this);this.__pw=f;}
,members:{__pw:null,__pE:null,__pF:null,__la:null,getPaneScroller:function(){return this.__pw;}
,getTable:function(){return this.__pw.getTable();}
,getBlocker:function(){return this.__la;}
,onColOrderChanged:function(){this._updateContent(true);}
,onPaneModelChanged:function(){this._updateContent(true);}
,onTableModelMetaDataChanged:function(){this._updateContent();}
,setColumnWidth:function(h,g,i){var j=this.getHeaderWidgetAtColumn(h);if(j!=null){j.setWidth(g);}
;}
,setMouseOverColumn:function(l){if(l!=this.__pF){if(this.__pF!=null){var k=this.getHeaderWidgetAtColumn(this.__pF);if(k!=null){k.removeState(b);}
;}
;if(l!=null){this.getHeaderWidgetAtColumn(l).addState(b);}
;this.__pF=l;}
;}
,getHeaderWidgetAtColumn:function(n){var m=this.getPaneScroller().getTablePaneModel().getX(n);return this._getChildren()[m];}
,showColumnMoveFeedback:function(y,x){var r=this.getContentLocation();if(this.__pE==null){var w=this.getTable();var o=this.getPaneScroller().getTablePaneModel().getX(y);var q=this._getChildren()[o];var s=w.getTableModel();var u=w.getTableColumnModel();var v={xPos:o,col:y,name:s.getColumnName(y),table:w};var t=u.getHeaderCellRenderer(y);var p=t.createHeaderCell(v);var z=q.getBounds();p.setWidth(z.width);p.setHeight(z.height);p.setZIndex(1000000);p.setOpacity(0.8);p.setLayoutProperties({top:r.top});this.getApplicationRoot().add(p);this.__pE=p;}
;this.__pE.setLayoutProperties({left:r.left+x});this.__pE.show();}
,hideColumnMoveFeedback:function(){if(this.__pE!=null){this.__pE.destroy();this.__pE=null;}
;}
,isShowingColumnMoveFeedback:function(){return this.__pE!=null;}
,_updateContent:function(B){var L=this.getTable();var E=L.getTableModel();var H=L.getTableColumnModel();var A=this.getPaneScroller().getTablePaneModel();var D=this._getChildren();var F=A.getColumnCount();var I=E.getSortColumnIndex();if(B){this._cleanUpCells();}
;var J={};J.sortedAscending=E.isSortAscending();for(var x=0;x<F;x++ ){var M=A.getColumnAtX(x);if(M===undefined){continue;}
;var K=H.getColumnWidth(M);var G=H.getHeaderCellRenderer(M);J.xPos=x;J.col=M;J.name=E.getColumnName(M);J.editable=E.isColumnEditable(M);J.sorted=(M==I);J.table=L;var C=D[x];if(C==null){C=G.createHeaderCell(J);C.set({width:K});this._add(C);}
else {G.updateHeaderCell(J,C);}
;if(x===0){C.addState(a);C.removeState(d);}
else if(x===F-1){C.removeState(a);C.addState(d);}
else {C.removeState(a);C.removeState(d);}
;}
;}
,_cleanUpCells:function(){var O=this._getChildren();for(var x=O.length-1;x>=0;x-- ){var N=O[x];N.destroy();}
;}
},destruct:function(){this.__la.dispose();this._disposeObjects(c);}
});}
)();
...
(function(){var a='<div>',b="</div>",c="overflow: hidden;",d="qx.event.type.Data",e='</div>',f="paneReloadsData",g="div",h='style="',j="_applyMaxCacheLines",k="qx.ui.table.pane.Pane",l="width: 100%;",m="qx.event.type.Event",n="_applyVisibleRowCount",o='>',p="",q='" ',r="line-height: ",s="paneUpdated",t="appear",u='class="',v="width:100%;",w="px;",z='<div ',A="'>",B="Number",C="_applyFirstVisibleRow",D="<div style='",E=";position:relative;";qx.Class.define(k,{extend:qx.ui.core.Widget,construct:function(F){qx.ui.core.Widget.call(this);this.__pw=F;this.__px=0;this.__of=0;this.__py=[];}
,events:{"paneReloadsData":d,"paneUpdated":m},properties:{firstVisibleRow:{check:B,init:0,apply:C},visibleRowCount:{check:B,init:0,apply:n},maxCacheLines:{check:B,init:1000,apply:j},allowShrinkX:{refine:true,init:false}},members:{__of:null,__px:null,__pw:null,__pz:null,__oc:null,__ob:null,__py:null,__pA:0,_applyFirstVisibleRow:function(H,G){this.updateContent(false,H-G);}
,_applyVisibleRowCount:function(J,I){this.updateContent(true);}
,_getContentHint:function(){return {width:this.getPaneScroller().getTablePaneModel().getTotalWidth(),height:400};}
,getPaneScroller:function(){return this.__pw;}
,getTable:function(){return this.__pw.getTable();}
,setFocusedCell:function(M,L,K){if(M!=this.__ob||L!=this.__oc){var N=this.__oc;this.__ob=M;this.__oc=L;if(L!=N&&!K){if(N!==null){this.updateContent(false,null,N,true);}
...
,setColumnWidth:function(P,O){this.updateContent(true);}
,onColOrderChanged:function(){this.updateContent(true);}
,onPaneModelChanged:function(){this.updateContent(true);}
,onTableModelDataChanged:function(T,V,R,Q){this.__pB();var S=this.getFirstVisibleRow();var U=this.getVisibleRowCount();if(V==-1||V>=S&&T<S+U){this.updateContent();}
;}
,onTableModelMetaDataChanged:function(){this.updateContent(true);}
,_applyMaxCacheLines:function(X,W){if(this.__pA>=X&&X!==-1){this.__pB();}
;}
,__pB:function(){this.__py=[];this.__pA=0;}
,__pC:function(bb,Y,ba){if(!Y&&!ba&&this.__py[bb]){return this.__py[bb];}
else {return null;}
;}
,__pD:function(bg,bf,bd,be){var bc=this.getMaxCacheLines();if(!bd&&!be&&!this.__py[bg]&&bc>0){this._applyMaxCacheLines(bc);this.__py[bg]=bf;this.__pA+=1;}
;}
,updateContent:function(bh,bk,bj,bi){if(bh){this.__pB();}
;if(bk&&Math.abs(bk)<=Math.min(10,this.getVisibleRowCount())){this._scrollContent(bk);}
else if(bi&&!this.getTable().getAlwaysUpdateCells()){this._updateRowStyles(bj);}
else {this._updateAllRows();}
;}
,_updateRowStyles:function(bq){var bp=this.getContentElement().getDomElement();if(!bp||!bp.firstChild){this._updateAllRows();return;}
;var bt=this.getTable();var bm=bt.getSelectionModel();var br=bt.getTableModel();var bu=bt.getDataRowRenderer();var bo=bp.firstChild.childNodes;var bn={table:bt};var bv=this.getFirstVisibleRow();var y=0;var bl=bo.length;if(bq!=null){var bs=bq-bv;if(bs>=0&&bs<bl){bv=bq;y=bs;bl=bs+1;}
else {return;}
;}
;for(;y<bl;y++ ,bv++ ){bn.row=bv;bn.selected=bm.isSelectedIndex(bv);bn.focusedRow=(this.__oc==bv);bn.rowData=br.getRowData(bv);bu.updateDataRowElement(bn,bo[y]);}
;}
,_getRowsHtml:function(bH,bQ){var bB=this.getTable();var bE=bB.getSelectionModel();var bz=bB.getTableModel();var bA=bB.getTableColumnModel();var bV=this.getPaneScroller().getTablePaneModel();var bK=bB.getDataRowRenderer();bz.prefetchRows(bH,bH+bQ-1);var bS=bB.getRowHeight();var bU=bV.getColumnCount();var ca=0;var by=[];for(var x=0;x<bU;x++ ){var bY=bV.getColumnAtX(x);var bD=bA.getColumnWidth(bY);by.push({col:bY,xPos:x,editable:bz.isColumnEditable(bY),focusedCol:this.__ob==bY,styleLeft:ca,styleWidth:bD});ca+=bD;}
;var bX=[];var bw=false;for(var bC=bH;bC<bH+bQ;bC++ ){var bF=bE.isSelectedIndex(bC);var bJ=(this.__oc==bC);var bL=this.__pC(bC,bF,bJ);if(bL){bX.push(bL);continue;}
;var bM=[];var bR={table:bB};bR.styleHeight=bS;bR.row=bC;bR.selected=bF;bR.focusedRow=bJ;bR.rowData=bz.getRowData(bC);if(!bR.rowData){bw=true;}
;bM.push(z);var bN=bK.getRowAttributes(bR);if(bN){bM.push(bN);}
;var bx=bK.getRowClass(bR);if(bx){bM.push(u,bx,q);}
;var bO=bK.createRowStyle(bR);bO+=E+bK.getRowHeightStyle(bS)+v;if(bO){bM.push(h,bO,q);}
;bM.push(o);var bW=false;for(x=0;x<bU&&!bW;x++ ){var bG=by[x];for(var bT in bG){bR[bT]=bG[bT];}
;var bY=bR.col;bR.value=bz.getValue(bY,bC);var bI=bA.getDataCellRenderer(bY);bR.style=bI.getDefaultCellStyle();bW=bI.createDataCellHtml(bR,bM)||false;}
;bM.push(e);var bP=bM.join(p);this.__pD(bC,bP,bF,bJ);bX.push(bP);}
;this.fireDataEvent(f,bw);return bX.join(p);}
,_scrollContent:function(cm){var cb=this.getContentElement().getDomElement();if(!(cb&&cb.firstChild)){this._updateAllRows();return;}
;var ck=cb.firstChild;var cc=ck.childNodes;var ce=this.getVisibleRowCount();var ch=this.getFirstVisibleRow();var cf=this.getTable().getTableModel();var cl=0;cl=cf.getRowCount();if(ch+ce>cl){this._updateAllRows();return;}
;var cn=cm<0?ce+cm:0;var cd=cm<0?0:ce-cm;for(var i=Math.abs(cm)-1;i>=0;i-- ){var cg=cc[cn];try{ck.removeChild(cg);}
catch(co){break;}
;}
;if(!this.__pz){this.__pz=document.createElement(g);}
;var cj=a;cj+=this._getRowsHtml(ch+cd,Math.abs(cm));cj+=e;this.__pz.innerHTML=cj;var ci=this.__pz.firstChild.childNodes;if(cm>0){for(var i=ci.length-1;i>=0;i-- ){var cg=ci[0];ck.appendChild(cg);}
;}
else {for(var i=ci.length-1;i>=0;i-- ){var cg=ci[ci.length-1];ck.insertBefore(cg,ck.firstChild);}
;}
;if(this.__oc!==null){this._updateRowStyles(this.__oc-cm);this._updateRowStyles(this.__oc);}
;this.fireEvent(s);}
,_updateAllRows:function(){var cs=this.getContentElement().getDomElement();if(!cs){this.addListenerOnce(t,arguments.callee,this);return;}
...
;var cr=cx.getTotalWidth();var cu;if(cq>0){cu=[D,l,(cy.getForceLineHeight()?r+cp+w:p),c,A,this._getRowsHtml(ct,cq),b];}
else {cu=[];}
;var cA=cu.join(p);cs.innerHTML=cA;this.setWidth(cr);this.__px=cw;this.__of=cq;this.fireEvent(s);}
...
(function(){var a="qx.ui.table.headerrenderer.Default",b="sortedAscending",c="sorted",d="String";qx.Class.define(a,{extend:qx.core.Object,implement:qx.ui.table.IHeaderRenderer,statics:{STATE_SORTED:c,STATE_SORTED_ASCENDING:b},properties:{toolTip:{check:d,init:null,nullable:true}},members:{createHeaderCell:function(e){var f=new qx.ui.table.headerrenderer.HeaderCell();this.updateHeaderCell(e,f);return f;}
,updateHeaderCell:function(j,i){var h=qx.ui.table.headerrenderer.Default;if(j.name&&j.name.translate){i.setLabel(j.name.translate());}
else {i.setLabel(j.name);}
;var g=i.getToolTip();if(this.getToolTip()!=null){if(g==null){g=new qx.ui.tooltip.ToolTip(this.getToolTip());i.setToolTip(g);qx.util.DisposeUtil.disposeTriggeredBy(g,i);}
else {g.setLabel(this.getToolTip());}
;}
;j.sorted?i.addState(h.STATE_SORTED):i.removeState(h.STATE_SORTED);j.sortedAscending?i.addState(h.STATE_SORTED_ASCENDING):i.removeState(h.STATE_SORTED_ASCENDING);}
}});}
)();
...
(function(){var a="_applySortIcon",b="_applyIcon",c="icon",d="label",e="table-header-cell",f="qx.ui.table.headerrenderer.HeaderCell",g="_applyLabel",h="String",i="sort-icon";qx.Class.define(f,{extend:qx.ui.container.Composite,construct:function(){qx.ui.container.Composite.call(this);var j=new qx.ui.layout.Grid();j.setRowFlex(0,1);j.setColumnFlex(1,1);j.setColumnFlex(2,1);this.setLayout(j);}
,properties:{appearance:{refine:true,init:e},label:{check:h,init:null,nullable:true,apply:g},sortIcon:{check:h,init:null,nullable:true,apply:a,themeable:true},icon:{check:h,init:null,nullable:true,apply:b}},members:{_applyLabel:function(l,k){if(l){this._showChildControl(d).setValue(l);}
else {this._excludeChildControl(d);}
;}
,_applySortIcon:function(n,m){if(n){this._showChildControl(i).setSource(n);}
else {this._excludeChildControl(i);}
;}
,_applyIcon:function(p,o){if(p){this._showChildControl(c).setSource(p);}
else {this._excludeChildControl(c);}
;}
,_createChildControlImpl:function(s,r){var q;switch(s){case d:q=new qx.ui.basic.Label(this.getLabel()).set({anonymous:true,allowShrinkX:true});this._add(q,{row:0,column:1});break;case i:q=new qx.ui.basic.Image(this.getSortIcon());q.setAnonymous(true);this._add(q,{row:0,column:2});break;case c:q=new qx.ui.basic.Image(this.getIcon()).set({anonymous:true,allowShrinkX:true});this._add(q,{row:0,column:0});break;};return q||qx.ui.container.Composite.prototype._createChildControlImpl.call(this,s);}
}});}
)();
...
(function(){var a="qooxdoo-table-cell",b="content",c='</div>',d="nowrap",e="default",f="changeTheme",g="}",h="width:",i=".qooxdoo-table-cell-right { text-align:right } ",j="css.boxsizing",k="css.boxmodel",l="0px 6px",m='<div class="',n="0px",o="height:",p="1px solid ",q=".qooxdoo-table-cell-bold { font-weight:bold } ",r="String",s="} ",t="qx.dyntheme",u='>',v="",w='" ',x="ellipsis",y="content-box",z="px;",A='left:',B="qx.ui.table.cellrenderer.Abstract",C='" style="',D="abstract",E="none",F=".qooxdoo-table-cell {",G="hidden",H="table-column-line",I='px;',J=".qooxdoo-table-cell-italic { font-style:italic} ",K="absolute";qx.Class.define(B,{type:D,implement:qx.ui.table.ICellRenderer,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);var L=qx.ui.table.cellrenderer.Abstract;if(!L.__oP){L.__oP=this.self(arguments);this._createStyleSheet();if(qx.core.Environment.get(t)){qx.theme.manager.Color.getInstance().addListener(f,this._onChangeTheme,this);}
;}
;}
,properties:{defaultCellStyle:{init:null,check:r,nullable:true}},members:{_onChangeTheme:qx.core.Environment.select(t,{"true":function(){qx.bom.Stylesheet.removeAllRules(qx.ui.table.cellrenderer.Abstract.__oP.stylesheet);this._createStyleSheet();}
,"false":null}),_insetX:6+6+1,_insetY:0,_createStyleSheet:function(){var M=qx.theme.manager.Color.getInstance();var N=F+qx.bom.element.Style.compile({position:K,top:n,overflow:G,whiteSpace:d,borderRight:p+M.resolve(H),padding:l,cursor:e,textOverflow:x,userSelect:E})+s+i+J+q;if(qx.core.Environment.get(j)){N+=F+qx.bom.element.BoxSizing.compile(y)+g;}
;qx.ui.table.cellrenderer.Abstract.__oP.stylesheet=qx.bom.Stylesheet.createElement(N);}
,_getCellClass:function(O){return a;}
,_getCellStyle:function(P){return P.style||v;}
,_getCellAttributes:function(Q){return v;}
,_getContentHtml:function(R){return R.value||v;}
,_getCellSizeStyle:function(S,W,U,V){var T=v;if(qx.core.Environment.get(k)==b){S-=U;W-=V;}
;T+=h+Math.max(S,0)+z;T+=o+Math.max(W,0)+z;return T;}
,createDataCellHtml:function(X,Y){Y.push(m,this._getCellClass(X),C,A,X.styleLeft,I,this._getCellSizeStyle(X.styleWidth,X.styleHeight,this._insetX,this._insetY),this._getCellStyle(X),w,this._getCellAttributes(X),u+this._getContentHtml(X),c);}
},destruct:function(){if(qx.core.Environment.get(t)){qx.theme.manager.Color.getInstance().removeListener(f,this._onChangeTheme,this);}
;}
});}
)();
(function(){var a="Boolean",b="",c="qx.ui.table.cellrenderer.Default",d="number",e=" qooxdoo-table-cell-bold",f=" qooxdoo-table-cell-right",g=" qooxdoo-table-cell-italic",h="string";qx.Class.define(c,{extend:qx.ui.table.cellrenderer.Abstract,statics:{STYLEFLAG_ALIGN_RIGHT:1,STYLEFLAG_BOLD:2,STYLEFLAG_ITALIC:4,_numberFormat:null},properties:{useAutoAlign:{check:a,init:true}},members:{_getStyleFlags:function(i){if(this.getUseAutoAlign()){if(typeof i.value==d){return qx.ui.table.cellrenderer.Default.STYLEFLAG_ALIGN_RIGHT;}
;}
;return 0;}
,_getCellClass:function(j){var k=qx.ui.table.cellrenderer.Abstract.prototype._getCellClass.call(this,j);if(!k){return b;}
;var l=this._getStyleFlags(j);if(l&qx.ui.table.cellrenderer.Default.STYLEFLAG_ALIGN_RIGHT){k+=f;}
;if(l&qx.ui.table.cellrenderer.Default.STYLEFLAG_BOLD){k+=e;}
;if(l&qx.ui.table.cellrenderer.Default.STYLEFLAG_ITALIC){k+=g;}
;return k;}
...
;if(typeof p==h){return p;}
else if(typeof p==d){if(!qx.ui.table.cellrenderer.Default._numberFormat){qx.ui.table.cellrenderer.Default._numberFormat=new qx.util.format.NumberFormat();qx.ui.table.cellrenderer.Default._numberFormat.setMaximumFractionDigits(2);}
...
(function(){var a="-",b="Boolean",c="^",d="Number string '",e="' does not match the number format",f="0",g="NaN",h='(',i="-Infinity",j="String",k='{0,1}[0-9]{3}){0,})',l="",m='([0-9]{1,3}(?:',n="qx.util.format.NumberFormat",o='([-+]){0,1}',p="g",q="$",r="Number",s='\\d+){0,1}',t=".",u="changeNumberFormat",v="Infinity";qx.Class.define(n,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(w){qx.core.Object.call(this);this.__jq=w;}
,properties:{minimumIntegerDigits:{check:r,init:0},maximumIntegerDigits:{check:r,nullable:true},minimumFractionDigits:{check:r,init:0},maximumFractionDigits:{check:r,nullable:true},groupingUsed:{check:b,init:true},prefix:{check:j,init:l,event:u},postfix:{check:j,init:l,event:u}},members:{__jq:null,format:function(C){switch(C){case Infinity:return v;case -Infinity:return i;case NaN:return g;};var A=(C<0);if(A){C=-C;}
;if(this.getMaximumFractionDigits()!=null){var I=Math.pow(10,this.getMaximumFractionDigits());C=Math.round(C*I)/I;}
;var G=String(Math.floor(C)).length;var x=l+C;var E=x.substring(0,G);while(E.length<this.getMinimumIntegerDigits()){E=f+E;}
...
;var D=x.substring(G+1);while(D.length<this.getMinimumFractionDigits()){D+=f;}
...
;if(this.getGroupingUsed()){var z=E;E=l;var H;for(H=z.length;H>3;H-=3){E=l+qx.locale.Number.getGroupSeparator(this.__jq)+z.substring(H-3,H)+E;}
;E=z.substring(0,H)+E;}
;var B=this.getPrefix()?this.getPrefix():l;var y=this.getPostfix()?this.getPostfix():l;var F=B+(A?a:l)+E;if(D.length>0){F+=l+qx.locale.Number.getDecimalSeparator(this.__jq)+D;}
;F+=y;return F;}
,parse:function(R){var N=qx.lang.String.escapeRegexpChars(qx.locale.Number.getGroupSeparator(this.__jq)+l);var L=qx.lang.String.escapeRegexpChars(qx.locale.Number.getDecimalSeparator(this.__jq)+l);var J=new RegExp(c+qx.lang.String.escapeRegexpChars(this.getPrefix())+o+m+N+k+h+L+s+qx.lang.String.escapeRegexpChars(this.getPostfix())+q);var M=J.exec(R);if(M==null){throw new Error(d+R+e);}
;var O=(M[1]==a);var Q=M[2];var P=M[3];Q=Q.replace(new RegExp(N,p),l);var K=(O?a:l)+Q;if(P!=null&&P.length!=0){P=P.replace(new RegExp(L),l);K+=t+P;}
;return parseFloat(K);}
}});}
)();
...
(function(){var a="-",b="QQQ",c='W',d="qqqq",e="h",f="KK",g='L',h=" is not supported",m="Z",n="Date string '",o="GGGGG",p='abbreviated',q="Y+",r=". Wildcard ",s="eeeee",t="(\\d\\d?)",u="LL",v="abbreviated",w="EEEE",x="^",y=":",z='y',A="quoted_literal",B="weekOfYear",C='3rd quarter',D="SSS",E='a',F="day",G="(-*",H="qqq",I="locale",J="K",K='q',L="Provided date is invalid",M="pm",N="Error parsing date '",O='Anno Domini',P='G',Q="a",R='2nd quarter',S="yyyy-MM-dd'T'HH:mm:ss",T="GMT",U="HH:mm:ss",V="dd",W="+",X="qx.util.format.DateFormat",Y='weekDay',en="am",ei="eee",eo="quarter",ek="ee",el="q",eh="QQQQ",em="HHmmss",es="ww",et="H",eu='Y+',ev="wide",ep='wide',eq="(\\d\\d\\d?)",ej="(GMT[\\+\\-]\\d\\d:\\d\\d)",er="_applyLocale",ez="Q",fd="c",eA="(",eB="weekOfMonth",ew="ccc",ex="long",ge="Malformed date format: ",ey="EEEEE",eC='Y',eD="EE",eE="GGG",eJ='-',eK="w",eL="mm",eF='h',eG="S",eH='s',eI='A',eQ="yyyy-MM-dd",eR="G",eS='z',eT="GG",eM=")",eN="kk",eO="ss",eP="([\\+\\-]\\d\\d\\d\\d)",eX="default",eY='BC',gi='H',fa='S',eU='1st quarter',eV="stand-alone",gg="0",eW="MMMM",fb='c',fc="d",fo='Q',fn='Q1',fm="eeee",fs="QQ",fr="LLLLL",fq="HH",fp='k',fh="m",fg="DDD",ff="D",fe='D',fl="LLLL",fk="(\\d\\d*?)",fj='e',fi="': the value for day or month is too large",fz="literal",fy='K',fx="L",fw="hh",fD="ms",fC="(\\d\\d?\\d?)",fB="cccc",fA="e",fv='y+',fu="|",ft="W",fO="dayOfYear",fN="GGGG",fM='Q2',fS='B',fR="MM",fQ='Q4',fP="y+",fH='narrow',fG="ccccc",fF="E",fE="wildcard",fL='',fK="' does not match the date format: ",fJ="short",fI='d',fY="unkown",fX="narrow",fW="\\d?",fV="'",gd="k",gc='Before Christ',gb='Z',ga="qx.debug",fU='m',fT=" ",dQ="yyyy-MM-dd'T'HH:mm:ss'Z'",dP="String",gj='Q3',dN="z",dO="DD",dM="min",gh="",dK='M',dL="sec",dJ='isoUtcDateTime',gf='4th quarter',dH="(\\d?)",dI="SS",dG="MMM",ea="s",eb="M",dX='w',dY="EEE",dV="$",dW="?",dU='AD',dF="\\d",dS="+?",dT="hour",dR="MMMMM",eg='E',ee='month',ef="LLL",ec="qq",ed="format";qx.Class.define(X,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(gl,gk){qx.core.Object.call(this);if(!gk){this.__jq=qx.locale.Manager.getInstance().getLocale();this.__oQ=qx.locale.Manager.getInstance().bind(I,this,I);}
else {this.__jq=gk;this.setLocale(gk);}
;this.__oR=this.__jq;if(gl!=null){this.__oS=gl.toString();if(this.__oS in qx.util.format.DateFormat.ISO_MASKS){if(this.__oS===dJ){this.__oT=true;}
;this.__oS=qx.util.format.DateFormat.ISO_MASKS[this.__oS];}
;}
else {this.__oS=qx.locale.Date.getDateFormat(ex,this.__jq)+fT+qx.locale.Date.getDateTimeFormat(em,U,this.__jq);}
;}
,properties:{locale:{apply:er,nullable:true,check:dP}},statics:{getDateInstance:function(){var gm=qx.util.format.DateFormat;var gn=qx.locale.Date.getDateFormat(fJ)+gh;if(gm._dateInstance==null||gm._dateInstance.__oS!=gn){gm._dateInstance=new gm(gn);}
;return gm._dateInstance;}
,getDateTimeInstance:function(){var go=qx.util.format.DateFormat;var gp=qx.locale.Date.getDateFormat(ex)+fT+qx.locale.Date.getDateTimeFormat(em,U);if(go._dateTimeInstance==null||go._dateTimeInstance.__oS!=gp){go._dateTimeInstance=new go(gp);}
;return go._dateTimeInstance;}
,ASSUME_YEAR_2000_THRESHOLD:30,ISO_MASKS:{isoDate:eQ,isoTime:U,isoDateTime:S,isoUtcDateTime:dQ},AM_MARKER:en,PM_MARKER:M},members:{__oQ:null,__jq:null,__oR:null,__oS:null,__oU:null,__oV:null,__oW:null,__oT:null,__oX:function(gr,gq){var gs=gh+(gr<0?((-1)*gr):gr);while(gs.length<gq){gs=gg+gs;}
;return gr<0?a+gs:gs;}
,__oY:function(gt){var gu=new Date(gt.getTime());var gv=gu.getDate();while(gu.getMonth()!=0){gu.setDate(-1);gv+=gu.getDate()+1;}
;return gv;}
,__pa:function(gw){return new Date(gw.getTime()+(3-((gw.getDay()+6)%7))*86400000);}
,__pb:function(gx){var gz=this.__pa(gx);var gA=gz.getFullYear();var gy=this.__pa(new Date(gA,0,4));return Math.floor(1.5+(gz.getTime()-gy.getTime())/86400000/7);}
,__pc:function(gB){var gD=this.__pa(gB);var gC=this.__pa(new Date(gB.getFullYear(),gB.getMonth(),4));return Math.floor(1.5+(gD.getTime()-gC.getTime())/86400000/7);}
,__pd:function(gE){var gF=this.__pa(gE);return gF.getFullYear();}
,__pe:function(gH){var gG=new Date(gH,2,1);gG.setDate(-1);return gG.getDate()+1===29;}
,__pf:function(gM,gN){var gL=0;var gO=0;if(!gN){gN=1971;}
;var gI=0;for(var i=1;i<=12;i++ ){var gK=new Date(gN,i,1);gK.setDate(-1);var gJ=gK.getDate()+1;gI+=gJ;if(gI<gM){gL++ ;gO+=gJ;}
else {gO=gM-(gI-gJ);break;}
;}
;return {month:gL,day:gO};}
,__pg:function(gS,gQ,gP){var gR;switch(gQ){case 11:gR=gS-1;if(gS!=this.__pd(new Date(gR,gQ,gP))){gR=gS;}
;break;case 0:gR=gS+1;if(gS!=this.__pd(new Date(gR,gQ,gP))){gR=gS;}
;break;default:gR=gS;};return gR;}
,_applyLocale:function(gU,gT){this.__jq=gU===null?this.setLocale(this.__oR):gU;}
,format:function(hl){if(hl==null){return null;}
;if(isNaN(hl.getTime())){if(qx.core.Environment.get(ga)){qx.log.Logger.error(L);}
;return null;}
;if(this.__oT){hl=new Date(hl.getUTCFullYear(),hl.getUTCMonth(),hl.getUTCDate(),hl.getUTCHours(),hl.getUTCMinutes(),hl.getUTCSeconds(),hl.getUTCMilliseconds());}
;var gX=this.__jq;var hd=hl.getFullYear();var gY=hl.getMonth();var hc=hl.getDate();var hg=hl.getDay();var hj=hl.getHours();var hq=hl.getMinutes();var hh=hl.getSeconds();var ho=hl.getMilliseconds();var hf=hl.getTimezoneOffset();var hk=hf>0?1:-1;var gV=Math.floor(Math.abs(hf)/60);var hn=Math.abs(hf)%60;this.__ph();var hp=gh;for(var i=0;i<this.__oW.length;i++ ){var he=this.__oW[i];if(he.type==fz){hp+=he.text;}
else {var gW=he.character;var hi=he.size;var hr=dW;switch(gW){case z:if(hi==2){hr=this.__oX(hd%100,2);}
else {var ha=Math.abs(hd);hr=ha+gh;if(hi>hr.length){for(var j=hr.length;j<hi;j++ ){hr=gg+hr;}
;}
;if(hd<0){hr=a+hr;}
;}
;break;case eC:hr=this.__pd(hl)+gh;var ha=hr.replace(eJ,fL);if(hi>hr.length){for(var j=ha.length;j<hi;j++ ){ha=gg+ha;}
;}
;hr=hr.indexOf(a)!=-1?a+ha:ha;break;case P:if(hi>=1&&hi<=3){hr=hd>0?dU:eY;}
else if(hi==4){hr=hd>0?O:gc;}
else if(hi==5){hr=hd>0?eI:fS;}
;break;case fo:if(hi==1||hi==2){hr=this.__oX(parseInt(gY/4)+1,hi);}
;if(hi==3){hr=fo+(parseInt(gY/4)+1);}
;break;case K:if(hi==1||hi==2){hr=this.__oX(parseInt(gY/4)+1,hi);}
;if(hi==3){hr=fo+(parseInt(gY/4)+1);}
;break;case fe:hr=this.__oX(this.__oY(hl),hi);break;case fI:hr=this.__oX(hc,hi);break;case dX:hr=this.__oX(this.__pb(hl),hi);break;case c:hr=this.__pc(hl);break;case eg:if(hi>=1&&hi<=3){hr=qx.locale.Date.getDayName(v,hg,gX,ed,true);}
else if(hi==4){hr=qx.locale.Date.getDayName(ev,hg,gX,ed,true);}
else if(hi==5){hr=qx.locale.Date.getDayName(fX,hg,gX,ed,true);}
;break;case fj:var hb=qx.locale.Date.getWeekStart(gX);var hm=1+((hg-hb>=0)?(hg-hb):7+(hg-hb));if(hi>=1&&hi<=2){hr=this.__oX(hm,hi);}
else if(hi==3){hr=qx.locale.Date.getDayName(v,hg,gX,ed,true);}
else if(hi==4){hr=qx.locale.Date.getDayName(ev,hg,gX,ed,true);}
else if(hi==5){hr=qx.locale.Date.getDayName(fX,hg,gX,ed,true);}
;break;case fb:var hb=qx.locale.Date.getWeekStart(gX);var hm=1+((hg-hb>=0)?(hg-hb):7+(hg-hb));if(hi==1){hr=fL+hm;}
else if(hi==3){hr=qx.locale.Date.getDayName(v,hg,gX,eV,true);}
else if(hi==4){hr=qx.locale.Date.getDayName(ev,hg,gX,eV,true);}
else if(hi==5){hr=qx.locale.Date.getDayName(fX,hg,gX,eV,true);}
;break;case dK:if(hi==1||hi==2){hr=this.__oX(gY+1,hi);}
else if(hi==3){hr=qx.locale.Date.getMonthName(v,gY,gX,ed,true);}
else if(hi==4){hr=qx.locale.Date.getMonthName(ev,gY,gX,ed,true);}
else if(hi==5){hr=qx.locale.Date.getMonthName(fX,gY,gX,ed,true);}
;break;case g:if(hi==1||hi==2){hr=this.__oX(gY+1,hi);}
else if(hi==3){hr=qx.locale.Date.getMonthName(v,gY,gX,eV,true);}
else if(hi==4){hr=qx.locale.Date.getMonthName(ev,gY,gX,eV,true);}
else if(hi==5){hr=qx.locale.Date.getMonthName(fX,gY,gX,eV,true);}
;break;case E:hr=(hj<12)?qx.locale.Date.getAmMarker(gX):qx.locale.Date.getPmMarker(gX);break;case gi:hr=this.__oX(hj,hi);break;case fp:hr=this.__oX((hj==0)?24:hj,hi);break;case fy:hr=this.__oX(hj%12,hi);break;case eF:hr=this.__oX(((hj%12)==0)?12:(hj%12),hi);break;case fU:hr=this.__oX(hq,hi);break;case eH:hr=this.__oX(hh,hi);break;case fa:hr=this.__oX(ho,3);if(hi<hr.length){hr=hr.substr(0,hi);}
else {while(hi>hr.length){hr+=gg;}
;}
;break;case eS:if(hi>=1&&hi<=4){hr=T+((hk>0)?a:W)+this.__oX(Math.abs(gV),2)+y+this.__oX(hn,2);}
;break;case gb:if(hi>=1&&hi<=3){hr=((hk>0)?a:W)+this.__oX(Math.abs(gV),2)+this.__oX(hn,2);}
else {hr=T+((hk>0)?a:W)+this.__oX(Math.abs(gV),2)+y+this.__oX(hn,2);}
;break;};hp+=hr;}
;}
;return hp;}
,parse:function(hy){this.__pi();var hB=this.__oU.regex.exec(hy);if(hB==null){throw new Error(n+hy+fK+this.__oS);}
;var hs={era:1,year:1970,quarter:1,month:0,day:1,dayOfYear:1,hour:0,ispm:false,weekDay:4,weekYear:1970,weekOfMonth:1,weekOfYear:1,min:0,sec:0,ms:0};var hw=1;var ht=false;var hu=false;for(var i=0;i<this.__oU.usedRules.length;i++ ){var hv=this.__oU.usedRules[i];var hx=hB[hw];if(hv.field!=null){hs[hv.field]=parseInt(hx,10);}
else {hv.manipulator(hs,hx,hv.pattern);}
;if(hv.pattern==q){var hz=false;for(var k=0;k<this.__oU.usedRules.length;k++ ){if(this.__oU.usedRules[k].pattern==fv){hz=true;break;}
;}
;if(!hz){ht=true;}
;}
;if(hv.pattern.indexOf(ff)!=-1){var hD=false;for(var k=0;k<this.__oU.usedRules.length;k++ ){if(this.__oU.usedRules[k].pattern.indexOf(fc)!=-1){hD=true;break;}
;}
;if(!hD){hu=true;}
;}
;hw+=(hv.groups==null)?1:hv.groups;}
;if(ht){hs.year=this.__pg(hs.weekYear,hs.month,hs.day);}
;if(hu){var hC=this.__pf(hs.dayOfYear,hs.year);hs.month=hC.month;hs.day=hC.day;}
;if(hs.era<0&&(hs.year*hs.era<0)){hs.year=hs.year*hs.era;}
;var hA=new Date(hs.year,hs.month,hs.day,(hs.ispm)?(hs.hour+12):hs.hour,hs.min,hs.sec,hs.ms);if(this.__oT){hA=new Date(hA.getUTCFullYear(),hA.getUTCMonth(),hA.getUTCDate(),hA.getUTCHours(),hA.getUTCMinutes(),hA.getUTCSeconds(),hA.getUTCMilliseconds());}
;if(hs.month!=hA.getMonth()||hs.year!=hA.getFullYear()){throw new Error(N+hy+fi);}
;return hA;}
,__ph:function(){if(this.__oW!=null){return;}
;this.__oW=[];var hI;var hG=0;var hK=gh;var hE=this.__oS;var hH=eX;var i=0;while(i<hE.length){var hJ=hE.charAt(i);switch(hH){case A:if(hJ==fV){if(i+1>=hE.length){i++ ;break;}
;var hF=hE.charAt(i+1);if(hF==fV){hK+=hJ;i++ ;}
else {i++ ;hH=fY;}
;}
else {hK+=hJ;i++ ;}
;break;case fE:if(hJ==hI){hG++ ;i++ ;}
else {this.__oW.push({type:fE,character:hI,size:hG});hI=null;hG=0;hH=eX;}
;break;default:if((hJ>=E&&hJ<=eS)||(hJ>=eI&&hJ<=gb)){hI=hJ;hH=fE;}
else if(hJ==fV){if(i+1>=hE.length){hK+=hJ;i++ ;break;}
;var hF=hE.charAt(i+1);if(hF==fV){hK+=hJ;i++ ;}
;i++ ;hH=A;}
else {hH=eX;}
;if(hH!=eX){if(hK.length>0){this.__oW.push({type:fz,text:hK});hK=gh;}
;}
else {hK+=hJ;i++ ;}
;break;};}
;if(hI!=null){this.__oW.push({type:fE,character:hI,size:hG});}
else if(hK.length>0){this.__oW.push({type:fz,text:hK});}
;}
,__pi:function(){if(this.__oU!=null){return;}
;var hO=this.__oS;this.__pk();this.__ph();var hU=[];var hP=x;for(var hM=0;hM<this.__oW.length;hM++ ){var hV=this.__oW[hM];if(hV.type==fz){hP+=qx.lang.String.escapeRegexpChars(hV.text);}
else {var hN=hV.character;var hQ=hV.size;var hS;for(var hW=0;hW<this.__oV.length;hW++ ){var hR=this.__oV[hW];if(this.__pj(hR,hN,hQ)){hS=hR;break;}
;}
;if(hS==null){var hT=gh;for(var i=0;i<hQ;i++ ){hT+=hN;}
;throw new Error(ge+hO+r+hT+h);}
else {hU.push(hS);hP+=hS.regex;}
;}
;}
;hP+=dV;var hL;try{hL=new RegExp(hP);}
catch(hX){throw new Error(ge+hO);}
;this.__oU={regex:hL,"usedRules":hU,pattern:hP};}
,__pj:function(hY,ia,ib){if(ia===z&&hY.pattern===fv){hY.regex=hY.regexFunc(ib);return true;}
else if(ia===eC&&hY.pattern===eu){hY.regex=hY.regexFunc(ib);return true;}
else {return ia==hY.pattern.charAt(0)&&ib==hY.pattern.length;}
;}
,__pk:function(){var ir=qx.util.format.DateFormat;var iu=qx.lang.String;if(this.__oV!=null){return;}
;var is=this.__oV=[];var im=qx.locale.Date.getAmMarker(this.__jq).toString()||ir.AM_MARKER;var iB=qx.locale.Date.getPmMarker(this.__jq).toString()||ir.PM_MARKER;var id=this.__jq;var ij=function(iE,iF){iF=parseInt(iF,10);if(iF>0){if(iF<ir.ASSUME_YEAR_2000_THRESHOLD){iF+=2000;}
else if(iF<100){iF+=1900;}
;}
;iE.year=iF;}
;var iD=function(iG,iH){iH=parseInt(iH,10);if(iH>0){if(iH<ir.ASSUME_YEAR_2000_THRESHOLD){iH+=2000;}
else if(iH<100){iH+=1900;}
;}
;iG.weekYear=iH;}
;var il=function(iI,iJ){iI.month=parseInt(iJ,10)-1;}
;var ih=function(iM,iN){var iL=qx.locale.Date.getWeekStart(id);var iK=(parseInt(iN,10)-1+iL)<=6?parseInt(iN,10)-1+iL:(parseInt(iN,10)-1+iL)-7;iM.weekDay=iK;}
;var ig=function(iO,iP){var iQ=qx.locale.Date.getPmMarker(id).toString()||ir.PM_MARKER;iO.ispm=(iP==iQ);}
;var it=function(iR,iS){iR.hour=parseInt(iS,10)%24;}
;var ie=function(iT,iU){iT.hour=parseInt(iU,10)%12;}
;var iw=function(iV,iW){return;}
;var ix=[eI,fS];var iy=function(iX,iY){iX.era=iY==eI?1:-1;}
;var iq=[dU,eY];var ip=function(ja,jb){ja.era=jb==dU?1:-1;}
;var iA=[O,gc];var io=function(jc,jd){jc.era=jd==O?1:-1;}
;var iC=[fn,fM,gj,fQ];var ic=function(je,jf){je.quarter=iC.indexOf(jf);}
;var iz=[eU,R,C,gf];var iv=function(jg,jh){jg.quarter=iz.indexOf(jh);}
;var ik={};var ii=function(jl){var jn=[g,dK];var jj=[fb,fj,eg];var jq=jl.charAt(0);var jk=jn.indexOf(jq)>=0;var jm=function(){var js=jk?jn:jj;var jt=jq===js[0]?eV:ed;var jr=jl.length;var ju=p;switch(jr){case 4:ju=ep;break;case 5:ju=fH;break;default:ju=p;};return [jt,ju];}
;if(!ik[jl]){ik[jl]={};var ji=jm();var jp=jk?qx.locale.Date.getMonthNames:qx.locale.Date.getDayNames;var jo=jp.call(qx.locale.Date,ji[1],id,ji[0],true);for(var i=0,l=jo.length;i<l;i++ ){jo[i]=iu.escapeRegexpChars(jo[i].toString());}
;ik[jl].data=jo;ik[jl].func=function(jv,jw){jw=iu.escapeRegexpChars(jw);jv[jk?ee:Y]=jo.indexOf(jw);}
;}
;return ik[jl];}
;is.push({pattern:fP,regexFunc:function(jy){var jx=G;for(var i=0;i<jy;i++ ){jx+=dF;if(i===jy-1&&i!==1){jx+=dS;}
;}
;jx+=eM;return jx;}
,manipulator:ij});is.push({pattern:q,regexFunc:function(jA){var jz=G;for(var i=0;i<jA;i++ ){jz+=dF;if(i===jA-1){jz+=dS;}
;}
;jz+=eM;return jz;}
,manipulator:iD});is.push({pattern:eR,regex:eA+iq.join(fu)+eM,manipulator:ip});is.push({pattern:eT,regex:eA+iq.join(fu)+eM,manipulator:ip});is.push({pattern:eE,regex:eA+iq.join(fu)+eM,manipulator:ip});is.push({pattern:fN,regex:eA+iA.join(fu)+eM,manipulator:io});is.push({pattern:o,regex:eA+ix.join(fu)+eM,manipulator:iy});is.push({pattern:ez,regex:fk,field:eo});is.push({pattern:fs,regex:t,field:eo});is.push({pattern:b,regex:eA+iC.join(fu)+eM,manipulator:ic});is.push({pattern:eh,regex:eA+iz.join(fu)+eM,manipulator:iv});is.push({pattern:el,regex:fk,field:eo});is.push({pattern:ec,regex:t,field:eo});is.push({pattern:H,regex:eA+iC.join(fu)+eM,manipulator:ic});is.push({pattern:d,regex:eA+iz.join(fu)+eM,manipulator:iv});is.push({pattern:eb,regex:fk,manipulator:il});is.push({pattern:fR,regex:t,manipulator:il});is.push({pattern:dG,regex:eA+ii(dG).data.join(fu)+eM,manipulator:ii(dG).func});is.push({pattern:eW,regex:eA+ii(eW).data.join(fu)+eM,manipulator:ii(eW).func});is.push({pattern:dR,regex:eA+ii(dR).data.join(fu)+eM,manipulator:ii(dR).func});is.push({pattern:fx,regex:fk,manipulator:il});is.push({pattern:u,regex:t,manipulator:il});is.push({pattern:ef,regex:eA+ii(ef).data.join(fu)+eM,manipulator:ii(ef).func});is.push({pattern:fl,regex:eA+ii(fl).data.join(fu)+eM,manipulator:ii(fl).func});is.push({pattern:fr,regex:eA+ii(fr).data.join(fu)+eM,manipulator:ii(fr).func});is.push({pattern:V,regex:t,field:F});is.push({pattern:fc,regex:fk,field:F});is.push({pattern:ff,regex:dH,field:fO});is.push({pattern:dO,regex:t,field:fO});is.push({pattern:fg,regex:eq,field:fO});is.push({pattern:fF,regex:eA+ii(fF).data.join(fu)+eM,manipulator:ii(fF).func});is.push({pattern:eD,regex:eA+ii(eD).data.join(fu)+eM,manipulator:ii(eD).func});is.push({pattern:dY,regex:eA+ii(dY).data.join(fu)+eM,manipulator:ii(dY).func});is.push({pattern:w,regex:eA+ii(w).data.join(fu)+eM,manipulator:ii(w).func});is.push({pattern:ey,regex:eA+ii(ey).data.join(fu)+eM,manipulator:ii(ey).func});is.push({pattern:fA,regex:dH,manipulator:ih});is.push({pattern:ek,regex:t,manipulator:ih});is.push({pattern:ei,regex:eA+ii(ei).data.join(fu)+eM,manipulator:ii(ei).func});is.push({pattern:fm,regex:eA+ii(fm).data.join(fu)+eM,manipulator:ii(fm).func});is.push({pattern:s,regex:eA+ii(s).data.join(fu)+eM,manipulator:ii(s).func});is.push({pattern:fd,regex:fW,manipulator:ih});is.push({pattern:ew,regex:eA+ii(ew).data.join(fu)+eM,manipulator:ii(ew).func});is.push({pattern:fB,regex:eA+ii(fB).data.join(fu)+eM,manipulator:ii(fB).func});is.push({pattern:fG,regex:eA+ii(fG).data.join(fu)+eM,manipulator:ii(fG).func});is.push({pattern:Q,regex:eA+im+fu+iB+eM,manipulator:ig});is.push({pattern:ft,regex:dH,field:eB});is.push({pattern:eK,regex:dH,field:B});is.push({pattern:es,regex:t,field:B});is.push({pattern:fq,regex:t,field:dT});is.push({pattern:et,regex:t,field:dT});is.push({pattern:eN,regex:t,manipulator:it});is.push({pattern:gd,regex:t,manipulator:it});is.push({pattern:f,regex:t,field:dT});is.push({pattern:J,regex:t,field:dT});is.push({pattern:fw,regex:t,manipulator:ie});is.push({pattern:e,regex:t,manipulator:ie});is.push({pattern:eL,regex:t,field:dM});is.push({pattern:fh,regex:t,field:dM});is.push({pattern:eO,regex:t,field:dL});is.push({pattern:ea,regex:t,field:dL});is.push({pattern:D,regex:fC,field:fD});is.push({pattern:dI,regex:fC,field:fD});is.push({pattern:eG,regex:fC,field:fD});is.push({pattern:m,regex:eP,manipulator:iw});is.push({pattern:dN,regex:ej,manipulator:iw});}
},destruct:function(){if(this.__oQ!=null){qx.locale.Manager.getInstance().removeBinding(this.__oQ);}
;this.__oW=this.__oU=this.__oV=null;}
});}
)();
(function(){var a='format',b="short",c="HH:mm",d='_',e="stand-alone",f="HHmmsszz",g="cldr_date_format_",h="HHmm",j="narrow",k="HHmmss",l="qx.debug",m="This case should never happen.",n="cldr_time_format_",o="sat",p="abbreviated",q="wide",r="HH:mm:ss zz",s="full",t="cldr_pm",u="cldr_day_",v="_",w="cldr_month_",x="long",y="wed",z="medium",A="fri",B="cldr_am",C="tue",D="qx.locale.Date",E="mon",F="cldr_date_time_format_",G="sun",H="format",I="thu",J="HH:mm:ss",K='stand-alone';qx.Class.define(D,{statics:{__pl:qx.locale.Manager.getInstance(),getAmMarker:function(L){return this.__pl.localize(B,[],L);}
,getPmMarker:function(M){return this.__pl.localize(t,[],M);}
...
;var Q=[G,E,C,y,I,A,o];var R=[];for(var i=0;i<Q.length;i++ ){var N=u+S+v+length+v+Q[i];R.push(P?this.__pm(S,S===a?K:a,N,O):this.__pl.localize(N,[],O));}
;return R;}
...
;var W=[G,E,C,y,I,A,o];var T=u+U+v+length+v+W[Y];return X?this.__pm(U,U===a?K:a,T,V):this.__pl.localize(T,[],V);}
...
;var bd=[];for(var i=0;i<12;i++ ){var ba=w+be+v+length+v+(i+1);bd.push(bc?this.__pm(be,be===a?K:a,ba,bb):this.__pl.localize(ba,[],bb));}
;return bd;}
...
;var bf=w+bg+v+length+v+(bi+1);return bj?this.__pm(bg,bg===a?K:a,bf,bh):this.__pl.localize(bf,[],bh);}
,getDateFormat:function(bm,bk){if(qx.core.Environment.get(l)){qx.core.Assert.assertInArray(bm,[b,z,x,s]);}
;var bl=g+bm;return this.__pl.localize(bl,[],bk);}
,getDateTimeFormat:function(br,bp,bn){var bq=F+br;var bo=this.__pl.localize(bq,[],bn);if(bo==bq){bo=bp;}
;return bo;}
,getTimeFormat:function(bv,bs){if(qx.core.Environment.get(l)){qx.core.Assert.assertInArray(bv,[b,z,x,s]);}
;var bu=n+bv;var bt=this.__pl.localize(bu,[],bs);if(bt!=bu){return bt;}
...
else {bK=this.__pl.getTerritory()||this.__pl.getLanguage();}
;return bK.toUpperCase();}
,__pm:function(bN,bM,bQ,bP){var bO=this.__pl.localize(bQ,[],bP);if(bO==bQ){var bL=bQ.replace(d+bN+d,d+bM+d);return this.__pl.localize(bL,[],bP);}
else {return bO;}
;}
}});}
)();
...
(function(){var a="Show move feedback",b="demobrowser.demo.test.Table_Header",c="Complete update",d="mouseover",f="changeValue",g="Hide move feedback",h="execute";qx.Class.define(b,{extend:qx.application.Standalone,include:[demobrowser.demo.table.MUtil],members:{main:function(){qx.application.Standalone.prototype.main.call(this);var i=new qx.ui.table.pane.Header(this.getScrollerMock());this.getRoot().add(i,{left:10,top:10});i.onPaneModelChanged();i.setColumnWidth(0,150);i.addListener(d,function(e){var l=e.getTarget();if(l instanceof qx.ui.table.headerrenderer.HeaderCell){var m=i._indexOf(l);i.setMouseOverColumn(m);}
;}
);var k=new qx.ui.form.ToggleButton(a);this.getRoot().add(k,{left:20,top:100});k.addListener(f,function(e){var n=e.getData();if(n){k.setLabel(g);i.showColumnMoveFeedback(1,50);}
else {k.setLabel(a);i.hideColumnMoveFeedback();}
;}
);var j=new qx.ui.form.Button(c);this.getRoot().add(j,{left:20,top:140});j.addListener(h,function(){i.onPaneModelChanged();}
);}
}});}
)();
