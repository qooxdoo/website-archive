...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormRendererDouble.13612ec8a3a9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="qx.ui.form.Form",b="";qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__vM=[];this._buttons=[];this._buttonOptions=[];this._validationManager=new qx.ui.form.validation.Manager();this._resetter=this._createResetter();}
,members:{__vM:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(e,d,f,name,c,g){if(this.__vN()){this.__vM.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
;this.__vM[this._groupCounter].items.push(e);this.__vM[this._groupCounter].labels.push(d);this.__vM[this._groupCounter].options.push(g);if(name==null){name=d.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,b);}
;this.__vM[this._groupCounter].names.push(name);this._validationManager.add(e,f,c);this._resetter.add(e);}
,addGroupHeader:function(k,h){if(!this.__vN()){this._groupCounter++ ;}
;this.__vM.push({title:k,items:[],labels:[],names:[],options:[],headerOptions:h});}
,addButton:function(l,m){this._buttons.push(l);this._buttonOptions.push(m||null);}
,__vN:function(){return this.__vM.length===0;}
...
,getGroups:function(){return this.__vM;}
,getButtons:function(){return this._buttons;}
,getButtonOptions:function(){return this._buttonOptions;}
,getItems:function(){var o={};for(var i=0;i<this.__vM.length;i++ ){var p=this.__vM[i];for(var j=0;j<p.names.length;j++ ){var name=p.names[j];o[name]=p.items[j];}
;}
;return o;}
,_createResetter:function(){return new qx.ui.form.Resetter();}
},destruct:function(){this.__vM=this._buttons=this._buttonOptions=null;this._validationManager.dispose();this._resetter.dispose();}
});}
)();
(function(){var a="Widgets supporting selection can only be validated ",b="Added widget not supported.",c="",d="in the form validator",f="changeValid",g="qx.event.type.Event",h="complete",j="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",k="qx.ui.form.validation.Manager",l="This field is required",m="String",n="qx.event.type.Data";qx.Class.define(k,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__vk=[];this.__vl={};this.setRequiredFieldMessage(qx.locale.Manager.tr(l));}
,events:{"changeValid":n,"complete":g},properties:{validator:{check:j,init:null,nullable:true},invalidMessage:{check:m,init:c},requiredFieldMessage:{check:m,init:c},context:{nullable:true}},members:{__vk:null,__vm:null,__vl:null,__vn:null,add:function(r,o,q){if(!this.__vs(r)){throw new Error(b);}
;if(this.__vi(r)&&!r.getValue){if(o!=null){throw new Error(a+d);}
;}
;var p={item:r,validator:o,valid:null,context:q};this.__vk.push(p);}
,remove:function(u){var s=this.__vk;for(var i=0,t=s.length;i<t;i++ ){if(u===s[i].item){s.splice(i,1);return u;}
;}
;return null;}
,getItems:function(){var v=[];for(var i=0;i<this.__vk.length;i++ ){v.push(this.__vk[i].item);}
;return v;}
,validate:function(){var A=true;this.__vn=true;var x=[];for(var i=0;i<this.__vk.length;i++ ){var y=this.__vk[i].item;var B=this.__vk[i].validator;x.push(y);if(B==null){var w=this.__vo(y);A=A&&w;this.__vn=w&&this.__vn;continue;}
;var w=this.__vp(this.__vk[i],y.getValue());A=w&&A;if(w!=null){this.__vn=w&&this.__vn;}
;}
;var z=this.__vq(x);if(qx.lang.Type.isBoolean(z)){this.__vn=z&&this.__vn;}
;A=z&&A;this.__vt(A);if(qx.lang.Object.isEmpty(this.__vl)){this.fireEvent(h);}
;return A;}
,__vo:function(D){if(D.getRequired()){if(this.__vi(D)){var C=!!D.getSelection()[0];}
...
;return true;}
,__vp:function(J,K){var L=J.item;var N=J.context;var M=J.validator;if(this.__vr(M)){this.__vl[L.toHashCode()]=null;M.validate(L,L.getValue(),this,N);return null;}
...
,__vq:function(P){var Q=this.getValidator();var R=this.getContext()||this;if(Q==null){return true;}
;this.setInvalidMessage(c);if(this.__vr(Q)){this.__vl[this.toHashCode()]=null;Q.validateForm(P,this,R);return null;}
...
;}
;return S;}
,__vr:function(U){var T=false;if(!qx.lang.Type.isFunction(U)){T=qx.Class.isSubClassOf(U.constructor,qx.ui.form.validation.AsyncValidator);}
;return T;}
,__vs:function(V){var W=V.constructor;return qx.Class.hasInterface(W,qx.ui.form.IForm);}
,__vi:function(X){var Y=X.constructor;return qx.Class.hasInterface(Y,qx.ui.core.ISingleSelection);}
,__vt:function(ba){var bb=this.__vm;this.__vm=ba;if(bb!=ba){this.fireDataEvent(f,ba,bb);}
;}
,getValid:function(){return this.__vm;}
,isValid:function(){return this.getValid();}
,getInvalidMessages:function(){var bc=[];for(var i=0;i<this.__vk.length;i++ ){var bd=this.__vk[i].item;if(!bd.getValid()){bc.push(bd.getInvalidMessage());}
;}
;if(this.getInvalidMessage()!=c){bc.push(this.getInvalidMessage());}
;return bc;}
,getInvalidFormItems:function(){var be=[];for(var i=0;i<this.__vk.length;i++ ){var bf=this.__vk[i].item;if(!bf.getValid()){be.push(bf);}
;}
;return be;}
,reset:function(){for(var i=0;i<this.__vk.length;i++ ){var bg=this.__vk[i];bg.item.setValid(true);}
;this.__vm=null;}
,setItemValid:function(bi,bh){this.__vl[bi.toHashCode()]=bh;bi.setValid(bh);this.__vu();}
,setFormValid:function(bj){this.__vl[this.toHashCode()]=bj;this.__vu();}
,__vu:function(){var bl=this.__vn;for(var bm in this.__vl){var bk=this.__vl[bm];bl=bk&&bl;if(bk==null){return;}
;}
;this.__vt(bl);this.__vl={};this.fireEvent(h);}
...
,members:{__vv:null,__vw:null,__eT:null,__vx:null,validate:function(c,e,d,f){this.__vx=false;this.__vw=c;this.__eT=d;this.__vv.call(f||this,this,e);}
,validateForm:function(g,h,i){this.__vx=true;this.__eT=h;this.__vv.call(i,g,this);}
,setValid:function(k,j){if(this.__vx){if(j!==undefined){this.__eT.setInvalidMessage(j);}
;this.__eT.setFormValid(k);}
else {if(j!==undefined){this.__vw.setInvalidMessage(j);}
;this.__eT.setItemValid(this.__vw,k);}
;}
},destruct:function(){this.__eT=this.__vw=null;}
});}
)();
...
(function(){var a="The given item has not been added.",b="qx.ui.form.Resetter",c=" not supported for reseting.",d="Item ";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__nM=[];}
,members:{__nM:null,add:function(e){if(this._supportsValue(e)){var f=e.getValue();}
else if(this.__vi(e)){var f=e.getSelection();}
else if(this.__vj(e)){var f=e.getSelection().concat();}
else {throw new Error(d+e+c);}
;this.__nM.push({item:e,init:f});}
,reset:function(){for(var i=0;i<this.__nM.length;i++ ){var g=this.__nM[i];this.__vg(g.item,g.init);}
;}
,resetItem:function(h){var j;for(var i=0;i<this.__nM.length;i++ ){var k=this.__nM[i];if(k.item===h){j=k.init;break;}
;}
;if(j===undefined){throw new Error(a);}
;this.__vg(h,j);}
,__vg:function(l,m){if(this._supportsValue(l)){l.setValue(m);}
else if(this.__vi(l)||this.__vj(l)){l.setSelection(m);}
;}
,redefine:function(){for(var i=0;i<this.__nM.length;i++ ){var n=this.__nM[i].item;this.__nM[i].init=this.__vh(n);}
;}
,redefineItem:function(o){var p;for(var i=0;i<this.__nM.length;i++ ){if(this.__nM[i].item===o){p=this.__nM[i];break;}
;}
;if(p===undefined){throw new Error(a);}
;p.init=this.__vh(p.item);}
,__vh:function(q){if(this._supportsValue(q)){return q.getValue();}
else if(this.__vi(q)||this.__vj(q)){return q.getSelection();}
;}
,__vi:function(r){var s=r.constructor;return qx.Class.hasInterface(s,qx.ui.core.ISingleSelection);}
,__vj:function(t){var u=t.constructor;return qx.Class.hasInterface(u,qx.data.controller.ISelection);}
...
,properties:{wrap:{check:a,init:true,apply:d},appearance:{refine:true,init:i},singleStep:{check:h,init:20},minimalLineHeight:{check:h,apply:g,init:4},autoSize:{check:a,apply:w,init:false}},members:{__vD:null,__vE:null,__vF:null,setValue:function(M){M=qx.ui.form.AbstractField.prototype.setValue.call(this,M);this.__vG();return M;}
...
;}
,__vG:function(){if(this.isAutoSize()){var R=this.__vH();if(R&&this.getBounds()){this.__vF=this.__vF||this._getAreaHeight();var T=this._getScrolledAreaHeight();if(this.getMaxHeight()){var P=this.getInsets();var Q=-P.top+this.getMaxHeight()-P.bottom;if(T>Q){this.getContentElement().setStyle(H,j);}
else {this.getContentElement().setStyle(H,G);}
;}
;var S=Math.max(T,this.__vF);this._setAreaHeight(S);}
else {this.getContentElement().addListenerOnce(x,function(){this.__vG();}
,this);}
;}
;}
,_getAreaHeight:function(){return this.getInnerSize().height;}
,_setAreaHeight:function(U){if(this._getAreaHeight()!==U){this.__vE=U;qx.ui.core.queue.Layout.add(this);qx.ui.core.queue.Manager.flush();this.__vK();}
;}
,_getScrolledAreaHeight:function(){var W=this.__vH();var V=W.getDomElement();if(V){if(!V.parentNode){qx.html.Element.flush();return this._getScrolledAreaHeight();}
...
;this.__vJ(W);if(qx.core.Environment.get(C)==B&&qx.core.Environment.get(s)==8){if(!V.scrollTop){qx.html.Element.flush();}
;}
;return V.scrollTop;}
;}
,__vH:function(){this.__vD=this.__vD||this.__vI();return this.__vD;}
,__vI:function(){var ba,Y,bb,X;ba=this.getContentElement();if(!ba.getDomElement()){return null;}
;bb=qx.bom.Element.clone(ba.getDomElement());X=new qx.html.Input(i);X.useElement(bb);Y=X;Y.setStyles({position:K,top:0,left:n,height:0,overflow:G},true);Y.removeAttribute(F);Y.removeAttribute(z);Y.setAttribute(y,f);Y.setValue(ba.getValue()||t);Y.insertBefore(ba);this.__vJ(Y);return Y;}
,__vJ:function(bc){bc=bc.getDomElement();if(bc){bc.scrollTop=10000;}
;}
...
,_applyAutoSize:function(bh,bg){if(qx.core.Environment.get(J)){this.__vL();}
;if(bh){this.__vG();this.addListener(o,this.__vG,this);if(!this.getBounds()){this.addListenerOnce(x,function(){this.getContentElement().setStyle(H,G);}
);}
else {this.getContentElement().setStyle(H,G);}
;}
else {this.removeListener(o,this.__vG);this.getContentElement().setStyle(H,j);}
;}
,_applyDimension:function(bi){qx.ui.form.AbstractField.prototype._applyDimension.call(this);if(qx.core.Environment.get(J)){this.__vL();}
;if(bi===this.getMaxHeight()){this.__vG();}
;}
,__vK:function(){var content=this.getContentElement();var bk=content.getDomElement();var bj=content.getStyle(u);content.setStyle(u,parseInt(bj,10)+1000+b,true);if(bk){qx.bom.element.Dimension.getWidth(bk);}
;content.setStyle(u,bj,true);}
,__vL:function(){if(this.isAutoSize()&&this.getHeight()){this.warn(m+I+v);}
;}
,_getContentHint:function(){var bl=qx.ui.form.AbstractField.prototype._getContentHint.call(this);bl.height=bl.height*this.getMinimalLineHeight();bl.width=this._getTextSize().width*20;if(this.isAutoSize()){bl.height=this.__vE||bl.height;}
;return bl;}
},destruct:function(){this.setAutoSize(false);if(this.__vD){this.__vD.dispose();}
;}
});}
)();
