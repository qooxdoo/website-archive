...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.c6109e35a955.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b=" => ",c="#autoActionsBeforeOnexit",d=": Could not find friendly name for '",f=": event '",g="__IU",h=" blocked.  Re-queuing.",j="string",k="'",l="Attempt to transition to nonexistent state ",m="Saved-state stack is empty",n="Internal error: invalid nextState",o=" not handled.  Ignoring.",p="State ",q="#autoActionsBeforeOntransition",r=" returned a value other than ",s="#",u=" already exists",v="Attempt to transition to POP_STATE_STACK ",w=":",x="Transition ",y=" does not exist",z="' on '",A="qx.util.fsm.FiniteStateMachine",B="String",C="#autoActionsAfterOntransition",D="",E="#actionsAfterOnentry",F="Saved-state stack is full",G="#autoActionsBeforeOnentry",H=": no transition found.  No state change.",I="#autoActionsAfterOnentry",J="#actionsBeforeOnentry",K=": Pushed event: ",L="__IT",M="Machine started with no available states",N=": Process event: ",O="#entry",P="#ontransition",Q=" on ",R="Invalid state: not an instance of ",S="Number",T="#exit",U="true, false, or null.",V="Explicit transition ",W="  ",X="qx.util.fsm.State",Y="while state stack is empty.",bj=".  Ignoring.",bk="#autoActionsAfterOnexit",bl=": Event '",bg="object",bh=" not handled for target ",bi=": Queued event: ";qx.Class.define(A,{extend:qx.core.Object,construct:function(bm){qx.core.Object.call(this);this.setName(bm);this.__iI={};this.__IR=null;this.__IS=[];this.__IT=[];this.__IU=[];this.__IV={};this.__IW={};this.__IX={};this.__IY={};this.__Ja={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:B,nullable:true},state:{check:B,nullable:true},previousState:{check:B,nullable:true},nextState:{check:B,nullable:true},maxSavedStates:{check:S,init:2},debugFlags:{check:S,init:7}},members:{__iI:null,__IR:null,__IT:null,__IU:null,__IS:null,__IV:null,__IW:null,__IX:null,__IY:null,__Ja:null,__Jb:false,addState:function(bn){if(!bn instanceof qx.util.fsm.State){throw new Error(R+X);}
;var bo=bn.getName();if(bo in this.__iI){throw new Error(p+bo+u);}
;if(this.__IR==null){this.__IR=bo;}
;this.__iI[bo]=bn;}
,replaceState:function(bs,br){if(!bs instanceof qx.util.fsm.State){throw new Error(R+X);}
;var bq=bs.getName();var bp=this.__iI[bq];this.__iI[bq]=bs;if(br){bp._bNeedDispose=true;}
;return bp;}
,addObject:function(bw,bt,bu){var bx=bt.toHashCode();this.__IW[bw]=bx;this.__IX[bx]=bw;this.__IV[bw]=bt;if(!bu){return;}
;if(typeof (bu)==j){bu=[bu];}
;for(var i=0;i<bu.length;i++ ){var bv=bu[i];if(!this.__IY[bv]){this.__IY[bv]={};}
;this.__IY[bv][bw]=true;if(!this.__Ja[bw]){this.__Ja[bw]=[];}
;this.__Ja[bw].push(bv);}
;}
,removeObject:function(bB){var by;var bz;var bA;var bC;by=this.__IW[bB];if(this.__Ja[bB]){for(var i=0;i<this.__Ja[bB].length;i++ ){bz=this.__Ja[bB][i];delete this.__IY[bz][bB];bC=true;for(bA in this.__IY[bz]){bC=false;break;}
;if(bC){delete this.__IY[bz];}
;}
;delete this.__Ja[bB];}
;delete this.__IX[by];delete this.__IW[bB];delete this.__IV[bB];}
,getObject:function(bD){return this.__IV[bD];}
,getFriendlyName:function(bE){var bF=bE?bE.toHashCode():null;return bF?this.__IX[bF]:null;}
,getGroupObjects:function(bG){var a=[];for(var name in this.__IY[bG]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bH in this.__IW){var bJ=this.__IW[bH];var bI=this.getObject(bH);this.debug(bH+b+bJ);this.debug(W+bJ+b+this.__IX[bJ]);this.debug(W+bH+b+this.getObject(bH));this.debug(W+this.getObject(bH)+b+this.getFriendlyName(bI));}
;}
,_getInternalData:function(){return ({"states":this.__iI,"startState":this.__IR,"eventQueue":this.__IT,"blockedEvents":this.__IU,"savedStates":this.__IS,"friendlyToObject":this.__IV,"friendlyToHash":this.__IW,"hashToFriendly":this.__IX,"groupToFriendly":this.__IY,"friendlyToGroups":this.__Ja});}
,start:function(){var bL=this.__IR;if(bL==null){throw new Error(M);}
...
;this.__iI[bL].getAutoActionsBeforeOnentry()(this);if(bK){this.debug(this.getName()+s+bL+O);}
;this.__iI[bL].getOnentry()(this,null);if(bK){this.debug(this.getName()+s+bL+E);}
;this.__iI[bL].getAutoActionsAfterOnentry()(this);}
,pushState:function(bM){if(this.__IS.length>=this.getMaxSavedStates()){throw new Error(F);}
;if(bM===true){this.__IS.push(this.getState());}
else if(bM){this.__IS.push(bM);}
else {this.__IS.push(this.getPreviousState());}
;}
,popState:function(){if(this.__IS.length==0){throw new Error(m);}
;return this.__IS.pop();}
,postponeEvent:function(event){this.__IU.unshift(event);}
,enqueueEvent:function(event,bN){if(bN){this.__IT.push(event);}
else {this.__IT.unshift(event);}
...
;}
;}
,eventListener:function(event){var e=event.clone();this.enqueueEvent(e,false);this.__Jc();}
...
,this,bV||20);}
,__Jc:function(){if(this.__Jb){return;}
;this.__Jb=true;while(this.__IT.length>0){var event=this.__IT.pop();var bW=this.__Jd(event);if(bW){event.dispose();}
;}
;this.__Jb=false;}
,__Jd:function(event){var ca;var cb;var bY;var cd;var cg;var e;var cf;var bX=this.getDebugFlags();var ck=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var ci=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var ch=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var ce=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var cj=this.getFriendlyName(event.getTarget());if(ck){this.debug(this.getName()+N+event.getType()+(cj?Q+cj:D));}
;ca=this.getState();cd=this.__iI[ca];cg=cd.transitions;e=cd.getEvents()[event.getType()];if(!e){if(ck){this.debug(this.getName()+bl+event.getType()+k+o);}
;return true;}
;if(typeof (e)==bg){if(!cj){if(ce){this.debug(this.getName()+d+event.getType()+z+event.getTarget()+k);}
;return true;}
...
;this.__IU.unshift(event);return false;default:if(typeof (cf)==j){if(cg[cf]){var cc=cg[cf];cg={};cg[cf]=cc;}
else {throw new Error(V+cf+y);}
;break;}
;};for(var t in cg){var cc=cg[t];switch(cc.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(x+ca+w+t+r+U);};cb=cc.getNextState();if(typeof (cb)==j){if(!cb in this.__iI){throw new Error(l+cb);}
;this.setNextState(cb);}
else {switch(cb){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cb=ca;this.setNextState(cb);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__IS.length==0){throw new Error(v+Y);}
;cb=this.__IS.pop();this.setNextState(cb);break;default:throw new Error(n);break;};}
...
;cd=this.__iI[this.getNextState()];this.setPreviousState(ca);this.setState(this.getNextState());this.setNextState(null);bY=ca;ca=cb;cb=undefined;if(ch){this.debug(this.getName()+s+ca+G);}
...
;cd.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__IU.length;i++ ){e=this.__IU.pop();this.__IT.unshift(e);}
;if(ci){this.debug(this.getName()+s+bY+b+this.getName()+s+ca);}
;return true;}
;if(ci){this.debug(this.getName()+s+ca+f+event.getType()+k+H);}
;return true;}
},destruct:function(){this._disposeArray(L);this._disposeArray(g);this.__IS=this.__iI=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="__IM",h="  var groupObjects = ",k="parameters",l=")",m=" fsm.getObject('",n="groups",o="Invalid function parameters: ",p="Invalid onexit type: ",q="  for (var i = 0; i < groupObjects.length; i++)",r="__IN",s="context",t="string",u="autoActionsAfterOnentry",v="Invalid onentry type: ",w='"',x="Invalid numeric value in events object: ",y="Invalid state name",z=" value: ",A="');",B="__IP",C="State ",D="autoActionsBeforeOnexit",E="autoActionsBeforeOnentry",F="}",G="): ",H="events must be an object",I="onexit",J="{",K="Invalid function request type: ",L="__IK",M=": ",N="__IJ",O="expected array, found ",P="Invalid friendly name in 'objects' list: ",Q="autoActionsAfterOnexit",R="  fsm.debug(ex);",S="(",T="__II",U="    fsm.getObject(objName).",V=")(fsm)",W="events",X="undefined",Y="Invalid ",ba="try",bb="    var objName = groupObjects[i];",bc="Invalid value in events object: ",bd="').",be="objects",bf="expected object, found ",bg="qx.util.fsm.Transition",bh="The events object must be provided in new state info",bi="catch(ex)",bj="  {",bk="    fsm.getGroupObjects('",bl="Invalid value in events object ",bm="Invalid transition: not an instance of ",bn="  }",bo="Invalid numeric value in events object ",bp="Invalid 'objects' list: expected array, got ",bq="__IL",br="qx.util.fsm.State",bs="onentry",bt="__IO",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(br,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(s,bE);for(var bC in bF){switch(bC){case bs:this.setOnentry(this.__IQ(bF[bC],bE));break;case I:this.setOnexit(this.__IQ(bF[bC],bE));break;case E:this.setAutoActionsBeforeOnentry(bF[bC]);break;case u:this.setAutoActionsAfterOnentry(bF[bC]);break;case D:this.setAutoActionsBeforeOnexit(bF[bC]);break;case Q:this.setAutoActionsAfterOnexit(bF[bC]);break;case W:this.setEvents(bF[bC]);break;case s:break;default:this.setUserData(bC,bF[bC]);this.debug(C+bD+M+bz+bC);break;};}
...
;var funcFragment;var func=ba+J;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(K+O+typeof (functionRequest));}
...
else {if(!params instanceof Array){throw new Error(o+O+typeof (params));}
;}
;funcFragment=f+S;for(var j=0;j<params.length;j++ ){if(j!=0){funcFragment+=by;}
;if(typeof (params[j])==c){funcFragment+=S+params[j]+V;}
else if(typeof (params[j])==t){funcFragment+=w+params[j]+w;}
else {funcFragment+=params[j];}
;}
;funcFragment+=l;var a=objectAndGroupList[be];if(!a){a=[];}
else if(!a instanceof Array){throw new Error(bp+typeof (a));}
;for(var j=0;j<a.length;j++ ){if(typeof (a[j])!=t){throw new Error(P+a[j]);}
;func+=m+a[j]+bd+funcFragment+bu;}
;var g=objectAndGroupList[n];if(g){if(!g instanceof Array){throw new Error(bw+typeof (g));}
;for(j=0;j<g.length;j++ ){func+=h+bk+g[j]+A+q+bj+bb+U+funcFragment+bu+bn;}
;}
;}
;}
;func+=F+bi+J+R+F;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:T,nullable:true},onentry:{transform:N,nullable:true,init:function(bG,event){}
},onexit:{transform:L,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:d,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:r,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:bt,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:B,nullable:true,init:function(bL,event){}
},events:{transform:bq,nullable:true}},members:{__II:function(bM){if(typeof (bM)!=t||bM.length<1){throw new Error(y);}
;return bM;}
,__IJ:function(bN){switch(typeof (bN)){case X:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(s));default:throw new Error(v+typeof (bN));};}
,__IK:function(bP){switch(typeof (bP)){case X:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(s));default:throw new Error(p+typeof (bP));return null;};}
,__IL:function(bT){if(typeof (bT)!=bB){throw new Error(H);}
...
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==bA&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(bo+S+e+G+bS+M+bR[bS]);}
else if(typeof (bR[bS])!=t&&typeof (bR[bS])!=bA){throw new Error(bl+S+e+G+bS+M+bR[bS]);}
;}
;}
else if(typeof (bR)!=t&&typeof (bR)!=bA){throw new Error(bc+e+M+bT[e]);}
;}
;return bT;}
,__IM:function(bU){return qx.util.fsm.State._commonTransformAutoActions(E,bU,this.getUserData(s));}
,__IN:function(bV){return qx.util.fsm.State._commonTransformAutoActions(u,bV,this.getUserData(s));}
,__IO:function(bW){return qx.util.fsm.State._commonTransformAutoActions(D,bW,this.getUserData(s));}
,__IP:function(bX){return qx.util.fsm.State._commonTransformAutoActions(Q,bX,this.getUserData(s));}
,__IQ:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bm+bg);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="__Jf",c="or one of the Fsm.StateChange constants",d="autoActionsAfterOntransition",e="__Jh",g="string",h="predicate",i="__Ji",j="autoActionsBeforeOntransition",k="nextState must be an explicit state name, ",l="Invalid transition predicate type: ",m="Transition ",n=": ",o="String",p="Invalid transition nextState type: ",q="number",r="__Jg",s="__Je",t="Adding user-provided field to transition: ",u="ontransition",v="context",w="boolean",x="nextState",y="Invalid transition nextState value: ",z="Invalid ontransition type: ",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(v,E);for(var C in D){switch(C){case h:this.setPredicate(this.__IQ(D[C],E));break;case x:this.setNextState(D[C]);break;case j:this.setAutoActionsBeforeOntransition(this.__IQ(D[C],E));break;case d:this.setAutoActionsAfterOntransition(this.__IQ(D[C],E));break;case u:this.setOntransition(this.__IQ(D[C],E));break;case v:break;default:this.setUserData(C,D[C]);this.debug(m+F+n+t+C);break;};}
;}
,properties:{name:{check:o,nullable:true},predicate:{init:function(G,event){return true;}
,transform:s},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:b},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:r},autoActionsAfterOntransition:{init:function(I,event){}
,transform:e},ontransition:{init:function(J,event){}
,transform:i}},members:{__Je:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
;case w:return function(M,event){return K;}
;case a:return qx.lang.Function.bind(K,this.getUserData(v));default:throw new Error(l+typeof (K));break;};}
,__Jf:function(N){switch(typeof (N)){case g:return N;case q:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(y+N+n+k+c);};break;default:throw new Error(p+typeof (N));break;};}
,__Jg:function(O){return qx.util.fsm.State._commonTransformAutoActions(j,O,this.getUserData(v));}
,__Jh:function(P){return qx.util.fsm.State._commonTransformAutoActions(d,P,this.getUserData(v));}
,__Ji:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(v));default:throw new Error(z+typeof (Q));break;};}
,__IQ:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
