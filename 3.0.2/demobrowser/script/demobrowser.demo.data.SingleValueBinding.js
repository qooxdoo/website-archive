...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SingleValueBinding.b55082c20538.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="demobrowser.demo.data.SingleValueBinding",b="Reverse",c="Binding a number value (slider value) to a labels string (content).",d="child.names[last]",f="child.names[0]",g="First:",h="changeValue",i="Bigger than 50!",j="of names.<br/>The reverse button reverses the current selected child ",k="execute",l=" The default conversion will handle the type conversion.",m="Binding an data event, containing the change of the textfield to",n="Last:",o="Binding the textfield to a property with a validation. The validator",p=" in the object only accepts numbers. On a validation fail, a red ",q=" the label.",r="value",s="No number!",t="With validation",u="10",v="containing ['Bart', 'Lisa'] as names.",w=" above or below 50.",x="Bart",y="",z="number",A="Toggle child",B="from a node containing ['Homer', 'Marge'] as names to a node ",C="Binding of node elements containing a node as child and an array ",D="With own conversion",E="names array.<br/>Toggle child will change the child of the root node ",F="with a self written converter to a string only saying it its ",G="Event binding",H="Array binding",I="Lower than 50!",J="Lisa",K="Change me...",L="Binding a slider value to a labels text but conversion the number ",M="With default conversion",N="<b>Description</b><br/>",O=" border will surround the textfield.";qx.Class.define(a,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);this.getRoot().add(new qx.ui.basic.Label(G),{top:10,left:10});var bc=new qx.ui.form.TextField(K);bc.setLiveUpdate(true);this.getRoot().add(bc,{top:30,left:10});var V=new qx.ui.basic.Label(K);this.getRoot().add(V,{top:32,left:120});bc.bind(h,V,r);this.getRoot().add(new qx.ui.basic.Label(M),{top:80,left:10});var bb=new qx.ui.form.Slider();bb.setWidth(100);this.getRoot().add(bb,{top:100,left:10});var Y=new qx.ui.basic.Label(y);this.getRoot().add(Y,{top:100,left:120});bb.bind(r,Y,r);this.getRoot().add(new qx.ui.basic.Label(D),{top:150,left:10});var S=new qx.ui.form.Slider();S.setWidth(100);this.getRoot().add(S,{top:180,left:10});var bi=new qx.ui.basic.Label(y);this.getRoot().add(bi,{top:180,left:120});var bh={converter:function(bl){if(bl<50){return I;}
else {return i;}
;}
};S.bind(r,bi,r,bh);this.getRoot().add(new qx.ui.basic.Label(H),{top:240,left:10});var ba=new demobrowser.demo.data.model.Node();var be=new demobrowser.demo.data.model.Node();var bg=new demobrowser.demo.data.model.Node();bg.setNames(new qx.data.Array(x,J));ba.setChild(be);var bd=new qx.ui.form.Button(b);this.getRoot().add(bd,{top:260,left:10});bd.addListener(k,function(){ba.getChild().getNames().reverse();}
,this);var bf=new qx.ui.form.Button(A);this.getRoot().add(bf,{top:260,left:80});bf.addListener(k,function(){if(ba.getChild()==be){ba.setChild(bg);}
else {ba.setChild(be);}
;}
,this);this.getRoot().add(new qx.ui.basic.Label(g),{left:10,top:295});this.getRoot().add(new qx.ui.basic.Label(n),{left:60,top:295});var bj=new qx.ui.basic.Label();this.getRoot().add(bj,{top:310,left:10});var Q=new qx.ui.basic.Label();this.getRoot().add(Q,{top:310,left:60});ba.bind(f,bj,r);ba.bind(d,Q,r);this.getRoot().add(new qx.ui.basic.Label(t),{top:390,left:10});var bk=new qx.ui.form.TextField(u);bk.setLiveUpdate(true);this.getRoot().add(bk,{top:410,left:10});var X=new qx.ui.basic.Label(y);this.getRoot().add(X,{top:412,left:120});var bh={onUpdate:function(){X.setValue(y);bk.setValid(true);}
,onSetFail:function(e){X.setValue(s);bk.setValid(false);}
};bk.bind(h,ba,z,bh);var R=new qx.ui.basic.Label();R.setRich(true);R.setWidth(450);R.setValue(N+m+q);this.getRoot().add(R,{left:250,top:10});var U=new qx.ui.basic.Label();U.setRich(true);U.setWidth(450);U.setValue(N+c+l);this.getRoot().add(U,{left:250,top:80});var P=new qx.ui.basic.Label();P.setRich(true);P.setWidth(450);P.setValue(N+L+F+w);this.getRoot().add(P,{left:250,top:150});var W=new qx.ui.basic.Label();W.setRich(true);W.setWidth(450);W.setValue(N+C+j+E+B+v);this.getRoot().add(W,{left:250,top:240});var T=new qx.ui.basic.Label();T.setRich(true);T.setWidth(450);T.setValue(N+o+p+O);this.getRoot().add(T,{left:250,top:390});}
}});}
)();
...
(function(){var a="changeNames",b="changeName",c="Validation Error: ",d="- ",e="",f="changeChild",g="qx.data.Array",h="changeColor",j="Marge",k="AFFE",l="is no number (parseFloat says so!).",m="__Rx",n="Homer",o="\n",p="demobrowser.demo.data.model.Node",q="String",r="changeName2",s="changeChildren";qx.Class.define(p,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.setChildren(new qx.data.Array());}
,properties:{children:{check:g,event:s},child:{check:p,event:f,nullable:true},names:{check:g,event:a,init:new qx.data.Array(n,j)},name:{check:q,init:k,event:b},name2:{check:q,event:r},number:{init:10,validate:m},color:{event:h,nullable:true}},members:{__Rx:function(t){if(!isNaN(parseFloat(t))){if(/^\d+$/.test(t)){return;}
;}
