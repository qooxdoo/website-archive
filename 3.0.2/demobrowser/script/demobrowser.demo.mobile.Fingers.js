...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.mobile.Fingers.b9a7f55b3719.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;this.__RG=[];this.__RH=[];J.addListener(s,this._onTouchStart,this);J.addListener(a,this._onTouchMove,this);}
,_onTouchStart:function(e){var L=e.getAllTouches();for(var i=0;i<L.length;i++ ){var K=L[i];this.__RG[i]=parseInt(K.target.style.left)-K.pageX;this.__RH[i]=parseInt(K.target.style.top)-K.pageY;}
;}
,_onTouchMove:function(e){var N=e.getAllTouches();for(var i=0;i<N.length;i++ ){var M=N[i];if(N.target==document.body){continue;}
;qx.bom.element.Style.setStyles(M.target,{"left":(M.pageX+this.__RG[i])+c,"top":(M.pageY+this.__RH[i])+c});}
;e.stop();}
}});}
)();
