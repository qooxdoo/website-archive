...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Flash.4176c21f7293.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;}
;}
;var N=this.__HS(O,Q,P,L);this._flashObjects[Q.id]=N;return N;}
,destroy:qx.core.Environment.select(C,{"mshtml":function(S,R){S=this.__HN(S);if(S.readyState==4){this.__HO(S);}
else {if(!R){R=window;}
;qx.bom.Event.addNativeListener(R,p,function(){qx.bom.Flash.__HO(S);}
);}
;}
,"default":function(U,T){U=this.__HN(U);if(U.parentNode){U.parentNode.removeChild(U);}
;delete this._flashObjects[U.id];}
}),__HN:function(V){if(!V){throw new Error(y);}
;if(V.tagName.toLowerCase()!==K){V=V.firstChild;}
;if(!V||V.tagName.toLowerCase()!==K){throw new Error(z);}
;return V;}
,__HO:qx.core.Environment.select(C,{"mshtml":qx.event.GlobalError.observeMethod(function(W){for(var i in W){if(typeof W[i]==b){W[i]=null;}
;}
;if(W.parentNode){W.parentNode.removeChild(W);}
;delete this._flashObjects[W.id];}
),"default":null}),__HP:qx.event.GlobalError.observeMethod(function(){for(var X in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[X]);}
;window.__HQ=function(){}
;window.__HR=function(){}
;qx.bom.Event.removeNativeListener(window,E,qx.bom.Flash.__HP);}
),__HS:qx.core.Environment.select(C,{"mshtml":function(bb,bd,bc,Y){bc.movie=bd.data;delete bd.data;delete bd.classid;var ba=t;for(name in bc){ba+=m+name+I+bc[name]+D;}
...
})},defer:function(bk){if(qx.core.Environment.get(C)==u){qx.bom.Event.addNativeListener(window,E,bk.__HP);}
;}
});}
)();
