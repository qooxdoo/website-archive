...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.showcase.Translation.b344cc1c43ef.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="MessageId:",b="<b>Locale and language:</b>",c="new Locale must not be empty string",d="Choose Locale:",f="The quick brown fox",g='Territory: ',h=" already exists",j="en",k="Simple Translate: this.tr('Hello World')",l='Locale: ',m="Orange",n="Add new Locale<br> at runtime",o="en_QX",p="execute",q="A comment: 'The mammal fox that is.'<br>is added in the .po file",r="Translation",s=" has been added for messageId: ",t="Singular/Plural Translate: this.trn('I have an apple', I have %1 apples, ",u="changeLocale",v="MessageId",w="qx.debug",x="The mammal fox that is.",y="Number of apples",z="show Translation",A="Hello World",B="value",C="Locale Name (ex: en_QX)",D='bottom',E="changeSelection",F="Mark the string 'Orange' for later<br>translation so we do not forget it",G='top',H="I have an apple",I="",J="demo",K="Add translation<br>at runtime for: <b>",L="Add new Locale",M="</b>",N='Territory',O="I have %1 apples",P="Simple messageId translation",Q=" for locale: ",R="Mark the string 'Orange' for later translation so we do not forget it: this.marktr('Orange')",S=", applesNumber)",T=" has been added",U="<b>Dynamic translation. This is added at runtime.</b>",V="<b>Static translation [found in .po files]. Hover the labels below to see more info.</b>",W="Add Translation",X='Language',Y='middle',bo="Singular/Plural Translate<br>Choose a number from the select box<br>to see plural translation",bp="with .po comment: this.trc('The mammal fox that is.', 'The quick brown fox')",bq='Language: ',bk="<i>MessageId</i>",bl="Number of apples:",bm='right',bn='Locale',br="click 'Show Translation' button to translate<br>the messageId in the current selected locale",bs="<i>Translated Text</i>",bt="demobrowser.demo.showcase.Translation";qx.Class.define(bt,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);if((qx.core.Environment.get(w))){qx.log.appender.Native;qx.log.appender.Console;}
;var bJ=new qx.ui.container.Composite(new qx.ui.layout.Grid(8,8));this.getRoot().add(bJ,{top:20,left:20});var bP=new qx.ui.basic.Label(b);bP.setRich(true);bJ.add(bP,{row:0,column:0,colSpan:3});var bR=new qx.ui.basic.Label(d);bR.setMarginTop(5);bR.setAlignY(Y);bJ.add(bR,{row:1,column:0});var cj=qx.locale.Manager.getInstance();var cb=cj.getAvailableLocales().sort();var bH=cj.getLocale();var bG=new qx.ui.form.SelectBox();bG.setAllowGrowY(false);bG.setAlignY(Y);var bQ=null;for(var i=0;i<cb.length;i++ ){var bX=new qx.ui.form.ListItem(cb[i]);bG.add(bX);if((!bQ&&cb[i]==j)||cb[i]==bH){bQ=bX;}
;}
;bG.addListener(E,function(e){var cn=e.getData()[0].getLabel();qx.locale.Manager.getInstance().setLocale(cn);}
);bJ.add(bG,{row:1,column:1});var ck=new qx.ui.basic.Label(bn);ck.setMarginTop(5);ck.setWidth(80);var ch=new qx.ui.basic.Label(N);ch.setMarginTop(5);ch.setWidth(80);var ci=new qx.ui.basic.Label(X);ci.setMarginTop(5);ci.setWidth(80);var bB=new qx.ui.container.Composite(new qx.ui.layout.VBox(3));bB.add(ck);bB.add(ch);bB.add(ci);bJ.add(bB,{row:1,column:2});qx.locale.Manager.getInstance().addListener(u,function(){var co=qx.locale.Manager.getInstance();ck.setValue(l+co.getLocale());ch.setValue(g+co.getTerritory());ci.setValue(bq+co.getLanguage());bu.apply(this);cm.setValue(K+cj.getLocale()+M);}
,this);function bu(){bD.setValue(this.tr(A));bD.setToolTipText(k);var cp=parseInt(cf.getSelection()[0].getLabel());bC.setValue(this.trn(H,O,cp,cp));bC.setToolTipText(t+cp+S);bT.setValue(this.trc(x,f));bT.setToolTipText(bp);bA.setValue(this.marktr(m));bA.setToolTipText(R);var cq=cd.getValue();if(cq&&cq.length>0){bS.setValue(cj.translate(cq,[],cj.getLocale()));}
;}
;var ce=new qx.ui.basic.Label(V);ce.setRich(true);ce.setMarginTop(15);bJ.add(ce,{row:2,column:0,colSpan:4});var bv=new qx.ui.basic.Label(bk);bv.setRich(true);bJ.add(bv,{row:3,column:1});var bN=new qx.ui.basic.Label(bs);bN.setRich(true);bJ.add(bN,{row:3,column:2});var bV=new qx.ui.basic.Label(A);bJ.add(bV,{row:4,column:1});var bD=new qx.ui.basic.Label(this.tr(A));bJ.add(bD,{row:4,column:2});var cg=new qx.ui.basic.Label(P);bJ.add(cg,{row:4,column:3});var bF=new qx.ui.basic.Label(bl);bF.setAlignX(bm);bF.setAlignY(D);bJ.add(bF,{row:5,column:0});var cf=new qx.ui.form.SelectBox();for(var i=1;i<5;i++ ){var bE=new qx.ui.form.ListItem(i+I);cf.add(bE);}
;cf.setToolTipText(y);cf.setAlignY(G);cf.setAlignX(bm);bJ.add(cf,{row:6,column:0});var bw=new qx.ui.basic.Label(H);bw.setAlignY(Y);bJ.add(bw,{row:5,column:1,rowSpan:2});var bC=new qx.ui.basic.Label(this.trn(H,O,1,1));bC.setAlignY(Y);bJ.add(bC,{row:5,column:2,rowSpan:2});var ca=new qx.ui.basic.Label(bo);ca.setAlignY(Y);ca.setRich(true);bJ.add(ca,{row:5,column:3,rowSpan:2});cf.addListener(E,function(e){bu.apply(this);}
);var bI=new qx.ui.basic.Label(f);bJ.add(bI,{row:7,column:1});var bT=new qx.ui.basic.Label(this.trc(x,f));bT.setWidth(140);bJ.add(bT,{row:7,column:2});var bW=new qx.ui.basic.Label(q);bW.setRich(true);bJ.add(bW,{row:7,column:3});var bL=new qx.ui.basic.Label(m);bJ.add(bL,{row:8,column:1});var bA=new qx.ui.basic.Label(this.marktr(m));bJ.add(bA,{row:8,column:2});var bK=new qx.ui.basic.Label(F);bK.setRich(true);bJ.add(bK,{row:8,column:3});var by=new qx.ui.basic.Label(U);by.setRich(true);by.setMarginTop(15);bJ.add(by,{row:9,column:0,colSpan:3});var cm=new qx.ui.basic.Label(K+cj.getLocale()+M);cm.setRich(true);cm.setWidth(120);bJ.add(cm,{row:10,column:0});var bx=new qx.ui.form.TextField(I);bx.setToolTipText(v);bJ.add(bx,{row:10,column:1});var bU=new qx.ui.form.TextField(I);bU.setToolTipText(r);bJ.add(bU,{row:10,column:2});var bM=new qx.ui.form.Button(W);bM.setAllowGrowY(false);bJ.add(bM,{row:10,column:3});bM.addListener(p,function(e){var cr={};cr[bx.getValue()]=bU.getValue();cj.addTranslation(cj.getLocale(),cr);alert(bU.getValue()+s+bx.getValue()+Q+cj.getLocale());}
