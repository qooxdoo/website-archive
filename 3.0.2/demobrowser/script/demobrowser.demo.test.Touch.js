...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Touch.142f01162d1c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;bd.add(this.__Sq(10,10,k,W));bd.add(this.__Sq(110,10,Y,b));this.__So=this.__Sr(10,110);bd.add(this.__So);this.__Sp=this.__Sr(110,110);bd.add(this.__Sp);bd.add(this.__Ss(210,10));}
,__Sq:function(x,y,bh,bi){var bf={"position":X,"left":x+d,"top":y+d,"width":T,"height":T,"backgroundColor":bh,"fontFamily":J,"fontSize":s};var bg=new qx.html.Element(l,bf,{"id":bi});bg.setAttribute(D,bb+bi+P);bg.addListener(F,this.__St,this);bg.addListener(a,this.__St,this);bg.addListener(A,this.__St,this);bg.addListener(S,this.__St,this);return bg;}
,__Sr:function(x,y){var bj={"position":X,"left":x+d,"top":y+d,"width":T,"fontFamily":J,"fontSize":I};return new qx.html.Element(l,bj);}
,__Ss:function(x,y){var bk={"position":X,"left":x+d,"top":y+d,"width":n,"height":n,"backgroundColor":O,"fontFamily":J,"fontSize":s};var bl=new qx.html.Element(l,bk);bl.setAttribute(D,h);bl.addListener(L,function(){window.setTimeout(function(){alert(w);}
,0);}
,this);return bl;}
,__St:function(e){var bm=[];bm.push(M+e.getType());bm.push(f);bm.push(j+e.isMultiTouch());bm.push(t+e.getAllTouches().length);bm.push(c+e.getTargetTouches().length);bm.push(g+e.getChangedTargetTouches().length);bm.push(f);bm.push(z+e.getDocumentLeft()+r+e.getDocumentTop());bm.push(m+e.getScreenLeft()+r+e.getScreenTop());bm.push(B+e.getViewportLeft()+r+e.getViewportTop());bm.push(q+e.getIdentifier());var bo=e.getTarget();var bn=false;var bq=e.getTargetTouches().length>0?e.getTargetTouches():e.getChangedTargetTouches();for(var i=0;i<bq.length;i++ ){bn=bq[0].target==bo;if(!bn){break;}
;}
