...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.SettingsList.5514c47dd28d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="name",b="group.checked",c="#ddd",d="Fruits",f="label",g="Apple",h="Carrot",j="Vegetables",k="demobrowser.demo.virtual.SettingsList",l="value",m="changeValue",n="Orange",o="Potato",p="checked";qx.Class.define(k,{extend:qx.application.Standalone,members:{__cL:null,main:function(){qx.application.Standalone.prototype.main.call(this);var q=[];var s=[g,n,o,h];for(var i=0;i<s.length;i++ ){q[i]={name:s[i],checked:i<2,group:null};}
;this.__cL=qx.data.marshal.Json.createModel(q,true);var t=[];for(var i=0;i<2;i++ ){t[i]={name:null,checked:false};}
;t=qx.data.marshal.Json.createModel(t,true);t.getItem(0).setName(d);t.getItem(1).setName(j);this.__cL.toArray().forEach(function(u,v){v=v<2?0:1;u.setGroup(t.getItem(v));}
);var r=new qx.ui.list.List();r.setWidth(150);this.getRoot().add(r,{left:20,top:20});r.setDelegate(this);r.setModel(this.__cL);}
...
;var y=e.getData();this.__cL.forEach(function(z){if(x.getLabel()==z.getGroup().getName()){z.setChecked(y);}
;}
);}
,this);}
,bindItem:function(B,A,D){B.bindProperty(a,f,null,A,D);B.bindProperty(p,l,null,A,D);B.bindPropertyReverse(p,l,null,A,D);var C=this.__cL;B.bindPropertyReverse(b,l,{converter:function(H,J){var I=J.getItem(D);var F=I.getGroup();var G=C.toArray().filter(function(L){return L.getGroup()==F;}
...
(function(){var c="changeModel",d="resize",f="_applyLabelOptions",g="_applyLabelPath",h="qx.data.Array",j="_applyGroupLabelOptions",m="changeLength",n="Integer",o="_applyModel",p="changeGroups",q="Boolean",r="GroupingTypeError: You can't mix 'Objects' and 'Strings' as",s="sorter",t="row-layer",u="group",v="_applyIconPath",w="_applyDelegate",x="changeDelegate",y=" group identifier!",z="???",A="_applyRowHeight",B="qx.data.IListData",C="filter",D="virtual-list",E="String",F="_applyGroupLabelPath",G="_applyIconOptions",H="qx.ui.list.List";qx.Class.define(H,{extend:qx.ui.virtual.core.Scroller,include:[qx.ui.virtual.selection.MModel],implement:qx.data.controller.ISelection,construct:function(I){qx.ui.virtual.core.Scroller.call(this,0,1,20,100);this._init();this.__yi=new qx.data.Array();this.initGroups(this.__yi);if(I!=null){this.initModel(I);}
;this.initItemHeight();}
,properties:{appearance:{refine:true,init:D},focusable:{refine:true,init:true},width:{refine:true,init:100},height:{refine:true,init:200},model:{check:B,apply:o,event:c,nullable:true,deferredInit:true},itemHeight:{check:n,init:25,apply:A,themeable:true},labelPath:{check:E,apply:g,nullable:true},iconPath:{check:E,apply:v,nullable:true},groupLabelPath:{check:E,apply:F,nullable:true},labelOptions:{apply:f,nullable:true},iconOptions:{apply:G,nullable:true},groupLabelOptions:{apply:j,nullable:true},delegate:{apply:w,event:x,init:null,nullable:true},autoGrouping:{check:q,init:true},groups:{check:h,event:p,nullable:false,deferredInit:true}},members:{_background:null,_provider:null,_layer:null,__yj:null,__yk:null,__yl:null,__ym:false,__yn:false,__yo:false,__yi:null,refresh:function(){this.__yq();}
...
,_init:function(){this._provider=new qx.ui.list.provider.WidgetProvider(this);this.__yj=[];this.__yk=[];this.__yl={};this.__ym=false;this.__yn=false;this.__yo=false;this.getPane().addListener(d,this._onResize,this);this._initBackground();this._initLayer();}
...
;}
,_getLookupTable:function(){return this.__yj;}
,_lookup:function(P){return this.__yj[P];}
,_lookupGroup:function(Q){return this.__yk.indexOf(Q);}
,_reverseLookup:function(R){if(R<0){return -1;}
;return this.__yj.indexOf(R);}
...
,_applyDelegate:function(bl,bk){this._provider.setDelegate(bl);this.__yq();}
,_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);}
,_onModelChange:function(e){this.__yq();this._applyDefaultSelection();}
,__yp:function(){this.getPane().getRowConfig().setItemCount(this.__yj.length);this.getPane().fullUpdate();}
,__yq:function(){this.__yj=[];this.__yk=[];this.__yl={};if(this.isAutoGrouping()){this.getGroups().removeAll();}
;var bm=this.getModel();if(bm!=null){this._runDelegateFilter(bm);this._runDelegateSorter(bm);this._runDelegateGroup(bm);}
;this._updateSelection();this.__yp();}
,_runDelegateFilter:function(bo){var bn=qx.util.Delegate.getMethod(this.getDelegate(),C);for(var i=0,l=bo.length;i<l; ++i){if(bn==null||bn(bo.getItem(i))){this.__yj.push(i);}
;}
;}
,_runDelegateSorter:function(bp){if(this.__yj.length==0){return;}
;var bq=qx.util.Delegate.getMethod(this.getDelegate(),s);if(bq!=null){this.__yj.sort(function(a,b){return bq(bp.getItem(a),bp.getItem(b));}
);}
;}
,_runDelegateGroup:function(bu){var bv=qx.util.Delegate.getMethod(this.getDelegate(),u);if(bv!=null){for(var i=0,l=this.__yj.length;i<l; ++i){var br=this.__yj[i];var bt=this.getModel().getItem(br);var bs=bv(bt);this.__yr(bs,br);}
;this.__yj=this.__ys();}
;}
,__yr:function(bx,bw){if(bx==null){this.__yo=true;bx=z;}
;var name=this.__yt(bx);if(this.__yl[name]==null){this.__yl[name]=[];if(this.isAutoGrouping()){this.getGroups().push(bx);}
;}
;this.__yl[name].push(bw);}
,__ys:function(){this.__yu();var bB=[];var bD=0;var bz=this.getGroups();for(var i=0;i<bz.getLength();i++ ){var by=bz.getItem(i);bB.push(-1);this.__yk.push(bD);bD++ ;var bC=this.__yt(by);var bA=this.__yl[bC];if(bA!=null){for(var k=0;k<bA.length;k++ ){bB.push(bA[k]);bD++ ;}
;}
;}
;return bB;}
,__yt:function(bF){var name=null;if(!qx.lang.Type.isString(bF)){var bE=this.getGroups().indexOf(bF);this.__yn=true;name=u;if(bE==-1){name+=this.getGroups().getLength();}
else {name+=bE;}
;}
else {this.__ym=true;var name=bF;}
;return name;}
,__yu:function(){if(this.__yn&&this.__yo||this.__yn&&this.__ym){throw new Error(r+y);}
;}
...
;this._background.dispose();this._provider.dispose();this._layer.dispose();this._background=this._provider=this._layer=this.__yj=this.__yk=this.__yl=null;if(this.__yi){this.__yi.dispose();}
;}
});}
)();
...
;this.__yv={};this.__yw={};}
,properties:{colorEven:{nullable:true,check:c,apply:b,themeable:true},colorOdd:{nullable:true,check:c,apply:a,themeable:true}},members:{__yx:null,__yy:null,__yv:null,__yw:null,setColor:function(h,g){if(g){this.__yv[h]=qx.theme.manager.Color.getInstance().resolve(g);}
else {delete (this.__yv[h]);}
;}
,clearCustomColors:function(){this.__yv={};this.updateLayerData();}
,getColor:function(i){var j=this.__yv[i];if(j){return j;}
else {return i%2==0?this.__yx:this.__yy;}
;}
,_applyColorEven:function(l,k){if(l){this.__yx=qx.theme.manager.Color.getInstance().resolve(l);}
else {this.__yx=null;}
;this.updateLayerData();}
,_applyColorOdd:function(n,m){if(n){this.__yy=qx.theme.manager.Color.getInstance().resolve(n);}
else {this.__yy=null;}
;this.updateLayerData();}
,setBackground:function(o,p){if(p){this.__yw[o]=qx.theme.manager.Decoration.getInstance().resolve(p);}
else {delete (this.__yw[o]);}
;this.updateLayerData();}
,getBackground:function(q){return this.__yw[q];}
},destruct:function(){this.__yv=this.__yw=null;}
});}
)();
...
;}
,setColor:function(J,I){qx.ui.virtual.layer.AbstractBackground.prototype.setColor.call(this,J,I);if(this.__yz(J)){this.updateLayerData();}
;}
,setBackground:function(K,L){qx.ui.virtual.layer.AbstractBackground.prototype.setBackground.call(this,K,L);if(this.__yz(K)){this.updateLayerData();}
;}
,__yz:function(M){var O=this.getFirstRow();var N=O+this.getRowSizes().length-1;return M>=O&&M<=N;}
}});}
)();
...
(function(){var a="cell.type",b="model[",c="groups[",d="model",f="",g="changeDelegate",h="label",i="qx.ui.list.core.MWidgetController",j="icon",k="]",l="value",m="BindingIds",n=".",o="group",p="String";qx.Mixin.define(i,{construct:function(){this.__xu=[];}
...
;}
,bindProperty:function(u,x,v,t,s){var y=t.getUserData(a);var w=this.__xv(s,u,y);if(v){v.ignoreConverter=d;}
;var z=this._list.bind(w,t,x,v);this.__xw(t,z);}
,bindPropertyReverse:function(D,B,E,F,A){var G=F.getUserData(a);var C=this.__xv(A,D,G);var H=F.bind(B,this._list,C,E);this.__xw(F,H);}
,removeBindings:function(){while(this.__xu.length>0){var I=this.__xu.pop();this._removeBindingsFrom(I);}
;}
...
;}
,_removeBindingsFrom:function(T){var U=this.__xx(T);while(U.length>0){var V=U.pop();try{this._list.removeBinding(V);}
catch(e){T.removeBinding(V);}
;}
;if(qx.lang.Array.contains(this.__xu,T)){qx.lang.Array.remove(this.__xu,T);}
;}
,__xv:function(X,ba,Y){var W=b+X+k;if(Y==o){W=c+X+k;}
;if(ba!=null&&ba!=f){W+=n+ba;}
;return W;}
,__xw:function(bc,bd){var bb=this.__xx(bc);if(!qx.lang.Array.contains(bb,bd)){bb.push(bd);}
;if(!qx.lang.Array.contains(this.__xu,bc)){this.__xu.push(bc);}
;}
,__xx:function(be){var bf=be.getUserData(m);if(bf==null){bf=[];be.setUserData(m,bf);}
;return bf;}
...
,styleSelectabled:function(v){var u=this.__xy(v);this._styleSelectabled(u);}
,styleUnselectabled:function(x){var w=this.__xy(x);this._styleUnselectabled(w);}
...
;}
,_styleSelectabled:function(A){this.__xz(A,{selected:1});}
,_styleUnselectabled:function(B){this.__xz(B,{});}
...
,__xy:function(G){return this._list._layer.getRenderedCellWidget(G,0);}
,__xz:function(I,H){if(I==null){return;}
