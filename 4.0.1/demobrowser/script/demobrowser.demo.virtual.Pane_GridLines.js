...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Pane_GridLines.f745f8fe214b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;this.__xn=[];this.__xo=[];this._isHorizontal=(w||c)==c;}
,properties:{defaultLineColor:{init:h,check:s,themeable:true},defaultLineSize:{init:1,check:t,themeable:true}},members:{__xn:null,__xo:null,isHorizontal:function(){return this._isHorizontal;}
...
;this.__xn[B]=A;if(this.__xp(B)){this.updateLayerData();}
;}
...
;this.__xo[C]=D;if(this.__xp(C)){this.updateLayerData();}
;}
,__xp:function(E){if(this._isHorizontal){var I=this.getFirstColumn();var G=I+this.getColumnSizes().length-1;return E>=I&&E<=G;}
else {var H=this.getFirstRow();var F=H+this.getRowSizes().length-1;return E>=H&&E<=F;}
;}
,getLineSize:function(J){return this.__xo[J]||this.getDefaultLineSize();}
,getLineColor:function(K){return this.__xn[K]||this.getDefaultLineColor();}
,__xq:function(P,M,O){var top=0;var L,N;for(var y=0;y<O.length-1;y++ ){L=this.getLineColor(M+y);N=this.getLineSize(M+y);top+=O[y];P.push(m,g,f+N+q,j,d,top-((N>1)?Math.floor(N/2):1),q,b,L,n,a);}
;}
,__xr:function(T,Q,S){var V=0;var R,U;for(var x=0;x<S.length-1;x++ ){R=this.getLineColor(Q+x);U=this.getLineSize(Q+x);V+=S[x];T.push(m,g,k+U+q,l,u,p,V-((U>1)?Math.floor(U/2):1),q,b,R,n,a);}
;}
,_fullUpdate:function(ba,bb,X,W){var Y=[];if(this._isHorizontal){this.__xq(Y,ba,X);}
else {this.__xr(Y,bb,W);}
...
;}
},destruct:function(){this.__xn=this.__xo=null;}
});}
)();
