...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Flow.913ddbaa51db.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var x=new qx.ui.layout.LineSizeIterator(D,this.getSpacingX());var B=z.top;while(x.hasMoreLines()){var C=x.computeNextLine(A);this.__DK(C,B,A,z);B+=C.height+this.getSpacingY();}
;}
,__DK:function(L,K,I,E){var H=qx.ui.layout.Util;var F=E.left;if(this.getAlignX()!=l){F=E.left+I-L.width;if(this.getAlignX()==j){F=E.left+Math.round((I-L.width)/2);}
;}
...
;}
,_computeSizeHint:function(){return this.__DL(Infinity);}
,hasHeightForWidth:function(){return true;}
,getHeightForWidth:function(P){return this.__DL(P).height;}
...
;return S;}
,__DL:function(T){var U=new qx.ui.layout.LineSizeIterator(this._getLayoutChildren(),this.getSpacingX());var V=0;var W=0;var X=0;while(U.hasMoreLines()){var Y=U.computeNextLine(T);X+=1;W=Math.max(W,Y.width);V+=Y.height;}
;return {width:W,height:V+this.getSpacingY()*(X-1)};}
}});}
)();
(function(){var a="No more lines to compute",b="qx.ui.layout.LineSizeIterator";qx.Class.define(b,{extend:Object,construct:function(d,c){this.__gI=d;this.__Bt=c;this.__Bu=d.length>0;this.__Bv=0;}
,members:{__gI:null,__Bt:null,__Bu:null,__Bv:null,computeNextLine:function(k){var k=k||Infinity;if(!this.__Bu){throw new Error(a);}
;var p=this.__gI;var n=0;var f=0;var o=[];var j=[];for(var i=this.__Bv;i<p.length;i++ ){var m=p[i];var q=m.getSizeHint();var g=this.__Bw(i);var h=q.width+g;var e=i==this.__Bv;if(!e&&f+h>k){this.__Bv=i;break;}
;var l=q.height+m.getMarginTop()+m.getMarginBottom();o.push(m);j.push(g);f+=h;n=Math.max(n,l);if(m.getLayoutProperties().lineBreak){this.__Bv=i+1;break;}
;}
;if(i>=p.length){this.__Bu=false;}
;return {height:n,width:f,children:o,gapsBefore:j};}
,__Bw:function(r){var s=r==this.__Bv;if(s){return this.__gI[r].getMarginLeft();}
else {return Math.max(this.__gI[r-1].getMarginRight(),this.__gI[r].getMarginLeft(),this.__Bt);}
;}
,hasMoreLines:function(){return this.__Bu;}
}});}
)();
