...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Placeholder.1915179d152e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var J=L.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);if(!J){J=qx.module.Placeholder.__GO(L);}
;var K=L.isRendered();var M=J.isRendered();if(K&&!M){L.before(J);}
else if(!K&&M){J.remove();return this;}
;qx.module.Placeholder.__GN(L);}
;}
;return this;}
,__GN:function(T){var U=T.getAttribute(o);var Q=T.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);var S=T.getStyle(A);var P=parseInt(T.getStyle(p))+2*parseInt(T.getStyle(m));var R=parseInt(T.getStyle(y))+2*parseInt(T.getStyle(E));Q.setHtml(U).setStyles({display:T.getValue()==t?g:z,zIndex:S==j?1:S+1,textAlign:T.getStyle(n),width:(T.getWidth()-P-4)+d,height:(T.getHeight()-R-4)+d,left:T.getPosition().left+d,top:T.getPosition().top+d,fontFamily:T.getStyle(l),fontStyle:T.getStyle(H),fontVariant:T.getStyle(f),fontWeight:T.getStyle(q),fontSize:T.getStyle(c),paddingTop:(parseInt(T.getStyle(y))+2)+d,paddingRight:(parseInt(T.getStyle(m))+2)+d,paddingBottom:(parseInt(T.getStyle(E))+2)+d,paddingLeft:(parseInt(T.getStyle(p))+2)+d});}
,__GO:function(V){var W=qxWeb.create(b).setStyles({position:G,color:r,overflow:D,pointerEvents:z});V.setProperty(qx.module.Placeholder.PLACEHOLDER_NAME,W);V.on(w,function(X){var Y=X.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);Y.setStyle(x,X.getValue()==t?g:z);}
