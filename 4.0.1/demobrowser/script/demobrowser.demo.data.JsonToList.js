...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.JsonToList","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.JsonToList.5d195ee1a37f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="icon/16/mimetypes/",b="name",c="demobrowser/demo/data/list.json",d="model.items",e="Loading...",f=" will be created by a converter which converts the type to an icon url.",g=".png",h="model",i="state",j="type",k="list.json</a> and bind the items to the list widget. The icons ",l="' target='_blank'>",m="demobrowser.demo.data.JsonToList",n="<b>List bound to data in a json file</b><br/>",o="Loading the json file <a href='",p="value";qx.Class.define(m,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var s=new qx.ui.form.List();this.getRoot().add(s,{left:10,top:80});var t=new qx.data.controller.List(null,s);t.setLabelPath(b);t.setIconOptions({converter:function(v){return a+v+g;}
});t.setIconPath(j);var r=qx.util.ResourceManager.getInstance().toUri(c);var u=new qx.data.store.Json(r);var status=new qx.ui.basic.Label(e);this.getRoot().add(status,{left:120,top:80});u.bind(d,t,h);u.bind(i,status,p);var q=new qx.ui.basic.Label();q.setRich(true);q.setWidth(450);q.setValue(n+o+r+l+k+f);this.getRoot().add(q,{left:10,top:10});}
}});}
)();
