...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ExtendedTree.cbe59afff2c9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="changeModel",b="_applyDelegate",c="qx.core.Object",d="_applyLabelPath",f="icon",g="change",h="label",k="' twice.",l="_applyTarget",m="changeTarget",n="_applyIconPath",o="Can not reverse bind the same target property '",p="_applyLabelOptions",q="_applyChildPath",r="_applyIconOptions",s="qx.data.controller.Tree",t="String",u="Can not bind the same target property '",v="_applyModel",w="get";qx.Class.define(s,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(y,z,A,x){qx.core.Object.call(this);this.__bc={};this.__CD=[];this.__GV={};if(A!=null){this.setChildPath(A);}
;if(x!=null){this.setLabelPath(x);}
;if(y!=null){this.setModel(y);}
;if(z!=null){this.setTarget(z);}
;}
,properties:{model:{check:c,apply:v,event:a,nullable:true,dereference:true},target:{apply:l,event:m,init:null,nullable:true,dereference:true},childPath:{check:t,apply:q,nullable:true},labelPath:{check:t,apply:d,nullable:true},iconPath:{check:t,apply:n,nullable:true},labelOptions:{apply:p,nullable:true},iconOptions:{apply:r,nullable:true},delegate:{apply:b,init:null,nullable:true}},members:{__GV:null,__bc:null,__CD:null,__GW:null,_applyDelegate:function(C,B){this._setConfigureItem(C,B);this._setCreateItem(C,B);this._setBindItem(C,B);}
,_applyIconOptions:function(E,D){this.__CN();}
,_applyLabelOptions:function(G,F){this.__CN();}
,_applyTarget:function(I,H){if(H!=undefined){this.__yG(H);}
;if(this.getModel()!=null){this.__Ha();}
;this._addChangeTargetListener(I,H);}
,_applyModel:function(K,J){this.__Ha();}
,_applyChildPath:function(M,L){this.__GW=L;this.__Ha();this.__GW=null;}
,_applyIconPath:function(O,N){this.__CN();}
,_applyLabelPath:function(Q,P){this.__Ha();}
,__GX:function(R){var T=R.getTarget();var S=this.__GV[T.toHashCode()].treeNode;var U=this.__GV[T.toHashCode()].modelNode;this.__Hb(S,U);this._updateSelection();}
,__GY:function(e){var X=e.getData();var bc=e.getOldData();var V=this.__GV[bc.toHashCode()];bc.removeListenerById(V.changeListenerId);delete this.__GV[bc.toHashCode()];V.modelNode.removeListenerById(V.changeChildernListenerId);var bd=V.modelNode;var bb=qx.Class.getPropertyDefinition(V.modelNode.constructor,this.getChildPath());var Y=bb.event;var ba=bd.addListener(Y,this.__GY,this);var W=V.treeNode;this.__GV[X.toHashCode()]={modelNode:bd,treeNode:W,changeListenerId:V.changeListenerId,changeChildernListenerId:ba};this.__Hb(W,bd);this._updateSelection();}
...
;return be;}
,__Ha:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__yG();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.__xw(this.getModel(),bg);this.__Hb(bg,this.getModel());this.getTarget().setRoot(bg);}
;}
,__Hb:function(bn,br){if(br[w+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bj=br[w+qx.lang.String.firstUp(this.getChildPath())]();if(this.__GV[bj.toHashCode()]==undefined){var bl=bj.addListener(g,this.__GX,this);var bp=qx.Class.getPropertyDefinition(br.constructor,this.getChildPath());var bm=bp.event;var bq=br.addListener(bm,this.__GY,this);this.__GV[bj.toHashCode()]={modelNode:br,treeNode:bn,changeListenerId:bl,changeChildernListenerId:bq};}
...
;}
else {var bo=this._createItem();bo.setModel(bj.getItem(i));bn.addAt(bo,i);this.__xw(bj.getItem(i),bo);this.__Hb(bo,bj.getItem(i));}
;}
;}
;for(var i=bn.getChildren().length-1;i>=bj.length;i-- ){var bk=bn.getChildren()[i];this.__Hd(bk,bn);}
;}
,__yG:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__Hc(bv);var bu=bv.getModel();if(bu){this.__He(bu);}
;bv.destroy();this.__GV={};}
;}
,__Hc:function(bw){var bx=bw.getChildren()||[];for(var i=bx.length-1;i>=0;i-- ){if(bx[i].getChildren().length>0){this.__Hc(bx[i]);}
;this.__Hd(bx[i],bw);}
;}
,__Hd:function(bz,bC){var bD=bz.getModel();var by=this.__GW||this.getChildPath();var bA=w+qx.lang.String.firstUp(by);if(bD[bA]!=undefined){var bE=bD[bA]();var bB=this.__GV[bE.toHashCode()];bE.removeListenerById(bB.changeListenerId);bD.removeListenerById(bB.changeChildernListenerId);this.__Hc(bz);delete this.__GV[bE.toHashCode()];}
;this.__He(bD);bC.remove(bz);bz.destroy();}
...
;if(!qx.lang.Array.contains(this.__CD,bJ)){this.__CD.push(bJ);}
;}
...
;if(!qx.lang.Array.contains(this.__CD,bM)){this.__CD.push(bM);}
;}
...
;}
,__CN:function(){var bW;for(var ca in this.__bc){bW=ca;break;}
;for(var bX in this.__bc[bW]){var bV=this.__bc[bW][bX].treeNode;var bY=qx.core.ObjectRegistry.fromHashCode(bX);this.__He(bY);this.__xw(bY,bV);}
;}
,__xw:function(cd,cb){var cc=this.getDelegate();if(cc!=null&&cc.bindItem!=null){cc.bindItem(this,cb,cd);}
else {this.bindDefaultProperties(cb,cd);}
;}
,__He:function(cg){for(var i=0;i<this.__CD.length;i++ ){var ce=this.__CD[i];var cf=this.__bc[ce][cg.toHashCode()];if(cf!=null){if(cf.id){cg.removeBinding(cf.id);cf.id=null;}
...
;this._startSelectionModification();this.__yG();this.__Ha();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cn,cm){if(cn!=null&&cn.bindItem!=null){if(cm!=null&&cm.bindItem!=null&&cn.bindItem==cm.bindItem){return;}
;this.__Ha();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__bc=this.__GV=this.__CD=null;}
});}
)();
...
(function(){var a="changeNames",b="changeName",c="__NS",d="",e="Validation Error: ",f="changeChild",g="- ",h="qx.data.Array",j="changeColor",k="Marge",l="AFFE",m="is no number (parseFloat says so!).",n="Homer",o="\n",p="demobrowser.demo.data.model.Node",q="String",r="changeName2",s="changeChildren";qx.Class.define(p,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.setChildren(new qx.data.Array());}
,properties:{children:{check:h,event:s},child:{check:p,event:f,nullable:true},names:{check:h,event:a,init:new qx.data.Array(n,k)},name:{check:q,init:l,event:b},name2:{check:q,event:r},number:{init:10,validate:c},color:{event:j,nullable:true}},members:{__NS:function(t){if(!isNaN(parseFloat(t))){if(/^\d+$/.test(t)){return;}
;}
