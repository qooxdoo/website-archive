...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormRenderer.b2ad2dc2508c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="female",b="Name",c="Registration",d="male",e="Send",f="send...",g="Bio",h="Gender",i="Country",j="Personal Information",k="Save?",l="Password",m="execute",n="demobrowser.demo.ui.FormRenderer",o="Reset",p="Age";qx.Class.define(n,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var r=new qx.ui.form.Form();r.addGroupHeader(c);var q=new qx.ui.form.TextField();q.setRequired(true);r.add(q,b);var v=new qx.ui.form.PasswordField();v.setRequired(true);r.add(v,l);r.add(new qx.ui.form.CheckBox(),k);r.addGroupHeader(j);r.add(new qx.ui.form.Spinner(),p);r.add(new qx.ui.form.TextField(),i);var u=new qx.ui.form.SelectBox();u.add(new qx.ui.form.ListItem(d));u.add(new qx.ui.form.ListItem(a));r.add(u,h);r.add(new qx.ui.form.TextArea(),g);var t=new qx.ui.form.Button(e);t.addListener(m,function(){if(r.validate()){alert(f);}
;}
,this);r.addButton(t);var s=new qx.ui.form.Button(o);s.addListener(m,function(){r.reset();}
,this);r.addButton(s);this.getRoot().add(new qx.ui.form.renderer.Single(r),{left:10,top:10});}
}});}
)();
(function(){var a="change",b="qx.ui.form.Form",c="qx.event.type.Event",d="";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__Fq=[];this._buttons=[];this._buttonOptions=[];this._validationManager=this._createValidationManager();this._resetter=this._createResetter();}
,events:{"change":c},members:{__Fq:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(g,f,h,name,e,k){if(this.__Fr()){this.__Fq.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
;this.__Fq[this._groupCounter].items.push(g);this.__Fq[this._groupCounter].labels.push(f);this.__Fq[this._groupCounter].options.push(k);if(name==null){name=f.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,d);}
;this.__Fq[this._groupCounter].names.push(name);this._validationManager.add(g,h,e);this._resetter.add(g);this.fireEvent(a);}
,addGroupHeader:function(m,l){if(!this.__Fr()){this._groupCounter++ ;}
;this.__Fq.push({title:m,items:[],labels:[],names:[],options:[],headerOptions:l});this.fireEvent(a);}
,addButton:function(n,o){this._buttons.push(n);this._buttonOptions.push(o||null);this.fireEvent(a);}
,__Fr:function(){return this.__Fq.length===0;}
,remove:function(p){for(var i=0;i<this.__Fq.length;i++ ){var r=this.__Fq[i];for(var j=0;j<r.items.length;j++ ){var q=r.items[j];if(q===p){r.items.splice(j,1);r.labels.splice(j,1);r.names.splice(j,1);r.options.splice(j,1);this._validationManager.remove(p);this._resetter.remove(p);this.fireEvent(a);return true;}
;}
;}
;return false;}
,removeGroupHeader:function(u){for(var i=0;i<this.__Fq.length;i++ ){var t=this.__Fq[i];if(t.title===u){var s;if(i==0){if(this.__Fq.length==1){t.title=null;t.headerOptions={};this.fireEvent(a);return true;}
else {s=this.__Fq[i+1];}
;}
else {s=this.__Fq[i-1];}
;s.items=s.items.concat(t.items);s.labels=s.labels.concat(t.labels);s.names=s.names.concat(t.names);s.options=s.options.concat(t.options);this.__Fq.splice(i,1);this._groupCounter-- ;this.fireEvent(a);return true;}
;}
;return false;}
...
,getGroups:function(){return this.__Fq;}
,getButtons:function(){return this._buttons;}
,getButtonOptions:function(){return this._buttonOptions;}
,getItems:function(){var y={};for(var i=0;i<this.__Fq.length;i++ ){var z=this.__Fq[i];for(var j=0;j<z.names.length;j++ ){var name=z.names[j];y[name]=z.items[j];}
;}
;return y;}
...
},destruct:function(){this.__Fq=this._buttons=this._buttonOptions=null;this._validationManager.dispose();this._resetter.dispose();}
});}
)();
(function(){var a="Widgets supporting selection can only be validated ",b="Added widget not supported.",c="",d="in the form validator",f="mouse",g="changeValid",h="qx.event.type.Event",j="complete",k="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",l="qx.ui.form.validation.Manager",m="This field is required",n="String",o="qx.event.type.Data";qx.Class.define(l,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__Bd=[];this.__Be={};this.setRequiredFieldMessage(qx.locale.Manager.tr(m));}
,events:{"changeValid":o,"complete":h},properties:{validator:{check:k,init:null,nullable:true},invalidMessage:{check:n,init:c},requiredFieldMessage:{check:n,init:c},context:{nullable:true}},members:{__Bd:null,__Bf:null,__Be:null,__Bg:null,add:function(s,p,r){if(!this.__Bl(s)){throw new Error(b);}
;if(this.__Ae(s)&&!s.getValue){if(p!=null){throw new Error(a+d);}
;}
;var q={item:s,validator:p,valid:null,context:r};this.__Bd.push(q);}
,remove:function(v){var t=this.__Bd;for(var i=0,u=t.length;i<u;i++ ){if(v===t[i].item){t.splice(i,1);return v;}
;}
;return null;}
,getItems:function(){var w=[];for(var i=0;i<this.__Bd.length;i++ ){w.push(this.__Bd[i].item);}
;return w;}
,validate:function(){var B=true;this.__Bg=true;var y=[];for(var i=0;i<this.__Bd.length;i++ ){var z=this.__Bd[i].item;var C=this.__Bd[i].validator;y.push(z);if(C==null){var x=this.__Bh(z);B=B&&x;this.__Bg=x&&this.__Bg;continue;}
;var x=this.__Bi(this.__Bd[i],z.getValue());B=x&&B;if(x!=null){this.__Bg=x&&this.__Bg;}
;}
;var A=this.__Bj(y);if(qx.lang.Type.isBoolean(A)){this.__Bg=A&&this.__Bg;}
;B=A&&B;this.__Bm(B);if(qx.lang.Object.isEmpty(this.__Be)){this.fireEvent(j);}
;return B;}
,__Bh:function(E){if(E.getRequired()){if(this.__Ae(E)){var D=!!E.getSelection()[0];}
...
;return true;}
,__Bi:function(K,L){var M=K.item;var O=K.context;var N=K.validator;if(this.__Bk(N)){this.__Be[M.toHashCode()]=null;N.validate(M,M.getValue(),this,O);return null;}
...
,__Bj:function(Q){var R=this.getValidator();var S=this.getContext()||this;if(R==null){return true;}
;this.setInvalidMessage(c);if(this.__Bk(R)){this.__Be[this.toHashCode()]=null;R.validateForm(Q,this,S);return null;}
...
;}
;return T;}
,__Bk:function(V){var U=false;if(!qx.lang.Type.isFunction(V)){U=qx.Class.isSubClassOf(V.constructor,qx.ui.form.validation.AsyncValidator);}
;return U;}
,__Bl:function(W){var X=W.constructor;return qx.Class.hasInterface(X,qx.ui.form.IForm);}
,__Ae:function(Y){var ba=Y.constructor;return qx.Class.hasInterface(ba,qx.ui.core.ISingleSelection);}
,__Bm:function(bb){this._showToolTip(bb);var bc=this.__Bf;this.__Bf=bb;if(bc!=bb){this.fireDataEvent(g,bb,bc);}
;}
,_showToolTip:function(bg){if(!qx.ui.tooltip||!qx.ui.tooltip.Manager){return;}
;var be=qx.ui.tooltip.Manager.getInstance().getSharedErrorTooltip();if(!bg){var bd;for(var i=0;i<this.__Bd.length;i++ ){var bf=this.__Bd[i].item;if(!bf.isValid()){bd=bf;if(!(bf.getContentLocation)){return;}
...
;}
,getValid:function(){return this.__Bf;}
,isValid:function(){return this.getValid();}
,getInvalidMessages:function(){var bh=[];for(var i=0;i<this.__Bd.length;i++ ){var bi=this.__Bd[i].item;if(!bi.getValid()){bh.push(bi.getInvalidMessage());}
;}
;if(this.getInvalidMessage()!=c){bh.push(this.getInvalidMessage());}
;return bh;}
,getInvalidFormItems:function(){var bj=[];for(var i=0;i<this.__Bd.length;i++ ){var bk=this.__Bd[i].item;if(!bk.getValid()){bj.push(bk);}
;}
;return bj;}
,reset:function(){for(var i=0;i<this.__Bd.length;i++ ){var bl=this.__Bd[i];bl.item.setValid(true);}
;this.__Bf=null;}
,setItemValid:function(bn,bm){this.__Be[bn.toHashCode()]=bm;bn.setValid(bm);this.__Bn();}
,setFormValid:function(bo){this.__Be[this.toHashCode()]=bo;this.__Bn();}
,__Bn:function(){var bq=this.__Bg;for(var br in this.__Be){var bp=this.__Be[br];bq=bp&&bq;if(bp==null){return;}
;}
;this.__Bm(bq);this.__Be={};this.fireEvent(j);}
},destruct:function(){this.__Bd=null;}
});}
)();
...
,members:{__Bo:null,__Bp:null,__gm:null,__Bq:null,validate:function(c,e,d,f){this.__Bq=false;this.__Bp=c;this.__gm=d;this.__Bo.call(f||this,this,e);}
,validateForm:function(g,h,i){this.__Bq=true;this.__gm=h;this.__Bo.call(i,g,this);}
,setValid:function(k,j){if(this.__Bq){if(j!==undefined){this.__gm.setInvalidMessage(j);}
;this.__gm.setFormValid(k);}
else {if(j!==undefined){this.__Bp.setInvalidMessage(j);}
;this.__gm.setItemValid(this.__Bp,k);}
;}
},destruct:function(){this.__gm=this.__Bp=null;}
});}
)();
...
(function(){var a="The given item has not been added.",b="qx.ui.form.Resetter",c=" not supported for reseting.",d="Item ";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__rP=[];}
,members:{__rP:null,add:function(e){if(this._supportsValue(e)){var f=e.getValue();}
else if(this.__Ae(e)){var f=e.getSelection();}
else if(this.__Af(e)){var f=e.getSelection().concat();}
else {throw new Error(d+e+c);}
;this.__rP.push({item:e,init:f});}
,remove:function(g){for(var i=0;i<this.__rP.length;i++ ){var h=this.__rP[i];if(h.item===g){this.__rP.splice(i,1);return true;}
;}
;return false;}
,reset:function(){for(var i=0;i<this.__rP.length;i++ ){var j=this.__rP[i];this.__Ac(j.item,j.init);}
;}
,resetItem:function(k){var l;for(var i=0;i<this.__rP.length;i++ ){var m=this.__rP[i];if(m.item===k){l=m.init;break;}
;}
;if(l===undefined){throw new Error(a);}
;this.__Ac(k,l);}
,__Ac:function(n,o){if(this._supportsValue(n)){n.setValue(o);}
else if(this.__Ae(n)||this.__Af(n)){n.setSelection(o);}
;}
,redefine:function(){for(var i=0;i<this.__rP.length;i++ ){var p=this.__rP[i].item;this.__rP[i].init=this.__Ad(p);}
;}
,redefineItem:function(q){var r;for(var i=0;i<this.__rP.length;i++ ){if(this.__rP[i].item===q){r=this.__rP[i];break;}
;}
;if(r===undefined){throw new Error(a);}
;r.init=this.__Ad(r.item);}
,__Ad:function(s){if(this._supportsValue(s)){return s.getValue();}
else if(this.__Ae(s)||this.__Af(s)){return s.getSelection();}
;}
,__Ae:function(t){var u=t.constructor;return qx.Class.hasInterface(u,qx.ui.core.ISingleSelection);}
,__Af:function(v){var w=v.constructor;return qx.Class.hasInterface(w,qx.data.controller.ISelection);}
...
,_applyNumberFormat:function(bp,bo){var bn=this.getChildControl(t);bn.setFilter(this._getFilterRegExp());this.getNumberFormat().addListener(F,this._onChangeNumberFormat,this);this._applyValue(this.__wB,undefined);}
...
,_countUp:function(){if(this.__wC){var bC=this.getValue()+this.getPageStep();}
...
;}
;this.gotoValue(bC);}
,_countDown:function(){if(this.__wD){var bE=this.getValue()-this.getPageStep();}
