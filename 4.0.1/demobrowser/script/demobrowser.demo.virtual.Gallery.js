...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Gallery.539ce532050c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
});qx.Class.define(R,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var bo=new qx.ui.basic.Atom().set({iconPosition:B});bo.getChildControl(K).set({padding:[0,4]});bo.getChildControl(v).set({padding:4});return bo;}
,updateData:function(bp,bq){bp.set({icon:bq.icon,label:bq.label});}
,updateStates:function(bs,br){var bt=bs.getChildControl(K);var bu=bs.getChildControl(v);if(br.selected){bt.setBackgroundColor(j);bt.setTextColor(Q);bu.setDecorator(A);bu.setBackgroundColor(c);}
else {bt.resetBackgroundColor();bt.resetTextColor();bu.resetDecorator();bu.resetBackgroundColor();}
;}
}});qx.Class.define(f,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bv){demobrowser.demo.virtual.AbstractGallery.call(this,bv);this.__OP=new demobrowser.demo.virtual.GalleryCell();}
,members:{__OP:null,_createScroller:function(){var bw=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.WidgetCell(this);bw.getPane().addLayer(this.layer);new qx.ui.virtual.behavior.Prefetch(bw,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:200,maxAbove:300,minBelow:600,maxBelow:800}).set({interval:500});return bw;}
...
;if(by){this.__OP.updateStates(bx,{selected:1});}
else {this.__OP.updateStates(bx,{});}
;}
;}
...
;var bE=this.__OP.getCellWidget(bF,bD);bE.setUserData(n,bH);return bE;}
,poolCellWidget:function(bJ){this.__OP.pool(bJ);}
},destruct:function(){this._disposeObjects(F,q);}
...
,members:{_createScroller:function(){var bN=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.HtmlCell(this);bN.getPane().addLayer(this.layer);var bM=new qx.ui.virtual.layer.GridLines(D,I);bN.getPane().addLayer(bM);var bM=new qx.ui.virtual.layer.GridLines(t,I);bN.getPane().addLayer(bM);return bN;}
...
(function(){var a="_applyInterval",b="__kh",c="appear",d="qx.debug",e="interval",f="qx.ui.virtual.behavior.Prefetch",g="scroll",h="_applyScroller",i="qx.ui.virtual.core.Scroller",j="scrollbar-x",k="Integer",l="scrollbar-y";qx.Class.define(f,{extend:qx.core.Object,construct:function(m,n){if(qx.core.Environment.get(d)){this.assertObject(n);this.assertPositiveInteger(n.minLeft);this.assertPositiveInteger(n.maxLeft);this.assertPositiveInteger(n.minRight);this.assertPositiveInteger(n.maxRight);this.assertPositiveInteger(n.minAbove);this.assertPositiveInteger(n.maxAbove);this.assertPositiveInteger(n.minBelow);this.assertPositiveInteger(n.maxBelow);}
;qx.core.Object.call(this);this.setPrefetchX(n.minLeft,n.maxLeft,n.minRight,n.maxRight);this.setPrefetchY(n.minAbove,n.maxAbove,n.minBelow,n.maxBelow);this.__kh=new qx.event.Timer(this.getInterval());this.__kh.addListener(e,this._onInterval,this);if(m){this.setScroller(m);}
;}
,properties:{scroller:{check:i,nullable:true,init:null,apply:h},interval:{check:k,init:200,apply:a}},members:{__FV:null,__FW:null,__kh:null,__FX:null,__FY:null,setPrefetchX:function(o,r,p,q){this.__FV=[o,r,p,q];}
,setPrefetchY:function(v,u,t,s){this.__FW=[v,u,t,s];}
,_onInterval:function(){var w=this.__FV;if(w[1]&&w[3]){this.getScroller().getPane().prefetchX(w[0],w[1],w[2],w[3]);qx.ui.core.queue.Manager.flush();}
;var x=this.__FW;if(x[1]&&x[3]){this.getScroller().getPane().prefetchY(x[0],x[1],x[2],x[3]);qx.ui.core.queue.Manager.flush();}
;}
,_applyScroller:function(z,y){if(y){if(this.__FX){y.getChildControl(j).removeListenerById(this.__FX);}
;if(this.__FY){y.getChildControl(l).removeListenerById(this.__FY);}
;}
;if(z){if(!z.getContentElement().getDomElement()){this.__kh.stop();z.addListenerOnce(c,this.__kh.start,this.__kh);}
else {this.__kh.restart();}
;this.__FX=z.getChildControl(j).addListener(g,this.__kh.restart,this.__kh);this.__FY=z.getChildControl(l).addListener(g,this.__kh.restart,this.__kh);}
else {this.__kh.stop();}
;}
,_applyInterval:function(B,A){this.__kh.setInterval(B);}
},destruct:function(){this.setScroller(null);this.__FV=this.__FW=null;this._disposeObjects(b);}
});}
)();
