...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.OwnCodeInModel","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.OwnCodeInModel.09b2de37fd29.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var c="icon/16/mimetypes/",d="name",e="demobrowser/demo/data/list.json",f="model",g="model.items",h="demobrowser.demo.data.OwnCodeInModel",i=".png",j="type",k="Using the delegation to bring two sort function to the root model ",l="Sort By Name",m="items",n="class of this demo. The buttons just calling those sort functions.",o="<b>Adding own code to the model</b><br/>",p="demobrowser.demo.data.ItemsMixin",q="execute",r="Sort By Type";qx.Class.define(h,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var u=new qx.ui.form.List();this.getRoot().add(u,{left:10,top:80});var w=new qx.data.controller.List(null,u);w.setLabelPath(d);w.setIconOptions({converter:function(A){return c+A+i;}
});w.setIconPath(j);var x={getModelMixins:function(B){if(B==m){return demobrowser.demo.data.ItemsMixin;}
;}
};var t=qx.util.ResourceManager.getInstance().toUri(e);var s=new qx.data.store.Json(t,x);s.bind(g,w,f);var y=new qx.ui.form.Button(r);y.setWidth(120);this.getRoot().add(y,{left:130,top:80});y.addListener(q,function(){s.getModel().sortByType();}
,this);var z=new qx.ui.form.Button(l);z.setWidth(120);this.getRoot().add(z,{left:130,top:110});z.addListener(q,function(){s.getModel().sortByName();}
,this);var v=new qx.ui.basic.Label();v.setRich(true);v.setWidth(410);v.setValue(o+k+n);this.getRoot().add(v,{left:10,top:10});}
}});qx.Mixin.define(p,{members:{sortByType:function(){var C=this.getItems();C.sort(function(a,b){if(a.getType()>b.getType()){return 1;}
