...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.9f4d3eaf9bc1.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b=" => ",c="#autoActionsBeforeOnexit",d=": Could not find friendly name for '",f=": event '",g=" blocked.  Re-queuing.",h="string",j="'",k="Attempt to transition to nonexistent state ",l="Saved-state stack is empty",m="Internal error: invalid nextState",n=" not handled.  Ignoring.",o="State ",p="#autoActionsBeforeOntransition",q=" returned a value other than ",r="#",s=" already exists",u="Attempt to transition to POP_STATE_STACK ",v=":",w="Transition ",x=" does not exist",y="' on '",z="qx.util.fsm.FiniteStateMachine",A="String",B="#autoActionsAfterOntransition",C="",D="__EB",E="#actionsAfterOnentry",F="Saved-state stack is full",G="#autoActionsBeforeOnentry",H=": no transition found.  No state change.",I="#autoActionsAfterOnentry",J="#actionsBeforeOnentry",K=": Pushed event: ",L="Machine started with no available states",M=": Process event: ",N="#entry",O="#ontransition",P=" on ",Q="Invalid state: not an instance of ",R="Number",S="#exit",T="true, false, or null.",U="__EC",V="Explicit transition ",W="  ",X="qx.util.fsm.State",Y="while state stack is empty.",bj=".  Ignoring.",bk="#autoActionsAfterOnexit",bl=": Event '",bg="object",bh=" not handled for target ",bi=": Queued event: ";qx.Class.define(z,{extend:qx.core.Object,construct:function(bm){qx.core.Object.call(this);this.setName(bm);this.__jy={};this.__Ez=null;this.__EA=[];this.__EB=[];this.__EC=[];this.__ED={};this.__EE={};this.__EF={};this.__EG={};this.__EH={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:A,nullable:true},state:{check:A,nullable:true},previousState:{check:A,nullable:true},nextState:{check:A,nullable:true},maxSavedStates:{check:R,init:2},debugFlags:{check:R,init:7}},members:{__jy:null,__Ez:null,__EB:null,__EC:null,__EA:null,__ED:null,__EE:null,__EF:null,__EG:null,__EH:null,__EI:false,addState:function(bn){if(!bn instanceof qx.util.fsm.State){throw new Error(Q+X);}
;var bo=bn.getName();if(bo in this.__jy){throw new Error(o+bo+s);}
;if(this.__Ez==null){this.__Ez=bo;}
;this.__jy[bo]=bn;}
,replaceState:function(bs,br){if(!bs instanceof qx.util.fsm.State){throw new Error(Q+X);}
;var bq=bs.getName();var bp=this.__jy[bq];this.__jy[bq]=bs;if(br){bp._bNeedDispose=true;}
;return bp;}
,addObject:function(bw,bt,bu){var bx=bt.toHashCode();this.__EE[bw]=bx;this.__EF[bx]=bw;this.__ED[bw]=bt;if(!bu){return;}
;if(typeof (bu)==h){bu=[bu];}
;for(var i=0;i<bu.length;i++ ){var bv=bu[i];if(!this.__EG[bv]){this.__EG[bv]={};}
;this.__EG[bv][bw]=true;if(!this.__EH[bw]){this.__EH[bw]=[];}
;this.__EH[bw].push(bv);}
;}
,removeObject:function(bB){var by;var bz;var bA;var bC;by=this.__EE[bB];if(this.__EH[bB]){for(var i=0;i<this.__EH[bB].length;i++ ){bz=this.__EH[bB][i];delete this.__EG[bz][bB];bC=true;for(bA in this.__EG[bz]){bC=false;break;}
;if(bC){delete this.__EG[bz];}
;}
;delete this.__EH[bB];}
;delete this.__EF[by];delete this.__EE[bB];delete this.__ED[bB];}
,getObject:function(bD){return this.__ED[bD];}
,getFriendlyName:function(bE){var bF=bE?bE.toHashCode():null;return bF?this.__EF[bF]:null;}
,getGroupObjects:function(bG){var a=[];for(var name in this.__EG[bG]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bH in this.__EE){var bJ=this.__EE[bH];var bI=this.getObject(bH);this.debug(bH+b+bJ);this.debug(W+bJ+b+this.__EF[bJ]);this.debug(W+bH+b+this.getObject(bH));this.debug(W+this.getObject(bH)+b+this.getFriendlyName(bI));}
;}
,_getInternalData:function(){return ({"states":this.__jy,"startState":this.__Ez,"eventQueue":this.__EB,"blockedEvents":this.__EC,"savedStates":this.__EA,"friendlyToObject":this.__ED,"friendlyToHash":this.__EE,"hashToFriendly":this.__EF,"groupToFriendly":this.__EG,"friendlyToGroups":this.__EH});}
,start:function(){var bL=this.__Ez;if(bL==null){throw new Error(L);}
...
;this.__jy[bL].getAutoActionsBeforeOnentry()(this);if(bK){this.debug(this.getName()+r+bL+N);}
;this.__jy[bL].getOnentry()(this,null);if(bK){this.debug(this.getName()+r+bL+E);}
;this.__jy[bL].getAutoActionsAfterOnentry()(this);}
,pushState:function(bM){if(this.__EA.length>=this.getMaxSavedStates()){throw new Error(F);}
;if(bM===true){this.__EA.push(this.getState());}
else if(bM){this.__EA.push(bM);}
else {this.__EA.push(this.getPreviousState());}
;}
,popState:function(){if(this.__EA.length==0){throw new Error(l);}
;return this.__EA.pop();}
,postponeEvent:function(event){this.__EC.unshift(event);}
,enqueueEvent:function(event,bN){if(bN){this.__EB.push(event);}
else {this.__EB.unshift(event);}
...
;}
;}
,eventListener:function(event){var e=event.clone();this.enqueueEvent(e,false);this.__EJ();}
...
,this,bV||20);}
,__EJ:function(){if(this.__EI){return;}
;this.__EI=true;while(this.__EB.length>0){var event=this.__EB.pop();var bW=this.__EK(event);if(bW){event.dispose();}
;}
;this.__EI=false;}
,__EK:function(event){var ca;var cb;var bY;var cd;var cg;var e;var cf;var bX=this.getDebugFlags();var ck=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var ci=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var ch=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var ce=bX&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var cj=this.getFriendlyName(event.getTarget());if(ck){this.debug(this.getName()+M+event.getType()+(cj?P+cj:C));}
;ca=this.getState();cd=this.__jy[ca];cg=cd.transitions;e=cd.getEvents()[event.getType()];if(!e){if(ck){this.debug(this.getName()+bl+event.getType()+j+n);}
;return true;}
...
;this.__EC.unshift(event);return false;default:if(typeof (cf)==h){if(cg[cf]){var cc=cg[cf];cg={};cg[cf]=cc;}
else {throw new Error(V+cf+x);}
;break;}
;};for(var t in cg){var cc=cg[t];switch(cc.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(w+ca+v+t+q+T);};cb=cc.getNextState();if(typeof (cb)==h){if(!cb in this.__jy){throw new Error(k+cb);}
;this.setNextState(cb);}
else {switch(cb){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cb=ca;this.setNextState(cb);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__EA.length==0){throw new Error(u+Y);}
;cb=this.__EA.pop();this.setNextState(cb);break;default:throw new Error(m);};}
...
;cd=this.__jy[this.getNextState()];this.setPreviousState(ca);this.setState(this.getNextState());this.setNextState(null);bY=ca;ca=cb;cb=undefined;if(ch){this.debug(this.getName()+r+ca+G);}
...
;cd.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__EC.length;i++ ){e=this.__EC.pop();this.__EB.unshift(e);}
;if(ci){this.debug(this.getName()+r+bY+b+this.getName()+r+ca);}
;return true;}
;if(ci){this.debug(this.getName()+r+ca+f+event.getType()+j+H);}
;return true;}
},destruct:function(){this._disposeArray(D);this._disposeArray(U);this.__EA=this.__jy=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="parameters",k="__Eq",l=")",m=" fsm.getObject('",n="groups",o="Invalid function parameters: ",p="Invalid onexit type: ",q="  for (var i = 0; i < groupObjects.length; i++)",r="context",s="string",t="autoActionsAfterOnentry",u="Invalid numeric value in events object: ",v='"',w="__Et",x="Invalid numeric value in events object ",y="Invalid state name",z="__Eu",A=" value: ",B="');",C="State ",D="autoActionsBeforeOnexit",E="autoActionsBeforeOnentry",F="}",G="): ",H="events must be an object",I="onexit",J="{",K="Invalid function request type: ",L=": ",M="expected array, found ",N="Invalid friendly name in 'objects' list: ",O="autoActionsAfterOnexit",P="  fsm.debug(ex);",Q="(",R="Invalid onentry type: ",S="    fsm.getObject(objName).",T=")(fsm)",U="events",V="undefined",W="Invalid ",X="__Er",Y="try",ba="__Ev",bb="    var objName = groupObjects[i];",bc="Invalid value in events object: ",bd="').",be="__Es",bf="objects",bg="expected object, found ",bh="qx.util.fsm.Transition",bi="The events object must be provided in new state info",bj="catch(ex)",bk="  {",bl="__Ew",bm="    fsm.getGroupObjects('",bn="Invalid value in events object ",bo="Invalid transition: not an instance of ",bp="  }",bq="Invalid 'objects' list: expected array, got ",br="__Ex",bs="qx.util.fsm.State",bt="onentry",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(bs,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(r,bE);for(var bC in bF){switch(bC){case bt:this.setOnentry(this.__Ey(bF[bC],bE));break;case I:this.setOnexit(this.__Ey(bF[bC],bE));break;case E:this.setAutoActionsBeforeOnentry(bF[bC]);break;case t:this.setAutoActionsAfterOnentry(bF[bC]);break;case D:this.setAutoActionsBeforeOnexit(bF[bC]);break;case O:this.setAutoActionsAfterOnexit(bF[bC]);break;case U:this.setEvents(bF[bC]);break;case r:break;default:this.setUserData(bC,bF[bC]);this.debug(C+bD+L+bz+bC);break;};}
;if(!this.getEvents()){throw new Error(bi);}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(actionType,value,context){if(typeof (value)!=bB){throw new Error(W+actionType+A+typeof (value));}
;var funcFragment;var func=Y+J;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(K+M+typeof (functionRequest));}
...
else {if(!params instanceof Array){throw new Error(o+M+typeof (params));}
;}
...
;for(j=0;j<g.length;j++ ){func+=d+bm+g[j]+B+q+bk+bb+S+funcFragment+bu+bp;}
;}
;}
;}
;func+=F+bj+J+P+F;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:k,nullable:true},onentry:{transform:X,nullable:true,init:function(bG,event){}
},onexit:{transform:be,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:z,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:ba,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:bl,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:br,nullable:true,init:function(bL,event){}
},events:{transform:w,nullable:true}},members:{__Eq:function(bM){if(typeof (bM)!=s||bM.length<1){throw new Error(y);}
;return bM;}
,__Er:function(bN){switch(typeof (bN)){case V:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(r));default:throw new Error(R+typeof (bN));};}
,__Es:function(bP){switch(typeof (bP)){case V:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(r));default:throw new Error(p+typeof (bP));};}
,__Et:function(bT){if(typeof (bT)!=bB){throw new Error(H);}
;for(var e in bT){var bR=bT[e];if(typeof (bR)==bA&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(u+e+L+bR);}
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==bA&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(x+Q+e+G+bS+L+bR[bS]);}
else if(typeof (bR[bS])!=s&&typeof (bR[bS])!=bA){throw new Error(bn+Q+e+G+bS+L+bR[bS]);}
;}
;}
else if(typeof (bR)!=s&&typeof (bR)!=bA){throw new Error(bc+e+L+bT[e]);}
;}
;return bT;}
,__Eu:function(bU){return qx.util.fsm.State._commonTransformAutoActions(E,bU,this.getUserData(r));}
,__Ev:function(bV){return qx.util.fsm.State._commonTransformAutoActions(t,bV,this.getUserData(r));}
,__Ew:function(bW){return qx.util.fsm.State._commonTransformAutoActions(D,bW,this.getUserData(r));}
,__Ex:function(bX){return qx.util.fsm.State._commonTransformAutoActions(O,bX,this.getUserData(r));}
,__Ey:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bo+bh);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="or one of the Fsm.StateChange constants",d="autoActionsAfterOntransition",e="context",g="string",h="predicate",i="__EO",j="nextState must be an explicit state name, ",k="__EP",l="__EM",m="Transition ",n=": ",o="String",p="number",q="Invalid transition nextState type: ",r="Invalid transition nextState value: ",s="Adding user-provided field to transition: ",t="ontransition",u="Invalid transition predicate type: ",v="boolean",w="nextState",x="__EN",y="Invalid ontransition type: ",z="__EL",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(e,E);for(var C in D){switch(C){case h:this.setPredicate(this.__Ey(D[C],E));break;case w:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__Ey(D[C],E));break;case d:this.setAutoActionsAfterOntransition(this.__Ey(D[C],E));break;case t:this.setOntransition(this.__Ey(D[C],E));break;case e:break;default:this.setUserData(C,D[C]);this.debug(m+F+n+s+C);break;};}
;}
,properties:{name:{check:o,nullable:true},predicate:{init:function(G,event){return true;}
,transform:z},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:l},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:x},autoActionsAfterOntransition:{init:function(I,event){}
,transform:i},ontransition:{init:function(J,event){}
,transform:k}},members:{__EL:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
;case v:return function(M,event){return K;}
;case a:return qx.lang.Function.bind(K,this.getUserData(e));default:throw new Error(u+typeof (K));};}
,__EM:function(N){switch(typeof (N)){case g:return N;case p:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(r+N+n+j+c);};break;default:throw new Error(q+typeof (N));};}
,__EN:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(e));}
,__EO:function(P){return qx.util.fsm.State._commonTransformAutoActions(d,P,this.getUserData(e));}
,__EP:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(e));default:throw new Error(y+typeof (Q));};}
,__Ey:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
