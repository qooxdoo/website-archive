...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.showcase.Browser.314b1310879e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="demobrowser.demo.showcase.Browser",b="separator-vertical",c="icon/16/actions/go-previous.png",d="keypress",f="Enter",g="icon/16/actions/go-next.png",h="Web Browser",i="This feature is currently not supported.",j="icon/16/actions/media-playback-start.png",k="http://qooxdoo.org",l="icon/16/categories/internet.png",m="http://",n="middle",o="execute";qx.Class.define(a,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);this.getRoot().add(this._createBrowser(),{left:40,top:30});}
,_createBrowser:function(){var r=new qx.ui.window.Window(h,l);var p=new qx.ui.layout.VBox();p.setSeparator(b);r.setLayout(p);r.setAllowClose(false);r.setContentPadding(0);r.open();var toolbar=new qx.ui.toolbar.ToolBar();r.add(toolbar);var s=new qx.ui.toolbar.Button(null,c);s.addListener(o,function(e){this.iframe.getWindow().history.back();}
,this);toolbar.add(s);var t=new qx.ui.toolbar.Button(null,g);t.addListener(o,function(e){this.iframe.getWindow().history.forward();}
,this);toolbar.add(t);t.setEnabled(false);s.setEnabled(false);t.setToolTipText(i);s.setToolTipText(i);this.txtUrl=new qx.ui.form.TextField().set({marginLeft:1,value:k,padding:2,alignY:n});this.txtUrl.addListener(d,function(e){if(e.getKeyIdentifier()==f){this.surfTo(this.txtUrl.getValue());}
;}
,this);toolbar.add(this.txtUrl,{flex:1});var q=new qx.ui.toolbar.Button(null,j);q.addListener(o,function(e){this.surfTo(this.txtUrl.getValue());}
...
,surfTo:function(u){if(u.indexOf(m)!==0){u=m+u;this.txtUrl.setValue(u);}
...
(function(){var a="qx.ui.window.Manager",b="__rd";qx.Class.define(a,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__rd:null,setDesktop:function(c){this.__rd=c;this.updateStack();}
,getDesktop:function(){return this.__rd;}
,changeActiveWindow:function(d,e){if(d){this.bringToFront(d);d.setActive(true);}
;if(e){e.resetActive();}
;}
,_minZIndex:1e5,updateStack:function(){qx.ui.core.queue.Widget.add(this);}
,syncWidget:function(){this.__rd.forceUnblock();var h=this.__rd.getWindows();var g=this._minZIndex;var m=g+h.length*2;var j=g+h.length*4;var k=null;for(var i=0,l=h.length;i<l;i++ ){var f=h[i];if(!f.isVisible()){continue;}
;k=k||f;if(f.isModal()){f.setZIndex(j);this.__rd.blockContent(j-1);j+=2;k=f;}
else if(f.isAlwaysOnTop()){f.setZIndex(m);m+=2;}
else {f.setZIndex(g);g+=2;}
;if(!k.isModal()&&f.isActive()||f.getZIndex()>k.getZIndex()){k=f;}
;}
;this.__rd.setActiveWindow(k);}
,bringToFront:function(o){var n=this.__rd.getWindows();var p=qx.lang.Array.remove(n,o);if(p){n.push(o);this.updateStack();}
;}
,sendToBack:function(r){var q=this.__rd.getWindows();var s=qx.lang.Array.remove(q,r);if(s){q.unshift(r);this.updateStack();}
;}
},destruct:function(){this._disposeObjects(b);}
});}
)();
(function(){var a="__rf",b="pointerup",c="Boolean",d="losecapture",f="__re",g="move",h="qx.ui.core.MMovable",i="The move handle could not be redefined!",j="pointerdown",k="maximized",l="pointermove",m="roll",n="move-frame";qx.Mixin.define(h,{properties:{movable:{check:c,init:true},useMoveFrame:{check:c,init:false}},members:{__re:null,__rf:null,__rg:null,__rh:null,__ri:null,__rj:null,__rk:null,__rl:false,__rm:null,__rn:0,_activateMoveHandle:function(o){if(this.__re){throw new Error(i);}
;this.__re=o;o.addListener(j,this._onMovePointerDown,this);o.addListener(b,this._onMovePointerUp,this);o.addListener(l,this._onMovePointerMove,this);o.addListener(d,this.__rr,this);}
,__ro:function(){var p=this.__rf;if(!p){p=this.__rf=new qx.ui.core.Widget();p.setAppearance(n);p.exclude();qx.core.Init.getApplication().getRoot().add(p);}
;return p;}
,__rp:function(){var location=this.getContentLocation();var r=this.getBounds();var q=this.__ro();q.setUserBounds(location.left,location.top,r.width,r.height);q.show();q.setZIndex(this.getZIndex()+1);}
,__rq:function(e){var t=this.__rg;var s=Math.max(t.left,Math.min(t.right,e.getDocumentLeft()));var v=Math.max(t.top,Math.min(t.bottom,e.getDocumentTop()));var u=this.__rh+s;var w=this.__ri+v;return {viewportLeft:parseInt(u,10),viewportTop:parseInt(w,10),parentLeft:parseInt(u-this.__rj,10),parentTop:parseInt(w-this.__rk,10)};}
,_onMoveRoll:function(e){e.stop();}
,_onMovePointerDown:function(e){if(!this.getMovable()||this.hasState(k)){return;}
;this.addListener(m,this._onMoveRoll,this);var parent=this.getLayoutParent();var x=parent.getContentLocation();var z=parent.getBounds();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isBlocked()){this.__rm=parent.getBlockerColor();this.__rn=parent.getBlockerOpacity();parent.setBlockerColor(null);parent.setBlockerOpacity(1);parent.blockContent(this.getZIndex()-1);this.__rl=true;}
;}
;this.__rg={left:x.left,top:x.top,right:x.left+z.width,bottom:x.top+z.height};var y=this.getContentLocation();this.__rj=x.left;this.__rk=x.top;this.__rh=y.left-e.getDocumentLeft();this.__ri=y.top-e.getDocumentTop();this.addState(g);this.__re.capture();if(this.getUseMoveFrame()){this.__rp();}
;e.stop();}
,_onMovePointerMove:function(e){if(!this.hasState(g)){return;}
;var B=this.__rq(e);if(this.getUseMoveFrame()){this.__ro().setDomPosition(B.viewportLeft,B.viewportTop);}
...
;this.removeState(g);var parent=this.getLayoutParent();if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__rl){parent.unblock();parent.setBlockerColor(this.__rm);parent.setBlockerOpacity(this.__rn);this.__rm=null;this.__rn=0;this.__rl=false;}
;}
;this.__re.releaseCapture();var D=this.__rq(e);var C=this.getLayoutParent().getInsets();this.setLayoutProperties({left:D.parentLeft-(C.left||0),top:D.parentTop-(C.top||0)});if(this.getUseMoveFrame()){this.__ro().exclude();}
;e.stopPropagation();}
,__rr:function(e){if(!this.hasState(g)){return;}
;this.removeState(g);if(this.getUseMoveFrame()){this.__ro().exclude();}
;}
},destruct:function(){this._disposeObjects(a,f);this.__rg=null;}
});}
)();
...
(function(){var a="Boolean",b="w-resize",c="touch",d="sw-resize",f="n-resize",g="resizableRight",h="ne-resize",i="se-resize",j="Integer",k="e-resize",l="resizableLeft",m="move",n="shorthand",o="maximized",p="resize",q="nw-resize",r="pointerup",s="pointerdown",t="qx.ui.core.MResizable",u="losecapture",v="resize-frame",w="resizableBottom",x="s-resize",y="pointermove",z="resizableTop",A="pointerout";qx.Mixin.define(t,{construct:function(){var content=this.getContentElement();content.addListener(s,this.__rE,this,true);content.addListener(r,this.__rF,this);content.addListener(y,this.__rH,this);content.addListener(A,this.__rI,this);content.addListener(u,this.__rG,this);var B=content.getDomElement();if(B==null){B=window;}
;this.__rs=qx.event.Registration.getManager(B).getHandler(qx.event.handler.DragDrop);}
,properties:{resizableTop:{check:a,init:true},resizableRight:{check:a,init:true},resizableBottom:{check:a,init:true},resizableLeft:{check:a,init:true},resizable:{group:[z,g,w,l],mode:n},resizeSensitivity:{check:j,init:5},useResizeFrame:{check:a,init:true}},members:{__rs:null,__rt:null,__ru:null,__rv:null,__rw:null,__rx:null,__ry:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__rt;if(!C){C=this.__rt=new qx.ui.core.Widget();C.setAppearance(v);C.exclude();qx.core.Init.getApplication().getRoot().add(C);}
;return C;}
,__rz:function(){var location=this.getContentLocation();var D=this._getResizeFrame();D.setUserBounds(location.left,location.top,location.right-location.left,location.bottom-location.top);D.show();D.setZIndex(this.getZIndex()+1);}
,__rA:function(e){var F=this.__ru;var E=this.getSizeHint();var I=this.__ry;var H=this.__rx;var J=H.width;var G=H.height;var L=H.left;var top=H.top;var K;if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){K=Math.max(I.top,Math.min(I.bottom,e.getDocumentTop()))-this.__rw;if(F&this.RESIZE_TOP){G-=K;}
else {G+=K;}
;if(G<E.minHeight){G=E.minHeight;}
else if(G>E.maxHeight){G=E.maxHeight;}
;if(F&this.RESIZE_TOP){top+=H.height-G;}
;}
;if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){K=Math.max(I.left,Math.min(I.right,e.getDocumentLeft()))-this.__rv;if(F&this.RESIZE_LEFT){J-=K;}
else {J+=K;}
...
,__rB:{'1':f,'2':x,'4':b,'8':k,'5':q,'6':d,'9':h,'10':i},__rC:function(e){var location=this.getContentLocation();var O=this.getResizeSensitivity();var N=e.getDocumentLeft();var P=e.getDocumentTop();var M=this.__rD(location,N,P,O);if(M>0){M=M|this.__rD(location,N,P,O*2);}
;this.__ru=M;}
,__rD:function(location,T,R,S){var Q=0;if(this.getResizableTop()&&Math.abs(location.top-R)<S&&T>location.left-S&&T<location.right+S){Q+=this.RESIZE_TOP;}
...
;return Q;}
,__rE:function(e){if(!this.__ru||!this.getEnabled()||e.getPointerType()==c){return;}
;this.addState(p);this.__rv=e.getDocumentLeft();this.__rw=e.getDocumentTop();var location=this.getContentLocation();var V=this.getBounds();this.__rx={top:location.top,left:location.left,width:location.right-location.left,height:location.bottom-location.top,bounds:qx.lang.Object.clone(V)};var parent=this.getLayoutParent();var W=parent.getContentLocation();var U=parent.getBounds();this.__ry={left:W.left,top:W.top,right:W.left+U.width,bottom:W.top+U.height};if(this.getUseResizeFrame()){this.__rz();}
;this.capture();e.stop();}
,__rF:function(e){if(!this.hasState(p)||!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.getUseResizeFrame()){this._getResizeFrame().exclude();}
;var X=this.__rA(e);this.setWidth(X.width);this.setHeight(X.height);if(this.getResizableLeft()||this.getResizableTop()){this.setLayoutProperties({left:X.parentLeft,top:X.parentTop});}
;this.__ru=0;this.removeState(p);this.resetCursor();this.getApplicationRoot().resetGlobalCursor();this.releaseCapture();e.stopPropagation();}
...
;}
,__rH:function(e){if(!this.getEnabled()||e.getPointerType()==c){return;}
;if(this.hasState(p)){var ba=this.__rA(e);if(this.getUseResizeFrame()){var Y=this._getResizeFrame();Y.setUserBounds(ba.viewportLeft,ba.viewportTop,ba.width,ba.height);}
...
;}
;e.stopPropagation();}
else if(!this.hasState(o)&&!this.__rs.isSessionActive()){this.__rC(e);var bd=this.__ru;var bc=this.getApplicationRoot();if(bd){var bb=this.__rB[bd];this.setCursor(bb);bc.setGlobalCursor(bb);}
else if(this.getCursor()){this.resetCursor();bc.resetGlobalCursor();}
;}
;}
,__rI:function(e){if(e.getPointerType()==c){return;}
;if(this.getCursor()&&!this.hasState(p)){this.resetCursor();this.getApplicationRoot().resetGlobalCursor();}
;}
},destruct:function(){if(this.__rt!=null&&!qx.core.ObjectRegistry.inShutDown){this.__rt.destroy();this.__rt=null;}
;this.__rs=null;}
});}
)();
...
;var br=this.getLayoutProperties();this.__rK=br.left===undefined?0:br.left;this.__rJ=br.top===undefined?0:br.top;this.setLayoutProperties({left:null,top:null,edge:0});this.addState(F);this._updateCaptionBar();this.fireEvent(Q);}
;}
;}
,minimize:function(){if(!this.isVisible()){return;}
;if(this.fireNonBubblingEvent(b,qx.event.type.Event,[false,true])){var bs=this.getLayoutProperties();this.__rK=bs.left===undefined?0:bs.left;this.__rJ=bs.top===undefined?0:bs.top;this.removeState(F);this.hide();this.fireEvent(v);}
;}
...
;var bt=this.__rK;var top=this.__rJ;this.setLayoutProperties({edge:null,left:bt,top:top});this.removeState(F);this._updateCaptionBar();this.fireEvent(S);}
;}
...
(function(){var a="qx.ui.embed.AbstractIframe",b="name",c="Abstract method call",d="",f="about:blank",g="_applySource",h="navigate",i="qx.event.type.Event",j="String",k="_applyFrameName",l="qx.event.type.Data";qx.Class.define(a,{extend:qx.ui.core.Widget,construct:function(m){qx.ui.core.Widget.call(this);if(m){this.setSource(m);}
;this._getIframeElement().addListener(h,this.__wE,this);}
,events:{"load":i,"navigate":l},properties:{source:{check:j,apply:g,init:f},frameName:{check:j,init:d,apply:k}},members:{_getIframeElement:function(){throw new Error(c);}
,_applySource:function(o,n){this._getIframeElement().setSource(o);}
,_applyFrameName:function(q,p){this._getIframeElement().setAttribute(b,q);}
...
,__wE:function(e){var r=e.getData();if(r){this.setSource(r);}
;this.fireDataEvent(h,r);}
}});}
)();
(function(){var a="no",b="Boolean",c="px",d="event.help",f="gecko",g="auto",h="_applyScrollbar",i="Unable to set 'nativeHelp' property, possibly due to security restrictions",j="qx.debug",k="DOMNodeInserted",l="load",m="_applyNativeHelp",n="yes",o="scrolling",p="/",q="help",r="pointerup",s="appear",t="pointerdown",u="engine.name",v="block",w="qx.ui.embed.Iframe",x="losecapture",y="contextmenu",z="none",A="iframe",B="__wI",C="display";qx.Class.define(w,{extend:qx.ui.embed.AbstractIframe,construct:function(D){if(D!=null){this.__wH=D;}
;qx.ui.embed.AbstractIframe.call(this,D);qx.event.Registration.addListener(document.body,t,this.block,this,true);qx.event.Registration.addListener(document.body,r,this.release,this,true);qx.event.Registration.addListener(document.body,x,this.release,this,true);this.__wI=this._createBlockerElement();if((qx.core.Environment.get(u)==f)){this.addListenerOnce(s,function(e){var E=this.getContentElement().getDomElement();qx.bom.Event.addNativeListener(E,k,this._onDOMNodeInserted);}
);this._onDOMNodeInserted=qx.lang.Function.listener(this._syncSourceAfterDOMMove,this);}
;}
,properties:{appearance:{refine:true,init:A},nativeContextMenu:{refine:true,init:false},nativeHelp:{check:b,init:false,apply:m},scrollbar:{check:[g,a,n],nullable:true,themeable:true,apply:h}},members:{__wH:null,__wI:null,renderLayout:function(J,top,H,F){qx.ui.embed.AbstractIframe.prototype.renderLayout.call(this,J,top,H,F);var I=c;var G=this.getInsets();this.__wI.setStyles({"left":(J+G.left)+I,"top":(top+G.top)+I,"width":(H-G.left-G.right)+I,"height":(F-G.top-G.bottom)+I});}
,_createContentElement:function(){var K=new qx.html.Iframe(this.__wH);K.addListener(l,this._onIframeLoad,this);return K;}
,_getIframeElement:function(){return this.getContentElement();}
,_createBlockerElement:function(){var L=new qx.html.Blocker();L.setStyles({"zIndex":20,"display":z});return L;}
,_onIframeLoad:function(e){this._applyNativeContextMenu(this.getNativeContextMenu(),null);this._applyNativeHelp(this.getNativeHelp(),null);this.fireNonBubblingEvent(l);}
,block:function(){this.__wI.setStyle(C,v);}
,release:function(){this.__wI.setStyle(C,z);}
,_applyNativeContextMenu:function(O,N){if(O!==false&&N!==false){return;}
;var M=this.getDocument();if(!M){return;}
;try{var P=M.documentElement;}
catch(e){return;}
;if(N===false){qx.event.Registration.removeListener(P,y,this._onNativeContextMenu,this,true);}
;if(O===false){qx.event.Registration.addListener(P,y,this._onNativeContextMenu,this,true);}
;}
,_onNativeContextMenu:function(e){e.preventDefault();}
,_applyNativeHelp:function(R,Q){if(qx.core.Environment.get(d)){var document=this.getDocument();if(!document){return;}
;try{if(Q===false){qx.bom.Event.removeNativeListener(document,q,(function(){return false;}
));}
;if(R===false){qx.bom.Event.addNativeListener(document,q,(function(){return false;}
));}
;}
catch(e){if(qx.core.Environment.get(j)){this.warn(i);}
;}
;}
;}
,_syncSourceAfterDOMMove:function(){var T=this.getContentElement().getDomElement();var S=T.src;if(S.charAt(S.length-1)==p){S=S.substring(0,S.length-1);}
;if(S!=this.getSource()){qx.bom.Iframe.getWindow(T).stop();T.src=this.getSource();}
;}
,_applyScrollbar:function(U){this.getContentElement().setAttribute(o,U);}
,setLayoutParent:function(parent){qx.ui.embed.AbstractIframe.prototype.setLayoutParent.call(this,parent);if(parent){this.getLayoutParent().getContentElement().add(this.__wI);}
;}
},destruct:function(){if(this.getLayoutParent()&&this.__wI.getParent()){this.getLayoutParent().getContentElement().remove(this.__wI);}
;this._disposeObjects(B);qx.event.Registration.removeListener(document.body,t,this.block,this,true);qx.event.Registration.removeListener(document.body,r,this.release,this,true);qx.event.Registration.removeListener(document.body,x,this.release,this,true);}
});}
)();
(function(){var a="source",b="element",c="name",d="qx.event.type.Event",f="iframe",g="qx.html.Iframe",h="navigate",i="qx.event.type.Data";qx.Class.define(g,{extend:qx.html.Element,construct:function(j,k,l){qx.html.Element.call(this,f,k,l);this.setSource(j);this.addListener(h,this.__wE,this);qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(b);}
,events:{"load":d,"navigate":i},members:{_applyProperty:function(name,n){qx.html.Element.prototype._applyProperty.call(this,name,n);if(name==a){var o=this.getDomElement();var m=qx.bom.Iframe.queryCurrentUrl(o);if(n===m){return;}
;qx.bom.Iframe.setSource(o,n);}
;}
...
,getSource:function(){return this._getProperty(a);}
,setName:function(name){this.setAttribute(c,name);return this;}
,getName:function(){return this.getAttribute(c);}
,reload:function(){var u=this.getDomElement();if(u){var t=this.getSource();this.setSource(null);this.setSource(t);}
;}
,__wE:function(e){var v=e.getData();if(v){this.setSource(v);}
;}
}});}
)();
