...
;this.__handleRollImpulse(be.velocityX,be.velocityY,Y,be.target);this.__fireTrack(z,Y,be.target);if(be.isTap){if(ba!==be.target){delete this.__gesture[Y.pointerId];return;}
;this._fireEvent(Y,B,Y.target||ba);if(Object.keys(this.__lastTap).length>0){var bc=Date.now()-qx.event.handler.GestureCore.DOUBLETAP_TIME;for(var bd in this.__lastTap){if(bd<bc){delete this.__lastTap[bd];}
...
;}
,updateGestureTarget:function(bh,bi){this.__gesture[bh].target=bi;}
,__handleRollImpulse:function(bn,bo,bk,bl,bq){var bp=bk.timeoutId;if((Math.abs(bo)<1&&Math.abs(bn)<1)||this.__stopMomentum[bp]){delete this.__stopMomentum[bp];return;}
;if(!bq){bq=1;var bm=2.8;bo=bo/bm;bn=bn/bm;}
;bq+=0.0006;bo=bo/bq;bn=bn/bq;var bj=qx.bom.AnimationFrame.request(qx.lang.Function.bind(function(br,bs,bt,bu,bv){this.__handleRollImpulse(br,bs,bt,bu,bv);}
...
,_calcAngle:function(){var bx=null;var by=null;for(var bw in this.__gesture){var bz=this.__gesture[bw];if(bx===null){bx=bz;}
else {by=bz;}
;}
;var x=bx.clientX-by.clientX;var y=bx.clientY-by.clientY;return (360+Math.atan2(y,x)*(180/Math.PI))%360;}
,_calcDistance:function(){var bA=null;var bB=null;for(var bD in this.__gesture){var bE=this.__gesture[bD];if(bA===null){bA=bE;}
else {bB=bE;}
;}
...
,__isBelowDoubleTapDistance:function(bI,bM,bN,bO,bP){var bL=qx.event.handler.GestureCore;var bJ=Math.abs(bI-bN)<bL.TAP_MAX_DISTANCE[bP];var bK=Math.abs(bM-bO)<bL.TAP_MAX_DISTANCE[bP];return bJ&&bK;}
,_getDeltaCoordinates:function(bS){var bT=this.__gesture[bS.pointerId];if(!bT){return null;}
;var bQ=bS.clientX-bT.startX;var bR=bS.clientY-bT.startY;var bU=E;if(Math.abs(bQ/bR)<1){bU=n;}
;return {"x":bQ,"y":bR,"axis":bU};}
,_fireEvent:function(bW,bY,bX){if(!this.__defaultTarget){return;}
...
else if(this.__emitter){bV=new qx.event.type.dom.Custom(bY,bW,{target:this.__defaultTarget,currentTarget:this.__defaultTarget,srcElement:this.__defaultTarget,swipe:bW.swipe,scale:bW.scale,angle:bW.angle,delta:bW.delta,pointerType:bW.pointerType,momentum:bW.momentum});this.__emitter.emit(bY,bW);}
;}
,_onDblClick:function(ca){var cb=qx.bom.Event.getTarget(ca);this._fireEvent(ca,B,cb);this._fireEvent(ca,p,cb);}
,__getSwipeGesture:function(ce,cf){var cl=this.__gesture[ce.pointerId];if(!cl){return null;}
...
,__fireTrack:function(cn,co,cp){co.delta=this._getDeltaCoordinates(co);this._fireEvent(co,cn,co.target||cp);}
...
else {var ct=this.__gesture[cr.pointerId];cr.delta={x:-ct.velocityX,y:-ct.velocityY,axis:Math.abs(ct.velocityX/ct.velocityY)<1?n:E};}
;this._fireEvent(cr,h,cr.target||cs);}
,__fireRotate:function(cu,cw){if(!cu.isPrimary){var cv=this._calcAngle();cu.angle=Math.round((cv-this.__initialAngle)%360);this._fireEvent(cu,k,this.__primaryTarget);}
;}
,__firePinch:function(cz,cA){if(!cz.isPrimary){var cx=this._calcDistance();var cy=cx/this.__initialDistance;cz.scale=(Math.round(cy*100)/100);this._fireEvent(cz,b,this.__primaryTarget);}
;}
,__fireLongTap:function(cB,cC){var cD=this.__gesture[cB.pointerId];if(cD){this._fireEvent(cB,f,cB.target||cC);cD.longTapTimer=null;cD.isTap=false;}
;}
,__stopLongTapTimer:function(cE){if(cE.longTapTimer){window.clearTimeout(cE.longTapTimer);cE.longTapTimer=null;}
;}
...
,dispose:function(){for(var cH in this.__gesture){this.__stopLongTapTimer(cH);}
