...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.root.Inline_Dynamic_Resize.8cccc36d0fff.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="resize",l="qx.ui.root.Page",k="mshtml",j="gecko",i="paddingLeft",h="$$widget",g="left",f="paddingTop",d="qxIsRootPage",c="absolute",a="engine.name",b="div";qx.Class.define(l,{extend:qx.ui.root.Abstract,construct:function(n){this.__lP=n;qx.ui.root.Abstract.call(this);this._setLayout(new qx.ui.layout.Basic());this.setZIndex(10000);qx.ui.core.queue.Layout.add(this);this.addListener(m,this.__lr,this);qx.ui.core.FocusHandler.getInstance().connectTo(this);if((qx.core.Environment.get(a)==k)){this.setKeepFocus(true);}
;}
,members:{__ks:null,__lP:null,_createContainerElement:function(){var p=this.__lP.createElement(b);this.__lP.body.appendChild(p);var o=new qx.html.Root(p);o.setStyles({position:c,textAlign:g});o.setAttribute(h,this.toHashCode());if((qx.core.Environment.get(a)==j)){o.setAttribute(d,1);}
;return o;}
,_createContentElement:function(){return new qx.html.Element(b);}
...
,__lr:function(e){this.getContainerElement().setStyles({width:0,height:0});this.getContentElement().setStyles({width:0,height:0});}
,supportsMaximize:function(){return false;}
,_applyPadding:function(s,t,name){if(s&&(name==f||name==i)){throw new Error("The root widget does not support 'left', or 'top' paddings!");}
;qx.ui.root.Abstract.prototype._applyPadding.call(this,s,t,name);}
,_applyDecorator:function(u,v){qx.ui.root.Abstract.prototype._applyDecorator.call(this,u,v);if(!u){return;}
;var w=this.getDecoratorElement().getInsets();if(w.left||w.top){throw new Error("The root widget does not support decorators with 'left', or 'top' insets!");}
;}
},destruct:function(){this.__lP=null;}
});}
)();
...
(function(){var t="widget",s="drive-optical.png",r="computer.png",q="audio-card.png",p="battery.png",o="isle1",n="input-keyboard.png",m="icon/48/devices/",l="demobrowser.demo.root.Inline_Dynamic_Resize",k="isle2",d="drive-harddisk.png",j="camera-web.png",g="network-wired.png",c="icon/48/devices/computer.png",b="camera-photo.png",f="network-wireless.png",e="audio-input-microphone.png",h="display.png",a="main";qx.Class.define(l,{extend:qx.application.Inline,members:{main:function(){qx.application.Inline.prototype.main.call(this);var x=new qx.ui.root.Inline(document.getElementById(o),false,true);x.setLayout(new qx.ui.layout.Canvas());x.add(new qx.ui.form.Button(null,c),{edge:0});var v=new qx.ui.root.Inline(document.getElementById(k),true,false).set({padding:[10,0],decorator:null,appearance:t});v.setLayout(new qx.ui.layout.Canvas());var w=new qx.ui.container.SlideBar();w.setLayout(new qx.ui.layout.HBox(3));v.add(w,{edge:0});var u=[q,e,p,b,j,r,h,d,s,n,g,f];for(var i=0;i<u.length;i++ ){w.add((new qx.ui.basic.Image(m+u[i])).set({decorator:a,padding:4}));}
;}
}});}
)();
(function(){var t="",s="px",r="position",q="qx.debug",p="engine.version",o="qx.ui.root.Inline",n="$$widget",m="hidden",l="div",k="left",c="appear",h="Please use a DOM element to create an inline root.",g="mshtml",b="engine.name",a="height",f="resize",d="relative";qx.Class.define(o,{extend:qx.ui.root.Abstract,include:[qx.ui.core.MLayoutHandling],construct:function(u,v,w){if(qx.core.Environment.get(q)){this.assertElement(u,h);}
;this.__ob=u;u.style.overflow=m;u.style.textAlign=k;this.__oc=v||false;this.__od=w||false;this.__oe();qx.ui.root.Abstract.call(this);this._setLayout(new qx.ui.layout.Basic());qx.ui.core.queue.Layout.add(this);qx.ui.core.FocusHandler.getInstance().connectTo(this);if((qx.core.Environment.get(b)==g)){this.setKeepFocus(true);}
;var window=qx.dom.Node.getWindow(u);qx.event.Registration.addListener(window,f,this._onWindowResize,this);}
,members:{__oc:false,__od:false,__ob:null,__oe:function(){if(this.__oc||this.__od){var x=qx.bom.element.Dimension.getSize(this.__ob);if(this.__oc&&x.width<1){throw new Error("The root element "+this.__ob+" of "+this+" needs a width when its width size should be used!");}
;if(this.__od){if(x.height<1){throw new Error("The root element "+this.__ob+" of "+this+" needs a height when its height size should be used!");}
;if(x.height>=1&&qx.bom.element.Style.get(this.__ob,a,3)==t){qx.bom.element.Style.set(this.__ob,a,x.height+s);}
;}
;qx.event.Registration.addListener(this.__ob,f,this._onResize,this);}
;}
,_createContainerElement:function(){var y=this.__ob;if(this.__oc||this.__od){var C=document.createElement(l);y.appendChild(C);if((qx.core.Environment.get(b)==g)&&qx.core.Environment.get(p)==6){var B=qx.dom.Node.getBodyElement(y);var A;var E;var D=false;var z=qx.dom.Hierarchy.getAncestors(y);for(var i=0,j=z.length;i<j;i++ ){A=z[i];if(A!=B){E=qx.bom.element.Style.get(A,r);if(E==d){D=true;break;}
;}
else {break;}
;}
;if(D){y.style.position=d;}
;}
;}
else {C=y;}
;var F=new qx.html.Root(C);C.style.position=d;F.setAttribute(n,this.toHashCode());qx.event.Timer.once(function(e){this.fireEvent(c);}
,this,0);return F;}
,_onResize:function(e){var G=e.getData();if((G.oldWidth!==G.width)&&this.__oc||(G.oldHeight!==G.height)&&this.__od){qx.ui.core.queue.Layout.add(this);}
;}
,_onWindowResize:function(){if(qx.ui.popup&&qx.ui.popup.Manager){qx.ui.popup.Manager.getInstance().hideAll();}
;if(qx.ui.menu&&qx.ui.menu.Manager){qx.ui.menu.Manager.getInstance().hideAll();}
;}
,_computeSizeHint:function(){var L=this.__oc;var I=this.__od;if(!L||!I){var H=qx.ui.root.Abstract.prototype._computeSizeHint.call(this);}
else {H={};}
;var M=qx.bom.element.Dimension;if(L){var K=M.getContentWidth(this.__ob);H.width=K;H.minWidth=K;H.maxWidth=K;}
;if(I){var J=M.getContentHeight(this.__ob);H.height=J;H.minHeight=J;H.maxHeight=J;}
;return H;}
},defer:function(N,O){qx.ui.core.MLayoutHandling.remap(O);}
,destruct:function(){qx.event.Registration.removeListener(this.__ob,f,this._onResize,this);this.__ob=null;}
});}
)();
