...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.TreeController.2551f5ef1d64.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var s="Change the label binding",r="Write data to log",q="Remove an item from root",p="'Thing i'.",o="A list containing the selected items of the tree.",n="multi",m="<b>Tree binding</b><br/>",l="Add an item to root",k="<b>Selected Items</b><br/>",j="This tree is bound to a set of randomly generated nodes. Every node",c="children",h="demobrowser.demo.data.TreeController",f=" does have a name and a name2 property, containing 'Item i' and ",b="name2",a="Thing ",e="Item ",d="name",g="execute";qx.Class.define(h,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var z=[];for(var i=0;i<50;i++ ){z[i]=new demobrowser.demo.data.model.Node();z[i].setName(e+i);z[i].setName2(a+i);if(i!=0){z[parseInt(Math.random()*i)].getChildren().push(z[i]);}
;}
;var t=new qx.ui.tree.Tree();t.setSelectionMode(n);this.getRoot().add(t,{left:10,top:130});t.setWidth(250);t.setHeight(300);var B=new qx.data.controller.Tree(z[0],t,c,d);t.getRoot().setOpen(true);var u=new qx.ui.form.List();u.setHeight(300);this.getRoot().add(u,{left:270,top:130});new qx.data.controller.List(B.getSelection(),u,b);var A=new qx.ui.form.Button(l);A.setWidth(180);this.getRoot().add(A,{left:390,top:130});A.addListener(g,function(){var D=new demobrowser.demo.data.model.Node();D.setName(e+t.getItems(true).length);D.setName2(a+t.getItems(true).length);z[0].getChildren().push(D);}
,this);var v=new qx.ui.form.Button(q);v.setWidth(180);this.getRoot().add(v,{left:390,top:165});v.addListener(g,function(){z[0].getChildren().pop();}
,this);var w=new qx.ui.form.Button(s);w.setWidth(180);this.getRoot().add(w,{left:390,top:200});w.addListener(g,function(){if(B.getLabelPath()==d){B.setLabelPath(b);}
else {B.setLabelPath(d);}
;}
,this);var x=new qx.ui.form.Button(r);x.setWidth(180);this.getRoot().add(x,{left:390,top:235});x.addListener(g,function(){this.info(z[0].toString());}
,this);var y=new qx.ui.basic.Label();y.setRich(true);y.setWidth(240);y.setValue(m+j+f+p);this.getRoot().add(y,{left:20,top:10});var C=new qx.ui.basic.Label();C.setRich(true);C.setWidth(100);C.setValue(k+o);this.getRoot().add(C,{left:270,top:10});}
}});}
)();
...
(function(){var q="changeNames",p="changeName",o="- ",n="",m="changeChild",l="changeColor",k="Marge",j="AFFE",h="Homer",g="\n",c="__QX",f="changeName2",e="changeChildren",b="qx.data.Array",a="demobrowser.demo.data.model.Node",d="String";qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.setChildren(new qx.data.Array());}
,properties:{children:{check:b,event:e},child:{check:a,event:m,nullable:true},names:{check:b,event:q,init:new qx.data.Array(h,k)},name:{check:d,init:j,event:p},name2:{check:d,event:f},number:{init:10,validate:c},color:{event:l,nullable:true}},members:{__QX:function(r){if(!isNaN(parseFloat(r))){if(/^\d+$/.test(r)){return;}
;}
;throw new qx.core.ValidationError("Validation Error: "+r+"is no number (parseFloat says so!).");}
,toString:function(s){if(s==undefined){s=0;}
;var t=n;for(var i=0;i<s;i++ ){t+=o;}
;t+=this.getName();for(var i=0;i<this.getChildren().length;i++ ){t+=g+this.getChildren().getItem(i).toString(s+1);}
;return t;}
}});}
)();
...
(function(){var m="open",k="__gk",j="abstract",h="__oH",g="addItem",f="removeItem",e="Invalid child index: ",d="visible",c="excluded",b="qx.ui.tree.core.AbstractTreeItem",a="qx.debug";qx.Class.define(b,{extend:qx.ui.tree.core.AbstractItem,type:j,construct:function(n){qx.ui.tree.core.AbstractItem.call(this,n);this.__gk=[];}
,properties:{parent:{check:b,nullable:true}},members:{__gk:null,__oH:null,getTree:function(){var p=this;while(p.getParent()){p=p.getParent();}
...
;}
;}
,getChildrenContainer:function(){if(!this.__oH){this.__oH=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?d:c});}
;return this.__oH;}
,hasChildrenContainer:function(){return this.__oH;}
...
;}
,__Hs:function(){if(this.getParentChildrenContainer()){this.getParentChildrenContainer()._addAfter(this.getChildrenContainer(),this);}
;}
...
;this.__gk.push(P);if(!M){this.__Hs();}
;if(O){P.recursiveAddToWidgetQueue();O.fireNonBubblingEvent(g,qx.event.type.Data,[P]);}
;}
;if(O){qx.ui.core.queue.Widget.add(this);}
;}
,addAt:function(Q,R){if(qx.core.Environment.get(a)){this.assert(R<=this.__gk.length&&R>=0,e+R);}
;if(R==this.__gk.length){this.add(Q);return;}
;var V=Q.getParent();if(V){V.remove(Q);}
;var T=this.getChildrenContainer();Q.setParent(this);var U=this.hasChildren();var S=this.__gk[R];T.addBefore(Q,S);if(Q.hasChildren()){T.addAfter(Q.getChildrenContainer(),Q);}
;qx.lang.Array.insertAt(this.__gk,Q,R);if(!U){this.__Hs();}
;if(this.getTree()){Q.recursiveAddToWidgetQueue();qx.ui.core.queue.Widget.add(this);}
;}
,addBefore:function(W,X){if(qx.core.Environment.get(a)){this.assert(this.__gk.indexOf(X)>=0);}
;var Y=W.getParent();if(Y){Y.remove(W);}
;this.addAt(W,this.__gk.indexOf(X));}
,addAfter:function(ba,bb){if(qx.core.Environment.get(a)){this.assert(this.__gk.indexOf(bb)>=0);}
;var bc=ba.getParent();if(bc){bc.remove(ba);}
;this.addAt(ba,this.__gk.indexOf(bb)+1);}
,addAtBegin:function(bd){this.addAt(bd,0);}
,remove:function(be){for(var i=0,l=arguments.length;i<l;i++ ){var bi=arguments[i];if(this.__gk.indexOf(bi)==-1){this.warn("Cannot remove treeitem '"+bi+"'. It is not a child of this tree item.");return;}
...
;}
;qx.lang.Array.remove(this.__gk,bi);bi.setParent(null);bf.remove(bi);}
;var bg=this.getTree();if(bg){bg.fireNonBubblingEvent(f,qx.event.type.Data,[bi]);}
;qx.ui.core.queue.Widget.add(this);}
,removeAt:function(bj){var bk=this.__gk[bj];if(bk){this.remove(bk);}
;}
,removeAll:function(){var bl=this.__gk.concat();for(var i=this.__gk.length-1;i>=0;i-- ){this.remove(this.__gk[i]);}
;return bl;}
},destruct:function(){this._disposeArray(k);this._disposeObjects(h);}
});}
)();
...
(function(){var l="_applyOpenMode",k="Space",j="Left",h="Enter",g="changeOpenMode",f="_applyRootOpenClose",d="changeSelection",c="qx.ui.tree.Tree",b="qx.ui.tree.core.AbstractTreeItem",a="tree",z="_applyHideRoot",y="changeRoot",x="_applyRoot",w="__oM",v="keypress",u="none",t="pane",s="Right",r="Boolean",q="excluded",o="visible",p="qx.event.type.Data",m="click",n="dblclick";qx.Class.define(c,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IModelSelection,qx.ui.form.IForm],include:[qx.ui.core.MMultiSelectionHandling,qx.ui.core.MContentPadding,qx.ui.form.MModelSelection,qx.ui.form.MForm],construct:function(){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__oM=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({allowShrinkY:false,allowGrowX:true});this.getChildControl(t).add(this.__oM);this.initOpenMode();this.initRootOpenClose();this.addListener(d,this._onChangeSelection,this);this.addListener(v,this._onKeyPress,this);}
,events:{addItem:p,removeItem:p},properties:{openMode:{check:[m,n,u],init:n,apply:l,event:g,themeable:true},root:{check:b,init:null,nullable:true,event:y,apply:x},hideRoot:{check:r,init:false,apply:z},rootOpenClose:{check:r,init:false,apply:f},appearance:{refine:true,init:a},focusable:{refine:true,init:true}},members:{__oM:null,SELECTION_MANAGER:qx.ui.tree.selection.SelectionManager,getChildrenContainer:function(){return this.__oM;}
...
,_getContentPaddingTarget:function(){return this.__oM;}
...
(function(){var g="change",f="qx.data.Array",e="qx.data.controller.MSelection",d="_applySelection",c="target",b="qx.event.type.Data",a="changeSelection";qx.Mixin.define(e,{construct:function(){if(!qx.Class.hasProperty(this.constructor,c)){throw new Error("Target property is needed.");}
;if(this.getSelection()==null){this.__EK=new qx.data.Array();this.setSelection(this.__EK);}
;}
,properties:{selection:{check:f,event:a,apply:d,init:null}},events:{"changeSelection":b},members:{_modifingSelection:0,__EL:null,__EM:null,__EK:null,_applySelection:function(h,j){if(this.__EM!=undefined&&j!=undefined){j.removeListenerById(this.__EM);}
;this.__EM=h.addListener(g,this.__EN,this);this._updateSelection();}
,__EN:function(){this._updateSelection();}
,_changeTargetSelection:function(){if(this.getTarget()==null){return;}
;if(!this.__EO()&&!this.__EP()){return;}
;if(this._inSelectionModification()){return;}
;var l=this.getTarget().getSelection();var k=this.getSelection();if(k==null){k=new qx.data.Array();this.__EK=k;this.setSelection(k);}
...
,_addChangeTargetListener:function(n,o){if(this.__EL!=undefined&&o!=undefined){o.removeListenerById(this.__EL);}
;if(n!=null){if(this.__EO()||this.__EP()){this.__EL=n.addListener(a,this._changeTargetSelection,this);}
;}
;}
,_updateSelection:function(){if(!this.getTarget()){return;}
;this._startSelectionModification();if(this.__EO()){var r=[];for(var i=0;i<this.getSelection().length;i++ ){var s=this.getSelection().getItem(i);var t=this.__ER(s);if(t!=null){r.push(t);}
;}
...
;}
;}
else if(this.__EP()){var q=this.getSelection().getItem(this.getSelection().length-1);if(q!==undefined){this.__EQ(q);this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();}
else {this.getTarget().resetSelection();}
;}
;this._endSelectionModification();}
,__EO:function(){var u=this.getTarget().constructor;return qx.Class.implementsInterface(u,qx.ui.core.IMultiSelection);}
,__EP:function(){var v=this.getTarget().constructor;return qx.Class.implementsInterface(v,qx.ui.core.ISingleSelection);}
,__EQ:function(w){var x=this.__ER(w);if(x==null){return;}
;if(this.__EO()){this.getTarget().addToSelection(x);}
else if(this.__EP()){this.getTarget().setSelection([x]);}
;}
,__ER:function(y){var z=this.getTarget().getSelectables(true);for(var i=0;i<z.length;i++ ){if(z[i].getModel()==y){return z[i];}
;}
;return null;}
...
},destruct:function(){if(this.__EK){this.__EK.dispose();}
;}
});}
)();
...
(function(){var t="changeModel",s="_applyDelegate",r="qx.core.Object",q="_applyLabelPath",p="icon",o="change",n="label",m="_applyTarget",l="changeTarget",k="_applyIconPath",c="_applyLabelOptions",h="_applyChildPath",g="_applyIconOptions",b="qx.data.controller.Tree",a="_applyModel",f="String",d="get";qx.Class.define(b,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(u,v,w,x){qx.core.Object.call(this);this.__bg={};this.__ES=[];this.__Jq={};if(w!=null){this.setChildPath(w);}
;if(x!=null){this.setLabelPath(x);}
;if(u!=null){this.setModel(u);}
;if(v!=null){this.setTarget(v);}
;}
,properties:{model:{check:r,apply:a,event:t,nullable:true,dereference:true},target:{apply:m,event:l,init:null,nullable:true,dereference:true},childPath:{check:f,apply:h,nullable:true},labelPath:{check:f,apply:q,nullable:true},iconPath:{check:f,apply:k,nullable:true},labelOptions:{apply:c,nullable:true},iconOptions:{apply:g,nullable:true},delegate:{apply:s,init:null,nullable:true}},members:{__Jq:null,__bg:null,__ES:null,__Jr:null,_applyDelegate:function(y,z){this._setConfigureItem(y,z);this._setCreateItem(y,z);this._setBindItem(y,z);}
,_applyIconOptions:function(A,B){this.__Fd();}
,_applyLabelOptions:function(C,D){this.__Fd();}
,_applyTarget:function(E,F){if(F!=undefined){this.__yp(F);}
;if(this.getModel()!=null){this.__Ju();}
;this._addChangeTargetListener(E,F);}
,_applyModel:function(G,H){this.__Ju();}
,_applyChildPath:function(I,J){this.__Jr=J;this.__Ju();this.__Jr=null;}
,_applyIconPath:function(K,L){this.__Fd();}
,_applyLabelPath:function(M,N){this.__Ju();}
,__Js:function(O){var Q=O.getTarget();var P=this.__Jq[Q.toHashCode()].treeNode;var R=this.__Jq[Q.toHashCode()].modelNode;this.__Jv(P,R);this._updateSelection();}
,__Jt:function(e){var Y=e.getData();var U=e.getOldData();var V=this.__Jq[U.toHashCode()];U.removeListenerById(V.changeListenerId);delete this.__Jq[U.toHashCode()];V.modelNode.removeListenerById(V.changeChildernListenerId);var ba=V.modelNode;var X=qx.Class.getPropertyDefinition(V.modelNode.constructor,this.getChildPath());var S=X.event;var W=ba.addListener(S,this.__Jt,this);var T=V.treeNode;this.__Jq[Y.toHashCode()]={modelNode:ba,treeNode:T,changeListenerId:V.changeListenerId,changeChildernListenerId:W};this.__Jv(T,ba);this._updateSelection();}
,_createItem:function(){var bc=this.getDelegate();if(bc!=null&&bc.createItem!=null){var bb=bc.createItem();}
else {var bb=new qx.ui.tree.TreeFolder();}
;if(bc!=null&&bc.configureItem!=null){bc.configureItem(bb);}
;return bb;}
,__Ju:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__yp();if(this.getModel()!=null){var bd=this._createItem();bd.setModel(this.getModel());this.getTarget().setRoot(bd);this.__xf(this.getModel(),bd);this.__Jv(bd,this.getModel());}
;}
,__Jv:function(be,bf){if(bf[d+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var bo=bf[d+qx.lang.String.firstUp(this.getChildPath())]();if(this.__Jq[bo.toHashCode()]==undefined){var bj=bo.addListener(o,this.__Js,this);var bn=qx.Class.getPropertyDefinition(bf.constructor,this.getChildPath());var bk=bn.event;var bl=bf.addListener(bk,this.__Jt,this);this.__Jq[bo.toHashCode()]={modelNode:bf,treeNode:be,changeListenerId:bj,changeChildernListenerId:bl};}
;for(var i=0;i<bo.length;i++ ){if(be.getChildren()[i]==null||bo.getItem(i)!=be.getChildren()[i].getModel()){for(var j=i;j<be.getChildren().length;j++ ){if(be.getChildren()[j].getModel()===bo.getItem(i)){var bg=j;break;}
;}
;if(bg!=undefined){var bh=be.getChildren()[bg];if(this.getTarget().isSelected(bh)){var bp=true;}
;be.removeAt(bg);be.addAt(bh,i);if(bp){this.getTarget().addToSelection(bh);}
;}
else {var bm=this._createItem();bm.setModel(bo.getItem(i));be.addAt(bm,i);this.__xf(bo.getItem(i),bm);this.__Jv(bm,bo.getItem(i));}
;}
;}
;for(var i=be.getChildren().length-1;i>=bo.length;i-- ){var bi=be.getChildren()[i];this.__Jx(bi,be);}
;}
,__yp:function(bq){if(bq==null){bq=this.getTarget();}
;if(bq==null){return;}
;var bs=bq.getRoot();if(bs!=null){bq.setRoot(null);this.__Jw(bs);var br=bs.getModel();if(br){this.__Jy(br);}
;bs.destroy();this.__Jq={};}
;}
,__Jw:function(bt){var bu=bt.getChildren()||[];for(var i=bu.length-1;i>=0;i-- ){if(bu[i].getChildren().length>0){this.__Jw(bu[i]);}
;this.__Jx(bu[i],bt);}
;}
,__Jx:function(bv,bw){var by=bv.getModel();var bA=this.__Jr||this.getChildPath();var bB=d+qx.lang.String.firstUp(bA);if(by[bB]!=undefined){var bz=by[bB]();var bx=this.__Jq[bz.toHashCode()];bz.removeListenerById(bx.changeListenerId);by.removeListenerById(bx.changeChildernListenerId);this.__Jw(bv);delete this.__Jq[bz.toHashCode()];}
;this.__Jy(by);bw.remove(bv);bv.destroy();}
,bindProperty:function(bC,bD,bE,bF,bG){var bI=bG.bind(bC,bF,bD,bE);if(this.__bg[bD]==null){this.__bg[bD]={};}
;var bH=this.__bg[bD];if(bH[bG.toHashCode()]){if(bH[bG.toHashCode()].id){throw new Error("Can not bind the same target property '"+bD+"' twice.");}
;bH[bG.toHashCode()].id=bI;}
else {bH[bG.toHashCode()]={id:bI,reverseId:null,treeNode:bF};}
;if(!qx.lang.Array.contains(this.__ES,bD)){this.__ES.push(bD);}
;}
,bindPropertyReverse:function(bJ,bK,bL,bM,bN){var bP=bM.bind(bK,bN,bJ,bL);if(this.__bg[bK]==null){this.__bg[bK]={};}
;var bO=this.__bg[bK];if(bO[bN.toHashCode()]){if(bO[bN.toHashCode()].reverseId){throw new Error("Can not reverse bind the same target property '"+bJ+"' twice.");}
;bO[bN.toHashCode()].reverseId=bP;}
else {bO[bN.toHashCode()]={id:null,reverseId:bP,treeNode:bM};}
;if(!qx.lang.Array.contains(this.__ES,bK)){this.__ES.push(bK);}
;}
,bindDefaultProperties:function(bQ,bR){this.bindProperty(this.getLabelPath(),n,this.getLabelOptions(),bQ,bR);if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),p,this.getIconOptions(),bQ,bR);}
;}
,__Fd:function(){var bT;for(var bV in this.__bg){bT=bV;break;}
;for(var bS in this.__bg[bT]){var bW=this.__bg[bT][bS].treeNode;var bU=qx.core.ObjectRegistry.fromHashCode(bS);this.__Jy(bU);this.__xf(bU,bW);}
;}
,__xf:function(bX,bY){var ca=this.getDelegate();if(ca!=null&&ca.bindItem!=null){ca.bindItem(this,bY,bX);}
else {this.bindDefaultProperties(bY,bX);}
;}
,__Jy:function(cb){for(var i=0;i<this.__ES.length;i++ ){var cd=this.__ES[i];var cc=this.__bg[cd][cb.toHashCode()];if(cc!=null){if(cc.id){cb.removeBinding(cc.id);cc.id=null;}
;if(cc.reverseId){cc.treeNode.removeBinding(cc.reverseId);cc.reverseId=null;}
;delete this.__bg[cd][cb.toHashCode()];}
;}
;}
,_setConfigureItem:function(ce,cf){if(ce!=null&&ce.configureItem!=null&&this.getTarget()!=null&&this.getModel()!=null){var cg=this.getTarget().getRoot().getItems(true,true,false);for(var i=0;i<cg.length;i++ ){ce.configureItem(cg[i]);}
;}
;}
,_setCreateItem:function(ch,ci){if(this.getTarget()==null||this.getModel()==null){return;}
;if(ch==null||ch.createItem==null){return;}
;if(ci&&ci.createItem&&ch&&ch.createItem&&ci.createItem==ch.createItem){return;}
;this._startSelectionModification();this.__yp();this.__Ju();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cj,ck){if(cj!=null&&cj.bindItem!=null){if(ck!=null&&ck.bindItem!=null&&cj.bindItem==ck.bindItem){return;}
;this.__Ju();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__bg=this.__Jq=this.__ES=null;}
});}
)();
...
(function(){var m="qx.ui.form.List",k="text",j="Boolean",h="one",g="addChildWidget",f="_applySpacing",d="Enter",c="Integer",b="action",a="keyinput",A="addItem",z="removeChildWidget",y="_applyOrientation",x="single",w="__oM",v="keypress",u="list",t="label",s="pane",r="removeItem",p="qx.event.type.Data",q="vertical",n="",o="horizontal";qx.Class.define(m,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MMultiSelectionHandling,qx.ui.form.MForm,qx.ui.form.MModelSelection],construct:function(B){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__oM=this._createListItemContainer();this.__oM.addListener(g,this._onAddChild,this);this.__oM.addListener(z,this._onRemoveChild,this);this.getChildControl(s).add(this.__oM);if(B){this.setOrientation(o);}
else {this.initOrientation();}
;this.addListener(v,this._onKeyPress);this.addListener(a,this._onKeyInput);this.__sb=n;}
,events:{addItem:p,removeItem:p},properties:{appearance:{refine:true,init:u},focusable:{refine:true,init:true},orientation:{check:[o,q],init:q,apply:y},spacing:{check:c,init:0,apply:f,themeable:true},enableInlineFind:{check:j,init:true}},members:{__sb:null,__sc:null,__oM:null,SELECTION_MANAGER:qx.ui.core.selection.ScrollArea,getChildrenContainer:function(){return this.__oM;}
...
,_applyOrientation:function(C,D){var E=C===o;var F=E?new qx.ui.layout.HBox():new qx.ui.layout.VBox();var content=this.__oM;content.setLayout(F);content.setAllowGrowX(!E);content.setAllowGrowY(E);this._applySpacing(this.getSpacing());}
,_applySpacing:function(G,H){this.__oM.getLayout().setSpacing(G);}
...
;var J=this.getSelectionMode();if(!(J===x||J===h)){return;}
;if(((new Date).valueOf()-this.__sc)>1000){this.__sb=n;}
;this.__sb+=e.getChar();var K=this.findItemByLabelFuzzy(this.__sb);if(K){this.setSelection([K]);}
;this.__sc=(new Date).valueOf();}
...
(function(){var m="changeModel",k="_applyLabelOptions",h="_applyLabelPath",g="changeTarget",f="_applyModel",e="icon",d="qx.data.controller.List",c="_applyIconPath",b="_applyDelegate",a="changeDelegate",z="_applyTarget",y="qx.data.IListData",x="model",w="label",v="_applyIconOptions",u="change",t="ReverseBindingId",s="BindingId",r="]",q="model[",o="String",p=".",n="";qx.Class.define(d,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(A,B,C){qx.core.Object.call(this);this.__xR=[];this.__ES=[];this.__ET=[];this.__EU={};if(C!=null){this.setLabelPath(C);}
;if(A!=null){this.setModel(A);}
;if(B!=null){this.setTarget(B);}
;}
,properties:{model:{check:y,apply:f,event:m,nullable:true,dereference:true},target:{apply:z,event:g,nullable:true,init:null,dereference:true},labelPath:{check:o,apply:h,nullable:true},iconPath:{check:o,apply:c,nullable:true},labelOptions:{apply:k,nullable:true},iconOptions:{apply:v,nullable:true},delegate:{apply:b,event:a,init:null,nullable:true}},members:{__yn:null,__xR:null,__EU:null,__ES:null,__ET:null,__EV:null,__EW:null,update:function(){this.__EY();this.__Fd();this._updateSelection();}
,_applyDelegate:function(D,E){this._setConfigureItem(D,E);this._setFilter(D,E);this._setCreateItem(D,E);this._setBindItem(D,E);}
,_applyIconOptions:function(F,G){this.__Fd();}
,_applyLabelOptions:function(H,I){this.__Fd();}
,_applyIconPath:function(J,K){this.__Fd();}
,_applyLabelPath:function(L,M){this.__Fd();}
,_applyModel:function(N,O){if(O!=undefined){if(this.__yn!=undefined){O.removeListenerById(this.__yn);}
;}
;if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();}
;if(N!=null){this.__yn=N.addListener(u,this.__EX,this);this.__xY();this.__EY();if(O==null){this._changeTargetSelection();}
else {this.__EV=true;qx.ui.core.queue.Widget.add(this);}
;}
else {var P=this.getTarget();if(P!=null){var length=P.getChildren().length;for(var i=0;i<length;i++ ){this.__Fc();}
;}
;}
;}
...
;if(Q!=null){if(this.getModel()!=null){for(var i=0;i<this.__xR.length;i++ ){this.__Fb(this.__Ff(i));}
;}
;}
;}
,__EX:function(){this.__EW=true;qx.ui.core.queue.Widget.add(this);if(this.__xR.length!=this.getModel().getLength()){this.update();}
;}
,syncWidget:function(){if(this.__EV){this._changeTargetSelection();}
;if(this.__EW){this._updateSelection();}
;this.__EW=this.__EV=null;}
,__EY:function(){if(this.getTarget()==null){return;}
;this.__xY();var U=this.__xR.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++ ){this.__Fb(this.__Ff(j));}
;}
else if(U<T){for(var j=T;j>U;j-- ){this.__Fc();}
;}
;this.__EV=true;qx.ui.core.queue.Widget.add(this);}
,__Fa:function(){var V=this.getModel();if(V!=null){V.removeListenerById(this.__yn);this.__yn=V.addListener(u,this.__EX,this);}
;}
...
;return W;}
,__Fb:function(Y){var ba=this._createItem();this._bindListItem(ba,Y);this.getTarget().add(ba);}
,__Fc:function(){this._startSelectionModification();var bc=this.getTarget().getChildren();var bb=bc.length-1;var bd=bc[bb];this._removeBindingsFrom(bd);this.getTarget().removeAt(bb);bd.destroy();this._endSelectionModification();}
...
;}
,bindProperty:function(bm,bn,bo,bp,bq){if(bo!=null){var bo=qx.lang.Object.clone(bo);this.__EU[bn]=bo.onUpdate;delete bo.onUpdate;}
else {bo={};this.__EU[bn]=null;}
;bo.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,bq);var br=q+bq+r;if(bm!=null&&bm!=n){br+=p+bm;}
;var bs=this.bind(br,bp,bn,bo);bp.setUserData(bn+s,bs);if(!qx.lang.Array.contains(this.__ES,bn)){this.__ES.push(bn);}
;}
,bindPropertyReverse:function(bt,bu,bv,bw,bx){var by=q+bx+r;if(bt!=null&&bt!=n){by+=p+bt;}
;var bz=bw.bind(bu,this,by,bv);bw.setUserData(bt+t,bz);if(!qx.lang.Array.contains(this.__ET,bt)){this.__ET.push(bt);}
;}
,_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;}
;for(var i=0;i<this.__ES.length;i++ ){if(this.__EU[this.__ES[i]]!=null){this.__EU[this.__ES[i]]();}
;}
;}
,_removeBindingsFrom:function(bD){for(var i=0;i<this.__ES.length;i++ ){var bE=bD.getUserData(this.__ES[i]+s);if(bE!=null){this.removeBinding(bE);}
;}
;for(var i=0;i<this.__ET.length;i++ ){var bE=bD.getUserData(this.__ET[i]+t);if(bE!=null){bD.removeBinding(bE);}
;}
;}
,__Fd:function(){if(this.getTarget()==null||this.getModel()==null){return;}
;var bF=this.getTarget().getChildren();for(var i=0;i<bF.length;i++ ){this._removeBindingsFrom(bF[i]);this._bindListItem(bF[i],this.__Ff(i));}
;this.__Fa();}
...
,_setFilter:function(bP,bQ){if((bP==null||bP.filter==null)&&(bQ!=null&&bQ.filter!=null)){this.__Fe();}
...
;var bS=this.__xR;this.__xY();if(bS.length>this.__xR.length){for(var j=bS.length;j>this.__xR.length;j-- ){this.getTarget().removeAt(j-1).destroy();}
;}
else if(bS.length<this.__xR.length){for(var j=bS.length;j<this.__xR.length;j++ ){var bT=this._createItem();this.getTarget().add(bT);}
;}
;var bR=this.getTarget().getChildren();for(var i=0;i<bR.length;i++ ){this._bindListItem(bR[i],this.__Ff(i));}
;this.__Fa();this._endSelectionModification();this._updateSelection();}
,__Fe:function(){this.__xY();this.__EY();this.__Fd();this.__EW=true;qx.ui.core.queue.Widget.add(this);}
,__xY:function(){var bW=this.getModel();if(bW==null){return;}
;var bX=this.getDelegate();if(bX!=null){var bV=bX.filter;}
;this.__xR=[];for(var i=0;i<bW.getLength();i++ ){if(bV==null||bV(bW.getItem(i))){this.__xR.push(i);}
;}
;}
,__Ff:function(bY){return this.__xR[bY];}
},destruct:function(){this.__xR=this.__EU=this.__ES=null;this.__ET=null;qx.ui.core.queue.Widget.remove(this);}
});}
)();
