...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.MenuBar.45fc6525a807.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var bE="Online Forum",bD="New",bC="icon/16/actions/edit-cut.png",bB="icon/16/actions/format-justify-fill.png",bA="Search in files",bz="C",by="Left aligned",bx="Undo",bw="_openCommand",bv="Uppercase",bk="View",bj="C Dialect",bi="changeSelection",bh="enabled",bg="Save as...",bf="Show line numbers",be="Search previous...",bd="Quickstart",bc="Cut",bb="Ctrl+N",bL="icon/16/actions/edit-paste.png",bM="Show console",bJ="Search",bK="Help",bH="_redoCommand",bI="Redo",bF="Show status bar",bG="icon/16/actions/format-justify-left.png",bN="ANSI to OEM",bO="Centered",bo="Paragraph",bn="Edit",bq="Ctrl+O",bp="icon/16/actions/document-print.png",bs="icon/16/actions/document-open.png",br="C Sharp",bu="icon/16/actions/edit-undo.png",bt="Close",bm="icon/16/actions/document-save-as.png",bl="Python",a="icon/16/apps/utilities-help.png",b="Show tree",c="Spaces to tabs",d="Right aligned",f="OEM to ANSI",g="JavaScript",h="demobrowser.demo.widget.MenuBar",i="_copyCommand",j="XML",k="Save",bS="_undoCommand",bR="Capitals",bQ="icon/16/actions/application-exit.png",bP="C Plus Plus",bW="_saveCommand",bV="Lowercase",bU="Search next...",bT="_newCommand",bY="Replace in files",bX="Perl",J="Ctrl+P",K="Topics",H="Panes",I="Justified",N="Ctrl+X",O="Show macros",L="Show ruler",M="Enabled",F="Copy",G="Paste",s="_pasteCommand",r="icon/16/actions/format-justify-right.png",u="Ctrl+Z",t="Ctrl+R",o="icon/16/actions/edit-redo.png",n="Show tags",q="Info...",p="Format",m="File",l="Show tabs",T="value",U="Print",V="icon/16/actions/document-save.png",W="icon/16/actions/edit-copy.png",P="Search...",Q="icon/16/actions/format-justify-center.png",R="Open",S="_cutCommand",X="Objective C",Y="Exit",C="Ctrl+S",B="Syntax",A="Ctrl+C",z="icon/16/actions/document-new.png",y="Tabs to spaces",x="ASCII table",w="Replace",v="HTML",E="icon/16/actions/system-search.png",D="changeValue",ba="execute";qx.Class.define(h,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var cd=new qx.ui.container.Scroll();var cc=new qx.ui.container.Composite(new qx.ui.layout.Canvas);cc.setPadding(20);cc.setAllowStretchX(false);cd.add(cc);this.getRoot().add(cd,{edge:0});this.createCommands();var ca=this.getMenuBar();cc.add(ca,{left:20,top:50});var cb=new qx.ui.form.ToggleButton(M);cb.setValue(true);cc.add(cb,{left:20,top:10});cb.bind(T,ca,bh);}
...
,createCommands:function(){this._newCommand=new qx.ui.core.Command(bb);this._newCommand.addListener(ba,this.debugCommand);this._openCommand=new qx.ui.core.Command(bq);this._openCommand.addListener(ba,this.debugCommand);this._saveCommand=new qx.ui.core.Command(C);this._saveCommand.addListener(ba,this.debugCommand);this._undoCommand=new qx.ui.core.Command(u);this._undoCommand.addListener(ba,this.debugCommand);this._redoCommand=new qx.ui.core.Command(t);this._redoCommand.addListener(ba,this.debugCommand);this._cutCommand=new qx.ui.core.Command(N);this._cutCommand.addListener(ba,this.debugCommand);this._copyCommand=new qx.ui.core.Command(A);this._copyCommand.addListener(ba,this.debugCommand);this._pasteCommand=new qx.ui.core.Command(J);this._pasteCommand.addListener(ba,this.debugCommand);this._pasteCommand.setEnabled(false);}
,getMenuBar:function(){var cg=new qx.ui.container.Composite(new qx.ui.layout.Grow);var menubar=new qx.ui.menubar.MenuBar;menubar.setWidth(600);cg.add(menubar);var cf=new qx.ui.menubar.Button(m,null,this.getFileMenu());var ce=new qx.ui.menubar.Button(bn,null,this.getEditMenu());var ch=new qx.ui.menubar.Button(bJ,null,this.getSearchMenu());var ck=new qx.ui.menubar.Button(bk,null,this.getViewMenu());var cj=new qx.ui.menubar.Button(p,null,this.getFormatMenu());var ci=new qx.ui.menubar.Button(bK,null,this.getHelpMenu());menubar.add(cf);menubar.add(ce);menubar.add(ch);menubar.add(ck);menubar.add(cj);menubar.add(ci);return cg;}
,getFileMenu:function(){var cn=new qx.ui.menu.Menu;var cl=new qx.ui.menu.Button(bD,z,this._newCommand);var cp=new qx.ui.menu.Button(R,bs,this._openCommand);var co=new qx.ui.menu.Button(bt);var cs=new qx.ui.menu.Button(k,V,this._saveCommand);var cr=new qx.ui.menu.Button(bg,bm);var cq=new qx.ui.menu.Button(U,bp);var cm=new qx.ui.menu.Button(Y,bQ);cl.addListener(ba,this.debugButton);cp.addListener(ba,this.debugButton);co.addListener(ba,this.debugButton);cs.addListener(ba,this.debugButton);cr.addListener(ba,this.debugButton);cq.addListener(ba,this.debugButton);cm.addListener(ba,this.debugButton);cn.add(cl);cn.add(cp);cn.add(co);cn.add(cs);cn.add(cr);cn.add(cq);cn.add(cm);return cn;}
,getEditMenu:function(){var cu=new qx.ui.menu.Menu;var cx=new qx.ui.menu.Button(bx,bu,this._undoCommand);var cw=new qx.ui.menu.Button(bI,o,this._redoCommand);var ct=new qx.ui.menu.Button(bc,bC,this._cutCommand);var cy=new qx.ui.menu.Button(F,W,this._copyCommand);var cv=new qx.ui.menu.Button(G,bL,this._pasteCommand);cx.addListener(ba,this.debugButton);cw.addListener(ba,this.debugButton);ct.addListener(ba,this.debugButton);cy.addListener(ba,this.debugButton);cv.addListener(ba,this.debugButton);cu.add(cx);cu.add(cw);cu.addSeparator();cu.add(ct);cu.add(cy);cu.add(cv);return cu;}
,getSearchMenu:function(){var cC=new qx.ui.menu.Menu;var cA=new qx.ui.menu.Button(P,E);var cF=new qx.ui.menu.Button(bU);var cB=new qx.ui.menu.Button(be);var cE=new qx.ui.menu.Button(w);var cz=new qx.ui.menu.Button(bA,E);var cD=new qx.ui.menu.Button(bY);cB.setEnabled(false);cA.addListener(ba,this.debugButton);cF.addListener(ba,this.debugButton);cB.addListener(ba,this.debugButton);cE.addListener(ba,this.debugButton);cz.addListener(ba,this.debugButton);cD.addListener(ba,this.debugButton);cC.add(cA);cC.add(cF);cC.add(cB);cC.add(cE);cC.addSeparator();cC.add(cz);cC.add(cD);return cC;}
,getViewMenu:function(){var cG=new qx.ui.menu.Menu;var cI=new qx.ui.menu.Button(H,null,null,this.getPanesMenu());var cL=new qx.ui.menu.Button(B,null,null,this.getSyntaxMenu());var cK=new qx.ui.menu.CheckBox(L);var cJ=new qx.ui.menu.CheckBox(bf);var cH=new qx.ui.menu.Button(x);cK.addListener(D,this.debugCheckBox);cJ.addListener(D,this.debugCheckBox);cH.addListener(ba,this.debugButton);cG.add(cI);cG.add(cL);cG.addSeparator();cG.add(cK);cG.add(cJ);cG.addSeparator();cG.add(cH);return cG;}
,getPanesMenu:function(){var cN=new qx.ui.menu.Menu;var cP=new qx.ui.menu.CheckBox(l);var cR=new qx.ui.menu.CheckBox(bF);var cS=new qx.ui.menu.CheckBox(b);var cO=new qx.ui.menu.CheckBox(O);var cQ=new qx.ui.menu.CheckBox(n);var cM=new qx.ui.menu.CheckBox(bM);cP.setValue(true);cR.setValue(true);cO.setValue(true);cP.addListener(D,this.debugCheckBox);cR.addListener(D,this.debugCheckBox);cS.addListener(D,this.debugCheckBox);cO.addListener(D,this.debugCheckBox);cQ.addListener(D,this.debugCheckBox);cM.addListener(D,this.debugCheckBox);cN.add(cR);cN.add(cP);cN.addSeparator();cN.add(cS);cN.add(cO);cN.add(cQ);cN.add(cM);return cN;}
,getSyntaxMenu:function(){var cX=new qx.ui.menu.Menu;var cT=new qx.ui.menu.RadioButton(v);var cV=new qx.ui.menu.RadioButton(j);var cU=new qx.ui.menu.RadioButton(g);var cW=new qx.ui.menu.Button(bj,null,null,this.getSyntaxCMenu());var db=new qx.ui.menu.RadioButton(bX);var cY=new qx.ui.menu.RadioButton(bl);cX.add(cT);cX.add(cV);cX.add(cU);cX.add(cW);cX.add(db);cX.add(cY);var da=new qx.ui.form.RadioGroup;da.add(cT,cV,cU,db,cY);da.add.apply(da,cW.getMenu().getChildren());da.addListener(bi,this.debugRadio);return cX;}
,getSyntaxCMenu:function(){var dd=new qx.ui.menu.Menu;var dc=new qx.ui.menu.RadioButton(bz);var dg=new qx.ui.menu.RadioButton(br);var df=new qx.ui.menu.RadioButton(X);var de=new qx.ui.menu.RadioButton(bP);dd.add(dc);dd.add(dg);dd.add(df);dd.add(de);return dd;}
,getFormatMenu:function(){var dk=new qx.ui.menu.Menu;var dj=new qx.ui.menu.Button(bo,null,null,this.getParagraphMenu());var dm=new qx.ui.menu.Button(y);var dn=new qx.ui.menu.Button(c);var di=new qx.ui.menu.Button(bv);var dl=new qx.ui.menu.Button(bV);var dh=new qx.ui.menu.Button(bR);var dq=new qx.ui.menu.Button(f);var dp=new qx.ui.menu.Button(bN);dm.addListener(ba,this.debugButton);dn.addListener(ba,this.debugButton);di.addListener(ba,this.debugButton);dl.addListener(ba,this.debugButton);dh.addListener(ba,this.debugButton);dq.addListener(ba,this.debugButton);dp.addListener(ba,this.debugButton);dk.add(dj);dk.add(dm);dk.add(dn);dk.addSeparator();dk.add(di);dk.add(dl);dk.add(dh);dk.addSeparator();dk.add(dq);dk.add(dp);return dk;}
,getParagraphMenu:function(){var dr=new qx.ui.menu.Menu;var du=new qx.ui.menu.Button(by,bG);var dv=new qx.ui.menu.Button(d,r);var ds=new qx.ui.menu.Button(bO,Q);var dt=new qx.ui.menu.Button(I,bB);du.addListener(ba,this.debugButton);dv.addListener(ba,this.debugButton);ds.addListener(ba,this.debugButton);dt.addListener(ba,this.debugButton);dr.add(du);dr.add(dv);dr.add(ds);dr.add(dt);return dr;}
,getHelpMenu:function(){var dw=new qx.ui.menu.Menu;var dy=new qx.ui.menu.Button(K,a);var dA=new qx.ui.menu.Button(bd);var dz=new qx.ui.menu.Button(bE);var dx=new qx.ui.menu.Button(q);dy.addListener(ba,this.debugButton);dA.addListener(ba,this.debugButton);dz.addListener(ba,this.debugButton);dx.addListener(ba,this.debugButton);dw.add(dy);dw.add(dA);dw.addSeparator();dw.add(dz);dw.addSeparator();dw.add(dx);return dw;}
},destruct:function(){this._disposeObjects(bT,bw,bW,bS,bH,S,i,s);}
});}
)();
