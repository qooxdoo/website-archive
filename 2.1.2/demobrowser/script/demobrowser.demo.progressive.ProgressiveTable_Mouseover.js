...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.progressive.ProgressiveTable_Mouseover.2b99973f663c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="px",k="  font-size: 11px;",j="content",h="px ",g=';',f='',d="padding: ",c="end",b="div",a="  position: absolute;",ba="qx.tableResizeDebug",Y="__columnData",X="css.boxmodel",W="  top: 0px;",V="  width : 100%;",U="  overflow:hidden;",T=" {",S="  border-right:1px solid #f2f2f2;",R="  cursor:default;",Q="  font-family: 'Segoe UI', Corbel, Calibri, Tahoma, 'Lucida Sans Unicode', sans-serif;",s="-row {",t="qx.debug",q="resize",r="start",o="relative",p="__oN",m=':ellipsis;',n=':',w="  white-space:nowrap;",x="  padding : 0px 6px 0px 6px;",E="__An",C="qx.ui.progressive.renderer.table.Row",I="progressive-table-row-background-odd",G="  height: 100%;",M="progressive-table-row-background-even",K="  border-bottom:1px solid #eeeeee;",z="left: ",P="css.userselect.none",O="changeTheme",N="css.userselect",y="css.textoverflow",A="}",B="",D="width: ",F="0px ",H="qx-progressive-",J="-row",L="-col-",u="px;",v=".qx-progressive-";qx.Class.define(C,{extend:qx.ui.progressive.renderer.Abstract,construct:function(bb){qx.ui.progressive.renderer.Abstract.call(this);this.__Al=bb;this.__Am={};this.__An=new qx.ui.progressive.renderer.table.cell.Default();this.__Ao=null;this.__Ap={};this.__At();this.__oN=new qx.ui.layout.HBox();this.__oN.connectToWidget(this);qx.theme.manager.Appearance.getInstance().addListener(O,this.__At,this);}
,statics:{__tS:null,__Aq:6,__Ar:a+W+G+U+(qx.core.Environment.get(y)?qx.bom.Style.getCssName(qx.core.Environment.get(y))+m:B)+w+S+K+x+R+k+Q+(qx.core.Environment.get(N)?qx.bom.Style.getCssName(qx.core.Environment.get(N))+n+qx.core.Environment.get(P)+g:f)},properties:{defaultRowHeight:{init:16}},members:{__Ao:null,__rB:null,__As:null,__Al:null,__Am:null,__An:null,__Ap:null,__oN:null,__At:function(){var bc=qx.theme.manager.Color.getInstance();this.__Ap.bgcol=[];this.__Ap.bgcol[0]=bc.resolve(M);this.__Ap.bgcol[1]=bc.resolve(I);}
,join:function(bd,name){if(this.__Ao){throw new Error("Renderer is already joined to a Progressive.");}
;this.__Ao=bd;this.__rB=name;var bf=qx.ui.progressive.renderer.table.Row;if(!bf.__tS){bf.__tS={};}
;var be=bd.toHashCode();if(!bf.__tS[be]){bf.__tS[be]={rowstylesheet:null,cellstylesheet:[]};var bg=v+be+s+V+A;bf.__tS[be].rowstylesheet=qx.bom.Stylesheet.createElement(bg);var bh=this.__Al.getData();for(var i=0;i<bh.length;i++ ){var bg=v+be+L+i+T+bf.__Ar+A;bf.__tS[be].cellstylesheet[i]=qx.bom.Stylesheet.createElement(bg);}
;this.__As=be;var bi=bd.getStructure().getPane();bi.addListener(q,this._resizeColumns,this);}
;}
,addRenderer:function(bj,bk){var bl=this.__Al.getData();if(bj<0||bj>=bl.length){throw new Error("Column "+bj+" out of range (max: "+(bl.length-1)+")");}
;this.__Am[bj]=bk;}
,removeRenderer:function(bm){var bn=this.__Al.getData();if(bm<0||bm>=bn.length){throw new Error("Column "+bm+" out of range (max: "+(bn.length-1)+")");}
;if(!this.__Am[bm]){throw new Error("No existing renderer for column "+bm);}
;delete this.__Am[bm];}
,render:function(bo,bp){var bz=bp.data;var bu=[];var bw;var bv;var bs=0;if(bo.getRendererData()[this.__rB].end===undefined){bo.getRendererData()[this.__rB]={end:0,start:1,rows:0,totalHeight:0};}
;var by=document.createElement(b);for(var i=0;i<bz.length;i++ ){var bt=H+this.__As+L+i;bv=this.__Am[i]||this.__An;bw={state:bo,rowDiv:by,stylesheet:bt,element:bp,dataIndex:i,cellData:bz[i],height:bs};bu.push(bv.render(bw));if(bw.height>bs){bs=bw.height;}
;}
;bs=(bs>0?bs:this.getDefaultRowHeight());var bx=bo.getRendererData()[this.__rB];bx.totalHeight+=bs;by.style.position=o;by.style.height=bs+l;by.className=H+this.__As+J;by.innerHTML=bu.join(B);switch(bp.location){case c:var bq=bx.end||0;by.style.backgroundColor=this.__Ap.bgcol[bq];bx.end=(bq==0?1:0);bo.getPane().getContentElement().getDomElement().appendChild(by);break;case r:var br=bo.getPane().getContentElement().getDomElement();var bA=br.childNodes;if(bA.length>0){var bq=bx.start;by.style.backgroundColor=this.__Ap.bgcol[bq];bx.start=(bq==0?1:0);br.insertBefore(by,bA[0]);break;}
else {br.appendChild(by);}
;break;default:throw new Error("Invalid location: "+bp.location);}; ++bx.rows;}
,getLayoutChildren:function(){return this.__Al.getData();}
,_resizeColumns:function(e){var bI=this.__Ao.getStructure().getPane();var bF=bI.getBounds().width-qx.bom.element.Scroll.getScrollbarWidth();var bG=v+this.__As+J;var bC=qx.ui.progressive.renderer.table.Row;qx.bom.Stylesheet.removeRule(bC.__tS[this.__As].rowstylesheet,bG);var bE=D+bF+u;qx.bom.Stylesheet.addRule(bC.__tS[this.__As].rowstylesheet,bG,bE);this.__oN.renderLayout(bF,100);var bH=this.__Al.getData();for(var i=0,bL=0;i<bH.length;i++ ,bL+=bF){var bG=v+this.__As+L+i;var bC=qx.ui.progressive.renderer.table.Row;qx.bom.Stylesheet.removeRule(bC.__tS[this.__As].cellstylesheet[i],bG);bF=bH[i].getComputedWidth();if(qx.core.Environment.get(t)){if(qx.core.Environment.get(ba)){this.debug("col "+i+": width="+bF);}
;}
;var bD;if(qx.core.Environment.get(X)==j){bD=qx.ui.progressive.renderer.table.Row.__Aq*2;}
else {bD=-1;}
;var bK=(bF-bD)+u;var bJ=F+qx.ui.progressive.renderer.table.Row.__Aq+h+F+qx.ui.progressive.renderer.table.Row.__Aq+u;var bB=bL+u;var bE=bC.__Ar+D+bK+z+bB+d+bJ;qx.bom.Stylesheet.addRule(bC.__tS[this.__As].cellstylesheet[i],bG,bE);}
;}
},destruct:function(){qx.theme.manager.Appearance.getInstance().removeListener(O,this.__At,this);var name;for(name in this.__Am){this.__Am[name]=null;}
;var bN=qx.ui.progressive.renderer.table.Row;var bM=this.__Ao.toHashCode();if(bN.__tS&&bN.__tS[bM]){if(bN.__tS[bM].rowstylesheet){var bP=v+this.__As+J;var bN=qx.ui.progressive.renderer.table.Row;qx.bom.Stylesheet.removeRule(bN.__tS[this.__As].rowstylesheet,bP);}
;if(bN.__tS[bM].cellstylesheet){for(var i=bN.__tS[bM].cellstylesheet.length-1;i>=0;i-- ){var bP=v+this.__As+L+i;var bO=bN.__tS[this.__As].cellstylesheet[i];var bN=qx.ui.progressive.renderer.table.Row;qx.bom.Stylesheet.removeRule(bO,bP);}
;}
;}
;if(this.__Ao&&this.__Ao.getRendererData){var bQ=this.__Ao.getRendererData();if(bQ&&bQ[this.__rB]&&bQ[this.__rB].end!==undefined){bQ[this.__rB]=null;}
;}
;this.__Ap=this.__Am=this.__Ao=this.__Al=null;this._disposeObjects(p,E,Y);}
});}
)();
...
}});qx.Class.define(b,{extend:qx.ui.progressive.renderer.table.cell.Default,statics:{onmouseover:function(L){L.__RA=qx.bom.element.Style.get(L,f);L.style.color=r;L.parentNode.style.height=32;L.__RB=L.innerHTML;var N=Date.parse(L.attributes.getNamedItem(q).nodeValue);var M=new Date(N);L.innerHTML=M.toUTCString()+j+M.toString();}
,onmouseout:function(O){O.style.color=O.__RA;O.parentNode.style.height=16;O.innerHTML=O.__RB;}
...
(function(){var f="resize",d="progressive-table-header",c="_layout",b="qx.ui.progressive.headfoot.TableHeading",a="progressive-table-header-cell";qx.Class.define(b,{extend:qx.ui.progressive.headfoot.Abstract,construct:function(g,h){qx.ui.progressive.headfoot.Abstract.call(this);this.__Al=g;var k=g.getData();this.__Eb=[];for(var i=0;i<k.length;i++ ){var l=new qx.ui.basic.Atom(h[i]);l.setAppearance(a);this.add(l);this.__Eb[i]=l;}
;var j=new qx.ui.core.Widget();j.set({height:16,appearance:a,minWidth:0,width:0});this.add(j,{flex:1});this.addListener(f,this._resizeColumns,this);this.__oN=new qx.ui.layout.HBox();this.__oN.connectToWidget(this);}
,properties:{appearance:{refine:true,init:d}},members:{__Al:null,__FL:null,__Eb:null,__oN:null,join:function(m){qx.ui.progressive.headfoot.Abstract.prototype.join.call(this,m);}
,getLayoutChildren:function(){if(this.__FL){return this.__Al.getData();}
else {return qx.ui.progressive.headfoot.Abstract.prototype.getLayoutChildren.call(this);}
;}
,_resizeColumns:function(e){var n=this.getBounds().width-qx.bom.element.Scroll.getScrollbarWidth();this.__FL=true;this.__oN.renderLayout(n,100);this.__FL=false;var o=this.__Al.getData();for(var i=0;i<o.length;i++ ){this.__Eb[i].setWidth(o[i].getComputedWidth());}
;}
},destruct:function(){this.__Al=this.__Eb=null;this._disposeObjects(c);}
});}
)();
