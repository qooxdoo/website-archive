...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormValidator.63eded040dcc.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var o="All fields are required. Some by a custom validator, some by a ",n="complete",m="Please enter a password at with least 3 characters.",l="username",k="Server said no!",j="Passwords must be equal.",i="<b>Client side form validation</b><br/>",h="Validating...",g="email address",f="demobrowser.demo.ui.FormValidator",c="Accept",e="execute",d="predefined validator and some by the required flag.",b="password",a="Send";qx.Class.define(f,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var p=new qx.ui.form.TextField();p.setPlaceholder(l);p.setWidth(150);p.setRequired(true);this.getRoot().add(p,{left:20,top:70});var y=new qx.ui.form.TextField();y.setPlaceholder(g);y.setWidth(150);this.getRoot().add(y,{left:20,top:100});var q=new qx.ui.form.TextField();q.setPlaceholder(b);q.setWidth(150);this.getRoot().add(q,{left:20,top:130});var r=new qx.ui.form.TextField();r.setPlaceholder(b);r.setWidth(150);this.getRoot().add(r,{left:20,top:160});var x=new qx.ui.form.CheckBox(c);x.setRequired(true);this.getRoot().add(x,{left:20,top:190});var u=new qx.ui.form.validation.Manager();var s=function(z,A){var B=z!=null&&z.length>2;if(!B){A.setInvalidMessage(m);}
;return B;}
;var v=new qx.ui.form.validation.AsyncValidator(function(C,D){window.setTimeout(function(){if(D==null||D.length==0){C.setValid(false,k);}
else {C.setValid(true);}
;}
,1000);}
);u.add(p,v);u.add(y,qx.util.Validate.email());u.add(q,s);u.add(r,s);u.add(x);u.setValidator(function(E){var G=q.getValue()==r.getValue();if(!G){var F=j;q.setInvalidMessage(F);r.setInvalidMessage(F);q.setValid(false);r.setValid(false);}
;return G;}
);var t=new qx.ui.form.Button(a);this.getRoot().add(t,{left:20,top:215});t.addListener(e,function(){t.setEnabled(false);t.setLabel(h);u.validate();}
,this);u.addListener(n,function(){t.setEnabled(true);t.setLabel(a);if(u.getValid()){alert("You can send...");}
else {alert(u.getInvalidMessages().join("\n"));}
;}
,this);var w=new qx.ui.basic.Label();w.setRich(true);w.setWidth(400);w.setValue(i+o+d);this.getRoot().add(w,{left:20,top:10});}
}});}
)();
