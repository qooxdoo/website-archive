...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Offline.587957c4c472.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,properties:{model:{nullable:true,event:c,apply:a}},members:{_storage:null,__Df:null,_applyModel:function(j,k){if(k){k.removeListenerById(this.__Df);k.dispose();this.__Df=null;}
;if(j){this.__Df=j.addListener(b,this.__Dg,this);this.__Dg();}
else {this._storage.removeItem(this._key);}
;}
,__Dg:function(){var l=qx.util.Serializer.toNativeObject(this.getModel());this._storage.setItem(this._key,l);}
...
(function(){var c="qx.bom.Storage",b="html.storage.local",a="html.storage.userdata";qx.Bootstrap.define(c,{statics:{__zE:null,getLocal:function(){if(qx.core.Environment.get(b)){return qx.bom.storage.Web.getLocal();}
...
(function(){var d="local",c="session",b="Storage",a="qx.bom.storage.Web";qx.Bootstrap.define(a,{statics:{__zF:null,__zG:null,getLocal:function(){if(this.__zF){return this.__zF;}
;return this.__zF=new qx.bom.storage.Web(d);}
,getSession:function(){if(this.__zG){return this.__zG;}
;return this.__zG=new qx.bom.storage.Web(c);}
},construct:function(f){this.__nk=f;}
,members:{__nk:null,getStorage:function(){return window[this.__nk+b];}
,getLength:function(){return this.getStorage(this.__nk).length;}
,setItem:function(g,h){h=qx.lang.Json.stringify(h);try{this.getStorage(this.__nk).setItem(g,h);}
catch(e){throw new Error("Storage full.");}
;}
,getItem:function(j){var k=this.getStorage(this.__nk).getItem(j);if(qx.lang.Type.isString(k)){k=qx.lang.Json.parse(k);}
else if(k&&k.value&&qx.lang.Type.isString(k.value)){k=qx.lang.Json.parse(k.value);}
;return k;}
,removeItem:function(l){this.getStorage(this.__nk).removeItem(l);}
,clear:function(){var m=this.getStorage(this.__nk);if(!m.clear){for(var i=m.length-1;i>=0;i-- ){m.removeItem(m.key(i));}
;}
else {m.clear();}
;}
,getKey:function(n){return this.getStorage(this.__nk).key(n);}
,forEach:function(o,p){var length=this.getLength();for(var i=0;i<length;i++ ){var q=this.getKey(i);o.call(p,q,this.getItem(q));}
;}
}});}
)();
(function(){var k="session",j="head",h="none",g="qx.bom.storage.UserData",f="div",e="local",d="qxtest",c="#default#userdata",b="display",a="qx";qx.Bootstrap.define(g,{statics:{__zF:null,__zG:null,__bV:0,getLocal:function(){if(this.__zF){return this.__zF;}
;return this.__zF=new qx.bom.storage.UserData(e);}
,getSession:function(){if(this.__zG){return this.__zG;}
;return this.__zG=new qx.bom.storage.UserData(k);}
},construct:function(l){this.__zH=document.createElement(f);this.__zH.style[b]=h;document.getElementsByTagName(j)[0].appendChild(this.__zH);this.__zH.addBehavior(c);this.__zI=l;this.__zH.load(this.__zI);this.__fd={};this.__zJ={};var m=this.__zH.getAttribute(a+qx.bom.storage.UserData.__bV);while(m!=undefined){m=qx.lang.Json.parse(m);this.__fd[m.key]=m.value;this.__zJ[m.key]=a+qx.bom.storage.UserData.__bV;qx.bom.storage.UserData.__bV++ ;m=this.__zH.getAttribute(a+qx.bom.storage.UserData.__bV);}
;}
,members:{__zH:null,__zI:d,__fd:null,__zJ:null,getStorage:function(){return this.__fd;}
,getLength:function(){return Object.keys(this.__fd).length;}
,setItem:function(n,o){if(this.__zJ[n]){var p=this.__zJ[n];}
else {var p=a+qx.bom.storage.UserData.__bV;qx.bom.storage.UserData.__bV++ ;}
;var q=qx.lang.Json.stringify({key:n,value:o});this.__zH.setAttribute(p,q);this.__zH.save(this.__zI);this.__fd[n]=o;this.__zJ[n]=p;}
,getItem:function(r){return this.__fd[r]||null;}
,removeItem:function(s){var v=this.__zJ[s];if(v==undefined){return;}
;this.__zH.removeAttribute(v);qx.bom.storage.UserData.__bV-- ;delete this.__fd[s];delete this.__zJ[s];var u=a+qx.bom.storage.UserData.__bV;if(this.__zH.getAttribute(u)){var w=this.__zH.getAttribute(a+qx.bom.storage.UserData.__bV);this.__zH.removeAttribute(u);this.__zH.setAttribute(v,w);var t=qx.lang.Json.parse(w).key;this.__zJ[t]=v;}
;this.__zH.save(this.__zI);}
,clear:function(){for(var x in this.__zJ){this.__zH.removeAttribute(this.__zJ[x]);}
;this.__zH.save(this.__zI);this.__fd={};this.__zJ={};}
,getKey:function(y){return Object.keys(this.__fd)[y];}
,forEach:function(z,A){var length=this.getLength();for(var i=0;i<length;i++ ){var B=this.getKey(i);z.call(A,B,this.getItem(B));}
;}
}});}
)();
(function(){var a="qx.bom.storage.Memory";qx.Bootstrap.define(a,{statics:{__zF:null,__zG:null,getLocal:function(){if(this.__zF){return this.__zF;}
;return this.__zF=new qx.bom.storage.Memory();}
,getSession:function(){if(this.__zG){return this.__zG;}
;return this.__zG=new qx.bom.storage.Memory();}
},construct:function(){this.__fd={};}
,members:{__fd:null,getStorage:function(){return this.__fd;}
,getLength:function(){return Object.keys(this.__fd).length;}
,setItem:function(b,c){c=qx.lang.Json.stringify(c);this.__fd[b]=c;}
,getItem:function(d){var e=this.__fd[d];if(qx.lang.Type.isString(e)){e=qx.lang.Json.parse(e);}
;return e;}
,removeItem:function(f){delete this.__fd[f];}
,clear:function(){this.__fd={};}
,getKey:function(g){var h=Object.keys(this.__fd);return h[g];}
