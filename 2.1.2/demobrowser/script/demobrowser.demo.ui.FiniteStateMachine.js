...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.97ef33b809b6.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j="__GU",h="__GV",g="qx.util.fsm.FiniteStateMachine",f="object",d="Number",c="string",b="String";qx.Class.define(g,{extend:qx.core.Object,construct:function(k){qx.core.Object.call(this);this.setName(k);this.__ju={};this.__GS=null;this.__GT=[];this.__GU=[];this.__GV=[];this.__GW={};this.__GX={};this.__GY={};this.__Ha={};this.__Hb={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},properties:{name:{check:b,nullable:true},state:{check:b,nullable:true},previousState:{check:b,nullable:true},nextState:{check:b,nullable:true},maxSavedStates:{check:d,init:2},debugFlags:{check:d,init:7}},members:{__ju:null,__GS:null,__GU:null,__GV:null,__GT:null,__GW:null,__GX:null,__GY:null,__Ha:null,__Hb:null,__Hc:false,addState:function(l){if(!l instanceof qx.util.fsm.State){throw new Error("Invalid state: not an instance of "+"qx.util.fsm.State");}
;var m=l.getName();if(m in this.__ju){throw new Error("State "+m+" already exists");}
;if(this.__GS==null){this.__GS=m;}
;this.__ju[m]=l;}
...
;var q=n.getName();var p=this.__ju[q];this.__ju[q]=n;if(o){p._bNeedDispose=true;}
;return p;}
,addObject:function(r,s,u){var w=s.toHashCode();this.__GX[r]=w;this.__GY[w]=r;this.__GW[r]=s;if(!u){return;}
;if(typeof (u)==c){u=[u];}
;for(var i=0;i<u.length;i++ ){var v=u[i];if(!this.__Ha[v]){this.__Ha[v]={};}
;this.__Ha[v][r]=true;if(!this.__Hb[r]){this.__Hb[r]=[];}
;this.__Hb[r].push(v);}
;}
,removeObject:function(x){var z;var y;var A;var B;z=this.__GX[x];if(this.__Hb[x]){for(var i=0;i<this.__Hb[x].length;i++ ){y=this.__Hb[x][i];delete this.__Ha[y][x];B=true;for(A in this.__Ha[y]){B=false;break;}
;if(B){delete this.__Ha[y];}
;}
;delete this.__Hb[x];}
;delete this.__GY[z];delete this.__GX[x];delete this.__GW[x];}
,getObject:function(C){return this.__GW[C];}
,getFriendlyName:function(D){var E=D?D.toHashCode():null;return E?this.__GY[E]:null;}
,getGroupObjects:function(F){var a=[];for(var name in this.__Ha[F]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var G in this.__GX){var I=this.__GX[G];var H=this.getObject(G);this.debug(G+" => "+I);this.debug("  "+I+" => "+this.__GY[I]);this.debug("  "+G+" => "+this.getObject(G));this.debug("  "+this.getObject(G)+" => "+this.getFriendlyName(H));}
;}
,_getInternalData:function(){return ({"states":this.__ju,"startState":this.__GS,"eventQueue":this.__GU,"blockedEvents":this.__GV,"savedStates":this.__GT,"friendlyToObject":this.__GW,"friendlyToHash":this.__GX,"hashToFriendly":this.__GY,"groupToFriendly":this.__Ha,"friendlyToGroups":this.__Hb});}
,start:function(){var K=this.__GS;if(K==null){throw new Error("Machine started with no available states");}
;this.setState(K);this.setPreviousState(null);this.setNextState(null);var J=(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL);if(J){this.debug(this.getName()+"#"+K+"#actionsBeforeOnentry");}
;this.__ju[K].getAutoActionsBeforeOnentry()(this);if(J){this.debug(this.getName()+"#"+K+"#entry");}
;this.__ju[K].getOnentry()(this,null);if(J){this.debug(this.getName()+"#"+K+"#actionsAfterOnentry");}
;this.__ju[K].getAutoActionsAfterOnentry()(this);}
,pushState:function(L){if(this.__GT.length>=this.getMaxSavedStates()){throw new Error("Saved-state stack is full");}
;if(L===true){this.__GT.push(this.getState());}
else if(L){this.__GT.push(L);}
else {this.__GT.push(this.getPreviousState());}
;}
,popState:function(){if(this.__GT.length==0){throw new Error("Saved-state stack is empty");}
;return this.__GT.pop();}
,postponeEvent:function(event){this.__GV.unshift(event);}
,enqueueEvent:function(event,M){if(M){this.__GU.push(event);}
else {this.__GU.unshift(event);}
;if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){var N=this.getFriendlyName(event.getTarget());if(M){this.debug(this.getName()+": Pushed event: "+event.getType()+(N?" on "+N:""));}
else {this.debug(this.getName()+": Queued event: "+event.getType()+(N?" on "+N:""));}
;}
;}
,eventListener:function(event){var e=event.clone();this.enqueueEvent(e,false);this.__Hd();}
,fireImmediateEvent:function(O,P,Q){if(Q){var event=qx.event.Registration.createEvent(O,qx.event.type.Data,[Q,null,false]);}
else {var event=qx.event.Registration.createEvent(O,qx.event.type.Event,[false,false]);}
;event.setTarget(P);this.eventListener(event);}
,scheduleEvent:function(R,S,T,U){qx.event.Timer.once(function(){this.fireImmediateEvent(R,S,T);}
,this,U||20);}
,__Hd:function(){if(this.__Hc){return;}
;this.__Hc=true;while(this.__GU.length>0){var event=this.__GU.pop();var V=this.__He(event);if(V){event.dispose();}
;}
;this.__Hc=false;}
,__He:function(event){var Y;var bb;var X;var bj;var bh;var e;var bf;var W=this.getDebugFlags();var bi=W&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var bc=W&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var bg=W&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var be=W&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ba=this.getFriendlyName(event.getTarget());if(bi){this.debug(this.getName()+": Process event: "+event.getType()+(ba?" on "+ba:""));}
;Y=this.getState();bj=this.__ju[Y];bh=bj.transitions;e=bj.getEvents()[event.getType()];if(!e){if(bi){this.debug(this.getName()+": Event '"+event.getType()+"'"+" not handled.  Ignoring.");}
;return true;}
;if(typeof (e)==f){if(!ba){if(be){this.debug(this.getName()+": Could not find friendly name for '"+event.getType()+"' on '"+event.getTarget()+"'");}
;return true;}
;bf=e[ba];if(!bf){if(bi){this.debug(this.getName()+": Event '"+event.getType()+"'"+" not handled for target "+ba+".  Ignoring.");}
;return true;}
;}
else {bf=e;}
;switch(bf){case qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE:break;case qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED:if(bi){this.debug(this.getName()+": Event '"+event.getType()+"'"+" blocked.  Re-queuing.");}
;this.__GV.unshift(event);return false;default:if(typeof (bf)==c){if(bh[bf]){var bd=bh[bf];bh={};bh[bf]=bd;}
else {throw new Error("Explicit transition "+bf+" does not exist");}
;break;}
;};for(var t in bh){var bd=bh[t];switch(bd.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error("Transition "+Y+":"+t+" returned a value other than "+"true, false, or null.");};bb=bd.getNextState();if(typeof (bb)==c){if(!bb in this.__ju){throw new Error("Attempt to transition to nonexistent state "+bb);}
;this.setNextState(bb);}
else {switch(bb){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:bb=Y;this.setNextState(bb);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__GT.length==0){throw new Error("Attempt to transition to POP_STATE_STACK "+"while state stack is empty.");}
;bb=this.__GT.pop();this.setNextState(bb);break;default:throw new Error("Internal error: invalid nextState");break;};}
;if(bg){this.debug(this.getName()+"#"+Y+"#"+t+"#autoActionsBeforeOntransition");}
;bd.getAutoActionsBeforeOntransition()(this);if(bg){this.debug(this.getName()+"#"+Y+"#"+t+"#ontransition");}
;bd.getOntransition()(this,event);if(bg){this.debug(this.getName()+"#"+Y+"#"+t+"#autoActionsAfterOntransition");}
;bd.getAutoActionsAfterOntransition()(this);if(bg){this.debug(this.getName()+"#"+Y+"#autoActionsBeforeOnexit");}
;bj.getAutoActionsBeforeOnexit()(this);if(bg){this.debug(this.getName()+"#"+Y+"#exit");}
;bj.getOnexit()(this,event);if(bg){this.debug(this.getName()+"#"+Y+"#autoActionsAfterOnexit");}
;bj.getAutoActionsAfterOnexit()(this);if(bj._bNeedDispose){bj.dispose();}
;bj=this.__ju[this.getNextState()];this.setPreviousState(Y);this.setState(this.getNextState());this.setNextState(null);X=Y;Y=bb;bb=undefined;if(bg){this.debug(this.getName()+"#"+Y+"#autoActionsBeforeOnentry");}
;bj.getAutoActionsBeforeOnentry()(this);if(bg){this.debug(this.getName()+"#"+Y+"#entry");}
;bj.getOnentry()(this,event);if(bg){this.debug(this.getName()+"#"+Y+"#autoActionsAfterOnentry");}
;bj.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__GV.length;i++ ){e=this.__GV.pop();this.__GU.unshift(e);}
;if(bc){this.debug(this.getName()+"#"+X+" => "+this.getName()+"#"+Y);}
;return true;}
;if(bc){this.debug(this.getName()+"#"+Y+": event '"+event.getType()+"'"+": no transition found.  No state change.");}
;return true;}
},destruct:function(){this._disposeArray(j);this._disposeArray(h);this.__GT=this.__ju=null;}
});}
)();
(function(){var p="function",o="  var groupObjects = ",n="parameters",m=")",l=" fsm.getObject('",k="groups",h="  for (var i = 0; i < groupObjects.length; i++)",d="autoActionsAfterOnentry",c="__GQ",b="fsm",Y="');",X="autoActionsBeforeOnexit",W="autoActionsBeforeOnentry",V="__GL",U="onexit",T="__GN",S="__GJ",R="autoActionsAfterOnexit",Q="  fsm.debug(ex);",P="    fsm.getObject(objName).",w=")(fsm)",x="events",u="    var objName = groupObjects[i];",v="').",s="objects",t="catch(ex)",q="  {",r="    fsm.getGroupObjects('",y="  }",z="__GP",G="__GK",E="__GM",J="qx.util.fsm.State",I="onentry",L="try",K="__GO",B=",",O="context",N="string",M='"',A="}",C="{",D="(",F=";",H="object";qx.Class.define(J,{extend:qx.core.Object,construct:function(ba,bb){var bd;qx.core.Object.call(this);this.setName(ba);if(typeof (bb)!=H){throw new Error("State info must be an object");}
;bd=bb.context||window;this.setUserData(O,bd);for(var bc in bb){switch(bc){case I:this.setOnentry(this.__GR(bb[bc],bd));break;case U:this.setOnexit(this.__GR(bb[bc],bd));break;case W:this.setAutoActionsBeforeOnentry(bb[bc]);break;case d:this.setAutoActionsAfterOnentry(bb[bc]);break;case X:this.setAutoActionsBeforeOnexit(bb[bc]);break;case R:this.setAutoActionsAfterOnexit(bb[bc]);break;case x:this.setEvents(bb[bc]);break;case O:break;default:this.setUserData(bc,bb[bc]);this.debug("State "+ba+": "+"Adding user-provided field to state: "+bc);break;};}
;if(!this.getEvents()){throw new Error("The events object must be provided in new state info");}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(be,bf,bg){if(typeof (bf)!=H){throw new Error("Invalid "+be+" value: "+typeof (bf));}
;var bl;var bm=L+C;var bj;var bh;for(var f in bf){var bi=bf[f];if(!bi instanceof Array){throw new Error("Invalid function request type: "+"expected array, found "+typeof (bi));}
;for(var i=0;i<bi.length;i++ ){bh=bi[i];if(typeof (bh)!=H){throw new Error("Invalid function request parameter type: "+"expected object, found "+typeof (bi[bj]));}
;var bk=bh[n];if(!bk){bk=[];}
else {if(!bk instanceof Array){throw new Error("Invalid function parameters: "+"expected array, found "+typeof (bk));}
;}
;bl=f+D;for(var j=0;j<bk.length;j++ ){if(j!=0){bl+=B;}
;if(typeof (bk[j])==p){bl+=D+bk[j]+w;}
else if(typeof (bk[j])==N){bl+=M+bk[j]+M;}
else {bl+=bk[j];}
;}
;bl+=m;var a=bh[s];if(!a){a=[];}
...
;bm+=l+a[j]+v+bl+F;}
;var g=bh[k];if(g){if(!g instanceof Array){throw new Error("Invalid 'groups' list: expected array, got "+typeof (g));}
;for(j=0;j<g.length;j++ ){bm+=o+r+g[j]+Y+h+q+u+P+bl+F+y;}
;}
;}
;}
;bm+=A+t+C+Q+A;return qx.lang.Function.bind(new Function(b,bm),bg);}
},properties:{name:{transform:S,nullable:true},onentry:{transform:G,nullable:true,init:function(bn,event){}
},onexit:{transform:V,nullable:true,init:function(bo,event){}
},autoActionsBeforeOnentry:{transform:T,nullable:true,init:function(bp,event){}
},autoActionsAfterOnentry:{transform:K,nullable:true,init:function(bq,event){}
},autoActionsBeforeOnexit:{transform:z,nullable:true,init:function(br,event){}
},autoActionsAfterOnexit:{transform:c,nullable:true,init:function(bs,event){}
},events:{transform:E,nullable:true}},members:{__GJ:function(bt){if(typeof (bt)!="string"||bt.length<1){throw new Error("Invalid state name");}
;return bt;}
,__GK:function(bu){switch(typeof (bu)){case "undefined":return function(bv,event){}
;case "function":return qx.lang.Function.bind(bu,this.getUserData("context"));default:throw new Error("Invalid onentry type: "+typeof (bu));};}
,__GL:function(bw){switch(typeof (bw)){case "undefined":return function(bx,event){}
;case "function":return qx.lang.Function.bind(bw,this.getUserData("context"));default:throw new Error("Invalid onexit type: "+typeof (bw));return null;};}
,__GM:function(by){if(typeof (by)!="object"){throw new Error("events must be an object");}
;for(var e in by){var bz=by[e];if(typeof (bz)=="number"&&bz!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bz!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error("Invalid numeric value in events object: "+e+": "+bz);}
...
;}
;return by;}
,__GN:function(bB){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnentry",bB,this.getUserData("context"));}
,__GO:function(bC){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnentry",bC,this.getUserData("context"));}
,__GP:function(bD){return qx.util.fsm.State._commonTransformAutoActions("autoActionsBeforeOnexit",bD,this.getUserData("context"));}
,__GQ:function(bE){return qx.util.fsm.State._commonTransformAutoActions("autoActionsAfterOnexit",bE,this.getUserData("context"));}
,__GR:function(f,bF){if(typeof (f)=="function"){f=qx.lang.Function.bind(f,bF);}
;return f;}
...
(function(){var s="string",r="predicate",q="nextState",p="__Hj",o="number",n="__Hf",m="String",l="__Hh",k="__Hg",j="qx.util.fsm.Transition",c="__Hi",i="ontransition",g="boolean",b="undefined",a="autoActionsBeforeOntransition",e="function",d="autoActionsAfterOntransition",h="context";qx.Class.define(j,{extend:qx.core.Object,construct:function(t,u){var w;qx.core.Object.call(this);this.setName(t);w=u.context||window;this.setUserData(h,w);for(var v in u){switch(v){case r:this.setPredicate(this.__GR(u[v],w));break;case q:this.setNextState(u[v]);break;case a:this.setAutoActionsBeforeOntransition(this.__GR(u[v],w));break;case d:this.setAutoActionsAfterOntransition(this.__GR(u[v],w));break;case i:this.setOntransition(this.__GR(u[v],w));break;case h:break;default:this.setUserData(v,u[v]);this.debug("Transition "+t+": "+"Adding user-provided field to transition: "+v);break;};}
;}
,properties:{name:{check:m,nullable:true},predicate:{init:function(x,event){return true;}
,transform:n},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:k},autoActionsBeforeOntransition:{init:function(y,event){}
,transform:l},autoActionsAfterOntransition:{init:function(z,event){}
,transform:c},ontransition:{init:function(A,event){}
,transform:p}},members:{__Hf:function(B){switch(typeof (B)){case b:return function(C,event){return true;}
;case g:return function(D,event){return B;}
;case e:return qx.lang.Function.bind(B,this.getUserData(h));default:throw new Error("Invalid transition predicate type: "+typeof (B));break;};}
,__Hg:function(E){switch(typeof (E)){case s:return E;case o:switch(E){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return E;default:throw new Error("Invalid transition nextState value: "+E+": "+"nextState must be an explicit state name, "+"or one of the Fsm.StateChange constants");};break;default:throw new Error("Invalid transition nextState type: "+typeof (E));break;};}
,__Hh:function(F){return qx.util.fsm.State._commonTransformAutoActions(a,F,this.getUserData(h));}
,__Hi:function(G){return qx.util.fsm.State._commonTransformAutoActions(d,G,this.getUserData(h));}
,__Hj:function(H){switch(typeof (H)){case b:return function(I,event){}
;case e:return qx.lang.Function.bind(H,this.getUserData(h));default:throw new Error("Invalid ontransition type: "+typeof (H));break;};}
,__GR:function(f,J){if(typeof (f)=="function"){f=qx.lang.Function.bind(f,J);}
;return f;}
}});}
)();
