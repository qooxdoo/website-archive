...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.List.3f3ba22732f5.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var bI="Julian",bH="Show Icon",bG="Kevin",bF="Marvin",bE="Marc",bD="Configurable",bC="icon/16/places/folder.png",bB="computer.png",bA="Jason",bz="Jakob",bo="Lukas",bn="Julius",bm="network-wired.png",bl="Jan",bk="Tobias",bj="Mika",bi="Nils",bh="Noah",bg="Tom",bf="Nico",bP="Multi Selection",bQ="battery.png",bN="Felix",bO="Leon",bL="input-keyboard.png",bM="camera-web.png",bJ="Enable drag selection",bK="icon",bR="Show Both",bS="48",bs="icon/48/devices/",br="Fabian",bu="multimedia-player.png",bt="camera-photo.png",bw="media-flash.png",bv="input-mouse.png",by="Joel",bx="Dominic",bq="Show Label",bp="demobrowser.demo.widget.List",a="Enable quick selection",b="Finn",c="One as selection mode",d="Jannis",f="audio-card.png",g="Alexander",h="Additive Selection",j="display.png",k="Erik",l="addaptive",bW="Johannes",bV="pda.png",bU="16",bT="Jonas",cb="icon/",ca="David",bY="Maximilian",bX="Tim",cd="Justin",cc="Single Selection",K="Linus",L="Daniel",I="Paul",J="Philipp",O="Niclas",P="One Selection",M="Nick",N="Jonathan",G="Ben",H="on",t="Vincent",s="/places/folder.png",v="Robin",u="both",p="Jannick",o="Luis",r="drive-harddisk.png",q="Lennard",n="Moritz",m="Florian",U="drive-optical.png",V="media-optical.png",W="Marlon",X="Luca",Q="network-wireless.png",R="audio-input-microphone.png",S="phone.png",T="Simon",Y="Colin",ba="label",D="printer.png",C="Max",B="Additive selection",A="Horizontal, Icons only",z="Elias",y="changeValue",x="Item No ",w="changeSelection",F="additive",E="single",bb="one",bc="bold",bd="multi",be="value";qx.Class.define(bp,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var ce=new qx.ui.container.Scroll();var cu=new qx.ui.container.Composite(new qx.ui.layout.Basic());cu.setAllowStretchX(false);ce.add(cu);this.getRoot().add(ce,{edge:0});var co=new qx.ui.basic.Label(bD);co.setFont(bc);cu.add(co,{left:20,top:20});var cw=new qx.ui.form.List;cw.setScrollbarX(H);cw.set({height:280,width:150,selectionMode:bd});var cx;for(var i=1;i<=25;i++ ){cx=new qx.ui.form.ListItem(x+i,cb+((i%4)?bU:bS)+s);cw.add(cx);!(i%9)&&(cx.setEnabled(false));if(i==20){cw.setSelection([cx]);}
;}
;cw.addListener(w,function(e){var cD=e.getData();if(cD[0]){this.debug("Value: "+cD[0].getLabel());}
;}
);cu.add(cw,{left:20,top:40});var ck=new qx.ui.form.RadioButton(cc);var ch=new qx.ui.form.RadioButton(bP);var ci=new qx.ui.form.RadioButton(h);var cg=new qx.ui.form.RadioButton(P);ck.setUserData(be,E);ch.setUserData(be,bd);ci.setUserData(be,F);cg.setUserData(be,bb);ch.setValue(true);cu.add(ck,{left:180,top:40});cu.add(ch,{left:180,top:60});cu.add(ci,{left:180,top:80});cu.add(cg,{left:180,top:100});var cj=new qx.ui.form.RadioGroup(ck,ch,ci,cg);var cA=new qx.ui.form.RadioButton(bq);var cz=new qx.ui.form.RadioButton(bH);var cC=new qx.ui.form.RadioButton(bR);cA.setUserData(be,ba);cz.setUserData(be,bK);cC.setUserData(be,u);cC.setValue(true);cu.add(cA,{left:180,top:140});cu.add(cz,{left:180,top:160});cu.add(cC,{left:180,top:180});var cv=new qx.ui.form.RadioGroup(cA,cz,cC);cv.addListener(w,function(e){for(var i=0;i<cw.getChildren().length;i++ ){cw.getChildren()[i].setShow(e.getData()[0].getUserData(be));}
;}
);var cs=new qx.ui.form.CheckBox(bJ);var cp=new qx.ui.form.CheckBox(a).set({enabled:false});cu.add(cs,{left:180,top:220});cu.add(cp,{left:180,top:240});cs.addListener(y,function(e){if(e.getData()){var cE=cw.getSelectionMode();if(cE==E||cE==bb){this.debug("Drag selection is only available for the modes multi or additive");}
;}
;cw.setDragSelection(e.getData());}
);cp.addListener(y,function(e){if(e.getData()){var cF=cw.getSelectionMode();if(cF==bd||cF==F){this.debug("Quick selection is only available for the modes multi or additive");}
;}
;cw.setQuickSelection(e.getData());}
);cj.addListener(w,function(e){var cG=e.getData()[0].getUserData(be);cw.setSelectionMode(cG);if(cG==E||cG==bb){cs.setEnabled(false);cp.setEnabled(true);}
else if(cG==bd||cG==l){cs.setEnabled(true);cp.setEnabled(false);}
;}
);var cB=new qx.ui.basic.Label(c);cB.setFont(bc);cu.add(cB,{left:330,top:20});var cn=new qx.ui.form.List();cn.set({height:280,width:150,selectionMode:bb});var cx;for(var i=1;i<=25;i++ ){cx=new qx.ui.form.ListItem(x+i,bC);cn.add(cx);if(i==16){cn.setSelection([cx]);}
;}
;cu.add(cn,{left:330,top:40});var co=new qx.ui.basic.Label(B);co.setFont(bc);cu.add(co,{left:520,top:20});var cf=new qx.ui.form.List;var cl;cf.set({width:150,selectionMode:F});var cr=[bO,bo,X,b,bX,bN,bT,o,bY,bI,C,I,O,bl,G,z,p,J,bh,bg,n,bf,ca,bi,T,br,k,cd,g,bz,m,M,K,bj,bA,L,q,bF,d,bk,bx,W,bE,bW,N,bn,Y,by,bG,t,v];for(var i=0;i<cr.length;i++ ){cl=new qx.ui.form.ListItem(cr[i]);cf.add(cl);if(i==10||i==12||i==16){cf.addToSelection(cl);}
;}
;cu.add(cf,{left:520,top:40});var co=new qx.ui.basic.Label(A);co.setFont(bc);cu.add(co,{left:20,top:350});var cy=new qx.ui.form.List(true);var cm;cy.set({width:550,selectionMode:bd,height:null});var cq=[f,R,bQ,bt,bM,bB,j,r,U,bL,bv,bw,V,bu,bm,Q,bV,S,D];var ct=bs;for(var i=0;i<cq.length;i++ ){cm=new qx.ui.form.ListItem(null,ct+cq[i]);cy.add(cm);if(i==12){cy.setSelection([cm]);}
;}
