...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.mobile.Fingers.a1c78c809766.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;this.__Rh=[];this.__Ri=[];J.addListener(b,this._onTouchStart,this);J.addListener(l,this._onTouchMove,this);}
,_onTouchStart:function(e){var L=e.getAllTouches();for(var i=0;i<L.length;i++ ){var K=L[i];this.__Rh[i]=parseInt(K.target.style.left)-K.pageX;this.__Ri[i]=parseInt(K.target.style.top)-K.pageY;}
;}
,_onTouchMove:function(e){var N=e.getAllTouches();for(var i=0;i<N.length;i++ ){var M=N[i];if(N.target==document.body){continue;}
;qx.bom.element.Style.setStyles(M.target,{"left":(M.pageX+this.__Rh[i])+m,"top":(M.pageY+this.__Ri[i])+m});}
;e.stop();}
}});}
)();
