...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.event.ElementResize","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.event.ElementResize.55a9307a0b4d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var f="demobrowser.demo.event.ElementResize",d="width: ",c="px",b="px<br>height: ",a="resize";qx.Class.define(f,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var g=document.getElementById(a);qx.bom.Element.addListener(g,a,this._onResize,this);this.update(g,g.offsetWidth,g.offsetHeight);}
,_onResize:function(e){var h=e.getData();this.update(e.getTarget(),h.width,h.height);}
,update:function(i,j,k){i.innerHTML=d+j+b+k+c;}
}});}
)();
(function(){var f="resize",d="interval",c="__ks",b="body",a="qx.event.handler.ElementResize";qx.Class.define(a,{extend:qx.core.Object,implement:qx.event.IEventHandler,construct:function(g){qx.core.Object.call(this);this.__fN=g;this.__of={};this.__ks=new qx.event.Timer(200);this.__ks.addListener(d,this._onInterval,this);}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{resize:true},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__of:null,__fN:null,__ks:null,canHandleEvent:function(h,i){return h.tagName.toLowerCase()!==b;}
,registerEvent:function(j,k,l){var n=qx.core.ObjectRegistry.toHashCode(j);var m=this.__of;if(!m[n]){m[n]={element:j,width:qx.bom.element.Dimension.getWidth(j),height:qx.bom.element.Dimension.getHeight(j)};this.__ks.start();}
;}
,unregisterEvent:function(o,p,q){var s=qx.core.ObjectRegistry.toHashCode(o);var r=this.__of;if(r[s]){delete r[s];if(qx.lang.Object.isEmpty(r)){this.__ks.stop();}
;}
;}
,_onInterval:function(e){var u=this.__of;for(var x in u){var y=u[x];var t=y.element;var w=qx.bom.element.Dimension.getWidth(t);var v=qx.bom.element.Dimension.getHeight(t);if(y.height!==v||y.width!==w){qx.event.Registration.fireNonBubblingEvent(t,f,qx.event.type.Data,[{width:w,oldWidth:y.width,height:v,oldHeight:y.height}]);y.width=w;y.height=v;}
;}
;}
},destruct:function(){this.__fN=this.__of=null;this._disposeObjects(c);}
