...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Service.ee00c49a2237.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="qxc/application/datademo/identica.png",j="one",h="http",g="callback",f="error",e="selection[0]",d="Location: ",c="Details",b="Name: ",a="source",A="... service unavailable!",z="</a>",y="' target='_blank'>",x="qxc.application.datademo.Demo",w="user.name",v="qxc/application/datademo/service.js",u="user.location",t="<a href='",s="Avatar: ",r="Message: ",p="text",q=" ",n="user.profile_image_url",o="icon",l="value",m="model";qx.Class.define(x,{extend:qx.ui.container.Composite,construct:function(){qx.ui.container.Composite.call(this);this._createView();}
,members:{_createView:function(){this.setLayout(new qx.ui.layout.Canvas());var H=new qx.ui.basic.Image(k);this.add(H,{left:10,top:15});var D=new qx.ui.form.List();this.add(D,{left:10,top:175,bottom:5});D.set({selectionMode:j,width:300,maxHeight:400,height:300});var E=new qx.data.controller.List(null,D);E.setDelegate(this);E.setLabelPath(p);E.setIconPath(n);var B=v;var J=new qx.data.store.Jsonp();J.setCallbackName(g);J.setUrl(B);J.bind(m,E,m);var G=new qx.ui.basic.Label(A);G.setTextColor("invalid");G.hide();this.add(G,{left:290,top:145});J.addListener(f,function(){G.show();D.setEnabled(false);C.setEnabled(false);}
,this);var C=new qx.ui.groupbox.GroupBox(c);this.add(C,{left:320,top:156,bottom:5});C.setWidth(270);C.setHeight(320);C.setAllowGrowY(false);C.setLayout(new qx.ui.layout.Grid(5,5));C.add(new qx.ui.basic.Label(b),{row:0,column:0});C.add(new qx.ui.basic.Label(d),{row:1,column:0});C.add(new qx.ui.basic.Label(r),{row:2,column:0});var name=new qx.ui.basic.Label();C.add(name,{row:0,column:1});var location=new qx.ui.basic.Label();C.add(location,{row:1,column:1});var I=new qx.ui.basic.Label();I.setRich(true);I.setWidth(150);I.setSelectable(true);C.add(I,{row:2,column:1});var F=new qx.data.controller.Object();F.addTarget(name,l,w);F.addTarget(location,l,u);F.addTarget(I,l,p,false,{converter:function(L){var M=L.split(q);for(var i=M.length-1;i>=0;i-- ){if(M[i].indexOf(h)==0){M[i]=t+M[i]+y+M[i]+z;}
;}
;return M.join(q);}
});C.add(new qx.ui.basic.Label(s),{row:3,column:0});var K=new qx.ui.basic.Image();C.add(K,{row:3,column:1});F.addTarget(K,a,n);E.bind(e,F,m);}
,configureItem:function(N){N.setRich(true);N.getChildControl(o).setWidth(48);N.getChildControl(o).setHeight(48);N.getChildControl(o).setScale(true);}
}});}
)();
