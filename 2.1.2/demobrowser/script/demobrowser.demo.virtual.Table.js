...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Table.3a40e306a9ed.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g=" / ",f="horizontal",e="__Si",d="demobrowser.demo.virtual.Table",c="white",b="#EEE",a="vertical";qx.Class.define(d,{extend:qx.application.Standalone,members:{__Si:null,main:function(){qx.application.Standalone.prototype.main.call(this);var h=new qx.ui.virtual.core.Scroller(10000,10000,20,100);for(var i=0;i<10000;i++ ){h.getPane().getRowConfig().setItemSize(i,20+Math.round(Math.random()*40));h.getPane().getColumnConfig().setItemSize(i,50+Math.round(Math.random()*80));}
;this.getRoot().add(h,{edge:20});h.getPane().addLayer(new qx.ui.virtual.layer.Row(c,b));h.getPane().addLayer(new qx.ui.virtual.layer.GridLines(f));h.getPane().addLayer(new qx.ui.virtual.layer.GridLines(a));h.getPane().addLayer(new qx.ui.virtual.layer.HtmlCell(this));this.__Si=new qx.ui.virtual.cell.Cell();}
,getCellProperties:function(j,k){return this.__Si.getCellProperties(j+g+k);}
},destruct:function(){this._disposeObjects(e);}
});}
)();
...
;this.__wX=[];this.__wY=[];this._isHorizontal=(v||g)==g;}
,properties:{defaultLineColor:{init:r,check:e,themeable:true},defaultLineSize:{init:1,check:k,themeable:true}},members:{__wX:null,__wY:null,isHorizontal:function(){return this._isHorizontal;}
...
;this.__wX[A]=B;if(this.__xa(A)){this.updateLayerData();}
;}
...
;this.__wY[C]=D;if(this.__xa(C)){this.updateLayerData();}
;}
,__xa:function(E){if(this._isHorizontal){var I=this.getFirstColumn();var G=I+this.getColumnSizes().length-1;return E>=I&&E<=G;}
else {var H=this.getFirstRow();var F=H+this.getRowSizes().length-1;return E>=H&&E<=F;}
;}
,getLineSize:function(J){return this.__wY[J]||this.getDefaultLineSize();}
,getLineColor:function(K){return this.__wX[K]||this.getDefaultLineColor();}
,__xb:function(L,M,N){var top=0;var O,P;for(var y=0;y<N.length-1;y++ ){O=this.getLineColor(M+y);P=this.getLineSize(M+y);top+=N[y];L.push(a,f,s+P+d,q,u,top-((P>1)?Math.floor(P/2):1),d,b,O,j,c);}
;}
,__xc:function(Q,R,S){var V=0;var T,U;for(var x=0;x<S.length-1;x++ ){T=this.getLineColor(R+x);U=this.getLineSize(R+x);V+=S[x];Q.push(a,f,p+U+d,o,h,m,V-((U>1)?Math.floor(U/2):1),d,b,T,j,c);}
;}
,_fullUpdate:function(W,X,Y,ba){var bb=[];if(this._isHorizontal){this.__xb(bb,W,Y);}
else {this.__xc(bb,X,ba);}
...
;}
},destruct:function(){this.__wX=this.__wY=null;}
});}
)();
