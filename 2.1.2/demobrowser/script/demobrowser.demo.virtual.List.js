...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.List.e931c7654f28.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var cf=qx.data.marshal.Json.createModel(cc);var bW=this.__So=new qx.ui.list.List(cf).set({scrollbarX:bM,selectionMode:H,height:280,width:150,labelPath:J,iconPath:I,iconOptions:{converter:function(cj){return d+cj+y;}
...
,onSelectionModeChange:function(e){var cC=e.getData()[0].getUserData(O);this.__So.setSelectionMode(cC);if(cC==N||cC==G){this.__Sp.setEnabled(false);this.__Sq.setEnabled(true);}
else if(cC==H||cC==ba){this.__Sp.setEnabled(true);this.__Sq.setEnabled(false);}
;}
,onShowModeChange:function(e){var cD=e.getData()[0].getUserData(O);this.setShowMode(cD);}
,onDragCheckChange:function(e){if(e.getData()){var cE=this.__So.getSelectionMode();if(cE==N||cE==G){this.debug("Drag selection is only available for the modes multi or additive");}
;}
;this.__So.setDragSelection(e.getData());}
,onQuickCheckChange:function(e){if(e.getData()){var cF=this.__So.getSelectionMode();if(cF==H||cF==K){this.debug("Quick selection is only available for the modes multi or additive");}
;}
;this.__So.setQuickSelection(e.getData());}
}});}
)();
