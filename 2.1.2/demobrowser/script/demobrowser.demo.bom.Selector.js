...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Selector.c9a28401bd12.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j="text",i="backgroundColor",h="<li>inserted via after()</li>",g=":first-child,:last-child",f="underline",e="green",d="for",c="ul,ol",b="<li>inserted via append()</li>",a="<h3>Dynamically Inserted H3</h3>",bf="opacity",be=" Title ",bd="li:first-child",bc="#detail2",bb="li a",ba="center",Y="h1",X="#eee",W="0.5",V="[type=submit]",r="header",s="name",o='<div style="background:#FFFFBB"></div>',p="<li>inserted via before()</li>",m="click",n="form",k="label[for=field-mail]",l="submit",v="demobrowser.demo.bom.Selector",w="ul li:contains('List Item 3')",E="<b></b>",C="p:eq(1)",L="label",G="red",R="#detail1,#detail2,#detail3",P="Click to follow the link",y="input[type=submit]",U="a",T="orange",S="li:last-child",x="id",A="<option>Mr</option><option>Mrs</option><option>Dr</option><option>Prof</option>",B="color",D="field-title",F="title",H=":contains('List Item 1')",M="fontWeight",Q="p",t="bold",u="h1,h2",z="#field-mail",K="input",J="h2",I="select",O="li",N="ul";qx.Class.define(v,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var bk=qx.log.Logger;q(Q).setStyle(B,G);q(C).setStyle(M,t).setStyle(bf,W);q(Y).setStyles({"textAlign":ba,"textDecoration":f,"color":e});bk.debug("href of first link: "+q("a").getAttribute("href"));q(U).setAttribute(F,P);q(u).addClass(r);q(Q).on(m,function(){alert(this.innerHTML);}
);q().concat(q(J)).setStyle(B,T).concat(q(O)).setStyle(i,X);bk.debug("Found "+q("ul,ol").getChildren().length+" children of ul/ol elements using children().");bk.debug("Found "+q("ul,ol").getChildren(":first-child").length+" first children of ul/ol elements using children(selector)");bk.debug("Number of ul/ol elements which are closest to a li element: "+q("li").getClosest("ul,ol").length);bk.debug("Number of child nodes of the body: "+q(document.body).getContents().length);var bg=q(N).find(O);bk.debug("Found "+bg.length+" li elements in uls using find()");var bg=q(document.body).find(u);bk.debug("Found "+bg.length+" headers using find()");var bg=q(N).getNext()[0].tagName;bk.debug("Tag following the UL element: "+bg);var bg=q(N).getNextAll().length;bk.debug("Number of elements following the UL: "+bg);var bg=q(O).getParents().length;bk.debug("Number of parent elements of all li elems: "+bg);var bg=q(y).getAncestors().length;bk.debug("Number of all parent elements of the submit button: "+bg);var bg=q(N).getPrev()[0].tagName;bk.debug("Tag before the UL element: "+bg);var bg=q(N).getPrevAll().length;bk.debug("Number of elements preceding the UL: "+bg);var bg=q(w).getSiblings().length;bk.debug("List Item 3 has "+bg+" siblings");var bg=q(I).getOffsetParent()[0].tagName;bk.debug("Offset parent of select box: "+bg);var bg=q(I).getOffset();bk.debug("Offset of select box: ");bk.debug(bg);q(bb).eq(1).setStyle(M,t);var bg=q(K).filter(V);bk.debug("Found "+bg.length+" submit buttons using filter(selector)");var bg=q(K).filter(function(bm,bn,bo){return bm.type===l;}
);bk.debug("Found "+bg.length+" submit button using filter(function)");var bg=q(J).hasClass(n);bk.debug("Any H2 with form has a class 'form': "+bg);var bg=q(N).find(O).is(H);bk.debug("Found li element with text 'List Item 1': "+bg);var bg=q(N).find(O).not(H);bk.debug("Number of found li elements without text 'List Item 1': "+bg.length);var bg=q(N).find(O).not(g);bk.debug("Number of found li elements in the middle: "+bg.length);q(c).append(b);q(bd).after(h);q(S).before(p);var bl=document.createElement(I);var bh=document.createElement(L);q(bl).setAttribute(x,D).setAttribute(s,F);q(bh).setAttribute(d,D).setAttribute(j,be);q(I).after(bl).after(bh);q.create(A).appendTo(bl);qxWeb.create(a).insertAfter(J);q(z).on(K,function(){bk.debug("Typing in e-mail field...");}
);var bi=q(z);var bj=bi.add(q(k)[0]).reverse();bi.after(bj.clone(true));q(R).wrap(E);q(bc).wrap(o);}
}});}
)();
