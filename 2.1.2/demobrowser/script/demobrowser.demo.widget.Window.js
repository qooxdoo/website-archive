...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.Window.6b6798b601f8.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="demobrowser.demo.widget.Window",k="Yes",j="Second Window",h="Page 1",g="bottom",f="Use move frame",d="Show Statusbar",c="icon/16/actions/dialog-cancel.png",b="icon/32/status/dialog-error.png",a="Show Close",bb="icon/32/apps/office-address-book.png",ba="Third Window",Y="icon/22/apps/utilities-calculator.png",X="Show Maximize",W="Allow Close",V="Use resize frame",U="No",T="resizable",S="Allow Maximize",R="value",s="The second window",t="Page 3",q="move",r="Resizable ",o="First Window",p="resize",m="Moveable",n="Demo loaded",w="Allow Minimize",x="Resizable",F="Open Modal Dialog 2",D="Open Modal Dialog 1",J="First Modal Dialog",H="top",N="Movable",L="Show Minimize",z="Basics",Q="Modal",P="Do you want to fly to Berlin?",O="Welcome to your first own Window.<br/>Have fun!",y="Application is ready",B="icon/16/apps/internet-feed-reader.png",C="Second Modal Dialog",E="icon/16/actions/dialog-ok.png",G="Page 2",I="left",K="icon/16/apps/internet-telephony.png",M="right",u="icon/16/apps/office-calendar.png",v="execute",A="changeValue";qx.Class.define(l,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);this.createWindow1();this.createWindow2();this.createWindow3();}
,createWindow1:function(){var be=new qx.ui.window.Window(o,u);be.setLayout(new qx.ui.layout.VBox(10));be.setShowStatusbar(true);be.setStatus(n);be.open();this.getRoot().add(be,{left:20,top:20});be.addListener(q,function(e){this.debug("Moved to: "+e.getData().left+"x"+e.getData().top);}
,this);be.addListener(p,function(e){this.debug("Resized to: "+e.getData().width+"x"+e.getData().height);}
,this);var bd=new qx.ui.basic.Atom(O,bb);bd.setRich(true);be.add(bd);var bc=new qx.ui.tabview.TabView;be.add(bc,{flex:1});var bh=new qx.ui.tabview.Page(h);bc.add(bh);var bf=new qx.ui.tabview.Page(G);bc.add(bf);var bg=new qx.ui.tabview.Page(t);bc.add(bg);}
,createWindow2:function(){var bo=new qx.ui.window.Window(j,B);bo.setLayout(new qx.ui.layout.VBox(10));bo.setStatus(y);bo.open();this.getRoot().add(bo,{left:350,top:120});var bx=new qx.ui.basic.Atom(s,Y);bo.add(bx);var bi=new qx.ui.container.Composite;bi.setLayout(new qx.ui.layout.HBox(10));bo.add(bi,{flex:1});var bq=new qx.ui.groupbox.GroupBox(z);bq.setLayout(new qx.ui.layout.VBox(4));bi.add(bq,{flex:1});var bl=new qx.ui.form.CheckBox(a);bl.setValue(true);bl.addListener(A,function(e){bo.setShowClose(e.getData());}
);bq.add(bl);var bw=new qx.ui.form.CheckBox(X);bw.setValue(true);bw.addListener(A,function(e){bo.setShowMaximize(e.getData());}
);bq.add(bw);var bp=new qx.ui.form.CheckBox(L);bp.setValue(true);bp.addListener(A,function(e){bo.setShowMinimize(e.getData());}
);bq.add(bp);var bs=new qx.ui.form.CheckBox(W);bs.setValue(true);bs.addListener(A,function(e){bo.setAllowClose(e.getData());}
);bq.add(bs);var by=new qx.ui.form.CheckBox(S);by.setValue(true);by.addListener(A,function(e){bo.setAllowMaximize(e.getData());}
);bq.add(by);var bn=new qx.ui.form.CheckBox(w);bn.setValue(true);bn.addListener(A,function(e){bo.setAllowMinimize(e.getData());}
);bq.add(bn);var bj=new qx.ui.form.CheckBox(d);bj.setValue(false);bj.addListener(A,function(e){bo.setShowStatusbar(e.getData());}
);bq.add(bj);var bA=new qx.ui.groupbox.GroupBox(x);bA.setLayout(new qx.ui.layout.VBox(4));bi.add(bA,{flex:1});var bu=new qx.ui.form.CheckBox(V);bu.setValue(true);bu.addListener(A,function(e){bo.setUseResizeFrame(e.getData());}
);bA.add(bu);var bv=[H,M,g,I];for(var i=0;i<bv.length;i++ ){var bt=bv[i];var bm=new qx.ui.form.CheckBox(r+bt).set({value:true});bm.bind(R,bo,T+qx.lang.String.firstUp(bt));bA.add(bm);}
;var bk=new qx.ui.groupbox.GroupBox(m);bk.setLayout(new qx.ui.layout.VBox(4));bi.add(bk,{flex:1});var br=new qx.ui.form.CheckBox(N);br.setValue(true);br.addListener(A,function(e){bo.setMovable(e.getData());}
);bk.add(br);var bz=new qx.ui.form.CheckBox(f);bz.addListener(A,function(e){bo.setUseMoveFrame(e.getData());}
);bk.add(bz);}
,createWindow3:function(){var bB=new qx.ui.window.Window(ba,K);bB.setLayout(new qx.ui.layout.VBox);bB.setMinWidth(200);bB.setMaxWidth(450);bB.setMaxHeight(400);bB.setAllowMaximize(false);bB.open();this.getRoot().add(bB,{left:100,top:250});var bC=this.getModalWindow1();var bD=new qx.ui.form.Button(D,u);bD.addListener(v,bC.open,bC);bB.add(bD);}
,getModalWindow1:function(){var bF=new qx.ui.window.Window(J);bF.setLayout(new qx.ui.layout.VBox(10));bF.setModal(true);bF.moveTo(150,150);this.getRoot().add(bF);var bE=this.getModalWindow2();var bH=new qx.ui.form.Button(F,u);bH.addListener(v,bE.open,bE);bF.add(bH);var bG=new qx.ui.form.CheckBox(Q);bG.setValue(true);bF.add(bG);bG.addListener(A,function(e){bF.setModal(e.getData());}
);return bF;}
,getModalWindow2:function(){var bJ=new qx.ui.window.Window(C);bJ.setLayout(new qx.ui.layout.VBox(10));bJ.setModal(true);bJ.setShowClose(false);bJ.moveTo(300,300);this.getRoot().add(bJ);var bI=new qx.ui.basic.Atom(P,b);bJ.add(bI);var bL=new qx.ui.container.Composite;bL.setLayout(new qx.ui.layout.HBox(10,M));bJ.add(bL);var bM=new qx.ui.form.Button(k,E);bM.addListener(v,function(e){bJ.close();}
);bL.add(bM);var bK=new qx.ui.form.Button(U,c);bK.addListener(v,function(e){alert("Sorry, please click 'Yes'!");}
);bL.add(bK);return bJ;}
}});}
)();
