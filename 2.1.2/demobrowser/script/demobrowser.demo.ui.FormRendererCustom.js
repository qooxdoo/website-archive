...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormRendererCustom.529309bb5018.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j=" <span style='color:red'>*</span> ",h="abstract",g="qx.ui.form.renderer.AbstractRenderer",f=" :",d="",c="changeLocale",b="visibility",a="qx.dynlocale";qx.Class.define(g,{type:h,extend:qx.ui.core.Widget,implement:qx.ui.form.renderer.IFormRenderer,construct:function(k){qx.ui.core.Widget.call(this);this._visibilityBindingIds=[];this._labels=[];if(qx.core.Environment.get(a)){qx.locale.Manager.getInstance().addListener(c,this._onChangeLocale,this);this._names=[];}
;var o=k.getGroups();for(var i=0;i<o.length;i++ ){var n=o[i];this.addItems(n.items,n.labels,n.title,n.options,n.headerOptions);}
;var l=k.getButtons();var m=k.getButtonOptions();for(var i=0;i<l.length;i++ ){this.addButton(l[i],m[i]);}
;}
,members:{_names:null,_visibilityBindingIds:null,_labels:null,_connectVisibility:function(p,q){var r=p.bind(b,q,b);this._visibilityBindingIds.push({id:r,item:p});}
,_onChangeLocale:qx.core.Environment.select(a,{"true":function(e){for(var i=0;i<this._names.length;i++ ){var s=this._names[i];if(s.name&&s.name.translate){s.name=s.name.translate();}
;var t=this._createLabelText(s.name,s.item);s.label.setValue(t);}
;}
,"false":null}),_createLabelText:function(name,u){var v=d;if(u.getRequired()){v=j;}
;var w=name.length>0||u.getRequired()?f:d;return name+v+w;}
...
;for(var i=0;i<this._visibilityBindingIds.length;i++ ){var B=this._visibilityBindingIds[i];B.item.removeBinding(B.id);}
;}
});}
)();
...
(function(){var v="Name",u="demobrowser.demo.ui.FormRendererCustom",t="Registration",s="demobrowser.demo.ui.CustomRenderer",r="bio",q="Bio",p="Send",o="email",n="Contact",m="phone",e="Personal Information",l="Password",h="Country",c="Phone",b="Email",g="abc@def.gh",f="right",j="+01 234 56789",a="Reset",k="Please tell us some details about you...",d="execute";qx.Class.define(u,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var x=new qx.ui.form.Form();x.addGroupHeader(t);var w=new qx.ui.form.TextField();w.setRequired(true);x.add(w,v);var B=new qx.ui.form.TextField();B.setRequired(true);x.add(B,l);x.addGroupHeader(n);x.add(new qx.ui.form.TextField(),b,qx.util.Validate.email(),o,null,{placeholder:g});x.add(new qx.ui.form.TextField(),c,null,m,null,{placeholder:j});x.addGroupHeader(e);x.add(new qx.ui.form.TextField(),h);x.add(new qx.ui.form.TextArea(),q,null,r,null,{placeholder:k});var z=new qx.ui.form.Button(p);z.addListener(d,function(){if(x.validate()){alert("send...");}
;}
,this);x.addButton(z);var y=new qx.ui.form.Button(a);y.addListener(d,function(){x.reset();}
,this);x.addButton(y);var A=new demobrowser.demo.ui.CustomRenderer(x);this.getRoot().add(A,{left:10,top:10});}
}});qx.Class.define(s,{extend:qx.ui.form.renderer.Single,members:{__Sf:0,__Sg:0,addItems:function(C,D,E,F){var I=0;if(E!=null){this._add(this._createHeader(E),{row:I,column:this.__Sf,colSpan:2});I++ ;}
;for(var i=0;i<C.length;i++ ){var H=this._createLabel(D[i],C[i]);this._add(H,{row:I,column:this.__Sf});var G=C[i];H.setBuddy(G);if(G instanceof qx.ui.form.RadioGroup){G=this._createWidgetForRadioGroup(G);}
;this._add(G,{row:I,column:this.__Sf+1});if(F[i]&&F[i].placeholder){G.setPlaceholder(F[i].placeholder);}
;I++ ;}
;this.__Sf+=2;this.__Sg<I?this.__Sg=I:null;}
,addButton:function(J){if(this._buttonRow==null){this._buttonRow=new qx.ui.container.Composite();this._buttonRow.setMarginTop(5);var K=new qx.ui.layout.HBox();K.setAlignX(f);K.setSpacing(5);this._buttonRow.setLayout(K);this._add(this._buttonRow,{row:this.__Sg,column:0,colSpan:this.__Sf});}
;this._buttonRow.add(J);}
}});}
)();
(function(){var b="qx.ui.form.Form",a="";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__HD=[];this._buttons=[];this._buttonOptions=[];this._validationManager=new qx.ui.form.validation.Manager();this._resetter=this._createResetter();}
,members:{__HD:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(c,d,e,name,f,g){if(this.__HE()){this.__HD.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
;this.__HD[this._groupCounter].items.push(c);this.__HD[this._groupCounter].labels.push(d);this.__HD[this._groupCounter].options.push(g);if(name==null){name=d.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,a);}
;this.__HD[this._groupCounter].names.push(name);this._validationManager.add(c,e,f);this._resetter.add(c);}
,addGroupHeader:function(h,k){if(!this.__HE()){this._groupCounter++ ;}
;this.__HD.push({title:h,items:[],labels:[],names:[],options:[],headerOptions:k});}
,addButton:function(l,m){this._buttons.push(l);this._buttonOptions.push(m||null);}
,__HE:function(){return this.__HD.length===0;}
...
,getGroups:function(){return this.__HD;}
,getButtons:function(){return this._buttons;}
,getButtonOptions:function(){return this._buttonOptions;}
,getItems:function(){var o={};for(var i=0;i<this.__HD.length;i++ ){var p=this.__HD[i];for(var j=0;j<p.names.length;j++ ){var name=p.names[j];o[name]=p.items[j];}
;}
;return o;}
,_createResetter:function(){return new qx.ui.form.Resetter();}
},destruct:function(){this.__HD=this._buttons=this._buttonOptions=null;this._validationManager.dispose();this._resetter.dispose();}
});}
)();
(function(){var k="changeValid",j="qx.event.type.Event",h="value instanceof Function || qx.Class.isSubClassOf(value.constructor, qx.ui.form.validation.AsyncValidator)",g="qx.ui.form.validation.Manager",f="This field is required",d="qx.event.type.Data",c="complete",b="String",a="";qx.Class.define(g,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__Di=[];this.__Dj={};this.setRequiredFieldMessage(qx.locale.Manager.tr(f));}
,events:{"changeValid":d,"complete":j},properties:{validator:{check:h,init:null,nullable:true},invalidMessage:{check:b,init:a},requiredFieldMessage:{check:b,init:a},context:{nullable:true}},members:{__Di:null,__Dk:null,__Dj:null,__Dl:null,add:function(l,m,n){if(!this.__Dq(l)){throw new Error("Added widget not supported.");}
;if(this.__Af(l)&&!l.getValue){if(m!=null){throw new Error("Widgets supporting selection can only be validated "+"in the form validator");}
;}
;var o={item:l,validator:m,valid:null,context:n};this.__Di.push(o);}
,remove:function(p){var q=this.__Di;for(var i=0,r=q.length;i<r;i++ ){if(p===q[i].item){q.splice(i,1);return p;}
;}
;return null;}
,getItems:function(){var s=[];for(var i=0;i<this.__Di.length;i++ ){s.push(this.__Di[i].item);}
;return s;}
,validate:function(){var x=true;this.__Dl=true;var u=[];for(var i=0;i<this.__Di.length;i++ ){var v=this.__Di[i].item;var y=this.__Di[i].validator;u.push(v);if(y==null){var t=this.__Dm(v);x=x&&t;this.__Dl=t&&this.__Dl;continue;}
;var t=this.__Dn(this.__Di[i],v.getValue());x=t&&x;if(t!=null){this.__Dl=t&&this.__Dl;}
;}
;var w=this.__Do(u);if(qx.lang.Type.isBoolean(w)){this.__Dl=w&&this.__Dl;}
;x=w&&x;this.__Dr(x);if(qx.lang.Object.isEmpty(this.__Dj)){this.fireEvent(c);}
;return x;}
,__Dm:function(z){if(z.getRequired()){if(this.__Af(z)){var A=!!z.getSelection()[0];}
...
;return true;}
,__Dn:function(E,F){var K=E.item;var J=E.context;var I=E.validator;if(this.__Dp(I)){this.__Dj[K.toHashCode()]=null;I.validate(K,K.getValue(),this,J);return null;}
...
,__Do:function(L){var N=this.getValidator();var O=this.getContext()||this;if(N==null){return true;}
;this.setInvalidMessage(a);if(this.__Dp(N)){this.__Dj[this.toHashCode()]=null;N.validateForm(L,this,O);return null;}
...
;}
;return P;}
,__Dp:function(Q){var R=false;if(!qx.lang.Type.isFunction(Q)){R=qx.Class.isSubClassOf(Q.constructor,qx.ui.form.validation.AsyncValidator);}
;return R;}
,__Dq:function(S){var T=S.constructor;return qx.Class.hasInterface(T,qx.ui.form.IForm);}
,__Af:function(U){var V=U.constructor;return qx.Class.hasInterface(V,qx.ui.core.ISingleSelection);}
,__Dr:function(W){var X=this.__Dk;this.__Dk=W;if(X!=W){this.fireDataEvent(k,W,X);}
;}
,getValid:function(){return this.__Dk;}
,isValid:function(){return this.getValid();}
,getInvalidMessages:function(){var Y=[];for(var i=0;i<this.__Di.length;i++ ){var ba=this.__Di[i].item;if(!ba.getValid()){Y.push(ba.getInvalidMessage());}
;}
;if(this.getInvalidMessage()!=a){Y.push(this.getInvalidMessage());}
;return Y;}
,getInvalidFormItems:function(){var bb=[];for(var i=0;i<this.__Di.length;i++ ){var bc=this.__Di[i].item;if(!bc.getValid()){bb.push(bc);}
;}
;return bb;}
,reset:function(){for(var i=0;i<this.__Di.length;i++ ){var bd=this.__Di[i];bd.item.setValid(true);}
;this.__Dk=null;}
,setItemValid:function(be,bf){this.__Dj[be.toHashCode()]=bf;be.setValid(bf);this.__Ds();}
,setFormValid:function(bg){this.__Dj[this.toHashCode()]=bg;this.__Ds();}
,__Ds:function(){var bi=this.__Dl;for(var bj in this.__Dj){var bh=this.__Dj[bj];bi=bh&&bi;if(bh==null){return;}
;}
;this.__Dr(bi);this.__Dj={};this.fireEvent(c);}
},destruct:function(){this.__Di=null;}
});}
)();
(function(){var a="qx.ui.form.validation.AsyncValidator";qx.Class.define(a,{extend:qx.core.Object,construct:function(b){qx.core.Object.call(this);this.__Dt=b;}
,members:{__Dt:null,__Du:null,__fN:null,__Dv:null,validate:function(c,d,e,f){this.__Dv=false;this.__Du=c;this.__fN=e;this.__Dt.call(f||this,this,d);}
,validateForm:function(g,h,i){this.__Dv=true;this.__fN=h;this.__Dt.call(i,g,this);}
,setValid:function(j,k){if(this.__Dv){if(k!==undefined){this.__fN.setInvalidMessage(k);}
;this.__fN.setFormValid(j);}
else {if(k!==undefined){this.__Du.setInvalidMessage(k);}
;this.__fN.setItemValid(this.__Du,j);}
;}
},destruct:function(){this.__fN=this.__Du=null;}
});}
)();
...
(function(){var a="qx.ui.form.Resetter";qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__qC=[];}
,members:{__qC:null,add:function(b){if(this._supportsValue(b)){var c=b.getValue();}
else if(this.__Af(b)){var c=b.getSelection();}
else if(this.__Ag(b)){var c=b.getSelection().concat();}
else {throw new Error("Item "+b+" not supported for reseting.");}
;this.__qC.push({item:b,init:c});}
,reset:function(){for(var i=0;i<this.__qC.length;i++ ){var d=this.__qC[i];this.__Ad(d.item,d.init);}
;}
,resetItem:function(e){var f;for(var i=0;i<this.__qC.length;i++ ){var g=this.__qC[i];if(g.item===e){f=g.init;break;}
;}
;if(f===undefined){throw new Error("The given item has not been added.");}
;this.__Ad(e,f);}
,__Ad:function(h,j){if(this._supportsValue(h)){h.setValue(j);}
else if(this.__Af(h)||this.__Ag(h)){h.setSelection(j);}
;}
,redefine:function(){for(var i=0;i<this.__qC.length;i++ ){var k=this.__qC[i].item;this.__qC[i].init=this.__Ae(k);}
;}
,redefineItem:function(l){var m;for(var i=0;i<this.__qC.length;i++ ){if(this.__qC[i].item===l){m=this.__qC[i];break;}
;}
;if(m===undefined){throw new Error("The given item has not been added.");}
;m.init=this.__Ae(m.item);}
,__Ae:function(n){if(this._supportsValue(n)){return n.getValue();}
else if(this.__Af(n)||this.__Ag(n)){return n.getSelection();}
;}
,__Af:function(o){var p=o.constructor;return qx.Class.hasInterface(p,qx.ui.core.ISingleSelection);}
,__Ag:function(q){var r=q.constructor;return qx.Class.hasInterface(r,qx.data.controller.ISelection);}
,_supportsValue:function(s){var t=s.constructor;return (qx.Class.hasInterface(t,qx.ui.form.IBooleanForm)||qx.Class.hasInterface(t,qx.ui.form.IColorForm)||qx.Class.hasInterface(t,qx.ui.form.IDateForm)||qx.Class.hasInterface(t,qx.ui.form.INumberForm)||qx.Class.hasInterface(t,qx.ui.form.IStringForm));}
