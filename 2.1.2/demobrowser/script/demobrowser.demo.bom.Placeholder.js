...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Placeholder.49c4948d405c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var d="getPreviousSiblings",c="getSiblings",b="qx.module.Traversing",a="getNextSiblings";qx.Bootstrap.define(b,{statics:{add:function(e){this.push(e);return this;}
,getChildren:function(f){var h=[];for(var i=0;i<this.length;i++ ){var g=qx.dom.Hierarchy.getChildElements(this[i]);if(f){g=qx.bom.Selector.matches(f,g);}
;h=h.concat(g);}
;return qxWeb.$init(h);}
,forEach:function(j,k){for(var i=0;i<this.length;i++ ){j.call(k,this[i],i,this);}
;return this;}
,getParents:function(m){var o=[];for(var i=0;i<this.length;i++ ){var n=qx.dom.Element.getParentElement(this[i]);if(m){n=qx.bom.Selector.matches(m,[n]);}
;o=o.concat(n);}
;return qxWeb.$init(o);}
,getAncestors:function(p){return this.__lU(null,p);}
,getAncestorsUntil:function(q,r){return this.__lU(q,r);}
,__lU:function(s,t){var v=[];for(var i=0;i<this.length;i++ ){var parent=qx.dom.Element.getParentElement(this[i]);while(parent){var u=[parent];if(s&&qx.bom.Selector.matches(s,u).length>0){break;}
;if(t){u=qx.bom.Selector.matches(t,u);}
;v=v.concat(u);parent=qx.dom.Element.getParentElement(parent);}
;}
;return qxWeb.$init(v);}
,getClosest:function(w){var y=[];var x=function x(z){var A=qx.bom.Selector.matches(w,z);if(A.length){y.push(A[0]);}
else {z=z.getParents();if(z[0]&&z[0].parentNode){x(z);}
;}
;}
;for(var i=0;i<this.length;i++ ){x(qxWeb(this[i]));}
;return qxWeb.$init(y);}
,find:function(B){var C=[];for(var i=0;i<this.length;i++ ){C=C.concat(qx.bom.Selector.query(B,this[i]));}
;return qxWeb.$init(C);}
,getContents:function(){var D=[];for(var i=0;i<this.length;i++ ){D=D.concat(qx.lang.Array.fromCollection(this[i].childNodes));}
;return qxWeb.$init(D);}
,is:function(E){if(qx.lang.Type.isFunction(E)){return this.filter(E).length>0;}
;return !!E&&qx.bom.Selector.matches(E,this).length>0;}
...
,has:function(G){var H=[];for(var i=0;i<this.length;i++ ){var I=qx.bom.Selector.matches(G,this.eq(i).getContents());if(I.length>0){H.push(this[i]);}
;}
;return qxWeb.$init(H);}
,getNext:function(J){var K=this.map(qx.dom.Hierarchy.getNextElementSibling,qx.dom.Hierarchy);if(J){K=qx.bom.Selector.matches(J,K);}
;return K;}
,getNextAll:function(L){var M=qx.module.Traversing.__lV(this,a,L);return qxWeb.$init(M);}
,getNextUntil:function(N){var O=[];this.forEach(function(P,Q){var R=qx.dom.Hierarchy.getNextSiblings(P);for(var i=0,l=R.length;i<l;i++ ){if(qx.bom.Selector.matches(N,[R[i]]).length>0){break;}
;O.push(R[i]);}
;}
);return qxWeb.$init(O);}
,getPrev:function(S){var T=this.map(qx.dom.Hierarchy.getPreviousElementSibling,qx.dom.Hierarchy);if(S){T=qx.bom.Selector.matches(S,T);}
;return T;}
,getPrevAll:function(U){var V=qx.module.Traversing.__lV(this,d,U);return qxWeb.$init(V);}
,getPrevUntil:function(W){var X=[];this.forEach(function(Y,ba){var bb=qx.dom.Hierarchy.getPreviousSiblings(Y);for(var i=0,l=bb.length;i<l;i++ ){if(qx.bom.Selector.matches(W,[bb[i]]).length>0){break;}
;X.push(bb[i]);}
;}
);return qxWeb.$init(X);}
,getSiblings:function(bc){var bd=qx.module.Traversing.__lV(this,c,bc);return qxWeb.$init(bd);}
,not:function(be){if(qx.lang.Type.isFunction(be)){return this.filter(function(bg,bh,bi){return !be(bg,bh,bi);}
);}
;var bf=qx.bom.Selector.matches(be,this);return this.filter(function(bj){return bf.indexOf(bj)===-1;}
);}
...
,isElement:function(bk){return qx.dom.Node.isElement(bk);}
,isNode:function(bl){return qx.dom.Node.isNode(bl);}
,isDocument:function(bm){return qx.dom.Node.isDocument(bm);}
,getWindow:function(bn){return qx.dom.Node.getWindow(bn);}
,getDocument:function(bo){return qx.dom.Node.getDocument(bo);}
,__lV:function(bp,bq,br){var bt=[];var bs=qx.dom.Hierarchy;for(var i=0,l=bp.length;i<l;i++ ){bt.push.apply(bt,bs[bq](bp[i]));}
;var bu=qx.lang.Array.unique(bt);if(br){bu=qx.bom.Selector.matches(br,bu);}
;return bu;}
},defer:function(bv){qxWeb.$attach({"add":bv.add,"getChildren":bv.getChildren,"forEach":bv.forEach,"getParents":bv.getParents,"getAncestors":bv.getAncestors,"getAncestorsUntil":bv.getAncestorsUntil,"__lU":bv.__lU,"getClosest":bv.getClosest,"find":bv.find,"getContents":bv.getContents,"is":bv.is,"eq":bv.eq,"getFirst":bv.getFirst,"getLast":bv.getLast,"has":bv.has,"getNext":bv.getNext,"getNextAll":bv.getNextAll,"getNextUntil":bv.getNextUntil,"getPrev":bv.getPrev,"getPrevAll":bv.getPrevAll,"getPrevUntil":bv.getPrevUntil,"getSiblings":bv.getSiblings,"not":bv.not,"getOffsetParent":bv.getOffsetParent,"isRendered":bv.isRendered});qxWeb.$attachStatic({"isElement":bv.isElement,"isNode":bv.isNode,"isDocument":bv.isDocument,"getDocument":bv.getDocument,"getWindow":bv.getWindow});}
});}
)();
...
(function(){var r="ipod",q="tablet",p="g",o="xbox",n="pc",m="\.",l="mobile",k="(",j=")",i="iPhone",c="|",h="qx.bom.client.Device",f="wii",b="desktop",a="ipad",e="device.name",d="ds",g="device.type";qx.Bootstrap.define(h,{statics:{__cX:{"iPod":r,"iPad":a,"iPhone":i,"PSP":"psp","PLAYSTATION 3":"ps3","Nintendo Wii":f,"Nintendo DS":d,"XBOX":"xbox","Xbox":o},getName:function(){var u=[];for(var t in this.__cX){u.push(t);}
;var v=new RegExp(k+u.join(c).replace(/\./g,m)+j,p);var s=v.exec(navigator.userAgent);if(s&&s[1]){return qx.bom.client.Device.__cX[s[1]];}
;return n;}
...
;return b;}
,detectMobileDevice:function(x){return /android.+mobile|ip(hone|od)|bada\/|blackberry|maemo|opera m(ob|in)i|fennec|NetFront|phone|psp|symbian|IEMobile|windows (ce|phone)|xda/i.test(x);}
,detectTabletDevice:function(y){var A=(/MSIE 10/i.test(y))&&(/ARM/i.test(y))&&!(/windows phone/i.test(y));var z=(!(/Fennec|HTC.Magic|Nexus|android.+mobile|Tablet PC/i.test(y))&&(/Android|ipad|tablet|playbook|silk|kindle|psp/i.test(y)));return A||z;}
},defer:function(B){qx.core.Environment.add(e,B.getName);qx.core.Environment.add(g,B.getType);}
});}
)();
(function(){var h="engine.name",g="device.type",f="engine.version",e="qx.module.Environment",d="browser.version",c="browser.quirksmode",b="browser.name",a="browser.documentmode";qx.Bootstrap.define(e,{statics:{get:function(i){return qx.core.Environment.get(i);}
,add:function(j,k){qx.core.Environment.add(j,k);return this;}
},defer:function(l){qx.core.Environment.get(b);qx.core.Environment.get(d);qx.core.Environment.get(c);qx.core.Environment.get(a);qx.core.Environment.get(h);qx.core.Environment.get(f);qx.core.Environment.get(g);qxWeb.$attachStatic({"env":{get:l.get,add:l.add}});}
});}
)();
(function(){var b="qx.module.Attribute",a="html";qx.Bootstrap.define(b,{statics:{getHtml:function(){if(this[0]){return qx.bom.element.Attribute.get(this[0],a);}
;return null;}
,setHtml:function(c){c=qx.bom.Html.fixEmptyTags(c);for(var i=0;i<this.length;i++ ){qx.bom.element.Attribute.set(this[i],a,c);}
;return this;}
...
;return this;}
,getAttributes:function(f){var g={};for(var i=0;i<f.length;i++ ){g[f[i]]=this.getAttribute(f[i]);}
;return g;}
...
(function(){var m="<fieldset>",k="<select multiple='multiple'>",h="</div>",g="</select>",f="</tr></tbody></table>",e="<col",d="div",c="<table><tbody><tr>",b=">",a="<table><tbody></tbody><colgroup>",J="<th",I="</tbody></table>",H="<td",G="</colgroup></table>",F="<opt",E="text/javascript",D="",C="</fieldset>",B="<table><tbody>",A="div<div>",t="<table",u="mshtml",r="qx.bom.Html",s="<leg",p="tbody",q="<tr",n="</table>",o="></",v="string",w="script",y="<table>",x="engine.name",z="undefined";qx.Bootstrap.define(r,{statics:{__lR:function(K,L,M){return M.match(/^(abbr|br|col|img|input|link|meta|param|hr|area|embed)$/i)?K:L+o+M+b;}
,__lS:{opt:[1,k,g],leg:[1,m,C],table:[1,y,n],tr:[2,B,I],td:[3,c,f],col:[2,a,G],def:qx.core.Environment.select(x,{"mshtml":[1,A,h],"default":null})},fixEmptyTags:function(N){return N.replace(/(<(\w+)[^>]*?)\/>/g,this.__lR);}
,__lT:function(O,P){var V=P.createElement(d);O=qx.bom.Html.fixEmptyTags(O);var R=O.replace(/^\s+/,D).substring(0,5).toLowerCase();var U,Q=this.__lS;if(!R.indexOf(F)){U=Q.opt;}
else if(!R.indexOf(s)){U=Q.leg;}
else if(R.match(/^<(thead|tbody|tfoot|colg|cap)/)){U=Q.table;}
else if(!R.indexOf(q)){U=Q.tr;}
else if(!R.indexOf(H)||!R.indexOf(J)){U=Q.td;}
else if(!R.indexOf(e)){U=Q.col;}
else {U=Q.def;}
;if(U){V.innerHTML=U[1]+O+U[2];var T=U[0];while(T-- ){V=V.lastChild;}
;}
else {V.innerHTML=O;}
;if((qx.core.Environment.get(x)==u)){var W=/<tbody/i.test(O);var S=!R.indexOf(t)&&!W?V.firstChild&&V.firstChild.childNodes:U[1]==y&&!W?V.childNodes:[];for(var j=S.length-1;j>=0; --j){if(S[j].tagName.toLowerCase()===p&&!S[j].childNodes.length){S[j].parentNode.removeChild(S[j]);}
;}
;if(/^\s/.test(O)){V.insertBefore(P.createTextNode(O.match(/^\s*/)[0]),V.firstChild);}
;}
;return qx.lang.Array.fromCollection(V.childNodes);}
,clean:function(X,Y,ba){Y=Y||document;if(typeof Y.createElement===z){Y=Y.ownerDocument||Y[0]&&Y[0].ownerDocument||document;}
;if(!ba&&X.length===1&&typeof X[0]===v){var bg=/^<(\w+)\s*\/?>$/.exec(X[0]);if(bg){return [Y.createElement(bg[1])];}
;}
;var bc,bd=[];for(var i=0,l=X.length;i<l;i++ ){bc=X[i];if(typeof bc===v){bc=this.__lT(bc,Y);}
;if(bc.nodeType){bd.push(bc);}
else if(bc instanceof qx.type.BaseArray||(typeof qxWeb!==z&&bc instanceof qxWeb)){bd.push.apply(bd,Array.prototype.slice.call(bc,0));}
else if(bc.toElement){bd.push(bc.toElement());}
else {bd.push.apply(bd,bc);}
;}
;if(ba){var bf=[],be;for(var i=0;bd[i];i++ ){be=bd[i];if(be.nodeType==1&&be.tagName.toLowerCase()===w&&(!be.type||be.type.toLowerCase()===E)){if(be.parentNode){be.parentNode.removeChild(bd[i]);}
;bf.push(be);}
else {if(be.nodeType===1){var bb=qx.lang.Array.fromCollection(be.getElementsByTagName(w));bd.splice.apply(bd,[i+1,0].concat(bb));}
;ba.appendChild(be);}
;}
;return bf;}
;return bd;}
...
(function(){var d="scrollLeft",c="qx.module.Manipulating",b="scrollTop",a="ease-in";qx.Bootstrap.define(c,{statics:{create:function(e){return qxWeb.$init(qx.bom.Html.clean([e]));}
,clone:function(f){var g=[];for(var i=0;i<this.length;i++ ){g[i]=this[i].cloneNode(true);}
;if(f===true&&this.copyEventsTo){this.copyEventsTo(g);}
;return qxWeb(g);}
,append:function(h){var k=qx.bom.Html.clean([h]);var n=qxWeb.$init(k);for(var i=0,l=this.length;i<l;i++ ){for(var j=0,m=n.length;j<m;j++ ){if(i==0){qx.dom.Element.insertEnd(n[j],this[i]);}
else {qx.dom.Element.insertEnd(n.eq(j).clone(true)[0],this[i]);}
;}
;}
;return this;}
,appendTo:function(parent){parent=qx.module.Manipulating.__lW(parent);for(var i=0,l=parent.length;i<l;i++ ){for(var j=0,m=this.length;j<m;j++ ){if(i==0){qx.dom.Element.insertEnd(this[j],parent[i]);}
else {qx.dom.Element.insertEnd(this.eq(j).clone(true)[0],parent[i]);}
;}
;}
;return this;}
,insertBefore:function(o){o=qx.module.Manipulating.__lW(o);for(var i=0,l=o.length;i<l;i++ ){for(var j=0,m=this.length;j<m;j++ ){if(i==0){qx.dom.Element.insertBefore(this[j],o[i]);}
else {qx.dom.Element.insertBefore(this.eq(j).clone(true)[0],o[i]);}
;}
;}
;return this;}
,insertAfter:function(p){p=qx.module.Manipulating.__lW(p);for(var i=0,l=p.length;i<l;i++ ){for(var j=this.length-1;j>=0;j-- ){if(i==0){qx.dom.Element.insertAfter(this[j],p[i]);}
else {qx.dom.Element.insertAfter(this.eq(j).clone(true)[0],p[i]);}
;}
;}
;return this;}
,__lW:function(q){if(!qx.lang.Type.isArray(q)){var r=qxWeb(q);q=r.length>0?r:[q];}
;return q;}
,wrap:function(s){var s=qx.module.Manipulating.__lX(s);if(s.length==0||!qx.dom.Node.isElement(s[0])){return this;}
;for(var i=0,l=this.length;i<l;i++ ){var t=s.eq(0).clone(true);qx.dom.Element.insertAfter(t[0],this[i]);var u=qx.module.Manipulating.__lY(t[0]);qx.dom.Element.insertEnd(this[i],u);}
;return this;}
,__lX:function(v){var w;if(qx.lang.Type.isArray(v)){w=qxWeb(v);}
else {var x=qx.bom.Html.clean([v]);if(x.length>0&&qx.dom.Node.isElement(x[0])){w=qxWeb(x);}
else {w=qxWeb(v);}
;}
;return w;}
,__lY:function(y){if(y.childNodes.length==0){return y;}
;for(var i=0,l=y.childNodes.length;i<l;i++ ){if(y.childNodes[i].nodeType===1){return this.__lY(y.childNodes[i]);}
;}
;return y;}
,remove:function(){for(var i=0;i<this.length;i++ ){qx.dom.Element.remove(this[i]);}
;return this;}
,empty:function(){for(var i=0;i<this.length;i++ ){while(this[i].firstChild){this[i].removeChild(this[i].firstChild);}
;}
;return this;}
,before:function(content){if(!qx.lang.Type.isArray(content)){content=[content];}
;var z=document.createDocumentFragment();qx.bom.Html.clean(content,document,z);this.forEach(function(A,B){var C=qx.lang.Array.cast(z.childNodes,Array);for(var i=0,l=C.length;i<l;i++ ){var D;if(B<this.length-1){D=C[i].cloneNode(true);}
else {D=C[i];}
;A.parentNode.insertBefore(D,A);}
;}
,this);return this;}
,after:function(content){if(!qx.lang.Type.isArray(content)){content=[content];}
;var E=document.createDocumentFragment();qx.bom.Html.clean(content,document,E);this.forEach(function(F,G){var H=qx.lang.Array.cast(E.childNodes,Array);for(var i=H.length-1;i>=0;i-- ){var I;if(G<this.length-1){I=H[i].cloneNode(true);}
else {I=H[i];}
;F.parentNode.insertBefore(I,F.nextSibling);}
;}
,this);return this;}
,getScrollLeft:function(){var J=this[0];if(!J){return null;}
;var Node=qx.dom.Node;if(Node.isWindow(J)||Node.isDocument(J)){return qx.bom.Viewport.getScrollLeft();}
;return J.scrollLeft;}
,getScrollTop:function(){var K=this[0];if(!K){return null;}
;var Node=qx.dom.Node;if(Node.isWindow(K)||Node.isDocument(K)){return qx.bom.Viewport.getScrollTop();}
;return K.scrollTop;}
,_animationDescription:{scrollLeft:{duration:700,timing:a,keep:100,keyFrames:{'0':{},'100':{scrollLeft:1}}},scrollTop:{duration:700,timing:a,keep:100,keyFrames:{'0':{},'100':{scrollTop:1}}}},__ma:function(L,M,N){var O=qx.lang.Object.clone(qx.module.Manipulating._animationDescription[L],true);O.keyFrames[100][L]=M;return this.animate(O,N);}
,setScrollLeft:function(P,Q){var Node=qx.dom.Node;if(Q&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__ma.bind(this,d,P,Q)();}
;for(var i=0,l=this.length,R;i<l;i++ ){R=this[i];if(Node.isElement(R)){if(!(Q&&qx.bom.element&&qx.bom.element.AnimationJs)){R.scrollLeft=P;}
;}
else if(Node.isWindow(R)){R.scrollTo(P,this.getScrollTop(R));}
else if(Node.isDocument(R)){Node.getWindow(R).scrollTo(P,this.getScrollTop(R));}
;}
;return this;}
,setScrollTop:function(S,T){var Node=qx.dom.Node;if(T&&qx.bom.element&&qx.bom.element.AnimationJs){qx.module.Manipulating.__ma.bind(this,b,S,T)();}
;for(var i=0,l=this.length,U;i<l;i++ ){U=this[i];if(Node.isElement(U)){if(!(T&&qx.bom.element&&qx.bom.element.AnimationJs)){U.scrollTop=S;}
;}
else if(Node.isWindow(U)){U.scrollTo(this.getScrollLeft(U),S);}
else if(Node.isDocument(U)){Node.getWindow(U).scrollTo(this.getScrollLeft(U),S);}
;}
;return this;}
,focus:function(){try{this[0].focus();}
catch(V){}
;return this;}
,blur:function(){this.forEach(function(W,X){try{W.blur();}
catch(Y){}
;}
);return this;}
},defer:function(ba){qxWeb.$attachStatic({"create":ba.create});qxWeb.$attach({"append":ba.append,"appendTo":ba.appendTo,"remove":ba.remove,"empty":ba.empty,"before":ba.before,"insertBefore":ba.insertBefore,"after":ba.after,"insertAfter":ba.insertAfter,"wrap":ba.wrap,"clone":ba.clone,"getScrollLeft":ba.getScrollLeft,"setScrollLeft":ba.setScrollLeft,"getScrollTop":ba.getScrollTop,"setScrollTop":ba.setScrollTop,"focus":ba.focus,"blur":ba.blur});}
});}
)();
...
(function(){var g="placeholder",f="qx.module.Placeholder",e="tagName",d="INPUT",c="TEXTAREA",b="input[placeholder], textarea[placeholder]",a="css.placeholder";qx.Bootstrap.define(f,{statics:{PLACEHOLDER_NAME:"$qx_placeholder",update:function(){if(!qxWeb.env.get(a)){qxWeb(b).updatePlaceholder();}
;}
,updatePlaceholder:function(){if(!qxWeb.env.get(a)){for(var i=0;i<this.length;i++ ){var k=qxWeb(this[i]);var n=k.getAttribute(g);var m=k.getProperty(e);if(!n||(m!=c&&m!=d)){continue;}
;var h=k.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);if(!h){h=qx.module.Placeholder.__HI(k);}
;var j=k.isRendered();var l=h.isRendered();if(j&&!l){k.before(h);}
else if(!j&&l){h.remove();return this;}
;qx.module.Placeholder.__HH(k);}
;}
;return this;}
,__HH:function(o){var r=o.getAttribute("placeholder");var q=o.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);var p=o.getStyle("z-index");var s=parseInt(o.getStyle("padding-left"))+2*parseInt(o.getStyle("padding-right"));var t=parseInt(o.getStyle("padding-top"))+2*parseInt(o.getStyle("padding-bottom"));q.setHtml(r).setStyles({display:o.getValue()==""?"inline":"none",zIndex:p=="auto"?1:p+1,textAlign:o.getStyle("text-align"),width:(o.getWidth()-s-4)+"px",height:(o.getHeight()-t-4)+"px",left:o.getOffset().left+"px",top:o.getOffset().top+"px",fontFamily:o.getStyle("font-family"),fontStyle:o.getStyle("font-style"),fontVariant:o.getStyle("font-variant"),fontWeight:o.getStyle("font-weight"),fontSize:o.getStyle("font-size"),paddingTop:(parseInt(o.getStyle("padding-top"))+2)+"px",paddingRight:(parseInt(o.getStyle("padding-right"))+2)+"px",paddingBottom:(parseInt(o.getStyle("padding-bottom"))+2)+"px",paddingLeft:(parseInt(o.getStyle("padding-left"))+2)+"px"});}
,__HI:function(u){var v=qxWeb.create("<label>").setStyles({position:"absolute",color:"#989898",overflow:"hidden",pointerEvents:"none"});u.setProperty(qx.module.Placeholder.PLACEHOLDER_NAME,v);u.on("keyup",function(w){var x=w.getProperty(qx.module.Placeholder.PLACEHOLDER_NAME);x.setStyle("display",w.getValue()==""?"inline":"none");}
.bind(this,u));if(!qxWeb.env.get("event.pointer")){v.setStyle("cursor","text").on("click",function(y){y.focus();}
.bind(this,u));}
;return v;}
},defer:function(z){qxWeb.$attachStatic({"placeholder":{update:z.update}});qxWeb.$attach({"updatePlaceholder":z.updatePlaceholder});}
});}
)();
...
(function(){var p="monospace",o="cursive",n="input[type=button]",m="bold",l="sans-serif",k="css.placeholder",j="<input type='text' placeholder='",i="italic",h="#support",g="demobrowser.demo.bom.Placeholder",c="serif",f="disabled",e="'/><br>",b="click",a="normal",d="px";qx.Class.define(g,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);q(h).setHtml(q.env.get(k));q(n).setAttribute(f,null).on(b,this.addTextField);q.placeholder.update();}
,addTextField:function(){var s=Date.now();var r=q.create(j+s+e).appendTo(document.body);r.setStyles({width:((s%100)+50)+d,padding:(s%20)+d,fontSize:((s%30)+10)+d,fontWeight:s%2?m:a,fontStyle:s%2?i:a,fontFamily:[p,c,l,o][s%4]}).updatePlaceholder();}
}});}
)();
