...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.ListWithFilter.0a1a75aa4d4b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,_init:function(){this._provider=new qx.ui.list.provider.WidgetProvider(this);this.__xR=[];this.__xS=[];this.__xT={};this.__xU=false;this.__xV=false;this.__xW=false;this.getPane().addListener(v,this._onResize,this);this._initBackground();this._initLayer();}
...
;}
,_getLookupTable:function(){return this.__xR;}
,_lookup:function(M){return this.__xR[M];}
,_lookupGroup:function(N){return this.__xS.indexOf(N);}
,_reverseLookup:function(O){if(O<0){return -1;}
;return this.__xR.indexOf(O);}
...
,_applyDelegate:function(bh,bi){this._provider.setDelegate(bh);this.__xY();}
,_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);}
,_onModelChange:function(e){this.__xY();this._applyDefaultSelection();}
,__xX:function(){this.getPane().getRowConfig().setItemCount(this.__xR.length);this.getPane().fullUpdate();}
,__xY:function(){this.__xR=[];this.__xS=[];this.__xT={};if(this.isAutoGrouping()){this.getGroups().removeAll();}
;var bj=this.getModel();if(bj!=null){this._runDelegateFilter(bj);this._runDelegateSorter(bj);this._runDelegateGroup(bj);}
;this._updateSelection();this.__xX();}
,_runDelegateFilter:function(bk){var bl=qx.util.Delegate.getMethod(this.getDelegate(),g);for(var i=0,l=bk.length;i<l; ++i){if(bl==null||bl(bk.getItem(i))){this.__xR.push(i);}
;}
;}
,_runDelegateSorter:function(bm){if(this.__xR.length==0){return;}
;var bn=qx.util.Delegate.getMethod(this.getDelegate(),c);if(bn!=null){this.__xR.sort(function(a,b){return bn(bm.getItem(a),bm.getItem(b));}
);}
;}
,_runDelegateGroup:function(bo){var bs=qx.util.Delegate.getMethod(this.getDelegate(),r);if(bs!=null){for(var i=0,l=this.__xR.length;i<l; ++i){var bp=this.__xR[i];var br=this.getModel().getItem(bp);var bq=bs(br);this.__ya(bq,bp);}
;this.__xR=this.__yb();}
;}
,__ya:function(bt,bu){if(bt==null){this.__xW=true;bt=B;}
;var name=this.__yc(bt);if(this.__xT[name]==null){this.__xT[name]=[];if(this.isAutoGrouping()){this.getGroups().push(bt);}
;}
;this.__xT[name].push(bu);}
,__yb:function(){this.__yd();var by=[];var bA=0;var bw=this.getGroups();for(var i=0;i<bw.getLength();i++ ){var bv=bw.getItem(i);by.push(-1);this.__xS.push(bA);bA++ ;var bz=this.__yc(bv);var bx=this.__xT[bz];if(bx!=null){for(var k=0;k<bx.length;k++ ){by.push(bx[k]);bA++ ;}
;}
;}
;return by;}
,__yc:function(bB){var name=null;if(!qx.lang.Type.isString(bB)){var bC=this.getGroups().indexOf(bB);this.__xV=true;name=r;if(bC==-1){name+=this.getGroups().getLength();}
else {name+=bC;}
;}
else {this.__xU=true;var name=bB;}
;return name;}
,__yd:function(){if(this.__xV&&this.__xW||this.__xV&&this.__xU){throw new Error("GroupingTypeError: You can't mix 'Objects' and 'Strings' as"+" group identifier!");}
;}
...
;this._background.dispose();this._provider.dispose();this._layer.dispose();this._background=this._provider=this._layer=this.__xR=this.__xS=this.__xT=null;if(this.__xQ){this.__xQ.dispose();}
;}
});}
)();
...
;this.__ye={};this.__yf={};}
,properties:{colorEven:{nullable:true,check:a,apply:c,themeable:true},colorOdd:{nullable:true,check:a,apply:d,themeable:true}},members:{__yg:null,__yh:null,__ye:null,__yf:null,setColor:function(g,h){if(h){this.__ye[g]=qx.theme.manager.Color.getInstance().resolve(h);}
else {delete (this.__ye[g]);}
;}
,clearCustomColors:function(){this.__ye={};this.updateLayerData();}
,getColor:function(i){var j=this.__ye[i];if(j){return j;}
else {return i%2==0?this.__yg:this.__yh;}
;}
,_applyColorEven:function(k,l){if(k){this.__yg=qx.theme.manager.Color.getInstance().resolve(k);}
else {this.__yg=null;}
;this.updateLayerData();}
,_applyColorOdd:function(m,n){if(m){this.__yh=qx.theme.manager.Color.getInstance().resolve(m);}
else {this.__yh=null;}
;this.updateLayerData();}
,setBackground:function(o,p){if(p){this.__yf[o]=qx.theme.manager.Decoration.getInstance().resolve(p);}
else {delete (this.__yf[o]);}
;this.updateLayerData();}
,getBackground:function(q){return this.__yf[q];}
},destruct:function(){this.__ye=this.__yf=null;}
});}
)();
...
;}
,setColor:function(J,K){qx.ui.virtual.layer.AbstractBackground.prototype.setColor.call(this,J,K);if(this.__yi(J)){this.updateLayerData();}
;}
,setBackground:function(L,M){qx.ui.virtual.layer.AbstractBackground.prototype.setBackground.call(this,L,M);if(this.__yi(L)){this.updateLayerData();}
;}
,__yi:function(N){var P=this.getFirstRow();var O=P+this.getRowSizes().length-1;return N>=P&&N<=O;}
}});}
)();
...
(function(){var p="model[",o="groups[",n="model",m="changeDelegate",l="label",k="qx.ui.list.core.MWidgetController",j="icon",i="value",h=".",g="",c="]",f="BindingIds",d="group",b="cell.type",a="String";qx.Mixin.define(k,{construct:function(){this.__xd=[];}
...
;}
,bindProperty:function(s,t,u,v,w){var y=v.getUserData(b);var x=this.__xe(w,s,y);var z=this._list.bind(x,v,t,u);this.__xf(v,z);}
,bindPropertyReverse:function(A,B,C,D,E){var G=D.getUserData(b);var F=this.__xe(E,A,G);var H=D.bind(B,this._list,F,C);this.__xf(D,H);}
,removeBindings:function(){while(this.__xd.length>0){var I=this.__xd.pop();this._removeBindingsFrom(I);}
;}
...
;}
,_removeBindingsFrom:function(T){var U=this.__xg(T);while(U.length>0){var V=U.pop();try{this._list.removeBinding(V);}
catch(e){T.removeBinding(V);}
;}
;if(qx.lang.Array.contains(this.__xd,T)){qx.lang.Array.remove(this.__xd,T);}
;}
,__xe:function(W,X,Y){var ba=p+W+c;if(Y==d){ba=o+W+c;}
;if(X!=null&&X!=g){ba+=h+X;}
;return ba;}
,__xf:function(bb,bc){var bd=this.__xg(bb);if(!qx.lang.Array.contains(bd,bc)){bd.push(bc);}
;if(!qx.lang.Array.contains(this.__xd,bb)){this.__xd.push(bb);}
;}
,__xg:function(be){var bf=be.getUserData(f);if(bf==null){bf=[];be.setUserData(f,bf);}
;return bf;}
...
,styleSelectabled:function(u){var v=this.__xh(u);this._styleSelectabled(v);}
,styleUnselectabled:function(w){var x=this.__xh(w);this._styleUnselectabled(x);}
