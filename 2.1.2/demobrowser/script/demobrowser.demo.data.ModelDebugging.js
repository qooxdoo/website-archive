...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ModelDebugging.606e7c22ba9a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var s="head",r="unknown",q="readystatechange",p="script",o="load",n="opera",m="abort",l="on",k="timeout",j="Unknown response headers",d="error",i="qx.debug",g="mshtml",c="",b="loadend",f="browser.documentmode",e="engine.name",h="qx.debug.io",a="qx.bom.request.Script";qx.Bootstrap.define(a,{construct:function(){this.__mR();this.__mI=qx.Bootstrap.bind(this._onNativeLoad,this);this.__mJ=qx.Bootstrap.bind(this._onNativeError,this);this.__mK=qx.Bootstrap.bind(this._onTimeout,this);this.__mL=document.head||document.getElementsByTagName(s)[0]||document.documentElement;this._emitter=new qx.event.Emitter();this.timeout=this.__mT()?0:15000;}
,events:{"readystatechange":a,"error":a,"loadend":a,"timeout":a,"abort":a,"load":a},members:{readyState:null,status:null,statusText:null,timeout:null,__mM:null,on:function(name,t,u){this._emitter.on(name,t,u);return this;}
,open:function(v,w){if(this.__mP){return;}
;this.__mR();this.__mN=null;this.__mO=w;if(this.__mW(h)){qx.Bootstrap.debug(qx.bom.request.Script,"Open native request with "+"url: "+w);}
;this._readyStateChange(1);}
,setRequestHeader:function(x,y){if(this.__mP){return null;}
;var z={};if(this.readyState!==1){throw new Error("Invalid state");}
;z[x]=y;this.__mO=qx.util.Uri.appendParamsToUrl(this.__mO,z);return this;}
,send:function(){if(this.__mP){return null;}
;var C=this.__mU(),A=this.__mL,B=this;if(this.timeout>0){this.__fK=window.setTimeout(this.__mK,this.timeout);}
;if(this.__mW(h)){qx.Bootstrap.debug(qx.bom.request.Script,"Send native request");}
;A.insertBefore(C,A.firstChild);window.setTimeout(function(){B._readyStateChange(2);B._readyStateChange(3);}
);return this;}
,abort:function(){if(this.__mP){return null;}
;this.__mN=true;this.__mV();this._emit(m);return this;}
...
,onabort:function(){}
,ontimeout:function(){}
,getResponseHeader:function(D){if(this.__mP){return null;}
;if(this.__mW(i)){qx.Bootstrap.debug("Response header cannot be determined for "+"requests made with script transport.");}
;return r;}
,getAllResponseHeaders:function(){if(this.__mP){return null;}
;if(this.__mW(i)){qx.Bootstrap.debug("Response headers cannot be determined for"+"requests made with script transport.");}
;return j;}
,setDetermineSuccess:function(E){this.__mM=E;}
,dispose:function(){var F=this.__mQ;if(!this.__mP){if(F){F.onload=F.onreadystatechange=null;this.__mV();}
;if(this.__fK){window.clearTimeout(this.__fK);}
;this.__mP=true;}
;}
,_getUrl:function(){return this.__mO;}
,_getScriptElement:function(){return this.__mQ;}
,_onTimeout:function(){this.__mS();if(!this.__mT()){this._emit(d);}
;this._emit(k);if(!this.__mT()){this._emit(b);}
;}
,_onNativeLoad:function(){var I=this.__mQ,G=this.__mM,H=this;if(this.__mN){return;}
;if(this.__mW(e)===g&&this.__mW(f)<9){if(!(/loaded|complete/).test(I.readyState)){return;}
else {if(this.__mW(h)){qx.Bootstrap.debug(qx.bom.request.Script,"Received native readyState: loaded");}
;}
;}
;if(this.__mW(h)){qx.Bootstrap.debug(qx.bom.request.Script,"Received native load");}
;if(G){if(!this.status){this.status=G()?200:500;}
;}
;if(this.status===500){if(this.__mW(h)){qx.Bootstrap.debug(qx.bom.request.Script,"Detected error");}
;}
;if(this.__fK){window.clearTimeout(this.__fK);}
;window.setTimeout(function(){H._success();H._readyStateChange(4);H._emit(o);H._emit(b);}
);}
,_onNativeError:function(){this.__mS();this._emit(d);this._emit(b);}
,__mQ:null,__mL:null,__mO:c,__mI:null,__mJ:null,__mK:null,__fK:null,__mN:null,__mP:null,__mR:function(){this.readyState=0;this.status=0;this.statusText=c;}
,_readyStateChange:function(J){this.readyState=J;this._emit(q);}
,_success:function(){this.__mV();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=c+this.status;}
,__mS:function(){this.__mV();this.readyState=4;this.status=0;this.statusText=null;}
,__mT:function(){var L=this.__mW(e)===g&&this.__mW(f)<9;var K=this.__mW(e)===n;return !(L||K);}
,__mU:function(){var M=this.__mQ=document.createElement(p);M.src=this.__mO;M.onerror=this.__mJ;M.onload=this.__mI;if(this.__mW(e)===g&&this.__mW(f)<9){M.onreadystatechange=this.__mI;}
;return M;}
,__mV:function(){var N=this.__mQ;if(N&&N.parentNode){this.__mL.removeChild(N);}
;}
,__mW:function(O){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(O);}
...
(function(){var k="qx.bom.request.Jsonp",j="callback",i="open",h="dispose",g="qx.debug",f="_onNativeLoad",e="qx",d=".callback",c="qx.bom.request.Jsonp.",b="",a="qx.debug.io";qx.Bootstrap.define(k,{extend:qx.bom.request.Script,construct:function(){qx.bom.request.Script.apply(this);this.__yI();}
,members:{responseJson:null,__bV:null,__yB:null,__yC:null,__yD:null,__yE:null,__yF:null,__mP:null,__yG:b,open:function(l,m){if(this.__mP){return;}
;var n={},p,o,q=this;this.responseJson=null;this.__yD=false;p=this.__yB||j;o=this.__yC||this.__yG+c+this.__bV+d;if(!this.__yC){this.constructor[this.__bV]=this;}
else {if(!window[this.__yC]){this.__yE=true;window[this.__yC]=function(r){q.callback(r);}
;}
else {if(qx.core.Environment.get(a)){qx.Bootstrap.debug(qx.bom.request.Jsonp,"Callback "+this.__yC+" already exists");}
;}
;}
;if(qx.core.Environment.get(a)){qx.Bootstrap.debug(qx.bom.request.Jsonp,"Expecting JavaScript response to call: "+o);}
;n[p]=o;this.__yF=m=qx.util.Uri.appendParamsToUrl(m,n);this.__ic(i,[l,m]);}
,callback:function(s){if(this.__mP){return;}
;this.__yD=true;if(qx.core.Environment.get(g)){s=qx.lang.Json.stringify(s);s=qx.lang.Json.parse(s);}
;this.responseJson=s;this.constructor[this.__bV]=undefined;this.__yH();}
,setCallbackParam:function(t){this.__yB=t;return this;}
,setCallbackName:function(name){this.__yC=name;return this;}
,setPrefix:function(u){this.__yG=u;}
,getGeneratedUrl:function(){return this.__yF;}
,dispose:function(){this.__yH();this.__ic(h);}
,_onNativeLoad:function(){this.status=this.__yD?200:500;this.__ic(f);}
,__yH:function(){if(this.__yE&&window[this.__yC]){window[this.__yC]=undefined;this.__yE=false;}
;}
,__ic:function(v,w){qx.bom.request.Script.prototype[v].apply(this,w||[]);}
,__yI:function(){this.__bV=e+(new Date().valueOf())+(b+Math.random()).substring(2,5);}
}});}
)();
...
;return bp;}
;return A;}
,startDisposeProfiling:qx.core.Environment.select(v,{"true":function(){this.disposeProfilingActive=true;this.__Ra=qx.core.ObjectRegistry.getNextHash();}
,"default":(function(){}
)}),stopDisposeProfiling:qx.core.Environment.select(v,{"true":function(br){if(!this.__Ra){qx.log.Logger.error("Call "+this.classname+".startDisposeProfiling first.");return [];}
;this.disposeProfilingActive=false;var bt=[];while(!qx.ui.core.queue.Dispose.isEmpty()){qx.ui.core.queue.Dispose.flush();}
;var bu=qx.core.ObjectRegistry.getNextHash();var bw=qx.core.ObjectRegistry.getPostId();var bx=qx.core.ObjectRegistry.getStackTraces();for(var bs=this.__Ra;bs<bu;bs++ ){var bv=qx.core.ObjectRegistry.fromHashCode(bs+bw);if(bv&&bv.isDisposed&&!bv.isDisposed()){if(br&&typeof br==g&&!br(bv)){continue;}
