...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Table_Header.6bd0a1504b83.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,properties:{selectionModel:{check:b}},members:{__uC:null,handleMouseDown:function(c,d){if(d.isLeftPressed()){var e=this.getSelectionModel();if(!e.isSelectedIndex(c)){this._handleSelectEvent(c,d);this.__uC=true;}
else {this.__uC=false;}
;}
else if(d.isRightPressed()&&d.getModifiers()==0){var e=this.getSelectionModel();if(!e.isSelectedIndex(c)){e.setSelectionInterval(c,c);}
;}
;}
,handleMouseUp:function(f,g){if(g.isLeftPressed()&&!this.__uC){this._handleSelectEvent(f,g);}
;}
...
(function(){var n="]",m="..",l="changeSelection",k=" [",h='ie',g="browser.version",f="qx.event.type.Event",d="Ranges:",c="qx.ui.table.selection.Model",b="browser.name",a="_applySelectionMode";qx.Class.define(c,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__uO=[];this.__uP=-1;this.__uQ=-1;this.hasBatchModeRefCount=0;this.__uR=false;}
,events:{"changeSelection":f},statics:{NO_SELECTION:1,SINGLE_SELECTION:2,SINGLE_INTERVAL_SELECTION:3,MULTIPLE_INTERVAL_SELECTION:4,MULTIPLE_INTERVAL_SELECTION_TOGGLE:5},properties:{selectionMode:{init:2,check:[1,2,3,4,5],apply:a}},members:{__uR:null,__uP:null,__uQ:null,__uO:null,_applySelectionMode:function(o){this.resetSelection();}
,setBatchMode:function(p){if(p){this.hasBatchModeRefCount+=1;}
else {if(this.hasBatchModeRefCount==0){throw new Error("Try to turn off batch mode althoug it was not turned on.");}
;this.hasBatchModeRefCount-=1;if(this.__uR){this.__uR=false;this._fireChangeSelection();}
;}
;return this.hasBatchMode();}
,hasBatchMode:function(){return this.hasBatchModeRefCount>0;}
,getAnchorSelectionIndex:function(){return this.__uP;}
,_setAnchorSelectionIndex:function(q){this.__uP=q;}
,getLeadSelectionIndex:function(){return this.__uQ;}
,_setLeadSelectionIndex:function(r){this.__uQ=r;}
,_getSelectedRangeArr:function(){return this.__uO;}
,resetSelection:function(){if(!this.isSelectionEmpty()){this._resetSelection();this._fireChangeSelection();}
;}
,isSelectionEmpty:function(){return this.__uO.length==0;}
,getSelectedCount:function(){var t=0;for(var i=0;i<this.__uO.length;i++ ){var s=this.__uO[i];t+=s.maxIndex-s.minIndex+1;}
;return t;}
,isSelectedIndex:function(u){for(var i=0;i<this.__uO.length;i++ ){var v=this.__uO[i];if(u>=v.minIndex&&u<=v.maxIndex){return true;}
;}
;return false;}
,getSelectedRanges:function(){var w=[];for(var i=0;i<this.__uO.length;i++ ){w.push({minIndex:this.__uO[i].minIndex,maxIndex:this.__uO[i].maxIndex});}
;return w;}
,iterateSelection:function(x,y){for(var i=0;i<this.__uO.length;i++ ){for(var j=this.__uO[i].minIndex;j<=this.__uO[i].maxIndex;j++ ){x.call(y,j);}
;}
;}
...
,removeSelectionInterval:function(F,G){this.__uP=F;this.__uQ=G;var H=Math.min(F,G);var J=Math.max(F,G);for(var i=0;i<this.__uO.length;i++ ){var L=this.__uO[i];if(L.minIndex>J){break;}
else if(L.maxIndex>=H){var M=(L.minIndex>=H)&&(L.minIndex<=J);var K=(L.maxIndex>=H)&&(L.maxIndex<=J);if(M&&K){this.__uO.splice(i,1);i-- ;}
else if(M){L.minIndex=J+1;}
else if(K){L.maxIndex=H-1;}
else {var I={minIndex:J+1,maxIndex:L.maxIndex};this.__uO.splice(i+1,0,I);L.maxIndex=H-1;break;}
;}
;}
;this._fireChangeSelection();}
,_resetSelection:function(){this.__uO=[];this.__uP=-1;this.__uQ=-1;}
,_addSelectionInterval:function(N,O){this.__uP=N;this.__uQ=O;var P=Math.min(N,O);var R=Math.max(N,O);var Q=0;for(;Q<this.__uO.length;Q++ ){var S=this.__uO[Q];if(S.minIndex>P){break;}
;}
;this.__uO.splice(Q,0,{minIndex:P,maxIndex:R});var T=this.__uO[0];for(var i=1;i<this.__uO.length;i++ ){var S=this.__uO[i];if(T.maxIndex+1>=S.minIndex){T.maxIndex=Math.max(T.maxIndex,S.maxIndex);this.__uO.splice(i,1);i-- ;}
else {T=S;}
;}
;}
,_dumpRanges:function(){var U=d;for(var i=0;i<this.__uO.length;i++ ){var V=this.__uO[i];U+=k+V.minIndex+m+V.maxIndex+n;}
;this.debug(U);}
,_fireChangeSelection:function(){if(this.hasBatchMode()){this.__uR=true;}
...
,properties:{highlightFocusRow:{check:i,init:true}},members:{_colors:null,__ve:null,__vf:null,initThemeValues:function(){this.__vf=k;this.__ve={};this._colors={};this._renderFont(qx.theme.manager.Font.getInstance().resolve(g));var x=qx.theme.manager.Color.getInstance();this._colors.bgcolFocusedSelected=x.resolve(p);this._colors.bgcolFocused=x.resolve(c);this._colors.bgcolSelected=x.resolve(t);this._colors.bgcolEven=x.resolve(j);this._colors.bgcolOdd=x.resolve(w);this._colors.colSelected=x.resolve(q);this._colors.colNormal=x.resolve(d);this._colors.horLine=x.resolve(u);}
,_insetY:1,_renderFont:function(y){if(y){this.__ve=y.getStyles();this.__vf=qx.bom.element.Style.compile(this.__ve);this.__vf=this.__vf.replace(/"/g,e);}
else {this.__vf=k;this.__ve=qx.bom.Font.getDefaultStyles();}
;}
,updateDataRowElement:function(z,A){var C=this.__ve;var B=A.style;qx.bom.element.Style.setStyles(A,C);if(z.focusedRow&&this.getHighlightFocusRow()){B.backgroundColor=z.selected?this._colors.bgcolFocusedSelected:this._colors.bgcolFocused;}
...
;return f+D+o;}
,createRowStyle:function(E){var F=[];F.push(l);F.push(this.__vf);F.push(s);if(E.focusedRow&&this.getHighlightFocusRow()){F.push(E.selected?this._colors.bgcolFocusedSelected:this._colors.bgcolFocused);}
...
},destruct:function(){this._colors=this.__ve=this.__vf=null;qx.theme.manager.Appearance.getInstance().removeListener(m,this.initThemeValues,this);}
});}
)();
...
(function(){var e="__vh",d="qx.ui.table.pane.Header",c="hovered",b="first",a="last";qx.Class.define(d,{extend:qx.ui.core.Widget,construct:function(f){qx.ui.core.Widget.call(this);this._setLayout(new qx.ui.layout.HBox());this.__lk=new qx.ui.core.Blocker(this);this.__vh=f;}
,members:{__vh:null,__vp:null,__vq:null,__lk:null,getPaneScroller:function(){return this.__vh;}
...
;}
,setMouseOverColumn:function(k){if(k!=this.__vq){if(this.__vq!=null){var l=this.getHeaderWidgetAtColumn(this.__vq);if(l!=null){l.removeState(c);}
;}
...
,showColumnMoveFeedback:function(o,x){var s=this.getContainerLocation();if(this.__vp==null){var y=this.getTable();var p=this.getPaneScroller().getTablePaneModel().getX(o);var r=this._getChildren()[p];var t=y.getTableModel();var v=y.getTableColumnModel();var w={xPos:p,col:o,name:t.getColumnName(o),table:y};var u=v.getHeaderCellRenderer(o);var q=u.createHeaderCell(w);var z=r.getBounds();q.setWidth(z.width);q.setHeight(z.height);q.setZIndex(1000000);q.setOpacity(0.8);q.setLayoutProperties({top:s.top});this.getApplicationRoot().add(q);this.__vp=q;}
;this.__vp.setLayoutProperties({left:s.left+x});this.__vp.show();}
,hideColumnMoveFeedback:function(){if(this.__vp!=null){this.__vp.destroy();this.__vp=null;}
;}
,isShowingColumnMoveFeedback:function(){return this.__vp!=null;}
...
;}
},destruct:function(){this.__lk.dispose();this._disposeObjects(e);}
});}
)();
...
(function(){var k='<div>',j="</div>",h="overflow: hidden;",g="qx.event.type.Data",f="paneReloadsData",e="div",d='style="',c="_applyMaxCacheLines",b="qx.ui.table.pane.Pane",a="width: 100%;",E="qx.event.type.Event",D="_applyVisibleRowCount",C='>',B="line-height: ",A="appear",z='class="',w="width:100%;",v="px;",u='<div ',t="'>",r="_applyFirstVisibleRow",s="<div style='",p=";position:relative;",q='</div>',n='" ',o="paneUpdated",l="Number",m="";qx.Class.define(b,{extend:qx.ui.core.Widget,construct:function(F){qx.ui.core.Widget.call(this);this.__vh=F;this.__vi=0;this.__uY=0;this.__vj=[];}
,events:{"paneReloadsData":g,"paneUpdated":E},properties:{firstVisibleRow:{check:l,init:0,apply:r},visibleRowCount:{check:l,init:0,apply:D},maxCacheLines:{check:l,init:1000,apply:c},allowShrinkX:{refine:true,init:false}},members:{__uY:null,__vi:null,__vh:null,__vk:null,__uV:null,__uU:null,__vj:null,__vl:0,_applyFirstVisibleRow:function(G,H){this.updateContent(false,G-H);}
,_applyVisibleRowCount:function(I,J){this.updateContent(true);}
,_getContentHint:function(){return {width:this.getPaneScroller().getTablePaneModel().getTotalWidth(),height:400};}
,getPaneScroller:function(){return this.__vh;}
,getTable:function(){return this.__vh.getTable();}
,setFocusedCell:function(K,L,M){if(K!=this.__uU||L!=this.__uV){var N=this.__uV;this.__uU=K;this.__uV=L;if(L!=N&&!M){if(N!==null){this.updateContent(false,null,N,true);}
...
,onTableModelDataChanged:function(Q,R,S,T){this.__vm();var V=this.getFirstVisibleRow();var U=this.getVisibleRowCount();if(R==-1||R>=V&&Q<V+U){this.updateContent();}
;}
,onTableModelMetaDataChanged:function(){this.updateContent(true);}
,_applyMaxCacheLines:function(W,X){if(this.__vl>=W&&W!==-1){this.__vm();}
;}
,__vm:function(){this.__vj=[];this.__vl=0;}
,__vn:function(Y,ba,bb){if(!ba&&!bb&&this.__vj[Y]){return this.__vj[Y];}
else {return null;}
;}
,__vo:function(bc,bd,be,bf){var bg=this.getMaxCacheLines();if(!be&&!bf&&!this.__vj[bc]&&bg>0){this._applyMaxCacheLines(bg);this.__vj[bc]=bd;this.__vl+=1;}
;}
,updateContent:function(bh,bi,bj,bk){if(bh){this.__vm();}
...
;by.push(q);var bQ=by.join(m);this.__vo(bE,bQ,bH,bK);bX.push(bQ);}
...
;}
;if(!this.__vk){this.__vk=document.createElement(e);}
;var ck=k;ck+=this._getRowsHtml(ci+ce,Math.abs(cb));ck+=q;this.__vk.innerHTML=ck;var cf=this.__vk.firstChild.childNodes;if(cb>0){for(var i=cf.length-1;i>=0;i-- ){var ch=cf[0];cl.appendChild(ch);}
;}
else {for(var i=cf.length-1;i>=0;i-- ){var ch=cf[cf.length-1];cl.insertBefore(ch,cl.firstChild);}
;}
;if(this.__uV!==null){this._updateRowStyles(this.__uV-cb);this._updateRowStyles(this.__uV);}
;this.fireEvent(o);}
...
;var cA=cu.join(m);cs.innerHTML=cA;this.setWidth(cr);this.__vi=cw;this.__uY=cq;this.fireEvent(o);}
},destruct:function(){this.__vk=this.__vh=this.__vj=null;}
});}
)();
...
;qx.ui.table.cellrenderer.Abstract.__tS.stylesheet=qx.bom.Stylesheet.createElement(M);}
...
(function(){var t='(',s="g",r="Boolean",q="$",p="NaN",o='([0-9]{1,3}(?:',n='{0,1}[0-9]{3}){0,})',m='\\d+){0,1}',l="qx.util.format.NumberFormat",k="Infinity",d="^",j=".",g="-Infinity",c='([-+]){0,1}',b="0",f="String",e="changeNumberFormat",h="-",a="Number",i="";qx.Class.define(l,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(u){qx.core.Object.call(this);this.__jH=u;}
,properties:{minimumIntegerDigits:{check:a,init:0},maximumIntegerDigits:{check:a,nullable:true},minimumFractionDigits:{check:a,init:0},maximumFractionDigits:{check:a,nullable:true},groupingUsed:{check:r,init:true},prefix:{check:f,init:i,event:e},postfix:{check:f,init:i,event:e}},members:{__jH:null,format:function(v){switch(v){case Infinity:return k;case -Infinity:return g;case NaN:return p;};var z=(v<0);if(z){v=-v;}
...
;if(this.getGroupingUsed()){var y=C;C=i;var E;for(E=y.length;E>3;E-=3){C=i+qx.locale.Number.getGroupSeparator(this.__jH)+y.substring(E-3,E)+C;}
;C=y.substring(0,E)+C;}
;var A=this.getPrefix()?this.getPrefix():i;var x=this.getPostfix()?this.getPostfix():i;var D=A+(z?h:i)+C;if(B.length>0){D+=i+qx.locale.Number.getDecimalSeparator(this.__jH)+B;}
;D+=x;return D;}
,parse:function(H){var M=qx.lang.String.escapeRegexpChars(qx.locale.Number.getGroupSeparator(this.__jH)+i);var K=qx.lang.String.escapeRegexpChars(qx.locale.Number.getDecimalSeparator(this.__jH)+i);var I=new RegExp(d+qx.lang.String.escapeRegexpChars(this.getPrefix())+c+o+M+n+t+K+m+qx.lang.String.escapeRegexpChars(this.getPostfix())+q);var L=I.exec(H);if(L==null){throw new Error("Number string '"+H+"' does not match the number format");}
...
(function(){var cO='W',cN="The constant 'LOGGING_DATE_TIME__format' will be removed (hasn't been used anyway!)",cM="qqqq",cL="h",cK="KK",cJ="Z",cI="GGGGG",cH="LL",cG="LOGGING_DATE_TIME__format",cF="^",bP='4th quarter',bO="SSS",bN="qqq",bM="K",bL='q',bK='G',bJ="a",bI='2nd quarter',bH="dd",bG="qx.util.format.DateFormat",cV='weekDay',cW="ee",cT="q",cU="QQQQ",cR="ww",cS="H",cP='Y+',cQ='wide',cX="(\\d\\d\\d?)",cY="(GMT[\\+\\-]\\d\\d:\\d\\d)",co="_applyLocale",cn="Q",cq="c",cp="weekOfMonth",cs="GGG",cr='-',cu="w",ct="mm",cm='h',cl="S",o='s',p="QQQ",q="G",r="GG",s="kk",t="ss",u="([\\+\\-]\\d\\d\\d\\d)",v='H',w='S',x='1st quarter',dn='Q1',dm='3rd quarter',dl="QQ",dk="HH",ds='k',dr="m",dq="DDD",dp='D',du='K',dt="L",bg="hh",bh="e",be="W",bf="GGGG",bk='Q2',bl="MM",bi="y+",bj='narrow',bc="yyyy-MM-dd HH:mm:ss",bd="short",O='d',N="unkown",Q="\\d?",P="k",K='m',J="String",M='Q3',L="z",I="DD",H='isoUtcDateTime',bq="SS",br="s",bs="M",bt='w',bm="$",bn="?",bo='Q4',bp='month',bu="qq",bv='L',Y='abbreviated',X="Y+",W=":",V='y',U="quoted_literal",T="weekOfYear",S='a',R="day",bb="(-*",ba="locale",bw="GMT",bx="HH:mm:ss",by='y+',bz="HHmmss",bA="long",bB='Y',bC='z',bD='BC',bE='c',bF="d",bT="D",bS='e',bR='B',bQ='',bX='Before Christ',bW='Z',bV="qx.debug",bU=" ",ca="min",bY='M',ch="sec",ci="\\d",cf="+?",cg='E',cd="eeeee",ce="EEEE",cb='Anno Domini',cc="+",cj="eee",ck="ccc",cy="EEEEE",cx="EE",cA="MMMM",cz='Q',cC="eeee",cB="LLLLL",cE="LLLL",cD="ms",cw="(\\d\\d?\\d?)",cv="cccc",dg="dayOfYear",dh='AD',di="ccccc",dj="E",dc="MMM",dd="EEE",de="MMMMM",df="LLL",da="quarter",db='A',n="default",m="0",h="literal",g="wildcard",f="'",e="(\\d?)",d="hour",c="abbreviated",b="wide",a="(\\d\\d*?)",A="narrow",B="-",y="stand-alone",z="",E="format",F="(\\d\\d?)",C="(",D="|",G=")";qx.Class.define(bG,{extend:qx.core.Object,implement:qx.util.format.IFormat,construct:function(dv,dw){qx.core.Object.call(this);if(!dw){this.__jH=qx.locale.Manager.getInstance().getLocale();this.__sU=qx.locale.Manager.getInstance().bind(ba,this,ba);}
else {this.__jH=dw;this.setLocale(dw);}
;this.__sV=this.__jH;if(dv!=null){this.__sW=dv.toString();if(this.__sW in qx.util.format.DateFormat.ISO_MASKS){if(this.__sW===H){this.__sX=true;}
;this.__sW=qx.util.format.DateFormat.ISO_MASKS[this.__sW];}
;}
else {this.__sW=qx.locale.Date.getDateFormat(bA,this.__jH)+bU+qx.locale.Date.getDateTimeFormat(bz,bx,this.__jH);}
;}
,properties:{locale:{apply:co,nullable:true,check:J}},statics:{getDateInstance:function(){var dy=qx.util.format.DateFormat;var dx=qx.locale.Date.getDateFormat(bd)+z;if(dy._dateInstance==null||dy._dateInstance.__sW!=dx){dy._dateInstance=new dy(dx);}
;return dy._dateInstance;}
,getDateTimeInstance:function(){var dA=qx.util.format.DateFormat;var dz=qx.locale.Date.getDateFormat(bA)+bU+qx.locale.Date.getDateTimeFormat(bz,bx);if(dA._dateTimeInstance==null||dA._dateTimeInstance.__sW!=dz){dA._dateTimeInstance=new dA(dz);}
;return dA._dateTimeInstance;}
,ASSUME_YEAR_2000_THRESHOLD:30,LOGGING_DATE_TIME__format:bc,ISO_MASKS:{isoDate:"yyyy-MM-dd",isoTime:"HH:mm:ss",isoDateTime:"yyyy-MM-dd'T'HH:mm:ss",isoUtcDateTime:"yyyy-MM-dd'T'HH:mm:ss'Z'"},AM_MARKER:"am",PM_MARKER:"pm"},members:{__sU:null,__jH:null,__sV:null,__sW:null,__sY:null,__ta:null,__tb:null,__sX:null,__tc:function(dB,dC){var dD=z+(dB<0?((-1)*dB):dB);while(dD.length<dC){dD=m+dD;}
;return dB<0?B+dD:dD;}
,__td:function(dE){var dF=new Date(dE.getTime());var dG=dF.getDate();while(dF.getMonth()!=0){dF.setDate(-1);dG+=dF.getDate()+1;}
;return dG;}
,__te:function(dH){return new Date(dH.getTime()+(3-((dH.getDay()+6)%7))*86400000);}
,__tf:function(dI){var dK=this.__te(dI);var dL=dK.getFullYear();var dJ=this.__te(new Date(dL,0,4));return Math.floor(1.5+(dK.getTime()-dJ.getTime())/86400000/7);}
,__tg:function(dM){var dO=this.__te(dM);var dN=this.__te(new Date(dM.getFullYear(),dM.getMonth(),4));return Math.floor(1.5+(dO.getTime()-dN.getTime())/86400000/7);}
,__th:function(dP){var dQ=this.__te(dP);return dQ.getFullYear();}
,__ti:function(dR){var dS=new Date(dR,2,1);dS.setDate(-1);return dS.getDate()+1===29;}
,__tj:function(dT,dU){var dY=0;var ea=0;if(!dU){dU=1971;}
...
,__tk:function(eb,ec,ed){var ee;switch(ec){case 11:ee=eb-1;if(eb!=this.__th(new Date(ee,ec,ed))){ee=eb;}
;break;case 0:ee=eb+1;if(eb!=this.__th(new Date(ee,ec,ed))){ee=eb;}
;break;default:ee=eb;};return ee;}
,_applyLocale:function(ef,eg){this.__jH=ef===null?this.setLocale(this.__sV):ef;}
...
;return null;}
;if(this.__sX){eh=new Date(eh.getUTCFullYear(),eh.getUTCMonth(),eh.getUTCDate(),eh.getUTCHours(),eh.getUTCMinutes(),eh.getUTCSeconds(),eh.getUTCMilliseconds());}
;var ek=this.__jH;var ep=eh.getFullYear();var el=eh.getMonth();var eo=eh.getDate();var es=eh.getDay();var ev=eh.getHours();var eB=eh.getMinutes();var et=eh.getSeconds();var ez=eh.getMilliseconds();var er=eh.getTimezoneOffset();var ew=er>0?1:-1;var ei=Math.floor(Math.abs(er)/60);var ey=Math.abs(er)%60;this.__tl();var eA=z;for(var i=0;i<this.__tb.length;i++ ){var eq=this.__tb[i];if(eq.type==h){eA+=eq.text;}
else {var ej=eq.character;var eu=eq.size;var eC=bn;switch(ej){case V:if(eu==2){eC=this.__tc(ep%100,2);}
else {var em=Math.abs(ep);eC=em+z;if(eu>eC.length){for(var j=eC.length;j<eu;j++ ){eC=m+eC;}
;}
;if(ep<0){eC=B+eC;}
;}
;break;case bB:eC=this.__th(eh)+z;var em=eC.replace(cr,bQ);if(eu>eC.length){for(var j=em.length;j<eu;j++ ){em=m+em;}
;}
;eC=eC.indexOf(B)!=-1?B+em:em;break;case bK:if(eu>=1&&eu<=3){eC=ep>0?dh:bD;}
else if(eu==4){eC=ep>0?cb:bX;}
else if(eu==5){eC=ep>0?db:bR;}
;break;case cz:if(eu==1||eu==2){eC=this.__tc(parseInt(el/4)+1,eu);}
;if(eu==3){eC=cz+(parseInt(el/4)+1);}
;break;case bL:if(eu==1||eu==2){eC=this.__tc(parseInt(el/4)+1,eu);}
;if(eu==3){eC=cz+(parseInt(el/4)+1);}
;break;case dp:eC=this.__tc(this.__td(eh),eu);break;case O:eC=this.__tc(eo,eu);break;case bt:eC=this.__tc(this.__tf(eh),eu);break;case cO:eC=this.__tg(eh);break;case cg:if(eu>=1&&eu<=3){eC=qx.locale.Date.getDayName(c,es,ek,E,true);}
else if(eu==4){eC=qx.locale.Date.getDayName(b,es,ek,E,true);}
else if(eu==5){eC=qx.locale.Date.getDayName(A,es,ek,E,true);}
;break;case bS:var en=qx.locale.Date.getWeekStart(ek);var ex=1+((es-en>=0)?(es-en):7+(es-en));if(eu>=1&&eu<=2){eC=this.__tc(ex,eu);}
...
;break;case bY:if(eu==1||eu==2){eC=this.__tc(el+1,eu);}
...
;break;case bv:if(eu==1||eu==2){eC=this.__tc(el+1,eu);}
...
;break;case S:eC=(ev<12)?qx.locale.Date.getAmMarker(ek):qx.locale.Date.getPmMarker(ek);break;case v:eC=this.__tc(ev,eu);break;case ds:eC=this.__tc((ev==0)?24:ev,eu);break;case du:eC=this.__tc(ev%12,eu);break;case cm:eC=this.__tc(((ev%12)==0)?12:(ev%12),eu);break;case K:eC=this.__tc(eB,eu);break;case o:eC=this.__tc(et,eu);break;case w:eC=this.__tc(ez,3);if(eu<eC.length){eC=eC.substr(0,eu);}
else {while(eu>eC.length){eC+=m;}
;}
;break;case bC:if(eu>=1&&eu<=4){eC=bw+((ew>0)?B:cc)+this.__tc(Math.abs(ei),2)+W+this.__tc(ey,2);}
;break;case bW:if(eu>=1&&eu<=3){eC=((ew>0)?B:cc)+this.__tc(Math.abs(ei),2)+this.__tc(ey,2);}
else {eC=bw+((ew>0)?B:cc)+this.__tc(Math.abs(ei),2)+W+this.__tc(ey,2);}
;break;};eA+=eC;}
;}
;return eA;}
,parse:function(eD){this.__tm();var eM=this.__sY.regex.exec(eD);if(eM==null){throw new Error("Date string '"+eD+"' does not match the date format: "+this.__sW);}
;var eE={era:1,year:1970,quarter:1,month:0,day:1,dayOfYear:1,hour:0,ispm:false,weekDay:4,weekYear:1970,weekOfMonth:1,weekOfYear:1,min:0,sec:0,ms:0};var eG=1;var eF=false;var eH=false;for(var i=0;i<this.__sY.usedRules.length;i++ ){var eI=this.__sY.usedRules[i];var eJ=eM[eG];if(eI.field!=null){eE[eI.field]=parseInt(eJ,10);}
else {eI.manipulator(eE,eJ,eI.pattern);}
;if(eI.pattern==X){var eK=false;for(var k=0;k<this.__sY.usedRules.length;k++ ){if(this.__sY.usedRules[k].pattern==by){eK=true;break;}
;}
;if(!eK){eF=true;}
;}
;if(eI.pattern.indexOf(bT)!=-1){var eO=false;for(var k=0;k<this.__sY.usedRules.length;k++ ){if(this.__sY.usedRules[k].pattern.indexOf(bF)!=-1){eO=true;break;}
;}
;if(!eO){eH=true;}
;}
;eG+=(eI.groups==null)?1:eI.groups;}
;if(eF){eE.year=this.__tk(eE.weekYear,eE.month,eE.day);}
;if(eH){var eN=this.__tj(eE.dayOfYear,eE.year);eE.month=eN.month;eE.day=eN.day;}
;if(eE.era<0&&(eE.year*eE.era<0)){eE.year=eE.year*eE.era;}
;var eL=new Date(eE.year,eE.month,eE.day,(eE.ispm)?(eE.hour+12):eE.hour,eE.min,eE.sec,eE.ms);if(this.__sX){eL=new Date(eL.getUTCFullYear(),eL.getUTCMonth(),eL.getUTCDate(),eL.getUTCHours(),eL.getUTCMinutes(),eL.getUTCSeconds(),eL.getUTCMilliseconds());}
...
;this.__tb=[];var eT;var eR=0;var eV=z;var eP=this.__sW;var eS=n;var i=0;while(i<eP.length){var eU=eP.charAt(i);switch(eS){case U:if(eU==f){if(i+1>=eP.length){i++ ;break;}
;var eQ=eP.charAt(i+1);if(eQ==f){eV+=eU;i++ ;}
else {i++ ;eS=N;}
;}
else {eV+=eU;i++ ;}
;break;case g:if(eU==eT){eR++ ;i++ ;}
else {this.__tb.push({type:g,character:eT,size:eR});eT=null;eR=0;eS=n;}
;break;default:if((eU>=S&&eU<=bC)||(eU>=db&&eU<=bW)){eT=eU;eS=g;}
else if(eU==f){if(i+1>=eP.length){eV+=eU;i++ ;break;}
;var eQ=eP.charAt(i+1);if(eQ==f){eV+=eU;i++ ;}
;i++ ;eS=U;}
else {eS=n;}
;if(eS!=n){if(eV.length>0){this.__tb.push({type:h,text:eV});eV=z;}
;}
else {eV+=eU;i++ ;}
;break;};}
;if(eT!=null){this.__tb.push({type:g,character:eT,size:eR});}
else if(eV.length>0){this.__tb.push({type:h,text:eV});}
;}
,__tm:function(){if(this.__sY!=null){return;}
;var fa=this.__sW;this.__to();this.__tl();var fg=[];var fc=cF;for(var eX=0;eX<this.__tb.length;eX++ ){var fh=this.__tb[eX];if(fh.type==h){fc+=qx.lang.String.escapeRegexpChars(fh.text);}
else {var eY=fh.character;var fd=fh.size;var fb;for(var fi=0;fi<this.__ta.length;fi++ ){var fe=this.__ta[fi];if(this.__tn(fe,eY,fd)){fb=fe;break;}
;}
...
;this.__sY={regex:eW,"usedRules":fg,pattern:fc};}
,__tn:function(fk,fl,fm){if(fl===V&&fk.pattern===by){fk.regex=fk.regexFunc(fm);return true;}
else if(fl===bB&&fk.pattern===cP){fk.regex=fk.regexFunc(fm);return true;}
else {return fl==fk.pattern.charAt(0)&&fm==fk.pattern.length;}
;}
,__to:function(){var fA=qx.util.format.DateFormat;var fC=qx.lang.String;if(this.__ta!=null){return;}
;var fB=this.__ta=[];var fw=qx.locale.Date.getAmMarker(this.__jH).toString()||fA.AM_MARKER;var fJ=qx.locale.Date.getPmMarker(this.__jH).toString()||fA.PM_MARKER;var fo=this.__jH;var ft=function(fN,fO){fO=parseInt(fO,10);if(fO>0){if(fO<fA.ASSUME_YEAR_2000_THRESHOLD){fO+=2000;}
...
},destruct:function(){if(this.__sU!=null){qx.locale.Manager.getInstance().removeBinding(this.__sU);}
;this.__tb=this.__sY=this.__ta=null;}
});if(qx.core.Environment.get(bV)){qx.log.Logger.deprecatedConstantWarning(qx.util.format.DateFormat,cG,cN);}
;}
)();
(function(){var k="HH:mm",j="HHmmsszz",h="cldr_date_format_",g="HHmm",f="HHmmss",e="cldr_time_format_",d="HH:mm:ss zz",c="cldr_pm",b="cldr_am",a="qx.locale.Date",J="cldr_date_time_format_",I="HH:mm:ss",H="sat",G="cldr_day_",F="cldr_month_",E="wed",D="fri",C="tue",B="mon",A="sun",r="thu",s="short",p="full",q="long",n="medium",o='_',l="stand-alone",m="narrow",t="abbreviated",u="wide",w='stand-alone',v="qx.debug",y='format',x="format",z="_";qx.Class.define(a,{statics:{__tp:qx.locale.Manager.getInstance(),getAmMarker:function(K){return this.__tp.localize(b,[],K);}
,getPmMarker:function(L){return this.__tp.localize(c,[],L);}
...
;var Q=[A,B,C,E,r,D,H];var R=[];for(var i=0;i<Q.length;i++ ){var P=G+N+z+length+z+Q[i];R.push(O?this.__tq(N,N===y?w:y,P,M):this.__tp.localize(P,[],M));}
;return R;}
...
;var X=[A,B,C,E,r,D,H];var W=G+U+z+length+z+X[S];return V?this.__tq(U,U===y?w:y,W,T):this.__tp.localize(W,[],T);}
...
;var bd=[];for(var i=0;i<12;i++ ){var bc=F+ba+z+length+z+(i+1);bd.push(bb?this.__tq(ba,ba===y?w:y,bc,Y):this.__tp.localize(bc,[],Y));}
;return bd;}
...
;var bi=F+bg+z+length+z+(be+1);return bh?this.__tq(bg,bg===y?w:y,bi,bf):this.__tp.localize(bi,[],bf);}
,getDateFormat:function(bj,bk){if(qx.core.Environment.get(v)){qx.core.Assert.assertInArray(bj,[s,n,q,p]);}
;var bl=h+bj;return this.__tp.localize(bl,[],bk);}
,getDateTimeFormat:function(bm,bn,bo){var bq=J+bm;var bp=this.__tp.localize(bq,[],bo);if(bp==bq){bp=bn;}
;return bp;}
,getTimeFormat:function(br,bs){if(qx.core.Environment.get(v)){qx.core.Assert.assertInArray(br,[s,n,q,p]);}
;var bu=e+br;var bt=this.__tp.localize(bu,[],bs);if(bt!=bu){return bt;}
...
else {bJ=this.__tp.getTerritory()||this.__tp.getLanguage();}
;return bJ.toUpperCase();}
,__tq:function(bK,bL,bM,bN){var bP=this.__tp.localize(bM,[],bN);if(bP==bM){var bO=bM.replace(o+bK+o,o+bL+o);return this.__tp.localize(bO,[],bN);}
else {return bP;}
;}
}});}
)();
...
(function(){var h="demobrowser.demo.test.Table_Header",g="Complete update",f="mouseover",d="changeValue",c="Hide move feedback",b="execute",a="Show move feedback";qx.Class.define(h,{extend:qx.application.Standalone,include:[demobrowser.demo.table.MUtil],members:{main:function(){qx.application.Standalone.prototype.main.call(this);var i=new qx.ui.table.pane.Header(this.getScrollerMock());this.getRoot().add(i,{left:10,top:10});i.onPaneModelChanged();i.setColumnWidth(0,150);i.addListener(f,function(e){var l=e.getTarget();if(l instanceof qx.ui.table.headerrenderer.HeaderCell){var m=i._indexOf(l);i.setMouseOverColumn(m);}
;}
);var k=new qx.ui.form.ToggleButton(a);this.getRoot().add(k,{left:20,top:100});k.addListener(d,function(e){var n=e.getData();if(n){k.setLabel(c);i.showColumnMoveFeedback(1,50);}
else {k.setLabel(a);i.hideColumnMoveFeedback();}
;}
);var j=new qx.ui.form.Button(g);this.getRoot().add(j,{left:20,top:140});j.addListener(b,function(){i.onPaneModelChanged();}
);}
}});}
)();
