...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Decorator_Box.8e3dfd476977.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var s="sliceBottom",r="horizontal",q="qx.debug.dispose",p="qx.ui.decoration.AbstractBox",o="-l",n="qx.debug",m="sliceLeft",l="_applyBaseImage",k="-b",j="sliceRight",d="_applyFill",i="shorthand",g="-t",c="-r",b="String",f="sliceTop",e="set",h="Number",a="_applySlices";qx.Class.define(p,{extend:qx.ui.decoration.Abstract,implement:[qx.ui.decoration.IDecorator],construct:function(t,u,v){qx.ui.decoration.Abstract.call(this);this._setOrientation(v);if(qx.ui.decoration.css3.BorderImage.IS_SUPPORTED){this.__zE=new qx.ui.decoration.css3.BorderImage();if(t){this.__Dx(t,v);}
;if(u!=null){this.__zE.setInsets(u);}
;}
else {this.__zE=new qx.ui.decoration.BoxDiv(t,u,v);}
;if(qx.core.Environment.get(q)){this.__zE.$$ignoreDisposeWarning=true;}
;}
,properties:{baseImage:{check:b,nullable:true,apply:l},sliceLeft:{check:h,nullable:true,apply:a},sliceRight:{check:h,nullable:true,apply:a},sliceBottom:{check:h,nullable:true,apply:a},sliceTop:{check:h,nullable:true,apply:a},slices:{group:[f,j,s,m],mode:i},fill:{apply:d}},members:{__zE:null,_isHorizontal:null,_setOrientation:function(w){this._isHorizontal=w==r;}
,getMarkup:function(){return this.__zE.getMarkup();}
,resize:function(x,y,z){this.__zE.resize(x,y,z);}
,tint:function(A,B){}
,getInsets:function(){return this.__zE.getInsets();}
,_applyInsets:function(C,D,name){var E=e+qx.lang.String.firstUp(name);this.__zE[E](C);}
,_applySlices:function(F,G,name){var H=e+qx.lang.String.firstUp(name);if(this.__zE[H]){this.__zE[H](F);}
;}
,_applyFill:function(I,J,name){if(this.__zE.setFill){this.__zE.setFill(I);}
;}
,_applyBaseImage:function(K,L){if(this.__zE instanceof qx.ui.decoration.BoxDiv){this.__zE.setBaseImage(K);}
else {this.__Dx(K);}
;}
,__Dx:function(M){this.__zE.setBorderImage(M);var S=qx.util.AliasManager.getInstance().resolve(M);var T=/(.*)(\.[a-z]+)$/.exec(S);var Q=T[1];var R=T[2];var P=qx.util.ResourceManager.getInstance();if(qx.core.Environment.get(n)){if(!P.has(S)){this.warn("The border image '"+S+"' is not available!");}
;}
;if(this._isHorizontal){var V=P.getImageWidth(Q+o+R);var N=P.getImageWidth(Q+c+R);if(N&&V){this.__zE.setSlice([0,N,0,V]);}
;}
else {var O=P.getImageHeight(Q+k+R);var U=P.getImageHeight(Q+g+R);if(U&&O){this.__zE.setSlice([U,0,O,0]);}
;}
;}
},destruct:function(){this.__wq=this.__Dz=this.__DA=null;this.__zE.dispose();}
});}
)();
...
;}
,properties:{baseImage:{check:w,nullable:true,apply:c}},members:{__wq:null,__Dz:null,__DA:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};}
,_isInitialized:function(){return !!this.__wq;}
,_setOrientation:function(A){this._isHorizontal=A==q;}
,getMarkup:function(){if(this.__wq){return this.__wq;}
;var B=qx.bom.element.Decoration;var C=this.__Dz;var D=this.__DA;var E=[];E.push(e);if(this._isHorizontal){E.push(B.create(C.l,m,{top:0,left:0}));E.push(B.create(C.c,p,{top:0,left:D.left+k}));E.push(B.create(C.r,m,{top:0,right:0}));}
...
;E.push(i);return this.__wq=E.join(v);}
,resize:function(F,G,H){F.style.width=G+k;F.style.height=H+k;var I=this.__DA;if(this._isHorizontal){var innerWidth=G-I.left-I.right;innerWidth=innerWidth<0?0:innerWidth;F.childNodes[1].style.width=innerWidth+k;}
...
;}
,tint:function(J,K){}
,_applyBaseImage:function(L,M){if(qx.core.Environment.get(d)){if(this.__wq){throw new Error("This decorator is already in-use. Modification is not possible anymore!");}
;}
;var N=qx.util.ResourceManager.getInstance();if(L){var P=qx.util.AliasManager.getInstance();var R=P.resolve(L);var S=/(.*)(\.[a-z]+)$/.exec(R);var Q=S[1];var O=S[2];var T=this.__Dz={t:Q+h+O,b:Q+b+O,c:Q+t+O,l:Q+j+O,r:Q+f+O};this.__DA={top:N.getImageHeight(T.t),bottom:N.getImageHeight(T.b),left:N.getImageWidth(T.l),right:N.getImageWidth(T.r)};}
;}
},destruct:function(){this.__wq=this.__Dz=this.__DA=null;}
});}
)();
