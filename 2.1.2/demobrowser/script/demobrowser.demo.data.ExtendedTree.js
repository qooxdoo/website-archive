...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.ExtendedTree","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.ExtendedTree.9105af41132c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var t="The tree has bound its label to the name of the modeln and its ",s="name",r="Write data to log",q="demobrowser.demo.data.ExtendedTree",p="textColor to some random colors.",o="gray",n="red",m="<b>A tree configures with bindItem</b><br/>",l="all boring black",k="textColor",d="label",j="green",g="children",c="yellow",b="Item ",f="color",e="blue",h="black",a="execute";qx.Class.define(q,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var x=[n,e,j,h,o,c];var z=[];for(var i=0;i<50;i++ ){z[i]=new demobrowser.demo.data.model.Node();z[i].setName(b+i);z[i].setColor(x[i%6]);if(i!=0){z[parseInt(Math.random()*i)].getChildren().push(z[i]);}
;}
;var w=new qx.ui.tree.Tree();this.getRoot().add(w,{left:10,top:80});w.setWidth(250);w.setHeight(300);var A=new qx.data.controller.Tree(z[0],w,g);A.setDelegate({bindItem:function(B,C,D){B.bindProperty(s,d,null,C,D);B.bindProperty(f,k,null,C,D);}
});w.getRoot().setOpen(true);var v=new qx.ui.form.Button(l);v.setWidth(120);this.getRoot().add(v,{left:280,top:80});v.addListener(a,function(){for(var i=0;i<z.length;i++ ){z[i].setColor(h);}
;}
,this);var y=new qx.ui.form.Button(r);y.setWidth(120);this.getRoot().add(y,{left:280,top:115});y.addListener(a,function(){this.info(z[0].toString());}
,this);var u=new qx.ui.basic.Label();u.setRich(true);u.setWidth(300);u.setValue(m+t+p);this.getRoot().add(u,{left:10,top:10});}
}});}
)();
