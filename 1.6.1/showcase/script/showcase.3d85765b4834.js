...
this.__wC();
},__wB:function(){this.getView().addListener(d,this._onButtonPress,this);
},_onButtonPress:function(e){this.getModel().readToken(e.getData());
},__wC:function(){var w=this.getModel();
...
this.__wA(s);
return t;
},__wA:function(u){if(!(g in document.documentElement.style)){return;
}var w=true;
...
this.initState();
},properties:{state:{check:[j,h,g],event:r,init:j,apply:q},errorMessage:{check:D,init:o,event:k},input:{check:D,nullable:true,event:y},maxInputLength:{check:x,init:20},operator:{check:[b,f,C,a],nullable:true,event:m},operant:{check:e,nullable:true,event:t},value:{check:e,nullable:true,event:z},memory:{check:e,nullable:true,event:s}},members:{readToken:function(E){if(E.match(/^[0123456789]$/)){this.__wF(E);
}else if(E.match(/^[\+\-\*\/]$/)){this.__wI(E);
}else if(E==v){this.__wG();
}else if(E==d){this.__wH();
}else if(E==l){this.__wJ();
}else if(E==B){this.__wK();
}else if(E==c){this.__wL(E);
}else if(E==n){this.__wL(E);
}else if(E==w){this.__wN();
}else if(E==p){this.__wM();
}},__wD:function(){return parseFloat(this.getInput());
},__wE:function(F,G,H){switch(H){case b:return F+G;
case f:return F-G;
case C:return F*G;
...
}},__wF:function(K){this.setState(j);
var L=this.getInput();
...
},__wI:function(Q){var S=this.getState();
...
if(T!==null){this.setValue(this.__wE(T,R,this.getOperator()));
}else{this.setValue(R);
}this.setOperant(R);
this.setOperator(Q);
},__wJ:function(){var U=this.getOperator();

if(!U){return;
}var W=this.getValue();

if(this.getState()==h){this.setValue(this.__wE(W,this.getOperant(),U));
return;
}this.setState(h);
var V=this.__wD();
this.setOperant(V);
this.setValue(this.__wE(W,V,U));
},__wK:function(){this.setState(j);
this.setOperator(null);
this.setValue(null);
this.setInput(i);
},__wL:function(X){var Y=this.getState();
var ba;
...
}},__wM:function(){var bc=this.getMemory();

if(bc==null){return;
}this.setState(j);
this.setInput(bc.toString());
},__wN:function(){this.setMemory(null);
}}});
})();
