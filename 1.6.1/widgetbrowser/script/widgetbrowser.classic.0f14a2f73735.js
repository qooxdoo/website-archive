...
this.__mS=null;
},properties:{filter:{check:h,nullable:true,apply:d}},members:{__Hi:null,__mS:null,__Hj:null,__Hk:null,_rowArr:null,_nodeArr:null,_nodeRowMap:null,_treeColumn:null,_selections:null,setTree:function(o){this.__Hi=o;
},getTree:function(){return this.__Hi;
},setEditable:function(p){this.__mS=[];

for(var q=0;q<this.getColumnCount();q++){this.__mS[q]=p;
}this.fireEvent(l);
},setColumnEditable:function(r,s){if(s!=this.isColumnEditable(r)){if(this.__mS==null){this.__mS=[];
}this.__mS[r]=s;
this.fireEvent(l);
}},isColumnEditable:function(t){if(t==this._treeColumn){return false;
}return (this.__mS?this.__mS[t]==true:false);
...
},__Hl:function(bd,be){var bh=this.getFilter();
var bk=null;
var bj;
...
bk=this._nodeArr[bj];
if(bk==null||(bh&&!bh.call(this,bk))){this.__Hk=true;
continue;
}bl.push(bk);
bk.level=be;
bk.bFirstChild=(bf==0);
if(!this.__Hk){this.__Hn(bk,i==bg-1);
...
}if(bk.bOpened){this.__Hl(bj,be+1);
}bf++;
}},__Hm:function(bm){var bn=this.__Hj;
var br=bn[bm];
var bp=br.length;
...
if(bo){this.__Hm(bq.nodeId);
}}},__Hn:function(bs,bt){bs.lastChild=[bt];
...
}},__DA:function(){this.__Hj=[];
this.__Hk=false;
this._rowArr=[];
this._nodeRowMap=[];
this._selections={};
this.__Hl(0,1);
if(this.__Hk){this.__Hm(0);
}this.__Hj=null;
...
}},destruct:function(){this._rowArr=this._nodeArr=this._nodeRowMap=this._selections=this.__Hi=this.__Hj=null;
...
(function(){var j="visible",h="excluded",g="qx.ui.tree.core.AbstractTreeItem",f="open",e="abstract",d="addItem",c="__dS",b="removeItem",a="__lC";
...
this.__dS=[];
},properties:{parent:{check:g,nullable:true}},members:{__dS:null,__lC:null,getTree:function(){var n=this;
...
}}},getChildrenContainer:function(){if(!this.__lC){this.__lC=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?j:h});
}return this.__lC;
...
}},addAt:function(O,P){if(P==this.__dS.length){this.add(O);
return;
}var T=O.getParent();
...
}qx.lang.Array.insertAt(this.__dS,O,P);
...
if(W){W.remove(U);
}this.addAt(U,this.__dS.indexOf(V));
},addAfter:function(X,Y){var ba=X.getParent();

if(ba){ba.remove(X);
}this.addAt(X,this.__dS.indexOf(Y)+1);
},addAtBegin:function(bb){this.addAt(bb,0);
},remove:function(bc){for(var i=0,l=arguments.length;i<l;i++){var bg=arguments[i];

if(this.__dS.indexOf(bg)==-1){this.warn("Cannot remove treeitem '"+bg+"'. It is not a child of this tree item.");
return;
...
}}qx.lang.Array.remove(this.__dS,bg);
bg.setParent(null);
bd.remove(bg);
}var be=this.getTree();
...
},removeAt:function(bh){var bi=this.__dS[bh];

if(bi){this.remove(bi);
}},removeAll:function(){var bj=this.__dS.concat();
...
},getLookupTable:function(){return this.__yh;
},_reverseLookup:function(bb){return bb;
},_getDataFromRow:function(bc){return this.__yh.getItem(bc);
},_getSelectables:function(){return this.__yh;
},getOpenNodes:function(){return this.__Ia;
},isNode:function(bd){return qx.Class.hasProperty(bd.constructor,this.getChildProperty());
},getLevel:function(be){return this.__Ib[be];
...
},_applyModel:function(bD,bE){this.__Ia=[];
...
if(qx.lang.String.startsWith(bI,this.getChildProperty())){this.__Ie();
...
}}},_beforeApplySelection:function(bP){if(bP.length===0&&this.getSelectionMode()===d){var bQ=this.__Ij();
var bR=this.getLookupTable().indexOf(bQ);

if(bR>=0){bP.push(bR);
}}},_afterApplySelection:function(){var bS=this.getSelection();

if(bS.getLength()>0&&this.getSelectionMode()===d){this.__Ii(bS.getItem(0));
}else{this.__Id=[];
}},__Ie:function(){this.buildLookupTable();
...
if(this.isNodeOpen(bV)){var bT=this.__If(bV,bU);
bW=bW.concat(bT);
...
if(this.isNode(cd)){this.__Ib.push(bY);
ca.push(cd);

if(this.isNodeOpen(cd)){var cb=this.__If(cd,bY);
ca=ca.concat(cb);
}}else{if(this.isShowLeafs()){this.__Ib.push(bY);
ca.push(cd);
}}}return ca;
},__Ig:function(ce){if(!qx.lang.Array.contains(this.__Ia,ce)){this.__Ia.push(ce);
this.fireDataEvent(s,ce);
}},__Ih:function(cf,cg){if(cf===cg){this.__Ig(cg);
return true;
}
...
var ch=this.__Ih(cj,cg);

if(ch===true){this.__Ig(cj);
return true;
}}return false;
},__yn:function(){this.getPane().getRowConfig().setItemCount(this.__yh.getLength());
this.getPane().fullUpdate();
},getParent:function(ck){var cm=this.__yh.indexOf(ck);
...
if(cl<cn){return this.__yh.getItem(cm);
}}return null;
},__Ii:function(co){this.__Id=[];
var parent=this.getParent(co);

while(parent!=null){this.__Id.unshift(parent);
parent=this.getParent(parent);
}},__Ij:function(){if(this.__Id==null){return this.getModel();
}var cp=this.getLookupTable();
var parent=this.__Id.pop();

while(parent!=null){if(cp.contains(parent)){return parent;
}parent=this.__Id.pop();
...
this.__yh.dispose();
this._layer=this._provider=this.__yh=this.__Ia=null;
}});
})();
...
qx.Mixin.define(d,{construct:function(){this.__yt=[];
},properties:{labelPath:{check:q,nullable:true},iconPath:{check:q,nullable:true},labelOptions:{nullable:true},iconOptions:{nullable:true},childProperty:{check:q,nullable:true},delegate:{event:m,init:null,nullable:true}},members:{__yt:null,bindDefaultProperties:function(r,s){this.bindProperty(o,n,null,r,s);
...
}},bindProperty:function(u,v,w,x,y){var z=this.__yu(y,u);
var A=this._tree.getLookupTable();
var B=A.bind(z,x,v,w);
this.__yv(x,B);
},bindPropertyReverse:function(C,D,E,F,G){var H=this.__yu(G,C);
var I=this._tree.getLookupTable();
var J=F.bind(D,I,H,E);
this.__yv(F,J);
},removeBindings:function(){while(this.__yt.length>0){var K=this.__yt.pop();
...
}},_removeBindingsFrom:function(O){var P=this.__yw(O);
...
if(qx.lang.Array.contains(this.__yt,O)){qx.lang.Array.remove(this.__yt,O);
}},__yu:function(R,S){var T=b+R+g;

if(S!=null&&S!=o){T+=a+S;
}return T;
},__yv:function(U,V){var W=this.__yw(U);

if(!qx.lang.Array.contains(W,V)){W.push(V);
}
if(!qx.lang.Array.contains(this.__yt,U)){this.__yt.push(U);
}},__yw:function(X){var Y=X.getUserData(p);
...
q.setUserData(f,r);
q.addListener(l,this.__Ik,this);
...
return q;
},poolCellWidget:function(t){t.removeListener(l,this.__Ik,this);
...
},__Ik:function(event){var J=event.getTarget();
...
this.__nD=false;
this.__nE=false;
},properties:{behavior:{check:j,init:null,nullable:true,apply:h,event:b}},members:{__nE:null,__nD:null,__mi:null,_applyBehavior:function(p,q){if(q!=null){q.dispose();
q=null;
...
},_onappear:function(event){if(this.__nD){return ;
}this.__nD=true;
this.getBehavior().onAppear(event,event.getType()!==n);
this.__mi._updateScrollerWidths();
this.__mi._updateScrollBarVisibility();
this.__nD=false;
this.__nE=true;
},_onTableWidthChanged:function(event){if(this.__nD||!this.__nE){return ;
}this.__nD=true;
this.getBehavior().onTableWidthChanged(event);
this.__nD=false;
},_onverticalscrollbarchanged:function(event){if(this.__nD||!this.__nE){return ;
}this.__nD=true;
this.getBehavior().onVerticalScrollBarChanged(event);
qx.event.Timer.once(function(){if(this.__mi&&!this.__mi.isDisposed()){this.__mi._updateScrollerWidths();
this.__mi._updateScrollBarVisibility();
}},this,0);
this.__nD=false;
},_oncolumnwidthchanged:function(event){if(this.__nD||!this.__nE){return ;
}this.__nD=true;
this.getBehavior().onColumnWidthChanged(event);
this.__nD=false;
},_onvisibilitychanged:function(event){if(this.__nD||!this.__nE){return ;
}this.__nD=true;
...
this.__qk=qx.util.AliasManager.getInstance();
this.__Hp=qx.util.ResourceManager.getInstance();
this.__Hq=qx.theme.manager.Appearance.getInstance();
this.BLANK=this.__Hp.toUri(this.__qk.resolve("static/blank.gif"));
},statics:{__Dt:{},__Ho:true,__Hr:function(){qx.theme.manager.Meta.getInstance().initialize();
...
bb.__Dt.line=bd.styleFrom(G);
bg(bb.__Dt.line.icon);
bb.__Dt.contract=bd.styleFrom(L);
bg(bb.__Dt.contract.icon);
bb.__Dt.expand=bd.styleFrom(C);
bg(bb.__Dt.expand.icon);
bb.__Dt.onlyContract=bd.styleFrom(H);
bg(bb.__Dt.onlyContract.icon);
bb.__Dt.onlyExpand=bd.styleFrom(S);
bg(bb.__Dt.onlyExpand.icon);
bb.__Dt.startContract=bd.styleFrom(W);
bg(bb.__Dt.startContract.icon);
bb.__Dt.startExpand=bd.styleFrom(N);
bg(bb.__Dt.startExpand.icon);
bb.__Dt.endContract=bd.styleFrom(v);
bg(bb.__Dt.endContract.icon);
bb.__Dt.endExpand=bd.styleFrom(B);
bg(bb.__Dt.endExpand.icon);
bb.__Dt.crossContract=bd.styleFrom(x);
bg(bb.__Dt.crossContract.icon);
bb.__Dt.crossExpand=bd.styleFrom(E);
bg(bb.__Dt.crossExpand.icon);
bb.__Dt.end=bd.styleFrom(p);
bg(bb.__Dt.end.icon);
bb.__Dt.cross=bd.styleFrom(q);
bg(bb.__Dt.cross.icon);
}},properties:{useTreeLines:{check:j,init:true},excludeFirstLevelTreeLines:{check:j,init:false},alwaysShowOpenCloseSymbol:{check:j,init:false}},members:{__qk:null,__Hq:null,__Hp:null,_getCellStyle:function(bh){var bi=bh.value;
...
var br=this.__Hp.toUri(this.__qk.resolve(bq.url));
...
if(!bI){if(bG.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.LEAF){var o=this.__Hq.styleFrom(U);
}else{var bJ={opened:bG.bOpened};
var o=this.__Hq.styleFrom(z,bJ);
}bI=o.icon;
...
if(bW==0&&cb){bY=false;
}if(bW<bX.level-1){return (bY&&!bX.lastChild[bW]?cc.__Dt.line:{icon:this.BLANK});
}var cd=bX.lastChild[bX.lastChild.length-1];
if(bX.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH&&!bX.bHideOpenClose){if(bX.children.length>0||ca){if(!bY){return (bX.bOpened?cc.__Dt.contract:cc.__Dt.expand);
}if(bW==0&&bX.bFirstChild){if(cd){return (bX.bOpened?cc.__Dt.onlyContract:cc.__Dt.onlyExpand);
}else{return (bX.bOpened?cc.__Dt.startContract:cc.__Dt.startExpand);
}}if(cd){return (bX.bOpened?cc.__Dt.endContract:cc.__Dt.endExpand);
}return (bX.bOpened?cc.__Dt.crossContract:cc.__Dt.crossExpand);
}}if(bY){if(bX.parentNodeId==0){if(cd&&bX.bFirstChild){return {icon:this.BLANK};
}if(cd){return cc.__Dt.end;
}if(bX.bFirstChild&&bX.type==qx.ui.treevirtual.SimpleTreeDataModel.Type.BRANCH){return (bX.bOpened?cc.__Dt.startContract:cc.__Dt.startExpand);
}}return (cd?cc.__Dt.end:cc.__Dt.cross);
}return {icon:this.BLANK};
}},destruct:function(){this.__qk=this.__Hp=this.__Hq=null;
}});
})();
...
this.__mi=d;
},members:{__mi:null,getTable:function(){return this.__mi;
...
},members:{__nF:null,renderLayout:function(f,top,g,h){this.__nF=g;
},getComputedWidth:function(){return this.__nF;
...
(function(){var j="Function",h="Boolean",g="minWidth",f="width",e="qx.ui.table.columnmodel.Resize",d="qx.ui.table.columnmodel.resizebehavior.Default",c="__nI",b="maxWidth",a="__nH";
...
this.__nG=[];
this.__nH=new qx.ui.layout.HBox();
this.__nH.connectToWidget(this);
this.__nI=new qx.util.DeferredCall(this._computeColumnsFlexWidth,this);
},properties:{newResizeBehaviorColumnData:{check:j,init:function(k){return new qx.ui.core.ColumnData();
}},initializeWidthsOnEveryAppear:{check:h,init:false},tableColumnModel:{check:e}},members:{__nH:null,__nJ:null,__nG:null,__nI:null,__nK:false,setWidth:function(m,n,o){if(m>=this.__nG.length){throw new Error("Column number out of range");
}this.__nG[m].setColumnWidth(n,o);
this.__nI.schedule();
},setMinWidth:function(p,q){if(p>=this.__nG.length){throw new Error("Column number out of range");
}this.__nG[p].setMinWidth(q);
this.__nI.schedule();
},setMaxWidth:function(r,s){if(r>=this.__nG.length){throw new Error("Column number out of range");
}this.__nG[r].setMaxWidth(s);
this.__nI.schedule();
...
}}},onAppear:function(event,w){if(w===true||!this.__nK||this.getInitializeWidthsOnEveryAppear()){this._computeColumnsFlexWidth();
this.__nK=true;
...
},_setNumColumns:function(y){var z=this.__nG;
if(y<=z.length){z.splice(y,z.length);
return;
}for(var i=z.length;i<y;i++){z[i]=this.getNewResizeBehaviorColumnData()();
z[i].columnNumber=i;
}},getLayoutChildren:function(){return this.__nJ;
},_computeColumnsFlexWidth:function(){this.__nI.cancel();
...
this.__nL();
this.__nH.renderLayout(E,100);
for(i=0,l=G.length;i<l;i++){var F=G[i].getComputedWidth();
A.setColumnWidth(C[i],F);
}},__nL:function(){this.__nH.invalidateChildrenCache();
var H=this.__nJ;
...
}},_getResizeColumnData:function(){return this.__nG;
}},destruct:function(){this.__nG=this.__nJ=null;
...
(function(){var f="Inherits from qx.ui.table.Table. Suited for multi-column trees.",e="TreeVirtual (Legacy)",d="Virtual implementation of Tree. Shares appearance.",c="widgetbrowser.pages.Tree",b="VirtualTree",a="Tree";
...
this.add(this.__lR);
this.initWidgets();
},members:{__lR:null,initWidgets:function(){var l;
var g=this._widgets;
l=new qx.ui.basic.Label(a);
this.__lR.add(l,{row:0,column:0});
var j=this.__MF();
this.__lR.add(j,{row:1,column:0});
g.push(j);
l=new qx.ui.basic.Label(b);
l.setToolTip(new qx.ui.tooltip.ToolTip(d));
this.__lR.add(l,{row:0,column:1});
var k=this.__MG();
this.__lR.add(k,{row:1,column:1});
g.push(k);
l=new qx.ui.basic.Label(e);
l.setToolTip(new qx.ui.tooltip.ToolTip(f));
this.__lR.add(l,{row:0,column:2});
var h=this.__MH();
this.__lR.add(h,{row:1,column:2});
g.push(h);
},__MF:function(){var v=new qx.ui.tree.Tree().set({width:200,height:400});
var A=new qx.ui.tree.TreeFolder("root");
A.setOpen(true);
v.setRoot(A);
var y=new qx.ui.tree.TreeFolder("Desktop");
y.setOpen(true);
A.add(y);
var q=new qx.ui.tree.TreeFolder("Files");
var o=new qx.ui.tree.TreeFolder("Workspace");
var p=new qx.ui.tree.TreeFolder("Network");
var n=new qx.ui.tree.TreeFolder("Trash");
y.add(q,o,p,n);
var w=new qx.ui.tree.TreeFile("Windows (C:)");
var m=new qx.ui.tree.TreeFile("Documents (D:)");
o.add(w,m);
var z=new qx.ui.tree.TreeFolder("Inbox");
var r=new qx.ui.tree.TreeFolder("Presets");
var u=new qx.ui.tree.TreeFolder("Sent");
var t=new qx.ui.tree.TreeFolder("Trash");

for(var i=0;i<30;i++){t.add(new qx.ui.tree.TreeFile("Junk #"+i));
}var x=new qx.ui.tree.TreeFolder("Data");
var s=new qx.ui.tree.TreeFolder("Edit");
z.add(r,u,t,x,s);
A.add(z);
return v;
},__MG:function(){var C=new qx.ui.tree.VirtualTree(null,"name","children").set({width:200,height:400});
var B="widgetbrowser/tree.json";
var D=new qx.data.store.Json(B);
D.bind("model",C,"model");
D.addListener("loaded",function(){C.openNode(C.getModel().getChildren().getItem(0));
},this);
return C;
},__MH:function(){var G=new qx.ui.treevirtual.TreeVirtual("TreeVirtual");
G.setWidth(200);
var H=G.getDataModel();
var E=H.addBranch(null,"Inbox",true,false);
var F=H.addBranch(E,"Spam",false);

for(var i=1;i<3000;i++){H.addLeaf(F,"Spam Message #"+i);
}H.addBranch(E,"Sent",true);
H.addBranch(E,"Trash",true);
H.addBranch(E,"Data",true);
H.addBranch(E,"Edit",true);
H.setData();
return G;
}}});
})();
