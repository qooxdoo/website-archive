...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.List-qx.theme.Modern.a90d81374ddd.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
qx.Class.define(v,{extend:qx.application.Standalone,properties:{showMode:{init:m,check:[p,n,m],event:bf}},members:{__PR:null,__PS:null,__PT:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var bJ=this.getRoot();
...
var bR=this.__PR=new qx.ui.list.List(ca).set({scrollbarX:"on",selectionMode:"multi",height:280,width:150,labelPath:"label",iconPath:"icon",iconOptions:{converter:function(ce){return "icon/"+ce+"/places/folder.png";
}}});
bM.add(bR,{top:20});
var bQ=this;
...
var bT=this.__PS=new qx.ui.form.CheckBox("Enable drag selection");
var bX=this.__PT=new qx.ui.form.CheckBox("Enable quick selection").set({enabled:false});
...
this.__PR.setSelectionMode(cx);

if(cx==k||cx==o){this.__PS.setEnabled(false);
this.__PT.setEnabled(true);
}else if(cx==l||cx==N){this.__PS.setEnabled(true);
this.__PT.setEnabled(false);
}},onShowModeChange:function(e){var cy=e.getData()[0].getUserData(q);
this.setShowMode(cy);
},onDragCheckChange:function(e){if(e.getData()){var cz=this.__PR.getSelectionMode();

if(cz==k||cz==o){this.debug("Drag selection is only available for the modes multi or additive");
}}this.__PR.setDragSelection(e.getData());
},onQuickCheckChange:function(e){if(e.getData()){var cA=this.__PR.getSelectionMode();

if(cA==l||cA==j){this.debug("Quick selection is only available for the modes multi or additive");
}}this.__PR.setQuickSelection(e.getData());
}}});
})();
