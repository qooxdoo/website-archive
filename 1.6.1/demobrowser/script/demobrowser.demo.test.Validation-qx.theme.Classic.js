...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Validation","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Validation-qx.theme.Classic.d5782258e75e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
R.setHtml(o);
R.setHtml(R.getHtml()+D+Q.getName()+j);
R.setHtml(R.getHtml()+N+Q.getAge()+j);
R.setHtml(R.getHtml()+y+Q.getGender()+j);
...
try{Q.setName(p);
R.setHtml(R.getHtml()+i+Q.getName()+j);
}catch(T){R.setHtml(R.getHtml()+F+P+Q.getName()+j);
...
try{Q.setAge(true);
R.setHtml(R.getHtml()+e+Q.getAge()+j);
}catch(U){R.setHtml(R.getHtml()+B+x+Q.getAge()+j);
...
try{Q.setAge(M);
R.setHtml(R.getHtml()+h+Q.getGender()+j);
}catch(V){R.setHtml(R.getHtml()+t+G+Q.getGender()+j);
}var S=new qx.ui.basic.Label();
S.setRich(true);
S.setWidth(250);
S.setValue(u+z+a+r+l);
this.getRoot().add(S,{left:280,top:10});
}}});
qx.Class.define(n,{extend:qx.core.Object,properties:{name:{init:v,validate:w},age:{init:18,validate:qx.util.Validate.number()},gender:{init:g,validate:qx.util.Validate.inArray([g,f])}},members:{__Pz:function(W){if(W.length<2){throw new qx.core.ValidationError(C+I+W+O);
}}}});
})();
