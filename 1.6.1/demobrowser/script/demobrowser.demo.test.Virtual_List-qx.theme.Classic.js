...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Virtual_List","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Virtual_List-qx.theme.Classic.181d54fe6c64.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
this.__PA=new qx.ui.virtual.layer.WidgetCell(this);
this.getPane().addLayer(this.__PA);
this.getPane().addListener(z,this._onResize,this);
this.__mJ=[];
this.__cO=[];
this.__nN=new qx.ui.virtual.selection.Row(this.getPane());
this.__nN.addListener(h,this._onChangeSelection,this);
this.__nN.attachMouseEvents();
this.__nN.attachKeyEvents(this);
this.__nN.attachListEvents(this);
...
this.fireDataEvent(h,this.__PE(e.getData()));
},getCellWidget:function(I,J){var L=this.__mJ[I];

if(!L){return null;
}var K=this.__cO.pop()||new qx.ui.form.ListItem();
K.set({label:L.getLabel(),icon:L.getIcon()});

if(this.__nN.isItemSelected(I)){K.addState(n);
}else{K.removeState(n);
}K.setUserData(g,I);
return K;
},poolCellWidget:function(M){this.__cO.push(M);
...
},hasChildren:function(){return this.__mJ.length>0;
...
qx.ui.core.queue.Widget.add(this);
},add:function(W){this.__mJ.push(W);
this._addHelper(W);
return this;
},remove:function(X){qx.lang.Array.remove(this.__mJ,X);
this._removeHelper(X);
return this;
},removeAll:function(){for(var i=0,j=this.__mJ.length;i<j;i++){this._removeHelper(this.__mJ[i]);
}this.__mJ=[];
},indexOf:function(Y){return this.__mJ.indexOf(Y);
},addAt:function(ba,bb){qx.lang.Array.insertAt(this.__mJ,ba,bb);
this._addHelper(ba);
},addBefore:function(bc,bd){qx.lang.Array.insertBefore(this.__mJ,bc,bd);
this._addHelper(bc);
},addAfter:function(be,bf){qx.lang.Array.insertAfter(this.__mJ,be,bf);
this._addHelper(be);
},removeAt:function(bg){this.__mJ.splice(bg,1);
this._removeHelper(this.__mJ[bg]);
},__PB:function(bh){var bi=this.__mJ.indexOf(bh);

if(bi<0){return null;
}else{return bi;
}},__PC:function(bj){var bk=this.__mJ[bj];
return bk;
},__PD:function(bl){var bm=[];

for(var i=0;i<bl.length;i++){bm.push(this.__PB(bl[i]));
}return bm;
},__PE:function(bn){var bo=[];

for(var i=0;i<bn.length;i++){bo.push(this.__mJ[bn[i]]);
}return bo;
},selectAll:function(){this.__nN.selectAll();
},setSelected:function(bp){this.__nN.selectItem(this.__PB(bp));
},isSelected:function(bq){return this.__nN.isItemSelected(this.__PB(bq));
},addToSelection:function(br){this.__nN.addItem(this.__PB(br));
},removeFromSelection:function(bs){this.__nN.removeItem(this.__PB(bs));
},selectRange:function(bt,bu){this.__nN.selectItemRange(this.__PB(bt),this.__PB(bu));
},resetSelection:function(){this.__nN.clearSelection();
},setSelection:function(bv){this.__nN.replaceSelection(this.__PD(bv));
},getSelectedItem:function(){return this.__mJ[this.getSelected()];
},getSelected:function(){return this.__mJ[this.__nN.getSelectedItem()];
},getSelection:function(){return this.__PE(this.__nN.getSelection());
},getSortedSelection:function(){return this.__PE(this.__nN.getSortedSelection());
},isSelectionEmpty:function(){return this.__nN.isSelectionEmpty();
},getSelectionContext:function(){return this.__nN.getSelectionContext();
},getSelectables:function(){return this.__PE(this.__nN.getSelectables());
},_applySelectionMode:function(bw,bx){this.__nN.setMode(bw);
},_applyDragSelection:function(by,bz){this.__nN.setDrag(by);
},_applyQuickSelection:function(bA,bB){this.__nN.setQuick(bA);
}}});
})();
