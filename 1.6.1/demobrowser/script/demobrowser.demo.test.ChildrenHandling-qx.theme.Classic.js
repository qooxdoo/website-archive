...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.ChildrenHandling","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.ChildrenHandling-qx.theme.Classic.c508b33d2180.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var q="Remove ",p="_gridWidgets",o="yellow",n="demobrowser.demo.test.ChildrenHandling",m="Horizontal box layout:",l="blue",k="Add ",j="_widgets",h="_hbox",g="green",c="Grid layout:",f="black",e="orange",b="_grid",a="red",d="execute";
...
v.setSpacing(10);
var s=new qx.ui.container.Composite(v).set({padding:10});
s.add(new qx.ui.basic.Label(m));
this._hbox=new qx.ui.container.Composite(new qx.ui.layout.HBox()).set({height:100});
this._widgets=[];
var w=[g,l,o,f,e,a];

for(var i=0;i<w.length;i++){var r=new qx.ui.core.Widget();
r.setBackgroundColor(w[i]);
this._hbox.add(r);
this._widgets.push(r);
}s.add(this._hbox);
s.add(new qx.ui.basic.Label(c));
var u=new qx.ui.layout.Grid();
u.setRowFlex(0,1);
this._grid=new qx.ui.container.Composite(u).set({height:100});
this._gridWidgets=[];

for(var i=0;i<w.length;i++){var r=new qx.ui.core.Widget();
r.setBackgroundColor(w[i]);
this._grid.add(r,{row:0,column:i});
this._gridWidgets.push(r);
}s.add(this._grid);
var t=new qx.ui.container.Composite(new qx.ui.layout.HBox());

for(var i=0;i<this._widgets.length;i++){t.add(this.createRemoveButton(this._widgets[i],i));
}s.add(t);
this.getRoot().add(s,{left:0,top:0});
},createRemoveButton:function(x,y){var A=true;
var z=new qx.ui.form.Button(q+x.getBackgroundColor()).set({width:100});
z.addListener(d,function(){if(A){this._hbox.remove(x);
this._grid.remove(this._gridWidgets[y]);
z.setLabel(k+x.getBackgroundColor());
}else{this._hbox.add(x);
this._grid.add(this._gridWidgets[y],{row:0,column:y});
z.setLabel(q+x.getBackgroundColor());
}A=!A;
},this);
return z;
