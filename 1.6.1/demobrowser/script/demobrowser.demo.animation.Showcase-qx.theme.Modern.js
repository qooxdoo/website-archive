...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.animation.Showcase-qx.theme.Modern.da029ad1dd59.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
this.__Hv=qx.fx.effect.core.Scale.originalStyle;
this.__Hw=qx.fx.effect.core.Scale.fontTypes;
},properties:{scaleX:{init:true,check:p},scaleY:{init:true,check:p},scaleContent:{init:true,check:p},scaleFromCenter:{init:true,check:p},scaleFrom:{init:100.0,check:o},scaleTo:{init:100,check:o},restoreAfterFinish:{init:false,check:p},alternateDimensions:{init:[],check:k}},statics:{originalStyle:{'top':null,'left':null,'width':null,'height':null,'fontSize':null},fontTypes:{'em':c,'px':q,'%':f,'pt':h}},members:{__Hx:null,__EQ:null,__EP:null,__Hy:null,__Hz:null,__HA:null,__HB:null,__Hv:null,__Hw:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
var w=this._getElement();
this.__Hx=qx.bom.element.Style.get(w,g);

for(var u in this.__Hv){this.__Hv[u]=w.style[u];
}this.__EQ=qx.bom.element.Location.getTop(w);
this.__EP=qx.bom.element.Location.getLeft(w);

try{var v=qx.bom.element.Style.get(w,n);
}catch(x){if(typeof (v)!=a){v=(qx.core.Environment.get(l)==m)?i:b;
}}
for(var s in this.__Hw){if(v.indexOf(s)>0){this.__Hy=parseFloat(v);
this.__Hz=s;
break;
}}this.__HA=(this.getScaleTo()-this.getScaleFrom())/100;
var t=this.getAlternateDimensions();

if(t.length==0){this.__HB=[w.offsetWidth,w.offsetHeight];
}else{this.__HB=t;
}},update:function(y){var A=this._getElement();
qx.fx.Base.prototype.update.call(this);
var z=(this.getScaleFrom()/100.0)+(this.__HA*y);

if(this.getScaleContent()&&this.__Hy){qx.bom.element.Style.set(A,n,this.__Hy*z+this.__Hz);
}this._setDimensions(this.__HB[0]*z,this.__HB[1]*z);
},finish:function(){qx.fx.Base.prototype.finish.call(this);
var D=this._getElement();

if(this.getRestoreAfterFinish()){for(var B in this.__Hv){var C=this.__Hv[B];
...
if(this.getScaleFromCenter()){var K=(E-this.__HB[0])/2;
var L=(F-this.__HB[1])/2;

if(this.__Hx==e){if(I){d.top=this.__EQ-L+q;
}
if(H){d.left=this.__EP-K+q;
}}else{if(I){d.top=-L+q;
}
if(H){d.left=-K+q;
}}}
for(var G in d){qx.bom.element.Style.set(J,G,d[G]);
}}},destruct:function(){this.__HB=this.__Hv=this.__Hw=null;
}});
})();
...
qx.Class.define(e,{extend:qx.fx.Base,properties:{startColor:{init:d,check:h},endColor:{init:a,check:h},restoreBackground:{init:true,check:i},keepBackgroundImage:{init:false,check:i}},members:{__HD:null,__HE:null,__HF:null,__HG:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
var j=this._getElement();
this.__HD={backgroundImage:qx.bom.element.Style.get(j,"backgroundImage"),backgroundColor:qx.bom.element.Style.get(j,"backgroundColor")};

if(!this.getKeepBackgroundImage()){qx.bom.element.Style.set(j,b,f);
}this.__HE=qx.util.ColorUtil.cssStringToRgb(this.getStartColor());
this.__HF=qx.util.ColorUtil.cssStringToRgb(this.getEndColor());
this.__HG=[this.__HF[0]-this.__HE[0],this.__HF[1]-this.__HE[1],this.__HF[2]-this.__HE[2]];
},update:function(k){qx.fx.Base.prototype.update.call(this);
var l=[this.__HE[0]+Math.round(this.__HG[0]*k),this.__HE[1]+Math.round(this.__HG[1]*k),this.__HE[2]+Math.round(this.__HG[2]*k)];
...
for(var n in this.__HD){qx.bom.element.Style.set(o,n,this.__HD[n]);
}}},destruct:function(){this.__HE=this.__HF=this.__HG=null;
}});
})();
...
(function(){var r="linear",q="Number",p="reverse",o="Boolean",n="none",m="wobble",l="flicker",k="pulse",j="sinodial",i="easeOutQuad",c="spring",h="easeInQuad",f="full",b="Color",a="#ffffff",e="__HC",d="qx.fx.effect.combination.ColorFlow",g="#ffffaa";
qx.Class.define(d,{extend:qx.fx.Base,construct:function(s){qx.fx.Base.call(this,s);
this.__HC=[new qx.fx.effect.core.Highlight(s),new qx.fx.effect.core.Highlight(s)];
},properties:{startColor:{init:a,check:b},endColor:{init:g,check:b},forwardTransition:{init:r,check:[r,h,i,j,p,l,m,k,c,n,f]},backwardTransition:{init:r,check:[r,h,i,j,p,l,m,k,c,n,f]},forwardDuration:{init:1.0,check:q},backwardDuration:{init:1.0,check:q},delayBetween:{init:0.3,check:q},restoreBackground:{init:true,check:o},keepBackgroundImage:{init:false,check:o}},members:{__HD:null,__HC:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var t=this._getElement();
this.setDuration(this.getForwardDuration()+this.getDelayBetween()+this.getBackwardDuration());
this.__HD={backgroundImage:qx.bom.element.Style.get(t,"backgroundImage"),backgroundColor:qx.bom.element.Style.get(t,"backgroundColor")};
this.__HC[0].set({startColor:this.getStartColor(),endColor:this.getEndColor(),duration:this.getForwardDuration(),transition:this.getForwardTransition(),restoreBackground:false,keepBackgroundImage:this.getKeepBackgroundImage()});
this.__HC[1].set({startColor:this.getEndColor(),endColor:this.getStartColor(),duration:this.getBackwardDuration(),transition:this.getBackwardTransition(),restoreBackground:this.getRestoreBackground(),keepBackgroundImage:this.getKeepBackgroundImage(),delay:this.getDelayBetween()});
var self=this;
this.__HC[0].afterFinishInternal=function(){self.__HC[1].start();
};
this.__HC[0].start();
}},destruct:function(){this._disposeArray(e);
}});
})();
(function(){var e="opacity",d="qx.fx.effect.combination.Pulsate",c="__Jh",b="__Jg",a="sinodial";
qx.Class.define(d,{extend:qx.fx.Base,construct:function(f){qx.fx.Base.call(this,f);
var g=this.getDuration()/6;
var h=0;
this.__Jg=[new qx.fx.effect.core.Fade(f),new qx.fx.effect.core.Fade(f),new qx.fx.effect.core.Fade(f),new qx.fx.effect.core.Fade(f),new qx.fx.effect.core.Fade(f),new qx.fx.effect.core.Fade(f)];

for(var i=0,l=this.__Jg.length;i<l;i++){this.__Jg[i].set({duration:g,to:((h%2)!=0)?1:0,from:((h%2)!=0)?0:1,transition:a,modifyDisplay:false});
h++;
}},properties:{duration:{init:2,refine:true}},members:{__mq:null,__Jg:null,__Jh:null,__Ji:false,beforeSetup:function(){this.__mq=qx.bom.element.Style.get(this._getElement(),e);
...
for(var i=0,l=this.__Jg.length;i<l;i++){this.__Jg[i].id=j;
this.__Jh.push(this.__Jg[i]);
this.__Jg[i].afterFinishInternal=function(){qx.lang.Array.remove(self.__Jh,this);
if(this.id<5){self.__Jg[this.id+1].start();
}else{if(self.__Ji){self.end();
}}};
j++;
}this.__Jg[0].start();
},end:function(){if(this.__Jh.length==0){qx.fx.Base.prototype.end.call(this);
}else{this.__Ji=true;
}},afterFinish:function(){qx.bom.element.Style.set(this._getElement(),e,this.__mq);
},_applyDuration:function(k,m){var n=k/6;

for(var i=0,l=this.__Jg.length;i<l;i++){this.__Jg[i].set({duration:n});
}},cancel:function(){for(var i=0,l=this.__Jh.length;i<l;i++){this.__Jh[i].cancel();
}qx.fx.Base.prototype.cancel.call(this);
}},destruct:function(){this._disposeArray(b);
this._disposeArray(c);
}});
})();
(function(){var i="opacity",h="Boolean",g="none",f="qx.fx.effect.combination.Puff",e="_effects",d="__JB",c="__Hu",b="__Ht",a="display";
qx.Class.define(f,{extend:qx.fx.Base,construct:function(j){qx.fx.Base.call(this,j);
this.__Ht=new qx.fx.effect.core.Scale(j);
this.__JB=new qx.fx.effect.core.Fade(j);
this.__Hu=new qx.fx.effect.core.Parallel(this.__Ht,this.__JB);
},properties:{modifyDisplay:{init:true,check:h}},members:{__JB:null,__Ht:null,__Hu:null,afterFinishInternal:function(){if(this.getModifyDisplay()){qx.bom.element.Style.set(this._getElement(),a,g);
}},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var l=this._getElement();
var k={opacity:qx.bom.element.Style.get(l,i)};
this.__JB.afterFinishInternal=function(){var n=this._getElement();

for(var m in k){qx.bom.element.Style.set(n,m,k[m]);
}};
this.__Ht.set({scaleTo:200,sync:true,scaleFromCenter:true,scaleContent:true,restoreAfterFinish:true});
this.__JB.set({sync:true,to:0.0,modifyDisplay:false});
this.__Hu.setDuration(this.getDuration());
this.__Hu.start();
}},destruct:function(){this._disposeArray(e);
this._disposeObjects(c,b,d);
}});
})();
...
(function(){var j="sinodial",i="none",h="overflow",g="flicker",f="center",e="pulse",d="spring",c="full",b="wobble",a="easeOutQuad",E="easeInQuad",D="reverse",C="linear",B="Boolean",A="px",z='bottom-left',y='top-left',x='bottom-right',w="bottom-right",v="bottom-left",q="visible",r="__Ht",o="qx.fx.effect.combination.Shrink",p="__Hs",m="top-left",n="top-right",k='top-right',l='center',s="hidden",t="__Hu",u="display";
qx.Class.define(o,{extend:qx.fx.Base,construct:function(F){qx.fx.Base.call(this,F);
this.__Hs=new qx.fx.effect.core.Move(F);
this.__Ht=new qx.fx.effect.core.Scale(F);
this.__Hu=new qx.fx.effect.core.Parallel(this.__Hs,this.__Ht);
},properties:{direction:{init:f,check:[m,n,v,w,f]},moveTransition:{init:j,check:[C,E,a,j,D,g,b,e,d,i,c]},scaleTransition:{init:j,check:[C,E,a,j,D,g,b,e,d,i,c]},modifyDisplay:{init:true,check:B}},members:{__HD:null,__Hs:null,__Ht:null,__Hu:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
...
var I=this._getElement();
qx.bom.element.Style.set(I,h,q);
var H;

for(var G in this.__HD){H=this.__HD[G];
...
var J,K;
this.__HD={top:qx.bom.element.Location.getTop(L,"scroll"),left:qx.bom.element.Location.getLeft(L,"scroll"),width:qx.bom.element.Dimension.getContentWidth(L),height:qx.bom.element.Dimension.getContentHeight(L),opacity:qx.bom.element.Style.get(L,"opacity")};

switch(this.getDirection()){case y:J=K=0;
break;
case k:J=this.__HD.width;
K=0;
break;
case z:J=0;
K=this.__HD.height;
break;
case x:J=this.__HD.width;
K=this.__HD.height;
break;
case l:J=this.__HD.width/2;
K=this.__HD.height/2;
break;
}this.__Hs.set({x:J,y:K,sync:true,transition:this.getMoveTransition()});
this.__Ht.set({scaleTo:0,sync:true,transition:this.getScaleTransition(),restoreAfterFinish:true});
this.__Hu.setDuration(this.getDuration());
this.__Hu.start();
}},destruct:function(){this._disposeObjects(p,r,t);
}});
})();
(function(){var j="sinodial",i="px",h="flicker",g="0px",f="center",e="pulse",d="spring",c="full",b="wobble",a="easeOutQuad",J="easeInQuad",I="reverse",H="none",G="overflow",F="linear",E="height",D='bottom-left',C='top-left',B='bottom-right',A="bottom-right",q="qx.fx.effect.combination.Grow",r="bottom-left",o="visible",p="__Ht",m="width",n="__Hs",k="top-left",l="top",s="top-right",t='top-right',v="block",u='center',x="hidden",w="left",z="__Hu",y="display";
qx.Class.define(q,{extend:qx.fx.Base,construct:function(K){qx.fx.Base.call(this,K);
this.__Hs=new qx.fx.effect.core.Move(K);
this.__Ht=new qx.fx.effect.core.Scale(K);
this.__Hu=new qx.fx.effect.core.Parallel(this.__Hs,this.__Ht);
},properties:{direction:{init:f,check:[k,s,r,A,f]},scaleTransition:{init:j,check:[F,J,a,j,I,h,b,e,d,H,c]},moveTransition:{init:j,check:[F,J,a,j,I,h,b,e,d,H,c]}},members:{__Ht:null,__Hs:null,__Hu:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
...
this.__Ht.afterFinishInternal=function(){var S;
var T=this._getElement();
...
break;
}this.__Hs.set({x:P,y:Q,sync:true,transition:this.getMoveTransition()});
this.__Ht.set({scaleTo:100,sync:true,scaleFrom:0,scaleFromCenter:false,transition:this.getScaleTransition(),alternateDimensions:[N.width,N.height]});
...
this.__Hu.setDuration(this.getDuration());
this.__Hu.start();
}},destruct:function(){this._disposeObjects(n,p,z);
}});
})();
(function(){var u="out",t="px",s="south",r="Number",q="north-west",p="west",o="in",n="north",m="south-east",l="south-west",e="east",k="north-east",h="display",c="top",b="Boolean",g="block",f="__Hs",i="left",a="__Hu",j="__JB",d="qx.fx.effect.combination.Drop";
qx.Class.define(d,{extend:qx.fx.Base,construct:function(v){qx.fx.Base.call(this,v);
this.__Hs=new qx.fx.effect.core.Move(v);
this.__JB=new qx.fx.effect.core.Fade(v);
this.__Hu=new qx.fx.effect.core.Parallel(this.__Hs,this.__JB);
},properties:{direction:{init:s,check:[s,p,e,n,l,m,k,q]},xAmount:{init:100,check:r},yAmount:{init:100,check:r},mode:{init:u,check:[o,u]},modifyDisplay:{init:true,check:b}},members:{__JB:null,__Hs:null,__Hu:null,start:function(){qx.fx.Base.prototype.start.call(this);
...
this.__JB.afterFinishInternal=function(){for(var B in z){qx.bom.element.Style.set(A,B,z[B]+t);
}};
this.__JB.set({duration:Math.min(0.5,this.getDuration()),sync:true,from:(this.getMode()==u)?1:0,to:(this.getMode()==u)?0:1,modifyDisplay:true});
this.__Hu.setDuration(this.getDuration());
this.__Hu.start();
}},destruct:function(){this._disposeObjects(f,j,a);
}});
})();
(function(){var q="display",p="in",o="overflow",n="block",m="none",l="visible",k="0px",j="hidden",i="visiblity",h="__Lr",c="Boolean",g="height",f="width",b="px",a="qx.fx.effect.combination.Fold",e="__Ls",d="out";
qx.Class.define(a,{extend:qx.fx.Base,construct:function(r){qx.fx.Base.call(this,r);
},properties:{modifyDisplay:{init:true,check:c},mode:{init:p,check:[p,d]}},members:{__Lr:null,__Ls:null,__HD:null,afterFinish:function(){var s=this._getElement();

if((this.getModifyDisplay())&&(this.getMode()==p)){qx.bom.element.Style.set(s,q,n);
}},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var t=this._getElement();
var self=this;
this.__Lr=new qx.fx.effect.core.Scale(t);
this.__Ls=new qx.fx.effect.core.Scale(t);
this.__Lr.afterFinishInternal=function(){self.__Ls.start();
};
this.__Ls.afterFinishInternal=function(){self._cleanUp();
};
this.__HD=this._getStyle();
qx.bom.element.Style.set(t,o,j);

if(this.getMode()==p){this.__Lr.set({scaleTo:5,scaleContent:false,scaleX:false,duration:this.getDuration()/2,scaleFrom:100,scaleFromCenter:true,alternateDimensions:[]});
this.__Ls.set({scaleTo:5,scaleContent:false,scaleY:false,duration:this.getDuration()/2,scaleFrom:100,scaleFromCenter:true,alternateDimensions:[]});
}else{this.__Lr.set({scaleTo:100,scaleContent:false,scaleY:false,duration:this.getDuration()/2,scaleFrom:0,scaleFromCenter:true,alternateDimensions:[this.__HD.width,this.__HD.height]});
this.__Ls.set({scaleTo:100,scaleContent:false,scaleX:false,duration:this.getDuration()/2,scaleFrom:0,scaleFromCenter:false,alternateDimensions:[this.__HD.width,this.__HD.height]});
...
for(var u in this.__HD){v=this.__HD[u];
...
}},destruct:function(){this._disposeObjects(h,e);
}});
})();
(function(){var j="off",i="overflow",h="Boolean",g="qx.fx.effect.combination.Switch",f="none",e="flicker",d="hidden",c="__LY",b="__Ht",a="display";
qx.Class.define(g,{extend:qx.fx.Base,construct:function(k){qx.fx.Base.call(this,k);
this.setTransition(e);
var l=this.__Ht=new qx.fx.effect.core.Scale(k);
this.__Ht.beforeSetup=function(){qx.bom.element.Style.set(k,i,d);
};
this.__LY=new qx.fx.effect.core.Fade(k);
this.__LY.afterFinishInternal=function(){l.start();
};
},properties:{duration:{init:0.5,refine:true},from:{init:0.0,refine:true},to:{init:1.0,refine:true},modifyDisplay:{init:true,check:h},mode:{init:j,check:[j]}},members:{__Ht:null,__LY:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
var n=this._getElement();
var m=qx.bom.element.Style.get(n,i);
this.__Ht.afterFinishInternal=function(){qx.bom.element.Style.set(n,i,m);
};
},afterFinish:function(){if(this.getModifyDisplay()&&(this.getMode()==j)){qx.bom.element.Style.set(this._getElement(),a,f);
}},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}
if(this.getMode()==j){this.__Ht.set({scaleTo:1.0,duration:this.getDuration()/2,scaleFromCenter:true,scaleX:false,scaleContent:false,restoreAfterFinish:true});
this.__LY.set({duration:this.getDuration()/2,from:this.getFrom(),to:1});
}else{}this.__LY.start();
},_applyDuration:function(o,p){this.__Ht.setDuration(o/2);
this.__LY.setDuration(o/2);
