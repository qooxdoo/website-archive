...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Remote_Model-qx.theme.Modern.603494a36e60.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
this.__mT=null;
this.__mS=null;
},properties:{blockSize:{check:c,init:50},maxCachedBlockCount:{check:c,init:15},clearCacheOnRemove:{check:b,init:false},blockConcurrentLoadRowCount:{check:b,init:true}},members:{__Kj:null,__Ko:null,__Kk:null,__Kl:null,__Km:null,__Kn:null,__Kp:null,__Kq:null,__mQ:null,__mU:null,__mS:null,__mT:null,__Kr:false,_getIgnoreCurrentRequest:function(){return this.__Ko;
},getRowCount:function(){if(this.__Kj==-1){if(!this.__Kr||!this.getBlockConcurrentLoadRowCount()){this.__Kr=true;
this._loadRowCount();
}return (this.__Kj==-1)?0:this.__Kj;
}else{return this.__Kj;
}},_loadRowCount:function(){throw new Error("_loadRowCount is abstract");
},_onRowCountLoaded:function(f){if(this.getBlockConcurrentLoadRowCount()){this.__Kr=false;
}if(f==null||f<0){f=0;
}this.__Kj=Number(f);
...
if(this.__Kl!=-1){var h=this._cancelCurrentRequest();
...
this.__Kr=true;
this._loadRowCount();
},clearCache:function(){this.__Kp={};
this.__Kq=0;
},getCacheContent:function(){return {sortColumnIndex:this.__mQ,sortAscending:this.__mU,rowCount:this.__Kj,lruCounter:this.__Kk,rowBlockCache:this.__Kp,rowBlockCount:this.__Kq};
},restoreCacheContent:function(j){if(this.__Kl!=-1){var k=this._cancelCurrentRequest();

if(k){this.__Kl=-1;
this.__Ko=false;
}else{this.__Ko=true;
}}this.__mQ=j.sortColumnIndex;
this.__mU=j.sortAscending;
this.__Kj=j.rowCount;
this.__Kk=j.lruCounter;
this.__Kp=j.rowBlockCache;
this.__Kq=j.rowBlockCount;
var l={firstRow:0,lastRow:this.__Kj-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
for(var w=0;w<=o;w++){var q=this.__Kp[w];
...
if(r!=null){u[t]=r;
}}}}},prefetchRows:function(x,y){if(this.__Kl==-1){var z=this.getBlockSize();
var E=Math.ceil(this.__Kj/z);
...
for(var F=D;F<=C;F++){if(this.__Kp[F]==null||this.__Kp[F].isDirty){if(B==-1){B=F;
}A=F;
}}if(B!=-1){this.__Km=-1;
this.__Kn=-1;
this.__Kl=B;
...
},_onRowDataLoaded:function(I){if(I!=null&&!this.__Ko){var L=this.getBlockSize();
var J=Math.ceil(I.length/L);

if(J==1){this._setRowBlockData(this.__Kl,I);
}else{for(var i=0;i<J;i++){var O=i*L;
var N=[];
var K=Math.min(L,I.length-O);

for(var P=0;P<K;P++){N.push(I[O+P]);
}this._setRowBlockData(this.__Kl+i,N);
}}var M={firstRow:this.__Kl*L,lastRow:(this.__Kl+J+1)*L-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(e,M);
}this.__Kl=-1;
this.__Ko=false;
if(this.__Km!=-1){this.prefetchRows(this.__Km,this.__Kn);
}},_setRowBlockData:function(Q,R){if(this.__Kp[Q]==null){this.__Kq++;

while(this.__Kq>this.getMaxCachedBlockCount()){var V;
var U=this.__Kk;

for(var T in this.__Kp){var S=this.__Kp[T].lru;

if(S<U&&T>1){U=S;
V=T;
}}delete this.__Kp[V];
this.__Kq--;
}}this.__Kp[Q]={lru:++this.__Kk,rowDataArr:R};
...
for(var bf=bc;bf<=bb;bf++){var X=this.__Kp[bf];
...
if(bf==bb-1){if(X.rowDataArr.length==0){delete this.__Kp[bf];
}}else{var bd=this.__Kp[bf+1];

if(bd!=null){X.rowDataArr.push(bd.rowDataArr[0]);
}else{X.isDirty=true;
}}}}
if(this.__Kj!=-1){this.__Kj--;
...
if(bi.lru!=this.__Kk){bi.lru=++this.__Kk;
}return bj;
...
}}},setEditable:function(bv){this.__mS=[];

for(var bw=0;bw<this.getColumnCount();bw++){this.__mS[bw]=bv;
}this.fireEvent(d);
},setColumnEditable:function(bx,by){if(by!=this.isColumnEditable(bx)){if(this.__mS==null){this.__mS=[];
}this.__mS[bx]=by;
this.fireEvent(d);
}},isColumnEditable:function(bz){return (this.__mS?(this.__mS[bz]==true):false);
},setColumnSortable:function(bA,bB){if(bB!=this.isColumnSortable(bA)){if(this.__mT==null){this.__mT=[];
}this.__mT[bA]=bB;
this.fireEvent(d);
}},isColumnSortable:function(bC){return (this.__mT?(this.__mT[bC]!==false):true);
},sortByColumn:function(bD,bE){if(this.__mQ!=bD||this.__mU!=bE){this.__mQ=bD;
this.__mU=bE;
this.clearCache();
this.fireEvent(d);
}},getSortColumnIndex:function(){return this.__mQ;
},isSortAscending:function(){return this.__mU;
},setSortColumnIndexWithoutSortingData:function(bF){this.__mQ=bF;
},setSortAscendingWithoutSortingData:function(bG){this.__mU=bG;
}},destruct:function(){this.__mT=this.__mS=this.__Kp=null;
}});
})();
...
},members:{__Pi:null,__Pj:false,_loadRowCount:function(){this.__Pk();

if(this.__Pj){return;
}
if(this.__Pi){this.__Pl();
}else{this.__Po();
}},_loadRowData:function(p,q){this.__Pk();

if(this.__Pj){return;
}
if(this.__Pi){this.__Pm(p,q);
}else{this.__Pp(p,q);
}},__Pk:function(){if(this.__Pj||this.__Pi!==null){return;
}this.__Pj=true;
this.__Pn(j,function(r){this.__Pj=false;
this.__Pi=(r==b);
this._loadRowCount();
});
},__Pl:function(){var s=d;
this.__Pn(s,function(t){this._onRowCountLoaded(parseInt(t));
});
},__Pm:function(u,v){var w=e+u+n+v;
this.__Pn(w,function(x){this._onRowDataLoaded(qx.lang.Json.parse(x));
});
},__Pn:function(y,z){var A=new qx.io.request.Xhr();
A.setUrl(f+y);
A.addListener(g,function(){z.call(this,A.getResponseText());
},this);
A.send();
},__Po:function(){var self=this;
window.setTimeout(function(){self._onRowCountLoaded(1000000);
},0);
},__Pp:function(B,C){var self=this;
