...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Animation-qx.theme.Classic.0409e28c0129.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
if(qx.core.Environment.get(m)==null){var P=document.createElement(o);
P.innerHTML=x;
document.body.appendChild(P);
return;
}var Y={duration:1000,timing:E,keyFrames:{0:{"width":a},70:{"width":s},100:{"width":a}}};
var N={duration:1000,keyFrames:{0:{"scale":[1,1],"rotate":[k,k],"skew":k,"translate":j},50:{"scale":[1,0.2],"rotate":[k,i],"skew":y,"translate":F},100:{"scale":[1,1],"rotate":[k,k],"skew":k,"translate":j}}};
var K={duration:1000,keyFrames:{0:{"translate":[j,j]},50:{"translate":[q,u]},100:{"translate":[j,j]}}};
var J={duration:1000,origin:H,keyFrames:{0:{"scale":[1,1]},50:{"scale":[2,0.1]},100:{"scale":[1,1]}}};
var V={duration:1000,keep:100,keyFrames:{0:{"rotate":k},100:{"rotate":D}}};
var U={duration:1000,keyFrames:{0:{"rotate":[k,k,k]},50:{"rotate":[c,c,i]},100:{"rotate":[k,k,k]}}};
var L={duration:1000,origin:C,keyFrames:{0:{"skew":k},50:{"skew":b},100:{"skew":k}}};
...
var M={duration:1000,keep:100,timing:l,origin:H,keyFrames:{0:{rotate:[k],skew:[k]},30:{rotate:[i],skew:[b]},100:{rotate:[k],skew:[k]}}};
var O={"Tada":W,"Shake":T,"Skew":L,"Rotate":V,"Scale":J,"translate":K,"Width":Y,"Rotate Out":I,"Rotate In":R,"Up and out":X,"3D Rotate":U,"3D Transform":N,"3D Fall":M};

for(var Q in O){var S=document.createElement(B);
S.innerHTML=Q;
S.className=z;
...
(function(){var j=" ",h="name",g="ms ",f="",d="alternate",c="qx.debug",b="qx.bom.element.Animation",a="end-event";
qx.Bootstrap.define(b,{statics:{__DI:null,__DJ:"Anni",__cq:0,__DK:{},__DL:{"scale":true,"rotate":true,"skew":true,"translate":true},__DM:qx.core.Environment.get("css.animation"),animate:function(k,l){this.__DP(l);
if(qx.core.Environment.get(c)){this.__DQ(l);
}
if(!this.__DI){this.__DI=qx.bom.Stylesheet.createElement();
}var p=l.keyFrames;
if(this.__DM!=null){var name=this.__DR(p,l.reverse);
var n=name+j+l.duration+g+l.repeat+j+l.timing+j+(l.alternate?d:f);
var m=this.__DM[a];
qx.bom.Event.addNativeListener(k,m,this.__DN);
k.style[this.__DM[h]]=n;
}var o=new qx.bom.element.AnimationHandle();
o.desc=l;
o.el=k;
k.$$animation=o;
if(l.origin!=null){qx.bom.element.Transform.setOrigin(k,l.origin);
}if(this.__DM==null){window.setTimeout(function(){qx.bom.element.Animation.__DN({target:k});
},0);
}return o;
},__DN:function(e){var q=e.target;
var s=q.$$animation;
if(!s){return;
}var t=s.desc;

if(qx.bom.element.Animation.__DM!=null){q.style[qx.bom.element.Animation.__DM["name"]]="";
qx.bom.Event.removeNativeListener(q,qx.bom.element.Animation.__DM["name"],qx.bom.element.Animation.__DN);
}
if(t.origin!=null){qx.bom.element.Transform.setOrigin(q,"");
}
if(t.keep!=null){qx.bom.element.Animation.__DO(q,t.keyFrames[t.keep]);
}q.$$animation=null;
s.el=null;
s.ended=true;
var r=s.getOnEnd();

for(var i=0;i<r.length;i++){r[i].callback.call(r[i].ctx,q);
}},__DO:function(u,v){var x;

for(var w in v){if(w in qx.bom.element.Animation.__DL){if(!x){x={};
}x[w]=v[w];
}else{u.style[w]=v[w];
}}if(x){qx.bom.element.Transform.transform(u,x);
}},__DP:function(y){if(!y.hasOwnProperty("alternate")){y.alternate=false;
}
if(!y.hasOwnProperty("keep")){y.keep=null;
}
if(!y.hasOwnProperty("reverse")){y.reverse=false;
}
if(!y.hasOwnProperty("repeat")){y.repeat=1;
}
if(!y.hasOwnProperty("timing")){y.timing="linear";
}
if(!y.hasOwnProperty("origin")){y.origin=null;
}},__DQ:qx.core.Environment.select("qx.debug",{"true":function(z){var A=["origin","duration","keep","keyFrames","repeat","timing","alternate","reverse"];
for(var name in z){if(!(A.indexOf(name)!=-1)){qx.Bootstrap.warn("Unknown key '"+name+"' in the animation description.");
}}if(z.duration==null||z.duration<=0){qx.Bootstrap.warn("No 'duration' given > 0");
}
if(z.keyFrames==null){qx.Bootstrap.warn("No 'keyFrames' given > 0");
}else{for(var B in z.keyFrames){if(B<0||B>100){qx.Bootstrap.warn("Keyframe position needs to be between 0 and 100");
}}}},"default":null}),__DR:function(frames,C){var F="";
for(var I in frames){F+=(C?-(I-100):I)+"% {";
var E=frames[I];
var H;
for(var D in E){if(D in this.__DL){if(!H){H={};
}H[D]=E[D];
}else{F+=D+":"+E[D]+";";
}}if(H){F+=qx.bom.element.Transform.getCss(H);
}F+="} ";
}if(this.__DK[F]){return this.__DK[F];
}var name=this.__DJ+this.__cq++;
var G=this.__DM["keyframes"]+" "+name;
qx.bom.Stylesheet.addRule(this.__DI,G,F);
this.__DK[F]=name;
return name;
}}});
})();
