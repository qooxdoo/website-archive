...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.Github","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Github-qx.theme.Simple.a30f17c33eda.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var s="undefined",r="qx.debug",q="qx.event.type.Rest",p="Success",o="Error",n="success",m="function",l="",k="Check must be object with params as keys",j="loadEnd",c="qx.io.rest.Resource",i="interval",g="Content-Type",b="fail",a="get",f="{",d="=?(\\w+)?}",h="error";
...
try{if(typeof t!==s){if(qx.core.Environment.get(r)){qx.core.Assert.assertMap(t);
}this.__LB(t);
}}catch(e){this.dispose();
throw e;
}},events:{"success":q,"actionSuccess":q,"error":q,"actionError":q},statics:{POLL_THROTTLE_LIMIT:100,POLL_THROTTLE_COUNT:30,REQUIRED:true},members:{__Lt:null,__ES:null,__Lw:null,__Lu:null,__Lv:null,__Lx:null,configureRequest:function(u){this.__Lx=u;
},_getRequest:function(){return new qx.io.request.Xhr();
},__Ly:function(v){var w=this._getRequest();

if(!qx.lang.Type.isArray(this.__Lt[v])){this.__Lt[v]=[];
}this.__Lt[v].push(w);
return w;
},map:function(x,y,z,A){this.__ES[x]=[y,z,A];
this.__Lt[x]=[];
if(x==a){this[x]=undefined;
}if(typeof this[x]!==s&&this[x].action!==true){if(this[x]!==qx.lang.Function.empty){throw new Error("Method with name of action ("+x+") already exists");
}}this.__LC(x+p);
this.__LC(x+o);
this[x]=qx.lang.Function.bind(function(){Array.prototype.unshift.call(arguments,x);
return this.invoke.apply(this,arguments);
},this);
this[x].action=true;
},invoke:function(B,C){var F=this.__Ly(B),I=this._getRequestConfig(B,C),C=C==null?{}:C,E=qx.lang.Object.clone(C);
this.__ES[B].params=C;
if(C){this.__LA(this.__ES[B][1]).forEach(function(J){delete E[J];
});
}this.__Lz(C,I.check);

if(this.__Lx){this.__Lx.call(this,F,B,C);
}this.__Iw(F,E,B,I);
F.addListenerOnce(n,function H(){var K=[F.getResponse(),null,false,F,B,F.getPhase()];
this.fireEvent(B+p,qx.event.type.Rest,K);
this.fireEvent(n,qx.event.type.Rest,K);
},this);
F.addListenerOnce(b,function D(){var L=[F.getResponse(),null,false,F,B,F.getPhase()];
this.fireEvent(B+o,qx.event.type.Rest,L);
this.fireEvent(h,qx.event.type.Rest,L);
},this);
F.addListenerOnce(j,function G(){F.dispose();
},this);
F.send();
return parseInt(F.toHashCode(),10);
},setBaseUrl:function(M){this.__Lw=M;
},__Lz:function(N,O){if(typeof O!==s){if(qx.core.Environment.get(r)){qx.core.Assert.assertObject(O,k);
}qx.lang.Object.getKeys(O).forEach(function(P){if(qx.core.Environment.get(r)){if(O[P]!==true){if(qx.core.Environment.get(r)){qx.core.Assert.assertRegExp(O[P]);
}}}if(O[P]===qx.io.rest.Resource.REQUIRED&&typeof N[P]===s){throw new Error("Missing parameter '"+P+"'");
}if(!(O[P]&&typeof O[P].test==m)){return;
}if(!O[P].test(N[P])){throw new Error("Parameter '"+P+"' is invalid");
}});
}},__Iw:function(Q,R,S,T){Q.set({method:T.method,url:T.url});

if(R){var U=Q.getRequestHeader(g);

if(qx.util.Request.methodAllowsRequestBody(Q.getMethod())){if((/application\/.*\+?json/).test(U)){R=qx.lang.Json.stringify(R);
}}Q.setRequestData(R);
}},abort:function(V){if(qx.lang.Type.isNumber(V)){var ba=V;
var X=qx.core.ObjectRegistry.getPostId();
var Y=qx.core.ObjectRegistry.fromHashCode(ba+X);

if(Y){Y.abort();
}}else{var W=V;
var bb=this.__Lt[W];

if(this.__Lt[W]){bb.forEach(function(bc){bc.abort();
});
}}},refresh:function(bd){this.invoke(bd,this.__ES[bd].params);
},poll:function(be,bf,bg){if(this.__Lu[be]){this.__Lu[be].dispose();
}if(typeof bg==s){bg=this.__ES[be].params;
}this.invoke(be,bg);
var bh=this.__Lu[be]=new qx.event.Timer(bf);
bh.addListener(i,function bi(){var bj=this.__Lt[be][0];

if(bj.isDone()||bj.isDisposed()){this.refresh(be);
}},this);
bh.start();
return bh;
},longPoll:function(bk){var bm=this,bo=true,bp,bq=0;
function bl(){var bs=bp&&((new Date())-bp)<bm._getThrottleLimit();

if(bs){bq+=1;

if(bq>bm._getThrottleCount()){if(qx.core.Environment.get(r)){bm.debug("Received successful response more than "+bm._getThrottleCount()+" times subsequently, each within "+bm._getThrottleLimit()+" ms. Throttling.");
}return true;
}}if(!bs){bq=0;
}return false;
}var bn=this.__Lv[bk]=this.addListener(bk+p,function br(){if(bm.isDisposed()){return;
}
if(!bl()){bp=new Date();
bm.refresh(bk);
}},bo);
this.invoke(bk);
return bn;
},_getRequestConfig:function(bt,bu){var bw=this.__ES[bt];

if(!qx.lang.Type.isArray(bw)){throw new Error("No route for action "+bt);
}var by=bw[0],bv=this.__Lw!==null?this.__Lw+bw[1]:bw[1],bz=bw[2],bx=this.__LA(bv);
bu=bu||{};
bx.forEach(function(bA){var bB=new RegExp(f+bA+d),bC=bv.match(bB)[1];
if(typeof bu[bA]===s){if(bC){bu[bA]=bC;
}else{bu[bA]=l;
}}bv=bv.replace(bB,bu[bA]);
});
return {method:by,url:bv,check:bz};
...
},__LA:function(bD){var bF=/\{(\w+)(=\w+)?\}/g,bG,bE=[];
while((bG=bF.exec(bD))){bE.push(bG[1]);
}return bE;
},__LB:function(bH){qx.lang.Object.getKeys(bH).forEach(function(bI){var bK=bH[bI],bL=bK.method,bJ=bK.url,bM=bK.check;

if(qx.core.Environment.get("qx.debug")){qx.core.Assert.assertString(bL,"Method must be string for route '"+bI+"'");
qx.core.Assert.assertString(bJ,"URL must be string for route '"+bI+"'");
}this.map(bI,bL,bJ,bM);
},this);
},__LC:function(bN){if(!this.constructor.$$events){this.constructor.$$events={};
}
if(!this.constructor.$$events[bN]){this.constructor.$$events[bN]="qx.event.type.Rest";
}}},destruct:function(){var bO;

for(bO in this.__Lt){if(this.__Lt[bO]){this.__Lt[bO].forEach(function(bR){bR.dispose();
});
}}
if(this.__Lu){for(bO in this.__Lu){var bQ=this.__Lu[bO];
bQ.stop();
bQ.dispose();
}}
if(this.__Lv){for(bO in this.__Lv){var bP=this.__Lv[bO];
this.removeListenerById(bP);
}}this.__Lt=this.__ES=this.__Lu=null;
}});
})();
...
qx.Class.define(D,{extend:qx.application.Standalone,members:{__Ot:null,__Ou:null,__Ov:null,__Ow:null,__Cp:null,__Ox:null,main:function(){if(typeof GITHUB==u){return;
...
this._setUpBinding();
this.__Ot.get();
this.__Cp.addListener(e,function(I){var J=I.getData();
this.__Cp.getSelection().push(J.getItem(0));
},this);
this.__Cp.getSelection().addListener(A,function(K){var L=this.__Cp.getSelection().getItem(0).getId();
this.__Ou.get({id:L});
},this);
},_setUpResources:function(){this.__Ot=new github.rest.Resource({"get":{method:i,url:B}});
this.__Ou=new github.rest.Resource({"get":{method:i,url:G}});
},_setUpStores:function(){this.__Ov=new qx.data.store.Rest(this.__Ot,g);
this.__Ow=new qx.data.store.Rest(this.__Ou,g);
...
M.add(N,{edge:n});
this.__Cp=new qx.ui.list.List();
this.__Cp.setWidth(200);
this.__Ox=new github.view.Gist();
M.add(this.__Cp,{edge:H});
M.add(this.__Ox,{edge:y});
this.getRoot().add(M,{edge:0});
},_setUpBinding:function(){var P=this.__Cp,O=this.__Ov,Q=this.__Ow;
P.setLabelPath(v);
...
be.setFont(j);
this.__BA=new qx.ui.basic.Label();
bc.add(be);
bc.add(this.__BA);
this.add(bc,{row:0,column:0});
var bd=new qx.ui.container.Composite(new qx.ui.layout.VBox(5));
this.__Oy=new qx.ui.basic.Image();
this.__Oy.set({width:40,height:40,scale:true,backgroundColor:q});
this.__Oz=new qx.ui.basic.Label();
bd.add(this.__Oy);
bd.add(this.__Oz);
...
this.__md=new qx.ui.embed.Html();
this.__md.setMinHeight(1000);
scroll.add(this.__md);
this.add(be,{row:1,column:0});
this.add(scroll,{row:2,column:0,colSpan:2});
},members:{getDescription:function(){return this.__BA;
},getGravatar:function(){return this.__Oy;
},getUsername:function(){return this.__Oz;
},getContent:function(){return this.__md;
}}});
})();
...
if(j&&qx.lang.Type.isFunction(j.configureRequest)){this.__Iw();
}this.__Iv=qx.lang.Function.bind(this.__Ix,this);
this.__uW();
},properties:{resource:{check:d},actionName:{check:b},model:{nullable:true,event:g}},members:{_marshaler:null,_delegate:null,__Iv:null,__Iw:function(){var k=this.getResource(),l=this._delegate;
k.configureRequest(l.configureRequest);
},__uW:function(){var n=this.getResource(),m=this.getActionName();

if(n&&m){n.addListener(this.getActionName()+c,this.__Iv);
}},__Ix:function(e){var p=e.getData(),o=this._marshaler,q,r=this.getModel(),s=this._delegate;
