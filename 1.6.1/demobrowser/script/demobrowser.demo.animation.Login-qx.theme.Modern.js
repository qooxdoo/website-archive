...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.animation.Login","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.animation.Login-qx.theme.Modern.1d23b298003c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var p="top",o="right",n="resize",m="Name",l="Password",k="__Bz",j="Login",h="appear",g="__DE",f="__Oh",b="demobrowser.demo.animation.Login",d="50%",c="execute",a="30%";
qx.Class.define(b,{extend:qx.application.Standalone,members:{__Bz:null,__DE:null,__Oh:null,checkInput:function(){this.__Bz.start();
},__Oi:function(){this.__Bz=new qx.fx.effect.combination.Shake(this.__DE.getContainerElement().getDomElement());
},main:function(){qx.application.Standalone.prototype.main.call(this);
var r=new qx.ui.layout.Grid(9,5);
r.setColumnAlign(0,o,p);
r.setColumnAlign(2,o,p);
this.__DE=new qx.ui.groupbox.GroupBox().set({contentPadding:[16,16,16,16]});
this.__DE.setLayout(r);
this.__DE.addListener(n,function(e){var v=this.__DE.getBounds();
this.__DE.set({marginTop:Math.round(-v.height/2),marginLeft:Math.round(-v.width/2)});
},this);
this.getRoot().add(this.__DE,{left:d,top:a});
var q=[m,l];

for(var i=0;i<q.length;i++){this.__DE.add(new qx.ui.basic.Label(q[i]).set({allowShrinkX:false,paddingTop:3}),{row:i,column:0});
}var s=new qx.ui.form.TextField();
var t=new qx.ui.form.PasswordField();
this.__DE.add(s.set({allowShrinkX:false,paddingTop:3}),{row:0,column:1});
this.__DE.add(t.set({allowShrinkX:false,paddingTop:3}),{row:1,column:1});
var u=this.__Oh=new qx.ui.form.Button(j);
u.setAllowStretchX(false);
this.__DE.add(u,{row:3,column:1});
u.addListener(c,this.checkInput,this);
this.__DE.addListener(h,this.__Oi,this);
}},destruct:function(){this._disposeObjects(g,f,k);
}});
})();
...
this.__Bp=qx.fx.Base.EffectState.IDLE;
this.__dF=u;
},events:{"setup":s,"update":s,"finish":s},properties:{duration:{init:0.5,check:t,apply:g},fps:{init:100,check:t},sync:{init:false,check:o},from:{init:0,check:t},to:{init:1,check:t},delay:{init:0.0,check:t},queue:{check:i,dereference:true},transition:{init:q,check:[q,f,c,k,p,d,h,j,b,m,e]}},statics:{EffectState:{IDLE:'idle',PREPARING:'preparing',RUNNING:'running'}},members:{__Bp:null,__Bq:null,__Br:null,__Bs:null,__Bt:null,__Bu:null,__Bv:null,__Bw:null,__dF:null,_getElement:function(){return this.__dF;
},_setElement:function(v){this.__dF=v;
},_applyDuration:function(w,x){},init:function(){this.__Bp=qx.fx.Base.EffectState.PREPARING;
this.__Bq=0;
this.__Br=this.getDelay()*1000+(new Date().getTime());
this.__Bs=this.__Br+(this.getDuration()*1000);
this.__Bt=this.getTo()-this.getFrom();
this.__Bu=this.__Bs-this.__Br;
this.__Bv=this.getFps()*this.getDuration();
...
},start:function(){if(this.__Bp!=qx.fx.Base.EffectState.IDLE){return false;
}this.init();
...
this.afterFinish();
},render:function(z){if(this.__Bp==qx.fx.Base.EffectState.PREPARING){this.__Bp=qx.fx.Base.EffectState.RUNNING;
...
if(this.__Bp==qx.fx.Base.EffectState.RUNNING){this.__Bw=qx.fx.Transition.get(this.getTransition())(z)*this.__Bt+this.getFrom();
this.beforeUpdateInternal();
this.beforeUpdate();
this.update(this.__Bw);
this.afterUpdateInternal();
this.afterUpdate();

if(this.hasListener(r)){this.fireEvent(r);
}}},loop:function(A){if(A>=this.__Br){if(A>=this.__Bs){this.end();
}var C=(A-this.__Br)/this.__Bu;
var B=Math.round(C*this.__Bv);
if(B>this.__Bq){this.render(C);
this.__Bq=B;
}}},cancel:function(){if(!this.getSync()){this.getQueue().remove(this);
}this.__Bp=qx.fx.Base.EffectState.IDLE;
},resetState:function(){this.__Bp=qx.fx.Base.EffectState.IDLE;
},isActive:function(){return this.__Bp===qx.fx.Base.EffectState.RUNNING||this.__Bp===qx.fx.Base.EffectState.PREPARING;
}},destruct:function(){this.__dF=this.__Bp=null;
}});
})();
...
this.__Bf=[];
},properties:{limit:{init:Infinity,check:a}},members:{__Bg:null,__Bf:null,add:function(c){var d=new Date().getTime();
c._startOn+=d;
c._finishOn+=d;

if(this.__Bf.length<this.getLimit()){this.__Bf.push(c);
}else{c.resetState();
}
if(!this.__Bg){this.__Bg=qx.lang.Function.periodical(this.loop,15,this);
}},remove:function(e){qx.lang.Array.remove(this.__Bf,e);

if(this.__Bf.length==0){window.clearInterval(this.__Bg);
delete this.__Bg;
}},loop:function(){var f=new Date().getTime();

for(var i=0,g=this.__Bf.length;i<g;i++){this.__Bf[i]&&this.__Bf[i].loop(f);
...
(function(){var e="horizontal",d="vertical",c="__Bf",b="qx.fx.effect.combination.Shake",a="Number";
qx.Class.define(b,{extend:qx.fx.Base,construct:function(f){qx.fx.Base.call(this,f);
this.__Bf=[new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f),new qx.fx.effect.core.Move(f)];
},properties:{direction:{init:e,check:[e,d]},duration:{init:0.5,refine:true},distance:{init:20,check:a}},members:{__Bf:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var g=parseFloat(this.getDistance());
var h=parseFloat(this.getDuration())/10.0;

if(this.getDirection()==e){this.__Bf[0].set({x:g,y:0,duration:h});
this.__Bf[1].set({x:-g*2,y:0,duration:h*2});
this.__Bf[2].set({x:g*2,y:0,duration:h*2});
this.__Bf[3].set({x:-g*2,y:0,duration:h*2});
this.__Bf[4].set({x:g*2,y:0,duration:h*2});
this.__Bf[5].set({x:-g,y:0,duration:h*2});
}else if(this.getDirection()==d){this.__Bf[0].set({y:g,x:0,duration:h});
this.__Bf[1].set({y:-g*2,x:0,duration:h*2});
this.__Bf[2].set({y:g*2,x:0,duration:h*2});
this.__Bf[3].set({y:-g*2,x:0,duration:h*2});
this.__Bf[4].set({y:g*2,x:0,duration:h*2});
this.__Bf[5].set({y:-g,x:0,duration:h*2});
}var k=this.__Bf;

for(var i=0,j=this.__Bf.length;i<j;i++){this.__Bf[i].id=i;

if(i<5){this.__Bf[i].afterFinishInternal=function(){k[this.id+1].start();
};
}}this.__Bf[0].start();
...
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:g,check:[g,e]},x:{init:0,check:f},y:{init:0,check:f}},members:{__EN:null,__EO:null,__EP:null,__EQ:null,__ER:null,setup:function(){var j=this._getElement();
qx.fx.Base.prototype.setup.call(this);

if(j.parentNode){this.__EP=qx.bom.element.Location.getLeft(j)-qx.bom.element.Location.getLeft(j.parentNode);
this.__EQ=qx.bom.element.Location.getTop(j)-qx.bom.element.Location.getTop(j.parentNode);
}else{this.__EP=qx.bom.element.Location.getLeft(j);
this.__EQ=qx.bom.element.Location.getTop(j);
}this.__ER=qx.bom.element.Style.get(j,"position");
qx.bom.element.Style.set(j,h,e);

if(this.getMode()==b){this.__EN=this.getX()-this.__EP;
this.__EO=this.getY()-this.__EQ;
}else{this.__EN=this.getX();
this.__EO=this.getY();
}},update:function(k){var m=this._getElement();
qx.fx.Base.prototype.update.call(this);
var l=Math.round(this.__EN*k+this.__EP);
var top=Math.round(this.__EO*k+this.__EQ);
qx.bom.element.Style.set(m,c,l+i);
qx.bom.element.Style.set(m,d,top+i);
},afterFinishInternal:function(){qx.bom.element.Style.set(this._getElement(),h,this.__ER);
}}});
})();
