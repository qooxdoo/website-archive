...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:todo.33a944a1e430.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var p="",o="task",n="add",m="html.storage.local",l="qx-todo",k="task-",j="change",h="checkbox",g="Task name",f="My first ToDo",b="click",d="todo.Application",c="changeBubble",a="tasks";
qx.Class.define(d,{extend:qx.application.Native,members:{__cq:0,main:function(){qx.application.Native.prototype.main.call(this);
var t;

if(qx.core.Environment.get(m)){var u=new qx.data.store.Offline(l);
t=u.getModel();
}if(t==null){t=qx.data.marshal.Json.createModel([{done:false,name:f}],true);
u&&u.setModel(t);
}var self=this;
var q=document.getElementById(a);
var s=new qx.data.controller.website.List(t,q,o);
s.setDelegate({configureItem:function(v){var w;

for(var i=0;i<v.childNodes.length;i++){if(v.childNodes[i].type==h){w=v.childNodes[i];
}}qx.bom.Event.addNativeListener(w,j,qx.lang.Function.bind(self.__BI,self,t));
},createItem:function(x){x.id=k+self.__cq++;
return qx.bom.Template.get(o,x);
}});
var r=document.getElementById(n);
qx.bom.Event.addNativeListener(r,b,function(){var name=prompt(g,p)||p;
self.__BF(t,name);
});
t.addListener(c,function(e){this.__BG(t);
},this);
this.__BG(t);
},__BE:function(y){for(var i=y.length-1;i>=0;i--){var z=y.getItem(i);

if(z.getDone()){y.remove(z);
}}},__BF:function(A,name){var B=qx.data.marshal.Json.createModel({done:false,name:name},true);
A.push(B);
},__BG:function(C){var D=true;

for(var i=0;i<C.length;i++){if(C.getItem(i).getDone()){D=false;
break;
}}this.__BH(D,C);
},__BH:function(E,F){var G=document.getElementById("clear");
var self=this;

if(E){qx.bom.Event.removeNativeListener(G,"click",function(){self.__BE(F);
});
qx.bom.element.Class.replace(G,"button","button-disabled");
}else{qx.bom.Event.addNativeListener(G,"click",function(){self.__BE(F);
});
qx.bom.element.Class.replace(G,"button-disabled","button");
}},__BI:function(H,e){var I=qx.bom.Event.getTarget(e);
I.parentNode.$$model.setDone(I.checked);
}}});
})();
...
},properties:{model:{nullable:true,event:a,apply:b}},members:{_storage:null,__BJ:null,_applyModel:function(h,i){if(i){i.removeListenerById(this.__BJ);
i.dispose();
this.__BJ=null;
}
if(h){this.__BJ=h.addListener("changeBubble",this.__BK,this);
this.__BK();
}else{this._storage.removeItem(this._key);
}},__BK:function(){var j=qx.util.Serializer.toNativeObject(this.getModel());
...
}},properties:{model:{check:g,apply:c,event:n,nullable:true,dereference:true},target:{check:l,apply:f,event:d,nullable:true,init:null,dereference:true},templateId:{apply:e,event:h,nullable:true,init:null},delegate:{apply:m,event:k,init:null,nullable:true}},members:{__Ao:null,__BM:null,_applyModel:function(r,s){if(s!=undefined){if(this.__Ao!=undefined){s.removeListenerById(this.__Ao);
}
if(this.__BM!=undefined){s.removeListenerById(this.__BM);
}}if(r!=null){if(r instanceof qx.core.Object){this.__Ao=r.addListener(j,this.update,this);
this.__BM=r.addListener("changeBubble",this.update,this);
...
},__BN:function(){var B=this.getTarget();
