...
},renderSeparator:function(cW,cX){var cY=qx.ui.core.Widget.__gW.getDecoratorElement(cW);
this.getContentElement().add(cY);
cY.resize(cX.width,cX.height);
cY.setStyles({left:cX.left+cb,top:cX.top+cb});
if(!this.__hg){this.__hg=[cY];
}else{this.__hg.push(cY);
...
}},_getContentHint:function(){var dl=this.__he;
...
if(this.__gY){var dx=this.__gY.getInsets();
top+=dx.top;
dy+=dx.right;
dA+=dx.bottom;
dz+=dx.left;
...
},__hh:function(){var dG=this._createContentElement();
...
},__hi:null,getLayoutChildren:function(){var dI=this.__hi;

if(!dI){return this.__hj;
}var dJ;
...
},invalidateLayoutChildren:function(){var dK=this.__he;

if(dK){dK.invalidateChildrenCache();
}qx.ui.core.queue.Layout.add(this);
},hasLayoutChildren:function(){var dL=this.__hi;
...
},_indexOf:function(dN){var dO=this.__hi;

if(!dO){return -1;
}return dO.indexOf(dN);
},_hasChildren:function(){var dP=this.__hi;
return dP!=null&&(!!dP[0]);
},addChildrenToQueue:function(dQ){var dR=this.__hi;
...
}},_add:function(dT,dU){if(dT.getLayoutParent()==this){qx.lang.Array.remove(this.__hi,dT);
}
if(this.__hi){this.__hi.push(dT);
}else{this.__hi=[dT];
}this.__hk(dT,dU);
},_addAt:function(dV,dW,dX){if(!this.__hi){this.__hi=[];
}if(dV.getLayoutParent()==this){qx.lang.Array.remove(this.__hi,dV);
}var dY=this.__hi[dW];

if(dY===dV){dV.setLayoutProperties(dX);
}
if(dY){qx.lang.Array.insertBefore(this.__hi,dV,dY);
}else{this.__hi.push(dV);
}this.__hk(dV,dX);
},_addBefore:function(ea,eb,ec){if(ea==eb){return;
}
if(!this.__hi){this.__hi=[];
}if(ea.getLayoutParent()==this){qx.lang.Array.remove(this.__hi,ea);
}qx.lang.Array.insertBefore(this.__hi,ea,eb);
this.__hk(ea,ec);
},_addAfter:function(ed,ee,ef){if(ed==ee){return;
}
if(!this.__hi){this.__hi=[];
}if(ed.getLayoutParent()==this){qx.lang.Array.remove(this.__hi,ed);
}qx.lang.Array.insertAfter(this.__hi,ed,ee);
this.__hk(ed,ef);
},_remove:function(eg){if(!this.__hi){throw new Error("This widget has no children!");
}qx.lang.Array.remove(this.__hi,eg);
this.__hl(eg);
},_removeAt:function(eh){if(!this.__hi){throw new Error("This widget has no children!");
}var ei=this.__hi[eh];
qx.lang.Array.removeAt(this.__hi,eh);
this.__hl(ei);
return ei;
},_removeAll:function(){if(!this.__hi){return [];
}var ej=this.__hi.concat();
this.__hi.length=0;

for(var i=ej.length-1;i>=0;i--){this.__hl(ej[i]);
}qx.ui.core.queue.Layout.add(this);
return ej;
},_afterAddChild:null,_afterRemoveChild:null,__hk:function(ek,em){var parent=ek.getLayoutParent();
...
}},__hl:function(en){if(en.getLayoutParent()!==this){throw new Error("Remove Error: "+en+" is not a child of this widget!");
...
}var er=this.__hb=new qx.html.Element;
er.setStyles({position:ch,top:0,left:0,zIndex:7});
var es=this.getBounds();

if(es){this.__hb.setStyles({width:es.width+cb,height:es.height+cb});
}if((qx.core.Environment.get(bR)==bt)){er.setStyles({backgroundImage:X+qx.util.ResourceManager.getInstance().toUri(bG)+Y,backgroundRepeat:cv});
}this.getContainerElement().add(er);
},_applyDecorator:function(et,eu){var ex=qx.ui.core.Widget.__gW;
var ev=this.getContainerElement();
if(!this.__hb&&!qx.core.Environment.get(h)){this._createProtectorElement();
}if(eu){ev.remove(this.__gY);
ex.poolDecorator(this.__gY);
}if(et){var ew=this.__gY=ex.getDecoratorElement(et);
ew.setStyle(bN,5);
ev.add(ew);
}else{delete this.__gY;
}this._applyBackgroundColor(this.getBackgroundColor());
if(this.__hf(eu,et)){this._updateInsets=true;
qx.ui.core.queue.Layout.add(this);
}else if(et){var ey=this.getBounds();

if(ey){ew.resize(ey.width,ey.height);
this.__hb&&this.__hb.setStyles({width:ey.width+cb,height:ey.height+cb});
}}},_applyShadow:function(ez,eA){var eH=qx.ui.core.Widget.__gX;
var eC=this.getContainerElement();
if(eA){eC.remove(this.__ha);
eH.poolDecorator(this.__ha);
}if(ez){var eE=this.__ha=eH.getDecoratorElement(ez);
eC.add(eE);
var eG=eE.getInsets();
...
}},_applyToolTipText:function(eI,eJ){if(qx.core.Environment.get(bK)){if(this.__hd){return;
}var eK=qx.locale.Manager.getInstance();
this.__hd=eK.addListener(bF,function(){var eL=this.getToolTipText();
...
}this.getContainerElement().setStyle(T,eW,qx.core.Environment.get(bR)==bC);
},_applyBackgroundColor:function(eY,fa){var fb=this.getBackgroundColor();
var fd=this.getContainerElement();

if(this.__gY){this.__gY.tint(fb);
fd.setStyle(ce,null);
}else{var fc=qx.theme.manager.Color.getInstance().resolve(fb);
fd.setStyle(ce,fc);
}},_applyFont:function(fe,ff){},__hm:null,$$stateChanges:null,_forwardStates:null,hasState:function(fg){var fh=this.__hm;
return !!fh&&!!fh[fg];
},addState:function(fi){var fj=this.__hm;
...
}}}},removeState:function(fn){var fo=this.__hm;
...
}}}},replaceState:function(fs,ft){var fu=this.__hm;
...
}}}},__hn:null,__ho:null,syncAppearance:function(){var fC=this.__hm;
var fB=this.__hn;
...
fB=fG.reverse().join(bh).replace(/#[0-9]+/g,bi);
this.__hn=fB;
...
if((qx.core.Environment.get(bR)==bt&&parseFloat(qx.core.Environment.get(f))<8)||(qx.core.Environment.get(bR)==bt&&qx.core.Environment.get(o)<8)){fR.setAttribute(s,A);
}else{fR.setStyle(u,r);
...
}this.getContainerElement().setAttribute(cu,gh?bu:null);
},_applyDroppable:function(gj,gk){if(!this.isEnabled()&&gj===true){gj=false;
}this.getContainerElement().setAttribute(U,gj?bu:null);
...
},scrollChildIntoView:function(gn,go,gp,gq){gq=typeof gq==bm?true:gq;
...
},hasChildControl:function(gz){if(!this.__hp){return false;
}return !!this.__hp[gz];
},__hp:null,_getCreatedChildControls:function(){return this.__hp;
},getChildControl:function(gA,gB){if(!this.__hp){if(gB){return null;
}this.__hp={};
}var gC=this.__hp[gA];
...
},_createChildControl:function(gJ){if(!this.__hp){this.__hp={};
}else if(this.__hp[gJ]){throw new Error("Child control '"+gJ+"' already created!");
}var gN=gJ.indexOf(S);
...
return this.__hp[gJ]=gK;
},_createChildControlImpl:function(gO,gP){return null;
},_disposeChildControls:function(){var gT=this.__hp;
...
if(this.__gY){hi.remove(this.__gY);
hj.__gW.poolDecorator(this.__gY);
}
if(this.__ha){hi.remove(this.__ha);
hj.__gX.poolDecorator(this.__ha);
...
(function(){var j="static",i="user",h="Surfin' Safari",g="Heise",f="http://blogs.msdn.com/ie/rss.xml",e="changeStaticFeedFolder",d="http://blogs.msdn.com/jscript/rss.xml",c="http://webkit.org/blog/feed/",b="http://feeds2.feedburner.com/qooxdoo/news/content",a="changeFeedFolder",w="http://www.heise.de/newsticker/heise-atom.xml",v="http://my.opera.com/desktopteam/xml/rss/blog/",u="JScript Team Blog",t="Feeds",s="feedreader.model.Model",r="IEBlog",q="User Feeds",p="http://daringfireball.net/index.xml",o="Static Feeds",n="qooxdoo News",l="Opera Desktop Blog",m="Daring Fireball",k="changeUserFeedFolder";
qx.Class.define(s,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this._initializeModel();
},properties:{feedFolder:{init:null,event:a,nullable:true},staticFeedFolder:{init:null,event:e,nullable:true},userFeedFolder:{init:null,event:k,nullable:true}},members:{_initializeModel:function(){var x=new feedreader.model.FeedFolder(t);
var y=new feedreader.model.FeedFolder(qx.locale.Manager.tr(o));
x.getFeeds().push(y);
y.getFeeds().push(new feedreader.model.Feed(n,b,j));
y.getFeeds().push(new feedreader.model.Feed(u,d,j));
y.getFeeds().push(new feedreader.model.Feed(m,p,j));
y.getFeeds().push(new feedreader.model.Feed(h,c,j));
var z=new feedreader.model.FeedFolder(qx.locale.Manager.tr(q));
x.getFeeds().push(z);
z.getFeeds().push(new feedreader.model.Feed(g,w,i));
z.getFeeds().push(new feedreader.model.Feed(r,f,i));
