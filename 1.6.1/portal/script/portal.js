...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:portal.f89f95ce8367.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="portal.Application",b="engine.version",a="os.name";
...
this.__Eb=[];
this.__Ec={};
},statics:{dataKeyRoot:h,dataKeyGroupBoxes:m,dataKeyBoxes:d},events:{"loaded":e},properties:{activeBox:{init:null,check:g,apply:c}},members:{__Ea:null,__Eb:null,__Ec:null,_applyActiveBox:function(n,o){if(o!=null){o.setActive(false);
}n.setActive(true);
},load:function(){this.__bf(window[portal.box.Manager.dataKeyRoot]);
qx.event.Registration.fireEvent(this,b);
},__bf:function(p){var t,w,u,v,x,p;
...
this.__Ea.push(x);
this.__Ec[w].push(x);
}}},getBoxes:function(){return this.__Ea;
},getGroupBoxes:function(){return this.__Eb;
},updateGroupBoxMembers:function(y,z,A){var B=qx.dom.Hierarchy.getElementIndex(A.getElement());
qx.lang.Array.remove(this.__Ec[z],A);
qx.lang.Array.insertAt(this.__Ec[y],A,B);
},getGroupBoxDataOfBox:function(C){for(var i=0,j=this.__Eb.length,D;i<j;i++){D=this.__Ec[this.__Eb[i].id];

for(var k=0,l=D.length;k<l;k++){if(D[k].getId()===C){return this.__Eb[i];
}}}return null;
}},destruct:function(){this.__Ea=this.__Eb=this.__Ec=null;
}});
})();
...
this.__Ee=new portal.box.Resizable(this,x);
}if(t.draggable){this.setDraggable(true);
this.__Ef=new portal.box.Draggable(this);
}this.__Eh();
},properties:{element:{init:null},groupBoxId:{init:null,check:h},active:{check:s,init:false,apply:j},resizable:{check:s,init:false},draggable:{check:s,init:false}},members:{__cq:null,__Ed:null,__Ef:null,__Ee:null,_applyActive:function(y,z){if(y){qx.bom.element.Style.set(this.getElement(),q,a);
}else{qx.bom.element.Style.set(this.getElement(),q,n);
}},getId:function(){return this.getElement()?this.getElement().id:null;
},getData:function(){return this.__Ed;
},getDraggableComponent:function(){return this.__Ef;
},getResizableComponent:function(){return this.__Ee;
},__Eg:function(A,B){if(B){for(var C in B){qx.bom.element.Style.set(C,B[C]);
}}qx.bom.element.Style.set(A,l,b);
qx.bom.element.Style.set(A,k,0);
qx.bom.Element.addListener(A,o,this.__Ei,this,true);
},__Eh:function(){var E=f+g;
E=E+r+E+r+E;
var D=qx.bom.Element.create(d,{"class":i});
D.innerHTML=E;
qx.dom.Element.insertEnd(D,this.getElement());
},__Ei:function(e){portal.box.Manager.getInstance().setActiveBox(this);
...
this.__Ej=v;
this.__dF=this.__Ej.getElement();
this.__Ek={};
this.__El=false;
this.__Em={orientation:null,lastCoord:{left:null,top:null}};
this.__En={minWidth:10,minHeight:10,maxWidth:null,maxHeight:null,autoHide:true,handles:3};

for(var name in this.__En){if(w[name]){this.__En[name]=w[name];
}}this.__Eg();
},members:{__Em:null,__Ej:null,__dF:null,__Ek:null,__En:null,__El:false,getBox:function(){return this.__Ej;
},__Eg:function(){switch(this.__En.handles){case 1:this.__Ek.east=null;
break;
case 2:this.__Ek.south=null;
break;
case 3:this.__Ek.south=null;
this.__Ek.southeast=null;
this.__Ek.east=null;
break;
}for(var y in this.__Ek){var x=this.__Ek[y]=qx.bom.Element.create(f);
qx.bom.element.Attribute.set(x,o,y);
qx.dom.Element.insertEnd(x,this.__dF);

if(this.__dF.id){x.id=this.__dF.id+h+y;
}this.__Eo(y);
this.__Ep(y);
}},__Eo:function(z){qx.bom.element.Class.add(this.__Ek[z],z+j);
qx.bom.element.Style.set(this.__Ek[z],u,this.__En.autoHide?t:q);
},__Ep:function(A){var C=qx.event.Registration;
var B=this.__Ek[A];
C.addListener(B,i,this.__Es,this);
C.addListener(document,m,this.__Et,this);

if(this.__En.autoHide){C.addListener(this.__Ej.getElement(),s,this.__Eq,this);
C.addListener(this.__Ej.getElement(),r,this.__Er,this);
}},__Eq:function(e){for(var D in this.__Ek){qx.bom.element.Style.set(this.__Ek[D],u,q);
}},__Er:function(e){if(!this.__El){for(var E in this.__Ek){qx.bom.element.Style.set(this.__Ek[E],u,t);
}}},__Es:function(e){e.stopPropagation();
if(e.isLeftPressed()){qx.event.Registration.addListener(document,p,this.__Eu,this,true);
this.__El=true;
var F=e.getTarget();
this.__Em.orientation=qx.bom.element.Attribute.get(F,o);
this.__Em.lastCoord.left=e.getDocumentLeft();
this.__Em.lastCoord.top=e.getDocumentTop();
portal.box.Util.bringToFront(this.__dF);
}},__Et:function(e){e.stopPropagation();

if(this.__El){qx.event.Registration.removeListener(document,p,this.__Eu,this,true);
portal.box.Util.sendToBack(this.__dF);

if(this.__En.autoHide){for(var G in this.__Ek){qx.bom.element.Style.set(this.__Ek[G],u,t);
}}this.__El=false;
qx.bom.Selection.clear(document.body);
}},__Eu:function(e){e.stopPropagation();

if(this.__El){var H={left:null,top:null};

switch(this.__Em.orientation){case n:H.top=e.getDocumentTop();
break;
...
break;
}this.__Ev(H,this.__Em.lastCoord);
this.__Em.lastCoord.left=e.getDocumentLeft();
this.__Em.lastCoord.top=e.getDocumentTop();
}},__Ev:function(I,J){var Q=qx.bom.element.Dimension;
var N=portal.box.Manager.getInstance().getGroupBoxDataOfBox(this.__Ej.getId());
var P=Q.getContentSize(N.element);

if(I.left){var M=I.left-J.left;

if(M!==0){var L=Q.getContentWidth(this.__dF)+M;
var O=Q.getWidth(this.__dF)+M;

if((this.__En.minWidth==null||L>this.__En.minWidth)&&(this.__En.maxWidth==null||L<=this.__En.maxWidth)){if(P.width>=O){qx.bom.element.Style.set(this.__dF,k,L+d);
this.__Ej.getData().width=L;
}}}}
if(I.top){var M=I.top-J.top;

if(M!==0){var K=Q.getContentHeight(this.__dF)+M;

if((this.__En.minHeight==null||K>this.__En.minHeight)&&(this.__En.maxHeight==null||K<=this.__En.maxHeight)){qx.bom.element.Style.set(this.__dF,c,K+b);
}}}}},destruct:function(){if(this.__En.autoHide){qx.event.Registration.removeListener(this.__Ej.getElement(),s,this.__Eq,this);
qx.event.Registration.removeListener(this.__Ej.getElement(),r,this.__Er,this);
}this.__Ej=this.__En=this.__dF=this.__Ek=null;
this.__Em=null;
}});
})();
...
qx.Class.define(b,{statics:{__Ew:1000,bringToFront:function(d){var e=qx.bom.element.Style.get(d,c);
e=e==a?0:e;
qx.bom.element.Style.set(d,c,e+this.__Ew);
},sendToBack:function(f){var g=qx.bom.element.Style.get(f,c);
qx.bom.element.Style.set(f,c,g-this.__Ew);
}}});
})();
...
(function(){var m="dragstart",l="mouseup",k="mousedown",j="portal.box.Draggable",i="mouseover",h="drag",g="dragend",f="div",d="move",c="dragHandle",a="cursor",b="mousemove";
qx.Class.define(j,{extend:qx.core.Object,construct:function(n){qx.core.Object.call(this);
this.__Ej=n;
this.__dF=n.getElement();
this.__Ex=null;
this.__Ey=null;
this.__Eg();
},members:{__Ej:null,__dF:null,__Ex:null,__Ey:null,getBox:function(){return this.__Ej;
},getElement:function(){return this.__dF;
},__Eg:function(){this.__Ez();
this.__Ep();
},__Ez:function(){this.__Ex=qx.bom.Element.create(f);
qx.bom.element.Class.add(this.__Ex,c);
qx.dom.Element.insertBegin(this.__Ex,this.__dF);
},__Ep:function(){qx.bom.Element.addListener(this.__Ex,k,this.__nv,this);
qx.bom.Element.addListener(this.__Ex,i,function(e){qx.bom.element.Style.set(this,a,d);
},this.__Ex);
qx.bom.Element.addListener(this.__Ex,m,this.__sl,this);
qx.bom.Element.addListener(this.__Ex,g,portal.dragdrop.Manager.getInstance().stopSession,portal.dragdrop.Manager.getInstance());
qx.bom.Element.addListener(this.__Ex,h,this.__EC,this);
},__nv:function(e){if(e.isLeftPressed()){this.__Ey={left:e.getDocumentLeft()-qx.bom.element.Location.getLeft(this.__dF),top:e.getDocumentTop()-qx.bom.element.Location.getTop(this.__dF)};
qx.bom.Element.addListener(document.body,l,this.__EB,this,true);
this.__EA();
qx.event.Registration.fireEvent(this.__Ex,m,qx.event.type.Event);
}},__EA:qx.core.Environment.select("engine.name",{"mshtml":function(){var p=this;
var o=qx.lang.Function.bind(this.__EB,p);
document.getElementsByTagName("html")[0].onmouseleave=o;
},"default":qx.lang.Function.empty}),__EB:function(e){try{e.stopPropagation();
}catch(q){}
if(portal.dragdrop.Manager.getInstance().isSessionActive()){qx.event.Registration.removeListener(document.body,"mousemove",this.__EC,this,true);
portal.dragdrop.Manager.getInstance().stopSession();
}qx.bom.Element.removeListener(document.body,"mouseup",this.__EB,this,true);
},__sl:function(e){if(portal.dragdrop.Manager.getInstance().isSessionActive()){return;
}portal.box.Manager.getInstance().setActiveBox(this.__Ej);
portal.dragdrop.Manager.getInstance().startSession(this.__Ej);
qx.event.Registration.addListener(document.body,b,this.__EC,this,true);
},__EC:function(e){e.stopPropagation();
var r=e.getDocumentLeft()-this.__Ey.left;
var top=e.getDocumentTop()-this.__Ey.top;
qx.event.Timer.once(function(){this.checkGroupBox(r,parseInt(this.getActiveBox().getData().width));
this.onDragMove(top,r);
},portal.dragdrop.Manager.getInstance(),0);
}},destruct:function(){this.__Ej=this.__Ex=this.__dF=this.__Ey=null;
}});
})();
...
this.__ED={top:null,height:null};
this.__EE={top:0,left:0};
},properties:{activeBox:{check:m,init:null,nullable:true},sessionActive:{check:k,init:false}},members:{__ED:null,__EF:null,__EG:false,__EE:null,__EH:null,__EI:null,startSession:function(o){this.setSessionActive(true);
this.setActiveBox(o);
this.__EF=o.getGroupBoxId();
this.__EJ();
if(this.__EH==null){this.__EH={};
var p=portal.box.Manager.getInstance().getGroupBoxes();

for(var i=0,j=p.length;i<j;i++){this.__EH[p[i].element.id]={left:qx.bom.element.Location.getLeft(p[i].element),right:qx.bom.element.Location.getLeft(p[i].element)+qx.bom.element.Dimension.getWidth(p[i].element)};
}}},__EJ:function(){if(!this.__EI){this.__EI=qx.bom.Element.create("div");
}var s=this.getActiveBox().getElement();
var q=qx.bom.element.Dimension.getContentSize(s);
this.__EK(s,this.__EI);
qx.bom.element.Class.add(this.__EI,qx.bom.element.Class.get(s));
...
qx.bom.element.Style.setCss(this.__EI,r);
qx.bom.element.Style.set(s,"border","1px dashed red");
qx.dom.Node.getBodyElement(s).appendChild(this.__EI);
portal.box.Util.bringToFront(this.__EI);
},__EK:function(t,u){while(t.firstChild){u.appendChild(t.firstChild);
}},stopSession:function(e){this.setSessionActive(false);
var w=this.getActiveBox();
var x=w.getElement();
portal.box.Manager.getInstance().updateGroupBoxMembers(w.getGroupBoxId(),this.__EF,w);
w.setGroupBoxId(this.__EF);
var v=new qx.fx.effect.core.Move(this.__EI);
v.set({x:qx.bom.element.Location.getLeft(x),y:qx.bom.element.Location.getTop(x),mode:a,duration:0.5,transition:d});
v.start();
v.addListener(l,function(){this.__EK(this.__EI,x);
qx.bom.element.Style.reset(x,h);
this.__EI.parentNode.removeChild(this.__EI);

if(w.isActive()){w._applyActive(true);
}else{portal.box.Manager.getInstance().setActiveBox(w);
}},this);
},onDragMove:function(top,y){if(this.__EE.left!=y){qx.bom.element.Style.set(this.__EI,f,y+n);
}
if(this.__EE.top!=top){qx.bom.element.Style.set(this.__EI,g,top+n);
}var z=this.getActiveBox().getElement();

if(this.__ED.top==null){this.__ED.top=qx.bom.element.Location.getTop(z);
}
if(this.__ED.height==null){this.__ED.height=qx.bom.element.Dimension.getHeight(z);
}if(this.__EG==false&&top>=this.__ED.top&&top<=(this.__ED.top+this.__ED.height)){return;
}var A;
if(this.__EG){A=qx.dom.Hierarchy.getFirstDescendant(document.getElementById(this.__EF));
while(A!=null){if(this.__EL(true,top,z,A)){this.__EG=false;
return;
}A=qx.dom.Hierarchy.getNextElementSibling(A);
}}else{A=z;
}if(top-this.__EE.top>0){A=qx.dom.Hierarchy.getNextElementSibling(A);

while(A!=null){if(A){if(this.__EL(true,top,z,A)){return;
}}A=qx.dom.Hierarchy.getNextElementSibling(A);
}}else{var A=qx.dom.Hierarchy.getPreviousElementSibling(A);

while(A!=null){if(A){if(this.__EL(false,top,z,A)){return;
}}A=qx.dom.Hierarchy.getPreviousElementSibling(A);
}}this.__EE.left=y;
this.__EE.top=top;
},__EL:function(B,top,C,D){var E=qx.bom.element.Location.getTop(D);
...
this.__ED.top=qx.bom.element.Location.getTop(C);
}return true;
...
},checkGroupBox:function(J,K){if(this.__EH[this.__EF].left<=J&&this.__EH[this.__EF].right>=J){return;
}
for(var L in this.__EH){if(L!=this.__EF){if(this.__EH[L].left<=J+K&&this.__EH[L].right>=J){this.__EF=L;
this.__EG=true;
return;
}}}}},destruct:function(){this.__ED=this.__EE=this.__EF=null;
this.__EM=this.__EI=this.__EH=null;
}});
})();
