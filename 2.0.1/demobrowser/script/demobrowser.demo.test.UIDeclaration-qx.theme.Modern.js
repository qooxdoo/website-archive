...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.UIDeclaration-qx.theme.Modern.0b030140feda.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="Last Name",k="Page 1",j="First Name",h="firstname",g="Email",f="Demo loaded",d="icon/16/apps/office-calendar.png",c="Page 3",b="move",a="First Window",C="Resized to: ",B="Page 2",A="resize",z="Date",y="win",x="Moved to: ",w="Cancel",v="icon/32/apps/office-address-book.png",u="Welcome to your first own Window.<br/>Have fun!",t="Save",r="Simple Form",s="demobrowser.demo.test.UIDeclaration",p="Company",q="Header",n="x",o="Button",m="Item";qx.Class.define(s,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var D={clazz:qx.ui.form.Form,children:[{type:q,label:r},{type:m,label:j,validator:null,name:h,widget:{clazz:qx.ui.form.TextField,required:true,width:200}},{type:m,label:l,widget:{clazz:qx.ui.form.TextField}},{type:m,label:p,widget:{clazz:qx.ui.form.TextField}},{type:m,label:g,widget:{clazz:qx.ui.form.TextField}},{type:m,label:z,widget:{clazz:qx.ui.form.DateField}},{type:o,widget:{clazz:qx.ui.form.Button,label:t}},{type:o,widget:{clazz:qx.ui.form.Button,label:w}}]};var E={clazz:qx.ui.window.Window,layout:{clazz:qx.ui.layout.VBox,spacing:10},layoutProperties:{left:20,top:20},showStatusbar:true,status:f,caption:a,icon:d,id:y,listeners:[{event:b,handler:function(e){this.debug(x+e.getData().left+n+e.getData().top);}
},{event:A,handler:function(e){this.debug(C+e.getData().width+n+e.getData().height);}
}],children:[{clazz:qx.ui.basic.Atom,label:u,icon:v,rich:true},{clazz:qx.ui.tabview.TabView,layoutProperties:{flex:1},children:[{clazz:qx.ui.tabview.Page,label:k,layoutProperties:{flex:1},layout:{clazz:qx.ui.layout.Canvas},children:[D]},{clazz:qx.ui.tabview.Page,label:B,layoutProperties:{flex:1}},{clazz:qx.ui.tabview.Page,label:c,layoutProperties:{flex:1}}]}]};this.addFromDefinition(E);this.win.open();}
,addFromDefinition:function(F){var G=this.getRoot();this.__Bm(G,F);}
,__Bm:function(H,I){var J=new I.clazz();var N=qx.lang.Object.clone(I);delete N.clazz;delete N.layout;delete N.children;delete N.id;delete N.listeners;J.set(N);if(I.id!=null){this[I.id]=J;}
;if(I.layout!=null){var K=new I.layout.clazz();var N=qx.lang.Object.clone(I.layout);delete N.clazz;K.set(N);J.setLayout(K);}
;var M=I.children;if(M!=null){for(var i=0;i<M.length;i++){if(J instanceof qx.ui.form.Form){this.__Ra(J,M[i]);}
else {this.__Bm(J,M[i]);}
;}
;}
;if(J instanceof qx.ui.form.Form){H.add(new qx.ui.form.renderer.Single(J));}
else {H.add(J,I.layoutProperties);}
;if(I.listeners){for(var i=0;i<I.listeners.length;i++){var L=I.listeners[i];J.addListener(L.event,L.handler,L.context);}
;}
;}
,__Ra:function(O,P){if(P.type===q){O.addGroupHeader(P.label);return;}
;var Q=new P.widget.clazz();var R=qx.lang.Object.clone(P.widget);delete R.clazz;Q.set(R);if(P.type===m){O.add(Q,P.label,P.validator,P.name);}
else if(P.type===o){O.addButton(Q);}
;}
}});}
)();
...
(function(){var m="qx.event.type.Data",k="barRight",j="beforeChangeSelection",h="horizontal",g="close-button",f="Index should be less than : ",d="tabview",c="vertical",b="_applyBarPosition",a="barLeft",E="qx.ui.tabview.TabView",D="barTop",C="barBottom",B="__qO",A="bottom",z="button",y="qx.debug",x="changeSelection",w="Incompatible child for TabView: ",v="top",t="left",u="close",r="right",s="page",p="firstTab",q="bar",n="lastTab",o="pane";qx.Class.define(E,{extend:qx.ui.core.Widget,implement:qx.ui.core.ISingleSelection,include:[qx.ui.core.MContentPadding],construct:function(F){qx.ui.core.Widget.call(this);this.__qN={top:D,right:k,bottom:C,left:a};this._createChildControl(q);this._createChildControl(o);var G=this.__qO=new qx.ui.form.RadioGroup;G.setWrap(false);G.addListener(x,this._onChangeSelection,this);if(F!=null){this.setBarPosition(F);}
else {this.initBarPosition();}
;}
,events:{"changeSelection":m},properties:{appearance:{refine:true,init:d},barPosition:{check:[t,r,v,A],init:v,apply:b}},members:{__qO:null,_createChildControlImpl:function(H,I){var J;switch(H){case q:J=new qx.ui.container.SlideBar();J.setZIndex(10);this._add(J);break;case o:J=new qx.ui.container.Stack;J.setZIndex(5);this._add(J,{flex:1});break;};return J||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,H);}
,_getContentPaddingTarget:function(){return this.getChildControl(o);}
,add:function(K){if(qx.core.Environment.get(y)){if(!(K instanceof qx.ui.tabview.Page)){throw new Error(w+K);}
;}
;var L=K.getButton();var M=this.getChildControl(q);var O=this.getChildControl(o);K.exclude();M.add(L);O.add(K);this.__qO.add(L);K.addState(this.__qN[this.getBarPosition()]);K.addState(n);var N=this.getChildren();if(N[0]==K){K.addState(p);}
else {N[N.length-2].removeState(n);}
;K.addListener(u,this._onPageClose,this);}
,addAt:function(P,Q){if(qx.core.Environment.get(y)){if(!(P instanceof qx.ui.tabview.Page)){throw new Error(w+P);}
;}
;var T=this.getChildren();if(!(Q==null)&&Q>T.length){throw new Error(f+T.length);}
;if(Q==null){Q=T.length;}
;var R=P.getButton();var S=this.getChildControl(q);var U=this.getChildControl(o);P.exclude();S.addAt(R,Q);U.addAt(P,Q);this.__qO.add(R);P.addState(this.__qN[this.getBarPosition()]);T=this.getChildren();if(Q==T.length-1){P.addState(n);}
;if(T[0]==P){P.addState(p);}
else {T[T.length-2].removeState(n);}
;P.addListener(u,this._onPageClose,this);}
,remove:function(V){var bb=this.getChildControl(o);var Y=this.getChildControl(q);var X=V.getButton();var ba=bb.getChildren();if(this.getSelection()[0]==V){var W=ba.indexOf(V);if(W==0){if(ba[1]){this.setSelection([ba[1]]);}
else {this.resetSelection();}
;}
else {this.setSelection([ba[W-1]]);}
;}
;Y.remove(X);bb.remove(V);this.__qO.remove(X);V.removeState(this.__qN[this.getBarPosition()]);if(V.hasState(p)){V.removeState(p);if(ba[0]){ba[0].addState(p);}
;}
;if(V.hasState(n)){V.removeState(n);if(ba.length>0){ba[ba.length-1].addState(n);}
;}
;V.removeListener(u,this._onPageClose,this);}
,getChildren:function(){return this.getChildControl(o).getChildren();}
,indexOf:function(bc){return this.getChildControl(o).indexOf(bc);}
,__qN:null,_applyBarPosition:function(bd,be){var bf=this.getChildControl(q);var bk=bd==t||bd==r;var bi=bd==r||bd==A;var bj=bk?qx.ui.layout.HBox:qx.ui.layout.VBox;var bm=this._getLayout();if(bm&&bm instanceof bj){}
else {this._setLayout(bm=new bj);}
;bm.setReversed(bi);bf.setOrientation(bk?c:h);var bl=this.getChildren();if(be){var bg=this.__qN[be];bf.removeState(bg);for(var i=0,l=bl.length;i<l;i++){bl[i].removeState(bg);}
;}
;if(bd){var bh=this.__qN[bd];bf.addState(bh);for(var i=0,l=bl.length;i<l;i++){bl[i].addState(bh);}
;}
;}
,getSelection:function(){var bn=this.__qO.getSelection();var bo=[];for(var i=0;i<bn.length;i++){bo.push(bn[i].getUserData(s));}
;return bo;}
,setSelection:function(bp){var bq=[];for(var i=0;i<bp.length;i++){bq.push(bp[i].getChildControl(z));}
;this.__qO.setSelection(bq);}
,resetSelection:function(){this.__qO.resetSelection();}
,isSelected:function(br){var bs=br.getChildControl(z);return this.__qO.isSelected(bs);}
,isSelectionEmpty:function(){return this.__qO.isSelectionEmpty();}
,getSelectables:function(bt){var bu=this.__qO.getSelectables(bt);var bv=[];for(var i=0;i<bu.length;i++){bv.push(bu[i].getUserData(s));}
;return bv;}
,_onChangeSelection:function(e){var bA=this.getChildControl(o);var bx=e.getData()[0];var bz=e.getOldData()[0];var bw=[];var by=[];if(bx){bw=[bx.getUserData(s)];bA.setSelection(bw);bx.focus();this.scrollChildIntoView(bx,null,null,false);}
else {bA.resetSelection();}
;if(bz){by=[bz.getUserData(s)];}
;this.fireDataEvent(x,bw,by);}
,_onBeforeChangeSelection:function(e){if(!this.fireNonBubblingEvent(j,qx.event.type.Event,[false,true])){e.preventDefault();}
;}
,_onRadioChangeSelection:function(e){var bB=e.getData()[0];if(bB){this.setSelection([bB.getUserData(s)]);}
else {this.resetSelection();}
;}
,_onPageClose:function(e){var bD=e.getTarget();var bC=bD.getButton().getChildControl(g);bC.reset();this.remove(bD);}
},destruct:function(){this._disposeObjects(B);this.__qN=null;}
});}
)();
...
,__mQ:function(e){var m=e.getOldData()[0];var n=e.getData()[0];if(m){if(this.isDynamic()){m.exclude();}
else {m.hide();}
;}
;if(n){n.show();}
;}
