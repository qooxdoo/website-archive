...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.FormValidator","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormValidator-qx.theme.Modern.b1ddd5f68886.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var q="You can send...",p="All fields are required. Some by a custom validator, some by a ",o="complete",n="\n",m="Please enter a password at with least 3 characters.",l="username",k="Server said no!",j="Passwords must be equal.",i="<b>Client side form validation</b><br/>",h="Validating...",c="email address",g="demobrowser.demo.ui.FormValidator",f="Accept",b="execute",a="predefined validator and some by the required flag.",e="password",d="Send";qx.Class.define(g,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var r=new qx.ui.form.TextField();r.setPlaceholder(l);r.setWidth(150);r.setRequired(true);this.getRoot().add(r,{left:20,top:70});var A=new qx.ui.form.TextField();A.setPlaceholder(c);A.setWidth(150);this.getRoot().add(A,{left:20,top:100});var s=new qx.ui.form.TextField();s.setPlaceholder(e);s.setWidth(150);this.getRoot().add(s,{left:20,top:130});var t=new qx.ui.form.TextField();t.setPlaceholder(e);t.setWidth(150);this.getRoot().add(t,{left:20,top:160});var z=new qx.ui.form.CheckBox(f);z.setRequired(true);this.getRoot().add(z,{left:20,top:190});var w=new qx.ui.form.validation.Manager();var u=function(B,C){var D=B!=null&&B.length>2;if(!D){C.setInvalidMessage(m);}
;return D;}
;var x=new qx.ui.form.validation.AsyncValidator(function(E,F){window.setTimeout(function(){if(F==null||F.length==0){E.setValid(false,k);}
else {E.setValid(true);}
;}
,1000);}
);w.add(r,x);w.add(A,qx.util.Validate.email());w.add(s,u);w.add(t,u);w.add(z);w.setValidator(function(G){var I=s.getValue()==t.getValue();if(!I){var H=j;s.setInvalidMessage(H);t.setInvalidMessage(H);s.setValid(false);t.setValid(false);}
;return I;}
);var v=new qx.ui.form.Button(d);this.getRoot().add(v,{left:20,top:215});v.addListener(b,function(){v.setEnabled(false);v.setLabel(h);w.validate();}
,this);w.addListener(o,function(){v.setEnabled(true);v.setLabel(d);if(w.getValid()){alert(q);}
else {alert(w.getInvalidMessages().join(n));}
;}
,this);var y=new qx.ui.basic.Label();y.setRich(true);y.setWidth(400);y.setValue(i+p+a);this.getRoot().add(y,{left:20,top:10});}
}});}
)();
