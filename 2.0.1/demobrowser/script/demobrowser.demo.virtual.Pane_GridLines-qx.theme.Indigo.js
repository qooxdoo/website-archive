...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Pane_GridLines-qx.theme.Indigo.180cab5c063f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var e="demobrowser.demo.virtual.Pane_GridLines",d="red",c="blue",b="vertical",a="horizontal";qx.Class.define(e,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var i=new qx.ui.layout.Grid(5,5);i.setRowFlex(0,1);i.setColumnFlex(0,1);i.setColumnFlex(1,1);i.setColumnFlex(2,1);var f=new qx.ui.container.Composite(i);this.getRoot().add(f,{edge:5});var g=new qx.ui.virtual.core.Scroller(100,10000,60,100);var h=new qx.ui.virtual.layer.GridLines(a);h.setLineSize(1,3);h.setLineColor(2,d);var j=new qx.ui.virtual.layer.GridLines(b);g.getPane().addLayer(h);g.getPane().addLayer(j);f.add(g,{row:0,column:0});var g=new qx.ui.virtual.core.Scroller(100,10000,60,100);var k=new qx.ui.virtual.layer.GridLines(b);k.setLineSize(1,5);k.setLineColor(1,c);g.getPane().addLayer(k);f.add(g,{row:0,column:1});var g=new qx.ui.virtual.core.Scroller(100,10000,60,100);var l=new qx.ui.virtual.layer.GridLines(a);g.getPane().addLayer(l);f.add(g,{row:0,column:2});}
}});}
)();
...
;this.__II=[];this.__IJ=[];this._isHorizontal=(i||b)==b;}
,properties:{defaultLineColor:{init:g,check:e,themeable:true},defaultLineSize:{init:1,check:d,themeable:true}},members:{__II:null,__IJ:null,isHorizontal:function(){return this._isHorizontal;}
...
;this.__II[l]=m;if(this.__IK(l)){this.updateLayerData();}
;}
...
;this.__IJ[n]=o;if(this.__IK(n)){this.updateLayerData();}
;}
,__IK:function(p){if(this._isHorizontal){var t=this.getFirstColumn();var r=t+this.getColumnSizes().length-1;return p>=t&&p<=r;}
else {var s=this.getFirstRow();var q=s+this.getRowSizes().length-1;return p>=s&&p<=q;}
;}
,getLineSize:function(u){return this.__IJ[u]||this.getDefaultLineSize();}
,getLineColor:function(v){return this.__II[v]||this.getDefaultLineColor();}
,__IL:function(w,z,A){var top=0;var B,C;for(var y=0;y<A.length-1;y++){B=this.getLineColor(z+y);C=this.getLineSize(z+y);top+=A[y];w.push("<div style='","position: absolute;","height: "+C+"px;","width: 100%;","top:",top-((C>1)?Math.floor(C/2):1),"px;","background-color:",B,"'>","</div>");}
;}
,__IM:function(D,E,F){var I=0;var G,H;for(var x=0;x<F.length-1;x++){G=this.getLineColor(E+x);H=this.getLineSize(E+x);I+=F[x];D.push("<div style='","position: absolute;","width: "+H+"px;","height: 100%;","top: 0px;","left:",I-((H>1)?Math.floor(H/2):1),"px;","background-color:",G,"'>","</div>");}
;}
,_fullUpdate:function(J,K,L,M){var N=[];if(this._isHorizontal){this.__IL(N,J,L);}
else {this.__IM(N,K,M);}
