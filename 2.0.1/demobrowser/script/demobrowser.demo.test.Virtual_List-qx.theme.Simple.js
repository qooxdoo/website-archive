...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Virtual_List","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Virtual_List-qx.theme.Simple.8727dcb8ab14.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var p="Julian",o="Moritz",n="Jannis",m="Kevin",l="Tim",k="Felix",j="Vincent",h="Erik",g="Additive Selection",f="/places/folder.png",bE="Nils",bD="Nick",bC="Leon",bB="Jannick",bA="Linus",bz="Configurable",by="Marc",bx="icon/16/places/folder.png",bw="demobrowser.demo.test.Virtual_List",bv="addaptive",w="Colin",x="Lennard",u="Quick selection is only available for the modes multi or additive",v="Jason",s="Tobias",t="Florian",q="16",r="Jonas",E="icon/",F="Maximilian",W="Jakob",S="Enable drag selection",bf="Joel",ba="David",br="changeSelection",bl="48",M="Marlon",bu="Tom",bt="Lukas",bs="Luca",K="Julius",O="Fabian",Q="Drag selection is only available for the modes multi or additive",U="Justin",X="Single Selection",bb="Simon",bh="Luis",bn="Johannes",y="Nico",z="Alexander",N="Jan",be="Dominic",bd="Noah",bc="Robin",bj="Enable quick selection",bi="Daniel",Y="Paul",bg="Philipp",a="Niclas",bm="Max",A="One Selection",B="Additive selection",T="Jonathan",b="Value: ",d="Finn",J="Ben",C="on",D="One as selection mode",H="Elias",V="Mika",bp="Multi Selection",bo="Marvin",P="Item No ",bq="changeValue",L="single",bk="additive",G="bold",I="one",c="multi",R="value";qx.Class.define(bw,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var bQ=new qx.ui.basic.Label(bz);bQ.setFont(G);this.getRoot().add(bQ,{left:20,top:20});var bR=new demobrowser.demo.virtual.list.List();bR.setScrollbarX(C);bR.set({height:280,width:150,selectionMode:c});var bN;for(var i=1;i<=2500;i++){bN=new demobrowser.demo.virtual.list.ListItem(P+i,E+((i%4)?q:bl)+f);if(i%4==0){bN.setHeight(56);}
;bR.add(bN);if(i==20){bR.setSelected(bN);}
;}
;bR.addListener(bq,function(e){this.debug(b+e.getData());}
);this.getRoot().add(bR,{left:20,top:40});var bJ=new qx.ui.form.RadioButton(X);var bG=new qx.ui.form.RadioButton(bp);var bH=new qx.ui.form.RadioButton(g);var bF=new qx.ui.form.RadioButton(A);bJ.setUserData(R,L);bG.setUserData(R,c);bH.setUserData(R,bk);bF.setUserData(R,I);bG.setValue(true);this.getRoot().add(bJ,{left:180,top:40});this.getRoot().add(bG,{left:180,top:60});this.getRoot().add(bH,{left:180,top:80});this.getRoot().add(bF,{left:180,top:100});var bI=new qx.ui.form.RadioGroup(bJ,bG,bH,bF);var bM=new qx.ui.form.CheckBox(S);var bP=new qx.ui.form.CheckBox(bj).set({enabled:false});this.getRoot().add(bM,{left:180,top:220});this.getRoot().add(bP,{left:180,top:240});bM.addListener(bq,function(e){if(e.getData()){var bU=bR.getSelectionMode();if(bU==L||bU==I){this.debug(Q);}
;}
;bR.setDragSelection(e.getData());}
);bP.addListener(bq,function(e){if(e.getData()){var bV=bR.getSelectionMode();if(bV==c||bV==bk){this.debug(u);}
;}
;bR.setQuickSelection(e.getData());}
);bI.addListener(br,function(e){var bW=e.getData()[0].getUserData(R);bR.setSelectionMode(bW);if(bW==L||bW==I){bM.setEnabled(false);bP.setEnabled(true);}
else if(bW==c||bW==bv){bM.setEnabled(true);bP.setEnabled(false);}
;}
);var bT=new qx.ui.basic.Label(D);bT.setFont(G);this.getRoot().add(bT,{left:330,top:20});var bO=new demobrowser.demo.virtual.list.List();bO.set({height:280,width:150,selectionMode:I});var bN;for(var i=1;i<=25;i++){bN=new demobrowser.demo.virtual.list.ListItem(P+i,bx);bO.add(bN);if(i==16){bO.setSelected(bN);}
;}
;this.getRoot().add(bO,{left:330,top:40});var bQ=new qx.ui.basic.Label(B);bQ.setFont(G);this.getRoot().add(bQ,{left:520,top:20});var bS=new demobrowser.demo.virtual.list.List;var bK;bS.set({width:150,selectionMode:bk});var bL=[bC,bt,bs,d,l,k,r,bh,F,p,bm,Y,a,N,J,H,bB,bg,bd,bu,o,y,ba,bE,bb,O,h,U,z,W,t,bD,bA,V,v,bi,x,bo,n,s,be,M,by,bn,T,K,w,bf,m,j,bc];for(var i=0;i<bL.length;i++){bK=new demobrowser.demo.virtual.list.ListItem(bL[i]);bS.add(bK);if(i==10||i==12||i==16){bS.addToSelection(bK);}
;}
;this.getRoot().add(bS,{left:520,top:40});}
}});}
)();
...
(function(){var n="one",m="_applySpacing",k="Integer",h="_applyDragSelection",g="String",f="resize",d="demobrowser.demo.virtual.list.List",c="list",b="multi",a="_applyQuickSelection",C="_applyItemHeight",B="_applySelectionMode",A="additive",z="changeName",y="Number",x="addItem",w="removeItem",v="changeSelection",u="row",t="single",r="changeHeight",s=",",p="Boolean",q="selected",o="qx.event.type.Data";qx.Class.define(d,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider],construct:function(){qx.ui.virtual.core.Scroller.call(this,0,1,this.getItemHeight(),10);this.__Rc=new qx.ui.virtual.layer.WidgetCell(this);this.getPane().addLayer(this.__Rc);this.getPane().addListener(f,this._onResize,this);this.__mP=[];this.__cQ=[];this.__nT=new qx.ui.virtual.selection.Row(this.getPane());this.__nT.addListener(v,this._onChangeSelection,this);this.__nT.attachMouseEvents();this.__nT.attachKeyEvents(this);this.__nT.attachListEvents(this);new qx.ui.virtual.behavior.Prefetch(this,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:400,maxAbove:600,minBelow:400,maxBelow:600}).set({interval:500});}
,events:{addItem:o,removeItem:o,changeValue:o,changeSelection:o},properties:{appearance:{refine:true,init:c},focusable:{refine:true,init:true},spacing:{check:k,init:0,apply:m,themeable:true},enableInlineFind:{check:p,init:true},name:{check:g,nullable:true,event:z},itemHeight:{init:24,themeable:true,check:y,apply:C},selectionMode:{check:[t,b,A,n],init:t,apply:B},dragSelection:{check:p,init:false,apply:h},quickSelection:{check:p,init:false,apply:a}},members:{__mP:null,__cQ:null,__nT:null,__Rc:null,syncWidget:function(){this.update();}
,update:function(){var E=this.getPane().getRowConfig();E.setItemCount(this.__mP.length);E.resetItemSizes();for(var i=0;i<this.__mP.length;i++){var D=this.__mP[i].getHeight();if(D!==null){E.setItemSize(i,D);}
;}
;}
,updateSelection:function(){var F=this.__Rc.getChildren();for(var i=0;i<F.length;i++){var G=F[i];var H=G.getUserData(u);if(this.__nT.isItemSelected(H)){G.addState(q);}
else {G.removeState(q);}
;}
;}
,_onResize:function(e){this.getPane().getColumnConfig().setItemSize(0,e.getData().width);qx.ui.core.queue.Widget.add(this);}
,_onChangeItemHeight:function(e){qx.ui.core.queue.Widget.add(this);}
,_onChangeSelection:function(e){this.updateSelection();this.fireDataEvent(v,this.__Rg(e.getData()));}
,getCellWidget:function(I,J){var L=this.__mP[I];if(!L){return null;}
;var K=this.__cQ.pop()||new qx.ui.form.ListItem();K.set({label:L.getLabel(),icon:L.getIcon()});if(this.__nT.isItemSelected(I)){K.addState(q);}
else {K.removeState(q);}
;K.setUserData(u,I);return K;}
,poolCellWidget:function(M){this.__cQ.push(M);}
,getValue:function(){var N=this.getSelection();var O=[];var P;for(var i=0,l=N.length;i<l;i++){P=N[i].getValue();if(P==null){P=N[i].getLabel();}
;O.push(P);}
;return O.join(s);}
,setValue:function(Q){var T=Q.split(s);var S=[];var R;for(var i=0,l=T.length;i<l;i++){R=this.findItem(T[i]);if(R){S.push(R);}
;}
;this.setSelection(S);}
,getChildren:function(){return this.__mP;}
,hasChildren:function(){return this.__mP.length>0;}
,_addHelper:function(U){this.fireDataEvent(x,U);U.addListener(r,this._onChangeItemHeight,this);qx.ui.core.queue.Widget.add(this);}
,_removeHelper:function(V){this.fireDataEvent(w,V);V.removeListener(r,this._onChangeItemHeight,this);qx.ui.core.queue.Widget.add(this);}
,add:function(W){this.__mP.push(W);this._addHelper(W);return this;}
,remove:function(X){qx.lang.Array.remove(this.__mP,X);this._removeHelper(X);return this;}
,removeAll:function(){for(var i=0,j=this.__mP.length;i<j;i++){this._removeHelper(this.__mP[i]);}
;this.__mP=[];}
,indexOf:function(Y){return this.__mP.indexOf(Y);}
,addAt:function(ba,bb){qx.lang.Array.insertAt(this.__mP,ba,bb);this._addHelper(ba);}
,addBefore:function(bc,bd){qx.lang.Array.insertBefore(this.__mP,bc,bd);this._addHelper(bc);}
,addAfter:function(be,bf){qx.lang.Array.insertAfter(this.__mP,be,bf);this._addHelper(be);}
,removeAt:function(bg){this.__mP.splice(bg,1);this._removeHelper(this.__mP[bg]);}
,__Rd:function(bh){var bi=this.__mP.indexOf(bh);if(bi<0){return null;}
else {return bi;}
;}
,__Re:function(bj){var bk=this.__mP[bj];return bk;}
,__Rf:function(bl){var bm=[];for(var i=0;i<bl.length;i++){bm.push(this.__Rd(bl[i]));}
;return bm;}
,__Rg:function(bn){var bo=[];for(var i=0;i<bn.length;i++){bo.push(this.__mP[bn[i]]);}
;return bo;}
,selectAll:function(){this.__nT.selectAll();}
,setSelected:function(bp){this.__nT.selectItem(this.__Rd(bp));}
,isSelected:function(bq){return this.__nT.isItemSelected(this.__Rd(bq));}
,addToSelection:function(br){this.__nT.addItem(this.__Rd(br));}
,removeFromSelection:function(bs){this.__nT.removeItem(this.__Rd(bs));}
,selectRange:function(bt,bu){this.__nT.selectItemRange(this.__Rd(bt),this.__Rd(bu));}
,resetSelection:function(){this.__nT.clearSelection();}
,setSelection:function(bv){this.__nT.replaceSelection(this.__Rf(bv));}
,getSelectedItem:function(){return this.__mP[this.getSelected()];}
,getSelected:function(){return this.__mP[this.__nT.getSelectedItem()];}
,getSelection:function(){return this.__Rg(this.__nT.getSelection());}
,getSortedSelection:function(){return this.__Rg(this.__nT.getSortedSelection());}
...
,getSelectables:function(){return this.__Rg(this.__nT.getSelectables());}
,_applySelectionMode:function(bw,bx){this.__nT.setMode(bw);}
,_applyDragSelection:function(by,bz){this.__nT.setDrag(by);}
,_applyQuickSelection:function(bA,bB){this.__nT.setQuick(bA);}
}});}
)();
...
(function(){var l="_applyInterval",k="appear",j="qx.debug",i="interval",h="__iG",g="qx.ui.virtual.behavior.Prefetch",f="_applyScroller",e="qx.ui.virtual.core.Scroller",d="Integer",c="scroll",a="scrollbar-x",b="scrollbar-y";qx.Class.define(g,{extend:qx.core.Object,construct:function(m,n){if(qx.core.Environment.get(j)){this.assertObject(n);this.assertPositiveInteger(n.minLeft);this.assertPositiveInteger(n.maxLeft);this.assertPositiveInteger(n.minRight);this.assertPositiveInteger(n.maxRight);this.assertPositiveInteger(n.minAbove);this.assertPositiveInteger(n.maxAbove);this.assertPositiveInteger(n.minBelow);this.assertPositiveInteger(n.maxBelow);}
;qx.core.Object.call(this);this.setPrefetchX(n.minLeft,n.maxLeft,n.minRight,n.maxRight);this.setPrefetchY(n.minAbove,n.maxAbove,n.minBelow,n.maxBelow);this.__iG=new qx.event.Timer(this.getInterval());this.__iG.addListener(i,this._onInterval,this);if(m){this.setScroller(m);}
;}
,properties:{scroller:{check:e,nullable:true,init:null,apply:f},interval:{check:d,init:200,apply:l}},members:{__uQ:null,__uR:null,__iG:null,__uS:null,__uT:null,setPrefetchX:function(o,p,q,r){this.__uQ=[o,p,q,r];}
,setPrefetchY:function(s,t,u,v){this.__uR=[s,t,u,v];}
,_onInterval:function(){var w=this.__uQ;if(w[1]&&w[3]){this.getScroller().getPane().prefetchX(w[0],w[1],w[2],w[3]);qx.ui.core.queue.Manager.flush();}
;var x=this.__uR;if(x[1]&&x[3]){this.getScroller().getPane().prefetchY(x[0],x[1],x[2],x[3]);qx.ui.core.queue.Manager.flush();}
;}
,_applyScroller:function(y,z){if(z){if(this.__uS){z.getChildControl(a).removeListenerById(this.__uS);}
;if(this.__uT){z.getChildControl(b).removeListenerById(this.__uT);}
;}
;if(y){if(!y.getContainerElement().getDomElement()){this.__iG.stop();y.addListenerOnce(k,this.__iG.start,this.__iG);}
else {this.__iG.restart();}
;this.__uS=y.getChildControl(a).addListener(c,this.__iG.restart,this.__iG);this.__uT=y.getChildControl(b).addListener(c,this.__iG.restart,this.__iG);}
else {this.__iG.stop();}
;}
,_applyInterval:function(A,B){this.__iG.setInterval(A);}
},destruct:function(){this.setScroller(null);this.__uQ=this.__uR=null;this._disposeObjects(h);}
});}
)();
...
(function(){var i="changeWidth",h="qx.ui.form.RadioGroup",g="_applyManager",f="changeHeight",e="qx.event.type.Event",d="changeValue",c="demobrowser.demo.virtual.list.ListItem",b="Integer",a="String";qx.Class.define(c,{extend:qx.core.Object,construct:function(j,k,l){qx.core.Object.call(this);if(j!=null){this.setLabel(j);}
;if(k!=null){this.setIcon(k);}
;if(l!=null){this.setValue(l);}
;}
,events:{"action":e},properties:{label:{nullable:true,dereference:true,check:a},icon:{check:a,nullable:true},manager:{check:h,nullable:true,apply:g},value:{check:a,nullable:true,event:d},width:{check:b,nullable:true,event:i,init:null},height:{check:b,nullable:true,event:f,init:null}},members:{_applyManager:function(m,n){if(n){n.remove(this);}
;if(m){m.add(this);}
;}
,getFormValue:function(){var o=this.getValue();if(o==null){o=this.getLabel();}
;return o;}
}});}
)();
