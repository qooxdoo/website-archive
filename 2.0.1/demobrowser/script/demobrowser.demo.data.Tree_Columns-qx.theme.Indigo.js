...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Tree_Columns-qx.theme.Indigo.0a2501d7e8ab.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var w="changeModel",v="_applyDelegate",u="qx.core.Object",t="_applyLabelPath",s="icon",r="change",q="label",p="_applyTarget",o="changeTarget",n="_applyIconPath",d="Can not reverse bind the same target property '",m="_applyLabelOptions",h="_applyChildPath",c="_applyIconOptions",b="qx.data.controller.Tree",g="Can not bind the same target property '",f="_applyModel",k="' twice.",a="String",l="get";qx.Class.define(b,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(x,y,z,A){qx.core.Object.call(this);this.__bg={};this.__Bd=[];this.__Dh={};if(z!=null){this.setChildPath(z);}
;if(A!=null){this.setLabelPath(A);}
;if(x!=null){this.setModel(x);}
;if(y!=null){this.setTarget(y);}
;}
,properties:{model:{check:u,apply:f,event:w,nullable:true,dereference:true},target:{apply:p,event:o,init:null,nullable:true,dereference:true},childPath:{check:a,apply:h,nullable:true},labelPath:{check:a,apply:t,nullable:true},iconPath:{check:a,apply:n,nullable:true},labelOptions:{apply:m,nullable:true},iconOptions:{apply:c,nullable:true},delegate:{apply:v,init:null,nullable:true}},members:{__Dh:null,__bg:null,__Bd:null,__Di:null,_applyDelegate:function(B,C){this._setConfigureItem(B,C);this._setCreateItem(B,C);this._setBindItem(B,C);}
,_applyIconOptions:function(D,E){this.__Bo();}
,_applyLabelOptions:function(F,G){this.__Bo();}
,_applyTarget:function(H,I){if(I!=undefined){this.__CE(I);}
;if(this.getModel()!=null){this.__Dl();}
;this._addChangeTargetListener(H,I);}
,_applyModel:function(J,K){this.__Dl();}
,_applyChildPath:function(L,M){this.__Di=M;this.__Dl();this.__Di=null;}
,_applyIconPath:function(N,O){this.__Bo();}
,_applyLabelPath:function(P,Q){this.__Dl();}
,__Dj:function(R){var T=R.getTarget();var S=this.__Dh[T.toHashCode()].treeNode;var U=this.__Dh[T.toHashCode()].modelNode;this.__Dm(S,U);this._updateSelection();}
,__Dk:function(e){var bc=e.getData();var X=e.getOldData();var Y=this.__Dh[X.toHashCode()];X.removeListenerById(Y.changeListenerId);delete this.__Dh[X.toHashCode()];Y.modelNode.removeListenerById(Y.changeChildernListenerId);var bd=Y.modelNode;var bb=qx.Class.getPropertyDefinition(Y.modelNode.constructor,this.getChildPath());var V=bb.event;var ba=bd.addListener(V,this.__Dk,this);var W=Y.treeNode;this.__Dh[bc.toHashCode()]={modelNode:bd,treeNode:W,changeListenerId:Y.changeListenerId,changeChildernListenerId:ba};this.__Dm(W,bd);this._updateSelection();}
...
;return be;}
,__Dl:function(){if(this.getTarget()==null||this.getChildPath()==null){return;}
...
;this.__CE();if(this.getModel()!=null){var bg=this._createItem();bg.setModel(this.getModel());this.getTarget().setRoot(bg);this.__zM(this.getModel(),bg);this.__Dm(bg,this.getModel());}
;}
,__Dm:function(bh,bi){if(bi[l+qx.lang.String.firstUp(this.getChildPath())]==undefined){return;}
;var br=bi[l+qx.lang.String.firstUp(this.getChildPath())]();if(this.__Dh[br.toHashCode()]==undefined){var bm=br.addListener(r,this.__Dj,this);var bq=qx.Class.getPropertyDefinition(bi.constructor,this.getChildPath());var bn=bq.event;var bo=bi.addListener(bn,this.__Dk,this);this.__Dh[br.toHashCode()]={modelNode:bi,treeNode:bh,changeListenerId:bm,changeChildernListenerId:bo};}
;for(var i=0;i<br.length;i++){if(bh.getChildren()[i]==null||br.getItem(i)!=bh.getChildren()[i].getModel()){for(var j=i;j<bh.getChildren().length;j++){if(bh.getChildren()[j].getModel()===br.getItem(i)){var bj=j;break;}
;}
;if(bj!=undefined){var bk=bh.getChildren()[bj];if(this.getTarget().isSelected(bk)){var bs=true;}
;bh.removeAt(bj);bh.addAt(bk,i);if(bs){this.getTarget().addToSelection(bk);}
;}
else {var bp=this._createItem();bp.setModel(br.getItem(i));bh.addAt(bp,i);this.__zM(br.getItem(i),bp);this.__Dm(bp,br.getItem(i));}
;}
;}
;for(var i=bh.getChildren().length-1;i>=br.length;i--){var bl=bh.getChildren()[i];this.__Do(bl,bh);}
;}
,__CE:function(bt){if(bt==null){bt=this.getTarget();}
;if(bt==null){return;}
;var bv=bt.getRoot();if(bv!=null){bt.setRoot(null);this.__Dn(bv);var bu=bv.getModel();if(bu){this.__rb(bu);}
;bv.destroy();this.__Dh={};}
;}
,__Dn:function(bw){var bx=bw.getChildren()||[];for(var i=bx.length-1;i>=0;i--){if(bx[i].getChildren().length>0){this.__Dn(bx[i]);}
;this.__Do(bx[i],bw);}
;}
,__Do:function(by,bz){var bB=by.getModel();var bD=this.__Di||this.getChildPath();var bE=l+qx.lang.String.firstUp(bD);if(bB[bE]!=undefined){var bC=bB[bE]();var bA=this.__Dh[bC.toHashCode()];bC.removeListenerById(bA.changeListenerId);bB.removeListenerById(bA.changeChildernListenerId);this.__Dn(by);delete this.__Dh[bC.toHashCode()];}
;this.__rb(bB);bz.remove(by);by.destroy();}
,bindProperty:function(bF,bG,bH,bI,bJ){var bL=bJ.bind(bF,bI,bG,bH);if(this.__bg[bG]==null){this.__bg[bG]={};}
;var bK=this.__bg[bG];if(bK[bJ.toHashCode()]){if(bK[bJ.toHashCode()].id){throw new Error(g+bG+k);}
;bK[bJ.toHashCode()].id=bL;}
else {bK[bJ.toHashCode()]={id:bL,reverseId:null,treeNode:bI};}
;if(!qx.lang.Array.contains(this.__Bd,bG)){this.__Bd.push(bG);}
;}
,bindPropertyReverse:function(bM,bN,bO,bP,bQ){var bS=bP.bind(bN,bQ,bM,bO);if(this.__bg[bN]==null){this.__bg[bN]={};}
;var bR=this.__bg[bN];if(bR[bQ.toHashCode()]){if(bR[bQ.toHashCode()].reverseId){throw new Error(d+bM+k);}
;bR[bQ.toHashCode()].reverseId=bS;}
else {bR[bQ.toHashCode()]={id:null,reverseId:bS,treeNode:bP};}
;if(!qx.lang.Array.contains(this.__Bd,bN)){this.__Bd.push(bN);}
;}
,bindDefaultProperties:function(bT,bU){this.bindProperty(this.getLabelPath(),q,this.getLabelOptions(),bT,bU);if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),s,this.getIconOptions(),bT,bU);}
;}
,__Bo:function(){var bW;for(var bY in this.__bg){bW=bY;break;}
;for(var bV in this.__bg[bW]){var ca=this.__bg[bW][bV].treeNode;var bX=qx.core.ObjectRegistry.fromHashCode(bV);this.__rb(bX);this.__zM(bX,ca);}
;}
,__zM:function(cb,cc){var cd=this.getDelegate();if(cd!=null&&cd.bindItem!=null){cd.bindItem(this,cc,cb);}
else {this.bindDefaultProperties(cc,cb);}
;}
,__rb:function(ce){for(var i=0;i<this.__Bd.length;i++){var cg=this.__Bd[i];var cf=this.__bg[cg][ce.toHashCode()];if(cf!=null){if(cf.id){ce.removeBinding(cf.id);cf.id=null;}
;if(cf.reverseId){cf.treeNode.removeBinding(cf.reverseId);cf.reverseId=null;}
;delete this.__bg[cg][ce.toHashCode()];}
;}
;}
,_setConfigureItem:function(ch,ci){if(ch!=null&&ch.configureItem!=null&&this.getTarget()!=null&&this.getModel()!=null){var cj=this.getTarget().getRoot().getItems(true,true,false);for(var i=0;i<cj.length;i++){ch.configureItem(cj[i]);}
;}
;}
,_setCreateItem:function(ck,cl){if(this.getTarget()==null||this.getModel()==null){return;}
;if(ck==null||ck.createItem==null){return;}
;if(cl&&cl.createItem&&ck&&ck.createItem&&cl.createItem==ck.createItem){return;}
;this._startSelectionModification();this.__CE();this.__Dl();this._endSelectionModification();this._updateSelection();}
,_setBindItem:function(cm,cn){if(cm!=null&&cm.bindItem!=null){if(cn!=null&&cn.bindItem!=null&&cm.bindItem==cn.bindItem){return;}
;this.__Dl();}
;}
},destruct:function(){this.setTarget(null);this.setModel(null);this.__bg=this.__Dh=this.__Bd=null;}
});}
)();
