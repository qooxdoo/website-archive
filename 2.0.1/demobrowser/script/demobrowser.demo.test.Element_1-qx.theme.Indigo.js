...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Element_1","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Element_1-qx.theme.Indigo.032ec789c7de.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="after2",k="text",j="hello",h="el3b",g="height",f="sans-serif",e="Move, remove & add",d="html",c="<b>hello</b>",b="color",M="Sort completely",L="18px",K="Remove and Add and Remove and Add",J="blue",I="before2",H="Rotation",G="el2b",F="Positioned Insert #2",E="demobrowser.demo.test.Element_1",D="el2a",t="width",u="after1",r="Move",s="Remove",p="test",q="Basics",n="before1",o="bold",v="doc",w="Positioned Insert #1",y="Switch",x="Re-add",A="el3a",z="el1",C="100px",B="id";qx.Class.define(E,{extend:qx.application.Native,members:{main:function(){qx.application.Native.prototype.main.call(this);var O=new qx.html.Root(document.getElementById(p));O.setAttribute(B,v);this.info(q);var W=new qx.html.Element;var T=new qx.html.Element;var V=new qx.html.Element;var P=new qx.html.Element;var N=new qx.html.Element;W.setAttribute(B,z);T.setAttribute(B,D);V.setAttribute(B,G);P.setAttribute(B,A);N.setAttribute(B,h);W.add(T,V);T.add(P,N);O.add(W);P.setAttribute(d,c);N.setAttribute(k,j);N.setStyles({fontWeight:o,fontSize:L,fontFamily:f});W.setStyle(b,J);W.setStyle(t,C).setStyle(g,C);qx.html.Element.flush();this.info(w);var X=new qx.html.Element;X.setAttribute(B,n);X.insertBefore(W);var S=new qx.html.Element;S.setAttribute(B,u);S.insertAfter(W);qx.html.Element.flush();this.info(F);var Y=new qx.html.Element;Y.setAttribute(B,I);O.addAt(Y,0);var U=new qx.html.Element;U.setAttribute(B,m);O.addAt(U,10);qx.html.Element.flush();this.info(r);Y.moveAfter(X);qx.html.Element.flush();U.moveBefore(S);qx.html.Element.flush();this.info(y);var Q=O.indexOf(Y);var R=O.indexOf(U);Y.moveTo(R);U.moveTo(Q);qx.html.Element.flush();this.info(s);O.remove(Y);O.remove(U);qx.html.Element.flush();this.info(x);O.add(Y,U);qx.html.Element.flush();this.info(K);O.remove(Y,U);O.add(Y,U);O.remove(Y,U);O.add(Y,U);qx.html.Element.flush();this.info(e);O.remove(Y);X.moveAfter(S);Y.insertBefore(X);qx.html.Element.flush();this.info(M);var a=O.getChildren();for(var i=1,l=a.length;i<l;i++){a[i].moveTo(0);}
;qx.html.Element.flush();this.info(H);X.moveAfter(Y);qx.html.Element.flush();}
}});}
)();
