...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Filtered_Model","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Filtered_Model-qx.theme.Classic.4289214ad2e9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="<b>HTML-Test <i>",j="icon/22/actions/edit-undo.png",i="between",h=", additional Status. All Values are shown.",g="Hide 1k-5k",f="Table with data filtering",d=", additional Status. 1k - 5k Values are hidden.",c="Show All",b=", additional Status. False Values are hidden.",a="Hide True",z="_tableModel",y="demobrowser.demo.table.Table_Filtered_Model",x=", additional Status. True Values are hidden.",w="!=",v="==",u="Hide False",t="ID",s="A date",r="HTML",q="</i></b>",o=", additional Status.",p="A number",m="A boolean",n="icon/22/actions/list-remove.png",l="execute";qx.Class.define(y,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return f;}
,createTable:function(){var C=this._tableModel=new qx.ui.table.model.Filtered();C.setColumns([t,p,s,m,r]);var D=[];var G=new Date().getTime();var A=400*24*60*60*1000;for(var H=0;H<20;H++){var E=new Date(G+Math.random()*A-A/2);var B=k+H+q;D.push([H,Math.random()*10000,E,(Math.random()>0.5),B]);}
;C.setData(D);C.setColumnEditable(1,true);C.setColumnEditable(2,true);var F=this._table=new qx.ui.table.Table(C);F.setMetaColumnCounts([1,-1]);F.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);F.getTableColumnModel().setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());F.getTableColumnModel().setDataCellRenderer(4,new qx.ui.table.cellrenderer.Html());F.setAdditionalStatusBarText(o);return F;}
,createControls:function(){var I=new qx.ui.toolbar.ToolBar();var N;N=new qx.ui.toolbar.Part();I.add(N);var P=this._table;var J=this._tableModel;var M=new qx.ui.toolbar.Button(a,n);N.add(M);M.addListener(l,function(e){J.addNumericFilter(v,1,m);J.applyFilters();P.setAdditionalStatusBarText(x);}
);var L=new qx.ui.toolbar.Button(u,n);N.add(L);L.addListener(l,function(e){J.addNumericFilter(w,1,m);J.applyFilters();P.setAdditionalStatusBarText(b);}
);var K=new qx.ui.toolbar.Button(g,n);N.add(K);K.addListener(l,function(e){J.addBetweenFilter(i,1000,5000,p);J.applyFilters();P.setAdditionalStatusBarText(d);}
);var O=new qx.ui.toolbar.Button(c,j);N.add(O);O.addListener(l,function(e){J.resetHiddenRows();P.setAdditionalStatusBarText(h);}
);return I;}
...
(function(){var t="regex cannot be null!",s="qx.ui.table.model.Filtered",r="Filter not recognized or value1/value2 is null!",q='',p="notregex cannot be null!",o="Filter not recognized: value or target is null!",n="==",m=">",l="notregex",k="between",d="<",j="regex",g='gi',c="!between",b=">=",f="dataChanged",e="!=",h="<=",a='g';qx.Class.define(s,{extend:qx.ui.table.model.Simple,construct:function(){qx.ui.table.model.Simple.call(this);this.numericAllowed=new Array(n,e,m,d,b,h);this.betweenAllowed=new Array(k,c);this.__Mu=false;this.Filters=new Array();}
,members:{__Mv:null,__Mu:null,_js_in_array:function(u,v){var w=v.toString();if(w==q){return false;}
;var y=new RegExp(u,a);var x=y.test(v);return x;}
,addBetweenFilter:function(z,A,B,C){if(this._js_in_array(z,this.betweenAllowed)&&C!=null){if(A!=null&&B!=null){var D=new Array(z,A,B,C);}
;}
;if(D!=null){this.Filters.push(D);}
else {throw new Error(r);}
;}
,addNumericFilter:function(E,F,G){var H=null;if(this._js_in_array(E,this.numericAllowed)&&G!=null){if(F!=null){H=[E,F,G];}
;}
;if(H!=null){this.Filters.push(H);}
else {throw new Error(o);}
;}
,addRegex:function(I,J,K){var M;if(K){M=g;}
else {M=a;}
;if(I!=null&&J!=null){var L=new Array(j,I,J,M);}
;if(L!=null){this.Filters.push(L);}
else {throw new Error(t);}
;}
,addNotRegex:function(N,O,P){var R;if(P){R=g;}
else {R=a;}
;if(N!=null&&O!=null){var Q=new Array(l,N,O,R);}
;if(Q!=null){this.Filters.push(Q);}
else {throw new Error(p);}
;}
,applyFilters:function(){var i;var W;var S;var V=this.getData();var T=V.length;for(var Y=0;Y<T;Y++){W=false;for(i in this.Filters){if(this._js_in_array(this.Filters[i][0],this.numericAllowed)&&W==false){S=this.getValueById(this.Filters[i][2],Y);switch(this.Filters[i][0]){case n:if(S==this.Filters[i][1]){W=true;}
;break;case e:if(S!=this.Filters[i][1]){W=true;}
;break;case m:if(S>this.Filters[i][1]){W=true;}
;break;case d:if(S<this.Filters[i][1]){W=true;}
;break;case b:if(S>=this.Filters[i][1]){W=true;}
;break;case h:if(S<=this.Filters[i][1]){W=true;}
;break;};}
else if(this._js_in_array(this.Filters[i][0],this.betweenAllowed)&&W==false){S=this.getValueById(this.Filters[i][3],Y);switch(this.Filters[i][0]){case k:if(S>=this.Filters[i][1]&&S<=this.Filters[i][2]){W=true;}
;break;case c:if(S<this.Filters[i][1]&&S>this.Filters[i][2]){W=true;}
;break;};}
else if(this.Filters[i][0]==j&&W==false){S=this.getValueById(this.Filters[i][2],Y);var U=new RegExp(this.Filters[i][1],this.Filters[i][3]);W=U.test(S);}
else if(this.Filters[i][0]==l&&W==false){S=this.getValueById(this.Filters[i][2],Y);var U=new RegExp(this.Filters[i][1],this.Filters[i][3]);W=!U.test(S);}
;;;}
;if(W==true){this.hideRows(Y,1,false);Y--;T--;}
;}
;var X={firstRow:0,lastRow:T-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(f,X);}
,hideRows:function(ba,bb,dispatchEvent){var bd=this.getData();dispatchEvent=(dispatchEvent!=null?dispatchEvent:true);if(!this.__Mu){this.__Mv=bd.slice(0);this.__Mu=true;}
;if(bb==null||bb<1){bb=1;}
;for(var bc=ba;bc<(bd.length-bb);bc++){bd[bc]=bd[bc+bb];}
;this.removeRows(bc,bb);if(dispatchEvent){var be={firstRow:0,lastRow:bd.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};this.fireDataEvent(f,be);}
;}
,resetHiddenRows:function(){if(!this.__Mv){return;}
;this.Filters=[];this.setData(qx.lang.Array.clone(this.__Mv));}
,setData:function(bf,bg){this.__Mv=qx.lang.Array.clone(bf);this.Filters=[];qx.ui.table.model.Simple.prototype.setData.call(this,bf,bg);}
},destruct:function(){this.__Mv=this.numericAllowed=this.betweenAllowed=this.Filters=null;}
});}
)();
...
(function(){var g="_applyIconTrue",f="decoration/table/boolean-true.png",e="qx.ui.table.cellrenderer.Boolean",d=";padding-top:4px;",c="decoration/table/boolean-false.png",b="_applyIconFalse",a="String";qx.Class.define(e,{extend:qx.ui.table.cellrenderer.AbstractImage,construct:function(){qx.ui.table.cellrenderer.AbstractImage.call(this);this.__zc=qx.util.AliasManager.getInstance();this.initIconTrue();this.initIconFalse();}
,properties:{iconTrue:{check:a,init:f,apply:g},iconFalse:{check:a,init:c,apply:b}},members:{__zd:null,__ze:false,__zc:null,_applyIconTrue:function(h){this.__zd=this.__zc.resolve(h);}
,_applyIconFalse:function(i){this.__ze=this.__zc.resolve(i);}
,_insetY:5,_getCellStyle:function(j){return qx.ui.table.cellrenderer.AbstractImage.prototype._getCellStyle.call(this,j)+d;}
,_identifyImage:function(k){var l={imageWidth:11,imageHeight:11};switch(k.value){case true:l.url=this.__zd;break;case false:l.url=this.__ze;break;default:l.url=null;break;};return l;}
},destruct:function(){this.__zc=null;}
});}
)();
(function(){var v="regex cannot be null!",u="font-weight",t="Condition not recognized or value1/value2 is null!",s="Condition not recognized or value is null!",r="text-align",q=";",p='g',o=":",n="qx.ui.table.cellrenderer.Conditional",m="color",e="font-style",l="==",h=">",c="between",b="<",g="regex",f="!between",j=">=",a="!=",k="<=",d="";qx.Class.define(n,{extend:qx.ui.table.cellrenderer.Default,construct:function(w,x,y,z){qx.ui.table.cellrenderer.Default.call(this);this.numericAllowed=[l,a,h,b,j,k];this.betweenAllowed=[c,f];this.conditions=[];this.__yW=w||d;this.__yX=x||d;this.__yY=y||d;this.__za=z||d;}
,members:{__yW:null,__yX:null,__yY:null,__za:null,__zb:function(A,B){if(A[1]!=null){B[r]=A[1];}
;if(A[2]!=null){B[m]=A[2];}
;if(A[3]!=null){B[e]=A[3];}
;if(A[4]!=null){B[u]=A[4];}
;}
,addNumericCondition:function(C,D,E,F,G,H,I){var J=null;if(qx.lang.Array.contains(this.numericAllowed,C)){if(D!=null){J=[C,E,F,G,H,D,I];}
;}
...
;}
,addRegex:function(T,U,V,W,X,Y){if(T!=null){var ba=[g,U,V,W,X,T,Y];}
;if(ba!=null){this.conditions.push(ba);}
else {throw new Error(v);}
;}
,_getCellStyle:function(bb){if(!this.conditions.length){return bb.style||d;}
;var bg=bb.table.getTableModel();var i;var bi;var bc;var be={"text-align":this.__yW,"color":this.__yX,"font-style":this.__yY,"font-weight":this.__za};for(i in this.conditions){bi=false;if(qx.lang.Array.contains(this.numericAllowed,this.conditions[i][0])){if(this.conditions[i][6]==null){bc=bb.value;}
else {bc=bg.getValueById(this.conditions[i][6],bb.row);}
;switch(this.conditions[i][0]){case l:if(bc==this.conditions[i][5]){bi=true;}
...
;break;case j:if(bc>=this.conditions[i][5]){bi=true;}
;break;case k:if(bc<=this.conditions[i][5]){bi=true;}
;break;};}
else if(qx.lang.Array.contains(this.betweenAllowed,this.conditions[i][0])){if(this.conditions[i][7]==null){bc=bb.value;}
else {bc=bg.getValueById(this.conditions[i][7],bb.row);}
;switch(this.conditions[i][0]){case c:if(bc>=this.conditions[i][5]&&bc<=this.conditions[i][6]){bi=true;}
;break;case f:if(bc<this.conditions[i][5]||bc>this.conditions[i][6]){bi=true;}
;break;};}
else if(this.conditions[i][0]==g){if(this.conditions[i][6]==null){bc=bb.value;}
else {bc=bg.getValueById(this.conditions[i][6],bb.row);}
;var bd=new RegExp(this.conditions[i][5],p);bi=bd.test(bc);}
;;if(bi==true){this.__zb(this.conditions[i],be);}
;}
;var bh=[];for(var bf in be){if(be[bf]){bh.push(bf,o,be[bf],q);}
;}
;return bh.join(d);}
},destruct:function(){this.numericAllowed=this.betweenAllowed=this.conditions=null;}
});}
)();
(function(){var c="qooxdoo-table-cell",b="qx.ui.table.cellrenderer.Html",a="";qx.Class.define(b,{extend:qx.ui.table.cellrenderer.Conditional,members:{_getContentHtml:function(d){return (d.value||a);}
