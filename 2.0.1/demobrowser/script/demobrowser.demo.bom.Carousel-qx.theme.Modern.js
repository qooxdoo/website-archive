...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.bom.Carousel-qx.theme.Modern.535c7b6f1d4e.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="repeat",j="timing",i="end",h="Anni",g="The key 'reverse' is deprecated: Please use the method ",f="keep",d=":",c="} ",b="origin",a="end-event",y="reverse",x="css.animation",w="ms ",v="% {",u="'animateReverse' instead.",t="linear",s=";",r="qx.bom.element.AnimationCss",q="keyframes",p="alternate",n="qx.debug",o="name",l="",m=" ";qx.Bootstrap.define(r,{statics:{__su:null,__sv:h,__bV:0,__sw:{},__sx:{"scale":true,"rotate":true,"skew":true,"translate":true},__sy:qx.core.Environment.get(x),animateReverse:function(z,A,B){return this._animate(z,A,B,true);}
,animate:function(C,D,E){return this._animate(C,D,E,false);}
,_animate:function(F,G,H,I){this.__sB(G);if(G.hasOwnProperty(y)){I=G.reverse;if(qx.core.Environment.get(n)){qx.log.Logger.warn(g+u);qx.log.Logger.trace();}
;}
;if(qx.core.Environment.get(n)){this.__BO(G);}
;if(!this.__su){this.__su=qx.bom.Stylesheet.createElement();}
;var M=G.keyFrames;if(H==undefined){H=G.duration;}
;if(this.__sy!=null){var name=this.__sC(M,I);var K=name+m+H+w+G.repeat+m+G.timing+m+(G.alternate?p:l);var J=this.__sy[a];qx.bom.Event.addNativeListener(F,J,this.__sz);F.style[qx.lang.String.camelCase(this.__sy[o])]=K;}
;var L=new qx.bom.element.AnimationHandle();L.desc=G;L.el=F;F.$$animation=L;if(G.origin!=null){qx.bom.element.Transform.setOrigin(F,G.origin);}
;if(this.__sy==null){window.setTimeout(function(){qx.bom.element.AnimationCss.__sz({target:F});}
,0);}
;return L;}
,__sz:function(e){var N=e.target;var O=N.$$animation;if(!O){return;}
;var Q=O.desc;if(qx.bom.element.AnimationCss.__sy!=null){var P=qx.lang.String.camelCase(qx.bom.element.AnimationCss.__sy[o]);N.style[P]=l;qx.bom.Event.removeNativeListener(N,qx.bom.element.AnimationCss.__sy[o],qx.bom.element.AnimationCss.__sz);}
;if(Q.origin!=null){qx.bom.element.Transform.setOrigin(N,l);}
;if(Q.keep!=null){qx.bom.element.AnimationCss.__sA(N,Q.keyFrames[Q.keep]);}
;N.$$animation=null;O.el=null;O.ended=true;O.emit(i,N);}
,__sA:function(R,S){var U;for(var T in S){if(T in qx.bom.element.AnimationCss.__sx){if(!U){U={};}
;U[T]=S[T];}
else {R.style[qx.lang.String.camelCase(T)]=S[T];}
;}
;if(U){qx.bom.element.Transform.transform(R,U);}
;}
,__sB:function(V){if(!V.hasOwnProperty(p)){V.alternate=false;}
;if(!V.hasOwnProperty(f)){V.keep=null;}
;if(!V.hasOwnProperty(k)){V.repeat=1;}
;if(!V.hasOwnProperty(j)){V.timing=t;}
;if(!V.hasOwnProperty(b)){V.origin=null;}
;}
,__BO:qx.core.Environment.select("qx.debug",{"true":function(W){var X=["origin","duration","keep","keyFrames","repeat","timing","alternate","reverse"];for(var name in W){if(!(X.indexOf(name)!=-1)){qx.Bootstrap.warn("Unknown key '"+name+"' in the animation description.");}
;}
;if(W.keyFrames==null){qx.Bootstrap.warn("No 'keyFrames' given > 0");}
else {for(var Y in W.keyFrames){if(Y<0||Y>100){qx.Bootstrap.warn("Keyframe position needs to be between 0 and 100");}
;}
;}
;}
,"default":null}),__sC:function(frames,ba){var bd=l;for(var bg in frames){bd+=(ba?-(bg-100):bg)+v;var bc=frames[bg];var bf;for(var bb in bc){if(bb in this.__sx){if(!bf){bf={};}
;bf[bb]=bc[bb];}
else {bd+=bb+d+bc[bb]+s;}
;}
;if(bf){bd+=qx.bom.element.Transform.getCss(bf);}
;bd+=c;}
;if(this.__sw[bd]){return this.__sw[bd];}
;var name=this.__sv+this.__bV++;var be=this.__sy[q]+m+name;qx.bom.Stylesheet.addRule(this.__su,be,bd);this.__sw[bd]=name;return name;}
}});}
)();
...
(function(){var g="Element",f="qx.bom.element.AnimationHandle",e="play-state",d="paused",c="running",b="",a="css.animation";qx.Bootstrap.define(f,{extend:qx.event.Emitter,construct:function(){var h=qx.core.Environment.get(a);this.__sD=h&&h[e];this.__sE=true;}
,events:{"end":g},members:{__sD:null,__sE:false,__sF:false,isPlaying:function(){return this.__sE;}
,isEnded:function(){return this.__sF;}
,pause:function(){if(this.el){this.el.style[this.__sD]=d;this.el.$$animation.__sE=false;if(this.animationId&&qx.bom.element.AnimationJs){qx.bom.element.AnimationJs.pause(this);}
;}
;}
,play:function(){if(this.el){this.el.style[this.__sD]=c;this.el.$$animation.__sE=true;if(this.i!=undefined&&qx.bom.element.AnimationJs){qx.bom.element.AnimationJs.play(this);}
;}
;}
,stop:function(){if(this.el&&qx.core.Environment.get(a)){this.el.style[this.__sD]=b;this.el.style[qx.core.Environment.get(a).name]=b;this.el.$$animation.__sE=false;this.el.$$animation.__sF=true;}
...
,getCss:function(G){var I=this.__ss(G);if(this.__sr!=null){var H=this.__sr[l];return qx.lang.String.hyphenate(H)+n+I+o;}
;return a;}
,setOrigin:function(J,K){if(this.__sr!=null){J.style[this.__sr[f]]=K;}
;}
,getOrigin:function(L){if(this.__sr!=null){return L.style[this.__sr[f]];}
;return a;}
,setStyle:function(M,N){if(this.__sr!=null){M.style[this.__sr[e]]=N;}
;}
,getStyle:function(O){if(this.__sr!=null){return O.style[this.__sr[e]];}
;return a;}
,setPerspective:function(P,Q){if(this.__sr!=null){P.style[this.__sr[k]]=Q;}
;}
,getPerspective:function(R){if(this.__sr!=null){return R.style[this.__sr[k]];}
;return a;}
,setPerspectiveOrigin:function(S,T){if(this.__sr!=null){S.style[this.__sr[h]]=T;}
;}
,getPerspectiveOrigin:function(U){if(this.__sr!=null){var W=U.style[this.__sr[h]];if(W!=a){return W;}
else {var X=U.style[this.__sr[h]+c];var V=U.style[this.__sr[h]+b];if(X!=a){return X+s+V;}
;}
;}
;return a;}
,setBackfaceVisibility:function(Y,ba){if(this.__sr!=null){Y.style[this.__sr[m]]=ba?d:q;}
;}
,getBackfaceVisibility:function(bb){if(this.__sr!=null){return bb.style[this.__sr[m]]==d;}
;return true;}
,__ss:function(bc){var bf=a;for(var be in bc){var bd=bc[be];if(qx.Bootstrap.isArray(bd)){for(var i=0;i<bd.length;i++){if(bd[i]==undefined){continue;}
;bf+=be+this.__sq[i]+j;bf+=bd[i];bf+=g;}
;}
...
;var R=L.keyFrames;var P=this.__sO(R);var Q=this.__sN(M,P);var S=parseInt(M/Q,10);this.__sI(R,K);var T=this.__sJ(S,Q,P,R,M,L.timing);if(N){T.reverse();}
;var O=new qx.bom.element.AnimationHandle();O.desc=L;O.el=K;O.delta=T;O.stepTime=Q;O.steps=S;K.$$animation=O;O.i=0;O.initValues={};O.repeatSteps=this.__sL(S,L.repeat);return this.play(O);}
,__sI:function(U,V){var W={};for(var ba in U){for(var name in U[ba]){if(W[name]==undefined){var X=U[ba][name];if(typeof X==g){W[name]=X.substring((parseInt(X,10)+p).length,X.length);}
else {W[name]=p;}
;}
;}
;}
...
;if(Y[name]==p&&this.__sH.indexOf(W[name])!=-1){Y[name]=e+W[name];}
;}
;}
;}
;}
,__sJ:function(bb,bc,bd,be,bf,bg){var bo=new Array(bb);var bq=1;bo[0]=be[0];var bi=be[0];var bk=be[bd[bq]];for(var i=1;i<bo.length;i++){if(i*bc/bf*100>bd[bq]){bi=bk;bq++;bk=be[bd[bq]];}
;bo[i]={};for(var name in bk){var bp=bk[name]+p;if(bp.charAt(0)==C){var bj=qx.util.ColorUtil.cssStringToRgb(bi[name]);var bn=qx.util.ColorUtil.cssStringToRgb(bp);var bh=[];for(var j=0;j<bj.length;j++){var bl=bj[j]-bn[j];bh[j]=parseInt(bj[j]-bl*this.__sK(bg,i/bb),10);}
;bo[i][name]=qx.util.ColorUtil.rgbToHexString(bh);}
else if(!isNaN(parseInt(bp,10))){var bm=bp.substring((parseInt(bp,10)+p).length,bp.length);var bl=parseFloat(bp)-parseFloat(bi[name]);bo[i][name]=(parseFloat(bi[name])+bl*this.__sK(bg,i/bb))+bm;}
...
;}
;}
;qx.bom.element.AnimationJs.__sM(br.el,bt);br.i++;if(br.i%br.steps==0){if(br.desc.alternate){br.delta.reverse();}
;}
...
,stop:function(bv){var by=bv.desc;var bw=bv.el;var bx=bv.initValues;window.clearInterval(bv.animationId);if(by.keep!=undefined){this.__sM(bw,by.keyFrames[by.keep]);}
else {this.__sM(bw,bx);}
;bw.$$animation=null;bv.el=null;bv.ended=true;bv.animationId=null;bv.emit(h,bw);}
,__sK:function(bz,x){if(bz==B){var a=[3.1223e-7,0.0757,1.2646,-0.167,-0.4387,0.2654];}
else if(bz==o){var a=[-7.0198e-8,1.652,-0.551,-0.0458,0.1255,-0.1807];}
else if(bz==r){return x;}
else if(bz==t){var a=[2.482e-7,-0.2289,3.3466,-1.0857,-1.7354,0.7034];}
else {var a=[-0.0021,0.2472,9.8054,-21.6869,17.7611,-5.1226];}
;;;var y=0;for(var i=0;i<a.length;i++){y+=a[i]*Math.pow(x,i);}
;return y;}
,__sL:function(bA,bB){if(bB==undefined){return bA;}
;if(bB==w){return Number.MAX_VALUE;}
;return bA*bB;}
,__sM:function(bC,bD){for(var bE in bD){if(bD[bE]===undefined){continue;}
;var name=qx.lang.String.camelCase(bE);if(qx.bom.element.Style){qx.bom.element.Style.set(bC,name,bD[bE]);}
else {bC.style[name]=bD[bE];}
;}
;}
,__sN:function(bF,bG){var bI=100;for(var i=0;i<bG.length-1;i++){bI=Math.min(bI,bG[i+1]-bG[i]);}
;var bH=bF*bI/100;while(bH>this.__sG){bH=bH/2;}
;return Math.round(bH);}
,__sO:function(bJ){var bK=qx.Bootstrap.getKeys(bJ);for(var i=0;i<bK.length;i++){bK[i]=parseInt(bK[i],10);}
;bK.sort(function(a,b){return a-b;}
);return bK;}
}});}
)();
