...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.ui.Label_Reflow","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.Label_Reflow-qx.theme.Classic.ce14b6f51257.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="<b>qooxdoo is cool.</b>",l="qooxdoo (pronounced [’ku:ksdu:]) is a comprehensive and innovative Ajax application framework. Leveraging object-oriented JavaScript allows developers to build impressive cross-browser applications. No HTML, CSS nor DOM knowledge is needed. qooxdoo includes a platform-independent development tool chain, a state-of-the-art GUI toolkit and an advanced client-server communication layer. It is Open Source under an LGPL/EPL dual license.",k="#FFF",j="It includes a platform-independent development tool chain, a state-of-the-art GUI toolkit and an advanced client-server communication layer. It is Open Source under an LGPL/EPL dual license.",i="demobrowser.demo.ui.Label_Reflow",h="Toggle first paragraph",g="Resize Pane",f="#EEE",d="qooxdoo is a comprehensive and innovative Ajax application framework. Leveraging object-oriented JavaScript allows developers to build impressive cross-browser applications. No HTML, CSS nor DOM knowledge is needed.",c="click",a="#CCC",b="main";qx.Class.define(i,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var p=new qx.ui.container.Scroll();var n=new qx.ui.layout.Grid();n.setColumnFlex(0,1);n.setRowFlex(0,1);n.setSpacing(10);var o=new qx.ui.container.Composite(n).set({backgroundColor:f,decorator:b,padding:10,allowGrowX:false,allowGrowY:false,allowStretchX:false,allowStretchY:false});p.add(o);this.getRoot().add(p,{edge:0});var t=this.getPane();o.add(t,{row:0,column:0,colSpan:3});var r=new qx.ui.basic.Label(h).set({backgroundColor:a,decorator:b,padding:[4,8]});o.add(r,{row:1,column:1});var q=false;r.addListener(c,function(){var v=t.getChildren()[0];v.setValue(q?d:m);q=!q;}
);var s=new qx.ui.basic.Label(g).set({backgroundColor:a,decorator:b,padding:[4,8]});o.add(s,{row:1,column:2});var u=false;s.addListener(c,function(e){if(u){t.setWidth(600);}
else {t.setWidth(300);}
;u=!u;}
);}
,getPane:function(){var z=new qx.ui.container.Composite(new qx.ui.layout.VBox(14)).set({backgroundColor:k,width:600,decorator:b,padding:10});var w=new qx.ui.basic.Label().set({rich:true,value:d});z.add(w);var x=new qx.ui.basic.Label().set({rich:true,value:j});z.add(x);var y=new qx.ui.basic.Label().set({rich:true,value:l});z.add(y);return z;}
}});}
)();
