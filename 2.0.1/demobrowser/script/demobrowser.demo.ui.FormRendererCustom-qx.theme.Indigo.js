...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormRendererCustom-qx.theme.Indigo.e2a511b8cf2c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
}});qx.Class.define(c,{extend:qx.ui.form.renderer.Single,members:{__Rk:0,__Rl:0,addItems:function(D,E,F,G){var J=0;if(F!=null){this._add(this._createHeader(F),{row:J,column:this.__Rk,colSpan:2});J++;}
;for(var i=0;i<D.length;i++){var I=this._createLabel(E[i],D[i]);this._add(I,{row:J,column:this.__Rk});var H=D[i];I.setBuddy(H);if(H instanceof qx.ui.form.RadioGroup){H=this._createWidgetForRadioGroup(H);}
;this._add(H,{row:J,column:this.__Rk+1});if(G[i]&&G[i].placeholder){H.setPlaceholder(G[i].placeholder);}
;J++;}
;this.__Rk+=2;this.__Rl<J?this.__Rl=J:null;}
,addButton:function(K){if(this._buttonRow==null){this._buttonRow=new qx.ui.container.Composite();this._buttonRow.setMarginTop(5);var L=new qx.ui.layout.HBox();L.setAlignX(q);L.setSpacing(5);this._buttonRow.setLayout(L);this._add(this._buttonRow,{row:this.__Rl,column:0,colSpan:this.__Rk});}
