...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.root.Inline_Window","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.root.Inline_Window-qx.theme.Modern.78729ed4bbc9.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var o="resize",n="qx.ui.root.Page",m="mshtml",l="The root widget does not support decorators with 'left', or 'top' insets!",k="gecko",j="paddingLeft",i="$$widget",h="The root widget does not support 'left', or 'top' paddings!",g="left",f="paddingTop",b="qxIsRootPage",d="absolute",c="engine.name",a="div";qx.Class.define(n,{extend:qx.ui.root.Abstract,construct:function(p){this.__kp=p;qx.ui.root.Abstract.call(this);this._setLayout(new qx.ui.layout.Basic());this.setZIndex(10000);qx.ui.core.queue.Layout.add(this);this.addListener(o,this.__jP,this);qx.ui.core.FocusHandler.getInstance().connectTo(this);if((qx.core.Environment.get(c)==m)){this.setKeepFocus(true);}
;}
,members:{__iG:null,__kp:null,_createContainerElement:function(){var r=this.__kp.createElement(a);this.__kp.body.appendChild(r);var q=new qx.html.Root(r);q.setStyles({position:d,textAlign:g});q.setAttribute(i,this.toHashCode());if((qx.core.Environment.get(c)==k)){q.setAttribute(b,1);}
;return q;}
,_createContentElement:function(){return new qx.html.Element(a);}
,_computeSizeHint:function(){var s=qx.bom.Document.getWidth(this._window);var t=qx.bom.Document.getHeight(this._window);return {minWidth:s,width:s,maxWidth:s,minHeight:t,height:t,maxHeight:t};}
,__jP:function(e){this.getContainerElement().setStyles({width:0,height:0});this.getContentElement().setStyles({width:0,height:0});}
,supportsMaximize:function(){return false;}
,_applyPadding:function(u,v,name){if(u&&(name==f||name==j)){throw new Error(h);}
;qx.ui.root.Abstract.prototype._applyPadding.call(this,u,v,name);}
,_applyDecorator:function(w,x){qx.ui.root.Abstract.prototype._applyDecorator.call(this,w,x);if(!w){return;}
;var y=this.getDecoratorElement().getInsets();if(y.left||y.top){throw new Error(l);}
;}
...
(function(){var j="demobrowser.demo.root.Inline_Window",i="Add window",h="Window #",g="isle",f="Modal Window #",d="white",c="main",b="Add modal window",a="execute";qx.Class.define(j,{extend:qx.application.Inline,members:{main:function(){qx.application.Inline.prototype.main.call(this);var n=new qx.ui.root.Inline(document.getElementById(g)).set({decorator:c,padding:10,backgroundColor:d});n.setLayout(new qx.ui.layout.HBox(10));var k=new qx.ui.form.Button(i);var m=1;k.addListener(a,function(e){var o=new qx.ui.window.Window(h+m++).set({width:200,height:150});o.moveTo(m*15,m*15);o.open();}
);n.add(k);var l=new qx.ui.form.Button(b);l.addListener(a,this.newModalWindow,this);n.add(l);}
,cModWin:1,newModalWindow:function(){var p=new qx.ui.window.Window(f+this.cModWin++).set({modal:true});p.moveTo(300+this.cModWin*15,this.cModWin*15);p.setLayout(new qx.ui.layout.HBox());var q=new qx.ui.form.Button(b);q.addListener(a,this.newModalWindow,this);p.add(q);p.open();}
}});}
)();
(function(){var q="appear",p="position",o="qx.debug",n="engine.version",m="qx.ui.root.Inline",l="$$widget",k="hidden",h="div",g="left",f="Please use a DOM element to create an inline root.",b="resize",d="mshtml",c="engine.name",a="relative";qx.Class.define(m,{extend:qx.ui.root.Abstract,include:[qx.ui.core.MLayoutHandling],construct:function(r,s,t){if(qx.core.Environment.get(o)){this.assertElement(r,f);}
;this.__BS=r;r.style.overflow=k;r.style.textAlign=g;this.__BT=s||false;this.__BU=t||false;this.__BV();qx.ui.root.Abstract.call(this);this._setLayout(new qx.ui.layout.Basic());qx.ui.core.queue.Layout.add(this);qx.ui.core.FocusHandler.getInstance().connectTo(this);if((qx.core.Environment.get(c)==d)){this.setKeepFocus(true);}
;var window=qx.dom.Node.getWindow(r);qx.event.Registration.addListener(window,b,this._onWindowResize,this);}
,members:{__BT:false,__BU:false,__BS:null,__BV:function(){if(this.__BT||this.__BU){var u=qx.bom.element.Dimension.getSize(this.__BS);if(this.__BT&&u.width<1){throw new Error("The root element "+this.__BS+" of "+this+" needs a width when its width size should be used!");}
;if(this.__BU){if(u.height<1){throw new Error("The root element "+this.__BS+" of "+this+" needs a height when its height size should be used!");}
;if(u.height>=1&&qx.bom.element.Style.get(this.__BS,"height",3)==""){qx.bom.element.Style.set(this.__BS,"height",u.height+"px");}
;}
;qx.event.Registration.addListener(this.__BS,"resize",this._onResize,this);}
;}
,_createContainerElement:function(){var v=this.__BS;if(this.__BT||this.__BU){var z=document.createElement(h);v.appendChild(z);if((qx.core.Environment.get(c)==d)&&qx.core.Environment.get(n)==6){var y=qx.dom.Node.getBodyElement(v);var x;var B;var A=false;var w=qx.dom.Hierarchy.getAncestors(v);for(var i=0,j=w.length;i<j;i++){x=w[i];if(x!=y){B=qx.bom.element.Style.get(x,p);if(B==a){A=true;break;}
;}
else {break;}
;}
;if(A){v.style.position=a;}
;}
;}
else {z=v;}
;var C=new qx.html.Root(z);z.style.position=a;C.setAttribute(l,this.toHashCode());qx.event.Timer.once(function(e){this.fireEvent(q);}
,this,0);return C;}
,_onResize:function(e){var D=e.getData();if((D.oldWidth!==D.width)&&this.__BT||(D.oldHeight!==D.height)&&this.__BU){qx.ui.core.queue.Layout.add(this);}
;}
,_onWindowResize:function(){if(qx.ui.popup&&qx.ui.popup.Manager){qx.ui.popup.Manager.getInstance().hideAll();}
;if(qx.ui.menu&&qx.ui.menu.Manager){qx.ui.menu.Manager.getInstance().hideAll();}
;}
,_computeSizeHint:function(){var I=this.__BT;var F=this.__BU;if(!I||!F){var E=qx.ui.root.Abstract.prototype._computeSizeHint.call(this);}
else {E={};}
;var J=qx.bom.element.Dimension;if(I){var H=J.getContentWidth(this.__BS);E.width=H;E.minWidth=H;E.maxWidth=H;}
;if(F){var G=J.getContentHeight(this.__BS);E.height=G;E.minHeight=G;E.maxHeight=G;}
;return E;}
},defer:function(K,L){qx.ui.core.MLayoutHandling.remap(L);}
,destruct:function(){qx.event.Registration.removeListener(this.__BS,b,this._onResize,this);this.__BS=null;}
});}
)();
(function(){var f="resize",d="interval",c="__iG",b="body",a="qx.event.handler.ElementResize";qx.Class.define(a,{extend:qx.core.Object,implement:qx.event.IEventHandler,construct:function(g){qx.core.Object.call(this);this.__dz=g;this.__Cq={};this.__iG=new qx.event.Timer(200);this.__iG.addListener(d,this._onInterval,this);}
,statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{resize:true},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__Cq:null,__dz:null,__iG:null,canHandleEvent:function(h,i){return h.tagName.toLowerCase()!==b;}
,registerEvent:function(j,k,l){var n=qx.core.ObjectRegistry.toHashCode(j);var m=this.__Cq;if(!m[n]){m[n]={element:j,width:qx.bom.element.Dimension.getWidth(j),height:qx.bom.element.Dimension.getHeight(j)};this.__iG.start();}
;}
,unregisterEvent:function(o,p,q){var s=qx.core.ObjectRegistry.toHashCode(o);var r=this.__Cq;if(r[s]){delete r[s];if(qx.lang.Object.isEmpty(r)){this.__iG.stop();}
;}
;}
,_onInterval:function(e){var u=this.__Cq;for(var x in u){var y=u[x];var t=y.element;var w=qx.bom.element.Dimension.getWidth(t);var v=qx.bom.element.Dimension.getHeight(t);if(y.height!==v||y.width!==w){qx.event.Registration.fireNonBubblingEvent(t,f,qx.event.type.Data,[{width:w,oldWidth:y.width,height:v,oldHeight:y.height}]);y.width=w;y.height=v;}
;}
;}
},destruct:function(){this.__dz=this.__Cq=null;this._disposeObjects(c);}
