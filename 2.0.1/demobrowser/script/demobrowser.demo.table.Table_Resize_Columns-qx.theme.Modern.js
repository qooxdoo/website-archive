...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Resize_Columns","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Resize_Columns-qx.theme.Modern.b11147de5d8a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var t="Add 10 rows",s="Remove 10 rows",r="Whether the the first visible row should ",q="demobrowser.demo.table.Table_Resize_Columns",p="A number",o="10 rows removed",n="_tableModel",m="1*",l="icon/22/actions/list-add.png",k="50%",d="A date",j="changeValue",g="Boolean test",c="be rendered completely when scrolling.",b="Table with automatic column sizes",f="Keep first row",e="icon/22/actions/list-remove.png",h="ID",a="10 rows added",i="execute";qx.Class.define(q,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return b;}
,createTable:function(){var y=this.createRandomRows(50);var w=this._tableModel=this._tableModel=new qx.ui.table.model.Simple();w.setColumns([h,p,d,g]);w.setData(y);var v={tableColumnModel:function(A){return new qx.ui.table.columnmodel.Resize(A);}
};var z=this._table=new qx.ui.table.Table(w,v);z.set({width:600,height:400});z.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);var x=z.getTableColumnModel();var u=x.getBehavior();u.set(0,{width:m,minWidth:40,maxWidth:80});u.setWidth(1,k);u.setMinWidth(1,100);u.setMaxWidth(1,320);var x=z.getTableColumnModel();x.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());return z;}
,createControls:function(){var E=new qx.ui.toolbar.ToolBar();var C,D,F;D=new qx.ui.toolbar.Part();E.add(D);C=new qx.ui.toolbar.Button(t,l);C.addListener(i,function(G){var H=this.createRandomRows(10);this._tableModel.addRows(H);this.info(a);}
,this);D.add(C);C=new qx.ui.toolbar.Button(s,e);C.addListener(i,function(I){var J=this._tableModel.getRowCount();this._tableModel.removeRows(J-10,10);this.info(o);}
,this);D.add(C);D=new qx.ui.toolbar.Part();E.add(D);var B=this._table;F=new qx.ui.toolbar.CheckBox(f);F.set({value:B.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(r+c)});F.addListener(j,function(K){B.setKeepFirstVisibleRowComplete(this.getValue());}
,F);D.add(F);return E;}
