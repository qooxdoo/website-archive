...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.widget.List","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.widget.List-qx.theme.Classic.fd85430e7d77.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var bL="Julian",bK="Show Icon",bJ="Kevin",bI="Marvin",bH="Marc",bG="Configurable",bF="icon/16/places/folder.png",bE="computer.png",bD="Jason",bC="Jakob",br="Lukas",bq="Julius",bp="network-wired.png",bo="Jan",bn="Tobias",bm="Mika",bl="Nils",bk="Noah",bj="Quick selection is only available for the modes multi or additive",bi="Tom",bS="Drag selection is only available for the modes multi or additive",bT="Multi Selection",bQ="battery.png",bR="Felix",bO="Leon",bP="input-keyboard.png",bM="camera-web.png",bN="Enable drag selection",bU="icon",bV="Show Both",bv="48",bu="icon/48/devices/",bx="Fabian",bw="Nico",bz="camera-photo.png",by="media-flash.png",bB="input-mouse.png",bA="Joel",bt="Dominic",bs="Show Label",a="demobrowser.demo.widget.List",b="Enable quick selection",c="Finn",d="One as selection mode",f="Jannis",g="audio-card.png",h="Alexander",j="Additive Selection",k="display.png",l="Erik",ca="addaptive",bY="Johannes",bX="pda.png",bW="16",ce="Jonas",cd="icon/",cc="David",cb="Maximilian",cg="Tim",cf="Justin",K="Single Selection",L="Linus",I="Daniel",J="Paul",O="Philipp",P="Niclas",M="One Selection",N="Nick",G="Jonathan",H="Ben",t="on",s="Value: ",v="Vincent",u="/places/folder.png",p="Robin",o="both",r="Jannick",q="Luis",n="multimedia-player.png",m="drive-harddisk.png",U="Lennard",V="Moritz",W="Florian",X="drive-optical.png",Q="media-optical.png",R="Marlon",S="Luca",T="network-wireless.png",Y="audio-input-microphone.png",ba="phone.png",D="Simon",C="Colin",B="label",A="printer.png",z="Max",y="Additive selection",x="Horizontal, Icons only",w="Elias",F="changeValue",E="Item No ",bb="changeSelection",bc="additive",bd="single",be="one",bf="bold",bg="multi",bh="value";qx.Class.define(a,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var ch=new qx.ui.container.Scroll();var cx=new qx.ui.container.Composite(new qx.ui.layout.Basic());cx.setAllowStretchX(false);ch.add(cx);this.getRoot().add(ch,{edge:0});var cr=new qx.ui.basic.Label(bG);cr.setFont(bf);cx.add(cr,{left:20,top:20});var cz=new qx.ui.form.List;cz.setScrollbarX(t);cz.set({height:280,width:150,selectionMode:bg});var cA;for(var i=1;i<=25;i++){cA=new qx.ui.form.ListItem(E+i,cd+((i%4)?bW:bv)+u);cz.add(cA);!(i%9)&&(cA.setEnabled(false));if(i==20){cz.setSelection([cA]);}
;}
;cz.addListener(bb,function(e){var cG=e.getData();if(cG[0]){this.debug(s+cG[0].getLabel());}
;}
);cx.add(cz,{left:20,top:40});var cn=new qx.ui.form.RadioButton(K);var ck=new qx.ui.form.RadioButton(bT);var cl=new qx.ui.form.RadioButton(j);var cj=new qx.ui.form.RadioButton(M);cn.setUserData(bh,bd);ck.setUserData(bh,bg);cl.setUserData(bh,bc);cj.setUserData(bh,be);ck.setValue(true);cx.add(cn,{left:180,top:40});cx.add(ck,{left:180,top:60});cx.add(cl,{left:180,top:80});cx.add(cj,{left:180,top:100});var cm=new qx.ui.form.RadioGroup(cn,ck,cl,cj);var cD=new qx.ui.form.RadioButton(bs);var cC=new qx.ui.form.RadioButton(bK);var cF=new qx.ui.form.RadioButton(bV);cD.setUserData(bh,B);cC.setUserData(bh,bU);cF.setUserData(bh,o);cF.setValue(true);cx.add(cD,{left:180,top:140});cx.add(cC,{left:180,top:160});cx.add(cF,{left:180,top:180});var cy=new qx.ui.form.RadioGroup(cD,cC,cF);cy.addListener(bb,function(e){for(var i=0;i<cz.getChildren().length;i++){cz.getChildren()[i].setShow(e.getData()[0].getUserData(bh));}
;}
);var cv=new qx.ui.form.CheckBox(bN);var cs=new qx.ui.form.CheckBox(b).set({enabled:false});cx.add(cv,{left:180,top:220});cx.add(cs,{left:180,top:240});cv.addListener(F,function(e){if(e.getData()){var cH=cz.getSelectionMode();if(cH==bd||cH==be){this.debug(bS);}
;}
;cz.setDragSelection(e.getData());}
);cs.addListener(F,function(e){if(e.getData()){var cI=cz.getSelectionMode();if(cI==bg||cI==bc){this.debug(bj);}
;}
;cz.setQuickSelection(e.getData());}
);cm.addListener(bb,function(e){var cJ=e.getData()[0].getUserData(bh);cz.setSelectionMode(cJ);if(cJ==bd||cJ==be){cv.setEnabled(false);cs.setEnabled(true);}
else if(cJ==bg||cJ==ca){cv.setEnabled(true);cs.setEnabled(false);}
;}
);var cE=new qx.ui.basic.Label(d);cE.setFont(bf);cx.add(cE,{left:330,top:20});var cq=new qx.ui.form.List();cq.set({height:280,width:150,selectionMode:be});var cA;for(var i=1;i<=25;i++){cA=new qx.ui.form.ListItem(E+i,bF);cq.add(cA);if(i==16){cq.setSelection([cA]);}
;}
;cx.add(cq,{left:330,top:40});var cr=new qx.ui.basic.Label(y);cr.setFont(bf);cx.add(cr,{left:520,top:20});var ci=new qx.ui.form.List;var co;ci.set({width:150,selectionMode:bc});var cu=[bO,br,S,c,cg,bR,ce,q,cb,bL,z,J,P,bo,H,w,r,O,bk,bi,V,bw,cc,bl,D,bx,l,cf,h,bC,W,N,L,bm,bD,I,U,bI,f,bn,bt,R,bH,bY,G,bq,C,bA,bJ,v,p];for(var i=0;i<cu.length;i++){co=new qx.ui.form.ListItem(cu[i]);ci.add(co);if(i==10||i==12||i==16){ci.addToSelection(co);}
;}
;cx.add(ci,{left:520,top:40});var cr=new qx.ui.basic.Label(x);cr.setFont(bf);cx.add(cr,{left:20,top:350});var cB=new qx.ui.form.List(true);var cp;cB.set({width:550,selectionMode:bg,height:null});var ct=[g,Y,bQ,bz,bM,bE,k,m,X,bP,bB,by,Q,n,bp,T,bX,ba,A];var cw=bu;for(var i=0;i<ct.length;i++){cp=new qx.ui.form.ListItem(null,cw+ct[i]);cB.add(cp);if(i==12){cB.setSelection([cp]);}
;}
