...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.showcase.Translation-qx.theme.Modern.2a6619f41967.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var m="MessageId:",l="<b>Locale and language:</b>",k="new Locale must not be empty string",j="Choose Locale:",h='Territory: ',g=" already exists",f="en",d="Simple Translate: this.tr('Hello World')",c='Locale: ',b="Add new Locale<br> at runtime",bj="en_QX",bi="A comment: 'The mammal fox that is.'<br>is added in the .po file",bh="Translation",bg=" has been added for messageId: ",bf="Singular/Plural Translate: this.trn('I have an apple', I have %1 apples, ",be="changeLocale",bd="qx.debug",bc="Number of apples",bb="show Translation",ba="Locale Name (ex: en_QX)",t='bottom',u="Mark the string 'Orange' for later<br>translation so we do not forget it",r='top',s="demo",p="Add new Locale",q='Territory',n="Simple messageId translation",o=" for locale: ",x="Mark the string 'Orange' for later translation so we do not forget it: this.marktr('Orange')",y=", applesNumber)",G=" has been added",E="<b>Dynamic translation. This is added at runtime.</b>",O="<b>Static translation [found in .po files]. Hover the labels below to see more info.</b>",J="Add Translation",V='Language',T="Singular/Plural Translate<br>Choose a number from the select box<br>to see plural translation",A="with .po comment: this.trc('The mammal fox that is.', 'The quick brown fox')",Y='Language: ',X="<i>MessageId</i>",W="Number of apples:",z='Locale',C="click 'Show Translation' button to translate<br>the messageId in the current selected locale",D="<i>Translated Text</i>",F="demobrowser.demo.showcase.Translation",H="MessageId",K="The mammal fox that is.",Q="changeSelection",U="Add translation<br>at runtime for: <b>",v="</b>",w="I have %1 apples",B='right',N="The quick brown fox",M="Orange",L="execute",S="Hello World",R="I have an apple",I="value",P="",a='middle';qx.Class.define(F,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);if((qx.core.Environment.get(bd))){qx.log.appender.Native;qx.log.appender.Console;}
;var bz=new qx.ui.container.Composite(new qx.ui.layout.Grid(8,8));this.getRoot().add(bz,{top:20,left:20});var bF=new qx.ui.basic.Label(l);bF.setRich(true);bz.add(bF,{row:0,column:0,colSpan:3});var bY=new qx.ui.basic.Label(j);bY.setMarginTop(5);bY.setAlignY(a);bz.add(bY,{row:1,column:0});var bH=qx.locale.Manager.getInstance();var bQ=bH.getAvailableLocales().sort();var bx=bH.getLocale();var bw=new qx.ui.form.SelectBox();bw.setAllowGrowY(false);bw.setAlignY(a);var bG=null;for(var i=0;i<bQ.length;i++){var bN=new qx.ui.form.ListItem(bQ[i]);bw.add(bN);if((!bG&&bQ[i]==f)||bQ[i]==bx){bG=bN;}
;}
;bw.addListener(Q,function(e){var cd=e.getData()[0].getLabel();qx.locale.Manager.getInstance().setLocale(cd);}
);bz.add(bw,{row:1,column:1});var ca=new qx.ui.basic.Label(z);ca.setMarginTop(5);ca.setWidth(80);var bW=new qx.ui.basic.Label(q);bW.setMarginTop(5);bW.setWidth(80);var bX=new qx.ui.basic.Label(V);bX.setMarginTop(5);bX.setWidth(80);var br=new qx.ui.container.Composite(new qx.ui.layout.VBox(3));br.add(ca);br.add(bW);br.add(bX);bz.add(br,{row:1,column:2});qx.locale.Manager.getInstance().addListener(be,function(){var ce=qx.locale.Manager.getInstance();ca.setValue(c+ce.getLocale());bW.setValue(h+ce.getTerritory());bX.setValue(Y+ce.getLanguage());bk.apply(this);cc.setValue(U+bH.getLocale()+v);}
,this);function bk(){bt.setValue(this.tr(S));bt.setToolTipText(d);var cf=parseInt(bU.getSelection()[0].getLabel());bs.setValue(this.trn(R,w,cf,cf));bs.setToolTipText(bf+cf+y);bJ.setValue(this.trc(K,N));bJ.setToolTipText(A);bq.setValue(this.marktr(M));bq.setToolTipText(x);var cg=bS.getValue();if(cg&&cg.length>0){bI.setValue(bH.translate(cg,[],bH.getLocale()));}
;}
;var bT=new qx.ui.basic.Label(O);bT.setRich(true);bT.setMarginTop(15);bz.add(bT,{row:2,column:0,colSpan:4});var bl=new qx.ui.basic.Label(X);bl.setRich(true);bz.add(bl,{row:3,column:1});var bE=new qx.ui.basic.Label(D);bE.setRich(true);bz.add(bE,{row:3,column:2});var bL=new qx.ui.basic.Label(S);bz.add(bL,{row:4,column:1});var bt=new qx.ui.basic.Label(this.tr(S));bz.add(bt,{row:4,column:2});var bV=new qx.ui.basic.Label(n);bz.add(bV,{row:4,column:3});var bv=new qx.ui.basic.Label(W);bv.setAlignX(B);bv.setAlignY(t);bz.add(bv,{row:5,column:0});var bU=new qx.ui.form.SelectBox();for(var i=1;i<5;i++){var bu=new qx.ui.form.ListItem(i+P);bU.add(bu);}
;bU.setToolTipText(bc);bU.setAlignY(r);bU.setAlignX(B);bz.add(bU,{row:6,column:0});var bm=new qx.ui.basic.Label(R);bm.setAlignY(a);bz.add(bm,{row:5,column:1,rowSpan:2});var bs=new qx.ui.basic.Label(this.trn(R,w,1,1));bs.setAlignY(a);bz.add(bs,{row:5,column:2,rowSpan:2});var bP=new qx.ui.basic.Label(T);bP.setAlignY(a);bP.setRich(true);bz.add(bP,{row:5,column:3,rowSpan:2});bU.addListener(Q,function(e){bk.apply(this);}
);var by=new qx.ui.basic.Label(N);bz.add(by,{row:7,column:1});var bJ=new qx.ui.basic.Label(this.trc(K,N));bJ.setWidth(140);bz.add(bJ,{row:7,column:2});var bM=new qx.ui.basic.Label(bi);bM.setRich(true);bz.add(bM,{row:7,column:3});var bB=new qx.ui.basic.Label(M);bz.add(bB,{row:8,column:1});var bq=new qx.ui.basic.Label(this.marktr(M));bz.add(bq,{row:8,column:2});var bA=new qx.ui.basic.Label(u);bA.setRich(true);bz.add(bA,{row:8,column:3});var bo=new qx.ui.basic.Label(E);bo.setRich(true);bo.setMarginTop(15);bz.add(bo,{row:9,column:0,colSpan:3});var cc=new qx.ui.basic.Label(U+bH.getLocale()+v);cc.setRich(true);cc.setWidth(120);bz.add(cc,{row:10,column:0});var bn=new qx.ui.form.TextField(P);bn.setToolTipText(H);bz.add(bn,{row:10,column:1});var bK=new qx.ui.form.TextField(P);bK.setToolTipText(bh);bz.add(bK,{row:10,column:2});var bC=new qx.ui.form.Button(J);bC.setAllowGrowY(false);bz.add(bC,{row:10,column:3});bC.addListener(L,function(e){var ch={};ch[bn.getValue()]=bK.getValue();bH.addTranslation(bH.getLocale(),ch);alert(bK.getValue()+bg+bn.getValue()+o+bH.getLocale());}
,this);var bD=new qx.ui.basic.Label(m);bz.add(bD,{row:11,column:0});var bS=new qx.ui.form.TextField(P);bS.setToolTipText(H);bz.add(bS,{row:11,column:1});var bR=new qx.ui.form.Button(bb);bz.add(bR,{row:11,column:2});bR.setAllowGrowY(false);bR.addListener(L,function(e){var ci=bS.getValue();bI.setValue(bH.translate(ci,[],bH.getLocale()));}
,this);var bI=new qx.ui.basic.Label(C);bI.setRich(true);bI.setAlignY(a);bz.add(bI,{row:11,column:3});var cb=new qx.ui.basic.Label(b);cb.setRich(true);bz.add(cb,{row:12,column:0});var bp=new qx.ui.form.TextField(P);bp.setPlaceholder(bj);bp.setToolTipText(ba);bz.add(bp,{row:12,column:1});var bO=new qx.ui.form.Button(p);bO.setAllowGrowY(false);bz.add(bO,{row:12,column:2});bO.addListener(L,function(e){var cj=bp.getValue();if(cj!=null&&cj.length<1){alert(k);return;}
;var ck=bH.getAvailableLocales();if(ck.indexOf(cj)!=-1){alert(cj+g);}
else {bH.addTranslation(cj,{"demo":s});alert(cj+G);bw.add(new qx.ui.form.ListItem(cj));}
;}
,this);if(bG){bw.setSelection([bG]);}
;bn.bind(I,bS,I);bS.bind(I,bn,I);return;}
}});}
)();
