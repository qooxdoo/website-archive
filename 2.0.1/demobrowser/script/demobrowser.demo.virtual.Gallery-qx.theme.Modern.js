...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.virtual.Gallery","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Gallery-qx.theme.Modern.c43926975e62.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="Gallery (HTML - divs)",k="manager",j="default",h="demobrowser.demo.virtual.Gallery",g="demobrowser.demo.virtual.WidgetGallery",f="text-align: center;",d="auto",c="color: white; background-color: #00398D;",b="user-desktop.png",a="<img src='",P="__Rq",O="group",N="'></img>",M="resize",L="folder.png",K="vertical",J="multi",I="network-server.png",H="user-trash.png",G="off",s="top",t="icon/32/places/",q="horizontal",r="demobrowser.demo.virtual.AbstractGallery",o="abstract",p="position: absolute;",m="demobrowser.demo.virtual.HtmlGallery",n="Gallery (widgets)",u="<br>",v="Icon #",y="network-workgroup.png",x="text-selected",A="demobrowser.demo.virtual.GalleryCell",z="selected",C="cell",B="layer",w="icon",F="#f3f3f3",E="label",D="";qx.Class.define(h,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);new demobrowser.demo.virtual.WidgetGallery(n);var Q=new demobrowser.demo.virtual.HtmlGallery(l);Q.moveTo(400,50);}
}});qx.Class.define(r,{extend:qx.ui.window.Window,type:o,construct:function(R){qx.ui.window.Window.call(this,R);this.set({contentPadding:0,showClose:false,showMinimize:false,width:300,height:400});this.setLayout(new qx.ui.layout.Grow());this.moveTo(30,50);this.open();this.itemHeight=60;this.itemWidth=60;this.itemCount=431;this.itemPerLine=1;this.items=this._generateItems(this.itemCount);var S=this._createScroller();S.set({scrollbarX:G,scrollbarY:d});S.getPane().addListener(M,this._onPaneResize,this);this.add(S);this.manager=new qx.ui.virtual.selection.CellRectangle(S.getPane(),this).set({mode:J,drag:true});this.manager.attachMouseEvents();this.manager.attachKeyEvents(S);}
,members:{getItemData:function(T,U){return this.items[T*this.itemPerLine+U];}
,_createScroller:function(){}
,isItemSelectable:function(V){return !!this.getItemData(V.row,V.column);}
,styleSelectable:function(W,X,Y){}
,_onPaneResize:function(e){var bc=e.getTarget();var ba=e.getData().width;var bb=Math.floor(ba/this.itemWidth);if(bb==this.itemsPerLine){return;}
;this.itemPerLine=bb;var bd=Math.ceil(this.itemCount/bb);bc.getColumnConfig().setItemCount(bb);bc.getRowConfig().setItemCount(bd);}
,_generateItems:function(be){var bh=[];var bk=[L,H,I,y,b];var bi=qx.util.AliasManager.getInstance();var bg=qx.util.ResourceManager.getInstance();for(var i=0;i<be;i++){var bl=t+bk[Math.floor(Math.random()*bk.length)];var bf=bi.resolve(bl);var bj=bg.toUri(bf);bh[i]={label:v+(i+1),icon:bl,resolvedIcon:bj};}
;return bh;}
},destruct:function(){this.items=null;this._disposeObjects(k);}
});qx.Class.define(A,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var bm=new qx.ui.basic.Atom().set({iconPosition:s});bm.getChildControl(E).set({padding:[0,4]});bm.getChildControl(w).set({padding:4});return bm;}
,updateData:function(bn,bo){bn.set({icon:bo.icon,label:bo.label});}
,updateStates:function(bp,bq){var bs=bp.getChildControl(E);var br=bp.getChildControl(w);if(bq.selected){bs.setDecorator(z);bs.setTextColor(x);br.setDecorator(O);}
else {bs.resetDecorator();bs.resetTextColor();br.resetDecorator();}
;}
}});qx.Class.define(g,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bt){demobrowser.demo.virtual.AbstractGallery.call(this,bt);this.__Rq=new demobrowser.demo.virtual.GalleryCell();}
,members:{_createScroller:function(){var bu=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.WidgetCell(this);bu.getPane().addLayer(this.layer);new qx.ui.virtual.behavior.Prefetch(bu,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:200,maxAbove:300,minBelow:600,maxBelow:800}).set({interval:500});return bu;}
,styleSelectable:function(bv,bw,bx){if(bw!==z){return;}
;var by=this.layer.getChildren();for(var i=0;i<by.length;i++){var bz=by[i];var bA=bz.getUserData(C);if(bv.row!==bA.row||bv.column!==bA.column){continue;}
;if(bx){this.__Rq.updateStates(bz,{selected:1});}
else {this.__Rq.updateStates(bz,{});}
;}
;}
,getCellWidget:function(bB,bC){var bG=this.getItemData(bB,bC);if(!bG){return null;}
;var bD={row:bB,column:bC};var bE={};if(this.manager.isItemSelected(bD)){bE.selected=true;}
;var bF=this.__Rq.getCellWidget(bG,bE);bF.setUserData(C,bD);return bF;}
,poolCellWidget:function(bH){this.__Rq.pool(bH);}
},destruct:function(){this._disposeObjects(P,B);}
});qx.Class.define(m,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(bI){demobrowser.demo.virtual.AbstractGallery.call(this,bI);var bJ=qx.theme.manager.Font.getInstance().resolve(j).getStyles();this._fontCss=qx.bom.element.Style.compile(bJ);}
,members:{_createScroller:function(){var bL=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);this.layer=new qx.ui.virtual.layer.HtmlCell(this);bL.getPane().addLayer(this.layer);var bK=new qx.ui.virtual.layer.GridLines(q,F);bL.getPane().addLayer(bK);var bK=new qx.ui.virtual.layer.GridLines(K,F);bL.getPane().addLayer(bK);return bL;}
...
,getCellProperties:function(bP,bQ){var bT=this.getItemData(bP,bQ);if(!bT){return D;}
;var bS=this.manager.isItemSelected({row:bP,column:bQ});var bR=bS?c:D;return {style:[p,f,this._fontCss,bR].join(D),content:[a,bT.resolvedIcon,N,u,bT.label].join(D)};}
},destruct:function(){this._disposeObjects(B);}
});}
)();
