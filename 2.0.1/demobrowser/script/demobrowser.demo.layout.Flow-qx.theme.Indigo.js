...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Flow-qx.theme.Indigo.3109caf430db.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var o="lineBreak",n="' is not supported by the flow layout!",m="middle",l="qx.debug",k="bottom",j="qx.ui.layout.Flow",h="right",g="The property '",f="Boolean",e="center",b="top",d="Integer",c="left",a="_applyLayoutChange";qx.Class.define(j,{extend:qx.ui.layout.Abstract,construct:function(p,q,r){qx.ui.layout.Abstract.call(this);if(p){this.setSpacingX(p);}
;if(q){this.setSpacingY(q);}
;if(r){this.setAlignX(r);}
;}
,properties:{alignX:{check:[c,e,h],init:c,apply:a},alignY:{check:[b,m,k],init:b,apply:a},spacingX:{check:d,init:0,apply:a},spacingY:{check:d,init:0,apply:a},reversed:{check:f,init:false,apply:a}},members:{verifyLayoutProperty:qx.core.Environment.select(l,{"true":function(s,name,t){this.assertEquals(o,name,g+name+n);}
,"false":null}),connectToWidget:function(u){qx.ui.layout.Abstract.prototype.connectToWidget.call(this,u);u.setAllowShrinkY(false);}
,renderLayout:function(v,w){var A=this._getLayoutChildren();if(this.getReversed()){A=A.concat().reverse();}
;var x=new qx.ui.layout.LineSizeIterator(A,this.getSpacingX());var y=0;while(x.hasMoreLines()){var z=x.computeNextLine(v);this.__Ll(z,y,v);y+=z.height+this.getSpacingY();}
;}
,__Ll:function(B,C,D){var F=qx.ui.layout.Util;var I=0;if(this.getAlignX()!=c){I=D-B.width;if(this.getAlignX()==e){I=Math.round(I/2);}
;}
...
;}
,_computeSizeHint:function(){return this.__Lm(Infinity);}
,hasHeightForWidth:function(){return true;}
,getHeightForWidth:function(K){return this.__Lm(K).height;}
,__Lm:function(L){var M=new qx.ui.layout.LineSizeIterator(this._getLayoutChildren(),this.getSpacingX());var Q=0;var N=0;var O=0;while(M.hasMoreLines()){var P=M.computeNextLine(L);O+=1;N=Math.max(N,P.width);Q+=P.height;}
;return {width:N,height:Q+this.getSpacingY()*(O-1)};}
}});}
)();
(function(){var b="No more lines to compute",a="qx.ui.layout.LineSizeIterator";qx.Class.define(a,{extend:Object,construct:function(c,d){this.__dW=c;this.__JO=d;this.__JP=c.length>0;this.__JQ=0;}
,members:{__dW:null,__JO:null,__JP:null,__JQ:null,computeNextLine:function(e){var e=e||Infinity;if(!this.__JP){throw new Error(b);}
;var p=this.__dW;var n=0;var g=0;var o=[];var k=[];for(var i=this.__JQ;i<p.length;i++){var m=p[i];var q=m.getSizeHint();var j=this.__JR(i);var h=q.width+j;var f=i==this.__JQ;if(!f&&g+h>e){this.__JQ=i;break;}
;var l=q.height+m.getMarginTop()+m.getMarginBottom();o.push(m);k.push(j);g+=h;n=Math.max(n,l);if(m.getLayoutProperties().lineBreak){this.__JQ=i+1;break;}
;}
;if(i>=p.length){this.__JP=false;}
;return {height:n,width:g,children:o,gapsBefore:k};}
,__JR:function(r){var s=r==this.__JQ;if(s){return this.__dW[r].getMarginLeft();}
else {return Math.max(this.__dW[r-1].getMarginRight(),this.__dW[r].getMarginLeft(),this.__JO);}
;}
,hasMoreLines:function(){return this.__JP;}
}});}
)();
