...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.table.Table_Cell_Editor","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Indigo"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.table.Table_Cell_Editor-qx.theme.Indigo.d58c2375c506.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j='Use only numbers!',i="You did not enter a valid email address",h='[0-9]+',g='Doubleclick on cell to edit value',f="' and entered value '",e='newsletter',d='admin',c='editable combobox',b='secret',a="demobrowser.demo.table.Table_Cell_Editor",bf='status',be="validationFunc",bd='1',bc="regExp",bb='regExp',ba="You need to supply a value here",Y='Newsletter',X='Inactive',W='User Name',V='telephone',q="dataChanged",r='Status',o='Active',p="required",m='validationFunc',n='enter website',k="editable",l='role',u='failMsg',v='username',D='jdoe',B='Telephone',L="'.",G='manager',R='enter valid telephone number',P='Waiting',x='Email',U='Role',T="Property editor table",S='2',w='user',z='website',A='Password',C="email",E='Website',H="User edited property '",M='enter valid email',Q='Property',s="options",t="password",y="type",K='email',J='password',I='options',O='type',N='0',F="checkbox";qx.Class.define(a,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return T;}
,main:function(){demobrowser.demo.table.TableDemo.prototype.main.call(this);this._container.resetWidth();this._container.setResizable(false);}
,createTable:function(){var bo=[[v,W,D,{'required':true}],[J,A,b,{'type':J,'required':true}],[l,U,c,{'options':[d,G,w],'editable':true}],[bf,r,N,{'options':[[X,null,N],[o,null,bd],[P,null,S]]}],[K,x,M,{'type':K}],[V,B,R,{'regExp':h,'failMsg':j}],[z,E,n,{'validationFunc':function(bp){if(bp.search(/^http:/)!=-1){window.open(bp);}
;return bp;}
}],[e,Y,true,{'type':F}]];var bn=function(bq){var bw=bq.table;var bt=bw.getTableModel();var bu=bt.getRowData(bq.row);var bx=bu[3];var bv;for(var bs in bx){switch(bs){case y:switch(bx[O]){case F:return new qx.ui.table.cellrenderer.Boolean;case t:return new qx.ui.table.cellrenderer.Password;};break;case s:var bv=new qx.ui.table.cellrenderer.Replace;var br={};bx[I].forEach(function(by){if(by instanceof Array){br[by[0]]=by[2];}
;}
);bv.setReplaceMap(br);bv.addReversedReplaceMap();return bv;};}
;return new qx.ui.table.cellrenderer.Default();}
;var bm=new qx.ui.table.cellrenderer.Dynamic(bn);var bk=function(bz){var bE=bz.table;var bC=bE.getTableModel();var bD=bC.getRowData(bz.row);var bG=bD[3];var bB=new qx.ui.table.celleditor.TextField;var bF=null;for(var bA in bG){switch(bA){case s:if(bG.editable){bB=new qx.ui.table.celleditor.ComboBox();}
else {bB=new qx.ui.table.celleditor.SelectBox();}
;bB.setListData(bG[I]);break;case k:break;case y:switch(bG[O]){case t:bB=new qx.ui.table.celleditor.PasswordField;break;case F:bB=new qx.ui.table.celleditor.CheckBox;break;case C:bB.setValidationFunction(function(bH,bI){var bJ=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{2}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/;if(bJ.test(bH)){return bH;}
;alert(i);return bI;}
);break;};break;case bc:bB.setValidationFunction(function(bK,bL){var bM=new RegExp(bG[bb]);if(bM.test(bK)){return bK;}
;alert(bG[u]);return bL;}
);break;case be:bB.setValidationFunction(bG[m]);break;case p:bF=function(bN,bO){if(!bN){alert(ba);return bO;}
;return bN;}
;break;};}
;return bB;}
;var bj=new qx.ui.table.celleditor.Dynamic(bk);var bh=new qx.ui.table.model.Simple();bh.setColumns([Q,Q,g]);var bg={tableColumnModel:function(bP){return new qx.ui.table.columnmodel.Resize(bP);}
};var bi=new qx.ui.table.Table(bh,bg);bi.setDecorator(null);bi.setColumnVisibilityButtonVisible(false);bi.setKeepFirstVisibleRowComplete(true);bi.setStatusBarVisible(false);bi.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);var bl=bi.getTableColumnModel();bl.setColumnVisible(0,false);bl.getBehavior().setWidth(1,100);bl.getBehavior().setWidth(2,300);bh.setColumnEditable(2,true);bl.setDataCellRenderer(2,bm);bl.setCellEditorFactory(2,bj);bi.getTableModel().setData(bo);bi.getTableModel().addListener(q,function(event){if(!(event instanceof qx.event.type.Data)){return;}
;var bT=event.getData();var bQ=this.getTableModel();var bS=bQ.getValue(0,bT.firstRow);var bR=bQ.getValue(bT.firstColumn,bT.firstRow);this.info(H+bS+f+bR+L);}
,bi);return bi;}
}});}
)();
...
,getCellEditorValue:function(t){var u=t.getSelection();var w=b;if(u&&u[0]){w=u[0].getUserData(a)||u[0].getLabel();}
;var v=this.getValidationFunction();if(v){w=v(w,t.originalValue);}
;if(typeof t.originalValue==f){w=parseFloat(w);}
;return w;}
}});}
)();
...
;this.__MJ={};}
,properties:{cellEditorFactoryFunction:{check:d,nullable:true,init:null}},members:{__pv:null,__MJ:null,createCellEditor:function(f){var g=this.getCellEditorFactoryFunction();if(qx.core.Environment.get(a)){this.assertFunction(g,b);}
;this.__pv=g(f);var h=this.__pv.createCellEditor(f);this.__MJ[h.toHashCode()]=f;return h;}
...
;var j=this.__MJ[i.toHashCode()];this.__pv=k(j);var l=this.__pv.getCellEditorValue(i);return l;}
