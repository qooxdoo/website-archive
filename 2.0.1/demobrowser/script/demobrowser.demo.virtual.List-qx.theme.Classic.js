...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.List-qx.theme.Classic.5be100125a5c.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var ci=qx.data.marshal.Json.createModel(cf);var ca=this.__Rt=new qx.ui.list.List(ci).set({scrollbarX:A,selectionMode:Q,height:280,width:150,labelPath:x,iconPath:R,iconOptions:{converter:function(cm){return h+cm+H;}
...
,onSelectionModeChange:function(e){var cF=e.getData()[0].getUserData(w);this.__Rt.setSelectionMode(cF);if(cF==K||cF==L){this.__Ru.setEnabled(false);this.__Rv.setEnabled(true);}
else if(cF==Q||cF==c){this.__Ru.setEnabled(true);this.__Rv.setEnabled(false);}
;}
,onShowModeChange:function(e){var cG=e.getData()[0].getUserData(w);this.setShowMode(cG);}
,onDragCheckChange:function(e){if(e.getData()){var cH=this.__Rt.getSelectionMode();if(cH==K||cH==L){this.debug(bg);}
;}
;this.__Rt.setDragSelection(e.getData());}
,onQuickCheckChange:function(e){if(e.getData()){var cI=this.__Rt.getSelectionMode();if(cI==Q||cI==P){this.debug(S);}
;}
;this.__Rt.setQuickSelection(e.getData());}
}});}
)();
