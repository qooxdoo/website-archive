...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.Finder","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Finder-qx.theme.Classic.a6e6c620b14b.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="Finder",f="model.files",e="demobrowser/demo/data/finder.json",d="demobrowser.demo.data.Finder",c="selection[0].files",b="name",a="model";qx.Class.define(d,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var i=new qx.ui.window.Window(g);i.setLayout(new qx.ui.layout.HBox());i.setContentPadding(0);i.setShowMaximize(false);i.setShowMinimize(false);i.setShowClose(false);i.open();var j=new qx.ui.form.List();i.add(j);var l=new qx.ui.form.List();i.add(l);var k=new qx.ui.form.List();i.add(k);var o=new qx.data.controller.List(null,j);o.setLabelPath(b);var n=new qx.data.controller.List(null,l);n.setLabelPath(b);var m=new qx.data.controller.List(null,k);m.setLabelPath(b);var h=qx.util.ResourceManager.getInstance().toUri(e);var p=new qx.data.store.Json(h);p.bind(f,o,a);o.bind(c,n,a);n.bind(c,m,a);}
}});}
)();
...
;if(this.getSelection()==null){this.__Br=new qx.data.Array();this.setSelection(this.__Br);}
;}
,properties:{selection:{check:f,event:a,apply:d,init:null}},members:{_modifingSelection:0,__Bs:null,__Bt:null,__Br:null,_applySelection:function(h,j){if(this.__Bt!=undefined&&j!=undefined){j.removeListenerById(this.__Bt);}
;this.__Bt=h.addListener(g,this.__Bu,this);this._updateSelection();}
,__Bu:function(){this._updateSelection();}
,_changeTargetSelection:function(){if(this.getTarget()==null){return;}
;if(!this.__Bv()&&!this.__Bw()){return;}
;if(this._inSelectionModification()){return;}
;var l=this.getTarget().getSelection();var k=this.getSelection();if(k==null){k=new qx.data.Array();this.__Br=k;this.setSelection(k);}
...
,_addChangeTargetListener:function(n,o){if(this.__Bs!=undefined&&o!=undefined){o.removeListenerById(this.__Bs);}
;if(n!=null){if(this.__Bv()||this.__Bw()){this.__Bs=n.addListener(a,this._changeTargetSelection,this);}
;}
;}
,_updateSelection:function(){if(!this.getTarget()){return;}
;this._startSelectionModification();if(this.__Bv()){var r=[];for(var i=0;i<this.getSelection().length;i++){var s=this.getSelection().getItem(i);var t=this.__Bx(s);if(t!=null){r.push(t);}
;}
...
;}
;}
else if(this.__Bw()){var q=this.getSelection().getItem(this.getSelection().length-1);if(q!==undefined){this.__rD(q);this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();}
else {this.getTarget().resetSelection();}
;}
;this._endSelectionModification();}
,__Bv:function(){var u=this.getTarget().constructor;return qx.Class.implementsInterface(u,qx.ui.core.IMultiSelection);}
,__Bw:function(){var v=this.getTarget().constructor;return qx.Class.implementsInterface(v,qx.ui.core.ISingleSelection);}
,__rD:function(w){var x=this.__Bx(w);if(x==null){return;}
;if(this.__Bv()){this.getTarget().addToSelection(x);}
else if(this.__Bw()){this.getTarget().setSelection([x]);}
;}
,__Bx:function(y){var z=this.getTarget().getSelectables(true);for(var i=0;i<z.length;i++){if(z[i].getModel()==y){return z[i];}
;}
;return null;}
...
},destruct:function(){if(this.__Br){this.__Br.dispose();}
;}
});}
)();
...
(function(){var m="changeModel",k="_applyLabelOptions",h="_applyLabelPath",g="changeTarget",f="_applyModel",e="icon",d="qx.data.controller.List",c="_applyIconPath",b="_applyDelegate",a="changeDelegate",z="_applyTarget",y="qx.data.IListData",x="model",w="label",v="_applyIconOptions",u="change",t="ReverseBindingId",s="BindingId",r="]",q="model[",o="String",p=".",n="";qx.Class.define(d,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(A,B,C){qx.core.Object.call(this);this.__zy=[];this.__Bd=[];this.__Be=[];this.__Bf={};if(C!=null){this.setLabelPath(C);}
;if(A!=null){this.setModel(A);}
;if(B!=null){this.setTarget(B);}
;}
,properties:{model:{check:y,apply:f,event:m,nullable:true,dereference:true},target:{apply:z,event:g,nullable:true,init:null,dereference:true},labelPath:{check:o,apply:h,nullable:true},iconPath:{check:o,apply:c,nullable:true},labelOptions:{apply:k,nullable:true},iconOptions:{apply:v,nullable:true},delegate:{apply:b,event:a,init:null,nullable:true}},members:{__Bg:null,__zy:null,__Bf:null,__Bd:null,__Be:null,__Bh:null,__Bi:null,update:function(){this.__Bk();this.__Bo();this._updateSelection();}
,_applyDelegate:function(D,E){this._setConfigureItem(D,E);this._setFilter(D,E);this._setCreateItem(D,E);this._setBindItem(D,E);}
,_applyIconOptions:function(F,G){this.__Bo();}
,_applyLabelOptions:function(H,I){this.__Bo();}
,_applyIconPath:function(J,K){this.__Bo();}
,_applyLabelPath:function(L,M){this.__Bo();}
,_applyModel:function(N,O){if(O!=undefined){if(this.__Bg!=undefined){O.removeListenerById(this.__Bg);}
;}
;if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();}
;if(N!=null){this.__Bg=N.addListener(u,this.__Bj,this);this.__zF();this.__Bk();if(O==null){this._changeTargetSelection();}
else {this.__Bh=true;qx.ui.core.queue.Widget.add(this);}
;}
else {var P=this.getTarget();if(P!=null){var length=P.getChildren().length;for(var i=0;i<length;i++){this.__Bn();}
;}
;}
;}
...
;if(Q!=null){if(this.getModel()!=null){for(var i=0;i<this.__zy.length;i++){this.__Bm(this.__Bq(i));}
;}
;}
;}
,__Bj:function(){this.__Bi=true;qx.ui.core.queue.Widget.add(this);if(this.__zy.length!=this.getModel().getLength()){this.update();}
;}
,syncWidget:function(){if(this.__Bh){this._changeTargetSelection();}
;if(this.__Bi){this._updateSelection();}
;this.__Bi=this.__Bh=null;}
,__Bk:function(){if(this.getTarget()==null){return;}
;this.__zF();var U=this.__zy.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++){this.__Bm(this.__Bq(j));}
;}
else if(U<T){for(var j=T;j>U;j--){this.__Bn();}
;}
;this.__Bh=true;qx.ui.core.queue.Widget.add(this);}
,__Bl:function(){var V=this.getModel();if(V!=null){V.removeListenerById(this.__Bg);this.__Bg=V.addListener(u,this.__Bj,this);}
;}
...
;return W;}
,__Bm:function(Y){var ba=this._createItem();this._bindListItem(ba,Y);this.getTarget().add(ba);}
,__Bn:function(){this._startSelectionModification();var bc=this.getTarget().getChildren();var bb=bc.length-1;var bd=bc[bb];this._removeBindingsFrom(bd);this.getTarget().removeAt(bb);bd.destroy();this._endSelectionModification();}
...
;}
,bindProperty:function(bm,bn,bo,bp,bq){if(bo!=null){var bo=qx.lang.Object.clone(bo);this.__Bf[bn]=bo.onUpdate;delete bo.onUpdate;}
else {bo={};this.__Bf[bn]=null;}
;bo.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,bq);var br=q+bq+r;if(bm!=null&&bm!=n){br+=p+bm;}
;var bs=this.bind(br,bp,bn,bo);bp.setUserData(bn+s,bs);if(!qx.lang.Array.contains(this.__Bd,bn)){this.__Bd.push(bn);}
;}
,bindPropertyReverse:function(bt,bu,bv,bw,bx){var by=q+bx+r;if(bt!=null&&bt!=n){by+=p+bt;}
;var bz=bw.bind(bu,this,by,bv);bw.setUserData(bt+t,bz);if(!qx.lang.Array.contains(this.__Be,bt)){this.__Be.push(bt);}
;}
,_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;}
;for(var i=0;i<this.__Bd.length;i++){if(this.__Bf[this.__Bd[i]]!=null){this.__Bf[this.__Bd[i]]();}
;}
;}
,_removeBindingsFrom:function(bD){for(var i=0;i<this.__Bd.length;i++){var bE=bD.getUserData(this.__Bd[i]+s);if(bE!=null){this.removeBinding(bE);}
;}
;for(var i=0;i<this.__Be.length;i++){var bE=bD.getUserData(this.__Be[i]+t);if(bE!=null){bD.removeBinding(bE);}
;}
;}
,__Bo:function(){if(this.getTarget()==null||this.getModel()==null){return;}
;var bF=this.getTarget().getChildren();for(var i=0;i<bF.length;i++){this._removeBindingsFrom(bF[i]);this._bindListItem(bF[i],this.__Bq(i));}
;this.__Bl();}
...
,_setFilter:function(bP,bQ){if((bP==null||bP.filter==null)&&(bQ!=null&&bQ.filter!=null)){this.__Bp();}
...
;var bS=this.__zy;this.__zF();if(bS.length>this.__zy.length){for(var j=bS.length;j>this.__zy.length;j--){this.getTarget().removeAt(j-1).destroy();}
;}
else if(bS.length<this.__zy.length){for(var j=bS.length;j<this.__zy.length;j++){var bT=this._createItem();this.getTarget().add(bT);}
;}
;var bR=this.getTarget().getChildren();for(var i=0;i<bR.length;i++){this._bindListItem(bR[i],this.__Bq(i));}
;this.__Bl();this._endSelectionModification();this._updateSelection();}
,__Bp:function(){this.__zF();this.__Bk();this.__Bo();this.__Bi=true;qx.ui.core.queue.Widget.add(this);}
,__zF:function(){var bW=this.getModel();if(bW==null){return;}
;var bX=this.getDelegate();if(bX!=null){var bV=bX.filter;}
;this.__zy=[];for(var i=0;i<bW.getLength();i++){if(bV==null||bV(bW.getItem(i))){this.__zy.push(i);}
;}
;}
,__Bq:function(bY){return this.__zy[bY];}
},destruct:function(){this.__zy=this.__Bf=this.__Bd=null;this.__Be=null;qx.ui.core.queue.Widget.remove(this);}
});}
)();
