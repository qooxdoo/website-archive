...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.data.SingleValueBinding","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Simple"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SingleValueBinding-qx.theme.Simple.d1cade0b88cc.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="demobrowser.demo.data.SingleValueBinding",j="Reverse",i="Binding a number value (slider value) to a labels string (content).",h="child.names[last]",g="child.names[0]",f="First:",d="Bigger than 50!",c="of names.<br/>The reverse button reverses the current selected child ",b=" The default conversion will handle the type conversion.",a="Binding an data event, containing the change of the textfield to",O="Last:",N="Binding the textfield to a property with a validation. The validator",M=" in the object only accepts numbers. On a validation fail, a red ",L=" the label.",K="No number!",J="With validation",I="10",H="containing ['Bart', 'Lisa'] as names.",G=" above or below 50.",F="Bart",r="number",s="Toggle child",p="from a node containing ['Homer', 'Marge'] as names to a node ",q="Binding of node elements containing a node as child and an array ",n="With own conversion",o="names array.<br/>Toggle child will change the child of the root node ",l="with a self written converter to a string only saying it its ",m="Event binding",t="Array binding",u="Lower than 50!",x="Lisa",w="Binding a slider value to a labels text but conversion the number ",z="With default conversion",y=" border will surround the textfield.",B="changeValue",A="execute",v="Change me...",E="",D="<b>Description</b><br/>",C="value";qx.Class.define(k,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);this.getRoot().add(new qx.ui.basic.Label(m),{top:10,left:10});var bd=new qx.ui.form.TextField(v);bd.setLiveUpdate(true);this.getRoot().add(bd,{top:30,left:10});var W=new qx.ui.basic.Label(v);this.getRoot().add(W,{top:32,left:120});bd.bind(B,W,C);this.getRoot().add(new qx.ui.basic.Label(z),{top:80,left:10});var bc=new qx.ui.form.Slider();bc.setWidth(100);this.getRoot().add(bc,{top:100,left:10});var R=new qx.ui.basic.Label(E);this.getRoot().add(R,{top:100,left:120});bc.bind(C,R,C);this.getRoot().add(new qx.ui.basic.Label(n),{top:150,left:10});var T=new qx.ui.form.Slider();T.setWidth(100);this.getRoot().add(T,{top:180,left:10});var bi=new qx.ui.basic.Label(E);this.getRoot().add(bi,{top:180,left:120});var X={converter:function(bl){if(bl<50){return u;}
else {return d;}
;}
};T.bind(C,bi,C,X);this.getRoot().add(new qx.ui.basic.Label(t),{top:240,left:10});var bb=new demobrowser.demo.data.model.Node();var be=new demobrowser.demo.data.model.Node();var bh=new demobrowser.demo.data.model.Node();bh.setNames(new qx.data.Array(F,x));bb.setChild(be);var bk=new qx.ui.form.Button(j);this.getRoot().add(bk,{top:260,left:10});bk.addListener(A,function(){bb.getChild().getNames().reverse();}
,this);var bg=new qx.ui.form.Button(s);this.getRoot().add(bg,{top:260,left:80});bg.addListener(A,function(){if(bb.getChild()==be){bb.setChild(bh);}
else {bb.setChild(be);}
;}
,this);this.getRoot().add(new qx.ui.basic.Label(f),{left:10,top:295});this.getRoot().add(new qx.ui.basic.Label(O),{left:60,top:295});var bj=new qx.ui.basic.Label();this.getRoot().add(bj,{top:310,left:10});var ba=new qx.ui.basic.Label();this.getRoot().add(ba,{top:310,left:60});bb.bind(g,bj,C);bb.bind(h,ba,C);this.getRoot().add(new qx.ui.basic.Label(J),{top:390,left:10});var P=new qx.ui.form.TextField(I);P.setLiveUpdate(true);this.getRoot().add(P,{top:410,left:10});var Y=new qx.ui.basic.Label(E);this.getRoot().add(Y,{top:412,left:120});var X={onUpdate:function(){Y.setValue(E);P.setValid(true);}
,onSetFail:function(e){Y.setValue(K);P.setValid(false);}
};P.bind(B,bb,r,X);var S=new qx.ui.basic.Label();S.setRich(true);S.setWidth(450);S.setValue(D+a+L);this.getRoot().add(S,{left:250,top:10});var V=new qx.ui.basic.Label();V.setRich(true);V.setWidth(450);V.setValue(D+i+b);this.getRoot().add(V,{left:250,top:80});var Q=new qx.ui.basic.Label();Q.setRich(true);Q.setWidth(450);Q.setValue(D+w+l+G);this.getRoot().add(Q,{left:250,top:150});var bf=new qx.ui.basic.Label();bf.setRich(true);bf.setWidth(450);bf.setValue(D+q+c+o+p+H);this.getRoot().add(bf,{left:250,top:240});var U=new qx.ui.basic.Label();U.setRich(true);U.setWidth(450);U.setValue(D+N+M+y);this.getRoot().add(U,{left:250,top:390});}
}});}
)();
...
(function(){var q="changeNames",p="changeName",o="",n="__PR",m="changeChild",l="- ",k="changeColor",j="Marge",h="AFFE",g="Homer",c="\n",f="changeName2",e="changeChildren",b="qx.data.Array",a="demobrowser.demo.data.model.Node",d="String";qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.setChildren(new qx.data.Array());}
,properties:{children:{check:b,event:e},child:{check:a,event:m,nullable:true},names:{check:b,event:q,init:new qx.data.Array(g,j)},name:{check:d,init:h,event:p},name2:{check:d,event:f},number:{init:10,validate:n},color:{event:k,nullable:true}},members:{__PR:function(r){if(!isNaN(parseFloat(r))){if(/^\d+$/.test(r)){return;}
;}
...
;var t=o;for(var i=0;i<s;i++){t+=l;}
;t+=this.getName();for(var i=0;i<this.getChildren().length;i++){t+=c+this.getChildren().getItem(i).toString(s+1);}
;return t;}
}});}
)();
