...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.test.Validation","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.test.Validation-qx.theme.Classic.24ed6fe1b169.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var j="demobrowser.demo.test.Validation",i="<b>Setting the age ",h="property to 27</b><br>",g="- name: Name is validated to a minimum length of two.<br/>",f="<br>old name: ",e=")",d="age:",c="fmale",b="<br><b>Setting the name to 'Hans'</b><br>",a="property to 'female'</b><br>",P="<br><b>Setting the gender ",O="must be longer than one characters. (",N="<b>Testing the gender ",M="<br>old gender: ",L="<br><b>Setting the age ",K="- age: Age is validated to be a number.<br/>",J="<b>Initial property values</b><br>",I="__Rb",H="",G="<b>Setting the name to ''</b><br>",q="demobrowser.demo.test.Person",r="property to 'true'</b><br>",o="- gender: Gender is validated to be either male or female.",p="property to 'fmale'</b><br>",m="<font color='red'>GENDER NOT SET</font>",n="gender:",k="<br>old age: ",l="Hans",s="The demo creates an internal class containing three properties:<br/>",t="Validation Error: Name ",y="<font color='red'>AGE NOT SET</font>",x="My name",A="<font color='red'>NAME NOT SET</font>",z="white",C="name:",B="<b>Description</b><br/>",v="new name: ",F="new gender: ",E="male",D="female",u="new age: ",w="<br>";qx.Class.define(j,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var Q=new demobrowser.demo.test.Person();var R=new qx.ui.embed.Html();R.setBackgroundColor(z);R.set({padding:10,width:250,height:360});this.getRoot().add(R,{left:10,top:10});R.setHtml(J);R.setHtml(R.getHtml()+C+Q.getName()+w);R.setHtml(R.getHtml()+d+Q.getAge()+w);R.setHtml(R.getHtml()+n+Q.getGender()+w);R.setHtml(R.getHtml()+b);Q.setName(l);R.setHtml(R.getHtml()+v+Q.getName()+w);R.setHtml(R.getHtml()+G);try{Q.setName(H);R.setHtml(R.getHtml()+v+Q.getName()+w);}
catch(T){R.setHtml(R.getHtml()+A+f+Q.getName()+w);}
;R.setHtml(R.getHtml()+L+h);Q.setAge(27);R.setHtml(R.getHtml()+u+Q.getAge()+w);R.setHtml(R.getHtml()+i+r);try{Q.setAge(true);R.setHtml(R.getHtml()+u+Q.getAge()+w);}
catch(U){R.setHtml(R.getHtml()+y+k+Q.getAge()+w);}
;R.setHtml(R.getHtml()+P+a);Q.setGender(D);R.setHtml(R.getHtml()+F+Q.getGender()+w);R.setHtml(R.getHtml()+N+p);try{Q.setAge(c);R.setHtml(R.getHtml()+F+Q.getGender()+w);}
catch(V){R.setHtml(R.getHtml()+m+M+Q.getGender()+w);}
;var S=new qx.ui.basic.Label();S.setRich(true);S.setWidth(250);S.setValue(B+s+g+K+o);this.getRoot().add(S,{left:280,top:10});}
}});qx.Class.define(q,{extend:qx.core.Object,properties:{name:{init:x,validate:I},age:{init:18,validate:qx.util.Validate.number()},gender:{init:E,validate:qx.util.Validate.inArray([E,D])}},members:{__Rb:function(W){if(W.length<2){throw new qx.core.ValidationError(t+O+W+e);}
;}
}});}
)();
