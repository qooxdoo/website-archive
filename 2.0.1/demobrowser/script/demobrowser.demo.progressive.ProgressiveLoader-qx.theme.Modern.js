...
var envinfo = {"locale":"en","locale.variant":"US","qx.allowUrlSettings":true,"qx.allowUrlVariants":true,"qx.application":"demobrowser.demo.progressive.ProgressiveLoader","qx.optimization.basecalls":true,"qx.optimization.privates":true,"qx.optimization.strings":true,"qx.optimization.variables":true,"qx.optimization.whitespace":true,"qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.progressive.ProgressiveLoader-qx.theme.Modern.fe8e294cf1fe.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var k="demobrowser.demo.progressive.ProgressiveLoader",j="black",i="Red",h="http://resources.qooxdoo.org/images/logo.gif",g="green",f="yellow",d="col ",c="gray",b="blue",a="Green",x="Yellow",w="Blue",v="renderStart",u=", ",t="purple",s="red",r="renderEnd",q="row ",p="Purple",n="func",l="100%",m="white";qx.Class.define(k,{extend:qx.application.Standalone,members:{main:function(){qx.application.Standalone.prototype.main.call(this);var F=new qx.ui.progressive.renderer.table.Widths(1);F.setWidth(0,l);var z=new qx.ui.progressive.headfoot.Progress(F);var D=new qx.ui.progressive.structure.Default(null,z);var y=new qx.ui.progressive.Progressive(D);y.setFlushWidgetQueueAfterBatch(true);var C=function(L){var M={renderer:n,data:L};return M;}
;var J=new qx.ui.progressive.model.Default();var G=new qx.ui.progressive.renderer.FunctionCaller();y.addRenderer(n,G);var E=h;var H=new qx.ui.basic.Image(E,l,l);y.add(H);y.set({height:100,width:272,zIndex:99999,backgroundColor:c,opacity:0.86,batchSize:10});this.getRoot().add(y,{top:-1000,left:-1000});var I=[{text:i,background:s,color:m},{text:a,background:g,color:m},{text:w,background:b,color:m},{text:p,background:t,color:m},{text:x,background:f,color:j}];this.context={document:this.getRoot(),pages:I};y.addListener(v,function(e){var R=e.getData().state;var N=e.getData().initial;var Q=this.getRoot().getBounds();var O=e.getData().state.getProgressive();var P=O.getBounds();var S=Math.floor((Q.width-P.width)/2);var top=Math.floor((Q.height-P.height)/2);O.setLayoutProperties({left:S<0?0:S,top:top<0?0:top});R.getUserData().context=this.context;R.getUserData().initialNum=N;}
,this);y.addListener(r,function(e){qx.event.Timer.once(function(){this.getLayoutParent().remove(this);this.dispose();}
,this,0);}
);J.addElement(C(function(T){var U=T.context;U.tabview=new qx.ui.tabview.TabView();U.tabview.set({});U.document.add(U.tabview,{left:20,right:20,top:20,bottom:20});U.page=0;U.currentPage=null;}
));for(var A=0;A<I.length;A++){J.addElement(C(function(V){var X=V.context;var W=X.pages[X.page];X.currentPage=new qx.ui.tabview.Page(W.text);X.currentPage.setLayout(new qx.ui.layout.VBox(4));X.tabview.add(X.currentPage);X.row=0;X.col=0;if(X.page==0){X.firstPage=X.currentPage;}
;}
));for(var K=0;K<20;K++){J.addElement(C(function(Y){var ba=Y.context;ba.hBox=new qx.ui.container.Composite(new qx.ui.layout.HBox(4));ba.currentPage.add(ba.hBox);ba.tabview.setSelection([ba.currentPage]);ba.col=0;}
));for(var B=0;B<10;B++){J.addElement(C(function(bb){var bd=bb.context;var bc=bd.pages[bd.page];var o=new qx.ui.basic.Atom(q+bd.row+u+d+bd.col);o.set({width:90,backgroundColor:bc.background,textColor:bc.color});bd.hBox.add(o);}
...
(function(){var a="qx.ui.progressive.renderer.FunctionCaller";qx.Class.define(a,{extend:qx.ui.progressive.renderer.Abstract,members:{render:function(b,c){c.data(b.getUserData());}
}});}
)();
