...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree-qx.theme.Simple.799ce4718fbb.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,getLookupTable:function(){return this.__zy;}
,_reverseLookup:function(bh){return bh;}
,_getDataFromRow:function(bi){return this.__zy.getItem(bi);}
,_getSelectables:function(){return this.__zy;}
,getOpenNodes:function(){return this.__IP;}
,isNode:function(bj){return qx.Class.hasProperty(bj.constructor,this.getChildProperty());}
,getLevel:function(bk){return this.__IQ[bk];}
...
,_applyModel:function(bJ,bK){this.__IP=[];if(bJ!=null){if(qx.core.Environment.get(O)){if(!qx.Class.hasMixin(bJ.constructor,qx.data.marshal.MEventBubbling)){this.warn(q+h+f);}
;}
;bJ.addListener(x,this._onChangeBubble,this);this.__IV(bJ);}
...
;if(qx.lang.String.startsWith(bO,this.getChildProperty())){this.__IT();}
;}
,_onUpdated:function(event){if(this.__ei==null){this.__ei=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__ei.schedule();}
,_onOpen:function(event){var bQ=event.getRow();var bP=this.__zy.getItem(bQ);if(this.isNode(bP)){if(this.isNodeOpen(bP)){this.closeNode(bP);}
...
;break;};}
;}
,_beforeApplySelection:function(bV){if(bV.length===0&&this.getSelectionMode()===C){var bW=this.__IY();var bX=this.getLookupTable().indexOf(bW);if(bX>=0){bV.push(bX);}
;}
;}
,_afterApplySelection:function(){var bY=this.getSelection();if(bY.getLength()>0&&this.getSelectionMode()===C){this.__IX(bY.getItem(0));}
else {this.__IS=[];}
;}
,__IT:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(v+k);}
;this.__IR=0;var cd=[];this.__IQ=[];var cb=-1;var cc=this.getModel();if(cc!=null){if(!this.isHideRoot()){cb++;cd.push(cc);this.__IQ.push(cb);}
;if(this.isNodeOpen(cc)){var ca=this.__IU(cc,cb);cd=cd.concat(ca);}
;}
;this._provider.removeBindings();this.__zy.removeAll();this.__zy.append(cd);this.__zE();this._updateSelection();}
,__IU:function(ce,cf){var cg=[];cf++;if(!this.isNode(ce)){return cg;}
;var ci=ce.get(this.getChildProperty());if(ci==null){return cg;}
;for(var i=0;i<ci.getLength();i++){var cj=ci.getItem(i);if(this.isNode(cj)){this.__IQ.push(cf);cg.push(cj);if(this.isNodeOpen(cj)){var ch=this.__IU(cj,cf);cg=cg.concat(ch);}
;}
else {if(this.isShowLeafs()){this.__IQ.push(cf);cg.push(cj);}
;}
;}
;return cg;}
,__IV:function(ck){if(!qx.lang.Array.contains(this.__IP,ck)){this.__IP.push(ck);this.fireDataEvent("open",ck);}
;}
,__IW:function(cl,cm){if(cl===cm){this.__IV(cm);return true;}
;if(!this.isNode(cl)){return false;}
;var co=cl.get(this.getChildProperty());if(co==null){return false;}
;for(var i=0;i<co.getLength();i++){var cp=co.getItem(i);var cn=this.__IW(cp,cm);if(cn===true){this.__IV(cp);return true;}
;}
;return false;}
,__zE:function(){this.getPane().getRowConfig().setItemCount(this.__zy.getLength());this.getPane().fullUpdate();}
,getParent:function(cq){var cs=this.__zy.indexOf(cq);if(cs<0){return null;}
;var ct=this.__IQ[cs];while(cs>0){cs--;var cr=this.__IQ[cs];if(cr<ct){return this.__zy.getItem(cs);}
;}
;return null;}
,__IX:function(cu){this.__IS=[];var parent=this.getParent(cu);while(parent!=null){this.__IS.unshift(parent);parent=this.getParent(parent);}
;}
,__IY:function(){if(this.__IS==null){return this.getModel();}
;var cv=this.getLookupTable();var parent=this.__IS.pop();while(parent!=null){if(cv.contains(parent)){return parent;}
;parent=this.__IS.pop();}
...
;this._layer.removeListener(E,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__zy.dispose();this._layer=this._provider=this.__zy=this.__IP=this.__ei=null;}
});}
)();
...
(function(){var q="model",p="changeDelegate",o="virtual-tree-file",n="label",m="appearance",l="icon",k="bindItem",j=".length",i="]",h="virtual-tree-folder",c="[",g=".",f="qx.ui.tree.core.MWidgetController",b="BindingIds",a="",d="String";qx.Mixin.define(f,{construct:function(){this.__zK=[];}
...
;}
,bindProperty:function(u,v,w,x,y){var z=this.__zL(y,u);var A=this._tree.getLookupTable();var B=A.bind(z,x,v,w);this.__zM(x,B);}
,bindPropertyReverse:function(C,D,E,F,G){var H=this.__zL(G,C);var I=this._tree.getLookupTable();var J=F.bind(D,I,H,E);this.__zM(F,J);}
,removeBindings:function(){while(this.__zK.length>0){var K=this.__zK.pop();this._removeBindingsFrom(K);}
;}
...
;}
,_removeBindingsFrom:function(O){var P=this.__zN(O);while(P.length>0){var Q=P.pop();try{this._tree.getLookupTable().removeBinding(Q);}
catch(e){O.removeBinding(Q);}
;}
;if(qx.lang.Array.contains(this.__zK,O)){qx.lang.Array.remove(this.__zK,O);}
;}
,__zL:function(R,S){var T=c+R+i;if(S!=null&&S!=a){T+=g+S;}
;return T;}
,__zM:function(U,V){var W=this.__zN(U);if(!qx.lang.Array.contains(W,V)){W.push(V);}
;if(!qx.lang.Array.contains(this.__zK,U)){this.__zK.push(U);}
;}
,__zN:function(X){var Y=X.getUserData(b);if(Y==null){Y=[];X.setUserData(b,Y);}
;return Y;}
...
;var q=this._renderer.getCellWidget();q.setOpen(r&&this._tree.isNodeOpen(p));q.setUserData(g,r);q.addListener(b,this.__Ja,this);if(this._tree.getSelection().contains(p)){this._styleSelectabled(q);}
...
,poolCellWidget:function(t){t.removeListener(b,this.__Ja,this);this._removeBindingsFrom(t);this._renderer.pool(t);this._onPool(t);}
...
,__Ja:function(event){var J=event.getTarget();var K=J.getUserData(c);var I=this._tree.getLookupTable().getItem(K);if(event.getData()){this._tree.openNode(I);}
