...
var y=document.getElementById(h).getElementsByTagName(c);
...
}function o(t,u){return parseInt(qx.bom.element.Style.get(t,u,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var q=this.href;
var s=q.substr(q.indexOf(g)+1);
var p=document.getElementById(s);
var location=qx.bom.element.Location.get(p,document.getElementById(f).value);
...
}else{r.style.width=(location.right-location.left-o(r,d)-o(r,b))+n;
r.style.height=(location.bottom-location.top-o(r,e)-o(r,j))+n;
}return false;
},randomize:function(){var x=document.body.getElementsByTagName(a);

for(var i=0,l=x.length;i<l;i++){if(x[i].id&&x[i].id!=m){x[i].style.margin=Math.round(Math.random()*10)+n;
x[i].style.padding=Math.round(Math.random()*10)+n;
x[i].style.borderWidth=Math.round(Math.random()*10)+n;
}}}}});
})();
