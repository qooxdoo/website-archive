...
var bc=new qx.ui.progressive.renderer.table.Widths(1);
bc.setWidth(0,j);
var V=new qx.ui.progressive.headfoot.Progress(bc);
var ba=new qx.ui.progressive.structure.Default(null,V);
var U=new qx.ui.progressive.Progressive(ba);
U.setFlushWidgetQueueAfterBatch(true);
var Y=function(P){var Q={renderer:i,data:P};
return Q;
};
var bg=new qx.ui.progressive.model.Default();
var bd=new qx.ui.progressive.renderer.FunctionCaller();
U.addRenderer(i,bd);
var bb=d;
var be=new qx.ui.basic.Image(bb,j,j);
U.add(be);
U.set({height:100,width:272,zIndex:99999,backgroundColor:x,opacity:0.86,batchSize:10});
this.getRoot().add(U,{top:-1000,left:-1000});
var bf=[{text:f,background:p,color:k},{text:v,background:c,color:k},{text:t,background:w,color:k},{text:m,background:q,color:k},{text:u,background:b,color:g}];
this.context={document:this.getRoot(),pages:bf};
U.addListener(s,function(e){var I=e.getData().state;
var E=e.getData().initial;
var H=this.getRoot().getBounds();
var F=e.getData().state.getProgressive();
var G=F.getBounds();
var J=Math.floor((H.width-G.width)/2);
var top=Math.floor((H.height-G.height)/2);
F.setLayoutProperties({left:J<0?0:J,top:top<0?0:top});
I.getUserData().context=this.context;
I.getUserData().initialNum=E;
},this);
U.addListener(n,function(e){qx.event.Timer.once(function(){this.getLayoutParent().remove(this);
this.dispose();
},this,0);
});
bg.addElement(Y(function(C){var D=C.context;
...
for(var W=0;W<bf.length;W++){bg.addElement(Y(function(y){var A=y.context;
var z=A.pages[A.page];
A.currentPage=new qx.ui.tabview.Page(z.text);
A.currentPage.setLayout(new qx.ui.layout.VBox(4));
A.tabview.add(A.currentPage);
A.row=0;
A.col=0;
if(A.page==0){A.firstPage=A.currentPage;
}}));
for(var bh=0;bh<20;bh++){bg.addElement(Y(function(R){var S=R.context;
...
for(var X=0;X<10;X++){bg.addElement(Y(function(K){var M=K.context;
var L=M.pages[M.page];
var o=new qx.ui.basic.Atom(l+M.row+r+a+M.col);
o.set({width:90,backgroundColor:L.background,textColor:L.color});
M.hBox.add(o);
}));
bg.addElement(Y(function(B){++B.context.col;
}));
}bg.addElement(Y(function(O){++O.context.row;
}));
}bg.addElement(Y(function(N){++N.context.page;
}));
}bg.addElement(Y(function(T){T.context.tabview.setSelection([T.context.firstPage]);
}));
U.setDataModel(bg);
U.render();
}}});
