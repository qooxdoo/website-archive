...
H.addListener(k,function(V){var W=this._tableModel.getRowCount();
...
H.addListener(k,function(Q){this._table.resetSelection();
},this);
J.add(H);
H=new qx.ui.toolbar.Button(z);
H.addListener(k,function(P){this._table.resetCellFocus();
},this);
J.add(H);
var K=new qx.ui.toolbar.CheckBox(p);
K.addListener(d,function(R){this._table.setShowCellFocusIndicator(!K.isValue());
},this);
J.add(K);
H=new qx.ui.toolbar.Button(n,c);
H.addListener(k,function(S){var T=[];
this._table.getSelectionModel().iterateSelection(function(U){T.push(U+i);
});
this.showDialog(m+T.join(q));
},this);
J.add(H);
return F;
