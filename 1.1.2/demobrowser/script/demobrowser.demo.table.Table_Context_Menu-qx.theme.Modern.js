...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.table.Table_Context_Menu","qx.theme":"qx.theme.Modern"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.table.Table_Context_Menu-qx.theme.Modern.js"]],
  urisBefore : [],
  packageHashes : {"0":"f0eff4677133"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
},createTable:function(){var bi=this.createRandomRows(50);
var bh=this._tableModel=new qx.ui.table.model.Simple();
bh.setColumns([s,a,f,B]);
bh.setData(bi);
var bj=new qx.ui.table.Table(bh);
bj.set({width:600,height:400,decorator:null});
var bg=bj.getTableColumnModel();
bg.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
bj.setContextMenuHandler(3,this._contextMenuHandlerBoolean);
bj.setContextMenuHandler(2,this._contextMenuHandlerNull);
return bj;
},_contextMenuHandlerBoolean:function(Q,R,S,T,U){var V;

for(var i=0;i<=1;i++){V=new qx.ui.menu.Button(i?c:o);
V.setUserData(j,i?true:false);
V.addListener(l,function(e){T.setValue(Q,R,this.getUserData(j));
});
U.add(V);
}return true;
},_contextMenuHandlerNull:function(L,M,N,O,P){return true;
},createControls:function(){var bn=new qx.ui.toolbar.ToolBar();
var bl,bm,bo;
bm=new qx.ui.toolbar.Part();
bn.add(bm);
bl=new qx.ui.toolbar.Button(b,h);
bl.addListener(l,function(F){var G=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,G[0][i]);
}this.info("Row 10 changed");
},this);
bm.add(bl);
bl=new qx.ui.toolbar.Button(C,t);
bl.addListener(l,function(bp){var bq=this.createRandomRows(10);
this._tableModel.addRows(bq);
this.info("10 rows added");
},this);
bm.add(bl);
bl=new qx.ui.toolbar.Button(D,x);
bl.addListener(l,function(br){var bs=this._tableModel.getRowCount();
this._tableModel.removeRows(bs-5,5);
this.info("5 rows removed");
},this);
bm.add(bl);
bl=new qx.ui.toolbar.Button(m,d);
bl.addListener(l,function(be){var bf=[];
bk.getSelectionModel().iterateSelection(function(E){bf.push(E+A);
});
this.showDialog(q+bf.join(v));
},this);
bm.add(bl);
bm=new qx.ui.toolbar.Part();
bn.add(bm);
var bk=this._table;
bo=new qx.ui.toolbar.CheckBox(r);
bo.set({value:bk.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(w+p)});
bo.addListener(k,function(bt){bk.setKeepFirstVisibleRowComplete(this.getValue());
},bo);
bm.add(bo);
bo=new qx.ui.toolbar.CheckBox(g);
bo.set({value:bk.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(n)});
bo.addListener(k,function(bb){if(bb.getData()){var bd=function(W,X){var Y=W[arguments.callee.columnIndex];
var ba=X[arguments.callee.columnIndex];

if(Y%2==1&&ba%2==0){return 1;
}
if(ba%2==1&&Y%2==0){return -1;
}return (Y>ba)?1:((Y==ba)?0:-1);
};
var bc=function(H,I){var J=H[arguments.callee.columnIndex];
var K=I[arguments.callee.columnIndex];

if(J%2==1&&K%2==0){return -1;
}
if(K%2==1&&J%2==0){return 1;
}return (J<K)?1:((J==K)?0:-1);
};
bk.getTableModel().setSortMethods(0,{ascending:bd,descending:bc});
}else{bk.getTableModel().setSortMethods(0,null);
}},bo);
bm.add(bo);
return bn;
...
var i=this.getContextMenuHandler(j);
if(i==null){return;
}var h=this.getTableModel();
this._contextMenu=new qx.ui.menu.Menu();
this._contextMenu.addListener(c,function(e){e.preventDefault();
});
var k=i(j,e.getRow(),this,h,this._contextMenu);
if(!k){this._contextMenu.dispose();
return;
}this.setContextMenu(this._contextMenu);
},this);
this.__Dv=[];
},members:{__Dv:null,setContextMenuHandler:function(d,f){this.__Dv[d]=f;
},getContextMenuHandler:function(g){return this.__Dv[g];
