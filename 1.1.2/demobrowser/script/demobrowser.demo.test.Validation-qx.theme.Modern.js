...
},checkNumber:function(v){if((typeof v!==j&&(!(v instanceof Number)))||(!(isFinite(v)))){throw new qx.core.ValidationError(i,qx.locale.Manager.tr(n,v));
}},email:function(){return qx.util.Validate.checkEmail;
},checkEmail:function(p){var q=/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;

if(q.test(p)===false){throw new qx.core.ValidationError(o,qx.locale.Manager.tr(h,(p||l)));
}},string:function(){return qx.util.Validate.checkString;
},checkString:function(C){if(typeof C!==c&&(!(C instanceof String))){throw new qx.core.ValidationError(o,qx.locale.Manager.tr(k,C));
}},url:function(){return qx.util.Validate.checkUrl;
},checkUrl:function(x){var y=/([A-Za-z0-9])+:\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;

if(y.test(x)===false){throw new qx.core.ValidationError(o,qx.locale.Manager.tr(f,x));
}},color:function(){return qx.util.Validate.checkColor;
},checkColor:function(t){try{qx.util.ColorUtil.stringToRgb(t);
}catch(e){throw new qx.core.ValidationError(o,qx.locale.Manager.tr(m,t,e));
}},range:function(z,A){return function(w){if(w<z||w>A){throw new qx.core.ValidationError(o,qx.locale.Manager.tr(a,w,z,A));
}};
},inArray:function(s){return function(r){if(s.indexOf(r)===-1){throw new qx.core.ValidationError(o,qx.locale.Manager.tr(b,r,s));
}};
},regExp:function(u){return function(B){if(u.test(B)===false){throw new qx.core.ValidationError(o,qx.locale.Manager.tr(d,B,u));
}};
}}});
})();
(function(){var j="<br>",i="new name: ",h="new gender: ",g="male",f="female",e="new age: ",d="demobrowser.demo.test.Validation",c="<b>Setting the age ",b="property to 27</b><br>",a="- name: Name is validated to a minimum length of two.<br/>",P="<br>old name: ",O=")",N="age:",M="fmale",L="<br><b>Setting the name to 'Hans'</b><br>",K="property to 'female'</b><br>",J="<br><b>Setting the gender ",I="must be longer than one characters. (",H="<b>Testing the gender ",G="<br>old gender: ",q="<br><b>Setting the age ",r="- age: Age is validated to be a number.<br/>",o="<b>Initial property values</b><br>",p="",m="<b>Setting the name to ''</b><br>",n="demobrowser.demo.test.Person",k="property to 'true'</b><br>",l="- gender: Gender is validated to be either male or female.",s="property to 'fmale'</b><br>",t="<font color='red'>GENDER NOT SET</font>",y="gender:",x="<br>old age: ",A="Hans",z="The demo creates an internal class containing three properties:<br/>",C="Validation Error: Name ",B="<font color='red'>AGE NOT SET</font>",v="__Lf",F="My name",E="<font color='red'>NAME NOT SET</font>",D="white",u="name:",w="<b>Description</b><br/>";
...
var U=new demobrowser.demo.test.Person();
var V=new qx.ui.embed.Html();
V.setBackgroundColor(D);
V.set({padding:10,width:250,height:360});
this.getRoot().add(V,{left:10,top:10});
V.setHtml(o);
V.setHtml(V.getHtml()+u+U.getName()+j);
V.setHtml(V.getHtml()+N+U.getAge()+j);
V.setHtml(V.getHtml()+y+U.getGender()+j);
V.setHtml(V.getHtml()+L);
U.setName(A);
V.setHtml(V.getHtml()+i+U.getName()+j);
V.setHtml(V.getHtml()+m);
try{U.setName(p);
V.setHtml(V.getHtml()+i+U.getName()+j);
}catch(R){V.setHtml(V.getHtml()+E+P+U.getName()+j);
}V.setHtml(V.getHtml()+q+b);
U.setAge(27);
V.setHtml(V.getHtml()+e+U.getAge()+j);
V.setHtml(V.getHtml()+c+k);
try{U.setAge(true);
V.setHtml(V.getHtml()+e+U.getAge()+j);
}catch(S){V.setHtml(V.getHtml()+B+x+U.getAge()+j);
}V.setHtml(V.getHtml()+J+K);
U.setGender(f);
V.setHtml(V.getHtml()+h+U.getGender()+j);
V.setHtml(V.getHtml()+H+s);
try{U.setAge(M);
V.setHtml(V.getHtml()+h+U.getGender()+j);
}catch(Q){V.setHtml(V.getHtml()+t+G+U.getGender()+j);
}var W=new qx.ui.basic.Label();
W.setRich(true);
W.setWidth(250);
W.setValue(w+z+a+r+l);
this.getRoot().add(W,{left:280,top:10});
}}});
qx.Class.define(n,{extend:qx.core.Object,properties:{name:{init:F,validate:v},age:{init:18,validate:qx.util.Validate.number()},gender:{init:g,validate:qx.util.Validate.inArray([g,f])}},members:{__Lf:function(T){if(T.length<2){throw new qx.core.ValidationError(C+I+T+O);
}}}});
})();
