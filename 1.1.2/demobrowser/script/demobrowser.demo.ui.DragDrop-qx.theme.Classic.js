...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.ui.DragDrop","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.ui.DragDrop-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"e9a186e1c14a"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var r="drop",q="value",p="items",o="move",n="multi",m="dragover",k="icon/16/places/folder.png",j="dragstart",h="Item ",g="copy",F="drag",E="TextArea Target",D="solid",C="__Nr",B="Reorderable",A="qx.ui.form.ListItem",z="droprequest",y="#33508D",x="Source",w="demobrowser.demo.ui.DragDrop",u="Simple Target",v="Enable drag",s="\n",t="dragend";
qx.Class.define(w,{extend:qx.application.Standalone,members:{__Nr:null,__Ns:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var T=this.getRoot();
var N=new qx.ui.container.Scroll();
var I=new qx.ui.layout.Basic();
var L=new qx.ui.container.Composite(I).set({allowStretchY:false,allowStretchX:false});
N.add(L);
T.add(N,{edge:0});
var S=new qx.ui.basic.Label(x);
L.add(S,{left:20,top:20});
var Q=new qx.ui.form.List;
Q.setDraggable(true);
Q.setSelectionMode(n);
L.add(Q,{left:20,top:40});

for(var i=0;i<20;i++){Q.add(new qx.ui.form.ListItem(h+i,k));
}var U=new qx.ui.form.CheckBox(v);
U.setValue(true);
L.add(U,{left:20,top:260});
Q.addListener(j,function(e){if(!U.isValue()){e.preventDefault();
}e.addType(q);
e.addType(p);
e.addAction(g);
e.addAction(o);
});
Q.addListener(z,function(e){this.debug("Related of droprequest: "+e.getRelatedTarget());
var c=e.getCurrentAction();
var f=e.getCurrentType();
var b;

switch(f){case p:b=this.getSelection();

if(c==g){var d=[];

for(var i=0,l=b.length;i<l;i++){d[i]=b[i].clone();
}b=d;
}break;
case q:b=this.getSelection()[0].getLabel();
break;
}if(c==o){var a=this.getSelection();

for(var i=0,l=a.length;i<l;i++){this.remove(a[i]);
}}e.addData(f,b);
});
var O=new qx.ui.basic.Label(u);
L.add(O,{left:140,top:20});
var M=new qx.ui.form.List;
M.setDroppable(true);
M.setSelectionMode(n);
L.add(M,{left:140,top:40});
M.addListener(r,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
var V=e.getData(p);

for(var i=0,l=V.length;i<l;i++){this.add(V[i]);
}});
M.addListener(m,function(e){if(!e.supportsType(p)){e.preventDefault();
}});
var O=new qx.ui.basic.Label(E);
L.add(O,{left:260,top:20});
var R=new qx.ui.form.TextArea;
R.setDroppable(true);
R.setHeight(100);
L.add(R,{left:260,top:40});
R.addListener(r,function(e){this.debug("Related of drop: "+e.getRelatedTarget());
this.setValue(e.getData(q).replace(/,/g,s));
});
R.addListener(m,function(e){if(!e.supportsType(q)){e.preventDefault();
}});
var P=new qx.ui.basic.Label(B);
L.add(P,{left:500,top:20});
var J=this.__Nr=new qx.ui.form.List;
J.setDraggable(true);
J.setDroppable(true);
J.setSelectionMode(n);
L.add(J,{left:500,top:40});

for(var i=0;i<20;i++){J.add(new qx.ui.form.ListItem(h+i,k));
}var K=new qx.ui.core.Widget;
K.setDecorator(new qx.ui.decoration.Single().set({top:[1,D,y]}));
K.setHeight(0);
K.setOpacity(0.5);
K.setZIndex(100);
K.setLayoutProperties({left:-1000,top:-1000});
K.setDroppable(true);
this.getRoot().add(K);
J.addListener(j,function(e){e.addAction(o);
});
J.addListener(t,function(e){K.setDomPosition(-1000,-1000);
});
J.addListener(F,function(e){var H=e.getOriginalTarget();
if(H instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__Ns=H;
}
if(!qx.ui.core.Widget.contains(this,H)&&H!=K){return;
}var G=H.getContainerLocation();
K.setWidth(H.getBounds().width);
K.setDomPosition(G.left,G.top);
});
J.addListener(m,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
J.addListener(r,function(e){this.__Nt(e.getOriginalTarget());
},this);
K.addListener(r,function(e){this.__Nt(this.__Ns);
},this);
},__Nt:function(W){if(W.classname!=A){return;
}var X=this.__Nr.getSortedSelection();

for(var i=0,l=X.length;i<l;i++){this.__Nr.addBefore(X[i],W);
this.__Nr.addToSelection(X[i]);
