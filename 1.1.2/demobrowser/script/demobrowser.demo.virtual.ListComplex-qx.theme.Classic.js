...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.virtual.ListComplex","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.virtual.ListComplex-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"40eedc50df52"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
(function(){var d="demobrowser.demo.virtual.ListComplex",c="horizontal",b="rgb(238, 243, 255)",a="white";
...
var e=new qx.ui.virtual.core.Scroller(100,15,32,120);
e.getPane().setWidth(450);
for(var i=2;i<10;i++){e.getPane().getRowConfig().setItemSize(i,50+Math.round(Math.random()*40));
e.getPane().getColumnConfig().setItemSize(i,50+Math.round(Math.random()*80));
}this.getRoot().add(e,{left:20,top:10});
e.getPane().addLayer(new qx.ui.virtual.layer.Row(a,b));
e.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
e.getPane().addLayer(new demobrowser.demo.virtual.DemoLayer);
new qx.ui.virtual.behavior.Prefetch(e,500,600,1000,1200,0,0,0,0);
}}});
})();
...
qx.Class.define(a,{extend:qx.ui.virtual.layer.Abstract,construct:function(n,o){qx.ui.virtual.layer.Abstract.call(this);

if(n){this.setColorEven(n);
}
if(o){this.setColorOdd(o);
}this.__CE={};
this.__CF={};
},properties:{colorEven:{nullable:true,check:d,apply:b,themeable:true},colorOdd:{nullable:true,check:d,apply:c,themeable:true}},members:{__CG:null,__CH:null,__CE:null,__CF:null,setColor:function(p,q){if(q){this.__CE[p]=qx.theme.manager.Color.getInstance().resolve(q);
}else{delete (this.__CE[p]);
}},clearCustomColors:function(){this.__CE={};
this.updateLayerData();
},getColor:function(l){var m=this.__CE[l];

if(m){return m;
}else{return l%2==0?this.__CG:this.__CH;
}},_applyColorEven:function(j,k){if(j){this.__CG=qx.theme.manager.Color.getInstance().resolve(j);
}else{this.__CG=null;
}this.updateLayerData();
},_applyColorOdd:function(e,f){if(e){this.__CH=qx.theme.manager.Color.getInstance().resolve(e);
}else{this.__CH=null;
}this.updateLayerData();
},setBackground:function(h,i){if(i){this.__CF[h]=qx.theme.manager.Decoration.getInstance().resolve(i);
}else{delete (this.__CF[h]);
}this.updateLayerData();
},getBackground:function(g){return this.__CF[g];
...
qx.Class.define(f,{extend:qx.ui.virtual.layer.AbstractBackground,properties:{appearance:{refine:true,init:b}},members:{_fullUpdate:function(t,u,v,w){var D=[];
var C=qx.lang.Array.sum(w);
var E=[];
var top=0;
var G=t;
var A=0;

for(var y=0;y<v.length;y++){var F=this.getBackground(G);

if(F){E.push({childIndex:A,decorator:F,width:C,height:v[y]});
D.push(k,m,p,n,top,q,j,F.getMarkup(),o);
A++;
}else{var B=this.getColor(G);

if(B){D.push(k,m,p,n,top,q,d,v[y],q,e,C,q,h,B,j,o);
A++;
}}top+=v[y];
G+=1;
}var x=this.getContentElement().getDomElement();
x.style.display=a;
x.innerHTML=D.join(g);
for(var i=0,l=E.length;i<l;i++){var z=E[i];
z.decorator.resize(x.childNodes[z.childIndex].firstChild,z.width,z.height);
}x.style.display=c;
this._width=C;
},_updateLayerWindow:function(J,K,L,M){if(J!==this.getFirstRow()||L.length!==this.getRowSizes().length||this._width<qx.lang.Array.sum(M)){this._fullUpdate(J,K,L,M);
}},setColor:function(r,s){qx.ui.virtual.layer.AbstractBackground.prototype.setColor.call(this,r,s);

if(this.__CI(r)){this.updateLayerData();
}},setBackground:function(H,I){qx.ui.virtual.layer.AbstractBackground.prototype.setBackground.call(this,H,I);

if(this.__CI(H)){this.updateLayerData();
}},__CI:function(N){var P=this.getFirstRow();
...
(function(){var r=" / ",q="row",p="foobar!",o="column",n="icon/",m="",l="folder.png",k="places/",i="demobrowser.demo.virtual.DemoLayer",h="network-server.png",c="mouseover",g="user-trash.png",f="changeValue",b="network-workgroup.png",a="qx.ui.basic.Atom",e="/",d="user-desktop.png";
qx.Class.define(i,{extend:qx.ui.virtual.layer.WidgetCell,construct:function(){qx.ui.virtual.layer.WidgetCell.call(this,this);
this._pool={atom:[],checkbox:[]};
this.__LY=[];
},members:{_pool:null,__LY:null,getCellData:function(w,x){if(!this.__LY[w]){this.__LY[w]=[];
}
if(!this.__LY[w][x]){this.__LY[w][x]={label:this.__Ma(),icon:this.__Mb()};
}return this.__LY[w][x];
},getCellWidget:function(s,t){var u;

if(t%2==0){u=this._pool.atom.pop();

if(!u){u=new qx.ui.basic.Atom();
u.addListener(c,function(){var v=this.__Mb();
u.setIcon(v);
this.__LY[s][t].icon=v;
},this);
}u.set(this.getCellData(s,t));
}else{u=this._pool.checkbox.pop();

if(!u){u=new qx.ui.form.CheckBox();
u.addListener(f,function(){this.setLabel(this.getLabel()==p?u.getUserData(q)+r+u.getUserData(o):p);
},u);
}u.set({value:s%2==0,label:s+r+t});
}u.setUserData(q,s);
u.setUserData(o,t);
return u;
},poolCellWidget:function(y){if(y.classname==a){this._pool.atom.push(y);
}else{this._pool.checkbox.push(y);
}},__Ma:function(){var name=m;

for(var j=0;j<10;j++){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);
}return name;
},__Mb:function(){var z=n;
var B=k;
var C=[l,g,h,b,d];
var A=Math.floor(Math.random()*4);
return (z+16+e+B+C[A]);
}},destruct:function(){this._pool=this.__LY=null;
}});
})();
