...
},addSimpleStack:function(){var A=new qx.ui.container.Stack();
A.setDecorator(f);
A.setWidth(200);
A.setHeight(120);
...
y.add(D);
y.add(B);
this.getRoot().add(y,{left:20,top:150});
...
A.add(z);
}A.addListener(h,function(e){this.debug("Selected: "+e.getData()[0].getBackgroundColor());
});
},addDynamicStack:function(){var u=new qx.ui.container.Stack();
u.setDecorator(f);
u.setDynamic(true);
this.getRoot().add(u,{left:250,top:20});
...
s.add(x);
s.add(v);
this.getRoot().add(s,{left:250,top:150});
...
t.setWidth((Math.round(i/2)+1)*50);
t.setHeight((Math.round(i/3)+1)*40);
u.add(t);
}u.addListener(h,function(e){var F=e.getData()[0];
this.debug("Selected: "+F.getBackgroundColor()+" ("+F.getWidth()+"x"+F.getHeight()+")");
});
u.addListener(a,function(e){var E=this.getBounds();
this.debug("Resize to: "+E.width+"x"+E.height);
});
}}});
})();
