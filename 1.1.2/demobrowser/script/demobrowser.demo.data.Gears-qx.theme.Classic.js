...
var J=new qx.ui.groupbox.GroupBox(r);
this.getRoot().add(J,{left:10,top:50});
J.setLayout(new qx.ui.layout.Grid(0,3));
J.add(new qx.ui.basic.Label(z),{row:0,column:0});
J.add(new qx.ui.basic.Label(u),{row:1,column:0});
J.add(new qx.ui.basic.Label(c),{row:2,column:0});
var D=new qx.ui.form.TextField();
J.add(D,{row:0,column:1});
var F=new qx.ui.form.TextField();
J.add(F,{row:1,column:1});
var G=new qx.ui.form.Spinner(0,0,1000);
J.add(G,{row:2,column:1});
var H=new qx.data.controller.Object();
H.addTarget(D,k,C,true);
H.addTarget(F,k,a,true);
H.addTarget(G,k,B,true);
var I=new demobrowser.demo.data.store.Gears();
I.bind(h,H,h);
...
}this.__MF=this.initDB();
this.__MG();
},members:{__MG:function(R){var S=this.getPerson();
this._marshaler.toClass(S,true);
this.setModel(this._marshaler.toModel(S));
this.getModel().addListener("changeBubble",this.__MH,this);
this.fireDataEvent("loaded",this.getModel());
},__MH:function(N){var O=this.getModel();
this.update(O.getFirstname(),O.getLastname(),O.getAge());
},update:function(T,U,V){if(!google.gears.factory||!this.__MF){return;
}this.__MF.execute(v);
this.__MF.execute(o,[T,U,V]);
},getPerson:function(){var L={};
L.firstname=i;
L.lastname=i;
L.age=0;

if(!google.gears.factory||!this.__MF){return L;
}var M=this.__MF.execute(w);

if(M.isValidRow()){L.firstname=M.field(0);
L.lastname=M.field(1);
L.age=parseInt(M.field(2));
}M.close();
return L;
},initDB:function(){if(window.google&&google.gears){try{var P=google.gears.factory.create(l);

if(P){P.open(y);
P.execute(q+t);
}}catch(K){this.error('Could not create database: '+K.message);
}}return P;
},initGears:function(){if(window.google&&google.gears){return;
}var Q=null;
if(typeof GearsFactory!=j){Q=new GearsFactory();
}else{try{Q=new ActiveXObject(x);
if(Q.getBuildInfo().indexOf(s)!=-1){Q.privateSetGlobalObject(this);
}}catch(e){if((typeof navigator.mimeTypes!=j)&&navigator.mimeTypes[g]){Q=document.createElement(m);
Q.style.display=p;
Q.width=0;
Q.height=0;
Q.type=g;
...
if(!google.gears){google.gears={factory:Q};
}}}});
})();
