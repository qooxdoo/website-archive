...
this.__Ld(O,N);
},__Ld:function(C,D){var E=new D.clazz();
var I=qx.lang.Object.clone(D);
delete I.clazz;
delete I.layout;
delete I.children;
delete I.id;
delete I.listeners;
E.set(I);
if(D.id!=null){this[D.id]=E;
}if(D.layout!=null){var F=new D.layout.clazz();
var I=qx.lang.Object.clone(D.layout);
delete I.clazz;
F.set(I);
E.setLayout(F);
}var H=D.children;

if(H!=null){for(var i=0;i<H.length;i++){if(E instanceof qx.ui.form.Form){this.__Le(E,H[i]);
}else{this.__Ld(E,H[i]);
}}}if(E instanceof qx.ui.form.Form){C.add(new qx.ui.form.renderer.Single(E));
}else{C.add(E,D.layoutProperties);
}if(D.listeners){for(var i=0;i<D.listeners.length;i++){var G=D.listeners[i];
E.addListener(G.event,G.handler,G.context);
}}},__Le:function(J,K){if(K.type===j){J.addGroupHeader(K.label);
return;
