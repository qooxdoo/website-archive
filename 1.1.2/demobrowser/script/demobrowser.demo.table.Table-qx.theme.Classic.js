...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.table.Table","qx.theme":"qx.theme.Classic"};
...
(function(){var p="execute",o="changeValue",n="A date",m="Boolean",l="icon/22/actions/edit-undo.png",k="Change ID sort method",j="icon/22/status/dialog-information.png",h="Change row with ID 10",g="A number",f="icon/16/apps/office-calendar.png",F="Remove 5 rows",E="Add 10 rows",D="",C="demobrowser.demo.table.Table",B="_tableModel",A="Table",z="icon/22/actions/list-remove.png",y="Whether the the first visible row should ",x=", ",w="icon/22/actions/list-add.png",u="Selected rows:<br>",v="Keep first row",s="be rendered completely when scrolling.",t="Show selection",q="Demonstrate use of alternate sorting algorithm.",r="ID";
qx.Class.define(C,{extend:demobrowser.demo.table.TableDemo,members:{getCaption:function(){return A;
},createTable:function(){var ba=this.createRandomRows(50);
var Y=this._tableModel=new qx.ui.table.model.Simple();
Y.setColumns([r,g,n,m]);
Y.setData(ba);
Y.setColumnEditable(1,true);
Y.setColumnEditable(2,true);
Y.setColumnSortable(3,false);
var bb=new qx.ui.table.Table(Y);
bb.set({width:600,height:400,decorator:null});
bb.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
var X=bb.getTableColumnModel();
X.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
X.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(f,n));
return bb;
},createControls:function(){var d=new qx.ui.toolbar.ToolBar();
var b,c,e;
c=new qx.ui.toolbar.Part();
d.add(c);
b=new qx.ui.toolbar.Button(h,l);
b.addListener(p,function(P){var Q=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,Q[0][i]);
}this.info("Row 10 changed");
},this);
c.add(b);
b=new qx.ui.toolbar.Button(E,w);
b.addListener(p,function(be){var bf=this.createRandomRows(10);
this._tableModel.addRows(bf);
this.info("10 rows added");
},this);
c.add(b);
b=new qx.ui.toolbar.Button(F,z);
b.addListener(p,function(bc){var bd=this._tableModel.getRowCount();
this._tableModel.removeRows(bd-5,5);
this.info("5 rows removed");
},this);
c.add(b);
b=new qx.ui.toolbar.Button(t,j);
b.addListener(p,function(N){var O=[];
a.getSelectionModel().iterateSelection(function(S){O.push(S+D);
});
this.showDialog(u+O.join(x));
},this);
c.add(b);
c=new qx.ui.toolbar.Part();
d.add(c);
var a=this._table;
e=new qx.ui.toolbar.CheckBox(v);
e.set({value:a.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(y+s)});
e.addListener(o,function(R){a.setKeepFirstVisibleRowComplete(this.getValue());
},e);
c.add(e);
e=new qx.ui.toolbar.CheckBox(k);
e.set({value:a.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(q)});
e.addListener(o,function(K){if(K.getData()){var M=function(G,H){var I=G[arguments.callee.columnIndex];
var J=H[arguments.callee.columnIndex];

if(I%2==1&&J%2==0){return 1;
}
if(J%2==1&&I%2==0){return -1;
}return (I>J)?1:((I==J)?0:-1);
};
var L=function(T,U){var V=T[arguments.callee.columnIndex];
var W=U[arguments.callee.columnIndex];

if(V%2==1&&W%2==0){return -1;
}
if(W%2==1&&V%2==0){return 1;
}return (V<W)?1:((V==W)?0:-1);
};
a.getTableModel().setSortMethods(0,{ascending:M,descending:L});
}else{a.getTableModel().setSortMethods(0,null);
}},e);
c.add(e);
return d;
