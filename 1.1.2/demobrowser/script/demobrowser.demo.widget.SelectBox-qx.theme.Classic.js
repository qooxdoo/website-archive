...
var S=new qx.ui.container.Composite(new qx.ui.layout.HBox(50));
S.setPadding(20);
S.add(this.createBox1());
S.add(this.createBox2());
S.add(this.createBox3());
S.add(this.createBox4());
T.add(S,{left:20,top:20});
this.getRoot().add(T,{edge:0});
},createBox1:function(){var N=new qx.ui.container.Composite(new qx.ui.layout.VBox(2));
N.add(new qx.ui.basic.Label(f));
var M=new qx.ui.form.SelectBox();

for(var i=0;i<30;i++){var O=new qx.ui.form.ListItem(v+(i+1));
M.add(O);
if(i==5){M.setSelection([O]);
}}M.addListener(B,function(e){this.debug("changeSelection: "+e.getData()[0]);
});
N.add(M);
return N;
},createBox2:function(){var Q=new qx.ui.container.Composite(new qx.ui.layout.VBox(2));
Q.add(new qx.ui.basic.Label(w));
var P=new qx.ui.form.SelectBox();

for(var i=0;i<30;i++){var R=new qx.ui.form.ListItem(G+Math.round(Math.random()*100000000));
P.add(R);
}Q.add(P);
return Q;
},createBox3:function(){var I=new qx.ui.container.Composite(new qx.ui.layout.VBox(2));
I.add(new qx.ui.basic.Label(p));
var K=[j,c,h,x,n,a,d,m,E,F,r,t,C,u,z,s,A,k,g];
var L=l;
var H=new qx.ui.form.SelectBox();

for(var i=0;i<K.length;i++){var J=new qx.ui.form.ListItem(K[i].substr(0,K[i].indexOf(y)),L+K[i]);
H.add(J);
}I.add(H);
return I;
