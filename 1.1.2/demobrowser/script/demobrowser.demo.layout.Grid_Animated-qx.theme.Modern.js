...
m.setSpacing(20);
var l=new qx.ui.container.Composite(m);
l.setPadding(20);
this.getRoot().add(l,{left:0,top:0});
l.add(this.getAnimGrid(),{row:0,column:0});
},getNewWidget:function(){var k=new qx.ui.core.Widget().set({width:50,height:50,backgroundColor:j});
k.addListener(h,function(e){if(this._active==k){return;
}var r=[];
var p=0.3;

if(this._active){this._active.set({backgroundColor:j,width:50,height:50});
}k.set({backgroundColor:b});
var s=k.getBounds();
r.push(new demobrowser.demo.layout.Grid_Animated_Property(k,f).set({from:s.width,to:200,duration:p,transition:i}));
r.push(new demobrowser.demo.layout.Grid_Animated_Property(k,d).set({from:s.height,to:200,duration:p,transition:i}));
var q=new qx.fx.effect.core.Parallel(r[0],r[1]);
q.start();
this._active=k;
},this);
return k;
},getAnimGrid:function(){var n=new qx.ui.container.Composite().set({decorator:a,backgroundColor:g,width:500,height:500});
var o=new qx.ui.layout.Grid();
o.setSpacing(3);
n.setLayout(o);
this._active=null;

for(var x=0;x<7;x++){o.setColumnFlex(x,1);
o.setRowFlex(x,1);

for(var y=0;y<7;y++){n.add(this.getNewWidget(),{row:y,column:x});
}}return n;
}}});
})();
...
(function(){var d="qx.fx.effect.core.Parallel",c="__uh";
qx.Class.define(d,{extend:qx.fx.Base,construct:function(g){qx.fx.Base.call(this);
this.__uh=arguments;
},members:{__uh:null,finish:function(){qx.fx.Base.prototype.finish.call(this);
var f=this.__uh;
...
f[i].afterFinish();
}},update:function(a){qx.fx.Base.prototype.update.call(this);
var b=this.__uh;

for(var i=0;i<b.length;i++){b[i].render(a);
