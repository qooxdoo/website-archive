...
var U=new demobrowser.demo.data.store.Flickr(k);
var M=new qx.ui.form.TextField(k);
this.getRoot().add(M,{left:30,top:50});
var N=new qx.ui.form.Button(q);
this.getRoot().add(N,{left:140,top:49});
N.addListener(C,function(){U.searchForTag(M.getValue());
},this);
M.addListener(b,function(e){if(e.getKeyIdentifier()==c){U.searchForTag(M.getValue());
}},this);
...
Q.setHeight(110);
Q.setOrientation(o);
this.getRoot().add(Q,{left:30,right:30,top:80});
var S=new qx.data.controller.List(null,Q);
S.setLabelPath(r);
S.setDelegate({configureItem:function(V){V.setShow(s);
}});
var O={converter:function(J,K){return (d+K.getFarm()+i+K.getServer()+j+J+f+K.getSecret()+z);
}};
S.setIconOptions(O);
S.setIconPath(t);
U.bind(a,S,l);
var P=new qx.ui.basic.Image();
this.getRoot().add(P,{left:30,top:200});
var T={converter:function(E){if(E){return (d+E.getFarm()+i+E.getServer()+j+E.getId()+f+E.getSecret()+D);
}return g;
}};
S.bind(A,P,B,T);
...
qx.Class.define(v,{extend:qx.data.store.Json,statics:{RESULT:null},construct:function(G){jsonFlickrApi=function(F){demobrowser.demo.data.store.Flickr.RESULT=F;
};
var H=h+G;
qx.data.store.Json.call(this,H);
},members:{searchForTag:function(I){if(I!=g){this.setUrl(h+I);
}},_createRequest:function(X){var Y=new qx.io.ScriptLoader();
this.setState(n);
X+=m;
Y.load(X,function(W){this.__MK();
},this);
},__MK:function(){this.setState("completed");
var L=demobrowser.demo.data.store.Flickr.RESULT;

if(L==undefined){this.setState("failed");
return;
}this._marshaler.toClass(L);
