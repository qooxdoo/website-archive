...
var w=document.getElementById(j).getElementsByTagName(d);

for(var i=0,l=w.length;i<l;i++){w[i].onclick=demobrowser.demo.bom.Location_StandardMode.jump;
...
},randomize:function(){var t=document.body.getElementsByTagName(c);

for(var i=0,l=t.length;i<l;i++){if(t[i].id&&t[i].id!=m){t[i].style.margin=Math.round(Math.random()*10)+n;
t[i].style.padding=Math.round(Math.random()*10)+n;
t[i].style.borderWidth=Math.round(Math.random()*10)+n;
}}}}});
})();
