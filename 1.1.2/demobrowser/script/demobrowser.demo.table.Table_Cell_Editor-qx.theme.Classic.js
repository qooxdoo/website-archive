...
},createTable:function(){var bC=[[K,l,y,{'required':true}],[e,L,T,{'type':e,'required':true}],[M,B,U,{'options':[V,O,H],'editable':true}],[R,w,b,{'options':[[o,null,b],[E,null,r],[x,null,F]]}],[f,A,z,{'type':f}],[m,P,N,{'regExp':X,'failMsg':ba}],[J,u,I,{'validationFunc':function(bN){if(bN.search(/^http:/)!=-1){window.open(bN);
}return bN;
}}],[W,n,true,{'type':k}]];
var bB=function(bm){var bs=bm.table;
var bp=bs.getTableModel();
var bq=bp.getRowData(bm.row);
var bt=bq[3];
var br;

for(var bo in bt){switch(bo){case g:switch(bt[c]){case k:return new qx.ui.table.cellrenderer.Boolean;
case h:return new qx.ui.table.cellrenderer.Password;
}break;
case i:var br=new qx.ui.table.cellrenderer.Replace;
var bn={};
bt[d].forEach(function(bD){if(bD instanceof Array){bn[bD[0]]=bD[2];
}});
br.setReplaceMap(bn);
br.addReversedReplaceMap();
return br;
}}return new qx.ui.table.cellrenderer.Default();
};
var bA=new qx.ui.table.cellrenderer.Dynamic(bB);
var by=function(bb){var bg=bb.table;
...
case s:bd.setValidationFunction(function(bG,bH){var bI=new RegExp(bi[Y]);

if(bI.test(bG)){return bG;
}alert(bi['failMsg']);
return bH;
});
break;
...
break;
case p:bh=function(bE,bF){if(!bE){alert("You need to supply a value here");
return bF;
}return bE;
};
break;
...
var bx=new qx.ui.table.celleditor.Dynamic(by);
var bv=new qx.ui.table.model.Simple();
bv.setColumns([j,j,q]);
var bu={tableColumnModel:function(a){return new qx.ui.table.columnmodel.Resize(a);
}};
var bw=new qx.ui.table.Table(bv,bu);
bw.setDecorator(null);
bw.setColumnVisibilityButtonVisible(false);
bw.setKeepFirstVisibleRowComplete(true);
bw.setStatusBarVisible(false);
bw.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var bz=bw.getTableColumnModel();
bz.setColumnVisible(0,false);
bz.getBehavior().setWidth(1,100);
bz.getBehavior().setWidth(2,300);
bv.setColumnEditable(2,true);
bz.setDataCellRenderer(2,bA);
bz.setCellEditorFactory(2,bx);
bw.getTableModel().setData(bC);
bw.getTableModel().addListener(v,function(event){if(!(event instanceof qx.event.type.Data)){return;
}var bM=event.getData();
var bJ=this.getTableModel();
var bL=bJ.getValue(0,bM.firstRow);
var bK=bJ.getValue(bM.firstColumn,bM.firstRow);
this.info("User edited property '"+bL+"' and entered value '"+bK+"'.");
},bw);
return bw;
}}});
})();
...
}},properties:{cellEditorFactoryFunction:{check:b,nullable:true,init:null}},members:{__GN:null,createCellEditor:function(d){var e=this.getCellEditorFactoryFunction();
{};
this.__GN=e(d);
var f=this.__GN.createCellEditor(d);
return f;
},getCellEditorValue:function(g){var h=this.getCellEditorFactoryFunction();
{};
var i=this.__GN.getCellEditorValue(g);
return i;
}},destruct:function(){this.__GN=null;
}});
})();
