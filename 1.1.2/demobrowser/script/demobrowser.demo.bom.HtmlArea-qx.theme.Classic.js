...
return cA;
},__Nm:function(){var cG=qx.bom.Element.create(cb);
qx.bom.element.Style.setStyles(cG,{"marginRight":bT,"float":bw});
var cI;
var cH=qx.bom.Element.create(cj,{title:x});

if(qx.core.Variant.isSet(bU,bY)){qx.bom.element.Attribute.set(cH,bX,bV);
}else{qx.bom.element.Style.set(cH,ca,bW);
}cI=qx.bom.Collection.html(bu);
qx.dom.Element.insertEnd(cI[0],cH);
var cJ;
var cF;

for(var i=1;i<=7;i++){cF=ba+br+i+bs+i+bp;
cJ=qx.bom.Collection.html(cF);

if(qx.core.Variant.isSet(bU,bY)){qx.bom.element.Attribute.set(cJ[0],bX,bV);
}else{qx.bom.element.Style.set(cJ[0],ca,bW);
}qx.dom.Element.insertEnd(cJ[0],cH);
}qx.event.Registration.addListener(cH,bn,function(e){var cW=e.getTarget();
var cX=cW.selectedIndex;

if(cX!=0){this.setFontSize(cW.options[cX].value);
cW.options[0].selected=true;
}},this.__Nc);
qx.dom.Element.insertBegin(cH,cG);
return cG;
},__Nn:function(){var cL=[{bold:{text:q,image:G,action:this.__Nc.setBold},italic:{text:bP,image:bA,action:this.__Nc.setItalic},underline:{text:bC,image:bO,action:this.__Nc.setUnderline},strikethrough:{text:bD,image:s,action:this.__Nc.setStrikeThrough},removeFormat:{text:S,image:J,action:this.__Nc.removeFormat}},{alignLeft:{text:n,image:B,action:this.__Nc.setJustifyLeft},alignCenter:{text:bi,image:cq,action:this.__Nc.setJustifyCenter},alignRight:{text:bM,image:H,action:this.__Nc.setJustifyRight},alignJustify:{text:co,image:by,action:this.__Nc.setJustifyFull}},{fontFamily:{custom:this.__Nl},fontSize:{custom:this.__Nm},fontColor:{text:Q,image:R,action:this.__Nf},textBackgroundColor:{text:cw,image:bN,action:this.__Ng}},{indent:{text:bL,image:c,action:this.__Nc.insertIndent},outdent:{text:bE,image:bf,action:this.__Nc.insertOutdent}},{insertImage:{text:be,image:U,action:this.__Nh},insertTable:{text:cf,image:P,action:this.__Ni},insertLink:{text:cv,image:E,action:this.__Nj},insertHTML:{text:F,image:cn,action:this.__Nk},insertHR:{text:cu,image:ct,action:this.__Nc.insertHorizontalRuler}},{ol:{text:D,image:b,action:this.__Nc.insertOrderedList},ul:{text:bG,image:cs,action:this.__Nc.insertUnorderedList}},{undo:{text:bm,image:I,action:this.__Nc.undo},redo:{text:r,image:bj,action:this.__Nc.redo}}];
var cN,cR,cK,cP;

for(var i=0,j=cL.length;i<j;i++){cR={};
cR[o]=i==j-1?cc:bT;
cR[h]=bw;
cN=qx.bom.Element.create(cb);
...
cP={type:W,src:qx.util.ResourceManager.getInstance().toUri(cM),title:cQ.text,qxKeepFocus:M,qxSelectable:t};
cK=qx.bom.Element.create(cp,cP);
qx.bom.element.Style.setStyles(cK,{width:bT,height:bT,padding:y});

if(qx.core.Variant.isSet(bU,bY)){qx.bom.element.Attribute.set(cK,bX,bV);
}else{qx.bom.element.Style.set(cK,ca,bW);
}qx.event.Registration.addListener(cK,Y,cQ.action,this.__Nc);
}qx.dom.Element.insertEnd(cK,cN);
}qx.dom.Element.insertEnd(cN,this.__Na);
}qx.dom.Element.insertEnd(qx.bom.Element.create(p,{style:ci}),this.__Na);
}}});
})();
