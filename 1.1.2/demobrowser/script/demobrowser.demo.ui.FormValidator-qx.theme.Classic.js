...
var C=new qx.ui.form.CheckBox(d);
C.setRequired(true);
this.getRoot().add(C,{left:20,top:190});
var z=new qx.ui.form.validation.Manager();
var x=function(p,q){var r=p!=null&&p.length>2;

if(!r){q.setInvalidMessage(k);
}return r;
};
var A=new qx.ui.form.validation.AsyncValidator(function(s,t){window.setTimeout(function(){if(t==null||t.length==0){s.setValid(false,i);
}else{s.setValid(true);
}},1000);
});
z.add(u,A);
...
z.add(w,x);
z.add(C);
z.setValidator(function(E){var G=v.getValue()==w.getValue();

if(!G){var F=h;
v.setInvalidMessage(F);
w.setInvalidMessage(F);
v.setValid(false);
w.setValid(false);
}return G;
});
...
y.addListener(b,function(){y.setEnabled(false);
y.setLabel(f);
z.validate();
},this);
z.addListener(l,function(){y.setEnabled(true);
y.setLabel(n);
