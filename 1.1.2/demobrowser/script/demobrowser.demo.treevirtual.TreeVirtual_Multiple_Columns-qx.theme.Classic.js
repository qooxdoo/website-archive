...
var U=new qx.ui.treevirtual.TreeVirtual([m,g,I]);
U.set({width:400});
U.setAlwaysShowOpenCloseSymbol(true);
var T=U.getTableColumnModel().getBehavior();
T.set(0,{width:L,minWidth:180});
bb.add(U);
var ba=U.getDataModel();
var W=ba.addBranch(null,b,true);
U.nodeSetLabelStyle(W,u+O+N);
var S;
ba.addBranch(W,F,true);
S=ba.addBranch(W,r,true);
var Y=ba.addLeaf(S,d);
ba.setColumnData(Y,1,h);
ba.setColumnData(Y,2,k);
Y=ba.addLeaf(S,f);
ba.setColumnData(Y,1,h);
ba.setColumnData(Y,2,k);
ba.addBranch(W,K,true);
Y=ba.addBranch(W,j,true);
U.nodeSetCellStyle(Y,v);
var X=ba.addBranch(null,c,true);
Y=ba.addBranch(X,n,false);

for(var i=1;i<3000;i++){ba.addLeaf(Y,C+i);
}ba.addBranch(X,G,false);
ba.addBranch(X,j,false);
ba.addBranch(X,w,false);
ba.addBranch(X,E,false);
ba.setData();
var V=new qx.ui.groupbox.GroupBox(J);
...
U.addListener(H,function(e){var Q=e.getData();
this.setValue(U.getHierarchy(Q[0].nodeId).join(B));
R.setEnabled(true);
},o);
var R=new qx.ui.form.Button(p);
R.set({enabled:false});
V.add(R,{top:50,left:0});
R.addListener(a,function(e){var bc=U.getSelectedNodes();

for(var i=0;i<bc.length;i++){ba.prune(bc[i].nodeId,true);
ba.setData();
}});
o=new qx.ui.form.CheckBox(x);
o.set({value:true});
V.add(o,{top:80,left:0});
o.addListener(l,function(e){U.setUseTreeLines(e.getData());
});
o=new qx.ui.form.CheckBox(D);
o.set({value:false});
V.add(o,{top:100,left:0});
o.addListener(l,function(e){U.setExcludeFirstLevelTreeLines(e.getData());
});
o=new qx.ui.form.CheckBox(z);
o.set({value:true});
V.add(o,{top:120,left:0});
o.addListener(l,function(e){U.setAlwaysShowOpenCloseSymbol(e.getData());
});
o=new qx.ui.form.CheckBox(A);
o.set({value:true});
V.add(o,{top:140,left:0});
U.addListener(q,function(e){if(this.getValue()){var P=e.getData();
U.nodeSetHideOpenClose(P,true);
}},o);
o=new qx.ui.form.CheckBox(s);
o.set({value:false});
V.add(o,{top:160,left:0});
o.addListener(l,function(e){U.setOpenCloseClickSelectsRow(e.getData());
});
o=new qx.ui.form.CheckBox(M);
o.set({value:false});
V.add(o,{top:180,left:0});
o.addListener(l,function(e){U.setEnabled(!e.getData());
});
}}});
})();
