...
bM.addListener(l,function(e){if(e.getData()){var bT=bO.getSelectionMode();

if(bT==m||bT==j){this.debug("Quick selection is only available for the modes multi or additive");
}}bO.setQuickSelection(e.getData());
});
bF.addListener(T,function(e){var bR=e.getData()[0].getUserData(o);
bO.setSelectionMode(bR);

if(bR==k||bR==n){bJ.setEnabled(false);
bM.setEnabled(true);
}else if(bR==m||bR==q){bJ.setEnabled(true);
bM.setEnabled(false);
}});
...
(function(){var o="selected",n="qx.event.type.Data",m="Boolean",k="changeSelection",h="row",g="single",f="changeHeight",d=",",c="one",b="_applySpacing",D="Integer",C="_applyDragSelection",B="String",A="resize",z="demobrowser.demo.virtual.list.List",y="list",x="multi",w="_applyQuickSelection",v="_applyItemHeight",u="_applySelectionMode",s="additive",t="changeName",q="Number",r="addItem",p="removeItem";
qx.Class.define(z,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider],construct:function(){qx.ui.virtual.core.Scroller.call(this,0,1,this.getItemHeight(),10);
this.__Mn=new qx.ui.virtual.layer.WidgetCell(this);
this.getPane().addLayer(this.__Mn);
this.getPane().addListener(A,this._onResize,this);
this.__Mo=[];
this.__Mp=[];
this.__Mq=new qx.ui.virtual.selection.Row(this.getPane());
this.__Mq.addListener(k,this._onChangeSelection,this);
this.__Mq.attachMouseEvents();
this.__Mq.attachKeyEvents(this);
this.__Mq.attachListEvents(this);
new qx.ui.virtual.behavior.Prefetch(this,0,0,0,0,400,600,400,600).set({interval:500});
},events:{addItem:n,removeItem:n,changeValue:n,changeSelection:n},properties:{appearance:{refine:true,init:y},focusable:{refine:true,init:true},spacing:{check:D,init:0,apply:b,themeable:true},enableInlineFind:{check:m,init:true},name:{check:B,nullable:true,event:t},itemHeight:{init:24,themeable:true,check:q,apply:v},selectionMode:{check:[g,x,s,c],init:g,apply:u},dragSelection:{check:m,init:false,apply:C},quickSelection:{check:m,init:false,apply:w}},members:{__Mo:null,__Mp:null,__Mq:null,__Mn:null,syncWidget:function(){this.update();
},update:function(){var P=this.getPane().getRowConfig();
P.setItemCount(this.__Mo.length);
P.resetItemSizes();

for(var i=0;i<this.__Mo.length;i++){var O=this.__Mo[i].getHeight();

if(O!==null){P.setItemSize(i,O);
}}},updateSelection:function(){var V=this.__Mn.getChildren();

for(var i=0;i<V.length;i++){var W=V[i];
var X=W.getUserData(h);

if(this.__Mq.isItemSelected(X)){W.addState(o);
}else{W.removeState(o);
...
this.fireDataEvent(k,this.__Mu(e.getData()));
},getCellWidget:function(H,I){var K=this.__Mo[H];

if(!K){return null;
}var J=this.__Mp.pop()||new qx.ui.form.ListItem();
J.set({label:K.getLabel(),icon:K.getIcon()});

if(this.__Mq.isItemSelected(H)){J.addState(o);
}else{J.removeState(o);
}J.setUserData(h,H);
return J;
},poolCellWidget:function(bh){this.__Mp.push(bh);
},getValue:function(){var Q=this.getSelection();
var R=[];
var S;

for(var i=0,l=Q.length;i<l;i++){S=Q[i].getValue();
if(S==null){S=Q[i].getLabel();
}R.push(S);
}return R.join(d);
},setValue:function(by){var bB=by.split(d);
var bA=[];
var bz;

for(var i=0,l=bB.length;i<l;i++){bz=this.findItem(bB[i]);

if(bz){bA.push(bz);
}}this.setSelection(bA);
},getChildren:function(){return this.__Mo;
},hasChildren:function(){return this.__Mo.length>0;
},_addHelper:function(F){this.fireDataEvent(r,F);
F.addListener(f,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},_removeHelper:function(bi){this.fireDataEvent(p,bi);
bi.removeListener(f,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},add:function(bg){this.__Mo.push(bg);
this._addHelper(bg);
return this;
},remove:function(a){qx.lang.Array.remove(this.__Mo,a);
this._removeHelper(a);
return this;
},removeAll:function(){for(var i=0,j=this.__Mo.length;i<j;i++){this._removeHelper(this.__Mo[i]);
}this.__Mo=[];
},indexOf:function(bd){return this.__Mo.indexOf(bd);
},addAt:function(bm,bn){qx.lang.Array.insertAt(this.__Mo,bm,bn);
this._addHelper(bm);
},addBefore:function(T,U){qx.lang.Array.insertBefore(this.__Mo,T,U);
this._addHelper(T);
},addAfter:function(bu,bv){qx.lang.Array.insertAfter(this.__Mo,bu,bv);
this._addHelper(bu);
},removeAt:function(ba){this.__Mo.splice(ba,1);
this._removeHelper(this.__Mo[ba]);
},__Mr:function(bj){var bk=this.__Mo.indexOf(bj);

if(bk<0){return null;
}else{return bk;
}},__Ms:function(bs){var bt=this.__Mo[bs];
return bt;
},__Mt:function(be){var bf=[];

for(var i=0;i<be.length;i++){bf.push(this.__Mr(be[i]));
}return bf;
},__Mu:function(bq){var br=[];

for(var i=0;i<bq.length;i++){br.push(this.__Mo[bq[i]]);
}return br;
},selectAll:function(){this.__Mq.selectAll();
},setSelected:function(E){this.__Mq.selectItem(this.__Mr(E));
},isSelected:function(Y){return this.__Mq.isItemSelected(this.__Mr(Y));
},addToSelection:function(N){this.__Mq.addItem(this.__Mr(N));
},removeFromSelection:function(G){this.__Mq.removeItem(this.__Mr(G));
},selectRange:function(bb,bc){this.__Mq.selectItemRange(this.__Mr(bb),this.__Mr(bc));
},resetSelection:function(){this.__Mq.clearSelection();
},setSelection:function(bl){this.__Mq.replaceSelection(this.__Mt(bl));
},getSelectedItem:function(){return this.__Mo[this.getSelected()];
},getSelected:function(){return this.__Mo[this.__Mq.getSelectedItem()];
},getSelection:function(){return this.__Mu(this.__Mq.getSelection());
},getSortedSelection:function(){return this.__Mu(this.__Mq.getSortedSelection());
},isSelectionEmpty:function(){return this.__Mq.isSelectionEmpty();
},getSelectionContext:function(){return this.__Mq.getSelectionContext();
},getSelectables:function(){return this.__Mu(this.__Mq.getSelectables());
},_applySelectionMode:function(L,M){this.__Mq.setMode(L);
},_applyDragSelection:function(bo,bp){this.__Mq.setDrag(bo);
},_applyQuickSelection:function(bw,bx){this.__Mq.setQuick(bw);
}}});
})();
...
qx.Class.define(a,{extend:qx.core.Object,construct:function(m,n,o){qx.core.Object.call(this);
...
if(j){j.add(this);
}},getFormValue:function(){var l=this.getValue();
