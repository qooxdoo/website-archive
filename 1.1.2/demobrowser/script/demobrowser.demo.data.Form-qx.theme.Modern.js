...
var F=new qx.ui.groupbox.GroupBox(o);
F.setWidth(200);
this.getRoot().add(F,{left:10,top:10});
var C=new qx.ui.layout.Grid();
C.setSpacing(5);
C.setColumnAlign(0,f,a);
F.setLayout(C);
var s=new qx.ui.basic.Label(b);
F.add(s,{row:0,column:0});
var v=new qx.ui.form.TextField();
F.add(v,{row:0,column:1});
var I=new qx.ui.basic.Label(d);
F.add(I,{row:1,column:0});
var D=new qx.ui.form.SelectBox();
var E=new qx.ui.form.ListItem(i);
D.add(E);
var A=new qx.ui.form.ListItem(n);
D.add(A);
var y=new qx.ui.form.ListItem(p);
D.add(y);
F.add(D,{row:1,column:1});
var K=new qx.ui.basic.Label(k);
F.add(K,{row:2,column:0});
var H=new qx.ui.form.CheckBox();
F.add(H,{row:2,column:1});
var z=new qx.ui.form.Button(m);
F.add(z,{row:3,column:0});
var u={name:null,gender:null,ok:0,hidden:e};
var G=qx.data.marshal.Json.createModel(u);
var x=new qx.data.controller.Object(G);
x.addTarget(v,r,g,true);
x.addTarget(D,j,l,true);
var t={converter:function(P){return P===1;
}};
var w={converter:function(O){return O?1:0;
}};
x.addTarget(H,r,h,true,t,w);
v.setRequired(true);
H.setRequired(true);
var J=new qx.ui.form.validation.Manager();
J.add(v,qx.util.Validate.required);
J.add(H);
J.setValidator(function(M){var N=D.getSelection()[0]!==E;
D.setValid(N);
return N;
});
var B=function(L){if(L instanceof qx.ui.form.ListItem){return L.getLabel();
}};
z.addListener(c,function(){if(J.validate()){alert("You are sending: "+qx.util.Serializer.toUriParameter(G,B));
}},this);
}}});
})();
