...
qx.Class.define(l,{extend:qx.ui.form.renderer.Single,members:{__Np:0,__Nq:0,addItems:function(s,t,u){var x=0;
if(u!=null){this._add(this._createHeader(u),{row:x,column:this.__Np,colSpan:2});
x++;
}for(var i=0;i<s.length;i++){var w=this._createLabel(t[i],s[i]);
this._add(w,{row:x,column:this.__Np});
var v=s[i];
w.setBuddy(v);

if(v instanceof qx.ui.form.RadioGroup){v=this._createWidgetForRadioGroup(v);
}this._add(v,{row:x,column:this.__Np+1});
x++;
}this.__Np+=2;
this.__Nq<x?this.__Nq=x:null;
...
this._add(this._buttonRow,{row:this.__Nq,column:0,colSpan:this.__Np});
