...
bb.forEach(function(bH){bH.setMode(bG);
});
...
bm.addListener(E,function(e){if(e.getData()){var bz=bb[0].getMode();

if(bz==F||bz==G){this.debug("Drag selection is only available for the modes multi or additive");
}}bb.forEach(function(bx){bx.setDrag(e.getData());
});
});
var bk=new qx.ui.form.CheckBox(u).set({enabled:false});
bg.add(bk,{row:2,column:1});
bk.addListener(E,function(e){if(e.getData()){var a=bb[0].getMode();

if(a==k||a==C){this.debug("Quick selection is only available for the modes multi or additive");
}}bb.forEach(function(by){by.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bM=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var bK=new qx.ui.virtual.layer.Row(g,d);
bM.getPane().addLayer(bK);
bM.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var bL=new qx.ui.virtual.cell.Cell();
var bJ=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bu,bv){var bw={};

if(bI.isItemSelected(bu)){bw.selected=true;
}return bL.getCellProperties(bu+j+bv,bw);
}});
bM.getPane().addLayer(bJ);
var bI=new qx.ui.virtual.selection.Row(bM.getPane(),{styleSelectable:function(bN,bO,bP){if(bO!==f){return;
}
if(bP){bK.setBackground(bN,f);
}else{bK.setBackground(bN,null);
}bJ.updateLayerData();
}});
bI.attachMouseEvents(bM.getPane());
bI.attachKeyEvents(bM);
bI.set({mode:k});
bI.addItem(0);
bM.setUserData(l,bI);
return bM;
},createColumnSelectionScroller:function(){var K=new qx.ui.virtual.core.Scroller(1000,100,20,100);
K.getPane().addLayer(new qx.ui.virtual.layer.Row(g,d));
var J=new qx.ui.virtual.layer.Column();
K.getPane().addLayer(J);
K.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var L=new qx.ui.virtual.cell.Cell();
var I=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bo,bp){var bq={};

if(H.isItemSelected(bp)){bq.selected=true;
}return L.getCellProperties(bo+j+bp,bq);
}});
K.getPane().addLayer(I);
var H=new qx.ui.virtual.selection.Column(K.getPane(),{styleSelectable:function(W,X,Y){if(X!==f){return;
}
if(Y){J.setBackground(W,f);
}else{J.setBackground(W,null);
}I.updateLayerData();
}});
...
K.setUserData(l,H);
return K;
},createCellRectangleSelectionScroller:function(){var U=new qx.ui.virtual.core.Scroller(1000,100,20,100);
U.getPane().addLayer(new qx.ui.virtual.layer.Row(g,d));
var T=new qx.ui.virtual.cell.Cell();
var V=new qx.ui.virtual.cell.Cell().set({backgroundColor:D});
var S=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bQ,bR){var bS={};

if(R.isItemSelected({row:bQ,column:bR})){bS.selected=true;
}
if(bS.selected){return V.getCellProperties(bQ+j+bR,bS);
}else{T.resetBackgroundColor();
return T.getCellProperties(bQ+j+bR,bS);
}}});
U.getPane().addLayer(S);
U.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var R=new qx.ui.virtual.selection.CellRectangle(U.getPane(),{styleSelectable:function(bA,bB,bC){S.updateLayerData();
}});
R.attachMouseEvents(U.getPane());
R.attachKeyEvents(U);
R.set({mode:k});
R.addItem({row:0,column:0});
U.setUserData(l,R);
return U;
},createCellLineSelectionScroller:function(){var P=new qx.ui.virtual.core.Scroller(1000,100,20,100);
P.getPane().addLayer(new qx.ui.virtual.layer.Row(g,d));
var O=new qx.ui.virtual.cell.Cell();
var Q=new qx.ui.virtual.cell.Cell().set({backgroundColor:D});
var N=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bD,bE){var bF={};

if(M.isItemSelected({row:bD,column:bE})){bF.selected=true;
}
if(bF.selected){return Q.getCellProperties(bD+j+bE,bF);
}else{O.resetBackgroundColor();
return O.getCellProperties(bD+j+bE,bF);
}}});
P.getPane().addLayer(N);
P.getPane().addLayer(new qx.ui.virtual.layer.GridLines(c));
var M=new qx.ui.virtual.selection.CellLines(P.getPane(),{styleSelectable:function(br,bs,bt){N.updateLayerData();
}});
M.attachMouseEvents(P.getPane());
M.attachKeyEvents(P);
M.set({mode:k});
M.addItem({row:0,column:0});
P.setUserData(l,M);
return P;
}}});
})();
...
},_getSelectableLocationY:function(d){return {top:0,bottom:this._pane.getRowConfig().getTotalSize()-1};
}}});
})();
