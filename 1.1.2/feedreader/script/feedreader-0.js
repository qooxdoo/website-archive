...
qx.Class.define(f,{extend:qx.core.Object,type:a,members:{loadAll:function(q){var s=q.getFeeds().getItem(0).getFeeds();

for(var i=0;i<s.length;i++){this.load(s.getItem(i));
}var r=q.getFeeds().getItem(1).getFeeds();

for(i=0;i<r.length;i++){this.load(r.getItem(i));
}},load:function(n){n.setState(d);
var o=j+n.getUrl()+h;
var p=new qx.data.store.Yql(o,{manipulateData:function(w){if(w.query.results){return w.query.results.item||w.query.results.entry;
}return m;
}},qx.bom.client.Feature.SSL);
p.addListener(k,this.__lJ(n),this);
p.addListener(c,qx.lang.Function.bind(this.__lI,this,n),this);
},__lI:function(t,e){if(e.getData()==b||e.getData()==g||e.getData()==m){t.setState(l);
}},__lJ:function(u){return function(e){var v=e.getData();
if(v==m){u.setState(l);
return;
}u.setArticles(v);
u.setState(k);
...
qx.Class.define(b,{extend:qx.data.store.Json,construct:function(o,p,q){if(q!=undefined){this.setCallbackParam(q);
}qx.data.store.Json.call(this,o,p);
},properties:{callbackParam:{check:a,nullable:false}},members:{__mh:null,_createRequest:function(j){if(this.__mh){this.__mh.dispose();
}this.__mh=new qx.io.ScriptLoader();
var l=this._delegate;

if(l&&qx.lang.Type.isFunction(l.configureRequest)){this._delegate.configureRequest(this.__mh);
}var k=j.indexOf(i)==-1?i:h;
j+=k+this.getCallbackParam()+c;
var m=parseInt(this.toHashCode());
qx.data.store.Jsonp[m]=this;
j+=f+m+e;
this.__mh.load(j,function(n){delete this[m];
},this);
},callback:function(t){if(this.isDisposed()){return;
}this.__mi(t);
},__mi:function(r){if(r==undefined){this.setState(d);
return;
}var s=this._delegate;

if(s&&qx.lang.Type.isFunction(s.manipulateData)){r=this._delegate.manipulateData(r);
}this._marshaler.toClass(r);
this.setModel(this._marshaler.toModel(r));
this.fireDataEvent(g,this.getModel());
}},destruct:function(){if(this.__mh){this.__mh.dispose();
}this.__mh=null;
}});
})();
...
(function(){var r="static",q="user",p="execute",o="loading",n="title",m="north",l="main",k="new",j="Surfin' Safari",h="changeModel",bz="__ov",by="__oy",bx="selection[0].state",bw="Heise",bv="F1",bu="article",bt="partLoaded",bs="__ot",br="Control+A",bq="__oB",y="http://blogs.msdn.com/ie/rss.xml",z="http://my.opera.com/desktopteam/xml/rss/blog/",w="settings",x="feedreader/images/loading22.gif",u="error",v="selection[0]",s="change",t="http://blogs.msdn.com/jscript/rss.xml",G="__oC",H="__oA",T="loaded",P="__oE",bc="feeds",W="__oG",bm="The Mozilla Blog",bh="enabled",L="http://webkit.org/blog/feed/",bp="separator-vertical",bo="http://feeds2.feedburner.com/qooxdoo/news/content",bn="http://blog.mozilla.com/feed/",K="vertical",N="__ox",O="http://www.heise.de/newsticker/heise-atom.xml",R="__oz",U="state",X="JScript Team Blog",be="Feeds",bj="http://feeds2.feedburner.com/ajaxian",A="Control+D",B="icon/22/actions/process-stop.png",M="selection[0].articles",bb="white",ba="IEBlog",Y="Ajaxian",bg="User Feeds",bf="icon/22/apps/internet-feed-reader.png",V="model",bd="feedreader.Application",d="http://daringfireball.net/index.xml",bi="Control+P",C="Static Feeds",D="__ow",Q="Control+R",f="qooxdoo News",g="selection[0].category",J="__oD",E="Opera Desktop Blog",F="feedreader.model.Feed",I="Daring Fireball",S="__os",bl="__oF",bk="addfeed";
qx.Class.define(bd,{extend:qx.application.Standalone,members:{__os:null,__ot:null,__ou:null,__ov:null,__ow:null,__ox:null,__oy:null,__oz:null,__oA:null,__oB:null,__oC:null,__oD:null,__oE:null,__oF:null,__oG:null,main:function(){qx.application.Standalone.prototype.main.call(this);
{};
...
this._setUpBinding();
this.__ov.getRoot().setOpen(true);
this.__ov.getRoot().getChildren()[0].setOpen(true);
this.__ov.getRoot().getChildren()[1].setOpen(true);
this.__ov.setHideRoot(true);
this.__os.getSelection().push(this.__oG.getFeeds().getItem(0));
},finalize:function(){qx.application.Standalone.prototype.finalize.call(this);
this.reload();
},_initializeModel:function(){this.__oE=new feedreader.model.FeedFolder(be);
this.__oG=new feedreader.model.FeedFolder(this.tr(C));
this.__oE.getFeeds().push(this.__oG);
this.__oG.getFeeds().push(new feedreader.model.Feed(f,bo,r));
this.__oG.getFeeds().push(new feedreader.model.Feed(X,t,r));
this.__oG.getFeeds().push(new feedreader.model.Feed(I,d,r));
this.__oG.getFeeds().push(new feedreader.model.Feed(j,L,r));
this.__oG.getFeeds().push(new feedreader.model.Feed(Y,bj,r));
this.__oF=new feedreader.model.FeedFolder(this.tr(bg));
this.__oE.getFeeds().push(this.__oF);
this.__oF.getFeeds().push(new feedreader.model.Feed(bw,O,q));
this.__oF.getFeeds().push(new feedreader.model.Feed(ba,y,q));
this.__oF.getFeeds().push(new feedreader.model.Feed(bm,bn,q));
this.__oF.getFeeds().push(new feedreader.model.Feed(E,z,q));
},_setUpBinding:function(){var bL={converter:this._state2iconConverter};
this.__os=new qx.data.controller.Tree(this.__oE,this.__ov,bc,n);
this.__os.setIconOptions(bL);
this.__os.setIconPath(U);
this.__ot=new qx.data.controller.List(null,this.__oy.getList(),n);
this.__os.bind(M,this.__ot,V);
this.__ot.bind(v,this.__oC,bu);
this.__ot.getSelection().addListener(s,this._listControllerChange,this);
this.__ot.addListener(h,this._treeControllerChange,this);
var bM={converter:this._state2loadingConverter};
this.__os.bind(bx,this.__oy,o,bM);
bM={converter:this._category2enabledConverter};
this.__os.bind(g,this.__ox.getRemoveButton(),bh,bM);
},_category2enabledConverter:function(bP){if(bP==q){return true;
}return false;
},_state2iconConverter:function(bA){if(bA==k||bA==o){return x;
}else if(bA==T){return bf;
}else if(bA==u){return B;
}return null;
},_state2loadingConverter:function(bI){if(bI==k||bI==o){return true;
}return false;
},_listControllerChange:function(bQ){var bR=this.__os.getSelection().getItem(0);
var bS=this.__ot.getSelection().getItem(0);
if(bS!=undefined){bR.setSelectedArticle(bS);
}},_treeControllerChange:function(bG){var bH=this.__os.getSelection().getItem(0);
if(bH!=null&&bH.classname==F&&bH.getSelectedArticle()!=null&&bH.getArticles().contains(bH.getSelectedArticle())){this.__ot.getSelection().push(bH.getSelectedArticle());
}else{this.__oy.getList().scrollToY(0);
}},_createLayout:function(){var a=new qx.ui.layout.Dock();
a.setSeparatorY(bp);
var b=new qx.ui.container.Composite(a);
this.getRoot().add(b,{edge:0});
this.__ow=new feedreader.view.Header();
b.add(this.__ow,{edge:m});
this.__ox=new feedreader.view.ToolBar(this);
b.add(this.__ox,{edge:m});
this.__oz=new qx.ui.splitpane.Pane();
b.add(this.__oz);
this.__ov=new qx.ui.tree.Tree();
this.__ov.setWidth(250);
this.__ov.setBackgroundColor(bb);
this.__oz.add(this.__ov,0);
this.__oA=new qx.ui.splitpane.Pane(K);
this.__oA.setDecorator(null);
this.__oz.add(this.__oA,1);
this.__oy=new feedreader.view.List(this.__oE);
this.__oy.setHeight(200);
this.__oy.setDecorator(l);
this.__oA.add(this.__oy,0);
this.__oC=new feedreader.view.Article();
this.__oC.setDecorator(l);
this.__oA.add(this.__oC,1);
},_initializeCommands:function(){var bJ={};
bJ.reload=new qx.ui.core.Command(Q);
bJ.reload.addListener(p,this.reload,this);
bJ.about=new qx.ui.core.Command(bv);
bJ.about.addListener(p,this.showAbout,this);
bJ.preferences=new qx.ui.core.Command(bi);
bJ.preferences.addListener(p,this.showPreferences,this);
bJ.addFeed=new qx.ui.core.Command(br);
bJ.addFeed.addListener(p,this.showAddFeed,this);
bJ.removeFeed=new qx.ui.core.Command(A);
bJ.removeFeed.addListener(p,this.removeFeed,this);
this.__ou=bJ;
},getCommand:function(c){return this.__ou[c];
},addFeed:function(bB,bC,bD){var bE=new feedreader.model.Feed(bB,bC,bD);
this.__oF.getFeeds().push(bE);
var bF=feedreader.io.FeedLoader.getInstance();
bF.load(bE);
},removeFeed:function(){var bN=this.__os.getSelection().getItem(0);
if(bN&&bN.getCategory&&bN.getCategory()!==r){var bO=this.__oF.getFeeds();
for(var i=0;i<bO.length;i++){if(bN===bO.getItem(i)){bO.splice(i,1);
return;
}}}},reload:function(){var bK=feedreader.io.FeedLoader.getInstance();
bK.loadAll(this.__oE);
},showPreferences:function(){qx.io.PartLoader.require([w],function(){if(!this.__oB){this.__oB=new feedreader.view.PreferenceWindow();
this.getRoot().add(this.__oB);
}this.__oB.center();
this.__oB.open();
},this);
},showAbout:function(){alert(this.tr("FeedReader (qooxdoo powered)"));
},showAddFeed:function(){qx.io.PartLoader.require([bk],function(){if(!this.__oD){this.__oD=new feedreader.view.AddFeedWindow(this);
this.getRoot().add(this.__oD);
}this.__oD.center();
this.__oD.open();
},this);
}},destruct:function(){this.__ou=null;
this._disposeObjects(N,by,G,bz,P,R,H,D,W,bl,S,bs,bq,J);
}});
})();
...
C.add(D);
this.__pd=new qx.ui.toolbar.Button(this.tr(l),o);
this.__pd.setCommand(v.getCommand(s));
...
B.add(A);
},members:{__pd:null,getRemoveButton:function(){return this.__pd;
...
(function(){var p="separator-vertical",o="__pH",n="background-medium",m="Posts",l="bold",k="_applyLoading",j="__pG",i="center",h="feedreader.view.List",g="__pI",d="single",f="middle",e="feedreader/images/loading66.gif",c="Boolean";
qx.Class.define(h,{extend:qx.ui.core.Widget,construct:function(){qx.ui.core.Widget.call(this);
var r=new qx.ui.layout.VBox();
r.setSeparator(p);
this._setLayout(r);
var s=new qx.ui.basic.Label(this.tr(m));
s.setBackgroundColor(n);
s.setPadding(5);
s.setAllowGrowX(true);
s.setFont(l);
this._add(s);
this.__pG=new qx.ui.container.Stack();
this._add(this.__pG,{flex:1});
this.__pH=new qx.ui.form.List();
this.__pH.setDecorator(null);
this.__pH.setSelectionMode(d);
this.__pG.add(this.__pH);
this.__pI=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,i));
var q=new qx.ui.basic.Image(e);
q.setAlignY(f);
this.__pI.add(q);
this.__pG.add(this.__pI);
},properties:{loading:{check:c,init:false,apply:k}},members:{__pG:null,__pH:null,__pI:null,_applyLoading:function(a,b){if(a){this.__pG.setSelection([this.__pI]);
}else{this.__pG.setSelection([this.__pH]);
}},getList:function(){return this.__pH;
}},destruct:function(){this._disposeObjects(o,j,g);
}});
})();
...
if(!feedreader.view.Article.__qq){feedreader.view.Article.__qq=true;
...
w.setLineHeight(1.8);
this.setFont(w);
},properties:{article:{apply:e,nullable:true,check:d}},members:{_applyArticle:function(C,D){var E=this.__qr();
this.setHtml(E);
qx.html.Element.flush();
var G=this.getContentElement().getDomElement();
var F=G.getElementsByTagName(b);
for(var i=0;i<F.length;i++){F[i].target=t;
}},__qr:function(){var y=this.getArticle();
if(!y){return v;
